...
constexpr long double round_to_64bit_mantissa(long double value) noexcept {
  if (!std::isfinite(value) || value == 0.0L) return value;
...
struct common_type<wide::integer<Bits, Signed>, Arithmetic> {
  ...
  using type = std::conditional_t < std::is_floating_point_v<Arithmetic>, Arithmetic,
        std::conditional_t<
            sizeof(Arithmetic) * 8 < Bits, wide::integer<Bits, Signed>,
            std::conditional_t<
                Bits<sizeof(Arithmetic) * 8, Arithmetic,
                     std::conditional_t<Bits == sizeof(Arithmetic) * 8 && (std::is_same_v<Signed, signed> || std::is_signed_v<Arithmetic>),
...
struct integer<Bits, Signed>::_impl {
  ...
  constexpr static void wide_integer_from_builtin(integer<Bits, Signed> &self, Integral rhs) noexcept {
    ...
    if constexpr (std::is_signed_v<Integral>) {
      if (rhs < 0) {
        for (unsigned i = 1; i < item_count; ++i) self.items[little(i)] = -1;
        return;
      }
    }
  ...
  constexpr static void wide_integer_from_wide_integer(integer<Bits, Signed> &self, const integer<Bits2, Signed2> &rhs) noexcept {
    ...
    for (unsigned i = 0; i < to_copy; ++i) self.items[little(i)] = rhs.items[integer<Bits2, Signed2>::_impl::little(i)];

    if constexpr (Bits > Bits2) {
      if constexpr (std::is_signed_v<Signed2>) {
        if (rhs < 0) {
          for (unsigned i = to_copy; i < item_count; ++i) self.items[little(i)] = -1;
          return;
        }
      }

      for (unsigned i = to_copy; i < item_count; ++i) self.items[little(i)] = 0;
    }
  }
  ...
  constexpr static integer<Bits, Signed> shift_left(const integer<Bits, Signed> &rhs, unsigned n) noexcept {
    ...
    if (unsigned bit_shift = n % base_bits) {
      ...
      for (unsigned i = 0; i < item_count - items_shift; ++i) lhs.items[big(i)] = rhs.items[big(items_shift + i)];
    }

    for (unsigned i = 0; i < items_shift; ++i) lhs.items[little(i)] = 0;
    return lhs;
  }
  ...
  constexpr static integer<Bits, Signed> shift_right(const integer<Bits, Signed> &rhs, unsigned n) noexcept {
    ...
    if (bit_shift) {
      ...
      for (unsigned i = 0; i < item_count - items_shift; ++i) lhs.items[little(i)] = rhs.items[little(items_shift + i)];
    }

    if (is_negative(rhs)) {
      if (bit_shift) lhs.items[big(items_shift)] |= std::numeric_limits<base_type>::max() << (base_bits - bit_shift);

      for (unsigned i = 0; i < items_shift; ++i) lhs.items[big(i)] = std::numeric_limits<base_type>::max();
    } else {
      for (unsigned i = 0; i < items_shift; ++i) lhs.items[big(i)] = 0;
    }
 ...
 private:
  ...
  constexpr static base_type get_item(const T &x, unsigned idx) {
    if constexpr (IsWideInteger<T>::value) {
      if (idx < T::_impl::item_count) return x.items[idx];
      return 0;
    } else {
      if constexpr (sizeof(T) <= sizeof(base_type)) {
        if (little(0) == idx) return static_cast<base_type>(x);
  ...
  constexpr static integer<Bits, Signed> minus(const integer<Bits, Signed> &lhs, T rhs) {
    ...
    for (unsigned i = 1; i < item_count; ++i) {
      if (underflows[i - 1]) {
        base_type &res_item = res.items[little(i)];
        if (res_item == 0) underflows[i] = true;
        --res_item;
      }
    }
  ...
  constexpr static integer<Bits, Signed> plus(const integer<Bits, Signed> &lhs, T rhs) {
    ...
    for (unsigned i = 1; i < item_count; ++i) {
      if (overflows[i - 1]) {
        base_type &res_item = res.items[little(i)];
        ++res_item;
        if (res_item == 0) overflows[i] = true;
      }
    }
  ...
  constexpr static integer<Bits, Signed> multiply(const integer<Bits, Signed> &lhs, const T &rhs) {
    if constexpr (Bits == 256 && sizeof(base_type) == 8) {
      ...
      uint64_t b1 = 0;
      HalfType b23 = 0;
      if constexpr (sizeof(T) > 8) b1 = b01 >> 64;
      if constexpr (sizeof(T) > 16) b23 = (HalfType(rhs.items[little(3)]) << 64) + rhs.items[little(2)];
      ...
      if constexpr (sizeof(T) > 8) {
        ...
        r12_x += r12_y;
        if (r12_x < r12_y) ++res.items[little(3)];
      }

      r12 += r12_x;
      if (r12 < r12_x) ++res.items[little(3)];
...
#endif
          if (rhs_item & 1) res = plus(res, shift_left(lhs, pos));
 ...
 public:
  constexpr static integer<Bits, Signed> operator_unary_tilda(const integer<Bits, Signed> &lhs) noexcept {
    ...
    for (unsigned i = 0; i < item_count; ++i) res.items[any(i)] = ~lhs.items[any(i)];
    return res;
  }

  constexpr static integer<Bits, Signed> operator_unary_minus(const integer<Bits, Signed> &lhs) noexcept(std::is_same_v<Signed, unsigned>) {
  ...
  constexpr static auto operator_star(const integer<Bits, Signed> &lhs, const T &rhs) {
    if constexpr (should_keep_size<T>()) {
      if constexpr (use_BitInt256) {
        ...
        if (std::is_same_v<Signed, signed> && is_negative(lhs) != is_negative(rhs)) res = operator_unary_minus(res);
  ...
  constexpr static bool operator_greater(const integer<Bits, Signed> &lhs, const T &rhs) noexcept {
    if constexpr (should_keep_size<T>()) {
      if (std::numeric_limits<T>::is_signed && (is_negative(lhs) != is_negative(rhs))) return is_negative(rhs);

      integer<Bits, Signed> t = rhs;
      for (unsigned i = 0; i < item_count; ++i) {
        base_type rhs_item = get_item(t, big(i));

        if (lhs.items[big(i)] != rhs_item) return lhs.items[big(i)] > rhs_item;
      }
  ...
  constexpr static bool operator_less(const integer<Bits, Signed> &lhs, const T &rhs) noexcept {
    if constexpr (should_keep_size<T>()) {
      if (std::numeric_limits<T>::is_signed && (is_negative(lhs) != is_negative(rhs))) return is_negative(lhs);
  ...
  constexpr static bool operator_eq(const integer<Bits, Signed> &lhs, const T &rhs) noexcept {
    if constexpr (should_keep_size<T>()) {
      ...
      for (unsigned i = 0; i < item_count; ++i) {
        ...
        if (lhs.items[any(i)] != rhs_item) return false;
      }
  ...
  constexpr static auto operator_pipe(const integer<Bits, Signed> &lhs, const T &rhs) noexcept {
    if constexpr (should_keep_size<T>()) {
      ...
      for (unsigned i = 0; i < item_count; ++i) res.items[little(i)] = lhs.items[little(i)] | get_item(rhs, little(i));
      return res;
    } else {
      static_assert(IsWideInteger<T>::value);
  ...
  constexpr static integer<Bits2, unsigned> divide(integer<Bits2, unsigned> &numerator, integer<Bits2, unsigned> denominator) {
    ...
    if (is_zero(denominator)) throwError("Division by zero");
  ...
  constexpr static auto operator_slash(const integer<Bits, Signed> &lhs, const T &rhs) {
    if constexpr (should_keep_size<T>()) {
      if constexpr (use_BitInt256) {
        ...
        if (std::is_same_v<Signed, signed> && is_negative(rhs) != is_negative(lhs)) quotient = operator_unary_minus(quotient);
        return quotient;
      }
    } else {
      static_assert(IsWideInteger<T>::value);
  ...
  constexpr static auto operator_percent(const integer<Bits, Signed> &lhs, const T &rhs) {
    if constexpr (should_keep_size<T>()) {
      if constexpr (use_BitInt256) {
        ...
        if (std::is_same_v<Signed, signed> && is_negative(lhs)) remainder = operator_unary_minus(remainder);
        return remainder;
      }
    } else {
      static_assert(IsWideInteger<T>::value);
  ...
  constexpr static auto operator_circumflex(const integer<Bits, Signed> &lhs, const T &rhs) noexcept {
    if constexpr (should_keep_size<T>()) {
      ...
      for (unsigned i = 0; i < item_count; ++i) res.items[any(i)] ^= t.items[any(i)];
      return res;
    } else {
      static_assert(IsWideInteger<T>::value);
  ...
  constexpr static integer<Bits, Signed> from_str(const char *c) {
    ...
    if (*c == '0' && (*(c + 1) == 'x' || *(c + 1) == 'X')) {  // hex
      ...
      while (*c) {
        if (*c < '0' || *c > '9') throwError("Invalid char from");
    ...
    if (is_neg) res = operator_unary_minus(res);
...
constexpr integer<Bits, Signed>::operator long double() const noexcept {
  if (_impl::operator_eq(*this, 0)) return 0;

  integer<Bits, Signed> tmp = *this;
  if (_impl::is_negative(*this)) tmp = -tmp;
  ...
  if (_impl::is_negative(*this)) res = -res;
...
// Binary operators
template <size_t Bits, typename Signed, size_t Bits2, typename Signed2>
std::common_type_t<integer<Bits, Signed>, integer<Bits2, Signed2>> constexpr operator*(const integer<Bits, Signed> &lhs,
...
constexpr integer<Bits, Signed> operator<<(const integer<Bits, Signed> &lhs, int n) noexcept {
  if (static_cast<size_t>(n) >= Bits) return integer<Bits, Signed>(0);
  if (n <= 0) return lhs;
...
struct hash<wide::integer<Bits, Signed>> {
  std::size_t operator()(const wide::integer<Bits, Signed> &lhs) const {
    ...
    size_t res = 0;
    for (unsigned i = 0; i < count; ++i) res ^= ptr[i];
    return res;
  }
};
