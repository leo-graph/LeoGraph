...
POCO_IMPLEMENT_EXCEPTION(JSONException, Poco::Exception,
                         "JSONException")  // NOLINT(cert-err60-cpp, modernize-use-noexcept, hicpp-use-noexcept)
...
static UInt64 readUIntText(const char *buf, const char *end) {
  ...
  if (buf == end) throw JSONException("JSON: cannot parse unsigned integer: unexpected end of data.");
...
static Int64 readIntText(const char *buf, const char *end) {
  ...
  if (buf == end) throw JSONException("JSON: cannot parse signed integer: unexpected end of data.");
...
static double readFloatText(const char *buf, const char *end) {
  ...
  if (buf == end) throw JSONException("JSON: cannot parse floating point number: unexpected end of data.");
...
void JSON::checkInit() const {
  if (!(ptr_begin < ptr_end)) throw JSONException("JSON: begin >= end.");

  if (level > JSON_MAX_DEPTH) throw JSONException("JSON: too deep.");
}
...
JSON::ElementType JSON::getType() const {
  switch (*ptr_begin) {
    ...
    case '"': {
      ...
      if (after_string < ptr_end && *after_string == ':') return TYPE_NAME_VALUE_PAIR;
      return TYPE_STRING;
    }
    default:
...
void JSON::checkPos(Pos pos) const {
  if (pos >= ptr_end || ptr_begin == nullptr) throw JSONException("JSON: unexpected end of data.");
}

JSON::Pos JSON::skipString() const {
  Pos pos = ptr_begin;
  checkPos(pos);
  if (*pos != '"') throw JSONException(std::string("JSON: expected \", got ") + *pos);
  ++pos;
  ...
  if (nullptr != closing_quote && closing_quote[-1] != '\\') return closing_quote + 1;
...
JSON::Pos JSON::skipNumber() const {
  ...
  while (pos < ptr_end && *pos >= '0' && *pos <= '9') ++pos;
  if (pos < ptr_end && *pos == '.') ++pos;
  while (pos < ptr_end && *pos >= '0' && *pos <= '9') ++pos;
  if (pos < ptr_end && (*pos == 'e' || *pos == 'E')) ++pos;
  if (pos < ptr_end && *pos == '-') ++pos;
  while (pos < ptr_end && *pos >= '0' && *pos <= '9') ++pos;
...
JSON::Pos JSON::skipNull() const { return ptr_begin + 4; }

JSON::Pos JSON::skipNameValuePair() const {
  Pos pos = skipString();
  checkPos(pos);

  if (*pos != ':') throw JSONException("JSON: expected :.");
  ++pos;
...
JSON::Pos JSON::skipArray() const {
  if (!isArray()) throw JSONException("JSON: expected [");
  Pos pos = ptr_begin;
  ++pos;
  checkPos(pos);
  if (*pos == ']') return ++pos;
...
JSON::Pos JSON::skipObject() const {
  if (!isObject()) throw JSONException("JSON: expected {");
  Pos pos = ptr_begin;
  ++pos;
  checkPos(pos);
  if (*pos == '}') return ++pos;
...
JSON JSON::operator[](size_t n) const {
  ...
  if (type != TYPE_ARRAY) throw JSONException("JSON: not array when calling operator[](size_t) method.");
  ...
  if (i != n) throw JSONException("JSON: array index " + std::to_string(n) + " out of bounds.");
...
JSON::Pos JSON::searchField(const char *data, size_t size) const {
  ...
  if (type != TYPE_OBJECT) throw JSONException("JSON: not object when calling operator[](const char *) or has(const char *) method.");

  const_iterator it = begin();
  for (; it != end(); ++it) {
    if (!it->hasEscapes()) {
      const auto current_name = it->getRawName();
      if (current_name.size() == size && 0 == memcmp(current_name.data(), data, size)) break;
    } else {
...
bool JSON::hasSpecialChars() const {
  ...
  if (*pos == '"') return false;
  if (pos < ptr_end) return true;
  throw JSONException("JSON: unexpected end of data.");
}

JSON JSON::operator[](const std::string &name) const {
  Pos pos = searchField(name);
  if (!pos) throw JSONException("JSON: there is no element '" + std::string(name) + "' in object.");
...
double JSON::getDouble() const { return readFloatText(ptr_begin, ptr_end); }

Int64 JSON::getInt() const { return readIntText(ptr_begin, ptr_end); }

UInt64 JSON::getUInt() const { return readUIntText(ptr_begin, ptr_end); }

bool JSON::getBool() const {
  if (*ptr_begin == 't') return true;
  if (*ptr_begin == 'f') return false;
...
std::string JSON::getString() const {
  ...
  if (*s != '"') throw JSONException(std::string("JSON: expected \", got ") + *s);
  ++s;
  checkPos(s);
      ...
      case '\\':
        ...
        switch (*s) {
          ...
          case 'u': {
            ...
            int res = utf8.convert(unicode, reinterpret_cast<unsigned char *>(const_cast<char *>(buf.data())) + buf.size() - 6, 6);
            if (!res) throw JSONException("JSON: cannot convert unicode " + std::to_string(unicode) + " to UTF8.");
...
std::string JSON::getName() const { return getString(); }
...
std::string_view JSON::getRawString() const {
  ...
  ++s;
  while (s != ptr_end && *s != '"') ++s;
  if (s != ptr_end) return std::string_view(ptr_begin + 1, s - ptr_begin - 1);
  throw JSONException("JSON: incorrect syntax (expected end of string, found end of JSON).");
}

std::string_view JSON::getRawName() const { return getRawString(); }
...
double JSON::toDouble() const {
  ...
  if (type == TYPE_NUMBER) return getDouble();
  if (type == TYPE_STRING) return JSON(ptr_begin + 1, ptr_end, level + 1).getDouble();
...
Int64 JSON::toInt() const {
  ...
  if (type == TYPE_NUMBER) return getInt();
  if (type == TYPE_STRING) return JSON(ptr_begin + 1, ptr_end, level + 1).getInt();
...
UInt64 JSON::toUInt() const {
  ...
  if (type == TYPE_NUMBER) return getUInt();
  if (type == TYPE_STRING) return JSON(ptr_begin + 1, ptr_end, level + 1).getUInt();
...
std::string JSON::toString() const {
  ...
  if (type == TYPE_STRING) return getString();
...
JSON::iterator JSON::iterator::begin() const {
  ...
  if (type != TYPE_ARRAY && type != TYPE_OBJECT) throw JSONException("JSON: not array or object when calling begin() method.");

  Pos pos = ptr_begin + 1;
  checkPos(pos);
  if (*pos == '}' || *pos == ']') return end();
