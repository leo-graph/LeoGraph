...
/// We used to have only ABORT_ON_LOGICAL_ERROR macro, but most of its uses were actually in places where we didn't care about logical
/// errors but wanted to check exactly if the current build type is debug or with sanitizer. This new macro is introduced to fix those
/// places.
