...
struct DecomposedFloat {
  ...
  uint16_t exponent() const { return (x_uint >> (Traits::mantissa_bits)) & ((1ull << Traits::exponent_bits) - 1); }

  int16_t normalizedExponent() const { return int16_t(exponent()) - ((1ull << (Traits::exponent_bits - 1)) - 1); }
  ...
  bool isFinite() const { return exponent() != ((1ull << Traits::exponent_bits) - 1); }

  bool isNaN() const { return !isFinite() && (mantissa() != 0); }
  ...
  int compare(Int rhs) const {
    ...
    if (normalizedExponent() < 0) {
      if (!isNegative()) return rhs > 0 ? -1 : 1;
      return rhs >= 0 ? -1 : 1;
    }
    ...
    if constexpr (is_signed_v<Int>) {
      if (rhs == std::numeric_limits<Int>::lowest()) {
        ...
        if (normalizedExponent() < static_cast<int16_t>(8 * sizeof(Int) - is_signed_v<Int>)) return 1;
        if (normalizedExponent() > static_cast<int16_t>(8 * sizeof(Int) - is_signed_v<Int>)) return -1;
    ...
    if (normalizedExponent() >= static_cast<int16_t>(8 * sizeof(Int) - is_signed_v<Int>)) return isNegative() ? -1 : 1;
    ...
    if (uint_rhs < (static_cast<UInt>(1) << normalizedExponent())) return isNegative() ? -1 : 1;
    ...
    UInt a = static_cast<UInt>(large_and_always_integer ? static_cast<UInt>(mantissa()) << (normalizedExponent() - Traits::mantissa_bits)
    ...
    if (large_and_always_integer) return 0;
