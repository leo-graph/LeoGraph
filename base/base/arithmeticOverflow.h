...
inline bool addOverflow(Int128 x, Int128 y, Int128& res) {
  ...
  return (y > 0 && x > std::numeric_limits<Int128>::max() - y) || (y < 0 && x < std::numeric_limits<Int128>::min() - y);
}
...
inline bool addOverflow(Int256 x, Int256 y, Int256& res) {
  res = addIgnoreOverflow(x, y);
  return (y > 0 && x > std::numeric_limits<Int256>::max() - y) || (y < 0 && x < std::numeric_limits<Int256>::min() - y);
}
