...
struct SearchSymbols {
  ...
  explicit SearchSymbols(std::string in) : str(std::move(in)) {
    ...
    if (str.size() > BUFFER_SIZE) {
      throw std::runtime_error("SearchSymbols may contain at most " + std::to_string(BUFFER_SIZE) + " symbols but " +
                               std::to_string(str.size()) + " symbols were provided");
    }
...
inline const char *find_last_symbols_sse2(const char *const begin, const char *const end) {
  ...
  for (; pos - 16 >= begin; pos -= 16)  /// Assuming the pointer cannot overflow. Assuming we can compare these pointers.
  {
    ...
    uint16_t bit_mask = maybe_negate<positive>(uint16_t(_mm_movemask_epi8(eq)));
    if (bit_mask) return pos - 1 - (__builtin_clz(bit_mask) - 16);  /// because __builtin_clz works with mask as uint32.
  }
#endif
...
template <bool positive, ReturnMode return_mode, size_t num_chars, char c01, char c02 = 0, char c03 = 0, char c04 = 0, char c05 = 0,
          char c06 = 0, char c07 = 0, char c08 = 0, char c09 = 0, char c10 = 0, char c11 = 0, char c12 = 0, char c13 = 0, char c14 = 0,
...
inline const char *find_first_symbols_sse42(const char *const begin, const char *const end) {
  ...
  for (; pos + 15 < end; pos += 16) {
    ...
    if constexpr (positive) {
      if (_mm_cmpestrc(set, num_chars, bytes, 16, mode)) return pos + _mm_cmpestri(set, num_chars, bytes, 16, mode);
    } else {
  ...
  for (; pos < end; ++pos)
    if ((num_chars == 1 && maybe_negate<positive>(is_in<c01>(*pos))) || (num_chars == 2 && maybe_negate<positive>(is_in<c01, c02>(*pos))) ||
        ...
        (num_chars == 15 &&
         maybe_negate<positive>(is_in<c01, c02, c03, c04, c05, c06, c07, c08, c09, c10, c11, c12, c13, c14, c15>(*pos))) ||
        (num_chars == 16 &&
         maybe_negate<positive>(is_in<c01, c02, c03, c04, c05, c06, c07, c08, c09, c10, c11, c12, c13, c14, c15, c16>(*pos))))
      return pos;
  return return_mode == ReturnMode::End ? end : nullptr;
}
...
{
#if defined(__SSE4_2__)
  if (sizeof...(symbols) >= 5) return find_first_symbols_sse42<positive, return_mode, sizeof...(symbols), symbols...>(begin, end);
#endif
...
#endif
  return find_first_symbols_sse2<positive, return_mode>(haystack.data(), haystack.data() + haystack.size(), symbols.str.data(),
                                                        symbols.str.size());
}
...
inline const char *find_last_symbols_dispatch(const std::string_view haystack, const SearchSymbols &symbols) {
  return find_last_symbols_sse2<positive, return_mode>(haystack.data(), haystack.data() + haystack.size(), symbols.str.data(),
                                                       symbols.str.size());
}
...
inline size_t count_symbols(const char *begin, const char *end) {
  size_t res = 0;
  for (const auto *ptr = begin; ptr < end; ++ptr) res += detail::is_in<symbols...>(*ptr);
  return res;
}
