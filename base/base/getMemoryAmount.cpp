...
std::optional<uint64_t> getCgroupsV2MemoryLimit() {
  ...
  if (current_cgroup.empty()) return {};
  ...
  while (current_cgroup != default_cgroups_mount.parent_path()) {
    ...
    if (setting_file.is_open()) {
      uint64_t value;
      if (setting_file >> value) return {value};
...
uint64_t getMemoryAmountOrZero() {
  ...
  if (auto total_numa_memory = DB::getNumaNodesTotalMemory(); total_numa_memory.has_value()) memory_amount = *total_numa_memory;
  ...
  if (limit_v2.has_value() && *limit_v2 < memory_amount)
    ...
    if (limit_file_v1.is_open()) {
      uint64_t limit_v1;
      if (limit_file_v1 >> limit_v1) memory_amount = std::min(memory_amount, limit_v1);
    }
  }
...
uint64_t getMemoryAmount() {
  ...
  if (!res) throw std::runtime_error("Cannot determine memory amount");
  return res;
}
