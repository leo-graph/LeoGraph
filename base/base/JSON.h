...
class JSON {
 ...
 public:
  JSON(Pos ptr_begin_, Pos ptr_end_, unsigned level_ = 0) : ptr_begin(ptr_begin_), ptr_end(ptr_end_), level(level_) { checkInit(); }

  explicit JSON(std::string_view s) : ptr_begin(s.data()), ptr_end(s.data() + s.size()), level(0) { checkInit(); }
...
T JSON::getWithDefault(const std::string &key, const T &default_) const {
  if (has(key)) {
    ...
    if (key_json.isType<T>()) return key_json.get<T>();
    return default_;
  }
  return default_;
}
