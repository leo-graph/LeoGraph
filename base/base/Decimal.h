...
struct Decimal {
  ...
  template <typename U>
  constexpr Decimal<T> operator&(const Decimal<U>& x) const {
    return Decimal<T>(value & static_cast<T>(x.value));
  }
...
template <typename T>
bool operator<(const Decimal<T>& x, const Decimal<T>& y);
template <typename T>
bool operator>(const Decimal<T>& x, const Decimal<T>& y);
template <typename T>
bool operator<=(const Decimal<T>& x, const Decimal<T>& y);
...
template <typename T>
Decimal<T> operator+(const Decimal<T>& x, const Decimal<T>& y);
template <typename T>
Decimal<T> operator-(const Decimal<T>& x, const Decimal<T>& y);
template <typename T>
Decimal<T> operator*(const Decimal<T>& x, const Decimal<T>& y);
...
Decimal<T> operator/(const Decimal<T>& x, const Decimal<T>& y);
template <typename T>
Decimal<T> operator-(const Decimal<T>& x);
template <typename T>
Decimal<T> NO_SANITIZE_UNDEFINED negateOverflow(const Decimal<T>& x);
...
struct hash<DB::Decimal128> {
  size_t operator()(const DB::Decimal128& x) const {
    return std::hash<Int64>()(static_cast<Int64>(x.value >> 64)) ^ std::hash<Int64>()(static_cast<Int64>(x.value & max_uint_mask));
  }
};

template <>
struct hash<DB::DateTime64> {
  size_t operator()(const DB::DateTime64& x) const { return std::hash<DB::DateTime64::NativeType>()(x); }
};

template <>
struct hash<DB::Time64> {
  size_t operator()(const DB::Time64& x) const { return std::hash<DB::Time64::NativeType>()(x); }
};
