...
class [[nodiscard]] BasicScopeGuard {
 public:
  ...
  constexpr BasicScopeGuard(BasicScopeGuard&& src)
  ...
  constexpr BasicScopeGuard(BasicScopeGuard<G>&& src)
      : function{src.release()} {
  }  // NOLINT(google-explicit-constructor, cppcoreguidelines-rvalue-reference-param-not-moved, cppcoreguidelines-noexcept-move-operations)

  template <typename G>
    requires std::is_convertible_v<G, F>
  constexpr BasicScopeGuard& operator=(
      BasicScopeGuard<G>&& src)  // NOLINT(cppcoreguidelines-rvalue-reference-param-not-moved, cppcoreguidelines-noexcept-move-operations)
  {
  ...
  template <typename G>
    requires std::is_convertible_v<G, F>
  constexpr BasicScopeGuard(G&& function_)
      : function{std::move(function_)} {}  // NOLINT(google-explicit-constructor, bugprone-forwarding-reference-overload,
                                           // bugprone-move-forwarding-reference, cppcoreguidelines-missing-std-forward)
  ...
  explicit operator bool() const {
    if constexpr (is_nullable) return static_cast<bool>(function);
    return true;
  }
...
inline BasicScopeGuard<F> make_scope_guard(F&& function_) {
  return std::forward<F>(function_);
}
