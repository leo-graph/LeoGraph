...
uintptr_t* allocate(size_t size) {
  ...
  if (MAP_FAILED == map) return nullptr;
...
void __sanitizer_cov_trace_pc_guard_init(uint32_t* start, uint32_t* stop) {
  if (pc_guards_initialized) return;
...
void __sanitizer_cov_pcs_init(const uintptr_t* pcs_begin, const uintptr_t* pcs_end) {
  if (pc_table_initialized) return;
  ...
  for (size_t i = 0; i < all_addresses_array_size; ++i) all_addresses_array[i] = pcs_begin[i] & 0x7FFFFFFFFFFFFFFFULL;
}
...
__attribute__((no_sanitize("coverage"))) std::span<const uintptr_t> getCurrentCoverage() {
  return {current_coverage_array, coverage_array_size};
}

__attribute__((no_sanitize("coverage"))) std::span<const uintptr_t> getCumulativeCoverage() {
  return {cumulative_coverage_array, coverage_array_size};
}

__attribute__((no_sanitize("coverage"))) std::span<const uintptr_t> getAllInstrumentedAddresses() {
...
__attribute__((no_sanitize("coverage"))) void resetCoverage() {
  ...
  for (uint32_t* x = guards_start; x < guards_end; ++x) *x = 1;
}

#else

std::span<const uintptr_t> getCoverage() { return {}; }

std::span<const uintptr_t> getAllInstrumentedAddresses() { return {}; }
