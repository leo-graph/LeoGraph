...
#  endif
  ...
  typedef typename std::conditional<std::is_same<uint64_t, unsigned long>::value, unsigned long long, unsigned long>::type uint_missing_t;
...
#  else
  for (uint8_t i = 4; i != 0; /* dec in loop */) {
    --i;
#  endif
    if (v.wa[i] != 0) return trailingzeros(v.wa[i]) + (sizeof(U) * CHAR_BIT) * i;
  }
...
std::pair<uint_x4<UInt, UIntX2>, uint_x4<UInt, UIntX2> > divmod(const uint_x4<UInt, UIntX2>& orig_dividend,
  ...
  if (orig_dividend < divisor) return {uint_x4<UInt, UIntX2>(0UL), orig_dividend};
