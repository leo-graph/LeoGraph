...
class StringPart : public Part {
 public:
  ...
  void render(const Var& data, std::ostream& out) const { out << _content; }
...
class MultiPart : public Part {
 public:
  ...
  virtual void addPart(Part* part) { _parts.push_back(part); }
...
class LogicElseQuery : public LogicQuery {
 public:
  ...
  virtual bool apply(const Var& data) const { return true; }
};
...
class IncludePart : public Part {
 public:
  IncludePart(const Path& parentPath, const Path& path) : Part(), _path(path) {
...
Template::Template(const Path& templatePath) : _parts(0), _currentPart(0), _templatePath(templatePath) {}

Template::Template() : _parts(0), _currentPart(0) {}

Template::~Template() { delete _parts; }
...
void Template::parse(std::istream& in) {
  ...
  while (in.good()) {
    ...
    if (in.bad()) break;  // Nothing to do anymore
...
void Template::render(const Var& data, std::ostream& out) const { _parts->render(data, out); }
