...
ParserImpl::~ParserImpl() { delete _pJSON; }

void ParserImpl::handle(const std::string& json) {
  if (!_allowNullByte && json.find("\\u0000") != json.npos) throw JSONException("Null bytes in strings not allowed.");
  ...
  try {
    ...
    handle();
    checkError();
    if (JSON_DONE != json_next(_pJSON)) throw JSONException("Excess characters found after JSON end.");
    json_close(_pJSON);
  } catch (std::exception&) {
    json_close(_pJSON);
    throw;
  ...
  if (_allowComments) {
    ...
    for (; it != json.end();) {
      if (*it == '"' && !inString)
        ...
        inString = false;
      if (!inString) {
        if (*it == '/' && it + 1 != json.end() && *(it + 1) == '*') inComment = true;
      }
      if (inComment) {
        char c = *it;
        it = json.erase(it);
...
void ParserImpl::handleArray() {
  ...
  if (tok == JSON_ARRAY_END)
    ...
    throw JSONException("JSON array end not found");
}
...
void ParserImpl::handleObject() {
  ...
  if (tok == JSON_OBJECT_END)
    ...
    throw JSONException("JSON object end not found");
}
