...
#  define json_error(json, format, ...)                                                                                            \
    if (!(json->flags & JSON_FLAG_ERROR)) {                                                                                        \
      ...
      _snprintf_s(json->errmsg, sizeof(json->errmsg), _TRUNCATE, "error: %lu: " format, (unsigned long)json->lineno, __VA_ARGS__); \
    }
...
#  define json_error(json, format, ...)                                                                              \
    if (!(json->flags & JSON_FLAG_ERROR)) {                                                                          \
      json->flags |= JSON_FLAG_ERROR;                                                                                \
      snprintf(json->errmsg, sizeof(json->errmsg), "error: %lu: " format, (unsigned long)json->lineno, __VA_ARGS__); \
    }
...
static enum json_type push(json_stream *json, enum json_type type) {
  ...
  if (json->stack_top >= json->stack_size) {
    ...
    stack = (struct json_stack *)json->alloc.realloc(json->stack, (json->stack_size + STACK_INC) * sizeof(*json->stack));
    if (stack == NULL) {
...
static enum json_type is_match(json_stream *json, const char *pattern, enum json_type type) {
  for (const char *p = pattern; *p; p++)
    if (*p != json->source.get(&json->source)) return JSON_ERROR;
  return type;
}
...
static int encode_utf8(json_stream *json, unsigned long c) {
  if (c < 0x80UL) {
    ...
    return !((pushchar(json, (c >> 6 & 0x1F) | 0xC0) == 0) && (pushchar(json, (c >> 0 & 0x3F) | 0x80) == 0));
  } else if (c < 0x010000UL) {
    if (c >= 0xd800 && c <= 0xdfff) {
      json_error(json, "invalid codepoint %06lx", c);
      return -1;
    }
    return !((pushchar(json, (c >> 12 & 0x0F) | 0xE0) == 0) && (pushchar(json, (c >> 6 & 0x3F) | 0x80) == 0) &&
             (pushchar(json, (c >> 0 & 0x3F) | 0x80) == 0));
  } else if (c < 0x110000UL) {
    return !((pushchar(json, (c >> 18 & 0x07) | 0xF0) == 0) && (pushchar(json, (c >> 12 & 0x3F) | 0x80) == 0) &&
             (pushchar(json, (c >> 6 & 0x3F) | 0x80) == 0) && (pushchar(json, (c >> 0 & 0x3F) | 0x80) == 0));
  } else {
...
static int read_unicode(json_stream *json) {
  ...
  if (cp >= 0xd800 && cp <= 0xdbff) {
    ...
    if (l < 0xdc00 || l > 0xdfff) {
      json_error(json,
                 "invalid surrogate pair continuation \\u%04lx out "
                 "of range (dc00-dfff)",
                 (unsigned long)l);
      return -1;
    }
...
int read_escaped(json_stream *json) {
  ...
  if (c == END_OF_SOURCE) {
    ...
    if (read_unicode(json) != 0) return -1;
  } else {
    switch (c) {
      case '\\':
      case 'b':
...
static enum json_type read_string(json_stream *json) {
  if (init_string(json) != 0) return JSON_ERROR;
  while (1) {
    int c = json->source.get(&json->source);
    if (c == END_OF_SOURCE) {
      ...
      if (read_escaped(json) != 0) return JSON_ERROR;
    } else {
      if (pushchar(json, c) != 0) return JSON_ERROR;
    }
  }
}
...
static int read_digits(json_stream *json) {
  ...
  while (is_digit(json->source.peek(&json->source))) {
    if (pushchar(json, json->source.get(&json->source)) != 0) return -1;
...
static enum json_type read_number(json_stream *json, int c) {
  ...
  if (c == '-') {
    ...
    if (is_digit(c)) {
      if (read_digits(json) != 0) return JSON_ERROR;
    }
  }
...
enum json_type json_next(json_stream *json) {
  if (json->flags & JSON_FLAG_ERROR) return JSON_ERROR;
  if (json->next != 0) {
  ...
  int c = next(json);
  if (json->stack_top == (size_t)-1) return read_value(json, c);
...
const char *json_get_error(json_stream *json) { return json->flags & JSON_FLAG_ERROR ? json->errmsg : NULL; }

size_t json_get_lineno(json_stream *json) { return json->lineno; }

size_t json_get_position(json_stream *json) { return json->source.position; }

size_t json_get_depth(json_stream *json) { return json->stack_top + 1; }
...
void json_open_string(json_stream *json, const char *string) { json_open_buffer(json, string, strlen(string)); }
...
static int user_get(struct json_source *json) { return json->source.user.get(json->source.user.ptr); }

static int user_peek(struct json_source *json) { return json->source.user.peek(json->source.user.ptr); }
...
void json_set_allocator(json_stream *json, json_allocator *a) { json->alloc = *a; }
