...
const char* Application::name() const { return "Application"; }
...
void Application::setUnixOptions(bool flag) { _unixOptions = flag; }
...
std::string Application::commandName() const { return _pConfig->getString("application.baseName"); }

std::string Application::commandPath() const { return _pConfig->getString("application.path"); }

void Application::stopOptionsProcessing() { _stopOptionsProcessing = true; }
...
int Application::main(const ArgVec& args) { return EXIT_OK; }
...
void Application::getApplicationPath(Poco::Path& appPath) const {
  ...
  if (_command.find('/') != std::string::npos) {
    ...
    if (!Path::find(Environment::get("PATH"), _command, appPath)) appPath = Path(_workingDirAtLaunch, _command);
...
void Application::setLogger(Logger& logger) { _pLogger = &logger; }
