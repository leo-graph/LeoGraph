...
PropertyFileConfiguration::PropertyFileConfiguration(std::istream& istr) { load(istr); }

PropertyFileConfiguration::PropertyFileConfiguration(const std::string& path) { load(path); }
...
void PropertyFileConfiguration::save(const std::string& path) const {
  ...
  if (ostr.good()) {
    ...
    throw Poco::CreateFileException(path);
}
