...
OptionProcessor::OptionProcessor(const OptionSet& options) : _options(options), _unixStyle(true), _ignore(false) {}

OptionProcessor::~OptionProcessor() {}

void OptionProcessor::setUnixStyle(bool flag) { _unixStyle = flag; }
...
void OptionProcessor::checkRequired() const {
  for (OptionSet::Iterator it = _options.begin(); it != _options.end(); ++it) {
    if (it->required() && _specifiedOptions.find(it->fullName()) == _specifiedOptions.end()) throw MissingOptionException(it->fullName());
  }
  if (!_deferredOption.empty()) {
    ...
    if (*it == '-') {
      ...
      if (it != end) {
        if (*it == '-') {
          ...
          return processCommon(std::string(it, end), true, optionName, optionArg);
      }
    }
  }
  return false;
...
bool OptionProcessor::processCommon(const std::string& optionStr, bool isShort, std::string& optionName, std::string& optionArg) {
  ...
  if (option.argumentRequired() && ((!isShort && optionStr.find_first_of(":=") == std::string::npos) ||
                                    (isShort && optionStr.length() == option.shortName().length()))) {
