...
XMLConfiguration::XMLConfiguration() : _delim('.') { loadEmpty("config"); }

XMLConfiguration::XMLConfiguration(char delim) : _delim(delim) { loadEmpty("config"); }

XMLConfiguration::XMLConfiguration(Poco::XML::InputSource* pInputSource) : _delim('.') { load(pInputSource); }

XMLConfiguration::XMLConfiguration(Poco::XML::InputSource* pInputSource, char delim) : _delim(delim) { load(pInputSource); }

XMLConfiguration::XMLConfiguration(std::istream& istr) : _delim('.') { load(istr); }

XMLConfiguration::XMLConfiguration(std::istream& istr, char delim) : _delim(delim) { load(istr); }

XMLConfiguration::XMLConfiguration(const std::string& path) : _delim('.') { load(path); }

XMLConfiguration::XMLConfiguration(const std::string& path, char delim) : _delim(delim) { load(path); }

XMLConfiguration::XMLConfiguration(const Poco::XML::Document* pDocument) : _delim('.') { load(pDocument); }

XMLConfiguration::XMLConfiguration(const Poco::XML::Document* pDocument, char delim) : _delim(delim) { load(pDocument); }

XMLConfiguration::XMLConfiguration(const Poco::XML::Node* pNode) : _delim('.') { load(pNode); }

XMLConfiguration::XMLConfiguration(const Poco::XML::Node* pNode, char delim) : _delim(delim) { load(pNode); }
...
void XMLConfiguration::load(Poco::XML::InputSource* pInputSource) { load(pInputSource, POCO_XML_NAMEPOOL_DEFAULT_SIZE); }
...
void XMLConfiguration::setRaw(const std::string& key, const std::string& value) {
  ...
  if (pNode) {
    ...
    throw NotFoundException("Node not found in XMLConfiguration", key);
}
...
Poco::XML::Node* XMLConfiguration::findNode(std::string::const_iterator& it, const std::string::const_iterator& end, Poco::XML::Node* pNode,
                                            bool create) const {
  if (pNode && it != end) {
    if (*it == '[') {
      ++it;
...
Poco::XML::Node* XMLConfiguration::findElement(const std::string& name, Poco::XML::Node* pNode, bool create) {
  ...
  while (pChild) {
    if (pChild->nodeType() == Poco::XML::Node::ELEMENT_NODE && pChild->nodeName() == name) return pChild;
    pChild = pChild->nextSibling();
  }
  if (create) {
...
Poco::XML::Node* XMLConfiguration::findElement(int index, Poco::XML::Node* pNode, bool create) {
  ...
  if (!pNode && create) {
    if (index == 1) {
      ...
      throw Poco::InvalidArgumentException("Element index out of range.");
  }
  return pNode;
}
