...
Option::Option() : _required(false), _repeatable(false), _argRequired(false), _pValidator(0), _pCallback(0), _pConfig(0) {}
...
Option& Option::binding(const std::string& propertyName) { return binding(propertyName, 0); }
...
bool Option::matchesShort(const std::string& option) const {
  return option.length() > 0 && !_shortName.empty() && option.compare(0, _shortName.length(), _shortName) == 0;
}
...
bool Option::matchesFull(const std::string& option) const {
  ...
  return len == _fullName.length() && icompare(option, 0, len, _fullName, 0, len) == 0;
}

bool Option::matchesPartial(const std::string& option) const {
  std::string::size_type pos = option.find_first_of(":=");
  std::string::size_type len = pos == std::string::npos ? option.length() : pos;
  return option.length() > 0 && icompare(option, 0, len, _fullName, 0, len) == 0;
}
...
void Option::process(const std::string& option, std::string& arg) const {
  ...
  if (icompare(option, 0, len, _fullName, 0, len) == 0) {
    if (takesArgument()) {
      if (argumentRequired() && pos == std::string::npos) throw MissingArgumentException(_fullName + " requires " + argumentName());
