 ...
 public:
  ...
  bool set(const XMLString& qname, const XMLString& namespaceURI, const XMLString& localName) {
    if (!_used) {
      ...
      return _name.equals(qname, namespaceURI, localName);
  }

  const Name& get() const { return _name; }
...
NamePool::NamePool(unsigned long size) : _size(size), _salt(0), _rc(1) {
  poco_assert(size > 1);
...
NamePool::~NamePool() { delete[] _pItems; }

void NamePool::duplicate() { ++_rc; }
...
const Name& NamePool::insert(const XMLString& qname, const XMLString& namespaceURI, const XMLString& localName) {
  ...
  while (!_pItems[n].set(qname, namespaceURI, localName) && i++ < _size) n = (n + 1) % _size;
...
const Name& NamePool::insert(const Name& name) { return insert(name.qname(), name.namespaceURI(), name.localName()); }
