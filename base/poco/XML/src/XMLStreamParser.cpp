...
struct StreamExceptionController {
  StreamExceptionController(std::istream& is) : _istr(is), _oldState(_istr.exceptions()) {
  ...
  ~StreamExceptionController() {
    ...
    if (!(_oldState & s)) {
      ...
      //
      if (_istr.fail() && _istr.eof()) _istr.clear(s);
...
static const char* parserEventStrings[] = {"start element",
                                           "end element",
                                           "start attribute",
                                           "end attribute",
                                           "characters",
...
std::ostream& operator<<(std::ostream& os, XMLStreamParser::EventType e) { return os << parserEventStrings[e]; }

XMLStreamParser::XMLStreamParser(std::istream& is, const std::string& iname, FeatureType f) : _size(0), _inputName(iname), _feature(f) {
  _data.is = &is;
  init();
}

XMLStreamParser::XMLStreamParser(const void* data, std::size_t size, const std::string& iname, FeatureType f)
    : _size(size), _inputName(iname), _feature(f) {
...
void XMLStreamParser::init() {
  ...
  if ((_feature & RECEIVE_ATTRIBUTE_MAP) != 0 && (_feature & RECEIVE_ATTRIBUTES_EVENT) != 0) _feature &= ~RECEIVE_ATTRIBUTE_MAP;
  ...
  if (_parser == 0) throw std::bad_alloc();
  ...
  if ((_feature & RECEIVE_CHARACTERS) != 0) XML_SetCharacterDataHandler(_parser, &handleCharacters);

  if ((_feature & RECEIVE_NAMESPACE_DECLS) != 0) XML_SetNamespaceDeclHandler(_parser, &handleStartNamespaceDecl, &handleEndNamespaceDecl);
}
...
XMLStreamParser::EventType XMLStreamParser::next() {
  if (_parserState == state_next)
    ...
    switch (_currentEvent) {
      case EV_END_ELEMENT: {
        if (!_elementState.empty() && _elementState.back().depth == _depth) popElement();
...
void XMLStreamParser::nextExpect(EventType e) {
  if (next() != e) throw XMLStreamParserException(*this, std::string(parserEventStrings[e]) + " expected");
}
...
const XMLStreamParser::ElementEntry* XMLStreamParser::getElementImpl() const {
  ...
  if (_elementState[n].depth == _depth)
    ...
    n--;
    if (_elementState[n].depth == _depth) r = &_elementState[n];
  }
...
void XMLStreamParser::popElement() {
  ...
  if (e.attributesUnhandled != 0) {
    ...
    for (const auto& p : e.attributeMap) {
      if (!p.second.handled) throw XMLStreamParserException(*this, "unexpected attribute '" + p.first.toString() + "'");
    }
    poco_assert(false);
  }
...
XMLStreamParser::EventType XMLStreamParser::nextBody() {
    ...
    case XML_SUSPENDED: {
      switch (XML_ResumeParser(_parser)) {
        ...
        case XML_STATUS_OK: {
          ...
          //
          if (ps.finalBuffer) return _currentEvent = EV_EOF;
  ...
  // or reach eof.
  //
  if (!_accumulateContent) _currentEvent = EV_EOF;
  ...
  do {
    if (_size != 0) {
      ...
      if (s == XML_STATUS_ERROR) handleError();
      ...
      //
      if (is.bad() || (is.fail() && !is.eof())) throw XMLStreamParserException(*this, "io failure");
...
void XMLCALL XMLStreamParser::handleStartElement(void* v, const XML_Char* name, const XML_Char** atts) {
  ...
  //
  if (ps.parsing == XML_FINISHED) return;
  ...
  if (*atts != 0) {
    ...
    if (am || ae) {
      ...
      if (am) pe->attributesUnhandled = pe->attributeMap.size();
    }
  }
