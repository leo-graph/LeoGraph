...
const std::string& XMLWriter::getNewLine() const { return _newLine; }

void XMLWriter::setIndent(const std::string& indent) { _indent = indent; }

const std::string& XMLWriter::getIndent() const { return _indent; }

void XMLWriter::startDocument() {
  if (_depth != -1) throw XMLException("Cannot start a document in another document");
  ...
  if (_options & WRITE_XML_DECLARATION) writeXMLDeclaration();
...
void XMLWriter::endDocument() {
  if (_depth > 0) throw XMLException("Not well-formed (at least one tag has no matching end tag)");
  if (_elementCount == 0) throw XMLException("No document element");
...
void XMLWriter::startFragment() {
  if (_depth != -1) throw XMLException("Cannot start a fragment in another fragment or document");
...
void XMLWriter::endFragment() {
  if (_depth > 1) throw XMLException("Not well-formed (at least one tag has no matching end tag)");
...
void XMLWriter::endElement(const XMLString& namespaceURI, const XMLString& localName, const XMLString& qname) {
  if (_depth < 1) throw XMLException("No unclosed tag");
...
void XMLWriter::emptyElement(const XMLString& namespaceURI, const XMLString& localName, const XMLString& qname,
                             const Attributes& attributes) {
  if (_depth == 0 && _elementCount > 1) throw XMLException("Not well-formed. Second root element found.");
...
void XMLWriter::ignorableWhitespace(const XMLChar ch[], int start, int length) { characters(ch, start, length); }
...
void XMLWriter::dataElement(const XMLString& namespaceURI, const XMLString& localName, const XMLString& qname, const XMLString& data,
                            const XMLString& attr1, const XMLString& value1, const XMLString& attr2, const XMLString& value2,
...
void XMLWriter::unparsedEntityDecl(const XMLString& name, const XMLString* publicId, const XMLString& systemId,
...
void XMLWriter::writeStartElement(const XMLString& namespaceURI, const XMLString& localName, const XMLString& qname,
                                  const Attributes& attributes) {
  if (!_nsContextPushed) _namespaces.pushContext();
  _nsContextPushed = false;
  ++_elementCount;
  ...
  if (!localName.empty() && (qname.empty() || localName == qname)) {
    ...
    throw XMLException("Tag mismatch", nameToString(localName, qname));
...
void XMLWriter::writeCanonicalStartElement(const XMLString& namespaceURI, const XMLString& localName, const XMLString& qname,
                                           const Attributes& attributes) {
  if (!_nsContextPushed) _namespaces.pushContext();
  _nsContextPushed = false;
  ++_elementCount;
...
void XMLWriter::declareNamespaces(const XMLString& namespaceURI, const XMLString& localName, const XMLString& qname,
                                  const Attributes& attributes) {
  ...
  Name::split(qname, prefix, local);
  if (elementNamespaceURI.empty()) elementNamespaceURI = _namespaces.getURI(prefix);
  if (!elementNamespaceURI.empty()) {
    usedNamespaces[prefix].insert(elementNamespaceURI);
    if (!defaultNamespaceURI.empty() && elementNamespaceURI == defaultNamespaceURI) defaultNameSpaceUsed = true;
  }
  for (int i = 0; i < attributes.getLength(); i++) {
    ...
    if (attributeNamespaceURI.empty()) attributeNamespaceURI = _namespaces.getURI(prefix);
  ...
  for (const auto& p : usedNamespaces) {
    ...
    for (const auto& nsURI : namespaceURIs) {
      ...
      if (prefix.empty()) prefix = _namespaces.getPrefix(nsURI);
      if (prefix.empty() && !_namespaces.isMapped(nsURI)) {
        if (defaultNameSpaceUsed) {
          if (nsURI != defaultNamespaceURI) prefix = uniquePrefix();
        } else {
...
void XMLWriter::addAttributes(AttributeMap& attributeMap, const Attributes& attributes, const XMLString& elementNamespaceURI) {
  for (int i = 0; i < attributes.getLength(); i++) {
    ...
    if (!localName.empty()) {
      XMLString prefix;
      if (!namespaceURI.empty()) prefix = _namespaces.getPrefix(namespaceURI);
      if (!prefix.empty()) {
        qname = prefix;
        qname.append(toXMLString(MARKUP_COLON));
      } else
    ...
    for (auto c : ap.second) {
      switch (c) {
        ...
        case '>':
          ...
          break;
        case '\t':
          writeMarkup(MARKUP_TABENC);
          break;
        case '\r':
          writeMarkup(MARKUP_CRENC);
          break;
        case '\n':
          writeMarkup(MARKUP_LFENC);
          break;
        default:
          if (c >= 0 && c < 32)
            throw XMLException("Invalid character token.");
...
void XMLWriter::writeXML(XMLChar ch) const { _pTextConverter->write((const char*)&ch, sizeof(ch)); }
...
void XMLWriter::writeNewLine() const {
  if (_options & PRETTY_PRINT) writeMarkup(_newLine);
}

void XMLWriter::writeIndent() const { writeIndent(_depth); }

void XMLWriter::writeIndent(int depth) const {
  for (int i = 0; i < depth; ++i) writeMarkup(_indent);
}
...
bool XMLWriter::isNamespaceMapped(const XMLString& namespc) const { return _namespaces.isMapped(namespc); }
