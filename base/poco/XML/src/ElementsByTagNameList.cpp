...
ElementsByTagNameList::ElementsByTagNameList(const Node* pParent, const XMLString& name) : _pParent(pParent), _name(name), _count(0) {
  poco_check_ptr(pParent);

  _pParent->duplicate();
}

ElementsByTagNameList::~ElementsByTagNameList() { _pParent->release(); }
...
void ElementsByTagNameList::autoRelease() { _pParent->ownerDocument()->autoReleasePool().add(this); }
...
ElementsByTagNameListNS::ElementsByTagNameListNS(const Node* pParent, const XMLString& namespaceURI, const XMLString& localName)
    : _pParent(pParent), _localName(localName), _namespaceURI(namespaceURI), _count(0) {
  poco_check_ptr(pParent);

  _pParent->duplicate();
}

ElementsByTagNameListNS::~ElementsByTagNameListNS() { _pParent->release(); }
...
Node* ElementsByTagNameListNS::find(const Node* pParent, unsigned long index) const {
  ...
  Node* pCur = pParent->firstChild();
  while (pCur) {
    if (pCur->nodeType() == Node::ELEMENT_NODE && (_localName == asterisk || pCur->localName() == _localName) &&
        (_namespaceURI == asterisk || pCur->namespaceURI() == _namespaceURI)) {
      if (_count == index) return pCur;
      _count++;
    }
    Node* pNode = find(pCur, index);
...
void ElementsByTagNameListNS::autoRelease() { _pParent->ownerDocument()->autoReleasePool().add(this); }
