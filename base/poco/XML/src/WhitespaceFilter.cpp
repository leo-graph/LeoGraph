...
WhitespaceFilter::WhitespaceFilter() : _pLexicalHandler(0), _filter(true) {}

WhitespaceFilter::WhitespaceFilter(XMLReader* pReader) : XMLFilterImpl(pReader), _pLexicalHandler(0), _filter(true) {}
...
void WhitespaceFilter::characters(const XMLChar ch[], int start, int length) {
  if (_filter) {
    ...
    XMLFilterImpl::characters(ch, start, length);
}
...
void WhitespaceFilter::startDTD(const XMLString& name, const XMLString& publicId, const XMLString& systemId) {
  if (_pLexicalHandler) _pLexicalHandler->startDTD(name, publicId, systemId);
}
...
void WhitespaceFilter::startEntity(const XMLString& name) {
  if (_pLexicalHandler) _pLexicalHandler->startEntity(name);
  _filter = true;
  _data.clear();
}

void WhitespaceFilter::endEntity(const XMLString& name) {
  if (_pLexicalHandler) _pLexicalHandler->endEntity(name);
  _filter = true;
  _data.clear();
}
...
void WhitespaceFilter::comment(const XMLChar ch[], int start, int length) {
  if (_pLexicalHandler) _pLexicalHandler->comment(ch, start, length);
  _filter = true;
  _data.clear();
}
