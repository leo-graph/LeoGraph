...
Element::Element(Document* pOwnerDocument, const XMLString& namespaceURI, const XMLString& localName, const XMLString& qname)
    : AbstractContainerNode(pOwnerDocument), _name(pOwnerDocument->namePool().insert(qname, namespaceURI, localName)), _pFirstAttr(0) {}

Element::Element(Document* pOwnerDocument, const Element& element)
    : AbstractContainerNode(pOwnerDocument, element), _name(pOwnerDocument->namePool().insert(element._name)), _pFirstAttr(0) {
...
Attr* Element::setAttributeNode(Attr* newAttr) {
  ...
  if (newAttr->ownerDocument() != ownerDocument()) throw DOMException(DOMException::WRONG_DOCUMENT_ERR);
  ...
  if (_pOwner->events()) dispatchAttrModified(newAttr, MutationEvent::ADDITION, EMPTY_STRING, newAttr->getValue());
...
Attr* Element::removeAttributeNode(Attr* oldAttr) {
  ...
  if (_pOwner->events()) dispatchAttrModified(oldAttr, MutationEvent::REMOVAL, oldAttr->getValue(), EMPTY_STRING);
  ...
  if (oldAttr != _pFirstAttr) {
    ...
    _pFirstAttr = static_cast<Attr*>(_pFirstAttr->_pNext);
  oldAttr->_pNext = 0;
  oldAttr->_pParent = 0;
  oldAttr->autoRelease();
...
NodeList* Element::getElementsByTagName(const XMLString& name) const { return new ElementsByTagNameList(this, name); }
...
const XMLString& Element::nodeName() const { return tagName(); }

NamedNodeMap* Element::attributes() const { return new AttrMap(const_cast<Element*>(this)); }

unsigned short Element::nodeType() const { return Node::ELEMENT_NODE; }
...
Attr* Element::getAttributeNodeNS(const XMLString& namespaceURI, const XMLString& localName) const {
  Attr* pAttr = _pFirstAttr;
  while (pAttr && (pAttr->_name.namespaceURI() != namespaceURI || pAttr->_name.localName() != localName))
    pAttr = static_cast<Attr*>(pAttr->_pNext);
  return pAttr;
}
...
bool Element::hasAttribute(const XMLString& name) const { return getAttributeNode(name) != 0; }
...
const XMLString& Element::namespaceURI() const { return _name.namespaceURI(); }

XMLString Element::prefix() const { return _name.prefix(); }

const XMLString& Element::localName() const { return _name.localName(); }

bool Element::hasAttributes() const { return _pFirstAttr != 0; }
...
Element* Element::getChildElement(const XMLString& name) const {
  ...
  while (pNode && !(pNode->nodeType() == Node::ELEMENT_NODE && pNode->nodeName() == name)) pNode = pNode->nextSibling();
...
Element* Element::getElementById(const XMLString& elementId, const XMLString& idAttribute) const {
  if (getAttribute(idAttribute) == elementId) return const_cast<Element*>(this);
...
Element* Element::getElementByIdNS(const XMLString& elementId, const XMLString& idAttributeURI,
                                   const XMLString& idAttributeLocalName) const {
  if (getAttributeNS(idAttributeURI, idAttributeLocalName) == elementId) return const_cast<Element*>(this);
