...
AbstractContainerNode::AbstractContainerNode(Document* pOwnerDocument) : AbstractNode(pOwnerDocument), _pFirstChild(0) {}

AbstractContainerNode::AbstractContainerNode(Document* pOwnerDocument, const AbstractContainerNode& node)
    : AbstractNode(pOwnerDocument, node), _pFirstChild(0) {}
...
Node* AbstractContainerNode::firstChild() const { return _pFirstChild; }
...
Node* AbstractContainerNode::insertBefore(Node* newChild, Node* refChild) {
  ...
  if (refChild && static_cast<AbstractNode*>(refChild)->_pParent != this) throw DOMException(DOMException::NOT_FOUND_ERR);
  if (newChild == refChild) return newChild;
  if (this == newChild) throw DOMException(DOMException::HIERARCHY_REQUEST_ERR);
...
Node* AbstractContainerNode::replaceChild(Node* newChild, Node* oldChild) {
  ...
  if (static_cast<AbstractNode*>(newChild)->_pOwner != _pOwner && static_cast<AbstractNode*>(newChild)->_pOwner != this)
    throw DOMException(DOMException::WRONG_DOCUMENT_ERR);
  if (static_cast<AbstractNode*>(oldChild)->_pParent != this) throw DOMException(DOMException::NOT_FOUND_ERR);
...
Node* AbstractContainerNode::appendChild(Node* newChild) { return insertBefore(newChild, 0); }
...
bool AbstractContainerNode::hasChildNodes() const { return _pFirstChild != 0; }

bool AbstractContainerNode::hasAttributes() const { return false; }
...
const Node* AbstractContainerNode::findNode(XMLString::const_iterator& it, const XMLString::const_iterator& end, const Node* pNode,
                                            const NSMap* pNSMap, bool& indexBound) {
  indexBound = false;
  if (pNode && it != end) {
    if (*it == '[') {
      ++it;
...
const Node* AbstractContainerNode::findElement(const XMLString& name, const Node* pNode, const NSMap* pNSMap) {
  while (pNode) {
    if (pNode->nodeType() == Node::ELEMENT_NODE && namesAreEqual(pNode, name, pNSMap)) return pNode;
...
bool AbstractContainerNode::namesAreEqual(const Node* pNode, const XMLString& name, const NSMap* pNSMap) {
  if (pNSMap) {
    ...
    if (name == WILDCARD) {
      ...
      return (pNode->namespaceURI() == namespaceURI || namespaceURI == WILDCARD) &&
             (pNode->localName() == localName || localName == WILDCARD);
    } else
      return false;
  } else {
