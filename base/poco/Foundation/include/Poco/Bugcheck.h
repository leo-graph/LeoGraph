...
#    define poco_assert_msg_dbg(cond, text)                         \
      if (!(cond))                                                  \
        Poco::Bugcheck::assertion(#cond, __FILE__, __LINE__, text); \
      else                                                          \
        (void)0
#  else
#    define poco_assert_msg_dbg(cond, text)
...
#if defined(_DEBUG)
#  define poco_stdout_dbg(outstr) std::cout << __FILE__ << '(' << std::dec << __LINE__ << "):" << outstr << std::endl;
#else
#  define poco_stdout_dbg(outstr)
#endif

#if defined(_DEBUG)
#  define poco_stderr_dbg(outstr) std::cerr << __FILE__ << '(' << std::dec << __LINE__ << "):" << outstr << std::endl;
#else
...
#  define poco_static_assert(B) \
    typedef char POCO_JOIN(poco_static_assert_typedef_, __LINE__)[POCO_STATIC_ASSERTION_FAILURE<(bool)(B)>::value]
#else
#  define poco_static_assert(B)                                                                                                        \
