...
// inlines
//
inline size_t RefCountedObject::referenceCount() const { return _counter.load(std::memory_order_acquire); }

inline size_t RefCountedObject::duplicate() const { return _counter.fetch_add(1, std::memory_order_acq_rel); }
...
inline size_t RefCountedObject::release() const throw() {
  ...
  try {
    if (reference_count_before == 1) delete this;
  } catch (...) {
    poco_unexpected();
  }
