...
// inlines
//
inline NDCScope::NDCScope(const std::string& info) { NestedDiagnosticContext::current().push(info); }
...
// helper macros
//
#define poco_ndc(func) Poco::NDCScope _theNdcScope(#func, __LINE__, __FILE__)

#if defined(_DEBUG)
#  define poco_ndc_dbg(func) Poco::NDCScope _theNdcScope(#func, __LINE__, __FILE__)
#else
