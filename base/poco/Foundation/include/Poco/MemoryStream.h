 ...
 public:
  ...
  virtual pos_type seekoff(off_type off, std::ios_base::seekdir way,
    ...
    if ((which & std::ios_base::in) != 0) {
      ...
      if ((newoff + off) < 0 || (this->egptr() - this->eback()) < (newoff + off)) return fail;
      this->setg(this->eback(), this->eback() + newoff + off, this->egptr());
    }

    if ((which & std::ios_base::out) != 0) {
      if (this->pptr() == 0) return fail;

      if (way == std::ios_base::beg) {
        newoff = 0;
      } else if (way == std::ios_base::cur) {
        // cur is not valid if both in and out are specified (Condition 3)
        if ((which & std::ios_base::in) != 0) return fail;
      ...
      if (newoff + off < 0 || (this->epptr() - this->pbase()) < newoff + off) return fail;
...
class Foundation_API MemoryInputStream : public MemoryIOS,
                                         public std::istream
...
class Foundation_API MemoryOutputStream : public MemoryIOS,
                                          public std::ostream
/// An input stream for reading from a memory area.
{
 public:
...
// inlines
//
inline MemoryStreamBuf *MemoryIOS::rdbuf() { return &_buf; }

inline std::streamsize MemoryOutputStream::charsWritten() const { return _buf.charsWritten(); }
