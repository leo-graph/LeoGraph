...
/// is needed.
{
 public:
  ...
  /// is true, InvalidAccessException is thrown.
  {
    ...
    if (preserveContent && (newSize < _used)) throw InvalidAccessException("Can not resize FIFO without data loss.");
  ...
  /// Returns the length of data written.
  {
    if (length == 0 || length > buffer.size()) length = buffer.size();
  ...
  /// the used buffer size.
  {
    ...
    if (length > available()) throw Poco::InvalidAccessException("Cannot extend buffer.");

    if (!isWritable()) throw Poco::InvalidAccessException("Buffer not writable.");
  ...
  {
    Mutex::ScopedLock lock(_mutex);
    if (index >= _used) throw InvalidAccessException(format("Index out of bounds: %z (max index allowed: %z)", index, _used - 1));
