...
/// VarHolderImpl is available. For supported types, see VarHolder documentation.
{
 public:
  ...
  /// Throws InvalidAccessException if Var is empty.
  {
    ...
    if (!pHolder) throw InvalidAccessException("Can not convert empty value.");
  ...
  /// Throws InvalidAccessException if Var is empty.
  {
    ...
    if (pHolder && pHolder->type() == typeid(T)) {
      ...
      throw BadCastException(format("Can not convert %s to %s.", std::string(pHolder->type().name()), std::string(typeid(T).name())));
  }
 ...
 private:
  ...
  VarHolder *content() const { return _pHolder; }
  ...
  VarHolder *content() const { return _placeholder.content(); }
...
inline Var::ConstIterator Var::end() const { return ConstIterator(const_cast<Var *>(this), true); }
...
inline Var::Iterator Var::end() { return Iterator(this, true); }

inline Var &Var::operator[](const std::string &name) { return getAt(name); }

inline const Var &Var::operator[](const std::string &name) const { return const_cast<Var *>(this)->getAt(name); }

inline const Var Var::operator+(const char *other) const { return convert<std::string>() + other; }

inline Var &Var::operator+=(const char *other) { return *this = convert<std::string>() + other; }

inline bool Var::operator!() const { return !convert<bool>(); }

inline bool Var::isEmpty() const { return 0 == content(); }
