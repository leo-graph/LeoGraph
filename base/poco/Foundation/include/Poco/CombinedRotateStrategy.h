...
class CombinedRotateStrategy : public RotateStrategy {
 public:
  CombinedRotateStrategy(const std::string& _rotation, const std::string& _times) {
    ...
    if (rotation.size) addRotateStrategy(new RotateBySizeStrategy(*rotation.size));

    if (rotation.interval) addRotateStrategy(new RotateByIntervalStrategy(*rotation.interval));
  ...
  void addRotateStrategy(RotateStrategy* strategy) { _pRotateStrategies.emplace_back(strategy); }

  bool mustRotate(LogFile* pFile) override {
    for (auto& strategy : _pRotateStrategies) {
      if (strategy->mustRotate(pFile)) return true;
    }
 ...
 private:
  ...
  static bool parseNever(std::string_view str) { return str == "never"; }
  ...
  RotationInfo parseRotation(std::string_view rotation) {
    ...
    if (!res.size) res.size = parseSize(rotation, pos);  /// To handle the case when an interval is specified before a size

    if (!res.size && !res.interval && !res.time && !parseNever(rotation))
      throw InvalidArgumentException("Invalid rotation format '" + std::string(rotation) +
