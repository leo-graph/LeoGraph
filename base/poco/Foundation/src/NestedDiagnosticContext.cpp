...
NestedDiagnosticContext::NestedDiagnosticContext(const NestedDiagnosticContext& ctx) { _stack = ctx._stack; }

NestedDiagnosticContext::~NestedDiagnosticContext() {}

NestedDiagnosticContext& NestedDiagnosticContext::operator=(const NestedDiagnosticContext& ctx) {
  if (&ctx != this) _stack = ctx._stack;
  return *this;
}
...
int NestedDiagnosticContext::depth() const { return int(_stack.size()); }
...
void NestedDiagnosticContext::dump(std::ostream& ostr) const { dump(ostr, "\n"); }

void NestedDiagnosticContext::dump(std::ostream& ostr, const std::string& delimiter) const {
  for (Stack::const_iterator it = _stack.begin(); it != _stack.end(); ++it) {
    ostr << it->info;
    if (it->file) ostr << " (in \"" << it->file << "\", line " << it->line << ")";
    ostr << delimiter;
  }
}

void NestedDiagnosticContext::clear() { _stack.clear(); }

namespace {
static ThreadLocal<NestedDiagnosticContext> ndc;
}

NestedDiagnosticContext& NestedDiagnosticContext::current() { return ndc.get(); }
