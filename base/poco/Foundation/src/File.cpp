...
Timestamp File::created() const { return createdImpl(); }
...
void File::linkTo(const std::string& path, LinkType type) const { linkToImpl(path, type); }
...
void File::handleLastError(const std::string& path) { handleLastErrorImpl(path); }
