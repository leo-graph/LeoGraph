...
Glob::Glob(const std::string& pattern, int options) : _pattern(pattern), _options(options) {}
...
bool Glob::matchSet(TextIterator& itp, const TextIterator& endp, int c) {
  if (_options & GLOB_CASELESS) c = Unicode::toLower(c);
...
void Glob::collect(const Path& pathPattern, const Path& base, const Path& current, const std::string& pattern, std::set<std::string>& files,
                   int options) {
  try {
