...
void PurgeStrategy::list(const std::string& path, std::vector<File>& files) {
  ...
  while (it != end) {
    if (it.name().compare(0, baseName.size(), baseName) == 0 && it.name().find(".incomplete") == std::string::npos) {
      files.push_back(*it);
    }
    ++it;
  }
...
PurgeByCountStrategy::PurgeByCountStrategy(int count) : _count(count) { poco_assert(count > 0); }
...
void PurgeOneFileStrategy::purge(const std::string& path) {
  ...
  for (++it; it != files.end(); ++it)
    if (it->getLastModified() < purge_it->getLastModified()) purge_it = it;
