...
Clock::Clock(ClockVal tv) { _clock = tv; }

Clock::Clock(const Clock& other) { _clock = other._clock; }
...
void Clock::swap(Clock& timestamp) { std::swap(_clock, timestamp._clock); }
...
void Clock::update() {
  ...
  struct timespec ts;
  if (clock_gettime(CLOCK_MONOTONIC, &ts)) throw SystemException("cannot get system clock");
  _clock = ClockVal(ts.tv_sec) * resolution() + ts.tv_nsec / 1000;
...
Clock::ClockDiff Clock::accuracy() {
  ...
  struct timespec ts;
  if (clock_getres(CLOCK_MONOTONIC, &ts)) throw SystemException("cannot get system clock");
