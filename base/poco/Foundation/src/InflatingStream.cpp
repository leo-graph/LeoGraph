...
InflatingStreamBuf::InflatingStreamBuf(std::istream& istr, StreamType type)
    : BufferedStreamBuf(STREAM_BUFFER_SIZE, std::ios::in), _pIstr(&istr), _pOstr(0), _eof(false), _check(type != STREAM_ZIP) {
  _zstr.next_in = 0;
  _zstr.avail_in = 0;
  _zstr.total_in = 0;
  _zstr.next_out = 0;
...
InflatingStreamBuf::InflatingStreamBuf(std::istream& istr, int windowBits)
    : BufferedStreamBuf(STREAM_BUFFER_SIZE, std::ios::in), _pIstr(&istr), _pOstr(0), _eof(false), _check(false) {
  _zstr.zalloc = Z_NULL;
  _zstr.zfree = Z_NULL;
  _zstr.opaque = Z_NULL;
  _zstr.next_in = 0;
...
InflatingStreamBuf::InflatingStreamBuf(std::ostream& ostr, StreamType type)
    : BufferedStreamBuf(STREAM_BUFFER_SIZE, std::ios::out), _pIstr(0), _pOstr(&ostr), _eof(false), _check(type != STREAM_ZIP) {
  _zstr.zalloc = Z_NULL;
  _zstr.zfree = Z_NULL;
  _zstr.opaque = Z_NULL;
  _zstr.next_in = 0;
...
InflatingStreamBuf::InflatingStreamBuf(std::ostream& ostr, int windowBits)
    : BufferedStreamBuf(STREAM_BUFFER_SIZE, std::ios::out), _pIstr(0), _pOstr(&ostr), _eof(false), _check(false) {
  _zstr.zalloc = Z_NULL;
  _zstr.zfree = Z_NULL;
  _zstr.opaque = Z_NULL;
  _zstr.next_in = 0;
...
int InflatingStreamBuf::readFromDevice(char* buffer, std::streamsize length) {
  ...
  for (;;) {
    ...
    if (_zstr.avail_out == 0) return static_cast<int>(length);
    if (_zstr.avail_in == 0) {
      int n = 0;
      if (_pIstr->good()) {
        _pIstr->read(_buffer, INFLATE_BUFFER_SIZE);
...
InflatingIOS::InflatingIOS(std::ostream& ostr, InflatingStreamBuf::StreamType type) : _buf(ostr, type) { poco_ios_init(&_buf); }

InflatingIOS::InflatingIOS(std::ostream& ostr, int windowBits) : _buf(ostr, windowBits) { poco_ios_init(&_buf); }
...
InflatingStreamBuf* InflatingIOS::rdbuf() { return &_buf; }
...
InflatingOutputStream::InflatingOutputStream(std::ostream& ostr, int windowBits) : std::ostream(&_buf), InflatingIOS(ostr, windowBits) {}

InflatingOutputStream::~InflatingOutputStream() {}

int InflatingOutputStream::close() { return _buf.close(); }
...
InflatingInputStream::InflatingInputStream(std::istream& istr, int windowBits) : std::istream(&_buf), InflatingIOS(istr, windowBits) {}
