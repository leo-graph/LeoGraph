...
void SHA1Engine::updateImpl(const void* buffer_, std::size_t count) {
  ...
  /* Update bitcount */
  if ((_context.countLo + ((UInt32)count << 3)) < _context.countLo) _context.countHi++; /* Carry from low to high bitCount */
...
std::size_t SHA1Engine::digestLength() const { return DIGEST_SIZE; }
...
const DigestEngine::Digest& SHA1Engine::digest() {
  ...
  for (count = 0; count < DIGEST_SIZE; count++) hash[count] = (BYTE)((_context.digest[count >> 2]) >> (8 * (3 - (count & 0x3)))) & 0xff;
  _digest.clear();
...
void SHA1Engine::transform() {
  ...
  for (i = 0; i < 16; i++) W[i] = _context.data[i];
