...
FIFOBufferStreamBuf::FIFOBufferStreamBuf(FIFOBuffer& fifoBuffer)
    : BufferedBidirectionalStreamBuf(fifoBuffer.size() + 4, std::ios::in | std::ios::out), _pFIFOBuffer(0), _fifoBuffer(fifoBuffer) {
...
FIFOBufferStreamBuf::~FIFOBufferStreamBuf() { delete _pFIFOBuffer; }
...
FIFOIOS::FIFOIOS(const char* pBuffer, std::size_t length) : _buf(pBuffer, length) { poco_ios_init(&_buf); }
...
FIFOBufferStreamBuf* FIFOIOS::rdbuf() { return &_buf; }

void FIFOIOS::close() { _buf.sync(); }
...
FIFOBufferStream::FIFOBufferStream(FIFOBuffer& fifoBuffer)
    : FIFOIOS(fifoBuffer), std::iostream(&_buf), readable(_buf.fifoBuffer().readable), writable(_buf.fifoBuffer().writable) {}

FIFOBufferStream::FIFOBufferStream(char* pBuffer, std::size_t length)
    : FIFOIOS(pBuffer, length), std::iostream(&_buf), readable(_buf.fifoBuffer().readable), writable(_buf.fifoBuffer().writable) {}
...
FIFOBufferStream::FIFOBufferStream(std::size_t length)
    : FIFOIOS(length), std::iostream(&_buf), readable(_buf.fifoBuffer().readable), writable(_buf.fifoBuffer().writable) {}
