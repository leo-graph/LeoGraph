...
Channel* Logger::getChannel() const { return _pChannel; }

void Logger::setLevel(int level) { _level = level; }

void Logger::setLevel(const std::string& level) { setLevel(parseLevel(level)); }
...
void Logger::log(const Exception& exc) { error(exc.displayText()); }

void Logger::log(const Exception& exc, const char* file, int line) { error(exc.displayText(), file, line); }
...
inline LoggerPtr makeLoggerPtr(Logger& logger, bool owned_by_shared_ptr) {
  if (owned_by_shared_ptr) return LoggerPtr(&logger, LoggerDeleter());
...
LoggerPtr Logger::getShared(const std::string& name, bool should_be_owned_by_shared_ptr_if_created) {
   ...
   */
  if (inserted && should_be_owned_by_shared_ptr_if_created) it->second.owned_by_shared_ptr = true;
...
std::pair<Logger::LoggerMapIterator, bool> Logger::unsafeGet(const std::string& name, bool get_shared) {
  ...
  if (optional_logger_it) {
    ...
    if (logger_it->second.owned_by_shared_ptr) {
      ...
      if (!get_shared) logger_it->second.owned_by_shared_ptr = false;
    }
  }
...
Logger* Logger::unsafeGetRawPtr(const std::string& name) { return unsafeGet(name, false /*get_shared*/).first->second.logger; }
...
void Logger::shutdown() {
  ...
  if (_pLoggerMap) {
    for (auto& it : *_pLoggerMap) {
      if (it.second.owned_by_shared_ptr) continue;
...
std::optional<Logger::LoggerMapIterator> Logger::find(const std::string& name) {
  if (_pLoggerMap) {
    ...
    if (it != _pLoggerMap->end()) return it;
...
std::pair<Logger::LoggerMapIterator, bool> Logger::add(Logger* pLogger) {
  if (!_pLoggerMap) _pLoggerMap = new Logger::LoggerMap;
