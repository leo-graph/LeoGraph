...
VarIterator::VarIterator(Var* pVar, bool positionEnd) : _pVar(pVar), _position(positionEnd ? POSITION_END : 0) {}

VarIterator::VarIterator(const VarIterator& other) : _pVar(other._pVar), _position(other._position) {}
...
void VarIterator::increment() const {
  if (POSITION_END == _position) throw RangeException("End of iterator reached.");
...
Var& VarIterator::operator*() const {
  if (POSITION_END == _position) throw InvalidAccessException("End of iterator reached.");
