...
FileStreamBuf::FileStreamBuf() : BufferedBidirectionalStreamBuf(BUFFER_SIZE, std::ios::in | std::ios::out), _fd(-1), _pos(0) {}

FileStreamBuf::~FileStreamBuf() { close(); }
...
void FileStreamBuf::open(const std::string& path, std::ios::openmode mode) {
  ...
  if (_fd == -1) File::handleLastError(_path);
...
int FileStreamBuf::readFromDevice(char* buffer, std::streamsize length) {
  ...
  if (n == -1) File::handleLastError(_path);
  _pos += n;
  return n;
}
...
std::streampos FileStreamBuf::seekoff(std::streamoff off, std::ios::seekdir dir, std::ios::openmode mode) {
  if (_fd == -1 || !(getMode() & mode)) return -1;
