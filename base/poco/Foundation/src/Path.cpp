...
Path::Path(const std::string& path) { assign(path); }

Path::Path(const std::string& path, Style style) { assign(path, style); }
...
Path::Path(const Path& path)
    : _node(path._node), _device(path._device), _name(path._name), _version(path._version), _dirs(path._dirs), _absolute(path._absolute) {}
...
Path& Path::operator=(const std::string& path) { return assign(path); }
...
Path& Path::assign(const char* path) { return assign(std::string(path)); }

std::string Path::toString() const { return buildUnix(); }
...
Path& Path::resolve(const Path& path) {
  if (path.isAbsolute()) {
    ...
    for (int i = 0; i < path.depth(); ++i) pushDirectory(path[i]);
    _name = path._name;
  }
  return *this;
}
...
std::string Path::current() { return PathImpl::currentImpl(); }

std::string Path::home() { return PathImpl::homeImpl(); }
...
std::string Path::temp() { return PathImpl::tempImpl(); }
...
std::string Path::null() { return PathImpl::nullImpl(); }

std::string Path::expand(const std::string& path) { return PathImpl::expandImpl(path); }

void Path::listRoots(std::vector<std::string>& roots) { PathImpl::listRootsImpl(roots); }
...
void Path::parseWindows(const std::string& path) {
  ...
  if (!_node.empty() && _dirs.empty() && !_name.empty()) makeDirectory();
}
          ...
          if (it != end) {
            ...
            if (it != end && *it == '[') {
              ...
              while (it != end && *it != ']') {
                ...
                if (!name.empty()) {
                  if (name == "-") {
                    if (_dirs.size() > d) _dirs.pop_back();
                  } else
                    _dirs.push_back(name);
                }
                if (it != end && *it != ']') ++it;
...
std::string Path::buildVMS() const {
  ...
  if (!_dirs.empty()) {
    result.append("[");
    if (!_absolute && _dirs[0] != "..") result.append(".");
    for (StringVec::const_iterator it = _dirs.begin(); it != _dirs.end(); ++it) {
      if (it != _dirs.begin() && *it != "..") result.append(".");
      if (*it == "..")
        result.append("-");
      else
        result.append(*it);
...
std::string Path::transcode(const std::string& path) { return path; }
