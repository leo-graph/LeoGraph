...
#  if (defined(__linux__) || defined(__QNX__)) && \
      !(defined(__ANDROID__) && (defined(HAVE_PTHREAD_COND_TIMEDWAIT_MONOTONIC) || __ANDROID_API__ <= 21))
...
SemaphoreImpl::SemaphoreImpl(int n, int max) : _n(n), _max(max) {
  ...
  if (pthread_mutex_init(&_mutex, NULL)) throw SystemException("cannot create semaphore (mutex)");
...
void SemaphoreImpl::waitImpl() {
  if (pthread_mutex_lock(&_mutex)) throw SystemException("wait for semaphore failed (lock)");
  while (_n < 1) {
...
bool SemaphoreImpl::waitImpl(long milliseconds) {
  ...
  if (pthread_mutex_lock(&_mutex) != 0) throw SystemException("wait for semaphore failed (lock)");
  while (_n < 1) {
