...
FileImpl::FileImpl(const std::string& path) : _path(path) {
  ...
  if (n > 1 && _path[n - 1] == '/') _path.resize(n - 1);
}

FileImpl::~FileImpl() {}

void FileImpl::swapImpl(FileImpl& file) { std::swap(_path, file._path); }
...
Timestamp FileImpl::createdImpl() const {
  ...
  struct stat64 st;
  if (stat64(_path.c_str(), &st) == 0) return Timestamp::fromEpochTime(st.st_birthtime);
#elif defined(__FreeBSD__)
  struct stat st;
  if (stat(_path.c_str(), &st) == 0) return Timestamp::fromEpochTime(st.st_birthtime);
#else
  struct stat st;
  if (stat(_path.c_str(), &st) == 0) return Timestamp::fromEpochTime(st.st_ctime);
#endif
  else
    handleLastErrorImpl(_path);
  return 0;
...
void FileImpl::setLastModifiedImpl(const Timestamp& ts) {
  ...
  if (utime(_path.c_str(), &tb) != 0) handleLastErrorImpl(_path);
}
...
void FileImpl::setSizeImpl(FileSizeImpl size) {
  ...
  if (truncate(_path.c_str(), size) != 0) handleLastErrorImpl(_path);
}
...
void FileImpl::setWriteableImpl(bool flag) {
  ...
  struct stat st;
  if (stat(_path.c_str(), &st) != 0) handleLastErrorImpl(_path);
  mode_t mode;
  if (flag) {
  ...
  if (chmod(_path.c_str(), mode) != 0) handleLastErrorImpl(_path);
}
...
void FileImpl::setExecutableImpl(bool flag) {
  ...
  if (flag) {
    ...
    if (st.st_mode & S_IRGRP) mode |= S_IXGRP;
    if (st.st_mode & S_IROTH) mode |= S_IXOTH;
  } else {
...
void FileImpl::copyToImpl(const std::string& path) const {
  ...
  try {
    ...
    while ((n = read(sd, buffer.begin(), blockSize)) > 0) {
      if (write(dd, buffer.begin(), n) != n) handleLastErrorImpl(path);
    }
    if (n < 0) handleLastErrorImpl(_path);
  } catch (...) {
    close(sd);
    close(dd);
    throw;
  ...
  if (close(dd) != 0) handleLastErrorImpl(path);
}

void FileImpl::renameToImpl(const std::string& path) {
  poco_assert(!_path.empty());

  if (rename(_path.c_str(), path.c_str()) != 0) handleLastErrorImpl(_path);
}
...
void FileImpl::linkToImpl(const std::string& path, int type) const {
  ...
  if (type == 0) {
    if (link(_path.c_str(), path.c_str()) != 0) handleLastErrorImpl(_path);
  } else {
    if (symlink(_path.c_str(), path.c_str()) != 0) handleLastErrorImpl(_path);
  }
}
...
bool FileImpl::createDirectoryImpl() {
  ...
  if (existsImpl() && isDirectoryImpl()) return false;
  if (mkdir(_path.c_str(), S_IRWXU | S_IRWXG | S_IRWXO) != 0) {
    if (errno == EEXIST && isDirectoryImpl()) return false;
    handleLastErrorImpl(_path);
  }
  return true;
}
...
FileImpl::FileSizeImpl FileImpl::totalSpaceImpl() const {
  ...
  if (STATFSFN(const_cast<char*>(_path.c_str()), &stats) != 0) handleLastErrorImpl(_path);
