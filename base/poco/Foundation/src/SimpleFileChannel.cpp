...
SimpleFileChannel::SimpleFileChannel() : _limit(0), _flush(true), _pFile(0) {}

SimpleFileChannel::SimpleFileChannel(const std::string& path)
    : _path(path), _secondaryPath(path + ".0"), _limit(0), _flush(true), _pFile(0) {}
...
void SimpleFileChannel::setProperty(const std::string& name, const std::string& value) {
  ...
  if (name == PROP_PATH) {
    _path = value;
    if (_secondaryPath.empty()) _secondaryPath = _path + ".0";
...
const std::string& SimpleFileChannel::path() const { return _path; }

const std::string& SimpleFileChannel::secondaryPath() const { return _secondaryPath; }
...
void SimpleFileChannel::setFlush(const std::string& flush) { _flush = icompare(flush, "true") == 0; }
