...
/// Used only internally.
{
  ...
  for (; it != str.begin();) {
    ...
    if (++thCount == 3 && std::isdigit(prevChr)) it = str.insert(pos, thSep);
...
void floatToFixedStr(char* buffer, int bufferSize, float value, int precision) {
  ...
  StringBuilder builder(buffer, bufferSize);
  int flags = DoubleToStringConverter::UNIQUE_ZERO | DoubleToStringConverter::EMIT_POSITIVE_EXPONENT_SIGN;
  DoubleToStringConverter dc(flags, POCO_FLT_INF, POCO_FLT_NAN, POCO_FLT_EXP, -std::numeric_limits<float>::digits10,
...
std::string& floatToStr(std::string& str, float value, int precision, int width, char thSep, char decSep) {
  ...
  if (decSep && (decSep != '.') && (str.find('.') != std::string::npos)) replaceInPlace(str, '.', decSep);
...
void doubleToFixedStr(char* buffer, int bufferSize, double value, int precision) {
  ...
  StringBuilder builder(buffer, bufferSize);
  int flags = DoubleToStringConverter::UNIQUE_ZERO | DoubleToStringConverter::EMIT_POSITIVE_EXPONENT_SIGN;
  DoubleToStringConverter dc(flags, POCO_FLT_INF, POCO_FLT_NAN, POCO_FLT_EXP, -std::numeric_limits<double>::digits10,
...
bool strToFloat(const std::string& str, float& result, char decSep, char thSep) {
  ...
  return !FPEnvironment::isInfinite(result) && !FPEnvironment::isNaN(result);
}
