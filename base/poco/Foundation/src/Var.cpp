...
#else
{
  if ((this != &other) && !other.isEmpty()) construct(other);
}
#endif
...
Var& Var::operator++() {
  if (!isInteger()) throw InvalidArgumentException("Invalid operation for this data type.");
...
Var& Var::getAt(std::size_t n) {
  if (isVector())
    return holderImpl<std::vector<Var>, InvalidAccessException>("Not a vector.")->operator[](n);
  else if (isList())
    return holderImpl<std::list<Var>, InvalidAccessException>("Not a list.")->operator[](n);
  else if (isDeque())
    return holderImpl<std::deque<Var>, InvalidAccessException>("Not a deque.")->operator[](n);
  else if (isStruct())
    return structIndexOperator(holderImpl<Struct<int>, InvalidAccessException>("Not a struct."), static_cast<int>(n));
...
char& Var::at(std::size_t n) {
  if (isString()) {
    return holderImpl<std::string, InvalidAccessException>("Not a string.")->operator[](n);
  }

  throw InvalidAccessException("Not a string.");
}

Var& Var::getAt(const std::string& name) { return holderImpl<DynamicStruct, InvalidAccessException>("Not a struct.")->operator[](name); }
...
Var Var::parseObject(const std::string& val, std::string::size_type& pos) {
  ...
  while (val[pos] != '}' && pos < val.size()) {
    ...
    skipWhiteSpace(val, pos);
    if (val[pos] != ':') throw DataFormatException("Incorrect object, must contain: key : value pairs");
    ++pos;  // skip past :
  ...
  if (val[pos] != '}') throw DataFormatException("Unterminated object");
  ++pos;
  return aStruct;
}
...
Var Var::parseArray(const std::string& val, std::string::size_type& pos) {
  ...
  if (val[pos] != ']') throw DataFormatException("Unterminated array");
  ++pos;
  return result;
}
...
std::string Var::parseString(const std::string& val, std::string::size_type& pos) {
  ...
  if (val[pos] == '"') {
    ...
    std::string result;
    while (pos < val.size() && !Poco::Ascii::isSpace(val[pos]) && val[pos] != ',' && val[pos] != ']' && val[pos] != '}') {
      result += val[pos++];
    }
    return result;
  }
...
void Var::skipWhiteSpace(const std::string& val, std::string::size_type& pos) {
  poco_assert_dbg(pos < val.size());
  while (std::isspace(val[pos]) && pos < val.size()) ++pos;
}
...
Var& Var::structIndexOperator(VarHolderImpl<Struct<int> >* pStr, int n) const { return pStr->operator[](n); }
