...
void MD5Engine::updateImpl(const void* input_, std::size_t inputLen) {
  ...
  if ((_context.count[0] += ((UInt32)inputLen << 3)) < ((UInt32)inputLen << 3)) _context.count[1]++;
  ...
  if (inputLen >= partLen) {
    ...
    for (i = partLen; i + 63 < inputLen; i += 64) transform(_context.state, &input[i]);
...
std::size_t MD5Engine::digestLength() const { return DIGEST_SIZE; }
