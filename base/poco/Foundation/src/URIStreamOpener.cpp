...
URIStreamOpener::URIStreamOpener() { registerStreamFactory("file", new FileStreamFactory); }

URIStreamOpener::~URIStreamOpener() {
  for (FactoryMap::iterator it = _map.begin(); it != _map.end(); ++it) delete it->second;
}
...
void URIStreamOpener::registerStreamFactory(const std::string& scheme, URIStreamFactory* pFactory) {
  ...
  if (_map.find(scheme) == _map.end()) {
    ...
    throw ExistsException("An URIStreamFactory for the given scheme has already been registered", scheme);
}
...
void URIStreamOpener::unregisterStreamFactory(const std::string& scheme) {
  ...
  if (it != _map.end()) {
    ...
    throw NotFoundException("No URIStreamFactory has been registered for the given scheme", scheme);
}
...
URIStreamOpener& URIStreamOpener::defaultOpener() { return *sh.get(); }
