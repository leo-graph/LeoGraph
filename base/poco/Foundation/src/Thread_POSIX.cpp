...
void ThreadImpl::setPriorityImpl(int prio) {
  if (prio != _pData->prio) {
    ...
    if (isRunningImpl()) {
      ...
      if (pthread_setschedparam(_pData->thread, SCHED_OTHER, &par)) throw SystemException("cannot set thread priority");
    }
  }
}
...
void ThreadImpl::setOSPriorityImpl(int prio, int policy) {
  if (prio != _pData->osPrio || policy != _pData->policy) {
    if (_pData->pRunnableTarget) {
      ...
      if (pthread_setschedparam(_pData->thread, policy, &par)) throw SystemException("cannot set thread priority");
    }
...
void ThreadImpl::startImpl(SharedPtr<Runnable> pTarget) {
  ...
  if (_pData->policy == SCHED_OTHER) {
    ...
    if (pthread_setschedparam(_pData->thread, _pData->policy, &par)) throw SystemException("cannot set thread priority");
  }
}
...
void ThreadImpl::joinImpl() {
  ...
  _pData->done.wait();
  void* result;
  if (pthread_join(_pData->thread, &result)) throw SystemException("cannot join thread");
  _pData->joined = true;
}
...
ThreadImpl* ThreadImpl::currentImpl() { return _currentThreadHolder.get(); }

ThreadImpl::TIDImpl ThreadImpl::currentTidImpl() { return pthread_self(); }
...
int ThreadImpl::reverseMapPrio(int prio, int policy) {
  if (policy == SCHED_OTHER) {
    ...
    if (prio == pmax) return PRIO_HIGHEST_IMPL;
    if (prio > normal)
