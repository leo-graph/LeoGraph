...
UUID::UUID() : _timeLow(0), _timeMid(0), _timeHiAndVersion(0), _clockSeq(0) { std::memset(_node, 0, sizeof(_node)); }

UUID::UUID(const UUID& uuid)
    : _timeLow(uuid._timeLow), _timeMid(uuid._timeMid), _timeHiAndVersion(uuid._timeHiAndVersion), _clockSeq(uuid._clockSeq) {
...
UUID::UUID(UInt32 timeLow, UInt32 timeMid, UInt32 timeHiAndVersion, UInt16 clockSeq, UInt8 node[])
    : _timeLow(timeLow), _timeMid(timeMid), _timeHiAndVersion(timeHiAndVersion), _clockSeq(clockSeq) {
...
void UUID::parse(const std::string& uuid) {
  if (!tryParse(uuid)) throw SyntaxException(uuid);
}
...
std::string UUID::toString() const {
  ...
  result += '-';
  for (int i = 0; i < sizeof(_node); ++i) appendHex(result, _node[i]);
  return result;
}
...
const UUID& UUID::null() { return uuidNull; }

const UUID& UUID::dns() { return uuidDNS; }

const UUID& UUID::uri() { return uuidURI; }

const UUID& UUID::oid() { return uuidOID; }

const UUID& UUID::x500() { return uuidX500; }
