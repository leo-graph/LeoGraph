...
SyslogChannel::SyslogChannel() : _options(SYSLOG_CONS), _facility(SYSLOG_USER), _open(false) {}

SyslogChannel::SyslogChannel(const std::string& name, int options, int facility)
    : _name(name), _options(options), _facility(facility), _open(false) {}

SyslogChannel::~SyslogChannel() { close(); }
