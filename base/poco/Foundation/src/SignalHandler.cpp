...
SignalHandler::~SignalHandler() { jumpBufferVec().pop_back(); }

sigjmp_buf& SignalHandler::jumpBuffer() { return jumpBufferVec().back().buf; }
...
void SignalHandler::handleSignal(int sig) {
  ...
  if (!jb.empty()) siglongjmp(jb.back().buf, sig);
