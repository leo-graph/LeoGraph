...
LineEndingConverterStreamBuf::LineEndingConverterStreamBuf(std::istream& istr)
    : _pIstr(&istr), _pOstr(0), _newLine(LineEnding::NEWLINE_DEFAULT), _lastChar(0) {
  _it = _newLine.end();
}

LineEndingConverterStreamBuf::LineEndingConverterStreamBuf(std::ostream& ostr)
    : _pIstr(0), _pOstr(&ostr), _newLine(LineEnding::NEWLINE_DEFAULT), _lastChar(0) {
  _it = _newLine.end();
}
...
const std::string& LineEndingConverterStreamBuf::getNewLine() const { return _newLine; }
...
int LineEndingConverterStreamBuf::writeToDevice(char c) {
  ...
  if (c == '\r' || (c == '\n' && _lastChar != '\r')) _pOstr->write(_newLine.data(), (std::streamsize)_newLine.length());
  if (c != '\n' && c != '\r') _pOstr->put(c);
  _lastChar = c;
  return charToInt(c);
}
...
LineEndingConverterIOS::LineEndingConverterIOS(std::ostream& ostr) : _buf(ostr) { poco_ios_init(&_buf); }

LineEndingConverterIOS::~LineEndingConverterIOS() {}

void LineEndingConverterIOS::setNewLine(const std::string& newLineCharacters) { _buf.setNewLine(newLineCharacters); }

const std::string& LineEndingConverterIOS::getNewLine() const { return _buf.getNewLine(); }

LineEndingConverterStreamBuf* LineEndingConverterIOS::rdbuf() { return &_buf; }

InputLineEndingConverter::InputLineEndingConverter(std::istream& istr) : LineEndingConverterIOS(istr), std::istream(&_buf) {
  poco_ios_init(&_buf);
}

InputLineEndingConverter::InputLineEndingConverter(std::istream& istr, const std::string& newLineCharacters)
    : LineEndingConverterIOS(istr), std::istream(&_buf) {
  setNewLine(newLineCharacters);
}

InputLineEndingConverter::~InputLineEndingConverter() {}

OutputLineEndingConverter::OutputLineEndingConverter(std::ostream& ostr) : LineEndingConverterIOS(ostr), std::ostream(&_buf) {
  poco_ios_init(&_buf);
}

OutputLineEndingConverter::OutputLineEndingConverter(std::ostream& ostr, const std::string& newLineCharacters)
    : LineEndingConverterIOS(ostr), std::ostream(&_buf) {
  setNewLine(newLineCharacters);
}
