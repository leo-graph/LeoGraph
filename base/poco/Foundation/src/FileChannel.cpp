...
void FileChannel::setProperty(const std::string& name, const std::string& value) {
  ...
  if (name == PROP_TIMES) {
    ...
    if (!_rotation.empty()) setRotation(_rotation);

    if (!_archive.empty()) setArchive(_archive);
...
const std::string& FileChannel::path() const { return _path; }
...
void FileChannel::setCompress(const std::string& compress) {
  ...
  if (_pArchiveStrategy) _pArchiveStrategy->compress(_compress && !_streamCompress);
}
...
void FileChannel::setFlush(const std::string& flush) { _flush = icompare(flush, "true") == 0; }

void FileChannel::setRotateOnOpen(const std::string& rotateOnOpen) { _rotateOnOpen = icompare(rotateOnOpen, "true") == 0; }
...
/// always has the number zero.
{
  ...
  while (n >= 0) {
    ...
    newPath.append(ext);
    f = oldPath;
    if (f.exists()) f.renameTo(newPath);
    --n;
  }
}
  ...
  if (file.exists()) {
    ...
    if (size > 0) {
      ...
      if (size < 4) {
        ...
        if (istr.gcount() != size) err = true;

        if (std::memcmp(buffer.begin(), "\0\0\0\0", 4) != 0) err = true;
      }
...
int FileChannel::extractDigit(const std::string& value, std::string::const_iterator* nextToDigit) const {
  ...
  if (digit == 0) throw InvalidArgumentException("Zero is not valid purge age.");
...
Timespan::TimeDiff FileChannel::extractFactor(const std::string& value, std::string::const_iterator start) const {
  ...
  if (unit == "seconds") return Timespan::SECONDS;
  if (unit == "minutes")
