...
const char* UTF16Encoding::_names[] = {"UTF-16", "UTF16", NULL};
...
UTF16Encoding::UTF16Encoding(ByteOrderType byteOrder) { setByteOrder(byteOrder); }

UTF16Encoding::UTF16Encoding(int byteOrderMark) { setByteOrder(byteOrderMark); }
...
void UTF16Encoding::setByteOrder(int byteOrderMark) { _flipBytes = byteOrderMark != 0xFEFF; }

const char* UTF16Encoding::canonicalName() const { return _names[0]; }
...
const TextEncoding::CharacterMap& UTF16Encoding::characterMap() const { return _charMap; }
...
int UTF16Encoding::queryConvert(const unsigned char* bytes, int length) const {
  ...
  if (length >= 2) {
    ...
    *p++ = *bytes++;
    *p++ = *bytes++;
    if (_flipBytes) ByteOrder::flipBytes(uc);
    if (uc >= 0xd800 && uc < 0xdc00) {
      if (length >= 4) {
        UInt16 uc2;
        p = (unsigned char*)&uc2;
        *p++ = *bytes++;
        *p++ = *bytes++;
        if (_flipBytes) ByteOrder::flipBytes(uc2);
