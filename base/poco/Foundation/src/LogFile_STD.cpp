...
LogFileImpl::LogFileImpl(const std::string& path) : _path(path), _str(_path, std::ios::app) {
  if (sizeImpl() == 0)
...
UInt64 LogFileImpl::sizeImpl() const {
  if (!_str.good()) throw FileException(_path);
  return (UInt64)_str.tellp();
}

Timestamp LogFileImpl::creationDateImpl() const { return _creationDate; }

const std::string& LogFileImpl::pathImpl() const { return _path; }
