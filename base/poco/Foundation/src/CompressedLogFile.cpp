...
CompressedLogFile::CompressedLogFile(const std::string &path) : LogFile(path + ".lz4"), _buffer(0) {
  memset(&_kPrefs, 0, sizeof(_kPrefs));

  size_t ret = LZ4F_createCompressionContext(&_ctx, LZ4F_VERSION);

  if (LZ4F_isError(ret)) throw IOException(LZ4F_getErrorName(ret));
  ...
  if (LZ4F_isError(header_size)) throw IOException(LZ4F_getErrorName(header_size));
...
CompressedLogFile::~CompressedLogFile() {
  ...
  if (!LZ4F_isError(end_size)) writeBinary(_buffer.begin(), end_size, true);
...
void CompressedLogFile::write(const std::string &text, bool flush) {
  ...
  while (in_capacity > 0) {
    ...
    while (_buffer.capacity() < LZ4F_compressBound(cur_buffer_size, &_kPrefs)) cur_buffer_size /= 2;

    size_t compressed_size = LZ4F_compressUpdate(_ctx, static_cast<void *>(_buffer.begin()), _buffer.capacity(),
                                                 static_cast<const void *>(in_data), cur_buffer_size, nullptr);

    if (LZ4F_isError(compressed_size)) throw IOException(LZ4F_getErrorName(compressed_size));

    if (compressed_size > 0) writeBinary(_buffer.begin(), compressed_size, false);
  ...
  size_t compressed_size =
      LZ4F_compressUpdate(_ctx, static_cast<void *>(_buffer.begin()), _buffer.capacity(), &newline, sizeof(newline), nullptr);
  ...
  if (compressed_size > 0) writeBinary(_buffer.begin(), compressed_size, flush);
}
