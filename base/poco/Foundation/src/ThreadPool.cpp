...
class PooledThread : public Runnable {
 public:
  explicit PooledThread(const std::string& name, int stackSize = POCO_THREAD_STACK_SIZE, size_t globalProfilerRealTimePeriodNs_ = 0,
...
PooledThread::PooledThread(const std::string& name, int stackSize, size_t globalProfilerRealTimePeriodNs_,
...
void PooledThread::join() {
  ...
  _mutex.unlock();
  if (pTarget) _targetCompleted.wait();
}
...
void PooledThread::release() {
  ...
  if (_thread.isRunning()) _targetReady.set();
...
ThreadPool::ThreadPool(int minCapacity, int maxCapacity, int idleTime, int stackSize, size_t globalProfilerRealTimePeriodNs_,
                       size_t globalProfilerCPUTimePeriodNs_)
...
ThreadPool::ThreadPool(const std::string& name, int minCapacity, int maxCapacity, int idleTime, int stackSize,
                       size_t globalProfilerRealTimePeriodNs_, size_t globalProfilerCPUTimePeriodNs_)
    : _name(name),
      _minCapacity(minCapacity),
      _maxCapacity(maxCapacity),
      _idleTime(idleTime),
...
void ThreadPool::start(Runnable& target, const std::string& name) { getThread()->start(Thread::PRIO_NORMAL, target, name); }

void ThreadPool::startWithPriority(Thread::Priority priority, Runnable& target) { getThread()->start(priority, target); }
 ...
 public:
  ...
  ThreadPool* pool() {
    ...
    if (!_pPool) {
      ...
      if (POCO_THREAD_STACK_SIZE > 0) _pPool->setStackSize(POCO_THREAD_STACK_SIZE);
    }
    return _pPool;
  }
