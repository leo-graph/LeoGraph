...
Context::Context(Usage usage, const Params& params)
    : _usage(usage), _mode(params.verificationMode), _pSSLContext(0), _extendedCertificateVerification(true) {
  init(params);
}

Context::Context(Usage usage, const std::string& privateKeyFile, const std::string& certificateFile, const std::string& caLocation,
                 VerificationMode verificationMode, int verificationDepth, bool loadDefaultCAs, const std::string& cipherList)
    : _usage(usage), _mode(verificationMode), _pSSLContext(0), _extendedCertificateVerification(true) {
  Params params;
...
Context::Context(Usage usage, const std::string& caLocation, VerificationMode verificationMode, int verificationDepth, bool loadDefaultCAs,
...
static bool poco_dir_contains_certs(const std::string& dir) {
  ...
  try {
    for (DirectoryIterator it(dir), end; it != end; ++it)
      if (re.match(Path(it->path()).getFileName())) return true;
...
void Context::init(const Params& params) {
  ...
  try {
    ...
    if (params.loadDefaultCAs) {
      ...
      if (!file) file = X509_get_default_cert_file();
      ...
      if (poco_file_cert(file)) {
        ...
        if (poco_dir_cert(dir)) {
          errCode = 0;
          if (!poco_dir_contains_certs(dir)) errCode = poco_ssl_probe_and_set_default_ca_location(_pSSLContext, _caPaths);
...
bool Context::sessionCacheEnabled() const { return SSL_CTX_get_session_cache_mode(_pSSLContext) != SSL_SESS_CACHE_OFF; }
...
void Context::enableExtendedCertificateVerification(bool flag) { _extendedCertificateVerification = flag; }
...
const Context::CAPaths& Context::getCAPaths() { return _caPaths; }
...
void Context::initDH(const std::string& dhParamsFile) {
  ...
  static const unsigned char dh1024_p[] = {
      ,
  };

  static const unsigned char dh1024_g[] = {
      ,
  };
...
#  endif
  ...
  DH_free(dh);
#else
  if (!dhParamsFile.empty()) throw SSLContextException("OpenSSL does not support DH");
#endif
}
