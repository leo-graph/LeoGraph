...
SSLManager& SSLManager::instance() { return *singleton.get(); }

void SSLManager::initializeServer(PrivateKeyPassphraseHandlerPtr ptrPassphraseHandler, InvalidCertificateHandlerPtr ptrHandler,
...
void SSLManager::initializeClient(PrivateKeyPassphraseHandlerPtr ptrPassphraseHandler, InvalidCertificateHandlerPtr ptrHandler,
                                  Context::Ptr ptrContext) {
...
Context::Ptr SSLManager::defaultServerContext() {
  ...
  if (!_ptrDefaultServerContext) initDefaultContext(true);
...
SSLManager::PrivateKeyPassphraseHandlerPtr SSLManager::serverPassphraseHandler() {
  ...
  if (!_ptrServerPassphraseHandler) initPassphraseHandler(true);
...
SSLManager::PrivateKeyPassphraseHandlerPtr SSLManager::clientPassphraseHandler() {
  ...
  if (!_ptrClientPassphraseHandler) initPassphraseHandler(false);
...
SSLManager::InvalidCertificateHandlerPtr SSLManager::serverCertificateHandler() {
  ...
  if (!_ptrServerCertificateHandler) initCertificateHandler(true);
...
SSLManager::InvalidCertificateHandlerPtr SSLManager::clientCertificateHandler() {
  ...
  if (!_ptrClientCertificateHandler) initCertificateHandler(false);
...
int SSLManager::privateKeyPassphraseCallback(char* pBuf, int size, int, void*) {
  ...
  pBuf[size - 1] = '\0';
  if (size > pwd.length()) size = (int)pwd.length();
...
void SSLManager::initPassphraseHandler(bool server) {
  ...
  if (pFactory) {
    ...
    throw Poco::Util::UnknownOptionException(std::string("No passphrase handler known with the name ") + className);
}
...
void SSLManager::initCertificateHandler(bool server) {
  ...
  if (pFactory) {
    ...
    throw Poco::Util::UnknownOptionException(std::string("No InvalidCertificate handler known with the name ") + className);
}

Context::Ptr SSLManager::getCustomServerContext(const std::string& name) {
  Poco::FastMutex::ScopedLock lock(_mutex);
  auto it = _mapPtrServerContexts.find(name);
  if (it != _mapPtrServerContexts.end()) return it->second;
  return nullptr;
}
...
void uninitializeSSL() { SSLManager::instance().shutdown(); }
