...
HTTPSClientSession::HTTPSClientSession()
    : HTTPClientSession(SecureStreamSocket()), _pContext(SSLManager::instance().defaultClientContext()) {
  setPort(HTTPS_PORT);
  _proxySessionFactory.registerProtocol("https", new HTTPSSessionInstantiator);
}

HTTPSClientSession::HTTPSClientSession(const SecureStreamSocket& socket) : HTTPClientSession(socket), _pContext(socket.context()) {
  setPort(HTTPS_PORT);
  _proxySessionFactory.registerProtocol("https", new HTTPSSessionInstantiator);
}

HTTPSClientSession::HTTPSClientSession(const SecureStreamSocket& socket, Session::Ptr pSession)
    : HTTPClientSession(socket), _pContext(socket.context()), _pSession(pSession) {
  setPort(HTTPS_PORT);
  _proxySessionFactory.registerProtocol("https", new HTTPSSessionInstantiator);
}
...
HTTPSClientSession::HTTPSClientSession(Context::Ptr pContext, Session::Ptr pSession)
    : HTTPClientSession(SecureStreamSocket(pContext, pSession)), _pContext(pContext), _pSession(pSession) {
  _proxySessionFactory.registerProtocol("https", new HTTPSSessionInstantiator(pContext));
}

HTTPSClientSession::HTTPSClientSession(const std::string& host, Poco::UInt16 port, Context::Ptr pContext)
    : HTTPClientSession(SecureStreamSocket(pContext)), _pContext(pContext) {
  setHost(host);
  setPort(port);
  _proxySessionFactory.registerProtocol("https", new HTTPSSessionInstantiator(pContext));
}
...
HTTPSClientSession::~HTTPSClientSession() { _proxySessionFactory.unregisterProtocol("https"); }

bool HTTPSClientSession::secure() const { return true; }
...
Session::Ptr HTTPSClientSession::sslSession() { return _pSession; }
