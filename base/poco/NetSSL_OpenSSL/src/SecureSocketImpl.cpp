...
SecureSocketImpl::SecureSocketImpl(Poco::AutoPtr<SocketImpl> pSocketImpl, Context::Ptr pContext)
    : _pSSL(nullptr), _pSocket(pSocketImpl), _pContext(pContext), _needHandshake(false) {
...
SocketImpl* SecureSocketImpl::acceptConnection(SocketAddress& clientAddr) {
  ...
  Poco::AutoPtr<SecureStreamSocketImpl> pSecureStreamSocketImpl =
      new SecureStreamSocketImpl(static_cast<StreamSocketImpl*>(ss.impl()), _pContext);
...
int SecureSocketImpl::receiveBytes(void* buffer, int length, int flags) {
  ...
  if ((flags & MSG_DONTWAIT) && (flags & MSG_PEEK)) return _pSocket->receiveBytes(buffer, length, flags);
...
Poco::Timespan SecureSocketImpl::getMaxTimeoutOrLimit() {
  ...
  if (remaining_time < send_timeout) remaining_time = send_timeout;
  ...
  if (remaining_time == 0) remaining_time = Poco::Timespan(std::numeric_limits<int>::max());
...
void SecureSocketImpl::setBlocking(bool flag) { _pSocket->setBlocking(flag); }

bool SecureSocketImpl::getBlocking() const { return _pSocket->getBlocking(); }
