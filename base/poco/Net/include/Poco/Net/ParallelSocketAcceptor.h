...
/// details.
{
 public:
  ...
  explicit ParallelSocketAcceptor(ServerSocket &socket, unsigned threads = Poco::Environment::processorCount())
      : _socket(socket),
        _pReactor(0),
        _threads(threads),
        _next(0)
 ...
 protected:
  ...
  /// Populates the reactors vector.
  {
    ...
    for (unsigned i = 0; i < _threads; ++i) _reactors.push_back(new ParallelReactor);
  }
