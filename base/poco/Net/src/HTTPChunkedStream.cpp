...
HTTPChunkedStreamBuf::HTTPChunkedStreamBuf(HTTPSession& session, openmode mode)
    : HTTPBasicStreamBuf(HTTP_DEFAULT_BUFFER_SIZE, mode), _session(session), _mode(mode), _chunk(0) {}
...
static inline int assertNonEOF(int c) {
  if (c == std::char_traits<char>::eof()) throw MessageException("Unexpected EOF");
  return c;
}

static inline bool isCRLF(char c1, char c2) { return c1 == '\r' && c2 == '\n'; }
...
unsigned int HTTPChunkedStreamBuf::parseChunkLen() {
  ...
  if (size_t pos = line.find(';'); pos != std::string::npos) line.resize(pos);
...
void HTTPChunkedStreamBuf::skipCRLF() {
  ...
  if (!isCRLF(c1, c2)) throw MessageException("Malformed chunked encoding");
}

int HTTPChunkedStreamBuf::readFromDevice(char* buffer, std::streamsize length) {
  static const int eof = std::char_traits<char>::eof();
  if (_chunk == eof) return 0;
...
HTTPChunkedIOS::HTTPChunkedIOS(HTTPSession& session, HTTPChunkedStreamBuf::openmode mode) : _buf(session, mode) { poco_ios_init(&_buf); }
...
HTTPChunkedStreamBuf* HTTPChunkedIOS::rdbuf() { return &_buf; }

//
// HTTPChunkedInputStream
//

HTTPChunkedInputStream::HTTPChunkedInputStream(HTTPSession& session) : HTTPChunkedIOS(session, std::ios::in), std::istream(&_buf) {
  poco_ios_init(&_buf);
}
...
HTTPChunkedOutputStream::HTTPChunkedOutputStream(HTTPSession& session) : HTTPChunkedIOS(session, std::ios::out), std::ostream(&_buf) {
  poco_ios_init(&_buf);
}
