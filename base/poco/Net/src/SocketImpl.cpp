...
SocketImpl::SocketImpl() : _sockfd(POCO_INVALID_SOCKET), _blocking(true), _recvThrottlerBudget(0), _sndThrottlerBudget(0) {}

SocketImpl::SocketImpl(poco_socket_t sockfd) : _sockfd(sockfd), _blocking(true), _recvThrottlerBudget(0), _sndThrottlerBudget(0) {}
...
void SocketImpl::connect(const SocketAddress& address, const Poco::Timespan& timeout) {
  ...
  try {
    ...
    if (rc != 0) {
      int err = lastError();
      if (err != POCO_EINPROGRESS && err != POCO_EWOULDBLOCK) error(err, address.toString());
      if (!poll(timeout, SELECT_READ | SELECT_WRITE | SELECT_ERROR)) throw Poco::TimeoutException("connect timed out", address.toString());
      err = socketError();
      if (err != 0) error(err);
    }
...
void SocketImpl::bind(const SocketAddress& address, bool reuseAddress) { bind(address, reuseAddress, reuseAddress); }
...
void SocketImpl::bind6(const SocketAddress& address, bool reuseAddress, bool ipV6Only) { bind6(address, reuseAddress, true, ipV6Only); }

void SocketImpl::bind6(const SocketAddress& address, bool reuseAddress, bool reusePort, bool ipV6Only) {
#if defined(POCO_HAVE_IPv6)
  if (address.family() != SocketAddress::IPv6) throw Poco::InvalidArgumentException("SocketAddress must be an IPv6 address");
...
int SocketImpl::sendBytes(const void* buffer, int length, int flags) {
  ...
  do {
    if (needPoll && remainingTime.totalMicroseconds() > 0) {
      if (!pollImpl(remainingTime, SELECT_WRITE)) throw TimeoutException();
    }
    ...
    if (rc < 0) err = lastError();
    if (blocking && rc < 0 && err == POCO_EINTR) {
      remainingTime -= Poco::Timestamp() - start;
      if (remainingTime.totalMicroseconds() <= 0) throw TimeoutException();
      needPoll = true;
    }
...
int SocketImpl::receiveBytes(void* buffer, int length, int flags) {
  ...
  do {
    if (needPoll && remainingTime.totalMicroseconds() > 0) {
      if (!pollImpl(remainingTime, SELECT_READ)) throw TimeoutException();
    }
...
void SocketImpl::setSendBufferSize(int size) { setOption(SOL_SOCKET, SO_SNDBUF, size); }
...
void SocketImpl::setReceiveBufferSize(int size) { setOption(SOL_SOCKET, SO_RCVBUF, size); }
...
Poco::Timespan SocketImpl::getSendTimeout() { return _sndTimeout; }
...
Poco::Timespan SocketImpl::getReceiveTimeout() { return _recvTimeout; }
...
Poco::Net::ThrottlerPtr SocketImpl::getSendThrottler() { return _sndThrottler; }
...
Poco::Net::ThrottlerPtr SocketImpl::getReceiveThrottler() { return _recvThrottler; }
...
void SocketImpl::setOption(int level, int option, int value) { setRawOption(level, option, &value, sizeof(value)); }

void SocketImpl::setOption(int level, int option, unsigned value) { setRawOption(level, option, &value, sizeof(value)); }

void SocketImpl::setOption(int level, int option, unsigned char value) { setRawOption(level, option, &value, sizeof(value)); }
...
void SocketImpl::reset(poco_socket_t aSocket) { _sockfd = aSocket; }
...
void SocketImpl::error(const std::string& arg) { error(lastError(), arg); }
