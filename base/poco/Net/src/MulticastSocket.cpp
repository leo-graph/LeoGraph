...
#  if defined(POCO_OS_FAMILY_UNIX)
  if (family == SocketAddress::UNIX_LOCAL) throw Poco::InvalidArgumentException("Cannot create a MulticastSocket with UNIX_LOCAL socket");
#  endif
}
...
void MulticastSocket::joinGroup(const IPAddress& groupAddress) { joinGroup(groupAddress, findFirstInterface(groupAddress)); }
...
NetworkInterface MulticastSocket::findFirstInterface(const IPAddress& groupAddress) {
  ...
  if (groupAddress.family() == IPAddress::IPv4) {
    for (NetworkInterface::Map::const_iterator it = m.begin(); it != m.end(); ++it) {
      if (it->second.supportsIPv4() && it->second.firstAddress(IPAddress::IPv4).isUnicast() && !it->second.isLoopback() &&
...
#  ifdef POCO_HAVE_IPv6
    ...
    for (NetworkInterface::Map::const_iterator it = m.begin(); it != m.end(); ++it) {
      if (it->second.supportsIPv6() && it->second.firstAddress(IPAddress::IPv6).isUnicast() && !it->second.isLoopback() &&
          !it->second.isPointToPoint()) {
        return it->second;
      }
    }
