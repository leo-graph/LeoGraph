...
MultipartWriter::MultipartWriter(std::ostream& ostr, const std::string& boundary) : _ostr(ostr), _boundary(boundary), _firstPart(true) {
  if (_boundary.empty()) _boundary = createBoundary();
}
...
const std::string& MultipartWriter::boundary() const { return _boundary; }
