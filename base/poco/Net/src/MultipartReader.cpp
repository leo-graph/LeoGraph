...
MultipartStreamBuf::MultipartStreamBuf(std::istream& istr, const std::string& boundary)
    : BufferedStreamBuf(STREAM_BUFFER_SIZE, std::ios::in), _istr(istr), _boundary(boundary), _lastPart(false) {}
...
bool MultipartStreamBuf::lastPart() const { return _lastPart; }
...
MultipartStreamBuf* MultipartIOS::rdbuf() { return &_buf; }

bool MultipartIOS::lastPart() const { return _buf.lastPart(); }
...
MultipartInputStream::MultipartInputStream(std::istream& istr, const std::string& boundary)
    : MultipartIOS(istr, boundary), std::istream(&_buf) {
  poco_ios_init(&_buf);
}
...
MultipartReader::MultipartReader(std::istream& istr) : _istr(istr) {}

MultipartReader::MultipartReader(std::istream& istr, const std::string& boundary) : _istr(istr), _boundary(boundary) {}
...
bool MultipartReader::hasNextPart() { return (!_pMPI || !_pMPI->lastPart()) && _istr.good(); }
...
const std::string& MultipartReader::boundary() const { return _boundary; }
...
void MultipartReader::guessBoundary() {
  static const int eof = std::char_traits<char>::eof();
  int ch = _istr.get();
  while (Poco::Ascii::isSpace(ch)) ch = _istr.get();
  if (ch == '-' && _istr.peek() == '-') {
    _istr.get();
    ch = _istr.peek();
