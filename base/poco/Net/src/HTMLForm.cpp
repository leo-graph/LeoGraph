...
class HTMLFormCountingOutputStream : public CountingOutputStream {
 public:
  HTMLFormCountingOutputStream() : _valid(true) {}
...
HTMLForm::HTMLForm() : _fieldLimit(DFL_FIELD_LIMIT), _valueLengthLimit(DFL_MAX_VALUE_LENGTH), _encoding(ENCODING_URL) {}

HTMLForm::HTMLForm(const std::string& encoding)
    : _fieldLimit(DFL_FIELD_LIMIT), _valueLengthLimit(DFL_MAX_VALUE_LENGTH), _encoding(encoding) {}

HTMLForm::HTMLForm(const HTTPRequest& request, std::istream& requestBody, PartHandler& handler)
    : _fieldLimit(DFL_FIELD_LIMIT), _valueLengthLimit(DFL_MAX_VALUE_LENGTH) {
...
HTMLForm::HTMLForm(const HTTPRequest& request) : _fieldLimit(DFL_FIELD_LIMIT), _valueLengthLimit(DFL_MAX_VALUE_LENGTH) { load(request); }
...
void HTMLForm::setEncoding(const std::string& encoding) { _encoding = encoding; }
...
void HTMLForm::read(std::istream& istr) { readUrl(istr); }
...
std::streamsize HTMLForm::calculateContentLength() {
  if (_encoding == ENCODING_MULTIPART && _boundary.empty()) throw HTMLFormException("Form must be prepared");
...
void HTMLForm::readUrl(std::istream& istr) {
  ...
  bool isFirst = true;
  while (ch != eof) {
    if (_fieldLimit > 0 && fields == _fieldLimit) throw HTMLFormException("Too many form fields");
    std::string name;
    std::string value;
    while (ch != eof && ch != '=' && ch != '&') {
      if (ch == '+') ch = ' ';
