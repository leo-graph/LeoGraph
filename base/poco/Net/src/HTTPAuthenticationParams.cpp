...
bool mustBeQuoted(const std::string& name) {
  return icompare(name, "cnonce") == 0 || icompare(name, "domain") == 0 || icompare(name, "nonce") == 0 || icompare(name, "opaque") == 0 ||
         icompare(name, "qop") == 0 || icompare(name, "realm") == 0 || icompare(name, "response") == 0 || icompare(name, "uri") == 0 ||
...
HTTPAuthenticationParams::HTTPAuthenticationParams(const std::string& authInfo) { fromAuthInfo(authInfo); }

HTTPAuthenticationParams::HTTPAuthenticationParams(const HTTPRequest& request) { fromRequest(request); }
...
void HTTPAuthenticationParams::fromAuthInfo(const std::string& authInfo) { parse(authInfo.begin(), authInfo.end()); }
...
void HTTPAuthenticationParams::fromRequest(const HTTPRequest& request) {
  ...
  if (icompare(scheme, "Digest") != 0) throw InvalidArgumentException("Could not parse non-Digest authentication information", scheme);
...
void HTTPAuthenticationParams::fromResponse(const HTTPResponse& response, const std::string& header) {
  ...
  if (it == response.end()) throw NotAuthenticatedException("HTTP response has no authentication header");
...
const std::string& HTTPAuthenticationParams::getRealm() const { return get(REALM); }

void HTTPAuthenticationParams::setRealm(const std::string& realm) { set(REALM, realm); }
...
void HTTPAuthenticationParams::parse(std::string::const_iterator first, std::string::const_iterator last) {
  ...
  if (state == STATE_VALUE) add(token, value);

  if (!(state & STATE_FINAL)) throw SyntaxException("Invalid authentication information");
}
