...
MessageHeader::MessageHeader()
    : _fieldLimit(DFL_FIELD_LIMIT), _nameLengthLimit(DFL_NAME_LENGTH_LIMIT), _valueLengthLimit(DFL_VALUE_LENGTH_LIMIT) {}
...
void MessageHeader::read(std::istream& istr) {
  ...
  while (ch != eof && ch != '\r' && ch != '\n') {
    if (_fieldLimit > 0 && fields == _fieldLimit) throw MessageException("Too many header fields");
    name.clear();
    value.clear();
    while (ch != eof && ch != ':' && ch != '\n' && name.length() < _nameLengthLimit) {
      name += ch;
      ch = buf.sbumpc();
    }
    if (ch == '\n') {
    ...
    while (ch != eof && ch != '\r' && ch != '\n' && value.length() < _valueLengthLimit) {
      value += ch;
      ch = buf.sbumpc();
    }
...
int MessageHeader::getFieldLimit() const { return _fieldLimit; }
...
int MessageHeader::getNameLengthLimit() const { return _nameLengthLimit; }
...
int MessageHeader::getValueLengthLimit() const { return _valueLengthLimit; }
...
bool MessageHeader::hasToken(const std::string& fieldName, const std::string& token) const {
  ...
  for (std::vector<std::string>::const_iterator it = tokens.begin(); it != tokens.end(); ++it) {
    if (Poco::icompare(*it, token) == 0) return true;
  }
  return false;
}
...
void MessageHeader::splitElements(const std::string& s, std::vector<std::string>& elements, bool ignoreEmpty) {
  ...
  while (it != end) {
    if (*it == '"') {
      ...
      if (!ignoreEmpty || !elem.empty()) elements.push_back(elem);
      elem.clear();
      ++it;
    } else
      elem += *it++;
...
void MessageHeader::splitParameters(const std::string::const_iterator& begin, const std::string::const_iterator& end,
