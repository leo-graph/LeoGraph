...
class TCPConnectionNotification : public Notification {
 public:
  TCPConnectionNotification(const StreamSocket& socket) : _socket(socket) {}

  ~TCPConnectionNotification() {}

  const StreamSocket& socket() const { return _socket; }

 private:
  StreamSocket _socket;
};

TCPServerDispatcher::TCPServerDispatcher(TCPServerConnectionFactory::Ptr pFactory, Poco::ThreadPool& threadPool,
  ...
  if (!_pParams) _pParams = new TCPServerParams;

  if (_pParams->getMaxThreads() == 0) _pParams->setMaxThreads(threadPool.capacity());
}

TCPServerDispatcher::~TCPServerDispatcher() {}

void TCPServerDispatcher::duplicate() { ++_rc; }
      ...
      if (pNf && !_stopped) {
        ...
        if (pCNf) {
          ...
          if (!_stopped) {
            ...
            if (pConnection) pConnection->start();
          }
  ...
  if (_queue.size() < _pParams->getMaxQueued()) {
    ...
    if (!_queue.hasIdleThreads() && _currentThreads < _pParams->getMaxThreads()) {
      try {
        ...
        ErrorHandler::logMessage(Message::PRIO_WARNING,
                                 "Got an exception while starting thread for connection from " + socket.peerAddress().toString());
        ErrorHandler::handle(exc);
        this->release();
        ++_refusedConnections;
        return;
      ...
      ErrorHandler::logMessage(Message::PRIO_TRACE, "Don't have idle threads, adding connection from " + socket.peerAddress().toString() +
                                                        " to the queue, size: " + std::to_string(_queue.size()));
    }
...
int TCPServerDispatcher::currentThreads() const { return _currentThreads; }
...
int TCPServerDispatcher::totalConnections() const { return _totalConnections; }

int TCPServerDispatcher::currentConnections() const { return _currentConnections; }

int TCPServerDispatcher::maxConcurrentConnections() const { return _maxConcurrentConnections; }

int TCPServerDispatcher::queuedConnections() const { return _queue.size(); }

int TCPServerDispatcher::refusedConnections() const { return _refusedConnections; }
...
void TCPServerDispatcher::beginConnection() {
  ...
  ++_totalConnections;
  ++_currentConnections;
  if (_currentConnections > _maxConcurrentConnections) _maxConcurrentConnections.store(_currentConnections);
}

void TCPServerDispatcher::endConnection() { --_currentConnections; }
