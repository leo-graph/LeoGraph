...
HTTPRequest::HTTPRequest() : _method(HTTP_GET), _uri("/") {}

HTTPRequest::HTTPRequest(const std::string& version) : HTTPMessage(version), _method(HTTP_GET), _uri("/") {}

HTTPRequest::HTTPRequest(const std::string& method, const std::string& uri) : _method(method), _uri(uri) {}

HTTPRequest::HTTPRequest(const std::string& method, const std::string& uri, const std::string& version)
    : HTTPMessage(version), _method(method), _uri(uri) {}
...
void HTTPRequest::setHost(const std::string& host) { set(HOST, host); }
...
const std::string& HTTPRequest::getHost() const { return get(HOST); }
...
void HTTPRequest::setCookies(const NameValueCollection& cookies) {
  ...
  for (NameValueCollection::ConstIterator it = cookies.begin(); it != cookies.end(); ++it) {
    if (it != cookies.begin()) cookie.append("; ");
...
bool HTTPRequest::hasCredentials() const { return has(AUTHORIZATION); }

void HTTPRequest::getCredentials(std::string& scheme, std::string& authInfo) const { getCredentials(AUTHORIZATION, scheme, authInfo); }
...
bool HTTPRequest::hasProxyCredentials() const { return has(PROXY_AUTHORIZATION); }
