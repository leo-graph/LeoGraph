...
IPv4AddressImpl::IPv4AddressImpl() { std::memset(&_addr, 0, sizeof(_addr)); }

IPv4AddressImpl::IPv4AddressImpl(const void* addr) { std::memcpy(&_addr, addr, sizeof(_addr)); }
...
IPv4AddressImpl::IPv4AddressImpl(const IPv4AddressImpl& addr) { std::memcpy(&_addr, &addr._addr, sizeof(_addr)); }
...
poco_socklen_t IPv4AddressImpl::length() const { return sizeof(_addr); }

const void* IPv4AddressImpl::addr() const { return &_addr; }

IPAddressImpl::Family IPv4AddressImpl::family() const { return AddressFamily::IPv4; }

int IPv4AddressImpl::af() const { return AF_INET; }

unsigned IPv4AddressImpl::prefixLength() const { return maskBits(ntohl(_addr.s_addr), 32); }

Poco::UInt32 IPv4AddressImpl::scope() const { return 0; }

bool IPv4AddressImpl::isWildcard() const { return _addr.s_addr == INADDR_ANY; }

bool IPv4AddressImpl::isBroadcast() const { return _addr.s_addr == INADDR_NONE; }
...
bool IPv4AddressImpl::isIPv4Compatible() const { return true; }

bool IPv4AddressImpl::isIPv4Mapped() const { return true; }

bool IPv4AddressImpl::isWellKnownMC() const {
  return (ntohl(_addr.s_addr) & 0xFFFFFF00) == 0xE0000000;  // 224.0.0.0/8
}

bool IPv4AddressImpl::isNodeLocalMC() const { return false; }
...
IPAddressImpl* IPv4AddressImpl::clone() const { return new IPv4AddressImpl(&_addr); }
...
bool IPv4AddressImpl::operator==(const IPv4AddressImpl& addr) const { return 0 == std::memcmp(&addr._addr, &_addr, sizeof(_addr)); }

bool IPv4AddressImpl::operator!=(const IPv4AddressImpl& addr) const { return !(*this == addr); }
...
IPv6AddressImpl::IPv6AddressImpl() : _scope(0) { std::memset(&_addr, 0, sizeof(_addr)); }

IPv6AddressImpl::IPv6AddressImpl(const void* addr) : _scope(0) { std::memcpy(&_addr, addr, sizeof(_addr)); }

IPv6AddressImpl::IPv6AddressImpl(const void* addr, Poco::UInt32 scope) : _scope(scope) { std::memcpy(&_addr, addr, sizeof(_addr)); }
...
IPv6AddressImpl::IPv6AddressImpl(unsigned prefix) : _scope(0) {
  unsigned i = 0;
...
poco_socklen_t IPv6AddressImpl::length() const { return sizeof(_addr); }

const void* IPv6AddressImpl::addr() const { return &_addr; }

IPAddressImpl::Family IPv6AddressImpl::family() const { return AddressFamily::IPv6; }

int IPv6AddressImpl::af() const { return AF_INET6; }
...
Poco::UInt32 IPv6AddressImpl::scope() const { return _scope; }

bool IPv6AddressImpl::isWildcard() const {
  const UInt16* words = reinterpret_cast<const UInt16*>(&_addr);
  return words[0] == 0 && words[1] == 0 && words[2] == 0 && words[3] == 0 && words[4] == 0 && words[5] == 0 && words[6] == 0 &&
         words[7] == 0;
}

bool IPv6AddressImpl::isBroadcast() const { return false; }

bool IPv6AddressImpl::isLoopback() const {
  const UInt16* words = reinterpret_cast<const UInt16*>(&_addr);
  return words[0] == 0 && words[1] == 0 && words[2] == 0 && words[3] == 0 && words[4] == 0 && words[5] == 0 && words[6] == 0 &&
         ByteOrder::fromNetwork(words[7]) == 0x0001;
}
...
IPv6AddressImpl IPv6AddressImpl::parse(const std::string& addr) {
  ...
  if (std::string::npos != pos) {
    ...
    if (!(scopeId = if_nametoindex(scope.c_str()))) return IPv6AddressImpl();
...
IPAddressImpl* IPv6AddressImpl::clone() const { return new IPv6AddressImpl(*this); }

IPv6AddressImpl IPv6AddressImpl::operator&(const IPv6AddressImpl& addr) const {
  if (_scope != addr._scope) throw Poco::InvalidArgumentException("Scope ID of passed IPv6 address does not match with the source one.");
...
bool IPv6AddressImpl::operator!=(const IPv6AddressImpl& addr) const { return !(*this == addr); }
