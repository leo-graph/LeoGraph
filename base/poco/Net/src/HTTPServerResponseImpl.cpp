...
HTTPServerResponseImpl::HTTPServerResponseImpl(HTTPServerSession& session)
    : _session(session), _pRequest(0), _pStream(0), _pHeaderStream(0) {}

HTTPServerResponseImpl::~HTTPServerResponseImpl() {
  if (_pHeaderStream && _pHeaderStream != _pStream) delete _pHeaderStream;
...
std::ostream& HTTPServerResponseImpl::send() {
  ...
  if ((_pRequest && _pRequest->getMethod() == HTTPRequest::HTTP_HEAD) || getStatus() < 200 ||
      getStatus() == HTTPResponse::HTTP_NO_CONTENT || getStatus() == HTTPResponse::HTTP_NOT_MODIFIED) {
