...
HTTPClientSession::~HTTPClientSession() { _proxySessionFactory.unregisterProtocol("http"); }
...
void HTTPClientSession::setProxy(const std::string& host, Poco::UInt16 port, const std::string& protocol, bool tunnel) {
  if (protocol != "http" && protocol != "https") throw IllegalStateException("Protocol must be either http or https");
  ...
  if (!connected()) {
    ...
    throw IllegalStateException("Cannot set the proxy host, port and protocol for an already connected session");
}
...
void HTTPClientSession::setProxyUsername(const std::string& username) { _proxyConfig.username = username; }

void HTTPClientSession::setProxyPassword(const std::string& password) { _proxyConfig.password = password; }

void HTTPClientSession::setProxyConfig(const ProxyConfig& config) { _proxyConfig = config; }

void HTTPClientSession::setGlobalProxyConfig(const ProxyConfig& config) { _globalProxyConfig = config; }

void HTTPClientSession::setKeepAliveTimeout(const Poco::Timespan& timeout) {
  if (connected()) {
    throw Poco::IllegalStateException(
        "cannot change keep alive timeout on initiated connection, "
...
void HTTPClientSession::setKeepAliveMaxRequests(int max_requests) {
  if (connected()) {
    throw Poco::IllegalStateException(
        "cannot change keep alive max requests on initiated connection, "
        "That value is managed privately after connection is established.");
  }
  _keepAliveMaxRequests = max_requests;
}

void HTTPClientSession::setKeepAliveRequest(int request) { _keepAliveCurrentRequest = request; }

void HTTPClientSession::setLastRequest(Poco::Timestamp time) {
  if (connected()) {
    throw Poco::IllegalStateException(
        "cannot change last request on initiated connection, "
        "That value is managed privately after connection is established.");
  }
  _lastRequest = time;
}
...
std::ostream& HTTPClientSession::sendRequest(HTTPRequest& request, uint64_t* connect_time, uint64_t* first_byte_time) {
  ...
  try {
    if (!connected()) reconnect(connect_time);
    if (!request.has(HTTPMessage::CONNECTION)) request.setKeepAlive(keepAlive);
    ...
    if (!request.has(HTTPRequest::HOST) && !_host.empty()) request.setHost(_host, _port);
    ...
    } else if ((method != HTTPRequest::HTTP_PUT && method != HTTPRequest::HTTP_POST && method != HTTPRequest::HTTP_PATCH) ||
    ...
    if (first_byte_time) *first_byte_time = first_byte_watch.elapsed();
    _lastRequest.update();
...
std::istream& HTTPClientSession::receiveResponse(HTTPResponse& response) {
  ...
  if (!_mustReconnect) {
    ...
    if (timeout > 0) _keepAliveTimeout = std::min(_keepAliveTimeout, Poco::Timespan(timeout, 0));
    auto max_requests = response.getKeepAliveMaxRequests();
    if (max_requests > 0) _keepAliveMaxRequests = std::min(_keepAliveMaxRequests, max_requests);
  }
...
void HTTPClientSession::reset() { close(); }

bool HTTPClientSession::secure() const { return false; }
...
void HTTPClientSession::reconnect(uint64_t* connect_time) {
  ...
  try {
    ...
    if (connect_time) *connect_time = connect_watch.elapsed();
  } catch (Poco::TimeoutException&) {
    if (connect_time) *connect_time = getConnectionTimeout().totalMicroseconds();
    throw;
  } catch (...) {
    if (connect_time) *connect_time = connect_watch.elapsed();
    throw;
...
bool HTTPClientSession::mustReconnect() const {
  if (!_mustReconnect) return isKeepAliveExpired(_defaultKeepAliveReliabilityLevel);
  return true;
}

void HTTPClientSession::proxyAuthenticate(HTTPRequest& request) { proxyAuthenticateImpl(request); }
...
void HTTPClientSession::assign(Poco::Net::HTTPClientSession& session) {
  ...
  if (session.buffered()) throw Poco::LogicException("assign a session with not empty buffered data");

  if (buffered()) throw Poco::LogicException("assign to a session with not empty buffered data");
