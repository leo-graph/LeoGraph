...
class PollSetImpl {
 public:
  PollSetImpl() : _epollfd(-1), _events(1024) {
  ...
  void add(const Socket& socket, int mode) {
    ...
    if (_socketMap.find(sockImpl) == _socketMap.end()) _socketMap[sockImpl] = socket;
  }
  ...
  PollSet::SocketModeMap poll(const Poco::Timespan& timeout) {
    ...
    for (int i = 0; i < rc; i++) {
      ...
      if (it != _socketMap.end()) {
        if (_events[i].events & EPOLLIN) result[it->second] |= PollSet::POLL_READ;
        if (_events[i].events & EPOLLOUT) result[it->second] |= PollSet::POLL_WRITE;
        if (_events[i].events & EPOLLERR) result[it->second] |= PollSet::POLL_ERROR;
      }
    }
 ...
 public:
  ...
  void update(const Socket& socket, int mode) {
    ...
    for (std::vector<pollfd>::iterator it = _pollfds.begin(); it != _pollfds.end(); ++it) {
      if (it->fd == fd) {
        it->events = 0;
        if (mode & PollSet::POLL_READ) it->events |= POLLIN;
        if (mode & PollSet::POLL_WRITE) it->events |= POLLOUT;
      }
    }
  }
  ...
  PollSet::SocketModeMap poll(const Poco::Timespan& timeout) {
    PollSet::SocketModeMap result;
    {
      ...
      for (std::map<poco_socket_t, int>::iterator it = _addMap.begin(); it != _addMap.end(); ++it) {
        ...
        pfd.events = 0;
        pfd.revents = 0;
        if (it->second & PollSet::POLL_READ) pfd.events |= POLLIN;
        if (it->second & PollSet::POLL_WRITE) pfd.events |= POLLOUT;
      ...
      if (!_socketMap.empty()) {
        for (std::vector<pollfd>::iterator it = _pollfds.begin(); it != _pollfds.end(); ++it) {
          ...
          if (its != _socketMap.end()) {
            if (it->revents & POLLIN) result[its->second] |= PollSet::POLL_READ;
            if (it->revents & POLLOUT) result[its->second] |= PollSet::POLL_WRITE;
            if (it->revents & POLLERR) result[its->second] |= PollSet::POLL_ERROR;
          }
          it->revents = 0;
        }
      }
...
PollSet::SocketModeMap PollSet::poll(const Poco::Timespan& timeout) { return _pImpl->poll(timeout); }
