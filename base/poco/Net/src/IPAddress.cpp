...
IPAddress::IPAddress(const IPAddress& addr) {
  if (addr.family() == IPv4) newIPv4(addr.addr());
#if defined(POCO_HAVE_IPv6)
  else
...
IPAddress::IPAddress(const void* addr, poco_socklen_t length) : _pImpl(0) {
  if (length == sizeof(struct in_addr)) newIPv4(addr);
#if defined(POCO_HAVE_IPv6)
...
IPAddress::IPAddress(const struct sockaddr& sockaddr) {
  ...
  if (family == AF_INET) newIPv4(&reinterpret_cast<const struct sockaddr_in*>(&sockaddr)->sin_addr);
#if defined(POCO_HAVE_IPv6)
...
IPAddress& IPAddress::operator=(const IPAddress& addr) {
  if (&addr != this) {
    if (addr.family() == IPAddress::IPv4) newIPv4(addr.addr());
#if defined(POCO_HAVE_IPv6)
...
IPAddress::Family IPAddress::family() const { return pImpl()->family(); }

Poco::UInt32 IPAddress::scope() const { return pImpl()->scope(); }

std::string IPAddress::toString() const { return pImpl()->toString(); }
...
bool IPAddress::isNodeLocalMC() const { return pImpl()->isNodeLocalMC(); }

bool IPAddress::isLinkLocalMC() const { return pImpl()->isLinkLocalMC(); }

bool IPAddress::isSiteLocalMC() const { return pImpl()->isSiteLocalMC(); }
...
bool IPAddress::operator!=(const IPAddress& a) const { return !(*this == a); }
...
bool IPAddress::operator<=(const IPAddress& a) const { return !(a < *this); }
...
poco_socklen_t IPAddress::length() const { return pImpl()->length(); }
...
IPAddress IPAddress::parse(const std::string& addr) { return IPAddress(addr); }
...
IPAddress IPAddress::wildcard(Family family) { return IPAddress(family); }
