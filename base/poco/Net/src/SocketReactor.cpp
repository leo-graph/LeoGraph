  ...
  while (!_stop) {
    try {
      ...
      if (nSockets == 0) {
        ...
        for (Socket::SocketList::iterator it = readable.begin(); it != readable.end(); ++it) dispatch(*it, _pReadableNotification);
        for (Socket::SocketList::iterator it = writable.begin(); it != writable.end(); ++it) dispatch(*it, _pWritableNotification);
        for (Socket::SocketList::iterator it = except.begin(); it != except.end(); ++it) dispatch(*it, _pErrorNotification);
      } else
        onTimeout();
    } catch (Exception& exc) {
...
bool SocketReactor::hasEventHandler(const Socket& socket, const Poco::AbstractObserver& observer) {
  NotifierPtr pNotifier;
  {
    ...
    EventHandlerMap::iterator it = _handlers.find(socket);
    if (it != _handlers.end()) {
      if (it->second->hasObserver(observer)) return true;
    }
  }
...
void SocketReactor::dispatch(SocketNotification* pNotification) {
  ...
  {
    FastMutex::ScopedLock lock(_mutex);
    for (EventHandlerMap::iterator it = _handlers.begin(); it != _handlers.end(); ++it) delegates.push_back(it->second);
  }
