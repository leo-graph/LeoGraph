...
static int match_in_dir(const char *d, const char *p, int flags, int (*errfunc)(const char *path, int err), struct match **tail) {
  ...
  while ((de = readdir(dir))) {
    ...
    if (p2 && (flags & GLOB_PERIOD) && de->d_name[0] == '.' && (!de->d_name[1] || (de->d_name[1] == '.' && !de->d_name[2])) &&
...
int glob(const char *restrict pat, int flags, int (*errfunc)(const char *path, int err), glob_t *restrict g) {
  ...
  for (i = 0, tail = head.next; i < cnt; tail = tail->next, i++) g->gl_pathv[offs + i] = tail->name;
...
void globfree(glob_t *g) {
  size_t i;
  for (i = 0; i < g->gl_pathc; i++) free(g->gl_pathv[g->gl_offs + i] - offsetof(struct match, name));
  free(g->gl_pathv);
  g->gl_pathc = 0;
  g->gl_pathv = NULL;
}
