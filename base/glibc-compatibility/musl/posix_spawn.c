...
static int child(void *args_vp) {
  ...
  pthread_sigmask(SIG_SETMASK, (attr->__flags & POSIX_SPAWN_SETSIGMASK) ? &attr->__ss : &args->oldmask, 0);
...
static int __posix_spawnx(pid_t *restrict res, const char *restrict path, posix_spawn_exec_fn exec, const posix_spawn_file_actions_t *fa,
...
int posix_spawn(pid_t *restrict res, const char *restrict path, const posix_spawn_file_actions_t *fa,
                const posix_spawnattr_t *restrict attr, char *const argv[restrict], char *const envp[restrict]) {
  posix_spawn_exec_fn fn = execve;
  if (attr && attr->__fn) fn = (posix_spawn_exec_fn)attr->__fn;
