...
#  if __has_feature(memory_sanitizer) || __has_feature(thread_sanitizer)
  ...
  /// and to get its value it uses getauxval
  /// (https://github.com/llvm/llvm-project/blob/20eff684203287828d6722fc860b9d3621429542/compiler-rt/lib/sanitizer_common/sanitizer_linux_libcdep.cpp#L985-L988).
  ...
  ///
  ///    ==301455==ERROR: MemorySanitizer: SEGV on unknown address 0x2ffc6d721550 (pc 0x5622c1cc0073 bp 0x000000000003 sp 0x7ffc6d721530
  ///    T301455)
  ...
  /// #4 0x5622c0d95356 in __msan_init (./ClickHouse/build_msan/contrib/google-protobuf-cmake/protoc+0x256356) (BuildId:
  /// 6411d3c88b898ba3f7d49760555977d3e61f0741) #5 0x5622c0dfe878 in msan.module_ctor main.cc #6 0x5622c1cc156c in __libc_csu_init
  /// (./ClickHouse/build_msan/contrib/google-protobuf-cmake/protoc+0x118256c) (BuildId: 6411d3c88b898ba3f7d49760555977d3e61f0741) #7
  /// 0x73dc05dd7ea3 in __libc_start_main /usr/src/debug/glibc/glibc/csu/../csu/libc-start.c:343:6 #8 0x5622c0d6b7cd in _start
  /// (./ClickHouse/build_msan/contrib/google-protobuf-cmake/protoc+0x22c7cd) (BuildId: 6411d3c88b898ba3f7d49760555977d3e61f0741)
  ...
  if (type == AT_EXECFN) return (unsigned long)"/proc/self/exe";
#  endif
#endif
...
unsigned long NO_SANITIZE_THREAD __getauxval_environ(unsigned long type) {
  if (type == AT_SECURE) return __auxv_secure;

  if (__auxv_environ) {
    size_t index = __find_auxv(type);
    if (index != ((size_t)-1)) return __auxv_environ[index];
  }
...
static unsigned long NO_SANITIZE_THREAD __auxv_init_environ(unsigned long type) {
  ...
  if (secure_idx != ((size_t)-1)) __auxv_secure = __auxv_environ[secure_idx];
...
unsigned long NO_SANITIZE_THREAD getauxval(unsigned long type) { return ((unsigned long (*)(unsigned long))getauxval_func)(type); }
