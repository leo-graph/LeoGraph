...
static int cgt_time32_wrap(clockid_t clk, struct timespec *ts) {
  long ts32[2];
  int (*f)(clockid_t, long[2]) = (int (*)(clockid_t, long[2]))vdso_func_32;
  int r = f(clk, ts32);
  if (!r) {
...
#  endif
  int (*f)(clockid_t, struct timespec *) = (int (*)(clockid_t, struct timespec *))p;
...
int clock_gettime(clockid_t clk, struct timespec *ts) {
...
#ifdef VDSO_CGT_SYM
  int (*f)(clockid_t, struct timespec *) = (int (*)(clockid_t, struct timespec *))vdso_func;
  if (f) {
    r = f(clk, ts);
    if (!r) return r;
  ...
  r = -ENOSYS;
  if (sizeof(time_t) > 4) r = __syscall(SYS_clock_gettime64, clk, ts);
  if (SYS_clock_gettime == SYS_clock_gettime64 || r != -ENOSYS) return __syscall_ret(r);
  long ts32[2];
