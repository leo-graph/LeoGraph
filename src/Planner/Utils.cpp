...
Block buildCommonHeaderForUnion(const SharedHeaders &queries_headers, SelectUnionMode union_mode, bool use_variant_as_common_type) {
  ...
  for (size_t query_number = 1; query_number < num_selects; ++query_number) {
    ...
    if (union_mode == SelectUnionMode::UNION_DEFAULT || union_mode == SelectUnionMode::UNION_ALL ||
    ...
    if (queries_headers.at(query_number)->columns() != columns_size)
      throw Exception(error_code, "Different number of columns in {} elements: {} and {}", toString(union_mode), common_header.dumpNames(),
  ...
  for (size_t column_number = 0; column_number < columns_size; ++column_number) {
    for (size_t i = 0; i < num_selects; ++i) columns[i] = &queries_headers[i]->getByPosition(column_number);
...
void addConvertingToCommonHeaderActionsIfNeeded(std::vector<std::unique_ptr<QueryPlan>> &query_plans, const Block &union_common_header,
                                                SharedHeaders &query_plans_headers, ContextPtr context) {
  size_t queries_size = query_plans.size();
  for (size_t i = 0; i < queries_size; ++i) {
    auto &query_node_plan = query_plans[i];
    if (blocksHaveEqualStructure(*query_node_plan->getCurrentHeader(), union_common_header)) continue;

    auto actions_dag =
        ActionsDAG::makeConvertingActions(query_node_plan->getCurrentHeader()->getColumnsWithTypeAndName(),
                                          union_common_header.getColumnsWithTypeAndName(), ActionsDAG::MatchColumnsMode::Position, context,
                                          false /*ignore_constant_values*/, false /*add_cast_columns*/, nullptr /*new_names*/);
...
ASTPtr queryNodeToSelectQuery(const QueryTreeNodePtr &query_node, bool set_subquery_cte_name) {
  ...
  auto result_ast = query_node_typed.toAST({.qualify_indentifiers_with_database = false, .set_subquery_cte_name = set_subquery_cte_name});

  while (true) {
    if (auto * /*select_query*/ _ = result_ast->as<ASTSelectQuery>()) break;
  ...
  if (result_ast == nullptr) throw Exception(ErrorCodes::LOGICAL_ERROR, "Query node invalid conversion to select query");
...
StreamLocalLimits getLimitsForStorage(const Settings &settings, const SelectQueryOptions &options) {
  ...
  limits.size_limits =
      SizeLimits(settings[Setting::max_rows_to_read], settings[Setting::max_bytes_to_read], settings[Setting::read_overflow_mode]);
...
StorageLimits buildStorageLimits(const Context &context, const SelectQueryOptions &options) {
  ...
  if (!options.ignore_limits) {
    ...
    leaf_limits = SizeLimits(settings[Setting::max_rows_to_read_leaf], settings[Setting::max_bytes_to_read_leaf],
                             settings[Setting::read_overflow_mode_leaf]);
  }

  return {limits, leaf_limits};
}

std::pair<ActionsDAG, CorrelatedSubtrees> buildActionsDAGFromExpressionNode(const QueryTreeNodePtr &expression_node,
...
bool sortDescriptionIsPrefix(const SortDescription &prefix, const SortDescription &full) {
  ...
  if (prefix_size > full.size()) return false;
  ...
  while (!join_tree_nodes_to_process.empty()) {
    ...
    switch (join_tree_node_type) {
      ...
      case QueryTreeNodeType::CROSS_JOIN: {
        ...
        for (const auto &expr : cross_join_node.getTableExpressions()) join_tree_nodes_to_process.push_back(expr);
  ...
  while (!join_tree_nodes_to_process.empty()) {
    ...
    switch (join_tree_node_type) {
      ...
      case QueryTreeNodeType::QUERY: {
        ...
        if (query_node_to_process.isGroupByWithTotals()) return true;
      ...
      case QueryTreeNodeType::UNION: {
        ...
        for (const auto &union_query : union_queries) join_tree_nodes_to_process.push_back(union_query.get());
        break;
      }
      case QueryTreeNodeType::ARRAY_JOIN: {
      ...
      case QueryTreeNodeType::CROSS_JOIN: {
        ...
        for (const auto &expr : cross_join_node.getTableExpressions()) join_tree_nodes_to_process.push_back(expr.get());
...
QueryTreeNodePtr replaceTableExpressionsWithDummyTables(const QueryTreeNodePtr &query_node, const QueryTreeNodes &table_nodes,
                                                        const ContextPtr &context, ResultReplacementMap *result_replacement_map) {
  ...
  for (const auto &table_expression : table_nodes) {
    ...
    if (table_node || table_function_node) {
      ...
      auto storage_dummy = std::make_shared<StorageDummy>(
          ...
          storage_snapshot->metadata->getColumns(), storage_snapshot, storage.supportsReplication());
      ...
      if (result_replacement_map) result_replacement_map->emplace(table_expression, dummy_table_node);
...
FilterDAGInfo buildFilterInfo(ASTPtr filter_expression, const QueryTreeNodePtr &table_expression, PlannerContextPtr &planner_context,
...
FilterDAGInfo buildFilterInfo(QueryTreeNodePtr filter_query_tree, const QueryTreeNodePtr &table_expression,
                              PlannerContextPtr &planner_context, NameSet table_expression_required_names_without_filter) {
  ...
  if (expression_nodes.size() != 1)
    throw Exception(ErrorCodes::BAD_ARGUMENTS, "Filter actions must return single output node. Actual {}", expression_nodes.size());
...
ASTPtr parseAdditionalResultFilter(const Settings &settings) {
  ...
  if (additional_result_filter.empty()) return {};

  ParserExpression parser;
  auto additional_result_filter_ast =
      parseQuery(parser, additional_result_filter.data(), additional_result_filter.data() + additional_result_filter.size(),
                 "additional result filter", settings[Setting::max_query_size], settings[Setting::max_parser_depth],
...
void appendSetsFromActionsDAG(const ActionsDAG &dag, UsefulSets &useful_sets) {
  for (const auto &node : dag.getNodes()) {
    if (node.column) {
      ...
      if (const auto *column_const = typeid_cast<const ColumnConst *>(column)) column = &column_const->getDataColumn();

      if (const auto *column_set = typeid_cast<const ColumnSet *>(column)) useful_sets.insert(column_set->getData());
    }
...
QueryPlanStepPtr projectOnlyUsedColumns(const SharedHeader &stream_header, const ColumnIdentifiers &used_column_identifiers) {
  ...
  for (const auto &column : stream_header->getColumnsWithTypeAndName()) {
    ...
    if (used_column_identifiers_set.contains(column.name)) outputs.push_back(input_node);
  }
