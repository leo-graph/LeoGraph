...
String calculateActionNodeNameWithCastIfNeeded(const ConstantNode &constant_node, Int64 optimize_const_name_size) {
  ...
  if (requires_cast_call) buffer << "_CAST(";
...
class ActionNodeNameHelper {
 public:
  ActionNodeNameHelper(QueryTreeNodeToName &node_to_name_, const PlannerContext &planner_context_,
  ...
  String calculateActionNodeName(const QueryTreeNodePtr &node) {
    ...
    if (it != node_to_name.end()) return it->second;
    ...
    switch (node_type) {
      case QueryTreeNodeType::COLUMN: {
        ...
        if (use_column_identifier_as_action_node_name) column_identifier = planner_context.getColumnNodeIdentifierOrNull(node);
      ...
      case QueryTreeNodeType::CONSTANT: {
        ...
        if (planner_context.isASTLevelOptimizationAllowed()) {
          result = calculateActionNodeNameWithCastIfNeeded(
              constant_node, planner_context.getQueryContext()->getSettingsRef()[Setting::optimize_const_name_size]);
        } else {
          ...
          if (constant_node.hasSourceExpression() && constant_node.getSourceExpression()->getNodeType() != QueryTreeNodeType::QUERY) {
            ...
            result = calculateConstantActionNodeName(
                constant_node, planner_context.getQueryContext()->getSettingsRef()[Setting::optimize_const_name_size]);
        }
        break;
      }
      case QueryTreeNodeType::FUNCTION: {
        ...
        if (isNameOfInFunction(function_node.getFunctionName())) {
          ...
          in_function_second_argument_node_name =
              PlannerContext::createSetKey(in_first_argument_node->getResultType(), in_second_argument_node);
        }
        ...
        if (!function_parameters_nodes.empty()) {
          ...
          for (size_t i = 0; i < function_parameters_nodes_size; ++i) {
            ...
            if (i + 1 != function_parameters_nodes_size) buffer << ", ";
          }
        ...
        for (size_t i = 0; i < function_arguments_nodes_size; ++i) {
          ...
          if (i + 1 != function_arguments_nodes_size) buffer << ", ";
        }
      ...
      case QueryTreeNodeType::LAMBDA: {
        ...
        for (size_t i = 0; i < lambda_arguments_nodes_size; ++i) {
          ...
          if (i + 1 != lambda_arguments_nodes_size) buffer << ", ";
        }
      ...
      case QueryTreeNodeType::QUERY: {
        ...
        if (query_node.isCorrelated())
          ...
          throw Exception(ErrorCodes::LOGICAL_ERROR, "Only correlated QueryNode can be used as action query tree node, but got {}",
  ...
  String calculateWindowNodeActionName(const QueryTreeNodePtr &function_nodew_node_, const QueryTreeNodePtr &window_node_) {
    ...
    if (window_node.hasPartitionBy()) {
      ...
      for (size_t i = 0; i < partition_by_nodes_size; ++i) {
        ...
        if (i + 1 != partition_by_nodes_size) buffer << ", ";
      }
    }

    if (window_node.hasOrderBy()) {
      if (window_node.hasPartitionBy()) buffer << ' ';
      ...
      for (size_t i = 0; i < order_by_nodes_size; ++i) {
        ...
        if (nulls_sort_direction) buffer << " NULLS " << (nulls_sort_direction == sort_direction ? "LAST" : "FIRST");

        if (auto collator = sort_node.getCollator()) buffer << " COLLATE " << collator->getLocale();

        if (sort_node.withFill()) {
          buffer << " WITH FILL";

          if (sort_node.hasFillFrom()) buffer << " FROM " << calculateActionNodeName(sort_node.getFillFrom());

          if (sort_node.hasFillTo()) buffer << " TO " << calculateActionNodeName(sort_node.getFillTo());

          if (sort_node.hasFillStep()) buffer << " STEP " << calculateActionNodeName(sort_node.getFillStep());

          if (sort_node.hasFillStaleness()) buffer << " STALENESS " << calculateActionNodeName(sort_node.getFillStaleness());
        }

        if (i + 1 != order_by_nodes_size) buffer << ", ";
      }
    }
    ...
    if (window_frame_opt) {
      ...
      if (window_node.hasPartitionBy() || window_node.hasOrderBy()) buffer << ' ';
...
class ActionsScopeNode {
 public:
  explicit ActionsScopeNode(ActionsDAG &actions_dag_, QueryTreeNodePtr scope_node_)
      : actions_dag(actions_dag_), scope_node(std::move(scope_node_)) {
    for (const auto &node : actions_dag.getNodes()) node_name_to_node[node.result_name] = &node;
  }
  ...
  const QueryTreeNodePtr &getScopeNode() const { return scope_node; }

  [[maybe_unused]] bool containsNode(const std::string &node_name) { return node_name_to_node.contains(node_name); }

  [[maybe_unused]] bool containsInputNode(const std::string &node_name) {
    const auto *node = tryGetNode(node_name);
    if (node && node->type == ActionsDAG::ActionType::INPUT) return true;
  ...
  [[maybe_unused]] const ActionsDAG::Node *tryGetNode(const std::string &node_name) {
    ...
    if (it == node_name_to_node.end()) return {};
  ...
  const ActionsDAG::Node *getNodeOrThrow(const std::string &node_name) {
    ...
    if (it == node_name_to_node.end())
      throw Exception(ErrorCodes::LOGICAL_ERROR, "No node with name {}. There are only nodes {}", node_name, actions_dag.dumpNames());
  ...
  const ActionsDAG::Node *addInputColumnIfNecessary(const std::string &node_name, const DataTypePtr &column_type) {
    auto it = node_name_to_node.find(node_name);
    if (it != node_name_to_node.end()) return it->second;
  ...
  const ActionsDAG::Node *addConstantIfNecessary(const std::string &node_name, const ColumnWithTypeAndName &column, bool is_deterministic) {
    ...
    if (it != node_name_to_node.end()) {
      ...
      if (!materialized_input) return it->second;
    }
  ...
  const ActionsDAG::Node *addFunctionIfNecessary(const std::string &node_name, ActionsDAG::NodeRawConstPtrs children,
                                                 const FunctionOrOverloadResolver &function) {
    auto it = node_name_to_node.find(node_name);
    if (it != node_name_to_node.end()) return it->second;
...
class PlannerActionsVisitorImpl {
 public:
  PlannerActionsVisitorImpl(ActionsDAG &actions_dag, const PlannerContextPtr &planner_context_,
                            const ColumnNodePtrWithHashSet &correlated_columns_set_, bool use_column_identifier_as_action_node_name_);
  ...
  class Levels {
   public:
    ...
    void set(size_t level) {
      check(level);
      if (level) mask |= (uint64_t(1) << (level - 1));
    }
   ...
   private:
    ...
    void check(size_t level) {
      if (level > 64) throw Exception(ErrorCodes::INCORRECT_QUERY, "Maximum lambda depth exceeded. Maximum 64.");
    }
  };
...
PlannerActionsVisitorImpl::PlannerActionsVisitorImpl(ActionsDAG &actions_dag, const PlannerContextPtr &planner_context_,
...
PlannerActionsVisitorImpl::NodeNameAndNodeMinLevel PlannerActionsVisitorImpl::visitImpl(QueryTreeNodePtr node) {
  ...
  switch (node_type) {
    ...
    default:
      throw Exception(ErrorCodes::UNSUPPORTED_METHOD, "Expected column, constant, function. Actual {} with type: {}",
...
PlannerActionsVisitorImpl::NodeNameAndNodeMinLevel PlannerActionsVisitorImpl::visitColumn(const QueryTreeNodePtr &node) {
  ...
  if (correlated_columns_set.contains(column_node_ptr)) return visitCorrelatedColumn(column_node_ptr);
...
PlannerActionsVisitorImpl::NodeNameAndNodeMinLevel PlannerActionsVisitorImpl::visitCorrelatedColumn(const ColumnNodePtr &node) {
  ...
  for (size_t i = 1; i < actions_stack.size(); ++i) actions_stack[i].addInputColumnIfNecessary(column_node_name, node->getColumnType());

  return {column_node_name, Levels(0)};
}

PlannerActionsVisitorImpl::NodeNameAndNodeMinLevel PlannerActionsVisitorImpl::visitConstant(const QueryTreeNodePtr &node,
                                                                                            const std::string &override_column_name) {
  const auto &constant_node = node->as<ConstantNode &>();
  ...
  auto constant_node_name = !override_column_name.empty() ? override_column_name : [&]() {
     ...
     */
    if (planner_context->isASTLevelOptimizationAllowed()) {
      return calculateActionNodeNameWithCastIfNeeded(
          constant_node, planner_context->getQueryContext()->getSettingsRef()[Setting::optimize_const_name_size]);
    }
    ...
    return calculateConstantActionNodeName(constant_node,
                                           planner_context->getQueryContext()->getSettingsRef()[Setting::optimize_const_name_size]);
  }();
...
PlannerActionsVisitorImpl::NodeNameAndNodeMinLevel PlannerActionsVisitorImpl::visitLambda(const QueryTreeNodePtr &node) {
  ...
  if (!result_type)
    throw Exception(ErrorCodes::LOGICAL_ERROR, "Lambda {} is not resolved during query analysis", lambda_node.formatASTForErrorMessage());
  ...
  auto function_capture = std::make_shared<FunctionCaptureOverloadResolver>(
      std::move(lambda_actions_dag), expression_actions_settings, captured_column_names, lambda_arguments_names_and_types,
      lambda_node.getExpression()->getResultType(), lambda_expression_node_name, true);
  ...
  if (!result_type->equals(*actions_node->result_type))
    throw Exception(ErrorCodes::LOGICAL_ERROR, "Lambda resolved type {} is not equal to type from actions DAG {}", result_type,
                    actions_node->result_type);
...
PlannerActionsVisitorImpl::NodeNameAndNodeMinLevel PlannerActionsVisitorImpl::makeSetForInFunction(const QueryTreeNodePtr &node) {
  ...
  bool subquery_or_table = in_second_argument_node_type == QueryTreeNodeType::QUERY ||
                           in_second_argument_node_type == QueryTreeNodeType::UNION ||
                           in_second_argument_node_type == QueryTreeNodeType::TABLE;

  bool in_second_is_deterministic = false;
  if (const auto *const_node = in_second_argument->as<const ConstantNode>()) in_second_is_deterministic = const_node->isDeterministic();
  ...
  if (!subquery_or_table) {
    ...
    if (left_tuple_type && left_tuple_type->getElements().size() > 1) set_element_types = left_tuple_type->getElements();

    set_element_types = Set::getElementTypes(std::move(set_element_types),
                                             planner_context->getQueryContext()->getSettingsRef()[Setting::transform_null_in]);
    ...
    if (!set) set = planner_context->getPreparedSets().findStorage(set_key);
  }

  if (!set)
    throw Exception(ErrorCodes::LOGICAL_ERROR, "No set is registered for key {}", PreparedSets::toString(set_key, set_element_types));
...
PlannerActionsVisitorImpl::NodeNameAndNodeMinLevel PlannerActionsVisitorImpl::visitIndexHintFunction(const QueryTreeNodePtr &node) {
  ...
  for (const auto &argument : function_node.getArguments()) {
    ...
    for (auto &expression_dag_node : index_hint_argument_expression_dag_nodes) {
      if (index_hint_actions_dag_output_node_names.contains(expression_dag_node->result_name)) continue;
...
PlannerActionsVisitorImpl::NodeNameAndNodeMinLevel PlannerActionsVisitorImpl::visitExistsFunction(const QueryTreeNodePtr &node) {
  ...
  const QueryTreeNodes &correlated_columns =
      query_node ? query_node->getCorrelatedColumns().getNodes() : union_node->getCorrelatedColumns().getNodes();
  ...
  correlated_subtrees.subqueries.emplace_back(function_node.getArguments().getNodes().front(), CorrelatedSubqueryKind::EXISTS,
                                              function_node_name, std::move(correlated_column_identifiers));
...
PlannerActionsVisitorImpl::NodeNameAndNodeMinLevel PlannerActionsVisitorImpl::visitFunction(const QueryTreeNodePtr &node) {
  ...
  if (function_node.getFunctionName() == "indexHint") return visitIndexHintFunction(node);
  if (function_node.getFunctionName() == "exists") return visitExistsFunction(node);

  std::optional<NodeNameAndNodeMinLevel> in_function_second_argument_node_name_with_level;

  if (isNameOfInFunction(function_node.getFunctionName())) in_function_second_argument_node_name_with_level = makeSetForInFunction(node);
  ...
  if (function_node.getFunctionName() == "arrayJoin") {
    if (level != 0)
      throw Exception(ErrorCodes::BAD_ARGUMENTS, "Expression in arrayJoin cannot depend on lambda argument: {} ",
    ...
    if (!argument_types_match) {
      if (auto resolver = FunctionFactory::instance().tryGet(function_node.getFunctionName(), planner_context->getQueryContext()))
...
PlannerActionsVisitorImpl::NodeNameAndNodeMinLevel PlannerActionsVisitorImpl::visitQuery(const QueryTreeNodePtr &node) {
  auto &query_node = node->as<QueryNode &>();
  if (!query_node.isCorrelated())
    throw Exception(ErrorCodes::LOGICAL_ERROR, "Only correlated QueryNode can be used as an action node, but got: {}",
                    node->formatASTForErrorMessage());
  ...
  correlated_subtrees.subqueries.emplace_back(node, CorrelatedSubqueryKind::SCALAR, correlated_subquery_name,
...
std::pair<ActionsDAG::NodeRawConstPtrs, CorrelatedSubtrees> PlannerActionsVisitor::visit(ActionsDAG &actions_dag,
                                                                                         QueryTreeNodePtr expression_node) {
  PlannerActionsVisitorImpl actions_visitor_impl(actions_dag, planner_context, correlated_columns_set,
...
String calculateWindowNodeActionName(const QueryTreeNodePtr &function_node, const QueryTreeNodePtr &window_node,
                                     const PlannerContext &planner_context, bool use_column_identifier_as_action_node_name) {
  QueryTreeNodeToName empty_map;
  ActionNodeNameHelper helper(empty_map, planner_context, use_column_identifier_as_action_node_name);
