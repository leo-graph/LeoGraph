...
static bool canUseTableForParallelReplicas(const TableNode &table_node, const ContextPtr &context [[maybe_unused]]) {
  ...
  if (mv) {
    if (!context->getSettingsRef()[Setting::parallel_replicas_allow_materialized_views]) return false;
  ...
  if (!storage->isMergeTree() && !typeid_cast<const StorageDummy *>(storage.get())) return false;

  if (!storage->supportsReplication() && !context->getSettingsRef()[Setting::parallel_replicas_for_non_replicated_merge_tree]) return false;

  /// Parallel replicas not supported with FINAL.
  if (table_node.hasTableExpressionModifiers() && table_node.getTableExpressionModifiers()->hasFinal()) return false;
  ...
  while (query_tree_node) {
    ...
    switch (join_tree_node_type) {
      case QueryTreeNodeType::TABLE: {
        ...
        if (canUseTableForParallelReplicas(table_node, context)) return res;
      ...
      case QueryTreeNodeType::JOIN: {
        ...
        std::unordered_set<QueryTreeNodeType> supported_table_expression_types = {QueryTreeNodeType::TABLE, QueryTreeNodeType::QUERY,
...
const QueryNode *findQueryForParallelReplicas(std::vector<const QueryNode *> stack,
  ...
  while (!stack.empty()) {
    ...
    while (!nodes_to_check.empty()) {
      ...
      if (children.empty()) {
        ...
        if (!join && !join_logical) return res;

        for (const auto &child : children) nodes_to_check.push({.node = child, .inside_join = true});
      }
    }
...
const QueryNode *findQueryForParallelReplicas(const QueryTreeNodePtr &query_tree_node, const SelectQueryOptions &select_query_options) {
  if (select_query_options.only_analyze) return nullptr;
  ...
  if (!context->canUseParallelReplicasOnInitiator()) return nullptr;
  ...
  if (stack.back() == query_tree_node.get()) return nullptr;
...
static const TableNode *findTableForParallelReplicas(const IQueryTreeNode *query_tree_node, const ContextPtr &context) {
  ...
  while (query_tree_node || !join_nodes.empty()) {
    ...
    switch (join_tree_node_type) {
      case QueryTreeNodeType::TABLE: {
        const auto &table_node = query_tree_node->as<TableNode &>();
        if (canUseTableForParallelReplicas(table_node, context)) return &table_node;
      ...
      case QueryTreeNodeType::UNION: {
        ...
        query_tree_node = nullptr;
        if (!union_queries.empty()) query_tree_node = union_queries.front().get();
...
const TableNode *findTableForParallelReplicas(const QueryTreeNodePtr &query_tree_node, const SelectQueryOptions &select_query_options) {
  ...
  if (!context->getSettingsRef()[Setting::serialize_query_plan] && !context->canUseParallelReplicasOnFollower()) return nullptr;
...
JoinTreeQueryPlan buildQueryPlanForParallelReplicas(const QueryNode &query_node, const PlannerContextPtr &planner_context,
  ...
  if (!table_node) throw Exception(ErrorCodes::LOGICAL_ERROR, "Can't determine table for parallel replicas");

  QueryPlan query_plan;
  ClusterProxy::executeQueryWithParallelReplicas(query_plan, table_node->getStorageID(), header, processed_stage, modified_query_ast,
                                                 std::move(modified_query_tree), std::move(new_planner_context), context, storage_limits,
                                                 nullptr);

  auto converting = ActionsDAG::makeConvertingActions(header->getColumnsWithTypeAndName(), initial_header->getColumnsWithTypeAndName(),
                                                      ActionsDAG::MatchColumnsMode::Position, context, false /*ignore_constant_values*/,
