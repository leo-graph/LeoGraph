...
std::optional<FilterAnalysisResult> analyzeFilter(const QueryTreeNodePtr &filter_expression_node,
                                                  const ColumnsWithTypeAndName &input_columns, const PlannerContextPtr &planner_context,
                                                  const ColumnNodePtrWithHashSet &correlated_columns_set, ActionsChain &actions_chain) {
  FilterAnalysisResult result;

  auto [filter_expression_dag, correlated_subtrees] =
      buildActionsDAGFromExpressionNode(filter_expression_node, input_columns, planner_context, correlated_columns_set);
  ...
  if (output->column && ConstantFilterDescription(*output->column).always_true) return {};
...
bool canRemoveConstantFromGroupByKey(const ConstantNode &root) {
  ...
  if (!source_expression) return true;
  ...
  while (!nodes.empty()) {
    ...
    if (constant_node) {
      if (!canRemoveConstantFromGroupByKey(*constant_node)) return false;
      ...
      if (function_node->getFunctionOrThrow()->isServerConstant()) return false;

      for (const auto &child : function_node->getArguments()) nodes.push(child.get());
    }
    // else
    //     return false;
  }
 ...
 */
std::optional<AggregationAnalysisResult> analyzeAggregation(const QueryTreeNodePtr &query_tree, const ColumnsWithTypeAndName &input_columns,
  ...
  for (auto &aggregate_description : aggregates_descriptions)
    available_columns_after_aggregation.emplace_back(nullptr, aggregate_description.function->getResultType(),
                                                     aggregate_description.column_name);
    ...
    if (query_node.isGroupByWithGroupingSets()) {
      for (auto &grouping_set_keys_list_node : query_node.getGroupBy().getNodes()) {
        ...
        for (auto &grouping_set_key_node : grouping_set_keys_list_node_typed.getNodes()) {
          ...
          if (constant_key && !aggregates_descriptions.empty() &&
          ...
          for (auto &expression_dag_node : expression_dag_nodes) {
            ...
            if (before_aggregation_actions_output_node_names.contains(expression_dag_node->result_name)) continue;

            auto expression_type_after_aggregation =
                group_by_use_nulls ? makeNullableSafe(expression_dag_node->result_type) : expression_dag_node->result_type;
            auto column_after_aggregation = group_by_use_nulls && expression_dag_node->column != nullptr
                                                ? makeNullableSafe(expression_dag_node->column)
                                                : expression_dag_node->column;
            available_columns_after_aggregation.emplace_back(std::move(column_after_aggregation), expression_type_after_aggregation,
                                                             expression_dag_node->result_name);
      ...
      for (auto &grouping_sets_parameter : grouping_sets_parameters_list) {
        ...
        for (auto &key : grouping_sets_parameter.used_keys) {
          ...
          if (inserted) grouping_sets_keys.push_back(key);
        }

        for (auto &key : aggregation_keys) {
          if (grouping_sets_used_keys.contains(key)) continue;
  ...
  for (auto &aggregate_function_node : aggregate_function_nodes) {
    ...
    for (const auto &aggregate_function_node_argument : aggregate_function_node_typed.getArguments().getNodes()) {
      auto [expression_dag_nodes, correlated_subtrees] =
          actions_visitor.visit(before_aggregation_actions->dag, aggregate_function_node_argument);
  ...
  if (aggregation_keys.empty() && aggregates_descriptions.empty()) return {};
  ...
  auto aggregate_step = std::make_unique<ActionsChainStep>(before_aggregation_actions, false /*use_actions_nodes_as_output_columns*/,
 ...
 */
std::optional<WindowAnalysisResult> analyzeWindow(const QueryTreeNodePtr &query_tree, const ColumnsWithTypeAndName &input_columns,
                                                  const PlannerContextPtr &planner_context,
                                                  const ColumnNodePtrWithHashSet &correlated_columns_set, ActionsChain &actions_chain) {
  auto window_function_nodes = collectWindowFunctionNodes(query_tree);
  if (window_function_nodes.empty()) return {};
  ...
  for (auto &window_function_node : window_function_nodes) {
    ...
    auto [expression_dag_nodes, correlated_subtrees] =
        actions_visitor.visit(before_window_actions->dag, window_function_node_typed.getArgumentsNode());
    correlated_subtrees.assertEmpty("in window function arguments");

    for (auto &expression_dag_node : expression_dag_nodes) {
      if (before_window_actions_output_node_names.contains(expression_dag_node->result_name)) continue;
    ...
    std::tie(expression_dag_nodes, correlated_subtrees) =
        actions_visitor.visit(before_window_actions->dag, window_node.getPartitionByNode());
    ...
    for (auto &sort_node : order_by_node_list.getNodes()) {
      ...
      std::tie(expression_dag_nodes, correlated_subtrees) =
          actions_visitor.visit(before_window_actions->dag, sort_node_typed.getExpression());
  ...
  for (auto &window_description : window_descriptions) {
    for (auto &window_function : window_description.window_functions) {
      ...
      if (types_changed) {
        ...
        auto new_function =
            AggregateFunctionFactory::instance().get(window_function.aggregate_function->getName(), NullsAction::EMPTY,
                                                     actual_argument_types, window_function.function_parameters, properties);
  ...
  for (auto &window_description : window_descriptions)
    for (auto &window_function : window_description.window_functions)
      window_functions_additional_columns.emplace_back(nullptr, window_function.aggregate_function->getResultType(),
                                                       window_function.column_name);

  auto before_window_step = std::make_unique<ActionsChainStep>(before_window_actions, true /*use_actions_nodes_as_output_columns*/,
 ...
 */
ProjectionAnalysisResult analyzeProjection(const QueryNode &query_node, const ColumnsWithTypeAndName &input_columns,
                                           const PlannerContextPtr &planner_context, const ColumnNodePtrWithHashSet &correlated_columns_set,
                                           ActionsChain &actions_chain) {
  auto [projection_actions_dag, correlated_subtrees] =
      buildActionsDAGFromExpressionNode(query_node.getProjectionNode(), input_columns, planner_context, correlated_columns_set);
  ...
  if (projection_columns_size != projection_outputs_size)
    throw Exception(ErrorCodes::LOGICAL_ERROR, "QueryTree projection nodes size mismatch. Expected {}. Actual {}", projection_columns_size,
 ...
 */
SortAnalysisResult analyzeSort(const QueryNode &query_node, const ColumnsWithTypeAndName &input_columns,
                               const PlannerContextPtr &planner_context, const ColumnNodePtrWithHashSet &correlated_columns_set,
                               ActionsChain &actions_chain) {
  ...
  for (const auto &sort_node : order_by_node_list.getNodes()) {
    ...
    for (auto &action_dag_node : expression_dag_nodes) {
      if (before_sort_actions_dag_output_node_names.contains(action_dag_node->result_name)) continue;
  ...
  if (has_with_fill) {
    for (auto &output_node : before_sort_actions_outputs) output_node = &before_sort_actions->dag.materializeNode(*output_node);
  }
  ...
  if (query_node.hasInterpolate()) {
    ...
    for (auto &interpolate_node : interpolate_list_node.getNodes()) {
      ...
      auto [nodes, correlated_subtrees] =
          interpolate_actions_visitor.visit(before_interpolate_actions_dag, interpolate_node_typed.getExpression());
      correlated_subtrees.assertEmpty("in expression to interpolate");
      if (nodes.size() != 1) throw Exception(ErrorCodes::LOGICAL_ERROR, "Interpolate expression must return single column");
 ...
 */
LimitByAnalysisResult analyzeLimitBy(const QueryNode &query_node, const ColumnsWithTypeAndName &input_columns,
                                     const PlannerContextPtr &planner_context, const NameSet &required_output_nodes_names,
                                     const ColumnNodePtrWithHashSet &correlated_columns_set, ActionsChain &actions_chain) {
  auto [before_limit_by_actions_dag, correlated_subtrees] =
      buildActionsDAGFromExpressionNode(query_node.getLimitByNode(), input_columns, planner_context, correlated_columns_set);
  ...
  for (const auto &node : before_limit_by_actions->dag.getNodes()) {
    if (required_output_nodes_names.contains(node.result_name) && !limit_by_column_names_set.contains(node.result_name))
...
PlannerExpressionsAnalysisResult buildExpressionAnalysisResult(const QueryTreeNodePtr &query_tree,
  ...
  if (query_node.hasWhere()) {
    where_analysis_result_optional =
        analyzeFilter(query_node.getWhere(), current_output_columns, planner_context, correlated_columns_set, actions_chain);
  ...
  auto aggregation_analysis_result_optional =
      analyzeAggregation(query_tree, current_output_columns, planner_context, correlated_columns_set, actions_chain);
  if (aggregation_analysis_result_optional) current_output_columns = actions_chain.getLastStepAvailableOutputColumns();
  ...
  if (query_node.hasHaving()) {
    having_analysis_result_optional =
        analyzeFilter(query_node.getHaving(), current_output_columns, planner_context, correlated_columns_set, actions_chain);
  ...
  auto window_analysis_result_optional =
      analyzeWindow(query_tree, current_output_columns, planner_context, correlated_columns_set, actions_chain);
  if (window_analysis_result_optional) current_output_columns = actions_chain.getLastStepAvailableOutputColumns();
  ...
  if (query_node.hasQualify()) {
    qualify_analysis_result_optional =
        analyzeFilter(query_node.getQualify(), current_output_columns, planner_context, correlated_columns_set, actions_chain);
  ...
  auto projection_analysis_result =
      analyzeProjection(query_node, current_output_columns, planner_context, correlated_columns_set, actions_chain);
  current_output_columns = actions_chain.getLastStepAvailableOutputColumns();

  std::optional<SortAnalysisResult> sort_analysis_result_optional;
  if (query_node.hasOrderBy()) {
    sort_analysis_result_optional = analyzeSort(query_node, current_output_columns, planner_context, correlated_columns_set, actions_chain);
    current_output_columns = actions_chain.getLastStepAvailableOutputColumns();
  }
  ...
  if (query_node.hasLimitBy()) {
    ...
    if (sort_analysis_result_optional.has_value() &&
        (sort_analysis_result_optional->has_with_fill ||
         (planner_query_processing_info.isFirstStage() && planner_query_processing_info.getToStage() != QueryProcessingStage::Complete))) {
    ...
    limit_by_analysis_result_optional = analyzeLimitBy(query_node, current_output_columns, planner_context, required_output_nodes_names,
                                                       correlated_columns_set, actions_chain);
    current_output_columns = actions_chain.getLastStepAvailableOutputColumns();
  }
  ...
  if (planner_query_processing_info.isToAggregationState()) {
    for (auto &column : project_names_input) column.column = nullptr;
  }
  ...
  if (has_with_fill || query_node.isDistinct()) {
    ...
    for (auto &column : project_names_input) {
      if (has_with_fill || projection_column_names.contains(column.name)) column.column = nullptr;
    }
  }
  ...
  if (query_node.hasInterpolate()) {
    ...
    for (auto &[name, alias] : project_names)
      if (interpolate_names.contains(alias)) name = alias;
  ...
  if (where_analysis_result_optional && where_action_step_index_optional) {
    ...
    where_analysis_result.remove_filter_column =
        !where_actions_chain_node->getChildRequiredOutputColumnsNames().contains(where_analysis_result.filter_column_name);
    expressions_analysis_result.addWhere(std::move(where_analysis_result));
  }

  if (aggregation_analysis_result_optional) expressions_analysis_result.addAggregation(std::move(*aggregation_analysis_result_optional));
  ...
  if (having_analysis_result_optional && having_action_step_index_optional) {
    ...
    having_analysis_result.remove_filter_column =
        !having_actions_chain_node->getChildRequiredOutputColumnsNames().contains(having_analysis_result.filter_column_name);
    expressions_analysis_result.addHaving(std::move(having_analysis_result));
  }

  if (window_analysis_result_optional) expressions_analysis_result.addWindow(std::move(*window_analysis_result_optional));
  ...
  if (qualify_analysis_result_optional && qualify_action_step_index_optional) {
    ...
    qualify_analysis_result.remove_filter_column =
        !qualify_actions_chain_node->getChildRequiredOutputColumnsNames().contains(qualify_analysis_result.filter_column_name);
    expressions_analysis_result.addQualify(std::move(qualify_analysis_result));
  }

  if (sort_analysis_result_optional) expressions_analysis_result.addSort(std::move(*sort_analysis_result_optional));

  if (limit_by_analysis_result_optional) expressions_analysis_result.addLimitBy(std::move(*limit_by_analysis_result_optional));
