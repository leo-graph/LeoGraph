...
WindowDescription extractWindowDescriptionFromWindowNode(const QueryTreeNodePtr &func_node_, const PlannerContext &planner_context) {
  ...
  window_description.full_sort_description.insert(window_description.full_sort_description.end(), window_description.order_by.begin(),
...
std::vector<WindowDescription> extractWindowDescriptions(const QueryTreeNodes &window_function_nodes,
  ...
  for (auto &&[_, window_description] : window_name_to_description) result.push_back(std::move(window_description));
  ...
  auto window_description_comparator = [](const WindowDescription &lhs, const WindowDescription &rhs) -> bool {
    ...
    for (size_t i = 0; i < std::min(left.size(), right.size()); ++i) {
      ...
      if (left[i].collator || right[i].collator) {
        if (!left[i].collator) return true;
        if (!right[i].collator) return false;
        if (left[i].collator->getLocale() < right[i].collator->getLocale()) return true;
        if (left[i].collator->getLocale() > right[i].collator->getLocale()) return false;
      }
