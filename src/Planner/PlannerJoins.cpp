...
void JoinClause::dump(WriteBuffer &buffer) const {
  ...
  if (!left_filter_condition_nodes.empty()) buffer << " left_condition_nodes: " + dump_dag_nodes(left_filter_condition_nodes);

  if (!right_filter_condition_nodes.empty()) buffer << " right_condition_nodes: " + dump_dag_nodes(right_filter_condition_nodes);
  ...
  if (!asof_conditions.empty()) {
    ...
    for (size_t i = 0; i < asof_conditions_size; ++i) {
      ...
      if (i + 1 != asof_conditions_size) buffer << ',';
    }
  }
}
...
JoinClause JoinClause::concatClauses(const JoinClause &lhs, const JoinClause &rhs) {
  const auto concat_ptrs_into = [](const ActionsDAG::NodeRawConstPtrs &lhs_ptrs, const ActionsDAG::NodeRawConstPtrs &rhs_ptrs,
  ...
  std::transform(rhs.asof_conditions.begin(), rhs.asof_conditions.end(), std::back_inserter(result.asof_conditions),
  ...
  // And offset the indices from rhs according to lhs size
  for (const auto key_index : rhs.nullsafe_compare_key_indexes) result.nullsafe_compare_key_indexes.insert(key_index + lhs_key_size);
...
TableExpressionSet extractTableExpressionsSet(const QueryTreeNodePtr &node) {
  ...
  for (const auto &expr : extractTableExpressions(node, true)) res.insert(expr.get());

  return res;
}

std::set<JoinTableSide> extractJoinTableSidesFromExpression(const IQueryTreeNode *expression_root_node,
                                                            const TableExpressionSet &left_table_expressions,
                                                            const TableExpressionSet &right_table_expressions, const JoinNode &join_node) {
  ...
  while (!nodes_to_process.empty()) {
    ...
    if (const auto *function_node = node_to_process->as<FunctionNode>()) {
      for (const auto &child : function_node->getArguments()) nodes_to_process.push_back(child.get());

      continue;
    }

    const auto *column_node = node_to_process->as<ColumnNode>();
    if (!column_node) continue;
    ...
    if (!is_column_from_left_expr && !is_column_from_right_expr)
      throw Exception(ErrorCodes::INVALID_JOIN_ON_EXPRESSION,
                      ...
                      join_node.formatASTForErrorMessage(), column_source->formatASTForErrorMessage(),
...
const ActionsDAG::Node *appendExpression(ActionsDAG &dag, const QueryTreeNodePtr &expression, const PlannerContextPtr &planner_context,
  ...
  if (join_expression_dag_node_raw_pointers.size() != 1)
    throw Exception(ErrorCodes::LOGICAL_ERROR, "JOIN {} ON clause contains multiple expressions", join_node.formatASTForErrorMessage());

  return join_expression_dag_node_raw_pointers[0];
}

void buildJoinClauseImpl(ActionsDAG &left_dag, ActionsDAG &right_dag, ActionsDAG &joined_dag, const PlannerContextPtr &planner_context,
                         const QueryTreeNodePtr &join_expression, const TableExpressionSet &left_table_expressions,
                         const TableExpressionSet &right_table_expressions, const JoinNode &join_node, const bool is_simple,
  ...
  if (function_name == "equals" || function_name == "isNotDistinctFrom" || is_asof_join_inequality) {
    ...
    if (left_expression_sides.empty() && right_expression_sides.empty()) {
      throw Exception(ErrorCodes::INVALID_JOIN_ON_EXPRESSION, "JOIN {} ON expression expected non-empty left and right table expressions",
      ...
      if (left_expression_side != right_expression_side) {
        if (is_simple && (function_name == "or" || function_name == "and"))
          throw Exception(ErrorCodes::LOGICAL_ERROR,
                          "Cannot build simple join clause for '{}' expression containing expressions from both tables", function_name);
...
void buildSimpleJoinClause(ActionsDAG &left_dag, ActionsDAG &right_dag, ActionsDAG &joined_dag, const PlannerContextPtr &planner_context,
                           const QueryTreeNodePtr &join_expression, const TableExpressionSet &left_table_expressions,
                           const TableExpressionSet &right_table_expressions, const JoinNode &join_node, JoinClause &join_clause) {
  buildJoinClauseImpl(left_dag, right_dag, joined_dag, planner_context, join_expression, left_table_expressions, right_table_expressions,
                      join_node, true, join_clause);
}

void buildJoinClause(ActionsDAG &left_dag, ActionsDAG &right_dag, ActionsDAG &joined_dag, const PlannerContextPtr &planner_context,
                     const QueryTreeNodePtr &join_expression, const TableExpressionSet &left_table_expressions,
                     const TableExpressionSet &right_table_expressions, const JoinNode &join_node, JoinClause &join_clause) {
  std::string function_name;
  auto *function_node = join_expression->as<FunctionNode>();
  ...
  if (function_name == "and") {
    for (const auto &child : function_node->getArguments()) {
      buildJoinClause(left_dag, right_dag, joined_dag, planner_context, child, left_table_expressions, right_table_expressions, join_node,
                      join_clause);
    }
  ...
  buildJoinClauseImpl(left_dag, right_dag, joined_dag, planner_context, join_expression, left_table_expressions, right_table_expressions,
                      join_node, false, join_clause);
}

JoinClauses buildJoinClauses(ActionsDAG &left_dag, ActionsDAG &right_dag, ActionsDAG &joined_dag, const PlannerContextPtr &planner_context,
                             const QueryTreeNodePtr &join_expression, const TableExpressionSet &left_table_expressions,
                             const TableExpressionSet &right_table_expressions, const JoinNode &join_node) {
  if (join_expression->getNodeType() != QueryTreeNodeType::FUNCTION) {
    JoinClauses result;
    result.emplace_back();
    buildSimpleJoinClause(left_dag, right_dag, joined_dag, planner_context, join_expression, left_table_expressions,
                          right_table_expressions, join_node, result.front());
    return result;
  }
  ...
  while (!nodes_to_process.empty()) {
    ...
    // If the expression is a logical expression and it contains expressions from both sides, let's combine the clauses, otherwise let's
    // just build one join clause
    if ((function_name == "and" || function_name == "or") && expression_sides.size() == 2) {
      ...
      if (const auto it = built_clauses.find(first_argument); it == built_clauses.end()) {
        for (auto &argument : arguments) nodes_to_process.push(argument);
      ...
      if (function_name == "or") {
        ...
        // When some expressions have key expressions and some doesn't, then let's plan the whole OR expression as a single clause to
        // eliminate the chance that some clauses might end up without key expressions
        // TODO(antaljanosbenjamin): Analyze the expressions first, so join clauses are not built unnecessarily.
        const auto with_key_expression = static_cast<size_t>(
            std::count_if(result.begin(), result.end(), [](const JoinClause &clause) { return !clause.getLeftKeyNodes().empty(); }));

        if (result.size() > 1 && with_key_expression != 0 && with_key_expression < result.size()) {
          result.clear();
          result.emplace_back();
          buildSimpleJoinClause(left_dag, right_dag, joined_dag, planner_context, node, left_table_expressions, right_table_expressions,
                                join_node, result.front());
        }
      } else {
...
std::pair<JoinClauses, bool /*is_inequal_join*/> buildAllJoinClauses(
    ActionsDAG &left_join_actions, ActionsDAG &right_join_actions, ActionsDAG &post_join_actions, const PlannerContextPtr &planner_context,
    const QueryTreeNodePtr &join_expression, const TableExpressionSet &join_left_table_expressions,
    const TableExpressionSet &join_right_table_expressions, const JoinNode &join_node, const FunctionNode &function_node) {
  ...
  if (is_hash_join_enabled && planner_context->getQueryContext()->getSettingsRef()[Setting::allow_general_join_planning]) {
    auto join_clauses = buildJoinClauses(left_join_actions, right_join_actions, post_join_actions, planner_context, join_expression,
                                         join_left_table_expressions, join_right_table_expressions, join_node);

    const auto has_residual_filters = std::any_of(join_clauses.begin(), join_clauses.end(), [](const JoinClause &clause) {
      return !clause.getResidualFilterConditionNodes().empty();
    });
  ...
  if (function_name == "or") {
    for (const auto &child : function_node.getArguments()) {
      ...
      buildJoinClause(left_join_actions, right_join_actions, post_join_actions, planner_context, child, join_left_table_expressions,
                      join_right_table_expressions, join_node, join_clauses.back());
      has_residual_filters |= !join_clauses.back().getResidualFilterConditionNodes().empty();
    }
  } else {
    join_clauses.emplace_back();

    buildJoinClause(left_join_actions, right_join_actions, post_join_actions, planner_context, join_expression, join_left_table_expressions,
                    join_right_table_expressions, join_node, join_clauses.back());
    has_residual_filters |= !join_clauses.back().getResidualFilterConditionNodes().empty();
  }
  return std::make_pair(std::move(join_clauses), has_residual_filters);
...
JoinClausesAndActions buildJoinClausesAndActions(const ColumnsWithTypeAndName &left_table_expression_columns,
                                                 const ColumnsWithTypeAndName &right_table_expression_columns, const JoinNode &join_node,
  ...
  auto *function_node = join_expression->as<FunctionNode>();
  if (!function_node)
    throw Exception(ErrorCodes::INVALID_JOIN_ON_EXPRESSION, "JOIN {} join expression expected function",
                    join_node.formatASTForErrorMessage());
  ...
  std::tie(result.join_clauses, has_residual_filters) =
      buildAllJoinClauses(left_join_actions, right_join_actions, post_join_actions, planner_context, join_expression,
                          join_left_table_expressions, join_right_table_expressions, join_node, *function_node);
  ...
  auto add_necessary_name_if_needed = [&](JoinTableSide join_table_side, const String &name) {
    ...
    if (inserted) necessary_names.push_back(name);
  };
  ...
  for (auto &join_clause : result.join_clauses) {
    ...
    for (size_t i = 0; i < join_clause_key_nodes_size; ++i) {
      ...
      if (!left_key_node->result_type->equals(*right_key_node->result_type)) {
        ...
        try {
          ...
          ex.addMessage("JOIN {} cannot infer common type in ON section for keys. Left key {} type {}. Right key {} type {}",
                        join_node.formatASTForErrorMessage(), left_key_node->result_name, left_key_node->result_type->getName(),
                        right_key_node->result_name, right_key_node->result_type->getName());
          throw;
        }
      ...
      if (join_clause.isNullsafeCompareKey(i) && isNullableOrLowCardinalityNullable(left_key_node->result_type) &&
          isNullableOrLowCardinalityNullable(right_key_node->result_type)) {
        /**
  ...
  if (has_residual_filters) {
    ...
    if (result.join_clauses.size() > 1) {
      ...
      ColumnNodePtrWithHashSet empty_correlated_columns_set;
      PlannerActionsVisitor join_expression_visitor(planner_context, empty_correlated_columns_set);
      auto [join_expression_dag_node_raw_pointers, correlated_subtrees] =
          join_expression_visitor.visit(residual_join_expressions_actions, join_expression);
    ...
    if (outputs.size() != 1) {
      throw Exception(ErrorCodes::LOGICAL_ERROR, "Only one output is expected, got: {}",
                      result.residual_join_expressions_actions->dumpDAG());
    }
...
JoinClausesAndActions buildJoinClausesAndActions(const ColumnsWithTypeAndName &left_table_expression_columns,
                                                 ...
                                                 const QueryTreeNodePtr &join_node, const PlannerContextPtr &planner_context) {
  auto &join_node_typed = join_node->as<JoinNode &>();
  if (!join_node_typed.isOnJoinExpression())
    throw Exception(ErrorCodes::LOGICAL_ERROR, "JOIN {} join does not have ON section", join_node_typed.formatASTForErrorMessage());
...
std::optional<bool> tryExtractConstantFromJoinNode(const QueryTreeNodePtr &join_node) {
  auto &join_node_typed = join_node->as<JoinNode &>();
  if (!join_node_typed.getJoinExpression()) return {};
...
void trySetStorageInTableJoin(const QueryTreeNodePtr &table_expression, std::shared_ptr<TableJoin> &table_join) {
  ...
  if (!table_join->isEnabledAlgorithm(JoinAlgorithm::DIRECT) && !table_join->isEnabledAlgorithm(JoinAlgorithm::DEFAULT)) return;
...
std::shared_ptr<DirectKeyValueJoin> tryDirectJoin(const std::shared_ptr<TableJoin> &table_join,
  ...
  if (!table_join->isEnabledAlgorithm(JoinAlgorithm::DIRECT)) return {};
  ...
  bool allowed_left =
      isLeft(table_join->kind()) && (table_join->strictness() == JoinStrictness::Any || table_join->strictness() == JoinStrictness::All ||
                                     table_join->strictness() == JoinStrictness::Semi || table_join->strictness() == JoinStrictness::Anti);
  if (!allowed_inner && !allowed_left) return {};
  ...
  if (!only_one_key) return {};

  const String &key_name = clauses[0].key_names_right[0];

  if (auto table_column_name_it = right_table_expression.column_mapping.find(key_name);
      table_column_name_it != right_table_expression.column_mapping.end()) {
    const auto &storage_primary_key = storage->getPrimaryKey();
    if (storage_primary_key.size() != 1 || storage_primary_key[0] != table_column_name_it->second) return {};
  } else {
    return {};
  }
   ...
   *
   * Example:
   * CREATE DICTIONARY test_dictionary (id UInt64, value String) PRIMARY KEY id SOURCE(CLICKHOUSE(TABLE 'test_dictionary_table'))
   * LIFETIME(0); SELECT t1.id FROM test_table AS t1 INNER JOIN test_dictionary AS t2 ON t1.id = t2.id;
   *
  ...
  for (const auto &right_table_expression_column : right_table_expression_header_) {
    ...
    if (table_column_name_it == right_table_expression.column_mapping.end()) return {};
  ...
  return std::make_shared<DirectKeyValueJoin>(table_join, right_table_expression_header_, storage,
                                              right_table_expression_header_with_storage_column_names);
}
...
QueryTreeNodePtr getJoinExpressionFromNode(const JoinNode &join_node) {
  ...
  if (!join_expression) return nullptr;
  const auto *constant_join_expression = join_expression->as<ConstantNode>();
  if (constant_join_expression && constant_join_expression->hasSourceExpression()) return constant_join_expression->getSourceExpression();
  return join_expression;
}

static std::shared_ptr<IJoin> tryCreateJoin(JoinAlgorithm algorithm, std::shared_ptr<TableJoin> &table_join,
                                            const PreparedJoinStorage &right_table_expression, SharedHeader &left_table_expression_header,
                                            SharedHeader &right_table_expression_header, const JoinAlgorithmParams &params) {
  if (table_join->kind() == JoinKind::Paste) return std::make_shared<PasteJoin>(table_join, right_table_expression_header);
  ...
  if (algorithm == JoinAlgorithm::DIRECT || algorithm == JoinAlgorithm::DEFAULT) {
    ...
    if (direct_join) return direct_join;
  }

  if (algorithm == JoinAlgorithm::PARTIAL_MERGE || algorithm == JoinAlgorithm::PREFER_PARTIAL_MERGE) {
    if (MergeJoin::isSupported(table_join)) return std::make_shared<MergeJoin>(table_join, right_table_expression_header);
  }
  ...
  if (algorithm == JoinAlgorithm::HASH ||
    ...
    if (table_join->allowParallelHashJoin()) {
      ...
      if (use_parallel_hash) {
        StatsCollectingParams stats_collecting_params{params.hash_table_key_hash, params.collect_hash_table_stats_during_joins,
                                                      params.max_entries_for_hash_table_stats, params.max_size_to_preallocate_for_joins};
  ...
  if (algorithm == JoinAlgorithm::GRACE_HASH) {
    if (GraceHashJoin::isSupported(table_join)) {
      return std::make_shared<GraceHashJoin>(params.grace_hash_join_initial_buckets, params.grace_hash_join_max_buckets, table_join,
                                             left_table_expression_header, right_table_expression_header,
                                             Context::getGlobalContextInstance()->getTempDataOnDisk());
    }
  }

  if (algorithm == JoinAlgorithm::AUTO) {
    if (MergeJoin::isSupported(table_join)) return std::make_shared<JoinSwitcher>(table_join, right_table_expression_header);
...
JoinAlgorithmParams::JoinAlgorithmParams(const JoinSettings &join_settings, UInt64 max_threads_, UInt64 hash_table_key_hash_,
                                         UInt64 max_entries_for_hash_table_stats_, String initial_query_id_,
...
std::shared_ptr<IJoin> chooseJoinAlgorithm(std::shared_ptr<TableJoin> &table_join, const PreparedJoinStorage &right_table_expression,
                                           SharedHeader left_table_expression_header, SharedHeader right_table_expression_header,
                                           const JoinAlgorithmParams &params) {
  if (table_join->getMixedJoinExpression() && !table_join->isEnabledAlgorithm(JoinAlgorithm::HASH) &&
      !table_join->isEnabledAlgorithm(JoinAlgorithm::PARALLEL_HASH) && !table_join->isEnabledAlgorithm(JoinAlgorithm::GRACE_HASH)) {
    throw Exception(ErrorCodes::NOT_IMPLEMENTED, "JOIN with mixed conditions supports only hash join or grace hash join");
  }
   ...
   */
  if (table_join->kind() == JoinKind::Cross) return std::make_shared<HashJoin>(table_join, right_table_expression_header);

  if (!table_join->oneDisjunct() && !table_join->isEnabledAlgorithm(JoinAlgorithm::HASH) &&
      !table_join->isEnabledAlgorithm(JoinAlgorithm::AUTO))
  ...
  for (auto algorithm : table_join->getEnabledJoinAlgorithms()) {
    auto join =
        tryCreateJoin(algorithm, table_join, right_table_expression, left_table_expression_header, right_table_expression_header, params);
    if (join) return join;
  }
