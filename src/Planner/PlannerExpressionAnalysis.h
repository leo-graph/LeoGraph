...
class PlannerExpressionsAnalysisResult {
 public:
  ...
  ProjectionAnalysisResult& getProjection() { return projection_analysis_result; }

  bool hasWhere() const { return where_analysis_result.filter_actions != nullptr; }

  FilterAnalysisResult& getWhere() { return where_analysis_result; }

  void addWhere(FilterAnalysisResult where_analysis_result_) { where_analysis_result = std::move(where_analysis_result_); }

  bool hasAggregation() const {
    return !aggregation_analysis_result.aggregation_keys.empty() || !aggregation_analysis_result.aggregate_descriptions.empty();
  }

  AggregationAnalysisResult& getAggregation() { return aggregation_analysis_result; }
  ...
  bool hasHaving() const { return having_analysis_result.filter_actions != nullptr; }

  FilterAnalysisResult& getHaving() { return having_analysis_result; }

  void addHaving(FilterAnalysisResult having_analysis_result_) { having_analysis_result = std::move(having_analysis_result_); }

  bool hasWindow() const { return !window_analysis_result.window_descriptions.empty(); }

  WindowAnalysisResult& getWindow() { return window_analysis_result; }

  void addWindow(WindowAnalysisResult window_analysis_result_) { window_analysis_result = std::move(window_analysis_result_); }

  bool hasQualify() const { return qualify_analysis_result.filter_actions != nullptr; }

  FilterAnalysisResult& getQualify() { return qualify_analysis_result; }

  void addQualify(FilterAnalysisResult qualify_analysis_result_) { qualify_analysis_result = std::move(qualify_analysis_result_); }

  bool hasSort() const { return sort_analysis_result.before_order_by_actions != nullptr; }

  SortAnalysisResult& getSort() { return sort_analysis_result; }

  void addSort(SortAnalysisResult sort_analysis_result_) { sort_analysis_result = std::move(sort_analysis_result_); }

  bool hasLimitBy() const { return limit_by_analysis_result.before_limit_by_actions != nullptr; }

  LimitByAnalysisResult& getLimitBy() { return limit_by_analysis_result; }

  void addLimitBy(LimitByAnalysisResult limit_by_analysis_result_) { limit_by_analysis_result = std::move(limit_by_analysis_result_); }
