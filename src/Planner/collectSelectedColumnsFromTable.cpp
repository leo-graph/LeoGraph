...
class CollectSelectedColumnsFromTableVisitor : public InDepthQueryTreeVisitorWithContext<CollectSelectedColumnsFromTableVisitor> {
 public:
  ...
  void enterImpl(QueryTreeNodePtr &node) {
    ...
    if (!source_table || source_table->getStorageID() != storage_id) return;
    ...
    /// for them.
    if (is_inside_index_hint_function && source_table->getStorage()->isMergeTree()) return;
  ...
  std::vector<String> getSelectedColumns() const { return std::vector<String>(selected_columns.begin(), selected_columns.end()); }
