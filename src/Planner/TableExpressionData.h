...
class TableExpressionData {
 public:
  ...
  bool hasColumn(const std::string& column_name) const { return column_name_to_column.contains(column_name); }
  ...
  void addColumn(const NameAndTypePair& column, const ColumnIdentifier& column_identifier, bool is_selected_column = true) {
    if (hasColumn(column.name)) throw Exception(ErrorCodes::LOGICAL_ERROR, "Column with name {} already exists", column.name);
  ...
  /// Add alias column
  void addAliasColumn(const NameAndTypePair& column, const ColumnIdentifier& column_identifier, ActionsDAG actions_dag,
                      bool is_selected_column = true) {
  ...
  void markSelectedColumn(const std::string& column_name) {
    ...
    if (inserted) selected_column_names.push_back(column_name);
  }

  /// Get columns that are requested from table expression, including ALIAS columns
  const Names& getSelectedColumnsNames() const { return selected_column_names; }

  /// Get ALIAS columns names mapped to expressions
  AliasColumnExpressions& getAliasColumnExpressions() { return alias_column_expressions; }

  const AliasColumnExpressions& getAliasColumnExpressions() const { return alias_column_expressions; }

  bool hasAliasColumn(const std::string& column_name) const { return alias_column_names_set.contains(column_name); }

  /// Get column name to column map
  const ColumnNameToColumn& getColumnNameToColumn() const { return column_name_to_column; }
  ...
  NamesAndTypes getColumns() const {
    ...
    for (const auto& column_name : column_names) result.push_back(column_name_to_column.at(column_name));

    return result;
  }

  /// Get column identifier to column name map
  const ColumnNameToColumnIdentifier& getColumnIdentifierToColumnName() const { return column_identifier_to_column_name; }
  ...
  const NameAndTypePair& getColumnOrThrow(const std::string& column_name) const {
    ...
    if (it == column_name_to_column.end()) {
      throw Exception(ErrorCodes::LOGICAL_ERROR, "Column for column name {} does not exist. There are only column names: {}", column_name,
  ...
  const ColumnIdentifier& getColumnIdentifierOrThrow(const std::string& column_name) const {
    ...
    if (it == column_name_to_column_identifier.end()) {
      throw Exception(ErrorCodes::LOGICAL_ERROR, "Column identifier for column name {} does not exist. There are only column names: {}",
                      column_name, fmt::join(column_names.begin(), column_names.end(), ", "));
    }
  ...
  const ColumnIdentifier* getColumnIdentifierOrNull(const std::string& column_name) const {
    auto it = column_name_to_column_identifier.find(column_name);
    if (it == column_name_to_column_identifier.end()) return nullptr;
  ...
  const std::string* getColumnNameOrNull(const ColumnIdentifier& column_identifier) const {
    ...
    if (it == column_identifier_to_column_name.end()) return nullptr;
   ...
   */
  bool isRemote() const { return is_remote; }

  /// Set is storage remote value
  void setIsRemote(bool is_remote_value) { is_remote = is_remote_value; }

  bool isMergeTree() const { return is_merge_tree; }

  void setIsMergeTree(bool is_merge_tree_value) { is_merge_tree = is_merge_tree_value; }

  const std::optional<ActionsDAG>& getPrewhereFilterActions() const { return prewhere_filter_actions; }
  ...
  const std::optional<ActionsDAG>& getRowLevelFilterActions() const { return row_level_filter_actions; }
  ...
  const std::optional<ActionsDAG>& getFilterActions() const { return filter_actions; }

  void setFilterActions(ActionsDAG filter_actions_value) { filter_actions = std::move(filter_actions_value); }

  const PrewhereInfoPtr& getPrewhereInfo() const { return prewhere_info; }

  void setPrewhereInfo(PrewhereInfoPtr prewhere_info_value) { prewhere_info = std::move(prewhere_info_value); }

 private:
  void addColumnImpl(const NameAndTypePair& column, const ColumnIdentifier& column_identifier, bool add_to_selected_columns) {
    if (add_to_selected_columns) markSelectedColumn(column.name);
