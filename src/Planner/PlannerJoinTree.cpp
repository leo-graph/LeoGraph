...
NameSet checkAccessRights(const TableNode &table_node, const Names &column_names, const ContextPtr &query_context) {
  ...
  if (typeid_cast<const StorageDummy *>(table_node.getStorage().get())) return {};
  ...
  if (column_names.empty()) {
    ...
    if (accessible_columns.empty()) {
      throw Exception(
          ...
          query_context->getUserName(), storage_id.getFullTableName());
    }
  ...
  if (storage_id.hasDatabase()) query_context->checkAccess(AccessType::SELECT, storage_id, column_names);
...
void checkAccessRightsForSubquery(const QueryTreeNodePtr &subquery_node, const ContextPtr &query_context) {
  ...
  for (const auto &table_node_ptr : table_nodes) {
    ...
    if (typeid_cast<const StorageDummy *>(table_node.getStorage().get())) continue;

    const auto &storage_id = table_node.getStorageID();
    if (storage_id.hasDatabase()) query_context->checkAccess(AccessType::SELECT, storage_id);
  }
}

bool shouldIgnoreQuotaAndLimits(const TableNode &table_node) {
  const auto &storage_id = table_node.getStorageID();
  if (!storage_id.hasDatabase()) return false;
  if (storage_id.database_name == DatabaseCatalog::SYSTEM_DATABASE) {
    static const boost::container::flat_set<std::string_view> tables_ignoring_quota{"quotas", "quota_limits", "quota_usage", "quotas_usage",
                                                                                    "one"};
    if (tables_ignoring_quota.contains(storage_id.table_name)) return true;
  }
  return false;
}

NameAndTypePair chooseSmallestColumnToReadFromStorage(const StoragePtr &storage, const StorageSnapshotPtr &storage_snapshot,
                                                      const NameSet &column_names_allowed_to_select) {
  ...
  class ColumnWithSize {
   public:
    ...
    bool operator<(const ColumnWithSize &rhs) const {
      return std::tie(compressed_size, type_size, uncompressed_size) < std::tie(rhs.compressed_size, rhs.type_size, rhs.uncompressed_size);
    }
  ...
  if (!column_sizes.empty()) {
    for (auto &column_name_and_type : column_names_and_types) {
      ...
      if (it == column_sizes.end()) continue;
...
bool applyTrivialCountIfPossible(QueryPlan &query_plan, SelectQueryInfo &select_query_info, const TableNode *table_node,
                                 const TableFunctionNode *table_function_node, const QueryTreeNodePtr &query_tree,
                                 ContextMutablePtr &query_context, const Names &columns_names) {
  const auto &settings = query_context->getSettingsRef();
  if (!settings[Setting::optimize_trivial_count_query]) return false;

  const auto &storage = table_node ? table_node->getStorage() : table_function_node->getStorage();
  if (!storage->supportsTrivialCountOptimization(table_node ? table_node->getStorageSnapshot() : table_function_node->getStorageSnapshot(),
                                                 query_context))
    return false;
  ...
  if (row_policy_filter) return {};

  if (select_query_info.additional_filter_ast) return false;

  /** Transaction check here is necessary because
   * MergeTree maintains total count for all parts in Active state and it simply returns that number for trivial select count() from table
   * query. But if we have current transaction, then we should return number of rows in current snapshot (that may include parts in Outdated
   * state), so we have to use totalRowsByPartitionPredicate() instead of totalRows even for trivial query See
   * https://github.com/ClickHouse/ClickHouse/pull/24258/files#r828182031
   */
  if (query_context->getCurrentTransaction()) return false;
  ...
  if (main_query_node.hasGroupBy() || main_query_node.hasPrewhere() || main_query_node.hasWhere()) return false;
  ...
  if (aggregates.size() != 1) return false;
  ...
  argument_types.reserve(columns_names.size());
  {
    ...
    for (const auto &column_name : columns_names) argument_types.push_back(source_header.getByName(column_name).type);
  }

  auto block_with_count = std::make_shared<const Block>(
      Block{{std::move(column), std::make_shared<DataTypeAggregateFunction>(function_node.getAggregateFunction(), argument_types, Array{}),
             columns_names.front()}});
...
void prepareBuildQueryPlanForTableExpression(const QueryTreeNodePtr &table_expression, const SelectQueryOptions &select_query_options,
  ...
  if (settings[Setting::max_columns_to_read] && columns_names.size() > settings[Setting::max_columns_to_read])
    throw Exception(ErrorCodes::TOO_MANY_COLUMNS, "Limit for number of columns to read exceeded. Requested: {}, maximum: {}",
                    columns_names.size(), settings[Setting::max_columns_to_read].value);
}

void updatePrewhereOutputsIfNeeded(SelectQueryInfo &table_expression_query_info, const Names &column_names,
                                   const StorageSnapshotPtr &storage_snapshot) {
  if (!table_expression_query_info.prewhere_info) return;

  auto &prewhere_actions = table_expression_query_info.prewhere_info->prewhere_actions;

  NameSet required_columns;
  if (column_names.size() == 1) required_columns.insert(column_names[0]);
  ...
  if (table_expression_modifiers) {
    if (table_expression_modifiers->hasSampleSizeRatio() ||
      ...
      if (storage_snapshot->storage.getName() == "Merge") {
        ...
        for (const auto &column : columns) required_columns.insert(column.name);
      } else {
  ...
  for (const auto *input : prewhere_actions.getInputs()) {
    if (required_columns.contains(input->result_name)) required_output_nodes.insert(input);
  }

  if (required_output_nodes.empty()) return;
  ...
  for (const auto &output : prewhere_outputs) {
    ...
    if (required_output_node_it == required_output_nodes.end()) continue;
...
std::optional<FilterDAGInfo> buildRowPolicyFilterIfNeeded(const StoragePtr &storage, SelectQueryInfo &table_expression_query_info,
                                                          PlannerContextPtr &planner_context, std::set<std::string> &used_row_policies) {
  auto storage_id = storage->getStorageID();
  const auto &query_context = planner_context->getQueryContext();

  auto row_policy_filter =
      query_context->getRowPolicyFilter(storage_id.getDatabaseName(), storage_id.getTableName(), RowPolicyFilterType::SELECT_FILTER);
  if (!row_policy_filter || row_policy_filter->isAlwaysTrue()) return {};
...
std::optional<FilterDAGInfo> buildCustomKeyFilterIfNeeded(const StoragePtr &storage, SelectQueryInfo &table_expression_query_info,
                                                          PlannerContextPtr &planner_context) {
  const auto &query_context = planner_context->getQueryContext();
  const auto &settings = query_context->getSettingsRef();

  if (settings[Setting::parallel_replicas_count] <= 1 || settings[Setting::parallel_replicas_custom_key].value.empty()) return {};
  ...
  LOG_TRACE(getLogger("Planner"), "Processing query on a replica using custom_key '{}'",
            settings[Setting::parallel_replicas_custom_key].value);
  ...
  auto parallel_replicas_custom_filter_ast = getCustomKeyFilterForParallelReplica(
      ...
      storage->getInMemoryMetadataPtr()->columns, query_context);
...
std::optional<FilterDAGInfo> buildAdditionalFiltersIfNeeded(const StoragePtr &storage, const String &table_expression_alias,
                                                            SelectQueryInfo &table_expression_query_info,
                                                            const PrewhereInfoPtr &prewhere_info, PlannerContextPtr &planner_context) {
  const auto &query_context = planner_context->getQueryContext();
  const auto &settings = query_context->getSettingsRef();

  auto const &additional_filters = settings[Setting::additional_table_filters].value;
  if (additional_filters.empty()) return {};
  ...
  for (const auto &additional_filter : additional_filters) {
    ...
    if (table == table_expression_alias ||
      ...
      additional_filter_ast =
          parseQuery(parser, filter.data(), filter.data() + filter.size(), "additional filter", settings[Setting::max_query_size],
                     settings[Setting::max_parser_depth], settings[Setting::max_parser_backtracks]);
      break;
    }
  }

  if (!additional_filter_ast) return {};
  ...
  if (prewhere_info) {
    for (const auto *input : filter_info.actions.getInputs()) prewhere_info->prewhere_actions.tryRestoreColumn(input->result_name);
  }
  return filter_info;
}
...
UInt64 mainQueryNodeBlockSizeByLimit(const SelectQueryInfo &select_query_info) {
  ...
  if (main_query_node.hasLimit() && !main_query_node.isDistinct() && !main_query_node.isLimitWithTies() && !main_query_node.hasPrewhere() &&
      ...
      !main_query_node.hasHaving() && !main_query_node.hasOrderBy() && !main_query_node.hasLimitBy() && !select_query_info.need_aggregate &&
      !select_query_info.has_window && limit_length <= std::numeric_limits<UInt64>::max() - limit_offset)
    return limit_length + limit_offset;
  return 0;
}

std::unique_ptr<ExpressionStep> createComputeAliasColumnsStep(AliasColumnExpressions &alias_column_expressions,
  ...
  for (const auto *output_node : action_dag_outputs) merged_alias_columns_actions_dag.addOrReplaceInOutputs(*output_node);
...
bool extractRequiredNonTableColumnsFromStorage(const Names &columns_names, const StoragePtr &storage,
                                               const StorageSnapshotPtr &storage_snapshot, const QueryProcessingStage::Enum processed_stage,
                                               Names &extracted_column_names) {
  if (processed_stage != QueryProcessingStage::FetchColumns) return false;

  if (std::dynamic_pointer_cast<StorageMerge>(storage)) return false;

  if (std::dynamic_pointer_cast<StorageDistributed>(storage)) return false;
...
JoinTreeQueryPlan buildQueryPlanForTableExpression(QueryTreeNodePtr table_expression, const QueryTreeNodePtr &parent_join_tree,
                                                   const SelectQueryInfo &select_query_info, const SelectQueryOptions &select_query_options,
                                                   PlannerContextPtr &planner_context, bool is_single_table_expression,
  ...
  if (table_node || table_function_node) {
    ...
    bool is_trivial_count_applied =
        !select_query_options.only_analyze && !select_query_options.build_logical_plan && is_single_table_expression &&
        (table_node || table_function_node) && select_query_info.has_aggregates &&
        settings[Setting::additional_table_filters].value.empty() &&
        applyTrivialCountIfPossible(query_plan, table_expression_query_info, table_node, table_function_node, select_query_info.query_tree,
                                    planner_context->getMutableQueryContext(), table_expression_data.getColumnNames());
      ...
      if (!select_query_options.only_analyze) {
        ...
        if (prewhere_actions && select_query_options.build_logical_plan) {
          ...
          auto additional_filters_info_temp =
              buildAdditionalFiltersIfNeeded(storage, table_expression_alias, table_expression_query_info, prewhere_info, planner_context);
          ...
          where_filters.emplace_back(FilterDAGInfo{std::move(prewhere_actions_clone), prewhere_column_name, remove_prewhere_column},
        ...
        if (row_policy_filter_info) {
          ...
          /// TODO: Never put row-level security filter in WHERE clause for storages that do not support PREWHERE to avoid merging of
          /// filters.
        ...
        if (query_context->canUseParallelReplicasCustomKey()) {
          if (settings[Setting::parallel_replicas_count] > 1) {
            if (auto parallel_replicas_custom_key_filter_info =
              ...
              where_filters.emplace_back(std::move(*parallel_replicas_custom_key_filter_info),
        ...
        const auto &table_expression_alias = table_expression->getOriginalAlias();
        if (auto additional_filters_info = buildAdditionalFiltersIfNeeded(storage, table_expression_alias, table_expression_query_info,
                                                                          prewhere_info, planner_context)) {
        ...
        if (!select_query_options.build_logical_plan)
          till_stage =
              storage->getQueryProcessingStage(query_context, select_query_options.to_stage, storage_snapshot, table_expression_query_info);
        ...
        if (select_query_options.build_logical_plan) {
          ...
          if (table_node) {
            ...
            auto reading_from_table = std::make_unique<ReadFromTableStep>(
                sample_block, table_name, table_expression_query_info.table_expression_modifiers.value_or(TableExpressionModifiers{}));
            ...
            IAST::FormatSettings format_settings(
                /*one_line=*/true, IdentifierQuotingRule::WhenNecessary, IdentifierQuotingStyle::Backticks,
                /*show_secrets_=*/false);
            ...
            auto reading_from_table_function = std::make_unique<ReadFromTableFunctionStep>(
                sample_block, std::move(table_function_serialized_ast),
          ...
          if (no_tables_or_another_table_chosen_for_reading_with_parallel_replicas_mode) {
            ...
            storage->read(query_plan, storage_column_names, storage_snapshot, table_expression_query_info, std::move(mutable_context),
                          till_stage, max_block_size, max_streams);
          } else {
            storage->read(query_plan, storage_column_names, storage_snapshot, table_expression_query_info, query_context, till_stage,
        ...
        auto parallel_replicas_enabled_for_storage = [](const StoragePtr &current_storage, const Settings &query_settings) {
          ...
          if (mv) {
            if (!query_settings[Setting::parallel_replicas_allow_materialized_views]) return false;

            // address refreshable MVs separately, currently leads to logical error
            if (mv->isRefreshable()) return false;

            table_ptr = mv->getTargetTable().get();
          }

          if (!table_ptr->isMergeTree()) return false;

          if (!table_ptr->supportsReplication() && !query_settings[Setting::parallel_replicas_for_non_replicated_merge_tree]) return false;
        ...
        if (query_plan.isInitialized() && !select_query_options.build_logical_plan &&
          ...
          auto allow_parallel_replicas_for_join_tree = [&parallel_replicas_enabled_for_storage](const QueryTreeNodePtr &join_tree_node,
                                                                                                const Settings &query_settings) {
            if (join_tree_node->as<CrossJoinNode>()) return false;

            const JoinNode *join_node = join_tree_node->as<JoinNode>();
            if (!join_node) return true;
            ...
            if (left_table && left_table->getStorage()->isView()) return false;
            ...
            if ((join_kind == JoinKind::Inner && join_strictness == JoinStrictness::All) || join_kind == JoinKind::Left) {
              ...
              if (left_table) return parallel_replicas_enabled_for_storage(left_table->getStorage(), query_settings);

              const auto *left_table_function = left_table_expr->as<TableFunctionNode>();
              if (left_table_function) return parallel_replicas_enabled_for_storage(left_table_function->getStorage(), query_settings);
            ...
            if (join_kind == JoinKind::Right) {
              ...
              if (!right_table && !right_table_function) return false;
          ...
          if (query_context->canUseParallelReplicasCustomKey() && query_context->getClientInfo().distributed_depth == 0) {
            if (auto cluster = query_context->getClusterForParallelReplicas();
              ...
              ClusterProxy::executeQueryWithParallelReplicasCustomKey(query_plan_parallel_replicas, storage->getStorageID(),
                                                                      modified_query_info, storage->getInMemoryMetadataPtr()->getColumns(),
                                                                      storage_snapshot, till_stage, table_expression_query_info.query_tree,
                                                                      query_context);
            ...
            while (node) {
              ...
              if (!node->children.empty()) {
                ...
                throw Exception(ErrorCodes::LOGICAL_ERROR, "Step is expected to be ReadFromMergeTree but it's {}",
            ...
            if (settings[Setting::parallel_replicas_min_number_of_rows_per_replica] > 0) {
              ...
              if (max_block_size_limited && (max_block_size_limited < rows_to_read)) rows_to_read = max_block_size_limited;

              const size_t number_of_replicas_to_use = rows_to_read / settings[Setting::parallel_replicas_min_number_of_rows_per_replica];
              LOG_TRACE(getLogger("Planner"), "Estimated {} rows to read. It is enough work for {} parallel replicas", rows_to_read,
            ...
            if (planner_context->getQueryContext()->canUseParallelReplicasOnInitiator()) {
              ...
              ClusterProxy::executeQueryWithParallelReplicas(query_plan_parallel_replicas, storage->getStorageID(), till_stage,
                                                             table_expression_query_info.query_tree,
                                                             table_expression_query_info.planner_context, query_context,
                                                             table_expression_query_info.storage_limits, std::move(reading_step));
              query_plan = std::move(query_plan_parallel_replicas);
            } else {
              QueryPlan query_plan_no_parallel_replicas;
              storage->read(query_plan_no_parallel_replicas, storage_column_names, storage_snapshot, table_expression_query_info,
                            query_context, till_stage, max_block_size, max_streams);
        ...
        for (auto &&[filter_info, description] : where_filters) {
          if (query_plan.isInitialized() && till_stage == QueryProcessingStage::FetchColumns) {
            auto filter_step = std::make_unique<FilterStep>(query_plan.getCurrentHeader(), std::move(filter_info.actions),
        ...
        if (query_context->hasQueryContext() && !select_query_options.is_internal) {
          ...
          query_context->getQueryContext()->addQueryAccessInfo(backQuoteIfNeed(local_storage_id.getDatabaseName()),
                                                               local_storage_id.getFullTableName(), columns_names);
        }
      }
  ...
  if (till_stage == QueryProcessingStage::FetchColumns) {
    ...
    rename_step->setStepDescription(select_query_options.ignore_rename_columns ? "Change column identifiers to column names"
    ...
    if (!blocksHaveEqualStructure(*query_plan.getCurrentHeader(), *expected_header)) {
      ...
      auto rename_actions_dag = ActionsDAG::makeConvertingActions(
          query_plan.getCurrentHeader()->getColumnsWithTypeAndName(), expected_block.getColumnsWithTypeAndName(),
          ActionsDAG::MatchColumnsMode::Position, planner_context->getQueryContext(), true /*ignore_constant_values*/,
          false /*add_cast_columns*/, nullptr /*new_names*/);
      auto rename_step = std::make_unique<ExpressionStep>(query_plan.getCurrentHeader(), std::move(rename_actions_dag));
...
void joinCastPlanColumnsToNullable(QueryPlan &plan_to_add_cast, PlannerContextPtr &planner_context,
                                   const FunctionOverloadResolverPtr &to_nullable_function) {
...
std::optional<ActionsDAG> createStepToDropColumns(const Block &header, const ColumnIdentifierSet &outer_scope_columns,
  ...
  for (size_t i = 0; i < drop_unused_columns_after_join_actions_dag_outputs_size; ++i) {
    ...
    if (!outer_scope_columns.contains(output->result_name)) {
      if (!first_skipped_column_node_index) first_skipped_column_node_index = i;
      continue;
    }
  ...
  if (!first_skipped_column_node_index) return {};
  ...
  if (drop_unused_columns_after_join_actions_dag_updated_outputs.empty() && first_skipped_column_node_index)
    drop_unused_columns_after_join_actions_dag_updated_outputs.push_back(
        drop_unused_columns_after_join_actions_dag_outputs[*first_skipped_column_node_index]);
...
std::tuple<QueryPlan, JoinPtr> buildJoinQueryPlan(QueryPlan left_plan, QueryPlan right_plan, std::shared_ptr<TableJoin> &table_join,
                                                  JoinClausesAndActions &join_clauses_and_actions, UsefulSets &left_useful_sets,
                                                  const QueryTreeNodePtr &right_table_expression,
                                                  const ColumnIdentifierSet &outer_scope_columns, PlannerContextPtr &planner_context,
                                                  const SelectQueryInfo &select_query_info, size_t max_step_description_length) {
  ...
  /// but the assertion expects the arithmetic sum.
  {
    ...
    for (const auto &col : columns_from_left_table) left_column_names.insert(col.name);

    NameSet right_key_names;
    for (const auto &clause : table_join->getClauses())
      for (const auto &key_name : clause.key_names_right) right_key_names.insert(key_name);
    ...
    if (has_overlapping) {
      ...
      for (const auto *output : dag.getOutputs()) {
        if (!left_column_names.contains(output->result_name) || right_key_names.contains(output->result_name))
          updated_outputs.push_back(output);
      }
      dag.getOutputs() = std::move(updated_outputs);

      auto step = std::make_unique<ExpressionStep>(right_plan.getCurrentHeader(), std::move(dag));
  ...
  if (table_join->getOutputColumns(JoinTableSide::Left).empty() && table_join->getOutputColumns(JoinTableSide::Right).empty()) {
    ...
    auto set_used_column_with_duplicates = [&](const NamesAndTypesList &columns, JoinTableSide join_table_side) {
      ...
      for (const auto &column : columns)
        if (column.name == column_name) table_join->setUsedColumn(column, join_table_side);
    };
  ...
  auto join_algorithm = chooseJoinAlgorithm(table_join, prepared_join_storage, left_header, right_header, params);
  ...
  if (is_filled_join) {
    ...
    auto add_sorting = [&](QueryPlan &plan, const Names &key_names, JoinTableSide join_table_side) {
      ...
      for (const auto &key_name : key_names) sort_description.emplace_back(key_name);

      SortingStep::Settings sort_settings(query_context->getSettingsRef());

      auto sorting_step = std::make_unique<SortingStep>(plan.getCurrentHeader(), std::move(sort_description), 0 /*limit*/, sort_settings,
                                                        true /*is_sorting_for_merge_join*/);
    ...
    auto add_create_set = [&settings, crosswise_connection, max_step_description_length](QueryPlan &plan, const Names &key_names,
                                                                                         JoinTableSide join_table_side) {
    ...
    std::erase_if(required_columns_after_join, [&](const String &name) { return !left_header->has(name) && !right_header->has(name); });
    ...
    auto join_step = std::make_unique<JoinStep>(left_plan.getCurrentHeader(), right_plan.getCurrentHeader(), join_algorithm,
                                                settings[Setting::max_block_size], settings[Setting::min_joined_block_size_rows],
                                                settings[Setting::min_joined_block_size_bytes], settings[Setting::max_threads],
                                                required_columns_after_join, false /*optimize_read_in_order*/,
                                                true /*optimize_skip_unused_shards*/, settings[Setting::use_join_disjunctions_push_down]);
  ...
  if (join_clauses_and_actions.residual_join_expressions_actions) {
    ...
    auto filter_step = std::make_unique<FilterStep>(
        result_plan.getCurrentHeader(), std::move(*join_clauses_and_actions.residual_join_expressions_actions), outputs[0]->result_name,
  ...
  if (header_after_join->columns() > outer_scope_columns.size()) {
    ...
    if (drop_unused_columns_after_join_actions_dag) {
      auto drop_unused_columns_after_join_transform_step =
          std::make_unique<ExpressionStep>(result_plan.getCurrentHeader(), std::move(*drop_unused_columns_after_join_actions_dag));
...
JoinTreeQueryPlan joinPlansWithStep(QueryPlanStepPtr join_step, JoinTreeQueryPlan left_join_tree_query_plan,
  ...
  for (const auto &useful_set : right_join_tree_query_plan.useful_sets) result_useful_sets.insert(useful_set);
...
JoinTreeQueryPlan buildQueryPlanForCrossJoinNode(const QueryTreeNodePtr &join_table_expression, std::vector<JoinTreeQueryPlan> plans,
                                                 const ColumnIdentifierSet &outer_scope_columns, PlannerContextPtr &planner_context,
                                                 const SelectQueryInfo &select_query_info, size_t max_step_description_length) {
  ...
  for (const auto &plan : plans) {
    if (plan.stage != QueryProcessingStage::FetchColumns)
      throw Exception(ErrorCodes::UNSUPPORTED_METHOD,
                      ...
                      cross_join_node.formatASTForErrorMessage(), QueryProcessingStage::toString(plan.stage));
  }
  ...
  for (size_t i = 1; i < plans.size(); ++i) {
    ...
    if (settings[Setting::query_plan_use_new_logical_join_step]) {
      ...
      auto join_step_logical =
          std::make_unique<JoinStepLogical>(left_header, right_header, JoinOperator{JoinKind::Cross}, std::move(join_expression_actions),
                                            outer_scope_columns, std::unordered_map<String, const ActionsDAG::Node *>{},
                                            settings[Setting::join_use_nulls], JoinSettings(settings), SortingStep::Settings(settings));
    ...
    auto [result_plan, join_algorithm] = buildJoinQueryPlan(
        std::move(left_plan), std::move(right_plan), table_join, join_clauses_and_actions, left_join_tree_query_plan.useful_sets,
        cross_join_node.getTableExpressions()[i], outer_scope_columns, planner_context, select_query_info, max_step_description_length);
    ...
    if (!join_algorithm->isFilled())
      for (const auto &useful_set : right_join_tree_query_plan.useful_sets) left_join_tree_query_plan.useful_sets.insert(useful_set);
...
JoinTreeQueryPlan buildQueryPlanForJoinNodeLegacy(const QueryTreeNodePtr &join_table_expression,
                                                  JoinTreeQueryPlan left_join_tree_query_plan, JoinTreeQueryPlan right_join_tree_query_plan,
                                                  const ColumnIdentifierSet &outer_scope_columns, PlannerContextPtr &planner_context,
                                                  const SelectQueryInfo &select_query_info, size_t max_step_description_length) {
  ...
  if (right_join_tree_query_plan.stage != QueryProcessingStage::FetchColumns)
    throw Exception(ErrorCodes::UNSUPPORTED_METHOD,
                    ...
                    join_node.formatASTForErrorMessage(), QueryProcessingStage::toString(right_join_tree_query_plan.stage));
  ...
  if (!join_constant && join_node.isOnJoinExpression()) {
    join_clauses_and_actions =
        buildJoinClausesAndActions(left_plan_output_columns, right_plan_output_columns, join_table_expression, planner_context);
    ...
    auto check_pre_filter = [](JoinTableSide side, const auto &pre_filters) {
      if (!pre_filters.empty() && pre_filters.size() != 1)
        throw Exception(ErrorCodes::LOGICAL_ERROR, "Expected only one {} pre-filter condition node. Actual [{}]", side,
                        fmt::join(pre_filters | std::views::transform([](const auto &node) { return node->result_name; }), ", "));
    };
    check_pre_filter(JoinTableSide::Left, left_pre_filters);
    check_pre_filter(JoinTableSide::Right, right_pre_filters);

    can_move_out_residuals = join_clauses_and_actions.join_clauses.size() == 1 && join_strictness == JoinStrictness::All &&
    ...
    auto add_pre_filter = [can_move_out_residuals](ActionsDAG &join_expressions_actions, QueryPlan &plan, UsefulSets &useful_sets,
                                                   const auto &pre_filters) {
      ...
      if (can_move_out_residuals && !pre_filters.empty())
        join_expressions_actions_step =
            std::make_unique<FilterStep>(plan.getCurrentHeader(), std::move(join_expressions_actions), pre_filters[0]->result_name, false);
      else
    ...
    add_pre_filter(join_clauses_and_actions.left_join_expressions_actions, left_plan, left_join_tree_query_plan.useful_sets,
                   left_pre_filters);
    add_pre_filter(join_clauses_and_actions.right_join_expressions_actions, right_plan, right_join_tree_query_plan.useful_sets,
                   right_pre_filters);
  }
  ...
  if (join_node.isUsingJoinExpression()) {
    ...
    for (auto &join_node_using_node : join_node_using_columns_list.getNodes()) {
      ...
      if (inner_columns_list.getNodes().size() < 2)
        throw Exception(ErrorCodes::LOGICAL_ERROR, "JOIN USING clause expected at least two column identifiers, got: {}",
      ...
      if (!left_inner_column)
        throw Exception(ErrorCodes::BAD_ARGUMENTS, "JOIN USING clause expected column identifier. Actual {}: {}",
                        left_inner_column_node->getNodeTypeName(), left_inner_column_node->formatASTForErrorMessage());
      ...
      if (!right_inner_column)
        throw Exception(ErrorCodes::BAD_ARGUMENTS, "JOIN USING clause expected column identifier. Actual {}: {}",
                        right_inner_column_node->getNodeTypeName(), right_inner_column_node->formatASTForErrorMessage());
  ...
  auto join_cast_plan_output_nodes = [&](QueryPlan &plan_to_add_cast,
                                         std::unordered_map<std::string, DataTypePtr> &plan_column_name_to_cast_type) {
    ActionsDAG cast_actions_dag(plan_to_add_cast.getCurrentHeader()->getColumnsWithTypeAndName());

    for (auto &output_node : cast_actions_dag.getOutputs()) {
      auto it = plan_column_name_to_cast_type.find(output_node->result_name);
      if (it == plan_column_name_to_cast_type.end()) continue;
  ...
  if (!left_plan_column_name_to_cast_type.empty()) join_cast_plan_output_nodes(left_plan, left_plan_column_name_to_cast_type);

  if (!right_plan_column_name_to_cast_type.empty()) join_cast_plan_output_nodes(right_plan, right_plan_column_name_to_cast_type);

  const auto &query_context = planner_context->getQueryContext();
  const auto &settings = query_context->getSettingsRef();

  if (settings[Setting::join_use_nulls]) joinCastPlanColumnsToNullable(left_plan, right_plan, planner_context, join_kind);
  ...
  if (join_constant) {
     ...
     */
    if (*join_constant) join_kind = JoinKind::Cross;
  }

  if (join_kind == JoinKind::Comma) join_kind = JoinKind::Cross;
  ...
  if (join_node.isOnJoinExpression()) {
    ...
    if (join_clauses.size() != 1 && is_asof) {
      throw Exception(ErrorCodes::NOT_IMPLEMENTED, "ASOF join doesn't support JOIN ON expression {}", join_node.formatASTForErrorMessage());
    }
    ...
    for (const auto &join_clause : join_clauses) {
      ...
      for (size_t i = 0; i < join_clause_key_nodes_size; ++i) {
        table_join_clause.addKey(join_clause_left_key_nodes[i]->result_name, join_clause_right_key_nodes[i]->result_name,
      ...
      if (!join_clause_get_left_filter_condition_nodes.empty()) {
        if (join_clause_get_left_filter_condition_nodes.size() != 1)
          throw Exception(ErrorCodes::LOGICAL_ERROR, "JOIN {} left filter conditions size must be 1. Actual {}",
                          join_node.formatASTForErrorMessage(), join_clause_get_left_filter_condition_nodes.size());
      ...
      if (!join_clause_get_right_filter_condition_nodes.empty()) {
        if (join_clause_get_right_filter_condition_nodes.size() != 1)
          throw Exception(ErrorCodes::LOGICAL_ERROR, "JOIN {} right filter conditions size must be 1. Actual {}",
                          join_node.formatASTForErrorMessage(), join_clause_get_right_filter_condition_nodes.size());
      ...
      if (is_asof) {
        if (!join_clause.hasASOF())
          throw Exception(ErrorCodes::INVALID_JOIN_ON_EXPRESSION, "JOIN {} no inequality in ASOF JOIN ON section",
      ...
      if (join_clause.hasASOF()) {
        ...
        if (asof_conditions.size() > 1) {
          throw Exception(ErrorCodes::INVALID_JOIN_ON_EXPRESSION, "JOIN {} ASOF JOIN expects exactly one inequality in ON section",
                          join_node.formatASTForErrorMessage());
        }
    ...
    if (!can_move_out_residuals && join_clauses_and_actions.residual_join_expressions_actions) {
      ...
      mixed_join_expression = std::make_shared<ExpressionActions>(std::move(*join_clauses_and_actions.residual_join_expressions_actions),
  ...
  auto [result_plan, join_algorithm] = buildJoinQueryPlan(
      std::move(left_plan), std::move(right_plan), table_join, join_clauses_and_actions, left_join_tree_query_plan.useful_sets,
      join_node.getRightTableExpression(), outer_scope_columns, planner_context, select_query_info, max_step_description_length);
...
void tryMakeDirectJoinWithMergeTree(const JoinOperator &join_operator, QueryPlan &right_query_plan, PreparedJoinStorage &prepared_join,
                                    PlannerContextPtr &planner_context) {
  const auto &query_context = planner_context->getQueryContext();
  const auto &settings = query_context->getSettingsRef();
  ...
  if (settings[Setting::join_algorithm].value != std::vector{JoinAlgorithm::DIRECT}) return;
  ...
  if (!allow_strictness || !isInnerOrLeft(join_operator.kind)) return;
  if (!join_operator.residual_filter.empty() || join_operator.expression.size() != 1) return;
  auto [predicate_type, lhs, rhs] = join_operator.expression[0].asBinaryPredicate();
  if (predicate_type != JoinConditionOperator::Equals) return;
  ...
  if (!root_node || !root_node->step) return;
  const auto *expr_step = root_node->step.get();
  if (!typeid_cast<const ExpressionStep *>(expr_step) && !typeid_cast<const FilterStep *>(expr_step)) return;
  if (root_node->children.size() != 1 || !root_node->children.front()) return;
  ...
  for (const auto &column_name : lookup_plan.getCurrentHeader()->getNames()) prepared_join.column_mapping[column_name] = column_name;
...
JoinTreeQueryPlan buildQueryPlanForJoinNode(const QueryTreeNodePtr &join_table_expression, JoinTreeQueryPlan left_join_tree_query_plan,
                                            JoinTreeQueryPlan right_join_tree_query_plan, const ColumnIdentifierSet &outer_scope_columns,
                                            PlannerContextPtr &planner_context, const SelectQueryInfo &select_query_info,
  ...
  if (left_join_tree_query_plan.stage != QueryProcessingStage::FetchColumns)
    throw Exception(ErrorCodes::UNSUPPORTED_METHOD,
                    ...
                    join_node.formatASTForErrorMessage(), QueryProcessingStage::toString(left_join_tree_query_plan.stage));

  const auto &query_context = planner_context->getQueryContext();
  const auto &settings = query_context->getSettingsRef();
  if (!settings[Setting::query_plan_use_new_logical_join_step])
    return buildQueryPlanForJoinNodeLegacy(join_table_expression, std::move(left_join_tree_query_plan),
                                           std::move(right_join_tree_query_plan), outer_scope_columns, planner_context, select_query_info,
                                           max_step_description_length);

  auto join_step_logical =
      buildJoinStepLogical(left_join_tree_query_plan.query_plan.getCurrentHeader(),
                           right_join_tree_query_plan.query_plan.getCurrentHeader(), outer_scope_columns, join_node, planner_context);
  ...
  if (allow_storage_join) prepared_join = tryGetStorageInTableJoin(join_node.getRightTableExpression(), planner_context);
  if (prepared_join) {
    bool use_nulls = settings[Setting::join_use_nulls] && isLeftOrFull(join_node.getKind());
    auto join_lookup_step =
        std::make_unique<JoinStepLogicalLookup>(std::move(right_join_tree_query_plan.query_plan), std::move(prepared_join), use_nulls);
    right_join_tree_query_plan.query_plan = {};
    right_join_tree_query_plan.query_plan.addStep(std::move(join_lookup_step));
  } else {
    tryMakeDirectJoinWithMergeTree(join_step_logical->getJoinOperator(), right_join_tree_query_plan.query_plan, prepared_join,
                                   planner_context);
  }
...
JoinTreeQueryPlan buildQueryPlanForArrayJoinNode(const QueryTreeNodePtr &array_join_table_expression,
                                                 JoinTreeQueryPlan join_tree_query_plan, const ColumnIdentifierSet &outer_scope_columns,
                                                 PlannerContextPtr &planner_context) {
  ...
  if (join_tree_query_plan.stage != QueryProcessingStage::FetchColumns)
    throw Exception(ErrorCodes::UNSUPPORTED_METHOD,
                    ...
                    array_join_node.formatASTForErrorMessage(), QueryProcessingStage::toString(join_tree_query_plan.stage));
  ...
  for (size_t i = 0; i < drop_unused_columns_before_array_join_actions_dag_outputs_size; ++i) {
    ...
    if (drop_unused_columns_before_array_join_actions_dag_updated_outputs_names.contains(output->result_name)) continue;

    if (!array_join_expressions_output_nodes.contains(output->result_name) && !outer_scope_columns.contains(output->result_name)) continue;
  ...
  auto array_join_step =
      std::make_unique<ArrayJoinStep>(plan.getCurrentHeader(), ArrayJoin{std::move(array_join_column_names), array_join_node.isLeft()},
                                      settings[Setting::enable_unaligned_array_join], settings[Setting::max_block_size],
...
JoinTreeQueryPlan buildJoinTreeQueryPlan(const QueryTreeNodePtr &query_node, const SelectQueryInfo &select_query_info,
                                         SelectQueryOptions &select_query_options, const ColumnIdentifierSet &outer_scope_columns,
                                         PlannerContextPtr &planner_context) {
  ...
  for (size_t i = 0; i < table_expressions_stack_size; ++i) {
    ...
    if (table_expression_type == QueryTreeNodeType::ARRAY_JOIN) continue;
    ...
    if (table_expression_type == QueryTreeNodeType::JOIN) {
      ...
      if (join_kind == JoinKind::Full) is_full_join = true;

      if (join_node.getLocality() == JoinLocality::Global) is_global_join = true;
      ...
      if (join_kind == JoinKind::Right) last_right_join_pos = static_cast<int>(i);
  ...
  auto should_disable_parallel_replicas = [&]() -> bool {
    ...
    if (first_join_pos >= 0 && last_right_join_pos >= 0 && first_join_pos < last_right_join_pos) return true;

    /// for n-way join with FULL JOIN or GLOBAL JOINS or CROSS JOIN
    if (joins_count > 1 && (is_full_join || is_global_join || is_cross_join)) return true;

    /// For RIGHT JOIN with distributed table on the right side
    if (is_right_join_with_remote_table) return true;

    return false;
  };

  if (should_disable_parallel_replicas()) planner_context->getMutableQueryContext()->setSetting("enable_parallel_replicas", Field{0});
  ...
  auto left_table_expression_query_plan =
      buildQueryPlanForTableExpression(left_table_expression, parent_join_tree, select_query_info, select_query_options, planner_context,
                                       is_single_table_expression, should_wrap_left_table /*wrap_read_columns_in_subquery*/);
  if (left_table_expression_query_plan.stage != QueryProcessingStage::FetchColumns) return left_table_expression_query_plan;
  ...
  for (size_t i = 0; i < table_expressions_stack_size; ++i) {
    ...
    if (table_expression_node_type == QueryTreeNodeType::ARRAY_JOIN) {
      if (query_plans_stack.empty())
        throw Exception(ErrorCodes::LOGICAL_ERROR, "Expected at least 1 query plan on stack before ARRAY JOIN processing. Actual {}",
                        query_plans_stack.size());

      auto query_plan = std::move(query_plans_stack.back());
      query_plans_stack.back() = buildQueryPlanForArrayJoinNode(table_expression, std::move(query_plan),
      ...
      if (query_plans_stack.size() < 2)
        throw Exception(ErrorCodes::LOGICAL_ERROR, "Expected at least 2 query plans on stack before JOIN processing. Actual {}",
                        query_plans_stack.size());
      ...
      query_plans_stack.push_back(buildQueryPlanForJoinNode(table_expression, std::move(left_query_plan), std::move(right_query_plan),
                                                            table_expressions_outer_scope_columns[i], planner_context, select_query_info,
      ...
      if (query_plans_stack.size() < num_tables)
        throw Exception(ErrorCodes::LOGICAL_ERROR, "Expected at least {} query plans on stack before CROSS JOIN processing. Actual {}",
                        num_tables, query_plans_stack.size());
      ...
      query_plans_stack.push_back(buildQueryPlanForCrossJoinNode(table_expression, std::move(plans),
                                                                 table_expressions_outer_scope_columns[i], planner_context,
                                                                 select_query_info, select_query_options.max_step_description_length));
    } else {
      ...
      query_plans_stack.push_back(buildQueryPlanForTableExpression(table_expression, parent_join_tree, select_query_info,
                                                                   select_query_options, planner_context, is_single_table_expression,
