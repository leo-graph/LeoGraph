...
const ActionsDAG::Node *appendExpression(ActionsDAG &dag, const QueryTreeNodePtr &expression, const PlannerContextPtr &planner_context,
  ...
  if (join_expression_dag_node_raw_pointers.size() != 1)
    throw Exception(ErrorCodes::LOGICAL_ERROR, "Expression {} expected be a single node, got {}", expression->formatASTForErrorMessage(),
                    dag.dumpDAG());
  ...
  if (input_count != dag.getInputs().size()) {
    ...
    throw Exception(ErrorCodes::NOT_FOUND_COLUMN_IN_BLOCK, "Columns [{}] are not found in blocks [{}], [{}]",
...
struct JoinOperatorBuildContext {
  explicit JoinOperatorBuildContext(const JoinNode &join_node_, SharedHeader left_header_, SharedHeader right_header_,
  ...
  JoinSource getExpressionSource(const QueryTreeNodePtr &node) {
    ...
    if (res.empty()) return JoinSource::None;
    if (res.size() == 1) {
      if (*res.begin() == JoinTableSide::Left) return JoinSource::Left;
...
std::unordered_map<String, const ActionsDAG::Node *> buildJoinUsingCondition(const QueryTreeNodePtr &node,
                                                                             JoinOperatorBuildContext &builder_context) {
  ...
  if (num_nodes == 0) throw Exception(ErrorCodes::INVALID_JOIN_ON_EXPRESSION, "JOIN USING expression expected at least one column");
  ...
  for (size_t i = 0; i < num_nodes; ++i) {
    ...
    if (inner_columns.size() < 2)
      throw Exception(ErrorCodes::LOGICAL_ERROR, "Using column {} expected to have at least 2 inner columns, in query tree {}",
    ...
    for (const auto &inner_column : inner_columns) {
      ...
      if (arg.fromNone()) arg.setSourceRelations(BitSet().set(0));
    }
...
void buildJoinCondition(const QueryTreeNodePtr &node, JoinOperatorBuildContext &builder_context, JoinCondition &join_condition) {
  ...
  if (function_name == "and") {
    for (const auto &child : function_node->getArguments()) buildJoinCondition(child, builder_context, join_condition);
    return;
  }
...
void buildDisjunctiveJoinConditions(const QueryTreeNodePtr &node, JoinOperatorBuildContext &builder_context,
                                    std::vector<JoinCondition> &join_conditions) {
  auto *function_node = node->as<FunctionNode>();
  if (!function_node)
    throw Exception(ErrorCodes::INVALID_JOIN_ON_EXPRESSION, "JOIN {} join expression expected function", node->formatASTForErrorMessage());

  const auto &function_name = function_node->getFunctionName();

  if (function_name == "or") {
    for (const auto &child : function_node->getArguments()) buildDisjunctiveJoinConditions(child, builder_context, join_conditions);
    return;
  }
...
static bool hasEquiConditions(const JoinCondition &condition) {
  for (const auto &conjunct : condition.conjuncts) {
    ...
    if (function_name == NameEquals::name || function_name == FunctionIsNotDistinctFrom::name) return true;
  }
  return false;
}
...
String getQueryDisplayLabel(const QueryTreeNodePtr &node, bool display_internal_aliases) {
  ...
  if (display_internal_aliases && !internal_alias.empty()) return internal_alias;

  const auto &printable_alias = node->getOriginalAlias();

  if (!printable_alias.empty() && printable_alias != internal_alias) return printable_alias;
...
std::unique_ptr<JoinStepLogical> buildJoinStepLogical(SharedHeader left_header, SharedHeader right_header,
                                                      const NameSet &outer_scope_columns, const JoinNode &join_node,
                                                      const PlannerContextPtr &planner_context) {
  ...
  auto join_step = std::make_unique<JoinStepLogical>(
      left_header, right_header, std::move(build_context.join_operator), std::move(build_context.expression_actions), outer_scope_columns,
      changed_types, settings[Setting::join_use_nulls], JoinSettings(settings), SortingStep::Settings(settings));
...

  {
    ...
    if (auto it = query_params.find("_internal_join_table_stat_hints"); it != query_params.end()) join_step->setDummyStats(it->second);
  }

  if (shouldForbidReordering(build_context)) join_step->setOptimized();
...
PreparedJoinStorage tryGetStorageInTableJoin(const QueryTreeNodePtr &table_expression, const PlannerContextPtr &planner_context) {
  ...
  if (result.storage_join) return result;
  ...
  if (result.storage_key_value) return result;
