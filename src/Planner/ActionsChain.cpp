...
ActionsChainStep::ActionsChainStep(ActionsAndProjectInputsFlagPtr actions_, bool use_actions_nodes_as_output_columns_,
...
void ActionsChainStep::finalizeInputAndOutputColumns(const NameSet& child_input_columns) {
  ...
  for (auto& output_node : actions->dag.getOutputs()) output_nodes_names.insert(output_node->result_name);

  for (const auto& node : actions->dag.getNodes()) {
    auto it = child_input_columns_copy.find(node.result_name);
    if (it == child_input_columns_copy.end()) continue;

    child_input_columns_copy.erase(it);
    child_required_output_columns_names.insert(node.result_name);

    if (output_nodes_names.contains(node.result_name)) continue;
...
void ActionsChainStep::initialize() {
  ...
  if (use_actions_nodes_as_output_columns) {
    ...
    for (const auto& node : actions->dag.getNodes()) {
      if (available_output_columns_names.contains(node.result_name)) continue;
