...
void CorrelatedSubtrees::assertEmpty(std::string_view reason) const {
  if (notEmpty()) throw Exception(ErrorCodes::NOT_IMPLEMENTED, "Correlated subqueries {} are not supported", reason);
}
...
CorrelatedPlanStepMap buildCorrelatedPlanStepMap(QueryPlan &correlated_query_plan) {
  ...
  while (!nodes_to_process.empty()) {
    ...
    if (nodes_to_process[current_index].processed_children) {
      ...
      for (auto *child : current->children) value |= result[child];

      nodes_to_process.pop_back();
    } else {
      for (auto *child : nodes_to_process[current_index].node->children) nodes_to_process.push_back({.node = child});
...
struct EquivalenceClasses {
  ...
  std::shared_ptr<const std::list<String>> getClass(const String &name) const {
    ...
    if (it == member_to_class.end()) return {};
    return it->second;
  }
 ...
 private:
  void mergeFromTo(std::shared_ptr<std::list<String>> class_from, std::shared_ptr<std::list<String>> class_to) {
    ...
    for (const auto &member_from : *class_from) member_to_class[member_from] = class_to;
...
QueryPlan decorrelateQueryPlan(DecorrelationContext &context, QueryPlan::Node *node) {
  if (!context.correlated_plan_steps[node]) {
    ...
    if (settings[Setting::correlated_subqueries_substitute_equivalent_expressions]) {
      ...
      for (const auto *output : outputs) decorrelated_nodes_names[output->result_name] = output;
      ...
      if (context.correlated_subquery.correlated_column_identifiers.size() == expression_renamings.size()) {
        for (const auto &[from, to] : expression_renamings) outputs.push_back(&dag.addAlias(*from, to));
    ...
    rhs_plan.addStep(std::make_unique<CommonSubplanReferenceStep>(buffer_header, context.query_plan.getRootNode(),
    ...
    if (default_join_kind == DecorrelationJoinKind::LEFT) std::swap(lhs_plan, rhs_plan);

    auto lhs_plan_header = lhs_plan.getCurrentHeader();
    auto rhs_plan_header = rhs_plan.getCurrentHeader();

    JoinExpressionActions join_expression_actions(lhs_plan_header->getColumnsWithTypeAndName(),
    ...
    auto decorrelated_join = std::make_unique<JoinStepLogical>(
        ...
        /*right_header_=*/rhs_plan_header, JoinOperator(JoinKind::Cross), std::move(join_expression_actions), output_columns,
        std::unordered_map<String, const ActionsDAG::Node *>{}, settings[Setting::join_use_nulls], JoinSettings(settings),
  ...
  if (auto *expression_step = typeid_cast<ExpressionStep *>(node->step.get())) {
    ...
    for (const auto &column : input_header->getColumnsWithTypeAndName()) expression_step->getExpression().tryRestoreColumn(column.name);
  ...
  if (auto *filter_step = typeid_cast<FilterStep *>(node->step.get())) {
    ...
    for (const auto *conjunct : conjuncts_list) {
      ...
      if (is_equality) {
        ...
        if (arguments.size() != 2)
          throw Exception(ErrorCodes::LOGICAL_ERROR, "Correlated subquery equality predicate must have exactly two arguments, but has {}",
                          arguments.size());

        if (!arguments[0]->result_type->equals(*arguments[1]->result_type)) continue;
    ...
    for (const auto &column : input_header->getColumnsWithTypeAndName()) filter_step->getExpression().tryRestoreColumn(column.name);
  ...
  if (auto *union_step = typeid_cast<UnionStep *>(node->step.get())) {
    ...
    /// )
    const auto &settings = context.planner_context->getQueryContext()->getSettingsRef();
    auto process_isolated_subplan = [](DecorrelationContext &current_context, QueryPlan::Node *subplan_root) -> QueryPlan {
    ...
    Block union_common_header = buildCommonHeaderForUnion(query_plans_headers, SelectUnionMode::UNION_ALL,
  ...
  if (auto *aggeregating_step = typeid_cast<AggregatingStep *>(node->step.get())) {
    ...
    auto result_step = std::make_unique<AggregatingStep>(
        std::move(input_header), std::move(new_aggregator_params), aggeregating_step->getGroupingSetsParamsList(),
        aggeregating_step->getFinal(), aggeregating_step->getMaxBlockSize(), aggeregating_step->getMaxBlockSizeForAggregationInOrder(),
        aggeregating_step->getMergeThreads(), aggeregating_step->getTemporaryDataMergeThreads(),
        false /*storage_has_evenly_distributed_read_*/, aggeregating_step->isGroupByUseNulls(),
        SortDescription{} /*sort_description_for_merging_*/, SortDescription{} /*group_by_sort_description_*/,
        aggeregating_step->shouldProduceResultsInBucketOrder(), aggeregating_step->usingMemoryBoundMerging(),
  ...
  throw Exception(ErrorCodes::NOT_IMPLEMENTED, "Cannot decorrelate query, because '{}' step is not supported", node->step->getName());
}

void buildRenamingForScalarSubquery(QueryPlan &query_plan, const CorrelatedSubquery &correlated_subquery) {
  ...
  for (const auto &column_name : correlated_subquery.correlated_column_identifiers) {
    new_outputs.push_back(
        &dag.addAlias(dag.findInOutputs(column_name), fmt::format("{}.{}", correlated_subquery.action_node_name, column_name)));
  }
...
void buildExistsResultExpression(QueryPlan &query_plan, const CorrelatedSubquery &correlated_subquery,
                                 bool project_only_correlated_columns) {
  ActionsDAG dag(query_plan.getCurrentHeader()->getNamesAndTypesList());
  auto result_type = std::make_shared<DataTypeUInt8>();
  auto column = result_type->createColumnConst(1, 1);
  const auto *exists_result =
      &dag.materializeNode(dag.addColumn(ColumnWithTypeAndName(column, result_type, correlated_subquery.action_node_name)));
...
QueryPlan buildLogicalJoin(const PlannerContextPtr &planner_context, QueryPlan input_stream_plan, QueryPlan decorrelated_plan,
                           const CorrelatedSubquery &correlated_subquery, bool referenced_input_subplan) {
  ...
  JoinExpressionActions join_expression_actions(lhs_plan_header->getColumnsWithTypeAndName(), rhs_plan_header->getColumnsWithTypeAndName());
  ...
  auto join_kind_to_use =
      settings[Setting::correlated_subqueries_default_join_kind] == DecorrelationJoinKind::RIGHT ? JoinKind::Right : JoinKind::Left;

  /// Add ANY OUTER JOIN
  auto result_join = std::make_unique<JoinStepLogical>(
      lhs_plan_header, rhs_plan_header,
      JoinOperator(join_kind_to_use, JoinStrictness::Any, JoinLocality::Unspecified, std::move(predicates)),
      std::move(join_expression_actions), output_columns, std::unordered_map<String, const ActionsDAG::Node *>{},
      /*join_use_nulls=*/false, JoinSettings(settings), SortingStep::Settings(settings));
...
Planner buildPlannerForCorrelatedSubquery(const PlannerContextPtr &planner_context, const CorrelatedSubquery &correlated_subquery,
  ...
  Planner subquery_planner(correlated_subquery.query_tree, subquery_options, std::move(global_planner_context));
  subquery_planner.buildQueryPlanIfNeeded();

  return subquery_planner;
}

void addStepForResultRenaming(const CorrelatedSubquery &correlated_subquery, QueryPlan &correlated_subquery_plan,
                              const PlannerContextPtr &planner_context) {
  ...
  if (subquery_result_columns.size() != 1)
    throw Exception(ErrorCodes::LOGICAL_ERROR, "Expected to get only 1 result column of correlated subquery, but got {}",
  ...
  if (!expected_result_type->equals(*makeNullableOrLowCardinalityNullableSafe(result_column.type)))
    throw Exception(ErrorCodes::LOGICAL_ERROR, "Expected {} as correlated subquery result, but got {}", expected_result_type->getName(),
  ...
  if (!expected_result_type->equals(*result_column.type)) {
    result_node =
        &dag.addCast(*dag.getOutputs()[0], expected_result_type, correlated_subquery.action_node_name, planner_context->getQueryContext());
  } else {
...
void buildQueryPlanForCorrelatedSubquery(const PlannerContextPtr &planner_context, QueryPlan &query_plan,
  ...
  switch (correlated_subquery.kind) {
    case DB::CorrelatedSubqueryKind::SCALAR: {
      ...
      for (const auto &ctx : correlated_plan.getInterpretersContexts()) query_plan.addInterpreterContext(ctx);

      DecorrelationContext context{.correlated_subquery = correlated_subquery,
      ...
      query_plan = buildLogicalJoin(planner_context, std::move(context.query_plan), std::move(decorrelated_plan), correlated_subquery,
