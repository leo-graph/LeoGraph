...
const ColumnIdentifier &GlobalPlannerContext::createColumnIdentifier(const NameAndTypePair &column,
                                                                     const QueryTreeNodePtr &column_source_node) {
  ...
  if (!inserted) throw Exception(ErrorCodes::LOGICAL_ERROR, "Column identifier {} is already registered", column_identifier);
...
void GlobalPlannerContext::collectTableExpressionDataForCorrelatedColumns(const QueryTreeNodePtr &table_expression_node,
...
PlannerContext::PlannerContext(ContextMutablePtr query_context_, GlobalPlannerContextPtr global_planner_context_,
    ...
    : query_context(std::move(query_context_)),
      ...
      is_ast_level_optimization_allowed(!(query_context->getClientInfo().query_kind == ClientInfo::QueryKind::SECONDARY_QUERY ||
                                          select_query_options_.ignore_ast_optimizations)) {}
...
TableExpressionData &PlannerContext::getOrCreateTableExpressionData(const QueryTreeNodePtr &table_expression_node) {
  ...
  if (auto it = shared_table_expression_data.find(table_expression_node); it != shared_table_expression_data.end()) return *it->second;
...
const TableExpressionData &PlannerContext::getTableExpressionDataOrThrow(const QueryTreeNodePtr &table_expression_node) const {
  ...
  if (table_expression_data_it == table_expression_node_to_data.end()) {
    ...
    throw Exception(ErrorCodes::LOGICAL_ERROR, "Table expression {} is not registered in planner context",
...
TableExpressionData &PlannerContext::getTableExpressionDataOrThrow(const QueryTreeNodePtr &table_expression_node) {
  ...
  if (table_expression_data_it == table_expression_node_to_data.end()) {
    ...
    if (it != shared_table_expression_data.end()) return *it->second;
