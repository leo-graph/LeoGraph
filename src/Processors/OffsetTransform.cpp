  ...
  auto process_pair = [&](size_t pos) {
    ...
    switch (status) {
      ...
      case IProcessor::Status::NeedData:
        return;
      default:
        throw Exception(ErrorCodes::LOGICAL_ERROR, "Unexpected status for OffsetTransform::preparePair : {}",
...
OffsetTransform::Status OffsetTransform::preparePair(PortsData& data) {
  ...
  if (!(rows <= std::numeric_limits<UInt64>::max() - offset && rows_read >= offset + rows)) splitChunk(data);
