...
IProcessor::IProcessor() { processor_index = CurrentThread::isInitialized() ? CurrentThread::get().getNextPipelineProcessorIndex() : 0; }

IProcessor::IProcessor(InputPorts inputs_, OutputPorts outputs_) : inputs(std::move(inputs_)), outputs(std::move(outputs_)) {
  for (auto& port : inputs) port.processor = this;
...
void IProcessor::work() { throw Exception(ErrorCodes::NOT_IMPLEMENTED, "Method 'work' is not implemented for {} processor", getName()); }
...
#ifdef OS_LINUX
std::pair<int, uint32_t> IProcessor::scheduleForEvent() { return {schedule(), EPOLLIN | EPOLLERR}; }
#endif
...
UInt64 IProcessor::getInputPortNumber(const InputPort* input_port) const {
  ...
  for (const auto& port : inputs) {
    if (&port == input_port) return number;
...
UInt64 IProcessor::getOutputPortNumber(const OutputPort* output_port) const {
  ...
  for (const auto& port : outputs) {
    if (&port == output_port) return number;
...
void IProcessor::dump() const { std::cerr << debug(); }

std::string IProcessor::statusToName(std::optional<Status> status) {
  if (status == std::nullopt) return "NotStarted";
