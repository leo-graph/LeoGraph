...
template <typename T>
  requires std::is_same_v<T, InputPorts> || std::is_same_v<T, OutputPorts>
...
PingPongProcessor::PingPongProcessor(const Block &header, size_t num_ports, Order order_)
    : IProcessor(createPortsWithExtra<InputPorts>(header, num_ports), createPortsWithExtra<OutputPorts>(header, num_ports)),
...
bool PingPongProcessor::isPairsFinished() const { return num_finished_pairs == port_pairs.size(); }

IProcessor::Status PingPongProcessor::processRegularPorts() {
  if (isPairsFinished()) return Status::Finished;

  bool need_data = false;

  for (auto &pair : port_pairs) need_data = processPair(pair) || need_data;
...
bool PingPongProcessor::canSend() const { return !is_send && (ready_to_send || isPairsFinished()); }
...
IProcessor::Status PingPongProcessor::prepare() {
  ...
  if (order == Second || is_received) {
    if (!is_send && canSend()) {
      bool sent = sendPing();
      if (!sent) return Status::PortFull;
    }
  }
...
std::pair<InputPort *, OutputPort *> PingPongProcessor::getAuxPorts() { return std::make_pair(&aux_in_port, &aux_out_port); }
