...
InputPorts createInputPorts(SharedHeader header, size_t num_ports, IProcessor::PortNumbers delayed_ports, bool assert_main_ports_empty) {
  if (!assert_main_ports_empty) return InputPorts(num_ports, header);
...
DelayedPortsProcessor::DelayedPortsProcessor(SharedHeader header, size_t num_ports, const PortNumbers& delayed_ports,
  ...
  for (const auto& delayed : delayed_ports) port_pairs[delayed].is_delayed = true;
...
void DelayedPortsProcessor::finishPair(PortsPair& pair) {
  if (!pair.is_finished) {
    if (pair.output_port) pair.output_port->finish();
    ...
    if (pair.output_port) ++num_finished_outputs;

    if (!pair.is_delayed) ++num_finished_main_inputs;
  }
}
...
bool DelayedPortsProcessor::processPair(PortsPair& pair) {
  ...
  if (pair.output_port && !pair.output_port->canPush()) return false;

  pair.input_port->setNeeded();
  if (pair.input_port->hasData()) {
    if (!pair.output_port)
      throw Exception(ErrorCodes::LOGICAL_ERROR, "Input port for DelayedPortsProcessor is assumed to have no data, but it has one");
...
bool DelayedPortsProcessor::shouldSkipDelayed() const { return num_finished_main_inputs + num_delayed_ports < port_pairs.size(); }
...
IProcessor::Status DelayedPortsProcessor::prepare(const PortNumbers& updated_inputs, const PortNumbers& updated_outputs) {
  ...
  if (!are_inputs_initialized && !updated_outputs.empty()) {
    ...
    for (const auto& pair : port_pairs)
      if (!pair.output_port) pair.input_port->setNeeded();
  ...
  if (num_finished_outputs == outputs.size()) {
    for (auto& pair : port_pairs) finishPair(pair);

    return Status::Finished;
  }

  for (const auto& input_number : updated_inputs) {
    if (!skip_delayed || !port_pairs[input_number].is_delayed) need_data = processPair(port_pairs[input_number]) || need_data;
  }
  ...
  if (skip_delayed && !shouldSkipDelayed()) {
    for (auto& pair : port_pairs)
      if (pair.is_delayed) need_data = processPair(pair) || need_data;
  }

  if (num_finished_inputs == port_pairs.size()) return Status::Finished;
