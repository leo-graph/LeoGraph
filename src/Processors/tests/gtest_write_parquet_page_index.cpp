...
void validatePageIndex(String path, std::optional<std::function<void(std::vector<bool>)>> validate_null_pages = std::nullopt,
    ...
    for (int k = 0; k < row_group->num_columns(); k++) {
      ...
      for (size_t l = 0; l < num_pages; l++) {
        ...
        if (!column_index->null_pages().at(l)) {
          ...
          if (column_index->has_null_counts()) ASSERT_GT(header.data_page_header.num_values, column_index->null_counts().at(l));
        } else {
