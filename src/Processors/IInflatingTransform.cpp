...
IInflatingTransform::IInflatingTransform(SharedHeader input_header, SharedHeader output_header)
    : IProcessor({std::move(input_header)}, {std::move(output_header)}), input(inputs.front()), output(outputs.front()) {}
...
void IInflatingTransform::work() {
  if (can_generate) {
    if (generated) throw Exception(ErrorCodes::LOGICAL_ERROR, "IInflatingTransform cannot consume chunk because it already was generated");
    ...
    if (!has_input) throw Exception(ErrorCodes::LOGICAL_ERROR, "IInflatingTransform cannot consume chunk because it wasn't read");
