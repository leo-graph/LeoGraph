...
NegativeLimitTransform::NegativeLimitTransform(SharedHeader header_, UInt64 limit_, UInt64 offset_, size_t num_streams)
    : IProcessor(InputPorts(num_streams, header_), OutputPorts(num_streams, header_)), limit(limit_), offset(offset_) {
  ...
  if (stage == Stage::Pull) {
    ...
    auto process = [&](size_t pos) {
      ...
      switch (status) {
        ...
        default:
          throw Exception(ErrorCodes::LOGICAL_ERROR, "Unexpected status in NegativeLimitTransform::advancePort : {}",
    ...
    if (has_data_need) return Status::NeedData;
...
IProcessor::Status NegativeLimitTransform::tryPushChunkSuffixWithinLimit() {
  ...
  if (queued_row_count <= offset || queued_row_count - offset <= limit) return Status::Finished;
  ...
  for (UInt64 i = 0; i < num_columns; ++i) columns[i] = columns[i]->cut(start, take);
...
IProcessor::Status NegativeLimitTransform::tryPushWholeFrontChunk() {
  ...
  while (queued_row_count > offset) {
    ...
    if (queued_row_count - front_chunk_rows < offset) return Status::Finished;
...
IProcessor::Status NegativeLimitTransform::tryPushChunkPrefixWithinLimit() {
  /// Need to keep at least 'offset' rows queued.
  if (queued_row_count <= offset) return Status::Finished;
  ...
  const UInt64 num_columns = chunk.getNumColumns();
  auto columns = chunk.detachColumns();
  for (UInt64 i = 0; i < num_columns; ++i) columns[i] = columns[i]->cut(0, take);
