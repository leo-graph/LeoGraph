...
ResizeProcessor::Status ResizeProcessor::prepare() {
  ...
  auto inc_current_input = [&]() {
    ...
    if (current_input == inputs.end()) current_input = inputs.begin();
  };
  ...
  auto inc_current_output = [&]() {
    ...
    if (current_output == outputs.end()) current_output = outputs.begin();
  };
  ...
  auto get_status_if_no_outputs = [&]() -> Status {
    ...
    if (all_outs_full_or_unneeded) {
      for (auto& in : inputs) in.setNotNeeded();
  ...
  auto get_status_if_no_inputs = [&]() -> Status {
    if (all_inputs_finished) {
      for (auto& out : outputs) out.finish();
  ...
  while (!is_end_input() && !is_end_output()) {
    ...
    if (output == outputs.end()) return get_status_if_no_outputs();

    auto input = get_next_input();

    if (input == inputs.end()) return get_status_if_no_inputs();

    output->pushData(input->pullData());
  }

  if (is_end_input()) return get_status_if_no_outputs();
...
IProcessor::Status ResizeProcessor::prepare(const PortNumbers& updated_inputs, const PortNumbers& updated_outputs) {
  if (!initialized) {
    ...
    for (auto& input : inputs) input_ports.push_back({.port = &input, .status = InputStatus::NotActive});

    for (auto& output : outputs) output_ports.push_back({.port = &output, .status = OutputStatus::NotActive});
  }
  ...
  if (!is_reading_started && !waiting_outputs.empty()) {
    for (auto& input : inputs) input.setNeeded();
  ...
  if (!waiting_outputs.empty()) return Status::NeedData;
...
IProcessor::Status StrictResizeProcessor::prepare(const PortNumbers& updated_inputs, const PortNumbers& updated_outputs) {
  if (!initialized) {
    ...
    for (auto& input : inputs) input_ports.push_back({.port = &input, .status = InputStatus::NotActive, .waiting_output = -1});

    for (UInt64 i = 0; i < input_ports.size(); ++i) disabled_input_ports.push(i);
  ...
  while (!inputs_with_data.empty()) {
    ...
    if (input_with_data.waiting_output == -1) throw Exception(ErrorCodes::LOGICAL_ERROR, "No associated output for input with data");
  ...
  while (!waiting_outputs.empty()) {
    ...
    if (output.status != OutputStatus::Finished) ++num_finished_outputs;
  ...
  if (disabled_input_ports.empty()) return Status::NeedData;
