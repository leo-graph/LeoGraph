...
Chunk::Chunk(DB::Columns columns_, UInt64 num_rows_) : columns(std::move(columns_)), num_rows(num_rows_) { checkNumRowsIsConsistent(); }

static Columns unmuteColumns(MutableColumns&& mutable_columns) {
  Columns columns;
  columns.reserve(mutable_columns.size());
  for (auto& col : mutable_columns) columns.emplace_back(std::move(col));
...
MutableColumns Chunk::mutateColumns() {
  ...
  for (size_t i = 0; i < num_columns; ++i) mutable_columns[i] = IColumn::mutate(std::move(columns[i]));
...
MutableColumns Chunk::cloneEmptyColumns() const {
  size_t num_columns = columns.size();
  MutableColumns mutable_columns(num_columns);
  for (size_t i = 0; i < num_columns; ++i) mutable_columns[i] = columns[i]->cloneEmpty();
  return mutable_columns;
}
...
void Chunk::addColumn(ColumnPtr column) {
  if (empty())
    ...
    throw Exception(ErrorCodes::LOGICAL_ERROR, "Invalid number of rows in Chunk {} column {}: expected {}, got {}", dumpStructure(),
...
void Chunk::addColumn(size_t position, ColumnPtr column) {
  if (position >= columns.size())
    throw Exception(ErrorCodes::POSITION_OUT_OF_BOUND, "Position {} out of bound in Chunk::addColumn(), max position = {}", position,
                    !columns.empty() ? columns.size() - 1 : 0);
  if (empty())
    num_rows = column->size();
  else if (column->size() != num_rows)
    throw Exception(ErrorCodes::LOGICAL_ERROR, "Invalid number of rows in Chunk column {}: expected {}, got {}", column->getName(),
                    num_rows, column->size());
...
void Chunk::erase(size_t position) {
  if (columns.empty()) throw Exception(ErrorCodes::POSITION_OUT_OF_BOUND, "Chunk is empty");

  if (position >= columns.size())
    throw Exception(ErrorCodes::POSITION_OUT_OF_BOUND, "Position {} out of bound in Chunk::erase(), max position = {}", toString(position),
                    toString(!columns.empty() ? columns.size() - 1 : 0));
...
void Chunk::append(const Chunk& chunk) { append(chunk, 0, chunk.getNumRows()); }
...
void convertToFullIfConst(Chunk& chunk) {
  ...
  for (auto& column : columns) column = column->convertToFullColumnIfConst();
...
void convertToFullIfSparse(Chunk& chunk) {
  size_t num_rows = chunk.getNumRows();
  auto columns = chunk.detachColumns();
  for (auto& column : columns) column = recursiveRemoveSparse(column);
  chunk.setColumns(std::move(columns), num_rows);
}
...
void removeSpecialColumnRepresentations(Chunk& chunk) {
  size_t num_rows = chunk.getNumRows();
  auto columns = chunk.detachColumns();
  for (auto& column : columns) column = removeSpecialRepresentations(column);
  chunk.setColumns(std::move(columns), num_rows);
}
