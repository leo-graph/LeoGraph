...
class IOutputFormat : public IProcessor {
 public:
  ...
  void setStartTime(UInt64 start, bool is_running) {
    ...
    if (!is_running) statistics.watch.stop();
  }
  ...
  void setProgressWriteFrequencyMicroseconds(size_t value) { progress_write_frequency_us = value; }
 ...
 protected:
  ...
  virtual bool writesProgressConcurrently() const { return false; }
