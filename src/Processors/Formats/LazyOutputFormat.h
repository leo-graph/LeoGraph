...
class LazyOutputFormat : public IOutputFormat {
 public:
  ...
  void onCancel() noexcept override { queue.clearAndFinish(); }

  void finalizeImpl() override { queue.finish(); }
 ...
 protected:
  void consume(Chunk chunk) override { (void)(queue.emplace(std::move(chunk))); }
