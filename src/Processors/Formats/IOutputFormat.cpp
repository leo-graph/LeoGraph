...
IOutputFormat::IOutputFormat(SharedHeader header_, WriteBuffer &out_) : IProcessor({header_, header_, header_}, {}), out(out_) {}
...
IOutputFormat::Status IOutputFormat::prepare() {
  ...
  for (auto kind : {Main, Totals, Extremes}) {
    ...
    if (kind != Main && !input.isConnected()) continue;
  ...
  if (!finalized) return Status::Ready;
...
void IOutputFormat::work() {
  ...
  if (finished && !finalized) {
    if (rows_before_limit_counter && rows_before_limit_counter->hasAppliedStep()) setRowsBeforeLimit(rows_before_limit_counter->get());
...
void IOutputFormat::flushImpl() {
  ...
  if (auto *out_with_nested = dynamic_cast<WriteBufferWithOwnMemoryDecorator *>(&out)) out_with_nested->getNestedBuffer()->next();
}
...
void IOutputFormat::onProgress(const Progress &progress) {
  fiu_do_on(FailPoints::output_format_sleep_on_progress, { sleepForMilliseconds(100); });
...
void IOutputFormat::setProgress(Progress progress) { statistics.progress = std::move(progress); }

InputPort &IOutputFormat::getPort(PortKind kind) { return *std::next(inputs.begin(), kind); }
