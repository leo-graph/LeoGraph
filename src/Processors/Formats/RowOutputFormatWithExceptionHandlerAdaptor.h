...
class RowOutputFormatWithExceptionHandlerAdaptor : public Base {
 public:
  ...
  void consume(DB::Chunk chunk) override {
    ...
    for (size_t row = 0; row < num_rows; ++row) {
      ...
      if (Base::haveWrittenData()) writeRowBetweenDelimiter();
  ...
  void flushImpl() override {
    if (peekable_out) peekable_out->next();

    Base::flushImpl();
  }

  void finalizeBuffers() override {
    if (peekable_out && !peekable_out->isCanceled()) peekable_out->finalize();
    Base::finalizeBuffers();
  }

  void resetFormatterImpl() override {
    Base::resetFormatterImpl();
    if (peekable_out) peekable_out = std::make_unique<PeekableWriteBuffer>(*Base::getWriteBufferPtr());
  }
 ...
 protected:
  ...
  WriteBuffer* getWriteBufferPtr() override {
    if (peekable_out) return peekable_out.get();
