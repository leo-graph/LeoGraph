...
PullingOutputFormat::PullingOutputFormat(SharedHeader header, std::atomic_bool& consume_data_flag_)
    : IOutputFormat(header, out), has_data_flag(consume_data_flag_) {}

void PullingOutputFormat::consume(Chunk chunk) {
  if (data) throw Exception(ErrorCodes::LOGICAL_ERROR, "PullingOutputFormat cannot consume chunk because it already has data");
...
void PullingOutputFormat::setRowsBeforeLimit(size_t rows_before_limit) { info.setRowsBeforeLimit(rows_before_limit); }
