...
std::pair<String, String> RowInputFormatWithDiagnosticInfo::getDiagnosticAndRawDataImpl(bool is_errors_record) {
  ...
  if (in->eof())
    return std::make_pair("Buffer has gone, cannot extract information about what has been parsed.",
  ...
  if (!is_errors_record && offset_of_prev_row <= in->buffer().size()) {
    ...
    out_diag << "\nRow " << getRowNum() - 1 << ":\n";
    if (!parseRowAndPrintDiagnosticInfo(columns, out_diag)) return std::make_pair(out_diag.str(), out_data.str());
  } else {
...
std::pair<String, String> RowInputFormatWithDiagnosticInfo::getDiagnosticAndRawData() { return getDiagnosticAndRawDataImpl(true); }
...
bool RowInputFormatWithDiagnosticInfo::deserializeFieldAndPrintDiagnosticInfo(const String& col_name, const DataTypePtr& type,
                                                                              IColumn& column, WriteBuffer& out, size_t file_column) {
  out << "Column " << file_column << ", "
      << std::string((file_column < 10    ? 2
                      : file_column < 100 ? 1
                                          : 0),
                     ' ')
  ...
  if (curr_position < prev_position) throw Exception(ErrorCodes::LOGICAL_ERROR, "Parsing is non-deterministic.");
