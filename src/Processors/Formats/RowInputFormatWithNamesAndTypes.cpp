...
bool checkIfAllValuesAreTypeNames(const std::vector<String> &names) {
  for (const auto &name : names) {
    if (!DataTypeFactory::instance().tryGet(name)) return false;
  }
  return true;
}
...
RowInputFormatWithNamesAndTypes<FormatReaderImpl>::RowInputFormatWithNamesAndTypes(SharedHeader header_, ReadBuffer &in_,
                                                                                   const Params &params_, bool is_binary_, bool with_names_,
                                                                                   bool with_types_, const FormatSettings &format_settings_,
...
void RowInputFormatWithNamesAndTypes<FormatReaderImpl>::readPrefix() {
  ...
  if (with_names) column_names = format_reader->readNames();
  ...
  if (!with_names && !with_types && try_detect_header) tryDetectHeader(column_names, type_names);
  ...
  if (!type_names.empty() && format_settings.with_types_use_header) {
    if (type_names.size() != column_mapping->column_indexes_for_input_fields.size())
      throw Exception(ErrorCodes::INCORRECT_DATA, "The number of data types differs from the number of column names in input data");
    ...
    for (size_t i = 0; i < type_names.size(); ++i) {
      if (column_mapping->column_indexes_for_input_fields[i] &&
        ...
        throw Exception(ErrorCodes::INCORRECT_DATA, "Type of '{}' must be {}, not {}",
  ...
  if (format_settings.force_null_for_omitted_fields) {
    for (auto index : column_mapping->not_presented_columns)
      if (!isNullableOrLowCardinalityNullable(data_types[index]))
        throw Exception(ErrorCodes::TYPE_MISMATCH, "Cannot insert NULL value into a column type '{}' at index {}",
                        data_types[index]->getName(), index);
  }
}

template <typename FormatReaderImpl>
void RowInputFormatWithNamesAndTypes<FormatReaderImpl>::tryDetectHeader(std::vector<String> &column_names_out,
                                                                        std::vector<String> &type_names_out) {
...
bool RowInputFormatWithNamesAndTypes<FormatReaderImpl>::readRow(MutableColumns &columns, RowReadExtension &ext) {
  ...
  if (likely(getRowNum() != 0 || with_names || with_types || is_header_detected)) format_reader->skipRowBetweenDelimiter();
  ...
  if (allow_variable_number_of_columns) {
    ...
    for (; file_column < column_mapping->column_indexes_for_input_fields.size(); ++file_column) {
        ...
        while (file_column < column_mapping->column_indexes_for_input_fields.size()) {
          ...
          if (rem_column_index) {
            if (format_settings.force_null_for_omitted_fields && !isNullableOrLowCardinalityNullable(data_types[*rem_column_index]))
              throw Exception(ErrorCodes::TYPE_MISMATCH, "Cannot insert NULL value into a column type '{}' at index {}",
                              data_types[*rem_column_index]->getName(), *rem_column_index);
      ...
      if (file_column != 0) format_reader->skipFieldDelimiter();
      ...
      if (column_index)
        ext.read_columns[*column_index] =
            format_reader->readField(*columns[*column_index], data_types[*column_index], serializations[*column_index], is_last_file_column,
  ...
  if (!format_settings.defaults_for_omitted_fields) ext.read_columns.assign(ext.read_columns.size(), true);
...
size_t RowInputFormatWithNamesAndTypes<FormatReaderImpl>::countRows(size_t max_block_size) {
  if (unlikely(end_of_stream)) return 0;
  ...
  if (num_rows == 0 || num_rows < max_block_size) end_of_stream = true;
...
bool RowInputFormatWithNamesAndTypes<FormatReaderImpl>::parseRowAndPrintDiagnosticInfo(MutableColumns &columns, WriteBuffer &out) {
  ...
  if (!format_reader->tryParseSuffixWithDiagnosticInfo(out)) return false;

  if (likely(getRowNum() != 0) && !format_reader->parseRowBetweenDelimiterWithDiagnosticInfo(out)) return false;

  if (!format_reader->parseRowStartWithDiagnosticInfo(out)) return false;
  ...
  for (size_t file_column = 0; file_column < column_mapping->column_indexes_for_input_fields.size(); ++file_column) {
    if (column_mapping->column_indexes_for_input_fields[file_column].has_value()) {
      ...
      if (!deserializeFieldAndPrintDiagnosticInfo(header.getByPosition(col_idx).name, data_types[col_idx], *columns[col_idx], out,
                                                  file_column))
        return false;
    } else {
      ...
      if (!deserializeFieldAndPrintDiagnosticInfo(skipped_column_str, skipped_column_type, *skipped_column, out, file_column)) return false;
    }

    /// Delimiters
    if (file_column + 1 != column_mapping->column_indexes_for_input_fields.size()) {
      if (!format_reader->parseFieldDelimiterWithDiagnosticInfo(out)) return false;
    }
  }
...
FormatWithNamesAndTypesSchemaReader::FormatWithNamesAndTypesSchemaReader(ReadBuffer &in_, const FormatSettings &format_settings_,
                                                                         bool with_names_, bool with_types_,
                                                                         FormatWithNamesAndTypesReader *format_reader_,
                                                                         DataTypePtr default_type_, bool try_detect_header_)
    : IRowSchemaReader(in_, format_settings_, default_type_),
      with_names(with_names_),
      with_types(with_types_),
      format_reader(format_reader_),
      try_detect_header(try_detect_header_) {}

NamesAndTypesList FormatWithNamesAndTypesSchemaReader::readSchema() {
  if (with_names || with_types) skipBOMIfExists(in);
  ...
  if (!with_names && !with_types && try_detect_header) tryDetectHeader(column_names, data_type_names);

  if (!data_type_names.empty()) {
    if (data_type_names.size() != column_names.size())
      throw Exception(ErrorCodes::INCORRECT_DATA, "The number of column names {} differs with the number of types {}", column_names.size(),
                      data_type_names.size());
  ...
  if (!column_names.empty()) setColumnNames(column_names);
...
bool checkIfAllTypesAreString(const DataTypes &types) {
  for (const auto &type : types)
    if (!type || !isString(removeNullable(removeLowCardinality(type)))) return false;
  return true;
}

bool haveNotStringAndNotNullType(const DataTypes &types) {
  for (const auto &type : types)
    if (type && !isString(removeNullable(removeLowCardinality(type))) && !type->onlyNull()) return true;
  return false;
}
}  // namespace
...
void FormatWithNamesAndTypesSchemaReader::tryDetectHeader(std::vector<String> &column_names, std::vector<String> &type_names) {
  ...
  bool second_row_can_be_type_names =
  ...
  for (size_t i = 0; i != first_row_types.size(); ++i) default_colum_names.push_back("c" + std::to_string(i + 1));
  ...
  while (true) {
    ...
    if (haveNotStringAndNotNullType(data_types)) {
      ...
      if (second_row_can_be_type_names) type_names = readNamesFromFields(second_row_values);
      return;
    }
    ...
    chooseResultColumnTypes(*this, data_types, *next_row_types_maybe, getDefaultDataTypeForEscapingRule(FormatSettings::EscapingRule::CSV),
                            default_colum_names, row);
  }
}
