...
class ISchemaReader {
 public:
  ...
  virtual void transformTypesFromDifferentFilesIfNeeded(DataTypePtr &type, DataTypePtr &new_type) {
    transformTypesIfNeeded(type, new_type);
  }
...
void chooseResultColumnType(SchemaReader &schema_reader, DataTypePtr &type, DataTypePtr &new_type, const DataTypePtr &default_type,
                            const String &column_name, size_t row, const String &hints_parsing_error = "") {
  if (!type) {
    type = new_type;
    return;
  }

  if (!new_type || type->equals(*new_type)) return;

  schema_reader.transformTypesIfNeeded(type, new_type);
  if (type->equals(*new_type)) return;
  ...
  if (default_type)
    ...
    if (hints_parsing_error.empty())
      throw Exception(ErrorCodes::TYPE_MISMATCH,
                      ...
                      new_type->getName(), column_name, row, type->getName());
    throw Exception(ErrorCodes::TYPE_MISMATCH,
                    "Automatically defined type {} for column '{}' in row {} differs from type defined by previous rows: {}. "
                    "Column types from setting schema_inference_hints couldn't be parsed because of error: {}",
                    new_type->getName(), column_name, row, type->getName(), hints_parsing_error);
  }
}

template <class SchemaReader>
void chooseResultColumnTypes(SchemaReader &schema_reader, DataTypes &types, DataTypes &new_types, const DataTypePtr &default_type,
                             const std::vector<String> &column_names, size_t row) {
  if (types.size() != new_types.size()) throw Exception(ErrorCodes::INCORRECT_DATA, "Rows have different amount of values");

  if (types.size() != column_names.size())
    throw Exception(ErrorCodes::INCORRECT_DATA, "The number of column names {} differs from the number of types {}", column_names.size(),
                    types.size());
