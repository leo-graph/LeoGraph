...
ChunkInfoRowNumbers::Ptr ChunkInfoRowNumbers::clone() const {
  ...
  if (applied_filter.has_value()) res->applied_filter.emplace(applied_filter->begin(), applied_filter->end());
  return res;
}
...
Chunk IInputFormat::generate() {
  try {
    ...
    if (!file_name.empty()) e.addMessage(fmt::format("(in file/uri {})", file_name));
    throw;
  }
}
...
void IInputFormat::setReadBuffer(ReadBuffer& in_) { in = &in_; }
...
void IInputFormat::resetOwnedBuffers() { owned_buffers.clear(); }

void IInputFormat::onFinish() { resetReadBuffer(); }
