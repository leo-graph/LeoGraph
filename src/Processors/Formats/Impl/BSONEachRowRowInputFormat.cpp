...
BSONEachRowRowInputFormat::BSONEachRowRowInputFormat(ReadBuffer &in_, SharedHeader header_, Params params_,
...
inline size_t BSONEachRowRowInputFormat::columnIndex(std::string_view name, size_t key_index) {
  ...
  if (prev_positions.size() > key_index && prev_positions[key_index] != BlockNameMap::const_iterator{} &&
  ...
  if (it != name_map.end()) {
    if (key_index < prev_positions.size()) prev_positions[key_index] = it;
...
static void readAndInsertInteger(ReadBuffer &in, IColumn &column, const DataTypePtr &data_type, BSONType bson_type) {
  ...
  if (bson_type == BSONType::INT32) {
    ...
    throw Exception(ErrorCodes::ILLEGAL_COLUMN, "Cannot insert BSON {} into column with type {}", getBSONTypeName(bson_type),
                    data_type->getName());
  }
}
...
static void readAndInsertIPv4(ReadBuffer &in, IColumn &column, BSONType bson_type) {
  ...
  if (bson_type != BSONType::INT32) throw Exception(ErrorCodes::ILLEGAL_COLUMN, "Cannot insert BSON Int32 into column with type IPv4");
...
static void readAndInsertBigInteger(ReadBuffer &in, IColumn &column, const DataTypePtr &data_type, BSONType bson_type) {
  ...
  if (size != sizeof(ValueType))
    throw Exception(ErrorCodes::INCORRECT_DATA,
                    ...
                    data_type->getName(), size, sizeof(ValueType));
...
static void readAndInsertString(ReadBuffer &in, IColumn &column, BSONType bson_type) {
  if (bson_type == BSONType::STRING || bson_type == BSONType::SYMBOL || bson_type == BSONType::JAVA_SCRIPT_CODE) {
    auto size = readBSONSize(in);
    if (size == 0) throw Exception(ErrorCodes::INCORRECT_DATA, "Incorrect size of a string (zero) in BSON");
    ...
    if (subtype == BSONBinarySubtype::BINARY || subtype == BSONBinarySubtype::BINARY_OLD)
      ...
      throw Exception(ErrorCodes::ILLEGAL_COLUMN, "Cannot insert BSON Binary subtype {} into String column",
...
static void readAndInsertIPv6(ReadBuffer &in, IColumn &column, BSONType bson_type) {
  ...
  if (size != sizeof(IPv6))
    throw Exception(ErrorCodes::INCORRECT_DATA,
                    "Cannot parse value of type IPv6, size of binary data is not equal to the binary size of IPv6 value: {} != {}", size,
                    sizeof(IPv6));
...
static void readAndInsertUUID(ReadBuffer &in, IColumn &column, BSONType bson_type) {
  ...
  if (subtype != BSONBinarySubtype::UUID && subtype != BSONBinarySubtype::UUID_OLD)
    throw Exception(ErrorCodes::ILLEGAL_COLUMN, "Cannot insert BSON Binary subtype {} into UUID column", getBSONBinarySubtypeName(subtype));

  if (size != sizeof(UUID))
    throw Exception(ErrorCodes::INCORRECT_DATA,
                    "Cannot parse value of type UUID, size of binary data is not equal to the binary size of UUID value: {} != {}", size,
                    sizeof(UUID));
  ...
  auto read_tuple = [&]() {
    ...
    if (read_nested_columns != elements_size)
      throw Exception(ErrorCodes::INCORRECT_DATA,
                      ...
                      data_type->getName(), elements_size, read_nested_columns);
  };
...
bool BSONEachRowRowInputFormat::readField(IColumn &column, const DataTypePtr &data_type, BSONType bson_type) {
  if (bson_type == BSONType::NULL_VALUE) {
    ...
    if (!format_settings.null_as_default)
      throw Exception(ErrorCodes::ILLEGAL_COLUMN, "Cannot insert BSON Null value into non-nullable column with type {}",
                      data_type->getName());
...
static void skipBSONField(ReadBuffer &in, BSONType type) {
  switch (type) {
    ...
    case BSONType::JAVA_SCRIPT_CODE_W_SCOPE: {
      BSONSizeT size;
      readBinaryLittleEndian(size, in);
      if (size < sizeof(BSONSizeT)) throw Exception(ErrorCodes::INCORRECT_DATA, "Invalid java code_w_scope size: {}", size);
      in.ignore(size - sizeof(size));
      break;
    }
...
DataTypePtr BSONEachRowSchemaReader::getDataTypeFromBSONField(BSONType type, bool allow_to_skip_unsupported_types, bool &skip) {
  switch (type) {
    ...
    case BSONType::DOCUMENT: {
      ...
      if (nested_types.empty() || !nested_types[0]) return nullptr;

      for (size_t i = 1; i != nested_types.size(); ++i) {
        if (!nested_types[i]) return nullptr;

        types_are_equal &= nested_types[i]->equals(*nested_types[0]);
      }

      if (types_are_equal) return std::make_shared<DataTypeMap>(std::make_shared<DataTypeString>(), nested_types[0]);
    ...
    case BSONType::ARRAY: {
      ...
      if (types_are_equal) return std::make_shared<DataTypeArray>(nested_types[0]);
...
NamesAndTypesList BSONEachRowSchemaReader::getDataTypesFromBSONDocument(bool allow_to_skip_unsupported_types) {
  ...
  while (in.count() - document_start + sizeof(BSON_DOCUMENT_END) != document_size) {
    ...
    if (!skip) names_and_types.emplace_back(name, type);
  }
...
void BSONEachRowSchemaReader::transformTypesIfNeeded(DataTypePtr &type, DataTypePtr &new_type) {
  ...
  if (least_supertype) type = new_type = least_supertype;
}

static std::pair<bool, size_t> fileSegmentationEngineBSONEachRow(ReadBuffer &in, DB::Memory<> &memory, size_t min_bytes, size_t max_rows) {
  ...
  while (!in.eof() && memory.size() < min_bytes && number_of_rows < max_rows) {
    ...
    if (document_size < sizeof(document_size)) throw Exception(ErrorCodes::INCORRECT_DATA, "Size of BSON document is invalid");
...
void registerInputFormatBSONEachRow(FormatFactory &factory) {
  factory.registerInputFormat(
      "BSONEachRow", [](ReadBuffer &buf, const Block &sample, IRowInputFormat::Params params, const FormatSettings &settings) {
