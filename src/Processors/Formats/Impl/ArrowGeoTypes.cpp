...
const std::string *extractGeoMetadata(std::shared_ptr<const arrow::KeyValueMetadata> metadata) {
  ...
  for (Int64 i = 0; i < metadata->size(); ++i)
    if (metadata->key(i) == "geo") return &metadata->value(i);
...
std::unordered_map<String, GeoColumnMetadata> parseGeoMetadataEncoding(const std::string *geo_json_str) {
  if (!geo_json_str) return {};

  Poco::JSON::Parser parser;
  Poco::Dynamic::Var result = parser.parse(*geo_json_str);
  const Poco::JSON::Object::Ptr &obj = result.extract<Poco::JSON::Object::Ptr>();

  if (!obj->has("columns")) throw Exception(ErrorCodes::BAD_ARGUMENTS, "Incorrect geo json metadata: missing \"columns\"");
...
inline LineString<CartesianPoint> parseWKTLine(ReadBuffer &in_buffer) {
  ...
  while (true) {
    ...
    if (readItemEnding(in_buffer)) break;
  }
  return ls;
}
...
GeometricObject parseWKTFormat(ReadBuffer &in_buffer) {
  ...
  while (true) {
    char current_symbol;
    if (!in_buffer.peek(current_symbol)) break;
  ...
  if (type == "LINESTRING") return parseWKTLine(in_buffer);
  if (type == "POLYGON") return parseWKTPolygon(in_buffer);
  if (type == "MULTILINESTRING") return parseWKTPolygon(in_buffer);
  if (type == "MULTIPOLYGON") return parseWKTMultiPolygon(in_buffer);
...
DataTypePtr getGeoDataType(GeoType type) {
  switch (type) {
    ...
    case GeoType::LineString:
      return DataTypeFactory::instance().get("LineString");
    case GeoType::Polygon:
      return DataTypeFactory::instance().get("Polygon");
    case GeoType::MultiLineString:
      return DataTypeFactory::instance().get("MultiLineString");
...
static void appendLineStringToGeoColumn(const LineString<CartesianPoint> &line, IColumn &col) {
  ...
  for (const auto &point : line) appendPointToGeoColumn(point, array.getData());
...
static void appendMultiLineStringToGeoColumn(const MultiLineString<CartesianPoint> &multilinestring, IColumn &col) {
  ...
  for (const auto &line : multilinestring) appendLineStringToGeoColumn(line, array.getData());
...
static void appendMultiPolygonToGeoColumn(const MultiPolygon<CartesianPoint> &multipolygon, IColumn &col) {
  ...
  for (const auto &polygon : multipolygon) appendPolygonToGeoColumn(polygon, array.getData());
