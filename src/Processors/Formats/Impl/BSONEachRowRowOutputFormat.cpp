...
BSONEachRowRowOutputFormat::BSONEachRowRowOutputFormat(WriteBuffer &out_, SharedHeader header_, const FormatSettings &settings_)
    : IRowOutputFormat(header_, out_), settings(settings_) {
  const auto &sample = getPort(PortKind::Main).getHeader();
  fields.reserve(sample.columns());
  for (const auto &field : sample.getNamesAndTypes()) fields.emplace_back(toValidUTF8String(field.name, settings), field.type);
}
...
size_t BSONEachRowRowOutputFormat::countBSONFieldSize(const IColumn &column, const DataTypePtr &data_type, size_t row_num,
  ...
  switch (data_type->getTypeId()) {
    ...
    case TypeIndex::UInt8: {
      if (isBool(data_type)) return size + 1;
    ...
    case TypeIndex::String: {
      ...
      return size + sizeof(BSONSizeT) + string_column.getDataAt(row_num).size() +
             1;  // Size of data + data + \0 or BSON subtype (in case of BSON binary)
    }
    ...
    case TypeIndex::Nullable: {
      ...
      if (column_nullable.isNullAt(row_num)) return size;  /// Null has no value, just type
    ...
    case TypeIndex::Array: {
      ...
      for (size_t i = 0; i < array_size; ++i)
        document_size += countBSONFieldSize(nested_column, nested_type, offset + i, std::to_string(i), current_path,
                                            nested_document_sizes);  // Add size of each value from array
    ...
    case TypeIndex::Tuple: {
      ...
      for (size_t i = 0; i < nested_columns.size(); ++i) {
        ...
        document_size += countBSONFieldSize(*nested_columns[i], nested_types[i], row_num, key_name, current_path,
                                            nested_document_sizes);  // Add size of each value from tuple
      }
    ...
    case TypeIndex::Map: {
      ...
      for (size_t i = 0; i < map_size; ++i) {
        ...
        auto s = countBSONFieldSize(*value_column, value_type, offset + i, toValidUTF8String(buf.str(), settings), current_path,
                                    nested_document_sizes);
        document_size += s;
        buf.restart();
      }
...
void BSONEachRowRowOutputFormat::serializeField(const IColumn &column, const DataTypePtr &data_type, size_t row_num, const String &name,
                                                const String &path, std::unordered_map<String, size_t> &nested_document_sizes) {
  switch (data_type->getTypeId()) {
    case TypeIndex::Float32: {
    ...
    case TypeIndex::Tuple: {
      ...
      for (size_t i = 0; i < nested_columns.size(); ++i)
        serializeField(*nested_columns[i], nested_types[i], row_num, toValidUTF8String(nested_names[i], settings), current_path,
                       nested_document_sizes);
...
void BSONEachRowRowOutputFormat::write(const Columns &columns, size_t row_num) {
  ...
  if (actual_document_size != document_size)
    throw Exception(ErrorCodes::LOGICAL_ERROR, "The actual size of the BSON document does not match the estimated size: {} != {}",
                    actual_document_size, document_size);
}

void registerOutputFormatBSONEachRow(FormatFactory &factory) {
  factory.registerOutputFormat("BSONEachRow", [](WriteBuffer &buf, const Block &sample, const FormatSettings &_format_settings,
