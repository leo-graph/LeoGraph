...
void SQLInsertRowOutputFormat::writeRowStartDelimiter() {
  if (rows_in_line == 0) printLineStart();
  writeChar('(', out);
}
...
void SQLInsertRowOutputFormat::printLineStart() {
  ...
  if (format_settings.sql_insert.include_column_names) printColumnNames();
...
void SQLInsertRowOutputFormat::printColumnNames() {
  ...
  for (size_t i = 0; i != column_names.size(); ++i) {
    if (format_settings.sql_insert.quote_names) writeChar('`', out);

    writeString(column_names[i], out);

    if (format_settings.sql_insert.quote_names) writeChar('`', out);

    if (i + 1 != column_names.size()) writeCString(", ", out);
  }
  writeChar(')', out);
}
...
void SQLInsertRowOutputFormat::writeFieldDelimiter() { writeCString(", ", out); }
...
void SQLInsertRowOutputFormat::writeSuffix() { writeCString(";\n", out); }

void SQLInsertRowOutputFormat::resetFormatterImpl() { rows_in_line = 0; }

void registerOutputFormatSQLInsert(FormatFactory& factory) {
  factory.registerOutputFormat(
      "SQLInsert", [](WriteBuffer& buf, const Block& sample, const FormatSettings& settings, FormatFilterInfoPtr /*format_filter_info*/) {
