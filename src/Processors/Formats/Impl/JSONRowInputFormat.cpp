...
JSONRowInputFormat::JSONRowInputFormat(std::unique_ptr<PeekableReadBuffer> buf, SharedHeader header_, DB::IRowInputFormat::Params params_,
                                       const DB::FormatSettings& format_settings_)
    : JSONEachRowRowInputFormat(*buf, header_, params_, format_settings_, false),
      validate_types_from_metadata(format_settings_.json.validate_types_from_metadata),
      peekable_buf(std::move(buf)) {}
...
void JSONRowInputFormat::readPrefix() {
  ...
  if (JSONUtils::checkAndSkipObjectStart(*peekable_buf) &&
      ...
      JSONUtils::checkAndSkipComma(*peekable_buf) && JSONUtils::skipUntilFieldInObject(*peekable_buf, "data", format_settings.json) &&
...
JSONRowSchemaReader::JSONRowSchemaReader(std::unique_ptr<PeekableReadBuffer> buf, const DB::FormatSettings& format_settings_,
                                         bool fallback_to_json_each_row_)
    : JSONEachRowSchemaReader(*buf, format_settings_),
      peekable_buf(std::move(buf)),
      fallback_to_json_each_row(fallback_to_json_each_row_) {}
...
NamesAndTypesList JSONRowSchemaReader::readSchema() {
  ...
  if (fallback_to_json_each_row) {
    ...
    if (JSONUtils::checkAndSkipObjectStart(*peekable_buf) &&
        JSONUtils::tryReadMetadata(*peekable_buf, names_and_types, format_settings.json))
...
void registerInputFormatJSON(FormatFactory& factory) {
  factory.registerInputFormat(
      "JSON", [](ReadBuffer& buf, const Block& sample, IRowInputFormat::Params params, const FormatSettings& settings) {
...
void registerJSONSchemaReader(FormatFactory& factory) {
  auto register_schema_reader = [&](const String& format, bool fallback_to_json_each_row) {
    factory.registerSchemaReader(format, [fallback_to_json_each_row](ReadBuffer& buf, const FormatSettings& format_settings) {
      return std::make_unique<JSONRowSchemaReader>(buf, format_settings, fallback_to_json_each_row);
    });
