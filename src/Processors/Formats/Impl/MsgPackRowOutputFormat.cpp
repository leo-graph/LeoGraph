...
void MsgPackRowOutputFormat::serializeField(const IColumn &column, DataTypePtr data_type, size_t row_num) {
  switch (data_type->getTypeId()) {
    ...
    case TypeIndex::Tuple: {
      ...
      for (size_t i = 0; i < nested_types.size(); ++i) serializeField(*nested_columns[i], nested_types[i], row_num);
      return;
    }
...
void registerOutputFormatMsgPack(FormatFactory &factory) {
  factory.registerOutputFormat(
      "MsgPack", [](WriteBuffer &buf, const Block &sample, const FormatSettings &settings, FormatFilterInfoPtr /*format_filter_info*/) {
