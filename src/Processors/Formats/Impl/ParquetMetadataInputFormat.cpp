...
static NamesAndTypesList getHeaderForParquetMetadata() {
  NamesAndTypesList names_and_types{
      ...
      {"columns", std::make_shared<DataTypeArray>(std::make_shared<DataTypeTuple>(
                      DataTypes{std::make_shared<DataTypeString>(), std::make_shared<DataTypeString>(), std::make_shared<DataTypeUInt64>(),
                                std::make_shared<DataTypeUInt64>(), std::make_shared<DataTypeString>(), std::make_shared<DataTypeString>(),
                                std::make_shared<DataTypeString>(), std::make_shared<DataTypeUInt64>(), std::make_shared<DataTypeUInt64>(),
                                std::make_shared<DataTypeString>(), std::make_shared<DataTypeArray>(std::make_shared<DataTypeString>())},
                      Names{"name", "path", "max_definition_level", "max_repetition_level", "physical_type", "logical_type", "compression",
                            "total_uncompressed_size", "total_compressed_size", "space_saved", "encodings"}))},
      {"row_groups",
       std::make_shared<DataTypeArray>(std::make_shared<DataTypeTuple>(
           DataTypes{
               std::make_shared<DataTypeUInt64>(), std::make_shared<DataTypeUInt64>(), std::make_shared<DataTypeUInt64>(),
               ...
               std::make_shared<DataTypeArray>(std::make_shared<DataTypeTuple>(
                   DataTypes{
                       ...
                       std::make_shared<DataTypeTuple>(DataTypes{std::make_shared<DataTypeNullable>(std::make_shared<DataTypeUInt64>()),
...
void checkHeader(const Block &header) {
  ...
  for (const auto &[name, type] : expected_names_and_types) name_to_type[name] = type;
  ...
  for (const auto &[name, type] : header.getNamesAndTypes()) {
    ...
    if (!it->second->equals(*type))
      throw Exception(ErrorCodes::BAD_ARGUMENTS, "Unexpected type {} for column {}. Expected type: {}", type->getName(), name,
                      it->second->getName());
  }
}

static std::shared_ptr<parquet::FileMetaData> getFileMetadata(ReadBuffer &in, const FormatSettings &format_settings,
...
Chunk ParquetMetadataInputFormat::read() {
  ...
  for (const auto &name : header.getNames()) {
    ...
    /// total_uncompressed_size
      ...
      for (int32_t i = 0; i != metadata->num_row_groups(); ++i) total_uncompressed_size += metadata->RowGroup(i)->total_byte_size();
    ...
    /// total_compressed_size
      ...
      for (int32_t i = 0; i != metadata->num_row_groups(); ++i) total_compressed_size += metadata->RowGroup(i)->total_compressed_size();
...
void ParquetMetadataInputFormat::fillColumnsMetadata(const std::shared_ptr<parquet::FileMetaData> &metadata, MutableColumnPtr &column) {
  ...
  for (int32_t column_i = 0; column_i != num_columns; ++column_i) {
    ...
    if (metadata->num_row_groups() > 0) {
      ...
      /// space_saved
      String space_saved =
          fmt::format("{:.4}%", (1 - static_cast<double>(total_compressed_size) / static_cast<double>(total_uncompressed_size)) * 100);
...
void ParquetMetadataInputFormat::fillColumnChunksMetadata(const std::unique_ptr<parquet::RowGroupMetaData> &row_group_metadata,
                                                          IColumn &column) {
  auto &array_column = assert_cast<ColumnArray &>(column);
  auto &tuple_column = assert_cast<ColumnTuple &>(array_column.getData());
  for (int32_t column_i = 0; column_i != row_group_metadata->num_columns(); ++column_i) {
    ...
    if (have_statistics)
      fillColumnStatistics(column_chunk_metadata->statistics(), tuple_column.getColumn(5),
...
void ParquetMetadataInputFormat::fillColumnStatistics(const std::shared_ptr<parquet::Statistics> &statistics, IColumn &column,
                                                      int32_t type_length) {
...
ParquetMetadataSchemaReader::ParquetMetadataSchemaReader(ReadBuffer &in_) : ISchemaReader(in_) {}

NamesAndTypesList ParquetMetadataSchemaReader::readSchema() { return getHeaderForParquetMetadata(); }
...
void registerInputFormatParquetMetadata(FormatFactory &factory) {
  factory.registerRandomAccessInputFormat(
      ...
      [](ReadBuffer &buf, const Block &sample, const FormatSettings &settings, const ReadSettings &, bool /* is_remote_fs */,
         FormatParserSharedResourcesPtr, FormatFilterInfoPtr) -> InputFormatPtr {
...
void registerParquetMetadataSchemaReader(FormatFactory &factory) {
  factory.registerSchemaReader("ParquetMetadata",
                               [](ReadBuffer &buf, const FormatSettings &) { return std::make_shared<ParquetMetadataSchemaReader>(buf); });
}
