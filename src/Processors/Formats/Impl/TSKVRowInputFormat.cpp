...
TSKVRowInputFormat::TSKVRowInputFormat(ReadBuffer& in_, SharedHeader header_, Params params_, const FormatSettings& format_settings_)
    : IRowInputFormat(std::move(header_), in_, std::move(params_)),
      ...
      name_map(getPort().getHeader().columns()) {
...
static bool readName(ReadBuffer& buf, std::string_view& ref, String& tmp) {
  ...
  while (!buf.eof()) {
    ...
    if (buf.eof()) throw Exception(ErrorCodes::CANNOT_PARSE_ESCAPE_SEQUENCE, "Cannot parse escape sequence");
...
bool TSKVRowInputFormat::readRow(MutableColumns& columns, RowReadExtension& ext) {
  ...
  if (unlikely(*in->position() == '\n')) {
    ...
    while (true) {
      ...
      if (has_value) {
        ...
        if (!it) {
          ...
          if (format_settings.null_as_default && !isNullableOrLowCardinalityNullable(type))
            read_columns[index] =
                SerializationNullable::deserializeNullAsDefaultOrNestedTextEscaped(*columns[index], *in, format_settings, serialization);
          else
      ...
      throw Exception(ErrorCodes::CANNOT_PARSE_INPUT_ASSERTION_FAILED, "Found garbage after field in TSKV format: {}", name_ref);
    }
  }
  ...
  for (size_t i = 0; i < num_columns; ++i)
    if (!seen_columns[i]) {
      ...
      if (format_settings.force_null_for_omitted_fields && !isNullableOrLowCardinalityNullable(type))
        throw Exception(ErrorCodes::TYPE_MISMATCH, "Cannot insert NULL value into a column `{}` of type '{}'", header.getByPosition(i).name,
                        type->getName());
...
void TSKVRowInputFormat::syncAfterError() { skipToUnescapedNextLineOrEOF(*in); }
...
NamesAndTypesList TSKVSchemaReader::readRowAndGetNamesAndDataTypes(bool& eof) {
  ...
  do {
    ...
    if (has_value) {
      ...
      names_and_types.emplace_back(std::move(name),
...
void registerInputFormatTSKV(FormatFactory& factory) {
  factory.registerInputFormat(
      "TSKV", [](ReadBuffer& buf, const Block& sample, IRowInputFormat::Params params, const FormatSettings& settings) {
...
void registerTSKVSchemaReader(FormatFactory& factory) {
  factory.registerSchemaReader(
      "TSKV", [](ReadBuffer& buf, const FormatSettings& settings) { return std::make_shared<TSKVSchemaReader>(buf, settings); });
