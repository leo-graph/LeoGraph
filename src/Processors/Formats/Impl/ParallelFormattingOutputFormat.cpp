...
void ParallelFormattingOutputFormat::finalizeImpl() {
  ...
  if (collected_prefix && collected_suffix && collected_finalize) return;
  ...
  if (!collected_prefix && (need_write_prefix || started_prefix)) formatter->writePrefix();

  if (!collected_suffix && (need_write_suffix || started_suffix)) formatter->writeSuffix();

  if (!collected_finalize) formatter->finalizeImpl();
...
void ParallelFormattingOutputFormat::addChunk(Chunk chunk, ProcessingUnitType type, bool can_throw_exception) {
  {
    std::lock_guard lock(mutex);
    if (background_exception && can_throw_exception) rethrowBackgroundException();
  }
...

  {
    ...
    writer_condvar.wait(lock, [&] { return unit.status == READY_TO_INSERT || emergency_stop; });
  }
...
void ParallelFormattingOutputFormat::collectorThreadFunction(const ThreadGroupPtr& thread_group) {
  ...
  try {
    while (!emergency_stop) {
      ...
      {
        std::unique_lock<std::mutex> lock(mutex);
        collector_condvar.wait(lock, [&] { return unit.status == READY_TO_READ || emergency_stop; });
      }
      ...
      if (auto_flush || need_flush.exchange(false) || unit.type == ProcessingUnitType::FINALIZE) out.next();
...
void ParallelFormattingOutputFormat::formatterThreadFunction(size_t current_unit_number, size_t first_row_num,
  ...
  try {
    ...
    switch (unit.type) {
      ...
      case ProcessingUnitType::EXTREMES: {
        if (are_totals_written) formatter->setTotalsAreWritten();
