...
class CustomSeparatedRowInputFormat final : public RowInputFormatWithNamesAndTypes<CustomSeparatedFormatReader> {
 public:
  CustomSeparatedRowInputFormat(SharedHeader header_, ReadBuffer& in_, const Params& params_, bool with_names_, bool with_types_,
                                bool ignore_spaces_, const FormatSettings& format_settings_);
 ...
 private:
  CustomSeparatedRowInputFormat(SharedHeader header_, std::unique_ptr<PeekableReadBuffer> in_buf_, const Params& params_, bool with_names_,
                                bool with_types_, bool ignore_spaces_, const FormatSettings& format_settings_);
...
class CustomSeparatedFormatReader final : public FormatWithNamesAndTypesReader {
 public:
  ...
  void skipSpaces() {
    if (ignore_spaces) skipWhitespaceIfAny(*buf, true);
  }
...
class CustomSeparatedSchemaReader : public FormatWithNamesAndTypesSchemaReader {
 public:
  CustomSeparatedSchemaReader(ReadBuffer& in_, bool with_names_, bool with_types_, bool ignore_spaces_,
