...
parquet::Compression::type getParquetCompression(FormatSettings::ParquetCompression method) {
  if (method == FormatSettings::ParquetCompression::NONE) return parquet::Compression::type::UNCOMPRESSED;

#  if USE_SNAPPY
  if (method == FormatSettings::ParquetCompression::SNAPPY) return parquet::Compression::type::SNAPPY;
#  endif

#  if USE_BROTLI
  if (method == FormatSettings::ParquetCompression::BROTLI) return parquet::Compression::type::BROTLI;
#  endif

  if (method == FormatSettings::ParquetCompression::ZSTD) return parquet::Compression::type::ZSTD;

  if (method == FormatSettings::ParquetCompression::LZ4) return parquet::Compression::type::LZ4;

  if (method == FormatSettings::ParquetCompression::GZIP) return parquet::Compression::type::GZIP;
  ...
  if (format_settings.parquet.use_custom_encoder) {
    if (format_settings.parquet.output_version < FormatSettings::ParquetVersion::V2_6)
      throw Exception(ErrorCodes::NOT_IMPLEMENTED,
                      "Custom parquet encoder doesn't support parquet versions < 2.6. Use output_format_parquet_use_custom_encoder = 0.");

    if (format_settings.parquet.parallel_encoding && format_settings.max_threads > 1)
      pool = std::make_unique<ThreadPool>(CurrentMetrics::ParquetEncoderThreads, CurrentMetrics::ParquetEncoderThreadsActive,
                                          CurrentMetrics::ParquetEncoderThreadsScheduled, format_settings.max_threads);
    ...
    switch (format_settings.parquet.output_compression_method) {
      case C::NONE:
        ...
        break;
      case C::SNAPPY:
        options.compression = CompressionMethod::Snappy;
        break;
      case C::ZSTD:
      ...
      case C::LZ4:
        ...
        break;
      case C::GZIP:
        options.compression = CompressionMethod::Gzip;
        break;
      case C::BROTLI:
        options.compression = CompressionMethod::Brotli;
        break;
    }
...
void ParquetBlockOutputFormat::consume(Chunk chunk) {
  ...
  if (pool) {
    ...
    while (true) {
      ...
      if (row_groups.size() < 2) break;
      if (bytes_in_flight <= format_settings.parquet.row_group_bytes * 4 && task_queue.size() <= format_settings.max_threads * 4) break;
  ...
  if (staging_rows < target_rows && staging_bytes < format_settings.parquet.row_group_bytes) return;
  ...
  if (staging_rows >= target_rows * 2) {
    ...
    for (size_t i = 1; i < staging_chunks.size(); ++i) concatenated.append(staging_chunks[i]);
    ...
    for (size_t offset = 0; offset < staging_rows; offset += row_group_size) {
      ...
      for (size_t i = 0; i < columns.size(); ++i) columns[i]->insertRangeFrom(*concatenated.getColumns()[i], offset, count);
...
void ParquetBlockOutputFormat::finalizeImpl() {
  if (!staging_chunks.empty()) writeRowGroup(std::move(staging_chunks));
  ...
  if (format_settings.parquet.use_custom_encoder) {
    ...
    if (file_writer) {
      ...
      if (!status.ok()) throw Exception(ErrorCodes::UNKNOWN_EXCEPTION, "Error while closing a table: {}", status.ToString());
    }
  }
}
...
void ParquetBlockOutputFormat::onCancel() noexcept { is_stopped = true; }
...
void ParquetBlockOutputFormat::writeUsingArrow(std::vector<Chunk> chunks) {
  ...
  if (format_filter_info && format_filter_info->column_mapper)
    ch_column_to_arrow_column->chChunkToArrowTable(arrow_table, chunks, columns_num,
                                                   format_filter_info->column_mapper->getStorageColumnEncoding());
  else
    ch_column_to_arrow_column->chChunkToArrowTable(arrow_table, chunks, columns_num);
  ...
  if (!file_writer) {
    ...
    if (!format_settings.parquet.write_page_index) builder.disable_write_page_index();
    ...
    auto result = parquet::arrow::FileWriter::Open(*arrow_table->schema(), arrow::default_memory_pool(), sink, builder.build(),
                                                   writer_props_builder.build());
    if (!result.ok()) throw Exception(ErrorCodes::UNKNOWN_EXCEPTION, "Error while opening a table: {}", result.status().ToString());
    file_writer = std::move(result.ValueOrDie());
  }

  auto status = file_writer->WriteTable(*arrow_table, INT64_MAX);

  if (!status.ok()) throw Exception(ErrorCodes::UNKNOWN_EXCEPTION, "Error while writing a table: {}", status.ToString());
}
...
void ParquetBlockOutputFormat::writeRowGroupInOneThread(Chunk chunk) {
  ...
  for (size_t i = 0; i < header.columns(); ++i)
    prepareColumnForWrite(chunk.getColumns()[i], header.getByPosition(i).type, header.getByPosition(i).name, options, &columns_to_write);
...
void ParquetBlockOutputFormat::threadFunction() {
  ...
  while (true) {
    ...
    if (task.column_type) {
      ...
      prepareColumnForWrite(std::move(concatenated), task.column_type, task.column_name, options, &subcolumns);
...
void registerOutputFormatParquet(FormatFactory &factory) {
  factory.registerOutputFormat(
      "Parquet", [](WriteBuffer &buf, const Block &sample, const FormatSettings &format_settings, FormatFilterInfoPtr format_filter_info) {
