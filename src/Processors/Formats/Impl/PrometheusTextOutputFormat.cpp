...
void getColumnPos(const Block &header, const String &col_name, bool (*pred)(const DataTypePtr &), ResType &res) {
  ...
  if (header.has(col_name, true)) {
    ...
    if (!pred(is_optional ? removeNullable(col.type) : col.type)) {
      throw Exception(ErrorCodes::BAD_ARGUMENTS, "Illegal type '{}' of column '{}' for output format '{}'", col.type->getName(), col_name,
                      FORMAT_NAME);
    }
  } else {
...
PrometheusTextOutputFormat::PrometheusTextOutputFormat(WriteBuffer &out_, SharedHeader header_, const FormatSettings &format_settings_)
    : IRowOutputFormat(header_, out_), string_serialization(DataTypeString().getDefaultSerialization()), format_settings(format_settings_) {
 ...
 * > The buckets of a histogram and the quantiles of a summary must appear in increasing numerical order of their label values (for the le
 * or the quantile label, respectively).
 */
void PrometheusTextOutputFormat::fixupBucketLabels(CurrentMetric &metric) {
  ...
  ::sort(metric.values.begin(), metric.values.end(), [&bucket_label](const auto &lhs, const auto &rhs) {
    ...
    if (lhs_labels_contain_sum && rhs_labels_contain_count) return true;
    if (lhs_labels_contain_count && rhs_labels_contain_sum) return false;
    if (rhs_labels_contain_sum_or_count && !lhs_labels_contain_sum_or_count) return true;
    if (lhs_labels_contain_sum_or_count && !rhs_labels_contain_sum_or_count) return false;
  ...
  if (metric.type == "histogram") {
    ...
    if (inf_bucket.has_value() && !count_bucket.has_value()) metric.values.emplace_back(*inf_bucket);

    if (!inf_bucket.has_value() && count_bucket.has_value()) metric.values.emplace_back(*count_bucket);
  }
}
...
static void columnMapToContainer(const ColumnMap *col_map, size_t row_num, Container &result) {
  ...
  for (const auto &map_element : map_field) {
    ...
    String entry_key;
    String entry_value;
    if (map_entry.size() == 2 && map_entry[0].tryGet<String>(entry_key) && map_entry[1].tryGet<String>(entry_value)) {
...
void PrometheusTextOutputFormat::finalizeImpl() { flushCurrentMetric(); }

void registerOutputFormatPrometheus(FormatFactory &factory) {
  factory.registerOutputFormat(
      FORMAT_NAME, [](WriteBuffer &buf, const Block &sample, const FormatSettings &settings, FormatFilterInfoPtr /*format_filter_info*/) {
