...
enum { UNKNOWN_FIELD = size_t(-1), NESTED_FIELD = size_t(-2) };

}

JSONEachRowRowInputFormat::JSONEachRowRowInputFormat(ReadBuffer& in_, SharedHeader header_, Params params_,
                                                     const FormatSettings& format_settings_, bool yield_strings_)
...
const String& JSONEachRowRowInputFormat::columnName(size_t i) const { return getPort().getHeader().getByPosition(i).name; }
...
inline bool JSONEachRowRowInputFormat::advanceToNextKey(size_t key_index) {
  ...
  if (in->eof()) throw Exception(ErrorCodes::CANNOT_READ_ALL_DATA, "Unexpected end of stream while parsing JSONEachRow format");
  if (*in->position() == '}') {
    ++in->position();
    return false;
  }

  if (key_index > 0) JSONUtils::skipComma(*in);
  return true;
}
...
bool JSONEachRowRowInputFormat::readRow(MutableColumns& columns, RowReadExtension& ext) {
  if (!allow_new_rows) return false;
  skipWhitespaceIfAny(*in);

  bool is_first_row = getRowNum() == 0;
  if (checkEndOfData(is_first_row)) return false;
  ...
  for (size_t i = 0; i < num_columns; ++i)
    if (!seen_columns[i]) {
      ...
      if (format_settings.force_null_for_omitted_fields && !isNullableOrLowCardinalityNullable(type))
        throw Exception(ErrorCodes::TYPE_MISMATCH, "Cannot insert NULL value into a column `{}` of type '{}'", columnName(i),
                        type->getName());
...
void JSONEachRowRowInputFormat::syncAfterError() { skipToUnescapedNextLineOrEOF(*in); }
...
void JSONEachRowRowInputFormat::readSuffix() {
  skipWhitespaceIfAny(*in);
  if (data_in_square_brackets) JSONUtils::skipArrayEnd(*in);
...
size_t JSONEachRowRowInputFormat::countRows(size_t max_block_size) {
  if (unlikely(!allow_new_rows)) return 0;
...
NamesAndTypesList JSONEachRowSchemaReader::readRowAndGetNamesAndDataTypes(bool& eof) {
  if (first_row) {
    ...
    if (data_in_square_brackets && checkChar(']', in)) return {};

    /// ';' means end of data.
    if (checkChar(';', in)) return {};
...
void registerInputFormatJSONEachRow(FormatFactory& factory) {
  auto register_format = [&](const String& format_name, bool json_strings) {
    factory.registerInputFormat(
        format_name, [json_strings](ReadBuffer& buf, const Block& sample, IRowInputFormat::Params params, const FormatSettings& settings) {
          return std::make_shared<JSONEachRowRowInputFormat>(buf, std::make_shared<const Block>(sample), std::move(params), settings,
                                                             json_strings);
        });
  };
