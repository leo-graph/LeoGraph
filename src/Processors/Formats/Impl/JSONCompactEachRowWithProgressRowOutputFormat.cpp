...
void JSONCompactEachRowWithProgressRowOutputFormat::writeRowStartDelimiter() { writeCString("{\"row\":[", *ostr); }

void JSONCompactEachRowWithProgressRowOutputFormat::writeRowEndDelimiter() { writeCString("]}\n", *ostr); }
...
void registerOutputFormatJSONCompactEachRowWithProgress(FormatFactory& factory) {
  factory.registerOutputFormat(
      ...
      [](WriteBuffer& buf, const Block& sample, const FormatSettings& _format_settings, FormatFilterInfoPtr /*format_filter_info*/) {
        ...
        return std::make_shared<JSONCompactEachRowWithProgressRowOutputFormat>(buf, std::make_shared<const Block>(sample), settings, false,
                                                                               false);
      });
