...
CustomSeparatedRowInputFormat::CustomSeparatedRowInputFormat(SharedHeader header_, ReadBuffer &in_buf_, const Params &params_,
                                                             bool with_names_, bool with_types_, bool ignore_spaces_,
                                                             const FormatSettings &format_settings_)
    : CustomSeparatedRowInputFormat(header_, std::make_unique<PeekableReadBuffer>(in_buf_), params_, with_names_, with_types_,
                                    ignore_spaces_, format_settings_) {}

CustomSeparatedRowInputFormat::CustomSeparatedRowInputFormat(SharedHeader header_, std::unique_ptr<PeekableReadBuffer> buf_,
                                                             const Params &params_, bool with_names_, bool with_types_, bool ignore_spaces_,
                                                             const FormatSettings &format_settings_)
    : RowInputFormatWithNamesAndTypes(header_, *buf_, params_, false, with_names_, with_types_, format_settings_,
                                      std::make_unique<CustomSeparatedFormatReader>(*buf_, ignore_spaces_, format_settings_),
                                      format_settings_.custom.try_detect_header, format_settings_.custom.allow_variable_number_of_columns),
      buf(std::move(buf_)),
...
void CustomSeparatedRowInputFormat::readPrefix() {
  ...
  for (const auto &column_index : column_mapping->column_indexes_for_input_fields) {
    if (column_index)
      checkSupportedDelimiterAfterField(format_settings.custom.escaping_rule, format_settings.custom.field_delimiter,
...
CustomSeparatedFormatReader::CustomSeparatedFormatReader(PeekableReadBuffer &buf_, bool ignore_spaces_,
                                                         const FormatSettings &format_settings_)
...
void CustomSeparatedFormatReader::skipField() {
  ...
  if (format_settings.custom.escaping_rule == FormatSettings::EscapingRule::CSV)
    readCSVFieldWithTwoPossibleDelimiters(*buf, format_settings.csv, format_settings.custom.field_delimiter,
...
bool CustomSeparatedFormatReader::checkForEndOfRow() {
  ...
  skipSpaces();
  if (!checkString(format_settings.custom.row_after_delimiter, *buf)) return false;

  skipSpaces();

  /// At the end of row after row_after_delimiter we expect result_after_delimiter or row_between_delimiter.

  if (checkString(format_settings.custom.row_between_delimiter, *buf)) return true;
...
String CustomSeparatedFormatReader::readFieldIntoString(bool is_first, bool is_last, bool is_unknown) {
  if (!is_first) skipFieldDelimiter();
  skipSpaces();
  ...
  if constexpr (mode != ReadFieldMode::AS_FIELD) {
    ...
    if (is_unknown && format_settings.custom.escaping_rule == FormatSettings::EscapingRule::CSV)
      return readCSVStringWithTwoPossibleDelimiters(*buf, format_settings.csv, format_settings.custom.field_delimiter,
                                                    format_settings.custom.row_after_delimiter);
    ...
    if (is_unknown && format_settings.custom.escaping_rule == FormatSettings::EscapingRule::CSV)
      return readCSVFieldWithTwoPossibleDelimiters(*buf, format_settings.csv, format_settings.custom.field_delimiter,
                                                   format_settings.custom.row_after_delimiter);
...
std::vector<String> CustomSeparatedFormatReader::readRowImpl() {
  ...
  if (columns == 0 || format_settings.custom.allow_variable_number_of_columns) {
    ...
    for (size_t i = 0; i != columns; ++i) values.push_back(readFieldIntoString<mode>(i == 0, i + 1 == columns, false));
  }
...
void CustomSeparatedFormatReader::updateFormatSettings(bool is_last_column) {
  if (format_settings.custom.escaping_rule != FormatSettings::EscapingRule::CSV) return;
...
bool CustomSeparatedFormatReader::readField(IColumn &column, const DataTypePtr &type, const SerializationPtr &serialization,
                                            bool is_last_file_column, const String &) {
  skipSpaces();
...
bool CustomSeparatedFormatReader::checkForSuffix() {
  PeekableReadBufferCheckpoint checkpoint{*buf};
  if (checkForSuffixImpl(true)) return true;
  buf->rollbackToCheckpoint();
  return false;
}

bool CustomSeparatedFormatReader::parseRowStartWithDiagnosticInfo(WriteBuffer &out) {
  return parseDelimiterWithDiagnosticInfo(out, *buf, format_settings.custom.row_before_delimiter, "delimiter before first field",
                                          ignore_spaces);
}
...
bool CustomSeparatedFormatReader::parseRowEndWithDiagnosticInfo(WriteBuffer &out) {
  return parseDelimiterWithDiagnosticInfo(out, *buf, format_settings.custom.row_after_delimiter, "delimiter after last field",
                                          ignore_spaces);
}
...
CustomSeparatedSchemaReader::CustomSeparatedSchemaReader(ReadBuffer &in_, bool with_names_, bool with_types_, bool ignore_spaces_,
                                                         const FormatSettings &format_setting_)
    : FormatWithNamesAndTypesSchemaReader(buf, format_setting_, with_names_, with_types_, &reader,
...
std::optional<std::pair<std::vector<String>, DataTypes>> CustomSeparatedSchemaReader::readRowAndGetFieldsAndDataTypes() {
  ...
  if (!first_row || with_names || with_types) reader.skipRowBetweenDelimiter();
  ...
  for (bool ignore_spaces : {false, true}) {
    auto register_func = [&](const String &format_name, bool with_names, bool with_types) {
      factory.registerInputFormat(
          format_name, [=](ReadBuffer &buf, const Block &sample, IRowInputFormat::Params params, const FormatSettings &settings) {
            return std::make_shared<CustomSeparatedRowInputFormat>(std::make_shared<const Block>(sample), buf, params, with_names,
                                                                   with_types, ignore_spaces, settings);
          });
    };
    ...
    auto register_func = [&](const String &format_name, bool with_names, bool with_types) {
      ...
      if (!with_types) {
        factory.registerAdditionalInfoForSchemaCacheGetter(format_name, [with_names](const FormatSettings &settings) {
          ...
          if (!with_names)
            result += fmt::format(", column_names_for_schema_inference={}, try_detect_header={}",
                                  settings.column_names_for_schema_inference, settings.custom.try_detect_header);
          return result + fmt::format(
                              ...
                              settings.custom.result_before_delimiter, settings.custom.row_before_delimiter,
                              settings.custom.field_delimiter, settings.custom.row_after_delimiter, settings.custom.row_between_delimiter,
