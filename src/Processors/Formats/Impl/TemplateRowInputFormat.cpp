...
[[noreturn]] static void throwUnexpectedEof(size_t row_num) {
  throw Exception(ErrorCodes::CANNOT_READ_ALL_DATA,
                  "Unexpected EOF while parsing row {}. "
                  "Maybe last row has wrong format or input doesn't contain specified suffix before EOF.",
                  std::to_string(row_num));
}

static void updateFormatSettingsIfNeeded(FormatSettings::EscapingRule escaping_rule, FormatSettings &settings,
                                         const ParsedTemplateFormatString &row_format, char default_csv_delimiter, size_t file_column) {
  if (escaping_rule != FormatSettings::EscapingRule::CSV) return;
  ...
  if (row_format.delimiters[file_column + 1].empty()) settings.csv.delimiter = default_csv_delimiter;
...
TemplateRowInputFormat::TemplateRowInputFormat(SharedHeader header_, ReadBuffer &in_, const Params &params_, FormatSettings settings_,
                                               bool ignore_spaces_, ParsedTemplateFormatString format_,
                                               ParsedTemplateFormatString row_format_, std::string row_between_delimiter_)
    : TemplateRowInputFormat(header_, std::make_unique<PeekableReadBuffer>(in_), params_, settings_, ignore_spaces_, format_, row_format_,
                             row_between_delimiter_) {}
...
TemplateRowInputFormat::TemplateRowInputFormat(SharedHeader header_, std::unique_ptr<PeekableReadBuffer> buf_, const Params &params_,
                                               FormatSettings settings_, bool ignore_spaces_, ParsedTemplateFormatString format_,
                                               ParsedTemplateFormatString row_format_, std::string row_between_delimiter_)
    : RowInputFormatWithDiagnosticInfo(header_, *buf_, params_),
      buf(std::move(buf_)),
      data_types(header_->getDataTypes()),
      settings(std::move(settings_)),
      ignore_spaces(ignore_spaces_),
      format(std::move(format_)),
      row_format(std::move(row_format_)),
  ...
  for (size_t i = 0; i < row_format.columnsCount(); ++i) {
    ...
    if (column_index) {
      if (header_->columns() <= *column_index)
        row_format.throwInvalidFormat("Column index " + std::to_string(*column_index) + " must be less then number of columns (" +
                                          std::to_string(header_->columns()) + ")",
                                      i);
      ...
      if (column_in_format[col_idx]) row_format.throwInvalidFormat("Duplicate column", i);
  ...
  for (size_t i = 0; i < header_->columns(); ++i)
    if (!column_in_format[i]) always_default_columns.push_back(i);
}

void TemplateRowInputFormat::readPrefix() { format_reader->readPrefix(); }
...
bool TemplateRowInputFormat::readRow(MutableColumns &columns, RowReadExtension &extra) {
  ...
  if (unlikely(end_of_stream)) return false;
  ...
  if (likely(getRowNum() != 0)) format_reader->skipRowBetweenDelimiter();
  ...
  for (const auto &idx : always_default_columns) data_types[idx]->insertDefaultInto(*columns[idx]);

  return true;
}

bool TemplateRowInputFormat::deserializeField(const DataTypePtr &type, const SerializationPtr &serialization, IColumn &column,
                                              size_t file_column) {
  ...
  try {
    ...
    if (e.code() == ErrorCodes::ATTEMPT_TO_READ_AFTER_EOF) throwUnexpectedEof(getRowNum());
    throw;
  }
}
...
bool TemplateRowInputFormat::parseRowAndPrintDiagnosticInfo(MutableColumns &columns, WriteBuffer &out) {
  ...
  if (likely(getRowNum() != 0) &&
      !parseDelimiterWithDiagnosticInfo(out, *buf, row_between_delimiter, "delimiter between rows", ignore_spaces))
    return false;

  for (size_t i = 0; i < row_format.columnsCount(); ++i) {
    if (!parseDelimiterWithDiagnosticInfo(out, *buf, row_format.delimiters[i], "delimiter before field " + std::to_string(i),
                                          ignore_spaces))
      return false;
    ...
    if (row_format.format_idx_to_column_idx[i]) {
      ...
      if (!deserializeFieldAndPrintDiagnosticInfo(header.getByPosition(col_idx).name, data_types[col_idx], *columns[col_idx], out, i)) {
        out << "Maybe it's not possible to deserialize field " + std::to_string(i) + " as " +
                   escapingRuleToString(row_format.escaping_rules[i]);
        return false;
      }
    } else {
      ...
      if (!deserializeFieldAndPrintDiagnosticInfo(skipped_column_str, skipped_column_type, *skipped_column, out, i)) return false;
    }
  }
...
bool parseDelimiterWithDiagnosticInfo(WriteBuffer &out, ReadBuffer &buf, const String &delimiter, const String &description,
                                      bool skip_spaces) {
...
bool TemplateRowInputFormat::allowSyncAfterError() const { return !row_format.delimiters.back().empty() || !row_between_delimiter.empty(); }
...
TemplateFormatReader::TemplateFormatReader(PeekableReadBuffer &buf_, bool ignore_spaces_, const ParsedTemplateFormatString &format_,
                                           const ParsedTemplateFormatString &row_format_, std::string row_between_delimiter_,
  ...
  for (size_t i = 0; i < format.columnsCount(); ++i) {
    if (format.format_idx_to_column_idx[i]) {
      if (*format.format_idx_to_column_idx[i] != 0) format.throwInvalidFormat("Invalid input part", i);
      if (has_data) format.throwInvalidFormat("${data} can occur only once", i);
      ...
      if (format.escaping_rules[i] == EscapingRule::XML) format.throwInvalidFormat("XML deserialization is not supported", i);
    }
  }

  /// Validate format string for rows
  for (size_t i = 0; i < row_format.columnsCount(); ++i) {
    if (row_format.escaping_rules[i] == EscapingRule::XML) row_format.throwInvalidFormat("XML deserialization is not supported", i);
  }
}
...
void TemplateFormatReader::skipField(EscapingRule escaping_rule) {
  try {
    ...
    if (e.code() == ErrorCodes::ATTEMPT_TO_READ_AFTER_EOF) throwUnexpectedEof(row_num);
    throw;
  }
}
...
ReturnType TemplateFormatReader::tryReadPrefixOrSuffix(size_t &input_part_beg, size_t input_part_end) {
  ...
  if constexpr (throw_exception)
    ...
    if (likely(!checkString(format.delimiters[input_part_beg], *buf))) return ReturnType(false);
  }
  ...
  while (input_part_beg < input_part_end) {
    ...
    if constexpr (throw_exception)
      ...
      try {
        ...
        if (e.code() != ErrorCodes::ATTEMPT_TO_READ_AFTER_EOF && e.code() != ErrorCodes::CANNOT_PARSE_ESCAPE_SEQUENCE &&
  ...
  if constexpr (!throw_exception) return ReturnType(true);
}
...
TemplateSchemaReader::TemplateSchemaReader(ReadBuffer &in_, bool ignore_spaces_, const ParsedTemplateFormatString &format_,
                                           const ParsedTemplateFormatString &row_format_, std::string row_between_delimiter,
                                           const FormatSettings &format_settings_)
...
std::optional<DataTypes> TemplateSchemaReader::readRowAndGetDataTypes() {
  if (first_row) format_reader.readPrefix();

  if (format_reader.checkForSuffix()) return {};
...
void TemplateSchemaReader::transformTypesIfNeeded(DataTypePtr &type, DataTypePtr &new_type) {
  transformInferredTypesByEscapingRuleIfNeeded(type, new_type, format_settings, row_format.escaping_rules[field_index],
                                               &json_inference_info);
}
...
static ParsedTemplateFormatString fillResultSetFormat(const FormatSettings &settings) {
  ...
  if (settings.template_settings.resultset_format.empty()) {
    ...
    resultset_format = ParsedTemplateFormatString(FormatSchemaInfo(
...
static ParsedTemplateFormatString fillRowFormat(const FormatSettings &settings, ParsedTemplateFormatString::ColumnIdxGetter idx_getter,
                                                bool allow_indexes) {
  return ParsedTemplateFormatString(FormatSchemaInfo(
                                        /*format_schema_source=*/FormatSettings::FORMAT_SCHEMA_SOURCE_FILE,
...
void registerInputFormatTemplate(FormatFactory &factory) {
  for (bool ignore_spaces : {false, true}) {
    factory.registerInputFormat(
        ignore_spaces ? "TemplateIgnoreSpaces" : "Template",
        [=](ReadBuffer &buf, const Block &sample, IRowInputFormat::Params params, const FormatSettings &settings) {
          auto idx_getter = [&](const String &colName) -> std::optional<size_t> { return sample.getPositionByName(colName); };

          return std::make_shared<TemplateRowInputFormat>(std::make_shared<const Block>(sample), buf, params, settings, ignore_spaces,
                                                          fillResultSetFormat(settings), fillRowFormat(settings, idx_getter, true),
...
void registerTemplateSchemaReader(FormatFactory &factory) {
  for (bool ignore_spaces : {false, true}) {
    ...
    factory.registerSchemaReader(format_name, [ignore_spaces](ReadBuffer &buf, const FormatSettings &settings) {
      ...
      return std::make_shared<TemplateSchemaReader>(buf, ignore_spaces, fillResultSetFormat(settings), row_format,
                                                    settings.template_settings.row_between_delimiter, settings);
    });
    factory.registerAdditionalInfoForSchemaCacheGetter(format_name, [](const FormatSettings &settings) {
      size_t index = 0;
      auto idx_getter = [&](const String &) -> std::optional<size_t> { return index++; };
      ParsedTemplateFormatString row_format;
      if (!settings.template_settings.row_format.empty()) row_format = fillRowFormat(settings, idx_getter, false);
      std::unordered_set<FormatSettings::EscapingRule> visited_escaping_rules;
      String result = fmt::format("row_format={}, resultset_format={}, row_between_delimiter={}", settings.template_settings.row_format,
                                  settings.template_settings.resultset_format, settings.template_settings.row_between_delimiter);
