...
ProtobufRowOutputFormat::ProtobufRowOutputFormat(WriteBuffer& out_, SharedHeader header_, const ProtobufSchemaInfo& schema_info_,
                                                 const FormatSettings& settings_, bool with_length_delimiter_)
    : IRowOutputFormat(header_, out_),
      ...
      serializer(ProtobufSerializer::create(header_->getNames(), header_->getDataTypes(), descriptor_holder, with_length_delimiter_,
                                            /* with_envelope = */ false, settings_.protobuf.output_nullables_with_google_wrappers,
                                            *writer)),
...
void registerOutputFormatProtobuf(FormatFactory& factory) {
  for (bool with_length_delimiter : {false, true}) {
    ...
    factory.registerOutputFormat(name, [with_length_delimiter](WriteBuffer& buf, const Block& header, const FormatSettings& settings,
                                                               FormatFilterInfoPtr /*format_filter_info*/) {
      return std::make_shared<ProtobufRowOutputFormat>(
          buf, std::make_shared<const Block>(header),
          ProtobufSchemaInfo(settings, "Protobuf", header, settings.protobuf.use_autogenerated_schema), settings, with_length_delimiter);
    });
