...
static void extractLiteralTokensImpl(const ASTPtr &ast, const LiteralTokenMap &token_map,
                                     std::vector<std::optional<LiteralTokenInfo>> &result, ContextPtr context) {
  ...
  if (auto *func = ast->as<ASTFunction>()) {
    if (func->name == "lambda") return;

    if (func->name.starts_with("toInterval")) return;
...
static std::vector<std::optional<LiteralTokenInfo>> extractLiteralTokens(const ASTPtr &ast, const LiteralTokenMap &token_map,
                                                                         ContextPtr context) {
...
struct SpecialParserType {
  ...
  bool useDefaultParser() const {
    return main_type == Field::Types::String ||
           (!nested_types.empty() &&
            std::all_of(nested_types.begin(), nested_types.end(), [](const auto &type) { return type.first == Field::Types::String; }));
  }
};
...
static void fillLiteralInfo(DataTypes &nested_types, LiteralInfo &info) {
  ...
  for (auto &nested_type : nested_types) {
    ...
    if (is_nullable) nested_type = std::make_shared<DataTypeNullable>(nested_type);
...
class ReplaceLiteralsVisitor {
 public:
  ...
  void visit(ASTPtr &ast, bool force_nullable) {
    if (visitIfLiteral(ast, force_nullable)) return;
 ...
 private:
  ...
  void visit(ASTFunction &function, bool force_nullable) {
    if (function.name == "lambda") return;
    ...
    if (function.name.starts_with("toInterval")) return;
    ...
    for (auto &idx : can_always_be_nullable)
      if (idx < force_nullable_arguments.size()) force_nullable_arguments[idx] = true;
  ...
  bool visitIfLiteral(ASTPtr &ast, bool force_nullable) {
    ...
    Field::Types::Which field_type = literal->value.getType();
    if (field_type == Field::Types::Null || field_type == Field::Types::Bool) return true;
    ...
    if (token_info.has_value()) {
      ...
      if (literal->value.getType() == Field::Types::Array) {
        ...
        if (not_null == array.end()) return true;
...
ConstantExpressionTemplate::TemplateStructure::TemplateStructure(LiteralsInfo &replaced_literals, TokenIterator expression_begin,
                                                                 TokenIterator expression_end, ASTPtr &expression,
                                                                 const IDataType &result_type, bool null_as_default_, ContextPtr context) {
  ...
  for (size_t i = 0; i < replaced_literals.size(); ++i) {
    ...
    while (prev_end < expression_end && prev_end->begin < info.token_info.end) ++prev_end;
...
String ConstantExpressionTemplate::TemplateStructure::dumpTemplate() const {
  ...
  while (cur_column < num_columns) {
    ...
    while (cur_token < skip_tokens_until) res << quote << tokens[cur_token++] << ", ";
  ...
  while (cur_token < tokens.size()) res << quote << tokens[cur_token++] << ", ";

  res << "eof";
  return res.str();
}

UInt64 ConstantExpressionTemplate::TemplateStructure::getTemplateHash(const ASTPtr &expression, const LiteralsInfo &replaced_literals,
                                                                      const DataTypePtr &result_column_type, bool null_as_default,
                                                                      const String &salt) {
  ...
  for (const auto &info : replaced_literals) hash_state.update(info.type->getName());
...
ConstantExpressionTemplate::TemplateStructurePtr ConstantExpressionTemplate::Cache::getFromCacheOrConstruct(
    const DataTypePtr &result_column_type, bool null_as_default, TokenIterator expression_begin, TokenIterator expression_end,
    const ASTPtr &expression_, const LiteralTokenMap &token_map, ContextPtr context, bool *found_in_cache, const String &salt) {
  ...
  if (iter == cache.end()) {
    if (max_size <= cache.size()) cache.clear();
    res = std::make_shared<TemplateStructure>(visitor.replaced_literals, expression_begin, expression_end, expression, *result_column_type,
                                              null_as_default, context);
    cache.insert({template_hash, res});
    if (found_in_cache) *found_in_cache = false;
  } else {
    /// FIXME process collisions correctly
    res = iter->second;
    if (found_in_cache) *found_in_cache = true;
  }

  return res;
}

bool ConstantExpressionTemplate::parseExpression(ReadBuffer &istr, const TokenIterator &token_iterator,
  ...
  try {
    ...
    for (size_t i = 0; i < cur_column; ++i) columns[i]->popBack(1);

    if (!isParseError(e.code())) throw;
...
bool ConstantExpressionTemplate::tryParseExpression(ReadBuffer &istr, const TokenIterator &token_iterator,
                                                    const FormatSettings &format_settings, size_t &cur_column, const Settings &settings) {
  size_t cur_token = 0;
  ...
  while (cur_column < num_columns) {
    ...
    while (cur_token < skip_tokens_until) {
      ...
      if (!checkString(structure->tokens[cur_token++], istr)) return false;
    }
    skipWhitespaceIfAny(istr);
    ...
    if (format_settings.values.accurate_types_of_literals && !structure->special_parser[cur_column].useDefaultParser()) {
      if (!parseLiteralAndAssertType(istr, token_iterator, type.get(), cur_column, settings)) return false;
    } else
...
bool ConstantExpressionTemplate::parseLiteralAndAssertType(ReadBuffer &istr, const TokenIterator &token_iterator,
                                                           const IDataType *complex_type, size_t column_idx, const Settings &settings) {
  ...
  if (type_info.is_array || type_info.is_tuple || type_info.is_map) {
    ...
    IParser::Pos iterator(token_iterator, static_cast<unsigned>(settings[Setting::max_parser_depth]),
                          static_cast<unsigned>(settings[Setting::max_parser_backtracks]));
    while (iterator->begin < istr.position()) ++iterator;
    Expected expected;
    ASTPtr ast;
    if (!parser_array.parse(iterator, ast, expected) && !parser_tuple.parse(iterator, ast, expected)) return false;
    ...
    for (size_t i = 0; i < nested_types.size(); ++i) {
      ...
      if (!are_types_compatible) return false;
    }
  ...
  if (type_info.is_nullable && 4 <= istr.available() && 0 == strncasecmp(istr.position(), "NULL", 4)) {
    ...
    if (negative || *istr.position() == '+') ++istr.position();
    ...
    if (pos_double == buf || errno == ERANGE || float_value < 0) return false;
...
ColumnPtr ConstantExpressionTemplate::evaluateAll(BlockMissingValues &nulls, size_t column_idx, const DataTypePtr &expected_type,
                                                  size_t offset) {
  ...
  if (!structure->literals.columns())
    evaluated.insert(
        {ColumnConst::create(ColumnUInt8::create(1, static_cast<UInt8>(0)), rows_count), std::make_shared<DataTypeUInt8>(), "_dummy"});
...
void ConstantExpressionTemplate::TemplateStructure::addNodesToCastResult(const IDataType &result_column_type, ASTPtr &expr,
  ...
  /// or with "(if(isNull(_dummy_0 AS _expression), defaultValueOfTypeName('TypeName'), _CAST(_expression, 'TypeName')),
  /// isNull(_expression))" if null_as_default is true
  if (null_as_default) {
