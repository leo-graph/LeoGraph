...
Chunk ArrowBlockInputFormat::read() {
  ...
  if (stream) {
    if (!stream_reader) prepareReader();
    ...
    if (need_only_count && batch_result.ok()) return getChunkForCount((*batch_result)->num_rows());
  } else {
    if (!file_reader) prepareReader();
  ...
  if (!batch_result.ok())
    throw Exception(ErrorCodes::CANNOT_READ_ALL_DATA, "Error while reading batch of Arrow data: {}", batch_result.status().ToString());

  auto table_result = arrow::Table::FromRecordBatches({*batch_result});
  if (!table_result.ok())
    throw Exception(ErrorCodes::CANNOT_READ_ALL_DATA, "Error while reading batch of Arrow data: {}", table_result.status().ToString());
  ...
  res = arrow_column_to_ch_column->arrowTableToCHChunk(*table_result, (*table_result)->num_rows(),
                                                       file_reader ? file_reader->metadata() : nullptr, block_missing_values_ptr);
  ...
  if (batch_end > batch_start) approx_bytes_read_for_chunk = batch_end - batch_start;
  return res;
}
...
const BlockMissingValues* ArrowBlockInputFormat::getMissingValues() const { return &block_missing_values; }
...
static std::shared_ptr<arrow::RecordBatchReader> createStreamReader(ReadBuffer& in) {
  ...
  if (in.eof()) throw Exception(ErrorCodes::INCORRECT_DATA, "The Arrow stream is empty");
  ...
  if (!stream_reader_status.ok())
    throw Exception(ErrorCodes::UNKNOWN_EXCEPTION, "Error while opening a table: {}", stream_reader_status.status().ToString());
  return *stream_reader_status;
}

static std::shared_ptr<arrow::ipc::RecordBatchFileReader> createFileReader(ReadBuffer& in, const FormatSettings& format_settings,
                                                                           std::atomic<int>& is_stopped) {
  auto arrow_file = asArrowFile(in, format_settings, is_stopped, "Arrow", ARROW_MAGIC_BYTES);
  if (is_stopped) return nullptr;

  auto options = arrow::ipc::IpcReadOptions::Defaults();
  options.memory_pool = arrow::default_memory_pool();
  auto file_reader_status = arrow::ipc::RecordBatchFileReader::Open(arrow_file, options);
  if (!file_reader_status.ok())
    throw Exception(ErrorCodes::UNKNOWN_EXCEPTION, "Error while opening a table: {}", file_reader_status.status().ToString());
...
void ArrowBlockInputFormat::prepareReader() {
  ...
  arrow_column_to_ch_column = std::make_unique<ArrowColumnToCHColumn>(
      getPort().getHeader(), "Arrow", format_settings, std::nullopt, std::nullopt, format_settings.arrow.allow_missing_columns,
      format_settings.null_as_default, format_settings.date_time_overflow_behavior, format_settings.parquet.allow_geoparquet_parser,
      format_settings.arrow.case_insensitive_column_matching, stream);
...
void ArrowSchemaReader::initializeIfNeeded() {
  if (file_reader || stream_reader) return;
...
NamesAndTypesList ArrowSchemaReader::readSchema() {
  ...
  auto header = ArrowColumnToCHColumn::arrowSchemaToCHHeader(
      *schema, file_reader ? file_reader->metadata() : nullptr, stream ? "ArrowStream" : "Arrow", format_settings,
      format_settings.arrow.skip_columns_with_unsupported_types_in_schema_inference,
      format_settings.schema_inference_make_columns_nullable != 0, false, format_settings.parquet.allow_geoparquet_parser);
...
void registerInputFormatArrow(FormatFactory& factory) {
  factory.registerInputFormat(
      "Arrow", [](ReadBuffer& buf, const Block& sample, const RowInputFormatParams& /* params */, const FormatSettings& format_settings) {
  ...
  factory.registerInputFormat("ArrowStream", [](ReadBuffer& buf, const Block& sample, const RowInputFormatParams& /* params */,
...
void registerArrowSchemaReader(FormatFactory& factory) {
  factory.registerSchemaReader(
      "Arrow", [](ReadBuffer& buf, const FormatSettings& settings) { return std::make_shared<ArrowSchemaReader>(buf, false, settings); });
  ...
  factory.registerSchemaReader("ArrowStream", [](ReadBuffer& buf, const FormatSettings& settings) {
