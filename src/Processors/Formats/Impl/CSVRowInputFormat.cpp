...
void checkBadDelimiter(char delimiter, bool allow_whitespace_or_tab_as_delimiter) {
  ...
  if (bad_delimiters.contains(delimiter))
    throw Exception(ErrorCodes::BAD_ARGUMENTS,
                    "CSV format may not work correctly with delimiter '{}'. Try use CustomSeparated format instead", delimiter);
}
}  // namespace

CSVRowInputFormat::CSVRowInputFormat(SharedHeader header_, ReadBuffer &in_, const Params &params_, bool with_names_, bool with_types_,
                                     const FormatSettings &format_settings_)
    : CSVRowInputFormat(header_, std::make_shared<PeekableReadBuffer>(in_), params_, with_names_, with_types_, format_settings_) {}

CSVRowInputFormat::CSVRowInputFormat(SharedHeader header_, std::shared_ptr<PeekableReadBuffer> in_, const Params &params_, bool with_names_,
                                     bool with_types_, const FormatSettings &format_settings_,
                                     std::unique_ptr<CSVFormatReader> format_reader_)
    : RowInputFormatWithNamesAndTypes(header_, *in_, params_, false, with_names_, with_types_, format_settings_, std::move(format_reader_),
                                      format_settings_.csv.try_detect_header, format_settings_.csv.allow_variable_number_of_columns),
      buf(std::move(in_)) {
...
CSVRowInputFormat::CSVRowInputFormat(SharedHeader header_, std::shared_ptr<PeekableReadBuffer> in_, const Params &params_, bool with_names_,
                                     bool with_types_, const FormatSettings &format_settings_)
    : RowInputFormatWithNamesAndTypes(header_, *in_, params_, false, with_names_, with_types_, format_settings_,
                                      std::make_unique<CSVFormatReader>(*in_, format_settings_), format_settings_.csv.try_detect_header,
                                      format_settings_.csv.allow_variable_number_of_columns),
      buf(std::move(in_)) {
  checkBadDelimiter(format_settings_.csv.delimiter, format_settings_.csv.allow_whitespace_or_tab_as_delimiter);
}

void CSVRowInputFormat::syncAfterError() { skipToNextLineOrEOF(*buf); }
...
void CSVFormatReader::skipRow() {
  ...
  while (!istr.eof()) {
    if (quotes) {
      ...
      if (pos == istr.buffer().end()) continue;
      if (*pos == '"') {
        ++istr.position();
      ...
      if (*pos == '\n') {
        ++istr.position();
        if (!istr.eof() && *istr.position() == '\r') ++istr.position();
        return;
      }
      if (*pos == '\r') {
        ++istr.position();
        if (format_settings.csv.allow_cr_end_of_line) return;
        if (!istr.eof() && *pos == '\n') {
          ++pos;
          return;
        }
...
static void skipEndOfLine(ReadBuffer &in, bool allow_cr_end_of_line) {
  ...
  if (*in.position() == '\n') {
    ++in.position();
    if (!in.eof() && *in.position() == '\r') ++in.position();
...
CSVFormatReader::CSVFormatReader(PeekableReadBuffer &buf_, const FormatSettings &format_settings_)
...
void CSVFormatReader::skipRowEndDelimiter() {
  ...
  if (*buf->position() == format_settings.csv.delimiter) ++buf->position();
...
bool CSVFormatReader::readField(IColumn &column, const DataTypePtr &type, const SerializationPtr &serialization, bool is_last_file_column,
  ...
  if (format_settings.csv.use_default_on_bad_values) return readFieldOrDefault(column, type, serialization);
  return readFieldImpl(*buf, column, type, serialization);
}

bool CSVFormatReader::readFieldImpl(ReadBuffer &istr, DB::IColumn &column, const DB::DataTypePtr &type,
...
bool CSVFormatReader::readFieldOrDefault(DB::IColumn &column, const DB::DataTypePtr &type, const DB::SerializationPtr &serialization) {
  ...
  try {
    ...
    if (!field.empty() && !tmp_buf.eof()) is_bad_value = true;
  } catch (const Exception &) {
    is_bad_value = true;
  }

  if (!is_bad_value) return res;

  if (column.size() == col_size + 1) column.popBack(1);
  column.insertDefault();
  return false;
}

void CSVFormatReader::skipPrefixBeforeHeader() {
  for (size_t i = 0; i != format_settings.csv.skip_first_lines; ++i) readRow();
}
...
bool CSVFormatReader::checkForSuffix() {
  if (!format_settings.csv.skip_trailing_empty_lines) return buf->eof();
...
CSVSchemaReader::CSVSchemaReader(ReadBuffer &in_, bool with_names_, bool with_types_, const FormatSettings &format_settings_)
    : FormatWithNamesAndTypesSchemaReader(buf, format_settings_, with_names_, with_types_, &reader,
...
void registerInputFormatCSV(FormatFactory &factory) {
  auto register_func = [&](const String &format_name, bool with_names, bool with_types) {
    factory.registerInputFormat(format_name, [with_names, with_types](ReadBuffer &buf, const Block &sample, IRowInputFormat::Params params,
                                                                      const FormatSettings &settings) {
      return std::make_shared<CSVRowInputFormat>(std::make_shared<const Block>(sample), buf, std::move(params), with_names, with_types,
                                                 settings);
    });
  };
...
std::pair<bool, size_t> fileSegmentationEngineCSVImpl(ReadBuffer &in, DB::Memory<> &memory, size_t min_bytes, size_t min_rows,
                                                      size_t max_rows, const FormatSettings &settings) {
  ...
  while (loadAtPosition(in, memory, pos) && need_more_data) {
    if (quotes) {
      ...
      if (*pos == '\n') {
        ++pos;
        if (loadAtPosition(in, memory, pos) && *pos == '\r') ++pos;
      } else if (*pos == '\r') {
        ++pos;
        if (settings.csv.allow_cr_end_of_line) continue;
        if (loadAtPosition(in, memory, pos) && *pos == '\n')
          ++pos;
        else
          continue;
  ...
  auto register_func = [&](const String &format_name, bool with_names, bool with_types) {
    ...
    if (!with_types) {
      factory.registerAdditionalInfoForSchemaCacheGetter(format_name, [with_names](const FormatSettings &settings) {
        ...
        if (!with_names)
          result += fmt::format(", column_names_for_schema_inference={}, try_detect_header={}, skip_first_lines={}",
                                settings.column_names_for_schema_inference, settings.csv.try_detect_header, settings.csv.skip_first_lines);
        return result;
      });
    }
  };
