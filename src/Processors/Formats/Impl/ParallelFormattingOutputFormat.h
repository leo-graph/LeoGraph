...
class ParallelFormattingOutputFormat : public IOutputFormat {
 public:
  ...
  explicit ParallelFormattingOutputFormat(Params params)
      : IOutputFormat(params.header, params.out),
        ...
        pool(CurrentMetrics::ParallelFormattingOutputFormatThreads, CurrentMetrics::ParallelFormattingOutputFormatThreadsActive,
             CurrentMetrics::ParallelFormattingOutputFormatThreadsScheduled, params.max_threads_for_parallel_formatting)

  {
    ...
    collector_thread = ThreadFromGlobalPool([thread_group = CurrentThread::getGroup(), this] { collectorThreadFunction(thread_group); });
  }

  ~ParallelFormattingOutputFormat() override { finishAndWait(); }

  String getName() const override { return "ParallelFormattingOutputFormat"; }

  void flushImpl() override {
    if (!auto_flush) need_flush = true;
  }
  ...
  void onCancel() noexcept override { finishAndWait(); }
 ...
 private:
  void consume(Chunk chunk) final { addChunk(std::move(chunk), ProcessingUnitType::PLAIN, /*can_throw_exception*/ true); }
  ...
  enum ProcessingUnitStatus { READY_TO_INSERT, READY_TO_FORMAT, READY_TO_READ };
