...
MsgPackRowInputFormat::MsgPackRowInputFormat(SharedHeader header_, std::unique_ptr<PeekableReadBuffer> buf_, Params params_,
                                             const FormatSettings &settings)
    : IRowInputFormat(header_, *buf_, std::move(params_)),
      buf(std::move(buf_)),
      visitor(settings.null_as_default),
      parser(visitor),
...
void MsgPackVisitor::reset() { info_stack = {}; }
...
static void insertInteger(IColumn &column, DataTypePtr type, UInt64 value) {
  auto insert_func = [&](IColumn &column_, DataTypePtr type_) { insertInteger(column_, type_, value); };

  if (checkAndInsertNullable(column, type, insert_func) || checkAndInsertLowCardinality(column, type, insert_func)) return;
...
static void insertString(IColumn &column, DataTypePtr type, const char *value, size_t size, bool bin) {
  auto insert_func = [&](IColumn &column_, DataTypePtr type_) { insertString(column_, type_, value, size, bin); };
...
{
  auto insert_func = [&](IColumn &column_, DataTypePtr type_) { insertFloat32(column_, type_, value); };
...
{
  auto insert_func = [&](IColumn &column_, DataTypePtr type_) { insertFloat64(column_, type_, value); };
...
static void insertNull(IColumn &column, DataTypePtr type, UInt8 *read, bool null_as_default) {
  auto insert_func = [&](IColumn &column_, DataTypePtr type_) { insertNull(column_, type_, read, null_as_default); };

  /// LowCardinality(Nullable(...))
  if (checkAndInsertLowCardinality(column, type, insert_func)) return;
...
static void insertUUID(IColumn &column, DataTypePtr type, const char *value, size_t size) {
  auto insert_func = [&](IColumn &column_, DataTypePtr type_) { insertUUID(column_, type_, value, size); };
...
bool MsgPackVisitor::start_array(size_t size)  // NOLINT
{
  if (isArray(info_stack.top().type)) {
    ...
    if (size > 0) info_stack.push(Info{nested_column, nested_type, false, size, nullptr});
    ...
    if (size != nested_types.size())
      throw Exception(ErrorCodes::ILLEGAL_COLUMN, "Cannot insert MessagePack array with size {} into Tuple column with {} elements", size,
                      nested_types.size());
    ...
    if (nested_types.empty()) column_tuple.addSize(1);
  } else {
    throw Exception(ErrorCodes::ILLEGAL_COLUMN, "Cannot insert MessagePack array into column with type {}",
                    info_stack.top().type->getName());
  }
...
bool MsgPackVisitor::end_array_item()  // NOLINT
{
  if (info_stack.top().is_tuple_element)
    ...
    --current_array_size;
    if (current_array_size == 0) info_stack.pop();
  }
  return true;
}
...
bool MsgPackVisitor::start_map(uint32_t size)  // NOLINT
{
  if (!isMap(info_stack.top().type))
    throw Exception(ErrorCodes::ILLEGAL_COLUMN, "Cannot insert MessagePack map into column with type {}.",
                    info_stack.top().type->getName());
...
bool MsgPackRowInputFormat::readObject(Parser &msgpack_parser) {
  ...
  while (!msgpack_parser.execute(buf->position(), buf->available(), offset)) {
    ...
    if (buf->eof()) throw Exception(ErrorCodes::INCORRECT_DATA, "Unexpected end of file while parsing msgpack object.");
    buf->position() = buf->buffer().end();
...
size_t MsgPackRowInputFormat::countRows(size_t max_block_size) {
  ...
  while (!buf->eof() && num_rows < max_block_size) {
    for (size_t i = 0; i < columns; ++i) readObject(null_parser);
    ++num_rows;
  }
...
bool MsgPackRowInputFormat::readRow(MutableColumns &columns, RowReadExtension &ext) {
  ...
  if (!has_more_data) {
    if (column_index != 0) throw Exception(ErrorCodes::INCORRECT_DATA, "Not enough values to complete the row.");
    return false;
  }
  return true;
}
...
msgpack::object_handle MsgPackSchemaReader::readObject() {
  if (buf.eof()) throw Exception(ErrorCodes::UNEXPECTED_END_OF_FILE, "Unexpected eof while parsing msgpack object");
  ...
  while (need_more_data) {
    ...
    try {
      ...
      buf.position() = buf.buffer().end();
      if (buf.eof()) throw Exception(ErrorCodes::UNEXPECTED_END_OF_FILE, "Unexpected end of file while parsing msgpack object");
      buf.position() = buf.buffer().end();
      buf.makeContinuousMemoryFromCheckpointToPos();
      buf.rollbackToCheckpoint();
    }
...
DataTypePtr MsgPackSchemaReader::getDataType(const msgpack::object &object) {
  switch (object.type) {
    ...
    case msgpack::type::object_type::ARRAY: {
      ...
      if (!object_array.size) return nullptr;
      ...
      if (nested_types_are_equal) return std::make_shared<DataTypeArray>(nested_types[0]);
    ...
    case msgpack::type::object_type::MAP: {
      ...
      if (object_map.size) {
        ...
        if (key_type && value_type) return std::make_shared<DataTypeMap>(key_type, value_type);
      }
      return nullptr;
    }
    ...
    case msgpack::type::object_type::EXT: {
      ...
      if (object_ext.type() == int8_t(MsgPackExtensionTypes::UUIDType)) return std::make_shared<DataTypeUUID>();
...
void registerInputFormatMsgPack(FormatFactory &factory) {
  factory.registerInputFormat(
      "MsgPack", [](ReadBuffer &buf, const Block &sample, const RowInputFormatParams &params, const FormatSettings &settings) {
...
void registerMsgPackSchemaReader(FormatFactory &factory) {
  factory.registerSchemaReader(
      "MsgPack", [](ReadBuffer &buf, const FormatSettings &settings) { return std::make_shared<MsgPackSchemaReader>(buf, settings); });
