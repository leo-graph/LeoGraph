...
namespace DB {
ODBCDriver2BlockOutputFormat::ODBCDriver2BlockOutputFormat(WriteBuffer& out_, SharedHeader header_, const FormatSettings& format_settings_)
...
void ODBCDriver2BlockOutputFormat::write(Chunk chunk, PortKind) {
  ...
  for (size_t i = 0; i < rows; ++i) writeRow(columns, i, text_value);
}

void ODBCDriver2BlockOutputFormat::consume(Chunk chunk) { write(std::move(chunk), PortKind::Main); }

void ODBCDriver2BlockOutputFormat::consumeTotals(Chunk chunk) { write(std::move(chunk), PortKind::Totals); }
...
void ODBCDriver2BlockOutputFormat::writePrefix() {
  ...
  for (size_t i = 0; i < columns; ++i) {
    ...
    if (type->lowCardinality()) type = recursiveRemoveLowCardinality(type);
    writeODBCString(out, type->getName());
  }
}

void registerOutputFormatODBCDriver2(FormatFactory& factory) {
  factory.registerOutputFormat("ODBCDriver2", [](WriteBuffer& buf, const Block& sample, const FormatSettings& format_settings,
