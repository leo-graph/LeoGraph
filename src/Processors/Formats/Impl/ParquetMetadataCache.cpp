...
ParquetMetadataCacheCell::ParquetMetadataCacheCell(parquet::format::FileMetaData metadata_)
    : metadata(std::move(metadata_)), memory_bytes(calculateMemorySize() + SIZE_IN_MEMORY_OVERHEAD) {}
size_t ParquetMetadataCacheCell::calculateMemorySize() const {
  ...
  for (const parquet::format::RowGroup &row_group : metadata.row_groups) {
    ...
    for (const parquet::format::ColumnChunk &column_chunk : row_group.columns) {
      ...
      for (const auto &path : column_chunk.meta_data.path_in_schema) total_size += path.capacity();

      for (const auto &kv : column_chunk.meta_data.key_value_metadata) total_size += kv.key.capacity() + kv.value.capacity();
  ...
  for (const auto &kv : metadata.key_value_metadata) total_size += kv.key.capacity() + kv.value.capacity();

  // ColumnOrder parquet::format::ColumnOrders
  for (const auto &order : metadata.column_orders) total_size += sizeof(order);
...
size_t ParquetMetadataCacheWeightFunction::operator()(const ParquetMetadataCacheCell &cell) const { return cell.memory_bytes; }
...
ParquetMetadataCache::ParquetMetadataCache(const String &cache_policy, size_t max_size_in_bytes, size_t max_count, double size_ratio)
    : Base(cache_policy, CurrentMetrics::ParquetMetadataCacheBytes, CurrentMetrics::ParquetMetadataCacheFiles, max_size_in_bytes, max_count,
           size_ratio),
