...
Chunk ORCBlockInputFormat::read() {
  ...
  if (stripe_current >= stripe_total) return {};

  if (need_only_count) return getChunkForCount(file_reader->GetRawORCReader()->getStripe(stripe_current++)->getNumberOfRows());
  ...
  if (!table_result.ok())
    throw Exception(ErrorCodes::CANNOT_READ_ALL_DATA, "Error while reading batch of ORC data: {}", table_result.status().ToString());
...
const BlockMissingValues *ORCBlockInputFormat::getMissingValues() const { return &block_missing_values; }

static void getFileReaderAndSchema(ReadBuffer &in, std::unique_ptr<arrow::adapters::orc::ORCFileReader> &file_reader,
                                   std::shared_ptr<arrow::Schema> &schema, const FormatSettings &format_settings,
  ...
  if (!result.ok()) throw Exception::createDeprecated(result.status().ToString(), ErrorCodes::BAD_ARGUMENTS);
  file_reader = std::move(result).ValueOrDie();

  auto read_schema_result = file_reader->ReadSchema();
  if (!read_schema_result.ok()) throw Exception::createDeprecated(read_schema_result.status().ToString(), ErrorCodes::BAD_ARGUMENTS);
...
void ORCBlockInputFormat::prepareReader() {
  ...
  arrow_column_to_ch_column = std::make_unique<ArrowColumnToCHColumn>(
      getPort().getHeader(), "ORC", format_settings, std::nullopt, std::nullopt, format_settings.orc.allow_missing_columns,
...
NamesAndTypesList ORCSchemaReader::readSchema() {
  ...
  auto header = ArrowColumnToCHColumn::arrowSchemaToCHHeader(
      *schema, metadata, "ORC", format_settings, format_settings.orc.skip_columns_with_unsupported_types_in_schema_inference,
...
void registerInputFormatORC(FormatFactory &factory) {
  factory.registerRandomAccessInputFormat(
      "ORC", [](ReadBuffer &buf, const Block &sample, const FormatSettings &settings, const ReadSettings &read_settings, bool is_remote_fs,
                FormatParserSharedResourcesPtr, FormatFilterInfoPtr format_filter_info) {
        InputFormatPtr res;
        if (settings.orc.use_fast_decoder) {
          ...
          res = std::make_shared<NativeORCBlockInputFormat>(buf, std::make_shared<const Block>(sample), settings, use_prefetch,
                                                            min_bytes_for_seek, format_filter_info);
        } else
...
void registerORCSchemaReader(FormatFactory &factory) {
  factory.registerSchemaReader("ORC", [](ReadBuffer &buf, const FormatSettings &settings) {
    SchemaReaderPtr res;
    if (settings.orc.use_fast_decoder)
