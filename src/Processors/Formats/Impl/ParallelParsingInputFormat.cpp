...
void ParallelParsingInputFormat::parserThreadFunction(size_t current_ticket_number) {
  ...
  try {
    ...
    if (current_ticket_number != 0) input_format->setColumnMapping(column_mapping);
    ...
    while (!parsing_finished && (chunk = parser.getChunk())) {
      ...
      approx_chunk_size = static_cast<size_t>(std::ceil(static_cast<double>(approx_chunk_size) / static_cast<double>(unit.segment.size()) *
                                                        static_cast<double>(unit.original_segment_size)));
...
void ParallelParsingInputFormat::onBackgroundException() {
  ...
  if (is_server) tryLogCurrentException(__PRETTY_FUNCTION__);
...
Chunk ParallelParsingInputFormat::read() {
  ...
  if (!next_block_in_current_unit.has_value()) {
    while (true) {
      ...
      if (!unit->chunk_ext.chunk.empty()) break;
