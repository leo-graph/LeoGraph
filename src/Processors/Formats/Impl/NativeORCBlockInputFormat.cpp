...
ORCInputStream::ORCInputStream(SeekableReadBuffer &in_, size_t file_size_, bool use_prefetch)
  ...
  if (supports_read_at) async_runner = threadPoolCallbackRunnerUnsafe<void>(getIOThreadPool().get(), ThreadName::ORC_FILE);
}

UInt64 ORCInputStream::getLength() const { return file_size; }

UInt64 ORCInputStream::getNaturalReadSize() const { return 128 * 1024; }
...
void ORCInputStream::read(void *buf, UInt64 length, UInt64 offset) {
  if (supports_read_at) {
    ...
    if (offset != static_cast<UInt64>(in.getPosition())) in.seek(offset, SEEK_SET);
  ...
  if (supports_read_at) {
    return async_runner(
        [this, buf, length, offset] {
          ...
          LOG_TEST(getLogger("NativeORCBlockInputFormat"), "Read {} bytes from {} offset in {} ms", length, offset,
...
std::unique_ptr<orc::InputStream> asORCInputStreamLoadIntoMemory(ReadBuffer &in, std::atomic<int> &is_cancelled) {
  ...
  if (bytes_read < magic_size || file_data != ORC_MAGIC_BYTES) throw Exception(ErrorCodes::INCORRECT_DATA, "Not an ORC file");
...
static DataTypePtr parseORCType(const orc::Type *orc_type, bool skip_columns_with_unsupported_types, bool dictionary_as_low_cardinality,
                                const orc::StripeInformation *stripe_info, bool &skipped) {
  ...
  switch (orc_type->getKind()) {
    ...
    case orc::TypeKind::LIST: {
      if (subtype_count != 1) throw Exception(ErrorCodes::LOGICAL_ERROR, "Invalid Orc List type {}", orc_type->toString());

      DataTypePtr nested_type =
          parseORCType(orc_type->getSubtype(0), skip_columns_with_unsupported_types, dictionary_as_low_cardinality, stripe_info, skipped);
      if (skipped) return {};

      return std::make_shared<DataTypeArray>(nested_type);
    }
    case orc::TypeKind::MAP: {
      if (subtype_count != 2) throw Exception(ErrorCodes::LOGICAL_ERROR, "Invalid Orc Map type {}", orc_type->toString());
      ...
      DataTypePtr value_type =
          parseORCType(orc_type->getSubtype(1), skip_columns_with_unsupported_types, dictionary_as_low_cardinality, stripe_info, skipped);
      if (skipped) return {};
...
static String getColumnNameFromKeyCondition(const KeyCondition &key_condition, size_t indice) {
  ...
  for (const auto &[name, i] : key_columns) {
    if (i == indice) return name;
  }
  throw Exception(ErrorCodes::LOGICAL_ERROR, "Can't get column from KeyCondition with indice {}", indice);
}

static std::optional<orc::Literal> convertFieldToORCLiteral(const orc::Type &orc_type, const Field &field,
  ...
  try {
    ...
    switch (orc_type.getKind()) {
      ...
      case orc::DOUBLE: {
        Float64 val;
        if (field.tryGet(val)) return orc::Literal(val);
        break;
      }
      case orc::VARCHAR:
      case orc::CHAR:
      case orc::STRING: {
        String str;
        if (field.tryGet(str)) return orc::Literal(str.data(), str.size());
        break;
      }
      case orc::DATE: {
        Int64 val;
        if (field.tryGet(val)) return orc::Literal(orc::PredicateDataType::DATE, val);
        break;
      }
      case orc::TIMESTAMP: {
        if (type_hint && isDateTime64(type_hint)) {
          const auto *datetime64_type = typeid_cast<const DataTypeDateTime64 *>(type_hint.get());
          if (datetime64_type->getScale() != 9) return std::nullopt;
        }
      ...
      case orc::DECIMAL: {
        ...
        if (precision <= DecimalUtils::max_precision<Decimal32>) {
          ...
          if (field.tryGet(val)) {
            ...
            return orc::Literal(orc::Int128(right), static_cast<Int32>(orc_type.getPrecision()), static_cast<Int32>(orc_type.getScale()));
          }
          ...
          if (field.tryGet(val)) {
            ...
            return orc::Literal(orc::Int128(high, low), static_cast<Int32>(orc_type.getPrecision()),
                                static_cast<Int32>(orc_type.getScale()));
          }
        }
        break;
      }
...
///     2. if elem function is FUNCTION_IN_RANGE/FUNCTION_NOT_IN_RANGE, `set_index` is not null and `set_index->getOrderedSet().size()`
///     is 1.
///     3. elem function should be
///     FUNCTION_IN_RANGE/FUNCTION_NOT_IN_RANGE/FUNCTION_IN_SET/FUNCTION_NOT_IN_SET/FUNCTION_IS_NULL/FUNCTION_IS_NOT_NULL
static bool evaluateRPNElement(const Field &field, const KeyCondition::RPNElement &elem) {
  ...
  switch (elem.function) {
    ...
    case KeyCondition::RPNElement::FUNCTION_NOT_IN_RANGE: {
      ...
      if (elem.function == KeyCondition::RPNElement::FUNCTION_NOT_IN_RANGE) res = !res;
      return res;
    }
    ...
    case KeyCondition::RPNElement::FUNCTION_NOT_IN_SET: {
      ...
      if (elem.function == KeyCondition::RPNElement::FUNCTION_NOT_IN_SET) res = !res;
      return res;
    }
    case KeyCondition::RPNElement::FUNCTION_IS_NULL:
    case KeyCondition::RPNElement::FUNCTION_IS_NOT_NULL: {
      if (field.isNull()) return elem.function == KeyCondition::RPNElement::FUNCTION_IS_NULL;
...
static void buildORCSearchArgumentImpl(const KeyCondition &key_condition, const Block &header, const orc::Type &schema,
                                       KeyCondition::RPN &rpn_stack, orc::SearchArgumentBuilder &builder,
                                       const FormatSettings &format_settings) {
  if (rpn_stack.empty()) throw Exception(ErrorCodes::LOGICAL_ERROR, "Empty rpn stack in buildORCSearchArgumentImpl");
  ...
  switch (curr.function) {
    ...
    case KeyCondition::RPNElement::FUNCTION_IS_NOT_NULL: {
      ...
      const bool contains_is_null =
          curr.function == KeyCondition::RPNElement::FUNCTION_IS_NULL || curr.function == KeyCondition::RPNElement::FUNCTION_IS_NOT_NULL;
      const bool contains_in_set =
          curr.function == KeyCondition::RPNElement::FUNCTION_IN_SET || curr.function == KeyCondition::RPNElement::FUNCTION_NOT_IN_SET;
      const bool contains_in_range =
          curr.function == KeyCondition::RPNElement::FUNCTION_IN_RANGE || curr.function == KeyCondition::RPNElement::FUNCTION_NOT_IN_RANGE;
      ...
      /// If null_as_default is true, the only difference is nullable, and the evaluations of current RPNElement based on default and null
      /// field have the same result, we still should push down current filter.
      ...
      if (need_wrap_not) builder.startNot();
      ...
      if (contains_is_null) {
        ...
        if (!has_left_bound && !has_right_bound) {
          ...
          if (has_left_bound) left_literal = convertFieldToORCLiteral(*orc_type, range.left);

          std::optional<orc::Literal> right_literal;
          if (has_right_bound) right_literal = convertFieldToORCLiteral(*orc_type, range.right);

          if (has_left_bound && has_right_bound) builder.startAnd();
          ...
          if (has_left_bound && has_right_bound) builder.end();
        }
...
std::unique_ptr<orc::SearchArgument> buildORCSearchArgument(const KeyCondition &key_condition, const Block &header, const orc::Type &schema,
                                                            const FormatSettings &format_settings) {
...
static void getFileReader(ReadBuffer &in, std::unique_ptr<orc::Reader> &file_reader, const FormatSettings &format_settings,
                          bool use_prefetch, size_t min_bytes_for_seek, std::atomic<int> &is_stopped) {
...
static const orc::Type *traverseDownORCTypeByName(const std::string &target, const orc::Type *orc_type, DataTypePtr &type,
                                                  bool ignore_case) {
  if (target.empty()) return orc_type;
  ...
  auto search_struct_field = [&](const std::string &target_, const orc::Type *type_) -> std::pair<std::string, const orc::Type *> {
    ...
    if (ignore_case) boost::to_lower(target_copy);

    for (size_t i = 0; i < type_->getSubtypeCount(); ++i) {
      auto field_name = type_->getFieldName(i);
      if (ignore_case) boost::to_lower(field_name);
  ...
  if (orc::LIST == orc_type->getKind()) {
    ...
    /// For example, "a Nested(x String, y Int64)" is flattened to "a.x Array(String), a.y Array(Int64)", and ORC file schema is still "a
    /// array<struct<x string, y long>>". In this case, we should skip possible array type and traverse down to its nested struct type.
...
static void updateIncludeTypeIds(DataTypePtr type, const orc::Type *orc_type, bool ignore_case,
  ...
  switch (orc_type->getKind()) {
    ...
    case orc::STRUCT: {
      /// To make sure tuple field pruning work fine, we should include only the fields of ORC struct type which are also contained in CH
      /// tuple types, instead of all fields of ORC struct type. For example, CH tupe type in header is "x Tuple(a String)", ORC struct type
      /// is "x struct<a:string, b:long>", then only type id of field "x.a" should be included. For tuple field pruning purpose, we should
      /// never include "x.b" for it is not required in format header.
      ...
      if (tuple_type) {
        if (tuple_type->hasExplicitNames()) {
          ...
          for (size_t tuple_i = 0; tuple_i < element_names.size(); ++tuple_i) {
            ...
            if (ignore_case) boost::to_lower(element_name);

            if (orc_field_name_to_index.contains(element_name)) {
              updateIncludeTypeIds(tuple_type->getElement(tuple_i), orc_type->getSubtype(orc_field_name_to_index[element_name]),
                                   ignore_case, include_typeids);
            }
          }
        } else {
...
NativeORCBlockInputFormat::NativeORCBlockInputFormat(ReadBuffer &in_, SharedHeader header_, const FormatSettings &format_settings_,
                                                     bool use_prefetch_, size_t min_bytes_for_seek_,
...
void NativeORCBlockInputFormat::prepareFileReader() {
  ...
  if (format_filter_info) format_filter_info->initOnce([&] { format_filter_info->initKeyCondition(getPort().getHeader()); });

  std::unique_ptr<orc::StripeInformation> stripe_info;
  if (file_reader->getNumberOfStripes()) stripe_info = file_reader->getStripe(0);

  orc_column_to_ch_column = std::make_unique<ORCColumnToCHColumn>(
      getPort().getHeader(), format_settings.orc.allow_missing_columns, format_settings.null_as_default,
      format_settings.orc.case_insensitive_column_matching, format_settings.orc.dictionary_as_low_cardinality);
  ...
  for (const auto &column : header) {
    ...
    if (orc_type) updateIncludeTypeIds(adjusted_type, orc_type, ignore_case, include_typeids);
  }
  ...
  if (use_prefetch) prefetchStripes();
}

void NativeORCBlockInputFormat::prefetchStripes() {
  if (prefetch_iterator >= selected_stripes.size()) return;
  ...
  LOG_TEST(getLogger("NativeORCBlockInputFormat"), "Prefetch {} stripes with {} columns and {} bytes takes {} ms", stripes.size(),
           include_indices.size(), total_stripe_size, time.elapsedMilliseconds());
}
...
std::vector<int> NativeORCBlockInputFormat::calculateSelectedStripes(int num_stripes, const std::unordered_set<int> &skip_stripes) {
  ...
  for (int stripe = 0; stripe < num_stripes; ++stripe) {
    if (skip_stripes.contains(stripe)) continue;
...
bool NativeORCBlockInputFormat::prepareStripeReader() {
  ...
  if (read_iterator >= selected_stripes.size()) return false;
...
Chunk NativeORCBlockInputFormat::read() {
  ...
  if (!file_reader) prepareFileReader();

  if (need_only_count) {
    if (read_iterator >= selected_stripes.size()) return {};
  ...
  if (!stripe_reader) {
    if (!prepareStripeReader()) return {};
  }

  if (is_stopped) return {};

  /// TODO: figure out why reuse batch would cause asan fatals in
  /// https://s3.amazonaws.com/clickhouse-test-reports/55330/be39d23af2d7e27f5ec7f168947cf75aeaabf674/stateless_tests__asan__[4_4].html Not
  /// sure if it is a false positive case. Notice that reusing batch will speed up reading ORC by 1.15x.
  Stopwatch time;
...
const BlockMissingValues *NativeORCBlockInputFormat::getMissingValues() const { return &block_missing_values; }
...
NamesAndTypesList NativeORCSchemaReader::readSchema() {
  ...
  for (size_t i = 0; i < schema.getSubtypeCount(); ++i) {
    ...
    bool skipped = false;
    DataTypePtr type = parseORCType(orc_type, format_settings.orc.skip_columns_with_unsupported_types_in_schema_inference,
                                    format_settings.orc.dictionary_as_low_cardinality, stripe_info.get(), skipped);
    if (!skipped) header.insert(ColumnWithTypeAndName{type, name});
  }

  /// ORC doesn't have non-nullable data types.
  if (format_settings.schema_inference_make_columns_nullable != 0) return getNamesAndRecursivelyNullableTypes(header, format_settings);
  return header.getNamesAndTypesList();
}

ORCColumnToCHColumn::ORCColumnToCHColumn(const Block &header_, bool allow_missing_columns_, bool null_as_default_,
                                         bool case_insensitive_matching_, bool dictionary_as_low_cardinality_)
    : header(header_),
...
void ORCColumnToCHColumn::orcTableToCHChunk(Chunk &res, const orc::Type *schema, const orc::ColumnVectorBatch *table, size_t num_rows,
                                            BlockMissingValues *block_missing_values) {
  const auto *struct_batch = dynamic_cast<const orc::StructVectorBatch *>(table);
  if (!struct_batch) throw Exception(ErrorCodes::LOGICAL_ERROR, "ORC table must be StructVectorBatch but is {}", struct_batch->toString());
  ...
  for (size_t i = 0; i < field_num; ++i) {
    ...
    if (!field) throw Exception(ErrorCodes::LOGICAL_ERROR, "ORC table field {} is null", name);

    if (case_insensitive_matching) boost::to_lower(name);
...
static ColumnPtr readByteMapFromORCColumn(const orc::ColumnVectorBatch *orc_column) {
  if (!orc_column->hasNulls) return ColumnUInt8::create(orc_column->numElements, static_cast<UInt8>(0));
  ...
  for (size_t i = 0; i < orc_column->numElements; ++i) bytemap_data[i] = 1 - orc_column->notNull[i];
  return nullmap_column;
}

static const orc::ColumnVectorBatch *getNestedORCColumn(const orc::ListVectorBatch *orc_column) { return orc_column->elements.get(); }
...
static ColumnPtr readOffsetsFromORCListColumn(const BatchType *orc_column) {
  ...
  for (size_t i = 0; i < orc_column->numElements; ++i) offsets_data.push_back(orc_column->offsets[i + 1]);

  return offsets_column;
}

static ColumnWithTypeAndName readColumnWithBooleanData(const orc::ColumnVectorBatch *orc_column, const String &column_name) {
...
static ColumnWithTypeAndName readColumnWithNumericData(const orc::ColumnVectorBatch *orc_column, const String &column_name) {
...
static ColumnWithTypeAndName readColumnWithEncodedStringOrFixedStringData(const orc::ColumnVectorBatch *orc_column,
                                                                          const orc::Type *orc_type, const String &column_name,
                                                                          bool nullable) {
  /// Fill CH holder_column with ORC dictionary
  /// Note that holder_column is always a ColumnString or ColumnFixedstring whether nullable is true or false, because ORC dictionary
  ...
  auto call_by_type = [&](auto index_type) -> MutableColumnPtr {
    ...
    if (!concrete_index_column) return nullptr;
    ...
    if (!orc_str_column.hasNulls) {
      ...
      for (size_t i = 0; i < rows; ++i) {
        ...
        /// Otherwise 0 represents default string value, it is reasonable because null values are converted to default values when casting
        /// nullable column to non-nullable.
  ...
  if (!internal_column) internal_column = call_by_type(UInt8());
  if (!internal_column) internal_column = call_by_type(UInt16());
  if (!internal_column) internal_column = call_by_type(UInt32());
  if (!internal_column) internal_column = call_by_type(UInt64());
  return {std::move(internal_column), internal_type, column_name};
}

static ColumnWithTypeAndName readColumnWithStringData(const orc::ColumnVectorBatch *orc_column, const String &column_name) {
  ...
  for (size_t i = 0; i < orc_str_column->numElements; ++i) {
    if (!orc_str_column->hasNulls || orc_str_column->notNull[i]) reserve_size += orc_str_column->length[i];
  }
...
static ColumnWithTypeAndName readColumnWithFixedStringData(const orc::ColumnVectorBatch *orc_column, const orc::Type *orc_type,
...
static ColumnWithTypeAndName readColumnWithDecimalDataCast(const orc::ColumnVectorBatch *orc_column, const String &column_name,
...
static ColumnWithTypeAndName readIPv6ColumnFromBinaryData(const orc::ColumnVectorBatch *orc_column, const String &column_name) {
  const auto *orc_str_column = dynamic_cast<const orc::StringVectorBatch *>(orc_column);
...
static ColumnWithTypeAndName readIPv4ColumnWithInt32Data(const orc::ColumnVectorBatch *orc_column, const String &column_name) {
...
static ColumnWithTypeAndName readColumnWithBigNumberFromBinaryData(const orc::ColumnVectorBatch *orc_column, const String &column_name,
  ...
  for (size_t i = 0; i < orc_str_column->numElements; ++i) {
    if (!orc_str_column->hasNulls || orc_str_column->notNull[i]) {
      if (sizeof(typename ColumnType::ValueType) != orc_str_column->length[i])
        throw Exception(ErrorCodes::INCORRECT_DATA, "ValueType size {} of column {} is not equal to size of binary data {}",
                        sizeof(typename ColumnType::ValueType), integer_column.getName(), orc_str_column->length[i]);
...
static ColumnWithTypeAndName readColumnWithDateData(const orc::ColumnVectorBatch *orc_column, const String &column_name,
    ...
    if (!orc_int_column->hasNulls || orc_int_column->notNull[i]) {
      ...
      if (check_date32_range && (days_num > DATE_LUT_MAX_EXTEND_DAY_NUM || days_num < -DAYNUM_OFFSET_EPOCH))
        throw Exception(ErrorCodes::VALUE_IS_OUT_OF_RANGE_OF_DATA_TYPE,
                        "Input value {} of a column \"{}\" exceeds the range of type Date32, which is [{}, {}]", days_num, column_name,
                        -DAYNUM_OFFSET_EPOCH, DATE_LUT_MAX_EXTEND_DAY_NUM);

      if (check_date_range && (days_num > DATE_LUT_MAX_DAY_NUM || days_num < 0))
        throw Exception(ErrorCodes::VALUE_IS_OUT_OF_RANGE_OF_DATA_TYPE,
                        "Input value {} of a column \"{}\" exceeds the range of type Date, which is [0, {}]", days_num, column_name,
...
static ColumnWithTypeAndName readColumnWithTimestampData(const orc::ColumnVectorBatch *orc_column, const String &column_name) {
  ...
  for (size_t i = 0; i < orc_ts_column->numElements; ++i) {
    if (!orc_ts_column->hasNulls || orc_ts_column->notNull[i]) {
      ...
      if (overflow)
        throw Exception(ErrorCodes::VALUE_IS_OUT_OF_RANGE_OF_DATA_TYPE,
                        "Timestamp value in column \"{}\" is out of range for DateTime64: seconds={}, nanoseconds={}", column_name, seconds,
                        nanoseconds);
...
ColumnWithTypeAndName ORCColumnToCHColumn::readColumnFromORCColumn(const orc::ColumnVectorBatch *orc_column, const orc::Type *orc_type,
                                                                   const std::string &column_name, bool inside_nullable,
  ...
  if (!inside_nullable && (orc_column->hasNulls || (type_hint && type_hint->isNullable())) && !orc_column->isEncoded &&
    ...
    if (type_hint) nested_type_hint = removeNullable(type_hint);
  ...
  switch (orc_type->getKind()) {
    ...
    case orc::INT: {
      ...
      if (type_hint && isIPv4(type_hint)) return readIPv4ColumnWithInt32Data(orc_column, column_name);
    ...
    case orc::DECIMAL: {
      ...
      throw Exception(ErrorCodes::ARGUMENT_OUT_OF_BOUND, "Decimal precision {} in ORC type {} is out of bound", precision,
                      orc_type->toString());
    }
    case orc::MAP: {
    ...
    case orc::LIST: {
      ...
      if (type_hint) {
        ...
        if (array_type_hint) nested_type_hint = array_type_hint->getNestedType();
      }
    ...
    default:
      throw Exception(ErrorCodes::UNKNOWN_TYPE, "Unsupported ORC type {} while reading column {}.", orc_type->toString(), column_name);
  }
}

void ORCColumnToCHColumn::orcColumnsToCHChunk(Chunk &res, NameToColumnPtr &name_to_column_ptr, size_t num_rows,
                                              BlockMissingValues *block_missing_values) {
  ...
  for (size_t column_i = 0, columns = header.columns(); column_i < columns; ++column_i) {
    ...
    if (case_insensitive_matching) boost::to_lower(search_column_name);
    ...
    if (!name_to_column_ptr.contains(search_column_name)) {
      ...
      if (case_insensitive_matching) boost::to_lower(search_nested_table_name);
      if (name_to_column_ptr.contains(search_nested_table_name)) {
        if (!nested_tables.contains(search_nested_table_name)) {
          ...
          for (const auto &name_and_type : header.getNamesAndTypesList()) {
            if (name_and_type.name.starts_with(nested_table_name + ".")) nested_columns.push_back(name_and_type);
          }
          ...
          ColumnsWithTypeAndName cols = {readColumnFromORCColumn(orc_column_with_type.first, orc_column_with_type.second, nested_table_name,
                                                                 false, nested_table_type)};
        ...
        if (nested_column) {
          ...
          if (case_insensitive_matching) column.name = header_column.name;
      ...
      if (!read_from_nested) {
        ...
        if (block_missing_values) block_missing_values->setBits(column_i, num_rows);
        continue;
      }
    } else {
      auto orc_column_with_type = name_to_column_ptr[search_column_name];
      column =
          readColumnFromORCColumn(orc_column_with_type.first, orc_column_with_type.second, header_column.name, false, header_column.type);
    }

    if (null_as_default) insertNullAsDefaultIfNeeded(column, header_column, column_i, block_missing_values);

    try {
      column.column = castColumn(column, header_column.type);
    } catch (Exception &e) {
      e.addMessage(fmt::format("while converting column {} from type {} to type {}", backQuote(header_column.name), column.type->getName(),
