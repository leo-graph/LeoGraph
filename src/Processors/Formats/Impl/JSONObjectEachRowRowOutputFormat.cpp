...
JSONObjectEachRowRowOutputFormat::JSONObjectEachRowRowOutputFormat(WriteBuffer& out_, SharedHeader header_, const FormatSettings& settings_)
    : JSONEachRowRowOutputFormat(out_, header_, settings_),
      field_index_for_object_name(getColumnIndexForJSONObjectEachRowObjectName(*header_, settings_)) {}
...
void JSONObjectEachRowRowOutputFormat::writePrefix() { JSONUtils::writeObjectStart(*ostr); }

void JSONObjectEachRowRowOutputFormat::writeRowStartDelimiter() { JSONUtils::writeCompactObjectStart(*ostr, 1, object_name.c_str()); }
...
void JSONObjectEachRowRowOutputFormat::writeRowBetweenDelimiter() { JSONUtils::writeFieldDelimiter(*ostr, 1); }
...
void registerOutputFormatJSONObjectEachRow(FormatFactory& factory) {
  factory.registerOutputFormat("JSONObjectEachRow", [](WriteBuffer& buf, const Block& sample, const FormatSettings& _format_settings,
