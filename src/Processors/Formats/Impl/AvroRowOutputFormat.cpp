...
class AvroSerializerTraits {
 public:
  explicit AvroSerializerTraits(const FormatSettings &settings_) : string_to_string_regexp(settings_.avro.string_column_pattern) {
    if (!string_to_string_regexp.ok())
      throw Exception(ErrorCodes::CANNOT_COMPILE_REGEXP,
                      "Avro: cannot compile re2: {}, error: {}. "
                      "Look at https://github.com/google/re2/wiki/Syntax for reference.",
                      settings_.avro.string_column_pattern, string_to_string_regexp.error());
  }

  bool isStringAsString(const String &column_name) { return RE2::PartialMatch(column_name, string_to_string_regexp); }
...
AvroSerializer::SchemaWithSerializeFn createBigIntegerSchemaWithSerializeFn(const DataTypePtr &data_type, size_t type_name_increment) {
  auto schema = avro::FixedSchema(sizeof(BigIntegerType),
                                  boost::algorithm::to_lower_copy(data_type->getName()) + std::to_string(type_name_increment));
  return {schema, [](const IColumn &column, size_t row_num, avro::Encoder &encoder) {
...
AvroSerializer::SchemaWithSerializeFn AvroSerializer::createSchemaWithSerializeFn(const DataTypePtr &data_type, size_t &type_name_increment,
  ...
  switch (data_type->getTypeId()) {
    ...
    case TypeIndex::Variant: {
      ...
      return {static_cast<avro::Schema>(union_schema), [serializers = std::move(nested_serializers), null_union_index](
                                                           const IColumn &column, const size_t row_num, avro::Encoder &encoder) {
    ...
    case TypeIndex::Tuple: {
      ...
      return {schema, [nested_serializers](const IColumn &column, size_t row_num, avro::Encoder &encoder) {
                ...
                for (size_t i = 0; i != nested_serializers.size(); ++i) nested_serializers[i](*nested_columns[i], row_num, encoder);
              }};
    }
    case TypeIndex::Map: {
      ...
      return {schema, [keys_serializer, values_mapping](const IColumn &column, size_t row_num, avro::Encoder &encoder) {
                ...
                encoder.mapStart();
                if (row_count > 0) encoder.setItemCount(row_count);
...
#  endif
  ...
  if (codec_name == "zstd") return avro::Codec::ZSTD_CODEC;
#  ifdef SNAPPY_CODEC_AVAILABLE
...
AvroRowOutputFormat::AvroRowOutputFormat(WriteBuffer &out_, SharedHeader header_, const FormatSettings &settings_)
...
void AvroRowOutputFormat::createFileWriter() {
  file_writer_ptr =
      std::make_unique<avro::DataFileWriterBase>(std::make_unique<OutputStreamWriteBufferAdapter>(out), serializer.getSchema(),
                                                 settings.avro.output_sync_interval, getCodec(settings.avro.output_codec));
}
...
void AvroRowOutputFormat::write(const Columns &columns, size_t row_num) {
  if (!file_writer_ptr) createFileWriter();
  file_writer_ptr->syncIfNeeded();
...
void AvroRowOutputFormat::resetFormatterImpl() { file_writer_ptr.reset(); }

void registerOutputFormatAvro(FormatFactory &factory) {
  factory.registerOutputFormat(
      "Avro", [](WriteBuffer &buf, const Block &sample, const FormatSettings &settings, FormatFilterInfoPtr /*format_filter_info*/) {
