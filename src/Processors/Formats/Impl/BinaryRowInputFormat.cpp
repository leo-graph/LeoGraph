...
BinaryRowInputFormat<with_defaults>::BinaryRowInputFormat(ReadBuffer& in_, SharedHeader header, IRowInputFormat::Params params_,
    ...
    : RowInputFormatWithNamesAndTypes<BinaryFormatReader<with_defaults>>(
          header, in_, params_, true, with_names_, with_types_, format_settings_,
          std::make_unique<BinaryFormatReader<with_defaults>>(in_, format_settings_), false, false) {}
...
bool BinaryRowInputFormat<with_defaults>::readRow(DB::MutableColumns& columns, DB::RowReadExtension& ext) {
  ...
  if (!this->format_settings.defaults_for_omitted_fields) ext.read_columns.assign(ext.read_columns.size(), true);

  return true;
}

template <bool with_defaults>
BinaryFormatReader<with_defaults>::BinaryFormatReader(ReadBuffer& in_, const FormatSettings& format_settings_)
    : FormatWithNamesAndTypesReader(in_, format_settings_) {}
...
std::vector<String> BinaryFormatReader<with_defaults>::readTypes() {
  ...
  if (format_settings.binary.decode_types_in_binary_format) {
    ...
    for (const auto& type_name : type_names) read_data_types.push_back(DataTypeFactory::instance().get(type_name));
  }
...
template <bool with_defaults>
bool BinaryFormatReader<with_defaults>::readField(IColumn& column, const DataTypePtr& /*type*/, const SerializationPtr& serialization,
...
void BinaryFormatReader<with_defaults>::skipHeaderRow() {
  String tmp;
  for (size_t i = 0; i < read_columns; ++i) readStringBinary(tmp, *in);
}
...
void registerInputFormatRowBinary(FormatFactory& factory) {
  auto register_func = [&](const String& format_name, bool with_names, bool with_types) {
    factory.registerInputFormat(format_name,
                                [with_names, with_types](ReadBuffer& buf, const Block& sample, const IRowInputFormat::Params& params,
                                                         const FormatSettings& settings) {
                                  return std::make_shared<BinaryRowInputFormat<false>>(buf, std::make_shared<const Block>(sample), params,
                                                                                       with_names, with_types, settings);
                                });
  };
  ...
  factory.registerInputFormat("RowBinaryWithDefaults", [](ReadBuffer& buf, const Block& sample, const IRowInputFormat::Params& params,
                                                          const FormatSettings& settings) {
