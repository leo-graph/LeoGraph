...
static NamesAndTypesList getHeaderForDWARF() {
  ...
  cols[COL_ANCESTOR_TAGS] = {"ancestor_tags",
  ...
  cols[COL_RANGES] = {"ranges", std::make_shared<DataTypeArray>(std::make_shared<DataTypeTuple>(
                                    DataTypes{std::make_shared<DataTypeUInt64>(), std::make_shared<DataTypeUInt64>()}))};
  cols[COL_ATTR_NAME] = {"attr_name",
                         std::make_shared<DataTypeArray>(std::make_shared<DataTypeLowCardinality>(std::make_shared<DataTypeString>()))};
  cols[COL_ATTR_FORM] = {"attr_form",
                         std::make_shared<DataTypeArray>(std::make_shared<DataTypeLowCardinality>(std::make_shared<DataTypeString>()))};
  cols[COL_ATTR_INT] = {"attr_int", std::make_shared<DataTypeArray>(std::make_shared<DataTypeUInt64>())};
  cols[COL_ATTR_STR] = {"attr_str",
                        std::make_shared<DataTypeArray>(std::make_shared<DataTypeLowCardinality>(std::make_shared<DataTypeString>()))};
...
DWARFBlockInputFormat::UnitState::UnitState(llvm::DWARFUnit *u)
    : dwarf_unit(u), end_offset(dwarf_unit->getNextUnitOffset()), offset(dwarf_unit->getOffset() + dwarf_unit->getHeaderSize()) {
  ...
  if (err) throw Exception(ErrorCodes::CANNOT_PARSE_DWARF, "Failed to parse compilation unit entry: {}", llvm::toString(std::move(err)));
}

static llvm::StringRef removePrefix(llvm::StringRef s, size_t prefix_len) {
  if (s.size() >= prefix_len) s = llvm::StringRef(s.data() + prefix_len, s.size() - prefix_len);
  return s;
}
...
DWARFBlockInputFormat::DWARFBlockInputFormat(ReadBuffer &in_, SharedHeader header_, const FormatSettings &format_settings_,
                                             size_t num_threads_)
...
DWARFBlockInputFormat::~DWARFBlockInputFormat() { stopThreads(); }
...
void DWARFBlockInputFormat::initializeIfNeeded() {
  ...
  if (!info_section.has_value()) throw Exception(ErrorCodes::CANNOT_PARSE_ELF, "No .debug_info section");
  auto abbrev_section = elf->findSectionByName(".debug_abbrev");
  if (!abbrev_section.has_value()) throw Exception(ErrorCodes::CANNOT_PARSE_ELF, "No .debug_abbrev section");
  LOG_DEBUG(getLogger("DWARF"), ".debug_abbrev is {:.3f} MiB, .debug_info is {:.3f} MiB",
            static_cast<double>(abbrev_section->size()) * 1. / (1 << 20), static_cast<double>(info_section->size()) * 1. / (1 << 20));
  ...
  if (auto addr_section = elf->findSectionByName(".debug_addr")) debug_addr_section.emplace(addr_section->begin(), addr_section->size());
  if (auto rnglists_section = elf->findSectionByName(".debug_rnglists"))
    debug_rnglists_extractor.emplace(llvm::StringRef(rnglists_section->begin(), rnglists_section->size()), /*IsLittleEndian*/ true,
                                     /*AddressSize*/ 8);
  if (auto ranges_section = elf->findSectionByName(".debug_ranges"))
    debug_ranges_extractor.emplace(llvm::StringRef(ranges_section->begin(), ranges_section->size()), /*IsLittleEndian*/ true,
                                   /*AddressSize*/ 8);
  ...
  elf->iterateSections([&](const Elf::Section &section, size_t /*idx*/) {
    ...
    sections.try_emplace(name_without_dot, llvm::MemoryBuffer::getMemBuffer(llvm::StringRef(section.begin(), section.size()),
                                                                            /*BufferName*/ name, /*RequiresNullTerminator*/ false));
    return false;
  });
  dwarf_context = llvm::DWARFContext::create(sections, /*AddrSize*/ 8);

  for (std::unique_ptr<llvm::DWARFUnit> &unit : dwarf_context->info_section_units()) units_queue.emplace_back(unit.get());
  ...
  for (size_t i = 0; i < num_threads; ++i)
    runner.value().enqueueAndKeepTrack([this, thread_group = CurrentThread::getGroup()]() {
      try {
        ...
        while (!units_queue.empty() && !is_stopped) {
          ...
          if (!unit.eof()) units_queue.push_front(std::move(unit));
        }
      } catch (...) {
...
llvm::DWARFFormValue DWARFBlockInputFormat::parseAttribute(const llvm::DWARFAbbreviationDeclaration::AttributeSpec &attr, uint64_t *offset,
...
void DWARFBlockInputFormat::skipAttribute(const llvm::DWARFAbbreviationDeclaration::AttributeSpec &attr, uint64_t *offset,
                                          const UnitState &unit) const {
  if (attr.Form == llvm::dwarf::DW_FORM_strx3) {
    ...
    if (!llvm::DWARFFormValue::skipValue(attr.Form, *extractor, offset, unit.dwarf_unit->getFormParams()))
      throw Exception(ErrorCodes::CANNOT_PARSE_DWARF, "Failed to skip attribute {} of form {} at offset {}",
                      llvm::dwarf::AttributeString(attr.Attr).operator std::string_view(), attr.Form, *offset);
  }
}

uint64_t DWARFBlockInputFormat::parseAddress(llvm::dwarf::Attribute attr, const llvm::DWARFFormValue &val, const UnitState &unit) {
  if (val.getForm() == llvm::dwarf::DW_FORM_addr) return val.getRawUValue();
  if (val.getForm() == llvm::dwarf::DW_FORM_addrx ||
      (val.getForm() >= llvm::dwarf::DW_FORM_addrx1 && val.getForm() <= llvm::dwarf::DW_FORM_addrx4))
...
Chunk DWARFBlockInputFormat::parseEntries(UnitState &unit) {
  ...
  for (const std::string &name : header.getNames()) need[column_name_to_idx.at(name)] = true;
  ...
  while (num_rows < 65536) {
    ...
    if (need[COL_OFFSET]) col_offset->insertValue(die_offset);
    if (need[COL_ANCESTOR_TAGS]) {
      for (size_t i = unit.stack.size() - 1; i != UINT64_MAX; --i) {
        col_ancestor_tags->insertValue(unit.stack[i].tag);
        if (need[COL_ANCESTOR_OFFSETS]) col_ancestor_dwarf_offsets->insertValue(unit.stack[i].offset);
      }
    ...
    if (abbrev_code == 0) {
      if (need[COL_SIZE]) col_size->insertValue(static_cast<UInt32>(unit.offset - die_offset));
      if (need[COL_TAG]) col_tag->insertValue(0);  // "null"
      ...
      if (unit.stack.empty()) throw Exception(ErrorCodes::CANNOT_PARSE_DWARF, "Stack underflow");
      unit.stack.pop_back();
    } else {
      ...
      if (abbrev == nullptr || abbrev_code > UINT32_MAX)
        throw Exception(ErrorCodes::CANNOT_PARSE_DWARF, "Abbrev code in DIE header is out of bounds: {}, offset {}", abbrev_code,
                        unit.offset);

      auto tag = abbrev->getTag();
      if (need[COL_TAG]) col_tag->insertValue(tag);
      ...
      if (tag == llvm::dwarf::DW_TAG_compile_unit) {
        ...
        for (auto attr : abbrev->attributes()) {
          if (attr.Attr == llvm::dwarf::DW_AT_addr_base || attr.Attr == llvm::dwarf::DW_AT_rnglists_base) {
        ...
        /// May use addr_base.
        if (low_pc.has_value()) unit.base_address = parseAddress(llvm::dwarf::DW_AT_low_pc, *low_pc, unit);
      }
      ...
      for (auto attr : abbrev->attributes()) {
        ...
        if (need[COL_ATTR_NAME]) col_attr_name->insertValue(attr.Attr);
        ...
        if (need[COL_ATTR_FORM]) col_attr_form->insertValue(attr.Form);
        ...
        if (attr.Attr == llvm::dwarf::DW_AT_stmt_list && unit.filename_table == nullptr) {
          ...
          if (offset.has_value()) parseFilenameTable(unit, offset.value());
        }
        ...
        switch (val.getForm())  // (may be different from attr.Form because of DW_FORM_indirect)
        {
          ...
          case llvm::dwarf::DW_FORM_implicit_const:
                ...
                val.getRawUValue() < unit.filename_table_size)  // filename
            {
              ...
              if (attr.Attr == llvm::dwarf::DW_AT_decl_file && std::exchange(need_decl_file, false)) col_decl_file.insertIndex(idx);
              ...
              if (attr.Attr == llvm::dwarf::DW_AT_language)  // programming language
                append(col_attr_str, removePrefix(llvm::dwarf::LanguageString(static_cast<uint32_t>(val.getRawUValue())), DW_LANG_.size()));
              else if (attr.Attr == llvm::dwarf::DW_AT_encoding)  // primitive type
                append(col_attr_str,
                       removePrefix(llvm::dwarf::AttributeEncodingString(static_cast<uint32_t>(val.getRawUValue())), DW_ATE_.size()));
              else
          ...
          case llvm::dwarf::DW_FORM_GNU_strp_alt: {
            ...
            if (auto e = res.takeError())
              throw Exception(ErrorCodes::CANNOT_PARSE_DWARF, "Error parsing string attribute: {}", llvm::toString(std::move(e)));
            size_t len = strlen(*res);

            if (attr.Attr == llvm::dwarf::DW_AT_name) {
              if (std::exchange(need_name, false)) col_name->insertData(*res, len);
              if (tag == llvm::dwarf::DW_TAG_compile_unit) unit.unit_name = *res;
            }
      ...
      if (abbrev->hasChildren()) unit.stack.push_back(StackEntry{.offset = die_offset, .tag = tag});
    }
  ...
  for (const std::string &name : header.getNames()) {
    switch (column_name_to_idx.at(name)) {
      ...
      case COL_UNIT_NAME: {
        ...
        cols.push_back(ColumnLowCardinality::create(ColumnUnique<ColumnString>::create(std::move(dict), /*is_nullable*/ false), indices,
                                                    /*is_shared*/ false));
        break;
      }
      case COL_UNIT_OFFSET: {
        ...
        index->insert(1);
        auto indices = index->replicate({num_rows});
        cols.push_back(ColumnLowCardinality::create(ColumnUnique<ColumnVector<UInt64>>::create(std::move(dict), /*is_nullable*/ false),
                                                    indices, /*is_shared*/ false));
        break;
      }
      case COL_ANCESTOR_TAGS:
        cols.push_back(ColumnArray::create(
            ColumnLowCardinality::create(tag_dict_column, std::exchange(col_ancestor_tags, nullptr), /*is_shared*/ true),
            immutable_ancestor_array_offsets));
        break;
      ...
      case COL_RANGES:
        cols.push_back(ColumnArray::create(
            ColumnTuple::create(Columns{std::exchange(col_ranges_start, nullptr), std::exchange(col_ranges_end, nullptr)}),
            std::exchange(col_ranges_offsets, nullptr)));
        break;
      case COL_ATTR_NAME:
        cols.push_back(ColumnArray::create(
            ColumnLowCardinality::create(attr_name_dict_column, std::exchange(col_attr_name, nullptr), /*is_shared*/ true),
            immutable_attr_offsets));
        break;
      case COL_ATTR_FORM:
        cols.push_back(ColumnArray::create(
            ColumnLowCardinality::create(attr_form_dict_column, std::exchange(col_attr_form, nullptr), /*is_shared*/ true),
            immutable_attr_offsets));
        break;
      case COL_ATTR_INT:
...
void DWARFBlockInputFormat::parseFilenameTable(UnitState &unit, uint64_t offset) {
  ...
  auto error = prologue.parse(
      *debug_line_extractor, &offset, /*RecoverableErrorHandler*/
      [&](auto e) {
        if (++seen_debug_line_warnings < 10) LOG_INFO(getLogger("DWARF"), "Parsing error: {}", llvm::toString(std::move(e)));
      },
  ...
  if (prologue.getVersion() <= 4) append(col, "<invalid>");
...
uint64_t DWARFBlockInputFormat::fetchFromDebugAddr(uint64_t addr_base, uint64_t idx) const {
  if (!debug_addr_section.has_value()) throw Exception(ErrorCodes::CANNOT_PARSE_DWARF, "Missing .debug_addr section.");
  if (addr_base == UINT64_MAX) throw Exception(ErrorCodes::CANNOT_PARSE_DWARF, "Missing DW_AT_addr_base");
...
void DWARFBlockInputFormat::parseRanges(uint64_t offset, bool form_rnglistx, const UnitState &unit,
  ...
  if (unit.base_address != UINT64_MAX) base_addr = llvm::object::SectionedAddress{.Address = unit.base_address};

  llvm::DWARFAddressRangesVector ranges;

  if (unit.dwarf_unit->getFormParams().Version < 5) {
    if (!debug_ranges_extractor.has_value()) throw Exception(ErrorCodes::CANNOT_PARSE_DWARF, "Missing .debug_ranges section");

    llvm::DWARFDebugRangeList list;
    auto err = list.extract(*debug_ranges_extractor, &offset);
    if (err) throw Exception(ErrorCodes::CANNOT_PARSE_DWARF, "Error parsing .debug_ranges list: {}", llvm::toString(std::move(err)));

    ranges = list.getAbsoluteRanges(base_addr);
  } else {
    if (!debug_rnglists_extractor.has_value()) throw Exception(ErrorCodes::CANNOT_PARSE_DWARF, "Missing .debug_rnglists section");

    if (form_rnglistx) {
      /// Fetch offset from a table.
      if (unit.rnglists_base == UINT64_MAX) throw Exception(ErrorCodes::CANNOT_PARSE_DWARF, "Missing DW_AT_rnglists_base");
      ...
      if (lists_offset + entry_size > debug_rnglists_extractor->size())
        throw Exception(ErrorCodes::CANNOT_PARSE_DWARF, "DW_FORM_rnglistx offset out of bounds: {} vs {}", lists_offset,
                        debug_rnglists_extractor->size());
    ...
    auto err = list.extract(*debug_rnglists_extractor, /*HeaderOffset, essentially unused*/ 0, &offset, ".debug_rnglists", "rnglists");
    if (err) throw Exception(ErrorCodes::CANNOT_PARSE_DWARF, "Error parsing .debug_rnglists list: {}", llvm::toString(std::move(err)));
...
Chunk DWARFBlockInputFormat::read() {
  ...
  while (true) {
    if (is_stopped) return {};
    if (background_exception) std::rethrow_exception(background_exception);
    ...
    if (units_queue.empty() && units_in_progress == 0) return {};
...
DWARFSchemaReader::DWARFSchemaReader(ReadBuffer &in_) : ISchemaReader(in_) {}

NamesAndTypesList DWARFSchemaReader::readSchema() { return getHeaderForDWARF(); }

void registerDWARFSchemaReader(FormatFactory &factory) {
  factory.registerSchemaReader("DWARF", [](ReadBuffer &buf, const FormatSettings &) { return std::make_shared<DWARFSchemaReader>(buf); });
}

void registerInputFormatDWARF(FormatFactory &factory) {
  factory.registerRandomAccessInputFormat(
      "DWARF",
      [](ReadBuffer &buf, const Block &sample, const FormatSettings &settings, const ReadSettings &, bool /* is_remote_fs */,
         FormatParserSharedResourcesPtr parser_shared_resources, FormatFilterInfoPtr) -> InputFormatPtr {
        return std::make_shared<DWARFBlockInputFormat>(buf, std::make_shared<const Block>(sample), settings,
