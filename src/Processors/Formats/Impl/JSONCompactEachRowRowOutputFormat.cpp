...
JSONCompactEachRowRowOutputFormat::JSONCompactEachRowRowOutputFormat(WriteBuffer& out_, SharedHeader header_,
                                                                     const FormatSettings& settings_, bool with_names_, bool with_types_)
...
void JSONCompactEachRowRowOutputFormat::writeFieldDelimiter() { writeCString(", ", *ostr); }

void JSONCompactEachRowRowOutputFormat::writeRowStartDelimiter() { writeChar('[', *ostr); }

void JSONCompactEachRowRowOutputFormat::writeRowEndDelimiter() { writeCString("]\n", *ostr); }
...
void JSONCompactEachRowRowOutputFormat::writePrefix() {
  ...
  if (with_names) writeLine(JSONUtils::makeNamesValidJSONStrings(header.getNames(), settings, settings.json.validate_utf8));

  if (with_types) writeLine(JSONUtils::makeNamesValidJSONStrings(header.getDataTypeNames(), settings, settings.json.validate_utf8));
}

void JSONCompactEachRowRowOutputFormat::writeSuffix() {
  if (!exception_message.empty()) {
    if (haveWrittenData()) writeRowBetweenDelimiter();
  ...
  for (bool yield_strings : {false, true}) {
    auto register_func = [&](const String& format_name, bool with_names, bool with_types) {
      factory.registerOutputFormat(
          format_name, [yield_strings, with_names, with_types](WriteBuffer& buf, const Block& sample, const FormatSettings& format_settings,
                                                               FormatFilterInfoPtr /*format_filter_info*/) {
            FormatSettings settings = format_settings;
            settings.json.serialize_as_strings = yield_strings;

            return std::make_shared<JSONCompactEachRowRowOutputFormat>(buf, std::make_shared<const Block>(sample), settings, with_names,
                                                                       with_types);
          });
