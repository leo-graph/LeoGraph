...
JSONColumnsBlockOutputFormat::JSONColumnsBlockOutputFormat(WriteBuffer& out_, SharedHeader header_, const FormatSettings& format_settings_,
                                                           bool validate_utf8, size_t indent_)
...
void JSONColumnsBlockOutputFormat::writeChunkStart() { JSONUtils::writeObjectStart(*ostr, indent); }
...
void registerOutputFormatJSONColumns(FormatFactory& factory) {
  factory.registerOutputFormat("JSONColumns", [](WriteBuffer& buf, const Block& sample, const FormatSettings& format_settings,
                                                 FormatFilterInfoPtr /*format_filter_info*/) {
    return std::make_shared<JSONColumnsBlockOutputFormat>(buf, std::make_shared<const Block>(sample), format_settings,
                                                          format_settings.json.validate_utf8);
  });
