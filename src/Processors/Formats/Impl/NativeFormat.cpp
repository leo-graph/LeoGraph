 ...
 public:
  NativeInputFormat(ReadBuffer &buf, SharedHeader header_, const FormatSettings &settings_)
      : IInputFormat(header_, &buf),
        reader(std::make_unique<NativeReader>(buf, *header_, 0, settings_,
  ...
  void setReadBuffer(ReadBuffer &in_) override {
    reader =
        std::make_unique<NativeReader>(in_, *header, 0, settings, settings.defaults_for_omitted_fields ? &block_missing_values : nullptr);
...
class NativeOutputFormat final : public IOutputFormat {
 public:
  NativeOutputFormat(WriteBuffer &buf, SharedHeader header, const FormatSettings &settings, UInt64 client_protocol_version = 0)
      : IOutputFormat(header, buf), writer(buf, client_protocol_version, header, settings) {}
...
void registerOutputFormatNative(FormatFactory &factory) {
  factory.registerOutputFormat(
      "Native", [](WriteBuffer &buf, const Block &sample, const FormatSettings &settings, FormatFilterInfoPtr /*format_filter_info*/) {
...
void registerNativeSchemaReader(FormatFactory &factory) {
  factory.registerSchemaReader(
      "Native", [](ReadBuffer &buf, const FormatSettings &settings) { return std::make_shared<NativeSchemaReader>(buf, settings); });
}
