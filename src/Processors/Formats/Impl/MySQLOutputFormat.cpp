...
MySQLOutputFormat::MySQLOutputFormat(WriteBuffer& out_, SharedHeader header_, const FormatSettings& settings_)
  ...
  for (const auto& type : data_types) serializations.emplace_back(type->getDefaultSerialization());

  packet_endpoint = std::make_shared<MySQLProtocol::PacketEndpoint>(out, *sequence_id);
}

void MySQLOutputFormat::setContext(ContextPtr context_) { context = context_; }
...
void MySQLOutputFormat::finalizeImpl() {
  if (!use_binary_result_set) {
    ...
    if (QueryStatusPtr process_list_elem = getContext()->getProcessListElement()) {
      ...
      human_readable_info = fmt::format("Read {} rows, {} in {} sec., {} rows/sec., {}/sec.", info.read_rows, ReadableSize(info.read_bytes),
                                        elapsed_seconds, static_cast<size_t>(static_cast<double>(info.read_rows) / elapsed_seconds),
...
void MySQLOutputFormat::flushImpl() { packet_endpoint->out->next(); }

void registerOutputFormatMySQLWire(FormatFactory& factory) {
  factory.registerOutputFormat(
      "MySQLWire", [](WriteBuffer& buf, const Block& sample, const FormatSettings& settings, FormatFilterInfoPtr /*format_filter_info*/) {
        return std::make_shared<MySQLOutputFormat>(buf, std::make_shared<const Block>(sample), settings);
      });
