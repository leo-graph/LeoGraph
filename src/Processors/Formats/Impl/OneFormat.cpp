...
void registerInputFormatOne(FormatFactory &factory) {
  factory.registerInputFormat("One", [](ReadBuffer &buf, const Block &sample, const RowInputFormatParams &, const FormatSettings &) {
...
void registerOneSchemaReader(FormatFactory &factory) {
  factory.registerExternalSchemaReader("One", [](const FormatSettings &) { return std::make_shared<OneSchemaReader>(); });
}
