...
JSONColumnsBlockInputFormatBase::JSONColumnsBlockInputFormatBase(ReadBuffer& in_, SharedHeader header_,
                                                                 const FormatSettings& format_settings_,
                                                                 std::unique_ptr<JSONColumnsReaderBase> reader_)
...
size_t JSONColumnsBlockInputFormatBase::readColumn(IColumn& column, const DataTypePtr& type, const SerializationPtr& serialization,
                                                   const String& column_name) {
  /// Check for empty column.
  if (reader->checkColumnEnd()) return 0;
...
Chunk JSONColumnsBlockInputFormatBase::read() {
  ...
  if (reader->checkChunkEnd()) return Chunk(std::move(columns), 0);
  ...
  do {
    ...
    size_t columns_size =
        readColumn(*columns[column_index], fields[column_index].type, serializations[column_index], fields[column_index].name);
  ...
  if (rows <= 0) return Chunk(std::move(columns), 0);
  ...
  for (size_t i = 0; i != seen_columns.size(); ++i) {
    if (!seen_columns[i]) {
      if (format_settings.force_null_for_omitted_fields && !isNullableOrLowCardinalityNullable(fields[i].type))
        throw Exception(ErrorCodes::TYPE_MISMATCH, "Cannot insert NULL value into a column `{}` of type '{}'", fields[i].name,
                        fields[i].type->getName());
      columns[i]->insertManyDefaults(rows);
      if (format_settings.defaults_for_omitted_fields) block_missing_values.setBits(i, rows);
    }
  }
...
JSONColumnsSchemaReaderBase::JSONColumnsSchemaReaderBase(ReadBuffer& in_, const FormatSettings& format_settings_,
                                                         std::unique_ptr<JSONColumnsReaderBase> reader_)
    : ISchemaReader(in_),
      format_settings(format_settings_),
...
NamesAndTypesList JSONColumnsSchemaReaderBase::readSchema() {
  ...
  while (total_rows_read < max_rows_to_read && in.count() < max_bytes_to_read) {
    ...
    reader->readChunkStart();
    /// Check for empty block.
    if (reader->checkChunkEnd()) continue;
    ...
    do {
      ...
      if (!names_to_types.contains(column_name)) names_order.push_back(column_name);
      ...
      if (const auto it = hints.find(column_name); it != hints.end()) {
        ...
        rows_in_block = 0;
        auto column_type =
            readColumnAndGetDataType(column_name, rows_in_block, format_settings.max_rows_to_read_for_schema_inference - total_rows_read);
...
DataTypePtr JSONColumnsSchemaReaderBase::readColumnAndGetDataType(const String& column_name, size_t& rows_read, size_t max_rows) {
  /// Check for empty column.
  if (reader->checkColumnEnd()) return nullptr;
