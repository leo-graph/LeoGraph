...
void AvroInputStreamReadBufferAdapter::backup(size_t len) { in.position() -= len; }

void AvroInputStreamReadBufferAdapter::skip(size_t len) { in.tryIgnore(len); }

size_t AvroInputStreamReadBufferAdapter::byteCount() const { return in.count(); }
...
static AvroDeserializer::DeserializeFn createDecimalDeserializeFn(const avro::NodePtr &root_node, const DataTypePtr &target_type,
  ...
  size_t fixed_size = 0;
  if (is_fixed) fixed_size = root_node->fixedSize();
  const auto &decimal_type = assert_cast<const DecimalType &>(*target_type);
  if (decimal_type.getScale() != static_cast<UInt32>(logical_type.scale()) ||
      decimal_type.getPrecision() != static_cast<UInt32>(logical_type.precision()))
    throw Exception(ErrorCodes::BAD_ARGUMENTS,
                    "Cannot insert Avro decimal with scale {} and precision {} to ClickHouse type {} with scale {} and precision {}",
                    logical_type.scale(), logical_type.precision(), target_type->getName(), decimal_type.getScale(),
  ...
  return [tmp = std::vector<uint8_t>(), target_type, fixed_size](IColumn &column, avro::Decoder &decoder) mutable {
    ...
    if (tmp.size() > field_type_size || tmp.empty())
      throw Exception(ErrorCodes::VALUE_IS_OUT_OF_RANGE_OF_DATA_TYPE,
                      ...
                      target_type->getName(), field_type_size, tmp.size());
...
static std::string nodeName(avro::NodePtr node) {
  if (node->hasName()) return node->name().simpleName();
...
AvroDeserializer::DeserializeFn AvroDeserializer::createDeserializeFn(const avro::NodePtr &root_node, const DataTypePtr &target_type) {
    ...
    case avro::AVRO_BYTES:
      if (target.isUUID()) {
        return [tmp = std::string()](IColumn &column, avro::Decoder &decoder) mutable {
          ...
          if (tmp.length() != 36) throw Exception(ErrorCodes::CANNOT_PARSE_UUID, "Cannot parse uuid {}", tmp);
      ...
      if (target.isDecimal32()) return createDecimalDeserializeFn<DataTypeDecimal32>(root_node, target_type, false);
      if (target.isDecimal64()) return createDecimalDeserializeFn<DataTypeDecimal64>(root_node, target_type, false);
      if (target.isDecimal128()) return createDecimalDeserializeFn<DataTypeDecimal128>(root_node, target_type, false);
      if (target.isDecimal256()) return createDecimalDeserializeFn<DataTypeDecimal256>(root_node, target_type, false);
      if (target.isDateTime64()) return createDecimalDeserializeFn<DataTypeDateTime64>(root_node, target_type, false);
      break;
    case avro::AVRO_INT:
    ...
    case avro::AVRO_UNION: {
      ...
      if (root_node->leaves() == 2 &&
        ...
        throw Exception(ErrorCodes::BAD_ARGUMENTS,
                        ...
                        avro::toString(root_node->leafAt(non_null_union_index)->type()), target_type->getName());
      }
      ...
      if (target.isVariant()) {
        ...
        for (size_t i = 0; i != root_node->leaves(); ++i) {
          ...
          if (!corresponding_discriminator)
            throw Exception(ErrorCodes::ILLEGAL_COLUMN,
                            ...
                            variant_type.getName(), variant->getName());
        ...
        if (root_node->leaves() != nested_types.size() + (union_has_null ? 1 : 0))
          throw Exception(
              ...
              "type ({}).",
              root_node->leaves() - (union_has_null ? 1 : 0), nested_types.size());

        return [union_has_null, deserializers = std::move(nested_deserializers),
    ...
    case avro::AVRO_FIXED: {
      ...
      if (target.isUUID()) {
        return [tmp = std::vector<uint8_t>(), fixed_size](IColumn &column, avro::Decoder &decoder) mutable {
          ...
          if (tmp.size() != 36)
            throw Exception(ErrorCodes::CANNOT_PARSE_UUID,
                            "Cannot parse UUID from type Fixed, because it's size ({}) is not equal to the size of UUID (36)", fixed_size);
      ...
      if (target.isDecimal32()) return createDecimalDeserializeFn<DataTypeDecimal32>(root_node, target_type, true);
      if (target.isDecimal64()) return createDecimalDeserializeFn<DataTypeDecimal64>(root_node, target_type, true);
      if (target.isDecimal128()) return createDecimalDeserializeFn<DataTypeDecimal128>(root_node, target_type, true);
      if (target.isDecimal256()) return createDecimalDeserializeFn<DataTypeDecimal256>(root_node, target_type, true);
      if (target.isDateTime64()) return createDecimalDeserializeFn<DataTypeDateTime64>(root_node, target_type, true);
      break;
    }
    ...
    case avro::AVRO_RECORD: {
      if (target.isTuple()) {
        ...
        return [nested_deserializers](IColumn &column, avro::Decoder &decoder) {
          ...
          auto read_tuple = [&]() {
            for (const auto &[nested_deserializer, pos] : nested_deserializers) nested_deserializer(*nested_columns[pos], decoder);
          };
  ...
  throw Exception(ErrorCodes::ILLEGAL_COLUMN, "Type {} is not compatible with Avro {}:\n{}", target_type->getName(),
                  avro::toString(root_node->type()), nodeToJSON(root_node));
}
  ...
  switch (root_node->type()) {
    ...
    case avro::AVRO_RECORD: {
      ...
      return [field_skip_fns](avro::Decoder &decoder) {
        for (const auto &skip_fn : field_skip_fns) skip_fn(decoder);
      };
    }
    case avro::AVRO_SYMBOLIC: {
      ...
      return [&skip_fn = it->second](avro::Decoder &decoder) { skip_fn(decoder); };
    }
    default:
...
void AvroDeserializer::Action::deserializeNested(MutableColumns &columns, avro::Decoder &decoder, RowReadExtension &ext) const {
  ...
  for (auto &offsets : arrays_offsets) offsets->push_back(offsets->back() + total);
}

static inline std::string concatPath(const std::string &a, const std::string &b) { return a.empty() ? b : a + "." + b; }
...
AvroDeserializer::Action AvroDeserializer::createAction(const Block &header, const avro::NodePtr &node, const std::string &current_path) {
  if (node->type() == avro::AVRO_SYMBOLIC) {
    ...
    auto keep_going = std::any_of(header.begin(), header.end(),
                                  [&current_path](const ColumnWithTypeAndName &col) { return col.name.starts_with(current_path); });
    auto resolved_node = avro::resolveSymbol(node);
    if (keep_going) return createAction(header, resolved_node, current_path);
  ...
  if (header.has(current_path)) {
    ...
    if (nested_names.empty() || nested_avro_node->type() != avro::AVRO_RECORD) return AvroDeserializer::Action(createSkipFn(node));
    ...
    for (const auto &name : nested_names) {
      ...
      if (!isArray(type)) return AvroDeserializer::Action(createSkipFn(node));
    ...
    for (int i = 0; i != static_cast<int>(nested_avro_node->leaves()); ++i) {
      ...
      if (!nested_types.contains(name)) return AvroDeserializer::Action(createSkipFn(node));
...
AvroDeserializer::AvroDeserializer(DataTypePtr data_type, const std::string &column_name, avro::ValidSchema schema,
                                   bool allow_missing_fields, bool null_as_default_, const FormatSettings &settings_)
    : null_as_default(null_as_default_), settings(settings_) {
  const auto &schema_root = schema.root();
  if (schema_root->type() != avro::AVRO_RECORD) throw Exception(ErrorCodes::TYPE_MISMATCH, "Root schema must be a record");
...
void AvroDeserializer::deserializeRow(MutableColumns &columns, avro::Decoder &decoder, RowReadExtension &ext) const {
  ...
  for (size_t i = 0; i < ext.read_columns.size(); ++i) {
    ...
    if (columns[i]->size() != row) columns[i]->insertDefault();
  }
}
...
void AvroRowInputFormat::readPrefix() {
  ...
  deserializer_ptr =
      std::make_unique<AvroDeserializer>(output.getHeader(), file_reader_ptr->dataSchema(), format_settings.avro.allow_missing_fields,
                                         format_settings.null_as_default, format_settings);
...
class AvroConfluentRowInputFormat::SchemaRegistry {
 public:
  explicit SchemaRegistry(const std::string &base_url_, size_t schema_cache_max_size = 1000)
    ...
    if (base_url.empty()) throw Exception(ErrorCodes::BAD_ARGUMENTS, "Empty Schema Registry URL");
  }

  avro::ValidSchema getSchema(uint32_t id) {
    auto [schema, loaded] = schema_cache.getOrSet(id, [this, id]() { return std::make_shared<avro::ValidSchema>(fetchSchema(id)); });
    return *schema;
  }
...
static CacheBase<std::string, ConfluentSchemaRegistry> schema_registry_cache(CurrentMetrics::AvroSchemaRegistryCacheBytes,
                                                                             CurrentMetrics::AvroSchemaRegistryCacheCells,
...
static std::shared_ptr<ConfluentSchemaRegistry> getConfluentSchemaRegistry(const FormatSettings &format_settings) {
  ...
  auto [schema_registry, loaded] =
      schema_registry_cache.getOrSet(base_url, [base_url]() { return std::make_shared<ConfluentSchemaRegistry>(base_url); });
...
static uint32_t readConfluentSchemaId(ReadBuffer &in) {
  ...
  if (magic != 0x00) {
    throw Exception(ErrorCodes::INCORRECT_DATA,
                    "Invalid magic byte before AvroConfluent schema identifier. "
                    "Must be zero byte, found {} instead",
                    int(magic));
  }
...
AvroConfluentRowInputFormat::AvroConfluentRowInputFormat(SharedHeader header_, ReadBuffer &in_, Params params_,
...
const AvroDeserializer &AvroConfluentRowInputFormat::getOrCreateDeserializer(SchemaId schema_id) {
  ...
  if (it == deserializer_cache.end()) {
    ...
    AvroDeserializer deserializer(output.getHeader(), schema, format_settings.avro.allow_missing_fields, format_settings.null_as_default,
                                  format_settings);
...
NamesAndTypesList AvroSchemaReader::readSchema() {
  ...
  if (root_node->type() != avro::Type::AVRO_RECORD) throw Exception(ErrorCodes::TYPE_MISMATCH, "Root schema must be a record");
...
DataTypePtr AvroSchemaReader::avroNodeToDataType(avro::NodePtr node) {
  switch (node->type()) {
    case avro::Type::AVRO_INT: {
      if (node->logicalType().type() == avro::LogicalType::DATE) return {std::make_shared<DataTypeDate32>()};

      return {std::make_shared<DataTypeInt32>()};
    }
    case avro::Type::AVRO_LONG: {
      auto logical_type = node->logicalType();
      if (logical_type.type() == avro::LogicalType::TIMESTAMP_MILLIS) return {std::make_shared<DataTypeDateTime64>(3)};
      if (logical_type.type() == avro::LogicalType::TIMESTAMP_MICROS) return {std::make_shared<DataTypeDateTime64>(6)};
    ...
    case avro::Type::AVRO_BYTES: {
      auto logical_type = node->logicalType();
      if (logical_type.type() == avro::LogicalType::UUID) return std::make_shared<DataTypeUUID>();
    ...
    case avro::Type::AVRO_ENUM: {
      if (node->names() < 128) {
        ...
        for (int i = 0; i != static_cast<int>(node->names()); ++i) values.emplace_back(node->nameAt(i), i);
    ...
    case avro::Type::AVRO_UNION: {
      ...
      for (int i = 0; i != avro_union_size; ++i) {
        // We skip the null union type in Variant, since it is encoded using the null discriminator (implicitly all Variants can "contain
        // null").
...
void registerInputFormatAvro(FormatFactory &factory) {
  ...
  factory.registerInputFormat(
      "AvroConfluent", [](ReadBuffer &buf, const Block &sample, const RowInputFormatParams &params, const FormatSettings &settings) {
...
void registerAvroSchemaReader(FormatFactory &factory) {
  factory.registerSchemaReader(
      "Avro", [](ReadBuffer &buf, const FormatSettings &settings) { return std::make_shared<AvroSchemaReader>(buf, false, settings); });
