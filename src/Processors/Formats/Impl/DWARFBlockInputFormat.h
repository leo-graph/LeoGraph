...
class DWARFBlockInputFormat : public IInputFormat {
 ...
 private:
  ...
  llvm::DWARFFormValue parseAttribute(const llvm::DWARFAbbreviationDeclaration::AttributeSpec& attr, uint64_t* offset,
                                      const UnitState& unit) const;
  void skipAttribute(const llvm::DWARFAbbreviationDeclaration::AttributeSpec& attr, uint64_t* offset, const UnitState& unit) const;
  ...
  void parseRanges(uint64_t offset, bool form_rnglistx, const UnitState& unit, const ColumnVector<UInt64>::MutablePtr& col_ranges_start,
