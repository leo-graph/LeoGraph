...
arrow::Result<int64_t> ArrowBufferedOutputStream::Tell() const { return arrow::Result<int64_t>(total_length); }
...
RandomAccessFileFromSeekableReadBuffer::RandomAccessFileFromSeekableReadBuffer(ReadBuffer &in_, std::optional<off_t> file_size_,
                                                                               bool avoid_buffering_)
    : in{in_},
      seekable_in{dynamic_cast<SeekableReadBuffer &>(in_)},
      file_size{file_size_},
      is_open{true},
      avoid_buffering(avoid_buffering_) {}

arrow::Result<int64_t> RandomAccessFileFromSeekableReadBuffer::GetSize() {
  if (!file_size) {
    if (isBufferWithFileSize(in)) file_size = getFileSizeFromReadBuffer(in);
  }
...
arrow::Result<int64_t> RandomAccessFileFromSeekableReadBuffer::Tell() const { return seekable_in.getPosition(); }

arrow::Result<int64_t> RandomAccessFileFromSeekableReadBuffer::Read(int64_t nbytes, void *out) {
  try {
    if (avoid_buffering) in.setReadUntilPosition(seekable_in.getPosition() + nbytes);
...
arrow::Future<std::shared_ptr<arrow::Buffer>> RandomAccessFileFromSeekableReadBuffer::ReadAsync(const arrow::io::IOContext &,
...
arrow::Status ArrowInputStreamFromReadBuffer::Abort() { return arrow::Status(); }

arrow::Result<int64_t> ArrowInputStreamFromReadBuffer::Tell() const { return in.count(); }
...
RandomAccessFileFromRandomAccessReadBuffer::RandomAccessFileFromRandomAccessReadBuffer(SeekableReadBuffer &in_, size_t file_size_,
                                                                                       std::shared_ptr<ThreadPool> io_pool_)
    : in(in_), file_size(file_size_), io_pool(std::move(io_pool_)) {
  if (io_pool) async_runner = threadPoolCallbackRunnerUnsafe<void>(*io_pool, ThreadName::ARROW_FILE);
}

arrow::Result<int64_t> RandomAccessFileFromRandomAccessReadBuffer::GetSize() { return file_size; }
...
arrow::Future<std::shared_ptr<arrow::Buffer>> RandomAccessFileFromRandomAccessReadBuffer::ReadAsync(const arrow::io::IOContext &,
                                                                                                    int64_t position, int64_t nbytes) {
...
void RandomAccessFileFromRandomAccessReadBuffer::asyncThreadFunction(arrow::Future<std::shared_ptr<arrow::Buffer>> future, int64_t position,
                                                                     int64_t nbytes) {
...
arrow::Result<std::shared_ptr<arrow::Buffer>> RandomAccessFileFromRandomAccessReadBuffer::Read(int64_t) {
...
std::shared_ptr<arrow::io::RandomAccessFile> asArrowFile(ReadBuffer &in, const FormatSettings &settings, std::atomic<int> &is_cancelled,
                                                         const std::string &format_name, const std::string &magic_bytes,
                                                         bool avoid_buffering, std::shared_ptr<ThreadPool> io_pool) {
...
std::shared_ptr<arrow::io::RandomAccessFile> asArrowFileLoadIntoMemory(ReadBuffer &in, std::atomic<int> &is_cancelled,
                                                                       const std::string &format_name, const std::string &magic_bytes) {
