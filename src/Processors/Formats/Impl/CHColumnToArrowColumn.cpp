...
static PaddedPODArray<UInt8> revertNullByteMap(const PaddedPODArray<UInt8> *null_bytemap, size_t start, size_t end) {
  ...
  for (size_t i = start; i < end; ++i) res.emplace_back(!(*null_bytemap)[i]);
  return res;
}

template <typename NumericType, typename ArrowBuilderType>
static void fillArrowArrayWithNumericColumnData(ColumnPtr write_column, const PaddedPODArray<UInt8> *null_bytemap,
                                                const String &format_name, arrow::ArrayBuilder *array_builder, size_t start, size_t end) {
  ...
  if constexpr (std::is_same_v<NumericType, UInt8>)
    status = builder.AppendValues(reinterpret_cast<const uint8_t *>(internal_data.data() + start), end - start,
                                  reinterpret_cast<const uint8_t *>(arrow_null_bytemap_raw_ptr));
  else if constexpr (std::is_same_v<NumericType, Int8>)
    status = builder.AppendValues(reinterpret_cast<const int8_t *>(internal_data.data() + start), end - start,
                                  reinterpret_cast<const uint8_t *>(arrow_null_bytemap_raw_ptr));
  else
...
static void fillArrowArrayWithBoolColumnData(ColumnPtr write_column, const PaddedPODArray<UInt8> *null_bytemap, const String &format_name,
                                             arrow::ArrayBuilder *array_builder, size_t start, size_t end) {
...
static void fillArrowArrayWithDateTime64ColumnData(const DataTypePtr &type, ColumnPtr write_column,
                                                   const PaddedPODArray<UInt8> *null_bytemap, const String &format_name,
                                                   arrow::ArrayBuilder *array_builder, size_t start, size_t end) {
  ...
  if (null_bytemap) {
    for (size_t value_i = start; value_i < end; ++value_i) {
      if ((*null_bytemap)[value_i]) {
        ...
        if (need_rescale) {
          if (common::mulOverflow(value, rescale_multiplier, value)) throw Exception(ErrorCodes::DECIMAL_OVERFLOW, "Decimal math overflow");
        }
...
static void fillArrowArray(const String &column_name, ColumnPtr &column, const DataTypePtr &column_type,
                           const PaddedPODArray<UInt8> *null_bytemap, arrow::ArrayBuilder *array_builder, String format_name, size_t start,
                           size_t end, const CHColumnToArrowColumn::Settings &settings,
                           std::unordered_map<String, MutableColumnPtr> &dictionary_values);

template <typename Builder>
static void fillArrowArrayWithArrayColumnData(const String &column_name, ColumnPtr &column, const DataTypePtr &column_type,
                                              const PaddedPODArray<UInt8> *, arrow::ArrayBuilder *array_builder, String format_name,
                                              size_t start, size_t end, const CHColumnToArrowColumn::Settings &settings,
                                              std::unordered_map<String, MutableColumnPtr> &dictionary_values) {
  ...
  for (size_t array_idx = start; array_idx < end; ++array_idx) {
    ...
    /// Pass null null_map, because fillArrowArray will decide whether nested_type is nullable, if nullable, it will create a new null_map
    /// from nested_column Note that it is only needed by gluten(https://github.com/oap-project/gluten), because array type in gluten is by
    /// default nullable. And it does not influence the original ClickHouse logic, because null_map passed to
    /// fillArrowArrayWithArrayColumnData is always nullptr for ClickHouse doesn't allow nullable complex types including array type.
    fillArrowArray(column_name, nested_column, nested_type, nullptr, value_builder, format_name, offsets[array_idx - 1], offsets[array_idx],
                   settings, dictionary_values);
  }
}

static void fillArrowArrayWithTupleColumnData(const String &column_name, ColumnPtr &column, const DataTypePtr &column_type,
                                              const PaddedPODArray<UInt8> *null_bytemap, arrow::ArrayBuilder *array_builder,
                                              String format_name, size_t start, size_t end, const CHColumnToArrowColumn::Settings &settings,
                                              std::unordered_map<String, MutableColumnPtr> &dictionary_values) {
  ...
  for (size_t i = 0; i != column_tuple->tupleSize(); ++i) {
    ...
    fillArrowArray(column_name + "." + nested_names[i], nested_column, nested_types[i], null_bytemap,
                   builder.field_builder(static_cast<int>(i)), format_name, start, end, settings, dictionary_values);
  }
...
static PaddedPODArray<Int64> extractIndexesWithRemapping(ColumnPtr indexes, ColumnPtr mapping, size_t start, size_t end, bool shift) {
  ...
  PaddedPODArray<Int64> result;
  result.reserve(end - start);
  std::transform(indexes_data.begin() + start, indexes_data.begin() + end, std::back_inserter(result),
                 [&](IndexesType value) { return mapping_data[Int64(value)] - shift; });
  return result;
}
...
static void checkIfIndexesTypeIsExceeded(const std::shared_ptr<arrow::DataType> &arrow_type, size_t dict_size) {
  ...
  if ((indexes_int32_type && dict_size > INT32_MAX) || (indexes_uint32_type && dict_size > UINT32_MAX) ||
      (indexes_int64_type && dict_size > INT64_MAX))
    throw Exception(ErrorCodes::ILLEGAL_COLUMN,
                    "Cannot convert ClickHouse LowCardinality column to Arrow Dictionary column:"
                    " resulting dictionary size exceeds the max value of index type {}",
                    dict_indexes_arrow_type->name());
}

template <typename ValueType>
static void fillArrowArrayWithLowCardinalityColumnDataImpl(const String &column_name, ColumnPtr &column, const DataTypePtr &column_type,
                                                           const PaddedPODArray<UInt8> *, arrow::ArrayBuilder *array_builder,
                                                           String format_name, size_t start, size_t end,
  ...
  fillArrowArray(column_name, dict_column, dict_type, nullptr, values_builder.get(), format_name, is_nullable, dict_column->size(),
                 settings, dictionary_values);
  ...
  if (column_type->isLowCardinalityNullable()) {
    ...
    for (size_t i = start; i < end; ++i) arrow_null_bytemap.emplace_back(!column_lc->isNullAt(i));
...
static void fillArrowArrayWithLowCardinalityColumnData(const String &column_name, ColumnPtr &column, const DataTypePtr &column_type,
                                                       const PaddedPODArray<UInt8> *null_bytemap, arrow::ArrayBuilder *array_builder,
                                                       String format_name, size_t start, size_t end,
                                                       const CHColumnToArrowColumn::Settings &settings,
                                                       std::unordered_map<String, MutableColumnPtr> &dictionary_values) {
...
#  define DISPATCH(ARROW_TYPE_ID, ARROW_TYPE)                                                                                   \
    if (arrow::Type::ARROW_TYPE_ID == value_type->id()) {                                                                       \
      fillArrowArrayWithLowCardinalityColumnDataImpl<ARROW_TYPE>(column_name, column, column_type, null_bytemap, array_builder, \
                                                                 format_name, start, end, settings, dictionary_values);         \
      return;                                                                                                                   \
    }
...
static void fillArrowArrayWithStringColumnData(ColumnPtr write_column, const PaddedPODArray<UInt8> *null_bytemap, const String &format_name,
                                               arrow::ArrayBuilder *array_builder, size_t start, size_t end) {
...
static void fillArrowArrayWithFixedStringColumnData(ColumnPtr write_column, const PaddedPODArray<UInt8> *null_bytemap,
                                                    const String &format_name, arrow::ArrayBuilder *array_builder, size_t start,
                                                    size_t end) {
...
static void fillArrowArrayWithIPv6ColumnData(ColumnPtr write_column, const PaddedPODArray<UInt8> *null_bytemap, const String &format_name,
                                             arrow::ArrayBuilder *array_builder, size_t start, size_t end) {
...
static void fillArrowArrayWithIPv4ColumnData(ColumnPtr write_column, const PaddedPODArray<UInt8> *null_bytemap, const String &format_name,
                                             arrow::ArrayBuilder *array_builder, size_t start, size_t end) {
  ...
  PaddedPODArray<UInt8> arrow_null_bytemap = revertNullByteMap(null_bytemap, start, end);
  const UInt8 *arrow_null_bytemap_raw_ptr = arrow_null_bytemap.empty() ? nullptr : arrow_null_bytemap.data();
  arrow::Status status = builder.AppendValues(&(internal_data.data() + start)->toUnderType(), end - start,
                                              reinterpret_cast<const uint8_t *>(arrow_null_bytemap_raw_ptr));
  checkStatus(status, write_column->getName(), format_name);
}

static void fillArrowArrayWithDateColumnData(ColumnPtr write_column, const PaddedPODArray<UInt8> *null_bytemap, const String &format_name,
                                             arrow::ArrayBuilder *array_builder, size_t start, size_t end, bool as_uint16) {
...
static void fillArrowArrayWithDateTimeColumnData(ColumnPtr write_column, const PaddedPODArray<UInt8> *null_bytemap,
                                                 const String &format_name, arrow::ArrayBuilder *array_builder, size_t start, size_t end) {
...
static void fillArrowArrayWithDate32ColumnData(ColumnPtr write_column, const PaddedPODArray<UInt8> *null_bytemap, const String &format_name,
                                               arrow::ArrayBuilder *array_builder, size_t start, size_t end) {
...
static void fillArrowArrayWithDecimalColumnData(ColumnPtr write_column, const PaddedPODArray<UInt8> *null_bytemap,
                                                arrow::ArrayBuilder *array_builder, const String &format_name, size_t start, size_t end) {
...
static void fillArrowArrayWithBigIntegerColumnData(ColumnPtr write_column, const PaddedPODArray<UInt8> *null_bytemap,
                                                   const String &format_name, arrow::ArrayBuilder *array_builder, size_t start,
                                                   size_t end) {
  const auto &internal_column = assert_cast<const ColumnType &>(*write_column);
  const auto &internal_data = internal_column.getData();
...
static void fillArrowArray(const String &column_name, ColumnPtr &column, const DataTypePtr &column_type,
  ...
  switch (column_type->getTypeId()) {
    case TypeIndex::Nullable: {
      ...
      fillArrowArray(column_name, nested_column, nested_type, &bytemap, array_builder, format_name, start, end, settings,
                     dictionary_values);
      break;
    }
    case TypeIndex::String: {
      if (settings.output_string_as_string)
        fillArrowArrayWithStringColumnData<ColumnString, arrow::StringBuilder>(column, null_bytemap, format_name, array_builder, start,
                                                                               end);
      else
        fillArrowArrayWithStringColumnData<ColumnString, arrow::BinaryBuilder>(column, null_bytemap, format_name, array_builder, start,
                                                                               end);
      break;
    }
    case TypeIndex::FixedString: {
      if (settings.output_fixed_string_as_fixed_byte_array)
        ...
        fillArrowArrayWithStringColumnData<ColumnFixedString, arrow::StringBuilder>(column, null_bytemap, format_name, array_builder, start,
                                                                                    end);
      else
        fillArrowArrayWithStringColumnData<ColumnFixedString, arrow::BinaryBuilder>(column, null_bytemap, format_name, array_builder, start,
                                                                                    end);
      break;
    }
    case TypeIndex::IPv6:
    ...
    case TypeIndex::Array:
      fillArrowArrayWithArrayColumnData<arrow::ListBuilder>(column_name, column, column_type, null_bytemap, array_builder, format_name,
                                                            start, end, settings, dictionary_values);
      break;
    case TypeIndex::Tuple:
      fillArrowArrayWithTupleColumnData(column_name, column, column_type, null_bytemap, array_builder, format_name, start, end, settings,
                                        dictionary_values);
      break;
    case TypeIndex::LowCardinality:
      fillArrowArrayWithLowCardinalityColumnData(column_name, column, column_type, null_bytemap, array_builder, format_name, start, end,
                                                 settings, dictionary_values);
      break;
    case TypeIndex::Map: {
      ...
      fillArrowArrayWithArrayColumnData<arrow::MapBuilder>(column_name, column_array, array_type, null_bytemap, array_builder, format_name,
                                                           start, end, settings, dictionary_values);
      break;
    }
    case TypeIndex::Decimal32:
      fillArrowArrayWithDecimalColumnData<DataTypeDecimal32, Int128, arrow::Decimal128, arrow::Decimal128Builder>(
          column, null_bytemap, array_builder, format_name, start, end);
      break;
    case TypeIndex::Decimal64:
      fillArrowArrayWithDecimalColumnData<DataTypeDecimal64, Int128, arrow::Decimal128, arrow::Decimal128Builder>(
          column, null_bytemap, array_builder, format_name, start, end);
      break;
    case TypeIndex::Decimal128:
      fillArrowArrayWithDecimalColumnData<DataTypeDecimal128, Int128, arrow::Decimal128, arrow::Decimal128Builder>(
          column, null_bytemap, array_builder, format_name, start, end);
      break;
    case TypeIndex::Decimal256:
      fillArrowArrayWithDecimalColumnData<DataTypeDecimal256, Int256, arrow::Decimal256, arrow::Decimal256Builder>(
          column, null_bytemap, array_builder, format_name, start, end);
      break;
...
#  define DISPATCH(CPP_NUMERIC_TYPE, ARROW_BUILDER_TYPE)                                                                                 \
    case TypeIndex::CPP_NUMERIC_TYPE:                                                                                                    \
      fillArrowArrayWithNumericColumnData<CPP_NUMERIC_TYPE, ARROW_BUILDER_TYPE>(column, null_bytemap, format_name, array_builder, start, \
                                                                                end);                                                    \
      break;
      FOR_INTERNAL_NUMERIC_TYPES(DISPATCH)
#  undef DISPATCH
    default:
      throw Exception(ErrorCodes::UNKNOWN_TYPE, "Internal type '{}' of a column '{}' is not supported for conversion into {} data format.",
...
static std::shared_ptr<arrow::DataType> getArrowTypeForLowCardinalityIndexes(ColumnPtr indexes_column,
  ...
  switch (indexes_column->getDataType()) {
    ...
    case TypeIndex::UInt16:
    case TypeIndex::UInt32:
      if (!settings.use_64_bit_indexes_for_dictionary) return settings.use_signed_indexes_for_dictionary ? arrow::int32() : arrow::uint32();
      [[fallthrough]];
    case TypeIndex::UInt64:
    ...
    default:
      throw Exception(ErrorCodes::LOGICAL_ERROR, "Indexes column for getUniqueIndex must be ColumnUInt, got {}.",
                      indexes_column->getName());
  }
}

static arrow::TimeUnit::type getArrowTimeUnit(const DataTypeDateTime64 *type) {
  UInt32 scale = type->getScale();
  if (scale == 0) return arrow::TimeUnit::SECOND;
  if (scale > 0 && scale <= 3) return arrow::TimeUnit::MILLI;
  if (scale > 3 && scale <= 6) return arrow::TimeUnit::MICRO;
  return arrow::TimeUnit::NANO;
}

static std::shared_ptr<arrow::DataType> getArrowType(DataTypePtr column_type, ColumnPtr column, const std::string &column_name,
                                                     const std::string &format_name, const CHColumnToArrowColumn::Settings &settings,
  ...
  if (isDecimal(column_type)) {
    ...
    const auto create_arrow_type = [&](const auto &types) -> bool {
      ...
      if constexpr (std::is_same_v<ToDataType, DataTypeDecimal<Decimal32>> || std::is_same_v<ToDataType, DataTypeDecimal<Decimal64>> ||
                    std::is_same_v<ToDataType, DataTypeDecimal<Decimal128>> || std::is_same_v<ToDataType, DataTypeDecimal<Decimal256>>) {
  ...
  if (isTuple(column_type)) {
    ...
    for (size_t i = 0; i != nested_types.size(); ++i) {
      ...
      auto nested_arrow_type =
          getArrowType(nested_types[i], tuple_column->getColumnPtr(i), nested_names[i], format_name, settings, &is_field_nullable);
  ...
  if (column_type->lowCardinality()) {
    ...
    return arrow::dictionary(getArrowTypeForLowCardinalityIndexes(indexes_column, settings),
  ...
  if (isMap(column_type)) {
    ...
    return arrow::map(key_arrow_type, std::make_shared<arrow::Field>("value", val_arrow_type, is_val_nullable));
  }
  ...
  if (isStringOrFixedString(column_type) && settings.output_string_as_string) return arrow::utf8();

  if (isBool(column_type)) return arrow::boolean();

  if (isIPv6(column_type)) return arrow::fixed_size_binary(sizeof(IPv6));

  if (isIPv4(column_type)) return arrow::uint32();

  if (isDate(column_type) && settings.output_date_as_uint16) return arrow::uint16();
  ...
  throw Exception(ErrorCodes::UNKNOWN_TYPE, "The type '{}' of a column '{}' is not supported for conversion into {} data format.",
...
CHColumnToArrowColumn::CHColumnToArrowColumn(const ColumnsWithTypeAndName &header_columns_, const std::string &format_name_,
...
std::unique_ptr<CHColumnToArrowColumn> CHColumnToArrowColumn::clone(bool copy_arrow_schema) const {
  ...
  if (copy_arrow_schema) res->arrow_schema = arrow_schema;
  return res;
}

void CHColumnToArrowColumn::initializeArrowSchema(const Chunk *chunk, std::optional<size_t> columns_num,
                                                  const std::optional<std::unordered_map<String, Int64>> &column_to_field_id) {
  if (arrow_schema) return;

  if (!columns_num) columns_num = header_columns.size();
  ...
  for (size_t column_i = 0; column_i < *columns_num; ++column_i) {
    ...
    if (!settings.low_cardinality_as_dictionary) column = recursiveRemoveLowCardinality(column);

    bool is_column_nullable = false;
    auto arrow_type = getArrowType(header_column.type, column, header_column.name, format_name, settings, &is_column_nullable);
    if (column_to_field_id && column_to_field_id->contains(header_column.name)) {
      ...
      auto key_value_metadata = arrow::key_value_metadata({"PARQUET:field_id"}, {std::to_string(field_id)});
...
std::shared_ptr<arrow::Schema> CHColumnToArrowColumn::getArrowSchema() const {
  if (!arrow_schema) throw Exception(ErrorCodes::LOGICAL_ERROR, "Arrow schema is not initialized");
  return arrow_schema;
}

void CHColumnToArrowColumn::chChunkToArrowTable(std::shared_ptr<arrow::Table> &res, const std::vector<Chunk> &chunks, size_t columns_num,
                                                const std::optional<std::unordered_map<String, Int64>> &column_to_field_id) {
  ...
  for (const auto &chunk : chunks) {
    ...
    for (size_t column_i = 0; column_i < columns_num; ++column_i) {
      ...
      arrow::Status status =
          MakeBuilder(arrow::default_memory_pool(), arrow_schema->field(static_cast<int>(column_i))->type(), &array_builder);
      checkStatus(status, column->getName(), format_name);

      fillArrowArray(header_column.name, column, header_column.type, nullptr, array_builder.get(), format_name, 0, column->size(), settings,
                     dictionary_values);
