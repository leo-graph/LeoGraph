...
HiveTextRowInputFormat::HiveTextRowInputFormat(SharedHeader header_, ReadBuffer& in_, const Params& params_,
                                               const FormatSettings& format_settings_)
    : HiveTextRowInputFormat(header_, std::make_unique<PeekableReadBuffer>(in_), params_,
                             updateFormatSettings(format_settings_, *header_)) {}

HiveTextRowInputFormat::HiveTextRowInputFormat(SharedHeader header_, std::shared_ptr<PeekableReadBuffer> buf_, const Params& params_,
                                               const FormatSettings& format_settings_)
    : CSVRowInputFormat(header_, buf_, params_, true, false, format_settings_,
...
std::vector<String> HiveTextFormatReader::readNames() { return input_field_names; }
...
void registerFileSegmentationEngineHiveText(FormatFactory& factory) {
  factory.registerFileSegmentationEngineCreator("HiveText", [](const FormatSettings& settings) -> FormatFactory::FileSegmentationEngine {
