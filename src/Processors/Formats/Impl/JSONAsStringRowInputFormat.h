...
class JSONAsStringRowInputFormat final : public JSONAsRowInputFormat {
 ...
 private:
  JSONAsStringRowInputFormat(SharedHeader header_, std::unique_ptr<PeekableReadBuffer> buf_, Params params_,
...
class JSONAsStringExternalSchemaReader : public IExternalSchemaReader {
 public:
  NamesAndTypesList readSchema() override { return {{"json", std::make_shared<DataTypeString>()}}; }
};
...
class JSONAsObjectExternalSchemaReader : public IExternalSchemaReader {
 public:
  ...
  NamesAndTypesList readSchema() override { return {{"json", std::make_shared<DataTypeObject>(DataTypeObject::SchemaFormat::JSON)}}; }
