...
LineAsStringRowInputFormat::LineAsStringRowInputFormat(SharedHeader header_, ReadBuffer &in_, Params params_)
  ...
  if (header_->columns() != 1 || !typeid_cast<const ColumnString *>(header_->getByPosition(0).column.get())) {
...
void LineAsStringRowInputFormat::resetParser() { IRowInputFormat::resetParser(); }
...
void registerLineAsStringSchemaReader(FormatFactory &factory) {
  factory.registerExternalSchemaReader("LineAsString", [](const FormatSettings &) { return std::make_shared<LinaAsStringSchemaReader>(); });
}
