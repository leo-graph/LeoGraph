...
void CapnProtoOutputStream::write(const void *buffer, size_t size) { out.write(reinterpret_cast<const char *>(buffer), size); }

CapnProtoRowOutputFormat::CapnProtoRowOutputFormat(WriteBuffer &out_, SharedHeader header_, const CapnProtoSchemaInfo &info,
...
void registerOutputFormatCapnProto(FormatFactory &factory) {
  factory.registerOutputFormat("CapnProto", [](WriteBuffer &buf, const Block &sample, const FormatSettings &format_settings,
                                               FormatFilterInfoPtr /*format_filter_info*/) {
    return std::make_shared<CapnProtoRowOutputFormat>(
        buf, std::make_shared<const Block>(sample),
        CapnProtoSchemaInfo(format_settings, "CapnProto", sample, format_settings.capn_proto.use_autogenerated_schema), format_settings);
  });
