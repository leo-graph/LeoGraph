...
ArrowBlockOutputFormat::ArrowBlockOutputFormat(WriteBuffer& out_, SharedHeader header_, bool stream_,
...
void ArrowBlockOutputFormat::consume(Chunk chunk) {
  ...
  if (!writer) prepareWriter(arrow_table->schema());
...
void ArrowBlockOutputFormat::prepareWriter(const std::shared_ptr<arrow::Schema>& schema) {
  ...
  if (!writer_status.ok())
    throw Exception(ErrorCodes::UNKNOWN_EXCEPTION, "Error while opening a table writer: {}", writer_status.status().ToString());

  writer = *writer_status;
}

void registerOutputFormatArrow(FormatFactory& factory) {
  factory.registerOutputFormat("Arrow", [](WriteBuffer& buf, const Block& sample, const FormatSettings& format_settings,
  ...
  factory.registerOutputFormat("ArrowStream", [](WriteBuffer& buf, const Block& sample, const FormatSettings& format_settings,
                                                 FormatFilterInfoPtr /*format_filter_info*/) {
