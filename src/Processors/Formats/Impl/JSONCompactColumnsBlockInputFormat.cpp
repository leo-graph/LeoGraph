...
void JSONCompactColumnsReader::readChunkStart() { JSONUtils::skipArrayStart(*in); }
...
bool JSONCompactColumnsReader::checkChunkEnd() { return JSONUtils::checkAndSkipArrayEnd(*in); }

void registerInputFormatJSONCompactColumns(FormatFactory &factory) {
  factory.registerInputFormat("JSONCompactColumns",
                              [](ReadBuffer &buf, const Block &sample, const RowInputFormatParams &, const FormatSettings &settings) {
                                return std::make_shared<JSONColumnsBlockInputFormatBase>(
                                    buf, std::make_shared<const Block>(sample), settings, std::make_unique<JSONCompactColumnsReader>(buf));
                              });
}

void registerJSONCompactColumnsSchemaReader(FormatFactory &factory) {
  factory.registerSchemaReader("JSONCompactColumns", [](ReadBuffer &buf, const FormatSettings &settings) {
  ...
  factory.registerAdditionalInfoForSchemaCacheGetter("JSONCompactColumns", [](const FormatSettings &settings) {
    auto result = getAdditionalFormatInfoForAllRowBasedFormats(settings) +
