...
std::shared_ptr<arrow::io::RandomAccessFile> asArrowFile(ReadBuffer& in, const FormatSettings& settings, std::atomic<int>& is_cancelled,
                                                         ...
                                                         // If true, we'll use ReadBuffer::setReadUntilPosition() to avoid buffering and
                                                         // readahead as much as possible. For HTTP or S3 ReadBuffer, this means that each
                                                         // RandomAccessFile read call will do a new HTTP request. Used in parquet
                                                         // pre-buffered reading mode, which makes arrow do its own buffering and coalescing
                                                         // of reads. (ReadBuffer is not a good abstraction in this case, but it works.)
                                                         bool avoid_buffering = false, std::shared_ptr<ThreadPool> io_pool = nullptr);
