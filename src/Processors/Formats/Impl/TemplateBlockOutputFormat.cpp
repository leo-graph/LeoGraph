...
TemplateBlockOutputFormat::TemplateBlockOutputFormat(SharedHeader header_, WriteBuffer& out_, const FormatSettings& settings_,
  ...
  for (size_t i = 0; i < format.format_idx_to_column_idx.size(); ++i) {
    if (!format.format_idx_to_column_idx[i]) format.throwInvalidFormat("Output part name cannot be empty.", i);
      ...
      case static_cast<size_t>(ResultsetPart::RowsBeforeAggregation):
        if (format.escaping_rules[i] == EscapingRule::None)
          format.throwInvalidFormat(
              ...
              "rows_read or bytes_read is not specified",
              i);
        break;
      default:
  ...
  if (data_idx != 0) format.throwInvalidFormat("${data} must be the first output part", 0);

  /// Validate format string for rows
  if (row_format.delimiters.size() == 1) row_format.throwInvalidFormat("No columns specified", 0);
  for (size_t i = 0; i < row_format.columnsCount(); ++i) {
    if (!row_format.format_idx_to_column_idx[i]) row_format.throwInvalidFormat("Cannot skip format field for output, it's a bug.", i);
    if (header_->columns() <= *row_format.format_idx_to_column_idx[i])
      row_format.throwInvalidFormat("Column index " + std::to_string(*row_format.format_idx_to_column_idx[i]) +
                                        " must be less then number of columns (" + std::to_string(header_->columns()) + ")",
                                    i);
...
TemplateBlockOutputFormat::ResultsetPart TemplateBlockOutputFormat::stringToResultsetPart(const String& part) {
  if (part == "data") return ResultsetPart::Data;
  if (part == "totals") return ResultsetPart::Totals;
  if (part == "min") return ResultsetPart::ExtremesMin;
  if (part == "max") return ResultsetPart::ExtremesMax;
  if (part == "rows") return ResultsetPart::Rows;
  if (part == "rows_before_limit") return ResultsetPart::RowsBeforeLimit;
  if (part == "time") return ResultsetPart::TimeElapsed;
  if (part == "rows_read") return ResultsetPart::RowsRead;
  if (part == "bytes_read") return ResultsetPart::BytesRead;
  if (part == "rows_before_aggregation") return ResultsetPart::RowsBeforeAggregation;
...
void TemplateBlockOutputFormat::writeRow(const Chunk& chunk, size_t row_num) {
  ...
  for (size_t j = 0; j < columns; ++j) {
    ...
    serializeFieldByEscapingRule(*chunk.getColumns()[col_idx], *serializations[col_idx], out, row_num, row_format.escaping_rules[j],
                                 settings);
  }
...
void TemplateBlockOutputFormat::writeValue(U value, EscapingRule escaping_rule) {
...
void TemplateBlockOutputFormat::consume(Chunk chunk) {
  ...
  for (size_t i = 0; i < rows; ++i) {
    if (row_count) writeString(row_between_delimiter, out);
...
void TemplateBlockOutputFormat::writePrefix() { writeString(format.delimiters.front(), out); }
  ...
  for (size_t i = 0; i < parts; ++i) {
    ...
    switch (static_cast<ResultsetPart>(*format.format_idx_to_column_idx[i])) {
      case ResultsetPart::Totals:
        if (!statistics.totals || !statistics.totals.hasRows()) format.throwInvalidFormat("Cannot print totals for this request", i);
        writeRow(statistics.totals, 0);
        break;
      case ResultsetPart::ExtremesMin:
        if (!statistics.extremes) format.throwInvalidFormat("Cannot print extremes for this request", i);
      ...
      case ResultsetPart::RowsBeforeLimit:
        if (!statistics.applied_limit) format.throwInvalidFormat("Cannot print rows_before_limit for this request", i);
      ...
      case ResultsetPart::RowsBeforeAggregation:
        if (!statistics.applied_aggregation) format.throwInvalidFormat("Cannot print rows_before_aggregation for this request", i);
...
void registerOutputFormatTemplate(FormatFactory& factory) {
  factory.registerOutputFormat(
      "Template", [](WriteBuffer& buf, const Block& sample, const FormatSettings& settings, FormatFilterInfoPtr /*format_filter_info*/) {
        ...
        if (settings.template_settings.resultset_format.empty()) {
          ...
          if (!settings.template_settings.resultset_format_template.empty()) {
            throw Exception(DB::ErrorCodes::INVALID_TEMPLATE_FORMAT,
                            "Expected either format_template_resultset or format_template_resultset_format, but not both");
          }
        }

        ParsedTemplateFormatString row_format;
        auto idx_row_by_name = [&](const String& colName) { return sample.getPositionByName(colName); };
        if (settings.template_settings.row_format.empty()) {
          ...
          row_format = ParsedTemplateFormatString(FormatSchemaInfo(
                                                      /*format_schema_source=*/FormatSettings::FORMAT_SCHEMA_SOURCE_FILE,
          ...
          if (!settings.template_settings.row_format_template.empty()) {
            throw Exception(DB::ErrorCodes::INVALID_TEMPLATE_FORMAT,
                            "Expected either format_template_row or format_template_row_format, but not both");
          }
        }
        return std::make_shared<TemplateBlockOutputFormat>(std::make_shared<const Block>(sample), buf, settings, resultset_format,
                                                           row_format, settings.template_settings.row_between_delimiter);
      });

  factory.registerAppendSupportChecker("Template", [](const FormatSettings& settings) {
    if (settings.template_settings.resultset_format.empty()) return true;
