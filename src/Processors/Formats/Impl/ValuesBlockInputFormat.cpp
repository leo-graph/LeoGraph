...
ValuesBlockInputFormat::ValuesBlockInputFormat(ReadBuffer &in_, SharedHeader header_, const RowInputFormatParams &params_,
                                               const FormatSettings &format_settings_)
    : ValuesBlockInputFormat(std::make_unique<PeekableReadBuffer>(in_), header_, params_, format_settings_) {}

ValuesBlockInputFormat::ValuesBlockInputFormat(std::unique_ptr<PeekableReadBuffer> buf_, SharedHeader header_,
                                               const RowInputFormatParams &params_, const FormatSettings &format_settings_)
...
bool ValuesBlockInputFormat::skipToNextRow(ReadBuffer *buf, size_t min_chunk_bytes, int balance) {
  ...
  if (buf->eof() || *buf->position() == ';') return false;
  bool quoted = false;
  ...
  while (!buf->eof() && (balance || buf->count() - chunk_begin_buf_count < min_chunk_bytes)) {
    ...
    if (buf->position() == buf->buffer().end()) continue;
    if (*buf->position() == '\\') {
      ++buf->position();
...
Chunk ValuesBlockInputFormat::read() {
  ...
  for (; rows_in_block < params.max_block_size_rows; ++rows_in_block) {
    try {
      skipWhitespaceIfAny(*buf);
      if (buf->eof() || *buf->position() == ';') break;
      if (need_only_count)
        skipToNextRow(buf.get(), 1, 0);
      else
        readRow(columns, rows_in_block);
    } catch (Exception &e) {
      if (isParseError(e.code())) e.addMessage(" at row " + std::to_string(total_rows));
      throw;
    }
  }
  ...
  for (size_t i = 0; i < columns.size(); ++i) {
    if (!templates[i] || !templates[i]->rowsCount()) continue;
  ...
  for (const auto &column : columns) column->finalize();
...
void ValuesBlockInputFormat::readUntilTheEndOfRowAndReTokenize(size_t current_column_idx) {
  if (tokens && token_iterator &&
    ...
    while ((*token_iterator)->begin < buf->position() && !(*token_iterator)->isError() && !(*token_iterator)->isEnd()) ++(*token_iterator);

    if (!(*token_iterator)->isError() && !(*token_iterator)->isEnd()) return;
  }
  ...
  token_iterator.emplace(*tokens, static_cast<unsigned>(context->getSettingsRef()[Setting::max_parser_depth]),
  ...
  if (first.isError() || first.isEnd()) {
    ...
    throw Exception(ErrorCodes::SYNTAX_ERROR, "Cannot parse expression of type {} here: {}", type.getName(),
...
void ValuesBlockInputFormat::readRow(MutableColumns &columns, size_t row_num) {
  ...
  for (size_t column_idx = 0; column_idx < num_columns; ++column_idx) {
    ...
    if (!read) block_missing_values.setBit(column_idx, row_num);
...
bool ValuesBlockInputFormat::tryReadValue(IColumn &column, size_t column_idx) {
  ...
  try {
    ...
    if (!isParseError(e.code()) || decimal_overflow) throw;
    if (rollback_on_exception) column.popBack(1);
...
void tryToReplaceNullFieldsInComplexTypesWithDefaultValues(Field &value, const IDataType &data_type) {
  ...
  if (type.isTuple() && value.getType() == Field::Types::Tuple) {
    ...
    if (src_tuple_size != dst_tuple_size)
      throw Exception(ErrorCodes::TYPE_MISMATCH, "Bad size of tuple. Expected size: {}, actual size: {}.", src_tuple_size, dst_tuple_size);

    for (size_t i = 0; i < src_tuple_size; ++i) {
      const auto &element_type = *(type_tuple.getElements()[i]);

      if (tuple_value[i].isNull() && !element_type.isNullable()) tuple_value[i] = element_type.getDefault();
    ...
    for (size_t i = 0; i < map_size; ++i) {
      ...
      if (entry_key.isNull() && !key_type.isNullable()) entry_key = key_type.getDefault();

      tryToReplaceNullFieldsInComplexTypesWithDefaultValues(entry_key, key_type);

      if (entry_value.isNull() && !value_type.isNullable()) entry_value = value_type.getDefault();
...
bool ValuesBlockInputFormat::parseExpression(IColumn &column, size_t column_idx) {
  ...
  if (!(*token_iterator)->isError() && !(*token_iterator)->isEnd()) {
    ...
    ti_start = IParser::Pos(*token_iterator, static_cast<unsigned>(settings[Setting::max_parser_depth]),
  ...
  if (parser_type_for_column[column_idx] != ParserType::Streaming && dynamic_cast<const ASTLiteral *>(ast.get())) {
    ...
    try {
      ...
      rollback_on_exception = true;
      skipWhitespaceIfAny(*buf);
      if (checkDelimiterAfterValue(column_idx)) ok = true;
    } catch (const Exception &e) {
      bool decimal_overflow = e.code() == ErrorCodes::ARGUMENT_OUT_OF_BOUND;
      if (!isParseError(e.code()) || decimal_overflow) throw;
    }
  ...
  if (shouldDeduceNewTemplate(column_idx)) {
    ...
    try {
      ...
      auto structure =
          templates_cache.getFromCacheOrConstruct(result_type, !result_type->isNullable() && format_settings.null_as_default, *ti_start,
                                                  *token_iterator, ast, literal_token_map, context, &found_in_cache, delimiter);

      LOG_TEST(getLogger("ValuesBlockInputFormat"), "Will use an expression template to parse column {}: {}", column_idx,
  ...
  if (format_settings.null_as_default) tryToReplaceNullFieldsInComplexTypesWithDefaultValues(expression_value, type);
  ...
  if (value.isNull() && !type.isNullable() && !type.isLowCardinalityNullable()) {
    ...
    buf->rollbackToCheckpoint();
    throw Exception(ErrorCodes::TYPE_MISMATCH, "Cannot insert NULL value into a column of type '{}' at: {}", type.getName(),
...
void ValuesBlockInputFormat::assertDelimiterAfterValue(size_t column_idx) {
  if (unlikely(!checkDelimiterAfterValue(column_idx))) throwAtAssertionFailed((column_idx + 1 == num_columns) ? ")" : ",", *buf);
}
...
bool ValuesBlockInputFormat::checkDelimiterAfterValue(size_t column_idx) {
  ...
  if (checkChar(',', *buf)) skipWhitespaceIfAny(*buf);
  return checkChar(')', *buf);
}

bool ValuesBlockInputFormat::shouldDeduceNewTemplate(size_t column_idx) {
  if (!format_settings.values.deduce_templates_of_expressions) return false;
  ...
  double attempts_weighted = 1.5 * static_cast<double>(attempts_to_deduce_template[column_idx]) +
                             0.5 * static_cast<double>(attempts_to_deduce_template_cached[column_idx]);

  constexpr size_t max_attempts = 100;
  if (attempts_weighted < max_attempts) return true;
...
void ValuesBlockInputFormat::readSuffix() {
  if (!buf->eof() && *buf->position() == ';') {
    ++buf->position();
    skipWhitespaceIfAny(*buf);
    if (buf->hasUnreadData()) throw Exception(ErrorCodes::CANNOT_READ_ALL_DATA, "Cannot read data after semicolon");
    return;
  }
...
void ValuesBlockInputFormat::setContext(const ContextPtr &context_) { context = Context::createCopy(context_); }

void ValuesBlockInputFormat::setQueryParameters(const NameToNameMap &parameters) {
  if (parameters == context->getQueryParameters()) return;
...
void registerInputFormatValues(FormatFactory &factory) {
  factory.registerInputFormat(
      "Values", [](ReadBuffer &buf, const Block &header, const RowInputFormatParams &params, const FormatSettings &settings) {
