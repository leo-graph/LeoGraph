...
void writeNumpyNumbers(const ColumnPtr &column, WriteBuffer &buf) {
  ...
  for (size_t i = 0; i < number_column->size(); ++i) writeBinaryLittleEndian(ValueType(number_column->getElement(i)), buf);
}
...
bool NpyOutputFormat::getNumpyDataType(const DataTypePtr &type) {
  switch (type->getTypeId()) {
    ...
    case TypeIndex::FixedString:
      numpy_data_type = std::make_shared<NumpyDataTypeString>(NumpyDataType::Endianness::NONE,
                                                              assert_cast<const DataTypeFixedString *>(type.get())->getN());
      break;
...
void NpyOutputFormat::initShape(const ColumnPtr &column) {
  ...
  if (invalid_shape) throw Exception(ErrorCodes::ILLEGAL_COLUMN, "Shape ({}) is invalid, as dimension size cannot be 0", shapeStr());
}
    ...
    for (size_t i = 0; i < array_offset.size(); ++i)
      if (array_offset[i] - array_offset[i - 1] != numpy_shape[dim]) {
        ...
        throw Exception(ErrorCodes::ILLEGAL_COLUMN,
                        "ClickHouse doesn't support object types, cannot format ragged nested sequences (which is a list of arrays with "
                        "different shapes)");
      }
...
void NpyOutputFormat::updateSizeIfTypeString(const ColumnPtr &column) {
  if (nested_data_type->getTypeId() == TypeIndex::String) {
    ...
    for (size_t i = 0; i < string_offsets.size(); ++i) {
      ...
      if (numpy_data_type->getSize() < string_length) numpy_data_type->setSize(string_length);
    }
  }
}
...
void NpyOutputFormat::writeColumns() {
  for (const auto &column : columns) {
    switch (nested_data_type->getTypeId()) {
      case TypeIndex::Int8:
        writeNumpyNumbers<ColumnInt8, Int8>(column, out);
        break;
      case TypeIndex::Int16:
        writeNumpyNumbers<ColumnInt16, Int16>(column, out);
        break;
      case TypeIndex::Int32:
        writeNumpyNumbers<ColumnInt32, Int32>(column, out);
        break;
      case TypeIndex::Int64:
        writeNumpyNumbers<ColumnInt64, Int64>(column, out);
        break;
      case TypeIndex::UInt8:
        writeNumpyNumbers<ColumnUInt8, UInt8>(column, out);
        break;
      case TypeIndex::UInt16:
        writeNumpyNumbers<ColumnUInt16, UInt16>(column, out);
        break;
      case TypeIndex::UInt32:
        writeNumpyNumbers<ColumnUInt32, UInt32>(column, out);
        break;
      case TypeIndex::UInt64:
        writeNumpyNumbers<ColumnUInt64, UInt64>(column, out);
        break;
      case TypeIndex::Float32:
        writeNumpyNumbers<ColumnFloat32, Float32>(column, out);
        break;
      case TypeIndex::Float64:
        writeNumpyNumbers<ColumnFloat64, Float64>(column, out);
        break;
      case TypeIndex::FixedString:
...
void registerOutputFormatNpy(FormatFactory &factory) {
  factory.registerOutputFormat(
      "Npy", [](WriteBuffer &buf, const Block &sample, const FormatSettings &, FormatFilterInfoPtr /*format_filter_info*/) {
