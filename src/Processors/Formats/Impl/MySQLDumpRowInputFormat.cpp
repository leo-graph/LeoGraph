...
MySQLDumpRowInputFormat::MySQLDumpRowInputFormat(ReadBuffer &in_, SharedHeader header_, Params params_,
...
static String readTableName(ReadBuffer &in) {
  ...
  if (in.eof()) throw Exception(ErrorCodes::INCORRECT_DATA, "Unexpected EOF while parsing MySQLDump format: expected table name");
...
static void skipQuery(ReadBuffer &in) { readUntilEndOfQuery(in); }

static bool skipUntilRowStartedWithOneOfKeywords(const std::unordered_set<String> &keywords, ReadBuffer &in,
                                                 String *keyword_out = nullptr) {
  while (true) {
    skipWhitespaceIfAny(in);
    ...
    if (checkString("/*", in))
      ...
      if (keywords.contains(keyword)) {
        if (keyword_out) *keyword_out = keyword;
...
static NamesAndTypesList getColumnsList(const ASTExpressionList *columns_definition) {
  ...
  for (const auto &declare_column_ast : columns_definition->children) {
    ...
    if (!declare_column || !declare_column->data_type) throw Exception(ErrorCodes::UNKNOWN_TYPE, "Missing type in definition of column.");
    ...
    if (declare_column->column_options) {
      if (const auto *options = declare_column->column_options->as<MySQLParser::ASTDeclareOptions>()) {
        if (options->changes.contains("is_null")) is_nullable = options->changes.at("is_null")->as<ASTLiteral>()->value.safeGet<UInt64>();
    ...
    if (data_type_node) {
      ...
      if (is_unsigned) {
        ...
        if (type_name_upper.contains("INT") && !endsWith(type_name_upper, "SIGNED") && !endsWith(type_name_upper, "UNSIGNED"))
          data_type_node->name = type_name_upper + " UNSIGNED";
      }

      if (type_name_upper == "SET") data_type_node->resetArguments();
      ...
      if (type_name_upper == "DATE") data_type_node->name = "Date32";
    }
    if (is_nullable) data_type = makeASTDataType("Nullable", data_type);
...
void tryReadColumnNames(ReadBuffer &in, std::vector<String> *column_names) {
  ...
  while (!in.eof() && *in.position() != ')') {
    ...
    if (column_names) column_names->push_back(name);
  }
...
static MySQLQueryType skipToInsertOrCreateQuery(String &table_name, ReadBuffer &in, bool skip_create_query = false) {
  ...
  if (!skip_create_query) keywords.insert("create");

  do {
    if (!skipUntilRowStartedWithOneOfKeywords(keywords, in, &keyword)) return MySQLQueryType::NONE;
    ...
    if (keyword != "create") {
      ...
      if (!checkStringCaseInsensitive("table", in)) continue;
      type = MySQLQueryType::CREATE;
    }

    current_table_name = readTableName(in);
    if (table_name.empty()) table_name = current_table_name;
...
static bool tryToExtractStructureFromCreateQuery(ReadBuffer &in, NamesAndTypesList &structure) {
  ...
  ASTPtr query = tryParseQuery(parser, start, end, error, false, "MySQL create query", false, DBMS_DEFAULT_MAX_QUERY_SIZE,
...
static void skipEndOfInsertQueryIfNeeded(ReadBuffer &in, String &table_name) {
  ...
  if (!in.eof() && *in.position() == ';') {
    ...
    ++in.position();
    if (skipToInsertQuery(table_name, in)) skipToDataInInsertQuery(in);
  }
}
...
static void readFirstCreateAndInsertQueries(ReadBuffer &in, String &table_name, NamesAndTypesList &structure_from_create,
                                            Names &column_names) {
  ...
  if (type == MySQLQueryType::CREATE) {
    ...
    if (tryToExtractStructureFromCreateQuery(in, structure_from_create)) column_names = structure_from_create.getNames();
    skipQuery(in);
...
bool MySQLDumpRowInputFormat::readRow(MutableColumns &columns, RowReadExtension &ext) {
  ...
  for (size_t file_column = 0; file_column < column_mapping->column_indexes_for_input_fields.size(); ++file_column) {
    if (file_column != 0) skipFieldDelimiter(*in);
...
NamesAndTypesList MySQLDumpSchemaReader::readSchema() {
  ...
  if (!structure_from_create.empty()) return structure_from_create;

  if (!names.empty()) setColumnNames(names);
...
std::optional<DataTypes> MySQLDumpSchemaReader::readRowAndGetDataTypes() {
  ...
  String value;
  while (!in.eof() && *in.position() != ')') {
    if (!data_types.empty()) skipFieldDelimiter(in);
...
void registerInputFormatMySQLDump(FormatFactory &factory) {
  factory.registerInputFormat(
      "MySQLDump", [](ReadBuffer &buf, const Block &header, const RowInputFormatParams &params, const FormatSettings &settings) {
...
void registerMySQLSchemaReader(FormatFactory &factory) {
  factory.registerSchemaReader(
      "MySQLDump", [](ReadBuffer &buf, const FormatSettings &settings) { return std::make_shared<MySQLDumpSchemaReader>(buf, settings); });
