...
class ConstantExpressionTemplate : boost::noncopyable {
  struct TemplateStructure : boost::noncopyable {
    TemplateStructure(LiteralsInfo& replaced_literals, TokenIterator expression_begin, TokenIterator expression_end, ASTPtr& expr,
    ...
    static UInt64 getTemplateHash(const ASTPtr& expression, const LiteralsInfo& replaced_literals, const DataTypePtr& result_column_type,
                                  bool null_as_default, const String& salt);
 ...
 public:
  ...
  class Cache : boost::noncopyable {
   ...
   public:
    ...
    TemplateStructurePtr getFromCacheOrConstruct(const DataTypePtr& result_column_type, bool null_as_default,
                                                 TokenIterator expression_begin, TokenIterator expression_end, const ASTPtr& expression_,
                                                 const LiteralTokenMap& token_map, ContextPtr context, bool* found_in_cache = nullptr,
                                                 const String& salt = {});
  };
  ...
  bool parseExpression(ReadBuffer& istr, const TokenIterator& token_iterator, const FormatSettings& format_settings,
 ...
 private:
  bool tryParseExpression(ReadBuffer& istr, const TokenIterator& token_iterator, const FormatSettings& format_settings, size_t& cur_column,
                          const Settings& settings);
  bool parseLiteralAndAssertType(ReadBuffer& istr, const TokenIterator& token_iterator, const IDataType* type, size_t column_idx,
                                 const Settings& settings);
