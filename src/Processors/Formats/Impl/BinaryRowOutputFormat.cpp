...
BinaryRowOutputFormat::BinaryRowOutputFormat(WriteBuffer& out_, SharedHeader header, bool with_names_, bool with_types_,
...
void BinaryRowOutputFormat::writePrefix() {
  ...
  if (with_types) {
    if (format_settings.binary.encode_types_in_binary_format) {
      for (size_t i = 0; i < columns; ++i) encodeDataType(header.safeGetByPosition(i).type, out);
    } else {
      for (size_t i = 0; i < columns; ++i) writeStringBinary(header.safeGetByPosition(i).type->getName(), out);
    }
  }
}
