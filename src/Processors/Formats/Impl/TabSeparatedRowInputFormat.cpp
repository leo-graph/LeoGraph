...
TabSeparatedRowInputFormat::TabSeparatedRowInputFormat(SharedHeader header_, ReadBuffer &in_, const Params &params_, bool with_names_,
                                                       bool with_types_, bool is_raw_, const FormatSettings &format_settings_)
    : TabSeparatedRowInputFormat(header_, std::make_unique<PeekableReadBuffer>(in_), params_, with_names_, with_types_, is_raw_,
                                 format_settings_) {}

TabSeparatedRowInputFormat::TabSeparatedRowInputFormat(SharedHeader header_, std::unique_ptr<PeekableReadBuffer> in_, const Params &params_,
                                                       bool with_names_, bool with_types_, bool is_raw,
    ...
    : RowInputFormatWithNamesAndTypes(header_, *in_, params_, false, with_names_, with_types_, format_settings_,
                                      ...
                                      format_settings_.tsv.try_detect_header, format_settings_.tsv.allow_variable_number_of_columns),
      buf(std::move(in_)) {}
...
void TabSeparatedFormatReader::skipFieldDelimiter() { assertChar('\t', *buf); }
...
bool TabSeparatedFormatReader::readField(IColumn &column, const DataTypePtr &type, const SerializationPtr &serialization,
                                         bool is_last_file_column, const String & /*column_name*/) {
  ...
  if (is_raw) {
    if (as_nullable) return SerializationNullable::deserializeNullAsDefaultOrNestedTextRaw(column, *buf, format_settings, serialization);

    serialization->deserializeTextRaw(column, *buf, format_settings);
    return true;
  }

  if (as_nullable) return SerializationNullable::deserializeNullAsDefaultOrNestedTextEscaped(column, *buf, format_settings, serialization);
...
void TabSeparatedFormatReader::skipPrefixBeforeHeader() {
  for (size_t i = 0; i != format_settings.tsv.skip_first_lines; ++i) readRow();
}

void TabSeparatedRowInputFormat::syncAfterError() { skipToUnescapedNextLineOrEOF(*buf); }
...
bool TabSeparatedFormatReader::checkForSuffix() {
  if (!format_settings.tsv.skip_trailing_empty_lines) return buf->eof();
...
TabSeparatedSchemaReader::TabSeparatedSchemaReader(ReadBuffer &in_, bool with_names_, bool with_types_, bool is_raw_,
                                                   const FormatSettings &format_settings_)
    : FormatWithNamesAndTypesSchemaReader(
          buf, format_settings_, with_names_, with_types_, &reader,
  ...
  for (bool is_raw : {false, true}) {
    auto register_func = [&](const String &format_name, bool with_names, bool with_types) {
      factory.registerInputFormat(
          format_name, [with_names, with_types, is_raw](ReadBuffer &buf, const Block &sample, IRowInputFormat::Params params,
                                                        const FormatSettings &settings) {
            return std::make_shared<TabSeparatedRowInputFormat>(std::make_shared<const Block>(sample), buf, std::move(params), with_names,
    ...
    if (is_raw) registerWithNamesAndTypes("Raw", register_func);
  }
}
    ...
    auto register_func = [&](const String &format_name, bool with_names, bool with_types) {
      ...
      if (!with_types) {
        factory.registerAdditionalInfoForSchemaCacheGetter(format_name, [with_names, is_raw](const FormatSettings &settings) {
          ...
          if (!with_names)
            result +=
                fmt::format(", column_names_for_schema_inference={}, try_detect_header={}, skip_first_lines={}",
                            settings.column_names_for_schema_inference, settings.tsv.try_detect_header, settings.tsv.skip_first_lines);
          return result;
        });
      }
    };
...
static std::pair<bool, size_t> fileSegmentationEngineTabSeparatedImpl(ReadBuffer &in, DB::Memory<> &memory, bool is_raw, size_t min_bytes,
  ...
  for (bool is_raw : {false, true}) {
    auto register_func = [&](const String &format_name, bool, bool) {
      ...
      factory.registerFileSegmentationEngine(
          format_name, [is_raw](ReadBuffer &in, DB::Memory<> &memory, size_t min_bytes, size_t max_rows) {
    ...
    if (is_raw) markFormatWithNamesAndTypesSupportsSamplingColumns("Raw", factory);
  }
