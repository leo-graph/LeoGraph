...
VerticalRowOutputFormat::VerticalRowOutputFormat(WriteBuffer& out_, SharedHeader header_, const FormatSettings& format_settings_)
  ...
  for (size_t i = 0; i < columns; ++i) {
    ...
    auto [name_cut, width] = truncateName(name, format_settings.pretty.max_column_name_width_cut_to,
...
void VerticalRowOutputFormat::writeField(const IColumn& column, const ISerialization& serialization, size_t row_num) {
  if (row_number > format_settings.pretty.max_rows) return;
...
void VerticalRowOutputFormat::writeValue(const IColumn& column, const ISerialization& serialization, const size_t row_num) const {
  ...
  /// If we need highlighting.
  else if (color && ((format_settings.pretty.highlight_digit_groups && is_number[field_number]) ||
    ...
    if (format_settings.pretty.highlight_digit_groups && is_number[field_number]) serialized_value = highlightDigitGroups(serialized_value);

    /// Highlight trailing spaces.
    if (format_settings.pretty.highlight_trailing_spaces) serialized_value = highlightTrailingSpaces(serialized_value);
  ...
  /// Write a tip.
  if (is_number[field_number]) writeReadableNumberTip(out, column, row_num, format_settings, color);
}
...
void VerticalRowOutputFormat::writeRowStartDelimiter() {
  ...
  for (size_t i = 0; i < width; ++i) writeCString("─", out);
  writeChar('\n', out);
}
...
void VerticalRowOutputFormat::writeBeforeExtremes() {
  if (!areTotalsWritten()) writeCString("\n", out);

  writeCString("\n", out);
}

void VerticalRowOutputFormat::writeMinExtreme(const Columns& columns, size_t row_num) { writeSpecialRow(columns, row_num, "Min"); }

void VerticalRowOutputFormat::writeMaxExtreme(const Columns& columns, size_t row_num) { writeSpecialRow(columns, row_num, "Max"); }

void VerticalRowOutputFormat::writeTotals(const Columns& columns, size_t row_num) { writeSpecialRow(columns, row_num, "Totals"); }
...
void VerticalRowOutputFormat::writeSpecialRow(const Columns& columns, size_t row_num, const char* title) {
  ...
  for (size_t i = 0; i < columns_size; ++i) writeField(*columns[i], *serializations[i], row_num);
}

void registerOutputFormatVertical(FormatFactory& factory) {
  factory.registerOutputFormat(
      "Vertical", [](WriteBuffer& buf, const Block& sample, const FormatSettings& settings, FormatFilterInfoPtr /*format_filter_info*/) {
