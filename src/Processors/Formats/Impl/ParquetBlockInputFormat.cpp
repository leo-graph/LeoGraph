...
String removeListElement(const String &value) {
  ...
  String result = value;
  size_t pos;
  while ((pos = result.find(pattern)) != std::string::npos) result.erase(pos, pattern.size());
  return result;
}

void traverseAllFields(const parquet::schema::NodePtr &node, std::unordered_map<Int64, String> &fields_mapping,
...
static Field decodePlainParquetValueSlow(const std::string &data, parquet::Type::type physical_type, const parquet::ColumnDescriptor &descr,
  ...
  auto decode_integer = [&](bool signed_) -> UInt64 {
    ...
    if (data.size() != size) throw Exception(ErrorCodes::CANNOT_PARSE_NUMBER, "Unexpected size: {}", data.size());

    UInt64 val = 0;
    memcpy(&val, data.data(), size);

    /// Sign-extend.
    if (signed_ && size < 8 && (val >> (size * 8 - 1)) != 0) val |= 0 - (1ul << (size * 8));
  ...
  do  // while (false)
  {
    ...
    Int256 val = 0;
    memcpy(&val, data.data(), size);
    if (big_endian) std::reverse(reinterpret_cast<char *>(&val), reinterpret_cast<char *>(&val) + size);
    /// Sign-extend.
    if (size < 32 && (val >> (size * 8 - 1)) != 0) val |= ~((Int256(1) << (size * 8)) - 1);

    auto narrow = [&](auto x) -> Field {
      memcpy(&x, &val, sizeof(x));
      return Field(DecimalField<decltype(x)>(x, static_cast<UInt32>(scale)));
    };
    if (size <= 4) return narrow(Decimal32(0));
    if (size <= 8) return narrow(Decimal64(0));
    if (size <= 16) return narrow(Decimal128(0));
  ...
  /// Timestamp (decimal).
  {
    ...
    if (descr.logical_type() && (descr.logical_type()->is_time() || descr.logical_type()->is_timestamp())) {
      ...
      switch (unit) {
        case LogicalType::TimeUnit::unit::MILLIS:
          scale = 3;
          break;
        case LogicalType::TimeUnit::unit::MICROS:
          scale = 6;
          break;
        case LogicalType::TimeUnit::unit::NANOS:
          scale = 9;
          break;
        default:
      ...
      switch (descr.converted_type()) {
        case ConvertedType::type::TIME_MILLIS:
          scale = 3;
          break;
        case ConvertedType::type::TIME_MICROS:
          scale = 6;
          break;
        case ConvertedType::type::TIMESTAMP_MILLIS:
          scale = 3;
          break;
        case ConvertedType::type::TIMESTAMP_MICROS:
          scale = 6;
          break;
        default:
          is_timestamp = false;
  ...
  if (physical_type == Type::type::FLOAT) {
    if (data.size() != 4) throw Exception(ErrorCodes::CANNOT_PARSE_NUMBER, "Unexpected float size");
    Float32 val;
  ...
  if (physical_type == Type::type::DOUBLE) {
    if (data.size() != 8) throw Exception(ErrorCodes::CANNOT_PARSE_NUMBER, "Unexpected float size");
    Float64 val;
    memcpy(&val, data.data(), data.size());
    return Field(val);
  }
  ...
  if (physical_type == Type::type::BYTE_ARRAY || physical_type == Type::type::FIXED_LEN_BYTE_ARRAY) {
    ...
    ///
    ///   insert into function file('t.parquet') select arrayStringConcat(range(number*1000000)) from numbers(2) settings
    ///   output_format_parquet_use_custom_encoder=0 select tupleElement(tupleElement(row_groups[1], 'columns')[1], 'statistics') from
    ///   file('t.parquet', ParquetMetadata)
    ///
  ...
  if (physical_type == Type::type::INT96) return Field();
  ...
  if (descr.logical_type() && descr.logical_type()->is_int())
    ...
    signed_ = descr.converted_type() != ConvertedType::type::UINT_8 && descr.converted_type() != ConvertedType::type::UINT_16 &&
              descr.converted_type() != ConvertedType::type::UINT_32 && descr.converted_type() != ConvertedType::type::UINT_64;
...
struct ParquetBloomFilter final : public KeyCondition::BloomFilter {
  explicit ParquetBloomFilter(std::unique_ptr<parquet::BloomFilter> &&parquet_bf_) : parquet_bf(std::move(parquet_bf_)) {}
...
static KeyCondition::ColumnIndexToBloomFilter buildColumnIndexToBF(
    parquet::BloomFilterReader &bf_reader, int row_group,
...
static std::vector<Range> getHyperrectangleForRowGroup(const parquet::FileMetaData &file, int row_group_idx, const Block &header,
                                                       const FormatSettings &format_settings) {
  auto column_name_for_lookup = [&](std::string column_name) -> std::string {
    if (format_settings.parquet.case_insensitive_column_matching) boost::to_lower(column_name);
    return column_name;
  };
  ...
  for (int i = 0; i < row_group->num_columns(); ++i) {
    ...
    if (s->descr()->schema_node()->is_repeated()) continue;

    auto path = c->path_in_schema()->ToDotVector();
    if (path.size() != 1) continue;  // compound types not supported
  ...
  for (size_t idx = 0; idx < header.columns(); ++idx) {
    ...
    if (it == name_to_statistics.end()) continue;
    auto stats = it->second;

    DataTypePtr type = header.getByPosition(idx).type;
    if (type->lowCardinality()) type = assert_cast<const DataTypeLowCardinality &>(*type).getDictionaryType();
    if (type->isNullable()) type = assert_cast<const DataTypeNullable &>(*type).getNestedType();
    ...
    if (stats->HasMinMax()) {
      try {
        ...
        auto ok_cast = [&](Field::Types::Which from, Field::Types::Which to) -> bool {
          ...
          if (Field::isDecimal(from) || Field::isDecimal(to)) return Field::isDecimal(from) && Field::isDecimal(to) && to >= from;
          /// Integer -> IP.
          if (to == Field::Types::IPv4) return from == Field::Types::UInt64;
          /// Disable index for everything else, especially string <-> number.
          return false;
        };
        if (!(ok_cast(min_type, min.getType()) && ok_cast(max_type, max.getType())) && !(min == max) &&
        ...
        e.addMessage(
            " (When parsing Parquet statistics for column {}, physical type {}, {}. Please report an issue and use "
            "input_format_parquet_filter_push_down = false to work around.)",
            name, static_cast<int>(stats->physical_type()), stats->descr()->ToString());
        throw;
      }
    }
    ...
    bool always_null = stats->descr()->max_definition_level() != 0 && stats->HasNullCount() && stats->num_values() == 0;
    bool can_be_null = stats->descr()->max_definition_level() != 0 && (!stats->HasNullCount() || stats->null_count() != 0);
    ...
    if (can_be_null) {
      if (null_as_default) {
        ...
        if (!min.isNull() && accurateLess(default_value, min)) min = default_value;
        if (!max.isNull() && accurateLess(max, default_value)) max = default_value;
      } else {
        /// Make sure the range reaches infinity on at least one side.
        if (!min.isNull() && !max.isNull()) min = Field();
      }
    } else {
      /// If the column doesn't have nulls, exclude both infinities.
      if (min.isNull()) hyperrectangle[idx].left_included = false;
      if (max.isNull()) hyperrectangle[idx].right_included = false;
    }

    if (!min.isNull()) hyperrectangle[idx].left = std::move(min);
    if (!max.isNull()) hyperrectangle[idx].right = std::move(max);
  }
...
void ParquetFileBucketInfo::serialize(WriteBuffer &buffer) {
  ...
  for (auto chunk : row_group_ids) writeVarUInt(chunk, buffer);
}
...
String ParquetFileBucketInfo::getIdentifier() const {
  String result;
  for (auto chunk : row_group_ids) result += "_" + std::to_string(chunk);
  return result;
}

ParquetFileBucketInfo::ParquetFileBucketInfo(const std::vector<size_t> &row_group_ids_) : row_group_ids(row_group_ids_) {}
...
ParquetBlockInputFormat::ParquetBlockInputFormat(ReadBuffer &buf, SharedHeader header_, const FormatSettings &format_settings_,
                                                 FormatParserSharedResourcesPtr parser_shared_resources_,
                                                 FormatFilterInfoPtr format_filter_info_, size_t min_bytes_for_seek_)
  ...
  if (parser_shared_resources->max_parsing_threads > 1)
    pool = std::make_unique<ThreadPool>(CurrentMetrics::FormatParsingThreads, CurrentMetrics::FormatParsingThreadsActive,
                                        CurrentMetrics::FormatParsingThreadsScheduled, parser_shared_resources->max_parsing_threads);
  ...
  if (row_group_prefetch)
    io_pool = std::make_shared<ThreadPool>(CurrentMetrics::IOThreads, CurrentMetrics::IOThreadsActive, CurrentMetrics::IOThreadsScheduled,
...
ParquetBlockInputFormat::~ParquetBlockInputFormat() {
  is_stopped = true;
  if (pool) pool->wait();
  if (io_pool) io_pool->wait();
}

void ParquetBlockInputFormat::initializeIfNeeded() {
  if (std::exchange(is_initialized, true)) return;
  ...
  if (buckets_to_read) {
    ...
    for (int i = 0; i < metadata->num_row_groups(); ++i) {
      if (!set_to_read.contains(i)) skip_row_groups.insert(i);
    }
  }
  ...
  if (format_filter_info && format_filter_info->column_mapper) {
    ...
    for (int i = 0; i < group_node->field_count(); ++i) traverseAllFields(group_node->field(i), parquet_field_ids);
  ...
  auto adaptive_chunk_size = [&](int row_group_idx) -> size_t {
    ...
    const size_t preferred_num_rows =
        static_cast<size_t>(floor(static_cast<double>(format_settings.parquet.prefer_block_bytes) / average_row_bytes));
  ...
  if (format_filter_info && format_filter_info->key_condition) {
    ...
    if (format_settings.parquet.bloom_filter_push_down) {
      ...
      auto hash_one = [&](size_t column_idx, const Field &f) -> std::optional<uint64_t> {
        const auto *parquet_column_descriptor = getColumnDescriptorIfBloomFilterIsPresent(metadata->RowGroup(0), index_mapping, column_idx);
  ...
  auto skip_row_group_based_on_filters = [&](int row_group) {
    ...
    bool maybe_exists = key_condition_with_bloom_filter_data
                            ->checkInHyperrectangle(hyperrectangle, getPort().getHeader().getDataTypes(), column_index_to_bloom_filter)
                            .can_be_true;
...
void ParquetBlockInputFormat::initializeRowGroupBatchReader(size_t row_group_batch_idx) {
  ...
  if (row_group_prefetch) {
    row_group_batch.prefetch_iterator =
        std::make_unique<RowGroupPrefetchIterator>(parquet_file_reader, row_group_batch, column_indices, min_bytes_for_seek);
    row_group_batch.record_batch_reader = row_group_batch.prefetch_iterator->nextRowGroupReader();
  } else {
    Stopwatch fetch_wait_time;
    THROW_ARROW_NOT_OK(row_group_batch.file_reader->GetRecordBatchReader(row_group_batch.row_groups_idxs, column_indices,
  ...
  row_group_batch.arrow_column_to_ch_column = std::make_unique<ArrowColumnToCHColumn>(
      getPort().getHeader(), "Parquet", format_settings, parquet_names_to_clickhouse, clickhouse_names_to_parquet,
      format_settings.parquet.allow_missing_columns, format_settings.null_as_default, format_settings.date_time_overflow_behavior,
      format_settings.parquet.allow_geoparquet_parser, format_settings.parquet.case_insensitive_column_matching, false, /* is_stream_ */
...
void ParquetBlockInputFormat::scheduleRowGroup(size_t row_group_batch_idx) {
  ...
  pool->scheduleOrThrowOnError([this, row_group_batch_idx, thread_group = CurrentThread::getGroup()]() {
    try {
...
void ParquetBlockInputFormat::threadFunction(size_t row_group_batch_idx) {
  ...
  while (true) {
    ...
    if (row_group_batch.status == RowGroupBatchState::Status::Done) return;
...
void ParquetBlockInputFormat::RowGroupPrefetchIterator::prefetchNextRowGroups() {
  if (next_row_group_idx < row_group_batch.row_groups_idxs.size()) {
    ...
    file_reader->PreBuffer(prefetched_row_groups, column_indices, row_group_batch.file_reader->properties().io_context(),
                           row_group_batch.file_reader->properties().cache_options());
  }
}
...
void ParquetBlockInputFormat::decodeOneChunk(size_t row_group_batch_idx, std::unique_lock<std::mutex> &lock) {
  ...
  auto get_approx_original_chunk_size = [&](size_t num_rows) {
    return static_cast<size_t>(std::ceil(static_cast<double>(row_group_batch.total_bytes_compressed) /
                                         static_cast<double>(row_group_batch.total_rows) * static_cast<double>(num_rows)));
  };

  if (!row_group_batch.record_batch_reader) initializeRowGroupBatchReader(row_group_batch_idx);
  ...
  auto fetch_batch = [&] {
    ...
    if (!batch.ok()) throw Exception(ErrorCodes::CANNOT_READ_ALL_DATA, "Error while reading Parquet data: {}", batch.status().ToString());
    return batch;
  };
...
void ParquetBlockInputFormat::scheduleMoreWorkIfNeeded(std::optional<size_t> row_group_batch_touched) {
  while (row_group_batches_completed < row_group_batches.size()) {
    ...
    if (row_group.status != RowGroupBatchState::Status::Done || row_group.num_pending_chunks != 0) break;
  ...
  if (pool) {
    ...
    if (row_group_batch_touched) {
      ...
      if (row_group.status == RowGroupBatchState::Status::Paused && row_group.num_pending_chunks < max_pending_chunks_per_row_group_batch)
...
Chunk ParquetBlockInputFormat::read() {
  ...
  if (is_stopped || row_group_batches_completed == row_group_batches.size()) return {};

  if (need_only_count) {
    auto chunk = getChunkForCount(row_group_batches[row_group_batches_completed].total_rows);
    size_t total_rows_before = std::accumulate(row_group_batches_skipped_rows.begin(),
                                               row_group_batches_skipped_rows.begin() + row_group_batches_completed + 1, 0ull) +
                               std::accumulate(row_group_batches.begin(), row_group_batches.begin() + row_group_batches_completed, 0ull,
  ...
  while (true) {
    ...
    if (!pending_chunks.empty() &&
        (!format_settings.parquet.preserve_order || pending_chunks.top().row_group_batch_idx == row_group_batches_completed)) {
      ...
      size_t total_rows_before = std::accumulate(row_group_batches_skipped_rows.begin(),
                                                 row_group_batches_skipped_rows.begin() + chunk.row_group_batch_idx + 1, 0ull) +
                                 std::accumulate(row_group_batches.begin(), row_group_batches.begin() + chunk.row_group_batch_idx, 0ull,
                                                 [](size_t sum, const RowGroupBatchState &batch) { return sum + batch.total_rows; })
    ...
    if (row_group_batches_completed == row_group_batches.size()) return {};
...
void ParquetBlockInputFormat::resetParser() {
  ...
  while (!pending_chunks.empty()) pending_chunks.pop();
...
const BlockMissingValues *ParquetBlockInputFormat::getMissingValues() const { return &previous_block_missing_values; }
...
NamesAndTypesList ArrowParquetSchemaReader::readSchema() {
  ...
  auto header = ArrowColumnToCHColumn::arrowSchemaToCHHeader(
      *schema, metadata->key_value_metadata(), "Parquet", format_settings,
      format_settings.parquet.skip_columns_with_unsupported_types_in_schema_inference,
      format_settings.schema_inference_make_columns_nullable != 0, format_settings.parquet.case_insensitive_column_matching,
      format_settings.parquet.allow_geoparquet_parser, format_settings.parquet.enable_json_parsing);
  if (format_settings.schema_inference_make_columns_nullable == 1) return getNamesAndRecursivelyNullableTypes(header, format_settings);
...
std::vector<FileBucketInfoPtr> ParquetBucketSplitter::splitToBuckets(size_t bucket_size, ReadBuffer &buf,
  ...
  for (int i = 0; i < metadata->num_row_groups(); ++i) bucket_sizes.push_back(metadata->RowGroup(i)->total_byte_size());

  std::vector<std::vector<size_t>> buckets;
  size_t current_weight = 0;
  for (size_t i = 0; i < bucket_sizes.size(); ++i) {
    if (current_weight + bucket_sizes[i] <= bucket_size) {
      if (buckets.empty()) buckets.emplace_back();
...
void registerInputFormatParquet(FormatFactory &factory) {
  factory.registerFileBucketInfo("Parquet", [] { return std::make_shared<ParquetFileBucketInfo>(); });
  factory.registerRandomAccessInputFormatWithMetadata(
      "Parquet",
      [](ReadBuffer &buf, const Block &sample, const FormatSettings &settings, const ReadSettings &read_settings, bool is_remote_fs,
        ...
        if (settings.parquet.use_native_reader_v3) {
          ...
          return std::make_shared<ParquetV3BlockInputFormat>(buf, std::make_shared<const Block>(sample), settings,
                                                             std::move(parser_shared_resources), std::move(format_filter_info),
                                                             min_bytes_for_seek, metadata_cache, object_with_metadata);
        } else {
  ...
  factory.registerRandomAccessInputFormat(
      "Parquet",
      [](ReadBuffer &buf, const Block &sample, const FormatSettings &settings, const ReadSettings &read_settings, bool is_remote_fs,
         FormatParserSharedResourcesPtr parser_shared_resources, FormatFilterInfoPtr format_filter_info) -> InputFormatPtr {
        auto lambda_logger = getLogger("ParquetMetadataCache");
        size_t min_bytes_for_seek =
            is_remote_fs ? read_settings.remote_read_min_bytes_for_seek : settings.parquet.local_read_min_bytes_for_seek;
        if (settings.parquet.use_native_reader_v3) {
          ...
          return std::make_shared<ParquetV3BlockInputFormat>(buf, std::make_shared<const Block>(sample), settings,
                                                             std::move(parser_shared_resources), std::move(format_filter_info),
                                                             min_bytes_for_seek, nullptr, std::nullopt);
        } else {
          LOG_TRACE(lambda_logger, "using arrow reader in ParquetBlockInputFormat without metadata cache");
          return std::make_shared<ParquetBlockInputFormat>(buf, std::make_shared<const Block>(sample), settings,
                                                           std::move(parser_shared_resources), std::move(format_filter_info),
                                                           min_bytes_for_seek);
        }
      });
  factory.markFormatSupportsSubsetOfColumns("Parquet");
  factory.registerPrewhereSupportChecker("Parquet", [](const FormatSettings &settings) { return settings.parquet.use_native_reader_v3; });
}

void registerParquetSchemaReader(FormatFactory &factory) {
  factory.registerSplitter("Parquet", [] { return std::make_shared<ParquetBucketSplitter>(); });
  factory.registerSchemaReader("Parquet", [](ReadBuffer &buf, const FormatSettings &settings) -> SchemaReaderPtr {
    auto lambda_logger = getLogger("ParquetMetadataCache");
    if (settings.parquet.use_native_reader_v3) {
  ...
  factory.registerAdditionalInfoForSchemaCacheGetter("Parquet", [](const FormatSettings &settings) {
    return fmt::format("schema_inference_make_columns_nullable={};enable_json_parsing={};use_native_reader_v3={}",
                       settings.schema_inference_make_columns_nullable, settings.parquet.enable_json_parsing,
