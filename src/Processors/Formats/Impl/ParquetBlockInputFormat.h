...
class ParquetBlockInputFormat : public IInputFormat {
 public:
  ParquetBlockInputFormat(ReadBuffer& buf, SharedHeader header, const FormatSettings& format_settings_,
 ...
 private:
    ...
    struct Compare {
      ...
      bool operator()(const PendingChunk& a, const PendingChunk& b) const {
        auto tuplificate = [this](const PendingChunk& c) {
          return row_group_first ? std::tie(c.row_group_batch_idx, c.chunk_idx) : std::tie(c.chunk_idx, c.row_group_batch_idx);
        };
  ...
  class RowGroupPrefetchIterator {
   public:
    RowGroupPrefetchIterator(parquet::ParquetFileReader* file_reader_, RowGroupBatchState& row_group_batch_,
                             const std::vector<int>& column_indices_, size_t min_bytes_for_seek_)
        : file_reader(file_reader_),
          row_group_batch(row_group_batch_),
