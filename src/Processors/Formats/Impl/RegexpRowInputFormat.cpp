...
RegexpFieldExtractor::RegexpFieldExtractor(const FormatSettings &format_settings)
    : regexp_str(format_settings.regexp.regexp), regexp(regexp_str), skip_unmatched(format_settings.regexp.skip_unmatched) {
  if (regexp_str.empty())
    throw Exception(
        ErrorCodes::BAD_ARGUMENTS,
        "The regular expression is not set for the `Regexp` format. It requires setting the value of the `format_regexp` setting.");
...
bool RegexpFieldExtractor::parseRow(PeekableReadBuffer &buf) {
  ...
  if (line_size > 0 && buf.position()[line_size - 1] == '\r') --line_to_match;

  bool match = re2::RE2::FullMatchN(re2::StringPiece(buf.position(), line_to_match), regexp, re2_arguments_ptrs.data(),
                                    static_cast<int>(re2_arguments_ptrs.size()));

  if (!match && !skip_unmatched)
    throw Exception(ErrorCodes::INCORRECT_DATA, "Line \"{}\" doesn't match the regexp: `{}`", std::string(buf.position(), line_to_match),
                    regexp_str);

  buf.position() += line_size;
  if (!buf.eof() && !checkChar('\n', buf)) throw Exception(ErrorCodes::LOGICAL_ERROR, "No \\n at the end of line.");

  return match;
}

RegexpRowInputFormat::RegexpRowInputFormat(ReadBuffer &in_, SharedHeader header_, Params params_, const FormatSettings &format_settings_)
    : RegexpRowInputFormat(std::make_unique<PeekableReadBuffer>(in_), header_, params_, format_settings_) {}

RegexpRowInputFormat::RegexpRowInputFormat(std::unique_ptr<PeekableReadBuffer> buf_, SharedHeader header_, Params params_,
...
bool RegexpRowInputFormat::readRow(MutableColumns &columns, RowReadExtension &ext) {
  ...
  if (field_extractor.parseRow(*buf)) readFieldsFromMatch(columns, ext);
  return true;
}

RegexpSchemaReader::RegexpSchemaReader(ReadBuffer &in_, const FormatSettings &format_settings_)
    : IRowSchemaReader(buf, format_settings_, getDefaultDataTypeForEscapingRule(format_settings_.regexp.escaping_rule)),
...
std::optional<DataTypes> RegexpSchemaReader::readRowAndGetDataTypes() {
  ...
  for (size_t i = 0; i != field_extractor.getMatchedFieldsSize(); ++i) {
    ...
    data_types.push_back(
        tryInferDataTypeByEscapingRule(field, format_settings, format_settings.regexp.escaping_rule, &json_inference_info));
  }
...
void registerInputFormatRegexp(FormatFactory &factory) {
  factory.registerInputFormat(
      "Regexp", [](ReadBuffer &buf, const Block &sample, IRowInputFormat::Params params, const FormatSettings &settings) {
...
static std::pair<bool, size_t> segmentationEngine(ReadBuffer &in, DB::Memory<> &memory, size_t min_bytes, size_t max_rows) {
  ...
  while (loadAtPosition(in, memory, pos) && need_more_data) {
    ...
    ++number_of_rows;
    if ((memory.size() + static_cast<size_t>(pos - in.position()) >= min_bytes) || (number_of_rows == max_rows)) need_more_data = false;
...
void registerFileSegmentationEngineRegexp(FormatFactory &factory) { factory.registerFileSegmentationEngine("Regexp", &segmentationEngine); }

void registerRegexpSchemaReader(FormatFactory &factory) {
  factory.registerSchemaReader(
      "Regexp", [](ReadBuffer &buf, const FormatSettings &settings) { return std::make_shared<RegexpSchemaReader>(buf, settings); });
