...
PrettyBlockOutputFormat::PrettyBlockOutputFormat(WriteBuffer &out_, SharedHeader header_, const FormatSettings &format_settings_,
                                                 Style style_, bool mono_block_, bool color_, bool glue_chunks_)
  ...
  if (!header_->getColumns().empty()) {
    ...
    if (isNumber(type)) readable_number_tip = true;
  }
...
void PrettyBlockOutputFormat::calculateWidths(const Block &header, const Chunk &chunk, bool split_by_lines, bool &out_has_newlines,
  ...
  for (size_t i = 0; i < num_columns; ++i) {
    ...
    for (size_t j = 0; j < num_rows; ++j) {
      if (cutInTheMiddle(j, num_rows, format_settings.pretty.max_rows)) continue;
      ...
      if (format_settings.pretty.max_value_width) {
        ...
        if (serialized_value.size() > max_byte_size) serialized_value.resize(max_byte_size);
      }
      ...
      while (start_from_offset < serialized_value.size()) {
        if (split_by_lines) {
          ...
          if (next_nl < end) out_has_newlines = true;
        ...
        widths[i][displayed_row] = std::max(widths[i][displayed_row],
                                            UTF8::computeWidth(reinterpret_cast<const UInt8 *>(serialized_value.data() + start_from_offset),
                                                               next_offset - start_from_offset, prefix));

        max_padded_widths[i] =
            std::max<UInt64>(max_padded_widths[i], std::min<UInt64>({format_settings.pretty.max_column_pad_width,
                                                                     format_settings.pretty.max_value_width, widths[i][displayed_row]}));

        start_from_offset = next_offset;
        if (start_from_offset < serialized_value.size()) ++start_from_offset;
      }
...
void PrettyBlockOutputFormat::writeChunk(const Chunk &chunk, PortKind port_kind) {
  ...
  for (size_t width : max_widths) table_width += width;
  ...
  if (use_vertical_format) {
    ...
    for (size_t i = 0; i < num_rows && displayed_rows < format_settings.pretty.max_rows; ++i) {
      if (i != 0) vertical_format_fallback->writeRowBetweenDelimiter();
  ...
  String left_blank;
  if (format_settings.pretty.row_numbers) left_blank.assign(row_number_width, ' ');
  ...
  if (style == Style::Full) {
    ...
    rows_end_out << grid[3][0];
    for (size_t i = 0; i < num_columns; ++i) {
      if (i != 0) rows_end_out << grid[3][2];
      for (size_t j = 0; j < max_widths[i] + 2; ++j) rows_end_out << grid[3][1];
    }
    rows_end_out << grid[3][3] << "\n";
...

  {
    ...
    for (size_t i = 0; i < num_columns; ++i) vertical_filler_size += max_widths[i] + 3;
    ...
    for (size_t i = 0; i < vertical_filler_size; ++i) vertical_filler_out << (i % 2 ? " " : horizontal_bar);
  ...
  if (glue_chunks && port_kind == PortKind::Main && (!format_settings.pretty.row_numbers || row_number_width == prev_row_number_width) &&
    ...
    if (!rows_end.empty()) writeCString("\033[1A\033[2K\033[G", out);
    if (had_footer) {
      size_t times = !footer_begin.empty() + !footer_end.empty() + rows_end.empty();
      for (size_t i = 0; i < times; ++i) writeCString("\033[1A\033[2K\033[G", out);
    }
    if (!rows_separator.empty()) writeString(rows_separator, out);
  } else {
  ...
  for (size_t i = 0; i < num_rows && displayed_rows < format_settings.pretty.max_rows; ++i) {
    if (cutInTheMiddle(i, num_rows, format_settings.pretty.max_rows)) {
      ...
      if (i != 0) writeString(rows_separator, out);
      ...
      while (true) {
        if (format_settings.pretty.row_numbers) {
          if (first_line) {
            ...
            for (size_t j = 0; j < row_number_width - row_num_string.size(); ++j) writeChar(' ', out);

            if (color) out << "\033[90m";
        ...
        for (size_t j = 0; j < num_columns; ++j) {
          ...
          writeValueWithPadding(*columns[j], *serializations[j], i, format_settings.pretty.multiline_fields, serialized_values[j],
                                offsets_inside_serialized_values[j], widths[j].empty() ? max_widths[j] : widths[j][displayed_row],
                                max_widths[j], cut_to_width, type->shouldAlignRightInPrettyFormats(), isNumber(removeNullable(type)));

          if (offsets_inside_serialized_values[j] != serialized_values[j]->size()) all_lines_printed = false;
        }

        if (style != Style::Space) out << vertical_bar;
        ...
        if (readable_number_tip) {
          ...
          for (size_t w : max_widths) visible_table_width += w;
          ...
          if (remaining_width > 0) writeReadableNumberTip(out, *columns.back(), i, format_settings, color, remaining_width);
        }

        out << "\n";
        if (all_lines_printed) break;
      }
...
void PrettyBlockOutputFormat::writeValueWithPadding(const IColumn &column, const ISerialization &serialization, size_t row_num,
  ...
  if (color && is_number && format_settings.pretty.highlight_digit_groups) serialized_fragment = highlightDigitGroups(serialized_fragment);

  /// Highlight trailing spaces.
  if (color && format_settings.pretty.highlight_trailing_spaces) serialized_fragment = highlightTrailingSpaces(serialized_fragment);
  ...
  if (cut_to_width && value_width > cut_to_width) {
    is_cut = true;
    serialized_fragment.resize(UTF8::computeBytesBeforeWidth(reinterpret_cast<const UInt8 *>(serialized_fragment.data()),
                                                             serialized_fragment.size(), prefix, format_settings.pretty.max_value_width));
  ...
  auto write_padding = [&]() {
    if (pad_to_width > value_width)
      for (size_t k = 0; k < pad_to_width - value_width; ++k) writeChar(' ', out);
  };
...
void PrettyBlockOutputFormat::consume(Chunk chunk) { write(std::move(chunk), PortKind::Main); }
...
void PrettyBlockOutputFormat::writeSuffixImpl() {
  if (total_rows >= format_settings.pretty.max_rows) {
    if (style == Style::Space) out << "\n";

    out << "Showed " << displayed_rows << " out of " << total_rows << " rows.\n";
  }
}

void PrettyBlockOutputFormat::onRowsReadBeforeUpdate() { total_rows = getRowsReadBefore(); }
  ...
  for (auto style :
    ...
    for (bool no_escapes : {false, true}) {
      for (bool mono_block : {false, true}) {
        ...
        if (no_escapes) name += "NoEscapes";
        if (mono_block) name += "MonoBlock";

        factory.registerOutputFormat(name, [style, no_escapes, mono_block](WriteBuffer &buf, const Block &sample,
          ...
          bool glue_chunks = !no_escapes && (format_settings.pretty.glue_chunks == 1 ||
                                             (format_settings.pretty.glue_chunks == 2 && format_settings.is_writing_to_terminal));
          return std::make_shared<PrettyBlockOutputFormat>(buf, std::make_shared<const Block>(sample), format_settings, style, mono_block,
                                                           color, glue_chunks);
        });
      }
    }
