...
orc::CompressionKind getORCCompression(FormatSettings::ORCCompression method) {
  if (method == FormatSettings::ORCCompression::NONE) return orc::CompressionKind::CompressionKind_NONE;

#  if USE_SNAPPY
  if (method == FormatSettings::ORCCompression::SNAPPY) return orc::CompressionKind::CompressionKind_SNAPPY;
#  endif

  if (method == FormatSettings::ORCCompression::ZSTD) return orc::CompressionKind::CompressionKind_ZSTD;

  if (method == FormatSettings::ORCCompression::LZ4) return orc::CompressionKind::CompressionKind_LZ4;

  if (method == FormatSettings::ORCCompression::ZLIB) return orc::CompressionKind::CompressionKind_ZLIB;
...
uint64_t ORCOutputStream::getLength() const { return out.count(); }
...
void ORCOutputStream::write(const void *buf, size_t length) { out.write(static_cast<const char *>(buf), length); }
...
ORCBlockOutputFormat::ORCBlockOutputFormat(WriteBuffer &out_, SharedHeader header_, const FormatSettings &format_settings_)
  ...
  for (const auto &type : header_->getDataTypes()) data_types.push_back(recursiveRemoveLowCardinality(type));
}

std::unique_ptr<orc::Type> ORCBlockOutputFormat::getORCType(const DataTypePtr &type) {
  switch (type->getTypeId()) {
    case TypeIndex::UInt8: {
      if (isBool(type)) return orc::createPrimitiveType(orc::TypeKind::BOOLEAN);
    ...
    case TypeIndex::String: {
      if (format_settings.orc.output_string_as_string) return orc::createPrimitiveType(orc::TypeKind::STRING);
      return orc::createPrimitiveType(orc::TypeKind::BINARY);
    }
    case TypeIndex::IPv6: {
      return orc::createPrimitiveType(orc::TypeKind::BINARY);
    ...
    case TypeIndex::Tuple: {
      ...
      for (size_t i = 0; i < nested_types.size(); ++i) struct_type->addStructField(nested_names[i], getORCType(nested_types[i]));
      return struct_type;
    }
    case TypeIndex::Map: {
      const auto *map_type = assert_cast<const DataTypeMap *>(type.get());
      return orc::createMapType(getORCType(map_type->getKeyType()), getORCType(map_type->getValueType()));
    }
    default: {
...
void ORCBlockOutputFormat::writeNumbers(orc::ColumnVectorBatch &orc_column, const IColumn &column,
  ...
  for (size_t i = 0; i != number_column.size(); ++i) number_orc_column.data[i] = convert(number_column.getElement(i));
}

template <typename Decimal, typename DecimalVectorBatch, typename ConvertFunc>
void ORCBlockOutputFormat::writeDecimals(orc::ColumnVectorBatch &orc_column, const IColumn &column, DataTypePtr &type,
                                         const PaddedPODArray<UInt8> * /*null_bytemap*/, ConvertFunc convert) {
  ...
  for (size_t i = 0; i != decimal_column.size(); ++i) decimal_orc_column.values[i] = convert(decimal_column.getElement(i).value);
}

template <typename ColumnType>
void ORCBlockOutputFormat::writeStrings(orc::ColumnVectorBatch &orc_column, const IColumn &column,
...
void ORCBlockOutputFormat::writeDateTimes(orc::ColumnVectorBatch &orc_column, const IColumn &column,
                                          const PaddedPODArray<UInt8> * /*null_bytemap*/, GetSecondsFunc get_seconds,
...
void ORCBlockOutputFormat::writeColumn(orc::ColumnVectorBatch &orc_column, const IColumn &column, DataTypePtr &type,
                                       const PaddedPODArray<UInt8> *null_bytemap) {
  ...
  if (orc_column.hasNulls) {
    for (size_t i = 0; i < rows; ++i) orc_column.notNull[i] = !(*null_bytemap)[i];
  } else {
    for (size_t i = 0; i < rows; ++i) orc_column.notNull[i] = 1;
  }
  ...
  switch (type->getTypeId()) {
    ...
    case TypeIndex::IPv4: {
      writeNumbers<IPv4, orc::LongVectorBatch>(orc_column, column, null_bytemap,
                                               [](const IPv4 &value) { return Int64(Int32(value.toUnderType())); });
      break;
    }
    case TypeIndex::Int64: {
    ...
    case TypeIndex::DateTime: {
      writeDateTimes<ColumnUInt32>(orc_column, column, null_bytemap, [](UInt32 value) { return value; }, [](UInt32) { return 0; });
      break;
    }
    case TypeIndex::DateTime64: {
      ...
      writeDateTimes<DataTypeDateTime64::ColumnType>(
          orc_column, column, null_bytemap, [scale](Int64 value) { return value / Int64(std::pow(10, scale)); },
          [scale](Int64 value) { return (value % Int64(std::pow(10, scale))) * Int64(std::pow(10, 9 - scale)); });
      break;
    }
    case TypeIndex::Decimal32: {
      writeDecimals<Decimal32, orc::Decimal64VectorBatch>(orc_column, column, type, null_bytemap, [](Int32 value) { return value; });
      break;
    }
    case TypeIndex::Decimal64: {
      writeDecimals<Decimal64, orc::Decimal64VectorBatch>(orc_column, column, type, null_bytemap, [](Int64 value) { return value; });
      break;
    }
    case TypeIndex::Decimal128: {
      writeDecimals<Decimal128, orc::Decimal128VectorBatch>(orc_column, column, type, null_bytemap, [](Int128 value) {
        return orc::Int128(static_cast<Int64>(value >> 64), static_cast<UInt64>((value << 64) >> 64));
      });
      break;
    }
    case TypeIndex::Decimal256: {
    ...
    case TypeIndex::Nullable: {
      ...
      const PaddedPODArray<UInt8> &new_null_bytemap =
          assert_cast<const ColumnVector<UInt8> &>(*nullable_column.getNullMapColumnPtr()).getData();
    ...
    case TypeIndex::Array: {
      ...
      for (size_t i = 0; i != column_size; ++i) list_orc_column.offsets[i + 1] = offsets[i];
    ...
    case TypeIndex::Map: {
      ...
      for (size_t i = 0; i != column_size; ++i) map_orc_column.offsets[i + 1] = offsets[i];
...
void ORCBlockOutputFormat::consume(Chunk chunk) {
  ...
  for (auto &column : columns) column = recursiveRemoveLowCardinality(column);

  for (size_t i = 0; i != columns_num; ++i) writeColumn(*root.fields[i], *columns[i], data_types[i], nullptr);
...
void ORCBlockOutputFormat::resetFormatterImpl() { writer.reset(); }
...
void registerOutputFormatORC(FormatFactory &factory) {
  factory.registerOutputFormat(
      "ORC", [](WriteBuffer &buf, const Block &sample, const FormatSettings &format_settings, FormatFilterInfoPtr /*format_filter_info*/) {
