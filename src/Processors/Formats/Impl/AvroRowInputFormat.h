...
class AvroDeserializer {
 public:
  ...
  AvroDeserializer(DataTypePtr data_type, const std::string& column_name, avro::ValidSchema schema, bool allow_missing_fields,
                   bool null_as_default_, const FormatSettings& settings_);
 ...
 private:
  ...
  struct Action {
    ...
    Action(int target_column_idx_, DeserializeFn deserialize_fn_)
        : type(Deserialize), target_column_idx(target_column_idx_), deserialize_fn(deserialize_fn_) {}

    explicit Action(SkipFn skip_fn_) : type(Skip), target_column_idx(0), skip_fn(skip_fn_) {}

    Action(const std::vector<size_t>& nested_column_indexes_, const std::vector<DeserializeFn>& nested_deserializers_)
        : type(Nested), target_column_idx(0), nested_column_indexes(nested_column_indexes_), nested_deserializers(nested_deserializers_) {}
    ...
    void execute(MutableColumns& columns, avro::Decoder& decoder, RowReadExtension& ext) const {
      switch (type) {
        ...
        case Skip:
          ...
          break;
        case Record:
          for (const auto& action : actions) action.execute(columns, decoder, ext);
          break;
        case Nested:
   ...
   private:
    Action(Type type_, std::vector<Action> actions_) : type(type_), target_column_idx(0), actions(actions_) {}
