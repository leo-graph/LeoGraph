...
void MarkdownRowOutputFormat::writeRowStartDelimiter() { writeCString("| ", out); }

void MarkdownRowOutputFormat::writeFieldDelimiter() { writeCString(" | ", out); }

void MarkdownRowOutputFormat::writeRowEndDelimiter() { writeCString(" |\n", out); }
...
void registerOutputFormatMarkdown(FormatFactory& factory) {
  auto registerWithName = [&](const auto& name) {
    factory.registerOutputFormat(
        name, [](WriteBuffer& buf, const Block& sample, const FormatSettings& settings, FormatFilterInfoPtr /*format_filter_info*/) {
