...
class ParallelParsingInputFormat : public IInputFormat {
 public:
  ...
  ~ParallelParsingInputFormat() override { finishAndWait(); }

  void resetParser() final { throw Exception(ErrorCodes::LOGICAL_ERROR, "resetParser() is not allowed for {}", getName()); }

  const BlockMissingValues* getMissingValues() const final { return &last_block_missing_values; }

  void setSerializationHints(const SerializationInfoByName& hints) override { serialization_hints = hints; }
 ...
 private:
  ...
  class InternalParser {
   public:
    explicit InternalParser(const InputFormatPtr& input_format_)
        : input_format(input_format_), port(input_format->getPort().getHeader(), input_format.get()) {
  ...
  enum ProcessingUnitStatus { READY_TO_INSERT, READY_TO_PARSE, READY_TO_READ };
  ...
  struct ProcessingUnit {
    ProcessingUnit() : status(ProcessingUnitStatus::READY_TO_INSERT) {}
  ...
  void scheduleParserThreadForUnitWithNumber(size_t ticket_number) {
    runner.enqueueAndKeepTrack([this, ticket_number]() { parserThreadFunction(ticket_number); });
    /// We have to wait here to possibly extract ColumnMappingPtr from the first parser.
    if (ticket_number == 0) first_parser_finished.wait();
  }
  ...
  void finishAndWait() noexcept {
    ...
    if (finish_and_wait_called) return;
    ...
    if (segmentator_thread.joinable()) segmentator_thread.join();
