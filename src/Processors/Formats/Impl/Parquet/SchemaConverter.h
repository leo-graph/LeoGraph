...
struct SchemaConverter {
 ...
 private:
  ...
  struct TraversalNode {
    ...
    const String &getParquetName() const { return parquet_name.has_value() ? *parquet_name : name; }

    String getNameForLogging() const {
      if (parquet_name.has_value() && *parquet_name != name) return fmt::format("{} (mapped to {})", *parquet_name, name);
      return name;
    }
  ...
  void processPrimitiveColumn(const parq::SchemaElement &element, DataTypePtr type_hint, PageDecoderInfo &out_decoder,
