...
SchemaConverter::SchemaConverter(const parq::FileMetaData &file_metadata_, const ReadOptions &options_, const Block *sample_block_)
...
void SchemaConverter::checkHasColumns() {
  if (file_metadata.schema.size() < 2) throw Exception(ErrorCodes::INCORRECT_DATA, "Parquet file has no columns");
  if (file_metadata.schema.at(0).num_children <= 0) throw Exception(ErrorCodes::INCORRECT_DATA, "Schema root has no children");
}
...
void SchemaConverter::prepareForReading() {
  ...
  for (const auto &col : output_columns) {
    if (!col.idx_in_output_block.has_value()) continue;
    size_t idx = col.idx_in_output_block.value();
    if (found_columns.at(idx))
      throw Exception(ErrorCodes::DUPLICATE_COLUMN, "There are multiple columns with name `{}` in the parquet file",
  ...
  for (const auto &p : primitive_columns) chassert(p.idx_in_output_block != UINT64_MAX);
...
std::string_view SchemaConverter::useColumnMapperIfNeeded(const parq::SchemaElement &element, const String &current_path) const {
  if (!column_mapper) return element.name;
  ...
  if (it == map.end())
    throw Exception(ErrorCodes::ICEBERG_SPECIFICATION_VIOLATION,
                    "Parquet file has column {} with field_id {} that is not in datalake metadata", element.name, element.field_id);
  ...
  if (current_path.empty()) return it->second;

  /// Strip "current_path." prefix to get the child name (preserves dots in child names)
  std::string_view mapped = it->second;
  if (mapped.starts_with(current_path) && mapped.size() > current_path.size() && mapped[current_path.size()] == '.')
...
void SchemaConverter::processSubtree(TraversalNode &node) {
  if (node.type_hint) chassert(node.requested);
  if (schema_idx >= file_metadata.schema.size()) throw Exception(ErrorCodes::INCORRECT_DATA, "Invalid parquet schema tree");
  ...
  if (node.schema_context == SchemaContext::None) {
    ...
    if (sample_block) {
      ...
      if (pos.has_value()) {
        if (node.requested)
          throw Exception(ErrorCodes::COLUMN_QUERIED_MORE_THAN_ONCE, "Requested column {} is part of another requested column",
          ...
          if (levels[i].is_array) {
            ...
            if (!array)
              throw Exception(
                  ...
                  "Requested type of nested column {} doesn't match parquet schema: parquet type is Array, requested type is {}",
  ...
  if (node.element->repetition_type != parq::FieldRepetitionType::REQUIRED) {
    ...
    if (prev.def == UINT8_MAX)
      throw Exception(ErrorCodes::TOO_DEEP_RECURSION, "Parquet column {} has extremely deeply nested (>255 levels) arrays or nullables",
                      node.getNameForLogging());
      ...
      if (node.type_hint) {
        ...
        if (!array_type)
          throw Exception(ErrorCodes::TYPE_MISMATCH,
                          "Requested type of column {} doesn't match parquet schema: parquet type is Array, requested type is {}",
                          node.getNameForLogging(), node.type_hint->getName());
  ...
  if (!processSubtreePrimitive(node) && !processSubtreeMap(node) && !processSubtreeArrayOuter(node) && !processSubtreeArrayInner(node)) {
    processSubtreeTuple(node);
  }

  if (!node.output_idx.has_value()) return;
  if (!node.requested) return;  // we just needed to recurse to children, not interested in output_idx
  ...
  if (idx_in_output_block.has_value()) {
    ...
    for (size_t i = 0; i < wrap_in_arrays; ++i) make_array(static_cast<UInt8>(levels[prev_levels_size - 1].rep - i));
...
bool SchemaConverter::processSubtreePrimitive(TraversalNode &node) {
  if (!isPrimitiveNode(*node.element)) return false;

  primitive_column_idx += 1;
  if (!node.requested) return true;
  ...
  for (const auto &level : levels)
    if (level.is_array) primitive.max_array_def = level.def;
  ...
  if (!primitive.decoded_type) primitive.decoded_type = inferred_type;
...
bool SchemaConverter::processSubtreeMap(TraversalNode &node) {
  ...
  if (node.element->converted_type != parq::ConvertedType::MAP && node.element->converted_type != parq::ConvertedType::MAP_KEY_VALUE &&
      !node.element->logicalType.__isset.MAP)
    return false;
  ...
  if (node.schema_context != SchemaContext::None && node.schema_context != SchemaContext::ListElement) return false;
  if (typeid_cast<const DataTypeTuple *>(node.type_hint.get())) return false;
  if (node.element->num_children != 1) return false;
  const parq::SchemaElement &child = file_metadata.schema.at(schema_idx);
  if (child.repetition_type != parq::FieldRepetitionType::REPEATED || child.num_children != 2) return false;
  ...
  if (node.type_hint) {
    if (const DataTypeMap *map_type = typeid_cast<const DataTypeMap *>(node.type_hint.get())) {
      ...
      throw Exception(ErrorCodes::TYPE_MISMATCH,
                      "Requested type of column {} doesn't match parquet schema: parquet type is Map, requested type is {}",
                      node.getNameForLogging(), node.type_hint->getName());
    }
  }
  ...
  if (!node.requested || !subnode.output_idx.has_value()) return true;
...
bool SchemaConverter::processSubtreeArrayOuter(TraversalNode &node) {
  ...
  if (node.element->converted_type != parq::ConvertedType::LIST && !node.element->logicalType.__isset.LIST) return false;
  if (node.schema_context != SchemaContext::None && node.schema_context != SchemaContext::ListElement) return false;
  if (node.element->num_children != 1) return false;
  const parq::SchemaElement &child = file_metadata.schema.at(schema_idx);
  if (child.repetition_type != parq::FieldRepetitionType::REPEATED) return false;
...
bool SchemaConverter::processSubtreeArrayInner(TraversalNode &node) {
  if (node.schema_context != SchemaContext::ListTuple) return false;

  /// Array (middle schema element).
  chassert(node.element->repetition_type == parq::FieldRepetitionType::REPEATED && node.element->num_children == 1);  // caller checked this
...
void SchemaConverter::processSubtreeTuple(TraversalNode &node) {
  ...
  if (tuple_type_hint) {
    if (tuple_type_hint->hasExplicitNames() && !tuple_type_hint->getElements().empty() && node.schema_context != SchemaContext::MapTuple) {
      ...
      if (tuple_type_hint->getElements().size() != size_t(node.element->num_children))
        throw Exception(ErrorCodes::TYPE_MISMATCH,
                        "Requested type of column {} doesn't match parquet schema: parquet type is Tuple with {} elements, requested type "
                        "is Tuple with {} elements",
                        node.getNameForLogging(), node.element->num_children, tuple_type_hint->getElements().size());
    }
  }
  if (!lookup_by_name && node.requested) elements.resize(size_t(node.element->num_children), UINT64_MAX);
  ...
  for (size_t i = 0; i < size_t(node.element->num_children); ++i) {
    const String &element_name =
        element_names_in_file.emplace_back(useColumnMapperIfNeeded(file_metadata.schema.at(schema_idx), node.name));
    ...
    if (lookup_by_name) {
      ...
      if (idx_in_output_tuple.has_value() && elements.at(idx_in_output_tuple.value()) != UINT64_MAX)
        throw Exception(ErrorCodes::DUPLICATE_COLUMN, "Parquet tuple {} has multiple elements with name `{}`", node.getNameForLogging(),
                        element_name);
    }

    DataTypePtr element_type_hint;
    if (tuple_type_hint && idx_in_output_tuple.has_value()) element_type_hint = tuple_type_hint->getElement(idx_in_output_tuple.value());
    ...
    if (node.schema_context == SchemaContext::MapTuple && idx_in_output_tuple == 0) subnode.schema_context = SchemaContext::MapKey;
  ...
  if (node.schema_context == SchemaContext::MapTuple) names = {"keys", "values"};

  DataTypePtr output_type;
  if (tuple_type_hint) {
    chassert(elements.size() == tuple_type_hint->getElements().size());
    for (size_t i = 0; i < elements.size(); ++i) {
      if (elements[i] != UINT64_MAX) continue;
      if (!options.format.parquet.allow_missing_columns)
        throw Exception(ErrorCodes::THERE_IS_NO_COLUMN, "Requested tuple element {} of column {} was not found in parquet schema ({})",
                        tuple_type_hint->getNameByPosition(i + 1), node.getNameForLogging(), element_names_in_file);

      elements[i] = output_columns.size();
      OutputColumnInfo &missing_output = output_columns.emplace_back();
      missing_output.name =
          node.name + "." + (tuple_type_hint->hasExplicitNames() ? tuple_type_hint->getNameByPosition(i + 1) : std::to_string(i + 1));
...
void SchemaConverter::processPrimitiveColumn(const parq::SchemaElement &element, DataTypePtr type_hint, PageDecoderInfo &out_decoder,
                                             DataTypePtr &out_decoded_type, DataTypePtr &out_inferred_type,
                                             std::optional<GeoColumnMetadata> geo_metadata) const {
  /// Inputs:
    ...
    if (which.isNativeInteger())
      ...
      switch (which.idx) {
        ...
        case TypeIndex::DateTime:
          if (!allow_datetime_and_ipv4) return false;
          converter.field_signed = false;
          break;
  ...
  auto is_output_type_string = [&]() -> bool { return get_output_type_index() == TypeIndex::String; };
  ...
  if (const DataTypeFixedString *fixed_string_type = typeid_cast<const DataTypeFixedString *>(type_hint.get())) {
    ...
    switch (type) {
      ...
      case parq::Type::FIXED_LEN_BYTE_ARRAY:
        size = size_t(element.type_length);
        break;
  ...
  if (logical.__isset.STRING || logical.__isset.JSON || logical.__isset.BSON || logical.__isset.ENUM || converted == CONV::UTF8 ||
      converted == CONV::JSON || converted == CONV::BSON || converted == CONV::ENUM) {
    if (type != parq::Type::BYTE_ARRAY && type != parq::Type::FIXED_LEN_BYTE_ARRAY)
      throw Exception(ErrorCodes::INCORRECT_DATA, "Unexpected non-string physical type for string logical type: {}",
                      thriftToString(element));
    ...
    if (physical_bits == 64) {
      out_decoded_type = is_signed ? std::static_pointer_cast<IDataType>(std::make_shared<DataTypeInt64>())
    ...
    return;
  } else if (logical.__isset.TIMESTAMP || logical.__isset.TIME || converted == CONV::TIMESTAMP_MILLIS ||
             converted == CONV::TIMESTAMP_MICROS || converted == CONV::TIME_MILLIS || converted == CONV::TIME_MICROS) {
    ...
    UInt32 scale;
    if (logical.TIMESTAMP.unit.__isset.MILLIS || logical.TIME.unit.__isset.MILLIS || converted == CONV::TIMESTAMP_MILLIS ||
        converted == CONV::TIME_MILLIS)
      scale = 3;
    else if (logical.TIMESTAMP.unit.__isset.MICROS || logical.TIME.unit.__isset.MICROS || converted == CONV::TIMESTAMP_MICROS ||
    ...
    if (!options.format.parquet.local_time_as_utc &&
        ((logical.__isset.TIMESTAMP && !logical.TIMESTAMP.isAdjustedToUTC) || (logical.__isset.TIME && !logical.TIME.isAdjustedToUTC)))
      timezone = "";
    ...
    if (precision > max_precision)
      throw Exception(ErrorCodes::INCORRECT_DATA, "Parquet decimal type precision or scale is too big ({} digits) for physical type {}",
                      precision, thriftToString(type));
    ...
    return;
  } else if (logical.__isset.MAP || logical.__isset.LIST || converted == CONV::MAP || converted == CONV::MAP_KEY_VALUE ||
             converted == CONV::LIST) {
  ...
  switch (type) {
    ...
    case parq::Type::FIXED_LEN_BYTE_ARRAY: {
      if (type_hint) {
        ...
        if (which.isInteger() && !which.isNativeInteger() && type_hint->getSizeOfValueInMemory() == size_t(element.type_length)) {
          out_inferred_type = type_hint;
        }
      }

      if (!out_inferred_type) out_inferred_type = std::make_shared<DataTypeFixedString>(size_t(element.type_length));
      auto converter = std::make_shared<FixedStringConverter>();
