...
struct StatisticsNumeric {
  T min = std::numeric_limits<T>::has_infinity ? std::numeric_limits<T>::infinity() : std::numeric_limits<T>::max();
...
struct StatisticsFixedStringRef {
  ...
  parq::Statistics get(const WriteOptions &options) const {
    parq::Statistics s;
    if (min == nullptr || fixed_string_size > options.max_statistics_size) return s;
  ...
  void addMin(const uint8_t *p) {
    if (min == nullptr || memcmp(p, min, fixed_string_size) < 0) min = p;
  }
  void addMax(const uint8_t *p) {
    if (max == nullptr || memcmp(p, max, fixed_string_size) > 0) max = p;
  }
};
...
struct StatisticsFixedStringCopy {
  ...
  void addMin(const uint8_t *p) {
    if (empty || compare(p, min.data()) < 0) memcpy(min.data(), p, S);
  }
  void addMax(const uint8_t *p) {
    if (empty || compare(p, max.data()) > 0) memcpy(max.data(), p, S);
  }
};
...
struct StatisticsStringRef {
  ...
  void addMin(parquet::ByteArray x) {
    if (min.ptr == nullptr || compare(x, min) < 0) min = x;
  }

  void addMax(parquet::ByteArray x) {
    if (max.ptr == nullptr || compare(x, max) > 0) max = x;
  }
...
template <typename Col, typename To,
          typename MinMaxType =
              typename std::conditional_t<std::is_signed_v<typename Col::Container::value_type>, To, typename std::make_unsigned_t<To>>>
struct ConverterNumeric {
  ...
  const To *getBatch(size_t offset, size_t count) {
    if constexpr (sizeof(*column.getData().data()) == sizeof(To) && !std::is_same_v<To, bool>)
      ...
      buf.resize(count);
      for (size_t i = 0; i < count; ++i) buf[i] = static_cast<To>(column.getData()[offset + i]);  // NOLINT
      return buf.data();
    }
  }
};
...
struct ConverterDateTime64WithMultiplier {
  ...
  ConverterDateTime64WithMultiplier(const ColumnPtr &c, Int64 multiplier_)
      : column(assert_cast<const Col &>(*c)), multiplier(multiplier_) {}
    ...
    for (size_t i = 0; i < count; ++i) {
      ...
      if (common::mulOverflow(value, multiplier, buf[i]))
        throw Exception(ErrorCodes::VALUE_IS_OUT_OF_RANGE_OF_DATA_TYPE,
                        "DateTime64 value {} is out of range for Parquet timestamp (multiplier {})", value, multiplier);
    }
    return buf.data();
  }
};
...
struct ConverterDateTime {
  ...
  const Int64 *getBatch(size_t offset, size_t count) {
    buf.resize(count);
    for (size_t i = 0; i < count; ++i) buf[i] = static_cast<Int64>(column.getData()[offset + i]) * 1000;
    return buf.data();
  }
};
...
struct ConverterFixedStringAsString {
  ...
  const parquet::ByteArray *getBatch(size_t offset, size_t count) {
    buf.resize(count);
    for (size_t i = 0; i < count; ++i)
      buf[i] = parquet::ByteArray(static_cast<UInt32>(column.getN()),
                                  reinterpret_cast<const uint8_t *>(column.getChars().data() + (offset + i) * column.getN()));
    return buf.data();
  }
};
...
struct ConverterNumberAsFixedString {
  ...
  const parquet::FixedLenByteArray *getBatch(size_t offset, size_t count) {
    buf.resize(count);
    for (size_t i = 0; i < count; ++i) buf[i].ptr = reinterpret_cast<const uint8_t *>(column.getData().data() + offset + i);
    return buf.data();
  }
...
struct ConverterJSON {
  ...
  explicit ConverterJSON(const ColumnPtr &c, const DataTypePtr &data_type_, const FormatSettings &format_settings_)
      : column(assert_cast<const ColumnObject &>(*c)), data_type(data_type_), format_settings(format_settings_) {}
...
#pragma clang diagnostic pop
      ...
      int compressed_size =
          LZ4_compress_default(source.data(), scratch.data(), static_cast<int>(source.size()), static_cast<int>(max_dest_size));
    ...
    case CompressionMethod::Snappy: {
      ...
      if (max_dest_size > std::numeric_limits<int>::max())
        throw Exception(ErrorCodes::CANNOT_COMPRESS, "Cannot compress column of size {}",
                        formatReadableSizeWithBinarySuffix(source.size()));
    ...
    default: {
      ...
      auto compressed_buf = wrapWriteBufferWithCompressionMethod(std::move(dest_buf), method, level,
                                                                 /*zstd_window_log*/ 0, source.size(),
...
void encodeRepDefLevelsRLE(const UInt8 *data, size_t size, UInt8 max_level, PODArray<char> &out) {
  ...
  auto max_rle_size = RleBitPackedEncoder::MaxBufferSize(bit_width, static_cast<int>(size)) + RleBitPackedEncoder::MinBufferSize(bit_width);
  ...
  for (size_t i = 0; i < size; ++i) encoder.Put(data[i]);
  encoder.Flush();
...
void writePage(const parq::PageHeader &header, const PODArray<char> &compressed, ColumnChunkWriteState &s, bool add_to_offset_index,
               size_t first_row_index, WriteBuffer &out) {
...
void makeBloomFilter(const HashSet<UInt64, TrivialHash> &hashes, ColumnChunkIndexes &indexes, const WriteOptions &options) {
  ...
  while (static_cast<double>(num_blocks) < requested_num_blocks) {
    if (num_blocks >= 4 * 1024 * 1024) return;
    num_blocks *= 2;
  }
...
void writeColumnImpl(ColumnChunkWriteState &s, const WriteOptions &options, WriteBuffer &out, Converter &&converter) {
  ...
  if constexpr (std::is_same_v<ParquetDType, parquet::FLBAType>) {
    ...
    fixed_string_descr.emplace(
        parquet::schema::PrimitiveNode::Make("", parquet::Repetition::REQUIRED, parquet::Type::FIXED_LEN_BYTE_ARRAY,
                                             parquet::ConvertedType::NONE, static_cast<int>(converter.fixedStringSize())),
        0, 0);
  ...
  if (s.max_rep > 0) s.column_chunk.meta_data.size_statistics.__set_repetition_level_histogram(std::vector<Int64>(s.max_rep + 1));
  if (s.max_def > 0) s.column_chunk.meta_data.size_statistics.__set_definition_level_histogram(std::vector<Int64>(s.max_def + 1));
  ...
  if (options.write_bloom_filter) hashes_for_bloom_filter.emplace();  // allocates memory for initial size
  ...
  auto flush_page = [&](size_t def_count, size_t data_count) {
    ...
    if (encoded.size() > INT32_MAX) throw Exception(ErrorCodes::CANNOT_COMPRESS, "Uncompressed page is too big: {}", encoded.size());

    size_t uncompressed_size = encoded.size();
    auto &compressed = compress(encoded, compressed_maybe, s.compression, s.compression_level);

    if (compressed.size() > INT32_MAX) throw Exception(ErrorCodes::CANNOT_COMPRESS, "Compressed page is too big: {}", compressed.size());
    ...
    if (has_null_count) page_stats.__set_null_count(static_cast<Int64>(def_count - data_count));

    if (options.write_page_statistics) d.__set_statistics(page_stats);
  ...
  auto flush_dict = [&] -> bool {
    ...
    for (auto &p : dict_encoded_pages) writePage(p.header, p.data, s, options.write_page_index, p.first_row_index, out);
  ...
  while (def_offset < num_values) {
    ...
    while (true) {
      ...
      if (s.max_def == 0)  // no arrays or nullables
        ...
        for (size_t i = 0; i < def_count; ++i) data_count += s.def[next_def_offset + i] == s.max_def;
...
#  pragma clang loop vectorize(disable)
#endif
        for (size_t i = 0; i < data_count; ++i) page_statistics.add(converted[i]);
      ...
      if (use_dictionary && is_dict_too_big()) {
        ...
        encoder = parquet::MakeTypedEncoder<ParquetDType>(static_cast<parquet::Encoding::type>(encoding), /* use_dictionary */ false,
                                                          fixed_string_descr ? &*fixed_string_descr : nullptr);
        break;
      }

      if (next_def_offset == num_values || static_cast<size_t>(encoder->EstimatedDataEncodedSize()) >= options.data_page_size) {
  ...
  if (s.max_rep > 0 || s.max_def > 0) addToEncodingsUsed(s, parq::Encoding::RLE);  // levels
  if (use_dictionary) {
  ...
  if (hashes_for_bloom_filter.has_value()) makeBloomFilter(*hashes_for_bloom_filter, s.indexes, options);
}

}  // namespace

void writeColumnChunkBody(ColumnChunkWriteState &s, const WriteOptions &options, const FormatSettings &format_settings, WriteBuffer &out) {
...
#define N(source_type, parquet_dtype)                      \
  writeColumnImpl<parquet::parquet_dtype>(s, options, out, \
                                          ConverterNumeric<ColumnVector<source_type>, parquet::parquet_dtype::c_type>(s.primitive_column))

    case TypeIndex::UInt8:
      if (s.is_bool)
        writeColumnImpl<parquet::BooleanType>(s, options, out, ConverterNumeric<ColumnVector<UInt8>, bool, bool>(s.primitive_column));
      else
        N(UInt8, Int32Type);
      break;
    ...
    case TypeIndex::Int8: {
      if (options.output_enum_as_byte_array && isEnum8(s.type))
        writeColumnImpl<parquet::ByteArrayType>(s, options, out, ConverterEnumAsString<Int8>(s.primitive_column, s.type));
      else
        N(Int8, Int32Type);
      break;
    }
    case TypeIndex::Int16: {
      if (options.output_enum_as_byte_array && isEnum16(s.type))
        writeColumnImpl<parquet::ByteArrayType>(s, options, out, ConverterEnumAsString<Int16>(s.primitive_column, s.type));
      else
        N(Int16, Int32Type);
      break;
    }
...
#undef N
    ...
    case TypeIndex::Float32:
      writeColumnImpl<parquet::FloatType>(s, options, out, ConverterNumeric<ColumnVector<Float32>, Float32, Float32>(s.primitive_column));
      break;

    case TypeIndex::Float64:
      writeColumnImpl<parquet::DoubleType>(s, options, out, ConverterNumeric<ColumnVector<Float64>, Float64, Float64>(s.primitive_column));
      break;

    case TypeIndex::DateTime64:
      if (s.datetime_multiplier == 1)
        writeColumnImpl<parquet::Int64Type>(s, options, out, ConverterNumeric<ColumnDecimal<DateTime64>, Int64, Int64>(s.primitive_column));
      else
        writeColumnImpl<parquet::Int64Type>(s, options, out, ConverterDateTime64WithMultiplier(s.primitive_column, s.datetime_multiplier));
      break;

    case TypeIndex::IPv4:
      writeColumnImpl<parquet::Int32Type>(s, options, out, ConverterNumeric<ColumnVector<IPv4>, Int32, UInt32>(s.primitive_column));
      break;

    case TypeIndex::String:
      writeColumnImpl<parquet::ByteArrayType>(s, options, out, ConverterString(s.primitive_column));
      break;

    case TypeIndex::FixedString:
      if (options.output_fixed_string_as_fixed_byte_array)
        writeColumnImpl<parquet::FLBAType>(s, options, out, ConverterFixedString(s.primitive_column));
      else
        writeColumnImpl<parquet::ByteArrayType>(s, options, out, ConverterFixedStringAsString(s.primitive_column));
      break;
    case TypeIndex::Object:
      writeColumnImpl<parquet::ByteArrayType>(s, options, out, ConverterJSON(s.primitive_column, s.type, format_settings));
      break;

#define F(source_type) writeColumnImpl<parquet::FLBAType>(s, options, out, ConverterNumberAsFixedString<source_type>(s.primitive_column))
...
#define D(source_type) writeColumnImpl<parquet::FLBAType>(s, options, out, ConverterDecimal<source_type>(s.primitive_column))
...
void finalizeColumnChunkAndWriteFooter(ColumnChunkWriteState s, FileWriteState &file, WriteBuffer &out) {
  for (auto &location : s.indexes.offset_index.page_locations) location.offset += file.offset;

  if (s.column_chunk.meta_data.data_page_offset != -1) s.column_chunk.meta_data.data_page_offset += file.offset;
  if (s.column_chunk.meta_data.__isset.dictionary_page_offset) s.column_chunk.meta_data.dictionary_page_offset += file.offset;
...
static void flushBloomFilters(FileWriteState &file, WriteBuffer &out) {
  while (file.row_groups_with_flushed_bloom_filter < file.completed_row_groups.size()) {
    ...
    for (size_t col_idx = 0; col_idx < rg.column_indexes.size(); ++col_idx) {
      ...
      if (indexes.bloom_filter_data.empty()) continue;
...
void finalizeRowGroup(FileWriteState &file, size_t num_rows, const WriteOptions &options, WriteBuffer &out) {
  ...
  if (!file.completed_row_groups.empty()) chassert(r.columns.size() == file.completed_row_groups[0].row_group.columns.size());
...
void writeFileFooter(FileWriteState &file, SchemaElements schema, const WriteOptions &options, WriteBuffer &out, const Block &header) {
  chassert(file.offset != 0);
  chassert(file.current_row_group.row_group.columns.empty());

  if (options.write_bloom_filter) flushBloomFilters(file, out);

  if (options.write_page_index) writePageIndex(file, out);
  ...
  meta.__set_created_by(
      fmt::format("ClickHouse version {}.{}.{} (build {})", VERSION_MAJOR, VERSION_MINOR, VERSION_PATCH, VERSION_GITHASH));

  if (options.write_page_statistics || options.write_column_chunk_statistics) {
    meta.__set_column_orders({});
    for (auto &s : meta.schema)
      if (!s.__isset.num_children) meta.column_orders.emplace_back();
    for (auto &c : meta.column_orders) c.__set_TYPE_ORDER({});
  }
  ...
  if (options.write_geometadata) {
    ...
    for (const auto &[column_name, type] : header.getNamesAndTypesList()) {
      if (type->getCustomName() && (type->getCustomName()->getName() == WKBPointTransform::name ||
  ...
  if (footer_size > INT32_MAX) throw Exception(ErrorCodes::LIMIT_EXCEEDED, "Parquet file metadata too big: {}", footer_size);
...
size_t ColumnChunkWriteState::allocatedBytes() const {
  ...
  if (primitive_column) r += primitive_column->allocatedBytes();
  return r;
}
