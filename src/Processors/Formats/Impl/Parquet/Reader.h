...
//           insert into function file('p4.parquet') select [((number*100 + 10,number*100+11),number*100+20)::Tuple(x Tuple(x Int64, y
//           String), y Int64)] as a from numbers(3)
...
//     - prewhere and other skipping with defaults (it probably doesn't fill them correctly, see
//     MergeTreeRangeReader::executeActionsBeforePrewhere)
...
//     - `insert into function file('t.parquet') select number as k, toString(number)||':'||randomPrintableASCII(1000) as v from
//     numbers(1000000) settings engine_file_truncate_on_insert=1; select count(), sum(length(v)) from file('t.parquet')` - new reader is
//     slower than default
...
//     - input_format_parquet_skip_columns_with_unsupported_types_in_schema_inference, make a file by hacking PrepareForWrite.cpp to use
//     garbage physical type for some data type (~/t/invalid_type.parquet has column `invalid` that's FixedString(5) but has type=42 instead
//     of type=FIXED_LEN_BYTE_ARRAY)
...
struct Reader {
  ...
  void determinePagesToPrefetch(ColumnChunk &column, const RowSubgroup &row_subgroup, const RowGroup &row_group,
                                std::vector<PrefetchHandle *> &out);

  /// Guess how much memory ColumnSubchunk::{column, arrays_offsets} will use, per row.
  double estimateColumnMemoryBytesPerRow(const ColumnChunk &column, const RowGroup &row_group,
                                         const PrimitiveColumnInfo &column_info) const;

  void decodePrimitiveColumn(ColumnChunk &column, const PrimitiveColumnInfo &column_info, ColumnSubchunk &subchunk,
 ...
 private:
  ...
  void decodeDictionaryPageImpl(const parq::PageHeader &header, std::span<const char> data, ColumnChunk &column,
  ...
  bool initializeDataPage(const char *&data_ptr, const char *data_end, size_t next_row_idx, std::optional<size_t> end_row_idx,
  ...
  void readRowsInPage(size_t end_row_idx, ColumnSubchunk &subchunk, ColumnChunk &column, const PrimitiveColumnInfo &column_info,
                      const RowSubgroup *row_subgroup = nullptr);
};
