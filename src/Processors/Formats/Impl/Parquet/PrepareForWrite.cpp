...
void updateRepDefLevelsAndFilterColumnForNullable(ColumnChunkWriteState &s, const NullMap &null_map) {
  ...
  if (s.max_def == 1) {
    ...
    for (size_t i = 0; i < s.def.size(); ++i) s.def[i] = !null_map[i];
  ...
  for (size_t i = 0; i < s.def.size(); ++i) {
    ...
    if (s.def[i] == s.max_def - 1) filter.push_back(!null_map[row_idx]);
...
parq::CompressionCodec::type compressionMethodToParquet(CompressionMethod c) {
  switch (c) {
    case CompressionMethod::None:
      return parq::CompressionCodec::UNCOMPRESSED;
    case CompressionMethod::Snappy:
      return parq::CompressionCodec::SNAPPY;
    case CompressionMethod::Gzip:
      return parq::CompressionCodec::GZIP;
    case CompressionMethod::Brotli:
      return parq::CompressionCodec::BROTLI;
    case CompressionMethod::Lz4:
      return parq::CompressionCodec::LZ4_RAW;
    case CompressionMethod::Zstd:
      return parq::CompressionCodec::ZSTD;

    default:
      throw Exception(ErrorCodes::UNKNOWN_COMPRESSION_METHOD, "Compression method {} is not supported by Parquet",
                      toContentEncodingName(c));
  }
}

/// Depth-first traversal of the schema tree for this column.
void prepareColumnRecursive(ColumnPtr column, DataTypePtr type, const std::string &name, const WriteOptions &options,
                            ColumnChunkWriteStates &states, SchemaElements &schemas,
                            const std::optional<std::unordered_map<String, Int64>> &column_field_ids);

void preparePrimitiveColumn(ColumnPtr column, DataTypePtr type, const std::string &name, const WriteOptions &options,
  ...
  if (field_id) schema.__set_field_id(static_cast<Int32>(*field_id));
  ...
  auto types = [&](T::type type_, std::optional<C::type> converted = std::nullopt,
                   std::optional<parq::LogicalType> logical = std::nullopt) {
    state.column_chunk.meta_data.__set_type(type_);
    schema.__set_type(type_);
    if (converted) schema.__set_converted_type(*converted);
    if (logical) schema.__set_logicalType(*logical);
  };
  ...
  auto fixed_string = [&](size_t size, std::optional<C::type> converted = std::nullopt,
                          std::optional<parq::LogicalType> logical = std::nullopt) {
  ...
  switch (type->getTypeId()) {
    ...
    case TypeIndex::UInt32:
      types(T::INT32, C::UINT_32, int_type(32, false));
      break;
    case TypeIndex::UInt64:
      types(T::INT64, C::UINT_64, int_type(64, false));
      break;
    case TypeIndex::Int8:
    ...
    case TypeIndex::FixedString: {
      if (options.output_fixed_string_as_fixed_byte_array && type->getTypeId() == TypeIndex::FixedString) {
    ...
    case TypeIndex::Decimal32:
      decimal(4, getDecimalPrecision(*type), getDecimalScale(*type));
      break;
    case TypeIndex::Decimal64:
      decimal(8, getDecimalPrecision(*type), getDecimalScale(*type));
      break;
    case TypeIndex::Decimal128:
      decimal(16, getDecimalPrecision(*type), getDecimalScale(*type));
      break;
    case TypeIndex::Decimal256:
      decimal(32, getDecimalPrecision(*type), getDecimalScale(*type));
      break;

    default:
      throw Exception(ErrorCodes::UNKNOWN_TYPE,
                      "Internal type '{}' of column '{}' is not supported for conversion into Parquet data format.", type->getFamilyName(),
                      name);
  }
}

void prepareColumnNullable(ColumnPtr column, DataTypePtr type, const std::string &name, const WriteOptions &options,
                           ColumnChunkWriteStates &states, SchemaElements &schemas,
                           const std::optional<std::unordered_map<String, Int64>> &field_ids) {
...
void prepareColumnTuple(ColumnPtr column, DataTypePtr type, const std::string &name, const WriteOptions &options,
                        ColumnChunkWriteStates &states, SchemaElements &schemas) {
  ...
  if (num_elements == 0) throw Exception(ErrorCodes::BAD_ARGUMENTS, "Parquet doesn't support empty tuples");
  ...
  for (size_t i = 0; i < num_elements; ++i)
    prepareColumnRecursive(column_tuple->getColumnPtr(i), type_tuple->getElement(i), type_tuple->getNameByPosition(i + 1), options, states,
                           schemas, std::nullopt);
...
void prepareColumnArray(ColumnPtr column, DataTypePtr type, const std::string &name, const WriteOptions &options,
                        ColumnChunkWriteStates &states, SchemaElements &schemas) {
...
void prepareColumnMap(ColumnPtr column, DataTypePtr type, const std::string &name, const WriteOptions &options,
                      ColumnChunkWriteStates &states, SchemaElements &schemas) {
...
void prepareColumnRecursive(ColumnPtr column, DataTypePtr type, const std::string &name, const WriteOptions &options,
  ...
  switch (type->getTypeId()) {
    case TypeIndex::Nullable:
      prepareColumnNullable(column, type, name, options, states, schemas, column_field_ids);
      break;
    case TypeIndex::Array:
      prepareColumnArray(column, type, name, options, states, schemas);
      break;
    case TypeIndex::Tuple:
      prepareColumnTuple(column, type, name, options, states, schemas);
      break;
    case TypeIndex::Map:
      prepareColumnMap(column, type, name, options, states, schemas);
      break;
    case TypeIndex::LowCardinality: {
      auto nested_type = assert_cast<const DataTypeLowCardinality &>(*type).getDictionaryType();
      if (nested_type->isNullable())
        prepareColumnNullable(column->convertToFullColumnIfLowCardinality(), nested_type, name, options, states, schemas, column_field_ids);
      else
        ...
        preparePrimitiveColumn(column, nested_type, name, options, states, schemas,
                               column_field_ids ? std::optional(column_field_ids->at(name)) : std::nullopt);
      break;
    }
    default:
...
SchemaElements convertSchema(const Block &sample, const WriteOptions &options,
  ...
  for (const auto &c : sample) prepareColumnForWrite(c.column, c.type, c.name, options, nullptr, &schema, column_field_ids);
...
void prepareGeoColumn(ColumnPtr &column, DataTypePtr &type) {
  ...
  if (type->getCustomName()->getName() == WKBPointTransform::name) transform = std::make_shared<WKBPointTransform>();
  if (type->getCustomName()->getName() == WKBLineStringTransform::name) transform = std::make_shared<WKBLineStringTransform>();
  if (type->getCustomName()->getName() == WKBPolygonTransform::name) transform = std::make_shared<WKBPolygonTransform>();
  if (type->getCustomName()->getName() == WKBMultiLineStringTransform::name) transform = std::make_shared<WKBMultiLineStringTransform>();
  if (type->getCustomName()->getName() == WKBMultiPolygonTransform::name) transform = std::make_shared<WKBMultiPolygonTransform>();
...
void prepareColumnForWrite(ColumnPtr column, DataTypePtr type, const std::string &name, const WriteOptions &options,
  ...
  if (options.write_geometadata) prepareGeoColumn(column, type);
  prepareColumnRecursive(column, type, name, options, states, schemas, column_field_ids);

  if (out_columns_to_write)
    for (auto &s : states) out_columns_to_write->push_back(std::move(s));
  if (out_schema) out_schema->insert(out_schema->end(), schemas.begin(), schemas.end());

  if (column->empty()) states.clear();
}
