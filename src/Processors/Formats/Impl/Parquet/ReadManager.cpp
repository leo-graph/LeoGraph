...
void AtomicBitSet::resize(size_t bits) { a = std::vector<std::atomic<UInt64>>((bits + 63) / 64); }

std::optional<size_t> AtomicBitSet::findFirst() {
  for (size_t i = 0; i < a.size(); ++i) {
    UInt64 x = a[i].load(std::memory_order_relaxed);
    if (x) return (i << 6) + getTrailingZeroBitsUnsafe(x);
  }
  return std::nullopt;
}
...
void ReadManager::init(FormatParserSharedResourcesPtr parser_shared_resources_,
  ...
  if (reader.file_metadata.schema.empty()) reader.file_metadata = Reader::readFileMetaData(reader.prefetcher);

  if (buckets_to_read_) {
    row_groups_to_read = std::unordered_set<UInt64>{};
    for (auto rg : *buckets_to_read_) row_groups_to_read->insert(rg);
  }
  ...
  for (const Stage &stage : stages) sum += stage.memory_target_fraction;
  for (Stage &stage : stages) stage.memory_target_fraction /= sum;

  /// The NotStarted stage completed for all row groups, transition to next stage.
  MemoryUsageDiff diff(ReadStage::NotStarted);
  for (size_t i = 0; i < reader.row_groups.size(); ++i) finishRowGroupStage(i, ReadStage::NotStarted, diff);
  flushMemoryUsageDiff(std::move(diff));
}

ReadManager::~ReadManager() { shutdown->shutdown(); }
...
void ReadManager::finishRowGroupStage(size_t row_group_idx, ReadStage stage, MemoryUsageDiff &diff) {
  ...
  if (stage == ReadStage::BloomFilterBlocksOrDictionary) {
    if (!reader.applyBloomAndDictionaryFilters(row_group)) stage = ReadStage::Deliver;  // skip the row group
    for (auto &c : row_group.columns) {
      c.bloom_filter_header_prefetch.reset(&diff);
      c.bloom_filter_data_prefetch.reset(&diff);
      for (auto &b : c.bloom_filter_blocks) b.prefetch.reset(&diff);
  ...
  while (true)  // loop over skipped stages
  {
    ...
    switch (stage) {
      ...
      case ReadStage::BloomFilterHeader:
        for (size_t i = 0; i < row_group.columns.size(); ++i)
          if (row_group.columns[i].bloom_filter_header_prefetch)
            add_tasks.push_back(Task{.stage = ReadStage::BloomFilterHeader, .row_group_idx = row_group_idx, .column_idx = i});
        break;
      case ReadStage::BloomFilterBlocksOrDictionary:
        for (size_t i = 0; i < row_group.columns.size(); ++i) {
          ...
          if (!c.bloom_filter_blocks.empty() || c.use_dictionary_filter)
            add_tasks.push_back(Task{.stage = ReadStage::BloomFilterBlocksOrDictionary, .row_group_idx = row_group_idx, .column_idx = i});
        }
        break;
      case ReadStage::ColumnIndexAndOffsetIndex:
        for (size_t i = 0; i < row_group.columns.size(); ++i)
          if (row_group.columns[i].use_column_index)
            add_tasks.push_back(Task{.stage = ReadStage::ColumnIndexAndOffsetIndex, .row_group_idx = row_group_idx, .column_idx = i});
        break;
      ...
      case ReadStage::Deallocated:
        ...
        if (row_group.subgroups.empty()) {
          for (auto &c : row_group.columns) clearColumnChunk(c, diff);
        }
        break;
    }

    if (!add_tasks.empty() || stage == ReadStage::Deallocated) break;
  ...
  if (!add_tasks.empty()) setTasksToSchedule(row_group_idx, stage, std::move(add_tasks), diff);
}

void ReadManager::setTasksToSchedule(size_t row_group_idx, ReadStage stage, std::vector<Task> add_tasks, MemoryUsageDiff &diff) {
  LOG_DEBUG(getLogger("ParquetReadManager"), "setTasksToSchedule: row_group_idx={}, stage={}, add_tasks={}", row_group_idx,
            static_cast<Int32>(stage), add_tasks.size());
  for (const auto &task : add_tasks) {
    LOG_DEBUG(getLogger("ParquetReadManager"), "setTasksToSchedule: {} {} {} {} {}", task.column_idx, task.row_group_idx,
              task.cost_estimate_bytes, static_cast<Int32>(task.stage), task.step_idx);
  }
  ...
  bool changed =
      stage_state.schedulable_row_groups.set(row_group_idx, std::memory_order_release);  /// NOLINT(clang-analyzer-deadcode.DeadStores)
...
void ReadManager::addTasksToReadColumns(size_t row_group_idx, size_t row_subgroup_idx, ReadStage stage, size_t step_idx,
  ...
  while (true)  // offset index, then data
  {
    ...
    for (size_t i = 0; i < reader.primitive_columns.size(); ++i) {
      if (reader.primitive_columns[i].first_step_to_calculate != step_idx) continue;

      ColumnChunk &c = row_group.columns.at(i);
      if (is_offset_index) {
        if (c.offset_index_prefetch && c.offset_index.page_locations.empty()) {
          LOG_DEBUG(getLogger("ParquetReadManager"),
                    "addTasksToReadColumns: added OffsetIndex: i={} step_idx={} row_group_idx={} row_subgroup_idx={}", i, step_idx,
          ...
          add_tasks.push_back(Task{.stage = ReadStage::OffsetIndex,
                                   .step_idx = step_idx,
          ...
          LOG_DEBUG(getLogger("ParquetReadManager"),
                    "addTasksToReadColumns: not added due locations empty i={} step_idx={} row_group_idx={} row_subgroup_idx={}", i,
                    step_idx, row_group_idx, row_subgroup_idx);
        }
      } else {
        LOG_DEBUG(getLogger("ParquetReadManager"),
                  "addTasksToReadColumns: added ColumnData: i={} step_idx={} row_group_idx={} row_subgroup_idx={}", i, step_idx,
                  row_group_idx, row_subgroup_idx);
        add_tasks.push_back(Task{.stage = ReadStage::ColumnData,
                                 .step_idx = step_idx,
                                 .row_group_idx = row_group_idx,
                                 .row_subgroup_idx = row_subgroup_idx,
                                 .column_idx = i});
...
void ReadManager::finishRowSubgroupStage(size_t row_group_idx, size_t row_subgroup_idx, ReadStage stage, size_t step_idx,
                                         MemoryUsageDiff &diff) {
  RowGroup &row_group = reader.row_groups[row_group_idx];
  RowSubgroup &row_subgroup = row_group.subgroups[row_subgroup_idx];
  ...
  LOG_DEBUG(getLogger("ParquetReadManager"), "finishRowSubgroupStage: rg={} sg={} stage={} step={} rows_pass={} rows_total={}",
            row_group_idx, row_subgroup_idx, magic_enum::enum_name(stage), step_idx, row_subgroup.filter.rows_pass,
  ...
  switch (stage) {
    ...
    case ReadStage::ColumnData: {
      if (row_subgroup.filter.rows_pass == 0) break;
      if (step_idx > 0 && step_idx <= reader.steps.size()) {
        ...
        LOG_DEBUG(getLogger("ParquetReadManager"),
                  "finishRowSubgroupStage: rg={} sg={} main step finished, moving to Deliver, advancing read_ptr", row_group_idx,
                  row_subgroup_idx);
        ...
        /// Must add to delivery_queue before advancing read_ptr to deliver subgroups in order.
        {
          ...
          LOG_DEBUG(getLogger("ParquetReadManager"), "finishRowSubgroupStage: rg={} sg={} added to delivery_queue, size={}", row_group_idx,
                    row_subgroup_idx, delivery_queue.size());
        }

        row_group.read_ptr.store(row_subgroup_idx + 1);
        advanced_ptr = row_subgroup_idx + 1;
        LOG_DEBUG(getLogger("ParquetReadManager"), "finishRowSubgroupStage: rg={} sg={} advanced read_ptr -> {}", row_group_idx,
                  row_subgroup_idx, row_subgroup_idx + 1);
  ...
  LOG_DEBUG(getLogger("ParquetReadManager"),
            "finishRowSubgroupStage: rg={} starting next subgroup, read_ptr={} subgroups={} delivery_ptr={}", row_group_idx, main_ptr,
            row_group.subgroups.size(), row_group.delivery_ptr.load());
  ...
  while (main_ptr < row_group.subgroups.size()) {
    ...
    if (!next_subgroup.stage.compare_exchange_strong(next_subgroup_stage, ReadStage::OffsetIndex)) break;
  ...
  if (advanced_ptr.has_value()) {
    ...
    if (*advanced_ptr == row_group.subgroups.size()) {
      ...
      for (size_t i = 0; i < reader.primitive_columns.size(); ++i) clearColumnChunk(row_group.columns.at(i), diff);
    }
  }
}
...
void ReadManager::advanceDeliveryPtrIfNeeded(size_t row_group_idx, MemoryUsageDiff &diff) {
  ...
  while (delivery_ptr < row_group.subgroups.size() && row_group.subgroups[delivery_ptr].stage.load() == ReadStage::Deallocated) {
    if (!row_group.delivery_ptr.compare_exchange_weak(delivery_ptr, delivery_ptr + 1)) continue;
    ...
    LOG_DEBUG(getLogger("ParquetReadManager"), "advanceDeliveryPtrIfNeeded: rg={} advanced delivery_ptr {} -> {}", row_group_idx,
              old_delivery_ptr, delivery_ptr);
  }

  if (delivery_ptr > initial_delivery_ptr) {
    LOG_DEBUG(getLogger("ParquetReadManager"), "advanceDeliveryPtrIfNeeded: rg={} final delivery_ptr={} read_ptr={}", row_group_idx,
              row_group.delivery_ptr.load(), row_group.read_ptr.load());
  }
}

static bool checkTaskSchedulingLimits(size_t memory_usage, size_t added_memory, size_t batches_in_progress, size_t added_tasks,
                                      const SharedResourcesExt::Limits &limits) {
  if (added_tasks == 0) {
    ...
    return added_memory < limits.memory_low_watermark ||
           (memory_usage + added_memory <= limits.memory_high_watermark && added_tasks < limits.parsing_threads);
  }
}
...
void ReadManager::flushMemoryUsageDiff(MemoryUsageDiff &&diff) {
  ...
  for (size_t i = 0; i < diff.by_stage.size(); ++i) {
    ...
    if (!should_schedule && d < 0) {
      ...
      should_schedule = checkTaskSchedulingLimits(stage.memory_usage.load(std::memory_order_relaxed), 0,
                                                  stage.batches_in_progress.load(std::memory_order_relaxed), 0, limits);
    }
    if (should_schedule) scheduleTasksIfNeeded(ReadStage(i));
  }
}
...
void ReadManager::scheduleTasksIfNeeded(ReadStage stage_idx) {
  ...
  LOG_DEBUG(getLogger("ParquetReadManager"),
            "scheduleTasksIfNeeded: stage={} memory_usage={} batches_in_progress={} limits: mem_low={} mem_high={} threads={}",
            magic_enum::enum_name(stage_idx), memory_usage, batches_in_progress, limits.memory_low_watermark, limits.memory_high_watermark,
            limits.parsing_threads);
  ...
  auto is_privileged_task = [&](size_t row_group_idx) {
    size_t i = first_incomplete_row_group.load();
    if (row_group_idx != i) return false;
    ...
    if (row_group.stage.load(std::memory_order_acquire) < ReadStage::ColumnData) return false;
  ...
  while (true) {
    ...
    bool can_schedule =
        checkTaskSchedulingLimits(memory_usage, size_t(diff.by_stage[size_t(stage_idx)]), batches_in_progress, tasks.size(), limits);
    ...
    if (!can_schedule && !is_privileged) break;
    ...
    LOG_DEBUG(getLogger("ParquetReadManager"), "scheduleTasksIfNeeded: want to schedule tasks rg={}, stage_tasks.size()={}", row_group_idx,
              stage_tasks.size());
    chassert(!stage_tasks.empty());
    for (size_t i = 0; i < stage_tasks.size(); ++i) scheduleTask(stage_tasks[i], i == 0, diff, tasks);
    stage_tasks.clear();
  }
  ...
  if (!tasks.empty()) {
    ...
    LOG_DEBUG(getLogger("ParquetReadManager"), "scheduleTasksIfNeeded: stage={} no tasks to schedule", magic_enum::enum_name(stage_idx));
  }
}

void ReadManager::scheduleTask(Task task, bool is_first_in_group, MemoryUsageDiff &diff, std::vector<Task> &out_tasks) {
  LOG_DEBUG(getLogger("ParquetReadManager"),
            "scheduleTask: schedule task.row_group_idx={}, task.row_subgroup_idx={}, task.stage={}, task.column_idx={}, task.step_idx={}",
            task.row_group_idx, task.row_subgroup_idx, static_cast<Int32>(task.stage), task.column_idx, task.step_idx);
  ...
  if (task.column_idx != UINT64_MAX) {
    ...
    switch (task.stage) {
      case ReadStage::BloomFilterHeader:
        ...
        break;
      case ReadStage::BloomFilterBlocksOrDictionary:
        if (column.use_dictionary_filter) prefetches.push_back(&column.dictionary_page_prefetch);
        for (auto &b : column.bloom_filter_blocks) prefetches.push_back(&b.prefetch);
        break;
      case ReadStage::ColumnIndexAndOffsetIndex: {
  ...
  if (task.stage == ReadStage::ColumnData && is_first_in_group) {
    ...
    if (!row_subgroup.filter.memory) row_subgroup.filter.memory = MemoryUsageToken(row_subgroup.filter.rows_total, &diff);
  }
...
void ReadManager::runBatchOfTasks(const std::vector<Task> &tasks) noexcept {
  ...
  try {
    ...
    if (column_idx != UINT64_MAX) e.addMessage("column: {}", reader.primitive_columns[column_idx].name);
...
void ReadManager::runTask(Task task, bool last_in_batch, MemoryUsageDiff &diff) {
  ...
  if (task.column_idx != UINT64_MAX) {
    ...
    switch (task.stage) {
      ...
      case ReadStage::ColumnData: {
        ...
        reader.decodePrimitiveColumn(column, column_info, row_subgroup.columns.at(task.column_idx), row_group, row_subgroup);
  ...
  if (last_in_batch) {
    ...
    size_t prev_batches_in_progress =
        stages.at(size_t(task.stage))
            .batches_in_progress.fetch_sub(1, std::memory_order_relaxed);  /// NOLINT(clang-analyzer-deadcode.DeadStores)
  ...
  if (task.row_subgroup_idx != UINT64_MAX) {
    ...
    if (remaining == 1) finishRowSubgroupStage(task.row_group_idx, task.row_subgroup_idx, task.stage, task.step_idx, diff);
  } else {
    size_t remaining = row_group.stage_tasks_remaining.fetch_sub(1);
    chassert(remaining > 0);
    if (remaining == 1) finishRowGroupStage(task.row_group_idx, task.stage, diff);
  }
}
...
void ReadManager::clearColumnChunk(ColumnChunk &column, MemoryUsageDiff &diff) {
  ...
  for (auto &page : column.data_pages) page.prefetch.reset(&diff);
  column.bloom_filter_header_prefetch.reset(&diff);
  ...
  column.offset_index_prefetch.reset(&diff);
  column.data_pages_prefetch.reset(&diff);
  for (auto &block : column.bloom_filter_blocks) block.prefetch.reset(&diff);
...
void ReadManager::clearRowSubgroup(RowSubgroup &row_subgroup, MemoryUsageDiff &diff) {
  ...
  for (ColumnSubchunk &col : row_subgroup.columns) col.column_and_offsets_memory.reset(&diff);
}
...
std::string ReadManager::collectDeadlockDiagnostics() {
  ...
  for (size_t i = 0; i < size_t(ReadStage::Deallocated); ++i) {
    ...
    for (const auto &tasks : stage.row_group_tasks_to_schedule) tasks_to_schedule += tasks.size();
  ...
  for (size_t rg_idx = 0; rg_idx < reader.row_groups.size(); ++rg_idx) {
    ...
    result += " del_ptr: " + std::to_string(row_group.delivery_ptr.load(std::memory_order_relaxed)) + "/" +
              std::to_string(row_group.subgroups.size());
    ...
    result += " subrgs: ns=" + std::to_string(subgroups_not_started) + ", in_pr=" + std::to_string(subgroups_in_progress) +
              ", deliv=" + std::to_string(subgroups_delivered) + ", deal=" + std::to_string(subgroups_deallocated) + " ";
  }
...
ReadManager::ReadResult ReadManager::read() {
  Task task;
  {
    ...
    while (true) {
      ...
      if (exception) std::rethrow_exception(copyMutableException(exception));
      ...
      bool can_deliver =
          !delivery_queue.empty() && (!reader.options.format.parquet.preserve_order || delivery_queue.top().row_group_idx == first_inc);
      ...
      if (can_deliver) {
        ...
        delivery_queue.pop();
        LOG_DEBUG(getLogger("ParquetReadManager"), "read: delivering task rg={} sg={}", task.row_group_idx, task.row_subgroup_idx);
        break;
      }
      ...
      if (first_incomplete_row_group.load(std::memory_order_relaxed) == reader.row_groups.size()) {
        ...
        for (const RowGroup &row_group : reader.row_groups) {
          ...
          for (size_t i = 0; i < stages.size(); ++i) {
            ...
            size_t unsched = 0;
            for (const auto &tasks : stages[i].row_group_tasks_to_schedule) unsched += tasks.size();
            if (mem != 0 || batches != 0 || unsched != 0)
              throw Exception(ErrorCodes::LOGICAL_ERROR,
                              "Leak in memory or task accounting in parquet reader: got {} bytes, {} batches, {} tasks in stage {}", mem,
                              batches, unsched, i);
          }
        }
        return {};
  ...
  for (size_t i = 0; i < output_columns.size(); ++i) output_columns[i] = std::move(reader.getOrFormOutputColumn(row_subgroup, i));
  ...
  auto row_numbers_info = std::make_shared<ChunkInfoRowNumbers>(row_subgroup.start_row_idx + row_group.start_global_row_idx);
  ...
  size_t virtual_bytes_read = size_t(row_group.meta->total_compressed_size) * row_subgroup.filter.rows_total /
