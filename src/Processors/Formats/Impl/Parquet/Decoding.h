...
struct Dictionary {
  ...
  void decode(parq::Encoding::type encoding, const PageDecoderInfo &info, size_t num_values, std::span<const char> data_,
...
struct PageDecoder {
  ...
  void requireRemainingBytes(size_t n) const {
    if (size_t(end - data) < n) throw Exception(ErrorCodes::INCORRECT_DATA, "Unexpected end of page data");
  }
};
...
struct StringConverter {
  ...
  virtual void convertColumn(std::span<const char> chars, const UInt64 *offsets, size_t separator_bytes, size_t num_values,
                             IColumn &) const = 0;
  virtual void convertField(std::span<const char> /*data*/, bool /*is_max*/, Field &) const {
...
struct TrivialStringConverter : public StringConverter {
  ...
  void convertColumn(std::span<const char> chars, const UInt64 *offsets, size_t separator_bytes, size_t num_values,
                     IColumn &col) const override;
  void convertField(std::span<const char> data, bool /*is_max*/, Field &out) const override;
};
...
struct BigEndianDecimalFixedSizeConverter : public FixedSizeConverter {
  ...
  BigEndianDecimalFixedSizeConverter(size_t input_size_, UInt32 scale_) : scale(scale_), helper(input_size_) { input_size = input_size_; }
