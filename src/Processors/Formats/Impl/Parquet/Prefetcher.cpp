...
void Prefetcher::determineReadModeAndFileSize(ReadBuffer *reader_, const ReadOptions &options) {
  if (options.seekable_read) {
    ...
    if (has_file_size && seekable) {
      ...
      if (reader) file_size = getFileSizeFromReadBuffer(*seekable);
    }
  }
...
void Prefetcher::readSync(char *to, size_t n, size_t offset) {
  ...
  if (nread != n)
    throw Exception(ErrorCodes::INCORRECT_DATA, "Unexpected eof: offset {}, length {}, bytes read {}, expected file size {}", offset, n,
                    nread, file_size);
}
...
void Prefetcher::finalizeRanges() {
  ...
  std::sort(ranges.begin(), ranges.end(), [](const RangeState &a, const RangeState &b) { return a.start < b.start; });
...
void Prefetcher::startPrefetch(const std::vector<PrefetchHandle *> &requests_to_start, MemoryUsageDiff *diff) {
  ...
  for (const PrefetchHandle *handle : requests_to_start)
    if (*handle) handle->request->allow_incidental_read.store(true, std::memory_order_relaxed);
...
std::vector<PrefetchHandle> Prefetcher::splitRange(PrefetchHandle request,
                                                   const std::vector<std::pair</*global_offset*/ size_t, /*length*/ size_t>> &subranges,
                                                   bool likely_to_be_used) {
  chassert(ranges_finalized.load(std::memory_order_relaxed));
...

  {
    ...
    for (size_t i = 0; i < subranges.size(); ++i) out_handles.push_back(PrefetchHandle(&requests.emplace_back()));
    ...
    if (parent_req->state.load(std::memory_order_relaxed) == RequestState::State::HasRange) {
      ...
      for (const auto &[start, length] : subranges) {
        if (start < range.start || length > range.end - start)
          throw Exception(ErrorCodes::INCORRECT_DATA, "Subrange out of bounds: [{}, {}) not in [{}, {})", start, start + length,
...
void Prefetcher::pickRangesAndCreateTaskIfNotExists(RequestState *initial_req, const PrefetchHandle &, bool splitting, size_t start_offset,
                                                    size_t end_offset, std::unique_lock<std::mutex> lock) {
  ...
  for (size_t idx = range_idx + 1; idx < ranges.size(); ++idx) {
    ...
    if (end_offset + min_bytes_for_seek <= r.start || initial_offset + bytes_per_read_task <= r.end ||
...
void Prefetcher::decreaseTaskRefcount(Task *task, size_t amount) {
  ...
  if (task->state.exchange(Task::State::Deallocated) != Task::State::Running) task->buf = {};
}
...
void Prefetcher::scheduleTask(Task *task) {
  if (parser_shared_resources && !parser_shared_resources->io_runner.isDisabled())
    parser_shared_resources->io_runner([this, task, _shutdown = shutdown] {
      ...
      if (!shutdown_lock.owns_lock()) return;
      runTask(task);
    });
}
...
std::span<const char> Prefetcher::getRangeData(const PrefetchHandle &request) {
  ...
  if (s == Task::State::Exception) rethrowException(task);
...
Prefetcher::Task::State Prefetcher::runTask(Task *task) {
  ...
  if (!task->state.compare_exchange_strong(s, Task::State::Running)) return s;
...
PrefetchHandle::PrefetchHandle(PrefetchHandle &&rhs) noexcept { *this = std::move(rhs); }
...
PrefetchHandle::~PrefetchHandle() { reset(nullptr); }

void PrefetchHandle::reset(MemoryUsageDiff *diff) {
  if (!request) return;

  if (diff) memory.reset(diff);
