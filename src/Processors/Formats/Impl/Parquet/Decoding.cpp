...
struct BitPackedRLEDecoder : public PageDecoder {
  ...
  BitPackedRLEDecoder(std::span<const char> data_, size_t limit_, bool has_header_byte, bool has_length_bytes = false)
    ...
    if (has_header_byte) {
      ...
      data += 1;
      if (bit_width > 8 * sizeof(T)) throw Exception(ErrorCodes::INCORRECT_DATA, "Invalid dict indices bit width: {}", bit_width);
    } else {
      chassert(limit > 0);
  ...
  void skip(size_t num_values) override { skipOrDecode<true>(num_values, nullptr); }
  ...
  void startRun() {
    ...
    if (len & 1) {
      ...
      if (val >= limit) throw Exception(ErrorCodes::INCORRECT_DATA, "Dict index or rep/def level out of bounds (rle)");
      run_is_rle = true;
      data += byte_width;
    }
  }
  ...
  void skipOrDecode(size_t num_values, T *out) {
    if (bit_width == 0) {
      ...
      if constexpr (!skip) memset(out, 0, num_values * sizeof(T));
      return;
    }
      ...
      if (run_is_rle) {
        ...
        if constexpr (!skip) {
          for (size_t i = 0; i < n; ++i) {
            ...
            if (x >= limit) throw Exception(ErrorCodes::INCORRECT_DATA, "Dict index or rep/def level out of bounds (bp)");
            *out = static_cast<T>(x);
            ++out;
            bit_idx += bit_width;
          }
...
struct PlainFixedSizeDecoder : public PageDecoder {
  ...
  PlainFixedSizeDecoder(std::span<const char> data_, std::shared_ptr<FixedSizeConverter> converter_)
  ...
  void decode(size_t num_values, IColumn &col, const UInt8 *filter, size_t filter_offset) override {
    ...
    for (size_t i = 0; i < num_values; ++i) {
      ...
      if (filter[filter_offset + i]) converter->convertColumn(std::span(from + i * value_size, value_size), 1, col);
      data += value_size;
    }
  }
};
...
struct PlainBooleanDecoder : public PageDecoder {
  ...
  PlainBooleanDecoder(std::span<const char> data_, std::shared_ptr<FixedSizeConverter> converter_)
      : PageDecoder(data_), converter(std::move(converter_)) {}

  void skip(size_t num_values) override { bit_idx += num_values; }
  ...
  void decode(size_t num_values, IColumn &col, const UInt8 *filter, size_t filter_offset) override {
    ...
    if ((end_bit_idx + 7) / 8 > size_t(end - data)) throw Exception(ErrorCodes::INCORRECT_DATA, "Unexpected end of page data");
    char *to;
    ...
    for (size_t i = 0; i < num_values; ++i) {
      ...
      if (filter[filter_offset + i]) to[out_i++] = bit;
    }
    bit_idx = end_bit_idx;
    if (!direct) converter->convertColumn(std::span(temp_buffer.data(), out_i), out_i, col);
  }
  ...
  void decodeNoFilter(size_t num_values, IColumn &col) {
    ...
    if (!direct) converter->convertColumn(std::span(to, num_values), num_values, col);
  }
};
...
struct PlainStringDecoder : public PageDecoder {
  ...
  PlainStringDecoder(std::span<const char> data_, std::shared_ptr<StringConverter> converter_)
      : PageDecoder(data_), converter(std::move(converter_)) {}
  ...
  void decode(size_t num_values, IColumn &col, const UInt8 *filter, size_t filter_offset) override {
    if (converter->isTrivial()) {
      ...
      for (size_t i = 0; filter && i < num_values; ++i) to_reserve += filter[filter_offset + i];
      ...
      for (size_t i = 0; i < num_values; ++i) {
        ...
        size_t len = 4 + size_t(x);
        requireRemainingBytes(len);
        if (!filter || filter[filter_offset + i]) col_str.insertData(data + 4, size_t(x));
        data += len;
      }
    } else {
      offsets.clear();
      if (filter) {
        ...
        size_t num_pass = 0;
        for (size_t i = 0; i < num_values; ++i) num_pass += filter[filter_offset + i];
        if (num_pass == 0) {
          for (size_t i = 0; i < num_values; ++i) {
            UInt32 x;
            memcpy(&x, data, 4);
...
struct DeltaBinaryPackedDecoder : public PageDecoder {
  ...
  DeltaBinaryPackedDecoder(std::span<const char> data_, std::shared_ptr<FixedSizeConverter> converter_)
      : PageDecoder(data_), converter(std::move(converter_)) {
    ...
    if (values_per_block == 0 || values_per_block % 128 != 0 || miniblocks_per_block == 0 || values_per_block % miniblocks_per_block != 0 ||
        values_per_block / miniblocks_per_block % 32 != 0)
    ...
    if (total_values_remaining > max_values)
      throw Exception(ErrorCodes::INCORRECT_DATA, "DELTA_BINARY_PACKED header claims {} values but data can contain at most {}",
                      total_values_remaining, max_values);
  }
  ...
  void nextMiniblock() {
    ++miniblock_idx;
    if (miniblock_idx >= miniblocks_per_block || miniblock_bit_widths == nullptr) nextBlock();
  ...
  void skip(size_t num_values) override {
    ...
    switch (converter->input_size) {
      case 4:
        decodeImpl<UInt32>(num_values, to, identity<UInt32>);
        break;
      case 8:
        decodeImpl<UInt64>(num_values, to, identity<UInt64>);
        break;
      default:
        chassert(false);
    }
  ...
  void decode(size_t num_values, IColumn &col, const UInt8 *filter, size_t filter_offset) override {
    ...
    for (size_t i = 0; i < num_values; ++i) {
      ...
      if (filter[filter_offset + i]) ++out_idx;
    }
    if (!direct && to) converter->convertColumn(std::span(to, pass_count * value_size), pass_count, col);
  }
  ...
  void decodeImplFull(size_t num_values, IColumn &col) {
    ...
    if (!direct) converter->convertColumn(std::span(to, num_values * converter->input_size), num_values, col);
  }
  ...
  void decodeImpl(size_t num_values, char *out_bytes, F func) {
    ...
    while (num_values) {
      if (!miniblock_values_remaining) nextMiniblock();
...
struct DeltaLengthByteArrayDecoder : public PageDecoder {
  ...
  DeltaLengthByteArrayDecoder(std::span<const char> data_, std::shared_ptr<StringConverter> converter_)
      : PageDecoder(data_), converter(std::move(converter_)) {
    ...
    lengths_decoder.decodeImpl<UInt64>(lengths_decoder.total_values_remaining, reinterpret_cast<char *>(offsets.data()), [&](UInt64 len) {
  ...
  void skip(size_t num_values) override {
    if (num_values > offsets.size() - idx) throw Exception(ErrorCodes::INCORRECT_DATA, "Too few values in page");
    idx += num_values;
  }
  ...
  void decode(size_t num_values, IColumn &col, const UInt8 *filter, size_t filter_offset) override {
    ...
    if (converter->isTrivial()) {
      ...
      for (size_t i = 0; i < num_values; ++i) {
        ...
        if (filter[filter_offset + i]) col_str.insertData(data + prev, len);
        prev = off[i];
      }
    } else {
      PaddedPODArray<char> chars_buf;
...
struct DeltaByteArrayDecoder : public PageDecoder {
  ...
  DeltaByteArrayDecoder(std::span<const char> data_, std::shared_ptr<StringConverter> string_converter_,
                        std::shared_ptr<FixedSizeConverter> fixed_size_converter_)
      : PageDecoder(data_), string_converter(std::move(string_converter_)), fixed_size_converter(fixed_size_converter_) {
    for (auto *lengths : {&prefixes, &suffixes}) {
      ...
      decoder.decodeImpl<UInt64>(decoder.total_values_remaining, reinterpret_cast<char *>(lengths->data()), identity<UInt64>);
      data = decoder.data;
    }

    if (prefixes.size() != suffixes.size()) throw Exception(ErrorCodes::INCORRECT_DATA, "Value count mismatch in DELTA_BYTE_ARRAY headers");
  }
  ...
  void decode(size_t num_values, IColumn &col, const UInt8 *filter, size_t filter_offset) override {
    if (fixed_size_converter) {
      ...
      if (!direct) fixed_size_converter->convertColumn(to, pass_count, col);
    } else {
      if (!filter) {
        ...
        if (direct)
          ...
          if (!temp_column) temp_column = ColumnString::create();
        ...
        if (!direct)
          string_converter->convertColumn(std::span(reinterpret_cast<char *>(col_str->getChars().data()), col_str->getChars().size()),
                                          col_str->getOffsets().data(), /*separator_bytes*/ 0, num_values, col);
        return;
      }
      size_t pass_count = 0;
      for (size_t i = 0; i < num_values; ++i) pass_count += filter[filter_offset + i];
      ...
      if (!direct)
        string_converter->convertColumn(std::span(reinterpret_cast<char *>(col_str->getChars().data()), col_str->getChars().size()),
                                        col_str->getOffsets().data(), /*separator_bytes*/ 0, pass_count, col);
    }
  }
  ...
  void decodeImpl(size_t num_values, ColumnString *out_str, char *out_fixed_size, const UInt8 *filter, size_t filter_offset) {
    if (num_values > prefixes.size() - idx) throw Exception(ErrorCodes::INCORRECT_DATA, "Too few values in page");
    size_t fixed_size = is_fixed_size ? fixed_size_converter->input_size : 0;

    for (size_t i = 0; i < num_values; ++i) {
      if (prefixes[idx] > current_value.size()) throw Exception(ErrorCodes::INCORRECT_DATA, "DELTA_BYTE_ARRAY too long");
...
struct ByteStreamSplitDecoder : public PageDecoder {
  ...
  ByteStreamSplitDecoder(std::span<const char> data_, std::shared_ptr<FixedSizeConverter> converter_)
      : PageDecoder(data_), converter(std::move(converter_)) {
  ...
  void decode(size_t num_values, IColumn &col, const UInt8 *filter, size_t filter_offset) override {
    ...
    requireRemainingBytes(num_values);
    for (size_t i = 0; i < num_values; ++i) {
      for (size_t stream = 0; stream < num_streams; ++stream) to[i * num_streams + stream] = data[i + stream * stream_size];
    }
    data += num_values;
    size_t out_idx = 0;
    for (size_t i = 0; i < num_values; ++i) {
      if (filter[filter_offset + i]) {
        for (size_t stream = 0; stream < num_streams; ++stream) to[out_idx * num_streams + stream] = to[i * num_streams + stream];
        ++out_idx;
      }
    }
    if (direct) {
  ...
  void decodeNoFilter(size_t num_values, IColumn &col, size_t num_streams) {
    ...
    if (!direct) converter->convertColumn(std::span(to, num_values * num_streams), num_values, col);
  }
};

bool PageDecoderInfo::canReadDirectlyIntoColumn(parq::Encoding::type encoding, size_t num_values, IColumn &col,
                                                std::span<char> &out) const {
  if (encoding == parq::Encoding::PLAIN && fixed_size_converter && physical_type != parq::Type::BOOLEAN &&
      fixed_size_converter->isTrivial()) {
...
std::unique_ptr<PageDecoder> PageDecoderInfo::makeDecoder(parq::Encoding::type encoding, std::span<const char> data) const {
  switch (encoding) {
    ...
    case parq::Encoding::BIT_PACKED:
      throw Exception(ErrorCodes::INCORRECT_DATA, "Unexpected BIT_PACKED encoding for values");
    ...
    case parq::Encoding::BYTE_STREAM_SPLIT:
      ...
      if (fixed_size_converter) return std::make_unique<ByteStreamSplitDecoder>(data, fixed_size_converter);
      break;
    default:
      throw Exception(ErrorCodes::INCORRECT_DATA, "Unexpected page encoding: {}", thriftToString(encoding));
  }

  throw Exception(ErrorCodes::INCORRECT_DATA, "Unexpected encoding {} for type {}", thriftToString(encoding),
                  thriftToString(physical_type));
}

void decodeRepOrDefLevels(parq::Encoding::type encoding, UInt8 max, size_t num_values, std::span<const char> data,
                          PaddedPODArray<UInt8> &out) {
  if (max == 0) return;
  switch (encoding) {
    case parq::Encoding::RLE:
...
std::unique_ptr<PageDecoder> makeDictionaryIndicesDecoder(parq::Encoding::type encoding, size_t dictionary_size,
                                                          std::span<const char> data) {
  switch (encoding) {
    case parq::Encoding::RLE_DICTIONARY:
      return std::make_unique<BitPackedRLEDecoder<UInt32>>(data, dictionary_size, /*has_header_byte=*/true);
    default:
      throw Exception(ErrorCodes::INCORRECT_DATA, "Unexpected dictionary indices encoding: {}", thriftToString(encoding));
  }
}
...
bool Dictionary::isInitialized() const { return mode != Mode::Uninitialized; }

double Dictionary::getAverageValueSize() const {
  switch (mode) {
    case Mode::FixedSize:
      return static_cast<double>(value_size);
    case Mode::StringPlain:
      return std::max(0., double(data.size()) / static_cast<double>(std::max(offsets.size(), 1ul)) - 4);
    case Mode::Column:
      return double(col->byteSize()) / static_cast<double>(std::max(col->size(), 1ul));
...
void Dictionary::decode(parq::Encoding::type encoding, const PageDecoderInfo &info, size_t num_values, std::span<const char> data_,
                        const IDataType &raw_decoded_type) {
  chassert(mode == Mode::Uninitialized);
  chassert(info.fixed_size_converter || info.string_converter);
  if (encoding == parq::Encoding::PLAIN_DICTIONARY) encoding = parq::Encoding::PLAIN;
  count = num_values;
  ...
  if (encoding != parq::Encoding::PLAIN) {
    ...
    for (size_t i = 0; i < num_values; ++i) {
      ...
      size_t len = 4 + size_t(x);
      if (len > size_t(end - ptr)) throw Exception(ErrorCodes::INCORRECT_DATA, "Encoded string is out of bounds");
      ptr += len;
...
static void indexImpl(const PaddedPODArray<UInt32> &indexes, std::span<const char> data, std::span<char> to) {
  ...
  for (size_t i = 0; i < size; ++i) memcpy(to.data() + i * value_size, data.data() + indexes[i] * value_size, value_size);
}
...
void Dictionary::index(const ColumnUInt32 &indexes_col, IColumn &out) {
  ...
  switch (mode) {
    case Mode::FixedSize: {
      ...
      switch (value_size) {
        case 1:
          indexImpl<1>(indexes, data, to);
          break;
        case 2:
          indexImpl<2>(indexes, data, to);
          break;
        case 3:
          indexImpl<3>(indexes, data, to);
          break;
        case 4:
          indexImpl<4>(indexes, data, to);
          break;
        case 8:
          indexImpl<8>(indexes, data, to);
          break;
        case 16:
          indexImpl<16>(indexes, data, to);
          break;
        default:
          for (size_t i = 0; i < indexes.size(); ++i) memcpy(to.data() + i * value_size, data.data() + indexes[i] * value_size, value_size);
      }
      break;
    }
    case Mode::StringPlain: {
      ...
      for (UInt32 idx : indexes) {
        ...
        /// TODO [parquet]: Try optimizing short memcpy by taking advantage of padding (maybe memcpySmall.h helps). Also in
        /// PlainStringDecoder.
      ...
      if (days_num > DATE_LUT_MAX_EXTEND_DAY_NUM || days_num < -DAYNUM_OFFSET_EPOCH) {
        if (date_overflow_behavior == FormatSettings::DateTimeOverflowBehavior::Saturate)
          ...
          throw Exception{ErrorCodes::VALUE_IS_OUT_OF_RANGE_OF_DATA_TYPE,
                          "Input value {} is out of allowed Date32 range, which is [{}, {}]", days_num, -DAYNUM_OFFSET_EPOCH,
...
void IntConverter::convertField(std::span<const char> data, bool /*is_max*/, Field &out) const {
  ...
  switch (input_size) {
    case 1:
      val = unalignedLoad<UInt8>(data.data());
      break;
    case 2:
      val = unalignedLoad<UInt16>(data.data());
      break;
    case 4:
      val = unalignedLoad<UInt32>(data.data());
      break;
    case 8:
      val = unalignedLoad<UInt64>(data.data());
      break;
    default:
      chassert(false);
  }
  ...
  /// Sign-extend.
  if (input_signed && input_size < 8 && (val >> (input_size * 8 - 1)) != 0) val |= 0 - (1ul << (input_size * 8));

  /// Check for overflow in signed <-> unsigned conversion.
  if (input_signed && !field_signed && Int64(val) < 0) return;
  if (!input_signed && field_signed && val > UInt64(INT64_MAX)) return;

  if (field_ipv4) {
    if (val <= UInt64(UINT32_MAX)) out = Field(IPv4(UInt32(val)));
    ...
    val /= 1000;
    if (val <= UInt64(UINT32_MAX)) out = Field(val);
  } else if (field_decimal_scale.has_value()) {
    switch (output_size.value_or(input_size)) {
      case 4:
        out = DecimalField<Decimal32>(Int32(val), *field_decimal_scale);
        break;
      case 8:
        out = DecimalField<Decimal64>(val, *field_decimal_scale);
        break;
      default:
        chassert(false);
    }
...
void TrivialStringConverter::convertColumn(std::span<const char> chars, const UInt64 *offsets, size_t separator_bytes, size_t num_values,
                                           IColumn &col) const {
  auto &col_str = assert_cast<ColumnString &>(col);
  ...
  if (separator_bytes == 0) {
    ...
    for (size_t i = 0; i < num_values; ++i) out_offsets[prev_count + i] = offsets[i] + diff;  // (wrapping overflow is ok)
  } else {
...
void BigEndianHelper<T>::fixupValue(T &x) const {
  ...
  /// Sign-extend.
  if (x & sign_mask) x |= sign_extension_mask;
}
...
template <typename T>
void BigEndianDecimalStringConverter<T>::convertColumn(std::span<const char> chars, const UInt64 *offsets, size_t separator_bytes,
                                                       size_t num_values, IColumn &col) const {
  auto to_bytes = col.insertRawUninitialized(num_values);
  chassert(to_bytes.size() == num_values * sizeof(T));
  T *to = reinterpret_cast<T *>(to_bytes.data());
...
void BigEndianDecimalStringConverter<T>::convertField(std::span<const char> data, bool /*is_max*/, Field &out) const {
  if (data.size() > sizeof(T))
    throw Exception(ErrorCodes::CANNOT_PARSE_NUMBER, "Unexpectedly wide value in Decimal statistics: {} > {} bytes", data.size(),
                    sizeof(T));
...
Int96Converter::Int96Converter() { input_size = 12; }
...
void Int96Converter::convertColumn(std::span<const char> data, size_t num_values, IColumn &col) const {
  ...
  for (size_t i = 0; i < num_values; ++i) {
    ...
    if (nanos < -day_nanos || nanos > day_nanos)
      throw Exception(ErrorCodes::VALUE_IS_OUT_OF_RANGE_OF_DATA_TYPE,
                      "INT96 timestamp out of range: time of day component {} is outside [0, {}]", nanos, day_nanos);
    ...
    if (overflow)
      throw Exception(ErrorCodes::VALUE_IS_OUT_OF_RANGE_OF_DATA_TYPE, "INT96 timestamp out of range: julian day {}, time of day {} ns",
                      julian_day, nanos);
...
void GeoConverter::convertColumn(std::span<const char> chars, const UInt64 *offsets, size_t separator_bytes, size_t num_values,
                                 IColumn &col) const {
