...
static void decompressLZ4Raw(const char *data, size_t compressed_size, size_t uncompressed_size, char *out) {
  ...
  if (n < 0) throw Exception(ErrorCodes::INCORRECT_DATA, "Malformed compressed page");
  if (size_t(n) != uncompressed_size) throw Exception(ErrorCodes::INCORRECT_DATA, "Unexpected uncompressed page size");
}
...
static bool tryDecompressLZ4Hadoop(const char *data, size_t compressed_size, size_t uncompressed_size, char *out) {
  ...
  while (compressed_size > 0) {
    if (compressed_size < 8) return false;
    ...
    data += 8;
    compressed_size -= 8;
    if (frame_compressed_size > compressed_size || frame_uncompressed_size > uncompressed_size) return false;

    int n = LZ4_decompress_safe(data, out, int(frame_compressed_size), int(frame_uncompressed_size));
    if (n < 0 || size_t(n) != frame_uncompressed_size) return false;
...
static void decompress(const char *data, size_t compressed_size, size_t uncompressed_size, parq::CompressionCodec::type codec, char *out) {
  ...
  switch (codec) {
...
#if USE_SNAPPY
    {
      ...
      if (actual_uncompressed_size != uncompressed_size) throw Exception(ErrorCodes::INCORRECT_DATA, "Unexpected uncompressed page size");
...
#else
      throw Exception(ErrorCodes::FEATURE_IS_NOT_ENABLED_AT_BUILD_TIME,
                      "Cannot decompress Snappy: ClickHouse was compiled without Snappy support");
#endif
  ...
  std::unique_ptr<ReadBuffer> decompressor = wrapReadBufferWithCompressionMethod(std::move(mem_buf), method,
                                                                                 /*zstd_window_log_max*/ 0, uncompressed_size, out);
  size_t pos = 0;
...
parq::FileMetaData Reader::readFileMetaData(Prefetcher &prefetcher) {
  ...
  if (file_size <= 8) throw Exception(ErrorCodes::INCORRECT_DATA, "Parquet file too short: {} bytes", file_size);
...
void Reader::getHyperrectangleForRowGroup(const parq::RowGroup *meta, Hyperrectangle &hyperrectangle) const {
  for (const PrimitiveColumnInfo &column_info : primitive_columns) {
    if (!column_info.used_by_key_condition) continue;
    if (!column_info.decoder.allow_stats) continue;
    try {
      const auto &column_meta = meta->columns.at(column_info.column_idx).meta_data;
      if (!column_meta.__isset.statistics) continue;

      Range &range = hyperrectangle[column_info.idx_in_output_block];

      bool nullable = column_info.levels.back().def > 0;
      bool always_null = column_meta.statistics.__isset.null_count && column_meta.statistics.null_count == column_meta.num_values;
      bool can_be_null = !column_meta.statistics.__isset.null_count || column_meta.statistics.null_count != 0;
...
void Reader::prefilterAndInitRowGroups(const std::optional<std::unordered_set<UInt64>> &row_groups_to_read) {
  ...
  for (const auto &col : format_filter_info->additional_columns) extended_sample_block.insert(col);
  ...
  if (row_level_filter) add_prewhere_outputs(row_level_filter->actions);
  if (prewhere_info) add_prewhere_outputs(prewhere_info->prewhere_actions);
  ...
  if (format_filter_info->key_condition) {
    for (size_t idx_in_output_block : format_filter_info->key_condition->getUsedColumns()) {
      ...
      if (!output_idx.has_value()) throw Exception(ErrorCodes::LOGICAL_ERROR, "KeyCondition uses PREWHERE output");
      const OutputColumnInfo &output_info = output_columns[output_idx.value()];

      if (output_info.is_primitive) primitive_columns[output_info.primitive_start].used_by_key_condition = true;
    }
  }
  ...
  for (size_t row_group_idx = 0; row_group_idx < file_metadata.row_groups.size(); ++row_group_idx) {
    ...
    if (meta->num_rows <= 0) throw Exception(ErrorCodes::INCORRECT_DATA, "Row group {} has <= 0 rows: {}", row_group_idx, meta->num_rows);
    if (meta->columns.size() != total_primitive_columns_in_file)
      throw Exception(ErrorCodes::INCORRECT_DATA, "Row group {} has unexpected number of columns: {} != {}", row_group_idx,
                      meta->columns.size(), total_primitive_columns_in_file);
    ...
    if (options.format.parquet.filter_push_down && format_filter_info->key_condition) {
      ...
      if (!format_filter_info->key_condition->checkInHyperrectangle(hyperrectangle, extended_sample_block_data_types).can_be_true) continue;
    }
    ...
    for (size_t column_idx = 0; column_idx < primitive_columns.size(); ++column_idx) {
      ...
      bool null_count_is_known_to_be_zero =
          column.meta->meta_data.statistics.__isset.null_count && column.meta->meta_data.statistics.null_count == 0;
      column.need_null_map = is_nullable && !null_count_is_known_to_be_zero;
    }
  }

  if (row_groups.empty()) return;  // all row groups were skipped

  if (options.format.parquet.bloom_filter_push_down && format_filter_info->key_condition) prepareBloomFilterCondition();
  ...
  if (options.format.parquet.page_filter_push_down) {
    ...
    for (const auto &[idx_in_output_block, key_condition] : column_conditions) {
      const auto &output_idx = sample_block_to_output_columns_idx.at(idx_in_output_block);
      if (!output_idx.has_value()) throw Exception(ErrorCodes::LOGICAL_ERROR, "Column condition uses PREWHERE output");
      const OutputColumnInfo &output_info = output_columns[output_idx.value()];

      if (!output_info.is_primitive || !primitive_columns[output_info.primitive_start].decoder.allow_stats) continue;
...
void Reader::prepareBloomFilterCondition() {
  ...
  std::vector<std::optional<std::pair</*primitive_idx*/ size_t, parquet::ColumnDescriptor>>> bf_eligible_columns(
      extended_sample_block.columns());
  ...
  for (size_t primitive_idx = 0; primitive_idx < primitive_columns.size(); ++primitive_idx) {
    ...
    if (!column_chunk_meta->meta_data.__isset.bloom_filter_offset) continue;
  ...
  if (any_column_eligible_for_bf) {
    ...
    auto hash_many = [&](size_t column_idx, const ColumnPtr &column) -> std::optional<std::vector<uint64_t>> {
      const auto &pair = bf_eligible_columns.at(column_idx);
      if (!pair.has_value()) return std::nullopt;
      if (column->size() > options.bloom_filter_max_set_size) return std::nullopt;
      const auto &[primitive_idx, descriptor] = *pair;
      auto hashes = parquetTryHashColumn(column.get(), &descriptor);
      if (!hashes.has_value()) return std::nullopt;
    ...
    if (!any_column_uses_bf) bloom_filter_condition.reset();
  }
}

void Reader::initializePrefetches() {
  bool use_offset_index =
      options.format.parquet.use_offset_index || format_filter_info->prewhere_info || format_filter_info->row_level_filter ||
  ...
  for (RowGroup &row_group : row_groups) {
    ...
    for (size_t column_idx = 0; column_idx < primitive_columns.size(); ++column_idx) {
      ...
      if (column.meta->meta_data.__isset.dictionary_page_offset) {
        ...
        column.dictionary_page_prefetch = prefetcher.registerRange(start, dict_page_length, /*likely_to_be_used=*/true);

        /// Dictionary filter.
        if (primitive_columns[column_idx].used_by_key_condition && dict_page_length < options.dictionary_filter_limit_bytes &&
          ...
          for (const parq::PageEncodingStats &s : column.meta->meta_data.encoding_stats)
            all_pages_are_dictionary_encoded &= (s.page_type != parq::PageType::DATA_PAGE && s.page_type != parq::PageType::DATA_PAGE_V2) ||
                                                s.encoding == parq::Encoding::PLAIN_DICTIONARY ||
                                                s.encoding == parq::Encoding::RLE_DICTIONARY || s.count == 0;
          column.use_dictionary_filter = all_pages_are_dictionary_encoded;
        }
      }

      /// Bloom filter.
      if (!column.use_dictionary_filter && primitive_columns[column_idx].use_bloom_filter &&
        ...
        if (!column.meta->meta_data.__isset.bloom_filter_length)
          ...
          column.bloom_filter_data_prefetch =
              prefetcher.registerRange(size_t(column.meta->meta_data.bloom_filter_offset), len, /*likely_to_be_used=*/false);
        }
        /// bloom_filter_header_prefetch and bloom_filter_data_prefetch overlap, that's ok.
        column.use_bloom_filter = true;
        column.bloom_filter_header_prefetch =
            prefetcher.registerRange(size_t(column.meta->meta_data.bloom_filter_offset), max_header_length, /*likely_to_be_used=*/true);
      }

      /// Offset index.
      if (use_offset_index && column.meta->__isset.offset_index_offset && column.meta->__isset.offset_index_length) {
        column.offset_index_prefetch = prefetcher.registerRange(size_t(column.meta->offset_index_offset),
                                                                size_t(column.meta->offset_index_length), /*likely_to_be_used*/ true);
      }

      /// Column index.
      column.use_column_index = primitive_columns[column_idx].column_index_condition && column.offset_index_prefetch &&
                                column.meta->__isset.column_index_offset && column.meta->__isset.column_index_length;
      if (column.use_column_index)
        column.column_index_prefetch = prefetcher.registerRange(size_t(column.meta->column_index_offset),
      ...
      if (file_metadata.created_by == "parquet-mr" && !column.meta->meta_data.__isset.dictionary_page_offset &&
          !column.meta->__isset.offset_index_offset)
        data_pages_extra_bytes =
            std::min(100ul, prefetcher.getFileSize() - size_t(column.meta->meta_data.data_page_offset) - column.data_pages_bytes);

      column.data_pages_prefetch =
          prefetcher.registerRange(size_t(column.meta->meta_data.data_page_offset), column.data_pages_bytes + data_pages_extra_bytes,
  ...
  if (need_to_find_bloom_filter_lengths_the_hard_way) {
    ...
    for (const auto &rg : file_metadata.row_groups) {
      for (const auto &col : rg.columns) {
        ...
        if (col.__isset.offset_index_offset) all_offsets.push_back(col.offset_index_offset);
        if (col.__isset.column_index_offset) all_offsets.push_back(col.column_index_offset);
        if (col.meta_data.__isset.dictionary_page_offset) all_offsets.push_back(col.meta_data.dictionary_page_offset);
        all_offsets.push_back(col.meta_data.data_page_offset);
        if (col.meta_data.__isset.bloom_filter_offset) all_offsets.push_back(col.meta_data.bloom_filter_offset);
      }
    }
    std::sort(all_offsets.begin(), all_offsets.end());
    for (RowGroup &row_group : row_groups) {
      for (ColumnChunk &column : row_group.columns) {
        if (!column.use_bloom_filter) continue;
        ...
        column.bloom_filter_data_prefetch = prefetcher.registerRange(offset, end - offset, /*likely_to_be_used=*/false);
      }
    }
  }
...
void Reader::preparePrewhere() {
  ...
  auto add_single_step = [&](const ActionsDAG &dag, const String &filter_column_name, bool needs_filter, size_t step_idx) {
    if (!actions_settings.has_value()) actions_settings.emplace();
    Step step{.actions = ExpressionActions(dag.clone(), actions_settings.value())};
    if (needs_filter) step.filter_column_name = filter_column_name;
    ...
    for (const auto &col : step.actions.getRequiredColumnsWithTypes()) {
      ...
      if (output_idx.has_value()) {
        ...
        if (!prewhere_output_column_idxs.contains(idx_in_output_block))
          throw Exception(ErrorCodes::LOGICAL_ERROR, "PREWHERE appears to use its own output as input: column '{}' (idx {})", col.name,
                          idx_in_output_block);
      }
  ...
  auto add_step = [&](const ActionsDAG &dag, const String &filter_column_name, bool needs_filter) {
    ...
    bool success = tryBuildPrewhereSteps(prewhere_info_patched, *actions_settings, prewhere_expr_info,
    ...
    if (success) {
      ...
      for (size_t i = 0; i < prewhere_expr_info.steps.size(); ++i) {
        ...
        if (needs_filter) add_single_step(filter->actions->getActionsDAG(), filter->filter_column_name, true, i);
      }
    } else {
  ...
  if (row_level_filter) add_step(row_level_filter->actions, row_level_filter->column_name, true);
  if (prewhere_info) add_step(prewhere_info->prewhere_actions, prewhere_info->prewhere_column_name, prewhere_info->need_filter);
...
void Reader::processBloomFilterHeader(ColumnChunk &column, const PrimitiveColumnInfo &column_info) {
  ...
  if (!column.bloom_filter_header.algorithm.__isset.BLOCK || !column.bloom_filter_header.hash.__isset.XXHASH ||
      !column.bloom_filter_header.compression.__isset.UNCOMPRESSED)
    throw Exception(ErrorCodes::INCORRECT_DATA,
                    "Unsupported bloom filter format. Use setting input_format_parquet_bloom_filter_push_down=0 to ignore.");
  ...
  for (size_t block_idx : block_idxs) subranges.emplace_back(base_offset + block_idx * bytes_per_block, bytes_per_block);
...
void Reader::decodeDictionaryPageImpl(const parq::PageHeader &header, std::span<const char> data, ColumnChunk &column,
  ...
  column.dictionary.decode(header.dictionary_page_header.encoding, column_info.decoder, size_t(header.dictionary_page_header.num_values),
                           data, *column_info.decoded_type);
}
...
bool Reader::BloomFilterLookup::findAnyHash(const std::vector<uint64_t> &hashes) {
  ...
  for (size_t h : hashes) {
    size_t block_idx = ((h >> 32) * num_blocks) >> 32;
    auto it = std::partition_point(column.bloom_filter_blocks.begin(), column.bloom_filter_blocks.end(),
                                   [&](const BloomFilterBlock &block) { return block.block_idx < block_idx; });
    ...
    /// https://parquet.apache.org/docs/file-format/bloomfilter/
    static constexpr UInt32 salt[8] = {0x47b6137bU, 0x44974d91U, 0x8824ad5bU, 0xa2b7289dU,
...
bool Reader::applyBloomAndDictionaryFilters(RowGroup &row_group) {
  ...
  for (size_t i = 0; i < row_group.columns.size(); ++i) {
    if (row_group.columns[i].use_bloom_filter)
      filter_map.emplace(primitive_columns[i].idx_in_output_block, std::make_unique<BloomFilterLookup>(prefetcher, row_group.columns[i]));
  }
  ...
  return bloom_filter_condition->checkInHyperrectangle(row_group.hyperrectangle, extended_sample_block_data_types, filter_map).can_be_true;
}
...
void Reader::applyColumnIndex(ColumnChunk &column, const PrimitiveColumnInfo &column_info, const RowGroup &row_group) {
  try {
    ...
    if (column_index.min_values.size() != num_pages || column_index.max_values.size() != num_pages ||
      ...
      throw Exception(ErrorCodes::INCORRECT_DATA,
                      "Unexpected number of pages: {} null_pages, {} null_counts, {} min_values, {} max_values, {} pages in offset index",
                      column_index.null_pages.size(), column_index.null_counts.size(), column_index.min_values.size(),
                      column_index.max_values.size(), num_pages);
    ...
    for (size_t page_idx = 0; page_idx < num_pages; ++page_idx) {
      ...
      bool passes_filter =
          column_info.column_index_condition->checkInHyperrectangle(hyperrectangle, extended_sample_block_data_types).can_be_true;

      if (!passes_filter) {
        size_t start_row = column.offset_index.page_locations[page_idx].first_row_index;
        size_t end_row =
            page_idx + 1 < num_pages ? column.offset_index.page_locations[page_idx + 1].first_row_index : row_group.meta->num_rows;
        chassert(end_row > start_row);  // validated in decodeOffsetIndex
        if (start_row > prev_row_idx) column.row_ranges_after_column_index.emplace_back(prev_row_idx, start_row);
...
void Reader::adjustRangeFromIndexIfNeeded(Range &range, const PrimitiveColumnInfo &column_info, bool can_be_null) const {
  if (accurateLess(range.right, range.left))
    throw Exception(ErrorCodes::INCORRECT_DATA, "Statistics have min_value > max_value: {} > {}.", static_cast<const Field &>(range.left),
                    static_cast<const Field &>(range.right));
  ...
  if (nullable && can_be_null) {
    if (null_as_default) {
      ...
      if (!range.left.isNull() && accurateLess(default_value, range.left)) range.left = default_value;
      if (!range.right.isNull() && accurateLess(range.right, default_value)) range.right = default_value;
    } else {
      ...
      if (!range.left.isNull() && !range.right.isNull()) range.left = NEGATIVE_INFINITY;
    }
  } else {
    /// If the column doesn't have nulls, exclude both infinities.
    if (range.left.isNull()) range.left_included = false;
    if (range.right.isNull()) range.right_included = false;
  }
}
...
void Reader::intersectColumnIndexResultsAndInitSubgroups(RowGroup &row_group) {
  ...
  size_t num_rows = 0;
  {
    ...
    for (auto &col : row_group.columns) {
      if (!col.use_column_index) continue;
  ...
  for (const auto [start, end] : row_ranges) {
    for (size_t substart = start; substart < end; substart += rows_per_subgroup) {
      ...
      if (options.format.defaults_for_omitted_fields) row_subgroup.block_missing_values.init(sample_block->columns());
    }
  }
...
void Reader::decodeOffsetIndex(ColumnChunk &column, const RowGroup &row_group) {
  ...
  int64_t end_offset = meta.total_compressed_size +
                       std::min({meta.data_page_offset, meta.__isset.dictionary_page_offset ? meta.dictionary_page_offset : INT64_MAX,
  ...
  for (const auto &loc : locations) {
    if (loc.offset < prev_offset || loc.first_row_index <= prev_row_index || loc.compressed_page_size <= 0 ||
        loc.compressed_page_size > end_offset - loc.offset || loc.first_row_index >= num_rows)
      throw Exception(ErrorCodes::INCORRECT_DATA, "Invalid offset index: {}, prev offset: {}, prev row: {}, end offset: {}, num rows: {}",
                      thriftToString(loc), prev_offset, prev_row_index, end_offset, num_rows);
    prev_offset = loc.offset + loc.compressed_page_size;
    prev_row_index = loc.first_row_index;
  }
}

void Reader::determinePagesToPrefetch(ColumnChunk &column, const RowSubgroup &row_subgroup, const RowGroup &row_group,
                                      std::vector<PrefetchHandle *> &out) {
  chassert(row_subgroup.filter.rows_pass > 0);
  if (column.offset_index.page_locations.empty()) return;  // no offset index, can't prefetch individual pages
  ...
  if (column.data_pages.empty()) {
    ...
    if (!column.meta->meta_data.__isset.dictionary_page_offset) {
      ...
      if (locations.at(0).offset > column.meta->meta_data.data_page_offset) {
        page_byte_ranges.emplace_back(size_t(column.meta->meta_data.data_page_offset),
    ...
    for (size_t page_idx = 0; page_idx < locations.size(); ++page_idx) {
      ...
      while (ranges_idx < row_ranges.size() && row_ranges[ranges_idx].second <= size_t(loc.first_row_index)) ++ranges_idx;
    ...
    if (has_undeclared_dictionary_page) column.dictionary_page_prefetch = std::move(handles.at(0));
  ...
  while (column.data_pages_prefetch_idx < column.data_pages.size()) {
    ...
    if (page_start >= subgroup_end) break;
    ...
    if (passes_filter && row_subgroup.filter.rows_pass < row_subgroup.filter.rows_total)
      passes_filter = !memoryIsZero(row_subgroup.filter.filter.data(), start_row_idx - row_subgroup.start_row_idx,
...
double Reader::estimateAverageStringLengthPerRow(const ColumnChunk &column, const RowGroup &row_group) const {
  ...
  if (column.meta->meta_data.__isset.size_statistics && column.meta->meta_data.size_statistics.__isset.unencoded_byte_array_data_bytes) {
...
double Reader::estimateColumnMemoryBytesPerRow(const ColumnChunk &column, const RowGroup &row_group,
                                               const PrimitiveColumnInfo &column_info) const {
  double res;
  ...
  /// Outer array offsets.
  if (column_info.levels.back().rep > 0) res += 8;
  ...
  if (column_info.levels.back().rep > 1)
    res += (column_info.levels.back().rep - 1) * 8. * static_cast<double>(column.meta->meta_data.num_values) /
           static_cast<double>(row_group.meta->num_rows);

  return res;
}

void Reader::decodePrimitiveColumn(ColumnChunk &column, const PrimitiveColumnInfo &column_info, ColumnSubchunk &subchunk,
                                   const RowGroup &row_group, RowSubgroup &row_subgroup) {
  ...
  if (column_info.levels.back().rep == 0)
    ...
    output_num_values_estimate =
        size_t(1.2 * static_cast<double>(row_subgroup.filter.rows_pass) / static_cast<double>(row_group.meta->num_rows) *
               static_cast<double>(column.meta->meta_data.num_values));
  ...
  const bool use_filter_in_decoder = (column_info.levels.back().rep == 0) && !row_subgroup.filter.filter.empty() &&
                                     column.page.initialized && !column.page.is_dictionary_encoded;
  ...
  if (use_filter_in_decoder) {
    ...
    while (true)  // loop over pages
    {
      ...
      if (page.next_row_idx == subgroup_end_row_idx &&
          (page.value_idx < page.num_values || page.end_row_idx.has_value() || column.next_page_offset >= column.data_pages_bytes))
        break;
    ...
    while (true)  // loop over row ranges that pass the filter
    {
      ...
      if (!row_subgroup.filter.filter.empty()) {
        while (row_subidx < row_subgroup.filter.rows_total && !row_subgroup.filter.filter[row_subidx]) row_subidx += 1;
        num_rows = 0;
        while (row_subidx + num_rows < row_subgroup.filter.rows_total && row_subgroup.filter.filter[row_subidx + num_rows]) num_rows += 1;
      }
      if (!num_rows) break;
  ...
  if (subchunk.null_map && !column_info.output_nullable && !options.format.null_as_default) {
    ...
    if (memchr(null_map.data(), 0, null_map.size()) != nullptr)
      throw Exception(ErrorCodes::CANNOT_INSERT_NULL_IN_ORDINARY_COLUMN, "Cannot convert NULL value to non-Nullable type for column {}",
                      column_info.name);
  ...
  if (subchunk.arrays_offsets.empty() && subchunk.column->size() != row_subgroup.filter.rows_pass)
    throw Exception(ErrorCodes::LOGICAL_ERROR, "Unexpected number of rows in column subchunk {} {}", subchunk.column->size(),
                    row_subgroup.filter.rows_pass);

  if (column_info.output_nullable) {
    if (!subchunk.null_map) subchunk.null_map = ColumnUInt8::create(subchunk.column->size(), false);
...
void Reader::skipToRowOrNextPage(std::optional<size_t> row_idx, ColumnChunk &column, const PrimitiveColumnInfo &column_info) {
  ...
  if (!row_idx.has_value()) chassert(page.initialized);

  if (row_idx.has_value() && page.initialized && page.value_idx < page.num_values && page.end_row_idx.has_value() &&
      *page.end_row_idx > *row_idx)
  ...
  if (!found_page && !column.data_pages.empty()) {
    ...
    if (!row_idx.has_value()) row_idx = column.data_pages[column.data_pages_idx].end_row_idx;
    while (column.data_pages_idx < column.data_pages.size() && column.data_pages[column.data_pages_idx].end_row_idx <= *row_idx)
  ...
  while (true) {
    ...
    if (row_idx.has_value() && page.initialized && page.value_idx < page.num_values && skipRowsInPage(*row_idx, page, column, column_info))
      return;
...
std::tuple<parq::PageHeader, std::span<const char>> Reader::decodeAndCheckPageHeader(const char *&data_ptr, const char *data_end) const {
  ...
  if (header.__isset.crc && options.format.parquet.verify_checksums) {
    ...
    if (crc != uint32_t(header.crc)) throw Exception(ErrorCodes::CHECKSUM_DOESNT_MATCH, "Page CRC checksum verification failed");
  }

  return {header, page_data};
}

bool Reader::initializeDataPage(const char *&data_ptr, const char *data_end, size_t next_row_idx, std::optional<size_t> end_row_idx,
                                size_t target_row_idx, ColumnChunk &column, const PrimitiveColumnInfo &column_info) {
  ...
  else if (header.type == parq::PageType::DATA_PAGE && column_info.levels.back().rep == 0)  // no arrays => num_values == num_rows
  ...
  if (num_rows_in_page.has_value()) {
    if (end_row_idx.has_value() && *end_row_idx - next_row_idx != *num_rows_in_page)
      throw Exception(ErrorCodes::INCORRECT_DATA, "Number of rows in page header doesn't match offset index: {} != {}", *num_rows_in_page,
  ...
  if (header.type == parq::PageType::DATA_PAGE) {
    ...
    if (column_info.levels.size() == 1) {
      ...
      UInt32 n;
      if (column_info.levels.back().rep > 0) {
        if (page.data.size() < 4) throw Exception(ErrorCodes::INCORRECT_DATA, "Decompressed data is too short (rep size)");
        memcpy(&n, page.data.data(), 4);
        if (n > page.data.size() - 4) throw Exception(ErrorCodes::INCORRECT_DATA, "Decompressed data is too short (rep)");
        encoded_rep = page.data.data() + 4;
        encoded_rep_size = n;
        page.data = page.data.subspan(4 + n);
      }

      if (page.data.size() < 4) throw Exception(ErrorCodes::INCORRECT_DATA, "Decompressed data is too short (def size)");
      memcpy(&n, page.data.data(), 4);
      if (n > page.data.size() - 4) throw Exception(ErrorCodes::INCORRECT_DATA, "Decompressed data is too short (def)");
    ...
    if (header.data_page_header_v2.__isset.is_compressed && !header.data_page_header_v2.is_compressed) {
  ...
  if (page.encoding == parq::Encoding::PLAIN_DICTIONARY) page.encoding = parq::Encoding::RLE_DICTIONARY;
  ...
  if (page.rep.empty()) {
    ...
    while (new_value_idx < page.num_values) {
      if (page.rep[new_value_idx] == 0) {
        if (page.next_row_idx == end_row_idx) break;
        page.next_row_idx += 1;
      }
      new_value_idx += 1;
    }
...
bool Reader::skipRowsInPage(size_t target_row_idx, PageState &page, ColumnChunk &column, const PrimitiveColumnInfo &column_info) {
  ...
  if (page.def.empty()) {
    ...
    UInt8 max_def = column_info.levels.back().def;
    for (size_t i = prev_value_idx; i < page.value_idx; ++i) encoded_values_to_skip += page.def[i] == max_def;
  }

  if (encoded_values_to_skip > 0) {
    decompressPageIfCompressed(page);
    if (!page.decoder) createPageDecoder(page, column, column_info);
...
static void processDefLevelsForInnermostColumn(size_t num_values, const UInt8 *def, UInt8 max_def, UInt8 max_array_def,
                                               size_t &out_num_encoded_values, ColumnUInt8::Container *out_null_map) {
  size_t num_encoded_values = 0;
  for (size_t i = 0; i < num_values; ++i) {
    if constexpr (has_arrays)
      if (def[i] < max_array_def) continue;  // empty array
...
static void processRepDefLevelsForArray(size_t num_values, const UInt8 *def, const UInt8 *rep, UInt8 array_rep, UInt8 array_def,
...
void Reader::readRowsInPage(size_t end_row_idx, ColumnSubchunk &subchunk, ColumnChunk &column, const PrimitiveColumnInfo &column_info,
  ...
  if (!page.rep.empty()) {
    ...
    for (size_t level_idx = 1; level_idx < column_info.levels.size(); ++level_idx) {
      ...
      processRepDefLevelsForArray(page.value_idx - prev_value_idx, page.def.data() + prev_value_idx, page.rep.data() + prev_value_idx,
                                  level.rep, level.def, parent_array_def, offsets);
...
#define X(has_arrays, has_nulls, null_map)                                                                                     \
  processDefLevelsForInnermostColumn<has_arrays, has_nulls>(page.value_idx - prev_value_idx, page.def.data() + prev_value_idx, \
                                                            column_info.levels.back().def, column_info.max_array_def,          \
                                                            encoded_values_to_read, null_map)
    if (subchunk.null_map) {
  ...
  if (!has_filter && !page.is_dictionary_encoded && prev_value_idx == 0 && page.value_idx == page.num_values &&
    ...
    if (column_info.decoder.canReadDirectlyIntoColumn(page.encoding, encoded_values_to_read, *subchunk.column, span)) {
      if (span.size() != page.values_uncompressed_size) throw Exception(ErrorCodes::INCORRECT_DATA, "Unexpected uncompressed page size");
  ...
  if (encoded_values_to_read > 0) {
    ...
    if (page.is_dictionary_encoded) {
      if (!page.indices_column) page.indices_column = ColumnUInt32::create();
  ...
  if (page.value_idx == page.num_values) page.decoder.reset();
}

void Reader::decompressPageIfCompressed(PageState &page) {
  if (page.codec == parq::CompressionCodec::UNCOMPRESSED) return;
...
MutableColumnPtr Reader::formOutputColumn(RowSubgroup &row_subgroup, size_t output_column_idx, size_t num_rows) {
  ...
  if (output_info.is_primitive) {
    ...
    if (output_info.idx_in_output_block.has_value() &&
        *output_info.idx_in_output_block < row_subgroup.block_missing_values.getNumColumns() && subchunk.null_map) {
    ...
    for (size_t i = output_info.primitive_start + 1; i < output_info.primitive_end; ++i) {
      ...
      if (offsets != other_offsets)
        throw Exception(ErrorCodes::INCORRECT_DATA, "Invalid array of tuples: tuple elements {} and {} have different array lengths",
                        primitive_columns.at(output_info.primitive_start).name, primitive_columns.at(i).name);
    }
    ...
    for (size_t idx : output_info.nested_columns) columns.push_back(formOutputColumn(row_subgroup, idx, num_rows));
    if (columns.empty())
  ...
  if (output_info.needs_cast) {
    auto col = castColumn({std::move(res), output_info.input_type, output_info.name}, output_info.output_type);
...
ColumnPtr &Reader::getOrFormOutputColumn(RowSubgroup &row_subgroup, size_t idx_in_output_block) {
  ...
  if (output_idx.has_value()) {
    ...
    if (!state.column) state.column = formOutputColumn(row_subgroup, *output_idx, row_subgroup.filter.rows_pass);
  }
  chassert(state.column);
...
void Reader::applyPrewhere(RowSubgroup &row_subgroup, const RowGroup &row_group, size_t step_idx) {
  {
    ...
    if (!step.filter_column_name.has_value()) return;
    ...
    for (auto &state : row_subgroup.output)
      if (state.column) state.column = state.column->filter(filter, /*result_size_hint=*/rows_pass);
    ...
    if (row_subgroup.filter.rows_pass != row_subgroup.filter.rows_total) mut_filter.expand(row_subgroup.filter.filter, /*inverted*/ false);
