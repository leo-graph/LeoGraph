...
JSONCompactRowInputFormat::JSONCompactRowInputFormat(SharedHeader header_, ReadBuffer& in_, Params params_,
                                                     const FormatSettings& format_settings_)
    : RowInputFormatWithNamesAndTypes<JSONCompactFormatReader>(header_, in_, params_, false, false, false, format_settings_,
                                                               std::make_unique<JSONCompactFormatReader>(in_, format_settings_), false,
                                                               false) {}
...
void JSONCompactRowInputFormat::readPrefix() {
  ...
  if (format_settings.json.validate_types_from_metadata) {
    ...
    Names column_names;
    for (const auto& [name, type] : names_and_types) column_names.push_back(name);
...
void JSONCompactRowInputFormat::syncAfterError() { skipToUnescapedNextLineOrEOF(*in); }
...
bool JSONCompactFormatReader::checkForSuffix() { return JSONUtils::checkAndSkipArrayEnd(*in); }

void registerInputFormatJSONCompact(FormatFactory& factory) {
  factory.registerInputFormat(
      "JSONCompact", [](ReadBuffer& buf, const Block& sample, IRowInputFormat::Params params, const FormatSettings& settings) {
