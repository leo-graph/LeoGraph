...
static ColumnWithTypeAndName readColumnWithNumericData(const std::shared_ptr<arrow::ChunkedArray> &arrow_column,
...
static ColumnWithTypeAndName readColumnWithJSONData(const std::shared_ptr<arrow::ChunkedArray> &arrow_column, const String &column_name,
                                                    DataTypePtr type_hint, const FormatSettings &format_settings) {
...
static ColumnWithTypeAndName readColumnWithFixedStringData(const std::shared_ptr<arrow::ChunkedArray> &arrow_column,
                                                           const String &column_name) {
...
static ColumnWithTypeAndName readColumnWithBigIntegerFromFixedBinaryData(const std::shared_ptr<arrow::ChunkedArray> &arrow_column,
                                                                         const String &column_name, const DataTypePtr &column_type) {
  const auto *fixed_type = assert_cast<arrow::FixedSizeBinaryType *>(arrow_column->type().get());
  size_t fixed_len = fixed_type->byte_width();
  if (fixed_len != sizeof(ValueType))
    throw Exception(ErrorCodes::BAD_ARGUMENTS,
                    "Cannot insert data into {} column from fixed size binary, expected data with size {}, got {}", column_type->getName(),
                    sizeof(ValueType), fixed_len);
...
static ColumnWithTypeAndName readColumnWithBigNumberFromBinaryData(const std::shared_ptr<arrow::ChunkedArray> &arrow_column,
                                                                   const String &column_name, const DataTypePtr &column_type) {
  size_t total_size = 0;
  for (int chunk_i = 0, num_chunks = arrow_column->num_chunks(); chunk_i < num_chunks; ++chunk_i) {
    ...
    for (size_t i = 0; i != chunk_length; ++i) {
      /// If at least one value size is not equal to the size if big integer, fallback to reading String column and further cast to result
      /// type.
...
static ColumnWithTypeAndName readColumnWithBooleanData(const std::shared_ptr<arrow::ChunkedArray> &arrow_column,
                                                       const String &column_name) {
  ...
  for (int chunk_i = 0, num_chunks = arrow_column->num_chunks(); chunk_i < num_chunks; ++chunk_i) {
    ...
    for (size_t bool_i = 0; bool_i != static_cast<size_t>(chunk.length()); ++bool_i) column_data.emplace_back(chunk.Value(bool_i));
  }
  return {std::move(internal_column), internal_type, column_name};
}
        ...
        if (days_num > DATE_LUT_MAX_EXTEND_DAY_NUM || days_num < -DAYNUM_OFFSET_EPOCH) {
          switch (date_time_overflow_behavior) {
            ...
            default:
              ...
              /// In order to leave this behavior as default, we also throw when `date_time_overflow_mode == ignore`, as it is the setting's
              /// default value (As we want to make this backwards compatible, not break any workflows.)
...
static ColumnWithTypeAndName readColumnWithTimestampData(const std::shared_ptr<arrow::ChunkedArray> &arrow_column,
                                                         const String &column_name, bool empty_timezone_as_utc) {
  ...
  if (timezone.empty() && empty_timezone_as_utc) timezone = "UTC";
...
static ColumnWithTypeAndName readColumnWithDecimalDataImpl(const std::shared_ptr<arrow::ChunkedArray> &arrow_column,
                                                           const String &column_name, DataTypePtr internal_type) {
  auto internal_column = internal_type->createColumn();
  ...
  for (int chunk_i = 0, num_chunks = arrow_column->num_chunks(); chunk_i < num_chunks; ++chunk_i) {
    ...
    for (size_t value_i = 0, length = static_cast<size_t>(chunk.length()); value_i < length; ++value_i) {
      column_data.emplace_back(chunk.IsNull(value_i) ? DecimalType(0)
                                                     : *reinterpret_cast<const DecimalType *>(chunk.Value(value_i)));  // TODO: copy column
    }
  }
  return {std::move(internal_column), internal_type, column_name};
}

static ColumnWithTypeAndName readColumnWithFloat16Data(const std::shared_ptr<arrow::ChunkedArray> &arrow_column,
                                                       const String &column_name) {
...
static ColumnWithTypeAndName readColumnWithDecimalData(const std::shared_ptr<arrow::ChunkedArray> &arrow_column,
                                                       const String &column_name) {
...
static ColumnPtr readByteMapFromArrowColumn(const std::shared_ptr<arrow::ChunkedArray> &arrow_column) {
  if (!arrow_column->null_count()) return ColumnUInt8::create(arrow_column->length(), static_cast<UInt8>(0));
  ...
  for (int chunk_i = 0; chunk_i != arrow_column->num_chunks(); ++chunk_i) {
    ...
    for (size_t value_i = 0; value_i != static_cast<size_t>(chunk->length()); ++value_i) bytemap_data.emplace_back(chunk->IsNull(value_i));
  }
  return nullmap_column;
}

static ColumnWithTypeAndName readColumnWithGeoData(const std::shared_ptr<arrow::ChunkedArray> &arrow_column, const String &column_name,
...
static ColumnPtr readOffsetsFromFixedArrowListColumn(const std::shared_ptr<arrow::ChunkedArray> &arrow_column, int32_t length) {
  ...
  for (int64_t i = 0; i < size; ++i) offsets_data.emplace_back((i + 1) * length);
  return offsets_column;
}
 ...
 * */
template <typename NumericType, typename VectorType = ColumnVector<NumericType>>
static ColumnWithTypeAndName readColumnWithIndexesDataImpl(std::shared_ptr<arrow::ChunkedArray> &arrow_column, const String &column_name,
                                                           Int64 default_value_index, NumericType dict_size, bool is_nullable) {
  auto internal_type = std::make_shared<DataTypeNumber<NumericType>>();
  auto internal_column = internal_type->createColumn();
  auto &column_data = static_cast<VectorType &>(*internal_column).getData();
  column_data.reserve(arrow_column->length());
  ...
  for (int chunk_i = 0, num_chunks = arrow_column->num_chunks(); chunk_i < num_chunks; ++chunk_i) {
    ...
    for (int64_t i = 0; i != chunk->length(); ++i) {
      if (!chunk->IsNull(i) && (data[i] < 0 || data[i] >= dict_size))
        throw Exception(ErrorCodes::INCORRECT_DATA, "Index {} in Dictionary column is out of bounds, dictionary size is {}", Int64(data[i]),
                        UInt64(dict_size));
    }
...
static ColumnPtr readColumnWithIndexesData(std::shared_ptr<arrow::ChunkedArray> &arrow_column, Int64 default_value_index, UInt64 dict_size,
                                           bool is_nullable) {
  switch (arrow_column->type()->id()) {
#  define DISPATCH(ARROW_NUMERIC_TYPE, CPP_NUMERIC_TYPE)                                                            \
    case ARROW_NUMERIC_TYPE: {                                                                                      \
      return readColumnWithIndexesDataImpl<CPP_NUMERIC_TYPE>(arrow_column, "", default_value_index,                 \
                                                             static_cast<CPP_NUMERIC_TYPE>(dict_size), is_nullable) \
          .column;                                                                                                  \
    }
...
static std::shared_ptr<arrow::ChunkedArray> getNestedArrowColumn(const std::shared_ptr<arrow::ChunkedArray> &arrow_column) {
  ...
  for (int chunk_i = 0, num_chunks = arrow_column->num_chunks(); chunk_i < num_chunks; ++chunk_i) {
    ...
    if (flatten_result.ok()) {
      ...
      throw Exception(ErrorCodes::INCORRECT_DATA, "Failed to flatten chunk '{}' of column of type '{}' ", chunk_i,
                      arrow_column->type()->id());
    }
  }
  return std::make_shared<arrow::ChunkedArray>(array_vector);
}

static ColumnWithTypeAndName readIPv6ColumnFromBinaryData(const std::shared_ptr<arrow::ChunkedArray> &arrow_column,
                                                          const String &column_name) {
  size_t total_size = 0;
  for (int chunk_i = 0, num_chunks = arrow_column->num_chunks(); chunk_i < num_chunks; ++chunk_i) {
    auto &chunk = dynamic_cast<arrow::BinaryArray &>(*(arrow_column->chunk(chunk_i)));
...
static ColumnWithTypeAndName readIPv4ColumnWithInt32Data(const std::shared_ptr<arrow::ChunkedArray> &arrow_column,
                                                         const String &column_name) {
...
static bool isColumnJSON(const std::shared_ptr<arrow::Field> &field, DataTypePtr type_hint) {
  if (type_hint && type_hint->getTypeId() == TypeIndex::Object) return true;

  if (!field || !field->HasMetadata()) return false;
...
static ColumnWithTypeAndName readColumnFromArrowColumn(
    const std::shared_ptr<arrow::ChunkedArray> &arrow_column, std::string column_name, std::string full_column_name,
    std::unordered_map<String, ArrowColumnToCHColumn::DictionaryInfo> dictionary_infos, DataTypePtr type_hint, bool is_nullable_column,
    bool is_map_nested_column, std::optional<GeoColumnMetadata> geo_metadata, const ReadColumnFromArrowColumnSettings &settings,
...
static ColumnWithTypeAndName readNonNullableColumnFromArrowColumn(
    const std::shared_ptr<arrow::ChunkedArray> &arrow_column, std::string column_name, std::string full_column_name,
    std::unordered_map<String, ArrowColumnToCHColumn::DictionaryInfo> dictionary_infos, DataTypePtr type_hint, bool is_map_nested_column,
    bool make_nullable_if_low_cardinality, std::optional<GeoColumnMetadata> geo_metadata, const ReadColumnFromArrowColumnSettings &settings,
    const std::shared_ptr<arrow::Field> &arrow_field,
    const std::optional<std::unordered_map<String, String>> &parquet_columns_to_clickhouse,
    const std::optional<std::unordered_map<String, String>> &clickhouse_columns_to_parquet) {
  switch (arrow_column->type()->id()) {
    ...
    case arrow::Type::INT32: {
      ...
      if (type_hint && isIPv4(type_hint)) return readIPv4ColumnWithInt32Data(arrow_column, column_name);
    ...
    case arrow::Type::MAP: {
      ...
      auto nested_column = readColumnFromArrowColumn(arrow_nested_column, column_name, full_column_name, dictionary_infos, nested_type_hint,
                                                     false /*is_nullable_column*/, true /*is_map_nested_column*/, geo_metadata, settings,
                                                     arrow_field, parquet_columns_to_clickhouse, clickhouse_columns_to_parquet);
      if (!nested_column.column) return {};
    ...
    case arrow::Type::FIXED_SIZE_LIST: {
      enum class ListType { List, LargeList, FixedSizeList };
      ListType list_type = [&] {
        switch (arrow_column->type()->id()) {
          case arrow::Type::LIST:
      ...
      auto nested_column = readColumnFromArrowColumn(arrow_nested_column, column_name, full_column_name, dictionary_infos, nested_type_hint,
                                                     is_nested_nullable_column, false /*is_map_nested_column*/, geo_metadata, settings,
                                                     arrow_field, parquet_columns_to_clickhouse, clickhouse_columns_to_parquet);
      if (!nested_column.column) return {};
      ...
      const auto *tuple_type =
          type_hint && isNested(type_hint) ? typeid_cast<const DataTypeTuple *>(removeNullable(nested_column.type).get()) : nullptr;
      if (tuple_type) {
    ...
    case arrow::Type::STRUCT: {
      ...
      for (int chunk_i = 0, num_chunks = arrow_column->num_chunks(); chunk_i < num_chunks; ++chunk_i) {
        ...
        for (int i = 0; i < arrow_struct_type->num_fields(); ++i) nested_arrow_columns[i].emplace_back(struct_chunk.field(i));
      }
      ...
      for (int i = 0; i != arrow_struct_type->num_fields(); ++i) {
        ...
        if (parquet_columns_to_clickhouse) {
          ...
          if (auto it = parquet_columns_to_clickhouse->find(full_name); it != parquet_columns_to_clickhouse->end()) {
            ...
            if (pos != std::string::npos) field_name = field_name.substr(pos + 1);
          }
        }

        auto nested_arrow_column = std::make_shared<arrow::ChunkedArray>(nested_arrow_columns[i]);
        auto column_with_type_and_name =
            readColumnFromArrowColumn(nested_arrow_column, field_name, Nested::concatenateName(full_column_name, field_name),
                                      dictionary_infos, nested_type_hint, field->nullable(), false /*is_map_nested_column*/, geo_metadata,
                                      settings, arrow_field, parquet_columns_to_clickhouse, clickhouse_columns_to_parquet);
        if (!column_with_type_and_name.column) return {};
    ...
    case arrow::Type::DICTIONARY: {
      ...
      if (!dict_info.values) {
        ...
        auto dict_column =
            readColumnFromArrowColumn(arrow_dict_column, column_name, full_column_name, dictionary_infos, nullptr /*nested_type_hint*/,
                                      false /*is_nullable_column*/, false /*is_map_nested_column*/, geo_metadata, settings, arrow_field,
                                      parquet_columns_to_clickhouse, clickhouse_columns_to_parquet);

        if (!dict_column.column) return {};
        ...
        auto lc_type =
            std::make_shared<DataTypeLowCardinality>(make_nullable_if_low_cardinality ? makeNullable(dict_column.type) : dict_column.type);
        ...
        if (tmp_dict_column->size() != expected_dictionary_size) {
          throw Exception(ErrorCodes::INCORRECT_DATA,
                          ...
                          expected_dictionary_size, tmp_dict_column->size());
        }
      ...
      auto indexes_column = readColumnWithIndexesData(arrow_indexes_column, dict_info.default_value_index, dict_info.dictionary_size,
      ...
      auto lc_type = std::make_shared<DataTypeLowCardinality>(make_nullable_if_low_cardinality ? makeNullable(dict_info.values->type)
                                                                                               : dict_info.values->type);
...
#  undef DISPATCH
    ...
    default: {
      if (settings.skip_columns_with_unsupported_types) return {};
      ...
      throw Exception(ErrorCodes::UNKNOWN_TYPE,
                      ...
                      settings.format_name, arrow_column->type()->name(), column_name,
...
static ColumnWithTypeAndName readColumnFromArrowColumn(
    ...
    const std::optional<std::unordered_map<String, String>> &parquet_columns_to_clickhouse,
    const std::optional<std::unordered_map<String, String>> &clickhouse_columns_to_parquet) {
  bool read_as_nullable_column = (arrow_column->null_count() || is_nullable_column ||
                                  (type_hint && (type_hint->isNullable() || type_hint->isLowCardinalityNullable()))) &&
                                 !geo_metadata && settings.allow_inferring_nullable_columns;
  if (read_as_nullable_column && arrow_column->type()->id() != arrow::Type::LIST && arrow_column->type()->id() != arrow::Type::LARGE_LIST &&
      arrow_column->type()->id() != arrow::Type::FIXED_SIZE_LIST && arrow_column->type()->id() != arrow::Type::MAP &&
      arrow_column->type()->id() != arrow::Type::STRUCT && arrow_column->type()->id() != arrow::Type::DICTIONARY) {
    DataTypePtr nested_type_hint;
    if (type_hint) nested_type_hint = removeNullable(type_hint);

    auto nested_column = readNonNullableColumnFromArrowColumn(arrow_column, column_name, full_column_name, dictionary_infos,
                                                              nested_type_hint, is_map_nested_column,
                                                              /*make_nullable_if_low_cardinality*/ false, geo_metadata, settings,
                                                              arrow_field, parquet_columns_to_clickhouse, clickhouse_columns_to_parquet);
  ...
  return readNonNullableColumnFromArrowColumn(arrow_column, column_name, full_column_name, dictionary_infos, type_hint,
                                              is_map_nested_column,
                                              /*make_nullable_if_low_cardinality*/ read_as_nullable_column, geo_metadata, settings,
                                              arrow_field, parquet_columns_to_clickhouse, clickhouse_columns_to_parquet);
}
...
Block ArrowColumnToCHColumn::arrowSchemaToCHHeader(const arrow::Schema &schema, std::shared_ptr<const arrow::KeyValueMetadata> metadata,
                                                   const std::string &format_name, const FormatSettings &format_settings,
                                                   bool skip_columns_with_unsupported_types, bool allow_inferring_nullable_columns,
                                                   bool case_insensitive_matching, bool allow_geoparquet_parser, bool enable_json_parsing,
                                                   const std::optional<std::unordered_map<String, String>> &parquet_columns_to_clickhouse,
                                                   const std::optional<std::unordered_map<String, String>> &clickhouse_columns_to_parquet) {
  ...
  for (const auto &field : schema.fields()) {
    ...
    auto sample_column = readColumnFromArrowColumn(
        arrow_column, field->name(), field->name(), dict_infos, nullptr /*nested_type_hint*/, field->nullable() /*is_nullable_column*/,
        false /*is_map_nested_column*/, geo_columns.contains(field->name()) ? std::optional(geo_columns[field->name()]) : std::nullopt,
        settings, field, parquet_columns_to_clickhouse, clickhouse_columns_to_parquet);

    if (sample_column.column) sample_columns.emplace_back(std::move(sample_column));
  }

  return Block(std::move(sample_columns));
}

ArrowColumnToCHColumn::ArrowColumnToCHColumn(const Block &header_, const std::string &format_name_, const FormatSettings &format_settings_,
                                             ...
                                             bool allow_missing_columns_, bool null_as_default_,
                                             FormatSettings::DateTimeOverflowBehavior date_time_overflow_behavior_,
                                             bool allow_geoparquet_parser_, bool case_insensitive_matching_, bool is_stream_,
...
Chunk ArrowColumnToCHColumn::arrowTableToCHChunk(const std::shared_ptr<arrow::Table> &table, size_t num_rows,
  ...
  for (auto column_name : table->ColumnNames()) {
    ...
    if (!arrow_column) throw Exception(ErrorCodes::DUPLICATE_COLUMN, "Column '{}' is duplicated", column_name);
    ...
    if (parquet_columns_to_clickhouse) {
      ...
      if (column_name_it == parquet_columns_to_clickhouse->end()) {
        throw Exception(ErrorCodes::LOGICAL_ERROR, "Column '{}' is not present in input data. Column name mapping has {} columns",
                        column_name, parquet_columns_to_clickhouse->size());
      }
      column_name = column_name_it->second;
    }

    if (case_insensitive_matching) boost::to_lower(column_name);
...
Chunk ArrowColumnToCHColumn::arrowColumnsToCHChunk(const NameToArrowColumn &name_to_arrow_column, size_t num_rows,
                                                   std::shared_ptr<const arrow::KeyValueMetadata> metadata,
                                                   BlockMissingValues *block_missing_values) {
  ReadColumnFromArrowColumnSettings settings{
      .format_name = format_name,
  ...
  for (size_t column_i = 0, header_columns = header.columns(); column_i < header_columns; ++column_i) {
    ...
    if (!name_to_arrow_column.contains(search_column_name)) {
      ...
      if (name_to_arrow_column.contains(search_nested_table_name)) {
        if (!nested_tables.contains(search_nested_table_name)) {
          ...
          ColumnsWithTypeAndName cols = {readColumnFromArrowColumn(
              arrow_column.column, nested_table_name, nested_table_name, dictionary_infos, nested_table_type,
              arrow_column.field->nullable() /*is_nullable_column*/, false /*is_map_nested_column*/,
              geo_columns.contains(header_column.name) ? std::optional(geo_columns[header_column.name]) : std::nullopt, settings,
              arrow_column.field, parquet_columns_to_clickhouse, clickhouse_columns_to_parquet)};
      ...
      column = readColumnFromArrowColumn(
          arrow_column.column, header_column.name, header_column.name, dictionary_infos, header_column.type, arrow_column.field->nullable(),
    ...
    if (is_stream) dictionary_infos.clear();
