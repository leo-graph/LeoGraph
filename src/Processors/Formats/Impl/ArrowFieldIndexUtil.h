...
class ArrowFieldIndexUtil {
 public:
  ...
  std::vector<ClickHouseIndexToParquetIndex> findRequiredIndices(
      const Block &header, const arrow::Schema &schema, const parquet::FileMetaData &file,
    ...
    for (size_t i = 0, n = header.columns(); i < n; ++i) {
      ...
      if (clickhouse_to_parquet_names) {
        if (auto it = clickhouse_to_parquet_names->find(col_name); it != clickhouse_to_parquet_names->end()) transformed_name = it->second;
      }
      if (ignore_case) {
        boost::to_lower(col_name);
        boost::to_lower(transformed_name);
      }
      findRequiredIndices(col_name, transformed_name, i, named_col.type, fields_indices, added_indices, required_indices, file,
                          clickhouse_to_parquet_names);
    }
  ...
  size_t countIndicesForType(std::shared_ptr<arrow::DataType> type) {
    ...
    if (type->id() == arrow::Type::STRUCT) {
      ...
      for (int i = 0; i != struct_type->num_fields(); ++i) indices += countIndicesForType(struct_type->field(i)->type());
      return indices;
    }
 ...
 private:
  ...
  void calculateFieldIndices(const arrow::Field &field, std::string field_name, int &current_start_index,
  ...
  void findRequiredIndices(const String &name, const String &transformed_name, std::size_t header_index, DataTypePtr data_type,
                           const std::unordered_map<std::string, std::pair<int, int>> &field_indices,
                           std::unordered_set<int> &added_indices, std::vector<ClickHouseIndexToParquetIndex> &required_indices,
    ...
    if (const DB::DataTypeTuple *type_tuple = typeid_cast<const DB::DataTypeTuple *>(nested_type.get())) {
      if (type_tuple->hasExplicitNames()) {
        ...
        for (size_t i = 0, n = field_names.size(); i < n; ++i) {
          ...
          findRequiredIndices(Nested::concatenateName(name, field_name), full_name, header_index, field_type, field_indices, added_indices,
                              required_indices, file, clickhouse_to_parquet_names);
        }
        return;
      }
    } else if (const auto *type_array = typeid_cast<const DB::DataTypeArray *>(nested_type.get())) {
      findRequiredIndices(name, transformed_name, header_index, type_array->getNestedType(), field_indices, added_indices, required_indices,
                          file, clickhouse_to_parquet_names);
      return;
    } else if (const auto *type_map = typeid_cast<const DB::DataTypeMap *>(nested_type.get())) {
      findRequiredIndices(name, transformed_name, header_index, type_map->getKeyType(), field_indices, added_indices, required_indices,
                          file, clickhouse_to_parquet_names);
      findRequiredIndices(Nested::concatenateName(name, "value"), Nested::concatenateName(transformed_name, "value"), header_index,
                          type_map->getValueType(), field_indices, added_indices, required_indices, file, clickhouse_to_parquet_names);
      return;
    }
    auto it = field_indices.find(transformed_name);
    if (it == field_indices.end()) {
      if (!allow_missing_columns) throw Exception(ErrorCodes::THERE_IS_NO_COLUMN, "Not found field ({})", name);
    } else {
