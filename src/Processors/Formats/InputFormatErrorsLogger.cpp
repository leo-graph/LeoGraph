...
InputFormatErrorsLogger::InputFormatErrorsLogger(const ContextPtr& context)
    : max_block_size(context->getSettingsRef()[Setting::max_block_size]) {
  String output_format = context->getSettingsRef()[Setting::errors_output_format];
  if (!FormatFactory::instance().isOutputFormat(output_format)) output_format = DEFAULT_OUTPUT_FORMAT;
  if (context->hasInsertionTable()) table = context->getInsertionTable().getTableName();
  if (context->getInsertionTable().hasDatabase()) database = context->getInsertionTable().getDatabaseName();
  ...
  if (context->getApplicationType() == Context::ApplicationType::SERVER) {
    ...
    if (!fileOrSymlinkPathStartsWith(errors_file_path, user_files_path))
      throw Exception(ErrorCodes::DATABASE_ACCESS_DENIED, "Cannot log errors in path `{}`, because it is not inside `{}`", errors_file_path,
                      user_files_path);
  } else {
  ...
  header = Block{{std::make_shared<DataTypeDateTime>(), "time"},
...
InputFormatErrorsLogger::~InputFormatErrorsLogger() {
  try {
    if (!errors_columns[0]->empty()) writeErrors();
    writer->finalize();
    writer->flush();
    write_buf->finalize();
  } catch (...) {
...
void InputFormatErrorsLogger::logErrorImpl(ErrorEntry entry) {
  ...
  if (errors_columns[0]->size() >= max_block_size) writeErrors();
}
...
void InputFormatErrorsLogger::logError(ErrorEntry entry) { logErrorImpl(entry); }
