...
void checkFinalInferredType(DataTypePtr& type, const String& name, const FormatSettings& settings, const DataTypePtr& default_type,
                            size_t rows_read, const String& hints_parsing_error) {
  if (!checkIfTypeIsComplete(type)) {
    if (!default_type) {
      if (hints_parsing_error.empty())
        throw Exception(ErrorCodes::ONLY_NULLS_WHILE_READING_SCHEMA,
      ...
      throw Exception(ErrorCodes::ONLY_NULLS_WHILE_READING_SCHEMA,
                      ...
                      name, rows_read, hints_parsing_error);
    }
...
void IIRowSchemaReader::setContext(const ContextPtr& context) {
  ...
  if (tryParseColumnsListFromString(hints_str, columns, context, hints_parsing_error)) {
    for (const auto& [name, type] : columns.getAll()) hints[name] = type;
  } else {
    LOG_WARNING(getLogger("IIRowSchemaReader"), "Couldn't parse schema inference hints: {}. This setting will be ignored",
                hints_parsing_error);
  }
}
...
IRowSchemaReader::IRowSchemaReader(ReadBuffer& in_, const FormatSettings& format_settings_, DataTypePtr default_type_)
    : IIRowSchemaReader(in_, format_settings_, default_type_),
      column_names(splitColumnNames(format_settings.column_names_for_schema_inference)) {}
...
NamesAndTypesList IRowSchemaReader::readSchema() {
  if (max_rows_to_read == 0 || max_bytes_to_read == 0)
    throw Exception(ErrorCodes::BAD_ARGUMENTS,
                    ...
                    "Most likely setting input_format_max_rows_to_read_for_schema_inference or "
                    "input_format_max_bytes_to_read_for_schema_inference is set to 0");

  auto data_types_maybe = readRowAndGetDataTypes();

  /// Check that we read at list one column.
  if (!data_types_maybe) throw Exception(ErrorCodes::EMPTY_DATA_PASSED, "Cannot read rows from the data");
  ...
  if (use_default_column_names) {
    ...
    for (size_t i = 0; i != data_types.size(); ++i) column_names.push_back("c" + std::to_string(i + 1));
  }
  /// If column names were set, check that the number of names match the number of types.
    ...
    if (column_names.size() != data_types.size()) data_types.resize(column_names.size());
  ...
  for (size_t i = 0; i != column_names.size(); ++i) {
    ...
    if (hint_it != hints.end()) data_types[i] = hint_it->second;
  }
  ...
  for (rows_read = 1; rows_read < max_rows_to_read && in.count() < max_bytes_to_read; ++rows_read) {
    ...
    if (new_data_types.size() != data_types.size()) {
      if (!allowVariableNumberOfColumns()) throw Exception(ErrorCodes::INCORRECT_DATA, "Rows have different amount of values");

      if (use_default_column_names) {
        /// Current row contains new columns, add new default names.
        if (new_data_types.size() > data_types.size()) {
          for (size_t i = data_types.size(); i < new_data_types.size(); ++i) column_names.push_back("c" + std::to_string(i + 1));
    ...
    for (field_index = 0; field_index != data_types.size(); ++field_index) {
      ...
      if (!new_data_types[field_index] || hints.contains(column_names[field_index])) continue;

      chooseResultColumnType(*this, data_types[field_index], new_data_types[field_index], getDefaultType(field_index),
                             std::to_string(field_index + 1), rows_read, hints_parsing_error);
    }
  }
...
Strings splitColumnNames(const String& column_names_str) {
  if (column_names_str.empty()) return {};
  ...
  for (auto& column_name : column_names) {
    ...
    if (!col_name_trimmed.empty()) column_name = col_name_trimmed;
  }
  return column_names;
}

DataTypePtr IRowSchemaReader::getDefaultType(size_t column) const {
  if (default_type) return default_type;
  if (column < default_types.size() && default_types[column]) return default_types[column];
  return nullptr;
}
...
NamesAndTypesList IRowWithNamesSchemaReader::readSchema() {
  ...
  /// Check that we read at list one column.
  if (names_to_types.empty()) throw Exception(ErrorCodes::EMPTY_DATA_PASSED, "Cannot read rows from the data");
