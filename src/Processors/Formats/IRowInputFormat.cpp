...
bool isParseError(int code) {
  return code == ErrorCodes::CANNOT_PARSE_INPUT_ASSERTION_FAILED || code == ErrorCodes::CANNOT_PARSE_QUOTED_STRING ||
         code == ErrorCodes::CANNOT_PARSE_DATE || code == ErrorCodes::CANNOT_PARSE_DATETIME || code == ErrorCodes::CANNOT_PARSE_NUMBER ||
         code == ErrorCodes::CANNOT_PARSE_UUID || code == ErrorCodes::CANNOT_PARSE_BOOL ||
         code == ErrorCodes::CANNOT_READ_ARRAY_FROM_TEXT || code == ErrorCodes::CANNOT_READ_MAP_FROM_TEXT ||
         code == ErrorCodes::CANNOT_READ_ALL_DATA || code == ErrorCodes::TOO_LARGE_STRING_SIZE ||
         code == ErrorCodes::ARGUMENT_OUT_OF_BOUND  /// For Decimals
         || code == ErrorCodes::INCORRECT_DATA      /// For some ReadHelpers
         || code == ErrorCodes::CANNOT_PARSE_DOMAIN_VALUE_FROM_STRING || code == ErrorCodes::CANNOT_PARSE_IPV4 ||
         code == ErrorCodes::CANNOT_PARSE_IPV6 || code == ErrorCodes::UNKNOWN_ELEMENT_OF_ENUM ||
         code == ErrorCodes::CANNOT_PARSE_ESCAPE_SEQUENCE || code == ErrorCodes::UNEXPECTED_DATA_AFTER_PARSED_VALUE;
}
...
Chunk IRowInputFormat::read() {
  if (got_connection_exception) return {};
      ...
      try {
        ...
        for (size_t column_idx = 0; column_idx < info.read_columns.size(); ++column_idx) {
          if (!info.read_columns[column_idx]) {
            ...
            if (column_size == 0) throw Exception(ErrorCodes::INCORRECT_NUMBER_OF_COLUMNS, "Unexpected empty column");
        ...
        if (!columns.empty()) num_rows = columns.front()->size();

        if (!continue_reading) break;

        /// The case when there is no columns. Just count rows.
        if (columns.empty()) ++num_rows;

        if (min_block_size_bytes || max_block_size_bytes) {
          for (const auto& column : columns) total_bytes += column->byteSizeAt(column->size() - 1);
        }

        if (max_block_wait_ms != 0 && num_rows > 0 && watch.elapsedMilliseconds() >= max_block_wait_ms) break;
      } catch (Exception& e) {
        ++total_rows;
        ...
        if (params.allow_errors_num == 0 && params.allow_errors_ratio == 0) throw;

        if (errors_logger) logError();

        ++num_errors;
        Float64 current_error_ratio = static_cast<Float64>(num_errors) / static_cast<double>(total_rows);

        if (num_errors > params.allow_errors_num && current_error_ratio > params.allow_errors_ratio) {
          e.addMessage("(Already have " + toString(num_errors) +
                       " errors"
                       " out of " +
                       toString(total_rows) +
                       " rows"
        ...
        for (size_t column_idx = 0; column_idx < num_columns; ++column_idx) {
          ...
          if (column->size() > num_rows) column->popBack(column->size() - num_rows);
        }
      }
    }
  } catch (Exception& e) {
...
void IRowInputFormat::setSerializationHints(const SerializationInfoByName& hints) {
  if (supportsCustomSerializations()) serializations = getPort().getHeader().getSerializations(hints);
}
