...
RemoteSource::RemoteSource(RemoteQueryExecutorPtr executor, bool add_aggregation_info_, bool async_read_, bool async_query_sending_)
  ...
  for (auto &type : sample.getDataTypes())
    if (typeid_cast<const DataTypeAggregateFunction *>(type.get())) add_aggregation_info = true;

  /// Progress method will be called on Progress packet.
  query_executor->setProgressCallback([this](const Progress &value) {
    if (value.total_rows_to_read) addTotalRowsApprox(value.total_rows_to_read);
    if (value.total_bytes_to_read) addTotalBytes(value.total_bytes_to_read);
  ...
  query_executor->setProfileInfoCallback([this](const ProfileInfo &info) {
    ...
    if (rows_before_aggregation) {
      if (info.hasAppliedAggregation()) rows_before_aggregation->add(info.getRowsBeforeAggregation());
    }
  });
}
...
void RemoteSource::setStorageLimits(const std::shared_ptr<const StorageLimitsList> &storage_limits_) {
  ...
  for (const auto &value : *storage_limits_) list.emplace_back(StorageLimits{value.local_limits, {}});
...
std::optional<Chunk> RemoteSource::tryGenerate() {
  ...
  if (block.empty()) {
    if (manually_add_rows_before_limit_counter) rows_before_limit->add(rows);
    query_executor->finish();
    return {};
  }
...
void RemoteSource::onUpdatePorts() {
  if (isCancelled()) return;
  if (getPort().isFinished()) query_executor->finish();
}

RemoteTotalsSource::RemoteTotalsSource(RemoteQueryExecutorPtr executor)
    : ISource(executor->getSharedHeader()), query_executor(std::move(executor)) {}
...
void UnmarshallBlocksTransform::transform(Chunk &chunk) {
  ...
  for (auto &column : columns) {
    if (const auto *col = typeid_cast<const ColumnBLOB *>(column.get())) column = col->convertFrom();
  }
  chunk.setColumns(std::move(columns), rows);
}

Pipe createRemoteSourcePipe(RemoteQueryExecutorPtr query_executor, bool add_aggregation_info, bool add_totals, bool add_extremes,
                            bool async_read, bool async_query_sending, size_t parallel_marshalling_threads) {
  ...
  if (add_totals) pipe.addTotalsSource(std::make_shared<RemoteTotalsSource>(query_executor));

  if (add_extremes) pipe.addExtremesSource(std::make_shared<RemoteExtremesSource>(query_executor));
