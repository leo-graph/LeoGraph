...
SQLiteSource::SQLiteSource(SQLitePtr sqlite_db_, const String &query_str_, const Block &sample_block, const UInt64 max_block_size_)
  ...
  int status = sqlite3_prepare_v2(sqlite_db.get(), query_str.c_str(), static_cast<int>(query_str.size() + 1), &compiled_stmt, nullptr);

  if (status != SQLITE_OK)
    throw Exception(ErrorCodes::SQLITE_ENGINE_ERROR, "Cannot prepare sqlite statement. Status: {}. Message: {}", status,
...
Chunk SQLiteSource::generate() {
  ...
  while (true) {
    ...
    if (status != SQLITE_ROW) {
      throw Exception(ErrorCodes::SQLITE_ENGINE_ERROR, "Expected SQLITE_ROW status, but got status {}. Error: {}, Message: {}", status,
                      sqlite3_errstr(status), sqlite3_errmsg(sqlite_db.get()));
    }
