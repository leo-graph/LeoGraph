...
LazyReadFromMergeTreeSource::LazyReadFromMergeTreeSource(SharedHeader header, size_t max_block_size_, size_t max_threads_,
                                                         size_t min_marks_for_concurrent_read_, ExpressionActionsSettings actions_settings_,
                                                         MergeTreeReaderSettings reader_settings_,
                                                         MergeTreeData::MutationsSnapshotPtr mutations_snapshot_,
                                                         StorageSnapshotPtr storage_snapshot_, ContextPtr context_,
                                                         const std::string& log_name_, LazyMaterializingRowsPtr lazy_materializing_rows_,
  ...
  for (auto& part : parts_with_ranges) {
    ...
    while (!part.ranges.empty()) {
      if (marks <= marks_per_stream) {
        split_parts_and_ranges.emplace_back(part.data_part, part.parent_part, part.part_index_in_query, part.part_starting_offset_in_query,
      ...
      while (added_marks < marks_per_stream && !part.ranges.empty()) {
        ...
        bool split_range = range_marks + added_marks > marks_per_stream         /// range overflows limit
                           && range_marks >= 2 * min_marks_for_concurrent_read  /// range is big enough for 2 concurrent reads
                           && (range_marks + added_marks - marks_per_stream) >
                                  min_marks_for_concurrent_read;  /// what's rest is big enough for concurrent read
...
IProcessor::Status LazyReadFromMergeTreeSource::prepare(const PortNumbers& updated_input_ports,
  ...
  if (lazy_materializing_rows) return Status::ExpandPipeline;
...
Processors LazyReadFromMergeTreeSource::expandPipeline() {
  if (!lazy_materializing_rows) throw Exception(ErrorCodes::LOGICAL_ERROR, "LazyReadFromMergeTreeSource: No lazy materializing rows");
...
Processors LazyReadFromMergeTreeSource::buildReaders() {
  ...
  auto pool = std::make_shared<MergeTreeReadPoolInOrder>(
      has_limit_below_one_block, MergeTreeReadType::InOrder, ranges_in_data_parts, mutations_snapshot, shared_virtual_fields,
      /*index_read_tasks*/ IndexReadTasks{}, storage_snapshot,
      /* row_level_filter */ nullptr,
      /* prewhere_info */ nullptr, actions_settings, reader_settings, outputs.front().getHeader().getNames(), pool_settings, block_size,
      context, updater);
  ...
  for (size_t i = 0; i < ranges_in_data_parts.size(); ++i) {
    ...
    auto processor = std::make_unique<MergeTreeSelectProcessor>(pool, std::move(algorithm), nullptr, nullptr,
                                                                /*index_read_tasks*/ IndexReadTasks{}, actions_settings, reader_settings,
                                                                /*index_build_context*/ nullptr, lazy_materializing_rows);
