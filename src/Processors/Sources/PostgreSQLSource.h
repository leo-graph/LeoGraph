...
class PostgreSQLSource : public ISource {
 public:
  PostgreSQLSource(postgres::ConnectionHolderPtr connection_holder_, const String& query_str_, SharedHeader sample_block,
 ...
 protected:
  PostgreSQLSource(std::shared_ptr<T> tx_, const std::string& query_str_, SharedHeader sample_block, UInt64 max_block_size_,
                   bool auto_commit_);
...
class PostgreSQLTransactionSource : public PostgreSQLSource<T> {
 public:
  ...
  PostgreSQLTransactionSource(std::shared_ptr<T> tx_, const std::string& query_str_, SharedHeader sample_block_,
