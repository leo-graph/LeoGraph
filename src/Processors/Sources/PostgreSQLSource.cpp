...
template <typename T>
PostgreSQLSource<T>::PostgreSQLSource(postgres::ConnectionHolderPtr connection_holder_, const std::string &query_str_,
                                      SharedHeader sample_block, UInt64 max_block_size_)
...
template <typename T>
PostgreSQLSource<T>::PostgreSQLSource(std::shared_ptr<T> tx_, const std::string &query_str_, SharedHeader sample_block,
                                      UInt64 max_block_size_, bool auto_commit_)
    : ISource(std::make_shared<const Block>(sample_block->cloneEmpty())),
      max_block_size(max_block_size_),
...
void PostgreSQLSource<T>::init(const Block &sample_block) {
  ...
  if (query_str.ends_with(';')) query_str.resize(query_str.size() - 1);
}
...
Chunk PostgreSQLSource<T>::generate() {
  ...
  if (is_completed.load() || isCancelled()) return {};
  ...
  while (!isCancelled() && !is_completed.load()) {
    ...
    if (row->size() > description.sample_block.columns())
      throw Exception(ErrorCodes::TOO_MANY_COLUMNS, "Row has too many columns: {}, expected structure: {}", row->size(),
                      description.sample_block.dumpStructure());
    ...
    for (const auto idx : collections::range(0, row->size())) {
      ...
      if ((*row)[idx].c_str()) {
        if (description.types[idx].second) {
          ...
          insertPostgreSQLValue(column_nullable.getNestedColumn(), (*row)[idx], description.types[idx].first, data_type.getNestedType(),
                                array_info, idx);

          column_nullable.getNullMapData().emplace_back(false);
        } else {
          insertPostgreSQLValue(*columns[idx], (*row)[idx], description.types[idx].first, sample.type, array_info, idx);
        }
      } else {
    ...
    if (++num_rows == max_block_size) break;
  }
...
void PostgreSQLSource<T>::onCancel() noexcept {
  ...
  if (!started.load() && tx && tx->conn().is_open()) {
    ...
    if (connection_holder) connection_holder->setBroken();
  }
}
...
template class PostgreSQLSource<pqxx::ReplicationTransaction>;

template class PostgreSQLSource<pqxx::ReadTransaction>;
