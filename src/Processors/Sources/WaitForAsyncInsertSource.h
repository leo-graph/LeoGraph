...
class WaitForAsyncInsertSource : public ISource, WithContext {
 public:
  WaitForAsyncInsertSource(std::future<void> insert_future_, size_t timeout_ms_)
      : ISource(std::make_shared<const Block>()), insert_future(std::move(insert_future_)), timeout_ms(timeout_ms_) {
 ...
 protected:
  Chunk generate() override {
    ...
    if (status == std::future_status::deferred) throw Exception(ErrorCodes::LOGICAL_ERROR, "Got future in deferred state");
