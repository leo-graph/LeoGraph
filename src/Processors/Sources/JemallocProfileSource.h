...
class JemallocProfileSource : public ISource {
 public:
  JemallocProfileSource(const std::string& filename_, const SharedHeader& header_, size_t max_block_size_, JemallocProfileFormat mode_,
                        bool symbolize_with_inline_, bool collapsed_use_count_ = false);
 ...
 private:
  enum class SymbolizedPhase { CollectingAddresses, OutputtingSymbolHeader, OutputtingSymbols, OutputtingHeapHeader, OutputtingHeap, Done };
...
void symbolizeJemallocHeapProfile(const std::string& input_filename, const std::string& output_filename,
                                  JemallocProfileFormat format = JemallocProfileFormat::Symbolized, bool symbolize_with_inline = true);
