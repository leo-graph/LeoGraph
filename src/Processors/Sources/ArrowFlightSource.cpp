...
ArrowFlightSource::ArrowFlightSource(std::shared_ptr<ArrowFlightConnection> connection_, const String& dataset_name_,
                                     const Block& sample_block_, ContextPtr context_)
    : ISource(std::make_shared<const Block>(sample_block_.cloneEmpty())), connection(connection_), sample_block(sample_block_) {
  initializeEndpoints(dataset_name_, context_);
}

ArrowFlightSource::ArrowFlightSource(std::shared_ptr<ArrowFlightConnection> connection_,
                                     std::vector<arrow::flight::FlightEndpoint> endpoints_, const Block& sample_block_)
    : ISource(std::make_shared<const Block>(sample_block_.cloneEmpty())),
      connection(connection_),
      sample_block(sample_block_),
      endpoints(std::move(endpoints_)) {}

ArrowFlightSource::ArrowFlightSource(std::unique_ptr<arrow::flight::MetadataRecordBatchReader> stream_reader_, const Block& sample_block_)
    : ISource(std::make_shared<const Block>(sample_block_.cloneEmpty())),
      sample_block(sample_block_),
...
void ArrowFlightSource::initializeEndpoints(const String& dataset_name_, ContextPtr context) {
  ...
  if (!flight_info_res.ok()) {
    throw Exception(ErrorCodes::ARROWFLIGHT_FETCH_SCHEMA_ERROR, "Failed to get FlightInfo from Arrow Flight server: {}",
...
bool ArrowFlightSource::nextEndpoint() {
  if (current_endpoint >= endpoints.size()) return false;
  ...
  if (!stream_reader_res.ok()) {
    throw Exception(ErrorCodes::ARROWFLIGHT_CONNECTION_FAILURE, "Failed to initialize Arrow Flight stream: {}",
                    stream_reader_res.status().ToString());
  }
...
Chunk ArrowFlightSource::generate() {
  ...
  ArrowColumnToCHColumn converter(sample_block, "Arrow",
                                  ...
                                  /* null_as_default = */ true, FormatSettings::DateTimeOverflowBehavior::Throw,
