...
class PrimesSource final : public ISource {
 public:
  PrimesSource(UInt64 block_size_, UInt64 offset_, std::optional<UInt64> limit_, UInt64 step_, const std::string& column_name)
      : ISource(createHeader(column_name)), block_size(block_size_), limit(limit_), step(step_), next_selected_index(offset_) {
    if (step_ == 0) throw Exception(ErrorCodes::LOGICAL_ERROR, "Step size in PrimesSource cannot be zero");
  }
 ...
 protected:
  Chunk generate() override {
    if (limit && produced_rows >= *limit) return {};

    UInt64 target = block_size;
    if (limit) target = std::min<UInt64>(target, *limit - produced_rows);
    ...
    while (written < target) {
      ...
      if (cur_prime_index < next_selected_index) continue;
...
class PrimesRangedSource final : public ISource {
 public:
  PrimesRangedSource(UInt64 block_size_, std::vector<Interval> intervals_, UInt64 offset_, std::optional<UInt64> limit_, UInt64 step_,
    ...
    if (step_ == 0) throw Exception(ErrorCodes::LOGICAL_ERROR, "Step size in PrimesRangedSource cannot be zero");

    if (intervals.empty()) throw Exception(ErrorCodes::LOGICAL_ERROR, "Intervals in PrimesRangedSource cannot be empty");
 ...
 protected:
  Chunk generate() override {
    if (interval_idx >= intervals.size()) return {};
    ...
    while (written < target) {
      ...
      while (interval_idx < intervals.size() && prime > intervals[interval_idx].second) ++interval_idx;

      if (interval_idx >= intervals.size()) break;

      if (prime < intervals[interval_idx].first) continue;
...
class PrimesSimpleRangedSource final : public ISource {
 public:
  PrimesSimpleRangedSource(UInt64 block_size_, std::vector<Interval> intervals_, std::optional<UInt64> limit_,
                           const std::string& column_name)
      : ISource(PrimesSource::createHeader(column_name)), block_size(block_size_), intervals(std::move(intervals_)), limit(limit_) {
    if (intervals.empty()) throw Exception(ErrorCodes::LOGICAL_ERROR, "Intervals in PrimesSimpleRangedSource cannot be empty");
 ...
 protected:
  Chunk generate() override {
    ...
    while (written < target) {
      ...
      if (!prime_opt) {
        ++interval_idx;
        if (interval_idx < intervals.size()) prime_generator.setRange(intervals[interval_idx].first, intervals[interval_idx].second);
        continue;
      }
