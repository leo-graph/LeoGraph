...
void MongoDBSource::insertValue(IColumn &column, const size_t &idx, const DataTypePtr &type, const std::string &name,
  ...
  switch (type->getTypeId()) {
    ...
    case TypeIndex::Date: {
      if (value.type() != bsoncxx::type::k_date)
        throw Exception(ErrorCodes::TYPE_MISMATCH, "Type mismatch, expected date, got {} for column {}", bsoncxx::to_string(value.type()),
                        name);

      assert_cast<ColumnUInt16 &>(column).insertValue(
          static_cast<UInt16>(DateLUT::instance().toDayNum(value.get_date().to_int64() / 1000)));
      break;
    }
    ...
    case TypeIndex::Array: {
      if (value.type() != bsoncxx::type::k_array)
        throw Exception(ErrorCodes::TYPE_MISMATCH, "Type mismatch, expected array, got {} for column {}", bsoncxx::to_string(value.type()),
                        name);

      assert_cast<ColumnArray &>(column).insert(BSONArrayAsArray(arrays_info[idx].first, value.get_array(), arrays_info[idx].second.first,
                                                                 arrays_info[idx].second.second, name, json_format_settings));
      break;
    }
    default:
...
MongoDBSource::MongoDBSource(const mongocxx::uri &uri, const std::string &collection_name, const bsoncxx::document::view_or_value &query,
                             const mongocxx::options::find &options, SharedHeader sample_block_, const UInt64 &max_block_size_)
...
Chunk MongoDBSource::generate() {
  ...
  if (num_rows < max_block_size) all_read = true;
