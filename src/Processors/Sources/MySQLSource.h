...
class MySQLSource : public ISource {
 public:
  MySQLSource(const mysqlxx::PoolWithFailover::Entry& entry, const std::string& query_str, const Block& sample_block,
...
class MySQLWithFailoverSource final : public MySQLSource {
 public:
  MySQLWithFailoverSource(mysqlxx::PoolWithFailoverPtr pool_, const std::string& query_str_, const Block& sample_block_,
                          const StreamSettings& settings_);
