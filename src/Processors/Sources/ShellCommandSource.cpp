...
static bool tryMakeFdNonBlocking(int fd) {
  ...
  if (-1 == fcntl(fd, F_SETFL, flags | O_NONBLOCK)) return false;
...
static void makeFdNonBlocking(int fd) {
  ...
  if (!result) throw ErrnoException(ErrorCodes::CANNOT_FCNTL, "Cannot set non-blocking mode of pipe");
}
...
static void makeFdBlocking(int fd) {
  ...
  if (!result) throw ErrnoException(ErrorCodes::CANNOT_FCNTL, "Cannot set blocking mode of pipe");
}
...
static int pollWithTimeout(pollfd* pfds, size_t num, size_t timeout_milliseconds) {
  ...
  while (true) {
    ...
    if (res < 0) {
      if (errno != EINTR) throw ErrnoException(ErrorCodes::CANNOT_POLL, "Cannot poll");
  ...
  LOG_TEST(logger, "Poll for descriptors: {} returned {}",
           fmt::join(std::span(pfds, pfds + num) | std::views::transform(describe_fd), ", "), res);
...
class TimeoutReadBufferFromFileDescriptor : public BufferWithOwnMemory<ReadBuffer> {
 public:
  explicit TimeoutReadBufferFromFileDescriptor(int stdout_fd_, int stderr_fd_, size_t timeout_milliseconds_,
                                               ExternalCommandStderrReaction stderr_reaction_)
      : stdout_fd(stdout_fd_), stderr_fd(stderr_fd_), timeout_milliseconds(timeout_milliseconds_), stderr_reaction(stderr_reaction_) {
  ...
  bool nextImpl() override {
    ...
    while (!bytes_read) {
      ...
      if (has_stderr) {
        if (stderr_read_buf == nullptr) stderr_read_buf.reset(new char[BUFFER_SIZE]);
        ...
        if (res > 0) {
          ...
          if (stderr_reaction == ExternalCommandStderrReaction::THROW) {
            ...
            if (current_size < MAX_STDERR_SIZE) {
              if (!stderr_full_output) stderr_full_output.emplace();
            ...
            if (res > 0) stderr_result_buf.insert(stderr_result_buf.end(), str.begin(), str.begin() + res);
      ...
      if (has_stdout) {
        ...
        if (-1 == res && errno != EINTR) throw ErrnoException(ErrorCodes::CANNOT_READ_FROM_FILE_DESCRIPTOR, "Cannot read from pipe");
  ...
  ~TimeoutReadBufferFromFileDescriptor() override {
    ...
    if (!stderr_result_buf.empty()) {
      ...
      if (stderr_reaction == ExternalCommandStderrReaction::LOG_FIRST || stderr_reaction == ExternalCommandStderrReaction::LOG_LAST) {
        LOG_WARNING(getLogger("ShellCommandSource"), "Executable generates stderr at the {}: {}",
                    stderr_reaction == ExternalCommandStderrReaction::LOG_FIRST ? "beginning" : "end", stderr_result);
      }
    }
  }
...
class TimeoutWriteBufferFromFileDescriptor : public BufferWithOwnMemory<WriteBuffer> {
 public:
  ...
  void reset() const { makeFdBlocking(fd); }

  ~TimeoutWriteBufferFromFileDescriptor() override { tryMakeFdBlocking(fd); }
...
class ShellCommandHolder {
 public:
  ...
  explicit ShellCommandHolder(ShellCommandBuilderFunc&& func_) : func(std::move(func_)) {}

  std::unique_ptr<ShellCommand> buildCommand() {
    if (returned_command) return std::move(returned_command);
...
class ShellCommandSource final : public ISource {
 public:
  ...
  ShellCommandSource(ContextPtr context_, const std::string& format_, size_t command_read_timeout_milliseconds,
                     ExternalCommandStderrReaction stderr_reaction, bool check_exit_code_, SharedHeader sample_block_,
                     std::unique_ptr<ShellCommand>&& command_, std::vector<SendDataTask>&& send_data_tasks = {},
                     const ShellCommandSourceConfiguration& configuration_ = {},
                     std::unique_ptr<ShellCommandHolder>&& command_holder_ = nullptr, std::shared_ptr<ProcessPool> process_pool_ = nullptr)
    ...
    if (configuration.read_fixed_number_of_rows) context_for_reading->setSetting("input_format_parallel_parsing", false);
  ...
  ~ShellCommandSource() override { cleanup(); }
 ...
 protected:
  void cleanup() {
    ...
    if (command_is_invalid) command = nullptr;
    ...
    if (command_holder && process_pool) {
      ...
      if (command && valid_command) command_holder->returnCommand(std::move(command));
  ...
  Chunk generate() override {
    ...
    try {
      if (configuration.read_fixed_number_of_rows) {
        ...
        if (current_read_rows >= configuration.number_of_rows_to_read) return {};
      }

      if (!executor->pull(chunk)) return {};
  ...
  Status prepare() override {
    ...
    if (status == Status::Finished) {
      ...
      if (timeout_command_out.hasStderr()) {
        throw Exception(ErrorCodes::UNSUPPORTED_METHOD, "Executable generates stderr: {}", timeout_command_out.getStderr());
      }
...
class SendingChunkHeaderTransform final : public ISimpleTransform {
 public:
  SendingChunkHeaderTransform(SharedHeader header, WriteBuffer& buffer_) : ISimpleTransform(header, header, false), buffer(buffer_) {}
...
ShellCommandSourceCoordinator::ShellCommandSourceCoordinator(const Configuration& configuration_) : configuration(configuration_) {
...
Pipe ShellCommandSourceCoordinator::createPipe(const std::string& command, const VectorWithMemoryTracking<std::string>& arguments,
                                               std::vector<Pipe>&& input_pipes, Block sample_block, ContextPtr context,
  ...
  for (size_t i = 1; i < input_pipes.size(); ++i) command_config.write_fds.emplace_back(i + 2);
  ...
  if (is_executable_pool) {
    ...
    bool result = process_pool->tryBorrowObject(
        ...
        [command_config, execute_direct]() {
          ShellCommandHolder::ShellCommandBuilderFunc func = [command_config, execute_direct]() mutable {
            if (execute_direct) return ShellCommand::executeDirect(command_config);
    ...
    if (!result)
      throw Exception(ErrorCodes::TIMEOUT_EXCEEDED, "Could not get process from pool, max command execution timeout exceeded {} seconds",
  ...
  auto source = std::make_unique<ShellCommandSource>(context, configuration.format, configuration.command_read_timeout_milliseconds,
                                                     configuration.stderr_reaction, configuration.check_exit_code,
                                                     std::make_shared<const Block>(std::move(sample_block)), std::move(process),
                                                     std::move(tasks), source_configuration, std::move(process_holder), process_pool);
