...
std::optional<UInt64> parseHexAddress(std::string_view &src) {
  if (src.size() >= 2 && src[0] == '0' && (src[1] == 'x' || src[1] == 'X')) src.remove_prefix(2);
...
JemallocProfileSource::JemallocProfileSource(const std::string &filename_, const SharedHeader &header_, size_t max_block_size_,
                                             JemallocProfileFormat mode_, bool symbolize_with_inline_, bool collapsed_use_count_)
    : ISource(header_),
      filename(filename_),
...
Chunk JemallocProfileSource::generateRaw() {
  if (!file_input) file_input = std::make_unique<ReadBufferFromFile>(filename);
  ...
  if (file_input->eof()) is_finished = true;
  ...
  while (column->size() < max_block_size) {
    ...
    if (symbolized_phase == SymbolizedPhase::OutputtingSymbolHeader) {
      ...
      if (!symbol_header_line_output) {
        ...
        if (column->size() >= max_block_size) break;
      }
    ...
    if (symbolized_phase == SymbolizedPhase::OutputtingSymbols) {
      ...
      while (current_address_index < addresses.size() && column->size() < max_block_size) {
        ...
        if (auto cached = symbolization_cache.get(address)) symbols = std::move(*cached);

        if (symbols.empty()) {
          FramePointers fp;
          fp[0] = reinterpret_cast<void *>(address);

          auto symbolize_callback = [&](const StackTrace::Frame &frame) { symbols.push_back(frame.symbol.value_or("??")); };
        ...
        if (second_colon != std::string::npos) {
          ...
          if (metric > 0) {
            ...
            for (UInt64 address : std::ranges::reverse_view(current_stack)) {
              ...
              if (auto cached = symbolization_cache.get(address)) addr_symbols = std::move(*cached);
              if (!addr_symbols.empty()) {
                for (const auto &symbol : addr_symbols) all_symbols.push_back(symbol);
              } else {
                FramePointers fp;
                fp[0] = reinterpret_cast<void *>(address);

                std::vector<std::string> frame_symbols;
                auto symbolize_callback = [&](const StackTrace::Frame &frame) { frame_symbols.push_back(frame.symbol.value_or("??")); };
...
void symbolizeJemallocHeapProfile(const std::string &input_filename, const std::string &output_filename, JemallocProfileFormat format,
                                  bool symbolize_with_inline) {
  Block header;
  header.insert({ColumnString::create(), std::make_shared<DataTypeString>(), "line"});
  auto source = std::make_shared<JemallocProfileSource>(input_filename, std::make_shared<const Block>(std::move(header)),
                                                        DEFAULT_BLOCK_SIZE, format, symbolize_with_inline);
