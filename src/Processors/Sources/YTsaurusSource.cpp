...
YTsaurusTableSourceStaticTable::YTsaurusTableSourceStaticTable(YTsaurusClientPtr client_, const String& cypress_path_,
                                                               std::pair<size_t, size_t> rows_range_,
                                                               const YTsaurusTableSourceOptions& source_options_,
                                                               const SharedHeader& sample_block_, const UInt64& max_block_size_)
...
YTsaurusTableSourceDynamicTable::YTsaurusTableSourceDynamicTable(YTsaurusClientPtr client_, const String& cypress_path,
                                                                 const YTsaurusTableSourceOptions& source_options_,
                                                                 const SharedHeader& sample_block_, const UInt64& max_block_size_)
    : ISource(sample_block_),
      client(std::move(client_)),
...
Pipe YTsaurusSourceFactory::createPipe(YTsaurusClientPtr client, const String& table_cypress_path,
                                       YTsaurusTableSourceOptions source_options, const SharedHeader& sample_block,
                                       const UInt64 max_block_size, UInt64 max_streams) {
  ...
  if (settings[YTsaurusSetting::use_lock]) {
    source_options.table_lock = std::make_shared<YTsaurusTableLock>(
        client, table_cypress_path, source_options.settings[YTsaurusSetting::transaction_timeout_ms].totalMilliseconds());
    cypress_path = source_options.table_lock->getNodePath();
  } else {
    cypress_path = table_cypress_path;
  }

  if (source_options.settings[YTsaurusSetting::check_table_schema] &&
      !client->checkSchemaCompatibility(cypress_path, sample_block, reason, source_options.check_types_allow_nullable)) {
  ...
  if (yt_node_type == YTsaurusNodeType::STATIC_TABLE) {
    ...
    if (min_rows_for_spawn_stream)
      pipes_num = std::min(max_streams_allowed, std::max<size_t>(1u, rows_count / settings[YTsaurusSetting::min_rows_for_spawn_stream]));

    size_t rows_batch_count = rows_count / pipes_num;
    Pipes pipes;
    LOG_DEBUG(::getLogger("YTsaurusSourceFactory"), "Will read static table {} with {} streams and rows {} in each", table_cypress_path,
              pipes_num, rows_batch_count);
    ...
    for (auto i = 0u; i < pipes_num; ++i) {
      ...
      pipes.emplace_back(std::make_shared<YTsaurusTableSourceStaticTable>(client_for_source, cypress_path, std::make_pair(row_from, row_to),
                                                                          source_options, sample_block, max_block_size));
    }
