...
IProcessor::Status DelayedSource::prepare() {
  ...
  if (inputs.empty()) {
    ...
    if (!first_output.isNeeded()) return Status::PortFull;

    /// Call creator callback to get processors.
    if (processors.empty()) return Status::Ready;
...
void DelayedSource::work() {
  ...
  if (rows_before_limit) {
    for (auto& processor : processors) processor->setRowsBeforeLimitCounter(rows_before_limit);
  }

  if (rows_before_aggregation) {
    for (auto& processor : processors) processor->setRowsBeforeAggregationCounter(rows_before_aggregation);
  }
...
Processors DelayedSource::expandPipeline() {
  ...
  for (const auto& output : {main_output, totals_output, extremes_output}) {
    ...
    if (output == main_output) inputs.back().setNeeded();
  }
