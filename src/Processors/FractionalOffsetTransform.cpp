...
FractionalOffsetTransform::FractionalOffsetTransform(const Block& header_, Float64 fractional_offset_, size_t num_streams)
    : IProcessor(InputPorts(num_streams, header_), OutputPorts(num_streams, header_)), fractional_offset(fractional_offset_) {
    ...
    auto process = [&](size_t pos) {
      ...
      switch (status) {
        ...
        case IProcessor::Status::NeedData:
          return;
        default:
          throw Exception(ErrorCodes::LOGICAL_ERROR, "Unexpected status for FractionalOffsetTransform::pullData : {}",
                          IProcessor::statusToName(status));
      }
    };

    for (size_t pos = 0; pos < ports_data.size(); ++pos) process(pos);
...
FractionalOffsetTransform::Status FractionalOffsetTransform::pullData(PortsData& data) {
  ...
  if (rows_before_limit_at_least) rows_before_limit_at_least->add(rows);
  ...
  /// example: if we have 10 blocks with the same num of rows and offset 0.1 we can freely drop the first block even before reading all
  /// data.
  while (!chunks_cache.empty() && static_cast<UInt64>(std::ceil(static_cast<double>(rows_cnt) * fractional_offset)) - evicted_rows_cnt >=
                                      chunks_cache.front().getNumRows()) {
...
void FractionalOffsetTransform::splitChunk(Chunk& current_chunk) const {
  ...
  for (UInt64 i = 0; i < num_columns; ++i) columns[i] = columns[i]->cut(offset, length);
