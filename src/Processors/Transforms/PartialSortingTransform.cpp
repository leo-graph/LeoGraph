...
ColumnRawPtrs extractRawColumns(const Block& block, const SortDescriptionWithPositions& description) {
  ...
  for (size_t i = 0; i < size; ++i) result[i] = block.safeGetByPosition(description[i].column_number).column.get();

  return result;
}

size_t getFilterMask(const ColumnRawPtrs& raw_block_columns, const Columns& threshold_columns, const SortDescription& description,
                     size_t num_rows, IColumn::Filter& filter, PaddedPODArray<UInt64>& rows_to_compare,
  ...
  if (description.size() == 1) {
    ...
    raw_block_columns[0]->compareColumn(*threshold_columns[0], 0, nullptr, compare_results, description[0].direction,
    ...
    for (size_t i = 0; i < size; ++i) {
      raw_block_columns[i]->compareColumn(*threshold_columns[i], 0, &rows_to_compare, compare_results, description[i].direction,
                                          description[i].nulls_direction);

      if (rows_to_compare.empty()) break;
    }
  }
...
bool compareWithThreshold(const ColumnRawPtrs& raw_block_columns, size_t min_block_index, const Columns& threshold_columns,
...
PartialSortingTransform::PartialSortingTransform(SharedHeader header_, const SortDescription& description_, UInt64 limit_,
                                                 TopKThresholdTrackerPtr threshold_tracker_)
    : ISimpleTransform(header_, header_, false), description(description_), limit(limit_), threshold_tracker(threshold_tracker_) {
...
void PartialSortingTransform::transform(Chunk& chunk) {
  ...
  if (read_rows) read_rows->add(chunk.getNumRows());
  ...
  if (!sort_description_threshold_columns.empty() && !threshold_tracker) {
    ...
    size_t result_size_hint =
        getFilterMask(block_columns, sort_description_threshold_columns, description, rows_num, filter, rows_to_compare, compare_results);

    /// Everything was filtered. Skip whole chunk.
    if (result_size_hint == 0) return;

    if (result_size_hint < rows_num) {
      for (auto& column : block) column.column = column.column->filter(filter, result_size_hint);
    }
  }
