...
/// Presumption: inputs return chunks with increasing bucket and block number, there is at most one chunk with the given bucket and block
/// number.
 ...
 public:
  ...
  Status prepare() override {
    ...
    for (size_t input_num = 0; input_num < num_inputs; ++input_num, ++in) {
      ...
      /// We want to keep not more than `num_inputs` chunks in memory (and there will be only a single chunk with the given (bucket_id,
      /// chunk_num)).
 ...
 private:
  bool tryPushChunk() {
    ...
    for (size_t input = 0; input < num_inputs; ++input)
      if (!is_input_finished[input] && last_chunk_id[input] < current_chunk_id) return false;
  ...
  void addChunk(Chunk chunk, size_t from_input) {
    ...
    if (!agg_info)
      throw Exception(ErrorCodes::LOGICAL_ERROR,
                      "Chunk should have AggregatedChunkInfo in SortingAggregatedForMemoryBoundMergingTransform.");
    ...
    if (is_overflows)
      ...
      if (chunks.contains(chunk_id)) {
        throw Exception(ErrorCodes::LOGICAL_ERROR, "SortingAggregatedForMemoryBoundMergingTransform already got bucket with number {}",
                        bucket_id);
      }
