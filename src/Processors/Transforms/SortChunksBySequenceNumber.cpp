...
struct ChunkSequenceNumber : public DB::ChunkInfoCloneable<ChunkSequenceNumber> {
  explicit ChunkSequenceNumber(UInt64 sequence_number_) : sequence_number(sequence_number_) {}
...
IProcessor::Status SortChunksBySequenceNumber::prepare() {
  ...
  if (!all_finished) throw Exception(ErrorCodes::LOGICAL_ERROR, "SortChunksBySequenceNumber has read bucket, but couldn't push it.");
...
bool SortChunksBySequenceNumber::tryPushChunk() {
  ...
  for (size_t i = 0; i < num_inputs; ++i) {
    if (is_input_finished[i] && chunk_snums[i] == -1) continue;

    if (chunk_snums[i] == -1) return false;
...
void SortChunksBySequenceNumber::addChunk(Chunk chunk, size_t input) {
  ...
  if (!info) throw Exception(ErrorCodes::LOGICAL_ERROR, "Chunk should have ChunkSequenceNumber in SortChunksBySequenceNumber.");
