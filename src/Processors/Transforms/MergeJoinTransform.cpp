...
FullMergeJoinCursor createCursor(const Block &block, const Names &columns, JoinStrictness strictness) {
  ...
  for (const auto &name : columns) indices.emplace_back(block.getPositionByName(name));
  return FullMergeJoinCursor(std::move(indices), strictness == JoinStrictness::Asof);
}

template <typename T>
int nullableCompareAt(const T &left_column, const NullMap *left_null_map, const T &right_column, const NullMap *right_null_map, size_t lpos,
                      size_t rpos, int null_direction_hint) {
  if (left_null_map && (*left_null_map)[lpos]) return null_direction_hint;

  if (right_null_map && (*right_null_map)[rpos]) return -null_direction_hint;
...
int ALWAYS_INLINE compareCursors(const TCursor &lhs, size_t lpos, const TCursor &rhs, size_t rpos, int null_direction_hint) {
  ...
  for (size_t i = 0; i < key_length; ++i) {
    ...
    if constexpr (TCursor::has_null_maps)
      cmp = nullableCompareAt(left_column, getNullMapData(lhs.null_maps[i]), right_column, getNullMapData(rhs.null_maps[i]), lpos, rpos,
                              null_direction_hint);
    else
...
int compareAsofCursors(const FullMergeJoinCursor &lhs, const FullMergeJoinCursor &rhs, int null_direction_hint) {
  return nullableCompareAt(*lhs.asof_column, getNullMapData(lhs.null_maps.back()), *rhs.asof_column, getNullMapData(rhs.null_maps.back()),
...
int ALWAYS_INLINE totallyCompare(FullMergeJoinCursor &lhs, FullMergeJoinCursor &rhs, int null_direction_hint) {
  if (totallyLess(lhs, rhs, null_direction_hint)) return -1;
  if (totallyLess(rhs, lhs, null_direction_hint)) return 1;
  return 0;
}
...
bool ALWAYS_INLINE sameNext(const FullMergeJoinCursor &impl) {
  if (impl.isLast()) return false;

  size_t pos = impl.getRow();
  for (size_t i = 0; i < impl.sort_columns.size(); ++i) {
    const auto *nm = getNullMapData(impl.null_maps[i]);
    if (nm && ((*nm)[pos] != (*nm)[pos + 1])) return false;

    if (nm && (*nm)[pos]) continue;

    const auto &col = *impl.sort_columns[i];
    if (auto cmp = col.compareAt(pos, pos + 1, col, 1); cmp != 0) return false;
  }
  return true;
}
...
size_t ALWAYS_INLINE nextDistinct(FullMergeJoinCursor &impl) {
  ...
  if (impl.isValid()) return impl.getRow() - start_pos;
...
void inline addRange(PaddedPODArray<UInt64> &values, UInt64 start, UInt64 end) {
  assert(end > start);
  for (UInt64 i = start; i < end; ++i) values.push_back(i);
}

void inline addMany(PaddedPODArray<UInt64> &values, UInt64 value, size_t num) { values.resize_fill(values.size() + num, value); }

}  // namespace

JoinKeyRow::JoinKeyRow(const FullMergeJoinCursor &cursor, size_t pos) {
  bool is_null =
      std::ranges::any_of(cursor.null_maps | std::views::transform(getNullMapData), [pos](const auto *nm) { return nm && (*nm)[pos]; });
  if (is_null) return;
...
void JoinKeyRow::reset() { row.clear(); }
...
bool JoinKeyRow::asofMatch(const FullMergeJoinCursor &cursor, ASOFJoinInequality asof_inequality) const {
  ...
  return (asof_inequality == ASOFJoinInequality::Less && cmp < 0) || (asof_inequality == ASOFJoinInequality::LessOrEquals && cmp <= 0) ||
...
void AnyJoinState::setValue(Chunk value_) { value = std::move(value_); }
...
FullMergeJoinCursor::FullMergeJoinCursor(std::vector<size_t> key_indices_, bool is_asof_)
    : key_indices(std::move(key_indices_)), is_asof(is_asof_) {
  if (key_indices.empty()) throw Exception(ErrorCodes::LOGICAL_ERROR, "Got empty sort description for FullMergeJoinCursor");
}

const Chunk &FullMergeJoinCursor::getCurrent() const { return current_chunk; }
...
bool FullMergeJoinCursor::fullyCompleted() const { return !isValid() && received_all_blocks; }
...
MergeJoinAlgorithm::MergeJoinAlgorithm(JoinKind kind_, JoinStrictness strictness_, const TableJoin::JoinOnClause &on_clause_,
                                       SharedHeaders &input_headers_, size_t max_block_size_)
    : input_headers(input_headers_),
      kind(kind_),
      strictness(strictness_),
      max_block_size(max_block_size_),
      log(getLogger("MergeJoinAlgorithm")) {
  if (input_headers.size() != 2) throw Exception(ErrorCodes::LOGICAL_ERROR, "MergeJoinAlgorithm requires exactly two inputs");
...
MergeJoinAlgorithm::MergeJoinAlgorithm(JoinPtr join_ptr, SharedHeaders &input_headers_, size_t max_block_size_)
    : MergeJoinAlgorithm(join_ptr->getTableJoin().kind(), join_ptr->getTableJoin().strictness(), join_ptr->getTableJoin().getOnlyClause(),
                         input_headers_, max_block_size_) {
  ...
  if (smj_ptr) null_direction_hint = smj_ptr->getNullDirection();

  if (strictness == JoinStrictness::Asof) setAsofInequality(join_ptr->getTableJoin().getAsofInequality());
}

void MergeJoinAlgorithm::setAsofInequality(ASOFJoinInequality asof_inequality_) {
  if (strictness != JoinStrictness::Asof) throw Exception(ErrorCodes::LOGICAL_ERROR, "setAsofInequality is only supported for ASOF joins");

  if (asof_inequality_ == ASOFJoinInequality::None) throw Exception(ErrorCodes::LOGICAL_ERROR, "ASOF inequality cannot be None");
...
void MergeJoinAlgorithm::logElapsed(double seconds) {
  LOG_TRACE(log,
            ...
            seconds, stat.num_blocks[0], stat.num_rows[0], stat.num_blocks[1], stat.num_rows[1], stat.max_blocks_loaded);
}
...
void MergeJoinAlgorithm::initialize(Inputs inputs) {
  if (inputs.size() != 2) throw Exception(ErrorCodes::LOGICAL_ERROR, "Two inputs are required, got {}", inputs.size());
...
void MergeJoinAlgorithm::consume(Input &input, size_t source_num) {
  if (input.skip_last_row) throw Exception(ErrorCodes::NOT_IMPLEMENTED, "skip_last_row is not supported");

  if (input.permutation) throw DB::Exception(ErrorCodes::NOT_IMPLEMENTED, "permutation is not supported");
  ...
  if (!input.chunk) cursors[source_num].setCompleted();
...
struct AllJoinImpl {
  ...
  static void join(FullMergeJoinCursor &left_cursor, FullMergeJoinCursor &right_cursor, size_t max_block_size,
                   PaddedPODArray<UInt64> &left_map, PaddedPODArray<UInt64> &right_map, std::unique_ptr<AllJoinState> &state,
    ...
    while (left_cursor.isValid() && right_cursor.isValid()) {
      ...
      if (cmp == 0) {
        ...
        if constexpr (isLeftOrFull(kind)) {
          ...
          for (size_t i = lpos; i < left_cursor.getRow(); ++i) left_map.push_back(i);
        }
      } else {
        ...
        if constexpr (isRightOrFull(kind)) {
          ...
          for (size_t i = rpos; i < right_cursor.getRow(); ++i) right_map.push_back(i);
        }
      }
    }
  }
...
void dispatchKind(JoinKind kind, Args &&...args) {
  if (Impl<JoinKind::Inner>::enabled && kind == JoinKind::Inner) return Impl<JoinKind::Inner>::join(std::forward<Args>(args)...);
  if (Impl<JoinKind::Left>::enabled && kind == JoinKind::Left) return Impl<JoinKind::Left>::join(std::forward<Args>(args)...);
  if (Impl<JoinKind::Right>::enabled && kind == JoinKind::Right) return Impl<JoinKind::Right>::join(std::forward<Args>(args)...);
  if (Impl<JoinKind::Full>::enabled && kind == JoinKind::Full) return Impl<JoinKind::Full>::join(std::forward<Args>(args)...);
...
MutableColumns MergeJoinAlgorithm::getEmptyResultColumns() const {
  ...
  for (size_t i = 0; i < 2; ++i) getEmptyResultColumns(result_cols, i);
  return result_cols;
}
...
Columns MergeJoinAlgorithm::getEmptyResultColumns(size_t pos) const {
  ...
  Columns result_cols;
  for (auto &&col : mutable_cols) result_cols.push_back(std::move(col));
  return result_cols;
}
...
std::optional<MergeJoinAlgorithm::Status> MergeJoinAlgorithm::handleAllJoinState() {
  ...
  if (all_join_state) {
    ...
    while (!max_block_size || total_rows < max_block_size) {
      ...
      size_t i = 0;
      /// Copy left block
      for (const auto &col : left_range.chunk.getColumns()) result_cols[i++]->insertRangeFrom(*col, left_range.begin, left_range.length);
      /// And replicate current right column
      for (const auto &col : right_range.chunk.getColumns()) result_cols[i++]->insertManyFrom(*col, right_range.current, left_range.length);
      bool valid = all_join_state->next();
      if (!valid) break;
    }

    if (total_rows) return Status(Chunk(std::move(result_cols), total_rows));
  }
  return {};
}

std::optional<MergeJoinAlgorithm::Status> MergeJoinAlgorithm::handleAsofJoinState() {
  if (strictness != JoinStrictness::Asof) return {};

  if (!cursors[1].fullyCompleted()) return {};
  ...
  while (left_cursor.isValid() && asof_join_state.hasMatch(left_cursor, asof_inequality)) {
    size_t i = 0;
    for (const auto &col : left_columns) result_cols[i++]->insertFrom(*col, left_cursor.getRow());
    for (const auto &col : asof_join_state.value.getColumns()) result_cols[i++]->insertFrom(*col, asof_join_state.value_row);
  ...
  while (isLeft(kind) && left_cursor.isValid()) {
    ...
    size_t i = 0;
    for (const auto &col : left_columns) result_cols[i++]->insertFrom(*col, left_cursor.getRow());
    for (; i < result_cols.size(); ++i) result_cols[i]->insertDefault();
    chassert(i == result_cols.size());
  ...
  if (result_rows) return Status(Chunk(std::move(result_cols), result_rows));
...
MergeJoinAlgorithm::Status MergeJoinAlgorithm::allJoin() {
  ...
  for (auto &col : lcols) result.addColumn(std::move(col));

  for (auto &col : rcols) result.addColumn(std::move(col));
...
struct AnyJoinImpl {
  ...
  static void join(FullMergeJoinCursor &left_cursor, FullMergeJoinCursor &right_cursor, PaddedPODArray<UInt64> &left_map,
                   PaddedPODArray<UInt64> &right_map, AnyJoinState &any_join_state, int null_direction_hint) {
    assert(enabled);
    ...
    if constexpr (isLeft(kind) || isInner(kind)) right_map.reserve(num_rows);

    if constexpr (isRight(kind) || isInner(kind)) left_map.reserve(num_rows);
    ...
    while (left_cursor.isValid() && right_cursor.isValid()) {
      ...
      if (cmp == 0) {
        ...
        size_t num = nextDistinct(left_cursor);
        if constexpr (isLeftOrFull(kind)) right_map.resize_fill(right_map.size() + num, right_cursor.rows);
      } else {
        size_t num = nextDistinct(right_cursor);
        if constexpr (isRightOrFull(kind)) left_map.resize_fill(left_map.size() + num, left_cursor.rows);
      }
    }
    ...
    if (!left_cursor.isValid()) {
      ...
      if (cmp == 0 && isLeft(kind)) any_join_state.setValue(getRowFromChunk(right_cursor.getCurrent(), rpos));
    }

    if (!right_cursor.isValid()) {
      any_join_state.set(1, right_cursor);
      if (cmp == 0 && isRight(kind)) any_join_state.setValue(getRowFromChunk(left_cursor.getCurrent(), lpos));
    }
  }
};

std::optional<MergeJoinAlgorithm::Status> MergeJoinAlgorithm::handleAnyJoinState() {
  if (any_join_state.empty()) return {};
  ...
  for (size_t source_num = 0; source_num < 2; ++source_num) {
    ...
    if (any_join_state.keys[source_num].equals(current)) {
      ...
      if (current.isValid()) any_join_state.keys[source_num].reset();
    } else {
      any_join_state.keys[source_num].reset();
    }
  }

  if (result) return Status(std::move(result));
  return {};
}

MergeJoinAlgorithm::Status MergeJoinAlgorithm::anyJoin() {
  if (auto result = handleAnyJoinState()) return std::move(*result);

  auto &current_left = cursors[0];
  if (!current_left.isValid()) return Status(0);

  auto &current_right = cursors[1];
  if (!current_right.isValid()) return Status(1);
...
MergeJoinAlgorithm::Status MergeJoinAlgorithm::asofJoin() {
  auto &left_cursor = cursors[0];
  if (!left_cursor.isValid()) return Status(0);

  auto &right_cursor = cursors[1];
  if (!right_cursor.isValid()) return Status(1);
  ...
  while (left_cursor.isValid() && right_cursor.isValid()) {
    ...
    if (cmp == 0) {
      ...
      if (lhs_null_map && (*lhs_null_map)[lpos]) cmp = -1;
      const auto *rhs_null_map = getNullMapData(right_cursor.null_maps.back());
      if (rhs_null_map && (*rhs_null_map)[rpos]) cmp = 1;
    }
    ...
    if (cmp == 0) {
      ...
      if ((asof_inequality == ASOFJoinInequality::Less && asof_cmp <= -1) ||
        ...
        size_t i = 0;
        for (const auto &col : left_columns) result_cols[i++]->insertFrom(*col, lpos);
        for (const auto &col : right_columns) result_cols[i++]->insertFrom(*col, rpos);
        chassert(i == result_cols.size());
    ...
    if (cmp < 0) {
      ...
      if (isLeft(kind) && num) {
        ...
        size_t i = 0;
        for (const auto &col : left_columns) result_cols[i++]->insertRangeFrom(*col, lpos, num);
        for (; i < result_cols.size(); ++i) result_cols[i]->insertManyDefaults(num);
        chassert(i == result_cols.size());
      }
    } else {
...
IMergingAlgorithm::Status MergeJoinAlgorithm::merge() {
  if (!cursors[0].isValid() && !cursors[0].fullyCompleted()) return Status(0);

  if (!cursors[1].isValid() && !cursors[1].fullyCompleted()) return Status(1);

  if (auto result = handleAllJoinState()) return std::move(*result);

  if (auto result = handleAsofJoinState()) return std::move(*result);

  if (cursors[0].fullyCompleted() || cursors[1].fullyCompleted()) {
    if (!cursors[0].fullyCompleted() && isLeftOrFull(kind)) return Status(createBlockWithDefaults(0));

    if (!cursors[1].fullyCompleted() && isRightOrFull(kind)) return Status(createBlockWithDefaults(1));
  ...
  if (int cmp = totallyCompare(cursors[0], cursors[1], null_direction_hint); cmp != 0 && strictness != JoinStrictness::Asof) {
    if (cmp < 0) {
      if (isLeftOrFull(kind)) return Status(createBlockWithDefaults(0));
      cursors[0].setChunk(Chunk());
      return Status(0);
    }

    if (cmp > 0) {
      if (isRightOrFull(kind)) return Status(createBlockWithDefaults(1));
      cursors[1].setChunk(Chunk());
      return Status(1);
    }
  }

  if (strictness == JoinStrictness::Any) return anyJoin();

  if (strictness == JoinStrictness::All) return allJoin();

  if (strictness == JoinStrictness::Asof) return asofJoin();

  throw Exception(ErrorCodes::NOT_IMPLEMENTED, "Unsupported strictness '{}'", strictness);
}

MergeJoinTransform::MergeJoinTransform(JoinPtr table_join, SharedHeaders &input_headers, SharedHeader output_header, size_t max_block_size,
                                       UInt64 limit_hint_)
    : IMergingTransform<MergeJoinAlgorithm>(input_headers, output_header,
                                            /* have_all_inputs_= */ true, limit_hint_,
                                            /* always_read_till_end_= */ false,
                                            /* empty_chunk_on_finish_= */ true, table_join, input_headers, max_block_size) {}

MergeJoinTransform::MergeJoinTransform(JoinKind kind_, JoinStrictness strictness_, const TableJoin::JoinOnClause &on_clause_,
                                       SharedHeaders &input_headers, SharedHeader output_header, size_t max_block_size, UInt64 limit_hint_)
    : IMergingTransform<MergeJoinAlgorithm>(input_headers, output_header,
                                            /* have_all_inputs_= */ true, limit_hint_,
                                            /* always_read_till_end_= */ false,
                                            /* empty_chunk_on_finish_= */ true, kind_, strictness_, on_clause_, input_headers,
                                            max_block_size) {}

void MergeJoinTransform::onFinish() { algorithm.logElapsed(static_cast<double>(merging_elapsed_ns) / 1000000000ULL); }
