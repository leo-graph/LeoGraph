...
struct AggregatingTransformParams {
  ...
  AggregatingTransformParams(const Block& header, const Aggregator::Params& params_, const AggregatorListPtr& aggregator_list_ptr_,
                             bool final_)
      : params(params_),
...
struct ManyAggregatedData {
  ...
  explicit ManyAggregatedData(size_t num_threads = 0) : variants(num_threads) {
    for (auto& elem : variants) elem = std::make_shared<AggregatedDataVariants>();
  }
  ...
  ~ManyAggregatedData() {
    try {
      ...
      // Without the following code, they would be destroyed in the destructor of AggregatedDataVariants in the current thread (i.e.
      // sequentially).
      const auto pool =
          std::make_unique<ThreadPool>(CurrentMetrics::DestroyAggregatesThreads, CurrentMetrics::DestroyAggregatesThreadsActive,
                                       CurrentMetrics::DestroyAggregatesThreadsScheduled, variants.size());
      ...
      for (auto&& variant : variants) {
        ...
        if (variant->aggregator) {
          pool->scheduleOrThrowOnError([my_variant = std::move(variant), thread_group = CurrentThread::getGroup()]() mutable {
 ...
 * If final = false, the aggregate functions are not finalized, that is, they are not replaced by their value, but contain an intermediate
 * state of calculations. This is necessary so that aggregation can continue (for example, by combining streams of partially aggregated
 * data).
 *
...
class AggregatingTransform final : public IProcessor {
 public:
  ...
  AggregatingTransform(SharedHeader header, AggregatingTransformParamsPtr params_, ManyAggregatedDataPtr many_data, size_t current_variant,
                       size_t max_threads, size_t temporary_data_merge_threads,
                       bool should_produce_results_in_order_of_bucket_number_ = true, bool skip_merging_ = false,
