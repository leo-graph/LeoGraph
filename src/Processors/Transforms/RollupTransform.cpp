...
GroupByModifierTransform::GroupByModifierTransform(SharedHeader header, AggregatingTransformParamsPtr params_, bool use_nulls_)
    : IAccumulatingTransform(std::move(header),
                             std::make_shared<const Block>(generateOutputHeader(params_->getHeader(), params_->params.keys, use_nulls_))),
      params(std::move(params_)),
      use_nulls(use_nulls_) {
  keys.reserve(params->params.keys_size);
  for (const auto& key : params->params.keys) keys.emplace_back(input.getHeader().getPositionByName(key));
...
void GroupByModifierTransform::consume(Chunk chunk) { consumed_chunks.emplace_back(std::move(chunk)); }
...
void GroupByModifierTransform::mergeConsumed() {
  ...
  if (use_nulls) {
    for (auto key : keys) columns[key] = makeNullableSafe(columns[key]);
  }
...
Chunk GroupByModifierTransform::merge(Chunks&& chunks, bool is_input, bool final) {
  ...
  BlocksList blocks;
  for (auto& chunk : chunks) blocks.emplace_back(header.cloneWithColumns(chunk.detachColumns()));
...
Chunk RollupTransform::generate() {
  ...
  if (!gen_chunk.empty()) gen_chunk.addColumn(0, ColumnUInt64::create(gen_chunk.getNumRows(), set_counter++));
  return gen_chunk;
}
