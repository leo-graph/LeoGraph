...
CheckConstraintsTransform::CheckConstraintsTransform(const StorageID &table_id_, SharedHeader header,
...
void CheckConstraintsTransform::onConsume(Chunk chunk) {
  if (chunk.getNumRows() > 0) {
    if (rows_written == 0)
      for (const auto &expression : expressions) VirtualColumnUtils::buildSetsForDAG(expression->getActionsDAG(), context);
    ...
    for (size_t i = 0; i < expressions.size(); ++i) {
      ...
      if (const auto *column_nullable = checkAndGetColumn<ColumnNullable>(&*result_column)) {
        ...
        if (null_map_contains_null)
          throw Exception(ErrorCodes::VIOLATED_CONSTRAINT,
                          ...
                          backQuote(constraint_ptr->name), table_id.getNameForLogs(), constraint_ptr->expr->formatForErrorMessage());
      ...
      if (!memoryIsByte(res_data, 0, size, 1)) {
        ...
        for (const auto &name : related_columns) {
          ...
          if (!first) column_values_msg.append(", ");
          ...
          size_t value_max_bytes =
              UTF8::computeBytesBeforeWidth(reinterpret_cast<const UInt8 *>(value.data()), value.size(), 0, max_value_length);
          if (value_max_bytes < value.size()) {
            value.resize(value_max_bytes);
            value.append("…");
            /// Cosmetics.
            if (value.starts_with("'")) value.append("'");
          }
        ...
        throw Exception(ErrorCodes::VIOLATED_CONSTRAINT,
                        ...
                        constraint_ptr->expr->formatForErrorMessage(), column_values_msg);
      }
    }
  }
