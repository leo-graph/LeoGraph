...
void CheckSortedTransform::transform(Chunk& chunk) {
    ...
    for (const auto& elem : sort_description_map) {
      ...
      if (res > 0) {
        throw Exception(ErrorCodes::LOGICAL_ERROR,
                        "Sort order of blocks violated for column number {}, left: {}, right: {}. Chunk {}, rows read {}.{}", column_number,
                        applyVisitor(FieldVisitorDump(), (*left_col)[left_index]),
                        applyVisitor(FieldVisitorDump(), (*right_col)[right_index]), chunk_num, rows_read,
  ...
  if (!last_row.empty()) check(last_row, 0, chunk_columns, 0);
