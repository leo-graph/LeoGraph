...
BuildRuntimeFilterTransform::BuildRuntimeFilterTransform(SharedHeader header_, String filter_column_name_,
                                                         const DataTypePtr& filter_column_type_, String filter_name_,
                                                         size_t filters_to_merge_, UInt64 exact_values_limit_, UInt64 bloom_filter_bytes_,
                                                         UInt64 bloom_filter_hash_functions_, Float64 pass_ratio_threshold_for_disabling_,
  ...
  if (allow_to_use_not_exact_filter_) {
    if (ApproximateRuntimeFilter::isDataTypeSupported(filter_column_target_type)) {
      built_filter = std::make_unique<ApproximateRuntimeFilter>(
          filters_to_merge_, filter_column_target_type, pass_ratio_threshold_for_disabling_, blocks_to_skip_before_reenabling_,
          bloom_filter_bytes_, exact_values_limit_, bloom_filter_hash_functions_, max_ratio_of_set_bits_in_bloom_filter_);
    } else {
      built_filter =
          std::make_unique<ExactContainsRuntimeFilter>(filters_to_merge_, filter_column_target_type, pass_ratio_threshold_for_disabling_,
                                                       blocks_to_skip_before_reenabling_, bloom_filter_bytes_, exact_values_limit_);
    }
  } else {
    built_filter =
        std::make_unique<ExactNotContainsRuntimeFilter>(filters_to_merge_, filter_column_target_type, pass_ratio_threshold_for_disabling_,
                                                        blocks_to_skip_before_reenabling_, bloom_filter_bytes_, exact_values_limit_);
  }
}
...
IProcessor::Status BuildRuntimeFilterTransform::prepare() {
  ...
  if (status == IProcessor::Status::Finished) finish();
...
void BuildRuntimeFilterTransform::transform(Chunk& chunk) {
  ...
  if (cast_to_target_type) {
    filter_column = cast_to_target_type->execute({ColumnWithTypeAndName(filter_column, filter_column_original_type, "")},
                                                 filter_column_target_type, filter_column->size(), false);
  }
