...
class TotalsHavingTransform : public ISimpleTransform {
 public:
  TotalsHavingTransform(const Block& header, const ColumnsMask& aggregates_mask_, bool overflow_row_,
                        const ExpressionActionsPtr& expression_, const std::string& filter_column_, bool remove_filter_,
                        TotalsMode totals_mode_, double auto_include_threshold_, bool final_);
  ...
  static Block transformHeader(Block block, const ActionsDAG* expression, const std::string& filter_column_name, bool remove_filter,
                               bool final, const ColumnsMask& aggregates_mask);
