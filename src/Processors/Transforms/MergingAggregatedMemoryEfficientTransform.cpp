...
bool GroupingAggregatedTransform::tryPushTwoLevelData() {
  auto try_push_by_iter = [&](auto batch_it) {
    if (batch_it == chunks_map.end()) return false;
  ...
  if (all_inputs_finished) {
    ...
    while (!chunks_map.empty())
      if (try_push_by_iter(chunks_map.begin())) return true;
  } else {
    ...
    for (; next_bucket_to_push < current_bucket; ++next_bucket_to_push) {
      ...
      if (out_of_order_buckets.contains(next_bucket_to_push)) continue;

      if (try_push_by_iter(chunks_map.find(next_bucket_to_push))) return true;
    }
  }
...
bool GroupingAggregatedTransform::tryPushSingleLevelData() {
  if (single_level_chunks.empty()) return false;
...
bool GroupingAggregatedTransform::tryPushOverflowData() {
  if (overflow_chunks.empty()) return false;
...
IProcessor::Status GroupingAggregatedTransform::prepare(const PortNumbers &updated_input_ports, const PortNumbers &) {
  ...
  if (!initialized_index_to_input) {
    ...
    for (size_t i = 0; i < num_inputs; ++i, ++in) index_to_input[i] = in;
  }
  ...
  if (!wait_input_ports_numbers.empty()) {
    for (const auto &updated_input_port_number : updated_input_ports) {
      if (!wait_input_ports_numbers.contains(updated_input_port_number)) continue;
      ...
      if (!input->isFinished() && need_input(updated_input_port_number)) continue;

      wait_input_ports_numbers.erase(updated_input_port_number);
    }

    if (!wait_input_ports_numbers.empty()) return Status::NeedData;
  }
  ...
  /// Output if has data.
  if (has_two_level) pushed_to_output = tryPushTwoLevelData();
  ...
  for (;; ++current_bucket) {
    ...
    if (out_of_order_buckets.contains(current_bucket)) continue;
  ...
  if (has_two_level) {
    if (tryPushTwoLevelData()) return Status::PortFull;
...
void GroupingAggregatedTransform::addChunk(Chunk chunk, size_t input) {
  ...
  if (auto agg_info = chunk.getChunkInfos().get<AggregatedChunkInfo>()) {
    ...
    if (is_overflows)
      ...
      for (const auto ooo_bucket : input_out_of_order_buckets[input]) out_of_order_buckets[ooo_bucket]--;
...
void MergingAggregatedBucketTransform::transform(Chunk &chunk) {
  ...
  if (!required_sort_description.empty()) sortBlock(block, required_sort_description);
...
bool SortingAggregatedTransform::tryPushChunk() {
  ...
  for (size_t input = 0; input < num_inputs; ++input)
    if (!is_input_finished[input] && last_bucket_number[input] < cur_bucket) return false;
...
void SortingAggregatedTransform::addChunk(Chunk chunk, size_t from_input) {
  ...
  if (!agg_info) throw Exception(ErrorCodes::LOGICAL_ERROR, "Chunk should have AggregatedChunkInfo in SortingAggregatedTransform.");
  ...
  if (is_overflows)
    ...
    if (chunks[bucket]) {
      throw Exception(ErrorCodes::LOGICAL_ERROR, "SortingAggregatedTransform already got bucket with number {}", bucket);
    }
...
IProcessor::Status SortingAggregatedTransform::prepare() {
  ...
  if (!all_finished) throw Exception(ErrorCodes::LOGICAL_ERROR, "SortingAggregatedTransform has read bucket, but couldn't push it.");
...
void addMergingAggregatedMemoryEfficientTransform(Pipe &pipe, AggregatingTransformParamsPtr params, size_t num_merging_processors,
  ...
  pipe.addSimpleTransform([params](const SharedHeader &) { return std::make_shared<MergingAggregatedBucketTransform>(params); });
