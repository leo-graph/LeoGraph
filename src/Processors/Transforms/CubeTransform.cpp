...
CubeTransform::CubeTransform(SharedHeader header, AggregatingTransformParamsPtr params_, bool use_nulls_)
  ...
  if (keys.size() >= 8 * sizeof(mask))
    throw Exception(ErrorCodes::TOO_MANY_COLUMNS, "Too many keys ({}) are used for CubeTransform, the maximum is {}.", keys.size(),
                    8 * sizeof(mask) - 1);
}
...
Chunk CubeTransform::generate() {
  if (!consumed_chunks.empty()) {
    ...
    for (auto key : keys) current_zero_columns.emplace_back(getColumnWithDefaults(key, num_rows));
  }
  ...
  if (mask) {
    ...
    for (size_t i = 0; i < size; ++i)
      ...
      if ((mask & (UInt64(1) << (size - i - 1))) == 0) columns[keys[i]] = current_zero_columns[i];
  ...
  if (!gen_chunk.empty()) gen_chunk.addColumn(0, ColumnUInt64::create(gen_chunk.getNumRows(), grouping_set++));
  return gen_chunk;
}
