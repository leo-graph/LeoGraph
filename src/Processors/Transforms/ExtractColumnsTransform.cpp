...
ExtractColumnsTransform::ExtractColumnsTransform(SharedHeader header_, const NamesAndTypesList& requested_columns_)
    : ISimpleTransform(header_, std::make_shared<const Block>(transformHeader(*header_, requested_columns_)), false),
...
void ExtractColumnsTransform::transform(Chunk& chunk) {
  ...
  for (const auto& required_column : requested_columns) columns.emplace_back(getColumnFromBlock(block, required_column));
