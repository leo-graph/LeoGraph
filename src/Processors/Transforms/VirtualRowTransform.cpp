...
VirtualRowTransform::Status VirtualRowTransform::prepare() {
  ...
  if (can_generate) return Status::Ready;
...
void VirtualRowTransform::work() {
  if (can_generate) {
    if (generated) throw Exception(ErrorCodes::LOGICAL_ERROR, "VirtualRowTransform cannot consume chunk because it already was generated");

    generated = true;
    can_generate = false;

    if (!is_first) {
      if (current_chunk.empty()) throw Exception(ErrorCodes::LOGICAL_ERROR, "Can't generate chunk in VirtualRowTransform");
      return;
    }
    ...
    if (!has_input) throw Exception(ErrorCodes::LOGICAL_ERROR, "VirtualRowTransform cannot consume chunk because it wasn't read");
