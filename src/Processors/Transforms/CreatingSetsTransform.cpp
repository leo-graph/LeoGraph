...
CreatingSetsTransform::CreatingSetsTransform(SharedHeader in_header_, SharedHeader out_header_, SetAndKeyPtr set_and_key_,
  ...
  if (prepared_sets_cache && !set_and_key->external_table) {
    ...
    while (true) {
      ...
      /// Note, that findOrPromiseToBuild() can also call shared_future::get()
        ...
        throw Exception::createRuntime(ErrorCodes::UNKNOWN_EXCEPTION,
                                       getExceptionMessage(std::current_exception(), /* with_stacktrace= */ false));
      }
    }
  }

  if (set_and_key->set && !set_from_cache) LOG_TRACE(log, "Creating set, key: {}", set_and_key->key);
  if (set_and_key->external_table) LOG_TRACE(log, "Filling temporary table.");

  if (set_and_key->external_table)
    /// TODO: make via port
    table_out = QueryPipeline(
        set_and_key->external_table->write({}, set_and_key->external_table->getInMemoryMetadataPtr(), nullptr, /*async_insert=*/false));

  done_with_set = !set_and_key->set || set_from_cache;
  done_with_table = !set_and_key->external_table;

  if ((done_with_set && !set_from_cache) && done_with_table) throw Exception(ErrorCodes::LOGICAL_ERROR, "Nothing to do with subquery");
...
void CreatingSetsTransform::finishSubquery() {
  ...
  if (set_from_cache) {
    ...
    if (set_and_key->external_table) LOG_DEBUG(log, "Created Table with {} rows in {} sec.", read_rows, seconds);
  } else {
...
void CreatingSetsTransform::consume(Chunk chunk) {
  ...
  if (!done_with_set) {
    if (!set_and_key->set->insertFromBlock(block.getColumnsWithTypeAndName())) done_with_set = true;
  }
  ...
  if (done_with_set && done_with_table) finishConsume();
}
