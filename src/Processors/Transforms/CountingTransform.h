...
class CountingTransform final : public ExceptionKeepingTransform {
 public:
  explicit CountingTransform(SharedHeader header, std::shared_ptr<const EnabledQuota> quota_ = nullptr)
      : ExceptionKeepingTransform(header, header), quota(std::move(quota_)) {}
  ...
  void setProcessListElement(QueryStatusPtr elem) { process_elem = elem; }
