...
bool FilterTransform::canUseType(const DataTypePtr& filter_type) { return filter_type->canBeUsedInBooleanContext(); }
...
auto incrementProfileEvents = [](size_t num_rows, const Columns& columns) {
  ...
  for (const auto& column : columns) {
    if (column) num_bytes += column->byteSize();
  }
  ProfileEvents::increment(ProfileEvents::FilterTransformPassedBytes, num_bytes);
};

Block FilterTransform::transformHeader(const Block& header, const ActionsDAG* expression, const String& filter_column_name,
  ...
  if (!canUseType(filter_type))
    throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_COLUMN_FOR_FILTER,
                    "Illegal type {} of column {} for filter. Must be UInt8 or Nullable(UInt8).", filter_type->getName(),
                    filter_column_name);

  if (remove_filter_column) result.erase(filter_column_name);

  return result;
}

FilterTransform::FilterTransform(SharedHeader header_, ExpressionActionsPtr expression_, String filter_column_name_,
                                 bool remove_filter_column_, bool on_totals_, std::shared_ptr<std::atomic<size_t>> rows_filtered_,
                                 std::optional<std::pair<UInt64, String>> condition_)
    : ISimpleTransform(header_,
                       std::make_shared<const Block>(transformHeader(*header_, expression_ ? &expression_->getActionsDAG() : nullptr,
                                                                     filter_column_name_, remove_filter_column_)),
                       true),
  ...
  if (expression) {
    ...
    /// Special check to stop queries like "WHERE ignore(...)"
    {
      ...
      while (node->type == ActionsDAG::ActionType::ALIAS) node = node->children[0];

      if (node->type == ActionsDAG::ActionType::FUNCTION && node->function_base->getName() == "ignore") always_false = true;
    }
  }
  ...
  if (column) always_false = always_false || ConstantFilterDescription(*column).always_false;

  if (condition.has_value()) query_condition_cache = Context::getGlobalContextInstance()->getQueryConditionCache();
}

IProcessor::Status FilterTransform::prepare() {
  if (!on_totals && (always_false
  ...
  if (status != IProcessor::Status::PortFull) are_prepared_sets_initialized = true;

  if (status == IProcessor::Status::Finished) writeIntoQueryConditionCache({});

  return status;
}

void FilterTransform::removeFilterIfNeed(Columns& columns) const {
  if (remove_filter_column) columns.erase(columns.begin() + filter_column_position);
}

void FilterTransform::transform(Chunk& chunk) {
  auto chunk_rows_before = chunk.getNumRows();
  doTransform(chunk);
  if (rows_filtered) *rows_filtered += chunk_rows_before - chunk.getNumRows();
}
...
void FilterTransform::doTransform(Chunk& chunk) {
...

  {
    ...
    if (expression) expression->execute(block, num_rows_before_filtration);
  ...
  if (isColumnConst(*filter_column)) filter_column = filter_column->convertToFullColumnIfConst();
  ...
  for (size_t i = 0; i < num_columns; ++i) {
    ...
    if (i == filter_column_position && remove_filter_column) continue;

    if (i == first_non_constant_column) continue;
...
void FilterTransform::writeIntoQueryConditionCache(const MarkRangesInfoPtr& mark_ranges_info) {
  if (!query_condition_cache) return;

  if (!mark_ranges_info) {
    /// FilterTransform has finished, we need to flush to the query result cache.

    if (!buffered_mark_ranges_info) return;

    query_condition_cache->write(buffered_mark_ranges_info->table_uuid, buffered_mark_ranges_info->part_name, condition->first,
                                 condition->second, buffered_mark_ranges_info->mark_ranges, buffered_mark_ranges_info->marks_count,
