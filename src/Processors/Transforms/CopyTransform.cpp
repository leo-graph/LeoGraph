...
CopyTransform::CopyTransform(SharedHeader header, size_t num_outputs)
    : IProcessor(InputPorts(1, header), OutputPorts(num_outputs, header)), was_output_processed(num_outputs, 0) {}
...
IProcessor::Status CopyTransform::prepareConsume() {
  ...
  for (auto& was_processed : was_output_processed) was_processed = false;
