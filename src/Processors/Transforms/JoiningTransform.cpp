...
JoiningTransform::JoiningTransform(SharedHeader input_header, SharedHeader output_header, JoinPtr join_, size_t max_block_size_,
                                   bool on_totals_, bool default_totals_, FinishCounterPtr finish_counter_)
  ...
  if (!join->isFilled()) inputs.emplace_back(Block(), this);  // Wait for FillingRightJoinSideTransform
}
...
IProcessor::Status JoiningTransform::prepare() {
  ...
  if (inputs.size() > 1) {
    ...
    if (last_in.isFinished() && join->alwaysReturnsEmptySet() && !on_totals) stop_reading = true;
  }
  ...
  if (!output.canPush()) {
    for (auto &input : inputs) input.setNotNeeded();
  ...
  if (inputs.size() > 1) {
    ...
    if (!last_in.isFinished()) {
      last_in.setNeeded();
      if (last_in.hasData()) throw Exception(ErrorCodes::LOGICAL_ERROR, "No data is expected from second JoiningTransform port");

      return Status::NeedData;
    }
  }

  if (has_input) return Status::Ready;

  auto &input = inputs.front();
  if (input.isFinished()) {
    if (process_non_joined) return Status::Ready;
...
void JoiningTransform::transform(Chunk &chunk) {
  ...
  if (on_totals) {
    ...
    if (default_totals && right_totals.empty()) return;
...
Block JoiningTransform::readExecute(Chunk &chunk) {
  ...
  if (data.is_last) join_result.reset();
...
InputPort *FillingRightJoinSideTransform::addTotalsPort() {
  if (inputs.size() > 1) throw Exception(ErrorCodes::LOGICAL_ERROR, "Totals port was already added to FillingRightJoinSideTransform");
...
IProcessor::Status FillingRightJoinSideTransform::prepare() {
  ...
  if (inputs.size() > 1) {
    ...
    if (!totals_input.isFinished()) {
      ...
      if (!totals_input.hasData()) return Status::NeedData;
  ...
  if (finish_counter->isLast()) join->onBuildPhaseFinish();
...
void FillingRightJoinSideTransform::work() {
  ...
  if (input.isFinished() && !join->supportParallelJoin()) join->tryRerangeRightTableData();
...
DelayedJoinedBlocksWorkerTransform::DelayedJoinedBlocksWorkerTransform(SharedHeader output_header_,
                                                                       NonJoinedStreamBuilder non_joined_stream_builder_)
    : IProcessor(InputPorts{Block()}, OutputPorts{output_header_}), non_joined_stream_builder(std::move(non_joined_stream_builder_)) {}
...
IProcessor::Status DelayedJoinedBlocksWorkerTransform::prepare() {
  ...
  if (!task) {
    ...
    if (!task) throw Exception(ErrorCodes::LOGICAL_ERROR, "DelayedJoinedBlocksWorkerTransform must have chunk info");
  } else {
    input.setNotNeeded();
  }
...
void DelayedJoinedBlocksWorkerTransform::work() {
  ...
  if (!task->delayed_blocks->isFinished()) {
    ...
    if (block.empty()) block = nextNonJoinedBlock();
  } else {
...
DelayedJoinedBlocksTransform::DelayedJoinedBlocksTransform(size_t num_streams, JoinPtr join_)
    : IProcessor(InputPorts{}, OutputPorts(num_streams, Block())), join(std::move(join_)) {}
...
NonJoinedBlocksTransform::NonJoinedBlocksTransform(SharedHeader output_header, JoinPtr join_, Block left_sample_block_,
                                                   UInt64 max_block_size_, size_t stream_index_, size_t num_streams_)
