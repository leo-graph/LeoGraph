...
class SelectByIndicesTransform : public ISimpleTransform {
 public:
  explicit SelectByIndicesTransform(SharedHeader header) : ISimpleTransform(header, header, true) {}
  ...
  void transform(Chunk& chunk) override {
    ...
    if (select_all_rows_info) return;
    ...
    if (index_column->size() != num_rows) {
      ...
      for (auto& column : columns) column = column->index(*index_column, 0);
