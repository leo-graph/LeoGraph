...
RestoreChunkInfosTransform::RestoreChunkInfosTransform(Chunk::ChunkInfoCollection chunk_infos_, SharedHeader header_)
    : ISimpleTransform(header_, header_, true), chunk_infos(std::move(chunk_infos_)) {}
...
void RestoreChunkInfosTransform::transform(Chunk& chunk) {
  ...
  LOG_TEST(getLogger("RestoreChunks"), "Restoring chunk infos, result: {}", chunk.getChunkInfos().debug());
}
...
UpdateDeduplicationInfoWithViewIDTransform::UpdateDeduplicationInfoWithViewIDTransform(StorageIDMaybeEmpty view_id_, SharedHeader header_)
    : ISimpleTransform(header_, header_, true), view_id(std::move(view_id_)) {}
...
SelectPartitionTransform::SelectPartitionTransform(std::string partition_id_, StorageMetadataPtr metadata_snapshot_, ContextPtr contex_,
...
void SelectPartitionTransform::transform(Chunk& chunk) {
  ...
  LOG_DEBUG(getLogger("Deduplication::SelectPartitionTransform"),
            "Selecting partition '{}' with {} rows from chunk with rows {}, total partitions in chunk: {}, partitions: {}", partition_id,
            result_chunk.getNumRows(), rows_in_source_chunk, all_partitions.size(), fmt::join(all_partitions, ","));
...
AddDeduplicationInfoTransform::AddDeduplicationInfoTransform(SharedHeader header_) : ISimpleTransform(header_, header_, true) {}

AddDeduplicationInfoTransform::AddDeduplicationInfoTransform(InsertDependenciesBuilderConstPtr insert_dependencies_,
                                                             StorageIDMaybeEmpty root_view_id_, std::string user_token_,
                                                             InsertDeduplicationVersions unification_stage_, SharedHeader header_)
    : ISimpleTransform(header_, header_, true),
