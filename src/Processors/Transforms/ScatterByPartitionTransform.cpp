...
IProcessor::Status ScatterByPartitionTransform::prepare() {
  ...
  if (!all_outputs_processed) {
    ...
    for (size_t i = 0; i < output_size; ++i, ++output_it)
      if (!was_output_processed[i] && output_it->canPush()) can_push = true;
    if (!can_push) return Status::PortFull;
    return Status::Ready;
  }
...
void ScatterByPartitionTransform::work() {
  if (all_outputs_processed) generateOutputChunks();
...
void ScatterByPartitionTransform::generateOutputChunks() {
  ...
  if (key_columns.empty()) {
    ...
    if (output_size > 0) output_chunks[0] = Chunk(columns, num_rows);
    /// All other buckets are empty
    if (output_size > 1) {
      Chunk empty_chunk(chunk.cloneEmptyColumns(), 0);
      for (size_t i = 1; i < output_size; ++i) output_chunks[i] = Chunk(empty_chunk.getColumns(), 0);
    }
  ...
  for (const auto& column_number : key_columns) hash.update(columns[column_number]->getWeakHash32());
  ...
  for (const auto& column : columns) {
    ...
    for (size_t i = 0; i < output_size; ++i) output_chunks[i].addColumn(std::move(filtered_columns[i]));
  }
}
