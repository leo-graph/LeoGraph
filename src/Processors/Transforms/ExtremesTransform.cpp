...
ExtremesTransform::ExtremesTransform(SharedHeader header) : ISimpleTransform(header, header, true) {
  /// Port for Extremes.
...
IProcessor::Status ExtremesTransform::prepare() {
  ...
  if (!extremes && !extremes_columns.empty()) return Status::Ready;

  if (extremes) totals_output.push(std::move(extremes));
...
void ExtremesTransform::work() {
  if (finished_transform) {
    if (!extremes && !extremes_columns.empty()) extremes.setColumns(std::move(extremes_columns), 2);
  } else
...
void ExtremesTransform::transform(DB::Chunk& chunk) {
  ...
  if (extremes_columns.empty()) {
    ...
    for (size_t i = 0; i < num_columns; ++i) {
      if (isColumnConst(*extremes_columns[i])) continue;
      ...
      if (isNullORNaN(min_value) || (!isNullORNaN(cur_min_value) && cur_min_value < min_value)) min_value = cur_min_value;
      if (isNullORNaN(max_value) || (!isNullORNaN(cur_max_value) && cur_max_value > max_value)) max_value = cur_max_value;
