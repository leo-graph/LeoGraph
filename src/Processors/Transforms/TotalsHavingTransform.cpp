...
Block TotalsHavingTransform::transformHeader(Block block, const ActionsDAG *expression, const std::string &filter_column_name,
                                             bool remove_filter, bool final, const ColumnsMask &aggregates_mask) {
  if (final) finalizeBlock(block, aggregates_mask);

  if (expression) {
    block = expression->updateHeader(block);
    if (remove_filter) block.erase(filter_column_name);
  }

  return block;
}

TotalsHavingTransform::TotalsHavingTransform(const Block &header, const ColumnsMask &aggregates_mask_, bool overflow_row_,
                                             const ExpressionActionsPtr &expression_, const std::string &filter_column_,
                                             bool remove_filter_, TotalsMode totals_mode_, double auto_include_threshold_, bool final_)
    : ISimpleTransform(header,
                       transformHeader(header, expression_ ? &expression_->getActionsDAG() : nullptr, filter_column_, remove_filter_,
                                       final_, aggregates_mask_),
                       true),
  ...
  if (expression) {
    ...
    if (remove_filter) totals_header.erase(filter_column_name);
    outputs.emplace_back(totals_header, this);
  } else {
    if (!filter_column_name.empty()) filter_column_pos = finalized_header.getPositionByName(filter_column_name);
...
IProcessor::Status TotalsHavingTransform::prepare() {
  ...
  /// Check can output.
  if (totals_output.isFinished()) return Status::Finished;

  if (!totals_output.canPush()) return Status::PortFull;

  if (!total_prepared) return Status::Ready;
...
void TotalsHavingTransform::transform(Chunk &chunk) {
  ...
  if (overflow_row) {
    ...
    if (!agg_info) throw Exception(ErrorCodes::LOGICAL_ERROR, "Chunk should have AggregatedChunkInfo in TotalsHavingTransform.");
  ...
  if (final) finalizeChunk(finalized, aggregates_mask);
  ...
  if (filter_column_name.empty()) {
    ...
    if (remove_filter) finalized_block.erase(filter_column_name);
    ...
    if (const_filter_description.always_false) {
      if (totals_mode == TotalsMode::BEFORE_HAVING) addToTotals(chunk, nullptr);
...
void TotalsHavingTransform::addToTotals(const Chunk &chunk, const IColumn::Filter *filter) {
  ...
  for (size_t col = 0; col < num_columns; ++col) {
    ...
    if (const auto *column = typeid_cast<const ColumnAggregateFunction *>(current.get())) {
      ...
      if (filter && filter->size() != size) throw Exception(ErrorCodes::LOGICAL_ERROR, "Filter has size which differs from column size");

      if (filter) {
        for (size_t row = 0; row < size; ++row)
          if ((*filter)[row]) totals_column.insertMergeFrom(vec[row]);
      } else {
        for (size_t row = 0; row < size; ++row) totals_column.insertMergeFrom(vec[row]);
      }
    }
  }
}
...
void TotalsHavingTransform::prepareTotals() {
  ...
  if (overflow_aggregates) {
    if (totals_mode == TotalsMode::BEFORE_HAVING || totals_mode == TotalsMode::AFTER_HAVING_INCLUSIVE ||
