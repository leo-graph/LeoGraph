...
void CountingTransform::onConsume(Chunk chunk) {
  ...
  if (quota) quota->used(QuotaType::WRITTEN_BYTES, written_bytes);
  ...
  if (process_elem) process_elem->updateProgressOut(local_progress);

  if (progress_callback) progress_callback(local_progress);
