...
class ColumnGathererStream final : public IMergingAlgorithm {
 public:
  ColumnGathererStream(size_t num_inputs, ReadBuffer &row_sources_buf_, size_t block_preferred_size_rows_,
                       size_t block_preferred_size_bytes_, std::optional<size_t> max_dynamic_subcolumns_, bool is_result_sparse_);
...
class ColumnGathererTransform final : public IMergingTransform<ColumnGathererStream> {
 public:
  ColumnGathererTransform(SharedHeader header, size_t num_inputs, std::unique_ptr<ReadBuffer> row_sources_buf_,
...
void ColumnGathererStream::gather(Column &column_res) {
  ...
  do {
    if (row_source_pos >= row_sources_end) break;
    ...
    size_t len = 1;
    size_t max_len =
        std::min(static_cast<size_t>(row_sources_end - row_source_pos), source.size - source.pos);  // interval should be in the same block
