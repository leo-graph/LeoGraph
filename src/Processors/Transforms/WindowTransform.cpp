...
static int compareValuesWithOffset(const IColumn *_compared_column, size_t compared_row, const IColumn *_reference_column,
                                   size_t reference_row, const Field &_offset, bool offset_is_preceding) {
...
template <typename ColumnType>
static int compareValuesWithOffsetFloat(const IColumn *_compared_column, size_t compared_row, const IColumn *_reference_column,
                                        size_t reference_row, const Field &_offset, bool offset_is_preceding) {
  // Casting the columns to the known type here makes it faster, probably
  // because the getData call can be devirtualized.
  const auto *compared_column = assert_cast<const ColumnType *>(_compared_column);
...
#define APPLY_FOR_NEST_TYPES(FUNCTION)                                                                                      \
  ...
  APPLY_FOR_ONE_NEST_TYPE(FUNCTION##Float, ColumnVector<Float64>)                                                           \
    ...
    throw Exception(ErrorCodes::NOT_IMPLEMENTED, "The RANGE OFFSET frame for '{}' ORDER BY nest column is not implemented", \
                    demangle(typeid(nest_compared_column).name()));                                                         \
  }

// A specialization of compareValuesWithOffset for nullable.
template <typename ColumnType>
static int compareValuesWithOffsetNullable(const IColumn *_compared_column, size_t compared_row, const IColumn *_reference_column,
                                           size_t reference_row, const Field &_offset, bool offset_is_preceding) {
  const auto *compared_column = assert_cast<const ColumnType *>(_compared_column);
  const auto *reference_column = assert_cast<const ColumnType *>(_reference_column);
  ...
  std::function<int(const IColumn *compared_column, size_t compared_row, const IColumn *reference_column, size_t reference_row,
                    const Field &offset, bool offset_is_preceding)>
      nest_compare_function;
  APPLY_FOR_NEST_TYPES(compareValuesWithOffset)
  return nest_compare_function(nest_compared_column.get(), compared_row, nest_reference_column.get(), reference_row, _offset,
                               offset_is_preceding);
}
...
#define APPLY_FOR_TYPES(FUNCTION)                                                                                      \
  ...
  APPLY_FOR_ONE_TYPE(FUNCTION##Nullable, ColumnNullable)                                                               \
    ...
    throw Exception(ErrorCodes::NOT_IMPLEMENTED, "The RANGE OFFSET frame for '{}' ORDER BY column is not implemented", \
                    demangle(typeid(*column).name()));                                                                 \
  }

WindowTransform::WindowTransform(SharedHeader input_header_, SharedHeader output_header_, const WindowDescription &window_description_,
  ...
  for (const auto &f : functions) {
    ...
    if (window_description.frame.is_default && functions.size() == 1 && workspace.window_function_impl) {
      ...
      if (custom_default_frame) window_description.frame = *custom_default_frame;
    }
  ...
  if (window_description.frame.type == WindowFrame::FrameType::RANGE &&
    ...
    if (window_description.frame.begin_type == WindowFrame::BoundaryType::Offset) {
      ...
      if (accurateLess(window_description.frame.begin_offset, Field(0))) {
        throw Exception(ErrorCodes::BAD_ARGUMENTS, "Window frame start offset must be nonnegative, {} given",
...
void WindowTransform::advanceFrameStartRowsOffset() {
  ...
  const auto [moved_row, offset_left] = moveRowNumber(
      current_row, window_description.frame.begin_offset.safeGet<UInt64>() * (window_description.frame.begin_preceding ? -1 : 1));
  ...
  if (frame_start <= partition_start || (window_description.frame.begin_preceding && offset_left < 0)) {
...
void WindowTransform::advanceFrameStartRangeOffset() {
  ...
  for (; frame_start < partition_end; advanceRowNumber(frame_start)) {
    ...
    if (compare_values_with_offset(compared_column, frame_start.row, reference_column, current_row.row,
                                   window_description.frame.begin_offset, preceding) *
            direction >=
        0) {
      frame_started = true;
      return;
    ...
    case WindowFrame::BoundaryType::Offset:
      switch (window_description.frame.type) {
        ...
        case WindowFrame::FrameType::RANGE:
          ...
          break;
        default:
          throw Exception(ErrorCodes::NOT_IMPLEMENTED, "Frame start type '{}' for frame '{}' is not implemented",
                          window_description.frame.begin_type, window_description.frame.type);
      }
      break;
  }
...
void WindowTransform::advanceFrameEndRowsOffset() {
  ...
  const auto [moved_row, offset_left] = moveRowNumber(
      current_row, window_description.frame.end_offset.safeGet<UInt64>() * (window_description.frame.end_preceding ? -1 : 1) + 1);
  ...
  // We must check offset_left < 0 first because partition_start might point
  // to a block that has already been freed, making the comparison unreliable.
  if (moved_row <= partition_start || (window_description.frame.end_preceding && offset_left < 0)) {
...
void WindowTransform::advanceFrameEndRangeOffset() {
  ...
  for (; frame_end < partition_end; advanceRowNumber(frame_end)) {
    ...
    if (compare_values_with_offset(compared_column, frame_end.row, reference_column, current_row.row, window_description.frame.end_offset,
                                   preceding) *
            direction >
        0) {
      frame_ended = true;
    ...
    case WindowFrame::BoundaryType::Offset:
      switch (window_description.frame.type) {
        ...
        case WindowFrame::FrameType::RANGE:
          ...
          break;
        default:
          throw Exception(ErrorCodes::NOT_IMPLEMENTED, "The frame end type '{}' is not implemented", window_description.frame.end_type);
      }
      break;
  }
  ...
  if (!input_is_finished) {
    ...
    for (auto &column : columns)
      column = recursiveRemoveLowCardinality(
          std::move(column)->convertToFullColumnIfReplicated()->convertToFullColumnIfConst()->convertToFullColumnIfSparse());
    block.input_columns = std::move(columns);

    // Initialize output columns.
    for (auto &ws : workspaces) {
      block.cast_columns.push_back(
          ws.window_function_impl ? ws.window_function_impl->castColumn(block.input_columns, ws.argument_column_indices) : nullptr);
...
struct WindowFunctionHelpers {
  ...
  ALWAYS_INLINE static bool checkPartitionEnterLastRow(const WindowTransform *transform) {
    ...
    if (!transform->partition_ended) return false;
    ...
    if (current_row != partition_end_row) {
      ...
      /// input row.
      if (current_row.row < transform->blockRowsNumber(current_row)) return false;
      if (partition_end_row.block != current_row.block + 1 || partition_end_row.row) return false;
    }
    return true;
  }
};
...
struct WindowFunctionExponentialTimeDecayedSum final : public StatefulWindowFunction<ExponentialTimeDecayedSumState> {
  ...
  WindowFunctionExponentialTimeDecayedSum(const std::string &name_, const DataTypes &argument_types_, const Array &parameters_)
    ...
    if (!isNumber(argument_types[ARGUMENT_VALUE])) {
      throw Exception(ErrorCodes::BAD_ARGUMENTS, "Argument {} must be a number, '{}' given", ARGUMENT_VALUE,
    ...
    if (!isNumber(argument_types[ARGUMENT_TIME]) && !isDateTime(argument_types[ARGUMENT_TIME]) &&
      ...
      throw Exception(ErrorCodes::BAD_ARGUMENTS, "Argument {} must be DateTime, DateTime64 or a number, '{}' given", ARGUMENT_TIME,
...
struct WindowFunctionNtile final : public StatefulWindowFunction<NtileState> {
  WindowFunctionNtile(const std::string &name_, const DataTypes &argument_types_, const Array &parameters_)
    ...
    if (type_id != TypeIndex::UInt8 && type_id != TypeIndex::UInt16 && type_id != TypeIndex::UInt32 && type_id != TypeIndex::UInt64)
      throw Exception(ErrorCodes::BAD_ARGUMENTS, "'{}' argument type must be an unsigned integer (not larger than 64-bit), got {}", name_,
  ...
  bool checkWindowFrameType(const WindowTransform *transform) const override {
    ...
    if (!is_frame_supported) {
      LOG_ERROR(getLogger("WindowFunctionNtile"),
...
void NtileState::windowInsertResultInto(const WindowTransform *transform, size_t function_index, const DataTypes &argument_types) {
  if (!buckets) [[unlikely]] {
    ...
    if (!isColumnConst(arg_col)) throw Exception(ErrorCodes::BAD_ARGUMENTS, "Argument of 'ntile' function must be a constant");
    auto type_id = argument_types[0]->getTypeId();
  ...
  if (!WindowFunctionHelpers::checkPartitionEnterLastRow(transform)) return;
 ...
 public:
  ...
  bool checkWindowFrameType(const WindowTransform *transform) const override {
    ...
    if (transform->window_description.frame != default_window_frame) {
      LOG_ERROR(getLogger("WindowFunctionPercentRank"), "Window frame for function 'percent_rank' should be '{}'",
                default_window_frame->toString());
      return false;
    }
    return true;
  }
  ...
  void windowInsertResultInto(const WindowTransform *transform, size_t function_index) const override {
    ...
    while (remaining_rows > 0) {
      ...
      if (available_block_rows <= remaining_rows) {
        ...
        for (size_t i = state.start_row.row; i < block_rows_number; ++i) data[i] = (data[i] - 1) / percent_rank_denominator;
...
struct WindowFunctionCumeDist final : public StatefulWindowFunction<CumeDistState> {
 public:
  WindowFunctionCumeDist(const std::string &name_, const DataTypes &argument_types_, const Array &parameters_)
      : StatefulWindowFunction(name_, argument_types_, parameters_, std::make_shared<DataTypeFloat64>()) {}
  ...
  bool checkWindowFrameType(const WindowTransform *transform) const override {
    auto default_window_frame = getDefaultFrame();
    if (transform->window_description.frame != default_window_frame) {
      LOG_ERROR(getLogger("WindowFunctionCumeDist"), "Window frame for function 'cume_dist' should be '{}'",
                default_window_frame->toString());
      return false;
    }
    return true;
  ...
  void windowInsertResultInto(const WindowTransform *transform, size_t function_index) const override {
    ...
    while (remaining_rows > 0) {
      ...
      if (available_block_rows <= remaining_rows) {
        auto &to_column = *transform->blockAt(state.start_row).output_columns[function_index];
        auto &data = assert_cast<ColumnFloat64 &>(to_column).getData();
        for (size_t i = state.start_row.row; i < block_rows_number; ++i) data[i] = data[i] / cume_dist_denominator;
  ...
  inline void insertPeerGroupEndRowNumberIntoColumn(const WindowTransform *transform, size_t function_index) const {
    ...
    while (true) {
      ...
      if (next >= transform->partition_end || !transform->arePeers(transform->current_row, next)) break;
      check_row = next;
...
struct WindowFunctionLagLeadImpl final : public StatelessWindowFunction {
  ...
  WindowFunctionLagLeadImpl(const std::string &name_, const DataTypes &argument_types_, const Array &parameters_)
    ...
    if (argument_types.size() > 3) {
      throw Exception(ErrorCodes::TOO_MANY_ARGUMENTS_FOR_FUNCTION, "Function '{}' accepts at most 3 arguments, {} given", name,
                      argument_types.size());
    }

    if (argument_types[0]->equals(*argument_types[2])) return;

    const auto supertype = tryGetLeastSupertype(DataTypes{argument_types[0], argument_types[2]});
    if (!supertype) {
      throw Exception(ErrorCodes::BAD_ARGUMENTS, "There is no supertype for the argument type '{}' and the default value type '{}'",
                      argument_types[0]->getName(), argument_types[2]->getName());
    }
    if (!argument_types[0]->equals(*supertype)) {
      throw Exception(ErrorCodes::BAD_ARGUMENTS,
                      ...
                      supertype->getName(), argument_types[0]->getName(), argument_types[2]->getName());
    }
  ...
  ColumnPtr castColumn(const Columns &columns, const VectorWithMemoryTracking<size_t> &idx) override {
    if (!func_cast) return nullptr;

    ColumnsWithTypeAndName arguments{{columns[idx[2]], argument_types[2], ""},
  ...
  std::optional<WindowFrame> getDefaultFrame() const override {
    if constexpr (!full_partition_default_frame) return {};
  ...
  void windowInsertResultInto(const WindowTransform *transform, size_t function_index) const override {
    ...
    Int64 offset = 1;
    if (argument_types.size() > 1) {
      offset = (*current_block.input_columns[workspace.argument_column_indices[1]])[transform->current_row.row].safeGet<Int64>();

      /// Either overflow or really negative value, both is not acceptable.
      if (offset < 0) {
        throw Exception(ErrorCodes::BAD_ARGUMENTS, "The offset for function {} must be in (0, {}], {} given", getName(), INT64_MAX, offset);
      }
    }
    ...
    if (offset_left != 0 || target_row < transform->frame_start || transform->frame_end <= target_row) {
      ...
      if (argument_types.size() > 2) {
        ...
        const IColumn &default_column = current_block.cast_columns[function_index]
...
struct WindowFunctionNthValue final : public StatelessWindowFunction {
  ...
  void windowInsertResultInto(const WindowTransform *transform, size_t function_index) const override {
    ...
    Int64 offset = (*current_block.input_columns[workspace.argument_column_indices[1]])[transform->current_row.row].safeGet<Int64>();

    /// Either overflow or really negative value, both is not acceptable.
    if (offset <= 0) {
      throw Exception(ErrorCodes::BAD_ARGUMENTS, "The offset for function {} must be in (1, {}], {} given", getName(), INT64_MAX, offset);
    }
...
struct NonNegativeDerivativeParams {
  ...
  NonNegativeDerivativeParams(const std::string &name_, const DataTypes &argument_types, const Array &parameters) {
    ...
    if (!isNumber(argument_types[ARGUMENT_METRIC])) {
      throw Exception(ErrorCodes::BAD_ARGUMENTS, "Argument {} must be a number, '{}' given", ARGUMENT_METRIC,
    ...
    if (!isDateTime(argument_types[ARGUMENT_TIMESTAMP]) && !isDateTime64(argument_types[ARGUMENT_TIMESTAMP])) {
      throw Exception(ErrorCodes::BAD_ARGUMENTS, "Argument {} must be DateTime or DateTime64, '{}' given", ARGUMENT_TIMESTAMP,
    ...
    if (argument_types.size() == 3) {
      ...
      if (!interval_datatype) {
        throw Exception(ErrorCodes::BAD_ARGUMENTS, "Argument {} must be an INTERVAL, '{}' given", ARGUMENT_INTERVAL,
                        argument_types[ARGUMENT_INTERVAL]->getName());
      }
      if (!interval_datatype->getKind().isFixedLength()) {
        throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT, "The INTERVAL must be a week or shorter, '{}' given",
                        argument_types[ARGUMENT_INTERVAL]->getName());
      }
...
struct WindowFunctionNonNegativeDerivative final : public StatefulWindowFunction<NonNegativeDerivativeState>,
  ...
  void windowInsertResultInto(const WindowTransform *transform, size_t function_index) const override {
    ...
    if (unlikely(!transform->current_row.row)) result = 0;
...
void registerWindowFunctions(AggregateFunctionFactory &factory) {
  ...
  const AggregateFunctionProperties properties = {// By default, if an aggregate function has a null argument, it will be
  ...
  factory.registerFunction("rank",
                           {[](const std::string &name, const DataTypes &argument_types, const Array &parameters, const Settings *) {
                              return std::make_shared<WindowFunctionRank>(name, argument_types, parameters);
                            },
                            {},
                            properties},
                           AggregateFunctionFactory::Case::Insensitive);

  factory.registerFunction("denseRank",
                           {[](const std::string &name, const DataTypes &argument_types, const Array &parameters, const Settings *) {
                              return std::make_shared<WindowFunctionDenseRank>(name, argument_types, parameters);
                            },
                            {},
                            properties});
  ...
  factory.registerFunction("percentRank",
                           {[](const std::string &name, const DataTypes &argument_types, const Array &parameters, const Settings *) {
                              return std::make_shared<WindowFunctionPercentRank>(name, argument_types, parameters);
                            },
                            {},
                            properties});
  ...
  factory.registerFunction("cume_dist",
                           {[](const std::string &name, const DataTypes &argument_types, const Array &parameters, const Settings *) {
                              return std::make_shared<WindowFunctionCumeDist>(name, argument_types, parameters);
                            },
                            {},
                            properties});

  factory.registerFunction("row_number",
                           {[](const std::string &name, const DataTypes &argument_types, const Array &parameters, const Settings *) {
                              return std::make_shared<WindowFunctionRowNumber>(name, argument_types, parameters);
                            },
                            {},
                            properties},
                           AggregateFunctionFactory::Case::Insensitive);

  factory.registerFunction("ntile",
                           {[](const std::string &name, const DataTypes &argument_types, const Array &parameters, const Settings *) {
                              return std::make_shared<WindowFunctionNtile>(name, argument_types, parameters);
                            },
                            {},
                            properties},
                           AggregateFunctionFactory::Case::Insensitive);

  factory.registerFunction("nth_value",
                           {[](const std::string &name, const DataTypes &argument_types, const Array &parameters, const Settings *) {
                              return std::make_shared<WindowFunctionNthValue>(name, argument_types, parameters);
                            },
                            {},
                            properties},
                           AggregateFunctionFactory::Case::Insensitive);

  factory.registerFunction("lagInFrame",
                           {[](const std::string &name, const DataTypes &argument_types, const Array &parameters, const Settings *) {
                              return std::make_shared<WindowFunctionLagLeadInFrame<false>>(name, argument_types, parameters);
                            },
                            {},
                            properties});

  factory.registerFunction("lag",
                           {[](const std::string &name, const DataTypes &argument_types, const Array &parameters, const Settings *) {
                              return std::make_shared<WindowFunctionLagLead<false>>(name, argument_types, parameters);
                            },
                            {},
                            properties},
                           AggregateFunctionFactory::Case::Insensitive);

  factory.registerFunction("leadInFrame",
                           {[](const std::string &name, const DataTypes &argument_types, const Array &parameters, const Settings *) {
                              return std::make_shared<WindowFunctionLagLeadInFrame<true>>(name, argument_types, parameters);
                            },
                            {},
                            properties});

  factory.registerFunction("lead",
                           {[](const std::string &name, const DataTypes &argument_types, const Array &parameters, const Settings *) {
                              return std::make_shared<WindowFunctionLagLead<true>>(name, argument_types, parameters);
                            },
                            {},
                            properties},
                           AggregateFunctionFactory::Case::Insensitive);
...
Returns the sum of exponentially smoothed moving average values of a time series at the index `t` in time.
  ...
  FunctionDocumentation::Arguments exponentialTimeDecayedSum_arguments = {
      {"v", "Value.", {"(U)Int*", "Float*", "Decimal"}}, {"t", "Time.", {"(U)Int*", "Float*", "Decimal", "DateTime", "DateTime64"}}};
  ...
  FunctionDocumentation::Examples exponentialTimeDecayedSum_examples = {{"Window function usage with visual representation",
                                                                         R"(
SELECT
    value,
    time,
...
└───────┴──────┴──────────────────────┴───────────────────────────────────────────────────┘
  ...
  FunctionDocumentation exponentialTimeDecayedSum_documentation = {
      ...
      exponentialTimeDecayedSum_parameters,    exponentialTimeDecayedSum_returned_value, exponentialTimeDecayedSum_examples,
      exponentialTimeDecayedSum_introduced_in, exponentialTimeDecayedSum_category};
  factory.registerFunction("exponentialTimeDecayedSum",
                           {[](const std::string &name, const DataTypes &argument_types, const Array &parameters, const Settings *) {
                              return std::make_shared<WindowFunctionExponentialTimeDecayedSum>(name, argument_types, parameters);
                            },
...
Returns the maximum of the computed exponentially smoothed moving average at index `t` in time with that at `t-1`.
  ...
  FunctionDocumentation::Parameters exponentialTimeDecayedMax_parameters = {{"x", "Half-life period.", {"(U)Int*", "Float*", "Decimal"}}};
  FunctionDocumentation::ReturnedValue exponentialTimeDecayedMax_returned_value = {
      "Returns the maximum of the exponentially smoothed weighted moving average at `t` and `t-1`.", {"Float64"}};
  FunctionDocumentation::Examples exponentialTimeDecayedMax_examples = {{"Window function usage with visual representation",
                                                                         R"(
SELECT
    value,
    time,
...
└───────┴──────┴──────────────────────┴────────────┘
  ...
  FunctionDocumentation exponentialTimeDecayedMax_documentation = {
      exponentialTimeDecayedMax_description,   exponentialTimeDecayedMax_syntax,         exponentialTimeDecayedMax_arguments,
      exponentialTimeDecayedMax_parameters,    exponentialTimeDecayedMax_returned_value, exponentialTimeDecayedMax_examples,
      exponentialTimeDecayedMax_introduced_in, exponentialTimeDecayedMax_category};
  factory.registerFunction("exponentialTimeDecayedMax",
                           {[](const std::string &name, const DataTypes &argument_types, const Array &parameters, const Settings *) {
                              return std::make_shared<WindowFunctionExponentialTimeDecayedMax>(name, argument_types, parameters);
                            },
...
Returns the cumulative exponential decay over a time series at the index `t` in time.
  ...
  FunctionDocumentation::Parameters exponentialTimeDecayedCount_parameters = {{"x", "Half-life period.", {"(U)Int*", "Float*", "Decimal"}}};
  ...
  FunctionDocumentation::Examples exponentialTimeDecayedCount_examples = {{"Window function usage with visual representation",
                                                                           R"(
SELECT
    value,
    time,
...
└───────┴──────┴──────────────────────┴────────────────────────────┘
  ...
  FunctionDocumentation exponentialTimeDecayedCount_documentation = {
      exponentialTimeDecayedCount_description,   exponentialTimeDecayedCount_syntax,         exponentialTimeDecayedCount_arguments,
      exponentialTimeDecayedCount_parameters,    exponentialTimeDecayedCount_returned_value, exponentialTimeDecayedCount_examples,
      exponentialTimeDecayedCount_introduced_in, exponentialTimeDecayedCount_category};
  factory.registerFunction("exponentialTimeDecayedCount",
                           {[](const std::string &name, const DataTypes &argument_types, const Array &parameters, const Settings *) {
                              return std::make_shared<WindowFunctionExponentialTimeDecayedCount>(name, argument_types, parameters);
                            },
...
Returns the exponentially smoothed weighted moving average of values of a time series at point `t` in time.
  ...
  FunctionDocumentation::Arguments exponentialTimeDecayedAvg_arguments = {
      {"v", "Value.", {"(U)Int*", "Float*", "Decimal"}}, {"t", "Time.", {"(U)Int*", "Float*", "Decimal", "DateTime", "DateTime64"}}};
  FunctionDocumentation::Parameters exponentialTimeDecayedAvg_parameters = {{"x", "Half-life period.", {"(U)Int*", "Float*", "Decimal"}}};
  ...
  FunctionDocumentation::Examples exponentialTimeDecayedAvg_examples = {{"Window function usage with visual representation",
                                                                         R"(
SELECT
    value,
    time,
...
└───────┴──────┴──────────────────────┴────────────┘
  ...
  FunctionDocumentation exponentialTimeDecayedAvg_documentation = {
      ...
      exponentialTimeDecayedAvg_parameters,    exponentialTimeDecayedAvg_returned_value, exponentialTimeDecayedAvg_examples,
      exponentialTimeDecayedAvg_introduced_in, exponentialTimeDecayedAvg_category};
  factory.registerFunction("exponentialTimeDecayedAvg",
                           {[](const std::string &name, const DataTypes &argument_types, const Array &parameters, const Settings *) {
                              return std::make_shared<WindowFunctionExponentialTimeDecayedAvg>(name, argument_types, parameters);
                            },
                            exponentialTimeDecayedAvg_documentation, properties});

  factory.registerFunction("nonNegativeDerivative",
                           {[](const std::string &name, const DataTypes &argument_types, const Array &parameters, const Settings *) {
                              return std::make_shared<WindowFunctionNonNegativeDerivative>(name, argument_types, parameters);
                            },
                            {},
                            properties});
}
