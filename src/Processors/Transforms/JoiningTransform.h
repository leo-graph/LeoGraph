...
class JoiningTransform : public IProcessor {
 public:
  JoiningTransform(SharedHeader input_header, SharedHeader output_header, JoinPtr join_, size_t max_block_size_, bool on_totals_ = false,
                   bool default_totals_ = false, FinishCounterPtr finish_counter_ = nullptr);
...
class NonJoinedBlocksTransform : public ISource {
 public:
  NonJoinedBlocksTransform(SharedHeader output_header, JoinPtr join_, Block left_sample_block_, UInt64 max_block_size_,
                           size_t stream_index_, size_t num_streams_);
