...
class WindowTransform final : public IProcessor {
 public:
  WindowTransform(SharedHeader input_header_, SharedHeader output_header_, const WindowDescription &window_description_,
                  const std::vector<WindowFunctionDescription> &functions);

  ~WindowTransform() override;

  String getName() const override { return "WindowTransform"; }
  ...
  const Columns &inputAt(const RowNumber &x) const { return const_cast<WindowTransform *>(this)->inputAt(x); }

  WindowTransformBlock &blockAt(UInt64 block_number);
  const WindowTransformBlock &blockAt(UInt64 block_number) const { return const_cast<WindowTransform *>(this)->blockAt(block_number); }
  WindowTransformBlock &blockAt(const RowNumber &x) { return blockAt(x.block); }
  const WindowTransformBlock &blockAt(const RowNumber &x) const { return const_cast<WindowTransform *>(this)->blockAt(x); }
  ...
  RowNumber blocksEnd() const { return RowNumber{first_block_number + blocks.size(), 0}; }
  RowNumber blocksBegin() const { return RowNumber{first_block_number, 0}; }
