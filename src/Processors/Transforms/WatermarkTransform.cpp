...
WatermarkTransform::WatermarkTransform(SharedHeader header_, StorageWindowView &storage_, const String &window_column_name_,
...
WatermarkTransform::~WatermarkTransform() {
  if (max_watermark) storage.updateMaxWatermark(max_watermark);
  if (lateness_upper_bound) storage.addFireSignal(late_signals);
}
...
void WatermarkTransform::transform(Chunk &chunk) {
  ...
  for (const auto &ts : window_end_data) {
    ...
    if (lateness_upper_bound && ts <= lateness_upper_bound) late_signals.insert(ts);
  }
