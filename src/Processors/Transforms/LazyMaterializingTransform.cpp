...
LazyMaterializingTransform::LazyMaterializingTransform(SharedHeader main_header, SharedHeader lazy_header,
                                                       LazyMaterializingRowsPtr lazy_materializing_rows_,
                                                       RuntimeDataflowStatisticsCacheUpdaterPtr updater_)
    : IProcessor(InputPorts({main_header, lazy_header}),
...
LazyMaterializingTransform::Status LazyMaterializingTransform::prepare() {
  ...
  if (!main_input.isFinished()) {
    ...
    if (!main_input.hasData()) return Status::NeedData;

    auto chunk = main_input.pull();
    if (chunk.hasRows()) chunks.emplace_back(std::move(chunk));

    if (!main_input.isFinished()) return Status::NeedData;
  }
  ...
  if (!lazy_input.isFinished()) {
    ...
    if (!lazy_input.hasData()) return Status::NeedData;

    auto chunk = lazy_input.pull();
    if (chunk.hasRows()) chunks.emplace_back(std::move(chunk));

    if (!lazy_input.isFinished()) return Status::NeedData;
  }
...
void LazyMaterializingRows::filterRangesAndFillRows(const PaddedPODArray<UInt64> &sorted_indexes) {
  ...
  for (auto &part : ranges_in_data_parts) {
    ...
    while (!part.ranges.empty()) {
      if (!(next_index < sorted_indexes.size())) break;
      ...
      if (sorted_indexes[next_index] < offset + row_begin)
        throw Exception(ErrorCodes::LOGICAL_ERROR, "LazyMaterializingTransform: Row {} is not in any range", sorted_indexes[next_index]);
      ...
      while (range.begin < range.end) {
        ...
        if (sorted_indexes[next_index] < offset + row_begin) break;
      ...
      // std::cerr << subrange_begin << " " << subrange_end << " || " << subrange_row_begin << " " << subrange_row_end <<  " || " <<
      // rows_in_part.size() << "\n";
    ...
    if (!rows_in_part.empty()) rows_in_parts[part.part_index_in_query] = std::move(rows_in_part);
  }
  ...
  LOG_TRACE(getLogger("LazyMaterializingTransform"), "Lazily reading {} rows from {} parts, {} ranges, {} marks", sorted_indexes.size(),
            ranges_in_data_parts.size(), total_ranges, total_marks);
}
...
void LazyMaterializingTransform::prepareMainChunk() {
...

  {
    ...
    for (size_t offset = 0; offset < rows;) {
      ...
      size_t count = 1;
      while (offset + count < rows && sorted_indexes[offset + count] == idx) ++count;
  ...
  if (total_ms >= 100)
    LOG_TRACE(getLogger("LazyMaterializingTransform"),
              "Preparing chunk {} ms, squashing {} ms, sorting {} ms, permute {} ms, prepare offsets {} ms, filter intervals {} ms",
...
void LazyMaterializingTransform::prepareLazyChunk() {
  ...
  if (chunk.getNumRows() != offsets.size())
    throw Exception(ErrorCodes::LOGICAL_ERROR,
                    "LazyMaterializingTransform: Number of rows in lazy chunk {} does not match number of offsets {}", chunk.getNumRows(),
                    offsets.size());
  ...
  if (!is_identity_permutation) {
    ...
    for (size_t i = 0; i < rows; ++i) inverted_permutation[permutation[i]] = i;
...

  {
    ...
    for (auto &col : lazy_columns) {
      if (should_replicate) col = col->replicate(offsets);

      if (!is_identity_permutation) col = col->permute(inverted_permutation, rows);
    }
  ...
  if (updater) updater->recordOutputChunk(*result_chunk, getOutputs().front().getHeader());

  auto total_ms = total_watch.elapsedMilliseconds();
  if (total_ms >= 100)
    LOG_TRACE(getLogger("LazyMaterializingTransform"),
              "Preparing lazy chunk {} ms, squashing {} ms, reversing permutations {} ms, permuting {} ms", total_ms, squash_ms,
              reverse_permutation_ms, permute_ms);
}
