...
DistinctTransform::DistinctTransform(SharedHeader header_, const SizeLimits &set_size_limits_, const UInt64 limit_hint_,
                                     const Names &columns_)
    : ISimpleTransform(header_, header_, true), limit_hint(limit_hint_), set_size_limits(set_size_limits_) {
  ...
  for (size_t i = 0; i < num_columns; ++i) {
    ...
    if (col && !isColumnConst(*col)) key_columns_pos.emplace_back(pos);
  }
}

template <typename Method>
void DistinctTransform::buildFilter(Method &method, const ColumnRawPtrs &columns, IColumn::Filter &filter, const size_t rows,
                                    SetVariants &variants, const IColumn::Filter *mask) const {
...
IColumn::Filter DistinctTransform::buildLowCardinalityMask(const ColumnLowCardinality &column, size_t num_rows) {
  ...
  if (state.seen_count == dict_size) return {};  /// empty mask == no candidates
  ...
  auto handle_index = [&](size_t idx, size_t row) {
    ...
    if (!seen[idx]) {
      ...
      if (mask.empty()) mask.resize_fill(num_rows);
  ...
  switch (index_type_size) {
    case sizeof(UInt8): {
      ...
      for (size_t row = 0; row < num_rows; ++row) handle_index(static_cast<size_t>(col[row]), row);
      break;
    }
    case sizeof(UInt16): {
...
void DistinctTransform::transform(Chunk &chunk) {
  ...
  if (unlikely(key_columns_pos.empty())) {
    for (auto &column : columns) column = column->cut(0, 1);
  ...
  for (auto pos : key_columns_pos) column_ptrs.emplace_back(columns[pos].get());
  ...
  if (key_columns_pos.size() == 1) {
    if (const auto *lc = typeid_cast<const ColumnLowCardinality *>(column_ptrs[0])) {
      ...
      if (lc_mask->empty()) return;
    }
  }

  if (data.empty()) data.init(SetVariants::chooseMethod(column_ptrs, key_sizes));
...
#undef M
  ...
  if (new_set_size == old_set_size) return;

  if (!set_size_limits.check(new_set_size, data.getTotalByteCount(), "DISTINCT", ErrorCodes::SET_SIZE_LIMIT_EXCEEDED)) return;
