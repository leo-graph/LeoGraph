 ...
 public:
  struct Range {
    ...
    explicit Range(Chunk chunk_, size_t begin_, size_t length_)
        : begin(begin_), length(length_), current(begin_), chunk(std::move(chunk_)) {
  ...
  AllJoinState(const FullMergeJoinCursor &lcursor, size_t lpos, const FullMergeJoinCursor &rcursor, size_t rpos)
...
class MergeJoinAlgorithm final : public IMergingAlgorithm {
 public:
  MergeJoinAlgorithm(JoinKind kind_, JoinStrictness strictness_, const TableJoin::JoinOnClause &on_clause_, SharedHeaders &input_headers,
...
class MergeJoinTransform final : public IMergingTransform<MergeJoinAlgorithm> {
 ...
 public:
  MergeJoinTransform(JoinPtr table_join, SharedHeaders &input_headers, SharedHeader output_header, size_t max_block_size,
                     UInt64 limit_hint = 0);

  MergeJoinTransform(JoinKind kind_, JoinStrictness strictness_, const TableJoin::JoinOnClause &on_clause_, SharedHeaders &input_headers,
                     SharedHeader output_header, size_t max_block_size, UInt64 limit_hint_ = 0);
