...
static void checkCalculated(const ColumnWithTypeAndName &col_read, const ColumnWithTypeAndName &col_defaults, size_t defaults_needed) {
  ...
  if (column_size < defaults_needed) throw Exception(ErrorCodes::SIZES_OF_COLUMNS_DOESNT_MATCH, "Unexpected defaults count");

  if (!col_read.type->equals(*col_defaults.type)) throw Exception(ErrorCodes::TYPE_MISMATCH, "Mismatch column types while adding defaults");
}
...
static void mixNumberColumns(TypeIndex type_idx, MutableColumnPtr &column_mixed, const ColumnPtr &col_defaults,
    ...
    if constexpr (!std::is_same_v<DataType, DataTypeString> && !std::is_same_v<DataType, DataTypeFixedString>) {
      ...
      if (auto const_col_defs = checkAndGetColumnConst<ColVecType>(col_defaults.get())) {
        ...
        for (size_t i = 0; i < defaults_mask.size(); ++i)
          if (defaults_mask[i]) dst[i] = value;
      ...
      if (auto col_defs = checkAndGetColumn<ColVecType>(col_defaults.get())) {
        ...
        for (size_t i = 0; i < defaults_mask.size(); ++i)
          if (defaults_mask[i]) dst[i] = src[i];
  ...
  if (!callOnIndexAndDataType<void>(type_idx, call)) throw Exception(ErrorCodes::LOGICAL_ERROR, "Unexpected type on mixNumberColumns");
}
...
static MutableColumnPtr mixColumns(const ColumnWithTypeAndName &col_read, const ColumnWithTypeAndName &col_defaults,
  ...
  for (size_t i = defaults_needed; i < column_size; ++i) column_mixed->insertFrom(*col_read.column, i);

  return column_mixed;
}

AddingDefaultsTransform::AddingDefaultsTransform(SharedHeader header, const ColumnsDescription &columns_, IInputFormat &input_format_,
                                                 ContextPtr context_)
...
void AddingDefaultsTransform::transform(Chunk &chunk) {
  if (column_defaults.empty()) return;

  const auto *block_missing_values = input_format.getMissingValues();
  if (!block_missing_values) return;
  ...
  for (const auto &[col_name, col_default] : column_defaults) {
    if (!res.has(col_name)) continue;
    size_t column_idx = res.getPositionByName(col_name);
    if (block_missing_values->hasDefaultBits(column_idx)) columns_needing_defaults.emplace_back(col_name, column_idx);
  }
  ...
  for (const auto &[col_name, col_idx] : columns_needing_defaults) columns_needing_defaults_set.insert(col_name);
  ...
  for (const auto &[col_name, col_idx] : columns_needing_defaults) {
    ...
    for (const auto &req : required) {
      if (columns_needing_defaults_set.contains(req)) deps.insert(req);
    }
  ...
  while (processed.size() < columns_needing_defaults.size()) {
    ...
    for (const auto &[col_name, col_idx] : columns_needing_defaults) {
      if (processed.contains(col_name)) continue;
      ...
      if (all_deps_ready) ready.emplace_back(col_name, col_idx);
    }
    ...
    if (ready.empty()) {
      ...
      for (const auto &[col_name, col_idx] : columns_needing_defaults) {
        if (!processed.contains(col_name)) ready.emplace_back(col_name, col_idx);
      }
    }
    ...
    for (const auto &[col_name, col_idx] : ready) {
      if (evaluate_block.has(col_name)) evaluate_block.erase(col_name);
      const auto &col = header.getByPosition(col_idx);
      required_columns_list.emplace_back(col.name, col.type);
    }

    if (!evaluate_block.columns())
      evaluate_block.insert(
          {ColumnConst::create(ColumnUInt8::create(1, static_cast<UInt8>(0)), num_rows), std::make_shared<DataTypeUInt8>(), "_dummy"});
    ...
    if (dag) {
      ...
      auto actions = std::make_shared<ExpressionActions>(ActionsDAG::merge(std::move(extracting_subcolumns_dag), std::move(*dag)),
    ...
    if (!mixed_columns.empty()) {
      ...
      for (auto &[position, mixed_col] : mixed_columns) mutation[position] = std::move(mixed_col);
