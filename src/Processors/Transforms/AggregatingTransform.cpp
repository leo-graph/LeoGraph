...
const AggregatedChunkInfo* getInfoFromChunk(const Chunk& chunk) {
  ...
  if (!agg_info) throw Exception(ErrorCodes::LOGICAL_ERROR, "Chunk should have AggregatedChunkInfo.");
...
class SourceFromNativeStream final : public ISource {
 public:
  explicit SourceFromNativeStream(SharedHeader header, TemporaryBlockStreamReaderHolder tmp_stream_)
      : ISource(header), tmp_stream(std::move(tmp_stream_)) {}

  String getName() const override { return "SourceFromNativeStream"; }

  Chunk generate() override {
    if (!tmp_stream) return {};
...
class ConvertingAggregatedToChunksWithMergingSourceForFixedHashMap final : public ISource {
 public:
  ...
  ConvertingAggregatedToChunksWithMergingSourceForFixedHashMap(AggregatingTransformParamsPtr params_, ManyAggregatedDataVariantsPtr data_,
                                                               UInt32 thread_index_, UInt32 num_threads_, Arena* arena_)
...
class ConvertingAggregatedToChunksWithMergingSource final : public ISource {
 public:
  ...
  struct SharedData {
    ...
    SharedData() {
      for (auto& flag : is_bucket_processed) flag = false;
    }
  };

  using SharedDataPtr = std::shared_ptr<SharedData>;

  ConvertingAggregatedToChunksWithMergingSource(AggregatingTransformParamsPtr params_, ManyAggregatedDataVariantsPtr data_,
                                                SharedDataPtr shared_data_, Arena* arena_,
 ...
 protected:
  Chunk generate() override {
    ...
    Block block =
        params->aggregator.mergeAndConvertOneBucketToBlock(*data, arena, params->final, bucket_num, shared_data->is_cancelled, updater);
...
class FlattenChunksToMergeTransform final : public IProcessor {
 ...
 private:
  ...
  void process(Chunk&& chunk) {
    ...
    if (!chunks_to_merge) throw Exception(ErrorCodes::LOGICAL_ERROR, "Expected chunk with ChunksToMerge info in {}", getName());

    if (chunks_to_merge->chunks)
      for (auto& cur_chunk : *chunks_to_merge->chunks) chunks.emplace_back(std::move(cur_chunk));
  }
  ...
  Status prepare() override {
    ...
    if (!chunks.empty()) {
      ...
      if (!chunks.empty()) return Status::Ready;
    }
    ...
    if (input.isFinished()) return Status::Ready;

    input.setNeeded();

    if (!input.hasData()) return Status::NeedData;
...
class ConvertingAggregatedToChunksTransform final : public IProcessor {
 public:
  ...
  IProcessor::Status prepare() override {
    ...
    if (!is_initialized) return Status::Ready;

    if (!processors.empty()) return Status::ExpandPipeline;

    if (!single_level_chunks.empty()) return preparePushToOutput();
 ...
 private:
  bool worthParallelMergeSingleLevel() {
    ...
    if (!params->aggregator.isTypeFixedSize(*data)) return false;
  ...
  IProcessor::Status prepareParallelizeSingleLevel() {
    for (auto& input : inputs) {
      if (!input.isFinished()) return Status::NeedData;
    }
  ...
  IProcessor::Status preparePushToOutput() {
    if (single_level_chunks.empty()) throw Exception(ErrorCodes::LOGICAL_ERROR, "Some ready chunks expected");
  ...
  IProcessor::Status prepareTwoLevel() {
    ...
    auto get_bucket_if_ready = [&](UInt32 bucket_num) -> Chunk {
      if (!shared_data->is_bucket_processed[bucket_num]) return {};

      if (!two_level_chunks[bucket_num]) return {};
    ...
    while (current_bucket_num < NUM_BUCKETS) {
      ...
      // No ready buckets.
      if (!chunk) return Status::NeedData;
    ...
    if (!out_of_order_buckets.empty()) return Status::NeedData;
  ...
  void initialize() {
    ...
    if (num_threads > first->aggregates_pools.size()) {
      ...
      for (size_t j = first_pool.size(); j < num_threads; ++j) first_pool.emplace_back(std::make_shared<Arena>());
    }
    ...
    if (first->type == AggregatedDataVariants::Type::without_key || params->params.overflow_row) {
      ...
      if (updater) updater->recordAggregationStateSizes(*first, /*bucket=*/-1);
      auto block =
          params->aggregator.prepareBlockAndFillWithoutKey(*first, params->final, first->type != AggregatedDataVariants::Type::without_key);
      if (updater) updater->recordAggregationKeySizes(params->aggregator, block);

      if (block.rows() > 0) single_level_chunks.emplace_back(convertToChunk(block));
    }
  }
...
AggregatingTransform::AggregatingTransform(SharedHeader header, AggregatingTransformParamsPtr params_,
                                           RuntimeDataflowStatisticsCacheUpdaterPtr updater_)
    : AggregatingTransform(std::move(header), std::move(params_), std::make_unique<ManyAggregatedData>(1), 0, 1, 1,
                           true /* should_produce_results_in_order_of_bucket_number */, false /* skip_merging */, updater_) {}

AggregatingTransform::AggregatingTransform(SharedHeader header, AggregatingTransformParamsPtr params_, ManyAggregatedDataPtr many_data_,
                                           size_t current_variant, size_t max_threads_, size_t temporary_data_merge_threads_,
                                           bool should_produce_results_in_order_of_bucket_number_, bool skip_merging_,
                                           RuntimeDataflowStatisticsCacheUpdaterPtr updater_)
...
IProcessor::Status AggregatingTransform::prepare() {
  ...
  if (is_generate_initialized.test() && !is_pipeline_created && !processors.empty()) return Status::ExpandPipeline;

  /// Only possible while consuming.
  if (read_current_chunk) return Status::Ready;
...
Processors AggregatingTransform::expandPipeline() {
  if (processors.empty()) throw Exception(ErrorCodes::LOGICAL_ERROR, "Can not expandPipeline in AggregatingTransform. This is a bug.");
...
void AggregatingTransform::consume(Chunk chunk) {
  ...
  if (num_rows == 0 && params->params.empty_result_for_aggregation_by_empty_set) return;

  if (!is_consume_started) {
    LOG_TRACE(log, "Aggregating");
    is_consume_started = true;
  }
  if (rows_before_aggregation) rows_before_aggregation->add(num_rows);
  src_rows += num_rows;
  ...
  if (params->params.only_merge) {
    ...
    if (!params->aggregator.mergeOnBlock(block, variants, no_more_keys, is_cancelled)) is_consume_finished = true;
  } else {
...
void AggregatingTransform::initGenerate() {
  if (is_generate_initialized.test_and_set()) return;
  ...
  LOG_TRACE(log, "Aggregated. {} to {} rows (from {}) in {} sec. ({:.3f} rows/sec., {}/sec.)", src_rows, rows, ReadableSize(src_bytes),
  ...
  if (params->aggregator.hasTemporaryData()) {
    if (variants.isConvertibleToTwoLevel()) variants.convertToTwoLevel();

    /// Flush data in the RAM to disk also. It's easier than merging on-disk and RAM data.
    if (!variants.empty()) params->aggregator.writeToTemporaryFile(variants);
  }
  ...
  auto aggregator_has_temporary_data = [&]() {
    return params->aggregator.hasTemporaryData() || std::any_of(params->aggregator_list_ptr->begin(), params->aggregator_list_ptr->end(),
  ...
  if (!aggregator_has_temporary_data()) {
    if (!skip_merging) {
      ...
      if (updater) updater->markUnsupportedCase();
      ...
      if (!pipe.empty()) {
        if (should_produce_results_in_order_of_bucket_number) {
          ...
          /// So now we can insert transform that will keep chunks size under control. It makes few times difference in exec time in some
          /// cases.
          if (params->final) {
            pipe.addSimpleTransform([this](const SharedHeader& header) {
    ...
    if (many_data->variants.size() > 1) {
      ...
      for (auto& cur_variants : many_data->variants) {
        if (cur_variants->isConvertibleToTwoLevel()) cur_variants->convertToTwoLevel();

        if (!cur_variants->empty()) params->aggregator.writeToTemporaryFile(*cur_variants);
      }
    }
    ...
    for (auto& aggregator : *params->aggregator_list_ptr) {
      ...
      for (auto& tmp_stream : new_tmp_files) {
        ...
        pipes.emplace_back(Pipe(
            std::make_unique<SourceFromNativeStream>(std::make_shared<const Block>(tmp_stream.getHeader()), tmp_stream.getReadStream())));
      }

      tmp_files.splice(tmp_files.end(), new_tmp_files);
    }

    LOG_DEBUG(log, "Will merge {} temporary files of size {} compressed, {} uncompressed.", num_streams, ReadableSize(compressed_size),
