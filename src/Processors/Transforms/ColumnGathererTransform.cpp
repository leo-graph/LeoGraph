...
ColumnGathererStream::ColumnGathererStream(size_t num_inputs, ReadBuffer& row_sources_buf_, size_t block_preferred_size_rows_,
                                           size_t block_preferred_size_bytes_, std::optional<size_t> max_dynamic_subcolumns_,
  ...
  if (num_inputs == 0) throw Exception(ErrorCodes::EMPTY_DATA_PASSED, "There are no streams to gather");
}
...
void ColumnGathererStream::initialize(Inputs inputs) {
  ...
  for (size_t i = 0; i < inputs.size(); ++i) {
    if (!inputs[i].chunk) continue;

    if (!is_result_sparse) removeSpecialColumnRepresentations(inputs[i].chunk);
  ...
  if (is_result_sparse && !result_column->isSparse()) result_column = ColumnSparse::create(std::move(result_column));

  if (result_column->hasDynamicStructure()) result_column->takeDynamicStructureFromSourceColumns(source_columns, max_dynamic_subcolumns);
}

IMergingAlgorithm::Status ColumnGathererStream::merge() {
  /// Nothing to read after initialize.
  if (!result_column) return Status(Chunk(), true);
  ...
  if (sources.size() == 1 && row_sources_buf.eof()) {
    ...
    if (next_required_source == -1) return Status(Chunk(), true);
  ...
  if (next_required_source != -1) return Status(next_required_source);
...
void ColumnGathererStream::consume(Input& input, size_t source_num) {
  ...
  if (input.chunk) {
    if (!is_result_sparse) removeSpecialColumnRepresentations(input.chunk);
...
ColumnGathererTransform::ColumnGathererTransform(SharedHeader header, size_t num_inputs, std::unique_ptr<ReadBuffer> row_sources_buf_,
                                                 size_t block_preferred_size_rows_, size_t block_preferred_size_bytes_,
                                                 std::optional<size_t> max_dynamic_subcolumns_, bool is_result_sparse_)
    : IMergingTransform<ColumnGathererStream>(num_inputs, header, header, /*have_all_inputs_=*/true, /*limit_hint_=*/0,
                                              /*always_read_till_end_=*/false, num_inputs, *row_sources_buf_, block_preferred_size_rows_,
                                              block_preferred_size_bytes_, max_dynamic_subcolumns_, is_result_sparse_),
      row_sources_buf_holder(std::move(row_sources_buf_)),
      log(getLogger("ColumnGathererStream")) {
  if (header->columns() != 1)
    throw Exception(ErrorCodes::INCORRECT_NUMBER_OF_COLUMNS, "Header should have 1 column, but contains {}", toString(header->columns()));
}
