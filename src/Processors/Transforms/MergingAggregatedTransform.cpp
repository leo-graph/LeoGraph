...
Block MergingAggregatedTransform::appendGroupingIfNeeded(const Block &in_header, Block out_header) {
  ...
  if (in_header.has("__grouping_set")) out_header.insert(0, in_header.getByName("__grouping_set"));
...
static ActionsDAG makeReorderingActions(const Block &in_header, const GroupingSetsParams &params) {
  ...
  for (size_t pos = 0; pos < outputs.size(); ++pos) index.emplace(outputs[pos]->result_name, pos);
  ...
  for (const auto &used_name : params.used_keys) index[used_name] = outputs.size();
  for (const auto &missing_name : params.missing_keys) index[missing_name] = outputs.size();

  for (const auto *output : outputs) {
    if (index[output->result_name] != outputs.size()) new_outputs.push_back(output);
  }
...
MergingAggregatedTransform::MergingAggregatedTransform(SharedHeader header_, Aggregator::Params params, bool final,
  ...
  if (!grouping_sets_params.empty()) {
    ...
    for (const auto &grouping_set_params : grouping_sets_params) {
      ...
      Aggregator::Params set_params(grouping_set_params.used_keys, params.aggregates, params.overflow_row, params.max_threads,
      ...
      auto transform_params = std::make_shared<AggregatingTransformParams>(
          std::make_shared<const Block>(reordering.updateHeader(in_header)), std::move(set_params), final);

      auto creating = AggregatingStep::makeCreatingMissingKeysForGroupingSetDAG(transform_params->getHeader(), out_header,
...
void MergingAggregatedTransform::addBlock(Block block) {
  if (grouping_sets.size() == 1) {
    ...
    if (grouping_sets[0].reordering_key_columns_actions) grouping_sets[0].reordering_key_columns_actions->execute(block);
  ...
  for (size_t row = 1; row < num_rows; ++row) {
    ...
    if (selector.empty()) selector.reserve(num_rows);
  ...
  if (max_group >= grouping_sets.size())
    throw Exception(ErrorCodes::LOGICAL_ERROR, "Invalid group number {}. Number of groups {}.", last_group, grouping_sets.size());
  ...
  for (size_t group_id = 0; group_id < num_groups; ++group_id) split_blocks[group_id] = block.cloneEmpty();
...
Chunk MergingAggregatedTransform::generate() {
  if (!generate_started) {
    ...
    for (auto &grouping_set : grouping_sets) {
      ...
      if (grouping_set.creating_missing_keys_actions)
        for (auto &block : merged_blocks) grouping_set.creating_missing_keys_actions->execute(block);
  ...
  if (next_block == blocks.end()) return {};
