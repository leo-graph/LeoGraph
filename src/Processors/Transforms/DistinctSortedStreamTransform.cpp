...
DistinctSortedStreamTransform::DistinctSortedStreamTransform(SharedHeader header_, const SizeLimits& output_size_limits_,
                                                             UInt64 limit_hint_, const SortDescription& sorted_columns_descr_,
  ...
  for (const auto& source_column : source_columns) {
    ...
    if (std::find(sorted_columns_pos.begin(), sorted_columns_pos.end(), pos) != sorted_columns_pos.end()) continue;

    const auto& col = header_->getByPosition(pos).column;
    if (col && !isColumnConst(*col)) other_columns_pos.emplace_back(pos);
  }
...
void DistinctSortedStreamTransform::initChunkProcessing(const Columns& input_columns) {
  ...
  for (size_t pos : sorted_columns_pos) sorted_columns.emplace_back(input_columns[pos].get());

  other_columns.clear();
  for (size_t pos : other_columns_pos) other_columns.emplace_back(input_columns[pos].get());
...
size_t DistinctSortedStreamTransform::buildFilterForRange(Method& method, IColumnFilter& filter, const size_t range_begin,
  ...
  for (size_t i = range_begin; i < range_end; ++i) {
    ...
    if (emplace_result.isInserted()) ++count;
  }
  return count;
}
...
bool DistinctSortedStreamTransform::isLatestKeyFromPrevChunk(const size_t row_pos) const {
  for (size_t i = 0, s = sorted_columns.size(); i < s; ++i) {
    ...
    if (unlikely(sorted_column.size() <= row_pos))
      throw Exception(ErrorCodes::LOGICAL_ERROR, "Unexpected size of a sorted column: size {}, row_pos {}, column position {}, type {}",
                      sorted_column.size(), row_pos, i, sorted_column.getFamilyName());
...
std::pair<size_t, size_t> DistinctSortedStreamTransform::continueWithPrevRange(const size_t chunk_rows, IColumnFilter& filter) {
  ...
  if (prev_chunk_latest_key.empty() || !isLatestKeyFromPrevChunk(0)) return {0, 0};
...
void DistinctSortedStreamTransform::transform(Chunk& chunk) {
  ...
  if (unlikely(0 == chunk_rows)) return;
  ...
  for (auto& input_column : input_columns) input_column = input_column->filter(filter, output_rows);
