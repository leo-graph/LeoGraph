...
static void calcSortPrefixPositionsInHeader(const Block& header, const SortDescription& sort_description,
                                            const ColumnNumbers& column_positions, const ColumnNumbers& const_column_positions,
  ...
  for (const auto& column_sort_descr : sort_description) {
    ...
    if (!header.has(column_sort_descr.column_name)) break;
    ...
    if (std::find(begin(column_positions), end(column_positions), pos) == column_positions.end()) {
      ...
      if (std::find(begin(const_column_positions), end(const_column_positions), pos) != const_column_positions.end()) continue;
...
DistinctSortedTransform::DistinctSortedTransform(SharedHeader header, const SortDescription& sort_description,
                                                 const SizeLimits& set_size_limits_, const UInt64 limit_hint_, const Names& column_names)
    : ISimpleTransform(header, header, true), limit_hint(limit_hint_), set_size_limits(set_size_limits_) {
  if (sort_description.empty()) throw Exception(ErrorCodes::LOGICAL_ERROR, "DistinctSortedTransform: sort description can't be empty");
  ...
  if (sort_prefix_positions.empty())
    throw Exception(ErrorCodes::LOGICAL_ERROR, "DistinctSortedTransform: columns have to form a sort prefix for provided sort description");
...
void DistinctSortedTransform::transform(Chunk& chunk) {
  if (unlikely(!chunk.hasRows())) return;
  ...
  if (data.type == ClearableSetVariants::Type::EMPTY) data.init(ClearableSetVariants::chooseMethod(column_ptrs, key_sizes));
...
#undef M
  ...
  if (limit_hint && data_total_row_count >= limit_hint) stopReading();
  ...
  Chunk res_chunk;
  for (size_t i = 0; i < all_columns; ++i) res_chunk.addColumn(prev_chunk.chunk.getColumns().at(i)->filter(filter, -1));

  chunk = std::move(res_chunk);
}

template <typename Method>
bool DistinctSortedTransform::buildFilter(Method& method, const ColumnRawPtrs& columns, const ColumnRawPtrs& clearing_hint_columns,
                                          IColumn::Filter& filter, const size_t rows, ClearableSetVariants& variants) const {
  ...
  {
    if (!rowsEqual(clearing_hint_columns, 0, prev_chunk.clearing_hint_columns, prev_chunk.chunk.getNumRows() - 1)) method.data.clear();
  }
  ...
  for (size_t i = 1; i < rows; ++i) {
    ...
    /// If rows are not equal, we can clear HashSet
    if (!rowsEqual(clearing_hint_columns, i, clearing_hint_columns, i - 1)) method.data.clear();
