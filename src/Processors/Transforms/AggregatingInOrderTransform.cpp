...
AggregatingInOrderTransform::AggregatingInOrderTransform(SharedHeader header, AggregatingTransformParamsPtr params_,
                                                         const SortDescription &sort_description_for_merging,
                                                         const SortDescription &group_by_description_, size_t max_block_size_,
                                                         size_t max_block_bytes_)
    : AggregatingInOrderTransform(std::move(header), std::move(params_), sort_description_for_merging, group_by_description_,
                                  max_block_size_, max_block_bytes_, std::make_unique<ManyAggregatedData>(1), 0) {}

AggregatingInOrderTransform::AggregatingInOrderTransform(SharedHeader header, AggregatingTransformParamsPtr params_,
                                                         const SortDescription &sort_description_for_merging,
                                                         const SortDescription &group_by_description_, size_t max_block_size_,
                                                         size_t max_block_bytes_, ManyAggregatedDataPtr many_data_, size_t current_variant)
...
static Int64 getCurrentMemoryUsage() {
  ...
  if (auto *memory_tracker = CurrentThread::getMemoryTracker()) current_memory_usage = memory_tracker->get();
...
void AggregatingInOrderTransform::consume(Chunk chunk) {
  ...
  for (size_t i = 0; i < params->params.keys_size; ++i) {
    ...
    if (group_by_key) key_columns_raw[i] = materialized_columns.back().get();
  }
  ...
  if (!params->params.only_merge) {
    params->aggregator.prepareAggregateInstructions(columns, aggregate_columns, materialized_columns, aggregate_function_instructions,
  ...
  if (!cur_block_size) {
    ...
    for (size_t i = 0; i < params->params.keys_size; ++i) res_key_columns[i] = res_header.safeGetByPosition(i).type->createColumn();
  ...
  while (key_end != rows) {
    ...
    auto it = std::upper_bound(indices.begin(), indices.end(), cur_block_size - 1, [&](size_t lhs_row, size_t rhs_row) {
    ...
    if (key_end != rows) {
      if (!group_by_key) params->aggregator.addSingleKeyToAggregateColumns(variants, res_aggregate_columns);
      ...
      if (cur_block_size >= max_block_size || cur_block_bytes + current_memory_usage >= max_block_bytes) {
        if (group_by_key)
          group_by_block = params->aggregator.prepareBlockAndFillSingleLevel</* return_single_block */ true>(variants, /* final= */ false);
        cur_block_bytes += current_memory_usage;
        finalizeCurrentChunk(std::move(chunk), key_end);
        if (rows_before_aggregation) rows_before_aggregation->add(key_end);
        return;
      }
  ...
  if (rows_before_aggregation) rows_before_aggregation->add(rows);
...
void AggregatingInOrderTransform::finalizeCurrentChunk(Chunk chunk, size_t key_end) {
  ...
  for (auto &source_column : source_columns) source_column = source_column->cut(key_end, rows - key_end);
...
void AggregatingInOrderTransform::generate() {
  ...
  if (!group_by_key || group_by_key_needs_empty_block) {
    ...
    for (size_t i = 0; i < res_key_columns.size(); ++i) res.getByPosition(i).column = std::move(res_key_columns[i]);
  ...
  if (!to_push_chunk.getNumRows()) return;
