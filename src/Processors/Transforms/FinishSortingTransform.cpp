...
static bool isPrefix(const SortDescription& pref_descr, const SortDescription& descr) {
  if (pref_descr.size() > descr.size()) return false;

  for (size_t i = 0; i < pref_descr.size(); ++i)
    if (pref_descr[i] != descr[i]) return false;

  return true;
}

FinishSortingTransform::FinishSortingTransform(SharedHeader header, const SortDescription& description_sorted_,
                                               const SortDescription& description_to_sort_, size_t max_merged_block_size_, UInt64 limit_,
  ...
  for (const auto& column_description : description_sorted_) {
    ...
    if (pos < num_columns && !const_columns_to_remove[pos]) description_sorted_without_constants.push_back(column_description);
  }
...
void FinishSortingTransform::generate() {
  ...
  if (!generated_chunk) {
    merge_sorter.reset();
    if (tail_chunk) chunks.push_back(std::move(tail_chunk));
    stage = Stage::Consume;
  } else
