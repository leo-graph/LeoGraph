...
static bool isTTLExpired(time_t ttl, time_t current_time) { return ttl && (ttl <= current_time); }

static TTLExpressions buildTTLExpressions(const TTLDescription &ttl_descr, PreparedSets::Subqueries &subqueries_for_sets,
...
std::pair<std::shared_ptr<const TTLDeleteFilterTransform::SharedState>, PreparedSets::Subqueries> TTLDeleteFilterTransform::build(
    const ContextPtr &context, const StorageMetadataPtr &metadata_snapshot, const IMergeTreeDataPart::TTLInfos &old_ttl_infos,
    time_t current_time, bool force) {
  ...
  if (metadata_snapshot->hasRowsTTL()) {
    ...
    if (force || isTTLExpired(old_ttl_infos.table_ttl.min, current_time)) {
      ...
      if (isTTLExpired(old_ttl_infos.table_ttl.max, current_time) && !rows_ttl.where_expression_ast) state->all_data_dropped = true;
  ...
  if (!state->all_data_dropped) {
    for (const auto &where_ttl : metadata_snapshot->getRowsWhereTTLs()) {
      ...
      if (it != old_ttl_infos.rows_where_ttl.end()) old_ttl_info = it->second;

      if (!force && !isTTLExpired(old_ttl_info.min, current_time)) continue;
...
TTLDeleteFilterTransform::TTLDeleteFilterTransform(const SharedHeader &header_, std::shared_ptr<const SharedState> shared_state_)
...
void TTLDeleteFilterTransform::extractTimestamps(const IColumn *ttl_column, size_t num_rows) {
  ...
  if (const auto *col_date = typeid_cast<const ColumnUInt16 *>(ttl_column)) {
    ...
    for (size_t i = 0; i < num_rows; ++i) timestamps[i] = date_lut.fromDayNum(DayNum(data[i]));
  } else if (const auto *col_datetime = typeid_cast<const ColumnUInt32 *>(ttl_column)) {
    const auto &data = col_datetime->getData();
    for (size_t i = 0; i < num_rows; ++i) timestamps[i] = static_cast<Int64>(data[i]);
  } else if (const auto *col_date32 = typeid_cast<const ColumnInt32 *>(ttl_column)) {
    const auto &data = col_date32->getData();
    for (size_t i = 0; i < num_rows; ++i) timestamps[i] = date_lut.fromDayNum(ExtendedDayNum(data[i]));
    ...
    for (size_t i = 0; i < num_rows; ++i) timestamps[i] = data[i] / scale;
...
void TTLDeleteFilterTransform::transform(Chunk &chunk) {
  ...
  for (const auto &entry : shared_state->entries) {
    ...
    auto ttl_column = ITTLAlgorithm::executeExpressionAndGetColumn(entry.expressions.expression, block, entry.description.result_column);
    extractTimestamps(ttl_column.get(), num_rows);

    /// Phase 2: apply TTL expiration and WHERE filter to produce the filter mask.
    auto where_column =
        ITTLAlgorithm::executeExpressionAndGetColumn(entry.expressions.where_expression, block, entry.description.where_result_column);

    for (size_t i = 0; i < num_rows; ++i) {
      if (!filter_vec[i]) continue;
