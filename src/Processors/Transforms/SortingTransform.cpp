...
MergeSorter::MergeSorter(SharedHeader header, Chunks chunks_, SortDescription& description_, size_t max_merged_block_size_, UInt64 limit_)
  ...
  for (size_t chunk_index = 0; chunk_index < chunks_size; ++chunk_index) {
    ...
    if (chunk.getNumRows() == 0) continue;
...
Chunk MergeSorter::read() {
  ...
  Chunk result = queue_variants.callOnBatchVariant([&](auto& queue) { return mergeBatchImpl(queue); });
...
Chunk MergeSorter::mergeBatchImpl(TSortingQueue& queue) {
  ...
  while (queue.isValid()) {
    ...
    if (merged_rows + batch_size > max_merged_block_size) batch_size -= merged_rows + batch_size - max_merged_block_size;
    ...
    if (merged_rows >= max_merged_block_size) break;
  }

  if (!queue.isValid()) chunks.clear();

  if (merged_rows == 0) return {};
...
MutableColumns MergeSorter::createMergedColumns() const {
  ...
  for (const auto& chunk : chunks) {
    for (size_t i = 0; i != chunk.getNumColumns(); ++i) is_replicated[i] = is_replicated[i] || chunk.getColumns()[i]->isReplicated();
  }
  MutableColumns merged_columns = chunks[0].cloneEmptyColumns();
  for (size_t i = 0; i != num_columns; ++i) {
    if (is_replicated[i] && !merged_columns[i]->isReplicated()) merged_columns[i] = ColumnReplicated::create(std::move(merged_columns[i]));
  }

  return merged_columns;
}

SortingTransform::SortingTransform(SharedHeader header, const SortDescription& description_, size_t max_merged_block_size_, UInt64 limit_,
                                   bool increase_sort_description_compile_attempts)
    : IProcessor({header}, {header}), description(description_), max_merged_block_size(max_merged_block_size_), limit(limit_) {
  ...
  if (SortQueueVariants(sort_description_types, description).variantSupportJITCompilation())
    compileSortDescriptionIfNeeded(description, sort_description_types,
                                   increase_sort_description_compile_attempts /*increase_compile_attempts*/);
}
...
IProcessor::Status SortingTransform::prepare() {
  ...
  if (!generated_prefix || !chunks.empty()) return Status::Ready;
...
IProcessor::Status SortingTransform::prepareConsume() {
  ...
  if (generated_chunk) output.push(std::move(generated_chunk));
...
IProcessor::Status SortingTransform::prepareSerialize() {
  ...
  if (output.isFinished()) return Status::Finished;

  if (!output.canPush()) return Status::PortFull;

  if (current_chunk) output.push(std::move(current_chunk));

  if (merge_sorter) return Status::Ready;
...
IProcessor::Status SortingTransform::prepareGenerate() {
  ...
  if (merge_sorter) {
    if (!generated_chunk) return Status::Ready;
...
void SortingTransform::work() {
  if (stage == Stage::Consume) consume(std::move(current_chunk));

  if (stage == Stage::Serialize) serialize();

  if (stage == Stage::Generate) generate();
}
...
void SortingTransform::removeConstColumns(Chunk& chunk) {
  ...
  if (num_columns != const_columns_to_remove.size())
    throw Exception(ErrorCodes::LOGICAL_ERROR, "Block has different number of columns with header: {} vs {}", num_columns,
  ...
  for (size_t position = 0; position < num_columns; ++position) {
    if (!const_columns_to_remove[position]) column_without_constants.push_back(std::move(columns[position]));
  }
