...
SaveSubqueryResultToBufferTransform::SaveSubqueryResultToBufferTransform(SharedHeader header_, ChunkBufferPtr chunk_buffer_,
...
void SaveSubqueryResultToBufferTransform::transform(Chunk& chunk) {
  ...
  for (size_t index : columns_to_save_indices) columns_to_save.push_back(chunk.getColumns()[index]);
