...
FilterSortedStreamByRange::FilterSortedStreamByRange(SharedHeader header_, ExpressionActionsPtr expression_, String filter_column_name_,
                                                     bool remove_filter_column_, bool on_totals_)
    : ISimpleTransform(header_,
                       std::make_shared<const Block>(FilterTransform::transformHeader(*header_, &expression_->getActionsDAG(),
  ...
  assertBlocksHaveEqualStructure(*header_, getOutputPort().getHeader(),
...
void FilterSortedStreamByRange::transform(Chunk& chunk) {
  ...
  for (auto row : {static_cast<UInt64>(0), rows_before_filtration - 1}) {
    for (size_t col = 0; col < quick_check_columns.size(); ++col) quick_check_columns[col]->insertFrom(*src_columns[col].get(), row);
  }
  ...
  if (!all_rows_will_pass_filter) filter_transform.transform(chunk);
}
