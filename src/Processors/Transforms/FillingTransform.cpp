...
Block FillingTransform::transformHeader(
    Block header, const SortDescription &sort_description /*, const InterpolateDescription & interpolate_description*/) {
  NameSet sort_keys;
  for (const auto &key : sort_description) sort_keys.insert(key.column_name);
...
template <typename T>
static FillColumnDescription::StepFunction getStepFunction(IntervalKind::Kind kind, Int64 step, const DateLUTImpl &date_lut,
                                                           UInt16 scale = DataTypeDateTime64::default_scale) {
...
#define DECLARE_CASE(NAME)                                                                                                       \
  case IntervalKind::Kind::NAME:                                                                                                 \
    return [step, scale, &date_lut](Field &field, Int64 jumps_count) {                                                           \
      field = Add##NAME##sImpl::execute(static_cast<T>(field.safeGet<T>()), step * jumps_count, date_lut, utc_time_zone, scale); \
    };
...
static FillColumnDescription::StepFunction getStepFunction(const Field &step, const std::optional<IntervalKind> &step_kind,
  ...
  if (step_kind) {
    if (which.isDate() || which.isDate32()) {
      ...
      if (std::abs(avg_seconds) < 86400)
        throw Exception(ErrorCodes::INVALID_WITH_FILL_EXPRESSION, "Value of step is to low ({} seconds). Must be >= 1 day",
                        std::abs(avg_seconds));
    }
...
#define DECLARE_CASE(NAME)                                                                                                   \
  case IntervalKind::Kind::NAME:                                                                                             \
    return [converted_step, &time_zone = date_time64->getTimeZone()](Field &field, Int64 jumps_count) {                      \
      ...
      auto res = Add##NAME##sImpl::execute(field_decimal.getValue(), converted_step * jumps_count, time_zone, utc_time_zone, \
                                           static_cast<UInt16>(field_decimal.getScale()));                                   \
...
#undef DECLARE_CASE
    ...
    return [step](Field &field, Int64 jumps_count) {
      ...
      if (jumps_count != 1) applyVisitor(FieldVisitorScale(jumps_count), shifted_step);
...
static bool tryConvertFields(FillColumnDescription &descr, const DataTypePtr &type) {
  ...
  if (descr.fill_to_type) {
    ...
    if ((which_to.isDateOrDate32() || which_to.isDateTime() || which_to.isDateTime64()) && !descr.fill_to_type->equals(*type)) return false;
  }
  ...
  if (descr.fill_from.getType() > max_type || descr.fill_to.getType() > max_type || descr.fill_step.getType() > max_type ||
      descr.fill_staleness.getType() > max_type)
    return false;

  if (!descr.fill_from.isNull()) descr.fill_from = convertFieldToTypeOrThrow(descr.fill_from, *to_type);
  if (!descr.fill_to.isNull()) descr.fill_to = convertFieldToTypeOrThrow(descr.fill_to, *to_type);
  if (!descr.fill_step.isNull()) descr.fill_step = convertFieldToTypeOrThrow(descr.fill_step, *to_type);
  if (!descr.fill_staleness.isNull()) descr.fill_staleness = convertFieldToTypeOrThrow(descr.fill_staleness, *to_type);
...
SortDescription deduplicateSortDescription(const SortDescription &sort_description, const Block &header) {
  ...
  for (const auto &desc : sort_description) {
    if (header.findByName(desc.column_name) == nullptr) continue;
...
FillingTransform::FillingTransform(SharedHeader header_, const SortDescription &sort_description_, const SortDescription &fill_description_,
                                   InterpolateDescriptionPtr interpolate_description_, const bool use_with_fill_by_sorting_prefix_)
  ...
  if (interpolate_description) {
    ...
    for (const auto &description : sort_description_)
      if (interpolate_description->result_columns_set.contains(description.alias))
        throw Exception(ErrorCodes::INVALID_WITH_FILL_EXPRESSION,
                        "Column '{}' is participating in ORDER BY expression and can't be INTERPOLATE output", description.alias);
  }
  ...
  for (size_t i = 0, size = fill_description.size(); i < size; ++i) {
    ...
    if (!tryConvertFields(descr, type))
      throw Exception(ErrorCodes::INVALID_WITH_FILL_EXPRESSION, "Incompatible types of WITH FILL expression values with column type {}",
                      type->getName());

    if (isUInt(type) && ((!descr.fill_from.isNull() && less(descr.fill_from, Field{0}, 1)) ||
                         (!descr.fill_to.isNull() && less(descr.fill_to, Field{0}, 1)))) {
      throw Exception(ErrorCodes::INVALID_WITH_FILL_EXPRESSION, "WITH FILL bound values cannot be negative for unsigned type {}",
                      type->getName());
    }
  }
  ...
  for (const auto &desc : sort_description) {
    if (!desc.with_fill) ordinary_sort_positions.insert(header_->getPositionByName(desc.column_name));
  }
  ...
  for (auto pos : fill_column_positions) {
    if (!unique_positions.insert(pos).second)
      throw Exception(ErrorCodes::INVALID_WITH_FILL_EXPRESSION,
                      "Multiple WITH FILL for identical expressions is not supported in ORDER BY");
    if (ordinary_sort_positions.contains(pos))
      throw Exception(ErrorCodes::INVALID_WITH_FILL_EXPRESSION,
                      "ORDER BY containing the same expression with and without WITH FILL modifier is not supported");
  }

  if (use_with_fill_by_sorting_prefix) {
    /// build sorting prefix for first fill column
    for (const auto &desc : sort_description) {
      if (desc.column_name == fill_description[0].column_name) break;
...
void FillingTransform::insertFromFillingRow(const MutableColumnRawPtrs &filling_columns, const MutableColumnRawPtrs &interpolate_columns,
                                            const MutableColumnRawPtrs &other_columns, const Block &interpolate_block) {
  ...
  if (size_t size = interpolate_block.columns()) {
    ...
    for (size_t i = 0; i < size; ++i) interpolate_columns[i]->insertFrom(*columns[i]->convertToFullColumnIfConst(), 0);
  } else {
    for (auto *interpolate_column : interpolate_columns) interpolate_column->insertDefault();
  }

  for (auto *other_column : other_columns) other_column->insertDefault();
...
static void copyRowFromColumns(const MutableColumnRawPtrs &dest, const Columns &source, size_t row_num) {
  ...
  for (size_t i = 0, size = source.size(); i < size; ++i) dest[i]->insertFrom(*source[i], row_num);
}

static void initColumnsByPositions(const Columns &input_columns, Columns &input_columns_by_positions, const MutableColumns &output_columns,
                                   MutableColumnRawPtrs &output_columns_by_position, const std::vector<size_t> &positions) {
...
void FillingTransform::initColumns(const Columns &input_columns, Columns &input_fill_columns, Columns &input_interpolate_columns,
                                   Columns &input_sort_prefix_columns, Columns &input_other_columns, MutableColumns &output_columns,
                                   MutableColumnRawPtrs &output_fill_columns, MutableColumnRawPtrs &output_interpolate_columns,
                                   MutableColumnRawPtrs &output_sort_prefix_columns, MutableColumnRawPtrs &output_other_columns) {
  ...
  for (const auto &column : non_const_columns) output_columns.push_back(column->cloneEmpty());
...
bool FillingTransform::generateSuffixIfNeeded(const Columns &input_columns, MutableColumns &result_columns) {
  ...
  initColumns(input_columns, input_fill_columns, input_interpolate_columns, input_sort_prefix_columns, input_other_columns, result_columns,
              res_fill_columns, res_interpolate_columns, res_sort_prefix_columns, res_other_columns);
...
bool FillingTransform::generateSuffixIfNeeded(const MutableColumns &result_columns, MutableColumnRawPtrs res_fill_columns,
                                              MutableColumnRawPtrs res_interpolate_columns, MutableColumnRawPtrs res_sort_prefix_columns,
                                              MutableColumnRawPtrs res_other_columns) {
  ...
  for (size_t i = 0, size = filling_row.size(); i < size; ++i) next_row[i] = Field{};
  ...
  if (should_insert_first) {
    ...
    if (!last_range_sort_prefix.empty())
      ...
      for (auto *sort_prefix_column : res_sort_prefix_columns) sort_prefix_column->insertDefault();
  }

  bool filling_row_changed = false;
  size_t rows_since_last_cancel_check = 0;
  while (true) {
    if (!filling_row.next(next_row, filling_row_changed)) break;
  ...
  if (filling_row_changed) filling_row_inserted = false;
...
void FillingTransform::transformRange(const Columns &input_fill_columns, const Columns &input_interpolate_columns,
                                      const Columns &input_sort_prefix_columns, const Columns &input_other_columns,
                                      const MutableColumns &result_columns, const MutableColumnRawPtrs &res_fill_columns,
                                      const MutableColumnRawPtrs &res_interpolate_columns,
                                      const MutableColumnRawPtrs &res_sort_prefix_columns, const MutableColumnRawPtrs &res_other_columns,
                                      std::pair<size_t, size_t> range, const bool new_sorting_prefix) {
  ...
  if (new_sorting_prefix) filling_row.updateConstraintsWithStalenessRow(input_fill_columns, range_begin);
  ...
  for (size_t row_ind = range_begin; row_ind < range_end; ++row_ind) {
    ...
    for (size_t i = 0, size = filling_row.size(); i < size; ++i) next_row[i] = (*input_fill_columns[i])[row_ind];
...
void FillingTransform::transform(Chunk &chunk) {
  ...
  if (!chunk.hasRows() && !all_chunks_processed) return;
  ...
  if (sort_prefix.empty() || !use_with_fill_by_sorting_prefix) {
    transformRange(input_fill_columns, input_interpolate_columns, input_sort_prefix_columns, input_other_columns, result_columns,
                   res_fill_columns, res_interpolate_columns, res_sort_prefix_columns, res_other_columns, {0, num_rows}, last_row.empty());
  ...
  if (!last_row.empty()) {
    ...
    for (size_t pos : sort_prefix_positions) last_sort_prefix_columns.push_back(last_row[pos].get());
  ...
  for (size_t row_ind = 0; row_ind < num_rows;) {
    /// find next range
    auto current_sort_prefix_end_pos = getRangeEnd(row_ind, num_rows, [&](size_t pos_with_current_sort_prefix, size_t row_pos) {
      for (size_t i = 0; i < input_sort_prefix_columns.size(); ++i) {
        const int res = input_sort_prefix_columns[i]->compareAt(pos_with_current_sort_prefix, row_pos, *input_sort_prefix_columns[i],
                                                                sort_prefix[i].nulls_direction);
    ...
    transformRange(input_fill_columns, input_interpolate_columns, input_sort_prefix_columns, input_other_columns, result_columns,
                   res_fill_columns, res_interpolate_columns, res_sort_prefix_columns, res_other_columns,
                   {row_ind, current_sort_prefix_end_pos}, new_sort_prefix);
