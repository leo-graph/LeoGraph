...
LimitByTransform::LimitByTransform(SharedHeader header, UInt64 group_length_, UInt64 group_offset_, bool in_order_, const Names& columns)
    : ISimpleTransform(header, header, true), group_length(group_length_), group_offset(group_offset_), in_order(in_order_) {
  ...
  for (const auto& name : columns) {
    ...
    if (!(column && isColumnConst(*column))) key_positions.emplace_back(position);
  }
}

String LimitByTransform::getName() const { return fmt::format("LimitByTransform{}", in_order ? " (InOrder)" : ""); }
...
void LimitByTransform::transformCommon(Chunk& chunk) {
  ...
  for (UInt64 row = 0; row < num_rows; ++row) {
    SipHash hash;
    for (auto position : key_positions) columns[position]->updateHashWithValue(row, hash);
...
void LimitByTransform::finalizeChunk(Chunk& chunk, Columns&& columns, const IColumn::Filter& filter, UInt64 num_rows,
                                     UInt64 inserted_count) {
  ...
  if (rows_before_limit_at_least) rows_before_limit_at_least->add(inserted_count);
