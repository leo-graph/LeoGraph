...
void applyPermutation(std::vector<T>& data, const std::vector<size_t>& permutation) {
  ...
  for (size_t i : permutation) res.push_back(data[i]);
  data = std::move(res);
}
...
ColumnPermuteTransform::ColumnPermuteTransform(SharedHeader header_, const std::vector<size_t>& permutation_)
    : ISimpleTransform(header_, std::make_shared<const Block>(permute(*header_, permutation_)), false), permutation(permutation_) {}

void ColumnPermuteTransform::transform(Chunk& chunk) { permuteChunk(chunk, permutation); }
