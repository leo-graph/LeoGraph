...
IProcessor::Status ExceptionKeepingTransform::prepare() {
  ...
  if (output.isFinished()) throw Exception(ErrorCodes::LOGICAL_ERROR, "Output port is finished for {}", getName());
  ...
  if (stage == Stage::Generate) return Status::Ready;

  while (!ready_input) {
    if (input.isFinished()) {
      if (stage != Stage::Exception && stage != Stage::Finish) {
        stage = Stage::Finish;
        if (!ignore_on_start_and_finish) return Status::Ready;
      }
  ...
  if (stage == Stage::Start) {
    ...
    if (stage == Stage::Generate) {
      ...
      if (auto exception = runStep([this, &res] { res = onGenerate(); }, thread_group)) {
        ...
        if (res.is_done) stage = Stage::Consume;
      }
    }
...
void ExceptionKeepingTransform::setRuntimeData(ThreadGroupPtr thread_group_) { thread_group = thread_group_; }
