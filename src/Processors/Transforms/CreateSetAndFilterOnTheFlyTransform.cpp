...
std::vector<size_t> getColumnIndices(const Block &block, const Names &column_names) {
  ...
  for (const auto &name : column_names) indices.push_back(block.getPositionByName(name));
  return indices;
}
...
CreatingSetsOnTheFlyTransform::CreatingSetsOnTheFlyTransform(SharedHeader header_, const Names &column_names_, size_t num_streams_,
...
IProcessor::Status CreatingSetsOnTheFlyTransform::prepare() {
  ...
  if (set->state == SetWithState::State::Creating) {
    if (input.isFinished()) {
      ...
      LOG_DEBUG(log, "{}: finish building set for [{}] with {} rows, set size is {}", getDescription(), fmt::join(column_names, ", "),
                set->getTotalRowCount(), formatReadableSizeWithBinarySuffix(set->getTotalByteCount()));
      set.reset();
    } else {
...
FilterBySetOnTheFlyTransform::FilterBySetOnTheFlyTransform(SharedHeader header_, const Names &column_names_, SetWithStatePtr set_)
  ...
  for (size_t idx : key_column_indices) key_sample_block.insert(header.getByPosition(idx));
}

IProcessor::Status FilterBySetOnTheFlyTransform::prepare() {
  auto status = ISimpleTransform::prepare();

  if (set && set->state == SetWithState::State::Suspended) set.reset();

  if (status == Status::Finished) {
    bool has_filter = set && set->state == SetWithState::State::Finished;
    if (has_filter) {
      LOG_DEBUG(
          log, "Finished {} by [{}]: consumed {} rows in total, {} rows bypassed, result {} rows, {:.2f}% filtered",
          Poco::toLower(getDescription()), fmt::join(column_names, ", "), stat.consumed_rows, stat.consumed_rows_before_set,
          stat.result_rows,
...
void FilterBySetOnTheFlyTransform::transform(Chunk &chunk) {
  ...
  if (!can_filter) stat.consumed_rows_before_set += chunk.getNumRows();
