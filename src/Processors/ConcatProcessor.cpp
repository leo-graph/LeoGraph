...
ConcatProcessor::Status ConcatProcessor::prepare() {
  ...
  if (output.isFinished()) {
    for (; current_input != inputs.end(); ++current_input) current_input->close();

    return Status::Finished;
  }

  if (!output.isNeeded()) {
    if (current_input != inputs.end()) current_input->setNotNeeded();
  ...
  while (current_input != inputs.end() && current_input->isFinished()) ++current_input;
