...
NegativeOffsetTransform::NegativeOffsetTransform(const Block& header_, UInt64 offset_, size_t num_streams)
    : IProcessor(InputPorts(num_streams, header_), OutputPorts(num_streams, header_)), offset(offset_) {
  ...
  if (stage == Stage::Pull) {
    ...
    auto process = [&](size_t pos) {
      ...
      switch (status) {
        ...
        default:
          throw Exception(ErrorCodes::LOGICAL_ERROR, "Unexpected status in NegativeOffsetTransform::advancePort : {}",
...
NegativeOffsetTransform::Status NegativeOffsetTransform::advancePort(PortsData& data) {
  ...
  if (push_status != Status::Finished) return push_status;
