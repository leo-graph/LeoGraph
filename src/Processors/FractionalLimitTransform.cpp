...
void FractionalLimitTransform::finalizeLimits() {
  ...
  if (with_ties && rows_processed < limit_rows + offset_rows) ties_last_row = {};

  limits_are_final = true;
}

FractionalLimitTransform::FractionalLimitTransform(SharedHeader header_, Float64 limit_fraction_, Float64 offset_fraction_, UInt64 offset_,
                                                   size_t num_streams, bool with_ties_, SortDescription limit_with_ties_sort_description_)
  ...
  for (const auto& desc : limit_with_ties_sort_description) sort_key_positions.push_back(header_->getPositionByName(desc.column_name));
}
...
FractionalLimitTransform::Status FractionalLimitTransform::prepare() {
  if (allOutputsFinished()) {
    ...
    for (auto& port : ports_data) port.input_port->close();
  ...
  if (num_finished_input_ports != ports_data.size()) {
    auto process_port = [&](size_t port_idx) {
      ...
      switch (status) {
        ...
        case IProcessor::Status::NeedData:
          return;
        default:
          throw Exception(ErrorCodes::LOGICAL_ERROR, "Unexpected status for FractionalLimitTransform::pullData : {}",
                          IProcessor::statusToName(status));
      }
    };

    for (size_t port_idx = 0; port_idx < ports_data.size(); ++port_idx) process_port(port_idx);
    ...
    if (offset_fraction == 0.0) {
      ...
      while (!cached_chunks.empty()) {
        ...
        if (!output) {
          if (num_finished_input_ports == ports_data.size()) finalizeLimits();
        ...
        if (rows_processed < offset_rows) offset_rows_remaining = offset_rows - rows_processed;
        ...
        const UInt64 limit_rows_for_current_input = static_cast<UInt64>(std::ceil(static_cast<double>(total_input_rows) * limit_fraction));
        if (limit_rows_for_current_input + offset_rows_remaining < chunk_rows + early_pushed_rows) break;
        ...
        if (offset_rows_remaining) {
          ...
          if (offset_rows_remaining >= chunk_rows)
            throw Exception(ErrorCodes::LOGICAL_ERROR, "Remaining offset ({}) must be less than chunk rows ({})", offset_rows_remaining,
                            chunk_rows);
          ...
          for (UInt64 i = 0; i < num_columns; ++i) columns[i] = columns[i]->cut(offset_rows_remaining, rows_after_offset);
...
bool FractionalLimitTransform::allOutputsFinished() const {
  for (const auto& data : ports_data)
    if (!data.output_port->isFinished()) return false;
  return true;
}
...
FractionalLimitTransform::Status FractionalLimitTransform::pullData(PortsData& data) {
  ...
  /// Check can input?
  if (input.isFinished()) return Status::Finished;
  ...
  if (rows_before_limit_at_least) rows_before_limit_at_least->add(chunk_rows);
...
FractionalLimitTransform::Status FractionalLimitTransform::pushData() {
  ...
  while (!cached_chunks.empty()) {
    ...
    if (is_limit_reached) return Status::Finished;

    auto* output = getAvailableOutputPort();
    if (!output) return Status::PortFull;
    ...
    if (chunk_rows <= std::numeric_limits<UInt64>::max() - offset_rows && rows_processed >= offset_rows + chunk_rows &&
      ...
      if (with_ties && rows_processed == offset_rows + limit_rows) ties_last_row = makeChunkWithPreviousRow(chunk, chunk_rows - 1);
    } else
...
void FractionalLimitTransform::splitChunk(Chunk& current_chunk) {
  ...
  if (ties_last_row && rows_processed >= offset_rows + limit_rows) {
    ...
    for (; current_row_num < chunk_rows; ++current_row_num) {
      if (!sortColumnsEqualAt(current_chunk_sort_columns, current_row_num)) break;
    }
    ...
    if (current_row_num < chunk_rows) {
      ties_last_row = {};
      for (UInt64 i = 0; i < num_columns; ++i) columns[i] = columns[i]->cut(0, current_row_num);
    }
  ...
  if (offset_rows + chunk_rows > rows_processed) cut_start = offset_rows + chunk_rows - rows_processed;
  ...
  if (cut_length == chunk_rows) return;

  auto columns = current_chunk.detachColumns();

  for (UInt64 i = 0; i < num_columns; ++i) columns[i] = columns[i]->cut(cut_start, cut_length);
...
ColumnRawPtrs FractionalLimitTransform::extractSortColumns(const Columns& columns) const {
  ...
  for (size_t pos : sort_key_positions) res.push_back(columns[pos].get());
...
bool FractionalLimitTransform::sortColumnsEqualAt(const ColumnRawPtrs& current_chunk_sort_columns, UInt64 current_chunk_row_num) const {
  ...
  for (size_t i = 0; i < num_sort_columns; ++i)
    if (0 != current_chunk_sort_columns[i]->compareAt(current_chunk_row_num, 0, *ties_last_row_sort_columns[i], 1)) return false;
  return true;
}
