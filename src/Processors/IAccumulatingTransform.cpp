...
IAccumulatingTransform::Status IAccumulatingTransform::prepare() {
  ...
  /// Output if has data.
  if (current_output_chunk) output.push(std::move(current_output_chunk));
  ...
  if (input.isFinished()) finished_input = true;
...
void IAccumulatingTransform::work() {
  if (!finished_input) {
    ...
    if (!current_output_chunk) finished_generate = true;
  }
}
