...
class PushingAsyncSource : public ISource {
 public:
  explicit PushingAsyncSource(SharedHeader header) : ISource(header) {}
...
struct PushingAsyncPipelineExecutor::Data {
  ...
  void rethrowExceptionIfHas() {
    if (has_exception.exchange(false)) std::rethrow_exception(exception);
  }
};
...
static void threadFunction(PushingAsyncPipelineExecutor::Data& data, ThreadGroupPtr thread_group, size_t num_threads,
  ...
  if (data.source) data.source->finish();
...
const Block& PushingAsyncPipelineExecutor::getHeader() const { return pushing_source->getPort().getHeader(); }
...
void PushingAsyncPipelineExecutor::push(Chunk chunk) {
  ...
  if (!is_pushed) throwOnExecutionStatus(data->executor->getExecutionStatus());
}

void PushingAsyncPipelineExecutor::push(Block block) { push(Chunk(block.getColumns(), block.rows())); }
...
void PushingAsyncPipelineExecutor::finish() {
  ...
  if (data && data->thread.joinable()) data->thread.join();

  /// Rethrow exception to not swallow it in destructor.
  if (data) data->rethrowExceptionIfHas();
}

void PushingAsyncPipelineExecutor::cancel() {
  /// Cancel execution if it wasn't finished.
  if (data && !data->is_finished && data->executor) data->executor->cancel();
