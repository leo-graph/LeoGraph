...
CompletedPipelineExecutor::CompletedPipelineExecutor(QueryPipeline& pipeline_) : pipeline(pipeline_) {
  if (!pipeline.completed()) throw Exception(ErrorCodes::LOGICAL_ERROR, "Pipeline for CompletedPipelineExecutor must be completed");
}
...
void CompletedPipelineExecutor::execute() {
  if (interactive_timeout_ms) {
    ...
    while (!data->is_finished) {
      if (data->finish_event.tryWait(interactive_timeout_ms)) break;
    ...
    if (data->has_exception) std::rethrow_exception(data->exception);
  } else {
