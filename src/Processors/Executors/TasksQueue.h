...
class TaskQueue {
 public:
  ...
  size_t getAnyThreadWithTasks(size_t from_thread) {
    if (num_tasks == 0) throw Exception(ErrorCodes::LOGICAL_ERROR, "TaskQueue is empty");

    if (from_thread >= use_queues) from_thread = 0;

    for (size_t i = 0, max_iterations = use_queues; i < max_iterations; ++i) {
      if (!queues[from_thread].empty()) return from_thread;
