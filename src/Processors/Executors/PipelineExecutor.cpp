...
PipelineExecutor::PipelineExecutor(std::shared_ptr<Processors> &processors, QueryStatusPtr elem) : process_list_element(std::move(elem)) {
...
PipelineExecutor::~PipelineExecutor() {
  if (process_list_element) process_list_element->removePipelineExecutor(this);
}

const Processors &PipelineExecutor::getProcessors() const { return graph->getProcessors(); }

void PipelineExecutor::cancel(ExecutionStatus reason) {
  /// It is allowed to cancel not started query by user.
  if (reason == ExecutionStatus::CancelledByUser) tryUpdateExecutionStatus(ExecutionStatus::NotStarted, reason);
...
void PipelineExecutor::finish() { tasks.finish(); }
...
void PipelineExecutor::execute(size_t num_threads, bool concurrency_control) {
  ...
  try {
    ...
    for (auto &node : graph->nodes)
      if (node->exception && getExceptionErrorCode(node->exception) == ErrorCodes::LOGICAL_ERROR) tryLogException(node->exception, log);

    /// Rethrow the first exception.
    for (auto &node : graph->nodes)
      if (node->exception) std::rethrow_exception(node->exception);
...
bool PipelineExecutor::executeStep(std::atomic_bool *yield_flag) {
  if (!is_execution_initialized) {
    ...
    if (yield_flag && *yield_flag) return true;
  }

  executeStepImpl(0, single_thread_cpu_slot.get(), yield_flag);

  if (!tasks.isFinished()) return true;
...
bool PipelineExecutor::checkTimeLimitSoft() {
  if (process_list_element) {
    ...
    if (!continuing) cancel(ExecutionStatus::CancelledByTimeout);
...
bool PipelineExecutor::checkTimeLimit() {
  ...
  if (!continuing) process_list_element->checkTimeLimit();  // Will throw if needed

  return continuing;
}

void PipelineExecutor::setReadProgressCallback(ReadProgressCallbackPtr callback) { read_progress_callback = std::move(callback); }
...
void PipelineExecutor::finalizeExecution() {
  ...
  if (status == ExecutionStatus::CancelledByTimeout || status == ExecutionStatus::CancelledByUser) return;
  ...
  for (auto &node : graph->nodes) {
    ...
    if (node->processor && read_progress_callback) {
      ...
      if (auto read_progress = node->processor->getReadProgress()) {
        ...
        if (read_progress->counters.total_bytes) read_progress_callback->addTotalBytes(read_progress->counters.total_bytes);
...
void PipelineExecutor::executeSingleThread(size_t thread_num, IAcquiredSlot *cpu_slot) {
  ...
  LOG_TEST(log, "Thread finished. Total time: {} sec. Execution time: {} sec. Processing time: {} sec. Wait time: {} sec.",
           static_cast<double>(context.total_time_ns) / 1e9, static_cast<double>(context.execution_time_ns) / 1e9,
           static_cast<double>(context.processing_time_ns) / 1e9, static_cast<double>(context.wait_time_ns) / 1e9);
#endif
}
...
struct CPUHelper {
  ...
  explicit CPUHelper(IAcquiredSlot *cpu_slot) : lease(dynamic_cast<ISlotLease *>(cpu_slot)) {
    if (lease) {
...
void PipelineExecutor::executeStepImpl(size_t thread_num, IAcquiredSlot *cpu_slot, std::atomic_bool *yield_flag) {
  ...
  while (!tasks.isFinished() && !yield) {
    ...
    while (!tasks.isFinished() && !context.hasTask()) tasks.tryGetTask(context);

    while (!tasks.isFinished() && context.hasTask() && !yield) {
      if (!context.executeTask()) cancel(ExecutionStatus::Exception);
      ...
      ExecutorTasks::SpawnStatus spawn_status = ExecutorTasks::DO_NOT_SPAWN;
      {
        ...
        if (status == ExecutingGraph::UpdateNodeStatus::Exception) cancel(ExecutionStatus::Exception);

        /// Push other tasks to global queue.
        if (status == ExecutingGraph::UpdateNodeStatus::Done) spawn_status = tasks.pushTasks(queue, async_queue, context);
      }
      ...
      if (yield_flag && *yield_flag) yield = true;
    }
  }
...
SlotAllocationPtr PipelineExecutor::allocateCPU(size_t num_threads, bool concurrency_control) {
  ...
  if (concurrency_control) {
    ...
    if (workload_cpu_scheduling_is_enabled) {
      if (master_thread_link ||
          worker_thread_link)  // Only use resource scheduler if at least one resource link is specified, otherwise unlimited
      {
        ...
        if (query_context->getCPUSlotPreemption()) {
          ...
          return std::make_shared<CPULeaseAllocation>(
              num_threads, master_thread_link, worker_thread_link,
              CPULeaseSettings{
...
void PipelineExecutor::initializeExecution(size_t num_threads, bool concurrency_control) {
  ...
  if (num_threads > 1)
    pool = std::make_unique<ThreadPool>(CurrentMetrics::QueryPipelineExecutorThreads, CurrentMetrics::QueryPipelineExecutorThreadsActive,
                                        CurrentMetrics::QueryPipelineExecutorThreadsScheduled, num_threads);
}

void PipelineExecutor::spawnThreads(AcquiredSlotPtr slot) {
  while (cpu_slots) {
    if (!slot) slot = cpu_slots->tryAcquire();
    if (!slot) return;
    ...
    if (spawn_status == ExecutorTasks::DO_NOT_SPAWN) return;
  }
}
