...
void ExecutorTasks::finish() {
  ...
  for (auto &context : executor_contexts) context->wakeUp();
}
...
void ExecutorTasks::rethrowFirstThreadException() {
  for (auto &executor_context : executor_contexts) executor_context->rethrowExceptionIfHas();
}

ExecutorTasks::SpawnStatus ExecutorTasks::tryWakeUpAnyOtherThreadWithTasks(ExecutionThreadContext &self,
...
ExecutorTasks::SpawnStatus ExecutorTasks::tryWakeUpAnyOtherThreadWithTasksInQueue(ExecutionThreadContext &self,
                                                                                  TaskQueue<ExecutingGraph::Node> &queue,
                                                                                  std::unique_lock<std::mutex> &lock) {
  ...
  if (thread_to_wake >= use_threads) throw Exception(ErrorCodes::LOGICAL_ERROR, "Non-empty queue without allocated thread");
...
void ExecutorTasks::tryGetTask(ExecutionThreadContext &context) {
  {
    ...
    if (!fast_task_queue.empty()) {
      ...
      if (fast_task_queue.empty()) has_fast_tasks = false;
    } else if (!task_queue.empty()) {
...
void ExecutorTasks::init(size_t num_threads_, size_t use_threads_, const SlotAllocationPtr &cpu_slots_, bool profile_processors,
                         bool trace_processors, ReadProgressCallback *callback) {
...
void ExecutorTasks::fill(Queue &queue, [[maybe_unused]] Queue &async_queue) {
  ...
  while (!async_queue.empty()) {
    ...
    if (next_thread >= use_threads) next_thread = 0;
  }
#endif
...
void ExecutorTasks::downscale(size_t slot_id) {
  ...
  if (slot_id >= slot_count.size() || slot_count[slot_id] == 0) return;
  --slot_count[slot_id];
...
void ExecutorTasks::processAsyncTasks() {
#if defined(OS_LINUX)
  {
    ...
    while (auto task = async_task_queue.wait(lock)) {
      ...
      if (fast_task_queue.empty()) has_fast_tasks = true;
...
String ExecutorTasks::dump() {
  ...
  for (size_t i = 0; i < slot_count.size(); ++i) buffer << "  slot_count[" << i << "]: " << slot_count[i] << "\n";
