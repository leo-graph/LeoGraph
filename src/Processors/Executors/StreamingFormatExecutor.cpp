...
StreamingFormatExecutor::StreamingFormatExecutor(const Block &header_, InputFormatPtr format_, ErrorCallback on_error_, size_t total_bytes_,
                                                 size_t total_chunks_, SimpleTransformPtr adding_defaults_transform_)
    : header(header_),
  ...
  for (size_t i = 0; i < result_columns.size(); ++i) checkpoints[i] = result_columns[i]->getCheckpoint();
}
...
void StreamingFormatExecutor::setQueryParameters(const NameToNameMap &parameters) {
  ...
  if (auto *values_format = typeid_cast<ValuesBlockInputFormat *>(format.get())) values_format->setQueryParameters(parameters);
}

void StreamingFormatExecutor::preallocateResultColumns(size_t num_bytes, const Chunk &chunk) {
  if (!try_preallocate) return;
...
size_t StreamingFormatExecutor::execute(size_t num_bytes) {
  for (size_t i = 0; i < result_columns.size(); ++i) result_columns[i]->updateCheckpoint(*checkpoints[i]);
  ...
  {
    format->resetParser();
    auto exception = Exception(ErrorCodes::UNKNOWN_EXCEPTION, "Unknown exception while executing StreamingFormatExecutor with format {}",
                               format->getName());
    return on_error(result_columns, checkpoints, exception);
  }
}
...
size_t StreamingFormatExecutor::insertChunk(Chunk chunk, size_t num_bytes) {
  ...
  if (adding_defaults_transform) adding_defaults_transform->transform(chunk);
