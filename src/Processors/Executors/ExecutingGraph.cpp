...
ExecutingGraph::ExecutingGraph(std::shared_ptr<Processors> processors_, bool profile_processors_)
    : processors(std::move(processors_)), profile_processors(profile_processors_) {
  ...
  /// Create edges.
  for (uint64_t node = 0; node < num_processors; ++node) addEdges(node);
}
...
ExecutingGraph::Edge &ExecutingGraph::addEdge(Edges &edges, Edge edge, const IProcessor *from, const IProcessor *to) {
  ...
  if (it == processors_map.end())
    throw Exception(ErrorCodes::LOGICAL_ERROR, "Processor {} was found as {} for processor {}, but not found in list of processors",
                    to->getName(), edge.backward ? "input" : "output", from->getName());
...
ExecutingGraph::UpdateNodeStatus ExecutingGraph::expandPipeline(boost::container::devector<uint64_t> &stack, uint64_t pid) {
...

  {
    ...
    // Do not consider sources added during pipeline expansion as cancelable to avoid tricky corner cases (e.g.
    // ConvertingAggregatedToChunksWithMergingSource cancellation)
    source_processors.resize(source_processors.size() + new_processors.size(), false);

    if (cancelled) {
      for (auto &processor : new_processors) processor->cancel();
  ...
  for (uint64_t node = 0; node < num_processors; ++node) {
    if (addEdges(node)) updated_nodes.push_back(node);
  }
  ...
  for (auto updated_node : updated_nodes) {
    ...
    for (uint64_t edge = back_edges_sizes[updated_node]; edge < num_back_edges; ++edge) node.updated_input_ports.emplace_back(edge);

    for (uint64_t edge = direct_edge_sizes[updated_node]; edge < num_direct_edges; ++edge) node.updated_output_ports.emplace_back(edge);
...
ExecutingGraph::UpdateNodeStatus ExecutingGraph::updateNode(uint64_t pid, Queue &queue, Queue &async_queue) {
  ...
  while (!updated_processors.empty() || !updated_edges.empty()) {
    ...
    if (!updated_processors.empty()) {
      ...
      if (!stack_top_lock) stack_top_lock.emplace(node.status_mutex);
      ...
      if (need_expand_pipeline) {
        ...
        read_lock.unlock();
        {
          ...
          if (status != UpdateNodeStatus::Done) return status;
        }
        read_lock.lock();
...
void ExecutingGraph::cancel(bool cancel_all_processors) {
...

  {
    ...
    if (cancel_all_processors) cancelled = true;
  }
