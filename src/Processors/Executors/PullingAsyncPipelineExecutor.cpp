...
PullingAsyncPipelineExecutor::PullingAsyncPipelineExecutor(QueryPipeline& pipeline_) : pipeline(pipeline_) {
  if (!pipeline.pulling()) throw Exception(ErrorCodes::LOGICAL_ERROR, "Pipeline for PullingAsyncPipelineExecutor must be pulling");
...
const Block& PullingAsyncPipelineExecutor::getHeader() const { return lazy_format->getPort(IOutputFormat::PortKind::Main).getHeader(); }
...
bool PullingAsyncPipelineExecutor::pull(Block& block, uint64_t milliseconds) {
  ...
  if (!pull(chunk, milliseconds)) return false;
...
void PullingAsyncPipelineExecutor::cancel() {
  ...
  cancelWithExceptionHandling([&]() {
    if (!data->is_finished && data->executor) data->executor->cancel();
  });
  ...
  if (data->thread.joinable()) data->thread.join();
...
void PullingAsyncPipelineExecutor::cancelReading() {
  ...
  cancelWithExceptionHandling([&]() {
    if (!data->is_finished && data->executor) data->executor->cancelReading();
  });
}
...
Chunk PullingAsyncPipelineExecutor::getTotals() { return lazy_format->getTotals(); }

Chunk PullingAsyncPipelineExecutor::getExtremes() { return lazy_format->getExtremes(); }
...
ProfileInfo& PullingAsyncPipelineExecutor::getProfileInfo() { return lazy_format->getProfileInfo(); }
