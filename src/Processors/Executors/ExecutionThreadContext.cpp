...
void ExecutionThreadContext::wait(std::atomic_bool& finished) {
  ...
  condvar.wait(lock, [&] { return finished || wake_flag; });
...
static bool checkCanAddAdditionalInfoToException(const DB::Exception& exception) {
  ...
  return exception.code() != ErrorCodes::TOO_MANY_ROWS_OR_BYTES && exception.code() != ErrorCodes::QUOTA_EXCEEDED &&
         exception.code() != ErrorCodes::QUERY_WAS_CANCELLED && exception.code() != ErrorCodes::QUERY_WAS_CANCELLED_BY_CLIENT;
}
...
static void executeJob(ExecutingGraph::Node* node, ReadProgressCallback* read_progress_callback) {
  try {
    ...
    if (is_source && read_progress_callback) {
      if (auto read_progress = node->processor->getReadProgress()) {
        ...
        if (!read_progress_callback->onProgress(read_progress->counters.read_rows, read_progress->counters.read_bytes,
                                                read_progress->limits))
  ...
  } catch (Exception exception)  /// NOLINT
  {
    ...
    if (checkCanAddAdditionalInfoToException(exception)) exception.addMessage("While executing " + node->processor->getName());
    throw exception;
  }
}
...
bool ExecutionThreadContext::executeTask() {
...
#else
  if (profile_processors) execution_time_watch.emplace();
#endif
  ...
  if (profile_processors) {
    ...
    if (trace_processors) span->addAttribute("execution_time_ms", elapsed_ns / 1000U);
  }
#ifndef NDEBUG
