...
struct ThreadsQueue {
  ...
  void push(size_t thread) {
    if (unlikely(has(thread))) throw Exception(ErrorCodes::LOGICAL_ERROR, "Can't push thread because it is already in threads queue");
  ...
  void pop(size_t thread) {
    if (unlikely(!has(thread))) throw Exception(ErrorCodes::LOGICAL_ERROR, "Can't pop thread because it is not in threads queue");
  ...
  size_t popAny() {
    if (unlikely(stack_size == 0)) throw Exception(ErrorCodes::LOGICAL_ERROR, "Can't pop from empty queue");
