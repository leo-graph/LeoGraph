...
FractionalOffsetStep::FractionalOffsetStep(const SharedHeader &input_header_, Float64 fractional_offset_)
    : ITransformingStep(input_header_, input_header_, getTraits()), fractional_offset(fractional_offset_) {}
...
void FractionalOffsetStep::describeActions(JSONBuilder::JSONMap &map) const { map.add("Fractional Offset", fractional_offset); }

void FractionalOffsetStep::serialize(Serialization &ctx) const { writeFloatBinary(fractional_offset, ctx.out); }
