...
class CommonSubplanReferenceStep : public ISourceStep {
 public:
  explicit CommonSubplanReferenceStep(const SharedHeader &header_, QueryPlan::Node *subplan_root_, ColumnIdentifiers columns_to_use_)
      : ISourceStep(header_), subplan_root(subplan_root_), columns_to_use(std::move(columns_to_use_)) {}
