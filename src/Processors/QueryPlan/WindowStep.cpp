...
WindowStep::WindowStep(const SharedHeader &input_header_, const WindowDescription &window_description_,
                       const std::vector<WindowFunctionDescription> &window_functions_, bool streams_fan_out_)
    : ITransformingStep(input_header_, std::make_shared<const Block>(addWindowFunctionResultColumns(*input_header_, window_functions_)),
                        getTraits(!streams_fan_out_)),
...
void WindowStep::transformPipeline(QueryPipelineBuilder &pipeline, const BuildQueryPipelineSettings &) {
  ...
  // have resized it.
  if (window_description.full_sort_description.empty()) pipeline.resize(1);

  pipeline.addSimpleTransform([&](const SharedHeader & /*header*/) {
    return std::make_shared<WindowTransform>(input_headers.front(), output_header, window_description, window_functions);
  });
  ...
  assertBlocksHaveEqualStructure(pipeline.getHeader(), *output_header, "WindowStep transform for '" + window_description.window_name + "'");
}
...
void WindowStep::describeActions(JSONBuilder::JSONMap &map) const {
  if (!window_description.partition_by.empty()) {
    ...
    for (const auto &descr : window_description.partition_by) partion_columns_array->add(descr.column_name);

    map.add("Partition By", std::move(partion_columns_array));
  }

  if (!window_description.order_by.empty()) map.add("Sort Description", explainSortDescription(window_description.order_by));

  auto functions_array = std::make_unique<JSONBuilder::JSONArray>();
  for (const auto &func : window_functions) functions_array->add(func.column_name);
...
const WindowDescription &WindowStep::getWindowDescription() const { return window_description; }
