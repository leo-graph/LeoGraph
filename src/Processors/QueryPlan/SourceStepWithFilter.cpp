...
Block SourceStepWithFilter::applyPrewhereActions(Block block, const FilterDAGInfoPtr &row_level_filter,
  ...
  if (row_level_filter) {
    ...
    if (row_level_filter->do_remove_column) block.erase(row_level_filter->column_name);
  }
      ...
      if (prewhere_info->remove_prewhere_column) {
        ...
        if (const auto *type = typeid_cast<const DataTypeNullable *>(prewhere_column.type.get()); type && type->onlyNull()) {
          ...
          if (which.isNativeInt() || which.isNativeUInt())
            ...
            throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_COLUMN_FOR_FILTER, "Illegal type {} of column for filter",
                            prewhere_column.type->getName());
        }
      }
    }
...
void SourceStepWithFilter::updatePrewhereInfo(const PrewhereInfoPtr &prewhere_info_value) {
  ...
  output_header =
      std::make_shared<const Block>(applyPrewhereActions(*output_header, query_info.row_level_filter, query_info.prewhere_info));
}
