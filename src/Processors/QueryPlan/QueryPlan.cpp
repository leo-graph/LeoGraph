...
void QueryPlan::checkInitialized() const {
  if (!isInitialized()) throw Exception(ErrorCodes::LOGICAL_ERROR, "QueryPlan was not initialized");
}

void QueryPlan::checkNotCompleted() const {
  if (isCompleted()) throw Exception(ErrorCodes::LOGICAL_ERROR, "QueryPlan was already completed");
}

bool QueryPlan::isCompleted() const { return isInitialized() && !root->step->hasOutputHeader(); }
...
void QueryPlan::unitePlans(QueryPlanStepPtr step, std::vector<std::unique_ptr<QueryPlan>> plans) {
  if (isInitialized()) throw Exception(ErrorCodes::LOGICAL_ERROR, "Cannot unite plans because current QueryPlan is already initialized");
  ...
  if (num_inputs != plans.size())
    throw Exception(ErrorCodes::LOGICAL_ERROR,
                    ...
                    step->getName(), plans.size(), num_inputs);
  ...
  for (size_t i = 0; i < num_inputs; ++i) {
    ...
    if (!blocksHaveEqualStructure(*step_header, *plan_header))
      throw Exception(ErrorCodes::LOGICAL_ERROR,
                      ...
                      step->getName(), plans[i]->root->step->getName(), plan_header->dumpStructure(), step_header->dumpStructure());
  }

  for (auto &plan : plans) nodes.splice(nodes.end(), std::move(plan->nodes));

  nodes.emplace_back(Node{.step = std::move(step)});
  root = &nodes.back();

  for (auto &plan : plans) root->children.emplace_back(plan->root);
...
void QueryPlan::addStep(QueryPlanStepPtr step) {
  ...
  if (num_input_streams == 0) {
    if (isInitialized())
      throw Exception(ErrorCodes::LOGICAL_ERROR,
                      "Cannot add step {} to QueryPlan because step has no inputs, but QueryPlan is already initialized", step->getName());
  ...
  if (num_input_streams == 1) {
    if (!isInitialized())
      throw Exception(ErrorCodes::LOGICAL_ERROR, "Cannot add step {} to QueryPlan because step has input, but QueryPlan is not initialized",
                      step->getName());
    ...
    if (!blocksHaveEqualStructure(*root_header, *step_header))
      throw Exception(
          ...
          step->getName(), root->step->getName(), root_header->dumpStructure(), step_header->dumpStructure());
  ...
  throw Exception(ErrorCodes::LOGICAL_ERROR, "Cannot add step {} to QueryPlan because it has {} inputs but {} input expected",
                  step->getName(), num_input_streams, isInitialized() ? 1 : 0);
}
...
static void explainStep(const IQueryPlanStep &step, JSONBuilder::JSONMap &map, const ExplainPlanOptions &options) {
  ...
  if (options.description) {
    ...
    if (!description.empty()) map.add("Description", description);
  }
  ...
  const auto dump_column = [](JSONBuilder::JSONArray &header_array, const ColumnWithTypeAndName &column) {
    ...
    if (column.type) column_map->add("Type", column.type->getName());
  ...
  if (options.header && step.hasOutputHeader()) {
    ...
    for (const auto &output_column : *step.getOutputHeader()) dump_column(*header_array, output_column);
  ...
  if (options.input_headers && !step.getInputHeaders().empty()) {
    ...
    for (const auto &input_header : step.getInputHeaders()) {
      ...
      for (const auto &input_column : *input_header) dump_column(*header_array, input_column);
  ...
  if (options.actions) step.describeActions(map);

  if (options.indexes) step.describeIndexes(map);

  if (options.projections) step.describeProjections(map);
}
...
JSONBuilder::ItemPtr QueryPlan::explainPlan(const ExplainPlanOptions &options) const {
  ...
  while (!stack.empty()) {
    ...
    if (frame.next_child == 0) {
      if (!frame.node->children.empty()) frame.children_array = std::make_unique<JSONBuilder::JSONArray>();
    ...
    if (frame.next_child < frame.node->children.size()) {
      ...
      if (!frame.children_array && !child_plans.empty()) frame.children_array = std::make_unique<JSONBuilder::JSONArray>();

      for (const auto &child_plan : child_plans) frame.children_array->add(child_plan->explainPlan(options));

      if (frame.children_array) frame.node_map->add("Plans", std::move(frame.children_array));

      tree.swap(frame.node_map);
      stack.pop();

      if (!stack.empty()) stack.top().children_array->add(std::move(tree));
    }
  }
...
static void explainStep(IQueryPlanStep &step, IQueryPlanStep::FormatSettings &settings, const ExplainPlanOptions &options,
  ...
  if (max_description_length) description = description.substr(0, max_description_length);
  if (options.description && !description.empty()) settings.out << " (" << description << ')';
  ...
  if (options.input_headers) {
    ...
    if (step.getInputHeaders().empty()) {
      ...
      for (const auto &input_header : step.getInputHeaders()) {
        if (!first_input_header) settings.out << '\n' << prefix << input_header_indent;
  ...
  if (options.actions) step.describeActions(settings);

  if (options.indexes) step.describeIndexes(settings);

  if (options.projections) step.describeProjections(settings);

  if (options.distributed) step.describeDistributedPlan(settings, options);
}
...
void QueryPlan::explainPlan(WriteBuffer &buffer, const ExplainPlanOptions &options, size_t indent, size_t max_description_length) const {
  ...
  while (!stack.empty()) {
    ...
    if (frame.next_child < frame.node->children.size()) {
      ...
      for (const auto &child_plan : child_plans) child_plan->explainPlan(buffer, options, indent + stack.size());
...
static void explainPipelineStep(IQueryPlanStep &step, IQueryPlanStep::FormatSettings &settings) {
  ...
  if (current_offset == settings.offset) settings.offset += settings.indent;
}
...
void QueryPlan::optimize(const QueryPlanOptimizationSettings &optimization_settings) {
  ...
  if (optimization_settings.remove_redundant_sorting) QueryPlanOptimizations::tryRemoveRedundantSorting(root);

  QueryPlanOptimizations::optimizeTreeFirstPass(optimization_settings, *root, nodes);
  QueryPlanOptimizations::optimizeTreeSecondPass(optimization_settings, *root, nodes, *this);
  if (optimization_settings.build_sets) QueryPlanOptimizations::addStepsToBuildSets(optimization_settings, *this, *root, nodes);
}
...
void QueryPlan::explainEstimate(MutableColumns &columns) const {
  ...
  processNodeFuncType process_node = [&counters, &process_node](const Node *node) {
    ...
    for (const auto *child : node->children) process_node(child);
  };
  process_node(root);
...
//             throw Exception(ErrorCodes::LOGICAL_ERROR, "Node {} {} is not known", node->step->getName(), reinterpret_cast<const void
//             *>(node));
...
//             throw Exception(ErrorCodes::LOGICAL_ERROR, "Node {} {} is not used", node->step->getName(), reinterpret_cast<const void
//             *>(node));
// }
...
QueryPlan QueryPlan::extractSubplan(Node *root, Nodes &nodes) {
  ...
  while (it != nodes.end()) {
    ...
    if (used.contains(&*curr)) new_plan.nodes.splice(new_plan.nodes.end(), nodes, curr);
  }
...
void QueryPlan::cloneInplace(Node *node_to_replace, Node *subplan_root) {
  if (!subplan_root) throw Exception(ErrorCodes::LOGICAL_ERROR, "Cannot clone subplan in place because subplan root is null");
...
void QueryPlan::replaceNodeWithPlan(Node *node, QueryPlan plan) {
  ...
  if (node->step) {
    ...
    if (!blocksHaveEqualStructure(*header, *plan_header)) {
      auto converting_dag = ActionsDAG::makeConvertingActions(plan_header->getColumnsWithTypeAndName(), header->getColumnsWithTypeAndName(),
                                                              ActionsDAG::MatchColumnsMode::Name, nullptr);
