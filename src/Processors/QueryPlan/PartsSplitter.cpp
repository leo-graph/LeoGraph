...
std::string toString(const Values &value) { return fmt::format("({})", fmt::join(value, ", ")); }
  ...
  switch (type_id) {
    ...
    case TypeIndex::Tuple: {
      ...
      for (const auto &data_type_tuple_element : data_type_tuple_elements)
        if (!isSafePrimaryDataKeyType(*data_type_tuple_element)) return false;
...
bool isSafePrimaryKey(const KeyDescription &primary_key) {
  for (const auto &type : primary_key.data_types) {
    if (!isSafePrimaryDataKeyType(*type)) return false;
  }
...
int compareValues(const Values &lhs, const Values &rhs, bool in_reverse_order) {
  ...
  if (in_reverse_order) {
    for (size_t i = 0; i < size; ++i) {
      if (accurateLess(rhs[i], lhs[i])) return -1;

      if (!accurateEquals(rhs[i], lhs[i])) return 1;
    }
  } else {
    for (size_t i = 0; i < size; ++i) {
      if (accurateLess(lhs[i], rhs[i])) return -1;

      if (!accurateEquals(lhs[i], rhs[i])) return 1;
    }
  }
...
class IndexAccess {
 public:
  explicit IndexAccess(const RangesInDataParts &parts_, size_t max_columns_in_index = std::numeric_limits<size_t>::max())
    ...
    for (const auto &part : parts) indices.push_back(part.data_part->getIndex());

    for (const auto &index : indices) loaded_columns = std::min(loaded_columns, index->size());
  }
  ...
  Values getValue(size_t part_idx, size_t mark) const {
    ...
    for (size_t i = 0; i < loaded_columns; ++i) {
      ...
      if (values[i].isNull()) values[i] = POSITIVE_INFINITY;
    }
    return values;
  }

  std::optional<size_t> findRightmostMarkLessThanValueInRange(size_t part_index, Values value, size_t range_begin, size_t range_end,
    ...
    if (right == range_begin) return {};

    return right - 1;
  }

  std::optional<size_t> findRightmostMarkLessThanValueInRange(size_t part_index, Values value, MarkRange mark_range,
                                                              bool in_reverse_order) const {
    return findRightmostMarkLessThanValueInRange(part_index, value, mark_range.begin, mark_range.end, in_reverse_order);
  }

  std::optional<size_t> findLeftmostMarkGreaterThanValueInRange(size_t part_index, Values value, size_t range_begin, size_t range_end,
                                                                bool in_reverse_order) const {
    size_t left = range_begin;
    size_t right = range_end;
    ...
    if (left == range_end) return {};

    return left;
  }

  std::optional<size_t> findLeftmostMarkGreaterThanValueInRange(size_t part_index, Values value, MarkRange mark_range,
                                                                bool in_reverse_order) const {
...
class RangesInDataPartsBuilder {
 public:
  explicit RangesInDataPartsBuilder(const RangesInDataParts &initial_ranges_in_data_parts_)
      : initial_ranges_in_data_parts(initial_ranges_in_data_parts_) {}
  ...
  void addRange(size_t part_index, MarkRange mark_range) {
    ...
    if (inserted) {
      ranges_in_data_parts.emplace_back(initial_ranges_in_data_parts[part_index].data_part,
                                        initial_ranges_in_data_parts[part_index].parent_part,
                                        initial_ranges_in_data_parts[part_index].part_index_in_query,
                                        initial_ranges_in_data_parts[part_index].part_starting_offset_in_query, MarkRanges{mark_range});
  ...
  RangesInDataParts &getCurrentRangesInDataParts() { return ranges_in_data_parts; }
...
struct PartsRangesIterator {
  ...
  [[maybe_unused]] bool operator<(const PartsRangesIterator &other) const {
    ...
    if (event == other.event) {
      if (!selected && other.selected) return true;
  ...
  [[maybe_unused]] bool operator==(const PartsRangesIterator &other) const {
    ...
    for (size_t i = 0; i < value.size(); ++i)
      if (!accurateEquals(value[i], other.value[i])) return false;

    return range == other.range && part_index == other.part_index && event == other.event;
  }

  [[maybe_unused]] bool operator>(const PartsRangesIterator &other) const {
    if (operator<(other) || operator==(other)) return false;
...
struct PartRangeIndex {
  explicit PartRangeIndex(PartsRangesIterator &ranges_iterator) : part_index(ranges_iterator.part_index), range(ranges_iterator.range) {}
...
void dump(const std::vector<PartsRangesIterator> &ranges_iterators, WriteBuffer &buffer) {
  for (const auto &range_iterator : ranges_iterators) range_iterator.dump(buffer);
}
...
SplitPartsRangesResult splitPartsRanges(RangesInDataParts ranges_in_data_parts, bool in_reverse_order, const LoggerPtr &logger) {
  ...
  for (size_t part_index = 0; part_index < ranges_in_data_parts.size(); ++part_index) {
    for (const auto &range : ranges_in_data_parts[part_index].ranges) {
      ...
      parts_ranges.push_back({index_access.getValue(part_index, range.begin), in_reverse_order, range, part_index,
      ...
      if (!value_is_defined_at_end_mark) continue;

      parts_ranges.push_back({index_access.getValue(part_index, range.end), in_reverse_order, range, part_index,
                              PartsRangesIterator::EventType::RangeEnd, false});
    }
  }
  ...
  for (size_t i = 1; i < parts_ranges_size; ++i) {
    ...
    if (range_start) {
      ...
      if (!inserted) throw Exception(ErrorCodes::LOGICAL_ERROR, "PartsSplitter expected unique range");

      if (intersecting_parts != 1) continue;
      ...
      if (previous_part_range.event == PartsRangesIterator::EventType::RangeStart) {
        ...
        if (ranges_in_data_parts[previous_part_range.part_index].data_part->info.level == 0) continue;
      ...
      if (ranges_in_data_parts[other_interval_part_index].data_part->info.level == 0) continue;
  ...
  std::stable_sort(intersecting_ranges_in_data_parts.begin(), intersecting_ranges_in_data_parts.end(),
                   [](const auto &lhs, const auto &rhs) { return lhs.part_index_in_query < rhs.part_index_in_query; });
...
SplitPartsByRanges splitIntersectingPartsRangesIntoLayers(RangesInDataParts ranges_in_data_parts, size_t max_layers,
                                                          size_t max_columns_in_index, bool in_reverse_order, const LoggerPtr &logger) {
  ...
  for (size_t part_index = 0; part_index < ranges_in_data_parts.size(); ++part_index) {
    for (const auto &range : ranges_in_data_parts[part_index].ranges) {
      const auto &index_granularity = ranges_in_data_parts[part_index].data_part->index_granularity;
      PartsRangesIterator parts_range_start{index_access.getValue(part_index, range.begin),
                                            in_reverse_order,
                                            range,
                                            part_index,
      ...
      PartsRangesIterator parts_range_end{index_access.getValue(part_index, range.end),
                                          in_reverse_order,
                                          range,
                                          part_index,
  ...
  while (!parts_ranges_queue.empty()) {
    ...
    while (rows_in_current_layer < rows_per_layer || layers_intersection_is_too_big() || result_layers.size() == max_layers) {
      ...
      if (parts_ranges_queue.empty()) break;
    ...
    for (const auto &[current_range_index, last_mark] : current_part_range_end) {
      current_layer_builder.addRange(current_range_index.part_index,
                                     MarkRange{current_part_range_begin[current_range_index], last_mark + 1});
  ...
  for (size_t i = 0; i < result_layers_size; ++i) {
    ...
    if (in_reverse_order) {
      LOG_TEST(logger, "Layer {} {} filter values in [{}, {}))", i, layer.getDescriptions().describe(),
...
static ASTs buildFilters(const KeyDescription &primary_key, const std::vector<Values> &borders, bool in_reverse_order) {
  ...
  for (size_t layer = 0; layer <= borders.size(); ++layer) {
    if (layer > 0) add_and_condition(filters[layer], lexicographically_greater(borders[layer - 1]));
    if (layer < borders.size()) add_and_condition(filters[layer], makeASTFunction("not", lexicographically_greater(borders[layer])));
  }
  return filters;
}

RangesInDataParts findPKRangesForFinalAfterSkipIndexImpl(RangesInDataParts &ranges_in_data_parts, bool cannot_sort_primary_key,
  ...
  for (size_t part_index = 0; part_index < ranges_in_data_parts.size(); ++part_index) {
    ...
    for (const auto &range : ranges_in_data_parts[part_index].ranges) {
      ...
      for (auto i = range.begin; i < range.end; ++i) part_selected_ranges[part_index].push_back(i);
    }
  }

  if (selected_ranges.empty()) return result.getCurrentRangesInDataParts();
  ...
  for (size_t part_index = 0; part_index < ranges_in_data_parts.size(); ++part_index) {
    ...
    auto candidates_start = index_access.findRightmostMarkLessThanValueInRange(
        part_index, selected_lower_bound.value, MarkRange{part_ranges_after_pk_analysis_begin, part_ranges_after_pk_analysis_end + 1},
        false);
    if (!candidates_start) candidates_start = 0;

    auto candidates_end = index_access.findLeftmostMarkGreaterThanValueInRange(
        part_index, selected_upper_bound.value, MarkRange{part_ranges_after_pk_analysis_begin, part_ranges_after_pk_analysis_end + 1},
        false);
    if (!candidates_end) candidates_end = part_ranges_after_pk_analysis_end;
    ...
    for (const auto &part_candidate_range : ranges) {
      for (auto range_begin = std::max(part_candidate_range.begin, *candidates_start);
           range_begin < std::min(part_candidate_range.end, *candidates_end + 1); range_begin++) {
        if (std::binary_search(part_selected_ranges[part_index].begin(), part_selected_ranges[part_index].end(), range_begin)) continue;

        MarkRange rejected_range(range_begin, range_begin + 1);
        rejected_ranges.push_back({index_access.getValue(part_index, rejected_range.begin), false, rejected_range, part_index,
                                   PartsRangesIterator::EventType::RangeStart, false});
      }
    }
  }
  ...
  std::stable_sort(result_final_ranges.begin(), result_final_ranges.end(),
                   [](const auto &lhs, const auto &rhs) { return lhs.part_index_in_query < rhs.part_index_in_query; });
  ...
  LOG_TRACE(logger,
            "findPKRangesForFinalAfterSkipIndex : processed {} parts, initially selected {} ranges & rejected {}, more {} ranges added",
            ranges_in_data_parts.size(), selected_ranges.size(), rejected_ranges.size(), more_ranges_added);
...
static void reorderColumns(ActionsDAG &dag, const Block &header, const std::string &filter_column) {
  ...
  for (const auto *input : dag.getInputs()) inputs_map[input->result_name] = input;

  for (const auto &col : header) {
    auto &input = inputs_map[col.name];
    if (!input) input = &dag.addInput(col);
  }
...
SplitPartsWithRangesByPrimaryKeyResult splitPartsWithRangesByPrimaryKey(
    const KeyDescription &primary_key, const KeyDescription &sorting_key, ExpressionActionsPtr sorting_expr, RangesInDataParts parts,
    size_t max_layers, ContextPtr context, ReadingInOrderStepGetter &&in_order_reading_step_getter,
    bool split_parts_ranges_into_intersecting_and_non_intersecting_final, bool split_intersecting_parts_ranges_into_layers) {
  if (!max_layers) throw Exception(ErrorCodes::LOGICAL_ERROR, "max_layers should be greater than 0");
  ...
  if (!isSafePrimaryKey(primary_key)) {
    if (!intersecting_parts_ranges.empty()) result.merging_pipes.emplace_back(create_merging_pipe(intersecting_parts_ranges));
    return result;
  }
  ...
  auto split_ranges = splitIntersectingPartsRangesIntoLayers(intersecting_parts_ranges, max_layers, primary_key.column_names.size(),
                                                             in_reverse_order, logger);
  result.merging_pipes = readByLayers(std::move(split_ranges), primary_key, create_merging_pipe, context);
  return result;
}

Pipes readByLayers(SplitPartsByRanges split_ranges, const KeyDescription &primary_key, ReadingInOrderStepGetter &&step_getter,
                   ContextPtr context) {
  ...
  for (size_t i = 0; i < layers.size(); ++i) {
    ...
    auto description = in_reverse_order ? fmt::format("filter values in [{}, {})", i < borders.size() ? ::toString(borders[i]) : "-inf",
                                                      i ? ::toString(borders[i - 1]) : "+inf")
                                        : fmt::format("filter values in ({}, {}]", i ? ::toString(borders[i - 1]) : "-inf",
                                                      i < borders.size() ? ::toString(borders[i]) : "+inf");
    merging_pipes[i].addSimpleTransform([&](const SharedHeader &header) {
...
RangesInDataParts findPKRangesForFinalAfterSkipIndex(const KeyDescription &primary_key, const KeyDescription &sorting_key,
                                                     RangesInDataParts &ranges_in_data_parts, const LoggerPtr &logger) {
