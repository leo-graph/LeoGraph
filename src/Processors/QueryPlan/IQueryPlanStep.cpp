...
IQueryPlanStep::IQueryPlanStep() { step_index = CurrentThread::isInitialized() ? CurrentThread::get().getNextPlanStepIndex() : 0; }
...
void IQueryPlanStep::updateInputHeader(SharedHeader input_header, size_t idx) {
  if (idx >= input_headers.size())
    throw Exception(ErrorCodes::LOGICAL_ERROR, "Cannot update input header {} for step {} because it has only {} headers", idx, getName(),
                    input_headers.size());
...
bool IQueryPlanStep::canRemoveColumnsFromOutput() const { return false; }
...
const SharedHeader& IQueryPlanStep::getOutputHeader() const {
  if (!hasOutputHeader()) throw Exception(ErrorCodes::LOGICAL_ERROR, "QueryPlanStep {} does not have output stream.", getName());

  return output_header;
}

std::string_view IQueryPlanStep::getStepDescription() const {
  if (std::holds_alternative<std::string_view>(step_description)) return std::get<std::string_view>(step_description);
  if (std::holds_alternative<std::string>(step_description)) return std::get<std::string>(step_description);
...
void IQueryPlanStep::setStepDescription(const IQueryPlanStep& step) { step_description = step.step_description; }

QueryPlanStepPtr IQueryPlanStep::clone() const { throw Exception(ErrorCodes::NOT_IMPLEMENTED, "Cannot clone {} plan step", getName()); }
...
static void doDescribeHeader(const Block& header, size_t count, IQueryPlanStep::FormatSettings& settings) {
  ...
  if (count > 1) prefix += " × " + std::to_string(count) + " ";
...
static void doDescribeProcessor(const IProcessor& processor, size_t count, IQueryPlanStep::FormatSettings& settings) {
  ...
  if (count > 1) settings.out << " × " << std::to_string(count);

  size_t num_inputs = processor.getInputs().size();
  size_t num_outputs = processor.getOutputs().size();
  if (num_inputs != 1 || num_outputs != 1) settings.out << " " << std::to_string(num_inputs) << " → " << std::to_string(num_outputs);
  ...
  if (settings.write_header) {
    ...
    if (last_header) doDescribeHeader(*last_header, num_equal_headers, settings);
  }
...
void IQueryPlanStep::describePipeline(const Processors& processors, FormatSettings& settings) {
  ...
  if (prev) doDescribeProcessor(*prev, count, settings);
}
...
String IQueryPlanStep::getUniqID() const { return fmt::format("{}_{}", getName(), step_index); }
