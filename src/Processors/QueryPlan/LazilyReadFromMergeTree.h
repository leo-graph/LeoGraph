...
class LazilyReadFromMergeTree final : public ISourceStep {
 public:
  LazilyReadFromMergeTree(SharedHeader header, size_t max_block_size, size_t min_marks_for_concurrent_read_,
                          MergeTreeReaderSettings reader_settings_, MergeTreeData::MutationsSnapshotPtr mutations_snapshot_,
                          StorageSnapshotPtr storage_snapshot, ContextPtr context_, const std::string& log_name_);
