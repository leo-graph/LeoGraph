...
LimitByStep::LimitByStep(const SharedHeader &input_header_, size_t group_length_, size_t group_offset_, Names columns_)
...
QueryPlanStepPtr LimitByStep::deserialize(Deserialization &ctx) {
  ...
  for (auto &column : columns) readStringBinary(column, ctx.in);

  return std::make_unique<LimitByStep>(ctx.input_headers.front(), group_length, group_offset, std::move(columns));
}

void LimitByStep::applyOrder(SortDescription sort_description) { in_order = sort_description.hasPrefix(columns); }

void registerLimitByStep(QueryPlanStepRegistry &registry) { registry.registerStep("LimitBy", LimitByStep::deserialize); }
