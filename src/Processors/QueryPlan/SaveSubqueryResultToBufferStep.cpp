...
SaveSubqueryResultToBufferStep::SaveSubqueryResultToBufferStep(const SharedHeader& header_, ColumnIdentifiers columns_to_save_,
...
void SaveSubqueryResultToBufferStep::transformPipeline(QueryPipelineBuilder& pipeline, const BuildQueryPipelineSettings& /*settings*/) {
  ...
  pipeline.addSimpleTransform(
      [this, &columns_to_save_indices](const SharedHeader& in_header, Pipe::StreamType stream_type) -> std::shared_ptr<IProcessor> {
        if (stream_type != Pipe::StreamType::Main) return nullptr;
        return std::make_shared<SaveSubqueryResultToBufferTransform>(in_header, chunk_buffer, columns_to_save_indices);
      });
}
