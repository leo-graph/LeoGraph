...
ReadFromLocalParallelReplicaStep::ReadFromLocalParallelReplicaStep(QueryPlanPtr query_plan_)
    : ISourceStep(query_plan_->getCurrentHeader()), query_plan(std::move(query_plan_)) {}
...
void ReadFromLocalParallelReplicaStep::addFilter(FilterDAGInfo filter) {
  ...
  auto filter_step = std::make_unique<FilterStep>(query_plan->getCurrentHeader(), std::move(filter.actions), std::move(filter.column_name),
