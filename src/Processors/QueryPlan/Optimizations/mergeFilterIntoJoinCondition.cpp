...
auto getInputNodes(const ActionsDAG &filter_dag, const Names &allowed_inputs_names) {
  ...
  for (const auto &input_node : filter_dag.getInputs()) inputs_map[input_node->result_name].emplace_back(input_node);
  ...
  while (!nodes_to_process.empty()) {
    ...
    if (current->type == ActionsDAG::ActionType::INPUT) {
      ...
      for (const auto *child : current->children) {
        if (!visited.contains(child)) nodes_to_process.push_back(child);
      }
    }
  }
  return result;
...
ExpressionSide getExpressionSide(const ActionsDAG::Node *expr, const std::unordered_set<const ActionsDAG::Node *> &left_allowed_inputs,
...
const ActionsDAG::Node &createResultPredicate(ActionsDAG &filter_dag, const ActionsDAG::Node *original_predicate,
...
std::pair<JoinConditionParts, bool> extractActionsForJoinCondition(ActionsDAG &filter_dag, const std::string &filter_name,
  ...
  if (predicate->type == ActionsDAG::ActionType::COLUMN) return {};
  ...
  for (const auto *conjunct : conjuncts_list) {
    ...
    if (is_equality) {
      ...
      if (!lhs->result_type->equals(*rhs->result_type)) continue;
  ...
  if (!result.empty()) {
    ...
    for (const auto *&output : filter_dag.getOutputs()) {
      ...
      if (it != conjuncts_to_replace.end()) {
        output = &filter_dag.addColumn(
            ColumnWithTypeAndName(output->result_type->createColumnConst(1, 1), output->result_type, output->result_name));
      }
    }
    ...
    if (rejected_conjuncts.size() == 1) {
      ...
      filter_dag.addOrReplaceInOutputs(
          createResultPredicate(filter_dag, predicate, &filter_dag.addFunction(func_builder_and, std::move(rejected_conjuncts), {})));
    }
...
size_t tryMergeFilterIntoJoinCondition(QueryPlan::Node *parent_node, QueryPlan::Nodes & /*nodes*/, const Optimization::ExtraSettings &) {
  ...
  if (!filter_step || !join_step) return 0;

  auto &join_operator = join_step->getJoinOperator();

  auto kind = join_operator.kind;
  if (kind != JoinKind::Inner && kind != JoinKind::Cross && kind != JoinKind::Comma) return 0;
  ...
  if (strictness != JoinStrictness::Unspecified && strictness != JoinStrictness::All) return 0;
  ...
  auto get_available_columns = [&join_header](const Block &input_header) {
    ...
    for (const auto &input_column : input_header.getColumnsWithTypeAndName()) {
      if (!join_header->has(input_column.name)) continue;

      /// Skip if type is changed. Push down expression expect equal types.
      if (!input_column.type->equals(*join_header->getByName(input_column.name).type)) continue;
  ...
  auto [equality_predicates, trivial_filter] = extractActionsForJoinCondition(
      filter_dag, filter_step->getFilterColumnName(), left_stream_available_columns, right_stream_available_columns);

  if (equality_predicates.empty()) return 0;

  for (auto &&predicate : equality_predicates) {
    join_step->addConditions(std::move(predicate));
  }

  if (kind == JoinKind::Cross || kind == JoinKind::Comma) join_operator.kind = JoinKind::Inner;

  /// Remove FilterStep if filter expression is always true
  if (trivial_filter) {
    if (filter_step->removesFilterColumn()) filter_dag.removeUnusedResult(filter_step->getFilterColumnName());
