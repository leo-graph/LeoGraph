...
bool canUseProjectionForReadingStep(ReadFromMergeTree *reading) {
  if (reading->getAnalyzedResult() && reading->getAnalyzedResult()->readFromProjection()) return false;

  if (reading->isQueryWithFinal()) return false;

  if (reading->isQueryWithSampling()) return false;

  if (reading->readsInOrder()) return false;
  ...
  if (reading->isParallelReadingEnabled()) {
    ...
    bool enable_aggregation_in_order =
        query_settings[Setting::optimize_aggregation_in_order] || query_settings[Setting::force_aggregation_in_order];

    if (!support_projection || enable_aggregation_in_order) return false;
  }

  // Currently projection don't support deduplication when moving parts between shards.
  if (query_settings[Setting::allow_experimental_query_deduplication]) return false;

  // Currently projection don't support settings which implicitly modify aggregate functions.
  if (query_settings[Setting::aggregate_functions_null_for_empty]) return false;
...
const ActionsDAG::Node *findInOutputs(ActionsDAG &dag, const std::string &name, bool remove) {
  ...
  for (auto it = outputs.begin(); it != outputs.end(); ++it) {
    if ((*it)->result_name == name) {
      ...
      if (node->result_type->onlyNull()) return nullptr;

      if (!isUInt8(removeNullable(removeLowCardinality(node->result_type))))
        throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_COLUMN_FOR_FILTER,
                        "Illegal type {} of column {} for filter. Must be UInt8 or Nullable(UInt8).", node->result_type->getName(), name);
...
bool QueryDAG::buildImpl(QueryPlan::Node &node, ActionsDAG::NodeRawConstPtrs &filter_nodes) {
  ...
  if (!buildImpl(*node.children.front(), filter_nodes)) return false;

  if (auto *expression = typeid_cast<ExpressionStep *>(step)) {
    const auto &actions = expression->getExpression();
    if (actions.hasArrayJoin()) return false;
  ...
  if (auto *filter = typeid_cast<FilterStep *>(step)) {
    ...
    if (!filter_expression) return false;
...
bool QueryDAG::build(QueryPlan::Node &node) {
  ...
  if (!buildImpl(node, filter_nodes)) return false;
...
size_t filterPartsByProjection(ReadFromMergeTree::AnalysisResult &reading_select_result,
                               const std::unordered_set<const IMergeTreeDataPart *> &valid_parts) {
...
bool analyzeProjectionCandidate(ProjectionCandidate &candidate, const MergeTreeDataSelectExecutor &reader,
                                MergeTreeData::MutationsSnapshotPtr empty_mutations_snapshot, const Names &required_column_names,
                                ReadFromMergeTree::AnalysisResult &parent_reading_select_result,
                                const SelectQueryInfo &projection_query_info, const ContextPtr &context) {
  ...
  for (const auto &part_with_ranges : parent_reading_select_result.parts_with_ranges) {
    ...
    if (it != created_projections.end() && !it->second->is_broken) {
      projection_parts.push_back(RangesInDataPart(it->second, part_with_ranges.data_part, part_with_ranges.part_index_in_query,
  ...
  if (projection_parts.empty()) return false;

  ReadFromMergeTree::AnalysisResultPtr projection_result_ptr = reader.estimateNumMarksToRead(
      std::move(projection_parts), empty_mutations_snapshot, required_column_names, candidate.projection->metadata, projection_query_info,
      context, context->getSettingsRef()[Setting::max_threads]);

  /// If projection analysis exceeded limits, skip this candidate
  if (!projection_result_ptr->isUsable()) return false;

  std::unordered_set<const IMergeTreeDataPart *> valid_parts = candidate.parent_parts;
  for (auto &part : projection_result_ptr->parts_with_ranges) valid_parts.emplace(part.data_part->getParentPart());
...
void filterPartsAndCollectProjectionCandidates(ReadFromMergeTree &reading, const ProjectionDescription &projection,
                                               const MergeTreeDataSelectExecutor &reader,
                                               MergeTreeData::MutationsSnapshotPtr empty_mutations_snapshot,
                                               ReadFromMergeTree::AnalysisResult &parent_reading_select_result,
                                               const SelectQueryInfo &projection_query_info, const ActionsDAG::Node *filter_node,
  ...
  for (const auto &part_with_ranges : parent_reading_select_result.parts_with_ranges) {
    ...
    if (it != created_projections.end() && !it->second->is_broken) {
      RangesInDataPart projection_part(it->second, part_with_ranges.data_part, part_with_ranges.part_index_in_query,
                                       part_with_ranges.part_starting_offset_in_query);
  ...
  if (projection_parts.empty()) return;
  ...
  auto projection_result_ptr =
      reader.estimateNumMarksToRead(std::move(projection_parts), empty_mutations_snapshot, required_column_names, projection.metadata,
                                    projection_query_info, context, context->getSettingsRef()[Setting::max_threads], nullptr);

  /// Projection has no filtering effect, skip it
  if (projection_result_ptr->selected_marks == projection_marks_to_read) return;
  ...
  if (in_use) {
    ...
    for (const auto &column : projection.sample_block) available_inputs.emplace(column.name);

    auto prewhere_info = std::make_shared<PrewhereInfo>();
    prewhere_info->prewhere_actions = projection_query_info.filter_actions_dag->restrictFilterDAGToInputs(filter_node, available_inputs);
...
void fallbackToLocalProjectionReading(const QueryPlanStepPtr &projection_reading) {
  ...
  /// -------------------------------------------------------------------------------------------
  /// The coordinator does not support reading from two streams at the moment, so read projections are performed directly on the initial
  /// replica.
