...
static bool canUseLazyMaterializationForReadingStep(ReadFromMergeTree *reading) {
  if (reading->isQueryWithFinal()) return false;

  if (reading->isQueryWithSampling()) return false;

  if (reading->isVectorColumnReplaced()) return false;
...
std::pair<std::vector<size_t>, std::vector<size_t>> mapInputsToHeaderPositions(const ActionsDAG::NodeRawConstPtrs &inputs,
                                                                               const Block &header) {
  std::unordered_map<std::string, std::list<size_t>> name_to_position;
  for (size_t i = 0; i < header.columns(); ++i) name_to_position[header.getByPosition(i).name].push_back(i);

  std::vector<size_t> positions;
  positions.reserve(inputs.size());
  for (const auto *input : inputs) {
    auto &lst = name_to_position[input->result_name];
    if (lst.empty()) throw Exception(ErrorCodes::LOGICAL_ERROR, "Unknown identifier: '{}'", input->result_name);
  ...
  for (size_t i = 0; i < header.columns(); ++i)
    for (auto idx : name_to_position[header.getByPosition(i).name]) non_mapped.push_back(idx);
...
std::vector<bool> getRequiredHeaderPositions(const ActionsDAG &dag, const Block &header, std::vector<bool> required_output_positions) {
  ...
  for (size_t i = 0; i < dag.getOutputs().size(); ++i) {
    if (required_output_positions[i]) stack.push(dag.getOutputs()[i]);
  }
  ...
  while (!stack.empty()) {
    ...
    for (const auto *child : current_node->children) stack.push(child);
  }

  const auto &inputs = dag.getInputs();
  const auto [header_positions, non_mapped] = mapInputsToHeaderPositions(inputs, header);
  for (size_t i = 0; i < inputs.size(); ++i)
    if (required_nodes.contains(inputs[i])) required_input_positions[header_positions[i]] = true;
  ...
  for (size_t i = 0; num_outputs + i < required_output_positions.size(); ++i)
    if (required_output_positions[num_outputs + i]) required_input_positions[non_mapped[i]] = true;
...
void updateRequiredColumnsForFilterDAG(std::vector<bool> &required_output_positions, const FilterStep &filter_step) {
  ...
  for (; i < outputs.size(); ++i) {
    if (outputs[i]->result_name == name) break;
  }

  if (filter_step.removesFilterColumn()) {
    required_output_positions.push_back(false);
    for (size_t j = required_output_positions.size() - 1; j > i; --j) required_output_positions[j] = required_output_positions[j - 1];
  }
...
void removeDanglingNodes(ActionsDAG &dag) {
  ...
  for (const auto &node : dag.getNodes())
    for (const auto *child : node.children) used_nodes.insert(child);

  std::unordered_set<const ActionsDAG::Node *> inputs;
  for (const auto &input : dag.getInputs()) inputs.insert(input);

  auto &outputs = dag.getOutputs();
  size_t next_pos = 0;
  for (size_t i = 0; i < outputs.size(); ++i) {
    bool is_dangling = inputs.contains(outputs[i]) && !used_nodes.contains(outputs[i]);
    if (!is_dangling) outputs[next_pos++] = outputs[i];
  }
...
SplitExpressionStepResult splitExpressionStep(const ExpressionStep &expression_step, std::vector<bool> required_output_positions) {
  ...
  for (size_t i = 0; i < outputs.size(); ++i) {
    if (required_output_positions[i]) split_nodes.insert(outputs[i]);
  }
  auto split_result = expression.split(split_nodes, true, true);

  auto required_input_positions =
      getRequiredHeaderPositions(expression, *expression_step.getInputHeaders().front(), std::move(required_output_positions));
...
SplitFilterResult splitFilterStep(const FilterStep &filter_step, std::vector<bool> required_output_positions) {
  ...
  auto required_input_positions =
      getRequiredHeaderPositions(expression, *filter_step.getInputHeaders().front(), std::move(required_output_positions));
...
std::unique_ptr<LazilyReadFromMergeTree> removeUnusedColumnsFromReadingStep(ReadFromMergeTree &reading_step,
                                                                            const std::vector<bool> &required_output_positions) {
  ...
  for (size_t i = 0; i < cols.size(); ++i)
    if (required_output_positions[i]) required_names.insert(cols[i].name);
...
ActionsDAG calculateGlobalOffset(ReadFromMergeTree &reading_step) {
  ...
  if (!added_part_starting_offset) dag.getOutputs().push_back(dag.getInputs()[0]);
  if (!added_part_offset) dag.getOutputs().push_back(dag.getInputs()[1]);
...
static ReadFromMergeTree *findReadingStep(QueryPlan::Node &node, StepStack &backward_path) {
  ...
  if (auto *read_from_merge_tree = typeid_cast<ReadFromMergeTree *>(step)) return read_from_merge_tree;

  if (node.children.size() != 1) return nullptr;

  if (typeid_cast<ExpressionStep *>(step) || typeid_cast<FilterStep *>(step)) return findReadingStep(*node.children.front(), backward_path);

  return nullptr;
}

bool optimizeLazyMaterialization2(QueryPlan::Node &root, QueryPlan &query_plan, QueryPlan::Nodes &nodes,
  ...
  if (limit_step->withTies()) return false;

  auto *sorting_step = typeid_cast<SortingStep *>(root.children.front()->step.get());
  if (!sorting_step) return false;

  if (sorting_step->getType() != SortingStep::Type::Full && sorting_step->getType() != SortingStep::Type::FinishSorting) return false;

  bool reading_in_order = sorting_step->getType() == SortingStep::Type::FinishSorting;

  const auto limit = limit_step->getLimit();
  if (limit == 0 || (max_limit_for_lazy_materialization != 0 && limit > max_limit_for_lazy_materialization)) return false;
  ...
  if (!reading_step) return false;

  if (!canUseLazyMaterializationForReadingStep(reading_step)) return false;

  const auto &sorting_header = *sorting_step->getOutputHeader();
  std::vector<bool> required_columns(sorting_header.columns(), false);

  for (const auto &descr : sorting_step->getSortDescription()) required_columns[sorting_header.getPositionByName(descr.column_name)] = true;
  ...
  while (!node->children.empty()) {
    ...
    if (const auto *expr_step = typeid_cast<ExpressionStep *>(step))
      required_columns =
          getRequiredHeaderPositions(expr_step->getExpression(), *expr_step->getInputHeaders().front(), std::move(required_columns));
    else if (const auto *filter_step = typeid_cast<FilterStep *>(step)) {
      updateRequiredColumnsForFilterDAG(required_columns, *filter_step);
      required_columns =
          getRequiredHeaderPositions(filter_step->getExpression(), *filter_step->getInputHeaders().front(), std::move(required_columns));
      has_filter = true;
    } else
      return false;
  ...
  if (!read_from_merge_tree) return false;

  if (read_from_merge_tree->getPrewhereInfo() || read_from_merge_tree->getRowLevelFilter()) has_filter = true;

  /// Disable the case with read-in-order and no filter.
  /// It's not likely we can optimize it more.
  if (reading_in_order && !has_filter) return false;

  auto lazy_reading = removeUnusedColumnsFromReadingStep(*read_from_merge_tree, required_columns);
  if (!lazy_reading) return false;
  ...
  for (auto &step : main_steps) {
    if (std::holds_alternative<ActionsDAG>(step)) {
      ...
      main_plan.addStep(std::make_unique<FilterStep>(main_plan.getCurrentHeader(), std::move(filter_dag_info.actions),
                                                     filter_dag_info.column_name, filter_dag_info.do_remove_column));
    }
  }

  auto new_sorting_step = std::move(
      root.children.front()->step);  // = std::make_unique<SortingStep>(main_plan.getCurrentHeader(), sorting_step->getSortDescription(),
                                     // sorting_step->getLimit(), sorting_step->getSettings());
  ...
  while (!lazy_steps.empty()) {
    ...
    if (!lazy_steps.empty()) removeDanglingNodes(dag);
  ...
  if (result_header->columns() != expected_output_header->columns()) {
    ...
    for (const auto &col : *expected_output_header) expected_columns.push_back(col.name);
