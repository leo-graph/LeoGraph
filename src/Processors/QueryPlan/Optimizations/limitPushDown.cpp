...
static bool tryUpdateLimitForSortingSteps(QueryPlan::Node *node, size_t limit) {
  ...
  if (child) tryUpdateLimitForSortingSteps(child, limit);
...
size_t tryPushDownLimit(QueryPlan::Node *parent_node, QueryPlan::Nodes &, const Optimization::ExtraSettings & /*settings*/) {
  ...
  if (tryUpdateLimitForSortingSteps(child_node, limit->getLimitForSorting())) return 0;
  ...
  if (!transform_traits.preserves_number_of_rows) return 0;
