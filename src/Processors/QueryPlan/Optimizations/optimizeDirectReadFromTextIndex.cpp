...
bool hasSubexpression(const ActionsDAG::Node *node, const String &subexpression_name) {
  if (getNameWithoutAliases(node) == subexpression_name) return true;

  for (const auto *child : node->children) {
    if (hasSubexpression(child, subexpression_name)) return true;
  }
...
const ActionsDAG::Node *replaceNodes(ActionsDAG &dag, const ActionsDAG::Node *node, const NodesReplacementMap &replacements) {
  if (auto it = replacements.find(node); it != replacements.end()) {
    ...
    if (old_child != new_child) return &dag.addAlias(*new_child, node->result_name);
    ...
    for (const auto &child : old_children) new_children.push_back(replaceNodes(dag, child, replacements));

    if (new_children != old_children) return &dag.addFunction(node->function_base, new_children, "");
  }
...
void collectTextIndexReadInfos(const ReadFromMergeTree *read_from_merge_tree_step, TextIndexReadInfos &text_index_read_infos) {
  ...
  if (!indexes || indexes->skip_indexes.useful_indices.empty()) return;
  ...
  for (const auto &part : parts_with_ranges) unique_parts.insert(part.data_part);

  for (const auto &index : indexes->skip_indexes.useful_indices) {
    if (!typeid_cast<MergeTreeIndexConditionText *>(index.condition.get())) continue;

    /// Index may be not materialized in some parts, e.g. after ALTER ADD INDEX query.
    size_t num_materialized_parts = std::ranges::count_if(
        unique_parts, [&](const auto &part) { return !!index.index->getDeserializedFormat(part->checksums, index.index->getFileName()); });

    text_index_read_infos[index.index->index.name] = {.index = &index,
...
ASTPtr convertNodeToAST(const ActionsDAG::Node &node) {
  switch (node.type) {
    ...
    case ActionsDAG::ActionType::FUNCTION: {
      if (!node.function_base) return nullptr;
      ...
      if (const auto *function_capture = dynamic_cast<const FunctionCapture *>(node.function_base.get())) {
        ...
        if (capture_dag.getOutputs().size() != 1) return nullptr;

        auto required_columns = capture_dag.getRequiredColumnsNames();
        if (required_columns.size() != 1) return nullptr;
        ...
        for (const auto *child : node.children) {
          if (auto arg_ast = convertNodeToAST(*child)) function->arguments->children.push_back(arg_ast);
        }
      }
...
class TextIndexDAGReplacer {
 public:
  ...
  ResultReplacement replace(const ContextPtr &context, const String &filter_column_name) {
    ...
    for (const auto *node : nodes_ptrs) {
      ...
      if (replaced.node != node) replacements[node] = replaced.node;
    ...
    for (auto &output : actions_dag.outputs) {
      ...
      if (is_filter_node) filter_node = output;
    }
    ...
    for (const auto &column : original_inputs) {
      if (!replaced_columns_set.contains(column)) result.removed_columns.push_back(column);
    }
 ...
 private:
  ...
  static bool needApplyTokenizer(const String &function_name) { return function_name == "hasAllTokens" || function_name == "hasAnyTokens"; }
  ...
  std::vector<SelectedCondition> selectConditions(const ActionsDAG::Node &function_node) {
    ...
    for (const auto &[index_name, info] : text_index_read_infos) {
      ...
      if (index_header.columns() != 1 || used_index_columns.contains(index_header.begin()->name)) continue;

      auto search_query = text_index_condition.createTextSearchQuery(function_node);
      if (!search_query || search_query->direct_read_mode == TextIndexDirectReadMode::None) continue;

      auto virtual_column_name = text_index_condition.replaceToVirtualColumn(*search_query, index_name);
      if (!virtual_column_name) continue;
  ...
  NodeReplacement processFunctionNode(const ActionsDAG::Node &function_node,
                                      std::unordered_map<String, const ActionsDAG::Node *> &virtual_column_to_node,
    ...
    if (!function_node.result_type->canBeUsedInBooleanContext()) return replacement;
    ...
    if (!need_preprocess_function && !direct_read_from_text_index) return replacement;

    auto selected_conditions = selectConditions(function_node);
    if (selected_conditions.empty()) return replacement;

    /// Sort conditions to produce stable output for EXPLAIN query.
    std::ranges::sort(selected_conditions,
                      [](const auto &lhs, const auto &rhs) { return lhs.virtual_column_name < rhs.virtual_column_name; });

    if (need_preprocess_function) preprocessTextIndexFunction(replacement, selected_conditions, context);

    if (direct_read_from_text_index) replaceFunctionsToVirtualColumns(replacement, selected_conditions, virtual_column_to_node, context);

    return replacement;
  }

  /// Applies preprocessor and tokenizer for text-search functions.
  void preprocessTextIndexFunction(NodeReplacement &replacement, const std::vector<SelectedCondition> &selected_conditions,
                                   const ContextPtr &context) {
    const auto &function_node = *replacement.node;
    if (selected_conditions.size() != 1 || function_node.children.size() != 2) return;

    auto new_children = function_node.children;
    const auto &arg_haystack = new_children[0];
    const auto &arg_needles = new_children[1];

    if (arg_needles->type != ActionsDAG::ActionType::COLUMN || !arg_needles->column) return;

    if (arg_needles->column->empty() || arg_needles->column->isNullAt(0)) return;
  ...
  void replaceFunctionsToVirtualColumns(NodeReplacement &replacement, const std::vector<SelectedCondition> &selected_conditions,
                                        std::unordered_map<String, const ActionsDAG::Node *> &virtual_column_to_node,
                                        const ContextPtr &context) {
    const auto &function_node = *replacement.node;
    ...
    if (!has_materialized_index) return;
    ...
    auto add_condition_to_input = [&](const SelectedCondition &condition) {
      ...
      if (inserted) {
        ...
        VirtualColumnDescription virtual_column(condition.virtual_column_name, std::make_shared<DataTypeUInt8>(), /*codec=*/nullptr,
                                                condition.index_name, VirtualsKind::Ephemeral);
    ...
    } else  /// Otherwise, combine all conditions with the AND function.
    {
      ...
      for (const auto &condition : selected_conditions) children.push_back(add_condition_to_input(condition));

      if (!has_exact_search) children.push_back(&function_node);
...
static const ActionsDAG::Node *processAndOptimizeTextIndexDAG(ReadFromMergeTree &read_from_merge_tree_step, ActionsDAG &filter_dag,
                                                              const TextIndexReadInfos &text_index_read_infos,
                                                              const String &filter_column_name, bool direct_read_from_text_index) {
  ...
  if (result.added_columns.empty()) return result.filter_node;
  ...
  for (const auto &[index_name, info] : text_index_read_infos) {
    if (!info.is_fully_materialied)
      LOG_DEBUG(logger, "Text index '{}' is not fully materialized. In some parts, direct read from text index cannot be used.",
                index_name);
  }
...
static bool processAndOptimizeTextIndexFunctionsInPrewhere(ReadFromMergeTree &read_from_merge_tree_step,
  ...
  const auto *result_filter_node =
      processAndOptimizeTextIndexDAG(read_from_merge_tree_step, cloned_prewhere_info.prewhere_actions, text_index_read_infos,
                                     cloned_prewhere_info.prewhere_column_name, direct_read_from_text_index);
...
void processAndOptimizeTextIndexFunctions(const Stack &stack, QueryPlan::Nodes & /*nodes*/, bool direct_read_from_text_index) {
  ...
  if (!read_from_merge_tree_step) return;

  TextIndexReadInfos text_index_read_infos;
  collectTextIndexReadInfos(read_from_merge_tree_step, text_index_read_infos);
  if (text_index_read_infos.empty()) return;

  bool optimized = false;
  if (auto prewhere_info = read_from_merge_tree_step->getPrewhereInfo())
    optimized = processAndOptimizeTextIndexFunctionsInPrewhere(*read_from_merge_tree_step, prewhere_info, text_index_read_infos,
  ...
  if (!filter_step) return;

  ActionsDAG &filter_dag = filter_step->getExpression();
  const auto *result_filter_node =
      processAndOptimizeTextIndexDAG(*read_from_merge_tree_step, filter_dag, text_index_read_infos, filter_step->getFilterColumnName(),
                                     direct_read_from_text_index && !optimized);

  if (!result_filter_node) return;

  bool removes_filter_column = filter_step->removesFilterColumn();
  auto new_filter_column_name = result_filter_node->result_name;
  filter_node->step = std::make_unique<FilterStep>(read_from_merge_tree_step->getOutputHeader(), filter_dag.clone(), new_filter_column_name,
