...
void enableMemoryBoundMerging(QueryPlan::Node &node) {
  ...
  if (local_plan == nullptr || (reading_steps.empty() && async_reading_steps.empty())) return;
  ...
  if (sort_description.empty()) return;

  for (auto &reading : reading_steps) {
    reading->enableMemoryBoundMerging();
    if (enforce_aggregation_in_order || reading->hasSerializedPlan()) reading->enforceAggregationInOrder(sort_description);
  }

  for (auto &reading : async_reading_steps) {
    reading->enableMemoryBoundMerging();
    if (enforce_aggregation_in_order) reading->enforceAggregationInOrder(sort_description);
  }
