...
ActionsDAG splitAndFillPrewhereInfo(PrewhereInfoPtr &prewhere_info, bool remove_prewhere_column, ActionsDAG filter_expression,
  ...
  {
    std::unordered_set<const ActionsDAG::Node *> first_outputs(split_result.first.getOutputs().begin(),
                                                               split_result.first.getOutputs().end());
  ...
  for (const auto *condition : prewhere_nodes_list) conditions.push_back(split_result.split_nodes_mapping.at(condition));
  ...
  if (conditions.size() == 1) {
    ...
    if (prewhere_info->remove_prewhere_column) prewhere_info->prewhere_actions.getOutputs().push_back(conditions.front());
  } else {
...
void optimizePrewhere(QueryPlan::Node &parent_node, const bool remove_unused_columns) {
  ...
  if (!source_step_with_filter) return;

  if (typeid_cast<ReadFromMerge *>(child_node->step.get())) return;

  const auto &storage_snapshot = source_step_with_filter->getStorageSnapshot();
  const auto &storage = storage_snapshot->storage;
  if (!storage.canMoveConditionsToPrewhere()) return;

  if (source_step_with_filter->getPrewhereInfo()) return;
  ...
  if (read_from_merge_tree_step && read_from_merge_tree_step->getVectorSearchParameters().has_value() &&
      !settings[Setting::vector_search_with_rescoring])
    return;
  ...
  auto optimize_result = where_optimizer.optimize(filter_step->getExpression(), filter_step->getFilterColumnName(),
                                                  source_step_with_filter->getContext(), is_final);

  if (optimize_result.prewhere_nodes.empty()) return;

  PrewhereInfoPtr prewhere_info = std::make_shared<PrewhereInfo>();

  auto remaining_expr = splitAndFillPrewhereInfo(
      prewhere_info, optimize_result.fully_moved_to_prewhere && filter_step->removesFilterColumn(), std::move(filter_step->getExpression()),
      filter_step->getFilterColumnName(), optimize_result.prewhere_nodes, optimize_result.prewhere_nodes_list);
  ...
  if (!optimize_result.fully_moved_to_prewhere) {
    new_step = std::make_unique<FilterStep>(source_step_with_filter->getOutputHeader(), std::move(remaining_expr),
    ...
    new_step = std::make_unique<ExpressionStep>(source_step_with_filter->getOutputHeader(), std::move(remaining_expr));
  }

  new_step->setStepDescription(*filter_step);
  parent_node.step = std::move(new_step);

  if (!remove_unused_columns) return;
  ...
  if (source_step_with_filter->canRemoveUnusedColumns() && source_step_with_filter->canRemoveColumnsFromOutput() &&
    ...
    for (const auto &column_name_and_type : *parent_step->getOutputHeader()) required_outputs.insert(column_name_and_type.name);
    ...
    if (result == IQueryPlanStep::RemovedUnusedColumns::OutputAndInput) {
      ...
      for (const auto &column_name_and_type : *parent_step->getInputHeaders().at(0)) required_outputs.insert(column_name_and_type.name);
      ...
      if (!blocksHaveEqualStructure(*parent_step->getInputHeaders().at(0), *source_step_with_filter->getOutputHeader()))
        throw Exception(
            ...
            "output header: {}",
            parent_step->getInputHeaders().at(0)->dumpStructure(), source_step_with_filter->getOutputHeader()->dumpStructure());
    }
  }
}
