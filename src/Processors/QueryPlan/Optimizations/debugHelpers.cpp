...
RelationStats getDummyStats(const String& dummy_stats_str, const String& table_name) {
  try {
    ...
    if (!object->has(table_name)) return {};

    auto stat_object = object->getObject(table_name);
    if (!stat_object) return {};

    RelationStats stats;
    stats.table_name = table_name;

    if (stat_object->has("cardinality")) stats.estimated_rows = stat_object->getValue<UInt64>("cardinality");

    if (stat_object->isObject("distinct_keys")) {
      auto distinct_keys = stat_object->getObject("distinct_keys");
      for (const auto& [key, value] : *distinct_keys) stats.column_stats[key].num_distinct_values = value.convert<UInt64>();
    }
    LOG_DEBUG(getLogger("optimizeJoin"),
              "Got dummy join stats for table '{}' from '{}' query parameter, it's supposed to be used only for testing, do not use it in "
              "production",
...
RelationStats getDummyStats(ContextPtr context, const String& table_name) {
  ...
  if (auto it = query_params.find(DUMMY_JOIN_STATS_PARAM_NAME); it != query_params.end()) return getDummyStats(it->second, table_name);
  return {};
}
