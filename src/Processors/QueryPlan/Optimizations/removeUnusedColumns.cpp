...
bool addDiscardingExpressionStepIfNeeded(QueryPlan::Nodes &nodes, QueryPlan::Node &parent, const size_t child_id) {
  ...
  if (input_it != input_header->end()) throw Exception(ErrorCodes::LOGICAL_ERROR, "Input header is not a subset of output header");

  for (; output_it != output_header->end(); ++output_it) columns_to_discard.push_back(&(*output_it));
  ...
  for (const auto *column : columns_to_discard) discarding_dag.addInput(*column);
...
bool canAllChildrenCanRemoveOutputs(const QueryPlan::Node &node) {
  return std::all_of(node.children.begin(), node.children.end(), [](const QueryPlan::Node *child) {
    return child->step->canRemoveUnusedColumns() && child->step->canRemoveColumnsFromOutput();
  });
}

bool updatedAnything(const IQueryPlanStep::RemovedUnusedColumns &result) { return result != IQueryPlanStep::RemovedUnusedColumns::None; }
...
bool absorbExtraChildColumns(QueryPlan::Node &node, size_t child_id) {
  ...
  for (const auto &col : *parent_input) parent_input_names.insert(col.name);
...
RemoveChildrenOutputResult removeChildrenOutputs(QueryPlan::Nodes &nodes, QueryPlan::Node &node) {
  ...
  for (auto child_id = 0U; child_id < node.children.size(); ++child_id) {
    ...
    // Right now this is mostly relevant for JoinStepLogical, as it must keep at least one column in its output, even if its parent requires
    // no input. However in the future we might have other steps with similar behavior.
    if (updated_anything) {
      ...
      if (added_discarding_step) {
        ...
        assertBlocksHaveEqualStructure(*discarding_step.getInputHeaders()[0], *child_step->getOutputHeader(),
                                       "after adding discarding step");
#endif
    ...
    /// Otherwise, add a discarding ExpressionStep between the parent and child to remove the extra columns.
    {
      ...
      if (child_output->columns() != current_parent_input->columns()) {
        if (!absorbExtraChildColumns(node, child_id)) {
          if (addDiscardingExpressionStepIfNeeded(nodes, node, child_id)) added_any_discarding_step = true;
        }
      }
    }
...
#if defined(DEBUG_OR_SANITIZER_BUILD)
    {
      ...
      assertBlocksHaveEqualStructure(*node.children[child_id]->step->getOutputHeader(), *final_parent_inputs[child_id],
                                     "after removing unused columns");
    }
#endif

    if (updated_anything) updated = true;
  }

  if (added_any_discarding_step) return RemoveChildrenOutputResult::AddedDiscardingStep;

  if (updated) return RemoveChildrenOutputResult::Updated;
...
size_t tryRemoveUnusedColumns(QueryPlan::Node *node, QueryPlan::Nodes &nodes, const Optimization::ExtraSettings &) {
  ...
  for (auto child_id = 0U; child_id < node->children.size(); ++child_id) {
    ...
    if (!child_step->canRemoveUnusedColumns()) continue;
    ...
    if (updatedAnything(remove_result)) {
      ...
      if (addDiscardingExpressionStepIfNeeded(nodes, *node, child_id)) ++current_update_depth;
    }
