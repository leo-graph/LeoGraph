...
DPJoinEntry::DPJoinEntry(size_t id, std::optional<UInt64> rows)
    : relations(), cost(0.0), estimated_rows(rows), relation_id(static_cast<int>(id)) {
  relations.set(id);
}

DPJoinEntry::DPJoinEntry(DPJoinEntryPtr lhs, DPJoinEntryPtr rhs, double cost_, std::optional<UInt64> cardinality_,
                         JoinOperator join_operator_, JoinMethod join_method_)
...
String DPJoinEntry::dump() const {
  if (isLeaf()) return fmt::format("Leaf({})", relation_id);
...
class JoinOrderOptimizer {
 public:
  JoinOrderOptimizer(QueryGraph query_graph_, const std::vector<JoinOrderAlgorithm> &enabled_algorithms_)
      : query_graph(std::move(query_graph_)), enabled_algorithms(enabled_algorithms_) {
...
void JoinOrderOptimizer::checkLimits() {
  ...
  if (interactive_cancel_callback) interactive_cancel_callback();
}
...
size_t JoinOrderOptimizer::getColumnStats(BitSet rels, const String &column_name) {
  ...
  if (!rel_id.has_value()) {
    ...
    if (auto it = dp_table.find(rels); it != dp_table.end()) return it->second->estimated_rows.value_or(0);
    return 0;
  }

  const auto &relation_stat = relation_stats.at(rel_id.value());
  const auto &column_stats = relation_stat.column_stats;
  if (auto it = column_stats.find(column_name); it != column_stats.end()) return it->second.num_distinct_values;
...
double JoinOrderOptimizer::computeSelectivity(const JoinActionRef &edge) {
  ...
  if (!inserted) return selectivity;

  auto [op, lhs, rhs] = edge.asBinaryPredicate();

  if (op != JoinConditionOperator::Equals && op != JoinConditionOperator::NullSafeEquals) return 1.0;
  ...
  if (max_ndv > 0) selectivity = std::min(selectivity, 1.0 / static_cast<double>(max_ndv));
  return selectivity;
}

double JoinOrderOptimizer::computeSelectivity(const std::vector<JoinActionRef *> &edges) {
  double selectivity = 1.0;
  for (const auto &edge : edges) selectivity = std::min(selectivity, computeSelectivity(*edge));
  return selectivity;
}

static std::optional<UInt64> estimateJoinCardinality(const std::shared_ptr<DPJoinEntry> &left, const std::shared_ptr<DPJoinEntry> &right,
                                                     double selectivity, JoinKind join_kind = JoinKind::Inner) {
  if (!left->estimated_rows || !right->estimated_rows) return {};
  ...
  if (join_kind == JoinKind::Left) joined_rows = std::max(joined_rows, lhs);
  if (join_kind == JoinKind::Right) joined_rows = std::max(joined_rows, rhs);
  if (join_kind == JoinKind::Full) joined_rows = std::max(joined_rows, lhs + rhs);
  ...
  if (joined_rows >= static_cast<double>(std::numeric_limits<UInt64>::max())) return std::numeric_limits<UInt64>::max();
  if (joined_rows < 1) return 1;
  return static_cast<UInt64>(joined_rows);
}

static double computeJoinCost(const std::shared_ptr<DPJoinEntry> &left, const std::shared_ptr<DPJoinEntry> &right, double selectivity) {
...
std::shared_ptr<DPJoinEntry> JoinOrderOptimizer::solve() {
  ...
  for (const auto &algorithm : enabled_algorithms) {
    ...
    switch (algorithm) {
      ...
      case JoinOrderAlgorithm::GREEDY:
        ...
        if (!best_plan) throw Exception(ErrorCodes::LOGICAL_ERROR, "Failed to find a valid join order with greedy algorithm");
        break;
    }
  ...
  if (!best_plan)
    throw Exception(ErrorCodes::EXPERIMENTAL_FEATURE_ERROR,
                    "Failed to find a valid join order, try adding 'greedy' algorithm as fallback to "
                    "query_plan_optimize_join_order_algorithm setting.");

  LOG_TRACE(log, "Optimized join order in {:.2f} ms, best plan cost: {}, estimated cardinality: {}",
            static_cast<double>(watch.elapsed()) / 1000.0, best_plan->cost,
...
std::vector<JoinActionRef *> JoinOrderOptimizer::getApplicableExpressions(const BitSet &left, const BitSet &right) {
  ...
  for (auto &edge : query_graph.edges) {
    ...
    if (!isSubsetOf(edge_sources, joined_rels)) continue;
    ...
    if (pin_it != query_graph.pinned.end()) {
       ...
       */
      if (!joined_rels.test(pin_it->second)) continue;
    }
...
std::shared_ptr<DPJoinEntry> JoinOrderOptimizer::solveGreedy() {
  ...
  while (components.size() > 1) {
    ...
    for (size_t i = 0; i < components.size(); i++) {
      for (size_t j = i + 1; j < components.size(); j++) {
        ...
        if (!best_plan || current_cost < best_plan->cost) {
          if (!edge.empty() && join_kind == JoinKind::Cross) join_kind = JoinKind::Inner;
    ...
    if (!best_plan) {
      ...
      if (best_i == best_j) throw Exception(ErrorCodes::LOGICAL_ERROR, "Cannot find two smallest components");
      ...
      best_plan = std::make_shared<DPJoinEntry>(components[std::min(best_i, best_j)], components[std::max(best_i, best_j)], cost,
                                                cardinality, join_operator);
      applied_edge.clear();
    }

    if (best_i == best_j) throw Exception(ErrorCodes::LOGICAL_ERROR, "Cannot find components to join");

    LOG_TEST(log, "Best plan for '{}' as '{} JOIN {}', cost: {}, cardinality: {}, join operator: {}", best_plan->dump(),
             best_plan->left->dump(), best_plan->right->dump(), best_plan->cost,
             best_plan->estimated_rows ? toString(*best_plan->estimated_rows) : "unknown", best_plan->join_operator.dump());
    ...
    for (auto *edge : applied_edge) *edge = nullptr;
  }
...
std::shared_ptr<DPJoinEntry> JoinOrderOptimizer::solveDPsize() {
    ...
    for (size_t smaller_component_size = 1; smaller_component_size <= component_size / 2; ++smaller_component_size) {
      ...
      for (const auto &[_, right] : components[smaller_component_size]) {
        ...
        for (const auto &[_, left] : components[bigger_component_size]) {
          ...
          if (left->relations & right->relations) continue;

          /// If both components are of the same size then check each pair just once, not twice
          if (smaller_component_size == bigger_component_size && *left->relations.begin() > *right->relations.begin()) continue;
          ...
          /// FIXME: Restrict to Inner joins for now because isValidJoinOrder seems to not handle non-Inner joins with swapped inputs
          /// correctly
          if (*join_kind != JoinKind::Inner) continue;
          ...
          if (current_best == dp_table.end() || new_cost < current_best->second->cost) {
            ...
            LOG_TEST(log, "New best plan for '{}' as '{} JOIN {}', cost: {}, cardinality: {}, operator: {}", new_best_plan->dump(),
                     new_best_plan->left->dump(), new_best_plan->right->dump(), new_best_plan->cost,
                     new_best_plan->estimated_rows ? toString(*new_best_plan->estimated_rows) : "unknown",
  ...
  if (best_full_plan != dp_table.end()) return best_full_plan->second;
...
std::optional<JoinKind> JoinOrderOptimizer::isValidJoinOrder(const BitSet &left_mask, const BitSet &right_mask) const {
  auto check = [&](const auto &lhs, const auto &rhs) -> std::optional<JoinKind> {
    ...
    if (rel_id.has_value()) {
      ...
      if (it != query_graph.join_kinds.end()) {
        if (isSubsetOf(it->second.first, rhs)) return it->second.second;
        return {};
      }
    }
  ...
  if (left_join_type == JoinKind::Inner) return right_join_type;
  if (right_join_type == JoinKind::Inner) return left_join_type;
  /// Allow FULL join as it's restricted to table swapping and no reordering
  if (left_join_type == JoinKind::Full && right_join_type == JoinKind::Full) return JoinKind::Full;
...
DPJoinEntryPtr optimizeJoinOrder(QueryGraph query_graph, const QueryPlanOptimizationSettings &optimization_settings) {
  ...
  if (!best_plan) throw Exception(ErrorCodes::LOGICAL_ERROR, "Failed to find a valid join order");
