...
struct ConditionList {
  enum ConditionType { SingleCondition, And, Or };
  ...
  bool assumedTrue() const {
    return existing_nodes.empty() && (subconditions.empty() || (subconditions.size() == 1 && subconditions.front().assumedTrue()));
  }
};
...
bool onlyDependsOnAvailableColumns(const ActionsDAG::Node &node, const NameSet &available_columns) {
  if (node.type == ActionsDAG::ActionType::INPUT) {
    ...
    for (const auto *child : node.children) {
      if (!onlyDependsOnAvailableColumns(*child, available_columns)) return false;
    }
    return true;
  }
}
...
ConditionList extractPartialPredicate(const ActionsDAG::Node &node, const NameSet &available_columns) {
  ...
  if (is_conjunction) {
    ...
    for (const auto *child : node.children) {
      ...
      if (!child_condition_template.assumedTrue()) conjuction_template.subconditions.emplace_back(std::move(child_condition_template));
    }
    ...
    if (!onlyDependsOnAvailableColumns(node, available_columns))
      return ConditionList{};  /// If the subtree depends on other columns then we cannot include it into partial predicate, so assume it's
                               /// True

    return ConditionList{.condition_type = ConditionList::SingleCondition, .existing_nodes = {&node}, .subconditions = {}};
  }
}

const ActionsDAG::Node *buildPredicateFromTemplate(ActionsDAG &full_dag, const ConditionList &predicate_template) {
  if (predicate_template.assumedTrue()) return nullptr;
  ...
  for (const auto &subcondition : predicate_template.subconditions) {
    ...
    if (subcondition_node) children.push_back(subcondition_node);
  }
  ...
  FunctionOverloadResolverPtr function = predicate_template.condition_type == ConditionList::And
...
std::optional<ActionsDAG> tryToExtractPartialPredicate(const ActionsDAG &original_dag, const std::string &filter_name,
  ...
  if (!original_dag.tryFindInOutputs(filter_name)) return {};
  ...
  if (full_dag.hasArrayJoin()) return {};
...
void addFilterOnTop(QueryPlan::Node &join_node, size_t child_idx, QueryPlan::Nodes &nodes, ActionsDAG filter_dag) {
  ...
  for (const auto *input : filter_dag.getInputs()) filter_inputs.insert({input->result_name, input});
