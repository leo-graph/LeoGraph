...
UInt64 calculateHashFromStep(const SourceStepWithFilter &read) {
  ...
  if (const auto &dag = read.getPrewhereInfo()) dag->prewhere_actions.updateHash(hash);
  return hash.get64();
}
...
UInt64 calculateHashFromStep(const ITransformingStep &transform) {
  ...
  if (!transform.getTransformTraits().preserves_number_of_rows) {
    ...
    if (transform.isSerializable()) transform.serialize(ctx);
...
UInt64 calculateHashFromStep(const JoinStepLogical &join_step, JoinTableSide side) {
  ...
  for (const auto &condition : join_step.getJoinOperator().expression) {
    ...
    if (op == JoinConditionOperator::Equals || op == JoinConditionOperator::NullSafeEquals) {
      if (side == JoinTableSide::Left && lhs.fromLeft()) lhs.getNode()->updateHash(hash);
      if (side == JoinTableSide::Left && rhs.fromLeft()) rhs.getNode()->updateHash(hash);
      if (side == JoinTableSide::Right && lhs.fromRight()) lhs.getNode()->updateHash(hash);
      if (side == JoinTableSide::Right && rhs.fromRight()) rhs.getNode()->updateHash(hash);
    }
  }
...
void calculateHashTableCacheKeys(const QueryPlan::Node &root, std::unordered_map<const QueryPlan::Node *, UInt64> &cache_keys) {
  ...
  while (!stack.empty()) {
    ...
    if (auto *join_step = dynamic_cast<JoinStepLogical *>(node.step.get())) {
      // `HashTablesStatistics` is used currently only for `parallel_hash_join`, i.e. the following calculation doesn't make sense for other
      // join algorithms.
      const auto &join_expression = join_step->getJoinOperator().expression;
      bool single_disjunct =
          join_expression.size() > 1 || (join_expression.size() == 1 && !join_expression.front().isFunction(JoinConditionOperator::Or));
      const bool calculate = allowParallelHashJoin(join_step->getJoinSettings().join_algorithms, join_step->getJoinOperator().kind,
                                                   join_step->getJoinOperator().strictness,
                                                   typeid_cast<JoinStepLogicalLookup *>(node.children.back()->step.get()), single_disjunct);
    ...
    for (const auto *child : node.children) frame.hash.update(cache_keys[child]);
    ...
    if (const auto *source = dynamic_cast<const ReadFromParallelRemoteReplicasStep *>(node.step.get()))
      ...
      if (auto hash = calculateHashFromStep(*transform)) frame.hash.update(hash);
