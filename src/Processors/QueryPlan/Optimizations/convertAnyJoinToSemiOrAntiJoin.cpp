...
FilterResult filterResultForMatchedRows(ActionsDAG pre_actions_dag, const ActionsDAG &filter_dag, const String &filter_column_name) {
  ...
  try {
    filter_output = ActionsDAG::evaluatePartialResult(combined_dag_input, {combined_dag.tryFindInOutputs(filter_column_name)},
                                                      /*input_rows_count=*/1, {.skip_materialize = true});
...
bool isSimpleFilter(const ActionsDAG::Node *filter_node, bool must_negate) {
  /// Skip aliases
  while (filter_node->type == ActionsDAG::ActionType::ALIAS) filter_node = filter_node->children.front();
...
QueryPlanStepPtr convertToExpressionStep(FilterStep *filter_node) {
  ...
  if (filter_node->removesFilterColumn()) dag.removeFromOutputs(filter_node->getFilterColumnName());
...
enum class JoinSide { Left, Right };

template <JoinSide join_side_to_check>
ActionsDAG getPreFilterActionsDAG(QueryPlan::Node *join_node, JoinStepLogical *join) {
  if (auto *expression_before_join_step =
          typeid_cast<ExpressionStep *>(join_node->children[join_side_to_check == JoinSide::Left ? 0 : 1]->step.get())) {
...
size_t tryConvertAnyJoinToSemiOrAntiJoin(QueryPlan::Node *parent_node, QueryPlan::Nodes & /*nodes*/,
  ...
  if (join_operator.strictness != JoinStrictness::Any) return 0;

  if (!isLeftOrRight(join_operator.kind)) return 0;
  ...
  switch (join_operator.kind) {
    case JoinKind::Left: {
      ...
      auto result_for_matched_rows =
          filterResultForMatchedRows(getPreFilterActionsDAG<JoinSide::Right>(child_node, join), filter_dag, filter_column_name);
      ...
      if (result_for_not_matched_rows == FilterResult::FALSE) {
        ...
        if (result_for_matched_rows == FilterResult::TRUE && isSimpleFilter(filter_dag.tryFindInOutputs(filter_column_name), false)) {
          ...
          LOG_DEBUG(getLogger("QueryPlanConvertAnyJoinToSemiOrAntiJoin"),
                    "Removing filter '{}' after SEMI JOIN because it's always TRUE for matched rows", filter_column_name);
      ...
      if (result_for_matched_rows == FilterResult::FALSE) {
        ...
        if (result_for_not_matched_rows == FilterResult::TRUE && isSimpleFilter(filter_dag.tryFindInOutputs(filter_column_name), true)) {
          ...
          LOG_DEBUG(getLogger("QueryPlanConvertAnyJoinToSemiOrAntiJoin"),
                    "Removing filter '{}' after ANTI JOIN because it's always TRUE for not matched rows", filter_column_name);
          parent_node->step = convertToExpressionStep(filter);
        }
        return 2;
      }
    ...
    case JoinKind::Right: {
      ...
      auto result_for_matched_rows =
          filterResultForMatchedRows(getPreFilterActionsDAG<JoinSide::Left>(child_node, join), filter_dag, filter_column_name);
