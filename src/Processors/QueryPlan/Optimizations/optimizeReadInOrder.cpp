...
ISourceStep *checkSupportedReadingStep(IQueryPlanStep *step, bool allow_existing_order) {
  if (auto *reading = typeid_cast<ReadFromMergeTree *>(step)) {
    ...
    if (!allow_existing_order && reading->getQueryInfo().input_order_info) return nullptr;

    const auto &sorting_key = reading->getStorageMetadata()->getSortingKey();
    if (sorting_key.column_names.empty()) return nullptr;

    return reading;
  }

  if (auto *merge = typeid_cast<ReadFromMerge *>(step)) {
    if (const auto &order = merge->getInputOrder(); !allow_existing_order && order && order->used_prefix_of_sorting_key_size)
      return nullptr;
...
QueryPlan::Node *findReadingStep(QueryPlan::Node &node, FindReadingStepContext &data) {
  ...
  if (checkSupportedReadingStep(step, data.allow_existing_order) != nullptr) return &node;
  ...
  if (data.read_in_order_through_join) {
    ...
    if (const auto *join_step = typeid_cast<const JoinStep *>(step)) join_ptr = join_step->getJoin().get();
    if (const auto *join_step = typeid_cast<const FilledJoinStep *>(step)) join_ptr = join_step->getJoin().get();
    ...
    if (join_ptr) {
      ...
      if ((strictness == JoinStrictness::Any || strictness == JoinStrictness::All) && isInnerOrLeft(kind)) {
        ...
        if (auto *join_step = typeid_cast<JoinStep *>(step); reading_step && join_step) data.joins_to_keep_in_order.push_back(join_step);
        return reading_step;
      }
    }
  }
    ...
    if (node->type == ActionsDAG::ActionType::FUNCTION) {
      ...
      if (name == "and") {
        ...
        if (maybe_fixed_column && num_constant_columns + 1 == node->children.size()) {
          // std::cerr << "====== Added fixed column " << maybe_fixed_column->result_name << ' ' << static_cast<const void
          // *>(maybe_fixed_column) << std::endl;
          ...
          while (maybe_injective->type == ActionsDAG::ActionType::FUNCTION && maybe_injective->children.size() == 1 &&
...
void buildSortingDAG(QueryPlan::Node &node, std::optional<ActionsDAG> &dag, FixedColumns &fixed_columns, size_t &limit) {
  ...
  if (auto *expression = typeid_cast<ExpressionStep *>(step)) {
    ...
    if (actions.hasArrayJoin()) limit = 0;
  ...
  if (auto *array_join = typeid_cast<ArrayJoinStep *>(step)) {
    ...
    if (dag) {
      ...
      for (const auto &output : dag->getOutputs()) {
        if (!keys_set.contains(output->result_name)) outputs.push_back(output);
      }
...
void enrichFixedColumns(const ActionsDAG &dag, FixedColumns &fixed_columns) {
  ...
  for (const auto *node : fixed_columns) {
    if (node->type == ActionsDAG::ActionType::INPUT) fixed_input_names.insert(node->result_name);
  }

  /// Add all INPUT nodes with matching names to fixed_columns.
  for (const auto &node : dag.getNodes()) {
    if (node.type == ActionsDAG::ActionType::INPUT && fixed_input_names.contains(node.result_name)) fixed_columns.insert(&node);
  }
  ...
  for (const auto &node : dag.getNodes()) {
    ...
    while (!stack.empty()) {
      ...
      for (; frame.next_child < frame.node->children.size(); ++frame.next_child)
        if (!visited.contains(frame.node->children[frame.next_child])) break;
      ...
      if (frame.next_child < frame.node->children.size()) {
        ...
        if (!frame.node->column) {
          if (frame.node->type == ActionsDAG::ActionType::ALIAS) {
            if (fixed_columns.contains(frame.node->children.at(0))) fixed_columns.insert(frame.node);
            ...
            if (frame.node->function_base->isDeterministicInScopeOfQuery()) {
              ...
              for (const auto *child : frame.node->children) {
                if (!child->column && !fixed_columns.contains(child)) {
                  // std::cerr << "*** enrichFixedColumns fail " << child->result_name <<  ' ' << static_cast<const void *>(child) <<
                  // std::endl;
              ...
              if (all_args_fixed_or_const) {
                // std::cerr << "*** enrichFixedColumns add " << frame.node->result_name << ' ' << static_cast<const void *>(frame.node) <<
                // std::endl;
...
const ActionsDAG::Node *addMonotonicChain(ActionsDAG &dag, const ActionsDAG::Node *node, const MatchedTrees::Match *match,
                                          const std::string &input_name) {
  if (!match->monotonicity) return &dag.addInput(input_name, node->result_type);
...
SortingInputOrder buildInputOrderFromSortDescription(const FixedColumns &fixed_columns, const std::optional<ActionsDAG> &dag,
                                                     const SortDescription &description, const KeyDescription &sorting_key,
                                                     const Names &pk_column_names, size_t limit) {
  ...
  if (dag) {
    ...
    for (const auto &[node, match] : matches) {
      ...
      if (!match.monotonicity || match.monotonicity->strict) {
        if (match.node && fixed_columns.contains(node)) fixed_key_columns.insert(match.node);
      }
    }
  ...
  while (next_description_column < description.size() && next_sort_key < sorting_key.column_names.size()) {
    ...
    if (sort_column_description.collator) break;
    ...
    const auto column_is_nullable =
        isNullableOrLowCardinalityNullable(sorting_key.data_types[next_sort_key]) || isFloat(*sorting_key.data_types[next_sort_key]);
    if (column_is_nullable && sort_column_description.nulls_direction == -1) break;
    ...
    if (!sort_column_node) break;

    if (!dag) {
      /// This is possible if there were no Expression or Filter steps in Plan.
      /// Example: SELECT * FROM tab ORDER BY a, b

      if (sort_column_node->type != ActionsDAG::ActionType::INPUT) break;

      if (sort_column_description.column_name != sorting_key_column) break;
      ...
      // std::cerr << "====== Finding match for " << sort_column_node->result_name << ' ' << static_cast<const void *>(sort_column_node) <<
      // std::endl;
      ...
      if (match.node && match.node == sort_column_node) {
        ...
        // std::cerr << "====== Check for fixed const : " << bool(sort_node->column) << " fixed : " << fixed_columns.contains(sort_node) <<
        // std::endl;
        bool is_fixed_column = sort_node->column || fixed_columns.contains(sort_node);
        if (!is_fixed_column) break;
    ...
    if (current_direction && read_direction && current_direction != read_direction) break;

    if (read_direction == 0) read_direction = current_direction;

    if (current_direction) order_key_prefix_descr.push_back(sort_column_description);

    if (current_direction && !strict_monotonic) break;
  }

  if (order_key_prefix_descr.empty()) return {};
  ...
  if (order_key_prefix_descr.size() < description.size() || pk_column_names.size() < next_sort_key) can_optimize_virtual_row = false;
...
InputOrder buildInputOrderFromUnorderedKeys(const FixedColumns &fixed_columns, const std::optional<ActionsDAG> &dag,
                                            const Names &unordered_keys, const ActionsDAG &sorting_key_dag,
  ...
  if (dag) {
    ...
    for (const auto *output : dag->getOutputs()) {
      ...
      if (match->node) {
        ...
        if (!not_matched_keys.contains(output->result_name)) continue;
  ...
  while (!not_matched_keys.empty() && next_sort_key < sorting_key_columns.size()) {
    ...
    if (!dag) {
      ...
      if (group_by_key_it == not_matched_keys.end()) break;
      ...
      if (match && match->node) group_by_key_it = not_matched_keys.find(group_by_key_node->result_name);
    ...
    if (read_direction == 0 && current_direction != 0) read_direction = current_direction;
  ...
  if (read_direction == 0 || sort_description.empty()) return {};
...
SortingInputOrder buildInputOrderFromSortDescription(const ReadFromMergeTree *reading, const FixedColumns &fixed_columns,
                                                     const std::optional<ActionsDAG> &dag, const SortDescription &description,
                                                     size_t limit) {
  const auto &sorting_key = reading->getStorageMetadata()->getSortingKey();
  const auto &pk_column_names = reading->getStorageMetadata()->getPrimaryKey().column_names;

  return buildInputOrderFromSortDescription(fixed_columns, dag, description, sorting_key, pk_column_names, limit);
}

SortingInputOrder buildInputOrderFromSortDescription(const ReadFromObjectStorageStep *reading, const FixedColumns &fixed_columns,
                                                     const std::optional<ActionsDAG> &dag, const SortDescription &description,
                                                     size_t limit) {
  const auto &sorting_key = reading->getStorageMetadata()->getSortingKey();
...
SortingInputOrder buildInputOrderFromSortDescription(ReadFromMerge *merge, const FixedColumns &fixed_columns,
                                                     const std::optional<ActionsDAG> &dag, const SortDescription &description,
                                                     size_t limit) {
  const auto &tables = merge->getSelectedTables();
  ...
  for (const auto &table : tables) {
    ...
    if (sorting_key.column_names.empty()) return {};

    auto table_order_info = buildInputOrderFromSortDescription(fixed_columns, dag, description, sorting_key, {}, limit);

    if (!table_order_info.input_order) return {};
...
InputOrder buildInputOrderFromUnorderedKeys(ReadFromMergeTree *reading, const FixedColumns &fixed_columns,
                                            const std::optional<ActionsDAG> &dag, const Names &unordered_keys) {
  const auto &sorting_key = reading->getStorageMetadata()->getSortingKey();
  const auto &sorting_key_columns = sorting_key.column_names;

  return buildInputOrderFromUnorderedKeys(fixed_columns, dag, unordered_keys, sorting_key.expression->getActionsDAG(), sorting_key_columns);
}

InputOrder buildInputOrderFromUnorderedKeys(ReadFromObjectStorageStep *reading, const FixedColumns &fixed_columns,
                                            const std::optional<ActionsDAG> &dag, const Names &unordered_keys) {
  const auto &sorting_key = reading->getStorageMetadata()->getSortingKey();
  const auto &sorting_key_columns = sorting_key.column_names;
...
InputOrder buildInputOrderFromUnorderedKeys(ReadFromMerge *merge, const FixedColumns &fixed_columns, const std::optional<ActionsDAG> &dag,
  ...
  for (const auto &table : tables) {
    ...
    if (sorting_key_columns.empty()) return {};

    auto table_order_info =
        buildInputOrderFromUnorderedKeys(fixed_columns, dag, unordered_keys, sorting_key.expression->getActionsDAG(), sorting_key_columns);
...
InputOrderInfoPtr buildInputOrderInfo(SortingStep &sorting, bool &apply_virtual_row, QueryPlan::Node &node,
  ...
  if (!reading_node) return nullptr;
  ...
  if (dag && !fixed_columns.empty()) enrichFixedColumns(*dag, fixed_columns);
  ...
  if (auto *reading = typeid_cast<ReadFromMergeTree *>(reading_node->step.get())) {
    ...
    if (reading->isParallelReadingFromReplicas() && !find_reading_ctx.joins_to_keep_in_order.empty()) return nullptr;
    ...
    if (order_info.input_order) {
      ...
      bool can_read = reading->requestReadingInOrder(order_info.input_order->used_prefix_of_sorting_key_size,
                                                     order_info.input_order->direction, order_info.input_order->limit);

      if (!can_read) return nullptr;

      for (auto *join_step : find_reading_ctx.joins_to_keep_in_order) join_step->keepLeftPipelineInOrder(/* disable_squashing */ true);
    }
...
InputOrder buildInputOrderInfo(AggregatingStep &aggregating, QueryPlan::Node &node,
  ...
  if (auto *reading = typeid_cast<ReadFromMergeTree *>(reading_node->step.get())) {
    ...
    if (reading->isParallelReadingFromReplicas() && !find_reading_ctx.joins_to_keep_in_order.empty()) return {};
  ...
  if (auto *object_storage_step = typeid_cast<ReadFromObjectStorageStep *>(reading_node->step.get())) {
    auto order_info = buildInputOrderFromUnorderedKeys(object_storage_step, fixed_columns, dag, keys);
...
bool canImproveOrderForDistinct(InputOrder &required_order, const InputOrderInfoPtr &existing_order) {
  if (!required_order.input_order) return false;

  if (!existing_order) return true;

  /// We only allow improving existing order.
  /// In order to reuse previous order, applySorting.cpp is used.
  if (required_order.input_order->used_prefix_of_sorting_key_size <= existing_order->used_prefix_of_sorting_key_size) return false;

  if (existing_order && required_order.input_order->direction != existing_order->direction) {
    /// Take read direction from existing order.
    for (auto &column : required_order.sort_description) column.direction *= -1;

    required_order.input_order = std::make_shared<InputOrderInfo>(required_order.input_order->sort_description_for_merging,
                                                                  required_order.input_order->used_prefix_of_sorting_key_size,
                                                                  existing_order->direction, existing_order->limit);
  }
...
InputOrder buildInputOrderInfo(DistinctStep &distinct, QueryPlan::Node &node, const QueryPlanOptimizationSettings &optimization_settings) {
  ...
  if (auto *reading = typeid_cast<ReadFromMergeTree *>(reading_node->step.get())) {
    ...
    if (!canImproveOrderForDistinct(order_info, reading->getInputOrder())) return {};

    if (!reading->requestReadingInOrder(order_info.input_order->used_prefix_of_sorting_key_size, order_info.input_order->direction,
  ...
  if (auto *merge = typeid_cast<ReadFromMerge *>(reading_node->step.get())) {
    ...
    if (!canImproveOrderForDistinct(order_info, merge->getInputOrder())) return {};

    if (!merge->requestReadingInOrder(order_info.input_order)) return {};
  ...
  if (auto *object_storage_step = typeid_cast<ReadFromObjectStorageStep *>(reading_node->step.get())) {
    ...
    if (!canImproveOrderForDistinct(order_info, object_storage_step->getDataOrder())) return {};

    if (!object_storage_step->requestReadingInOrder()) return {};
...
void optimizeReadInOrder(QueryPlan::Node &node, QueryPlan::Nodes &nodes, const QueryPlanOptimizationSettings &optimization_settings) {
  ...
  if (sorting->getType() != SortingStep::Type::Full) return;

  if (sorting->hasPartitions() && !optimization_settings.reuse_storage_ordering_for_window_functions) return;
  ...
  if (typeid_cast<UnionStep *>(node.children.front()->step.get())) {
    ...
    for (const auto *child : union_node->children) {
      ...
      if (readingFromParallelReplicas(child)) return;
    }
    ...
    if (!max_sort_descr || max_sort_descr->empty()) return;
    ...
    for (size_t i = 0; i < infos.size(); ++i) {
      ...
      if (!info) {
        ...
        additional_sorting = std::make_unique<SortingStep>(child->step->getOutputHeader(), descr,
                                                           limit,  /// TODO: support limit with ties
                                                           sorting->getSettings());
      } else if (info->sort_description_for_merging.size() < max_sort_descr->size()) {
        additional_sorting = std::make_unique<SortingStep>(child->step->getOutputHeader(), info->sort_description_for_merging,
                                                           *max_sort_descr, sorting->getSettings().max_block_size,
...
void optimizeAggregationInOrder(QueryPlan::Node &node, QueryPlan::Nodes &, const QueryPlanOptimizationSettings &optimization_settings) {
  ...
  if ((aggregating->inOrder() && !aggregating->explicitSortingRequired()) || aggregating->isGroupingSets()) return;

  /// It just does not work, see 02515_projections_with_totals
  if (aggregating->getParams().overflow_row) return;
  ...
  if (order_info.input_order) {
    ...
    for (const auto &desc : order_info.sort_description) used_keys.insert(desc.column_name);
    ...
    for (const auto &key : aggregating->getParams().keys)
      if (used_keys.emplace(key).second) group_by_sort_description.push_back(SortColumnDescription(std::string(key)));
...
void optimizeDistinctInOrder(QueryPlan::Node &node, QueryPlan::Nodes &, const QueryPlanOptimizationSettings &optimization_settings) {
  ...
  if (!distinct->isPreliminary()) return;

  if (!distinct->getSortDescription().empty()) return;

  auto order_info = buildInputOrderInfo(*distinct, *node.children.front(), optimization_settings);
  if (order_info.input_order) distinct->applyOrder(std::move(order_info.sort_description));
}
...
size_t tryReuseStorageOrderingForWindowFunctions(QueryPlan::Node *parent_node, QueryPlan::Nodes & /*nodes*/,
  ...
  if (typeid_cast<ExpressionStep *>(possible_read_from_merge_tree_node->step.get())) {
    if (possible_read_from_merge_tree_node->children.size() != 1) return 0;
  ...
  if (!query_info.syntax_analyzer_result) return 0;
  ...
  auto order_optimizer = std::make_shared<ReadInOrderOptimizer>(
      *select_query, order_by_elements_actions, window->getWindowDescription().full_sort_description, query_info.syntax_analyzer_result);

  /// If we don't have filtration, we can pushdown limit to reading stage for optimizations.
  UInt64 limit =
      (select_query->hasFiltration() || select_query->groupBy()) ? 0 : InterpreterSelectQuery::getLimitForSorting(*select_query, context);
