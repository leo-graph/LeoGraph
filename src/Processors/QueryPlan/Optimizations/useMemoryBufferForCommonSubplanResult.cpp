...
void useMemoryBufferForCommonSubplanResult(QueryPlan::Node &node, const QueryPlanOptimizationSettings &settings) {
  ...
  node.step = std::make_unique<ReadFromCommonBufferStep>(subplan_reference->getOutputHeader(), common_buffer, settings.max_threads);
  subplan_reference_root->step = std::make_unique<SaveSubqueryResultToBufferStep>(subplan_reference_root->step->getOutputHeader(),
                                                                                  std::move(columns_to_use), common_buffer);
}
