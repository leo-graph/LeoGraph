...
size_t tryConvertOuterJoinToInnerJoinLegacy(QueryPlan::Node *parent_node, QueryPlan::Nodes &) {
  ...
  if (!join || !join->getJoin()->isCloneSupported()) return 0;
  ...
  if (table_join.strictness() != JoinStrictness::All) return 0;

  /// TODO: Support join_use_nulls
  if (table_join.joinUseNulls()) return 0;
  ...
  if (!check_left_stream && !check_right_stream) return 0;
  ...
  if (!left_stream_safe || !right_stream_safe) return 0;
...
size_t tryConvertAnyOuterJoinToInnerJoin(FilterStep *filter, QueryPlan::Node *join_node, JoinStepLogical *join_step) {
  ...
  auto result_for_not_matched_rows =
      filterResultForNotMatchedRows(filter->getExpression(), filter->getFilterColumnName(), *interesting_side_header,
  ...
  if (result_for_not_matched_rows != FilterResult::FALSE) return 0;
  ...
  const auto add_key_recursively = [&](const ActionsDAG::Node *node) -> void {
    if (node->type == ActionsDAG::ActionType::FUNCTION) {
      ...
      while (!nodes_to_process.empty()) {
        ...
        if (current_node->type == ActionsDAG::ActionType::INPUT) join_keys_interesting_side.insert(current_node->result_name);

        if (current_node->type == ActionsDAG::ActionType::FUNCTION && current_node->function_base->isInjective({})) {
          for (const auto *child : current_node->children) nodes_to_process.push(child);
        }
      }
      return;
    }
  ...
  for (const auto &expr : join_step->getJoinOperator().expression) {
    ...
    if (predicate_op != JoinConditionOperator::Equals) return 0;
  ...
  if (join_keys_interesting_side.empty()) return 0;
    ...
    for (const auto &output : expr_step->getExpression().getOutputs()) {
      if (output->type == ActionsDAG::ActionType::ALIAS) {
        if (join_keys_interesting_side.contains(output->result_name)) {
          ...
          while (current_node->type == ActionsDAG::ActionType::ALIAS) current_node = current_node->children.front();
  ...
  if (!aggregating_step) return 0;

  /// GROUPING SETS does not guarantee unique rows.
  if (aggregating_step->isGroupingSets()) return 0;
  ...
  for (const auto &aggregation_key : aggregating_step->getParams().keys) {
    if (!join_keys_interesting_side.contains(aggregation_key)) return 0;
  }
...
size_t tryConvertOuterJoinToInnerJoin(QueryPlan::Node *parent_node, QueryPlan::Nodes &nodes,
  ...
  if (size_t legacy_result = tryConvertOuterJoinToInnerJoinLegacy(parent_node, nodes); legacy_result > 0) return legacy_result;
  ...
  if (!join || child_node->children.size() != 2) return 0;
  ...
  if (isStorageJoin(child_node->children.back()->step)) return 0;

  auto &join_operator = join->getJoinOperator();
  if (join_operator.strictness == JoinStrictness::Any) return tryConvertAnyOuterJoinToInnerJoin(filter, child_node, join);
  if (join_operator.strictness != JoinStrictness::All) return 0;
