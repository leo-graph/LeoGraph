...
MatchedTrees::Matches matchTrees(const ActionsDAG::NodeRawConstPtrs &inner_dag, const ActionsDAG &outer_dag, bool check_monotonicity) {
...

  {
    ...
    for (const auto *out : inner_dag) {
      if (inner_parents.contains(out)) continue;
      ...
      while (!stack.empty()) {
        ...
        if (node->type == ActionsDAG::ActionType::INPUT) inner_inputs.emplace(node->result_name, node);

        for (const auto *child : node->children) {
          auto [it, inserted] = inner_parents.emplace(child, Parents());
          it->second.emplace(node);

          if (inserted) stack.push(child);
        }
      }
    }
  }
  ...
  for (const auto &node : outer_dag.getNodes()) {
    if (matches.contains(&node)) continue;
    ...
    while (!stack.empty()) {
      ...
      if (frame.mapped_children.size() < frame.node->children.size()) continue;
      ...
      if (frame.node->type == ActionsDAG::ActionType::INPUT) {
        ...
        if (auto it = inner_inputs.find(frame.node->result_name); it != inner_inputs.end()) mapped = it->second;
        ...
        if (found_all_children && any_child) {
          ...
          if (frame.mapped_children.size() > 1) {
            ...
            for (size_t i = 1; i < mapped_children_size; ++i)
              if (frame.mapped_children[i]) other_parents.push_back(&inner_parents[frame.mapped_children[i]]);
            ...
            for (const auto *parent : *intersection) {
              ...
              if (is_common) container.insert(parent);
            }
                  ...
                  for (size_t i = 0; all_children_matched && i < num_children; ++i) {
                    if (frame.mapped_children[i] == nullptr) {
                      all_children_matched = children[i]->column && isColumnConst(*children[i]->column) &&
                                             ...
                                             assert_cast<const ColumnConst &>(*children[i]->column).getField() ==
            ...
            if (child_match.node) {
              ...
              if (info.is_monotonic) {
                ...
                if (child_match.monotonicity) {
                  ...
                  if (!child_match.monotonicity->strict) monotonicity.strict = false;
                }
...
static PossiblyMonotonicChain buildPossiblyMonitinicChain(const ActionsDAG::Node *node) {
  ...
  while (node->type != ActionsDAG::ActionType::INPUT) {
    ...
    if (node->type != ActionsDAG::ActionType::FUNCTION) break;

    size_t num_children = node->children.size();
    if (num_children == 0) break;

    const auto &func = node->function_base;
    if (!func->hasInformationAboutMonotonicity()) break;

    std::optional<size_t> non_const_arg;
    for (size_t i = 0; i < num_children; ++i) {
      const auto *child = node->children[i];
      if (child->type == ActionsDAG::ActionType::COLUMN) continue;
    ...
    if (non_const_arg == std::nullopt) break;

    chain.push_back(*non_const_arg);
    node = node->children[*non_const_arg];
  }

  if (node->type != ActionsDAG::ActionType::INPUT) return {};
...
bool isMonotonicChain(const ActionsDAG::Node *node, PossiblyMonotonicChain &chain) {
  ...
  while (node != chain.input_node) {
    ...
    if (!monotonicity.is_monotonic) break;

    if (!monotonicity.is_positive) chain.changes_order = !chain.changes_order;
...
void applyActionsToSortDescription(SortDescription &description, const ActionsDAG &dag, const ActionsDAG::Node *output_to_skip) {
  if (description.empty()) return;

  if (dag.hasArrayJoin()) return;
...

  {
    ...
    for (size_t pos = 0; pos < descr_size; ++pos) desc_name_to_pos.emplace(description[pos].column_name, pos);
  ...
  for (const auto *output : dag.getOutputs()) {
    ...
    if (sort_column.output && !is_monotonicity_improved && sort_column.is_strict) break;

    if (has_functions && !isMonotonicChain(output, chain)) break;

    bool is_strictness_improved = chain.is_strict && !sort_column.is_strict;
    if (sort_column.output && !is_strictness_improved) break;
  ...
  while (prefix_size < descr_size) {
    ...
    if (sort_colunm.input && !sort_colunm.output) break;

    auto &descr = description[prefix_size];
    ++prefix_size;

    if (sort_colunm.output) descr.column_name = sort_colunm.output->result_name;

    if (sort_colunm.changes_order) descr.direction *= -1;

    if (!sort_colunm.is_strict) break;
  }
...
std::optional<std::unordered_map<const ActionsDAG::Node *, const ActionsDAG::Node *>> resolveMatchedInputs(
    const MatchedTrees::Matches &matches, const std::unordered_set<const ActionsDAG::Node *> &allowed_inputs,
  ...
  for (const auto *node : nodes) {
    ...
    while (!stack.empty()) {
      ...
      if (frame.node->type == ActionsDAG::ActionType::INPUT) return std::nullopt;
...
bool isInjectiveFunction(const ActionsDAG::Node *node) {
  if (node->function_base->isInjective({})) return true;

  size_t fixed_args = 0;
  for (const auto &child : node->children)
    if (child->type == ActionsDAG::ActionType::COLUMN) ++fixed_args;
...
NodeSet removeInjectiveFunctionsFromResultsRecursively(const ActionsDAG &actions) {
  NodeSet irreducible;
  NodeSet visited;
  for (const auto &node : actions.getOutputs()) removeInjectiveFunctionsFromResultsRecursively(node, irreducible, visited);
  return irreducible;
}
  ...
  switch (node->type) {
    ...
    case ActionsDAG::ActionType::FUNCTION:
      if (!isInjectiveFunction(node)) {
        ...
        for (const auto &child : node->children) removeInjectiveFunctionsFromResultsRecursively(child, irreducible, visited);
      }
      break;
...
bool allOutputsDependsOnlyOnAllowedNodes(const NodeSet &irreducible_nodes, const MatchedTrees::Matches &matches,
                                         const ActionsDAG::Node *node, NodeMap &visited) {
  ...
  if (matches.contains(node)) {
    ...
    if (match.node && !match.monotonicity) res = irreducible_nodes.contains(match.node);
  }
  ...
  if (!res) {
    switch (node->type) {
      ...
      case ActionsDAG::ActionType::FUNCTION:
        res = true;
        for (const auto &child : node->children) res &= allOutputsDependsOnlyOnAllowedNodes(irreducible_nodes, matches, child, visited);
        break;
      case ActionsDAG::ActionType::INPUT:
        break;
      case ActionsDAG::ActionType::PLACEHOLDER:
...
/// No need to explicitly check that each function is deterministic, because it is a guaranteed property of partition key expression
/// (checked on table creation). So it is left only to check that each output node depends only on the allowed set of nodes
/// (`irreducible_nodes`).
bool allOutputsDependsOnlyOnAllowedNodes(const ActionsDAG &partition_actions, const NodeSet &irreducible_nodes,
                                         const MatchedTrees::Matches &matches) {
  NodeMap visited;
  bool res = true;
  for (const auto &node : partition_actions.getOutputs())
    if (node->type != ActionsDAG::ActionType::INPUT) res &= allOutputsDependsOnlyOnAllowedNodes(irreducible_nodes, matches, node, visited);
  return res;
}
