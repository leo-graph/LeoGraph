...
inline const auto &getOptimizations() {
  static const std::array<Optimization, 18> optimizations = {{
      ...
      {tryReuseStorageOrderingForWindowFunctions, "reuseStorageOrderingForWindowFunctions",
      ...
      {tryAggregatePartitionsIndependently, "aggregatePartitionsIndependently",
      ...
      {tryConvertAnyJoinToSemiOrAntiJoin, "convertAnyJoinToSemiOrAntiJoin",
...
// Should be called once the query plan tree structure is finalized, i.e. no nodes addition, deletion or pushing down should happen after
// that call. Since those hashes are used for join optimization, the calculation performed before join optimization.
...
bool addPlansForSets(const QueryPlanOptimizationSettings &optimization_settings, QueryPlan &plan, QueryPlan::Node &node,
                     QueryPlan::Nodes &nodes);
