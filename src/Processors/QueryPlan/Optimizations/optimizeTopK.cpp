...
size_t tryOptimizeTopK(QueryPlan::Node *parent_node, QueryPlan::Nodes &nodes, const Optimization::ExtraSettings &settings) {
  ...
  if (!read_from_mergetree_step) return 0;

  size_t n = limit_step->getLimitForSorting();
  if (!n || (settings.max_limit_for_top_k_optimization && n > settings.max_limit_for_top_k_optimization)) return 0;

  SortingStep::Type sorting_step_type = sorting_step->getType();
  if (sorting_step_type != SortingStep::Type::Full) return 0;
  ...
  if (!sort_column.type->isValueRepresentedByNumber() || sort_column.type->isNullable()) return 0;

  const bool where_clause = filter_step || read_from_mergetree_step->getPrewhereInfo();

  /// remove alias
  if (sort_column_name.contains('.')) {
    if (!expression_step && !filter_step) return 0;
    ...
    if (unlikely(!column_node)) return 0;
  ...
  if ((settings.use_skip_indexes_for_top_k && read_from_mergetree_step->isSkipIndexAvailableForTopK(sort_column_name) &&
  ...
  if (settings.use_top_k_dynamic_filtering && !read_from_mergetree_step->getPrewhereInfo()) {
    ...
    const auto &prewhere_node =
        new_prewhere_info->prewhere_actions.addFunction(filter_function, {new_prewhere_info->prewhere_actions.getInputs().front()}, {});
    ...
    if (!blocksHaveEqualStructure(*initial_header, *updated_header)) {
      auto dag = ActionsDAG::makeConvertingActions(updated_header->getColumnsWithTypeAndName(), initial_header->getColumnsWithTypeAndName(),
  ...
  read_from_mergetree_step->setTopKColumn(
      {sort_column_name, sort_column.type, num_sort_columns, n, direction, where_clause, threshold_tracker});
