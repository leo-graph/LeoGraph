...
class RuntimeDataflowStatisticsCacheUpdater {
 ...
 public:
  RuntimeDataflowStatisticsCacheUpdater(size_t cache_key_, size_t total_rows_to_read_)
      : cache_key(cache_key_), total_rows_to_read(total_rows_to_read_) {
    if (cache_key == 0) throw Exception(ErrorCodes::BAD_ARGUMENTS, "Cache key for RuntimeDataflowStatisticsCacheUpdater cannot be zero");

    if (total_rows_to_read == 0) throw Exception(ErrorCodes::BAD_ARGUMENTS, "Total rows from storage cannot be zero");
  }
  ...
  void recordInputColumns(const ColumnsWithTypeAndName& input_columns, const NamesAndTypesList& part_columns,
                          const ColumnSizeByName& column_sizes, size_t read_bytes, std::optional<bool>& should_continue_sampling);
