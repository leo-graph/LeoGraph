...
/// 1. Optimization is applicable if partition by expression is a deterministic function of col1, ..., coln and group by key is injective
/// functions of these col1, ..., coln.
...
bool isPartitionKeySuitsGroupByKey(const ReadFromMergeTree &reading, const ActionsDAG &group_by_actions,
                                   const AggregatingStep &aggregating) {
  if (aggregating.isGroupingSets()) return false;

  if (group_by_actions.hasArrayJoin() || group_by_actions.hasStatefulFunctions() || group_by_actions.hasNonDeterministic()) return false;
  ...
  for (const auto &col : partition_actions.getRequiredColumnsNames())
    if (std::ranges::find(gb_key_required_columns, col) == gb_key_required_columns.end()) return false;
...
size_t tryAggregatePartitionsIndependently(QueryPlan::Node *node, QueryPlan::Nodes &, const Optimization::ExtraSettings & /*settings*/) {
  if (!node || node->children.size() != 1) return 0;
  ...
  if (const auto * /*filter*/ _ = typeid_cast<const FilterStep *>(maybe_reading_step)) {
    ...
    if (filter_node->children.size() != 1 || !filter_node->children.front()->step) return 0;
  ...
  if (!reading->willOutputEachPartitionThroughSeparatePort() &&
    ...
    if (reading->requestOutputEachPartitionThroughSeparatePort()) aggregating_step->skipMerging();
  }
