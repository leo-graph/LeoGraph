...
static size_t functionDoesNotChangeNumberOfValues(std::string_view function_name, size_t num_args) {
  if (function_name == "materialize" || function_name == "_CAST" || function_name == "CAST" || function_name == "toNullable") return 1;
  if (function_name == "firstNonDefault") return num_args;
  return 0;
}
...
NameSet backTrackColumnsInDag(const String &input_name, const ActionsDAG &actions) {
  ...
  for (const auto *node : actions.getInputs()) {
    if (input_name == node->result_name) input_nodes.insert(node);
  }
  ...
  for (const auto *out_node : actions.getOutputs()) {
    ...
    while (!nodes_to_process.empty()) {
      ...
      if (!inserted) {
        ...
        if (input_nodes.contains(node)) output_names.insert(out_node->result_name);
        break;
      }
...
void remapColumnStats(std::unordered_map<String, ColumnStats> &mapped, const ActionsDAG &actions) {
  ...
  for (const auto &[name, value] : original) {
    for (const auto &remapped : backTrackColumnsInDag(name, actions)) mapped[remapped] = value;
  }
}
...
struct RuntimeHashStatisticsContext {
  ...
  RuntimeHashStatisticsContext(const QueryPlanOptimizationSettings &optimization_settings, const QueryPlan::Node &root_node)
      : params{/*key_=*/0,
               ...
               optimization_settings.max_entries_for_hash_table_stats, optimization_settings.max_size_to_preallocate_for_joins} {
  ...
  UInt64 getCachedKey(const QueryPlan::Node *node) {
    if (auto it = cache_keys.find(node); it != cache_keys.end()) return it->second;
    return 0;
  }
  ...
  std::optional<size_t> getCachedHint(const QueryPlan::Node *node) {
    if (auto cache_key = getCachedKey(node)) {
      ...
      if (auto hint = hash_table_stats.getSizeHint(params.setKey(cache_key))) return hint->source_rows;
    }
    return {};
  }
};
...
RelationStats estimateAggregatingStepStats(const AggregatingStep &aggregating_step, const RelationStats &input_stats) {
  ...
  for (const auto &key : aggregator_params.keys) {
    ...
    if (input_stats.estimated_rows) key_number_of_distinct_values = std::min(key_number_of_distinct_values, *input_stats.estimated_rows);
    ...
    if (total_number_of_distinct_values) *total_number_of_distinct_values *= static_cast<Float64>(key_number_of_distinct_values);
  }
...
RelationStats estimateReadRowsCount(QueryPlan::Node &node, const ActionsDAG::Node *filter = nullptr) {
  ...
  if (const auto *reading = typeid_cast<const ReadFromMergeTree *>(step)) {
    ...
    if (reading->getContext()->getSettingsRef()[Setting::use_statistics]) {
      if (auto estimator = reading->getConditionSelectivityEstimator(reading->getAllColumnNames())) {
        ...
        const ActionsDAG::Node *prewhere_node =
            prewhere_info ? static_cast<const ActionsDAG::Node *>(
        ...
        RelationStats stats{
            .estimated_rows = relation_profile.rows, .column_stats = relation_profile.column_stats, .table_name = table_display_name};
        LOG_TRACE(getLogger("optimizeJoin"), "estimate statistics {}", dumpStatsForLogs(stats));
        return stats;
      }
    }
    if (auto dummy_stats = getDummyStats(reading->getContext(), table_display_name); !dummy_stats.table_name.empty()) return dummy_stats;
    ...
    if (!analyzed_result) return RelationStats{.estimated_rows = {}, .table_name = table_display_name};
    ...
    if (has_filter && !is_filtered_by_index) return RelationStats{.estimated_rows = {}, .table_name = table_display_name};
  ...
  if (const auto *join_step = typeid_cast<const JoinStepLogical *>(step); join_step && join_step->isOptimized()) {
    return RelationStats{
        .estimated_rows = join_step->getResultRowsEstimation(), .column_stats = {}, .table_name = join_step->getReadableRelationName()};
  }
  ...
  if (const auto *limit_step = typeid_cast<const LimitStep *>(step)) {
    ...
    if (!estimated.estimated_rows || estimated.estimated_rows > limit) estimated.estimated_rows = limit;
    return estimated;
  }
  ...
  if (const auto *sorting_step = typeid_cast<const SortingStep *>(step)) {
    ...
    if (sorting_step->getLimit()) {
      if (!stats.estimated_rows || stats.estimated_rows > sorting_step->getLimit()) stats.estimated_rows = sorting_step->getLimit();
    }
    return stats;
  }
...
bool optimizeJoinLegacy(QueryPlan::Node &node, QueryPlan::Nodes & /*nodes*/, const QueryPlanOptimizationSettings &) {
  ...
  if (!join_step || node.children.size() != 2 || join_step->isOptimized()) return false;

  const auto &join = join_step->getJoin();
  if (join->pipelineType() != JoinPipelineType::FillRightFirst || !join->isCloneSupported()) return true;
  ...
  if (table_join.kind() != JoinKind::Inner && !allow_outer_join) return true;
  ...
  if (table_join.hasUsing() || table_join.strictness() != JoinStrictness::All) return true;
  ...
  if (!join_step->swap_join_tables.has_value()) {
    ...
    LOG_TRACE(getLogger("optimizeJoinLegacy"), "Left table estimation: {}, right table estimation: {}",
              lhs_extimation ? toString(lhs_extimation.value()) : "unknown", rhs_extimation ? toString(rhs_extimation.value()) : "unknown");

    if (lhs_extimation && rhs_extimation && lhs_extimation < rhs_extimation) need_swap = true;
  } else if (join_step->swap_join_tables.value()) {
    need_swap = true;
  }

  if (!need_swap) return true;
  ...
  for (const auto &col : *original_output) {
    if (!swapped_algorithm_header.has(col.name)) return true;
  }
...
bool convertLogicalJoinToPhysical(QueryPlan::Node &node, QueryPlan::Nodes &nodes,
                                  const QueryPlanOptimizationSettings &optimization_settings) {
  bool keep_logical = optimization_settings.keep_logical_steps;
  if (keep_logical) return false;
  if (!typeid_cast<JoinStepLogical *>(node.step.get())) return false;
...
struct QueryGraphBuilder {
  ...
  explicit QueryGraphBuilder(std::shared_ptr<BuilderContext> context_) : context(std::move(context_)) {}
  ...
  bool hasCompatibleSettings(const JoinStepLogical &join_step) const {
    return context &&
           join_step.getJoinSettings() == context->join_settings;  // && join_step.getSortingSettings() == context->sorting_settings;
  }
};
...
void uniteGraphs(QueryGraphBuilder &lhs, QueryGraphBuilder rhs) {
  ...
  auto rhs_pinned_raw = std::ranges::to<std::unordered_map>(
      rhs.pinned | std::views::transform([](const auto &e) { return std::make_pair(e.first.getNode(), e.second); }));
  ...
  for (auto &[node, sources] : rhs_expression_sources) sources.shift(shift);
  ...
  for (auto &&[sources, nodes] : rhs.type_changes) lhs.type_changes[sources + shift] = std::move(nodes);

  for (auto [action, pin] : rhs_pinned_raw) lhs.pinned[JoinActionRef(action, lhs.expression_actions)] = pin + shift;
}

void buildQueryGraph(QueryGraphBuilder &query_graph, QueryPlan::Node &node, QueryPlan::Nodes &nodes, int join_steps_limit);

static String dumpStatsForLogs(const RelationStats &stats) {
  return fmt::format("{}: {} rows, columns: [{}]", stats.table_name.empty() ? "<unknown>" : stats.table_name,
                     stats.estimated_rows ? toString(stats.estimated_rows.value()) : "unknown",
                     fmt::join(stats.column_stats | std::views::transform([](const auto &p) {
...
static bool isTrivialStep(const QueryPlan::Node *node) {
  ...
  if (!expression_step) return false;

  return isPassthroughActions(expression_step->getExpression());
}

void optimizeJoinLogicalImpl(JoinStepLogical *join_step, QueryPlan::Node &node, QueryPlan::Nodes &nodes,
                             const QueryPlanOptimizationSettings &optimization_settings);

constexpr bool isInnerOrCross(JoinKind kind) { return kind == JoinKind::Inner || kind == JoinKind::Cross || kind == JoinKind::Comma; }

size_t addChildQueryGraph(QueryGraphBuilder &graph, QueryPlan::Node *node, QueryPlan::Nodes &nodes, const String &label,
                          int join_steps_limit) {
  if (isTrivialStep(node)) node = node->children[0];
  ...
  if (!label.empty()) stats.table_name = label;

  LOG_TRACE(getLogger("optimizeJoin"), "Estimated statistics{} for {} {}", num_rows_from_cache.has_value() ? " (from cache)" : "",
...
void buildQueryGraph(QueryGraphBuilder &query_graph, QueryPlan::Node &node, QueryPlan::Nodes &nodes, int join_steps_limit) {
  ...
  if (!join_step) throw Exception(ErrorCodes::LOGICAL_ERROR, "JoinStepLogical expected");
  if (node.children.size() != 2)
    throw Exception(ErrorCodes::LOGICAL_ERROR, "JoinStepLogical should have exactly 2 children, but has {}", node.children.size());
  ...
  size_t rhs_count = addChildQueryGraph(query_graph, rhs_plan, nodes, rhs_label,
                                        allow_right_subgraph ? static_cast<int>(join_steps_limit - lhs_count) : 0);

  size_t total_inputs = query_graph.inputs.size();
  if (join_kind == JoinKind::Cross || join_kind == JoinKind::Comma) query_graph.join_kinds[0] = std::make_pair(BitSet{}, JoinKind::Cross);
  ...
  for (const auto &[old_node, sources] : expression_actions_sources) {
    ...
    if (lhs_count == 1 && sources.count() == 1 && sources.test(0)) new_sources[new_node] = BitSet().set(0);
    if (rhs_count == 1 && sources.count() == 1 && sources.test(1)) new_sources[new_node] = BitSet().set(total_inputs - 1);
  }
  ...
  if (!left_changes_types.empty()) query_graph.type_changes[0] = std::move(left_changes_types);
  if (!right_changes_types.empty()) query_graph.type_changes[total_inputs - 1] = std::move(right_changes_types);
  ...
  if (isRightOrFull(join_kind)) {
    if (lhs_count != 1)
      throw Exception(ErrorCodes::LOGICAL_ERROR, "JoinStepLogical with RIGHT or FULL join must have exactly one left input, but has {}",
                      lhs_count);
  ...
  if (isLeftOrFull(join_kind)) {
    if (rhs_count != 1)
      throw Exception(ErrorCodes::LOGICAL_ERROR, "JoinStepLogical with LEFT or FULL join must have exactly one right input, but has {}",
                      rhs_count);
  ...
  if (!residual_filter.empty()) throw Exception(ErrorCodes::LOGICAL_ERROR, "Residual filter is not supported in join reorder");
}
...
static std::vector<DPJoinEntry *> getJoinTreePostOrderSequence(DPJoinEntryPtr root) {
  ...
  while (!stack.empty()) {
    ...
    if (!node->isLeaf()) {
      if (!node->left || !node->right) throw Exception(ErrorCodes::LOGICAL_ERROR, "Join node should have both left and right children");
  ...
  while (!node->children.empty()) {
    if (node->type == ActionsDAG::ActionType::FUNCTION) {
      ...
      if (function_name != "toNullable" && function_name != "_CAST" && function_name != "CAST")
        throw Exception(ErrorCodes::LOGICAL_ERROR, "Node {} is a function '{}', expected toNullable or CAST", node->result_name,
                        function_name);
    }

    size_t non_const_children = 0;
    for (const auto *child_node : node->children) {
      if (child_node->type == ActionsDAG::ActionType::COLUMN) continue;
      node = child_node;
      non_const_children++;
    }
...
constexpr bool isSwapOnlyJoinKind(JoinKind kind) { return kind == JoinKind::Full; }
...
QueryPlan::Node chooseJoinOrder(QueryGraphBuilder query_graph_builder, QueryPlan::Nodes &nodes, JoinStrictness join_strictness) {
  ...
  LOG_DEBUG(&Poco::Logger::get("QueryPlanOptimizations"), "Optimizing join order for query graph with {} relations",
  ...
  if (!global_actions_dag) throw Exception(ErrorCodes::LOGICAL_ERROR, "Global expression actions DAG is not set");
  ...
  if (sequence.empty()) throw Exception(ErrorCodes::LOGICAL_ERROR, "Join tree is empty");
  ...
  std::unordered_map<const ActionsDAG::Node *, size_t> usage_level_map;
  {
    ...
    for (size_t i = 0; i < sequence.size(); ++i) {
      ...
      for (const auto &action : entry->join_operator.expression) stack.emplace_back(action.getNode(), i);
      for (const auto &action : entry->join_operator.residual_filter) stack.emplace_back(action.getNode(), i);
    }
    /// Outputs at the end
    for (const auto &out : global_actions_dag->getOutputs()) stack.emplace_back(out, sequence.size());
    ...
    while (!stack.empty()) {
      ...
      for (const auto *child : node->children) stack.push_back(std::make_pair(child, level));
    }
  }
  ...
  if (!query_graph_builder.context) throw Exception(ErrorCodes::LOGICAL_ERROR, "QueryGraphBuilder context is not set");
  ...
  for (size_t input_idx = 0; input_idx < global_inputs.size(); ++input_idx) {
    ...
    if (!rel_idx.has_value())
      throw Exception(ErrorCodes::LOGICAL_ERROR, "Input node {} has {} source relations, expected 1", input->result_name,
                      toString(src_rels));
  ...
  for (size_t entry_idx = 0; entry_idx < sequence.size(); ++entry_idx) {
    ...
    if (entry->isLeaf()) {
      ...
      /// fixme: USING clause handled specially in join algorithm, so swap breaks it
      /// At the time of writing, we're not able to swap inputs for ANY or SEMI partial merge join, because it only supports inner or left
      /// joins, but not right ANTI partial merge join is not supported for any join kind
      const bool partial_merge_join_can_be_selected = std::ranges::any_of(join_settings.join_algorithms, [](JoinAlgorithm alg) {
        return alg == JoinAlgorithm::PARTIAL_MERGE || alg == JoinAlgorithm::PREFER_PARTIAL_MERGE || alg == JoinAlgorithm::AUTO;
      });
      const bool should_worry_about_partial_merge_join =
          partial_merge_join_can_be_selected && (!MergeJoin::isSupported(join_operator.kind, join_operator.strictness) ||
      ...
      if (join_operator.strictness != JoinStrictness::All && !suitable_swap_only_join) flip_join = false;
      ...
      for (auto rel_id : {left_rels.getSingleBit(), right_rels.getSingleBit()}) {
        ...
        for (const auto *new_input : new_inputs) {
          ...
          if (it == input_node_map.end())
            throw Exception(ErrorCodes::LOGICAL_ERROR, "Column {} not found in inputs of dag {}", input_node->result_name,
                            global_actions_dag->dumpDAG());
      ...
      for (size_t input_pos = 0; input_pos < current_input_nodes.size(); ++input_pos) {
        ...
        if (!joined_mask.test(rel_idx) || !input_node) continue;

        current_inputs[input_node] = input_node;

        const auto *out_node = input_node;
        if (auto it2 = current_step_type_changes.find(input_pos); it2 != current_step_type_changes.end()) out_node = it2->second;
        /// add input (possibly which changed type) to required output
        required_output_nodes.push_back(out_node);
      }

      for (const auto &action : join_operator.expression) required_output_nodes.push_back(action.getNode());
      for (const auto &action : join_operator.residual_filter) required_output_nodes.push_back(action.getNode());

      if (entry_idx == sequence.size() - 1) {
        for (const auto *output_node : global_actions_dag->getOutputs()) required_output_nodes.push_back(output_node);
      }
      ...
      if (required_output_nodes.size() != dag_outputs.size())
        throw Exception(ErrorCodes::LOGICAL_ERROR, "Required output nodes size {} does not match current output nodes size in dag {}",
                        required_output_nodes.size(), current_dag->dumpDAG());
      ...
      for (auto &action : join_operator.expression) {
        ...
        if (!mapped_node)
          throw Exception(ErrorCodes::LOGICAL_ERROR, "Node {} not found in current dag {}", action.getNode()->result_name,
                          current_dag->dumpDAG());
      ...
      if (dag_outputs.empty()) {
        if (!first_dropped_node) throw Exception(ErrorCodes::LOGICAL_ERROR, "No columns returned from join: {}", current_dag->dumpDAG());
      ...
      auto join_step =
          std::make_unique<JoinStepLogical>(left_header_ptr, right_header_ptr, std::move(join_operator),
                                            std::move(current_expression_actions), dag_outputs, join_settings, sorting_settings);

      auto left_label = relation_names[left_rels];
      auto right_label = relation_names[right_rels];

      if (!right_label.empty() && right_rels.count() > 1) right_label = fmt::format("({})", right_label);
      ...
      if (right_table_key) join_step->setRightHashTableCacheKey(right_table_key);
...
void optimizeJoinLogical(QueryPlan::Node &node, QueryPlan::Nodes &nodes, const QueryPlanOptimizationSettings &optimization_settings) {
  auto *join_step = typeid_cast<JoinStepLogical *>(node.step.get());
  if (!join_step || join_step->isOptimized()) return;
...
void optimizeJoinLogicalImpl(JoinStepLogical *join_step, QueryPlan::Node &node, QueryPlan::Nodes &nodes,
  ...
  for (auto *child : node.children) {
    if (auto *lookup_step = typeid_cast<JoinStepLogicalLookup *>(child->step.get())) lookup_step->optimize(optimization_settings);
  }
  ...
  if (!optimization_settings.query_plan_optimize_join_order_limit ||
      (strictness != JoinStrictness::All && !isSwapOnlyJoinStrictness(strictness)) || locality != JoinLocality::Unspecified ||
      kind == JoinKind::Paste || !join_operator.residual_filter.empty()) {
