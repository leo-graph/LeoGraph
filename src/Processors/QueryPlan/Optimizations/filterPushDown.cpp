...
static void checkChildrenSize(QueryPlan::Node *node, size_t child_num) {
  ...
  if (child_num > child->getInputHeaders().size() || child_num > node->children.size())
    throw Exception(ErrorCodes::LOGICAL_ERROR, "Wrong number of children: expected at least {}, got {} children and {} streams", child_num,
                    child->getInputHeaders().size(), node->children.size());
}
...
static bool identifiersIsAmongAllGroupingSets(const GroupingSetsParamsList &grouping_sets_params, const NameSet &identifiers_in_predicate) {
  for (const auto &grouping_set : grouping_sets_params) {
    for (const auto &identifier : identifiers_in_predicate) {
      if (std::find(grouping_set.used_keys.begin(), grouping_set.used_keys.end(), identifier) == grouping_set.used_keys.end()) return false;
    }
  }
  return true;
}
...
bool isFilterColumnConst(const FilterStep &filter) {
  if (filter.removesFilterColumn()) return false;

  return filter.getOutputHeader()->getByName(filter.getFilterColumnName()).column->isConst();
}

void materializeFilterColumnIfNeededAfterPushDown(FilterStep &filter, const bool was_filter_const_before,
                                                  const bool is_filter_const_after) {
  if (was_filter_const_before || !is_filter_const_after) return;
...
static std::optional<ActionsDAG::ActionsForFilterPushDown> splitFilter(QueryPlan::Node *parent_node, bool step_changes_the_number_of_rows,
                                                                       const Names &available_inputs, size_t child_idx = 0) {
  ...
  auto result = expression.splitActionsForFilterPushDown(filter_column_name, removes_filter, available_inputs, all_inputs,
                                                         allow_deterministic_functions);
  if (result) materializeFilterColumnIfNeededAfterPushDown(*filter, is_filter_column_const_before, result->is_filter_const_after_push_down);
  return result;
}

static size_t addNewFilterStepOrThrow(QueryPlan::Node *parent_node, QueryPlan::Nodes &nodes,
                                      ActionsDAG::ActionsForFilterPushDown split_filter, size_t child_idx = 0,
                                      bool update_parent_filter = true) {
  QueryPlan::Node *child_node = parent_node->children.front();
  checkChildrenSize(child_node, child_idx + 1);
  ...
  if (update_parent_filter && !filter_node && !filter->removesFilterColumn())
    throw Exception(ErrorCodes::LOGICAL_ERROR, "Filter column {} was removed from ActionsDAG but it is needed in result. DAG:\n{}",
  ...
  node.step = std::make_unique<FilterStep>(node.children.at(0)->step->getOutputHeader(), std::move(split_filter.dag),
                                           std::move(split_filter_column_name), split_filter.remove_filter);
...
static size_t tryAddNewFilterStep(QueryPlan::Node *parent_node, bool step_changes_the_number_of_rows, QueryPlan::Nodes &nodes,
                                  const Names &allowed_inputs, size_t child_idx = 0) {
...
static size_t simplePushDownOverStep(QueryPlan::Node *parent_node, bool step_changes_the_number_of_rows, QueryPlan::Nodes &nodes,
                                     QueryPlanStepPtr &child) {
  if (typeid_cast<Step *>(child.get())) {
    Names allowed_inputs = child->getOutputHeader()->getNames();
    if (auto updated_steps = tryAddNewFilterStep(parent_node, step_changes_the_number_of_rows, nodes, allowed_inputs)) return updated_steps;
  }
  return 0;
}
...
class EquivalentJoinKeySet {
 public:
  ...
  JoinActionRef unite(JoinActionRef a, JoinActionRef b) {
    ...
    if (rank_a < rank_b) std::swap(root_a, root_b);

    parent.insert_or_assign(root_b, root_a);

    if (rank_a == rank_b) ++rank[root_a];

    return root_a;
  }

  bool connected(JoinActionRef a, JoinActionRef b) { return findOrAdd(a) == findOrAdd(b); }
  ...
  std::unordered_map<JoinActionRef, std::vector<JoinActionRef>> getClasses() {
    ...
    for (auto &[ref, _] : parent) classes[findOrAdd(ref)].push_back(ref);
    return classes;
  }
  ...
  std::vector<JoinActionRef> getClass(JoinActionRef ref) {
    ...
    for (auto &[other_ref, _] : parent) {
      if (findOrAdd(other_ref) == root) res.push_back(other_ref);
    }
    return res;
  }
...
std::vector<JoinActionRefPair> getJoiningKeysForJoinStep(const JoinOperator &join_operator) {
  ...
  for (const auto &predicate : join_operator.expression) {
    ...
    if (!left_column.type->equals(*right_column.type)) continue;
    joining_keys.emplace_back(lhs, rhs);
  }
  return joining_keys;
}

std::vector<JoinActionRefPair> buildEquialentSetsForJoinStepLogical(EquivalentJoinKeySet &equivalent_sets, const JoinStepLogical *join_step,
  ...
  for (const auto *child_node : child_nodes) {
    ...
    if (lookup_depth >= 32) break;

    const auto *child_join = typeid_cast<const JoinStepLogical *>(child_node->step.get());
    if (!child_join) continue;
    auto join_strictness = child_join->getJoinOperator().strictness;
    auto join_kind = child_join->getJoinOperator().kind;
    if (join_kind != JoinKind::Inner || (join_strictness != JoinStrictness::All && join_strictness != JoinStrictness::Any)) continue;

    for (const auto &output : child_join->getOutputActions()) {
      while (join_input_it != join_inputs.end() && output.getColumnName() != join_input_it->getColumnName()) ++join_input_it;
      if (join_input_it == join_inputs.end()) break;
  ...
  for (const auto &[lhs, rhs] : joining_keys) equivalent_sets.unite(lhs, rhs);
  return joining_keys;
}
...
static void projectDagInputs(ActionsDAG &actions_dag) {
  ...
  for (const auto *node : actions_dag.getInputs()) {
    if (existing_outputs.contains(node)) continue;
    outputs.push_back(node);
  }
}

std::optional<ActionsDAG> tryToExtractPartialPredicate(const ActionsDAG &original_dag, const std::string &filter_name,
...
static size_t tryPushDownOverJoinStep(QueryPlan::Node *parent_node, QueryPlan::Nodes &nodes, QueryPlan::Node *child_node) {
  ...
  if (!join && !filled_join && !logical_join) return 0;
   ...
   * 2. For LEFT/RIGHT JOIN we can push conditions that use columns from LEFT/RIGHT stream to LEFT/RIGHT JOIN side. We can also push
   * conditions that use columns from LEFT/RIGHT equivalent sets to RIGHT/LEFT JOIN side.
   *
   ...
   * 3. TODO: It is possible to pull up filter conditions from LEFT/RIGHT stream and push conditions that use columns from LEFT/RIGHT
   * equivalent sets to RIGHT/LEFT JOIN side.
   */
  ...
  if (table_join_ptr && table_join_ptr->kind() == JoinKind::Full) return 0;
  if (logical_join && logical_join->getJoinOperator().kind == JoinKind::Full) return 0;
  ...
  if (has_single_clause && !filled_join) {
    ...
    for (size_t i = 0; i < key_names_size; ++i) {
      ...
      if (!left_table_column.type->equals(*right_table_column.type)) continue;
    ...
    std::unordered_set<JoinActionRefPair, JoinActionRefPairHash> equivalent_expressions_set(equivalent_expressions.begin(),
                                                                                            equivalent_expressions.end());
    ...
    for (const auto &[lhs, rhs] : equivalent_expressions) {
      for (const auto &eq_expr : equivalent_sets.getClass(lhs)) {
        if (!eq_expr.isFromSameActions(lhs) || !eq_expr.fromRight()) continue;
        if (equivalent_expressions_set.contains({lhs, eq_expr})) continue;
      ...
      for (const auto &eq_expr : equivalent_sets.getClass(rhs)) {
        if (!eq_expr.isFromSameActions(rhs) || !eq_expr.fromLeft()) continue;
        if (equivalent_expressions_set.contains({eq_expr, rhs})) continue;
  ...
  auto get_available_columns_for_filter = [&](bool push_to_left_stream, bool filter_push_down_input_columns_available) {
    ...
    if (!filter_push_down_input_columns_available) return available_input_columns_for_filter;
    ...
    for (const auto &name : input_columns_names) {
      if (!join_header->has(name)) continue;
      ...
      if (!logical_join && !input_header->getByName(name).type->equals(*join_header->getByName(name).type)) continue;
  ...
  if (!allow_push_down_to_right) right_stream_filter_push_down_input_columns_available = false;
  ...
  for (auto &[lhs, rhs] : equivalent_expressions) {
    ...
    if (!changes_left_type) equivalent_left_stream_column_to_right_stream_column[lhs_original_name] = rhs_column;
    if (!changes_right_type) equivalent_right_stream_column_to_left_stream_column[rhs_original_name] = lhs_column;
  }

  Names left_stream_available_columns_to_push_down =
      get_available_columns_for_filter(true /*push_to_left_stream*/, left_stream_filter_push_down_input_columns_available);
  Names right_stream_available_columns_to_push_down =
      get_available_columns_for_filter(false /*push_to_left_stream*/, right_stream_filter_push_down_input_columns_available);

  if (left_stream_filter_push_down_input_columns_available) {
    for (const auto &[name, _] : equivalent_left_stream_column_to_right_stream_column) equivalent_columns_to_push_down.push_back(name);
  } else if (logical_join && logical_join->getJoinOperator().kind == JoinKind::Right &&
             logical_join->getJoinOperator().strictness == JoinStrictness::Semi) {
  ...
  if (right_stream_filter_push_down_input_columns_available) {
    for (const auto &[name, _] : equivalent_right_stream_column_to_left_stream_column) equivalent_columns_to_push_down.push_back(name);
  } else if (logical_join && logical_join->getJoinOperator().kind == JoinKind::Left &&
             logical_join->getJoinOperator().strictness == JoinStrictness::Semi) {
  ...
  auto join_filter_push_down_actions = filter->getExpression().splitActionsForJOINFilterPushDown(
      filter->getFilterColumnName(), filter->removesFilterColumn(), left_stream_available_columns_to_push_down, *left_stream_input_header,
      right_stream_available_columns_to_push_down, *right_stream_input_header, equivalent_columns_to_push_down,
      equivalent_left_stream_column_to_right_stream_column, equivalent_right_stream_column_to_left_stream_column);

  materializeFilterColumnIfNeededAfterPushDown(*filter, is_filter_column_const_before,
  ...
  /// Here we extract required expressions from JoinStepLogical actions DAG and apply them to the filter DAG that expects them to be
  /// calculated. In general, it could be not only toNullable, but arbitrary expressions, which may be the case once expression pushdown to
  /// JOIN is implemented.
  auto get_required_pre_actions = [&](const auto &join_actions, const auto &filter_dag_inputs) {
    ...
    for (const auto *node : filter_dag_inputs) filter_dag_inputs_map[node->result_name]++;
  ...
  if (join_filter_push_down_actions.left_stream_filter_to_push_down) {
    ...
    updated_steps += addNewFilterStepOrThrow(parent_node, nodes,
                                             {std::move(*join_filter_push_down_actions.left_stream_filter_to_push_down), 0,
                                              join_filter_push_down_actions.left_stream_filter_removes_filter, false},
                                             0 /*child_idx*/, false /*update_parent_filter*/);
    LOG_DEBUG(&Poco::Logger::get("QueryPlanOptimizations"), "Pushed down filter {} to the {} side of join", result_name, JoinKind::Left);
  }
  ...
  if (join_filter_push_down_actions.right_stream_filter_to_push_down && allow_push_down_to_right) {
    ...
    updated_steps += addNewFilterStepOrThrow(parent_node, nodes,
                                             {std::move(*join_filter_push_down_actions.right_stream_filter_to_push_down), 0,
                                              join_filter_push_down_actions.right_stream_filter_removes_filter, false},
                                             1 /*child_idx*/, false /*update_parent_filter*/);
    LOG_DEBUG(&Poco::Logger::get("QueryPlanOptimizations"), "Pushed down filter {} to the {} side of join", result_name, JoinKind::Right);
  }
  ...
  const bool disj_pushdown_enabled =
      (join && join->useJoinDisjunctionsPushDown()) || (logical_join && logical_join->getSettings().use_join_disjunctions_push_down);
  if (filter && disj_pushdown_enabled) {
    if ((join && join->isDisjunctionsOptimizationApplied()) || (logical_join && logical_join->isDisjunctionsOptimizationApplied()) ||
        (filled_join && filled_join->isDisjunctionsOptimizationApplied())) {
      return updated_steps;
    }

    {
      auto left_partial_filter_dag =
          tryToExtractPartialPredicate(filter->getExpression(), filter->getFilterColumnName(), left_stream_available_columns_to_push_down);
      if (left_partial_filter_dag.has_value()) {
        ...
        ++updated_steps;
        LOG_DEBUG(&Poco::Logger::get("QueryPlanOptimizations"), "Pushed down partial filter {} to the {} side of join",
                  partial_predicate_column_name, JoinKind::Left);
      }
    }

    {
      auto right_partial_filter_dag =
          tryToExtractPartialPredicate(filter->getExpression(), filter->getFilterColumnName(), right_stream_available_columns_to_push_down);
      if (right_partial_filter_dag.has_value()) {
        ...
        ++updated_steps;
        LOG_DEBUG(&Poco::Logger::get("QueryPlanOptimizations"), "Pushed down partial filter {} to the {} side of join",
                  partial_predicate_column_name, JoinKind::Right);
      }
    }

    if (join) join->setDisjunctionsOptimizationApplied(true);
    if (logical_join) logical_join->setDisjunctionsOptimizationApplied(true);
    if (filled_join) filled_join->setDisjunctionsOptimizationApplied(true);
  }
...
size_t tryPushDownFilter(QueryPlan::Node *parent_node, QueryPlan::Nodes &nodes, const Optimization::ExtraSettings &settings) {
  if (parent_node->children.size() != 1) return 0;
  ...
  if (aggregating || merging_aggregated) {
    ...
    if (is_grouping_sets) {
      ...
      if (aggregating && aggregating->isGroupByUseNulls()) return 0;
      ...
      if (!identifiersIsAmongAllGroupingSets(grouping_sets, identifiers_in_predicate)) return 0;
    }
    ...
    if (auto updated_steps = tryAddNewFilterStep(parent_node, true, nodes, keys)) return updated_steps;
  }
  ...
  if (auto *delayed = typeid_cast<DelayedCreatingSetsStep *>(child.get())) {
    ...
    child = std::make_unique<DelayedCreatingSetsStep>(filter->getOutputHeader(), delayed->detachSets(), delayed->getNetworkTransferLimits(),
  ...
  if (auto *totals_having = typeid_cast<TotalsHavingStep *>(child.get())) {
    ...
    if (totals_having->getActions()) return 0;

    Names keys;
    const auto &header = totals_having->getInputHeaders().front();
    for (const auto &column : *header)
      if (typeid_cast<const DataTypeAggregateFunction *>(column.type.get()) == nullptr) keys.push_back(column.name);
    ...
    if (auto updated_steps = tryAddNewFilterStep(parent_node, false, nodes, keys)) return updated_steps;
  }
  ...
  if (auto *array_join = typeid_cast<ArrayJoinStep *>(child.get())) {
    ...
    for (const auto &column : *array_join_header)
      if (!keys_set.contains(column.name)) allowed_inputs.push_back(column.name);

    if (auto updated_steps = tryAddNewFilterStep(parent_node, true, nodes, allowed_inputs)) return updated_steps;
  }

  if (auto updated_steps = simplePushDownOverStep<DistinctStep>(parent_node, true, nodes, child)) return updated_steps;

  if (auto updated_steps = simplePushDownOverStep<BuildRuntimeFilterStep>(parent_node, true, nodes, child)) return updated_steps;

  if (auto updated_steps = tryPushDownOverJoinStep(parent_node, nodes, child_node)) return updated_steps;
  ...
  if (typeid_cast<SortingStep *>(child.get())) {
    Names allowed_inputs = child->getOutputHeader()->getNames();
    if (auto updated_steps = tryAddNewFilterStep(parent_node, false, nodes, allowed_inputs)) return updated_steps;
  }
  ...
  if (auto *union_step = typeid_cast<UnionStep *>(child.get())) {
    ...
    for (auto &input_header : union_input_headers) input_header = expected_output;
    ...
    for (size_t i = 1; i < parent_node->children.size(); ++i) {
      ...
      filter_node.step =
          std::make_unique<FilterStep>(filter_node.children.front()->step->getOutputHeader(), filter->getExpression().clone(),
  ...
  if (auto *parallel_replicas_local_plan = typeid_cast<ReadFromLocalParallelReplicaStep *>(child.get())) {
    if (!settings.parallel_replicas_filter_pushdown) return 0;
  ...
  if (auto updated_steps = simplePushDownOverStep<CommonSubplanStep>(parent_node, false, nodes, child)) return updated_steps;
