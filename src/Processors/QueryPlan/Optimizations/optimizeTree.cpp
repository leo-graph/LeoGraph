...
void optimizeTreeFirstPass(const QueryPlanOptimizationSettings &optimization_settings, QueryPlan::Node &root, QueryPlan::Nodes &nodes) {
  ...
  while (!stack.empty()) {
    ...
    for (const auto &optimization : getOptimizations()) {
      if (!(optimization_settings.*(optimization.is_enabled))) continue;

      /// Just in case, skip optimization if it is not initialized.
      if (!optimization.apply) continue;

      if (max_optimizations_to_apply && max_optimizations_to_apply < total_applied_optimizations) {
        if (optimization_settings.is_explain) return;

        throw Exception(ErrorCodes::TOO_MANY_QUERY_PLAN_OPTIMIZATIONS, "Too many optimizations applied to query plan. Current limit {}",
...
void tryMakeDistributedAggregation(QueryPlan::Node &node, QueryPlan::Nodes &nodes,
...
void optimizeTreeSecondPass(const QueryPlanOptimizationSettings &optimization_settings, QueryPlan::Node &root, QueryPlan::Nodes &nodes,
  ...
  while (!stack.empty()) {
    if (optimization_settings.query_plan_optimize_primary_key) optimizePrimaryKeyConditionAndLimit(stack);
  ...
  if (!optimization_settings.correlated_subqueries_use_in_memory_buffer) {
    ...
    traverseQueryPlan(stack, root, [&](auto &frame_node) { materializeQueryPlanReferences(frame_node, nodes); });

    /// Remove CommonSubplanSteps (they must be not used at that point).
    traverseQueryPlan(stack, root, [&](auto &frame_node) { optimizeUnusedCommonSubplans(frame_node); });
  }
  ...
  if (optimization_settings.optimize_prewhere) {
    traverseQueryPlan(stack, root, [&](auto &frame_node) { optimizePrewhere(frame_node, optimization_settings.remove_unused_columns); });
  }

  traverseQueryPlan(
      stack, root,
      [&](auto &frame_node) {
        if (optimization_settings.read_in_order) optimizeReadInOrder(frame_node, nodes, optimization_settings);

        if (optimization_settings.distinct_in_order) optimizeDistinctInOrder(frame_node, nodes, optimization_settings);
      },
      [&](auto &frame_node) {
  ...
  while (!stack.empty()) {
        ...
        if (optimization_settings.optimize_projection) {
          auto applied_projection =
              optimizeUseAggregateProjections(*frame.node, nodes, optimization_settings.optimize_use_implicit_projections,
                                              ...
                                              optimization_settings.max_step_description_length);
          if (applied_projection) applied_projection_names.insert(*applied_projection);
        }

        if (optimization_settings.aggregation_in_order) optimizeAggregationInOrder(*frame.node, nodes, optimization_settings);
      }
    ...
    if (optimization_settings.optimize_projection) {
      /// Projection optimization relies on PK optimization
      if (auto applied_projection =
              optimizeUseNormalProjections(stack, nodes, optimization_settings.is_parallel_replicas_initiator_with_projection_support,
                                           optimization_settings.max_step_description_length)) {
        ...
        if (max_optimizations_to_apply && max_optimizations_to_apply < applied_projection_names.size()) {
          ...
          if (!optimization_settings.is_explain)
            throw Exception(ErrorCodes::TOO_MANY_QUERY_PLAN_OPTIMIZATIONS,
                            "Too many projection optimizations applied to query plan. Current limit {}", max_optimizations_to_apply);
        }
  ...
  while (!stack.empty()) {
    ...
    if (auto *read_from_local = typeid_cast<ReadFromLocalParallelReplicaStep *>(frame.node->step.get())) {
      ...
      if (optimization_settings.merge_expressions) tryMergeExpressions(local_plan_node, nodes, {});
    }

    stack.pop_back();
  }
  // local plan can contain redundant sorting
  if (read_from_local_parallel_replica_plan && optimization_settings.remove_redundant_sorting) tryRemoveRedundantSorting(&root);
  /// Optimize exchanges
  if (optimization_settings.make_distributed_plan && optimization_settings.distributed_plan_optimize_exchanges) optimizeExchanges(root);
  ...
  if (optimization_settings.try_use_vector_search && !extra_settings.vector_search_with_rescoring) {
    ...
    while (!stack.empty()) {
      ...
      if (frame.next_child == 0) {
        if (optimizeVectorSearchSecondPass(root, stack, nodes, extra_settings)) break;
      }
  ...
  if (optimization_settings.optimize_lazy_materialization) {
    ...
    while (!stack.empty()) {
      ...
      if (frame.next_child == 0) {
        if (optimizeLazyMaterialization2(*frame.node, query_plan, nodes, optimization_settings,
                                         optimization_settings.max_limit_for_lazy_materialization)) {
          stack.pop_back();
          continue;
        }
      }
  ...
  if (optimization_settings.query_plan_join_shard_by_pk_ranges) optimizeJoinByShards(root);

  considerEnablingParallelReplicas(optimization_settings, root, query_plan);
}

void addStepsToBuildSets(const QueryPlanOptimizationSettings &optimization_settings, QueryPlan &plan, QueryPlan::Node &root,
