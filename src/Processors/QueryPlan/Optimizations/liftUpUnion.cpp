...
size_t tryLiftUpUnion(QueryPlan::Node *parent_node, QueryPlan::Nodes &nodes, const Optimization::ExtraSettings & /*settings*/) {
  if (parent_node->children.empty()) return 0;
  ...
  if (auto *expression = typeid_cast<ExpressionStep *>(parent.get())) {
    ...
    for (size_t i = 1; i < parent_node->children.size(); ++i) {
      ...
      expr_node.step =
          std::make_unique<ExpressionStep>(expr_node.children.front()->step->getOutputHeader(), expression->getExpression().clone());
  ...
  if (auto *distinct = typeid_cast<DistinctStep *>(parent.get()); distinct && distinct->isPreliminary()) {
    ...
    for (size_t i = 1; i < parent_node->children.size(); ++i) {
      ...
      distinct_node.step =
          std::make_unique<DistinctStep>(distinct_node.children.front()->step->getOutputHeader(), distinct->getSetSizeLimits(),
                                         distinct->getLimitHint(), distinct->getColumnNames(), distinct->isPreliminary());
