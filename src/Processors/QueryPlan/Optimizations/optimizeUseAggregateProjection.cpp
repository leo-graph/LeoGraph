...
static DAGIndex buildDAGIndex(const ActionsDAG &dag) {
  DAGIndex index;
  for (const auto *output : dag.getOutputs()) index.emplace(output->result_name, output);
...
static AggregateProjectionInfo getAggregatingProjectionInfo(const ProjectionDescription &projection, const ContextPtr &context,
                                                            const StorageMetadataPtr &metadata_snapshot, const Block &key_virtual_columns) {
...
std::optional<AggregateFunctionMatches> matchAggregateFunctions(const AggregateProjectionInfo &info,
                                                                const AggregateDescriptions &aggregates,
                                                                const MatchedTrees::Matches &matches, const DAGIndex &query_index,
  ...
  for (size_t i = 0; i < info.aggregates.size(); ++i) projection_aggregate_functions[info.aggregates[i].function->getName()].push_back(i);

  for (const auto &aggregate : aggregates) {
    /// Get a list of candidates by name first.
    auto it = projection_aggregate_functions.find(aggregate.function->getName());
    if (it == projection_aggregate_functions.end()) return {};
    ...
    for (size_t idx : candidates) {
      ...
      /// not match.
      if (!candidate.function->getStateType()->equals(*aggregate.function->getStateType())) continue;
      ...
      if (num_args != candidate.argument_names.size()) continue;
      ...
      while (next_arg < num_args) {
        ...
        if (jt == query_index.end() || kt == proj_index.end()) break;

        const auto *query_node = jt->second;
        const auto *proj_node = kt->second;

        auto mt = matches.find(query_node);
        if (mt == matches.end()) break;

        const auto &node_match = mt->second;
        if (node_match.node != proj_node || node_match.monotonicity) break;

        argument_types.push_back(query_node->result_type);
        ++next_arg;
      }

      if (next_arg < aggregate.argument_names.size()) continue;
...
static void appendAggregateFunctions(ActionsDAG &proj_dag, const AggregateDescriptions &aggregates,
  ...
  for (size_t i = 0; i < num_aggregates; ++i) {
    ...
    if (!input) input = &proj_dag.addInput(match.description->column_name, type);
...
std::optional<ActionsDAG> analyzeAggregateProjection(const AggregateProjectionInfo &info, const QueryDAG &query,
                                                     const DAGIndex &query_index, const Names &keys,
                                                     const AggregateDescriptions &aggregates) {
...

  {
    ...
    if (query.filter_node) query_key_nodes.push_back(query.filter_node);

    for (const auto &key : keys) {
      auto it = query_index.find(key);
      /// This should not happen ideally.
      if (it == query_index.end()) return {};
...
AggregateProjectionCandidates getAggregateProjectionCandidates(QueryPlan::Node &node, AggregatingStep &aggregating,
                                                               ReadFromMergeTree &reading, const PartitionIdToMaxBlockPtr &max_added_blocks,
  ...
  for (const auto &projection : projections)
    if (projection.type == ProjectionDescription::Type::Aggregate) agg_projections.push_back(&projection);

  bool can_use_minmax_projection =
      allow_implicit_projections && metadata->minmax_count_projection && !reading.getMergeTreeData().has_lightweight_delete_parts.load();

  if (!can_use_minmax_projection && agg_projections.empty()) return candidates;

  QueryDAG dag;
  if (!dag.build(*node.children.front())) return candidates;
  ...
  if (dag.filter_node && !VirtualColumnUtils::isDeterministicInScopeOfQuery(dag.filter_node)) can_use_minmax_projection = false;
  ...
  if (can_use_minmax_projection) {
    ...
    if (auto proj_dag = analyzeAggregateProjection(info, dag, query_index, keys, aggregates)) {
      ...
      auto block = reading.getMergeTreeData().getMinMaxCountProjectionBlock(metadata, candidate.dag.getRequiredColumnsNames(),
                                                                            (dag.filter_node ? &*dag.dag : nullptr), reading.getParts(),
                                                                            max_added_blocks.get(), context);
...
AggregateProjectionCandidates getAggregateProjectionCandidates(QueryPlan::Node &node, DistinctStep &distinct, ReadFromMergeTree &reading) {
  ...
  if (agg_projections.empty()) return candidates;
...
static QueryPlan::Node *findReadingStep(QueryPlan::Node &node) {
  ...
  if (auto * /*reading*/ _ = typeid_cast<ReadFromMergeTree *>(step)) return &node;

  if (node.children.size() != 1) return nullptr;

  if (typeid_cast<ExpressionStep *>(step) || typeid_cast<FilterStep *>(step)) return findReadingStep(*node.children.front());
...
std::optional<String> optimizeUseAggregateProjections(QueryPlan::Node &node, QueryPlan::Nodes &nodes, bool allow_implicit_projections,
  ...
  if (!aggregating && !distinct) return {};

  if (aggregating && !aggregating->canUseProjection()) return {};
  ...
  if (!canUseProjectionForReadingStep(reading)) return {};
  ...
  if (candidates.minmax_projection) {
    ...
    if (!force_optimize_projection) {
      ...
      if (parent_reading_select_result->parts_with_ranges.empty()) return {};
    }
    ...
    if (!candidates.only_count_column.empty()) {
      ...
      for (auto &part_with_ranges : parent_parts_with_ranges) {
        ...
        if (exact_ranges.empty()) continue;
        ...
        for (auto &range : ranges) {
          while (i < len && exact_ranges[i].begin < range.end) {
            ...
            if (range.begin < exact_ranges[i].begin) new_ranges.emplace_back(range.begin, exact_ranges[i].begin);
          ...
          if (range.begin < range.end) new_ranges.emplace_back(range);
        }
        chassert(i == len);
      ...
      stat.description = fmt::format("Exact count optimization is applied: {} ranges with exact row count {}. Filtered parts: {}",
                                     stat.selected_ranges, stat.selected_rows, stat.filtered_parts);
    ...
    if (!parent_reading_select_result->parts_with_ranges.empty() || force_optimize_projection) {
      for (auto &candidate : candidates.real) {
        ...
        bool analyzed = analyzeProjectionCandidate(candidate, reader, empty_mutations_snapshot, required_column_names,
                                                   *parent_reading_select_result, projection_query_info, context);
        ...
        if (candidate.sum_marks > parent_reading_marks) {
          stat.description = fmt::format(
              ...
              candidate.projection->name, candidate.sum_marks, parent_reading_marks);

          LOG_DEBUG(logger, "{}", stat.description);
          continue;
        }

        if (best_candidate == nullptr || best_candidate->sum_marks > candidate.sum_marks) best_candidate = &candidate;
      }
    }

    /// No suitable projection found, and exact count optimization is not used.
    if (!best_candidate && exact_count == 0) return {};
  } else {
    return {};
  }
  ...
  if (best_candidate) {
    for (const auto &candidate : candidates.real) {
      ...
      if (&candidate == best_candidate) {
        ...
        candidate.stat->description =
            fmt::format("Projection {} is selected as the best with {} marks to read, while the original table requires scanning {} marks",
                        candidate.projection->name, candidate.sum_marks, parent_reading_select_result->selected_marks);
        LOG_DEBUG(logger, "{}", candidate.stat->description);
      } else if (candidate.stat && candidate.stat->description.empty()) {
        candidate.stat->description =
            fmt::format("Projection {} is usable but requires reading {} marks, which is less efficient than projection {} with {} marks",
                        candidate.projection->name, candidate.sum_marks, best_candidate->projection->name, best_candidate->sum_marks);
        LOG_DEBUG(logger, "{}", candidate.stat->description);
      }
    }
  }
  ...
  if (best_candidate) selected_projection_name = best_candidate->projection->name;

  bool is_parallel_reading_on_remote_replicas =
      reading->isParallelReadingEnabled() && !is_parallel_replicas_initiator_with_projection_support;
  ...
  if (candidates.minmax_projection) {
    ...
    Block block_with_count{
        {std::move(column), std::make_shared<DataTypeAggregateFunction>(agg_count, DataTypes{}, Array{}), candidates.only_count_column}};
    ...
    if (has_parent_parts) reading->setAnalyzedResult(std::move(inexact_ranges_select_result));
  } else {
    chassert(parent_reading_select_result);
    ...
    projection_reading = reader.readFromParts(
        /* parts = */ {}, reading->getMutationsSnapshot()->cloneEmpty(), best_candidate->dag.getRequiredColumnsNames(), proj_snapshot,
        projection_query_info, context, reading->getMaxBlockSize(), reading->getNumStreams(), max_added_blocks,
        best_candidate->merge_tree_projection_select_result_ptr, reading->isParallelReadingEnabled(),
    ...
    if (!projection_reading || should_skip_projection_reading_on_remote_replicas) {
      Pipe pipe(std::make_shared<NullSource>(
          std::make_shared<const Block>(proj_snapshot->getSampleBlockForColumns(best_candidate->dag.getRequiredColumnsNames()))));
      projection_reading = std::make_unique<ReadFromPreparedSource>(std::move(pipe));
    }

    if (has_parent_parts && is_parallel_replicas_initiator_with_projection_support) fallbackToLocalProjectionReading(projection_reading);
  }
  ...
  if (best_candidate) {
    ...
    if (candidates.has_filter) {
      ...
      aggregate_projection_node->step =
          std::make_unique<FilterStep>(projection_reading_node.step->getOutputHeader(), std::move(best_candidate->dag), result_name, true);
    } else
      aggregate_projection_node->step =
  ...
  if (aggregating) {
    ...
    if (blocksHaveEqualStructure(*projection_header, *expected_header)) {
      ...
      auto converting =
          ActionsDAG::makeConvertingActions(projection_header->getColumnsWithTypeAndName(), expected_header->getColumnsWithTypeAndName(),
                                            ActionsDAG::MatchColumnsMode::Name, context);
      auto &converting_node = nodes.emplace_back();
      converting_node.step = std::make_unique<ExpressionStep>(projection_header, std::move(converting));
