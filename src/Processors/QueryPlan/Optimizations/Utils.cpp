...
bool makeExpressionNodeOnTopOfImpl(QueryPlan::Node &node, ActionsDAG actions_dag, QueryPlan::Nodes &nodes,
  ...
  if (step_description) step_description->setStepDescription(*step);
...
bool makeExpressionNodeOnTopOf(QueryPlan::Node &node, ActionsDAG actions_dag, QueryPlan::Nodes &nodes,
...
bool makeFilterNodeOnTopOf(QueryPlan::Node &node, ActionsDAG actions_dag, const String &filter_column_name, bool remove_filer,
                           QueryPlan::Nodes &nodes, DescriptionHolderPtr step_description) {
  if (filter_column_name.empty())
    return makeExpressionNodeOnTopOfImpl<ExpressionStep>(node, std::move(actions_dag), nodes, std::move(step_description));
  return makeExpressionNodeOnTopOfImpl<FilterStep>(node, std::move(actions_dag), nodes, std::move(step_description), filter_column_name,
                                                   remove_filer);
}
...
FilterResult getFilterResult(const ColumnWithTypeAndName &column) {
  if (!column.column) return FilterResult::UNKNOWN;

  if (!column.type->canBeUsedInBooleanContext()) return FilterResult::UNKNOWN;
...
FilterResult filterResultForNotMatchedRows(const ActionsDAG &filter_dag, const String &filter_column_name, const Block &input_stream_header,
  ...
  try {
    filter_output = ActionsDAG::evaluatePartialResult(
        filter_input, {filter_dag.tryFindInOutputs(filter_column_name)},
        /*input_rows_count=*/1, {.skip_materialize = true, .allow_unknown_function_arguments = allow_unknown_function_arguments});
