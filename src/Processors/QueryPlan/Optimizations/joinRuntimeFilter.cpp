...
const ActionsDAG::Node &createRuntimeFilterCondition(ActionsDAG &actions_dag, const String &filter_name,
                                                     const ColumnWithTypeAndName &key_column, const DataTypePtr &filter_element_type) {
  const auto &filter_name_node = actions_dag.addColumn(
      ColumnWithTypeAndName(DataTypeString().createColumnConst(0, filter_name), std::make_shared<DataTypeString>(), filter_name));
...
bool tryAddJoinRuntimeFilter(QueryPlan::Node &node, QueryPlan::Nodes &nodes, const QueryPlanOptimizationSettings &optimization_settings) {
  ...
  if (typeid_cast<JoinStepLogicalLookup *>(node.children[1]->step.get())) return false;

  /// There are cases when either or both joined tables are replaced with const data at optimization time, e.g. when they are (SELECT 1 AS
  /// col). In such cases a header can be empty and all the const data is in the ActionsDAG in the Join step. There is no need (and no way)
  /// to build runtime filter in this scenario.
  if (node.children[0]->step->getOutputHeader()->empty() || node.children[1]->step->getOutputHeader()->empty()) return false;
  ...
  if (!can_use_runtime_filter) return false;

  /// Sometimes cross join can be represented by inner join without expressions
  if (join_operator.expression.empty()) return false;
  ...
  for (const auto &condition : join_operator.expression) {
    ...
    if (predicate_op != JoinConditionOperator::Equals) return false;

    /// For the case of ANTI JOIN (more specifically for check_left_does_not_contain) the hash table in JOIN can have extra rows that can be
    /// filtered out by post-condition. In this case we cannot build set of keys for runtime filter from right-side rows because the set
    /// will contain more rows and thus 'NOT IN set' operation will filter out rows that should not be filtered. So in this case we check
    /// that all JOIN predicates are equality between expr from left columns and expr from right columns, but not something like "func(left,
    /// right) = const"
...

  {
    ...
    if (key_dags) {
      ...
      join_keys_build_side =
          std::ranges::to<ColumnsWithTypeAndName>(key_dags->second.keys | std::views::transform(get_node_column_with_type_and_name));
      if (!isPassthroughActions(key_dags->first.actions_dag))
        makeExpressionNodeOnTopOf(*apply_filter_node, std::move(key_dags->first.actions_dag), nodes,
                                  makeDescription("Calculate left join keys"));
      if (!isPassthroughActions(key_dags->second.actions_dag))
        makeExpressionNodeOnTopOf(*build_filter_node, std::move(key_dags->second.actions_dag), nodes,
                                  makeDescription("Calculate right join keys"));
    }
  }
  ...
  /// When negation will be use for the set of rows in filter, double check that all original predicates were transformed into equality
  /// predicates between left and right side
  ...
  const String filter_name_prefix =
      fmt::format("{}_runtime_filter_{}", check_left_does_not_contain ? "_exclusion_" : "", thread_local_rng());

  {
    ...
    for (size_t i = 0; i < join_keys_build_side.size(); ++i) {
      ...
      if (!join_key_build_side.type->equals(*join_key_probe_side.type)) {
        try {
          ...
          ex.addMessage("JOIN cannot infer common type in ON section for keys. Left key '{}' type {}. Right key '{}' type {}",
                        join_key_probe_side.name, join_key_probe_side.type->getName(), join_key_build_side.name,
      ...
      LOG_TRACE(getLogger("joinRuntimeFilter"), "Runtime filter '{}' will be built from `{}` and applied to `{}`", filter_name,
                join_key_build_side.name, join_key_probe_side.name);
      ...
      /// Add building filter to the build subtree of join
      {
        ...
        new_build_filter_node->step = std::make_unique<BuildRuntimeFilterStep>(
            build_filter_node->step->getOutputHeader(), join_key_build_side.name, common_type, filter_name,
            optimization_settings.join_runtime_filter_exact_values_limit, optimization_settings.join_runtime_bloom_filter_bytes,
