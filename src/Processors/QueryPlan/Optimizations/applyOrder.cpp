...
SortingProperty applyOrder(QueryPlan::Node *parent, SortingProperty *properties,
  ...
  if (const auto *aggregating_step = typeid_cast<AggregatingStep *>(parent->step.get())) {
    ...
    if (!sort_description.empty()) return {std::move(sort_description), SortingProperty::SortScope::Global};
  }
  ...
  if (auto *distinct_step = typeid_cast<DistinctStep *>(parent->step.get())) {
    ...
    if (optimization_settings.distinct_in_order && distinct_step->getSortDescription().empty() &&
      ...
      for (auto &sort_column_desc : properties->sort_description) {
        if (!columns.contains(sort_column_desc.column_name)) break;
    ...
    if (properties->sort_scope == SortingProperty::SortScope::Global) return *properties;

    /// Preliminary Distinct also does not break stream order
    if (distinct_step->isPreliminary() && properties->sort_scope == SortingProperty::SortScope::Stream) return *properties;
  }
    ...
    if (filter_step->removesFilterColumn()) {
      ...
      if (!out_to_skip)
        throw Exception(ErrorCodes::LOGICAL_ERROR, "Output nodes for ActionsDAG do not contain filter column name {}. DAG:\n{}",
                        filter_step->getFilterColumnName(), expr.dumpDAG());
    }
  ...
  if (auto *transforming = dynamic_cast<ITransformingStep *>(parent->step.get())) {
    if (transforming->getDataStreamTraits().preserves_sorting) return std::move(*properties);
  }
  ...
  if (auto * /*union_step*/ _ = typeid_cast<UnionStep *>(parent->step.get())) {
    ...
    if (!common_sort_description.empty()) return {std::move(common_sort_description), sort_scope};
  }
