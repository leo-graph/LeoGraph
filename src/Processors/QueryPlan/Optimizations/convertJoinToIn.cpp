...
InConversion buildInConversion(const SharedHeader &lhs_input_header, const NamePairs &name_pairs, std::unique_ptr<QueryPlan> in_source,
                               bool transform_null_in, SizeLimits size_limits, size_t max_size_for_index) {
  ...
  for (const auto &output : lhs_dag.getOutputs()) lhs_outputs.emplace(output->result_name, output);
  ...
  for (const auto &output : rhs_dag.getOutputs()) rhs_outputs.emplace(output->result_name, output);
  ...
  for (const auto &name_pair : name_pairs) {
    ...
    if (it == lhs_outputs.end()) throw Exception(ErrorCodes::LOGICAL_ERROR, "Cannot find left key {} in JOIN step", name_pair.lhs_name);
    left_columns.push_back(it->second);

    auto jt = rhs_outputs.find(name_pair.rhs_name);
    if (jt == rhs_outputs.end()) throw Exception(ErrorCodes::LOGICAL_ERROR, "Cannot find right key {} in JOIN step", name_pair.lhs_name);
  ...
  const ActionsDAG::Node *in_lhs_arg =
      left_columns.size() == 1 ? left_columns.front() : &lhs_dag.addFunction(func_tuple_builder, std::move(left_columns), {});
  ...
  auto future_set = std::make_shared<FutureSetFromSubquery>(get_random_hash(), nullptr, std::move(in_source), nullptr, nullptr,
                                                            transform_null_in, size_limits, max_size_for_index);

  ColumnPtr set_col = ColumnSet::create(1, future_set);
  const ActionsDAG::Node *in_rhs_arg = &lhs_dag.addColumn({set_col, std::make_shared<DataTypeSet>(), "set column"});
...
size_t tryConvertJoinToIn(QueryPlan::Node *parent_node, QueryPlan::Nodes &nodes, const Optimization::ExtraSettings &settings) {
  ...
  if (parent_node->children.size() != 2) return 0;
  ...
  if (join_operator.strictness != JoinStrictness::Any && join_operator.strictness != JoinStrictness::All) return 0;

  /// TODO: support left in the future
  if (!isInner(join_operator.kind) /*&& !isLeft(join_operator.kind) && !isRight(join_operator.kind)*/) return 0;

  /// Do not support many condition for now.
  if (join_operator.expression.empty()) return 0;
  ...
  for (const auto &predicate : join_operator.expression) {
    ...
    if (op != JoinConditionOperator::Equals) return 0;
  ...
  if (!isInnerOrLeft(join_operator.kind) || std::ranges::any_of(join_output_actions, &JoinActionRef::fromRight)) return 0;

  /// Check input and output type match
  if (!join->typeChangingSides().empty()) return 0;
  ...
  if (build_set_from_left_part) {
    for (auto &p : key_pairs) std::swap(p.first, p.second);
  }

  NamePairs name_pairs;
  for (auto &[lhs, rhs] : key_pairs) name_pairs.emplace_back(lhs.getColumnName(), rhs.getColumnName());

  auto left_pre_join_actions =
      JoinExpressionActions::getSubDAG(key_pairs | std::views::transform([](const auto &key_pair) { return key_pair.first; }));
  auto right_pre_join_actions =
      JoinExpressionActions::getSubDAG(key_pairs | std::views::transform([](const auto &key_pair) { return key_pair.second; }));
  ...
  auto in_conversion = buildInConversion(lhs_in_node->step->getOutputHeader(), name_pairs,
                                         std::make_unique<QueryPlan>(QueryPlan::extractSubplan(rhs_in_node, nodes)), transform_null_in,
                                         settings.network_transfer_limits, settings.use_index_for_in_with_subqueries_max_values);
  ...
  auto creating_sets_step = std::make_unique<DelayedCreatingSetsStep>(output_header, PreparedSets::Subqueries{std::move(in_conversion.set)},
                                                                      settings.network_transfer_limits, nullptr);
