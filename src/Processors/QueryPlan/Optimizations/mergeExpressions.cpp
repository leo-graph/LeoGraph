...
size_t tryMergeExpressions(QueryPlan::Node *parent_node, QueryPlan::Nodes &, const Optimization::ExtraSettings &settings) {
  if (parent_node->children.size() != 1) return false;
  ...
  if (parent_expr && child_expr) {
    ...
    if (child_actions.hasArrayJoin() && parent_actions.hasStatefulFunctions()) return 0;
    ...
    expr->setStepDescription(fmt::format("({} + {})", parent_expr->getStepDescription(), child_expr->getStepDescription()),
  ...
  if (parent_filter && child_expr) {
    ...
    auto filter = std::make_unique<FilterStep>(child_expr->getInputHeaders().front(), std::move(merged),
                                               parent_filter->getFilterColumnName(), parent_filter->removesFilterColumn());
    filter->setStepDescription(fmt::format("({} + {})", parent_filter->getStepDescription(), child_expr->getStepDescription()),
                               settings.max_step_description_length);
...
size_t tryMergeFilters(QueryPlan::Node *parent_node, QueryPlan::Nodes &, const Optimization::ExtraSettings &settings) {
  ...
  if (parent_filter && child_filter) {
    ...
    if (child_actions.hasArrayJoin()) return 0;

    const auto &child_filter_node = child_actions.findInOutputs(child_filter->getFilterColumnName());
    if (child_filter->removesFilterColumn()) removeFromOutputs(child_actions, child_filter_node);
    ...
    if (parent_filter->removesFilterColumn()) removeFromOutputs(child_actions, parent_filter_node);
    ...
    auto filter =
        std::make_unique<FilterStep>(child_filter->getInputHeaders().front(), std::move(child_actions), condition.result_name, true);
    filter->setStepDescription(fmt::format("({} + {})", parent_filter->getStepDescription(), child_filter->getStepDescription()),
                               settings.max_step_description_length);
