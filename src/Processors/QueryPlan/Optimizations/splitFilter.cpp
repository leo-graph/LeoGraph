...
size_t trySplitJoin(QueryPlan::Node *node, QueryPlan::Nodes &nodes) {
  ...
  if (!join_step || node->children.size() != 2 || typeid_cast<JoinStepLogicalLookup *>(node->children.back()->step.get())) return 0;
...
size_t trySplitFilter(QueryPlan::Node *node, QueryPlan::Nodes &nodes, const Optimization::ExtraSettings &settings) {
  if (size_t join_split = trySplitJoin(node, nodes)) return join_split;

  auto *filter_step = typeid_cast<FilterStep *>(node->step.get());
  if (!filter_step) return 0;
  ...
  if (expr.hasStatefulFunctions()) return 0;
  ...
  if (filter_step->removesFilterColumn()) remove_filter = split.second.removeUnusedResult(filter_column_name);
  ...
  filter_node.step = std::make_unique<FilterStep>(filter_node.children.at(0)->step->getOutputHeader(), std::move(split.first),
                                                  std::move(split_filter_name), remove_filter);
