...
static std::optional<ActionsDAG> makeMaterializingDAG(const Block &proj_header, const Block &main_header) {
  ...
  if (proj_header.columns() != num_columns) return {};
  ...
  for (size_t i = 0; i < num_columns; ++i) {
    ...
    if (is_proj_const && !is_main_proj) const_positions.push_back(i);
  }

  if (const_positions.empty()) return {};

  ActionsDAG dag;
  auto &outputs = dag.getOutputs();
  for (const auto &col : proj_header.getColumnsWithTypeAndName()) outputs.push_back(&dag.addInput(col));
...
std::optional<String> optimizeUseNormalProjections(Stack &stack, QueryPlan::Nodes &nodes,
  ...
  while (std::next(iter) != stack.rend()) {
    ...
    if (!typeid_cast<FilterStep *>(iter->node->step.get()) && !typeid_cast<ExpressionStep *>(iter->node->step.get())) break;
  }

  /// Dangling query plan node. This might be generated by StorageMerge.
  if (iter->node->step.get() == reading) return {};
  ...
  for (const auto &projection : projections)
    if (projection.type == ProjectionDescription::Type::Normal) normal_projections.push_back(&projection);

  if (normal_projections.empty()) return {};
  ...
  bool with_parent_part_offset = std::any_of(normal_projections.begin(), normal_projections.end(),
  ...
  QueryDAG query;
  {
    ...
    if (query.dag) query.dag->removeUnusedActions();
  }
  ...
  if (!force_optimize_projection) {
    ...
    if (!query.dag || !query.filter_node) return {};
  }
  ...
  if (!parent_reading_select_result) parent_reading_select_result = reading->selectRangesToRead();
  ...
  auto has_all_required_columns = [&](const ProjectionDescription *projection) {
    for (const auto &col : required_columns) {
      if (!projection->sample_block.findColumnOrSubcolumnByName(col) && !projection_virtuals->has(col)) return false;
    }
  ...
  if (query.dag) projection_query_info.filter_actions_dag = std::make_unique<ActionsDAG>(query.dag->clone());
  ...
  for (const auto *projection : normal_projections) {
    if (!has_all_required_columns(projection)) {
      ...
      if (query.filter_node && optimize_use_projection_filtering) {
        ...
        filterPartsAndCollectProjectionCandidates(*reading, *projection, reader, empty_mutations_snapshot, *parent_reading_select_result,
                                                  projection_query_info, query.filter_node, context);
      }
    ...
    MergeTreeDataSelectExecutor reader(reading->getMergeTreeData(), projection);
    bool analyzed = analyzeProjectionCandidate(candidate, reader, empty_mutations_snapshot, required_columns, *parent_reading_select_result,
                                               projection_query_info, context);
    ...
    if (candidate.sum_marks > parent_reading_marks ||
      ...
      stat.description =
          fmt::format("Projection {} is usable but requires reading {} marks, which is not better than the original table with {} marks",
    ...
    if (best_candidate == nullptr || candidate.sum_marks < best_candidate->sum_marks) best_candidate = &candidate;

    /// All parts has been filtered out; no need to analyze further projections.
    if (parent_reading_marks == 0) break;
  }
  ...
  if (query.dag && query.filter_node) {
    ...
    query.dag = QueryPlanOptimizations::splitAndFillPrewhereInfo(
        ...
        std::move(*query.dag), query.filter_node->result_name, {query.filter_node}, {query.filter_node});
  }
  ...
  auto projection_reading = reader.readFromParts(
      /*parts=*/{}, reading->getMutationsSnapshot()->cloneEmpty(), required_columns, proj_snapshot, projection_query_info, context,
      reading->getMaxBlockSize(), reading->getNumStreams(), max_added_blocks, best_candidate->merge_tree_projection_select_result_ptr,
      reading->isParallelReadingEnabled(), reading->getParallelReadingExtension());
  ...
  bool should_skip_projection_reading_on_remote_replicas =
      reading->isParallelReadingEnabled() && !is_parallel_replicas_initiator_with_projection_support && has_parent_parts;
  if (!projection_reading || should_skip_projection_reading_on_remote_replicas) {
    ...
    if (projection_query_info.prewhere_info) {
      ...
      pipe.addSimpleTransform([&](const SharedHeader &header) {
        return std::make_shared<FilterTransform>(header, filter_actions, projection_query_info.prewhere_info->prewhere_column_name,
  ...
  if (parent_reading_select_result->parts_with_ranges.empty()) {
    ...
    if (!blocksHaveEqualStructure(*main_stream, **proj_stream)) return {};
