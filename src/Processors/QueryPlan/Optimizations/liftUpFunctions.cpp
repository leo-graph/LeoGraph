...
static bool areNodesConvertableToBlock(const ActionsDAG::NodeRawConstPtrs &nodes) {
  ...
  for (const auto &node : nodes) {
    if (!names.emplace(node->result_name).second) return false;
  }
...
size_t tryExecuteFunctionsAfterSorting(QueryPlan::Node *parent_node, QueryPlan::Nodes &nodes, const Optimization::ExtraSettings &settings) {
  ...
  if (!sorting_step || !expression_step) return 0;

  // Filling step position should be preserved
  if (!child_node->children.empty())
    if (typeid_cast<FillingStep *>(child_node->children.front()->step.get())) return 0;

  NameSet sort_columns;
  for (const auto &col : sorting_step->getSortDescription()) sort_columns.insert(col.column_name);
  ...
  const auto &expression = expression_step->getExpression();
  {
    ...
    for (const auto *output : expression.getOutputs()) output_names.insert(output->result_name);
    for (const auto &sort_column : sort_columns)
      if (!output_names.contains(sort_column)) return 0;
  }
  ...
  if (unneeded_for_sorting.trivial()) return 0;
