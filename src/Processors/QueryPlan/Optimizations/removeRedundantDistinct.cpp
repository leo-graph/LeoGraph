...
void logActionsDAG(const String &prefix, const ActionsDAG &actions) {
  if constexpr (debug_logging_enabled) LOG_DEBUG(getLogger("redundantDistinct"), "{} :\n{}", prefix, actions.dumpDAG());
}
...
DistinctColumns getDistinctColumns(const DistinctStep *distinct) {
  ...
  for (const auto &column : distinct_columns) {
    if (!isColumnConst(*column.column) && column_names.contains(column.name)) non_const_columns.emplace(column.name);
  }
...
bool compareAggregationKeysWithDistinctColumns(const Names &aggregation_keys, const DistinctColumns &distinct_columns,
...
bool checkStepToAllowOptimization(const IQueryPlanStep *step) {
  if (typeid_cast<const DistinctStep *>(step)) return true;

  if (const auto *const expr = typeid_cast<const ExpressionStep *>(step); expr) return !expr->getExpression().hasArrayJoin();

  if (const auto *const filter = typeid_cast<const FilterStep *>(step); filter) return !filter->getExpression().hasArrayJoin();
...
bool passTillAggregation(const QueryPlan::Node *distinct_node) {
  ...
  while (!node->children.empty()) {
    ...
    if (inner_distinct_step = typeid_cast<DistinctStep *>(node->step.get()); inner_distinct_step) break;
  }
  if (inner_distinct_step) return false;

  if (aggregation_before_distinct) {
    if (actions_chain.empty()) actions_chain.push_back(std::move(dag_stack));
    ...
    if (const auto *aggregating_step = typeid_cast<const AggregatingStep *>(aggregation_before_distinct); aggregating_step) {
      return compareAggregationKeysWithDistinctColumns(aggregating_step->getParams().keys, distinct_columns, std::move(actions_chain));
    }
    if (const auto *merging_aggregated_step = typeid_cast<const MergingAggregatedStep *>(aggregation_before_distinct);
      ...
      return compareAggregationKeysWithDistinctColumns(merging_aggregated_step->getParams().keys, distinct_columns,
...
bool passTillDistinct(const QueryPlan::Node *distinct_node) {
  ...
  if (inner_distinct_step->isPreliminary()) return false;

  auto inner_distinct_columns = getDistinctColumns(inner_distinct_step);
  if (distinct_columns.size() != inner_distinct_columns.size()) return false;
  ...
  if (!dag_stack.empty()) {
    ...
    for (const auto &column : distinct_columns) {
      const auto *alias_node = original_node_finder.find(String(column));
      if (!alias_node) return false;

      auto it = inner_distinct_columns.find(alias_node->result_name);
      if (it == inner_distinct_columns.end()) return false;

      inner_distinct_columns.erase(it);
    }
  } else {
    if (distinct_columns != inner_distinct_columns) return false;
  }
...
bool canRemoveDistinct(const QueryPlan::Node *distinct_node) {
  if (passTillAggregation(distinct_node)) return true;

  if (passTillDistinct(distinct_node)) return true;
...
size_t tryRemoveRedundantDistinct(QueryPlan::Node *parent_node, QueryPlan::Nodes & /* nodes*/,
  ...
  for (auto &node : parent_node->children) {
    ...
    if (typeid_cast<const DistinctStep *>(node->step.get()) == nullptr) continue;
