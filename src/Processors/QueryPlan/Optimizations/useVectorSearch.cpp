...
size_t tryUseVectorSearch(QueryPlan::Node *parent_node, QueryPlan::Nodes & /*nodes*/, const Optimization::ExtraSettings &settings) {
  ...
  if (!limit_step) return no_layers_updated;

  if (node->children.size() != 1) return no_layers_updated;
  node = node->children.front();
  auto *sorting_step = typeid_cast<SortingStep *>(node->step.get());
  if (!sorting_step) return no_layers_updated;

  if (node->children.size() != 1) return no_layers_updated;
  node = node->children.front();
  auto *expression_step = typeid_cast<ExpressionStep *>(node->step.get());
  if (!expression_step) return no_layers_updated;
  ...
  if (!read_from_mergetree_step) {
    ...
    if (!filter_step) return no_layers_updated;
    if (node->children.size() != 1) return no_layers_updated;
    node = node->children.front();
    read_from_mergetree_step = typeid_cast<ReadFromMergeTree *>(node->step.get());
    if (!read_from_mergetree_step) return no_layers_updated;
    additional_filters_present = true;
  }

  if (const auto &prewhere_info = read_from_mergetree_step->getPrewhereInfo()) additional_filters_present = true;
  ...
  if (n > settings.max_limit_for_vector_search_queries) return no_layers_updated;

  /// Not 100% sure but other sort types are likely not what we want
  SortingStep::Type sorting_step_type = sorting_step->getType();
  if (sorting_step_type != SortingStep::Type::Full) return no_layers_updated;

  /// Read ORDER BY clause
  const auto &sort_description = sorting_step->getSortDescription();
  if (sort_description.size() > 1) return no_layers_updated;
  ...
  if (sort_column_node == nullptr || sort_column_node->type != ActionsDAG::ActionType::FUNCTION) return no_layers_updated;
  ...
  for (const auto *child : sort_column_node_children) {
    if (child->type == ActionsDAG::ActionType::ALIAS)  /// the analyzer
    {
      ...
      if (search_column_node->type == ActionsDAG::ActionType::INPUT) search_column = search_column_node->result_name;
    } else if (child->type == ActionsDAG::ActionType::INPUT)  /// old analyzer
    {
      ...
      if (data_type_array == nullptr) continue;
      ...
      if (data_type_nested_float64 == nullptr && data_type_nested_float32 == nullptr && data_type_nested_bfloat16 == nullptr) continue;
      ...
      if (!literal_column || literal_column->size() != 1) continue;
      Field field;
      literal_column->get(0, field);
      Field::Types::Which field_type = field.getType();
      if (field_type != Field::Types::Array) continue;
      ...
      for (const auto &field_array_value : field_array) {
        ...
        if (field_array_value_type != Field::Types::Float64) return no_layers_updated;
  ...
  if (search_column.empty() || reference_vector.empty()) return no_layers_updated;
  ...
  for (const auto &index : indexes) {
    if (index.type != "vector_similarity") continue;
  ...
  if (!has_vector_similarity_index) return no_layers_updated;

  /// All set for 2nd pass
  auto vector_search_parameters =
      std::make_optional<VectorSearchParameters>(search_column, distance_function, n, reference_vector, additional_filters_present, true);
  read_from_mergetree_step->setVectorSearchParameters(std::move(vector_search_parameters));

  return no_layers_updated;
}

bool optimizeVectorSearchSecondPass(QueryPlan::Node & /*root*/, Stack &stack, QueryPlan::Nodes & /*nodes*/,
  ...
  if (frame.node->children.size() != 1) return false;
  ...
  if (!read_from_mergetree_step) {
    ...
    if (!filter_step && !prewhere_expression_step) return false;
    ...
    read_from_mergetree_step = typeid_cast<ReadFromMergeTree *>(node->step.get());
    if (!read_from_mergetree_step) return false;
  }
  ...
  if (!vector_search_parameters.has_value()) return false;

  /// The optimization is only possible if the index-analyis and query execution
  /// are both executed on the same node.
  if (read_from_mergetree_step->isParallelReadingFromReplicas()) return false;
  ...
  if (const auto &prewhere_info = read_from_mergetree_step->getPrewhereInfo()) return false;

  /// Not 100% sure but other sort types are likely not what we want
  SortingStep::Type sorting_step_type = sorting_step->getType();
  if (sorting_step_type != SortingStep::Type::Full) return false;

  /// Read ORDER BY clause
  const auto &sort_description = sorting_step->getSortDescription();
  if (sort_description.size() > 1) return false;
  const String &sort_column = sort_description.front().column_name;
  ...
  if (optimize_plan) {
    ...
    if (optimize_plan) {
      ...
      if (need_cast) distance_node = &expression.addCast(*distance_node, result_type, "_CAST_distance", nullptr);
      ...
      if (filter_or_prewhere_node) {
        ...
        if (output_result_to_delete.empty()) output_result_to_delete = search_column;  /// old analyzer
        ...
        if (prewhere_expression_step)
          ...
          new_step = std::make_unique<FilterStep>(read_from_mergetree_step->getOutputHeader(), std::move(filter_expression),
