...
ReadFromMergeTree *findReadingStep(const QueryPlan::Node &node) {
  ...
  if (auto *reading = typeid_cast<ReadFromMergeTree *>(step)) {
    if (reading->isQueryWithFinal()) return nullptr;

    if (reading->isParallelReadingEnabled()) return nullptr;
...
ActionsDAG makeSourceDAG(ReadFromMergeTree &source) {
  if (const auto &prewhere_info = source.getPrewhereInfo()) return prewhere_info->prewhere_actions.clone();
...
bool updateDAG(const QueryPlan::Node &node, ActionsDAG &dag) {
  ...
  if (typeid_cast<DistinctStep *>(step)) return true;
...
JoinStep::PrimaryKeySharding findCommonPrimaryKeyPrefixByJoinKey(ReadFromMergeTree *lhs_reading, const ActionsDAG &lhs_dag,
  ...
  if (lhs_pk.column_names.empty()) return {};

  const auto &rhs_pk = rhs_reading->getStorageMetadata()->getPrimaryKey();
  if (rhs_pk.column_names.empty()) return {};
  ...
  for (size_t pos = 0; pos < lhs_pk_colum_names.size() && pos < rhs_pk_colum_names.size(); ++pos) {
    ...
    if (ldesc != rdesc) break;

    if (first) {
      first = false;
      sharding.is_reverse_order = ldesc;
    } else {
      if (sharding.is_reverse_order != ldesc) break;
    }
    ...
    if (!lhs_pk_output || !rhs_pk_output) break;
    ...
    for (size_t i = 0; i < keys_size && sharding.size() <= pos; ++i) {
      ...
      if (it == lhs_outputs.end() || jt == rhs_outputs.end()) continue;
      ...
      if (lhs_match == lhs_matches.end() || rhs_match == rhs_matches.end()) continue;

      /// Match wrapped into monotonic function is not supported,
      /// but strict monotonic functions should work.
      if (lhs_match->second.monotonicity || rhs_match->second.monotonicity) continue;
...
static void apply(struct JoinsAndSourcesWithCommonPrimaryKeyPrefix &data) {
  ...
  if (data.common_prefix == 0 || data.joins.empty()) return;
  ...
  for (auto &source : data.sources) {
    ...
    if (!analysis_result) analysis_result = source->selectRangesToRead();
  ...
  auto all_split = splitIntersectingPartsRangesIntoLayers(all_parts, data.sources.front()->getNumStreams(), data.common_prefix,
                                                          data.is_reverse_order, logger);
  ...
  for (auto &layer : all_split.layers) {
    std::sort(layer.begin(), layer.end(),
              [](const RangesInDataPart &lhs, const RangesInDataPart &rhs) { return lhs.part_index_in_query < rhs.part_index_in_query; });
  ...
  for (size_t i = 0; i < splits.size(); ++i) analysis_results[i]->split_parts = std::move(splits[i]);
  ...
  for (const auto &sorting_step : data.sorting_steps) sorting_step->convertToPartitionedFinishSorting();

  /// Do not break shards after full_sorting_merge JOIN. For hash join it is automatically true.
  for (const auto &join_step : data.joins_to_keep_in_order) join_step->keepLeftPipelineInOrder();
}
...
void optimizeJoinByShards(QueryPlan::Node &root) {
  ...
  while (!stack.empty()) {
    ...
    if (frame.next_child_to_process > 0) frame.results.push_back(std::move(result));
    ...
    if (auto *join_step = typeid_cast<JoinStep *>(frame.node->step.get())) {
      ...
      bool can_split_join = frame.results.back() != std::nullopt && can_split_left_table &&
                            (isLeft(kind) || isRight(kind) || isInner(kind) || isFull(kind)) && strictness != JoinStrictness::Asof &&
                            clauses.size() == 1;
      ...
      if (can_split_join) {
        ...
        sharding = findCommonPrimaryKeyPrefixByJoinKey(frame.results.front()->joins.sources.front(), frame.results.front()->dag,
                                                       frame.results.back()->joins.sources.front(), frame.results.back()->dag, clauses[0]);
      }
      ...
      if (!sharding.empty()) {
        ...
        result->joins.joins_to_keep_in_order.splice(result->joins.joins_to_keep_in_order.end(),
                                                    std::move(frame.results.back()->joins.joins_to_keep_in_order));
      ...
      if (updateDAG(*frame.node, frame.results[0]->dag)) result = std::move(frame.results[0]);
    }

    for (auto &cur_result : frame.results)
      if (cur_result) apply(cur_result->joins);

    stack.pop();
  }

  if (result) apply(result->joins);
}
