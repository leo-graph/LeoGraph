...
class RemoveRedundantSorting : public QueryPlanVisitor<RemoveRedundantSorting, debug_logging_enabled> {
 ...
 private:
  bool tryRemoveSorting(QueryPlan::Node *sorting_node, QueryPlan::Node *parent_node) {
    if (!canRemoveCurrentSorting()) return false;
    ...
    for (StackWithParent::const_reverse_iterator it = stack.rbegin() + 1; it != stack.rend(); ++it) {
      ...
      if (typeid_cast<const SortingStep *>(step) && node != nodes_affect_order.back()) continue;
  ...
  static bool checkNodeAffectingOrder(QueryPlan::Node *node_affect_order) {
    ...
    if (const AggregatingStep *parent_aggr = typeid_cast<AggregatingStep *>(step_affect_order); parent_aggr) {
      if (parent_aggr->inOrder()) return false;

      auto const &aggregates = parent_aggr->getParams().aggregates;
      for (const auto &aggregate : aggregates) {
        auto action = NullsAction::EMPTY;
        auto aggregate_function_properties = AggregateFunctionFactory::instance().tryGetProperties(aggregate.function->getName(), action);
        if (aggregate_function_properties && aggregate_function_properties->is_order_dependent) return false;

        /// sum*() with Floats depends on order
        /// but currently, there is no way to specify property `is_order_dependent` for combination of aggregating function and data type as
        /// argument so, we check explicitly for sum*() functions with Floats here
        ...
        if (func_name.starts_with("sum")) {
          ...
          if (WhichDataType(removeNullable(data_type)).isFloat()) return false;
        }
      }
      return true;
    }
    ...
    if (const auto *next_sorting = typeid_cast<const SortingStep *>(step_affect_order); next_sorting) {
      if (next_sorting->getType() == SortingStep::Type::Full) return true;
    }
  ...
  bool checkPathFromCurrentSortingNode(const QueryPlan::Node *node_affect_order) {
    ...
    for (StackWithParent::const_reverse_iterator it = stack.rbegin() + 1; it != stack.rend(); ++it) {
      ...
      if (typeid_cast<const WindowStep *>(step)) return false;

      if (const auto *join_step = typeid_cast<const JoinStep *>(step); join_step) {
        if (typeid_cast<const FullSortingMergeJoin *>(join_step->getJoin().get())) return false;
      }
    }
...
void tryRemoveRedundantSorting(QueryPlan::Node *root) { RemoveRedundantSorting(root).visit(); }
