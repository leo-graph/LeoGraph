...
static Block deserializeHeader(ReadBuffer &in) {
  ...
  for (auto &column : columns) column.column = column.type->createColumn();
...
void QueryPlan::ensureSerialized(size_t max_supported_version) const {
  if (serialized_plan) return;  // Already serialized
...
std::string_view QueryPlan::getSerializedData() const {
  if (!serialized_plan) throw Exception(ErrorCodes::LOGICAL_ERROR, "Query plan is not serialized. Call ensureSerialized() first.");

  return serialized_plan->stringView();
}

bool QueryPlan::isSerialized() const { return serialized_plan != nullptr; }
...
QueryPlanAndSets QueryPlan::deserialize(ReadBuffer &in, const ContextPtr &context) {
  ...
  if (version > DBMS_QUERY_PLAN_SERIALIZATION_VERSION)
    throw Exception(ErrorCodes::NOT_IMPLEMENTED, "Query plan serialization version {} is not supported. The last supported version is {}",
...
QueryPlanAndSets QueryPlan::deserialize(ReadBuffer &in, const ContextPtr &context, const SerializationFlags &flags) {
  ...
  while (!stack.empty()) {
    ...
    for (const auto &child : frame.children) input_headers.push_back(child->step->getOutputHeader());
    ...
    if (step->hasOutputHeader()) {
      assertCompatibleHeader(*step->getOutputHeader(), *output_header, fmt::format("deserialization of query plan {} step", step_name));
    } else if (output_header->columns())
      throw Exception(ErrorCodes::INCORRECT_DATA, "Deserialized step {} has no output stream, but deserialized header is not empty : {}",
