...
NegativeLimitStep::NegativeLimitStep(const SharedHeader &input_header_, UInt64 limit_, UInt64 offset_)
    : ITransformingStep(input_header_, input_header_, getTraits()), limit(limit_), offset(offset_) {}
...
void registerNegativeLimitStep(QueryPlanStepRegistry &registry) { registry.registerStep("NegativeLimit", NegativeLimitStep::deserialize); }
