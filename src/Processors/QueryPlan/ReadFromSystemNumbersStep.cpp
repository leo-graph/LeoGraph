...
class NumbersSource : public ISource {
 public:
  NumbersSource(UInt64 block_size_, UInt64 start_, const std::string &column_name, UInt64 step_between_chunks_)
      : ISource(createHeader(column_name)), block_size(block_size_), next(start_), step_between_chunks(step_between_chunks_) {}
...
std::optional<RangeWithStep> steppedRangeFromRange(const Range &r, UInt64 step, UInt64 remainder) {
  if ((r.right.safeGet<UInt64>() == 0) && (!r.right_included)) return std::nullopt;
  UInt64 begin = (r.left.safeGet<UInt64>() / step) * step;
  if (begin > std::numeric_limits<UInt64>::max() - remainder) return std::nullopt;
  begin += remainder;

  while ((r.left_included <= r.left.safeGet<UInt64>()) && (begin <= r.left.safeGet<UInt64>() - r.left_included)) {
    if (std::numeric_limits<UInt64>::max() - step < begin) return std::nullopt;
    begin += step;
  }

  if ((begin >= r.right_included) && (begin - r.right_included >= r.right.safeGet<UInt64>())) return std::nullopt;
...
class NumbersRangedSource : public ISource {
 public:
  ...
  NumbersRangedSource(const RangesWithStep &ranges_, RangesStatePtr &ranges_state_, UInt64 base_block_size_, UInt64 step_,
 ...
 private:
  ...
  std::pair<RangesPos, UInt64> findRanges() {
    ...
    while (maximum_can_take != 0) {
      if (end.offset_in_ranges == ranges.size()) break;  /// No ranges left at all
...
ReadFromSystemNumbersStep::ReadFromSystemNumbersStep(const Names &column_names_, const SelectQueryInfo &query_info_,
                                                     const StorageSnapshotPtr &storage_snapshot_, const ContextPtr &context_,
                                                     StoragePtr storage_, size_t max_block_size_, size_t num_streams_)
    : SourceStepWithFilter(std::make_shared<const Block>(storage_snapshot_->getSampleBlockForColumns(column_names_)), column_names_,
                           query_info_, storage_snapshot_, context_),
...
QueryPlanStepPtr ReadFromSystemNumbersStep::clone() const {
  return std::make_unique<ReadFromSystemNumbersStep>(column_names, getQueryInfo(), getStorageSnapshot(), getContext(), storage,
...
Pipe ReadFromSystemNumbersStep::makePipe() {
  ...
  if (!numbers_storage.multithreaded) num_streams = 1;
  ...
  if (!numbers_storage.limit.has_value() && NumbersLikeUtils::isUniverse(extracted_ranges.ranges) && !can_pushdown_query_limit) {
    ...
    for (size_t i = 0; i < num_streams; ++i) {
      ...
      auto source = std::make_shared<NumbersSource>(max_block_size, source_start, numbers_storage.column_name, step_between_chunks);
  ...
  auto append_stepped_intersections = [&](const Range &domain_range, UInt64 domain_remainder) {
    ...
    for (const auto &r : extracted_ranges.ranges) {
      ...
      if (!maybe_intersected_range) continue;

      auto maybe_range_with_step = steppedRangeFromRange(*maybe_intersected_range, step, domain_remainder);
      if (maybe_range_with_step) intersected_ranges.push_back(*maybe_range_with_step);
    }
  };
  ...
  if (numbers_storage.limit.has_value()) {
    ...
    if (std::numeric_limits<UInt64>::max() - numbers_storage.offset >= storage_limit) {
      append_stepped_intersections(Range(FieldRef(numbers_storage.offset), true, FieldRef(numbers_storage.offset + storage_limit), false),
                                   remainder);
    }
    ...
    /// [18446744073709551614, 18446744073709551615] then [0, 2].
      ...
      append_stepped_intersections(Range(FieldRef(numbers_storage.offset), true, FieldRef(std::numeric_limits<UInt64>::max()), true),
                                   remainder);
  ...
  if (total_size / max_block_size < num_streams) num_streams = static_cast<size_t>(total_size / max_block_size);
  ...
  for (size_t i = 0; i < num_streams; ++i) {
    auto source = std::make_shared<NumbersRangedSource>(intersected_ranges, ranges_state, max_block_size, numbers_storage.step,
                                                        numbers_storage.column_name);

    if (i == 0) source->addTotalRowsApprox(static_cast<size_t>(total_size));
