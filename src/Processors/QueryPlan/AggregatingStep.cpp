...
static bool memoryBoundMergingWillBeUsed(bool should_produce_results_in_order_of_bucket_number,
...
Block appendGroupingSetColumn(Block header) {
  ...
  for (auto &col : header) res.insert(std::move(col));
...
Block generateOutputHeader(const Block &input_header, const Names &keys, bool use_nulls) {
  ...
  if (use_nulls) convertToNullable(header, keys);
  return header;
}

Block AggregatingStep::appendGroupingColumn(const Block &block, const Names &keys, bool has_grouping, bool use_nulls) {
  if (!has_grouping) return block;

  return generateOutputHeader(block, keys, use_nulls);
}

AggregatingStep::AggregatingStep(const SharedHeader &input_header_, Aggregator::Params params_,
                                 GroupingSetsParamsList grouping_sets_params_, bool final_, size_t max_block_size_,
                                 size_t aggregation_in_order_max_block_bytes_, size_t merge_threads_, size_t temporary_data_merge_threads_,
                                 bool storage_has_evenly_distributed_read_, bool group_by_use_nulls_,
                                 SortDescription sort_description_for_merging_, SortDescription group_by_sort_description_,
                                 bool should_produce_results_in_order_of_bucket_number_,
                                 bool memory_bound_merging_of_aggregation_results_enabled_,
                                 bool explicit_sorting_required_for_aggregation_in_order_)
    : ITransformingStep(input_header_,
                        std::make_shared<const Block>(appendGroupingColumn(params_.getHeader(*input_header_, final_), params_.keys,
                                                                           !grouping_sets_params_.empty(), group_by_use_nulls_)),
                        getTraits(should_produce_results_in_order_of_bucket_number_), false),
...
const SortDescription &AggregatingStep::getSortDescription() const {
  if (memoryBoundMergingWillBeUsed()) return group_by_sort_description;

  return IQueryPlanStep::getSortDescription();
}

static void updateThreadsValues(size_t &new_merge_threads, size_t &new_temporary_data_merge_threads, Aggregator::Params &params,
                                const BuildQueryPipelineSettings &settings) {
  /// Update values from settings if plan was deserialized.
  if (new_merge_threads == 0) new_merge_threads = settings.max_threads;

  if (new_temporary_data_merge_threads == 0) new_temporary_data_merge_threads = settings.aggregation_memory_efficient_merge_threads;
  if (new_temporary_data_merge_threads == 0) new_temporary_data_merge_threads = new_merge_threads;

  if (params.max_threads == 0) params.max_threads = settings.max_threads;
}

ActionsDAG AggregatingStep::makeCreatingMissingKeysForGroupingSetDAG(const Block &in_header, const Block &out_header,
                                                                     const GroupingSetsParamsList &grouping_sets_params, UInt64 group,
  ...
  const auto *grouping_node = &dag.addColumn({ColumnPtr(std::move(grouping_col)), std::make_shared<DataTypeUInt64>(), "__grouping_set"});
  ...
  for (size_t i = 0; i < out_header.columns(); ++i) {
    ...
    const auto missing_it =
        std::find_if(missing_columns.begin(), missing_columns.end(), [&](const auto &missing_col) { return missing_col == col.name; });
    const auto used_it = std::find_if(used_keys.begin(), used_keys.end(), [&](const auto &used_col) { return used_col == col.name; });
...
void AggregatingStep::transformPipeline(QueryPipelineBuilder &pipeline, const BuildQueryPipelineSettings &settings) {
  ...
  if (!grouping_sets_params.empty()) {
    ...
    pipeline.transform([&](OutputPortRawPtrs ports) {
        ...
        if (streams > 1) {
          ...
          for (size_t j = 0; j < streams; ++j) {
            auto aggregation_for_set = std::make_shared<AggregatingTransform>(
                input_header, transform_params_for_set, many_data, j, new_merge_threads, new_temporary_data_merge_threads,
                should_produce_results_in_order_of_bucket_number, skip_merging);
          ...
          auto aggregation_for_set = std::make_shared<AggregatingTransform>(input_header, transform_params_for_set, dataflow_cache_updater);
      ...
      if (group_by_use_nulls) convertToNullable(output_header, params.keys);
  ...
  if (!sort_description_for_merging.empty()) {
    ...
    if (explicit_sorting_required_for_aggregation_in_order) {
      ...
      SortingStep::fullSortStreams(pipeline, SortingStep::Settings(params.max_block_size), sort_description_for_merging, 0 /* limit */);
    }
    ...
    if (pipeline.getNumStreams() > 1) {
      ...
      pipeline.addSimpleTransform([&](const SharedHeader &header) {
        ...
        return std::make_shared<AggregatingInOrderTransform>(
            header, transform_params, sort_description_for_merging, group_by_sort_description, max_block_size,
            aggregation_in_order_max_block_bytes / new_merge_threads, many_data, counter++);
      });

      if (skip_merging) {
        pipeline.addSimpleTransform(
            [&](const SharedHeader &header) { return std::make_shared<FinalizeAggregatedTransform>(header, transform_params); });
        pipeline.resize(params.max_threads);
      ...
      auto transform = std::make_shared<FinishAggregatingInOrderTransform>(pipeline.getSharedHeader(), pipeline.getNumStreams(),
                                                                           transform_params, group_by_sort_description, max_block_size,
      ...
      pipeline.addSimpleTransform([&](const SharedHeader &header) {
        return std::make_shared<AggregatingInOrderTransform>(header, transform_params, sort_description_for_merging,
                                                             group_by_sort_description, max_block_size,
                                                             aggregation_in_order_max_block_bytes);
      });
  ...
  if (pipeline.getNumStreams() > 1) {
    ...
    size_t counter = 0;
    pipeline.addSimpleTransform([&](const SharedHeader &header) {
      return std::make_shared<AggregatingTransform>(header, transform_params, many_data, counter++, new_merge_threads,
                                                    new_temporary_data_merge_threads, should_produce_results_in_order_of_bucket_number,
                                                    skip_merging, dataflow_cache_updater);
    });

    pipeline.resize(should_produce_results_in_order_of_bucket_number ? 1 : params.max_threads, false,
...
void AggregatingStep::describeActions(JSONBuilder::JSONMap &map) const {
  params.explain(map);
  if (!sort_description_for_merging.empty()) map.add("Order", dumpSortDescription(sort_description_for_merging));
...
void AggregatingStep::requestOnlyMergeForAggregateProjection(const SharedHeader &input_header) {
  if (!canUseProjection()) throw Exception(ErrorCodes::LOGICAL_ERROR, "Cannot aggregate from projection");
...
std::unique_ptr<AggregatingProjectionStep> AggregatingStep::convertToAggregatingProjection(const SharedHeader &input_header) const {
  ...
  auto aggregating_projection = std::make_unique<AggregatingProjectionStep>(SharedHeaders{input_headers.front(), input_header}, params,
                                                                            final, merge_threads, temporary_data_merge_threads);
...
void AggregatingStep::updateOutputHeader() {
  output_header = std::make_shared<const Block>(appendGroupingColumn(params.getHeader(*input_headers.front(), final), params.keys,
                                                                     !grouping_sets_params.empty(), group_by_use_nulls));
}

bool AggregatingStep::memoryBoundMergingWillBeUsed() const {
  return DB::memoryBoundMergingWillBeUsed(should_produce_results_in_order_of_bucket_number,
                                          memory_bound_merging_of_aggregation_results_enabled, sort_description_for_merging);
}

AggregatingProjectionStep::AggregatingProjectionStep(SharedHeaders input_headers_, Aggregator::Params params_, bool final_,
                                                     size_t merge_threads_, size_t temporary_data_merge_threads_)
    : params(std::move(params_)),
      final(final_),
      merge_threads(merge_threads_),
      temporary_data_merge_threads(temporary_data_merge_threads_) {
...
void AggregatingProjectionStep::updateOutputHeader() {
  if (input_headers.size() != 2)
    throw Exception(ErrorCodes::LOGICAL_ERROR, "AggregatingProjectionStep is expected to have two input streams, got {}",
                    input_headers.size());
...
QueryPipelineBuilderPtr AggregatingProjectionStep::updatePipeline(QueryPipelineBuilders pipelines,
                                                                  const BuildQueryPipelineSettings &settings) {
  size_t new_merge_threads = merge_threads;
  size_t new_temporary_data_merge_threads = temporary_data_merge_threads;
  updateThreadsValues(new_merge_threads, new_temporary_data_merge_threads, params, settings);
  ...
  /// ...
  auto many_data =
      std::make_shared<ManyAggregatedData>(normal_parts_pipeline->getNumStreams() + projection_parts_pipeline->getNumStreams());
  size_t counter = 0;
  ...
  auto build_aggregate_pipeline = [&](QueryPipelineBuilder &pipeline, bool projection) {
    ...
    if (projection) params_copy.only_merge = true;

    AggregatingTransformParamsPtr transform_params =
        std::make_shared<AggregatingTransformParams>(pipeline.getHeader(), std::move(params_copy), aggregator_list_ptr, final);
...
void AggregatingStep::serializeSettings(QueryPlanSerializationSettings &settings) const {
  ...
  settings[QueryPlanSerializationSetting::min_hit_rate_to_use_consecutive_keys_optimization] =
      params.min_hit_rate_to_use_consecutive_keys_optimization;

  settings[QueryPlanSerializationSetting::collect_hash_table_stats_during_aggregation] =
      params.stats_collecting_params.isCollectionAndUseEnabled();
  settings[QueryPlanSerializationSetting::max_entries_for_hash_table_stats] =
      params.stats_collecting_params.max_entries_for_hash_table_stats;
  ...
  settings[QueryPlanSerializationSetting::enable_producing_buckets_out_of_order_in_aggregation] =
...
void AggregatingStep::serialize(Serialization &ctx) const {
  ...
  if (explicit_sorting_required_for_aggregation_in_order)
    throw Exception(ErrorCodes::NOT_IMPLEMENTED,
                    "Serialization of AggregatingStep explicit_sorting_required_for_aggregation_in_order is not supported.");
  ...
  /// * settings are something which already was in settings[QueryPlanSerializationSetting::h] and, usually, is passed to Aggregator
  /// unchanged Flags `final` and `group_by_use_nulls` change types, and `overflow_row` appends additional block to results. Settings like
  /// `max_rows_to_group_by` or `empty_result_for_aggregation_by_empty_set` affect the result, but does not change data format. Overall, the
  /// rule is not strict.

  UInt8 flags = 0;
  if (final && !ctx.skip_final_flag) flags |= 1;
  if (params.overflow_row) flags |= 2;
  if (group_by_use_nulls) flags |= 4;
  if (!grouping_sets_params.empty()) flags |= 8;
  ...
  if (params.stats_collecting_params.isCollectionAndUseEnabled()) flags |= 16;

  writeIntBinary(flags, ctx.out);

  if (explicit_sorting_required_for_aggregation_in_order) serializeSortDescription(group_by_sort_description, ctx.out);

  writeVarUInt(params.keys.size(), ctx.out);
  for (const auto &key : params.keys) writeStringBinary(key, ctx.out);
  ...
  if (!grouping_sets_params.empty()) {
    ...
    for (const auto &grouping_set : grouping_sets_params) {
      ...
      for (const auto &used_key : grouping_set.used_keys) writeStringBinary(used_key, ctx.out);
    }
  }
...
QueryPlanStepPtr AggregatingStep::deserialize(Deserialization &ctx) {
  if (ctx.input_headers.size() != 1) throw Exception(ErrorCodes::INCORRECT_DATA, "AggregatingStep must have one input stream");
  ...
  Names keys(num_keys);
  for (auto &key : keys) readStringBinary(key, ctx.in);
  ...
  if (has_grouping_sets) {
    ...
    for (size_t group_num = 0; group_num < num_groups; ++group_num) {
      ...
      if (num_keys > num_used_keys) grouping_set.missing_keys.reserve(num_keys - num_used_keys);
      for (const auto &key : keys)
        if (!used_keys_set.contains(key)) grouping_set.missing_keys.push_back(key);
    }
  }
  ...
  UInt64 stats_key = 0;
  if (has_stats_key) readIntBinary(stats_key, ctx.in);

  StatsCollectingParams stats_collecting_params(stats_key,
  ...
  auto aggregating_step =
      std::make_unique<AggregatingStep>(ctx.input_headers.front(), std::move(params), std::move(grouping_sets_params), final,
                                        ctx.settings[QueryPlanSerializationSetting::max_block_size],
                                        ...
                                        group_by_use_nulls, std::move(sort_description_for_merging), SortDescription{},
                                        ctx.settings[QueryPlanSerializationSetting::aggregation_in_order_memory_bound_merging],
                                        ctx.settings[QueryPlanSerializationSetting::aggregation_sort_result_by_bucket_number], false);
...
QueryPlanStepPtr AggregatingStep::clone() const {
  return std::make_unique<AggregatingStep>(
      input_headers.front(), params, grouping_sets_params, final, max_block_size, aggregation_in_order_max_block_bytes, merge_threads,
      temporary_data_merge_threads, storage_has_evenly_distributed_read, group_by_use_nulls, sort_description_for_merging,
      group_by_sort_description, should_produce_results_in_order_of_bucket_number, memory_bound_merging_of_aggregation_results_enabled,
      explicit_sorting_required_for_aggregation_in_order);
}

void AggregatingStep::setFinal(bool new_value) {
  if (new_value == final) return;
...
void registerAggregatingStep(QueryPlanStepRegistry &registry) { registry.registerStep("Aggregating", AggregatingStep::deserialize); }
