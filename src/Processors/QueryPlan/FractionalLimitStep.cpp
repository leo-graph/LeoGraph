...
FractionalLimitStep::FractionalLimitStep(const SharedHeader &input_header_, Float64 limit_fraction_, Float64 offset_fraction_,
                                         UInt64 offset_, bool with_ties_, SortDescription description_)
...
void FractionalLimitStep::transformPipeline(QueryPipelineBuilder &pipeline, const BuildQueryPipelineSettings &) {
  auto transform = std::make_shared<FractionalLimitTransform>(pipeline.getSharedHeader(), limit_fraction, offset_fraction, offset,
                                                              pipeline.getNumStreams(), with_ties, description);
...
void FractionalLimitStep::describeActions(FormatSettings &settings) const {
  ...
  if (with_ties) settings.out << prefix << "WITH TIES" << '\n';
}
...
void FractionalLimitStep::serialize(Serialization &ctx) const {
  ...
  if (with_ties) serializeSortDescription(description, ctx.out);
}
...
QueryPlanStepPtr FractionalLimitStep::deserialize(Deserialization &ctx) {
  ...
  if (with_ties) deserializeSortDescription(description, ctx.in);

  return std::make_unique<FractionalLimitStep>(ctx.input_headers.front(), limit_fraction, offset_fraction, offset, with_ties,
