...
bool isNodeOverSortingKey(const ActionsDAG::Node *node, const NameSet &sorting_key_set) {
  if (sorting_key_set.contains(node->result_name)) return true;
  if (node->type == ActionsDAG::ActionType::COLUMN) return true;  // constants are fine
  if (node->type == ActionsDAG::ActionType::INPUT) return false;  // already checked result_name
  for (const auto *child : node->children)
    if (!isNodeOverSortingKey(child, sorting_key_set)) return false;
  return true;
}
...
bool restorePrewhereInputs(FilterDAGInfo *row_level_filter, PrewhereInfo *info, const NameSet &inputs) {
  bool added = false;
  if (row_level_filter) added = added || restoreDAGInputs(row_level_filter->actions, inputs);

  if (info) added = added || restoreDAGInputs(info->prewhere_actions, inputs);
...
static bool checkAllPartsOnRemoteFS(const RangesInDataParts &parts) {
  for (const auto &part : parts) {
    if (!part.data_part->isStoredOnRemoteDisk()) return false;
  }
  return true;
}

/// build sort description for output stream
static SortDescription getSortDescriptionForOutputHeader(const SharedHeader &output_header, const Names &sorting_key_columns,
                                                         const std::vector<bool> &reverse_flags, const int sort_direction,
                                                         InputOrderInfoPtr input_order_info, const FilterDAGInfoPtr &row_level_filter,
                                                         const PrewhereInfoPtr &prewhere_info, bool enable_vertical_final) {
  ...
  if (prewhere_info) {
    {
      ...
      for (auto &column : original_header) {
        ...
        if (original_node) column.name = original_node->result_name;
      }
    }
  }
  ...
  for (size_t i = 0; i < sort_columns_size; ++i) {
    ...
    const auto it = std::find_if(original_header.begin(), original_header.end(),
                                 [&sorting_key](const auto &column) { return column.name == sorting_key; });
    if (it == original_header.end()) break;
  ...
  if (input_order_info && !enable_vertical_final) {
    ...
    if (sort_description.size() > used_prefix_of_sorting_key_size) sort_description.resize(used_prefix_of_sorting_key_size);
...
std::shared_ptr<QueryIdHolder> ReadFromMergeTree::AnalysisResult::checkLimits(const Context &context_, const MergeTreeData &data_,
                                                                              const MergeTreeSettings &data_settings_) const {
  ...
  if (max_partitions_to_read > 0) {
    ...
    for (const auto &part_with_ranges : parts_with_ranges) partitions.insert(part_with_ranges.data_part->info.getPartitionId());
    if (partitions.size() > static_cast<size_t>(max_partitions_to_read)) {
      throw Exception(ErrorCodes::TOO_MANY_PARTITIONS, "Too many partitions to read. Current {}, max {}", partitions.size(),
  ...
  if (data_settings_[MergeTreeSetting::max_concurrent_queries] > 0 &&
    ...
    if (!query_id.empty()) return data_.getQueryIdHolder(query_id, data_settings_[MergeTreeSetting::max_concurrent_queries]);
  }

  return nullptr;
}

ReadFromMergeTree::ReadFromMergeTree(RangesInDataPartsPtr parts_, MergeTreeData::MutationsSnapshotPtr mutations_, Names all_column_names_,
                                     const MergeTreeData &data_, MergeTreeSettingsPtr data_settings_, const SelectQueryInfo &query_info_,
                                     const StorageSnapshotPtr &storage_snapshot_, const ContextPtr &context_, size_t max_block_size_,
                                     size_t num_streams_, PartitionIdToMaxBlockPtr max_block_numbers_to_read_, LoggerPtr log_,
                                     AnalysisResultPtr analyzed_result_ptr_, bool enable_parallel_reading_,
    ...
    : SourceStepWithFilter(
          std::make_shared<const Block>(MergeTreeSelectProcessor::transformHeader(
              storage_snapshot_->getSampleBlockForColumns(all_column_names_), query_info_.row_level_filter, query_info_.prewhere_info)),
          all_column_names_, query_info_, storage_snapshot_, context_),
      ...
      block_size{.max_block_size_rows = max_block_size_,
      ...
      enable_remove_parts_from_snapshot_optimization(!context->getSettingsRef()[Setting::enable_shared_storage_snapshot_in_query] &&
                                                     query_info_.merge_tree_enable_remove_parts_from_snapshot_optimization),
...
std::unique_ptr<ReadFromMergeTree> ReadFromMergeTree::createLocalParallelReplicasReadingStep(
    ContextPtr &context_, AnalysisResultPtr analyzed_result_ptr_, MergeTreeAllRangesCallback all_ranges_callback_,
    MergeTreeReadTaskCallback read_task_callback_, size_t replica_number) {
  ...
  return std::make_unique<ReadFromMergeTree>(
      ...
      mutations_snapshot, all_column_names, data, data_settings, getQueryInfo(), getStorageSnapshot(), context_,
      block_size.max_block_size_rows, requested_num_streams, max_block_numbers_to_read, log, std::move(analyzed_result_ptr_),
      enable_parallel_reading, all_ranges_callback_, read_task_callback_, replica_number);
}

Pipe ReadFromMergeTree::readFromPoolParallelReplicas(RangesInDataParts parts_with_range,
                                                     const MergeTreeIndexBuildContextPtr &index_build_context, Names required_columns,
                                                     PoolSettings pool_settings) {
  const auto &client_info = context->getClientInfo();

  auto extension = ParallelReadingExtension{all_ranges_callback.value(), read_task_callback.value(),
  ...
  auto pool = std::make_shared<MergeTreeReadPoolParallelReplicas>(std::move(extension), std::move(parts_with_range), mutations_snapshot,
                                                                  shared_virtual_fields, index_read_tasks, storage_snapshot,
                                                                  query_info.row_level_filter, query_info.prewhere_info, actions_settings,
                                                                  reader_settings, required_columns, pool_settings, block_size, context);
  ...
  for (size_t i = 0; i < pool_settings.threads; ++i) {
    ...
    auto processor =
        std::make_unique<MergeTreeSelectProcessor>(pool, std::move(algorithm), query_info.row_level_filter, query_info.prewhere_info,
                                                   index_read_tasks, actions_settings, reader_settings, index_build_context);
...
Pipe ReadFromMergeTree::readFromPool(RangesInDataParts parts_with_range, const MergeTreeIndexBuildContextPtr &index_build_context,
                                     Names required_columns, PoolSettings pool_settings) {
  ...
  if (use_prefetched_read_pool) {
    pool = std::make_shared<MergeTreePrefetchedReadPool>(std::move(parts_with_range), mutations_snapshot, shared_virtual_fields,
                                                         index_read_tasks, storage_snapshot, query_info.row_level_filter,
                                                         query_info.prewhere_info, actions_settings, reader_settings, required_columns,
                                                         pool_settings, block_size, context, dataflow_cache_updater);
  } else {
    pool =
        std::make_shared<MergeTreeReadPool>(std::move(parts_with_range), mutations_snapshot, shared_virtual_fields, index_read_tasks,
                                            storage_snapshot, query_info.row_level_filter, query_info.prewhere_info, actions_settings,
                                            reader_settings, required_columns, pool_settings, block_size, context, dataflow_cache_updater);
  }
  ...
  for (size_t i = 0; i < pool_settings.threads; ++i) {
    ...
    if (i == 0) source->addTotalRowsApprox(total_rows);

    pipes.emplace_back(std::move(source));
  }

  auto pipe = Pipe::unitePipes(std::move(pipes));
  if (output_streams_limit && output_streams_limit < pipe.numOutputPorts()) pipe.resize(output_streams_limit);
  return pipe;
}

Pipe ReadFromMergeTree::readInOrder(RangesInDataParts parts_with_ranges, const MergeTreeIndexBuildContextPtr &index_build_context,
                                    Names required_columns, PoolSettings pool_settings, ReadType read_type, UInt64 read_limit) {
  ...
  if (is_parallel_reading_from_replicas) {
    const auto &client_info = context->getClientInfo();
    ParallelReadingExtension extension{all_ranges_callback.value(), read_task_callback.value(),
                                       number_of_current_replica.value_or(client_info.number_of_current_replica),
                                       context->getClusterForParallelReplicas()->getShardsInfo().at(0).getAllNodeCount()};

    CoordinationMode mode = read_type == ReadType::InOrder ? CoordinationMode::WithOrder : CoordinationMode::ReverseOrder;

    pool = std::make_shared<MergeTreeReadPoolParallelReplicasInOrder>(
        std::move(extension), mode, parts_with_ranges, mutations_snapshot, shared_virtual_fields, index_read_tasks,
        has_limit_below_one_block, storage_snapshot, query_info.row_level_filter, query_info.prewhere_info, actions_settings,
        reader_settings, required_columns, pool_settings, block_size, context);
  } else {
    pool = std::make_shared<MergeTreeReadPoolInOrder>(
        has_limit_below_one_block, read_type, parts_with_ranges, mutations_snapshot, shared_virtual_fields, index_read_tasks,
        storage_snapshot, query_info.row_level_filter, query_info.prewhere_info, actions_settings, reader_settings, required_columns,
        pool_settings, block_size, context, dataflow_cache_updater);
  }
  ...
  for (size_t i = 0; i < parts_with_ranges.size(); ++i) {
    ...
    LOG_TRACE(log, "Reading {} ranges in{}order from part {}, approx. {} rows starting from {}", part_with_ranges.ranges.size(),
              read_type == ReadType::InReverseOrder ? " reverse " : " ", part_with_ranges.data_part->name, total_rows,
    ...
    auto source = std::make_shared<MergeTreeSource>(std::move(processor), data.getLogName());
    if (set_total_rows_approx) source->addTotalRowsApprox(total_rows);
    ...
    if (virtual_row_conversion && (read_type == ReadType::InOrder)) {
      ...
      if (index->size() >= num_pk_columns_required) {
        ...
        pipe.addSimpleTransform(
            [&](const SharedHeader &header) { return std::make_shared<VirtualRowTransform>(header, pk_block, virtual_row_conversion); });
      }
    }
  ...
  if (read_type == ReadType::InReverseOrder) {
    pipe.addSimpleTransform([&](const SharedHeader &header) { return std::make_shared<ReverseTransform>(header); });
  }

  return pipe;
}

Pipe ReadFromMergeTree::read(RangesInDataParts parts_with_range, const MergeTreeIndexBuildContextPtr &index_build_context,
                             Names required_columns, ReadType read_type, size_t max_streams, size_t min_marks_for_concurrent_read,
  ...
  const size_t total_query_nodes = is_parallel_reading_from_replicas
                                       ? std::min<size_t>(context->getClusterForParallelReplicas()->getShardsInfo().at(0).getAllNodeCount(),
  ...
  if (read_type == ReadType::ParallelReplicas)
    return readFromPoolParallelReplicas(std::move(parts_with_range), index_build_context, std::move(required_columns),
                                        std::move(pool_settings));
  ...
  if (read_type == ReadType::Default && (max_streams > 1 || checkAllPartsOnRemoteFS(parts_with_range)))
    return readFromPool(std::move(parts_with_range), index_build_context, std::move(required_columns), std::move(pool_settings));
...
struct PartRangesReadInfo {
  ...
  PartRangesReadInfo(const RangesInDataParts &parts, const Settings &settings, const MergeTreeSettings &data_settings) {
    ...
    for (size_t i = 0; i < parts.size(); ++i) {
      ...
      if (parts[i].data_part->index_granularity_info.mark_type.adaptive) ++adaptive_parts;
    }

    if (adaptive_parts > parts.size() / 2) index_granularity_bytes = data_settings[MergeTreeSetting::index_granularity_bytes];

    max_marks_to_use_cache = MergeTreeDataSelectExecutor::roundRowsOrBytesToMarks(
        settings[Setting::merge_tree_max_rows_to_use_cache], settings[Setting::merge_tree_max_bytes_to_use_cache],
        data_settings[MergeTreeSetting::index_granularity], index_granularity_bytes);
    ...
    min_marks_for_concurrent_read = MergeTreeDataSelectExecutor::minMarksForConcurrentRead(
        min_rows_for_concurrent_read, min_bytes_for_concurrent_read, data_settings[MergeTreeSetting::index_granularity],
        index_granularity_bytes, settings[Setting::merge_tree_min_read_task_size], sum_marks);

    use_uncompressed_cache = settings[Setting::use_uncompressed_cache];
    if (sum_marks > max_marks_to_use_cache) use_uncompressed_cache = false;
  }
};

}  // namespace

Pipe ReadFromMergeTree::readByLayers(const RangesInDataParts &parts_with_ranges, SplitPartsByRanges split_parts,
                                     const MergeTreeIndexBuildContextPtr &index_build_context, const Names &column_names,
  ...
  if (reader_settings.read_in_order) {
    ...
    for (const auto &column_name : storage_snapshot->metadata->getColumnsRequiredForSortingKey()) {
      if (column_names_set.contains(column_name)) continue;
    ...
    reading_step_getter = [this, &index_build_context, &in_order_column_names_to_read, &info, sorting_expr, &sort_description](auto parts) {
      auto pipe = this->read(std::move(parts), index_build_context, in_order_column_names_to_read, ReadType::InOrder, 1 /* num_streams */,
                             0 /* min_marks_for_concurrent_read */, info.use_uncompressed_cache);

      if (pipe.empty()) {
        auto header = std::make_shared<const Block>(
            MergeTreeSelectProcessor::transformHeader(storage_snapshot->getSampleBlockForColumns(in_order_column_names_to_read),
                                                      query_info.row_level_filter, query_info.prewhere_info));
        pipe = Pipe(std::make_shared<NullSource>(header));
      }

      pipe.addSimpleTransform(
          [sorting_expr](const SharedHeader &header) { return std::make_shared<ExpressionTransform>(header, sorting_expr); });

      if (pipe.numOutputPorts() != 1) {
        auto transform = std::make_shared<MergingSortedTransform>(pipe.getSharedHeader(), pipe.numOutputPorts(), sort_description,
    ...
    reading_step_getter = [this, &index_build_context, &column_names, &info](auto parts) {
      return this->read(std::move(parts), index_build_context, column_names, ReadType::Default, 1 /* num_streams */,
                        info.min_marks_for_concurrent_read, info.use_uncompressed_cache);
    };
  }

  auto pipes = ::readByLayers(std::move(split_parts), storage_snapshot->metadata->getPrimaryKey(), std::move(reading_step_getter), context);
  return Pipe::unitePipes(std::move(pipes));
}

Pipe ReadFromMergeTree::spreadMarkRangesAmongStreams(RangesInDataParts &&parts_with_ranges,
                                                     const MergeTreeIndexBuildContextPtr &index_build_context, size_t num_streams,
  ...
  if (num_streams > 1) {
    ...
    if (info.sum_marks < num_streams * info.min_marks_for_concurrent_read && parts_with_ranges.size() < num_streams) {
      ...
      min_marks_for_concurrent_read, involve the system cores to get the num_streams. Increase the num_streams and decrease the
      min_marks_for_concurrent_read if the data is small but system has plentiful cores. It helps to improve the parallel performance of
      `MergeTreeRead` significantly. Make sure the new num_streams `num_streams * increase_num_streams_ratio` will not exceed the previous
      calculated prev_num_streams. The new info.min_marks_for_concurrent_read `info.min_marks_for_concurrent_read /
      increase_num_streams_ratio` should be larger than 8. https://github.com/ClickHouse/ClickHouse/pull/53867
      */
  ...
  if (read_type != ReadType::ParallelReplicas && num_streams > 1 &&
      read_split_ranges_into_intersecting_and_non_intersecting_injection_probability > 0.0 && fault(thread_local_rng) &&
      !isQueryWithFinal() && data.merging_params.is_deleted_column.empty() && !query_info.row_level_filter && !query_info.prewhere_info &&
      ...
      !isVectorColumnReplaced())                     /// Vector search optimization needs ranges & offsets to be stable
  {
    ...
    auto in_order_reading_step_getter = [this, &index_build_context, &in_order_column_names_to_read, &info](auto parts) {
      return this->read(std::move(parts), index_build_context, in_order_column_names_to_read, ReadType::InOrder, 1 /* num_streams */,
                        0 /* min_marks_for_concurrent_read */, info.use_uncompressed_cache);
    };
    ...
    SplitPartsWithRangesByPrimaryKeyResult split_ranges_result = splitPartsWithRangesByPrimaryKey(
        storage_snapshot->metadata->getPrimaryKey(), storage_snapshot->metadata->getSortingKey(), std::move(sorting_expr),
        std::move(parts_with_ranges), num_streams, context, std::move(in_order_reading_step_getter),
        true /*split_parts_ranges_into_intersecting_and_non_intersecting_final*/, true /*split_intersecting_parts_ranges_into_layers*/);

    auto merging_pipes = std::move(split_ranges_result.merging_pipes);
    auto non_intersecting_parts_ranges_read_pipe =
        read(std::move(split_ranges_result.non_intersecting_parts_ranges), index_build_context, tmp_column_names, read_type, num_streams,
             info.min_marks_for_concurrent_read, info.use_uncompressed_cache);

    if (merging_pipes.empty()) return non_intersecting_parts_ranges_read_pipe;
    ...
    auto conversion_action =
        ActionsDAG::makeConvertingActions(pipes[0].getHeader().getColumnsWithTypeAndName(),
                                          pipes[1].getHeader().getColumnsWithTypeAndName(), ActionsDAG::MatchColumnsMode::Name, context);
    auto converting_expr = std::make_shared<ExpressionActions>(std::move(conversion_action));
    pipes[0].addSimpleTransform(
        [converting_expr](const SharedHeader &header) { return std::make_shared<ExpressionTransform>(header, converting_expr); });
    return Pipe::unitePipes(std::move(pipes));
  }

  return read(std::move(parts_with_ranges), index_build_context, tmp_column_names, read_type, num_streams,
              info.min_marks_for_concurrent_read, info.use_uncompressed_cache);
}

static ActionsDAG createProjection(const Block &header) { return ActionsDAG(header.getNamesAndTypesList()); }

Pipe ReadFromMergeTree::spreadMarkRangesAmongStreamsWithOrder(RangesInDataParts &&parts_with_ranges,
                                                              const MergeTreeIndexBuildContextPtr &index_build_context, size_t num_streams,
                                                              const Names &column_names, std::optional<ActionsDAG> &out_projection,
                                                              const InputOrderInfoPtr &input_order_info) {
  const auto &settings = context->getSettingsRef();
  ...
  auto split_ranges = [rows_granularity = (*data_settings)[MergeTreeSetting::index_granularity],
                       my_max_block_size = block_size.max_block_size_rows](const auto &ranges, int direction) {
    ...
    if (direction == 1) {
      ...
      for (auto range : ranges) {
        while (!split && range.begin + marks_in_range < range.end) {
          ...
          if (marks_in_range > max_marks_in_range) split = true;
        }
  ...
  if (num_streams > 1) {
    ...
    if (info.sum_marks < num_streams * info.min_marks_for_concurrent_read && parts_with_ranges.size() < num_streams) {
      num_streams = std::max((info.sum_marks + info.min_marks_for_concurrent_read - 1) / info.min_marks_for_concurrent_read,
  ...
  if (is_parallel_reading_from_replicas) {
    pipes.emplace_back(
        readInOrder(std::move(parts_with_ranges), index_build_context, column_names, pool_settings, read_type, input_order_info->limit));
  } else {
    ...
    for (size_t i = 0; i < num_streams && !parts_with_ranges.empty(); ++i) {
      ...
      while (need_marks > 0 && !parts_with_ranges.empty()) {
        ...
        if (marks_in_part > need_marks && marks_in_part - need_marks < info.min_marks_for_concurrent_read) need_marks = marks_in_part;
        ...
        if (take_full_part) {
          ...
          while (need_marks > 0) {
            ...
            if (range.begin == range.end) part.ranges.pop_front();
          }
          parts_with_ranges.emplace_back(part);
        }

        ranges_to_get_from_part = split_ranges(ranges_to_get_from_part, input_order_info->direction);
        new_parts.emplace_back(part.data_part, part.parent_part, part.part_index_in_query, part.part_starting_offset_in_query,
    ...
    for (auto &&item : split_parts_and_ranges)
      pipes.emplace_back(
          readInOrder(std::move(item), index_build_context, column_names, pool_settings, read_type, input_order_info->limit));
  }

  Block pipe_header;
  if (!pipes.empty()) pipe_header = pipes.front().getHeader();
  ...
  if (need_preliminary_merge || output_each_partition_through_separate_port) {
    ...
    auto syntax_result = TreeRewriter(context).analyze(
        ...
        storage_snapshot->metadata->getColumns().get(GetColumnsOptions(GetColumnsOptions::AllPhysical).withSubcolumns()));
    ...
    auto merge_streams = [&](Pipe &pipe) {
      pipe.addSimpleTransform(
          [sorting_key_expr](const SharedHeader &header) { return std::make_shared<ExpressionTransform>(header, sorting_key_expr); });
    ...
    for (auto &pipe : pipes) merge_streams(pipe);
  }
...
static void addMergingFinal(Pipe &pipe, const SortDescription &sort_description, MergeTreeData::MergingParams merging_params,
                            const StorageMetadataPtr &metadata_snapshot, size_t max_block_size_rows, bool enable_vertical_final) {
  ...
  auto get_merging_processor = [&]() -> MergingTransformPtr {
    switch (merging_params.mode) {
      case MergeTreeData::MergingParams::Ordinary:
        return std::make_shared<MergingSortedTransform>(header, num_outputs, sort_description, max_block_size_rows,
                                                        /*max_block_size_bytes=*/0, /*max_dynamic_subcolumns*/ std::nullopt,
                                                        SortingQueueStrategy::Batch);

      case MergeTreeData::MergingParams::Collapsing:
        return std::make_shared<CollapsingSortedTransform>(header, num_outputs, sort_description, merging_params.sign_column, true,
                                                           max_block_size_rows, /*max_block_size_bytes=*/0,
      ...
      case MergeTreeData::MergingParams::Summing: {
        ...
        return std::make_shared<SummingSortedTransform>(header, num_outputs, sort_description, merging_params.columns_to_sum,
                                                        required_columns, max_block_size_rows, /*max_block_size_bytes=*/0,
                                                        /*max_dynamic_subcolumns*/ std::nullopt);
      }

      case MergeTreeData::MergingParams::Aggregating:
        return std::make_shared<AggregatingSortedTransform>(header, num_outputs, sort_description, max_block_size_rows,
                                                            /*max_block_size_bytes=*/0, /*max_dynamic_subcolumns*/ std::nullopt);

      case MergeTreeData::MergingParams::Replacing:
        return std::make_shared<ReplacingSortedTransform>(
            header, num_outputs, sort_description, merging_params.is_deleted_column, merging_params.version_column, max_block_size_rows,
            /*max_block_size_bytes=*/0, /*max_dynamic_subcolumns*/ std::nullopt, /*out_row_sources_buf_*/ nullptr,
            /*use_average_block_sizes*/ false, /*cleanup*/ !merging_params.is_deleted_column.empty(), enable_vertical_final);

      case MergeTreeData::MergingParams::VersionedCollapsing:
        return std::make_shared<VersionedCollapsingTransform>(header, num_outputs, sort_description, merging_params.sign_column,
                                                              max_block_size_rows, /*max_block_size_bytes=*/0,
                                                              /*max_dynamic_subcolumns*/ std::nullopt);

      case MergeTreeData::MergingParams::Graphite:
        return std::make_shared<GraphiteRollupSortedTransform>(header, num_outputs, sort_description, max_block_size_rows,
                                                               /*max_block_size_bytes=*/0, /*max_dynamic_subcolumns*/ std::nullopt,
                                                               merging_params.graphite_params, now);

      case MergeTreeData::MergingParams::Coalescing: {
        auto required_columns = metadata_snapshot->getPartitionKey().expression->getRequiredColumns();
        required_columns.append_range(metadata_snapshot->getSortingKey().expression->getRequiredColumns());
        return std::make_shared<CoalescingSortedTransform>(header, num_outputs, sort_description, merging_params.columns_to_sum,
                                                           required_columns, max_block_size_rows, /*max_block_size_bytes=*/0,
                                                           /*max_dynamic_subcolumns*/ std::nullopt);
      }
    }
  ...
  if (enable_vertical_final)
    pipe.addSimpleTransform([](const SharedHeader &header_) { return std::make_shared<SelectByIndicesTransform>(header_); });
}

static std::pair<std::shared_ptr<ExpressionActions>, String> createExpressionForPositiveSign(const String &sign_column_name,
                                                                                             const Block &header,
...
static std::pair<std::shared_ptr<ExpressionActions>, String> createExpressionForIsDeleted(const String &is_deleted_column_name,
                                                                                          const Block &header, const ContextPtr &context) {
...
bool ReadFromMergeTree::doNotMergePartsAcrossPartitionsFinal() const {
  ...
  if (settings[Setting::do_not_merge_across_partitions_select_final]) return true;
  ...
  if (!storage_snapshot->metadata->hasPrimaryKey() || !storage_snapshot->metadata->hasPartitionKey()) return false;
  ...
  if (partition_key_expression->getActionsDAG().hasNonDeterministic()) return false;
  ...
  for (const auto &partition_key_required_column : partition_key_required_columns)
    if (!primary_key_columns_set.contains(partition_key_required_column)) return false;

  return true;
}

Pipe ReadFromMergeTree::spreadMarkRangesAmongStreamsFinal(RangesInDataParts &&parts_with_ranges,
                                                          const MergeTreeIndexBuildContextPtr &index_build_context, size_t num_streams,
                                                          const Names &origin_column_names, const Names &column_names,
                                                          std::optional<ActionsDAG> &out_projection) {
  const size_t total_marks_to_read = parts_with_ranges.getMarksCountAllParts();
  if (total_marks_to_read == 0) return {};
  ...
  if (do_not_merge_across_partitions_select_final) {
    while (it != parts_with_ranges.end()) {
      it = std::find_if(it, parts_with_ranges.end(),
                        [&it](auto &part) { return it->data_part->info.getPartitionId() != part.data_part->info.getPartitionId(); });
      parts_to_merge_ranges.push_back(it);
    }
  } else {
  ...
  if (query_info.prewhere_info || query_info.row_level_filter) {
    ...
    if (!data.merging_params.version_column.empty()) columns_to_restore.insert(data.merging_params.version_column);
    if (!data.merging_params.sign_column.empty()) columns_to_restore.insert(data.merging_params.sign_column);
    if (!data.merging_params.is_deleted_column.empty()) columns_to_restore.insert(data.merging_params.is_deleted_column);
  ...
  for (size_t range_index = 0; range_index < parts_to_merge_ranges.size() - 1; ++range_index) {
    ...
    Pipes pipes;
    {
      ...
      for (auto part_it = parts_to_merge_ranges[range_index]; part_it != parts_to_merge_ranges[range_index + 1]; ++part_it) {
        new_parts.emplace_back(part_it->data_part, part_it->parent_part, part_it->part_index_in_query,
                               part_it->part_starting_offset_in_query, part_it->ranges);
        current_ranges_marks += part_it->getMarksCount();
      }

      if (new_parts.empty()) continue;

      /// Maximal number of streams could be very small compared to the number of parts. It gets even worse when we split those parts
      /// further. To not produce too many layers, i.e., to wide pipeline, let's limit the number of streams proportionally to the total
      /// number of marks in parts.
      ...
      if (storage_snapshot->metadata->hasPrimaryKey()) {
        ...
        auto in_order_reading_step_getter = [this, &index_build_context, &column_names, &info](auto parts) {
          return this->read(std::move(parts), index_build_context, column_names, ReadType::InOrder, 1 /* num_streams */,
                            0 /* min_marks_for_concurrent_read */, info.use_uncompressed_cache);
        };
        ...
        bool split_parts_ranges_into_intersecting_and_non_intersecting_final =
            settings[Setting::split_parts_ranges_into_intersecting_and_non_intersecting_final] && !reader_settings.read_in_order;

        SplitPartsWithRangesByPrimaryKeyResult split_ranges_result = splitPartsWithRangesByPrimaryKey(
            storage_snapshot->metadata->getPrimaryKey(), storage_snapshot->metadata->getSortingKey(), sorting_expr, std::move(new_parts),
            max_layers, context, std::move(in_order_reading_step_getter), split_parts_ranges_into_intersecting_and_non_intersecting_final,
        ...
        for (auto &&merging_pipe : split_ranges_result.merging_pipes) pipes.push_back(std::move(merging_pipe));
      } else {
        pipes.emplace_back(
            read(std::move(new_parts), index_build_context, column_names, ReadType::InOrder, max_layers, 0, info.use_uncompressed_cache));
      ...
      if (!out_projection && !pipes.empty()) out_projection = createProjection(pipes.front().getHeader());
    }
    ...
    for (auto &pipe : pipes)
      addMergingFinal(pipe, sort_description, data.merging_params, storage_snapshot->metadata, block_size.max_block_size_rows,
  ...
  if (!non_intersecting_parts_by_primary_key.empty()) {
    ...
    if (data.merging_params.mode == MergeTreeData::MergingParams::Collapsing) {
      ...
      pipe = spreadMarkRangesAmongStreams(std::move(non_intersecting_parts_by_primary_key), index_build_context, num_streams,
                                          columns_with_sign);
      auto [expression, filter_name] = createExpressionForPositiveSign(data.merging_params.sign_column, pipe.getHeader(), context);

      pipe.addSimpleTransform(
          [&](const SharedHeader &header) { return std::make_shared<FilterTransform>(header, expression, filter_name, true); });
...
ReadFromMergeTree::AnalysisResultPtr ReadFromMergeTree::selectRangesToRead(bool find_exact_ranges) const {
  analyzed_result_ptr = selectRangesToRead(
      getParts(), mutations_snapshot, vector_search_parameters, top_k_filter_info, storage_snapshot->metadata, query_info, context,
      requested_num_streams, max_block_numbers_to_read, data, data_settings, all_column_names, log, indexes, find_exact_ranges,
      is_parallel_reading_from_replicas, allow_query_condition_cache, supportsSkipIndexesOnDataRead());
...
bool areAllSkipIndexColumnsInPrimaryKey(const Names &primary_key_columns, const UsefulSkipIndexes &skip_indexes) {
  ...
  for (const auto &skip_index : skip_indexes.useful_indices) {
    for (const auto &column : skip_index.index->index.column_names) {
      if (!primary_key_columns_set.contains(column)) return false;
    }
  }
...
void ReadFromMergeTree::buildIndexes(std::optional<ReadFromMergeTree::Indexes> &indexes, const ActionsDAG *filter_actions_dag_,
                                     const MergeTreeData &data, const RangesInDataParts &parts,
                                     ...
                                     const ContextPtr &query_context, const SelectQueryInfo &query_info_,
                                     const StorageMetadataPtr &metadata_snapshot, bool skip_partition_pruning_) {
  indexes.reset();
  ...
  indexes.emplace(ReadFromMergeTree::Indexes{KeyCondition{filter_dag, query_context, primary_key_column_names, primary_key.expression,
                                                          /* single_point_ = */ false,
                                                          /* skip_analysis_ = */ !settings[Setting::use_primary_key]}});

  NamesAndTypesList dummy_names_and_types;
  indexes->key_condition_rpn_template =
      KeyCondition{filter_dag, query_context, {}, std::make_shared<ExpressionActions>(ActionsDAG(dummy_names_and_types))};
  ...
  if (query_info_.isFinal() && !settings[Setting::use_skip_indexes_if_final]) indexes->use_skip_indexes = false;

  if (!indexes->use_skip_indexes) return;
  ...
  for (const auto &index : all_indexes) {
    if (ignored_index_names.contains(index.name)) continue;
...
#endif
      if (!condition) throw Exception(ErrorCodes::LOGICAL_ERROR, "Unknown vector search index {}", index_helper->index.name);
    } else {
      if (filter_dag.predicate) condition = index_helper->createIndexCondition(filter_dag.predicate, query_context);
    }

    if (condition && !condition->alwaysUnknownOrTrue()) skip_indexes.useful_indices.emplace_back(index_helper, condition);

    auto can_skip_index_be_used_for_top_k_filtering = [top_k_filter_info](const MergeTreeIndexPtr &skip_index) {
      return top_k_filter_info && skip_index->index.isSimpleSingleColumnIndex() && skip_index->index.type == "minmax" &&
    ...
    if (settings[Setting::use_skip_indexes_for_top_k] && can_skip_index_be_used_for_top_k_filtering(index_helper)) {
      ...
      LOG_TRACE(getLogger("MergeTreeSkipIndexReader"),
                "Selected index {} on column {} for top-K optimization, k = {}, direction = {}, sort columns = {}",
                index_helper->index.name, top_k_filter_info->column_name, top_k_filter_info->limit_n, top_k_filter_info->direction,
                top_k_filter_info->num_sort_columns);
      if (settings[Setting::use_skip_indexes_on_data_read]) skip_indexes.threshold_tracker = top_k_filter_info->threshold_tracker;
    }
  }

  indexes->use_skip_indexes_for_disjunctions =
      settings[Setting::use_skip_indexes_for_disjunctions] && skip_indexes.useful_indices.size() > 1 &&
  ...
  indexes->use_skip_indexes_if_final_exact_mode = indexes->use_skip_indexes && !skip_indexes.empty() && query_info_.isFinal() &&
    ...
    for (const auto &part : parts) {
      ...
      for (const auto &idx : skip_indexes.useful_indices) {
        ...
        for (const auto &substream : format.substreams) {
          ...
          if (actual_stream_name) index_size += part.data_part->getFileSizeOrZero(*actual_stream_name + substream.extension);
        }
      ...
      std::stable_sort(index_order.begin(), index_order.end(),
                       [&idx_sizes = std::as_const(index_sizes), &useful_indices = std::as_const(skip_indexes.useful_indices)](
                         ...
                         if (l_is_vectorsimilarity) l_index_priority = 0;
                         if (r_is_vectorsimilarity) r_index_priority = 0;
#endif
                         ...
                         return std::tie(l_index_priority, neg_l_granularity, l_size) <
                                std::tie(r_index_priority, neg_r_granularity, r_size);
                       });
    }
  }
...
void ReadFromMergeTree::deferFiltersAfterFinalIfNeeded() {
  ...
  if (defer_row_policy && query_info.prewhere_info) {
    ...
    bool all_in_sorting_key =
        std::all_of(required.begin(), required.end(), [&](const auto &col) { return sorting_key_columns_set.contains(col); });
    if (!all_in_sorting_key) defer_prewhere = true;
  }

  if (defer_row_policy) deferred_row_level_filter = query_info.row_level_filter;
  ...
  if (!doNotMergePartsAcrossPartitionsFinal() && storage_snapshot->metadata->hasPartitionKey()) {
    ...
    bool exprs_match = std::all_of(partition_expr_names.begin(), partition_expr_names.end(),
    ...
    bool columns_match = std::all_of(partition_required_columns.begin(), partition_required_columns.end(),
...
void ReadFromMergeTree::applyFilters(ActionDAGNodes added_filter_nodes) {
  if (!indexes) {
    ...
    if (filter_actions_dag) query_info.filter_actions_dag = filter_actions_dag;
    ...
    if (deferred_row_level_filter || deferred_prewhere_info) {
      ...
      if (deferred_row_level_filter) deferred_column_names.insert(deferred_row_level_filter->column_name);
      if (deferred_prewhere_info) deferred_column_names.insert(deferred_prewhere_info->prewhere_column_name);
      ...
      std::function<void(const ActionsDAG::Node *)> collect_sorting_key_atoms = [&](const ActionsDAG::Node *n) {
        if (isNodeOverSortingKey(n, sorting_key_set)) {
          index_nodes.push_back(n);
          return;
        }
        if (n->type == ActionsDAG::ActionType::FUNCTION && n->function_base && n->function_base->getName() == "and") {
          for (const auto *child : n->children) collect_sorting_key_atoms(child);
        }
      };
      ...
      if (idx_dag) index_filter_dag_without_deferred = std::make_shared<const ActionsDAG>(std::move(*idx_dag));
      ...
      LOG_DEBUG(log, "Excluding deferred filters from index analysis: row_policy={}, prewhere={}", deferred_row_level_filter != nullptr,
    ...
    if (query_info.prewhere_info) VirtualColumnUtils::buildSetsForDAG(query_info.prewhere_info->prewhere_actions, context);
    if (query_info.row_level_filter) VirtualColumnUtils::buildSetsForDAG(query_info.row_level_filter->actions, context);

    buildIndexes(indexes, index_filter_dag, data, getParts(), vector_search_parameters, top_k_filter_info, context, query_info,
                 storage_snapshot->metadata, skip_partition_pruning);
  }
}
...
static IndexAnalysisPartsRanges filterPartsNamesByPrimaryKeyAndSkipIndexes(
    MergeTreeDataSelectExecutor::IndexAnalysisContext &filter_context, PartsRangesMap &parts_ranges_map,
    const std::vector<std::string_view> &parts_to_analyze) {
  /// Resolve part names to RangesInDataParts
  RangesInDataParts parts_ranges_to_analyze;
  for (const auto &part : parts_to_analyze) parts_ranges_to_analyze.push_back(*parts_ranges_map.at(std::string(part)));

  ReadFromMergeTree::IndexStats ignore_stats;
  auto parts_ranges_res =
      MergeTreeDataSelectExecutor::filterPartsByPrimaryKeyAndSkipIndexes(filter_context, parts_ranges_to_analyze, ignore_stats);
  ...
  for (const auto &part_name : parts_to_analyze) {
    if (processed_parts.contains(part_name)) continue;
...
ReadFromMergeTree::AnalysisResultPtr ReadFromMergeTree::selectRangesToRead(
    const RangesInDataParts &parts, MergeTreeData::MutationsSnapshotPtr mutations_snapshot,
    const std::optional<VectorSearchParameters> &vector_search_parameters, const std::optional<TopKFilterInfo> &top_k_filter_info,
    const StorageMetadataPtr &metadata_snapshot, const SelectQueryInfo &query_info_, ContextPtr context_, size_t num_streams,
    PartitionIdToMaxBlockPtr max_block_numbers_to_read, const MergeTreeData &data, const MergeTreeSettingsPtr &data_settings_,
    const Names &all_column_names, LoggerPtr log, std::optional<Indexes> &indexes, bool find_exact_ranges,
    bool is_parallel_reading_from_replicas_, bool allow_query_condition_cache_, bool supports_skip_indexes_on_data_read) {
  ...
  if (!indexes)
    buildIndexes(indexes, query_info_.filter_actions_dag.get(), data, parts, vector_search_parameters, top_k_filter_info, context_,
                 query_info_, metadata_snapshot);
  ...
  if (indexes->part_values && indexes->part_values->empty()) return std::make_shared<AnalysisResult>(std::move(result));
  ...
  if (indexes->part_offset_condition) LOG_DEBUG(log, "Part offset condition: {}", indexes->part_offset_condition->toString());

  if (indexes->total_offset_condition) LOG_DEBUG(log, "Total offset condition: {}", indexes->total_offset_condition->toString());

  if (indexes->key_condition.alwaysFalse()) return std::make_shared<AnalysisResult>(std::move(result));

  size_t total_marks_pk = 0;
  size_t parts_before_pk = 0;
  bool add_index_stat_row_for_pk_expand = false;

  res_parts = MergeTreeDataSelectExecutor::filterPartsByPartition(parts, indexes->partition_pruner, indexes->minmax_idx_condition,
                                                                  indexes->part_values, metadata_snapshot, data, context_,
                                                                  max_block_numbers_to_read.get(), log, result.index_stats);

  result.sampling = MergeTreeDataSelectExecutor::getSampling(query_info_, metadata_snapshot->getColumns().getAllPhysical(), res_parts,
                                                             indexes->key_condition, data, metadata_snapshot, context_, log);

  if (result.sampling.read_nothing) return std::make_shared<AnalysisResult>(std::move(result));

  for (const auto &part : res_parts) total_marks_pk += part.data_part->index_granularity->getMarksCountWithoutFinal();
  ...
  bool projection_parts_exist =
      std::any_of(res_parts.begin(), res_parts.end(), [](const auto &part) { return part.data_part->isProjectionPart(); });
  ...
  if (!allow_query_condition_cache_) reader_settings.use_query_condition_cache = false;
  ...
  if (context_->canUseParallelReplicasOnFollower() && settings[Setting::parallel_replicas_local_plan] &&
    ...
    MergeTreeDataSelectExecutor::filterPartsByQueryConditionCache(res_parts, query_info_, vector_search_parameters, mutations_snapshot,
                                                                  context_, log);

    auto get_indexes_size = [&](const MergeTreeData &data_) -> size_t {
      size_t res = 0;
      for (const auto &[_, size] : data_.getSecondaryIndexSizes()) res += size.data_uncompressed;
    ...
    UInt64 distributed_index_analysis_min_parts_to_activate =
        (*data_settings_)[MergeTreeSetting::distributed_index_analysis_min_parts_to_activate];
    UInt64 distributed_index_analysis_min_indexes_bytes_to_activate =
        (*data_settings_)[MergeTreeSetting::distributed_index_analysis_min_indexes_bytes_to_activate];
    bool distributed_index_analysis_enabled =
        !final_second_pass && settings[Setting::distributed_index_analysis] &&
        ...
        (!distributed_index_analysis_min_indexes_bytes_to_activate ||
         get_indexes_size(data) >= distributed_index_analysis_min_indexes_bytes_to_activate);

    if (!distributed_index_analysis_enabled) {
      result.parts_with_ranges =
          MergeTreeDataSelectExecutor::filterPartsByPrimaryKeyAndSkipIndexes(filter_context, res_parts, result.index_stats);
      ...
      for (const auto &part_ranges : res_parts) parts_ranges_map[part_ranges.data_part->name] = &part_ranges;

      LocalIndexAnalysisCallback local_index_analysis_callback =
          [&filter_context, &parts_ranges_map](const std::vector<std::string_view> &parts_to_analyze) -> IndexAnalysisPartsRanges {
        return filterPartsNamesByPrimaryKeyAndSkipIndexes(filter_context, parts_ranges_map, parts_to_analyze);
      };

      DistributedIndexAnalysisPartsRanges distributed_index_analysis = distributedIndexAnalysisOnReplicas(
          data.getStorageID(), query_info_.filter_actions_dag.get(), result.sampling.filter_function, indexes_column_names, res_parts,
          vector_search_parameters, local_index_analysis_callback, context_);
      ...
      /// Index stats
      {
        ...
        for (auto &[replica_address, parts_on_replica] : distributed_index_analysis) {
          ...
          for (const auto &[_, marks] : parts_on_replica) replica_granules_received += marks.getNumberOfMarks();

          size_t replica_granules_send = 0;
          for (const auto &[part, _] : parts_on_replica) replica_granules_send += parts_ranges_map.at(std::string(part))->getMarksCount();
    ...
    if (reader_settings.use_query_condition_cache && query_info_.filter_actions_dag && !query_info_.isFinal()) {
      ...
      if (outputs.size() == 1 && VirtualColumnUtils::isDeterministic(outputs.front())) condition_hash = outputs.front()->getHash();
    }
    ...
    if (condition_hash) {
        ...
        if (it_result != result.parts_with_ranges.end() && it_parts->part_index_in_query == it_result->part_index_in_query) {
          ...
          while (it_full != full_ranges.end()) {
            if (it_kept == kept_ranges.end() || it_full->end <= it_kept->begin) {
              ...
              if (it_full->begin < it_kept->begin) diff_ranges.push_back({it_full->begin, it_kept->begin});
          ...
          if (!diff_ranges.empty()) {
            remaining.emplace_back(it_parts->data_part, it_parts->parent_part, it_parts->part_index_in_query,
                                   it_parts->part_starting_offset_in_query, std::move(diff_ranges));
          }
      ...
      for (const auto &remaining_ranges : remaining) {
        ...
        query_condition_cache->write(data_part->storage.getStorageID().uuid, part_name, *condition_hash, output->result_name,
                                     remaining_ranges.ranges, data_part->index_granularity->getMarksCount(),
  ...
  for (const auto &stat : result.index_stats)
    if (stat.type == IndexType::PrimaryKey) sum_marks_pk = stat.num_granules_after;
  ...
  if (query_info_.input_order_info)
    result.read_type = (query_info_.input_order_info->direction > 0) ? ReadType::InOrder : ReadType::InReverseOrder;

  return std::make_shared<AnalysisResult>(std::move(result));
}

int ReadFromMergeTree::getSortDirection() const {
  if (query_info.input_order_info) return query_info.input_order_info->direction;
...
void ReadFromMergeTree::updateSortDescription() {
  result_sort_description = getSortDescriptionForOutputHeader(
      output_header, storage_snapshot->metadata->getSortingKeyColumns(), storage_snapshot->metadata->getSortingKeyReverseFlags(),
      getSortDirection(), query_info.input_order_info, query_info.row_level_filter, query_info.prewhere_info, enable_vertical_final);
}
...
bool ReadFromMergeTree::requestReadingInOrder(size_t prefix_size, int direction, size_t read_limit) {
  ...
  if (!direction) direction = getSortDirection();

  /// Disable read-in-order optimization for reverse order with final.
  /// Otherwise, it can lead to incorrect final behavior because the implementation may rely on the reading in direct order).
  if (direction != 1 && query_info.isFinal()) return false;
  ...
  if (output_streams_limit) requested_num_streams = output_streams_limit;
  ...
  if (analyzed_result_ptr && !analyzed_result_ptr->readFromProjection()) selectRangesToRead();
...
bool ReadFromMergeTree::setVirtualRowConversions(ActionsDAG virtual_row_conversion_) {
  ...
  if (isQueryWithFinal() || !context->getSettingsRef()[Setting::read_in_order_use_virtual_row]) return false;

  virtual_row_conversion = std::make_shared<ExpressionActions>(std::move(virtual_row_conversion_));
  return true;
}

bool ReadFromMergeTree::readsInOrder() const { return reader_settings.read_in_order; }

void ReadFromMergeTree::updatePrewhereInfo(const PrewhereInfoPtr &prewhere_info_value) {
  query_info.prewhere_info = prewhere_info_value;

  output_header = std::make_shared<const Block>(MergeTreeSelectProcessor::transformHeader(
      storage_snapshot->getSampleBlockForColumns(all_column_names), query_info.row_level_filter, prewhere_info_value));

  updateSortDescription();
}

void ReadFromMergeTree::replaceVectorColumnWithDistanceColumn(const String &vector_column) {
  if (isVectorColumnReplaced()) throw Exception(ErrorCodes::LOGICAL_ERROR, "Vector column unexpectedly already replaced.");
  std::erase(all_column_names, vector_column);
  all_column_names.emplace_back("_distance");
  output_header = std::make_shared<const Block>(MergeTreeSelectProcessor::transformHeader(
      storage_snapshot->getSampleBlockForColumns(all_column_names), query_info.row_level_filter, query_info.prewhere_info));

  /// if analysis has already been done (like in optimization for projections),
  /// then update columns to read in analysis result
  if (analyzed_result_ptr) analyzed_result_ptr->column_names_to_read = all_column_names;
}
...
bool ReadFromMergeTree::requestOutputEachPartitionThroughSeparatePort() {
  if (isQueryWithFinal()) return false;
  ...
  if (is_parallel_reading_from_replicas) return false;
  ...
  if (!settings[Setting::force_aggregate_partitions_independently] &&
    ...
    LOG_TRACE(log,
              ...
              "force_aggregate_partitions_independently to suppress this check",
              partitions_cnt, settings[Setting::max_number_of_partitions_for_independent_aggregation].value);
    return false;
  }

  if (!settings[Setting::force_aggregate_partitions_independently]) {
    std::unordered_map<String, size_t> partition_rows;
    for (const auto &part : getParts()) partition_rows[part.data_part->info.getPartitionId()] += part.data_part->rows_count;
    size_t sum_rows = 0;
    size_t max_rows = 0;
    ...
    /// exec time of ordinary aggr is proportional to sum of sizes divided by number of threads and multiplied by two (preaggregation +
    /// merging).
...
ReadFromMergeTree::AnalysisResult &ReadFromMergeTree::getAnalysisResultImpl() const {
  if (!analyzed_result_ptr) analyzed_result_ptr = selectRangesToRead();

  return *analyzed_result_ptr;
}

bool ReadFromMergeTree::isQueryWithSampling() const {
  if (context->getSettingsRef()[Setting::parallel_replicas_count] > 1 && data.supportsSampling()) return true;

  if (query_info.table_expression_modifiers) return query_info.table_expression_modifiers->getSampleSizeRatio() != std::nullopt;

  const auto &select = query_info.query->as<ASTSelectQuery &>();
  return select.sampleSize() != nullptr;
}

Pipe ReadFromMergeTree::spreadMarkRanges(RangesInDataParts &&parts_with_ranges, const MergeTreeIndexBuildContextPtr &index_build_context,
                                         size_t num_streams, AnalysisResult &result, std::optional<ActionsDAG> &result_projection) {
  ...
  if (result.sampling.use_sampling) {
    ...
    for (const auto &column : result.sampling.filter_expression->getRequiredColumns().getNames()) {
      if (names.emplace(column).second) column_names_to_read.push_back(column);
  ...
  if (final) {
    ...
    return spreadMarkRangesAmongStreamsFinal(std::move(parts_with_ranges), index_build_context, num_streams, original_column_names,
                                             column_names_to_read, result_projection);
  }

  if (!result.split_parts.layers.empty())
    return readByLayers(result.parts_with_ranges, std::move(result.split_parts), index_build_context, column_names_to_read,
                        query_info.input_order_info);

  if (query_info.input_order_info) {
    return spreadMarkRangesAmongStreamsWithOrder(std::move(parts_with_ranges), index_build_context, num_streams, column_names_to_read,
                                                 result_projection, query_info.input_order_info);
  }

  return spreadMarkRangesAmongStreams(std::move(parts_with_ranges), index_build_context, num_streams, column_names_to_read);
}

Pipe ReadFromMergeTree::groupStreamsByPartition(AnalysisResult &result, const MergeTreeIndexBuildContextPtr &index_build_context,
  ...
  for (auto begin = parts_with_ranges.begin(), end = begin; end != parts_with_ranges.end(); begin = end) {
    for (size_t i = 0; i < partitions_per_stream; ++i)
      end = std::find_if(end, parts_with_ranges.end(), [&end](const auto &part) {
        return end->data_part->info.getPartitionId() != part.data_part->info.getPartitionId();
      });

    RangesInDataParts partition_parts{std::make_move_iterator(begin), std::make_move_iterator(end)};

    pipes.emplace_back(spreadMarkRanges(std::move(partition_parts), index_build_context, num_streams, result, result_projection));
    if (!pipes.back().empty()) pipes.back().resize(1);
  }
...
QueryPlanStepPtr ReadFromMergeTree::clone() const {
  ...
  if (analyzed_result_ptr) analysis_result_copy = std::make_shared<AnalysisResult>(*analyzed_result_ptr);

  auto cloned_step = std::make_unique<ReadFromMergeTree>(
      prepared_parts, mutations_snapshot, all_column_names, data, data_settings, query_info, storage_snapshot, context,
      block_size.max_block_size_rows, requested_num_streams, max_block_numbers_to_read, log, std::move(analysis_result_copy),
      is_parallel_reading_from_replicas, all_ranges_callback, read_task_callback, number_of_current_replica);
...
std::unique_ptr<LazilyReadFromMergeTree> ReadFromMergeTree::keepOnlyRequiredColumnsAndCreateLazyReadStep(const NameSet &required_outputs) {
  if (output_header == nullptr) return {};

  NameSet columns_to_keep;

  for (const auto &column_name : required_outputs) columns_to_keep.insert(column_name);

  if (query_info.row_level_filter) {
    for (const auto *input : query_info.row_level_filter->actions.getInputs()) columns_to_keep.insert(input->result_name);
  }

  if (query_info.prewhere_info) {
    for (const auto *input : query_info.prewhere_info->prewhere_actions.getInputs()) columns_to_keep.insert(input->result_name);
  }
  ...
  auto lazy_reading_header =
      std::make_shared<const Block>(MergeTreeSelectProcessor::transformHeader(storage_snapshot->getSampleBlockForColumns(columns_to_remove),
  ...
  auto new_reading = std::make_unique<LazilyReadFromMergeTree>(std::move(lazy_reading_header), block_size.max_block_size_rows,
                                                               info.min_marks_for_concurrent_read, reader_settings, mutations_snapshot,
                                                               storage_snapshot, context, data.getLogName());
...
void ReadFromMergeTree::addStartingPartOffsetAndPartOffset(bool &added_part_starting_offset, bool &added_part_offset) {
  ...
  for (const auto &col_name : all_column_names) {
    if (col_name == "_part_starting_offset") added_part_starting_offset = false;
    if (col_name == "_part_offset") added_part_offset = false;
  }

  if (!added_part_starting_offset && !added_part_offset) return;

  Names new_column_names;
  if (added_part_starting_offset) new_column_names.push_back("_part_starting_offset");
  if (added_part_offset) new_column_names.push_back("_part_offset");
...
bool ReadFromMergeTree::supportsSkipIndexesOnDataRead() const {
  if (!indexes || !indexes->use_skip_indexes || indexes->skip_indexes.empty()) return false;

  const auto &settings = context->getSettingsRef();
  if (!settings[Setting::use_skip_indexes_on_data_read]) return false;
  ...
  if (query_info.query_tree) {
    ...
    if (join_tree_node &&
        (join_tree_node->getNodeType() == QueryTreeNodeType::JOIN || join_tree_node->getNodeType() == QueryTreeNodeType::CROSS_JOIN))
      return false;
  }

  if (query_info.isFinal() && settings[Setting::use_skip_indexes_if_final_exact_mode]) return false;
  ...
  if (settings[Setting::read_overflow_mode] == OverflowMode::THROW && settings[Setting::max_rows_to_read]) return false;

  if (mutations_snapshot->hasDataMutations() || mutations_snapshot->hasPatchParts()) return false;
...
void ReadFromMergeTree::initializePipeline(QueryPipelineBuilder &pipeline, const BuildQueryPipelineSettings &) {
  ...
  if (deferred_row_level_filter || deferred_prewhere_info) {
    if (deferred_row_level_filter) query_info.row_level_filter = nullptr;
    if (deferred_prewhere_info) query_info.prewhere_info = nullptr;
    ...
    if (deferred_row_level_filter) add_required_columns(deferred_row_level_filter->actions.getRequiredColumnsNames());

    if (deferred_prewhere_info) add_required_columns(deferred_prewhere_info->prewhere_actions.getRequiredColumnsNames());

    /// Recreate output_header without the deferred filters since they will be applied after FINAL
    output_header = std::make_shared<const Block>(MergeTreeSelectProcessor::transformHeader(
        storage_snapshot->getSampleBlockForColumns(all_column_names), query_info.row_level_filter, query_info.prewhere_info));

    LOG_DEBUG(log, "Deferring filters to after FINAL: row_policy={}, prewhere={}. columns_to_read={}", deferred_row_level_filter != nullptr,
              deferred_prewhere_info != nullptr, fmt::join(result.column_names_to_read, ","));
  }
  ...
  LOG_DEBUG(log,
            ...
            result.parts_before_pk, result.total_parts, result.selected_parts, result.selected_marks_pk, result.total_marks_pk,
            result.selected_marks, result.selected_ranges);

  // Adding partition info to QueryAccessInfo.
  if (context->hasQueryContext() && !query_info.is_internal) {
    Names partition_names;
    for (const auto &part : result.parts_with_ranges) {
      partition_names.emplace_back(fmt::format("{}.{}", data.getStorageID().getFullNameNotQuoted(), part.data_part->info.getPartitionId()));
    }
  ...
  if (!allow_query_condition_cache || !has_where_or_prewhere) reader_settings.use_query_condition_cache = false;
  ...
  if (result.parts_with_ranges.empty() && isParallelReplicasLocalPlanForInitiator()) {
    ...
    auto get_coordination_mode = [&] {
      if (!query_info.input_order_info) return CoordinationMode::Default;

      if (!query_info.input_order_info->direction) return CoordinationMode::Default;

      return query_info.input_order_info->direction > 0 ? CoordinationMode::WithOrder : CoordinationMode::ReverseOrder;
    };
  ...
  if (supportsSkipIndexesOnDataRead()) {
    ...
    if (!applicable_skip_indexes.empty()) {
      skip_index_reader = std::make_shared<MergeTreeSkipIndexReader>(
          applicable_skip_indexes, indexes->key_condition_rpn_template, indexes->use_skip_indexes_for_disjunctions,
          context->getIndexMarkCache(), context->getIndexUncompressedCache(), context->getVectorSimilarityIndexCache(), reader_settings,
      ...
      readers.emplace(
          ...
          SingleProjectionIndexReader(
              std::make_shared<MergeTreeReadPoolProjectionIndex>(
                  empty_mutations_snapshot, std::make_shared<StorageSnapshot>(storage_snapshot->storage, read_info.projection->metadata),
                  read_info.prewhere_info, actions_settings, reader_settings,
                  read_info.prewhere_info->prewhere_actions.getRequiredColumnsNames(), pool_settings, block_size, context),
              read_info.prewhere_info, actions_settings, reader_settings));
    }
  ...
  if (skip_index_reader || projection_index_reader) {
    ...
    index_build_context =
        std::make_shared<MergeTreeIndexBuildContext>(std::move(read_ranges), std::move(projection_index_read_desc.read_ranges),
                                                     std::move(index_read_result_pool), std::move(part_remaining_marks));
  }

  Pipe pipe = output_each_partition_through_separate_port ? groupStreamsByPartition(result, index_build_context, result_projection)
                                                          : spreadMarkRanges(std::move(result.parts_with_ranges), index_build_context,
                                                                             requested_num_streams, result, result_projection);

  for (const auto &processor : pipe.getProcessors()) processor->setStorageLimits(query_info.storage_limits);
  ...
  auto add_deferred_filter = [&pipe](ActionsDAG filter_dag, const String &column_name, bool remove_column) {
    NameSet input_names;
    for (const auto *input : filter_dag.getInputs()) input_names.insert(input->result_name);
  ...
  if (deferred_row_level_filter)
    add_deferred_filter(deferred_row_level_filter->actions.clone(), deferred_row_level_filter->column_name,
                        deferred_row_level_filter->do_remove_column);

  /// apply deferred PREWHERE after row policy
  if (deferred_prewhere_info)
    add_deferred_filter(deferred_prewhere_info->prewhere_actions.clone(), deferred_prewhere_info->prewhere_column_name,
  ...
  if (result_projection) cur_header = result_projection->updateHeader(cur_header);
  ...
  if (!isCompatibleHeader(cur_header, *getOutputHeader())) {
    auto converting =
        ActionsDAG::makeConvertingActions(cur_header.getColumnsWithTypeAndName(), getOutputHeader()->getColumnsWithTypeAndName(),
                                          ActionsDAG::MatchColumnsMode::Name, context);
  ...
  if (result_projection) {
    ...
    pipe.addSimpleTransform([&](const SharedHeader &header) { return std::make_shared<ExpressionTransform>(header, projection_actions); });
  }
  ...
  if (!blocksHaveEqualStructure(pipe.getHeader(), *getOutputHeader())) {
    auto convert_actions_dag =
        ActionsDAG::makeConvertingActions(pipe.getHeader().getColumnsWithTypeAndName(), getOutputHeader()->getColumnsWithTypeAndName(),
                                          ActionsDAG::MatchColumnsMode::Name, context, true);

    auto converting_dag_expr = std::make_shared<ExpressionActions>(std::move(convert_actions_dag));

    pipe.addSimpleTransform([&](const SharedHeader &header) { return std::make_shared<ExpressionTransform>(header, converting_dag_expr); });
  }

  for (const auto &processor : pipe.getProcessors()) processors.emplace_back(processor);

  pipeline.init(std::move(pipe));
  pipeline.addContext(context);
  // Attach QueryIdHolder if needed
  if (query_id_holder) pipeline.setQueryIdHolder(std::move(query_id_holder));
}
...
void ReadFromMergeTree::describeActions(FormatSettings &format_settings) const {
  ...
  if (query_info.prewhere_info) {
    ...
    if (query_info.prewhere_info->remove_prewhere_column) format_settings.out << " (removed)";
    format_settings.out << '\n';
  ...
  if (query_info.row_level_filter) {
    ...
    if (query_info.row_level_filter->do_remove_column) format_settings.out << " (removed)";
    format_settings.out << '\n';
...
void ReadFromMergeTree::describeActions(JSONBuilder::JSONMap &map) const {
  ...
  if (query_info.prewhere_info || query_info.row_level_filter) {
    ...
    if (query_info.prewhere_info) prewhere_info_map->add("Need filter", query_info.prewhere_info->need_filter);
  }
  ...
  if (prewhere_info_map) map.add("Prewhere info", std::move(prewhere_info_map));

  if (deferred_prewhere_info || deferred_row_level_filter) {
    auto deferred_map = std::make_unique<JSONBuilder::JSONMap>();
    if (deferred_row_level_filter) deferred_map->add("Deferred row level filter column", deferred_row_level_filter->column_name);
    if (deferred_prewhere_info) deferred_map->add("Deferred prewhere filter column", deferred_prewhere_info->prewhere_column_name);
...
void ReadFromMergeTree::describeIndexes(FormatSettings &format_settings) const {
  ...
  if (!index_stats.empty()) {
    ...
    if (index_stats.size() == 1 && index_stats.front().type == IndexType::None) return;
    ...
    for (size_t i = 0; i < index_stats.size(); ++i) {
      ...
      if (stat.type == IndexType::None) continue;
      ...
      if (i) format_settings.out << '/' << index_stats[i - 1].num_parts_after;
      format_settings.out << '\n';

      format_settings.out << prefix << indent << indent << "Granules: " << stat.num_granules_after;
      if (i) format_settings.out << '/' << index_stats[i - 1].num_granules_after;
      format_settings.out << '\n';

      auto search_algorithm = searchAlgorithmToString(stat.search_algorithm);
      if (!search_algorithm.empty()) format_settings.out << prefix << indent << indent << "Search Algorithm: " << search_algorithm << "\n";
...
void ReadFromMergeTree::describeIndexes(JSONBuilder::JSONMap &map) const {
  ...
  if (!index_stats.empty()) {
    ...
    for (size_t i = 0; i < index_stats.size(); ++i) {
      ...
      if (!stat.name.empty()) index_map->add("Name", stat.name);

      if (!stat.description.empty()) index_map->add("Description", stat.description);

      if (!stat.used_keys.empty()) {
        auto keys_array = std::make_unique<JSONBuilder::JSONArray>();

        for (const auto &used_key : stat.used_keys) keys_array->add(used_key);

        index_map->add("Keys", std::move(keys_array));
      }

      if (!stat.condition.empty()) index_map->add("Condition", stat.condition);

      auto search_algorithm = searchAlgorithmToString(stat.search_algorithm);
      if (!search_algorithm.empty()) index_map->add("Search Algorithm", search_algorithm);

      if (i) index_map->add("Initial Parts", index_stats[i - 1].num_parts_after);
      index_map->add("Selected Parts", stat.num_parts_after);

      if (i) index_map->add("Initial Granules", index_stats[i - 1].num_granules_after);
...
void ReadFromMergeTree::describeProjections(JSONBuilder::JSONMap &map) const {
  ...
  if (!projection_stats.empty()) {
    ...
    for (const auto &stat : projection_stats) {
      ...
      if (!stat.description.empty()) projection_map->add("Description", stat.description);

      if (!stat.condition.empty()) projection_map->add("Condition", stat.condition);

      auto search_algorithm = searchAlgorithmToString(stat.search_algorithm);
      if (!search_algorithm.empty()) projection_map->add("Search Algorithm", search_algorithm);
...
void ReadFromMergeTree::clearParallelReadingExtension() {
  if (!is_parallel_reading_from_replicas) return;
...
std::shared_ptr<ParallelReadingExtension> ReadFromMergeTree::getParallelReadingExtension() {
  if (!is_parallel_reading_from_replicas) return nullptr;

  chassert(all_ranges_callback.has_value() && read_task_callback.has_value());
  const auto &client_info = context->getClientInfo();
  return std::make_shared<ParallelReadingExtension>(all_ranges_callback.value(), read_task_callback.value(),
                                                    number_of_current_replica.value_or(client_info.number_of_current_replica),
                                                    context->getClusterForParallelReplicas()->getShardsInfo().at(0).getAllNodeCount());
}

void ReadFromMergeTree::createReadTasksForTextIndex(const UsefulSkipIndexes &skip_indexes, const IndexReadColumns &added_columns,
                                                    const Names &removed_columns, bool is_final) {
  index_read_tasks.clear();

  if (added_columns.empty()) return;
  ...
  storage_snapshot =
      std::make_shared<StorageSnapshot>(storage_snapshot->storage, storage_snapshot->metadata, std::move(new_virtual_columns));
...
bool ReadFromMergeTree::isSkipIndexAvailableForTopK(const String &sort_column) const {
  ...
  if (all_indexes.empty()) return false;

  for (const auto &index : all_indexes) {
    if (index.isSimpleSingleColumnIndex() && index.type == "minmax" && index.column_names[0] == sort_column) return true;
  }
  return false;
}
...
ConditionSelectivityEstimatorPtr ReadFromMergeTree::getConditionSelectivityEstimator(const Names &required_columns) const {
  ...
  if (!getStorageMetadata()->hasStatistics()) return nullptr;
...
bool ReadFromMergeTree::canRemoveUnusedColumns() const {
  ...
  if (data.merging_params.mode == MergeTreeData::MergingParams::Graphite) return false;

  if (query_info.row_level_filter && hasDuplicatedNamesInInputOrOutputs(query_info.row_level_filter->actions)) return false;

  if (query_info.prewhere_info && hasDuplicatedNamesInInputOrOutputs(query_info.prewhere_info->prewhere_actions)) return false;
  ...
  if (query_info.isFinal()) {
    ...
    if (!has_column_that_is_not_required_for_final) return false;
  }
  return true;
}
...
IQueryPlanStep::RemovedUnusedColumns ReadFromMergeTree::removeUnusedColumns(NameMultiSet required_outputs, bool /*remove_inputs*/) {
  if (output_header == nullptr) return RemovedUnusedColumns::None;

  NameSet columns_to_keep;

  if (query_info.isFinal()) columns_to_keep = getColumnsRequiredForMergingFinal(result_sort_description, data.merging_params);
  ...
  if (query_info.prewhere_info) {
    ...
    removed_output_from_prewhere =
        std::erase_if(prewhere_outputs, [&](const auto *output) {
          return output->result_name != query_info.prewhere_info->prewhere_column_name && !columns_to_keep.contains(output->result_name);
        }) > 0;
  ...
  if (query_info.row_level_filter) {
    ...
    removed_output_from_row_level_filter =
        std::erase_if(row_level_filter_outputs, [&](const auto *output) {
          return output->result_name != query_info.row_level_filter->column_name && !columns_to_keep.contains(output->result_name);
        }) > 0;
  ...
  Names new_column_names;
  for (const auto &column_name : all_column_names) {
    if (columns_to_keep.contains(column_name)) new_column_names.push_back(column_name);
  }
...
bool ReadFromMergeTree::canRemoveColumnsFromOutput() const {
  if (output_header == nullptr) return false;
