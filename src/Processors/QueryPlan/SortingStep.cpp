...
size_t getMaxBytesInQueryBeforeExternalSort(double max_bytes_ratio_before_external_sort) {
  if (max_bytes_ratio_before_external_sort == 0.) return 0;
...
SortingStep::Settings::Settings(const DB::Settings& settings) {
  ...
  size_limits =
      SizeLimits(settings[Setting::max_rows_to_sort], settings[Setting::max_bytes_to_sort], settings[Setting::sort_overflow_mode]);
...
SortingStep::Settings::Settings(size_t max_block_size_) { max_block_size = max_block_size_; }
...
SortingStep::Settings::Settings(const QueryPlanSerializationSettings& settings) {
  ...
  size_limits =
      SizeLimits(settings[QueryPlanSerializationSetting::max_rows_to_sort], settings[QueryPlanSerializationSetting::max_bytes_to_sort],
                 settings[QueryPlanSerializationSetting::sort_overflow_mode]);
  ...
  max_bytes_in_query_before_external_sort =
      getMaxBytesInQueryBeforeExternalSort(settings[QueryPlanSerializationSetting::max_bytes_ratio_before_external_sort]);
...
SortingStep::SortingStep(const SharedHeader& input_header, SortDescription description_, UInt64 limit_, const Settings& settings_,
                         bool is_sorting_for_merge_join_)
...
SortingStep::SortingStep(const SharedHeader& input_header, const SortDescription& description_,
                         const SortDescription& partition_by_description_, UInt64 limit_, const Settings& settings_)
    : SortingStep(input_header, description_, limit_, settings_) {
  partition_by_description = partition_by_description_;
}

SortingStep::SortingStep(const SharedHeader& input_header, SortDescription prefix_description_, SortDescription result_description_,
...
SortingStep::SortingStep(const SharedHeader& input_header, SortDescription sort_description_, size_t max_block_size_, UInt64 limit_,
...
void SortingStep::updateOutputHeader() { output_header = input_headers.front(); }
  ...
  if (!partition_by_description.empty() && threads > 1) {
    ...
    if (threads * streams > connection_count_limit)
      throw Exception(ErrorCodes::LIMIT_EXCEEDED,
                      "Parallelism limit exceeded in SortingStep: {} threads X {} streams, limit {}, try to reduce `max_threads` value",
...
void SortingStep::finishSorting(QueryPipelineBuilder& pipeline, const SortDescription& input_sort_desc,
                                const SortDescription& result_sort_desc, const UInt64 limit_) {
  pipeline.addSimpleTransform([&](const SharedHeader& header, QueryPipelineBuilder::StreamType stream_type) -> ProcessorPtr {
    if (stream_type != QueryPipelineBuilder::StreamType::Main) return nullptr;
  ...
  pipeline.addSimpleTransform([&, increase_sort_description_compile_attempts](const SharedHeader& header) mutable -> ProcessorPtr {
    ...
    if (increase_sort_description_compile_attempts) increase_sort_description_compile_attempts = false;

    return std::make_shared<FinishSortingTransform>(header, input_sort_desc, result_sort_desc, sort_settings.max_block_size, limit_,
...
void SortingStep::mergingSorted(QueryPipelineBuilder& pipeline, const SortDescription& result_sort_desc, const UInt64 limit_) {
  ...
  if (pipeline.getNumStreams() > 1) {
    ...
    auto transform = std::make_shared<MergingSortedTransform>(
        pipeline.getSharedHeader(), pipeline.getNumStreams(), result_sort_desc, sort_settings.max_block_size,
        /*max_block_size_bytes=*/0,
        /*max_dynamic_subcolumns*/ std::nullopt, SortingQueueStrategy::Batch, limit_, always_read_till_end,
        /*out_row_sources_buf=*/nullptr,
        /*filter_column_name=*/std::nullopt,
        /*use_average_block_sizes=*/false, apply_virtual_row_conversions);

    pipeline.addTransform(std::move(transform));
  }
}

void SortingStep::mergeSorting(QueryPipelineBuilder& pipeline, const Settings& sort_settings, const SortDescription& result_sort_desc,
  ...
  if (auto data = Context::getGlobalContextInstance()->getSharedTempDataOnDisk())
    tmp_data_on_disk = data->childScope({.current_metric = CurrentMetrics::TemporaryFilesForSort,
  ...
  pipeline.addSimpleTransform([&, increase_sort_description_compile_attempts](
                                  const SharedHeader& header, QueryPipelineBuilder::StreamType stream_type) mutable -> ProcessorPtr {
    if (stream_type == QueryPipelineBuilder::StreamType::Totals) return nullptr;
    ...
    return std::make_shared<MergeSortingTransform>(
        header, result_sort_desc, sort_settings.max_block_size, sort_settings.max_block_bytes, limit_,
        increase_sort_description_compile_attempts_current, sort_settings.max_bytes_before_remerge / pipeline.getNumStreams(),
        sort_settings.remerge_lowered_memory_bytes_ratio, sort_settings.max_bytes_in_block_before_external_sort / pipeline.getNumStreams(),
        sort_settings.max_bytes_in_query_before_external_sort, tmp_data_on_disk, sort_settings.min_free_disk_space, threshold_tracker);
  });
}

void SortingStep::fullSortStreams(QueryPipelineBuilder& pipeline, const Settings& sort_settings, const SortDescription& result_sort_desc,
                                  const UInt64 limit_, const bool skip_partial_sort, TopKThresholdTrackerPtr threshold_tracker) {
...
void SortingStep::transformPipeline(QueryPipelineBuilder& pipeline, const BuildQueryPipelineSettings&) {
  ...
  if (type == Type::MergingSorted) {
    ...
    if (dataflow_cache_updater)
      pipeline.addSimpleTransform(
          [&](const SharedHeader& header) { return std::make_shared<RuntimeDataflowStatisticsCollector>(header, dataflow_cache_updater); });
    return;
  }
  ...
  if (type == Type::FinishSorting) {
    ...
    if (need_finish_sorting) finishSorting(pipeline, prefix_description, result_description, limit);
...
void SortingStep::describeActions(JSONBuilder::JSONMap& map) const {
  ...
  if (limit) map.add("Limit", limit);
}

void SortingStep::serializeSettings(QueryPlanSerializationSettings& settings) const { sort_settings.updatePlanSettings(settings); }

void SortingStep::serialize(Serialization& ctx) const {
  if (type != Type::Full) throw Exception(ErrorCodes::NOT_IMPLEMENTED, "Serialization of SortingStep is implemented only for Full sorting");
...
QueryPlanStepPtr SortingStep::deserialize(Deserialization& ctx) {
  if (ctx.input_headers.size() != 1) throw Exception(ErrorCodes::INCORRECT_DATA, "SortingStep must have one input stream");
  ...
  return std::make_unique<SortingStep>(ctx.input_headers.front(), std::move(result_description), 0, std::move(sort_settings));
}

void registerSortingStep(QueryPlanStepRegistry& registry) { registry.registerStep("Sorting", SortingStep::deserialize); }
