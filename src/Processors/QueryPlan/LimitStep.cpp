...
LimitStep::LimitStep(const SharedHeader &input_header_, size_t limit_, size_t offset_, bool always_read_till_end_, bool with_ties_,
...
void LimitStep::transformPipeline(QueryPipelineBuilder &pipeline, const BuildQueryPipelineSettings &) {
  auto transform = std::make_shared<LimitTransform>(pipeline.getSharedHeader(), limit, offset, pipeline.getNumStreams(),
                                                    always_read_till_end, with_ties, description, dataflow_cache_updater);
...
void LimitStep::describeActions(FormatSettings &settings) const {
  ...
  if (with_ties || always_read_till_end) {
    ...
    if (with_ties) settings.out << "WITH TIES";

    if (always_read_till_end) {
      if (!with_ties) settings.out << ", ";
...
void LimitStep::serialize(Serialization &ctx) const {
  UInt8 flags = 0;
  if (always_read_till_end) flags |= 1;
  if (with_ties) flags |= 2;
...
void registerLimitStep(QueryPlanStepRegistry &registry) { registry.registerStep("Limit", LimitStep::deserialize); }
