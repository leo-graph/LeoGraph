...
void ArrayJoinStep::describeActions(JSONBuilder::JSONMap &map) const {
  ...
  for (const auto &column : array_join.columns) columns_array->add(column);
...
void ArrayJoinStep::serialize(Serialization &ctx) const {
  UInt8 flags = 0;
  if (array_join.is_left) flags |= 1;
  if (is_unaligned) flags |= 2;

  writeIntBinary(flags, ctx.out);

  writeVarUInt(array_join.columns.size(), ctx.out);
  for (const auto &column : array_join.columns) writeStringBinary(column, ctx.out);
}
...
QueryPlanStepPtr ArrayJoinStep::deserialize(Deserialization &ctx) {
  ...
  for (auto &column : array_join.columns) readStringBinary(column, ctx.in);

  return std::make_unique<ArrayJoinStep>(ctx.input_headers.front(), std::move(array_join), is_unaligned,
...
void registerArrayJoinStep(QueryPlanStepRegistry &registry) { registry.registerStep("ArrayJoin", ArrayJoinStep::deserialize); }
