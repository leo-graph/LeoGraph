...
static void addConvertingActions(Pipe &pipe, const Block &header, const ContextPtr &context, bool use_positions_to_match = false) {
  if (blocksHaveEqualStructure(pipe.getHeader(), header)) return;
...
static void enableMemoryBoundMerging(QueryProcessingStage::Enum stage, const ClusterProxy::SelectStreamFactory::Shards *shards,
                                     Context &context) {
  if (stage != QueryProcessingStage::WithMergeableState)
    throw Exception(ErrorCodes::LOGICAL_ERROR, "Cannot enforce sorting for ReadFromRemote step up to stage {}",
...
static void enforceAggregationInOrder(QueryProcessingStage::Enum stage, const ClusterProxy::SelectStreamFactory::Shards *shards,
                                      const SortDescription &sort_description, Context &context) {
  if (stage != QueryProcessingStage::WithMergeableState)
    throw Exception(ErrorCodes::LOGICAL_ERROR, "Cannot enforce aggregation in order for ReadFromRemote step up to stage {}",
                    QueryProcessingStage::toString(stage));
  ...
  for (const auto &shard : *shards) {
    ...
    auto sorting = std::make_unique<SortingStep>(shard.query_plan->getCurrentHeader(), sort_description, 0,
...
static String formattedAST(const ASTPtr &ast, bool enable_analyzer) {
  ...
  if (!enable_analyzer) ast_format_settings.collapse_identical_nodes_to_aliases = true;
  ast->format(buf, ast_format_settings);
  return buf.str();
}

ReadFromRemote::ReadFromRemote(ClusterProxy::SelectStreamFactory::Shards shards_, SharedHeader header_, QueryProcessingStage::Enum stage_,
                               StorageID main_table_, ASTPtr table_func_ptr_, ContextMutablePtr context_, ThrottlerPtr throttler_,
                               Scalars scalars_, Tables external_tables_, LoggerPtr log_, UInt32 shard_count_,
                               std::shared_ptr<const StorageLimitsList> storage_limits_, const String &cluster_name_)
...
void ReadFromRemote::enableMemoryBoundMerging() { DB::enableMemoryBoundMerging(stage, &shards, *context); }
...
ASTSelectQuery &getSelectQuery(ASTPtr ast) {
  if (const auto *explain = ast->as<ASTExplainQuery>()) ast = explain->getExplainedQuery();
...
ASTPtr tryBuildAdditionalFilterAST(const ActionsDAG &dag, const std::unordered_set<std::string> &projection_names,
                                   const std::unordered_map<std::string, QueryTreeNodePtr> &execution_name_to_projection_query_tree,
                                   Tables *external_tables, ContextMutablePtr &context) {
  ...
  while (!stack.empty()) {
    ...
    if (WhichDataType(node->result_type).isSet()) {
      ...
      if (const auto *col_const = typeid_cast<const ColumnConst *>(maybe_set.get())) maybe_set = col_const->getDataColumnPtr();

      if (const auto *col_set = typeid_cast<const ColumnSet *>(maybe_set.get())) node_to_ast[node] = col_set->getData()->getSourceAST();
    ...
    if (node->type == ActionsDAG::ActionType::ALIAS) res = node_to_ast[node->children.at(0)];

    if (node->type != ActionsDAG::ActionType::FUNCTION) continue;

    if (!node->function_base->isDeterministic() || node->function_base->isStateful()) continue;
    ...
    if (!has_all_args && !is_function_and) continue;

    if (is_function_and && arguments.empty()) continue;

    /// and() with 1 arg is not allowed. Make it AND(condition, 1)
    if (is_function_and && arguments.size() == 1) arguments.push_back(make_intrusive<ASTLiteral>(Field(1)));
    ...
    if (external_tables && isNameOfGlobalInFunction(func_name)) {
      ...
      if (const auto *col_set = typeid_cast<const ColumnSet *>(maybe_set.get())) {
        ...
        if (auto *set_from_subquery = typeid_cast<FutureSetFromSubquery *>(future_set.get());
          ...
          if (auto *table_identifier = source_ast->as<ASTTableIdentifier>()) temporary_table_name = table_identifier->name();
          ...
          if (!external_table) {
            ...
            auto external_storage_holder = TemporaryTableHolder(context, ColumnsDescription{columns}, ConstraintsDescription{},
...
static void addFilters(Tables *external_tables, ContextMutablePtr &context, const ASTPtr &query_ast, const QueryTreeNodePtr &query_tree,
                       const PlannerContextPtr &planner_context, const ActionsDAG &pushed_down_filters) {
  if (!query_tree || !planner_context) return;

  const auto &settings = context->getSettingsRef();
  if (!settings[Setting::allow_push_predicate_ast_for_distributed_subqueries]) return;

  const auto *query_node = query_tree->as<QueryNode>();
  if (!query_node) return;
  ...
  for (const auto &col : query_node->getProjectionColumns()) projection_names.insert(col.name);
  ...
  ASTPtr predicate =
      tryBuildAdditionalFilterAST(pushed_down_filters, projection_names, execution_name_to_projection_query_tree, external_tables, context);
  if (!predicate) return;

  auto table_expressions = extractTableExpressions(query_node->getJoinTree());
  /// Case with JOIN is not supported so far.
  if (table_expressions.size() != 1) return;
  ...
  TableWithColumnNamesAndTypes table_with_columns(DatabaseAndTableWithAlias(table_node->toASTIdentifier()),
...
void ReadFromRemote::addLazyPipe(Pipes &pipes, const ClusterProxy::SelectStreamFactory::Shard &shard, const SharedHeader &out_header,
  ...
  if (stage == QueryProcessingStage::Complete) {
    if (const auto *ast_select = shard.query->as<ASTSelectQuery>()) add_totals = ast_select->group_by_with_totals;
  ...
  auto lazily_create_stream = [my_shard = shard, my_shard_count = shard_count, query = shard.query, header = shard.header,
                               my_context = context, my_throttler = throttler, my_main_table = main_table,
                               my_table_func_ptr = table_func_ptr, my_scalars = scalars, my_external_tables = external_tables,
                               my_stage = stage, my_storage = storage, add_agg_info, add_totals, add_extremes, async_read,
                               async_query_sending, query_tree = shard.query_tree, planner_context = shard.planner_context,
                               pushed_down_filters, parallel_marshalling_threads]() mutable -> QueryPipelineBuilder {
    ...
    for (const auto &try_result : try_results) {
      if (!try_result.is_up_to_date) max_remote_delay = std::max(try_result.delay, max_remote_delay);
    }

    bool use_delayed_remote_source = false;
    fiu_do_on(FailPoints::use_delayed_remote_source, { use_delayed_remote_source = true; });
    ...
    if (!use_delayed_remote_source) {
      ...
      if (try_results.empty() && local_delay >= max_allowed_delay) {
        throw Exception(ErrorCodes::ALL_REPLICAS_ARE_STALE,
                        "Failed to connect to other replicas and the local replica's delay is {} which is higher than "
                        "max_replica_delay_for_distributed_queries",
                        local_delay);
      }

      if (try_results.empty() || (local_delay < max_remote_delay && local_delay < max_allowed_delay)) {
        auto plan = createLocalPlan(query, *header, my_context, my_stage, my_shard.shard_info.shard_num, my_shard_count);
    ...
    for (auto &try_result : try_results) connections.emplace_back(std::move(try_result.entry));
    ...
    if (pushed_down_filters) addFilters(nullptr, my_context, query, query_tree, planner_context, *pushed_down_filters);
    ...
    auto remote_query_executor =
        std::make_shared<RemoteQueryExecutor>(std::move(connections), query_string, header, my_context, my_throttler, my_scalars,
                                              my_external_tables, stage_to_use, my_shard.query_plan);
...
void ReadFromRemote::addPipe(Pipes &pipes, const ClusterProxy::SelectStreamFactory::Shard &shard, const SharedHeader &out_header,
                             size_t parallel_marshalling_threads) {
  bool add_agg_info = stage == QueryProcessingStage::WithMergeableState;
  bool add_totals = false;
  bool add_extremes = false;
  ...
  if (shard.shard_filter_generator) {
    for (size_t i = 0; i < shard.shard_info.per_replica_pools.size(); ++i) {
      ...
      if (shard_filter) {
        ...
        if (where_expression) shard_filter = makeASTFunction("and", where_expression, shard_filter);
      ...
      if (!priority_func_factory.has_value()) priority_func_factory = GetPriorityForLoadBalancing(LoadBalancing::ROUND_ROBIN, randomSeed());
      ...
      auto remote_query_executor =
          std::make_shared<RemoteQueryExecutor>(shard.shard_info.pool, query_string, shard.header, context, throttler, scalars,
                                                external_tables, stage_to_use, shard.query_plan, std::nullopt, priority_func);
      remote_query_executor->setLogger(log);
      remote_query_executor->setPoolMode(PoolMode::GET_ONE);

      if (!table_func_ptr) remote_query_executor->setMainTable(shard.main_table ? shard.main_table : main_table);

      pipes.emplace_back(createRemoteSourcePipe(remote_query_executor, add_agg_info, add_totals, add_extremes, async_read,
                                                async_query_sending, parallel_marshalling_threads));
    ...
    auto remote_query_executor = std::make_shared<RemoteQueryExecutor>(shard.shard_info.pool, query_string, shard.header, context,
                                                                       throttler, scalars, external_tables, stage_to_use, shard.query_plan);
    remote_query_executor->setLogger(log);
    ...
    if (context->canUseTaskBasedParallelReplicas() || parallel_replicas_disabled) {
      ...
      // establish a connection to a replica on the shard, the replica will instantiate coordinator to manage parallel reading from replicas
      // on the shard. The coordinator will return query result from the shard. Only one coordinator per shard is necessary. Therefore using
      // PoolMode::GET_ONE to establish only one connection per shard. Using PoolMode::GET_MANY for this mode will(can) lead to
      // instantiation of several coordinators (depends on max_parallel_replicas setting) each will execute parallel reading from replicas,
      // so the query result will be multiplied by the number of created coordinators
      //
...
void ReadFromRemote::initializePipeline(QueryPipelineBuilder &pipeline, const BuildQueryPipelineSettings &) {
  ...
  for (const auto &processor : pipe.getProcessors()) processor->setStorageLimits(storage_limits);
...
static void formatExplain(IQueryPlanStep::FormatSettings &settings, Pipes pipes) {
  ...
  for (auto &pipe : pipes) {
    ...
    while (executor.pull(chunk)) {
      if (!chunk.hasColumns() || !chunk.hasRows()) continue;

      const auto &col = chunk.getColumns().front();
      size_t num_rows = col->size();

      for (size_t row = 0; row < num_rows; ++row) settings.out << prefix << col->getDataAt(row) << '\n';
    }
  }
}
...
void ReadFromRemote::describeDistributedPlan(FormatSettings &settings, const ExplainPlanOptions &options) {
  auto header =
      std::make_shared<const Block>(Block{ColumnWithTypeAndName{ColumnString::create(), std::make_shared<DataTypeString>(), "explain"}});
...
bool ReadFromRemote::hasSerializedPlan() const {
  for (const auto &shard : shards)
    if (shard.query_plan) return true;

  return false;
}

ReadFromParallelRemoteReplicasStep::ReadFromParallelRemoteReplicasStep(
    ASTPtr query_ast_, const QueryTreeNodePtr &query_tree_, const PlannerContextPtr &planner_context_, ClusterPtr cluster_,
    const StorageID &storage_id_, ParallelReplicasReadingCoordinatorPtr coordinator_, SharedHeader header_,
    QueryProcessingStage::Enum stage_, ContextMutablePtr context_, ThrottlerPtr throttler_, Scalars scalars_, Tables external_tables_,
    LoggerPtr log_, std::shared_ptr<const StorageLimitsList> storage_limits_, std::vector<ConnectionPoolPtr> pools_to_use_,
    std::optional<size_t> exclude_pool_index_, ConnectionPoolWithFailoverPtr connection_pool_with_failover_,
  ...
  for (size_t i = 0, l = pools_to_use.size(); i < l; ++i) {
    if (exclude_pool_index.has_value() && i == exclude_pool_index) continue;
...
void ReadFromParallelRemoteReplicasStep::enableMemoryBoundMerging() { DB::enableMemoryBoundMerging(stage, nullptr, *context); }
...
Pipes ReadFromParallelRemoteReplicasStep::addPipes(ASTPtr ast, const SharedHeader &out_header) {
  ...
  for (size_t i = 0, l = pools_to_use.size(); i < l; ++i) {
    ...
    LOG_DEBUG(getLogger("ReadFromParallelRemoteReplicasStep"), "Replica number {} assigned to address {}",
              replica_info.number_of_current_replica, pools_to_use[i]->getAddress());
  ...
  fiu_do_on(FailPoints::parallel_replicas_wait_for_unused_replicas, { wait_for_unused_replicas = true; });
  if (!wait_for_unused_replicas) {
    coordinator->setReadCompletedCallback([sources = std::move(remote_sources)](const std::set<size_t> &used_replicas) {
      for (const auto &[replica_num, processor] : sources) {
        if (used_replicas.contains(replica_num)) continue;
...
Pipe ReadFromParallelRemoteReplicasStep::createPipeForSingeReplica(const ConnectionPoolPtr &pool, ASTPtr ast,
                                                                   IConnections::ReplicaInfo replica_info, const SharedHeader &out_header,
                                                                   size_t parallel_marshalling_threads) {
  bool add_agg_info = stage == QueryProcessingStage::WithMergeableState;
  bool add_totals = false;
  bool add_extremes = false;
  ...
  auto remote_query_executor = std::make_shared<RemoteQueryExecutor>(
      pool, query_string, out_header, context, throttler, scalars, external_tables, query_plan ? QueryProcessingStage::QueryPlan : stage,
      RemoteQueryExecutor::Extension{.parallel_reading_coordinator = coordinator, .replica_info = std::move(replica_info)},
      connection_pool_with_failover, query_plan);

  remote_query_executor->setLogger(log);
  remote_query_executor->setMainTable(storage_id);

  Pipe pipe = createRemoteSourcePipe(std::move(remote_query_executor), add_agg_info, add_totals, add_extremes, async_read,
                                     async_query_sending, parallel_marshalling_threads);
