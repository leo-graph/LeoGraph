...
static ColumnsWithTypeAndName getColumnSubset(const Block &block, const Names &column_names) {
  ...
  for (const auto &name : column_names) result.emplace_back(block.getByName(name));
  return result;
}
...
CreateSetAndFilterOnTheFlyStep::CreateSetAndFilterOnTheFlyStep(const SharedHeader &input_header_, const Names &column_names_,
    ...
    : ITransformingStep(input_header_, input_header_, getTraits()),
      ...
      position(position_) {
  if (crosswise_connection == nullptr) throw Exception(ErrorCodes::LOGICAL_ERROR, "Crosswise connection is not initialized");
...
void CreateSetAndFilterOnTheFlyStep::transformPipeline(QueryPipelineBuilder &pipeline, const BuildQueryPipelineSettings &) {
  ...
  pipeline.addSimpleTransform(
      [this, num_streams](const SharedHeader &header, QueryPipelineBuilder::StreamType stream_type) -> ProcessorPtr {
...
void CreateSetAndFilterOnTheFlyStep::describeActions(JSONBuilder::JSONMap &map) const { map.add(getName(), true); }
