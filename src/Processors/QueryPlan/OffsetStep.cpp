...
OffsetStep::OffsetStep(const SharedHeader &input_header_, size_t offset_)
    : ITransformingStep(input_header_, input_header_, getTraits()), offset(offset_) {}
...
void OffsetStep::describeActions(JSONBuilder::JSONMap &map) const { map.add("Offset", offset); }

void OffsetStep::serialize(Serialization &ctx) const { writeVarUInt(offset, ctx.out); }
...
void registerOffsetStep(QueryPlanStepRegistry &registry) { registry.registerStep("Offset", OffsetStep::deserialize); }
