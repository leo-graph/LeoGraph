...
LazilyReadFromMergeTree::LazilyReadFromMergeTree(SharedHeader header, size_t max_block_size_, size_t min_marks_for_concurrent_read_,
                                                 MergeTreeReaderSettings reader_settings_,
                                                 MergeTreeData::MutationsSnapshotPtr mutations_snapshot_,
                                                 StorageSnapshotPtr storage_snapshot_, ContextPtr context_, const std::string& log_name_)
...
void LazilyReadFromMergeTree::initializePipeline(QueryPipelineBuilder& pipeline, const BuildQueryPipelineSettings& settings) {
  if (!lazy_materializing_rows) throw Exception(ErrorCodes::LOGICAL_ERROR, "LazyReadFromMergeTree: lazy_materializing_rows is not set");

  auto source = std::make_shared<LazyReadFromMergeTreeSource>(
      getOutputHeader(), max_block_size, settings.max_threads, min_marks_for_concurrent_read, settings.actions_settings, reader_settings,
      mutations_snapshot, storage_snapshot, context, log_name, lazy_materializing_rows, dataflow_cache_updater);
...
void LazilyReadFromMergeTree::describeActions(JSONBuilder::JSONMap& map) const {
  ...
  for (const auto& column : *getOutputHeader()) json_array->add(column.name);
