...
TotalsHavingStep::TotalsHavingStep(SharedHeader input_header_, const AggregateDescriptions& aggregates_, bool overflow_row_,
                                   std::optional<ActionsDAG> actions_dag_, const std::string& filter_column_, bool remove_filter_,
                                   TotalsMode totals_mode_, float auto_include_threshold_, bool final_)
    : ITransformingStep(input_header_,
                        std::make_shared<const Block>(
                            TotalsHavingTransform::transformHeader(*input_header_, actions_dag_ ? &*actions_dag_ : nullptr, filter_column_,
                                                                   remove_filter_, final_, getAggregatesMask(*input_header_, aggregates_))),
...
void TotalsHavingStep::transformPipeline(QueryPipelineBuilder& pipeline, const BuildQueryPipelineSettings& settings) {
  ...
  auto totals_having = std::make_shared<TotalsHavingTransform>(pipeline.getHeader(), getAggregatesMask(pipeline.getHeader(), aggregates),
                                                               overflow_row, expression_actions, filter_column_name, remove_filter,
                                                               totals_mode, auto_include_threshold, final);

  pipeline.addTotalsHavingTransform(std::move(totals_having));

  /// Pipeline is resized to single stream before TotalsHavingTransform. So, we need to resize it back to "default" number of streams after
  /// transform.
...
void TotalsHavingStep::describeActions(FormatSettings& settings) const {
  ...
  if (remove_filter) settings.out << " (removed)";
  settings.out << '\n';
...
void TotalsHavingStep::updateOutputHeader() {
  output_header = std::make_shared<const Block>(
      TotalsHavingTransform::transformHeader(*input_headers.front(), getActions(), filter_column_name, remove_filter, final,
...
void TotalsHavingStep::serialize(Serialization& ctx) const {
  UInt8 flags = 0;
  if (final) flags |= 1;
  if (overflow_row) flags |= 2;
  if (actions_dag) flags |= 4;
  if (actions_dag && remove_filter) flags |= 8;
...
QueryPlanStepPtr TotalsHavingStep::deserialize(Deserialization& ctx) {
  if (ctx.input_headers.size() != 1) throw Exception(ErrorCodes::INCORRECT_DATA, "TotalsHaving must have one input stream");
  ...
  return std::make_unique<TotalsHavingStep>(ctx.input_headers.front(), std::move(aggregates), overflow_row, std::move(actions_dag),
                                            std::move(filter_column_name), remove_filter_column,
                                            ctx.settings[QueryPlanSerializationSetting::totals_mode],
                                            ctx.settings[QueryPlanSerializationSetting::totals_auto_threshold], final);
}

void registerTotalsHavingStep(QueryPlanStepRegistry& registry) { registry.registerStep("TotalsHaving", TotalsHavingStep::deserialize); }
