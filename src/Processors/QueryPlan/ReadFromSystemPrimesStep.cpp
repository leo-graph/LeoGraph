...
std::vector<Range> intersectWithPrimesDomain(const Ranges &ranges) {
  ...
  for (const auto &range : ranges)
    if (auto intersection = primes_domain.intersectWith(range)) intersections.push_back(*intersection);
...
std::vector<Interval> rangesToIntervals(const std::vector<Range> &ranges) {
  ...
  for (const auto &range : ranges) {
    ...
    if (!range.left_included) {
      if (l == std::numeric_limits<UInt64>::max()) continue;
      ++l;
    }
...
ReadFromSystemPrimesStep::ReadFromSystemPrimesStep(const Names &column_names_, const SelectQueryInfo &query_info_,
                                                   const StorageSnapshotPtr &storage_snapshot_, const ContextPtr &context_,
                                                   StoragePtr storage_, size_t max_block_size_)
  ...
  if (column_names.size() != 1) throw Exception(ErrorCodes::LOGICAL_ERROR, "SystemPrimes supports exactly one column");
...
QueryPlanStepPtr ReadFromSystemPrimesStep::clone() const {
  return std::make_unique<ReadFromSystemPrimesStep>(column_names, getQueryInfo(), getStorageSnapshot(), getContext(), storage,
                                                    max_block_size);
}
...
Pipe ReadFromSystemPrimesStep::makePipe() {
  ...
  auto intersect_ranges = [&](const Ranges &ranges) -> std::optional<std::vector<Interval>> {
    ...
    if (intersected.empty()) return std::nullopt;
  ...
  if (!filter_actions_dag) {
    ...
    if (effective_limit) NumbersLikeUtils::checkLimits(context->getSettingsRef(), static_cast<size_t>(*effective_limit));

    auto source = std::make_shared<PrimesSource>(static_cast<UInt64>(max_block_size), primes_storage.offset, effective_limit,
                                                 primes_storage.step, primes_storage.column_name);

    if (effective_limit) source->addTotalRowsApprox(*effective_limit);
  ...
  if (!primes_storage.limit && extracted_ranges.kind == NumbersLikeUtils::ExtractedRanges::Kind::ExactRanges) {
    ...
    if (primes_storage.offset == 0 && primes_storage.step == 1) {
      auto source = std::make_shared<PrimesSimpleRangedSource>(static_cast<UInt64>(max_block_size), std::move(intervals), effective_limit,
    ...
    auto source = std::make_shared<PrimesRangedSource>(static_cast<UInt64>(max_block_size), std::move(intervals), primes_storage.offset,
                                                       effective_limit, primes_storage.step, primes_storage.column_name);
  ...
  if (!primes_storage.limit && !NumbersLikeUtils::isUniverse(extracted_ranges.ranges)) {
    ...
    if (primes_storage.offset == 0 && primes_storage.step == 1) {
      pipe.addSource(std::make_shared<PrimesSimpleRangedSource>(static_cast<UInt64>(max_block_size), std::move(intervals), std::nullopt,
                                                                primes_storage.column_name));
      return pipe;
    }

    pipe.addSource(std::make_shared<PrimesRangedSource>(static_cast<UInt64>(max_block_size), std::move(intervals), primes_storage.offset,
                                                        std::nullopt, primes_storage.step, primes_storage.column_name));
    return pipe;
  }
