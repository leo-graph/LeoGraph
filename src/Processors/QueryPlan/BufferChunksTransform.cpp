...
BufferChunksTransform::BufferChunksTransform(SharedHeader header_, size_t max_rows_to_buffer_, size_t max_bytes_to_buffer_, size_t limit_)
...
Chunk BufferChunksTransform::pullChunk(bool& virtual_row) {
  ...
  if (!virtual_row) num_processed_rows += chunk.getNumRows();

  if (limit && num_processed_rows >= limit) input.close();
