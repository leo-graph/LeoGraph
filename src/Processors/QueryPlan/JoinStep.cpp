...
std::vector<size_t> getPermutationForBlock(const Block &block, const Block &lhs_block, const Block &rhs_block, const NameSet &name_filter) {
  ...
  for (const auto &other_block : {lhs_block, rhs_block}) {
    for (const auto &col : other_block) {
      if (!name_filter.contains(col.name)) continue;
  ...
  if (is_trivial && permutation.size() == block.columns()) return {};
...
JoinStep::JoinStep(const SharedHeader &left_header_, const SharedHeader &right_header_, JoinPtr join_, size_t max_block_size_,
                   size_t min_block_size_rows_, size_t min_block_size_bytes_, size_t max_streams_, NameSet required_output_,
                   bool keep_left_read_in_order_, bool use_new_analyzer_, bool use_join_disjunctions_push_down_)
    : join(std::move(join_)),
...
QueryPipelineBuilderPtr JoinStep::updatePipeline(QueryPipelineBuilders pipelines, const BuildQueryPipelineSettings &) {
  if (pipelines.size() != 2) throw Exception(ErrorCodes::LOGICAL_ERROR, "JoinStep expect two input steps");

  Block lhs_header = pipelines[0]->getHeader();
  Block rhs_header = pipelines[1]->getHeader();

  if (swap_streams) std::swap(pipelines[0], pipelines[1]);
  ...
  if (!use_sharding) {
    if (join->pipelineType() == JoinPipelineType::YShaped) {
      joined_pipeline = QueryPipelineBuilder::joinPipelinesYShaped(std::move(pipelines[0]), std::move(pipelines[1]), join,
                                                                   join_algorithm_header, max_block_size, &processors);
      joined_pipeline->resize(max_streams);
    } else {
      joined_pipeline = QueryPipelineBuilder::joinPipelinesRightLeft(
          std::move(pipelines[0]), std::move(pipelines[1]), join, join_algorithm_header, max_block_size, min_block_size_rows,
          min_block_size_bytes, max_streams, keep_left_read_in_order, &processors);
    }
  } else {
    if (join->pipelineType() == JoinPipelineType::YShaped) {
      joined_pipeline = QueryPipelineBuilder::joinPipelinesYShapedByShards(std::move(pipelines[0]), std::move(pipelines[1]), join,
                                                                           join_algorithm_header, max_block_size, &processors);
    } else {
      joined_pipeline = QueryPipelineBuilder::joinPipelinesByShards(std::move(pipelines[0]), std::move(pipelines[1]), join,
                                                                    join_algorithm_header, max_block_size, &processors);
    }
  }

  if (!use_new_analyzer) return joined_pipeline;
  ...
  if (!column_permutation.empty()) {
    joined_pipeline->addSimpleTransform(
        [&column_permutation](const SharedHeader &header) { return std::make_shared<ColumnPermuteTransform>(header, column_permutation); });
  }
  ...
  if (!isCompatibleHeader(pipeline_output_header, *expected_output_header)) {
    assertBlocksHaveEqualStructure(
        pipeline_output_header, *expected_output_header,
...
void JoinStep::describePipeline(FormatSettings &settings) const { IQueryPlanStep::describePipeline(processors, settings); }

void JoinStep::describeActions(FormatSettings &settings) const {
  String prefix(settings.offset, ' ');

  for (const auto &[name, value] : describeJoinActions(join)) settings.out << prefix << name << ": " << value << '\n';
...
void JoinStep::describeActions(JSONBuilder::JSONMap &map) const {
  for (const auto &[name, value] : describeJoinActions(join)) map.add(name, value);
  if (swap_streams) map.add("Swapped", true);
  if (!primary_key_sharding.empty()) {
...
void JoinStep::updateOutputHeader() {
  if (join_algorithm_header && !join_algorithm_header->empty()) return;
  ...
  if (!use_new_analyzer) {
    if (swap_streams) throw Exception(ErrorCodes::LOGICAL_ERROR, "Cannot swap streams without the analyzer");
...
FilledJoinStep::FilledJoinStep(const SharedHeader &input_header_, JoinPtr join_, size_t max_block_size_)
    : ITransformingStep(input_header_, std::make_shared<const Block>(JoiningTransform::transformHeader(*input_header_, join_)),
                        getStorageJoinTraits()),
      join(std::move(join_)),
      max_block_size(max_block_size_) {
  if (!join->isFilled()) throw Exception(ErrorCodes::LOGICAL_ERROR, "FilledJoinStep expects Join to be filled");
}
