...
static void addToNullableIfNeeded(JoinExpressionActions &expression_actions, JoinKind join_kind, bool use_nulls,
                                  const NameSet &required_output_columns, std::vector<const ActionsDAG::Node *> &actions_after_join,
  ...
  for (const auto *node : actions_dag->getInputs()) {
    if (!required_output_columns.contains(node->result_name) && node->result_name != join_dummy_result_name) continue;
    ...
    if (node->result_name != original_name) node = &actions_dag->addAlias(*node, std::move(original_name));
...
JoinStepLogical::JoinStepLogical(SharedHeader left_header_, SharedHeader right_header_, JoinOperator join_operator_,
                                 JoinExpressionActions join_expression_actions_, const NameSet &required_output_columns_,
                                 const std::unordered_map<String, const ActionsDAG::Node *> &changed_types, bool use_nulls_,
                                 JoinSettings join_settings_, SortingStep::Settings sorting_settings_)
...
JoinStepLogical::JoinStepLogical(const SharedHeader &left_header_, const SharedHeader &right_header_, JoinOperator join_operator_,
                                 JoinExpressionActions join_expression_actions_, std::vector<const ActionsDAG::Node *> actions_after_join_,
                                 JoinSettings join_settings_, SortingStep::Settings sorting_settings_)
    : expression_actions(std::move(join_expression_actions_)),
      join_operator(std::move(join_operator_)),
      join_settings(std::move(join_settings_)),
...
#ifndef NDEBUG
  auto all_nodes_set = std::ranges::to<std::unordered_set>(expression_actions.getActionsDAG()->getNodes() |
                                                           std::views::transform([](const auto &node) { return &node; }));
...
std::unordered_set<JoinTableSide> JoinStepLogical::typeChangingSides() const {
  ...
  for (const auto *node_after_join : actions_after_join) {
    if (node_after_join->type == ActionsDAG::ActionType::INPUT || node_after_join->type == ActionsDAG::ActionType::COLUMN) continue;
    if (JoinActionRef(node_after_join, expression_actions).fromLeft()) result.insert(JoinTableSide::Left);
    if (JoinActionRef(node_after_join, expression_actions).fromRight()) result.insert(JoinTableSide::Right);
...
QueryPipelineBuilderPtr JoinStepLogical::updatePipeline(QueryPipelineBuilders /* pipelines */,
...
void JoinStepLogical::describePipeline(FormatSettings &settings) const { IQueryPlanStep::describePipeline(processors, settings); }
...
std::string_view joinTypePretty(const JoinOperator &join_operator) { return joinTypePretty(join_operator.kind, join_operator.strictness); }

String JoinStepLogical::getReadableRelationName() const {
  if (left_table_label.empty() || right_table_label.empty()) return "";
...
std::vector<std::pair<String, String>> JoinStepLogical::describeJoinProperties() const {
  ...
  if (!readable_relation_name.empty()) description.emplace_back("Join", std::move(readable_relation_name));
...
void JoinStepLogical::describeActions(FormatSettings &settings) const {
  ...
  for (const auto &[name, value] : describeJoinProperties()) settings.out << prefix << name << ": " << value << '\n';
...
void JoinStepLogical::describeActions(JSONBuilder::JSONMap &map) const {
  for (const auto &[name, value] : describeJoinProperties()) map.add(name, value);

  auto actions_dag = expression_actions.getActionsDAG()->clone();
  map.add("Actions", ExpressionActions(std::move(actions_dag)).toTree());
}

bool JoinStepLogical::canRemoveUnusedColumns() const { return !hasDuplicatedNamesInInputOrOutputs(*expression_actions.getActionsDAG()); }
...
IQueryPlanStep::RemovedUnusedColumns JoinStepLogical::removeUnusedColumns(NameMultiSet required_outputs, bool remove_inputs) {
  ...
  for (const auto *output_node : actions_dag.getOutputs()) {
    if (auto it = required_outputs.find(output_node->result_name); it != required_outputs.end()) {
      ...
      new_actions_after_join.erase(std::remove(new_actions_after_join.begin(), new_actions_after_join.end(), output_node),
                                   new_actions_after_join.end());
    }
  }
  ...
  for (const auto &join_action : join_operator.expression) required_nodes.push_back(join_action.getNode());

  for (const auto &join_action : join_operator.residual_filter) required_nodes.push_back(join_action.getNode());
  ...
  for (const auto *required_node : required_nodes) {
    ...
    if (has_required_input_from_left && has_required_input_from_right) break;
  }
  ...
  if (!has_required_input_from_left && !actions_dag.getInputs().empty() && !getInputHeaders().at(0)->empty()) {
    ...
    if (JoinActionRef(maybe_left_input, expression_actions).fromLeft()) required_nodes.push_back(maybe_left_input);
  }
  ...
  if (!has_required_input_from_right && actions_dag.getInputs().size() >= number_of_left_inputs && !getInputHeaders().at(1)->empty()) {
    ...
    if (JoinActionRef(maybe_right_input, expression_actions).fromRight()) required_nodes.push_back(maybe_right_input);
  }
  ...
  if (!removed_any_actions && !removed_any_output) return RemovedUnusedColumns::None;
  ...
  for (const auto *input_node : actions_dag.getInputs()) required_input_names.insert(input_node->result_name);

  const auto remove_unused_inputs_from_header = [&required_input_names](const Block &header) {
    Block new_header;
    for (const auto &column : header) {
      if (required_input_names.contains(column.name)) new_header.insert(column);
    }
    return std::make_shared<const Block>(std::move(new_header));
  };

  SharedHeaders new_input_headers;
  for (const auto &input_header : input_headers) new_input_headers.push_back(remove_unused_inputs_from_header(*input_header));
...
void JoinStepLogical::updateOutputHeader() {
  ...
  for (auto &column : header) {
    if (!column.column) column.column = column.type->createColumn();
  }

  if (!header.columns()) throw Exception(ErrorCodes::LOGICAL_ERROR, "Output header is empty, actions_dag: {}", actions_dag->dumpDAG());
...
void JoinStepLogicalLookup::initializePipeline(QueryPipelineBuilder &pipeline_builder,
...
bool canPushDownFromOn(const JoinOperator &join_operator, std::optional<JoinTableSide> side = {}) {
  switch (join_operator.strictness) {
    ...
    case JoinStrictness::All: {
      ...
      bool is_suitable_kind = join_operator.kind == JoinKind::Inner || join_operator.kind == JoinKind::Cross ||
...
void predicateOperandsToCommonType(JoinActionRef &left_node, JoinActionRef &right_node, const JoinSettings &join_settings,
  ...
  if (!join_settings.allow_dynamic_type_in_join_keys) {
    ...
    if (is_left_key_dynamic || is_right_key_dynamic) {
      throw DB::Exception(ErrorCodes::ILLEGAL_COLUMN,
                          "JOIN on keys with Dynamic type is not supported: key {} has type {}. In order to use this key in JOIN you "
                          "should cast it to any other type",
  ...
  if (left_type->equals(*right_type)) return;
  ...
  try {
    ...
    ex.addMessage("JOIN cannot infer common type in ON section for keys. Left key '{}' type {}. Right key '{}' type {}",
                  left_node.getColumnName(), left_type->getName(), right_node.getColumnName(), right_type->getName());
    throw;
  }
  ...
  if (!left_type->equals(*common_type)) left_node = JoinActionRef::transform({left_node}, cast_transform);
  ...
  if (planning_context.is_storage_join) {
    ...
    if (!right_type->equals(*common_type)) right_node = JoinActionRef::transform({right_node}, cast_transform);
  }
}

bool addJoinPredicatesToTableJoin(std::vector<JoinActionRef> &predicates, TableJoin::JoinOnClause &table_join_clause,
                                  std::vector<JoinActionRef> &used_expressions, const JoinSettings &join_settings,
                                  const JoinPlanningContext &planning_context) {
  ...
  for (auto &pred : predicates) {
    ...
    if (predicate_op != JoinConditionOperator::Equals && predicate_op != JoinConditionOperator::NullSafeEquals) continue;
...
JoinActionRef concatConditions(std::vector<JoinActionRef> &conditions, std::optional<JoinTableSide> side = {},
  ...
  auto matching_point = std::ranges::partition(conditions, [side](const auto &node) {
...
bool tryAddDisjunctiveConditions(std::vector<JoinActionRef> &join_expressions, TableJoin::Clauses &table_join_clauses,
                                 std::vector<JoinActionRef> &used_expressions, const JoinSettings &join_settings,
                                 const JoinPlanningContext &planning_context, bool throw_on_error) {
  if (join_expressions.size() != 1) return false;

  auto &join_expression = join_expressions.front();
  if (!join_expression.isFunction(JoinConditionOperator::Or)) return false;
  ...
  for (const auto &expr : disjunctive_conditions) {
    ...
    if (expr.isFunction(JoinConditionOperator::And)) join_condition = expr.getArguments();
    ...
    if (!join_condition.empty()) has_residual_condition = true;
  }

  /// Clear join_expressions if there is no unhandled conditions, no need to calculate residual filter
  if (!has_residual_condition) join_expressions.clear();

  return true;
}

static void addSortingForMergeJoin(const FullSortingMergeJoin *join_ptr, QueryPlan::Node *&left_node, QueryPlan::Node *&right_node,
                                   QueryPlan::Nodes &nodes, const SortingStep::Settings &sort_settings, const JoinSettings &join_settings,
                                   const TableJoin &table_join, size_t max_step_description_length) {
  ...
  auto add_sorting = [&](QueryPlan::Node *&node, const Names &key_names, JoinTableSide join_table_side) {
    ...
    auto sorting_step = std::make_unique<SortingStep>(node->step->getOutputHeader(), std::move(sort_description), 0 /*limit*/,
                                                      sort_settings, true /*is_sorting_for_merge_join*/);
  ...
  /// No checks here because read in order is not applied if we have `CreateSetAndFilterOnTheFlyStep` in the pipeline between the reading
  /// and sorting steps.
...
static void constructPhysicalStep(QueryPlanNode &node, ActionsDAG left_pre_join_actions, ActionsDAG right_after_join_actions,
                                  ActionsDAG post_join_actions, std::pair<String, bool> residual_filter_condition, JoinPtr join_ptr,
                                  const JoinSettings &join_settings, QueryPlan::Nodes &nodes) {
  if (!join_ptr->isFilled()) throw Exception(ErrorCodes::LOGICAL_ERROR, "Join is not filled");
  ...
  makeFilterNodeOnTopOf(node, std::move(post_join_actions), residual_filter_condition.first, residual_filter_condition.second, nodes,
                        makeDescription("Post Join Actions"));
}

static void constructPhysicalStep(QueryPlanNode &node, ActionsDAG left_pre_join_actions, ActionsDAG right_pre_join_actions,
                                  ActionsDAG post_join_actions, std::pair<String, bool> residual_filter_condition, JoinPtr join_ptr,
                                  const QueryPlanOptimizationSettings &optimization_settings, const JoinSettings &join_settings,
                                  const SortingStep::Settings &sorting_settings, QueryPlan::Nodes &nodes) {
  if (node.children.size() != 2) throw Exception(ErrorCodes::LOGICAL_ERROR, "Expected 2 children, got {}", node.children.size());

  if (join_ptr->isFilled()) throw Exception(ErrorCodes::LOGICAL_ERROR, "Join is already filled");
  ...
  if (const auto *fsmjoin = dynamic_cast<const FullSortingMergeJoin *>(join_ptr.get()))
    addSortingForMergeJoin(fsmjoin, join_left_node, join_right_node, nodes, sorting_settings, join_settings, fsmjoin->getTableJoin(),
  ...
  auto join_step = std::make_unique<JoinStep>(join_left_node->step->getOutputHeader(), join_right_node->step->getOutputHeader(), join_ptr,
                                              join_settings.max_block_size, join_settings.min_joined_block_size_rows,
                                              join_settings.min_joined_block_size_bytes, optimization_settings.max_threads,
                                              NameSet(required_output_from_join.begin(), required_output_from_join.end()),
                                              false /*optimize_read_in_order*/, true /*use_new_analyzer*/);
...
static QueryPlanNode buildPhysicalJoinImpl(std::vector<QueryPlanNode *> children, JoinOperator join_operator,
                                           JoinExpressionActions expression_actions, JoinSettings join_settings,
                                           JoinAlgorithmParams join_algorithm_params, SortingStep::Settings sorting_settings,
                                           const ActionsDAG::NodeRawConstPtrs &actions_after_join,
                                           const QueryPlanOptimizationSettings &optimization_settings, QueryPlan::Nodes &nodes) {
  ...
  if (logical_lookup) {
    ...
    prepared_join_storage.visit([&table_join](const auto &storage_) { table_join->setStorageJoin(storage_); });
  }
  ...
  for (const auto *node : actions_after_join) {
    if (node->type == ActionsDAG::ActionType::ALIAS) planning_context.actions_after_join_map[node->result_name] = node->children.at(0);
  }
  ...
  if (!is_join_without_expression) {
    ...
    if (!has_keys && join_operator.strictness != JoinStrictness::Asof) {
      ...
      is_disjunctive_condition = tryAddDisjunctiveConditions(join_expression, table_join_clauses, used_expressions, join_settings,
                                                             planning_context, !can_convert_to_cross);
    ...
    throw Exception(ErrorCodes::LOGICAL_ERROR, "Unexpected JOIN ON expression {} for {} JOIN", formatJoinCondition(join_expression),
                    toString(join_operator.kind));
  }
  ...
  if (join_operator.strictness == JoinStrictness::Asof) {
    ...
    for (auto it = join_expression.begin(); it != join_expression.end(); ++it) {
      ...
      if (!asof_inequality_op) continue;
      ...
      if (found_asof_predicate_it != join_expression.end())
        throw Exception(ErrorCodes::INVALID_JOIN_ON_EXPRESSION,
                        "ASOF join does not support multiple inequality predicates in JOIN ON expression");
  ...
  if (auto left_pre_filter_condition = concatConditions(join_expression, JoinTableSide::Left)) {
    table_join_clauses.at(table_join_clauses.size() - 1).analyzer_left_filter_condition_column_name =
        left_pre_filter_condition.getColumnName();
    used_expressions.push_back(left_pre_filter_condition);
  }

  if (auto right_pre_filter_condition = concatConditions(join_expression, JoinTableSide::Right)) {
    table_join_clauses.at(table_join_clauses.size() - 1).analyzer_right_filter_condition_column_name =
        right_pre_filter_condition.getColumnName();
    used_expressions.push_back(right_pre_filter_condition);
  }
  ...
  if (residual_filter_condition) {
    ...
    while (!stack.empty()) {
      ...
      if (node->type == ActionsDAG::ActionType::INPUT) {
        if (actions_after_join_fold.contains(node)) continue;
        actions_after_join_fold[node] = node;
        required_residual_nodes.push_back(node);
      }
      for (const auto *child : node->children) stack.push(child);
    }
  }
  ...
  for (const auto *action : required_residual_nodes) used_expressions.emplace_back(action, expression_actions);
  ...
  for (const auto *child : children) {
    ...
    for (const auto *node : dag_inputs) name_to_nodes[node->result_name].push_back(node);
    ...
    for (const auto &column : *child->step->getOutputHeader()) {
      ...
      if (input_it == name_to_nodes.end() || input_it->second.empty())
        throw Exception(
            ...
            fmt::join(children |
                          std::views::transform([](const auto &c) { return fmt::format("[{}]", c->step->getOutputHeader()->dumpNames()); }),
                      ", "),
  ...
  ActionsDAG left_dag = JoinExpressionActions::getSubDAG(
      used_expressions | std::views::filter([](const auto &node) { return node.fromLeft() || node.fromNone(); }));
  ActionsDAG right_dag =
      JoinExpressionActions::getSubDAG(used_expressions | std::views::filter([](const auto &node) { return node.fromRight(); }));

  if (logical_lookup && prepared_join_storage.storage_key_value) {
    right_dag.mergeInplace(JoinExpressionActions::getSubDAG(
        actions_after_join | std::views::transform([&](const auto *action) { return JoinActionRef(action, expression_actions); }) |
  ...
  table_join->setInputColumns(left_dag.getNamesAndTypesList(), right_dag.getNamesAndTypesList());
  ...
  auto join_algorithm_ptr =
      chooseJoinAlgorithm(table_join, prepared_join_storage, left_sample_block, right_sample_block, join_algorithm_params);
  ...
  if (!join_algorithm_ptr->isFilled()) {
    constructPhysicalStep(node, std::move(left_dag), std::move(right_dag), std::move(residual_dag),
                          std::make_pair(residual_filter_condition_name, can_remove_residual_filter), std::move(join_algorithm_ptr),
                          optimization_settings, join_settings, sorting_settings, nodes);
  } else {
    ...
    if (logical_lookup && prepared_join_storage.storage_join) {
      right_type_correction_actions = JoinExpressionActions::getSubDAG(
          actions_after_join | std::views::transform([&](const auto *action) { return JoinActionRef(action, expression_actions); }) |
          std::views::filter(
              [](const auto &action) { return action.fromRight() && action.getNode()->type != ActionsDAG::ActionType::INPUT; }));
    }

    constructPhysicalStep(node, std::move(left_dag), std::move(right_type_correction_actions), std::move(residual_dag),
                          std::make_pair(residual_filter_condition_name, can_remove_residual_filter), std::move(join_algorithm_ptr),
                          join_settings, nodes);
  }
  return node;
...
void JoinStepLogical::buildPhysicalJoin(QueryPlanNode &node, const QueryPlanOptimizationSettings &optimization_settings,
  ...
  if (!join_step || node.children.empty()) {
    if (node.step) {
      ...
      throw Exception(ErrorCodes::LOGICAL_ERROR, "Expected JoinStepLogical, got '{}' of type {} with {} children", node.step->getName(),
                      type_name, node.children.size());
    }
  ...
  if (!join_step->join_algorithm_params) {
    join_step->join_algorithm_params =
        std::make_unique<JoinAlgorithmParams>(join_step->join_settings, optimization_settings.max_threads, hash_table_key_hash,
                                              optimization_settings.max_entries_for_hash_table_stats,
                                              optimization_settings.initial_query_id, optimization_settings.lock_acquire_timeout);

    if (join_step->right_rows_estimation) join_step->join_algorithm_params->rhs_size_estimation = join_step->right_rows_estimation;

    if (hash_table_key_hash) {
      StatsCollectingParams params{/*key_=*/hash_table_key_hash,
      ...
      if (auto hint = hash_table_stats.getSizeHint(params)) join_step->join_algorithm_params->rhs_size_estimation = hint->source_rows;
    }
  }

  auto new_node = buildPhysicalJoinImpl(node.children, join_step->join_operator, std::move(join_step->expression_actions),
                                        join_step->join_settings, *join_step->join_algorithm_params, join_step->sorting_settings,
                                        join_step->actions_after_join, optimization_settings, nodes);

  node = std::move(new_node);
}

std::optional<ActionsDAG::ActionsForFilterPushDown> JoinStepLogical::getFilterActions(JoinTableSide side,
                                                                                      const SharedHeader &stream_header) {
  if (!canPushDownFromOn(join_operator, side)) return {};
...
void remapNodes(ActionsDAG::NodeRawConstPtrs &keys, const ActionsDAG::NodeMapping &node_map) {
  for (const auto *&key : keys) {
    if (auto it = node_map.find(key); it != node_map.end()) key = it->second;
  }
}
...
std::optional<std::pair<JoinStepLogical::ActionsDAGWithKeys, JoinStepLogical::ActionsDAGWithKeys>> JoinStepLogical::preCalculateKeys(
    const SharedHeader &left_header, const SharedHeader &right_header) {
  auto &join_expression = join_operator.expression;
  ...
  for (auto &expr : join_expression) {
    ...
    if (predicate_op != JoinConditionOperator::Equals) continue;
    ...
    if (left_node->type != ActionsDAG::ActionType::INPUT || right_node->type != ActionsDAG::ActionType::INPUT) {
  ...
  if (left_keys.empty() || right_keys.empty()) return {};
...
std::vector<JoinActionRef> JoinStepLogical::getInputActions() const {
  ...
  for (const auto *node : raw_inputs) input_actions.emplace_back(node, expression_actions);
...
std::vector<JoinActionRef> JoinStepLogical::getOutputActions() const {
  ...
  for (const auto *node : raw_outputs) output_actions.emplace_back(node, expression_actions);
...
static void serializeNodeList(WriteBuffer &out, const std::unordered_map<const ActionsDAG::Node *, size_t> &node_to_id,
...
QueryPlanStepPtr JoinStepLogical::deserialize(Deserialization &ctx) {
  if (ctx.input_headers.size() != 2) throw Exception(ErrorCodes::INCORRECT_DATA, "JoinStepLogical must have two input streams");
  ...
  ActionsDAG actions_dag;
  {
    ...
    if (num_dags != 1) throw Exception(ErrorCodes::INCORRECT_DATA, "JoinStepLogical deserialization expect 3 DAGs, got {}", num_dags);
  ...
  return std::make_unique<JoinStepLogical>(std::move(left_header), std::move(right_header), std::move(join_operator),
                                           std::move(expression_actions), std::move(actions_after_join), std::move(join_settings),
...
QueryPlanStepPtr JoinStepLogical::clone() const {
  ...
  auto remap = [&](const auto *node) {
    ...
    if (it == node_map.end()) throw Exception(ErrorCodes::LOGICAL_ERROR, "Cannot find node in node map");
    return it->second;
  };

  auto new_actions_dag = new_expression_actions.getActionsDAG();
  auto new_actions_after_join = actions_after_join;
  for (const auto *&action : new_actions_after_join) action = remap(action);
  for (auto &action : new_join_operator.expression) action = JoinActionRef(remap(action.getNode()), new_expression_actions);
  for (auto &action : new_join_operator.residual_filter) action = JoinActionRef(remap(action.getNode()), new_expression_actions);

  auto result_step = std::make_unique<JoinStepLogical>(getInputHeaders().front(), getInputHeaders().back(), std::move(new_join_operator),
                                                       std::move(new_expression_actions), std::move(new_actions_after_join), join_settings,
                                                       sorting_settings);
...
void JoinStepLogical::addConditions(ActionsDAG actions_dag) {
  ...
  for (const auto *node : conditions) join_operator.expression.emplace_back(node, expression_actions);
}

void registerJoinStep(QueryPlanStepRegistry &registry) { registry.registerStep("Join", JoinStepLogical::deserialize); }
