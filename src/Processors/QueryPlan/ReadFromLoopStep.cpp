...
void buildSelectQueryPlan(QueryPlan &plan, const Names &column_names, const SelectQueryInfo &query_info, ContextPtr context,
                          const String &database = {}, const String &table = {}, const ASTPtr &table_function_ast = nullptr) {
  auto select_query = make_intrusive<ASTSelectQuery>();

  auto select_expr_list = make_intrusive<ASTExpressionList>();
  for (const auto &col_name : column_names) select_expr_list->children.push_back(make_intrusive<ASTIdentifier>(col_name));
  ...
  if (context->getSettingsRef()[Setting::allow_experimental_analyzer]) {
    ...
    if (query_info.storage_limits) interpreter.addStorageLimits(*query_info.storage_limits);
...
class LoopSource : public ISource {
 public:
  LoopSource(const Names &column_names_, const SelectQueryInfo &query_info_, const StorageSnapshotPtr &storage_snapshot_,
             ContextPtr &context_, QueryProcessingStage::Enum processed_stage_, StoragePtr inner_storage_, ASTPtr inner_table_function_ast_,
  ...
  void initLoop() {
    ...
    if (DatabaseCatalog::instance().isTableExist(inner_storage->getStorageID(), context)) {
      ...
      buildSelectQueryPlan(plan, column_names, query_info, inner_context, storage_id.database_name, storage_id.table_name);
    } else if (inner_table_function_ast) {
      inner_context = Context::createCopy(context);
      buildSelectQueryPlan(plan, column_names, query_info, inner_context, {}, {}, inner_table_function_ast);
    } else {
      auto inner_storage_snapshot = inner_storage->getStorageSnapshot(inner_storage->getInMemoryMetadataPtr(), context);
      inner_storage->read(plan, column_names, inner_storage_snapshot, query_info, context, processed_stage, max_block_size, num_streams);
    }
  ...
  Chunk generate() override {
    while (true) {
      ...
      if (query_info.trivial_limit > 0 && rows_read >= query_info.trivial_limit) return chunk;
      ...
      if (executor && executor->pull(chunk)) {
        ...
        for (auto &col : columns) col = col->cut(0, remaining_rows);
...
ReadFromLoopStep::ReadFromLoopStep(const Names &column_names_, const SelectQueryInfo &query_info_,
                                   const StorageSnapshotPtr &storage_snapshot_, const ContextPtr &context_,
                                   QueryProcessingStage::Enum processed_stage_, StoragePtr inner_storage_, ASTPtr inner_table_function_ast_,
                                   size_t max_block_size_, size_t num_streams_)
    : SourceStepWithFilter(std::make_shared<const Block>(storage_snapshot_->getSampleBlockForColumns(column_names_)), column_names_,
                           query_info_, storage_snapshot_, disableParallelReplicas(context_)),
      column_names(column_names_),
      processed_stage(processed_stage_),
      inner_storage(std::move(inner_storage_)),
      inner_table_function_ast(std::move(inner_table_function_ast_)),
...
Pipe ReadFromLoopStep::makePipe() {
  return Pipe(std::make_shared<LoopSource>(column_names, query_info, storage_snapshot, context, processed_stage, inner_storage,
