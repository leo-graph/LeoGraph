...
struct MarshallBlocksTransform : ISimpleTransform {
 public:
  explicit MarshallBlocksTransform(SharedHeader header_, BlockMarshallingCallback callback_)
      : ISimpleTransform(header_, header_, false), callback(std::move(callback_)) {}
...
void BlocksMarshallingStep::transformPipeline(QueryPipelineBuilder& pipeline, const BuildQueryPipelineSettings& settings) {
  // The getNumStreams() == 1 is special, because it may indicate that pipeline ended with a sorting or aggregation, i.e. we should preserve
  // chunks order.
  const bool single_stream = pipeline.getNumStreams() == 1;
  if (single_stream) pipeline.addTransform(std::make_shared<AddSequenceNumber>(pipeline.getSharedHeader()));
  const size_t num_threads = pipeline.getNumThreads();
  pipeline.resize(num_threads);
  pipeline.addSimpleTransform(
      [&](const SharedHeader& header) { return std::make_shared<MarshallBlocksTransform>(header, settings.block_marshalling_callback); });
  if (single_stream) pipeline.addTransform(std::make_shared<SortChunksBySequenceNumber>(pipeline.getHeader(), num_threads));
}
