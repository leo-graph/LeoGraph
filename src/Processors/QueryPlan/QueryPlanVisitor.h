...
class QueryPlanVisitor {
  ...
  void visit() {
    ...
    while (!stack.empty()) {
      ...
      if (0 == frame.next_child) {
        ...
        if (!visitTopDown(current_node, parent_node)) continue;
      }
 ...
 private:
  ...
  std::string getStepId(const IQueryPlanStep *step) {
    ...
    if (it != address2name.end()) return it->second;
 ...
 protected:
  void logStep(const char *prefix, const QueryPlan::Node *node) {
    if constexpr (debug_logging) {
      ...
      LOG_DEBUG(getLogger("QueryPlanVisitor"), "{}: {}: {}", prefix, getStepId(current_step), reinterpret_cast<const void *>(current_step));
    }
  }
};
