...
static std::optional<ActionsAndName> trySplitSingleAndFilter(ActionsDAG &dag, const std::string &filter_name) {
  ...
  while (filter->type == ActionsDAG::ActionType::ALIAS) filter = filter->children.at(0);

  if (filter->type != ActionsDAG::ActionType::FUNCTION || filter->function_base->getName() != "and") return {};
  ...
  while (!nodes.empty()) {
    ...
    if (node->type == ActionsDAG::ActionType::FUNCTION && node->function_base->getName() == "and") {
      ...
      for (const auto *child : node->children | std::ranges::views::reverse) nodes.push(child);

      continue;
    }

    if (isTrivialSubtree(node)) continue;
    ...
    if (condition_to_split) return splitSingleAndFilter(dag, condition_to_split);
...
std::vector<ActionsAndName> splitAndChainIntoMultipleFilters(ActionsDAG &dag, const std::string &filter_name) {
  ...
  while (auto condition = trySplitSingleAndFilter(dag, filter_name)) res.push_back(std::move(*condition));

  return res;
}

FilterStep::FilterStep(const SharedHeader &input_header_, ActionsDAG actions_dag_, String filter_column_name_, bool remove_filter_column_)
    : ITransformingStep(input_header_,
                        std::make_shared<const Block>(
                            FilterTransform::transformHeader(*input_header_, &actions_dag_, filter_column_name_, remove_filter_column_)),
                        getTraits()),
...
void FilterStep::transformPipeline(QueryPipelineBuilder &pipeline, const BuildQueryPipelineSettings &settings) {
  ...
  if (!blocksHaveEqualStructure(pipeline.getHeader(), *output_header)) {
    auto convert_actions_dag =
        ActionsDAG::makeConvertingActions(pipeline.getHeader().getColumnsWithTypeAndName(), output_header->getColumnsWithTypeAndName(),
                                          ActionsDAG::MatchColumnsMode::Name, nullptr);
    auto convert_actions = std::make_shared<ExpressionActions>(std::move(convert_actions_dag), settings.getActionsSettings());

    pipeline.addSimpleTransform(
        [&](const SharedHeader &header) { return std::make_shared<ExpressionTransform>(header, convert_actions, dataflow_cache_updater); });
  } else {
...
void FilterStep::describeActions(FormatSettings &settings) const {
  ...
  std::vector<ActionsAndName> and_atoms;
  if (!actions_dag.hasStatefulFunctions()) and_atoms = splitAndChainIntoMultipleFilters(cloned_dag, filter_column_name);
  ...
  if (remove_filter_column) settings.out << " (removed)";
  settings.out << '\n';
...
void FilterStep::updateOutputHeader() {
  output_header = std::make_shared<const Block>(
      FilterTransform::transformHeader(*input_headers.front(), &actions_dag, filter_column_name, remove_filter_column));

  if (!getDataStreamTraits().preserves_sorting) return;
}
...
bool FilterStep::canUseType(const DataTypePtr &filter_type) { return FilterTransform::canUseType(filter_type); }

void FilterStep::serialize(Serialization &ctx) const {
  UInt8 flags = 0;
  if (remove_filter_column) flags |= 1;
...
QueryPlanStepPtr FilterStep::deserialize(Deserialization &ctx) {
  if (ctx.input_headers.size() != 1) throw Exception(ErrorCodes::INCORRECT_DATA, "FilterStep must have one input stream");
  ...
  return std::make_unique<FilterStep>(ctx.input_headers.front(), std::move(actions_dag), std::move(filter_column_name),
...
IQueryPlanStep::RemovedUnusedColumns FilterStep::removeUnusedColumns(NameMultiSet required_outputs, bool remove_inputs) {
  if (output_header == nullptr) throw Exception(ErrorCodes::LOGICAL_ERROR, "Output header is not set in FilterStep");
  ...
  if (prevent_input_removal) remove_inputs = false;
  ...
  const auto build_required_inputs_set = [this, &not_output_names = split_results.not_output_names]() {
    ...
    for (const auto *input_node : actions_dag.getInputs()) required_inputs_set.insert(input_node->result_name);

    for (const auto &pass_through_input : not_output_names) required_inputs_set.insert(pass_through_input);
  ...
  if (has_to_add_input_to_actions) {
    ...
    for (const auto &name_and_type : *input_header)
      if (!required_inputs_set.contains(name_and_type.name)) actions_dag.addInput(name_and_type);
  ...
  if (!updated_actions && output_header->columns() == required_output_count) return RemovedUnusedColumns::None;
  ...
  if (update_inputs) {
    ...
    for (const auto &col_type_and_name : *input_header) {
      if (required_inputs_set.contains(col_type_and_name.name)) new_input_header.insert(col_type_and_name);
    }
...
bool FilterStep::canRemoveColumnsFromOutput() const {
  ...
  if (!remove_filter_column && output_header->columns() == 1) return false;

  return canRemoveUnusedColumns();
}

QueryPlanStepPtr FilterStep::clone() const { return std::make_unique<FilterStep>(*this); }

void registerFilterStep(QueryPlanStepRegistry &registry) { registry.registerStep("Filter", FilterStep::deserialize); }
