...
std::unique_ptr<QueryPlan> createLocalPlan(const ASTPtr& query_ast, const Block& header, ContextPtr context,
                                           QueryProcessingStage::Enum processed_stage, size_t shard_num, size_t shard_count,
                                           bool build_logical_plan, const std::string& default_database) {
  checkStackSize();

  auto query_plan = std::make_unique<QueryPlan>();
  auto new_context = Context::createCopy(context);

  if (build_logical_plan && !default_database.empty()) new_context->setCurrentDatabase(default_database);
