...
Identifier parseTableIdentifier(const std::string &str, const ContextPtr &context) {
  ...
  IParser::Pos pos(tokens, static_cast<unsigned>(settings[Setting::max_parser_depth]),
                   static_cast<unsigned>(settings[Setting::max_parser_backtracks]));
  Expected expected;

  ParserCompoundIdentifier parser(false, false);
  ASTPtr res;
  if (!parser.parse(pos, res, expected)) throw Exception(ErrorCodes::CANNOT_PARSE_TEXT, "Cannot parse itable identifier ({})", str);
...
std::shared_ptr<TableNode> resolveTable(const Identifier &identifier, const ContextPtr &context) {
  ...
  if (!resolve_result) throw Exception(ErrorCodes::UNKNOWN_TABLE, "Unknown table {}", identifier.getFullName());
...
static ASTPtr makeASTForReadingColumns(const Names &names, ASTPtr table_expression) {
  ...
  for (const auto &name : names) columns->children.push_back(make_intrusive<ASTIdentifier>(name));
...
static QueryPlanResourceHolder replaceReadingFromTable(QueryPlan::Node &node, QueryPlan::Nodes &nodes, const ContextPtr &context) {
  ...
  if (!reading_from_table && !reading_from_table_function) return {};
  ...
  if (reading_from_table) {
    ...
    const auto &settings = context->getSettingsRef();
    table_function_ast = parseQuery(parser, serialized_ast, settings[Setting::max_query_size], settings[Setting::max_parser_depth],
    ...
    if (auto *table_function_node = query_tree_node->as<TableFunctionNode>()) {
      ...
      throw Exception(ErrorCodes::INCORRECT_DATA, "Unexpected query tree node type {}\n{}", query_tree_node->getNodeTypeName(),
  ...
  if (storage->isRemote() || is_storage_merge) {
    ...
    if (reading_from_table) use_parallel_replicas = reading_from_table->useParallelReplicas();

    auto mutable_context = Context::createCopy(context);
    mutable_context->setSetting("allow_experimental_parallel_reading_from_replicas", use_parallel_replicas);

    storage->read(reading_plan, column_names, snapshot, select_query_info, mutable_context, QueryProcessingStage::FetchColumns,
                  context->getSettingsRef()[Setting::max_block_size], context->getSettingsRef()[Setting::max_threads]);
  ...
  auto converting_actions =
      ActionsDAG::makeConvertingActions(reading_plan.getCurrentHeader()->getColumnsWithTypeAndName(), header->getColumnsWithTypeAndName(),
...
void QueryPlan::resolveStorages(const ContextPtr &context) {
  ...
  while (!stack.empty()) {
    ...
    if (const auto *delayed_creating_sets = typeid_cast<const DelayedCreatingSetsStep *>(node->step.get())) {
      for (const auto &set : delayed_creating_sets->getSets()) set->getQueryPlan()->resolveStorages(context);
    }

    for (auto *child : node->children) stack.push(child);

    if (node->children.empty()) addResources(replaceReadingFromTable(*node, nodes, context));
  }
}
