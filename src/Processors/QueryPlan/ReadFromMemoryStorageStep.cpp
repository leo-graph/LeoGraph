...
class MemorySource : public ISource {
 ...
 public:
  MemorySource(Names column_names_, const StorageSnapshotPtr &storage_snapshot, std::shared_ptr<const Blocks> data_,
               std::shared_ptr<std::atomic<size_t>> parallel_execution_index_, InitializerFunc initializer_func_ = {})
      : ISource(std::make_shared<const Block>(storage_snapshot->getSampleBlockForColumns(column_names_))),
        requested_column_names_and_types(
    ...
    for (const auto &[name, type] : all_column_names_and_types) all_names_to_types[name] = type;
  }
...
ReadFromMemoryStorageStep::ReadFromMemoryStorageStep(const Names &columns_to_read_, const SelectQueryInfo &query_info_,
                                                     const StorageSnapshotPtr &storage_snapshot_, const ContextPtr &context_,
                                                     StoragePtr storage_, const size_t num_streams_,
                                                     const bool delay_read_for_global_sub_queries_)
    : SourceStepWithFilter(std::make_shared<const Block>(storage_snapshot_->getSampleBlockForColumns(columns_to_read_)), columns_to_read_,
...
QueryPlanStepPtr ReadFromMemoryStorageStep::clone() const { return std::make_unique<ReadFromMemoryStorageStep>(*this); }
...
Pipe ReadFromMemoryStorageStep::makePipe() {
  ...
  if (delay_read_for_global_sub_queries) {
    ...
    return Pipe(std::make_shared<MemorySource>(columns_to_read, storage_snapshot, nullptr /* data */,
  ...
  for (size_t stream = 0; stream < num_streams; ++stream) {
    ...
    if (stream == 0) source->addTotalRowsApprox(snapshot_data.rows_approx);
