...
LimitTransform::LimitTransform(SharedHeader header_, UInt64 limit_, UInt64 offset_, size_t num_streams, bool always_read_till_end_,
                               bool with_ties_, SortDescription description_, RuntimeDataflowStatisticsCacheUpdaterPtr updater_)
  ...
  if (num_streams != 1 && with_ties) throw Exception(ErrorCodes::LOGICAL_ERROR, "Cannot use LimitTransform with multiple ports and ties");
  ...
  for (const auto& desc : description) sort_column_positions.push_back(header_->getPositionByName(desc.column_name));
}
...
IProcessor::Status LimitTransform::prepare(const PortNumbers& updated_input_ports, const PortNumbers& updated_output_ports) {
  ...
  auto process_pair = [&](size_t pos) {
    ...
    switch (status) {
      ...
      case IProcessor::Status::NeedData:
        return;
      default:
        throw Exception(ErrorCodes::LOGICAL_ERROR, "Unexpected status for LimitTransform::preparePair : {}",
                        IProcessor::statusToName(status));
    }
  };

  for (auto pos : updated_input_ports) process_pair(pos);

  for (auto pos : updated_output_ports) process_pair(pos);

  /// All ports are finished. It may happen even before we reached the limit (has less data then limit).
  if (num_finished_port_pairs == ports_data.size()) return Status::Finished;
  ...
  if ((!limit_is_unreachable && rows_read >= offset + limit) && !previous_row_chunk && !always_read_till_end) {
  ...
  if (has_full_port) return Status::PortFull;
...
LimitTransform::Status LimitTransform::preparePair(PortsData& data) {
  ...
  if (rows_before_limit_at_least && !data.input_port_has_counter) rows_before_limit_at_least->add(rows);
  ...
  if (rows <= std::numeric_limits<UInt64>::max() - offset && rows_read >= offset + rows && !limit_is_unreachable &&
      rows_read <= offset + limit) {
  ...
  if (!always_read_till_end && !limit_is_unreachable && rows_read >= offset + limit && !may_need_more_data_for_ties) input.close();

  if (updater) updater->recordOutputChunk(data.current_chunk, output.getHeader());
...
void LimitTransform::splitChunk(PortsData& data) {
  ...
  if (offset + num_rows > rows_read) start = offset + num_rows - rows_read;
  ...
  if (length == num_rows) return;

  auto columns = data.current_chunk.detachColumns();

  for (UInt64 i = 0; i < num_columns; ++i) columns[i] = columns[i]->cut(start, length);
...
ColumnRawPtrs LimitTransform::extractSortColumns(const Columns& columns) const {
  ...
  for (size_t pos : sort_column_positions) res.push_back(columns[pos].get());
...
bool LimitTransform::sortColumnsEqualAt(const ColumnRawPtrs& current_chunk_sort_columns, UInt64 current_chunk_row_num) const {
  ...
  for (size_t i = 0; i < size; ++i)
    if (0 != current_chunk_sort_columns[i]->compareAt(current_chunk_row_num, 0, *previous_row_sort_columns[i], 1)) return false;
  return true;
}
