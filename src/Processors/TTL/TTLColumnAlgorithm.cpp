...
TTLColumnAlgorithm::TTLColumnAlgorithm(const TTLExpressions& ttl_expressions_, const TTLDescription& description_,
                                       const TTLInfo& old_ttl_info_, time_t current_time_, bool force_, const String& column_name_,
                                       const ExpressionActionsPtr& default_expression_, const String& default_column_name_,
...
void TTLColumnAlgorithm::execute(Block& block) {
  ...
  if (!block.has(column_name)) return;

  /// Nothing to do
  if (!isMinTTLExpired()) return;
  ...
  if (default_column) default_column = default_column->convertToFullColumnIfConst();
...
void TTLColumnAlgorithm::finalize(const MutableDataPartPtr& data_part) const {
  ...
  if (is_fully_empty) data_part->expired_columns.insert(column_name);
}
