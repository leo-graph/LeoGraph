...
TTLAggregationAlgorithm::TTLAggregationAlgorithm(const TTLExpressions& ttl_expressions_, const TTLDescription& description_,
                                                 const TTLInfo& old_ttl_info_, time_t current_time_, bool force_, const Block& header_,
                                                 const MergeTreeData& storage_)
    : ITTLAlgorithm(ttl_expressions_, description_, old_ttl_info_, current_time_, force_), header(header_) {
  ...
  Aggregator::Params params(
      keys, aggregates,
      /*overflow_row_=*/false, settings[Setting::max_rows_to_group_by], settings[Setting::group_by_overflow_mode],
      ...
      settings[Setting::empty_result_for_aggregation_by_empty_set], storage_.getContext()->getTempDataOnDisk(),
      settings[Setting::max_threads], settings[Setting::min_free_disk_space_for_temporary_data],
      ...
      /*stats_collecting_params_=*/{}, settings[Setting::enable_producing_buckets_out_of_order_in_aggregation],
      settings[Setting::serialize_string_in_memory_with_zero_byte]);

  aggregator = std::make_unique<Aggregator>(header, params);

  if (isMaxTTLExpired()) new_ttl_info.ttl_finished = true;
}
...
void TTLAggregationAlgorithm::execute(Block& block) {
  ...
  if (some_rows_were_aggregated) {
    ...
    for (size_t i = 0; i < block.rows(); ++i) {
      ...
      if (where_filter_passed) new_ttl_info.update(getTimestampByIndex(ttl_column_after_aggregation.get(), i));
    }
  }
}
...
void TTLAggregationAlgorithm::calculateAggregates(const MutableColumns& aggregate_columns, size_t start_pos, size_t length) {
  ...
  aggregator->executeOnBlock(aggregate_chunk, /* row_begin= */ 0, length, aggregation_result, key_columns, columns_for_aggregator,
                             no_more_keys);
}
      ...
      for (const auto& it : description.set_parts) {
        ...
        if (result_column_type->lowCardinality()) {
          ...
          if (!column_with_type.type->lowCardinality()) {
            ...
            column_with_type.column = recursiveLowCardinalityTypeConversion(column_with_type.column, nested_type, result_column_type);
