...
TTLDeleteAlgorithm::TTLDeleteAlgorithm(const TTLExpressions& ttl_expressions_, const TTLDescription& description_,
                                       const TTLInfo& old_ttl_info_, time_t current_time_, bool force_)
    : ITTLAlgorithm(ttl_expressions_, description_, old_ttl_info_, current_time_, force_) {
  if (!isMinTTLExpired()) new_ttl_info = old_ttl_info;

  if (isMaxTTLExpired()) new_ttl_info.ttl_finished = true;
}

void TTLDeleteAlgorithm::execute(Block& block) {
  if (block.empty() || !isMinTTLExpired()) return;
  ...
  for (auto it = column_names.begin(); it != column_names.end(); ++it) {
    ...
    for (size_t i = 0; i < block.rows(); ++i) {
      ...
      if (!isTTLExpired(cur_ttl) || !where_filter_passed) {
        ...
        if (where_filter_passed) new_ttl_info.update(cur_ttl);
