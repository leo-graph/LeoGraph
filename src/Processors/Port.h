...
class Port {
 ...
 public:
  struct UpdateInfo {
    ...
    void inline ALWAYS_INLINE update() {
      if (version == prev_version && update_list) update_list->push_back(id);
  ...
  class State {
   ...
   public:
    ...
    class DataPtr {
     public:
      DataPtr() : data(new Data()) {
        if (unlikely((getUInt(data) & FLAGS_MASK) != 0)) throw Exception(ErrorCodes::LOGICAL_ERROR, "Not alignment memory for Port");
      }
    ...
    void ALWAYS_INLINE push(DataPtr &data_, std::uintptr_t &flags) {
      ...
      if (unlikely(flags & HAS_DATA)) throw Exception(ErrorCodes::LOGICAL_ERROR, "Cannot push block to port which already has data");
    }

    void ALWAYS_INLINE pull(DataPtr &data_, std::uintptr_t &flags, bool set_not_needed = false) {
      uintptr_t mask = HAS_DATA;

      if (set_not_needed) mask |= IS_NEEDED;
      ...
      if (unlikely((flags & HAS_DATA) == 0)) throw Exception(ErrorCodes::LOGICAL_ERROR, "Cannot pull block from port which has no data");
    }
    ...
    std::uintptr_t ALWAYS_INLINE getFlags() const { return getUInt(data.load()) & FLAGS_MASK; }
 ...
 public:
  ...
  void ALWAYS_INLINE assumeConnected() const {
    if (unlikely(!isConnected())) throw Exception(ErrorCodes::LOGICAL_ERROR, "Port is not connected");
  }
  ...
  IProcessor &getProcessor() {
    if (!processor) throw Exception(ErrorCodes::LOGICAL_ERROR, "Port does not belong to Processor");
    return *processor;
  }
 ...
 protected:
  void inline ALWAYS_INLINE updateVersion() {
    if (likely(update_info)) update_info->update();
  }
...
class InputPort : public Port {
 ...
 public:
  ...
  Data ALWAYS_INLINE pullData(bool set_not_needed = false) {
    if (!set_not_needed) updateVersion();
    ...
    if (unlikely(!data->exception && data->chunk.getNumColumns() != header->columns())) {
      ...
      throw Exception(ErrorCodes::LOGICAL_ERROR,
                      ...
                      "Header: {}\n"
                      "Chunk: {}\n",
                      header->columns(), chunk.getNumColumns(), header->dumpStructure(), chunk.dumpStructure());
    }
  ...
  Chunk ALWAYS_INLINE pull(bool set_not_needed = false) {
    ...
    if (pull_data.exception) std::rethrow_exception(pull_data.exception);
  ...
  void ALWAYS_INLINE setNeeded() {
    ...
    if ((state->setFlags(State::IS_NEEDED, State::IS_NEEDED) & State::IS_NEEDED) == 0) updateVersion();
  }
  ...
  void ALWAYS_INLINE close() {
    ...
    if ((state->setFlags(State::IS_FINISHED, State::IS_FINISHED) & State::IS_FINISHED) == 0) updateVersion();
...
class OutputPort : public Port {
 ...
 public:
  ...
  void ALWAYS_INLINE push(Chunk chunk) { pushData({.chunk = std::move(chunk), .exception = {}}); }

  void ALWAYS_INLINE pushException(std::exception_ptr exception) { pushData({.chunk = {}, .exception = exception}); }
  ...
  void ALWAYS_INLINE pushData(Data data_) {
    if (unlikely(!data_.exception && data_.chunk.getNumColumns() != header->columns())) {
      throw Exception(ErrorCodes::LOGICAL_ERROR,
                      ...
                      "Header: {}\n"
                      "Chunk: {}\n",
                      header->columns(), data_.chunk.getNumColumns(), header->dumpStructure(), data_.chunk.dumpStructure());
    }
  ...
  void ALWAYS_INLINE finish() {
    ...
    if ((flags & State::IS_FINISHED) == 0) updateVersion();
  }
