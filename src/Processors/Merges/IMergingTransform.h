...
class IMergingTransformBase : public IProcessor {
 public:
  IMergingTransformBase(size_t num_inputs, SharedHeader& input_header, SharedHeader& output_header, bool have_all_inputs_,
                        UInt64 limit_hint_, bool always_read_till_end_);

  IMergingTransformBase(SharedHeaders& input_headers, SharedHeader& output_header, bool have_all_inputs_, UInt64 limit_hint_,
...
class IMergingTransform : public IMergingTransformBase {
 public:
  ...
  IMergingTransform(size_t num_inputs, SharedHeader input_header, SharedHeader output_header, bool have_all_inputs_, UInt64 limit_hint_,
                    bool always_read_till_end_, Args&&... args)
      : IMergingTransformBase(num_inputs, input_header, output_header, have_all_inputs_, limit_hint_, always_read_till_end_),
        algorithm(std::forward<Args>(args)...) {}

  template <typename... Args>
  IMergingTransform(SharedHeaders input_headers, SharedHeader output_header, bool have_all_inputs_, UInt64 limit_hint_,
                    bool always_read_till_end_, bool empty_chunk_on_finish_, Args&&... args)
  ...
  void work() override {
    ...
    if (!state.init_chunks.empty()) algorithm.initialize(std::move(state.init_chunks));
 ...
 protected:
  ...
  void logMergedStats(ProfileEvents::Event elapsed_ms_event, std::string_view transform_message, LoggerPtr log) const {
    ...
    if (stats.rows < 1000000) return;

    double seconds = static_cast<double>(merging_elapsed_ns) / 1000000000ULL;

    if (seconds == 0.0) {
      LOG_DEBUG(log, "{}, {} blocks, {} rows, {} bytes in 0 sec.", transform_message, stats.blocks, stats.rows, stats.bytes);
    } else {
      LOG_DEBUG(log, "{}, {} blocks, {} rows, {} bytes in {} sec., {} rows/sec., {}/sec.", transform_message, stats.blocks, stats.rows,
                stats.bytes, seconds, static_cast<double>(stats.rows) / seconds, ReadableSize(static_cast<double>(stats.bytes) / seconds));
    }
  }
