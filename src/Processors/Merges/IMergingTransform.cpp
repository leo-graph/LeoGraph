...
IMergingTransformBase::IMergingTransformBase(size_t num_inputs, SharedHeader& input_header, SharedHeader& output_header,
...
OutputPort& IMergingTransformBase::getOutputPort() { return outputs.front(); }

static InputPorts createPorts(const SharedHeaders& blocks) {
  InputPorts ports;
  for (const auto& block : blocks) ports.emplace_back(block);
  return ports;
}

IMergingTransformBase::IMergingTransformBase(SharedHeaders& input_headers, SharedHeader& output_header, bool have_all_inputs_,
                                             UInt64 limit_hint_, bool always_read_till_end_)
...
void IMergingTransformBase::addInput() {
  if (have_all_inputs) throw Exception(ErrorCodes::LOGICAL_ERROR, "IMergingTransform already have all inputs.");
...
IProcessor::Status IMergingTransformBase::prepareInitializeInputs() {
  ...
  if (input_states.empty()) {
    ...
    for (auto& input : inputs) input_states.emplace_back(input);
  ...
  for (size_t i = 0; it != inputs.end(); ++i, ++it) {
    auto& input = *it;
    if (input.isFinished()) continue;

    if (input_states[i].is_initialized) continue;
    ...
    if (limit_hint == 0 && !virtual_row) input.setNeeded();

    if (!virtual_row && ((limit_hint && chunk.getNumRows() < limit_hint) || always_read_till_end)) input.setNeeded();
  ...
  if (!all_inputs_has_data) return Status::NeedData;
...
IProcessor::Status IMergingTransformBase::prepare() {
  if (!have_all_inputs) return Status::NeedData;
  ...
  /// Push if has data.
  if ((state.output_chunk || !state.output_chunk.getChunkInfos().empty()) && !is_port_full) output.push(std::move(state.output_chunk));

  if (!is_initialized) return prepareInitializeInputs();

  if (state.is_finished) {
    if (is_port_full) return Status::PortFull;

    if (always_read_till_end) {
      for (auto& input : inputs) {
        if (!input.isFinished()) {
          input.setNeeded();
          if (input.hasData()) std::ignore = input.pull();
  ...
  if (state.need_data) {
    ...
    if (!input.isFinished()) {
      ...
      if (!state.input_chunk.chunk.hasRows() && !input.isFinished()) return Status::NeedData;
