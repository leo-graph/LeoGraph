...
ChunkSelectFinalIndices::ChunkSelectFinalIndices(MutableColumnPtr select_final_indices_)
    : column_holder(std::move(select_final_indices_)), select_final_indices(typeid_cast<const ColumnUInt64 *>(column_holder.get())) {
  if (!select_final_indices) throw Exception(ErrorCodes::LOGICAL_ERROR, "Column passed to 'ChunkSelectFinalIndices' must be ColumnUInt64");
}

ReplacingSortedAlgorithm::ReplacingSortedAlgorithm(SharedHeader header_, size_t num_inputs, SortDescription description_,
                                                   const String &is_deleted_column, const String &version_column,
                                                   size_t max_block_size_rows, size_t max_block_size_bytes,
                                                   std::optional<size_t> max_dynamic_subcolumns_, WriteBuffer *out_row_sources_buf_,
                                                   bool use_average_block_sizes, bool cleanup_, bool enable_vertical_final_)
    : IMergingAlgorithmWithSharedChunks(
          header_, num_inputs, std::move(description_), out_row_sources_buf_, max_row_refs,
          std::make_unique<MergedData>(use_average_block_sizes, max_block_size_rows, max_block_size_bytes, max_dynamic_subcolumns_)),
      cleanup(cleanup_),
      enable_vertical_final(enable_vertical_final_) {
  if (!is_deleted_column.empty()) is_deleted_column_number = header_->getPositionByName(is_deleted_column);

  if (!version_column.empty()) version_column_number = header_->getPositionByName(version_column);
}
...
void ReplacingSortedAlgorithm::insertRowImpl() {
  ...
  if (enable_vertical_final) {
    ...
    if (!selected_row.owned_chunk->replace_final_selection) selected_row.owned_chunk->replace_final_selection = ColumnUInt64::create();
    ...
    if (selected_row.current_cursor == nullptr) to_be_emitted.push(std::move(selected_row.owned_chunk));
  } else {
...
IMergingAlgorithm::Status ReplacingSortedAlgorithm::merge() {
  ...
  while (queue.isValid()) {
    ...
    if (key_differs) {
      ...
      if (merged_data->hasEnoughRows()) return Status(merged_data->pull());

      /// Write the data for the previous primary key.
      if (!selected_row.empty()) insertRow();

      selected_row.clear();
    }

    if (current->isFirst() && key_differs && is_deleted_column_number == -1  /// Ignore optimization if we need to filter deleted rows.
      ...
      if (merged_data->mergedRows() != 0) return Status(merged_data->pull());
      ...
      if (out_row_sources_buf) {
        ...
        for (size_t i = 0; i < chunk_num_rows; ++i) out_row_sources_buf->write(row_source.data);
      }
    ...
    if (out_row_sources_buf) current_row_sources.emplace_back(current.impl->order, true);

    if (is_deleted_column_number != -1) {
      const UInt8 is_deleted =
          assert_cast<const ColumnUInt8 &>(*current->all_columns[is_deleted_column_number]).getData()[current->getRow()];
      if (is_deleted > 1)
    ...
    if (version_column_number == -1 || selected_row.empty() ||
        current->all_columns[version_column_number]->compareAt(current->getRow(), selected_row.row_num,
...
void ReplacingSortedAlgorithm::saveChunkForSkippingFinalFromSource(size_t current_source_index) {
  if (enable_vertical_final) {
    ...
    if (selected_row.owned_chunk.get() == chunk.get()) {
      ...
      /// Later when we change the value of `selected_row`, if `selected_row` is the sole owner of its chunk and the chunk has selected
      /// rows, we will emit it
      selected_row.current_cursor = nullptr;
    } else {
      /// Otherwise, its safe to emit the chunk
      if (chunk->replace_final_selection) to_be_emitted.push(std::move(chunk));
    }
  }
}
