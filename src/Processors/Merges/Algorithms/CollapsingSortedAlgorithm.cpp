...
CollapsingSortedAlgorithm::CollapsingSortedAlgorithm(SharedHeader header_, size_t num_inputs, SortDescription description_,
                                                     const String &sign_column, bool only_positive_sign_, size_t max_block_size_rows_,
                                                     size_t max_block_size_bytes_, std::optional<size_t> max_dynamic_subcolumns_,
                                                     LoggerPtr log_, WriteBuffer *out_row_sources_buf_, bool use_average_block_sizes,
...
std::optional<Chunk> CollapsingSortedAlgorithm::insertRows() {
  ...
  if ((last_is_positive || count_positive != count_negative) && (count_positive > 0 || count_negative > 0)) {
    if (count_positive <= count_negative && !only_positive_sign) {
      ...
      if (out_row_sources_buf) current_row_sources[first_negative_pos].setSkipFlag(false);
    }

    if (count_positive >= count_negative) {
      if (merged_data->hasEnoughRows()) res = merged_data->pull();

      insertRow(last_positive_row);

      if (out_row_sources_buf) current_row_sources[last_positive_pos].setSkipFlag(false);
    }

    if (!(count_positive == count_negative || count_positive + 1 == count_negative || count_positive == count_negative + 1)) {
      if (count_incorrect_data < MAX_ERROR_MESSAGES) reportIncorrectData();
...
IMergingAlgorithm::Status CollapsingSortedAlgorithm::merge() {
  ...
  while (queue.isValid()) {
    ...
    if (last_row.empty()) setRowRef(last_row, current);
    ...
    if (key_differs) {
      ...
      if (res) return Status(std::move(*res));
    }
    ...
    if (sign == 1) {
      ...
      if (!only_positive_sign) {
        ...
        ++count_invalid;
        if (out_row_sources_buf) current_row_sources[current_pos].setSkipFlag(false);
      }

      if (count_invalid_sign < MAX_ERROR_MESSAGES) LOG_WARNING(log, "Incorrect data: Sign = {} (must be 1 or -1).", toString(sign));
      ++count_invalid_sign;
    } else
