 ...
 * corresponding to a one value of the primary key. For columns that are not part of the primary key and which do not have the
 * AggregateFunction type, when merged, the first value is selected.
 */
class AggregatingSortedAlgorithm final : public IMergingAlgorithmWithDelayedChunk {
 public:
  AggregatingSortedAlgorithm(SharedHeader header, size_t num_inputs, SortDescription description_, size_t max_block_size_rows_,
                             size_t max_block_size_bytes_, std::optional<size_t> max_dynamic_subcolumns_);
  ...
  struct SimpleAggregateDescription {
    ...
    SimpleAggregateDescription(AggregateFunctionPtr function_, size_t column_number_, DataTypePtr nested_type_, DataTypePtr real_type_);
 ...
 private:
  ...
  struct AggregatingMergedData : public MergedData {
   ...
   public:
    AggregatingMergedData(UInt64 max_block_size_rows_, UInt64 max_block_size_bytes_, std::optional<size_t> max_dynamic_subcolumns_,
