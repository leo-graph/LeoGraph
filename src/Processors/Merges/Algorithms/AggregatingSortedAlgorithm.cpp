...
static AggregatingSortedAlgorithm::ColumnsDefinition defineColumns(const Block &header, const SortDescription &description) {
  ...
  for (size_t i = 0; i < num_columns; ++i) {
    ...
    for (; it != description.end(); ++it)
      if (it->column_name == column.name) break;
    ...
    if (const auto *simple = dynamic_cast<const DataTypeCustomSimpleAggregateFunction *>(column.type->getCustomName())) {
      ...
      if (type.get() == column.type.get()) type = nullptr;
...
static void preprocessChunk(Chunk &chunk, const AggregatingSortedAlgorithm::ColumnsDefinition &def) {
  ...
  for (const auto &desc : def.columns_to_simple_aggregate)
    if (desc.nested_type) columns[desc.column_number] = recursiveRemoveLowCardinality(columns[desc.column_number]);
...
AggregatingSortedAlgorithm::SimpleAggregateDescription::SimpleAggregateDescription(AggregateFunctionPtr function_, size_t column_number_,
                                                                                   DataTypePtr nested_type_, DataTypePtr real_type_)
    : function(std::move(function_)),
      column_number(column_number_),
      nested_type(std::move(nested_type_)),
      real_type(std::move(real_type_)) {
...
AggregatingSortedAlgorithm::SimpleAggregateDescription::~SimpleAggregateDescription() { destroyState(); }

AggregatingSortedAlgorithm::AggregatingMergedData::AggregatingMergedData(UInt64 max_block_size_rows_, UInt64 max_block_size_bytes_,
...
void AggregatingSortedAlgorithm::AggregatingMergedData::startGroup(const ColumnRawPtrs &raw_columns, size_t row) {
  ...
  for (auto column_number : def.column_numbers_not_to_aggregate) columns[column_number]->insertFrom(*raw_columns[column_number], row);

  /// Add the empty aggregation state to the aggregate columns. The state will be updated in the `addRow` function.
  for (auto &column_to_aggregate : def.columns_to_aggregate) column_to_aggregate.column->insertDefault();

  /// Reset simple aggregation states for next row
  for (auto &desc : def.columns_to_simple_aggregate) desc.createState();
...
void AggregatingSortedAlgorithm::AggregatingMergedData::addRow(SortCursor &cursor) {
  if (!is_group_started) throw Exception(ErrorCodes::LOGICAL_ERROR, "Can't add a row to the group because it was not started.");

  for (auto &desc : def.columns_to_aggregate) desc.column->insertMergeFrom(*cursor->all_columns[desc.column_number], cursor->getRow());
...
Chunk AggregatingSortedAlgorithm::AggregatingMergedData::pull() {
  if (is_group_started) throw Exception(ErrorCodes::LOGICAL_ERROR, "Can't pull chunk because group was not finished.");
...
void AggregatingSortedAlgorithm::AggregatingMergedData::initAggregateDescription() {
  for (auto &desc : def.columns_to_simple_aggregate) desc.column = columns[desc.column_number].get();

  for (auto &desc : def.columns_to_aggregate) desc.column = typeid_cast<ColumnAggregateFunction *>(columns[desc.column_number].get());
}

AggregatingSortedAlgorithm::AggregatingSortedAlgorithm(SharedHeader header_, size_t num_inputs, SortDescription description_,
