...
void IMergingAlgorithmWithDelayedChunk::initializeQueue(Inputs inputs) {
  ...
  for (size_t source_num = 0; source_num < current_inputs.size(); ++source_num) {
    if (!current_inputs[source_num].chunk) continue;

    cursors[source_num] =
        SortCursorImpl(*header, current_inputs[source_num].chunk.getColumns(), current_inputs[source_num].chunk.getNumRows(), description,
                       source_num, current_inputs[source_num].permutation);
