...
class SharedChunkAllocator {
 public:
  explicit SharedChunkAllocator(size_t max_chunks) {
    ...
    for (size_t i = 0; i < max_chunks; ++i) free_chunks.push_back(i);
  }
...
inline void intrusive_ptr_add_ref(SharedChunk* ptr) { ++ptr->refcount; }

inline void intrusive_ptr_release(SharedChunk* ptr) {
  if (0 == --ptr->refcount) ptr->allocator->release(ptr);
}
...
struct RowRef {
  ...
  static size_t checkEqualsFirstNonEqual(size_t size, size_t offset, const IColumn** lhs, size_t lhs_row, const IColumn** rhs,
                                         size_t rhs_row);
...
struct RowRefWithOwnedChunk {
  ...
  size_t firstNonEqualSortColumnsWith(size_t offset, const RowRefWithOwnedChunk& other) const {
    return RowRef::checkEqualsFirstNonEqual(sort_columns->size(), offset, sort_columns->data(), row_num, other.sort_columns->data(),
                                            other.row_num);
  }

  bool hasEqualSortColumnsWith(const RowRefWithOwnedChunk& other) const {
    return RowRef::checkEquals(sort_columns->size(), sort_columns->data(), row_num, other.sort_columns->data(), other.row_num);
  }
};
