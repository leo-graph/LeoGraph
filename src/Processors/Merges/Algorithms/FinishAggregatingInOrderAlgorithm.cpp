...
FinishAggregatingInOrderAlgorithm::State::State(const Chunk& chunk, const SortDescriptionWithPositions& desc, Int64 total_bytes_)
  ...
  for (const auto& column_desc : desc) sorting_columns.emplace_back(all_columns[column_desc.column_number].get());
}

FinishAggregatingInOrderAlgorithm::FinishAggregatingInOrderAlgorithm(SharedHeader header_, size_t num_inputs_,
                                                                     AggregatingTransformParamsPtr params_,
                                                                     const SortDescription& description_, size_t max_block_size_rows_,
...
void FinishAggregatingInOrderAlgorithm::initialize(Inputs inputs) {
  ...
  for (size_t i = 0; i < num_inputs; ++i) consume(current_inputs[i], i);
}
...
void FinishAggregatingInOrderAlgorithm::consume(Input& input, size_t source_num) {
  ...
  if (auto arenas_info = input.chunk.getChunkInfos().get<ChunkInfoWithAllocatedBytes>()) allocated_bytes = arenas_info->allocated_bytes;
...
IMergingAlgorithm::Status FinishAggregatingInOrderAlgorithm::merge() {
  ...
  for (size_t i = 0; i < num_inputs; ++i) {
    if (!states[i].isValid()) continue;

    if (!best_input || less(states[i].sorting_columns, states[*best_input].sorting_columns, states[i].num_rows - 1,
                            states[*best_input].num_rows - 1, description)) {
      best_input = i;
    }
  }

  if (!best_input) return Status(prepareToMerge(), true);
  ...
  for (size_t i = 0; i < num_inputs; ++i) {
    if (!states[i].isValid() || i == *best_input) continue;

    auto indices = collections::range(states[i].current_row, states[i].num_rows);
    auto it = std::upper_bound(indices.begin(), indices.end(), best_state.num_rows - 1, [&](size_t lhs_pos, size_t rhs_pos) {
  ...
  if (accumulated_rows >= max_block_size_rows || accumulated_bytes >= max_block_size_bytes) status.chunk = prepareToMerge();
...
void FinishAggregatingInOrderAlgorithm::addToAggregation() {
  for (size_t i = 0; i < num_inputs; ++i) {
    ...
    if (!state.isValid() || state.current_row == state.to_row) continue;
    ...
    if (current_rows == state.num_rows) {
      ...
      for (const auto& column : state.all_columns) new_columns.emplace_back(column->cut(state.current_row, current_rows));
    ...
    accumulated_bytes += static_cast<size_t>(static_cast<double>(states[i].total_bytes) * static_cast<double>(current_rows) /
                                             static_cast<double>(states[i].num_rows));
    accumulated_rows += current_rows;

    if (!states[i].isValid()) inputs_to_update.push_back(i);
  }
}
