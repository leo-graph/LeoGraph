...
static GraphiteRollupSortedAlgorithm::ColumnsDefinition defineColumns(const Block &header, const Graphite::Params &params) {
  ...
  for (size_t i = 0; i < num_columns; ++i)
    if (i != def.time_column_num && i != def.value_column_num && i != def.version_column_num) def.unmodified_column_numbers.push_back(i);
...
GraphiteRollupSortedAlgorithm::GraphiteRollupSortedAlgorithm(SharedHeader header_, size_t num_inputs, SortDescription description_,
                                                             size_t max_block_size_rows_, size_t max_block_size_bytes_,
                                                             std::optional<size_t> max_dynamic_subcolumns_, Graphite::Params params_,
                                                             time_t time_of_merge_)
    : IMergingAlgorithmWithSharedChunks(
          header_, num_inputs, std::move(description_), nullptr, max_row_refs,
          std::make_unique<GraphiteRollupMergedData>(false, max_block_size_rows_, max_block_size_bytes_, max_dynamic_subcolumns_)),
...
GraphiteRollupSortedAlgorithm::~GraphiteRollupSortedAlgorithm() { merged_data.reset(); }
...
UInt32 GraphiteRollupSortedAlgorithm::selectPrecision(const Graphite::Retentions &retentions, time_t time) const {
  ...
  for (const auto &retention : retentions) {
    if (time_of_merge - time >= static_cast<time_t>(retention.age)) return retention.precision;
  }
...
IMergingAlgorithm::Status GraphiteRollupSortedAlgorithm::merge() {
  ...
  while (queue.isValid()) {
    ...
    if (is_new_key) {
      ...
      if (graphite_rollup_merged_data.wasGroupStarted()) accumulateRow(current_subgroup_newest_row);

      Graphite::RollupRule next_rule = graphite_rollup_merged_data.currentRule();
      if (new_path) next_rule = selectPatternForPath(this->params, next_path);
      ...
      if (will_be_new_key) {
        if (graphite_rollup_merged_data.wasGroupStarted()) {
          ...
          if (graphite_rollup_merged_data.hasEnoughRows()) return Status(graphite_rollup_merged_data.pull());
        }
    ...
    if (is_new_key || current->all_columns[columns_definition.version_column_num]->compareAt(
...
void GraphiteRollupSortedAlgorithm::accumulateRow(RowRef &row) { graphite_rollup_merged_data.accumulateRow(row, columns_definition); }

void GraphiteRollupSortedAlgorithm::GraphiteRollupMergedData::startNextGroup(const ColumnRawPtrs &raw_columns, size_t row,
  ...
  for (size_t j : def.unmodified_column_numbers) columns[j]->insertFrom(*raw_columns[j], row);
...
void GraphiteRollupSortedAlgorithm::GraphiteRollupMergedData::insertRow(time_t time, RowRef &row, ColumnsDefinition &def) {
...
GraphiteRollupSortedAlgorithm::GraphiteRollupMergedData::~GraphiteRollupMergedData() {
  const Graphite::AggregationPattern *aggregation_pattern = std::get<1>(current_rule);
  if (aggregate_state_created && aggregation_pattern) aggregation_pattern->function->destroy(place_for_aggregate_state.data());
}
