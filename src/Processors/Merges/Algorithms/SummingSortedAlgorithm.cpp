...
struct SummingSortedAlgorithm::AggregateDescription {
  ...
  void destroyState() {
    if (!created) return;
    if (!is_agg_func_type) function->destroy(state.data());
    created = false;
  }

  /// Explicitly destroy aggregation state if the stream is terminated
  ~AggregateDescription() { destroyState(); }
...
static bool isInSortingKey(const SortDescription &description, const std::string &name) {
  for (const auto &desc : description)
    if (desc.column_name == name) return true;
...
static bool mergeMap(const SummingSortedAlgorithm::MapDescription &desc, Row &row, std::vector<ColumnPtr> &row_columns,
                     const ColumnRawPtrs &raw_columns, size_t row_number) {
  /// Strongly non-optimal.
  Row &left = row;
  Row right(left.size());

  for (size_t col_num : desc.key_col_nums) {
    if (row_columns[col_num]) left[col_num] = (*row_columns[col_num])[0];
  ...
  auto at_ith_column_jth_row = [&](const Row &matrix, size_t i, size_t j) -> const Field & { return matrix[i].safeGet<Array>()[j]; };
  ...
  auto accumulate = [](Array &dst, const Array &src) {
    ...
    for (size_t i = 0; i < size; ++i)
      if (applyVisitor(FieldVisitorSum(src[i]), dst[i])) has_non_zero = true;
    return has_non_zero;
  };
  ...
  auto merge = [&](const Row &matrix) {
    ...
    for (size_t j = 0; j < rows; ++j) {
      ...
      if (merged.end() == it)
        ...
        if (!accumulate(it->second, value)) merged.erase(it);
      }
    }
  };
  ...
  for (size_t col_num : desc.key_col_nums) row[col_num] = Array(merged.size());
  for (size_t col_num : desc.val_col_nums) row[col_num] = Array(merged.size());
  ...
  for (size_t col_num : desc.key_col_nums) update_column_value_in_row(col_num, left[col_num]);
  for (size_t col_num : desc.val_col_nums) update_column_value_in_row(col_num, left[col_num]);

  return row_num != 0;
}

static SummingSortedAlgorithm::ColumnsDefinition defineColumns(const Block &header, const SortDescription &description,
                                                               const Names &column_names_to_sum,
                                                               const Names &partition_and_sorting_required_columns,
                                                               const String &sum_function_name, const String &sum_function_map_name,
                                                               bool remove_default_values, bool aggregate_all_columns) {
  ...
  for (size_t i = 0; i < num_columns; ++i) {
    ...
    bool is_non_empty_tuple = typeid_cast<const DataTypeTuple *>(column.type.get()) &&
                              !typeid_cast<const DataTypeTuple *>(column.type.get())->getElements().empty();
    if (aggregate_all_columns && (is_non_empty_tuple || typeid_cast<const DataTypeArray *>(column.type.get())) && !simple) {
      ...
      if (map_name == column.name || !endsWith(map_name, "Map")) {
        ...
        if (desc.real_type.get() == desc.nested_type.get()) desc.nested_type = nullptr;

        if (simple) {
          // simple aggregate function
          desc.init(simple->getFunction(), true);
          if (desc.function->allocatesMemoryInArena()) def.allocates_memory_in_arena = true;
        } else if (!is_agg_func) {
  ...
  for (const auto &map : discovered_maps) {
    ...
    if (map.second.size() < 2) {
      for (auto col : map.second) def.column_numbers_not_to_aggregate.push_back(col);
      continue;
    }
    ...
    for (; column_num_it != map.second.end(); ++column_num_it) {
      ...
      if (column_num_it == map.second.begin() || endsWith(name, "ID") || endsWith(name, "Key") || endsWith(name, "Type")) {
        if (!nested_type.isValueRepresentedByInteger() && !isStringOrFixedString(nested_type) &&
            !typeid_cast<const DataTypeIPv6 *>(&nested_type) && !typeid_cast<const DataTypeUUID *>(&nested_type))
          break;

        map_desc.key_col_nums.push_back(*column_num_it);
      } else {
        if (!nested_type.isSummable()) break;
...
static void postprocessChunk(Chunk &chunk, size_t num_result_columns, const SummingSortedAlgorithm::ColumnsDefinition &def) {
...
static void setRow(Row &row, std::vector<ColumnPtr> &row_columns, const ColumnRawPtrs &raw_columns, size_t row_num,
  ...
  const auto handle_exception = [&](const char *logger_name, const char *reason, const size_t column_index) {
    ...
    String column_name;
    if (column_index < column_names.size()) column_name = column_names[column_index];

    throw Exception(ErrorCodes::CORRUPTED_DATA, "SummingSortedAlgorithm failed to read row {} of column {}{}, reason: {}", row_num,
                    column_index, column_name.empty() ? "" : fmt::format(" ({})", column_name), reason);
  };
  ...
  for (size_t i = 0; i < num_columns; ++i) {
    try {
      ...
      if (e.code() == ErrorCodes::MEMORY_LIMIT_EXCEEDED) throw;
...
SummingSortedAlgorithm::SummingMergedData::SummingMergedData(UInt64 max_block_size_rows_, UInt64 max_block_size_bytes_,
                                                             std::optional<size_t> max_dynamic_subcolumns_, ColumnsDefinition &def_)
    : MergedData(false, max_block_size_rows_, max_block_size_bytes_, max_dynamic_subcolumns_),
      def(def_),
      current_row(def.column_names.size()),
      current_row_columns(def.column_names.size()) {}
...
void SummingSortedAlgorithm::SummingMergedData::initialize(const DB::Block &header, const IMergingAlgorithm::Inputs &inputs) {
  ...
  for (const auto &desc : def.columns_to_aggregate) {
    ...
    if (!desc.aggregate_all_columns && !desc.is_agg_func_type && !desc.is_simple_agg_func_type && isTuple(desc.function->getResultType())) {
      ...
      for (size_t i = 0; i < tuple_size; ++i) tuple_columns[i] = std::move(columns[desc.column_numbers[i]]);
  ...
  for (const auto &column_number : def.column_numbers_not_to_aggregate) new_columns.emplace_back(std::move(columns[column_number]));
...
void SummingSortedAlgorithm::SummingMergedData::startGroup(ColumnRawPtrs &raw_columns, size_t row) {
  ...
  for (auto &desc : def.columns_to_aggregate) desc.createState();
...
void SummingSortedAlgorithm::SummingMergedData::finishGroup() {
  ...
  if (def.columns_to_aggregate.empty()) current_row_is_zero = false;
  ...
  if (current_row_is_zero) {
    for (auto &desc : def.columns_to_aggregate) desc.merged_column->popBack(1);
...
void SummingSortedAlgorithm::SummingMergedData::addRow(ColumnRawPtrs &raw_columns, size_t row) {
  ...
  for (const auto &desc : def.maps_to_sum)
    if (mergeMap(desc, current_row, current_row_columns, raw_columns, row)) current_row_is_zero = false;
...
void SummingSortedAlgorithm::SummingMergedData::addRowImpl(ColumnRawPtrs &raw_columns, size_t row) {
  for (auto &desc : def.columns_to_aggregate) {
    if (!desc.created) throw Exception(ErrorCodes::LOGICAL_ERROR, "Logical error in SummingSortedAlgorithm, there are no description");
    ...
    if (desc.is_agg_func_type) {
      ...
      if (desc.column_numbers.size() == 1) {
        ...
        for (size_t i = 0; i < desc.column_numbers.size(); ++i) column_ptrs[i] = raw_columns[desc.column_numbers[i]];
...
SummingSortedAlgorithm::SummingSortedAlgorithm(SharedHeader header_, size_t num_inputs, SortDescription description_,
                                               const Names &column_names_to_sum, const Names &partition_and_sorting_required_columns,
                                               size_t max_block_size_rows, size_t max_block_size_bytes,
                                               std::optional<size_t> max_dynamic_subcolumns_, const String &sum_function_name,
                                               const String &sum_function_map_name, bool remove_default_values, bool aggregate_all_columns)
    : IMergingAlgorithmWithDelayedChunk(header_, num_inputs, std::move(description_)),
      columns_definition(defineColumns(*header_, description, column_names_to_sum, partition_and_sorting_required_columns,
                                       sum_function_name, sum_function_map_name, remove_default_values, aggregate_all_columns)),
...
void SummingSortedAlgorithm::initialize(Inputs inputs) {
  ...
  for (auto &input : inputs)
    if (input.chunk) preprocessChunk(input.chunk, columns_definition);
...
IMergingAlgorithm::Status SummingSortedAlgorithm::merge() {
  ...
  if (merged_data.isGroupStarted()) merged_data.finishGroup();
