...
static std::unordered_map<RuleType, const String> ruleTypeMap = {
    {RuleTypeAll, "all"}, {RuleTypePlain, "plain"}, {RuleTypeTagged, "tagged"}, {RuleTypeTagList, "tag_list"}};
...
RuleType ruleType(const String& s) {
  if (s == "all") return RuleTypeAll;
  if (s == "plain") return RuleTypePlain;
  if (s == "tagged") return RuleTypeTagged;
  if (s == "tag_list") return RuleTypeTagList;
...
void Pattern::updateHash(SipHash& hash) const {
  ...
  if (function) {
    ...
    for (const auto& p : function->getParameters()) hash.update(fieldToString(p));
  }
...
inline static const Patterns& selectPatternsForMetricType(const Graphite::Params& params, std::string_view path) {
  if (params.patterns_typed) {
    ...
    if (!path_view.contains("?"sv)) return params.patterns_plain;
...
static bool compareRetentions(const Retention& a, const Retention& b) {
  ...
  throw Exception(DB::ErrorCodes::BAD_ARGUMENTS, "Age and precision should only grow up: {}:{} vs {}:{}", a.age, a.precision, b.age,
                  b.precision);
}

bool operator==(const Retention& a, const Retention& b) { return a.age == b.age && a.precision == b.precision; }
...
bool operator==(const Pattern& a, const Pattern& b) {
  ...
  if (a.type != b.type || a.regexp_str != b.regexp_str || a.rule_type != b.rule_type) return false;

  if (a.function == nullptr) {
    if (b.function != nullptr) return false;
...
std::ostream& operator<<(std::ostream& stream, const Pattern& a) {
  ...
  if (!a.regexp_str.empty()) stream << ", regexp = '" << a.regexp_str << "'";
  if (a.function != nullptr) stream << ", function = " << a.function->getName();
  if (!a.retentions.empty()) {
    stream << ",\n  retentions = {\n";
    for (size_t i = 0; i < a.retentions.size(); i++) {
      stream << "    { " << a.retentions[i].age << ", " << a.retentions[i].precision << " }";
      if (i < a.retentions.size() - 1) stream << ",";
      stream << "\n";
    }
    stream << "  }\n";
  } else
...
std::string buildTaggedRegex(std::string regexp_str) {
  ...
  regexp_str += fmt::format("{}{}", fmt::join(tags, "&(.*&)?"), "(&.*)?$" /* close regex */
  );
 ...
 * </graphite_rollup>
 */
static const Pattern& appendGraphitePattern(const Poco::Util::AbstractConfiguration& config, const String& config_element,
                                            Patterns& patterns, bool default_rule, ContextPtr context) {
  Pattern pattern;
  ...
  for (const auto& key : keys) {
    if (key == "regexp") {
      ...
      getAggregateFunctionNameAndParametersArray(aggregate_function_name_with_params, aggregate_function_name, params_row,
                                                 "GraphiteMergeTree storage initialization", context);

      /// TODO Not only Float64
      auto action = NullsAction::EMPTY;
      AggregateFunctionProperties properties;
      pattern.function = AggregateFunctionFactory::instance().get(aggregate_function_name, action, {std::make_shared<DataTypeFloat64>()},
      ...
      pattern.retentions.emplace_back(Graphite::Retention{.age = config.getUInt(config_element + "." + key + ".age"),
  ...
  if (default_rule && pattern.rule_type != RuleTypeAll) {
    throw Exception(DB::ErrorCodes::BAD_ARGUMENTS, "Default must have rule_type all for rollup patterns in GraphiteMergeTree");
  }
  ...
  if (pattern.type & Graphite::Pattern::TypeAggregation)  /// TypeAggregation or TypeAll
    if (pattern.function->allocatesMemoryInArena())
      throw Exception(DB::ErrorCodes::NOT_IMPLEMENTED, "Aggregate function {} isn't supported in GraphiteMergeTree",
                      pattern.function->getName());
...
void Params::updateHash(SipHash& hash) const {
  ...
  for (const auto& p : patterns) p.updateHash(hash);
  for (const auto& p : patterns_plain) p.updateHash(hash);
  for (const auto& p : patterns_tagged) p.updateHash(hash);
}
