...
void MergedData::initialize(const Block& header, const IMergingAlgorithm::Inputs& inputs) {
  ...
  for (size_t i = 0; i != columns.size(); ++i) {
    ...
    if (is_replicated[i]) columns[i] = ColumnReplicated::create(std::move(columns[i]));
    if (columns[i]->hasDynamicStructure()) columns[i]->takeDynamicStructureFromSourceColumns(source_columns[i], max_dynamic_subcolumns);
  }
}
...
void MergedData::insertRow(const ColumnRawPtrs& raw_columns, size_t row, size_t block_size) {
  ...
  for (size_t i = 0; i < num_columns; ++i) columns[i]->insertFrom(*raw_columns[i], row);
...
void MergedData::insertChunk(Chunk&& chunk, size_t rows_size) {
  if (merged_rows) throw Exception(ErrorCodes::LOGICAL_ERROR, "Cannot insert to MergedData from Chunk because MergedData is not empty.");
  ...
  if (rows_size < num_rows) {
    ...
    for (auto& column : columns) column->popBack(pop_size);
  }
...
Chunk MergedData::pull() {
  ...
  for (const auto& column : columns) empty_columns.emplace_back(column->cloneEmpty());
...
bool MergedData::hasEnoughRows() const {
  ...
  if (merged_rows >= max_block_size) return true;
  ...
  if (max_block_size_bytes) {
    ...
    for (const auto& column : columns) {
      ...
      if (merged_bytes >= max_block_size_bytes) return true;
    }
  }

  if (!use_average_block_size) return false;

  /// Zero rows always not enough.
  if (merged_rows == 0) return false;
