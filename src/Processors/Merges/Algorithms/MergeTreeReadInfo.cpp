...
MergeTreeReadInfo::MergeTreeReadInfo(size_t part_level) : origin_merge_tree_part_level(part_level) {}
...
bool isVirtualRow(const Chunk& chunk) {
  const auto read_info = chunk.getChunkInfos().get<MergeTreeReadInfo>();
  if (read_info) return read_info->pk_block.columns() > 0;
  return false;
}
...
void setVirtualRow(Chunk& chunk, const Block& header, bool apply_virtual_row_conversions) {
  ...
  if (apply_virtual_row_conversions) read_info->virtual_row_conversions->execute(pk_block);
  ...
  for (size_t i = 0; i < header.columns(); ++i) {
    ...
    if (const auto* pk_col = pk_block.findByName(col.name)) {
      if (!col.type->equals(*pk_col->type))
        throw Exception(ErrorCodes::LOGICAL_ERROR, "Virtual row has different type for {}. Expected {}, got {}", col.name,
                        col.dumpStructure(), pk_col->dumpStructure());
