...
class SummingSortedAlgorithm final : public IMergingAlgorithmWithDelayedChunk {
 public:
  SummingSortedAlgorithm(SharedHeader header, size_t num_inputs, SortDescription description_,
                         /// List of columns to be summed. If empty, all numeric columns that are not in the description are taken.
                         const Names &column_names_to_sum,
                         /// List of partition key columns. They have to be excluded.
                         const Names &partition_and_sorting_required_columns, size_t max_block_size_rows, size_t max_block_size_bytes,
  ...
  class SummingMergedData : public MergedData {
   ...
   public:
    SummingMergedData(UInt64 max_block_size_rows, UInt64 max_block_size_bytes_, std::optional<size_t> max_dynamic_subcolumns_,
