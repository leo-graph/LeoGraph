...
MergingSortedAlgorithm::MergingSortedAlgorithm(SharedHeader header_, size_t num_inputs, const SortDescription &description_,
                                               size_t max_block_size_, size_t max_block_size_bytes_,
                                               std::optional<size_t> max_dynamic_subcolumns_, SortingQueueStrategy sorting_queue_strategy_,
                                               UInt64 limit_, WriteBuffer *out_row_sources_buf_,
                                               const std::optional<String> &filter_column_name_, bool use_average_block_sizes,
  ...
  if (filter_column_position != -1) {
    ...
    if (!WhichDataType(filter_type).isUInt8())
      throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_COLUMN_FOR_FILTER, "Illegal type {} of column for filter. Must be UInt8",
                      filter_type->getName());
  }
...
void MergingSortedAlgorithm::initialize(Inputs inputs) {
  for (auto &input : inputs) {
    if (!isVirtualRow(input.chunk)) continue;
...
void MergingSortedAlgorithm::consume(Input &input, size_t source_num) {
  ...
  if (sorting_queue_strategy == SortingQueueStrategy::Default) {
    queue_variants.callOnVariant([&](auto &queue) { queue.push(cursors[source_num]); });
  } else {
    queue_variants.callOnBatchVariant([&](auto &queue) { queue.push(cursors[source_num]); });
  }
}

IMergingAlgorithm::Status MergingSortedAlgorithm::merge() {
  if (sorting_queue_strategy == SortingQueueStrategy::Default) {
    IMergingAlgorithm::Status result = queue_variants.callOnVariant([&](auto &queue) { return mergeImpl(queue); });

    return result;
  }

  IMergingAlgorithm::Status result = queue_variants.callOnBatchVariant([&](auto &queue) { return mergeBatchImpl(queue); });
...
void MergingSortedAlgorithm::insertRow(const SortCursorImpl &current) {
  ...
  if (hasFilter()) {
    ...
    if (filter_data[current_row]) merged_data.insertRow(current.all_columns, current_row, current.rows);
...
void MergingSortedAlgorithm::insertRows(const SortCursorImpl &current, size_t num_rows) {
  if (hasFilter()) {
    ...
    if (out_row_sources_buf) {
      ...
      for (size_t i = 0; i < num_rows; ++i) out_row_sources_buf->write(row_source.data);
    }
  }
}
...
void MergingSortedAlgorithm::insertChunk(size_t source_num) {
  ...
  if (hasFilter()) {
    ...
    if (out_row_sources_buf) {
      ...
      for (size_t i = 0; i < chunk_num_rows; ++i) out_row_sources_buf->write(filter_data[i] ? row_source.data : row_source_skipped.data);
    }
...
IMergingAlgorithm::Status MergingSortedAlgorithm::mergeImpl(TSortingHeap &queue) {
  ...
  while (queue.isValid()) {
    ...
    if (current.impl->isFirst() &&
        ...
        && (queue.size() == 1 || (queue.size() >= 2 && current.totallyLessOrEquals(queue.nextChild())))) {
      ...
      if (limit && total_merged_rows_after_insertion > limit) chunk_num_rows -= total_merged_rows_after_insertion - limit;
      ...
      if (!limit_reached) status.required_source = source_num;
    ...
    if (limit && merged_data.totalMergedRows() >= limit) return Status(merged_data.pull(), true);
...
IMergingAlgorithm::Status MergingSortedAlgorithm::mergeBatchImpl(TSortingQueue &queue) {
  ...
  while (queue.isValid()) {
    ...
    if (unlikely(current.impl->isFirst() && current.impl->isLast(initial_batch_size) &&
      ...
      if (!limit_reached) result.required_source = current.impl->order;
