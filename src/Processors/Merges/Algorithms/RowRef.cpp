...
bool RowRef::checkEquals(size_t size, const IColumn** lhs, size_t lhs_row, const IColumn** rhs, size_t rhs_row) {
  for (size_t col_number = 0; col_number < size; ++col_number) {
    ...
    if (0 != cur_column->compareAt(lhs_row, rhs_row, *other_column, 1)) return false;
  }

  return true;
}

size_t RowRef::checkEqualsFirstNonEqual(size_t size, size_t offset, const IColumn** lhs, size_t lhs_row, const IColumn** rhs,
                                        size_t rhs_row) {
  if (size > 0 && 0 != lhs[offset]->compareAt(lhs_row, rhs_row, *rhs[offset], 1)) return offset;

  for (size_t col_number = 0; col_number < size; ++col_number) {
    if (col_number == offset) continue;

    auto& cur_column = lhs[col_number];
    auto& other_column = rhs[col_number];

    if (0 != cur_column->compareAt(lhs_row, rhs_row, *other_column, 1)) return col_number;
  }
...
SharedChunkPtr SharedChunkAllocator::alloc(Chunk& chunk) {
  if (free_chunks.empty())
    throw Exception(ErrorCodes::LOGICAL_ERROR, "Not enough space in SharedChunkAllocator. Chunks allocated: {}", chunks.size());
...
SharedChunkAllocator::~SharedChunkAllocator() {
  if (free_chunks.size() != chunks.size()) {
    LOG_ERROR(getLogger("SharedChunkAllocator"), "SharedChunkAllocator was destroyed before RowRef was released. StackTrace: {}",
              StackTrace().toString());
  }
}
