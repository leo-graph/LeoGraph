 ...
 *
 * If the number of positive and negative rows is the same, and the last row is positive, then the first negative and last positive rows are
 * written. If the number of positive and negative rows is the same, and the last line is negative, it writes nothing. If the positive by 1
 * is greater than the negative rows, then only the last positive row is written. If negative by 1 is greater than positive rows, then only
 * the first negative row is written. Otherwise, a logical error.
 */
class CollapsingSortedAlgorithm final : public IMergingAlgorithmWithSharedChunks {
 public:
  CollapsingSortedAlgorithm(SharedHeader header, size_t num_inputs, SortDescription description_, const String& sign_column,
                            bool only_positive_sign_,  /// For select final. Skip rows with sum(sign) < 0.
                            size_t max_block_size_rows_, size_t max_block_size_bytes_, std::optional<size_t> max_dynamic_subcolumns_,
                            LoggerPtr log_, WriteBuffer* out_row_sources_buf_ = nullptr, bool use_average_block_sizes = false,
