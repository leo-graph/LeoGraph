...
IMergingAlgorithmWithSharedChunks::IMergingAlgorithmWithSharedChunks(SharedHeader header_, size_t num_inputs, SortDescription description_,
                                                                     WriteBuffer* out_row_sources_buf_, size_t max_row_refs,
                                                                     std::unique_ptr<MergedData> merged_data_)
...
void IMergingAlgorithmWithSharedChunks::initialize(Inputs inputs) {
  ...
  for (size_t source_num = 0; source_num < inputs.size(); ++source_num) {
    if (!inputs[source_num].chunk) continue;
    ...
    cursors[source_num] = SortCursorImpl(*header, source.chunk->getColumns(), source.chunk->getNumRows(), description, source_num,
                                         inputs[source_num].permutation);
