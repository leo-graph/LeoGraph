...
VersionedCollapsingAlgorithm::VersionedCollapsingAlgorithm(SharedHeader header_, size_t num_inputs, SortDescription description_,
                                                           const String &sign_column_, size_t max_block_size_rows_,
                                                           size_t max_block_size_bytes_, std::optional<size_t> max_dynamic_subcolumns_,
                                                           WriteBuffer *out_row_sources_buf_, bool use_average_block_sizes)
    : IMergingAlgorithmWithSharedChunks(
          header_, num_inputs, std::move(description_), out_row_sources_buf_, MAX_ROWS_IN_MULTIVERSION_QUEUE,
...
IMergingAlgorithm::Status VersionedCollapsingAlgorithm::merge() {
  ...
  while (queue.isValid()) {
    ...
    if (out_row_sources_buf) current_row_sources.emplace(current->order, true);
