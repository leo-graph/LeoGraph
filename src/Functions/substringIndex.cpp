...
class FunctionSubstringIndex : public IFunction {
 public:
  FunctionSubstringIndex(const char *name_, bool is_utf8_) : function_name(name_), is_utf8(is_utf8_) {}

  static FunctionPtr create(const char *name, bool is_utf8) { return std::make_shared<FunctionSubstringIndex>(name, is_utf8); }
  ...
  DataTypePtr getReturnTypeImpl(const ColumnsWithTypeAndName &arguments) const override {
    FunctionArgumentDescriptors mandatory_args{
        {"s", &isString, nullptr, "String"}, {"delim", &isString, nullptr, "String"}, {"count", &isNativeInteger, nullptr, "UInt or Int"}};
  ...
  ColumnPtr executeImpl(const ColumnsWithTypeAndName &arguments, const DataTypePtr &, size_t input_rows_count) const override {
    ...
    if (!column_delim_const) throw Exception(ErrorCodes::ILLEGAL_COLUMN, "Second argument to {} must be a constant String", getName());

    String delim = column_delim_const->getValue<String>();
    if (!is_utf8) {
      if (delim.size() != 1) throw Exception(ErrorCodes::BAD_ARGUMENTS, "Second argument to {} must be a single character", getName());
    } else {
    ...
    if (column_string_const) {
      ...
      if (!col_str) throw Exception(ErrorCodes::ILLEGAL_COLUMN, "First argument to {} must be a String", getName());
 ...
 protected:
  void vectorVector(const ColumnString *str_column, const String &delim, const IColumn *count_column, ColumnString::Chars &res_data,
  ...
  void vectorConstant(const ColumnString *str_column, const String &delim, Int64 count, ColumnString::Chars &res_data,
                      ColumnString::Offsets &res_offsets, size_t input_rows_count) const {
    res_data.reserve(str_column->getChars().size() / 2);
    res_offsets.reserve(input_rows_count);
  ...
  void constantVector(const String &str, const String &delim, const IColumn *count_column, ColumnString::Chars &res_data,
  ...
  static std::string_view substringIndexUTF8(const PositionCaseSensitiveUTF8::SearcherInBigHaystack *searcher, std::string_view str_ref,
                                             const String &delim, Int64 count) {
    if (count == 0) return {};
    ...
    if (total + count < 0) return str_ref;
  ...
  static std::string_view substringIndex(std::string_view str_ref, char delim, Int64 count) {
    ...
    for (; count; pos += d) {
      if (pos == end) return str_ref;
      if (*pos == delim) count -= d;
    }
    pos -= d;
    return {d > 0 ? str_ref.data() : pos + 1,
            static_cast<size_t>(d > 0 ? pos - str_ref.data() : str_ref.data() + str_ref.size() - pos - 1)};
  }
...
Returns the substring of `s` before `count` occurrences of the delimiter `delim`, as in Spark or MySQL.
  ...
  FunctionDocumentation::Arguments arguments = {
      ...
      {"count",
       "The number of occurrences of the delimiter to count before extracting the substring. If count is positive, everything to the left "
       "of the final delimiter (counting from the left) is returned. If count is negative, everything to the right of the final delimiter "
  ...
  FunctionDocumentation::Examples examples = {{"Usage example", "SELECT substringIndex('www.clickhouse.com', '.', 2)",
                                               R"(
...
If this assumption is violated, no exception is thrown and the result is undefined.
  ...
  FunctionDocumentation::Arguments arguments_utf8 = {
      ...
      {"count",
       ...
       "(counting from the right) is returned.",
       {"UInt", "Int"}}};
  FunctionDocumentation::ReturnedValue returned_value_utf8 = {"Returns a substring of `s` before `count` occurrences of `delim`.",
                                                              {"String"}};
  FunctionDocumentation::Examples examples_utf8 = {
      {"UTF8 example", "SELECT substringIndexUTF8('www.straßen-in-europa.de', '.', 2)", "www.straßen-in-europa"}};
  ...
  factory.registerFunction(
      "substringIndex", [](ContextPtr) { return FunctionSubstringIndex::create("substringIndex", false); }, documentation);
  factory.registerFunction(
      "substringIndexUTF8", [](ContextPtr) { return FunctionSubstringIndex::create("substringIndexUTF8", true); }, documentation_utf8);
