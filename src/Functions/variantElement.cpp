...
class FunctionVariantElement : public IFunction {
 public:
  ...
  DataTypePtr getReturnTypeImpl(const ColumnsWithTypeAndName &arguments) const override {
    auto is_variant_or_array_of_variant = [](const IDataType &type) {
      ...
      while (const DataTypeArray *array = checkAndGetDataType<DataTypeArray>(current_type)) current_type = array->getNestedType().get();
      return isVariant(current_type);
    };

    FunctionArgumentDescriptors mandatory_args{{"variant", is_variant_or_array_of_variant, nullptr, "Variant or Array of Variant"},
                                               {"name", &isString, &isColumnConst, "const String"}};

    FunctionArgumentDescriptors optional_args{{"default", nullptr, nullptr, "Any"}};
    ...
    if (variant_global_discr.has_value()) {
      DataTypePtr return_type =
          makeExtractedSubcolumnsNullableOrLowCardinalityNullableSafe(variant_type->getVariant(variant_global_discr.value()));

      for (; count_arrays; --count_arrays) return_type = std::make_shared<DataTypeArray>(return_type);
  ...
  ColumnPtr executeImpl(const ColumnsWithTypeAndName &arguments, const DataTypePtr &, size_t input_rows_count) const override {
    ...
    if (!input_type_as_variant || !input_col_as_variant)
      throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT,
                      "First argument for function {} must be Variant or array of Variants. Actual {}", getName(),
    ...
    if (!variant_discr) return arguments[2].column;
 ...
 private:
  std::optional<size_t> getVariantGlobalDiscriminator(const ColumnPtr &index_column, const DataTypeVariant &variant_type,
                                                      size_t argument_size) const {
    const auto *name_col = checkAndGetColumnConst<ColumnString>(index_column.get());
    if (!name_col)
      throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT, "Second argument to {} with Variant argument must be a constant String",
                      getName());
    ...
    if (auto variant_element_type = DataTypeFactory::instance().tryGet(variant_element_name)) {
      if (auto discr = variant_type.tryGetVariantDiscriminator(variant_element_type->getName())) return discr;
    }

    if (argument_size == 2)
      throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT, "{} doesn't contain variant with type {}", variant_type.getName(),
                      variant_element_name);

    return std::nullopt;
  }

  ColumnPtr wrapInArraysAndConstIfNeeded(ColumnPtr res, const Columns &array_offsets, bool input_arg_is_const,
