...
class FunctionH3GetBaseCell : public IFunction {
 public:
  ...
  DataTypePtr getReturnTypeImpl(const ColumnsWithTypeAndName &arguments) const override {
    FunctionArgumentDescriptors mandatory_args{{"index", &isUInt64, nullptr, "UInt64"}};
...
Returns the base cell number of the [H3](#h3-index) index.
  ...
  FunctionDocumentation::ReturnedValue returned_value = {"Returns the hexagon base cell number.", {"UInt8"}};
  FunctionDocumentation::Examples examples = {{"Get base cell number", "SELECT h3GetBaseCell(612916788725809151) AS basecell",
                                               R"(
┌─basecell─┐
│       12 │
└──────────┘
