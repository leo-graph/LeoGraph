...
enum class IPKind { IPv4, IPv6, String };
 ...
 public:
  ...
  explicit IPAddressVariant(std::string_view addr_) {
    ...
    if (DB::parseIPv4whole(addr_.data(), addr_.data() + addr_.size(), reinterpret_cast<unsigned char *>(&v4))) {
      ...
      if (!success) throw DB::Exception(DB::ErrorCodes::CANNOT_PARSE_TEXT, "Neither IPv4 nor IPv6 address: '{}'", addr_);
    }
  }

  UInt32 asV4() const {
    if (const auto *val = std::get_if<IPv4AddrType>(&addr)) return *val;
    return 0;
  }

  const uint8_t *asV6() const {
    if (const auto *val = std::get_if<IPv6AddrType>(&addr)) return val->data();
    return nullptr;
  }
...
IPAddressCIDR parseIPWithCIDR(std::string_view cidr_str) {
  ...
  auto [parse_end, parse_error] =
      std::from_chars(prefix_str.data(), prefix_str_end, prefix);  /// NOLINT(bugprone-suspicious-stringview-data-usage)
  ...
  if (has_error) throw DB::Exception(DB::ErrorCodes::CANNOT_PARSE_TEXT, "The CIDR has a malformed prefix bits: {}", std::string(cidr_str));
...
inline bool isAddressInRange(const IPAddressVariant &address, const IPAddressCIDR &cidr) {
  if (const auto *cidr_v6 = cidr.address.asV6()) {
    if (const auto *addr_v6 = address.asV6()) return DB::matchIPv6Subnet(addr_v6, cidr_v6, cidr.prefix);
  } else {
    if (!address.asV6()) return DB::matchIPv4Subnet(address.asV4(), cidr.address.asV4(), cidr.prefix);
  }
  return false;
}
...
#define EXEC_IMPL(col, func, ...)                                                  \
  ...
  static std::optional<IPAddressVariant> parseConstantIP(const ColumnConst &col_addr) {
    ...
    else if (col_addr.onlyNull()) return std::nullopt;
    ...
    throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT,
                    "The IP column type must be one of: String, IPv4, IPv6, Nullable(IPv4), Nullable(IPv6), or Nullable(String).");
  }

  ColumnPtr executeImpl(const ColumnsWithTypeAndName &arguments, const DataTypePtr & /* return_type */,
    ...
    if (const auto *col_cidr_const = checkAndGetAnyColumnConst(col_cidr)) return executeImpl(*col_addr, *col_cidr_const, input_rows_count);
  ...
  DataTypePtr getReturnTypeImpl(const DataTypes &arguments) const override {
    ...
    if (!(type.isString() || type.isIPv4() || type.isIPv6()) || !isString(prefix_type))
      throw Exception(
          ...
          getName(), addr_type->getName(), prefix_type->getName());
 ...
 private:
  ...
  static const ColumnConst *checkAndGetAnyColumnConst(const IColumn *column) {
    if (!column || !isColumnConst(*column)) return nullptr;

    return assert_cast<const ColumnConst *>(column);
  }

  /// Both columns are constant.
  static ColumnPtr executeImpl(const ColumnConst &col_addr_const, const ColumnConst &col_cidr_const, size_t input_rows_count) {
  ...
  static ColumnPtr executeImpl(const ColumnConst &col_addr_const, const IColumn &col_cidr, size_t input_rows_count) {
    const auto addr = parseConstantIP(col_addr_const);
    if (!addr.has_value()) return ColumnUInt8::create(input_rows_count, static_cast<UInt8>(0));
  ...
  template <IPKind kind>
  static ColumnPtr executeImpl(const IPTrait<kind>::ColumnType *col_addr, const ColumnNullable *nullable_column, const IPAddressCIDR &cidr,
  ...
  template <IPKind kind>
  static ColumnPtr executeImpl(const IPTrait<kind>::ColumnType *col_addr, const ColumnNullable *nullable_column, const IColumn &col_cidr,
                               size_t input_rows_count) {
    ColumnUInt8::MutablePtr col_res = ColumnUInt8::create(input_rows_count);
    ColumnUInt8::Container &vec_res = col_res->getData();
...
This function accepts both IPv4 and IPv6 addresses (and networks) represented as strings. It returns `0` if the IP version of the address and the CIDR don't match.
  ...
  FunctionDocumentation::Arguments arguments = {{"address", "An IPv4 or IPv6 address.", {"String"}},
                                                {"prefix", "An IPv4 or IPv6 network prefix in CIDR.", {"String"}}};
  FunctionDocumentation::ReturnedValue returned_value = {"Returns `1` if the IP version of the address and the CIDR match, otherwise `0`.",
                                                         {"UInt8"}};
