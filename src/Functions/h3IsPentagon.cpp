...
In the H3 grid system, most cells are hexagonal, but there are exactly 12 pentagonal cells at each resolution to account for the topological constraints of mapping a sphere to a grid.
  ...
  FunctionDocumentation::ReturnedValue returned_value = {"Returns `1` if the index represents a pentagonal cell, `0` otherwise.",
                                                         {"UInt8"}};
  FunctionDocumentation::Examples examples = {{"Check if H3 index is a pentagon", "SELECT h3IsPentagon(644721767722457330) AS pentagon",
                                               R"(
┌─pentagon─┐
│        0 │
└──────────┘
