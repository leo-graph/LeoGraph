...
class FunctionGetScalar : public IFunction, WithContext {
 public:
  ...
  static FunctionPtr create(ContextPtr context_) { return std::make_shared<FunctionGetScalar>(context_); }
...
class FunctionGetSpecialScalar : public IFunction {
 public:
  ...
  static ColumnWithTypeAndName createScalar(ContextPtr context_, const char *scalar_name_) {
    if (auto block = context_->tryGetSpecialScalar(scalar_name_)) return block->getByPosition(0);
  ...
  DataTypePtr getReturnTypeImpl(const ColumnsWithTypeAndName &) const override { return scalar.type; }

  ColumnPtr executeImpl(const ColumnsWithTypeAndName &, const DataTypePtr &, size_t input_rows_count) const override {
    auto result = ColumnConst::create(scalar.column, input_rows_count);

    if (!isSuitableForConstantFolding()) return result->convertToFullColumnIfConst();
...
If a query is not distributed then a constant value `0` is returned.
  ...
  FunctionDocumentation::Examples examples_shardNum = {{"Usage example",
                                                        R"(
...
└───────┴────────────┴──────────────┘
  ...
  FunctionDocumentation documentation_shardNum = {description_shardNum,    syntax_shardNum,   arguments_shardNum,     {},
                                                  returned_value_shardNum, examples_shardNum, introduced_in_shardNum, category_shardNum};

  factory.registerFunction(
      "shardNum", [](ContextPtr context) { return FunctionGetSpecialScalar::create(context, "shardNum", "_shard_num"); },
      documentation_shardNum);
...
If a query is not distributed then constant value `0` is returned.
  ...
  FunctionDocumentation::Examples examples_shardCount = {{"Usage example",
                                                          R"(
...
└──────────────┘
  ...
  FunctionDocumentation documentation_shardCount = {
      description_shardCount,    syntax_shardCount,   arguments_shardCount,     {},
      returned_value_shardCount, examples_shardCount, introduced_in_shardCount, category_shardCount};

  factory.registerFunction(
      "shardCount", [](ContextPtr context) { return FunctionGetSpecialScalar::create(context, "shardCount", "_shard_count"); },
      documentation_shardCount);
}
