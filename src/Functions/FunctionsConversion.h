...
struct FunctionConvertSettings {
  ...
  explicit FunctionConvertSettings(const ContextPtr &context, FormatSettings::DateTimeOverflowBehavior datetime_overflow_behavior_)
      ...
      : date_time_overflow_behavior(context && datetime_overflow_behavior_ == default_date_time_overflow_behavior
                                        ? context->getSettingsRef()[Setting::date_time_overflow_behavior].value
                                        : datetime_overflow_behavior_),
        precise_float_parsing(context && context->getSettingsRef()[Setting::precise_float_parsing]),
        cast_ipv4_ipv6_default_on_conversion_error(context &&
                                                   context->getSettingsRef()[Setting::cast_ipv4_ipv6_default_on_conversion_error]),
        ...
        input_format_ipv4_default_on_conversion_error(context &&
                                                      context->getSettingsRef()[Setting::input_format_ipv4_default_on_conversion_error]),
        input_format_ipv6_default_on_conversion_error(context &&
                                                      context->getSettingsRef()[Setting::input_format_ipv6_default_on_conversion_error]),
        ...
        date_time_64_output_format_cut_trailing_zeros_align_to_groups_of_thousands(
            context && context->getSettingsRef()[Setting::date_time_64_output_format_cut_trailing_zeros_align_to_groups_of_thousands]),
        cast_string_to_date_time_mode(context ? context->getSettingsRef()[Setting::cast_string_to_date_time_mode]
    ...
    if constexpr (is_signed_v<FromType>)
      if (from < 0) {
        if constexpr (overflow_throw)
          throw Exception(ErrorCodes::VALUE_IS_OUT_OF_RANGE_OF_DATA_TYPE, "Value {} is out of bounds of type Date",
                          static_cast<Int64>(from));
        else
          return 0;
      }
...
struct ToDate32TransformFromSecondsOrDays {
  ...
  static NO_SANITIZE_UNDEFINED Int32 execute(const FromType &from, const DateLUTImpl &time_zone) {
    ...
    if constexpr (is_signed_v<FromType>) {
      bool is_nan = false;
      if constexpr (is_floating_point<FromType>) is_nan = isNaN(from);
      if (is_nan || from < daynum_min_offset) {
        if constexpr (overflow_throw)
          throw Exception(ErrorCodes::VALUE_IS_OUT_OF_RANGE_OF_DATA_TYPE, "Timestamp value {} is out of bounds of type Date32",
                          static_cast<Int64>(from));
    ...
    if constexpr (std::numeric_limits<FromType>::max() >= DATE_LUT_MAX_EXTEND_DAY_NUM)
      if (from >= DATE_LUT_MAX_EXTEND_DAY_NUM) return time_zone.toDayNum(std::min(time_t(Int64(from)), time_t(MAX_DATETIME64_TIMESTAMP)));
...
struct ToTimeTransformSigned {
  ...
  static NO_SANITIZE_UNDEFINED ToType execute(const FromType &from, const DateLUTImpl &) { return static_cast<ToType>(from); }
};
...
struct ToDateTime64TransformUnsigned {
  ...
  NO_SANITIZE_UNDEFINED DateTime64::NativeType execute(FromType from, const DateLUTImpl &) const {
    if constexpr (date_time_overflow_behavior == FormatSettings::DateTimeOverflowBehavior::Throw) {
      ...
      return DecimalUtils::decimalFromComponentsWithMultiplier<DateTime64>(std::min<time_t>(from, MAX_DATETIME64_TIMESTAMP), 0,
                                                                           scale_multiplier);
  }
};
...
struct ToTime64TransformUnsigned {
  ...
  NO_SANITIZE_UNDEFINED Time64::NativeType execute(FromType from, const DateLUTImpl &time_zone) const {
    ...
    if constexpr (date_time_overflow_behavior == FormatSettings::DateTimeOverflowBehavior::Throw) {
      ...
      return DecimalUtils::decimalFromComponentsWithMultiplier<Time64>(std::min<time_t>(converted, MAX_DATETIME64_TIMESTAMP), 0,
                                                                       scale_multiplier);
  }
};
...
struct FormatImpl<DataTypeDateTime64> {
  template <typename ReturnType = void>
  static ReturnType execute(const DataTypeDateTime64::FieldType x, WriteBuffer &wb, const DataTypeDateTime64 *type,
...
struct ConvertImplGenericToString {
  static ColumnPtr execute(const ColumnsWithTypeAndName &arguments, const DataTypePtr &result_type, size_t /*input_rows_count*/,
    ...
    {
      ColumnStringHelpers::WriteHelper<StringColumnType> write_helper(assert_cast<StringColumnType &>(*col_to), size);
    ...
    if (result_type->isNullable() && null_map) return ColumnNullable::create(std::move(col_to), std::move(null_map));
    return col_to;
  }
};
...
// NOTE: no need of extra overload of DateTime64, since readDateTimeText64 has different signature and that case is explicitly handled in
// the calling code.
template <>
...
inline bool tryParseImpl<DataTypeDate>(DataTypeDate::FieldType &x, ReadBuffer &rb, const DateLUTImpl *time_zone, bool) {
  DayNum tmp(0);
  if (!tryReadDateText(tmp, rb, *time_zone)) return false;
  x = tmp;
  return true;
}
...
inline bool tryParseImpl<DataTypeDateTime>(DataTypeDateTime::FieldType &x, ReadBuffer &rb, const DateLUTImpl *time_zone, bool) {
  time_t time = 0;
  if (!tryReadDateTimeText(time, rb, *time_zone)) return false;
  convertFromTime<DataTypeDateTime>(x, time);
  return true;
}
...
[[maybe_unused]] inline bool tryParseImpl<DataTypeTime>(DataTypeTime::FieldType &x, ReadBuffer &rb, const DateLUTImpl *time_zone, bool) {
  time_t time = 0;
  if (!tryReadTimeText(time, rb, *time_zone)) return false;
  convertFromTime<DataTypeTime>(x, time);
  return true;
}
...
inline bool tryParseImpl<DataTypeUUID>(DataTypeUUID::FieldType &x, ReadBuffer &rb, const DateLUTImpl *, bool) {
  UUID tmp;
  if (!tryReadUUIDText(tmp, rb)) return false;
...
inline bool tryParseImpl<DataTypeIPv4>(DataTypeIPv4::FieldType &x, ReadBuffer &rb, const DateLUTImpl *, bool) {
  IPv4 tmp;
  if (!tryReadIPv4Text(tmp, rb)) return false;
...
inline bool tryParseImpl<DataTypeIPv6>(DataTypeIPv6::FieldType &x, ReadBuffer &rb, const DateLUTImpl *, bool) {
  IPv6 tmp;
  if (!tryReadIPv6Text(tmp, rb)) return false;
...
struct ConvertThroughParsing {
  ...
  static bool isAllRead(ReadBuffer &in) {
    ...
    if constexpr (std::is_same_v<FromDataType, DataTypeFixedString>)
      while (!in.eof() && *in.position() == 0) ++in.position();
    ...
    if constexpr (std::is_same_v<ToDataType, DataTypeDate> || std::is_same_v<ToDataType, DataTypeDate32>) {
      if (!in.eof() && (*in.position() == ' ' || *in.position() == 'T')) {
        if (in.buffer().size() == strlen("YYYY-MM-DD hh:mm:ss")) return true;
  ...
  static ColumnPtr execute(const ColumnsWithTypeAndName &arguments, const DataTypePtr &res_type, size_t input_rows_count,
                           const FunctionConvertSettings &settings, Additions additions [[maybe_unused]] = Additions()) {
    ...
    if (std::is_same_v<FromDataType, DataTypeString> && !col_from_string)
      throw Exception(ErrorCodes::ILLEGAL_COLUMN, "Illegal column {} of first argument of function {}", col_from->getName(), Name::name);
      ...
      if constexpr (exception_mode == ConvertFromStringExceptionMode::Throw) {
        if constexpr (parsing_mode == ConvertFromStringParsingMode::BestEffort && (to_datetime || to_datetime64)) {
          ...
          if constexpr (to_datetime64) {
            ...
            SerializationDecimal<typename ToDataType::FieldType>::readText(vec_to[i], read_buffer, ToDataType::maxPrecision(),
                                                                           col_to->getScale());
          } else {
            ...
            do {
              ...
              if constexpr (std::is_same_v<Additions, AccurateConvertStrategyAdditions>) {
                if (!tryParseImpl<ToDataType>(vec_to[i], read_buffer, local_time_zone, settings.precise_float_parsing))
                  throw Exception(ErrorCodes::CANNOT_PARSE_TEXT, "Cannot parse string to type {}",
        ...
        if (!isAllRead(read_buffer)) throwExceptionForIncompletelyParsedValue(read_buffer, *res_type);
      } else {
        bool parsed;
        ...
        if constexpr (parsing_mode == ConvertFromStringParsingMode::BestEffort && (to_datetime || to_datetime64)) {
          ...
          if constexpr (to_datetime64) {
            ...
            parsed = SerializationDecimal<typename ToDataType::FieldType>::tryReadText(vec_to[i], read_buffer, ToDataType::maxPrecision(),
                                                                                       col_to->getScale());
          } else if (std::is_same_v<FromDataType, DataTypeFixedString> &&
        ...
        if (!parsed) {
          if constexpr (std::is_same_v<ToDataType, DataTypeDate32>) {
            vec_to[i] =
                -static_cast<Int32>(DateLUT::instance().getDayNumOffsetEpoch());  /// NOLINT(readability-static-accessed-through-instance)
          } else {
            vec_to[i] = static_cast<typename ToDataType::FieldType>(0);
          }
        }

        if constexpr (exception_mode == ConvertFromStringExceptionMode::Null) (*vec_null_map_to)[i] = !parsed;
      }
...
enum class BehaviourOnErrorFromString : uint8_t { ConvertDefaultBehaviorTag, ConvertReturnNullOnErrorTag, ConvertReturnZeroOnErrorTag };
...
static void NO_SANITIZE_UNDEFINED convertToBool(ColVecToData &vec_to, ColVecFromData &vec_from, size_t input_rows_count) {
  for (size_t i = 0; i < input_rows_count; i++) vec_to[i] = vec_from[i] != FromFieldType(0);
}
...
static void NO_SANITIZE_UNDEFINED convertFromUInt128toIPv6(ColVecToData &vec_to, ColVecFromData &vec_from, size_t input_rows_count) {
  static_assert(std::is_same_v<DataTypeUInt128::FieldType, DataTypeIPv6::FieldType::UnderlyingType>, "IPv6 and UInt128 types must be same");
...
static void NO_SANITIZE_UNDEFINED convertFromIPv6ToUInt128(ColVecToData &vec_to, ColVecFromData &vec_from, size_t input_rows_count) {
  static_assert(std::is_same_v<DataTypeUInt128::FieldType, DataTypeIPv6::FieldType::UnderlyingType>, "UInt128 and IPv6 types must be same");
...
static void NO_SANITIZE_UNDEFINED convertDateTimeToZero(ColVecToData &vec_to, size_t input_rows_count) {
  for (size_t i = 0; i < input_rows_count; i++) vec_to[i] = static_cast<ToFieldType>(0);
}

template <typename ColVecToData, typename ColVecFromData>
static void NO_SANITIZE_UNDEFINED convertFromIPv6ToIPv4(ColVecToData &vec_to, ColVecFromData &vec_from, size_t input_rows_count,
  ...
  for (size_t i = 0; i < input_rows_count; i++) {
    ...
    if (!matchIPv6Subnet(src, ip4_cidr, 96)) {
      ...
      throw Exception(ErrorCodes::CANNOT_CONVERT_TYPE, "IPv6 {} in column {} is not in IPv4 mapping block", addr,
...
static void NO_SANITIZE_UNDEFINED convertFromUInt64ToIPv4(ColVecToData &vec_to, ColVecFromData &vec_from, size_t input_rows_count) {
  for (size_t i = 0; i < input_rows_count; i++) vec_to[i] = static_cast<ToFieldType>(static_cast<IPv4::UnderlyingType>(vec_from[i]));
}
...
static void NO_SANITIZE_UNDEFINED convertToUnixTimestampFromDate(ColVecToData &vec_to, ColVecFromData &vec_from, size_t input_rows_count) {
  for (size_t i = 0; i < input_rows_count; i++) vec_to[i] = static_cast<ToFieldType>(vec_from[i] * DATE_SECONDS_PER_DAY);
}
...
static ColumnPtr NO_SANITIZE_UNDEFINED convertNumericGeneral(typename ColumnVector<ToFieldType>::MutablePtr &&col_to, ColVecToData &vec_to,
                                                             ColVecFromData &vec_from, size_t input_rows_count,
                                                             const ColumnWithTypeAndName &named_from, const DataTypePtr &result_type) {
  ColumnUInt8::MutablePtr col_null_map_to;
...
struct ConvertImpl {
  template <typename Additions = void *>
  static ColumnPtr NO_SANITIZE_UNDEFINED execute(const ColumnsWithTypeAndName &arguments, const DataTypePtr &result_type [[maybe_unused]],
                                                 size_t input_rows_count, BehaviourOnErrorFromString from_string_tag [[maybe_unused]],
                                                 const FunctionConvertSettings &settings, Additions additions = Additions()) {
    ...
    if constexpr ((std::is_same_v<FromDataType, ToDataType> && !FromDataType::is_parametric) ||
      ...
      /// Conversion of DateTime to Date: throw off time component.
      return DateTimeTransformImpl<FromDataType, ToDataType, ToDate32Impl, false>::template execute<Additions>(arguments, result_type,
                                                                                                               input_rows_count);
      ...
      return DateTimeTransformImpl<FromDataType, ToDataType, ToDateTimeImpl<date_time_overflow_behavior>,
      ...
      return DateTimeTransformImpl<DataTypeDateTime64, DataTypeDate32, TransformDateTime64<ToDate32Impl>,
     ...
     * instead of 65536.
     */
    else if constexpr ((std::is_same_v<FromDataType, DataTypeUInt16> || std::is_same_v<FromDataType, DataTypeUInt32> ||
                        std::is_same_v<FromDataType, DataTypeUInt64> || std::is_same_v<FromDataType, DataTypeUInt128> ||
                        std::is_same_v<FromDataType, DataTypeUInt256> || std::is_same_v<FromDataType, DataTypeInt8> ||
                        std::is_same_v<FromDataType, DataTypeInt16> || std::is_same_v<FromDataType, DataTypeInt32> ||
                        std::is_same_v<FromDataType, DataTypeInt64> || std::is_same_v<FromDataType, DataTypeInt128> ||
                        std::is_same_v<FromDataType, DataTypeInt256> || std::is_same_v<FromDataType, DataTypeFloat32> ||
                        std::is_same_v<FromDataType, DataTypeFloat64> || std::is_same_v<FromDataType, DataTypeBFloat16>) &&
                       std::is_same_v<ToDataType, DataTypeDate>) {
      return DateTimeTransformImpl<FromDataType, ToDataType,
                                   ToDateTransformFromSecondsOrDays<typename FromDataType::FieldType, default_date_time_overflow_behavior>,
                                   false>::template execute<Additions>(arguments, result_type, input_rows_count);
    } else if constexpr ((std::is_same_v<FromDataType, DataTypeUInt16> || std::is_same_v<FromDataType, DataTypeUInt32> ||
                          std::is_same_v<FromDataType, DataTypeUInt64> || std::is_same_v<FromDataType, DataTypeUInt128> ||
                          std::is_same_v<FromDataType, DataTypeUInt256> || std::is_same_v<FromDataType, DataTypeInt16> ||
                          std::is_same_v<FromDataType, DataTypeInt32> || std::is_same_v<FromDataType, DataTypeInt64> ||
                          std::is_same_v<FromDataType, DataTypeInt128> || std::is_same_v<FromDataType, DataTypeInt256> ||
      ...
      return DateTimeTransformImpl<
          ...
          ToDate32TransformFromSecondsOrDays<typename FromDataType::FieldType, default_date_time_overflow_behavior>,
          false>::template execute<Additions>(arguments, result_type, input_rows_count);
    } else if constexpr (std::is_same_v<FromDataType, DataTypeDateTime> && std::is_same_v<ToDataType, DataTypeTime>) {
      return DateTimeTransformImpl<
          FromDataType, ToDataType,
          ToTimeTransformFromDateTime<typename FromDataType::FieldType, Int32, default_date_time_overflow_behavior>,
          false>::template execute<Additions>(arguments, result_type, input_rows_count);
    }
    /// Special case of converting Int8, Int16, Int32 or (U)Int64 (and also, for convenience, Float32, Float64) to DateTime.
    else if constexpr ((std::is_same_v<FromDataType, DataTypeInt8> || std::is_same_v<FromDataType, DataTypeInt16> ||
      ...
      if constexpr (std::is_same_v<ToDataType, DataTypeDateTime>)
        ...
        return DateTimeTransformImpl<FromDataType, ToDataType,
                                     ToTimeTransformSigned<typename FromDataType::FieldType, Int32, default_date_time_overflow_behavior>,
                                     false>::template execute<Additions>(arguments, result_type, input_rows_count);
      ...
      if constexpr (std::is_same_v<ToDataType, DataTypeDateTime>)
        ...
        return DateTimeTransformImpl<FromDataType, ToDataType,
                                     ToTimeTransform64<typename FromDataType::FieldType, Int32, default_date_time_overflow_behavior>,
                                     false>::template execute<Additions>(arguments, result_type, input_rows_count);
    } else if constexpr ((std::is_same_v<FromDataType, DataTypeInt64> || std::is_same_v<FromDataType, DataTypeFloat32> ||
      ...
      if constexpr (std::is_same_v<ToDataType, DataTypeDateTime>)
        ...
        return DateTimeTransformImpl<FromDataType, ToDataType,
                                     ToTimeTransform64Signed<typename FromDataType::FieldType, Int32, default_date_time_overflow_behavior>,
                                     false>::template execute<Additions>(arguments, result_type, input_rows_count);
    } else if constexpr ((std::is_same_v<FromDataType, DataTypeInt8> || std::is_same_v<FromDataType, DataTypeInt16> ||
                          std::is_same_v<FromDataType, DataTypeInt32> || std::is_same_v<FromDataType, DataTypeInt64>) &&
      ...
      if constexpr (std::is_same_v<ToDataType, DataTypeDateTime64>)
        ...
        return DateTimeTransformImpl<FromDataType, ToDataType,
                                     ToTime64TransformSigned<typename FromDataType::FieldType, default_date_time_overflow_behavior>,
                                     false>::template execute<Additions>(arguments, result_type, input_rows_count, additions);
    } else if constexpr (std::is_same_v<FromDataType, DataTypeUInt64> &&
                         (std::is_same_v<ToDataType, DataTypeDateTime64> || std::is_same_v<ToDataType, DataTypeTime64>)) {
      if constexpr (std::is_same_v<ToDataType, DataTypeDateTime64>)
        return DateTimeTransformImpl<FromDataType, ToDataType, ToDateTime64TransformUnsigned<UInt64, default_date_time_overflow_behavior>,
                                     false>::template execute<Additions>(arguments, result_type, input_rows_count, additions);
      else
        return DateTimeTransformImpl<FromDataType, ToDataType, ToTime64TransformUnsigned<UInt64, default_date_time_overflow_behavior>,
                                     false>::template execute<Additions>(arguments, result_type, input_rows_count, additions);
    } else if constexpr ((std::is_same_v<FromDataType, DataTypeFloat32> || std::is_same_v<FromDataType, DataTypeFloat64>) &&
                         (std::is_same_v<ToDataType, DataTypeDateTime64> || std::is_same_v<ToDataType, DataTypeTime64>)) {
      if constexpr (std::is_same_v<ToDataType, DataTypeDateTime64>)
        return DateTimeTransformImpl<
            FromDataType, ToDataType,
        ...
        return DateTimeTransformImpl<
            FromDataType, ToDataType,
            ToTime64TransformFloat<FromDataType, typename FromDataType::FieldType, default_date_time_overflow_behavior>,
            false>::template execute<Additions>(arguments, result_type, input_rows_count, additions);
    }
    /// Conversion of DateTime64 to Date or DateTime: discards fractional part.
    else if constexpr (std::is_same_v<FromDataType, DataTypeDateTime64> && std::is_same_v<ToDataType, DataTypeDate>) {
      return DateTimeTransformImpl<FromDataType, ToDataType, TransformDateTime64<ToDateImpl<date_time_overflow_behavior>>,
                                   false>::template execute<Additions>(arguments, result_type, input_rows_count, additions);
    } else if constexpr (std::is_same_v<FromDataType, DataTypeDateTime64> && std::is_same_v<ToDataType, DataTypeDateTime>) {
      return DateTimeTransformImpl<FromDataType, ToDataType, TransformDateTime64<ToDateTimeImpl<date_time_overflow_behavior>>,
                                   false>::template execute<Additions>(arguments, result_type, input_rows_count, additions);
    } else if constexpr (std::is_same_v<FromDataType, DataTypeDateTime64> && std::is_same_v<ToDataType, DataTypeTime>) {
      return DateTimeTransformImpl<FromDataType, ToDataType, TransformDateTime64<ToTimeImpl<date_time_overflow_behavior>>,
                                   false>::template execute<Additions>(arguments, result_type, input_rows_count, additions);
    }
    /// Conversion of Date or DateTime to DateTime64: add zero sub-second part.
      ...
      return DateTimeTransformImpl<FromDataType, ToDataType, ToTime64Transform, false>::template execute<Additions>(
          arguments, result_type, input_rows_count, additions);
    } else if constexpr (std::is_same_v<FromDataType, DataTypeDateTime64> && std::is_same_v<ToDataType, DataTypeTime64>) {
      using ToFieldType = typename ToDataType::FieldType;
      ...
      if constexpr (std::is_same_v<Additions, AccurateOrNullConvertStrategyAdditions>)
        ...
        return col_to;
    } else if constexpr (IsDataTypeDateOrDateTimeOrTime<FromDataType> && std::is_same_v<ToDataType, DataTypeString>) {
      ...
      if (const auto col_from = checkAndGetColumn<ColVecType>(col_with_type_and_name.column.get())) {
        ...
        if (!null_map && arguments.size() > 1) null_map = copyNullMap(arguments[1].column->convertToFullColumnIfConst());
          ...
          for (size_t i = 0; i < size; ++i) {
            ...
            if constexpr (std::is_same_v<FromDataType, DataTypeDateTime64>) {
              if (cut_trailing_zeros_align_to_groups_of_thousands)
                writeDateTimeTextCutTrailingZerosAlignToGroupOfThousands(DateTime64(vec_from[i]), type.getScale(), write_buffer,
                                                                         *time_zone);
              else
        ...
        if (null_map) return ColumnNullable::create(std::move(col_to), std::move(null_map));
        return col_to;
      } else
    ...
    /// Cutting sequences of zero bytes from end of strings.
      ...
      if (const ColumnFixedString *col_from = checkAndGetColumn<ColumnFixedString>(nested.column.get())) {
        ...
        for (size_t i = 0; i < size; ++i) {
          if (!null_map || !null_map->getData()[i]) {
            ...
            while (bytes_to_copy > 0 && data_from[offset_from + bytes_to_copy - 1] == 0) --bytes_to_copy;
      ...
      if (const auto col_from = checkAndGetColumn<ColVecType>(col_with_type_and_name.column.get())) {
        ...
        throw Exception(ErrorCodes::ILLEGAL_COLUMN, "Illegal column {} of first argument of function {}", arguments[0].column->getName(),
                        Name::name);
    } else if constexpr (std::is_same_v<Name, NameToUnixTimestamp> && std::is_same_v<FromDataType, DataTypeString> &&
...
#define GENERATE_PARSING_MODE_CASE(PARSING_MODE, EXCEPTION_MODE)                                                                          \
  case FormatSettings::DateTimeInputFormat::PARSING_MODE: {                                                                               \
    return ConvertThroughParsing<FromDataType, ToDataType, Name, ConvertFromStringExceptionMode::EXCEPTION_MODE,                          \
                                 ConvertFromStringParsingMode::PARSING_MODE>::execute(arguments, result_type, input_rows_count, settings, \
                                                                                      additions);                                         \
  }
...
#undef GENERATE_PARSING_MODE_CASE
      ...
      if (from == to || arguments[0].column->empty()) return arguments[0].column;
      ...
      if (from_position < to_position) {
        for (int i = from_position; i < to_position; ++i) conversion_factor *= interval_conversions[i];
      ...
      if constexpr ((IsDataTypeDecimal<FromDataType> || IsDataTypeDecimal<ToDataType>) &&
        ...
        throw Exception(ErrorCodes::ILLEGAL_COLUMN, "Illegal column {} of first argument of function {}", named_from.column->getName(),
                        Name::name);
      }
...
#define RETURN_NULLABLE_IF_NEEDED(col_name)                                                   \
  do {                                                                                        \
    if constexpr (std::is_same_v<Additions, AccurateOrNullConvertStrategyAdditions>) {        \
      ...
      else if constexpr ((std::is_same_v<FromDataType, DataTypeIPv4> != std::is_same_v<ToDataType, DataTypeIPv4>) &&
      ...
      /// General numeric conversion
        ...
        return convertNumericGeneral<Additions, FromFieldType, ToFieldType>(std::move(col_to), vec_to, vec_from, input_rows_count,
                                                                            named_from, result_type);
...
struct ConvertImplGenericFromString {
  static ColumnPtr execute(ColumnsWithTypeAndName &arguments, const DataTypePtr &result_type, const ColumnNullable *column_nullable,
                           size_t input_rows_count, const FunctionConvertSettings &settings) {
  ...
  static void executeImpl(const IColumn &column_from, IColumn &column_to, const ISerialization &serialization_from, size_t input_rows_count,
                          const PaddedPODArray<UInt8> *null_map, const IDataType *result_type, const FunctionConvertSettings &settings) {
      ...
      if (!read_buffer.eof()) {
        if constexpr (throw_on_error) {
          if (result_type)
            ...
            throw Exception(ErrorCodes::CANNOT_PARSE_TEXT, "Cannot parse string to column {}. Expected eof", column_to.getName());
        } else {
          if (column_to.size() > i) column_to.popBack(column_to.size() - i);
          column_to.insertDefault();
        }
...
constexpr bool mightBeTime() {
  if constexpr (std::is_same_v<ToDataType, DataTypeTime64>)
    return true;
  else if constexpr (std::is_same_v<Name, NameToTime> || std::is_same_v<Name, NameParseDateTimeBestEffort> ||
                     std::is_same_v<Name, NameParseDateTimeBestEffortOrZero> || std::is_same_v<Name, NameParseDateTimeBestEffortOrNull>)
    return true;
...
class FunctionConvert : public IFunction {
 public:
  ...
  static FunctionPtr create(ContextPtr context) { return std::make_shared<FunctionConvert>(context, default_date_time_overflow_behavior); }

  static FunctionPtr createFromSettings(const FunctionConvertSettings &settings) { return std::make_shared<FunctionConvert>(settings); }
  ...
  explicit FunctionConvert(const FunctionConvertSettings &settings_) : settings(settings_) {}
  ...
  DataTypePtr getReturnTypeImplRemovedNullable(const ColumnsWithTypeAndName &arguments) const {
    ...
    if constexpr (to_decimal) {
      mandatory_args.push_back(
          {"scale", static_cast<FunctionArgumentDescriptor::TypeValidator>(&isNativeInteger), &isColumnConst, "const Integer"});
    }
    ...
    if ((std::is_same_v<Name, NameToString> && !arguments.empty() && (isDateTime64(arguments[0].type) || isDateTime(arguments[0].type)))
        ...
        || std::is_same_v<ToDataType, DataTypeDate>  // TODO: shall we allow timestamp argument for toDate? DateTime knows nothing about
                                                     // timezones and this argument is ignored below.
    ...
    if constexpr (std::is_same_v<ToDataType, DataTypeInterval>) {
      if (isDecimal(arguments[0].type))
        throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT, "Illegal type {} of argument of function {}", arguments[0].type->getName(),
                        getName());
  ...
  ColumnNumbers getArgumentsThatAreAlwaysConstant() const override {
    if constexpr (std::is_same_v<ToDataType, DataTypeString>) return {};
  ...
  ColumnPtr executeImpl(const ColumnsWithTypeAndName &arguments, const DataTypePtr &weird_result_type,
    ...
    try {
      ...
      if (result_type->isNullable() && !std::is_same_v<ToDataType, DataTypeString>) {
        if (result_type->onlyNull()) return result_type->createColumnConstWithDefaultValue(input_rows_count);
        ...
        for (const auto &arg : arguments) {
          ...
          if (result_null_map) {
            ...
            for (size_t i = 0; i < input_rows_count; ++i) result_null_map_data[i] |= null_map[i];
      ...
      if (e.code() == ErrorCodes::ATTEMPT_TO_READ_AFTER_EOF) {
        e.addMessage("Cannot parse " + result_type->getName() + " from " + arguments[0].type->getName() + ", because value is too short");
                 ...
                 e.code() == ErrorCodes::CANNOT_PARSE_IPV4 || e.code() == ErrorCodes::CANNOT_PARSE_IPV6) {
        e.addMessage("Cannot parse " + result_type->getName() + " from " + arguments[0].type->getName());
      }
 ...
 private:
  ...
  ColumnPtr executeInternal(const ColumnsWithTypeAndName &arguments, const DataTypePtr &result_type, size_t input_rows_count,
                            bool to_nullable) const {
    if (arguments.empty())
      ...
      if constexpr (IsDataTypeDecimal<RightDataType>) {
        if constexpr (std::is_same_v<RightDataType, DataTypeDateTime64> || std::is_same_v<RightDataType, DataTypeTime64>) {
          ...
          if (arguments.size() != 2 && arguments.size() != 3)
            throw Exception(ErrorCodes::TOO_FEW_ARGUMENTS_FOR_FUNCTION, "Function {} expects 2 or 3 arguments for DataTypeDateTime64.",
                            getName());
        ...
        switch (settings.date_time_overflow_behavior) {
          case FormatSettings::DateTimeOverflowBehavior::Throw:
            result_column = ConvertImpl<LeftDataType, RightDataType, Name, FormatSettings::DateTimeOverflowBehavior::Throw>::execute(
                arguments, result_type, input_rows_count, from_string_tag, settings, scale);
            break;
          case FormatSettings::DateTimeOverflowBehavior::Ignore:
            result_column = ConvertImpl<LeftDataType, RightDataType, Name, FormatSettings::DateTimeOverflowBehavior::Ignore>::execute(
                arguments, result_type, input_rows_count, from_string_tag, settings, scale);
            break;
          case FormatSettings::DateTimeOverflowBehavior::Saturate:
            result_column = ConvertImpl<LeftDataType, RightDataType, Name, FormatSettings::DateTimeOverflowBehavior::Saturate>::execute(
                arguments, result_type, input_rows_count, from_string_tag, settings, scale);
            break;
        }
        ...
        switch (settings.date_time_overflow_behavior) {
          case FormatSettings::DateTimeOverflowBehavior::Throw:
            result_column = ConvertImpl<LeftDataType, RightDataType, Name, FormatSettings::DateTimeOverflowBehavior::Throw>::execute(
                arguments, result_type, input_rows_count, from_string_tag, settings, dt64->getScale());
            break;
          case FormatSettings::DateTimeOverflowBehavior::Ignore:
            result_column = ConvertImpl<LeftDataType, RightDataType, Name, FormatSettings::DateTimeOverflowBehavior::Ignore>::execute(
                arguments, result_type, input_rows_count, from_string_tag, settings, dt64->getScale());
          ...
          case FormatSettings::DateTimeOverflowBehavior::Saturate:
            ...
            break;
                           ...
                           IsDataTypeDateOrDateTimeOrTime<RightDataType>) {
...
#undef GENERATE_OVERFLOW_MODE_CASE
        ...
        static constexpr bool bad_left = is_decimal<LeftT> || is_floating_point<LeftT> || is_big_int_v<LeftT> || is_signed_v<LeftT>;
        static constexpr bool bad_right = is_decimal<RightT> || is_floating_point<RightT> || is_big_int_v<RightT> || is_signed_v<RightT>;
        ...
        if constexpr ((bad_left && std::is_same_v<RightDataType, DataTypeUUID>) ||
          ...
          result_column =
              ConvertImpl<LeftDataType, RightDataType, Name>::execute(arguments, result_type, input_rows_count, from_string_tag, settings);
        }
      } else
        result_column =
            ConvertImpl<LeftDataType, RightDataType, Name>::execute(arguments, result_type, input_rows_count, from_string_tag, settings);
    ...
    if constexpr (mightBeTime<Name, ToDataType>()) {
      if (isTime64<Name, ToDataType>(arguments)) {
        ...
        {
          if (!callOnIndexAndDataType<DataTypeTime64>(from_type->getTypeId(), call, BehaviourOnErrorFromString::ConvertDefaultBehaviorTag))
            throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT, "Illegal type {} of argument of function {}",
                            arguments[0].type->getName(), getName());
...
class FunctionConvertFromString : public IFunction {
 public:
  ...
  static FunctionPtr createFromSettings(const FunctionConvertSettings &settings) {
    return std::make_shared<FunctionConvertFromString>(settings);
  }
  ...
  DataTypePtr getReturnTypeImpl(const ColumnsWithTypeAndName &arguments) const override {
    ...
    if (isDateTime64<Name, ToDataType>(arguments) || isTime64<Name, ToDataType>(arguments)) {
      ...
      const auto precision_with_timezone_opt = FunctionArgumentDescriptors{
          {"precision", static_cast<FunctionArgumentDescriptor::TypeValidator>(&isUInt8), isColumnConst, "const UInt8"},
          {"timezone", static_cast<FunctionArgumentDescriptor::TypeValidator>(&isStringOrFixedString), isColumnConst,
           "const String or FixedString"}};
      ...
      if (arguments.size() > 1) scale = extractToDecimalScale(arguments[1]);
      ...
      if constexpr (std::is_same_v<ToDataType, DataTypeDateTime>)
        ...
        throw Exception(ErrorCodes::LOGICAL_ERROR, "Unexpected behavior for function {}: DataTypeDateTime64 is not expected here.",
                        getName());
      else if constexpr (to_decimal) {
        UInt64 scale = extractToDecimalScale(arguments[1]);
        ...
        if (!res)
          throw Exception(ErrorCodes::LOGICAL_ERROR,
                          "Unexpected behavior for function {}: createDecimalMaxPrecision didn't return any value.", getName());
      } else
        res = std::make_shared<ToDataType>();
    }

    if constexpr (exception_mode == ConvertFromStringExceptionMode::Null) res = std::make_shared<DataTypeNullable>(res);
  ...
  ColumnPtr executeImpl(const ColumnsWithTypeAndName &arguments, const DataTypePtr &result_type, size_t input_rows_count) const override {
    ...
    if constexpr (to_decimal) {
      ...
      if (isTime64<Name, ToDataType>(arguments)) {
        ...
        if (scale == 0) {
          ...
          result_column = executeInternal<DataTypeTime64>(arguments, result_type, input_rows_count, static_cast<UInt32>(scale));
        }
      } else {
        result_column = executeInternal<ToDataType>(arguments, result_type, input_rows_count, 0);
      }
    ...
    if (!result_column)
      throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT,
                      ...
                      "use function without 'orZero' or 'orNull'.",
                      arguments[0].type->getName(), getName());
...
struct PositiveMonotonicity {
  ...
  static IFunction::Monotonicity get(const IDataType &, const Field &, const Field &) { return {.is_monotonic = true}; }
};
...
struct ToNumberMonotonicity {
  ...
  static IFunction::Monotonicity get(const IDataType &type, const Field &left, const Field &right) {
    ...
    if (low_cardinality) low_cardinality_dictionary_type = low_cardinality->getDictionaryType().get();

    WhichDataType which_type(type);
    WhichDataType which_inner_type = low_cardinality ? WhichDataType(low_cardinality_dictionary_type) : WhichDataType(type);
    ...
    if constexpr (is_floating_point<T>) {
      ...
      if (std::is_same_v<T, Float64> &&
          (which_inner_type.isBFloat16() || which_inner_type.isFloat32() || which_inner_type.isInt8() || which_inner_type.isInt16() ||
           which_inner_type.isInt32() || which_inner_type.isUInt8() || which_inner_type.isUInt16() || which_inner_type.isUInt32()))
        return {.is_monotonic = true, .is_always_monotonic = true, .is_strict = true};
    ...
    auto is_valid_uint64_or_int64_or_null = [&](const Field &f) {
      ...
      auto t = f.getType();
      return t == Field::Types::UInt64 || t == Field::Types::Int64;
    };

    if (!is_valid_uint64_or_int64_or_null(left) || !is_valid_uint64_or_int64_or_null(right)) return {};
    ...
    const bool left_in_first_half = left.isNull() ? from_is_unsigned : (left.safeGet<Int64>() >= 0);

    const bool right_in_first_half = right.isNull() ? !from_is_unsigned : (right.safeGet<Int64>() >= 0);

    /// Size of type is the same.
    if (size_of_from == size_of_to) {
      if (from_is_unsigned == to_is_unsigned) return {.is_monotonic = true, .is_always_monotonic = true, .is_strict = true};

      if (left_in_first_half == right_in_first_half) return {.is_monotonic = true};
    ...
    if (size_of_from < size_of_to) {
      ...
      if (!to_is_unsigned) return {.is_monotonic = true, .is_always_monotonic = true, .is_strict = true};
    ...
    if (size_of_from > size_of_to) {
      ...
      if (divideByRangeOfType(left.safeGet<UInt64>()) != divideByRangeOfType(right.safeGet<UInt64>())) return {};
...
struct ToDateMonotonicity {
  ...
  static IFunction::Monotonicity get(const IDataType &type, const Field &left, const Field &right) {
    ...
    if (which.isDateOrDate32() || which.isTime() || which.isTime64() || which.isDateTime() || which.isDateTime64() || which.isInt8() ||
      ...
      return {.is_monotonic = true, .is_always_monotonic = true};
    } else if (((left.getType() == Field::Types::UInt64 || left.isNull()) && (right.getType() == Field::Types::UInt64 || right.isNull()) &&
                ((left.isNull() || left.safeGet<UInt64>() <= DATE_LUT_MAX_DAY_NUM) &&
                 (right.isNull() || right.safeGet<UInt64>() > DATE_LUT_MAX_DAY_NUM))) ||
               ((left.getType() == Field::Types::Int64 || left.isNull()) && (right.getType() == Field::Types::Int64 || right.isNull()) &&
                ((left.isNull() || left.safeGet<Int64>() <= DATE_LUT_MAX_DAY_NUM) &&
                 (right.isNull() || right.safeGet<Int64>() > DATE_LUT_MAX_DAY_NUM))) ||
               (((left.getType() == Field::Types::Float64 || left.isNull()) &&
                 (right.getType() == Field::Types::Float64 || right.isNull()) &&
                 ((left.isNull() || left.safeGet<Float64>() <= DATE_LUT_MAX_DAY_NUM) &&
                  (right.isNull() || right.safeGet<Float64>() > DATE_LUT_MAX_DAY_NUM)))) ||
               !isNativeNumber(type)) {
      return {};
    } else {
      return {.is_monotonic = true, .is_always_monotonic = true};
...
struct ToDateTimeMonotonicity {
  ...
  static IFunction::Monotonicity get(const IDataType &type, const Field &, const Field &) {
    if (type.isValueRepresentedByNumber()) {
      ...
      if ((std::is_same_v<T, DataTypeTime> || std::is_same_v<T, DataTypeTime64>) && !which.isTimeOrTime64()) return {};
...
struct ToStringMonotonicity {
  ...
  static IFunction::Monotonicity get(const IDataType &type, const Field &left, const Field &right) {
    ...
    if (WhichDataType(*type_ptr).isEnum()) return not_monotonic;
    ...
    /// `toString` function is monotonous if the argument is Date or Date32 or DateTime or String, or non-negative numbers with the same
    /// number of symbols.
    if (checkDataTypes<DataTypeDate, DataTypeDate32, DataTypeDateTime, DataTypeTime, DataTypeString>(type_ptr)) return positive;
    ...
    if (left.getType() == Field::Types::Int64 && right.getType() == Field::Types::Int64) {
      return (left.safeGet<Int64>() == 0 && right.safeGet<Int64>() == 0) ||
                     (left.safeGet<Int64>() > 0 && right.safeGet<Int64>() > 0 &&
                      floor(log10(left.safeGet<Int64>())) == floor(log10(right.safeGet<Int64>())))
                 ? positive
                 : not_monotonic;
    }
...
struct NameToUInt128 {
  static constexpr auto name = "toUInt128";
};
struct NameToUInt256 {
  static constexpr auto name = "toUInt256";
};
struct NameToInt8 {
...
struct NameToInt256 {
  static constexpr auto name = "toInt256";
};
struct NameToBFloat16 {
  static constexpr auto name = "toBFloat16";
};
...
template <typename DataType, FormatSettings::DateTimeOverflowBehavior date_time_overflow_behavior = default_date_time_overflow_behavior>
struct FunctionTo;
...
template <>
struct FunctionTo<DataTypeUInt128> {
  using Type = FunctionToUInt128;
};
template <>
struct FunctionTo<DataTypeUInt256> {
  using Type = FunctionToUInt256;
};
template <>
...
template <>
struct FunctionTo<DataTypeInt256> {
  using Type = FunctionToInt256;
};
template <>
struct FunctionTo<DataTypeBFloat16> {
  using Type = FunctionToBFloat16;
};
template <>
...
template <>
struct FunctionTo<DataTypeDate32> {
  using Type = FunctionToDate32;
};
template <>
...
template <>
struct FunctionTo<DataTypeTime64> {
  using Type = FunctionToTime64;
};
template <>
...
template <>
struct FunctionTo<DataTypeDateTime64> {
  using Type = FunctionToDateTime64;
};
...
struct NameToUInt128OrZero {
  static constexpr auto name = "toUInt128OrZero";
};
struct NameToUInt256OrZero {
  static constexpr auto name = "toUInt256OrZero";
};
...
struct NameToInt128OrZero {
  static constexpr auto name = "toInt128OrZero";
};
struct NameToInt256OrZero {
  static constexpr auto name = "toInt256OrZero";
};
struct NameToBFloat16OrZero {
  static constexpr auto name = "toBFloat16OrZero";
};
...
struct NameToDate32OrZero {
  static constexpr auto name = "toDate32OrZero";
};
struct NameToTimeOrZero {
  static constexpr auto name = "toTimeOrZero";
};
struct NameToTime64OrZero {
  static constexpr auto name = "toTime64OrZero";
};
struct NameToDateTimeOrZero {
  static constexpr auto name = "toDateTimeOrZero";
};
struct NameToDateTime64OrZero {
  static constexpr auto name = "toDateTime64OrZero";
};
struct NameToDecimal32OrZero {
  static constexpr auto name = "toDecimal32OrZero";
};
struct NameToDecimal64OrZero {
  static constexpr auto name = "toDecimal64OrZero";
};
struct NameToDecimal128OrZero {
  static constexpr auto name = "toDecimal128OrZero";
};
struct NameToDecimal256OrZero {
  static constexpr auto name = "toDecimal256OrZero";
};
struct NameToUUIDOrZero {
  static constexpr auto name = "toUUIDOrZero";
};
struct NameToIPv4OrZero {
  static constexpr auto name = "toIPv4OrZero";
};
struct NameToIPv6OrZero {
  static constexpr auto name = "toIPv6OrZero";
};
...
using FunctionToDateTime64OrZero =
    FunctionConvertFromString<DataTypeDateTime64, NameToDateTime64OrZero, ConvertFromStringExceptionMode::Zero>;
using FunctionToDecimal32OrZero =
    FunctionConvertFromString<DataTypeDecimal<Decimal32>, NameToDecimal32OrZero, ConvertFromStringExceptionMode::Zero>;
using FunctionToDecimal64OrZero =
    FunctionConvertFromString<DataTypeDecimal<Decimal64>, NameToDecimal64OrZero, ConvertFromStringExceptionMode::Zero>;
using FunctionToDecimal128OrZero =
    FunctionConvertFromString<DataTypeDecimal<Decimal128>, NameToDecimal128OrZero, ConvertFromStringExceptionMode::Zero>;
using FunctionToDecimal256OrZero =
    FunctionConvertFromString<DataTypeDecimal<Decimal256>, NameToDecimal256OrZero, ConvertFromStringExceptionMode::Zero>;
...
struct NameToUInt128OrNull {
  static constexpr auto name = "toUInt128OrNull";
};
struct NameToUInt256OrNull {
  static constexpr auto name = "toUInt256OrNull";
};
...
struct NameToInt128OrNull {
  static constexpr auto name = "toInt128OrNull";
};
struct NameToInt256OrNull {
  static constexpr auto name = "toInt256OrNull";
};
struct NameToBFloat16OrNull {
  static constexpr auto name = "toBFloat16OrNull";
};
...
struct NameToDate32OrNull {
  static constexpr auto name = "toDate32OrNull";
};
struct NameToTimeOrNull {
  static constexpr auto name = "toTimeOrNull";
};
struct NameToTime64OrNull {
  static constexpr auto name = "toTime64OrNull";
};
struct NameToDateTimeOrNull {
  static constexpr auto name = "toDateTimeOrNull";
};
struct NameToDateTime64OrNull {
  static constexpr auto name = "toDateTime64OrNull";
};
struct NameToDecimal32OrNull {
  static constexpr auto name = "toDecimal32OrNull";
};
struct NameToDecimal64OrNull {
  static constexpr auto name = "toDecimal64OrNull";
};
struct NameToDecimal128OrNull {
  static constexpr auto name = "toDecimal128OrNull";
};
struct NameToDecimal256OrNull {
  static constexpr auto name = "toDecimal256OrNull";
};
struct NameToUUIDOrNull {
  static constexpr auto name = "toUUIDOrNull";
};
struct NameToIPv4OrNull {
  static constexpr auto name = "toIPv4OrNull";
};
struct NameToIPv6OrNull {
  static constexpr auto name = "toIPv6OrNull";
};
...
using FunctionToDateTime64OrNull =
    FunctionConvertFromString<DataTypeDateTime64, NameToDateTime64OrNull, ConvertFromStringExceptionMode::Null>;
using FunctionToDecimal32OrNull =
    FunctionConvertFromString<DataTypeDecimal<Decimal32>, NameToDecimal32OrNull, ConvertFromStringExceptionMode::Null>;
using FunctionToDecimal64OrNull =
    FunctionConvertFromString<DataTypeDecimal<Decimal64>, NameToDecimal64OrNull, ConvertFromStringExceptionMode::Null>;
using FunctionToDecimal128OrNull =
    FunctionConvertFromString<DataTypeDecimal<Decimal128>, NameToDecimal128OrNull, ConvertFromStringExceptionMode::Null>;
using FunctionToDecimal256OrNull =
    FunctionConvertFromString<DataTypeDecimal<Decimal256>, NameToDecimal256OrNull, ConvertFromStringExceptionMode::Null>;
...
extern template class FunctionConvertFromString<DataTypeDateTime, NameParseDateTimeBestEffort, ConvertFromStringExceptionMode::Throw,
                                                ConvertFromStringParsingMode::BestEffort>;
extern template class FunctionConvertFromString<DataTypeDateTime, NameParseDateTimeBestEffortOrZero, ConvertFromStringExceptionMode::Zero,
                                                ConvertFromStringParsingMode::BestEffort>;
extern template class FunctionConvertFromString<DataTypeDateTime, NameParseDateTimeBestEffortOrNull, ConvertFromStringExceptionMode::Null,
                                                ConvertFromStringParsingMode::BestEffort>;

extern template class FunctionConvertFromString<DataTypeDateTime, NameParseDateTimeBestEffortUS, ConvertFromStringExceptionMode::Throw,
                                                ConvertFromStringParsingMode::BestEffortUS>;
extern template class FunctionConvertFromString<DataTypeDateTime, NameParseDateTimeBestEffortUSOrZero, ConvertFromStringExceptionMode::Zero,
                                                ConvertFromStringParsingMode::BestEffortUS>;
extern template class FunctionConvertFromString<DataTypeDateTime, NameParseDateTimeBestEffortUSOrNull, ConvertFromStringExceptionMode::Null,
                                                ConvertFromStringParsingMode::BestEffortUS>;

extern template class FunctionConvertFromString<DataTypeDateTime, NameParseDateTime32BestEffort, ConvertFromStringExceptionMode::Throw,
                                                ConvertFromStringParsingMode::BestEffort>;
extern template class FunctionConvertFromString<DataTypeDateTime, NameParseDateTime32BestEffortOrZero, ConvertFromStringExceptionMode::Zero,
                                                ConvertFromStringParsingMode::BestEffort>;
extern template class FunctionConvertFromString<DataTypeDateTime, NameParseDateTime32BestEffortOrNull, ConvertFromStringExceptionMode::Null,
                                                ConvertFromStringParsingMode::BestEffort>;

extern template class FunctionConvertFromString<DataTypeDateTime64, NameParseDateTime64BestEffort, ConvertFromStringExceptionMode::Throw,
                                                ConvertFromStringParsingMode::BestEffort>;
extern template class FunctionConvertFromString<DataTypeDateTime64, NameParseDateTime64BestEffortOrZero,
                                                ConvertFromStringExceptionMode::Zero, ConvertFromStringParsingMode::BestEffort>;
extern template class FunctionConvertFromString<DataTypeDateTime64, NameParseDateTime64BestEffortOrNull,
                                                ConvertFromStringExceptionMode::Null, ConvertFromStringParsingMode::BestEffort>;

extern template class FunctionConvertFromString<DataTypeDateTime64, NameParseDateTime64BestEffortUS, ConvertFromStringExceptionMode::Throw,
                                                ConvertFromStringParsingMode::BestEffortUS>;
extern template class FunctionConvertFromString<DataTypeDateTime64, NameParseDateTime64BestEffortUSOrZero,
                                                ConvertFromStringExceptionMode::Zero, ConvertFromStringParsingMode::BestEffortUS>;
extern template class FunctionConvertFromString<DataTypeDateTime64, NameParseDateTime64BestEffortUSOrNull,
                                                ConvertFromStringExceptionMode::Null, ConvertFromStringParsingMode::BestEffortUS>;

using FunctionParseDateTimeBestEffort =
    FunctionConvertFromString<DataTypeDateTime, NameParseDateTimeBestEffort, ConvertFromStringExceptionMode::Throw,
                              ConvertFromStringParsingMode::BestEffort>;
using FunctionParseDateTimeBestEffortOrZero =
    FunctionConvertFromString<DataTypeDateTime, NameParseDateTimeBestEffortOrZero, ConvertFromStringExceptionMode::Zero,
                              ConvertFromStringParsingMode::BestEffort>;
using FunctionParseDateTimeBestEffortOrNull =
    FunctionConvertFromString<DataTypeDateTime, NameParseDateTimeBestEffortOrNull, ConvertFromStringExceptionMode::Null,
                              ConvertFromStringParsingMode::BestEffort>;

using FunctionParseDateTimeBestEffortUS =
    FunctionConvertFromString<DataTypeDateTime, NameParseDateTimeBestEffortUS, ConvertFromStringExceptionMode::Throw,
                              ConvertFromStringParsingMode::BestEffortUS>;
using FunctionParseDateTimeBestEffortUSOrZero =
    FunctionConvertFromString<DataTypeDateTime, NameParseDateTimeBestEffortUSOrZero, ConvertFromStringExceptionMode::Zero,
                              ConvertFromStringParsingMode::BestEffortUS>;
using FunctionParseDateTimeBestEffortUSOrNull =
    FunctionConvertFromString<DataTypeDateTime, NameParseDateTimeBestEffortUSOrNull, ConvertFromStringExceptionMode::Null,
                              ConvertFromStringParsingMode::BestEffortUS>;

using FunctionParseDateTime32BestEffort =
    FunctionConvertFromString<DataTypeDateTime, NameParseDateTime32BestEffort, ConvertFromStringExceptionMode::Throw,
                              ConvertFromStringParsingMode::BestEffort>;
using FunctionParseDateTime32BestEffortOrZero =
    FunctionConvertFromString<DataTypeDateTime, NameParseDateTime32BestEffortOrZero, ConvertFromStringExceptionMode::Zero,
                              ConvertFromStringParsingMode::BestEffort>;
using FunctionParseDateTime32BestEffortOrNull =
    FunctionConvertFromString<DataTypeDateTime, NameParseDateTime32BestEffortOrNull, ConvertFromStringExceptionMode::Null,
                              ConvertFromStringParsingMode::BestEffort>;

using FunctionParseDateTime64BestEffort =
    FunctionConvertFromString<DataTypeDateTime64, NameParseDateTime64BestEffort, ConvertFromStringExceptionMode::Throw,
                              ConvertFromStringParsingMode::BestEffort>;
using FunctionParseDateTime64BestEffortOrZero =
    FunctionConvertFromString<DataTypeDateTime64, NameParseDateTime64BestEffortOrZero, ConvertFromStringExceptionMode::Zero,
                              ConvertFromStringParsingMode::BestEffort>;
using FunctionParseDateTime64BestEffortOrNull =
    FunctionConvertFromString<DataTypeDateTime64, NameParseDateTime64BestEffortOrNull, ConvertFromStringExceptionMode::Null,
                              ConvertFromStringParsingMode::BestEffort>;

using FunctionParseDateTime64BestEffortUS =
    FunctionConvertFromString<DataTypeDateTime64, NameParseDateTime64BestEffortUS, ConvertFromStringExceptionMode::Throw,
                              ConvertFromStringParsingMode::BestEffortUS>;
using FunctionParseDateTime64BestEffortUSOrZero =
    FunctionConvertFromString<DataTypeDateTime64, NameParseDateTime64BestEffortUSOrZero, ConvertFromStringExceptionMode::Zero,
                              ConvertFromStringParsingMode::BestEffortUS>;
using FunctionParseDateTime64BestEffortUSOrNull =
    FunctionConvertFromString<DataTypeDateTime64, NameParseDateTime64BestEffortUSOrNull, ConvertFromStringExceptionMode::Null,
                              ConvertFromStringParsingMode::BestEffortUS>;
...
class ExecutableFunctionCast : public IExecutableFunction {
 public:
  ...
  explicit ExecutableFunctionCast(WrapperType &&wrapper_function_, const char *name_, std::optional<CastDiagnostic> diagnostic_)
 ...
 protected:
  ColumnPtr executeImpl(const ColumnsWithTypeAndName &arguments, const DataTypePtr &result_type, size_t input_rows_count) const override {
    ...
    if (arguments.size() > 2) new_arguments.insert(std::end(new_arguments), std::next(std::begin(arguments), 2), std::end(arguments));

    try {
      return wrapper_function(new_arguments, result_type, nullptr, input_rows_count);
    } catch (Exception &e) {
      if (diagnostic)
        e.addMessage("while converting source column " + backQuoteIfNeed(diagnostic->column_from) + " to destination column " +
                     backQuoteIfNeed(diagnostic->column_to));
      throw;
    }
  }
...
class FunctionCast final : public IFunctionBase {
 public:
  ...
  FunctionCast(ContextPtr context, const char *cast_name_, MonotonicityForRange &&monotonicity_for_range_, const DataTypes &argument_types_,
               const DataTypePtr &return_type_, std::optional<CastDiagnostic> diagnostic_, CastType cast_type_,
               FormatSettings::DateTimeOverflowBehavior date_time_overflow_behavior)
      : settings(context, date_time_overflow_behavior),
 ...
 private:
  ...
  static WrapperType createFunctionAdaptor(FunctionPtr function, const DataTypePtr &from_type) {
    ...
    return [function_adaptor](ColumnsWithTypeAndName &arguments, const DataTypePtr &result_type, const ColumnNullable *,
  ...
  static ColumnPtr convertArrayToQBit(ColumnsWithTypeAndName &arguments, const DataTypePtr &, const ColumnNullable *nullable_source,
  ...
  static ColumnPtr createVariantFromDescriptorsAndOneNonEmptyVariant(const DataTypes &variant_types, const ColumnPtr &discriminators,
                                                                     const ColumnPtr &variant, ColumnVariant::Discriminator variant_discr);
...
FunctionBasePtr createFunctionBaseCast(ContextPtr context, const char *name, const ColumnsWithTypeAndName &arguments,
                                       const DataTypePtr &return_type, std::optional<CastDiagnostic> diagnostic, CastType cast_type);
