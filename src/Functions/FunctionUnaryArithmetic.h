...
struct UnaryOperationImpl {
  ...
  MULTITARGET_FUNCTION_X86_V4_V3(MULTITARGET_FUNCTION_HEADER(static void NO_INLINE), vectorImpl,
                                 MULTITARGET_FUNCTION_BODY((const ArrayA &a, ArrayC &c)  /// NOLINT
                                                           {
                                                             size_t size = a.size();
...
struct FixedStringUnaryOperationImpl {
  MULTITARGET_FUNCTION_X86_V4_V3(MULTITARGET_FUNCTION_HEADER(static void NO_INLINE), vectorImpl,
                                 MULTITARGET_FUNCTION_BODY((const ColumnFixedString::Chars &a,  /// NOLINT
...
struct StringUnaryOperationReduceImpl {
  MULTITARGET_FUNCTION_X86_V4_V3(MULTITARGET_FUNCTION_HEADER(static UInt64 NO_INLINE), vectorImpl,
                                 MULTITARGET_FUNCTION_BODY((const UInt8 *start, const UInt8 *end)  /// NOLINT
                                                           {
                                                             UInt64 res = 0;
                                                             while (start < end) res += Op::apply(*start++);
                                                             return res;
                                                           }))
...
class FunctionUnaryArithmetic : public IFunction {
  ...
  static bool castType(const IDataType *type, F &&f) {
    return castTypeToEither<DataTypeUInt8, DataTypeUInt16, DataTypeUInt32, DataTypeUInt64, DataTypeUInt128, DataTypeUInt256, DataTypeInt8,
                            DataTypeInt16, DataTypeInt32, DataTypeInt64, DataTypeInt128, DataTypeInt256, DataTypeFloat32, DataTypeFloat64,
                            DataTypeDecimal<Decimal32>, DataTypeDecimal<Decimal64>, DataTypeDecimal<Decimal128>,
                            DataTypeDecimal<Decimal256>, DataTypeFixedString, DataTypeString, DataTypeInterval>(type, std::forward<F>(f));
  }

  static FunctionOverloadResolverPtr getFunctionForTupleArithmetic(const DataTypePtr &type, ContextPtr context) {
    if (!isTuple(type)) return {};

    /// Special case when the function is negate, argument is tuple.
    /// We construct another function (example: tupleNegate) and call it.

    if constexpr (!IsUnaryOperation<Op>::negate) return {};
 ...
 public:
  ...
  static DataTypePtr getReturnTypeImplStatic(const DataTypes &arguments, ContextPtr context) {
    ...
    bool valid = castType(arguments[0].get(), [&](const auto &type) {
      ...
      if constexpr (std::is_same_v<DataTypeFixedString, DataType>) {
        if constexpr (!allow_string_or_fixed_string) return false;
        ...
        if constexpr (!IsUnaryOperation<Op>::negate) return false;
  ...
  ColumnPtr executeImpl(const ColumnsWithTypeAndName &arguments, const DataTypePtr &result_type, size_t input_rows_count) const override {
        ...
        if constexpr (allow_string_or_fixed_string) {
          if (const auto *col = checkAndGetColumn<ColumnString>(arguments[0].column.get())) {
            if constexpr (is_bit_count) {
              ...
              for (size_t i = 0; i < size; ++i) {
                vec_res[i] = StringUnaryOperationReduceImpl<Op<UInt8>>::vector(chars.data() + offsets[i - 1], chars.data() + offsets[i]);
              }
              result_column = std::move(col_res);
              return true;
            } else {
    ...
    if (!valid) throw Exception(ErrorCodes::LOGICAL_ERROR, "{}'s argument does not match the expected data type", getName());
  ...
  bool isCompilableImpl(const DataTypes &arguments, const DataTypePtr &result_type) const override {
    if (1 != arguments.size()) return false;

    if (!canBeNativeType(*arguments[0]) || !canBeNativeType(*result_type)) return false;
    ...
    return castType(arguments[0].get(), [&](const auto &type) {
      ...
      if constexpr (std::is_same_v<DataTypeFixedString, DataType> || std::is_same_v<DataTypeString, DataType>)
        ...
        if constexpr (!std::is_same_v<T1, InvalidType> && !IsDataTypeDecimal<DataType> && Op<T0>::compilable) return true;
      }
