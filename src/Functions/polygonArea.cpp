...
class FunctionPolygonArea : public IFunction {
 public:
  ...
  static FunctionPtr create(ContextPtr) { return std::make_shared<FunctionPolygonArea>(); }
  ...
  ColumnPtr executeImpl(const ColumnsWithTypeAndName &arguments, const DataTypePtr & /*result_type*/,
    ...
    callOnGeometryDataType<Point>(arguments[0].type, [&](const auto &type) {
      ...
      if constexpr (std::is_same_v<ColumnToPointsConverter<Point>, Converter>)
        ...
        for (size_t i = 0; i < input_rows_count; ++i) res_data.emplace_back(boost::geometry::area(geometries[i]));
      }
    });
...
Calculates the surface area of a polygon.
  ...
  FunctionDocumentation::Examples examples_spherical = {{"Spherical example",
                                                         R"(
