...
class FunctionPrintf : public IFunction {
 private:
  ...
  struct Instruction {
    ...
    ColumnWithTypeAndName execute() const {
      if (is_literal) return executeLiteral(format);
      if (isColumnConst(*input.column)) return executeConstant(input);
   ...
   private:
    ...
    bool executeNumber(const IColumn &column, ColumnString::Chars &res_chars, ColumnString::Offsets &res_offsets) const {
      ...
      if (!concrete_column) return false;
    ...
    ColumnWithTypeAndName executeNonconstant(const ColumnWithTypeAndName &arg) const {
      ...
      if (which.isNativeNumber() &&
          (executeNumber<UInt8>(*arg.column, res_chars, res_offsets) || executeNumber<UInt16>(*arg.column, res_chars, res_offsets) ||
           executeNumber<UInt32>(*arg.column, res_chars, res_offsets) || executeNumber<UInt64>(*arg.column, res_chars, res_offsets) ||
           executeNumber<Int8>(*arg.column, res_chars, res_offsets) || executeNumber<Int16>(*arg.column, res_chars, res_offsets) ||
           executeNumber<Int32>(*arg.column, res_chars, res_offsets) || executeNumber<Int64>(*arg.column, res_chars, res_offsets) ||
           executeNumber<Float32>(*arg.column, res_chars, res_offsets) || executeNumber<Float64>(*arg.column, res_chars, res_offsets))) {
        return {std::move(res_col), std::make_shared<DataTypeString>(), arg.name};
      }
      if (which.isStringOrFixedString() && (executeString<ColumnString>(*arg.column, res_chars, res_offsets) ||
                                            executeString<ColumnFixedString>(*arg.column, res_chars, res_offsets))) {
        return {std::move(res_col), std::make_shared<DataTypeString>(), arg.name};
      }
      throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT,
                      "The argument type of function {} is {}, but native numeric or string type is expected", FunctionPrintf::name,
                      arg.type->getName());
    }
  };
 ...
 public:
  ...
  explicit FunctionPrintf(ContextPtr context) : function_concat(FunctionFactory::instance().get("concat", context)) {}
  ...
  DataTypePtr getReturnTypeImpl(const ColumnsWithTypeAndName &arguments) const override {
    auto is_native_number_or_string = [](const IDataType &type) { return isNativeNumber(type) || isStringOrFixedString(type); };
  ...
  ColumnPtr executeImpl(const ColumnsWithTypeAndName &arguments, const DataTypePtr &, size_t input_rows_count) const override {
    ...
    if (!c0_const_string) throw Exception(ErrorCodes::ILLEGAL_COLUMN, "First argument of function {} must be constant string", getName());
    ...
    for (size_t i = 0; i < instructions.size(); ++i) {
      ...
      try {
        ...
        if (instruction.is_literal)
          throw Exception(ErrorCodes::BAD_ARGUMENTS, "Bad format {} in function {} without input argument, reason: {}", instruction.format,
                          getName(), e.what());
        throw Exception(ErrorCodes::BAD_ARGUMENTS, "Bad format {} in function {} with {} as input argument, reason: {}",
                        instructions[i].format, getName(), instruction.input.dumpStructure(), e.what());
      }
    }

    auto res = function_concat->build(concat_args)
                   ->execute(concat_args, std::make_shared<DataTypeString>(), input_rows_count, /* dry_run = */ false);
    return res;
  }
    ...
    auto check_index_range = [&](size_t idx) {
      if (idx >= arguments.size())
        throw Exception(ErrorCodes::NUMBER_OF_ARGUMENTS_DOESNT_MATCH,
                        "Number of arguments for function {} doesn't match: passed {}, but format is {}", getName(), arguments.size(),
                        format);
    };
...
Literal `%` character can be escaped by `%%`.
  ...
  FunctionDocumentation::Examples examples = {{"C++-style formatting", "SELECT printf('%%%s %s %d', 'Hello', 'World', 2024);",
                                               R"(
