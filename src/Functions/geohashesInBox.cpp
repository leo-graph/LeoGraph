...
class FunctionGeohashesInBox : public IFunction {
 public:
  ...
  DataTypePtr getReturnTypeImpl(const ColumnsWithTypeAndName &arguments) const override {
    FunctionArgumentDescriptors args{{"longitute_min", static_cast<FunctionArgumentDescriptor::TypeValidator>(&isFloat), nullptr, "Float*"},
    ...
    if (!(arguments[0].type->equals(*arguments[1].type) && arguments[0].type->equals(*arguments[2].type) &&
      ...
      throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT,
                      ...
                      "instead they are:{}, {}, {}, {}.",
                      getName(), arguments[0].type->getName(), arguments[1].type->getName(), arguments[2].type->getName(),
  ...
  void execute(const IColumn *lon_min_column, const IColumn *lat_min_column, const IColumn *lon_max_column, const IColumn *lat_max_column,
               const IColumn *precision_column, ColumnPtr &result, size_t input_rows_count) const {
    ...
    if (lon_min_const) lon_min_column = &lon_min_const->getDataColumn();
    if (lat_min_const) lat_min_column = &lat_min_const->getDataColumn();
    if (lon_max_const) lon_max_column = &lon_max_const->getDataColumn();
    if (lat_max_const) lat_max_column = &lat_max_const->getDataColumn();
    if (precision_const) precision_column = &precision_const->getDataColumn();
    ...
    if (!lon_min || !lat_min || !lon_max || !lat_max || !precision) {
      throw Exception(ErrorCodes::LOGICAL_ERROR, "Unsupported argument types for function {} : {}, {}, {}, {}.", getName(),
                      lon_min_column->getName(), lat_min_column->getName(), lon_max_column->getName(), lat_max_column->getName());
    }
    ...
    for (size_t row = 0; row < input_rows_count; ++row) {
      ...
      const auto prepared_args = geohashesInBoxPrepare(lon_min_value, lat_min_value, lon_max_value, lat_max_value, precision_value);

      if (prepared_args.items_count > max_array_size) {
        throw Exception(ErrorCodes::TOO_LARGE_ARRAY_SIZE,
                        "{} would produce {} array elements, "
      ...
      for (UInt64 i = 1; i <= prepared_args.items_count; ++i) res_strings_offsets.push_back(starting_offset + prepared_args.precision * i);
    ...
    if (!res_offsets.empty() && res_offsets.back() != res_strings.size()) {
      throw Exception(ErrorCodes::LOGICAL_ERROR, "Array column size mismatch (internal logical error){} != {}", res_offsets.back(),
                      std::to_string(res_strings.size()));
    }
...
This function throws an exception if the size of the resulting array exceeds more than 10,000,000 items.
  ...
  FunctionDocumentation::Arguments arguments = {{"longitude_min", "Minimum longitude. Range: `[-180°, 180°]`.", {"Float32", "Float64"}},
  ...
  FunctionDocumentation::ReturnedValue returned_value = {
      "Returns an array of precision-long strings of geohash-boxes covering the provided area, or an empty array if the minimum longitude "
      "and latitude values aren't less than the corresponding maximum values.",
      {"Array(String)"}};
  FunctionDocumentation::Examples examples = {{"Basic usage", "SELECT geohashesInBox(24.48, 40.56, 24.785, 40.81, 4) AS thasos",
                                               R"(
┌─thasos──────────────────────────────────────┐
