...
template <class T>
concept Tokenizer = requires(T tok, std::string_view text, std::vector<std::string_view>& tokens, const std::string_view* start, size_t n,
                             std::string& ngram) {
...
struct BytePolicy {
  ...
  void tokenize(std::string_view text, std::vector<std::string_view>& tokens) const {
    ...
    for (size_t i = 0; i < text.size(); ++i) tokens.emplace_back(text.data() + i, 1);
  }

  void join(const std::string_view* start, size_t n, std::string& ngram) const {
    chassert(n != 0);
    ngram.resize(n);
    for (size_t i = 0; i < n; ++i) ngram[i] = (*start++)[0];
  }
};
...
struct CodePointPolicy {
  ...
  void join(const std::string_view* start, size_t n, std::string& ngram) const {
    chassert(n != 0);
    size_t total = 0;
    for (size_t i = 0; i < n; ++i) total += start[i].size();
    ngram.resize(total);
...
class NaiveBayesClassifier {
 ...
 public:
  ...
  NaiveBayesClassifier(const String& model_name, const String& model_path, ProbabilityMap&& priors, const UInt32 given_n,
                       const double given_alpha, Tok given_tokenizer = {})
      : n(given_n), alpha(given_alpha), start_token(Tok::start_token), end_token(Tok::end_token), tokenizer(std::move(given_tokenizer)) {
    ...
    while (!in.eof()) {
      ...
      if (ngram_length > MAX_NGRAM_LENGTH)
        throw Exception(ErrorCodes::CORRUPTED_DATA, "Corrupt model {} of model {}: ngram length {} exceeds maximum of {}", model_path,
                        model_name, ngram_length, MAX_NGRAM_LENGTH);

      if (ngram_length == 0)
        throw Exception(ErrorCodes::CORRUPTED_DATA, "Corrupt model {} of model {}: ngram length given is 0", model_path, model_name);
    ...
    if (priors.empty()) {
      ...
      for (const auto& [class_id, _] : class_totals) log_class_priors[class_id] = uniform_log_prob;
    } else  /// Priors are provided
    {
      if (priors.size() != class_totals.size()) {
        throw Exception(ErrorCodes::BAD_ARGUMENTS,
                        ...
                        priors.size(), class_totals.size(), model_name);
      }
      for (const auto& prior : priors) {
        ...
        if (!class_totals.contains(class_id))  /// Class present in config's <priors> not found in model
        {
          ...
          for (const auto& class_entry : class_totals) available_classes.push_back(class_entry.getKey());

          throw Exception(ErrorCodes::BAD_ARGUMENTS,
                          "Class {} from <priors> not found in the model at {} of model {}. Available classes: {}", class_id, model_path,
                          model_name, fmt::join(available_classes, ", "));
        }
      }

      for (const auto& [class_id, prior] : priors) log_class_priors[class_id] = std::log(prior);
    }
  ...
  UInt32 classify(const String& input) const {
    ...
    for (const auto& [class_id, prior] : log_class_priors) class_log_probabilities[class_id] = prior;
      ...
      for (size_t i = 0; i + n <= tokens.size(); ++i) {
        ...
        for (const auto& class_entry : class_totals) {
          ...
          if (token_exists) {
            ...
            if (it != token_class_map->end()) count = static_cast<double>(it->getMapped());
          }
  ...
  UInt64 getAllocatedBytes() const {
    ...
    for (const auto& m : all_ngram_class_counts) total += m.getBufferSizeInBytes();
