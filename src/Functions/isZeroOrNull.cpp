...
class FunctionIsZeroOrNull : public IFunction {
 public:
  ...
  static FunctionPtr create(ContextPtr) { return std::make_shared<FunctionIsZeroOrNull>(); }
  ...
  DataTypePtr getReturnTypeImpl(const DataTypes &types) const override {
    if (!isNumber(removeNullable(types.at(0))) && !isNothing(removeNullable(types.at(0))))
      throw Exception(ErrorCodes::BAD_ARGUMENTS, "The argument of function {} must have simple numeric type, possibly Nullable or Null",
                      name);
  ...
  ColumnPtr executeImpl(const ColumnsWithTypeAndName &arguments, const DataTypePtr &, size_t input_rows_count) const override {
    if (isNothing(removeNullable(arguments[0].type))) return DataTypeUInt8{}.createColumnConst(input_rows_count, 1);
    ...
    if (const ColumnNullable *input_column_nullable = checkAndGetColumn<ColumnNullable>(input_column.get())) {
      ...
      if (!castTypeToEither<ColumnUInt8, ColumnUInt16, ColumnUInt32, ColumnUInt64, ColumnInt8, ColumnInt16, ColumnInt32, ColumnInt64,
                            ColumnFloat32, ColumnFloat64>(nested_column, [&](const auto &column) {
            auto col = ColumnUInt8::create(input_rows_count);
            processNullable(column.getData(), null_map, col->getData(), input_rows_count);
            res = std::move(col);
            return true;
          })) {
        throw Exception(ErrorCodes::ILLEGAL_COLUMN, "The argument of function {} must have simple numeric type, possibly Nullable or Null",
                        name);
      }
    } else {
      if (!castTypeToEither<ColumnUInt8, ColumnUInt16, ColumnUInt32, ColumnUInt64, ColumnInt8, ColumnInt16, ColumnInt32, ColumnInt64,
                            ColumnFloat32, ColumnFloat64>(input_column.get(), [&](const auto &column) {
            auto col = ColumnUInt8::create(input_rows_count);
 ...
 private:
  ...
  void processNotNullable(const InputData &input_data, ColumnUInt8::Container &result_data, size_t input_rows_count) const {
    for (size_t i = 0; i < input_rows_count; ++i) result_data[i] = input_data[i] == 0;
  }

  template <typename InputData>
  void processNullable(const InputData &input_data, const NullMap &input_null_map, ColumnUInt8::Container &result_data,
                       size_t input_rows_count) const {
    for (size_t i = 0; i < input_rows_count; ++i) result_data[i] = input_null_map[i] || input_data[i] == 0;
  }
};
...
Checks if the argument is either zero (`0`) or `NULL`.
  ...
  FunctionDocumentation::Arguments arguments = {{"x", "A numeric value.", {"UInt"}}};
  FunctionDocumentation::ReturnedValue returned_value = {"Returns `1` if `x` is `NULL` or equal to zero, otherwise `0`.",
