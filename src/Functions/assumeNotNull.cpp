...
class FunctionAssumeNotNull : public IFunction {
 public:
  ...
  static FunctionPtr create(ContextPtr) { return std::make_shared<FunctionAssumeNotNull>(); }
  ...
  Monotonicity getMonotonicityForRange(const IDataType &type, const Field & /*left*/, const Field &right) const override {
    ...
    if (can_contain_null && right.isNull()) return {};

    return {.is_monotonic = true, .is_positive = true, .is_always_monotonic = !can_contain_null};
  }

  DataTypePtr getReturnTypeImpl(const ColumnsWithTypeAndName &arguments) const override { return removeNullable(arguments[0].type); }
  ...
  ColumnPtr executeImpl(const ColumnsWithTypeAndName &arguments, const DataTypePtr &, size_t) const override {
    ...
    if (const auto *nullable_col = checkAndGetColumn<ColumnNullable>(&*col)) return nullable_col->getNestedColumnPtr();
    return col;
  }
...
See also: functions [`ifNull`](#ifNull) and [`coalesce`](#coalesce).
  ...
  FunctionDocumentation::Arguments arguments = {{"x", "The original value of any nullable type.", {"Nullable(T)"}}};
  FunctionDocumentation::ReturnedValue returned_value = {
      "Returns the non-nullable value, if the original value was not `NULL`, otherwise an arbitrary value, if the input value is `NULL`.",
      {"Any"}};
