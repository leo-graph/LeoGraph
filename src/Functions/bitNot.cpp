...
struct BitNotImpl {
  ...
  static ResultType NO_SANITIZE_UNDEFINED apply(A a) { return static_cast<ResultType>(~static_cast<ResultType>(a)); }

#if USE_EMBEDDED_COMPILER
  static constexpr bool compilable = true;

  static llvm::Value *compile(llvm::IRBuilder<> &b, llvm::Value *arg, bool) {
    if (!arg->getType()->isIntegerTy()) throw Exception(ErrorCodes::LOGICAL_ERROR, "BitNotImpl expected an integral type");
...
REGISTER_FUNCTION(BitNot) {
  ...
  FunctionDocumentation::Arguments arguments = {{"a", "Value for which to apply bitwise NOT operation.", {"(U)Int*", "Float*", "String"}}};
