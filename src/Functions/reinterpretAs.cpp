...
class FunctionReinterpret : public IFunction {
  ...
  DataTypePtr getReturnTypeImpl(const ColumnsWithTypeAndName &arguments) const override {
    ...
    if (!type_col)
      throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT,
                      ...
                      getName(), arguments.back().type->getName());
    ...
    if (result_reinterpret_type.isFixedString()) {
      if (!from_type->isValueUnambiguouslyRepresentedInFixedSizeContiguousMemoryRegion())
        throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT,
                        "Cannot reinterpret {} as FixedString because it is not fixed size and contiguous in memory", from_type->getName());
    } else if (result_reinterpret_type.isString()) {
      if (!from_type->isValueUnambiguouslyRepresentedInContiguousMemoryRegion())
        throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT, "Cannot reinterpret {} as String because it is not contiguous in memory",
                        from_type->getName());
      ...
      if (!to_type->isValueUnambiguouslyRepresentedInContiguousMemoryRegion())
        throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT,
                        "Cannot reinterpret {} as {} because the array element type is not fixed length", from_type->getName(),
                        to_type->getName());
    } else {
      throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT,
    ...
    if (!callOnTwoTypeIndexes(from_type->getTypeId(), result_type->getTypeId(), [&](const auto &types) {
            ...
            if constexpr (std::is_same_v<FromType, DataTypeString>) {
              ...
              for (size_t i = 0; i < input_rows_count; ++i) {
                ...
                if constexpr (std::endian::native == std::endian::little)
                  ...
                  reverseMemcpy(reinterpret_cast<char *>(&vec_res[i]) + offset_to, &data_from[offset], copy_size);
                }
      ...
      if (WhichDataType(result_type).isArray()) {
        ...
        if (WhichDataType(from_type).isString())
          ...
          throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT, "Cannot reinterpret {} as {}, expected String or FixedString as source",
                          from_type->getName(), result_type->getName());
        ...
        throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT, "Cannot reinterpret {} as {}", from_type->getName(), result_type->getName());
      }
    }
 ...
 private:
  ...
  static constexpr auto CanBeReinterpretedAsNumeric =
      IsDataTypeDecimalOrNumber<T> || std::is_same_v<T, DataTypeDate> || std::is_same_v<T, DataTypeDateTime> ||
      std::is_same_v<T, DataTypeDateTime64> || std::is_same_v<T, DataTypeUUID>;

  static bool canBeReinterpretedAsNumeric(const WhichDataType &type) {
    return type.isUInt() || type.isInt() || type.isDate() || type.isDateTime() || type.isDateTime64() || type.isFloat() || type.isUUID() ||
           type.isDecimal();
  }
  ...
  static void NO_INLINE executeToString(const IColumn &src, ColumnString &dst, size_t input_rows_count) {
    ...
    for (size_t i = 0; i < input_rows_count; ++i) {
      ...
      while (!data.empty() && data.back() == 0) data.remove_suffix(1);
#else
      while (!data.empty() && data.front() == 0) data.remove_prefix(1);
#endif
  ...
  static ColumnsWithTypeAndName addTypeColumnToArguments(const ColumnsWithTypeAndName &arguments) {
    ...
    if constexpr (std::is_same_v<ToDataType, DataTypeFixedString>) {
      ...
      if (!type->isValueUnambiguouslyRepresentedInFixedSizeContiguousMemoryRegion())
        throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT,
                        "Cannot reinterpret {} as FixedString because it is not fixed size and contiguous in memory", type->getName());
    ...
    ColumnsWithTypeAndName arguments_with_type{argument, type_column};
...
struct NameReinterpretAsUInt128 {
  static constexpr auto name = "reinterpretAsUInt128";
};
struct NameReinterpretAsUInt256 {
  static constexpr auto name = "reinterpretAsUInt256";
};
...
struct NameReinterpretAsInt128 {
  static constexpr auto name = "reinterpretAsInt128";
};
struct NameReinterpretAsInt256 {
  static constexpr auto name = "reinterpretAsInt256";
};
...
struct NameReinterpretAsUUID {
  static constexpr auto name = "reinterpretAsUUID";
};
...
Unlike [`CAST`](#CAST), the function does not attempt to preserve the original value - if the target type is not able to represent the input type, the output is undefined.
  ...
  FunctionDocumentation::Examples examples_reinterpretAsUInt8 = {{"Usage example",
                                                                  R"(
SELECT
    toInt8(-1) AS val,
    toTypeName(val),
...
└─────┴─────────────────┴─────┴─────────────────┘
  ...
  FunctionDocumentation documentation_reinterpretAsUInt8 = {
      description_reinterpretAsUInt8,    syntax_reinterpretAsUInt8,   arguments_reinterpretAsUInt8,     {},
      returned_value_reinterpretAsUInt8, examples_reinterpretAsUInt8, introduced_in_reinterpretAsUInt8, category_reinterpretAsUInt8};
...
Unlike [`CAST`](#CAST), the function does not attempt to preserve the original value - if the target type is not able to represent the input type, the output is undefined.
  ...
  FunctionDocumentation::Examples examples_reinterpretAsUInt16 = {{"Usage example",
                                                                   R"(
SELECT
    toUInt8(257) AS x,
    toTypeName(x),
...
└───┴───────────────┴─────┴─────────────────┘
  ...
  FunctionDocumentation documentation_reinterpretAsUInt16 = {
      description_reinterpretAsUInt16,    syntax_reinterpretAsUInt16,   arguments_reinterpretAsUInt16,     {},
      returned_value_reinterpretAsUInt16, examples_reinterpretAsUInt16, introduced_in_reinterpretAsUInt16, category_reinterpretAsUInt16};
...
Unlike [`CAST`](#CAST), the function does not attempt to preserve the original value - if the target type is not able to represent the input type, the output is undefined.
  ...
  FunctionDocumentation::Examples examples_reinterpretAsUInt32 = {{"Usage example",
                                                                   R"(
SELECT
    toUInt16(257) AS x,
    toTypeName(x),
...
└─────┴───────────────┴─────┴─────────────────┘
  ...
  FunctionDocumentation documentation_reinterpretAsUInt32 = {
      description_reinterpretAsUInt32,    syntax_reinterpretAsUInt32,   arguments_reinterpretAsUInt32,     {},
      returned_value_reinterpretAsUInt32, examples_reinterpretAsUInt32, introduced_in_reinterpretAsUInt32, category_reinterpretAsUInt32};
...
Unlike [`CAST`](#CAST), the function does not attempt to preserve the original value - if the target type is not able to represent the input type, the output is undefined.
  ...
  FunctionDocumentation::Examples examples_reinterpretAsUInt64 = {{"Usage example",
                                                                   R"(
SELECT
    toUInt32(257) AS x,
    toTypeName(x),
...
└─────┴───────────────┴─────┴─────────────────┘
  ...
  FunctionDocumentation documentation_reinterpretAsUInt64 = {
      description_reinterpretAsUInt64,    syntax_reinterpretAsUInt64,   arguments_reinterpretAsUInt64,     {},
      returned_value_reinterpretAsUInt64, examples_reinterpretAsUInt64, introduced_in_reinterpretAsUInt64, category_reinterpretAsUInt64};
...
Unlike [`CAST`](#CAST), the function does not attempt to preserve the original value - if the target type is not able to represent the input type, the output is undefined.
  ...
  FunctionDocumentation::Examples examples_reinterpretAsUInt128 = {{"Usage example",
                                                                    R"(
SELECT
    toUInt64(257) AS x,
    toTypeName(x),
...
└─────┴───────────────┴─────┴─────────────────┘
  ...
  FunctionDocumentation documentation_reinterpretAsUInt128 = {description_reinterpretAsUInt128,    syntax_reinterpretAsUInt128,
                                                              arguments_reinterpretAsUInt128,      {},
                                                              returned_value_reinterpretAsUInt128, examples_reinterpretAsUInt128,
                                                              introduced_in_reinterpretAsUInt128,  category_reinterpretAsUInt128};
...
Unlike [`CAST`](#CAST), the function does not attempt to preserve the original value - if the target type is not able to represent the input type, the output is undefined.
  ...
  FunctionDocumentation::Examples examples_reinterpretAsUInt256 = {{"Usage example",
                                                                    R"(
SELECT
    toUInt128(257) AS x,
    toTypeName(x),
...
└─────┴───────────────┴─────┴─────────────────┘
  ...
  FunctionDocumentation documentation_reinterpretAsUInt256 = {description_reinterpretAsUInt256,    syntax_reinterpretAsUInt256,
                                                              arguments_reinterpretAsUInt256,      {},
                                                              returned_value_reinterpretAsUInt256, examples_reinterpretAsUInt256,
                                                              introduced_in_reinterpretAsUInt256,  category_reinterpretAsUInt256};
...
Unlike [`CAST`](#CAST), the function does not attempt to preserve the original value - if the target type is not able to represent the input type, the output is undefined.
  ...
  FunctionDocumentation::Examples examples_reinterpretAsInt8 = {{"Usage example",
                                                                 R"(
SELECT
    toUInt8(257) AS x,
    toTypeName(x),
...
└───┴───────────────┴─────┴─────────────────┘
  ...
  FunctionDocumentation documentation_reinterpretAsInt8 = {
      description_reinterpretAsInt8,    syntax_reinterpretAsInt8,   arguments_reinterpretAsInt8,     {},
      returned_value_reinterpretAsInt8, examples_reinterpretAsInt8, introduced_in_reinterpretAsInt8, category_reinterpretAsInt8};
...
Unlike [`CAST`](#CAST), the function does not attempt to preserve the original value - if the target type is not able to represent the input type, the output is undefined.
  ...
  FunctionDocumentation::Examples examples_reinterpretAsInt16 = {{"Usage example",
                                                                  R"(
SELECT
    toInt8(257) AS x,
    toTypeName(x),
...
└───┴───────────────┴─────┴─────────────────┘
  ...
  FunctionDocumentation documentation_reinterpretAsInt16 = {
      description_reinterpretAsInt16,    syntax_reinterpretAsInt16,   arguments_reinterpretAsInt16,     {},
      returned_value_reinterpretAsInt16, examples_reinterpretAsInt16, introduced_in_reinterpretAsInt16, category_reinterpretAsInt16};
...
Unlike [`CAST`](#CAST), the function does not attempt to preserve the original value - if the target type is not able to represent the input type, the output is undefined.
  ...
  FunctionDocumentation::Examples examples_reinterpretAsInt32 = {{"Usage example",
                                                                  R"(
SELECT
    toInt16(257) AS x,
    toTypeName(x),
...
└─────┴───────────────┴─────┴─────────────────┘
  ...
  FunctionDocumentation documentation_reinterpretAsInt32 = {
      description_reinterpretAsInt32,    syntax_reinterpretAsInt32,   arguments_reinterpretAsInt32,     {},
      returned_value_reinterpretAsInt32, examples_reinterpretAsInt32, introduced_in_reinterpretAsInt32, category_reinterpretAsInt32};
...
Unlike [`CAST`](#CAST), the function does not attempt to preserve the original value - if the target type is not able to represent the input type, the output is undefined.
  ...
  FunctionDocumentation::Examples examples_reinterpretAsInt64 = {{"Usage example",
                                                                  R"(
SELECT
    toInt32(257) AS x,
    toTypeName(x),
...
└─────┴───────────────┴─────┴─────────────────┘
  ...
  FunctionDocumentation documentation_reinterpretAsInt64 = {
      description_reinterpretAsInt64,    syntax_reinterpretAsInt64,   arguments_reinterpretAsInt64,     {},
      returned_value_reinterpretAsInt64, examples_reinterpretAsInt64, introduced_in_reinterpretAsInt64, category_reinterpretAsInt64};
...
Unlike [`CAST`](#CAST), the function does not attempt to preserve the original value - if the target type is not able to represent the input type, the output is undefined.
  ...
  FunctionDocumentation::Examples examples_reinterpretAsInt128 = {{"Usage example",
                                                                   R"(
SELECT
    toInt64(257) AS x,
    toTypeName(x),
...
└─────┴───────────────┴─────┴─────────────────┘
  ...
  FunctionDocumentation documentation_reinterpretAsInt128 = {
      description_reinterpretAsInt128,    syntax_reinterpretAsInt128,   arguments_reinterpretAsInt128,     {},
      returned_value_reinterpretAsInt128, examples_reinterpretAsInt128, introduced_in_reinterpretAsInt128, category_reinterpretAsInt128};
...
Unlike [`CAST`](#CAST), the function does not attempt to preserve the original value - if the target type is not able to represent the input type, the output is undefined.
  ...
  FunctionDocumentation::Examples examples_reinterpretAsInt256 = {{"Usage example",
                                                                   R"(
SELECT
    toInt128(257) AS x,
    toTypeName(x),
...
└─────┴───────────────┴─────┴─────────────────┘
  ...
  FunctionDocumentation documentation_reinterpretAsInt256 = {
      description_reinterpretAsInt256,    syntax_reinterpretAsInt256,   arguments_reinterpretAsInt256,     {},
      returned_value_reinterpretAsInt256, examples_reinterpretAsInt256, introduced_in_reinterpretAsInt256, category_reinterpretAsInt256};
...
Unlike [`CAST`](#CAST), the function does not attempt to preserve the original value - if the target type is not able to represent the input type, the output is undefined.
  ...
  FunctionDocumentation::Examples examples_reinterpretAsFloat32 = {{"Usage example",
                                                                    R"(
...
└────────────┴─────────────────────────┘
  ...
  FunctionDocumentation documentation_reinterpretAsFloat32 = {description_reinterpretAsFloat32,    syntax_reinterpretAsFloat32,
                                                              arguments_reinterpretAsFloat32,      {},
                                                              returned_value_reinterpretAsFloat32, examples_reinterpretAsFloat32,
                                                              introduced_in_reinterpretAsFloat32,  category_reinterpretAsFloat32};
...
Unlike [`CAST`](#CAST), the function does not attempt to preserve the original value - if the target type is not able to represent the input type, the output is undefined.
  ...
  FunctionDocumentation::Examples examples_reinterpretAsFloat64 = {{"Usage example",
                                                                    R"(
...
└─────────────────────┴─────────────────────────┘
  ...
  FunctionDocumentation documentation_reinterpretAsFloat64 = {description_reinterpretAsFloat64,    syntax_reinterpretAsFloat64,
                                                              arguments_reinterpretAsFloat64,      {},
                                                              returned_value_reinterpretAsFloat64, examples_reinterpretAsFloat64,
                                                              introduced_in_reinterpretAsFloat64,  category_reinterpretAsFloat64};
...
Reinterprets the input value as a Date value (assuming little endian order) which is the number of days since the beginning of the Unix epoch 1970-01-01
  ...
  FunctionDocumentation::Arguments arguments_reinterpretAsDate = {
      {"x",
       ...
       {"(U)Int*", "Float*", "Date", "DateTime", "UUID", "String", "FixedString"}}};
  FunctionDocumentation::ReturnedValue returned_value_reinterpretAsDate = {"Date.", {"Date"}};
  FunctionDocumentation::Examples examples_reinterpretAsDate = {{"Usage example",
                                                                 R"(
...
└───────────────────────┴────────────────────────┘
  ...
  FunctionDocumentation documentation_reinterpretAsDate = {
      description_reinterpretAsDate,    syntax_reinterpretAsDate,   arguments_reinterpretAsDate,     {},
      returned_value_reinterpretAsDate, examples_reinterpretAsDate, introduced_in_reinterpretAsDate, category_reinterpretAsDate};
...
Reinterprets the input value as a DateTime value (assuming little endian order) which is the number of days since the beginning of the Unix epoch 1970-01-01
  ...
  FunctionDocumentation::Examples examples_reinterpretAsDateTime = {{"Usage example",
                                                                     R"(
...
└───────────────────────────┴────────────────────────────┘
  ...
  FunctionDocumentation documentation_reinterpretAsDateTime = {description_reinterpretAsDateTime,    syntax_reinterpretAsDateTime,
                                                               arguments_reinterpretAsDateTime,      {},
                                                               returned_value_reinterpretAsDateTime, examples_reinterpretAsDateTime,
                                                               introduced_in_reinterpretAsDateTime,  category_reinterpretAsDateTime};
...
Accepts a 16 byte string and returns a UUID by interpreting each 8-byte half in little-endian byte order. If the string isn't long enough, the function works as if the string is padded with the necessary number of null bytes to the end. If the string is longer than 16 bytes, the extra bytes at the end are ignored.
  ...
  FunctionDocumentation::Arguments arguments_reinterpretAsUUID = {{"fixed_string", "Big-endian byte string.", {"FixedString"}}};
  FunctionDocumentation::ReturnedValue returned_value_reinterpretAsUUID = {"The UUID type value.", {"UUID"}};
  FunctionDocumentation::Examples examples_reinterpretAsUUID = {{"String to UUID",
                                                                 R"(
...
└───────────────────────────────────────────────────────────────────────┘
  ...
  FunctionDocumentation documentation_reinterpretAsUUID = {
      description_reinterpretAsUUID,    syntax_reinterpretAsUUID,   arguments_reinterpretAsUUID,     {},
      returned_value_reinterpretAsUUID, examples_reinterpretAsUUID, introduced_in_reinterpretAsUUID, category_reinterpretAsUUID};
...
Null bytes at the end are ignored, for example, the function returns for UInt32 value 255 a string with a single character.
  ...
  FunctionDocumentation::Examples examples_reinterpretAsString = {{"Usage example",
                                                                   R"(
SELECT
...
└────────────────────────────────────────────────────────┴───────────────────────────────────────────┘
  ...
  FunctionDocumentation documentation_reinterpretAsString = {
      description_reinterpretAsString,    syntax_reinterpretAsString,   arguments_reinterpretAsString,     {},
      returned_value_reinterpretAsString, examples_reinterpretAsString, introduced_in_reinterpretAsString, category_reinterpretAsString};
...
Reinterprets the input value as a fixed string (assuming little endian order).
  ...
  FunctionDocumentation::Arguments arguments_reinterpretAsFixedString = {
      {"x", "Value to reinterpret to string.", {"(U)Int*", "Float*", "Date", "DateTime"}}};
  FunctionDocumentation::ReturnedValue returned_value_reinterpretAsFixedString = {"Fixed string containing bytes representing `x`.",
                                                                                  {"FixedString"}};
  FunctionDocumentation::Examples examples_reinterpretAsFixedString = {{"Usage example",
                                                                        R"(
SELECT
...
└─────────────────────────────────────────────────────────────┴────────────────────────────────────────────────┘
  ...
  FunctionDocumentation documentation_reinterpretAsFixedString = {
      description_reinterpretAsFixedString,    syntax_reinterpretAsFixedString,
      arguments_reinterpretAsFixedString,      {},
      returned_value_reinterpretAsFixedString, examples_reinterpretAsFixedString,
      introduced_in_reinterpretAsFixedString,  category_reinterpretAsFixedString};
...
Uses the same source in-memory bytes sequence for the provided value `x` and reinterprets it to the destination type.
  ...
  FunctionDocumentation::Examples examples_reinterpret = {{"Usage example",
                                                           R"(
...
└────────────────────────────┘
  ...
  FunctionDocumentation documentation_reinterpret = {
      description_reinterpret,    syntax_reinterpret,   arguments_reinterpret,     {},
      returned_value_reinterpret, examples_reinterpret, introduced_in_reinterpret, category_reinterpret};
