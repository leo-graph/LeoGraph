...
struct RoundAgeImpl {
  ...
  static ResultType apply(A x) { return x < 1 ? 0 : (x < 18 ? 17 : (x < 25 ? 18 : (x < 35 ? 25 : (x < 45 ? 35 : (x < 55 ? 45 : 55))))); }
...
- Returns `55`, for `age ≥ 55`.
  ...
  FunctionDocumentation::Arguments arguments = {{"age", "A number representing an age in years.", {"(U)Int*", "Float*"}}};
  FunctionDocumentation::ReturnedValue returned_value = {"Returns either the highest or lowest age of the range `age` falls within.",
                                                         {"UInt8"}};
