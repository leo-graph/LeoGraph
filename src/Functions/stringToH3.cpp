  ...
  DataTypePtr getReturnTypeImpl(const DataTypes &arguments) const override {
    ...
    if (!WhichDataType(arg).isStringOrFixedString())
      throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT,
                      ...
                      "Must be String or FixedString",
                      arg->getName(), std::to_string(1), getName());
...
Converts the string representation of an H3 index to the `H3Index` ([UInt64](/sql-reference/data-types/int-uint)) representation.
  ...
  FunctionDocumentation::Arguments arguments = {{"index_str", "String representation of the H3 index.", {"String"}}};
  FunctionDocumentation::ReturnedValue returned_value = {"Returns the H3 index number, or `0` if the input is not a valid H3 index.",
                                                         {"UInt64"}};
  FunctionDocumentation::Examples examples = {{"Convert string to H3 index", "SELECT stringToH3('89184926cc3ffff') AS index",
                                               R"(
┌──────────────index─┐
│ 617420388351344639 │
└────────────────────┘
