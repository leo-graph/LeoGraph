...
class FunctionToDecimalString : public IFunction {
 ...
 private:
  ...
  void vectorConstant(const FromVectorType &vec_from, UInt8 precision, ColumnString::Chars &vec_to, ColumnString::Offsets &result_offsets,
  ...
  void vectorVector(const FirstArgVectorType &vec_from, const ColumnVector<UInt8>::Container &vec_precision, ColumnString::Chars &vec_to,
                    ColumnString::Offsets &result_offsets, size_t input_rows_count) const {
    result_offsets.resize(input_rows_count);
  ...
  template <typename FirstArgVectorType>
  void vectorConstant(const FirstArgVectorType &vec_from, UInt8 precision, ColumnString::Chars &vec_to,
                      ColumnString::Offsets &result_offsets, UInt8 from_scale, size_t input_rows_count) const {
  ...
  static void format(T value, DB::WriteBuffer &out, UInt8 precision) {
    ...
    if (!result) throw DB::Exception(DB::ErrorCodes::CANNOT_PRINT_FLOAT_OR_DOUBLE_NUMBER, "Error processing number: {}", value);
  ...
  static void format(T value, DB::WriteBuffer &out, UInt8 precision) {
    ...
    if (precision > 0) [[likely]] {
      writeChar('.', out);
      for (int i = 0; i < precision; ++i) writeChar('0', out);
    }
  }
    ...
    if constexpr (is_decimal<T>) {
      ...
      if (from_col) {
        ...
        if (precision_col_const)
          ...
          throw Exception(ErrorCodes::ILLEGAL_COLUMN, "Illegal column {} of second argument of function formatDecimal",
...
toDecimalString(number, scale)
  ...
  FunctionDocumentation::Arguments arguments = {
      {"number",
       "The numeric value to convert to a string. Can be any numeric type (Int, UInt, Float, Decimal).",
  ...
  FunctionDocumentation::Examples examples = {{"Round and format a number", R"(
