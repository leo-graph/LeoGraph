 ...
 private:
  ...
  DataTypePtr getReturnTypeImpl(const DataTypes &arguments) const override {
    ...
    if (!isNumber(argument))
      throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT, "Illegal type {} of argument of function {}", argument->getName(), getName());

    /// Integers are converted to Float64.
    if (Impl::always_returns_float64 || !isFloat(argument)) return std::make_shared<DataTypeFloat64>();
    return argument;
  }
  ...
  static void executeInIterations(const T *src_data, ReturnType *dst_data, size_t size) {
    if constexpr (Impl::rows_per_iteration == 0) {
      ...
      if constexpr (!is_floating_point<T>) {
        ...
        for (size_t i = 0; i < size; ++i) tmp_vec[i] = static_cast<Float64>(src_data[i]);
      ...
      for (size_t i = 0; i < rows_size; i += Impl::rows_per_iteration) Impl::execute(&src_data[i], &dst_data[i]);
      ...
      if (rows_remaining != 0) {
        ...
        if constexpr (is_big_int_v<T> || std::is_same_v<T, Decimal256>)
          for (size_t i = 0; i < rows_remaining; ++i) dst_data[rows_size + i] = dst_remaining[i];
        else
  ...
  static ColumnPtr execute(const ColumnDecimal<T> *col, size_t input_rows_count) {
    ...
    for (size_t i = 0; i < input_rows_count; ++i) dst_data[i] = DecimalUtils::convertTo<ReturnType>(src_data[i], scale);
  ...
  ColumnPtr executeImpl(const ColumnsWithTypeAndName &arguments, const DataTypePtr &, size_t input_rows_count) const override {
    ...
    auto call = [&](const auto &types) -> bool {
      ...
      if (col_vec == nullptr) return false;
    ...
    if (!callOnBasicType<void, true, true, true, false>(col.type->getTypeId(), call))
      throw Exception(ErrorCodes::ILLEGAL_COLUMN, "Illegal column {} of argument of function {}", col.column->getName(), getName());
