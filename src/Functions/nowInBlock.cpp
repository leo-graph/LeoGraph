...
class FunctionNowInBlock : public IFunction {
 public:
  ...
  static FunctionPtr create(ContextPtr context) { return std::make_shared<FunctionNowInBlock>(context); }
...
nowInBlock([timezone])
  ...
  FunctionDocumentation::Examples examples = {{"Difference with the now() function", R"(
SELECT
    now(),
    nowInBlock(),
    sleep(1)
