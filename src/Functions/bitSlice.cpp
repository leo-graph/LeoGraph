 ...
 public:
  ...
  ColumnPtr executeImpl(const ColumnsWithTypeAndName &arguments, const DataTypePtr &, size_t input_rows_count) const override {
    ...
    if (const ColumnFixedString *col_fixed = checkAndGetColumn<ColumnFixedString>(column_string.get()))
      return executeForSource(column_start, column_length, start_const, length_const, FixedStringSource(*col_fixed), input_rows_count);
    if (const ColumnConst *col_const = checkAndGetColumnConst<ColumnString>(column_string.get()))
      return executeForSource(column_start, column_length, start_const, length_const, ConstSource<StringSource>(*col_const),
                              input_rows_count);
    if (const ColumnConst *col_const_fixed = checkAndGetColumnConst<ColumnFixedString>(column_string.get()))
      return executeForSource(column_start, column_length, start_const, length_const, ConstSource<FixedStringSource>(*col_const_fixed),
                              input_rows_count);
  ...
  ColumnPtr executeForSource(const ColumnPtr &column_start, const ColumnPtr &column_length, std::optional<Int64> start_const,
                             std::optional<Int64> length_const, Source &&source, size_t input_rows_count) const {
    ...
    if (!column_length) {
      if (start_const) {
        ...
        if (start_value > 0)
          bitSliceFromLeftConstantOffsetUnbounded(source, StringSink(*col_res, input_rows_count), static_cast<size_t>(start_value - 1));
        else if (start_value < 0)
          bitSliceFromRightConstantOffsetUnbounded(source, StringSink(*col_res, input_rows_count), -static_cast<size_t>(start_value));
        else
      ...
      if (start_const && length_const) {
        ...
        if (start_value > 0)
          bitSliceFromLeftConstantOffsetBounded(source, StringSink(*col_res, input_rows_count), static_cast<size_t>(start_value - 1),
                                                length_value);
        else if (start_value < 0)
          bitSliceFromRightConstantOffsetBounded(source, StringSink(*col_res, input_rows_count), -static_cast<size_t>(start_value),
                                                 length_value);
        else
          throw Exception(ErrorCodes::ZERO_ARRAY_OR_TUPLE_INDEX, "Indices in strings are 1-based");
      } else
  ...
  void bitSliceFromLeftConstantOffsetUnbounded(Source &&src, StringSink &&sink, size_t offset) const {
    ...
    while (!src.isEnd()) {
      ...
      if (sl.size) writeSliceWithLeftShift(sl, sink, offset_bit);
  ...
  void bitSliceFromRightConstantOffsetUnbounded(Source &&src, StringSink &&sink, size_t offset) const {
    ...
    while (!src.isEnd()) {
      ...
      if (slice.size) writeSliceWithLeftShift(slice, sink, shift_bit);
  ...
  void bitSliceFromLeftConstantOffsetBounded(Source &&src, StringSink &&sink, size_t offset, ssize_t length) const {
    ...
    while (!src.isEnd()) {
      ...
      if (slice.size) writeSliceWithLeftShift(slice, sink, shift_bit, abandon_last_bit);
...
REGISTER_FUNCTION(BitSlice) {
  ...
  FunctionDocumentation::Arguments arguments = {{"s", "The String or Fixed String to slice.", {"String", "FixedString"}},
                                                {"offset",
                                                 R"(
...
- If length is not a multiple of 8, the result is padded with zeros on the right.
  ...
  FunctionDocumentation::ReturnedValue returned_value = {
      "Returns a string containing the extracted bits, represented as a binary sequence. The result is always padded to byte boundaries "
      "(multiples of 8 bits)",
      {"String"}};
