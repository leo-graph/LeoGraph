...
class FunctionIcebergHash : public IFunction {
 public:
  ...
  static FunctionPtr create(ContextPtr context_) { return std::make_shared<FunctionIcebergHash>(context_); }
...
REGISTER_FUNCTION(IcebergHash) {
  FunctionDocumentation::Description description =
      R"(Implements the logic of the iceberg [hashing transform](https://iceberg.apache.org/spec/#appendix-b-32-bit-hash-requirements))";
  FunctionDocumentation::Syntax syntax = "icebergHash(value)";
  FunctionDocumentation::Arguments arguments = {
      {"value",
       "Source value to take the hash of",
       {"Integer", "Bool", "Decimal", "Float*", "String", "FixedString", "UUID", "Date", "Time", "DateTime"}}};
...
class FunctionIcebergBucket : public IFunction {
 public:
  ...
  static FunctionPtr create(ContextPtr context_) { return std::make_shared<FunctionIcebergBucket>(context_); }
  ...
  DataTypePtr getReturnTypeImpl(const DataTypes &) const override { return std::make_shared<DataTypeUInt32>(); }
  ...
  ColumnPtr executeImpl(const ColumnsWithTypeAndName &arguments, const DataTypePtr & /* result_type */,
    ...
    if (value <= 0 || value > std::numeric_limits<Int32>::max())
      throw Exception(ErrorCodes::BAD_ARGUMENTS, "Function IcebergBucket accepts only positive bucket size which is suitable to Int32");
    ...
    auto max_int_with_type =
        ColumnWithTypeAndName(std::make_shared<DataTypeInt32>()->createColumnConst(input_rows_count, std::numeric_limits<Int32>::max()),
                              std::make_shared<DataTypeInt32>(), "");
    auto bitand_result_type = std::make_shared<DataTypeInt32>();
    auto bitand_result = FunctionFactory::instance()
                             .get("bitAnd", context)
    ...
    auto modulo_column =
        ColumnWithTypeAndName(std::make_shared<DataTypeUInt32>()->createColumnConst(input_rows_count, static_cast<UInt32>(value)),
...
REGISTER_FUNCTION(IcebergBucket) {
  ...
  FunctionDocumentation::Arguments arguments = {
      ...
      {"value",
       "The source value to transform.",
       {"(U)Int*", "Bool", "Decimal", "Float*", "String", "FixedString", "UUID", "Date", "Time", "DateTime"}}};
