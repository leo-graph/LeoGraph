...
constexpr Float64 gamma_fallback =
    1e-6;  /// substituted when the caller passes gamma = 0 so that 1 / gamma in gamma transfer function stays finite
...
/// linear-RGB -> LMS
constexpr Mat3x3 linear_to_lms_base = {0.4122214708, 0.5363325363, 0.0514459929, 0.2119034982, 0.6806995451,
                                       0.1073969566, 0.0883024619, 0.2817188376, 0.6299787005};
/// LMS -> OKLab
constexpr Mat3x3 lms_to_oklab_base = {0.2104542553,  0.7936177850, -0.0040720468, 1.9779984951, -2.4285922050,
                                      0.45059370996, 0.0259040371, 0.7827717662,  -0.8086757660};
/// OKLab -> LMS
constexpr Mat3x3 oklab_to_lms_base = {1, 0.3963377774, 0.2158037573, 1, -0.1055613458, -0.0638541728, 1, -0.0894841775, -1.2914855480};
/// LMS -> linear-RGB
constexpr Mat3x3 lms_to_linear_base = {4.0767416621,  -3.3077115913, 0.2309699292,  -1.2684380046, 2.6097574011,
...
inline Color srgbToOklab(const Color &rgb, Float64 gamma) {
  ...
  Color rgb_lin;
  for (size_t i = 0; i < channels; ++i) rgb_lin[i] = std::pow(rgb[i] / 255.0, gamma);
...
inline Color oklabToSrgb(const Color &oklab, Float64 gamma) {
  ...
  Color lms{};
  for (size_t i = 0; i < channels; ++i) {
    for (size_t channel = 0; channel < channels; ++channel) lms[i] = std::fma(oklab[channel], oklab_to_lms_base[(3 * i) + channel], lms[i]);
  ...
  Color rgb{};
  for (size_t i = 0; i < channels; ++i) {
    for (size_t channel = 0; channel < channels; ++channel) rgb[i] = std::fma(lms[channel], lms_to_linear_base[(3 * i) + channel], rgb[i]);
  }

  /// Step 3: Linear sRGB to sRGB (gamma correction)
  if (gamma == 0) gamma = gamma_fallback;
 ...
 public:
  ...
  DataTypePtr getReturnTypeImpl(const DataTypes &arguments) const override {
    if (arguments.empty() || arguments.size() > 2)
      throw Exception(ErrorCodes::NUMBER_OF_ARGUMENTS_DOESNT_MATCH, "Function {} requires 1 or 2 arguments, {} provided", getName(),
                      arguments.size());
    ...
    if (tuple_inner_types.size() != ColorConversion::channels)
      throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT,
                      "First argument of function {} must be a tuple of size {}, a tuple of size {} was provided", getName(),
                      ColorConversion::channels, tuple_inner_types.size());

    for (const auto &tuple_inner_type : tuple_inner_types) {
      if (!isNumber(tuple_inner_type))
        throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT, "Tuple elements of first argument of function {} must be numbers", getName());
    }
  ...
  ColumnPtr executeImpl(const ColumnsWithTypeAndName &arguments, const DataTypePtr &, size_t input_rows_count) const override {
    ...
    ColumnPtr gamma;
    if (arguments.size() == 2) gamma = castColumn(arguments[1], float64_type)->convertToFullColumnIfConst();
