...
class FunctionSerial : public IFunction {
 ...
 public:
  ...
  static FunctionPtr create(ContextPtr context) { return std::make_shared<FunctionSerial>(std::move(context)); }
  ...
  [[nodiscard]] UInt64 update(String series_name, UInt64 start_value, size_t &current_series, UInt64 increment,
                              zkutil::ZooKeeperPtr &keeper) const {
    ...
    if (current_series < max_series) {
      ...
      if (!keeper->exists(serial_path))
        throw Exception(
            ...
            "Too many series created by {} function, maximum: {}. This is controlled by the `max_autoincrement_series` setting.", name,
            max_series);
    }
  ...
  ColumnPtr executeImpl(const ColumnsWithTypeAndName &arguments, const DataTypePtr &, size_t input_rows_count) const override {
    ...
    if (has_start_value) start_value = arguments[1].column->getUInt(0);
    ...
    if (const auto *column_const = typeid_cast<const ColumnConst *>(arguments[0].column.get())) {
      ...
      for (size_t i = 0; i < input_rows_count; ++i) ++series[arguments[0].column->getDataAt(i)].num_rows;
      ...
      /// Populate the result:
      for (size_t i = 0; i < input_rows_count; ++i) vec_to[i] = ++series[arguments[0].column->getDataAt(i)].old_value;
    }
...
The series are stored in Keeper nodes under the path, which can be configured in [`series_keeper_path`](/operations/server-configuration-parameters/settings#series_keeper_path) in the server configuration.
  ...
  FunctionDocumentation::Arguments arguments = {
      ...
      {"start_value",
       "Optional. Starting value for the counter. Defaults to 0. Note: this value is only used when creating a new series and is ignored "
       "if the series already exists",
       {"UInt*"}},
  };
  FunctionDocumentation::ReturnedValue returned_value = {"Returns sequential numbers starting from the previous counter value.",
                                                         {"UInt64"}};
  FunctionDocumentation::Examples examples = {{"first call",
                                               R"(
