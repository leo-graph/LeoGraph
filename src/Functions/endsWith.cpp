...
Checks whether a string ends with the provided suffix.
  ...
  FunctionDocumentation::Arguments arguments = {{"s", "String to check.", {"String"}}, {"suffix", "Suffix to check for.", {"String"}}};
  FunctionDocumentation::ReturnedValue returned_value = {"Returns `1` if `s` ends with `suffix`, otherwise `0`.", {"UInt8"}};
  FunctionDocumentation::Examples examples = {{"Usage example", "SELECT endsWith('ClickHouse', 'House');",
                                               R"(
┌─endsWith('Cl⋯', 'House')─┐
│                        1 │
└──────────────────────────┘
...
Checks whether a string ends with the provided case-insensitive suffix.
  ...
  FunctionDocumentation::ReturnedValue returned_value = {"Returns `1` if `s` ends with case-insensitive `suffix`, otherwise `0`.",
                                                         {"UInt8"}};
  FunctionDocumentation::Examples examples = {{"Usage example", "SELECT endsWithCaseInsensitive('ClickHouse', 'HOUSE');",
                                               R"(
