...
struct TranslateImpl {
  ...
  static void fillMapWithValues(Map &map, const std::string &map_from, const std::string &map_to) {
    ...
    for (size_t i = min_size; i < map_from.size(); ++i) {
      if (!isASCII(map_from[i])) throw Exception(ErrorCodes::BAD_ARGUMENTS, "Second argument must be ASCII strings");
      map[static_cast<unsigned char>(map_from[i])] = ascii_upper_bound + 1;
    }
  }

  static void vector(const ColumnString::Chars &data, const ColumnString::Offsets &offsets, const std::string &map_from,
                     const std::string &map_to, ColumnString::Chars &res_data, ColumnString::Offsets &res_offsets,
  ...
  static void vectorFixed(const ColumnString::Chars &data, size_t /*n*/, const std::string &map_from, const std::string &map_to,
                          ColumnString::Chars &res_data) {
    if (map_from.size() != map_to.size()) throw Exception(ErrorCodes::BAD_ARGUMENTS, "Second and third arguments must be the same length");
...
struct TranslateUTF8Impl {
  ...
  static void fillMapWithValues(MapASCII &map_ascii, MapUTF8 &map, const std::string &map_from, const std::string &map_to) {
    ...
    while (map_from_ptr < map_from_end) {
      ...
      if (!res_from) throw Exception(ErrorCodes::BAD_ARGUMENTS, "Second argument must be a valid UTF-8 string");

      if (map_to_ptr < map_to_end) {
        size_t len_to = UTF8::seqLength(*map_to_ptr);

        if (map_to_ptr + len_to <= map_to_end) res_to = UTF8::convertUTF8ToCodePoint(reinterpret_cast<const char *>(map_to_ptr), len_to);

        if (!res_to) throw Exception(ErrorCodes::BAD_ARGUMENTS, "Third argument must be a valid UTF-8 string");
      ...
      while (src < src_end) {
        ...
        if (src + src_len <= src_end) {
          ...
          if (src_code_point) {
            ...
            if (it != map.end()) {
              src += src_len;
              if (it->getMapped() == max_uint32) continue;
  ...
  [[noreturn]] static void vectorFixed(const ColumnString::Chars & /*data*/, size_t /*n*/, const std::string & /*map_from*/,
                                       const std::string & /*map_to*/, ColumnString::Chars & /*res_data*/) {
 ...
 public:
  ...
  DataTypePtr getReturnTypeImpl(const DataTypes &arguments) const override {
    if (!isStringOrFixedString(arguments[0]))
      throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT, "Illegal type {} of first argument of function {}", arguments[0]->getName(),
                      getName());

    if (!isStringOrFixedString(arguments[1]))
      throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT, "Illegal type {} of second argument of function {}", arguments[1]->getName(),
                      getName());

    if (!isStringOrFixedString(arguments[2]))
      throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT, "Illegal type {} of third argument of function {}", arguments[2]->getName(),
                      getName());
  ...
  ColumnPtr executeImpl(const ColumnsWithTypeAndName &arguments, const DataTypePtr &, size_t input_rows_count) const override {
    ...
    if (map_from_size < map_to_size)
      throw Exception(ErrorCodes::BAD_ARGUMENTS,
                      "Second argument of function {} must not be shorter than the third argument. Size of the second argument: {}, size "
                      "of the third argument: {}",
                      getName(), map_from.size(), map_to.size());
...
Non-ASCII characters in `s` are not modified by the function.
  ...
  FunctionDocumentation::Examples translate_examples = {
      {"Character mapping", "SELECT translate('Hello, World!', 'delor', 'DELOR') AS res",
       R"(
┌─res───────────┐
│ HELLO, WORLD! │
└───────────────┘
        )"},
      {"Different lengths", "SELECT translate('clickhouse', 'clickhouse', 'CLICK') AS res",
       R"(
┌─res───┐
│ CLICK │
└───────┘
  ...
  FunctionDocumentation translate_documentation = {
      translate_description,    translate_syntax,   translate_arguments,     {},
      translate_returned_value, translate_examples, translate_introduced_in, translate_category};
...
Like [`translate`](#translate) but assumes `s`, `from` and `to` are UTF-8 encoded strings.
  ...
  FunctionDocumentation::Examples utf8_examples = {{"UTF-8 character translation",
...
└──────────────────┘
  ...
  FunctionDocumentation utf8_documentation = {utf8_description,    utf8_syntax,   utf8_arguments,          {},
                                              utf8_returned_value, utf8_examples, translate_introduced_in, translate_category};
