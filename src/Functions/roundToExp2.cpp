...
struct RoundToExp2Impl {
  ...
  static T apply(T x) { return roundDownToPowerOfTwo<T>(x); }
...
If the number is less than one, it returns `0`.
  ...
  FunctionDocumentation::Arguments arguments = {{"num", "A number to round.", {"(U)Int*", "Float*"}}};
  FunctionDocumentation::ReturnedValue returned_value = {
      "Returns `num` rounded down to the nearest (whole non-negative) power of two, otherwise `0` for `num < 1`.", {"(U)Int*", "Float*"}};
