...
class FunctionBaseToTimeZone : public IFunctionBase {
 public:
  FunctionBaseToTimeZone(bool is_constant_timezone_, DataTypes argument_types_, DataTypePtr return_type_)
      : is_constant_timezone(is_constant_timezone_), argument_types(std::move(argument_types_)), return_type(std::move(return_type_)) {}
...
class ToTimeZoneOverloadResolver : public IFunctionOverloadResolver {
 public:
  ...
  DataTypePtr getReturnTypeImpl(const ColumnsWithTypeAndName &arguments) const override {
    ...
    if (which_type.isDateTime()) return std::make_shared<DataTypeDateTime>(time_zone_name);
  ...
  FunctionBasePtr buildImpl(const ColumnsWithTypeAndName &arguments, const DataTypePtr &result_type) const override {
    ...
    if (arguments[1].column) is_constant_timezone = isColumnConst(*arguments[1].column);
...
Only the value's time zone attribute and the value's string representation changes.
  ...
  FunctionDocumentation::Arguments arguments = {{"date", "The value to convert.", {"DateTime", "DateTime64"}},
                                                {"timezone", "The target time zone name.", {"String"}}};
  FunctionDocumentation::ReturnedValue returned_value = {"Returns the same timestamp as the input, but with the specified time zone",
                                                         {"DateTime", "DateTime64"}};
