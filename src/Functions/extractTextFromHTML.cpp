...
bool processComment(const char *__restrict &src, const char *end) {
  if (!checkAndSkip(src, end, "<!--")) return false;
...
bool processCDATA(const char *__restrict &src, const char *end, char *__restrict &dst) {
  if (!checkAndSkip(src, end, "<![CDATA[")) return false;
...
bool processElementAndSkipContent(const char *__restrict &src, const char *end, const std::string_view tag_name) {
  ...
  while (true) {
    ...
    if (*src == '!') {
      --src;
      char *dst = nullptr;
      if (processCDATA(src, end, dst)) continue;
      ++src;
    }
    ...
    if (checkAndSkip(src, end, tag_name)) {
      while (src < end && isWhitespaceASCII(*src)) ++src;
...
void copyText(const char *__restrict &src, const char *end, char *__restrict &dst, bool needs_whitespace) {
  ...
  while (true) {
    ...
    src = ws;
    while (src < lt && isWhitespaceASCII(*src)) ++src;
...
size_t extract(const char *__restrict src, size_t size, char *__restrict dst) {
  ...
  while (src < end) {
    ...
    processComment(src, end) || processCDATA(src, end, dst) || processElementAndSkipContent(src, end, "script") ||
        processElementAndSkipContent(src, end, "style") || skipTag(src, end);
  }
...
class FunctionExtractTextFromHTML : public IFunction {
 public:
  ...
  ColumnPtr executeImpl(const ColumnsWithTypeAndName &arguments, const DataTypePtr &, size_t rows) const override {
    ...
    if (!src) throw Exception(ErrorCodes::ILLEGAL_COLUMN, "First argument for function {} must be string.", getName());
    ...
    for (size_t i = 0; i < rows; ++i) {
      ...
      res_offset += extract(reinterpret_cast<const char *>(&src_chars[src_offset]), next_src_offset - src_offset,
...
Note: HTML entities are not decoded and should be processed with a separate function if needed.
  ...
  FunctionDocumentation::Arguments arguments = {{"html", "String containing HTML content to extract text from.", {"String"}}};
