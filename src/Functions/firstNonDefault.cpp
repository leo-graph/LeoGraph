...
class FunctionFirstNonDefault : public IFunction {
 public:
  ...
  static FunctionPtr create(ContextPtr) { return std::make_shared<FunctionFirstNonDefault>(); }
  ...
  DataTypePtr getReturnTypeImpl(const DataTypes& arguments) const override {
    ...
    if (arguments.size() > max_args)
      throw Exception(ErrorCodes::NUMBER_OF_ARGUMENTS_DOESNT_MATCH, "Function {} requires at most {} arguments, got {}", getName(),
                      max_args, arguments.size());
  ...
  ColumnPtr executeImpl(const ColumnsWithTypeAndName& arguments, const DataTypePtr& result_type, size_t input_rows_count) const override {
    ...
    for (const auto& arg : arguments) {
      ...
      if (casted_column->getDataType() != result_col->getDataType()) {
        throw Exception(ErrorCodes::LOGICAL_ERROR, "All arguments must cast to the same type, got {} and {} for result type {}",
    ...
    for (size_t row = 0; row < input_rows_count; ++row) {
      ...
      if (!found) result_col->insertDefault();
    }
    return result_col;
  }
};
