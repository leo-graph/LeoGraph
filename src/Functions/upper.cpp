...
Converts the ASCII Latin symbols in a string to uppercase.
  ...
  FunctionDocumentation::Arguments arguments = {{"s", "The string to convert to uppercase.", {"String"}}};
  FunctionDocumentation::ReturnedValue returned_value = {"Returns an uppercase string from `s`.", {"String"}};
  FunctionDocumentation::Examples examples = {{"Usage example", "SELECT upper('clickhouse')",
                                               R"(
┌─upper('clickhouse')─┐
│ CLICKHOUSE          │
└─────────────────────┘
