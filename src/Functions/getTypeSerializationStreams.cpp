...
class FunctionGetTypeSerializationStreams : public IFunction {
 public:
  ...
  static FunctionPtr create(ContextPtr) { return std::make_shared<FunctionGetTypeSerializationStreams>(); }
  ...
  ColumnPtr executeImpl(const ColumnsWithTypeAndName &arguments, const DataTypePtr &, size_t input_rows_count) const override {
    ...
    serialization->enumerateStreams(
        settings, [&](const ISerialization::SubstreamPath &substream_path) { col_res_strings.insert(substream_path.toString()); },
 ...
 private:
  static DataTypePtr getType(const ColumnWithTypeAndName &argument) {
    ...
    if (!arg_string) return argument.type;
...
This function is intended for developmental use.
  ...
  FunctionDocumentation::ReturnedValue returned_value = {"Returns an array with all the serialization sub-stream paths.",
                                                         {"Array(String)"}};
  FunctionDocumentation::Examples examples = {
      {"tuple", "SELECT getTypeSerializationStreams(tuple('a', 1, 'b', 2))",
       "['{TupleElement(1), Regular}','{TupleElement(2), Regular}','{TupleElement(3), Regular}','{TupleElement(4), Regular}']"},
      {"map", "SELECT getTypeSerializationStreams('Map(String, Int64)')",
