...
class FunctionJSONMergePatch : public IFunction {
 public:
  ...
  DataTypePtr getReturnTypeImpl(const ColumnsWithTypeAndName &arguments) const override {
    FunctionArgumentDescriptor variadic_args{"json1", isString, nullptr, "String"};
  ...
  ColumnPtr executeImpl(const ColumnsWithTypeAndName &arguments, const DataTypePtr &, size_t input_rows_count) const override {
    ...
    auto parse_json_document = [](const ColumnString &column, rapidjson::Document &document, size_t i) {
      ...
      if (!document.IsObject()) throw Exception(ErrorCodes::BAD_ARGUMENTS, "Wrong JSON string to merge. Expected JSON object");
    };

    const bool is_first_const = isColumnConst(*arguments[0].column);
    const auto *first_column_arg_string = is_first_const ? checkAndGetColumnConstData<ColumnString>(arguments[0].column.get())
                                                         : checkAndGetColumn<ColumnString>(arguments[0].column.get());

    if (!first_column_arg_string) throw Exception(ErrorCodes::ILLEGAL_COLUMN, "Arguments of function {} must be strings", getName());
    ...
    for (size_t col_idx = 1; col_idx < arguments.size(); ++col_idx) {
      ...
      const auto *column_arg_string = is_const ? checkAndGetColumnConstData<ColumnString>(arguments[col_idx].column.get())
                                               : checkAndGetColumn<ColumnString>(arguments[col_idx].column.get());

      if (!column_arg_string) throw Exception(ErrorCodes::ILLEGAL_COLUMN, "Arguments of function {} must be strings", getName());
...
Returns the merged JSON object string which is formed by merging multiple JSON objects.
  ...
  FunctionDocumentation::Arguments arguments = {{"json1[, json2, ...]", "One or more strings with valid JSON.", {"String"}}};
  FunctionDocumentation::ReturnedValue returned_value = {"Returns the merged JSON object string, if the JSON object strings are valid.",
                                                         {"String"}};
