...
struct CRC32CHasher {
  size_t operator()(const char* data, size_t length) const { return updateWeakHash32(reinterpret_cast<const UInt8*>(data), length, 0); }
};
...
class SparseGramsImpl {
 private:
  ...
  class NGramSymbolIterator {
   public:
    ...
    NGramSymbolIterator(Pos data_, Pos end_, size_t n_) : data(data_), end(end_), n(n_) {}
    ...
    bool increment() {
      ...
      if (++num_increments >= n) left_iterator = getNextPosition(left_iterator);

      return true;
    }

    bool isEnd() const { return data + right_iterator >= end; }

    std::pair<size_t, size_t> getNGramPositions() const { return {left_iterator, right_iterator}; }

    size_t getRightSymbol() const { return num_increments; }
  ...
  bool consume() {
    if (symbol_iterator.isEnd()) return false;
    ...
    while (!convex_hull.empty() && convex_hull.back().hash < right_border_ngram_hash) {
      ...
      if (length > max_ngram_length) {
        /// If the current length is greater than the current right position, it will be greater at future right positions, so we can just
        /// delete them all.
        convex_hull.clear();
        break;
      }
      result.push_back({.left_index = possible_left_position, .right_index = next_right_position, .symbols_between = length});
    ...
    while (!convex_hull.empty() && convex_hull.back().hash == right_border_ngram_hash) convex_hull.pop_back();

    convex_hull.push_back(PositionAndHash{.position = right_position,
 ...
 public:
  ...
  explicit SparseGramsImpl(UInt64 min_ngram_length_, UInt64 max_ngram_length_, std::optional<UInt64> min_cutoff_length_)
      : min_ngram_length(min_ngram_length_), max_ngram_length(max_ngram_length_), min_cutoff_length(std::move(min_cutoff_length_)) {}
  ...
  void init(const ColumnsWithTypeAndName& arguments, bool /*max_substrings_includes_remaining_string*/) {
    if (arguments.size() > 4)
      throw Exception(ErrorCodes::NUMBER_OF_ARGUMENTS_DOESNT_MATCH,
                      "Number of arguments for function {} doesn't match: passed {}, must be from 1 to 4", name, arguments.size());

    if (arguments.size() >= 2) min_ngram_length = arguments[1].column->getUInt(0);

    if (min_ngram_length < 3) throw Exception(ErrorCodes::BAD_ARGUMENTS, "Argument 'min_ngram_length' must be greater or equal to 3");

    if (arguments.size() == 3) max_ngram_length = arguments[2].column->getUInt(0);
    ...
    if (arguments.size() == 4) min_cutoff_length = arguments[3].column->getUInt(0);
  ...
  void set(Pos pos_, Pos end_) {
    ...
    for (size_t i = 0; i < min_ngram_length - 2; ++i)
      if (!symbol_iterator.increment()) return;
  }
  ...
  bool get(Pos& token_begin, Pos& token_end) {
    while (true) {
      ...
      if (min_cutoff_length && *min_cutoff_length > length) continue;
