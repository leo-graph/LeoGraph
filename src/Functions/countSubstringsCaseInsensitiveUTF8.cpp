...
using FunctionCountSubstringsCaseInsensitiveUTF8 =
    FunctionsStringSearch<CountSubstringsImpl<NameCountSubstringsCaseInsensitiveUTF8, PositionCaseInsensitiveUTF8>>;
...
Like [`countSubstrings`](#countSubstrings) but counts case-insensitively and assumes that haystack is a UTF-8 string.
  ...
  FunctionDocumentation::Examples examples = {
      {"Usage example", "SELECT countSubstringsCaseInsensitiveUTF8('ложка, кошка, картошка', 'КА');",
       R"(
┌─countSubstri⋯шка', 'КА')─┐
│                        4 │
└──────────────────────────┘
        )"},
      {"With start_pos argument", "SELECT countSubstringsCaseInsensitiveUTF8('ложка, кошка, картошка', 'КА', 13);",
       R"(
┌─countSubstri⋯, 'КА', 13)─┐
│                        2 │
└──────────────────────────┘
