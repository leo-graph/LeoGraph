...
DataTypePtr FunctionValidateNestedArraySizes::getReturnTypeImpl(const DataTypes &arguments) const {
  ...
  if (num_args < 3)
    throw Exception(ErrorCodes::NUMBER_OF_ARGUMENTS_DOESNT_MATCH, "Function {} needs more than two arguments; passed {}.", getName(),
                    arguments.size());
...
ColumnPtr FunctionValidateNestedArraySizes::executeImpl(const ColumnsWithTypeAndName &arguments, const DataTypePtr & /*result_type*/,
  ...
  for (size_t i = 0; i < input_rows_count; ++i) {
    if (is_condition_const && !condition) break;

    if (!is_condition_const && !condition_column->getData()[i]) continue;
