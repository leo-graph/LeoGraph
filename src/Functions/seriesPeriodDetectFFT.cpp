...
class FunctionSeriesPeriodDetectFFT : public IFunction {
 public:
  ...
  bool executeNumbers(const IColumn &src_data, Float64 &period, ColumnArray::Offset &start, ColumnArray::Offset &end) const {
    ...
    for (size_t i = 0; i < shape.size(); ++i) axes.push_back(i);
...
└────────────────────────┘
      ...
      {"Period detection with complex pattern", "SELECT seriesPeriodDetectFFT(arrayMap(x -> abs((x % 6) - 3), range(1000))) AS print_0",
       R"(
┌─print_0─┐
│       6 │
└─────────┘
