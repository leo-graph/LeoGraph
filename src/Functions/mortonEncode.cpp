...
#define EXTRACT_VECTOR(INDEX) const ColumnPtr &col##INDEX = non_const_arguments[(INDEX) + vectorStartIndex].column;
...
#define EXPAND(IDX, ...) (mask) ? expand(mask->getColumn(IDX).getUInt(0), __VA_ARGS__) : __VA_ARGS__

#define MASK(ND, IDX, ...) (EXPAND(IDX, __VA_ARGS__) & MortonND_##ND##D_Enc.InputMask())
...
#define EXECUTE()                                                                                                                   \
  ...
  if (mask) {                                                                                                                       \
    ...
    for (size_t i = 0; i < nd; i++) {                                                                                               \
      ...
      if (ratio > 8 || ratio < 1)                                                                                                   \
        throw Exception(ErrorCodes::ARGUMENT_OUT_OF_BOUND, "Illegal argument {} of function {}, should be a number in range 1-8",   \
                        arguments[0].column->getName(), getName());                                                                 \
    }                                                                                                                               \
  }                                                                                                                                 \
                                                                                                                                    \
  auto non_const_arguments = arguments;                                                                                             \
  for (auto &argument : non_const_arguments) argument.column = argument.column->convertToFullColumnIfConst();                       \
                                                                                                                                    \
  ...
  if (nd == 1) {                                                                                                                    \
    ...
    return col_res;                                                                                                                 \
                                                                                                                                    ...
                                                                                                                                    \
  EXTRACT_VECTOR(1)                                                                                                                 \
  ENCODE(2, MASK(2, 0, col0->getUInt(i)), MASK(2, 1, col1->getUInt(i)))                                                             \
  EXTRACT_VECTOR(2)                                                                                                                 \
  ENCODE(3, MASK(3, 0, col0->getUInt(i)), MASK(3, 1, col1->getUInt(i)), MASK(3, 2, col2->getUInt(i)))                               \
  EXTRACT_VECTOR(3)                                                                                                                 \
  ENCODE(4, MASK(4, 0, col0->getUInt(i)), MASK(4, 1, col1->getUInt(i)), MASK(4, 2, col2->getUInt(i)), MASK(4, 3, col3->getUInt(i))) \
  EXTRACT_VECTOR(4)                                                                                                                 \
  ENCODE(5, MASK(5, 0, col0->getUInt(i)), MASK(5, 1, col1->getUInt(i)), MASK(5, 2, col2->getUInt(i)), MASK(5, 3, col3->getUInt(i)), \
         MASK(5, 4, col4->getUInt(i)))                                                                                              \
  EXTRACT_VECTOR(5)                                                                                                                 \
  ENCODE(6, MASK(6, 0, col0->getUInt(i)), MASK(6, 1, col1->getUInt(i)), MASK(6, 2, col2->getUInt(i)), MASK(6, 3, col3->getUInt(i)), \
         MASK(6, 4, col4->getUInt(i)), MASK(6, 5, col5->getUInt(i)))                                                                \
  EXTRACT_VECTOR(6)                                                                                                                 \
  ENCODE(7, MASK(7, 0, col0->getUInt(i)), MASK(7, 1, col1->getUInt(i)), MASK(7, 2, col2->getUInt(i)), MASK(7, 3, col3->getUInt(i)), \
         MASK(7, 4, col4->getUInt(i)), MASK(7, 5, col5->getUInt(i)), MASK(7, 6, col6->getUInt(i)))                                  \
  EXTRACT_VECTOR(7)                                                                                                                 \
  ENCODE(8, MASK(8, 0, col0->getUInt(i)), MASK(8, 1, col1->getUInt(i)), MASK(8, 2, col2->getUInt(i)), MASK(8, 3, col3->getUInt(i)), \
         MASK(8, 4, col4->getUInt(i)), MASK(8, 5, col5->getUInt(i)), MASK(8, 6, col6->getUInt(i)), MASK(8, 7, col7->getUInt(i)))    \
                                                                                                                                    \
  throw Exception(ErrorCodes::TOO_MANY_ARGUMENTS_FOR_FUNCTION, "Illegal number of UInt arguments of function {}, max: 8", getName());

DECLARE_DEFAULT_CODE(constexpr auto MortonND_2D_Enc = mortonnd::MortonNDLutEncoder<2, 32, 8>();
                     ...
                     class FunctionMortonEncode : public FunctionSpaceFillingCurveEncode {
                      public:
                       ...
                       static FunctionPtr create(ContextPtr) { return std::make_shared<FunctionMortonEncode>(); }
                       ...
                       ColumnPtr executeImpl(const ColumnsWithTypeAndName &arguments, const DataTypePtr &, size_t input_rows_count)
                           const override {
                         EXECUTE()
                       }
...
#  undef MASK
#  define MASK(ND, IDX, ...) (EXPAND(IDX, __VA_ARGS__))
...
ColumnPtr executeImpl(const ColumnsWithTypeAndName &arguments, const DataTypePtr &, size_t input_rows_count) const override { EXECUTE() }
}
;
) // DECLARE_AVX2_SPECIFIC_CODE
...
class FunctionMortonEncode: public TargetSpecific::Default::FunctionMortonEncode
{
 public:
  explicit FunctionMortonEncode(ContextPtr context) : selector(context) {
    selector.registerImplementation<TargetArch::Default, TargetSpecific::Default::FunctionMortonEncode>();

#if USE_MULTITARGET_CODE && defined(MORTON_ND_BMI2_ENABLED)
    selector.registerImplementation<TargetArch::x86_64_v3, TargetSpecific::x86_64_v3::FunctionMortonEncode>();
#endif
  }

  ColumnPtr executeImpl(const ColumnsWithTypeAndName &arguments, const DataTypePtr &result_type, size_t input_rows_count) const override {
    if (input_rows_count == 0) return ColumnUInt64::create();

    return selector.selectAndExecute(arguments, result_type, input_rows_count);
  }

  static FunctionPtr create(ContextPtr context) { return std::make_shared<FunctionMortonEncode>(context); }
...
mortonEncode(range_mask, args)
  ...
  FunctionDocumentation::Arguments arguments = {
      ...
      {"range_mask",
       "For the expanded mode, the mask for each argument. The mask is a tuple of unsigned integers from `1` - `8`. Each number in the "
       "mask configures the amount of range shrink.",
       {"Tuple(UInt8/16/32/64)"}},
  };
  FunctionDocumentation::ReturnedValue returned_value = {"Returns a `UInt64` code.", {"UInt64"}};
  FunctionDocumentation::Examples examples = {{"Simple mode", "SELECT mortonEncode(1, 2, 3)", "53"},
                                              {"Expanded mode",
                                               R"(
...
SELECT mortonEncode(1)
                                               ...
                                               "1"},
                                              {"Expanded single argument", "SELECT mortonEncode(tuple(2), 128)", "32768"},
                                              {"Column usage",
                                               R"(
