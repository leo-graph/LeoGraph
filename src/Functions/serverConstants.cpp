...
class FunctionBuildId : public FunctionServerConstantBase<FunctionBuildId, String, DataTypeString> {
 public:
  ...
  explicit FunctionBuildId(ContextPtr context)
      : FunctionServerConstantBase(SymbolIndex::instance().getBuildIDHex(), context->isDistributed()) {}
};
#endif
...
class FunctionHostName : public FunctionServerConstantBase<FunctionHostName, String, DataTypeString> {
 public:
  ...
  explicit FunctionHostName(ContextPtr context)
      : FunctionServerConstantBase(DNSResolver::instance().getHostName(), context->isDistributed()) {}
};
...
class FunctionServerTimezone : public FunctionServerConstantBase<FunctionServerTimezone, String, DataTypeString> {
 public:
  ...
  explicit FunctionServerTimezone(ContextPtr context)
      : FunctionServerConstantBase(DateLUT::serverTimezoneInstance().getTimeZone(), context->isDistributed()) {}
};
...
class FunctionRevision : public FunctionServerConstantBase<FunctionRevision, UInt32, DataTypeUInt32> {
 public:
  ...
  explicit FunctionRevision(ContextPtr context)
      : FunctionServerConstantBase(ClickHouseRevision::getVersionRevision(), context->isDistributed()) {}
};
...
class FunctionGetOSKernelVersion : public FunctionServerConstantBase<FunctionGetOSKernelVersion, String, DataTypeString> {
 public:
  ...
  explicit FunctionGetOSKernelVersion(ContextPtr context)
      : FunctionServerConstantBase(Poco::Environment::osName() + " " + Poco::Environment::osVersion(), context->isDistributed()) {}
...
class FunctionDisplayName : public FunctionServerConstantBase<FunctionDisplayName, String, DataTypeString> {
 public:
  ...
  explicit FunctionDisplayName(ContextPtr context)
      : FunctionServerConstantBase(context->getConfigRef().getString("display_name", getFQDNOrHostName()), context->isDistributed()) {}
...
The UUID is persisted, i.e. the second, third, etc. server start return the same UUID.
  ...
  FunctionDocumentation::Examples examples_serverUUID = {{"Usage example",
                                                          R"(
SELECT serverUUID();
        )",
                                                          R"(
...
└──────────────────────────────────────────┘
  ...
  FunctionDocumentation documentation_serverUUID = {
      description_serverUUID,    syntax_serverUUID,   arguments_serverUUID,     {},
      returned_value_serverUUID, examples_serverUUID, introduced_in_serverUUID, category_serverUUID};
...
Returns the uptime of the current ZooKeeper session in seconds.
  ...
  FunctionDocumentation::ReturnedValue returned_value_zookeeperSessionUptime = {
      "Returns the uptime of the current ZooKeeper session in seconds.", {"UInt32"}};
  FunctionDocumentation::Examples examples_zookeeperSessionUptime = {{"Usage example",
                                                                      R"(
...
└──────────────────────────┘
  ...
  FunctionDocumentation documentation_zookeeperSessionUptime = {description_zookeeperSessionUptime,    syntax_zookeeperSessionUptime,
                                                                arguments_zookeeperSessionUptime,      {},
                                                                returned_value_zookeeperSessionUptime, examples_zookeeperSessionUptime,
                                                                introduced_in_zookeeperSessionUptime,  category_zookeeperSessionUptime};
...
Returns a string with the OS kernel version.
  ...
  FunctionDocumentation::Examples examples_getOSKernelVersion = {{"Usage example",
                                                                  R"(
...
└─────────────────────────┘
  ...
  FunctionDocumentation documentation_getOSKernelVersion = {
      description_getOSKernelVersion,    syntax_getOSKernelVersion,   arguments_getOSKernelVersion,     {},
      returned_value_getOSKernelVersion, examples_getOSKernelVersion, introduced_in_getOSKernelVersion, category_getOSKernelVersion};
...
Returns the value of `display_name` from [config](/operations/configuration-files) or the server's Fully Qualified Domain Name (FQDN) if not set.
  ...
  FunctionDocumentation::Arguments arguments = {};
  FunctionDocumentation::ReturnedValue returned_value = {"Returns the value of `display_name` from config or server FQDN if not set.",
                                                         {"String"}};
  FunctionDocumentation::Examples examples = {{"Usage example",
                                               R"(
SELECT displayName();
