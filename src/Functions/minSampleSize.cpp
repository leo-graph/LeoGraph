...
static bool isBetweenZeroAndOne(Float64 v) { return v >= 0.0 && v <= 1.0 && fabs(v - 0.0) >= DBL_EPSILON && fabs(v - 1.0) >= DBL_EPSILON; }
...
struct ContinuousImpl {
  ...
  static void validateArguments(const ColumnsWithTypeAndName &arguments) {
    FunctionArgumentDescriptors mandatory_args{{"baseline", isNativeNumber, nullptr, "native numeric"},
...
struct ConversionImpl {
  ...
  static void validateArguments(const ColumnsWithTypeAndName &arguments) {
    FunctionArgumentDescriptors mandatory_args{{"p1", isFloat, nullptr, "Float"},
  ...
  static ColumnPtr process(const ColumnsWithTypeAndName &arguments, const Float64 *col_p1, const size_t input_rows_count) {
    ...
    for (size_t row_num = 0; row_num < input_rows_count; ++row_num) {
      ...
      Float64 min_sample_size =
          std::pow(quantile(nd, 1.0 - alpha / 2.0) * std::sqrt(2.0 * p_bar * q_bar) + quantile(nd, power) * std::sqrt(p1 * q1 + p2 * q2),
                   2) /
          std::pow(mde, 2);
...
Also assumes equal variance of the test metric in treatment and control groups.
  ...
  FunctionDocumentation::Arguments arguments_continuous = {
      ...
      {"mde",
       "Minimum detectable effect (MDE) as percentage of the baseline value (e.g. for a baseline value 112.25 the MDE 0.03 means an "
       "expected change to 112.25 ± 112.25*0.03).",
       {"(U)Int*", "Float*"}},
  ...
  FunctionDocumentation::ReturnedValue returned_value_continuous = {
      "Returns a named Tuple with 3 elements: `minimum_sample_size`, `detect_range_lower` and  `detect_range_upper`. These are "
      "respectively: the required sample size, the lower bound of the range of values not detectable with the returned required sample "
      "size, calculated as `baseline * (1 - mde)`, and the upper bound of the range of values not detectable with the returned required "
      "sample size, calculated as `baseline * (1 + mde)` (Float64).",
      {"Tuple(Float64, Float64, Float64)"}};
  FunctionDocumentation::Examples examples_continuous = {{"minSampleSizeContinuous",
                                                          R"(
...
(616.2931945826209,108.8825,115.6175)
  ...
  FunctionDocumentation documentation_continuous = {
      description_continuous,    syntax_continuous,   arguments_continuous, {},
      returned_value_continuous, examples_continuous, introduced_in,        category_continuous};
...
Uses the formula described in [this article](https://towardsdatascience.com/required-sample-size-for-a-b-testing-6f6608dd330a). Assumes equal sizes of treatment and control groups. Returns the sample size required for one group (i.e. the sample size required for the whole experiment is twice the returned value).
  ...
  FunctionDocumentation::Arguments arguments_conversion = {
      ...
      {"mde",
       "Minimum detectable effect (MDE) as percentage points (e.g. for a baseline conversion 0.25 the MDE 0.03 means an expected change to "
       "0.25 ± 0.03).",
       {"Float*"}},
  ...
  FunctionDocumentation::ReturnedValue returned_value_conversion = {
      "Returns a named Tuple with 3 elements: `minimum_sample_size`, `detect_range_lower`, `detect_range_upper`. These are, respectively: "
      "the required sample size, the lower bound of the range of values not detectable with the returned required sample size, calculated "
      "as `baseline - mde`, the upper bound of the range of values not detectable with the returned required sample size, calculated as "
      "`baseline + mde`.",
      {"Tuple(Float64, Float64, Float64)"}};
  FunctionDocumentation::Examples examples_conversion = {{"minSampleSizeConversion",
  ...
  FunctionDocumentation documentation_conversion = {
      description_conversion,    syntax_conversion,   arguments_conversion,     {},
      returned_value_conversion, examples_conversion, introduced_in_conversion, category_conversion};
