...
class FunctionToFunctionBaseAdaptor final : public IFunctionBase {
 public:
  ...
  ColumnPtr getConstantResultForNonConstArguments(const ColumnsWithTypeAndName& arguments_,
                                                  const DataTypePtr& result_type_) const override {
  ...
  bool isShortCircuit(ShortCircuitSettings& settings, size_t number_of_arguments) const override {
    return function->isShortCircuit(settings, number_of_arguments);
  }

  bool isSuitableForShortCircuitArgumentsExecution(const DataTypesWithConstInfo& args) const override {
    return function->isSuitableForShortCircuitArgumentsExecution(args);
  }
  ...
  FieldIntervalPtr getPreimage(const IDataType& type, const Field& point) const override { return function->getPreimage(type, point); }
...
class FunctionToOverloadResolverAdaptor : public IFunctionOverloadResolver {
 public:
  ...
  bool isServerConstant() const override { return function->isServerConstant(); }
  bool isShortCircuit(IFunctionBase::ShortCircuitSettings& settings, size_t number_of_arguments) const override {
    return function->isShortCircuit(settings, number_of_arguments);
  }
  ...
  DataTypePtr getReturnTypeForDefaultImplementationForDynamic() const override {
    return function->getReturnTypeForDefaultImplementationForDynamic();
  }
