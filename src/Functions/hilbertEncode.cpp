...
class FunctionHilbertEncode : public FunctionSpaceFillingCurveEncode {
 public:
  ...
  static FunctionPtr create(ContextPtr) { return std::make_shared<FunctionHilbertEncode>(); }
  ...
  ColumnPtr executeImpl(const ColumnsWithTypeAndName &arguments, const DataTypePtr &, size_t input_rows_count) const override {
    ...
    if (mask) {
      ...
      for (size_t i = 0; i < num_dimensions; i++) {
        ...
        if (ratio > 32)
          throw Exception(ErrorCodes::ARGUMENT_OUT_OF_BOUND, "Illegal argument {} of function {}, should be a number in range 0-32",
    ...
    const auto expand = [mask](const UInt64 value, const UInt8 column_num) {
      if (mask) return value << mask->getColumn(column_num).getUInt(0);
      return value;
    };
    ...
    if (num_dimensions == 2) {
      for (size_t i = 0; i < input_rows_count; ++i) {
        vec_res[i] = FunctionHilbertEncode2DWIthLookupTableImpl<3>::encode(expand(col0->getUInt(i), 0), expand(col1->getUInt(i), 1));
      }
      return col_res;
    }

    throw Exception(ErrorCodes::TOO_MANY_ARGUMENTS_FOR_FUNCTION,
                    "Illegal number of UInt arguments of function {}: should be not more than 2 dimensions", getName());
  }
};
...
hilbertEncode(range_mask, args)
  ...
  FunctionDocumentation::Examples examples = {{"Simple mode", "SELECT hilbertEncode(3, 4)", "31"},
                                              {"Expanded mode",
                                               R"(
