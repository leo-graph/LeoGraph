...
class FunctionRegexpQuoteMeta : public IFunction {
 public:
  ...
  static FunctionPtr create(ContextPtr) { return std::make_shared<FunctionRegexpQuoteMeta>(); }
  ...
  ColumnPtr executeImpl(const ColumnsWithTypeAndName &arguments, const DataTypePtr &, size_t input_rows_count) const override {
    ...
    for (size_t row_idx = 0; row_idx < input_rows_count; ++row_idx) {
      ...
      while (true) {
        const char *next_src_pos =
            find_first_symbols<'\0', '\\', '|', '(', ')', '^', '$', '.', '[', ']', '?', '*', '+', '{', ':', '-'>(src_pos, src_end);
...
It escapes zero byte as `\0` instead of `\x00` and it escapes only required characters.
  ...
  FunctionDocumentation::Arguments arguments = {{"s", "The input string containing characters to be escaped for regex.", {"String"}}};
  FunctionDocumentation::ReturnedValue returned_value = {"Returns a string with regex special characters escaped.", {"String"}};
  FunctionDocumentation::Examples examples = {{"Escape regex special characters", "SELECT regexpQuoteMeta('Hello. [World]? (Yes)*') AS res",
                                               R"(
┌─res───────────────────────────┐
│ Hello\. \[World\]\? \(Yes\)\* │
└───────────────────────────────┘
