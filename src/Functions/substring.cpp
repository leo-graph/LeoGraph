...
class FunctionSubstring : public IFunction {
 public:
  ...
  DataTypePtr getReturnTypeImpl(const DataTypes &arguments) const override {
    ...
    if (number_of_arguments < 2 || number_of_arguments > 3)
      throw Exception(ErrorCodes::NUMBER_OF_ARGUMENTS_DOESNT_MATCH,
                      ...
                      "passed {}, should be 2 or 3",
                      getName(), number_of_arguments);

    if (is_utf8) {
      /// UTF8 variant is not available for FixedString and Enum arguments.
      if (!isString(arguments[0]))
        throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT, "Illegal type {} of first argument of function {}, expected String",
                        arguments[0]->getName(), getName());
    } else {
      if (!isStringOrFixedString(arguments[0]) && !isEnum(arguments[0]))
        throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT,
                        "Illegal type {} of first argument of function {}, expected String, FixedString or Enum", arguments[0]->getName(),
                        getName());
    }

    if (!isNativeNumber(arguments[1]))
      throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT, "Illegal type {} of second argument of function {}, expected (U)Int*",
  ...
  ColumnPtr executeForSource(const ColumnPtr &column_offset, const ColumnPtr &column_length, bool column_offset_const,
                             bool column_length_const, Int64 offset, Int64 length, Source &&source, size_t input_rows_count) const {
  ...
  ColumnPtr executeImpl(const ColumnsWithTypeAndName &arguments, const DataTypePtr &, size_t input_rows_count) const override {
    ...
    if (number_of_arguments == 3) column_length = arguments[2].column;
    ...
    if (number_of_arguments == 3) column_length_const = checkAndGetColumn<ColumnConst>(column_length.get());

    Int64 offset = 0;
    Int64 length = 0;

    if (column_offset_const) offset = column_offset_const->getInt(0);
    if (column_length_const) length = column_length_const->getInt(0);

    if (is_utf8) {
      if (const ColumnString *col = checkAndGetColumn<ColumnString>(column_string.get())) {
        bool all_ascii = isAllASCII(col->getChars().data(), col->getChars().size());
        if (all_ascii)
          return executeForSource(column_offset, column_length, column_offset_const, column_length_const, offset, length,
                                  StringSource(*col), input_rows_count);
        return executeForSource(column_offset, column_length, column_offset_const, column_length_const, offset, length,
                                UTF8StringSource(*col), input_rows_count);
      }
      ...
      if (const ColumnConst *col_const = checkAndGetColumnConst<ColumnString>(column_string.get())) {
        ...
        if (all_ascii)
          return executeForSource(column_offset, column_length, column_offset_const, column_length_const, offset, length,
                                  ConstSource<StringSource>(*col_const), input_rows_count);
        return executeForSource(column_offset, column_length, column_offset_const, column_length_const, offset, length,
                                ConstSource<UTF8StringSource>(*col_const), input_rows_count);
      }
      throw Exception(ErrorCodes::ILLEGAL_COLUMN, "Illegal column {} of first argument of function {}", arguments[0].column->getName(),
                      getName());
    } else {
      if (const ColumnString *col = checkAndGetColumn<ColumnString>(column_string.get()))
        return executeForSource(column_offset, column_length, column_offset_const, column_length_const, offset, length, StringSource(*col),
                                input_rows_count);
      if (const ColumnFixedString *col_fixed = checkAndGetColumn<ColumnFixedString>(column_string.get()))
        return executeForSource(column_offset, column_length, column_offset_const, column_length_const, offset, length,
                                FixedStringSource(*col_fixed), input_rows_count);
      if (const ColumnConst *col_const = checkAndGetColumnConst<ColumnString>(column_string.get()))
        return executeForSource(column_offset, column_length, column_offset_const, column_length_const, offset, length,
                                ConstSource<StringSource>(*col_const), input_rows_count);
      if (const ColumnConst *col_const_fixed = checkAndGetColumnConst<ColumnFixedString>(column_string.get()))
        return executeForSource(column_offset, column_length, column_offset_const, column_length_const, offset, length,
                                ConstSource<FixedStringSource>(*col_const_fixed), input_rows_count);
      if (isEnum(arguments[0].type)) {
        if (const typename DataTypeEnum8::ColumnType *col_enum8 =
                checkAndGetColumn<typename DataTypeEnum8::ColumnType>(column_string.get())) {
          const auto *type_enum8 = assert_cast<const DataTypeEnum8 *>(arguments[0].type.get());
          return executeForSource(column_offset, column_length, column_offset_const, column_length_const, offset, length,
                                  EnumSource<DataTypeEnum8>(*col_enum8, *type_enum8), input_rows_count);
        }
        if (const typename DataTypeEnum16::ColumnType *col_enum16 =
                checkAndGetColumn<typename DataTypeEnum16::ColumnType>(column_string.get())) {
          const auto *type_enum16 = assert_cast<const DataTypeEnum16 *>(arguments[0].type.get());
          return executeForSource(column_offset, column_length, column_offset_const, column_length_const, offset, length,
                                  EnumSource<DataTypeEnum16>(*col_enum16, *type_enum16), input_rows_count);
        }
      }
...
An optional argument `length` specifies the maximum number of bytes the returned substring may have.
  ...
  FunctionDocumentation::Arguments arguments = {{"s", "The string to calculate a substring from.", {"String", "FixedString", "Enum"}},
                                                {"offset", "The starting position of the substring in `s`.", {"(U)Int*"}},
                                                {"length", "Optional. The maximum length of the substring.", {"(U)Int*"}}};
  FunctionDocumentation::ReturnedValue returned_value = {"Returns a substring of `s` with `length` many bytes, starting at index `offset`.",
                                                         {"String"}};
  FunctionDocumentation::Examples examples = {{"Basic usage", "SELECT 'database' AS db, substr(db, 5), substr(db, 5, 1)",
                                               R"(
...
:::
  ...
  FunctionDocumentation::Arguments arguments_utf8 = {{"s", "The string to calculate a substring from.", {"String", "FixedString", "Enum"}},
                                                     {"offset", "The starting position of the substring in `s`.", {"Int", "UInt"}},
                                                     {"length", "The maximum length of the substring. Optional.", {"Int", "UInt"}}};
  FunctionDocumentation::ReturnedValue returned_value_utf8 = {
  ...
  FunctionDocumentation::Examples examples_utf8 = {
      {"Usage example", "SELECT 'Täglich grüßt das Murmeltier.' AS str, substringUTF8(str, 9), substringUTF8(str, 9, 5)",
       "Täglich grüßt das Murmeltier.    grüßt das Murmeltier.    grüßt"}};
  FunctionDocumentation documentation_utf8 = {description_utf8,    syntax_utf8,   arguments_utf8, {},
                                              returned_value_utf8, examples_utf8, introduced_in,  category};

  factory.registerFunction(
      "substring", [](ContextPtr) { return FunctionSubstring::create("substring", false); }, documentation,
  ...
  factory.registerAlias("byteSlice", "substring",
                        FunctionFactory::Case::Insensitive);  /// resembles PostgreSQL's get_byte function, similar to ClickHouse's bitSlice

  factory.registerFunction(
      "substringUTF8", [](ContextPtr) { return FunctionSubstring::create("substringUTF8", true); }, documentation_utf8);
}
