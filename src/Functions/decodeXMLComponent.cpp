 ...
 private:
  ...
  static size_t execute(const char *src, size_t src_size, char *dst) {
    ...
    while (true) {
      ...
      if (entity_pos + strlen("lt;") >= src_end) break;
  ...
  static bool isValidNumericEntity(const char *src, const char *end, uint32_t &code_point) {
    if (src + strlen("#") >= end) return false;

    if (src[0] != '#' || (end - src > 1 + max_decimal_length_of_unicode_point)) return false;

    if (src + 2 < end && (src[1] == 'x' || src[1] == 'X')) {
      src += 2;
      for (; src < end; ++src) {
        if (!isHexDigit(*src)) return false;
        code_point *= 16;
      ...
      src += 1;
      for (; src < end; ++src) {
        if (!isNumericASCII(*src)) return false;
        code_point *= 10;
        code_point += *src - '0';
      }
    }
...
Decodes XML entities in a string to their corresponding characters.
  ...
  FunctionDocumentation::Arguments arguments = {{"s", "String containing XML entities to decode.", {"String"}}};
  FunctionDocumentation::ReturnedValue returned_value = {"Returns the provided string with XML entities decoded.", {"String"}};
  FunctionDocumentation::Examples examples = {{"Usage example", "SELECT decodeXMLComponent('&lt;tag&gt;Hello &amp; World&lt;/tag&gt;')",
                                               R"(
┌─decodeXMLCom⋯;/tag&gt;')─┐
│ <tag>Hello & World</tag> │
└──────────────────────────┘
