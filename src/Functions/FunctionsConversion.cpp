...
UInt32 extractToDecimalScale(const ColumnWithTypeAndName &named_column) {
  ...
  if (!ok) throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT, "Illegal type of toDecimal() scale {}", named_column.type->getName());
...
ColumnPtr ConvertImplFromDynamicToColumn::execute(
  ...
  for (size_t i = 0; i != variant_types.size(); ++i) {
    ...
    if (i == column_dynamic.getSharedVariantDiscriminator()) continue;
...
ExecutableFunctionPtr FunctionCast::prepare(const ColumnsWithTypeAndName & /*sample_columns*/) const {
  try {
    return std::make_unique<ExecutableFunctionCast>(prepareUnpackDictionaries(getArgumentTypes()[0], getResultType()), cast_name,
                                                    diagnostic);
  } catch (Exception &e) {
    if (diagnostic)
...
FunctionCast::WrapperType FunctionCast::createWrapper(const DataTypePtr &from_type, const ToDataType *const to_type,
  ...
  bool can_apply_accurate_cast =
      (cast_type == CastType::accurate || cast_type == CastType::accurateOrNull) && (which.isInt() || which.isUInt() || which.isFloat());
  ...
  if (requested_result_is_nullable && checkAndGetDataType<DataTypeString>(from_type.get())) {
    ...
    FunctionPtr function =
        FunctionConvertFromString<ToDataType, FunctionCastName, ConvertFromStringExceptionMode::Null>::createFromSettings(settings);
  ...
  return [this, from_type_index, to_type](ColumnsWithTypeAndName &arguments, const DataTypePtr &result_type,
                                          const ColumnNullable *column_nullable, size_t input_rows_count) {
...
#define GENERATE_OVERFLOW_MODE_CASE(OVERFLOW_MODE, ADDITIONS)                                                                         \
  case FormatSettings::DateTimeOverflowBehavior::OVERFLOW_MODE:                                                                       \
    result_column =                                                                                                                   \
        ConvertImpl<LeftDataType, RightDataType, FunctionCastName, FormatSettings::DateTimeOverflowBehavior::OVERFLOW_MODE>::execute( \
...
#undef GENERATE_OVERFLOW_MODE_CASE
          ...
          if (cast_type == CastType::accurate) {
            result_column = ConvertImpl<LeftDataType, RightDataType, FunctionCastName>::execute(
                arguments, result_type, input_rows_count, BehaviourOnErrorFromString::ConvertDefaultBehaviorTag, settings,
    ...
    if (!res) {
      if (cast_type == CastType::accurateOrNull) {
        ...
        throw Exception(ErrorCodes::CANNOT_CONVERT_TYPE, "Conversion from {} to {} is not supported", from_type_index, to_type->getName());
      }
    }
...
template <typename ToDataType>
FunctionCast::WrapperType FunctionCast::createBoolWrapper(const DataTypePtr &from_type, const ToDataType *const to_type,
                                                          bool requested_result_is_nullable) const {
  if (checkAndGetDataType<DataTypeString>(from_type.get())) {
    if (cast_type == CastType::accurateOrNull) {
      return [this](ColumnsWithTypeAndName &arguments, const DataTypePtr &result_type, const ColumnNullable *column_nullable,
...
FunctionCast::WrapperType FunctionCast::createUInt8ToBoolWrapper(const DataTypePtr from_type, const DataTypePtr to_type) const {
  return [from_type, to_type](ColumnsWithTypeAndName &arguments, const DataTypePtr &, const ColumnNullable *,
                              size_t /*input_rows_count*/) -> ColumnPtr {
    ...
    for (size_t i = 0; i != data_from.size(); ++i) data_to[i] = static_cast<bool>(data_from[i]);
    return res_column;
  };
}
...
FunctionCast::WrapperType FunctionCast::createFixedStringWrapper(const DataTypePtr &from_type, const size_t N) const {
  ...
  return [exception_mode_null, N](ColumnsWithTypeAndName &arguments, const DataTypePtr &, const ColumnNullable *,
...
#define GENERATE_INTERVAL_CASE(INTERVAL_KIND) \
  case IntervalKind::Kind::INTERVAL_KIND:     \
    return createFunctionAdaptor(             \
        FunctionConvert<DataTypeInterval, NameToInterval##INTERVAL_KIND, PositiveMonotonicity>::createFromSettings(settings), from_type);
...
FunctionCast::WrapperType FunctionCast::createDecimalWrapper(const DataTypePtr &from_type, const ToDataType *to_type,
                                                             bool requested_result_is_nullable) const {
  TypeIndex type_index = from_type->getTypeId();
  UInt32 scale = to_type->getScale();

  WhichDataType which(type_index);
  bool ok = which.isNativeInt() || which.isNativeUInt() || which.isDecimal() || which.isFloat() || which.isDateOrDate32() ||
            which.isDateTime() || which.isDateTime64() || which.isTime() || which.isTime64() || which.isStringOrFixedString();
  if (!ok) {
    if (cast_type == CastType::accurateOrNull)
      return createToNullableColumnWrapper();
    else
      throw Exception(ErrorCodes::CANNOT_CONVERT_TYPE, "Conversion from {} to {} is not supported", from_type->getName(),
                      to_type->getName());
  }

  return [this, type_index, scale, to_type, requested_result_is_nullable](ColumnsWithTypeAndName &arguments, const DataTypePtr &result_type,
                                                                          const ColumnNullable *column_nullable, size_t input_rows_count) {
    ColumnPtr result_column;
    auto res = callOnIndexAndDataType<ToDataType>(type_index, [&](const auto &types) -> bool {
      ...
      if constexpr (IsDataTypeDecimalOrNumber<LeftDataType> && IsDataTypeDecimalOrNumber<RightDataType> &&
                    !(std::is_same_v<DataTypeDateTime64, RightDataType> || std::is_same_v<DataTypeTime64, RightDataType>)) {
        if (cast_type == CastType::accurate) {
    ...
    if (!res) {
      if (cast_type == CastType::accurateOrNull) {
        ...
        throw Exception(ErrorCodes::CANNOT_CONVERT_TYPE, "Conversion from {} to {} is not supported", type_index, to_type->getName());
    }
...
FunctionCast::WrapperType FunctionCast::createAggregateFunctionWrapper(const DataTypePtr &from_type_untyped,
                                                                       const DataTypeAggregateFunction *to_type) const {
  ...
  if (checkAndGetDataType<DataTypeString>(from_type_untyped.get())) {
    ...
    if (agg_type->getFunction()->haveSameStateRepresentation(*to_type->getFunction())) {
      return [function = to_type->getFunction()](ColumnsWithTypeAndName &arguments, const DataTypePtr & /* result_type */,
                                                 const ColumnNullable * /* nullable_source */, size_t /*input_rows_count*/) -> ColumnPtr {
        ...
        if (col_agg) {
          ...
          throw Exception(ErrorCodes::LOGICAL_ERROR, "Illegal column {} for function CAST AS AggregateFunction",
  ...
  if (cast_type == CastType::accurateOrNull)
    ...
    throw Exception(ErrorCodes::CANNOT_CONVERT_TYPE, "Conversion from {} to {} is not supported", from_type_untyped->getName(),
                    to_type->getName());
}
...
FunctionCast::WrapperType FunctionCast::createArrayWrapper(const DataTypePtr &from_type_untyped, const DataTypeArray &to_type) const {
  ...
  if (!from_type) {
    throw Exception(ErrorCodes::TYPE_MISMATCH, "CAST AS Array can only be performed between same-dimensional Array, Map or String types");
  }
  ...
  if (from_type->getNumberOfDimensions() != to_type.getNumberOfDimensions() && !from_empty_array)
    throw Exception(ErrorCodes::TYPE_MISMATCH, "CAST AS Array can only be performed between same-dimensional array types");
  ...
  return [nested_function, from_nested_type, to_nested_type](ColumnsWithTypeAndName &arguments, const DataTypePtr &,
...
FunctionCast::ElementWrappers FunctionCast::getElementWrappers(const DataTypes &from_element_types,
...
FunctionCast::WrapperType FunctionCast::createTupleWrapper(const DataTypePtr &from_type_untyped, const DataTypeTuple *to_type) const {
  ...
  if (from_type->hasExplicitNames() && to_type->hasExplicitNames()) {
    ...
    for (size_t i = 0; i < from_names.size(); ++i) from_positions[from_names[i]] = i;
    ...
    for (size_t i = 0; i < to_element_types.size(); ++i) to_reverse_index.emplace_back(i);
  }

  return [element_wrappers, from_element_types, to_element_types, to_reverse_index](ColumnsWithTypeAndName &arguments, const DataTypePtr &,
                                                                                    const ColumnNullable *nullable_source,
                                                                                    size_t input_rows_count) -> ColumnPtr {
  ...
  if (from_qbit_type) {
    if (from_qbit_type->getDimension() != to_type.getDimension())
      throw Exception(ErrorCodes::TYPE_MISMATCH,
                      ...
                      from_qbit_type->getName(), to_type.getName());
    else if (!from_qbit_type->getElementType()->equals(*to_type.getElementType()))
      throw Exception(ErrorCodes::NOT_IMPLEMENTED,
                      "CAST AS between two QBits containing different types isn't implemented. From: {}, To: {}", from_qbit_type->getName(),
                      to_type.getName());
...
ColumnPtr FunctionCast::convertArrayToQBit(ColumnsWithTypeAndName &arguments, const DataTypePtr &, const ColumnNullable *nullable_source,
  ...
  if (!col_array)
    throw Exception(ErrorCodes::LOGICAL_ERROR, "Unexpected column type {} for Array source when converting to QBit", src_col->getName());
  ...
  for (size_t row = 0; row < arrays_count; ++row) {
    ...
    if (!(null_map && (*null_map)[row]) && array_size != n)
      throw Exception(ErrorCodes::SIZES_OF_ARRAYS_DONT_MATCH, "Array arguments must have size {} for QBit conversion, got {}", n,
                      array_size);
  ...
  if (arrays_count == 0) {
    ...
    for (size_t i = 0; i < size; ++i) empty_tuple_columns[i] = ColumnFixedString::create(bytes_per_fixedstring);
  ...
  for (size_t row = 0; row < arrays_count; ++row) {
    ...
    if (null_map && (*null_map)[row]) {
      for (size_t j = 0; j < size; ++j) assert_cast<ColumnFixedString &>(*tuple_columns[j]).insertDefault();
      prev_offset = off;
      continue;
    }
...
template <typename T>
FunctionCast::WrapperType FunctionCast::createArrayToQBitWrapper(const DataTypeArray &from_array_type,
                                                                 const DataTypeQBit &to_qbit_type) const {
  ...
  return [nested_function = prepareUnpackDictionaries(from_nested_type, to_nested_type), from_nested_type, to_nested_type,
          to_array_type = std::make_shared<DataTypeArray>(to_nested_type), dimension,
          element_size](ColumnsWithTypeAndName &arguments, const DataTypePtr &result_type, const ColumnNullable *nullable_source,
                        size_t /* input_rows_count */) -> ColumnPtr {
...
FunctionCast::WrapperType FunctionCast::createTupleToMapWrapper(const DataTypes &from_kv_types, const DataTypes &to_kv_types) const {
  return [element_wrappers = getElementWrappers(from_kv_types, to_kv_types), from_kv_types, to_kv_types](
             ColumnsWithTypeAndName &arguments, const DataTypePtr &, const ColumnNullable * /*nullable_source*/,
             size_t /*input_rows_count*/) -> ColumnPtr {
    const auto *col = arguments.front().column.get();
    ...
    if (keys_offsets != values_offsets)
      throw Exception(ErrorCodes::TYPE_MISMATCH, "CAST AS Map can only be performed from tuple of arrays with equal sizes.");
  ...
  if (const auto *from_tuple = checkAndGetDataType<DataTypeTuple>(from_type_untyped.get())) {
    if (from_tuple->getElements().size() != 2)
      throw Exception(ErrorCodes::TYPE_MISMATCH,
                      ...
                      "Left type: {}, right type: {}",
                      from_tuple->getName(), to_type->getName());
    ...
    for (const auto &elem : from_tuple->getElements()) {
      ...
      if (!type_array)
        throw Exception(ErrorCodes::TYPE_MISMATCH, "CAST AS Map can only be performed from tuples of array. Got: {}",
                        from_tuple->getName());
    ...
    if (!nested_tuple || nested_tuple->getElements().size() != 2)
      throw Exception(ErrorCodes::TYPE_MISMATCH,
                      ...
                      "Left type: {}, right type: {}",
                      from_array->getName(), to_type->getName());
    ...
    throw Exception(ErrorCodes::TYPE_MISMATCH,
                    "Unsupported types to CAST AS Map. "
                    "Left type: {}, right type: {}",
                    from_type_untyped->getName(), to_type->getName());
  }
}

FunctionCast::WrapperType FunctionCast::createObjectWrapper(const DataTypePtr &from_type, const DataTypeObject *to_object,
                                                            bool requested_result_is_nullable) const {
  if (checkAndGetDataType<DataTypeString>(from_type.get())) {
    return [this, requested_result_is_nullable](ColumnsWithTypeAndName &arguments, const DataTypePtr &result_type,
                                                const ColumnNullable *nullable_source, size_t input_rows_count) {
      if (requested_result_is_nullable && cast_type == CastType::accurateOrNull)
        return ConvertImplGenericFromString<false>::execute(arguments, makeNullable(result_type), nullable_source, input_rows_count,
                                                            settings);
  ...
  /// converted to JSON (for example, should we recursively convert nested Array(Tuple) to Array(JSON) or not, should we infer types from
  /// String fields, etc). Proper implementation of a conversion between 2 different JSON types is really complex, because different JSON
  /// types can have different parameters:
  /// - set of typed paths
  /// - set of skip rules
  ...
  if (checkAndGetDataType<DataTypeTuple>(from_type.get()) || checkAndGetDataType<DataTypeMap>(from_type.get()) ||
      checkAndGetDataType<DataTypeObject>(from_type.get())) {
    return [this](ColumnsWithTypeAndName &arguments, const DataTypePtr &result_type, const ColumnNullable *nullable_source,
                  size_t input_rows_count) {
  ...
  throw Exception(ErrorCodes::TYPE_MISMATCH, "Cast to {} can be performed only from String/Map/Object/Tuple/JSON. Got: {}",
                  to_object->getSchemaFormatString(), from_type->getName());
}

FunctionCast::WrapperType FunctionCast::createVariantToVariantWrapper(const DataTypeVariant &from_variant,
                                                                      const DataTypeVariant &to_variant) const {
  ...
  /// Check that the set of old variants types is a subset of new variant types and collect new global discriminator for each old global
  /// discriminator.
  ...
  for (const auto &[old_variant_type, old_discriminator] : old_variant_types_to_old_global_discriminator) {
    ...
    if (it == new_variant_types_to_new_global_discriminator.end())
      throw Exception(
          ...
          "of an initial one",
          from_variant.getName(), to_variant.getName());
  ...
  return [old_global_discriminator_to_new, variant_types_and_discriminators_to_add](ColumnsWithTypeAndName &arguments, const DataTypePtr &,
                                                                                    const ColumnNullable *, size_t) -> ColumnPtr {
    ...
    return ColumnVariant::create(column_variant.getLocalDiscriminatorsPtr(), column_variant.getOffsetsPtr(), new_variant_columns,
                                 new_local_to_global_discriminators);
  };
}
...
FunctionCast::WrapperType FunctionCast::createVariantToColumnWrapper(const DataTypeVariant &from_variant,
  ...
  return [variant_wrappers, variant_types, to_type](ColumnsWithTypeAndName &arguments, const DataTypePtr &result_type,
                                                    const ColumnNullable *, size_t input_rows_count) -> ColumnPtr {
    const auto &column_variant = assert_cast<const ColumnVariant &>(*arguments.front().column.get());
    ...
    for (size_t i = 0; i != variant_types.size(); ++i) {
      ...
      if (variant_wrapper) cast_variant = variant_wrapper(variant, result_type, nullptr, variant_col->size());
...
ColumnPtr FunctionCast::createVariantFromDescriptorsAndOneNonEmptyVariant(const DataTypes &variant_types, const ColumnPtr &discriminators,
                                                                          const ColumnPtr &variant,
                                                                          ColumnVariant::Discriminator variant_discr) {
  Columns variants;
...
FunctionCast::WrapperType FunctionCast::createColumnToVariantWrapper(const DataTypePtr &from_type,
                                                                     const DataTypeVariant &to_variant) const {
  ...
  if (settings.cast_string_to_variant_use_inference && isStringOrFixedString(removeNullable(removeLowCardinality(from_type))) &&
      (!variant_discr_opt || to_variant.getVariants().size() > 1))
    return createStringToVariantWrapper();

  if (!variant_discr_opt)
    throw Exception(ErrorCodes::CANNOT_CONVERT_TYPE,
                    "Cannot convert type {} to {}. Conversion to Variant allowed only for types from this Variant", from_type->getName(),
                    to_variant.getName());

  return [variant_discr = *variant_discr_opt](ColumnsWithTypeAndName &arguments, const DataTypePtr &result_type, const ColumnNullable *,
                                              size_t) -> ColumnPtr {
    ...
    if (const ColumnNullable *col_nullable = typeid_cast<const ColumnNullable *>(arguments.front().column.get())) {
      ...
      if (variant_size_hint == column->size()) variant_column = column;
      ...
      if (variant_size_hint == col_lc.size()) variant_column = col_lc.cloneWithDefaultOnNull();
...
FunctionCast::WrapperType FunctionCast::createDynamicToColumnWrapper(const DataTypePtr &) const {
  ...
  return [nested_convert](ColumnsWithTypeAndName &arguments, const DataTypePtr &result_type, const ColumnNullable *,
                          size_t input_rows_count) -> ColumnPtr {
...
FunctionCast::WrapperType FunctionCast::createVariantToDynamicWrapper(const DataTypeVariant &from_variant_type,
                                                                      const DataTypeDynamic &dynamic_type) const {
  ...
  return [old_to_new_variant_wrapper, variant_type_for_dynamic, number_of_variants, max_dynamic_types](
             ColumnsWithTypeAndName &arguments, const DataTypePtr &result_type, const ColumnNullable *col_nullable,
             size_t input_rows_count) -> ColumnPtr {
    ...
    auto tmp_dynamic_column =
        ColumnDynamic::create(variant_column_for_dynamic, variant_type_for_dynamic, number_of_variants, number_of_variants);
...
FunctionCast::WrapperType FunctionCast::createColumnToDynamicWrapper(const DataTypePtr &from_type,
                                                                     const DataTypeDynamic &dynamic_type) const {
...
FunctionCast::WrapperType FunctionCast::createDynamicToDynamicWrapper(const DataTypeDynamic &from_dynamic,
                                                                      const DataTypeDynamic &to_dynamic) const {
  size_t from_max_types = from_dynamic.getMaxDynamicTypes();
  size_t to_max_types = to_dynamic.getMaxDynamicTypes();
  if (from_max_types == to_max_types) return createIdentityWrapper(from_dynamic.getPtr());

  if (to_max_types > from_max_types) {
    return [to_max_types](ColumnsWithTypeAndName &arguments, const DataTypePtr &, const ColumnNullable *, size_t) -> ColumnPtr {
      ...
      return ColumnDynamic::create(dynamic_column.getVariantColumnPtr(), dynamic_column.getVariantInfo(),
                                   dynamic_column.getMaxDynamicTypes(), to_max_types);
    };
  }
  ...
  return [to_max_types](ColumnsWithTypeAndName &arguments, const DataTypePtr &, const ColumnNullable *, size_t) -> ColumnPtr {
    ...
    for (const auto &[name, discr] : variant_info.variant_name_to_discriminator) {
      ...
      if (discr == shared_variant_discr) continue;
      ...
      if (statistics) {
        ...
        if (statistics_it != statistics->variants_statistics.end()) size = statistics_it->second;
      }
    ...
    auto result_dynamic_column =
        ColumnDynamic::create(result_variant_type->createColumn(), result_variant_type, to_max_types, to_max_types);
      ...
      if (global_discr == ColumnVariant::NULL_DISCRIMINATOR) {
        ...
        if (result_global_discr == result_shared_variant_discr) {
          ...
          ColumnDynamic::serializeValueIntoSharedVariant(
              result_shared_variant, variant_column.getVariantByGlobalDiscriminator(global_discr), variants[global_discr],
              variants[global_discr]->getDefaultSerialization(), offsets[i]);
        } else {
...
FunctionCast::WrapperType FunctionCast::createEnumWrapper(const DataTypePtr &from_type, const DataTypeEnum<FieldType> *to_type) const {
  ...
  throw Exception(ErrorCodes::CANNOT_CONVERT_TYPE, "Conversion from {} to {} is not supported", from_type->getName(), to_type->getName());
}
...
void FunctionCast::checkEnumToEnumConversion(const EnumTypeFrom *from_type, const EnumTypeTo *to_type) const {
  ...
  for (const auto &name_value : name_intersection) {
    ...
    if (old_value != new_value)
      throw Exception(ErrorCodes::CANNOT_CONVERT_TYPE, "Enum conversion changes value for element '{}' from {} to {}", name_value.first,
...
FunctionCast::WrapperType FunctionCast::createStringToEnumWrapper() const {
  ...
  return [function_name](ColumnsWithTypeAndName &arguments, const DataTypePtr &res_type, const ColumnNullable *nullable_col,
                         size_t /*input_rows_count*/) {
    ...
    if (col) {
      ...
      if (nullable_col) {
        ...
        for (size_t i = 0; i < size; ++i) out_data[i] = result_type.getValue(col->getDataAt(i));
      }

      return res;
    } else
      throw Exception(ErrorCodes::LOGICAL_ERROR, "Unexpected column {} as first argument of function {}", first_col->getName(),
                      function_name);
  };
}
...
FunctionCast::WrapperType FunctionCast::createNumberToEnumWrapper() const {
  ...
  return [function_name = cast_name](ColumnsWithTypeAndName &arguments, const DataTypePtr &res_type, const ColumnNullable *nullable_col,
                                     size_t /*input_rows_count*/) {
    const auto &first_col = arguments.front().column.get();
    const auto &result_type = typeid_cast<const EnumType &>(*res_type);
    ...
    if (nullable_col) null_map = &nullable_col->getNullMapData();
...
FunctionCast::WrapperType FunctionCast::prepareUnpackDictionaries(const DataTypePtr &from_type, const DataTypePtr &to_type) const {
  ...
  if (isDynamic(to_type) || isDynamic(from_type)) return createDynamicWrapper(from_type, to_type);

  if (isVariant(to_type) || isVariant(from_type)) return createVariantWrapper(from_type, to_type);
  ...
  if (!from_low_cardinality && !to_low_cardinality) return wrapper;

  return [wrapper, from_low_cardinality, to_low_cardinality, skip_not_null_check](
             ColumnsWithTypeAndName &arguments, const DataTypePtr &result_type, const ColumnNullable *nullable_source,
             size_t input_rows_count) -> ColumnPtr {
...

    {
      ...
      if (to_low_cardinality) res_type = to_low_cardinality->getDictionaryType();
...
FunctionCast::WrapperType FunctionCast::prepareRemoveNullable(const DataTypePtr &from_type, const DataTypePtr &to_type,
  ...
  if (result_is_nullable) {
    return [wrapper, source_is_nullable](ColumnsWithTypeAndName &arguments, const DataTypePtr &result_type, const ColumnNullable *,
                                         size_t input_rows_count) -> ColumnPtr {
      ...
      if (source_is_nullable) {
        if (arguments.size() != 1) throw Exception(ErrorCodes::LOGICAL_ERROR, "Invalid number of arguments");
    ...
    return [wrapper, skip_not_null_check](ColumnsWithTypeAndName &arguments, const DataTypePtr &result_type, const ColumnNullable *,
                                          size_t input_rows_count) -> ColumnPtr {
...
FunctionCast::WrapperType FunctionCast::prepareImpl(const DataTypePtr &from_type, const DataTypePtr &to_type,
                                                    bool requested_result_is_nullable) const {
  if (isUInt8(from_type) && isBool(to_type)) return createUInt8ToBoolWrapper(from_type, to_type);
  ...
  if (from_type->equals(*to_type) && safe_convert_custom_types) {
    ...
    if (typeid_cast<const DataTypeAggregateFunction *>(from_type.get())) {
      if (DataTypeAggregateFunction::strictEquals(from_type, to_type)) return createIdentityWrapper(from_type);
    } else
  ...
  auto make_default_wrapper = [&](const auto &types) -> bool {
    ...
    if constexpr (is_any_of<ToDataType, DataTypeUInt16, DataTypeUInt32, DataTypeUInt64, DataTypeUInt128, DataTypeUInt256, DataTypeInt8,
                            DataTypeInt16, DataTypeInt32, DataTypeInt64, DataTypeInt128, DataTypeInt256, DataTypeBFloat16, DataTypeFloat32,
                            DataTypeFloat64, DataTypeDate, DataTypeDate32, DataTypeDateTime, DataTypeTime, DataTypeUUID, DataTypeIPv4,
                            DataTypeIPv6>) {
    ...
    if constexpr (std::is_same_v<ToDataType, DataTypeEnum8> || std::is_same_v<ToDataType, DataTypeEnum16>) {
      ret = createEnumWrapper(from_type, checkAndGetDataType<ToDataType>(to_type.get()));
      return true;
    }
    if constexpr (is_any_of<ToDataType, DataTypeDecimal<Decimal32>, DataTypeDecimal<Decimal64>, DataTypeDecimal<Decimal128>,
                            DataTypeDecimal<Decimal256>, DataTypeDateTime64, DataTypeTime64>) {
  ...
  auto make_custom_serialization_wrapper = [&, this](const auto &types) -> bool {
    ...
    if constexpr (WhichDataType(FromDataType::type_id).isStringOrFixedString()) {
      if constexpr (std::is_same_v<ToDataType, DataTypeIPv4>) {
        ret = [this, requested_result_is_nullable](ColumnsWithTypeAndName &arguments, const DataTypePtr &result_type,
                                                   const ColumnNullable *column_nullable, size_t) -> ColumnPtr {
      ...
      if constexpr (std::is_same_v<ToDataType, DataTypeIPv6>) {
        ret = [this, requested_result_is_nullable](ColumnsWithTypeAndName &arguments, const DataTypePtr &result_type,
          ...
          if (!WhichDataType(result_type).isIPv6())
            throw Exception(ErrorCodes::TYPE_MISMATCH, "Wrong result type {}. Expected IPv6", result_type->getName());
      ...
      if (to_type->getCustomSerialization() && to_type->getCustomName()) {
        ret = [requested_result_is_nullable, this](ColumnsWithTypeAndName &arguments, const DataTypePtr &result_type,
                                                   const ColumnNullable *column_nullable, size_t input_rows_count) -> ColumnPtr {
          auto wrapped_result_type = result_type;
          if (requested_result_is_nullable) wrapped_result_type = makeNullable(result_type);
          if (this->cast_type == CastType::accurateOrNull)
            return ConvertImplGenericFromString<false>::execute(arguments, wrapped_result_type, column_nullable, input_rows_count,
                                                                settings);
          return ConvertImplGenericFromString<true>::execute(arguments, wrapped_result_type, column_nullable, input_rows_count, settings);
        };
        return true;
      }
    ...
    if constexpr (WhichDataType(ToDataType::type_id).isStringOrFixedString()) {
      if constexpr (WhichDataType(FromDataType::type_id).isEnum()) {
        ...
        ret = [this](ColumnsWithTypeAndName &arguments, const DataTypePtr &result_type, const ColumnNullable *,
                     size_t input_rows_count) -> ColumnPtr {
  ...
  if (callOnTwoTypeIndexes(from_type->getTypeId(), to_type->getTypeId(), make_custom_serialization_wrapper)) return ret;
...
FunctionBasePtr createFunctionBaseCast(ContextPtr context, const char *name, const ColumnsWithTypeAndName &arguments,
  ...
  if (cast_type != CastType::accurateOrNull) monotonicity_result_type = removeNullable(monotonicity_result_type);

  if (isEnum(arguments.front().type) && castTypeToEither<DataTypeEnum8, DataTypeEnum16>(monotonicity_result_type.get(), [&](auto &type) {
        monotonicity = detail::FunctionTo<std::decay_t<decltype(type)>>::Type::Monotonic::get;
        return true;
      })) {
  } else if (castTypeToEither<DataTypeUInt8, DataTypeUInt16, DataTypeUInt32, DataTypeUInt64, DataTypeUInt128, DataTypeUInt256, DataTypeInt8,
                              DataTypeInt16, DataTypeInt32, DataTypeInt64, DataTypeInt128, DataTypeInt256, DataTypeFloat32, DataTypeFloat64,
...
bool castType(const IDataType *type, auto &&f) {
  using Types = TypeList<DataTypeUInt8, DataTypeUInt16, DataTypeUInt32, DataTypeUInt64, DataTypeUInt128, DataTypeUInt256, DataTypeInt8,
                         DataTypeInt16, DataTypeInt32, DataTypeInt64, DataTypeInt128, DataTypeInt256, DataTypeFloat32, DataTypeFloat64,
                         DataTypeDecimal32, DataTypeDecimal64, DataTypeDecimal128, DataTypeDecimal256, DataTypeDate, DataTypeDateTime,
                         DataTypeFixedString, DataTypeString, DataTypeInterval>;
...
bool convertIsCompilableImpl(const DataTypes &types, const DataTypePtr &result_type) {
  ...
  if (!canBeNativeType(types[0]) || !canBeNativeType(result_type)) return false;
...
llvm::Value *convertCompileImpl(llvm::IRBuilderBase &builder, const ValuesWithType &arguments, const DataTypePtr &result_type) {
  ...
  castBothTypes(arguments[0].type.get(), result_type.get(), [&](const auto &left, const auto &right) {
    using LeftDataType = std::decay_t<decltype(left)>;
    using RightDataType = std::decay_t<decltype(right)>;
    if constexpr (IsDataTypeDecimalOrNumber<LeftDataType> && IsDataTypeDecimalOrNumber<RightDataType>) {
...
bool FunctionCast::isCompilable() const {
  if (getName() != "CAST" || argument_types.size() != 2) return false;
  ...
  if (!canBeNativeType(denull_input_type) || !canBeNativeType(denull_result_type)) return false;
...
llvm::Value *FunctionCast::compile(llvm::IRBuilderBase &builder, const ValuesWithType &arguments) const {
  ...
  castBothTypes(denull_input_type.get(), denull_result_type.get(), [&](const auto &left, const auto &right) {
    using LeftDataType = std::decay_t<decltype(left)>;
    using RightDataType = std::decay_t<decltype(right)>;
  ...
  if (!result_value)
    throw Exception(ErrorCodes::LOGICAL_ERROR, "Cannot compile CAST function for types {} -> {}", input_type->getName(),
                    result_type->getName());
