...
class FunctionColorOKLABToSRGB : public ColorConversionToSRGBBase<FunctionColorOKLABToSRGB> {
 public:
  ...
  explicit FunctionColorOKLABToSRGB(ContextPtr context_) : ColorConversionToSRGBBase(context_) {}
...
REGISTER_FUNCTION(FunctionColorOKLABToSRGB) {
  ...
  FunctionDocumentation::Arguments arguments = {{"tuple",
                                                 "A tuple of three numeric values `L`, `a`, `b`, where `L` is in the range `[0...1]`.",
                                                 {"Tuple(Float64, Float64, Float64)"}},
                                                {"gamma",
                                                 "Optional. The exponent that is used to transform linear sRGB back to sRGB by applying "
                                                 "`(x ^ (1 / gamma)) * 255` for each channel `x`. Defaults to `2.2`.",
                                                 {"Float64"}}};
  FunctionDocumentation::ReturnedValue returned_value = {"Returns a tuple (R, G, B) representing sRGB color values.",
                                                         {"Tuple(Float64, Float64, Float64)"}};
  FunctionDocumentation::Examples examples = {{"Convert OKLAB to sRGB (Float)",
                                               R"(
