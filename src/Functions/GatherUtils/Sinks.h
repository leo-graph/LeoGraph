...
struct NumericArraySink : public ArraySinkImpl<NumericArraySink<T>> {
  ...
  void reserve(size_t num_elements) { elements.reserve_exact(num_elements); }
};
...
struct StringSink {
  ...
  StringSink(ColumnString &col, size_t column_size) : elements(col.getChars()), offsets(col.getOffsets()) {
    offsets.resize_exact(column_size);
  }
...
struct FixedStringSink {
  ...
  FixedStringSink(ColumnFixedString &col, size_t column_size) : elements(col.getChars()), string_size(col.getN()), total_rows(column_size) {
...
struct GenericArraySink : public ArraySinkImpl<GenericArraySink> {
  ...
  GenericArraySink(IColumn &elements_, ColumnArray::Offsets &offsets_, size_t column_size) : elements(elements_), offsets(offsets_) {
    offsets.resize_exact(column_size);
  }
