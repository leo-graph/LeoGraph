...
struct NumericArraySource : public ArraySourceImpl<NumericArraySource<T>> {
  ...
  MutableColumnPtr createValuesColumn() { return column.cloneEmpty(); }
  ...
  Slice getSliceFromLeft(size_t offset) const {
    ...
    if (offset >= elem_size) return {&elements[prev_offset], 0};
  ...
  Slice getSliceFromRight(size_t offset) const {
    size_t elem_size = offsets[row_num] - prev_offset;
    if (offset > elem_size) return {&elements[prev_offset], elem_size};
...
struct StringSource {
  ...
  explicit StringSource(const ColumnString &col) : elements(col.getChars()), offsets(col.getOffsets()) {}
  ...
  size_t getColumnSize() const { return offsets.size(); }
...
struct EnumSource {
  ...
  EnumSource(const Column &col, const EnumDataType &data_type_) : data(col.getData()), data_type(data_type_) {}
  ...
  Slice getSliceFromLeft(size_t offset) const {
    std::string_view name = data_type.getNameForValue(data[row_num]);
    if (offset >= name.size()) return {reinterpret_cast<const UInt8 *>(name.data()), 0};
  ...
  Slice getSliceFromRight(size_t offset) const {
    std::string_view name = data_type.getNameForValue(data[row_num]);
    if (offset > name.size()) return {reinterpret_cast<const UInt8 *>(name.data()), name.size()};
  ...
  Slice getSliceFromRight(size_t offset, size_t length) const {
    std::string_view name = data_type.getNameForValue(data[row_num]);
    if (offset > name.size())
      return {reinterpret_cast<const UInt8 *>(name.data()),
              length + name.size() > offset ? std::min(name.size(), length + name.size() - offset) : 0};
...
struct UTF8StringSource : public StringSource {
  ...
  size_t getElementSize() const { return UTF8::countCodePoints(&elements[prev_offset], StringSource::getElementSize()); }
  ...
  Slice getSliceFromLeft(size_t offset) const {
    ...
    if (res_begin >= end) return {begin, 0};
  ...
  Slice getSliceFromLeft(size_t offset, size_t length) const {
    ...
    if (res_end >= end) return {res_begin, size_t(end - res_begin)};
...
struct FixedStringSource {
  ...
  Slice getSliceFromLeft(size_t offset) const {
    if (offset >= string_size) return {pos, 0};
  ...
  Slice getSliceFromRight(size_t offset) const {
    if (offset > string_size) return {pos, string_size};
    return {pos + string_size - offset, offset};
  }

  Slice getSliceFromRight(size_t offset, size_t length) const {
    if (offset > string_size) return {pos, length + string_size > offset ? std::min(string_size, length + string_size - offset) : 0};
...
inline std::unique_ptr<IStringSource> createDynamicStringSource(const IColumn &col) {
  if (checkColumn<ColumnString>(&col)) return std::make_unique<DynamicStringSource<StringSource>>(col);
  if (checkColumn<ColumnFixedString>(&col)) return std::make_unique<DynamicStringSource<FixedStringSource>>(col);
  if (checkColumnConst<ColumnString>(&col)) return std::make_unique<DynamicStringSource<ConstSource<StringSource>>>(col);
  if (checkColumnConst<ColumnFixedString>(&col)) return std::make_unique<DynamicStringSource<ConstSource<FixedStringSource>>>(col);
...
struct GenericArraySource : public ArraySourceImpl<GenericArraySource> {
  ...
  MutableColumnPtr createValuesColumn() { return elements.cloneEmpty(); }

  explicit GenericArraySource(const ColumnArray &arr) : elements(arr.getData()), offsets(arr.getOffsets()) {}
...
struct NullableArraySource : public ArraySource {
  ...
  NullableArraySource(const ColumnArray &arr, const NullMap &null_map_) : ArraySource(arr), null_map(null_map_) {}
...
struct NullableValueSource : public ValueSource {
  ...
  NullableValueSource(const Column &col, const NullMap &null_map_) : ValueSource(col), null_map(null_map_) {}
