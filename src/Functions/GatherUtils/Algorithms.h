...
void concat(const std::vector<std::unique_ptr<IArraySource>> &array_sources, Sink &&sink) {
  ...
  auto check_and_get_size_to_reserve = [](auto source, IArraySource *array_source) {
    if (source == nullptr)
      throw Exception(ErrorCodes::LOGICAL_ERROR, "Concat function expected {} or {} but got {}", demangle(typeid(Source).name()),
                      demangle(typeid(ConstSource<Source>).name()), demangle(typeid(*array_source).name()));
...
void NO_INLINE sliceFromLeftConstantOffsetBounded(Source &&src, Sink &&sink, size_t offset, ssize_t length) {
  while (!src.isEnd()) {
    ssize_t size = length;
    if (size < 0) size += static_cast<ssize_t>(src.getElementSize()) - offset;

    if (size > 0) writeSlice(src.getSliceFromLeft(offset, size), sink);
...
void NO_INLINE sliceFromRightConstantOffsetBounded(Source &&src, Sink &&sink, size_t offset, ssize_t length) {
  while (!src.isEnd()) {
    ...
    if (size > 0) writeSlice(src.getSliceFromRight(offset, size), sink);
...
static void sliceDynamicOffsetBoundedImpl(Source &&src, Sink &&sink, const IColumn *offset_column, const IColumn *length_column) {
  ...
  while (!src.isEnd()) {
    ...
    if (size < 0) size += offset > 0 ? static_cast<Int64>(src.getElementSize()) - (offset - 1) : -UInt64(offset);
...
template <typename T>
bool insliceEqualElements(const NumericArraySlice<T> &first [[maybe_unused]], size_t first_ind [[maybe_unused]],
...
bool sliceHasImplStartsEndsWith(const FirstSliceType &first, const SecondSliceType &second, const UInt8 *first_null_map,
  ...
  for (size_t second_index = 0; second_index < second.size; ++second_index, ++first_index) {
    ...
    if (is_first_null != is_second_null) return false;
    if (!is_first_null && !is_second_null && !isEqual(first, second, first_index, second_index)) return false;
  }
  return true;
}
...
template <typename FirstSliceType, typename SecondSliceType,
          bool (*isEqual)(const FirstSliceType &, const SecondSliceType &, size_t, size_t),
          bool (*isEqualUnary)(const SecondSliceType &, size_t, size_t)>
bool sliceHasImplSubstr(const FirstSliceType &first, const SecondSliceType &second, const UInt8 *first_null_map,
                        const UInt8 *second_null_map) {
  ...
  if (has_second_null_map) {
    prefix_function = buildKMPPrefixFunction(second, [null_map = second_null_map](const SecondSliceType &pattern, size_t i, size_t j) {
      return !!null_map[i] == !!null_map[j] && (!!null_map[i] || isEqualUnary(pattern, i, j));
    });
  } else {
    prefix_function =
        buildKMPPrefixFunction(second, [](const SecondSliceType &pattern, size_t i, size_t j) { return isEqualUnary(pattern, i, j); });
  }
...
bool sliceHasImpl(const FirstSliceType &first, const SecondSliceType &second, const UInt8 *first_null_map, const UInt8 *second_null_map) {
  if constexpr (search_type == ArraySearchType::Substr)
    ...
    return sliceHasImplStartsEndsWith<search_type, FirstSliceType, SecondSliceType, isEqual>(first, second, first_null_map,
                                                                                             second_null_map);
  else
    ...
    if (has_size) {
      ...
      if (size >= 0) {
        ...
        if (length > MAX_ARRAY_SIZE)
          throw Exception(ErrorCodes::TOO_LARGE_ARRAY_SIZE, "Too large array size: {}, maximum: {}", length, MAX_ARRAY_SIZE);

        if (array_size <= length) {
          writeSlice(array_source.getWhole(), sink);
          for (size_t i = array_size; i < length; ++i) writeSlice(value_source.getWhole(), sink);
        } else
