...
struct ArraySourcePairSelector {
  ...
  static void selectSource(bool is_second_const, bool is_second_nullable, SecondSource &&second, bool is_first_const,
                           bool is_first_nullable, FirstSource &&first, Args &&...args) {
    if constexpr (std::is_same_v<FirstSource, SecondSource>) {
      Base::selectSourcePair(is_first_const, is_first_nullable, first, is_second_const, is_second_nullable, second, args...);
    }
  }
};
  ...
  static void selectImpl(Sink &&sink, IArraySource &array_source, IValueSource &value_source, Args &&...args) {
    ...
    auto check_type = [](auto source_ptr) {
      if (source_ptr == nullptr)
        throw Exception(ErrorCodes::LOGICAL_ERROR, "{} expected {} or {} or {} or {} but got {}", demangle(typeid(Base).name()),
    ...
    if (array_source.isConst() && value_source.isConst())
      ...
      check_type_and_call_concat(typeid_cast<ConstSource<ArraySource> *>(&array_source), typeid_cast<ValueSource *>(&value_source));
    else if (value_source.isConst())
      check_type_and_call_concat(typeid_cast<ArraySource *>(&array_source), typeid_cast<ConstSource<ValueSource> *>(&value_source));
    else
      check_type_and_call_concat(typeid_cast<ArraySource *>(&array_source), typeid_cast<ValueSource *>(&value_source));
  }
};
