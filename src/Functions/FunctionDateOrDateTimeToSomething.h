 ...
 public:
  ...
  DataTypePtr getReturnTypeImpl(const ColumnsWithTypeAndName &arguments) const override {
    ...
    if constexpr (std::is_same_v<ToDataType, DataTypeDateTime64>) {
      ...
      if (const auto *dt64 = checkAndGetDataType<DataTypeTime64>(arguments[0].type.get())) scale = dt64->getScale();
      auto source_scale = scale;
  ...
  DataTypePtr getReturnTypeForDefaultImplementationForDynamic() const override {
    ...
    if constexpr (!std::is_same_v<ToDataType, DataTypeDateTime> && !std::is_same_v<ToDataType, DataTypeTime> &&
  ...
  ColumnPtr executeImpl(const ColumnsWithTypeAndName &arguments, const DataTypePtr &result_type, size_t input_rows_count) const override {
    ...
    if (isTime64(from_type)) {
      ...
      return DateTimeTransformImpl<DataTypeTime64, ToDataType, decltype(transformer)>::execute(arguments, result_type, input_rows_count,
                                                                                               transformer);
    }
    ...
    if (isDateTime64(from_type)) {
      ...
      return DateTimeTransformImpl<DataTypeDateTime64, ToDataType, decltype(transformer)>::execute(arguments, result_type, input_rows_count,
                                                                                                   transformer);
    }
  ...
  FieldIntervalPtr getPreimage(const IDataType &type, const Field &point) const override {
    if constexpr (Transform::hasPreimage())
      ...
      throw Exception(ErrorCodes::NOT_IMPLEMENTED, "Function {} has no information about its preimage", Transform::name);
  }
};
