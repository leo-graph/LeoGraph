...
Matches all groups of a string using a regular expression and returns an array of arrays, where each array includes matching fragments from every group, grouped in order of appearance in the input string.
  ...
  FunctionDocumentation::ReturnedValue returned_value = {
      "Returns an array of arrays, where each inner array contains the captured groups from one match. Each match produces an array with "
      "elements corresponding to the capturing groups in the regular expression (group 1, group 2, etc.). If no matches are found, returns "
      "an empty array.",
      {"Array(Array(String))"}};
...
[['Server','nginx'],['Date','Tue, 22 Jan 2019 00:26:14 GMT'],['Content-Type','text/html; charset=UTF-8'],['Connection','keep-alive']]
  ...
  factory.registerFunction(
      ...
      [](ContextPtr ctx) {
        return FunctionExtractAllGroups::create(ctx, "extractAllGroupsVertical", ExtractAllGroupsResultKind::VERTICAL);
      },
      documentation);
