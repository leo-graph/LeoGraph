...
class FunctionFormatReadableTimeDelta : public IFunction {
 public:
  ...
  DataTypePtr getReturnTypeImpl(const DataTypes &arguments) const override {
    ...
    if (!isNumber(type)) throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT, "Cannot format {} as time delta", type.getName());
  ...
  enum Unit { Nanoseconds = 1, Microseconds = 2, Milliseconds = 3, Seconds = 4, Minutes = 5, Hours = 6, Days = 7, Months = 8, Years = 9 };
  ...
  ColumnPtr executeImpl(const ColumnsWithTypeAndName &arguments, const DataTypePtr &, size_t input_rows_count) const override {
    ...
    if (arguments.size() >= 2) {
      ...
      if (maximum_unit_const_col) maximum_unit_str = maximum_unit_const_col->getDataColumn().getDataAt(0);
      ...
      if (arguments.size() == 3) {
        ...
        if (minimum_unit_const_col) minimum_unit_str = minimum_unit_const_col->getDataColumn().getDataAt(0);
      }
    }
  ...
  static void processUnit(UInt64 unit_multiplier, UInt32 unit_scale, const char *unit_name, size_t unit_name_size, Float64 &whole_part,
    ...
    if (unlikely(whole_part + 1.0 == whole_part)) {
      ...
      writeText(std::floor(whole_part * static_cast<Float64>(DecimalUtils::scaleMultiplier<Int64>(unit_scale)) /
    ...
    if (unit_scale == 0)  /// dealing with whole number of seconds
    {
      ...
      if (!num_units) {
        ...
        if (unit_multiplier != 1 || has_output) return;
      }
    ...
    } else  /// dealing with sub-seconds, a bit more peculiar to avoid more precision issues
    {
      ...
      if (!num_units) {
        ...
        if (!is_minimum_unit || has_output) return;
      }
    }
    ...
    if (num_units != 1) writeChar('s', buf_to);
 ...
 private:
  ...
  Unit dispatchUnit(const std::string_view &unit_str, const Unit default_unit, const std::string &bound_name) const {
    if (unit_str.empty()) return default_unit;
    if (unit_str == "years") return Years;
    if (unit_str == "months") return Months;
    if (unit_str == "days") return Days;
    if (unit_str == "hours") return Hours;
    if (unit_str == "minutes") return Minutes;
    if (unit_str == "seconds") return Seconds;
    if (unit_str == "milliseconds") return Milliseconds;
    if (unit_str == "microseconds") return Microseconds;
    if (unit_str == "nanoseconds") return Nanoseconds;
    throw Exception(ErrorCodes::BAD_ARGUMENTS,
...
This function accepts any numeric type as input, but internally it casts them to `Float64`. Results might be suboptimal with large values.
  ...
  FunctionDocumentation::Arguments arguments = {
      ...
      {"maximum_unit",
       "Optional. Maximum unit to show. Acceptable values: `nanoseconds`, `microseconds`, `milliseconds`, `seconds`, `minutes`, `hours`, "
       "`days`, `months`, `years`. Default value: `years`.",
       {"const String"}},
      {"minimum_unit",
       "Optional. Minimum unit to show. All smaller units are truncated. Acceptable values: `nanoseconds`, `microseconds`, `milliseconds`, "
       "`seconds`, `minutes`, `hours`, `days`, `months`, `years`. If explicitly specified value is bigger than `maximum_unit`, an "
       "exception will be thrown. Default value: `seconds` if `maximum_unit` is `seconds` or bigger, `nanoseconds` otherwise.",
       {"const String"}}};
