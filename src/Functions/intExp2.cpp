...
struct IntExp2Impl {
  ...
  static ResultType apply([[maybe_unused]] A a) {
    if constexpr (is_big_int_v<A>) {
      ...
      if constexpr (std::is_floating_point_v<A>) {
        if (std::isnan(a)) throw Exception(ErrorCodes::BAD_ARGUMENTS, "intExp2 must not be called with nan");
        if (a < 0) return 0;
        if (a >= 64) return std::numeric_limits<UInt64>::max();
      }
  ...
  static llvm::Value *compile(llvm::IRBuilder<> &b, llvm::Value *arg, bool) {
    if (!arg->getType()->isIntegerTy()) throw Exception(ErrorCodes::LOGICAL_ERROR, "IntExp2Impl expected an integral type");
...
struct FunctionUnaryArithmeticMonotonicity<NameIntExp2> {
  ...
  static IFunction::Monotonicity get(const IDataType &, const Field &left, const Field &right) {
    ...
    if (left_float < 0 || right_float > 63) return {};
...
Like [exp2](#exp2) but returns a `UInt64` number.
  ...
  FunctionDocumentation::Arguments arguments = {{"x", "The exponent.", {"Int*", "UInt*", "Float*"}}};
