...
enum class ExtractAllGroupsResultKind : uint8_t { VERTICAL, HORIZONTAL };
 ...
 public:
  ...
  DataTypePtr getReturnTypeImpl(const ColumnsWithTypeAndName &arguments) const override {
    FunctionArgumentDescriptors args{
        {"haystack", static_cast<FunctionArgumentDescriptor::TypeValidator>(&isStringOrFixedString), nullptr,
         "const String or const FixedString"},
        {"needle", static_cast<FunctionArgumentDescriptor::TypeValidator>(&isStringOrFixedString), isColumnConst,
         "const String or const FixedString"},
    };
  ...
  ColumnPtr executeImpl(const ColumnsWithTypeAndName &arguments, const DataTypePtr &, size_t input_rows_count) const override {
    ...
    if (needle.empty()) throw Exception(ErrorCodes::BAD_ARGUMENTS, "Length of 'needle' argument must be greater than 0.");

    const OptimizedRegularExpression holder = Regexps::createRegexp<false, false, false>(needle);
    const auto &regexp = holder.getRE2();

    if (!regexp) throw Exception(ErrorCodes::BAD_ARGUMENTS, "There are no groups in regexp: {}", needle);

    const size_t groups_count = regexp->NumberOfCapturingGroups();

    if (!groups_count) throw Exception(ErrorCodes::BAD_ARGUMENTS, "There are no groups in regexp: {}", needle);

    if (groups_count > MAX_GROUPS_COUNT - 1)
      throw Exception(ErrorCodes::BAD_ARGUMENTS, "Too many groups in regexp: {}, max: {}", groups_count,
                      std::to_string(MAX_GROUPS_COUNT - 1));
    ...
    if (kind == ExtractAllGroupsResultKind::VERTICAL) {
      ...
      for (size_t i = 0; i < input_rows_count; ++i) {
        ...
        while (pos < end && regexp->Match({pos, static_cast<size_t>(end - pos)}, 0, end - pos, RE2::UNANCHORED, matched_groups.data(),
      ...
      for (size_t i = 0; i < input_rows_count; ++i) {
        ...
        while (pos < end && regexp->Match({pos, static_cast<size_t>(end - pos)}, 0, end - pos, RE2::UNANCHORED, matched_groups.data(),
                                          static_cast<int>(matched_groups.size()))) {
          // 1 is to exclude group #0 which is whole re match.
          for (size_t group = 1; group <= groups_count; ++group) all_matches.push_back(matched_groups[group]);

          ++matches_per_row;
          /// Additional limit to fail fast on supposedly incorrect usage.
          if (matches_per_row > regexp_max_matches_per_row)
            throw Exception(ErrorCodes::TOO_LARGE_ARRAY_SIZE, "Too many matches per row (> {}) in the result of function {}",
...

      {
        ...
        for (const auto &m : all_matches) total_matched_groups_string_len += m.length();
