...
class FunctionCaseWithExpression : public IFunction {
 public:
  ...
  DataTypePtr getReturnTypeImpl(const DataTypes& args) const override {
    if (args.empty()) throw Exception(ErrorCodes::TOO_FEW_ARGUMENTS_FOR_FUNCTION, "Function {} expects at least 1 arguments", getName());
    ...
    if (args.size() < 4 || args.size() % 2 != 0)
      throw Exception(ErrorCodes::BAD_ARGUMENTS, "Function {} expects an even number of arguments: (expr, when1, then1, ..., else)",
                      getName());
    ...
    for (size_t i = 2; i < args.size() - 1; i += 2) dst_array_types.push_back(args[i]);
  ...
  ColumnPtr caseWhenEquals(const ColumnWithTypeAndName& expr, const ColumnWithTypeAndName& when_value, size_t input_rows_count) const {
    ...
    auto is_null_expr =
        is_null_func->build(is_null_expr_args)->execute(is_null_expr_args, std::make_shared<DataTypeUInt8>(), input_rows_count, false);

    // isNull(when)
    ColumnsWithTypeAndName is_null_when_args{when_value};
    auto is_null_when =
        is_null_func->build(is_null_when_args)->execute(is_null_when_args, std::make_shared<DataTypeUInt8>(), input_rows_count, false);
    ...
    if (equals_return_type->isNullable()) {
      ...
      ColumnsWithTypeAndName if_null_args{{equals_result, equals_return_type, ""}, {zero_const, std::make_shared<DataTypeUInt8>(), ""}};
      equals_result = if_null_func->build(if_null_args)->execute(if_null_args, std::make_shared<DataTypeUInt8>(), input_rows_count, false);
    }

    // if (isNull(when)) then 0 else (expr = when)
    auto zero_const = DataTypeUInt8().createColumnConst(input_rows_count, 0u);
    ColumnsWithTypeAndName inner_if_args{{is_null_when, std::make_shared<DataTypeUInt8>(), ""},
                                         {zero_const, std::make_shared<DataTypeUInt8>(), ""},
                                         {equals_result, std::make_shared<DataTypeUInt8>(), ""}};
    auto inner_if_result =
        if_func->build(inner_if_args)->execute(inner_if_args, std::make_shared<DataTypeUInt8>(), input_rows_count, false);

    // if (isNull(expr)) then (isNull(when)) else inner_if_result
    ColumnsWithTypeAndName outer_if_args{{is_null_expr, std::make_shared<DataTypeUInt8>(), ""},
                                         {is_null_when, std::make_shared<DataTypeUInt8>(), ""},
                                         {inner_if_result, std::make_shared<DataTypeUInt8>(), ""}};
    return if_func->build(outer_if_args)->execute(outer_if_args, std::make_shared<DataTypeUInt8>(), input_rows_count, false);
  }

  ColumnPtr executeImpl(const ColumnsWithTypeAndName& args, const DataTypePtr& result_type, size_t input_rows_count) const override {
    if (args.empty()) throw Exception(ErrorCodes::TOO_FEW_ARGUMENTS_FOR_FUNCTION, "Function {} expects at least 1 argument", getName());
    ...
    src_array_col.column =
        fun_array->build(src_array_elems)->execute(src_array_elems, src_array_type, input_rows_count, /* dry_run = */ false);
    dst_array_col.column =
        fun_array->build(dst_array_elems)->execute(dst_array_elems, dst_array_type, input_rows_count, /* dry_run = */ false);

    /// If we have non-constant arguments, we execute the transform function, which is highly optimized, which uses precomputed lookup
    /// tables. Else we should use the multiIf implementation
...
Implements the `CASE expr WHEN val1 THEN result1 ... ELSE default END` expression. Internally transforms into a series of `multiIf` calls using equality comparison. Note that `NULL = NULL` evaluates to true in this context, unlike standard SQL equality.
  ...
  FunctionDocumentation::Arguments arguments = {{"expr", "The expression to compare.", {"Expression"}},
