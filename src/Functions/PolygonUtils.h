...
UInt64 getPolygonAllocatedBytes(const Polygon &polygon) {
  ...
  for (auto &inner : inners) size += size_of_ring(inner);
...
UInt64 getMultiPolygonAllocatedBytes(const MultiPolygon &multi_polygon) {
  ...
  for (const auto &polygon : multi_polygon) size += getPolygonAllocatedBytes(polygon);
...
class PointInPolygonTrivial {
 public:
  ...
  explicit PointInPolygonTrivial(const Polygon &polygon_) : polygon(polygon_) {}
  ...
  bool contains(CoordinateType x, CoordinateType y) const { return boost::geometry::covered_by(Point(x, y), polygon); }
...
class PointInPolygon {
 public:
  ...
  explicit PointInPolygon(const Polygon &polygon_) : polygon(polygon_) {
    ...
    if (min_corner.x() == max_corner.x() || min_corner.y() == max_corner.y()) has_empty_bound = true;
  }
  ...
  inline bool contains(CoordinateType x, CoordinateType y) const {
    ...
    if (!boost::geometry::within(point, box)) return false;
...
class PointInMultiPolygonRTree {
 public:
  ...
  explicit PointInMultiPolygonRTree(const MultiPolygon &multi_polygon_, UInt16 grid_size_ = 8) : multi_polygon(multi_polygon_) {
    build(grid_size_);
  }

  /// O(log N + K) where K = polygons that contain the point.
  bool contains(CoordinateType x, CoordinateType y) const {
    if (has_empty_bound || !isFinite(x) || !isFinite(y)) return false;

    for (auto it = rtree.qbegin(bgi::contains(Point(x, y))); it != rtree.qend(); ++it) {
      if (polygon_impls[it->second].contains(x, y)) return true;
    }
  ...
  UInt64 getAllocatedBytes() const {
    ...
    for (const auto &impl : polygon_impls) size += impl.getAllocatedBytes();
...
class PointInPolygonWithGrid {
 ...
 private:
  enum class CellType : uint8_t {
    ...
    pairOfLinesSingleNonConvexPolygons,  /// The cell is split to inner/outer part by a polyline of two sections and inner part is non
                                         /// convex.
  ...
  template <typename T>
  T getCellIndex(T row, T col) const {
...
UInt64 PointInPolygonWithGrid<CoordinateType>::getAllocatedBytes() const {
  ...
  for (const auto &elem : polygons) size += getMultiPolygonAllocatedBytes(elem);
...
void PointInPolygonWithGrid<CoordinateType>::calcGridAttributes(PointInPolygonWithGrid<CoordinateType>::Box &box) {
  ...
  if (!(isFinite(x_scale) && isFinite(y_scale) && isFinite(x_shift) && isFinite(y_shift) && isFinite(grid_size)))
...
bool PointInPolygonWithGrid<CoordinateType>::contains(CoordinateType x, CoordinateType y) const {
  if (has_empty_bound) return false;
  ...
  if (float_row < 0 || float_row > grid_size) return false;
  if (float_col < 0 || float_col > grid_size) return false;
...
bool PointInPolygonWithGrid<CoordinateType>::isConvex(const PointInPolygonWithGrid<CoordinateType>::Polygon &poly) {
  ...
  auto get_vector = [](const Point &from, const Point &to) -> Point { return Point(to.x() - from.x(), to.y() - from.y()); };
  ...
  for (auto i : collections::range(1, outer.size() - 1)) {
    ...
    if (vec_product(prev, cur) < 0) return false;
...
template <typename CoordinateType>
std::vector<typename PointInPolygonWithGrid<CoordinateType>::HalfPlane> PointInPolygonWithGrid<CoordinateType>::findHalfPlanes(
    const PointInPolygonWithGrid<CoordinateType>::Box &box, const PointInPolygonWithGrid<CoordinateType>::Polygon &intersection) {
  ...
  for (auto i : collections::range(0, outer.size() - 1)) {
    ...
    if (!((x1 == x2 && (x1 == box_x1 || x2 == box_x2)) || (y1 == y2 && (y1 == box_y1 || y2 == box_y2)))) {
...
template <typename CoordinateType>
void PointInPolygonWithGrid<CoordinateType>::addComplexPolygonCell(size_t index, const PointInPolygonWithGrid<CoordinateType>::Box &box) {
...
template <typename CoordinateType>
void PointInPolygonWithGrid<CoordinateType>::addCell(size_t index, const PointInPolygonWithGrid<CoordinateType>::Box &empty_box) {
...
template <typename CoordinateType>
void PointInPolygonWithGrid<CoordinateType>::addCell(size_t index, const PointInPolygonWithGrid<CoordinateType>::Box &box,
                                                     const PointInPolygonWithGrid<CoordinateType>::Polygon &intersection) {
  if (!intersection.inners().empty()) addComplexPolygonCell(index, box);
  ...
  if (half_planes.empty()) {
    ...
    cells[index].type = isConvex(intersection) ? CellType::pairOfLinesSingleConvexPolygon : CellType::pairOfLinesSingleNonConvexPolygons;
    cells[index].half_planes[0] = half_planes[0];
...
void PointInPolygonWithGrid<CoordinateType>::addCell(size_t index, const PointInPolygonWithGrid<CoordinateType>::Box &box,
  ...
  if (!first.inners().empty() || !second.inners().empty()) addComplexPolygonCell(index, box);
...
ColumnPtr pointInPolygon(const ColumnVector<T> &x, const ColumnVector<U> &y, PointInPolygonImpl &&impl) {
  ...
  if (impl.hasEmptyBound()) return ColumnVector<UInt8>::create(size, static_cast<UInt8>(0));
...
struct CallPointInPolygon<Type, Types...> {
  ...
  static ColumnPtr call(const IColumn &x, const IColumn &y, PointInPolygonImpl &&impl) {
    ...
    if (auto column = typeid_cast<const ColumnVector<Type> *>(&x)) return Impl::call(*column, y, std::forward<PointInPolygonImpl>(impl));
    return CallPointInPolygon<Types...>::call(x, y, std::forward<PointInPolygonImpl>(impl));
  }
};
