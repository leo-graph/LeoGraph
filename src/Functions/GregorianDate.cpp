...
namespace {
constexpr bool is_leap_year(int32_t year) { return (year % 4 == 0) && ((year % 400 == 0) || (year % 100 != 0)); }
...
constexpr I div(I x, J y) {
  ...
  if (x > 0 && y_cast < 0) return ((x - 1) / y_cast) - 1;
  if (x < 0 && y_cast > 0) return ((x + 1) / y_cast) - 1;
  return x / y_cast;
}
...
constexpr I mod(I x, J y) {
  ...
  if ((x > 0 && y_cast < 0) || (x < 0 && y_cast > 0)) return r == 0 ? static_cast<I>(0) : r + y_cast;
  return r;
}
...
void GregorianDate::init(ReadBuffer& in) {
  year_ = readDigit(in) * 1000 + readDigit(in) * 100 + readDigit(in) * 10 + readDigit(in);

  assertChar('-', in);

  month_ = readDigit(in) * 10 + readDigit(in);

  assertChar('-', in);

  day_of_month_ = readDigit(in) * 10 + readDigit(in);

  assertEOF(in);

  if (month_ < 1 || month_ > 12 || day_of_month_ < 1 || day_of_month_ > monthLength(is_leap_year(year_), month_))
    throw Exception(ErrorCodes::CANNOT_PARSE_DATE, "Invalid date, out of range (year: {}, month: {}, day_of_month: {}).", year_, month_,
                    day_of_month_);
}
...
bool GregorianDate::tryInit(ReadBuffer& in) {
  ...
  if (!tryReadDigit(in, c[0]) || !tryReadDigit(in, c[1]) || !tryReadDigit(in, c[2]) || !tryReadDigit(in, c[3]) || !checkChar('-', in) ||
      !tryReadDigit(in, c[4]) || !tryReadDigit(in, c[5]) || !checkChar('-', in) || !tryReadDigit(in, c[6]) || !tryReadDigit(in, c[7]) ||
      !in.eof()) {
    return false;
  }
  ...
  if (month_ < 1 || month_ > 12 || day_of_month_ < 1 || day_of_month_ > monthLength(is_leap_year(year_), month_)) return false;

  return true;
}

GregorianDate::GregorianDate(ReadBuffer& in) { init(in); }
...
bool GregorianDate::tryInit(int64_t modified_julian_day) {
  OrdinalDate ord;
  if (!ord.tryInit(modified_julian_day)) return false;
...
GregorianDate::GregorianDate(int64_t modified_julian_day) { init(modified_julian_day); }
...
bool GregorianDate::tryToModifiedJulianDay(int64_t& res) const {
  ...
  if (!ord.tryInit(year_, day_of_year)) return false;
...
ReturnType GregorianDate::writeImpl(WriteBuffer& buf) const {
  if (year_ < 0 || year_ > 9999) {
    if constexpr (std::is_same_v<ReturnType, void>)
      throw Exception(ErrorCodes::CANNOT_FORMAT_DATETIME, "Impossible to stringify: year too big or small: {}", year_);
    else
      return false;
  } else {
    auto y = year_;
    writeChar('0' + static_cast<char>(y / 1000), buf);
    y %= 1000;
    writeChar('0' + static_cast<char>(y / 100), buf);
    y %= 100;
    writeChar('0' + static_cast<char>(y / 10), buf);
    y %= 10;
...
void OrdinalDate::init(int64_t modified_julian_day) {
  if (!tryInit(modified_julian_day))
    throw Exception(ErrorCodes::CANNOT_FORMAT_DATETIME, "Value cannot be represented as date because it's out of range");
}
...
bool OrdinalDate::tryInit(int64_t modified_julian_day) {
  ...
  if (modified_julian_day < -678941) return false;

  if (modified_julian_day > 2973483) return false;
...
OrdinalDate::OrdinalDate(int32_t year, uint16_t day_of_year) { init(year, day_of_year); }

OrdinalDate::OrdinalDate(int64_t modified_julian_day) { init(modified_julian_day); }

int64_t OrdinalDate::toModifiedJulianDay() const noexcept {
  const auto y = year_ - 1;

  return day_of_year_ + 365 * y + div(y, 4) - div(y, 100) + div(y, 400) - 678576;
}

MonthDay::MonthDay(uint8_t month, uint8_t day_of_month) : month_(month), day_of_month_(day_of_month) {
  if (month < 1 || month > 12) throw Exception(ErrorCodes::LOGICAL_ERROR, "Invalid month: {}", month);
...
MonthDay::MonthDay(bool is_leap_year, uint16_t day_of_year) {
  if (day_of_year < 1 || day_of_year > (is_leap_year ? 366 : 365))
    throw Exception(ErrorCodes::LOGICAL_ERROR, "Invalid day of year: {}{}", (is_leap_year ? "leap, " : "non-leap, "), day_of_year);
...
uint16_t MonthDay::dayOfYear(bool is_leap_year) const {
  if (day_of_month_ < 1 || day_of_month_ > monthLength(is_leap_year, month_)) {
    throw Exception(ErrorCodes::LOGICAL_ERROR, "Invalid day of month: {}{}-{}", (is_leap_year ? "leap, " : "non-leap, "), month_,
                    day_of_month_);
  }
