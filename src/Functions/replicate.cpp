...
DataTypePtr FunctionReplicate::getReturnTypeImpl(const ColumnsWithTypeAndName &arguments) const {
  FunctionArgumentDescriptors mandatory_args{{"value", nullptr, nullptr, "Any"}, {"array", &isArray, nullptr, "Array"}};
...
ColumnPtr FunctionReplicate::executeImpl(const ColumnsWithTypeAndName &arguments, const DataTypePtr &, size_t) const {
  ...
  for (size_t i = 1; i < arguments.size(); ++i) {
    ...
    if (!array_column) {
      ...
      if (!const_array_column) throw Exception(ErrorCodes::ILLEGAL_COLUMN, "Unexpected column for replicate");
      temp_column = const_array_column->convertToFullColumn();
      array_column = checkAndGetColumn<ColumnArray>(temp_column.get());
    }

    if (!offsets || offsets->empty()) offsets = array_column->getOffsetsPtr();
  }
...
Creates an array with a single value.
  ...
  FunctionDocumentation::Arguments arguments = {{"x", "The value to fill the result array with.", {"Any"}},
                                                {"arr", "An array.", {"Array(T)"}}};
  FunctionDocumentation::ReturnedValue returned_value = {"Returns an array of the same length as `arr` filled with value `x`.",
                                                         {"Array(T)"}};
