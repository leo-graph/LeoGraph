...
class FunctionConsistentHashImpl : public IFunction {
 public:
  ...
  static FunctionPtr create(ContextPtr) { return std::make_shared<FunctionConsistentHashImpl<Impl>>(); }
  ...
  DataTypePtr getReturnTypeImpl(const DataTypes &arguments) const override {
    if (!isInteger(arguments[0]) && !isIPv4(arguments[0]))
      throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT, "Illegal type {} of the first argument of function {}", arguments[0]->getName(),
                      getName());

    if (arguments[0]->getSizeOfValueInMemory() > sizeof(HashType))
      throw Exception(ErrorCodes::BAD_ARGUMENTS, "Function {} accepts {}-bit integers at most, got {}", getName(), sizeof(HashType) * 8,
                      arguments[0]->getName());
  ...
  DataTypePtr getReturnTypeForDefaultImplementationForDynamic() const override { return std::make_shared<DataTypeNumber<ResultType>>(); }
  ...
  ColumnPtr executeImpl(const ColumnsWithTypeAndName &arguments, const DataTypePtr &, size_t /*input_rows_count*/) const override {
    if (isColumnConst(*arguments[1].column)) return executeConstBuckets(arguments);
 ...
 private:
  ...
  BucketsType checkBucketsRange(T buckets) const {
    if (unlikely(buckets <= 0))
      throw Exception(ErrorCodes::BAD_ARGUMENTS, "The second argument of function {} (number of buckets) must be positive number",
                      getName());

    if (unlikely(static_cast<UInt64>(buckets) > Impl::max_buckets))
      throw Exception(ErrorCodes::BAD_ARGUMENTS,
                      "The value of the second argument of function {} "
                      "(number of buckets) must not be greater than {}",
                      getName(), Impl::max_buckets);
  ...
  ColumnPtr executeConstBuckets(const ColumnsWithTypeAndName &arguments) const {
    ...
    if (which.isUInt8())
      ...
      throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT, "Illegal type {} of the first argument of function {}", hash_type->getName(),
                      getName());
  ...
  void executeType(const ColumnPtr &col_hash_ptr, BucketsType num_buckets, ColumnVector<ResultType> *col_result) const {
    ...
    if (!col_hash) throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT, "Illegal type of the first argument of function {}", getName());
    ...
    for (size_t i = 0; i < size; ++i) vec_result[i] = Impl::apply(static_cast<HashType>(vec_hash[i]), num_buckets);
  }
};
