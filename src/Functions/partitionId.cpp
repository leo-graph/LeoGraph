 ...
 public:
  ...
  ColumnPtr executeImpl(const ColumnsWithTypeAndName &arguments, const DataTypePtr &, size_t input_rows_count) const override {
    ...
    for (size_t j = 0; j < input_rows_count; ++j) {
      Row row(size);
      for (size_t i = 0; i < size; ++i) arguments[i].column->get(j, row[i]);
...
:::
  ...
  FunctionDocumentation::Arguments arguments = {{"column1, column2, ...", "Column for which to return the partition ID."}};
