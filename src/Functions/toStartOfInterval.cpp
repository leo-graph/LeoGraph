...
class FunctionToStartOfInterval : public IFunction {
 ...
 public:
  ...
  explicit FunctionToStartOfInterval(ContextPtr context_)
      : enable_extended_results_for_datetime_functions(
            context_->getSettingsRef()[Setting::enable_extended_results_for_datetime_functions]) {}
  ...
  DataTypePtr getReturnTypeImpl(const ColumnsWithTypeAndName &arguments) const override {
    ...
    enum class ResultType : uint8_t { Date, Date32, DateTime, DateTime64 };
    ...
    auto check_second_argument = [&] {
      ...
      if (!interval_type)
        throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT, "Illegal type {} of 2nd argument of function {}, expected a time interval",
    ...
    auto check_third_argument = [&] {
      ...
      if (isString(type_arg3)) {
        ...
        if (isDate(type_arg1) && isDate(type_arg3))
          ...
          throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT,
                          "Datetime argument and origin argument for function {} must have the same type", getName());
      } else
        throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT,
                        "Illegal type {} of 3rd argument of function {}. "
                        "This argument is optional and must be a constant String with timezone name or a Date/Date32/DateTime/DateTime64 "
                        "with a constant origin",
    ...
    if (arguments.size() == 2) {
      ...
      throw Exception(ErrorCodes::NUMBER_OF_ARGUMENTS_DOESNT_MATCH,
                      "Number of arguments for function {} doesn't match: passed {}, must be 2, 3 or 4", getName(), arguments.size());
    }
      ...
      case ResultType::DateTime64: {
        ...
        if (isDateTime64(arguments[0].type) && overload == Overload::Origin) {
          ...
          if (assert_cast<const DataTypeDateTime64 &>(*arguments[2].type.get()).getScale() != scale)
            throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT,
                            "Datetime argument and origin argument for function {} must have the same scale", getName());
        }
        ...
        return std::make_shared<DataTypeDateTime64>(scale,
                                                    extractTimeZoneNameFromFunctionArguments(arguments, time_zone_arg_num, 0, false));
      }
    }
  ...
  ColumnPtr executeImpl(const ColumnsWithTypeAndName &arguments, const DataTypePtr &result_type,
    ...
    if (overload == Overload::Origin) origin_column = arguments[2];
    ...
    } else  /// As we convert date to datetime and perform calculation, we don't need to take the timezone into account, so we set it to
            /// default
 ...
 private:
  ...
  ColumnPtr dispatchForTimeColumn(const ColumnWithTypeAndName &time_column, const ColumnWithTypeAndName &interval_column,
    ...
    if (isDate(time_column_type)) {
      ...
      if (time_column_vec)
        return dispatchForIntervalColumn<ReturnType, DataTypeDate32, ColumnDate32>(assert_cast<const DataTypeDate32 &>(time_column_type),
                                                                                   *time_column_vec, interval_column, origin_column,
                                                                                   result_type, time_zone);
      ...
      if (time_column_vec)
        return dispatchForIntervalColumn<ReturnType, DataTypeDateTime, ColumnDateTime>(
            assert_cast<const DataTypeDateTime &>(time_column_type), *time_column_vec, interval_column, origin_column, result_type,
            time_zone);
      ...
      if (time_column_vec)
        return dispatchForIntervalColumn<ReturnType, DataTypeDateTime64, ColumnDateTime64>(
            assert_cast<const DataTypeDateTime64 &>(time_column_type), *time_column_vec, interval_column, origin_column, result_type,
            time_zone, static_cast<UInt16>(scale));
    }
    throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT,
                    "Illegal column for 1st argument of function {}, expected a Date, Date32, DateTime or DateTime64", getName());
  }
  ...
  ColumnPtr dispatchForIntervalColumn(const TimeDataType &time_data_type, const TimeColumnType &time_column,
    ...
    switch (interval_type->getKind())  // NOLINT(bugprone-switch-missing-default-case)
    {
      ...
      case IntervalKind::Kind::Millisecond:
        if (isDateOrDate32(time_data_type) || isDateTime(time_data_type))
          throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT, "Illegal interval kind for argument data type {}",
    ...
    if (!interval_column_const_int64)
      throw Exception(ErrorCodes::ILLEGAL_COLUMN, "Illegal column for 2nd argument of function {}, must be a const time interval",
                      getName());
    ...
    switch (interval_type->getKind())  // NOLINT(bugprone-switch-missing-default-case)
    {
      ...
      case IntervalKind::Kind::Millisecond:
        return execute<ReturnType, TimeDataType, TimeColumnType, IntervalKind::Kind::Millisecond>(
            time_data_type, time_column, num_units, origin_column, result_type, time_zone, scale);
      case IntervalKind::Kind::Second:
        return execute<ReturnType, TimeDataType, TimeColumnType, IntervalKind::Kind::Second>(time_data_type, time_column, num_units,
                                                                                             origin_column, result_type, time_zone, scale);
      case IntervalKind::Kind::Minute:
        return execute<ReturnType, TimeDataType, TimeColumnType, IntervalKind::Kind::Minute>(time_data_type, time_column, num_units,
                                                                                             origin_column, result_type, time_zone, scale);
      case IntervalKind::Kind::Hour:
        return execute<ReturnType, TimeDataType, TimeColumnType, IntervalKind::Kind::Hour>(time_data_type, time_column, num_units,
                                                                                           origin_column, result_type, time_zone, scale);
      case IntervalKind::Kind::Day:
        return execute<ReturnType, TimeDataType, TimeColumnType, IntervalKind::Kind::Day>(time_data_type, time_column, num_units,
                                                                                          origin_column, result_type, time_zone, scale);
      case IntervalKind::Kind::Week:
        return execute<ReturnType, TimeDataType, TimeColumnType, IntervalKind::Kind::Week>(time_data_type, time_column, num_units,
                                                                                           origin_column, result_type, time_zone, scale);
      case IntervalKind::Kind::Month:
        return execute<ReturnType, TimeDataType, TimeColumnType, IntervalKind::Kind::Month>(time_data_type, time_column, num_units,
                                                                                            origin_column, result_type, time_zone, scale);
      case IntervalKind::Kind::Quarter:
        return execute<ReturnType, TimeDataType, TimeColumnType, IntervalKind::Kind::Quarter>(time_data_type, time_column, num_units,
                                                                                              origin_column, result_type, time_zone, scale);
      case IntervalKind::Kind::Year:
        return execute<ReturnType, TimeDataType, TimeColumnType, IntervalKind::Kind::Year>(time_data_type, time_column, num_units,
                                                                                           origin_column, result_type, time_zone, scale);
    }
  ...
  ColumnPtr execute(const TimeDataType &, const TimeColumnType &time_column_type, Int64 num_units,
                    const ColumnWithTypeAndName &origin_column, const DataTypePtr &result_type, const DateLUTImpl &time_zone,
                    UInt16 scale) const {
    ...
    if (origin_column.column)  // Overload: Origin
    {
      const bool is_small_interval =
          (unit == IntervalKind::Kind::Nanosecond || unit == IntervalKind::Kind::Microsecond || unit == IntervalKind::Kind::Millisecond);
      ...
      if (is_small_interval) result_scale = assert_cast<const DataTypeDateTime64 &>(*result_type).getScaleMultiplier();
      ...
      for (size_t i = 0; i != size; ++i) {
        ...
        if (origin > time_arg) throw Exception(ErrorCodes::BAD_ARGUMENTS, "The origin must be before the end date / date with time");

        if (is_result_date)  /// All internal calculations of ToStartOfInterval<...> expect arguments to be seconds or milli-, micro-,
                             /// nanoseconds.
        {
...
toStartOfInterval(value, INTERVAL x unit[, origin[, time_zone]])
    ...
    FunctionDocumentation::Examples examples = {{"Basic interval rounding", R"(
