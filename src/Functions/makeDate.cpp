 ...
 public:
  ...
  DataTypePtr getReturnTypeImpl(const ColumnsWithTypeAndName &arguments) const override {
    ...
    if (is_year_month_variant) {
      FunctionArgumentDescriptors args{{mandatory_argument_names_year_month_day[0],
                                        static_cast<FunctionArgumentDescriptor::TypeValidator>(&isNumber), nullptr, "Number"},
                                       {mandatory_argument_names_year_month_day[1],
                                        static_cast<FunctionArgumentDescriptor::TypeValidator>(&isNumber), nullptr, "Number"},
                                       {mandatory_argument_names_year_month_day[2],
                                        static_cast<FunctionArgumentDescriptor::TypeValidator>(&isNumber), nullptr, "Number"}};
      validateFunctionArguments(*this, arguments, args);
    } else {
      FunctionArgumentDescriptors args{{mandatory_argument_names_year_dayofyear[0],
                                        static_cast<FunctionArgumentDescriptor::TypeValidator>(&isNumber), nullptr, "Number"},
                                       {mandatory_argument_names_year_dayofyear[1],
                                        static_cast<FunctionArgumentDescriptor::TypeValidator>(&isNumber), nullptr, "Number"}};
      validateFunctionArguments(*this, arguments, args);
    }
    ...
    if (is_year_month_day_variant) {
      ...
      for (size_t i = 0; i < input_rows_count; ++i) {
        ...
        if (year >= Traits::MIN_YEAR && year <= Traits::MAX_YEAR && month >= 1 && month <= 12 && day >= 1 && day <= 31) {
          ...
          if (days_since_epoch <= max_days_since_epoch) day_num = days_since_epoch;
        }
...
class FunctionDateTimeBase : public FunctionWithNumericParamsBase {
 protected:
  ...
  static Int64 dateTime(T year, T month, T day_of_month, T hour, T minute, T second, const DateLUTImpl &lut) {
    ...
    if (std::isnan(year) || std::isnan(month) || std::isnan(day_of_month) || std::isnan(hour) || std::isnan(minute) || std::isnan(second) ||
        year < DATE_LUT_MIN_YEAR || month < 1 || month > 12 || day_of_month < 1 || day_of_month > 31 || hour < 0 || hour > 99 ||
        minute < 0 || minute > 99 || second < 0 || second > 99) [[unlikely]]
      return minDateTime(lut);

    if (year > DATE_LUT_MAX_YEAR) [[unlikely]]
      return maxDateTime(lut);

    return lut.makeDateTime(static_cast<Int16>(year), static_cast<UInt8>(month), static_cast<UInt8>(day_of_month), static_cast<UInt8>(hour),
                            static_cast<UInt8>(minute), static_cast<UInt8>(second));
  }

  static Int64 minDateTime(const DateLUTImpl &lut) { return lut.makeDateTime(DATE_LUT_MIN_YEAR - 1, 1, 1, 0, 0, 0); }

  static Int64 maxDateTime(const DateLUTImpl &lut) { return lut.makeDateTime(DATE_LUT_MAX_YEAR + 1, 1, 1, 23, 59, 59); }
  ...
  std::string extractTimezone(const ColumnWithTypeAndName &timezone_argument) const {
    if (!isStringOrFixedString(timezone_argument.type) || !timezone_argument.column ||
      ...
      throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT, "Argument 'timezone' for function {} must be const string", getName());
  ...
  UInt32 extractPrecision(const ColumnWithTypeAndName &precision_argument) const {
    if (!isNumber(precision_argument.type) || !precision_argument.column ||
      ...
      throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT, "Argument 'precision' for function {} must be constant number", getName());

    Int64 precision = precision_argument.column->getInt(0);

    if (precision < 0 || precision > 9)
      throw Exception(ErrorCodes::ARGUMENT_OUT_OF_BOUND, "Argument 'precision' for function {} must be in range [0, 9]", getName());
...
class FunctionMakeDateTime : public FunctionMakeDateTimeBase {
 ...
 public:
  ...
  DataTypePtr getReturnTypeImpl(const ColumnsWithTypeAndName &arguments) const override {
    ...
    std::string timezone;
    if (arguments.size() == mandatory_argument_names.size() + 1) timezone = extractTimezone(arguments.back());
 ...
 public:
  ...
  DataTypePtr getReturnTypeImpl(const ColumnsWithTypeAndName &arguments) const override {
    ...
    if (arguments.size() >= mandatory_argument_names.size() + 1) {
      ...
      if (!isNumber(fraction_argument.type))
        throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT, "Argument 'fraction' for function {} must be a number", getName());
    }
    ...
    /// Optional timezone argument
    std::string timezone;
    if (arguments.size() == mandatory_argument_names.size() + 3) timezone = extractTimezone(arguments.back());
  ...
  ColumnPtr executeImpl(const ColumnsWithTypeAndName &arguments, const DataTypePtr &, size_t input_rows_count) const override {
    ...
    for (size_t i = 0; i < input_rows_count; ++i) {
      ...
      result_data[i] = DecimalUtils::dateTimeFromComponents(date_time, static_cast<Int64>(fraction), static_cast<UInt32>(precision));
    }
...
class FunctionYYYYMMDDhhmmssToDateTime64 : public FunctionYYYYMMDDhhmmssToDateTimeBase {
 ...
 public:
  ...
  DataTypePtr getReturnTypeImpl(const ColumnsWithTypeAndName &arguments) const override {
    ...
    if (arguments.size() >= mandatory_argument_names.size() + 1) precision = extractPrecision(arguments[mandatory_argument_names.size()]);

    /// Optional timezone argument
    std::string timezone;
    if (arguments.size() == mandatory_argument_names.size() + 2) timezone = extractTimezone(arguments.back());
...
makeDate(year, day_of_year)
  ...
  FunctionDocumentation::Arguments arguments_makeDate = {{"year", "Year number.", {"(U)Int*", "Float*", "Decimal"}},
  ...
  FunctionDocumentation::ReturnedValue returned_value_makeDate = {"Returns a `Date` value constructed from the provided arguments",
                                                                  {"Date"}};
  FunctionDocumentation::Examples examples_makeDate = {{"Date from a year, month, day", R"(
...
└────────────┘
  ...
  FunctionDocumentation documentation_makeDate = {description_makeDate,    syntax_makeDate,   arguments_makeDate,     {},
                                                  returned_value_makeDate, examples_makeDate, introduced_in_makeDate, category_makeDate};
...
makeDate32(year, day_of_year)
  ...
  FunctionDocumentation::Arguments arguments_makeDate32 = {{"year", "Year number.", {"(U)Int*", "Float*", "Decimal"}},
                                                           {"month", "Month number (1-12).", {"(U)Int*", "Float*", "Decimal"}},
                                                           {"day", "Day of the month (1-31).", {"(U)Int*", "Float*", "Decimal"}},
                                                           {"day_of_year", "Day of the year (1-365).", {"(U)Int*", "Float*", "Decimal"}}};
  FunctionDocumentation::ReturnedValue returned_value_makeDate32 = {"Returns a `Date32` value constructed from the provided arguments",
                                                                    {"Date32"}};
  FunctionDocumentation::Examples examples_makeDate32 = {{"Date32 from a year, month, day", R"(
SELECT makeDate(2023, 2, 28) AS date;
        )",
                                                          R"(
┌───────date─┐
...
└────────────┘
  ...
  FunctionDocumentation documentation_makeDate32 = {
      description_makeDate32,    syntax_makeDate32,   arguments_makeDate32,     {},
      returned_value_makeDate32, examples_makeDate32, introduced_in_makeDate32, category_makeDate32};
...
makeDateTime(year, month, day, hour, minute, second[, timezone])
  ...
  FunctionDocumentation::Arguments arguments_makeDateTime = {{"year", "Year number.", {"(U)Int*", "Float*", "Decimal"}},
                                                             {"month", "Month number (1-12).", {"(U)Int*", "Float*", "Decimal"}},
                                                             {"day", "Day of the month (1-31).", {"(U)Int*", "Float*", "Decimal"}},
  ...
  FunctionDocumentation::ReturnedValue returned_value_makeDateTime = {"Returns a `DateTime` value constructed from the provided arguments",
                                                                      {"DateTime"}};
  FunctionDocumentation::Examples examples_makeDateTime = {{"DateTime from year, month, day, hour, minute, second", R"(
...
└─────────────────────┘
  ...
  FunctionDocumentation documentation_makeDateTime = {
      description_makeDateTime,    syntax_makeDateTime,   arguments_makeDateTime,     {},
      returned_value_makeDateTime, examples_makeDateTime, introduced_in_makeDateTime, category_makeDateTime};
...
makeDateTime64(year, month, day, hour, minute, second[, fraction[, precision[, timezone]]])
  ...
  FunctionDocumentation::Arguments arguments_makeDateTime64 = {
      ...
      {"timezone", "Timezone name.", {"String"}}};
  FunctionDocumentation::ReturnedValue returned_value_makeDateTime64 = {
      "Returns a `DateTime64` value constructed from the provided arguments", {"DateTime64"}};
  FunctionDocumentation::Examples examples_makeDateTime64 = {{"DateTime64 from year, month, day, hour, minute, second", R"(
...
└─────────────────────────────────────────────────┘
  ...
  FunctionDocumentation documentation_makeDateTime64 = {
      description_makeDateTime64,    syntax_makeDateTime64,   arguments_makeDateTime64,     {},
      returned_value_makeDateTime64, examples_makeDateTime64, introduced_in_makeDateTime64, category_makeDateTime64};
...
YYYYMMDDToDate(YYYYMMDD)
  ...
  FunctionDocumentation::Examples examples_yyyymmddtodate = {{"Example", R"(
...
└──────────────────────┘
  ...
  FunctionDocumentation documentation_yyyymmddtodate = {
      description_yyyymmddtodate,    syntax_yyyymmddtodate,   arguments_yyyymmddtodate,     {},
      returned_value_yyyymmddtodate, examples_yyyymmddtodate, introduced_in_yyyymmddtodate, category_yyyymmddtodate};
...
YYYYMMDDToDate32(YYYYMMDD)
  ...
  FunctionDocumentation::Arguments arguments_yyyymmddtodate32 = {
      {"YYYYMMDD", "Number containing the year, month and day.", {"(U)Int*", "Float*", "Decimal"}}};
  FunctionDocumentation::ReturnedValue returned_value_yyyymmddtodate32 = {"Returns a `Date32` value from the provided arguments",
                                                                          {"Date32"}};
  FunctionDocumentation::Examples examples_yyyymmddtodate32 = {{"Example", R"(
...
└────────────────────────────┘
  ...
  FunctionDocumentation documentation_yyyymmddtodate32 = {
      description_yyyymmddtodate32,    syntax_yyyymmddtodate32,   arguments_yyyymmddtodate32,     {},
      returned_value_yyyymmddtodate32, examples_yyyymmddtodate32, introduced_in_yyyymmddtodate32, category_yyyymmddtodate32};
...
YYYYMMDDhhmmssToDateTime(YYYYMMDDhhmmss[, timezone])
  ...
  FunctionDocumentation::Arguments arguments_yyyymmddhhmmsstodatetime = {
      ...
      {"timezone", "Timezone name.", {"String"}}};
  FunctionDocumentation::ReturnedValue returned_value_yyyymmddhhmmsstodatetime = {"Returns a `DateTime` value from the provided arguments",
                                                                                  {"DateTime"}};
  FunctionDocumentation::Examples examples_yyyymmddhhmmsstodatetime = {{"Example", R"(
...
└───────────────────────────────────────────────┘
  ...
  FunctionDocumentation documentation_yyyymmddhhmmsstodatetime = {
      description_yyyymmddhhmmsstodatetime,    syntax_yyyymmddhhmmsstodatetime,
      arguments_yyyymmddhhmmsstodatetime,      {},
...
YYYYMMDDhhmmssToDateTime64(YYYYMMDDhhmmss[, precision[, timezone]])
  ...
  FunctionDocumentation::Arguments arguments_yyyymmddhhmmsstodatetime64 = {
      ...
      {"precision", "Precision for the fractional part (0-9).", {"UInt8"}},
      {"timezone", "Timezone name.", {"String"}}};
  FunctionDocumentation::ReturnedValue returned_value_yyyymmddhhmmsstodatetime64 = {
      "Returns a `DateTime64` value from the provided arguments", {"DateTime64"}};
  FunctionDocumentation::Examples examples_yyyymmddhhmmsstodatetime64 = {{"Example", R"(
...
└──────────────────────────┘
  ...
  FunctionDocumentation documentation_yyyymmddhhmmsstodatetime64 = {
      description_yyyymmddhhmmsstodatetime64,    syntax_yyyymmddhhmmsstodatetime64,
      arguments_yyyymmddhhmmsstodatetime64,      {},
      returned_value_yyyymmddhhmmsstodatetime64, examples_yyyymmddhhmmsstodatetime64,
