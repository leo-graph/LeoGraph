...
using FunctionMultiMatchAny = FunctionsMultiStringSearch<
    MultiMatchAnyImpl<NameMultiMatchAny, /*ResultType*/ UInt8, MultiMatchTraits::Find::Any, /*WithEditDistance*/ false>>;
...
If you only want to search multiple substrings in a string, you can use function [`multiSearchAny`](#multiSearchAny) instead - it works much faster than this function.
  ...
  FunctionDocumentation::Examples examples = {{"Multiple pattern matching", "SELECT multiMatchAny('Hello World', ['Hello.*', 'foo.*'])",
                                               R"(
┌─multiMatchAny('Hello World', ['Hello.*', 'foo.*'])─┐
│                                                  1 │
└────────────────────────────────────────────────────┘
        )"},
                                              {"No patterns match", "SELECT multiMatchAny('Hello World', ['goodbye.*', 'foo.*'])",
                                               R"(
...
FunctionOverloadResolverPtr createInternalMultiMatchAnyOverloadResolver(bool allow_hyperscan, size_t max_hyperscan_regexp_length,
