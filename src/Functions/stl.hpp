...
inline bool est(const float* y, size_t n, size_t len, int ideg, float xs, float* ys, size_t nleft, size_t nright, float* w, bool userw,
                const float* rw) {
...
inline void ss(const float* y, size_t n, size_t np, size_t ns, int isdeg, size_t nsjump, bool userw, float* rw, float* season, float* work1,
               float* work2, float* work3, float* work4) {  /// NOLINT(readability-non-const-parameter)
...
inline void onestp(const float* y, size_t n, size_t np, size_t ns, size_t nt, size_t nl, int isdeg, int itdeg, int ildeg, size_t nsjump,
                   size_t ntjump, size_t nljump, size_t ni, bool userw, float* rw, float* season, float* trend, float* work1, float* work2,
                   float* work3, float* work4, float* work5) {
...
inline void stl(const float* y, size_t n, size_t np, size_t ns, size_t nt, size_t nl, int isdeg, int itdeg, int ildeg, size_t nsjump,
                size_t ntjump, size_t nljump, size_t ni, size_t no, float* rw, float* season, float* trend) {
  if (ns < 3) {
  ...
  while (true) {
    onestp(y, n, np, ns, nt, nl, isdeg, itdeg, ildeg, nsjump, ntjump, nljump, ni, userw, rw, season, trend, work1.data(), work2.data(),
           work3.data(), work4.data(), work5.data());
    k += 1;
    if (k > no) {
      break;
    }
...
class StlResult {
 public:
  ...
  float seasonal_strength() const { return strength(seasonal, remainder); }

  float trend_strength() const { return strength(trend, remainder); }
};
...
inline StlParams params() { return StlParams(); }
...
inline StlResult StlParams::fit(const float* y, size_t n, size_t np) {
  ...
  auto res = StlResult{std::vector<float>(n), std::vector<float>(n), std::vector<float>(), std::vector<float>(n)};
  ...
  stl(y, n, newnp, newns, nt, nl, isdeg, itdeg, ildeg, nsjump, ntjump, nljump, ni, no, res.weights.data(), res.seasonal.data(),
      res.trend.data());
...
inline StlResult StlParams::fit(const std::vector<float>& y, size_t np) { return StlParams::fit(y.data(), y.size(), np); }
