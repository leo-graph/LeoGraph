 ...
 public:
  ...
  ColumnPtr executeImpl(const ColumnsWithTypeAndName &arguments, const DataTypePtr &result_type, size_t input_rows_count) const override {
    ...
    if (col_str_const) {
      ...
      while (generator.get(token_begin, token_end)) dst.push_back(String(token_begin, token_end - token_begin));

      return result_type->createColumnConst(col_str_const->size(), dst);
    }
    throw Exception(ErrorCodes::ILLEGAL_COLUMN, "Illegal columns {}, {} of arguments of function {}", array_argument.column->getName(),
                    array_argument.column->getName(), getName());
  }
};

/// Helper functions for implementations
static inline std::optional<size_t> extractMaxSplits(const ColumnsWithTypeAndName &arguments, size_t max_substrings_argument_position) {
  if (max_substrings_argument_position >= arguments.size()) return std::nullopt;
...
static inline void checkArgumentsWithSeparatorAndOptionalMaxSubstrings(const IFunction &func, const ColumnsWithTypeAndName &arguments) {
