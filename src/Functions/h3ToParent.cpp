...
Returns the parent (coarser) [H3](#h3-index) index containing the given H3 index at the specified resolution.
  ...
  FunctionDocumentation::Arguments arguments = {{"index", "Child H3 index.", {"UInt64"}},
                                                {"resolution", "Resolution of the parent index with range `[0, 15]`.", {"UInt8"}}};
  FunctionDocumentation::ReturnedValue returned_value = {"Returns the parent H3 index at the specified resolution.", {"UInt64"}};
  FunctionDocumentation::Examples examples = {{"Get parent index at resolution 3", "SELECT h3ToParent(599405990164561919, 3) AS parent",
                                               R"(
┌─────────────parent─┐
│ 590398848891879423 │
└────────────────────┘
