...
ColumnPtr FunctionIsNull::getConstantResultForNonConstArguments(const ColumnsWithTypeAndName &arguments,
  ...
  if (elem.type->onlyNull()) return result_type->createColumnConst(1, UInt8(1));
...
ColumnPtr FunctionIsNull::executeImpl(const ColumnsWithTypeAndName &arguments, const DataTypePtr &, size_t) const {
  ...
  if (isVariant(elem.type) || isDynamic(elem.type)) {
    ...
    for (auto discr : discriminators) data.push_back(discr == ColumnVariant::NULL_DISCRIMINATOR);
    return res;
  }
  ...
  if (elem.type->isLowCardinalityNullable()) {
    ...
    for (size_t i = 0; i != low_cardinality_column.size(); ++i) data.push_back(low_cardinality_column.getIndexAt(i) == null_index);
    return res;
  }
...
llvm::Value *FunctionIsNull::compileImpl(llvm::IRBuilderBase &builder, const ValuesWithType &arguments,
                                         const DataTypePtr & /*result_type*/) const {
