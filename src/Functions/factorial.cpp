...
struct FactorialImpl {
  ...
  static NO_SANITIZE_UNDEFINED ResultType apply(A a) {
    ...
    if constexpr (is_integer<A>) {
      if (a > 20) throw Exception(ErrorCodes::BAD_ARGUMENTS, "The maximum value for the input argument of function factorial is 20");
 ...
 private:
  static const constexpr ResultType factorials[21] = {1,
                                                      1,
                                                      2,
                                                      6,
                                                      24,
...
struct FunctionUnaryArithmeticMonotonicity<NameFactorial> {
  ...
  static IFunction::Monotonicity get(const IDataType &, const Field &left, const Field &right) {
    ...
    if (!left.isNull() && !right.isNull()) {
      ...
      if (1 <= left_value && left_value <= right_value && right_value <= 20) is_strict = true;
    }
...
The maximum positive value for the input argument is `20`, a value of `21` or greater will cause an exception.
  ...
  FunctionDocumentation::Examples examples = {{"Usage example", "factorial(10)", "3628800"}};
