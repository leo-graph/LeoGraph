...
class FunctionMidpoint : public IFunction {
 ...
 private:
  ...
  struct WiderIntImpl<T> {
    ...
    using type = std::conditional_t<(sizeof(T) <= 8), std::conditional_t<is_signed_v, Int128, UInt128>,
  ...
  template <typename ResultColumnType>
  static void calculateForNumericType(auto &result_data, const Columns &input_columns,
                                      const std::vector<const ColumnUInt8::Container *> &input_null_maps,
                                      ColumnUInt8::Container &result_null_map_data, size_t input_rows_count) {
    using ResultType = typename ResultColumnType::ValueType;
    ...
    for (size_t row = 0; row < input_rows_count; ++row) {
      ...
      for (size_t i = 0; i < input_columns.size(); ++i) {
        ...
        if ((*input_null_maps[i])[row]) continue;
  ...
  template <typename ResultColumnType>
  static void calculateForDecimalType(auto &result_data, const Columns &input_columns,
                                      const std::vector<const ColumnUInt8::Container *> &input_null_maps,
                                      ColumnUInt8::Container &result_null_map_data, size_t input_rows_count) {
    using ResultType = typename ResultColumnType::ValueType;
    using Native = typename ResultType::NativeType;
  ...
  ColumnPtr executeImpl(const ColumnsWithTypeAndName &arguments, const DataTypePtr &result_type, size_t input_rows_count) const override {
    ...
    for (const auto &argument : arguments) {
      if (argument.type->onlyNull()) continue;
    ...
    if (converted_columns.empty()) return arguments[0].column->cloneResized(input_rows_count);

    if (converted_columns.size() == 1) return converted_columns[0];
    ...
    if (isDecimal(nested_result_type) || isDateTime64(nested_result_type) || isTime64(nested_result_type)) {
      using DecimalAndTemporalTypes =
          TypeList<DataTypeDecimal32, DataTypeDecimal64, DataTypeDecimal128, DataTypeDecimal256, DataTypeDateTime64,
                   DataTypeTime64>;  /// these temporal types behave like decimals

      castTypeToEither(DecimalAndTemporalTypes{}, nested_result_type.get(), [&](const auto &type) -> bool {
        ...
        if (result_is_nullable)
          calculateForDecimalType<ResultColumn>(result_column_data, columns, input_null_maps, *result_null_map_data, input_rows_count);
        else
          calculateForDecimalType<ResultColumn>(result_column_data, columns, input_rows_count);

        return true;
      });
    } else if (isNumber(nested_result_type) || isDate(nested_result_type) || isDateTime(nested_result_type) || isTime(nested_result_type) ||
               isDate32(nested_result_type)) {
      using NumericAndTemporalTypes =
          TypeList<DataTypeUInt8, DataTypeUInt16, DataTypeUInt32, DataTypeUInt64, DataTypeUInt128, DataTypeUInt256, DataTypeInt8,
                   DataTypeInt16, DataTypeInt32, DataTypeInt64, DataTypeInt128, DataTypeInt256, DataTypeBFloat16, DataTypeFloat32,
                   DataTypeFloat64, DataTypeDate, DataTypeDateTime, DataTypeTime,
                   DataTypeDate32>;  /// these temporal types behave like numeric

      castTypeToEither(NumericAndTemporalTypes{}, nested_result_type.get(), [&](const auto &type) -> bool {
        using DataType = std::decay_t<decltype(type)>;
        using FieldType = typename DataType::FieldType;
        ...
        if (result_is_nullable)
          calculateForNumericType<ResultColumn>(result_column_data, columns, input_null_maps, *result_null_map_data, input_rows_count);
        else
      ...
      throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT, "Function {} supports only numeric/temporal types, got {}", getName(),
                      result_type->getName());
    }

    if (result_is_nullable) return ColumnNullable::create(std::move(nested_result_column), std::move(result_null_map));
...
class MidpointResolver : public IFunctionOverloadResolver {
 public:
  static constexpr auto name = "midpoint";
  static FunctionOverloadResolverPtr create(ContextPtr context) { return std::make_unique<MidpointResolver<SpecializedFunction>>(context); }

  explicit MidpointResolver(ContextPtr context_) : context(context_) {}
