...
class FunctionCompareSubstrings : public IFunction {
 public:
  ...
  ColumnPtr executeImpl(const ColumnsWithTypeAndName &arguments, const DataTypePtr & /*result_type*/,
    ...
    if (input_rows_count == 0) return col_result;

    auto try_cast_column = [](const ColumnPtr &column) -> std::tuple<const ColumnString *, const ColumnFixedString *, const ColumnConst *> {
      return {checkAndGetColumn<ColumnString>(column.get()), checkAndGetColumn<ColumnFixedString>(column.get()),
    ...
    if (col_str1 && col_str2)
      ...
      executeFixedStringConst<false>(*fixed_str1_col, col_str2_const->getDataAt(0), str1_offset, str2_offset, num_bytes, input_rows_count,
                                     result);
    else if (col_str1_const && fixed_str2_col)
      executeFixedStringConst<true>(*fixed_str2_col, col_str1_const->getDataAt(0), str2_offset, str1_offset, num_bytes, input_rows_count,
                                    result);
    else
 ...
 private:
  ...
  /// If offset is beyond end of input string(s), compute result and return true, else return false.
  template <bool reverse>
  static bool isOverflowComparison(size_t str1_length, size_t str1_offset, size_t str2_length, size_t str2_offset, size_t input_rows_count,
    ...
    for (size_t i = 0; i < input_rows_count; ++i) result[i] = res;
    return is_overflow;
  }

  void executeStringString(const ColumnString &col_str1, const ColumnString &col_str2, size_t str1_offset, size_t str2_offset,
                           size_t num_bytes, size_t input_rows_count, PaddedPODArray<Int8> &result) const {
  ...
  template <bool reverse>
  void executeStringConst(const ColumnString &col_str1, std::string_view str2, size_t str1_offset, size_t str2_offset, size_t num_bytes,
                          size_t input_rows_count, PaddedPODArray<Int8> &result) const {
    const auto &str1_data = col_str1.getChars();
    const auto &str1_offsets = col_str1.getOffsets();
  ...
  void executeFixedStringFixedString(const ColumnFixedString &col_str1, const ColumnFixedString &col_str2, size_t str1_offset,
                                     size_t str2_offset, size_t num_bytes, size_t input_rows_count, PaddedPODArray<Int8> &result) const {
    size_t str1_length = col_str1.getN();
    size_t str2_length = col_str2.getN();

    if (isOverflowComparison<false>(str1_length, str1_offset, str2_length, str2_offset, input_rows_count, result)) return;
  ...
  template <bool reverse>
  void executeFixedStringString(const ColumnFixedString &col_str1, const ColumnString &col_str2, size_t str1_offset, size_t str2_offset,
                                size_t num_bytes, size_t input_rows_count, PaddedPODArray<Int8> &result) const {
  ...
  template <bool reverse>
  void executeFixedStringConst(const ColumnFixedString &col_str1, std::string_view str2, size_t str1_offset, size_t str2_offset,
                               size_t num_bytes, size_t input_rows_count, PaddedPODArray<Int8> &result) const {
    size_t str1_length = col_str1.getN();
    if (isOverflowComparison<reverse>(str1_length, str1_offset, str2.size(), str2_offset, input_rows_count, result)) return;
...
Compares two strings lexicographically.
  ...
  FunctionDocumentation::Arguments arguments = {
      ...
      {"num_bytes",
       "The maximum number of bytes to compare in both strings. If `s1_offset` (or `s2_offset`) + `num_bytes` exceeds the end of an input "
       "string, `num_bytes` will be reduced accordingly.",
       {"UInt*"}}};
...
- `1` if `s1`[`s1_offset` : `s1_offset` + `num_bytes`] > `s2`[`s2_offset` : `s2_offset` + `num_bytes`].
                                                         ...
                                                         {"Int8"}};
  FunctionDocumentation::Examples examples = {{"Usage example", "SELECT compareSubstrings('Saxony', 'Anglo-Saxon', 0, 6, 5) AS result",
                                               R"(
┌─result─┐
│      0 │
└────────┘
