...
class FunctionGlobalVariable : public IFunction {
 public:
  ...
  static FunctionPtr create(ContextPtr) { return std::make_shared<FunctionGlobalVariable>(); }
  ...
  DataTypePtr getReturnTypeImpl(const ColumnsWithTypeAndName &arguments) const override {
    ...
    if (variable == global_variable_map.end()) return std::make_shared<DataTypeInt32>();
  ...
  ColumnPtr executeImpl(const ColumnsWithTypeAndName &arguments, const DataTypePtr &result_type, size_t input_rows_count) const override {
    ...
    if (variable != global_variable_map.end()) return result_type->createColumnConst(input_rows_count, variable->second.value);
  ...
  std::unordered_map<String, TypeAndValue> global_variable_map = {
      ...
      {"sql_mode",
       {std::make_shared<DataTypeString>(),
        "ALLOW_INVALID_DATES,ANSI_QUOTES,IGNORE_SPACE,NO_AUTO_VALUE_ON_ZERO,NO_DIR_IN_CREATE,ONLY_FULL_GROUP_BY,PAD_CHAR_TO_FULL_LENGTH,"
...
Takes a constant string argument and returns the value of the global variable with that name. This function is intended for compatibility with MySQL and not needed or useful for normal operation of ClickHouse. Only few dummy global variables are defined.
  ...
  FunctionDocumentation::Arguments arguments = {{"name", "Global variable name.", {"String"}}};
  FunctionDocumentation::ReturnedValue returned_value = {"Returns the value of variable `name`.", {"Any"}};
  FunctionDocumentation::Examples examples = {{"globalVariable", "SELECT globalVariable('max_allowed_packet')", "67108864"}};
