...
struct FunctionBitTestMany : public IFunction {
  ...
  DataTypePtr getReturnTypeImpl(const DataTypes& arguments) const override {
    if (arguments.size() < 2)
      throw Exception(ErrorCodes::TOO_FEW_ARGUMENTS_FOR_FUNCTION,
                      ...
                      "passed {}, should be at least 2.",
                      getName(), arguments.size());

    const auto& first_arg = arguments.front();

    if (!isInteger(first_arg))
      throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT, "Illegal type {} of first argument of function {}", first_arg->getName(),
                      getName());
    ...
    for (size_t i = 1; i < arguments.size(); ++i) {
      ...
      if (!isUInt(pos_arg))
        throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT, "Illegal type {} of {} argument of function {}", pos_arg->getName(), i,
                        getName());
    }
 ...
 private:
  template <typename T>
  ColumnPtr execute(const ColumnsWithTypeAndName& arguments, const DataTypePtr& result_type, const IColumn* const value_col_untyped,
    ...
    if (const auto value_col = checkAndGetColumn<ColumnVector<T>>(value_col_untyped)) {
      ...
      if (is_const) {
        for (size_t i = 0; i < input_rows_count; ++i) out[i] = Impl::apply(val[i], const_mask);
      } else {
        const auto mask = createMask<T>(input_rows_count, arguments);

        for (size_t i = 0; i < input_rows_count; ++i) out[i] = Impl::apply(val[i], mask[i]);
      }
    ...
    if (const auto value_col_const = checkAndGetColumnConst<ColumnVector<T>>(value_col_untyped)) {
      ...
      for (size_t i = 0; i < input_rows_count; ++i) out[i] = Impl::apply(val, mask[i]);
    ...
    for (size_t i = 1; i < arguments.size(); ++i) {
      if (auto pos_col_const = checkAndGetColumnConst<ColumnVector<ValueType>>(arguments[i].column.get())) {
        ...
        if (pos < 8 * sizeof(ValueType))
          ...
          throw Exception(ErrorCodes::PARAMETER_OUT_OF_BOUND, "The bit position argument {} is out of bounds for number",
  ...
  PaddedPODArray<ValueType> createMask(const size_t size, const ColumnsWithTypeAndName& arguments) const {
    ...
    for (size_t i = 1; i < arguments.size(); ++i) {
      ...
      if (!addToMaskImpl<UInt8>(mask, pos_col) && !addToMaskImpl<UInt16>(mask, pos_col) && !addToMaskImpl<UInt32>(mask, pos_col) &&
  ...
  bool NO_SANITIZE_UNDEFINED addToMaskImpl(PaddedPODArray<ValueType>& mask, const IColumn* const pos_col_untyped) const {
    ...
    if (const auto pos_col_const = checkAndGetColumnConst<ColumnVector<PosType>>(pos_col_untyped)) {
      ...
      for (size_t i = 0; i < mask.size(); ++i) mask[i] = static_cast<ValueType>(mask[i] | new_mask);
