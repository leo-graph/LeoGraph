...
const String& getFunctionCanonicalNameIfAny(const String& name) { return FunctionFactory::instance().getCanonicalNameIfAny(name); }

void FunctionFactory::registerFunction(const std::string& name, FunctionCreator creator, FunctionDocumentation documentation,
...
void FunctionFactory::registerFunction(const std::string& name, FunctionSimpleCreator creator, FunctionDocumentation documentation,
                                       Case case_sensitiveness) {
  registerFunction(
      name,
      [my_creator = std::move(creator)](ContextPtr context) {
...
FunctionOverloadResolverPtr FunctionFactory::getImpl(const std::string& name, ContextPtr context) const {
...
std::vector<std::string> FunctionFactory::getAllNames() const {
  ...
  for (const auto& func : functions) res.emplace_back(func.first);
  return res;
}

FunctionOverloadResolverPtr FunctionFactory::get(const std::string& name, ContextPtr context) const { return getImpl(name, context); }

bool FunctionFactory::has(const std::string& name) const {
  String canonical_name = getAliasToOrName(name);
  if (functions.contains(canonical_name)) return true;
  canonical_name = Poco::toLower(canonical_name);
  return case_insensitive_functions.contains(canonical_name);
}

FunctionOverloadResolverPtr FunctionFactory::tryGetImpl(const std::string& name_param, ContextPtr context) const {
  ...
  if (functions.end() != it)
    ...
    if (case_insensitive_functions.end() != it) res = it->second.first(context);
  }
  ...
  if (CurrentThread::isInitialized()) {
    ...
    if (query_context && Poco::toLower(name) == "totime" && query_context->getSettingsRef()[Setting::use_legacy_to_time]) {
      ...
      if (functions.end() != it) res = it->second.first(context);
    }
  }
...
FunctionOverloadResolverPtr FunctionFactory::tryGet(const std::string& name, ContextPtr context) const {
...
FunctionDocumentation FunctionFactory::getDocumentation(const std::string& name) const {
  auto it = functions.find(name);
  if (it == functions.end()) throw Exception(ErrorCodes::UNKNOWN_FUNCTION, "Unknown function {}", name);
