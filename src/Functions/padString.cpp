  ...
  void init() {
    ...
    if constexpr (is_utf8) {
      ...
      while (true) {
        ...
        if (offset == pad_string.length()) break;
    ...
    while (numCharsInPadString() < 16) {
      ...
      if constexpr (is_utf8) {
        ...
        for (size_t i = 1; i != old_size; ++i) utf8_offsets.push_back(utf8_offsets[i] + base);
      }
    }
  }
...
class FunctionPadString : public IFunction {
 public:
  ...
  DataTypePtr getReturnTypeImpl(const DataTypes &arguments) const override {
    ...
    if (number_of_arguments != 2 && number_of_arguments != 3)
      throw Exception(ErrorCodes::NUMBER_OF_ARGUMENTS_DOESNT_MATCH,
                      "Number of arguments for function {} doesn't match: passed {}, should be 2 or 3", getName(), number_of_arguments);

    if (!isStringOrFixedString(arguments[0]))
      throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT, "Illegal type {} of the first argument of function {}, should be string",
                      arguments[0]->getName(), getName());

    if (!isInteger(arguments[1]))
      throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT,
                      "Illegal type {} of the second argument of function {}, should be unsigned integer", arguments[1]->getName(),
                      getName());

    if (number_of_arguments == 3 && !isStringOrFixedString(arguments[2]))
      throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT, "Illegal type {} of the third argument of function {}, should be const string",
  ...
  ColumnPtr executeImpl(const ColumnsWithTypeAndName &arguments, const DataTypePtr &, size_t input_rows_count) const override {
    ...
    if (arguments.size() == 3) {
      ...
      if (!column_pad_const)
        throw Exception(ErrorCodes::ILLEGAL_COLUMN, "Illegal column {}, third argument of function {} must be a constant string",
                        column_pad->getName(), getName());
    ...
    if (const ColumnString *col = checkAndGetColumn<ColumnString>(column_string.get()))
      ...
      throw Exception(ErrorCodes::ILLEGAL_COLUMN, "Illegal column {}, first argument of function {} must be a string",
 ...
 private:
  ...
  void executeForSource(SourceStrings &&strings, const ColumnPtr &column_length, const String &pad_string, StringSink &res_sink) const {
    ...
    bool all_ascii =
        isAllASCII(reinterpret_cast<const UInt8 *>(pad_string.data()), pad_string.size()) && isAllASCII(chars.data(), chars.size());
    ...
    if (!is_actually_utf8) {
      ...
      if (const auto *col_const = checkAndGetColumn<ColumnConst>(column_length.get()))
        executeForSourceAndLength<false>(std::forward<SourceStrings>(strings), ConstSource<GenericValueSource>{*col_const}, padding_chars,
                                         res_sink);
      else
      ...
      if (const auto *col_const = checkAndGetColumn<ColumnConst>(column_length.get()))
        executeForSourceAndLength<true>(std::forward<SourceStrings>(strings), ConstSource<GenericValueSource>{*col_const}, padding_chars,
                                        res_sink);
      else
  ...
  static void executePad(SourceStrings &&strings, SourceLengths &&lengths, const PaddingChars<is_actually_utf8> &padding_chars,
                         StringSink &res_sink) {
    ...
    for (; !res_sink.isEnd(); res_sink.next(), strings.next(), lengths.next()) {
      ...
      if (!res_sink.rowNum() || !is_const_new_length) {
        ...
        if (new_length < 0) {
          throw Exception(ErrorCodes::INDEX_OF_POSITIONAL_ARGUMENT_IS_OUT_OF_RANGE, "New padded length ({}) is negative",
      ...
      if (new_length == current_length) {
        ...
        if constexpr (!is_right_pad_) padding_chars.appendTo(res_sink, new_length - current_length);
  ...
  void executeForSourceAndLength(SourceStrings &&strings, SourceLengths &&lengths, const PaddingChars<is_actually_utf8> &padding_chars,
...
Pads a string from the left with spaces or with a specified string (multiple times, if needed) until the resulting string reaches the specified `length`.
  ...
  FunctionDocumentation::Arguments arguments_left = {
      ...
      {"length",
       "The length of the resulting string. If the value is smaller than the input string length, then the input string is shortened to "
       "`length` characters.",
       {"(U)Int*"}},
      {"pad_string",
  ...
  FunctionDocumentation::Examples examples_left = {{"Usage example", "SELECT leftPad('abc', 7, '*'), leftPad('def', 7)",
                                                    R"(
...
└────────────────────────┴───────────────────┘
  ...
  FunctionDocumentation documentation_left = {description_left,    syntax_left,   arguments_left, {},
                                              returned_value_left, examples_left, introduced_in,  category};
...
Unlike [`leftPad`](#leftPad) which measures the string length in bytes, the string length is measured in code points.
  ...
  FunctionDocumentation::Examples examples_left_utf8 = {{"Usage example", "SELECT leftPadUTF8('абвг', 7, '*'), leftPadUTF8('дежз', 7)",
                                                         R"(
┌─leftPadUTF8('абвг', 7, '*')─┬─leftPadUTF8('дежз', 7)─┐
│ ***абвг                     │    дежз                │
└─────────────────────────────┴────────────────────────┘
        )"}};
  FunctionDocumentation documentation_left_utf8 = {description_left_utf8, syntax_left_utf8,   arguments_left, {},
                                                   returned_value_left,   examples_left_utf8, introduced_in,  category};
...
Pads a string from the right with spaces or with a specified string (multiple times, if needed) until the resulting string reaches the specified `length`.
  ...
  FunctionDocumentation::Examples examples_right = {{"Usage example", "SELECT rightPad('abc', 7, '*'), rightPad('abc', 7)",
                                                     R"(
┌─rightPad('abc', 7, '*')─┬─rightPad('abc', 7)─┐
│ abc****                 │ abc                │
└─────────────────────────┴────────────────────┘
        )"}};
  FunctionDocumentation documentation_right = {description_right,    syntax_right,   arguments_left, {},
                                               returned_value_right, examples_right, introduced_in,  category};
...
Unlike [`rightPad`](#rightPad) which measures the string length in bytes, the string length is measured in code points.
  ...
  FunctionDocumentation::Examples examples_right_utf8 = {{"Usage example", "SELECT rightPadUTF8('абвг', 7, '*'), rightPadUTF8('абвг', 7)",
                                                          R"(
┌─rightPadUTF8('абвг', 7, '*')─┬─rightPadUTF8('абвг', 7)─┐
│ абвг***                      │ абвг                    │
└──────────────────────────────┴─────────────────────────┘
        )"}};
  FunctionDocumentation documentation_right_utf8 = {description_right_utf8,    syntax_right_utf8,   arguments_left, {},
                                                    returned_value_right_utf8, examples_right_utf8, introduced_in,  category};

  factory.registerFunction("leftPad", [](ContextPtr) { return FunctionPadString::create("leftPad", false, false); }, documentation_left);
  factory.registerFunction(
      "leftPadUTF8", [](ContextPtr) { return FunctionPadString::create("leftPadUTF8", false, true); }, documentation_left_utf8);
  factory.registerFunction("rightPad", [](ContextPtr) { return FunctionPadString::create("rightPad", true, false); }, documentation_right);
  factory.registerFunction(
      "rightPadUTF8", [](ContextPtr) { return FunctionPadString::create("rightPadUTF8", true, true); }, documentation_right_utf8);
