...
class FunctionVariantType : public IFunction {
 public:
  ...
  DataTypePtr getReturnTypeImpl(const ColumnsWithTypeAndName &arguments) const override {
    FunctionArgumentDescriptors mandatory_args{{"variant", isVariant, nullptr, "Variant"}};
    ...
    for (ColumnVariant::Discriminator i = 0; i != variants.size(); ++i) enum_values.emplace_back(variants[i]->getName(), i);
  ...
  ColumnPtr executeImpl(const ColumnsWithTypeAndName &arguments, const DataTypePtr &result_type, size_t input_rows_count) const override {
    ...
    if (!variant_column)
      throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT, "First argument for function {} must be Variant, got {} instead", getName(),
                      arguments[0].type->getName());

    auto res = result_type->createColumn();
    auto &res_data = typeid_cast<ColumnInt8 *>(res.get())->getData();
    res_data.reserve(input_rows_count);
    for (size_t i = 0; i != input_rows_count; ++i) res_data.push_back(variant_column->globalDiscriminatorAt(i));
...
Returns the variant type name for each row of `Variant` column. If row contains NULL, it returns 'None' for it.
  ...
  FunctionDocumentation::Arguments arguments = {{"variant", "Variant column.", {"Variant"}}};
