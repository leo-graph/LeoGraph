...
struct BitShiftRightImpl {
  ...
  static NO_SANITIZE_UNDEFINED void bitShiftRightForBytes(const UInt8 *op_pointer, const UInt8 *begin, UInt8 *out,
                                                          const size_t shift_right_bits) {
  ...
  static llvm::Value *compile(llvm::IRBuilder<> &b, llvm::Value *left, llvm::Value *right, bool is_signed) {
    if (!left->getType()->isIntegerTy()) throw Exception(ErrorCodes::LOGICAL_ERROR, "BitShiftRightImpl expected an integral type");
...
struct NameBitShiftRight {
  static constexpr auto name = "bitShiftRight";
};
