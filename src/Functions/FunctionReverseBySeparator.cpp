...
class TokenSplitter {
 public:
  ...
  std::string_view nextToken() {
    if (current_pos > haystack.size()) return {};
...
DataTypePtr FunctionReverseBySeparator::getReturnTypeImpl(const ColumnsWithTypeAndName& arguments) const {
  ...
  if (number_of_arguments < 1 || number_of_arguments > 2)
    throw Exception(ErrorCodes::BAD_ARGUMENTS, "Number of arguments for function {} doesn't match: passed {}, should be 1 or 2", getName(),
                    number_of_arguments);

  if (!isString(arguments[0].type))
    throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT, "First argument for function {} must be String, got {}", getName(),
                    arguments[0].type->getName());

  if (number_of_arguments == 2) {
    if (!isString(arguments[1].type))
      throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT, "Second argument for function {} must be String, got {}", getName(),
...
ColumnPtr FunctionReverseBySeparator::executeImpl(const ColumnsWithTypeAndName& arguments, const DataTypePtr& /*result_type*/,
                                                  size_t input_rows_count) const {
  const ColumnPtr& column_haystack = arguments[0].column;
  const ColumnString* col_haystack = checkAndGetColumn<ColumnString>(column_haystack.get());
  if (!col_haystack) throw Exception(ErrorCodes::ILLEGAL_COLUMN, "First argument for function {} must be String", getName());
  ...
  for (size_t i = 0; i < input_rows_count; ++i) {
    ...
    if (separator.empty()) {
      for (size_t j = 0; j < haystack.size(); ++j) res_data.push_back(haystack[haystack.size() - 1 - j]);
      res_offsets[i] = res_data.size();
      continue;
    }
    ...
    if (separator.size() == 1) {
      ...
      while (pos < haystack.size()) {
        ...
        /// Find next separator
        size_t next_pos = pos;
        while (next_pos < haystack.size() && haystack[next_pos] != sep_char) ++next_pos;
      ...
      if (pos == haystack.size()) token_starts.push_back(pos);

      /// Write tokens in reverse order
      for (int64_t j = static_cast<int64_t>(token_starts.size()) - 1; j >= 0; --j) {
        size_t start = token_starts[j];
        size_t end = (j + 1 < static_cast<int64_t>(token_starts.size())) ? token_starts[j + 1] - 1 : haystack.size();

        /// Add separator before token (except for the first token we write)
        if (j < static_cast<int64_t>(token_starts.size()) - 1) res_data.push_back(sep_char);

        /// Add the token content
        if (end > start) res_data.insert(res_data.end(), haystack.data() + start, haystack.data() + end);
      }
    } else {
      ...
      for (size_t j = 0; j < tokens.size(); ++j) {
        if (j > 0) res_data.insert(res_data.end(), separator.data(), separator.data() + separator.size());

        const std::string_view& token = tokens[tokens.size() - 1 - j];
        if (!token.empty()) res_data.insert(res_data.end(), token.data(), token.data() + token.size());
      }
    }
...
REGISTER_FUNCTION(ReverseBySeparator) {
  ...
  FunctionDocumentation::Examples examples = {{"Basic domain reversal", "SELECT reverseBySeparator('www.google.com')", "'com.google.www'"},
...
)", "reverseBySeparator(string[, separator])", arguments, parameters, returned_value, examples, introduced_in, category};
