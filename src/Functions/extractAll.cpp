...
class ExtractAllImpl {
 ...
 public:
  ...
  bool get(Pos &token_begin, Pos &token_end) {
    ...
    if (!re->match(pos, end - pos, matches) || !matches[0].length) return false;
...
If the regular expression has capturing groups (sub-patterns), the function matches the input string against the first capturing group.
  ...
  FunctionDocumentation::Examples examples = {
      {"Extract all numbers", "SELECT extractAll('hello 123 world 456', '[0-9]+')",
       R"(
┌─extractAll('hello 123 world 456', '[0-9]+')─┐
│ ['123','456']                               │
└─────────────────────────────────────────────┘
        )"},
      {"Extract using capturing group", "SELECT extractAll('test@example.com, user@domain.org', '([a-zA-Z0-9]+)@')",
       R"(
