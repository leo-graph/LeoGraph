...
std::string extractTimeZoneNameFromColumn(const IColumn* column, const String& column_name) {
  ...
  if (!time_zone_column)
    throw Exception(ErrorCodes::ILLEGAL_COLUMN, "Illegal column {} of time zone argument of function, must be a constant string",
                    column_name);
...
std::string extractTimeZoneNameFromFunctionArguments(const ColumnsWithTypeAndName& arguments, size_t time_zone_arg_num,
  ...
  if ((arguments.size() == time_zone_arg_num + 1) && (!allow_nonconst_timezone_arguments || arguments[time_zone_arg_num].column)) {
    return extractTimeZoneNameFromColumn(arguments[time_zone_arg_num].column.get(), arguments[time_zone_arg_num].name);
  }

  if (arguments.size() <= datetime_arg_num) return {};
...
const DateLUTImpl& extractTimeZoneFromFunctionArguments(const ColumnsWithTypeAndName& arguments, size_t time_zone_arg_num,
  ...
  if (arguments.size() <= datetime_arg_num) return DateLUT::instance();

  const auto& dt_arg = arguments[datetime_arg_num].type.get();
  /// If time zone is attached to an argument of type DateTime.
  if (const auto* type = checkAndGetDataType<DataTypeDateTime>(dt_arg)) return type->getTimeZone();
  if (const auto* type = checkAndGetDataType<DataTypeDateTime64>(dt_arg)) return type->getTimeZone();
