...
std::pair<const TColumn *, bool> getArgumentOrConstArgument(const ColumnsWithTypeAndName &arguments, size_t index,
  ...
  if (!column) {
    if (!column_const) {
      throw Exception(ErrorCodes::ILLEGAL_COLUMN, "Illegal column type {} of argument {} of function {}. Must be {}",
  ...
  ColumnPtr executeImpl(const ColumnsWithTypeAndName &arguments, const DataTypePtr &, size_t input_rows_count) const override {
    ...
    callOnGeometryDataType<SphericalPoint>(arguments[0].type, [&](const auto &type) {
      ...
      auto get_multi_polygon = [&]<typename T>(T &&geometry) -> SphericalMultiPolygon {
        ...
        if constexpr (std::is_same_v<ColumnToMultiPolygonsConverter<SphericalPoint>, Converter>) return std::forward<T>(geometry);
      ...
      if (is_const_resolution) resolution = get_resolution(0);

      if (is_const_array) multi_polygon = get_multi_polygon(std::move(geometries[0]));

      for (size_t row = 0; row < input_rows_count; ++row) {
        if (!is_const_resolution) resolution = get_resolution(row);

        if (!is_const_array) multi_polygon = get_multi_polygon(std::move(geometries[row]));

        for (auto &polygon : multi_polygon) {
          std::vector<LatLng> exterior;
          exterior.reserve(polygon.outer().size());
          for (auto &point : polygon.outer()) exterior.push_back(toH3LatLng(toRadianPoint(point)));
          ...
          for (auto &inner : polygon.inners()) {
            ...
            for (auto &point : inner) hole.push_back(toH3LatLng(toRadianPoint(point)));
          ...
          if (vec_size > MAX_ARRAY_SIZE)
            throw Exception(ErrorCodes::TOO_LARGE_ARRAY_SIZE,
                            "The result of function {} (array of {} elements) will be too large with resolution argument = {}", getName(),
                            vec_size, toString(resolution));
 ...
 private:
  class GeoPolygonContainer {
   ...
   public:
    ...
    explicit GeoPolygonContainer(std::vector<LatLng> &&mainLoop, std::vector<std::vector<LatLng>> &&holes = {})
    ...
    const GeoPolygon *unwrap() const {
      // Prepare main loop
      mutableMainLoop = {static_cast<int>(mainLoopVerts.size()), const_cast<LatLng *>(mainLoopVerts.data())};

      // Prepare holes
      mutableHoles.clear();
      mutableHoles.reserve(holeVerts.size());
      for (const auto &hole : holeVerts) {
        mutableHoles.push_back({static_cast<int>(hole.size()), const_cast<LatLng *>(hole.data())});
      }

      // Prepare full polygon
      mutablePolygon = {mutableMainLoop, static_cast<int>(mutableHoles.size()), mutableHoles.data()};
