 ...
 public:
  ...
  Monotonicity getMonotonicityForRange(const IDataType &type, const Field &left, const Field &right) const override {
    if constexpr (std::is_same_v<typename Transform::FactorTransform, ZeroTransform>)
      ...
      if (const auto *timezone = dynamic_cast<const TimezoneMixin *>(&type)) date_lut = &timezone->getTimeZone();
      ...
      if (checkAndGetDataType<DataTypeTime64>(type_ptr)) {
        ...
        return transformer_left.execute(left_time.getValue(), *date_lut) == transformer_right.execute(right_time.getValue(), *date_lut)
                   ? is_monotonic
                   : is_not_monotonic;
      }
