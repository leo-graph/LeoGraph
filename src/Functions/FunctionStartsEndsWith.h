...
class FunctionStartsEndsWith : public IFunction {
 public:
  ...
  DataTypePtr getReturnTypeImpl(const DataTypes &arguments) const override {
    ...
    if (isArray(arguments[0]) && isArray(arguments[1])) return std::make_shared<DataTypeUInt8>();

    throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT, "Illegal types {} {} of arguments of function {}. Both must be String or Array",
  ...
  ColumnPtr executeImpl(const ColumnsWithTypeAndName &arguments, const DataTypePtr &, size_t input_rows_count) const override {
    ...
    if (!is_utf8 && isStringOrFixedString(*data_type)) return executeImplString(arguments, {}, input_rows_count);
    if (is_utf8 && isString(*data_type)) return executeImplStringUTF8(arguments, {}, input_rows_count);
 ...
 private:
  ColumnPtr executeImplArray(const ColumnsWithTypeAndName &arguments, const DataTypePtr &, size_t input_rows_count) const {
    ...
    for (size_t i = 0; i < 2; ++i) preprocessed_columns[i] = castColumn(arguments[i], common_type);
  ...
  void dispatch(HaystackSource haystack_source, const IColumn *needle_column, PaddedPODArray<UInt8> &res_data) const {
    if (const ColumnString *needle = checkAndGetColumn<ColumnString>(needle_column))
      ...
      execute<HaystackSource, ConstSource<FixedStringSource>>(haystack_source, ConstSource<FixedStringSource>(*needle_const_fixed),
                                                              res_data);
    else
      throw Exception(ErrorCodes::ILLEGAL_COLUMN, "Illegal combination of columns as arguments of function {}", getName());
  }
    ...
    while (!haystack_source.isEnd()) {
      ...
      if (needle.size > haystack.size)
        ...
        if constexpr (std::is_same_v<Name, NameStartsWith>)  /// startsWith
          ...
          res_data[row_num] = std::string_view(reinterpret_cast<const char *>(haystack.data) + haystack.size - needle.size, needle.size) ==
                              std::string_view(reinterpret_cast<const char *>(needle.data), needle.size);
  ...
  using CaseInsensitiveComparator =
      std::variant<std::unique_ptr<ASCIICaseInsensitiveStringSearcher>, std::unique_ptr<UTF8CaseInsensitiveStringSearcher>>;
      ...
      if (needle.size > haystack.size)
        ...
        if constexpr (std::is_same_v<NeedleSource, ConstSource<StringSource>> ||
          ...
          if constexpr (std::is_same_v<Name, NameStartsWithCaseInsensitive>)
            res_data[row_num] = std::get<std::unique_ptr<ASCIICaseInsensitiveStringSearcher>>(const_comparator)
                                    ->compare(haystack.data, haystack.data + haystack.size, haystack.data);
          else if constexpr (std::is_same_v<Name, NameStartsWithCaseInsensitiveUTF8>)
            res_data[row_num] = std::get<std::unique_ptr<UTF8CaseInsensitiveStringSearcher>>(const_comparator)
                                    ->compare(haystack.data, haystack.data + haystack.size, haystack.data);
          else if constexpr (std::is_same_v<Name, NameEndsWithCaseInsensitive>)
            res_data[row_num] = std::get<std::unique_ptr<ASCIICaseInsensitiveStringSearcher>>(const_comparator)
                                    ->compare(haystack.data + haystack.size - needle.size, haystack.data + haystack.size,
                                              haystack.data + haystack.size - needle.size);
        ...
        {
          if constexpr (std::is_same_v<Name, NameStartsWithCaseInsensitive>)
            res_data[row_num] = ASCIICaseInsensitiveStringSearcher(needle.data, needle.size)
                                    .compare(haystack.data, haystack.data + haystack.size, haystack.data);
          else if constexpr (std::is_same_v<Name, NameStartsWithCaseInsensitiveUTF8>)
            res_data[row_num] = UTF8CaseInsensitiveStringSearcher(needle.data, needle.size)
                                    .compare(haystack.data, haystack.data + haystack.size, haystack.data);
          else if constexpr (std::is_same_v<Name, NameEndsWithCaseInsensitive>)
            res_data[row_num] = ASCIICaseInsensitiveStringSearcher(needle.data, needle.size)
                                    .compare(haystack.data + haystack.size - needle.size, haystack.data + haystack.size,
