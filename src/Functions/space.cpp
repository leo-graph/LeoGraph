...
class FunctionSpace : public IFunction {
 ...
 public:
  ...
  DataTypePtr getReturnTypeImpl(const ColumnsWithTypeAndName &arguments) const override {
    FunctionArgumentDescriptors args{{"n", static_cast<FunctionArgumentDescriptor::TypeValidator>(&isInteger), nullptr, "Integer"}};
  ...
  bool executeConstant(ColumnPtr col_times, ColumnString::Offsets &res_offsets, ColumnString::Chars &res_chars,
    ...
    if (!checkAndGetColumn<typename DataType::ColumnType>(col_times_const_internal.get())) return false;
  ...
  template <typename DataType>
  bool executeVector(ColumnPtr col_times_, ColumnString::Offsets &res_offsets, ColumnString::Chars &res_chars,
                     size_t input_rows_count) const {
    auto *col_times = checkAndGetColumn<typename DataType::ColumnType>(col_times_.get());
    if (!col_times) return false;
    ...
    for (size_t i = 0; i < input_rows_count; ++i) {
      ...
      if (pos + times + 1 > res_chars.size()) res_chars.resize(std::max(2 * res_chars.size(), static_cast<size_t>(pos + times)));
...
Concatenates a space (` `) as many times with itself as specified.
  ...
  FunctionDocumentation::Arguments arguments = {{"n", "The number of times to repeat the space.", {"(U)Int*"}}};
  FunctionDocumentation::ReturnedValue returned_value = {
      "Returns astring containing a space repeated `n` times. If `n <= 0`, the function returns the empty string.", {"String"}};
  FunctionDocumentation::Examples examples = {{"Usage example", "SELECT space(3) AS res, length(res);",
                                               R"(
┌─res─┬─length(res)─┐
│     │           3 │
└─────┴─────────────┘
