...
class FunctionArrayNorm : public IFunction {
 public:
  String getName() const override {
    static auto name = String("array") + Kernel::name + "Norm";
    return name;
  }
  ...
  DataTypePtr getReturnTypeImpl(const ColumnsWithTypeAndName &arguments) const override {
    ...
    if (!array_type) throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT, "Argument of function {} must be array.", getName());
...
#define SUPPORTED_TYPES(action)                                                                                                      \
  action(UInt8) action(UInt16) action(UInt32) action(UInt64) action(Int8) action(Int16) action(Int32) action(Int64) action(BFloat16) \
      action(Float32) action(Float64)

  template <typename ResultType>
  ColumnPtr executeWithResultType(const ColumnArray &array, const DataTypePtr &nested_type, size_t input_rows_count,
...
#undef ON_TYPE
  ...
  ColumnPtr executeWithTypes(const ColumnArray &array, size_t input_rows_count, const ColumnsWithTypeAndName &arguments) const {
    ...
    for (auto off : offsets) {
      ...
      ResultType result = 0;
      for (const auto &other_state : results) result = Kernel::template combine<ResultType>(result, other_state, kernel_params);
...
template <>
size_t FunctionArrayNorm<LpNorm>::getNumberOfArguments() const {
  return 2;
}

template <>
ColumnNumbers FunctionArrayNorm<LpNorm>::getArgumentsThatAreAlwaysConstant() const {
  return {1};
}

template <>
LpNorm::ConstParams FunctionArrayNorm<LpNorm>::initConstParams(const ColumnsWithTypeAndName &arguments) const {
  if (arguments.size() < 2) throw Exception(ErrorCodes::LOGICAL_ERROR, "Argument p of function {} was not provided", getName());
