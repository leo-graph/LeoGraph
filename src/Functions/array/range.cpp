 ...
 private:
  ...
  DataTypePtr getReturnTypeImpl(const DataTypes &arguments) const override {
    if (arguments.size() > 3 || arguments.empty()) {
      throw Exception(ErrorCodes::NUMBER_OF_ARGUMENTS_DOESNT_MATCH, "Function {} needs 1..3 arguments; passed {}.", getName(),
                      arguments.size());
    }
    ...
    for (size_t i = 0, size = arguments.size(); i < size; ++i) {
      ...
      if (i < 2 && WhichDataType(type_no_nullable).isIPv4())
        ...
        throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT, "Illegal type {} of argument of function {}", arguments[i]->getName(),
                        getName());
    }
  ...
  ColumnPtr executeInternal(const IColumn *arg) const {
    if (const auto in = checkAndGetColumn<ColumnVector<T>>(arg)) {
      ...
      const auto total_values = std::accumulate(std::begin(in_data), std::end(in_data), size_t{}, [this](const size_t lhs, const T rhs) {
        if (rhs < 0)
      ...
      for (size_t row_idx = 0, rows = in->size(); row_idx < rows; ++row_idx) {
        for (T elem_idx = 0, elems = in_data[row_idx]; elem_idx < elems; ++elem_idx) out_data[offset + elem_idx] = elem_idx;
  ...
  ColumnPtr executeConstStartStep(const IColumn *end_arg, const T start, const T step, const size_t input_rows_count) const {
    ...
    for (size_t row_idx = 0; row_idx < input_rows_count; ++row_idx) {
      if (step == 0)
        throw Exception(ErrorCodes::ARGUMENT_OUT_OF_BOUND, "A call to function {} overflows, the 3rd argument step can't be zero",
                        getName());

      if (start < end_data[row_idx] && step > 0)
        row_length[row_idx] = static_cast<size_t>(
            (static_cast<__int128_t>(end_data[row_idx]) - static_cast<__int128_t>(start) - 1) / static_cast<__int128_t>(step) + 1);
  ...
  ColumnPtr executeConstStep(const IColumn *start_arg, const IColumn *end_arg, const T step, const size_t input_rows_count) const {
    ...
    auto end_column = checkAndGetColumn<ColumnVector<T>>(end_arg);
    if (!end_column || !start_column) return nullptr;
    ...
    for (size_t row_idx = 0; row_idx < input_rows_count; ++row_idx) {
      ...
      if (start_data[row_idx] < end_data[row_idx] && step > 0)
        row_length[row_idx] =
            static_cast<size_t>((static_cast<__int128_t>(end_data[row_idx]) - static_cast<__int128_t>(start_data[row_idx]) - 1) /
                                    static_cast<__int128_t>(step) +
                                1);
  ...
  ColumnPtr executeConstStart(const IColumn *end_arg, const IColumn *step_arg, const T start, const size_t input_rows_count) const {
    ...
    if (!end_column || !step_column) return nullptr;
    ...
    for (size_t row_idx = 0; row_idx < input_rows_count; ++row_idx) {
      ...
      if (start < end_data[row_idx] && step_data[row_idx] > 0)
        row_length[row_idx] = static_cast<size_t>((static_cast<__int128_t>(end_data[row_idx]) - static_cast<__int128_t>(start) - 1) /
                                                      static_cast<__int128_t>(step_data[row_idx]) +
                                                  1);
  ...
  template <typename T>
  ColumnPtr executeGeneric(const IColumn *start_col, const IColumn *end_col, const IColumn *step_col, const size_t input_rows_count) const {
    ...
    if (!start_column || !end_column || !step_column) return nullptr;
    ...
    for (size_t row_idx = 0; row_idx < input_rows_count; ++row_idx) {
      ...
      if (start_data[row_idx] < end_start[row_idx] && step_data[row_idx] > 0)
        row_length[row_idx] =
            static_cast<size_t>((static_cast<__int128_t>(end_start[row_idx]) - static_cast<__int128_t>(start_data[row_idx]) - 1) /
                                    static_cast<__int128_t>(step_data[row_idx]) +
                                1);
  ...
  ColumnPtr executeImpl(const ColumnsWithTypeAndName &arguments, const DataTypePtr &result_type, size_t input_rows_count) const override {
    ...
    if (!which.isNativeUInt() && !which.isNativeInt()) {
      throw Exception(ErrorCodes::ILLEGAL_COLUMN,
                      ...
                      "for unsigned/signed integers up to 64 bit",
                      getName());
    }

    auto throw_if_null_value = [&](const ColumnWithTypeAndName &col) {
      if (!col.type->isNullable()) return;
      const ColumnNullable *nullable_col = checkAndGetColumn<ColumnNullable>(col.column.get());
      if (!nullable_col) nullable_col = checkAndGetColumnConstData<ColumnNullable>(col.column.get());
      if (!nullable_col) return;
      const auto &null_map = nullable_col->getNullMapData();
      if (!memoryIsZero(null_map.data(), 0, null_map.size()))
        throw Exception(ErrorCodes::BAD_ARGUMENTS, "Illegal (null) value column {} of argument of function {}", col.column->getName(),
                        getName());
    };
    ...
    if (arguments.size() == 2) {
      ...
      columns_holder[2] =
          castColumn({DataTypeUInt8().createColumnConst(input_rows_count, 1), std::make_shared<DataTypeUInt8>(), {}}, elem_type);
    ...
    if (is_start_const && is_step_const) {
      if (which.isNativeUInt()) {
        ...
        if ((res = executeConstStartStep<UInt8>(column_ptrs[1], static_cast<UInt8>(start), static_cast<UInt8>(step), input_rows_count)) ||
            (res =
                 executeConstStartStep<UInt16>(column_ptrs[1], static_cast<UInt16>(start), static_cast<UInt16>(step), input_rows_count)) ||
            (res =
                 executeConstStartStep<UInt32>(column_ptrs[1], static_cast<UInt32>(start), static_cast<UInt32>(step), input_rows_count)) ||
        ...
        if ((res = executeConstStartStep<Int8>(column_ptrs[1], static_cast<Int8>(start), static_cast<Int8>(step), input_rows_count)) ||
            ...
            (res = executeConstStartStep<Int32>(column_ptrs[1], static_cast<Int32>(start), static_cast<Int32>(step), input_rows_count)) ||
