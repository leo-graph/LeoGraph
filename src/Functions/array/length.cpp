...
It is ok to have ASCII NULL bytes in strings, and they will be counted as well.
  ...
  FunctionDocumentation::Arguments arguments = {
      {"x",
       "Value for which to calculate the number of bytes (for String/FixedString) or elements (for Array).",
       {"String", "FixedString", "Array(T)"}}};
  ...
  FunctionDocumentation::Examples examples{
      {"String example", "SELECT length('Hello, world!')", "13"},
      {"Array example", "SELECT length(['Hello', 'world'])", "2"},
      {"constexpr example",
       R"(
...
└──────┴──────────────┴──────────────────┴──────┴──────────────┴──────────────────┘
      ...
      {"ascii_vs_utf8 example", "SELECT 'ábc' AS str, length(str), lengthUTF8(str)",
       R"(
