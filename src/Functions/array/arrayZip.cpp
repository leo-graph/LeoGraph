...
class FunctionArrayZip : public IFunction {
 public:
  FunctionArrayZip(const char *name_, bool allow_unaligned_) : function_name(name_), allow_unaligned(allow_unaligned_) {}

  static FunctionPtr create(const char *name, bool allow_unaligned) { return std::make_shared<FunctionArrayZip>(name, allow_unaligned); }
  ...
  DataTypePtr getReturnTypeImpl(const ColumnsWithTypeAndName &arguments) const override {
    ...
    for (size_t index = 0; index < arguments.size(); ++index) {
      ...
      if (allow_unaligned) nested_type = makeNullable(nested_type);
  ...
  ColumnPtr executeImpl(const ColumnsWithTypeAndName &arguments, const DataTypePtr &result_type, size_t input_rows_count) const override {
    ...
    for (size_t i = 0; i < num_arguments; ++i) {
      ...
      if (!column_array)
        throw Exception(ErrorCodes::ILLEGAL_COLUMN, "Argument {} of function {} must be array. Found column {} instead.", i + 1, getName(),
                        holder->getName());
    ...
    if (!allow_unaligned) {
      if (has_unaligned)
        throw Exception(ErrorCodes::SIZES_OF_ARRAYS_DONT_MATCH, "The argument 1 and argument {} of function {} have different array sizes",
                        unaligned_index + 1, getName());
      return ColumnArray::create(ColumnTuple::create(std::move(tuple_columns)),
                                 static_cast<const ColumnArray &>(*holders[0]).getOffsetsPtr());
    } else
 ...
 private:
  ...
  ColumnPtr executeUnaligned(const Columns &holders, Columns &tuple_columns, size_t input_rows_count, bool has_unaligned) const {
    ...
    for (size_t i = 0; i < holders.size(); ++i) array_columns[i] = checkAndGetColumn<ColumnArray>(holders[i].get());

    for (auto &tuple_column : tuple_columns) tuple_column = makeNullable(tuple_column);

    if (!has_unaligned) return ColumnArray::create(ColumnTuple::create(std::move(tuple_columns)), array_columns[0]->getOffsetsPtr());
...
REGISTER_FUNCTION(ArrayZip) {
  FunctionDocumentation::Description description =
      "Combines multiple arrays into a single array. The resulting array contains the corresponding elements of the source arrays grouped "
  ...
  FunctionDocumentation::ReturnedValue returned_value = {
      "Returns an array with elements from the source arrays grouped in tuples. Data types in the tuple are the same as types of the input "
      "arrays and in the same order as arrays are passed",
      {"Array(T)"}};
  FunctionDocumentation::Examples example = {
      {"Usage example", "SELECT arrayZip(['a', 'b', 'c'], [5, 2, 1]);", "[('a', 5), ('b', 2), ('c', 1)]"}};
  ...
  factory.registerFunction("arrayZip", [](ContextPtr) { return FunctionArrayZip::create("arrayZip", false); }, documentation);

  FunctionDocumentation::Description description_unaligned =
      "Combines multiple arrays into a single array, allowing for unaligned arrays (arrays of differing lengths). The resulting array "
      "contains the corresponding elements of the source arrays grouped into tuples in the listed order of arguments.";
  FunctionDocumentation::Syntax syntax_unaligned = "arrayZipUnaligned(arr1, arr2, ..., arrN)";
  FunctionDocumentation::Arguments argument_unaligned = {
  ...
  FunctionDocumentation::ReturnedValue returned_value_unaligned = {
      ...
      "arrays and in the same order as arrays are passed.",
      {"Array(T)", "Tuple(T1, T2, ...)"}};
  FunctionDocumentation::Examples example_unaligned = {
      {"Usage example", "SELECT arrayZipUnaligned(['a'], [1, 2, 3]);", "[('a', 1),(NULL, 2),(NULL, 3)]"}};
  ...
  FunctionDocumentation documentation_unaligned = {
      description_unaligned,    syntax_unaligned,  argument_unaligned,      {},
      returned_value_unaligned, example_unaligned, introduced_in_unaligned, category_unaligned};

  factory.registerFunction(
      "arrayZipUnaligned", [](ContextPtr) { return FunctionArrayZip::create("arrayZipUnaligned", true); }, documentation_unaligned);
}
