...
class FunctionArrayDistance : public IFunction {
 public:
  ...
  DataTypePtr getReturnTypeImpl(const ColumnsWithTypeAndName &arguments) const override {
    if (arguments.size() < 3)
      throw Exception(ErrorCodes::TOO_FEW_ARGUMENTS_FOR_FUNCTION, "Number of arguments for function {} can't be {}, should be at least 3",
    ...
    if (validateOptimizedArguments(arguments)) return std::make_shared<DataTypeFloat64>();

    if (arguments.size() > 3)
      throw Exception(ErrorCodes::TOO_MANY_ARGUMENTS_FOR_FUNCTION, "Number of arguments for function {} is {}. Expected 3", getName(),
                      arguments.size());
    ...
    if (!zeroth_arg_type) throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT, "First argument of function {} must be a QBit", getName());
    ...
    if (!which.isUInt8())
      throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT, "The third argument of function {} must be a UInt8 constant, got {}", getName(),
                      precision_col.type->getName());

    if (!(precision_col.column && precision_col.column->isConst()))
      throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT,
                      "The third argument of function {} must be a UInt8 constant, got {} (not constant)", getName(),
                      precision_col.type->getName());
    ...
    if (precision == 0 || precision > element_size)
      throw Exception(ErrorCodes::BAD_ARGUMENTS,
                      "The third argument (precision) of function {} must be in range [1, {}] for {} QBit, got {}", getName(), element_size,
                      zeroth_arg_type->getElementType()->getName(), precision);
  ...
  bool validateOptimizedArguments(const ColumnsWithTypeAndName &arguments) const {
    ...
    for (size_t i = 0; i < precision; ++i) {
      ...
      if (!arg_type || arg_type->getN() != qbit_size_bytes) return false;
    }
    ...
    if (precision > max_precision_for_type) return false;
  ...
  ColumnPtr executeImpl(const ColumnsWithTypeAndName &arguments, const DataTypePtr & /* result_type */,
    ...
    if (which_last.isUInt8()) return executeWithQBitColumnConverted(arguments, input_rows_count);
    ...
    if (!offsets.empty()) {
      for (size_t i = 0; i < reference_vector.size(); ++i) {
        if (offsets[i] - offsets[i - 1] != qbit_size)
          throw Exception(ErrorCodes::BAD_ARGUMENTS,
                          "The reference vector in the last argument of function {} has wrong size. Got: {}, expected: {}", getName(),
                          offsets[i] - offsets[i - 1], qbit_size);
      }
    }
    ...
    for (size_t i = precision; i < arguments.size(); ++i) expanded_arguments.emplace_back(arguments[i]);
    ...
    auto execute_with_type = [&]<typename T>() -> ColumnPtr {
      return dispatch_by_accum_type.template operator()<T>([&]<typename RefT, typename CalcT>() {
        return ref_is_const
 ...
 private:
  ...
  ColumnPtr executeWithQBitColumnConverted(const ColumnsWithTypeAndName &arguments, size_t input_rows_count) const {
    ...
    for (size_t bit = 0; bit < precision; ++bit) {
      ...
      if (qbit_is_const) bit_plane_col = ColumnConst::create(std::move(bit_plane_col), input_rows_count);
  ...
  ColumnPtr executeDistanceCalculation(const ColumnArray &col_y, const ColumnsWithTypeAndName &arguments, const size_t qbit_size,
    ...
    if constexpr (!std::is_same_v<RefT, CalcT>) {
      ...
      for (size_t i = 0; i < array_data.size(); ++i) array_data_downcasted[i] = static_cast<CalcT>(array_data[i]);
