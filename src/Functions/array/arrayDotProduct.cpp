...
struct DotProduct {
  ...
  static DataTypePtr getReturnType(const DataTypePtr &left, const DataTypePtr &right) {
    using Types = TypeList<DataTypeFloat32, DataTypeFloat64, DataTypeUInt8, DataTypeUInt16, DataTypeUInt32, DataTypeUInt64, DataTypeInt8,
...
#if USE_MULTITARGET_CODE
  template <typename Type>
  X86_64_V4_FUNCTION_SPECIFIC_ATTRIBUTE static void accumulateCombine(const Type *__restrict data_x, const Type *__restrict data_y,
                                                                      size_t i_max, size_t &i, State<Type> &state) {
 ...
 public:
  ...
  DataTypePtr getReturnTypeImpl(const DataTypes &arguments) const override {
    ...
    for (size_t i = 0; i < 2; ++i) {
      ...
      if (!array_type) throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT, "Arguments for function {} must be of type Array", getName());

      const auto &nested_type = array_type->getNestedType();
      if (!isNativeNumber(nested_type))
        throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT, "Function {} cannot process values of type {}", getName(),
                        nested_type->getName());
...
#undef ON_TYPE
  ...
  ColumnPtr executeWithLeftAndRightType(ColumnPtr col_x, ColumnPtr col_y, size_t input_rows_count) const {
    ...
    using ResultType = std::conditional_t<std::is_same_v<LeftType, Float32> && std::is_same_v<RightType, Float32>, Float32,
    ...
    for (size_t row = 0; row < input_rows_count; ++row) {
      ...
      for (; i + VEC_SIZE < array_size; i += VEC_SIZE) {
        for (size_t j = 0; j < VEC_SIZE; ++j)
          Kernel::template accumulate<ResultType>(states[j], static_cast<ResultType>(data_x[current_offset + i + j]),
                                                  static_cast<ResultType>(data_y[current_offset + i + j]));
      }

      typename Kernel::template State<ResultType> state;
      for (const auto &other_state : states) Kernel::template combine<ResultType>(state, other_state);

      /// Process the tail
      for (; i < array_size; ++i)
        Kernel::template accumulate<ResultType>(state, static_cast<ResultType>(data_x[current_offset + i]),
                                                static_cast<ResultType>(data_y[current_offset + i]));
  ...
  ColumnPtr executeWithLeftArgConst(ColumnPtr col_x, ColumnPtr col_y, size_t input_rows_count) const {
    ...
    for (auto offset_y : offsets_y) {
      if (offsets_x[0] != offset_y - prev_offset) [[unlikely]] {
        throw Exception(ErrorCodes::SIZES_OF_ARRAYS_DONT_MATCH, "Arguments of function {} have different array sizes: {} and {}", getName(),
                        offsets_x[0], offset_y - prev_offset);
      }
    ...
    for (size_t row = 0; row < input_rows_count; ++row) {
...
#if USE_MULTITARGET_CODE
      if constexpr ((std::is_same_v<ResultType, Float32> || std::is_same_v<ResultType, Float64>) && std::is_same_v<ResultType, LeftType> &&
      ...
      for (; i + VEC_SIZE < array_size; i += VEC_SIZE) {
        for (size_t j = 0; j < VEC_SIZE; ++j)
          Kernel::template accumulate<ResultType>(states[j], static_cast<ResultType>(data_x[i + j]),
                                                  static_cast<ResultType>(data_y[current_offset + i + j]));
      }
      ...
      /// Process the tail
      for (; i < array_size; ++i)
        Kernel::template accumulate<ResultType>(state, static_cast<ResultType>(data_x[i]),
                                                static_cast<ResultType>(data_y[current_offset + i]));
...
:::
  ...
  FunctionDocumentation::Examples examples = {
      ...
      {"Tuple example", "SELECT dotProduct((1::UInt16, 2::UInt8, 3::Float32),(4::Int16, 5::Float32, 6::UInt8)) AS res, toTypeName(res);",
       "32    Float64"}};
