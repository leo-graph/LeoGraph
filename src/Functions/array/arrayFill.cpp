...
struct ArrayFillImpl {
  ...
  static ColumnPtr execute(const ColumnArray &array, ColumnPtr mapped) {
    ...
    if (column_fill) {
      ...
      if (!column_fill_const) throw Exception(ErrorCodes::ILLEGAL_COLUMN, "Unexpected type of cut column");

      if (column_fill_const->getValue<UInt8>()) return ColumnArray::create(array.getDataPtr(), array.getOffsetsPtr());
    ...
    return ColumnArray::create(std::move(column_data), array.getOffsetsPtr());
  }
};

struct NameArrayFill {
  static constexpr auto name = "arrayFill";
};
struct NameArrayReverseFill {
  static constexpr auto name = "arrayReverseFill";
};
...
regardless of any condition.
  ...
  FunctionDocumentation::Arguments arguments = {
      {"func(x [, y1, ..., yN])",
       "A lambda function `func(x [, y1, y2, ... yN]) → F(x [, y1, y2, ... yN])` which operates on elements of the source array (`x`) and "
       "condition arrays (`y`).",
       {"Lambda function"}},
  ...
  FunctionDocumentation::ReturnedValue returned_value = {"Returns an array", {"Array(T)"}};
  FunctionDocumentation::Examples examples = {
      {"Example with single array", "SELECT arrayFill(x -> not isNull(x), [1, null, 2, null]) AS res", "[1, 1, 2, 2]"},
      {"Example with two arrays", "SELECT arrayFill(x, y, z -> x > y AND x < z, [5, 3, 6, 2], [4, 7, 1, 3], [10, 2, 8, 5]) AS res",
       "[5, 5, 6, 6]"}};
...
regardless of any condition.
  ...
  FunctionDocumentation::Examples examples_reverse = {
      ...
      {"Example with two arrays", "SELECT arrayReverseFill(x, y, z -> x > y AND x < z, [5, 3, 6, 2], [4, 7, 1, 3], [10, 2, 8, 5]) AS res;",
       "[5, 6, 6, 2]"}};
  ...
  FunctionDocumentation documentation_reverse = {description_reverse,    syntax_reverse,   arguments_reverse,     {},
                                                 returned_value_reverse, examples_reverse, introduced_in_reverse, category_reverse};
