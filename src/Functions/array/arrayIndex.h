...
namespace Impl {
template <typename ConcreteAction, bool RightArgIsConstant = false, typename IntegralInitial = UInt64, typename IntegralResult = UInt64>
struct Main {
 private:
...
#pragma clang diagnostic ignored "-Wsign-compare"
  ...
  static constexpr bool compare(const PaddedPODArray<Initial> &left, const PaddedPODArray<Result> &right, size_t i, size_t j) noexcept {
    if constexpr (std::is_floating_point_v<Initial> && !std::is_floating_point_v<Result>) {
  ...
  /// LowCardinality
  static bool compare(const IColumn &left, const Result &right, size_t i, size_t) { return left.getUInt(i) == right; }
  ...
  static bool compare(const Array &arr, const Field &rhs, size_t pos, size_t) { return accurateEquals(arr[pos], rhs); }
  ...
  static bool lessOrEqual(const Array &arr, const Field &rhs, size_t pos, size_t) { return accurateLessOrEqual(rhs, arr[pos]); }
...
#pragma clang diagnostic pop
 ...
 public:
  ...
  static constexpr ResultType linearSearch(const Data &data, const Target &target, size_t array_size, const NullMap *const null_map_data,
                                           const NullMap *const null_map_item, size_t row_index, ArrOffset current_offset) {
    ResultType current = 0;
    for (size_t j = 0; j < array_size; ++j) {
      if constexpr (Case == 2)  /// Right arg is Nullable
        if (hasNull(null_map_item, row_index)) continue;

      if constexpr (Case == 3)  /// Left arg is an array of Nullables
        if (hasNull(null_map_data, current_offset + j)) continue;
      ...
      if constexpr (Case == 4)  /// Both args are nullable
      {
        ...
        if (right_is_null != left_is_null) continue;

        if (!right_is_null && !compare(data, target, current_offset + j, row_index)) continue;
      } else if (!compare(data, target, current_offset + j, row_index))
        continue;

      ConcreteAction::apply(current, j);

      if constexpr (!ConcreteAction::resume_execution) break;
    }
    return current;
  }
  ...
  static ResultType linearSearchConst(const Array &arr, const Field &value) {
    ...
    for (size_t i = 0, size = arr.size(); i < size; ++i) {
      if (!accurateEquals(arr[i], value)) continue;
 ...
 private:
  ...
  template <size_t Case, typename Data, typename Target>
  static constexpr ResultType getIndex(const Data &data, const Target &target, size_t array_size, const NullMap *const null_map_data,
                                       const NullMap *const null_map_item, size_t row_index, ArrOffset current_offset) {
     ...
     */
    if constexpr (std::is_same_v<ConcreteAction, IndexOfAssumeSorted> && !std::is_same_v<Target, PaddedPODArray<Result>> &&
  ...
  template <size_t Case, typename Data, typename Target>
  static void process(const Data &data, const ArrOffsets &offsets, const Target &target, ResultArr &result,
                      [[maybe_unused]] const NullMap *const null_map_data, [[maybe_unused]] const NullMap *const null_map_item) {
 ...
 public:
  template <typename Data, typename Target>
  static void vector(const Data &data, const ArrOffsets &offsets, const Target &value, ResultArr &result,
...
struct Null {
  ...
  static void process(const ColumnArray::Offsets &offsets, PaddedPODArray<ResultType> &result,
    ...
    for (size_t i = 0; i < size; ++i) {
      ...
      for (size_t j = 0; j < array_size; ++j) {
        if (!(*null_map_data)[current_offset + j]) continue;
...
struct String {
 private:
  ...
  using OffsetT = std::conditional_t<IsConst, Offset, const ColumnString::Offsets &>;
  using ArrayOffset = ColumnArray::Offset;
  using ResultType = typename ConcreteAction::ResultType;

  template <bool IsConst, bool HasNullMapData, bool HasNullMapItem>
  static void processImpl(const ColumnString::Chars &data, const ColumnArray::Offsets &offsets, const ColumnString::Offsets &string_offsets,
                          const ColumnString::Chars &item_values, OffsetT<IsConst> item_offsets, PaddedPODArray<ResultType> &result,
                          [[maybe_unused]] const NullMap *data_map, [[maybe_unused]] const NullMap *item_map) {
    const size_t size = offsets.size();
    result.resize(size);
      ...
      for (size_t j = 0; j < array_size; ++j) {
        ...
        if constexpr (IsConst) {
          if constexpr (HasNullMapData)
            if ((*data_map)[current_offset + j]) continue;

          if (!memequalSmallAllowOverflow15(item_values.data(), item_offsets, &data[string_pos], string_size)) continue;
        } else if constexpr (HasNullMapData) {
          if ((*data_map)[current_offset + j]) {
            if constexpr (!HasNullMapItem) continue;

            if (!(*item_map)[i]) continue;
  ...
  template <bool IsConst>
  static void invokeCheckNullMaps(const ColumnString::Chars &data, const ColumnArray::Offsets &offsets,
                                  const ColumnString::Offsets &str_offsets, const ColumnString::Chars &values,
                                  OffsetT<IsConst> item_offsets, PaddedPODArray<ResultType> &result, const NullMap *data_map,
 ...
 public:
  static void process(const ColumnString::Chars &data, const ColumnArray::Offsets &offsets, const ColumnString::Offsets &string_offsets,
                      const ColumnString::Chars &item_values, Offset item_offsets, PaddedPODArray<ResultType> &result,
  ...
  static void process(const ColumnString::Chars &data, const ColumnArray::Offsets &offsets, const ColumnString::Offsets &string_offsets,
                      const ColumnString::Chars &item_values, const ColumnString::Offsets &item_offsets, PaddedPODArray<ResultType> &result,
                      const NullMap *data_map, const NullMap *item_map) {
...
class FunctionArrayIndex : public IFunction {
 public:
  ...
  DataTypePtr getReturnTypeImpl(const ColumnsWithTypeAndName &arguments) const override {
    ...
    if constexpr (std::is_same_v<ConcreteAction, HasAction>) {
      if (!array_type && !map_type && !object_type)
        throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT, "First argument for function {} must be an array, map or JSON. Actual {}",
                        getName(), first_argument_type->getName());

      if (object_type) {
        if (!isStringOrFixedString(second_argument_type))
          throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT,
                          "Second argument for function {} must be String when the first argument is JSON. Actual {}", getName(),
      ...
      if (!array_type)
        throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT, "First argument for function {} must be an array. Actual {}", getName(),
    ...
    if (!second_argument_type->onlyNull() && !allowArguments(inner_type, second_argument_type)) {
      ...
      throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT,
                      ...
                      first_argument_type_name, getName(), first_argument_type->getName(), second_argument_type->getName());
    }
  ...
  ColumnPtr executeImpl(const ColumnsWithTypeAndName &arguments, const DataTypePtr &result_type,
    ...
    if (auto res = executeMap(arguments, result_type)) return res;

    if (auto res = executeObject(arguments, result_type)) return res;

    if (auto res = executeArrayLowCardinality(arguments)) return res;
 ...
 private:
  ...
  ColumnPtr executeArrayImpl(const ColumnsWithTypeAndName &arguments, const DataTypePtr &result_type) const {
    ...
    if (col_array) nullable = checkAndGetColumn<ColumnNullable>(&col_array->getData());
    ...
    if (nullable) {
      ...
      data.type = std::make_shared<DataTypeArray>(
          static_cast<const DataTypeNullable &>(*static_cast<const DataTypeArray &>(*arguments[0].type).getNestedType()).getNestedType());
...
#define INTEGRAL_PACK UInt8, UInt16, UInt32, UInt64, Int8, Int16, Int32, Int64, Float32, Float64
  ...
  ColumnPtr executeOnNonNullable(const ColumnsWithTypeAndName &arguments, const DataTypePtr &result_type) const {
    ColumnPtr res;
    if (!((res = executeNothing(arguments)) || (res = executeIntegral<INTEGRAL_PACK>(arguments)) ||
          (res = executeConst(arguments, result_type)) || (res = executeString(arguments)) || (res = executeGeneric(arguments))))
...
#undef INTEGRAL_PACK
  ...
  static NullMaps getNullMaps(const ColumnsWithTypeAndName &arguments) {
    if (arguments.size() < 3) return {nullptr, nullptr};

    const NullMap *null_map_data = nullptr;
    const NullMap *null_map_item = nullptr;

    if (const auto &data_map = arguments[2].column; data_map) null_map_data = &assert_cast<const ColumnUInt8 &>(*data_map).getData();

    if (const auto &item_map = arguments[3].column; item_map) null_map_item = &assert_cast<const ColumnUInt8 &>(*item_map).getData();
  ...
  static bool executeIntegralImpl(const ExecutionData &data, ResultColumnType &result) {
    ...
    if (!left_typed) return false;

    if (const auto *item_arg_const = checkAndGetColumnConst<ColumnVector<Resulting>>(&data.right))
      Impl::Main<ConcreteAction, true, Initial, Resulting>::vector(left_typed->getData(), data.offsets,
                                                                   item_arg_const->template getValue<Resulting>(), result.getData(),
                                                                   data.null_maps.first, nullptr);
    else if (const auto *item_arg_vector = checkAndGetColumn<ColumnVector<Resulting>>(&data.right))
      Impl::Main<ConcreteAction, false, Initial, Resulting>::vector(left_typed->getData(), data.offsets, item_arg_vector->getData(),
                                                                    result.getData(), data.null_maps.first, data.null_maps.second);
    else
      return false;
  ...
  static ColumnPtr executeArrayLowCardinality(const ColumnsWithTypeAndName &arguments) {
    ...
    if (!right->isNullAt(0)) {
      ...
      if (right->isNullable()) right = checkAndGetColumn<ColumnNullable>(*right).getNestedColumnPtr();
      ...
      if (std::optional<UInt64> maybe_index = left_dict.getOrFindValueIndex(elem); maybe_index) {
        ...
        if (col_array_const) return ColumnConst::create(std::move(col_result), left_size);

        return col_result;
      }
    }

    Impl::Main<ConcreteAction, true>::vector(left_lc->getIndexes(), col_array->getOffsets(),
                                             index, /** Assuming LowCardinality has index of NULL always as zero. */
                                             col_result->getData(), nullptr, nullptr);
  ...
  ColumnPtr executeMap(const ColumnsWithTypeAndName &arguments, const DataTypePtr &result_type) const {
    if constexpr (!std::is_same_v<ConcreteAction, HasAction>) return nullptr;

    if (!isMap(arguments[0].type)) return nullptr;
   ...
   */
  static bool hasPathInSharedData(const String &path, const String &prefix, const ColumnString *shared_paths,
                                  const ColumnVector<UInt64>::Container &shared_offsets, size_t row) {
    if (!shared_paths) return false;
    ...
    if (pos < end && shared_paths->getDataAt(pos) == path) return true;

    /// Check for prefix match (path + ".")
    pos = ColumnObject::findPathLowerBoundInSharedData(prefix, *shared_paths, start, end);
    if (pos < end && shared_paths->getDataAt(pos).starts_with(prefix)) return true;
   ...
   */
  static bool hasPathInObjectRow(const ColumnObject &object_column, size_t row, const String &path, const String &prefix,
                                 const ColumnString *shared_paths, const ColumnVector<UInt64>::Container &shared_offsets) {
    ...
    if (typed_paths.contains(path)) return true;
    ...
    if (auto it = dynamic_paths.find(path); it != dynamic_paths.end() && !it->second->isNullAt(row)) return true;

    for (const auto &[key, col] : dynamic_paths) {
      if (key.starts_with(prefix) && !col->isNullAt(row)) return true;
    }
  ...
  ColumnPtr executeObject(const ColumnsWithTypeAndName &arguments, const DataTypePtr & /*result_type*/) const {
    ...
    if (input_rows_count == 0) return ColumnUInt8::create();
    ...
    if (isColumnConst(path_column)) {
      ...
      if (auto it = dynamic_paths.find(path); it != dynamic_paths.end()) relevant_dynamic_columns.push_back(it->second);

      for (const auto &[key, col] : dynamic_paths) {
        if (key.starts_with(prefix)) relevant_dynamic_columns.push_back(col);
      }
  ...
  static ColumnPtr executeString(const ColumnsWithTypeAndName &arguments) {
    ...
    if (const auto *item_arg_const = checkAndGetColumnConstStringOrFixedString(&right)) {
      ...
      if (item_const_string)
        Impl::String<ConcreteAction>::process(left->getChars(), array->getOffsets(), left->getOffsets(), item_const_string->getChars(),
                                              item_const_string->getDataAt(0).size(), result->getData(), null_map_data, null_map_item);
      else if (item_const_fixedstring)
        Impl::String<ConcreteAction>::process(left->getChars(), array->getOffsets(), left->getOffsets(), item_const_fixedstring->getChars(),
                                              item_const_fixedstring->getN(), result->getData(), null_map_data, null_map_item);
      else
      ...
      Impl::String<ConcreteAction>::process(left->getChars(), array->getOffsets(), left->getOffsets(), item_arg_vector->getChars(),
                                            item_arg_vector->getOffsets(), result->getData(), null_map_data, null_map_item);
    } else {
      return nullptr;
    }
  ...
  static ColumnPtr executeConst(const ColumnsWithTypeAndName &arguments, const DataTypePtr &result_type) {
    ...
    if (arguments.size() > 2)
      if (const auto &col = arguments[3].column; col) null_map = &assert_cast<const ColumnUInt8 &>(*col).getData();
    ...
    for (size_t row = 0; row < size; ++row) {
      ...
      for (size_t i = 0, arr_size = arr.size(); i < arr_size; ++i) {
        if (arr[i].isNull()) {
          if (!null_map) continue;

          if (!(*null_map)[row]) continue;
        } else {
          if (null_map && (*null_map)[row]) continue;
          if (!accurateEquals(arr[i], value)) continue;
        }
  ...
  static ColumnPtr executeGeneric(const ColumnsWithTypeAndName &arguments) {
    ...
    if (const auto *item_arg_const = checkAndGetColumn<ColumnConst>(item_arg.get())) {
      Impl::Main<ConcreteAction, true>::vector(*col_nested, col_array->getOffsets(), item_arg_const->getDataColumn(),
                                               col_res->getData(),  /// TODO This is wrong.
                                               null_map_data, nullptr);
    } else {
      Impl::Main<ConcreteAction>::vector(*col_nested, col_array->getOffsets(), *item_arg, col_res->getData(), null_map_data, null_map_item);
    }
