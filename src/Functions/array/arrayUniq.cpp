...
/// Counts the number of different elements in the array, or the number of different tuples from the elements at the corresponding positions
/// in several arrays. NOTE The implementation partially matches arrayEnumerateUniq.
class FunctionArrayUniq : public IFunction {
 public:
  ...
  DataTypePtr getReturnTypeForDefaultImplementationForDynamic() const override { return std::make_shared<DataTypeUInt32>(); }
 ...
 private:
  ...
  struct MethodString {
    using Set = ClearableHashSetWithStackMemory<std::string_view, StringViewHash, INITIAL_SIZE_DEGREE>;
  ...
  struct MethodFixed {
    using Set = ClearableHashSetWithStackMemory<UInt128, UInt128HashCRC32, INITIAL_SIZE_DEGREE>;
...
template <typename Method, bool has_null_map>
void FunctionArrayUniq::executeMethodImpl(const ColumnArray::Offsets &offsets, const ColumnRawPtrs &columns, const Sizes &key_sizes,
                                          [[maybe_unused]] const NullMap *null_map, ColumnUInt32::Container &res_values) const {
  ...
  for (size_t i = 0; i < offsets.size(); ++i) {
    ...
    for (ColumnArray::Offset j = prev_off; j < off; ++j) {
      if constexpr (has_null_map) {  // NOLINT
        if ((*null_map)[j]) {
          found_null = true;
          continue;
        }
...
template <typename Method>
void FunctionArrayUniq::executeMethod(const ColumnArray::Offsets &offsets, const ColumnRawPtrs &columns, const Sizes &key_sizes,
...
template <typename T>
bool FunctionArrayUniq::executeNumber(const ColumnArray::Offsets &offsets, const IColumn &data, const NullMap *null_map,
                                      ColumnUInt32::Container &res_values) const {
...
bool FunctionArrayUniq::executeString(const ColumnArray::Offsets &offsets, const IColumn &data, const NullMap *null_map,
                                      ColumnUInt32::Container &res_values) const {
...
bool FunctionArrayUniq::executeFixedString(const ColumnArray::Offsets &offsets, const IColumn &data, const NullMap *null_map,
                                           ColumnUInt32::Container &res_values) const {
...
bool FunctionArrayUniq::execute128bit(const ColumnArray::Offsets &offsets, const ColumnRawPtrs &columns,
                                      ColumnUInt32::Container &res_values) const {
...
void FunctionArrayUniq::executeHashed(const ColumnArray::Offsets &offsets, const ColumnRawPtrs &columns,
                                      ColumnUInt32::Container &res_values) const {
...
:::
  ...
  FunctionDocumentation::Arguments arguments = {
      ...
      {"[, arr2, ..., arrN]",
       ...
       {"Array(T)"}}};
  FunctionDocumentation::Examples examples = {{"Single argument", "SELECT arrayUniq([1, 1, 2, 2])", "2"},
