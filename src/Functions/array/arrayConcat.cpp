...
ColumnPtr FunctionArrayConcat::executeImpl(const ColumnsWithTypeAndName &arguments, const DataTypePtr &result_type,
  ...
  for (size_t i = 0; i < num_args; ++i) {
    ...
    if (!arg.type->equals(*result_type)) preprocessed_column = castColumn(arg, result_type);
...
REGISTER_FUNCTION(ArrayConcat) {
  ...
  FunctionDocumentation::Arguments arguments = {{"arr1 [, arr2, ... , arrN]", "N number of arrays to concatenate.", {"Array(T)"}}};
