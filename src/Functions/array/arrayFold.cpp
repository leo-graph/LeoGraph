  ...
  void getLambdaArgumentTypes(DataTypes &arguments) const override {
    if (arguments.size() < 3)
      throw Exception(ErrorCodes::TOO_FEW_ARGUMENTS_FOR_FUNCTION,
                      "Function {} requires as arguments a lambda function, at least one array and an accumulator", getName());
    ...
    for (size_t i = 1; i < accumulator_and_array_types.size(); ++i) {
      ...
      if (!array_type)
        throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT, "Argument {} of function {} must be of type Array, found {} instead", i + 1,
    ...
    if (!lambda_function_type || lambda_function_type->getArgumentTypes().size() != accumulator_and_array_types.size())
      throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT,
                      "First argument of function {} must be a lambda function with {} arguments, found {} instead.", getName(),
                      accumulator_and_array_types.size(), arguments[0]->getName());
  ...
  DataTypePtr getReturnTypeImpl(const ColumnsWithTypeAndName &arguments) const override {
    ...
    if (!accumulator_type->equals(*lambda_type))
      throw Exception(ErrorCodes::TYPE_MISMATCH,
                      "Return type of lambda function must be the same as the accumulator type, inferred return type of lambda: {}, "
                      "inferred type of accumulator: {}",
  ...
  ColumnPtr executeImpl(const ColumnsWithTypeAndName &arguments, const DataTypePtr &, size_t input_rows_count) const override {
    ...
    for (size_t i = 1; i < arguments.size() - 1; ++i) {
      ...
      if (!array_col_concrete) {
        ...
        if (!aray_col_concrete_const) throw Exception(ErrorCodes::ILLEGAL_COLUMN, "Expected array column, found {}", array_col->getName());
      ...
      if (!first_array_col_offsets)
        ...
        if (array_col_concrete->getOffsetsPtr() != first_array_col_offsets &&
          ...
          throw Exception(ErrorCodes::SIZES_OF_ARRAYS_DONT_MATCH, "arrays_data_with_type_and_name passed to {} must have equal size",
                          getName());
      }
      ...
      ColumnWithTypeAndName data_type_name(array_col_concrete->getDataPtr(),
    ...
    const ssize_t num_elements_in_array_col =
        arrays_data_with_type_and_name[0]
            .column->size();  /// total number of array elements in the 1st array argument (the value is the same for other array arguments)

    if (num_rows == 0) return arguments.back().column->convertToFullColumnIfConst()->cloneEmpty();

    const auto &offsets =
        first_array_col_concrete
            ->getOffsets();  /// the internal offsets column of the first array argument (other array arguments have the same offsets)

    /// Find the first row which contains a non-empty array
    ssize_t first_row_with_non_empty_array = 0;
    if (num_elements_in_array_col)
      while (offsets[first_row_with_non_empty_array] == 0) ++first_row_with_non_empty_array;

    /// Build a selector which stores for every array element in the first array argument if the array element is the 0th, 1st, ...
    /// (horizontal) array element in the current row Better explained by an example:
    ...
    for (ssize_t i = 0; i < num_elements_in_array_col; ++i) {
      ...
      while (first_row_with_non_empty_array < num_rows &&
             cur_element_in_cur_array >= offsets[first_row_with_non_empty_array] - offsets[first_row_with_non_empty_array - 1]) {
    ...
    ///       - slice1: 'elem3'
    std::vector<MutableColumns> vertical_slices;  /// contains for every array argument, a vertical slice for the 0th array element, a
                                                  /// vertical slice for the 1st array element, ...
    ...
    for (size_t slice = 0; slice < max_array_size; ++slice) {
      ...
      for (size_t i = 0; i < prev_selector.size(); ++i) filter[i] = static_cast<UInt8>(prev_selector[i]);
      ColumnPtr lambda_col_filtered = lambda_col->filter(filter, lambda_col->size());
      IColumn::MutablePtr lambda_col_filtered_cloned =
          lambda_col_filtered->cloneResized(lambda_col_filtered->size());  /// clone so we can bind more arguments
      ...
      lambda->appendArguments(
          std::vector({ColumnWithTypeAndName(std::move(unfinished_accumulator_values), arguments.back().type, arguments.back().name)}));
      for (size_t array_col = 0; array_col < num_array_cols; ++array_col)
        lambda->appendArguments(
            std::vector({ColumnWithTypeAndName(std::move(vertical_slices[array_col][slice]), arrays_data_with_type_and_name[array_col].type,
                                               arrays_data_with_type_and_name[array_col].name)}));
    ...
    /// We have result_col containing result for every row and inverse_permutation which contains indexes of rows in input it corresponds
    /// to. Now we need to invert inverse_permuation and apply it to result_col to get rows in right order.
    IColumn::Permutation perm(num_rows);
    for (ssize_t row = 0; row < num_rows; ++row) perm[inverse_permutation[row]] = row;
...
REGISTER_FUNCTION(ArrayFold) {
  ...
  FunctionDocumentation::Arguments arguments = {
      {"λ(x, x1 [, x2, x3, ... xN])",
       "A lambda function `λ(acc, x1 [, x2, x3, ... xN]) → F(acc, x1 [, x2, x3, ... xN])` where `F` is an operation applied to `acc` and "
       "array values from `x` with the result of `acc` re-used.",
       {"Lambda function"}},
