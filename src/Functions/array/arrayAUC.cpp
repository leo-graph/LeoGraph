...
class FunctionArrayAUC : public IFunction {
 public:
  ...
  static FunctionPtr create(const char *name, bool is_pr) { return std::make_shared<FunctionArrayAUC>(name, is_pr); }
  ...
  DataTypePtr getReturnTypeImpl(const ColumnsWithTypeAndName &arguments) const override {
    ...
    if ((is_pr && (argument_count < 2 || argument_count > 3)) || (!is_pr && (argument_count < 2 || argument_count > 4)))
      throw Exception(ErrorCodes::NUMBER_OF_ARGUMENTS_DOESNT_MATCH,
                      "Number of arguments for function {} doesn't match: passed {}, should be {}", getName(), argument_count,
    ...
    if (!array_scores_type)
      throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT, "First argument (arr_scores) for function {} must be of type Array", getName());

    const auto &nested_array_scores_type = array_scores_type->getNestedType();
    if (!isNativeNumber(nested_array_scores_type))
      throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT, "First argument (arr_scores) for function {} is of type {}, should be a number",
                      getName(), nested_array_scores_type->getName());
    ...
    if (!array_labels_type)
      throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT, "Second argument (arr_labels) for function {} must be of type Array",
                      getName());
    ...
    if (!isNativeNumber(nested_arary_labels_type) && !isEnum(nested_arary_labels_type))
      throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT,
                      "Second argument (arr_labels) for function {} is of type {}, should be a number or Enum", getName(),
    ...
    if (argument_count >= array_partial_offsets_arg_index + 1) {
      ...
      if (!array_offsets_type)
        throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT, "{} argument (arr_partial_offsets) for function {} must be of type Array",
                        array_partial_offsets_arg_index == 2 ? "Third" : "Fourth", getName());
      ...
      if (!isInteger(nested_array_offsets_type))
        throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT,
                        ...
                        array_partial_offsets_arg_index == 2 ? "Third" : "Fourth", getName(), nested_array_offsets_type->getName());
    }
  ...
  ColumnPtr executeImpl(const ColumnsWithTypeAndName &arguments, const DataTypePtr &, size_t input_rows_count) const override {
    ...
    if (!col_array2)
      throw Exception(ErrorCodes::ILLEGAL_COLUMN, "Illegal column {} of second argument of function {}", arguments[1].column->getName(),
                      getName());
    ...
    bool scale = true;
    if (!is_pr && argument_count >= 3 && input_rows_count > 0) scale = arguments[2].column->getBool(0);
    ...
    if (argument_count == array_partial_offsets_arg_index + 1) {
      ...
      if ((col_array_offsets->getData().size() != array_partial_offsets_size * input_rows_count) || (offsets.size() != input_rows_count))
        throw Exception(ErrorCodes::BAD_ARGUMENTS, "{} argument (arr_partial_offsets) for function {} must contain Arrays of size {}",
                        array_partial_offsets_arg_index == 2 ? "Third" : "Fourth", getName(), array_partial_offsets_size);
      ...
      for (size_t i = 0; i < input_rows_count; ++i) {
        ...
        if (current - previous != array_partial_offsets_size)
          throw Exception(
              ErrorCodes::BAD_ARGUMENTS, "{} argument (arr_partial_offsets) for function {} must contain Arrays of size {}, not {}",
              array_partial_offsets_arg_index == 2 ? "Third" : "Fourth", getName(), array_partial_offsets_size, current - previous);
      }

      for (size_t i = 0; i < col_array_offsets->getData().size(); ++i) {
        if (col_array_offsets->getData().getInt(i) < 0)
          throw Exception(ErrorCodes::BAD_ARGUMENTS, "{} argument (arr_partial_offsets) for function {} must not contain negative values",
                          array_partial_offsets_arg_index == 2 ? "Third" : "Fourth", getName());
      }
    }
 ...
 private:
  ...
  static bool isConstBoolColumn(ColumnWithTypeAndName argument) {
    if (!isBool(argument.type)) return false;
    if (argument.column.get() == nullptr) return false;
    if (!isColumnConst(*argument.column)) return false;
    return true;
  }
  ...
  Float64 apply(const IColumn &scores, const IColumn &labels, ColumnArray::Offset current_offset, ColumnArray::Offset next_offset,
                bool scale, size_t higher_partitions_tp = 0, size_t higher_partitions_fp = 0, size_t total_positives = 0,
    ...
    if (size == 0) return is_pr ? 0.0 : std::numeric_limits<Float64>::quiet_NaN();
  ...
  void vector(const IColumn &scores, const IColumn &labels, const ColumnArray::Offsets &offsets, PaddedPODArray<Float64> &result,
              size_t input_rows_count, bool scale, const ColumnArray *partial_auc_offsets) const {
...
For more details, please see [here](https://developers.google.com/machine-learning/glossary#pr-auc-area-under-the-pr-curve), [here](https://developers.google.com/machine-learning/crash-course/classification/roc-and-auc#expandable-1) and [here](https://en.wikipedia.org/wiki/Receiver_operating_characteristic#Area_under_the_curve).
  ...
  FunctionDocumentation::Arguments arguments_roc = {
      ...
      {"scale",
       "Optional. Decides whether to return the normalized area. If false, returns the area under the TP (true positives) x FP (false "
       "positives) curve instead. Default value: true.",
       {"Bool"}},
      {"partial_offsets", R"(
...
:::
  ...
  FunctionDocumentation::ReturnedValue returned_value_roc = {"Returns area under the receiver operating characteristic (ROC) curve.",
                                                             {"Float64"}};
  ...
  FunctionDocumentation documentation_roc = {description_roc,    syntax_roc,   arguments_roc,     {},
                                             returned_value_roc, examples_roc, introduced_in_roc, category_roc};
...
└─────────────────────────────────────────────────┘
  ...
  FunctionDocumentation documentation_pr = {description_pr,    syntax_pr,   arguments_pr,     {},
                                            returned_value_pr, examples_pr, introduced_in_pr, category_pr};
