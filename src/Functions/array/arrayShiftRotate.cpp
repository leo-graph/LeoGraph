...
enum class ShiftRotateStrategy : uint8_t { Shift, Rotate };

enum class ShiftRotateDirection : uint8_t { Left, Right };
 ...
 public:
  ...
  DataTypePtr getReturnTypeImpl(const DataTypes &arguments) const override {
    ...
    if (!isNativeInteger(arguments[1]))
      throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT, "Illegal type {} of argument of function {}, expected Native Integer",
    ...
    if (arguments.size() == 3) {
      ...
      if (!ret || !ret->equals(*elem_type))
        throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT, "Illegal type {} of argument of function {}, expected {}",
                        arguments[2]->getName(), getName(), elem_type->getName());
    }
  ...
  ColumnPtr executeImpl(const ColumnsWithTypeAndName &arguments, const DataTypePtr &result_type, size_t input_rows_count) const override {
    ...
    if (!column_array) {
      ...
      if (!column_const_array) throw Exception(ErrorCodes::ILLEGAL_COLUMN, "Expected Array column, found {}", column_array_ptr->getName());
...
struct ArrayRotateImpl {
  ...
  static ColumnPtr execute(const ColumnArray &array, ColumnPtr shift_num_column, size_t input_rows_count) {
    ...
    for (size_t i = 0; i < input_rows_count; ++i) {
      ...
      if (nested_size > 0 && shift_num >= nested_size) shift_num %= nested_size;

      // Rotating left to N is the same as shifting right to (size - N).
      if (actual_direction == ShiftRotateDirection::Right) shift_num = nested_size - shift_num;

      for (size_t j = 0; j < nested_size; ++j) permutation[current_offset + j] = current_offset + (j + shift_num) % nested_size;
...
struct ArrayShiftImpl {
  ...
  static ColumnPtr execute(const ColumnArray &array, ColumnPtr shift_column, ColumnPtr default_column, size_t input_column_rows) {
    const IColumn::Offsets &offsets = array.getOffsets();
...
REGISTER_FUNCTION(ArrayShiftOrRotate) {
  FunctionDocumentation::Description description_rotateleft =
      "Rotates an array to the left by the specified number of elements. Negative values of `n` are treated as rotating to the right by "
      "the absolute value of the rotation.";
  ...
  FunctionDocumentation::ReturnedValue returned_value_rotateleft = {"An array rotated to the left by the specified number of elements",
                                                                    {"Array(T)"}};
  ...
  FunctionDocumentation documentation_rotateleft = {
      description_rotateleft,    syntax_rotateleft,   arguments_rotateleft,     {},
      returned_value_rotateleft, examples_rotateleft, introduced_in_rotateleft, category_rotateleft};

  factory.registerFunction<FunctionArrayRotateLeft>(documentation_rotateleft);

  FunctionDocumentation::Description description_rotateright =
      "Rotates an array to the right by the specified number of elements. Negative values of `n` are treated as rotating to the left by "
      "the absolute value of the rotation.";
  ...
  FunctionDocumentation::Arguments arguments_rotateright = {
      {"arr", "The array for which to rotate the elements.[`Array(T)`](/sql-reference/data-types/array)."},
      {"n", "Number of elements to rotate. [`(U)Int8/16/32/64`](/sql-reference/data-types/int-uint)."}};
  FunctionDocumentation::ReturnedValue returned_value_rotateright = {"An array rotated to the right by the specified number of elements",
                                                                     {"Array(T)"}};
  ...
  FunctionDocumentation documentation_rotateright = {
      description_rotateright,    syntax_rotateright,   arguments_rotateright,     {},
      returned_value_rotateright, examples_rotateright, introduced_in_rotateright, category_rotateright};
...
If the number of elements is negative, the array is shifted to the right.
  ...
  FunctionDocumentation::ReturnedValue returned_value_shiftleft = {"An array shifted to the left by the specified number of elements",
                                                                   {"Array(T)"}};
  ...
  FunctionDocumentation documentation_shiftleft = {
      description_shiftleft,    syntax_shiftleft,   arguments_shiftleft,     {},
      returned_value_shiftleft, examples_shiftleft, introduced_in_shiftleft, category_shiftleft};
...
If the number of elements is negative, the array is shifted to the left.
  ...
  FunctionDocumentation::ReturnedValue returned_value_shiftright = {"An array shifted to the right by the specified number of elements",
                                                                    {"Array(T)"}};
  ...
  FunctionDocumentation documentation_shiftright = {
      description_shiftright,    syntax_shiftright,   arguments_shiftright,     {},
      returned_value_shiftright, examples_shiftright, introduced_in_shiftright, category_shiftright};
