...
class FunctionArrayWithConstant : public IFunction {
 public:
  ...
  ColumnPtr executeImpl(const ColumnsWithTypeAndName &arguments, const DataTypePtr &, size_t num_rows) const override {
    ...
    if (num_rows == 0) return ColumnArray::create(col_value->replicate(offsets)->convertToFullColumnIfConst(), std::move(offsets_col));
    ...
    for (size_t i = 0; i < num_rows; ++i) {
      ...
      if (unlikely(array_size < 0))
        throw Exception(ErrorCodes::TOO_LARGE_ARRAY_SIZE, "Array size {} cannot be negative: while executing function {}", array_size,
                        getName());
      ...
      if (unlikely(common::mulOverflow(array_size, element_size, estimated_size)))
        throw Exception(ErrorCodes::TOO_LARGE_ARRAY_SIZE,
                        "Array size {} with element size {} bytes is too large: while executing function {}", array_size, element_size,
                        getName());
      ...
      if (unlikely(offset > max_arrays_size_in_columns))
        throw Exception(ErrorCodes::TOO_LARGE_ARRAY_SIZE,
                        "Too large array size {} (will generate at least {} elements) while executing function {}", array_size, offset,
                        getName());
