 ...
 public:
  ...
  DataTypePtr getReturnTypeImpl(const DataTypes &arguments) const override {
    if (arguments.size() > Traits::max_num_params || arguments.empty()) {
      throw Exception(ErrorCodes::NUMBER_OF_ARGUMENTS_DOESNT_MATCH, "Function '{}' needs from 1 to {} arguments, passed {}.", getName(),
                      Traits::max_num_params, arguments.size());
    }

    const DataTypeArray *array_type = checkAndGetDataType<DataTypeArray>(arguments[0].get());
    if (!array_type) throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT, "First argument of function '{}' must be array", getName());

    auto check_is_integral = [&](auto param_idx) {
      WhichDataType which(arguments[param_idx]);
      if (!which.isUInt() && !which.isInt())
        throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT, "Illegal type {} of arguments of function {} (must be UInt or Int)",
                        arguments[param_idx]->getName(), getName());
    };

    for (size_t idx = 1; idx < arguments.size(); ++idx) check_is_integral(idx);
...
ColumnPtr FunctionArrayShuffleImpl<Traits>::executeImpl(const ColumnsWithTypeAndName &arguments, const DataTypePtr &, size_t) const {
  ...
  const auto seed = [&]() -> uint64_t {
    ...
    if (arguments.size() != Traits::max_num_params) return randomSeed();
...
:::
  ...
  FunctionDocumentation::Arguments arguments = {
      ...
      {"seed (optional)",
       "Optional. The seed to be used with random number generation. If not provided a random one is used.",
       {"(U)Int*"}},
  };
...
:::
  ...
  examples = {{"no_limit1", "SELECT arrayPartialShuffle([1, 2, 3, 4], 0)", "[2, 4, 3, 1]"},
              ...
              {"materialize",
               "SELECT arrayPartialShuffle(materialize([1, 2, 3, 4]), 2, 42), arrayPartialShuffle([1, 2, 3], 2, 42) FROM numbers(10)", R"(
