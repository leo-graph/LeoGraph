...
class FunctionMapToArrayAdapter : public IFunction {
 public:
  ...
  DataTypePtr getReturnTypeImpl(const ColumnsWithTypeAndName &arguments) const override {
    if (arguments.empty())
      throw Exception(ErrorCodes::NUMBER_OF_ARGUMENTS_DOESNT_MATCH, "Function {} requires at least one argument, passed {}", getName(),
                      arguments.size());

    auto nested_arguments = arguments;
    Adapter::extractNestedTypesAndColumns(nested_arguments);

    constexpr bool impl_has_get_return_type = requires { impl.getReturnTypeImpl(nested_arguments); };
    ...
    if constexpr (std::is_same_v<Impl, FunctionArrayMap> || std::is_same_v<Impl, FunctionArrayConcat>) {
      ...
      if (!type_array) throw Exception(ErrorCodes::BAD_ARGUMENTS, "Expected Array(Tuple(key, value)) type, got {}", nested_type->getName());

      const auto *type_tuple = typeid_cast<const DataTypeTuple *>(type_array->getNestedType().get());
      if (!type_tuple) throw Exception(ErrorCodes::BAD_ARGUMENTS, "Expected Array(Tuple(key, value)) type, got {}", nested_type->getName());

      if (type_tuple->getElements().size() != 2)
        throw Exception(ErrorCodes::BAD_ARGUMENTS, "Expected Array(Tuple(key, value)) type, got {}", nested_type->getName());

      /// Recreate nested type with explicitly named tuple.
      return Adapter::wrapType(std::make_shared<DataTypeArray>(
          std::make_shared<DataTypeTuple>(DataTypes{type_tuple->getElement(0), type_tuple->getElement(1)}, Names{"keys", "values"})));
    } else
...
struct MapToNestedAdapter : public MapAdapterBase<MapToNestedAdapter<Name, returns_map>, Name> {
  ...
  static DataTypePtr extractNestedType(const DataTypeMap &type_map) { return type_map.getNestedType(); }

  static ColumnPtr extractNestedColumn(const ColumnMap &column_map) { return column_map.getNestedColumnPtr(); }

  static DataTypePtr extractResultType(const DataTypePtr &result_type) {
    if constexpr (returns_map) return assert_cast<const DataTypeMap &>(*result_type).getNestedType();
    return result_type;
  }

  static DataTypePtr wrapType(DataTypePtr type) {
    if constexpr (returns_map) return std::make_shared<DataTypeMap>(std::move(type));
    return type;
  }

  static ColumnPtr wrapColumn(ColumnPtr column) {
    if constexpr (returns_map) return ColumnMap::create(std::move(column));
    return column;
  }
};
...
struct MapToSubcolumnAdapter {
  ...
  static void extractNestedTypes(DataTypes &types) {
    if (types.empty())
      throw Exception(ErrorCodes::NUMBER_OF_ARGUMENTS_DOESNT_MATCH,
                      "Number of arguments for function {} doesn't match: passed {}, should be at least 1", Name::name, types.size());
  ...
  static void extractNestedTypesAndColumns(ColumnsWithTypeAndName &arguments) {
    if (arguments.empty())
      throw Exception(ErrorCodes::NUMBER_OF_ARGUMENTS_DOESNT_MATCH,
                      "Number of arguments for function {} doesn't match: passed {}, should be at least 1", Name::name, arguments.size());
...
class FunctionMapKeyLike : public IFunction {
 public:
  FunctionMapKeyLike()
      : impl(/*context*/ nullptr) {}  /// nullptr because getting a context here is hard and FunctionLike doesn't need context
...
struct MapLikeAdapter {
  ...
  static void checkTypes(const DataTypes &types) {
    if (types.size() != 2)
      throw Exception(ErrorCodes::NUMBER_OF_ARGUMENTS_DOESNT_MATCH,
                      "Number of arguments for function {} doesn't match: passed {}, should be 2", Name::name, types.size());

    const auto *map_type = checkAndGetDataType<DataTypeMap>(types[0].get());
    if (!map_type) throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT, "First argument for function {} must be a Map", Name::name);
    ...
    if (!isStringOrFixedString(subcolumn_type))
      throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT, "{} type of map for function {} must be String or FixedString",
                      position == 0 ? "Key" : "Value", Name::name);
  }
  ...
  static void extractNestedTypesAndColumns(ColumnsWithTypeAndName &arguments) {
    ...
    ColumnWithTypeAndName function_arg{function_column, function_type,
                                       position == 0 ? "__function_map_key_like" : "__function_map_value_like"};
  ...
  static DataTypePtr wrapType(DataTypePtr type) { return MapToNestedAdapter<Name, returns_map>::wrapType(std::move(type)); }

  static ColumnPtr wrapColumn(ColumnPtr column) { return MapToNestedAdapter<Name, returns_map>::wrapColumn(std::move(column)); }
};
...
using FunctionMapContainsKey = FunctionMapToArrayAdapter<FunctionArrayIndex<HasAction, NameMapContainsKey>,
                                                         MapToSubcolumnAdapter<NameMapContainsKey, 0>, NameMapContainsKey>;

struct NameMapContainsValue {
  static constexpr auto name = "mapContainsValue";
};
using FunctionMapContainsValue = FunctionMapToArrayAdapter<FunctionArrayIndex<HasAction, NameMapContainsValue>,
                                                           MapToSubcolumnAdapter<NameMapContainsValue, 1>, NameMapContainsValue>;
...
using FunctionMapContainsKeyLike =
...
using FunctionMapContainsValueLike =
...
using FunctionMapExtractKeyLike =
...
using FunctionMapExtractValueLike =
...
using FunctionMapReverseSort =
    FunctionMapToArrayAdapter<FunctionArrayReverseSort, MapToNestedAdapter<NameMapReverseSort>, NameMapReverseSort>;
using FunctionMapPartialSort =
    FunctionMapToArrayAdapter<FunctionArrayPartialSort, MapToNestedAdapter<NameMapPartialSort>, NameMapPartialSort>;
using FunctionMapPartialReverseSort =
    FunctionMapToArrayAdapter<FunctionArrayPartialReverseSort, MapToNestedAdapter<NameMapPartialReverseSort>, NameMapPartialReverseSort>;
...
If elements with the same key exist in more than one input map, all elements are added to the result map, but only the first one is accessible via operator [].
  ...
  FunctionDocumentation::Arguments arguments_mapConcat = {{"maps", "Arbitrarily many maps.", {"Map"}}};
  ...
  FunctionDocumentation::Examples examples_mapConcat = {
      {"Usage example", "SELECT mapConcat(map('k1', 'v1'), map('k2', 'v2'))", "{'k1':'v1','k2':'v2'}"}};
  ...
  FunctionDocumentation documentation_mapConcat = {
      description_mapConcat,    syntax_mapConcat,   arguments_mapConcat,     {},
      returned_value_mapConcat, examples_mapConcat, introduced_in_mapConcat, category_mapConcat};
...
The query `SELECT mapKeys(m) FROM table` is transformed to `SELECT m.keys FROM table`.
  ...
  FunctionDocumentation::Arguments arguments_mapKeys = {{"map", "Map to extract keys from.", {"Map(K, V)"}}};
  FunctionDocumentation::ReturnedValue returned_value_mapKeys = {"Returns array containing all keys from the map.", {"Array(T)"}};
  FunctionDocumentation::Examples examples_mapKeys = {{"Usage example", "SELECT mapKeys(map('k1', 'v1', 'k2', 'v2'))", "['k1','k2']"}};
  ...
  FunctionDocumentation documentation_mapKeys = {description_mapKeys,    syntax_mapKeys,   arguments_mapKeys,     {},
                                                 returned_value_mapKeys, examples_mapKeys, introduced_in_mapKeys, category_mapKeys};
...
The query `SELECT mapValues(m) FROM table` is transformed to `SELECT m.values FROM table`.
  ...
  FunctionDocumentation::Arguments arguments_mapValues = {{"map", "Map to extract values from.", {"Map(K, V)"}}};
  FunctionDocumentation::ReturnedValue returned_value_mapValues = {"Returns an array containing all the values from the map.",
                                                                   {"Array(T)"}};
  FunctionDocumentation::Examples examples_mapValues = {{"Usage example", "SELECT mapValues(map('k1', 'v1', 'k2', 'v2'))", "['v1','v2']"}};
  ...
  FunctionDocumentation documentation_mapValues = {
      description_mapValues,    syntax_mapValues,   arguments_mapValues,     {},
      returned_value_mapValues, examples_mapValues, introduced_in_mapValues, category_mapValues};
...
Determines if a key is contained in a map.
  ...
  FunctionDocumentation::Examples examples_mapContainsKey = {
      {"Usage example", "SELECT mapContainsKey(map('k1', 'v1', 'k2', 'v2'), 'k1')", "1"}};
  ...
  FunctionDocumentation documentation_mapContainsKey = {
      description_mapContainsKey,    syntax_mapContainsKey,   arguments_mapContainsKey,     {},
      returned_value_mapContainsKey, examples_mapContainsKey, introduced_in_mapContainsKey, category_mapContainsKey};
...
Determines if a value is contained in a map.
  ...
  FunctionDocumentation::ReturnedValue returned_value_mapContainsValue = {"Returns `1` if the map contains the value, `0` if not.",
                                                                          {"UInt8"}};
  FunctionDocumentation::Examples examples_mapContainsValue = {
      {"Usage example", "SELECT mapContainsValue(map('k1', 'v1', 'k2', 'v2'), 'v1')", "1"}};
  ...
  FunctionDocumentation documentation_mapContainsValue = {
      description_mapContainsValue,    syntax_mapContainsValue,   arguments_mapContainsValue,     {},
      returned_value_mapContainsValue, examples_mapContainsValue, introduced_in_mapContainsValue, category_mapContainsValue};
...
Filters a map by applying a function to each map element.
  ...
  FunctionDocumentation::Arguments arguments_mapFilter = {{"func", "Lambda function.", {"Lambda function"}},
                                                          {"map", "Map to filter.", {"Map(K, V)"}}};
  FunctionDocumentation::ReturnedValue returned_value_mapFilter = {
  ...
  FunctionDocumentation::Examples examples_mapFilter = {
      {"Usage example", "SELECT mapFilter((k, v) -> v > 1, map('k1', 1, 'k2', 2))", "{'k2':2}"}};
  ...
  FunctionDocumentation documentation_mapFilter = {
      description_mapFilter,    syntax_mapFilter,   arguments_mapFilter,     {},
      returned_value_mapFilter, examples_mapFilter, introduced_in_mapFilter, category_mapFilter};
...
Applies a function to each element of a map.
  ...
  FunctionDocumentation::Arguments arguments_mapApply = {{"func", "Lambda function.", {"Lambda function"}},
                                                         {"map", "Map to apply function to.", {"Map(K, V)"}}};
  FunctionDocumentation::ReturnedValue returned_value_mapApply = {
  ...
  FunctionDocumentation::Examples examples_mapApply = {
      {"Usage example", "SELECT mapApply((k, v) -> (k, v * 2), map('k1', 1, 'k2', 2))", "{'k1':2,'k2':4}"}};
  ...
  FunctionDocumentation documentation_mapApply = {description_mapApply,    syntax_mapApply,   arguments_mapApply,     {},
                                                  returned_value_mapApply, examples_mapApply, introduced_in_mapApply, category_mapApply};
...
You can pass a lambda function to it as the first argument.
  ...
  FunctionDocumentation::Arguments arguments_mapExists = {{"func", "Optional. Lambda function.", {"Lambda function"}},
                                                          {"map", "Map to check.", {"Map(K, V)"}}};
  FunctionDocumentation::ReturnedValue returned_value_mapExists = {
      "Returns `1` if at least one key-value pair satisfies the condition, `0` otherwise.", {"UInt8"}};
  FunctionDocumentation::Examples examples_mapExists = {{"Usage example", "SELECT mapExists((k, v) -> v = 1, map('k1', 1, 'k2', 2))", "1"}};
  ...
  FunctionDocumentation documentation_mapExists = {
      description_mapExists,    syntax_mapExists,   arguments_mapExists,     {},
      returned_value_mapExists, examples_mapExists, introduced_in_mapExists, category_mapExists};
...
You can pass a lambda function to it as the first argument.
  ...
  FunctionDocumentation::Arguments arguments_mapAll = {{"func", "Lambda function.", {"Lambda function"}},
                                                       {"map", "Map to check.", {"Map(K, V)"}}};
  FunctionDocumentation::ReturnedValue returned_value_mapAll = {"Returns `1` if all key-value pairs satisfy the condition, `0` otherwise.",
                                                                {"UInt8"}};
  FunctionDocumentation::Examples examples_mapAll = {{"Usage example", "SELECT mapAll((k, v) -> v = 1, map('k1', 1, 'k2', 2))", "0"}};
  ...
  FunctionDocumentation documentation_mapAll = {description_mapAll,    syntax_mapAll,   arguments_mapAll,     {},
                                                returned_value_mapAll, examples_mapAll, introduced_in_mapAll, category_mapAll};
...
If the func function is specified, the sorting order is determined by the result of the func function applied to the keys and values of the map.
  ...
  FunctionDocumentation::Arguments arguments_mapSort = {{"func", "Optional. Lambda function.", {"Lambda function"}},
  ...
  FunctionDocumentation::Examples examples_mapSort = {
      {"Usage example", "SELECT mapSort((k, v) -> v, map('k1', 3, 'k2', 1, 'k3', 2))", "{'k2':1,'k3':2,'k1':3}"}};
  ...
  FunctionDocumentation documentation_mapSort = {description_mapSort,    syntax_mapSort,   arguments_mapSort,     {},
                                                 returned_value_mapSort, examples_mapSort, introduced_in_mapSort, category_mapSort};
...
If the func function is specified, the sorting order is determined by the result of the func function applied to the keys and values of the map.
  ...
  FunctionDocumentation::Arguments arguments_mapReverseSort = {{"func", "Optional. Lambda function.", {"Lambda function"}},
                                                               {"map", "Map to sort.", {"Map(K, V)"}}};
  ...
  FunctionDocumentation::Examples examples_mapReverseSort = {
      {"Usage example", "SELECT mapReverseSort((k, v) -> v, map('k1', 3, 'k2', 1, 'k3', 2))", "{'k1':3,'k3':2,'k2':1}"}};
  ...
  FunctionDocumentation documentation_mapReverseSort = {
      description_mapReverseSort,    syntax_mapReverseSort,   arguments_mapReverseSort,     {},
      returned_value_mapReverseSort, examples_mapReverseSort, introduced_in_mapReverseSort, category_mapReverseSort};
...
If the func function is specified, the sorting order is determined by the result of the func function applied to the keys and values of the map.
  ...
  FunctionDocumentation::Arguments arguments_mapPartialSort = {{"func", "Optional. Lambda function.", {"Lambda function"}},
  ...
  FunctionDocumentation::Examples examples_mapPartialSort = {
      {"Usage example", "SELECT mapPartialSort((k, v) -> v, 2, map('k1', 3, 'k2', 1, 'k3', 2))", "{'k2':1,'k3':2,'k1':3}"}};
  ...
  FunctionDocumentation documentation_mapPartialSort = {
      description_mapPartialSort,    syntax_mapPartialSort,   arguments_mapPartialSort,     {},
      returned_value_mapPartialSort, examples_mapPartialSort, introduced_in_mapPartialSort, category_mapPartialSort};
...
If the func function is specified, the sorting order is determined by the result of the func function applied to the keys and values of the map.
  ...
  FunctionDocumentation::Arguments arguments_mapPartialReverseSort = {
      ...
      {"limit", "Elements in the range `[1..limit]` are sorted.", {"(U)Int*"}},
      {"map", "Map to sort.", {"Map(K, V)"}}};
  FunctionDocumentation::ReturnedValue returned_value_mapPartialReverseSort = {"Returns a partially sorted map in descending order.",
                                                                               {"Map(K, V)"}};
  FunctionDocumentation::Examples examples_mapPartialReverseSort = {
      {"Usage example", "SELECT mapPartialReverseSort((k, v) -> v, 2, map('k1', 3, 'k2', 1, 'k3', 2))", "{'k1':3,'k3':2,'k2':1}"}};
  ...
  FunctionDocumentation documentation_mapPartialReverseSort = {description_mapPartialReverseSort,    syntax_mapPartialReverseSort,
                                                               arguments_mapPartialReverseSort,      {},
                                                               returned_value_mapPartialReverseSort, examples_mapPartialReverseSort,
                                                               introduced_in_mapPartialReverseSort,  category_mapPartialReverseSort};
...
Checks whether map contains key `LIKE` specified pattern.
  ...
  FunctionDocumentation::Arguments arguments_mapContainsKeyLike = {{"map", "Map to search in.", {"Map(K, V)"}},
                                                                   {"pattern", "Pattern to match keys against.", {"const String"}}};
  FunctionDocumentation::ReturnedValue returned_value_mapContainsKeyLike = {
      "Returns `1` if `map` contains a key matching `pattern`, `0` otherwise.", {"UInt8"}};
  FunctionDocumentation::Examples examples_mapContainsKeyLike = {{"Usage example",
                                                                  R"(
...
└─────────────────────────────┘
  ...
  FunctionDocumentation documentation_mapContainsKeyLike = {
      description_mapContainsKeyLike,    syntax_mapContainsKeyLike,   arguments_mapContainsKeyLike,     {},
      returned_value_mapContainsKeyLike, examples_mapContainsKeyLike, introduced_in_mapContainsKeyLike, category_mapContainsKeyLike};
...
Checks whether a map contains a value `LIKE` the specified pattern.
  ...
  FunctionDocumentation::Arguments arguments_mapContainsValueLike = {{"map", "Map to search in.", {"Map(K, V)"}},
                                                                     {"pattern", "Pattern to match values against.", {"const String"}}};
  FunctionDocumentation::ReturnedValue returned_value_mapContainsValueLike = {
      "Returns `1` if `map` contains a value matching `pattern`, `0` otherwise.", {"UInt8"}};
  FunctionDocumentation::Examples examples_mapContainsValueLike = {{"Usage example",
                                                                    R"(
CREATE TABLE tab (a Map(String, String))
ENGINE = MergeTree
ORDER BY tuple();
...
└──────────────────────────┘
  ...
  FunctionDocumentation documentation_mapContainsValueLike = {description_mapContainsValueLike,    syntax_mapContainsValueLike,
                                                              arguments_mapContainsValueLike,      {},
                                                              returned_value_mapContainsValueLike, examples_mapContainsValueLike,
                                                              introduced_in_mapContainsValueLike,  category_mapContainsValueLike};
...
Give a map with string keys and a `LIKE` pattern, this function returns a map with elements where the key matches the pattern.
  ...
  FunctionDocumentation::Arguments arguments_mapExtractKeyLike = {{"map", "Map to extract from.", {"Map(K, V)"}},
                                                                  {"pattern", "Pattern to match keys against.", {"const String"}}};
  FunctionDocumentation::ReturnedValue returned_value_mapExtractKeyLike = {
      "Returns a map containing elements the key matching the specified pattern. If no elements match the pattern, an empty map is "
      "returned.",
      {"Map(K, V)"}};
  FunctionDocumentation::Examples examples_mapExtractKeyLike = {{"Usage example",
                                                                 R"(
CREATE TABLE tab (a Map(String, String))
ENGINE = MergeTree
ORDER BY tuple();
...
└────────────────────────────┘
  ...
  FunctionDocumentation documentation_mapExtractKeyLike = {
      description_mapExtractKeyLike,    syntax_mapExtractKeyLike,   arguments_mapExtractKeyLike,     {},
      returned_value_mapExtractKeyLike, examples_mapExtractKeyLike, introduced_in_mapExtractKeyLike, category_mapExtractKeyLike};
...
Given a map with string values and a `LIKE` pattern, this function returns a map with elements where the value matches the pattern.
  ...
  FunctionDocumentation::Arguments arguments_mapExtractValueLike = {{"map", "Map to extract from.", {"Map(K, V)"}},
                                                                    {"pattern", "Pattern to match values against.", {"const String"}}};
  FunctionDocumentation::ReturnedValue returned_value_mapExtractValueLike = {
      "Returns a map containing elements the value matching the specified pattern. If no elements match the pattern, an empty map is "
      "returned.",
      {"Map(K, V)"}};
  FunctionDocumentation::Examples examples_mapExtractValueLike = {{"Usage example",
                                                                   R"(
CREATE TABLE tab (a Map(String, String))
ENGINE = MergeTree
ORDER BY tuple();
...
└──────────────────────────────┘
  ...
  FunctionDocumentation documentation_mapExtractValueLike = {
      description_mapExtractValueLike,    syntax_mapExtractValueLike,   arguments_mapExtractValueLike,     {},
      returned_value_mapExtractValueLike, examples_mapExtractValueLike, introduced_in_mapExtractValueLike, category_mapExtractValueLike};
