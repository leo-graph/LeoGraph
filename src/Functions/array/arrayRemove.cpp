...
ColumnPtr BuildAndExecuteFunction(const std::string &func_name, const ColumnsWithTypeAndName &func_args, ContextPtr context,
...
ColumnPtr FunctionArrayRemove::executeImpl(const ColumnsWithTypeAndName &arguments, const DataTypePtr &return_type,
  ...
  if (!arr_col)
    throw Exception(ErrorCodes::ILLEGAL_COLUMN, "First argument for function {} must be Array, got {}", getName(),
  ...
  if (elem_type->onlyNull()) {
    ...
    filter_col =
        BuildAndExecuteFunction("not", {{is_arr_null_col, std::make_shared<DataTypeUInt8>(), "is_null_res"}}, context, arr_elements_count);
    ...
    filter_col = BuildAndExecuteFunction(
        "notEquals", {{arr_data_col, arr_data_type, "arr"}, {arguments[1].column->replicate(arr_offsets), elem_type, "elem"}}, context,
        arr_elements_count);
  } else {
    // Filter = NOT if(
  ...
  if (!filter_col_uint8)
    throw Exception(ErrorCodes::LOGICAL_ERROR, "Filter column for arrayRemove was not evaluated as ColumnUInt8 but as {}",
                    filter_col->getDataType());
...
NULLs are treated as equal.
  ...
  FunctionDocumentation::Examples examples = {{"Example 1", "SELECT arrayRemove([1, 2, 2, 3], 2)", "[1, 3]"},
  ...
  FunctionDocumentation documentation = {description, syntax,        {{"arr", "Array(T)"}, {"elem", "T"}},  {}, returned_value,
                                         examples,    introduced_in, FunctionDocumentation::Category::Array};
