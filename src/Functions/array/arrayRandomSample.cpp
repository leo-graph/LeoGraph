...
class FunctionArrayRandomSample : public IFunction {
 public:
  ...
  ColumnPtr executeImpl(const ColumnsWithTypeAndName &arguments, const DataTypePtr &, size_t input_rows_count) const override {
    ...
    if (!col_array) throw Exception(ErrorCodes::ILLEGAL_COLUMN, "First argument of function {} must be an array", getName());

    const IColumn *col_samples = arguments[1].column.get();
    if (!col_samples)
      throw Exception(ErrorCodes::ILLEGAL_COLUMN, "The second argument of function {} is empty or null, type = {}", getName(),
    ...
    for (size_t row = 0; row < input_rows_count; row++) {
      ...
      for (UInt64 i = 0; i < cur_samples; i++) col_res->getData().insertFrom(col_array->getData(), indices[i]);
...
REGISTER_FUNCTION(ArrayRandomSample) {
  FunctionDocumentation::Description description =
      "Returns a subset with `samples`-many random elements of an input array. If `samples` exceeds the size of the input array, the "
      "sample size is limited to the size of the array, i.e. all array elements are returned but their order is not guaranteed. The "
      "function can handle both flat arrays and nested arrays.";
  ...
  FunctionDocumentation::ReturnedValue returned_value = {"An array containing a random sample of elements from the input array",
                                                         {"Array(T)"}};
