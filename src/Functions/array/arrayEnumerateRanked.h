...
class FunctionArrayEnumerateRankedExtended : public IFunction {
 public:
  ...
  DataTypePtr getReturnTypeImpl(const ColumnsWithTypeAndName &arguments) const override {
    ...
    for (DepthType i = 0; i < arrays_depths.max_array_depth; ++i) type = std::make_shared<DataTypeArray>(type);
 ...
 private:
  ...
  void executeMethodImpl(const std::vector<const ColumnArray::Offsets *> &offsets_by_depth, const ColumnRawPtrs &columns,
                         const ArraysDepths &arrays_depths, ColumnUInt32::Container &res_values) const;
};
...
template <typename Derived>
ColumnPtr FunctionArrayEnumerateRankedExtended<Derived>::executeImpl(const ColumnsWithTypeAndName &arguments, const DataTypePtr &,
  ...
  for (size_t i = 0; i < num_arguments; ++i) {
    ...
    if (array_num == 0)  // TODO check with prev
    {
      ...
      if (*offsets_by_depth[0] != array->getOffsets()) {
        throw Exception(ErrorCodes::SIZES_OF_ARRAYS_DONT_MATCH, "Lengths and effective depths of all arrays passed to {} must be equal.",
                        getName());
      }
    }
    ...
    if (col_depth < arrays_depths.depths[array_num]) {
      throw Exception(ErrorCodes::SIZES_OF_ARRAYS_DONT_MATCH,
                      "{}: Passed array number {} depth ({}) is more than the actual array depth ({})", getName(), array_num,
                      std::to_string(arrays_depths.depths[array_num]), col_depth);
    }
  ...
  if (offsets_by_depth.empty()) throw Exception(ErrorCodes::BAD_ARGUMENTS, "No arrays passed to function {}", getName());
...
template <typename Derived>
void FunctionArrayEnumerateRankedExtended<Derived>::executeMethodImpl(const std::vector<const ColumnArray::Offsets *> &offsets_by_depth,
                                                                      const ColumnRawPtrs &columns, const ArraysDepths &arrays_depths,
  ...
  using Container = ClearableHashMapWithStackMemory<UInt128, UInt32, UInt128TrivialHash, INITIAL_SIZE_DEGREE>;
  ...
  for (size_t off : offsets) {
    ...
    for (size_t j = prev_off; j < off; ++j) {
      for (size_t col_n = 0; col_n < columns.size(); ++col_n) columns_indices[col_n] = indices_by_depth[arrays_depths.depths[col_n] - 1];
      ...
      for (ssize_t depth = depth_to_look - 1; depth >= 0; --depth) {
        ...
        if (indices_by_depth[depth] == (*offsets_by_depth[depth])[current_offset_n_by_depth[depth]]) {
          if (static_cast<int>(arrays_depths.clear_depth) == depth + 1) want_clear = true;
          last_offset_by_depth[depth] = (*offsets_by_depth[depth])[current_offset_n_by_depth[depth]];
          ++current_offset_n_by_depth[depth];
        } else {
          break;
