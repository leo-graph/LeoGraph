...
class FunctionArrayShingles : public IFunction {
 public:
  ...
  DataTypePtr getReturnTypeImpl(const ColumnsWithTypeAndName& arguments) const override {
    FunctionArgumentDescriptors args{{"array", static_cast<FunctionArgumentDescriptor::TypeValidator>(&isArray), nullptr, "Array"},
  ...
  ColumnPtr executeImpl(const ColumnsWithTypeAndName& arguments, const DataTypePtr& /*result_type*/,
    ...
    if (!col_array) throw Exception(ErrorCodes::ILLEGAL_COLUMN, "Expected array column for function {}", getName());
    ...
    return ColumnArray::create(ColumnArray::create(std::move(col_res_data), std::move(col_res_inner_offsets)),
...
REGISTER_FUNCTION(ArrayShingles) {
  FunctionDocumentation::Description description =
      ...
      "shingles"
      " (similar to ngrams for strings), i.e. consecutive sub-arrays with a specified length of the input array.";
