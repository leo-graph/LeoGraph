...
ColumnPtr executeConst(const ColumnsWithTypeAndName &arguments, const DataTypePtr &result_type, size_t input_rows_count) {
  if (const ColumnConst *const_array = checkAndGetColumnConst<ColumnArray>(arguments[0].column.get())) {
    if (const_array->getValue<Array>().empty()) {
      ...
      return result_type->createColumnConst(input_rows_count, Array{nested_type->getDefault()});
    }
...
bool executeNumber(const IColumn &src_data, const ColumnArray::Offsets &src_offsets, IColumn &res_data_col,
                   ColumnArray::Offsets &res_offsets, const NullMap *src_null_map, NullMap *res_null_map) {
  if (const ColumnVector<T> *src_data_concrete = checkAndGetColumn<ColumnVector<T>>(&src_data)) {
    ...
    if (nullable) res_null_map->reserve(src_null_map->size());
    ...
    for (size_t i = 0; i < size; ++i) {
      if (src_offsets[i] != src_prev_offset) {
        ...
        if (nullable) res_null_map->push_back(static_cast<uint8_t>(1));  /// Push NULL.
      }
...
bool executeFixedString(const IColumn &src_data, const ColumnArray::Offsets &src_offsets, IColumn &res_data_col,
                        ColumnArray::Offsets &res_offsets, const NullMap *src_null_map, NullMap *res_null_map) {
  if (const ColumnFixedString *src_data_concrete = checkAndGetColumn<ColumnFixedString>(&src_data)) {
    ...
    if (!concrete_res_data) throw Exception(ErrorCodes::LOGICAL_ERROR, "Internal error");
    ...
    for (size_t i = 0; i < size; ++i) {
      if (src_offsets[i] != src_prev_offset) {
        ...
        if (nullable) res_null_map->push_back(ColumnNullable::IS_NULL_MASK);
      }
...
template <bool nullable>
bool executeString(const IColumn &src_data, const ColumnArray::Offsets &src_array_offsets, IColumn &res_data_col,
                   ColumnArray::Offsets &res_array_offsets, const NullMap *src_null_map, NullMap *res_null_map) {
  if (const ColumnString *src_data_concrete = checkAndGetColumn<ColumnString>(&src_data)) {
    ...
    if (!concrete_res_string_offsets) throw Exception(ErrorCodes::LOGICAL_ERROR, "Internal error");
    ...
    for (size_t i = 0; i < size; ++i) {
      ...
      if (src_array_prev_offset) src_string_prev_offset = src_string_offsets[src_array_prev_offset - 1];
    }
...
template <bool nullable>
void executeGeneric(const IColumn &src_data, const ColumnArray::Offsets &src_offsets, IColumn &res_data, ColumnArray::Offsets &res_offsets,
                    const NullMap *src_null_map, NullMap *res_null_map) {
  size_t size = src_offsets.size();
  res_offsets.resize(size);
...
template <bool nullable>
void executeDispatch(const IColumn &src_data, const ColumnArray::Offsets &src_array_offsets, IColumn &res_data_col,
                     ColumnArray::Offsets &res_array_offsets, const NullMap *src_null_map, NullMap *res_null_map) {
...
DataTypePtr FunctionEmptyArrayToSingle::getReturnTypeImpl(const DataTypes &arguments) const {
  ...
  if (!array_type) throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT, "Argument for function {} must be array.", getName());

  return arguments[0];
}

ColumnPtr FunctionEmptyArrayToSingle::executeImpl(const ColumnsWithTypeAndName &arguments, const DataTypePtr &result_type,
                                                  size_t input_rows_count) const {
  if (auto res = FunctionEmptyArrayToSingleImpl::executeConst(arguments, result_type, input_rows_count)) return res;
  ...
  if (nullable_col)
    ...
    FunctionEmptyArrayToSingleImpl::executeDispatch<false>(*inner_col, src_offsets, *inner_res_col, res_offsets, src_null_map,
                                                           res_null_map);
