...
struct Impl {
  ...
  static void vectorConst(const PaddedPODArray<T1> &array_predicted_data, const ColumnArray::Offsets &array_predicted_offsets,
                          const PaddedPODArray<T2> &array_labels_const, PaddedPODArray<Float64> &col_gini_predicted,
                          PaddedPODArray<Float64> &col_gini_labels, PaddedPODArray<Float64> &col_gini_normalized) {
    size_t size = col_gini_predicted.size();
    size_t array_size = array_labels_const.size();

    if (array_size > MAX_ARRAY_SIZE)
      throw Exception(ErrorCodes::TOO_LARGE_ARRAY_SIZE, "Too large array size in function arrayNormalizedGini: {}, maximum: {}", array_size,
                      MAX_ARRAY_SIZE);
    ...
    for (size_t i = 0; i < size; ++i) {
      ...
      auto [gini_predicted, gini_labels, gini_normalized] =
          calculateNormalizedGini(array_predicted_data, array_predicted_offsets[i - 1], array2, array_size);
  ...
  template <typename T1, typename T2>
  static void vectorVector(const PaddedPODArray<T1> &array_predicted_data, const ColumnArray::Offsets &array_predicted_offsets,
                           const PaddedPODArray<T2> &array_labels_data, const ColumnArray::Offsets &array_labels_offsets,
                           PaddedPODArray<Float64> &col_gini_predicted, PaddedPODArray<Float64> &col_gini_labels,
                           PaddedPODArray<Float64> &col_gini_normalized) {
    size_t size = col_gini_predicted.size();
    size_t array_size = size > 0 ? array_predicted_offsets[0] - array_predicted_offsets[-1] : 0;

    if (array_size > MAX_ARRAY_SIZE)
      throw Exception(ErrorCodes::TOO_LARGE_ARRAY_SIZE, "Too large array size in arrayNormalizedGini: {}, maximum: {}", array_size,
                      MAX_ARRAY_SIZE);
    ...
    for (size_t i = 0; i < size; ++i) {
      ...
      PODArrayWithStackMemory<T2, 1024> array2(array_labels_data.data() + array_labels_offsets[i - 1],
                                               array_labels_data.data() + array_labels_offsets[i]);
  ...
  template <typename T1, typename T2>
  static void constVector(const PaddedPODArray<T1> &array_predicted_const, const PaddedPODArray<T2> &array_labels_data,
                          const ColumnArray::Offsets &array_labels_offsets, PaddedPODArray<Float64> &col_gini_predicted,
                          PaddedPODArray<Float64> &col_gini_labels, PaddedPODArray<Float64> &col_gini_normalized) {
    size_t size = col_gini_predicted.size();
 ...
 private:
  template <typename T1, typename T2>
  static std::tuple<Float64, Float64, Float64> calculateNormalizedGini(const PaddedPODArray<T1> &array1, size_t offset,
    ...
    for (size_t i = 0; i < array_size; ++i) sorted_array2[i] = array2[sort_idx[i]];
...
class FunctionArrayNormalizedGini : public IFunction {
 public:
  ...
  DataTypePtr getReturnTypeImpl(const DataTypes &arguments) const override {
    ...
    if (arg1_type == nullptr || !isNumber(arg1_type->getNestedType()))
      throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT, "First argument for function {} must be an Array of numeric type, got {}",
    ...
    if (arg2_type == nullptr || !isNumber(arg2_type->getNestedType()))
      throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT, "Second argument for function {} must be an Array of numeric typegot {}",
  ...
  ColumnPtr executeImpl(const ColumnsWithTypeAndName &arguments, const DataTypePtr &, size_t input_rows_count) const override {
    ...
    Columns result(3);
    for (size_t i = 0; i < 3; ++i) result[i] = DataTypeFloat64().createColumn();
    ...
    if (const ColumnArray *array_predicted = checkAndGetColumn<ColumnArray>(col_predicted.get())) {
      ...
      if (const ColumnConst *array_labels_const = checkAndGetColumn<ColumnConst>(col_labels.get())) {
        ...
        if (castBothTypes(array_predicted_type.get(), array_labels_type.get(), [&](const auto &type_predicted, const auto &type_labels) {
              using TypePredicted = typename std::decay_t<decltype(type_predicted)>::FieldType;
              const ColumnVector<TypePredicted> *array_predicted_data =
                  checkAndGetColumn<ColumnVector<TypePredicted>>(array_predicted->getDataPtr().get());

              using TypeLabels = typename std::decay_t<decltype(type_labels)>::FieldType;
              const ColumnVector<TypeLabels> *col_labels_data =
                  checkAndGetColumn<ColumnVector<TypeLabels>>(column_array_const_internal_array->getDataPtr().get());
              ...
              Impl::vectorConst(array_predicted_data->getData(), array_predicted_offsets, col_labels_data->getData(),
                                col_gini_predicted->getData(), col_gini_labels->getData(), col_gini_normalized->getData());
        ...
        if (castBothTypes(array_predicted_type.get(), array_labels_type.get(), [&](const auto &type_predicted, const auto &type_labels) {
              ...
              using TypeLabels = typename std::decay_t<decltype(type_labels)>::FieldType;
              const ColumnVector<TypeLabels> *col_labels_data =
                  checkAndGetColumn<ColumnVector<TypeLabels>>(array_labels->getDataPtr().get());

              auto col_gini_predicted = ColumnFloat64::create(input_rows_count);
              auto col_gini_labels = ColumnFloat64::create(input_rows_count);
              auto col_gini_normalized = ColumnFloat64::create(input_rows_count);

              Impl::vectorVector(array_predicted_data->getData(), array_predicted_offsets, col_labels_data->getData(), array_label_offsets,
                                 col_gini_predicted->getData(), col_gini_labels->getData(), col_gini_normalized->getData());
      ...
      if (castBothTypes(array_predicted_type.get(), array_labels_type.get(), [&](const auto &type_predicted, const auto &type_labels) {
            using TypePredicted = typename std::decay_t<decltype(type_predicted)>::FieldType;
            const ColumnVector<TypePredicted> *array_predicted_data =
                checkAndGetColumn<ColumnVector<TypePredicted>>(column_array_const->getDataPtr().get());
            ...
            Impl::constVector(array_predicted_data->getData(), col_labels_data->getData(), array_label_offsets,
                              col_gini_predicted->getData(), col_gini_labels->getData(), col_gini_normalized->getData());
 ...
 private:
  ...
  static bool castType(const IDataType *type, F &&f) {
    return castTypeToEither<DataTypeInt8, DataTypeInt16, DataTypeInt32, DataTypeInt64, DataTypeUInt8, DataTypeUInt16, DataTypeUInt32,
                            DataTypeUInt64, DataTypeFloat32, DataTypeFloat64>(type, std::forward<F>(f));
  }
...
REGISTER_FUNCTION(NormalizedGini) {
  ...
  FunctionDocumentation::Arguments doc_arguments = {{"predicted", "The predicted value.", {"Array(T)"}},
                                                    {"label", "The actual value.", {"Array(T)"}}};
  FunctionDocumentation::ReturnedValue doc_returned_value = {
      "A tuple containing the Gini coefficients of the predicted values, the Gini coefficient of the normalized values, and the normalized "
      "Gini coefficient (= the ratio of the former two Gini coefficients)",
      {"Tuple(Float64, Float64, Float64)"}};
  FunctionDocumentation::Examples doc_examples = {{"Usage example", "SELECT arrayNormalizedGini([0.9, 0.3, 0.8, 0.7],[6, 1, 0, 2]);",
  ...
  FunctionDocumentation documentation = {doc_description,    doc_syntax,   doc_arguments,     {},
                                         doc_returned_value, doc_examples, doc_introduced_in, doc_category};
