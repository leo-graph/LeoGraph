...
enum class OpTypes : uint8_t { ADD = 0, SUBTRACT = 1 };
...
class FunctionMapOp : public IFunction {
 ...
 private:
  ...
  void checkTypes(DataTypePtr &key_type, DataTypePtr &promoted_val_type, const DataTypePtr &check_key_type,
                  DataTypePtr &check_val_type) const {
    if (!(check_key_type->equals(*key_type)))
      throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT, "Expected same {} type for all keys in {}", key_type->getName(), getName());
    ...
    if (which_ch_val.isFloat() != which_val.isFloat())
      throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT, "All value types in {} should be either or float or integer", getName());

    if (!(check_val_type->equals(*promoted_val_type))) {
      throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT, "All value types in {} should be promotable to {}, got {}", getName(),
                      promoted_val_type->getName(), check_val_type->getName());
    }
  }
  ...
  DataTypePtr getReturnTypeForTuples(const DataTypes &arguments) const {
    ...
    for (const auto &arg : arguments) {
      ...
      if (!tup) throw Exception(ErrorCodes::NUMBER_OF_ARGUMENTS_DOESNT_MATCH, "{} accepts at least two map tuples", getName());

      auto elems = tup->getElements();
      if (elems.size() != 2)
        throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT, "Each tuple in {} arguments should consist of two arrays", getName());
  ...
  DataTypePtr getReturnTypeForMaps(const DataTypes &arguments) const {
    ...
    for (const auto &arg : arguments) {
      ...
      if (!map) throw Exception(ErrorCodes::NUMBER_OF_ARGUMENTS_DOESNT_MATCH, "{} accepts at least two maps", getName());
  ...
  DataTypePtr getReturnTypeImpl(const DataTypes &arguments) const override {
    ...
    if (arguments[0]->getTypeId() == TypeIndex::Tuple) return getReturnTypeForTuples(arguments);
    if (arguments[0]->getTypeId() == TypeIndex::Map) return getReturnTypeForMaps(arguments);
        ...
        for (size_t j = 0; j < len; ++j) {
          ...
          if constexpr (std::is_same_v<KeyType, String>) {
            if (const auto *col_fixed = checkAndGetColumn<ColumnFixedString>(arg.key_column.get()))
              ...
              throw Exception(ErrorCodes::LOGICAL_ERROR, "Expected String or FixedString, got {} in {}", arg.key_column->getDataType(),
                              getName());
          } else {
          ...
          if (op_type == OpTypes::ADD) {
            ...
            if (!inserted) it->second = common::addIgnoreOverflow(it->second, value);
          } else {
            const auto [it, inserted] = summing_map.insert({key, first ? value : common::negateIgnoreOverflow(value)});
            if (!inserted) it->second = common::subIgnoreOverflow(it->second, value);
          }
        }
  ...
  ColumnPtr executeImpl(const ColumnsWithTypeAndName &arguments, const DataTypePtr &, size_t) const override {
    ...
    if (tup_type) {
      ...
      res_type = std::make_shared<DataTypeTuple>(
          DataTypes{std::make_shared<DataTypeArray>(key_array_type->getNestedType()), std::make_shared<DataTypeArray>(res_value_type)});
...
Collect all the keys and sum corresponding values.
  ...
  FunctionDocumentation::Arguments arguments_mapAdd = {
      {"arg1[, arg2, ...]",
       "Maps or tuples of two arrays in which items in the first array represent keys, and the second array contains values for each key.",
       {"Map(K, V)", "Tuple(Array(T), Array(T))"}}};
  FunctionDocumentation::ReturnedValue returned_value_mapAdd = {
      "Returns a map or returns a tuple, where the first array contains the sorted keys and the second array contains values.",
      {"Map(K, V)", "Tuple(Array(T), Array(T))"}};
  ...
  FunctionDocumentation documentation_mapAdd = {description_mapAdd,    syntax_mapAdd,   arguments_mapAdd,     {},
                                                returned_value_mapAdd, examples_mapAdd, introduced_in_mapAdd, category_mapAdd};
...
Collect all the keys and subtract corresponding values.
  ...
  FunctionDocumentation::Arguments arguments_mapSubtract = {
      {"arg1[, arg2, ...]",
       "Maps or tuples of two arrays in which items in the first array represent keys, and the second array contains values for each key.",
       {"Map(K, V)", "Tuple(Array(T), Array(T))"}}};
  FunctionDocumentation::ReturnedValue returned_value_mapSubtract = {
      "Returns one map or tuple, where the first array contains the sorted keys and the second array contains values.",
      {"Map(K, V)", "Tuple(Array(T), Array(T))"}};
  FunctionDocumentation::Examples examples_mapSubtract = {
      {"With Map type", "SELECT mapSubtract(map(1, 1), map(1, 1))", "{1:0}"},
      {"With tuple map", "SELECT mapSubtract(([toUInt8(1), 2], [toInt32(1), 1]), ([toUInt8(1), 2], [toInt32(2), 1]))",
       "([1, 2], [-1, 0])"}};
  ...
  FunctionDocumentation documentation_mapSubtract = {
      description_mapSubtract,    syntax_mapSubtract,   arguments_mapSubtract,     {},
      returned_value_mapSubtract, examples_mapSubtract, introduced_in_mapSubtract, category_mapSubtract};
  factory.registerFunction(
      "mapSubtract", [](ContextPtr context) { return FunctionMapOp::create(context, OpTypes::SUBTRACT); }, documentation_mapSubtract);
}
