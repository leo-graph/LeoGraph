...
struct Less {
  ...
  explicit Less(const IColumn &column_) : column(assert_cast<const ColumnType &>(column_)) {}
...
struct NullableLess {
  ...
  explicit NullableLess(const IColumn &nested_column_, const NullMap &null_map_)
      : nested_column(assert_cast<const ColumnType &>(nested_column_)), null_map(null_map_) {}
...
ColumnPtr ArraySortImpl<positive, is_partial>::execute(const ColumnArray &array, ColumnPtr mapped,
                                                       const ColumnWithTypeAndName *fixed_arguments) {
  [[maybe_unused]] const auto limit = [&]() -> size_t {
    if constexpr (is_partial) {
      if (!fixed_arguments) throw Exception(ErrorCodes::LOGICAL_ERROR, "Expected fixed arguments to get the limit for partial array sort");

      /// During dryRun the input column might be empty
      if (!fixed_arguments[0].column->empty()) return fixed_arguments[0].column->getUInt(0);
    }
    return 0;
  }();
...
:::
  ...
  arguments = {{"f(arr[, arr1, ... ,arrN])", "The lambda function to apply to elements of array `x`.", {"Lambda function"}},
...
in ascending order. The remaining elements `(limit..N]` are in an unspecified order.
  ...
  examples = {{"simple_int", "SELECT arrayPartialSort(2, [5, 9, 1, 3])", "[1, 3, 5, 9]"},
...
in descending order. The remaining elements `(limit..N]` are in an unspecified order.
  ...
  examples = {{"simple_int", "SELECT arrayPartialReverseSort(2, [5, 9, 1, 3])", "[9, 5, 1, 3]"},
