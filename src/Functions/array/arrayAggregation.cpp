...
enum class AggregateOperation : uint8_t { min, max, sum, average, product };
...
struct ArrayAggregateResultImpl<ArrayElement, AggregateOperation::sum> {
  using Result = std::conditional_t<
      std::is_same_v<ArrayElement, Int128>, Int128,
      std::conditional_t<
          std::is_same_v<ArrayElement, UInt128>, UInt128,
          std::conditional_t<
              std::is_same_v<ArrayElement, Int256>, Int256,
              std::conditional_t<
                  std::is_same_v<ArrayElement, UInt256>, UInt256,
                  std::conditional_t<
                      std::is_same_v<ArrayElement, Decimal32>, Decimal128,
                      std::conditional_t<
                          std::is_same_v<ArrayElement, Decimal64>, Decimal128,
                          std::conditional_t<
                              std::is_same_v<ArrayElement, Decimal128>, Decimal128,
                              std::conditional_t<std::is_same_v<ArrayElement, Decimal256>, Decimal256,
                                                 std::conditional_t<std::is_same_v<ArrayElement, DateTime64>, Decimal128,
                                                                    std::conditional_t<is_floating_point<ArrayElement>, Float64,
                                                                                       std::conditional_t<std::is_signed_v<ArrayElement>,
                                                                                                          Int64, UInt64>>>>>>>>>>>;
};
...
struct ArrayAggregateImpl {
    ...
    for (auto end_of_array : offsets) {
      ...
      for (UInt64 i = index + 1; i < end_of_array; i++) {
        if constexpr (aggregate_operation == AggregateOperation::min) {
          if ((mapped->compareAt(i, index, *mapped, nan_null_direction_hint) < 0)) index = i;
        } else {
          if ((mapped->compareAt(i, index, *mapped, nan_null_direction_hint) > 0)) index = i;
        }
      }
  ...
  static NO_SANITIZE_UNDEFINED bool executeType(const ColumnPtr& mapped, const ColumnArray::Offsets& offsets, ColumnPtr& res_ptr) {
    ...
    static constexpr bool is_average_or_product_operation =
        aggregate_operation == AggregateOperation::average || aggregate_operation == AggregateOperation::product;
        ...
        if constexpr (aggregate_operation == AggregateOperation::sum) {
          ...
          if constexpr (is_decimal<Element>) {
            ...
            if (unlikely(result_scale > DecimalUtils::max_precision<AggregationType>))
              throw Exception(ErrorCodes::ARGUMENT_OUT_OF_BOUND, "Scale {} is out of bounds (max scale: {})", result_scale,
                              DecimalUtils::max_precision<AggregationType>);

            res[i] = DecimalUtils::convertTo<ResultType>(product, static_cast<UInt32>(result_scale));
          } else {
            for (size_t array_index = 1; array_index < array_size; ++array_index) product = product * x;
    ...
    for (size_t i = 0; i < offsets.size(); ++i) {
      ...
      for (; pos < offsets[i]; ++pos) {
        ...
        if constexpr (aggregate_operation == AggregateOperation::sum || aggregate_operation == AggregateOperation::average) {
  ...
  static ColumnPtr execute(const ColumnArray& array, ColumnPtr mapped) {
    ...
    if constexpr (aggregate_operation == AggregateOperation::min || aggregate_operation == AggregateOperation::max) {
      ...
      if (executeType<UInt8>(mapped, offsets, res) || executeType<UInt16>(mapped, offsets, res) ||
          executeType<UInt32>(mapped, offsets, res) || executeType<UInt64>(mapped, offsets, res) ||
          executeType<UInt128>(mapped, offsets, res) || executeType<UInt256>(mapped, offsets, res) ||
          executeType<Int8>(mapped, offsets, res) || executeType<Int16>(mapped, offsets, res) || executeType<Int32>(mapped, offsets, res) ||
          executeType<Int64>(mapped, offsets, res) || executeType<Int128>(mapped, offsets, res) ||
          executeType<Int256>(mapped, offsets, res) || executeType<Float32>(mapped, offsets, res) ||
          executeType<Float64>(mapped, offsets, res) || executeType<Decimal32>(mapped, offsets, res) ||
          executeType<Decimal64>(mapped, offsets, res) || executeType<Decimal128>(mapped, offsets, res) ||
          executeType<Decimal256>(mapped, offsets, res) || executeType<DateTime64>(mapped, offsets, res)) {
        return res;
      }
    }
...
struct NameArrayAverage {
  static constexpr auto name = "arrayAvg";
};
...
If a lambda function `func` is specified, returns the minimum element of the lambda results.
  ...
  FunctionDocumentation::Arguments arguments_min = {
      {"func(x[, y1, ..., yN])",
       "Optional. A lambda function which operates on elements of the source array (`x`) and condition arrays (`y`).",
       {"Lambda function"}},
  ...
  FunctionDocumentation::ReturnedValue returned_value_min = {
  ...
  FunctionDocumentation documentation_min = {description_min,    syntax_min,   arguments_min,     {},
                                             returned_value_min, examples_min, introduced_in_min, category_min};
...
If a lambda function `func` is specified, returns the maximum element of the lambda results.
  ...
  FunctionDocumentation::Arguments arguments_max = {
      ...
      {"source_arr", "The source array to process.", {"Array(T)"}},
      {"[, cond1_arr, ... , condN_arr]",
       "Optional. N condition arrays providing additional arguments to the lambda function.",
       {"Array(T)"}}};
  FunctionDocumentation::ReturnedValue returned_value_max = {
  ...
  FunctionDocumentation documentation_max = {description_max,    syntax_max,   arguments_max,     {},
                                             returned_value_max, examples_max, introduced_in_max, category_max};
...
If a lambda function `func` is specified, returns the sum of elements of the lambda results.
  ...
  FunctionDocumentation::Arguments arguments_sum = {
      ...
      {", cond1_arr, ... , condN_arr]",
       "Optional. N condition arrays providing additional arguments to the lambda function.",
       {"Array(T)"}}};
  FunctionDocumentation::ReturnedValue returned_value_sum = {
  ...
  FunctionDocumentation documentation_sum = {description_sum,    syntax_sum,   arguments_sum,     {},
                                             returned_value_sum, examples_sum, introduced_in_sum, category_sum};
...
If a lambda function `func` is specified, returns the average of elements of the lambda results.
  ...
  FunctionDocumentation::Arguments arguments_avg = {
      ...
      {"[, cond1_arr, ... , condN_arr]",
       "Optional. N condition arrays providing additional arguments to the lambda function.",
       {"Array(T)"}}};
  FunctionDocumentation::ReturnedValue returned_value_avg = {
  ...
  FunctionDocumentation documentation_avg = {description_avg,    syntax_avg,   arguments_avg,     {},
                                             returned_value_avg, examples_avg, introduced_in_avg, category_avg};
...
If a lambda function `func` is specified, returns the product of elements of the lambda results.
  ...
  FunctionDocumentation::Arguments arguments_prod = {
      ...
      {"[, cond1_arr, ... , condN_arr]",
       "Optional. N condition arrays providing additional arguments to the lambda function.",
       {"Array(T)"}}};
  FunctionDocumentation::ReturnedValue returned_value_prod = {
  ...
  FunctionDocumentation documentation_prod = {description_prod,    syntax_prod,   arguments_prod,     {},
                                              returned_value_prod, examples_prod, introduced_in_prod, category_prod};
