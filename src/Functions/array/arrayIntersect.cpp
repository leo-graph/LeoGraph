...
class FunctionArrayIntersect : public IFunction {
 ...
 private:
  ...
  static CastArgumentsResult castColumns(const ColumnsWithTypeAndName &arguments, const DataTypePtr &return_type,
  ...
  template <typename Map, typename ColumnType, bool is_numeric_column>
  static void insertElement(typename Map::LookupResult &pair, size_t &result_offset, ColumnType &result_data, NullMap &null_map,
                            const bool &use_null_map);
...
DataTypePtr FunctionArrayIntersect::getReturnTypeImpl(const DataTypes &arguments) const {
  ...
  for (auto i : collections::range(0, arguments.size())) {
    ...
    if (!array_type)
      throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT, "Argument {} for function {} must be an array but it has type {}.", i,
                      getName(), arguments[i]->getName());
    ...
    if (typeid_cast<const DataTypeNothing *>(nested_type.get())) {
      ...
      if (mode == ArraySetMode::Union) {
        ...
        if (has_non_decimal_type && has_decimal_type)
          throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT, "Illegal types of arguments for function {}: {} and {}.", getName(),
                          has_non_decimal_type->getName(), has_decimal_type);
      }
    }
  }
...
ColumnPtr FunctionArrayIntersect::castRemoveNullable(const ColumnPtr &column, const DataTypePtr &data_type) const {
  ...
  if (const auto *column_array = checkAndGetColumn<ColumnArray>(column.get())) {
    ...
    if (!array_type)
      throw Exception(ErrorCodes::LOGICAL_ERROR, "Cannot cast array column to column with type {} in function {}", data_type->getName(),
                      getName());
  ...
  if (const auto *column_tuple = checkAndGetColumn<ColumnTuple>(column.get())) {
    ...
    if (!tuple_type)
      throw Exception(ErrorCodes::LOGICAL_ERROR, "Cannot cast tuple column to type {} in function {}", data_type->getName(), getName());

    auto columns_number = column_tuple->tupleSize();

    /// Empty tuple
    if (columns_number == 0) return column;
...
FunctionArrayIntersect::CastArgumentsResult FunctionArrayIntersect::castColumns(const ColumnsWithTypeAndName &arguments,
...
FunctionArrayIntersect::UnpackedArrays FunctionArrayIntersect::prepareArrays(const ColumnsWithTypeAndName &columns,
  ...
  for (size_t i = 0; i < columns_number; ++i) {
    ...
    if (const auto *argument_column_array = typeid_cast<const ColumnArray *>(argument_column)) {
      if (!arg.is_const) all_const = false;
      ...
      if (const auto *column_nullable = typeid_cast<const ColumnNullable *>(arg.nested_column)) {
        ...
        if (initial_column->isNullable()) initial_column = &typeid_cast<const ColumnNullable &>(*initial_column).getNestedColumn();
      }
      ...
      if (arg.nested_column != initial_column) {
        ...
        if (isInteger(nested_init_type) || isDate(nested_init_type) || isDateTime(nested_init_type) || isDateTime64(nested_init_type)) {
          ...
          auto overflow_mask = callFunctionNotEquals({arg.nested_column->getPtr(), nested_cast_type, ""},
                                                     {initial_column->getPtr(), nested_init_type, ""}, context);
  ...
  if (all_const) {
    ...
    for (size_t i = 0; i < columns_number; ++i) {
      if (arrays.args[i].is_const) continue;
...
ColumnPtr FunctionArrayIntersect::executeImpl(const ColumnsWithTypeAndName &arguments, const DataTypePtr &result_type,
  ...
  for (size_t i = 0; i < num_args; ++i) data_types.push_back(arguments[i].type);
  ...
  using DateTimeMap =
      ClearableHashMapWithStackMemory<DataTypeDateTime::FieldType, size_t, DefaultHash<DataTypeDateTime::FieldType>, INITIAL_SIZE_DEGREE>;

  using StringMap = ClearableHashMapWithStackMemory<std::string_view, size_t, StringViewHash, INITIAL_SIZE_DEGREE>;
...
void FunctionArrayIntersect::NumberExecutor::operator()(TypeList<T>) {
  using Container = ClearableHashMapWithStackMemory<T, size_t, DefaultHash<T>, INITIAL_SIZE_DEGREE>;
...
ColumnPtr FunctionArrayIntersect::execute(const UnpackedArrays &arrays, MutableColumnPtr result_data_ptr, ArraySetMode mode) {
  ...
  for (const auto &arg : arrays.args) {
    ...
    if (!columns.back()) throw Exception(ErrorCodes::LOGICAL_ERROR, "Unexpected array type for function arrayIntersect");
  ...
  for (size_t row = 0; row < rows; ++row) {
    ...
    for (size_t arg_num = 0; arg_num < args; ++arg_num) {
      ...
      if (arg_num) {
        ...
        if (arg.is_const) prev_off[arg_num] = 0;
      }
    ...
    if (mode == ArraySetMode::Union) {
      ...
      for (auto i : collections::range(prev_off[0], off)) {
        ...
        if (!current_has_nullable) all_has_nullable = false;
    ...
    prev_off[0] = off;
    if (arg.is_const) prev_off[0] = 0;

    result_offsets.getElement(row) = result_offset;
  }
  ColumnPtr result_column = std::move(result_data_ptr);
  if (all_nullable) result_column = ColumnNullable::create(result_column, std::move(null_map_column));
...
void FunctionArrayIntersect::insertElement(typename Map::LookupResult &pair, size_t &result_offset, ColumnType &result_data,
                                           NullMap &null_map, const bool &use_null_map) {
  pair->getMapped() = -1;
  ++result_offset;
  if constexpr (is_numeric_column) {
  ...
  if (use_null_map) null_map.push_back(false);
}

REGISTER_FUNCTION(ArrayIntersect) {
  FunctionDocumentation::Description intersect_description =
      "Takes multiple arrays and returns an array with elements which are present in all source arrays. The result contains only unique "
      "values.";
  ...
  FunctionDocumentation::ReturnedValue intersect_returned_value = {
...
└────────────────────────┴────────────────────┘
  ...
  FunctionDocumentation intersect_documentation = {
      intersect_description,    intersect_syntax,  intersect_argument,      {},
      intersect_returned_value, intersect_example, intersect_introduced_in, intersect_category};
  ...
  FunctionDocumentation::Description union_description =
      "Takes multiple arrays and returns an array which contains all elements that are present in one of the source arrays.The result "
  ...
  FunctionDocumentation::ReturnedValue union_returned_value = {"Returns an array with distinct elements from the source arrays",
                                                               {"Array(T)"}};
...
└─────────────┴────────────────┴──────────────┘
  ...
  FunctionDocumentation union_documentation = {union_description,    union_syntax,  union_argument,      {},
                                               union_returned_value, union_example, union_introduced_in, union_category};

  factory.registerFunction(
      "arrayUnion", [](ContextPtr ctx) { return FunctionArrayIntersect::create("arrayUnion", ArraySetMode::Union, std::move(ctx)); },
      union_documentation);

  FunctionDocumentation::Description symdiff_description =
      R"(Takes multiple arrays and returns an array with elements that are not present in all source arrays. The result contains only unique values.
...
:::
  ...
  FunctionDocumentation::Arguments symdiff_argument = {
      {"arrN", "N arrays from which to make the new array. [`Array(T)`](/sql-reference/data-types/array)."}};
  FunctionDocumentation::ReturnedValue symdiff_returned_value = {"Returns an array of distinct elements not present in all source arrays",
                                                                 {"Array(T)"}};
...
└────────────────────────────┴────────────────────────────────┘
  ...
  FunctionDocumentation symdiff_documentation = {symdiff_description,    symdiff_syntax,  symdiff_argument,      {},
                                                 symdiff_returned_value, symdiff_example, symdiff_introduced_in, symdiff_category};

  factory.registerFunction(
      "arraySymmetricDifference",
      [](ContextPtr ctx) {
        return FunctionArrayIntersect::create("arraySymmetricDifference", ArraySetMode::SymmetricDifference, std::move(ctx));
      },
