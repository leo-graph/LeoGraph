 ...
 private:
  ...
  void checkTypes(const DataTypePtr &key_type, const DataTypePtr &value_type, const DataTypePtr &max_key_type) const {
    ...
    if (!(key_data_type.isInt() || key_data_type.isUInt())) {
      throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT,
                      "Function {} key argument should be of signed or unsigned integer type. Actual type {}", getName(),
                      key_type->getName());
    }
    ...
    if (!max_key_type) return;

    WhichDataType max_key_data_type(max_key_type);

    if (max_key_data_type.isNullable())
      throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT, "Function {} max key argument can not be Nullable. Actual type {}", getName(),
                      max_key_type->getName());

    if (!(max_key_data_type.isInt() || max_key_data_type.isUInt()))
      throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT,
                      "Function {} max key should be of signed or unsigned integer type. Actual type {}, max type {}.", getName(),
                      key_type->getName(), max_key_type->getName());
  }

  DataTypePtr getReturnTypeImpl(const DataTypes &arguments) const override {
    if (arguments.empty() || arguments.size() > 3)
      throw Exception(ErrorCodes::NUMBER_OF_ARGUMENTS_DOESNT_MATCH,
                      "Function {} accepts at least one map or two arrays arguments, and optional max key argument", getName());
    ...
    if (key_argument_data_type.isArray()) {
      if (arguments.size() < 2 || !arguments[1] || !isArray(arguments[1]))
        throw Exception(ErrorCodes::BAD_ARGUMENTS,
                        "Function {} if array argument is passed as key, additional array argument as value must be passed", getName());
      ...
      throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT, "Function {} only accepts one map or arrays, but got {}", getName(),
                      arguments[0]->getName());

    DataTypePtr max_key_argument_type;
    if (max_key_argument_index < arguments.size()) max_key_argument_type = arguments[max_key_argument_index];

    checkTypes(key_argument_series_type, value_argument_series_type, max_key_argument_type);

    if (key_argument_data_type.isArray()) return std::make_shared<DataTypeTuple>(DataTypes{arguments[0], arguments[1]});
    return arguments[0];
  }

  template <typename KeyType, typename ValueType>
  void executeImplTyped(const ColumnPtr &key_column, const ColumnPtr &value_column, const ColumnPtr &offsets_column,
                        const ColumnPtr &max_key_column, MutableColumnPtr result_key_column, MutableColumnPtr result_value_column,
    ...
    const PaddedPODArray<KeyType> *max_key_data =
        max_key_column ? &assert_cast<const ColumnVector<KeyType> &>(*max_key_column).getData() : nullptr;
    ...
    for (size_t offset_index = 0; offset_index < key_offsets_size; ++offset_index) {
      ...
      if (max_min_key_difference > MAX_ARRAY_SIZE)
        throw Exception(ErrorCodes::TOO_LARGE_ARRAY_SIZE, "Function {} too large array size in the result", getName());
      ...
      for (KeyType current_key = min_key; current_key <= max_key; ++current_key) {
        ...
        if (sorted_values_index < sorted_keys_values_size && sorted_keys_values[sorted_values_index].first == current_key) {
          ...
          ++sorted_values_index;
          while (sorted_values_index < sorted_keys_values_size && current_key == sorted_keys_values[sorted_values_index].first) {
            ++sorted_values_index;
          }
        }
  ...
  KeyAndValueInput extractKeyAndValueInput(const ColumnsWithTypeAndName &arguments) const {
    ...
    if (const auto *key_argument_array_column = typeid_cast<const ColumnArray *>(first_argument_column.get())) {
      ...
      if (key_offsets != value_offsets)
        throw Exception(ErrorCodes::BAD_ARGUMENTS, "Function {} key and value array should have same amount of elements", getName());
      ...
      throw Exception(ErrorCodes::ILLEGAL_COLUMN, "Function {} only accepts one map or arrays, but got {}", getName(),
    ...
    if (tuple_column && tuple_column->tupleSize() == 2) {
      ...
      if (!key_array_column_typed || !value_array_column_typed)
        throw Exception(ErrorCodes::LOGICAL_ERROR,
                        "Function {} result type should be Tuple with two nested Array columns or Map. Actual {}", getName(),
                        result_type->getName());
      ...
      throw Exception(ErrorCodes::LOGICAL_ERROR, "Function {} result type should be Tuple with two nested Array columns or Map. Actual {}",
  ...
  ColumnPtr executeImpl(const ColumnsWithTypeAndName &arguments, const DataTypePtr &result_type, size_t) const override {
      ...
      if constexpr (key_and_value_are_numbers) {
        if constexpr (is_floating_point<typename KeyType::FieldType>) {
          ...
          executeImplTyped<KeyFieldType, ValueFieldType>(
              input.key_column, input.value_column, input.offsets_column, input.max_key_column, std::move(result_columns.result_key_column),
              std::move(result_columns.result_value_column), std::move(result_columns.result_offset_column));
    ...
    if (!callOnTwoTypeIndexes(input.key_series_type->getTypeId(), input.value_series_type->getTypeId(), call))
      throw Exception(ErrorCodes::ILLEGAL_COLUMN, "Function {} illegal columns passed as arguments", getName());

    if (result_columns.result_array_additional_offset_column) {
      result_columns.result_array_additional_offset_column->insertRangeFrom(*result_columns.result_offset_column_raw, 0,
...
In case keys repeat, only the first value (in order of appearance) is associated with the key.
  ...
  FunctionDocumentation::ReturnedValue returned_value = {
      "Returns a map or a tuple of two arrays where the first has keys in sorted order, and the second values for the corresponding keys.",
      {"Map(K, V)", "Tuple(Array(UInt*), Array(Any))"}};
