...
struct ArrayCumSumNonNegativeImpl {
  ...
  static DataTypePtr getReturnType(const DataTypePtr& expression_return, const DataTypePtr& /*array_element*/) {
    ...
    if (which.isUInt()) {
      if (which.isNativeUInt()) return std::make_shared<DataTypeArray>(std::make_shared<DataTypeUInt64>());
      if (which.isUInt128()) return std::make_shared<DataTypeArray>(std::make_shared<DataTypeUInt128>());
      if (which.isUInt256()) return std::make_shared<DataTypeArray>(std::make_shared<DataTypeUInt256>());
      UNREACHABLE();
    }

    if (which.isInt()) {
      if (which.isNativeInt()) return std::make_shared<DataTypeArray>(std::make_shared<DataTypeInt64>());
      if (which.isInt128()) return std::make_shared<DataTypeArray>(std::make_shared<DataTypeInt128>());
      if (which.isInt256()) return std::make_shared<DataTypeArray>(std::make_shared<DataTypeInt256>());
      UNREACHABLE();
    }

    if (which.isFloat()) return std::make_shared<DataTypeArray>(std::make_shared<DataTypeFloat64>());
    ...
    throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT, "arrayCumSumNonNegativeImpl cannot add values of type {}",
                    expression_return->getName());
  }

  template <typename Src, typename Dst>
  static void NO_SANITIZE_UNDEFINED implVector(size_t size, const IColumn::Offset* __restrict offsets, Dst* __restrict res_values,
                                               const Src* __restrict src_values) {
    size_t pos = 0;
    for (const auto* end = offsets + size; offsets < end; ++offsets) {
      ...
      for (; pos < offset; ++pos) {
        ...
        if (accumulated < Dst{}) accumulated = {};
...
REGISTER_FUNCTION(ArrayCumSumNonNegative) {
  FunctionDocumentation::Description description =
      "Returns an array of the partial (running) sums of the elements in the source array, replacing any negative running sum with zero. "
      "If a lambda function is specified, the sum is computed from applying the lambda to the array elements at each position.";
  ...
  FunctionDocumentation::Arguments arguments = {
      ...
      {"[arr2, ..., arrN]",
       "Optional. Additional arrays of the same size, passed as arguments to the lambda function if specified.",
       {"Array(T)"}}};
  FunctionDocumentation::ReturnedValue returned_value = {
      "Returns an array of the partial sums of the elements in the source array, with any negative running sum replaced by zero. The "
      "result type matches the input array's numeric type.",
      {"Array(T)"}};
  FunctionDocumentation::Examples examples = {{"Basic usage", "SELECT arrayCumSumNonNegative([1, 1, -4, 1]) AS res", "[1, 2, 0, 1]"},
