...
DataTypePtr FunctionArrayReduceInRanges::getReturnTypeImpl(const ColumnsWithTypeAndName &arguments) const {
  ...
  if (arguments.size() < 3)
    throw Exception(ErrorCodes::NUMBER_OF_ARGUMENTS_DOESNT_MATCH,
                    "Number of arguments for function {} doesn't match: passed {}, should be at least 3.", getName(), arguments.size());
  ...
  if (!ranges_type_array)
    throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT, "Second argument for function {} must be an array of ranges.", getName());
  ...
  if (!WhichDataType(ranges_type_tuple->getElements()[1]).isNativeUInt())
    throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT, "Second tuple member in the second argument for function {} must be uints.",
                    getName());
  ...
  for (size_t i = 2, size = arguments.size(); i < size; ++i) {
    ...
    if (!arg)
      throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT, "Argument {} for function {} must be an array but it has type {}.", i,
                      getName(), arguments[i].type->getName());
  ...
  if (!aggregate_function) {
    ...
    Array params_row;
    getAggregateFunctionNameAndParametersArray(aggregate_function_name_with_params, aggregate_function_name, params_row,
                                               "function " + getName(), getContext());

    AggregateFunctionProperties properties;
    auto action = NullsAction::EMPTY;
    aggregate_function = AggregateFunctionFactory::instance().get(aggregate_function_name, action, argument_types, params_row, properties);
  }

  return std::make_shared<DataTypeArray>(aggregate_function->getResultType());
}

ColumnPtr FunctionArrayReduceInRanges::executeImpl(const ColumnsWithTypeAndName &arguments, const DataTypePtr &result_type,
  ...
  for (size_t i = 0; i < input_rows_count; ++i) {
    ...
    for (size_t j = 0; j < place_total; ++j) {
      ...
      try {
        ...
        for (size_t k = 0; k < j; ++k) agg_func.destroy(places[k]);
        throw;
      }
    }

    SCOPE_EXIT_MEMORY_SAFE({
      for (size_t j = 0; j < place_total; ++j) agg_func.destroy(places[j]);
    });

    const auto *true_func = &agg_func;
    /// Unnest consecutive trailing -State combinators
    while (const auto *func = typeid_cast<const AggregateFunctionState *>(true_func)) true_func = func->getNestedFunction().get();
    ...
    for (size_t j = 0; j < place_offsets[0]; ++j) {
      ...
      for (size_t k = local_begin; k < local_end; ++k) true_func->add(places[j], aggregate_arguments, begin + k, arena.get());
    }
    ...
    for (size_t j = ranges_begin; j < ranges_end; ++j) {
      ...
      SCOPE_EXIT_MEMORY_SAFE({ agg_func.destroy(place); });
      ...
      if (place_begin < place_end) {
        ...
        /// Aggregate using pre-aggretated data
        {
          ...
          while (place_curr < place_end) {
            while (((place_curr >> level) & 1) == 0 && place_curr + (2 << level) <= place_end) level += 1;
            while (place_curr + (1 << level) > place_end) level -= 1;

            size_t place_offset = 0;
            if (level) place_offset = place_offsets[level - 1];
        ...
        for (size_t k = place_end * minimum_step; k < local_end; ++k) true_func->add(place, aggregate_arguments, begin + k, arena.get());
      } else {
        /// In this case, we can not use pre-aggregated data.

        for (size_t k = local_begin; k < local_end; ++k) true_func->add(place, aggregate_arguments, begin + k, arena.get());
      }
...
The function will return the same result as multiple `arrayReduce(agg_func, arraySlice(arr1, index, length), ...)`.
  ...
  FunctionDocumentation::Arguments arguments = {
      ...
      {"ranges",
       "The range over which to aggregate. An array of tuples, `(i, r)` containing the index `i` from which to begin from and the range "
       "`r` over which to aggregate.",
       {"Array(T)", "Tuple(T)"}},
