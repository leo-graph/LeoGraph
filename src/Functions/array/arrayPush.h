...
class FunctionArrayPush : public IFunction {
 public:
  FunctionArrayPush(bool push_front_, const char *name_) : push_front(push_front_), name(name_) {}
  ...
  ColumnPtr executeImpl(const ColumnsWithTypeAndName &arguments, const DataTypePtr &return_type, size_t input_rows_count) const override {
    ...
    if (!arguments[1].type->equals(*return_nested_type)) appended_column = castColumn(arguments[1], return_nested_type);
