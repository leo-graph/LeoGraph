...
struct ArraySplitImpl {
  ...
  static DataTypePtr getReturnType(const DataTypePtr & /*expression_return*/, const DataTypePtr &array_element) {
    return std::make_shared<DataTypeArray>(std::make_shared<DataTypeArray>(array_element));
  }
  ...
  static ColumnPtr execute(const ColumnArray &array, ColumnPtr mapped) {
    ...
    if (column_cut) {
      ...
      for (auto in_offset : in_offsets) {
        if (pos < in_offset) {
          ...
          for (; pos < in_offset - reverse; ++pos)
            if (cut[pos]) out_offsets_2.push_back(pos + reverse);
          pos += reverse;
      ...
      if (!column_cut_const) throw Exception(ErrorCodes::ILLEGAL_COLUMN, "Unexpected type of cut column");

      if (column_cut_const->getValue<UInt8>()) {
        out_offsets_2.reserve(in_offsets.back());
        out_offsets_1.reserve(in_offsets.size());

        for (size_t i = 0; i < in_offsets.back(); ++i) out_offsets_2.push_back(i + 1);
        for (auto in_offset : in_offsets) out_offsets_1.push_back(in_offset);
      } else {
        size_t pos = 0;
    ...
    return ColumnArray::create(ColumnArray::create(array.getDataPtr(), std::move(column_offsets_2)), std::move(column_offsets_1));
  }
};

struct NameArraySplit {
  static constexpr auto name = "arraySplit";
};
struct NameArrayReverseSplit {
  static constexpr auto name = "arrayReverseSplit";
};
...
REGISTER_FUNCTION(ArraySplit) {
  FunctionDocumentation::Description description =
      "Split a source array into multiple arrays. When `func(x [, y1, ..., yN])` returns something other than zero, the array will be "
      "split to the left of the element. The array will not be split before the first element.";
  FunctionDocumentation::Syntax syntax = "arraySplit(func(x[, y1, ..., yN]), source_arr[, cond1_arr, ... , condN_arr])";
  FunctionDocumentation::Arguments arguments = {
      {"func(x[, y1, ..., yN])",
       "A lambda function which operates on elements of the source array (`x`) and condition arrays (`y`).[Lambda "
       "function](/sql-reference/functions/overview#arrow-operator-and-lambda)."},
  ...
  FunctionDocumentation::Examples examples = {
      {"Usage example", "SELECT arraySplit((x, y) -> y, [1, 2, 3, 4, 5], [1, 0, 0, 1, 0]) AS res", "[[1, 2, 3], [4, 5]]"}};
  ...
  FunctionDocumentation::Description description_split =
      "Split a source array into multiple arrays. When `func(x[, y1, ..., yN])` returns something other than zero, the array will be split "
      "to the right of the element. The array will not be split after the last element.";
  ...
  FunctionDocumentation::Arguments arguments_split = {
      {"func(x[, y1, ..., yN])",
       "A lambda function which operates on elements of the source array (`x`) and condition arrays (`y`).",
       {"Lambda function"}},
  ...
  FunctionDocumentation::Examples examples_split = {
      {"Usage example", "SELECT arrayReverseSplit((x, y) -> y, [1, 2, 3, 4, 5], [1, 0, 0, 1, 0]) AS res", "[[1], [2, 3, 4], [5]]"}};
  ...
  FunctionDocumentation documentation_split = {description_split,    syntax_split,   arguments_split,     {},
                                               returned_value_split, examples_split, introduced_in_split, category_split};
