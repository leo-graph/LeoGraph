...
ColumnPtr ArrayAllImpl::execute(const ColumnArray &array, ColumnPtr mapped) {
  ...
  if (!column_filter) {
    ...
    if (column_filter_const->getValue<UInt8>()) return DataTypeUInt8().createColumnConst(array.size(), 1u);
...
Returns `1` if lambda `func(x [, y1, y2, ... yN])` returns true for all elements. Otherwise, it returns `0`.
  ...
  FunctionDocumentation::ReturnedValue returned_value = {"Returns `1` if the lambda function returns true for all elements, `0` otherwise",
                                                         {"UInt8"}};
  FunctionDocumentation::Examples examples = {{"All elements match", "SELECT arrayAll(x, y -> x=y, [1, 2, 3], [1, 2, 3])", "1"},
