...
struct ArrayCompactImpl {
  ...
  static bool executeType(const ColumnPtr& mapped, const ColumnArray& array, ColumnPtr& res_ptr) {
    ...
    if (!src_values_column || !check_values_column) return false;
  ...
  static ColumnPtr execute(const ColumnArray& array, ColumnPtr mapped) {
    ...
    if (!(executeType<UInt8>(mapped, array, res) || executeType<UInt16>(mapped, array, res) || executeType<UInt32>(mapped, array, res) ||
          executeType<UInt64>(mapped, array, res) || executeType<Int8>(mapped, array, res) || executeType<Int16>(mapped, array, res) ||
          executeType<Int32>(mapped, array, res) || executeType<Int64>(mapped, array, res) || executeType<Float32>(mapped, array, res) ||
          executeType<Float64>(mapped, array, res)) ||
        executeType<Decimal32>(mapped, array, res) || executeType<Decimal64>(mapped, array, res) ||
        executeType<Decimal128>(mapped, array, res) || executeType<Decimal256>(mapped, array, res)) {
...
struct NameArrayCompact {
  static constexpr auto name = "arrayCompact";
};
using FunctionArrayCompact = FunctionArrayMapped<ArrayCompactImpl, NameArrayCompact>;

REGISTER_FUNCTION(ArrayCompact) {
  FunctionDocumentation::Description description =
      "Removes consecutive duplicate elements from an array, including `null` values. The order of values in the resulting array is "
      "determined by the order in the source array.";
  FunctionDocumentation::Syntax syntax = "arrayCompact(arr)";
  FunctionDocumentation::Arguments arguments = {{"arr", "An array to remove duplicates from.", {"Array(T)"}}};
