...
DataTypePtr FunctionArrayResize::getReturnTypeImpl(const ColumnsWithTypeAndName &arguments) const {
  ...
  FunctionArgumentDescriptors optional_args{{"extender", nullptr, nullptr, "Any type"}};

  validateFunctionArguments(*this, arguments, mandatory_args, optional_args);

  if (arguments[0].type->onlyNull()) return arguments[0].type;

  /// Issue #48398
  if (arguments[1].type->isNullable())
    throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT, "Second argument for function {} must not be Nullable.", getName());
...
ColumnPtr FunctionArrayResize::executeImpl(const ColumnsWithTypeAndName &arguments, const DataTypePtr &return_type,
  ...
  if (!arguments[0].type->equals(*return_type)) array_column = castColumn(arguments[0], return_type);
  ...
  if (arguments.size() == 3) {
    ...
    if (!arguments[2].type->equals(*return_nested_type)) appended_column = castColumn(arguments[2], return_nested_type);
  } else
...
REGISTER_FUNCTION(ArrayResize) {
  ...
  FunctionDocumentation::Arguments arguments = {{"arr", "Array to resize.", {"Array(T)"}},
                                                {"size", R"(
