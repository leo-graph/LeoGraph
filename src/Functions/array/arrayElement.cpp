...
class FunctionArrayElement : public IFunction {
 ...
 private:
  ColumnPtr perform(const ColumnsWithTypeAndName &arguments, const DataTypePtr &result_type, ArrayImpl::NullMapBuilder<mode> &builder,
                    size_t input_rows_count) const;

  template <typename DataType>
  static ColumnPtr executeNumberConst(const ColumnsWithTypeAndName &arguments, const DataTypePtr &result_type, const Field &index,
                                      ArrayImpl::NullMapBuilder<mode> &builder);

  template <typename IndexType, typename DataType>
  static ColumnPtr executeNumber(const ColumnsWithTypeAndName &arguments, const DataTypePtr &result_type,
                                 const PaddedPODArray<IndexType> &indices, ArrayImpl::NullMapBuilder<mode> &builder);

  /// Optimize arrayElement when first argument has type Array(T) or Array(Nullable(T)) in which T is number type
  template <typename DataType>
  static ColumnPtr executeArrayNumberConst(const ColumnsWithTypeAndName &arguments, const DataTypePtr &result_type, const Field &index,
                                           ArrayImpl::NullMapBuilder<mode> &builder);

  /// Optimize arrayElement when first argument has type Array(T) or Array(Nullable(T)) in which T is number type
  template <typename IndexType, typename DataType>
  static ColumnPtr executeArrayNumber(const ColumnsWithTypeAndName &arguments, const DataTypePtr &result_type,
                                      const PaddedPODArray<IndexType> &indices, ArrayImpl::NullMapBuilder<mode> &builder);

  static ColumnPtr executeStringConst(const ColumnsWithTypeAndName &arguments, const Field &index,
                                      ArrayImpl::NullMapBuilder<mode> &builder);

  template <typename IndexType>
  static ColumnPtr executeString(const ColumnsWithTypeAndName &arguments, const PaddedPODArray<IndexType> &indices,
                                 ArrayImpl::NullMapBuilder<mode> &builder);

  /// Optimize arrayElement when first argument has type Array(String) or Array(Nullable(String))
  static ColumnPtr executeArrayStringConst(const ColumnsWithTypeAndName &arguments, const Field &index,
                                           ArrayImpl::NullMapBuilder<mode> &builder);

  /// Optimize arrayElement when first argument has type Array(String) or Array(Nullable(String))
  template <typename IndexType>
  static ColumnPtr executeArrayString(const ColumnsWithTypeAndName &arguments, const PaddedPODArray<IndexType> &indices,
                                      ArrayImpl::NullMapBuilder<mode> &builder);

  static ColumnPtr executeGenericConst(const ColumnsWithTypeAndName &arguments, const Field &index,
                                       ArrayImpl::NullMapBuilder<mode> &builder);

  static ColumnPtr removeNullableIfNeeded(const ColumnPtr &column, const DataTypePtr &expect_type);

  template <typename IndexType>
  static ColumnPtr executeGeneric(const ColumnsWithTypeAndName &arguments, const PaddedPODArray<IndexType> &indices,
                                  ArrayImpl::NullMapBuilder<mode> &builder);
  ...
  ColumnPtr executeArgument(const ColumnsWithTypeAndName &arguments, const DataTypePtr &result_type,
                            ArrayImpl::NullMapBuilder<mode> &builder, size_t input_rows_count) const;
  ...
  static bool matchKeyToIndexNumber(const IColumn &data, const Offsets &offsets, bool is_key_const, const IColumn &index,
                                    PaddedPODArray<UInt64> &matched_idxs);

  static bool matchKeyToIndexNumberConst(const IColumn &data, const Offsets &offsets, const Field &index,
                                         PaddedPODArray<UInt64> &matched_idxs);

  static bool matchKeyToIndexString(const IColumn &data, const Offsets &offsets, bool is_key_const, const IColumn &index,
                                    PaddedPODArray<UInt64> &matched_idxs);

  static bool matchKeyToIndexStringConst(const IColumn &data, const Offsets &offsets, const Field &index,
                                         PaddedPODArray<UInt64> &matched_idxs);
  ...
  template <typename Matcher>
  static void executeMatchConstKeyToIndex(size_t num_rows, size_t num_values, PaddedPODArray<UInt64> &matched_idxs, const Matcher &matcher);
};
...
struct ArrayElementNumImpl {
  ...
  static void vectorConst(const PaddedPODArray<T> &data, const ColumnArray::Offsets &offsets, const ColumnArray::Offset index,
                          PaddedPODArray<T> &result, ArrayImpl::NullMapBuilder<mode> &builder) {
  ...
  static void vector(const PaddedPODArray<T> &data, const ColumnArray::Offsets &offsets, const PaddedPODArray<TIndex> &indices,
                     PaddedPODArray<T> &result, ArrayImpl::NullMapBuilder<mode> &builder) {
    size_t size = offsets.size();
    result.resize(size);
...
struct ArrayElementArrayNumImpl {
  ...
  static void vectorConst(const PaddedPODArray<T> &data, const ColumnArray::Offsets &offsets, const ColumnArray::Offsets &nested_offsets,
                          const NullMap *number_null_map, const ColumnArray::Offset index, PaddedPODArray<T> &result_data,
                          ColumnArray::Offsets &result_offsets, NullMap *result_number_null_map, ArrayImpl::NullMapBuilder<mode> &builder) {
    size_t size = offsets.size();
    ...
    if constexpr (nullable_number) result_number_null_map->resize(result_data_size);
  ...
  static void vector(const PaddedPODArray<T> &data, const ColumnArray::Offsets &offsets, const ColumnArray::Offsets &nested_offsets,
                     const NullMap *number_null_map, const PaddedPODArray<TIndex> &indices, PaddedPODArray<T> &result_data,
                     ColumnArray::Offsets &result_offsets, NullMap *result_number_null_map, ArrayImpl::NullMapBuilder<mode> &builder) {
    size_t size = offsets.size();
...
struct ArrayElementArrayStringImpl {
  ...
  static void vectorConst(const ColumnString::Chars &data, const ColumnArray::Offsets &offsets, const ColumnArray::Offsets &nested_offsets,
                          const ColumnString::Offsets &string_offsets, const NullMap *string_null_map, const ColumnArray::Offset index,
                          ColumnString::Chars &result_data, ColumnArray::Offsets &result_offsets,
                          ColumnArray::Offsets &result_string_offsets, NullMap *result_string_null_map,
                          ArrayImpl::NullMapBuilder<mode> &builder) {
    size_t size = offsets.size();
    ...
    if constexpr (nullable_string) result_string_null_map->reserve(result_strings_size);
    ...
    for (size_t i = 0; i < size; ++i) {
      ...
      if (index < array_size) {
        ...
        for (size_t k = 0; k < nested_array_size; ++k) {
          ...
          if constexpr (nullable_string) result_string_null_map->push_back((*string_null_map)[nested_array_start + k]);
        }
        current_offset += nested_array_size;
      } else {
  ...
  static void vector(const ColumnString::Chars &data, const ColumnArray::Offsets &offsets, const ColumnArray::Offsets &nested_offsets,
                     const ColumnString::Offsets &string_offsets, const NullMap *string_null_map, const PaddedPODArray<TIndex> &indices,
                     ColumnString::Chars &result_data, ColumnArray::Offsets &result_offsets, ColumnArray::Offsets &result_string_offsets,
                     NullMap *result_string_null_map, ArrayImpl::NullMapBuilder<mode> &builder) {
    size_t size = offsets.size();
...
struct ArrayElementStringImpl {
  template <bool negative>
  static void vectorConst(const ColumnString::Chars &data, const ColumnArray::Offsets &offsets, const ColumnString::Offsets &string_offsets,
                          const ColumnArray::Offset index, ColumnString::Chars &result_data, ColumnArray::Offsets &result_offsets,
                          ArrayImpl::NullMapBuilder<mode> &builder) {
    size_t size = offsets.size();
    result_offsets.resize(size);
   ...
   */
  template <typename TIndex>
  static void vector(const ColumnString::Chars &data, const ColumnArray::Offsets &offsets, const ColumnString::Offsets &string_offsets,
                     const PaddedPODArray<TIndex> &indices, ColumnString::Chars &result_data, ColumnArray::Offsets &result_offsets,
                     ArrayImpl::NullMapBuilder<mode> &builder) {
    size_t size = offsets.size();
    result_offsets.resize(size);
...
struct ArrayElementGenericImpl {
  template <bool negative>
  static void vectorConst(const IColumn &data, const ColumnArray::Offsets &offsets, const ColumnArray::Offset index, IColumn &result,
                          ArrayImpl::NullMapBuilder<mode> &builder) {
    size_t size = offsets.size();
    result.reserve(size);
   ...
   */
  template <typename TIndex>
  static void vector(const IColumn &data, const ColumnArray::Offsets &offsets, const PaddedPODArray<TIndex> &indices, IColumn &result,
                     ArrayImpl::NullMapBuilder<mode> &builder) {
    size_t size = offsets.size();
    result.reserve(size);
...
template <ArrayElementExceptionMode mode>
template <typename DataType>
ColumnPtr FunctionArrayElement<mode>::executeNumberConst(const ColumnsWithTypeAndName &arguments, const DataTypePtr &result_type,
                                                         const Field &index, ArrayImpl::NullMapBuilder<mode> &builder) {
  ...
  if (index.getType() == Field::Types::UInt64 || (index.getType() == Field::Types::Int64 && index.safeGet<Int64>() >= 0)) {
    ...
    ArrayElementNumImpl<DataType, mode>::template vectorConst<true>(col_nested->getData(), col_array->getOffsets(),
                                                                    -(static_cast<UInt64>(index.safeGet<Int64>()) + 1),
                                                                    col_res_vec->getData(), builder);
  } else
...
template <ArrayElementExceptionMode mode>
template <typename IndexType, typename DataType>
ColumnPtr FunctionArrayElement<mode>::executeNumber(const ColumnsWithTypeAndName &arguments, const DataTypePtr &result_type,
                                                    const PaddedPODArray<IndexType> &indices, ArrayImpl::NullMapBuilder<mode> &builder) {
  using ColVecType = ColumnVectorOrDecimal<DataType>;
  ...
  ArrayElementNumImpl<DataType, mode>::template vector<IndexType>(col_nested->getData(), col_array->getOffsets(), indices,
                                                                  col_res_vec->getData(), builder);
...
template <ArrayElementExceptionMode mode>
ColumnPtr FunctionArrayElement<mode>::executeStringConst(const ColumnsWithTypeAndName &arguments, const Field &index,
                                                         ArrayImpl::NullMapBuilder<mode> &builder) {
  const ColumnArray *col_array = checkAndGetColumn<ColumnArray>(arguments[0].column.get());
  ...
  if (index.getType() == Field::Types::UInt64 || (index.getType() == Field::Types::Int64 && index.safeGet<Int64>() >= 0)) {
    ArrayElementStringImpl<mode>::template vectorConst<false>(col_nested->getChars(), col_array->getOffsets(), col_nested->getOffsets(),
                                                              index.safeGet<UInt64>() - 1, col_res->getChars(), col_res->getOffsets(),
                                                              builder);
  } else if (index.getType() == Field::Types::Int64) {
    ArrayElementStringImpl<mode>::template vectorConst<true>(col_nested->getChars(), col_array->getOffsets(), col_nested->getOffsets(),
                                                             -(UInt64(index.safeGet<Int64>()) + 1), col_res->getChars(),
                                                             col_res->getOffsets(), builder);
  } else
    throw Exception(ErrorCodes::LOGICAL_ERROR, "Illegal type of array index");
...
template <ArrayElementExceptionMode mode>
ColumnPtr FunctionArrayElement<mode>::executeArrayStringConst(const ColumnsWithTypeAndName &arguments, const Field &index,
                                                              ArrayImpl::NullMapBuilder<mode> &builder) {
  const ColumnArray *col_array = checkAndGetColumn<ColumnArray>(arguments[0].column.get());
  if (!col_array) return nullptr;

  const ColumnArray *col_nested_array = checkAndGetColumn<ColumnArray>(&col_array->getData());
  if (!col_nested_array) return nullptr;
  ...
  if (!col_nested_elem) return nullptr;
  ...
  if (index.getType() == Field::Types::UInt64 || (index.getType() == Field::Types::Int64 && index.safeGet<Int64>() >= 0)) {
    if (col_nullable)
      ArrayElementArrayStringImpl<mode>::template vectorConst<false, true>(
          col_nested_elem->getChars(), col_array->getOffsets(), col_nested_array->getOffsets(), col_nested_elem->getOffsets(),
          &string_null_map->getData(), index.safeGet<UInt64>() - 1, res_string->getChars(), res_offsets->getData(),
          res_string->getOffsets(), &res_string_null_map->getData(), builder);
    else
      ArrayElementArrayStringImpl<mode>::template vectorConst<false, false>(
          col_nested_elem->getChars(), col_array->getOffsets(), col_nested_array->getOffsets(), col_nested_elem->getOffsets(), nullptr,
          index.safeGet<UInt64>() - 1, res_string->getChars(), res_offsets->getData(), res_string->getOffsets(), nullptr, builder);
  } else if (index.getType() == Field::Types::Int64) {
    if (col_nullable)
      ArrayElementArrayStringImpl<mode>::template vectorConst<true, true>(
          col_nested_elem->getChars(), col_array->getOffsets(), col_nested_array->getOffsets(), col_nested_elem->getOffsets(),
          &string_null_map->getData(), -(UInt64(index.safeGet<Int64>()) + 1), res_string->getChars(), res_offsets->getData(),
          res_string->getOffsets(), &res_string_null_map->getData(), builder);
    else
      ArrayElementArrayStringImpl<mode>::template vectorConst<true, false>(
          col_nested_elem->getChars(), col_array->getOffsets(), col_nested_array->getOffsets(), col_nested_elem->getOffsets(), nullptr,
          -(UInt64(index.safeGet<Int64>()) + 1), res_string->getChars(), res_offsets->getData(), res_string->getOffsets(), nullptr,
          builder);
  } else
    throw Exception(ErrorCodes::LOGICAL_ERROR, "Illegal type of array index");
...
template <ArrayElementExceptionMode mode>
template <typename DataType>
ColumnPtr FunctionArrayElement<mode>::executeArrayNumberConst(const ColumnsWithTypeAndName &arguments, const DataTypePtr &result_type,
                                                              const Field &index, ArrayImpl::NullMapBuilder<mode> &builder) {
  using ColVecType = ColumnVectorOrDecimal<DataType>;
  ...
  if (index.getType() == Field::Types::UInt64 || (index.getType() == Field::Types::Int64 && index.safeGet<Int64>() >= 0)) {
    if (col_nullable)
      ArrayElementArrayNumImpl<DataType, mode>::template vectorConst<false, true>(
          col_nested_elem->getData(), col_array->getOffsets(), col_nested_array->getOffsets(), null_map, index.safeGet<UInt64>() - 1,
          res_data->getData(), res_offsets, res_null_map, builder);
    else
    ...
    if (col_nullable)
      ArrayElementArrayNumImpl<DataType, mode>::template vectorConst<true, true>(
          col_nested_elem->getData(), col_array->getOffsets(), col_nested_array->getOffsets(), null_map,
          -(static_cast<UInt64>(index.safeGet<Int64>()) + 1), res_data->getData(), res_offsets, res_null_map, builder);
    else
...
template <ArrayElementExceptionMode mode>
template <typename IndexType, typename DataType>
ColumnPtr FunctionArrayElement<mode>::executeArrayNumber(const ColumnsWithTypeAndName &arguments, const DataTypePtr &result_type,
  ...
  if (col_nullable)
    ArrayElementArrayNumImpl<DataType, mode>::template vector<IndexType, true>(col_nested_elem->getData(), col_array->getOffsets(),
                                                                               col_nested_array->getOffsets(), null_map, indices,
                                                                               res_data->getData(), res_offsets, res_null_map, builder);
  else
    ArrayElementArrayNumImpl<DataType, mode>::template vector<IndexType, false>(col_nested_elem->getData(), col_array->getOffsets(),
                                                                                col_nested_array->getOffsets(), null_map, indices,
                                                                                res_data->getData(), res_offsets, res_null_map, builder);
  return res;
}
...
template <ArrayElementExceptionMode mode>
template <typename IndexType>
ColumnPtr FunctionArrayElement<mode>::executeString(const ColumnsWithTypeAndName &arguments, const PaddedPODArray<IndexType> &indices,
                                                    ArrayImpl::NullMapBuilder<mode> &builder) {
  const ColumnArray *col_array = checkAndGetColumn<ColumnArray>(arguments[0].column.get());
  ...
  ArrayElementStringImpl<mode>::template vector<IndexType>(col_nested->getChars(), col_array->getOffsets(), col_nested->getOffsets(),
                                                           indices, col_res->getChars(), col_res->getOffsets(), builder);
...
template <ArrayElementExceptionMode mode>
template <typename IndexType>
ColumnPtr FunctionArrayElement<mode>::executeArrayString(const ColumnsWithTypeAndName &arguments, const PaddedPODArray<IndexType> &indices,
                                                         ArrayImpl::NullMapBuilder<mode> &builder) {
  const ColumnArray *col_array = checkAndGetColumn<ColumnArray>(arguments[0].column.get());
  if (!col_array) return nullptr;
  ...
  if (col_nullable)
    ArrayElementArrayStringImpl<mode>::template vector<IndexType, true>(
        col_nested_elem->getChars(), col_array->getOffsets(), col_nested_array->getOffsets(), col_nested_elem->getOffsets(),
        &string_null_map->getData(), indices, res_string->getChars(), res_offsets->getData(), res_string->getOffsets(),
        &res_string_null_map->getData(), builder);
  else
    ArrayElementArrayStringImpl<mode>::template vector<IndexType, false>(
        col_nested_elem->getChars(), col_array->getOffsets(), col_nested_array->getOffsets(), col_nested_elem->getOffsets(), nullptr,
        indices, res_string->getChars(), res_offsets->getData(), res_string->getOffsets(), nullptr, builder);
...
template <ArrayElementExceptionMode mode>
ColumnPtr FunctionArrayElement<mode>::executeGenericConst(const ColumnsWithTypeAndName &arguments, const Field &index,
                                                          ArrayImpl::NullMapBuilder<mode> &builder) {
  const ColumnArray *col_array = checkAndGetColumn<ColumnArray>(arguments[0].column.get());
  ...
  if (index.getType() == Field::Types::UInt64 || (index.getType() == Field::Types::Int64 && index.safeGet<Int64>() >= 0))
    ArrayElementGenericImpl<mode>::template vectorConst<false>(col_nested, col_array->getOffsets(), index.safeGet<UInt64>() - 1, *col_res,
                                                               builder);
  else if (index.getType() == Field::Types::Int64)
    ArrayElementGenericImpl<mode>::template vectorConst<true>(col_nested, col_array->getOffsets(),
                                                              -(static_cast<UInt64>(index.safeGet<Int64>() + 1)), *col_res, builder);
  else
    throw Exception(ErrorCodes::LOGICAL_ERROR, "Illegal type of array index");
...
ColumnPtr FunctionArrayElement<mode>::removeNullableIfNeeded(const ColumnPtr &column, const DataTypePtr &expect_type) {
  ...
  if (nullable_column && !expect_type->isNullable()) {
    ...
    /// The former return type Tuple(T1, T2) because Tuple can't be wrapped into Nullable. But the latter return type Tuple(Nullable(T1),
    /// Nullable(T2)), which is different. To keep return type consistency, we need to unwrap the Nullable column returned by
    /// ArrayElementOrNull(T1) and ArrayElementOrNull(T2)
...
template <ArrayElementExceptionMode mode>
template <typename IndexType>
ColumnPtr FunctionArrayElement<mode>::executeGeneric(const ColumnsWithTypeAndName &arguments, const PaddedPODArray<IndexType> &indices,
                                                     ArrayImpl::NullMapBuilder<mode> &builder) {
  const ColumnArray *col_array = checkAndGetColumn<ColumnArray>(arguments[0].column.get());
...
template <ArrayElementExceptionMode mode>
template <typename IndexType>
ColumnPtr FunctionArrayElement<mode>::executeConst(const ColumnsWithTypeAndName &arguments, const DataTypePtr &result_type,
                                                   const PaddedPODArray<IndexType> &indices, ArrayImpl::NullMapBuilder<mode> &builder,
                                                   size_t input_rows_count) {
...
template <ArrayElementExceptionMode mode>
template <typename IndexType>
ColumnPtr FunctionArrayElement<mode>::executeArgument(const ColumnsWithTypeAndName &arguments, const DataTypePtr &result_type,
                                                      ArrayImpl::NullMapBuilder<mode> &builder, size_t input_rows_count) const {
  auto index = checkAndGetColumn<ColumnVector<IndexType>>(arguments[1].column.get());
  if (!index) return nullptr;
  const auto &index_data = index->getData();

  if (builder) builder.initSink(index_data.size());
...
ColumnPtr FunctionArrayElement<mode>::executeMap2(const ColumnsWithTypeAndName &arguments, size_t input_rows_count) const {
  ...
  const DataTypeMap &map_type = typeid_cast<const DataTypeMap &>(*typeid_cast<const DataTypeArray &>(*arguments[0].type).getNestedType());
...
template <ArrayElementExceptionMode mode>
ColumnPtr FunctionArrayElement<mode>::executeTuple(const ColumnsWithTypeAndName &arguments, ArrayImpl::NullMapBuilder<mode> &builder,
                                                   size_t input_rows_count) const {
  const ColumnArray *col_array = typeid_cast<const ColumnArray *>(arguments[0].column.get());
  if (!col_array) return nullptr;
  ...
  if (builder) {
    ...
    if (input_rows_count != 0) chassert(col_nested->size() == offsets.back());
    ...
    if (!isColumnConst(*index_col)) {
      ...
      if (index_field.getType() != Field::Types::UInt64 && index_field.getType() != Field::Types::Int64) return nullptr;
      ...
      if constexpr (!is_null_mode) {
        ...
        if (idx == 0) throw Exception(ErrorCodes::ZERO_ARRAY_OR_TUPLE_INDEX, "Array indices are 1-based");
      }
...
template <ArrayElementExceptionMode mode>
template <typename Matcher>
void FunctionArrayElement<mode>::executeMatchKeyToIndex(const Offsets &offsets, PaddedPODArray<UInt64> &matched_idxs,
  ...
  for (; i < rows; ++i) {
    ...
    if (!matched) matched_idxs.push_back(0);
  }
}

template <ArrayElementExceptionMode mode>
template <typename Matcher>
void FunctionArrayElement<mode>::executeMatchConstKeyToIndex(size_t num_rows, size_t num_values, PaddedPODArray<UInt64> &matched_idxs,
                                                             const Matcher &matcher) {
...
template <ArrayElementExceptionMode mode>
bool FunctionArrayElement<mode>::matchKeyToIndexStringConst(const IColumn &data, const Offsets &offsets, const Field &index,
                                                            PaddedPODArray<UInt64> &matched_idxs) {
  return castColumnString(&data, [&](const auto &data_column) {
    using DataColumn = std::decay_t<decltype(data_column)>;
    if (index.getType() != Field::Types::String) return false;
...
template <ArrayElementExceptionMode mode>
bool FunctionArrayElement<mode>::matchKeyToIndexString(const IColumn &data, const Offsets &offsets, bool is_key_const, const IColumn &index,
                                                       PaddedPODArray<UInt64> &matched_idxs) {
  return castColumnString(&data, [&](const auto &data_column) {
...
bool castColumnNumeric(const IColumn *column, F &&f) {
  return castTypeToEither<ColumnVector<UInt8>, ColumnVector<UInt16>, ColumnVector<UInt32>, ColumnVector<UInt64>, ColumnVector<UInt128>,
                          ColumnVector<UInt256>, ColumnVector<Int8>, ColumnVector<Int16>, ColumnVector<Int32>, ColumnVector<Int64>,
                          ColumnVector<Int128>, ColumnVector<Int256>, ColumnVector<UUID>, ColumnVector<IPv4>, ColumnVector<IPv6>>(
      column, std::forward<F>(f));
}

template <ArrayElementExceptionMode mode>
bool FunctionArrayElement<mode>::matchKeyToIndexNumberConst(const IColumn &data, const Offsets &offsets, const Field &index,
                                                            PaddedPODArray<UInt64> &matched_idxs) {
  return castColumnNumeric(&data, [&](const auto &data_column) {
    ...
    Field::dispatch(
        [&](const auto &value) {
          ...
          if constexpr (areConvertibleTypes<FieldType, DataType>) index_as_integer = static_cast<DataType>(value);
        },
        index);

    if (!index_as_integer) return false;
...
template <ArrayElementExceptionMode mode>
bool FunctionArrayElement<mode>::matchKeyToIndexNumber(const IColumn &data, const Offsets &offsets, bool is_key_const, const IColumn &index,
                                                       PaddedPODArray<UInt64> &matched_idxs) {
  return castColumnNumeric(&data, [&](const auto &data_column) {
...
template <ArrayElementExceptionMode mode>
ColumnPtr FunctionArrayElement<mode>::executeMap(const ColumnsWithTypeAndName &arguments, const DataTypePtr &result_type,
                                                 size_t input_rows_count) const {
  ...
  if (col_const_map) col_map = typeid_cast<const ColumnMap *>(&col_const_map->getDataColumn());
  ...
  if (!executed)
    throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT, "Illegal types of arguments: {}, {} for function {}",
  ...
  if (col_const_map) values_array = ColumnConst::create(values_array, input_rows_count);
  ...
  ColumnsWithTypeAndName new_arguments = {{values_array, std::make_shared<DataTypeArray>(type_map.getValueType()), ""},
...
DataTypePtr FunctionArrayElement<mode>::getReturnTypeImpl(const DataTypes &arguments) const {
  ...
  if (!array_type) {
    throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT, "First argument for function '{}' must be array, got '{}' instead", getName(),
                    arguments[0]->getName());
  }

  if (!isNativeInteger(arguments[1])) {
    throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT, "Second argument for function '{}' must be integer, got '{}' instead", getName(),
                    arguments[1]->getName());
  }
...
template <ArrayElementExceptionMode mode>
ColumnPtr FunctionArrayElement<mode>::executeImpl(const ColumnsWithTypeAndName &arguments, const DataTypePtr &result_type,
                                                  size_t input_rows_count) const {
  const auto *col_map = checkAndGetColumn<ColumnMap>(arguments[0].column.get());
  const auto *col_const_map = checkAndGetColumnConst<ColumnMap>(arguments[0].column.get());

  if (col_map || col_const_map) return executeMap(arguments, result_type, input_rows_count);
  ...
  if (!is_array_of_nullable) {
    ...
    if (builder && res->canBeInsideNullable()) return ColumnNullable::create(res, std::move(builder).getNullMapColumnPtr());
...
template <ArrayElementExceptionMode mode>
ColumnPtr FunctionArrayElement<mode>::perform(const ColumnsWithTypeAndName &arguments, const DataTypePtr &result_type,
                                              ArrayImpl::NullMapBuilder<mode> &builder, size_t input_rows_count) const {
  ColumnPtr res;
  if ((res = executeTuple(arguments, builder, input_rows_count))) return res;
  if ((res = executeMap2(arguments, input_rows_count))) return res;
  if (!isColumnConst(*arguments[1].column)) {
    ...
    if (builder) builder.initSink(input_rows_count);

    if constexpr (!is_null_mode) {
      if (index == 0u) {
        /// It is very confusing that arrayElement throws an exception only for a constant index 0, but return default value for a
        /// non-constant index 0. e.g. `select [1,2,3][0]` throws exception, but `select [1,2,3][number] from numbers(1)` returns 0.
...
Operator `[n]` provides the same functionality.
  ...
  FunctionDocumentation::Arguments arguments = {{"arr", "The array to search. [`Array(T)`](/sql-reference/data-types/array)."},
...
Negative indexes are supported. In this case, it selects the corresponding element numbered from the end. For example, `arr[-1]` is the last item in the array.
  ...
  FunctionDocumentation::Arguments arguments_null = {{"arrays", "Arbitrary number of array arguments.", {"Array"}}};
  ...
  FunctionDocumentation documentation_null = {description_null,    syntax_null,   arguments_null,     {},
                                              returned_value_null, examples_null, introduced_in_null, category_null};
