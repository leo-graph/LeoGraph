...
class FunctionArrayJaccardIndex : public IFunction {
 private:
  ...
  template <bool left_is_const, bool right_is_const>
  static void vector(const ColumnArray::Offsets &intersect_offsets, const ColumnArray::Offsets &left_offsets,
                     const ColumnArray::Offsets &right_offsets, PaddedPODArray<ResultType> &res) {
  ...
  template <bool left_is_const, bool right_is_const>
  static void vectorWithEmptyIntersect(const ColumnArray::Offsets &left_offsets, const ColumnArray::Offsets &right_offsets,
 ...
 public:
  ...
  ColumnPtr executeImpl(const ColumnsWithTypeAndName &arguments, const DataTypePtr &, size_t input_rows_count) const override {
    auto cast_to_array = [&](const ColumnWithTypeAndName &col) -> std::pair<const ColumnArray *, bool> {
      ...
      if (const ColumnArray *col_non_const_array = checkAndGetColumn<ColumnArray>(col.column.get())) return {col_non_const_array, false};
      throw Exception(ErrorCodes::ILLEGAL_COLUMN, "Argument for function {} must be array but it has type {}.", col.column->getName(),
                      getName());
    };
    ...
    if (!intersect_column_type) throw Exception(ErrorCodes::LOGICAL_ERROR, "Unexpected return type for function arrayIntersect");
...
#define EXECUTE_VECTOR(left_is_const, right_is_const)                                                                               \
  if (typeid_cast<const DataTypeNothing *>(intersect_column_type->getNestedType().get()))                                           \
    ...
    vector<left_is_const, right_is_const>(intersect_column_array.getOffsets(), left_array->getOffsets(), right_array->getOffsets(), \
                                          vec_res);                                                                                 \
  }
