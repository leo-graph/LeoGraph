...
void processImpl(ConstColumnInfo<ColumnT> source, ConstColumnInfo<ColumnT> exclude, ColumnInfo<ColumnT> result, size_t input_rows_count) {
  ...
  if constexpr (nullable) result.null_map->resize(source.null_map->size());
  ...
  auto reset = [&] {
    exclude_set.clear();
    if constexpr (nullable) exclude_has_null = false;
  };
  ...
  if constexpr (exclude_single_row) populate_excludes(0, exclude.col.size());
  ...
  for (size_t row = 0; row < input_rows_count; ++row) {
    ...
    if constexpr (!exclude_single_row) current_exclude_offset = exclude.offsets[row];
  }

  if constexpr (nullable) result.null_map->resize(current_result_offset);
}
...
class FunctionArrayExcept : public IFunction {
 public:
  ...
  ColumnPtr executeImpl(const ColumnsWithTypeAndName &arguments, const DataTypePtr &return_type, size_t input_rows_count) const override {
    ...
    const ColumnArray *exclude_col =
        exclude_is_const ? typeid_cast<const ColumnArray *>(&typeid_cast<const ColumnConst *>(arguments[1].column.get())->getDataColumn())
                         : checkAndGetColumn<ColumnArray>(arguments[1].column.get());

    if (!source_col || !exclude_col) throw Exception(ErrorCodes::LOGICAL_ERROR, "Arguments must be arrays");
...
3. NULL is handled as a separate value
  ...
  FunctionDocumentation::Arguments arguments = {{"source", "The source array containing elements to filter. ", {"Array(T)"}},
                                                {"except", "The array containing elements to exclude from the result. ", {"Array(T)"}}};

  FunctionDocumentation::ReturnedValue returned_value = {
      "Returns an array of the same type as the input array containing elements from `source` that weren't found in `except`. ",
      {"Array(T)"}};
