...
class FunctionArrayLevenshtein : public IFunction {
  ...
  DataTypePtr getReturnTypeImpl(const ColumnsWithTypeAndName &arguments) const override {
    ...
    for (size_t index = 2; index < 4; ++index) {
      ...
      if (!(isFloat(nested_type) || isInteger(nested_type)))
        throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT, "Argument {} of function {} must be a numeric array. Found {} instead.",
                        toString(index + 1), getName(), nested_type->getName());
    }

    if (nested_types[0]->getName() != nested_types[1]->getName())
      throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT,
                      "Arguments 3 and 4 of function {} must be arrays of the same types. Found {} and {} instead.", getName(),
                      nested_types[0]->getName(), nested_types[1]->getName());
    ...
    for (size_t i = 0; i < num_arguments; ++i) {
      ...
      if (holders[i]->size() != input_rows_count)
        throw Exception(ErrorCodes::LOGICAL_ERROR,
                        ...
                        getName(), input_rows_count, holders[i]->size(), holders[i]->getName());
 ...
 private:
  ...
  bool levenshteinString(std::vector<const ColumnArray *> columns, Result::Container &res_values) const {
    ...
    if (T::arguments == 4) {
      if constexpr (std::is_same_v<Result, ColumnFloat64>) {
        if (!(levenshteinWeightedString<N, UInt8>(columns, res_values) || levenshteinWeightedString<N, UInt16>(columns, res_values) ||
              levenshteinWeightedString<N, UInt32>(columns, res_values) || levenshteinWeightedString<N, UInt64>(columns, res_values) ||
              levenshteinWeightedString<N, UInt128>(columns, res_values) || levenshteinWeightedString<N, UInt256>(columns, res_values) ||
              levenshteinWeightedString<N, Int8>(columns, res_values) || levenshteinWeightedString<N, Int16>(columns, res_values) ||
              levenshteinWeightedString<N, Int32>(columns, res_values) || levenshteinWeightedString<N, Int64>(columns, res_values) ||
              levenshteinWeightedString<N, Int128>(columns, res_values) || levenshteinWeightedString<N, Int256>(columns, res_values) ||
              levenshteinWeightedString<N, Float32>(columns, res_values) || levenshteinWeightedString<N, Float64>(columns, res_values)))
          throw Exception(ErrorCodes::LOGICAL_ERROR, "Unexpected code branch of function {}. No fitting levenshteinWeightedString",
                          getName());
        return true;
      }
      throw Exception(ErrorCodes::LOGICAL_ERROR, "Unexpected code branch of function {}. Wrong expected result type in levenshteinString",
                      getName());
    }
    ...
    const auto extract_array = [](const N *data, size_t prev_offset, size_t count) {
      ...
      for (size_t j = 0; j < count; ++j) {
        temp.emplace_back(data->getDataAt(prev_offset + j));
      }
      return temp;
    };
  ...
  bool levenshteinNumber(std::vector<const ColumnArray *> columns, Result::Container &res_values) const {
    ...
    if (!column_from || !column_to) return false;

    if (T::arguments == 4) {
      if constexpr (std::is_same_v<Result, ColumnFloat64>) {
        if (!(levenshteinWeightedNumber<N, UInt8>(columns, res_values) || levenshteinWeightedNumber<N, UInt16>(columns, res_values) ||
              levenshteinWeightedNumber<N, UInt32>(columns, res_values) || levenshteinWeightedNumber<N, UInt64>(columns, res_values) ||
              levenshteinWeightedNumber<N, UInt128>(columns, res_values) || levenshteinWeightedNumber<N, UInt256>(columns, res_values) ||
              levenshteinWeightedNumber<N, Int8>(columns, res_values) || levenshteinWeightedNumber<N, Int16>(columns, res_values) ||
              levenshteinWeightedNumber<N, Int32>(columns, res_values) || levenshteinWeightedNumber<N, Int64>(columns, res_values) ||
              levenshteinWeightedNumber<N, Int128>(columns, res_values) || levenshteinWeightedNumber<N, Int256>(columns, res_values) ||
              levenshteinWeightedNumber<N, Float32>(columns, res_values) || levenshteinWeightedNumber<N, Float64>(columns, res_values)))
          throw Exception(ErrorCodes::LOGICAL_ERROR, "Unexpected code branch of function {}. No fitting levenshteinWeightedNumber",
                          getName());
        return true;
      }
      throw Exception(ErrorCodes::LOGICAL_ERROR, "Unexpected code branch of function {}. Wrong expected result type in levenshteinNumber",
                      getName());
    }
  ...
  void levenshteinGeneric(std::vector<const ColumnArray *> columns, Result::Container &res_values) const {
    if (T::arguments == 4) {
      if constexpr (std::is_same_v<Result, ColumnFloat64>) {
        if (!(levenshteinWeightedGeneric<UInt8>(columns, res_values) || levenshteinWeightedGeneric<UInt16>(columns, res_values) ||
              levenshteinWeightedGeneric<UInt32>(columns, res_values) || levenshteinWeightedGeneric<UInt64>(columns, res_values) ||
              levenshteinWeightedGeneric<UInt128>(columns, res_values) || levenshteinWeightedGeneric<UInt256>(columns, res_values) ||
              levenshteinWeightedGeneric<Int8>(columns, res_values) || levenshteinWeightedGeneric<Int16>(columns, res_values) ||
              levenshteinWeightedGeneric<Int32>(columns, res_values) || levenshteinWeightedGeneric<Int64>(columns, res_values) ||
              levenshteinWeightedGeneric<Int128>(columns, res_values) || levenshteinWeightedGeneric<Int256>(columns, res_values) ||
              levenshteinWeightedGeneric<Float32>(columns, res_values) || levenshteinWeightedGeneric<Float64>(columns, res_values)))
          throw Exception(ErrorCodes::LOGICAL_ERROR, "Unexpected code branch of function {}. No fitting levenshteinWeightedGeneric",
                          getName());
        return;
      }
      throw Exception(ErrorCodes::LOGICAL_ERROR, "Unexpected code branch of function {}. Wrong expected result type in levenshteinGeneric",
                      getName());
    }
  ...
  bool levenshteinWeightedString(std::vector<const ColumnArray *> columns, ColumnFloat64::Container &res_values) const {
    ...
    if (!column_from_weights || !column_to_weights) return false;
    ...
    for (size_t row = 0; row < columns[0]->size(); row++) {
      ...
      std::span<const W> from_weights(column_from_weights->getData().begin() + prev_from_weights_offset,
                                      from_weights_offsets[row] - prev_from_weights_offset);
      prev_from_weights_offset = from_weights_offsets[row];
      std::span<const W> to_weights(column_to_weights->getData().begin() + prev_to_weights_offset,
                                    to_weights_offsets[row] - prev_to_weights_offset);
  ...
  ColumnPtr levenshteinImpl(std::vector<const ColumnArray *> columns) const {
    ...
    if (tryLevenshteinNumber<ColumnUInt32, UInt8, UInt16, UInt32, UInt64, UInt128, UInt256, Int8, Int16, Int32, Int64, Int128, Int256,
                             Float32, Float64, Decimal32, Decimal64, Decimal128, Decimal256, DateTime64>(columns, res_values) ||
      ...
      for (size_t row = 0; row < hs_column->size(); row++) {
        ...
        if (hs.size() != weights.size())
          throw Exception(ErrorCodes::SIZES_OF_ARRAYS_DONT_MATCH,
                          "Arguments {} ({}, size {}) and {} ({}, size {}) of function {} must be arrays of the same size", toString(i + 1),
                          hs_column->getName(), hs.size(), toString(i + 3), weights_column->getName(), weights.size(), getName());
      }
    }
    auto res = ColumnFloat64::create();
    ColumnFloat64::Container &res_values = res->getData();
    res_values.resize(columns[0]->size());
    if (tryLevenshteinNumber<ColumnFloat64, UInt8, UInt16, UInt32, UInt64, UInt128, UInt256, Int8, Int16, Int32, Int64, Int128, Int256,
                             Float32, Float64, Decimal32, Decimal64, Decimal128, Decimal256, DateTime64>(columns, res_values) ||
  ...
  bool similarity(std::vector<const ColumnArray *> columns, ColumnPtr distance, ColumnFloat64::Container &res_values) const {
    ...
    for (size_t row = 0; row < distance->size(); row++) {
      ...
      W weights_sum =
          std::accumulate(from_weights.begin(), from_weights.end(), W{}) + std::accumulate(to_weights.begin(), to_weights.end(), W{});
      if (weights_sum == 0) {
        res_values[row] = 1.0;
        continue;
      }
...
ColumnPtr FunctionArrayLevenshtein<Similarity>::execute(std::vector<const ColumnArray *> columns) const {
  ...
  if (!(similarity<UInt8>(columns, distance, res_values) || similarity<UInt16>(columns, distance, res_values) ||
    ...
    throw Exception(ErrorCodes::LOGICAL_ERROR, "Unexpected code branch of function {}. No matching column types for {} and {}", getName(),
                    columns[2]->getName(), columns[3]->getName());
  return result;
}
...
REGISTER_FUNCTION(ArrayLevenshtein) {
  ...
  FunctionDocumentation::Arguments arguments_arrayLevDis = {{"from", "The first array. [`Array(T)`](/sql-reference/data-types/array)."},
                                                            {"to", "The second array. [`Array(T)`](/sql-reference/data-types/array)."}};
  FunctionDocumentation::ReturnedValue returned_value_arrayLevDis = {"Levenshtein distance between the first and the second arrays.",
                                                                     {"Float64"}};
  FunctionDocumentation::Examples example_arrayLevDis = {{"Usage example", "SELECT arrayLevenshteinDistance([1, 2, 4], [1, 2, 3])", "1"}};
  ...
  FunctionDocumentation documentation_arrayLevDis = {
      description_arrayLevDis,    syntax_arrayLevDis,  arguments_arrayLevDis,     {},
      returned_value_arrayLevDis, example_arrayLevDis, introduced_in_arrayLevDis, category_arrayLevDis};
...
The number of elements for the array and its weights should match.
  ...
  FunctionDocumentation::ReturnedValue returned_value_arrayLevDisW = {
  ...
  FunctionDocumentation::Examples examples_arrayLevDisW = {
      {"Usage example", "SELECT arrayLevenshteinDistanceWeighted(['A', 'B', 'C'], ['A', 'K', 'L'], [1.0, 2, 3], [3.0, 4, 5])", "14"}};
  ...
  FunctionDocumentation documentation_arrayLevDisW = {
      description_arrayLevDisW,    syntax_arrayLevDisW,   arguments_arrayLevDisW,     {},
      returned_value_arrayLevDisW, examples_arrayLevDisW, introduced_in_arrayLevDisW, category_arrayLevDisW};
...
Calculates the similarity of two arrays from `0` to `1` based on weighted Levenshtein distance.
  ...
  FunctionDocumentation::ReturnedValue returned_value_arraySim = {
  ...
  FunctionDocumentation::Examples examples_arraySim = {
      {"Usage example", "SELECT arraySimilarity(['A', 'B', 'C'], ['A', 'K', 'L'], [1.0, 2, 3], [3.0, 4, 5]);", "0.2222222222222222"}};
  ...
  FunctionDocumentation documentation_arraySim = {description_arraySim,    syntax_arraySim,   arguments_arraySim,     {},
                                                  returned_value_arraySim, examples_arraySim, introduced_in_arraySim, category_arraySim};
