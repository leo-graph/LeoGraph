...
using FunctionReplaceRegexpAll =
    FunctionStringReplace<ReplaceRegexpImpl<NameReplaceRegexpAll, ReplaceRegexpTraits::All>, NameReplaceRegexpAll>;
...
As an exception, if a regular expression worked on an empty substring, the replacement is not made more than once.
  ...
  FunctionDocumentation::Examples examples = {
      {"Replace all characters with doubled version", R"(SELECT replaceRegexpAll('Hello123', '.', '\\\\0\\\\0') AS res)",
       R"(
┌─res──────────────────┐
│ HHeelllloo112233     │
└──────────────────────┘
        )"},
      {"Empty substring replacement example", "SELECT replaceRegexpAll('Hello, World!', '^', 'here: ') AS res",
       R"(
┌─res─────────────────┐
│ here: Hello, World! │
└─────────────────────┘
