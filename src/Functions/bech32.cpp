...
void finalizeRow(DB::ColumnString::Offsets &offsets, char *&pos, const char *const begin, const size_t i) { offsets[i] = pos - begin; }
...
class EncodeToBech32Representation : public IFunction {
 public:
  ...
  DataTypePtr getReturnTypeImpl(const DataTypes &arguments) const override {
    if (arguments.size() < 2)
      throw Exception(ErrorCodes::TOO_FEW_ARGUMENTS_FOR_FUNCTION,
                      "At least two string arguments (human_readable_part, data) are required for function {}", getName());

    if (arguments.size() > 3)
      throw Exception(ErrorCodes::TOO_MANY_ARGUMENTS_FOR_FUNCTION,
                      "A maximum of 3 arguments (human_readable_part, data, witness_version) are allowed for function {}", getName());
    ...
    for (size_t i = 0; i < 2; ++i)
      if (!WhichDataType(arguments[i]).isStringOrFixedString())
        throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT,
                        "Illegal type {} of argument {} of function {}, expected String or FixedString", arguments[i]->getName(), i + 1,
                        getName());
    ...
    if (arguments.size() == 3 && !WhichDataType(arguments[arg_idx]).isNativeUInt())
      throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT, "Illegal type {} of argument {} of function {}, expected unsigned integer",
  ...
  ColumnPtr executeImpl(const ColumnsWithTypeAndName &arguments, const DataTypePtr &, size_t input_rows_count) const override {
    ...
    ColumnPtr col2;
    if (have_witness_version) col2 = arguments[2].column;
 ...
 private:
  ColumnPtr chooseCol1AndExecute(const ColumnString::Chars &col0_vec, const ColumnString::Offsets *col0_offsets, const ColumnPtr &col1,
                                 const ColumnPtr &col2, const size_t input_rows_count, const bool have_witness_version = false,
  ...
  static ColumnPtr execute(const ColumnString::Chars &human_readable_part_vec, const ColumnString::Offsets *human_readable_part_offsets,
                           const ColumnString::Chars &data_vec, const ColumnString::Offsets *data_offsets,
                           const ColumnPtr &witness_version_col, const size_t input_rows_count, const bool have_witness_version = false,
                           const size_t human_readable_part_width = 0, const size_t data_width = 0) {
    /// outputs
    ...
    for (size_t i = 0; i < input_rows_count; ++i) {
      size_t human_readable_part_new_offset =
          human_readable_part_width == 0 ? (*human_readable_part_offsets)[i] : human_readable_part_prev_offset + human_readable_part_width;
      ...
      std::string human_readable_part(reinterpret_cast<const char *>(&human_readable_part_vec[human_readable_part_prev_offset]),
                                      reinterpret_cast<const char *>(&human_readable_part_vec[human_readable_part_new_offset]));

      bech32_data input(reinterpret_cast<const uint8_t *>(&data_vec[data_prev_offset]),
      ...
      if (witness_version > 16) {
        throw Exception(ErrorCodes::BAD_ARGUMENTS, "Invalid witness version {} for function {}, expected value in range [0, 16]",
                        witness_version, name);
      }
      ...
      std::string address =
          bech32::encode(human_readable_part, input_5bit, witness_version > 0 ? bech32::Encoding::BECH32M : bech32::Encoding::BECH32);
    ...
    chassert(static_cast<size_t>(out_pos - out_begin) <= out_vec.size(),
...
class DecodeFromBech32Representation : public IFunction {
 public:
  ...
  DataTypePtr getReturnTypeImpl(const DataTypes &arguments) const override {
    ...
    for (size_t i = 0; i < tuple_size; ++i) types[i] = std::make_shared<DataTypeString>();
 ...
 private:
  static ColumnPtr execute(const ColumnString::Chars &in_vec, const ColumnString::Offsets *in_offsets, size_t input_rows_count,
                           size_t col_width = 0) {
    ...
    for (size_t i = 0; i < input_rows_count; ++i) {
      ...
      /// NUL chars are used to pad fixed width strings, so we remove them here since they are not valid inputs anyway
      while (col_width > 0 && in_vec[new_offset - 1] == 0 && new_offset > prev_offset) --new_offset;
      ...
      std::string input(reinterpret_cast<const char *>(&in_vec[prev_offset]), reinterpret_cast<const char *>(&in_vec[new_offset]));
    ...
    chassert(static_cast<size_t>(human_readable_part_pos - human_readable_part_begin) <= human_readable_part_vec.size(),
             fmt::format("too small amount of memory was preallocated: needed {}, but have only {}",
                         human_readable_part_pos - human_readable_part_begin, human_readable_part_vec.size()));
    chassert(
        static_cast<size_t>(data_pos - data_begin) <= data_vec.size(),
        fmt::format("too small amount of memory was preallocated: needed {}, but have only {}", data_pos - data_begin, data_vec.size()));
...
:::
  ...
  FunctionDocumentation::Arguments bech32Encode_arguments = {
      {"hrp",
       "A String of `1 - 83` lowercase characters specifying the \"human-readable part\" of the code. Usually 'bc' or 'tb'.",
       {"String", "FixedString"}},
      {"data", "A String of binary data to encode.", {"String", "FixedString"}},
      {"witver",
       "Optional. The witness version (default = 1). An `UInt*` specifying the version of the algorithm to run. `0` for Bech32 and `1` or "
       "greater for Bech32m.",
       {"UInt*"}}};
  FunctionDocumentation::ReturnedValue bech32Encode_returned_value = {
      "Returns a Bech32 address string, consisting of the human-readable part, a separator character which is always '1', and a data part. "
      "The length of the string will never exceed 90 characters. If the algorithm cannot generate a valid address from the input, it will "
      "return an empty string.",
      {"String"}};
  FunctionDocumentation::Examples bech32Encode_examples = {{"Default Bech32m",
                                                            R"(
...
SELECT bech32Encode('abcdefg', unhex('751e76e8199196d454941c45d1b3a323f1433bd6'), 10)
  ...
  FunctionDocumentation bech32Encode_documentation = {
      bech32Encode_description,    bech32Encode_syntax,   bech32Encode_arguments,     {},
      bech32Encode_returned_value, bech32Encode_examples, bech32Encode_introduced_in, bech32Encode_category};
...
:::
  ...
  FunctionDocumentation::Arguments bech32Decode_arguments = {{"address", "A Bech32 string to decode.", {"String", "FixedString"}}};
  FunctionDocumentation::ReturnedValue bech32Decode_returned_value = {
      "Returns a tuple consisting of `(hrp, data)` that was used to encode the string. The data is in binary format.",
      {"Tuple(String, String)"}};
  ...
  FunctionDocumentation bech32Decode_documentation = {
      bech32Decode_description,    bech32Decode_syntax,   bech32Decode_arguments,     {},
      bech32Decode_returned_value, bech32Decode_examples, bech32Decode_introduced_in, bech32Decode_category};
