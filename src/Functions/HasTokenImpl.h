...
struct HasTokenImpl {
  ...
  static void vectorConstant(const ColumnString::Chars &haystack_data, const ColumnString::Offsets &haystack_offsets,
                             const std::string &pattern, const ColumnPtr &start_pos, PaddedPODArray<UInt8> &res, ColumnUInt8 *res_null,
    ...
    if (pattern.empty()) {
      ...
      if (res_null) std::ranges::fill(res_null->getData(), false);
      return;
    }
    ...
    if (i < input_rows_count) memset(&res[i], negate, (input_rows_count - i) * sizeof(res[0]));
  }
