...
REGISTER_FUNCTION(MultiSearchFirstIndex) {
  ...
  FunctionDocumentation::Examples examples = {
      {"Usage example", "SELECT multiSearchFirstIndex('ClickHouse Database', ['Click', 'Database', 'Server']);",
       R"(
┌─multiSearchF⋯ 'Server'])─┐
│                        1 │
└──────────────────────────┘
        )"},
      {"Case-sensitive behavior", "SELECT multiSearchFirstIndex('ClickHouse Database', ['CLICK', 'Database', 'Server']);",
       R"(
┌─multiSearchF⋯ 'Server'])─┐
│                        2 │
└──────────────────────────┘
        )"},
      {"No match found", "SELECT multiSearchFirstIndex('Hello World', ['goodbye', 'test']);",
       R"(
┌─multiSearchF⋯', 'test'])─┐
│                        0 │
└──────────────────────────┘
