...
std::unique_ptr<ITokenizer> createTokenizer(const ColumnsWithTypeAndName &arguments, std::string_view function_name) {
  const auto tokenizer_str = arguments.size() < 2 || !arguments[arg_tokenizer].column ? SplitByNonAlphaTokenizer::getExternalName()
                                                                                      : arguments[arg_tokenizer].column->getDataAt(0);

  if (arguments.size() <= 2) return TokenizerFactory::instance().get(tokenizer_str);

  FieldVector params;
  for (size_t i = 2; i < arguments.size(); ++i) {
    const auto &col = arguments[i].column;
    if (!col) throw Exception(ErrorCodes::ILLEGAL_COLUMN, "Invalid argument of function {}", function_name);
    ...
    if (which_type.isUInt()) {
      ...
      if (!col_separators && !col_separators_const)
        throw Exception(ErrorCodes::ILLEGAL_COLUMN, "Argument {} of function {} should be Array(String), got: {}", i + 1 /*1-based*/,
                        function_name, col->getFamilyName());

      if (col_separators_const) col_separators = col_separators_const;
...
class ExecutableFunctionTokens : public IExecutableFunction {
 public:
  ...
  explicit ExecutableFunctionTokens(std::shared_ptr<const ITokenizer> tokenizer_) : tokenizer(std::move(tokenizer_)) {}
  ...
  ColumnPtr executeImpl(const ColumnsWithTypeAndName &arguments, const DataTypePtr &, size_t input_rows_count) const override {
    ...
    if (input_rows_count == 0) return ColumnArray::create(std::move(col_result), std::move(col_offsets));
 ...
 private:
  void executeWithTokenizer(const ITokenizer &tokenizer_, ColumnPtr col_input, ColumnArray::ColumnOffsets &col_offsets,
                            size_t input_rows_count, ColumnString &col_result) const {
  ...
  void executeImpl(const ITokenizer &tokenizer_, const StringColumnType &column_input, ColumnArray::ColumnOffsets &column_offsets_input,
                   size_t input_rows_count, ColumnString &column_result) const {
...
class FunctionBaseTokens : public IFunctionBase {
 public:
  ...
  FunctionBaseTokens(std::shared_ptr<const ITokenizer> tokenizer_, DataTypes argument_types_, DataTypePtr result_type_)
      : tokenizer(std::move(tokenizer_)), argument_types(std::move(argument_types_)), result_type(std::move(result_type_)) {}
...
class FunctionTokensOverloadResolver : public IFunctionOverloadResolver {
 public:
  ...
  DataTypePtr getReturnTypeImpl(const ColumnsWithTypeAndName &arguments) const override {
    ...
    if (arguments.size() > 1) {
      optional_args.emplace_back("tokenizer", static_cast<FunctionArgumentDescriptor::TypeValidator>(&isString), isColumnConst,
                                 "const String");
      ...
      if (arguments.size() == 3) {
        ...
        if (tokenizer == NgramsTokenizer::getExternalName())
          optional_args.emplace_back("ngrams", static_cast<FunctionArgumentDescriptor::TypeValidator>(&isUInt8), isColumnConst,
                                     "const UInt8");
        else if (tokenizer == SplitByStringTokenizer::getExternalName())
          optional_args.emplace_back("separators", static_cast<FunctionArgumentDescriptor::TypeValidator>(&isArray), isColumnConst,
                                     "const Array");
        ...
        if (tokenizer == SparseGramsTokenizer::getExternalName()) {
          optional_args.emplace_back("min_length", static_cast<FunctionArgumentDescriptor::TypeValidator>(&isUInt8), isColumnConst,
                                     "const UInt8");
          optional_args.emplace_back("max_length", static_cast<FunctionArgumentDescriptor::TypeValidator>(&isUInt8), isColumnConst,
                                     "const UInt8");
          if (arguments.size() == 5)
            optional_args.emplace_back("min_cutoff_length", static_cast<FunctionArgumentDescriptor::TypeValidator>(&isUInt8), isColumnConst,
                                       "const UInt8");
        }
      }
    }
...
tokens(value, 'array')
  ...
  FunctionDocumentation::Examples examples = {
      {"Default tokenizer", R"(SELECT tokens('test1,;\\\\ test2,;\\\\ test3,;\\\\   test4') AS tokens;)",
       R"(
['test1','test2','test3','test4']
        )"},
      {"Ngram tokenizer", "SELECT tokens('abc def', 'ngrams', 3) AS tokens;",
       R"(
['abc','bc ','c d',' de','def']
        )"}};
