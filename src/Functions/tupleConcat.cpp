 ...
 public:
  ...
  ColumnPtr executeImpl(const ColumnsWithTypeAndName &arguments, const DataTypePtr &, size_t input_rows_count) const override {
    ...
    for (size_t i = 0; i < num_arguments; i++) {
      ...
      if (!arg_type)
        throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT, "Illegal type {} of argument {} of function {}", arguments[i].type->getName(),
                        i + 1, getName());
      ...
      if (!tuple_col)
        throw Exception(ErrorCodes::ILLEGAL_COLUMN, "Illegal column {} of argument of function {}", arguments[i].column->getName(),
                        getName());

      for (const auto &inner_col : tuple_col->getColumns()) columns.push_back(inner_col);
    }

    if (columns.empty()) return ColumnTuple::create(input_rows_count);
...
Combines tuples passed as arguments.
  ...
  FunctionDocumentation::Arguments arguments = {{"tupleN", "Arbitrary number of arguments of Tuple type.", {"Tuple(T)"}}};
