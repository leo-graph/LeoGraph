...
class UTCTimestampTransform : public IFunction {
 public:
  ...
  static FunctionPtr create(ContextPtr, const char *name, bool to_utc) { return std::make_shared<UTCTimestampTransform>(name, to_utc); }
  ...
  DataTypePtr getReturnTypeImpl(const ColumnsWithTypeAndName &arguments) const override {
    FunctionArgumentDescriptors mandatory_args{{"datetime", &isDateTimeOrDateTime64, nullptr, "DateTime or DateTime64"},
    ...
    if (dynamic_cast<const TimezoneMixin *>(arguments[0].type.get())->hasExplicitTimeZone())
      throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT, "Function {}'s 1st argument should not have explicit time zone.",
                      function_name);
  ...
  ColumnPtr executeImpl(const ColumnsWithTypeAndName &arguments, const DataTypePtr &, size_t input_rows_count) const override {
    ...
    if (!time_zone_const_col)
      throw Exception(ErrorCodes::ILLEGAL_COLUMN, "Illegal column {} of 2nd argument of function {}. Excepted const(String).",
                      arg2.column->getName(), function_name);
    ...
    if (WhichDataType(arg1.type).isDateTime()) {
      ...
      auto safe_add = [](UInt32 value, UInt32 offset) -> UInt32 {
        if (value > std::numeric_limits<UInt32>::max() - offset) return std::numeric_limits<UInt32>::max();
        return value + offset;
      };
      ...
      for (size_t i = 0; i < input_rows_count; ++i) {
        ...
        if (to_utc) {
          ...
          result_data[i] = (time_zone_offset >= 0) ? safe_subtract(date_time_val, abs_offset) : safe_add(date_time_val, abs_offset);
        } else {
          // Convert from UTC to local time
          // Local = UTC + Offset (for positive offsets like UTC+3)
          // Local = UTC - |Offset| (for negative offsets like UTC-5)
          result_data[i] = (time_zone_offset >= 0) ? safe_add(date_time_val, abs_offset) : safe_subtract(date_time_val, abs_offset);
        }
      }
...
toUTCTimestamp(datetime, time_zone)
  ...
  FunctionDocumentation::ReturnedValue returned_value_toUTCTimestamp = {"Returns a date or date with time in UTC timezone.",
                                                                        {"DateTime", "DateTime64"}};
  FunctionDocumentation::Examples examples_toUTCTimestamp = {{"Convert timezone to UTC", R"(
...
└─────────────────────────────────────────────────────────┘
  ...
  FunctionDocumentation documentation_toUTCTimestamp = {
      description_toUTCTimestamp,    syntax_toUTCTimestamp,   arguments_toUTCTimestamp,     {},
      returned_value_toUTCTimestamp, examples_toUTCTimestamp, introduced_in_toUTCTimestamp, category_toUTCTimestamp};

  factory.registerFunction(
      "toUTCTimestamp", [](ContextPtr) { return UTCTimestampTransform::create({}, "toUTCTimestamp", true); }, documentation_toUTCTimestamp);
...
fromUTCTimestamp(datetime, time_zone)
  ...
  FunctionDocumentation::Arguments arguments_fromUTCTimestamp = {
      ...
      {"time_zone", "A String type const value or an expression representing the time zone.", {"String"}}};
  FunctionDocumentation::ReturnedValue returned_value_fromUTCTimestamp = {"Returns DateTime/DateTime64 in the specified timezone.",
                                                                          {"DateTime", "DateTime64"}};
  FunctionDocumentation::Examples examples_fromUTCTimestamp = {{"Convert UTC timezone to specified timezone", R"(
...
└─────────────────────────────────────────────────────────────────────────┘
  ...
  FunctionDocumentation documentation_fromUTCTimestamp = {
      description_fromUTCTimestamp,    syntax_fromUTCTimestamp,   arguments_fromUTCTimestamp,     {},
      returned_value_fromUTCTimestamp, examples_fromUTCTimestamp, introduced_in_fromUTCTimestamp, category_fromUTCTimestamp};

  factory.registerFunction(
      "fromUTCTimestamp", [](ContextPtr) { return UTCTimestampTransform::create({}, "fromUTCTimestamp", false); },
      documentation_fromUTCTimestamp);
