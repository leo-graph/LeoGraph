...
using FunctionMultiMatchAnyIndex = FunctionsMultiStringSearch<
    MultiMatchAnyImpl<NameMultiMatchAnyIndex, /*ResultType*/ UInt64, MultiMatchTraits::Find::AnyIndex, /*WithEditDistance*/ false>>;
...
REGISTER_FUNCTION(MultiMatchAnyIndex) {
  ...
  FunctionDocumentation::Examples examples = {{"Usage example", "SELECT multiMatchAnyIndex('ClickHouse', ['[0-9]', 'House', 'Click']);",
                                               R"(
┌─multiMatchAn⋯, 'Click'])─┐
│                        3 │
└──────────────────────────┘
