...
Results might be suboptimal with large values.
  ...
  FunctionDocumentation::Arguments arguments = {{"x", "A number to format.", {"UInt64"}}};
  FunctionDocumentation::ReturnedValue returned_value = {"Returns a rounded number with suffix as a string.", {"String"}};
  FunctionDocumentation::Examples examples = {{"Format numbers with suffixes",
                                               R"(
SELECT
...
└────────────────┴───────────────────┘
  ...
  factory.registerFunction(
      "formatReadableQuantity", [](ContextPtr) { return FunctionFormatReadable::create("formatReadableQuantity", formatReadableQuantity); },
      documentation);
}
