...
class CastOverloadResolverImpl : public IFunctionOverloadResolver {
 public:
  static const char* getNameImpl(CastType cast_type, bool internal) {
    if (cast_type == CastType::accurate) return "accurateCast";
    if (cast_type == CastType::accurateOrNull) return "accurateCastOrNull";
    if (internal) return "_CAST";
    return "CAST";
  }

  String getName() const override { return getNameImpl(cast_type, internal); }

  size_t getNumberOfArguments() const override { return 2; }

  ColumnNumbers getArgumentsThatAreAlwaysConstant() const override { return {1}; }

  explicit CastOverloadResolverImpl(ContextPtr context_, CastType cast_type_, bool internal_, std::optional<CastDiagnostic> diagnostic_,
                                    bool keep_nullable_, const DataTypeValidationSettings& data_type_validation_settings_)
      : context(context_),
  ...
  static FunctionOverloadResolverPtr create(ContextPtr context, CastType cast_type, bool internal,
    ...
    if (internal)
      return std::make_unique<CastOverloadResolverImpl>(context, cast_type, internal, diagnostic, false /*keep_nullable*/,
                                                        DataTypeValidationSettings{});

    const auto& settings_ref = context->getSettingsRef();
    return std::make_unique<CastOverloadResolverImpl>(context, cast_type, internal, diagnostic, settings_ref[Setting::cast_keep_nullable],
                                                      DataTypeValidationSettings(settings_ref));
  }

  static FunctionBasePtr createInternalCast(ColumnWithTypeAndName from, DataTypePtr to, CastType cast_type,
                                            std::optional<CastDiagnostic> diagnostic, ContextPtr context) {
    if (cast_type == CastType::accurateOrNull && !isVariant(to)) to = makeNullable(to);

    ColumnsWithTypeAndName arguments;
    arguments.emplace_back(std::move(from));
    arguments.emplace_back().type = std::make_unique<DataTypeString>();

    return createFunctionBaseCast(context, getNameImpl(cast_type, true), arguments, to, diagnostic, cast_type,
 ...
 protected:
  ...
  DataTypePtr getReturnTypeImpl(const ColumnsWithTypeAndName& arguments) const override {
    ...
    if (cast_type == CastType::accurateOrNull) {
      ...
      if (!isVariant(type)) return makeNullable(type);
    }

    if (internal) return type;

    if (keep_nullable && (arguments.front().type->isNullable() || arguments.front().type->isLowCardinalityNullable()) &&
...
REGISTER_FUNCTION(CastOverloadResolvers) {
  factory.registerFunction(
      "_CAST", [](ContextPtr context) { return CastOverloadResolverImpl::create(context, CastType::nonAccurate, true, {}); },
      FunctionDocumentation::INTERNAL_FUNCTION_DOCS, FunctionFactory::Case::Insensitive);
...
or x::T
  ...
  FunctionDocumentation::Arguments CAST_arguments = {{"x", "A value of any type.", {"Any"}}, {"T", "The target data type.", {"String"}}};
  FunctionDocumentation::ReturnedValue CAST_returned_value = {"Returns the converted value with the target data type.", {"Any"}};
  FunctionDocumentation::Examples CAST_examples = {{"Basic usage",
                                                    R"(
...
└───────────────────────┘
  ...
  FunctionDocumentation CAST_documentation = {CAST_description,    CAST_syntax,   CAST_arguments,     {},
                                              CAST_returned_value, CAST_examples, CAST_introduced_in, CAST_category};
...
This function is safer than regular `CAST` as it prevents precision loss and invalid conversions.
  ...
  FunctionDocumentation::Arguments accurateCast_arguments = {{"x", "A value to convert.", {"Any"}},
  ...
  FunctionDocumentation::Examples accurateCast_examples = {{"Successful conversion",
                                                            R"(
...
└───────────────────────────────────┘
  ...
  FunctionDocumentation accurateCast_documentation = {
      accurateCast_description,    accurateCast_syntax,   accurateCast_arguments,     {},
      accurateCast_returned_value, accurateCast_examples, accurateCast_introduced_in, accurateCast_category};
...
This function combines the safety of [`accurateCast`](#accurateCast) with graceful error handling.
  ...
  FunctionDocumentation::Arguments accurateCastOrNull_arguments = {{"x", "A value to convert.", {"Any"}},
                                                                   {"T", "The target data type name.", {"String"}}};
  FunctionDocumentation::ReturnedValue accurateCastOrNull_returned_value = {
      "Returns the converted value with the target data type, or `NULL` if conversion is not possible.", {"Any"}};
  FunctionDocumentation::Examples accurateCastOrNull_examples = {{"Successful conversion",
                                                                  R"(
...
└─────────────────────────────────────┘
  ...
  FunctionDocumentation accurateCastOrNull_documentation = {
      accurateCastOrNull_description,    accurateCastOrNull_syntax,   accurateCastOrNull_arguments,     {},
      accurateCastOrNull_returned_value, accurateCastOrNull_examples, accurateCastOrNull_introduced_in, accurateCastOrNull_category};

  factory.registerFunction(
      "CAST", [](ContextPtr context) { return CastOverloadResolverImpl::create(context, CastType::nonAccurate, false, {}); },
      CAST_documentation, FunctionFactory::Case::Insensitive);
  factory.registerFunction(
      "accurateCast", [](ContextPtr context) { return CastOverloadResolverImpl::create(context, CastType::accurate, false, {}); },
      accurateCast_documentation);
  factory.registerFunction(
      "accurateCastOrNull",
      [](ContextPtr context) { return CastOverloadResolverImpl::create(context, CastType::accurateOrNull, false, {}); },
