...
class FunctionNested : public IFunction {
 public:
  ...
  static FunctionPtr create(ContextPtr) { return std::make_shared<FunctionNested>(); }
  ...
  DataTypePtr getReturnTypeImpl(const ColumnsWithTypeAndName &arguments) const override {
    ...
    if (const auto *const_column = typeid_cast<const ColumnConst *>(arguments[0].column.get()))
      if (auto res_type = getType(0, const_column->getDataColumn(), arguments)) return res_type;

    throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT, "First argument for function {} must be constant array of strings, got {}",
  ...
  ColumnPtr executeImpl(const ColumnsWithTypeAndName &arguments, const DataTypePtr &result_type, size_t) const override {
    ...
    for (size_t i = 1; i < arguments_size; ++i) columns.push_back(arguments[i].column);
 ...
 private:
  ...
  DataTypePtr getTupleType(size_t array_depth, const ColumnString &column, const ColumnsWithTypeAndName &arguments) const {
    if (column.size() + 1 != arguments.size())
      throw Exception(ErrorCodes::BAD_ARGUMENTS,
                      "Size of nested names array for function {} does not match arrays arguments size. Actual {}. Expected {}", getName(),
                      column.size(), arguments.size() - 1);
    ...
    for (size_t i = 0; i < column.size(); ++i) {
      ...
      if (!type)
        throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT, "Argument {} for function {} must be {}-dimentional array. Actual {}", i + 1,
                        array_depth, getName(), argument.type->getName());
  ...
  ColumnPtr makeColumnForType(const IDataType *type, Columns columns) const {
    ...
    if (tuple_type) return ColumnTuple::create(std::move(columns));
    ...
    for (size_t i = 1; i < columns.size(); ++i) {
      ...
      if (!first_array.hasEqualOffsets(other_array))
        throw Exception(ErrorCodes::SIZES_OF_ARRAYS_DONT_MATCH,
                        "The argument 2 and argument {} of function {} have different array offsets", i + 2, getName());
