...
class FunctionCatBoostEvaluate final : public IFunction, WithContext {
 ...
 public:
  ...
  DataTypePtr getReturnTypeImpl(const ColumnsWithTypeAndName &arguments) const override {
    ...
    if (!name_col) throw Exception(ErrorCodes::ILLEGAL_COLUMN, "First argument of function {} must be a constant string", getName());
    ...
    for (size_t i = 1; i < arguments.size(); ++i) has_nullable = has_nullable || arguments[i].type->isNullable();

    if (has_nullable) {
      if (const auto *tuple = typeid_cast<const DataTypeTuple *>(type.get())) {
        auto elements = tuple->getElements();
        for (auto &element : elements) element = makeNullable(element);
  ...
  ColumnPtr executeImpl(const ColumnsWithTypeAndName &arguments, const DataTypePtr &, size_t) const override {
    ...
    if (null_map) {
      if (const auto *tuple = typeid_cast<const ColumnTuple *>(res.get())) {
        ...
        for (auto &col : nested) col = ColumnNullable::create(col, null_map);
...
See [Training and applying models](https://catboost.ai/docs/features/training.html#training) for how to train catboost models from a training data set.
  ...
  FunctionDocumentation::Arguments arguments = {{"path_to_model", "Path to catboost model.", {"const String"}},
