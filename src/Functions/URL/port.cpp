...
struct FunctionPortImpl : public IFunction {
 ...
 private:
  static void vector(UInt16 default_port, const ColumnString::Chars &data, const ColumnString::Offsets &offsets,
                     PaddedPODArray<UInt16> &res, size_t input_rows_count) {
  ...
  static UInt16 extractPort(UInt16 default_port, const ColumnString::Chars &buf, size_t offset, size_t size) {
    ...
    if (host.empty()) return default_port;
    if (host.size() == size) return default_port;
    ...
    while (p < end) {
      if (*p == '/') break;
      if (!isNumericASCII(*p)) return default_port;

      port = (port * 10) + (*p - '0');
      if (port < 0 || port > static_cast<UInt16>(-1)) return default_port;
      ++p;
    }
...
Returns the port of a URL, or the `default_port` if the URL contains no port or cannot be parsed.
  ...
  FunctionDocumentation::Arguments arguments_port = {
      {"url", "URL.", {"String"}}, {"default_port", "Optional. The default port number to be returned. `0` by default.", {"UInt16"}}};
  FunctionDocumentation::ReturnedValue returned_value_port = {
      "Returns the port of the URL, or the default port if there is no port in the URL or in case of a validation error.", {"UInt16"}};
  FunctionDocumentation::Examples examples_port = {{"Usage example",
                                                    R"(
...
└──────────────────────────────────────────┴──────────────────────────────────────────┘
  ...
  FunctionDocumentation documentation_port = {description_port,    syntax_port,   arguments_port,     {},
                                              returned_value_port, examples_port, introduced_in_port, category_port};
...
Similar to [`port`](#port), but [RFC 3986](https://datatracker.ietf.org/doc/html/rfc3986) conformant.
  ...
  FunctionDocumentation::Arguments arguments_portRFC = {
      {"url", "URL.", {"String"}}, {"default_port", "Optional. The default port number to be returned. `0` by default.", {"UInt16"}}};
  FunctionDocumentation::ReturnedValue returned_value_portRFC = {
      "Returns the port or the default port if there is no port in the URL or in case of a validation error.", {"UInt16"}};
  FunctionDocumentation::Examples examples_portRFC = {{"Usage example",
                                                       R"(
...
└──────────────────────────┴──────────────────────────┘
  ...
  FunctionDocumentation documentation_portRFC = {description_portRFC,    syntax_portRFC,   arguments_portRFC,     {},
                                                 returned_value_portRFC, examples_portRFC, introduced_in_portRFC, category_portRFC};
