  ...
  static std::string_view getNetworkLocation(const char* data, size_t size) {
    ...
    for (; pos < end; ++pos) {
      switch (*pos)  // NOLINT(bugprone-switch-missing-default-case)
      {
        ...
        case '[':
        case ']':
          return pos > start_of_host ? std::string_view(start_of_host, std::min({pos, question_mark_pos, slash_pos}) - start_of_host)
                                     : std::string_view();
      }
    }

    if (has_identification) return std::string_view(start_of_host, pos - start_of_host);
...
struct NameNetloc {
  static constexpr auto name = "netloc";
};
...
Extracts network locality (`username:password@host:port`) from a URL.
  ...
  FunctionDocumentation::Arguments arguments_netloc = {{"url", "URL.", {"String"}}};
  FunctionDocumentation::ReturnedValue returned_value_netloc = {"Returns `username:password@host:port` from a given URL.", {"String"}};
  FunctionDocumentation::Examples examples_netloc = {{"Usage example",
                                                      R"(
...
└──────────────────────────┘
  ...
  FunctionDocumentation documentation_netloc = {description_netloc,    syntax_netloc,   arguments_netloc,     {},
                                                returned_value_netloc, examples_netloc, introduced_in_netloc, category_netloc};
