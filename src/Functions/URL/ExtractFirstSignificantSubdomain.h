...
struct FirstSignificantSubdomainDefaultLookup {
  bool operator()(std::string_view host) const { return tldLookup::isValid(host.data(), host.size()); }
};
...
struct ExtractFirstSignificantSubdomain {
  ...
  static void execute(const Lookup &lookup, const Pos data, const size_t size, Pos &res_data, size_t &res_size,
    ...
    if (domain_length == 0) return;

    if (out_domain_end) *out_domain_end = tmp + domain_length;

    /// cut useless dot
    if (tmp[domain_length - 1] == '.') --domain_length;
    ...
    if (!last_periods[2]) last_periods[2] = begin - 1;
  ...
  template <class Lookup>
  static void executeCustom(const Lookup &lookup, const Pos data, const size_t size, Pos &res_data, size_t &res_size,
                            Pos *out_domain_end = nullptr) {
    res_data = data;
    res_size = 0;
    ...
    ///   return nothing
    if (last_periods[0] == begin - 1) return;
