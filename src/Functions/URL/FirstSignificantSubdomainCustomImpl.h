...
struct FirstSignificantSubdomainCustomLookup {
  ...
  TLDType operator()(std::string_view host) const { return tld_list.lookup(host); }
};
 ...
 public:
  ...
  DataTypePtr getReturnTypeImpl(const ColumnsWithTypeAndName &arguments) const override {
    if (!isString(arguments[0].type))
      throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT, "Illegal type {} of first argument of function {}. Must be String.",
    ...
    if (!column || !checkAndGetColumnConstStringOrFixedString(column))
      throw Exception(ErrorCodes::ILLEGAL_COLUMN,
                      "The second argument of function {} should be a constant string with the name of the custom TLD", getName());
  ...
  static void vector(FirstSignificantSubdomainCustomLookup &tld_lookup, const ColumnString::Chars &data,
                     const ColumnString::Offsets &offsets, ColumnString::Chars &res_data, ColumnString::Offsets &res_offsets,
