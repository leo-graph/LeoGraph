 ...
 public:
  ...
  ColumnPtr executeImpl(const ColumnsWithTypeAndName &arguments, const DataTypePtr &, size_t input_rows_count) const override {
    ...
    if (!col_needle && !col_needle_const_array)
      throw Exception(ErrorCodes::ILLEGAL_COLUMN, "Second argument of function {} must be constant string or constant array", getName());

    if (col_needle_const_array) {
      if (!col_needle_const_array->getData().empty() &&
          typeid_cast<const DataTypeArray &>(*arguments[1].type).getNestedType()->getTypeId() != TypeIndex::String)
        throw Exception(ErrorCodes::ILLEGAL_COLUMN, "Second argument of function {} must be constant array of strings", getName());
    }
  ...
  static void cutURL(ColumnString::Chars &data, String pattern, size_t prev_offset, size_t &cur_offset) {
    ...
    do {
      ...
      if (query_string_begin + 1 >= url_end) break;
  ...
  static void vector(const ColumnString::Chars &data, const ColumnString::Offsets &offsets, const ColumnConst *col_needle,
                     const ColumnArray *col_needle_const_array, ColumnString::Chars &res_data, ColumnString::Offsets &res_offsets,
...
This function does not encode or decode characters in parameter names, e.g. `Client ID` and `Client%20ID` are treated as different parameter names.
  ...
  FunctionDocumentation::Arguments arguments_cutURLParameter = {{"url", "URL.", {"String"}},
  ...
  FunctionDocumentation::Examples examples_cutURLParameter = {{"Usage example",
                                                               R"(
SELECT
...
└──────────────────────────────┴──────────────────────────┘
  ...
  FunctionDocumentation documentation_cutURLParameter = {
      description_cutURLParameter,    syntax_cutURLParameter,   arguments_cutURLParameter,     {},
      returned_value_cutURLParameter, examples_cutURLParameter, introduced_in_cutURLParameter, category_cutURLParameter};
