...
inline std::string_view checkAndReturnHost(const Pos& pos, const Pos& dot_pos, const Pos& start_of_host) {
  if (!dot_pos || start_of_host >= pos || pos - dot_pos == 1) return std::string_view{};

  auto after_dot = *(dot_pos + 1);
  if (after_dot == ':' || after_dot == '/' || after_dot == '?' || after_dot == '#') return std::string_view{};
...
inline std::string_view getURLHostRFC(const char* data, size_t size) {
  if (size < 2) return std::string_view{};
  ...
  for (; pos < end; ++pos) {
    switch (*pos)  /// NOLINT(bugprone-switch-missing-default-case)
    {
      case '.':
        ...
        if (has_at_symbol || colon_pos == nullptr) dot_pos = pos;
        break;
      case ':':
        if (has_open_bracket) continue;
  ...
  if (has_sub_delims) return std::string_view{};
  if (!has_at_symbol) {
    if (has_open_bracket && has_end_bracket) return std::string_view(start_of_host, pos - start_of_host);
