...
class FunctionCustomWeekToDateOrDate32 : public IFunctionCustomWeek<Transform> {
 ...
 public:
  static FunctionPtr create(ContextPtr context) { return std::make_shared<FunctionCustomWeekToDateOrDate32>(context); }
  ...
  ColumnPtr executeImpl(const ColumnsWithTypeAndName &arguments, const DataTypePtr &result_type, size_t input_rows_count) const override {
    ...
    if (which.isDate32()) {
      if (enable_extended_results_for_datetime_functions)
        return CustomWeekTransformImpl<DataTypeDate32, DataTypeDate32, /*is_extended_result*/ true>::execute(arguments, result_type,
                                                                                                             input_rows_count, Transform{});
