...
class FunctionToColumnTypeName : public IFunction {
 public:
  ...
  static FunctionPtr create(ContextPtr) { return std::make_shared<FunctionToColumnTypeName>(); }
...
Unlike function [`toTypeName`](#toTypeName), the returned data type potentially includes internal wrapper columns like `Const` and `LowCardinality`.
  ...
  FunctionDocumentation::Arguments arguments = {{"value", "Value for which to return the internal data type.", {"Any"}}};
