...
class FunctionOverlay : public IFunction {
 public:
  ...
  ColumnPtr executeImpl(const ColumnsWithTypeAndName &arguments, const DataTypePtr &, size_t input_rows_count) const override {
    if (input_rows_count == 0) return ColumnString::create();
...
#define OVERLAY_EXECUTE_CASE(IS_UTF8, HAS_FOUR_ARGS, OFFSET_IS_CONST, LENGTH_IS_CONST)                                                  \
  if (input_is_const && replace_is_const)                                                                                               \
    constantConstant<IS_UTF8, HAS_FOUR_ARGS, OFFSET_IS_CONST, LENGTH_IS_CONST>(input_rows_count, col_input_const->getDataAt(0),         \
                                                                               col_replace_const->getDataAt(0), col_offset, col_length, \
                                                                               offset, length, res_data, res_offsets);                  \
  else if (input_is_const && !replace_is_const)                                                                                         \
    constantVector<IS_UTF8, HAS_FOUR_ARGS, OFFSET_IS_CONST, LENGTH_IS_CONST>(                                                           \
        input_rows_count, col_input_const->getDataAt(0), col_replace_string->getChars(), col_replace_string->getOffsets(), col_offset,  \
        col_length, offset, length, res_data, res_offsets);                                                                             \
  else if (!input_is_const && replace_is_const)                                                                                         \
    vectorConstant<IS_UTF8, HAS_FOUR_ARGS, OFFSET_IS_CONST, LENGTH_IS_CONST>(                                                           \
        input_rows_count, col_input_string->getChars(), col_input_string->getOffsets(), col_replace_const->getDataAt(0), col_offset,    \
        col_length, offset, length, res_data, res_offsets);                                                                             \
  else                                                                                                                                  \
    vectorVector<IS_UTF8, HAS_FOUR_ARGS, OFFSET_IS_CONST, LENGTH_IS_CONST>(                                                             \
        input_rows_count, col_input_string->getChars(), col_input_string->getOffsets(), col_replace_string->getChars(),                 \
        col_replace_string->getOffsets(), col_offset, col_length, offset, length, res_data, res_offsets);
...
#undef OVERLAY_EXECUTE_CASE
 ...
 private:
  ...
  static size_t getValidOffset(Int64 offset, size_t input_size) {
    if (offset > 0) {
      if (static_cast<size_t>(offset) > input_size + 1) return input_size;
      return offset - 1;
    }

    if (input_size < -static_cast<size_t>(offset)) return 0;
  ...
  void constantConstant(size_t rows, const std::string_view &input, const std::string_view &replace, const ColumnPtr &column_offset,
                        const ColumnPtr &column_length, Int64 const_offset, Int64 const_length, ColumnString::Chars &res_data,
    ...
    if (has_four_args && length_is_const && const_length < 0) {
      constantConstant<is_utf8_, true, offset_is_const, false>(rows, input, replace, column_offset, column_length, const_offset, -1,
    ...
    if constexpr (offset_is_const) valid_offset = getValidOffset(const_offset, input_size);
  ...
  template <bool is_utf8_, bool has_four_args, bool offset_is_const, bool length_is_const>
  void vectorConstant(size_t rows, const ColumnString::Chars &input_data, const ColumnString::Offsets &input_offsets,
                      const std::string_view &replace, const ColumnPtr &column_offset, const ColumnPtr &column_length, Int64 const_offset,
                      Int64 const_length, ColumnString::Chars &res_data, ColumnString::Offsets &res_offsets) const {
    /// Free us from handling negative length in the code below
    if (has_four_args && length_is_const && const_length < 0) {
      vectorConstant<is_utf8_, true, offset_is_const, false>(rows, input_data, input_offsets, replace, column_offset, column_length,
                                                             const_offset, -1, res_data, res_offsets);
      return;
    }
    ...
    for (size_t i = 0; i < rows; ++i) {
      ...
      if constexpr (!is_utf8_) {
        ...
        /// copy suffix after replaced region. It is not necessary to copy if suffix_size is zero.
        if (suffix_size) {
          memcpySmallAllowReadWriteOverflow15(&res_data[res_offset], &input_data[input_offset + prefix_size + valid_length], suffix_size);
          res_offset += suffix_size;
        }
      } else {
  ...
  template <bool is_utf8_, bool has_four_args, bool offset_is_const, bool length_is_const>
  void constantVector(size_t rows, const std::string_view &input, const ColumnString::Chars &replace_data,
                      const ColumnString::Offsets &replace_offsets, const ColumnPtr &column_offset, const ColumnPtr &column_length,
                      Int64 const_offset, Int64 const_length, ColumnString::Chars &res_data, ColumnString::Offsets &res_offsets) const {
    /// Free us from handling negative length in the code below
    if (has_four_args && length_is_const && const_length < 0) {
      constantVector<is_utf8_, true, offset_is_const, false>(rows, input, replace_data, replace_offsets, column_offset, column_length,
                                                             const_offset, -1, res_data, res_offsets);
      return;
    }
  ...
  template <bool is_utf8_, bool has_four_args, bool offset_is_const, bool length_is_const>
  void vectorVector(size_t rows, const ColumnString::Chars &input_data, const ColumnString::Offsets &input_offsets,
                    const ColumnString::Chars &replace_data, const ColumnString::Offsets &replace_offsets, const ColumnPtr &column_offset,
                    const ColumnPtr &column_length, Int64 const_offset, Int64 const_length, ColumnString::Chars &res_data,
                    ColumnString::Offsets &res_offsets) const {
    /// Free us from handling negative length in the code below
    if (has_four_args && length_is_const && const_length < 0) {
      vectorVector<is_utf8_, true, offset_is_const, false>(rows, input_data, input_offsets, replace_data, replace_offsets, column_offset,
                                                           column_length, const_offset, -1, res_data, res_offsets);
      return;
    }
...
Replaces part of the string `input` with another string `replace`, starting at the 1-based index `offset`.
  ...
  FunctionDocumentation::Arguments arguments = {
      ...
      {"length",
       "Optional. An integer type `Int`. `length` specifies the length of the snippet within the input string `s` to be replaced. If "
       "`length` is not specified, the number of bytes removed from `s` equals the length of `replace`; otherwise `length` bytes are "
       "removed.",
       {"Int"}}};
  FunctionDocumentation::ReturnedValue returned_value = {"Returns a string with replacement.", {"String"}};
  FunctionDocumentation::Examples examples = {
      {"Basic replacement", "SELECT overlay('My father is from Mexico.', 'mother', 4) AS res;",
       R"(
┌─res──────────────────────┐
│ My mother is from Mexico.│
└──────────────────────────┘
        )"},
      {"Replacement with length", "SELECT overlay('My father is from Mexico.', 'dad', 4, 6) AS res;",
       R"(
┌─res───────────────────┐
...
└───────────────────────┘
  ...
  factory.registerFunction(
      "overlay", [](ContextPtr) { return FunctionOverlay::create("overlay", false); }, overlay_documentation,
...
If this assumption is violated, no exception is thrown and the result is undefined.
  ...
  FunctionDocumentation::Arguments utf8_arguments = {
      ...
      {"replace", "The replacement string.", {"const String"}},
      {"offset",
       "An integer type `Int` (1-based). If `offset` is negative, it is counted from the end of the input string `s`.",
       {"(U)Int*"}},
      {"length",
       "Optional. Specifies the length of the snippet within the input string `s` to be replaced. If `length` is not specified, the number "
       "of characters removed from `s` equals the length of `replace`, otherwise `length` characters are removed.",
       {"(U)Int*"}}};
  FunctionDocumentation::ReturnedValue utf8_returned_value = {"Returns a string with replacement.", {"String"}};
  FunctionDocumentation::Examples utf8_examples = {{"UTF-8 replacement",
...
└───────────────────────────────┘
  ...
  FunctionDocumentation overlayutf8_documentation = {utf8_description,    utf8_syntax,   utf8_arguments, {},
                                                     utf8_returned_value, utf8_examples, introduced_in,  category};

  factory.registerFunction(
      "overlayUTF8", [](ContextPtr) { return FunctionOverlay::create("overlayUTF8", true); }, overlayutf8_documentation,
