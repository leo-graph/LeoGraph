...
Returns the inverse hyperbolic tangent.
  ...
  FunctionDocumentation::Arguments arguments = {
      {"x",
       "Hyperbolic tangent of angle. Values from the interval: -1 < x < 1. `(U)Int*`, `Float*` or `Decimal*`.",
       {"(U)Int*", "Float*", "Decimal*"}}};
  FunctionDocumentation::ReturnedValue returned_value = {"Returns the angle, in radians. Values from the interval: -∞ < atanh(x) < +∞",
                                                         {"Float64"}};
  FunctionDocumentation::Examples examples = {{"Usage example", "SELECT atanh(0)", "0"}};
