...
struct LemmatizeImpl {
  static void vector(const ColumnString::Chars &data, const ColumnString::Offsets &offsets, ColumnString::Chars &res_data,
                     ColumnString::Offsets &res_offsets, Lemmatizers::LemmPtr &lemmatizer) {
    ...
    for (UInt64 i = 0; i < offsets.size(); ++i) {
      ...
      if (data_size + new_size > res_data.size()) res_data.resize(data_size + new_size);
...
class FunctionLemmatize : public IFunction {
 ...
 public:
  explicit FunctionLemmatize(Lemmatizers &lemmatizers_) : lemmatizers(lemmatizers_) {}
...
This function needs dictionaries to operate, which can be obtained from [github](https://github.com/vpodpecan/lemmagen3/tree/master/src/lemmagen3/models). For more details on loading a dictionary from a local file see page ["Defining Dictionaries"](/docs/sql-reference/
     +statements/create/dictionary/sources/local-file).
)";
  FunctionDocumentation::Syntax syntax = "lemmatize(lang, word)";
  FunctionDocumentation::Arguments arguments = {{"lang", "Language which rules will be applied.", {"String"}},
  ...
  FunctionDocumentation::Examples examples = {{"English lemmatization", "SELECT lemmatize('en', 'wolves')", "wolf"}};
