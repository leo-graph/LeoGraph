...
bool ParserJSONPathMemberAccess::parseImpl(Pos& pos, ASTPtr& node, Expected& expected) {
  ...
  if (pos->type != TokenType::Dot && pos->type != TokenType::Number) return false;
  ...
  if (pos->type == TokenType::Number) [[unlikely]] {
    ...
    if (pos->type != TokenType::BareWord && pos->type != TokenType::QuotedIdentifier) return false;

    ParserIdentifier name_p;
    if (!name_p.parse(pos, member_name, expected)) return false;
  }
