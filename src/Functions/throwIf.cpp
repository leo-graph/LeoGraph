...
class FunctionThrowIf : public IFunction {
 public:
  ...
  DataTypePtr getReturnTypeImpl(const DataTypes& arguments) const override {
    ...
    if (number_of_arguments < 1 || number_of_arguments > (allow_custom_error_code_argument ? 3 : 2))
      throw Exception(ErrorCodes::NUMBER_OF_ARGUMENTS_DOESNT_MATCH,
                      ...
                      allow_custom_error_code_argument ? "1 or 2 or 3" : "1 or 2");

    if (!isNativeNumber(arguments[0]))
      throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT, "First argument of function {} must be a number (passed: {})", getName(),
                      arguments[0]->getName());

    if (number_of_arguments > 1 && !isString(arguments[1]))
      throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT, "Second argument of function {} must be a string (passed: {})", getName(),
                      arguments[1]->getName());
    ...
    if (allow_custom_error_code_argument && number_of_arguments > 2) {
      ...
      if (!(which.isInt8() || which.isInt16() || which.isInt32()))
        throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT, "Third argument of function {} must be Int8, Int16 or Int32 (passed: {})",
                        getName(), arguments[2]->getName());
    }
  ...
  ColumnPtr executeImpl(const ColumnsWithTypeAndName& arguments, const DataTypePtr& result_type, size_t input_rows_count) const override {
    ...
    ColumnPtr res;
    if (!((res = execute<UInt8>(in, custom_message, custom_error_code)) || (res = execute<UInt16>(in, custom_message, custom_error_code)) ||
          (res = execute<UInt32>(in, custom_message, custom_error_code)) ||
          (res = execute<UInt64>(in, custom_message, custom_error_code)) || (res = execute<Int8>(in, custom_message, custom_error_code)) ||
          (res = execute<Int16>(in, custom_message, custom_error_code)) || (res = execute<Int32>(in, custom_message, custom_error_code)) ||
 ...
 private:
  template <typename T>
  ColumnPtr execute(const IColumn* in_untyped, const std::optional<String>& message,
                    const std::optional<ErrorCodes::ErrorCode>& error_code) const {
    const auto* in = checkAndGetColumn<ColumnVector<T>>(in_untyped);

    if (!in) in = checkAndGetColumnConstData<ColumnVector<T>>(in_untyped);

    if (in) {
      const auto& in_data = in->getData();
      if (!memoryIsZero(in_data.data(), 0, in_data.size() * sizeof(in_data[0]))) {
        if (message.has_value())
          throw Exception::createRuntime(error_code.value_or(ErrorCodes::FUNCTION_THROW_IF_VALUE_IS_NON_ZERO), *message);
        throw Exception(error_code.value_or(ErrorCodes::FUNCTION_THROW_IF_VALUE_IS_NON_ZERO), "Value passed to '{}' function is non-zero",
                        getName());
      }
...
To use the `error_code` argument, configuration parameter `allow_custom_error_code_in_throw` must be enabled.
  ...
  FunctionDocumentation::Arguments arguments = {{"x", "The condition to check.", {"Any"}},
