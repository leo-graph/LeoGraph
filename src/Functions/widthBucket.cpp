...
class FunctionWidthBucket : public IFunction {
  ...
  void throwIfInvalid(const size_t argument_index, const ColumnConst* col_const, const typename ColumnVector<TDataType>::Container* col_vec,
                      const size_t expected_size) const {
    if ((nullptr == col_const) ^ (nullptr != col_vec && col_vec->size() == expected_size)) {
      throw Exception(ErrorCodes::LOGICAL_ERROR, "Logical error in function {}: argument {} has unexpected type or size.", getName(),
                      argument_index);
    }
  }
  ...
  template <typename TDataType>
  static TDataType getValue(const ColumnConst* col_const, const typename ColumnVector<TDataType>::Container* col_vec, const size_t index) {
  ...
  std::optional<TResultType> checkArguments(const Float64 operand, const Float64 low, const Float64 high, const TCountType count) const {
    ...
    if (isNaN(operand) || isNaN(low) || isNaN(high)) {
      throw Exception(ErrorCodes::BAD_ARGUMENTS, "The first three arguments (operand, low, high) cannot be NaN in function {}", getName());
    }
  ...
  TResultType NO_SANITIZE_UNDEFINED calculate(const Float64 operand, const Float64 low, const Float64 high, const TCountType count) const {
    ...
    if (isNaN(relative_bucket) || !isFinite(relative_bucket)) {
      throw Exception(ErrorCodes::LOGICAL_ERROR, "The calculation resulted in NaN or Inf which is unexpected in function {}.", getName());
    }
  ...
  ColumnPtr executeForResultType(const ColumnsWithTypeAndName& arguments, size_t input_rows_count) const {
    ...
    if (are_all_const_cols) {
      throw Exception(ErrorCodes::LOGICAL_ERROR, "Logical error in function {}: unexpected combination of argument types.", getName());
    }
 ...
 public:
  ...
  DataTypePtr getReturnTypeImpl(const ColumnsWithTypeAndName& arguments) const override {
    FunctionArgumentDescriptors mandatory_args{{"operand", isNativeNumber, nullptr, "native numeric"},
...
There is also a case insensitive alias called `WIDTH_BUCKET` to provide compatibility with other databases.
  ...
  FunctionDocumentation::Arguments arguments = {{"operand", "The value for which to determine the bucket.", {"(U)Int8/16/32/64"}},
  ...
  FunctionDocumentation::Examples examples = {{"Usage example", "widthBucket(10.15, -8.6, 23, 18)", "11"}};
