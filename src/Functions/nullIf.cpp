...
class FunctionNullIf : public IFunction {
 ...
 public:
  ...
  static FunctionPtr create(ContextPtr context) { return std::make_shared<FunctionNullIf>(context); }
  ...
  DataTypePtr getReturnTypeImpl(const DataTypes& arguments) const override { return makeNullable(arguments[0]); }
...
Returns `NULL` if both arguments are equal.
  ...
  FunctionDocumentation::Arguments arguments = {{"x", "The first value.", {"Any"}}, {"y", "The second value.", {"Any"}}};
  FunctionDocumentation::ReturnedValue returned_value = {
      "Returns `NULL` if both arguments are equal, otherwise returns the first argument.", {"NULL", "Nullable(x)"}};
