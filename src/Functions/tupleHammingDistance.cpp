...
class FunctionTupleHammingDistance : public ITupleFunction {
 public:
  ...
  DataTypePtr getReturnTypeImpl(const ColumnsWithTypeAndName &arguments) const override {
    ...
    if (!right_tuple)
      throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT, "Argument 1 of function {} should be tuple, got {}", getName(),
...
:::
  ...
  FunctionDocumentation::Arguments arguments = {{"t1", "First tuple.", {"Tuple(*)"}}, {"t2", "Second tuple.", {"Tuple(*)"}}};
  ...
  FunctionDocumentation::Examples examples = {
      ...
      {"With MinHash to detect semi-duplicate strings",
       "SELECT tupleHammingDistance(wordShingleMinHash(string), wordShingleMinHashCaseInsensitive(string)) FROM (SELECT 'ClickHouse is a "
       "column-oriented database management system for online analytical processing of queries.' AS string)",
       "2"}};
