...
For arguments in the range `[-1, 1]` it returns the value in the range of `[-pi() / 2, pi() / 2]`.
  ...
  FunctionDocumentation::Arguments argument = {{"x", "Argument for which to calculate arcsine of.", {"(U)Int*", "Float*", "Decimal"}}};
  FunctionDocumentation::ReturnedValue returned_value = {"Returns the arcsine value of the provided argument `x`", {"Float64"}};
  FunctionDocumentation::Examples examples = {{"inverse", "SELECT asin(1.0) = pi() / 2, sin(asin(1)), asin(sin(1))", "1 1 1"},
