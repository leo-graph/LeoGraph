...
Like [`multiSearchAllPositions`](#multiSearchAllPositions) but assumes `haystack` and the `needle` substrings are UTF-8 encoded strings.
  ...
  FunctionDocumentation::ReturnedValue returned_value = {
      "Returns array of the starting position in bytes and counting from 1 (if the substring was found), `0` if the substring was not "
      "found.",
      {"Array"}};
  FunctionDocumentation::Examples examples = {{"UTF-8 multi-search", "SELECT multiSearchAllPositionsUTF8('ClickHouse',['C','H'])",
                                               R"(
