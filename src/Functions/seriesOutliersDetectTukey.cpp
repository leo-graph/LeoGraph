...
class FunctionSeriesOutliersDetectTukey : public IFunction {
 public:
  ...
  DataTypePtr getReturnTypeImpl(const ColumnsWithTypeAndName &arguments) const override {
    if (arguments.size() != 1 && arguments.size() != 4)
      throw Exception(ErrorCodes::NUMBER_OF_ARGUMENTS_DOESNT_MATCH, "Function {} needs either 1 or 4 arguments; passed {}.", getName(),
                      arguments.size());
  ...
  ColumnPtr executeImpl(const ColumnsWithTypeAndName &arguments, const DataTypePtr &, size_t input_rows_count) const override {
    ...
    ColumnPtr col_res;
    if (input_rows_count == 0) return ColumnArray::create(ColumnFloat64::create());
    ...
    if (arguments.size() > 1) {
      ...
      if (isnan(min_percentile) || !isFinite(min_percentile) || min_percentile < min_percentile_lower_bound ||
          min_percentile > max_percentile_upper_bound)
      ...
      if (isnan(max_percentile) || !isFinite(max_percentile) || max_percentile < min_percentile_lower_bound ||
          max_percentile > max_percentile_upper_bound || max_percentile < min_percentile)
 ...
 private:
  template <typename T>
  bool executeNumber(const IColumn &arr_data, const ColumnArray::Offsets &arr_offsets, Float64 min_percentile, Float64 max_percentile,
                     Float64 k, ColumnPtr &res_ptr) const {
    const ColumnVector<T> *src_data_concrete = checkAndGetColumn<ColumnVector<T>>(&arr_data);
    if (!src_data_concrete) return false;
    ...
    for (auto src_offset : arr_offsets) {
      ...
      if (len < 4) throw Exception(ErrorCodes::BAD_ARGUMENTS, "At least four data points are needed for function {}", getName());
      ...
      for (ColumnArray::Offset j = prev_src_offset; j < src_offset; ++j) {
        auto score = std::min((static_cast<Float64>(src_vec[j]) - lower_fence), 0.0) +
                     std::max((static_cast<Float64>(src_vec[j]) - upper_fence), 0.0);
...
Detects outliers in series data using [Tukey Fences](https://en.wikipedia.org/wiki/Outlier#Tukey%27s_fences).
  ...
  FunctionDocumentation::Arguments arguments = {
      ...
      {"min_percentile",
       ...
       "[(IQR)](https://en.wikipedia.org/wiki/Interquartile_range). The value must be in range [0.02,0.98]. The default is 0.25.",
       {"Float*"}},
      {"max_percentile",
       "Optional. The maximum percentile to be used to calculate inter-quantile range (IQR). The value must be in range [0.02,0.98]. The "
       "default is 0.75.",
       {"Float*"}},
      {"K", "Optional. Non-negative constant value to detect mild or stronger outliers. The default value is 1.5.", {"Float*"}}};
  FunctionDocumentation::ReturnedValue returned_value = {
      "Returns an array of the same length as the input array where each value represents score of possible anomaly of corresponding "
      "element in the series. A non-zero score indicates a possible anomaly.",
      {"Array(Float32)"}};
  FunctionDocumentation::Examples examples = {
      {"Basic outlier detection", "SELECT seriesOutliersDetectTukey([-3, 2, 15, 3, 5, 6, 4, 5, 12, 45, 12, 3, 3, 4, 5, 6]) AS print_0",
       R"(
┌───────────print_0─────────────────┐
