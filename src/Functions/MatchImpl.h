...
inline bool likePatternIsSubstring(std::string_view pattern, String &res) {
  ...
  if (pattern.size() < 2 || !pattern.starts_with('%') || !pattern.ends_with('%')) return false;
...
struct MatchTraits {
  enum class Syntax : uint8_t { Like, Re2 };

  enum class Case : uint8_t { Sensitive, Insensitive };

  enum class Result : uint8_t { DontNegate, Negate };
};
...
struct MatchImpl {
  ...
  static void vectorConstant(const ColumnString::Chars &haystack_data, const ColumnString::Offsets &haystack_offsets, const String &needle,
                             [[maybe_unused]] const ColumnPtr &start_pos_, PaddedPODArray<UInt8> &res,
                             [[maybe_unused]] ColumnUInt8 *res_null, size_t input_rows_count) {
    ...
    if (is_like && impl::likePatternIsSubstring(needle, strstr_pattern)) {
      ...
      if (i < res.size()) memset(&res[i], negate, (res.size() - i) * sizeof(res[0]));
    ...
    if (required_substring.empty()) {
      if (!regexp.getRE2())  /// An empty regexp. Always matches.
        ...
        for (size_t i = 0; i < input_rows_count; ++i) {
          const bool match =
              regexp.getRE2()->Match({reinterpret_cast<const char *>(&haystack_data[prev_offset]), haystack_offsets[i] - prev_offset}, 0,
                                     haystack_offsets[i] - prev_offset, re2::RE2::UNANCHORED, nullptr, 0);
          res[i] = negate ^ match;
      ...
      while (pos < end && end != (pos = searcher.search(pos, end - pos))) {
        ...
        if (pos + required_substring.size() <= begin + haystack_offsets[i]) {
          ...
          if (is_trivial)
            ...
            const bool match = regexp.getRE2()->Match({str_data, str_size}, start_pos, end_pos, re2::RE2::UNANCHORED, nullptr, 0);
            res[i] = negate ^ match;
          }
        } else
          res[i] = negate;
  ...
  static void vectorFixedConstant(const ColumnString::Chars &haystack, size_t N, const String &needle, PaddedPODArray<UInt8> &res,
                                  [[maybe_unused]] ColumnUInt8 *res_null, size_t input_rows_count) {
    /// `res_null` serves as an output parameter for implementing an XYZOrNull variant.
    chassert(!res_null);
    ...
    if (required_substring.empty()) {
      if (!regexp.getRE2())  /// An empty regexp. Always matches.
        ...
        for (size_t i = 0; i < input_rows_count; ++i) {
          const bool match =
              regexp.getRE2()->Match({reinterpret_cast<const char *>(&haystack[offset]), N}, 0, N, re2::RE2::UNANCHORED, nullptr, 0);
          res[i] = negate ^ match;
        ...
        while (pos < end && end != (pos = searcher.search(pos, end - pos))) {
          ...
          if (pos + required_substring.size() <= next_pos) {
            ...
            if (is_trivial)
              ...
              const bool match = regexp.getRE2()->Match({str_data, N}, start_pos, end_pos, re2::RE2::UNANCHORED, nullptr, 0);
              res[i] = negate ^ match;
            }
          } else
            res[i] = negate;
  ...
  static void vectorVector(const ColumnString::Chars &haystack_data, const ColumnString::Offsets &haystack_offsets,
                           const ColumnString::Chars &needle_data, const ColumnString::Offsets &needle_offset,
                           [[maybe_unused]] const ColumnPtr &start_pos_, PaddedPODArray<UInt8> &res, [[maybe_unused]] ColumnUInt8 *res_null,
    ...
    for (size_t i = 0; i < input_rows_count; ++i) {
      ...
      const auto &needle = String(reinterpret_cast<const char *>(cur_needle_data), cur_needle_length);
      ...
      if (is_like && impl::likePatternIsSubstring(needle, required_substr)) {
        ...
        if (required_substr.empty()) {
          if (!regexp->getRE2())  /// An empty regexp. Always matches.
            ...
            const bool match = regexp->getRE2()->Match({reinterpret_cast<const char *>(cur_haystack_data), cur_haystack_length}, 0,
                                                       cur_haystack_length, re2::RE2::UNANCHORED, nullptr, 0);
            res[i] = negate ^ match;
          }
        } else {
          Searcher searcher(required_substr.data(), required_substr.size(), cur_haystack_length);
          ...
          if (match == cur_haystack_data + cur_haystack_length)
            ...
            if (is_trivial)
              ...
              const bool match2 = regexp->getRE2()->Match({reinterpret_cast<const char *>(cur_haystack_data), cur_haystack_length},
                                                          start_pos, end_pos, re2::RE2::UNANCHORED, nullptr, 0);
              res[i] = negate ^ match2;
            }
          }
        }
  ...
  static void vectorFixedVector(const ColumnString::Chars &haystack, size_t N, const ColumnString::Chars &needle_data,
                                const ColumnString::Offsets &needle_offset, [[maybe_unused]] const ColumnPtr &start_pos_,
                                PaddedPODArray<UInt8> &res, [[maybe_unused]] ColumnUInt8 *res_null, size_t input_rows_count) {
    /// `res_null` serves as an output parameter for implementing an XYZOrNull variant.
    chassert(!res_null);
  ...
  static void constantVector(Args &&...) {
    throw Exception(ErrorCodes::ILLEGAL_COLUMN, "Function '{}' doesn't support search with non-constant needles in constant haystack",
                    name);
  }
};
