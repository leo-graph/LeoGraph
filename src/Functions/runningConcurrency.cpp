...
class ExecutableFunctionRunningConcurrency : public IExecutableFunction {
 public:
  String getName() const override { return "runningConcurrency"; }
 ...
 private:
  ...
  ColumnPtr executeTyped(const ColumnsWithTypeAndName &arguments, size_t input_rows_count) const {
    ...
    if (!col_begin || !col_end) throw Exception(ErrorCodes::ILLEGAL_COLUMN, "Constant columns are not supported at the moment");
    ...
    for (size_t i = 0; i < input_rows_count; ++i) {
      ...
      ongoing_until.erase(ongoing_until.begin(), ongoing_until.upper_bound(begin));
  ...
  ColumnPtr executeForType(const ColumnsWithTypeAndName &arguments, size_t input_rows_count) const {
    ...
    if (which.isDate()) return executeTyped<DataTypeDate>(arguments, input_rows_count);
    if (which.isDateTime()) return executeTyped<DataTypeDateTime>(arguments, input_rows_count);
    if (which.isDateTime64()) return executeTyped<DataTypeDateTime64>(arguments, input_rows_count);
    throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT,
                    "Arguments for function runningConcurrency must be Date, DateTime, or DateTime64.");
  }
};
...
class RunningConcurrencyOverloadResolver : public IFunctionOverloadResolver {
 public:
  ...
  static FunctionOverloadResolverPtr create(ContextPtr) { return std::make_unique<RunningConcurrencyOverloadResolver>(); }
  ...
  FunctionBasePtr buildImpl(const ColumnsWithTypeAndName &arguments, const DataTypePtr &return_type) const override {
    ...
    if (unlikely(!arguments[1].type->equals(*(arguments[0].type)))) {
      throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT, "Function {} must be called with two arguments having the same type.",
                      getName());
    }
...
:::
  ...
  FunctionDocumentation::Arguments arguments = {{"start", "A column with the start time of events.", {"Date", "DateTime", "DateTime64"}},
