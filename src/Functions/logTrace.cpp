 ...
 public:
  ...
  ColumnPtr executeImpl(const ColumnsWithTypeAndName &arguments, const DataTypePtr &, size_t input_rows_count) const override {
    ...
    if (const ColumnConst *col = checkAndGetColumnConst<ColumnString>(arguments[0].column.get()))
      ...
      throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT, "First argument for function {} must be Constant string", getName());
...
Emits a trace log message to the server log for each [Block](/development/architecture/#block).
  ...
  FunctionDocumentation::Arguments arguments = {{"message", "Message that is emitted to the server log.", {"const String"}}};
  FunctionDocumentation::ReturnedValue returned_value = {"Returns `0` always.", {"UInt8"}};
  FunctionDocumentation::Examples examples = {{"Basic example",
                                               R"(
