...
enum class Base64Variant : uint8_t { Normal, URL };
...
inline std::string preprocessBase64URL(std::string_view src) {
  ...
  switch (remainder) {
    ...
    case 1:
      padded_src.append(
          "===");  // this case is impossible to occur with valid base64-URL encoded input, however, we'll insert padding anyway
      break;
    case 2:
...
struct Base64EncodeTraits {
  ...
  static size_t perform(std::string_view src, UInt8 *dst) {
    ...
    if constexpr (variant == Base64Variant::URL) outlen = postprocessBase64URL(dst, outlen);
