...
using FunctionHasToken = FunctionsStringSearch<HasTokenImpl<NameHasToken, Volnitsky, false>>;
using FunctionHasTokenOrNull = FunctionsStringSearch<HasTokenImpl<NameHasTokenOrNull, Volnitsky, false>, ExecutionErrorPolicy::Null>;
...
Uses [splitByNonAlpha](/sql-reference/functions/splitting-merging-functions.md/#splitByNonAlpha) as tokenizer, i.e. a token is defined as the longest possible sub-sequence of consecutive characters `[0-9A-Za-z_]` (numbers, ASCII characters and underscore).
  ...
  FunctionDocumentation::Arguments arguments = {{"haystack", "String to be searched.", {"String"}},
  ...
  FunctionDocumentation::Examples examples = {{"Token search", "SELECT hasToken('clickhouse test', 'test')",
                                               R"(
...
Like [`hasToken`](#hasToken) but returns null if token is ill-formed.
  ...
  FunctionDocumentation::Arguments arguments_or_null = {{"haystack", "String to be searched. Must be constant.", {"String"}},
                                                        {"token", "Token to search for.", {"const String"}}};
  ...
  FunctionDocumentation::Examples examples_or_null = {{"Usage example", "SELECT hasTokenOrNull('apple banana cherry', 'ban ana');",
                                                       R"(
...
└──────────────────────────┘
  ...
  FunctionDocumentation documentation_or_null = {description_or_null,    syntax_or_null,   arguments_or_null,     {},
                                                 returned_value_or_null, examples_or_null, introduced_in_or_null, category_or_null};
