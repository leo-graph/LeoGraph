...
class ConcatImpl : public IFunction {
 public:
  ...
  ColumnPtr executeImpl(const ColumnsWithTypeAndName &arguments, const DataTypePtr &, size_t input_rows_count) const override {
    ...
    if (arguments.size() == 1) return arguments[0].column;
    ...
    if (arguments.size() == 2) return executeBinary(arguments, input_rows_count);
 ...
 private:
  ...
  ColumnPtr executeFormatImpl(const ColumnsWithTypeAndName &arguments, size_t input_rows_count) const {
    ...
    for (size_t i = 0; i < num_arguments; ++i) pattern += "{}";
...
class ConcatOverloadResolver : public IFunctionOverloadResolver {
 public:
  ...
  FunctionBasePtr buildImpl(const ColumnsWithTypeAndName &arguments, const DataTypePtr &return_type) const override {
    if (arguments.size() == 1) return FunctionFactory::instance().getImpl("toString", context)->build(arguments);
...
As this decreases performance, it is not recommended to use non-String/FixedString arguments.
  ...
  FunctionDocumentation::Arguments arguments = {{"s1, s2, ...", "Any number of values of arbitrary type.", {"Any"}}};
  FunctionDocumentation::ReturnedValue returned_value = {
      "Returns the String created by concatenating the arguments. If any of arguments is `NULL`, the function returns `NULL`. If there are "
      "no arguments, it returns an empty string.",
      {"Nullable(String)"}};
  FunctionDocumentation::Examples examples = {{"String concatenation", "SELECT concat('Hello, ', 'World!')",
                                               R"(
┌─concat('Hello, ', 'World!')─┐
│ Hello, World!               │
└─────────────────────────────┘
        )"},
                                              {"Number concatenation", "SELECT concat(42, 144)",
                                               R"(
┌─concat(42, 144)─┐
│ 42144           │
└─────────────────┘
...
Can be used for optimization of `GROUP BY`.
  ...
  FunctionDocumentation::ReturnedValue returned_value_injective = {
      "Returns the string created by concatenating the arguments. If any of argument values is `NULL`, the function returns `NULL`. If no "
      "arguments are passed, it returns an empty string.",
      {"String"}};
  FunctionDocumentation::Examples examples_injective = {
      {"Group by optimization", "SELECT concat(key1, key2), sum(value) FROM key_val GROUP BY concatAssumeInjective(key1, key2)",
       R"(
...
└────────────────────┴────────────┘
  ...
  factory.registerFunction(
      "concatAssumeInjective", [](ContextPtr ctx) { return ConcatImpl::create(ctx, "concatAssumeInjective", true); },
