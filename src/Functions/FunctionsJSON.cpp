...
template <typename T>
concept HasIndexOperator = requires(T t) { t[0]; };

template <typename T>
concept Reservable = requires(T t) { t.reserve(0); };

template <typename T>
concept Preparable = requires(T t) {
  t.prepare(std::declval<const char *>(), std::declval<const ColumnsWithTypeAndName &>(), std::declval<const DataTypePtr &>());
};
...
class FunctionJSONHelpers {
  ...
  class Executor {
   public:
    static ColumnPtr run(const ColumnsWithTypeAndName &arguments, const DataTypePtr &result_type, size_t input_rows_count,
      ...
      if (is_object_input) return runForObjectColumn<Name, Impl>(arguments, result_type, input_rows_count, format_settings);
      ...
      if (!col_json_string) throw Exception(ErrorCodes::ILLEGAL_COLUMN, "Illegal column {}", arg_json->getName());
      ...
      if constexpr (Reservable<JSONParser>) {
        ...
        if (max_size) parser.reserve(max_size);
      }

      Impl<JSONParser> impl;

      /// prepare() does Impl-specific preparation before handling each row.
      if constexpr (Preparable<Impl<JSONParser>>) impl.prepare(Name::name, arguments, result_type);
      ...
      for (size_t i = 0; i < input_rows_count; ++i) {
        ...
        if (document_ok) {
          ...
          if (moves_ok) added_to_column = impl.insertResultToColumn(*to, element, last_key, format_settings, error);
        }

        /// We add default value (=null or zero) if something goes wrong, we don't throw exceptions in these JSON functions.
        if (!added_to_column) to->insertDefault();
      }
      return to;
    }
   ...
   private:
    ...
    static ColumnPtr runForObjectColumn(const ColumnsWithTypeAndName &arguments, const DataTypePtr &result_type, size_t input_rows_count,
                                        const FormatSettings &format_settings) {
      const auto &first_column = arguments[0];
      ...
      const auto *col_object =
          typeid_cast<const ColumnObject *>(col_const ? col_const->getDataColumnPtr().get() : first_column.column.get());

      if (!col_object) throw Exception(ErrorCodes::ILLEGAL_COLUMN, "Invalid column type for Object parsing");
      ...
      for (size_t i = 1; i <= num_index_arguments; ++i) {
        ...
        if (!isString(arg.type) || !arg.column)
          throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT,
                          "Function {} with JSON type input supports only constant string path arguments", String(TName::name));
      ...
      if (path.empty()) return result_type->createColumnConstWithDefaultValue(input_rows_count)->convertToFullColumnIfConst();
 ...
 private:
  ...
  static std::vector<FunctionJSONHelpers::Move> prepareMoves(const char *function_name, const ColumnsWithTypeAndName &columns,
                                                             size_t first_index_argument, size_t num_index_arguments) {
  ...
  static bool performMoves(const ColumnsWithTypeAndName &arguments, size_t row, const typename JSONParser::Element &document,
                           const std::vector<Move> &moves, typename JSONParser::Element &element, std::string_view &last_key) {
    ...
    for (size_t j = 0, size = moves.size(); j != size; ++j) {
      switch (moves[j].type) {
        case MoveType::ConstIndex: {
          if (!moveToElementByIndex<JSONParser>(res_element, static_cast<int>(moves[j].index), key)) return false;
          break;
        }
        case MoveType::ConstKey: {
          key = moves[j].key;
          if constexpr (case_insensitive) {
            if (!moveToElementByKeyCaseInsensitive<JSONParser>(res_element, key)) return false;
          } else {
            if (!moveToElementByKey<JSONParser>(res_element, key)) return false;
          }
          break;
        }
        case MoveType::Index: {
          Int64 index = (*arguments[j + 1].column)[row].safeGet<Int64>();
          if (!moveToElementByIndex<JSONParser>(res_element, static_cast<int>(index), key)) return false;
          break;
        }
        case MoveType::Key: {
  ...
  static bool moveToElementByIndex(typename JSONParser::Element &element, int index, std::string_view &out_key) {
    if (element.isArray()) {
      ...
      if (static_cast<size_t>(index) >= array.size()) return false;
      element = array[index];
      out_key = {};
      return true;
    }
    ...
    if constexpr (HasIndexOperator<typename JSONParser::Object>) {
      if (element.isObject()) {
        ...
        if (static_cast<size_t>(index) >= object.size()) return false;
...
class ExecutableFunctionJSON : public IExecutableFunction {
 public:
  explicit ExecutableFunctionJSON(const NullPresence &null_presence_, bool allow_simdjson_, const DataTypePtr &json_return_type_,
      ...
      : null_presence(null_presence_),
        ...
        json_return_type(json_return_type_),
  ...
  bool useDefaultImplementationForLowCardinalityColumns() const override { return !functionForcesTheReturnType<Impl>(); }

  ColumnPtr executeImpl(const ColumnsWithTypeAndName &arguments, const DataTypePtr &result_type, size_t input_rows_count) const override {
    if (null_presence.has_null_constant) return result_type->createColumnConstWithDefaultValue(input_rows_count);
    ...
    if constexpr (functionForcesTheReturnType<Impl>()) {
      ...
      ColumnsWithTypeAndName temporary_columns =
          null_presence.has_nullable ? createBlockWithNestedColumns(columns_without_low_cardinality) : columns_without_low_cardinality;
      ...
      if (null_presence.has_nullable) temporary_result = wrapInNullable(temporary_result, arguments, result_type, input_rows_count);
 ...
 private:
  ColumnPtr chooseAndRunJSONParser(const ColumnsWithTypeAndName &arguments, const DataTypePtr &result_type, size_t input_rows_count) const {
#if USE_SIMDJSON
    if (allow_simdjson)
      return FunctionJSONHelpers::Executor<Name, Impl, SimdJSONParser, case_insensitive>::run(arguments, result_type, input_rows_count,
                                                                                              format_settings);
#endif

#if USE_RAPIDJSON
    return FunctionJSONHelpers::Executor<Name, Impl, RapidJSONParser, case_insensitive>::run(arguments, result_type, input_rows_count,
                                                                                             format_settings);
#else
    return FunctionJSONHelpers::Executor<Name, Impl, DummyJSONParser, case_insensitive>::run(arguments, result_type, input_rows_count,
                                                                                             format_settings);
#endif
  }
...
class FunctionBaseFunctionJSON : public IFunctionBase {
 public:
  explicit FunctionBaseFunctionJSON(const NullPresence &null_presence_, bool allow_simdjson_, DataTypes argument_types_,
                                    DataTypePtr return_type_, DataTypePtr json_return_type_, const FormatSettings &format_settings_)
      : null_presence(null_presence_),
        allow_simdjson(allow_simdjson_),
  ...
  ExecutableFunctionPtr prepare(const ColumnsWithTypeAndName &) const override {
    return std::make_unique<ExecutableFunctionJSON<Name, Impl, case_insensitive>>(null_presence, allow_simdjson, json_return_type,
                                                                                  format_settings);
  }
...
class JSONOverloadResolver : public IFunctionOverloadResolver {
 public:
  ...
  static FunctionOverloadResolverPtr create(ContextPtr context_) { return std::make_unique<JSONOverloadResolver>(context_); }

  explicit JSONOverloadResolver(ContextPtr context)
      : allow_simdjson(context->getSettingsRef()[Setting::allow_simdjson]), format_settings(getFormatSettings(context)) {}
  ...
  FunctionBasePtr build(const ColumnsWithTypeAndName &arguments) const override {
    ...
    for (const auto &arg : arguments) has_nothing_argument |= isNothing(arg.type);
    ...
    for (const auto &argument : arguments) argument_types.emplace_back(argument.type);
    return std::make_unique<FunctionBaseFunctionJSON<Name, Impl, case_insensitive>>(null_presence, allow_simdjson, argument_types,
                                                                                    return_type, json_return_type, format_settings);
  }
...
struct NameIsValidJSON {
  static constexpr auto name{"isValidJSON"};
};
struct NameJSONLength {
  static constexpr auto name{"JSONLength"};
};
struct NameJSONKey {
  static constexpr auto name{"JSONKey"};
};
struct NameJSONType {
  static constexpr auto name{"JSONType"};
};
struct NameJSONExtractInt {
  static constexpr auto name{"JSONExtractInt"};
};
struct NameJSONExtractUInt {
  static constexpr auto name{"JSONExtractUInt"};
};
struct NameJSONExtractFloat {
  static constexpr auto name{"JSONExtractFloat"};
};
struct NameJSONExtractBool {
  static constexpr auto name{"JSONExtractBool"};
};
struct NameJSONExtractString {
  static constexpr auto name{"JSONExtractString"};
};
struct NameJSONExtract {
  static constexpr auto name{"JSONExtract"};
};
struct NameJSONExtractKeysAndValues {
  static constexpr auto name{"JSONExtractKeysAndValues"};
};
struct NameJSONExtractRaw {
  static constexpr auto name{"JSONExtractRaw"};
};
struct NameJSONExtractArrayRaw {
  static constexpr auto name{"JSONExtractArrayRaw"};
};
struct NameJSONExtractKeysAndValuesRaw {
  static constexpr auto name{"JSONExtractKeysAndValuesRaw"};
};
struct NameJSONExtractKeys {
  static constexpr auto name{"JSONExtractKeys"};
};

// Case-insensitive variants
struct NameJSONExtractIntCaseInsensitive {
  static constexpr auto name{"JSONExtractIntCaseInsensitive"};
};
struct NameJSONExtractUIntCaseInsensitive {
  static constexpr auto name{"JSONExtractUIntCaseInsensitive"};
};
struct NameJSONExtractFloatCaseInsensitive {
  static constexpr auto name{"JSONExtractFloatCaseInsensitive"};
};
struct NameJSONExtractBoolCaseInsensitive {
  static constexpr auto name{"JSONExtractBoolCaseInsensitive"};
};
struct NameJSONExtractStringCaseInsensitive {
  static constexpr auto name{"JSONExtractStringCaseInsensitive"};
};
struct NameJSONExtractCaseInsensitive {
  static constexpr auto name{"JSONExtractCaseInsensitive"};
};
struct NameJSONExtractKeysAndValuesCaseInsensitive {
  static constexpr auto name{"JSONExtractKeysAndValuesCaseInsensitive"};
};
struct NameJSONExtractRawCaseInsensitive {
  static constexpr auto name{"JSONExtractRawCaseInsensitive"};
};
struct NameJSONExtractArrayRawCaseInsensitive {
  static constexpr auto name{"JSONExtractArrayRawCaseInsensitive"};
};
struct NameJSONExtractKeysAndValuesRawCaseInsensitive {
  static constexpr auto name{"JSONExtractKeysAndValuesRawCaseInsensitive"};
};
struct NameJSONExtractKeysCaseInsensitive {
  static constexpr auto name{"JSONExtractKeysCaseInsensitive"};
};
 ...
 public:
  ...
  static DataTypePtr getReturnType(const char *function_name, const ColumnsWithTypeAndName &arguments) {
    if (arguments.size() != 1) {
      ...
      throw Exception(ErrorCodes::NUMBER_OF_ARGUMENTS_DOESNT_MATCH, "Function {} needs exactly one argument", String(function_name));
    }
 ...
 public:
  ...
  static DataTypePtr getReturnType(const char *, const ColumnsWithTypeAndName &) {
    static const std::vector<std::pair<String, Int8>> values = {
        {"Array", '['},  {"Object", '{'}, {"String", '"'}, {"Int64", 'i'},
        {"UInt64", 'u'}, {"Double", 'd'}, {"Bool", 'b'},   {"Null", 0},  /// the default value for the column.
    };
...
class JSONExtractNumericImpl {
 public:
  ...
  static DataTypePtr getReturnType(const char *, const ColumnsWithTypeAndName &) { return std::make_shared<DataTypeNumber<NumberType>>(); }

  static size_t getNumberOfIndexArguments(const ColumnsWithTypeAndName &arguments) { return arguments.size() - 1; }

  static const std::unique_ptr<JSONExtractTreeNode<JSONParser>> &getInsertNode() {
    static const std::unique_ptr<JSONExtractTreeNode<JSONParser>> node =
        buildJSONExtractTree<JSONParser>(std::make_shared<DataTypeNumber<NumberType>>());
  }

  static bool insertResultToColumn(IColumn &dest, const Element &element, std::string_view, const FormatSettings &, String &error) {
    NumberType value;

    if (!tryGetNumericValueFromJSONElement<JSONParser, NumberType>(value, element, /*convert_bool_to_number=*/false,
                                                                   /*allow_type_conversion=*/true, error))
      return false;
...
class JSONExtractStringImpl {
 public:
  ...
  static bool insertResultToColumn(IColumn &dest, const Element &element, std::string_view, const FormatSettings &format_settings,
                                   String &error) {
    if (element.isNull()) return false;

    if (!element.isString()) return JSONExtractRawImpl<JSONParser>::insertResultToColumn(dest, element, {}, format_settings, error);
...
class JSONExtractKeysAndValuesImpl {
 public:
  ...
  bool insertResultToColumn(IColumn &dest, const Element &element, std::string_view, const FormatSettings &format_settings, String &error) {
    ...
    if (col_tuple.size() == old_size) return false;
...
class JSONExtractArrayRawImpl {
 public:
  ...
  static bool insertResultToColumn(IColumn &dest, const Element &element, std::string_view, const FormatSettings &format_settings,
    ...
    for (auto value : array) JSONExtractRawImpl<JSONParser>::insertResultToColumn(col_res.getData(), value, {}, format_settings, error);
...
Checks for the existence of the provided value(s) in the JSON document.
    ...
    FunctionDocumentation::Arguments arguments = {{"json", "JSON string to parse", {"String"}},
    ...
    FunctionDocumentation::Examples example = {{"Usage example",
                                                R"(
...
If the value does not exist or has the wrong type, `0` will be returned.
    ...
    FunctionDocumentation::ReturnedValue returned_value = {
        "Returns the length of the JSON array or JSON object, otherwise returns `0` if the value does not exist or has the wrong type.",
        {"UInt64"}};
    FunctionDocumentation::Examples example = {{"Usage example",
                                                R"(
...
Returns the key of a JSON object field by its index (1-based). If the JSON is passed as a string, it is parsed first. The second argument is a JSON path to navigate into nested objects. The function returns the key name at the specified position.
    ...
    FunctionDocumentation::Arguments arguments = {
        {"json", "JSON string to parse.", {"String"}},
        {"indices_or_keys",
         "Optional list of indices or keys specifying a path to a nested element. Each argument can be either a string (access by key) or "
         "an integer (access by index starting from 1).",
         {"String", "Int*"}}};
    FunctionDocumentation::ReturnedValue returned_value = {"Returns the key name at the specified position in the JSON object.",
                                                           {"String"}};
    FunctionDocumentation::Examples example = {
        {"Usage example", R"(SELECT JSONKey('{"a": "hello", "b": [-100, 200.0, 300]}', 1);)", R"(a)"}};
    FunctionDocumentation::IntroducedIn introduced_in = {20, 1};
    FunctionDocumentation::Category category = FunctionDocumentation::Category::JSON;
    FunctionDocumentation documentation = {description, syntax, arguments, {}, returned_value, example, introduced_in, category};
...
Parses JSON and extracts a value of String type.
    ...
    FunctionDocumentation::Arguments arguments = {
        {"json", "JSON string to parse.", {"String"}},
        {"indices_or_keys", "A list of zero or more arguments each of which can be either string or integer.", {"String", "(U)Int*"}}};
    FunctionDocumentation::ReturnedValue returned_value = {"Returns a String value if it exists, otherwise returns an empty string.",
                                                           {"String"}};
    FunctionDocumentation::Examples examples = {{"Usage example",
                                                 R"(
...
Parses key-value pairs from a JSON where the values are of the given ClickHouse data type.
    ...
    FunctionDocumentation::ReturnedValue returned_value = {"Returns an array of tuples with the parsed key-value pairs.",
...
Returns a part of JSON as unparsed string.
    ...
    FunctionDocumentation::Arguments arguments = {
        ...
        {"indices_or_keys", "A list of zero or more arguments each of which can be either string or integer.", {"String", "(U)Int*"}}};
    FunctionDocumentation::ReturnedValue returned_value = {
        "Returns the part of JSON as an unparsed string. If the part does not exist or has a wrong type, an empty string will be returned.",
        {"String"}};
    FunctionDocumentation::Examples examples = {{"Usage example",
                                                 R"(
...
Returns an array with elements of JSON array, each represented as unparsed string.
    ...
    FunctionDocumentation::Arguments arguments = {
        ...
        {"indices_or_keys", "A list of zero or more arguments each of which can be either string or integer.", {"String", "(U)Int*"}}};
    FunctionDocumentation::ReturnedValue returned_value = {
        "Returns an array of strings with JSON array elements. If the part is not an array or does not exist, an empty array will be "
        "returned.",
        {"Array(String)"}};
    FunctionDocumentation::Examples examples = {{"Usage example",
                                                 R"(
...
Parses JSON and extracts a value of Int type using case-insensitive key matching. This function is similar to [`JSONExtractInt`](#JSONExtractInt).
    ...
    FunctionDocumentation::Arguments arguments = {{"json", "JSON string to parse", {"String"}},
                                                  {"indices_or_keys",
                                                   "Optional. Indices or keys to navigate to the field. Keys use case-insensitive matching",
                                                   {"String", "(U)Int*"}}};
    FunctionDocumentation::ReturnedValue returned_value = {"Returns the extracted Int value, 0 if not found or cannot be converted.",
                                                           {"Int64"}};
...
Parses JSON and extracts a value of UInt type using case-insensitive key matching. This function is similar to [`JSONExtractUInt`](#JSONExtractUInt).
    ...
    FunctionDocumentation::Arguments arguments = {{"json", "JSON string to parse", {"String"}},
                                                  {"indices_or_keys",
                                                   "Optional. Indices or keys to navigate to the field. Keys use case-insensitive matching",
                                                   {"String", "(U)Int*"}}};
    FunctionDocumentation::ReturnedValue returned_value = {"Returns the extracted UInt value, 0 if not found or cannot be converted.",
                                                           {"UInt64"}};
    FunctionDocumentation::Examples examples = {{"basic", R"(SELECT JSONExtractUIntCaseInsensitive('{"COUNT": 789}', 'count'))", "789"}};
    FunctionDocumentation::IntroducedIn introduced_in = {25, 8};
    FunctionDocumentation::Category category = FunctionDocumentation::Category::JSON;
    FunctionDocumentation documentation = {description, syntax, arguments, {}, returned_value, examples, introduced_in, category};
...
Parses JSON and extracts a value of Float type using case-insensitive key matching. This function is similar to [`JSONExtractFloat`](#JSONExtractFloat).
    ...
    FunctionDocumentation::Arguments arguments = {{"json", "JSON string to parse", {"String"}},
                                                  {"indices_or_keys",
                                                   "Optional. Indices or keys to navigate to the field. Keys use case-insensitive matching",
                                                   {"String", "(U)Int*"}}};
    FunctionDocumentation::ReturnedValue returned_value = {"Returns the extracted Float value, 0 if not found or cannot be converted.",
                                                           {"Float64"}};
    FunctionDocumentation::Examples examples = {
...
Parses JSON and extracts a boolean value using case-insensitive key matching. This function is similar to [`JSONExtractBool`](#JSONExtractBool).
    ...
    FunctionDocumentation::Arguments arguments = {{"json", "JSON string to parse", {"String"}},
                                                  {"indices_or_keys",
                                                   "Optional. Indices or keys to navigate to the field. Keys use case-insensitive matching",
                                                   {"String", "(U)Int*"}}};
    FunctionDocumentation::ReturnedValue returned_value = {"Returns the extracted boolean value (1 for true, 0 for false), 0 if not found.",
                                                           {"UInt8"}};
    FunctionDocumentation::Examples examples = {
...
Parses JSON and extracts a string using case-insensitive key matching. This function is similar to [`JSONExtractString`](#JSONExtractString).
    ...
    FunctionDocumentation::Arguments arguments = {{"json", "JSON string to parse", {"String"}},
                                                  {"indices_or_keys",
                                                   "Optional. Indices or keys to navigate to the field. Keys use case-insensitive matching",
                                                   {"String", "(U)Int*"}}};
    FunctionDocumentation::ReturnedValue returned_value = {"Returns the extracted string value, empty string if not found.", {"String"}};
    FunctionDocumentation::Examples examples = {
...
Parses key-value pairs from JSON using case-insensitive key matching. This function is similar to [`JSONExtractKeysAndValues`](#JSONExtractKeysAndValues).
    ...
    FunctionDocumentation::Arguments arguments = {
        {"json", "JSON string to parse", {"String"}},
        {"indices_or_keys",
         "Optional. Indices or keys to navigate to the object. Keys use case-insensitive matching",
         {"String", "(U)Int*"}},
        {"value_type", "The ClickHouse data type of the values", {"String"}}};
    FunctionDocumentation::ReturnedValue returned_value = {"Returns an array of tuples containing key-value pairs.",
                                                           {"Array(Tuple(String, T))"}};
    FunctionDocumentation::Examples examples = {
        {"basic", R"(SELECT JSONExtractKeysAndValuesCaseInsensitive('{"Name": "Alice", "AGE": 30}', 'String'))",
         "[('Name','Alice'),('AGE','30')]"}};
    FunctionDocumentation::IntroducedIn introduced_in = {25, 8};
    FunctionDocumentation::Category category = FunctionDocumentation::Category::JSON;
    FunctionDocumentation documentation = {description, syntax, arguments, {}, returned_value, examples, introduced_in, category};
...
Returns part of the JSON as an unparsed string using case-insensitive key matching. This function is similar to [`JSONExtractRaw`](#JSONExtractRaw).
    ...
    FunctionDocumentation::Arguments arguments = {{"json", "JSON string to parse", {"String"}},
                                                  {"indices_or_keys",
                                                   "Optional. Indices or keys to navigate to the field. Keys use case-insensitive matching",
                                                   {"String", "(U)Int*"}}};
    FunctionDocumentation::ReturnedValue returned_value = {"Returns the raw JSON string of the extracted element.", {"String"}};
    FunctionDocumentation::Examples examples = {
...
Returns an array with elements of JSON array, each represented as unparsed string, using case-insensitive key matching. This function is similar to [`JSONExtractArrayRaw`](#JSONExtractArrayRaw).
    ...
    FunctionDocumentation::Arguments arguments = {{"json", "JSON string to parse", {"String"}},
                                                  {"indices_or_keys",
                                                   "Optional. Indices or keys to navigate to the array. Keys use case-insensitive matching",
                                                   {"String", "(U)Int*"}}};
    FunctionDocumentation::ReturnedValue returned_value = {"Returns an array of raw JSON strings.", {"Array(String)"}};
    FunctionDocumentation::Examples examples = {
...
Extracts raw key-value pairs from JSON using case-insensitive key matching. This function is similar to [`JSONExtractKeysAndValuesRaw`](#JSONExtractKeysAndValuesRaw).
    ...
    FunctionDocumentation::Arguments arguments = {
        ...
        {"indices_or_keys",
         "Optional. Indices or keys to navigate to the object. Keys use case-insensitive matching",
         {"String", "(U)Int*"}}};
    FunctionDocumentation::ReturnedValue returned_value = {"Returns an array of tuples containing key-value pairs as raw strings.",
                                                           {"Array(Tuple(String, String))"}};
    FunctionDocumentation::Examples examples = {{"basic",
                                                 R"(SELECT JSONExtractKeysAndValuesRawCaseInsensitive('{"Name": "Alice", "AGE": 30}'))",
                                                 "[('Name','\"Alice\"'),('AGE','30')]"}};
    FunctionDocumentation::IntroducedIn introduced_in = {25, 8};
    FunctionDocumentation::Category category = FunctionDocumentation::Category::JSON;
    FunctionDocumentation documentation = {description, syntax, arguments, {}, returned_value, examples, introduced_in, category};
...
Parses a JSON string and extracts the keys using case-insensitive key matching to navigate to nested objects. This function is similar to [`JSONExtractKeys`](#JSONExtractKeys).
    ...
    FunctionDocumentation::Arguments arguments = {
        ...
        {"indices_or_keys",
         "Optional. Indices or keys to navigate to the object. Keys use case-insensitive matching",
         {"String", "(U)Int*"}}};
    FunctionDocumentation::ReturnedValue returned_value = {"Returns an array of keys from the JSON object.", {"Array(String)"}};
    FunctionDocumentation::Examples examples = {
