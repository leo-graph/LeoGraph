...
class FunctionIfBase : public IFunction {
 ...
 public:
  bool isCompilableImpl(const DataTypes &types, const DataTypePtr &result_type) const override {
    if (!canBeNativeType(result_type)) return false;
    ...
    for (const auto &type : types) {
      ...
      if (which.isDate()) has_date = true;
      if (which.isDateTime()) has_datetime = true;

      if (has_date && has_datetime) return false;

      if (!canBeNativeType(type_removed_nullable)) return false;
    }
  ...
  llvm::Value *compileImpl(llvm::IRBuilderBase &builder, const ValuesWithType &arguments, const DataTypePtr &result_type) const override {
    ...
    for (const auto &[block, value] : returns) phi->addIncoming(value, block);
