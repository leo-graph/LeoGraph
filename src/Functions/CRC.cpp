...
struct CRCBase {
  ...
  explicit CRCBase(T polynomial) {
    for (size_t i = 0; i < 256; ++i) {
      T c = static_cast<T>(i);
      for (size_t j = 0; j < 8; ++j) c = c & 1 ? polynomial ^ (c >> 1) : c >> 1;
      tab[i] = c;
    }
  }
};
...
struct CRCImpl {
  ...
  static T makeCRC(const unsigned char *buf, size_t size) {
    ...
    T crc = 0;
    for (size_t i = 0; i < size; ++i) crc = base.tab[(crc ^ buf[i]) & 0xff] ^ (crc >> 8);
    return crc;
  }
};
...
struct CRC32ZLibImpl {
  ...
  static UInt32 makeCRC(const unsigned char *buf, size_t size) { return static_cast<UInt32>(crc32_z(0L, buf, size)); }
};
...
struct CRCFunctionWrapper {
  ...
  static void vectorFixedToConstant(const ColumnString::Chars &data, size_t n, ReturnType &res, size_t) { res = doCRC(data, 0, n); }
  ...
  static void vectorFixedToVector(const ColumnString::Chars &data, size_t n, PaddedPODArray<ReturnType> &res, size_t input_rows_count) {
    for (size_t i = 0; i < input_rows_count; ++i) res[i] = doCRC(data, i * n, n);
  }
...
Calculates the CRC32 checksum of a string using the CRC-32-IEEE 802.3 polynomial and initial value `0xffffffff` (zlib implementation).
  ...
  FunctionDocumentation::Arguments arguments_crc32 = {{"s", "String to calculate CRC32 for.", {"String"}}};
  FunctionDocumentation::ReturnedValue returned_value_crc32 = {"Returns the CRC32 checksum of the string.", {"UInt32"}};
  FunctionDocumentation::Examples examples_crc32 = {{"Usage example", "SELECT CRC32('ClickHouse')",
                                                     R"(
┌─CRC32('ClickHouse')─┐
│          1538217360 │
└─────────────────────┘
  ...
  FunctionDocumentation documentation_crc32 = {description_crc32,    syntax_crc32,   arguments_crc32, {},
                                               returned_value_crc32, examples_crc32, introduced_in,   category};
...
Calculates the CRC32 checksum of a string using the CRC-32-IEEE 802.3 polynomial.
  ...
  FunctionDocumentation::Arguments arguments_crc32ieee = {{"s", "String to calculate CRC32 for.", {"String"}}};
  FunctionDocumentation::ReturnedValue returned_value_crc32ieee = {"Returns the CRC32 checksum of the string.", {"UInt32"}};
  FunctionDocumentation::Examples examples_crc32ieee = {{"Usage example", "SELECT CRC32IEEE('ClickHouse');",
                                                         R"(
┌─CRC32IEEE('ClickHouse')─┐
│              3089448422 │
└─────────────────────────┘
        )"}};
  FunctionDocumentation documentation_crc32ieee = {description_crc32ieee,    syntax_crc32ieee,   arguments_crc32ieee, {},
                                                   returned_value_crc32ieee, examples_crc32ieee, introduced_in,       category};
...
Calculates the CRC64 checksum of a string using the CRC-64-ECMA polynomial.
  ...
  FunctionDocumentation::Arguments arguments_crc64 = {{"s", "String to calculate CRC64 for.", {"String"}}};
  FunctionDocumentation::ReturnedValue returned_value_crc64 = {"Returns the CRC64 checksum of the string.", {"UInt64"}};
  FunctionDocumentation::Examples examples_crc64 = {{"Usage example", "SELECT CRC64('ClickHouse');",
                                                     R"(
┌──CRC64('ClickHouse')─┐
│ 12126588151325169346 │
└──────────────────────┘
    )"}};
  FunctionDocumentation documentation_crc64 = {description_crc64,    syntax_crc64,   arguments_crc64, {},
                                               returned_value_crc64, examples_crc64, introduced_in,   category};
