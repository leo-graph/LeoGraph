...
struct ToValidUTF8Impl {
  static void toValidUTF8One(const char *begin, const char *end, WriteBuffer &write_buffer) {
    ...
    auto put_replacement = [&write_buffer, &just_put_replacement]() {
      if (just_put_replacement) return;
...
When multiple consecutive invalid characters are found, they are collapsed into a single replacement character.
  ...
  FunctionDocumentation::Arguments arguments = {{"s", "Any set of bytes represented as the String data type object.", {"String"}}};
  FunctionDocumentation::ReturnedValue returned_value = {"Returns a valid UTF-8 string.", {"String"}};
  FunctionDocumentation::Examples examples = {{"Usage example", R"(SELECT toValidUTF8('\\x61\\xF0\\x80\\x80\\x80b'))",
                                               R"(c
┌─toValidUTF8('a����b')─┐
│ a�b                   │
└───────────────────────┘
