...
using FunctionMatch =
    FunctionsStringSearch<MatchImpl<NameMatch, MatchTraits::Syntax::Re2, MatchTraits::Case::Sensitive, MatchTraits::Result::DontNegate>>;
...
Alternative operator syntax: `haystack REGEXP pattern`.
  ...
  FunctionDocumentation::Arguments arguments = {{"haystack", "String in which the pattern is searched.", {"String"}},
  ...
  FunctionDocumentation::Examples examples = {{"Basic pattern matching", "SELECT match('Hello World', 'Hello.*')",
                                               R"(
┌─match('Hello World', 'Hello.*')─┐
│                               1 │
└─────────────────────────────────┘
        )"},
                                              {"Pattern not matching", "SELECT match('Hello World', 'goodbye.*')",
                                               R"(
┌─match('Hello World', 'goodbye.*')─┐
│                                 0 │
└───────────────────────────────────┘
        )"},
                                              {"Matching a substring", "SELECT match('abcde', 'b.*d'), match('abcde', '^b.*d$')",
                                               R"(
