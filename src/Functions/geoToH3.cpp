...
class FunctionGeoToH3 : public IFunction {
 ...
 public:
  ...
  explicit FunctionGeoToH3(ContextPtr context) : geotoh3_argument_order(context->getSettingsRef()[Setting::geotoh3_argument_order]) {}
  ...
  DataTypePtr getReturnTypeImpl(const ColumnsWithTypeAndName &arguments) const override {
    FunctionArgumentDescriptors mandatory_args = geotoh3_argument_order == GeoToH3ArgumentOrder::LON_LAT
                                                     ? FunctionArgumentDescriptors{{"longitude", &isFloat, nullptr, "Float64"},
                                                                                   {"latitude", &isFloat, nullptr, "Float64"},
                                                                                   {"resolution", &isUInt8, nullptr, "UInt8"}}
                                                     : FunctionArgumentDescriptors{{"latitude", &isFloat, nullptr, "Float64"},
  ...
  ColumnPtr executeImpl(const ColumnsWithTypeAndName &arguments, const DataTypePtr &, size_t input_rows_count) const override {
    ...
    if (!col_res)
      throw Exception(ErrorCodes::ILLEGAL_COLUMN, "Illegal type {} of argument {} of function {}. Must be UInt8.",
    ...
    for (size_t row = 0; row < input_rows_count; ++row) {
      ...
      if (res > MAX_H3_RES)
        throw Exception(ErrorCodes::ARGUMENT_OUT_OF_BOUND,
                        ...
                        static_cast<uint8_t>(res), getName(), MAX_H3_RES);
      ...
      if (err)
        throw Exception(ErrorCodes::INCORRECT_DATA, "Incorrect coordinates latitude: {}, longitude: {}, error: {}", coord.lat, coord.lng,
                        err);
...
The previous behavior can be restored using setting `geotoh3_argument_order = 'lon_lat'`.
  ...
  FunctionDocumentation::Arguments arguments = {{"lat", "Latitude in degrees.", {"Float64"}},
                                                {"lon", "Longitude in degrees.", {"Float64"}},
                                                {"resolution", "Index resolution with range `[0, 15]`.", {"UInt8"}}};
  FunctionDocumentation::ReturnedValue returned_value = {"Returns the H3 index number, or `0` in case of error.", {"UInt64"}};
  FunctionDocumentation::Examples examples = {{"Convert coordinates to H3 index", "SELECT geoToH3(55.71290588, 37.79506683, 15) AS h3Index",
                                               R"(
┌────────────h3Index─┐
│ 644325524701193974 │
└────────────────────┘
