 ...
 public:
  ...
  DataTypePtr getReturnTypeImpl(const DataTypes &arguments) const override {
    ...
    if (!isStringOrFixedString(separator_arg))
      throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT, "Illegal type {} of first argument of function {}", separator_arg->getName(),
                      getName());
  ...
  ColumnPtr executeImpl(const ColumnsWithTypeAndName &arguments, const DataTypePtr &result_type, size_t input_rows_count) const override {
    ...
    if (arguments.size() == 1) return result_type->createColumnConstWithDefaultValue(input_rows_count);
    ...
    if (!col_sep)
      throw Exception(ErrorCodes::ILLEGAL_COLUMN, "Illegal column {} of first argument of function {}. Must be a constant String.",
    ...
    for (size_t i = 0; i < num_exprs; ++i) {
      if (i != 0) constant_strings[2 * i - 1] = sep_str;
      ...
      if (const ColumnString *col = checkAndGetColumn<ColumnString>(column.get())) {
        ...
        auto full_column = column->convertToFullColumnIfConst()->convertToFullColumnIfSparse()->convertToFullColumnIfLowCardinality();
    ...
    for (size_t i = 0; i < num_args; ++i) pattern += "{}";

    FormatStringImpl::formatExecute(has_column_string, has_column_fixed_string, std::move(pattern), data, offsets, fixed_string_sizes,
                                    constant_strings, col_res->getChars(), col_res->getOffsets(), input_rows_count);
...
Concatenates the provided strings, separating them by the specified separator.
  ...
  FunctionDocumentation::Arguments arguments = {
      ...
      {"exp1, exp2, ...",
       "Expression to be concatenated. Arguments which are not of type `String` or `FixedString` are converted to strings using their "
       "default serialization. As this decreases performance, it is not recommended to use non-String/FixedString arguments.",
       {"Any"}}};
  FunctionDocumentation::ReturnedValue returned_value = {
      ...
      {"String"}};
  FunctionDocumentation::Examples examples = {{"Usage example", "SELECT concatWithSeparator('a', '1', '2', '3', '4')",
                                               R"(
...
Can be used for optimization of `GROUP BY`.
  ...
  FunctionDocumentation::ReturnedValue returned_value_injective = {
      "Returns the String created by concatenating the arguments. If any of the argument values is `NULL`, the function returns `NULL`.",
      {"String"}};
  FunctionDocumentation::Examples examples_injective = {{"Usage example",
                                                         R"(
...
└─────────────┴─────────────┘
  ...
  FunctionDocumentation documentation_injective = {description_injective,    syntax_injective,   arguments_injective, {},
                                                   returned_value_injective, examples_injective, introduced_in,       category};

  factory.registerFunction(
      "concatWithSeparator", [](ContextPtr ctx) { return ConcatWithSeparatorImpl::create(ctx, "concatWithSeparator", false); },
      documentation);
  factory.registerFunction(
      "concatWithSeparatorAssumeInjective",
      [](ContextPtr ctx) { return ConcatWithSeparatorImpl::create(ctx, "concatWithSeparatorAssumeInjective", true); },
      documentation_injective);
