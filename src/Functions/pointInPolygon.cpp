...
inline void sipHashRing(SipHash &hash, const Ring &ring) {
  static_assert(std::contiguous_iterator<decltype(ring.data())>,
                "sipHashRing expects a container with contiguous storage (e.g. std::vector).");
...
UInt128 sipHash128(const Polygon &polygon) {
  ...
  for (const auto &inner_ring : inners) sipHashRing(hash, inner_ring);
...
class FunctionPointInPolygon : public IFunction {
 public:
  ...
  DataTypePtr getReturnTypeImpl(const DataTypes &arguments) const override {
     ...
     * pointInPolygon((x, y), [[[(x1, y1), (x2, y2), ...], [(x21, y21), (x22, y22), ...], ...]])
     * - multi polygon
     * pointInPolygon((x, y), [[(x1, y1), (x2, y2), ...], [(x21, y21), (x22, y22), ...]], [[(x1, y1), (x2, y2), ...], [(x21, y21), (x22,
     * y22), ...], ...])
     * - multi polygon, each polygon as a subsequent argument.
     */

    auto validate_tuple = [this](size_t i, const DataTypeTuple *tuple) {
      if (tuple == nullptr) throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT, "{} must contain a tuple", getMessagePrefix(i));

      const DataTypes &elements = tuple->getElements();

      if (elements.size() != 2) throw Exception(ErrorCodes::BAD_ARGUMENTS, "{} must have exactly two elements", getMessagePrefix(i));

      for (auto j : collections::range(0, elements.size())) {
        if (!isNativeNumber(elements[j])) {
          throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT, "{} must contain numeric tuple at position {}", getMessagePrefix(i), j + 1);
        }
      }
    };
  ...
  ColumnPtr executeImpl(const ColumnsWithTypeAndName &arguments, const DataTypePtr &result_type, size_t input_rows_count) const override {
    ...
    if (const_tuple_col) point_col = &const_tuple_col->getDataColumn();

    const auto *tuple_col = checkAndGetColumn<ColumnTuple>(point_col);
    if (!tuple_col)
      throw Exception(ErrorCodes::ILLEGAL_COLUMN, "First argument for function {} must be constant array of tuples.", getName());
    ...
    if (arguments.size() != 2)
      throw Exception(ErrorCodes::BAD_ARGUMENTS, "Multi-argument version of function {} works only with const Polygon/MultiPolygon",
                      getName());
    ...
    if (isThreeDimensionalArray(*arguments[1].type)) {
      ColumnPtr multi_polygon_column_float64 =
          castColumn(arguments[1],
      ...
      for (size_t i = 0; i < input_rows_count; ++i) {
        ...
        data[i] = isInsideMultiPolygon(tuple_columns[0]->getFloat64(point_index), tuple_columns[1]->getFloat64(point_index),
                                       *multi_polygon_column_float64, i);
      }
      ...
      for (size_t i = 0; i < input_rows_count; ++i) {
        size_t point_index = point_is_const ? 0 : i;
        data[i] = isInsidePolygonWithHoles(tuple_columns[0]->getFloat64(point_index), tuple_columns[1]->getFloat64(point_index),
                                           *polygon_column_float64, i);
      }
    } else {
      ColumnPtr polygon_column_float64 =
          castColumn(arguments[1], std::make_shared<DataTypeArray>(std::make_shared<DataTypeTuple>(
                                       DataTypes{std::make_shared<DataTypeFloat64>(), std::make_shared<DataTypeFloat64>()})));

      for (size_t i = 0; i < input_rows_count; ++i) {
        size_t point_index = point_is_const ? 0 : i;
        data[i] = isInsidePolygonWithoutHoles(tuple_columns[0]->getFloat64(point_index), tuple_columns[1]->getFloat64(point_index),
                                              *polygon_column_float64, i);
      }
    }
 ...
 private:
  ...
  std::string getMessagePrefix(size_t i) const { return "Argument " + toString(i + 1) + " for function " + getName(); }

  bool isTwoDimensionalArray(const IDataType &type) const {
    return WhichDataType(type).isArray() && WhichDataType(static_cast<const DataTypeArray &>(type).getNestedType()).isArray();
  }
  ...
  bool isInsideRing(Float64 point_x, Float64 point_y, const Float64 *ring_x_data, const Float64 *ring_y_data, size_t ring_begin,
    ...
    while (vertex1_idx < ring_end) {
      ...
      if (((ring_y_data[vertex1_idx] > point_y) != (ring_y_data[vertex2_idx] > point_y)) &&
          (point_x < (ring_x_data[vertex2_idx] - ring_x_data[vertex1_idx]) * (point_y - ring_y_data[vertex1_idx]) /
                             (ring_y_data[vertex2_idx] - ring_y_data[vertex1_idx]) +
  ...
  bool isInsidePolygonWithoutHoles(Float64 point_x, Float64 point_y, const IColumn &polygon_column, size_t i) const {
  ...
  bool isInsidePolygonWithHoles(Float64 point_x, Float64 point_y, const IColumn &polygon_column, size_t i) const {
    const auto &array_col = static_cast<const ColumnArray &>(polygon_column);
    ...
    for (size_t j = rings_begin; j < rings_end; ++j) {
      ...
      if (j == rings_begin) {
        if (!isInsideRing(point_x, point_y, x_data, y_data, begin, end)) return false;
      } else {
        if (isInsideRing(point_x, point_y, x_data, y_data, begin, end)) return false;
      }
    }
  ...
  bool isInsideMultiPolygon(Float64 point_x, Float64 point_y, const IColumn &multi_polygon_column, size_t i) const {
    ...
    for (size_t j = polys_begin; j < polys_end; ++j) {
      if (isInsidePolygonWithHoles(point_x, point_y, nested_array_col, j)) return true;
    }
  ...
  template <typename T>
  void parseRing(const Float64 *x_data, const Float64 *y_data, size_t begin, size_t end, T &out_container) const {
    ...
    for (size_t i = begin; i < end; ++i) {
      ...
      if (common::mulOverflow(static_cast<Int64>(x_data[i]), static_cast<Int64>(y_data[i]), result))
        throw Exception(ErrorCodes::BAD_ARGUMENTS,
                        "The coordinates of the point are such that "
  ...
  void parseConstPolygonWithHolesFromMultipleColumns(const ColumnsWithTypeAndName &arguments, Polygon &out_polygon) const {
    for (size_t i = 1; i < arguments.size(); ++i) {
      ...
      if (!tuple_col) throw Exception(ErrorCodes::ILLEGAL_COLUMN, "{} must be constant array of tuples", getMessagePrefix(i));

      const auto &tuple_columns = tuple_col->getColumns();
      const auto &column_x = tuple_columns[0];
      const auto &column_y = tuple_columns[1];

      if (!out_polygon.outer().empty()) out_polygon.inners().emplace_back();

      auto &container = out_polygon.outer().empty() ? out_polygon.outer() : out_polygon.inners().back();

      auto size = column_x->size();

      if (size == 0) throw Exception(ErrorCodes::ILLEGAL_COLUMN, "{} shouldn't be empty.", getMessagePrefix(i));
  ...
  void parseConstPolygonFromSingleColumn(const ColumnWithTypeAndName &argument, Polygon &out_polygon) const {
    if (isTwoDimensionalArray(*argument.type)) {
      ColumnPtr polygon_column_float64 =
          castColumn(argument, std::make_shared<DataTypeArray>(std::make_shared<DataTypeArray>(std::make_shared<DataTypeTuple>(
                                   DataTypes{std::make_shared<DataTypeFloat64>(), std::make_shared<DataTypeFloat64>()}))));
      ...
      ColumnPtr polygon_column_float64 =
          castColumn(argument, std::make_shared<DataTypeArray>(std::make_shared<DataTypeTuple>(
                                   DataTypes{std::make_shared<DataTypeFloat64>(), std::make_shared<DataTypeFloat64>()})));
  ...
  void NO_SANITIZE_UNDEFINED parseConstPolygon(const ColumnsWithTypeAndName &arguments, Polygon &out_polygon) const {
    ...
    if (validate) {
      ...
      if (!is_valid) throw Exception(ErrorCodes::BAD_ARGUMENTS, "Polygon is not valid: {}", failure_message);
    }
#endif
  }
  ...
  void parseConstMultiPolygonFromSingleColumn(const ColumnWithTypeAndName &argument, MultiPolygon &out_multi_polygon) const {
    ColumnPtr multi_polygon_column_float64 = castColumn(
        argument,
        std::make_shared<DataTypeArray>(std::make_shared<DataTypeArray>(std::make_shared<DataTypeArray>(
            std::make_shared<DataTypeTuple>(DataTypes{std::make_shared<DataTypeFloat64>(), std::make_shared<DataTypeFloat64>()})))));
...
:::
  ...
  FunctionDocumentation::Arguments arguments = {
      ...
      {"[(a, b), (c, d) ...]",
       "Polygon vertices as an array of coordinate pairs. Vertices should be in clockwise or counterclockwise order. Minimum 3 vertices "
       "required.",
       {"Array(Tuple(Float64, Float64))"}},
      {"...",
       "Optional. Additional arguments for polygons with holes (as separate arrays) or multipolygons (as separate polygons).",
       {"Array(Tuple(Float64, Float64))", "Polygon", "MultiPolygon"}}};
  FunctionDocumentation::ReturnedValue returned_value = {
      "Returns `1` if the point is inside the polygon, `0` if it is not. If the point is on the polygon boundary, the function may return "
      "either `0` or `1`.",
      {"UInt8"}};
  FunctionDocumentation::Examples examples = {{"Basic usage with a simple polygon",
