 ...
 public:
  ...
  DataTypePtr getReturnTypeImpl(const DataTypes &arguments) const override {
    ...
    for (size_t i = 0; i < arguments.size(); ++i) {
      if (!checkDataTypes<DataTypeUInt8, DataTypeUInt16, DataTypeUInt32, DataTypeUInt64>(arguments[i].get()))
        throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT, "Argument {} for function {} must have datatype UInt*, given type: {}.", i,
...
class FunctionSqidDecode : public IFunction {
 public:
  ...
  DataTypePtr getReturnTypeImpl(const ColumnsWithTypeAndName &arguments) const override {
    FunctionArgumentDescriptors args{{"sqid", static_cast<FunctionArgumentDescriptor::TypeValidator>(&isString), nullptr, "String"}};
  ...
  ColumnPtr executeImpl(const ColumnsWithTypeAndName &arguments, const DataTypePtr &, size_t input_rows_count) const override {
    ...
    if (const auto *col_non_const = typeid_cast<const ColumnString *>(&src_column)) {
      for (size_t i = 0; i < input_rows_count; ++i) {
        ...
        if (!integers.empty()) res_nested_data.insert(integers.begin(), integers.end());
...
Transforms numbers into a [sqid](https://sqids.org/), a Youtube-like ID string.
  ...
  FunctionDocumentation::Examples examples_sqidEncode = {{"Usage example", "SELECT sqidEncode(1, 2, 3, 4, 5);",
                                                          R"(
...
└───────────────────────────┘
  ...
  FunctionDocumentation documentation_sqidEncode = {
      description_sqidEncode,    syntax_sqidEncode,   arguments_sqidEncode,     {},
      returned_value_sqidEncode, examples_sqidEncode, introduced_in_sqidEncode, category_sqidEncode};
...
Transforms a [sqid](https://sqids.org/) back into an array of numbers.
  ...
  FunctionDocumentation::Examples examples_sqidDecode = {{"Usage example", "SELECT sqidDecode('gXHfJ1C6dN');",
                                                          R"(
...
└──────────────────────────────┘
  ...
  FunctionDocumentation documentation_sqidDecode = {
      description_sqidDecode,    syntax_sqidDecode,   arguments_sqidDecode,     {},
      returned_value_sqidDecode, examples_sqidDecode, introduced_in_sqidDecode, category_sqidDecode};
