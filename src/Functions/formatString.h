...
struct FormatStringImpl {
  ...
  static void format(String pattern, const std::vector<const ColumnString::Chars *> &data,
                     ...
                     const std::vector<std::optional<String>> &constant_strings, ColumnString::Chars &res_data,
    ...
    for (String &str : substrings) {
      ...
      if (!str.empty()) str.reserve(str.size() + right_padding);
      final_size += str.size();
    }
    ...
    for (size_t i = 1; i < substrings.size(); ++i) final_size += data[index_positions[i - 1]]->size();
