...
class FunctionIsNotNull : public IFunction {
 public:
  ...
  ColumnPtr getConstantResultForNonConstArguments(const ColumnsWithTypeAndName &arguments, const DataTypePtr &result_type) const override {
    ...
    if (!use_analyzer) return nullptr;
    ...
    if (isNothing(result_type)) return nullptr;

    const ColumnWithTypeAndName &elem = arguments[0];
    if (elem.type->onlyNull()) return result_type->createColumnConst(1, UInt8(0));

    if (canContainNull(*elem.type)) return nullptr;
  ...
  ColumnPtr executeImpl(const ColumnsWithTypeAndName &arguments, const DataTypePtr &, size_t input_rows_count) const override {
    ...
    if (isVariant(elem.type) || isDynamic(elem.type)) {
      const auto &column_variant = isVariant(elem.type) ? checkAndGetColumn<ColumnVariant>(*elem.column)
                                                        : checkAndGetColumn<ColumnDynamic>(*elem.column).getVariantColumn();
      ...
      for (size_t i = 0; i < discriminators.size(); ++i) data[i] = discriminators[i] != ColumnVariant::NULL_DISCRIMINATOR;
      return res;
    }
    ...
    if (elem.type->isLowCardinalityNullable()) {
      ...
      for (size_t i = 0; i != low_cardinality_column.size(); ++i) data[i] = (low_cardinality_column.getIndexAt(i) != null_index);
      return res;
    }
  ...
  llvm::Value *compileImpl(llvm::IRBuilderBase &builder, const ValuesWithType &arguments,
                           const DataTypePtr & /*result_type*/) const override {
 ...
 private:
  MULTITARGET_FUNCTION_X86_V3(MULTITARGET_FUNCTION_HEADER(static void NO_INLINE), vectorImpl,
                              MULTITARGET_FUNCTION_BODY((const PaddedPODArray<UInt8> &null_map, PaddedPODArray<UInt8> &res)  /// NOLINT
                                                        {
                                                          size_t size = null_map.size();
                                                          for (size_t i = 0; i < size; ++i) res[i] = !null_map[i];
                                                        }))
...
Also see: operator [`IS NOT NULL`](/sql-reference/operators#is_not_null).
  ...
  FunctionDocumentation::Arguments arguments = {{"x", "A value of non-compound data type.", {"Any"}}};
