...
Like [`hasSubsequence`](/sql-reference/functions/string-search-functions#hasSubsequence) but assumes haystack and needle are UTF-8 encoded strings.
  ...
  FunctionDocumentation::Arguments arguments = {{"haystack", "The string in which to search.", {"String"}},
                                                {"needle", "The subsequence to search for.", {"String"}}};
  FunctionDocumentation::ReturnedValue returned_value = {"Returns `1` if `needle` is a subsequence of `haystack`, otherwise `0`.",
                                                         {"UInt8"}};
  FunctionDocumentation::Examples examples = {{"Usage example", "SELECT hasSubsequenceUTF8('картошка', 'кошка');",
                                               R"(
┌─hasSubsequen⋯', 'кошка')─┐
│                        1 │
└──────────────────────────┘
        )"},
                                              {"Non-matching subsequence", "SELECT hasSubsequenceUTF8('картошка', 'апельсин');",
                                               R"(
