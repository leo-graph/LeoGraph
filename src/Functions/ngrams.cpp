...
class FunctionNgrams : public IFunction {
 public:
  ...
  static FunctionPtr create(ContextPtr) { return std::make_shared<FunctionNgrams>(); }
  ...
  DataTypePtr getReturnTypeImpl(const ColumnsWithTypeAndName &arguments) const override {
    FunctionArgumentDescriptors mandatory_args{{"s", &isStringOrFixedString, nullptr, "String or FixedString"},
 ...
 private:
  ...
  void executeImpl(const ExtractorType &tokenizer, StringColumnType &input_data_column, ResultStringColumnType &result_data_column,
                   ColumnArray::ColumnOffsets &offsets_column, size_t input_rows_count) const {
    ...
    for (size_t i = 0; i < input_rows_count; ++i) {
      ...
      forEachToken(tokenizer, data.data(), data.size(), [&](const char *token_start, size_t token_length) {
...
Splits a UTF-8 string into n-grams of length `N`.
  ...
  FunctionDocumentation::Arguments arguments = {{"s", "Input string.", {"String", "FixedString"}},
  ...
  FunctionDocumentation::Examples examples = {{"Usage example", "SELECT ngrams('ClickHouse', 3);",
                                               R"(
