...
class FunctionSeriesDecomposeSTL : public IFunction {
 public:
  ...
  template <typename T>
  bool executeNumber(const IColumn &src_data, UInt64 period, ColumnArray::Offset start, ColumnArray::Offset end,
                     std::vector<Float32> &seasonal, std::vector<Float32> &trend, std::vector<Float32> &residue) const {
    ...
    if (period > (len / 2))
      throw Exception(ErrorCodes::BAD_ARGUMENTS, "The series should have data of at least two period lengths for function {}", getName());

    std::vector<float> src(src_vec.begin() + start, src_vec.begin() + end);

    auto res = stl::params().fit(src, period);

    if (res.seasonal.empty()) return false;
...
Decomposes a series data using STL [(Seasonal-Trend Decomposition Procedure Based on Loess)](https://www.wessa.net/download/stl.pdf) into a season, a trend and a residual component.
  ...
  FunctionDocumentation::Arguments arguments = {{"series", "An array of numeric values", {"Array((U)Int8/16/32/64)", "Array(Float*)"}},
                                                {"period", "A positive integer", {"UInt8/16/32/64"}}};
  FunctionDocumentation::ReturnedValue returned_value = {
      "Returns an array of four arrays where the first array includes seasonal components, the second array - trend, the third array - "
      "residue component, and the fourth array - baseline(seasonal + trend) component.",
      {"Array(Array(Float32), Array(Float32), Array(Float32), Array(Float32))"}};
  FunctionDocumentation::Examples examples = {
      {"Decompose series data using STL",
       "SELECT seriesDecomposeSTL([10.1, 20.45, 40.34, 10.1, 20.45, 40.34, 10.1, 20.45, 40.34, 10.1, 20.45, 40.34, 10.1, 20.45, 40.34, "
       "10.1, 20.45, 40.34, 10.1, 20.45, 40.34, 10.1, 20.45, 40.34], 3) AS print_0",
       R"(
┌───────────print_0──────────────────────────────────────────────────────────────────────────────────────────────────────┐
│ [[
