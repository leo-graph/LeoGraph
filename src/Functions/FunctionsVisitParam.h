...
struct ExtractParamImpl {
  ...
  static void vectorConstant(const ColumnString::Chars &haystack_data, const ColumnString::Offsets &haystack_offsets, std::string needle,
                             const ColumnPtr &start_pos, PaddedPODArray<ResultType> &res, [[maybe_unused]] ColumnUInt8 *res_null,
    ...
    if (res.size() > i) memset(&res[i], 0, (res.size() - i) * sizeof(res[0]));
  }
...
struct ExtractParamToStringImpl {
  static void vector(const ColumnString::Chars &haystack_data, const ColumnString::Offsets &haystack_offsets, std::string needle,
