...
REGISTER_FUNCTION(NotILike) {
  FunctionDocumentation::Description description =
      "Checks whether a string does not match a pattern, case-insensitive. The pattern can contain special characters `%` and `_` for SQL "
      "LIKE matching.";
  ...
  FunctionDocumentation::Arguments arguments = {
      ...
      {"pattern",
       "The SQL LIKE pattern to match against. `%` matches any number of characters (including zero), `_` matches exactly one character.",
       {"String"}}};
  FunctionDocumentation::ReturnedValue returned_value = {
      "Returns `1` if the string does not match the pattern (case-insensitive), otherwise `0`.", {"UInt8"}};
  FunctionDocumentation::Examples examples = {{"Usage example", "SELECT notILike('ClickHouse', '%house%');",
                                               R"(
┌─notILike('Cl⋯ '%house%')─┐
│                        0 │
└──────────────────────────┘
