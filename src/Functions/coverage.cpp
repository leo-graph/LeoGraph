...
enum class Kind : uint8_t { Current, Cumulative, All };
...
class FunctionCoverage : public IFunction {
 ...
 public:
  String getName() const override { return kind == Kind::Current ? "coverage" : "coverageAll"; }
  ...
  ColumnPtr executeImpl(const ColumnsWithTypeAndName &, const DataTypePtr &, size_t input_rows_count) const override {
    auto coverage_table =
        kind == Kind::Current ? getCurrentCoverage() : (kind == Kind::Cumulative ? getCumulativeCoverage() : getAllInstrumentedAddresses());

    auto column_addresses = ColumnUInt64::create();
    auto &data = column_addresses->getData();

    for (auto ptr : coverage_table)
      if (ptr) data.push_back(ptr);

    auto column_array = ColumnArray::create(std::move(column_addresses), ColumnArray::ColumnOffsets::create(1, data.size()));
...
REGISTER_FUNCTION(Coverage) {
  factory.registerFunction(
      "coverageCurrent", [](ContextPtr) { return std::make_shared<FunctionCoverage>(Kind::Current); },
...
See https://clang.llvm.org/docs/SanitizerCoverage.html for more information.
                            ...
                            .examples{{"functions", "SELECT DISTINCT demangle(addressToSymbol(arrayJoin(coverageCurrent())))", ""}},
                            .introduced_in = {23, 11},
                            .category = FunctionDocumentation::Category::Introspection});

  factory.registerFunction(
      "coverageCumulative", [](ContextPtr) { return std::make_shared<FunctionCoverage>(Kind::Cumulative); },
      FunctionDocumentation{.description = R"(
This function is only available if ClickHouse was built with the SANITIZE_COVERAGE=1 option.
...
See the `coverageCurrent` function for the details.
  ...
  factory.registerFunction(
      "coverageAll", [](ContextPtr) { return std::make_shared<FunctionCoverage>(Kind::All); },
      FunctionDocumentation{.description = R"(
This function is only available if ClickHouse was built with the SANITIZE_COVERAGE=1 option.
