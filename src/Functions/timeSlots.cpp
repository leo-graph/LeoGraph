...
struct TimeSlotsImpl {
  ...
  static void vectorVector(const PaddedPODArray<UInt32> &starts, const PaddedPODArray<UInt32> &durations, UInt32 time_slot_size,
                           PaddedPODArray<UInt32> &result_values, ColumnArray::Offsets &result_offsets, size_t input_rows_count) {
    if (time_slot_size == 0) throw Exception(ErrorCodes::BAD_ARGUMENTS, "Time slot size cannot be zero");
  ...
  static void vectorConstant(const PaddedPODArray<UInt32> &starts, UInt32 duration, UInt32 time_slot_size,
                             PaddedPODArray<UInt32> &result_values, ColumnArray::Offsets &result_offsets, size_t input_rows_count) {
    if (time_slot_size == 0) throw Exception(ErrorCodes::BAD_ARGUMENTS, "Time slot size cannot be zero");
  ...
  static void constantVector(UInt32 start, const PaddedPODArray<UInt32> &durations, UInt32 time_slot_size,
                             PaddedPODArray<UInt32> &result_values, ColumnArray::Offsets &result_offsets, size_t input_rows_count) {
    if (time_slot_size == 0) throw Exception(ErrorCodes::BAD_ARGUMENTS, "Time slot size cannot be zero");
  ...
  */
  static NO_SANITIZE_UNDEFINED void vectorVector(const PaddedPODArray<DateTime64> &starts, const PaddedPODArray<Decimal64> &durations,
                                                 Decimal64 time_slot_size, PaddedPODArray<DateTime64> &result_values,
                                                 ColumnArray::Offsets &result_offsets, UInt16 dt_scale, UInt16 duration_scale,
                                                 UInt16 time_slot_scale, size_t input_rows_count) {
    result_offsets.resize(input_rows_count);
    result_values.reserve(input_rows_count);
    ...
    for (size_t i = 0; i < input_rows_count; ++i) {
      for (DateTime64 value = (starts[i] * dt_multiplier) / time_slot_size,
                      end = (starts[i] * dt_multiplier + durations[i] * dur_multiplier) / time_slot_size;
           value <= end; value += 1) {
        result_values.push_back(value * time_slot_size);
        ++current_offset;
      }
  ...
  static NO_SANITIZE_UNDEFINED void vectorConstant(const PaddedPODArray<DateTime64> &starts, Decimal64 duration, Decimal64 time_slot_size,
                                                   PaddedPODArray<DateTime64> &result_values, ColumnArray::Offsets &result_offsets,
                                                   UInt16 dt_scale, UInt16 duration_scale, UInt16 time_slot_scale,
    ...
    for (size_t i = 0; i < input_rows_count; ++i) {
      for (DateTime64 value = (starts[i] * dt_multiplier) / time_slot_size, end = (starts[i] * dt_multiplier + duration) / time_slot_size;
           value <= end; value += 1) {
        result_values.push_back(value * time_slot_size);
        ++current_offset;
      }
  ...
  static NO_SANITIZE_UNDEFINED void constantVector(DateTime64 start, const PaddedPODArray<Decimal64> &durations, Decimal64 time_slot_size,
                                                   PaddedPODArray<DateTime64> &result_values, ColumnArray::Offsets &result_offsets,
                                                   UInt16 dt_scale, UInt16 duration_scale, UInt16 time_slot_scale,
                                                   size_t input_rows_count) {
    result_offsets.resize(input_rows_count);
    ...
    for (size_t i = 0; i < input_rows_count; ++i) {
      for (DateTime64 value = start / time_slot_size, end = (start + durations[i] * dur_multiplier) / time_slot_size; value <= end;
           value += 1) {
        result_values.push_back(value * time_slot_size);
        ++current_offset;
      }
 ...
 public:
  ...
  DataTypePtr getReturnTypeImpl(const ColumnsWithTypeAndName &arguments) const override {
    ...
    if (WhichDataType(arguments[0].type).isDateTime()) {
      FunctionArgumentDescriptors mandatory_args = {{"start_time", &isDateTime, nullptr, "DateTime"},
                                                    {"duration", &isUInt32, nullptr, "UInt32"}};
      FunctionArgumentDescriptors optional_args = {{"size", &isNativeUInt, nullptr, "UInt*"}};
      validateFunctionArguments(getName(), arguments, mandatory_args, optional_args);
    } else  // DateTime64
    {
      FunctionArgumentDescriptors mandatory_args = {{"start_time", &isDateTime64, nullptr, "DateTime64"},
                                                    {"duration", &isDecimal64, nullptr, "Decimal64"}};
      FunctionArgumentDescriptors optional_args = {{"size", &isDecimal64, nullptr, "Decimal64"}};
      validateFunctionArguments(getName(), arguments, mandatory_args, optional_args);
    }
    ...
    if (WhichDataType(arguments[0].type).isDateTime()) {
      return std::make_shared<DataTypeArray>(
          std::make_shared<DataTypeDateTime>(extractTimeZoneNameFromFunctionArguments(arguments, 3, 0, false)));
    }
  ...
  ColumnPtr executeImpl(const ColumnsWithTypeAndName &arguments, const DataTypePtr &, size_t input_rows_count) const override {
    if (WhichDataType(arguments[0].type).isDateTime()) {
      ...
      if (dt_starts && const_durations) {
        TimeSlotsImpl::vectorConstant(dt_starts->getData(), const_durations->getValue<UInt32>(), time_slot_size, res_values,
                                      res->getOffsets(), input_rows_count);
        return res;
      }
      if (dt_const_starts && durations) {
        TimeSlotsImpl::constantVector(dt_const_starts->getValue<UInt32>(), durations->getData(), time_slot_size, res_values,
                                      res->getOffsets(), input_rows_count);
        return res;
      }
    } else {
      ...
      if (starts && durations) {
        TimeSlotsImpl::vectorVector(starts->getData(), durations->getData(), time_slot_size, res_values, res->getOffsets(),
                                    static_cast<UInt16>(start_time_scale), static_cast<UInt16>(duration_scale), time_slot_scale,
                                    input_rows_count);
        return res;
      }
      if (starts && const_durations) {
        TimeSlotsImpl::vectorConstant(starts->getData(), const_durations->getValue<Decimal64>(), time_slot_size, res_values,
                                      res->getOffsets(), static_cast<UInt16>(start_time_scale), static_cast<UInt16>(duration_scale),
                                      time_slot_scale, input_rows_count);
        return res;
      }
      if (const_starts && durations) {
        TimeSlotsImpl::constantVector(const_starts->getValue<DateTime64>(), durations->getData(), time_slot_size, res_values,
                                      res->getOffsets(), static_cast<UInt16>(start_time_scale), static_cast<UInt16>(duration_scale),
                                      time_slot_scale, input_rows_count);
        return res;
      }
    ...
    if (arguments.size() == 3) {
      throw Exception(ErrorCodes::ILLEGAL_COLUMN, "Illegal columns {}, {}, {} of arguments of function {}", arguments[0].column->getName(),
                      arguments[1].column->getName(), arguments[2].column->getName(), getName());
    }
...
timeSlots(StartTime, Duration[, Size])
  ...
  FunctionDocumentation::ReturnedValue returned_value = {
      "Returns an array of DateTime/DateTime64 (return type matches the type of `StartTime`). For DateTime64, the return value's scale can "
      "differ from the scale of `StartTime` - the highest scale among all given arguments is taken.",
      {"Array(DateTime)", "Array(DateTime64)"}};
  FunctionDocumentation::Examples examples = {{"Generate time slots for an interval", R"(
