...
bool isStringOrFixedStringOrNativeNumber(const IDataType &arg) { return isStringOrFixedString(arg) || isNativeNumber(arg); }
...
class FunctionConv : public IFunction {
 public:
  ...
  DataTypePtr getReturnTypeImpl(const ColumnsWithTypeAndName &arguments) const override {
    FunctionArgumentDescriptors mandatory_args{{"number", &isStringOrFixedStringOrNativeNumber, nullptr, "String, FixedString or Number"},
  ...
  ColumnPtr executeImpl(const ColumnsWithTypeAndName &arguments, const DataTypePtr &, size_t input_rows_count) const override {
    ...
    if (!number_col && !number_const_col)
      throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT, "Illegal column {} of first argument of function {}",
 ...
 private:
  static std::string convertNumber(const std::string &number, const int from_base, const int to_base) {
    if (from_base < 2 || from_base > 36 || to_base < 2 || to_base > 36)
      throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT, "Base is less than 2 or greater than 36 which is not supported");
    if (number.empty()) throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT, "Number for conversion is empty");
    ...
    if (clean_number.empty()) return "0";
    ...
    try {
      ...
      throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT, "Invalid number format for base conversion in function conv");
    }
  }
  ...
  static char digitToChar(int digit) {
    if (digit >= 0 && digit <= 9) return static_cast<char>('0' + digit);
    if (digit >= 10 && digit <= 35) return static_cast<char>('A' + digit - 10);
...
This function is compatible with MySQL's CONV() function.
                            ...
                            .arguments = {{"number", "The number to convert. Can be a string or numeric type."},
                            ...
                            .examples = {{"Convert decimal to binary", "SELECT conv('10', 10, 2)", "1010"},
