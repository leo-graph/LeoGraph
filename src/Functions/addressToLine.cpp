...
class FunctionAddressToLine : public FunctionAddressToLineBase<std::string_view, Dwarf::LocationInfoMode::FAST> {
 ...
 protected:
  DataTypePtr getDataType() const override { return std::make_shared<DataTypeString>(); }
  ...
  void setResult(std::string_view &result, const Dwarf::LocationInfo &location,
                 const std::vector<Dwarf::SymbolizedFrame> &) const override {
...
- Set setting [`allow_introspection_functions`](../../operations/settings/settings.md#allow_introspection_functions) to `1`.
  ...
  FunctionDocumentation::ReturnedValue returned_value = {
      "Returns a source code filename and line number delimited by a colon, for example, "
      "`/build/obj-x86_64-linux-gnu/../src/Common/ThreadPool.cpp:199`. Returns the name of a binary, if no debug information could be "
      "found, otherwise an empty string, if the address is not valid.",
      {"String"}};
