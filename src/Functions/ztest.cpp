...
class FunctionTwoSampleProportionsZTest : public IFunction {
 public:
  ...
  DataTypePtr getReturnTypeImpl(const ColumnsWithTypeAndName &arguments) const override {
    for (size_t i = 0; i < 4; ++i) {
      if (!isUInt(arguments[i].type)) {
        throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT, "The {}th Argument of function {} must be an unsigned integer.", i + 1,
                        getName());
      }
    }

    if (!isFloat(arguments[4].type)) {
      throw Exception{ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT, "The fifth argument {} of function {} should be a float,",
                      arguments[4].type->getName(), getName()};
    }

    /// There is an additional check for constancy in ExecuteImpl
    if (!isString(arguments[5].type) || !arguments[5].column) {
      throw Exception{ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT, "The sixth argument {} of function {} should be a constant string",
                      arguments[5].type->getName(), getName()};
    }
  ...
  ColumnPtr executeImpl(const ColumnsWithTypeAndName &const_arguments, const DataTypePtr &, size_t input_rows_count) const override {
    ...
    for (size_t i = 0; i < 5; ++i) arguments[i].column = arguments[i].column->convertToFullColumnIfConst();
    ...
    if (usevar != UNPOOLED && usevar != POOLED)
      throw Exception{ErrorCodes::BAD_ARGUMENTS, "The sixth argument {} of function {} must be equal to `pooled` or `unpooled`",
                      arguments[5].type->getName(), getName()};
    ...
    auto insert_values_into_result = [&data_z_statistic, &data_p_value, &data_ci_lower, &data_ci_upper](Float64 z_stat, Float64 p_value,
    ...
    for (size_t row_num = 0; row_num < input_rows_count; ++row_num) {
      ...
      Float64 se = std::sqrt(props_x * (1.0 - props_x) / static_cast<Float64>(trials_x) +
                             props_y * (1.0 - props_y) / static_cast<Float64>(trials_y));

      /// z-statistics
      /// z = \frac{ \bar{p_{1}} - \bar{p_{2}} }{ \sqrt{ \frac{ \bar{p_{1}} \left ( 1 - \bar{p_{1}} \right ) }{ n_{1} } \frac{ \bar{p_{2}}
      /// \left ( 1 - \bar{p_{2}} \right ) }{ n_{2} } } }
      Float64 zstat;
      if (is_unpooled) {
...
In the unpooled version, the two proportions are used separately.
  ...
  FunctionDocumentation::ReturnedValue returned_value = {
      "Returns a tuple containing: `z_stat` (Z statistic), `p_val` (P value), `ci_low` (lower confidence interval), `ci_high` (upper "
      "confidence interval).",
