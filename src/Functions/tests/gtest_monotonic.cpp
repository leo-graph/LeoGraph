...
FunctionBasePtr buildFunction(const String& name, const DataTypes& argument_types) {
  ...
  for (const auto& argument_type : argument_types) arguments.emplace_back(ColumnWithTypeAndName{nullptr, argument_type, ""});

  return resolver->build(arguments);
}

void checkMonotonicity(const FunctionBasePtr& function_base, const IDataType& argument_type, const Field& right_bound,
                       bool expected_is_monotonic, bool expected_is_always_monotonic) {
  ...
  if (expected_is_monotonic) ASSERT_TRUE(monotonicity.is_positive);
}
...
TEST(Monotonicity, AssumeNotNull) { testNullWrapperMonotonicity("assumeNotNull", {std::make_shared<DataTypeUInt8>()}); }
