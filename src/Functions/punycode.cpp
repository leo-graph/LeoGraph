...
struct PunycodeEncode {
  ...
  static void vector(const ColumnString::Chars &data, const ColumnString::Offsets &offsets, ColumnString::Chars &res_data,
    ...
    for (size_t row = 0; row < input_rows_count; ++row) {
      ...
      if (codepoints == 0) value_utf32.clear();  /// input was empty or no valid UTF-8

      const bool ok = ada::idna::utf32_to_punycode(value_utf32, value_puny);
      if (!ok) value_puny.clear();
...
struct NameTryPunycodeDecode {
  static constexpr auto name = "tryPunycodeDecode";
};
...
The string must be UTF8-encoded, otherwise the behavior is undefined.
  ...
  FunctionDocumentation::Arguments arguments_encode = {{"s", "Input value.", {"String"}}};
  FunctionDocumentation::ReturnedValue returned_value_encode = {"Returns a Punycode representation of the input value.", {"String"}};
  FunctionDocumentation::Examples examples_encode = {{"Usage example", "SELECT punycodeEncode('München')",
                                                      R"(
...
If no valid Punycode-encoded string is given, an exception is thrown.
  ...
  FunctionDocumentation::Arguments arguments_decode = {{"s", "Punycode-encoded string.", {"String"}}};
  FunctionDocumentation::ReturnedValue returned_value_decode = {"Returns the plaintext of the input value.", {"String"}};
  FunctionDocumentation::Examples examples_decode = {{"Usage example", "SELECT punycodeDecode('Mnchen-3ya')",
                                                      R"(
...
Like `punycodeDecode` but returns an empty string if no valid Punycode-encoded string is given.
  ...
  FunctionDocumentation::Arguments arguments_try_decode = {{"s", "Punycode-encoded string.", {"String"}}};
  FunctionDocumentation::ReturnedValue returned_value_try_decode = {
      "Returns the plaintext of the input value, or empty string if input is invalid.", {"String"}};
  FunctionDocumentation::Examples examples_try_decode = {{"Usage example", "SELECT tryPunycodeDecode('Mnchen-3ya')",
                                                          R"(
┌─tryPunycodeDecode('Mnchen-3ya')─┐
│ München                         │
└─────────────────────────────────┘
        )"}};
  FunctionDocumentation documentation_try_decode = {description_try_decode,    syntax_try_decode,   arguments_try_decode, {},
                                                    returned_value_try_decode, examples_try_decode, introduced_in,        category};
