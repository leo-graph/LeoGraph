...
struct ExtractImpl {
  static void vector(const ColumnString::Chars &data, const ColumnString::Offsets &offsets, const std::string &pattern,
    ...
    for (size_t i = 0; i < input_rows_count; ++i) {
      ...
      unsigned count = regexp.match(reinterpret_cast<const char *>(&data[prev_offset]), cur_offset - prev_offset, matches, capture + 1);
...
If the regular expression has capturing groups (sub-patterns), the function matches the input string against the first capturing group.
  ...
  FunctionDocumentation::Examples examples = {{"Extract domain from email", "SELECT extract('test@clickhouse.com', '.*@(.*)$')",
                                               R"(
┌─extract('test@clickhouse.com', '.*@(.*)$')─┐
│ clickhouse.com                            │
└───────────────────────────────────────────┘
        )"},
                                              {"No match returns empty string", "SELECT extract('test@clickhouse.com', 'no_match')",
                                               R"(
