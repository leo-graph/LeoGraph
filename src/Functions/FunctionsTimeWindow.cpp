...
std::tuple<IntervalKind::Kind, Int64> dispatchForIntervalColumns(const ColumnWithTypeAndName& interval_column,
                                                                 const String& function_name) {
  const auto* interval_type = checkAndGetDataType<DataTypeInterval>(interval_column.type.get());
  if (!interval_type)
    throw Exception(ErrorCodes::ILLEGAL_COLUMN, "Illegal column {} of argument of function {}", interval_column.name, function_name);
  const auto* interval_column_const_int64 = checkAndGetColumnConst<ColumnInt64>(interval_column.column.get());
  if (!interval_column_const_int64)
    throw Exception(ErrorCodes::ILLEGAL_COLUMN, "Illegal column {} of argument of function {}", interval_column.name, function_name);
  Int64 num_units = interval_column_const_int64->getValue<Int64>();
  if (num_units <= 0)
    throw Exception(ErrorCodes::ARGUMENT_OUT_OF_BOUND, "Value for column {} of function {} must be positive", interval_column.name,
                    function_name);
  ...
  if (const ColumnTuple* col_tuple = checkAndGetColumn<ColumnTuple>(column.get()); col_tuple) {
    if (index >= col_tuple->tupleSize() || (!checkColumn<ColumnVector<UInt32>>(*col_tuple->getColumnPtr(index)) &&
      ...
      throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT,
                      ...
                      "Must be a Tuple(DateTime, DateTime)",
                      function_name);
...
IntervalKind extractIntervalKind(const ColumnWithTypeAndName& argument) { return extractIntervalKind(argument.type.get()); }

bool isIntervalDate(IntervalKind kind) {
  return kind == IntervalKind::Kind::Year || kind == IntervalKind::Kind::Quarter || kind == IntervalKind::Kind::Month ||
         kind == IntervalKind::Kind::Week;
}

bool isTupleOfTwoDateTimesOrUInt32(const IDataType& type) {
  if (WhichDataType(type).isUInt32()) return true;

  if (!isTuple(type)) return false;

  const auto* tuple_type = checkAndGetDataType<DataTypeTuple>(&type);
  if (!tuple_type || tuple_type->getElements().size() != 2) return false;
...
enum TimeWindowFunctionName { TUMBLE, TUMBLE_START, TUMBLE_END, HOP, HOP_START, HOP_END, WINDOW_ID };
...
struct TimeWindowImpl<TUMBLE> {
  ...
  [[maybe_unused]] static DataTypePtr getReturnType(const ColumnsWithTypeAndName& arguments, const String& function_name) {
    FunctionArgumentDescriptors mandatory_args{{"time_column", &isDateTime, nullptr, "DateTime"},
                                               {"interval", &isInterval, nullptr, "Interval"}};
    FunctionArgumentDescriptors optional_args{{"timezone", &isString, nullptr, "String"}};
  ...
  static ColumnPtr dispatchForColumns(const ColumnsWithTypeAndName& arguments, const String& function_name, size_t input_rows_count) {
    ...
    if (!which_type.isDateTime() || !time_column_vec)
      throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT,
                      "Illegal column {} of function {}. "
                      "Must contain dates or dates with time",
                      time_column.name, function_name);
...
struct TimeWindowImpl<TUMBLE_START> {
  ...
  [[maybe_unused]] static ColumnPtr dispatchForColumns(const ColumnsWithTypeAndName& arguments, const String& function_name,
    ...
    if (arguments.size() == 1) {
      if (which_type.isUInt32()) return time_column.column;
      // isTuple
...
struct TimeWindowImpl<HOP> {
  ...
  [[maybe_unused]] static DataTypePtr getReturnType(const ColumnsWithTypeAndName& arguments, const String& function_name) {
    ...
    if (hop_interval_kind != extractIntervalKind(arguments.at(2)))
      throw Exception(ErrorCodes::ILLEGAL_COLUMN, "Illegal type of window and hop column of function {}, must be same", function_name);
  ...
  static ColumnPtr dispatchForColumns(const ColumnsWithTypeAndName& arguments, const String& function_name, size_t input_rows_count) {
    ...
    if (!WhichDataType(from_datatype).isDateTime() || !time_column_vec)
      throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT,
                      "Illegal column {} argument of function {}. "
                      "Must contain dates or dates with time",
                      time_column.name, function_name);
    ...
    if (std::get<1>(hop_interval) > std::get<1>(window_interval))
      throw Exception(ErrorCodes::ARGUMENT_OUT_OF_BOUND, "Value for hop interval of function {} must not larger than window interval",
                      function_name);

    switch (std::get<0>(window_interval)) {
      /// TODO: add proper support for fractional seconds
      case IntervalKind::Kind::Second:
        return executeHop<UInt32, IntervalKind::Kind::Second>(*time_column_vec, std::get<1>(hop_interval), std::get<1>(window_interval),
                                                              time_zone, input_rows_count);
      case IntervalKind::Kind::Minute:
        return executeHop<UInt32, IntervalKind::Kind::Minute>(*time_column_vec, std::get<1>(hop_interval), std::get<1>(window_interval),
                                                              time_zone, input_rows_count);
      case IntervalKind::Kind::Hour:
        return executeHop<UInt32, IntervalKind::Kind::Hour>(*time_column_vec, std::get<1>(hop_interval), std::get<1>(window_interval),
                                                            time_zone, input_rows_count);
      case IntervalKind::Kind::Day:
        return executeHop<UInt32, IntervalKind::Kind::Day>(*time_column_vec, std::get<1>(hop_interval), std::get<1>(window_interval),
                                                           time_zone, input_rows_count);
      case IntervalKind::Kind::Week:
        return executeHop<UInt16, IntervalKind::Kind::Week>(*time_column_vec, std::get<1>(hop_interval), std::get<1>(window_interval),
                                                            time_zone, input_rows_count);
      case IntervalKind::Kind::Month:
        return executeHop<UInt16, IntervalKind::Kind::Month>(*time_column_vec, std::get<1>(hop_interval), std::get<1>(window_interval),
                                                             time_zone, input_rows_count);
      case IntervalKind::Kind::Quarter:
        return executeHop<UInt16, IntervalKind::Kind::Quarter>(*time_column_vec, std::get<1>(hop_interval), std::get<1>(window_interval),
                                                               time_zone, input_rows_count);
      case IntervalKind::Kind::Year:
        return executeHop<UInt16, IntervalKind::Kind::Year>(*time_column_vec, std::get<1>(hop_interval), std::get<1>(window_interval),
                                                            time_zone, input_rows_count);
      default:
        throw Exception(ErrorCodes::SYNTAX_ERROR, "Fraction seconds are unsupported by windows yet");
    }
  ...
  static ColumnPtr executeHop(const ColumnDateTime& time_column, UInt64 hop_num_units, UInt64 window_num_units,
                              const DateLUTImpl& time_zone, size_t input_rows_count) {
    const auto& time_data = time_column.getData();
    auto start = ColumnVector<ToType>::create();
    auto end = ColumnVector<ToType>::create();
    auto& start_data = start->getData();
    ...
    for (size_t i = 0; i < input_rows_count; ++i) {
      ...
      if (wstart > wend) throw Exception(ErrorCodes::BAD_ARGUMENTS, "Time overflow in function {}", name);
...
struct TimeWindowImpl<WINDOW_ID> {
  ...
  static DataTypePtr getReturnType(const ColumnsWithTypeAndName& arguments, const String& function_name) {
    auto is_interval_or_string = [](const IDataType& type) { return isInterval(type) || isString(type); };

    if (arguments.size() < 2 || arguments.size() > 4) {
      throw Exception(ErrorCodes::NUMBER_OF_ARGUMENTS_DOESNT_MATCH,
                      "Number of arguments for function {} doesn't match: passed {}, should be 2, 3 or 4", function_name, arguments.size());
    }

    const FunctionArgumentDescriptors mandatory_args{{"time_column", &isDateTime, nullptr, "DateTime"},
                                                     {"window_interval", &isInterval, nullptr, "Interval"}};
    ...
    switch (arguments.size()) {
      ...
      case 4: {
        const FunctionArgumentDescriptors optional_args_4{{"hop_interval", &isInterval, nullptr, "Interval"},
    ...
    if (isIntervalDate(window_interval_kind)) return std::make_shared<DataTypeUInt16>();
  ...
  static ColumnPtr dispatchForHopColumns(const ColumnsWithTypeAndName& arguments, const String& function_name, size_t input_rows_count) {
    ...
    switch (std::get<0>(window_interval)) {
      /// TODO: add proper support for fractional seconds
      case IntervalKind::Kind::Second:
        return executeHopSlice<UInt32, IntervalKind::Kind::Second>(*time_column_vec, std::get<1>(hop_interval),
                                                                   std::get<1>(window_interval), time_zone, input_rows_count);
      case IntervalKind::Kind::Minute:
        return executeHopSlice<UInt32, IntervalKind::Kind::Minute>(*time_column_vec, std::get<1>(hop_interval),
                                                                   std::get<1>(window_interval), time_zone, input_rows_count);
      case IntervalKind::Kind::Hour:
        return executeHopSlice<UInt32, IntervalKind::Kind::Hour>(*time_column_vec, std::get<1>(hop_interval), std::get<1>(window_interval),
                                                                 time_zone, input_rows_count);
      case IntervalKind::Kind::Day:
        return executeHopSlice<UInt32, IntervalKind::Kind::Day>(*time_column_vec, std::get<1>(hop_interval), std::get<1>(window_interval),
                                                                time_zone, input_rows_count);
      case IntervalKind::Kind::Week:
        return executeHopSlice<UInt16, IntervalKind::Kind::Week>(*time_column_vec, std::get<1>(hop_interval), std::get<1>(window_interval),
                                                                 time_zone, input_rows_count);
      case IntervalKind::Kind::Month:
        return executeHopSlice<UInt16, IntervalKind::Kind::Month>(*time_column_vec, std::get<1>(hop_interval), std::get<1>(window_interval),
                                                                  time_zone, input_rows_count);
      case IntervalKind::Kind::Quarter:
        return executeHopSlice<UInt16, IntervalKind::Kind::Quarter>(*time_column_vec, std::get<1>(hop_interval),
                                                                    std::get<1>(window_interval), time_zone, input_rows_count);
      case IntervalKind::Kind::Year:
        return executeHopSlice<UInt16, IntervalKind::Kind::Year>(*time_column_vec, std::get<1>(hop_interval), std::get<1>(window_interval),
                                                                 time_zone, input_rows_count);
      default:
        throw Exception(ErrorCodes::SYNTAX_ERROR, "Fraction seconds are unsupported by windows yet");
    }
  ...
  template <typename ToType, IntervalKind::Kind kind>
  static ColumnPtr executeHopSlice(const ColumnDateTime& time_column, UInt64 hop_num_units, UInt64 window_num_units,
                                   const DateLUTImpl& time_zone, size_t input_rows_count) {
  ...
  static ColumnPtr dispatchForColumns(const ColumnsWithTypeAndName& arguments, const String& function_name, size_t input_rows_count) {
    if (arguments.size() == 2) return dispatchForTumbleColumns(arguments, function_name, input_rows_count);
...
struct TimeWindowImpl<HOP_START> {
  ...
  static DataTypePtr getReturnTypeImpl(const ColumnsWithTypeAndName& arguments, const String& function_name, size_t tuple_index) {
    ...
    return std::static_pointer_cast<const DataTypeTuple>(TimeWindowImpl<HOP>::getReturnType(arguments, function_name))
        ->getElement(tuple_index);
  }
...
template <TimeWindowFunctionName type>
ColumnPtr FunctionTimeWindow<type>::executeImpl(const ColumnsWithTypeAndName& arguments, const DataTypePtr& /*result_type*/,
...
A tumbling time window assigns records to non-overlapping, continuous windows with a fixed duration (`interval`).
  ...
  FunctionDocumentation::Arguments arguments_tumble = {{"time_attr", "Date and time.", {"DateTime"}},
                                                       {"interval", "Window interval in Interval.", {"Interval"}},
                                                       {"timezone", "Optional. Timezone name.", {"String"}}};
  FunctionDocumentation::ReturnedValue returned_value_tumble = {
      "Returns the inclusive lower and exclusive upper bound of the corresponding tumbling window.", {"Tuple(DateTime, DateTime)"}};
  FunctionDocumentation::Examples examples_tumble = {
      {"Tumbling window", "SELECT tumble(now(), toIntervalDay('1'))", "('2024-07-04 00:00:00','2024-07-05 00:00:00')"}};
  ...
  FunctionDocumentation documentation_tumble = {description_tumble,    syntax_tumble,   arguments_tumble,     {},
                                                returned_value_tumble, examples_tumble, introduced_in_tumble, category_tumble};
...
Returns the inclusive lower bound of the corresponding tumbling window.
  ...
  FunctionDocumentation::Arguments arguments_tumble_start = {{"time_attr", "Date and time.", {"DateTime"}},
                                                             {"interval", "Window interval in Interval.", {"Interval"}},
                                                             {"timezone", "Optional. Timezone name.", {"String"}}};
  FunctionDocumentation::ReturnedValue returned_value_tumble_start = {
      "Returns the inclusive lower bound of the corresponding tumbling window.", {"DateTime"}};
  FunctionDocumentation::Examples examples_tumble_start = {
      {"Tumbling window start", "SELECT tumbleStart(now(), toIntervalDay('1'))", "2024-07-04 00:00:00"}};
  ...
  FunctionDocumentation documentation_tumble_start = {
      description_tumble_start,    syntax_tumble_start,   arguments_tumble_start,     {},
      returned_value_tumble_start, examples_tumble_start, introduced_in_tumble_start, category_tumble_start};
...
Returns the exclusive upper bound of the corresponding tumbling window.
  ...
  FunctionDocumentation::Arguments arguments_tumble_end = {{"time_attr", "Date and time.", {"DateTime"}},
                                                           {"interval", "Window interval in Interval.", {"Interval"}},
                                                           {"timezone", "Optional. Timezone name.", {"String"}}};
  FunctionDocumentation::ReturnedValue returned_value_tumble_end = {
      "Returns the exclusive upper bound of the corresponding tumbling window.", {"DateTime"}};
  FunctionDocumentation::Examples examples_tumble_end = {
      {"Tumbling window end", "SELECT tumbleEnd(now(), toIntervalDay('1'))", "2024-07-05 00:00:00"}};
  ...
  FunctionDocumentation documentation_tumble_end = {
      description_tumble_end,    syntax_tumble_end,   arguments_tumble_end,     {},
      returned_value_tumble_end, examples_tumble_end, introduced_in_tumble_end, category_tumble_end};
...
Since one record can be assigned to multiple hop windows, the function only returns the bound of the first window when hop function is used without WINDOW VIEW.
  ...
  FunctionDocumentation::Arguments arguments_hop = {{"time_attr", "Date and time.", {"DateTime"}},
                                                    {"hop_interval", "Positive Hop interval.", {"Interval"}},
                                                    {"window_interval", "Positive Window interval.", {"Interval"}},
                                                    {"timezone", "Optional. Timezone name.", {"String"}}};
  FunctionDocumentation::ReturnedValue returned_value_hop = {
      "Returns the inclusive lower and exclusive upper bound of the corresponding hopping window.", {"Tuple(DateTime, DateTime)"}};
  FunctionDocumentation::Examples examples_hop = {
      {"Hopping window", "SELECT hop(now(), INTERVAL '1' DAY, INTERVAL '2' DAY)", "('2024-07-03 00:00:00','2024-07-05 00:00:00')"}};
  ...
  FunctionDocumentation documentation_hop = {description_hop,    syntax_hop,   arguments_hop,     {},
                                             returned_value_hop, examples_hop, introduced_in_hop, category_hop};
...
Since one record can be assigned to multiple hop windows, the function only returns the bound of the first window when hop function is used without `WINDOW VIEW`.
  ...
  FunctionDocumentation::Arguments arguments_hop_start = {{"time_attr", "Date and time.", {"DateTime"}},
                                                          {"hop_interval", "Positive Hop interval.", {"Interval"}},
                                                          {"window_interval", "Positive Window interval.", {"Interval"}},
                                                          {"timezone", "Optional. Timezone name.", {"String"}}};
  FunctionDocumentation::ReturnedValue returned_value_hop_start = {"Returns the inclusive lower bound of the corresponding hopping window.",
                                                                   {"DateTime"}};
  FunctionDocumentation::Examples examples_hop_start = {
      {"Hopping window start", "SELECT hopStart(now(), INTERVAL '1' DAY, INTERVAL '2' DAY)", "2024-07-03 00:00:00"}};
  ...
  FunctionDocumentation documentation_hop_start = {
      description_hop_start,    syntax_hop_start,   arguments_hop_start,     {},
      returned_value_hop_start, examples_hop_start, introduced_in_hop_start, category_hop_start};
...
Since one record can be assigned to multiple hop windows, the function only returns the bound of the first window when hop function is used without `WINDOW VIEW`.
  ...
  FunctionDocumentation::Arguments arguments_hop_end = {{"time_attr", "Date and time.", {"DateTime"}},
                                                        {"hop_interval", "Positive Hop interval.", {"Interval"}},
                                                        {"window_interval", "Positive Window interval.", {"Interval"}},
                                                        {"timezone", "Optional. Timezone name.", {"String"}}};
  FunctionDocumentation::ReturnedValue returned_value_hop_end = {"Returns the exclusive upper bound of the corresponding hopping window.",
                                                                 {"DateTime"}};
  FunctionDocumentation::Examples examples_hop_end = {
      {"Hopping window end", "SELECT hopEnd(now(), INTERVAL '1' DAY, INTERVAL '2' DAY)", "2024-07-05 00:00:00"}};
  ...
  FunctionDocumentation documentation_hop_end = {description_hop_end,    syntax_hop_end,   arguments_hop_end,     {},
                                                 returned_value_hop_end, examples_hop_end, introduced_in_hop_end, category_hop_end};
...
This function can only be used with `WINDOW VIEW`.
  ...
  FunctionDocumentation::Arguments arguments_window_id = {{"time_attr", "Date and time.", {"DateTime"}},
                                                          {"interval", "Window interval in Interval.", {"Interval"}},
                                                          {"timezone", "Optional. Timezone name.", {"String"}}};
  FunctionDocumentation::ReturnedValue returned_value_window_id = {"Returns the window identifier of the corresponding window.",
                                                                   {"UInt32"}};
  ...
  FunctionDocumentation documentation_window_id = {
      description_window_id,    syntax_window_id,   arguments_window_id,     {},
      returned_value_window_id, examples_window_id, introduced_in_window_id, category_window_id};
