...
const ColumnConst &checkAndGetColumnConst(const IColumn &column) {
  ...
  if (!checkColumn<Type>(&data_column))
    throw DB::Exception(DB::ErrorCodes::LOGICAL_ERROR, "Unexpected const column type: expected {}, got {}", demangle(typeid(Type).name()),
                        demangle(typeid(data_column).name()));
...
///
/// With multiple variadic arguments, e.g. f(a, b, ...[c]), provided arguments must match left-to-right. E.g. these calls are considered
/// valid:
///     f(a, b)
///     f(a, b, c)
///     f(a, b, c, c)
