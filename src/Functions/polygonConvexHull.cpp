...
class FunctionPolygonConvexHull : public IFunction {
 public:
  ...
  static FunctionPtr create(ContextPtr) { return std::make_shared<FunctionPolygonConvexHull>(); }
  ...
  DataTypePtr getReturnTypeImpl(const DataTypes &) const override { return DataTypeFactory::instance().get("Polygon"); }
...
Coordinates are in Cartesian coordinate system.
  ...
  FunctionDocumentation::Arguments arguments = {{"multipolygon", "A MultiPolygon value.", {"MultiPolygon"}}};
  FunctionDocumentation::ReturnedValue returned_value = {"Returns the convex hull as a Polygon.", {"Polygon"}};
  FunctionDocumentation::Examples examples = {{"Conve hull example",
                                               R"(
