  ...
  for (const auto &key : keys) {
    ...
    if (models.contains(model_name)) {
      throw Exception(ErrorCodes::EXCESSIVE_ELEMENT_IN_CONFIG,
                      "Duplicate model name {} found in <nb_models>. Please use unique names for each model", model_name);
    }
    ...
    for (const auto &prior_key : prior_keys) {
      ...
      if (prior <= 0.0) {
        throw Exception(ErrorCodes::BAD_ARGUMENTS, "Prior probability must be greater than 0 for model {} and class {}. Got {}", model_name,
                        class_id, prior);
      }

      if (prior > 1.0) {
        throw Exception(ErrorCodes::BAD_ARGUMENTS, "Prior probability must be less than or equal to 1 for model {} and class {}. Got {}",
                        model_name, class_id, prior);
      }
    }

    if (!priors.empty() && fabs(total_prior_prob - 1.0) > 1e-6) {
      throw Exception(ErrorCodes::BAD_ARGUMENTS, "Sum of provided priors probability is not equal to 1.0 for model {}. Sum: {}", model_name,
                      total_prior_prob);
    }
    ...
    if (alpha <= 0.0) {
      throw Exception(ErrorCodes::BAD_ARGUMENTS, "Laplace smoothing parameter 'alpha' must be greater than 0 for model {}", model_name);
    }

    const String mode = config.getString(model_mode_path);

    if (mode == "byte") {
      models.emplace(std::piecewise_construct, std::forward_as_tuple(model_name),
      ...
      throw Exception(ErrorCodes::BAD_ARGUMENTS, "Invalid mode {} for model {}. Only 'byte', 'codepoint', and 'token' are available", mode,
                      model_name);
...
class FunctionNaiveBayesClassifier : public IFunction {
 ...
 public:
  ...
  DataTypePtr getReturnTypeImpl(const ColumnsWithTypeAndName &arguments) const override {
    ...
    if (!model_name_argument_type.isStringOrFixedString())
      throw Exception(ErrorCodes::BAD_ARGUMENTS, "Function {} first argument type should be String. Actual {}", getName(),
                      arguments[0].type->getName());

    const auto input_text_argument_type = WhichDataType(arguments[1].type);
    if (!input_text_argument_type.isStringOrFixedString())
      throw Exception(ErrorCodes::BAD_ARGUMENTS, "Function {} second argument type should be String. Actual {}", getName(),
 ...
 private:
  void validateModelName(const String &model_name) const {
    ...
    if (!models.contains(model_name)) {
      throw Exception(ErrorCodes::BAD_ARGUMENTS, "Model {} not found. Available models: {}", model_name,
...
REGISTER_FUNCTION(NaiveBayesClassifier) {
  FunctionDocumentation::Description description =
      "Classifies input text using a Naive Bayes model with ngrams and Laplace smoothing. "
  ...
  FunctionDocumentation::Examples examples = {{"Classify the language of a text",
  ...
  FunctionDocumentation function_documentation = {.description = description,
                                                  .syntax = syntax,
                                                  .arguments = arguments,
