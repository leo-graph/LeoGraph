...
class FunctionNumericIndexedVectorHelper {
 public:
  ColumnPtr executeHelper(const DataTypePtr index_type, const DataTypePtr value_type, const Array &parameters,
  ...
  ColumnPtr executeResolveIndexType(const DataTypePtr index_type, const DataTypePtr value_type, const Array &parameters,
                                    const ColumnsWithTypeAndName &arguments, const DataTypePtr &result_type,
                                    size_t input_rows_count) const {
    WhichDataType which(index_type);

#define DISPATCH(TYPE) \
  if (which.idx == TypeIndex::TYPE) return executeResolveValueType<TYPE>(value_type, parameters, arguments, result_type, input_rows_count);
...
#undef DISPATCH
  ...
  ColumnPtr executeResolveValueType(const DataTypePtr value_type, const Array &parameters, const ColumnsWithTypeAndName &arguments,
                                    const DataTypePtr &result_type, size_t input_rows_count) const {
    WhichDataType which(value_type);
#define DISPATCH(TYPE) \
  if (which.idx == TypeIndex::TYPE) return executeDispatcher<IndexType, TYPE>(parameters, arguments, result_type, input_rows_count);
...
#undef DISPATCH
  ...
  ColumnPtr executeDispatcher(const Array &parameters, const ColumnsWithTypeAndName &arguments, const DataTypePtr &result_type,
                              size_t input_rows_count) const {
    if (parameters.empty() || applyVisitor(FieldVisitorToString(), parameters[0]) != "BSI") {
      return static_cast<const Derived *>(this)->template executeBSI<BSINumericIndexedVector<IndexType, ValueType>>(arguments, result_type,
                                                                                                                    input_rows_count);
    } else {
