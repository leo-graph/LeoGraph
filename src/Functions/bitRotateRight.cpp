...
struct BitRotateRightImpl {
  ...
  static llvm::Value* compile(llvm::IRBuilder<>& b, llvm::Value* left, llvm::Value* right, bool) {
    if (!left->getType()->isIntegerTy()) throw Exception(ErrorCodes::LOGICAL_ERROR, "BitRotateRightImpl expected an integral type");
...
struct NameBitRotateRight {
  static constexpr auto name = "bitRotateRight";
};
...
REGISTER_FUNCTION(BitRotateRight) {
  ...
  FunctionDocumentation::Arguments arguments = {{"a", "A value to rotate.", {"(U)Int8/16/32/64"}},
