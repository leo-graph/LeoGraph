...
class FunctionNeighbor : public IFunction {
 public:
  ...
  DataTypePtr getReturnTypeImpl(const DataTypes &arguments) const override {
    ...
    if (number_of_arguments < 2 || number_of_arguments > 3)
      throw Exception(ErrorCodes::NUMBER_OF_ARGUMENTS_DOESNT_MATCH,
                      "Number of arguments for function {} doesn't match: passed {}, should be from 2 to 3", getName(),
                      number_of_arguments);

    // second argument must be an integer
    if (!isInteger(arguments[1]))
      throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT, "Illegal type {} of second argument of function {} - should be an integer",
                      arguments[1]->getName(), getName());
    if (arguments[1]->isNullable())
      throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT, "Illegal type {} of second argument of function {} - can not be Nullable",
                      arguments[1]->getName(), getName());

    // check that default value column has supertype with first argument
    if (number_of_arguments == 3) return getLeastSupertype(DataTypes{arguments[0], arguments[2]});
  ...
  ColumnPtr executeImpl(const ColumnsWithTypeAndName &arguments, const DataTypePtr &result_type, size_t input_rows_count) const override {
    ...
    if (has_defaults) default_is_constant = isColumnConst(*default_column_cast);

    if (source_is_constant) source_column_cast = assert_cast<const ColumnConst &>(*source_column_cast).getDataColumnPtr();
    if (offset_is_constant) offset_column = assert_cast<const ColumnConst &>(*offset_column).getDataColumnPtr();
    if (default_is_constant) default_column_cast = assert_cast<const ColumnConst &>(*default_column_cast).getDataColumnPtr();
    ...
    if (offset_is_constant) {
      ...
      auto insert_range_from = [&](bool is_const, const ColumnPtr &src, Int64 begin, Int64 size) {
        ...
        if (size <= 0) return;
        if (size > static_cast<Int64>(input_rows_count)) size = input_rows_count;

        if (!src) {
          for (Int64 i = 0; i < size; ++i) result_column->insertDefault();
        } else if (is_const) {
          for (Int64 i = 0; i < size; ++i) result_column->insertFrom(*src, 0);
        } else {
      ...
      if (offset == 0) {
        ...
        return source_is_constant ? ColumnConst::create(source_column_cast, input_rows_count) : source_column_cast;
      }
      if (offset > 0) {
...
The function can be enabled by setting `allow_deprecated_error_prone_window_functions = 1`.
  ...
  FunctionDocumentation::Arguments arguments = {
      ...
      {"default_value",
       "Optional. The value to return if the offset goes beyond the data bounds. If not specified, uses the default value for the column "
       "type.",
       {"Any"}}};
  FunctionDocumentation::ReturnedValue returned_value = {"Returns a value from the specified offset, or default if out of bounds.",
                                                         {"Any"}};
