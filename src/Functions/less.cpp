...
REGISTER_FUNCTION(Less) {
  ...
  FunctionDocumentation::Examples examples = {{"Usage example", "SELECT 1 < 2, 2 < 1;", R"(
┌─less(1, 2)─┬─less(2, 1)─┐
│          1 │          0 │
└────────────┴────────────┘
)"}};
...
template <>
ColumnPtr FunctionComparison<LessOp, NameLess>::executeTupleImpl(const ColumnsWithTypeAndName& x, const ColumnsWithTypeAndName& y,
  ...
  return executeTupleLessGreaterImpl(less, less, func_builder_and, func_builder_or, func_builder_equals, x, y, tuple_size,
                                     input_rows_count);
}
