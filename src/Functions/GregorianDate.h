...
class GregorianDate {
 public:
   ...
   */
  void write(WriteBuffer& buf) const { writeImpl<void>(buf); }

  bool tryWrite(WriteBuffer& buf) const { return writeImpl<bool>(buf); }

  /** Convert to a string in text form 'YYYY-MM-DD'.
   */
  std::string toString() const;

  int32_t year() const noexcept { return year_; }

  uint8_t month() const noexcept { return month_; }

  uint8_t dayOfMonth() const noexcept { return day_of_month_; }
...
class OrdinalDate {
 public:
  ...
  uint16_t dayOfYear() const noexcept { return day_of_year_; }
