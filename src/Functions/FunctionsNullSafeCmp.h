...
enum class NullSafeCmpMode : uint8_t { NullSafeEqual, NullSafeNotEqual };

template <typename Name,                                  // Function Name
...
class FunctionsNullSafeCmp : public IFunction {
  ...
  static bool containsNothing(const DataTypePtr &type) {
    ...
    if (const auto *tuple_type = typeid_cast<const DataTypeTuple *>(type.get())) {
      for (const auto &elem : tuple_type->getElements()) {
        if (containsNothing(elem)) return true;
      }
    }
    return false;
  }
 ...
 public:
  ...
  DataTypePtr getReturnTypeImpl(const DataTypes &arguments) const override {
    if (arguments.size() != 2)
      throw Exception(ErrorCodes::NUMBER_OF_ARGUMENTS_DOESNT_MATCH, "Function {} expects exactly 2 arguments, got {}", backQuote(name),
                      arguments.size());

    const DataTypePtr &left_ele_type = arguments[0];
    const DataTypePtr &right_ele_type = arguments[1];

    if (containsNothing(left_ele_type) || containsNothing(right_ele_type)) return std::make_shared<DataTypeNothing>();

    if ((isMap(left_ele_type) && right_ele_type->onlyNull()) || (left_ele_type->onlyNull() && isMap(right_ele_type)) ||
        (isArray(left_ele_type) && right_ele_type->onlyNull()) || (left_ele_type->onlyNull() && isArray(right_ele_type))) {
  ...
  ColumnPtr ALWAYS_INLINE executeForVariantOrDynamicAndNull(const ColumnWithTypeAndName &variant_or_dynamic_col) const {
    ...
    const auto &column_variant_or_dynamic = isVariant(variant_or_dynamic_col.type)
  ...
  ColumnPtr executeImpl(const ColumnsWithTypeAndName &arguments, const DataTypePtr &result_type, size_t input_rows_count) const override {
    ...
    if (!left_col || !right_col) {
      throw Exception(ErrorCodes::BAD_ARGUMENTS,
                      ...
                      backQuote(name), left_col ? "NOT NULL" : "NULL", right_col ? "NOT NULL" : "NULL");
    }

    // for self null-safe cmp
    if (type_and_name_left_col.name == type_and_name_right_col.name && type_and_name_left_col.type->equals(*type_and_name_right_col.type) &&
        !isTuple(type_and_name_left_col.type) && left_col.get() == right_col.get()) {
    ...
    if (((isMap(type_and_name_left_col.type) || isArray(type_and_name_left_col.type)) && type_and_name_right_col.type->onlyNull()) ||
        ((isMap(type_and_name_right_col.type) || isArray(type_and_name_right_col.type)) && type_and_name_left_col.type->onlyNull())) {
    ...
    //   1. Variant vs null
    //   2. Dynamic vs null
    if (((isVariant(type_and_name_left_col.type) || isDynamic(type_and_name_left_col.type)) && type_and_name_right_col.type->onlyNull()) ||
        ((isVariant(type_and_name_right_col.type) || isDynamic(type_and_name_right_col.type)) && type_and_name_left_col.type->onlyNull())) {
      return executeForVariantOrDynamicAndNull(isVariant(type_and_name_left_col.type) || isDynamic(type_and_name_left_col.type)
    ...
    FunctionOverloadResolverPtr comparator = std::make_unique<FunctionToOverloadResolverAdaptor>(
        std::make_shared<FunctionComparison<CompareOp, CompareName, true /*is null safe cmp mode*/>>(params));
