...
struct SipHashKeyColumns {
  ...
  size_t size() const {
    ...
    if (offsets != nullptr && !offsets->empty()) return offsets->back();
    return key0->size();
  }
  ...
  SipHashKey getKey(size_t i) const {
    ...
    if (offsets != nullptr && i > 0) {
      ...
      if (upper != offsets->end()) i = upper - begin;
    }
...
static SipHashKeyColumns parseSipHashKeyColumns(const ColumnWithTypeAndName &key) {
  ...
  if (result.size() == 1) result.is_const = true;
...
struct HalfMD5Impl {
  ...
  static UInt64 apply(const char *begin, size_t size) {
    ...
    if (!ctx) throw Exception(ErrorCodes::OPENSSL_ERROR, "EVP_MD_CTX_new failed: {}", getOpenSSLErrors());
  ...
  static UInt64 combineHashes(UInt64 h1, UInt64 h2) { return combineHashesFunc<UInt64, HalfMD5Impl>(h1, h2); }
...
///     murmur2:
///     https://github.com/apache/kafka/blob/461c5cfe056db0951d9b74f5adc45973670404d7/clients/src/main/java/org/apache/kafka/common/utils/Utils.java#L480
///     Default Partitioner:
///     https://github.com/apache/kafka/blob/139f7709bd3f5926901a21e55043388728ccca78/clients/src/main/java/org/apache/kafka/clients/producer/internals/BuiltInPartitioner.java#L328
struct KafkaMurmurHashImpl {
  static constexpr auto name = "kafkaMurmurHash";

  using ReturnType = UInt32;

  static UInt32 apply(const char *data, const size_t size) { return MurmurHash2(data, size, 0x9747b28cU) & 0x7fffffff; }
...
struct JavaHashImpl {
  ...
  template <class T>
    requires(!std::same_as<T, int8_t> && !std::same_as<T, int16_t> && !std::same_as<T, int32_t>)
  static ReturnType apply(T x) {
    if (std::is_unsigned_v<T>) throw Exception(ErrorCodes::NOT_IMPLEMENTED, "Unsigned types are not supported");
  ...
  static ReturnType apply(const char *data, const size_t size) {
    UInt32 h = 0;
    for (size_t i = 0; i < size; ++i) h = 31 * h + static_cast<UInt32>(static_cast<Int8>(data[i]));
...
struct JavaHashUTF16LEImpl {
  ...
  static Int32 apply(const char *raw_data, const size_t raw_size) {
    ...
    if (size % 2 != 0) throw Exception(ErrorCodes::BAD_ARGUMENTS, "Arguments for javaHashUTF16LE must be in the form of UTF-16");
...
class FunctionIntHash : public TargetSpecific::Default::FunctionIntHash<Impl, Name> {
 public:
  explicit FunctionIntHash(ContextPtr context) : selector(context) {
    ...
    selector.registerImplementation<TargetArch::x86_64_v3, TargetSpecific::x86_64_v3::FunctionIntHash<Impl, Name>>();
    selector.registerImplementation<TargetArch::x86_64_v4, TargetSpecific::x86_64_v4::FunctionIntHash<Impl, Name>>();
#endif
  }
...
class FunctionAnyHash : public TargetSpecific::Default::FunctionAnyHash<Impl, Keyed, KeyType, KeyColumnsType> {
 public:
  explicit FunctionAnyHash(ContextPtr context) : selector(context) {
    selector.registerImplementation<TargetArch::Default, TargetSpecific::Default::FunctionAnyHash<Impl, Keyed, KeyType, KeyColumnsType>>();

#if USE_MULTITARGET_CODE
    selector
        .registerImplementation<TargetArch::x86_64_v3, TargetSpecific::x86_64_v3::FunctionAnyHash<Impl, Keyed, KeyType, KeyColumnsType>>();
    selector
...
struct URLHierarchyHashImpl {
  static size_t findLevelLength(const UInt64 level, const char *begin, const char *end) {
    ...
    while (pos < end && ((*pos > 'a' && *pos < 'z') || (*pos > '0' && *pos < '9'))) ++pos;
...
class FunctionURLHash : public IFunction {
 public:
  ...
  DataTypePtr getReturnTypeImpl(const DataTypes &arguments) const override {
    ...
    if (arg_count == 2) {
      ...
      if (!isInteger(second_arg))
        throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT, "Illegal type {} of argument of function {}", second_arg->getName(),
                        getName());
    }
  ...
  ColumnPtr executeImpl(const ColumnsWithTypeAndName &arguments, const DataTypePtr &, size_t /*input_rows_count*/) const override {
    ...
    if (arg_count == 1) return executeSingleArg(arguments);
    if (arg_count == 2) return executeTwoArgs(arguments);
 ...
 private:
  ColumnPtr executeSingleArg(const ColumnsWithTypeAndName &arguments) const {
    ...
    if (const auto *col_from = checkAndGetColumn<ColumnString>(col_untyped)) {
      ...
      ColumnString::Offset current_offset = 0;
      for (size_t i = 0; i < size; ++i) {
        out[i] = URLHashImpl::apply(reinterpret_cast<const char *>(&chars[current_offset]), offsets[i] - current_offset);
  ...
  ColumnPtr executeTwoArgs(const ColumnsWithTypeAndName &arguments) const {
    ...
    if (const auto *col_from = checkAndGetColumn<ColumnString>(col_untyped)) {
      ...
      ColumnString::Offset current_offset = 0;
      for (size_t i = 0; i < size; ++i) {
        out[i] = URLHierarchyHashImpl::apply(level_col->getUInt(i), reinterpret_cast<const char *>(&chars[current_offset]),
                                             offsets[i] - current_offset);
