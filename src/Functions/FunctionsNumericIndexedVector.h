...
class FunctionNumericIndexedVectorBuildImpl : public IFunction,
 ...
 public:
  ...
  DataTypePtr getReturnTypeImpl(const DataTypes &arguments) const override {
    ...
    if (!arg) throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT, "The argument for function {} must be Map type", getName());
    DataTypes argument_types = {arg->getKeyType(), arg->getValueType()};
    Array params_row;
    AggregateFunctionProperties properties;
    AggregateFunctionPtr numeric_indexed_vector_function;
    auto action = NullsAction::EMPTY;
    numeric_indexed_vector_function = AggregateFunctionFactory::instance().get(NameAggregateFunctionGroupNumericIndexedVector::name, action,
  ...
  ColumnPtr executeBSI(const ColumnsWithTypeAndName &arguments, const DataTypePtr &result_type, size_t input_rows_count) const {
    ...
    if (!map_column)
      throw Exception(ErrorCodes::ILLEGAL_COLUMN, "Argument for function {} must be map, got {} instead", getName(),
    ...
    for (size_t i = 0; i < input_rows_count; ++i) {
      ...
      for (size_t j = from; j < to; ++j) {
        res->vector.addValue(applyVisitor(FieldVisitorConvertToNumber<typename VectorImpl::IndexType>(), keys_data[j]),
...
class FunctionNumericIndexedVector : public IFunction, public FunctionNumericIndexedVectorHelper<FunctionNumericIndexedVector<FuncImpl>> {
 public:
  ...
  DataTypePtr getReturnTypeImpl(const DataTypes &arguments) const override {
    ...
    if (!(type0 && type0->getFunctionName() == NameAggregateFunctionGroupNumericIndexedVector::name))
      throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT,
                      "First argument for function {} must be a NumericIndexedVector but it has type {}", getName(),
                      arguments[0]->getName());
    ...
    if (type1 && type1->getFunctionName() != NameAggregateFunctionGroupNumericIndexedVector::name)
      throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT,
                      "Second argument for function {} must be a NumericIndexedVector or Numeric Value", getName());

    if (type1 && (type0->getArgumentsDataTypes()[0]->getTypeId() != type1->getArgumentsDataTypes()[0]->getTypeId() ||
  ...
  ColumnPtr executeBSI(const ColumnsWithTypeAndName &arguments, const DataTypePtr &, size_t input_rows_count) const {
    ...
    for (int i = 0; i < 2; ++i) is_column_const[i] = isColumnConst(*arguments[i].column);
    ...
    if (is_second_column_agg) {
      ...
      return executeBSIData<VectorImpl>(first_column_ptr, is_column_const[0], second_column_ptr, is_column_const[1], input_rows_count);
    } else {
      ...
      return executeBSIDataWithScalar<VectorImpl>(first_column_ptr, is_column_const[0], second_column_ptr, is_column_const[1],
                                                  input_rows_count);
    }
  }
 ...
 private:
  template <typename VectorImpl>
  ColumnPtr executeBSIData(const ColumnAggregateFunction *first_column_ptr, bool is_first_column_const,
                           const ColumnAggregateFunction *second_column_ptr, bool is_second_column_const, size_t input_rows_count) const {
  ...
  template <typename VectorImpl>
  ColumnPtr executeBSIDataWithScalar(const ColumnAggregateFunction *first_column_ptr, bool is_first_column_const,
                                     const PaddedPODArray<typename VectorImpl::ValueType> *second_column_ptr, bool is_second_column_const,
...
struct NumericIndexedVectorPointwiseAddImpl {
  ...
  static void apply(const AggregateFunctionGroupNumericIndexedVectorData<VectorImpl> &lhs, const typename VectorImpl::ValueType &rhs,
  ...
  static void apply(const AggregateFunctionGroupNumericIndexedVectorData<VectorImpl> &lhs,
...
#undef DISPATCH
    throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT, "Unexpected value type {} of argument of function {}",
                    type0->getArgumentsDataTypes()[1]->getName(), getName());
  }
