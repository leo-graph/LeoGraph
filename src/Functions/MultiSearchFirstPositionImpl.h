...
struct MultiSearchFirstPositionImpl {
  ...
  static void vectorConstant(const ColumnString::Chars &haystack_data, const ColumnString::Offsets &haystack_offsets,
                             const Array &needles_arr, PaddedPODArray<UInt64> &res, PaddedPODArray<UInt64> & /*offsets*/,
    ...
    if (needles_arr.size() > std::numeric_limits<UInt8>::max())
      throw Exception(ErrorCodes::TOO_MANY_ARGUMENTS_FOR_FUNCTION,
                      "Number of arguments for function {} doesn't match: passed {}, should be at most {}", name,
                      std::to_string(needles_arr.size()), std::to_string(std::numeric_limits<UInt8>::max()));
    ...
    while (searcher.hasMoreToSearch()) {
      ...
      for (size_t j = 0; j < input_rows_count; ++j) {
        ...
        if (iteration == 0 || res[j] == 0)
          ...
          if (result != 0) res[j] = std::min(result, res[j]);
        }
