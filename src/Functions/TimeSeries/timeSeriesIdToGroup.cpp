...
class FunctionTimeSeriesIdToGroup : public IFunction {
 public:
  ...
  static void checkArgumentTypes(const ColumnsWithTypeAndName& arguments) {
    if (arguments.size() != 1) {
      throw Exception(ErrorCodes::NUMBER_OF_ARGUMENTS_DOESNT_MATCH, "Function {} must be called with one argument: {}(id)", name, name);
    }
  ...
  ColumnPtr executeImpl(const ColumnsWithTypeAndName& arguments, const DataTypePtr& result_type, size_t input_rows_count) const override {
    ...
    if (id_type == typeid(UInt64)) return executeForIDType<UInt64>(arguments, result_type, input_rows_count);
    if (id_type == typeid(UInt128)) return executeForIDType<UInt128>(arguments, result_type, input_rows_count);
    UNREACHABLE();
  }
...
See also function [timeSeriesStoreTags()](/sql-reference/functions/time-series-functions#timeSeriesStoreTags).
  ...
  FunctionDocumentation::ReturnedValue returned_value = {"Returns a group of tags associated with the identifier `id` of a time series.",
                                                         {"UInt64"}};
