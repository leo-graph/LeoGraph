...
///
/// Function timeSeriesFromGrid(start_timestamp, end_timestamp, step, [value1, value2, value3, ..., valueN]) converts array of values
/// [value1, value2, value3, ...] to array of tuples [(start_timestamp, value1), (start_timestamp + step, value2), (start_timestamp + 2 *
/// step, value3), ..., (end_timestamp, valueN)].
...
class FunctionTimeSeriesRange : public IFunction {
 public:
  ...
  DataTypePtr getReturnTypeImpl(const ColumnsWithTypeAndName &arguments) const override {
    ...
    if (isDateTime64(start_timestamp_type)) timestamp_scale = std::max(timestamp_scale, getDecimalScale(*start_timestamp_type));
    if (isDateTime64(end_timestamp_type)) timestamp_scale = std::max(timestamp_scale, getDecimalScale(*end_timestamp_type));
    if (isDecimal(step_type)) timestamp_scale = std::max(timestamp_scale, getDecimalScale(*step_type));
  ...
  static void checkDataTypes(const ColumnsWithTypeAndName &arguments) {
    ...
    if (!(isDateTimeOrDateTime64(start_timestamp_type) || isUInt32(start_timestamp_type)))
      throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT,
                      "Argument #{} (start_timestamp) of function {} has wrong type {}, it must be {}", 1, name, start_timestamp_type,
                      "DateTime64 or DateTime or UInt32");
    ...
    if (!(WhichDataType(step_type).isDecimal32() || WhichDataType(step_type).isDecimal64() || isNativeInteger(step_type)))
      throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT, "Argument #{} (step) of function {} has wrong type {}, it must be {}", 3, name,
                      step_type, "a number");

    if constexpr (with_values) {
      const auto &values_type = arguments[3].type;
      if (!isArray(values_type))
        throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT, "Argument #{} of function {} has wrong type {}, it must be {}", 4, name,
                        values_type, "an array of values");
    }
  }
  ...
  ColumnPtr executeImpl(const ColumnsWithTypeAndName &arguments, const DataTypePtr &result_type, size_t input_rows_count) const override {
    ...
    if constexpr (with_values)
      result_timestamp_type =
          typeid_cast<const DataTypeTuple &>(*typeid_cast<const DataTypeArray &>(*result_type).getNestedType()).getElement(0);
    else
    ...
    if (isDateTime64(*start_timestamp_type)) start_timestamp_scale = getDecimalScale(*start_timestamp_type);
    chassert(start_timestamp_scale <= result_timestamp_scale);
    Int64 start_timestamp_multiplier = (start_timestamp_scale < result_timestamp_scale)
                                           ? DecimalUtils::scaleMultiplier<Int64>(result_timestamp_scale - start_timestamp_scale)
                                           : 1;

    UInt32 end_timestamp_scale = 0;
    if (isDateTime64(*end_timestamp_type)) end_timestamp_scale = getDecimalScale(*end_timestamp_type);
    chassert(end_timestamp_scale <= result_timestamp_scale);
    Int64 end_timestamp_multiplier = (end_timestamp_scale < result_timestamp_scale)
                                         ? DecimalUtils::scaleMultiplier<Int64>(result_timestamp_scale - end_timestamp_scale)
                                         : 1;

    UInt32 step_scale = 0;
    if (isDecimal(*step_type)) step_scale = getDecimalScale(*step_type);
    chassert(step_scale <= result_timestamp_scale);
    Int64 step_multiplier =
        (step_scale < result_timestamp_scale) ? DecimalUtils::scaleMultiplier<Int64>(result_timestamp_scale - step_scale) : 1;
    ...
    if constexpr (with_values) {
      const auto &values_type = arguments[3].type;
      values_are_nullable = (typeid_cast<const DataTypeArray *>(values_type.get()) &&
    ...
    if (isDateTime64(result_timestamp_type)) {
      return doExecute<DateTime64, Decimal64>(start_timestamp_column, start_timestamp_multiplier, end_timestamp_column,
                                              end_timestamp_multiplier, step_column, step_multiplier, values_column, values_are_nullable,
                                              result_type, input_rows_count);
    } else if (isDateTime(result_timestamp_type) || isUInt32(result_timestamp_type)) {
      return doExecute<UInt32, Int32>(start_timestamp_column, start_timestamp_multiplier, end_timestamp_column, end_timestamp_multiplier,
                                      step_column, step_multiplier, values_column, values_are_nullable, result_type, input_rows_count);
    } else {
      throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT, "Illegal result type {} of function {}, it must be {}", result_type, name,
                      "DateTime64 or DateTime or UInt32");
    }
  }

  template <typename TimestampType, typename IntervalType>
  static ColumnPtr doExecute(const IColumn &start_timestamp_column, Int64 start_timestamp_multiplier, const IColumn &end_timestamp_column,
                             Int64 end_timestamp_multiplier, const IColumn &step_column, Int64 step_multiplier,
                             const IColumn *values_column, bool values_are_nullable, const DataTypePtr &result_type, size_t num_rows) {
    ...
    if constexpr (with_values) {
      ...
      if (!array_column || (values_are_nullable && !checkColumn<ColumnNullable>(array_column->getData()))) {
        ...
        if (full_column.get() != values_column) {
          return doExecute<TimestampType, IntervalType>(start_timestamp_column, start_timestamp_multiplier, end_timestamp_column,
                                                        end_timestamp_multiplier, step_column, step_multiplier, full_column.get(),
                                                        values_are_nullable, result_type, num_rows);
        }
    ...
    for (size_t i = 0; i != num_rows; ++i) {
      ...
      if (end_timestamp < start_timestamp) throw Exception(ErrorCodes::BAD_ARGUMENTS, "End timestamp is less than start timestamp");

      size_t num_steps = 1;
      if (start_timestamp != end_timestamp) {
        if (step <= static_cast<IntervalType>(0)) throw Exception(ErrorCodes::BAD_ARGUMENTS, "Step should be greater than zero");
...
For example, for `[value1, NULL, x2]` the function returns `[(start_timestamp, x1), (start_timestamp + 2 * step, x2)]`.
  ...
  FunctionDocumentation::ReturnedValue returned_value = {
      "Returns values from the source array of values combined with timestamps on a regular time grid described by `start_timestamp` and "
      "`step`.",
