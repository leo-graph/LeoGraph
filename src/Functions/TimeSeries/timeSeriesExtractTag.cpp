...
class FunctionTimeSeriesExtractTag : public IFunction {
 public:
  ...
  static void checkArgumentTypes(const ColumnsWithTypeAndName& arguments) {
    if (arguments.size() != 2) {
      throw Exception(ErrorCodes::NUMBER_OF_ARGUMENTS_DOESNT_MATCH,
                      "Function {} must be called with two arguments: {}(group, tag_to_extract)", name, name);
    }
  ...
  ColumnPtr executeImpl(const ColumnsWithTypeAndName& arguments, const DataTypePtr& /* result_type */,
    ...
    for (size_t i = 0; i != input_rows_count; ++i) null_map->insertValue(tag_values->getDataAt(i).empty());
...
See also function [timeSeriesGroupToTags()](/sql-reference/functions/time-series-functions#timeSeriesGroupToTags).
  ...
  FunctionDocumentation::ReturnedValue returned_value = {"Returns the value of a specified tag.", {"Nullable(String)"}};
