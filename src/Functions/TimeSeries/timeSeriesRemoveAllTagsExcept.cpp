...
class FunctionTimeSeriesRemoveAllTagsExcept : public IFunction {
 public:
  ...
  explicit FunctionTimeSeriesRemoveAllTagsExcept(ContextPtr context)
  ...
  /// Function timeSeriesRemoveAllTagsExcept uses information stored in the query context, it's deterministic in the scope of the current
  /// query.
  ...
  static void checkArgumentTypes(const ColumnsWithTypeAndName& arguments) {
    if (arguments.size() != 2) {
      throw Exception(ErrorCodes::NUMBER_OF_ARGUMENTS_DOESNT_MATCH,
                      "Function {} must be called with two arguments: {}(group, tags_to_keep)", name, name);
    }
...
[timeSeriesRemoveTags()](/sql-reference/functions/time-series-functions#timeSeriesRemoveTags).
  ...
  FunctionDocumentation::ReturnedValue returned_value = {"A new group of tags with only the specified tags kept.", {"UInt64"}};
