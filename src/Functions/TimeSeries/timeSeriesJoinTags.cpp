  ...
  static void checkArgumentTypes(const ColumnsWithTypeAndName& arguments) {
    if (arguments.size() != 4) {
      throw Exception(ErrorCodes::NUMBER_OF_ARGUMENTS_DOESNT_MATCH,
                      "Function {} must be called with four arguments: {}(group, dest_tag, separator, src_tags)", name, name);
    }
...
[label_join()](https://prometheus.io/docs/prometheus/latest/querying/functions/#label_join).
  ...
  FunctionDocumentation::ReturnedValue returned_value = {"Returns a new group of tags with the `dest_tag` tag set to the joined result.",
                                                         {"UInt64"}};
