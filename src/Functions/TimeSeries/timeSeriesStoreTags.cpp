...
class FunctionTimeSeriesStoreTags : public IFunction {
 public:
  ...
  static void checkArgumentTypes(const ColumnsWithTypeAndName& arguments) {
    if (arguments.size() < 2) {
      throw Exception(ErrorCodes::NUMBER_OF_ARGUMENTS_DOESNT_MATCH,
                      "Function {} must be called with at least 2 arguments: {}(id, [('tag_name_1', 'tag_value_1), ...], 'tag_name_2', "
                      "'tag_value_2', ...)",
                      name, name);
    }
  ...
  ColumnPtr executeImpl(const ColumnsWithTypeAndName& arguments, const DataTypePtr& result_type, size_t input_rows_count) const override {
    ...
    if (id_type == typeid(UInt64)) return executeForIDType<UInt64, false>(arguments, result_type, input_rows_count);
    if (id_type == typeid(std::optional<UInt64>)) return executeForIDType<UInt64, true>(arguments, result_type, input_rows_count);
    if (id_type == typeid(UInt128)) return executeForIDType<UInt128, false>(arguments, result_type, input_rows_count);
    if (id_type == typeid(std::optional<UInt128>)) return executeForIDType<UInt128, true>(arguments, result_type, input_rows_count);
    UNREACHABLE();
  }

  template <typename IDType, bool id_is_nullable>
  ColumnPtr executeForIDType(const ColumnsWithTypeAndName& arguments, const DataTypePtr& /* result_type */,
...
can be used to access this mapping later during the query execution.
  ...
  FunctionDocumentation::ReturnedValue returned_value = {"Returns the identifier of a time series (i.e. just the first argument)."};
  FunctionDocumentation::Examples examples = {{"Example",
                                               R"(
