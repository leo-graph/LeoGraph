  ...
  static void checkArgumentTypes(const ColumnsWithTypeAndName& arguments) {
    if (arguments.size() != 2) {
      throw Exception(ErrorCodes::NUMBER_OF_ARGUMENTS_DOESNT_MATCH,
                      "Function {} must be called with two arguments: {}(group, tag_to_remove)", name, name);
    }
...
[timeSeriesRemoveAllTagsExcept()](/sql-reference/functions/time-series-functions#timeSeriesRemoveAllTagsExcept).
  ...
  FunctionDocumentation::ReturnedValue returned_value = {"A new group of tags without the specified tag.", {"UInt64"}};
