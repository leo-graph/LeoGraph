  ...
  static void checkArgumentTypes(const ColumnsWithTypeAndName& arguments) {
    if (arguments.size() != 5) {
      throw Exception(ErrorCodes::NUMBER_OF_ARGUMENTS_DOESNT_MATCH,
                      "Function {} must be called with five arguments: {}(group, dest_tag, replacement, src_tag, regex)", name, name);
    }
...
[label_replace()](https://prometheus.io/docs/prometheus/latest/querying/functions/#label_replace).
  ...
  FunctionDocumentation::Arguments arguments = {
      ...
      {"replacement",
       "A replacement pattern, can contain $1, $2 or $name to refer capturing groups in the regular expression 'regex'.",
       {"String"}},
  ...
  FunctionDocumentation::ReturnedValue returned_value = {"A new group of tags with maybe `dest_tag` added.", {"UInt64"}};
