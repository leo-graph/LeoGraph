...
class FunctionTimeSeriesCopyTags : public IFunction {
 public:
  ...
  static void checkArgumentTypes(const ColumnsWithTypeAndName& arguments) {
    if (arguments.size() != 3) {
      throw Exception(ErrorCodes::NUMBER_OF_ARGUMENTS_DOESNT_MATCH,
                      "Function {} must be called with three arguments: {}(dest_group, src_group, tags_to_copy)", name, name);
    }
  ...
  ColumnPtr executeImpl(const ColumnsWithTypeAndName& arguments, const DataTypePtr& /* result_type */,
    ...
    auto dest_groups =
        TimeSeriesTagsFunctionHelpers::extractGroupFromArgument(name, arguments, 0, /* return_single_element_if_const_column = */ true);
    auto src_groups =
        TimeSeriesTagsFunctionHelpers::extractGroupFromArgument(name, arguments, 1, /* return_single_element_if_const_column = */ true);
...
[group left/group right](https://prometheus.io/docs/prometheus/latest/querying/operators/#group-modifiers) modifiers.
  ...
  FunctionDocumentation::Arguments arguments = {{"dest_group", "The destination group of tags.", {"UInt64"}},
