...
class FunctionTimeSeriesThrowDuplicateSeriesIf : public IFunction {
 public:
  ...
  static void checkArgumentTypes(const ColumnsWithTypeAndName& arguments) {
    if (arguments.size() != 2) {
      throw Exception(ErrorCodes::NUMBER_OF_ARGUMENTS_DOESNT_MATCH, "Function {} must be called with two arguments: {}(condition, group)",
                      name, name);
    }

    if (!isNativeNumber(arguments[0].type)) {
      throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT, "First argument of function {} must be a number (passed: {})", name,
 ...
 private:
  size_t findPositionOfNonZero(const ColumnPtr& column, const DataTypePtr& data_type) const {
    ...
    if (which.isUInt8()) return findPositionOfNonZeroImpl<UInt8>(column, data_type);
    if (which.isUInt16()) return findPositionOfNonZeroImpl<UInt16>(column, data_type);
    if (which.isUInt32()) return findPositionOfNonZeroImpl<UInt32>(column, data_type);
    if (which.isUInt64()) return findPositionOfNonZeroImpl<UInt64>(column, data_type);
    if (which.isInt8()) return findPositionOfNonZeroImpl<Int8>(column, data_type);
    if (which.isInt16()) return findPositionOfNonZeroImpl<Int16>(column, data_type);
    if (which.isInt32()) return findPositionOfNonZeroImpl<Int32>(column, data_type);
    if (which.isInt64()) return findPositionOfNonZeroImpl<Int64>(column, data_type);
    if (which.isFloat32()) return findPositionOfNonZeroImpl<Float32>(column, data_type);
    if (which.isFloat64()) return findPositionOfNonZeroImpl<Float64>(column, data_type);
  ...
  size_t findPositionOfNonZeroImpl(const ColumnPtr& column, const DataTypePtr& data_type) const {
    ...
    if (!in) in = checkAndGetColumnConstData<ColumnVector<T>>(column.get());
