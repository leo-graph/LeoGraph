...
void checkTypeOfTagsArrayArgument(std::string_view function_name, const DataTypePtr &type, size_t argument_index) {
  ...
  if (const auto *map_type = typeid_cast<const DataTypeMap *>(nested_type.get())) nested_type = map_type->getNestedType();
  ...
  if (const auto *array_type = typeid_cast<const DataTypeArray *>(nested_type.get())) {
    if (const auto *tuple_type = typeid_cast<const DataTypeTuple *>(array_type->getNestedType().get())) {
      if (tuple_type->getElements().size() == 2) {
        ...
        if (isStringOrFixedString(first_type) && isStringOrFixedString(second_type)) return;
      }
    }

    if (isNothing(array_type->getNestedType())) return;
  }

  if (isNothing(nested_type)) return;

  throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT, "Argument #{} of function {} has wrong type {}, it must be {}", argument_index + 1,
                  function_name, type, "Array(Tuple(String, String))");
}

/// Extracts tags from a column of type Array(Tuple(String, String)) containing pairs (tag_name, tag_value).
void extractTagsArrayFromColumn(std::string_view function_name, size_t argument_index, const IColumn &column_tags_array,
  ...
  if (const auto *array_column = checkAndGetColumn<ColumnArray>(&column_tags_array)) {
    ...
    if (checkColumn<ColumnNothing>(&array_column->getData())) return;
  }
  ...
  if (const auto *nullable_column = checkAndGetColumn<ColumnNullable>(&column_tags_array)) {
    ...
    if (checkColumn<ColumnNothing>(&nested_column)) return;
  }
  ...
  throw Exception(ErrorCodes::ILLEGAL_COLUMN, "Illegal column {} of argument #{} of function {}, it must be {}",
...
void checkTypeOfTagNameArgument(std::string_view function_name, const DataTypePtr &type, size_t argument_index) {
  ...
  if (isStringOrFixedString(nested_type) || isNothing(nested_type)) return;

  throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT, "Argument #{} of function {} has wrong type {}, it must be {}", argument_index + 1,
                  function_name, type, "String or FixedString");
}
...
void extractTagNameAndValueFromTwoColumns(const IColumn &column_tag_name, const IColumn &column_tag_value,
                                          std::vector<TagNamesAndValues> &out_tags_vector) {
...
void sortTagsAndRemoveDuplicates(std::string_view function_name, std::vector<TagNamesAndValues> &tags_vector) {
  ...
  auto is_tag_name_empty = [](const std::pair<String, String> &x) { return x.first.empty(); };
  auto is_tag_value_empty = [](const std::pair<String, String> &x) { return x.second.empty(); };
...
std::vector<Group> extractGroupFromColumn(std::string_view function_name, size_t argument_index, const IColumn &column,
                                          bool return_single_element_if_const_column) {
  ...
  throw Exception(ErrorCodes::ILLEGAL_COLUMN, "Illegal column {} of argument #{} of function {}, it must be {}", column.getName(),
                  argument_index + 1, function_name, "UInt64");
}
...
void extractIDFromColumn(std::string_view function_name, size_t argument_index, const IColumn &column, std::vector<IDType> &out_ids) {
  ...
  if constexpr (std::is_convertible_v<UInt128, IDType>) {
    ...
    if (checkColumn<ColumnUInt128>(&column) || checkColumn<ColumnUUID>(&column) ||
  ...
  if (const auto *nullable_column = checkAndGetColumn<ColumnNullable>(&column)) {
    ...
    for (size_t i = 0; i != num_rows; ++i) {
      if (null_map[i]) out_ids[i] = IDType{};
    }
    return;
  }
  ...
  throw Exception(ErrorCodes::ILLEGAL_COLUMN, "Illegal column {} of argument #{} of function {}, it must be {}", column.getName(),
                  argument_index + 1, function_name, "UInt64 or UInt128 or UUID");
}
}  // namespace

void checkArgumentTypesForTagNamesAndValues(std::string_view function_name, const ColumnsWithTypeAndName &arguments,
                                            size_t start_argument_index) {
  if (start_argument_index >= arguments.size())
    throw Exception(ErrorCodes::NUMBER_OF_ARGUMENTS_DOESNT_MATCH, "Function {} should be called with at least {} arguments", function_name,
  ...
  if (start_argument_index + 1 < arguments.size()) {
    ...
    if ((arguments.size() % 2) != nargs_should_be_odd) {
      throw Exception(ErrorCodes::NUMBER_OF_ARGUMENTS_DOESNT_MATCH, "Expected {} number of arguments of function {}",
                      nargs_should_be_odd ? "odd" : "even", function_name);
    }
...
std::vector<TagNamesAndValuesPtr> extractTagNamesAndValuesFromArguments(std::string_view function_name,
...
void checkArgumentTypeForGroup(std::string_view function_name, const ColumnsWithTypeAndName &arguments, size_t argument_index) {
  if (argument_index >= arguments.size())
    throw Exception(ErrorCodes::NUMBER_OF_ARGUMENTS_DOESNT_MATCH, "Function {} can't be called with {} arguments", function_name,
                    arguments.size());
...
std::vector<Group> extractGroupFromArgument(std::string_view function_name, const ColumnsWithTypeAndName &arguments, size_t argument_index,
                                            bool return_single_element_if_const_column) {
...
std::vector<String> extractConstStringsFromArgument(std::string_view function_name, const ColumnsWithTypeAndName &arguments,
  ...
  if (!array_column)
    throw Exception(ErrorCodes::ILLEGAL_COLUMN, "Argument #{} of function {} must be a constant Array(String)", argument_index + 1,
                    function_name);

  if (checkAndGetColumn<ColumnNothing>(&array_column->getData())) return {};
  ...
  Strings res;
  res.reserve(count);
  for (size_t i = 0; i != count; ++i) res.emplace_back(String{string_column->getDataAt(i)});

  return res;
}

std::vector<String> extractConstTagNamesFromArgument(std::string_view function_name, const ColumnsWithTypeAndName &arguments,
                                                     size_t argument_index) {
...
const std::type_info &checkArgumentTypeForID(std::string_view function_name, const ColumnsWithTypeAndName &arguments, size_t argument_index,
                                             bool allow_nullable) {
  if (argument_index >= arguments.size())
    throw Exception(ErrorCodes::NUMBER_OF_ARGUMENTS_DOESNT_MATCH, "Function {} can't be called with {} arguments", function_name,
                    arguments.size());
  ...
  if (isUInt64(type)) return is_nullable ? typeid(std::optional<UInt64>) : typeid(UInt64);
  ...
  throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT, "Argument #{} of function {} has wrong type {}, it must be {}", argument_index + 1,
                  function_name, type, "UInt64 or UInt128 or UUID");
}
...
template std::vector<UInt64> extractIDFromArgument<UInt64>(std::string_view function_name, const ColumnsWithTypeAndName &arguments,
                                                           size_t argument_index);
template std::vector<std::optional<UInt64>> extractIDFromArgument<std::optional<UInt64>>(std::string_view function_name,
                                                                                         const ColumnsWithTypeAndName &arguments,
                                                                                         size_t argument_index);
template std::vector<UInt128> extractIDFromArgument<UInt128>(std::string_view function_name, const ColumnsWithTypeAndName &arguments,
                                                             size_t argument_index);
template std::vector<std::optional<UInt128>> extractIDFromArgument<std::optional<UInt128>>(std::string_view function_name,
                                                                                           const ColumnsWithTypeAndName &arguments,
                                                                                           size_t argument_index);
...
ColumnPtr makeColumnForGroup(const std::vector<Group> &groups) {
  ...
  for (auto group : groups) res->insertValue(group);
  return res;
}

ColumnPtr makeColumnForTagNamesAndValues(const std::vector<TagNamesAndValuesPtr> &tags_vector) {
  size_t total_tags = 0;
  for (const auto &tags : tags_vector) total_tags += tags->size();
