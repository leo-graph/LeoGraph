...
class FunctionMultiIf final : public FunctionIfBase {
 public:
  ...
  static FunctionPtr create(ContextPtr context_) {
    ...
    return std::make_shared<FunctionMultiIf>(settings[Setting::allow_execute_multiif_columnar],
  ...
  explicit FunctionMultiIf(bool allow_execute_multiif_columnar_, bool use_variant_as_common_type_)
      : allow_execute_multiif_columnar(allow_execute_multiif_columnar_), use_variant_as_common_type(use_variant_as_common_type_) {}
  ...
  ColumnNumbers getArgumentsThatDontImplyNullableReturnType(size_t number_of_arguments) const override {
    ColumnNumbers args;
    for (size_t i = 0; i + 1 < number_of_arguments; i += 2) args.push_back(i);
    return args;
  }
  ...
  DataTypePtr getReturnTypeImpl(const DataTypes &args) const override {
    ...
    auto for_conditions = [&args](auto &&f) {
      ...
      for (size_t i = 0; i < conditions_end; i += 2) f(args[i]);
    };

    auto for_branches = [&args](auto &&f) {
      size_t branches_end = args.size();
      for (size_t i = 1; i < branches_end; i += 2) f(args[i]);
      f(args.back());
    };
    ...
    for_conditions([&](const DataTypePtr &arg) {
      ...
      if (!WhichDataType(nested_type).isUInt8())
        throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT,
                        "Illegal type {} of argument (condition) of function {}. "
                        "Must be UInt8.",
                        arg->getName(), getName());
    });
    ...
    if (use_variant_as_common_type) return getLeastSupertypeOrVariant(types_of_branches);
  ...
  ColumnPtr executeImpl(const ColumnsWithTypeAndName &args, const DataTypePtr &result_type, size_t input_rows_count) const override {
    ...
    for (size_t i = 0; i < arguments.size(); i += 2) {
      ...
      if (instruction.source && isColumnConst(*instruction.source)) instruction.source_is_constant = true;

      instructions.emplace_back(std::move(instruction));

      if (instructions.back().condition_always_true) break;
    }
    ...
    bool execute_multiif_columnar = allow_execute_multiif_columnar && instructions.size() <= std::numeric_limits<UInt8>::max() &&
                                    (which.isInt() || which.isUInt() || which.isFloat() || which.isDecimal() ||
                                     which.isDateOrDate32OrDateTimeOrDateTime64() || which.isEnum() || which.isIPv4() || which.isIPv6());
...
#define ENUMERATE_NUMERIC_TYPES(M, INDEX)                                                                                  \
  ...
  M(IPv4, IPv4, INDEX)                                                                                                     \
  M(IPv6, IPv6, INDEX)                                                                                                     \
  throw Exception(ErrorCodes::NOT_IMPLEMENTED, "Columnar execution of function {} not implemented for type {}", getName(), \
                  result_type->getName());
 ...
 private:
  ...
  static NO_INLINE void calculateInserts(const std::vector<Instruction> &instructions, size_t rows, PaddedPODArray<S> &inserts) {
    for (S i = static_cast<S>(instructions.size() - 1); i != static_cast<S>(-1); --i) {
      ...
      if (instruction.condition_always_true) {
        for (size_t row_i = 0; row_i < rows; ++row_i) inserts[row_i] = i;
      } else if (!instruction.condition_is_nullable) {
  ...
  static NO_INLINE void executeInstructionsColumnar(const std::vector<Instruction> &instructions, size_t rows, PaddedPODArray<T> &res_data,
    ...
    if constexpr (nullable_result) {
      if (!res_null_map) throw Exception(ErrorCodes::BAD_ARGUMENTS, "Invalid result null_map while result type is nullable");
    ...
    if constexpr (nullable_result) {
      for (auto &col : null_map_cols) {
        if (!col) {
          if (!shared_null_map) shared_null_map = std::make_unique<PaddedPODArray<UInt8>>(rows, 0);
    ...
    for (size_t row_i = 0; row_i < rows; ++row_i) {
      ...
      if constexpr (nullable_result) (*res_null_map)[row_i] = *(null_map_cols[insert] + index);
    }
  }
  ...
  static void executeShortCircuitArguments(ColumnsWithTypeAndName &arguments) {
    ...
    if (last_short_circuit_argument_index < 0) return;
    ...
    while (i <= last_short_circuit_argument_index) {
      ...
      if (!condition_mask_info.has_zeros) break;

      ++i;
      if (i > last_short_circuit_argument_index) break;

      /// Extract mask only if it make sense.
      if (condition_mask_info.has_ones) mask_info = extractInvertedMask(mask, cond_column);
    ...
    for (; i <= last_short_circuit_argument_index; ++i) executeColumnIfNeeded(arguments[i], true);
  }
...
FunctionOverloadResolverPtr createInternalMultiIfOverloadResolver(bool allow_execute_multiif_columnar, bool use_variant_as_common_type) {
  return std::make_unique<FunctionToOverloadResolverAdaptor>(
      std::make_shared<FunctionMultiIf>(allow_execute_multiif_columnar, use_variant_as_common_type));
}
