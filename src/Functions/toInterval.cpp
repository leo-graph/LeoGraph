...
class FunctionToInterval : public IFunction {
 public:
  ...
  DataTypePtr getReturnTypeImpl(const ColumnsWithTypeAndName &arguments) const override {
    if (arguments.size() != 2) throw Exception(ErrorCodes::NUMBER_OF_ARGUMENTS_DOESNT_MATCH, "Function {} must be 2 arguments", getName());
...
toInterval(value, unit)
  ...
  FunctionDocumentation::Arguments arguments = {
      {"value",
       "The numeric value representing the number of units. Can be any numeric type.",
       {"Int8", "Int16", "Int32", "Int64", "UInt8", "UInt16", "UInt32", "UInt64", "Float32", "Float64"}},
      {"unit",
       R"(The unit of time. Must be a constant string. Valid values: 'nanosecond', 'microsecond', 'millisecond', 'second', 'minute', 'hour', 'day', 'week', 'month', 'quarter', 'year'.)",
       {"String"}}};
  FunctionDocumentation::ReturnedValue returned_value = {
      R"(Returns an Interval value of the specified type. The result type depends on the unit: IntervalNanosecond, IntervalMicrosecond, IntervalMillisecond, IntervalSecond, IntervalMinute, IntervalHour, IntervalDay, IntervalWeek, IntervalMonth, IntervalQuarter, or IntervalYear.)",
      {"Interval"}};
  FunctionDocumentation::Examples examples = {{"Create intervals with different units", R"(
SELECT
