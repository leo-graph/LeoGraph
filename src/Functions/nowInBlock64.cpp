...
class FunctionNowInBlock64 : public IFunction {
 public:
  ...
  DataTypePtr getReturnTypeImpl(const ColumnsWithTypeAndName &arguments) const override {
    ...
    if (arguments.empty()) return std::make_shared<DataTypeDateTime64>(DataTypeDateTime64::default_scale);

    auto scale = static_cast<UInt32>(arguments[0].column->get64(0));

    if (arguments.size() == 1) return std::make_shared<DataTypeDateTime64>(scale);
  ...
  ColumnPtr executeImpl(const ColumnsWithTypeAndName &, const DataTypePtr &type, size_t input_rows_count) const override {
    ...
    if (!data_type_datetime64) throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT, "Expected a DateTime64, got {}", type->getName());
    ...
    for (size_t i = 0; i < input_rows_count; ++i) vec_res[i] = now.getValue();
...
nowInBlock64([scale[, timezone]])
  ...
  FunctionDocumentation::Arguments arguments = {{"scale",
                                                 "Optional. Tick size (precision): 10^-precision seconds. Valid range: [0 : 9]. Typically, "
                                                 "are used - 3 (default) (milliseconds), 6 (microseconds), 9 (nanoseconds).",
                                                 {"UInt8"}},
  ...
  FunctionDocumentation::Examples examples = {{"Difference with the now64() function", R"(
SELECT
    now64(),
    nowInBlock64(),
    sleep(1)
