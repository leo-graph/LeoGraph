...
class FunctionDynamicElement : public IFunction {
  ...
  DataTypePtr getReturnTypeImpl(const ColumnsWithTypeAndName &arguments) const override {
    ...
    if (number_of_arguments != 2)
      throw Exception(ErrorCodes::NUMBER_OF_ARGUMENTS_DOESNT_MATCH,
                      "Number of arguments for function {} doesn't match: passed {}, should be 2", getName(), number_of_arguments);
    ...
    if (!isDynamic(*input_type))
      throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT, "First argument for function {} must be Variant or Array of Variant. Actual {}",
  ...
  ColumnPtr executeImpl(const ColumnsWithTypeAndName &arguments, const DataTypePtr &, size_t input_rows_count) const override {
    ...
    if (!input_col_as_dynamic || !input_type_as_dynamic)
      throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT,
                      "First argument for function {} must be Dynamic or array of Dynamics. Actual {}", getName(),
 ...
 private:
  DataTypePtr getRequestedType(const ColumnPtr &type_name_column) const {
    ...
    if (!name_col) throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT, "Second argument of {} must be a constant String", getName());
...
for scalar types or an empty array for array types.
  ...
  FunctionDocumentation::ReturnedValue returned_value = {
      ...
      {"Any"}};
  FunctionDocumentation::Examples examples = {{"Extracting different types from Dynamic column",
                                               R"(
