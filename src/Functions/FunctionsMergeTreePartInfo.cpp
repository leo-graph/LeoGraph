...
MergeTreePartInfo constructPartInfo(std::string_view data) {
  if (auto info = tryParseMergeTreePartInfo(data)) return std::move(info.value());
...
UnpackedPartSegments unpackPartName(std::string_view data) {
  ...
  if (number_of_segments < 4) throwInvalidPartName(data);
...
bool isAnyStringType(const IDataType &data_type) { return isStringOrFixedString(removeLowCardinality(data_type.getPtr())); }
...
class FunctionMergeTreePartCoverage : public IFunction {
  static MergeTreePartInfo constructCoveringPart(const ColumnPtr &covering_column, size_t row_number) {
    if (isColumnConst(*covering_column)) return unpackPartName(covering_column->getDataAt(0)).part_info;
 ...
 public:
  ...
  DataTypePtr getReturnTypeImpl(const ColumnsWithTypeAndName &arguments) const override {
    validateFunctionArguments(
        *this, arguments,
        FunctionArgumentDescriptors{{"nested_part", static_cast<FunctionArgumentDescriptor::TypeValidator>(&isAnyStringType), nullptr,
                                     "String or FixedString or LowCardinality String"},
                                    {"covering_part", static_cast<FunctionArgumentDescriptor::TypeValidator>(&isAnyStringType), nullptr,
                                     "String or FixedString or LowCardinality String"}});
...
class FunctionMergeTreePartInfo : public IFunction {
 public:
  ...
  DataTypePtr getReturnTypeImpl(const ColumnsWithTypeAndName &arguments) const override {
    validateFunctionArguments(
        *this, arguments,
        FunctionArgumentDescriptors{{"part_name", static_cast<FunctionArgumentDescriptor::TypeValidator>(&isAnyStringType), nullptr,
                                     "String or FixedString or LowCardinality String"}});

    DataTypes types = {
        std::make_shared<DataTypeString>(), std::make_shared<DataTypeString>(), std::make_shared<DataTypeString>(),
        std::make_shared<DataTypeInt64>(),  std::make_shared<DataTypeInt64>(),  std::make_shared<DataTypeInt64>(),
        std::make_shared<DataTypeInt64>(),
    };

    Names names = {
        "partition_id", "prefix", "suffix", "min_block", "max_block", "level", "mutation",
    };
...
Function which checks if the part of the first argument is covered by the part of the second argument.
  ...
  FunctionDocumentation::Arguments arguments_coverage = {{"nested_part", "Name of expected nested part.", {"String"}},
  ...
  FunctionDocumentation::Examples examples_coverage = {{"Basic example",
                                                        R"(
...
└────────────────────────────────────┴────────────────────────────────────┘
  ...
  FunctionDocumentation documentation_coverage = {description_coverage,    syntax_coverage,   arguments_coverage,     {},
                                                  returned_value_coverage, examples_coverage, introduced_in_coverage, category_coverage};
...
Function that helps to cut the useful values out of the `MergeTree` part name.
  ...
  FunctionDocumentation::Arguments arguments_info = {{"part_name", "Name of part to unpack.", {"String"}}};
  ...
  FunctionDocumentation::Examples examples_info = {{"Basic example",
                                                    R"(
...
└───────────────────┴────────────────┴────────────────┴────────────┴───────────────┘
  ...
  FunctionDocumentation documentation_info = {description_info,    syntax_info,   arguments_info,     {},
                                              returned_value_info, examples_info, introduced_in_info, category_info};
