 ...
 private:
  ...
  DataTypePtr getReturnTypeImpl(const DataTypes &arguments) const override {
    ...
    if (arguments.size() > 10000) {
      throw Exception(ErrorCodes::TOO_MANY_ARGUMENTS_FOR_FUNCTION, "Number of arguments of function {} is too large (maximum: 10000).",
                      getName());
    }
    ...
    for (const auto arg_idx : collections::range(0, arguments.size())) {
      ...
      if (!WhichDataType(arg).isFloat64()) {
        throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT,
                        ...
                        "Must be Float64",
                        arg->getName(), std::to_string(arg_idx + 1), getName());
      }
    }
  ...
  ColumnPtr executeImpl(const ColumnsWithTypeAndName &arguments, const DataTypePtr &, size_t input_rows_count) const override {
    ...
    for (const auto ellipse_idx : collections::range(0, ellipses_count)) {
      ...
      for (const auto idx : collections::range(0, 4)) {
        ...
        if (const auto *col = checkAndGetColumnConst<ColumnVector<Float64>>(column)) {
          ...
          throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT,
                          "Illegal type {} of argument {} of function {}. "
                          "Must be const Float64",
                          column->getName(), std::to_string(arg_idx + 1), getName());
        }
      }
    ...
    if (const_cnt == 0) {
      ...
      for (size_t row = 0; row < input_rows_count; ++row)
        dst_data[row] =
            isPointInEllipses(col_vec_x->getData()[row], col_vec_y->getData()[row], ellipses.data(), ellipses_count, start_index);
    ...
    if (const_cnt == 2) {
      ...
      size_t start_index = 0;
      UInt8 res = isPointInEllipses(col_const_x->getValue<Float64>(), col_const_y->getValue<Float64>(), ellipses.data(), ellipses_count,
                                    start_index);
    ...
    throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT,
                    ...
                    col_x->getName(), col_y->getName(), getName());
  }
...
Coordinates are geometric in the Cartesian coordinate system.
  ...
  FunctionDocumentation::Arguments arguments = {{"x, y", "Coordinates of a point on the plane.", {"Float64"}},
  ...
  FunctionDocumentation::ReturnedValue returned_value = {"Returns `1` if the point is inside at least one of the ellipses, `0` otherwise",
                                                         {"UInt8"}};
  FunctionDocumentation::Examples examples = {{"Basic usage", "SELECT pointInEllipses(10., 10., 10., 9.1, 1., 0.9999)",
                                               R"(
