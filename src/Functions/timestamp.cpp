 ...
 public:
  ...
  ColumnPtr executeImpl(const ColumnsWithTypeAndName &arguments, const DataTypePtr &, size_t input_rows_count) const override {
    ...
    if (const ColumnString *col_timestamp_string = checkAndGetColumn<ColumnString>(col_timestamp)) {
      ...
      throw Exception(ErrorCodes::ILLEGAL_COLUMN, "Illegal column {} of 1st argument of function {}. Must be String or FixedString",
                      col_timestamp->getName(), getName());
    }

    if (arguments.size() == 1) return col_result;
    ...
    if (const ColumnString *col_time_string = checkAndGetColumn<ColumnString>(col_time)) {
      ...
      throw Exception(ErrorCodes::ILLEGAL_COLUMN, "Illegal column {} of 2nd argument of function {}. Must be String or FixedString",
                      col_time->getName(), getName());
    }
...
If a second argument `expr_time` is provided, it adds the specified time to the converted value.
  ...
  FunctionDocumentation::Arguments arguments = {{"expr", "Date or date with time.", {"String"}},
                                                {"expr_time", "Optional. Time to add to the converted value.", {"String"}}};
  FunctionDocumentation::ReturnedValue returned_value = {"Returns the converted value of `expr`, or `expr` with added time",
                                                         {"DateTime64(6)"}};
  FunctionDocumentation::Examples examples = {{"Convert date string to DateTime64(6)", R"(
