...
DataTypePtr FunctionInitializeAggregation::getReturnTypeImpl(const ColumnsWithTypeAndName &arguments) const {
  FunctionArgumentDescriptors mandatory_args{{"aggregate_function_name", &isString, &isColumnConst, "const String"},
...
ColumnPtr FunctionInitializeAggregation::executeImpl(const ColumnsWithTypeAndName &arguments, const DataTypePtr &result_type,
  ...
  for (size_t i = 0; i < input_rows_count; ++i) {
    ...
    try {
      ...
      for (size_t j = 0; j < i; ++j) agg_func.destroy(places[j]);
      throw;
    }
  }

  SCOPE_EXIT_MEMORY_SAFE({
    for (size_t i = 0; i < input_rows_count; ++i) agg_func.destroy(places[i]);
  });
  ...
  if (agg_func.isState()) {
    ...
    for (size_t i = 0; i < input_rows_count; ++i) agg_func.insertMergeResultInto(places[i], res_col, arena.get());
  } else {
    for (size_t i = 0; i < input_rows_count; ++i) agg_func.insertResultInto(places[i], res_col, arena.get());
  }
...
You can create states of aggregate functions and insert them to columns of type [`AggregateFunction`](../../sql-reference/data-types/aggregatefunction.md) or use initialized aggregates as default values.
  ...
  FunctionDocumentation::Arguments arguments = {{"aggregate_function", "Name of the aggregation function to initialize.", {"String"}},
                                                {"arg1[, arg2, ...]", "Arguments of the aggregate function.", {"Any"}}};
  FunctionDocumentation::ReturnedValue returned_value = {
      "Returns the result of aggregation for every row passed to the function. The return type is the same as the return type of the "
      "function that `initializeAggregation` takes as a first argument.",
      {"Any"}};
  FunctionDocumentation::Examples examples = {{"Basic usage with uniqState",
                                               R"(
