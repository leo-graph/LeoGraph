...
class UserDefinedFunction final : public IFunction {
 public:
  explicit UserDefinedFunction(ExternalUserDefinedExecutableFunctionsLoader::UserDefinedExecutableFunctionPtr executable_function_,
                               ContextPtr context_, Array parameters_)
      : executable_function(std::move(executable_function_)), context(std::move(context_)) {
    ...
    if (command_parameters_size != parameters_.size())
      throw Exception(ErrorCodes::BAD_ARGUMENTS,
                      "Executable user defined function {} number of parameters does not match. Expected {}. Actual {}", configuration.name,
                      command_parameters_size, parameters_.size());
    ...
    for (size_t i = 0; i < command_parameters_size; ++i) {
      ...
      auto try_replace_parameter_placeholder_with_value = [&](std::string &command_part) {
        ...
        while (true) {
          ...
          if (parameter_placeholder_position == std::string::npos) break;
      ...
      for (auto &command_argument : command_arguments_with_parameters) try_replace_parameter_placeholder_with_value(command_argument);

      try_replace_parameter_placeholder_with_value(command_with_parameters);

      if (!find_placedholder) {
        throw Exception(ErrorCodes::BAD_ARGUMENTS, "Executable user defined function {} no placeholder for parameter {}",
                        configuration.name, command_parameter.name);
      }
    }
  }
  ...
  ColumnPtr executeImpl(const ColumnsWithTypeAndName &arguments, const DataTypePtr &result_type, size_t input_rows_count) const override {
    ...
    if (coordinator_configuration.execute_direct) {
      ...
      if (!fileOrSymlinkPathStartsWith(script_path, user_scripts_path))
        throw Exception(ErrorCodes::UNSUPPORTED_METHOD, "Executable file {} must be inside user scripts folder {}", command,
                        user_scripts_path);

      if (!FS::exists(script_path))
        throw Exception(ErrorCodes::UNSUPPORTED_METHOD, "Executable file {} does not exist inside user scripts folder {}", command,
                        user_scripts_path);

      if (!FS::canExecute(script_path))
        throw Exception(ErrorCodes::UNSUPPORTED_METHOD, "Executable file {} is not executable inside user scripts folder {}", command,
                        user_scripts_path);
    ...
    for (size_t i = 0; i < argument_size; ++i) {
      ...
      if (arguments_copy[i].type->equals(*argument_type)) continue;
    ...
    try {
      ...
      Pipe pipe = coordinator->createPipe(command, command_arguments_with_parameters, std::move(shell_input_pipes), result_block, context,
      ...
      if (result_column_size != input_rows_count)
        throw Exception(ErrorCodes::UNSUPPORTED_METHOD, "Function {}: wrong result, expected {} row(s), actual {}", quoteString(getName()),
                        input_rows_count, result_column_size);

      return result_column;
    } catch (...) {
      std::vector<String> quoted_arguments_with_parameters;
      for (const auto &argument : command_arguments_with_parameters) quoted_arguments_with_parameters.push_back("\"" + argument + "\"");
      ...
      throw Exception(ErrorCodes::UDF_EXECUTION_FAILED,
                      ...
                      "Command: '{}', Arguments: [{}]. "
                      "Original error: {}",
                      getName(), command_with_parameters, quoted_arguments_string, error_message);
    }
    UNREACHABLE();
  }
...
std::vector<String> UserDefinedExecutableFunctionFactory::getRegisteredNames(ContextPtr context) {
  ...
  for (auto &loaded_object : loaded_objects) registered_names.emplace_back(loaded_object->getLoadableName());
