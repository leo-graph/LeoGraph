...
void backupUserDefinedSQLObjects(BackupEntriesCollector& backup_entries_collector, const String& data_path_in_backup,
                                 UserDefinedSQLObjectType object_type, const std::vector<std::pair<String, ASTPtr>>& objects) {
  ...
  for (const auto& [object_name, create_object_query] : objects)
    backup_entries.emplace_back(escapeForFileName(object_name) + ".sql",
                                std::make_shared<BackupEntryFromMemory>(create_object_query->formatWithSecretsOneLine()));
  ...
  backup_entries_collector.addPostTask([my_backup_entries = std::move(backup_entries), my_replication_id = std::move(replication_id),
                                        object_type, &backup_entries_collector, backup_coordination] {
...
std::vector<std::pair<String, ASTPtr>> restoreUserDefinedSQLObjects(RestorerFromBackup& restorer, const String& data_path_in_backup,
  ...
  if (filenames.empty()) return {};  /// Nothing to restore.
  ...
  for (const auto& filename : filenames) {
    ...
    switch (object_type) {
      case UserDefinedSQLObjectType::Function: {
        ...
        ast = parseQuery(parser, statement_def.data(), statement_def.data() + statement_def.size(),
                         "in file " + filepath + " from backup " + backup->getNameForLogging(), 0,
