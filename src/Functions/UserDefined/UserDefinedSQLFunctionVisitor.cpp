...
void UserDefinedSQLFunctionVisitor::visit(ASTPtr& ast, ContextPtr context_) {
  ...
  for (auto& child : ast->children) {
    ...
    if (new_ptr != old_ptr) ast->updatePointerToChild(old_ptr, new_ptr);
  }
  ...
  if (const auto* function = ast->template as<ASTFunction>()) {
    ...
    if (replace_result) ast = replace_result;
  }
}
...
ASTPtr UserDefinedSQLFunctionVisitor::tryToReplaceFunction(const ASTFunction& function,
                                                           std::unordered_set<std::string>& udf_in_replace_process, ContextPtr context_) {
  ...
  if (!user_defined_function) return nullptr;
  ...
  if (!create_function_query)
    throw Exception(ErrorCodes::UNSUPPORTED_METHOD,
                    "The function '{}' is not a SQL defined function and is not supported when 'enable_analyzer' is set to false",
  ...
  if (function_arguments.size() != identifiers_raw.size())
    throw Exception(ErrorCodes::UNSUPPORTED_METHOD, "Function {} expects {} arguments actual arguments {}",
                    create_function_query->getFunctionName(), identifiers_raw.size(), function_arguments.size());

  for (const auto& arg : function_arguments) {
    if (isVariadic(arg))
      throw Exception(ErrorCodes::BAD_ARGUMENTS, "It is not possible to replace a variadic argument '{}' in UDF {}", arg->getColumnName(),
                      function.name);
  }

  if (isVariadic(function_core_expression->children.at(1)))
    throw Exception(ErrorCodes::BAD_ARGUMENTS, "It is not possible to replace a variadic argument '{}' in UDF {}",
                    function_core_expression->children.at(1)->getColumnName(), function.name);
  ...
  while (!ast_nodes_to_update.empty()) {
    ...
    for (auto& child : ast_node_to_update->children) {
      if (auto* inner_function = child->as<ASTFunction>()) {
        ...
        if (replace_result) child = replace_result;
      }
      ...
      if (identifier_name_opt) {
        ...
        if (function_argument_it == identifier_name_to_function_argument.end()) continue;

        auto child_alias = child->tryGetAlias();
        child = function_argument_it->second->clone();

        if (!child_alias.empty()) child->setAlias(child_alias);
  ...
  if (!function_alias.empty()) function_body_to_update->setAlias(function_alias);
