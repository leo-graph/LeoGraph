...
ASTPtr UserDefinedSQLObjectsDiskStorage::tryLoadObject(UserDefinedSQLObjectType object_type, const String& object_name, const String& path,
                                                       bool check_file_exists) {
  LOG_DEBUG(log, "Loading user defined object {} from file {}", backQuote(object_name), path);

  try {
    if (check_file_exists && !fs::exists(path)) return nullptr;
    ...
    switch (object_type) {
      case UserDefinedSQLObjectType::Function: {
        ...
        ASTPtr ast = parseQuery(parser, object_create_query.data(), object_create_query.data() + object_create_query.size(), "", 0,
...
void UserDefinedSQLObjectsDiskStorage::loadObjects() {
  if (!objects_loaded) loadObjectsImpl();
}

void UserDefinedSQLObjectsDiskStorage::reloadObjects() { loadObjectsImpl(); }
...
void UserDefinedSQLObjectsDiskStorage::loadObjectsImpl() {
  ...
  for (Poco::DirectoryIterator it(dir_path); it != dir_end; ++it) {
    ...
    if (!startsWith(file_name, "function_") || !endsWith(file_name, ".sql")) continue;
...
void UserDefinedSQLObjectsDiskStorage::createDirectory() {
  ...
  if (!fs::exists(dir_path) || !fs::is_directory(dir_path) || create_dir_error_code)
    throw Exception(ErrorCodes::DIRECTORY_DOESNT_EXIST, "Couldn't create directory {} reason: '{}'", dir_path,
                    create_dir_error_code.message());
}

bool UserDefinedSQLObjectsDiskStorage::storeObjectImpl(const ContextPtr& /*current_context*/, UserDefinedSQLObjectType object_type,
  ...
  try {
    ...
    out.next();
    if (settings[Setting::fsync_metadata]) out.sync();
    out.close();
...
bool UserDefinedSQLObjectsDiskStorage::removeObjectImpl(const ContextPtr& /*current_context*/, UserDefinedSQLObjectType object_type,
  ...
  if (!existed) {
    if (throw_if_not_exists) throw Exception(ErrorCodes::UNKNOWN_FUNCTION, "User-defined function '{}' doesn't exist", object_name);
    return false;
  }
