...
void validateSQLFunction(ASTPtr function, const String &name) {
  ...
  if (!lambda_function) throw Exception(ErrorCodes::BAD_ARGUMENTS, "Expected function, got: {}", function->formatForErrorMessage());

  auto &lambda_function_expression_list = lambda_function->arguments->children;

  if (lambda_function_expression_list.size() != 2) throw Exception(ErrorCodes::BAD_ARGUMENTS, "Lambda must have arguments and body");
  ...
  for (const auto &argument : tuple_function_arguments->arguments->children) {
    ...
    if (!argument_identifier) throw Exception(ErrorCodes::BAD_ARGUMENTS, "Lambda argument must be identifier");

    const auto &argument_name = argument_identifier->name();
    auto [_, inserted] = arguments.insert(argument_name);
    if (!inserted) throw Exception(ErrorCodes::BAD_ARGUMENTS, "Identifier {} already used as function parameter", argument_name);
  }

  ASTPtr function_body = lambda_function_expression_list[1];
  if (!function_body) throw Exception(ErrorCodes::BAD_ARGUMENTS, "Lambda must have valid function body");
...
UserDefinedSQLFunctionFactory::UserDefinedSQLFunctionFactory() : global_context(Context::getGlobalContextInstance()) {}
...
static void checkCanBeRegistered(const ContextPtr &context, const String &function_name, const IAST &create_function_query,
  ...
  if (UserDefinedExecutableFunctionFactory::instance().has(function_name,
                                                           context))  /// NOLINT(readability-static-accessed-through-instance)
...
bool UserDefinedSQLFunctionFactory::registerFunction(const ContextMutablePtr &context, const String &function_name,
                                                     ASTPtr create_function_query, bool throw_if_exists, bool replace_if_exists) {
  ...
  try {
    ...
    bool stored = loader.storeObject(context, UserDefinedSQLObjectType::Function, function_name, create_function_query, throw_if_exists,
                                     replace_if_exists, context->getSettingsRef());
...
bool UserDefinedSQLFunctionFactory::unregisterFunction(const ContextMutablePtr &context, const String &function_name,
...
bool UserDefinedSQLFunctionFactory::empty() const { return global_context->getUserDefinedSQLObjectsStorage().empty(); }
...
void UserDefinedSQLFunctionFactory::backup(BackupEntriesCollector &backup_entries_collector, const String &data_path_in_backup) const {
  backupUserDefinedSQLObjects(backup_entries_collector, data_path_in_backup, UserDefinedSQLObjectType::Function,
