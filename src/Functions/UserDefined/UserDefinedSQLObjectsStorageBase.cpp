...
ASTPtr UserDefinedSQLObjectsStorageBase::get(const String& object_name) const {
  ...
  if (it == object_name_to_create_object_map.end())
    throw Exception(ErrorCodes::UNKNOWN_FUNCTION, "The object name '{}' is not saved", object_name);
...
ASTPtr UserDefinedSQLObjectsStorageBase::tryGet(const std::string& object_name) const {
  ...
  auto it = object_name_to_create_object_map.find(object_name);
  if (it == object_name_to_create_object_map.end()) return nullptr;

  return it->second;
}

bool UserDefinedSQLObjectsStorageBase::has(const String& object_name) const { return tryGet(object_name) != nullptr; }
...
std::vector<std::string> UserDefinedSQLObjectsStorageBase::getAllObjectNames() const {
  ...
  for (const auto& [name, _] : object_name_to_create_object_map) object_names.emplace_back(name);
...
bool UserDefinedSQLObjectsStorageBase::storeObject(const ContextPtr& current_context, UserDefinedSQLObjectType object_type,
                                                   const String& object_name, ASTPtr create_object_query, bool throw_if_exists,
                                                   bool replace_if_exists, const Settings& settings) {
  std::lock_guard lock{mutex};
  auto it = object_name_to_create_object_map.find(object_name);
  if (it != object_name_to_create_object_map.end()) {
    if (throw_if_exists) throw Exception(ErrorCodes::FUNCTION_ALREADY_EXISTS, "User-defined object '{}' already exists", object_name);
    if (!replace_if_exists) return false;
  }

  bool stored =
      storeObjectImpl(current_context, object_type, object_name, create_object_query, throw_if_exists, replace_if_exists, settings);

  if (stored) object_name_to_create_object_map[object_name] = create_object_query;

  return stored;
}

bool UserDefinedSQLObjectsStorageBase::removeObject(const ContextPtr& current_context, UserDefinedSQLObjectType object_type,
  ...
  bool removed = removeObjectImpl(current_context, object_type, object_name, throw_if_not_exists);

  if (removed) object_name_to_create_object_map.erase(object_name);

  return removed;
}

std::unique_lock<std::recursive_mutex> UserDefinedSQLObjectsStorageBase::getLock() const { return std::unique_lock{mutex}; }
...
void UserDefinedSQLObjectsStorageBase::removeAllObjectsExcept(const Strings& object_names_to_keep) {
  ...
  for (auto it = object_name_to_create_object_map.begin(); it != object_name_to_create_object_map.end();) {
    auto current = it++;
    if (!names_set_to_keep.contains(current->first)) object_name_to_create_object_map.erase(current);
  }
}
