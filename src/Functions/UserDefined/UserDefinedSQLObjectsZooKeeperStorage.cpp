...
String makeWatchIdFromName(const String &name) { return fmt::format("UserDefinedSQLObjectsZooKeeperStorage(name={})", name); }
...
UserDefinedSQLObjectsZooKeeperStorage::UserDefinedSQLObjectsZooKeeperStorage(const ContextPtr &global_context_,
    ...
    : UserDefinedSQLObjectsStorageBase(global_context_),
      ...
      watch_queue{
          std::make_shared<ConcurrentBoundedQueue<std::pair<UserDefinedSQLObjectType, String>>>(std::numeric_limits<size_t>::max())},
      log{getLogger("UserDefinedSQLObjectsLoaderFromZooKeeper")} {
  if (zookeeper_path.empty()) throw Exception(ErrorCodes::BAD_ARGUMENTS, "ZooKeeper path must be non-empty");

  if (zookeeper_path.back() == '/') zookeeper_path.resize(zookeeper_path.size() - 1);

  /// If zookeeper chroot prefix is used, path should start with '/', because chroot concatenates without it.
  if (zookeeper_path.front() != '/') zookeeper_path = "/" + zookeeper_path;
}

UserDefinedSQLObjectsZooKeeperStorage::~UserDefinedSQLObjectsZooKeeperStorage() { SCOPE_EXIT_SAFE(stopWatchingThread()); }
...
void UserDefinedSQLObjectsZooKeeperStorage::stopWatchingThread() {
  if (watching_flag.exchange(false)) {
    watch_queue->finish();
    if (watching_thread.joinable()) watching_thread.join();
  }
}
...
void UserDefinedSQLObjectsZooKeeperStorage::initZooKeeperIfNeeded() { getZooKeeper(); }

void UserDefinedSQLObjectsZooKeeperStorage::resetAfterError() { zookeeper_getter.resetCache(); }

void UserDefinedSQLObjectsZooKeeperStorage::loadObjects() {
  /// loadObjects() is called at start from Server::main(), so it's better not to stop here on no connection to ZooKeeper or any other
  /// error. However the watching thread must be started anyway in case the connection will be established later.
  if (!objects_loaded) {
    try {
      reloadObjects();
    } catch (...) {
...
void UserDefinedSQLObjectsZooKeeperStorage::processWatchQueue() {
  ...
  while (watching_flag) {
    try {
      ...
      if (!watch_queue->tryPop(watched_object, /* timeout_ms: */ 10000)) continue;
...
void UserDefinedSQLObjectsZooKeeperStorage::stopWatching() { stopWatchingThread(); }
...
bool UserDefinedSQLObjectsZooKeeperStorage::storeObjectImpl(const ContextPtr & /*current_context*/, UserDefinedSQLObjectType object_type,
                                                            const String &object_name, ASTPtr create_object_query, bool throw_if_exists,
                                                            bool replace_if_exists, const Settings &) {
  ...
  while (true) {
    ...
    if (code == Coordination::Error::ZNODEEXISTS) {
      if (throw_if_exists) throw Exception(ErrorCodes::FUNCTION_ALREADY_EXISTS, "User-defined function '{}' already exists", object_name);
      if (!replace_if_exists) return false;
    ...
    if (!--num_attempts) throw zkutil::KeeperException::fromPath(code, path);
  }
...
bool UserDefinedSQLObjectsZooKeeperStorage::removeObjectImpl(const ContextPtr & /*current_context*/, UserDefinedSQLObjectType object_type,
                                                             const String &object_name, bool throw_if_not_exists) {
  ...
  if (code == Coordination::Error::ZNONODE) {
    if (throw_if_not_exists) throw Exception(ErrorCodes::UNKNOWN_FUNCTION, "User-defined object '{}' doesn't exist", object_name);
    return false;
  }
...
bool UserDefinedSQLObjectsZooKeeperStorage::getObjectDataAndSetWatch(const zkutil::ZooKeeperPtr &zookeeper, String &data,
                                                                     const String &path, UserDefinedSQLObjectType object_type,
...
ASTPtr UserDefinedSQLObjectsZooKeeperStorage::parseObjectData(const String &object_data, UserDefinedSQLObjectType object_type) {
  switch (object_type) {
    case UserDefinedSQLObjectType::Function: {
      ...
      ASTPtr ast = parseQuery(parser, object_data.data(), object_data.data() + object_data.size(), "", 0,
...
ASTPtr UserDefinedSQLObjectsZooKeeperStorage::tryLoadObject(const zkutil::ZooKeeperPtr &zookeeper, UserDefinedSQLObjectType object_type,
                                                            const String &object_name) {
  String path = getNodePath(zookeeper_path, object_type, object_name);
...
Strings UserDefinedSQLObjectsZooKeeperStorage::getObjectNamesAndSetWatch(const zkutil::ZooKeeperPtr &zookeeper,
  ...
  for (const auto &node_name : node_names) {
    if (node_name.starts_with(prefix) && node_name.ends_with(sql_extension)) {
      String object_name =
          unescapeForFileName(node_name.substr(prefix.length(), node_name.length() - prefix.length() - sql_extension.length()));
      if (!object_name.empty()) object_names.push_back(std::move(object_name));
    }
  }
...
void UserDefinedSQLObjectsZooKeeperStorage::refreshObject(const zkutil::ZooKeeperPtr &zookeeper, UserDefinedSQLObjectType object_type,
                                                          const String &object_name) {
