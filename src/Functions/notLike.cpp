...
-- haystack NOT LIKE pattern
  ...
  FunctionDocumentation::ReturnedValue returned_value = {"Returns `1` if the string does not match the `LIKE` pattern, otherwise `0`.",
                                                         {"UInt8"}};
  FunctionDocumentation::Examples examples = {{"Usage example", "SELECT notLike('ClickHouse', '%House%');",
                                               R"(
┌─notLike('Cli⋯ '%House%')─┐
│                        0 │
└──────────────────────────┘
        )"},
                                              {"Non-matching pattern", "SELECT notLike('ClickHouse', '%SQL%');",
                                               R"(
┌─notLike('Cli⋯', '%SQL%')─┐
│                        1 │
└──────────────────────────┘
