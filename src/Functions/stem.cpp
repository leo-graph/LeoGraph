...
struct StemImpl {
  static void vector(const ColumnString::Chars &data, const ColumnString::Offsets &offsets, ColumnString::Chars &res_data,
                     ColumnString::Offsets &res_offsets, const String &language, size_t input_rows_count) {
    std::unique_ptr<sb_stemmer, void (*)(sb_stemmer *)> stemmer(sb_stemmer_new(language.c_str(), "UTF_8"),
                                                                [](sb_stemmer *ptr) { sb_stemmer_delete(ptr); });

    if (!stemmer) throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT, "Language {} is not supported for function stem", language);
    ...
    for (UInt64 i = 0; i < input_rows_count; ++i) {
      ...
      const sb_symbol *result =
          sb_stemmer_stem(stemmer.get(), reinterpret_cast<const uint8_t *>(data.data() + offsets[i - 1]), static_cast<int>(original_size));
...
Performs stemming on a given word.
  ...
  FunctionDocumentation::Examples examples = {{"English stemming",
                                               R"(
