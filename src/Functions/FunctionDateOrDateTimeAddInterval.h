...
struct AddNanosecondsImpl {
  ...
  static NO_SANITIZE_UNDEFINED Time64 execute(Time64 t, Int64 delta, const DateLUTImpl &, const DateLUTImpl &, UInt16 scale) {
    Int64 multiplier = DecimalUtils::scaleMultiplier<Time64>(9 - scale);
    return Time64(DecimalUtils::multiplyAdd(t.value, multiplier, delta));
  }
...
struct AddMicrosecondsImpl {
  ...
  static NO_SANITIZE_UNDEFINED DateTime64 execute(DateTime64 t, Int64 delta, const DateLUTImpl &, const DateLUTImpl &, UInt16 scale) {
    ...
    return DateTime64(scale <= 6 ? DecimalUtils::multiplyAdd(t.value, multiplier, delta)
                                 : DecimalUtils::multiplyAdd(delta, multiplier, t.value));
  }
  static NO_SANITIZE_UNDEFINED Time64 execute(Time64 t, Int64 delta, const DateLUTImpl &, const DateLUTImpl &, UInt16 scale) {
    Int64 multiplier = DecimalUtils::scaleMultiplier<Time64>(std::abs(6 - scale));
    return Time64(scale <= 6 ? DecimalUtils::multiplyAdd(t.value, multiplier, delta)
                             : DecimalUtils::multiplyAdd(delta, multiplier, t.value));
  }
  static NO_SANITIZE_UNDEFINED DateTime64 execute(UInt32 t, Int64 delta, const DateLUTImpl &, const DateLUTImpl &, UInt16) {
...
struct AddMillisecondsImpl {
  ...
  static NO_SANITIZE_UNDEFINED DateTime64 execute(DateTime64 t, Int64 delta, const DateLUTImpl &, const DateLUTImpl &, UInt16 scale) {
    ...
    return DateTime64(scale <= 3 ? DecimalUtils::multiplyAdd(t.value, multiplier, delta)
                                 : DecimalUtils::multiplyAdd(delta, multiplier, t.value));
  }
  static NO_SANITIZE_UNDEFINED Time64 execute(Time64 t, Int64 delta, const DateLUTImpl &, const DateLUTImpl &, UInt16 scale) {
    Int64 multiplier = DecimalUtils::scaleMultiplier<Time64>(std::abs(3 - scale));
    return Time64(scale <= 3 ? DecimalUtils::multiplyAdd(t.value, multiplier, delta)
                             : DecimalUtils::multiplyAdd(delta, multiplier, t.value));
  }
  static NO_SANITIZE_UNDEFINED DateTime64 execute(UInt32 t, Int64 delta, const DateLUTImpl &, const DateLUTImpl &, UInt16) {
...
struct AddSecondsImpl {
  ...
  static NO_SANITIZE_UNDEFINED Int64 execute(Int64 d, Int64 delta, const DateLUTImpl &, const DateLUTImpl &, UInt16) { return d + delta; }
...
struct AddDaysImpl {
  ...
  static NO_SANITIZE_UNDEFINED DateTime64 execute(DateTime64 t, Int64 delta, const DateLUTImpl &time_zone, const DateLUTImpl &,
                                                  UInt16 scale) {
...
struct SubtractIntervalImpl : public Transform {
  ...
  template <typename T>
  NO_SANITIZE_UNDEFINED auto executeForTime(T t, Int64 delta, const DateLUTImpl &time_zone, const DateLUTImpl &utc_time_zone,
                                            UInt16 scale) const {
    /// Signed integer overflow is Ok.
...
struct SubtractNanosecondsImpl : SubtractIntervalImpl<AddNanosecondsImpl> {
  static constexpr auto name = "subtractNanoseconds";
};
struct SubtractMicrosecondsImpl : SubtractIntervalImpl<AddMicrosecondsImpl> {
  static constexpr auto name = "subtractMicroseconds";
};
struct SubtractMillisecondsImpl : SubtractIntervalImpl<AddMillisecondsImpl> {
  static constexpr auto name = "subtractMilliseconds";
};
...
struct SubtractQuartersImpl : SubtractIntervalImpl<AddQuartersImpl> {
  static constexpr auto name = "subtractQuarters";
};
...
struct Processor {
  ...
  explicit Processor(Transform transform_) : transform(std::move(transform_)) {}

  template <typename FromDataType, typename FromColumnType = typename FromDataType::ColumnType, typename ToColumnType>
  void NO_INLINE vectorConstant(const FromColumnType &col_from, ToColumnType &col_to, Int64 delta, const DateLUTImpl &time_zone,
                                UInt16 scale, size_t input_rows_count) const {
      ...
      if constexpr (std::is_same_v<FromDataType, DataTypeTime>) {
        for (size_t i = 0; i < input_rows_count; ++i)
          vec_to[i] = static_cast<typename ToColumnType::Container::value_type>(
              transform.executeForTime(vec_from[i], checkOverflow(delta), time_zone, utc_time_zone, scale));
      } else {
        for (size_t i = 0; i < input_rows_count; ++i)
          vec_to[i] = static_cast<typename ToColumnType::Container::value_type>(
              transform.execute(vec_from[i], checkOverflow(delta), time_zone, utc_time_zone, scale));
      }
    }
  }

  template <typename FromDataType, typename FromColumnType = typename FromDataType::ColumnType, typename ToColumnType>
  void vectorVector(const FromColumnType &col_from, ToColumnType &col_to, const IColumn &delta, const DateLUTImpl &time_zone, UInt16 scale,
                    size_t input_rows_count) const {
    castTypeToEither<ColumnUInt8, ColumnUInt16, ColumnUInt32, ColumnUInt64, ColumnInt8, ColumnInt16, ColumnInt32, ColumnInt64,
                     ColumnFloat32, ColumnFloat64>(&delta, [&](const auto &column) {
      vectorVector<FromDataType>(col_from, col_to, column, time_zone, scale, input_rows_count);
      return true;
    });
  }

  template <typename FromDataType, typename FromType = typename FromDataType::FieldType, typename ToColumnType>
  void constantVector(const FromType &from, ToColumnType &col_to, const IColumn &delta, const DateLUTImpl &time_zone, UInt16 scale,
                      size_t input_rows_count) const {
    castTypeToEither<ColumnUInt8, ColumnUInt16, ColumnUInt32, ColumnUInt64, ColumnInt8, ColumnInt16, ColumnInt32, ColumnInt64,
                     ColumnFloat32, ColumnFloat64>(&delta, [&](const auto &column) {
      constantVector<FromDataType>(from, col_to, column, time_zone, scale, input_rows_count);
      return true;
    });
  }
 ...
 private:
  ...
  static Int64 checkOverflow(Value val) {
    Int64 result;
    if (accurate::convertNumeric<Value, Int64, false>(val, result)) return result;
    throw DB::Exception(ErrorCodes::DECIMAL_OVERFLOW, "Numeric overflow");
  }

  template <typename FromDataType, typename FromColumnType = typename FromDataType::ColumnType, typename ToColumnType,
            typename DeltaColumnType>
  NO_INLINE NO_SANITIZE_UNDEFINED void vectorVector(const FromColumnType &col_from, ToColumnType &col_to, const DeltaColumnType &delta,
                                                    const DateLUTImpl &time_zone, UInt16 scale, size_t input_rows_count) const {
    static const DateLUTImpl &utc_time_zone = DateLUT::instance("UTC");
    ...
    if constexpr (std::is_same_v<FromColumnType, ColumnString>) {
      ...
      if constexpr (std::is_same_v<FromDataType, DataTypeTime>) {
        for (size_t i = 0; i < input_rows_count; ++i)
          vec_to[i] = static_cast<typename ToColumnType::Container::value_type>(
              transform.executeForTime(vec_from[i], checkOverflow(delta.getData()[i]), time_zone, utc_time_zone, scale));
      } else {
        for (size_t i = 0; i < input_rows_count; ++i)
          vec_to[i] = static_cast<typename ToColumnType::Container::value_type>(
              transform.execute(vec_from[i], checkOverflow(delta.getData()[i]), time_zone, utc_time_zone, scale));
      }
    }
  }

  template <typename FromDataType, typename FromType = typename FromDataType::FieldType, typename ToColumnType, typename DeltaColumnType>
  NO_INLINE NO_SANITIZE_UNDEFINED void constantVector(const FromType &from, ToColumnType &col_to, const DeltaColumnType &delta,
                                                      const DateLUTImpl &time_zone, UInt16 scale, size_t input_rows_count) const {
    static const DateLUTImpl &utc_time_zone = DateLUT::instance("UTC");
    ...
    if constexpr (std::is_same_v<FromDataType, DataTypeTime>) {
      for (size_t i = 0; i < input_rows_count; ++i)
        vec_to[i] = static_cast<typename ToColumnType::Container::value_type>(
            transform.executeForTime(from, checkOverflow(delta.getData()[i]), time_zone, utc_time_zone, scale));
    } else {
      for (size_t i = 0; i < input_rows_count; ++i)
        vec_to[i] = static_cast<typename ToColumnType::Container::value_type>(
            transform.execute(from, checkOverflow(delta.getData()[i]), time_zone, utc_time_zone, scale));
    }
  }
};
  ...
  static ColumnPtr execute(Transform transform, const ColumnsWithTypeAndName &arguments, const DataTypePtr &result_type, UInt16 scale,
    ...
    if (const auto *sources = checkAndGetColumn<FromColumnType>(&source_column)) {
      if (const auto *delta_const_column = typeid_cast<const ColumnConst *>(&delta_column))
        processor.template vectorConstant<FromDataType>(*sources, *col_to, delta_const_column->getInt(0), time_zone, scale,
                                                        input_rows_count);
      else
      ...
      processor.template constantVector<FromDataType>(sources_const->template getValue<FromValueType>(), *col_to, delta_column, time_zone,
                                                      scale, input_rows_count);
    } else {
...
struct ResultDataTypeMap {};
template <>
struct ResultDataTypeMap<UInt16> {
  using ResultDataType = DataTypeDate;
};
template <>
struct ResultDataTypeMap<UInt32> {
  using ResultDataType = DataTypeDateTime;
};
template <>
struct ResultDataTypeMap<Int32> {
  using ResultDataType = DataTypeDate32;
};
template <>
struct ResultDataTypeMap<DateTime64> {
  using ResultDataType = DataTypeDateTime64;
};
template <>
struct ResultDataTypeMap<Int64> {
  using ResultDataType = DataTypeTime;
};  // Time has generic type Int32, which interferres with Date32, so I needed to map Time with the vacant generic type
template <>
struct ResultDataTypeMap<Time64> {
  using ResultDataType = DataTypeTime64;
};
template <>
struct ResultDataTypeMap<Int8> {
...
class FunctionDateOrDateTimeAddInterval : public IFunction {
 public:
  ...
  DataTypePtr getReturnTypeImpl(const ColumnsWithTypeAndName &arguments) const override {
    ...
    if (arguments.size() == 2) {
      if (!isDateOrDate32OrTimeOrTime64OrDateTimeOrDateTime64(arguments[0].type) && !isString(arguments[0].type))
        throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT,
                        "Illegal type {} of first argument of function {}. "
                        "Must be a date, a date with time or a String",
                        arguments[0].type->getName(), getName());
    } else {
      if (!WhichDataType(arguments[0].type).isDateTimeOrDateTime64())
        throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT,
                        "Illegal type {} of first argument of function {}. "
                        "Must be a DateTime/DateTime64",
                        arguments[0].type->getName(), getName());
    ...
    switch (arguments[0].type->getTypeId()) {
      ...
      default:
        throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT,
                        "Invalid type of 1st argument of function {}: "
                        "{}, expected: Date, DateTime or DateTime64.",
  ...
  struct TransformExecuteReturnTypeImpl {
    ...
    using type = decltype(std::declval<Transform>().execute(std::declval<FieldType>(),    // e.g. Int32, UInt16, Int64...
                                                            0,                            // delta
  ...
  struct TransformExecuteReturnTypeImpl<DataTypeTime> {
    ...
    using type = decltype(std::declval<Transform>().executeForTime(std::declval<FieldType>(),    // Int32
                                                                   0,                            // delta
                                                                   std::declval<DateLUTImpl>(),  // timezone
                                                                   std::declval<DateLUTImpl>(),  // utc
                                                                   0                             // scale
  ...
  template <typename FromDataType>
  using TransformResultDataType =
      typename date_and_time_type_details::ResultDataTypeMap<TransformExecuteReturnType<FromDataType>>::ResultDataType;
  ...
  DataTypePtr resolveReturnType(const ColumnsWithTypeAndName &arguments) const {
    ...
    if constexpr (std::is_same_v<ResultDataType, DataTypeDate>)
      ...
      static constexpr auto target_scale = std::invoke([]() -> std::optional<UInt32> {
  ...
  ColumnPtr executeImpl(const ColumnsWithTypeAndName &arguments, const DataTypePtr &result_type, size_t input_rows_count) const override {
    ...
    if (which.isDate32())
      return DateTimeAddIntervalImpl<DataTypeDate32, TransformResultDataType<DataTypeDate32>, Transform>::execute(
          Transform{}, arguments, result_type, 0, input_rows_count);
    if (which.isTime())
      return DateTimeAddIntervalImpl<DataTypeTime, TransformResultDataType<DataTypeTime>, Transform>::execute(
          Transform{}, arguments, result_type, 0, input_rows_count);
    if (which.isTime64()) {
    ...
    if (which.isString())
      return DateTimeAddIntervalImpl<DataTypeString, DataTypeDateTime64, Transform>::execute(Transform{}, arguments, result_type, 3,
                                                                                             input_rows_count);
    throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT, "Illegal type {} of first argument of function {}", arguments[0].type->getName(),
                    getName());
  }
};
