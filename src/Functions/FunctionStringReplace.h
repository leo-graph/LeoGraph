 ...
 public:
  ...
  ColumnPtr executeImpl(const ColumnsWithTypeAndName &arguments, const DataTypePtr &, size_t input_rows_count) const override {
    ...
    if (col_haystack && col_needle_const && col_replacement_const) {
      Impl::vectorConstantConstant(col_haystack->getChars(), col_haystack->getOffsets(), col_needle_const->getValue<String>(),
                                   col_replacement_const->getValue<String>(), col_res->getChars(), col_res->getOffsets(), input_rows_count);
      return col_res;
    }
    if (col_haystack && col_needle_vector && col_replacement_const) {
      Impl::vectorVectorConstant(col_haystack->getChars(), col_haystack->getOffsets(), col_needle_vector->getChars(),
                                 col_needle_vector->getOffsets(), col_replacement_const->getValue<String>(), col_res->getChars(),
                                 col_res->getOffsets(), input_rows_count);
      return col_res;
    }
    if (col_haystack && col_needle_const && col_replacement_vector) {
      Impl::vectorConstantVector(col_haystack->getChars(), col_haystack->getOffsets(), col_needle_const->getValue<String>(),
                                 col_replacement_vector->getChars(), col_replacement_vector->getOffsets(), col_res->getChars(),
                                 col_res->getOffsets(), input_rows_count);
      return col_res;
    }
    if (col_haystack && col_needle_vector && col_replacement_vector) {
      Impl::vectorVectorVector(col_haystack->getChars(), col_haystack->getOffsets(), col_needle_vector->getChars(),
                               col_needle_vector->getOffsets(), col_replacement_vector->getChars(), col_replacement_vector->getOffsets(),
                               col_res->getChars(), col_res->getOffsets(), input_rows_count);
      return col_res;
    }
    if (col_haystack_fixed && col_needle_const && col_replacement_const) {
      Impl::vectorFixedConstantConstant(col_haystack_fixed->getChars(), col_haystack_fixed->getN(), col_needle_const->getValue<String>(),
                                        col_replacement_const->getValue<String>(), col_res->getChars(), col_res->getOffsets(),
                                        input_rows_count);
      return col_res;
    }
