...
class SplitByRegexpOverloadResolver : public IFunctionOverloadResolver {
 public:
  ...
  explicit SplitByRegexpOverloadResolver(ContextPtr context_)
      : context(context_), split_by_regexp(FunctionSplitByRegexp::create(context)) {}
  ...
  FunctionBasePtr buildImpl(const ColumnsWithTypeAndName &arguments, const DataTypePtr &return_type) const override {
    ...
    return std::make_unique<FunctionToFunctionBaseAdaptor>(
        split_by_regexp, DataTypes{std::from_range_t{}, arguments | std::views::transform([](auto &elem) { return elem.type; })},
        return_type);
  }
 ...
 private:
  bool patternIsTrivialChar(const ColumnsWithTypeAndName &arguments) const {
    if (!arguments[0].column.get()) return false;
    const ColumnConst *col = checkAndGetColumnConstStringOrFixedString(arguments[0].column.get());
    if (!col) return false;
...
:::
  ...
  FunctionDocumentation::Arguments arguments = {
      ...
      {"max_substrings",
       "Optional. When `max_substrings > 0`, the returned substrings will be no more than `max_substrings`, otherwise the function will "
       "return as many substrings as possible. Default value: `0`.",
       {"Int64"}}};
  FunctionDocumentation::ReturnedValue returned_value = {"Returns an array of the selected substrings of `s`.", {"Array(String)"}};
  FunctionDocumentation::Examples examples = {{"Usage example", "SELECT splitByRegexp('\\\\d+', 'a12bc23de345f');",
                                               R"(
┌─splitByRegex⋯c23de345f')─┐
│ ['a12bc23de345f']        │
└──────────────────────────┘
        )"},
                                              {"Empty regexp", "SELECT splitByRegexp('', 'abcde');",
                                               R"(
┌─splitByRegexp('', 'abcde')─┐
│ ['a','b','c','d','e']      │
└────────────────────────────┘
