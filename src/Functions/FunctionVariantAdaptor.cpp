...
ColumnPtr ExecutableFunctionVariantAdaptor::executeImpl(const ColumnsWithTypeAndName &arguments, const DataTypePtr &result_type, size_t,
                                                        bool dry_run) const {
  ...
  if (variant_column.empty()) return result_type->createColumn();
  ...
  if (auto non_empty_variant_discr_no_nulls = variant_column.getGlobalDiscriminatorOfOneNoneEmptyVariantNoNulls()) {
      ...
      if (!removeNullable(result_type)->equals(*removeNullable(nested_result_type))) {
        try {
          ...
          throw Exception(ErrorCodes::LOGICAL_ERROR,
                          "Cannot convert nested result of function {} with type {} to the expected result type {}: {}", getName(),
                          removeNullable(result_type)->getName(), removeNullable(nested_result_type)->getName(), e.message());
        }
      }
    ...
    try {
      ...
      throw Exception(ErrorCodes::LOGICAL_ERROR,
                      "Cannot convert nested result of function {} with type {} to the expected result type {}: {}", getName(),
                      nested_result_type->getName(), result_type->getName(), e.message());
    }
  }
  ...
  if (auto non_empty_variant_discr = variant_column.getGlobalDiscriminatorOfOneNoneEmptyVariant()) {
    ...
    for (const auto &discr : local_discriminators) filter.push_back(discr == local_discr);
    ...
    if (!isVariant(result_type)) {
      ...
      if (!nested_result_type->isNullable() && nested_result_type->canBeInsideNullable()) {
        for (auto &byte : filter) byte = !byte;
      ...
      if (!result_type->equals(*nested_result_type)) {
        try {
          ...
          throw Exception(ErrorCodes::LOGICAL_ERROR,
                          "Cannot convert nested result of function {} with type {} to the expected result type {}: {}", getName(),
                          result_type->getName(), nested_result_type->getName(), e.message());
        }
      }
    ...
    if (local_discr == ColumnVariant::NULL_DISCRIMINATOR) {
      ...
      if (!variants[global_discr].column) {
        variants[global_discr] =
            ColumnWithTypeAndName{variant_column.getVariantPtrByGlobalDiscriminator(global_discr), variant_types[global_discr], ""};
      }
  ...
  for (size_t i = 0; i != arguments.size(); ++i) {
    if (i == variant_argument_index) {
      ...
      for (size_t j = 0; j < num_variants; ++j) {
        if (variants[j].column) variants_arguments[j].push_back(variants[j]);
      }
    } else {
  ...
  for (size_t i = 0; i < num_variants; ++i) {
    ...
    /// In this case we return Nullable of this type (because Variant can contain NULLs).
      ...
      if (!removeNullable(result_type)->equals(*removeNullable(nested_result_type))) {
        try {
          variants_results[i] =
              castColumn(ColumnWithTypeAndName{makeNullableSafe(nested_result), makeNullableSafe(nested_result_type), ""}, result_type);
        } catch (const Exception &e) {
          throw Exception(ErrorCodes::LOGICAL_ERROR,
                          "Cannot convert nested result of function {} with type {} to the expected result type {}: {}", getName(),
                          result_type->getName(), nested_result_type->getName(), e.message());
  ...
  for (size_t i = 0; i < num_variants; ++i) {
    if (!variants[i].column || !variants_results[i]) continue;
  ...
  if (can_use_direct_construction) {
    ...
    for (size_t i = 0; i < num_variants; ++i) {
      ...
      if (!result_discriminators[i])
        throw Exception(ErrorCodes::LOGICAL_ERROR, "Cannot find variant type {} in result Variant type {} during execution of {}",
                        removeNullable(variant_result_type)->getName(), result_type->getName(), getName());
    }
  ...
  for (size_t i = 0; i < num_variants; ++i) {
    ...
    try {
      ...
      throw Exception(ErrorCodes::LOGICAL_ERROR,
                      "Cannot convert nested result of function {} with type {} to the expected result type {}: {}", getName(),
                      variant_result_type->getName(), result_type->getName(), e.message());
    }
  }
...
ColumnPtr ExecutableFunctionVariantAdaptor::executeImpl(const ColumnsWithTypeAndName &arguments, const DataTypePtr &result_type,
                                                        size_t input_rows_count) const {
  return executeImpl(arguments, result_type, input_rows_count, false);
}

ColumnPtr ExecutableFunctionVariantAdaptor::executeDryRunImpl(const ColumnsWithTypeAndName &arguments, const DataTypePtr &result_type,
                                                              size_t input_rows_count) const {
  return executeImpl(arguments, result_type, input_rows_count, true);
}

FunctionBaseVariantAdaptor::FunctionBaseVariantAdaptor(std::shared_ptr<const IFunctionOverloadResolver> function_overload_resolver_,
  ...
  for (const auto &arg : arguments_with_type_) arguments.push_back(arg.type);
  ...
  if (!variant_type) throw Exception(ErrorCodes::LOGICAL_ERROR, "Expected Variant type at position {}", variant_argument_index);
