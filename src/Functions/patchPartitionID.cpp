  ...
  ColumnPtr executeImpl(const ColumnsWithTypeAndName &arguments, const DataTypePtr &, size_t input_rows_count) const override {
    ...
    if (!column_structure_hash)
      throw Exception(ErrorCodes::ILLEGAL_COLUMN,
                      "The second argument of function {} must be a const string with hash of patch column names", name);
    ...
    for (size_t i = 0; i < input_rows_count; ++i) {
      ...
      if (!ref.empty()) {
        ...
        if (bytes_to_copy == ref.size()) throw Exception(ErrorCodes::BAD_ARGUMENTS, "Incorrect name of part: {}", ref);
...
REGISTER_FUNCTION(PatchPartitionID) { factory.registerFunction<FunctionPatchPartitionID>(FunctionDocumentation::INTERNAL_FUNCTION_DOCS); }
