...
struct FunctionUnaryArithmeticMonotonicity<NameAbs> {
  ...
  static IFunction::Monotonicity get(const IDataType &, const Field &left, const Field &right) {
    ...
    if ((left_float < 0 && right_float > 0) || (left_float > 0 && right_float < 0)) return {};
...
REGISTER_FUNCTION(Abs) {
  FunctionDocumentation::Description description =
      "Calculates the absolute value of `x`. Has no effect if `x` is of an unsigned type. If `x` is of a signed type, it returns an "
      "unsigned number.";
