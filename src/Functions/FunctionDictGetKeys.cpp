...
class FunctionDictGetKeys final : public IFunction {
 public:
  ...
  explicit FunctionDictGetKeys(ContextPtr context_) : helper(context_) {}
  ...
  DataTypePtr getReturnTypeImpl(const ColumnsWithTypeAndName &arguments) const override {
    ...
    if (!attr_name_const_col)
      throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT, "Illegal type {} of second argument of function {}, expected String.",
    ...
    if (key_types.empty()) throw Exception(ErrorCodes::LOGICAL_ERROR, "Dictionary has no keys");

    if (key_types.size() == 1) return std::make_shared<DataTypeArray>(key_types[0]);
 ...
 private:
  ...
  ColumnPtr executeConstPath(const String &dict_name, const String &attr_name, const ColumnWithTypeAndName &argument_values_column,
    ...
    while (executor.pull(block)) {
      ...
      for (size_t row_id = 0; row_id < rows_in_block; ++row_id) {
        ...
        if (value_hash != values_column_value_hash) continue;
  ...
  ///   Step 2. Check which bucket results can already be found in the shared Cache and store their result locally in `bucket_cached_bytes`.
  ///   Create an array
  ///           bucket_ids which are not available in the Cache.
  ///   Step 3. Scan the dictionary to get the result for the missing buckets, update the Cache and also update the local
  ///   `bucket_cached_bytes`. Step 4. Unpack the `bucket_cached_bytes` to IColumn format column `results_cols`. Storing IColumn format per
  ///   key in the Cache is
  ...
  ColumnPtr executeVectorPath(const String &dict_name, const String &attr_name, const ColumnWithTypeAndName &argument_values_column,
                              size_t input_rows_count) const {
    auto dict = helper.getDictionary(dict_name);
    chassert(dict != nullptr);
    ...
    if (!missing_bucket_ids.empty()) {
      ...
      for (size_t bucket_id : missing_bucket_ids) {
        if (!bucket_cached_bytes[bucket_id]) bucket_cached_bytes[bucket_id] = std::make_shared<SerializedKeys>();
        CacheKey key{domain_id, bucket_value_hashes[bucket_id]};
    ...
    for (size_t row_id = 0; row_id < input_rows_count; ++row_id) {
      ...
      if (start != std::numeric_limits<size_t>::max()) {
        if (len) {
          for (size_t key_pos = 0; key_pos < num_keys; ++key_pos) result_cols[key_pos]->insertRangeFrom(*result_cols[key_pos], start, len);
          out_offset += len;
        }
        offsets[row_id] = out_offset;
        continue;
  ...
  void fillMissingBucketsFromDict(const DictionaryPtr &dict, const String &attr_name, const DataTypes &key_types,
                                  std::vector<SerializedKeysPtr> &out, const std::vector<size_t> &missing_bucket_ids,
    ...
    while (executor.pull(block)) {
      ...
      for (size_t row_id = 0; row_id < rows_in_block; ++row_id) {
        ...
        if (it == value_hash_to_bucket_id.end()) continue;
        ...
        if (!is_missing[bucket_id]) continue;

        auto &mapped = out[bucket_id];
        if (!mapped) mapped = std::make_shared<SerializedKeys>();
...
This is most effective with large dictionaries when the input has low cardinality and the working set fits in the cache. Set to `0` to disable caching.
  ...
  FunctionDocumentation::ReturnedValue returned_value = {
      ...
      {}};
  FunctionDocumentation::Examples examples = {{"Sample usage",
                                               R"(
