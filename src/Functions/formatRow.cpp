 ...
 public:
  ...
  ColumnPtr executeImpl(const ColumnsWithTypeAndName &arguments, const DataTypePtr &, size_t input_rows_count) const override {
    ...
    if (!row_output_format)
      throw Exception(ErrorCodes::BAD_ARGUMENTS, "Cannot turn rows into a {} format strings. {} function supports only row output formats",
                      format_name, getName());
...
class FormatRowOverloadResolver : public IFunctionOverloadResolver {
 public:
  ...
  FunctionBasePtr buildImpl(const ColumnsWithTypeAndName &arguments, const DataTypePtr &return_type) const override {
    ...
    for (const auto &argument : arguments) arguments_column_names.push_back(argument.name);

    if (const auto *name_col = checkAndGetColumnConst<ColumnString>(arguments.at(0).column.get()))
      return std::make_unique<FunctionToFunctionBaseAdaptor>(
          std::make_shared<FunctionFormatRow>(function_name, no_newline, name_col->getValue<String>(), std::move(arguments_column_names),
                                              context),
          DataTypes{std::from_range_t{}, arguments | std::views::transform([](auto &elem) { return elem.type; })}, return_type);
...
:::
  ...
  FunctionDocumentation::Arguments formatRow_arguments = {{"format", "Text format. For example, CSV, TSV.", {"String"}},
                                                          {"x, y, ...", "Expressions.", {"Any"}}};
  FunctionDocumentation::ReturnedValue formatRow_returned_value = {
      "A formatted string. (for text formats it's usually terminated with the new line character).", {"String"}};
  FunctionDocumentation::Examples formatRow_examples = {{"Basic usage",
                                                         R"(
...
└──────────────────────────────────────────────┘
  ...
  FunctionDocumentation formatRow_documentation = {
      formatRow_description,    formatRow_syntax,   formatRow_arguments,     {},
      formatRow_returned_value, formatRow_examples, formatRow_introduced_in, formatRow_category};
...
Converts arbitrary expressions into a string via given format, but removes any trailing newline characters from the result.
  ...
  FunctionDocumentation::Arguments formatRowNoNewline_arguments = {{"format", "Text format. For example, CSV, TSV.", {"String"}},
                                                                   {"x, y, ...", "Expressions.", {"Any"}}};
  FunctionDocumentation::ReturnedValue formatRowNoNewline_returned_value = {"Returns a formatted string with newlines removed.",
                                                                            {"String"}};
  FunctionDocumentation::Examples formatRowNoNewline_examples = {{"Basic usage",
                                                                  R"(
...
└───────────────────────────────────────────┘
  ...
  FunctionDocumentation formatRowNoNewline_documentation = {
      formatRowNoNewline_description,    formatRowNoNewline_syntax,   formatRowNoNewline_arguments,     {},
      formatRowNoNewline_returned_value, formatRowNoNewline_examples, formatRowNoNewline_introduced_in, formatRowNoNewline_category};

  factory.registerFunction(
      "formatRow", [](ContextPtr ctx) { return FormatRowOverloadResolver::create("formatRow", false, std::move(ctx)); },
      formatRow_documentation);
  factory.registerFunction(
      "formatRowNoNewline", [](ContextPtr ctx) { return FormatRowOverloadResolver::create("formatRowNoNewline", true, std::move(ctx)); },
