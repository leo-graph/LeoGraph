...
const ColumnConst *checkAndGetColumnConstStringOrFixedString(const IColumn *column) {
  ...
  if (checkColumn<ColumnString>(&res->getDataColumn()) || checkColumn<ColumnFixedString>(&res->getDataColumn())) return res;
...
Columns convertConstTupleToConstantElements(const ColumnConst &column) {
  ...
  for (size_t i = 0; i < tuple_size; ++i) res[i] = ColumnConst::create(src_tuple_columns[i], rows);
...
ColumnsWithTypeAndName createBlockWithNestedColumns(const ColumnsWithTypeAndName &columns) {
  ...
  for (const auto &col : columns) res.emplace_back(columnGetNested(col));
...
void validateArgumentsImpl(const String &function_name, const ColumnsWithTypeAndName &arguments, size_t argument_offset,
                           const FunctionArgumentDescriptors &descriptors) {
  for (size_t i = 0; i < descriptors.size(); ++i) {
    const auto argument_index = i + argument_offset;
    if (argument_index >= arguments.size()) break;

    const auto &arg = arguments[i + argument_offset];
    const auto &descriptor = descriptors[i];
    if (int error_code = descriptor.isValid(arg.type, arg.column); error_code != 0)
      throw Exception(error_code, "A value of illegal type was provided as {} argument '{}' to function '{}'. Expected: {}, got: {}",
                      withOrdinalEnding(argument_offset + i), descriptor.name, function_name, descriptor.type_name,
                      arg.type ? arg.type->getName() : "<?>");
  }
}

void validateVariadicArgumentsImpl(const String &function_name, const ColumnsWithTypeAndName &arguments, size_t argument_offset,
  ...
  for (size_t i = argument_offset; i < arguments.size(); ++i) {
    ...
    if (int error_code = variadic_descriptor.isValid(arg.type, arg.column); error_code != 0)
      throw Exception(error_code, "A value of illegal type was provided as {} argument '{}' to function '{}'. Expected: {}, got: {}",
                      withOrdinalEnding(argument_offset + i), variadic_descriptor.name, function_name, variadic_descriptor.type_name,
                      arg.type ? arg.type->getName() : "<?>");
  }
}
...
int FunctionArgumentDescriptor::isValid(const DataTypePtr &data_type, const ColumnPtr &column) const {
  if (name.empty() || type_name.empty()) throw Exception(ErrorCodes::LOGICAL_ERROR, "name or type_name are not set");

  if (type_validator && (data_type == nullptr || !type_validator(*data_type))) return ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT;

  if (column_validator && (column == nullptr || !column_validator(*column))) return ErrorCodes::ILLEGAL_COLUMN;

  return 0;
}

void validateFunctionArguments(const IFunction &func, const ColumnsWithTypeAndName &arguments,
                               const FunctionArgumentDescriptors &mandatory_args, const FunctionArgumentDescriptors &optional_args) {
  validateFunctionArguments(func.getName(), arguments, mandatory_args, optional_args);
}

std::string_view getArgumentSingularOrPlural(const auto &args) { return args.size() == 1 ? "argument" : "arguments"; }
  ...
  if (arguments.size() < mandatory_args.size() || arguments.size() > mandatory_args.size() + optional_args.size()) {
    ...
    if (!mandatory_args.empty() && !optional_args.empty())
      expected_args_string =
          fmt::format("{} mandatory {} and {} optional {}", mandatory_args.size(), getArgumentSingularOrPlural(mandatory_args),
                      optional_args.size(), getArgumentSingularOrPlural(optional_args));
    else if (!mandatory_args.empty() && optional_args.empty())
      expected_args_string = fmt::format("{} {}", mandatory_args.size(),
                                         getArgumentSingularOrPlural(mandatory_args));  /// intentionally not "_mandatory_ arguments"
    else if (mandatory_args.empty() && !optional_args.empty())
    ...
    throw Exception(ErrorCodes::NUMBER_OF_ARGUMENTS_DOESNT_MATCH,
                    "An incorrect number of arguments was specified for function '{}'. Expected {}, got {}", function_name,
                    expected_args_string, fmt::format("{} {}", arguments.size(), getArgumentSingularOrPlural(arguments)));
  }

  validateArgumentsImpl(function_name, arguments, 0, mandatory_args);
  if (!optional_args.empty()) validateArgumentsImpl(function_name, arguments, mandatory_args.size(), optional_args);
}
...
void validateFunctionArgumentsWithVariadics(const IFunction &func, const ColumnsWithTypeAndName &arguments,
  ...
  if (arguments.size() < mandatory_args.size()) {
    ...
    if (!mandatory_args.empty())
      expected_args_string = fmt::format("{} {}", mandatory_args.size(), getArgumentSingularOrPlural(mandatory_args));
    ...
    throw Exception(ErrorCodes::NUMBER_OF_ARGUMENTS_DOESNT_MATCH,
                    "An incorrect number of arguments was specified for function '{}'. Expected at least {}, got {}", func.getName(),
                    expected_args_string, fmt::format("{} {}", arguments.size(), getArgumentSingularOrPlural(arguments)));
  }
...
void validateNumberOfFunctionArguments(const IFunction &func, const ColumnsWithTypeAndName &arguments, size_t expected_min_args,
                                       size_t expected_max_args) {
  if (arguments.size() < expected_min_args || arguments.size() > expected_max_args) {
    throw Exception(ErrorCodes::NUMBER_OF_ARGUMENTS_DOESNT_MATCH,
                    "Incorrect number of arguments for function {}. Expected {} or {}, got {}", func.getName(), expected_min_args,
                    expected_max_args, arguments.size());
  }
}

std::pair<std::vector<const IColumn *>, const ColumnArray::Offset *> checkAndGetNestedArrayOffset(const IColumn **columns,
...
ColumnPtr wrapInNullable(const ColumnPtr &src, const ColumnsWithTypeAndName &args, const DataTypePtr &result_type,
  ...
  for (const auto &elem : args) {
    ...
    if (const auto *nullable = checkAndGetColumn<ColumnNullable>(&*elem.column)) {
      ...
      if (!result_null_map_column) {
        ...
        for (size_t i = 0, size = result_null_map.size(); i < size; ++i) result_null_map[i] |= src_null_map[i];
...
ColumnPtr wrapInNullable(const ColumnPtr &src, ColumnPtr null_map) {
  ...
  if (const auto *nullable = checkAndGetColumn<ColumnNullable>(src.get())) {
    ...
    if (null_map) {
      ...
      for (size_t i = 0; i < result_null_map.size(); ++i) result_null_map[i] |= src_null_map[i];
...
bool isDecimalOrNullableDecimal(const DataTypePtr &type) {
  ...
  if (which.isDecimal()) return true;
  if (!which.isNullable()) return false;
  return isDecimal(assert_cast<const DataTypeNullable *>(type.get())->getNestedType());
}

bool isLowCardinalityType(const IDataType &type) { return typeid_cast<const DataTypeLowCardinality *>(&type) != nullptr; }
...
void checkFunctionArgumentSizes(const ColumnsWithTypeAndName &arguments, size_t input_rows_count) {
  for (size_t i = 0; i < arguments.size(); i++) {
    if (isColumnConst(*arguments[i].column)) continue;

    size_t current_size = arguments[i].column->size();

    if (current_size != input_rows_count)
      throw Exception(ErrorCodes::LOGICAL_ERROR, "Expected the argument №{} ('{}' of type {}) to have {} rows, but it has {}", i + 1,
                      arguments[i].name, arguments[i].type->getName(), input_rows_count, current_size);
  }
}
}  // namespace DB
