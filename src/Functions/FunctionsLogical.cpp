...
- `1`, otherwise
                                                           ...
                                                           {"Nullable(UInt8)"}};
    FunctionDocumentation::Examples examples = {{"Basic usage", "SELECT and(0, 1, -2);", "0"},
...
- `NULL`, if all arguments evaluate to `false` and at least one argument is `NULL`
                                                           ...
                                                           {"Nullable(UInt8)"}};
    FunctionDocumentation::Examples examples = {{"Basic usage", "SELECT or(1, 0, 0, 2, NULL);", "1"},
...
- `NULL`, if at least one of the inputs is `NULL`.
                                                           ...
                                                           {"Nullable(UInt8)"}};
    FunctionDocumentation::Examples examples = {{"Basic usage", "SELECT xor(0, 1, 1);", "0"}};
    FunctionDocumentation::IntroducedIn introduced_in = {1, 1};
    FunctionDocumentation::Category category = FunctionDocumentation::Category::Logical;
    FunctionDocumentation documentation = {description, syntax, arguments, {}, returned_value, examples, introduced_in, category};
...
Zero as an argument is considered `false`, non-zero values are considered `true`.
    ...
    FunctionDocumentation::Arguments arguments = {{"val", "The value.", {"(U)Int*", "Float*"}}};
    FunctionDocumentation::ReturnedValue returned_value = {R"(
Returns:
...
- `NULL`, if `val` is `NULL`.
                                                           ...
                                                           {"Nullable(UInt8)"}};
    FunctionDocumentation::Examples examples = {{"Basic usage", "SELECT NOT(1);", "0"}};
    FunctionDocumentation::IntroducedIn introduced_in = {1, 1};
    FunctionDocumentation::Category category = FunctionDocumentation::Category::Logical;
    FunctionDocumentation documentation = {description, syntax, arguments, {}, returned_value, examples, introduced_in, category};

    factory.registerFunction<FunctionNot>(documentation,
                                          FunctionFactory::Case::Insensitive);  /// Operator NOT(x) can be parsed as a function.
  }
}
...
MutableColumnPtr buildColumnFromTernaryData(const UInt8Container &ternary_data, bool make_nullable) {
  ...
  for (size_t i = 0; i < rows_count; ++i) new_column->getData()[i] = (ternary_data[i] == Ternary::True);

  if (!make_nullable) return new_column;

  auto null_column = ColumnUInt8::create(rows_count);
  for (size_t i = 0; i < rows_count; ++i) null_column->getData()[i] = (ternary_data[i] == Ternary::Null);
...
inline bool extractConstColumnsAsBool(ColumnRawPtrs &in, UInt8 &res) {
  return extractConstColumns<Op, false>(
      in, res, [](const Field &value) { return !value.isNull() && applyVisitor(FieldVisitorConvertToNumber<bool>(), value); });
}

template <class Op>
inline bool extractConstColumnsAsTernary(ColumnRawPtrs &in, UInt8 &res_3v) {
  return extractConstColumns<Op, true>(in, res_3v, [](const Field &value) {
    return value.isNull() ? Ternary::makeValue(false, true) : Ternary::makeValue(applyVisitor(FieldVisitorConvertToNumber<bool>(), value));
  });
}
...
class AssociativeApplierImpl {
 ...
 public:
  ...
  explicit AssociativeApplierImpl(const UInt8ColumnPtrs &in) : vec(in[in.size() - N]->getData()), next(in) {}
...
class AssociativeApplierImpl<Op, 1> {
 ...
 public:
  explicit AssociativeApplierImpl(const UInt8ColumnPtrs &in) : vec(in[in.size() - 1]->getData()) {}
...
struct TernaryValueBuilderImpl<> {
  [[noreturn]] static void build(const IColumn *x, UInt8 * /* nullable_ternary_column_data */) {
    throw Exception(ErrorCodes::LOGICAL_ERROR, "Unknown numeric column of type: {}", demangle(typeid(*x).name()));
  }
};

using TernaryValueBuilder = TernaryValueBuilderImpl<UInt8, UInt16, UInt32, UInt64, Int8, Int16, Int32, Int64, Float32, Float64>;
...
class AssociativeGenericApplierImpl {
 ...
 public:
  /// Remembers the last N columns from `in`.
  explicit AssociativeGenericApplierImpl(const ColumnRawPtrs &in) : vec(in[in.size() - N]->size()), next{in} {
    TernaryValueBuilder::build(in[in.size() - N], vec.data());
  }

  /// Returns a combination of values in the i-th row of all columns stored in the constructor.
  ResultValueType apply(const size_t i) const { return Op::ternaryApply(vec[i], next.apply(i)); }
...
class AssociativeGenericApplierImpl<Op, 1> {
 ...
 public:
  /// Remembers the last N columns from `in`.
  explicit AssociativeGenericApplierImpl(const ColumnRawPtrs &in) : vec(UInt8Container(in[in.size() - 1]->size())) {
...
template <typename Op, template <typename, size_t> typename OperationApplierImpl, size_t N = 8>
struct OperationApplier {
  ...
  static void apply(Columns &in, ResultData &result_data, bool use_result_data_as_input = false) {
    ...
    if (isArchSupported(TargetArch::x86_64_v4)) {
      if (!use_result_data_as_input) doBatchedApplyAVX512BW<false>(in, result_data.data(), result_data.size());
      while (!in.empty()) doBatchedApplyAVX512BW<true>(in, result_data.data(), result_data.size());
      return;
    }

    if (isArchSupported(TargetArch::x86_64_v3)) {
      if (!use_result_data_as_input) doBatchedApplyAVX2<false>(in, result_data.data(), result_data.size());
      while (!in.empty()) doBatchedApplyAVX2<true>(in, result_data.data(), result_data.size());
      return;
    }
#endif
    {
      if (!use_result_data_as_input) doBatchedApply<false>(in, result_data.data(), result_data.size());
      while (!in.empty()) doBatchedApply<true>(in, result_data.data(), result_data.size());
    }
  }
...
#define BATCH_BODY(FUNCTION_NAME)                                                                                                 \
  {                                                                                                                               \
    if (N > in.size()) {                                                                                                          \
      OperationApplier<Op, OperationApplierImpl, N - 1>::template FUNCTION_NAME<CarryResult>(in, result_data, size); /* NOLINT */ \
      return;                                                                                                                     \
    }                                                                                                                             \
                                                                                                                                  \
...
ColumnPtr executeForTernaryLogicImpl(ColumnRawPtrs arguments, const DataTypePtr &result_type, size_t input_rows_count) {
  ...
  const auto result_column = has_consts ? ColumnUInt8::create(input_rows_count, const_3v_value) : ColumnUInt8::create(input_rows_count);
...
template <typename Op>
using FastApplierImpl = TypedExecutorInvoker<Op, UInt8, UInt16, UInt32, UInt64, Int8, Int16, Int32, Int64, Float32, Float64>;

template <typename Op, typename Type, typename... Types>
struct TypedExecutorInvoker<Op, Type, Types...> {
  MULTITARGET_FUNCTION_X86_V4_V3(MULTITARGET_FUNCTION_HEADER(template <typename T, typename Result> static void), applyImpl,
                                 MULTITARGET_FUNCTION_BODY((const ColumnVector<T> &x, const ColumnVector<Type> &column, Result &result) {
                                   std::transform(
                                       x.getData().cbegin(), x.getData().cend(), column.getData().cbegin(), result.begin(),
...
ColumnPtr basicExecuteImpl(ColumnRawPtrs arguments, size_t input_rows_count) {
  ...
  if (has_consts && Op::apply(const_val, 0) == 0 && Op::apply(const_val, 1) == 1) has_consts = false;

  auto col_res = has_consts ? ColumnUInt8::create(input_rows_count, const_val) : ColumnUInt8::create(input_rows_count);
...
DataTypePtr FunctionAnyArityLogical<Impl, Name>::getReturnTypeImpl(const DataTypes &arguments) const {
  if (arguments.size() < 2)
    throw Exception(ErrorCodes::TOO_FEW_ARGUMENTS_FOR_FUNCTION, "Number of arguments for function \"{}\" should be at least 2: passed {}",
  ...
  for (size_t i = 0; i < arguments.size(); ++i) {
    ...
    if (isBool(arg_type)) has_bool_arguments = true;
    ...
    if (!has_nullable_arguments) {
      ...
      if (has_nullable_arguments && !Impl::specialImplementationForNulls())
        throw Exception(ErrorCodes::LOGICAL_ERROR,
                        "Unexpected type of argument for function \"{}\": "
                        " argument {} is of type {}",
                        getName(), i + 1, arg_type->getName());
    }

    if (!(isNativeNumber(arg_type) ||
          (Impl::specialImplementationForNulls() && (arg_type->onlyNull() || isNativeNumber(removeNullable(arg_type))))))
      throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT, "Illegal type ({}) of {} argument of function {}", arg_type->getName(), i + 1,
                      getName());
  }
...
static void applyTernaryLogicImpl(const IColumn::Filter &mask, IColumn::Filter &null_bytemap) {
  for (size_t i = 0; i != mask.size(); ++i) {
    UInt8 value = mask[i];
    if constexpr (inverted) value = !value;

    if (null_bytemap[i] && value) null_bytemap[i] = 0;
  }
}
...
template <typename Impl, typename Name>
ColumnPtr FunctionAnyArityLogical<Impl, Name>::executeShortCircuit(ColumnsWithTypeAndName &arguments,
  ...
  if (result_type->isNullable()) nulls = std::make_unique<IColumn::Filter>(arguments[0].column->size(), 0);
  ...
  for (size_t i = 1; i <= arguments.size(); ++i) {
    ...
    if (!mask_info.has_ones || i == arguments.size()) break;

    maskedExecute(arguments[i], mask, mask_info);
  }
  /// For OR function we need to inverse mask to get the resulting column.
  if (inverted) inverseMask(mask, mask_info);

  if (nulls) applyTernaryLogic<Name>(mask, *nulls);
...
template <typename Impl, typename Name>
ColumnPtr FunctionAnyArityLogical<Impl, Name>::executeImpl(const ColumnsWithTypeAndName &args, const DataTypePtr &result_type,
  ...
  if constexpr (std::is_same_v<Name, NameAnd> || std::is_same_v<Name, NameOr>) {
    ...
    if (not_short_circuit_args.size() > 1) {
      ...
      if (short_circuit_args_index.empty()) return partial_result;
      new_args.emplace_back(partial_result, result_type, "__partial_result");
      for (const auto &index : short_circuit_args_index) new_args.emplace_back(std::move(arguments[index]));
    } else
  ...
  for (const auto &arg_index : arguments) args_in.push_back(arg_index.column.get());

  if (result_type->isNullable()) return executeForTernaryLogicImpl<Impl>(std::move(args_in), result_type, input_rows_count);
  return basicExecuteImpl<Impl>(std::move(args_in), input_rows_count);
}

template <typename Impl, typename Name>
ColumnPtr FunctionAnyArityLogical<Impl, Name>::getConstantResultForNonConstArguments(const ColumnsWithTypeAndName &arguments,
                                                                                     const DataTypePtr &result_type) const {
   ...
   */
  if constexpr (!Impl::isSaturable()) return nullptr;
  ...
  if constexpr (std::is_same_v<Impl, AndImpl>) {
    if (has_false_constant) result_column = result_type->createColumnConst(1, static_cast<UInt8>(false));
  } else if constexpr (std::is_same_v<Impl, OrImpl>) {
    if (has_true_constant) result_column = result_type->createColumnConst(1, static_cast<UInt8>(true));
  }
...
template <template <typename> class Impl, typename Name>
ColumnPtr FunctionUnaryLogical<Impl, Name>::executeImpl(const ColumnsWithTypeAndName &arguments, const DataTypePtr &,
                                                        size_t /*input_rows_count*/) const {
  ColumnPtr res;
  if (!((res = functionUnaryExecuteType<Impl, UInt8>(arguments)) || (res = functionUnaryExecuteType<Impl, UInt16>(arguments)) ||
        (res = functionUnaryExecuteType<Impl, UInt32>(arguments)) || (res = functionUnaryExecuteType<Impl, UInt64>(arguments)) ||
        (res = functionUnaryExecuteType<Impl, Int8>(arguments)) || (res = functionUnaryExecuteType<Impl, Int16>(arguments)) ||
        (res = functionUnaryExecuteType<Impl, Int32>(arguments)) || (res = functionUnaryExecuteType<Impl, Int64>(arguments)) ||
        (res = functionUnaryExecuteType<Impl, Float32>(arguments)) || (res = functionUnaryExecuteType<Impl, Float64>(arguments))))
