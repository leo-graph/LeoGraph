...
enum class Vectorize : uint8_t { No, Yes };
...
inline BFloat16 roundWithMode(BFloat16 x, RoundingMode mode) {
  switch (mode) {
    case RoundingMode::Round:
      return BFloat16(nearbyintf(Float32(x)));
    case RoundingMode::Floor:
      return BFloat16(floorf(Float32(x)));
    case RoundingMode::Ceil:
      return BFloat16(ceilf(Float32(x)));
    case RoundingMode::Trunc:
      return BFloat16(truncf(Float32(x)));
  }
...
class FloatRoundingComputationBase<T, Vectorize::No> {
 public:
  ...
  static VectorType prepare(size_t scale) { return load1(ScalarType(scale)); }
};
...
struct IntegerRoundingImpl {
 ...
 public:
  ...
  static void applyOne(T in, size_t scale, T &out) { Op::compute(&in, scale, &out); }
};
...
struct Dispatcher {
  ...
  using FunctionRoundingImpl = std::conditional_t<is_floating_point<T>, FloatRoundingImpl<T, rounding_mode, scale_mode>,
  ...
  static ColumnPtr apply(const IColumn *value_col, const IColumn *scale_col = nullptr) {
    ...
    if (value_col_typed) {
      ...
      if (!vec_res.empty()) {
        ...
        if (scale_col == nullptr || isColumnConst(*scale_col)) {
          auto scale_arg = scale_col == nullptr ? 0 : getScaleArg(checkAndGetColumnConst<ColumnVector<ScaleType>>(scale_col));
          DecimalRoundingImpl<T, rounding_mode, tie_breaking_mode>::apply(vec_src, value_col_typed->getScale(), vec_res,
                                                                          static_cast<Scale>(scale_arg));
        }
        /// Non-const scale argument:
        else if (const auto *scale_col_typed = checkAndGetColumn<ColumnVector<ScaleType>>(scale_col)) {
          ...
          for (size_t i = 0; i < rows; ++i) {
            ...
            DecimalRoundingImpl<T, rounding_mode, tie_breaking_mode>::applyOne(
                value_col_typed->getElement(i), value_col_typed->getScale(),
 ...
 public:
  ...
  DataTypePtr getReturnTypeImpl(const ColumnsWithTypeAndName &arguments) const override {
    ...
    FunctionArgumentDescriptors optional_args{
        {"N", static_cast<FunctionArgumentDescriptor::TypeValidator>(&isNativeInteger), nullptr,
  ...
  ColumnPtr executeImpl(const ColumnsWithTypeAndName &arguments, const DataTypePtr &, size_t /*input_rows_count*/) const override {
    ...
    auto call_data = [&](const auto &types) -> bool {
      ...
      if (arguments.size() > 1) {
        ...
        auto call_scale = [&](const auto &scaleTypes) -> bool {
          ...
          res = Dispatcher<DataType, rounding_mode, tie_breaking_mode>::template apply<ScaleType>(value_arg.column.get(),
                                                                                                  scale_column.column.get());
          return true;
        };
    ...
    if constexpr (rounding_mode == RoundingMode::Round)
      if (0 != fesetround(FE_TONEAREST)) throw Exception(ErrorCodes::CANNOT_SET_ROUNDING_MODE, "Cannot set floating point rounding mode");
#endif
...
class FunctionRoundDown : public IFunction {
 public:
  ...
  DataTypePtr getReturnTypeImpl(const DataTypes &arguments) const override {
    ...
    if (!isNumber(type_x))
      throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT, "Unsupported type {} of first argument of function {}, must be numeric type.",
                      type_x->getName(), getName());

    const DataTypeArray *type_arr = checkAndGetDataType<DataTypeArray>(arguments[1].get());

    if (!type_arr)
      throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT, "Second argument of function {}, must be array of boundaries to round to.",
                      getName());

    const auto type_arr_nested = type_arr->getNestedType();

    if (!isNumber(type_arr_nested)) {
      throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT, "Elements of array of second argument of function {} must be numeric type.",
                      getName());
    }
  ...
  ColumnPtr executeImpl(const ColumnsWithTypeAndName &arguments, const DataTypePtr &result_type, size_t) const override {
    ...
    if (!in_type->equals(*return_type)) in_column = castColumn(arguments[0], return_type);

    if (!array_type->equals(*return_type)) array_column = castColumn(arguments[1], std::make_shared<DataTypeArray>(return_type));
    ...
    if (!num_boundaries) throw Exception(ErrorCodes::BAD_ARGUMENTS, "Empty array is illegal for boundaries in {} function", getName());

    if (!executeNum<UInt8>(in, out, boundaries) && !executeNum<UInt16>(in, out, boundaries) && !executeNum<UInt32>(in, out, boundaries) &&
        !executeNum<UInt64>(in, out, boundaries) && !executeNum<Int8>(in, out, boundaries) && !executeNum<Int16>(in, out, boundaries) &&
        !executeNum<Int32>(in, out, boundaries) && !executeNum<Int64>(in, out, boundaries) && !executeNum<Float32>(in, out, boundaries) &&
        !executeNum<Float64>(in, out, boundaries) && !executeDecimal<Decimal32>(in, out, boundaries) &&
        !executeDecimal<Decimal64>(in, out, boundaries) && !executeDecimal<Decimal128>(in, out, boundaries) &&
 ...
 private:
  ...
  void NO_INLINE executeImplNumToNum(const Container &src, Container &dst, const Array &boundaries) const {
    ...
    for (size_t i = 0; i < boundaries.size(); ++i) boundary_values[i] = static_cast<ValueType>(boundaries[i].safeGet<ValueType>());
