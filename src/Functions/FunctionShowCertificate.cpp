 ...
 public:
  ...
  ColumnPtr executeImpl(const ColumnsWithTypeAndName &, const DataTypePtr &, size_t input_rows_count) const override {
    ...
    if (input_rows_count) {
      ...
      if (!certificate.empty()) x509_cert = std::make_unique<X509Certificate>(certificate);
    ...
    auto nested_column = ColumnArray::create(ColumnTuple::create(Columns{std::move(keys), std::move(values)}), std::move(offsets));
...
See [Configuring TLS](/guides/sre/tls/configuring-tls) for more information on how to configure ClickHouse to use OpenSSL certificates to validate connections.
  ...
  FunctionDocumentation::ReturnedValue returned_value = {"Returns map of key-value pairs relating to the configured SSL certificate.",
                                                         {"Map(String, String)"}};
