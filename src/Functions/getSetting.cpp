...
class FunctionGetSetting : public IFunction, WithContext {
 public:
  static FunctionPtr create(ContextPtr context_, ErrorHandlingMode mode_) { return std::make_shared<FunctionGetSetting>(context_, mode_); }

  FunctionGetSetting(ContextPtr context_, ErrorHandlingMode mode_)
      : WithContext(context_), mode(mode_), function_name(mode == ErrorHandlingMode::Exception ? "getSetting" : "getSettingOrDefault") {}
  ...
  Field getValue(const ColumnsWithTypeAndName& arguments) const {
    if (!isString(arguments[0].type))
      throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT,
                      "The argument of function {} should be a constant string with the name of a setting", function_name);
    ...
    if (mode == ErrorHandlingMode::Exception)
      ...
      if (!getContext()->getSettingsRef().tryGet(setting_name, setting_value)) setting_value = (*default_value_column)[0];
    }
...
Returns the current value of a setting.
  ...
  FunctionDocumentation::Arguments arguments_getSetting = {{"setting_Name", "The setting name.", {"const String"}}};
  FunctionDocumentation::ReturnedValue returned_value_getSetting = {"Returns the setting's current value.", {"Any"}};
  FunctionDocumentation::Examples examples_getSetting = {{"Usage example",
                                                          R"(
...
└──────────────────────────┘
  ...
  FunctionDocumentation documentation_getSetting = {
      description_getSetting,    syntax_getSetting,   arguments_getSetting,     {},
      returned_value_getSetting, examples_getSetting, introduced_in_getSetting, category_getSetting};

  factory.registerFunction(
      "getSetting", [](ContextPtr context) { return FunctionGetSetting::create(context, ErrorHandlingMode::Exception); },
      documentation_getSetting);
...
Returns the current value of a setting or returns the default value specified in the second argument if the setting is not set in the current profile.
  ...
  FunctionDocumentation::ReturnedValue returned_value_getSettingOrDefault = {
      "Returns the current value of the specified setting or `default_value` if the setting is not set."};
  FunctionDocumentation::Examples examples_getSettingOrDefault = {{"Usage example",
                                                                   R"(
...
NULL
  ...
  FunctionDocumentation documentation_getSettingOrDefault = {
      description_getSettingOrDefault,    syntax_getSettingOrDefault,   arguments_getSettingOrDefault,     {},
      returned_value_getSettingOrDefault, examples_getSettingOrDefault, introduced_in_getSettingOrDefault, category_getSettingOrDefault};

  factory.registerFunction(
      "getSettingOrDefault", [](ContextPtr context) { return FunctionGetSetting::create(context, ErrorHandlingMode::Default); },
      documentation_getSettingOrDefault);
}
