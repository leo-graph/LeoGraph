...
struct KostikConsistentHashImpl {
  ...
  static ResultType apply(UInt64 hash, BucketsType n) { return static_cast<ResultType>(ConsistentHashing(hash, n)); }
};
...
Only efficient with `n <= 32768`.
  ...
  FunctionDocumentation::Arguments arguments = {{"input", "An integer key.", {"UInt64"}}, {"n", "The number of buckets.", {"UInt16"}}};
  FunctionDocumentation::ReturnedValue returned_value = {"Returns the computed hash value.", {"UInt16"}};
  FunctionDocumentation::Examples examples = {{"Usage example", "SELECT kostikConsistentHash(16045690984833335023, 2);",
                                               R"(
