...
class FunctionBitmaskToList : public IFunction {
 public:
  ...
  DataTypePtr getReturnTypeImpl(const DataTypes &arguments) const override {
    ...
    if (!isInteger(type)) throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT, "Cannot format {} as bitmask string", type->getName());
  ...
  ColumnPtr executeImpl(const ColumnsWithTypeAndName &arguments, const DataTypePtr &, size_t input_rows_count) const override {
    ColumnPtr res;
    if (!((res = executeType<UInt8>(arguments, input_rows_count)) || (res = executeType<UInt16>(arguments, input_rows_count)) ||
          (res = executeType<UInt32>(arguments, input_rows_count)) || (res = executeType<UInt64>(arguments, input_rows_count)) ||
          (res = executeType<Int8>(arguments, input_rows_count)) || (res = executeType<Int16>(arguments, input_rows_count)) ||
          (res = executeType<Int32>(arguments, input_rows_count)) || (res = executeType<Int64>(arguments, input_rows_count))))
 ...
 public:
  ...
  ColumnPtr executeImpl(const ColumnsWithTypeAndName &arguments, const DataTypePtr &, size_t /*input_rows_count*/) const override {
    ...
    if (tryExecute<UInt8>(in_column, out_column) || tryExecute<UInt16>(in_column, out_column) ||
        tryExecute<UInt32>(in_column, out_column) || tryExecute<UInt64>(in_column, out_column) || tryExecute<Int8>(in_column, out_column) ||
        tryExecute<Int16>(in_column, out_column) || tryExecute<Int32>(in_column, out_column) || tryExecute<Int64>(in_column, out_column))
      return out_column;
  ...
  ColumnPtr executeType(const IColumn *column, size_t input_rows_count) const {
    ...
    for (size_t row = 0; row < input_rows_count; ++row) {
      ...
      if constexpr (is_big_int_v<UnsignedType>) {
        ...
        while (x) {
          if (x & 1) result_array_values_data.push_back(position);
...
Signed input integers are first casted to an unsigned integer.
  ...
  FunctionDocumentation::Arguments bitPositionsToArray_arguments = {{"arg", "An integer value.", {"(U)Int*"}}};
  FunctionDocumentation::ReturnedValue bitPositionsToArray_returned_value = {
...
└───────────────────────────┘
  ...
  FunctionDocumentation bitPositionsToArray_documentation = {
      bitPositionsToArray_description,    bitPositionsToArray_syntax,   bitPositionsToArray_arguments,     {},
      bitPositionsToArray_returned_value, bitPositionsToArray_examples, bitPositionsToArray_introduced_in, bitPositionsToArray_category};
...
The powers of two are returned as an ascendingly ordered array.
  ...
  FunctionDocumentation::ReturnedValue bitmaskToArray_returned_value = {
  ...
  FunctionDocumentation::Examples bitmaskToArray_examples = {
      {"Basic example", "SELECT bitmaskToArray(50) AS powers_of_two",
       R"(
┌─powers_of_two───┐
│ [2, 16, 32]     │
└─────────────────┘
            )"},
      {"Single power of two", "SELECT bitmaskToArray(8) AS powers_of_two",
       R"(
┌─powers_of_two─┐
│ [8]           │
└───────────────┘
  ...
  FunctionDocumentation bitmaskToArray_documentation = {
      bitmaskToArray_description,    bitmaskToArray_syntax,   bitmaskToArray_arguments,     {},
      bitmaskToArray_returned_value, bitmaskToArray_examples, bitmaskToArray_introduced_in, bitmaskToArray_category};
...
Like bitmaskToArray but returns the powers of two as a comma-separated string.
  ...
  FunctionDocumentation::Arguments bitmaskToList_arguments = {{"num", "An integer value.", {"(U)Int*"}}};
  FunctionDocumentation::ReturnedValue bitmaskToList_returned_value = {"Returns a string containing comma-separated powers of two.",
                                                                       {"String"}};
...
└───────────────┘
  ...
  FunctionDocumentation bitmaskToList_documentation = {
      bitmaskToList_description,    bitmaskToList_syntax,   bitmaskToList_arguments,     {},
      bitmaskToList_returned_value, bitmaskToList_examples, bitmaskToList_introduced_in, bitmaskToList_category};
