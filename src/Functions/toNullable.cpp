...
class FunctionToNullable : public IFunction {
 public:
  ...
  static FunctionPtr create(ContextPtr) { return std::make_shared<FunctionToNullable>(); }
  ...
  DataTypePtr getReturnTypeImpl(const DataTypes &arguments) const override { return makeNullableOrLowCardinalityNullable(arguments[0]); }
...
Converts the provided argument type to `Nullable`.
  ...
  FunctionDocumentation::Arguments arguments = {{"x", "A value of any non-compound type.", {"Any"}}};
