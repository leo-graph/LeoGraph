...
enum class SupportInteger : uint8_t { Yes, No };

enum class FormatSyntax : uint8_t { MySQL, Joda };
...
struct InstructionValueTypeMap<DataTypeUInt64> {
  using InstructionValueType = UInt64;
};
template <>
...
String padValue(UInt32 val, size_t min_digits) {
  ...
  if (length >= min_digits) return str;
...
constexpr std::string_view monthsFull[] = {"January", "February", "March",     "April",   "May",      "June",
 ...
 *
 * select count() from (select formatDateTime(t, '%m/%d/%Y %H:%i:%S') from (select toDateTime('2018-01-01 00:00:00')+number as t from
 * numbers(100000000)));
 * - 53 million rows per second per core;
 *
 * select count() from (select formatDateTime(t, 'Hello %Y World') from (select toDateTime('2018-01-01 00:00:00')+number as t from
 * numbers(100000000)));
...
class FunctionFormatDateTimeImpl : public IFunction {
 private:
  ...
  static bool castType(const IDataType *type, F &&f) {
    return castTypeToEither<DataTypeInt8, DataTypeUInt8, DataTypeInt16, DataTypeUInt16, DataTypeInt32, DataTypeUInt32, DataTypeInt64,
  ...
  class Instruction {
   public:
    ...
    void perform(char *&dest, Time source, UInt64 fractional_second, UInt32 scale, const DateLUTImpl &timezone) {
      size_t shift = func_mysql ? std::invoke(func_mysql, this, dest, source, fractional_second, scale, timezone)
   ...
   public:
    ...
    size_t mysqlFractionalSecond(char *dest, Time /*source*/, UInt64 fractional_second, UInt32 scale, const DateLUTImpl & /*timezone*/) {
      ...
      for (size_t i = writes; i < 6; ++i) dest[i] = '0';

      return 6;
    }

    /// Prints the number of digits that were specified by the scale of the DateTime64. Legacy.
    size_t mysqlFractionalSecondScaleNumDigits(char *dest, Time /*source*/, UInt64 fractional_second, UInt32 scale,
    ...
    size_t mysqlFractionalSecondSingleZero(char *dest, Time /*source*/, UInt64 fractional_second, UInt32 scale,
                                           const DateLUTImpl & /*timezone*/) {
      if (scale == 0) scale = 1;
    ...
    static size_t jodaYearOfEra(size_t min_represent_digits, char *dest, Time source, UInt64, UInt32, const DateLUTImpl &timezone) {
      auto year = static_cast<Int32>(ToYearImpl::execute(source, timezone));
      if (min_represent_digits == 2) return writeNumberWithPadding(dest, std::abs(year) % 100, 2);
    ...
    static size_t jodaDayOfWeekText(size_t min_represent_digits, char *dest, Time source, UInt64 fractional_second, UInt32 scale,
                                    const DateLUTImpl &timezone) {
    ...
    static size_t jodaMonthOfYearText(size_t min_represent_digits, char *dest, Time source, UInt64 fractional_second, UInt32 scale,
                                      const DateLUTImpl &timezone) {
      bool abbreviate = min_represent_digits <= 3;
    ...
    static size_t jodaHalfDayOfDay(size_t /*min_represent_digits*/, char *dest, Time source, UInt64 fractional_second, UInt32 scale,
                                   const DateLUTImpl &timezone) {
      return AMPM(dest, source, fractional_second, scale, timezone);
    }
    ...
    static size_t jodaClockHourOfHalfDay(size_t min_represent_digits, char *dest, Time source, UInt64, UInt32,
                                         const DateLUTImpl &timezone) {
      auto hour = ToHourImpl::execute(source, timezone);
      hour = (hour + 11) % 12 + 1;
      return writeNumberWithPadding(dest, hour, min_represent_digits);
    ...
    static size_t jodaFractionOfSecond(size_t min_represent_digits, char *dest, Time /*source*/, UInt64 fractional_second, UInt32 scale,
                                       const DateLUTImpl & /*timezone*/) {
      min_represent_digits = std::min<size_t>(min_represent_digits, 9);
      if (fractional_second == 0) {
        for (UInt64 i = 0; i < min_represent_digits; ++i) dest[i] = '0';
      ...
      if (str.size() < scale) str.insert(0, scale - str.size(), '0');
      if (min_represent_digits > scale) {
        for (UInt64 i = 0; i < min_represent_digits - scale; ++i) str += '0';
    ...
    static size_t jodaTimezone(size_t min_represent_digits, char *dest, Time /*source*/, UInt64, UInt32, const DateLUTImpl &timezone) {
      if (min_represent_digits <= 3) throw Exception(ErrorCodes::NOT_IMPLEMENTED, "Short name time zone is not yet supported");
  ...
  static bool containsOnlyFixedWidthMySQLFormatters(std::string_view format, bool mysql_M_is_month_name,
    ...
    for (size_t i = 0; i < format.size(); ++i) {
      switch (format[i]) {
        case '%':
          if (i + 1 >= format.size()) throwLastCharacterIsPercentException();
          if (mysql_M_is_month_name) {
            if (std::any_of(variable_width_formatter_M_is_month_name.begin(), variable_width_formatter_M_is_month_name.end(),
                            [&](char c) { return c == format[i + 1]; }))
              return false;
          }
          if (mysql_format_ckl_without_leading_zeros) {
            if (std::any_of(variable_width_formatter_leading_zeros.begin(), variable_width_formatter_leading_zeros.end(),
                            [&](char c) { return c == format[i + 1]; }))
              return false;
          }
          if (!mysql_e_with_space_padding) {
            if (std::any_of(variable_width_formatter_e_with_space_padding.begin(), variable_width_formatter_e_with_space_padding.end(),
                            [&](char c) { return c == format[i + 1]; }))
              return false;
          } else {
 ...
 public:
  ...
  DataTypePtr getReturnTypeImpl(const ColumnsWithTypeAndName &arguments) const override {
    if constexpr (support_integer == SupportInteger::Yes) {
      if (arguments.size() != 1 && arguments.size() != 2 && arguments.size() != 3)
        throw Exception(ErrorCodes::NUMBER_OF_ARGUMENTS_DOESNT_MATCH,
                        "Number of arguments for function {} doesn't match: passed {}, should be 1, 2 or 3", getName(), arguments.size());
      ...
      if (arguments.size() > 1 && !(isInteger(arguments[0].type) || isDate(arguments[0].type) || isDateTime(arguments[0].type) ||
                                    isDate32(arguments[0].type) || isDateTime64(arguments[0].type)))
        throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT,
      ...
      if (arguments.size() != 2 && arguments.size() != 3)
        throw Exception(ErrorCodes::NUMBER_OF_ARGUMENTS_DOESNT_MATCH,
                        "Number of arguments for function {} doesn't match: passed {}, should be 2 or 3", getName(), arguments.size());
    ...
    if (arguments.size() == 2 && !WhichDataType(arguments[1].type).isString())
      throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT, "Illegal type {} of second argument of function {}. Must be String.",
                      arguments[1].type->getName(), getName());

    if (arguments.size() == 3 && !WhichDataType(arguments[2].type).isString())
      throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT, "Illegal type {} of third argument of function {}. Must be String.",
                      arguments[2].type->getName(), getName());

    if (arguments.size() == 1) return std::make_shared<DataTypeDateTime>();
    ...
    if constexpr (support_integer == SupportInteger::Yes) {
      ...
      if (!castType(arguments[0].type.get(), [&](const auto &type) {
        ...
        if (!((res = executeType<DataTypeDate>(arguments, result_type, input_rows_count)) ||
          ...
          throw Exception(ErrorCodes::ILLEGAL_COLUMN, "Illegal column {} of function {}, must be Integer or DateTime.",
                          arguments[0].column->getName(), getName());
      }
    } else {
      if (!((res = executeType<DataTypeDate>(arguments, result_type, input_rows_count)) ||
            ...
            (res = executeType<DataTypeDateTime>(arguments, result_type, input_rows_count)) ||
            (res = executeType<DataTypeDateTime64>(arguments, result_type, input_rows_count))))
        throw Exception(ErrorCodes::ILLEGAL_COLUMN, "Illegal column {} of function {}, must be Date or DateTime.",
                        arguments[0].column->getName(), getName());
    }
  ...
  ColumnPtr executeType(const ColumnsWithTypeAndName &arguments, const DataTypePtr &, size_t input_rows_count) const {
    ...
    if constexpr (std::is_same_v<DataType, DataTypeDateTime64>) scale = times->getScale();
    ...
    bool mysql_with_only_fixed_length_formatters =
        (format_syntax == FormatSyntax::MySQL)
            ? containsOnlyFixedWidthMySQLFormatters(format, mysql_M_is_month_name, mysql_format_ckl_without_leading_zeros,
    ...
    if (castType(arguments[0].type.get(), [&]([[maybe_unused]] const auto &type) { return true; })) {
      if (const_time_zone_column) time_zone = &extractTimeZoneFromFunctionArguments(arguments, 2, 0);
    ...
    for (size_t i = 0; i < input_rows_count; ++i) {
      ...
      if constexpr (std::is_same_v<DataType, DataTypeDateTime64>) {
        ...
        for (auto &instruction : instructions) instruction.perform(pos, static_cast<Int64>(c.whole), c.fractional, scale, *time_zone);
      } else {
        for (auto &instruction : instructions) instruction.perform(pos, static_cast<T>(vec[i]), 0, 0, *time_zone);
      }
  ...
  template <typename T>
  size_t parseFormat(const String &format, std::vector<Instruction<T>> &instructions, UInt32 scale,
                     bool mysql_with_only_fixed_length_formatters, String &out_template) const {
  ...
  template <typename T>
  size_t parseMySQLFormat(const String &format, std::vector<Instruction<T>> &instructions, UInt32 scale,
                          bool mysql_with_only_fixed_length_formatters, String &out_template) const {
    ...
    while (true) {
      ...
      if (percent_pos < end) {
        ...
        pos = percent_pos + 1;
        if (pos >= end) throwLastCharacterIsPercentException();
      ...
      if (*cur_token == '\'') {
        ...
        switch (*cur_token) {
          ...
          case 'K':
            ...
            add_instruction(std::bind_front(&Instruction<T>::jodaHourOfHalfDay, repetitions), padValue(0, repetitions));
            /// Hour of half day range [0, 11]
            reserve_size += std::max(repetitions, 2);
            break;
          case 'h':
            /// Default clock hour of half day is 12
            add_instruction(std::bind_front(&Instruction<T>::jodaClockHourOfHalfDay, repetitions), padValue(12, repetitions));
          ...
          case 'z': {
            if (repetitions <= 3) throw Exception(ErrorCodes::NOT_IMPLEMENTED, "Short name time zone is not yet supported");
...
using FunctionFormatDateTimeInJodaSyntax =
    FunctionFormatDateTimeImpl<NameFormatDateTimeInJodaSyntax, SupportInteger::No, FormatSyntax::Joda>;
using FunctionFromUnixTimestampInJodaSyntax =
    FunctionFormatDateTimeImpl<NameFromUnixTimeInJodaSyntax, SupportInteger::Yes, FormatSyntax::Joda>;
...
formatDateTime(datetime, format[, timezone])
  ...
  FunctionDocumentation::ReturnedValue returned_value_formatDateTime = {"Returns time and date values according to the determined format.",
                                                                        {"String"}};
  FunctionDocumentation::Examples examples_formatDateTime = {{"Format date with year placeholder", R"(
...
└─────────────────────┴───────────────────┴─────────────┘
  ...
  FunctionDocumentation documentation_formatDateTime = {
      description_formatDateTime,    syntax_formatDateTime,   arguments_formatDateTime,     {},
      returned_value_formatDateTime, examples_formatDateTime, introduced_in_formatDateTime, category_formatDateTime};
...
fromUnixTimestamp(timestamp[, format[, timezone]])
  ...
  FunctionDocumentation::ReturnedValue returned_value_fromUnixTimestamp = {
      "Returns `DateTime` of the timestamp when called with one argument, or a String  when called with two or three arguments.",
      {"DateTime", "String"}};
  FunctionDocumentation::Examples examples_fromUnixTimestamp = {{"Convert Unix timestamp to DateTime", R"(
...
└─────────────────────┘
  ...
  FunctionDocumentation documentation_fromUnixTimestamp = {
      description_fromUnixTimestamp,    syntax_fromUnixTimestamp,   arguments_fromUnixTimestamp,     {},
      returned_value_fromUnixTimestamp, examples_fromUnixTimestamp, introduced_in_fromUnixTimestamp, category_fromUnixTimestamp};
...
formatDateTimeInJodaSyntax(datetime, format[, timezone])
  ...
  FunctionDocumentation::Arguments arguments_formatDateTimeInJodaSyntax = {
      ...
      {"timezone", "Optional. Timezone name for the formatted time.", {"String"}}};
  FunctionDocumentation::ReturnedValue returned_value_formatDateTimeInJodaSyntax = {
      "Returns time and date values according to the determined format.", {"String"}};
  FunctionDocumentation::Examples examples_formatDateTimeInJodaSyntax = {{"Format datetime using Joda syntax", R"(
...
└─────────────────────────────────────────────────────────────────────────────────────────┘
  ...
  FunctionDocumentation documentation_formatDateTimeInJodaSyntax = {
      description_formatDateTimeInJodaSyntax,    syntax_formatDateTimeInJodaSyntax,
      arguments_formatDateTimeInJodaSyntax,      {},
      returned_value_formatDateTimeInJodaSyntax, examples_formatDateTimeInJodaSyntax,
      introduced_in_formatDateTimeInJodaSyntax,  category_formatDateTimeInJodaSyntax};
...
fromUnixTimestampInJodaSyntax(timestamp, format[, timezone])
  ...
  FunctionDocumentation::Arguments arguments_fromUnixTimestampInJodaSyntax = {
      ...
      {"timezone", "Optional. Constant time zone string.", {"String"}}};
  FunctionDocumentation::ReturnedValue returned_value_fromUnixTimestampInJodaSyntax = {
      "Returns a date with time when called with one argument, or a String when called with two or three arguments.}",
      {"DateTime", "String"}};
  FunctionDocumentation::Examples examples_fromUnixTimestampInJodaSyntax = {{"Convert Unix timestamp with Joda format", R"(
...
└─────────────────────┘
  ...
  FunctionDocumentation documentation_fromUnixTimestampInJodaSyntax = {
      description_fromUnixTimestampInJodaSyntax,    syntax_fromUnixTimestampInJodaSyntax,
      arguments_fromUnixTimestampInJodaSyntax,      {},
      returned_value_fromUnixTimestampInJodaSyntax, examples_fromUnixTimestampInJodaSyntax,
      introduced_in_fromUnixTimestampInJodaSyntax,  category_fromUnixTimestampInJodaSyntax};
