...
bool allArgumentsAreConstants(const ColumnsWithTypeAndName &args) {
  for (const auto &arg : args)
    if (!isColumnConst(*arg.column)) return false;
  return true;
}

ColumnPtr replaceLowCardinalityColumnsByNestedAndGetDictionaryIndexes(ColumnsWithTypeAndName &args,
                                                                      bool can_be_executed_on_default_arguments, size_t input_rows_count) {
    ...
    if (const auto *low_cardinality_column = checkAndGetColumn<ColumnLowCardinality>(column.column.get())) {
      ...
      if (indexes)
        throw Exception(ErrorCodes::LOGICAL_ERROR,
                        "Default functions implementation for LowCardinality is supported only with a single LowCardinality argument.");
...
ColumnPtr IExecutableFunction::defaultImplementationForConstantArguments(const ColumnsWithTypeAndName &args, const DataTypePtr &result_type,
  ...
  if (args.empty() || !useDefaultImplementationForConstants() || !allArgumentsAreConstants(args)) return nullptr;
  ...
  if (!have_converted_columns)
    throw Exception(ErrorCodes::NUMBER_OF_ARGUMENTS_DOESNT_MATCH,
                    "Number of arguments for function {} doesn't match: the function requires more arguments", getName());
  ...
  if (result_column->size() > 1) result_column = result_column->cloneResized(1);

  return ColumnConst::create(result_column, input_rows_count);
}

ColumnPtr IExecutableFunction::defaultImplementationForNulls(const ColumnsWithTypeAndName &args, const DataTypePtr &result_type,
                                                             size_t input_rows_count, bool dry_run) const {
  if (args.empty() || !useDefaultImplementationForNulls()) return nullptr;
  ...
  if (null_presence.has_nullable) {
    ...
    for (const auto &arg : args) {
      if (!isColumnConst(*arg.column)) all_columns_constant = false;
    ...
    size_t rows_with_nulls =
        result_null_map ? countBytesInFilter(assert_cast<const ColumnUInt8 &>(*result_null_map).getData().data(), 0, input_rows_count) : 0;
    ...
    if (!should_short_circuit) {
      ...
      for (size_t i = 0; i < input_rows_count; ++i) filter_mask[i] = !result_null_map_data[i];

      /// Filter every column by mask
      for (auto &col : temporary_columns) col.column = col.column->filter(filter_mask, rows_without_nulls);
...
ColumnPtr IExecutableFunction::defaultImplementationForNothing(const ColumnsWithTypeAndName &args, const DataTypePtr &result_type,
                                                               size_t input_rows_count) const {
  if (!useDefaultImplementationForNothing()) return nullptr;

  bool is_nothing_type_presented = false;
  for (const auto &arg : args) is_nothing_type_presented |= isNothing(arg.type);

  if (!is_nothing_type_presented) return nullptr;
  ...
  if (input_rows_count > 0) throw Exception(ErrorCodes::ILLEGAL_COLUMN, "Cannot create non-empty column with type Nothing");
  return ColumnNothing::create(0);
}

ColumnPtr IExecutableFunction::executeWithoutLowCardinalityColumns(const ColumnsWithTypeAndName &args, const DataTypePtr &result_type,
                                                                   size_t input_rows_count, bool dry_run) const {
  if (auto res = defaultImplementationForNothing(args, result_type, input_rows_count)) return res;

  if (auto res = defaultImplementationForConstantArguments(args, result_type, input_rows_count, dry_run)) return res;

  if (auto res = defaultImplementationForNulls(args, result_type, input_rows_count, dry_run)) return res;
  ...
  if (!res) throw Exception(ErrorCodes::LOGICAL_ERROR, "Empty column was returned by function {}", getName());

  return res;
}

static void convertSparseColumnsToFull(ColumnsWithTypeAndName &args) {
  for (auto &column : args) column.column = recursiveRemoveSparse(column.column);
}

static void convertReplicatedColumnsToFull(ColumnsWithTypeAndName &args) {
  for (auto &column : args) column.column = column.column->convertToFullColumnIfReplicated();
}
  ...
  if (CurrentThread::isInitialized()) {
    ...
    if (query_context && query_context->getSettingsRef()[Setting::short_circuit_function_evaluation_for_nulls]) {
      ...
      short_circuit_function_evaluation_for_nulls_threshold =
          query_context->getSettingsRef()[Setting::short_circuit_function_evaluation_for_nulls_threshold];
    }
  }
}

ColumnPtr IExecutableFunction::executeWithoutSparseColumns(const ColumnsWithTypeAndName &arguments, const DataTypePtr &result_type,
                                                           size_t input_rows_count, bool dry_run) const {
  ColumnPtr result;
  if (useDefaultImplementationForLowCardinalityColumns()) {
    ...
    if (const auto *res_low_cardinality_type = typeid_cast<const DataTypeLowCardinality *>(result_type.get())) {
      ...
      if (res_is_constant) result = ColumnConst::create(std::move(result), input_rows_count);
    } else {
...
ColumnPtr IExecutableFunction::execute(const ColumnsWithTypeAndName &arguments, const DataTypePtr &result_type, size_t input_rows_count,
                                       bool dry_run) const {
  ...
  if (useDefaultImplementationForReplicatedColumns()) {
    ...
    if (isLazyReplicationUseful(result)) return ColumnReplicated::create(result, common_replicated_indexes);
...
ColumnPtr IExecutableFunction::executeWithoutReplicatedColumns(const ColumnsWithTypeAndName &arguments, const DataTypePtr &result_type,
                                                               size_t input_rows_count, bool dry_run) const {
  ...
  if (result_type->getTypeId() != TypeIndex::Function && use_default_implementation_for_sparse_columns) {
    ...
    if (num_sparse_columns == 1 && num_full_columns == 0) {
      ...
      for (size_t i = 0; i < columns_without_sparse.size(); ++i) {
        if (i == sparse_column_position) continue;
      ...
      if (isColumnConst(*res)) return res->cloneResized(input_rows_count);
...
ColumnPtr IFunctionBase::execute(const DB::ColumnsWithTypeAndName &arguments, const DB::DataTypePtr &result_type, size_t input_rows_count,
                                 bool dry_run) const {
  checkFunctionArgumentSizes(arguments, input_rows_count);
...
IFunctionBase::Monotonicity IFunctionBase::getMonotonicityForRange(const IDataType & /*type*/, const Field & /*left*/,
...
void IFunctionOverloadResolver::checkNumberOfArguments(size_t number_of_arguments) const {
  ...
  if (number_of_arguments != expected_number_of_arguments)
    throw Exception(ErrorCodes::NUMBER_OF_ARGUMENTS_DOESNT_MATCH,
                    "Number of arguments for function {} doesn't match: passed {}, should be {}", getName(), number_of_arguments,
...
DataTypePtr IFunctionOverloadResolver::getReturnType(const ColumnsWithTypeAndName &arguments) const {
  if (useDefaultImplementationForLowCardinalityColumns()) {
    ...
    for (ColumnWithTypeAndName &arg : args_without_low_cardinality) {
      ...
      if (is_const) arg.column = arg.column->convertToFullColumnIfLowCardinality();
      ...
      if (const auto *low_cardinality_type = typeid_cast<const DataTypeLowCardinality *>(arg.type.get())) {
        ...
        if (!is_const) ++num_full_low_cardinality_columns;
      } else if (!is_const)
...
DataTypePtr IFunctionOverloadResolver::getReturnTypeWithoutLowCardinality(const ColumnsWithTypeAndName &arguments) const {
  ...
  if (!arguments.empty() && useDefaultImplementationForNothing()) {
    for (const auto &arg : arguments) {
      if (isNothing(arg.type)) return std::make_shared<DataTypeNothing>();
    }
  }
...
FunctionBasePtr IFunctionOverloadResolver::buildImpl(const ColumnsWithTypeAndName & /* arguments */,
...
IFunctionBase::Monotonicity IFunction::getMonotonicityForRange(const IDataType & /*type*/, const Field & /*left*/,
                                                               const Field & /*right*/) const {
  throw Exception(ErrorCodes::NOT_IMPLEMENTED, "Function {} has no information about its monotonicity", getName());
}
...
bool IFunction::isCompilable(const DataTypes &arguments, const DataTypePtr &result_type) const {
  if (useDefaultImplementationForNulls())
    if (auto denulled_arguments = removeNullables(arguments)) return isCompilableImpl(*denulled_arguments, result_type);
...
llvm::Value *IFunction::compile(llvm::IRBuilderBase &builder, const ValuesWithType &arguments, const DataTypePtr &result_type) const {
  ...
  for (const auto &argument : arguments) arguments_types.push_back(argument.type);
  ...
  if (useDefaultImplementationForNulls() && denulled_arguments_types) {
    ...
    if (!unwrapped_return_type->isNullable()) {
      ...
      for (auto *is_null_value : is_null_values) {
        if (is_null_value) nullable_structure_result_null = b.CreateOr(nullable_structure_result_null, is_null_value);
      }
...
llvm::Value *IFunction::compileImpl(llvm::IRBuilderBase & /*builder*/, const ValuesWithType & /*arguments*/,
                                    const DataTypePtr & /*result_type*/) const {
  throw Exception(ErrorCodes::NOT_IMPLEMENTED, "{} is not JIT-compilable", getName());
}
