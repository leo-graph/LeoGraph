...
#define MASK(IDX, ...) ((mask) ? shrink(mask->getColumn((IDX)).getUInt(0), std::get<IDX>(__VA_ARGS__)) : std::get<IDX>(__VA_ARGS__))
...
#define EXECUTE()                                                                                                                    \
  ...
  EXTRACT_VECTOR(1)                                                                                                                  \
  DECODE(2, vec0[i] = MASK(0, res); vec1[i] = MASK(1, res);)                                                                         \
  EXTRACT_VECTOR(2)                                                                                                                  \
  DECODE(3, vec0[i] = MASK(0, res); vec1[i] = MASK(1, res); vec2[i] = MASK(2, res);)                                                 \
  EXTRACT_VECTOR(3)                                                                                                                  \
  DECODE(4, vec0[i] = MASK(0, res); vec1[i] = MASK(1, res); vec2[i] = MASK(2, res); vec3[i] = MASK(3, res);)                         \
  EXTRACT_VECTOR(4)                                                                                                                  \
  DECODE(5, vec0[i] = MASK(0, res); vec1[i] = MASK(1, res); vec2[i] = MASK(2, res); vec3[i] = MASK(3, res); vec4[i] = MASK(4, res);) \
  EXTRACT_VECTOR(5)                                                                                                                  \
  DECODE(6, vec0[i] = MASK(0, res); vec1[i] = MASK(1, res); vec2[i] = MASK(2, res); vec3[i] = MASK(3, res); vec4[i] = MASK(4, res);  \
         vec5[i] = MASK(5, res);)                                                                                                    \
  EXTRACT_VECTOR(6)                                                                                                                  \
  DECODE(7, vec0[i] = MASK(0, res); vec1[i] = MASK(1, res); vec2[i] = MASK(2, res); vec3[i] = MASK(3, res); vec4[i] = MASK(4, res);  \
         vec5[i] = MASK(5, res); vec6[i] = MASK(6, res);)                                                                            \
  EXTRACT_VECTOR(7)                                                                                                                  \
  DECODE(8, vec0[i] = MASK(0, res); vec1[i] = MASK(1, res); vec2[i] = MASK(2, res); vec3[i] = MASK(3, res); vec4[i] = MASK(4, res);  \
         vec5[i] = MASK(5, res); vec6[i] = MASK(6, res); vec7[i] = MASK(7, res);)                                                    \
  switch (nd) {                                                                                                                      \
    case 2:                                                                                                                          \
      tuple_columns[0] = std::move(col0);                                                                                            \
...
DECLARE_DEFAULT_CODE(constexpr auto MortonND_2D_Dec = mortonnd::MortonNDLutDecoder<2, 32, 8>();
                     ...
                     class FunctionMortonDecode : public FunctionSpaceFillingCurveDecode<8, 1, 8> {
                      public:
                       ...
                       static FunctionPtr create(ContextPtr) { return std::make_shared<FunctionMortonDecode>(); }
...
class FunctionMortonDecode: public TargetSpecific::Default::FunctionMortonDecode
{
 public:
  explicit FunctionMortonDecode(ContextPtr context) : selector(context) {
    selector.registerImplementation<TargetArch::Default, TargetSpecific::Default::FunctionMortonDecode>();

#if USE_MULTITARGET_CODE && defined(MORTON_ND_BMI2_ENABLED)
    selector.registerImplementation<TargetArch::x86_64_v3, TargetSpecific::x86_64_v3::FunctionMortonDecode>();
#endif
  }
  ...
  static FunctionPtr create(ContextPtr context) { return std::make_shared<FunctionMortonDecode>(context); }
...
mortonDecode(range_mask, code)
  ...
  FunctionDocumentation::Arguments arguments = {
      ...
      {"range_mask",
       "For the expanded mode, the mask for each argument. The mask is a tuple of unsigned integers. Each number in the mask configures "
       "the amount of range shrink.",
