...
concept is_native_int_or_decimal_v = std::is_integral_v<ResultType> || (is_decimal<ResultType> && sizeof(ResultType) <= 8);
...
inline void fillConstantConstant(const ArrayCond &cond, A a, B b, ArrayResult &res) {
  ...
  if constexpr (is_over_big_int<ResultType>) {
    ...
    for (size_t i = 0; i < size; ++i) res[i] = cond[i] ? static_cast<ResultType>(a) : static_cast<ResultType>(b);
  }
}
...
class FunctionIf : public FunctionIfBase {
 public:
  ...
  explicit FunctionIf(bool use_variant_when_no_common_type_ = false)
      : FunctionIfBase(), use_variant_when_no_common_type(use_variant_when_no_common_type_) {}
 ...
 private:
  ...
  static UInt32 decimalScale(const ColumnsWithTypeAndName &arguments [[maybe_unused]]) {
    if constexpr (is_decimal<T0> && is_decimal<T1>) {
      ...
      if (left_scale != right_scale) throw Exception(ErrorCodes::NOT_IMPLEMENTED, "Conditional functions with different Decimal scales");
      return left_scale;
    } else
  ...
  ColumnPtr executeRightType([[maybe_unused]] const ColumnUInt8 *cond_col, [[maybe_unused]] const ColumnsWithTypeAndName &arguments,
    ...
    if constexpr (std::is_same_v<ResultType, NumberTraits::Error>) {
      ...
      if (const auto *col_right_vec = checkAndGetColumn<ColVecT1>(col_right_untyped)) {
        return NumIfImpl<T0, T1, ResultType>::vectorVector(cond_col->getData(), col_left->getData(), col_right_vec->getData(), scale);
      }
      if (const auto *col_right_const = checkAndGetColumnConst<ColVecT1>(col_right_untyped)) {
        return NumIfImpl<T0, T1, ResultType>::vectorConstant(cond_col->getData(), col_left->getData(),
                                                             col_right_const->template getValue<T1>(), scale);
      }
  ...
  template <typename T0, typename T1, typename ColVecT0, typename ColVecT1>
  ColumnPtr executeConstRightType([[maybe_unused]] const ColumnUInt8 *cond_col, [[maybe_unused]] const ColumnsWithTypeAndName &arguments,
    ...
    if constexpr (std::is_same_v<ResultType, NumberTraits::Error>) {
      ...
      if (const auto *col_right_vec = checkAndGetColumn<ColVecT1>(col_right_untyped)) {
        return NumIfImpl<T0, T1, ResultType>::constantVector(cond_col->getData(), col_left->template getValue<T0>(),
                                                             col_right_vec->getData(), scale);
      }
      if (const auto *col_right_const = checkAndGetColumnConst<ColVecT1>(col_right_untyped)) {
        return NumIfImpl<T0, T1, ResultType>::constantConstant(cond_col->getData(), col_left->template getValue<T0>(),
                                                               col_right_const->template getValue<T1>(), scale);
      }
  ...
  template <typename T0, typename T1, typename ColVecT0, typename ColVecT1>
  ColumnPtr executeRightTypeArray([[maybe_unused]] const ColumnUInt8 *cond_col, [[maybe_unused]] const ColumnsWithTypeAndName &arguments,
                                  [[maybe_unused]] const DataTypePtr result_type, [[maybe_unused]] const ColumnArray *col_left_array,
    ...
    if constexpr (std::is_same_v<ResultType, NumberTraits::Error>) {
      ...
      if (const auto *col_right_array = checkAndGetColumn<ColumnArray>(col_right_untyped)) {
        ...
        if (!col_right_vec) return nullptr;

        auto res = result_type->createColumn();
        auto &arr_res = assert_cast<ColumnArray &>(*res);

        conditional(NumericArraySource<T0>(*col_left_array), NumericArraySource<T1>(*col_right_array),
                    NumericArraySink<ResultType>(arr_res.getData(), arr_res.getOffsets(), input_rows_count), cond_col->getData());
      ...
      if (const auto *col_right_const_array = checkAndGetColumnConst<ColumnArray>(col_right_untyped)) {
        ...
        if (!checkColumn<ColVecT1>(&col_right_const_array_data->getData())) return nullptr;

        auto res = result_type->createColumn();
        auto &arr_res = assert_cast<ColumnArray &>(*res);

        conditional(NumericArraySource<T0>(*col_left_array), ConstSource<NumericArraySource<T1>>(*col_right_const_array),
                    NumericArraySink<ResultType>(arr_res.getData(), arr_res.getOffsets(), input_rows_count), cond_col->getData());
  ...
  template <typename T0, typename T1, typename ColVecT0, typename ColVecT1>
  ColumnPtr executeConstRightTypeArray([[maybe_unused]] const ColumnUInt8 *cond_col,
    ...
    if constexpr (std::is_same_v<ResultType, NumberTraits::Error>) {
      ...
      if (const auto *col_right_array = checkAndGetColumn<ColumnArray>(col_right_untyped)) {
        ...
        conditional(ConstSource<NumericArraySource<T0>>(*col_left_const_array), NumericArraySource<T1>(*col_right_array),
                    NumericArraySink<ResultType>(arr_res.getData(), arr_res.getOffsets(), input_rows_count), cond_col->getData());
      ...
      if (const auto *col_right_const_array = checkAndGetColumnConst<ColumnArray>(col_right_untyped)) {
        ...
        conditional(ConstSource<NumericArraySource<T0>>(*col_left_const_array), ConstSource<NumericArraySource<T1>>(*col_right_const_array),
                    NumericArraySink<ResultType>(arr_res.getData(), arr_res.getOffsets(), input_rows_count), cond_col->getData());
  ...
  template <typename T0, typename T1>
  ColumnPtr executeTyped(const ColumnUInt8 *cond_col, const ColumnsWithTypeAndName &arguments, const DataTypePtr &result_type,
    ...
    if (const auto *col_left = checkAndGetColumn<ColVecT0>(col_left_untyped)) {
      ...
      if (auto col_arr_left_elems = checkAndGetColumn<ColVecT0>(&col_arr_left->getData())) {
        right_column = executeRightTypeArray<T0, T1, ColVecT0, ColVecT1>(cond_col, arguments, result_type, col_arr_left, input_rows_count);
      }
      ...
      if (checkColumn<ColVecT0>(&assert_cast<const ColumnArray &>(col_const_arr_left->getDataColumn()).getData())) {
        right_column =
            executeConstRightTypeArray<T0, T1, ColVecT0, ColVecT1>(cond_col, arguments, result_type, col_const_arr_left, input_rows_count);
      }
    }
  ...
  static ColumnPtr executeString(const ColumnUInt8 *cond_col, const ColumnsWithTypeAndName &arguments, const DataTypePtr &result_type) {
    ...
    if (isFixedString(result_type)) {
      ...
      if (col_then_fixed && col_else_fixed)
        ...
        conditional(ConstSource<FixedStringSource>(*col_then_const_fixed), ConstSource<FixedStringSource>(*col_else_const_fixed), sink,
                    cond_data);
      else
        return nullptr;
  ...
  static ColumnPtr executeGenericArray(const ColumnUInt8 *cond_col, const ColumnsWithTypeAndName &arguments,
                                       const DataTypePtr &result_type) {
    /// For generic implementation, arrays must be of same type.
    if (!arguments[1].type->equals(*arguments[2].type)) return nullptr;
    ...
    if ((col_arr_then || col_arr_then_const) && (col_arr_else || col_arr_else_const)) {
      ...
      if (col_arr_then && col_arr_else)
        conditional(GenericArraySource(*col_arr_then), GenericArraySource(*col_arr_else),
                    GenericArraySink(col_res->getData(), col_res->getOffsets(), rows), cond_data);
      else if (col_arr_then && col_arr_else_const)
        conditional(GenericArraySource(*col_arr_then), ConstSource<GenericArraySource>(*col_arr_else_const),
                    GenericArraySink(col_res->getData(), col_res->getOffsets(), rows), cond_data);
      else if (col_arr_then_const && col_arr_else)
        conditional(ConstSource<GenericArraySource>(*col_arr_then_const), GenericArraySource(*col_arr_else),
                    GenericArraySink(col_res->getData(), col_res->getOffsets(), rows), cond_data);
  ...
  ColumnPtr executeMap(const ColumnsWithTypeAndName &arguments, const DataTypePtr &result_type, size_t input_rows_count) const {
    ...
    if (!key_array)
      throw Exception(ErrorCodes::LOGICAL_ERROR, "Key result column should be {} instead of {} in executeMap of function {}",
                      key_result_type->getName(), key_result->getName(), getName());
    if (!value_array)
      throw Exception(ErrorCodes::LOGICAL_ERROR, "Value result column should be {} instead of {} in executeMap of function {}",
                      key_result_type->getName(), value_result->getName(), getName());
    if (!key_array->hasEqualOffsets(*value_array))
      throw Exception(ErrorCodes::LOGICAL_ERROR, "Key array and value array must have equal sizes in executeMap of function {}", getName());

    auto nested_column =
        ColumnArray::create(ColumnTuple::create(Columns{key_array->getDataPtr(), value_array->getDataPtr()}), key_array->getOffsetsPtr());
    return ColumnMap::create(std::move(nested_column));
  }

  static ColumnPtr executeGenericWithType(const ColumnUInt8 *cond_col, const ColumnsWithTypeAndName &arguments, size_t input_rows_count,
  ...
  static ColumnPtr executeGeneric(const ColumnUInt8 *cond_col, const ColumnsWithTypeAndName &arguments, size_t input_rows_count,
  ...
  ColumnPtr executeForConstAndNullableCondition(const ColumnsWithTypeAndName &arguments, const DataTypePtr &result_type,
                                                size_t /*input_rows_count*/) const {
    ...
    if (const auto *const_arg = checkAndGetColumn<ColumnConst>(&*arg_cond.column)) {
      ...
      if (const auto *const_nullable_arg = checkAndGetColumn<ColumnNullable>(&*data_column)) {
        ...
        if (!data_column->empty()) cond_is_null = const_nullable_arg->getNullMapData()[0];
      }
    ...
    if (cond_is_true) return castColumn(column1, result_type);
    if (cond_is_false || cond_is_null) return castColumn(column2, result_type);
    ...
    if (const auto *nullable = checkAndGetColumn<ColumnNullable>(&*not_const_condition)) {
      ...
      if (checkAndGetColumn<ColumnUInt8>(&*new_cond_column)) {
        ...
        if (cond_is_const) new_cond_column = ColumnConst::create(new_cond_column, column_size);
      } else
  ...
  static ColumnPtr makeNullableColumnIfNot(const ColumnPtr &column) {
    ...
    if (isColumnNullable(*materialized)) return materialized;
  ...
  static ColumnPtr recursiveGetNullMapFromNullable(const ColumnPtr &column) {
    if (const auto *nullable = checkAndGetColumn<ColumnNullable>(&*column)) return nullable->getNullMapColumnPtr();
    if (const auto *column_const = checkAndGetColumn<ColumnConst>(&*column)) {
  ...
  ColumnPtr executeForNullableThenElse(const ColumnsWithTypeAndName &arguments, const DataTypePtr &result_type,
                                       size_t input_rows_count) const {
    /// If result type is Variant/Dynamic, we don't need to remove Nullable.
    if (isVariant(result_type) || isDynamic(result_type)) return nullptr;
    ...
    if (!then_is_nullable && !else_is_nullable) return nullptr;
    ...
    {
      ColumnsWithTypeAndName temporary_columns({arg_cond,
                                                {then_is_nullable ? recursiveGetNullMapFromNullable(arg_then.column)
                                                                  : DataTypeUInt8().createColumnConstWithDefaultValue(input_rows_count),
                                                 std::make_shared<DataTypeUInt8>(), ""},
                                                {else_is_nullable ? recursiveGetNullMapFromNullable(arg_else.column)
                                                                  : DataTypeUInt8().createColumnConstWithDefaultValue(input_rows_count),
                                                 std::make_shared<DataTypeUInt8>(), ""}});
...

    {
      ColumnsWithTypeAndName temporary_columns(
          {arg_cond,
           {recursiveGetNestedColumnWithoutNullable(arg_then.column), removeNullable(arg_then.type), ""},
           {recursiveGetNestedColumnWithoutNullable(arg_else.column), removeNullable(arg_else.type), ""}});

      result_nested_column = executeImpl(temporary_columns, removeNullable(result_type), temporary_columns.front().column->size());
    }

    return ColumnNullable::create(materializeColumnIfConst(result_nested_column), materializeColumnIfConst(result_null_mask));
  }
  ...
  ColumnPtr executeForNullThenElse(const ColumnsWithTypeAndName &arguments, const DataTypePtr &result_type, size_t input_rows_count) const {
    ...
    if (!then_is_null && !else_is_null) return nullptr;

    if (then_is_null && else_is_null) return result_type->createColumnConstWithDefaultValue(input_rows_count);
    ...
    if (then_is_null) {
      ...
      if (cond_const_col) {
        if (cond_const_col->getValue<UInt8>()) return result_type->createColumn()->cloneResized(input_rows_count);
        return makeNullableColumnIfNot(arg_else_column);
      }
      throw Exception(ErrorCodes::ILLEGAL_COLUMN,
                      "Illegal column {} of first argument of function {}. "
                      "Must be ColumnUInt8 or ColumnConstUInt8.",
                      arg_cond.column->getName(), getName());
    }
    ...
    if (else_is_null) {
      ...
      if (cond_col) {
        ...
        for (size_t i = 0; i < size; ++i) negated_null_map_data[i] = !null_map_data[i];

        return ColumnNullable::create(materializeColumnIfConst(result_column), std::move(negated_null_map));
      }
      if (cond_const_col) {
        if (cond_const_col->getValue<UInt8>()) return makeNullableColumnIfNot(arg_then_column);
 ...
 public:
  ...
  DataTypePtr getReturnTypeImpl(const DataTypes &arguments) const override {
    if (!arguments[0]->onlyNull()) {
      if (arguments[0]->isNullable()) return getReturnTypeImpl({removeNullable(arguments[0]), arguments[1], arguments[2]});
    ...
    if (use_variant_when_no_common_type) return getLeastSupertypeOrVariant(DataTypes{arguments[1], arguments[2]});
  ...
  ColumnPtr executeImpl(const ColumnsWithTypeAndName &args, const DataTypePtr &result_type, size_t input_rows_count) const override {
    ...
    if (cond_const_col) {
      ...
      if (arg.type->equals(*result_type)) return arg.column;
    ...
    if (isVariant(result_type)) return executeGeneric(cond_col, arguments, input_rows_count, use_variant_when_no_common_type);
    ...
    if (const auto *left_array = checkAndGetDataType<DataTypeArray>(arg_then.type.get())) left_type = left_array->getNestedType();

    if (const auto *right_array = checkAndGetDataType<DataTypeArray>(arg_else.type.get())) right_type = right_array->getNestedType();
    ...
    if (!(callOnBasicTypes<true, true, true, false>(left_id, right_id, call) ||
          ...
          (res = executeString(cond_col, arguments, result_type)) || (res = executeGenericArray(cond_col, arguments, result_type)) ||
          (res = executeTuple(arguments, result_type, input_rows_count)) || (res = executeMap(arguments, result_type, input_rows_count)))) {
  ...
  ColumnPtr getConstantResultForNonConstArguments(const ColumnsWithTypeAndName &arguments, const DataTypePtr &result_type) const override {
    const ColumnWithTypeAndName &arg_cond = arguments[0];
    if (!arg_cond.column || !isColumnConst(*arg_cond.column)) return {};

    const ColumnConst *cond_const_col = checkAndGetColumnConst<ColumnVector<UInt8>>(arg_cond.column.get());
    if (!cond_const_col) return {};
    ...
    if (!potential_const_column.column || !isColumnConst(*potential_const_column.column)) return {};

    auto result = castColumn(potential_const_column, result_type);
    if (!isColumnConst(*result)) return {};
...
`then` and `else` must be of a similar type.
  ...
  FunctionDocumentation::Arguments arguments = {{"cond", "The evaluated condition.", {"UInt8", "Nullable(UInt8)", "NULL"}},
  ...
  FunctionDocumentation::Examples examples = {{"Example usage", R"(
