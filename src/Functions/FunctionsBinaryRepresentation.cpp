...
struct HexImpl {
  ...
  static void executeOneUIntOrInt(T x, char *&out, bool skip_leading_zero = true) {
    ...
    for (int offset = (sizeof(T) - 1) * 8; offset >= 0; offset -= 8) {
      ...
      /// Skip leading zeros
      if (byte == 0 && !was_nonzero && offset && skip_leading_zero) continue;
...
struct UnhexImpl {
  ...
  static void decode(const char *pos, const char *end, char *&out) { hexStringDecode(pos, end, out, word_size); }
};
 ...
 public:
  ...
  DataTypePtr getReturnTypeImpl(const DataTypes &arguments) const override {
    ...
    if (!which.isStringOrFixedString() && !which.isDate() && !which.isDateTime() && !which.isDateTime64() && !which.isUInt() &&
        !which.isInt() && !which.isFloat() && !which.isDecimal() && !which.isUUID() && !which.isIPv4() && !which.isIPv6() &&
  ...
  ColumnPtr executeImpl(const ColumnsWithTypeAndName &arguments, const DataTypePtr &, size_t /*input_rows_count*/) const override {
    ...
    if (tryExecuteUIntOrInt<UInt8>(column, res_column) || tryExecuteUIntOrInt<UInt16>(column, res_column) ||
        ...
        tryExecuteUIntOrInt<UInt128>(column, res_column) || tryExecuteUIntOrInt<UInt256>(column, res_column) ||
        ...
        tryExecuteUIntOrInt<Int128>(column, res_column) || tryExecuteUIntOrInt<Int256>(column, res_column) ||
        tryExecuteString(column, res_column) || tryExecuteFixedString(column, res_column) ||
        tryExecuteFloat<BFloat16>(column, res_column) || tryExecuteFloat<Float32>(column, res_column) ||
        tryExecuteFloat<Float64>(column, res_column) || tryExecuteDecimal<Decimal32>(column, res_column) ||
        tryExecuteDecimal<Decimal64>(column, res_column) || tryExecuteDecimal<Decimal128>(column, res_column) ||
        tryExecuteDecimal<Decimal256>(column, res_column) || tryExecuteUUID(column, res_column) || tryExecuteIPv4(column, res_column) ||
  ...
  bool tryExecuteUIntOrInt(const IColumn *col, ColumnPtr &col_res) const {
    ...
    if (col_vec) {
      ...
      for (size_t i = 0; i < size; ++i) {
        ...
        if (pos + MAX_LENGTH > out_vec.size()) out_vec.resize(out_vec.size() * word_size + MAX_LENGTH);
  ...
  bool tryExecuteIPv6(const IColumn *col, ColumnPtr &col_res) const {
    ...
    for (size_t i = 0; i < size; ++i) {
      ...
      Impl::executeOneString(reinterpret_cast<const UInt8 *>(&ip[i].toUnderType().items[0]),
                             reinterpret_cast<const UInt8 *>(&ip[i].toUnderType().items[2]), end);
 ...
 public:
  ...
  ColumnPtr executeImpl(const ColumnsWithTypeAndName &arguments, const DataTypePtr &, size_t input_rows_count) const override {
    ...
    if (const ColumnString *col = checkAndGetColumn<ColumnString>(column.get())) {
      ...
      chassert(static_cast<size_t>(pos - begin) <= out_vec.size(),
...
The function uses uppercase letters `A-F` and not using any prefixes (like `0x`) or suffixes (like `h`).
  ...
  FunctionDocumentation::ReturnedValue hex_returned_value = {"Returns a string with the hexadecimal representation of the argument.",
                                                             {"String"}};
...
└──────────────────────────────────┘
  ...
  FunctionDocumentation hex_documentation = {hex_description,    hex_syntax,   hex_arguments,     {},
                                             hex_returned_value, hex_examples, hex_introduced_in, hex_category};
...
For a numeric argument the inverse of hex(N) is not performed by unhex().
  ...
  FunctionDocumentation::Examples unhex_examples = {{"Basic usage", "SELECT unhex('303132'), UNHEX('4D7953514C')",
                                                     R"(
...
└──────┘
  ...
  FunctionDocumentation unhex_documentation = {unhex_description,    unhex_syntax,   unhex_arguments,     {},
                                               unhex_returned_value, unhex_examples, unhex_introduced_in, unhex_category};
...
| `UUID`                     | Encoded as big-endian order string.                                                                                                                                                                                                                                   |
  ...
  FunctionDocumentation::ReturnedValue bin_returned_value = {"Returns a string with the binary representation of the argument.",
                                                             {"String"}};
...
└──────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────┘
  ...
  FunctionDocumentation bin_documentation = {bin_description,    bin_syntax,   bin_arguments,     {},
                                             bin_returned_value, bin_examples, bin_introduced_in, bin_category};
...
└─────┘
  ...
  FunctionDocumentation unbin_documentation = {unbin_description,    unbin_syntax,   unbin_arguments,     {},
                                               unbin_returned_value, unbin_examples, unbin_introduced_in, unbin_category};
