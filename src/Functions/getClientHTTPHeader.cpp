  ...
  explicit FunctionGetClientHTTPHeader(ContextPtr context_) : WithContext(context_) {
    if (!getContext()->getSettingsRef()[Setting::allow_get_client_http_header])
      throw Exception(ErrorCodes::FUNCTION_NOT_ALLOWED,
                      "The function getClientHTTPHeader requires setting `allow_get_client_http_header` to be enabled.");
  }
...
If the function is used in the context of a distributed query, it returns non-empty result only on the initiator node.
  ...
  FunctionDocumentation::Arguments arguments = {{"name", "The HTTP header name.", {"String"}}};
...
└─────────────────────────────────────┘
  ...
  factory.registerFunction(
      "getClientHTTPHeader", [](ContextPtr context) { return std::make_shared<FunctionGetClientHTTPHeader>(context); }, documentation);
}
