...
class FunctionRandomStringImpl : public IFunction {
 public:
  ...
  DataTypePtr getReturnTypeImpl(const ColumnsWithTypeAndName &arguments) const override {
    FunctionArgumentDescriptors mandatory_args{{"length", &isNumber, nullptr, "(U)Int*"}};

    FunctionArgumentDescriptors optional_args{{"x", nullptr, nullptr, "Any"}};
  ...
  ColumnPtr executeImpl(const ColumnsWithTypeAndName &arguments, const DataTypePtr &, size_t input_rows_count) const override {
    ...
    for (size_t row_num = 0; row_num < input_rows_count; ++row_num) {
      ...
      if (length > (1 << 30)) throw Exception(ErrorCodes::TOO_LARGE_STRING_SIZE, "Too large string size in function {}", getName());
...
class FunctionRandomString : public FunctionRandomStringImpl<TargetSpecific::Default::RandImpl> {
 public:
  explicit FunctionRandomString(ContextPtr context) : selector(context) {
    selector.registerImplementation<TargetArch::Default, FunctionRandomStringImpl<TargetSpecific::Default::RandImpl>>();

#if USE_MULTITARGET_CODE
    selector.registerImplementation<TargetArch::x86_64_v3, FunctionRandomStringImpl<TargetSpecific::x86_64_v3::RandImpl>>();
#endif
  }
  ...
  static FunctionPtr create(ContextPtr context) { return std::make_shared<FunctionRandomString>(context); }
...
The returned characters are not necessarily ASCII characters, i.e. they may not be printable.
  ...
  FunctionDocumentation::Examples examples = {{"Usage example", "SELECT randomString(5) AS str FROM numbers(2)", R"(
���
�v6B�
        )"}};
