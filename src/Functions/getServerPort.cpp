...
class GetServerPortOverloadResolver : public IFunctionOverloadResolver {
 public:
  ...
  static FunctionOverloadResolverPtr create(ContextPtr context_) { return std::make_unique<GetServerPortOverloadResolver>(context_); }

  explicit GetServerPortOverloadResolver(ContextPtr context) : is_distributed(context->isDistributed()) {}
  ...
  DataTypePtr getReturnTypeImpl(const DataTypes &data_types) const override {
    ...
    if (number_of_arguments != 1)
      throw Exception(ErrorCodes::NUMBER_OF_ARGUMENTS_DOESNT_MATCH,
                      "Number of arguments for function {} doesn't match: passed {}, should be 1", getName(), number_of_arguments);
  ...
  FunctionBasePtr buildImpl(const ColumnsWithTypeAndName &arguments, const DataTypePtr &return_type) const override {
    if (!isString(arguments[0].type))
      throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT,
                      "The argument of function {} should be a constant string with the name of a setting", getName());
...
Returns the server's port number for a given protocol.
  ...
  FunctionDocumentation::Arguments arguments = {{"port_name", "The name of the port.", {"String"}}};
