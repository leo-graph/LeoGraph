...
class FunctionByteSize : public IFunction {
 public:
  ...
  static FunctionPtr create(ContextPtr) { return std::make_shared<FunctionByteSize>(); }
  ...
  DataTypePtr getReturnTypeImpl(const ColumnsWithTypeAndName& /*arguments*/) const override { return std::make_shared<DataTypeUInt64>(); }
  ...
  ColumnPtr executeImpl(const ColumnsWithTypeAndName& arguments, const DataTypePtr& result_type, size_t input_rows_count) const override {
    ...
    if (all_constant) return result_type->createColumnConst(input_rows_count, constant_size);
    ...
    for (size_t arg_num = 0; arg_num < num_args; ++arg_num) {
      ...
      if (arg_num == 0)
        for (size_t row = 0; row < input_rows_count; ++row) vec_res[row] = column->byteSizeAt(row);
      else
        for (size_t row = 0; row < input_rows_count; ++row) vec_res[row] += column->byteSizeAt(row);
    }
