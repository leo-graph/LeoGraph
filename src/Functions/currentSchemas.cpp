...
class FunctionCurrentSchemas : public IFunction {
 ...
 public:
  ...
  static FunctionPtr create(ContextPtr context) { return std::make_shared<FunctionCurrentSchemas>(context->getCurrentDatabase()); }

  explicit FunctionCurrentSchemas(const String &db_name_) : db_name{db_name_} {}
  ...
  DataTypePtr getReturnTypeImpl(const DataTypes &arguments) const override {
    ...
    if (!argument_is_valid) throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT, "Argument for function {} must be bool", getName());
  ...
  ColumnPtr executeImpl(const ColumnsWithTypeAndName &, const DataTypePtr &, size_t input_rows_count) const override {
    return DataTypeArray(std::make_shared<DataTypeString>()).createColumnConst(input_rows_count, Array{db_name});
  }
};
...
Also see the [`SET` statement](/sql-reference/statements/use).
  ...
  FunctionDocumentation::Arguments arguments = {{"bool", "A boolean value, which is ignored.", {"Bool"}}};
  FunctionDocumentation::ReturnedValue returned_value = {"Returns a single-element array with the name of the current database.",
                                                         {"Array(String)"}};
