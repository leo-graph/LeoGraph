...
struct FunctionDetectTonalityImpl {
  static Float32 detectTonality(const UInt8* str, const size_t str_len, const FrequencyHolder::Map& emotional_dict) {
    ...
    for (size_t ind = 0; ind < str_len; ++ind) {
      ...
      if (isWhitespaceASCII(str[ind]) || isPunctuationASCII(str[ind])) continue;
    ...
    if (weight > 0) return static_cast<Float32>(weight / static_cast<Float64>(count_words) / 6);
    return static_cast<Float32>(weight / static_cast<Float64>(count_words) / 12);
  }

  static void vector(const ColumnString::Chars& data, const ColumnString::Offsets& offsets, PaddedPODArray<Float32>& res,
...
:::
  ...
  FunctionDocumentation::Arguments arguments = {{"s", "The text to be analyzed.", {"String"}}};
  FunctionDocumentation::ReturnedValue returned_value = {"Returns the average sentiment value of the words in text", {"Float32"}};
  FunctionDocumentation::Examples examples = {{"Russian sentiment analysis",
                                               R"(
SELECT
