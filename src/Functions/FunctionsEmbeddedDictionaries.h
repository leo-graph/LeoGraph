...
struct RegionsHierarchyGetter {
  ...
  static Dst &get(Src &src, const std::string &key) { return src.get(key); }
};
...
struct IdentityDictionaryGetter {
  ...
  static Dst &get(Src &src, const std::string &key) {
    if (key.empty()) return src;  /// NOLINT(bugprone-return-const-ref-from-parameter)
...
class FunctionTransformWithDictionary : public IFunction {
 ...
 public:
  explicit FunctionTransformWithDictionary(const std::shared_ptr<typename DictGetter::Src> &owned_dict_) : owned_dict(owned_dict_) {
    if (!owned_dict)
      throw Exception(ErrorCodes::DICTIONARIES_WAS_NOT_LOADED,
                      "Embedded dictionaries were not loaded. You need to check configuration file.");
  }
  ...
  DataTypePtr getReturnTypeImpl(const DataTypes &arguments) const override {
    ...
    if (arguments.size() == 2 && arguments[1]->getName() != "String")
      throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT,
                      "Illegal type {} of the second ('point of view') argument of function {} (must be String)", arguments[1]->getName(),
                      getName());
  ...
  ColumnPtr executeImpl(const ColumnsWithTypeAndName &arguments, const DataTypePtr &, size_t input_rows_count) const override {
    ...
    if (arguments.size() == 2) {
      ...
      if (!key_col)
        throw Exception(ErrorCodes::ILLEGAL_COLUMN,
                        "Illegal column {} of second ('point of view') argument of function {}. "
                        "Must be constant string.",
                        arguments[1].column->getName(), name);
    ...
    if (const ColumnVector<T> *col_from = checkAndGetColumn<ColumnVector<T>>(arguments[0].column.get())) {
      ...
      for (size_t i = 0; i < input_rows_count; ++i) vec_to[i] = Transform::apply(vec_from[i], dict);

      return col_to;
    }
    throw Exception(ErrorCodes::ILLEGAL_COLUMN, "Illegal column {} of first argument of function {}", arguments[0].column->getName(), name);
  }
};
...
class FunctionIsInWithDictionary : public IFunction {
 ...
 public:
  explicit FunctionIsInWithDictionary(const std::shared_ptr<typename DictGetter::Src> &owned_dict_) : owned_dict(owned_dict_) {
    if (!owned_dict)
      throw Exception(ErrorCodes::DICTIONARIES_WAS_NOT_LOADED,
                      "Embedded dictionaries were not loaded. You need to check configuration file.");
  }
  ...
  DataTypePtr getReturnTypeImpl(const DataTypes &arguments) const override {
    ...
    if (arguments.size() == 3 && arguments[2]->getName() != "String")
      throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT,
                      "Illegal type {} of the third ('point of view') argument of function {} (must be String)", arguments[2]->getName(),
                      getName());
  ...
  ColumnPtr executeImpl(const ColumnsWithTypeAndName &arguments, const DataTypePtr &, size_t input_rows_count) const override {
    ...
    if (arguments.size() == 3) {
      ...
      if (!key_col)
        throw Exception(ErrorCodes::ILLEGAL_COLUMN,
                        "Illegal column {} of third ('point of view') argument of function {}. "
                        "Must be constant string.",
                        arguments[2].column->getName(), name);
    ...
    if (col_vec1 && col_vec2) {
      ...
      for (size_t i = 0; i < input_rows_count; ++i) vec_to[i] = Transform::apply(vec_from1[i], vec_from2[i], dict);
    ...
    if (col_vec1 && col_const2) {
      ...
      for (size_t i = 0; i < input_rows_count; ++i) vec_to[i] = Transform::apply(vec_from1[i], const_from2, dict);
    ...
    if (col_const1 && col_vec2) {
      ...
      for (size_t i = 0; i < input_rows_count; ++i) vec_to[i] = Transform::apply(const_from1, vec_from2[i], dict);

      return col_to;
    }
    if (col_const1 && col_const2) {
      return DataTypeUInt8().createColumnConst(
          col_const1->size(), toField(Transform::apply(col_const1->template getValue<T>(), col_const2->template getValue<T>(), dict)));
    }
...
class FunctionHierarchyWithDictionary : public IFunction {
 ...
 public:
  explicit FunctionHierarchyWithDictionary(const std::shared_ptr<typename DictGetter::Src> &owned_dict_) : owned_dict(owned_dict_) {
    if (!owned_dict)
      throw Exception(ErrorCodes::DICTIONARIES_WAS_NOT_LOADED,
                      "Embedded dictionaries were not loaded. You need to check configuration file.");
  }
...
class FunctionRegionToName : public IFunction {
 ...
 public:
  explicit FunctionRegionToName(const MultiVersion<RegionsNames>::Version &owned_dict_) : owned_dict(owned_dict_) {
    if (!owned_dict)
      throw Exception(ErrorCodes::DICTIONARIES_WAS_NOT_LOADED,
                      "Embedded dictionaries were not loaded. You need to check configuration file.");
  }
  ...
  DataTypePtr getReturnTypeImpl(const DataTypes &arguments) const override {
    ...
    if (arguments[0]->getName() != "UInt32")
      throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT, "Illegal type {} of the first argument of function {} (must be UInt32)",
                      arguments[0]->getName(), getName());

    if (arguments.size() == 2 && arguments[1]->getName() != "String")
      throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT, "Illegal type {} of the second argument of function {} (must be String)",
                      arguments[0]->getName(), getName());
  ...
  ColumnPtr executeImpl(const ColumnsWithTypeAndName &arguments, const DataTypePtr &, size_t /*input_rows_count*/) const override {
    ...
    throw Exception(ErrorCodes::ILLEGAL_COLUMN, "Illegal column {} of the first argument of function {}", arguments[0].column->getName(),
                    getName());
  }
};
