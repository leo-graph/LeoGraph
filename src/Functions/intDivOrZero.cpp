...
struct DivideIntegralOrZeroImpl {
  ...
  static Result apply(A a, B b) {
    if (unlikely(divisionLeadsToFPE(a, b))) return 0;
...
minimal negative number by minus one.
  ...
  FunctionDocumentation::Arguments arguments = {{"a", "Left hand operand.", {"(U)Int*"}}, {"b", "Right hand operand.", {"(U)Int*"}}};
  FunctionDocumentation::ReturnedValue returned_value = {"Result of integer division of a and b, or zero."};
  FunctionDocumentation::Examples examples = {{"Integer division by zero", "SELECT intDivOrZero(1, 0)", "0"},
