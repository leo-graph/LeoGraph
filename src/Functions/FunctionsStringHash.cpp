...
struct Hash {
  ...
  static ALWAYS_INLINE inline UInt64 shingleHash(UInt64 crc, const UInt8 *start, size_t size) {
    if (size & 1) {
      ...
      if constexpr (CaseInsensitive) x |= 0x20u;  /// see toLowerIfAlphaASCII from StringUtils.h
    ...
    if (size & 2) {
      ...
      if constexpr (CaseInsensitive) x |= 0x2020u;
    ...
    if (size & 4) {
      ...
      if constexpr (CaseInsensitive) x |= 0x20202020u;
    ...
    while (size) {
      ...
      if constexpr (CaseInsensitive) x |= 0x2020202020202020u;
  ...
  static ALWAYS_INLINE inline UInt64 shingleHash(const std::vector<BytesRef> &shingle, size_t offset = 0) {
    ...
    for (size_t i = offset; i < shingle.size(); ++i) crc = shingleHash<CaseInsensitive>(crc, shingle[i].data, shingle[i].size);

    for (size_t i = 0; i < offset; ++i) crc = shingleHash<CaseInsensitive>(crc, shingle[i].data, shingle[i].size);
...
struct SimHashImpl {
  ...
  static ALWAYS_INLINE inline void updateFingerVector(Int64 *finger_vec, UInt64 hash_value) {
    for (size_t i = 0; i < 64; ++i) finger_vec[i] += (hash_value & (1ULL << i)) ? 1 : -1;
  }
  ...
  static ALWAYS_INLINE inline UInt64 ngramHashASCII(const UInt8 *data, size_t size, size_t shingle_size) {
    if (size < shingle_size) return Hash::shingleHash<CaseInsensitive>(-1ULL, data, size);
  ...
  static ALWAYS_INLINE inline UInt64 ngramHashUTF8(const UInt8 *data, size_t size, size_t shingle_size) {
    ...
    for (size_t i = 0; i < shingle_size; ++i) {
      if (word_end >= end) return Hash::shingleHash<CaseInsensitive>(-1ULL, data, size);
  ...
  static ALWAYS_INLINE inline UInt64 wordShingleHash(const UInt8 *data, size_t size, size_t shingle_size) {
    ...
    while (start < end && words.size() < shingle_size) {
      ...
      if (length >= min_word_size) words.emplace_back(BytesRef{word_start, length});
    }
    ...
    while (start < end) {
      ...
      if (length < min_word_size) continue;
      ...
      ++offset;
      if (offset >= shingle_size) offset = 0;
...
struct MinHashImpl {
  ...
  struct Heap {
    void update(UInt64 hash, BytesRef ref, size_t limit) {
      if (values.contains(hash)) return;

      values[hash] = ref;

      if (values.size() > limit) values.erase(values.begin());
    }
    ...
    UInt64 getHash() {
      ...
      UInt64 res = 0;
      for (auto it = values.begin(); it != values.end(); ++it) res = CityHash_v1_0_2::Hash128to64(CityHash_v1_0_2::uint128(res, it->first));
  ...
  static ALWAYS_INLINE inline void ngramHashASCII(MinHeap &min_heap, MaxHeap &max_heap, const UInt8 *data, size_t size, size_t shingle_size,
                                                  size_t heap_size) {
  ...
  static ALWAYS_INLINE inline void ngramHashUTF8(MinHeap &min_heap, MaxHeap &max_heap, const UInt8 *data, size_t size, size_t shingle_size,
                                                 size_t heap_size) {
    const UInt8 *start = data;
    const UInt8 *end = data + size;
  ...
  static ALWAYS_INLINE inline void wordShingleHash(MinHeap &min_heap, MaxHeap &max_heap, const UInt8 *data, size_t size,
                                                   size_t shingle_size, size_t heap_size) {
    const UInt8 *start = data;
    const UInt8 *end = data + size;
  ...
  static void apply(const ColumnString::Chars &data, const ColumnString::Offsets &offsets, size_t shingle_size, size_t heap_size,
                    PaddedPODArray<UInt64> *res1, PaddedPODArray<UInt64> *res2, ColumnTuple *res1_strings, ColumnTuple *res2_strings,
                    size_t input_rows_count) {
    MinHeap min_heap;
    MaxHeap max_heap;
    ...
    for (size_t i = 0; i < input_rows_count; ++i) {
      ...
      if (res1) (*res1)[i] = min_heap.getHash();
      if (res2) (*res2)[i] = max_heap.getHash();

      if (res1_strings) min_heap.fill(*res1_strings);
      if (res2_strings) max_heap.fill(*res2_strings);
    }
  }
};
...
using FunctionNgramSimHashCaseInsensitive = FunctionsStringHash<SimHashImpl<false, true, true>, NameNgramSimHashCaseInsensitive, true>;
...
using FunctionNgramMinHashCaseInsensitive = FunctionsStringHash<MinHashImpl<false, true, true>, NameNgramMinHashCaseInsensitive, false>;
...
using FunctionWordShingleMinHashUTF8 = FunctionsStringHash<MinHashImpl<true, false, false>, NameWordShingleMinHashUTF8, false>;
...
using FunctionWordShingleMinHashArgUTF8 = FunctionsStringHash<MinHashImpl<true, false, false>, NameWordShingleMinHashArgUTF8, false, true>;
...
The smaller the [Hamming distance](https://en.wikipedia.org/wiki/Hamming_distance) of the calculated `simhashes` of two strings, the more likely these strings are the same.
  ...
  FunctionDocumentation::Examples ngramSimHash_examples = {{"Usage example", "SELECT ngramSimHash('ClickHouse') AS Hash;",
                                                            R"(
┌───────Hash─┐
│ 1627567969 │
└────────────┘
  ...
  FunctionDocumentation ngramSimHash_documentation = {
      ngramSimHash_description,    ngramSimHash_syntax,   ngramSimHash_arguments,     {},
      ngramSimHash_returned_value, ngramSimHash_examples, ngramSimHash_introduced_in, ngramSimHash_category};
...
The smaller the [Hamming distance](https://en.wikipedia.org/wiki/Hamming_distance) of the calculated `simhashes` of two strings, the more likely these strings are the same.
  ...
  FunctionDocumentation::Arguments ngramSimHashCaseInsensitive_arguments = {
      ...
      {"ngramsize", "Optional. The size of an n-gram, any number from `1` to `25`. The default value is `3`.", {"UInt8"}}};
  FunctionDocumentation::ReturnedValue ngramSimHashCaseInsensitive_returned_value = {"Hash value. [UInt64](../data-types/int-uint.md).",
                                                                                     {"UInt64"}};
  FunctionDocumentation::Examples ngramSimHashCaseInsensitive_examples = {{"Usage example",
...
└───────────┘
  ...
  FunctionDocumentation ngramSimHashCaseInsensitive_documentation = {
      ngramSimHashCaseInsensitive_description,    ngramSimHashCaseInsensitive_syntax,
      ngramSimHashCaseInsensitive_arguments,      {},
      ngramSimHashCaseInsensitive_returned_value, ngramSimHashCaseInsensitive_examples,
      ngramSimHashCaseInsensitive_introduced_in,  ngramSimHashCaseInsensitive_category};
...
The smaller the [Hamming distance](https://en.wikipedia.org/wiki/Hamming_distance) of the calculated `simhashes` of two strings, the more likely these strings are the same.
  ...
  FunctionDocumentation::Examples ngramSimHashUTF8_examples = {{"Usage example", "SELECT ngramSimHashUTF8('ClickHouse') AS Hash;",
                                                                R"(
┌───────Hash─┐
│ 1628157797 │
└────────────┘
  ...
  FunctionDocumentation ngramSimHashUTF8_documentation = {
      ngramSimHashUTF8_description,    ngramSimHashUTF8_syntax,   ngramSimHashUTF8_arguments,     {},
      ngramSimHashUTF8_returned_value, ngramSimHashUTF8_examples, ngramSimHashUTF8_introduced_in, ngramSimHashUTF8_category};
...
Can be used for detection of semi-duplicate strings with [bitHammingDistance](../functions/bit-functions.md/#bitHammingDistance). The smaller is the [Hamming Distance](https://en.wikipedia.org/wiki/Hamming_distance) of the calculated `simhashes` of two strings, the more likely these strings are the same.
  ...
  FunctionDocumentation::Examples ngramSimHashCaseInsensitiveUTF8_examples = {
      {"Usage example", "SELECT ngramSimHashCaseInsensitiveUTF8('ClickHouse') AS Hash;",
       R"(
┌───────Hash─┐
│ 1636742693 │
└────────────┘
  ...
  FunctionDocumentation ngramSimHashCaseInsensitiveUTF8_documentation = {
      ngramSimHashCaseInsensitiveUTF8_description,    ngramSimHashCaseInsensitiveUTF8_syntax,
      ngramSimHashCaseInsensitiveUTF8_arguments,      {},
      ngramSimHashCaseInsensitiveUTF8_returned_value, ngramSimHashCaseInsensitiveUTF8_examples,
      ngramSimHashCaseInsensitiveUTF8_introduced_in,  ngramSimHashCaseInsensitiveUTF8_category};
...
The smaller the [Hamming distance](https://en.wikipedia.org/wiki/Hamming_distance) of the calculated `simhashes` of two strings, the more likely these strings are the same.
  ...
  FunctionDocumentation::Examples wordShingleSimHash_examples = {
      {"Usage example",
       "SELECT wordShingleSimHash('ClickHouse® is a column-oriented database management system (DBMS) for online analytical processing of "
       "queries (OLAP).') AS Hash;",
       R"(
┌───────Hash─┐
│ 2328277067 │
└────────────┘
  ...
  FunctionDocumentation wordShingleSimHash_documentation = {
      wordShingleSimHash_description,    wordShingleSimHash_syntax,   wordShingleSimHash_arguments,     {},
      wordShingleSimHash_returned_value, wordShingleSimHash_examples, wordShingleSimHash_introduced_in, wordShingleSimHash_category};
...
The smaller the [Hamming distance](https://en.wikipedia.org/wiki/Hamming_distance) of the calculated `simhashes` of two strings, the more likely these strings are the same.
  ...
  FunctionDocumentation::Examples wordShingleSimHashCaseInsensitive_examples = {
      {"Usage example",
       "SELECT wordShingleSimHashCaseInsensitive('ClickHouse® is a column-oriented database management system (DBMS) for online analytical "
       "processing of queries (OLAP).') AS Hash;",
       R"(
┌───────Hash─┐
│ 2194812424 │
└────────────┘
  ...
  FunctionDocumentation wordShingleSimHashCaseInsensitive_documentation = {
      wordShingleSimHashCaseInsensitive_description,    wordShingleSimHashCaseInsensitive_syntax,
      wordShingleSimHashCaseInsensitive_arguments,      {},
      wordShingleSimHashCaseInsensitive_returned_value, wordShingleSimHashCaseInsensitive_examples,
      wordShingleSimHashCaseInsensitive_introduced_in,  wordShingleSimHashCaseInsensitive_category};
...
The smaller the [Hamming distance](https://en.wikipedia.org/wiki/Hamming_distance) of the calculated `simhashes` of two strings, the more likely these strings are the same.
  ...
  FunctionDocumentation::Examples wordShingleSimHashUTF8_examples = {
      {"Usage example",
       "SELECT wordShingleSimHashUTF8('ClickHouse® is a column-oriented database management system (DBMS) for online analytical processing "
       "of queries (OLAP).') AS Hash;",
       R"(
┌───────Hash─┐
│ 2328277067 │
└────────────┘
  ...
  FunctionDocumentation wordShingleSimHashUTF8_documentation = {wordShingleSimHashUTF8_description,    wordShingleSimHashUTF8_syntax,
                                                                wordShingleSimHashUTF8_arguments,      {},
                                                                wordShingleSimHashUTF8_returned_value, wordShingleSimHashUTF8_examples,
                                                                wordShingleSimHashUTF8_introduced_in,  wordShingleSimHashUTF8_category};
...
The smaller the [Hamming Distance](https://en.wikipedia.org/wiki/Hamming_distance) of the calculated `simhashes` of two strings, the more likely these strings are the same.
  ...
  FunctionDocumentation::Syntax wordShingleSimHashCaseInsensitiveUTF8_syntax =
      "wordShingleSimHashCaseInsensitiveUTF8(string[, shinglesize])";
  FunctionDocumentation::Arguments wordShingleSimHashCaseInsensitiveUTF8_arguments = {
      {"string", "String for which to compute the hash.", {"String"}},
      {"shinglesize", "Optional. The size of a word shingle, any number from `1` to `25`. The default value is `3`.", {"UInt8"}}};
  FunctionDocumentation::ReturnedValue wordShingleSimHashCaseInsensitiveUTF8_returned_value = {"Returns the computed hash value.",
                                                                                               {"UInt64"}};
  FunctionDocumentation::Examples wordShingleSimHashCaseInsensitiveUTF8_examples = {
      {"Usage example",
       "SELECT wordShingleSimHashCaseInsensitiveUTF8('ClickHouse® is a column-oriented database management system (DBMS) for online "
       "analytical processing of queries (OLAP).') AS Hash;",
       R"(
┌───────Hash─┐
│ 2194812424 │
└────────────┘
  ...
  FunctionDocumentation wordShingleSimHashCaseInsensitiveUTF8_documentation = {
      wordShingleSimHashCaseInsensitiveUTF8_description,    wordShingleSimHashCaseInsensitiveUTF8_syntax,
      wordShingleSimHashCaseInsensitiveUTF8_arguments,      {},
      wordShingleSimHashCaseInsensitiveUTF8_returned_value, wordShingleSimHashCaseInsensitiveUTF8_examples,
      wordShingleSimHashCaseInsensitiveUTF8_introduced_in,  wordShingleSimHashCaseInsensitiveUTF8_category};
...
For two strings, if the returned hashes are the same for both strings, then those strings are the same.
  ...
  FunctionDocumentation::Arguments ngramMinHash_arguments = {
      ...
      {"ngramsize", "Optional. The size of an n-gram, any number from `1` to `25`. The default value is `3`.", {"UInt8"}},
      {"hashnum",
       "Optional. The number of minimum and maximum hashes used to calculate the result, any number from `1` to `25`. The default value is "
       "`6`.",
       {"UInt8"}}};
  FunctionDocumentation::ReturnedValue ngramMinHash_returned_value = {"Returns a tuple with two hashes — the minimum and the maximum.",
                                                                      {"Tuple"}};
  FunctionDocumentation::Examples ngramMinHash_examples = {{"Usage example", "SELECT ngramMinHash('ClickHouse') AS Tuple;",
                                                            R"(
┌─Tuple──────────────────────────────────────┐
...
└────────────────────────────────────────────┘
  ...
  FunctionDocumentation ngramMinHash_documentation = {
      ngramMinHash_description,    ngramMinHash_syntax,   ngramMinHash_arguments,     {},
      ngramMinHash_returned_value, ngramMinHash_examples, ngramMinHash_introduced_in, ngramMinHash_category};
...
For two strings, if the returned hashes are the same for both strings, then those strings are the same.
  ...
  FunctionDocumentation::Arguments ngramMinHashCaseInsensitive_arguments = {
      ...
      {"ngramsize",
       "The size of an n-gram. Optional. Possible values: any number from `1` to `25`. Default value: `3`. "
       "[UInt8](../data-types/int-uint.md)."},
      {"hashnum",
       "The number of minimum and maximum hashes used to calculate the result. Optional. Possible values: any number from `1` to `25`. "
       "Default value: `6`. [UInt8](../data-types/int-uint.md)."}};
  FunctionDocumentation::ReturnedValue ngramMinHashCaseInsensitive_returned_value = {
      "Tuple with two hashes — the minimum and the maximum. [Tuple](../data-types/tuple.md)([UInt64](../data-types/int-uint.md), "
      "[UInt64](../data-types/int-uint.md)).",
      {"Tuple"}};
  FunctionDocumentation::Examples ngramMinHashCaseInsensitive_examples = {{"Usage example",
...
└────────────────────────────────────────────┘
  ...
  FunctionDocumentation ngramMinHashCaseInsensitive_documentation = {
      ngramMinHashCaseInsensitive_description,    ngramMinHashCaseInsensitive_syntax,
      ngramMinHashCaseInsensitive_arguments,      {},
      ngramMinHashCaseInsensitive_returned_value, ngramMinHashCaseInsensitive_examples,
      ngramMinHashCaseInsensitive_introduced_in,  ngramMinHashCaseInsensitive_category};
...
For two strings, if the returned hashes are the same for both strings, then those strings are the same.
  ...
  FunctionDocumentation::Arguments ngramMinHashUTF8_arguments = {
      ...
      {"hashnum",
       ...
       "`6`.",
       {"UInt8"}}};
  FunctionDocumentation::ReturnedValue ngramMinHashUTF8_returned_value = {"Returns a tuple with two hashes — the minimum and the maximum.",
                                                                          {"Tuple"}};
  FunctionDocumentation::Examples ngramMinHashUTF8_examples = {{"Usage example", "SELECT ngramMinHashUTF8('ClickHouse') AS Tuple;",
                                                                R"(
┌─Tuple──────────────────────────────────────┐
...
└────────────────────────────────────────────┘
  ...
  FunctionDocumentation ngramMinHashUTF8_documentation = {
      ngramMinHashUTF8_description,    ngramMinHashUTF8_syntax,   ngramMinHashUTF8_arguments,     {},
      ngramMinHashUTF8_returned_value, ngramMinHashUTF8_examples, ngramMinHashUTF8_introduced_in, ngramMinHashUTF8_category};
...
For two strings, if the returned hashes are the same for both strings, then those strings are the same.
  ...
  FunctionDocumentation::Arguments ngramMinHashCaseInsensitiveUTF8_arguments = {
      ...
      {"hashnum",
       ...
       "`6`.",
       {"UInt8"}}};
  FunctionDocumentation::ReturnedValue ngramMinHashCaseInsensitiveUTF8_returned_value = {
  ...
  FunctionDocumentation::Examples ngramMinHashCaseInsensitiveUTF8_examples = {
      {"Usage example", "SELECT ngramMinHashCaseInsensitiveUTF8('ClickHouse') AS Tuple;",
       R"(
┌─Tuple───────────────────────────────────────┐
...
└─────────────────────────────────────────────┘
  ...
  FunctionDocumentation ngramMinHashCaseInsensitiveUTF8_documentation = {
      ngramMinHashCaseInsensitiveUTF8_description,    ngramMinHashCaseInsensitiveUTF8_syntax,
      ngramMinHashCaseInsensitiveUTF8_arguments,      {},
...
For two strings, if the returned hashes are the same for both strings, then those strings are the same.
  ...
  FunctionDocumentation::Arguments wordShingleMinHash_arguments = {
      ...
      {"hashnum",
       ...
       "`6`.",
       {"UInt8"}}};
  FunctionDocumentation::ReturnedValue wordShingleMinHash_returned_value = {
  ...
  FunctionDocumentation::Examples wordShingleMinHash_examples = {
      {"Usage example",
       "SELECT wordShingleMinHash('ClickHouse® is a column-oriented database management system (DBMS) for online analytical processing of "
       "queries (OLAP).') AS Tuple;",
       R"(
┌─Tuple──────────────────────────────────────┐
...
└────────────────────────────────────────────┘
  ...
  FunctionDocumentation wordShingleMinHash_documentation = {
      wordShingleMinHash_description,    wordShingleMinHash_syntax,   wordShingleMinHash_arguments,     {},
      wordShingleMinHash_returned_value, wordShingleMinHash_examples, wordShingleMinHash_introduced_in, wordShingleMinHash_category};
...
For two strings, if the returned hashes are the same for both strings, then those strings are the same.
  ...
  FunctionDocumentation::Syntax wordShingleMinHashCaseInsensitive_syntax =
  ...
  FunctionDocumentation::Arguments wordShingleMinHashCaseInsensitive_arguments = {
      ...
      {"hashnum",
       ...
       "`6`.",
       {"UInt8"}}};
  FunctionDocumentation::ReturnedValue wordShingleMinHashCaseInsensitive_returned_value = {
      "Returns a tuple with two hashes — the minimum and the maximum.", {"Tuple(UInt64, UInt64)"}};
  FunctionDocumentation::Examples wordShingleMinHashCaseInsensitive_examples = {
      {"Usage example",
       "SELECT wordShingleMinHashCaseInsensitive('ClickHouse® is a column-oriented database management system (DBMS) for online analytical "
       "processing of queries (OLAP).') AS Tuple;",
       R"(
┌─Tuple─────────────────────────────────────┐
...
└───────────────────────────────────────────┘
  ...
  FunctionDocumentation wordShingleMinHashCaseInsensitive_documentation = {
      wordShingleMinHashCaseInsensitive_description,    wordShingleMinHashCaseInsensitive_syntax,
      wordShingleMinHashCaseInsensitive_arguments,      {},
      wordShingleMinHashCaseInsensitive_returned_value, wordShingleMinHashCaseInsensitive_examples,
      wordShingleMinHashCaseInsensitive_introduced_in,  wordShingleMinHashCaseInsensitive_category};
...
For two strings, if the returned hashes are the same for both strings, then those strings are the same.
  ...
  FunctionDocumentation::Arguments wordShingleMinHashUTF8_arguments = {
      ...
      {"hashnum",
       ...
       "`6`.",
       {"UInt8"}}};
  FunctionDocumentation::ReturnedValue wordShingleMinHashUTF8_returned_value = {
      "Returns a tuple with two hashes — the minimum and the maximum.", {"Tuple(UInt64, UInt64)"}};
  FunctionDocumentation::Examples wordShingleMinHashUTF8_examples = {
      {"Usage example",
       "SELECT wordShingleMinHashUTF8('ClickHouse® is a column-oriented database management system (DBMS) for online analytical processing "
       "of queries (OLAP).') AS Tuple;",
       R"(
┌─Tuple──────────────────────────────────────┐
│ (16452112859864147620,5844417301642981317) │
└────────────────────────────────────────────┘
  ...
  FunctionDocumentation wordShingleMinHashUTF8_documentation = {wordShingleMinHashUTF8_description,    wordShingleMinHashUTF8_syntax,
                                                                wordShingleMinHashUTF8_arguments,      {},
                                                                wordShingleMinHashUTF8_returned_value, wordShingleMinHashUTF8_examples,
                                                                wordShingleMinHashUTF8_introduced_in,  wordShingleMinHashUTF8_category};
...
For two strings, if the returned hashes are the same for both strings, then those strings are the same.
  ...
  FunctionDocumentation::Syntax wordShingleMinHashCaseInsensitiveUTF8_syntax =
  ...
  FunctionDocumentation::Arguments wordShingleMinHashCaseInsensitiveUTF8_arguments = {
      ...
      {"hashnum",
       ...
       "`6`.",
       {"UInt8"}}};
  FunctionDocumentation::ReturnedValue wordShingleMinHashCaseInsensitiveUTF8_returned_value = {
      "Returns a tuple with two hashes — the minimum and the maximum.", {"Tuple(UInt64, UInt64)"}};
  FunctionDocumentation::Examples wordShingleMinHashCaseInsensitiveUTF8_examples = {
      {"Usage example",
       "SELECT wordShingleMinHashCaseInsensitiveUTF8('ClickHouse® is a column-oriented database management system (DBMS) for online "
       "analytical processing of queries (OLAP).') AS Tuple;",
       R"(
┌─Tuple─────────────────────────────────────┐
│ (3065874883688416519,1634050779997673240) │
└───────────────────────────────────────────┘
  ...
  FunctionDocumentation wordShingleMinHashCaseInsensitiveUTF8_documentation = {
      wordShingleMinHashCaseInsensitiveUTF8_description,    wordShingleMinHashCaseInsensitiveUTF8_syntax,
      wordShingleMinHashCaseInsensitiveUTF8_arguments,      {},
      wordShingleMinHashCaseInsensitiveUTF8_returned_value, wordShingleMinHashCaseInsensitiveUTF8_examples,
...
It is case sensitive.
  ...
  FunctionDocumentation::Arguments ngramMinHashArg_arguments = {
      ...
      {"hashnum",
       ...
       "`6`.",
       {"UInt8"}}};
  FunctionDocumentation::ReturnedValue ngramMinHashArg_returned_value = {"Returns a tuple with two tuples with `hashnum` n-grams each.",
                                                                         {"Tuple(String)"}};
  FunctionDocumentation::Examples ngramMinHashArg_examples = {{"Usage example", "SELECT ngramMinHashArg('ClickHouse') AS Tuple;",
                                                               R"(
┌─Tuple─────────────────────────────────────────────────────────────────────────┐
...
└───────────────────────────────────────────────────────────────────────────────┘
  ...
  FunctionDocumentation ngramMinHashArg_documentation = {
      ngramMinHashArg_description,    ngramMinHashArg_syntax,   ngramMinHashArg_arguments,     {},
      ngramMinHashArg_returned_value, ngramMinHashArg_examples, ngramMinHashArg_introduced_in, ngramMinHashArg_category};
...
It is case insensitive.
  ...
  FunctionDocumentation::Arguments ngramMinHashArgCaseInsensitive_arguments = {
      ...
      {"hashnum",
       ...
       "`6`.",
       {"UInt8"}}};
  FunctionDocumentation::ReturnedValue ngramMinHashArgCaseInsensitive_returned_value = {
  ...
  FunctionDocumentation::Examples ngramMinHashArgCaseInsensitive_examples = {
      {"Usage example", "SELECT ngramMinHashArgCaseInsensitive('ClickHouse') AS Tuple;",
       R"(
┌─Tuple─────────────────────────────────────────────────────────────────────────┐
...
└───────────────────────────────────────────────────────────────────────────────┘
  ...
  FunctionDocumentation ngramMinHashArgCaseInsensitive_documentation = {
      ngramMinHashArgCaseInsensitive_description,    ngramMinHashArgCaseInsensitive_syntax,
      ngramMinHashArgCaseInsensitive_arguments,      {},
      ngramMinHashArgCaseInsensitive_returned_value, ngramMinHashArgCaseInsensitive_examples,
      ngramMinHashArgCaseInsensitive_introduced_in,  ngramMinHashArgCaseInsensitive_category};
...
It is case sensitive.
  ...
  FunctionDocumentation::Arguments ngramMinHashArgUTF8_arguments = {
      ...
      {"hashnum",
       ...
       "`6`.",
       {"UInt8"}}};
  FunctionDocumentation::ReturnedValue ngramMinHashArgUTF8_returned_value = {"Returns a tuple with two tuples with `hashnum` n-grams each.",
                                                                             {"Tuple(Tuple(String))"}};
  FunctionDocumentation::Examples ngramMinHashArgUTF8_examples = {{"Usage example", "SELECT ngramMinHashArgUTF8('ClickHouse') AS Tuple;",
                                                                   R"(
┌─Tuple─────────────────────────────────────────────────────────────────────────┐
...
└───────────────────────────────────────────────────────────────────────────────┘
  ...
  FunctionDocumentation ngramMinHashArgUTF8_documentation = {
      ngramMinHashArgUTF8_description,    ngramMinHashArgUTF8_syntax,   ngramMinHashArgUTF8_arguments,     {},
      ngramMinHashArgUTF8_returned_value, ngramMinHashArgUTF8_examples, ngramMinHashArgUTF8_introduced_in, ngramMinHashArgUTF8_category};
...
It is case insensitive.
  ...
  FunctionDocumentation::Syntax ngramMinHashArgCaseInsensitiveUTF8_syntax =
  ...
  FunctionDocumentation::Arguments ngramMinHashArgCaseInsensitiveUTF8_arguments = {
      ...
      {"hashnum",
       ...
       "`6`.",
       {"UInt8"}}};
  FunctionDocumentation::ReturnedValue ngramMinHashArgCaseInsensitiveUTF8_returned_value = {
      "Returns a tuple with two tuples with `hashnum` n-grams each.", {"Tuple(Tuple(String))"}};
  FunctionDocumentation::Examples ngramMinHashArgCaseInsensitiveUTF8_examples = {
      {"Usage example", "SELECT ngramMinHashArgCaseInsensitiveUTF8('ClickHouse') AS Tuple;",
       R"(
┌─Tuple─────────────────────────────────────────────────────────────────────────┐
...
└───────────────────────────────────────────────────────────────────────────────┘
  ...
  FunctionDocumentation ngramMinHashArgCaseInsensitiveUTF8_documentation = {
      ngramMinHashArgCaseInsensitiveUTF8_description,    ngramMinHashArgCaseInsensitiveUTF8_syntax,
      ngramMinHashArgCaseInsensitiveUTF8_arguments,      {},
      ngramMinHashArgCaseInsensitiveUTF8_returned_value, ngramMinHashArgCaseInsensitiveUTF8_examples,
      ngramMinHashArgCaseInsensitiveUTF8_introduced_in,  ngramMinHashArgCaseInsensitiveUTF8_category};
...
It is case sensitive.
  ...
  FunctionDocumentation::Arguments wordShingleMinHashArg_arguments = {
      ...
      {"hashnum",
       ...
       "`6`.",
       {"UInt8"}}};
  FunctionDocumentation::ReturnedValue wordShingleMinHashArg_returned_value = {
  ...
  FunctionDocumentation::Examples wordShingleMinHashArg_examples = {
      {"Usage example",
       "SELECT wordShingleMinHashArg('ClickHouse® is a column-oriented database management system (DBMS) for online analytical processing "
       "of queries (OLAP).', 1, 3) AS Tuple;",
       R"(
┌─Tuple─────────────────────────────────────────────────────────────────┐
...
└───────────────────────────────────────────────────────────────────────┘
  ...
  FunctionDocumentation wordShingleMinHashArg_documentation = {wordShingleMinHashArg_description,    wordShingleMinHashArg_syntax,
                                                               wordShingleMinHashArg_arguments,      {},
                                                               wordShingleMinHashArg_returned_value, wordShingleMinHashArg_examples,
                                                               wordShingleMinHashArg_introduced_in,  wordShingleMinHashArg_category};
...
It is case insensitive.
  ...
  FunctionDocumentation::Syntax wordShingleMinHashArgCaseInsensitive_syntax =
  ...
  FunctionDocumentation::Arguments wordShingleMinHashArgCaseInsensitive_arguments = {
      ...
      {"hashnum",
       ...
       "`6`.",
       {"UInt8"}}};
  FunctionDocumentation::ReturnedValue wordShingleMinHashArgCaseInsensitive_returned_value = {
      "Returns a tuple with two tuples with `hashnum` word shingles each.", {"Tuple(Tuple(String))"}};
  FunctionDocumentation::Examples wordShingleMinHashArgCaseInsensitive_examples = {
      {"Usage example",
       "SELECT wordShingleMinHashArgCaseInsensitive('ClickHouse® is a column-oriented database management system (DBMS) for online "
       "analytical processing of queries (OLAP).', 1, 3) AS Tuple;",
       R"(
┌─Tuple──────────────────────────────────────────────────────────────────┐
...
└────────────────────────────────────────────────────────────────────────┘
  ...
  FunctionDocumentation wordShingleMinHashArgCaseInsensitive_documentation = {
      wordShingleMinHashArgCaseInsensitive_description,    wordShingleMinHashArgCaseInsensitive_syntax,
      wordShingleMinHashArgCaseInsensitive_arguments,      {},
      wordShingleMinHashArgCaseInsensitive_returned_value, wordShingleMinHashArgCaseInsensitive_examples,
...
It is case sensitive.
  ...
  FunctionDocumentation::Arguments wordShingleMinHashArgUTF8_arguments = {
      ...
      {"hashnum",
       ...
       "`6`.",
       {"UInt8"}}};
  FunctionDocumentation::ReturnedValue wordShingleMinHashArgUTF8_returned_value = {
      "Returns a tuple with two tuples with `hashnum` word shingles each.", {"Tuple(Tuple(String))"}};
  FunctionDocumentation::Examples wordShingleMinHashArgUTF8_examples = {
      {"Usage example",
       "SELECT wordShingleMinHashArgUTF8('ClickHouse® is a column-oriented database management system (DBMS) for online analytical "
       "processing of queries (OLAP).', 1, 3) AS Tuple;",
       R"(
┌─Tuple─────────────────────────────────────────────────────────────────┐
│ (('OLAP','database','analytical'),('online','oriented','processing')) │
└───────────────────────────────────────────────────────────────────────┘
  ...
  FunctionDocumentation wordShingleMinHashArgUTF8_documentation = {
      wordShingleMinHashArgUTF8_description,    wordShingleMinHashArgUTF8_syntax,
      wordShingleMinHashArgUTF8_arguments,      {},
      wordShingleMinHashArgUTF8_returned_value, wordShingleMinHashArgUTF8_examples,
      wordShingleMinHashArgUTF8_introduced_in,  wordShingleMinHashArgUTF8_category};
...
It is case insensitive.
  ...
  FunctionDocumentation::Syntax wordShingleMinHashArgCaseInsensitiveUTF8_syntax =
  ...
  FunctionDocumentation::Arguments wordShingleMinHashArgCaseInsensitiveUTF8_arguments = {
      ...
      {"hashnum",
       ...
       "`6`.",
       {"UInt8"}}};
  FunctionDocumentation::ReturnedValue wordShingleMinHashArgCaseInsensitiveUTF8_returned_value = {
      "Returns a tuple with two tuples with `hashnum` word shingles each.", {"Tuple(Tuple(String))"}};
  FunctionDocumentation::Examples wordShingleMinHashArgCaseInsensitiveUTF8_examples = {
      {"Usage example",
       "SELECT wordShingleMinHashArgCaseInsensitiveUTF8('ClickHouse® is a column-oriented database management system (DBMS) for online "
       "analytical processing of queries (OLAP).', 1, 3) AS Tuple;",
       R"(
┌─Tuple──────────────────────────────────────────────────────────────────┐
│ (('queries','database','analytical'),('oriented','processing','DBMS')) │
└────────────────────────────────────────────────────────────────────────┘
  ...
  FunctionDocumentation wordShingleMinHashArgCaseInsensitiveUTF8_documentation = {
      wordShingleMinHashArgCaseInsensitiveUTF8_description,    wordShingleMinHashArgCaseInsensitiveUTF8_syntax,
      wordShingleMinHashArgCaseInsensitiveUTF8_arguments,      {},
      wordShingleMinHashArgCaseInsensitiveUTF8_returned_value, wordShingleMinHashArgCaseInsensitiveUTF8_examples,
