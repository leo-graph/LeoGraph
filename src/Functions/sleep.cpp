...
enum class FunctionSleepVariant : uint8_t { PerBlock, PerRow };
...
class FunctionSleep : public IFunction {
 ...
 public:
  ...
  static FunctionPtr create(const char *name, FunctionSleepVariant variant, ContextPtr context) {
    return std::make_shared<FunctionSleep>(name, variant, context->getSettingsRef()[Setting::function_sleep_max_microseconds_per_block],
  ...
  DataTypePtr getReturnTypeImpl(const ColumnsWithTypeAndName &arguments) const override {
    ...
    FunctionArgumentDescriptors mandatory_args{{"seconds", is_float_or_native_uint, nullptr, "UInt* or Float*"}};
    validateFunctionArguments(getName(), arguments, mandatory_args);

    return std::make_shared<DataTypeUInt8>();
  }
  ColumnPtr executeImplDryRun(const ColumnsWithTypeAndName &arguments, const DataTypePtr &result_type,
  ...
  ColumnPtr execute(const ColumnsWithTypeAndName &arguments, const DataTypePtr &result_type, bool dry_run) const {
    ...
    if (!isColumnConst(*col)) throw Exception(ErrorCodes::ILLEGAL_COLUMN, "The argument of function {} must be constant.", getName());
    ...
    if (size > 0) {
      ...
      if (max_microseconds && microseconds > max_microseconds)
        throw Exception(ErrorCodes::TOO_SLOW, "The maximum sleep time is {} microseconds. Requested: {} microseconds", max_microseconds,
                        microseconds);
      ...
      if (!dry_run) {
        ...
        if (max_microseconds && microseconds > max_microseconds)
          throw Exception(ErrorCodes::TOO_SLOW,
                          "The maximum sleep time is {} microseconds. Requested: {} microseconds per block (of size {})", max_microseconds,
                          microseconds, size);

        UInt64 elapsed = 0;
        while (elapsed < microseconds) {
          UInt64 sleep_time = microseconds - elapsed;
          if (query_status) sleep_time = std::min(sleep_time, /* 1 second */ static_cast<UInt64>(1000000));

          sleepForMicroseconds(sleep_time);
          elapsed += sleep_time;

          if (query_status && !query_status->checkTimeLimit()) break;
        }
...
For security reasons, the function can only be executed in the default user profile (with `allow_sleep` enabled).
  ...
  FunctionDocumentation::Arguments arguments_sleep = {{"seconds",
                                                       "The number of seconds to pause the query execution to a maximum of 3 seconds. It "
                                                       "can be a floating-point value to specify fractional seconds.",
                                                       {"const UInt*", "const Float*"}}};
...
1 row in set. Elapsed: 2.012 sec.
  ...
  FunctionDocumentation documentation_sleep = {description_sleep,    syntax_sleep,   arguments_sleep,     {},
                                               returned_value_sleep, examples_sleep, introduced_in_sleep, category_sleep};

  factory.registerFunction(
      "sleep", [](ContextPtr ctx) { return FunctionSleep::create("sleep", FunctionSleepVariant::PerBlock, std::move(ctx)); },
      documentation_sleep);
...
:::
  ...
  FunctionDocumentation::Arguments arguments_sleepEachRow = {
      {"seconds",
       "The number of seconds to pause the query execution for each row in the result set to a maximum of 3 seconds. It can be a "
       "floating-point value to specify fractional seconds.",
       {"const UInt*", "const Float*"}}};
...
└────────┴───────────────────┘
  ...
  FunctionDocumentation documentation_sleepEachRow = {
      description_sleepEachRow,    syntax_sleepEachRow,   arguments_sleepEachRow,     {},
      returned_value_sleepEachRow, examples_sleepEachRow, introduced_in_sleepEachRow, category_sleepEachRow};

  factory.registerFunction(
      "sleepEachRow", [](ContextPtr ctx) { return FunctionSleep::create("sleepEachRow", FunctionSleepVariant::PerRow, std::move(ctx)); },
