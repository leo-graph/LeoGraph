...
struct BitShiftLeftImpl {
  ...
  /// For String
  static ALWAYS_INLINE NO_SANITIZE_UNDEFINED void apply(const UInt8 *pos [[maybe_unused]], const UInt8 *end [[maybe_unused]],
                                                        const B &b [[maybe_unused]], ColumnString::Chars &out_vec,
    ...
    if constexpr (is_big_int_v<B>)
      ...
      if (b < 0) throw Exception(ErrorCodes::ARGUMENT_OUT_OF_BOUND, "The number of shift positions needs to be a non-negative value");
      ...
      if (shift_left_bits)
        length = end + shift_left_bytes - begin +
                 1;  /// Moving to the left here will make a redundant byte to store the overflowing bits in the front
      else
        length = end + shift_left_bytes - begin;
      ...
      for (size_t i = 0; i < shift_left_bytes; ++i) *(out + i) = 0;
  ...
  /// For FixedString
  static ALWAYS_INLINE NO_SANITIZE_UNDEFINED void apply(const UInt8 *pos [[maybe_unused]], const UInt8 *end [[maybe_unused]],
                                                        const B &b [[maybe_unused]], ColumnFixedString::Chars &out_vec) {
    if constexpr (is_big_int_v<B>)
      throw Exception(ErrorCodes::NOT_IMPLEMENTED, "BitShiftLeft is not implemented for big integers as second argument");
    else {
  ...
  static llvm::Value *compile(llvm::IRBuilder<> &b, llvm::Value *left, llvm::Value *right, bool) {
    if (!left->getType()->isIntegerTy()) throw Exception(ErrorCodes::LOGICAL_ERROR, "BitShiftLeftImpl expected an integral type");
...
struct NameBitShiftLeft {
  static constexpr auto name = "bitShiftLeft";
};
...
On the contrary, a `String` value is extended with additional bytes, so no bits are lost.
  ...
  FunctionDocumentation::Arguments arguments = {{"a", "A value to shift.", {"(U)Int*", "String", "FixedString"}},
