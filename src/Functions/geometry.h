...
LineString<Point> getLineStringFromField(const Field &field) {
  ...
  for (const auto &tuple : array) linestring.push_back(getPointFromField<Point>(tuple));
  return linestring;
}
...
Ring<Point> getRingFromField(const Field &field) {
  Ring<Point> ring;
  const auto &array = field.safeGet<Array>();
  for (const auto &tuple : array) ring.push_back(getPointFromField<Point>(tuple));
  return ring;
}
...
MultiLineString<Point> getMultiLineStringFromField(const Field &field) {
  ...
  const auto &array = field.safeGet<Array>();
  for (const auto &tuple : array) multilinestring.push_back(getLineStringFromField<Point>(tuple));
...
MultiPolygon<Point> getMultiPolygonFromField(const Field &field) {
  ...
  const auto &array = field.safeGet<Array>();
  for (const auto &tuple : array) polygon.push_back(getPolygonFromField<Point>(tuple));
  return polygon;
}

enum class GeometryColumnType { Linestring = 0, MultiLinestring = 1, MultiPolygon = 2, Point = 3, Polygon = 4, Ring = 5, Null = 255 };
...
inline std::optional<GeometryColumnType> getGeometryColumnTypeFromDataType(const DataTypePtr &type) {
  ...
  auto is_point_type = [](const DataTypePtr &dt) -> bool {
    ...
    return elems.size() == 2 && WhichDataType(elems[0]).isFloat64() && WhichDataType(elems[1]).isFloat64();
  };

  /// Point: Tuple(Float64, Float64)
  if (is_point_type(type)) return GeometryColumnType::Point;

  /// Ring/LineString: Array(Tuple(Float64, Float64))
  const auto *array1 = typeid_cast<const DataTypeArray *>(type.get());
  if (!array1) return std::nullopt;

  if (is_point_type(array1->getNestedType())) return GeometryColumnType::Ring;
  ...
  if (!array2) return std::nullopt;

  if (is_point_type(array2->getNestedType())) return GeometryColumnType::Polygon;
  ...
  if (!array3) return std::nullopt;

  if (is_point_type(array3->getNestedType())) return GeometryColumnType::MultiPolygon;
...
template <>
struct magic_enum::customize::enum_range<DB::GeometryColumnType> {
...
class FunctionGeometry : public IFunction {
 public:
  ...
  static FunctionPtr create(ContextPtr) { return std::make_shared<FunctionGeometry>(); }
  ...
  DataTypePtr getReturnTypeImpl(const DataTypes &arguments) const override {
    if (arguments[0]->getName() != "Geometry" && !getGeometryColumnTypeFromDataType(arguments[0])) {
      throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT, "First argument of function {} should be Geometry, got {}", getName(),
                      arguments[0]->getName());
    }
  ...
  ColumnPtr executeImpl(const ColumnsWithTypeAndName &arguments, const DataTypePtr & /*result_type*/,
    ...
    if (column_variant) {
      ...
      for (size_t i = 0; i < input_rows_count; ++i) {
        ...
        if (!type) throw Exception(ErrorCodes::BAD_ARGUMENTS, "Unknown type of geometry {}", static_cast<Int32>(descriptors[i]));
