...
class FunctionToLowCardinality : public IFunction {
 public:
  ...
  DataTypePtr getReturnTypeImpl(const DataTypes &arguments) const override {
    if (arguments[0]->lowCardinality()) return arguments[0];
  ...
  ColumnPtr executeImpl(const ColumnsWithTypeAndName &arguments, const DataTypePtr &res_type, size_t /*input_rows_count*/) const override {
    ...
    if (arg.type->lowCardinality()) return arg.column;

    if (!res_type->lowCardinality())
      throw Exception(ErrorCodes::LOGICAL_ERROR, "Expected LowCardinality type as result type for toLowCardinality function, got: {}",
                      res_type->getName());
...
:::
  ...
  FunctionDocumentation::Arguments arguments = {{"expr",
                                                 ...
                                                 {"String", "FixedString", "Date", "DateTime", "(U)Int*", "Float*"}}};
  FunctionDocumentation::ReturnedValue returned_value = {"Returns the input value converted to the `LowCardinality` data type.",
                                                         {"LowCardinality"}};
