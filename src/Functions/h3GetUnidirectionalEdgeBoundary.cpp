...
class FunctionH3GetUnidirectionalEdgeBoundary : public IFunction {
 public:
  ...
  ColumnPtr executeImpl(const ColumnsWithTypeAndName &arguments, const DataTypePtr &, size_t input_rows_count) const override {
    ...
    return ColumnArray::create(ColumnTuple::create(Columns{std::move(latitude), std::move(longitude)}), std::move(offsets));
  }
};
...
Returns the coordinates defining the unidirectional edge [H3](#h3-index).
  ...
  FunctionDocumentation::Arguments arguments = {{"index", "Hexagon index number that represents a unidirectional edge.", {"UInt64"}}};
  FunctionDocumentation::ReturnedValue returned_value = {"Returns an array of (longitude, latitude) pairs defining a unidirectional edge.",
                                                         {"Array(Float64, Float64)"}};
  FunctionDocumentation::Examples examples = {{"Get boundary coordinates of a unidirectional edge",
