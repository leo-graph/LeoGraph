...
enum class ReplaceRegexpTraits : uint8_t { First, All };
...
struct ReplaceRegexpImpl {
        ...
        if (isNumericASCII(replacement[i + 1]))  /// substitution
        {
          ...
          if (substitution_num >= num_captures)
            throw Exception(ErrorCodes::BAD_ARGUMENTS,
                            "Substitution '\\{}' in replacement argument is invalid, regexp has only {} capturing groups", substitution_num,
                            num_captures - 1);
        }
      }
    }
  ...
  static Instructions createInstructions(std::string_view replacement, int num_captures) {
    ...
    if (!literals.empty()) instructions.emplace_back(literals);
  ...
  static bool canFallbackToStringReplacement(const String &needle, const String &replacement, const re2::RE2 &searcher, int num_captures) {
    if (searcher.NumberOfCapturingGroups()) return false;
  ...
  static void processString(const char *haystack_data, size_t haystack_length, ColumnString::Chars &res_data,
                            ColumnString::Offset &res_offset, const re2::RE2 &searcher, int num_captures,
    ...
    while (match_pos <= haystack_length) {
      ...
      if (searcher.Match(haystack, match_pos, haystack_length, re2::RE2::Anchor::UNANCHORED, matches, num_captures)) {
        ...
        if constexpr (replace == ReplaceRegexpTraits::First) can_finish_current_string = true;

        if (match.empty()) {
          /// Step one character to avoid infinite loop
          ++match_pos;
          if (match_pos > haystack_length) can_finish_current_string = true;
  ...
  static void vectorConstantConstant(const ColumnString::Chars &haystack_data, const ColumnString::Offsets &haystack_offsets,
                                     const String &needle, const String &replacement, ColumnString::Chars &res_data,
    ...
    if (!searcher.ok()) throw Exception(ErrorCodes::BAD_ARGUMENTS, "The pattern argument is not a valid re2 pattern: {}", searcher.error());
    ...
    if (canFallbackToStringReplacement(needle, replacement, searcher, num_captures)) {
      auto convert_trait = [](ReplaceRegexpTraits first_or_all) {
        switch (first_or_all) {
          case ReplaceRegexpTraits::First:
            return ReplaceStringTraits::Replace::First;
          case ReplaceRegexpTraits::All:
            return ReplaceStringTraits::Replace::All;
        }
      };
      ReplaceStringImpl<Name, convert_trait(replace)>::vectorConstantConstant(haystack_data, haystack_offsets, needle, replacement,
                                                                              res_data, res_offsets, input_rows_count);
      return;
    }
  ...
  static void vectorVectorConstant(const ColumnString::Chars &haystack_data, const ColumnString::Offsets &haystack_offsets,
                                   const ColumnString::Chars &needle_data, const ColumnString::Offsets &needle_offsets,
                                   const String &replacement, ColumnString::Chars &res_data, ColumnString::Offsets &res_offsets,
  ...
  static void vectorConstantVector(const ColumnString::Chars &haystack_data, const ColumnString::Offsets &haystack_offsets,
                                   const String &needle, const ColumnString::Chars &replacement_data,
                                   const ColumnString::Offsets &replacement_offsets, ColumnString::Chars &res_data,
                                   ColumnString::Offsets &res_offsets, size_t input_rows_count) {
  ...
  static void vectorVectorVector(const ColumnString::Chars &haystack_data, const ColumnString::Offsets &haystack_offsets,
                                 const ColumnString::Chars &needle_data, const ColumnString::Offsets &needle_offsets,
                                 const ColumnString::Chars &replacement_data, const ColumnString::Offsets &replacement_offsets,
                                 ColumnString::Chars &res_data, ColumnString::Offsets &res_offsets, size_t input_rows_count) {
    assert(haystack_offsets.size() == needle_offsets.size());
  ...
  static void vectorFixedConstantConstant(const ColumnString::Chars &haystack_data, size_t n, const String &needle,
                                          const String &replacement, ColumnString::Chars &res_data, ColumnString::Offsets &res_offsets,
                                          size_t input_rows_count) {
    if (needle.empty()) {
      ...
      res_offsets.resize(input_rows_count);
      for (size_t i = 0; i < input_rows_count; ++i) res_offsets[i] = (i + 1) * n;
      return;
    }
