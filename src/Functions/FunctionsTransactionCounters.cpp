...
class FunctionTransactionID : public FunctionConstantBase<FunctionTransactionID, Tuple, DataTypeNothing> {
 public:
  ...
  explicit FunctionTransactionID(ContextPtr context)
      : FunctionConstantBase(getValue(context->getCurrentTransaction()), context->isDistributed()) {}
};
...
class FunctionTransactionLatestSnapshot : public FunctionConstantBase<FunctionTransactionLatestSnapshot, UInt64, DataTypeUInt64> {
 ...
 public:
  ...
  explicit FunctionTransactionLatestSnapshot(ContextPtr context)
      : FunctionConstantBase(getLatestSnapshot(context), context->isDistributed()) {}
};
...
class FunctionTransactionOldestSnapshot : public FunctionConstantBase<FunctionTransactionOldestSnapshot, UInt64, DataTypeUInt64> {
 ...
 public:
  ...
  explicit FunctionTransactionOldestSnapshot(ContextPtr context)
      : FunctionConstantBase(getOldestSnapshot(context), context->isDistributed()) {}
};
...
- `host_id`: UUID of the host that has started this transaction.
  ...
  FunctionDocumentation::Examples examples_transactionID = {{"Usage example",
                                                             R"(
BEGIN TRANSACTION;
...
└────────────────────────────────────────────────┘
  ...
  FunctionDocumentation documentation_transactionID = {
      description_transactionID,    syntax_transactionID,   arguments_transactionID,     {},
      returned_value_transactionID, examples_transactionID, introduced_in_transactionID, category_transactionID};
...
:::
  ...
  FunctionDocumentation::ReturnedValue returned_value_transactionLatestSnapshot = {"Returns the latest snapshot (CSN) of a transaction.",
                                                                                   {"UInt64"}};
  FunctionDocumentation::Examples examples_transactionLatestSnapshot = {{"Usage example",
                                                                         R"(
BEGIN TRANSACTION;
...
└─────────────────────────────┘
  ...
  FunctionDocumentation documentation_transactionLatestSnapshot = {
      description_transactionLatestSnapshot,    syntax_transactionLatestSnapshot,
      arguments_transactionLatestSnapshot,      {},
      returned_value_transactionLatestSnapshot, examples_transactionLatestSnapshot,
      introduced_in_transactionLatestSnapshot,  category_transactionLatestSnapshot};
...
:::
  ...
  FunctionDocumentation::ReturnedValue returned_value_transactionOldestSnapshot = {"Returns the oldest snapshot (CSN) of a transaction.",
                                                                                   {"UInt64"}};
  FunctionDocumentation::Examples examples_transactionOldestSnapshot = {{"Usage example",
                                                                         R"(
BEGIN TRANSACTION;
...
└─────────────────────────────┘
  ...
  FunctionDocumentation documentation_transactionOldestSnapshot = {
      description_transactionOldestSnapshot,    syntax_transactionOldestSnapshot,
      arguments_transactionOldestSnapshot,      {},
      returned_value_transactionOldestSnapshot, examples_transactionOldestSnapshot,
      introduced_in_transactionOldestSnapshot,  category_transactionOldestSnapshot};
