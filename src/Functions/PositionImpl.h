...
struct PositionCaseSensitiveASCII {
  ...
  /// For searching single substring, that is different each time. This object is created for each row of data. It must have cheap
  /// initialization.
  ...
  static const char *advancePos(const char *pos, const char *end, size_t n) { return std::min(pos + n, end); }
...
struct PositionCaseSensitiveUTF8 {
  ...
  static size_t countChars(const char *begin, const char *end) {
    ...
    for (const auto *it = begin; it != end; ++it)
      if (!UTF8::isContinuationOctet(static_cast<UInt8>(*it))) ++res;
    return res;
  }
...
struct PositionImpl {
  ...
  static void vectorConstant(const ColumnString::Chars &haystack_data, const ColumnString::Offsets &haystack_offsets,
                             const std::string &needle, const ColumnPtr &start_pos, PaddedPODArray<UInt64> &res,
    ...
    if (needle.empty()) {
      ...
      if (const ColumnConst *start_pos_const = typeid_cast<const ColumnConst *>(&*start_pos)) {
        ...
        for (size_t i = 0; i < input_rows_count; ++i) {
          size_t haystack_size = Impl::countChars(reinterpret_cast<const char *>(pos),
                                                  reinterpret_cast<const char *>(pos + haystack_offsets[i] - prev_offset));
      ...
      for (size_t i = 0; i < input_rows_count; ++i) {
        size_t haystack_size =
            Impl::countChars(reinterpret_cast<const char *>(pos), reinterpret_cast<const char *>(pos + haystack_offsets[i] - prev_offset));
    ...
    while (pos < end && end != (pos = searcher.search(pos, end - pos))) {
      ...
      if (pos + needle.size() <= begin + haystack_offsets[i]) {
        auto res_pos =
            1 + Impl::countChars(reinterpret_cast<const char *>(begin + haystack_offsets[i - 1]), reinterpret_cast<const char *>(pos));
        if (res_pos < start) {
          pos = reinterpret_cast<const UInt8 *>(Impl::advancePos(
              reinterpret_cast<const char *>(pos), reinterpret_cast<const char *>(begin + haystack_offsets[i]), start - res_pos));
          continue;
        }
        res[i] = res_pos;
      } else {
    ...
    if (i < res.size()) memset(&res[i], 0, (res.size() - i) * sizeof(res[0]));
  }
  ...
  static void constantConstant(std::string data, std::string needle, const ColumnPtr &start_pos, PaddedPODArray<UInt64> &res,
  ...
  static void vectorVector(const ColumnString::Chars &haystack_data, const ColumnString::Offsets &haystack_offsets,
                           const ColumnString::Chars &needle_data, const ColumnString::Offsets &needle_offsets, const ColumnPtr &start_pos,
                           PaddedPODArray<UInt64> &res, [[maybe_unused]] ColumnUInt8 *res_null, size_t input_rows_count) {
    /// `res_null` serves as an output parameter for implementing an XYZOrNull variant.
    assert(!res_null);
    ...
    for (size_t i = 0; i < input_rows_count; ++i) {
      ...
      size_t haystack_chars_size = Impl::countChars(reinterpret_cast<const char *>(&haystack_data[prev_haystack_offset]),
      ...
      if (start > haystack_chars_size + 1) {
        ...
        const char *beg = Impl::advancePos(reinterpret_cast<const char *>(&haystack_data[prev_haystack_offset]),
                                           reinterpret_cast<const char *>(&haystack_data[haystack_offsets[i]]), start - 1);
  ...
  static void constantVector(const String &haystack, const ColumnString::Chars &needle_data, const ColumnString::Offsets &needle_offsets,
                             const ColumnPtr &start_pos, PaddedPODArray<UInt64> &res, [[maybe_unused]] ColumnUInt8 *res_null,
    ...
    for (size_t i = 0; i < input_rows_count; ++i) {
      ...
      if (start > haystack_size + 1) {
        ...
        size_t pos =
            searcher.search(reinterpret_cast<const UInt8 *>(beg), reinterpret_cast<const UInt8 *>(haystack.data()) + haystack.size()) -
