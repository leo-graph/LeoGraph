...
class KeyValuePairExtractorBuilder {
 public:
  ...
  KeyValuePairExtractorBuilder& withUnexpectedQuotingCharacterStrategy(
 ...
 private:
  ...
  /// The ideal default behavior should be `UnexpectedQuotingCharacterStrategy::INVALID`, but to make it backwards compatible, we are
  /// leaving it as `PROMOTE`
  extractKV::Configuration::UnexpectedQuotingCharacterStrategy unexpected_quoting_character_strategy =
      extractKV::Configuration::UnexpectedQuotingCharacterStrategy::PROMOTE;
};
