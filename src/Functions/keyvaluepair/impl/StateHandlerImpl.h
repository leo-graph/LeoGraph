 ...
 * Each method returns a `StateHandler::NextState` object which contains the next state itself and the number of characters consumed by the
 * previous state.
 *
 ...
 * `ReferencesMapStateHandler` does not handle escape sequences and only store references to the input data. Its output is a map and not a
 * column.
 * */
...
class StateHandlerImpl : public StateHandler {
 public:
      ...
      if (WITH_ESCAPING && isEscapeCharacter(*p)) {
        ...
        switch (configuration.unexpected_quoting_character_strategy) {
          ...
          case Configuration::UnexpectedQuotingCharacterStrategy::ACCEPT:
            // The quoting character should not be added to the search symbols list in case strategy =
            // Configuration::UnexpectedQuotingCharacterStrategy::ACCEPT See `NeedleFactory`
  ...
  /*
   * Check if next character is a valid value character and jumps to read-like states. Caveat here is that a pair delimiter must also lead
   * to read-like states because it indicates empty values.
   * */
 ...
 private:
   ...
   * */
  template <bool isKey>
  std::pair<bool, std::size_t> consumeWithEscapeSequence(std::string_view file, size_t start_pos, size_t character_pos,
                                                         auto& output) const {
  ...
  bool isKeyValueDelimiter(char character) const { return configuration.key_value_delimiter == character; }
  ...
  bool isQuotingCharacter(char character) const { return configuration.quoting_character == character; }

  bool isEscapeCharacter(char character) const { return character == '\\'; }
};
...
struct NoEscapingStateHandler : public StateHandlerImpl<false> {
  ...
  class PairWriter {
   ...
   public:
    PairWriter(ColumnString& key_col_, ColumnString& value_col_) : key_col(key_col_), value_col(value_col_) {}
    ...
    void appendKey(std::string_view new_data) { key = new_data; }
    ...
    void appendValue(std::string_view new_data) { value = new_data; }
    ...
    bool isKeyEmpty() const { return key.empty(); }
    ...
    std::string_view uncommittedKeyChunk() const { return key; }

    std::string_view uncommittedValueChunk() const { return value; }
  };

  template <typename... Args>
  explicit NoEscapingStateHandler(Args&&... args) : StateHandlerImpl<false>(std::forward<Args>(args)...) {}
};
...
struct InlineEscapingStateHandler : public StateHandlerImpl<true> {
  class PairWriter {
   ...
   public:
    ...
    void appendKey(std::string_view new_data) { key_chars.insert(new_data.begin(), new_data.end()); }
    ...
    void appendValue(std::string_view new_data) { value_chars.insert(new_data.begin(), new_data.end()); }
    ...
    void resetKey() { key_chars.resize_assume_reserved(key_prev_commit_pos); }

    void resetValue() { value_chars.resize_assume_reserved(value_prev_commit_pos); }

    bool isKeyEmpty() const { return key_chars.size() == key_prev_commit_pos; }

    bool isValueEmpty() const { return value_chars.size() == value_prev_commit_pos; }
  ...
  template <typename... Args>
  explicit InlineEscapingStateHandler(Args&&... args) : StateHandlerImpl<true>(std::forward<Args>(args)...) {}
};
...
struct ReferencesMapStateHandler : public StateHandlerImpl<false> {
  ...
  class PairWriter {
   ...
   public:
    explicit PairWriter(std::map<std::string_view, std::string_view>& map_) : map(map_) {}
  ...
  template <typename... Args>
  explicit ReferencesMapStateHandler(Args&&... args) : StateHandlerImpl<false>(std::forward<Args>(args)...) {}
};
