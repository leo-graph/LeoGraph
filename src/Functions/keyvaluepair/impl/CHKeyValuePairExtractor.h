...
class KeyValuePairExtractor {
 ...
 public:
  ...
  KeyValuePairExtractor(const Configuration& configuration_, uint64_t max_number_of_pairs_)
      : state_handler(StateHandler(configuration_)), max_number_of_pairs(max_number_of_pairs_) {}
    ...
    while (state != State::END) {
      ...
      if (next_state.position_in_string > data.size() && next_state.state != State::END) {
        throw Exception(
            ...
            magic_enum::enum_name(state), magic_enum::enum_name(next_state.state), next_state.position_in_string, data.size());
      }
