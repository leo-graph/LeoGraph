...
Configuration ConfigurationFactory::createWithoutEscaping(
    char key_value_delimiter, char quoting_character, std::vector<char> pair_delimiters,
  ...
  return Configuration(key_value_delimiter, quoting_character, pair_delimiters, unexpected_quoting_character_strategy);
}
...
Configuration ConfigurationFactory::createWithEscaping(
  ...
  if (key_value_delimiter == ESCAPE_CHARACTER || quoting_character == ESCAPE_CHARACTER ||
  ...
  return createWithoutEscaping(key_value_delimiter, quoting_character, pair_delimiters, unexpected_quoting_character_strategy);
}
...
void ConfigurationFactory::validate(char key_value_delimiter, char quoting_character, std::vector<char> pair_delimiters) {
  ...
  if (pair_delimiters.size() > MAX_NUMBER_OF_PAIR_DELIMITERS) {
    ...
    throw Exception(ErrorCodes::BAD_ARGUMENTS, "Invalid arguments, pair delimiters can contain at most {} characters",
                    MAX_NUMBER_OF_PAIR_DELIMITERS);
  }
