...
ArgumentExtractor::ParsedArguments ArgumentExtractor::extract(ColumnsWithTypeAndNameList arguments) {
  ...
  if (arguments.empty() || arguments.size() > MAX_NUMBER_OF_ARGUMENTS) {
    throw Exception(ErrorCodes::NUMBER_OF_ARGUMENTS_DOESNT_MATCH,
                    "Function extractKeyValuePairs requires at least 1 argument and at most {}. {} was provided", MAX_NUMBER_OF_ARGUMENTS,
                    arguments.size());
  }
  ...
  if (arguments.empty()) {
    return ParsedArguments{data_column, key_value_delimiter, pair_delimiters};
  }

  auto quoting_character = extractSingleCharacter(popFrontAndGet(arguments), "quoting_character");

  if (arguments.empty()) {
    return ParsedArguments{data_column, key_value_delimiter, pair_delimiters, quoting_character};
  }
  ...
  return ParsedArguments{data_column, key_value_delimiter, pair_delimiters, quoting_character, unexpected_quoting_character_strategy};
}

ArgumentExtractor::CharArgument ArgumentExtractor::extractSingleCharacter(const ColumnWithTypeAndName& argument,
...
ArgumentExtractor::VectorArgument ArgumentExtractor::extractVector(const ColumnWithTypeAndName& argument,
                                                                   const std::string& parameter_name) {
  const auto type = argument.type;
  const auto column = argument.column;
...
void ArgumentExtractor::validateColumnType(DataTypePtr type, const std::string& parameter_name) {
  if (!isStringOrFixedString(type)) {
    throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT, "Illegal type {} of argument {}. Must be String.", type, parameter_name);
  }
}
