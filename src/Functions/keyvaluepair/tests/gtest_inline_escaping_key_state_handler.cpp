...
void test_read(const auto& handler, std::string_view input, std::string_view expected_element, std::size_t expected_pos,
...
void test_read_quoted(const auto& handler, std::string_view input, std::string_view expected_element, std::size_t expected_pos,
                      State expected_state) {
...
TEST(extractKVPairInlineEscapingKeyStateHandler, Wait) {
  ...
  auto configuration =
      ConfigurationFactory::createWithEscaping(':', '"', pair_delimiters, Configuration::UnexpectedQuotingCharacterStrategy::PROMOTE);
...
TEST(extractKVPairInlineEscapingKeyStateHandler, ReadEnclosed) {
  ...
  test_read_quoted(handler, key_with_special_characters_with_end_quote, key_with_special_characters,
                   key_with_special_characters_with_end_quote.size(), State::READING_KV_DELIMITER);
