...
auto ToColumnMap(const auto& keys, const auto& values, const ColumnPtr offsets = nullptr) {
  return ColumnMap::create(std::move(keys->clone()), std::move(values->clone()), offsets ? offsets : ColumnUInt64::create(1, keys->size()));
}
...
INSTANTIATE_TEST_SUITE_P(Simple, extractKVPairKeyValuePairExtractorTest,
                         ::testing::ValuesIn(std::initializer_list<KeyValuePairExtractorTestParam>{
                             {KeyValuePairExtractorBuilder().withQuotingCharacter('\''),
                              R"in(name:'neymar';'age':31;team:psg;nationality:brazil,last_key:last_value)in", neymar_expected},
                             {// Different escaping char
                              KeyValuePairExtractorBuilder().withQuotingCharacter('"'),
                              R"in(name:"neymar";"age":31;team:psg;nationality:brazil,last_key:last_value)in", neymar_expected},
                             {// same as case 1, but with another handler
                              KeyValuePairExtractorBuilder().withQuotingCharacter('\''),
                              R"in(name:'neymar';'age':31;team:psg;nationality:brazil,last_key:last_value)in", neymar_expected, true}}));
...
INSTANTIATE_TEST_SUITE_P(InvalidEscapeSeqInValue, extractKVPairKeyValuePairExtractorTest,
                         ::testing::ValuesIn(std::initializer_list<KeyValuePairExtractorTestParam>{
                             {// Special case when invalid seq is the last symbol
                              KeyValuePairExtractorBuilder(), R"in(valid_key:valid_value key:invalid_val\)in",
                              ExpectedValues{{"valid_key", "valid_value"}, {"key", "invalid_val"}}, true},
                             // Not handling escape sequences == do not care of broken one, `invalid_val\` must be present
                             {KeyValuePairExtractorBuilder(), R"in(valid_key:valid_value key:invalid_val\ third_key:third_value)in",
                              ExpectedValues{{"valid_key", "valid_value"}, {"key", "invalid_val\\"}, {"third_key", "third_value"}}},
                             {// Special case when invalid seq is the last symbol
                              KeyValuePairExtractorBuilder(), R"in(valid_key:valid_value key:invalid_val\)in",
                              ExpectedValues{{"valid_key", "valid_value"}, {"key", "invalid_val\\"}}},
                             {KeyValuePairExtractorBuilder().withQuotingCharacter('"'),
