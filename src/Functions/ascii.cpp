...
struct AsciiImpl {
  ...
  static void vectorFixedToVector(const ColumnString::Chars &data, size_t n, PaddedPODArray<ReturnType> &res, size_t input_rows_count) {
    for (size_t i = 0; i < input_rows_count; ++i) res[i] = doAscii(data, i * n, n);
  }
...
Returns the ASCII code point of the first character of string `s` as an `Int32`.
  ...
  FunctionDocumentation::Arguments arguments = {{"s", "String input.", {"String"}}};
  FunctionDocumentation::ReturnedValue returned_value = {
      "Returns the ASCII code point of the first character. If `s` is empty, the result is `0`. If the first character is not an ASCII "
      "character or not part of the Latin-1 supplement range of UTF-16, the result is undefined.",
      {"Int32"}};
  FunctionDocumentation::Examples examples = {{"Usage example", "SELECT ascii('234')",
                                               R"(
┌─ascii('234')─┐
│           50 │
└──────────────┘
