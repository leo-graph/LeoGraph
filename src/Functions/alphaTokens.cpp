...
class SplitByAlphaImpl {
 ...
 public:
  ...
  bool get(Pos &token_begin, Pos &token_end) {
    /// Skip garbage
    while (pos < end && !isAlphaASCII(*pos)) ++pos;
...
Selects substrings of consecutive bytes from the ranges `a-z` and `A-Z` and returns an array of the selected substrings.
  ...
  FunctionDocumentation::Arguments arguments = {
      ...
      {"max_substrings",
       "Optional. When `max_substrings > 0`, the number of returned substrings will be no more than `max_substrings`, otherwise the "
       "function will return as many substrings as possible.",
       {"Int64"}}};
  FunctionDocumentation::ReturnedValue returned_value = {"Returns an array of selected substrings of `s`.", {"Array(String)"}};
  FunctionDocumentation::Examples examples = {{"Usage example", "SELECT alphaTokens('abca1abc');",
                                               R"(
