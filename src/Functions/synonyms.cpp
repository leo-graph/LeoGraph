...
class FunctionSynonyms : public IFunction {
 ...
 public:
  explicit FunctionSynonyms(SynonymsExtensions &extensions_) : extensions(extensions_) {}
  ...
  ColumnPtr executeImpl(const ColumnsWithTypeAndName &arguments, const DataTypePtr &result_type, size_t input_rows_count) const override {
    ...
    for (size_t i = 0; i < offsets.size(); ++i) {
      ...
      if (synset) {
        for (const auto &token : *synset) out_data.insert(Field(token.data(), token.size()));
...
The thesaurus must contain a WordNet sense index.
  ...
  FunctionDocumentation::Arguments arguments = {{"ext_name", "Name of the extension in which search will be performed.", {"String"}},
