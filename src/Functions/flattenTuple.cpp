...
class FunctionFlattenTuple : public IFunction {
 public:
  ...
  DataTypePtr getReturnTypeImpl(const DataTypes &arguments) const override {
    ...
    if (!type_tuple)
      throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT, "Argument for function '{}' must be Tuple or Nullable(Tuple). Got '{}'",
    ...
    if (!type_tuple || !type_tuple->hasExplicitNames())
      throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT, "Tuple argument for function '{}' must be named. Got '{}'", getName(),
                      type->getName());
...
The elements of the returned tuple are the paths of the input tuple.
  ...
  FunctionDocumentation::Arguments arguments = {{"input", "Named and nested tuple to flatten.", {"Tuple(n1 T1[, n2 T2, ... ])"}}};
  FunctionDocumentation::ReturnedValue returned_value = {"Returns an output tuple whose elements are paths from the original input.",
                                                         {"Tuple(T)"}};
