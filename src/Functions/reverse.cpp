...
class FunctionReverse : public IFunction {
 public:
  ...
  static FunctionPtr create(ContextPtr) { return std::make_shared<FunctionReverse>(); }
...
class ReverseOverloadResolver : public IFunctionOverloadResolver {
 public:
  ...
  FunctionBasePtr buildImpl(const ColumnsWithTypeAndName &arguments, const DataTypePtr &return_type) const override {
    if (isArray(arguments.at(0).type)) return FunctionFactory::instance().getImpl("arrayReverse", context)->build(arguments);
...
REGISTER_FUNCTION(Reverse) {
  ...
  FunctionDocumentation::Arguments arguments = {{"arr | str", "The source array or string.", {"Array(T)", "String"}}};
  ...
  FunctionDocumentation::Examples examples = {{"Reverse array", "SELECT reverse([1, 2, 3, 4]);", "[4, 3, 2, 1]"},
