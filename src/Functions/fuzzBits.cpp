 ...
 public:
  ...
  DataTypePtr getReturnTypeImpl(const ColumnsWithTypeAndName &arguments) const override {
    if (!isStringOrFixedString(arguments[0].type))
      throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT, "First argument of function {} must be String or FixedString", getName());
  ...
  ColumnPtr executeImpl(const ColumnsWithTypeAndName &arguments, const DataTypePtr &, size_t input_rows_count) const override {
    ...
    if (input_rows_count == 0) return col_in_untyped;
...
Flips the bits of the input string `s`, with probability `p` for each bit.
  ...
  FunctionDocumentation::Arguments arguments = {{"s", "String or FixedString to perform bit fuzzing on", {"String", "FixedString"}},
