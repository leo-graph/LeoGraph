...
class FunctionRunningDifferenceImpl : public IFunction {
 ...
 public:
  ...
  DataTypePtr getReturnTypeImpl(const DataTypes &arguments) const override {
    DataTypePtr res;
    dispatchForSourceType(*removeNullable(arguments[0]),
                          [&](auto field_type_tag) { res = std::make_shared<DataTypeNumber<DstFieldType<decltype(field_type_tag)>>>(); });

    if (arguments[0]->isNullable()) res = makeNullable(res);
  ...
  ColumnPtr executeImpl(const ColumnsWithTypeAndName &arguments, const DataTypePtr &result_type, size_t input_rows_count) const override {
    ...
    if (isColumnConst(*src.column)) return result_type->createColumnConstWithDefaultValue(input_rows_count);
    ...
    if (null_map_column) return ColumnNullable::create(std::move(res_column), null_map_column);
    return res_column;
  }
};
