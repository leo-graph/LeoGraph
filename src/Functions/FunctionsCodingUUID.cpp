...
namespace {
enum class Representation : uint8_t { BigEndian, LittleEndian };

std::pair<int, int> determineBinaryStartIndexWithIncrement(ptrdiff_t num_bytes, Representation representation) {
  if (representation == Representation::BigEndian) return {0, 1};
  if (representation == Representation::LittleEndian) return {num_bytes - 1, -1};
...
class UUIDSerializer {
 public:
  enum class Variant : uint8_t { Default = 1, Microsoft = 2 };
...
void checkArgumentCount(const DB::DataTypes &arguments, const std::string_view function_name) {
  if (const auto argument_count = std::ssize(arguments); argument_count < 1 || argument_count > 2)
    throw DB::Exception(DB::ErrorCodes::NUMBER_OF_ARGUMENTS_DOESNT_MATCH,
                        "Number of arguments for function {} doesn't match: passed {}, should be 1 or 2", function_name, argument_count);
}
...
void checkFormatArgument(const DB::DataTypes &arguments, const std::string_view function_name) {
  if (const auto argument_count = std::ssize(arguments);
    ...
    throw DB::Exception(DB::ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT,
                        "Illegal type {} of second argument of function {}, expected Int8 or UInt8 type", arguments[1]->getName(),
                        function_name);
}

UUIDSerializer::Variant parseVariant(const DB::ColumnsWithTypeAndName &arguments) {
  if (arguments.size() < 2) return UUIDSerializer::Variant::Default;
  ...
  if (!as_enum) throw DB::Exception(DB::ErrorCodes::ARGUMENT_OUT_OF_BOUND, "Expected UUID variant, got {}", representation);
 ...
 public:
  ...
  DataTypePtr getReturnTypeImpl(const DataTypes &arguments) const override {
    ...
    if (!ptr || ptr->getN() != uuid_bytes_length)
      throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT, "Illegal type {} of argument of function {}, expected FixedString({})",
    ...
    if (const auto *col_in = checkAndGetColumn<ColumnFixedString>(column.get())) {
      if (col_in->getN() != uuid_bytes_length)
        throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT,
                        ...
                        col_in->getName(), getName(), uuid_bytes_length);
  ...
  DataTypePtr getReturnTypeImpl(const DataTypes &arguments) const override {
    ...
    if (!isString(arguments[0])) {
      ...
      if (!ptr || ptr->getN() != uuid_text_length)
        throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT, "Illegal type {} of first argument of function {}, expected FixedString({})",
  ...
  ColumnPtr executeImpl(const ColumnsWithTypeAndName &arguments, const DataTypePtr &, size_t input_rows_count) const override {
    ...
    if (const auto *col_in_fixed = checkAndGetColumn<ColumnFixedString>(column.get())) {
      if (col_in_fixed->getN() != uuid_text_length)
        throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT,
                        "Illegal type {} of column {} argument of function {}, expected FixedString({})", col_type_name.type->getName(),
                        col_in_fixed->getName(), getName(), uuid_text_length);
 ...
 public:
  ...
  DataTypePtr getReturnTypeImpl(const DataTypes &arguments) const override {
    ...
    if (!isUUID(arguments[0])) {
      throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT, "Illegal type {} of first argument of function {}, expected UUID",
...
Takes a binary representation of a UUID, with its format optionally specified by `variant` (`Big-endian` by default), and returns a string containing 36 characters in text format.
  ...
  FunctionDocumentation::Arguments arguments_UUIDNumToString = {
      ...
      {"variant",
       "Variant as specified by [RFC4122](https://datatracker.ietf.org/doc/html/rfc4122#section-4.1.1). 1 = `Big-endian` (default), 2 = "
       "`Microsoft`.",
       {"(U)Int*"}}};
  FunctionDocumentation::ReturnedValue returned_value_UUIDNumToString = {"Returns the UUID as a string.", {"String"}};
  FunctionDocumentation::Examples examples_UUIDNumToString = {{"Usage example",
                                                               R"(
SELECT
    'a/<@];!~p{jTj={)' AS bytes,
...
└──────────────────┴──────────────────────────────────────┘
  ...
  FunctionDocumentation documentation_UUIDNumToString = {
      description_UUIDNumToString,    syntax_UUIDNumToString,   arguments_UUIDNumToString,     {},
      returned_value_UUIDNumToString, examples_UUIDNumToString, introduced_in_UUIDNumToString, category_UUIDNumToString};
...
Accepts a string containing 36 characters in the format `xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx`, and returns a [FixedString(16)](../data-types/fixedstring.md) as its binary representation, with its format optionally specified by `variant` (`Big-endian` by default).
  ...
  FunctionDocumentation::Arguments arguments_UUIDStringToNum = {
      ...
      {"variant",
       ...
       "`Microsoft`.",
       {"(U)Int*"}}};
  FunctionDocumentation::ReturnedValue returned_value_UUIDStringToNum = {"Returns the binary representation of `string`.",
                                                                         {"FixedString(16)"}};
  FunctionDocumentation::Examples examples_UUIDStringToNum = {{"Usage example",
                                                               R"(
SELECT
...
└──────────────────────────────────────┴──────────────────┘
  ...
  FunctionDocumentation documentation_UUIDStringToNum = {
      description_UUIDStringToNum,    syntax_UUIDStringToNum,   arguments_UUIDStringToNum,     {},
      returned_value_UUIDStringToNum, examples_UUIDStringToNum, introduced_in_UUIDStringToNum, category_UUIDStringToNum};
...
This function replaces calls to two separate functions `UUIDStringToNum(toString(uuid))` so no intermediate conversion from UUID to string is required to extract bytes from a UUID.
  ...
  FunctionDocumentation::Examples examples_UUIDToNum = {{"Usage example",
                                                         R"(
SELECT
...
└──────────────────────────────────────┴──────────────────┘
  ...
  FunctionDocumentation documentation_UUIDToNum = {
      description_UUIDToNum,    syntax_UUIDToNum,   arguments_UUIDToNum,     {},
      returned_value_UUIDToNum, examples_UUIDToNum, introduced_in_UUIDToNum, category_UUIDToNum};
...
Returns the timestamp component of a UUID version 7.
  ...
  FunctionDocumentation::Arguments arguments_UUIDv7ToDateTime = {
      ...
      {"timezone",
       "Optional. [Timezone name](../../operations/server-configuration-parameters/settings.md#timezone) for the returned value.",
       {"String"}}};
  FunctionDocumentation::ReturnedValue returned_value_UUIDv7ToDateTime = {
      "Returns a timestamp with milliseconds precision. If the UUID is not a valid version 7 UUID, it returns `1970-01-01 00:00:00.000`.",
      {"DateTime64(3)"}};
  FunctionDocumentation::Examples examples_UUIDv7ToDateTime = {{"Usage example",
                                                                R"(
...
└─────────────────────────────────────────────────────────────────────────────────────┘
  ...
  FunctionDocumentation documentation_UUIDv7ToDateTime = {
      description_UUIDv7ToDateTime,    syntax_UUIDv7ToDateTime,   arguments_UUIDv7ToDateTime,     {},
      returned_value_UUIDv7ToDateTime, examples_UUIDv7ToDateTime, introduced_in_UUIDv7ToDateTime, category_UUIDv7ToDateTime};
