...
class FunctionConstantBase : public IFunction {
 public:
  ...
  explicit FunctionConstantBase(const U &constant_value_, bool is_distributed_ = false)
      : constant_value(static_cast<T>(constant_value_)), is_distributed(is_distributed_) {}

  String getName() const override { return Derived::name; }

  size_t getNumberOfArguments() const override { return 0; }

  DataTypePtr getReturnTypeImpl(const DataTypes & /*arguments*/) const override { return std::make_shared<ColumnT>(); }
