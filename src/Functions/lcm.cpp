...
constexpr T abs(T value) noexcept {
  if constexpr (std::is_signed_v<T>) {
    if (value >= 0 || value == std::numeric_limits<T>::min()) return value;
...
struct LCMImpl : public GCDLCMImpl<A, B, LCMImpl<A, B>, NameLCM> {
  ...
  static ResultType applyImpl(A a, B b) {
    ...
    Unsigned val1 =
        abs_impl::abs<Int>(a) / boost::integer::gcd(Int(a), Int(b));  // NOLINT(clang-analyzer-core.UndefinedBinaryOperatorResult)
...
An exception is thrown when dividing by zero or when dividing a minimal negative number by minus one.
  ...
  FunctionDocumentation::Arguments arguments = {{"x", "First integer.", {"(U)Int*"}}, {"y", "Second integer.", {"(U)Int*"}}};
