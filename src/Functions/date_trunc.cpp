 ...
 public:
  ...
  DataTypePtr getReturnTypeImpl(const ColumnsWithTypeAndName &arguments) const override {
    ...
    auto check_second_argument = [&] {
      ...
      if (second_argument_is_date && ((datepart_kind == IntervalKind::Kind::Hour) || (datepart_kind == IntervalKind::Kind::Minute) ||
                                      (datepart_kind == IntervalKind::Kind::Second)))
        throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT, "Illegal type {} of argument for function {}", arguments[1].type->getName(),
                        getName());
      ...
      if ((isDateTime64(arguments[1].type) || isDate32(arguments[1].type)) &&
          context->getSettingsRef()[Setting::function_date_trunc_return_type_behavior] == 0) {
    ...
    if (result_type == ResultType::Date) return std::make_shared<DataTypeDate>();
    if (result_type == ResultType::Date32) return std::make_shared<DataTypeDate32>();
...
The type of interval to truncate the result. Possible values: `nanosecond` (only DateTime64), `microsecond` (only DateTime64), `millisecond` (only DateTime64), `second`, `minute`, `hour`, `day`, `week`, `month`, `quarter`, `year`.
      ...
      {"timezone",
       "Optional. Timezone name for the returned datetime. If not specified, the function uses the timezone of the `datetime` parameter.",
       {"String"}}};
...
| Nanosecond                  |                                       | with scale 3, 6, or 9                                                                  |
      ...
      {}};
  FunctionDocumentation::Examples examples = {{"Truncate without timezone", R"(
