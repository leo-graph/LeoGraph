  ...
  static void processCodePoint(const UInt8 *&src, const UInt8 *src_end, UInt8 *&dst, bool &prev_alphanum) {
    ...
    if (src_code_point) {
      ...
      if (alphanum && !prev_alphanum) {
        if (alpha) dst_code_point = Poco::Unicode::toUpper(*src_code_point);
      } else if (alpha) {
 ...
 private:
  static void array(const UInt8 *src, const UInt8 *src_end, const ColumnString::Offsets &offsets, UInt8 *dst) {
    ...
    while (src < src_end) {
      ...
      while (src < row_end) processCodePoint(src, row_end, dst, prev_alphanum);
      ++offset_it;
    }
  }
};
...
:::
  ...
  FunctionDocumentation::ReturnedValue returned_value = {"Returns `s` with the first letter of each word converted to upper case.",
                                                         {"String"}};
  FunctionDocumentation::Examples examples = {{"Usage example", "SELECT initcapUTF8('не тормозит')",
                                               R"(
