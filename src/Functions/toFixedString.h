...
enum class ConvertToFixedStringExceptionMode : uint8_t { Throw, Null };
...
class FunctionToFixedString : public IFunction {
 public:
  ...
  DataTypePtr getReturnTypeImpl(const ColumnsWithTypeAndName &arguments) const override {
    ...
    if (!arguments[1].column) throw Exception(ErrorCodes::ILLEGAL_COLUMN, "Second argument for function {} must be constant", getName());
  ...
  static ColumnPtr executeForN(const ColumnsWithTypeAndName &arguments, const size_t n) {
    ...
    if (const auto *column_string = checkAndGetColumn<ColumnString>(column.get())) {
      ...
      if (src_n == n) return column_fixed_string->cloneResized(column_fixed_string->size());
      ...
      for (size_t i = 0; i < size; ++i) memcpy(&out_chars[i * n], &in_chars[i * src_n], src_n);
