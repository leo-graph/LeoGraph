...
class FunctionDictHelper {
 public:
  ...
  std::shared_ptr<const IDictionary> getDictionary(const ColumnPtr &column) {
    ...
    if (!dict_name_col) throw Exception(ErrorCodes::UNSUPPORTED_METHOD, "Expected const String column");
  ...
  static void checkDictionaryHierarchySupport(const std::shared_ptr<const IDictionary> &dictionary) {
    ...
    if (!dictionary->hasHierarchy() || !hierarchical_attribute_index_optional.has_value())
      throw Exception(ErrorCodes::UNSUPPORTED_METHOD, "Dictionary {} does not support hierarchy", dictionary->getFullName());
  }
  ...
  bool isDictGetFunctionInjective(const Block &sample_columns) {
    ...
    if (sample_columns.empty()) return false;

    if (sample_columns.columns() < 3) throw Exception(ErrorCodes::NUMBER_OF_ARGUMENTS_DOESNT_MATCH, "Wrong arguments count");

    const auto *dict_name_col = checkAndGetColumnConst<ColumnString>(sample_columns.getByPosition(0).column.get());
    if (!dict_name_col) throw Exception(ErrorCodes::ILLEGAL_COLUMN, "First argument of function dictGet must be a constant string");

    const auto *attr_name_col = checkAndGetColumnConst<ColumnString>(sample_columns.getByPosition(1).column.get());
    if (!attr_name_col) throw Exception(ErrorCodes::ILLEGAL_COLUMN, "Second argument of function dictGet must be a constant string");
...
class FunctionDictHas final : public IFunction {
 public:
  ...
  static FunctionPtr create(ContextPtr context) { return std::make_shared<FunctionDictHas>(context); }
  ...
  DataTypePtr getReturnTypeImpl(const DataTypes &arguments) const override {
    if (arguments.size() < 2)
      throw Exception(ErrorCodes::NUMBER_OF_ARGUMENTS_DOESNT_MATCH, "Wrong argument count for function {}", getName());

    if (!isString(arguments[0]))
      throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT, "Illegal type {} of first argument of function {}, expected a string",
  ...
  ColumnPtr executeImpl(const ColumnsWithTypeAndName &arguments, const DataTypePtr &result_type, size_t input_rows_count) const override {
     ...
     */
    if (input_rows_count == 0) return result_type->createColumn();
    ...
    if (const auto *name_col = checkAndGetColumnConst<ColumnString>(arguments[0].column.get()))
      ...
      throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT, "Illegal type {} of first argument of function {}, expected a const string.",
    ...
    if (dictionary_special_key_type == DictionarySpecialKeyType::Range) {
      if (arguments.size() != 3)
        throw Exception(ErrorCodes::NUMBER_OF_ARGUMENTS_DOESNT_MATCH,
                        "Wrong argument count for function {} when dictionary has key type range", getName());
      ...
      if (!(range_col_type->isValueRepresentedByInteger() && range_col_type->getSizeOfValueInMemory() <= sizeof(Int64)))
        throw Exception(ErrorCodes::ILLEGAL_COLUMN, "Illegal type {} of fourth argument of function {} must be convertible to Int64.",
                        range_col_type->getName(), getName());
    }
      ...
      if (isTuple(key_column_type)) {
        ...
        if (keys_size > 1) {
          throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT,
                          ...
                          "Actual type {}.",
                          getName(), key_column_type->getName());
        }
...
enum class DictionaryGetFunctionType : uint8_t { get, getOrDefault, getAll };
...
class FunctionDictGetNoType final : public IFunction {
 public:
  ...
  static constexpr auto name =
      (dictionary_get_function_type == DictionaryGetFunctionType::get)
          ? "dictGet"
          : ((dictionary_get_function_type == DictionaryGetFunctionType::getOrDefault) ? "dictGetOrDefault" : "dictGetAll");

  static FunctionPtr create(ContextPtr context) { return std::make_shared<FunctionDictGetNoType>(context); }
  ...
  bool isShortCircuit(ShortCircuitSettings &settings, size_t number_of_arguments) const override {
    if constexpr (dictionary_get_function_type != DictionaryGetFunctionType::getOrDefault) return false;

    /// We execute lazily only last argument with default expression.
    for (size_t i = 0; i != number_of_arguments - 1; ++i) settings.arguments_with_disabled_lazy_execution.insert(i);
  ...
  DataTypePtr getReturnTypeImpl(const ColumnsWithTypeAndName &arguments) const override {
    ...
    if constexpr (dictionary_get_function_type == DictionaryGetFunctionType::getAll) {
      if (key_is_nullable) throw Exception(ErrorCodes::UNSUPPORTED_METHOD, "Function {} does not support nullable keys", getName());

      // Wrap all the attribute types in Array()
      for (auto &attr_type : attribute_types) attr_type = std::make_shared<DataTypeArray>(attr_type);
    }
    ...
    if (key_is_nullable) return makeNullable(attribute_types.front());
  ...
  ColumnPtr executeImpl(const ColumnsWithTypeAndName &arguments, const DataTypePtr &result_type, size_t input_rows_count) const override {
    ...
    if (dictionary_special_key_type == DictionarySpecialKeyType::Range) {
      if (current_arguments_index >= arguments.size())
        throw Exception(ErrorCodes::NUMBER_OF_ARGUMENTS_DOESNT_MATCH,
                        "Number of arguments for function {} doesn't match: passed {} should be {}", getName(), arguments.size(),
                        arguments.size() + 1);
    ...
    if (dictionary_get_function_type == DictionaryGetFunctionType::getOrDefault) {
      ...
      if (!column_function) {
        ColumnWithTypeAndName column_to_cast = {column_before_cast.column->convertToFullColumnIfConst(), column_before_cast.type,
        ...
        if (attribute_names.size() > 1) {
          ...
          if (!tuple_column)
            throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT, "Wrong argument for function {} default values column must be tuple",
                            getName());
      ...
      if (dictionary_get_function_type == DictionaryGetFunctionType::getAll && current_arguments_index < arguments.size()) {
        ...
        if (!limit_col || !isColumnConst(*limit_col))
          throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT,
                          ...
                          arguments[current_arguments_index].type->getName(), getName());

        collect_values_limit = limit_col->getUInt(0);
        ++current_arguments_index;
      }

      for (size_t i = 0; i < attribute_names.size(); ++i) default_cols.emplace_back(nullptr);
    }
    ...
    if (key_is_only_null) return result_type->createColumnConstWithDefaultValue(input_rows_count);

    bool key_is_nullable = key_col_with_type.type->isNullable();
    if (key_is_nullable) key_col_with_type = columnGetNested(key_col_with_type);
      ...
      if (isTuple(key_column_type)) {
        ...
        if (keys_size > 1) {
          throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT,
                          "Third argument of function {} must be tuple when dictionary is complex and key contains more than 1 attribute."
                          "Actual type {}.",
                          getName(), key_col_with_type.type->getName());
        }
    ...
    if (key_is_nullable) result_column = wrapInNullable(result_column, {arguments[2]}, result_type, input_rows_count);

    return result_column;
  }

 private:
  std::pair<ColumnPtr, ColumnPtr> getDefaultsShortCircuit(IColumn::Filter &&default_mask, const DataTypePtr &result_type,
  ...
  void restoreShortCircuitColumn(ColumnPtr &result_column, ColumnPtr defaults_column, ColumnPtr mask_column,
  ...
  ColumnPtr executeDictionaryRequest(std::shared_ptr<const IDictionary> &dictionary, const Strings &attribute_names,
                                     const Columns &key_columns, const DataTypes &key_types, const DataTypePtr &attribute_type,
                                     const Columns &default_cols, size_t collect_values_limit, const ColumnWithTypeAndName &last_argument,
                                     const DataTypePtr &result_type) const {
    ColumnPtr result;
    ...
    if (attribute_names.size() > 1) {
      ...
      if constexpr (dictionary_get_function_type == DictionaryGetFunctionType::getAll) {
        result_columns = dictionary->getColumnsAllValues(attribute_names, attribute_tuple_type.getElements(), key_columns, key_types,
                                                         default_cols, collect_values_limit);
        ...
        for (size_t col = 0; col < result_columns.size(); ++col) {
          restoreShortCircuitColumn(result_columns[col], tuple_defaults.getColumnPtr(col), mask_column,
                                    result_tuple_type.getElements()[col]);
        }
      } else {
        result_columns = dictionary->getColumns(attribute_names, attribute_tuple_type.getElements(), key_columns, key_types, default_cols);
      }

      result = ColumnTuple::create(std::move(result_columns));
    } else {
      if constexpr (dictionary_get_function_type == DictionaryGetFunctionType::getAll) {
        result = dictionary->getColumnAllValues(attribute_names[0], attribute_type, key_columns, key_types, default_cols.front(),
        ...
        result = dictionary->getColumn(attribute_names[0], attribute_type, key_columns, key_types, default_cols.front());
      }
    }
...
class FunctionDictGetImpl final : public IFunction {
 ...
 public:
  ...
  static FunctionPtr create(ContextPtr context) { return std::make_shared<FunctionDictGetImpl>(context); }
 ...
 private:
  ...
  bool isInjective(const ColumnsWithTypeAndName &sample_columns) const override { return impl.isInjective(sample_columns); }
  ...
  ColumnPtr executeImpl(const ColumnsWithTypeAndName &arguments, const DataTypePtr &result_type, size_t input_rows_count) const override {
    ...
    if (!return_type->equals(*result_type))
      throw Exception(ErrorCodes::TYPE_MISMATCH, "Function {} dictionary attribute has different type {} expected {}", getName(),
...
struct NameDictGetUInt8 {
  static constexpr auto name = "dictGetUInt8";
};
struct NameDictGetUInt16 {
  static constexpr auto name = "dictGetUInt16";
};
struct NameDictGetUInt32 {
  static constexpr auto name = "dictGetUInt32";
};
struct NameDictGetUInt64 {
  static constexpr auto name = "dictGetUInt64";
};
struct NameDictGetInt8 {
  static constexpr auto name = "dictGetInt8";
};
struct NameDictGetInt16 {
  static constexpr auto name = "dictGetInt16";
};
struct NameDictGetInt32 {
  static constexpr auto name = "dictGetInt32";
};
struct NameDictGetInt64 {
  static constexpr auto name = "dictGetInt64";
};
struct NameDictGetFloat32 {
  static constexpr auto name = "dictGetFloat32";
};
struct NameDictGetFloat64 {
  static constexpr auto name = "dictGetFloat64";
};
struct NameDictGetDate {
  static constexpr auto name = "dictGetDate";
};
struct NameDictGetDateTime {
  static constexpr auto name = "dictGetDateTime";
};
struct NameDictGetUUID {
  static constexpr auto name = "dictGetUUID";
};
struct NameDictGetIPv4 {
  static constexpr auto name = "dictGetIPv4";
};
struct NameDictGetIPv6 {
  static constexpr auto name = "dictGetIPv6";
};
struct NameDictGetString {
...
struct NameDictGetUInt8OrDefault {
  static constexpr auto name = "dictGetUInt8OrDefault";
};
struct NameDictGetUInt16OrDefault {
  static constexpr auto name = "dictGetUInt16OrDefault";
};
struct NameDictGetUInt32OrDefault {
  static constexpr auto name = "dictGetUInt32OrDefault";
};
struct NameDictGetUInt64OrDefault {
  static constexpr auto name = "dictGetUInt64OrDefault";
};
struct NameDictGetInt8OrDefault {
  static constexpr auto name = "dictGetInt8OrDefault";
};
struct NameDictGetInt16OrDefault {
  static constexpr auto name = "dictGetInt16OrDefault";
};
struct NameDictGetInt32OrDefault {
  static constexpr auto name = "dictGetInt32OrDefault";
};
struct NameDictGetInt64OrDefault {
  static constexpr auto name = "dictGetInt64OrDefault";
};
struct NameDictGetFloat32OrDefault {
  static constexpr auto name = "dictGetFloat32OrDefault";
};
struct NameDictGetFloat64OrDefault {
  static constexpr auto name = "dictGetFloat64OrDefault";
};
struct NameDictGetDateOrDefault {
  static constexpr auto name = "dictGetDateOrDefault";
};
struct NameDictGetDateTimeOrDefault {
  static constexpr auto name = "dictGetDateTimeOrDefault";
};
struct NameDictGetUUIDOrDefault {
  static constexpr auto name = "dictGetUUIDOrDefault";
};
struct NameDictGetIPv4OrDefault {
  static constexpr auto name = "dictGetIPv4OrDefault";
};
struct NameDictGetIPv6OrDefault {
  static constexpr auto name = "dictGetIPv6OrDefault";
};
struct NameDictGetStringOrDefault {
...
class FunctionDictGetOrNull final : public IFunction {
 public:
  ...
  static FunctionPtr create(ContextPtr context) { return std::make_shared<FunctionDictGetOrNull>(context); }

  explicit FunctionDictGetOrNull(ContextPtr context_) : dictionary_get_func_impl(context_), dictionary_has_func_impl(context_) {}
 ...
 private:
  ...
  DataTypePtr getReturnTypeImpl(const ColumnsWithTypeAndName &arguments) const override {
    ...
    if (result_data_type.isTuple()) {
      ...
      for (auto &element_type : elements_types_copy) element_type = makeNullable(element_type);
  ...
  ColumnPtr executeImpl(const ColumnsWithTypeAndName &arguments, const DataTypePtr &result_type, size_t input_rows_count) const override {
    ...
    auto is_key_in_dictionary_column =
        dictionary_has_func_impl.executeImpl(dict_has_arguments, std::make_shared<DataTypeUInt8>(), input_rows_count);
    ...
    for (auto &key : is_key_in_dictionary_data) key = !key;
  ...
  static void addNullMap(PaddedPODArray<UInt8> &null_map, PaddedPODArray<UInt8> &null_map_to_add) {
    ...
    for (size_t i = 0; i < null_map.size(); ++i) null_map[i] = null_map[i] || null_map_to_add[i];
  }
  ...
  static ColumnsWithTypeAndName filterAttributeNameArgumentForDictHas(const ColumnsWithTypeAndName &arguments) {
    ...
    for (size_t i = 0; i < arguments.size(); ++i) {
      if (i == attribute_name_argument_index) continue;
...
class FunctionDictGetHierarchy final : public IFunction {
 public:
  ...
  static FunctionPtr create(ContextPtr context) { return std::make_shared<FunctionDictGetHierarchy>(context); }
...
class FunctionDictIsIn final : public IFunction {
 public:
  ...
  static FunctionPtr create(ContextPtr context) { return std::make_shared<FunctionDictIsIn>(context); }

  explicit FunctionDictIsIn(ContextPtr context_) : helper(context_) {}
 ...
 private:
  ...
  DataTypePtr getReturnTypeImpl(const DataTypes &arguments) const override {
    if (!isString(arguments[0]))
      throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT,
                      "Illegal type of first argument of function {}. Expected String. Actual type {}", getName(), arguments[0]->getName());
...
class FunctionDictGetDescendantsExecutable final : public IExecutableFunction {
 public:
  FunctionDictGetDescendantsExecutable(String name_, size_t level_,
  ...
  ColumnPtr executeImpl(const ColumnsWithTypeAndName &arguments, const DataTypePtr &result_type, size_t input_rows_count) const override {
    ...
    return dictionary->getDescendants(key_column_cast, removeNullable(hierarchical_attribute.type), level,
                                      hierarchical_parent_to_child_index);
  }
...
class FunctionDictGetDescendantsBase final : public IFunctionBase {
 public:
  FunctionDictGetDescendantsBase(String name_, const DataTypes &argument_types_, const DataTypePtr &result_type_, size_t level_,
                                 DictionaryHierarchicalParentToChildIndexPtr hierarchical_parent_to_child_index_,
 ...
 public:
  ...
  FunctionBasePtr buildImpl(const ColumnsWithTypeAndName &arguments, const DataTypePtr &result_type) const override {
    ...
    if (arguments.size() == 3) {
      if (!arguments[2].column || !isColumnConst(*arguments[2].column))
        throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT,
                        "Illegal type of third argument of function {}. Expected const unsigned integer.", getName());
    ...
    return std::make_shared<FunctionDictGetDescendantsBase>(name, argument_types, result_type, level, hierarchical_parent_to_child_index,
                                                            dictionary_helper);
  }
  ...
  DataTypePtr getReturnTypeImpl(const ColumnsWithTypeAndName &arguments) const override {
    ...
    if (arguments_size < 2 || arguments_size > 3) {
      throw Exception(ErrorCodes::NUMBER_OF_ARGUMENTS_DOESNT_MATCH,
                      "Illegal arguments size of function {}. Expects 2 or 3 arguments size. Actual size {}", getName(), arguments_size);
    }

    if (!isString(arguments[0].type))
      throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT,
                      "Illegal type of first argument of function {}. Expected const String. Actual type {}", getName(),
                      arguments[0].type->getName());

    if (arguments.size() == 3 && !isInteger(arguments[2].type)) {
      throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT,
                      "Illegal type of third argument of function {}. Expected const unsigned integer. Actual type {}", getName(),
