...
class NowOverloadResolver : public IFunctionOverloadResolver {
 public:
  ...
  DataTypePtr getReturnTypeImpl(const ColumnsWithTypeAndName &arguments) const override {
    ...
    FunctionArgumentDescriptors optional_arguments{{"timezone", &isStringOrFixedString, nullptr, "String"}};

    validateFunctionArguments(getName(), arguments, mandatory_arguments, optional_arguments);
    if (arguments.size() == 1) {
      return std::make_shared<DataTypeDateTime>(
          extractTimeZoneNameFromFunctionArguments(arguments, 0, 0, allow_nonconst_timezone_arguments));
    }
    return std::make_shared<DataTypeDateTime>();
  }
...
now([timezone])
  ...
  FunctionDocumentation::Arguments arguments = {{"timezone", "Optional. Timezone name for the returned value.", {"String"}}};
  FunctionDocumentation::ReturnedValue returned_value = {"Returns the current date and time.", {"DateTime"}};
  FunctionDocumentation::Examples examples = {{"Query without timezone", R"(
SELECT now()
        )",
                                               R"(
┌───────────────now()─┐
