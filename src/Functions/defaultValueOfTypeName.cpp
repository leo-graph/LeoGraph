...
class FunctionDefaultValueOfTypeName : public IFunction {
 public:
  ...
  static FunctionPtr create(ContextPtr) { return std::make_shared<FunctionDefaultValueOfTypeName>(); }
  ...
  DataTypePtr getReturnTypeImpl(const ColumnsWithTypeAndName &arguments) const override {
    ...
    if (!col_type_const || !isString(arguments.front().type))
      throw Exception(ErrorCodes::ILLEGAL_COLUMN, "The argument of function {} must be a constant string describing type.", getName());
...
Returns the default value for the given type name.
  ...
  FunctionDocumentation::Arguments arguments = {{"type", "A string representing a type name.", {"String"}}};
  FunctionDocumentation::ReturnedValue returned_value = {
      "Returns the default value for the given type name: `0` for numbers, an empty string for strings, or `NULL` for Nullable",
      {"UInt8", "String", "NULL"}};
