...
struct BitRotateLeftImpl {
  ...
  static llvm::Value* compile(llvm::IRBuilder<>& b, llvm::Value* left, llvm::Value* right, bool) {
    if (!left->getType()->isIntegerTy()) throw Exception(ErrorCodes::LOGICAL_ERROR, "BitRotateLeftImpl expected an integral type");
...
struct NameBitRotateLeft {
  static constexpr auto name = "bitRotateLeft";
};
