...
struct NegateImpl {
  ...
  static NO_SANITIZE_UNDEFINED ResultType apply(A a) {
    if constexpr (is_decimal<A>) return negateOverflow(a);
...
struct FunctionUnaryArithmeticMonotonicity<NameNegate> {
  ...
  static IFunction::Monotonicity get(const IDataType &original_type, const Field &left, const Field &right) {
    ...
    if (const DataTypeLowCardinality *t = typeid_cast<const DataTypeLowCardinality *>(type)) type = t->getDictionaryType().get();
    if (const DataTypeNullable *t = typeid_cast<const DataTypeNullable *>(type)) type = t->getNestedType().get();

    /// If the input is signed, assume monotonic.
    /// Not fully correct because of the corner case -INT64_MIN == INT64_MIN and similar.
    if (!type->isValueRepresentedByUnsignedInteger()) return {.is_monotonic = true, .is_positive = false, .is_strict = true};
    ...
    auto will_overflow = [](const Field &f, bool if_null) -> bool {
      switch (f.getType()) {
        case Field::Types::UInt64:
          return f.safeGet<UInt64>() > 1ul << 63;
        case Field::Types::UInt128:
          return f.safeGet<UInt128>() > UInt128(1) << 127;
        case Field::Types::UInt256:
          return f.safeGet<UInt256>() > UInt256(1) << 255;
        default:
          break;
      }
      if (f.isPositiveInfinity()) return true;
      if (f.isNegativeInfinity()) return false;
      if (f.isNull()) return if_null;
