...
enum class OutputFormatting : uint8_t { SingleLine, MultiLine };

enum class ErrorHandling : uint8_t { Exception, Null };
...
class FunctionFormatQuery : public IFunction {
 public:
  ...
  DataTypePtr getReturnTypeImpl(const ColumnsWithTypeAndName &arguments) const override {
    ...
    if (error_handling == ErrorHandling::Null) return std::make_shared<DataTypeNullable>(string_type);
    return string_type;
  }
  ...
  ColumnPtr executeImpl(const ColumnsWithTypeAndName &arguments, const DataTypePtr &, size_t input_rows_count) const override {
    ...
    if (const ColumnString *col_query_string = checkAndGetColumn<ColumnString>(col_query.get())) {
      ...
      if (error_handling == ErrorHandling::Null) return ColumnNullable::create(std::move(col_res), std::move(col_null_map));
      return col_res;
    }
 ...
 private:
  void formatVector(const ColumnString::Chars &data, const ColumnString::Offsets &offsets, ColumnString::Chars &res_data,
                    ColumnString::Offsets &res_offsets, ColumnUInt8::MutablePtr &res_null_map, size_t input_rows_count) const {
    ...
    for (size_t i = 0; i < input_rows_count; ++i) {
      ...
      try {
        ...
        if (error_handling == ErrorHandling::Null) {
          ...
          if (res_data_new_size > res_data.size()) res_data.resize(2 * res_data_new_size);
...
REGISTER_FUNCTION(formatQuery) {
  factory.registerFunction(
      "formatQuery",
      [](ContextPtr context) {
        return std::make_shared<FunctionFormatQuery>(context, "formatQuery", OutputFormatting::MultiLine, ErrorHandling::Exception);
      },
      FunctionDocumentation{.description = "Returns a formatted, possibly multi-line, version of the given SQL query. Throws in case of a "
                                           "parsing error.\n[example:multiline]",
                            ...
                            .examples{{"multiline", "SELECT formatQuery('select a,    b FRom tab WHERE a > 3 and  b < 3');",
                                       "SELECT\n"
                                       "    a,\n"
                                       "    b\n"
                                       "FROM tab\n"
...
REGISTER_FUNCTION(formatQueryOrNull) {
  factory.registerFunction(
      "formatQueryOrNull",
      [](ContextPtr context) {
        return std::make_shared<FunctionFormatQuery>(context, "formatQueryOrNull", OutputFormatting::MultiLine, ErrorHandling::Null);
      },
      FunctionDocumentation{.description = "Returns a formatted, possibly multi-line, version of the given SQL query. Returns NULL in case "
                                           "of a parsing error.\n[example:multiline]",
...
REGISTER_FUNCTION(formatQuerySingleLine) {
  factory.registerFunction(
      ...
      [](ContextPtr context) {
        return std::make_shared<FunctionFormatQuery>(context, "formatQuerySingleLine", OutputFormatting::SingleLine,
                                                     ErrorHandling::Exception);
      },
      FunctionDocumentation{.description = "Like formatQuery() but the returned formatted string contains no line breaks. Throws in case "
                                           "of a parsing error.\n[example:multiline]",
                            .syntax = "formatQuerySingleLine(query)",
                            .arguments = {{"query", "The SQL query to be formatted. [String](../../sql-reference/data-types/string.md)"}},
                            .returned_value = {"The formatted query", {"String"}},
                            .examples{{"multiline", "SELECT formatQuerySingleLine('select a,    b FRom tab WHERE a > 3 and  b < 3');",
...
REGISTER_FUNCTION(formatQuerySingleLineOrNull) {
  factory.registerFunction(
      ...
      [](ContextPtr context) {
        return std::make_shared<FunctionFormatQuery>(context, "formatQuerySingleLineOrNull", OutputFormatting::SingleLine,
                                                     ErrorHandling::Null);
      },
      FunctionDocumentation{.description = "Like formatQuery() but the returned formatted string contains no line breaks. Returns NULL in "
