...
class FunctionIfNotFinite : public IFunction {
 public:
  ...
  static FunctionPtr create(ContextPtr context) { return std::make_shared<FunctionIfNotFinite>(context); }
  ...
  DataTypePtr getReturnTypeImpl(const ColumnsWithTypeAndName& arguments) const override {
    ...
    auto if_type =
        FunctionFactory::instance().get("if", context)->build({{nullptr, is_finite_type, ""}, arguments[0], arguments[1]})->getResultType();
    return if_type;
  }
...
You can get a similar result by using the [ternary operator](/sql-reference/functions/conditional-functions#if): `isFinite(x) ? x : y`.
  ...
  FunctionDocumentation::Arguments arguments = {{"x", "Value to check if infinite.", {"Float*"}}, {"y", "Fallback value.", {"Float*"}}};
