...
void setVariant(UUID& uuid) { UUIDHelpers::getLowBytes(uuid) = (UUIDHelpers::getLowBytes(uuid) & rand_b_bits_mask) | variant_2_mask; }
...
void CounterFields::incrementCounter(UUID& uuid) {
  if (++counter == counter_limit) [[unlikely]] {
    ...
    UUIDHelpers::getLowBytes(uuid) = (UUIDHelpers::getLowBytes(uuid) & rand_b_with_counter_bits_mask) | variant_2_mask |
                                     ((counter & counter_low_bits_mask) << rand_b_low_bits_count);
  }
}

void CounterFields::generate(UUID& uuid, uint64_t timestamp) {
  const bool need_to_increment_counter =
      (last_timestamp == timestamp) || ((last_timestamp > timestamp) & (last_timestamp < timestamp + 10000));
...
void Data::generate(UUID& uuid, uint64_t timestamp) { fields.generate(uuid, timestamp); }
