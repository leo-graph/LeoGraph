...
struct NormalizeNFCImpl {
  ...
  static const UNormalizer2 *getNormalizer(UErrorCode *err) { return unorm2_getNFCInstance(err); }
};
...
struct NormalizeNFDImpl {
  ...
  static const UNormalizer2 *getNormalizer(UErrorCode *err) { return unorm2_getNFDInstance(err); }
};
...
struct NormalizeNFKCImpl {
  ...
  static const UNormalizer2 *getNormalizer(UErrorCode *err) { return unorm2_getNFKCInstance(err); }
};
...
struct NormalizeNFKDImpl {
  ...
  static const UNormalizer2 *getNormalizer(UErrorCode *err) { return unorm2_getNFKDInstance(err); }
};
...
struct NormalizeUTF8Impl {
  static void vector(const ColumnString::Chars &data, const ColumnString::Offsets &offsets, ColumnString::Chars &res_data,
    ...
    if (U_FAILURE(err)) throw Exception(ErrorCodes::CANNOT_NORMALIZE_STRING, "Normalization failed (getNormalizer): {}", u_errorName(err));
    ...
    for (size_t i = 0; i < input_rows_count; ++i) {
      ...
      if (from_size > 0) {
        ...
        u_strFromUTF8(from_uchars.data(), static_cast<int32_t>(from_uchars.size()), &from_code_points,
                      reinterpret_cast<const char *>(&data[current_from_offset]), static_cast<int32_t>(from_size), &err);
        if (U_FAILURE(err))
        ...
        int32_t to_code_points = unorm2_normalize(normalizer, from_uchars.data(), from_code_points, to_uchars.data(),
                                                  static_cast<int32_t>(to_uchars.size()), &err);
        if (U_FAILURE(err)) throw Exception(ErrorCodes::CANNOT_NORMALIZE_STRING, "Normalization failed (normalize): {}", u_errorName(err));

        size_t max_to_size = current_to_offset + 4 * to_code_points;
        if (res_data.size() < max_to_size) res_data.resize(max_to_size);

        int32_t to_size = 0;
        u_strToUTF8(reinterpret_cast<char *>(&res_data[current_to_offset]), static_cast<int32_t>(res_data.size() - current_to_offset),
                    &to_size, to_uchars.data(), to_code_points, &err);
        if (U_FAILURE(err)) throw Exception(ErrorCodes::CANNOT_NORMALIZE_STRING, "Normalization failed (strToUTF8): {}", u_errorName(err));
...
Normalizes a UTF-8 string according to the [NFC normalization form](https://en.wikipedia.org/wiki/Unicode_equivalence#Normal_forms).
  ...
  FunctionDocumentation::Arguments arguments_nfc = {{"str", "UTF-8 encoded input string.", {"String"}}};
  FunctionDocumentation::ReturnedValue returned_value_nfc = {"Returns the NFC normalized form of the UTF-8 string.", {"String"}};
  FunctionDocumentation::Examples examples_nfc = {{"Usage example",
                                                   R"(
SELECT
...
└──────────┴──────────────────┴────────────────┴────────────────────────┘
  ...
  FunctionDocumentation documentation_nfc = {description_nfc,    syntax_nfc,   arguments_nfc, {},
                                             returned_value_nfc, examples_nfc, introduced_in, category};
...
Normalizes a UTF-8 string according to the [NFD normalization form](https://en.wikipedia.org/wiki/Unicode_equivalence#Normal_forms).
  ...
  FunctionDocumentation::Arguments arguments_nfd = {{"str", "UTF-8 encoded input string.", {"String"}}};
  FunctionDocumentation::ReturnedValue returned_value_nfd = {"Returns the NFD normalized form of the UTF-8 string.", {"String"}};
  FunctionDocumentation::Examples examples_nfd = {{"Usage example",
                                                   R"(
SELECT
...
└──────────┴──────────────────┴────────────────┴────────────────────────┘
  ...
  FunctionDocumentation documentation_nfd = {description_nfd,    syntax_nfd,   arguments_nfd, {},
                                             returned_value_nfd, examples_nfd, introduced_in, category};
...
Normalizes a UTF-8 string according to the [NFKC normalization form](https://en.wikipedia.org/wiki/Unicode_equivalence#Normal_forms).
  ...
  FunctionDocumentation::Arguments arguments_nfkc = {{"str", "UTF-8 encoded input string.", {"String"}}};
  FunctionDocumentation::ReturnedValue returned_value_nfkc = {"Returns the NFKC normalized form of the UTF-8 string.", {"String"}};
  FunctionDocumentation::Examples examples_nfkc = {{"Usage example",
                                                    R"(
SELECT
...
└──────────┴─────────────────┘
  ...
  FunctionDocumentation documentation_nfkc = {description_nfkc,    syntax_nfkc,   arguments_nfkc, {},
                                              returned_value_nfkc, examples_nfkc, introduced_in,  category};
...
Normalizes a UTF-8 string according to the [NFKD normalization form](https://en.wikipedia.org/wiki/Unicode_equivalence#Normal_forms).
  ...
  FunctionDocumentation::Arguments arguments_nfkd = {{"str", "UTF-8 encoded input string.", {"String"}}};
  FunctionDocumentation::ReturnedValue returned_value_nfkd = {"Returns the NFKD normalized form of the UTF-8 string.", {"String"}};
  FunctionDocumentation::Examples examples_nfkd = {{"Usage example",
                                                    R"(
SELECT
...
└──────────┴─────────────────┘
  ...
  FunctionDocumentation documentation_nfkd = {description_nfkd,    syntax_nfkd,   arguments_nfkd, {},
                                              returned_value_nfkd, examples_nfkd, introduced_in,  category};
