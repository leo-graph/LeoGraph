...
class FunctionGeohashDecode : public IFunction {
 public:
  ...
  ColumnPtr executeImpl(const ColumnsWithTypeAndName &arguments, const DataTypePtr &, size_t input_rows_count) const override {
    ...
    throw Exception(ErrorCodes::ILLEGAL_COLUMN, "Unsupported argument type:{} of argument of function {}", arguments[0].column->getName(),
                    getName());
  }
};
...
Decodes any [geohash](https://en.wikipedia.org/wiki/Geohash)-encoded string into longitude and latitude coordinates.
  ...
  FunctionDocumentation::Arguments arguments = {{"hash_str", "Geohash-encoded string to decode.", {"String", "FixedString"}}};
  FunctionDocumentation::ReturnedValue returned_value = {"Returns a tuple of `(longitude, latitude)` with `Float64` precision.",
                                                         {"Tuple(Float64, Float64)"}};
  FunctionDocumentation::Examples examples = {{"Basic usage", "SELECT geohashDecode('ezs42') AS res",
                                               R"(
┌─res─────────────────────────────┐
