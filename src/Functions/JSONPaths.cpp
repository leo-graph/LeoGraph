 ...
 public:
  ...
  DataTypePtr getReturnTypeImpl(const DataTypes &data_types) const override {
    ...
    if (data_types[0]->getTypeId() != TypeIndex::Object)
      throw Exception(ErrorCodes::NUMBER_OF_ARGUMENTS_DOESNT_MATCH, "Function {} requires argument with type JSON, got: {}", getName(),
                      data_types[0]->getName());
  ...
  ColumnPtr executeImpl(const ColumnsWithTypeAndName &arguments, const DataTypePtr &, size_t) const override {
    ...
    if constexpr (Impl::with_types) return executeWithTypes(*column_object, type_object);
 ...
 private:
  ColumnPtr executeWithoutTypes(const ColumnObject &column_object) const {
    ...
    if constexpr (Impl::paths_mode == PathsMode::DYNAMIC_PATHS) {
      ...
      for (const auto &[path, _] : dynamic_path_columns) dynamic_paths.push_back(path);
      ...
      for (size_t i = 0; i != size; ++i) {
        for (const auto path : dynamic_paths) {
          ...
          if (!dynamic_path_columns.find(path)->second->isNullAt(i)) data.insertData(path.data(), path.size());
        }
    ...
    for (const auto &[path, _] : typed_path_columns) sorted_dynamic_and_typed_paths.push_back(path);
    for (const auto &[path, _] : dynamic_path_columns) sorted_dynamic_and_typed_paths.push_back(path);
    ...
    for (size_t i = 0; i != shared_data_offsets.size(); ++i) {
      ...
      for (size_t j = start; j != end; ++j) {
        ...
        while (sorted_paths_index != sorted_dynamic_and_typed_paths.size() &&
               sorted_dynamic_and_typed_paths[sorted_paths_index] < shared_data_path) {
  ...
  ColumnPtr executeWithTypes(const ColumnObject &column_object, const DataTypeObject &type_object) const {
    ...
    if constexpr (Impl::paths_mode == PathsMode::DYNAMIC_PATHS) {
      ...
      for (const auto &[path, _] : dynamic_path_columns) sorted_dynamic_paths.push_back(path);
      ...
      return ColumnMap::create(ColumnPtr(std::move(paths_column)), ColumnPtr(std::move(types_column)),
                               ColumnPtr(std::move(offsets_column)));
    }
    ...
    for (const auto &[path, type] : typed_path_types) sorted_typed_and_dynamic_paths_with_types.emplace_back(path, type->getName());
    for (const auto &[path, _] : dynamic_path_columns) sorted_typed_and_dynamic_paths_with_types.emplace_back(path, "");
    ...
    for (size_t i = 0; i != shared_data_offsets.size(); ++i) {
      ...
      for (size_t j = start; j != end; ++j) {
        ...
        while (sorted_paths_index != sorted_typed_and_dynamic_paths_with_types.size() &&
               sorted_typed_and_dynamic_paths_with_types[sorted_paths_index].first < shared_data_path) {
      ...
      for (; sorted_paths_index != sorted_typed_and_dynamic_paths_with_types.size(); ++sorted_paths_index) {
        ...
        if (auto it = dynamic_path_columns.find(path); it != dynamic_path_columns.end()) {
          /// Skip NULL values.
          if (it->second->isNullAt(i)) continue;
          type = getDynamicValueType(it->second, i);
        }
        paths_column->insertData(path.data(), path.size());
        types_column->insertData(type.data(), type.size());
  ...
  std::optional<String> getDynamicValueTypeFromSharedData(std::string_view value) const {
    ReadBufferFromMemory buf(value);
    auto type = decodeDataType(buf);
    if (isNothing(type)) return std::nullopt;
    return type->getName();
  }
};
...
Returns the list of all paths stored in each row in JSON column.
    ...
    FunctionDocumentation::Arguments arguments = {{"json", "JSON column.", {"JSON"}}};
...
Returns the list of all paths and their data types stored in each row in JSON column.
    ...
    FunctionDocumentation::Arguments arguments = {{"json", "JSON column.", {"JSON"}}};
    FunctionDocumentation::ReturnedValue returned_value = {"Returns a map of all paths and their data types in the JSON column.",
                                                           {"Map(String, String)"}};
    FunctionDocumentation::Examples examples = {{"Usage example",
                                                 R"(
CREATE TABLE test (json JSON(max_dynamic_paths=1)) ENGINE = Memory;
...
Returns the list of dynamic paths that are stored as separate subcolumns and their types in each row in JSON column.
    ...
    FunctionDocumentation::Arguments arguments = {{"json", "JSON column.", {"JSON"}}};
    FunctionDocumentation::ReturnedValue returned_value = {"Returns a map of dynamic paths and their data types in the JSON column.",
                                                           {"Map(String, String)"}};
    FunctionDocumentation::Examples examples = {{"Usage example",
                                                 R"(
CREATE TABLE test (json JSON(max_dynamic_paths=1)) ENGINE = Memory;
...
Returns the list of paths that are stored in shared data structure in JSON column.
    ...
    FunctionDocumentation::Arguments arguments = {{"json", "JSON column.", {"JSON"}}};
    FunctionDocumentation::ReturnedValue returned_value = {"Returns an array of paths stored in shared data structure in the JSON column.",
                                                           {"Array(String)"}};
    FunctionDocumentation::Examples examples = {{"Usage example",
                                                 R"(
CREATE TABLE test (json JSON(max_dynamic_paths=1)) ENGINE = Memory;
