...
using SphericalPointInRadians =
    boost::geometry::model::point<Float64, 2, boost::geometry::cs::spherical_equatorial<boost::geometry::radian>>;
...
struct ColumnToPointsConverter {
  static std::vector<Point> convert(ColumnPtr col) {
    ...
    for (size_t i = 0; i < col->size(); ++i) {
      ...
      if (isNaN(first) || isNaN(second)) throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT, "Point's component must not be NaN");
...
class LineStringSerializer {
 public:
  LineStringSerializer() : offsets(ColumnUInt64::create()) {}

  explicit LineStringSerializer(size_t n) : offsets(ColumnUInt64::create(n)) {}

  void add(const LineString<Point> &ring) {
    size += ring.size();
    offsets->insertValue(size);
    for (const auto &point : ring) point_serializer.add(point);
  }

  ColumnPtr finalize() { return ColumnArray::create(point_serializer.finalize(), std::move(offsets)); }
...
class MultiLineStringSerializer {
 public:
  MultiLineStringSerializer() : offsets(ColumnUInt64::create()) {}

  explicit MultiLineStringSerializer(size_t n) : offsets(ColumnUInt64::create(n)) {}

  void add(const MultiLineString<Point> &multilinestring) {
    size += multilinestring.size();
    offsets->insertValue(size);
    for (const auto &linestring : multilinestring) linestring_serializer.add(linestring);
  }

  ColumnPtr finalize() { return ColumnArray::create(linestring_serializer.finalize(), std::move(offsets)); }
...
class RingSerializer {
 public:
  RingSerializer() : offsets(ColumnUInt64::create()) {}

  explicit RingSerializer(size_t n) : offsets(ColumnUInt64::create(n)) {}
...
class PolygonSerializer {
 public:
  PolygonSerializer() : offsets(ColumnUInt64::create()) {}

  explicit PolygonSerializer(size_t n) : offsets(ColumnUInt64::create(n)) {}
  ...
  void add(const Polygon<Point> &polygon) {
    ...
    for (const auto &ring : polygon.inners()) ring_serializer.add(ring);
  }

  ColumnPtr finalize() { return ColumnArray::create(ring_serializer.finalize(), std::move(offsets)); }
...
class MultiPolygonSerializer {
 public:
  MultiPolygonSerializer() : offsets(ColumnUInt64::create()) {}

  explicit MultiPolygonSerializer(size_t n) : offsets(ColumnUInt64::create(n)) {}
  ...
  ColumnPtr finalize() { return ColumnArray::create(polygon_serializer.finalize(), std::move(offsets)); }
...
static void callOnGeometryDataType(DataTypePtr type, F &&f) {
  ...
  if (factory.get("Point")->equals(*type)) return f(ConverterType<ColumnToPointsConverter<Point>>());
  ...
  if (factory.get("Ring")->equals(*type)) return f(ConverterType<ColumnToRingsConverter<Point>>());

  if (factory.get("Polygon")->equals(*type)) return f(ConverterType<ColumnToPolygonsConverter<Point>>());
  if (factory.get("MultiPolygon")->equals(*type)) return f(ConverterType<ColumnToMultiPolygonsConverter<Point>>());
