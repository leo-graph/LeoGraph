...
class FunctionGetFuzzerData : public IFunction {
 ...
 public:
  ...
  [[maybe_unused]] static void update(const String &fuzz_data_) { fuzz_data = fuzz_data_; }
};

}  // namespace

REGISTER_FUNCTION(GetFuzzerData) { factory.registerFunction<FunctionGetFuzzerData>({}); }
