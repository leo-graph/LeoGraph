...
struct FunctionDetectLanguageImpl {
  static std::string_view codeISO(std::string_view code_string) {
    if (code_string.ends_with("-Latn")) code_string.remove_suffix(code_string.size() - 5);

    if (code_string.ends_with("-Hant")) code_string.remove_suffix(code_string.size() - 5);

    // Old deprecated codes
    if (code_string == "iw") return "he";

    if (code_string == "jw") return "jv";

    if (code_string == "in") return "id";

    if (code_string == "mo") return "ro";

    // Some languages do not have 2 letter codes, for example code for Cebuano is ceb
    if (code_string.size() != 2) return "other";
  ...
  static void vector(const ColumnString::Chars &data, const ColumnString::Offsets &offsets, ColumnString::Chars &res_data,
    ...
    for (size_t i = 0; i < input_rows_count; ++i) {
      ...
      if (UTF8::isValidUTF8(str, str_len)) {
        auto lang = CLD2::DetectLanguage(reinterpret_cast<const char *>(str), static_cast<int>(str_len), true, &is_reliable);
 ...
 public:
  ...
  ColumnPtr executeImpl(const ColumnsWithTypeAndName &arguments, const DataTypePtr &result_type, size_t input_rows_count) const override {
    ...
    for (size_t i = 0; i < input_rows_count; ++i) {
      ...
      if (UTF8::isValidUTF8(str, str_len)) {
        CLD2::DetectLanguageSummary(reinterpret_cast<const char *>(str), static_cast<int>(str_len), true, result_lang_top3, pc, bytes,
                                    &is_reliable);
...
The longer the input, the more precise the language detection will be.
  ...
  FunctionDocumentation::Arguments arguments_detect = {{"text_to_be_analyzed", "The text to analyze.", {"String"}}};
  FunctionDocumentation::ReturnedValue returned_value_detect = {
      "Returns the 2-letter ISO code of the detected language. Other possible results: `un` = unknown, can not detect any language, "
      "`other` = the detected language does not have 2 letter code.",
      {"String"}};
  FunctionDocumentation::Examples examples_detect = {{"Mixed language text",
                                                      "SELECT detectLanguage('Je pense que je ne parviendrai jamais à parler français "
                                                      "comme un natif. Where there\\'s a will, there\\'s a way.')",
                                                      "fr"}};
  ...
  FunctionDocumentation documentation_detect = {description_detect,    syntax_detect,   arguments_detect,     {},
                                                returned_value_detect, examples_detect, introduced_in_detect, category_detect};
...
Similar to the [`detectLanguage`](#detectLanguage) function, but `detectLanguageMixed` returns a `Map` of 2-letter language codes that are mapped to the percentage of the certain language in the text.
  ...
  FunctionDocumentation::Arguments arguments_mixed = {{"s", "The text to analyze", {"String"}}};
  FunctionDocumentation::ReturnedValue returned_value_mixed = {
      "Returns a map with keys which are 2-letter ISO codes and corresponding values which are a percentage of the text found for that "
      "language",
      {"Map(String, Float32)"}};
  ...
  FunctionDocumentation documentation_mixed = {description_mixed,    syntax_mixed,   arguments_mixed,     {},
                                               returned_value_mixed, examples_mixed, introduced_in_mixed, category_mixed};
