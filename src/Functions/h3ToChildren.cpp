 ...
 public:
  ...
  DataTypePtr getReturnTypeImpl(const DataTypes &arguments) const override {
    ...
    if (!WhichDataType(arg).isUInt8())
      throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT, "Illegal type {} of argument {} of function {}. Must be UInt8", arg->getName(),
                      2, getName());
    ...
    for (size_t row = 0; row < input_rows_count; ++row) {
      ...
      if (vec_size > MAX_ARRAY_SIZE)
        throw Exception(ErrorCodes::TOO_LARGE_ARRAY_SIZE,
                        ...
                        vec_size, toString(child_resolution));
...
Returns an array of child indexes for the given [H3](#h3-index) index at the specified resolution.
  ...
  FunctionDocumentation::Arguments arguments = {{"index", "Parent H3 index.", {"UInt64"}},
                                                {"resolution", "Resolution of the child indexes with range `[0, 15]`.", {"UInt8"}}};
  FunctionDocumentation::ReturnedValue returned_value = {"Returns an array of child H3 indexes at the specified resolution.",
                                                         {"Array(UInt64)"}};
  FunctionDocumentation::Examples examples = {{"Get child indexes at resolution 6",
