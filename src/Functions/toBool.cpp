...
class FunctionToBool : public IFunction {
 private:
  static String getReturnTypeName(const DataTypePtr& argument) { return argument->isNullable() ? "Nullable(Bool)" : "Bool"; }

 public:
  static constexpr auto name = "toBool";

  static FunctionPtr create(ContextPtr) { return std::make_shared<FunctionToBool>(); }
  ...
  ColumnPtr executeImpl(const ColumnsWithTypeAndName& arguments, const DataTypePtr& result_type, size_t) const override {
    ColumnsWithTypeAndName cast_args{arguments[0],
...
Converts an input value to a value of type Bool.
  ...
  FunctionDocumentation::Arguments arguments = {
      {"expr",
       "Expression returning a number or a string. For strings, accepts 'true' or 'false' (case-insensitive).",
       {"(U)Int*", "Float*", "String", "Expression"}}};
