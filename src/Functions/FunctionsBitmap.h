...
class FunctionBitmapBuildImpl : public IFunction {
 public:
  ...
  DataTypePtr getReturnTypeImpl(const DataTypes &arguments) const override {
    ...
    if (which.isUInt8())
      bitmap_function = AggregateFunctionFactory::instance().get(AggregateFunctionGroupBitmapData<UInt8>::name(), action, argument_types,
                                                                 params_row, properties);
    else if (which.isUInt16())
      bitmap_function = AggregateFunctionFactory::instance().get(AggregateFunctionGroupBitmapData<UInt16>::name(), action, argument_types,
                                                                 params_row, properties);
    else if (which.isUInt32())
      bitmap_function = AggregateFunctionFactory::instance().get(AggregateFunctionGroupBitmapData<UInt32>::name(), action, argument_types,
                                                                 params_row, properties);
    else if (which.isUInt64())
      bitmap_function = AggregateFunctionFactory::instance().get(AggregateFunctionGroupBitmapData<UInt64>::name(), action, argument_types,
                                                                 params_row, properties);
    else if (which.isInt8())
      bitmap_function = AggregateFunctionFactory::instance().get(AggregateFunctionGroupBitmapData<Int8>::name(), action, argument_types,
                                                                 params_row, properties);
    else if (which.isInt16())
      bitmap_function = AggregateFunctionFactory::instance().get(AggregateFunctionGroupBitmapData<Int16>::name(), action, argument_types,
                                                                 params_row, properties);
    else if (which.isInt32())
      bitmap_function = AggregateFunctionFactory::instance().get(AggregateFunctionGroupBitmapData<Int32>::name(), action, argument_types,
                                                                 params_row, properties);
    else if (which.isInt64())
      bitmap_function = AggregateFunctionFactory::instance().get(AggregateFunctionGroupBitmapData<Int64>::name(), action, argument_types,
                                                                 params_row, properties);
    else
      throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT, "Unexpected type {} of argument of function {}", array_type->getName(),
                      getName());
  ...
  ColumnPtr executeImpl(const ColumnsWithTypeAndName &arguments, const DataTypePtr &, size_t input_rows_count) const override {
    ...
    WhichDataType which(nested_type);
    if (which.isUInt8()) return executeBitmapData<UInt8>(argument_types, arguments, input_rows_count);
    if (which.isUInt16()) return executeBitmapData<UInt16>(argument_types, arguments, input_rows_count);
    if (which.isUInt32()) return executeBitmapData<UInt32>(argument_types, arguments, input_rows_count);
    if (which.isUInt64()) return executeBitmapData<UInt64>(argument_types, arguments, input_rows_count);
    if (which.isInt8()) return executeBitmapData<Int8>(argument_types, arguments, input_rows_count);
    if (which.isInt16()) return executeBitmapData<Int16>(argument_types, arguments, input_rows_count);
    if (which.isInt32()) return executeBitmapData<Int32>(argument_types, arguments, input_rows_count);
    if (which.isInt64()) return executeBitmapData<Int64>(argument_types, arguments, input_rows_count);
    throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT, "Unexpected type {} of argument of function {}", from_type->getName(), getName());
  }
 ...
 private:
  ...
  ColumnPtr executeBitmapData(DataTypes &argument_types, const ColumnsWithTypeAndName &arguments, size_t input_rows_count) const {
    ...
    AggregateFunctionProperties properties;
    auto action = NullsAction::EMPTY;
    AggregateFunctionPtr bitmap_function = AggregateFunctionFactory::instance().get(AggregateFunctionGroupBitmapData<T>::name(), action,
                                                                                    argument_types, params_row, properties);
...
class FunctionBitmapToArrayImpl : public IFunction {
 public:
  ...
  DataTypePtr getReturnTypeImpl(const DataTypes &arguments) const override {
    ...
    if (!(bitmap_type && bitmap_type->getFunctionName() == "groupBitmap"))
      throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT, "First argument for function {} must be a bitmap but it has type {}.",
                      getName(), arguments[0]->getName());
  ...
  ColumnPtr executeImpl(const ColumnsWithTypeAndName &arguments, const DataTypePtr &result_type, size_t input_rows_count) const override {
    ...
    if (which.isUInt8())
      ...
      throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT, "Unexpected type {} of argument of function {}", from_type->getName(),
                      getName());
 ...
 private:
  ...
  template <typename T>
  void executeIntType(const ColumnsWithTypeAndName &arguments, size_t input_rows_count, IColumn &res_data_col,
                      ColumnArray::Offsets &res_offsets) const {
    const ColumnAggregateFunction *column = typeid_cast<const ColumnAggregateFunction *>(arguments[0].column.get());
...
class FunctionBitmapSubset : public IFunction {
 public:
  ...
  ColumnPtr executeImpl(const ColumnsWithTypeAndName &arguments, const DataTypePtr &, size_t input_rows_count) const override {
    ...
    const DataTypeAggregateFunction *aggr_type = typeid_cast<const DataTypeAggregateFunction *>(from_type);
    WhichDataType which(aggr_type->getArgumentsDataTypes()[0]);
    if (which.isUInt8()) return executeIntType<UInt8>(arguments, input_rows_count);
    if (which.isUInt16()) return executeIntType<UInt16>(arguments, input_rows_count);
    if (which.isUInt32()) return executeIntType<UInt32>(arguments, input_rows_count);
    if (which.isUInt64()) return executeIntType<UInt64>(arguments, input_rows_count);
    if (which.isInt8()) return executeIntType<Int8>(arguments, input_rows_count);
    if (which.isInt16()) return executeIntType<Int16>(arguments, input_rows_count);
    if (which.isInt32()) return executeIntType<Int32>(arguments, input_rows_count);
    if (which.isInt64()) return executeIntType<Int64>(arguments, input_rows_count);
    throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT, "Unexpected type {} of argument of function {}", from_type->getName(), getName());
  }
 ...
 private:
  ...
  ColumnPtr executeIntType(const ColumnsWithTypeAndName &arguments, size_t input_rows_count) const {
    ...
    for (size_t i = 0; i < input_rows_count; ++i) {
      ...
      const AggregateFunctionGroupBitmapData<T> &bitmap_data_0 = *reinterpret_cast<const AggregateFunctionGroupBitmapData<T> *>(data_ptr_0);
      ...
      col_to->insertDefault();
      AggregateFunctionGroupBitmapData<T> &bitmap_data_2 = *reinterpret_cast<AggregateFunctionGroupBitmapData<T> *>(col_to->getData()[i]);
...
struct BitmapSubsetInRangeImpl {
 public:
  ...
  template <typename T>
  static void apply(const AggregateFunctionGroupBitmapData<T> &bitmap_data_0, UInt64 range_start, UInt64 range_end,
  ...
  DataTypePtr getReturnTypeImpl(const DataTypes &arguments) const override {
    ...
    for (size_t i = 0; i < 2; ++i) {
      ...
      if (array_type) {
        auto nested_type = array_type->getNestedType();
        WhichDataType which(nested_type);
        if (!(which.isUInt8() || which.isUInt16() || which.isUInt32() || which.isUInt64())) has_error = true;
      } else {
        has_error = true;
      }
      ...
      if (has_error)
        throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT,
                        ...
                        "but one of them has type {}.",
                        getName(), arguments[i + 1]->getName());
    }
    return arguments[0];
  }
...
class FunctionBitmapSelfCardinalityImpl : public IFunction {
 ...
 private:
  ...
  template <typename T>
  void executeIntType(const ColumnsWithTypeAndName &arguments, size_t input_rows_count,
...
class FunctionBitmapContains : public IFunction {
 public:
  ...
  DataTypePtr getReturnTypeImpl(const DataTypes &arguments) const override {
    ...
    if (!(bitmap_type0 && bitmap_type0->getFunctionName() == "groupBitmap"))
      throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT, "First argument for function {} must be a bitmap but it has type {}", getName(),
                      arguments[0]->getName());
    ...
    if (!first_aggregate_argument.isNativeInt() && !first_aggregate_argument.isNativeUInt())
      throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT,
                      "First argument for function {} must be a bitmap of an integer but it is a bitmap of {}", getName(),
                      bitmap_type0->getArgumentsDataTypes()[0]->getName());

    WhichDataType which(arguments[1].get());
    if (!which.isNativeInt() && !which.isNativeUInt())
      throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT,
                      "Second argument for function {} must be an native integer type but it has type {}", getName(),
                      arguments[1]->getName());
 ...
 private:
  ...
  void executeIntType(const ColumnsWithTypeAndName &arguments, size_t input_rows_count,
    ...
    if (is_column_const[0])
      container0 =
          &typeid_cast<const ColumnAggregateFunction &>(typeid_cast<const ColumnConst &>(*column_ptrs[0]).getDataColumn()).getData();
    else
 ...
 public:
  ...
  DataTypePtr getReturnTypeImpl(const DataTypes &arguments) const override {
    ...
    if (!(bitmap_type1 && bitmap_type1->getFunctionName() == "groupBitmap"))
      throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT, "Second argument for function {} must be a bitmap but it has type {}",
                      getName(), arguments[1]->getName());
    ...
    if (bitmap_type0->getArgumentsDataTypes()[0]->getTypeId() != bitmap_type1->getArgumentsDataTypes()[0]->getTypeId())
      throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT,
                      ...
                      bitmap_type0->getArgumentsDataTypes()[0]->getName(), bitmap_type1->getArgumentsDataTypes()[0]->getName());
 ...
 private:
  ...
  void executeIntType(const ColumnsWithTypeAndName &arguments, size_t input_rows_count,
    ...
    for (size_t i = 0; i < input_rows_count; ++i) {
      ...
      const AggregateFunctionGroupBitmapData<T> &bitmap_data_1 = *reinterpret_cast<const AggregateFunctionGroupBitmapData<T> *>(data_ptr_0);
      const AggregateFunctionGroupBitmapData<T> &bitmap_data_2 = *reinterpret_cast<const AggregateFunctionGroupBitmapData<T> *>(data_ptr_1);
...
class FunctionBitmap : public IFunction {
 public:
  ...
  ColumnPtr executeImpl(const ColumnsWithTypeAndName &arguments, const DataTypePtr &, size_t input_rows_count) const override {
    ...
    const DataTypeAggregateFunction *aggr_type = typeid_cast<const DataTypeAggregateFunction *>(from_type);
    WhichDataType which(aggr_type->getArgumentsDataTypes()[0]);
    if (which.isUInt8()) return executeBitmapData<UInt8>(arguments, input_rows_count);
    if (which.isUInt16()) return executeBitmapData<UInt16>(arguments, input_rows_count);
    if (which.isUInt32()) return executeBitmapData<UInt32>(arguments, input_rows_count);
    if (which.isUInt64()) return executeBitmapData<UInt64>(arguments, input_rows_count);
    if (which.isInt8()) return executeBitmapData<Int8>(arguments, input_rows_count);
    if (which.isInt16()) return executeBitmapData<Int16>(arguments, input_rows_count);
    if (which.isInt32()) return executeBitmapData<Int32>(arguments, input_rows_count);
    if (which.isInt64()) return executeBitmapData<Int64>(arguments, input_rows_count);
    throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT, "Unexpected type {} of argument of function {}", from_type->getName(), getName());
  }
 ...
 private:
  ...
  ColumnPtr executeBitmapData(const ColumnsWithTypeAndName &arguments, size_t input_rows_count) const {
    ...
    for (size_t i = 0; i < input_rows_count; ++i) {
      ...
      // bitmapAnd(RoaringBitMap, SmallSet) is slower than bitmapAnd(SmallSet, RoaringBitMap), so we can exchange the position of two
      // arguments for the speed
      ...
      auto need_exchange =
          (name == NameBitmapAnd::name) && bm_1->roaring_bitmap_with_small_set.isLarge() && bm_2->roaring_bitmap_with_small_set.isSmall();
