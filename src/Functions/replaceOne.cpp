...
Replaces the first occurrence of the substring `pattern` in `haystack` by the `replacement` string.
  ...
  FunctionDocumentation::Arguments arguments = {{"haystack", "The input string to search in.", {"String"}},
  ...
  FunctionDocumentation::Examples examples = {{"Replace first occurrence", "SELECT replaceOne('Hello, Hello world', 'Hello', 'Hi') AS res;",
                                               R"(
┌─res─────────────┐
│ Hi, Hello world │
└─────────────────┘
