...
const EVP_MD *getHashAlgorithm(const std::string_view &mode) { return EVP_MD_fetch(nullptr, std::string{mode}.c_str(), nullptr); }
...
class FunctionHMAC : public IFunction {
  ...
  static void fetchAndGroupSupportedAlgorithms() {
    ...
    EVP_MD_do_all_sorted(
        [](const EVP_MD * /* md */, const char *md_name, const char *alias, void *arg) {
          ...
          if (alias) (*algos_map)[primary_name].insert(alias);
        },
        &algorithms_map);
 ...
 public:
  ...
  ColumnPtr executeImpl(const ColumnsWithTypeAndName &arguments, const DataTypePtr &, size_t input_rows_count) const override {
    ...
    if (evp_md == nullptr)
      throw Exception(ErrorCodes::BAD_ARGUMENTS, "Invalid hash algorithm: '{}'. Supported algorithms are: {}.", mode,
    ...
    for (size_t row_idx = 0; row_idx < input_rows_count; ++row_idx) {
      ...
      const unsigned char *hmac_result =
          HMAC(evp_md, key_value.data(), static_cast<int>(key_value.size()), reinterpret_cast<const unsigned char *>(message_value.data()),
               message_value.size(), reinterpret_cast<unsigned char *>(result_ptr), &actual_digest_length);

      if (hmac_result == nullptr) throw Exception(ErrorCodes::OPENSSL_ERROR, "HMAC computation failed: {}", getOpenSSLErrors());
...
{}
  ...
  FunctionDocumentation::ReturnedValue returned_value = {"Returns a binary string containing the HMAC digest.", {"String"}};

  FunctionDocumentation::Examples examples = {{"Basic HMAC-SHA256",
                                               R"(
