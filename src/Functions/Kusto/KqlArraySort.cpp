...
class FunctionKqlArraySort : public KqlFunctionBase {
 public:
  ...
  DataTypePtr getReturnTypeImpl(const ColumnsWithTypeAndName &arguments) const override {
    if (arguments.empty())
      throw Exception(ErrorCodes::TOO_FEW_ARGUMENTS_FOR_FUNCTION, "Function {} needs at least one argument; passed {}.", getName(),
                      arguments.size());

    auto array_count = arguments.size();

    if (!isArray(arguments.at(array_count - 1).type)) --array_count;
    ...
    for (size_t index = 0; index < array_count; ++index) {
      ...
      if (!array_type)
        throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT, "Argument {} of function {} must be array. Found {} instead.", index + 1,
    ...
    for (size_t i = 0; i < array_count; ++i) data_types[i] = std::make_shared<DataTypeArray>(makeNullable(nested_types[i]));
  ...
  ColumnPtr executeImpl(const ColumnsWithTypeAndName &arguments, const DataTypePtr &result_type, size_t input_rows_count) const override {
    ...
    auto zipped = FunctionFactory::instance()
                      .get("arrayZip", context)
                      ->build(new_args)
                      ->execute(new_args, result_type, input_rows_count, /* dry_run = */ false);

    ColumnsWithTypeAndName sort_arg({{zipped, std::make_shared<DataTypeArray>(result_type), "zipped"}});
    auto sorted_tuple = FunctionFactory::instance()
                            .get(sort_function, context)
                            ->build(sort_arg)
                            ->execute(sort_arg, result_type, input_rows_count, /* dry_run = */ false);
    ...
    for (size_t i = 0; i < array_count; ++i) {
      if (null_indices.contains(i)) {
        ...
        DataTypePtr arg_type = std::make_shared<DataTypeArray>(makeNullable(nested_types[i]));
      ...
      if (null_index_in_array > 0) {
        ...
        for (size_t i = 0; i < array_count; ++i) {
          if (null_indices.contains(i)) {
            ...
            ColumnsWithTypeAndName slice_args_right({{ColumnWithTypeAndName(tuple_columns[i], arg_type, "array")}, slice_index});
...
Sorts one or more arrays in ascending order. The first array is sorted, and subsequent arrays are reordered to match the first array's sorted order. Null values are placed at the end. This is a KQL (Kusto Query Language) compatibility function.
  ...
  FunctionDocumentation documentation_asc = {description_asc,    syntax_asc,   arguments_asc,     {},
                                             returned_value_asc, examples_asc, introduced_in_asc, category_asc};
...
Sorts one or more arrays in descending order. The first array is sorted, and subsequent arrays are reordered to match the first array's sorted order. Null values are placed at the end. This is a KQL (Kusto Query Language) compatibility function.
  ...
  FunctionDocumentation::ReturnedValue returned_value_desc = {"Returns a tuple of arrays sorted in descending order.",
                                                              {"Tuple(Array, ...)"}};
  ...
  FunctionDocumentation documentation_desc = {description_desc,    syntax_desc,   arguments_desc,     {},
                                              returned_value_desc, examples_desc, introduced_in_desc, category_desc};
