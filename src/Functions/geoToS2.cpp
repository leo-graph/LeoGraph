...
class FunctionGeoToS2 : public IFunction {
 public:
  ...
  static FunctionPtr create(ContextPtr) { return std::make_shared<FunctionGeoToS2>(); }
  ...
  ColumnPtr executeImpl(const ColumnsWithTypeAndName &arguments, const DataTypePtr &, size_t input_rows_count) const override {
    ...
    if (!col_lat)
      throw Exception(ErrorCodes::ILLEGAL_COLUMN, "Illegal type {} of argument {} of function {}. Must be Float64",
                      arguments[0].type->getName(), 2, getName());
    ...
    for (size_t row = 0; row < input_rows_count; ++row) {
      ...
      if (isNaN(lon))
        throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT, "Illegal argument for longitude in function {}. It must not be NaN",
                        getName());
      if (!isFinite(lon))
        throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT, "Illegal argument for longitude in function {}. It must not be infinite",
                        getName());

      if (isNaN(lat))
        throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT, "Illegal argument for latitude in function {}. It must not be NaN",
                        getName());
      if (!isFinite(lat))
        throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT, "Illegal argument for latitude in function {}. It must not be infinite",
                        getName());
...
An S2 point index is a number that internally encodes a point on the surface of a unit sphere, unlike traditional (longitude, latitude) pairs. Use `s2ToGeo` to get the coordinates from an S2 point index.
  ...
  FunctionDocumentation::Arguments arguments = {{"lon", "Longitude.", {"Float64"}}, {"lat", "Latitude.", {"Float64"}}};
