...
If rounding causes an overflow (for example, `floor(-128, -1)`), the result is undefined.
    ...
    FunctionDocumentation::Arguments arguments = {
        ...
        {"N",
         "Optional. The number of decimal places to round to. Defaults to zero, which means rounding to an integer. Can be negative.",
         {"(U)Int*"}}};
    FunctionDocumentation::ReturnedValue returned_value = {"Returns a rounded number of the same type as `x`.",
                                                           {"Float*", "Decimal*", "(U)Int*"}};
    FunctionDocumentation::Examples examples = {{"Usage example", "SELECT floor(123.45, 1) AS rounded",
                                                 R"(
┌─rounded─┐
│   123.4 │
└─────────┘
            )"},
                                                {"Negative precision", "SELECT floor(123.45, -1)",
                                                 R"(
┌─floor(123.45, -1)─┐
│               120 │
└───────────────────┘
...
If rounding causes an overflow (for example, `ceiling(255, -1)`), the result is undefined.
    ...
    FunctionDocumentation::ReturnedValue returned_value = {"Returns a rounded number of the same type as `x`.",
                                                           {"Float*", "Decimal*", "(U)Int*"}};
    FunctionDocumentation::Examples examples = {{"Basic usage", "SELECT ceiling(123.45, 1) AS rounded",
                                                 R"(
┌─rounded─┐
│   123.5 │
└─────────┘
            )"},
                                                {"Negative precision", "SELECT ceiling(123.45, -1)",
                                                 R"(
┌─ceiling(123.45, -1)─┐
│                 130 │
└─────────────────────┘
...
Like [`floor`](#floor) but returns the rounded number with the largest absolute value less than or equal to that of `x`.
    ...
    FunctionDocumentation::ReturnedValue returned_value = {"Returns a rounded number of the same type as `x`.",
                                                           {"Float*", "Decimal*", "(U)Int*"}};
    FunctionDocumentation::Examples examples = {{"Basic usage", "SELECT truncate(123.499, 1) AS res;", "┌───res─┐\n│ 123.4 │\n└───────┘"}};
    FunctionDocumentation::IntroducedIn introduced_in = {1, 1};
    FunctionDocumentation::Category category = FunctionDocumentation::Category::Rounding;
    FunctionDocumentation documentation = {description, syntax, arguments, {}, returned_value, examples, introduced_in, category};
...
If rounding causes an overflow (for example, `round(255, -1)`), the result is undefined.
    ...
    FunctionDocumentation::Examples examples = {
        {"Float inputs", "SELECT number / 2 AS x, round(x) FROM system.numbers LIMIT 3;",
         R"(
┌───x─┬─round(x)─┐
│   0 │        0 │
│ 0.5 │        0 │
...
└─────┴──────────┘
        ...
        {"Decimal inputs", "SELECT cast(number / 2 AS  Decimal(10,4)) AS x, round(x) FROM system.numbers LIMIT 3;",
         R"(
┌───x─┬─round(x)─┐
│   0 │        0 │
│ 0.5 │        1 │
...
:::
    ...
    FunctionDocumentation::Arguments arguments = {
        ...
        {"[, N]", "Optional. The number of decimal places to round to. Defaults to `0`.", {"(U)Int*"}}};
    FunctionDocumentation::ReturnedValue returned_value = {"Returns a value rounded by the banker's rounding method.",
                                                           {"(U)Int*", "Decimal*", "Float*"}};
    FunctionDocumentation::Examples examples = {{"Basic usage",
...
If the value is less than the lower bound, the lower bound is returned.
    ...
    FunctionDocumentation::ReturnedValue returned_value = {
        "Returns a number rounded down to an element in `arr`. If the value is less than the lowest bound, the lowest bound is returned.",
        {"(U)Int*", "Float*"}};
