...
enum class LeastGreatest : uint8_t { Least, Greatest };
 ...
 private:
  ...
  ColumnPtr executeImpl(const ColumnsWithTypeAndName& arguments, const DataTypePtr& result_type, size_t input_rows_count) const override {
    ...
    for (const auto& argument : arguments) {
      if (!legacy_null_behavior && argument.type->onlyNull()) continue;  /// ignore NULL arguments
...
class LeastGreatestOverloadResolver : public IFunctionOverloadResolver {
 public:
  ...
  explicit LeastGreatestOverloadResolver(ContextPtr context_)
      : context(context_), legacy_null_behavior(context->getSettingsRef()[Setting::least_greatest_legacy_null_behavior]) {}
  ...
  FunctionBasePtr buildImpl(const ColumnsWithTypeAndName& arguments, const DataTypePtr& return_type) const override {
    ...
    return std::make_unique<FunctionToFunctionBaseAdaptor>(FunctionLeastGreatestGeneric<kind>::create(context), argument_types,
                                                           return_type);
  }
  ...
  DataTypePtr getReturnTypeImpl(const DataTypes& types) const override {
    ...
    if (types.size() == 2) {
      ...
      if (isNumber(arg_0_type) && isNumber(arg_1_type)) return SpecializedFunction::create(context)->getReturnTypeImpl(types);
    }
