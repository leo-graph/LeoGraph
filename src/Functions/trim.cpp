...
class FunctionTrim : public IFunction {
 public:
  ...
  ColumnPtr executeImpl(const ColumnsWithTypeAndName &arguments, const DataTypePtr &, size_t input_rows_count) const override {
    ...
    if (const ColumnString *col_input_string = checkAndGetColumn<ColumnString>(col_input_full.get())) {
      vector(col_input_string->getChars(), col_input_string->getOffsets(), custom_trim_characters, col_res->getChars(),
             col_res->getOffsets(), input_rows_count);
    } else if (const ColumnFixedString *col_input_fixed_string = checkAndGetColumn<ColumnFixedString>(col_input_full.get())) {
      vectorFixed(col_input_fixed_string->getChars(), col_input_fixed_string->getN(), custom_trim_characters, col_res->getChars(),
                  col_res->getOffsets(), input_rows_count);
    } else {
 ...
 private:
  ...
  void vectorImpl(const ColumnString::Chars &input_data, const ColumnString::Offsets &input_offsets,
                  const std::optional<SearchSymbols> &custom_trim_characters, ColumnString::Chars &res_data,
                  ColumnString::Offsets &res_offsets, size_t input_rows_count) const {
    ...
    for (size_t i = 0; i < input_rows_count; ++i) {
      executeImpl<do_trim_left, do_trim_right>(reinterpret_cast<const UInt8 *>(&input_data[prev_offset]), input_offsets[i] - prev_offset,
                                               custom_trim_characters, start, length);
  ...
  void vector(const ColumnString::Chars &input_data, const ColumnString::Offsets &input_offsets,
              const std::optional<SearchSymbols> &custom_trim_characters, ColumnString::Chars &res_data, ColumnString::Offsets &res_offsets,
  ...
  template <bool do_trim_left, bool do_trim_right>
  void vectorFixedImpl(const ColumnString::Chars &input_data, size_t n, const std::optional<SearchSymbols> &custom_trim_characters,
                       ColumnString::Chars &res_data, ColumnString::Offsets &res_offsets, size_t input_rows_count) const {
    res_offsets.resize_exact(input_rows_count);
    res_data.reserve_exact(input_data.size());
    ...
    for (size_t i = 0; i < input_rows_count; ++i) {
      executeImpl<do_trim_left, do_trim_right>(reinterpret_cast<const UInt8 *>(&input_data[prev_offset]), n, custom_trim_characters, start,
                                               length);
  ...
  void vectorFixed(const ColumnString::Chars &input_data, size_t n, const std::optional<SearchSymbols> &custom_trim_characters,
                   ColumnString::Chars &res_data, ColumnString::Offsets &res_offsets, size_t input_rows_count) const {
    dispatch([&](auto trim_left_v, auto trim_right_v) {
  ...
  template <bool do_trim_left, bool do_trim_right>
  static void executeImpl(const UInt8 *data, size_t size, const std::optional<SearchSymbols> &custom_trim_characters,
                          const UInt8 *&res_data, size_t &res_size) {
...
By default, removes common whitespace (ASCII) characters.
  ...
  FunctionDocumentation::ReturnedValue returned_value_left = {"Returns the string with specified characters trimmed from the left.",
                                                              {"String"}};
  FunctionDocumentation::Examples examples_left = {{"Usage example", "SELECT trimLeft('ClickHouse', 'Click');",
                                                    R"(
┌─trimLeft('Cl⋯', 'Click')─┐
│ House                    │
└──────────────────────────┘
...
By default, removes common whitespace (ASCII) characters.
  ...
  FunctionDocumentation::Arguments arguments_right = {
      ...
      {"trim_characters", "Optional characters to trim. If not specified, common whitespace characters are removed.", {"String"}}};
  FunctionDocumentation::ReturnedValue returned_value_right = {"Returns the string with specified characters trimmed from the right.",
                                                               {"String"}};
  FunctionDocumentation::Examples examples_right = {{"Usage example", "SELECT trimRight('ClickHouse','House');",
                                                     R"(
┌─trimRight('C⋯', 'House')─┐
│ Click                    │
└──────────────────────────┘
        )"}};
  FunctionDocumentation documentation_right = {description_right,    syntax_right,   arguments_right, {},
...
By default, removes common whitespace (ASCII) characters.
  ...
  FunctionDocumentation::Arguments arguments_both = {
      {"s", "String to trim.", {"String"}},
      {"trim_characters", "Optional. Characters to trim. If not specified, common whitespace characters are removed.", {"String"}}};
  FunctionDocumentation::ReturnedValue returned_value_both = {"Returns the string with specified characters trimmed from both ends.",
                                                              {"String"}};
  FunctionDocumentation::Examples examples_both = {{"Usage example", "SELECT trimBoth('$$ClickHouse$$', '$')",
                                                    R"(
┌─trimBoth('$$⋯se$$', '$')─┐
│ ClickHouse               │
└──────────────────────────┘
        )"}};
  FunctionDocumentation documentation_both = {description_both,    syntax_both,   arguments_both, {},
                                              returned_value_both, examples_both, introduced_in,  category};
