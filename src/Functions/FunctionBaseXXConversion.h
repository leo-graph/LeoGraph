...
enum class BaseXXDecodeErrorHandling : uint8_t { ThrowException, ReturnEmptyString };
    ...
    for (size_t row = 0; row < input_rows_count; ++row) {
      ...
      if (!decoded_size) {
        if constexpr (ErrorHandling == BaseXXDecodeErrorHandling::ThrowException)
          throw Exception(ErrorCodes::INCORRECT_DATA, "Invalid {} value ({}), cannot be decoded", name,
...
class FunctionBaseXXConversion : public IFunction {
 public:
  ...
  ColumnPtr executeImpl(const ColumnsWithTypeAndName &arguments, const DataTypePtr &, size_t input_rows_count) const override {
    ...
    throw Exception(ErrorCodes::ILLEGAL_COLUMN, "Illegal column {} of first argument of function {}, must be String or FixedString",
