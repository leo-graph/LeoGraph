 ...
 public:
  ...
  DataTypePtr getReturnTypeImpl(const DataTypes &arguments) const override {
    if (arguments.empty())
      throw Exception(ErrorCodes::TOO_FEW_ARGUMENTS_FOR_FUNCTION,
                      "Number of arguments for function {} doesn't match: passed {}, should be 1.", getName(), arguments.size());
    ...
    if (!isString(type)) throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT, "Cannot format {} as time string.", type.getName());
  ...
  ColumnPtr executeImpl(const ColumnsWithTypeAndName &arguments, const DataTypePtr &, size_t input_rows_count) const override {
    ...
    for (size_t i = 0; i < input_rows_count; ++i) {
      ...
      while (last_pos >= 0 && (str[last_pos] == ' ' || str[last_pos] == '.')) --last_pos;

      /// no valid characters
      if (last_pos < 0) {
        throw Exception(ErrorCodes::BAD_ARGUMENTS, "Invalid argument of function {}, no valid characters, str: \"{}\".", getName(),
                        String(str));
      }
      ...
      while (token_tail <= last_pos) {
        ...
        if (!scanUnsignedInteger(str, token_tail, last_pos)) {
          throw Exception(ErrorCodes::BAD_ARGUMENTS, "Invalid argument of function {}, number not found, str: \"{}\".", getName(),
                          String(str));
        }
        ...
        if (token_tail <= last_pos && str[token_tail] == '.') {
          token_tail++;
          if (!scanUnsignedInteger(str, token_tail, last_pos)) {
            throw Exception(ErrorCodes::BAD_ARGUMENTS, "Invalid argument of function {}, number not found after '.', str: \"{}\".",
                            getName(), String(str));
          }
        }
        ...
        if (!value.has_value()) {
          throw Exception(ErrorCodes::BAD_ARGUMENTS, "Invalid argument of function {}, can't convert String to Float64: \"{}\".", getName(),
                          String(base_str));
        }
        base = value.get();
        ...
        if (!scanUnit(str, token_tail, last_pos)) {
          throw Exception(ErrorCodes::BAD_ARGUMENTS, "Invalid argument of function {}, time unit not found, str: \"{}\".", getName(),
                          String(str));
        }
        ...
        {
          throw Exception(ErrorCodes::BAD_ARGUMENTS, "Invalid argument of function {}, can't parse the unit: \"{}\".", getName(), unit);
        }
