...
class FunctionIfNull : public IFunction {
 public:
  ...
  explicit FunctionIfNull(ContextPtr context_, bool use_variant_as_common_type_)
      : context(context_), use_variant_as_common_type(use_variant_as_common_type_) {}
  ...
  DataTypePtr getReturnTypeImpl(const DataTypes& arguments) const override {
    if (arguments[0]->onlyNull()) return arguments[1];

    if (!canContainNull(*arguments[0])) return arguments[0];
    ...
    if (use_variant_as_common_type || has_variant) return getLeastSupertypeOrVariant(args);
    return getLeastSupertype(args);
  }

  ColumnPtr executeImpl(const ColumnsWithTypeAndName& arguments, const DataTypePtr& result_type, size_t input_rows_count) const override {
    /// Always null.
    if (arguments[0].type->onlyNull()) return arguments[1].column;

    /// Could not contain nulls, so nullIf makes no sense.
    if (!canContainNull(*arguments[0].type)) return arguments[0].column;
...
Returns an alternative value if the first argument is `NULL`.
  ...
  FunctionDocumentation::Arguments arguments = {{"x", "The value to check for `NULL`.", {"Any"}},
