...
#if USE_EMBEDDED_COMPILER
  static llvm::Value *apply(llvm::IRBuilder<> &builder, llvm::Value *a, llvm::Value *b) { return builder.CreateAnd(a, b); }
...
#if USE_EMBEDDED_COMPILER
  static llvm::Value *apply(llvm::IRBuilder<> &builder, llvm::Value *a, llvm::Value *b) { return builder.CreateOr(a, b); }
...
#if USE_EMBEDDED_COMPILER
  static llvm::Value *apply(llvm::IRBuilder<> &builder, llvm::Value *a, llvm::Value *b) { return builder.CreateXor(a, b); }
...
struct NotImpl {
  ...
  static ResultType apply(A a) { return !static_cast<bool>(a); }

#if USE_EMBEDDED_COMPILER
  static llvm::Value *apply(llvm::IRBuilder<> &builder, llvm::Value *a) { return builder.CreateNot(a); }
#endif
};
 ...
 public:
  ...
  bool isCompilableImpl(const DataTypes &arguments, const DataTypePtr &) const override {
    for (const auto &arg : arguments) {
      if (!canBeNativeType(arg)) return false;
    }
    return true;
  }
  ...
  llvm::Value *compileImpl(llvm::IRBuilderBase &builder, const ValuesWithType &values, const DataTypePtr &result_type) const override {
    ...
    if (useDefaultImplementationForNulls() || !result_type->isNullable()) {
      ...
      auto *nullable_result_with_value = b.CreateInsertValue(nullable_result, b.CreateSelect(is_true, b.getInt8(1), b.getInt8(0)), {0});
