...
UInt8 isValidASCII(const UInt8 *data, UInt64 len) {
  ...
  UInt8 res = 0;
  for (UInt64 i = 0; i < len; ++i) res |= data[i];
  return res <= 0x7F;
}
...
class FunctionIsValidASCII : public IFunction {
 public:
  ...
  static FunctionPtr create(ContextPtr) { return std::make_shared<DB::FunctionIsValidASCII>(); }
  ...
  ColumnPtr executeImpl(const ColumnsWithTypeAndName &arguments, const DataTypePtr &, size_t input_rows_count) const override {
    ...
    if (const auto *col_str = checkAndGetColumn<ColumnString>(column.column.get())) {
      ...
      throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT, "Cannot apply function {} to column {}, expected String or FixedString",
...
REGISTER_FUNCTION(IsValidASCII) {
  factory.registerFunction<DB::FunctionIsValidASCII>(DB::FunctionDocumentation{
      .description =
          R"(Returns 1 if the input String or FixedString contains only ASCII bytes (0x00–0x7F), otherwise 0. Optimized for the positive case (the input _is_ valid ASCII).)",
