 ...
 public:
  ...
  DataTypePtr getReturnTypeImpl(const ColumnsWithTypeAndName &arguments) const override {
    ...
    if (!isInterval(arguments[1].type))
      throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT, "Illegal type {} of 2nd argument of function {}. Should be an interval",
  ...
  ColumnPtr executeImpl(const ColumnsWithTypeAndName &arguments, const DataTypePtr &, size_t input_rows_count) const override {
    if (!isDateOrDate32OrDateTimeOrDateTime64(arguments[0].type) && !isString(arguments[0].type))
      throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT,
                      "Illegal type {} of 1st argument of function {}. Should be a date or a date with time", arguments[0].type->getName(),
                      getName());
...
addDate(datetime, interval)
  ...
  FunctionDocumentation::ReturnedValue returned_value_addDate = {
      "Returns date or date with time obtained by adding `interval` to `datetime`.", {"Date", "Date32", "DateTime", "DateTime64"}};
  FunctionDocumentation::Examples examples_addDate = {{"Add interval to date", R"(
...
└──────────────────────────┘
  ...
  FunctionDocumentation documentation_addDate = {description_addDate,    syntax_addDate,   arguments_addDate,     {},
                                                 returned_value_addDate, examples_addDate, introduced_in_addDate, category_addDate};
...
subDate(datetime, interval)
  ...
  FunctionDocumentation::ReturnedValue returned_value_subDate = {
      "Returns date or date with time obtained by subtracting `interval` from `datetime`.", {"Date", "Date32", "DateTime", "DateTime64"}};
  FunctionDocumentation::Examples examples_subDate = {{"Subtract interval from date", R"(
...
└──────────────────────────────────────────────────┘
  ...
  FunctionDocumentation documentation_subDate = {description_subDate,    syntax_subDate,   arguments_subDate,     {},
                                                 returned_value_subDate, examples_subDate, introduced_in_subDate, category_subDate};
