...
class FunctionColorSRGBToOKLAB : public ColorConversionFromSRGBBase<FunctionColorSRGBToOKLAB> {
 public:
  ...
  explicit FunctionColorSRGBToOKLAB(ContextPtr context_) : ColorConversionFromSRGBBase(context_) {}
...
REGISTER_FUNCTION(ColorSRGBToOKLAB) {
  ...
  FunctionDocumentation::Arguments arguments = {
      ...
      {"gamma",
       "Optional. Exponent that is used to linearize sRGB by applying `(x / 255)^gamma` to each channel `x`. Defaults to `2.2`.",
       {"Float64"}}};
  FunctionDocumentation::ReturnedValue returned_value = {"Returns a tuple (L, a, b) representing the OKLAB color space values.",
                                                         {"Tuple(Float64, Float64, Float64)"}};
  FunctionDocumentation::Examples examples = {{"Convert sRGB to OKLAB",
                                               R"(
