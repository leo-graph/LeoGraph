...
class FunctionConvertCharset : public IFunction {
 private:
  struct Converter : private boost::noncopyable {
    ...
    explicit Converter(const String &charset) {
      ...
      if (U_SUCCESS(status)) ucnv_setToUCallBack(impl, UCNV_TO_U_CALLBACK_SUBSTITUTE, nullptr, nullptr, nullptr, &status);

      if (U_SUCCESS(status)) ucnv_setFromUCallBack(impl, UCNV_FROM_U_CALLBACK_SUBSTITUTE, nullptr, nullptr, nullptr, &status);

      if (!U_SUCCESS(status))
        throw Exception(ErrorCodes::CANNOT_CREATE_CHARSET_CONVERTER, "Cannot create UConverter with charset {}, error: {}", charset,
                        String(u_errorName(status)));
    }

    ~Converter() { ucnv_close(impl); }
  };
  ...
  static void convert(const String &from_charset, const String &to_charset, const ColumnString::Chars &from_chars,
                      const ColumnString::Offsets &from_offsets, ColumnString::Chars &to_chars, ColumnString::Offsets &to_offsets,
    ...
    for (size_t i = 0; i < input_rows_count; ++i) {
      ...
      if (0 != from_string_size) {
        ...
        UErrorCode status = U_ZERO_ERROR;
        int32_t res = ucnv_toUChars(converter_from->impl, uchars.data(), static_cast<int32_t>(uchars.size()),
                                    reinterpret_cast<const char *>(&from_chars[current_from_offset]),
                                    static_cast<int32_t>(from_string_size), &status);

        if (!U_SUCCESS(status))
          throw Exception(ErrorCodes::CANNOT_CONVERT_CHARSET, "Cannot convert from charset {}, error: {}", from_charset,
                          String(u_errorName(status)));
        ...
        res = ucnv_fromUChars(converter_to->impl, reinterpret_cast<char *>(&to_chars[current_to_offset]), max_to_size, uchars.data(), res,
                              &status);

        if (!U_SUCCESS(status))
          throw Exception(ErrorCodes::CANNOT_CONVERT_CHARSET, "Cannot convert to charset {}, error: {}", to_charset,
                          String(u_errorName(status)));

        current_to_offset += res;
      }

      if (to_chars.size() < current_to_offset) to_chars.resize(current_to_offset);
 ...
 public:
  ...
  ColumnPtr executeImpl(const ColumnsWithTypeAndName &arguments, const DataTypePtr &, size_t input_rows_count) const override {
    ...
    throw Exception(ErrorCodes::ILLEGAL_COLUMN, "Illegal column passed as first argument of function {} (must be ColumnString).",
                    getName());
  }
};
...
Returns string `s` converted from the encoding `from` to encoding `to`.
  ...
  FunctionDocumentation::Arguments arguments = {{"s", "Input string.", {"String"}},
  ...
  FunctionDocumentation::Examples examples = {{"Usage example", "SELECT convertCharset('Café', 'UTF-8', 'ISO-8859-1');",
                                               R"(
