...
class ExecutableFunctionJoinGet final : public IExecutableFunction {
 public:
  ExecutableFunctionJoinGet(const char *name_, ContextPtr context_, TableLockHolder table_lock_, StorageJoinPtr storage_join_,
...
class FunctionJoinGet final : public IFunctionBase {
 public:
  FunctionJoinGet(const char *name_, ContextPtr context_, TableLockHolder table_lock_, StorageJoinPtr storage_join_, String attr_name_,
...
std::pair<std::shared_ptr<StorageJoin>, String> getJoin(const ColumnsWithTypeAndName &arguments, ContextPtr context) {
  String join_name;
  if (const auto *name_col = checkAndGetColumnConst<ColumnString>(arguments[0].column.get())) {
    join_name = name_col->getValue<String>();
  } else
    throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT, "Illegal type {} of first argument of function joinGet, expected a const string.",
  ...
  if (!storage_join) throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT, "Table {} should have engine StorageJoin", join_name);
  ...
  if (const auto *name_col = checkAndGetColumnConst<ColumnString>(arguments[1].column.get())) {
    ...
    throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT,
                    "Illegal type {} of second argument of function joinGet, expected a const string.", arguments[1].type->getName());
...
FunctionBasePtr JoinGetOverloadResolver::buildImpl(const ColumnsWithTypeAndName &arguments, const DataTypePtr &) const {
  if (arguments.size() < 3)
    throw Exception(ErrorCodes::NUMBER_OF_ARGUMENTS_DOESNT_MATCH,
                    "Number of arguments for function '{}' doesn't match: passed {}, should be greater or equal to 3", getName(),
                    arguments.size());
  ...
  for (size_t i = 0; i < arguments.size(); ++i) {
    if (i >= 2) data_types[i - 2] = arguments[i].type;
  ...
  auto table_lock =
      storage_join->lockForShare(getContext()->getInitialQueryId(), getContext()->getSettingsRef()[Setting::lock_acquire_timeout]);
...
:::
  ...
  FunctionDocumentation::Arguments arguments_joinGet = {
      {"join_storage_table_name",
       "An identifier which indicates where to perform the search. The identifier is searched in the default database (see parameter "
       "`default_database` in the config file). To override the default database, use the `USE database_name` query or specify the "
       "database and the table through a dot, like `database_name.table_name`.",
       {"String"}},
  ...
  FunctionDocumentation::Examples examples_joinGet = {{"Usage example",
                                                       R"(
...
└────────────────────────────────────┘
  ...
  FunctionDocumentation documentation_joinGet = {description_joinGet,    syntax_joinGet,   arguments_joinGet,     {},
                                                 returned_value_joinGet, examples_joinGet, introduced_in_joinGet, category_joinGet};
...
:::
  ...
  FunctionDocumentation::Arguments arguments_joinGetOrNull = {
      {"join_storage_table_name",
       "An identifier which indicates where to perform the search. The identifier is searched in the default database (see parameter "
       "default_database in the config file). To override the default database, use the `USE database_name` query or specify the database "
       "and the table through a dot, like `database_name.table_name`.",
       {"String"}},
      {"value_column", "The name of the column of the table that contains required data.", {"const String"}},
      {"join_keys", "A list of join keys.", {"Any"}}};
  FunctionDocumentation::ReturnedValue returned_value_joinGetOrNull = {
      "Returns a list of values corresponding to the list of keys, or `NULL` if a key is not found.", {"Any"}};
  FunctionDocumentation::Examples examples_joinGetOrNull = {{"Usage example",
                                                             R"(
CREATE TABLE db_test.id_val(`id` UInt32, `val` UInt32) ENGINE = Join(ANY, LEFT, id);
INSERT INTO db_test.id_val VALUES (1,11)(2,12)(4,13);
...
└───────────────────────────────────────────────────┴─────────────────────────────────────────────────────┘
  ...
  FunctionDocumentation documentation_joinGetOrNull = {
      description_joinGetOrNull,    syntax_joinGetOrNull,   arguments_joinGetOrNull,     {},
      returned_value_joinGetOrNull, examples_joinGetOrNull, introduced_in_joinGetOrNull, category_joinGetOrNull};

  // joinGet
  factory.registerFunction(
      "joinGet", [](ContextPtr ctx) { return JoinGetOverloadResolver::create("joinGet", false, std::move(ctx)); }, documentation_joinGet);
  // joinGetOrNull
  factory.registerFunction(
      "joinGetOrNull", [](ContextPtr ctx) { return JoinGetOverloadResolver::create("joinGetOrNull", true, std::move(ctx)); },
