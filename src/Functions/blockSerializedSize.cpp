...
class FunctionBlockSerializedSize : public IFunction {
 public:
  ...
  static FunctionPtr create(ContextPtr) { return std::make_shared<FunctionBlockSerializedSize>(); }
  ...
  ColumnPtr executeImpl(const ColumnsWithTypeAndName &arguments, const DataTypePtr &, size_t input_rows_count) const override {
    ...
    for (const auto &arg : arguments) size += columnsSerializedSizeOne(arg);
  ...
  static UInt64 columnsSerializedSizeOne(const ColumnWithTypeAndName &elem) {
    ...
    serialization->serializeBinaryBulkWithMultipleStreams(*full_column, 0 /** offset */, 0 /** limit */, settings, state);
