...
class FunctionChar : public IFunction {
 public:
  ...
  DataTypePtr getReturnTypeImpl(const DataTypes &arguments) const override {
    if (arguments.empty())
      throw Exception(ErrorCodes::TOO_FEW_ARGUMENTS_FOR_FUNCTION, "Number of arguments for function {} can't be {}, should be at least 1",
    ...
    for (const auto &arg : arguments) {
      ...
      if (!(which.isInt() || which.isUInt() || which.isFloat()))
        throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT,
                        "Illegal type {} of argument of function {}, must be Int, UInt or Float number", arg->getName(), getName());
    }
  ...
  ColumnPtr executeImpl(const ColumnsWithTypeAndName &arguments, const DataTypePtr &, size_t input_rows_count) const override {
    ...
    for (size_t row = 0; row < input_rows_count; ++row) out_offsets[row] = size_per_row + out_offsets[row - 1];
 ...
 private:
  template <typename T>
  bool executeNumber(const IColumn &src_data, ColumnString::Chars &out_vec, const size_t &column_idx, const size_t &rows,
                     const size_t &size_per_row) const {
    const ColumnVector<T> *src_data_concrete = checkAndGetColumn<ColumnVector<T>>(&src_data);

    if (!src_data_concrete) return false;

    for (size_t row = 0; row < rows; ++row) out_vec[row * size_per_row + column_idx] = static_cast<char>(src_data_concrete->getInt(row));
...
to `UInt8` with potential rounding and overflow.
  ...
  FunctionDocumentation::Examples examples = {{"Basic example", "SELECT char(104.1, 101, 108.9, 108.9, 111) AS hello;",
                                               R"(
┌─hello─┐
│ hello │
└───────┘
