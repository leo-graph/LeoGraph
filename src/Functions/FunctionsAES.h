...
std::string_view foldEncryptionKeyInMySQLCompatitableMode(size_t cipher_key_size, std::string_view key,
                                                          std::array<char, EVP_MAX_KEY_LENGTH> &folded_key);

const EVP_CIPHER *getCipherByName(std::string_view name);

enum class CompatibilityMode : uint8_t { MySQL, OpenSSL };
...
enum class CipherMode : uint8_t {
  ...
  RFC5116_AEAD_AES_GCM   // AEAD GCM with custom IV length and tag (HMAC) appended to the ciphertext, see
 ...
 private:
  ...
  DataTypePtr getReturnTypeImpl(const ColumnsWithTypeAndName &arguments) const override {
    auto optional_args = FunctionArgumentDescriptors{
        {"IV", static_cast<FunctionArgumentDescriptor::TypeValidator>(&isStringOrFixedString), nullptr,
         "Initialization vector binary string"},
    };

    if constexpr (compatibility_mode == OpenSSLDetails::CompatibilityMode::OpenSSL) {
      optional_args.emplace_back(FunctionArgumentDescriptor{"AAD",
                                                            static_cast<FunctionArgumentDescriptor::TypeValidator>(&isStringOrFixedString),
                                                            nullptr, "Additional authenticated data binary string for GCM mode"});
    }

    validateFunctionArguments(
        *this, arguments,
        FunctionArgumentDescriptors{
            {"mode", static_cast<FunctionArgumentDescriptor::TypeValidator>(&isStringOrFixedString), isColumnConst,
  ...
  ColumnPtr executeImpl(const ColumnsWithTypeAndName &arguments, const DataTypePtr &, size_t input_rows_count) const override {
    ...
    if (mode.empty() || !mode.starts_with("aes-")) throw Exception(ErrorCodes::BAD_ARGUMENTS, "Invalid mode: {}", mode);

    const auto *evp_cipher = getCipherByName(mode);
    if (evp_cipher == nullptr) throw Exception(ErrorCodes::BAD_ARGUMENTS, "Invalid mode: {}", mode);
    ...
    if (arguments.size() <= 3)
      ...
      if (arguments.size() <= 4) {
        ...
        if (cipher_mode != EVP_CIPH_GCM_MODE) throw Exception(ErrorCodes::BAD_ARGUMENTS, "AAD can be only set for GCM-mode");
  ...
  static ColumnPtr doEncrypt(const EVP_CIPHER *evp_cipher, size_t input_rows_count, const ColumnPtr &input_column,
                             const ColumnPtr &key_column, const ColumnPtr &iv_column, const ColumnPtr &aad_column) {
    if constexpr (compatibility_mode == OpenSSLDetails::CompatibilityMode::MySQL) {
      ...
      if (EVP_CIPHER_mode(evp_cipher) == EVP_CIPH_GCM_MODE) {
        return doEncryptImpl<CipherMode::RFC5116_AEAD_AES_GCM>(evp_cipher, input_rows_count, input_column, key_column, iv_column,
                                                               aad_column);
      }
  ...
  template <CipherMode mode>
  static ColumnPtr doEncryptImpl(const EVP_CIPHER *evp_cipher, size_t input_rows_count, const ColumnPtr &input_column,
                                 const ColumnPtr &key_column, [[maybe_unused]] const ColumnPtr &iv_column,
...

    {
      ...
      for (size_t row_idx = 0; row_idx < input_rows_count; ++row_idx) {
        ...
        if constexpr (mode == CipherMode::RFC5116_AEAD_AES_GCM) resulting_size += tag_size;
      }
    ...
    for (size_t row_idx = 0; row_idx < input_rows_count; ++row_idx) {
      ...
      if (iv_column) {
        ...
        if (iv_value.empty()) iv_value = std::string_view{};
      }
      ...
      if (!(input_value.empty() && block_size == 1 && mode != CipherMode::RFC5116_AEAD_AES_GCM)) {
        ...
        if constexpr (mode == CipherMode::RFC5116_AEAD_AES_GCM) {
          ...
          if (EVP_EncryptInit_ex(evp_ctx, evp_cipher, nullptr, nullptr, nullptr) != 1) onError("EVP_EncryptInit_ex");

          if (EVP_CIPHER_CTX_ctrl(evp_ctx, EVP_CTRL_AEAD_SET_IVLEN, safe_cast<int>(iv_value.size()), nullptr) != 1)
            onError("EVP_CIPHER_CTX_ctrl");

          if (EVP_EncryptInit_ex(evp_ctx, nullptr, nullptr, reinterpret_cast<const unsigned char *>(key_value.data()),
          ...
          if (aad_column) {
            ...
            int tmp_len = 0;
            if (!aad_data.empty() && EVP_EncryptUpdate(evp_ctx, nullptr, &tmp_len, reinterpret_cast<const unsigned char *>(aad_data.data()),
                                                       safe_cast<int>(aad_data.size())) != 1)
          ...
          if (EVP_EncryptInit_ex(evp_ctx, evp_cipher, nullptr, reinterpret_cast<const unsigned char *>(key_value.data()),
                                 reinterpret_cast<const unsigned char *>(iv_value.data())) != 1)
            onError("EVP_EncryptInit_ex");
        }
        ...
        if (EVP_EncryptUpdate(evp_ctx, reinterpret_cast<unsigned char *>(encrypted), &output_len,
        ...
        if (EVP_EncryptFinal_ex(evp_ctx, reinterpret_cast<unsigned char *>(encrypted), &output_len) != 1) onError("EVP_EncryptFinal_ex");
        __msan_unpoison(encrypted, output_len);  /// OpenSSL uses assembly which evades msan's analysis
        encrypted += output_len;
        ...
        // https://tools.ietf.org/html/rfc5116#section-5.1
        if constexpr (mode == CipherMode::RFC5116_AEAD_AES_GCM) {
          if (EVP_CIPHER_CTX_ctrl(evp_ctx, EVP_CTRL_AEAD_GET_TAG, tag_size, encrypted) != 1) onError("EVP_CIPHER_CTX_ctrl");
          encrypted += tag_size;
        }
      }
...
class FunctionDecrypt : public IFunction {
 ...
 private:
  ...
  DataTypePtr getReturnTypeImpl(const ColumnsWithTypeAndName &arguments) const override {
    ...
    if constexpr (use_null_when_decrypt_fail) return std::make_shared<DataTypeNullable>(std::make_shared<DataTypeString>());
  ...
  ColumnPtr executeImpl(const ColumnsWithTypeAndName &arguments, const DataTypePtr &, size_t input_rows_count) const override {
    ...
    if (arguments.size() <= 3) {
      ...
      if (arguments.size() <= 4) {
        ...
        const auto aad_column = arguments[4].column;
        result_column =
            doDecrypt<use_null_when_decrypt_fail>(evp_cipher, input_rows_count, input_column, key_column, iv_column, aad_column);
      }
    }
  ...
  static ColumnPtr doDecrypt(const EVP_CIPHER *evp_cipher, size_t input_rows_count, const ColumnPtr &input_column,
                             const ColumnPtr &key_column, const ColumnPtr &iv_column, const ColumnPtr &aad_column) {
    if constexpr (compatibility_mode == OpenSSLDetails::CompatibilityMode::MySQL) {
      return doDecryptImpl<CipherMode::MySQLCompatibility, use_null_when_decrypt_fail>(evp_cipher, input_rows_count, input_column,
                                                                                       key_column, iv_column, aad_column);
    } else {
      const auto cipher_mode = EVP_CIPHER_mode(evp_cipher);
      if (cipher_mode == EVP_CIPH_GCM_MODE) {
        return doDecryptImpl<CipherMode::RFC5116_AEAD_AES_GCM, use_null_when_decrypt_fail>(evp_cipher, input_rows_count, input_column,
                                                                                           key_column, iv_column, aad_column);
      }

      return doDecryptImpl<CipherMode::OpenSSLCompatibility, use_null_when_decrypt_fail>(evp_cipher, input_rows_count, input_column,
                                                                                         key_column, iv_column, aad_column);
    }
  ...
  static ColumnPtr doDecryptImpl(const EVP_CIPHER *evp_cipher, size_t input_rows_count, const ColumnPtr &input_column,
                                 const ColumnPtr &key_column, [[maybe_unused]] const ColumnPtr &iv_column,
                                 [[maybe_unused]] const ColumnPtr &aad_column) {
    using namespace OpenSSLDetails;
    ...
    for (size_t row_idx = 0; row_idx < input_rows_count; ++row_idx) {
      ...
      if constexpr (mode == CipherMode::RFC5116_AEAD_AES_GCM) {
        if (!input_value.empty()) {
          ...
          if (input_value.size() < tag_size)
            throw Exception(ErrorCodes::BAD_ARGUMENTS,
                            "Encrypted data is too short: only {} bytes, "
      ...
      if (!input_value.empty()) {
        // 1: Init CTX
        if constexpr (mode == CipherMode::RFC5116_AEAD_AES_GCM) {
          if (EVP_DecryptInit_ex(evp_ctx, evp_cipher, nullptr, nullptr, nullptr) != 1) onError("EVP_DecryptInit_ex");
          ...
          if (EVP_DecryptInit_ex(evp_ctx, nullptr, nullptr, reinterpret_cast<const unsigned char *>(key_value.data()),
                                 reinterpret_cast<const unsigned char *>(iv_value.data())) != 1)
          ...
          if (aad_column) {
            ...
            int tmp_len = 0;
            if (!aad_data.empty() && EVP_DecryptUpdate(evp_ctx, nullptr, &tmp_len, reinterpret_cast<const unsigned char *>(aad_data.data()),
                                                       safe_cast<int>(aad_data.size())) != 1)
          ...
          if (EVP_DecryptInit_ex(evp_ctx, evp_cipher, nullptr, reinterpret_cast<const unsigned char *>(key_value.data()),
                                 reinterpret_cast<const unsigned char *>(iv_value.data())) != 1)
            onError("EVP_DecryptInit_ex");
        }
        ...
        // 2: Feed the data to the cipher
        int output_len = 0;
        if (EVP_DecryptUpdate(evp_ctx, reinterpret_cast<unsigned char *>(decrypted), &output_len,
                              reinterpret_cast<const unsigned char *>(input_value.data()), static_cast<int>(input_value.size())) != 1) {
          if constexpr (!use_null_when_decrypt_fail) onError("EVP_DecryptUpdate");
          decrypt_fail = true;
        } else {
          ...
          if constexpr (mode == CipherMode::RFC5116_AEAD_AES_GCM) {
            ...
            if (EVP_CIPHER_CTX_ctrl(evp_ctx, EVP_CTRL_AEAD_SET_TAG, tag_size, tag) != 1) onError("EVP_CIPHER_CTX_ctrl");
          }
          ...
          if (!decrypt_fail && EVP_DecryptFinal_ex(evp_ctx, reinterpret_cast<unsigned char *>(decrypted), &output_len) != 1) {
            if constexpr (!use_null_when_decrypt_fail) onError("EVP_DecryptFinal_ex");
            decrypt_fail = true;
          } else {
            __msan_unpoison(decrypted, output_len);  /// OpenSSL uses assembly which evades msan's analysis
            decrypted += output_len;
