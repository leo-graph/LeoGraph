...
class FunctionIn : public IFunction {
 public:
  ...
  ColumnPtr executeImpl(const ColumnsWithTypeAndName &arguments, bool dry_run, size_t input_rows_count) const {
    if (ignore_set) return ColumnUInt8::create(input_rows_count, static_cast<UInt8>(0));
    ...
    if (!column_set) column_set = checkAndGetColumn<const ColumnSet>(column_set_ptr.get());
    if (!column_set)
      throw Exception(ErrorCodes::ILLEGAL_COLUMN, "Second argument for function '{}' must be Set; found {}", getName(),
                      column_set_ptr->getName());
    ...
    if (!future_set) {
      if (dry_run) return ColumnUInt8::create(input_rows_count, static_cast<UInt8>(0));
    ...
    if (tuple && set_types.size() != 1 && set_types.size() == tuple->tupleSize()) {
      ...
      for (size_t i = 0; i < tuple_size; ++i) columns_of_key_columns.emplace_back(tuple_columns[i], tuple_types[i], "_" + toString(i));
    } else
    ...
    if (columns_of_key_columns.size() == 1) {
      ...
      if (typeid_cast<const ColumnConst *>(arg.column.get())) is_const = true;
    }

    auto res = set->execute(columns_of_key_columns, negative);

    if (is_const) res = ColumnUInt8::create(input_rows_count, static_cast<UInt8>(res->getUInt(0)));

    if (res->size() != input_rows_count)
      throw Exception(ErrorCodes::LOGICAL_ERROR, "Output size is different from input size, expect {}, get {}", input_rows_count,
                      res->size());
...
Checks if the left operand is a member of the right operand set. Returns 1 if it is, 0 otherwise. NULL values in the left operand are skipped (treated as not in the set).
  ...
  FunctionDocumentation documentation_in = {description_in,    syntax_in,   arguments_in,     {},
                                            returned_value_in, examples_in, introduced_in_in, category_in};
  if (ignore_set) documentation_in.description = String(documentation_in.description) + ignore_set_description_suffix;
  String full_name_in = "in";
  full_name_in += suffix;
  factory.registerFunction(
      full_name_in, [ignore_set, n = full_name_in](ContextPtr) { return FunctionIn::create(n, false, true, ignore_set); },
...
Same as `in`, but uses global set distribution in distributed queries. The set is sent to all remote servers.
  ...
  FunctionDocumentation documentation_globalIn = {description_globalIn,    syntax_globalIn,   arguments_globalIn,     {},
                                                  returned_value_globalIn, examples_globalIn, introduced_in_globalIn, category_globalIn};
  if (ignore_set) documentation_globalIn.description = String(documentation_globalIn.description) + ignore_set_description_suffix;
  String full_name_globalIn = "globalIn";
  full_name_globalIn += suffix;
  factory.registerFunction(
      full_name_globalIn, [ignore_set, n = full_name_globalIn](ContextPtr) { return FunctionIn::create(n, false, true, ignore_set); },
...
Checks if the left operand is NOT a member of the right operand set. Returns 1 if it is not in the set, 0 otherwise. NULL values in the left operand are skipped.
  ...
  FunctionDocumentation documentation_notIn = {description_notIn,    syntax_notIn,   arguments_notIn,     {},
                                               returned_value_notIn, examples_notIn, introduced_in_notIn, category_notIn};
  if (ignore_set) documentation_notIn.description = String(documentation_notIn.description) + ignore_set_description_suffix;
  String full_name_notIn = "notIn";
  full_name_notIn += suffix;
  factory.registerFunction(
      full_name_notIn, [ignore_set, n = full_name_notIn](ContextPtr) { return FunctionIn::create(n, true, true, ignore_set); },
...
Same as `notIn`, but uses global set distribution in distributed queries. The set is sent to all remote servers.
  ...
  FunctionDocumentation documentation_globalNotIn = {
      description_globalNotIn,    syntax_globalNotIn,   arguments_globalNotIn,     {},
      returned_value_globalNotIn, examples_globalNotIn, introduced_in_globalNotIn, category_globalNotIn};
  if (ignore_set) documentation_globalNotIn.description = String(documentation_globalNotIn.description) + ignore_set_description_suffix;
  String full_name_globalNotIn = "globalNotIn";
  full_name_globalNotIn += suffix;
  factory.registerFunction(
      full_name_globalNotIn, [ignore_set, n = full_name_globalNotIn](ContextPtr) { return FunctionIn::create(n, true, true, ignore_set); },
...
Checks if the left operand is a member of the right operand set. Unlike `in`, NULL values are not skipped: NULL is compared with set elements, and NULL = NULL evaluates to true.
  ...
  FunctionDocumentation documentation_nullIn = {description_nullIn,    syntax_nullIn,   arguments_nullIn,     {},
                                                returned_value_nullIn, examples_nullIn, introduced_in_nullIn, category_nullIn};
  if (ignore_set) documentation_nullIn.description = String(documentation_nullIn.description) + ignore_set_description_suffix;
  String full_name_nullIn = "nullIn";
  full_name_nullIn += suffix;
  factory.registerFunction(
      full_name_nullIn, [ignore_set, n = full_name_nullIn](ContextPtr) { return FunctionIn::create(n, false, false, ignore_set); },
...
Same as `nullIn`, but uses global set distribution in distributed queries. The set is sent to all remote servers.
  ...
  FunctionDocumentation documentation_globalNullIn = {
      description_globalNullIn,    syntax_globalNullIn,   arguments_globalNullIn,      {},
      returned_value_globalNullIn, examples_globalNullIn, introduced_in_globalNulllIn, category_globalNullIn};
  if (ignore_set) documentation_globalNullIn.description = String(documentation_globalNullIn.description) + ignore_set_description_suffix;
  String full_name_globalNullIn = "globalNullIn";
  full_name_globalNullIn += suffix;
  factory.registerFunction(
      full_name_globalNullIn,
      [ignore_set, n = full_name_globalNullIn](ContextPtr) { return FunctionIn::create(n, false, false, ignore_set); },
...
Checks if the left operand is NOT a member of the right operand set. Unlike `notIn`, NULL values are not skipped: NULL is compared with set elements, and NULL = NULL evaluates to true.
  ...
  FunctionDocumentation documentation_notNullIn = {
      description_notNullIn,    syntax_notNullIn,   arguments_notNullIn,      {},
      returned_value_notNullIn, examples_notNullIn, introduced_in_notNulllIn, category_notNullIn};
  if (ignore_set) documentation_notNullIn.description = String(documentation_notNullIn.description) + ignore_set_description_suffix;
  String full_name_notNullIn = "notNullIn";
  full_name_notNullIn += suffix;
  factory.registerFunction(
      full_name_notNullIn, [ignore_set, n = full_name_notNullIn](ContextPtr) { return FunctionIn::create(n, true, false, ignore_set); },
...
Same as `notNullIn`, but uses global set distribution in distributed queries. The set is sent to all remote servers.
  ...
  FunctionDocumentation documentation_globalNotNullIn = {
      description_globalNotNullIn,    syntax_globalNotNullIn,   arguments_globalNotNullIn,      {},
      returned_value_globalNotNullIn, examples_globalNotNullIn, introduced_in_globalNotNulllIn, category_globalNotNullIn};
  if (ignore_set)
  ...
  factory.registerFunction(
      full_name_globalNotNullIn,
      [ignore_set, n = full_name_globalNotNullIn](ContextPtr) { return FunctionIn::create(n, true, false, ignore_set); },
