...
struct CountSubstringsImpl {
  ...
  static void vectorConstant(const ColumnString::Chars &haystack_data, const ColumnString::Offsets &haystack_offsets,
    ...
    if (needle.empty()) return;  // Return all zeros
    ...
    while (pos < end && end != (pos = searcher.search(pos, end - pos))) {
      ...
      while (i < input_rows_count && begin + haystack_offsets[i] <= pos) ++i;
      ...
      if (pos + needle.size() <= begin + haystack_offsets[i]) {
        auto found_offset =
            Impl::countChars(reinterpret_cast<const char *>(begin + haystack_offsets[i - 1]), reinterpret_cast<const char *>(pos));
        if (found_offset >= start) ++res[i];
    ...
    for (size_t i = 0; i < input_rows_count; ++i) {
      ...
      if (start > haystack_size) {
        ...
        typename Impl::SearcherInSmallHaystack searcher =
            Impl::createSearcherInSmallHaystack(reinterpret_cast<const char *>(&needle_data[prev_needle_offset]), needle_size);

        const UInt8 *end = reinterpret_cast<const UInt8 *>(&haystack_data[haystack_offsets[i]]);
        const UInt8 *beg = reinterpret_cast<const UInt8 *>(Impl::advancePos(
            reinterpret_cast<const char *>(&haystack_data[prev_haystack_offset]), reinterpret_cast<const char *>(end), start));
