...
inline int32_t JumpConsistentHash(uint64_t key, int32_t num_buckets) {
  ...
  while (j < num_buckets) {
    ...
    j = static_cast<int64_t>(static_cast<double>(b + 1) * (static_cast<double>(1LL << 31) / static_cast<double>((key >> 33) + 1)));
  }
...
struct JumpConsistentHashImpl {
  ...
  static ResultType apply(UInt64 hash, BucketsType n) { return JumpConsistentHash(hash, n); }
};
...
Calculates the [jump consistent hash](https://arxiv.org/pdf/1406.2294.pdf) for an integer.
  ...
  FunctionDocumentation::Arguments arguments = {{"key", "The input key.", {"UInt64"}}, {"buckets", "The number of buckets.", {"Int32"}}};
  FunctionDocumentation::ReturnedValue returned_value = {"Returns the computed hash value.", {"Int32"}};
  FunctionDocumentation::Examples examples = {{"Usage example", "SELECT jumpConsistentHash(256, 4)",
                                               R"(
