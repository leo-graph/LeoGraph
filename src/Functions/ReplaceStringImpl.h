...
struct ReplaceStringTraits {
  enum class Replace : uint8_t { First, All };
};
...
struct ReplaceStringImpl {
  ...
  static void copyToOutput(const CharT *what_start, size_t what_size, ColumnString::Chars &output, ColumnString::Offset &output_offset) {
  ...
  static void vectorVectorConstant(const ColumnString::Chars &haystack_data, const ColumnString::Offsets &haystack_offsets,
    ...
    for (size_t i = 0; i < input_rows_count; ++i) {
      ...
      if (cur_needle_length) {
        ...
        while (start_pos < cur_haystack_end) {
          if (const auto *const match = searcher.search(start_pos, cur_haystack_end); match != cur_haystack_end) {
            ...
            if constexpr (replace == ReplaceStringTraits::Replace::First) break;
          } else
            break;
        }
      }
      ...
      size_t bytes = (last_match == nullptr) ? (cur_haystack_end - cur_haystack_data) : (cur_haystack_end - last_match - cur_needle_length);
  ...
  static void vectorConstantVector(const ColumnString::Chars &haystack_data, const ColumnString::Offsets &haystack_offsets,
    ...
    for (size_t i = 0; i < input_rows_count; ++i) {
      ...
      /// Copy suffix after last match
      size_t bytes = (last_match == nullptr) ? (cur_haystack_end - cur_haystack_data) : (cur_haystack_end - last_match - needle.size());
      copyToOutput(start_pos, bytes, res_data, res_offset);
  ...
  static void vectorFixedConstantConstant(const ColumnString::Chars &haystack_data, size_t n, const String &needle,
    ...
    if (needle.empty()) {
      ...
      res_data.assign(haystack_data.begin(), haystack_data.end());
      res_offsets.resize(input_rows_count);
      for (size_t i = 1; i <= input_rows_count; ++i) res_offsets[i - 1] = i * n;
      return;
    }
