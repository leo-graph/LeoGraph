    ...
    for (size_t row = 0; row < input_rows_count; ++row) {
      if (data[row] > MAX_H3_RES)
        throw Exception(ErrorCodes::ARGUMENT_OUT_OF_BOUND,
                        ...
                        static_cast<uint8_t>(data[row]), getName(), MAX_H3_RES);
...
Returns all the pentagon [H3](#h3-index) indices at the specified resolution.
  ...
  FunctionDocumentation::ReturnedValue returned_value = {"Returns an array of all pentagon H3 indices at the specified resolution.",
                                                         {"Array(UInt64)"}};
  FunctionDocumentation::Examples examples = {{"Get all pentagon indices at resolution 3", "SELECT h3GetPentagonIndexes(3) AS indexes",
                                               R"(
┌─indexes────────────────────────────────────────────────────────┐
