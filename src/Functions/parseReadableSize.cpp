...
class FunctionParseReadable : public IFunction {
 public:
  FunctionParseReadable(const char *name_, ErrorHandling error_handling_) : function_name(name_), error_handling(error_handling_) {}
  ...
  ColumnPtr executeImpl(const ColumnsWithTypeAndName &arguments, const DataTypePtr &, size_t input_rows_count) const override {
    ...
    if (!col_str) {
      throw Exception(ErrorCodes::ILLEGAL_COLUMN, "Illegal column {} of first ('str') argument of function {}. Must be string.",
    ...
    if (error_handling == ErrorHandling::Null) col_null_map = ColumnUInt8::create(input_rows_count, false);
    ...
    for (size_t i = 0; i < input_rows_count; ++i) {
      ...
      try {
        ...
        if (error_handling == ErrorHandling::Exception) {
          ...
          res_data[i] = 0;
          if (error_handling == ErrorHandling::Null) col_null_map->getData()[i] = 1;
        }
      }
    }
    if (error_handling == ErrorHandling::Null)
  ...
  UInt64 parseReadableFormat(const std::string_view &value) const {
    ...
    if (buf.getPosition() > 0) {
      throw Exception(ErrorCodes::CANNOT_PARSE_INPUT_ASSERTION_FAILED,
                      "Invalid expression for function {} - Leading whitespace is not allowed (\"{}\")", getName(), value);
    }

    Float64 base = 0;
    if (!tryReadFloatTextPrecise(
            base, buf))  // If we use the default (fast) tryReadFloatText this returns True on garbage input so we use the Precise version
    {
      throw Exception(ErrorCodes::CANNOT_PARSE_NUMBER,
                      "Invalid expression for function {} - Unable to parse readable size numeric component (\"{}\")", getName(), value);
    }
    if (std::isnan(base) || !std::isfinite(base)) {
      throw Exception(ErrorCodes::BAD_ARGUMENTS, "Invalid expression for function {} - Invalid numeric component: {}", getName(), base);
    }
    if (base < 0) {
      throw Exception(ErrorCodes::BAD_ARGUMENTS, "Invalid expression for function {} - Negative sizes are not allowed ({})", getName(),
                      base);
    }
    ...
    if (iter == scale_factors.end()) {
      throw Exception(ErrorCodes::CANNOT_PARSE_TEXT, "Invalid expression for function {} - Unknown readable size unit (\"{}\")", getName(),
                      unit);
    }
    if (!buf.eof()) {
      throw Exception(ErrorCodes::UNEXPECTED_DATA_AFTER_PARSED_VALUE,
                      "Invalid expression for function {} - Found trailing characters after readable size string (\"{}\")", getName(),
                      value);
    }
...
#pragma clang diagnostic pop
    {
      throw Exception(ErrorCodes::BAD_ARGUMENTS, "Invalid expression for function {} - Result is too big for output type (\"{}\")",
                      getName(), num_bytes_with_decimals);
    }
    // As the input might be an arbitrary decimal number we might end up with a non-integer amount of bytes when parsing binary (eg MiB)
    // units. This doesn't make sense so we round up to indicate the byte size that can fit the passed size.
...
FunctionDocumentation::ReturnedValue returned_value_parseReadableSize = {"Returns the number of bytes, rounded up to the nearest integer.",
                                                                         {"UInt64"}};
...
FunctionDocumentation parseReadableSize_documentation = {
    description_parseReadableSize,    syntax_parseReadableSize,   arguments_parseReadableSize,     {},
    returned_value_parseReadableSize, examples_parseReadableSize, introduced_in_parseReadableSize, category_parseReadableSize};
...
FunctionDocumentation::Arguments arguments_parseReadableSizeOrNull = {
    {"x", "Readable size with ISO/IEC 80000-13 or decimal byte unit.", {"String"}}};
FunctionDocumentation::ReturnedValue returned_value_parseReadableSizeOrNull = {
...
FunctionDocumentation parseReadableSizeOrNull_documentation = {description_parseReadableSizeOrNull,    syntax_parseReadableSizeOrNull,
                                                               arguments_parseReadableSizeOrNull,      {},
                                                               returned_value_parseReadableSizeOrNull, examples_parseReadableSizeOrNull,
                                                               introduced_in_parseReadableSizeOrNull,  category_parseReadableSizeOrNull};
...
FunctionDocumentation::Arguments arguments_parseReadableSizeOrZero = {
    {"x", "Readable size with ISO/IEC 80000-13 or decimal byte unit.", {"String"}}};
FunctionDocumentation::ReturnedValue returned_value_parseReadableSizeOrZero = {
...
FunctionDocumentation parseReadableSizeOrZero_documentation = {description_parseReadableSizeOrZero,    syntax_parseReadableSizeOrZero,
                                                               arguments_parseReadableSizeOrZero,      {},
                                                               returned_value_parseReadableSizeOrZero, examples_parseReadableSizeOrZero,
                                                               introduced_in_parseReadableSizeOrZero,  category_parseReadableSizeOrZero};

REGISTER_FUNCTION(ParseReadableSize) {
  factory.registerFunction(
      "parseReadableSize", [](ContextPtr) { return FunctionParseReadable::create({}, "parseReadableSize", ErrorHandling::Exception); },
      parseReadableSize_documentation);
  factory.registerFunction(
      "parseReadableSizeOrNull",
      [](ContextPtr) { return FunctionParseReadable::create({}, "parseReadableSizeOrNull", ErrorHandling::Null); },
      parseReadableSizeOrNull_documentation);
  factory.registerFunction(
      "parseReadableSizeOrZero",
      [](ContextPtr) { return FunctionParseReadable::create({}, "parseReadableSizeOrZero", ErrorHandling::Zero); },
      parseReadableSizeOrZero_documentation);
}
}  // namespace DB
