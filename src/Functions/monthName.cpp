...
bool isDateOrDateTime(const IDataType& type) { return isDate(type) || isDateTime(type) || isDateTime64(type); }
...
class FunctionMonthName : public IFunction {
 public:
  ...
  explicit FunctionMonthName(ContextPtr context_) : function_resolver(FunctionFactory::instance().get("dateName", std::move(context_))) {}
  ...
  DataTypePtr getReturnTypeImpl(const ColumnsWithTypeAndName& arguments) const override {
    FunctionArgumentDescriptors mandatory_args{{"datetime", isDateOrDateTime, nullptr, "Date or DateTime"}};
  ...
  ColumnPtr executeImpl(const ColumnsWithTypeAndName& arguments, const DataTypePtr& result_type, size_t input_rows_count) const override {
    ...
    ColumnsWithTypeAndName temporary_columns{ColumnWithTypeAndName(month_column, std::make_shared<DataTypeString>(), ""), arguments[0]};
...
monthName(datetime)
  ...
  FunctionDocumentation::Arguments arguments = {{"datetime", "Date or date with time.", {"Date", "Date32", "DateTime", "DateTime64"}}};
  FunctionDocumentation::ReturnedValue returned_value = {"Returns the name of the month.", {"String"}};
  FunctionDocumentation::Examples examples = {{"Get month name from date", R"(
