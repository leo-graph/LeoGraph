...
class FunctionHasColumnInTable : public IFunction, WithContext {
 public:
  ...
  static FunctionPtr create(ContextPtr context_) { return std::make_shared<FunctionHasColumnInTable>(context_->getGlobalContext()); }
...
DataTypePtr FunctionHasColumnInTable::getReturnTypeImpl(const ColumnsWithTypeAndName &arguments) const {
  ...
  for (size_t i = 0; i < arguments.size(); ++i) {
    ...
    if (!checkColumnConst<ColumnString>(argument.column.get())) {
      throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT, "{} argument for function {} must be const String.", arg_pos_description[i],
                      getName());
    }
  }
...
ColumnPtr FunctionHasColumnInTable::executeImpl(const ColumnsWithTypeAndName &arguments, const DataTypePtr &,
  ...
  if (arguments.size() > 3) host_name = get_string_from_columns(arguments[arg++]);

  if (arguments.size() > 4) user_name = get_string_from_columns(arguments[arg++]);

  if (arguments.size() > 5) password = get_string_from_columns(arguments[arg++]);
  ...
  if (table_name.empty()) throw Exception(ErrorCodes::UNKNOWN_TABLE, "Table name is empty");
  ...
  if (host_name.empty()) {
    ...
    const StoragePtr &table = DatabaseCatalog::instance().getTable({database_name, table_name}, const_pointer_cast<Context>(getContext()));
    ...
    ClusterConnectionParameters params{!user_name.empty() ? user_name : "default",
                                       password,
    ...
    // because it might initialized a storage. Ideally, the tables required
    // by the query should be initialized at an earlier stage.
    auto remote_columns = getStructureOfRemoteTable(*cluster, {database_name, table_name}, const_pointer_cast<Context>(getContext()));
...
For the nested data structure itself, the function returns `0`.
  ...
  FunctionDocumentation::Arguments arguments = {{"database", "Name of the database.", {"const String"}},
  ...
  FunctionDocumentation::Examples examples = {{"Check an existing column",
                                               R"(
