...
class PerformanceStatistics {
 ...
 private:
  struct Element {
    ...
    int adjustedCount() const { return completed_count - NUM_INVOCATIONS_TO_THROW_OFF; }
    ...
    double sigma() const { return mean() / sqrt(adjustedCount()); }

    void run() { ++running_count; }

    void complete(double seconds, double bytes) {
      --running_count;
      ++completed_count;

      if (adjustedCount() > 0) sum += seconds / bytes;
    }
    ...
    double sample(pcg64 &stat_rng) const {
      ...
      if (adjustedCount() < 2) return adjustedCount() - 1 + running_count;
  ...
  ColumnPtr selectAndExecute(const ColumnsWithTypeAndName &arguments, const DataTypePtr &result_type, size_t input_rows_count) const {
    if (implementations.empty())
      throw Exception(ErrorCodes::NO_SUITABLE_FUNCTION_IMPLEMENTATION,
                      "There are no available implementations for function "
                      "TODO(dakovalkov): add name");
