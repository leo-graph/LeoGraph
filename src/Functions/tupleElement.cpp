...
ColumnPtr mergeNullMaps(const ColumnPtr &left, const ColumnPtr &right) {
  ...
  if (left_data.size() != right_data.size()) throw Exception(ErrorCodes::LOGICAL_ERROR, "Null maps have different sizes");

  auto merged_column = ColumnUInt8::create(left_data.size());
  auto &merged_data = merged_column->getData();

  for (size_t i = 0; i < merged_data.size(); ++i) merged_data[i] = left_data[i] || right_data[i];
 ...
 public:
  ...
  DataTypePtr getReturnTypeImpl(const ColumnsWithTypeAndName &arguments) const override {
    ...
    if (const DataTypeTuple *tuple = checkAndGetDataType<DataTypeTuple>(input_type)) {
      ...
      if (is_input_type_nullable) {
        throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT, "First argument for function {} cannot be Nullable(QBit). Actual {}",
                        getName(), arguments[0].type->getName());
      }

      std::optional<size_t> index = getQBitElementIndex(arguments[1].column, *qbit, number_of_arguments);
      if (index.has_value()) return wrapInArrays(qbit->getNestedTupleElementType(), count_arrays);

      return arguments[2].type;
    } else if (const DataTypeObject *object = checkAndGetDataType<DataTypeObject>(input_type)) {
      if (is_input_type_nullable) {
        throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT, "First argument for function {} cannot be Nullable(JSON). Actual {}",
                        getName(), arguments[0].type->getName());
      }

      if (number_of_arguments != 2)
        throw Exception(ErrorCodes::NUMBER_OF_ARGUMENTS_DOESNT_MATCH,
                        "Number of arguments for function {} with {} first argument doesn't match: passed {}, should be 2", getName(),
                        input_type->getName(), number_of_arguments);

      const auto *subcolumn_name_col = checkAndGetColumnConst<ColumnString>(arguments[1].column.get());
      if (!subcolumn_name_col)
        throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT, "Second argument of {} with {} first argument must be a constant String",
                        getName(), input_type->getName());
    ...
    throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT,
                    "First argument for function {} must be Tuple, Nullable(Tuple), QBit, JSON or array of these. Actual {}", getName(),
  ...
  ColumnPtr executeImpl(const ColumnsWithTypeAndName &arguments, const DataTypePtr &, size_t input_rows_count) const override {
    ...
    if (const DataTypeTuple *input_type_as_tuple = checkAndGetDataType<DataTypeTuple>(input_type)) {
      ...
      if (!index.has_value()) return arguments[2].column;
      ...
      if (null_map_column) {
        throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT, "First argument for function {} cannot be Nullable(QBit). Actual {}",
                        getName(), input_arg.type->getName());
      }
    ...
    /// Wrap into Arrays
    for (auto it = array_offsets.rbegin(); it != array_offsets.rend(); ++it) res = ColumnArray::create(res, *it);

    if (input_arg_is_const) res = ColumnConst::create(res, input_rows_count);
 ...
 private:
  std::optional<size_t> getTupleElementIndex(const ColumnPtr &index_column, const DataTypeTuple &tuple, size_t argument_size) const {
    if (checkAndGetColumnConst<ColumnUInt8>(index_column.get()) || checkAndGetColumnConst<ColumnUInt16>(index_column.get()) ||
      ...
      if (index > 0 && index <= tuple.getElements().size()) return {index - 1};

      if (argument_size == 2) throw Exception(ErrorCodes::NOT_FOUND_COLUMN_IN_BLOCK, "Tuple doesn't have element with index '{}'", index);
      return std::nullopt;
    }
    ...
    if (checkAndGetColumnConst<ColumnInt8>(index_column.get()) || checkAndGetColumnConst<ColumnInt16>(index_column.get()) ||
      ...
      if (index > 0 && index <= size) return {index - 1};

      if (index < 0 && index >= -size) return {index + size};
  ...
  std::optional<size_t> getQBitElementIndex(const ColumnPtr &index_column, const DataTypeQBit &qbit, size_t argument_size) const {
    if (checkAndGetColumnConst<ColumnUInt8>(index_column.get()) || checkAndGetColumnConst<ColumnUInt16>(index_column.get()) ||
      ...
      if (index > 0 && index <= qbit.getElementSize()) return {index - 1};

      if (argument_size == 2) throw Exception(ErrorCodes::NOT_FOUND_COLUMN_IN_BLOCK, "QBit doesn't have an element with index '{}'", index);
  ...
  DataTypePtr wrapInArrays(DataTypePtr nested_type, size_t count_arrays) const {
    for (; count_arrays; --count_arrays) nested_type = std::make_shared<DataTypeArray>(nested_type);
  ...
  ColumnPtr getObjectElement(const DataTypeObject &object_type, const ColumnPtr &object_column, const String &element_name) const {
    ...
    if (object_type.getTypedPaths().contains(element_name)) return literal_subcolumn;
    ...
    if (sub_object_subcolumn->getNumberOfDefaultRows() == sub_object_subcolumn->size()) return literal_subcolumn;
...
This function has zero runtime cost and implements the operators `x.index` and `x.name`.
  ...
  FunctionDocumentation::Examples examples = {{"Index access", "SELECT tupleElement((1, 'hello'), 2)", "hello"},
                                              {"Negative indexing", "SELECT tupleElement((1, 'hello'), -1)", "hello"},
...
SELECT tupleElement(values, 'name') FROM example;
                                               ...
                                               "Alice"},
                                              {"With default value", "SELECT tupleElement((1, 2), 5, 'not_found')", "not_found"},
                                              {"Operator syntax", "SELECT (1, 'hello').2", "hello"}};
