...
class FunctionMap : public IFunction {
 public:
  ...
  static FunctionPtr create(ContextPtr context) { return std::make_shared<FunctionMap>(context); }
  ...
  DataTypePtr getReturnTypeImpl(const DataTypes &arguments) const override {
    if (arguments.size() % 2 != 0)
      throw Exception(ErrorCodes::NUMBER_OF_ARGUMENTS_DOESNT_MATCH, "Function {} requires even number of arguments, but {} given",
  ...
  ColumnPtr executeImpl(const ColumnsWithTypeAndName &arguments, const DataTypePtr &result_type, size_t input_rows_count) const override {
    ...
    if (num_elements == 0) return result_type->createColumnConstWithDefaultValue(input_rows_count);
    ...
    ColumnPtr value_array =
        function_array->build(value_args)->execute(value_args, value_array_type, input_rows_count, /* dry_run = */ false);
...
class FunctionMapFromArrays : public IFunction {
 public:
  ...
  DataTypePtr getReturnTypeImpl(const DataTypes &arguments) const override {
    if (arguments.size() != 2)
      throw Exception(ErrorCodes::NUMBER_OF_ARGUMENTS_DOESNT_MATCH, "Function {} requires 2 arguments, but {} given", getName(),
                      arguments.size());
    ...
    auto get_nested_type = [&](const DataTypePtr &type) {
      ...
      if (const auto *type_as_array = checkAndGetDataType<DataTypeArray>(type.get()))
        ...
        throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT, "Arguments of function {} must be Array or Map, but {} is given", getName(),
                        type->getName());
    ...
    /// We accept Array(Nullable(T)) or Array(LowCardinality(Nullable(T))) as key types as long as the actual array doesn't contain NULL
    /// value(this is checked in executeImpl).
  ...
  ColumnPtr executeImpl(const ColumnsWithTypeAndName &arguments, const DataTypePtr & /* result_type */,
    ...
    auto get_array_column = [&](const ColumnPtr &column) -> std::pair<const ColumnArray *, ColumnPtr> {
      ...
      if (const auto *col_array = checkAndGetColumn<ColumnArray>(holder.get()))
        ...
        throw Exception(ErrorCodes::ILLEGAL_COLUMN, "Argument columns of function {} must be Array or Map, but {} is given", getName(),
                        holder->getName());
    ...
    if (isColumnNullableOrLowCardinalityNullable(*data_keys)) {
      if (const auto *nullable = checkAndGetColumn<ColumnNullable>(data_keys.get())) {
        ...
        if (null_map && !memoryIsZero(null_map->data(), 0, null_map->size()))
          throw Exception(ErrorCodes::BAD_ARGUMENTS, "The nested column of first argument in function {} must not contain NULLs",
                          getName());
...
class FunctionMapUpdate : public IFunction {
 public:
  ...
  DataTypePtr getReturnTypeImpl(const ColumnsWithTypeAndName &arguments) const override {
    ...
    if (!left || !right)
      throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT, "The two arguments for function {} must be both Map type", getName());

    if (!left->getKeyType()->equals(*right->getKeyType()) || !left->getValueType()->equals(*right->getValueType()))
      throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT, "The Key And Value type of Map for function {} must be the same", getName());
  ...
  ColumnPtr executeImpl(const ColumnsWithTypeAndName &arguments, const DataTypePtr &, size_t input_rows_count) const override {
    ...
    const auto *map_column_left = is_left_const ? checkAndGetColumnConstData<ColumnMap>(arguments[0].column.get())
                                                : checkAndGetColumn<ColumnMap>(arguments[0].column.get());

    const auto *map_column_right = is_right_const ? checkAndGetColumnConstData<ColumnMap>(arguments[1].column.get())
                                                  : checkAndGetColumn<ColumnMap>(arguments[1].column.get());

    if (!map_column_left || !map_column_right)
      throw Exception(ErrorCodes::ILLEGAL_COLUMN, "Arguments for function {} must be maps, got {} and {} instead", getName(),
                      arguments[0].column->getName(), arguments[1].column->getName());
    ...
    if (is_right_const) {
      for (size_t i = 0; i < keys_data_right.size(); ++i) right_keys_const.insert(keys_data_right.getDataAt(i));
    }
    ...
    for (size_t row_idx = 0; row_idx < input_rows_count; ++row_idx) {
      ...
      if (is_right_const) {
        ...
        Set right_keys;
        for (size_t i = right_from; i < right_to; ++i) right_keys.insert(keys_data_right.getDataAt(i));
    ...
    auto nested_column =
        ColumnArray::create(ColumnTuple::create(Columns{std::move(result_keys), std::move(result_values)}), std::move(result_offsets));
...
Creates a value of type `Map(key, value)` from key-value pairs.
  ...
  FunctionDocumentation::Arguments arguments_map = {{"key_n", "The keys of the map entries.", {"Any"}},
  ...
  FunctionDocumentation::Examples examples_map = {{"Usage example", "SELECT map('key1', number, 'key2', number * 2) FROM numbers(3)",
                                                   "{'key1':0,'key2':0}\n{'key1':1,'key2':2}\n{'key1':2,'key2':4}"}};
  ...
  FunctionDocumentation documentation_map = {description_map,    syntax_map,   arguments_map,     {},
                                             returned_value_map, examples_map, introduced_in_map, category_map};
...
The function is a convenient alternative to syntax `CAST([...], 'Map(key_type, value_type)')`.
  ...
  FunctionDocumentation::ReturnedValue returned_value_mapFromArrays = {
  ...
  FunctionDocumentation documentation_mapFromArrays = {
      description_mapFromArrays,    syntax_mapFromArrays,   arguments_mapFromArrays,     {},
      returned_value_mapFromArrays, examples_mapFromArrays, introduced_in_mapFromArrays, category_mapFromArrays};
...
For two maps, returns the first map with values updated on the values for the corresponding keys in the second map.
  ...
  FunctionDocumentation::Arguments arguments_mapUpdate = {{"map1", "The map to update.", {"Map(K, V)"}},
                                                          {"map2", "The map to use for updating.", {"Map(K, V)"}}};
  FunctionDocumentation::ReturnedValue returned_value_mapUpdate = {
  ...
  FunctionDocumentation documentation_mapUpdate = {
      description_mapUpdate,    syntax_mapUpdate,   arguments_mapUpdate,     {},
      returned_value_mapUpdate, examples_mapUpdate, introduced_in_mapUpdate, category_mapUpdate};
