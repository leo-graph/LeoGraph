...
enum class ParseSyntax : uint8_t { MySQL, Joda };

enum class ErrorHandling : uint8_t { Exception, Zero, Null };

enum class ReturnType : uint8_t { DateTime, DateTime64 };
...
constexpr Int32 cumulativeYearDaysFrom1970[] = {
    ...
    6209,   6574,   6940,   7305,   7670,   8035,   8401,   8766,   9131,   9496,   9862,   10227,  10592,  10957,  11323,  11688,  12053,
    12418,  12784,  13149,  13514,  13879,  14245,  14610,  14975,  15340,  15706,  16071,  16436,  16801,  17167,  17532,  17897,  18262,
    18628,  18993,  19358,  19723,  20089,  20454,  20819,  21184,  21550,  21915,  22280,  22645,  23011,  23376,  23741,  24106,  24472,
    24837,  25202,  25567,  25933,  26298,  26663,  27028,  27394,  27759,  28124,  28489,  28855,  29220,  29585,  29950,  30316,  30681,
    31046,  31411,  31777,  32142,  32507,  32872,  33238,  33603,  33968,  34333,  34699,  35064,  35429,  35794,  36160,  36525,  36890,
    37255,  37621,  37986,  38351,  38716,  39082,  39447,  39812,  40177,  40543,  40908,  41273,  41638,  42004,  42369,  42734,  43099,
    43465,  43830,  44195,  44560,  44926,  45291,  45656,  46021,  46387,  46752,  47117,  47482,  47847,  48212,  48577,  48942,  49308,
    49673,  50038,  50403,  50769,  51134,  51499,  51864,  52230,  52595,  52960,  53325,  53691,  54056,  54421,  54786,  55152,  55517,
    55882,  56247,  56613,  56978,  57343,  57708,  58074,  58439,  58804,  59169,  59535,  59900,  60265,  60630,  60996,  61361,  61726,
    62091,  62457,  62822,  63187,  63552,  63918,  64283,  64648,  65013,  65379,  65744,  66109,  66474,  66840,  67205,  67570,  67935,
    68301,  68666,  69031,  69396,  69762,  70127,  70492,  70857,  71223,  71588,  71953,  72318,  72684,  73049,  73414,  73779,  74145,
    74510,  74875,  75240,  75606,  75971,  76336,  76701,  77067,  77432,  77797,  78162,  78528,  78893,  79258,  79623,  79989,  80354,
    80719,  81084,  81450,  81815,  82180,  82545,  82911,  83276,  83641,  84006,  84371,  84736,  85101,  85466,  85832,  86197,  86562,
    86927,  87293,  87658,  88023,  88388,  88754,  89119,  89484,  89849,  90215,  90580,  90945,  91310,  91676,  92041,  92406,  92771,
    93137,  93502,  93867,  94232,  94598,  94963,  95328,  95693,  96059,  96424,  96789,  97154,  97520,  97885,  98250,  98615,  98981,
    99346,  99711,  100076, 100442, 100807, 101172, 101537, 101903, 102268, 102633, 102998, 103364, 103729, 104094, 104459, 104825, 105190,
    105555, 105920, 106286, 106651, 107016, 107381, 107747, 108112, 108477, 108842, 109208, 109573, 109938, 110303, 110669, 111034, 111399,
    111764, 112130, 112495, 112860, 113225, 113591, 113956, 114321, 114686, 115052, 115417, 115782, 116147, 116513, 116878, 117243, 117608,
    117974, 118339, 118704, 119069, 119435, 119800, 120165};

/// key: year, value: cumulative days from the epoch (1970-01-01) to the first day of the current year (exclusive), counting backwards from
/// 1970 to 1969, and so on. For example, the value -365 corresponds to the year 1969, indicating that there are 365 days from 1970-01-01 to
/// 1969-01-01.
constexpr Int32 cumulativeYearDaysBefore1970[] = {
    0,      -365,   -731,   -1096,  -1461,  -1826,  -2192,  -2557,  -2922,  -3287,  -3653,  -4018,  -4383,  -4748,  -5114,
    -5479,  -5844,  -6209,  -6575,  -6940,  -7305,  -7670,  -8036,  -8401,  -8766,  -9131,  -9497,  -9862,  -10227, -10592,
    -10958, -11323, -11688, -12053, -12419, -12784, -13149, -13514, -13880, -14245, -14610, -14975, -15341, -15706, -16071,
    -16436, -16802, -17167, -17532, -17897, -18263, -18628, -18993, -19358, -19724, -20089, -20454, -20819, -21185, -21550,
    -21915, -22280, -22646, -23011, -23376, -23741, -24107, -24472, -24837, -25202, -25567};

struct ErrorCodeAndMessage {
  int error_code;
  String error_message;

  explicit ErrorCodeAndMessage(int error_code_) : error_code(error_code_) {}

  template <typename... Args>
  ErrorCodeAndMessage(int error_code_, FormatStringHelper<Args...> formatter, Args &&...args)
      : error_code(error_code_), error_message(formatter.format(std::forward<Args>(args)...)) {}
};
...
#define RETURN_ERROR_IF_FAILED(function_call)                                                     \
  {                                                                                               \
    if (auto result = function_call; !result.has_value()) return std::unexpected(result.error()); \
  }
...
struct alignas(CH_CACHE_LINE_SIZE) ParsedValue {
  ...
  VoidOrError setHour(Int32 hour_, bool is_hour_of_half_day_ = false, bool hour_starts_at_1_ = false) {
    ...
    if (hour_ < min_hour || hour_ > max_hour)
      RETURN_ERROR(ErrorCodes::CANNOT_PARSE_DATETIME,
                   "Value {} for hour must be in the range [{}, {}] if_hour_of_half_day={} and hour_starts_at_1={}", hour, max_hour,
                   min_hour, is_hour_of_half_day_, hour_starts_at_1_)
  ...
  [[nodiscard]]
  static bool isLeapYear(Int32 year_) {
  ...
  static Int32OrError daysSinceEpochFromWeekDate(int32_t week_year_, int32_t week_of_year_, int32_t day_of_week_) {
    ...
    if (!yearIsInValidRange(week_year_)) RETURN_ERROR(ErrorCodes::CANNOT_PARSE_DATETIME, "Invalid week year {}", week_year_)
  ...
  static Int32OrError daysSinceEpochFromDate(Int32 year_, Int32 month_, Int32 day_) {
    if (!isDateValid(year_, month_, day_))
      RETURN_ERROR(ErrorCodes::CANNOT_PARSE_DATETIME, "Invalid date, out of range (year: {} month: {} day_of_month: {})", year_, month_,
                   day_)
    Int32 res = 0;
    if (year_ >= 1970)
  ...
  Int64OrError buildDateTime(const DateLUTImpl &time_zone) {
    if (is_hour_of_half_day && !is_am) hour += 12;
    ...
    if (!has_time_zone_offset) time_zone_offset = time_zone.timezoneOffset(seconds_since_epoch);

    /// Time zone is specified in format string.
    seconds_since_epoch -= time_zone_offset;
    if constexpr (return_type == ReturnType::DateTime)
      if (seconds_since_epoch < 0) RETURN_ERROR(ErrorCodes::VALUE_IS_OUT_OF_RANGE_OF_DATA_TYPE, "Seconds since epoch is negative")
...
class FunctionParseDateTimeImpl : public IFunction {
 public:
  ...
  DataTypePtr getReturnTypeImpl(const ColumnsWithTypeAndName &arguments) const override {
    ...
    if (error_handling == ErrorHandling::Null) return std::make_shared<DataTypeNullable>(data_type);
    return data_type;
  }
  ...
  ColumnPtr executeImpl2(const ColumnsWithTypeAndName &arguments, const DataTypePtr &result_type, size_t input_rows_count,
    ...
    if (!col_str)
      throw Exception(ErrorCodes::ILLEGAL_COLUMN, "Illegal type in 1st ('time') argument of function {}. Must be String.", getName());

    Int64 multiplier = 0;
    UInt32 scale = 0;
    if constexpr (return_type == ReturnType::DateTime64) {
      const DataTypeDateTime64 *result_type_without_nullable_cast =
          checkAndGetDataType<DataTypeDateTime64>(removeNullable(result_type).get());
      scale = result_type_without_nullable_cast->getScale();
      multiplier = DecimalUtils::scaleMultiplier<DateTime64>(scale);
    }

    ColumnUInt8::MutablePtr col_null_map;
    if constexpr (error_handling == ErrorHandling::Null) col_null_map = ColumnUInt8::create(input_rows_count, false);
    ...
    for (size_t i = 0; i < input_rows_count; ++i) {
      datetime.reset();
      if constexpr (return_type == ReturnType::DateTime64) datetime.setScale(scale, parse_syntax);
      ...
      if (cur < end) {
        result = std::unexpected(ErrorCodeAndMessage(ErrorCodes::CANNOT_PARSE_DATETIME, "Invalid format input {} is malformed at {}",
                                                     str_ref, std::string_view(cur, end - cur)));
      }
 ...
 private:
  class Instruction {
   private:
    enum class NeedCheckSpace : uint8_t { Yes, No };

    using Func = std::conditional_t<parse_syntax == ParseSyntax::MySQL,
   ...
   public:
    ...
    /// For debug
    [[maybe_unused]] String toString() const {
      if (func) return "func:" + func_name + ",fragment:" + fragment;
    ...
    PosOrError perform(Pos cur, Pos end, ParsedValue<error_handling, return_type> &parsed_value) const {
      if (func) return func(cur, end, fragment, parsed_value);
      ...
      if (std::string_view(cur, literal.size()) != literal)
        RETURN_ERROR(ErrorCodes::CANNOT_PARSE_DATETIME,
                     "Unable to parse fragment {} from {} because literal {} is expected but {} provided", fragment,
                     std::string_view(cur, end - cur), literal, std::string_view(cur, literal.size()))
      cur += literal.size();
      return cur;
    }
    ...
    static VoidOrError checkSpace(Pos cur, Pos end, size_t len, const String &msg, const String &fragment) {
      if (cur > end || cur + len > end) [[unlikely]]
        RETURN_ERROR(ErrorCodes::NOT_ENOUGH_SPACE, "Unable to parse fragment {} from {} because {}", fragment,
                     std::string_view(cur, end - cur), msg)
      return {};
    }
    ...
    static PosOrError assertChar(Pos cur, Pos end, char expected, const String &fragment) {
      ...
      if (*cur != expected) [[unlikely]]
        RETURN_ERROR(ErrorCodes::CANNOT_PARSE_DATETIME, "Unable to parse fragment {} from {} because char {} is expected but {} provided",
                     fragment, std::string_view(cur, end - cur), String(expected, 1), String(*cur, 1))
    ...
    static PosOrError assertNumber(Pos cur, Pos end, const String &fragment) {
      ...
      if (*cur < '0' || *cur > '9') [[unlikely]]
        RETURN_ERROR(ErrorCodes::CANNOT_PARSE_DATETIME, "Unable to parse fragment {} from {} because {} is not a number", fragment,
                     std::string_view(cur, end - cur), String(*cur, 1))
    ...
    [[nodiscard]]
    static PosOrError mysqlDayOfWeekTextShort(Pos cur, Pos end, const String &fragment,
                                              ParsedValue<error_handling, return_type> &parsed_value) {
      ...
      if (it == dayOfWeekMap.end())
        RETURN_ERROR(ErrorCodes::CANNOT_PARSE_DATETIME, "Unable to parse fragment {} from {} because of unknown day of week short text {} ",
                     fragment, std::string_view(cur, end - cur), text)
    ...
    [[nodiscard]]
    static PosOrError mysqlMonthOfYearTextShort(Pos cur, Pos end, const String &fragment,
                                                ParsedValue<error_handling, return_type> &parsed_value) {
      ...
      if (it == monthMap.end())
        RETURN_ERROR(ErrorCodes::CANNOT_PARSE_DATETIME,
                     "Unable to parse fragment {} from {} because of unknown month of year short text {}", fragment,
                     std::string_view(cur, end - cur), text)
    ...
    [[nodiscard]]
    static PosOrError mysqlMonthOfYearTextLong(Pos cur, Pos end, const String &fragment,
                                               ParsedValue<error_handling, return_type> &parsed_value) {
      ...
      if (it == monthMap.end())
        RETURN_ERROR(ErrorCodes::CANNOT_PARSE_DATETIME,
                     "Unable to parse first part of fragment {} from {} because of unknown month of year text: {}", fragment,
                     std::string_view(cur, end - cur), text1)
      cur += 3;

      size_t expected_remaining_size = it->second.first.size();
      RETURN_ERROR_IF_FAILED(
          checkSpace(cur, end, expected_remaining_size,
                     "mysqlMonthOfYearTextLong requires the second parg size >= " + std::to_string(expected_remaining_size), fragment))
      String text2(cur, expected_remaining_size);
      boost::to_lower(text2);
      if (text2 != it->second.first)
        RETURN_ERROR(ErrorCodes::CANNOT_PARSE_DATETIME,
                     "Unable to parse second part of fragment {} from {} because of unknown month of year text: {}", fragment,
                     std::string_view(cur, end - cur), text1 + text2)
    ...
    [[nodiscard]]
    static PosOrError mysqlMonthWithoutLeadingZero(Pos cur, Pos end, const String &fragment,
                                                   ParsedValue<error_handling, return_type> &parsed_value) {
      Int32 month;
    ...
    [[nodiscard]]
    static PosOrError mysqlDayOfMonthMandatorySpacePadding(Pos cur, Pos end, const String &fragment,
                                                           ParsedValue<error_handling, return_type> &parsed_value) {
    ...
    [[nodiscard]]
    static PosOrError mysqlDayOfMonthOptionalSpacePadding(Pos cur, Pos end, const String &fragment,
                                                          ParsedValue<error_handling, return_type> &parsed_value) {
    ...
    [[nodiscard]]
    static PosOrError mysqlDayOfWeekTextLong(Pos cur, Pos end, const String &fragment,
                                             ParsedValue<error_handling, return_type> &parsed_value) {
      ...
      if (it == dayOfWeekMap.end())
        RETURN_ERROR(ErrorCodes::CANNOT_PARSE_DATETIME,
                     "Unable to parse first part of fragment {} from {} because of unknown day of week text: {}", fragment,
                     std::string_view(cur, end - cur), text1)
      cur += 3;
      ...
      RETURN_ERROR_IF_FAILED(
          checkSpace(cur, end, expected_remaining_size,
                     "mysqlDayOfWeekTextLong requires the second parg size >= " + std::to_string(expected_remaining_size), fragment))
      String text2(cur, expected_remaining_size);
      boost::to_lower(text2);
      if (text2 != it->second.first)
        RETURN_ERROR(ErrorCodes::CANNOT_PARSE_DATETIME,
                     "Unable to parse second part of fragment {} from {} because of unknown day of week text: {}", fragment,
                     std::string_view(cur, end - cur), text1 + text2)
      cur += expected_remaining_size;
    ...
    [[nodiscard]]
    static PosOrError mysqlTimezoneOffset(Pos cur, Pos end, const String &fragment,
                                          ParsedValue<error_handling, return_type> &parsed_value) {
      ...
      if (*cur == '-')
        ...
        RETURN_ERROR(ErrorCodes::CANNOT_PARSE_DATETIME, "Unable to parse fragment {} from {} because of unknown sign time zone offset: {}",
                     fragment, std::string_view(cur, end - cur), std::string_view(cur, 1))
      ++cur;
    ...
    static PosOrError mysqlMicrosecond(Pos cur, Pos end, const String &fragment, ParsedValue<error_handling, return_type> &parsed_value) {
      if constexpr (return_type == ReturnType::DateTime) {
        ...
        for (size_t i = 0; i < 6; ++i) ASSIGN_RESULT_OR_RETURN_ERROR(cur, (assertNumber<NeedCheckSpace::No>(cur, end, fragment)))
      } else {
        if (parsed_value.scale != 6)
          RETURN_ERROR(ErrorCodes::CANNOT_PARSE_DATETIME, "Unable to parse fragment {} from {} because the datetime scale {} is not 6",
                       fragment, std::string_view(cur, end - cur), std::to_string(parsed_value.scale))
        Int32 microsecond = 0;
    ...
    [[nodiscard]]
    static PosOrError mysqlHour12WithoutLeadingZero(Pos cur, Pos end, const String &fragment,
                                                    ParsedValue<error_handling, return_type> &parsed_value) {
      Int32 hour;
    ...
    [[nodiscard]]
    static PosOrError mysqlHour24WithoutLeadingZero(Pos cur, Pos end, const String &fragment,
                                                    ParsedValue<error_handling, return_type> &parsed_value) {
      Int32 hour;
      ASSIGN_RESULT_OR_RETURN_ERROR(cur, (readNumberWithVariableLength(cur, end, false, false, false, 1, 2, fragment, hour)))
      RETURN_ERROR_IF_FAILED(parsed_value.setHour(hour, false, false))
    ...
    [[nodiscard]]
    static PosOrError readNumberWithVariableLength(Pos cur, Pos end, bool allow_negative, bool allow_plus_sign, bool is_year,
                                                   size_t repetitions, size_t max_digits_to_read, const String &fragment, Int32 &result) {
      bool negative = false;
      ...
      if (max_digits_to_read >= std::numeric_limits<decltype(number)>::digits10) [[unlikely]]
        RETURN_ERROR(ErrorCodes::CANNOT_PARSE_DATETIME, "Unable to parse fragment {} from {} because max_digits_to_read is too big",
                     fragment, std::string_view(start, cur - start))
      ...
      if (cur == start) [[unlikely]]
        RETURN_ERROR(ErrorCodes::CANNOT_PARSE_DATETIME, "Unable to parse fragment {} from {} because read number failed", fragment,
      ...
      if (number < std::numeric_limits<Int32>::min() || number > std::numeric_limits<Int32>::max()) [[unlikely]]
        RETURN_ERROR(ErrorCodes::CANNOT_PARSE_DATETIME, "Unable to parse fragment {} from {} because number is out of range of Int32",
                     fragment, std::string_view(start, cur - start))
    ...
    [[nodiscard]]
    static PosOrError jodaCenturyOfEra(size_t repetitions, Pos cur, Pos end, const String &fragment,
                                       ParsedValue<error_handling, return_type> &parsed_value) {
      Int32 century;
      ASSIGN_RESULT_OR_RETURN_ERROR(
    ...
    [[nodiscard]]
    static PosOrError jodaYearOfEra(size_t repetitions, Pos cur, Pos end, const String &fragment,
                                    ParsedValue<error_handling, return_type> &parsed_value) {
      Int32 year_of_era;
      size_t max_digits = repetitions > 2 ? std::max<size_t>(repetitions, 4) : repetitions;
      ASSIGN_RESULT_OR_RETURN_ERROR(
          cur, (readNumberWithVariableLength(cur, end, false, false, true, repetitions, max_digits, fragment, year_of_era)))
      RETURN_ERROR_IF_FAILED(parsed_value.setYear(year_of_era, true))
      return cur;
    }

    [[nodiscard]]
    static PosOrError jodaWeekYear(size_t repetitions, Pos cur, Pos end, const String &fragment,
                                   ParsedValue<error_handling, return_type> &parsed_value) {
      Int32 week_year;
      size_t max_digits = repetitions > 2 ? std::max<size_t>(repetitions, 4) : repetitions;
      ASSIGN_RESULT_OR_RETURN_ERROR(
          cur, (readNumberWithVariableLength(cur, end, true, true, true, repetitions, max_digits, fragment, week_year)))
      RETURN_ERROR_IF_FAILED(parsed_value.setYear(week_year, false, true))
      return cur;
    }

    [[nodiscard]]
    static PosOrError jodaWeekOfWeekYear(size_t repetitions, Pos cur, Pos end, const String &fragment,
                                         ParsedValue<error_handling, return_type> &parsed_value) {
      Int32 week;
      ASSIGN_RESULT_OR_RETURN_ERROR(
    ...
    [[nodiscard]]
    static PosOrError jodaDayOfWeek1Based(size_t repetitions, Pos cur, Pos end, const String &fragment,
                                          ParsedValue<error_handling, return_type> &parsed_value) {
      Int32 day_of_week;
      ASSIGN_RESULT_OR_RETURN_ERROR(
    ...
    [[nodiscard]]
    static PosOrError jodaDayOfWeekText(size_t /*min_represent_digits*/, Pos cur, Pos end, const String &fragment,
                                        ParsedValue<error_handling, return_type> &parsed_value) {
      ...
      auto it = dayOfWeekMap.find(text1);
      if (it == dayOfWeekMap.end())
        RETURN_ERROR(ErrorCodes::CANNOT_PARSE_DATETIME, "Unable to parse fragment {} from {} because of unknown day of week text: {}",
    ...
    [[nodiscard]]
    static PosOrError jodaYear(size_t repetitions, Pos cur, Pos end, const String &fragment,
                               ParsedValue<error_handling, return_type> &parsed_value) {
      Int32 year;
      size_t max_digits = repetitions > 2 ? std::max<size_t>(repetitions, 4) : repetitions;
      ASSIGN_RESULT_OR_RETURN_ERROR(cur,
                                    (readNumberWithVariableLength(cur, end, true, true, true, repetitions, max_digits, fragment, year)))
      RETURN_ERROR_IF_FAILED(parsed_value.setYear(year))
      return cur;
    }

    [[nodiscard]]
    static PosOrError jodaDayOfYear(size_t repetitions, Pos cur, Pos end, const String &fragment,
                                    ParsedValue<error_handling, return_type> &parsed_value) {
      Int32 day_of_year;
    ...
    [[nodiscard]]
    static PosOrError jodaMonthOfYear(size_t repetitions, Pos cur, Pos end, const String &fragment,
                                      ParsedValue<error_handling, return_type> &parsed_value) {
      Int32 month;
    ...
    [[nodiscard]]
    static PosOrError jodaMonthOfYearText(int, Pos cur, Pos end, const String &fragment,
                                          ParsedValue<error_handling, return_type> &parsed_value) {
      ...
      auto it = monthMap.find(text1);
      if (it == monthMap.end())
        RETURN_ERROR(ErrorCodes::CANNOT_PARSE_DATETIME, "Unable to parse fragment {} from {} because of unknown month of year text: {}",
                     fragment, std::string_view(cur, end - cur), text1)
      cur += 3;
      RETURN_ERROR_IF_FAILED(parsed_value.setMonth(it->second.second))
    ...
    [[nodiscard]]
    static PosOrError jodaDayOfMonth(size_t repetitions, Pos cur, Pos end, const String &fragment,
                                     ParsedValue<error_handling, return_type> &parsed_value) {
      Int32 day_of_month;
      ASSIGN_RESULT_OR_RETURN_ERROR(cur, (readNumberWithVariableLength(cur, end, false, false, false, repetitions,
                                                                       std::max(repetitions, 2uz), fragment, day_of_month)))
      RETURN_ERROR_IF_FAILED(parsed_value.setDayOfMonth(day_of_month))
    ...
    [[nodiscard]]
    static PosOrError jodaHalfDayOfDay(int, Pos cur, Pos end, const String &fragment,
                                       ParsedValue<error_handling, return_type> &parsed_value) {
    ...
    [[nodiscard]]
    static PosOrError jodaHourOfHalfDay(size_t repetitions, Pos cur, Pos end, const String &fragment,
                                        ParsedValue<error_handling, return_type> &parsed_value) {
      Int32 hour;
      ASSIGN_RESULT_OR_RETURN_ERROR(
    ...
    [[nodiscard]]
    static PosOrError jodaClockHourOfHalfDay(size_t repetitions, Pos cur, Pos end, const String &fragment,
                                             ParsedValue<error_handling, return_type> &parsed_value) {
      Int32 hour;
      ASSIGN_RESULT_OR_RETURN_ERROR(
          cur, (readNumberWithVariableLength(cur, end, false, false, false, repetitions, std::max(repetitions, 2uz), fragment, hour)))
    ...
    [[nodiscard]]
    static PosOrError jodaHourOfDay(size_t repetitions, Pos cur, Pos end, const String &fragment,
                                    ParsedValue<error_handling, return_type> &parsed_value) {
      Int32 hour;
      ASSIGN_RESULT_OR_RETURN_ERROR(
          cur, (readNumberWithVariableLength(cur, end, false, false, false, repetitions, std::max(repetitions, 2uz), fragment, hour)))
    ...
    [[nodiscard]]
    static PosOrError jodaClockHourOfDay(size_t repetitions, Pos cur, Pos end, const String &fragment,
                                         ParsedValue<error_handling, return_type> &parsed_value) {
      Int32 hour;
      ASSIGN_RESULT_OR_RETURN_ERROR(
          cur, (readNumberWithVariableLength(cur, end, false, false, false, repetitions, std::max(repetitions, 2uz), fragment, hour)))
    ...
    [[nodiscard]]
    static PosOrError jodaMinuteOfHour(size_t repetitions, Pos cur, Pos end, const String &fragment,
                                       ParsedValue<error_handling, return_type> &parsed_value) {
      Int32 minute;
      ASSIGN_RESULT_OR_RETURN_ERROR(
    ...
    [[nodiscard]]
    static PosOrError jodaSecondOfMinute(size_t repetitions, Pos cur, Pos end, const String &fragment,
                                         ParsedValue<error_handling, return_type> &parsed_value) {
      Int32 second;
      ASSIGN_RESULT_OR_RETURN_ERROR(
    ...
    [[nodiscard]]
    static PosOrError jodaMicrosecondOfSecond(size_t repetitions, Pos cur, Pos end, const String &fragment,
                                              ParsedValue<error_handling, return_type> &parsed_value) {
      Int32 microsecond;
      ASSIGN_RESULT_OR_RETURN_ERROR(cur, (readNumberWithVariableLength(cur, end, false, false, false, repetitions,
                                                                       std::max(repetitions, 2uz), fragment, microsecond)))
      RETURN_ERROR_IF_FAILED(parsed_value.setMicrosecond(microsecond))
      return cur;
    }
    ...
    [[nodiscard]]
    static PosOrError jodaTimezoneOffset(size_t repetitions, Pos cur, Pos end, const String &fragment,
                                         ParsedValue<error_handling, return_type> &parsed_value) {
      ...
      if (*cur == '-')
        ...
        RETURN_ERROR(ErrorCodes::CANNOT_PARSE_DATETIME,
                     "Unable to parse fragment {} from {} because of unknown sign in time zone offset: {}", fragment,
                     std::string_view(cur, end - cur), std::string_view(cur, 1))
      ++cur;
      ...
      if (hour < 0 || hour > 23)
        RETURN_ERROR(ErrorCodes::CANNOT_PARSE_DATETIME,
                     "Unable to parse fragment {} from {} because the hour of datetime not in range [0, 23]: {}", fragment,
                     std::string_view(cur, end - cur), std::string_view(cur, 1))
      Int32 minute;
      ASSIGN_RESULT_OR_RETURN_ERROR(
          cur, (readNumberWithVariableLength(cur, end, false, false, false, repetitions, std::max(repetitions, 2uz), fragment, minute)))
      if (minute < 0 || minute > 59)
        RETURN_ERROR(ErrorCodes::CANNOT_PARSE_DATETIME,
                     "Unable to parse fragment {} from {} because the minute of datetime not in range [0, 59]: {}", fragment,
                     std::string_view(cur, end - cur), std::string_view(cur, 1))
      parsed_value.has_time_zone_offset = true;
      parsed_value.time_zone_offset = sign * (hour * 3600 + minute * 60);
      return cur;
  ...
  std::vector<Instruction> parseFormat(const String &format) const {
    static_assert(parse_syntax == ParseSyntax::MySQL || parse_syntax == ParseSyntax::Joda, "parse syntax must be one of MySQL or Joda");
...
#define ACTION_ARGS(func) &(func), #func, std::string_view(pos - 1, 2)
    ...
    while (true) {
      ...
      if (next_percent_pos < end) {
        if (pos < next_percent_pos) instructions.emplace_back(String(pos, next_percent_pos - pos));
        ...
        switch (*pos) {
          ...
          default:
            throw Exception(ErrorCodes::BAD_ARGUMENTS, "Incorrect syntax '{}', symbol is not supported '{}' for function {}", format, *pos,
                            getName());
        }
        ...
        if (pos < end) instructions.emplace_back(String(pos, end - pos));
        break;
      }
    }
    return instructions;
    ...
    while (pos < end) {
      ...
      if (*cur_token == '\'') {
        ...
        if (pos + 1 < end && *(pos + 1) == '\'') {
          ...
          if (count == -1) throw Exception(ErrorCodes::BAD_ARGUMENTS, "No closing single quote for literal");
...
#undef ACTION_ARGS_WITH_BIND
  ...
  String getFormat(const ColumnsWithTypeAndName &arguments) const {
    if (arguments.size() == 1) {
      ...
      if (!col_format)
        throw Exception(ErrorCodes::ILLEGAL_COLUMN, "Illegal type in 'format' argument of function {}. Must be constant String.",
                        getName());
  ...
  const DateLUTImpl &getTimeZone(const ColumnsWithTypeAndName &arguments) const {
    if (arguments.size() < 3) return DateLUT::instance();

    const auto *col = checkAndGetColumnConst<ColumnString>(arguments[2].column.get());
    if (!col)
      throw Exception(ErrorCodes::ILLEGAL_COLUMN, "Illegal type in 'timezone' argument of function {}. Must be constant String.",
                      getName());
...
using FunctionParseDateTime =
    FunctionParseDateTimeImpl<NameParseDateTime, ParseSyntax::MySQL, ReturnType::DateTime, ErrorHandling::Exception>;
using FunctionParseDateTimeOrZero =
    FunctionParseDateTimeImpl<NameParseDateTimeOrZero, ParseSyntax::MySQL, ReturnType::DateTime, ErrorHandling::Zero>;
using FunctionParseDateTimeOrNull =
    FunctionParseDateTimeImpl<NameParseDateTimeOrNull, ParseSyntax::MySQL, ReturnType::DateTime, ErrorHandling::Null>;
using FunctionParseDateTime64 =
    FunctionParseDateTimeImpl<NameParseDateTime64, ParseSyntax::MySQL, ReturnType::DateTime64, ErrorHandling::Exception>;
using FunctionParseDateTime64OrZero =
    FunctionParseDateTimeImpl<NameParseDateTime64OrZero, ParseSyntax::MySQL, ReturnType::DateTime64, ErrorHandling::Zero>;
using FunctionParseDateTime64OrNull =
    FunctionParseDateTimeImpl<NameParseDateTime64OrNull, ParseSyntax::MySQL, ReturnType::DateTime64, ErrorHandling::Null>;
using FunctionParseDateTimeInJodaSyntax =
    FunctionParseDateTimeImpl<NameParseDateTimeInJodaSyntax, ParseSyntax::Joda, ReturnType::DateTime, ErrorHandling::Exception>;
using FunctionParseDateTimeInJodaSyntaxOrZero =
    FunctionParseDateTimeImpl<NameParseDateTimeInJodaSyntaxOrZero, ParseSyntax::Joda, ReturnType::DateTime, ErrorHandling::Zero>;
using FunctionParseDateTimeInJodaSyntaxOrNull =
    FunctionParseDateTimeImpl<NameParseDateTimeInJodaSyntaxOrNull, ParseSyntax::Joda, ReturnType::DateTime, ErrorHandling::Null>;
using FunctionParseDateTime64InJodaSyntax =
    FunctionParseDateTimeImpl<NameParseDateTime64InJodaSyntax, ParseSyntax::Joda, ReturnType::DateTime64, ErrorHandling::Exception>;
using FunctionParseDateTime64InJodaSyntaxOrZero =
    FunctionParseDateTimeImpl<NameParseDateTime64InJodaSyntaxOrZero, ParseSyntax::Joda, ReturnType::DateTime64, ErrorHandling::Zero>;
using FunctionParseDateTime64InJodaSyntaxOrNull =
    FunctionParseDateTimeImpl<NameParseDateTime64InJodaSyntaxOrNull, ParseSyntax::Joda, ReturnType::DateTime64, ErrorHandling::Null>;
}  // namespace
...
It parses a String argument using a format String. Returns a DateTime type.
  ...
  FunctionDocumentation::Arguments parseDateTime_arguments = {{"time_string", "String to be parsed into DateTime.", {"String"}},
                                                              {"format", "Format string specifying how to parse time_string.", {"String"}},
                                                              {"timezone", "Optional. Timezone.", {"String"}}};
  FunctionDocumentation::ReturnedValue parseDateTime_returned_value = {
      "Returns a DateTime parsed from the input string according to the MySQL style format string.", {"DateTime"}};
  FunctionDocumentation::Examples parseDateTime_examples = {{"Usage example",
                                                             R"(
...
└───────────────────────────────────────────────────────────┘
  ...
  FunctionDocumentation parseDateTime_documentation = {
      parseDateTime_description,    parseDateTime_syntax,   parseDateTime_arguments,     {},
      parseDateTime_returned_value, parseDateTime_examples, parseDateTime_introduced_in, parseDateTime_category};
...
Same as [`parseDateTime`](#parseDateTime) but returns zero date when it encounters an unparsable date format.
  ...
  FunctionDocumentation::Arguments parseDateTimeOrZero_arguments = {
      ...
      {"format", "Format string specifying how to parse time_string.", {"String"}},
      {"timezone", "Optional. Timezone.", {"String"}}};
  FunctionDocumentation::ReturnedValue parseDateTimeOrZero_returned_value = {
      "Returns DateTime parsed from input string, or zero DateTime if parsing fails.", {"DateTime"}};
  FunctionDocumentation::Examples parseDateTimeOrZero_examples = {{"Usage example",
                                                                   R"(
...
└─────────────────────────────────────────────────────────────────┘
  ...
  FunctionDocumentation parseDateTimeOrZero_documentation = {
      parseDateTimeOrZero_description,    parseDateTimeOrZero_syntax,   parseDateTimeOrZero_arguments,     {},
      parseDateTimeOrZero_returned_value, parseDateTimeOrZero_examples, parseDateTimeOrZero_introduced_in, parseDateTimeOrZero_category};
...
Same as [`parseDateTime`](#parseDateTime) but returns `NULL` when it encounters an unparsable date format.
  ...
  FunctionDocumentation::Arguments parseDateTimeOrNull_arguments = {
      ...
      {"format", "Format string specifying how to parse time_string.", {"String"}},
      {"timezone", "Optional. Timezone.", {"String"}}};
  FunctionDocumentation::ReturnedValue parseDateTimeOrNull_returned_value = {
      "Returns DateTime parsed from input string, or NULL if parsing fails.", {"Nullable(DateTime)"}};
  FunctionDocumentation::Examples parseDateTimeOrNull_examples = {{"Usage example",
                                                                   R"(
...
└─────────────────────────────────────────────────────────────────┘
  ...
  FunctionDocumentation parseDateTimeOrNull_documentation = {
      parseDateTimeOrNull_description,    parseDateTimeOrNull_syntax,   parseDateTimeOrNull_arguments,     {},
      parseDateTimeOrNull_returned_value, parseDateTimeOrNull_examples, parseDateTimeOrNull_introduced_in, parseDateTimeOrNull_category};
...
It parses a String argument using a format String. Returns a DateTime64 type which can represent dates from 1900 to 2299 with sub-second precision.
  ...
  FunctionDocumentation::Arguments parseDateTime64_arguments = {
      ...
      {"format", "Format string specifying how to parse time_string.", {"String"}},
      {"timezone", "Optional. Timezone.", {"String"}}};
  FunctionDocumentation::ReturnedValue parseDateTime64_returned_value = {
      "Returns a DateTime64 parsed from the input string according to the MySQL style format string.", {"DateTime64"}};
  FunctionDocumentation::Examples parseDateTime64_examples = {{"Usage example",
                                                               R"(
...
└─────────────────────────────────────────────────────────────────────┘
  ...
  FunctionDocumentation parseDateTime64_documentation = {
      parseDateTime64_description,    parseDateTime64_syntax,   parseDateTime64_arguments,     {},
      parseDateTime64_returned_value, parseDateTime64_examples, parseDateTime64_introduced_in, parseDateTime64_category};
...
Same as [`parseDateTime64`](#parseDateTime64) but returns zero date when it encounters an unparsable date format.
  ...
  FunctionDocumentation::Arguments parseDateTime64OrZero_arguments = {
      ...
      {"format", "Format string specifying how to parse time_string.", {"String"}},
      {"timezone", "Optional. Timezone.", {"String"}}};
  FunctionDocumentation::ReturnedValue parseDateTime64OrZero_returned_value = {
      "Returns DateTime64 parsed from input string, or zero DateTime64 if parsing fails.", {"DateTime64"}};
  FunctionDocumentation::Examples parseDateTime64OrZero_examples = {{"Usage example",
                                                                     R"(
...
└───────────────────────────────────────────────────────────────────────────┘
  ...
  FunctionDocumentation parseDateTime64OrZero_documentation = {parseDateTime64OrZero_description,    parseDateTime64OrZero_syntax,
                                                               parseDateTime64OrZero_arguments,      {},
                                                               parseDateTime64OrZero_returned_value, parseDateTime64OrZero_examples,
                                                               parseDateTime64OrZero_introduced_in,  parseDateTime64OrZero_category};
...
Same as [`parseDateTime64`](#parseDateTime64) but returns `NULL` when it encounters an unparsable date format.
  ...
  FunctionDocumentation::Arguments parseDateTime64OrNull_arguments = {
      ...
      {"format", "Format string specifying how to parse time_string.", {"String"}},
      {"timezone", "Optional. Timezone.", {"String"}}};
  FunctionDocumentation::ReturnedValue parseDateTime64OrNull_returned_value = {
      "Returns DateTime64 parsed from input string, or NULL if parsing fails.", {"Nullable(DateTime64)"}};
  FunctionDocumentation::Examples parseDateTime64OrNull_examples = {{"Usage example",
                                                                     R"(
...
└───────────────────────────────────────────────────────────────────────────┘
  ...
  FunctionDocumentation parseDateTime64OrNull_documentation = {parseDateTime64OrNull_description,    parseDateTime64OrNull_syntax,
                                                               parseDateTime64OrNull_arguments,      {},
                                                               parseDateTime64OrNull_returned_value, parseDateTime64OrNull_examples,
                                                               parseDateTime64OrNull_introduced_in,  parseDateTime64OrNull_category};
...
Refer to [Joda Time documentation](https://joda-time.sourceforge.net/apidocs/org/joda/time/format/DateTimeFormat.html) for the format patterns.
  ...
  FunctionDocumentation::Arguments parseDateTimeInJodaSyntax_arguments = {
      ...
      {"timezone", "Optional. Timezone.", {"String"}}};
  FunctionDocumentation::ReturnedValue parseDateTimeInJodaSyntax_returned_value = {
      "Returns a DateTime parsed from the input string according to the Joda style format string.", {"DateTime"}};
  FunctionDocumentation::Examples parseDateTimeInJodaSyntax_examples = {{"Usage example",
                                                                         R"(
...
└──────────────────────────────────────────────────────────────────────────┘
  ...
  FunctionDocumentation parseDateTimeInJodaSyntax_documentation = {
      parseDateTimeInJodaSyntax_description,    parseDateTimeInJodaSyntax_syntax,
      parseDateTimeInJodaSyntax_arguments,      {},
      parseDateTimeInJodaSyntax_returned_value, parseDateTimeInJodaSyntax_examples,
      parseDateTimeInJodaSyntax_introduced_in,  parseDateTimeInJodaSyntax_category};
...
Same as [`parseDateTimeInJodaSyntax`](#parseDateTimeInJodaSyntax) but returns zero date when it encounters an unparsable date format.
  ...
  FunctionDocumentation::Arguments parseDateTimeInJodaSyntaxOrZero_arguments = {
      ...
      {"format", "Format string in Joda syntax specifying how to parse time_string.", {"String"}},
      {"timezone", "Optional. Timezone.", {"String"}}};
  FunctionDocumentation::ReturnedValue parseDateTimeInJodaSyntaxOrZero_returned_value = {
      "Returns DateTime parsed from input string, or zero DateTime if parsing fails.", {"DateTime"}};
  FunctionDocumentation::Examples parseDateTimeInJodaSyntaxOrZero_examples = {{"Usage example",
                                                                               R"(
...
└────────────────────────────────────────────────────────────────────────────────┘
  ...
  FunctionDocumentation parseDateTimeInJodaSyntaxOrZero_documentation = {
      parseDateTimeInJodaSyntaxOrZero_description,    parseDateTimeInJodaSyntaxOrZero_syntax,
      parseDateTimeInJodaSyntaxOrZero_arguments,      {},
      parseDateTimeInJodaSyntaxOrZero_returned_value, parseDateTimeInJodaSyntaxOrZero_examples,
      parseDateTimeInJodaSyntaxOrZero_introduced_in,  parseDateTimeInJodaSyntaxOrZero_category};
...
Same as [`parseDateTimeInJodaSyntax`](#parseDateTimeInJodaSyntax) but returns `NULL` when it encounters an unparsable date format.
  ...
  FunctionDocumentation::Arguments parseDateTimeInJodaSyntaxOrNull_arguments = {
      ...
      {"format", "Format string in Joda syntax specifying how to parse time_string.", {"String"}},
      {"timezone", "Optional. Timezone.", {"String"}}};
  FunctionDocumentation::ReturnedValue parseDateTimeInJodaSyntaxOrNull_returned_value = {
      "Returns DateTime parsed from input string, or NULL if parsing fails.", {"Nullable(DateTime)"}};
  FunctionDocumentation::Examples parseDateTimeInJodaSyntaxOrNull_examples = {{"Usage example",
                                                                               R"(
...
└────────────────────────────────────────────────────────────────────────────────┘
  ...
  FunctionDocumentation parseDateTimeInJodaSyntaxOrNull_documentation = {
      parseDateTimeInJodaSyntaxOrNull_description,    parseDateTimeInJodaSyntaxOrNull_syntax,
      parseDateTimeInJodaSyntaxOrNull_arguments,      {},
      parseDateTimeInJodaSyntaxOrNull_returned_value, parseDateTimeInJodaSyntaxOrNull_examples,
      parseDateTimeInJodaSyntaxOrNull_introduced_in,  parseDateTimeInJodaSyntaxOrNull_category};
...
Refer to [Joda Time documentation](https://joda-time.sourceforge.net/apidocs/org/joda/time/format/DateTimeFormat.html) for the format patterns.
  ...
  FunctionDocumentation::Arguments parseDateTime64InJodaSyntax_arguments = {
      ...
      {"format", "Format string in Joda syntax specifying how to parse time_string.", {"String"}},
      {"timezone", "Optional. Timezone.", {"String"}}};
  FunctionDocumentation::ReturnedValue parseDateTime64InJodaSyntax_returned_value = {
      "Returns a DateTime64 parsed from the input string according to the Joda style format string.", {"DateTime64"}};
  FunctionDocumentation::Examples parseDateTime64InJodaSyntax_examples = {{"Usage example",
                                                                           R"(
...
└────────────────────────────────────────────────────────────────────────────────────┘
  ...
  FunctionDocumentation parseDateTime64InJodaSyntax_documentation = {
      parseDateTime64InJodaSyntax_description,    parseDateTime64InJodaSyntax_syntax,
      parseDateTime64InJodaSyntax_arguments,      {},
      parseDateTime64InJodaSyntax_returned_value, parseDateTime64InJodaSyntax_examples,
      parseDateTime64InJodaSyntax_introduced_in,  parseDateTime64InJodaSyntax_category};
...
Same as [`parseDateTime64InJodaSyntax`](#parseDateTime64InJodaSyntax) but returns zero date when it encounters an unparsable date format.
  ...
  FunctionDocumentation::Syntax parseDateTime64InJodaSyntaxOrZero_syntax =
  ...
  FunctionDocumentation::Arguments parseDateTime64InJodaSyntaxOrZero_arguments = {
      ...
      {"format", "Format string in Joda syntax specifying how to parse time_string.", {"String"}},
      {"timezone", "Optional. Timezone.", {"String"}}};
  FunctionDocumentation::ReturnedValue parseDateTime64InJodaSyntaxOrZero_returned_value = {
      "Returns DateTime64 parsed from input string, or zero DateTime64 if parsing fails.", {"DateTime64"}};
  FunctionDocumentation::Examples parseDateTime64InJodaSyntaxOrZero_examples = {{"Usage example",
                                                                                 R"(
...
└──────────────────────────────────────────────────────────────────────────────────────────┘
  ...
  FunctionDocumentation parseDateTime64InJodaSyntaxOrZero_documentation = {
      parseDateTime64InJodaSyntaxOrZero_description,    parseDateTime64InJodaSyntaxOrZero_syntax,
      parseDateTime64InJodaSyntaxOrZero_arguments,      {},
      parseDateTime64InJodaSyntaxOrZero_returned_value, parseDateTime64InJodaSyntaxOrZero_examples,
      parseDateTime64InJodaSyntaxOrZero_introduced_in,  parseDateTime64InJodaSyntaxOrZero_category};
...
Same as [`parseDateTime64InJodaSyntax`](#parseDateTime64InJodaSyntax) but returns `NULL` when it encounters an unparsable date format.
  ...
  FunctionDocumentation::Syntax parseDateTime64InJodaSyntaxOrNull_syntax =
  ...
  FunctionDocumentation::Arguments parseDateTime64InJodaSyntaxOrNull_arguments = {
      ...
      {"format", "Format string in Joda syntax specifying how to parse time_string.", {"String"}},
      {"timezone", "Optional. Timezone.", {"String"}}};
  FunctionDocumentation::ReturnedValue parseDateTime64InJodaSyntaxOrNull_returned_value = {
      "Returns DateTime64 parsed from input string, or NULL if parsing fails.", {"Nullable(DateTime64)"}};
  FunctionDocumentation::Examples parseDateTime64InJodaSyntaxOrNull_examples = {{"Usage example",
                                                                                 R"(
...
└──────────────────────────────────────────────────────────────────────────────────────────┘
  ...
  FunctionDocumentation parseDateTime64InJodaSyntaxOrNull_documentation = {
      parseDateTime64InJodaSyntaxOrNull_description,    parseDateTime64InJodaSyntaxOrNull_syntax,
      parseDateTime64InJodaSyntaxOrNull_arguments,      {},
      parseDateTime64InJodaSyntaxOrNull_returned_value, parseDateTime64InJodaSyntaxOrNull_examples,
