...
class FunctionGetSizeOfEnumType : public IFunction {
 public:
  ...
  static FunctionPtr create(ContextPtr) { return std::make_shared<FunctionGetSizeOfEnumType>(); }
  ...
  DataTypePtr getReturnTypeImpl(const DataTypes &arguments) const override {
    ...
    if (which.isEnum8()) return std::make_shared<DataTypeUInt8>();
    if (which.isEnum16()) return std::make_shared<DataTypeUInt16>();
...
Returns the number of fields in the given [`Enum`](../../sql-reference/data-types/enum.md).
  ...
  FunctionDocumentation::Arguments arguments = {{"x", "Value of type `Enum`.", {"Enum"}}};
