...
ColumnPtr executeDecimal(const ColumnWithTypeAndName &col_left, const ColumnWithTypeAndName &col_right, bool check_decimal_overflow) {
  ...
  auto call = [&](const auto &types) -> bool {
    ...
    return (res = DecimalComparison<LeftDataType, RightDataType, Operation>::apply(col_left, col_right, check_decimal_overflow)) != nullptr;
  };

  if (!callOnAtLeastOneDecimalType<true, false, true, true>(left_number, right_number, call))
    throw Exception(ErrorCodes::LOGICAL_ERROR, "Wrong call for {} with {} and {}", Name::name, col_left.type->getName(),
...
struct NumComparisonImpl {
  ...
  MULTITARGET_FUNCTION_X86_V4_V3(MULTITARGET_FUNCTION_HEADER(static void), vectorVectorImpl,
                                 MULTITARGET_FUNCTION_BODY((  /// NOLINT
  ...
  MULTITARGET_FUNCTION_X86_V4_V3(MULTITARGET_FUNCTION_HEADER(static void), vectorConstantImpl,
                                 MULTITARGET_FUNCTION_BODY((  /// NOLINT
...
struct StringComparisonImpl {
  static void NO_INLINE string_vector_string_vector(  /// NOLINT
      ...
      const ColumnString::Offsets &b_offsets, PaddedPODArray<UInt8> &c) {
    size_t size = a_offsets.size();
    ColumnString::Offset prev_a_offset = 0;
    ColumnString::Offset prev_b_offset = 0;

    for (size_t i = 0; i < size; ++i) {
      c[i] = Op::apply(memcmpSmallAllowOverflow15(a_data.data() + prev_a_offset, a_offsets[i] - prev_a_offset,
                                                  b_data.data() + prev_b_offset, b_offsets[i] - prev_b_offset),
                       0);
  ...
  static void NO_INLINE string_vector_fixed_string_vector(  /// NOLINT
    ...
    for (size_t i = 0; i < size; ++i) {
      c[i] = Op::apply(memcmpSmallLikeZeroPaddedAllowOverflow15(a_data.data() + prev_a_offset, a_offsets[i] - prev_a_offset,
  ...
  static void NO_INLINE string_vector_constant(  /// NOLINT
      const ColumnString::Chars &a_data, const ColumnString::Offsets &a_offsets, const ColumnString::Chars &b_data,
      ColumnString::Offset b_size, PaddedPODArray<UInt8> &c) {
    size_t size = a_offsets.size();
    ColumnString::Offset prev_a_offset = 0;

    for (size_t i = 0; i < size; ++i) {
      c[i] = Op::apply(memcmpSmallAllowOverflow15(a_data.data() + prev_a_offset, a_offsets[i] - prev_a_offset, b_data.data(), b_size), 0);
  ...
  static void NO_INLINE fixed_string_vector_fixed_string_vector_16(  /// NOLINT
      const ColumnString::Chars &a_data, const ColumnString::Chars &b_data, PaddedPODArray<UInt8> &c) {
    size_t size = a_data.size();

    for (size_t i = 0, j = 0; i < size; i += 16, ++j) c[j] = Op::apply(memcmp16(&a_data[i], &b_data[i]), 0);
  }
  ...
  static void NO_INLINE fixed_string_vector_fixed_string_vector(  /// NOLINT
      const ColumnString::Chars &a_data, ColumnString::Offset a_n, const ColumnString::Chars &b_data, ColumnString::Offset b_n,
  ...
  static void NO_INLINE fixed_string_vector_constant(  /// NOLINT
      const ColumnString::Chars &a_data, ColumnString::Offset a_n, const ColumnString::Chars &b_data, ColumnString::Offset b_size,
      PaddedPODArray<UInt8> &c) {
    if (a_n == 16 && b_size == 16) {
  ...
  static void constant_fixed_string_vector(  /// NOLINT
      const ColumnString::Chars &a_data, ColumnString::Offset a_size, const ColumnString::Chars &b_data, ColumnString::Offset b_n,
      PaddedPODArray<UInt8> &c) {
...
struct StringEqualsImpl {
  static void NO_INLINE string_vector_string_vector(  /// NOLINT
    ...
    for (size_t i = 0; i < size; ++i) {
      ...
      c[i] = positive == memequalSmallAllowOverflow15(a_data.data() + prev_a_offset, a_size, b_data.data() + prev_b_offset, b_size);
  ...
  static void NO_INLINE string_vector_fixed_string_vector(  /// NOLINT
    ...
    for (size_t i = 0; i < size; ++i) {
      ...
      c[i] = positive == memequalSmallLikeZeroPaddedAllowOverflow15(a_data.data() + prev_a_offset, a_size, b_data.data() + b_n * i, b_n);
  ...
  static void NO_INLINE string_vector_constant(  /// NOLINT
    ...
    if (b_size == 0) {
      ...
      for (size_t i = 0; i < size; ++i) {
        ...
        c[i] = positive == memequalSmallAllowOverflow15(a_data.data() + prev_a_offset, a_size, b_data.data(), b_size);
  ...
  static void NO_INLINE fixed_string_vector_fixed_string_vector_16(  /// NOLINT
    ...
    for (size_t i = 0; i < size; ++i) c[i] = positive == memequal16(a_data.data() + i * 16, b_data.data() + i * 16);
  }
  ...
  static void NO_INLINE fixed_string_vector_constant_16(  /// NOLINT
    ...
    for (size_t i = 0; i < size; ++i) c[i] = positive == memequal16(a_data.data() + i * 16, b_data.data());
  }
...
struct GenericComparisonImpl {
  static void NO_INLINE vectorVector(const IColumn &a, const IColumn &b, PaddedPODArray<UInt8> &c) {
    for (size_t i = 0, size = a.size(); i < size; ++i) c[i] = Op::apply(a.compareAt(i, i, b, 1), 0);
  }
  ...
  static void NO_INLINE vectorConstant(const IColumn &a, const IColumn &b, PaddedPODArray<UInt8> &c) {
    ...
    for (size_t i = 0, size = a.size(); i < size; ++i) c[i] = Op::apply(a.compareAt(i, 0, *b_materialized, 1), 0);
  }
  ...
  static void constantConstant(const IColumn &a, const IColumn &b, UInt8 &c) { c = Op::apply(a.compareAt(0, 0, b, 1), 0); }
};
...
template <>
struct CompileOp<IsNotDistinctFromOp> {
  static llvm::Value *compile(llvm::IRBuilder<> &b, llvm::Value *x, llvm::Value *y, bool /*is_signed*/) {
...
class FunctionComparison : public IFunction {
 public:
  ...
  static FunctionPtr create(ContextPtr context) {
    return std::make_shared<FunctionComparison>(context ? ComparisonParams(context) : ComparisonParams());
  }
  ...
  bool ALWAYS_INLINE useDefaultImplementationForVariant() const override {
    return is_null_safe_cmp_mode ? false : params.use_variant_default_implementation;
  }
 ...
 private:
  ...
  ColumnPtr executeNumConstRightType(const ColumnConst *col_left, const IColumn *col_right_untyped) const {
    ...
    if (auto col_right_const = checkAndGetColumnConst<ColumnVector<T1>>(col_right_untyped)) {
      UInt8 res = 0;
      NumComparisonImpl<T0, T1, Op<T0, T1>>::constantConstant(col_left->template getValue<T0>(), col_right_const->template getValue<T1>(),
                                                              res);
  ...
  ColumnPtr executeNumLeftType(const IColumn *col_left_untyped, const IColumn *col_right_untyped) const {
    ...
    if (const ColumnVector<T0> *col_left = checkAndGetColumn<ColumnVector<T0>>(col_left_untyped)) {
      if ((res = executeNumRightType<T0, UInt8>(col_left, col_right_untyped)) ||
        ...
        return res;
      throw Exception(ErrorCodes::ILLEGAL_COLUMN, "Illegal column {} of second argument of function {}", col_right_untyped->getName(),
                      getName());
    }
  ...
  ColumnPtr executeString(const IColumn *c0, const IColumn *c1) const {
    ...
    if (!((c0_string || c0_fixed_string || c0_const) && (c1_string || c1_fixed_string || c1_const))) return nullptr;
    ...
    if (c0_string && c1_string)
      StringImpl::string_vector_string_vector(c0_string->getChars(), c0_string->getOffsets(), c1_string->getChars(),
                                              c1_string->getOffsets(), c_res->getData());
    else if (c0_string && c1_fixed_string)
      StringImpl::string_vector_fixed_string_vector(c0_string->getChars(), c0_string->getOffsets(), c1_fixed_string->getChars(),
                                                    c1_fixed_string->getN(), c_res->getData());
    else if (c0_string && c1_const)
      StringImpl::string_vector_constant(c0_string->getChars(), c0_string->getOffsets(), *c1_const_chars, c1_const_size, c_res->getData());
    else if (c0_fixed_string && c1_string)
      StringImpl::fixed_string_vector_string_vector(c0_fixed_string->getChars(), c0_fixed_string->getN(), c1_string->getChars(),
                                                    c1_string->getOffsets(), c_res->getData());
    else if (c0_fixed_string && c1_fixed_string)
      StringImpl::fixed_string_vector_fixed_string_vector(c0_fixed_string->getChars(), c0_fixed_string->getN(), c1_fixed_string->getChars(),
                                                          c1_fixed_string->getN(), c_res->getData());
    else if (c0_fixed_string && c1_const)
      StringImpl::fixed_string_vector_constant(c0_fixed_string->getChars(), c0_fixed_string->getN(), *c1_const_chars, c1_const_size,
                                               c_res->getData());
    else if (c0_const && c1_string)
      StringImpl::constant_string_vector(*c0_const_chars, c0_const_size, c1_string->getChars(), c1_string->getOffsets(), c_res->getData());
    else if (c0_const && c1_fixed_string)
      StringImpl::constant_fixed_string_vector(*c0_const_chars, c0_const_size, c1_fixed_string->getChars(), c1_fixed_string->getN(),
                                               c_res->getData());
    else
      throw Exception(ErrorCodes::ILLEGAL_COLUMN, "Illegal columns {} and {} of arguments of function {}", c0->getName(), c1->getName(),
                      getName());

    return c_res;
  }

  ColumnPtr executeWithConstString(const DataTypePtr &result_type, const IColumn *col_left_untyped, const IColumn *col_right_untyped,
    ...
    if (!left_const && !right_const) return nullptr;
    ...
    auto is_string_not_in_enum = [this, &string_value]<typename T>(const EnumValues<T> *enum_values) -> bool {
      if constexpr (!IsOperation<Op>::equals && IsOperation<Op>::not_equals) return false;
      if (params.validate_enum_literals_in_operators) return false;
      if (!enum_values || string_value.getType() != Field::Types::String) return false;
      T res;
  ...
  ColumnPtr executeTuple(const DataTypePtr &result_type, const ColumnWithTypeAndName &c0, const ColumnWithTypeAndName &c1,
    ...
    if (0 == tuple_size) throw Exception(ErrorCodes::NOT_IMPLEMENTED, "Comparison of zero-sized tuples is not implemented.");
    ...
    if constexpr (!is_null_safe_cmp_mode) {
      ...
      for (size_t i = 0; i < tuple_size; ++i) {
        if (left_elems[i]->onlyNull() || isNothing(left_elems[i]) || right_elems[i]->onlyNull() || isNothing(right_elems[i])) {
  ...
  ColumnPtr executeTupleImpl(const ColumnsWithTypeAndName &x, const ColumnsWithTypeAndName &y, size_t tuple_size,
                             size_t input_rows_count) const;

  ColumnPtr executeTupleEqualityImpl(std::shared_ptr<IFunctionOverloadResolver> func_compare,
                                     std::shared_ptr<IFunctionOverloadResolver> func_convolution, const ColumnsWithTypeAndName &x,
                                     const ColumnsWithTypeAndName &y, size_t tuple_size, size_t input_rows_count) const {
    if (0 == tuple_size) throw Exception(ErrorCodes::NOT_IMPLEMENTED, "Comparison of zero-sized tuples is not implemented.");
  ...
  ColumnPtr executeTupleLessGreaterImpl(std::shared_ptr<IFunctionOverloadResolver> func_compare_head,
                                        ...
                                        std::shared_ptr<IFunctionOverloadResolver> func_equals, const ColumnsWithTypeAndName &x,
                                        const ColumnsWithTypeAndName &y, size_t tuple_size, size_t input_rows_count) const {
    if (0 == tuple_size) throw Exception(ErrorCodes::NOT_IMPLEMENTED, "Comparison of zero-sized tuples is not implemented");
    ...
    while (i > 0) {
      ...
      tmp_columns[0].column =
          func_and_adaptor->execute(tmp_columns, func_and_adaptor->getResultType(), input_rows_count, /* dry_run = */ false);
      ...
      tmp_columns[0].column =
          func_or_adaptor->execute(tmp_columns, func_or_adaptor->getResultType(), input_rows_count, /* dry_run = */ false);
  ...
  ColumnPtr executeGenericIdenticalTypes(const IColumn *c0, const IColumn *c1) const {
    ...
    if (c0->isNullable() != c1->isNullable())
      throw Exception(ErrorCodes::LOGICAL_ERROR, "Columns are assumed to be of identical types, but they are different in Nullable");
 ...
 public:
  ...
  DataTypePtr getReturnTypeImpl(const DataTypes &arguments) const override {
    if ((name == NameEquals::name || name == NameNotEquals::name)) {
      if (!arguments[0]->isComparableForEquality() || !arguments[1]->isComparableForEquality())
        throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT,
                        ...
                        backQuote(arguments[0]->getName()), backQuote(arguments[1]->getName()), backQuote(getName()));
    ...
    if (!((both_represented_by_number && !has_date)                           /// Do not allow to compare date and number.
          ...
          || ((left.isDate() || left.isDate32() || left.isDateTime() || left.isDateTime64()) &&
              (right.isDate() || right.isDate32() || right.isDateTime() || right.isDateTime64()) &&
              left.idx == right.idx)  /// only date vs date, or datetime vs datetime
          || (left.isUUID() && right.isUUID()) || ((left.isIPv4() || left.isIPv6()) && (right.isIPv4() || right.isIPv6())) ||
          (left.isEnum() && right.isEnum() &&
           arguments[0]->getName() == arguments[1]->getName())  /// only equivalent enum type values can be compared against
          || (left_tuple && right_tuple && left_tuple->getElements().size() == right_tuple->getElements().size()) ||
          (arguments[0]->equals(*arguments[1])))) {
      if (!tryGetLeastSupertype(arguments))
        throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT,
                        "Illegal types of arguments ({}, {})"
                        " of function {}",
                        backQuote(arguments[0]->getName()), backQuote(arguments[1]->getName()), backQuote(getName()));
    }
    ...
    if (both_tuples || (left_tuple && right.isStringOrFixedString()) || (left.isStringOrFixedString() && right_tuple)) {
      ...
      for (size_t i = 0; i < size; ++i) {
        ...
        if (both_tuples) {
          ColumnsWithTypeAndName args = {{nullptr, left_tuple->getElements()[i], ""}, {nullptr, right_tuple->getElements()[i], ""}};
      ...
      if (is_null_safe_cmp_mode) return std::make_shared<DataTypeUInt8>();

      if (has_nothing) return std::make_shared<DataTypeNullable>(std::make_shared<DataTypeUInt8>());
      ...
      if (has_null) return std::make_shared<DataTypeNullable>(std::make_shared<DataTypeUInt8>());
      if (has_nullable) return std::make_shared<DataTypeNullable>(std::make_shared<DataTypeUInt8>());
    }
  ...
  ColumnPtr executeImpl(const ColumnsWithTypeAndName &arguments, const DataTypePtr &result_type, size_t input_rows_count) const override {
    ...
    if (left_type->equals(*right_type) && !left_type->isNullable() && !isTuple(left_type) && !WhichDataType(left_type).isFloat() &&
        col_left_untyped == col_right_untyped) {
      ColumnPtr result_column;

      /// Always true: =, <=, >=
      if constexpr (IsOperation<Op>::equals || IsOperation<Op>::less_or_equals || IsOperation<Op>::greater_or_equals) {
      ...
      if (!isColumnConst(*col_left_untyped)) result_column = result_column->convertToFullColumnIfConst();
    ...
    bool date_and_time_datetime = (which_left.idx != which_right.idx) &&
                                  (which_left.isDate() || which_left.isDate32() || which_left.isTime() || which_left.isTime64() ||
                                   which_left.isDateTime() || which_left.isDateTime64()) &&
                                  (which_right.isDate() || which_right.isDate32() || which_right.isTime() || which_right.isTime64() ||
                                   which_right.isDateTime() || which_right.isDateTime64());
    ...
    ColumnPtr res;
    if (left_is_num && right_is_num && !date_and_time_datetime && (!left_is_interval || !right_is_interval || types_equal)) {
      if (!((res = executeNumLeftType<UInt8>(col_left_untyped, col_right_untyped)) ||
        ...
        throw Exception(ErrorCodes::ILLEGAL_COLUMN, "Illegal column {} of the first argument of function {}", col_left_untyped->getName(),
                        getName());
    ...
    if ((((left_is_ipv6 && right_is_fixed_string) || (right_is_ipv6 && left_is_fixed_string)) && fixed_string_size == IPV6_BINARY_LENGTH) ||
      ...
      ColumnPtr right_column = right_is_ipv6 ? col_with_type_and_name_right.column : castColumn(col_with_type_and_name_right, left_type);
    ...
    if ((isColumnedAsDecimal(left_type) || isColumnedAsDecimal(right_type))) {
      ...
      if (date_and_time_datetime && (isDateOrDate32(left_type) || isDateOrDate32(right_type))) {
        ...
        return executeDecimal<Op, Name>({c0_converted, common_type, "left"}, {c1_converted, common_type, "right"},
                                        params.check_decimal_overflow);
      }
      ...
      if (!allowDecimalComparison(left_type, right_type) && !date_and_time_datetime)
        throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT, "No operation {} between {} and {}", getName(), left_type->getName(),
                        right_type->getName());
      ...
      if (left_is_float || right_is_float) {
        ...
        auto new_arguments = ColumnsWithTypeAndName{{c0_converted, converted_type, "left"}, {c1_converted, converted_type, "right"}};
    ...
    if (date_and_time_datetime) {
      ...
      if (!((res = executeNumLeftType<UInt32>(c0_converted.get(), c1_converted.get())) ||
            ...
            (res = executeDecimal<Op, Name>({c0_converted, common_type, "left"}, {c1_converted, common_type, "right"},
                                            params.check_decimal_overflow))))
  ...
  ColumnPtr getConstantResultForNonConstArguments(const ColumnsWithTypeAndName &arguments, const DataTypePtr &result_type) const override {
    for (const auto &argument : arguments) {
      ...
      if (!column || !isColumnConst(*column)) continue;

      if (column->isNullAt(0)) return result_type->createColumnConst(1, Null());
    }
  ...
  static bool castType(const IDataType *type, F &&f) {
    return castTypeToEither<DataTypeUInt8, DataTypeUInt16, DataTypeUInt32, DataTypeUInt64, DataTypeInt8, DataTypeInt16, DataTypeInt32,
                            DataTypeInt64, DataTypeFloat32, DataTypeFloat64>(type, std::forward<F>(f));
  }
  ...
  bool isCompilableImpl(const DataTypes &arguments, const DataTypePtr &result_type) const override {
    ...
    if ((data_type_lhs.isDateOrDate32() || data_type_lhs.isDateTime()) || (data_type_rhs.isDateOrDate32() || data_type_rhs.isDateTime()))
      return false;
    ...
    castBothTypes(arguments[0].type.get(), arguments[1].type.get(), [&](const auto &left, const auto &right) {
      ...
      if constexpr (!std::is_same_v<DataTypeFixedString, LeftDataType> && !std::is_same_v<DataTypeFixedString, RightDataType> &&
        ...
        if constexpr (OpSpec::compilable) {
          ...
          result = b.CreateSelect(CompileOp<Op>::compile(b, left_value, right_value, std::is_signed_v<PromotedType>), b.getInt8(1),
                                  b.getInt8(0));
          return true;
        }
      }
