...
class FunctionCurrentRoles : public IFunction {
 public:
  static FunctionPtr create(const ContextPtr &context, Kind kind) { return std::make_shared<FunctionCurrentRoles>(context, kind); }

  bool isSuitableForShortCircuitArgumentsExecution(const DataTypesWithConstInfo & /*arguments*/) const override { return false; }

  String getName() const override { return toString(kind); }

  explicit FunctionCurrentRoles(const ContextPtr &context_, Kind kind_) : context(context_), kind(kind_) {}
  ...
  ColumnPtr executeImpl(const ColumnsWithTypeAndName &, const DataTypePtr &, size_t input_rows_count) const override {
    ...
    for (const String &role_name : role_names) res_strings.insertData(role_name.data(), role_name.length());
...
Returns an array of the roles which are assigned to the current user.
  ...
  FunctionDocumentation::ReturnedValue returned_value_currentRoles = {
      "Returns an array of the roles which are assigned to the current user.", {"Array(String)"}};
  FunctionDocumentation::Examples examples_currentRoles = {{"Usage example",
                                                            R"(
SELECT currentRoles();
        )",
                                                            R"(
...
└────────────────────────────────────────────────┘
  ...
  FunctionDocumentation documentation_currentRoles = {
      description_currentRoles,    syntax_currentRoles,   arguments_currentRoles,     {},
      returned_value_currentRoles, examples_currentRoles, introduced_in_currentRoles, category_currentRoles};
...
Returns an array of the roles which are enabled for the current user.
  ...
  FunctionDocumentation::ReturnedValue returned_value_enabledRoles = {
      "Returns an array of role names which are enabled for the current user.", {"Array(String)"}};
  FunctionDocumentation::Examples examples_enabledRoles = {{"Usage example",
                                                            R"(
SELECT enabledRoles();
        )",
                                                            R"(
...
└────────────────────────────────────────────────────────────────┘
  ...
  FunctionDocumentation documentation_enabledRoles = {
      description_enabledRoles,    syntax_enabledRoles,   arguments_enabledRoles,     {},
      returned_value_enabledRoles, examples_enabledRoles, introduced_in_enabledRoles, category_enabledRoles};
...
Returns an array of default roles for the current user.
  ...
  FunctionDocumentation::ReturnedValue returned_value_defaultRoles = {"Returns an array of default roles for the current user.",
                                                                      {"Array(String)"}};
  FunctionDocumentation::Examples examples_defaultRoles = {{"Usage example",
                                                            R"(
SELECT defaultRoles();
        )",
                                                            R"(
...
└────────────────────────────────────────────────┘
  ...
  FunctionDocumentation documentation_defaultRoles = {
      description_defaultRoles,    syntax_defaultRoles,   arguments_defaultRoles,     {},
      returned_value_defaultRoles, examples_defaultRoles, introduced_in_defaultRoles, category_defaultRoles};

  factory.registerFunction(
      "currentRoles", [](ContextPtr context) { return FunctionCurrentRoles::create(context, Kind::CURRENT_ROLES); },
      documentation_currentRoles);
  factory.registerFunction(
      "enabledRoles", [](ContextPtr context) { return FunctionCurrentRoles::create(context, Kind::ENABLED_ROLES); },
      documentation_enabledRoles);
  factory.registerFunction(
      "defaultRoles", [](ContextPtr context) { return FunctionCurrentRoles::create(context, Kind::DEFAULT_ROLES); },
      documentation_defaultRoles);
}
