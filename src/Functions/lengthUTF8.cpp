...
struct LengthUTF8Impl {
  ...
  static void vectorFixedToVector(const ColumnString::Chars &data, size_t n, PaddedPODArray<UInt64> &res, size_t input_rows_count) {
    for (size_t i = 0; i < input_rows_count; ++i) res[i] = UTF8::countCodePoints(&data[i * n], n);
  }
...
If this assumption is violated, no exception is thrown and the result is undefined.
  ...
  FunctionDocumentation::Arguments arguments = {{"s", "String containing valid UTF-8 encoded text.", {"String"}}};
  FunctionDocumentation::ReturnedValue returned_value = {"Length of the string `s` in Unicode code points.", {"UInt64"}};
  FunctionDocumentation::Examples examples = {{"Usage example", "SELECT lengthUTF8('Здравствуй, мир!')",
                                               R"(
