...
struct DataTypeToTimeTypeMap {};

template <>
struct DataTypeToTimeTypeMap<DataTypeDate> {
  using TimeType = UInt16;
};

template <>
struct DataTypeToTimeTypeMap<DataTypeDate32> {
  using TimeType = Int32;
};

template <>
struct DataTypeToTimeTypeMap<DataTypeDateTime> {
  using TimeType = UInt32;
};

template <>
struct DataTypeToTimeTypeMap<DataTypeDateTime64> {
...
class FunctionDateNameImpl : public IFunction {
 public:
  ...
  DataTypePtr getReturnTypeImpl(const ColumnsWithTypeAndName &arguments) const override {
    ...
    if (!(first_argument_type.isDate() || first_argument_type.isDateTime() || first_argument_type.isDate32() ||
          first_argument_type.isDateTime64()))
      throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT,
                      "Illegal type {} of 2 argument of function {}. Must be a date or a date with time", arguments[1].type->getName(),
                      getName());

    if (arguments.size() == 3 && !WhichDataType(arguments[2].type).isString())
      throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT, "Illegal type {} of 3 argument of function {}. Must be string",
 ...
 private:
  ...
  struct MonthWriter {
    static void write(WriteBuffer &buffer, Time source, const DateLUTImpl &timezone) {
      ...
      static constexpr std::string_view month_names[] = {"January", "February", "March",     "April",   "May",      "June",
  ...
  struct WeekDayWriter {
    static void write(WriteBuffer &buffer, Time source, const DateLUTImpl &timezone) {
      ...
      static constexpr std::string_view day_names[] = {"Monday", "Tuesday", "Wednesday", "Thursday", "Friday", "Saturday", "Sunday"};
...
dateName(date_part, date[, timezone])
  ...
  FunctionDocumentation::Examples examples = {{"Extract different date parts", R"(
