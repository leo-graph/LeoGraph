  ...
  ColumnPtr executeImpl(const ColumnsWithTypeAndName &arguments, const DataTypePtr &, size_t input_rows_count) const override {
    ...
    if (const auto *col_string = checkAndGetColumn<ColumnString>(string_arg)) {
      ...
      for (size_t i = 0; i < input_rows_count; ++i) {
        ...
        impl.set(start, end);
        while (impl.get(start, end)) res_data.push_back(static_cast<UInt32>(hasher(start, end - start)));
...
REGISTER_FUNCTION(SparseGramsHashes) {
  FunctionDocumentation::Arguments arguments_sparse = {
      ...
      {"max_ngram_length",
       "Optional. The maximum length of extracted ngram. The default value is 100. Should be not less than `min_ngram_length`.",
       {"UInt*"}},
      {"min_cutoff_length",
       "Optional. If specified, only n-grams with length greater or equal than `min_cutoff_length` are returned. The default value is the "
       "same as `min_ngram_length`. Should be not less than `min_ngram_length` and not greater than `max_ngram_length`.",
       {"UInt*"}}};
...
Uses `CRC32` as a hash function.
  ...
  FunctionDocumentation::Examples examples_hashes = {{"Usage example", "SELECT sparseGramsHashes('alice', 3)",
                                                      R"(
┌─sparseGramsHashes('alice', 3)──────────────────────┐
│ [1481062250,2450405249,4012725991,1918774096]      │
└────────────────────────────────────────────────────┘
        )"}};
  FunctionDocumentation documentation_hashes = {description_hashes,    syntax_hashes,   arguments_sparse, {},
                                                returned_value_hashes, examples_hashes, introduced_in,    category};
...
Uses `CRC32` as a hash function.
  ...
  FunctionDocumentation::ReturnedValue returned_value_hashes_utf8 = {"Returns an array of selected UTF-8 substrings CRC32 hashes.",
                                                                     {"Array(UInt32)"}};
  FunctionDocumentation::Examples examples_hashes_utf8 = {{"Usage example", "SELECT sparseGramsHashesUTF8('алиса', 3)",
                                                           R"(
┌─sparseGramsHashesUTF8('алиса', 3)─┐
│ [4178533925,3855635300,561830861] │
└───────────────────────────────────┘
        )"}};
  FunctionDocumentation documentation_hashes_utf8 = {description_hashes_utf8,    syntax_hashes_utf8,   arguments_sparse, {},
                                                     returned_value_hashes_utf8, examples_hashes_utf8, introduced_in,    category};
