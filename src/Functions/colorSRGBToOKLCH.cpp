...
class FunctionColorSRGBToOKLCH : public ColorConversionFromSRGBBase<FunctionColorSRGBToOKLCH> {
 public:
  ...
  explicit FunctionColorSRGBToOKLCH(ContextPtr context_) : ColorConversionFromSRGBBase(context_) {}
...
REGISTER_FUNCTION(ColorSRGBToOKLCH) {
  ...
  FunctionDocumentation::Arguments arguments = {
      ...
      {"gamma",
       ...
       {"Float64"}}};
  FunctionDocumentation::ReturnedValue returned_value = {"Returns a tuple (L, C, H) representing the OKLCH color space values.",
                                                         {"Tuple(Float64, Float64, Float64)"}};
  FunctionDocumentation::Examples examples = {{"Convert sRGB to OKLCH",
                                               R"(
