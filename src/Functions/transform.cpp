 ...
 *
 * transform(x, [from...], [to...])
 * - if `default` is not specified, then for values of `x` for which there is no corresponding element in `from`, the unchanged value of `x`
 * is returned.
 *
 * Types:
...
class FunctionTransform : public IFunction {
 public:
  ...
  DataTypePtr getReturnTypeImpl(const DataTypes &arguments) const override {
    ...
    if (!type_arr_from)
      throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT,
                      "Second argument of function {}, must be array of source values to transform from", getName());
    ...
    if (!type_arr_to)
      throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT,
                      "Third argument of function {}, must be array of destination values to transform to", getName());
  ...
  ColumnPtr executeImpl(const ColumnsWithTypeAndName &arguments, const DataTypePtr &result_type, size_t input_rows_count) const override {
    ...
    if (isColumnConst(*in)) return executeConst(arguments, result_type, input_rows_count);

    ColumnPtr default_non_const;
    if (!cache.default_column && arguments.size() == 4) {
      if (arguments[1].column.get()->size() > arguments[3].column.get()->size() ||
          arguments[2].column.get()->size() > arguments[3].column.get()->size()) {
    ...
    if (arguments.size() == 3) in_cast = castColumn(arguments[0], result_type);

    auto column_result = result_type->createColumn();
    if (cache.is_empty) {
      return default_non_const ? default_non_const : castColumn(arguments[0], result_type);
    }
 ...
 private:
  ...
  void executeAnything(const IColumn *in, IColumn &column_result, const ColumnPtr default_non_const, const IColumn &in_cast,
  ...
  void executeContiguous(const IColumn *in, IColumn &column_result, const ColumnPtr default_non_const, const IColumn &in_cast,
                         size_t input_rows_count) const {
  ...
  template <typename T>
  bool executeNum(const IColumn *in_untyped, IColumn &column_result, const ColumnPtr default_non_const, const IColumn &in_cast,
                  size_t input_rows_count) const {
    const auto *const in = checkAndGetColumn<T>(in_untyped);
    if (!in) return false;
    const auto &pod = in->getData();
    UInt32 in_scale = 0;
    if constexpr (std::is_same_v<ColumnDecimal<Decimal32>, T> || std::is_same_v<ColumnDecimal<Decimal64>, T>) in_scale = in->getScale();
  ...
  template <typename T>
  bool executeNumToString(const PaddedPODArray<T> &pod, IColumn &column_result, const ColumnPtr default_non_const,
                          size_t input_rows_count) const {
  ...
  void executeNumToStringHelper(const Table &table, const PaddedPODArray<In> &pod, ColumnString::Chars &out_data,
                                ColumnString::Offsets &out_offsets, const ColumnString::Chars &to_data,
                                const ColumnString::Offsets &to_offsets, const DefData &def_data, const DefOffs &def_offsets,
                                size_t input_rows_count) const {
  ...
  bool executeNumToNum(const PaddedPODArray<U> &pod, IColumn &column_result, ColumnPtr default_non_const, UInt32 in_scale,
                       size_t input_rows_count) const {
    ...
    UInt32 out_scale = 0;
    if constexpr (std::is_same_v<ColumnDecimal<Decimal32>, T> || std::is_same_v<ColumnDecimal<Decimal64>, T>) out_scale = out->getScale();
  ...
  void executeNumToNumHelper(const Table &table, const PaddedPODArray<In> &pod, PaddedPODArray<Out> &out_pod,
                             const PaddedPODArray<Out> &to_pod, const Def &def, size_t input_rows_count, UInt32 out_scale,
  ...
  bool executeString(const IColumn *in_untyped, IColumn &column_result, const ColumnPtr default_non_const, const IColumn &in_cast,
                     size_t input_rows_count) const {
  ...
  bool executeStringToString(const ColumnString::Chars &data, const ColumnString::Offsets &offsets, IColumn &column_result,
                             const ColumnPtr default_non_const, size_t input_rows_count) const {
    auto *out = typeid_cast<ColumnString *>(&column_result);
    if (!out) return false;
    auto &out_offs = out->getOffsets();
    out_offs.resize(input_rows_count);
  ...
  void executeStringToStringHelper(const Table &table, const ColumnString::Chars &data, const ColumnString::Offsets &offsets,
                                   ColumnString::Chars &out_data, ColumnString::Offsets &out_offsets, const ColumnString::Chars &to_data,
                                   const ColumnString::Offsets &to_offsets, const DefData &def_data, const DefOffs &def_offsets,
                                   size_t input_rows_count) const {
    ColumnString::Offset current_offset = 0;
    size_t out_cur_off = 0;
  ...
  template <typename T>
  bool executeStringToNum(const ColumnString::Chars &data, const ColumnString::Offsets &offsets, IColumn &column_result,
                          const ColumnPtr default_non_const, size_t input_rows_count) const {
    auto *out = typeid_cast<T *>(&column_result);
    if (!out) return false;
    auto &out_pod = out->getData();
  ...
  void executeStringToNumHelper(const Table &table, const ColumnString::Chars &data, const ColumnString::Offsets &offsets,
                                PaddedPODArray<Out> &out_pod, const PaddedPODArray<Out> &to_pod, const Def &def,
                                size_t input_rows_count) const {
    ColumnString::Offset current_offset = 0;
    for (size_t i = 0; i < input_rows_count; ++i) {
      const std::string_view ref{reinterpret_cast<const char *>(&data[current_offset]), offsets[i] - current_offset};
  ...
  static void checkAllowedTypeHelper(const DataTypePtr &type) {
    if (isStringOrFixedString(type)) return;

    if (type->haveMaximumSizeOfValue()) {
      auto data_type_size = type->getMaximumSizeOfValueInMemory();
      if (data_type_size <= sizeof(UInt64)) return;
    }
  ...
  void initialize(const ColumnsWithTypeAndName &arguments, const DataTypePtr &result_type) const {
    ...
    cache.from_column = castColumn(
        {from_column_uncast, typeid_cast<const DataTypeArray &>(*arguments[1].type).getNestedType(), arguments[1].name}, from_type);

    cache.to_column = castColumn(
        {array_to->getDataPtr(), typeid_cast<const DataTypeArray &>(*arguments[2].type).getNestedType(), arguments[2].name}, result_type);
...
#pragma clang diagnostic ignored "-Wunreachable-code"
          if constexpr (std::endian::native == std::endian::big) dst += sizeof(key) - ref.size();
...
- For 3-parameter version: `transform(T, Array(T), Array(T)) -> T` where all types must be the same
  ...
  FunctionDocumentation::Arguments arguments = {
      ...
      {"array_from",
       "Constant array of values to search for matches.",
       {"Array((U)Int*)", "Array(Decimal)", "Array(Float*)", "Array(String)", "Array(Date)", "Array(DateTime)"}},
      {"array_to",
       "Constant array of values to return for corresponding matches in `array_from`.",
       {"Array((U)Int*)", "Array(Decimal)", "Array(Float*)", "Array(String)", "Array(Date)", "Array(DateTime)"}},
      {"default",
       "Optional. Value to return if `x` is not found in `array_from`. If omitted, returns x unchanged.",
       {"(U)Int*", "Decimal", "Float*", "String", "Date", "DateTime"}}};
  FunctionDocumentation::ReturnedValue returned_value = {
      "Returns the corresponding value from `array_to` if x matches an element in `array_from`, otherwise returns default (if provided) or "
      "x (if default not provided).",
      {"Any"}};
  FunctionDocumentation::Examples examples = {{"transform(T, Array(T), Array(U), U) -> U",
                                               R"(
SELECT
