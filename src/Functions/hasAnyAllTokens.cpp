...
TokensWithPosition initializeSearchTokens(const ColumnsWithTypeAndName &arguments, const ITokenizer &tokenizer,
                                          std::string_view function_name) {
  if (arguments.size() < 2) return {};

  auto column_needles = arguments[arg_needles].column;
  if (!column_needles || column_needles->empty()) return {};

  Field needles_field = (*column_needles)[0];
  if (needles_field.isNull()) return {};
  ...
  if (needles_field.getType() == Field::Types::String) {
    ...
    for (const auto &element : array_data) {
      if (element.getType() != Field::Types::String)
        throw Exception(ErrorCodes::BAD_ARGUMENTS, "Needles argument for function '{}' has unsupported type of column '{}'", function_name,
                        column_needles->getName());
  ...
  for (const auto &token : tokens_array) {
    if (search_tokens.emplace(token, pos).second) ++pos;
  }
...
bool isStringOrFixedStringOrArrayOfStringOrFixedString(const IDataType &type) {
  ...
  if (const auto *nullable = typeid_cast<const DataTypeNullable *>(nested_type)) nested_type = nullable->getNestedType().get();

  if (isStringOrFixedString(*nested_type)) return true;
...
DataTypePtr FunctionHasAnyAllTokensOverloadResolver<HasTokensTraits>::getReturnTypeImpl(const ColumnsWithTypeAndName &arguments) const {
  FunctionArgumentDescriptors mandatory_args{
      {"input", static_cast<FunctionArgumentDescriptor::TypeValidator>(&isStringOrFixedStringOrArrayOfStringOrFixedString), nullptr,
       "String, FixedString, Array(String) or Array(FixedString)"},
      {"needles", static_cast<FunctionArgumentDescriptor::TypeValidator>(&isStringOrArrayOfStringType), isColumnConst,
  ...
  if (arguments[arg_input].type->isNullable()) return_type = std::make_shared<DataTypeNullable>(std::make_shared<DataTypeNumber<UInt8>>());

  return return_type;
}

template <class HasTokensTraits>
FunctionBasePtr FunctionHasAnyAllTokensOverloadResolver<HasTokensTraits>::buildImpl(const ColumnsWithTypeAndName &arguments,
  ...
  return std::make_shared<FunctionBaseHasAnyAllTokens<HasTokensTraits>>(std::move(tokenizer), std::move(search_tokens),
...
struct HasAnyTokensMatcher {
  explicit HasAnyTokensMatcher(const TokensWithPosition &tokens_) : tokens(tokens_) {}
...
struct HasAllTokensMatcher {
  explicit HasAllTokensMatcher(const TokensWithPosition &tokens_) : tokens(tokens_), mask(tokens.size()), num_set_bits(0) {}
...
void searchOnArray(const ColumnArray::Offsets &offsets, const StringColumnType auto &input_string, const ColumnNullable *null_map,
                   PaddedPODArray<UInt8> &col_result, size_t input_rows_count, const ITokenizer *tokenizer, MatcherType auto matcher) {
  ...
  for (size_t i = 0; i < input_rows_count; ++i) {
    ...
    for (size_t j = 0; j < array_size; ++j) {
      ...
      if (null_map && null_map->isNullAt(element_idx)) continue;
...
void searchOnString(const StringColumnType auto &col_input, PaddedPODArray<UInt8> &col_result, size_t input_rows_count,
                    const ITokenizer *tokenizer, MatcherType auto matcher) {
  for (size_t i = 0; i < input_rows_count; ++i) {
...
template <class HasTokensTraits>
void executeString(const StringColumnType auto &col_input, PaddedPODArray<UInt8> &col_result, size_t input_rows_count,
                   const ITokenizer *tokenizer, const TokensWithPosition &tokens) {
  if (tokens.empty()) {
...
template <class HasTokensTraits>
void executeArray(const ColumnArray *array, const StringColumnType auto &input_string, const ColumnNullable *null_map,
                  PaddedPODArray<UInt8> &col_result, const ITokenizer *tokenizer, const TokensWithPosition &tokens) {
...
template <class HasTokensTraits>
void executeStringOrArray(ColumnPtr col_input, PaddedPODArray<UInt8> &col_result, size_t input_rows_count, const ITokenizer *tokenizer,
                          const TokensWithPosition &tokens) {
...
template <class HasTokensTraits>
ColumnPtr ExecutableFunctionHasAnyAllTokens<HasTokensTraits>::executeImpl(const ColumnsWithTypeAndName &arguments, const DataTypePtr &,
                                                                          size_t input_rows_count) const {
  if (input_rows_count == 0) return ColumnVector<UInt8>::create();
...
hasAnyTokens(input, needles)
  ...
  FunctionDocumentation::Arguments arguments_hasAnyTokens = {
      {"input",
       "The input column.",
       {"String", "FixedString", "Nullable(String)", "Nullable(FixedString)", "Array(String)", "Array(FixedString)",
        "Array(Nullable(String))", "Array(Nullable(FixedString))"}},
      {"needles", "Tokens to be searched.", {"String", "Array(String)"}},
      {"tokenizer",
       "The tokenizer to use. Valid arguments are `splitByNonAlpha`, `ngrams`, `splitByString`, `array`, and `sparseGrams`. Optional, if "
       "not set explicitly, defaults to `splitByNonAlpha`.",
       {"const String"}},
  };
  FunctionDocumentation::ReturnedValue returned_value_hasAnyTokens = {"Returns `1`, if there was at least one match. `0`, otherwise.",
                                                                      {"UInt8"}};
  FunctionDocumentation::Examples examples_hasAnyTokens = {{"Basic usage with a string needle",
                                                            R"(
CREATE TABLE table (
    id UInt32,
    msg String,
...
└─────────┘
  ...
  FunctionDocumentation documentation_hasAnyTokens = {
      description_hasAnyTokens,    syntax_hasAnyTokens,   arguments_hasAnyTokens,     {},
      returned_value_hasAnyTokens, examples_hasAnyTokens, introduced_in_hasAnyTokens, category_hasAnyTokens};
...
hasAllTokens(input, needles)
  ...
  FunctionDocumentation::Examples examples_hasAllTokens = {{"Basic usage with a string needle",
                                                            R"(
CREATE TABLE table (
    id UInt32,
    msg String,
...
└─────────┘
  ...
  FunctionDocumentation documentation_hasAllTokens = {
      description_hasAllTokens,    syntax_hasAllTokens,   arguments_hasAllTokens,     {},
      returned_value_hasAllTokens, examples_hasAllTokens, introduced_in_hasAllTokens, category_hasAllTokens};
