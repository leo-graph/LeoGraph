...
struct RepeatImpl {
  ...
  static void checkStringSize(UInt64 size) {
    ...
    if (size > max_string_size)
      throw Exception(ErrorCodes::TOO_LARGE_STRING_SIZE, "Too large string size ({}) in function repeat, maximum is: {}", size,
                      max_string_size);
  }

  template <typename T>
  static void vectorStrConstRepeat(const ColumnString::Chars &data, const ColumnString::Offsets &offsets, ColumnString::Chars &res_data,
                                   ColumnString::Offsets &res_offsets, T repeat_time) {
    ...
    for (UInt64 i = 0; i < res_offsets.size(); ++i) {
      process(data.data() + offsets[i - 1], res_data.data() + res_offsets[i - 1], offsets[i] - offsets[i - 1],
              static_cast<UInt64>(repeat_time));
    }
  }

  template <typename T>
  static void vectorStrVectorRepeat(const ColumnString::Chars &data, const ColumnString::Offsets &offsets, ColumnString::Chars &res_data,
                                    ColumnString::Offsets &res_offsets, const PaddedPODArray<T> &col_num) {
    UInt64 data_size = 0;
    res_offsets.assign(offsets);
  ...
  template <typename T>
  static void constStrVectorRepeat(std::string_view copy_str, ColumnString::Chars &res_data, ColumnString::Offsets &res_offsets,
    ...
    for (UInt64 i = 0; i < col_size; ++i) {
      T repeat_time = col_num[i] < 0 ? static_cast<T>(0) : col_num[i];
      checkRepeatTime(static_cast<UInt64>(repeat_time));
      process(reinterpret_cast<UInt8 *>(const_cast<char *>(copy_str.data())), res_data.data() + res_offsets[i - 1], str_size,
              static_cast<UInt64>(repeat_time));
    }
  }
 ...
 private:
  ...
  static void process(const UInt8 *src, UInt8 *dst, UInt64 size, UInt64 repeat_time) {
    if (unlikely(repeat_time <= 0)) return;
...
class FunctionRepeat : public IFunction {
  ...
  static bool castType(const IDataType *type, F &&f) {
    return castTypeToEither<DataTypeInt8, DataTypeInt16, DataTypeInt32, DataTypeInt64, DataTypeInt128, DataTypeInt256, DataTypeUInt8,
                            DataTypeUInt16, DataTypeUInt32, DataTypeUInt64, DataTypeUInt128, DataTypeUInt256>(type, std::forward<F>(f));
  }
  ...
  ColumnPtr executeImpl(const ColumnsWithTypeAndName &arguments, const DataTypePtr & /*result_type*/,
    ...
    if (const ColumnString *col = checkAndGetColumn<ColumnString>(col_str.get())) {
      if (const ColumnConst *col_num_const = checkAndGetColumn<ColumnConst>(col_num.get())) {
        ...
        if (!success)
          throw Exception(ErrorCodes::ILLEGAL_COLUMN, "Illegal column type {} of 'n' of function {}", arguments[1].column->getName(),
                          getName());
      ...
      if (castType(arguments[1].type.get(), [&](const auto &type) {
            ...
            auto col_res = ColumnString::create();
            RepeatImpl::vectorStrVectorRepeat(col->getChars(), col->getOffsets(), col_res->getChars(), col_res->getOffsets(),
                                              column.getData());
...
Concatenates a string as many times with itself as specified.
  ...
  FunctionDocumentation::Arguments arguments = {{"s", "The string to repeat.", {"String"}},
  ...
  FunctionDocumentation::Examples examples = {{"Usage example", "SELECT repeat('abc', 10)",
                                               R"(
┌─repeat('abc', 10)──────────────┐
