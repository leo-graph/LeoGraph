...
template <class T>
using DataTypeFromFieldType = std::conditional_t<std::is_same_v<T, NumberTraits::Error>, InvalidType, DataTypeNumber<T>>;
...
template <typename DataType>
constexpr bool IsExtended = false;
template <>
inline constexpr bool IsExtended<DataTypeUInt128> = true;
template <>
inline constexpr bool IsExtended<DataTypeUInt256> = true;
template <>
inline constexpr bool IsExtended<DataTypeInt128> = true;
template <>
inline constexpr bool IsExtended<DataTypeInt256> = true;

template <typename DataType>
constexpr bool IsIntegralOrExtended = IsIntegral<DataType> || IsExtended<DataType>;
template <typename DataType>
constexpr bool IsIntegralOrExtendedOrDecimal = IsIntegralOrExtended<DataType> || IsDataTypeDecimal<DataType>;

template <typename DataType>
constexpr bool IsFloatingPoint = false;
template <>
inline constexpr bool IsFloatingPoint<DataTypeBFloat16> = true;
template <>
...
template <>
inline constexpr bool IsArray<DataTypeArray> = true;
...
template <typename DataType>
constexpr bool IsDateOrTimeOrDateTime = false;
template <>
inline constexpr bool IsDateOrTimeOrDateTime<DataTypeDate> = true;
template <>
inline constexpr bool IsDateOrTimeOrDateTime<DataTypeTime> = true;
template <>
inline constexpr bool IsDateOrTimeOrDateTime<DataTypeDateTime> = true;

template <typename DataType>
constexpr bool IsIPv4 = false;
template <>
inline constexpr bool IsIPv4<DataTypeIPv4> = true;

template <typename T0, typename T1>
constexpr bool UseLeftDecimal = false;
template <>
inline constexpr bool UseLeftDecimal<DataTypeDecimal<Decimal256>, DataTypeDecimal<Decimal128>> = true;
template <>
inline constexpr bool UseLeftDecimal<DataTypeDecimal<Decimal256>, DataTypeDecimal<Decimal64>> = true;
template <>
inline constexpr bool UseLeftDecimal<DataTypeDecimal<Decimal256>, DataTypeDecimal<Decimal32>> = true;
template <>
inline constexpr bool UseLeftDecimal<DataTypeDecimal<Decimal128>, DataTypeDecimal<Decimal32>> = true;
template <>
inline constexpr bool UseLeftDecimal<DataTypeDecimal<Decimal128>, DataTypeDecimal<Decimal64>> = true;
template <>
...
struct BinaryOperationTraits {
 ...
 public:
  ...
  using DecimalResultDataType = Switch<
      ...
      Case<IsDataTypeDecimal<LeftDataType> && IsDataTypeDecimal<RightDataType> && UseLeftDecimal<LeftDataType, RightDataType>,
           LeftDataType>,
  ...
  using ResultDataType = Switch<
      ...
      /// Date + Integral -> Date
      /// Integral + Date -> Date
      Case<IsOperation<Operation>::plus,
           Switch<Case<IsIntegral<RightDataType>, LeftDataType>, Case<IsIntegral<LeftDataType>, RightDataType>>>,

      /// Date - Date     -> Int32
      /// Date - Integral -> Date
      Case<IsOperation<Operation>::minus, Switch<Case<std::is_same_v<LeftDataType, RightDataType>, DataTypeInt32>,
      ...
      /// Date % Int32 -> Int32
      /// Date % Float -> Float64
      Case<IsOperation<Operation>::modulo || IsOperation<Operation>::positive_modulo,
           Switch<Case<IsDateOrTimeOrDateTime<LeftDataType> && IsIntegral<RightDataType>, RightDataType>,
...
enum class OpCase : uint8_t { Vector, LeftConstant, RightConstant };
...
struct BinaryOperation {
  ...
  static void NO_INLINE process(const A *__restrict a, const B *__restrict b, ResultType *__restrict c, size_t size,
    ...
    if constexpr (op_case == OpCase::RightConstant) {
      if (right_nullmap && (*right_nullmap)[0]) return;
      ...
      if (right_nullmap) {
        ...
        for (size_t i = 0; i < size; ++i) apply<op_case>(a, b, c, i);
    }
  }
...
struct StringIntegerOperationImpl {
  template <OpCase op_case>
  static void NO_INLINE processFixedString(const UInt8 *__restrict in_vec, const UInt64 n, const B *__restrict b,
                                           ColumnFixedString::Chars &out_vec, size_t size) {
  ...
  template <OpCase op_case>
  static void NO_INLINE processString(const UInt8 *__restrict in_vec, const UInt64 *__restrict in_offsets, const B *__restrict b,
                                      ColumnString::Chars &out_vec, ColumnString::Offsets &out_offsets, size_t size) {
    size_t prev_offset = 0;
...
struct FixedStringOperationImpl {
  template <OpCase op_case>
  static void NO_INLINE process(const UInt8 *__restrict a, const UInt8 *__restrict b, UInt8 *__restrict result, size_t size,
                                [[maybe_unused]] size_t N) {
    if constexpr (op_case == OpCase::Vector)
      for (size_t i = 0; i < size; ++i) result[i] = Op::template apply<UInt8>(a[i], b[i]);
 ...
 private:
  ...
  static void NO_INLINE withConst(const UInt8 *__restrict a, const UInt8 *__restrict b, UInt8 *__restrict c, size_t size, size_t N) {
    ...
    for (size_t i = 0; i < b_repeated_size; ++i) b_repeated[i] = b[i % N];
    ...
    for (size_t i = 0; i < size; i += 16) {
      ...
      for (size_t j = 0; j < 16; ++j)
        c[i + j] = inverted ? Op::template apply<UInt8>(a[i + j], b_repeated[b_offset + j])
...
struct StringReduceOperationImpl {
  static void vectorVector(const ColumnString::Chars &a, const ColumnString::Offsets &offsets_a, const ColumnString::Chars &b,
                           const ColumnString::Offsets &offsets_b, PaddedPODArray<UInt64> &res) {
    size_t size = res.size();
    for (size_t i = 0; i < size; ++i) {
      res[i] = process(a.data() + offsets_a[i - 1], a.data() + offsets_a[i], b.data() + offsets_b[i - 1], b.data() + offsets_b[i]);
    }
  }

  static void vectorConstant(const ColumnString::Chars &a, const ColumnString::Offsets &offsets_a, std::string_view b,
                             PaddedPODArray<UInt64> &res) {
    size_t size = res.size();
    for (size_t i = 0; i < size; ++i) {
      res[i] = process(a.data() + offsets_a[i - 1], a.data() + offsets_a[i], reinterpret_cast<const UInt8 *>(b.data()),
                       reinterpret_cast<const UInt8 *>(b.data()) + b.size());
    }
  }

  static UInt64 constConst(std::string_view a, std::string_view b) {
    return process(reinterpret_cast<const UInt8 *>(a.data()), reinterpret_cast<const UInt8 *>(a.data()) + a.size(),
                   reinterpret_cast<const UInt8 *>(b.data()), reinterpret_cast<const UInt8 *>(b.data()) + b.size());
  }

 private:
  static UInt64 process(const UInt8 *__restrict start_a, const UInt8 *__restrict end_a, const UInt8 *start_b, const UInt8 *end_b) {
    UInt64 res = 0;
    while (start_a < end_a && start_b < end_b) res += Op::template apply<UInt8>(*start_a++, *start_b++);

    while (start_a < end_a) res += Op::template apply<UInt8>(*start_a++, 0);
    while (start_b < end_b) res += Op::template apply<UInt8>(0, *start_b++);
    return res;
  }
};
...
struct DecimalBinaryOperation {
 ...
 public:
  ...
  static void NO_INLINE process(const auto &a, const auto &b, ResultContainerType &c, NativeResultType scale_a, NativeResultType scale_b,
                                const NullMap *right_nullmap = nullptr) {
    ...
    if constexpr (is_plus_minus_compare) {
      if (scale_a != 1) {
        for (size_t i = 0; i < size; ++i)
          c[i] = applyScaled<true>(static_cast<NativeResultType>(unwrap<op_case, OpCase::LeftConstant>(a, i)),
                                   static_cast<NativeResultType>(unwrap<op_case, OpCase::RightConstant>(b, i)), scale_a);
        return;
      }
      if (scale_b != 1) {
        for (size_t i = 0; i < size; ++i)
          c[i] = applyScaled<false>(static_cast<NativeResultType>(unwrap<op_case, OpCase::LeftConstant>(a, i)),
                                    static_cast<NativeResultType>(unwrap<op_case, OpCase::RightConstant>(b, i)), scale_b);
        return;
      }
    } else if constexpr (is_multiply) {
      if (scale_a != 1) {
        for (size_t i = 0; i < size; ++i)
          c[i] = applyScaled<true, false>(static_cast<NativeResultType>(unwrap<op_case, OpCase::LeftConstant>(a, i)),
                                          static_cast<NativeResultType>(unwrap<op_case, OpCase::RightConstant>(b, i)), scale_a);
        return;
      }
      if (scale_b != 1) {
        for (size_t i = 0; i < size; ++i)
          c[i] = applyScaled<false, false>(static_cast<NativeResultType>(unwrap<op_case, OpCase::LeftConstant>(a, i)),
                                           static_cast<NativeResultType>(unwrap<op_case, OpCase::RightConstant>(b, i)), scale_b);
        return;
      }
      ...
      processWithRightNullmapImpl<op_case>(a, b, c, size, right_nullmap, [&scale_a](const auto &left, const auto &right) {
        return applyScaledDiv<is_decimal_a>(static_cast<NativeResultType>(left), right, scale_a);
      });
      return;
    }

    processWithRightNullmapImpl<op_case>(a, b, c, size, right_nullmap, [](const auto &left, const auto &right) {
      return apply(static_cast<NativeResultType>(left), static_cast<NativeResultType>(right));
    });
  }
    ...
    requires(!is_decimal<A> && !is_decimal<B>)
  {
    if constexpr (is_division && is_decimal_b)
      ...
      if (scale_a != 1) return applyScaled<true>(a, b, scale_a);
      if (scale_b != 1) return applyScaled<false>(a, b, scale_b);
    }
 ...
 private:
  ...
  static void processWithRightNullmapImpl(const auto &a, const auto &b, ResultContainerType &c, size_t size, const NullMap *right_nullmap,
                                          ApplyFunc apply_func) {
    if (right_nullmap) {
      if constexpr (op_case == OpCase::RightConstant) {
        if ((*right_nullmap)[0]) {
          for (size_t i = 0; i < size; ++i) c[i] = ResultType();
          return;
        }

        for (size_t i = 0; i < size; ++i) c[i] = apply_func(undec(a[i]), undec(b));
      } else {
        for (size_t i = 0; i < size; ++i) {
          if ((*right_nullmap)[i])
            c[i] = ResultType();
  ...
  static constexpr bool is_int_division = IsOperation<Operation>::int_div || IsOperation<Operation>::int_div_or_zero;
  ...
  using Op = std::conditional_t<is_float_division, DivideIntegralImpl<NativeResultType, NativeResultType>,  /// substitute divide by intDiv
  ...
  static NativeResultType apply(NativeResultType a, NativeResultType b) {
    if constexpr (can_overflow && check_overflow) {
      ...
      if (Op::template apply<NativeResultType>(a, b, res)) throw Exception(ErrorCodes::DECIMAL_OVERFLOW, "Decimal math overflow");
      return res;
    } else
  ...
  static NO_SANITIZE_UNDEFINED NativeResultType applyScaled(NativeResultType a, NativeResultType b, NativeResultType scale) {
    ...
    if constexpr (check_overflow && may_check_overflow) {
      ...
      if (overflow) throw Exception(ErrorCodes::DECIMAL_OVERFLOW, "Decimal math overflow");
    } else {
      if constexpr (scale_left)
        a *= scale;
      else
  ...
  static NO_SANITIZE_UNDEFINED NativeResultType applyScaledDiv(NativeResultType a, NativeResultType b, NativeResultType scale) {
    if constexpr (is_division) {
      if constexpr (check_overflow) {
        bool overflow = false;
        if constexpr (!is_decimal_a) overflow |= common::mulOverflow(scale, scale, scale);
        overflow |= common::mulOverflow(a, scale, a);
        if (overflow) throw Exception(ErrorCodes::DECIMAL_OVERFLOW, "Decimal math overflow");
      } else {
        if constexpr (!is_decimal_a) scale *= scale;
        a *= scale;
      }
...
template <template <typename, typename> class Op, typename Name, bool valid_on_default_arguments = true,
          bool valid_on_float_arguments = true, bool division_by_nullable = false>
class FunctionBinaryArithmetic : public IFunction {
  ...
  static bool castType(const IDataType *type, auto &&f) {
    using IntegerTypes = TypeList<DataTypeUInt8, DataTypeUInt16, DataTypeUInt32, DataTypeUInt64, DataTypeUInt128, DataTypeUInt256,
    ...
    static constexpr bool needs_date_time =
        is_plus || is_minus || IsOperation<Op>::least || IsOperation<Op>::greatest || is_modulo || IsOperation<Op>::positive_modulo;
    ...
    using NumericTypes = std::conditional_t<needs_decimal, TypeListConcat<IntegerTypes, DecimalTypes>, IntegerTypes>;
    using NumericAndDateTypes =
        std::conditional_t<needs_date_time, TypeListConcat<NumericTypes, TypeList<DataTypeDate, DataTypeDateTime, DataTypeTime>>,
                           NumericTypes>;
    using WithStrings =
        std::conditional_t<needs_string_types, TypeListConcat<NumericAndDateTypes, TypeList<DataTypeFixedString, DataTypeString>>,
                           NumericAndDateTypes>;
    using WithInterval = std::conditional_t<needs_interval, TypeListConcat<WithStrings, TypeList<DataTypeInterval>>, WithStrings>;
    using ValidTypes = std::conditional_t<valid_on_float_arguments, TypeListConcat<WithInterval, Floats>, WithInterval>;
  ...
  static FunctionOverloadResolverPtr getFunctionForIntervalArithmetic(const DataTypePtr &type0, const DataTypePtr &type1,
                                                                      ContextPtr context) {
    ...
    if (first_arg_is_date_or_time_or_datetime_or_string == second_arg_is_date_or_time_or_datetime_or_string) return {};
    ...
    if constexpr (!is_plus && !is_minus) return {};
    ...
    if (second_arg_is_date_or_time_or_datetime_or_string && is_minus)
      throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT,
                      "Wrong order of arguments for function {}: "
                      "argument of type Interval cannot be first",
                      name);

    std::string function_name;
    if (interval_data_type) {
      function_name = fmt::format("{}{}s", is_plus ? "add" : "subtract", interval_data_type->getKind().toString());
    } else {
  ...
  static FunctionOverloadResolverPtr getFunctionForDateTupleOfIntervalsArithmetic(const DataTypePtr &type0, const DataTypePtr &type1,
                                                                                  ContextPtr context) {
    ...
    if (first_arg_is_date_or_datetime == second_arg_is_date_or_datetime) return {};

    if (!isTuple(type0) && !isTuple(type1)) return {};
    ...
    if (isTuple(type0) && second_arg_is_date_or_datetime && is_minus)
      throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT,
                      "Wrong order of arguments for function {}: "
                      "argument of Tuple type cannot be first",
                      name);
  ...
  static FunctionOverloadResolverPtr getFunctionForMergeIntervalsArithmetic(const DataTypePtr &type0, const DataTypePtr &type1,
                                                                            ContextPtr context) {
    ...
    if ((!tuple_data_type_0 && !interval_data_type_0) || !interval_data_type_1) return {};

    if (interval_data_type_0 && interval_data_type_0->equals(*interval_data_type_1)) return {};

    if (tuple_data_type_0) {
      const auto &tuple_types = tuple_data_type_0->getElements();
      for (const auto &type : tuple_types)
        if (!isInterval(type)) return {};
    }
  ...
  static FunctionOverloadResolverPtr getFunctionForTupleArithmetic(const DataTypePtr &type0, const DataTypePtr &type1, ContextPtr context) {
    if (!isTuple(removeNullable(type0)) || !isTuple(removeNullable(type1))) return {};
    ...
    if constexpr (!is_plus && !is_minus && !is_multiply) return {};
  ...
  static FunctionOverloadResolverPtr getFunctionForTupleAndNumberArithmetic(const DataTypePtr &type0, const DataTypePtr &type1,
                                                                            ContextPtr context) {
    ...
    if constexpr (!is_multiply && !is_division) return {};

    if (isNumber(type0) && is_division)
      throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT,
                      "Wrong order of arguments for function {}: "
                      "argument of numeric type cannot be first",
                      name);
  ...
  static bool isAggregateMultiply(const DataTypePtr &type0, const DataTypePtr &type1) {
    if constexpr (!is_multiply) return false;

    WhichDataType which0(type0);
    WhichDataType which1(type1);

    return (which0.isAggregateFunction() && which1.isNativeUInt()) || (which0.isNativeUInt() && which1.isAggregateFunction());
  }

  static bool isAggregateAddition(const DataTypePtr &type0, const DataTypePtr &type1) {
    if constexpr (!is_plus) return false;
  ...
  static bool isDateTime64Subtraction(const DataTypePtr &type0, const DataTypePtr &type1) {
    if constexpr (!is_minus) return false;

    WhichDataType which0(type0);
    WhichDataType which1(type1);

    if (which0.isDateTime64() && which1.isDateTimeOrDateTime64()) return true;
  ...
  static bool isTime64Subtraction(const DataTypePtr &type0, const DataTypePtr &type1) {
    ...
    if (which0.isTime64() && which1.isTimeOrTime64()) return true;
  ...
  ColumnPtr executeAggregateMultiply(const ColumnsWithTypeAndName &arguments, const DataTypePtr &, size_t input_rows_count) const {
    ...
    if (WhichDataType(new_arguments[1].type).isAggregateFunction()) std::swap(new_arguments[0], new_arguments[1]);

    if (!isColumnConst(*new_arguments[1].column))
      throw Exception(ErrorCodes::ILLEGAL_COLUMN,
                      "Illegal column {} of argument of aggregation state multiply. "
                      "Should be integer constant",
                      new_arguments[1].column->getName());
    ...
    while (m) {
      if (m % 2) {
        for (size_t i = 0; i < size; ++i) function->merge(vec_to[i], vec_from[i], arena.get());
        --m;
      } else {
        for (size_t i = 0; i < size; ++i) function->merge(vec_from[i], vec_from[i], arena.get());
        m /= 2;
      }
    }

    if (agg_state_is_const) return ColumnConst::create(std::move(column_to), input_rows_count);
    return column_to;
  }
  ...
  ColumnPtr executeAggregateAddition(const ColumnsWithTypeAndName &arguments, const DataTypePtr &, size_t input_rows_count) const {
    ...
    if (lhs_is_const && rhs_is_const) return ColumnConst::create(std::move(column_to), input_rows_count);
    return column_to;
  }
  ...
  ColumnPtr executeDateTime64Subtraction(const ColumnsWithTypeAndName &arguments, const DataTypePtr &result_type,
    ...
    auto process = [type](struct ColumnInfo &to_be_checked, const DataTypePtr &other_type) {
      ...
      if (to_be_checked.is_const) {
        ...
        if (!to_be_checked.col) return false;
      }
      return true;
    };

    /// Process both columns
    if (!process(cols[0], cols[1].argument.type)) return nullptr;
    if (!process(cols[1], cols[0].argument.type)) return nullptr;
  ...
  ColumnPtr executeDateTimeTupleOfIntervalsPlusMinus(const ColumnsWithTypeAndName &arguments, const DataTypePtr &result_type,
    ...
    if (isTuple(arguments[0].type)) std::swap(new_arguments[0], new_arguments[1]);
  ...
  ColumnPtr executeArraysImpl(const ColumnsWithTypeAndName &arguments, const DataTypePtr &result_type, size_t input_rows_count) const {
    ...
    if (!return_type_array) throw Exception(ErrorCodes::LOGICAL_ERROR, "Return type for function {} must be array", getName());
    ...
    if (!left_offsets.empty()) rows_count = left_offsets.back();
  ...
  ColumnPtr executeArrayWithNumericImpl(const ColumnsWithTypeAndName &args, const DataTypePtr &result_type, size_t input_rows_count) const {
    ...
    if (is_swapped) std::swap(arguments[1], arguments[0]);
    ...
    if (is_swapped) std::swap(new_arguments[1], new_arguments[0]);
    auto res = executeImpl(new_arguments, result_array_type, rows_count);

    return ColumnArray::create(res, left_array_col->getOffsetsPtr());
  }

  ColumnPtr executeTupleNumberOperator(const ColumnsWithTypeAndName &arguments, const DataTypePtr &result_type, size_t input_rows_count,
                                       const FunctionOverloadResolverPtr &function_builder) const {
    ColumnsWithTypeAndName new_arguments = arguments;

    /// Number argument must be second.
    if (isNumber(arguments[0].type)) std::swap(new_arguments[0], new_arguments[1]);
  ...
  void helperInvokeEither(const auto &left, const auto &right, auto &vec_res, auto scale_a, auto scale_b,
                          const NullMap *right_nullmap) const {
  ...
  ColumnPtr executeNumericWithDecimal(const auto &left, const auto &right, const ColumnConst *const col_left_const,
                                      const ColumnConst *const col_right_const, const auto *const col_left, const auto *const col_right,
    ...
    if (col_left_const && col_right_const) {
      ...
      if (!right_nullmap || !(*right_nullmap)[0]) {
        res = helperInvokeEither<left_is_decimal, right_is_decimal, OpImpl, OpImplCheck, ResultType>(const_a, const_b, scale_a, scale_b);
      }

      return ResultDataType(type.getPrecision(), type.getScale()).createColumnConst(col_left_const->size(), toField(res, type.getScale()));
    }
    ...
    if (col_left && col_right) {
      helperInvokeEither<OpCase::Vector, left_is_decimal, right_is_decimal, OpImpl, OpImplCheck>(col_left->getData(), col_right->getData(),
                                                                                                 vec_res, scale_a, scale_b, right_nullmap);
 ...
 public:
  ...
  explicit FunctionBinaryArithmetic(ContextPtr context_)
      : context(context_), check_decimal_overflow(decimalCheckArithmeticOverflow(context)) {}
  ...
  DataTypePtr getReturnTypeImpl(const DataTypes &arguments) const override { return getReturnTypeImplStatic(arguments, context); }
  ...
  static DataTypePtr getReturnTypeImplStatic(const DataTypes &arguments, ContextPtr context) {
    ...
    if (isAggregateMultiply(arguments[0], arguments[1])) {
      if (WhichDataType(arguments[0]).isAggregateFunction()) return arguments[0];
      return arguments[1];
    }
    ...
    if (isAggregateAddition(arguments[0], arguments[1])) {
      if (!arguments[0]->equals(*arguments[1]))
        throw Exception(ErrorCodes::CANNOT_ADD_DIFFERENT_AGGREGATE_STATES, "Cannot add aggregate states of different functions: {} and {}",
    ...
    if ((isIPv4(arguments[0]) && (isIPv4(arguments[1]) || isInteger(arguments[1]))) || (isIPv4(arguments[1]) && isInteger(arguments[0]))) {
    ...
    if ((isIPv6(arguments[0]) && (isIPv6(arguments[1]) || isInteger(arguments[1]))) || (isIPv6(arguments[1]) && isInteger(arguments[0]))) {
      DataTypes new_arguments{
  ...
  static DataTypePtr getReturnTypeImplStatic2(const DataTypes &arguments, ContextPtr context) {
    ...
    if (auto function_builder = getFunctionForIntervalArithmetic(arguments[0], arguments[1], context)) {
      ...
      for (size_t i = 0; i < 2; ++i) new_arguments[i].type = arguments[i];
    ...
    if (auto function_builder = getFunctionForDateTupleOfIntervalsArithmetic(arguments[0], arguments[1], context)) {
      ...
      /// Tuple argument must be second.
      if (isTuple(new_arguments[0].type)) std::swap(new_arguments[0], new_arguments[1]);
    ...
    if (auto function_builder = getFunctionForTupleAndNumberArithmetic(arguments[0], arguments[1], context)) {
      ...
      /// Number argument must be second.
      if (isNumber(new_arguments[0].type)) std::swap(new_arguments[0], new_arguments[1]);
    ...
    const bool valid = castBothTypes(arguments[0].get(), arguments[1].get(), [&](const auto &left, const auto &right) {
      ...
      if constexpr ((std::is_same_v<DataTypeFixedString, LeftDataType> || std::is_same_v<DataTypeString, LeftDataType>) ||
        ...
        if constexpr (is_bit_hamming_distance && std::is_same_v<DataTypeString, LeftDataType> &&
        ...
        if constexpr (std::is_same_v<LeftDataType, DataTypeInterval> && std::is_same_v<RightDataType, DataTypeInterval>) {
          if constexpr (is_plus || is_minus) {
        ...
        if constexpr (!std::is_same_v<ResultDataType, InvalidType>) {
          if constexpr (is_int_div || is_int_div_or_zero)
            ...
            if constexpr (std::is_same_v<RightDataType, DataTypeDateTime>) tz = &right;
            if constexpr (std::is_same_v<LeftDataType, DataTypeDateTime>) tz = &left;
    ...
    throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT, "Illegal types {} and {} of arguments of function {}", arguments[0]->getName(),
                    arguments[1]->getName(), String(name));
  }
  ...
  ColumnPtr executeFixedString(const ColumnsWithTypeAndName &arguments) const {
    ...
    if (const auto *col_left_const = checkAndGetColumnConst<ColumnFixedString>(col_left_raw)) {
      if (const auto *col_right_const = checkAndGetColumnConst<ColumnFixedString>(col_right_raw)) {
        ...
        if (col_left->getN() != col_right->getN()) return nullptr;
        ...
        if constexpr (is_bit_hamming_distance) {
          ...
          OpReduceImpl::template process<OpCase::Vector>(col_left->getChars().data(), col_right->getChars().data(), data.data(),
                                                         data.size(), col_left->getN());
          ...
          OpImpl::template process<OpCase::Vector>(col_left->getChars().data(), col_right->getChars().data(), out_chars.data(),
                                                   out_chars.size(), {});
    ...
    if (col_left && col_right) {
      ...
      if constexpr (is_bit_hamming_distance) {
        ...
        if (!is_left_column_const && !is_right_column_const) {
          ...
          OpReduceImpl::template process<OpCase::LeftConstant>(col_left->getChars().data(), col_right->getChars().data(), data.data(),
                                                               data.size(), col_left->getN());
        } else {
          OpReduceImpl::template process<OpCase::RightConstant>(col_left->getChars().data(), col_right->getChars().data(), data.data(),
                                                                data.size(), col_left->getN());
        }
        ...
        if (!is_left_column_const && !is_right_column_const) {
          ...
          OpImpl::template process<OpCase::LeftConstant>(col_left->getChars().data(), col_right->getChars().data(), out_chars.data(),
                                                         out_chars.size(), col_left->getN());
        } else {
          OpImpl::template process<OpCase::RightConstant>(col_left->getChars().data(), col_right->getChars().data(), out_chars.data(),
                                                          out_chars.size(), col_left->getN());
        }
  ...
  ColumnPtr executeString(const ColumnsWithTypeAndName &arguments) const {
    ...
    if (col_left && col_right) {
      ...
      if (!is_left_column_const && !is_right_column_const) {
        OpImpl::vectorVector(col_left->getChars(), col_left->getOffsets(), col_right->getChars(), col_right->getOffsets(), data);
      } else if (is_left_column_const) {
  ...
  ColumnPtr executeStringInteger(const ColumnsWithTypeAndName &arguments, const A &left, const B &right) const {
    ...
    if (!col_left_const && !col_right_const && col_right) {
      if constexpr (std::is_same_v<LeftDataType, DataTypeFixedString>) {
        OpImpl::template processFixedString<OpCase::Vector>(in_vec.data(), col_left->getN(), col_right->getData().data(), out_vec,
                                                            col_left->size());
      } else {
        ColumnString::Offsets &out_offsets = col_res->getOffsets();
        out_offsets.reserve(col_left->size());
        OpImpl::template processString<OpCase::Vector>(in_vec.data(), col_left->getOffsets().data(), col_right->getData().data(), out_vec,
                                                       out_offsets, col_left->size());
      }
    } else if (col_left_const && col_right) {
      if constexpr (std::is_same_v<LeftDataType, DataTypeFixedString>) {
        OpImpl::template processFixedString<OpCase::LeftConstant>(in_vec.data(), col_left->getN(), col_right->getData().data(), out_vec,
                                                                  col_right->size());
      } else {
        ColumnString::Offsets &out_offsets = col_res->getOffsets();
        out_offsets.reserve(col_right->size());
        OpImpl::template processString<OpCase::LeftConstant>(in_vec.data(), col_left->getOffsets().data(), col_right->getData().data(),
                                                             out_vec, out_offsets, col_right->size());
      }
      ...
      if constexpr (std::is_same_v<LeftDataType, DataTypeFixedString>) {
        ...
        ColumnString::Offsets &out_offsets = col_res->getOffsets();
        out_offsets.reserve(col_left->size());
        OpImpl::template processString<OpCase::RightConstant>(in_vec.data(), col_left->getOffsets().data(), &value, out_vec, out_offsets,
                                                              col_left->size());
      }
    } else
      return nullptr;
  ...
  ColumnPtr executeNumeric(const ColumnsWithTypeAndName &arguments, const A &left, const B &right, const NullMap *right_nullmap) const {
            ...
            // and that would produce the compile error.
    {
      ...
      if constexpr (IsDataTypeDecimal<ResultDataType>) {
        return executeNumericWithDecimal<LeftDataType, RightDataType, ResultDataType>(left, right, col_left_const, col_right_const,
                                                                                      col_left, col_right, col_left_size, right_nullmap);
      }
      /// Here we check if we have `intDiv` or `intDivOrZero` and at least one of the arguments is decimal, because in this case originally
      /// we had result as decimal, so we need to convert result into integer after calculations
      else if constexpr (!decimal_with_float && (is_int_div || is_int_div_or_zero) &&
        ...
        if constexpr (!std::is_same_v<DecimalResultType, InvalidType>) {
          ...
          auto res = executeNumericWithDecimal<LeftDataType, RightDataType, DecimalResultType>(
              left, right, col_left_const, col_right_const, col_left, col_right, col_left_size, right_nullmap);
              ...
              // ColVecResult for Decimals which would lead to a compile error.
      {
        ...
        if (col_left && col_right) {
          OpImpl::template process<OpCase::Vector>(col_left->getData().data(), col_right->getData().data(), vec_res.data(), vec_res.size(),
                                                   right_nullmap);
        } else if (col_left_const && col_right) {
          const T0 value = col_left_const->template getValue<T0>();

          OpImpl::template process<OpCase::LeftConstant>(&value, col_right->getData().data(), vec_res.data(), vec_res.size(),
                                                         right_nullmap);
        } else if (col_left && col_right_const) {
          const T1 value = col_right_const->template getValue<T1>();

          OpImpl::template process<OpCase::RightConstant>(col_left->getData().data(), &value, vec_res.data(), vec_res.size(),
                                                          right_nullmap);
        } else
          return nullptr;
  ...
  ColumnPtr executeImpl2(const ColumnsWithTypeAndName &arguments, const DataTypePtr &result_type, size_t input_rows_count,
    ...
    if (isIPv4(arguments[0].type) || isIPv4(arguments[1].type)) {
      ColumnsWithTypeAndName new_arguments{
          ...
          {isIPv4(arguments[1].type) ? castColumn(arguments[1], std::make_shared<DataTypeUInt32>()) : arguments[1].column,
           isIPv4(arguments[1].type) ? std::make_shared<DataTypeUInt32>() : arguments[1].type, arguments[1].name}};
    ...
    if (isIPv6(arguments[0].type) || isIPv6(arguments[1].type)) {
      ColumnsWithTypeAndName new_arguments{
          ...
          {isIPv6(arguments[1].type) ? castColumn(arguments[1], std::make_shared<DataTypeUInt128>()) : arguments[1].column,
           isIPv6(arguments[1].type) ? std::make_shared<DataTypeUInt128>() : arguments[1].type, arguments[1].name}};
    ...
    if constexpr (IsOperation<Op>::allow_decimal && valid_on_float_arguments && !IsOperation<Op>::int_div &&
                  !IsOperation<Op>::int_div_or_zero && !IsOperation<Op>::int_div_or_null) {
    ...
    const bool valid = castBothTypes(left_generic, right_generic, [&](const auto &left, const auto &right) {
      ...
      } else if constexpr (IsOperation<Op>::allow_decimal && valid_on_float_arguments && !IsOperation<Op>::int_div &&
                           !IsOperation<Op>::int_div_or_zero && !IsOperation<Op>::int_div_or_null &&
    ...
    if (!valid) {
      ...
      throw Exception(ErrorCodes::LOGICAL_ERROR,
                      ...
                      " '{}' of type '{}'",
                      getName(), left_argument.name, left_argument.type->getName(), right_argument.name, right_argument.type->getName());
    }
  ...
  bool isCompilableImpl(const DataTypes &arguments, const DataTypePtr &result_type) const override {
    if (2 != arguments.size()) return false;

    if (!canBeNativeType(*arguments[0]) || !canBeNativeType(*arguments[1]) || !canBeNativeType(*result_type)) return false;
    ...
    return castBothTypes(denull_left_type.get(), denull_right_type.get(), [&](const auto &left, const auto &right) {
      using LeftDataType = std::decay_t<decltype(left)>;
      using RightDataType = std::decay_t<decltype(right)>;
      if constexpr (!std::is_same_v<DataTypeFixedString, LeftDataType> && !std::is_same_v<DataTypeFixedString, RightDataType> &&
        ...
        if constexpr (!std::is_same_v<ResultDataType, InvalidType> && !IsDataTypeDecimal<ResultDataType> &&
  ...
  llvm::Value *compileImpl(llvm::IRBuilderBase &builder, const ValuesWithType &arguments, const DataTypePtr &result_type) const override {
    ...
    castBothTypes(denull_left_type.get(), denull_right_type.get(), [&](const auto &left, const auto &right) {
      using LeftDataType = std::decay_t<decltype(left)>;
      using RightDataType = std::decay_t<decltype(right)>;
      if constexpr (!std::is_same_v<DataTypeFixedString, LeftDataType> && !std::is_same_v<DataTypeFixedString, RightDataType> &&
                    !std::is_same_v<DataTypeString, LeftDataType> && !std::is_same_v<DataTypeString, RightDataType>) {
...
template <template <typename, typename> class Op, typename Name, bool valid_on_default_arguments = true,
          bool valid_on_float_arguments = true, bool division_by_nullable = false>
class FunctionBinaryArithmeticWithConstants
    : public FunctionBinaryArithmetic<Op, Name, valid_on_default_arguments, valid_on_float_arguments, division_by_nullable> {
 public:
  ...
  static FunctionPtr create(const ColumnWithTypeAndName &left_, const ColumnWithTypeAndName &right_, const DataTypePtr &return_type_,
                            ContextPtr context) {
    return std::make_shared<FunctionBinaryArithmeticWithConstants>(left_, right_, return_type_, context);
  }

  FunctionBinaryArithmeticWithConstants(const ColumnWithTypeAndName &left_, const ColumnWithTypeAndName &right_,
                                        const DataTypePtr &return_type_, ContextPtr context_)
      : Base(context_), left(left_), right(right_), return_type(return_type_) {}

  ColumnPtr executeImpl(const ColumnsWithTypeAndName &arguments, const DataTypePtr &result_type, size_t input_rows_count) const override {
    if (left.column && isColumnConst(*left.column) && arguments.size() == 1) {
      ColumnsWithTypeAndName columns_with_constant = {{left.column->cloneResized(input_rows_count), left.type, left.name}, arguments[0]};
    ...
    if (right.column && isColumnConst(*right.column) && arguments.size() == 1) {
      ColumnsWithTypeAndName columns_with_constant = {arguments[0], {right.column->cloneResized(input_rows_count), right.type, right.name}};
  ...
  Monotonicity getMonotonicityForRange(const IDataType &, const Field &left_point, const Field &right_point) const override {
    ...
    if (accurateEquals(left_point, right_point)) return {true, true, false, false};
    ...
    if (name_view == "minus" || name_view == "plus") {
      ...
      if (left.column && isColumnConst(*left.column)) {
        ...
        auto transform = [&](const Field &point) {
          ColumnsWithTypeAndName columns_with_constant = {{left_type->createColumnConst(1, (*left.column)[0]), left_type, left.name},
        ...
        bool is_positive_monotonicity =
            accurateLess(left_point, right_point) == accurateLess(transform(left_point), transform(right_point));

        if (name_view == "plus") {
          // Check if there is an overflow
          if (is_positive_monotonicity) return {true, true, false, true};
          return {false, true, false, false};
        }

        // Check if there is an overflow
        if (!is_positive_monotonicity) return {true, false, false, true};
      ...
      if (right.column && isColumnConst(*right.column)) {
        ...
        auto transform = [&](const Field &point) {
          ColumnsWithTypeAndName columns_with_constant = {{left_type->createColumnConst(1, point), left_type, left.name},
    ...
    if (name_view == "divide" || name_view == "intDiv") {
      ...
      if (left.column && isColumnConst(*left.column)) {
        ...
        if (accurateEquals(constant, Field(0))) return {true, true, false, false};  // 0 / 0 is undefined, thus it's not always monotonic

        bool is_constant_positive = accurateLess(Field(0), constant);
        if (accurateLess(left_point, Field(0)) && accurateLess(right_point, Field(0))) {
          return {true, is_constant_positive, false, is_strict};
        }
        if (accurateLess(Field(0), left_point) && accurateLess(Field(0), right_point)) {
          return {true, !is_constant_positive, false, is_strict};
        }
      }
      // variable / constant
...
template <template <typename, typename> class Op, typename Name, bool valid_on_default_arguments = true,
          bool valid_on_float_arguments = true>
class BinaryArithmeticOverloadResolver : public IFunctionOverloadResolver {
 public:
  static constexpr auto name = Name::name;
  static FunctionOverloadResolverPtr create(ContextPtr context) { return std::make_unique<BinaryArithmeticOverloadResolver>(context); }
  ...
  FunctionBasePtr buildImpl(const ColumnsWithTypeAndName &arguments, const DataTypePtr &return_type) const override {
    ...
    auto make_adaptor = [&](auto function) {
      return std::make_unique<FunctionToFunctionBaseAdaptor>(
          function, DataTypes{std::from_range_t{}, arguments | std::views::transform([](const auto &elem) { return elem.type; })},
          return_type);
    };

    /// More efficient specialization for two numeric arguments.
    if (arguments.size() == 2 &&
        ((arguments[0].column && isColumnConst(*arguments[0].column)) || (arguments[1].column && isColumnConst(*arguments[1].column)))) {
      if constexpr (can_have_division_by_nullable) {
  ...
  DataTypePtr getReturnTypeImpl(const DataTypes &arguments) const override {
    if (arguments.size() != 2)
      throw Exception(ErrorCodes::NUMBER_OF_ARGUMENTS_DOESNT_MATCH,
                      "Number of arguments for function {} doesn't match: passed {}, should be 2", getName(), arguments.size());
    return FunctionBinaryArithmetic<Op, Name, valid_on_default_arguments, valid_on_float_arguments>::getReturnTypeImplStatic(arguments,
                                                                                                                             context);
  }
