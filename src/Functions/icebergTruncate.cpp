...
class FunctionIcebergTruncate : public IFunction {
 public:
  ...
  static FunctionPtr create(ContextPtr context_) { return std::make_shared<FunctionIcebergTruncate>(context_); }
  ...
  DataTypePtr getReturnTypeImpl(const DataTypes &arguments) const override {
    ...
    if (!which_truncate.isDecimal64() && !which_truncate.isDecimal32() && !which_truncate.isStringOrFixedString() &&
        !which_truncate.isNativeInteger())
  ...
  ColumnPtr executeImpl(const ColumnsWithTypeAndName &arguments, const DataTypePtr & /* result_type */,
    ...
    if (value <= 0) throw Exception(ErrorCodes::BAD_ARGUMENTS, "Function icebergTruncate accepts only positive width");
    ...
    if (which_truncate.isStringOrFixedString()) {
      ...
      if (which_truncate.isNativeInteger()) {
        ...
        if (const auto *decimal_type = checkDecimal<Decimal32>(*arguments[1].type))
          decimal_scaled = arguments[1].type->createColumnConst(
              input_rows_count, DecimalField<Decimal32>(static_cast<Int32>(value), decimal_type->getScale()));
        ...
        if (!decimal_scaled) throw Exception(ErrorCodes::LOGICAL_ERROR, "Unexpected decimal data type");
