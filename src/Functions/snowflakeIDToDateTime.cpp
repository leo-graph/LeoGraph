...
class FunctionSnowflakeIDToDateTime : public IFunction {
 ...
 public:
  ...
  DataTypePtr getReturnTypeImpl(const ColumnsWithTypeAndName &arguments) const override {
    FunctionArgumentDescriptors args{{"value", static_cast<FunctionArgumentDescriptor::TypeValidator>(&isUInt64), nullptr, "UInt64"}};
    ...
    String timezone;
    if (arguments.size() == 3) timezone = extractTimeZoneNameFromFunctionArguments(arguments, 2, 0, allow_nonconst_timezone_arguments);
  ...
  ColumnPtr executeImpl(const ColumnsWithTypeAndName &arguments, const DataTypePtr &, size_t input_rows_count) const override {
    ...
    if (const auto *col_src_non_const = typeid_cast<const ColumnUInt64 *>(&col_src)) {
      ...
      for (size_t i = 0; i < input_rows_count; ++i) res_data[i] = static_cast<UInt32>(((src_data[i] >> time_shift) + epoch) / 1000);
    } else if (const auto *col_src_const = typeid_cast<const ColumnConst *>(&col_src)) {
      UInt64 src_val = col_src_const->getValue<UInt64>();
      for (size_t i = 0; i < input_rows_count; ++i) res_data[i] = static_cast<UInt32>(((src_val >> time_shift) + epoch) / 1000);
    } else
...
class FunctionSnowflakeIDToDateTime64 : public IFunction {
 ...
 public:
  ...
  ColumnPtr executeImpl(const ColumnsWithTypeAndName &arguments, const DataTypePtr &, size_t input_rows_count) const override {
    ...
    if (const auto *col_src_non_const = typeid_cast<const ColumnUInt64 *>(&col_src)) {
      const auto &src_data = col_src_non_const->getData();
      for (size_t i = 0; i < input_rows_count; ++i) res_data[i] = (src_data[i] >> time_shift) + epoch;
    } else if (const auto *col_src_const = typeid_cast<const ColumnConst *>(&col_src)) {
      UInt64 src_val = col_src_const->getValue<UInt64>();
      for (size_t i = 0; i < input_rows_count; ++i) res_data[i] = (src_val >> time_shift) + epoch;
    } else
      throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT, "Illegal argument for function {}", name);
...
Returns the timestamp component of a [Snowflake ID](https://en.wikipedia.org/wiki/Snowflake_ID) as a value of type [DateTime](../data-types/datetime.md).
  ...
  FunctionDocumentation::Arguments arguments_snowflakeIDToDateTime = {
      ...
      {"epoch",
       "Optional. Epoch of the Snowflake ID in milliseconds since 1970-01-01. Defaults to 0 (1970-01-01). For the Twitter/X epoch "
       "(2015-01-01), provide 1288834974657.",
       {"UInt*"}},
      {"time_zone",
  ...
  FunctionDocumentation::Examples examples_snowflakeIDToDateTime = {{"Usage example",
                                                                     R"(
...
└─────────────────────┘
  ...
  FunctionDocumentation documentation_snowflakeIDToDateTime = {description_snowflakeIDToDateTime,    syntax_snowflakeIDToDateTime,
                                                               arguments_snowflakeIDToDateTime,      {},
                                                               returned_value_snowflakeIDToDateTime, examples_snowflakeIDToDateTime,
                                                               introduced_in_snowflakeIDToDateTime,  category_snowflakeIDToDateTime};
...
Returns the timestamp component of a [Snowflake ID](https://en.wikipedia.org/wiki/Snowflake_ID) as a value of type [DateTime64](../data-types/datetime64.md).
  ...
  FunctionDocumentation::Arguments arguments_snowflakeIDToDateTime64 = {
      ...
      {"time_zone",
       ...
       "according to the timezone.",
       {"String"}}};
  FunctionDocumentation::ReturnedValue returned_value_snowflakeIDToDateTime64 = {
      "Returns the timestamp component of `value` as a `DateTime64` with scale = 3, i.e. millisecond precision.", {"DateTime64"}};
  FunctionDocumentation::Examples examples_snowflakeIDToDateTime64 = {{"Usage example",
                                                                       R"(
...
└─────────────────────┘
  ...
  FunctionDocumentation documentation_snowflakeIDToDateTime64 = {description_snowflakeIDToDateTime64,    syntax_snowflakeIDToDateTime64,
                                                                 arguments_snowflakeIDToDateTime64,      {},
                                                                 returned_value_snowflakeIDToDateTime64, examples_snowflakeIDToDateTime64,
                                                                 introduced_in_snowflakeIDToDateTime64,  category_snowflakeIDToDateTime64};
