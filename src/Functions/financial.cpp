...
std::optional<DayCountType> parseDayCount(std::string_view day_count) {
  ...
  if (day_count == "ACT_365F") return ACT_365F;
  if (day_count == "ACT_365_25") return ACT_365_25;
  return std::nullopt;
}

constexpr int daysBetweenAct(int d1, int d2) { return d2 - d1; }
...
struct NpvCalculator {
  ...
  explicit NpvCalculator(std::span<T> cashflows_) : cashflows(cashflows_) {
  ...
  double calculate(double rate) const {
    if (rate == 0) return std::accumulate(cashflows.begin(), cashflows.end(), 0.0);
    if (rate <= -1.0) return std::numeric_limits<double>::infinity();
  ...
  double derivative(double rate) const {
    if (rate <= -1.0) return std::numeric_limits<double>::quiet_NaN();
...
struct XnpvCalculator {
  ...
  XnpvCalculator(std::span<T> cashflows_, std::span<D> dates_) : cashflows(cashflows_), dates(dates_) {
  ...
  double derivative(double rate) const {
    ...
    for (size_t i = 0; i < cashflows.size(); ++i) {
      double time = yearFraction<day_count>(dates[0], dates[i]);
      if (time != 0.0) derivative -= static_cast<FloatType>(cashflows[i]) * time / std::pow(1.0 + rate, time + 1);
    }
...
std::expected<double, SolverErrorCode> solver(Function &&fun, Derivative &&der, double guess) {
  ...
  try {
    ...
    double result = boost::math::tools::newton_raphson_iterate([&fun, &der](double x) { return std::make_tuple(fun(x), der(x)); }, guess,
                                                               start_lower_bound, start_upper_bound, binary_precision, max_iter);

    if (result >= start_lower_bound && result <= start_upper_bound && std::abs(fun(result)) < tolerance) return result;

    // Fallback to TOMS748
    const double f_lower = fun(start_lower_bound);
    const double f_upper = fun(start_upper_bound);

    if (f_lower * f_upper >= 0.0) return std::unexpected(SolverErrorCode::NO_ROOT_FOUND_IN_BRACKET);
 ...
 public:
  ...
  DataTypePtr getReturnTypeImpl(const ColumnsWithTypeAndName &arguments) const override {
    auto mandatory_args = FunctionArgumentDescriptors{
        {"cashflow", static_cast<FunctionArgumentDescriptor::TypeValidator>(&isCashFlowColumn), nullptr,
  ...
  ColumnPtr executeImpl(const ColumnsWithTypeAndName &arguments, const DataTypePtr &, size_t input_rows_count) const override {
    ...
    if (!cashflow_array || !date_array) throw Exception(ErrorCodes::LOGICAL_ERROR, "Both cashflow and date arguments must be arrays");
    ...
    auto process_arrays = [&]<typename CashFlowCol, typename DateTypeCol>(const CashFlowCol *cashflow_values,
                                                                          const DateTypeCol *date_values) {
...
class FunctionIRR : public IFunction {
 public:
  ...
  ColumnPtr executeImpl(const ColumnsWithTypeAndName &arguments, const DataTypePtr &, size_t input_rows_count) const override {
    auto cashflow_col = arguments[0].column->convertToFullIfNeeded();
    const auto *cashflow_array = checkAndGetColumn<ColumnArray>(cashflow_col.get());
    if (!cashflow_array) throw Exception(ErrorCodes::LOGICAL_ERROR, "Cashflow argument must be an array");
    const ColumnArray::Offsets &cashflow_offsets = cashflow_array->getOffsets();
...
class FunctionXnpv : public IFunction {
 public:
  ...
  ColumnPtr executeImpl(const ColumnsWithTypeAndName &arguments, const DataTypePtr &, size_t input_rows_count) const override {
    ...
    auto process_arrays = [&]<typename CashFlowCol, typename DateTypeCol>(const CashFlowCol *cashflow_values,
                                                                          const DateTypeCol *date_values, const auto &rate_pod) {
      using CashFlowType = const typename CashFlowCol::ValueType;
      using DateType = const typename DateTypeCol::ValueType;
      auto loop = [&]<DayCountType day_count_type> {
        ColumnArray::Offset previous_offset = 0;
...
$$
  ...
  FunctionDocumentation::Arguments arguments = {
      {"cashflows",
       ...
       {"Array(Int8/16/32/64)", "Array(Float*)"}},
      {"[, guess]", "Optional initial guess (constant value) for the internal rate of return (default 0.1).", {"Float*"}},
  };
  FunctionDocumentation::ReturnedValue returned_value{
      "Returns the internal rate of return or `NaN` if the calculation cannot converge, input array is empty or has only one element, all "
      "cash flows are zero, or other calculation errors occur.",
      {"Float64"}};
  FunctionDocumentation::Examples examples = {
...
Arrays should be sorted by date in ascending order. Dates need to be unique.
  ...
  FunctionDocumentation::Arguments arguments = {
      ...
      {"[, daycount]",
       "Optional day count convention. Supported values: `'ACT_365F'` (default) — Actual/365 Fixed, `'ACT_365_25'` — Actual/365.25.",
       {"String"}}};
  FunctionDocumentation::ReturnedValue returned_value = {"Returns the net present value as a Float64 value.", {"Float64"}};
  FunctionDocumentation::Examples examples = {
      {"Basic usage",
       "SELECT financialNetPresentValueExtended(0.1, [-10000., 5750., 4250., 3250.], [toDate('2020-01-01'), toDate('2020-03-01'), "
       "toDate('2020-10-30'), toDate('2021-02-15')])",
       "2506.579458169746"},
      {"Using different day count convention",
       "SELECT financialNetPresentValueExtended(0.1, [-10000., 5750., 4250., 3250.], [toDate('2020-01-01'), toDate('2020-03-01'), "
       "toDate('2020-10-30'), toDate('2021-02-15')], 'ACT_365_25')",
       "2507.067268742502"}};
  FunctionDocumentation::IntroducedIn introduced_in = {25, 7};
  FunctionDocumentation::Category category = FunctionDocumentation::Category::Financial;
  FunctionDocumentation documentation = {description, syntax, arguments, {}, returned_value, examples, introduced_in, category};
...
$$
  ...
  FunctionDocumentation::Examples examples = {
      ...
      {"excel_compatible_calculation", "SELECT financialNetPresentValue(0.08, [-40000., 5000., 8000., 12000., 30000.], false)",
       "2838.1691372032656"}};
  FunctionDocumentation::IntroducedIn introduced_in = {25, 7};
  FunctionDocumentation::Category category = FunctionDocumentation::Category::Financial;
  FunctionDocumentation documentation = {description, syntax, arguments, {}, returned_value, examples, introduced_in, category};
