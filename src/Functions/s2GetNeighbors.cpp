...
class FunctionS2GetNeighbors : public IFunction {
 public:
  ...
  static FunctionPtr create(ContextPtr) { return std::make_shared<FunctionS2GetNeighbors>(); }
  ...
  ColumnPtr executeImpl(const ColumnsWithTypeAndName &arguments, const DataTypePtr &, size_t input_rows_count) const override {
    ...
    for (size_t row = 0; row < input_rows_count; ++row) {
      ...
      if (!cell_id.is_valid()) throw Exception(ErrorCodes::BAD_ARGUMENTS, "Cell (id {}) is not valid in function {}", id, getName());
...
Returns the S2 neighbor indices corresponding to the provided S2 cell index. Each cell in the S2 system is a quadrilateral bounded by four geodesics. Each cell has exactly 4 neighbors.
  ...
  FunctionDocumentation::Arguments arguments = {{"s2index", "The S2 cell identifier.", {"UInt64"}}};
  FunctionDocumentation::ReturnedValue returned_value = {"Returns an array of 4 neighbor S2 cell identifiers.", {"Array(UInt64)"}};
  FunctionDocumentation::Examples examples = {{"Basic usage", "SELECT s2GetNeighbors(5765131099823669248)",
                                               "[5765131099698741248, 5765131099950653440, 5765131100074868736, 5765131100088901632]"}};
