...
class FunctionNormalizedQueryHash : public IFunction {
 private:
  ...
  void process(const ColumnString::Chars &data, const ColumnString::Offsets &offsets, PaddedPODArray<UInt64> &res_data,
    ...
    for (size_t i = 0; i < input_rows_count; ++i) {
      ...
      res_data[i] = normalizedQueryHash(reinterpret_cast<const char *>(&data[prev_src_offset]),
                                        reinterpret_cast<const char *>(&data[curr_src_offset]), keep_names);
 ...
 public:
  ...
  String getName() const override { return keep_names ? "normalizedQueryHashKeepNames" : "normalizedQueryHash"; }
...
Can be helpful in analyzing query logs.
  ...
  FunctionDocumentation::Arguments normalizedQueryHash_arguments = {{"x", "Sequence of characters.", {"String"}}};
  FunctionDocumentation::ReturnedValue normalizedQueryHash_returned_value = {"Returns a 64 bit hash value.", {"UInt64"}};
  FunctionDocumentation::Examples normalizedQueryHash_examples = {{"Usage example",
                                                                   R"(
...
└─────┘
  ...
  FunctionDocumentation normalizedQueryHash_documentation = {
      normalizedQueryHash_description,    normalizedQueryHash_syntax,   normalizedQueryHash_arguments,     {},
      normalizedQueryHash_returned_value, normalizedQueryHash_examples, normalizedQueryHash_introduced_in, normalizedQueryHash_category};
...
Can be helpful in analyzing query logs.
  ...
  FunctionDocumentation::Arguments normalizedQueryHashKeepNames_arguments = {{"x", "Sequence of characters.", {"String"}}};
  FunctionDocumentation::ReturnedValue normalizedQueryHashKeepNames_returned_value = {"Returns a 64 bit hash value.", {"UInt64"}};
  FunctionDocumentation::Examples normalizedQueryHashKeepNames_examples = {{"Usage example",
                                                                            R"(
...
└──────────────────────────────┘
  ...
  FunctionDocumentation normalizedQueryHashKeepNames_documentation = {
      normalizedQueryHashKeepNames_description,    normalizedQueryHashKeepNames_syntax,
      normalizedQueryHashKeepNames_arguments,      {},
      normalizedQueryHashKeepNames_returned_value, normalizedQueryHashKeepNames_examples,
      normalizedQueryHashKeepNames_introduced_in,  normalizedQueryHashKeepNames_category};

  factory.registerFunction(
      "normalizedQueryHashKeepNames", [](ContextPtr) { return std::make_shared<FunctionNormalizedQueryHash>(true); },
      normalizedQueryHashKeepNames_documentation);
  factory.registerFunction(
      "normalizedQueryHash", [](ContextPtr) { return std::make_shared<FunctionNormalizedQueryHash>(false); },
      normalizedQueryHash_documentation);
}
