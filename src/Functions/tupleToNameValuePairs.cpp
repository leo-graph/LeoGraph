...
class FunctionTupleToNameValuePairs : public IFunction {
 public:
  ...
  static FunctionPtr create(ContextPtr) { return std::make_shared<FunctionTupleToNameValuePairs>(); }
  ...
  DataTypePtr getReturnTypeImpl(const ColumnsWithTypeAndName &arguments) const override {
    ...
    if (!tuple)
      throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT, "First argument for function {} must be a Tuple or a Nullable(Tuple).",
                      getName());

    const auto &element_types = tuple->getElements();

    if (element_types.empty())
      throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT, "The argument tuple for function {} must not be empty.", getName());

    const auto &first_element_type = element_types[0];

    bool all_value_types_equal =
        std::all_of(element_types.begin() + 1, element_types.end(), [&](const auto &other) { return first_element_type->equals(*other); });

    if (!all_value_types_equal) {
      throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT, "The argument tuple for function {} must contain just one type", getName());
    }

    DataTypePtr tuple_name_type = std::make_shared<DataTypeString>();
    DataTypes item_data_types = {tuple_name_type, first_element_type};
...
All values in the tuple must be of the same type.
  ...
  FunctionDocumentation::Arguments arguments = {{"tuple", "Named tuple with any types of values.", {"Tuple(n1 T1[, n2 T2, ...])"}}};
