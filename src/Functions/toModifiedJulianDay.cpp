 ...
 public:
  ...
  DataTypePtr getReturnTypeImpl(const DataTypes &arguments) const override {
    if (!isStringOrFixedString(arguments[0])) {
      throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT, "The argument of function {} must be String or FixedString", getName());
    }
...
toModifiedJulianDay(date)
  ...
  FunctionDocumentation::Arguments arguments_toModifiedJulianDay = {{"date", "The date in String form.", {"String", "FixedString"}}};
  FunctionDocumentation::ReturnedValue returned_value_toModifiedJulianDay = {"Returns Modified Julian Day number.", {"Int32"}};
  FunctionDocumentation::Examples examples_toModifiedJulianDay = {{"Convert date to Modified Julian Day", R"(
...
└───────────────────────────────────┘
  ...
  FunctionDocumentation documentation_toModifiedJulianDay = {
      description_toModifiedJulianDay,    syntax_toModifiedJulianDay,   arguments_toModifiedJulianDay,     {},
      returned_value_toModifiedJulianDay, examples_toModifiedJulianDay, introduced_in_toModifiedJulianDay, category_toModifiedJulianDay};

  factory.registerFunction<ToModifiedJulianDayOverloadResolver<NameToModifiedJulianDay, DataTypeInt32, false>>(
      documentation_toModifiedJulianDay);
...
toModifiedJulianDayOrNull(date)
  ...
  FunctionDocumentation::Arguments arguments_toModifiedJulianDayOrNull = {{"date", "Date in text form.", {"String", "FixedString"}}};
  FunctionDocumentation::ReturnedValue returned_value_toModifiedJulianDayOrNull = {
      "Returns the modified Julian day number for valid `date`, otherwise `null`.", {"Nullable(Int32)"}};
  FunctionDocumentation::Examples examples_toModifiedJulianDayOrNull = {{"Convert date to Modified Julian Day with null handling", R"(
...
└──────────────────────────┘
  ...
  FunctionDocumentation documentation_toModifiedJulianDayOrNull = {
      description_toModifiedJulianDayOrNull,    syntax_toModifiedJulianDayOrNull,
      arguments_toModifiedJulianDayOrNull,      {},
      returned_value_toModifiedJulianDayOrNull, examples_toModifiedJulianDayOrNull,
      introduced_in_toModifiedJulianDayOrNull,  category_toModifiedJulianDayOrNull};

  factory.registerFunction<ToModifiedJulianDayOverloadResolver<NameToModifiedJulianDayOrNull, DataTypeInt32, true>>(
      documentation_toModifiedJulianDayOrNull);
}
}  // namespace DB
