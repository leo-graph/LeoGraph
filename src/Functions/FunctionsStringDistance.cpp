...
struct FunctionStringDistanceImpl {
  ...
  static void vectorVector(const ColumnString::Chars &haystack_data, const ColumnString::Offsets &haystack_offsets,
                           ...
                           PaddedPODArray<ResultType> &res, size_t input_rows_count) {
    ...
    for (size_t i = 0; i < input_rows_count; ++i) {
      res[i] = Op::process(haystack + haystack_offsets[i - 1], haystack_offsets[i] - haystack_offsets[i - 1],
                           needle + needle_offsets[i - 1], needle_offsets[i] - needle_offsets[i - 1]);
    }
  }

  static void constantVector(const String &haystack, const ColumnString::Chars &needle_data, const ColumnString::Offsets &needle_offsets,
                             PaddedPODArray<ResultType> &res, size_t input_rows_count) {
    const char *haystack_data = haystack.data();
    size_t haystack_size = haystack.size();
    const char *needle = reinterpret_cast<const char *>(needle_data.data());
    for (size_t i = 0; i < input_rows_count; ++i) {
      res[i] = Op::process(haystack_data, haystack_size, needle + needle_offsets[i - 1], needle_offsets[i] - needle_offsets[i - 1]);
    }
  }

  static void vectorConstant(const ColumnString::Chars &data, const ColumnString::Offsets &offsets, const String &needle,
                             PaddedPODArray<ResultType> &res, size_t input_rows_count) {
...
#endif
    for (; haystack != haystack_end && needle != needle_end; ++haystack, ++needle) res += *haystack != *needle;
  ...
  while (data < end) {
    ...
    if (len == 1) {
      ...
      if (code_point.has_value()) {
        ...
        throw Exception(ErrorCodes::BAD_ARGUMENTS, "Illegal UTF-8 sequence, while processing '{}'", std::string_view(data, end - data));
      }
    }
  }
}
...
struct ByteJaccardIndexImpl {
  ...
  static ResultType process(const char *__restrict haystack, size_t haystack_size, const char *__restrict needle, size_t needle_size) {
    if (haystack_size == 0 || needle_size == 0) return 0;
    ...
    if constexpr (is_utf8) {
      parseUTF8String(haystack, haystack_size, [&](UInt32 data) { scratch.haystack_utf8_set.insert(data); });
      parseUTF8String(needle, needle_size, [&](UInt32 data) { scratch.needle_utf8_set.insert(data); });
    } else {
    ...
    if constexpr (is_utf8) {
      const auto &small =
          (scratch.haystack_utf8_set.size() < scratch.needle_utf8_set.size()) ? scratch.haystack_utf8_set : scratch.needle_utf8_set;
      const auto &large =
          (scratch.haystack_utf8_set.size() < scratch.needle_utf8_set.size()) ? scratch.needle_utf8_set : scratch.haystack_utf8_set;

      for (auto it = small.begin(); it != small.end(); ++it) {
        const auto &key = it->getKey();
        if (large.has(key)) ++intersection;
      }
...
struct ByteEditDistanceImpl {
  ...
  static ResultType process(const char *__restrict haystack, size_t haystack_size, const char *__restrict needle, size_t needle_size) {
    if (haystack_size == 0 || needle_size == 0) return haystack_size + needle_size;
    ...
    if (haystack_size > max_string_size || needle_size > max_string_size)
      throw Exception(ErrorCodes::TOO_LARGE_STRING_SIZE, "The string size is too big for function editDistance, should be at most {}",
                      max_string_size);
    ...
    for (size_t pos_needle = 0; pos_needle < needle_size; ++pos_needle) {
      ...
      for (size_t pos_haystack = 0; pos_haystack < haystack_size; pos_haystack++) {
        ...
        if constexpr (is_utf8) {
          if (needle_utf8[pos_needle] != haystack_utf8[pos_haystack]) substitution += 1;
        } else {
          if (*(needle + pos_needle) != *(haystack + pos_haystack)) substitution += 1;
        }
...
struct ByteJaroSimilarityImpl {
  ...
  static ResultType process(const char *__restrict haystack, size_t haystack_size, const char *__restrict needle, size_t needle_size) {
    /// Safety threshold against DoS
    if (haystack_size > max_string_size || needle_size > max_string_size)
      throw Exception(ErrorCodes::TOO_LARGE_STRING_SIZE, "The string size is too big for function jaroSimilarity, should be at most {}",
                      max_string_size);

    /// Shortcuts:

    if (haystack_size == 0) return static_cast<ResultType>(needle_size);

    if (needle_size == 0) return static_cast<ResultType>(haystack_size);

    if (haystack_size == needle_size && memcmp(haystack, needle, haystack_size) == 0) return 1.0;
    ...
    if (matching_characters == 0) return 0.0;

    /// Transpositions (one-way only)
    double transpositions = 0.0;
    for (size_t i = 0, s1i = 0, s2i = 0; i < matching_characters; i++) {
      while (s1_matching[s1i] == -1) s1i++;
      while (s2_matching[s2i] == -1) s2i++;
    ...
    double jaro_similarity = 1.0 / 3.0 * (m / static_cast<double>(s1len) + m / static_cast<double>(s2len) + (m - transpositions) / m);
...
struct ByteJaroWinklerSimilarityImpl {
  ...
  static ResultType process(const char *__restrict haystack, size_t haystack_size, const char *__restrict needle, size_t needle_size) {
    ...
    if (jaro_winkler_similarity > boost_threshold) {
      ...
      int common_prefix = 0;
      while (common_prefix < common_length && haystack[common_prefix] == needle[common_prefix]) common_prefix++;
...
using FunctionDamerauLevenshteinDistance =
    FunctionsStringSimilarity<FunctionStringDistanceImpl<ByteDamerauLevenshteinDistanceImpl>, NameDamerauLevenshteinDistance>;
...
using FunctionStringJaccardIndexUTF8 =
    FunctionsStringSimilarity<FunctionStringDistanceImpl<ByteJaccardIndexImpl<true>>, NameJaccardIndexUTF8>;
...
using FunctionJaroWinklerSimilarity =
    FunctionsStringSimilarity<FunctionStringDistanceImpl<ByteJaroWinklerSimilarityImpl>, NameJaroWinklerSimilarity>;
...
Calculates the [hamming distance](https://en.wikipedia.org/wiki/Hamming_distance) between two byte strings.
  ...
  FunctionDocumentation::Arguments arguments_hamming = {{"s1", "First input string.", {"String"}},
  ...
  FunctionDocumentation::Examples examples_hamming = {{"Usage example", "SELECT byteHammingDistance('karolin', 'kathrin')",
                                                       R"(
...
└───────────────────────────────────────────┘
  ...
  FunctionDocumentation documentation_hamming = {description_hamming,    syntax_hamming,   arguments_hamming, {},
                                                 returned_value_hamming, examples_hamming, introduced_in,     category};
...
Calculates the [edit distance](https://en.wikipedia.org/wiki/Edit_distance) between two byte strings.
  ...
  FunctionDocumentation::Arguments arguments_edit = {{"s1", "First input string.", {"String"}}, {"s2", "Second input string.", {"String"}}};
  FunctionDocumentation::ReturnedValue returned_value_edit = {"Returns the edit distance between the two strings.", {"UInt64"}};
  FunctionDocumentation::Examples examples_edit = {{"Usage example", "SELECT editDistance('clickhouse', 'mouse')",
                                                    R"(
┌─editDistance('clickhouse', 'mouse')─┐
│                                   6 │
└─────────────────────────────────────┘
        )"}};
  FunctionDocumentation documentation_edit = {description_edit,    syntax_edit,   arguments_edit, {},
                                              returned_value_edit, examples_edit, introduced_in,  category};
...
Calculates the [edit distance](https://en.wikipedia.org/wiki/Edit_distance) between two UTF8 strings.
  ...
  FunctionDocumentation::Arguments arguments_edit_utf8 = {{"s1", "First input string.", {"String"}},
                                                          {"s2", "Second input string.", {"String"}}};
  ...
  FunctionDocumentation::Examples examples_edit_utf8 = {{"Usage example", "SELECT editDistanceUTF8('我是谁', '我是我')",
                                                         R"(
...
└─────────────────────────────────────┘
  ...
  FunctionDocumentation documentation_edit_utf8 = {description_edit_utf8,    syntax_edit_utf8,   arguments_edit_utf8, {},
                                                   returned_value_edit_utf8, examples_edit_utf8, introduced_in_utf8,  category};
...
Calculates the [Damerau-Levenshtein distance](https://en.wikipedia.org/wiki/Damerau%E2%80%93Levenshtein_distance) between two byte strings.
  ...
  FunctionDocumentation::Arguments arguments_damerau = {{"s1", "First input string.", {"String"}},
                                                        {"s2", "Second input string.", {"String"}}};
  FunctionDocumentation::ReturnedValue returned_value_damerau = {"Returns the Damerau-Levenshtein distance between the two strings.",
                                                                 {"UInt64"}};
  FunctionDocumentation::Examples examples_damerau = {{"Usage example", "SELECT damerauLevenshteinDistance('clickhouse', 'mouse')",
                                                       R"(
...
└───────────────────────────────────────────────────┘
  ...
  FunctionDocumentation documentation_damerau = {description_damerau,    syntax_damerau,   arguments_damerau,     {},
                                                 returned_value_damerau, examples_damerau, introduced_in_damerau, category};
...
Calculates the [Jaccard similarity index](https://en.wikipedia.org/wiki/Jaccard_index) between two byte strings.
  ...
  FunctionDocumentation::Arguments arguments_jaccard = {{"s1", "First input string.", {"String"}},
                                                        {"s2", "Second input string.", {"String"}}};
  FunctionDocumentation::ReturnedValue returned_value_jaccard = {"Returns the Jaccard similarity index between the two strings.",
                                                                 {"Float64"}};
  FunctionDocumentation::Examples examples_jaccard = {{"Usage example", "SELECT stringJaccardIndex('clickhouse', 'mouse')",
                                                       R"(
...
└───────────────────────────────────────────┘
  ...
  FunctionDocumentation documentation_jaccard = {description_jaccard,    syntax_jaccard,   arguments_jaccard,     {},
                                                 returned_value_jaccard, examples_jaccard, introduced_in_jaccard, category};
...
Like [`stringJaccardIndex`](#stringJaccardIndex) but for UTF8-encoded strings.
  ...
  FunctionDocumentation::Arguments arguments_jaccard_utf8 = {{"s1", "First input UTF8 string.", {"String"}},
                                                             {"s2", "Second input UTF8 string.", {"String"}}};
  FunctionDocumentation::ReturnedValue returned_value_jaccard_utf8 = {"Returns the Jaccard similarity index between the two UTF8 strings.",
                                                                      {"Float64"}};
  FunctionDocumentation::Examples examples_jaccard_utf8 = {{"Usage example", "SELECT stringJaccardIndexUTF8('我爱你', '我也爱你')",
                                                            R"(
...
└─────────────────────────────────────────────┘
  ...
  FunctionDocumentation documentation_jaccard_utf8 = {
      description_jaccard_utf8,    syntax_jaccard_utf8,   arguments_jaccard_utf8,     {},
      returned_value_jaccard_utf8, examples_jaccard_utf8, introduced_in_jaccard_utf8, category};
...
Calculates the [Jaro similarity](https://en.wikipedia.org/wiki/Jaro%E2%80%93Winkler_distance#Jaro_similarity) between two byte strings.
  ...
  FunctionDocumentation::Arguments arguments_jaro = {{"s1", "First input string.", {"String"}}, {"s2", "Second input string.", {"String"}}};
  FunctionDocumentation::ReturnedValue returned_value_jaro = {"Returns the Jaro similarity between the two strings.", {"Float64"}};
  FunctionDocumentation::Examples examples_jaro = {{"Usage example", "SELECT jaroSimilarity('clickhouse', 'click')",
                                                    R"(
...
└───────────────────────────────────────┘
  ...
  FunctionDocumentation documentation_jaro = {description_jaro,    syntax_jaro,   arguments_jaro,     {},
                                              returned_value_jaro, examples_jaro, introduced_in_jaro, category};
...
Calculates the [Jaro-Winkler similarity](https://en.wikipedia.org/wiki/Jaro%E2%80%93Winkler_distance) between two byte strings.
  ...
  FunctionDocumentation::Arguments arguments_jaro_winkler = {{"s1", "First input string.", {"String"}},
                                                             {"s2", "Second input string.", {"String"}}};
  FunctionDocumentation::ReturnedValue returned_value_jaro_winkler = {"Returns the Jaro-Winkler similarity between the two strings.",
                                                                      {"Float64"}};
  FunctionDocumentation::Examples examples_jaro_winkler = {{"Usage example", "SELECT jaroWinklerSimilarity('clickhouse', 'click')",
                                                            R"(
...
└──────────────────────────────────────────────┘
  ...
  FunctionDocumentation documentation_jaro_winkler = {
      description_jaro_winkler,    syntax_jaro_winkler,   arguments_jaro_winkler,     {},
      returned_value_jaro_winkler, examples_jaro_winkler, introduced_in_jaro_winkler, category};
