...
bool isAggregateFunctionState(const IDataType &type) { return typeid_cast<const DataTypeAggregateFunction *>(&type) != nullptr; }
...
class FunctionEvalMLMethod : public IFunction {
 public:
  ...
  static FunctionPtr create(ContextPtr context) { return std::make_shared<FunctionEvalMLMethod>(context); }
  ...
  DataTypePtr getReturnTypeImpl(const ColumnsWithTypeAndName &arguments) const override {
    FunctionArgumentDescriptors mandatory_args{{"model", &isAggregateFunctionState, nullptr, "AggregateFunctionState"}};
    FunctionArgumentDescriptor optional_args{"xi", &isNumber, nullptr, "Float* or (U)Int*"};
  ...
  ColumnPtr executeImpl(const ColumnsWithTypeAndName &arguments, const DataTypePtr &, size_t /*input_rows_count*/) const override {
    if (arguments.empty()) throw Exception(ErrorCodes::BAD_ARGUMENTS, "Function {} requires at least one argument", getName());

    const auto *model = arguments[0].column.get();

    if (const auto *column_with_states = typeid_cast<const ColumnConst *>(model)) model = column_with_states->getDataColumnPtr().get();
...
Applies a trained machine learning model to input features to generate predictions.
  ...
  FunctionDocumentation::Arguments arguments = {{"model", "The trained machine learning model.", {"AggregateFunctionState"}},
  ...
  FunctionDocumentation::Examples examples = {{"Example usage",
                                               R"(
SELECT
