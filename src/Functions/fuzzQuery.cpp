...
class FunctionFuzzQuery : public IFunction {
 public:
  ...
  ColumnPtr executeImpl(const ColumnsWithTypeAndName &arguments, const DataTypePtr &, size_t input_rows_count) const override {
    ...
    if (const ColumnString *col_query_string = checkAndGetColumn<ColumnString>(col_query.get())) {
      ...
      fuzzVector(col_query_string->getChars(), col_query_string->getOffsets(), *col_res, input_rows_count);
      return col_res;
    }
 ...
 private:
  void fuzzVector(const ColumnString::Chars &data, const ColumnString::Offsets &offsets, ColumnString &col_res,
...
REGISTER_FUNCTION(fuzzQuery) {
  factory.registerFunction(
      "fuzzQuery", [](ContextPtr context) { return std::make_shared<FunctionFuzzQuery>(context); },
      FunctionDocumentation{
          .description =
              "Parses the given query string and applies random AST mutations (fuzzing) to it. Returns the fuzzed query as a string. "
          ...
          .examples{{"basic", "SET allow_fuzz_query_functions = 1; SELECT fuzzQuery('SELECT 1');", ""}},
          .introduced_in = {26, 2},
