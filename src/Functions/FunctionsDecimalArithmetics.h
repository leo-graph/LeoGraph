...
struct Processor {
  ...
  void NO_INLINE vectorConstant(const FirstArgVectorType &vec_first, const SecondArgType second_value,
                                PaddedPODArray<typename ResultType::FieldType> &vec_to, UInt16 scale_a, UInt16 scale_b, UInt16 result_scale,
                                size_t input_rows_count) const {
    vec_to.resize(input_rows_count);

    for (size_t i = 0; i < input_rows_count; ++i) vec_to[i] = transform.execute(vec_first[i], second_value, scale_a, scale_b, result_scale);
  }

  template <typename FirstArgVectorType, typename SecondArgVectorType>
  void NO_INLINE vectorVector(const FirstArgVectorType &vec_first, const SecondArgVectorType &vec_second,
                              PaddedPODArray<typename ResultType::FieldType> &vec_to, UInt16 scale_a, UInt16 scale_b, UInt16 result_scale,
                              size_t input_rows_count) const {
    vec_to.resize(input_rows_count);
  ...
  void NO_INLINE constantVector(const FirstArgType &first_value, const SecondArgVectorType &vec_second,
                                PaddedPODArray<typename ResultType::FieldType> &vec_to, UInt16 scale_a, UInt16 scale_b, UInt16 result_scale,
                                size_t input_rows_count) const {
    vec_to.resize(input_rows_count);

    for (size_t i = 0; i < input_rows_count; ++i) vec_to[i] = transform.execute(first_value, vec_second[i], scale_a, scale_b, result_scale);
  }
};
...
struct DecimalArithmeticsImpl {
  static ColumnPtr execute(Transform transform, const ColumnsWithTypeAndName &arguments, const DataTypePtr &result_type,
    ...
    if (first_col) {
      if (second_col_const)
        op.vectorConstant(first_col->getData(), second_col_const->template getValue<SecondArgValueType>(), col_to->getData(), scale_a,
                          scale_b, result_scale, input_rows_count);
      else
      ...
      op.constantVector(first_col_const->template getValue<FirstArgValueType>(), second_col->getData(), col_to->getData(), scale_a, scale_b,
                        result_scale, input_rows_count);
    } else {
...
class FunctionsDecimalArithmetics : public IFunction {
 public:
  ...
  bool isSuitableForShortCircuitArgumentsExecution(const DataTypesWithConstInfo & /*arguments*/) const override {
    return Transform::suitable_for_short_circuit;
  }

  DataTypePtr getReturnTypeImpl(const ColumnsWithTypeAndName &arguments) const override {
    if (arguments.size() != 2 && arguments.size() != 3)
      throw Exception(ErrorCodes::NUMBER_OF_ARGUMENTS_DOESNT_MATCH, "Number of arguments for function {} does not match: 2 or 3 expected",
                      getName());
    ...
    auto scale = static_cast<UInt8>(std::max(getDecimalScale(*arguments[0].type->getPtr()), getDecimalScale(*arguments[1].type->getPtr())));
    ...
    **/
    if (scale > DecimalUtils::max_precision<Decimal256>)
      throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT,
                      "Illegal value of third argument of function {}: "
                      "must be integer in range [0, 76]",
                      this->getName());
  ...
  ColumnPtr resolveOverload(const ColumnsWithTypeAndName &arguments, const DataTypePtr &result_type, size_t input_rows_count) const {
    ...
    if (which_dividend.isDecimal32()) {
      ...
      if (which_divisor.isDecimal32())
        return DecimalArithmeticsImpl<DividendType, DataTypeDecimal32, DataTypeDecimal256, Transform>::execute(
