...
REGISTER_FUNCTION(IsDistinctFrom) {
  ...
  FunctionDocumentation::Arguments arguments = {{"x", "First value to compare. Can be any ClickHouse data type.", {"Any"}},
  ...
  FunctionDocumentation::Examples examples = {{"Basic usage with numbers and NULLs", R"(
SELECT
...
template <>
ColumnPtr FunctionComparison<NotEqualsOp, NameNotEquals, true /* is null safe cmp*/>::executeTupleImpl(const ColumnsWithTypeAndName& x,
  ...
  return executeTupleEqualityImpl(func_builder_not_equals, func_builder_and, x, y, tuple_size, input_rows_count);
}
