...
class Now64OverloadResolver : public IFunctionOverloadResolver {
 public:
    ...
    if (!arguments.empty()) {
      ...
      if (!isInteger(argument.type) || !argument.column || !isColumnConst(*argument.column))
        throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT,
                        "Illegal type {} of 0 argument of function {}. "
                        "Expected const integer.",
                        argument.type->getName(), getName());
  ...
  FunctionBasePtr buildImpl(const ColumnsWithTypeAndName &arguments, const DataTypePtr &result_type) const override {
    ...
    if (const auto *type = typeid_cast<const DataTypeDateTime64 *>(res_type.get())) scale = type->getScale();

    DataTypes arg_types;
    arg_types.reserve(arguments.size());
    for (const auto &arg : arguments) arg_types.push_back(arg.type);
...
now64([scale[, timezone]])
  ...
  FunctionDocumentation::Examples examples = {{"Query with default and custom precision", R"(
