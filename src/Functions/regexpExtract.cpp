...
class FunctionRegexpExtract : public IFunction {
 public:
  ...
  DataTypePtr getReturnTypeImpl(const ColumnsWithTypeAndName &arguments) const override {
    if (arguments.size() != 2 && arguments.size() != 3)
      throw Exception(ErrorCodes::NUMBER_OF_ARGUMENTS_DOESNT_MATCH, "Number of arguments for function {} doesn't match: passed {}",
    ...
    if (arguments.size() == 3)
      args.emplace_back(
          FunctionArgumentDescriptor{"index", static_cast<FunctionArgumentDescriptor::TypeValidator>(&isInteger), nullptr, "Integer"});
  ...
  ColumnPtr executeImpl(const ColumnsWithTypeAndName &arguments, const DataTypePtr &, size_t /*input_rows_count*/) const override {
    ...
    if (!col_pattern) throw Exception(ErrorCodes::ILLEGAL_COLUMN, "Second argument of function {} must be constant string", getName());
 ...
 private:
  static void saveMatch(const OptimizedRegularExpression::MatchVec &matches, size_t match_index, const ColumnString::Chars &data,
                        size_t data_offset, ColumnString::Chars &res_data, ColumnString::Offsets &res_offsets, size_t &res_offset) {
  ...
  void vectorConstant(const ColumnString::Chars &data, const ColumnString::Offsets &offsets, const std::string &pattern, ssize_t index,
                      ColumnString::Chars &res_data, ColumnString::Offsets &res_offsets) const {
    ...
    if (index < 0 || index >= capture + 1)
      throw Exception(ErrorCodes::INDEX_OF_POSITIONAL_ARGUMENT_IS_OUT_OF_RANGE,
                      "Index value {} for regexp pattern `{}` in function {} is out-of-range, should be in [0, {})", index, pattern,
                      getName(), capture + 1);
    ...
    for (size_t cur_offset : offsets) {
      regexp.match(reinterpret_cast<const char *>(&data[prev_offset]), cur_offset - prev_offset, matches, static_cast<unsigned>(index + 1));
  ...
  void vectorVector(const ColumnString::Chars &data, const ColumnString::Offsets &offsets, const std::string &pattern,
                    const ColumnPtr &column_index, ColumnString::Chars &res_data, ColumnString::Offsets &res_offsets) const {
    res_data.reserve(data.size() / 5);
    res_offsets.reserve(offsets.size());
  ...
  void constantVector(const std::string &str, const std::string &pattern, const ColumnPtr &column_index, ColumnString::Chars &res_data,
...
Extracts the first string in `haystack` that matches the regexp pattern and corresponds to the regex group index.
  ...
  FunctionDocumentation::Arguments arguments = {
      ...
      {"pattern",
       "String, regexp expression. `pattern` may contain multiple regexp groups, `index` indicates which regex group to extract. An index "
  ...
  FunctionDocumentation::ReturnedValue returned_value = {"Returns a string match", {"String"}};
