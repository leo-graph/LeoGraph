...
This function accepts any numeric type as input, but internally it casts them to `Float64`. Results might be suboptimal with large values.
  ...
  FunctionDocumentation::Arguments arguments = {{"x", "Size in bytes.", {"UInt64"}}};
  FunctionDocumentation::ReturnedValue returned_value = {"Returns a readable, rounded size with suffix as a string.", {"String"}};
  FunctionDocumentation::Examples examples = {{"Format file sizes",
                                               R"(
SELECT
...
└────────────────┴────────────┘
  ...
  factory.registerFunction(
      "formatReadableSize",
      [](ContextPtr) { return FunctionFormatReadable::create("formatReadableSize", formatReadableSizeWithBinarySuffix); }, documentation);
