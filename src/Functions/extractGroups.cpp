...
class FunctionExtractGroups : public IFunction {
 public:
  ...
  ColumnPtr executeImpl(const ColumnsWithTypeAndName &arguments, const DataTypePtr &, size_t input_rows_count) const override {
    ...
    if (needle.empty()) throw Exception(ErrorCodes::BAD_ARGUMENTS, "{} length of 'needle' argument must be greater than 0.", getName());

    const OptimizedRegularExpression regexp = Regexps::createRegexp<false, false, false>(needle);
    const auto &re2 = regexp.getRE2();

    if (!re2) throw Exception(ErrorCodes::BAD_ARGUMENTS, "There are no groups in regexp: {}", needle);
    ...
    for (size_t i = 0; i < input_rows_count; ++i) {
      ...
      if (re2->Match({current_row.data(), current_row.size()}, 0, current_row.size(), re2::RE2::UNANCHORED, matched_groups.data(),
...
Extracts all groups from non-overlapping substrings matched by a regular expression.
  ...
  FunctionDocumentation::Arguments arguments = {{"s", "Input string to extract from.", {"String", "FixedString"}},
                                                {"regexp", "Regular expression. Constant.", {"const String", "const FixedString"}}};
  FunctionDocumentation::ReturnedValue returned_value = {
      "If the function finds at least one matching group, it returns Array(Array(String)) column, clustered by group_id (`1` to `N`, where "
      "`N` is number of capturing groups in regexp). If there is no matching group, it returns an empty array.",
      {"Array(Array(String))"}};
