...
class ExecutableFunctionExpression : public IExecutableFunction {
 public:
  ...
  ColumnPtr executeImpl(const ColumnsWithTypeAndName &arguments, const DataTypePtr &result_type, size_t input_rows_count) const override {
    ...
    if (!expression_actions) throw Exception(ErrorCodes::LOGICAL_ERROR, "No actions were passed to FunctionExpression");
 ...
 public:
  ...
  ColumnPtr executeImpl(const ColumnsWithTypeAndName &arguments, const DataTypePtr &, size_t input_rows_count) const override {
    ...
    bool constant_folding = capture->allow_constant_folding &&
                            std::all_of(arguments.begin(), arguments.end(), [](const auto &arg) { return arg.column->isConst(); });

    if (constant_folding) {
      ColumnsWithTypeAndName arguments_resized = arguments;
      for (auto &elem : arguments_resized) elem.column = elem.column->cloneResized(1);
...
class FunctionCapture : public IFunctionBase {
 public:
  FunctionCapture(ExpressionActionsPtr expression_actions_, LambdaCapturePtr capture_, DataTypePtr return_type_, String name_)
...
class FunctionCaptureOverloadResolver : public IFunctionOverloadResolver {
 public:
  FunctionCaptureOverloadResolver(ActionsDAG actions_dag, const ExpressionActionsSettings &actions_settings, const Names &captured_names,
                                  const NamesAndTypesList &lambda_arguments, const DataTypePtr &function_return_type,
                                  const String &expression_return_name, bool allow_constant_folding) {
    ...
    for (const auto *input : actions_dag.getInputs()) arguments_map[input->result_name] = input->result_type;
    ...
    for (const auto &lambda_argument : lambda_arguments) argument_types.push_back(lambda_argument.type);

    return_type = std::make_shared<DataTypeFunction>(argument_types, function_return_type);

    name = "Capture[" + toString(captured_types) + "](" + toString(argument_types) + ") -> " + function_return_type->getName();
