...
bool isStringOrNull(const IDataType& type) { return isString(type) || type.onlyNull(); }
...
class FunctionFile : public IFunction {
 public:
  ...
  DataTypePtr getReturnTypeImpl(const ColumnsWithTypeAndName& arguments) const override {
    FunctionArgumentDescriptors mandatory_args{{"path", &isString, nullptr, "String"}};
    FunctionArgumentDescriptors optional_args{{"default", &isStringOrNull, nullptr, "String or Null"}};

    validateFunctionArguments(*this, arguments, mandatory_args, optional_args);

    auto ret = std::make_shared<DataTypeString>();
    if (arguments.size() == 2 && arguments[1].type->onlyNull()) return makeNullable(ret);
    return ret;
  }
  ...
  ColumnPtr executeImpl(const ColumnsWithTypeAndName& arguments, const DataTypePtr& result_type, size_t input_rows_count) const override {
    ...
    for (size_t row = 0; row < input_rows_count; ++row) {
      ...
      if (file_path.is_relative()) file_path = user_files_absolute_path / file_path;
    ...
    if (vec_null_map_to) return ColumnNullable::create(std::move(result), std::move(col_null_map_to));
...
Also see the [`file`](../table-functions/file.md) table function.
  ...
  FunctionDocumentation::Arguments arguments = {
      {"path",
       "The path of the file relative to the `user_files_path`. Supports wildcards `*`, `**`, `?`, `{abc,def}` and `{N..M}` where `N`, `M` "
       "are numbers and `'abc', 'def'` are strings.",
       {"String"}},
  ...
  FunctionDocumentation::Examples examples = {{"Insert files into a table",
                                               R"(
