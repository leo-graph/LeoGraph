 ...
 *
 * Where R could be of arbitrary type, in case of (3) if R is DecimalUtils::DecimalComponents<DateTime64>, result is re-assembed back into
 * DateTime64.
 */
 ...
 public:
  ...
  auto NO_SANITIZE_UNDEFINED execute(const DateTime64 &t, Args &&...args) const {
    ...
    if constexpr (TransformHasExecuteOverload_v<DateTime64, decltype(scale_multiplier), Args...>) {
      ...
      auto components = DecimalUtils::splitWithScaleMultiplier(t, scale_multiplier);
      if (t.value < 0 && components.fractional) --components.whole;
