...
class ConcatReadBufferFromFile final : public ReadBufferFromFileBase {
 public:
  ...
  ConcatReadBufferFromFile(std::string file_name_, std::unique_ptr<SeekableReadBuffer> buf1, size_t size1,
                           std::unique_ptr<SeekableReadBuffer> buf2, size_t size2);

  ConcatReadBufferFromFile(std::string file_name_, SeekableReadBuffer& buf1, size_t size1, SeekableReadBuffer& buf2, size_t size2);
