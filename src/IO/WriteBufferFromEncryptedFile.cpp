...
WriteBufferFromEncryptedFile::WriteBufferFromEncryptedFile(size_t buffer_size_, std::unique_ptr<WriteBufferFromFileBase> out_,
                                                           const String& key_, const FileEncryption::Header& header_, size_t old_file_size,
                                                           bool use_adaptive_buffer_size_, size_t adaptive_buffer_initial_size)
    : WriteBufferDecorator<WriteBufferFromFileBase>(std::move(out_),
                                                    use_adaptive_buffer_size_ ? adaptive_buffer_initial_size : buffer_size_, nullptr, 0),
      header(header_),
...
WriteBufferFromEncryptedFile::~WriteBufferFromEncryptedFile() {
  ...
  if (!finalized) LOG_INFO(log, "WriteBufferFromEncryptedFile is not finalized in destructor");
}
