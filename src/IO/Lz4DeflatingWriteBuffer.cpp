...
class SinkToOut {
 public:
  SinkToOut(WriteBuffer* out_, Memory<>& mem_, size_t guaranteed_capacity) : sink(out_), tmp_out(mem_), cur_out(sink) {
    chassert(sink);
  ...
  size_t getCapacity() { return cur_out->available(); }

  BufferBase::Position getPosition() { return cur_out->position(); }
...
void Lz4DeflatingWriteBuffer::initialize(int compression_level) {
  ...
  if (LZ4F_isError(ret))
    throw Exception(ErrorCodes::LZ4_ENCODER_FAILED, "creation of LZ4 compression context failed. LZ4F version: {}, error: {}", LZ4F_VERSION,
...
void Lz4DeflatingWriteBuffer::nextImpl() {
  ...
  if (first_time) {
    ...
    size_t header_size = LZ4F_compressBegin(ctx, sink.getPosition(), sink.getCapacity(), &kPrefs);

    if (LZ4F_isError(header_size))
      throw Exception(ErrorCodes::LZ4_ENCODER_FAILED, "LZ4 failed to start stream encoding. LZ4F version: {}, error: {}", LZ4F_VERSION,
  ...
  while (in_capacity > 0) {
    ...
    {
      while (sink.getCapacity() < LZ4F_compressBound(cur_buffer_size, &kPrefs)) cur_buffer_size /= 2;
    }

    size_t compressed_size = LZ4F_compressUpdate(ctx, sink.getPosition(), sink.getCapacity(), in_data, cur_buffer_size, nullptr);

    if (LZ4F_isError(compressed_size))
      throw Exception(ErrorCodes::LZ4_ENCODER_FAILED, "LZ4 failed to encode stream. LZ4F version: {}, error {}, out_capacity {}",
...
void Lz4DeflatingWriteBuffer::finalFlushBefore() {
  ...
  if (!compress_empty && first_time) return;
  ...
  if (LZ4F_isError(end_size))
    throw Exception(ErrorCodes::LZ4_ENCODER_FAILED, "LZ4 failed to end stream encoding. LZ4F version: {}, error {}, out_capacity {}",
