...
class BitReader {
 ...
 public:
  ...
  ALWAYS_INLINE UInt64 readBits(UInt8 bits_to_read) {
    if (bits_to_read > bits_count) fillBitBuffer();
  ...
  ALWAYS_INLINE UInt8 readBit() { return static_cast<UInt8>(readBits(1)); }
  ...
  bool eof() const { return bits_count == 0 && source_current >= source_end; }

  // number of bits that was already read by clients with readBits()
  UInt64 count() const { return (source_current - source_begin) * 8 - bits_count; }

  UInt64 remaining() const { return (source_end - source_current) * 8 + bits_count; }
 ...
 private:
  ...
  ALWAYS_INLINE size_t fillBitBuffer() {
    ...
    if constexpr (std::endian::native == std::endian::little) tmp_buffer = std::byteswap(tmp_buffer);
...
class BitWriter {
 ...
 public:
  ...
  void flush() {
    ...
    while (bits_count != 0) doFlush();
  }

  UInt64 count() const { return (dest_current - dest_begin) * 8 + bits_count; }
  ...
  void doFlush() {
    ...
    if (available < to_write) {
      throw Exception(ErrorCodes::CANNOT_WRITE_AFTER_END_OF_BUFFER,
                      "Can not write past end of buffer. Space available is {} bytes, required to write {} bytes.", available, to_write);
    }
