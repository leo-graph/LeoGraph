...
ZstdInflatingReadBuffer::~ZstdInflatingReadBuffer() { ZSTD_freeDCtx(dctx); }
...
bool ZstdInflatingReadBuffer::nextImpl() {
  do {
    ...
    if (ZSTD_getErrorCode(ret)) {
      throw Exception(
          ErrorCodes::ZSTD_DECODER_FAILED, "ZSTD stream decoding failed: error '{}'{}; ZSTD version: {}{}", ZSTD_getErrorName(ret),
          ZSTD_error_frameParameter_windowTooLarge == ret ? ". You can increase the maximum window size with the 'zstd_window_log_max' "
                                                            "setting in ClickHouse. Example: 'SET zstd_window_log_max = 31'"
                                                          : "",
          ZSTD_VERSION_STRING, getExceptionEntryWithFileName(*in));
    }
