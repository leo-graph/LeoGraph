...
std::string AsynchronousReadBufferFromFileDescriptor::getFileName() const { return "(fd = " + toString(fd) + ")"; }
...
void AsynchronousReadBufferFromFileDescriptor::appendToPrefetchLog(FilesystemPrefetchState state, int64_t size,
...
bool AsynchronousReadBufferFromFileDescriptor::nextImpl() {
  ...
  if (prefetch_future.valid()) {
    ...
    prefetch_future = {};
    if (result.size - result.offset > 0) prefetch_buffer.swap(memory);

    if (prefetches_log) appendToPrefetchLog(FilesystemPrefetchState::USED, result.size, result.execution_watch);
  } else {
  ...
  if (throttler) throttler->throttle(result.size);
...
AsynchronousReadBufferFromFileDescriptor::AsynchronousReadBufferFromFileDescriptor(
    IAsynchronousReader& reader_, Priority priority_, int fd_, size_t buf_size, int flags, char* existing_memory, size_t alignment,
    std::optional<size_t> file_size_, ThrottlerPtr throttler_, FilesystemReadPrefetchesLogPtr prefetches_log_)
  ...
  if (required_alignment > buf_size)
    throw Exception(ErrorCodes::LOGICAL_ERROR,
                    "Too large alignment. Cannot have required_alignment greater than buf_size: {} > {}. It is a bug", required_alignment,
                    buf_size);
...
AsynchronousReadBufferFromFileDescriptor::~AsynchronousReadBufferFromFileDescriptor() { finalize(); }
...
off_t AsynchronousReadBufferFromFileDescriptor::seek(off_t offset, int whence) {
  ...
  while (true) {
    ...
    if (prefetch_future.valid()) {
      ...
      if (nextImpl()) continue;
    }

    /// Prefetch is cancelled because of seek.
    if (read_from_prefetch) {
      if (prefetches_log) appendToPrefetchLog(FilesystemPrefetchState::CANCELLED_WITH_SEEK, -1, nullptr);
    }
  ...
  if (bytes_to_ignore >= internal_buffer.size())
    throw Exception(ErrorCodes::LOGICAL_ERROR,
                    ...
                    bytes_to_ignore, internal_buffer.size());
...
std::optional<size_t> AsynchronousReadBufferFromFileDescriptor::tryGetFileSize() { return getSizeFromFileDescriptor(fd, getFileName()); }
