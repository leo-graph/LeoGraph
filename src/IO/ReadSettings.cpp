...
ReadSettings getReadSettings() {
  ...
  if (query_context) return query_context->getReadSettings();

  auto global_context = Context::getGlobalContextInstance();
  if (global_context) return global_context->getReadSettings();
...
ReadSettings ReadSettings::withNestedBuffer(bool seekable) const {
  ReadSettings res = *this;
  if (!seekable) res.remote_read_buffer_restrict_seek = true;
