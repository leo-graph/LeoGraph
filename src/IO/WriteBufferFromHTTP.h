...
class WriteBufferFromHTTP : public WriteBufferFromOStream {
  ...
  explicit WriteBufferFromHTTP(const HTTPConnectionGroupType& connection_group, const Poco::URI& uri,
                               ...
                               size_t buffer_size_ = DBMS_DEFAULT_BUFFER_SIZE, ProxyConfiguration proxy_configuration = {});
...
#define setterMember(name, member)                                                              \
  ...
  setterMember(withConnectionGroup, connection_group) setterMember(withMethod, method) setterMember(withBypassProxy, bypass_proxy)
      setterMember(withContentType, content_type) setterMember(withContentEncoding, content_encoding)
          setterMember(withAdditionalHeaders, additional_headers) setterMember(withTimeouts, timeouts)
