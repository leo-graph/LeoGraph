...
size_t WithFileSize::getFileSize() {
  if (auto maybe_size = tryGetFileSize()) return *maybe_size;
...
static std::optional<size_t> tryGetFileSize(T &in) {
  if (auto *with_file_size = dynamic_cast<WithFileSize *>(&in)) return with_file_size->tryGetFileSize();
...
static size_t getFileSize(T &in) {
  if (auto maybe_size = tryGetFileSize(in)) return *maybe_size;

  throw Exception(ErrorCodes::UNKNOWN_FILE_SIZE, "Cannot find out file size");
}

std::optional<size_t> tryGetFileSizeFromReadBuffer(ReadBuffer &in) {
  if (auto *delegate = dynamic_cast<ReadBufferFromFileDecorator *>(&in)) return tryGetFileSize(delegate->getWrappedReadBuffer());
  if (auto *compressed = dynamic_cast<CompressedReadBufferWrapper *>(&in)) return tryGetFileSize(compressed->getWrappedReadBuffer());
  return tryGetFileSize(in);
}

size_t getFileSizeFromReadBuffer(ReadBuffer &in) {
  if (auto maybe_size = tryGetFileSizeFromReadBuffer(in)) return *maybe_size;
