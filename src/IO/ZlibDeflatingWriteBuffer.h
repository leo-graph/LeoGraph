...
class ZlibDeflatingWriteBuffer : public WriteBufferWithOwnMemoryDecorator {
 public:
  ...
  ZlibDeflatingWriteBuffer(WriteBufferT&& out_, CompressionMethod compression_method, int compression_level,
        ...
        compress_empty(compress_empty_)  /// NOLINT(bugprone-move-forwarding-reference)
  {
    ...
    if (rc != Z_OK) throw Exception(ErrorCodes::ZLIB_DEFLATE_FAILED, "deflateInit2 failed: {}; zlib version: {}", zError(rc), ZLIB_VERSION);
  }
