...
ReadBufferFromFileView::ReadBufferFromFileView(std::unique_ptr<ReadBufferFromFileBase> impl_, const String& file_name_, off_t left_bound_,
                                               off_t right_bound_)
    : impl(std::move(impl_)),
...
void ReadBufferFromFileView::setReadUntilPosition(size_t position) {
  ...
  if (*read_until_position > right_bound)
    throw Exception(ErrorCodes::ARGUMENT_OUT_OF_BOUND, "Cannot read until position: {}. File size is {}", position, getFileSize());
...
off_t ReadBufferFromFileView::getPosition() { return (file_offset_of_buffer_end - left_bound) - (working_buffer.end() - pos); }

bool ReadBufferFromFileView::nextImpl() {
  size_t current_position = file_offset_of_buffer_end - (working_buffer.end() - pos);
  if (current_position == getRightBound()) return false;
...
off_t ReadBufferFromFileView::seek(off_t off, int whence) {
  ...
  if (result < 0) throw Exception(ErrorCodes::SEEK_POSITION_OUT_OF_BOUND, "Seek position ({}) underflow", result);

  if (static_cast<size_t>(result) < left_bound || static_cast<size_t>(result) > right_bound)
    throw Exception(ErrorCodes::SEEK_POSITION_OUT_OF_BOUND, "Seek position ({}) is out of bound. Available range: [{}, {}]", result,
...
size_t ReadBufferFromFileView::getRightBound() const { return read_until_position ? *read_until_position : right_bound; }
