...
void ZlibDeflatingWriteBuffer::nextImpl() {
  ...
  try {
    do {
      ...
      if (rc != Z_OK) throw Exception(ErrorCodes::ZLIB_DEFLATE_FAILED, "deflate failed: {}", zError(rc));
...
void ZlibDeflatingWriteBuffer::finalFlushBefore() {
  ...
  if (!compress_empty && zstr.total_out == 0) return;
  ...
  while (true) {
    ...
    if (rc != Z_OK) throw Exception(ErrorCodes::ZLIB_DEFLATE_FAILED, "deflate finalizeImpl() failed: {}", zError(rc));
  }
}
...
void ZlibDeflatingWriteBuffer::finalFlushAfter() {
  try {
    ...
    if (rc != Z_OK) throw Exception(ErrorCodes::ZLIB_DEFLATE_FAILED, "deflateEnd failed: {}", zError(rc));
  } catch (...) {
