...
class AsynchronousReadBufferFromFileWithDescriptorsCache : public AsynchronousReadBufferFromFileDescriptor {
 ...
 public:
  AsynchronousReadBufferFromFileWithDescriptorsCache(IAsynchronousReader& reader_, Priority priority_, const std::string& file_name_,
                                                     size_t buf_size = DBMS_DEFAULT_BUFFER_SIZE, int flags = -1,
      ...
      : AsynchronousReadBufferFromFileDescriptor(reader_, priority_, -1, buf_size, flags, existing_memory, alignment, file_size_,
                                                 throttler_, prefetches_log_),
