...
class SeekableReadBuffer : public ReadBuffer {
 public:
  SeekableReadBuffer(Position ptr, size_t size) : ReadBuffer(ptr, size) {}
  SeekableReadBuffer(Position ptr, size_t size, size_t offset) : ReadBuffer(ptr, size, offset) {}
  ...
  /// For tables that have an external storage (like S3) as their main storage we'd like to distinguish whether we're reading from this
  /// storage or from a local cache. It allows to reuse all the optimisations done for reading from local tables when reading from cache.
...
void copyFromIStreamWithProgressCallback(std::istream& istr, char* to, size_t n, const std::function<bool(size_t)>& progress_callback,
                                         size_t* out_bytes_copied, bool* out_cancelled = nullptr);
