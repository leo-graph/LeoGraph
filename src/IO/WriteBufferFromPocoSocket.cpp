...
void WriteBufferFromPocoSocket::socketSendBytes(const char *ptr, size_t size) {
  ...
  SCOPE_EXIT({
    ...
    if (write_event != ProfileEvents::end()) ProfileEvents::increment(write_event, bytes_written);
  });
  ...
  while (bytes_written < size) {
    ...
    try {
      ...
      if (size > INT_MAX) throw Exception(ErrorCodes::LOGICAL_ERROR, "Buffer overflow");

      res = socketSendBytesImpl(ptr + bytes_written, size - bytes_written);
    } catch (const Poco::Net::NetException &e) {
      throw NetException(ErrorCodes::NETWORK_ERROR, "{}, while writing to socket ({} -> {})", e.displayText(), our_address.toString(),
                         peer_address.toString());
    } catch (const Poco::TimeoutException &) {
      throw NetException(ErrorCodes::SOCKET_TIMEOUT, "Timeout exceeded while writing to socket ({}, {} ms)", peer_address.toString(),
    ...
    if (res < 0)
      throw NetException(ErrorCodes::CANNOT_WRITE_TO_SOCKET, "Cannot write to socket ({} -> {})", our_address.toString(),
                         peer_address.toString());
