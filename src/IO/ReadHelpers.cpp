...
inline void parseHex(IteratorSrc src, IteratorDst dst) {
  size_t src_pos = 0;
  size_t dst_pos = 0;
  for (; dst_pos < num_bytes; ++dst_pos, src_pos += 2) dst[dst_pos] = unhex2(reinterpret_cast<const char *>(&src[src_pos]));
}
...
bool checkString(const char *s, ReadBuffer &buf) {
  for (; *s; ++s) {
    if (buf.eof() || *buf.position() != *s) return false;
    ++buf.position();
  }
  return true;
}
...
bool checkStringCaseInsensitive(const char *s, ReadBuffer &buf) {
  for (; *s; ++s) {
    ...
    char c = *buf.position();
    if (!equalsCaseInsensitive(*s, c)) return false;
...
void assertString(const char *s, ReadBuffer &buf) {
  if (!checkString(s, buf)) throwAtAssertionFailed(s, buf);
}

void assertEOF(ReadBuffer &buf) {
  if (!buf.eof()) throwAtAssertionFailed("eof", buf);
}

void assertNotEOF(ReadBuffer &buf) {
  if (buf.eof()) throw Exception(ErrorCodes::ATTEMPT_TO_READ_AFTER_EOF, "Attempt to read after EOF");
}

void assertStringCaseInsensitive(const char *s, ReadBuffer &buf) {
  if (!checkStringCaseInsensitive(s, buf)) throwAtAssertionFailed(s, buf);
}
...
void skipStringUntilWhitespace(ReadBuffer &buf) { skipStringUntilChars<' '>(buf); }
...
void readString(String &s, ReadBuffer &buf, size_t n) {
  s.resize(n);
  if (n) s.resize(buf.read(s.data(), n));
}
...
void readEscapedStringUntilEOLInto(Vector &s, ReadBuffer &buf) {
  while (!buf.eof()) {
    ...
    if (!buf.hasPendingData()) continue;

    if (*buf.position() == '\n') return;

    if (*buf.position() == '\\') parseComplexEscapeSequence(s, buf);
  }
}
...
static ReturnType parseComplexEscapeSequence(Vector &s, ReadBuffer &buf) {
  ...
  auto error = [](const char *message [[maybe_unused]], int code [[maybe_unused]]) {
    if constexpr (throw_exception) throw Exception::createDeprecated(message, code);
  ...
  if (char_after_backslash == 'x') {
    ...
    if (decoded_char != '\\' && decoded_char != '\'' && decoded_char != '"' && decoded_char != '`'  /// MySQL style identifiers
...
bool parseComplexEscapeSequence(String &s, ReadBuffer &buf) { return parseComplexEscapeSequence<String, bool>(s, buf); }
...
static ReturnType parseJSONEscapeSequence(Vector &s, ReadBuffer &buf, bool keep_bad_sequences) {
  ...
  if (buf.eof()) {
    if (keep_bad_sequences) return ReturnType(true);
      ...
      if (keep_bad_sequences) {
        for (size_t i = 0; i != 4; ++i) {
          if (buf.eof() || *buf.position() == '"') {
            ...
            s.push_back('\\');
            s.push_back('u');
            for (size_t j = 0; j != i; ++j) s.push_back(hex_code[j]);
            return ReturnType(true);
          }
      ...
      if (code_point <= 0x7F) {
        ...
        if (code_point >= 0xD800 && code_point <= 0xDBFF) {
          auto restore_first_unicode = [&]() {
            s.push_back('\\');
            s.push_back('u');
            for (char &c : hex_code) s.push_back(c);
          };
          ...
          if (keep_bad_sequences) {
            for (size_t i = 0; i != 4; ++i) {
              if (buf.eof() || *buf.position() == '"') {
                ...
                s.push_back('\\');
                s.push_back('u');
                for (size_t j = 0; j != i; ++j) s.push_back(second_hex_code[j]);
                return ReturnType(true);
              }
          ...
          if (second_code_point >= 0xDC00 && second_code_point <= 0xDFFF) {
            ...
            s.push_back('\\');
            s.push_back('u');
            for (char &c : second_hex_code) s.push_back(c);
            return ReturnType(true);
          }
        } else {
...
static ReturnType readAnyQuotedStringInto(Vector &s, ReadBuffer &buf) {
  ...
  if (buf.eof() || *buf.position() != quote) {
    if constexpr (throw_exception)
      throw Exception(ErrorCodes::CANNOT_PARSE_QUOTED_STRING, "Cannot parse quoted string: expected opening quote '{}', got '{}'",
  ...
  while (!buf.eof()) {
    ...
    if (*buf.position() == '\\') {
      if constexpr (throw_exception)
        ...
        if (!parseComplexEscapeSequence<Vector, ReturnType>(s, buf)) return ReturnType(false);
      }
    }
  }
...
void readCSVStringInto(Vector &s, ReadBuffer &buf, const FormatSettings::CSV &settings) {
  ...
  if (custom_delimiter.empty() && maybe_quote == delimiter) return;

  if ((settings.allow_single_quotes && maybe_quote == '\'') || (settings.allow_double_quotes && maybe_quote == '"')) {
    if constexpr (include_quotes) s.push_back(maybe_quote);
    ...
    while (!buf.eof()) {
      ...
      if (nullptr == next_pos) next_pos = buf.buffer().end();
    ...
    if (!custom_delimiter.empty()) {
      ...
      if (!peekable_buf) {
        if constexpr (allow_throw)
          throw Exception(ErrorCodes::LOGICAL_ERROR,
                          "Reading CSV string with custom delimiter is allowed only when using PeekableReadBuffer");
        return;
      }
      ...
      while (true) {
        if (peekable_buf->eof()) {
          if constexpr (allow_throw)
            throw Exception(ErrorCodes::INCORRECT_DATA, "Unexpected EOF while reading CSV string, expected custom delimiter \"{}\"",
                            custom_delimiter);
          return;
        }

        char *next_pos = reinterpret_cast<char *>(memchr(peekable_buf->position(), custom_delimiter[0], peekable_buf->available()));
        if (!next_pos) next_pos = peekable_buf->buffer().end();
...

        {
          ...
          if (checkString(custom_delimiter, *peekable_buf)) return;
        }
    ...
    while (!buf.eof()) {
      ...
      if constexpr (WithResize<Vector>) {
        if (settings.trim_whitespaces) [[likely]] {
           ...
           */
          size_t size = s.size();
          while (size > 0 && (s[size - 1] == ' ' || s[size - 1] == '\t')) --size;
...
void readCSVWithTwoPossibleDelimitersImpl(String &s, PeekableReadBuffer &buf, const String &first_delimiter,
  ...
  while (true) {
    ...
    char *next_pos = buf.position();
    while (next_pos != buf.buffer().end() && *next_pos != first_delimiter[0] && *next_pos != second_delimiter[0]) ++next_pos;
    ...
    if (*buf.position() == first_delimiter[0]) {
      ...
      if (checkString(first_delimiter, buf)) return;
    }

    if (*buf.position() == second_delimiter[0]) {
      PeekableReadBufferCheckpoint checkpoint(buf, true);
      if (checkString(second_delimiter, buf)) return;
    }
...
String readCSVStringWithTwoPossibleDelimiters(PeekableReadBuffer &buf, const FormatSettings::CSV &settings, const String &first_delimiter,
                                              const String &second_delimiter) {
  String res;
...
String readCSVFieldWithTwoPossibleDelimiters(PeekableReadBuffer &buf, const FormatSettings::CSV &settings, const String &first_delimiter,
                                             const String &second_delimiter) {
  String res;
...
template void readCSVStringInto<PaddedPODArray<UInt8>, false, false>(PaddedPODArray<UInt8> &s, ReadBuffer &buf,
...
ReturnType readJSONStringInto(Vector &s, ReadBuffer &buf, const FormatSettings::JSON &settings) {
  ...
  auto error = [](FormatStringHelper<> message [[maybe_unused]], int code [[maybe_unused]]) {
    if constexpr (throw_exception) throw Exception(code, std::move(message));
    return ReturnType(false);
  };
  ...
  while (!buf.eof()) {
    ...
    if (*buf.position() == '\\') parseJSONEscapeSequence<Vector, ReturnType>(s, buf, !settings.throw_on_bad_escape_sequence);
  }
...
template void readJSONStringInto<PaddedPODArray<UInt8>, void>(PaddedPODArray<UInt8> &s, ReadBuffer &buf,
                                                              const FormatSettings::JSON &settings);
...
ReturnType readJSONObjectOrArrayPossiblyInvalid(Vector &s, ReadBuffer &buf) {
  ...
  while (!buf.eof()) {
    ...
    if (balance == 0) return ReturnType(true);
...
std::string_view readJSONObjectAsViewPossiblyInvalid(ReadBuffer &buf, String &object_buffer) {
  if (buf.eof() || *buf.position() != '{') throw Exception(ErrorCodes::INCORRECT_DATA, "JSON object should start with '{{'");
  ...
  while (true) {
    ...
    if (buf.eof()) throw Exception(ErrorCodes::INCORRECT_DATA, "Unexpected EOF while reading JSON object");

    char *next_pos = find_first_symbols<'\\', '{', '}', '"'>(buf.position(), buf.buffer().end());
    if (use_object_buffer) object_buffer.append(buf.position(), next_pos - buf.position());
    buf.position() = next_pos;

    if (!buf.hasPendingData()) continue;

    if (use_object_buffer) object_buffer.push_back(*buf.position());
    ...
    if (balance == 0) {
      if (use_object_buffer) return object_buffer;
...
ReturnType readDateTextFallback(LocalDate &date, ReadBuffer &buf, const char *allowed_delimiters) {
  ...
  auto error = [] {
    if constexpr (throw_exception) throw Exception(ErrorCodes::CANNOT_PARSE_DATE, "Cannot parse date: value is too short");
    return ReturnType(false);
  };
  ...
  if (!append_digit(year) || !append_digit(year)  // NOLINT
      || !append_digit(year)                      // NOLINT
      || !append_digit(year))                     // NOLINT
    return error();

  if (buf.eof()) return error();

  if (isNumericASCII(*buf.position())) {
    /// YYYYMMDD
    if (!append_digit(month) || !append_digit(month)  // NOLINT
        || !append_digit(day) || !append_digit(day))  // NOLINT
      return error();
  } else {
    if (!isSymbolIn(*buf.position(), allowed_delimiters)) return error();

    ++buf.position();

    if (!append_digit(month)) return error();
    append_digit(month);
    ...
    if (!append_digit(day)) return error();
    append_digit(day);
  }
...
NO_SANITIZE_UNDEFINED ReturnType readDateTimeTextFallback(time_t &datetime, ReadBuffer &buf, const DateLUTImpl &date_lut,
                                                          const char *allowed_date_delimiters, const char *allowed_time_delimiters,
  ...
  if (negative_multiplier == 1 && s_pos == s + 4 && !buf.eof() && !isNumericASCII(*buf.position())) {
    ...
    if constexpr (!throw_exception) {
      if (!isNumericASCII(s[0]) || !isNumericASCII(s[1]) || !isNumericASCII(s[2]) || !isNumericASCII(s[3]) || !isNumericASCII(s[5]) ||
          !isNumericASCII(s[6]) || !isNumericASCII(s[8]) || !isNumericASCII(s[9]))
        return false;

      if (!isSymbolIn(s[4], allowed_date_delimiters) || !isSymbolIn(s[7], allowed_date_delimiters)) return false;
    }
    ...
    if (!buf.eof() && (*buf.position() == ' ' || *buf.position() == 'T')) {
      ...
      if constexpr (!throw_exception) {
        if (!isNumericASCII(s[0]) || !isNumericASCII(s[1]) || !isNumericASCII(s[3]) || !isNumericASCII(s[4]) || !isNumericASCII(s[6]) ||
            !isNumericASCII(s[7]))
          return false;

        if (!isSymbolIn(s[2], allowed_time_delimiters) || !isSymbolIn(s[5], allowed_time_delimiters)) return false;
      }
    ...
    if constexpr (throw_exception) {
      ...
      if (saturate_on_overflow) {
        ...
        if (!datetime_maybe) return false;

        if constexpr (!dt64_mode) {
          if (*datetime_maybe < 0 || *datetime_maybe > static_cast<Int64>(UINT32_MAX)) return false;
        }
    ...
    if (!too_short || dt64_mode) {
      ...
      for (const char *digit_pos = s; digit_pos < s_pos; ++digit_pos) {
        if constexpr (!throw_exception) {
          if (!isNumericASCII(*digit_pos)) return false;
        }
...
NO_SANITIZE_UNDEFINED ReturnType readTimeTextFallback(time_t &time, ReadBuffer &buf, const DateLUTImpl &date_lut,
                                                      const char *allowed_date_delimiters, const char *allowed_time_delimiters) {
  static constexpr bool throw_exception = std::is_same_v<ReturnType, void>;
  ...
  if (!t64_mode && s_pos == s + 4 && !buf.eof() && !isNumericASCII(*buf.position())) {
    ...
    if (!buf.eof() && (*buf.position() == ' ' || *buf.position() == 'T')) {
      ...
      if constexpr (!throw_exception) {
        ...
        if (!isSymbolIn(s[3], allowed_time_delimiters) || !isSymbolIn(s[6], allowed_time_delimiters)) return false;
      }
...
ReturnType skipJSONFieldImpl(ReadBuffer &buf, std::string_view name_of_field, const FormatSettings::JSON &settings, size_t current_depth) {
  ...
  if (unlikely(current_depth > settings.max_depth)) {
    if constexpr (throw_exception) throw Exception(ErrorCodes::TOO_DEEP_RECURSION, "JSON is too deep for key '{}'", name_of_field);
    return ReturnType(false);
  }

  if (unlikely(current_depth > 0 && current_depth % 1024 == 0)) checkStackSize();

  if (buf.eof()) {
    if constexpr (throw_exception) throw Exception(ErrorCodes::INCORRECT_DATA, "Unexpected EOF for key '{}'", name_of_field);
    return ReturnType(false);
  }
  ...
  } else if (isNumericASCII(*buf.position()) || *buf.position() == '-' || *buf.position() == '+' || *buf.position() == '.')  /// skip number
  {
    ...
    if (!tryReadFloatText(v, buf)) {
      if constexpr (throw_exception) throw Exception(ErrorCodes::INCORRECT_DATA, "Expected a number field for key '{}'", name_of_field);
      return ReturnType(false);
    }
  ...
  } else if (*buf.position() == 'f')  /// skip false
  {
    ...
    while (true) {
      ...
      if (!buf.eof() && *buf.position() == ',') {
        ...
        if constexpr (throw_exception) throw Exception(ErrorCodes::INCORRECT_DATA, "Unexpected symbol for key '{}'", name_of_field);
        return ReturnType(false);
      }
    }
  } else if (*buf.position() == '{')  /// skip whole object
  {
    ...
    if constexpr (throw_exception)
      throw Exception(ErrorCodes::INCORRECT_DATA, "Cannot read JSON field here: '{}'. Unexpected symbol '{}'{}",
                      String(buf.position(), std::min(buf.available(), size_t(10))), std::string(1, *buf.position()),
...
Exception readException(ReadBuffer &buf, const String &additional_message, bool remote_exception) {
  ...
  if (!additional_message.empty()) out << additional_message << ". ";

  if (name != "DB::Exception") out << name << ". ";

  out << message << ".";

  if (!stack_trace.empty()) out << " Stack trace:\n\n" << stack_trace;

  return Exception::createDeprecated(out.str(), code, remote_exception);
}

void readAndThrowException(ReadBuffer &buf, const String &additional_message) { readException(buf, additional_message).rethrow(); }
...
bool loadAtPosition(ReadBuffer &in, Memory<> &memory, char *&current) {
  ...
  if (current < in.buffer().end()) return true;
...
static void findAndSkipNextDelimiter(PeekableReadBuffer &buf, const String &delimiter) {
  ...
  while (!buf.eof()) {
    ...
    if (checkString(delimiter, buf)) return;
...
void skipToNextRowOrEof(PeekableReadBuffer &buf, const String &row_after_delimiter, const String &row_between_delimiter, bool skip_spaces) {
  ...
  while (true) {
    ...
    if (skip_spaces) skipWhitespaceIfAny(buf);

    if (buf.eof() || checkString(row_between_delimiter, buf)) break;
  }
}
...
static ReturnType readQuotedStringFieldInto(Vector &s, ReadBuffer &buf) {
  ...
  if (buf.eof()) return ReturnType(false);
...
static ReturnType readQuotedFieldInBracketsInto(Vector &s, ReadBuffer &buf) {
  ...
  if (balance) return ReturnType(false);
...
ReturnType readQuotedFieldInto(Vector &s, ReadBuffer &buf) {
  ...
  if (*buf.position() == '\'') return readQuotedStringFieldInto<ReturnType>(s, buf);
  if (*buf.position() == '[') return readQuotedFieldInBracketsInto<ReturnType, '[', ']'>(s, buf);
  if (*buf.position() == '(') return readQuotedFieldInBracketsInto<ReturnType, '(', ')'>(s, buf);
  if (*buf.position() == '{') return readQuotedFieldInBracketsInto<ReturnType, '{', '}'>(s, buf);
...
String escapeDotInJSONKey(const String &key) { return boost::replace_all_copy(key, ".", "%2E"); }

String unescapeDotInJSONKey(const String &key) { return boost::replace_all_copy(key, "%2E", "."); }
