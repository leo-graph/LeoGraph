...
template <int base, typename T, typename ReturnType,
      ...
      handle_digit:
        ...
        if constexpr (check_overflow == ReadIntTextCheckOverflow::CHECK_OVERFLOW && !is_big_int_v<T>) {
          ...
          constexpr size_t max_digits =
              (base == 10) ? std::numeric_limits<T>::max_digits10
          ...
          if (buf.count() - initial_pos + 1 >= max_digits) {
            if (negative) {
              ...
              if (common::mulOverflow<T>(signed_res, base, signed_res) || common::subOverflow<T>(signed_res, digit, signed_res)) {
                if constexpr (throw_exception)
              ...
              T signed_res = res;
              if (common::mulOverflow<T>(signed_res, base, signed_res) || common::addOverflow<T>(signed_res, digit, signed_res)) {
                if constexpr (throw_exception)
                  throw Exception(ErrorCodes::CANNOT_PARSE_NUMBER, "Overflow while parsing a number");
                else
                  return ReturnType(false);
