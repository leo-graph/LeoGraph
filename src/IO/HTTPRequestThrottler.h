...
struct HTTPRequestThrottler {
  ...
  void throttleHTTPGet() const { throttleImpl(get_throttler, get_blocked, disk_get_amount, disk_get_blocked, disk_get_sleep_us); }

  void throttleHTTPPut() const { throttleImpl(put_throttler, put_blocked, disk_put_amount, disk_put_blocked, disk_put_sleep_us); }

 private:
  void throttleImpl(const ThrottlerPtr& throttler, ProfileEvents::Event blocked_event, ProfileEvents::Event disk_amount_event,
                    ProfileEvents::Event disk_blocked_event, ProfileEvents::Event disk_sleep_event) const;
};
