...
std::string ReadBufferFromFileDescriptor::getFileName() const { return "(fd = " + toString(fd) + ")"; }
...
size_t ReadBufferFromFileDescriptor::readImpl(char *to, size_t min_bytes, size_t max_bytes, size_t offset) const {
  ...
  if (file_size.has_value() && offset >= *file_size) return 0;
  ...
  while (bytes_read < min_bytes) {
    ...
    if (-1 == res && errno != EINTR) {
      ...
      ErrnoException::throwFromPath(ErrorCodes::CANNOT_READ_FROM_FILE_DESCRIPTOR, getFileName(), "Cannot read from file {}", getFileName());
    }

    if (res > 0) {
      bytes_read += res;
      if (throttler) throttler->throttle(res);
    }
  ...
  if (bytes_read) ProfileEvents::increment(ProfileEvents::ReadBufferFromFileDescriptorReadBytes, bytes_read);
...
off_t ReadBufferFromFileDescriptor::seek(off_t offset, int whence) {
  ...
  if (file_offset_of_buffer_end - working_buffer.size() <= new_pos && new_pos <= file_offset_of_buffer_end) {
    ...
    if (!use_pread) {
      ...
      if (-1 == res)
        ErrnoException::throwFromPath(ErrorCodes::CANNOT_SEEK_THROUGH_FILE, getFileName(), "Cannot seek through file {} at offset {}",
                                      getFileName(), seek_pos);

      /// Also note that seeking past the file size is not allowed.
      if (res != seek_pos)
        throw Exception(ErrorCodes::CANNOT_SEEK_THROUGH_FILE, "The 'lseek' syscall returned value ({}) that is not expected ({})", res,
                        seek_pos);
    ...
    if (offset_after_seek_pos > 0) ignore(offset_after_seek_pos);
...
void ReadBufferFromFileDescriptor::rewind() {
  if (!use_pread) {
    ...
    if (-1 == res)
      ErrnoException::throwFromPath(ErrorCodes::CANNOT_SEEK_THROUGH_FILE, getFileName(), "Cannot seek through file {}", getFileName());
  }
...
std::optional<size_t> ReadBufferFromFileDescriptor::tryGetFileSize() { return getSizeFromFileDescriptor(fd, getFileName()); }
