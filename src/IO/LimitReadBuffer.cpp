...
bool LimitReadBuffer::nextImpl() {
  ...
  if (bytes >= settings.read_no_less) {
    ...
    if (bytes >= settings.read_no_more) return false;
...
LimitReadBuffer::LimitReadBuffer(ReadBuffer& in_, Settings settings_)
    : ReadBuffer(in_.position(), 0), in(&in_), settings(std::move(settings_)) {
  chassert(in);
...
LimitReadBuffer::LimitReadBuffer(std::unique_ptr<ReadBuffer> in_, Settings settings_) : LimitReadBuffer(*in_, std::move(settings_)) {
  holder = std::move(in_);
}

LimitReadBuffer::~LimitReadBuffer() {
  /// Update underlying buffer's position in case when limit wasn't reached.
  if (!working_buffer.empty()) in->position() = position();
}

size_t LimitReadBuffer::getEffectiveBufferSize() const {
  if (settings.read_no_less) return settings.read_no_less;
