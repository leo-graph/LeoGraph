...
class WriteBufferFromVectorImpl : public WriteBuffer {
 public:
  ...
  WriteBufferFromVectorImpl(VectorType& vector_, AppendModeTag) : WriteBuffer(nullptr, 0), vector(vector_) {
 ...
 protected:
  void finalizeImpl() override {
    vector.resize(((position() - reinterpret_cast<Position>(vector.data()))  /// NOLINT
 ...
 private:
  void nextImpl() override {
    if (finalized) throw Exception(ErrorCodes::CANNOT_WRITE_AFTER_END_OF_BUFFER, "WriteBufferFromVector is finalized");
    ...
    internal_buffer =
        Buffer(reinterpret_cast<Position>(vector.data() + pos_offset), reinterpret_cast<Position>(vector.data() + vector.size()));
