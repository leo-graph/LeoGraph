...
class UploadHelper {
 public:
  UploadHelper(const CreateReadBuffer &create_read_buffer_, std::shared_ptr<const AzureBlobStorage::ContainerClient> client_,
               size_t offset_, size_t total_size_, const String &dest_container_for_logging_, const String &dest_blob_,
               std::shared_ptr<const AzureBlobStorage::RequestSettings> settings_, ThreadPoolCallbackRunnerUnsafe<void> schedule_,
               BlobStorageLogWriterPtr blob_storage_log_, LoggerPtr log_)
 ...
 protected:
  ...
  void calculatePartSize() {
    ...
    if (!max_part_number) throw Exception(ErrorCodes::INVALID_CONFIG_PARAMETER, "max_blocks_in_multipart_upload must not be 0");
 ...
 public:
    ...
    try {
      ...
      if (blob_storage_log)
        blob_storage_log->addEvent(BlobStorageLogElement::EventType::Upload,
                                   ...
                                   /* local_path */ {},
                                   /* data_size */ total_size, watch.elapsedMicroseconds(), error_code, error_message);
      throw;
    }
  }
  ...
  void completeMultipartUpload() {
    ...
    if (client->IsClientForDisk()) ProfileEvents::increment(ProfileEvents::DiskAzureCommitBlockList);
    ...
    try {
      ...
      if (blob_storage_log)
        blob_storage_log->addEvent(BlobStorageLogElement::EventType::MultiPartUploadComplete,
                                   ...
                                   /* remote_path */ dest_blob,
                                   /* local_path */ {},
                                   /* data_size */ 0, watch.elapsedMicroseconds(), error_code, error_message);
      throw;
    }
    ...
    if (blob_storage_log)
      blob_storage_log->addEvent(BlobStorageLogElement::EventType::MultiPartUploadComplete,
                                 ...
                                 /* remote_path */ dest_blob,
                                 /* local_path */ {},
                                 /* data_size */ 0, elapsed, error_code, error_message);
  }
  ...
  void performMultipartUpload() {
    ...
    try {
      ...
      tryLogCurrentException(
          log, fmt::format("While performing multipart upload of blob {} in container {}", dest_blob, dest_container_for_logging));
  ...
  void processUploadPartRequest(UploadPartTask &task) {
    ...
    if (client->IsClientForDisk()) ProfileEvents::increment(ProfileEvents::DiskAzureStageBlock);
    ...
    try {
      ...
      if (blob_storage_log)
        blob_storage_log->addEvent(BlobStorageLogElement::EventType::MultiPartUploadWrite,
                                   ...
                                   /* remote_path */ dest_blob,
                                   /* local_path */ {},
                                   /* data_size */ size_to_stage, watch.elapsedMicroseconds(), error_code, error_message);
      throw;
    }
    auto elapsed = watch.elapsedMicroseconds();
    ...
    if (blob_storage_log)
      blob_storage_log->addEvent(BlobStorageLogElement::EventType::MultiPartUploadWrite,
                                 ...
                                 /* remote_path */ dest_blob,
                                 /* local_path */ {},
                                 /* data_size */ size_to_stage, elapsed, error_code, error_message);

    LOG_TRACE(log, "Writing part. Container: {}, Blob: {}, block_id: {}, size: {}", dest_container_for_logging, dest_blob, block_id,
              size_to_stage);
  }
  ...
  void waitForAllBackgroundTasks() {
    ...
    bg_tasks_condvar.wait(lock, [this]() {
    ...
    for (const auto &task : tasks) block_ids.insert(block_ids.end(), task.block_ids.begin(), task.block_ids.end());
  }
};
}  // namespace

void copyDataToAzureBlobStorageFile(const std::function<std::unique_ptr<SeekableReadBuffer>()> &create_read_buffer, size_t offset,
                                    size_t size, std::shared_ptr<const AzureBlobStorage::ContainerClient> dest_client,
                                    const String &dest_container_for_logging, const String &dest_blob,
                                    std::shared_ptr<const AzureBlobStorage::RequestSettings> settings,
                                    ThreadPoolCallbackRunnerUnsafe<void> schedule, BlobStorageLogWriterPtr blob_storage_log) {
  auto log = getLogger("copyDataToAzureBlobStorageFile");
  UploadHelper helper{create_read_buffer,          dest_client, offset, size, dest_container_for_logging, dest_blob, settings, schedule,
                      std::move(blob_storage_log), log};
  helper.performCopy();
}

void copyAzureBlobStorageFile(std::shared_ptr<const AzureBlobStorage::ContainerClient> src_client,
                              std::shared_ptr<const AzureBlobStorage::ContainerClient> dest_client, const String &src_container_for_logging,
                              const String &src_blob, size_t offset, size_t size, const String &dest_container_for_logging,
                              const String &dest_blob, std::shared_ptr<const AzureBlobStorage::RequestSettings> settings,
                              const ReadSettings &read_settings, const std::optional<ObjectAttributes> &object_to_attributes,
                              ThreadPoolCallbackRunnerUnsafe<void> schedule, BlobStorageLogWriterPtr blob_storage_log) {
  ...
  if (settings->use_native_copy) {
    ...
    if (dest_client->IsClientForDisk()) ProfileEvents::increment(ProfileEvents::DiskAzureCopyObject);
    ...
    try {
      ...
      if (size < settings->max_single_part_copy_size) {
        ...
        if (object_to_attributes.has_value()) {
          for (const auto &[key, value] : *object_to_attributes) copy_options.Metadata[key] = value;
        }
        ...
        if (copy_status.HasValue() && copy_status.Value() == Azure::Storage::Blobs::Models::CopyStatus::Success) {
          ...
          if (copy_status.HasValue())
            throw Exception(ErrorCodes::AZURE_BLOB_STORAGE_ERROR,
                            "Copy from {} to {} failed with status {} description {} (operation is done {})", src_blob, dest_blob,
                            copy_status.Value().ToString(), copy_status_description.Value(), operation.IsDone());
          throw Exception(ErrorCodes::AZURE_BLOB_STORAGE_ERROR,
                          "Copy from {} to {} didn't complete with success status (operation is done {})", src_blob, dest_blob,
                          operation.IsDone());
        }
      }
      ...
      if (e.StatusCode == Azure::Core::Http::HttpStatusCode::Unauthorized) {
        LOG_TRACE(log,
                  "Copy operation has thrown unauthorized access error, which indicates that the storage account of the source & "
        ...
        LOG_TRACE(log,
                  "Copy operation has thrown bad argument error. e.what = {}. "
                  "Will attempt to copy using read & write. source container = {} blob = {} and destination container = {} blob = {}",
  ...
  if (!is_native_copy_done) {
    ...
    UploadHelper helper{create_read_buffer, dest_client, offset,           size, dest_container_for_logging, dest_blob,
                        settings,           schedule,    blob_storage_log, log};
    helper.performCopy();
  }
}
