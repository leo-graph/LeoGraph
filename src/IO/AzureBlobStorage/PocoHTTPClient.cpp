...
class IStreamBodyStream : public Azure::Core::IO::BodyStream {
 ...
 public:
  IStreamBodyStream(std::istream &stream_, DB::HTTPSessionPtr session_, Int64 length_)
      : stream(stream_), session(session_), length(length_) {
    start_pos = stream.tellg();
  }

  int64_t Length() const override { return length; }
...
std::unique_ptr<Azure::Core::Http::RawResponse> PocoAzureHTTPClient::Send(Azure::Core::Http::Request &request,
...
void PocoAzureHTTPClient::addMetric(const std::string &method, AzureMetricType type, ProfileEvents::Count amount) const {
  static const ProfileEvents::Event events_map[static_cast<size_t>(AzureMetricType::EnumSize)]
                                              [static_cast<size_t>(AzureMetricKind::EnumSize)] = {
  ...
  static const ProfileEvents::Event
      disk_s3_events_map[static_cast<size_t>(AzureMetricType::EnumSize)][static_cast<size_t>(AzureMetricKind::EnumSize)] = {
...
void PocoAzureHTTPClient::observeLatency(const std::string &method, AzureLatencyType type, HistogramMetrics::Value latency) const {
  ...
  const String attempt_label = [](const AzureLatencyType t) {
    switch (t) {
      case AzureLatencyType::FirstByteAttempt1:
        return "1";
      case AzureLatencyType::FirstByteAttempt2:
        return "2";
      case AzureLatencyType::FirstByteAttemptN:
        return "N";
      default:
        return "UNKNOWN";
    }
  ...
  HistogramMetrics::observe(HistogramMetrics::AzureFirstByte, first_byte_label_values, latency);

  if (for_disk_azure) {
    HistogramMetrics::observe(HistogramMetrics::DiskAzureFirstByte, first_byte_label_values, latency);
  }
}
...
ConnectionTimeouts PocoAzureHTTPClient::getTimeouts(const std::string &method, bool first_attempt, bool first_byte) const {
  if (!use_adaptive_timeouts) return timeouts;
...
static size_t getSDKRetryAttempt(const Azure::Core::Context &context) {
  ...
  if (retry_no < 0) return 1;  // First attempt is considered as attempt 1
...
static size_t getCHRetryAttempt(const Azure::Core::Context &context) {
  size_t number = 1;
  if (context.TryGetValue<size_t>(PocoAzureHTTPClient::getSDKContextKeyForBufferRetry(), number)) return number + 1;

  return number;
}

std::unique_ptr<Azure::Core::Http::RawResponse> PocoAzureHTTPClient::makeRequestInternalImpl(Azure::Core::Http::Request &request,
  ...
  try {
    ...
    auto session = makeHTTPSession(group, uri, adaptive_timeouts, ProxyConfiguration{}, &connect_time);
    ...
    auto response =
        std::make_unique<Azure::Core::Http::RawResponse>(1, 1,  // HTTP/1.1
                                                         static_cast<Azure::Core::Http::HttpStatusCode>(status), poco_response.getReason());

    response->SetBodyStream(std::make_unique<IStreamBodyStream>(response_stream, session, poco_response.getContentLength()));

    for (const auto &[header_name, header_value] : poco_response) response->SetHeader(header_name, header_value);
    ...
    auto response = std::make_unique<Azure::Core::Http::RawResponse>(1, 1,  // HTTP/1.1
                                                                     Azure::Core::Http::HttpStatusCode::RequestTimeout, error_message.text);
