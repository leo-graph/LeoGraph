...
ssize_t ReadBufferFromPocoSocketBase::socketReceiveBytesImpl(char *ptr, size_t size) {
  ...
  try {
    ...
    if (async_callback) {
      ...
      while (bytes_read < 0 && (errno == EAGAIN || (secure && (checkSSLWantRead(bytes_read) || checkSSLWantWrite(bytes_read))))) {
        ...
        if (secure && checkSSLWantWrite(bytes_read))
          async_callback(socket.impl()->sockfd(), socket.getSendTimeout(), AsyncEventTimeoutType::SEND, socket_description,
                         AsyncTaskExecutor::Event::WRITE | AsyncTaskExecutor::Event::ERROR);
        else
          async_callback(socket.impl()->sockfd(), socket.getReceiveTimeout(), AsyncEventTimeoutType::RECEIVE, socket_description,
    ...
    throw NetException(ErrorCodes::NETWORK_ERROR, "{}, while reading from socket (peer: {}, local: {})", e.displayText(),
    ...
    throw NetException(ErrorCodes::SOCKET_TIMEOUT, "Timeout exceeded while reading from socket (peer: {}, local: {}, {} ms)",
                       peer_address.toString(), socket.address().toString(), socket.impl()->getReceiveTimeout().totalMilliseconds());
  ...
  if (bytes_read < 0)
    throw NetException(ErrorCodes::CANNOT_READ_FROM_SOCKET, "Cannot read from socket (peer: {}, local: {})", peer_address.toString(),
...
bool ReadBufferFromPocoSocketBase::nextImpl() {
  if (internal_buffer.size() > INT_MAX) throw Exception(ErrorCodes::LOGICAL_ERROR, "Buffer overflow");

  ssize_t bytes_read = socketReceiveBytesImpl(internal_buffer.begin(), internal_buffer.size());

  if (read_event != ProfileEvents::end()) ProfileEvents::increment(read_event, bytes_read);
...
ReadBufferFromPocoSocketBase::ReadBufferFromPocoSocketBase(Poco::Net::Socket &socket_, const ProfileEvents::Event &read_event_,
                                                           size_t buf_size)
...
bool ReadBufferFromPocoSocketBase::poll(size_t timeout_microseconds) const {
  ...
  if (available() || socket.impl()->available()) return true;
