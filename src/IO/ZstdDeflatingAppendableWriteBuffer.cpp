...
ZstdDeflatingAppendableWriteBuffer::ZstdDeflatingAppendableWriteBuffer(
    std::unique_ptr<WriteBufferFromFileBase> out_, int compression_level, bool append_to_existing_file_,
    std::function<std::unique_ptr<ReadBufferFromFileBase>()> read_buffer_creator_, size_t buf_size, char *existing_memory, size_t alignment)
  ...
  if (ZSTD_isError(ret))
    throw Exception(ErrorCodes::ZSTD_ENCODER_FAILED, "ZSTD stream encoder option setting failed: error code: {}; zstd version: {}", ret,
                    ZSTD_VERSION_STRING);
  ...
  try {
    ...
    do {
      ...
      if (ZSTD_isError(compression_result))
        throw Exception(ErrorCodes::ZSTD_ENCODER_FAILED, "ZSTD stream decoding failed: error code: {}; ZSTD version: {}",
...
void ZstdDeflatingAppendableWriteBuffer::finalizeBefore() {
  ...
  while (remaining != 0) {
    if (ZSTD_isError(remaining))
      throw Exception(ErrorCodes::ZSTD_ENCODER_FAILED, "ZSTD stream encoder end failed: error: '{}' ZSTD version: {}",
...
void ZstdDeflatingAppendableWriteBuffer::finalizeAfter() { finalizeZstd(); }
...
void ZstdDeflatingAppendableWriteBuffer::finalizeZstd() {
  ...
  try {
    ...
    if (unlikely(err))
      throw Exception(ErrorCodes::ZSTD_ENCODER_FAILED, "ZSTD_freeCCtx failed: error: '{}'; zstd version: {}", ZSTD_getErrorName(err),
                      ZSTD_VERSION_STRING);
  } catch (...) {
...
void ZstdDeflatingAppendableWriteBuffer::addEmptyBlock() {
  ...
  if (out->buffer().size() - out->offset() < ZSTD_CORRECT_TERMINATION_LAST_BLOCK.size()) out->next();

  std::memcpy(out->buffer().begin() + out->offset(), ZSTD_CORRECT_TERMINATION_LAST_BLOCK.data(),
              ZSTD_CORRECT_TERMINATION_LAST_BLOCK.size());
...
bool ZstdDeflatingAppendableWriteBuffer::isNeedToAddEmptyBlock() {
  ...
  if (fsize > 0) {
    throw Exception(ErrorCodes::ZSTD_ENCODER_FAILED,
                    "Trying to write to non-empty file '{}' with tiny size {}. It can lead to data corruption", out->getFileName(), fsize);
  }
  return false;
}
