...
String fourSpaceIndent(size_t indent) { return std::string(indent * 4, ' '); }
...
size_t writeFloatTextFastPath(T x, char *buffer) {
  ...
  if (result <= 0) throw Exception(ErrorCodes::CANNOT_PRINT_FLOAT_OR_DOUBLE_NUMBER, "Cannot print floating point number");
  return result;
}
