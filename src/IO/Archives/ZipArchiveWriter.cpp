...
class ZipArchiveWriter::WriteBufferFromZipArchive : public WriteBufferFromFileBase {
 public:
  WriteBufferFromZipArchive(std::shared_ptr<ZipArchiveWriter> archive_writer_, const String &filename_)
      : WriteBufferFromFileBase(DBMS_DEFAULT_BUFFER_SIZE, nullptr, 0), filename(filename_) {
    ...
    int code = zipOpenNewFileInZip3_64(zip_handle, filename_.c_str(),
                                       /* zipfi= */ nullptr,
                                       /* extrafield_local= */ nullptr,
                                       /* size_extrafield_local= */ 0,
                                       /* extrafield_global= */ nullptr,
                                       /* size_extrafield_global= */ 0,
                                       /* comment= */ nullptr, compress_method, compress_level,
                                       /* raw= */ false,
                                       /* windowBits= */ 0,
                                       /* memLevel= */ 0,
                                       /* strategy= */ 0, password_cstr,
                                       /* crc_for_crypting= */ 0,
                                       /* zip64= */ true);
 ...
 private:
  void nextImpl() override {
    ...
    if (auto writer = archive_writer.lock()) writer->rethrowStoredException();
    checkResultCode(code);
  }
 ...
 public:
  ...
  ZipHandle makeZipHandle() {
    ...
    return zipOpen2_64(
        /* path= */ nullptr,
        /* append= */ false,
        /* globalcomment= */ nullptr, &func_def);
  }
 ...
 private:
  ...
  static long seekFunc(void *opaque, void *, ZPOS64_T, int)  // NOLINT(google-runtime-int)
  {
    ...
    if (!stream_info->stored_exception)
      stream_info->stored_exception =
          std::make_exception_ptr(Exception(ErrorCodes::NOT_IMPLEMENTED, "StreamInfo::seek is not implemented"));
    return -1;
  }
  ...
  static unsigned long readFileFunc(void *opaque, void *, void *, unsigned long)  // NOLINT(google-runtime-int)
  {
    ...
    if (!stream_info->stored_exception)
      stream_info->stored_exception =
          std::make_exception_ptr(Exception(ErrorCodes::NOT_IMPLEMENTED, "StreamInfo::readFile is not implemented"));
    return 0;
  }
...
ZipArchiveWriter::ZipArchiveWriter(const String &path_to_archive_) : ZipArchiveWriter(path_to_archive_, nullptr) {}
...
ZipArchiveWriter::ZipArchiveWriter(const String &path_to_archive_, std::unique_ptr<WriteBuffer> archive_write_buffer_)
  ...
  if (!zip_handle) throw Exception(ErrorCodes::CANNOT_PACK_ARCHIVE, "Couldn't create zip archive {}", quoteString(path_to_archive));
}
...
void ZipArchiveWriter::finalize() {
  ...
  if (is_writing_file)
    throw Exception(ErrorCodes::LOGICAL_ERROR,
                    "ZipArchiveWriter::finalize() is called in the middle of writing a file into the zip archive. That's not allowed");
...
int ZipArchiveWriter::compressionMethodToInt(const String &compression_method_) {
  if (compression_method_.empty()) return MZ_COMPRESS_METHOD_DEFLATE;  /// By default the compression method is "deflate".
  if (compression_method_ == kStore) return MZ_COMPRESS_METHOD_STORE;
  if (compression_method_ == kDeflate) return MZ_COMPRESS_METHOD_DEFLATE;
  if (compression_method_ == kBzip2) return MZ_COMPRESS_METHOD_BZIP2;
  if (compression_method_ == kLzma) return MZ_COMPRESS_METHOD_LZMA;
  if (compression_method_ == kZstd) return MZ_COMPRESS_METHOD_ZSTD;
  if (compression_method_ == kXz) return MZ_COMPRESS_METHOD_XZ;
...
ZipArchiveWriter::ZipHandle ZipArchiveWriter::startWritingFile() {
  std::lock_guard lock{mutex};
  if (is_writing_file) throw Exception(ErrorCodes::NOT_IMPLEMENTED, "Cannot write two files to a zip archive in parallel");
...
void ZipArchiveWriter::checkResultCode(int code) const { checkResultCodeImpl(code, path_to_archive); }
...
void ZipArchiveWriter::rethrowStoredExceptionLocked() {
  if (stream_info) stream_info->rethrowIfNeeded();
}
