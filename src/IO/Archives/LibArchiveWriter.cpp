 ...
 public:
  ...
  static ssize_t memory_write(struct archive *, void *client_data, const void *buff, size_t length) {
    ...
    try {
      ...
      if (!stream_info->stored_exception) stream_info->stored_exception = std::current_exception();
      return -1;
    }
  }
...
class LibArchiveWriter::WriteBufferFromLibArchive : public WriteBufferFromFileBase {
 public:
  WriteBufferFromLibArchive(std::shared_ptr<LibArchiveWriter> archive_writer_, const String &filename_, const size_t &size_,
                            const size_t buf_size_, bool use_adaptive_buffer_size_, size_t adaptive_buffer_max_size_)
 ...
 private:
      ...
      if (!available()) {
        if (memory.size() == adaptive_max_buffer_size)
          throw Exception(ErrorCodes::LIMIT_EXCEEDED,
                          ...
                          adaptive_max_buffer_size, filename);
  ...
  void writeDataChunk() {
    if (entry == nullptr) writeEntry();
    ssize_t to_write = offset();
    ssize_t written = archive_write_data(archive, working_buffer.begin(), offset());
    if (auto writer = archive_writer.lock()) writer->rethrowStoredException();
    if (written != to_write) {
      throw Exception(ErrorCodes::CANNOT_PACK_ARCHIVE, "Couldn't pack tar archive: Failed to write all bytes, {} of {}, filename={}",
                      written, to_write, quoteString(filename));
    }
  }
  ...
  void closeFile(bool throw_if_error) {
    ...
    if (throw_if_error and (!use_adaptive_buffer_size and bytes != expected_size)) {
      throw Exception(ErrorCodes::CANNOT_PACK_ARCHIVE, "Couldn't pack tar archive: Wrote {} of expected {} , filename={}", bytes,
                      expected_size, quoteString(filename));
    }
  }

  void endWritingFile() {
    if (auto archive_writer_ptr = archive_writer.lock()) archive_writer_ptr->endWritingFile();
  }

  void startWritingFile() {
    if (auto archive_writer_ptr = archive_writer.lock()) archive_writer_ptr->startWritingFile();
  }
...
LibArchiveWriter::LibArchiveWriter(const String &path_to_archive_, std::unique_ptr<WriteBuffer> archive_write_buffer_, size_t buf_size_,
                                   size_t adaptive_buffer_max_size_)
    : path_to_archive(path_to_archive_), buf_size(buf_size_), adaptive_buffer_max_size(std::max(adaptive_buffer_max_size_, buf_size_)) {
  if (archive_write_buffer_) stream_info = std::make_unique<StreamInfo>(std::move(archive_write_buffer_));
}
...
std::unique_ptr<WriteBufferFromFileBase> LibArchiveWriter::writeFile(const String &filename, size_t size) {
  return std::make_unique<WriteBufferFromLibArchive>(std::static_pointer_cast<LibArchiveWriter>(shared_from_this()), filename, size,
                                                     buf_size,
                                                     /*use_adaptive_buffer_size*/ false, adaptive_buffer_max_size);
}
...
std::unique_ptr<WriteBufferFromFileBase> LibArchiveWriter::writeFile(const String &filename) {
  ...
  return std::make_unique<WriteBufferFromLibArchive>(std::static_pointer_cast<LibArchiveWriter>(shared_from_this()), filename,
                                                     /*size*/ 0, buf_size,
                                                     /*use_adaptive_buffer_size*/ true, adaptive_buffer_max_size);
}
...
void LibArchiveWriter::finalize() {
  std::lock_guard lock{mutex};
  if (finalized) return;
  if (archive) archive_write_close(archive);
