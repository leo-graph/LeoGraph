  ...
  static ssize_t read(struct archive *, void *client_data, const void **buff) {
    ...
    try {
      *buff = reinterpret_cast<void *>(read_stream->buf);
      ...
      if (!read_stream->stored_exception) read_stream->stored_exception = std::current_exception();
      return -1;
    }
  }
 ...
 public:
  ...
  bool locateFile(NameFilter filter) {
    ...
    while (true) {
      ...
      if (err == ARCHIVE_RETRY) continue;

      if (err != ARCHIVE_OK) break;
  ...
  std::vector<std::string> getAllFiles(NameFilter filter) {
    ...
    while (error == ARCHIVE_OK || error == ARCHIVE_RETRY) {
      ...
      if (!filter || filter(name)) files.push_back(std::move(name));
  ...
  const String &getFileName() const {
    ...
    if (!valid) throw Exception(ErrorCodes::CANNOT_UNPACK_ARCHIVE, "No current file");
    if (!file_name) file_name.emplace(archive_entry_pathname(current_entry));
  ...
  void rethrowStreamException() {
    if (read_stream) read_stream->rethrowIfNeeded();
  }
 ...
 private:
  ...
  void checkError(int error) const {
    if (error == ARCHIVE_FATAL)
      throw Exception(ErrorCodes::CANNOT_UNPACK_ARCHIVE, "Failed to read archive while fetching all files: {}",
...
class LibArchiveReader::ReadBufferFromLibArchive : public ReadBufferFromFileBase {
 ...
 private:
  bool nextImpl() override {
    ...
    if (bytes_read < 0)
      throw Exception(ErrorCodes::CANNOT_READ_ALL_DATA, "Failed to read file {} from {}: {}", handle.getFileName(), path_to_archive,
...
LibArchiveReader::LibArchiveReader(std::string archive_name_, bool lock_on_reading_, std::string path_to_archive_,
                                   const ReadArchiveFunction &archive_read_function_)
...
const std::string &LibArchiveReader::getPath() const { return path_to_archive; }
...
LibArchiveReader::FileInfo LibArchiveReader::getFileInfo(const String &filename) {
  ...
  if (!handle.locateFile(filename))
    throw Exception(ErrorCodes::CANNOT_UNPACK_ARCHIVE, "Couldn't unpack archive {}: File {} was not found in archive", path_to_archive,
...
std::unique_ptr<ReadBufferFromFileBase> LibArchiveReader::readFile(NameFilter filter, bool throw_on_not_found) {
  ...
  if (!handle.locateFile(filter)) {
    if (throw_on_not_found)
      throw Exception(ErrorCodes::CANNOT_UNPACK_ARCHIVE, "Couldn't unpack archive {}: No file satisfying filter in archive",
                      path_to_archive);
    return nullptr;
  }
  return std::make_unique<ReadBufferFromLibArchive>(std::move(handle), path_to_archive);
...
std::vector<std::string> LibArchiveReader::getAllFiles() { return getAllFiles({}); }
