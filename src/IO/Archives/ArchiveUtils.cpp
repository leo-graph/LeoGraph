...
bool hasSupportedExtension(std::string_view path, const auto& supported_extensions) {
  for (auto supported_extension : supported_extensions) {
    if (path.ends_with(supported_extension)) return true;
  }
...
bool hasSupportedTarExtension(std::string_view path) { return hasSupportedExtension(path, tar_extensions); }

bool hasSupportedZipExtension(std::string_view path) { return hasSupportedExtension(path, zip_extensions); }

bool hasSupported7zExtension(std::string_view path) { return hasSupportedExtension(path, sevenz_extensiosns); }
...
std::pair<std::string, std::optional<std::string>> getURIAndArchivePattern(const std::string& source) {
  ...
  if (pos == std::string::npos) return {source, std::nullopt};
