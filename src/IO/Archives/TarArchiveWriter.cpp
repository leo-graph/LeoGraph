...
void TarArchiveWriter::setCompression(const String& compression_method_, int compression_level_) {
  ...
  if (compression_method_.empty() && compression_level_ == -1) return;
  throw Exception(ErrorCodes::NOT_IMPLEMENTED, "Using compression_method and compression_level options are not supported for tar archives");
}
