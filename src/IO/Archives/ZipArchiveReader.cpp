...
namespace {
void checkCompressionMethodIsEnabled(int compression_method_) { ZipArchiveWriter::checkCompressionMethodIsEnabled(compression_method_); }

void checkEncryptionIsEnabled() { ZipArchiveWriter::checkEncryptionIsEnabled(); }
...
class StreamFromReadBuffer {
 ...
 private:
  ...
  static StreamFromReadBuffer &get(void *ptr) { return *reinterpret_cast<StreamFromReadBuffer *>(ptr); }
  ...
  static ZPOS64_T tellFunc(void *, void *stream) {
    auto &strm = get(stream);
    try {
      if (strm.at_end) return strm.total_size;
  ...
  static long seekFunc(void *, void *stream, ZPOS64_T offset, int origin)  // NOLINT(google-runtime-int)
  {
    ...
    try {
      ...
      strm.at_end = false;
      if (origin == SEEK_SET) offset += strm.start_offset;
  ...
  static unsigned long writeFileFunc(void *, void *stream, const void *, unsigned long)  // NOLINT(google-runtime-int)
  {
    ...
    if (!strm.stored_exception)
      strm.stored_exception =
          std::make_exception_ptr(Exception(ErrorCodes::LOGICAL_ERROR, "StreamFromReadBuffer::writeFile must not be called"));
    return 0;
  }
};
}  // namespace
...
class ZipArchiveReader::HandleHolder {
 public:
  ...
  HandleHolder(HandleHolder &&src) noexcept { *this = std::move(src); }
  ...
  bool locateFile(const String &file_name_) {
    ...
    if (err == UNZ_END_OF_LIST_OF_FILE) return false;
  ...
  bool locateFile(NameFilter filter) {
    ...
    do {
      ...
      resetFileInfo();
      retrieveFileInfo();
      if (filter(getFileName())) return true;
  ...
  const String &getFileName() const {
    if (!file_name) retrieveFileInfo();
    return *file_name;
  }

  const FileInfoImpl &getFileInfo() const {
    if (!file_info) retrieveFileInfo();
    return *file_info;
  }
  ...
  std::vector<std::string> getAllFiles(NameFilter filter) {
    ...
    int err = unzGoToFirstFile(raw_handle);
    rethrowStreamException();
    if (err == UNZ_END_OF_LIST_OF_FILE) return files;

    do {
      checkResult(err);
      resetFileInfo();
      retrieveFileInfo();
      if (!filter || filter(getFileName())) files.push_back(*file_name);
      err = unzGoToNextFile(raw_handle);
      rethrowStreamException();
    } while (err != UNZ_END_OF_LIST_OF_FILE);
  ...
  void closeFile() {
    ...
    if (err != UNZ_PARAMERROR) checkResult(err);
  }
...
class ZipArchiveReader::ReadBufferFromZipArchive : public ReadBufferFromFileBase {
 public:
  explicit ReadBufferFromZipArchive(HandleHolder &&handle_)
      : ReadBufferFromFileBase(DBMS_DEFAULT_BUFFER_SIZE, nullptr, 0), handle(std::move(handle_)) {
    ...
    if (file_info.is_encrypted) {
      ...
      if (password_str.empty()) showError("Password is required");
    ...
    if (err == MZ_PASSWORD_ERROR) showError("Wrong password");
    checkResult(err);
  }
  ...
  off_t seek(off_t off, int whence) override {
    ...
    if (new_pos == current_pos) return current_pos;  /// The position is the same.

    if (new_pos < 0) throw Exception(ErrorCodes::SEEK_POSITION_OUT_OF_BOUND, "Seek position is out of bound");
 ...
 private:
  bool nextImpl() override {
    ...
    if (bytes_read < 0) checkResult(bytes_read);
...
ZipArchiveReader::ZipArchiveReader(const String &path_to_archive_) : path_to_archive(path_to_archive_) { init(); }

ZipArchiveReader::ZipArchiveReader(const String &path_to_archive_, const ReadArchiveFunction &archive_read_function_, UInt64 archive_size_)
...
const std::string &ZipArchiveReader::getPath() const { return path_to_archive; }

bool ZipArchiveReader::fileExists(const String &filename) { return acquireHandle().tryLocateFile(filename); }

ZipArchiveReader::FileInfo ZipArchiveReader::getFileInfo(const String &filename) {
  auto handle = acquireHandle();
  if (!handle.locateFile(filename)) showError(fmt::format("File {} was not found in archive", quoteString(filename)));
...
std::unique_ptr<ZipArchiveReader::FileEnumerator> ZipArchiveReader::firstFile() {
  auto handle = acquireHandle();
  if (!handle.firstFile()) return nullptr;
...
std::unique_ptr<ReadBufferFromFileBase> ZipArchiveReader::readFile(const String &filename, bool throw_on_not_found) {
  ...
  if (!handle.locateFile(filename)) {
    if (throw_on_not_found) showError(fmt::format("File {} was not found in archive", quoteString(filename)));
...
std::unique_ptr<ReadBufferFromFileBase> ZipArchiveReader::readFile(NameFilter filter, bool throw_on_not_found) {
  ...
  if (!handle.locateFile(filter)) {
    if (throw_on_not_found) showError(fmt::format("No file satisfying filter in archive"));
...
std::unique_ptr<ZipArchiveReader::FileEnumerator> ZipArchiveReader::nextFile(std::unique_ptr<ReadBuffer> read_buffer) {
  ...
  if (!handle.nextFile()) return nullptr;
  return std::make_unique<FileEnumeratorImpl>(std::move(handle));
}
...
std::vector<std::string> ZipArchiveReader::getAllFiles() { return getAllFiles({}); }
...
ZipArchiveReader::RawHandleWithStream ZipArchiveReader::acquireRawHandle() {
  ...
  if (!result.handle) throw Exception(ErrorCodes::CANNOT_UNPACK_ARCHIVE, "Couldn't open zip archive {}", quoteString(path_to_archive));

  return result;
}

void ZipArchiveReader::releaseRawHandle(RawHandleWithStream handle_info) {
  if (!handle_info.handle) return;
...
void ZipArchiveReader::checkResult(int code) const {
  ...
  switch (code) {
    case UNZ_OK:
      return;
    case UNZ_ERRNO:
      message += "ERRNO, errno = " + errnoToString();
      break;
    case UNZ_PARAMERROR:
      message += "PARAMERROR";
      break;
