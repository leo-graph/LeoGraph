...
class InitVector {
 public:
  ...
  InitVector operator++(int) {
    InitVector res = *this;
    ++counter;
    return res;
  }  /// NOLINT
  InitVector& operator+=(size_t offset) {
    counter += offset;
    return *this;
  }
  InitVector operator+(size_t offset) const {
    InitVector res = *this;
    return res += offset;
  }
...
struct Header {
  /// Versions:
  /// 1 - Initial version
  /// 2 - The header of an encrypted file contains the fingerprint of a used encryption key instead of a pair {key_id,
  /// very_small_hash(key)}.
