...
void WriteBufferFromFileDecorator::finalizeImpl() {
  ...
  if (!is_prefinalized) WriteBufferFromFileDecorator::preFinalize();
...
std::string WriteBufferFromFileDecorator::getFileName() const {
  if (WriteBufferFromFileBase* buffer = dynamic_cast<WriteBufferFromFileBase*>(impl.get())) return buffer->getFileName();
  return std::string();
}
