...
inline void readDecimalNumberImpl(T &res, const char *src) {
  ...
  if constexpr (digit > 0) readDecimalNumberImpl<digit - 1, power_of_ten * 10>(res, src);
}
...
#define READ_DECIMAL_NUMBER(N)      \
  ...
  while (num_digits) {
    switch (num_digits) {
      case 3:
        READ_DECIMAL_NUMBER(3);
        break;
      case 2:
        READ_DECIMAL_NUMBER(2);
        break;
      case 1:
        READ_DECIMAL_NUMBER(1);
        break;
      default:
        READ_DECIMAL_NUMBER(4);
        break;
    }
  }
...
ReturnType parseDateTimeBestEffortImpl(time_t &res, ReadBuffer &in, const DateLUTImpl &local_time_zone, const DateLUTImpl &utc_time_zone,
                                       DateTimeSubsecondPart *fractional, const char *allowed_date_delimiters = nullptr) {
  auto on_error = [&]<typename... FmtArgs>(int error_code [[maybe_unused]], FormatStringHelper<FmtArgs...> fmt_string [[maybe_unused]],
  ...
  while (!in.eof()) {
    ...
    if (!year || !has_time) {
      ...
      if (num_digits == 10 && !year && !has_time) {
        if (strict) return on_error(ErrorCodes::CANNOT_PARSE_DATETIME, "Strict best effort parsing doesn't allow timestamps");
      ...
      if (num_digits == 14 && !year && !has_time) {
        ...
        if (!in.eof()) {
          ...
          if (delimiter_after_year < 0x20 || delimiter_after_year == ',' || delimiter_after_year == ';' || delimiter_after_year == '\'' ||
          ...
          if (num_digits == 2)
            ...
            return on_error(ErrorCodes::CANNOT_PARSE_DATETIME, "Cannot read DateTime: unexpected number of decimal digits after year: {}",
                            num_digits);
          ...
          if (!day_of_month && checkChar(delimiter_after_year, in)) {
            ...
            if (num_digits == 2)
              ...
              return on_error(ErrorCodes::CANNOT_PARSE_DATETIME,
                              "Cannot read DateTime: unexpected number of decimal digits after year and month: {}", num_digits);
          }

          if (!isSymbolIn(delimiter_after_year, allowed_date_delimiters))
            return on_error(ErrorCodes::CANNOT_PARSE_DATETIME, "Cannot read DateTime: '{}' delimiter between date parts is not allowed",
                            delimiter_after_year);
        }
        ...
        if (checkChar(':', in)) {
          if (has_time) return on_error(ErrorCodes::CANNOT_PARSE_DATETIME, "Cannot read DateTime: time component is duplicated");
          ...
          if (num_digits == 2)
            ...
            return on_error(ErrorCodes::CANNOT_PARSE_DATETIME, "Cannot read DateTime: unexpected number of decimal digits after hour: {}",
                            num_digits);
          ...
          if (checkChar(':', in)) {
            ...
            if (num_digits == 2)
              ...
              return on_error(ErrorCodes::CANNOT_PARSE_DATETIME,
                              "Cannot read DateTime: unexpected number of decimal digits after hour and minute: {}", num_digits);
          }
        } else if (checkChar(',', in)) {
          if (month && !day_of_month) day_of_month = hour_or_day_of_month_or_month;
        } else if ((!in.eof() && isSymbolIn(*in.position(), allowed_date_delimiters)) &&
                   (checkChar('/', in) || checkChar('.', in) || checkChar('-', in))) {
          if (day_of_month) return on_error(ErrorCodes::CANNOT_PARSE_DATETIME, "Cannot read DateTime: day of month is duplicated");

          if (month) return on_error(ErrorCodes::CANNOT_PARSE_DATETIME, "Cannot read DateTime: month is duplicated");
          ...
          if constexpr (is_us_style) {
            ...
            if (num_digits == 2)
              ...
              return on_error(ErrorCodes::CANNOT_PARSE_DATETIME,
                              "Cannot read DateTime: unexpected number of decimal digits after month: {}", num_digits);
          } else {
            ...
            if (num_digits == 2)
              ...
              if (num_alpha < 3)
                return on_error(ErrorCodes::CANNOT_PARSE_DATETIME,
                                "Cannot read DateTime: unexpected number of alphabetical characters after day of month: {}", num_alpha);
              ...
              return on_error(ErrorCodes::CANNOT_PARSE_DATETIME,
                              "Cannot read DateTime: unexpected number of decimal digits after day of month: {}", num_digits);
          }

          if (month > 12) std::swap(month, day_of_month);

          if ((!in.eof() && isSymbolIn(*in.position(), allowed_date_delimiters)) &&
              (checkChar('/', in) || checkChar('.', in) || checkChar('-', in))) {
            if (year) return on_error(ErrorCodes::CANNOT_PARSE_DATETIME, "Cannot read DateTime: year component is duplicated");
            ...
            if (num_digits == 4)
              ...
              return on_error(ErrorCodes::CANNOT_PARSE_DATETIME,
                              "Cannot read DateTime: unexpected number of decimal digits after day of month and month: {}", num_digits);
          }
        } else {
          if (day_of_month) {
            if (strict && hour) return on_error(ErrorCodes::CANNOT_PARSE_DATETIME, "Cannot read DateTime: hour component is duplicated");
    ...
    if (num_digits == 0) {
      ...
      {
        if (!has_time) return on_error(ErrorCodes::CANNOT_PARSE_DATETIME, "Cannot read DateTime: unexpected point symbol");
        ...
        if (num_digits == 6 && !has_time && year && month && day_of_month) {
          ...
          has_time_zone_offset = true;
          if (c == '-') time_zone_offset_negative = true;
          ...
          if (num_digits == 4) {
            ...
            return on_error(ErrorCodes::CANNOT_PARSE_DATETIME,
                            "Cannot read DateTime: unexpected number of decimal digits for time zone offset: {}", num_digits);
          ...
          if (num_digits < 3 && checkChar(':', in)) {
            ...
            if (num_digits == 2) {
              ...
              return on_error(ErrorCodes::CANNOT_PARSE_DATETIME,
                              "Cannot read DateTime: unexpected number of decimal digits for time zone offset in minutes: {}", num_digits);
          }
        }
      } else {
        char alpha[3];
        ...
        if (num_alpha == 2) {
          ...
          if (num_rest > 0) {
            ...
            bool is_valid_name =
                ...
                (full_len == 5 && 0 == strncasecmp(full_word, "March", 5)) || (full_len == 5 && 0 == strncasecmp(full_word, "April", 5)) ||
                (full_len == 4 && 0 == strncasecmp(full_word, "June", 4)) || (full_len == 4 && 0 == strncasecmp(full_word, "July", 4)) ||
            ...
            if (!is_valid_name) return on_error(ErrorCodes::CANNOT_PARSE_DATETIME, "Cannot read DateTime: unexpected word");
          }

          /// For RFC 2822
          if (has_day_of_week) checkChar(',', in);
        } else
          return on_error(ErrorCodes::LOGICAL_ERROR, "Cannot read DateTime: logical error, unexpected branch in code");
      }
    }
  ...
  if (!day_of_month) {
    if constexpr (strict) return on_error(ErrorCodes::CANNOT_PARSE_DATETIME, "Cannot read DateTime: day of month is required");
    day_of_month = 1;
  }

  if (!month) {
    if constexpr (strict) return on_error(ErrorCodes::CANNOT_PARSE_DATETIME, "Cannot read DateTime: month is required");
    month = 1;
  }

  if (!year) {
    if constexpr (strict) return on_error(ErrorCodes::CANNOT_PARSE_DATETIME, "Cannot read DateTime: year is required");
  ...
  auto check_date = [](const auto &is_leap_year_, const auto &month_, const auto &day_) {
    if ((month_ == 1 || month_ == 3 || month_ == 5 || month_ == 7 || month_ == 8 || month_ == 10 || month_ == 12) && day_ >= 1 &&
        day_ <= 31)
      return true;
    if (month_ == 2 && ((is_leap_year_ && day_ >= 1 && day_ <= 29) || (!is_leap_year_ && day_ >= 1 && day_ <= 28))) return true;
    if ((month_ == 4 || month_ == 6 || month_ == 9 || month_ == 11) && day_ >= 1 && day_ <= 30) return true;
    return false;
  };

  if (!check_date(is_leap_year, month, day_of_month))
    return on_error(ErrorCodes::CANNOT_PARSE_DATETIME, "Cannot read DateTime: unexpected date: {}-{}-{}", year, static_cast<UInt16>(month),
...
ReturnType parseDateTime64BestEffortImpl(DateTime64 &res, UInt32 scale, ReadBuffer &in, const DateLUTImpl &local_time_zone,
                                         const DateLUTImpl &utc_time_zone, const char *allowed_date_delimiters = nullptr) {
  time_t whole;
  ...
  if constexpr (std::is_same_v<ReturnType, bool>) {
    if (!parseDateTimeBestEffortImpl<bool, is_us_style, strict, true>(whole, in, local_time_zone, utc_time_zone, &subsecond,
                                                                      allowed_date_delimiters))
      return false;
  } else {
    parseDateTimeBestEffortImpl<ReturnType, is_us_style, strict, true>(whole, in, local_time_zone, utc_time_zone, &subsecond,
                                                                       allowed_date_delimiters);
  }
  ...
  if constexpr (std::is_same_v<ReturnType, bool>) return DecimalUtils::tryGetDateTimeFromComponents(whole, fractional, scale, res);
...
void parseDateTime64BestEffort(DateTime64 &res, UInt32 scale, ReadBuffer &in, const DateLUTImpl &local_time_zone,
                               const DateLUTImpl &utc_time_zone) {
  parseDateTime64BestEffortImpl<void, false>(res, scale, in, local_time_zone, utc_time_zone);
}

void parseDateTime64BestEffortUS(DateTime64 &res, UInt32 scale, ReadBuffer &in, const DateLUTImpl &local_time_zone,
                                 const DateLUTImpl &utc_time_zone) {
  parseDateTime64BestEffortImpl<void, true>(res, scale, in, local_time_zone, utc_time_zone);
}

bool tryParseDateTime64BestEffort(DateTime64 &res, UInt32 scale, ReadBuffer &in, const DateLUTImpl &local_time_zone,
                                  const DateLUTImpl &utc_time_zone) {
  return parseDateTime64BestEffortImpl<bool, false>(res, scale, in, local_time_zone, utc_time_zone);
}

bool tryParseDateTime64BestEffortUS(DateTime64 &res, UInt32 scale, ReadBuffer &in, const DateLUTImpl &local_time_zone,
                                    const DateLUTImpl &utc_time_zone) {
  return parseDateTime64BestEffortImpl<bool, true>(res, scale, in, local_time_zone, utc_time_zone);
}

bool tryParseDateTimeBestEffortStrict(time_t &res, ReadBuffer &in, const DateLUTImpl &local_time_zone, const DateLUTImpl &utc_time_zone,
...
bool tryParseDateTimeBestEffortUSStrict(time_t &res, ReadBuffer &in, const DateLUTImpl &local_time_zone, const DateLUTImpl &utc_time_zone,
                                        const char *allowed_date_delimiters) {
  return parseDateTimeBestEffortImpl<bool, true, true>(res, in, local_time_zone, utc_time_zone, nullptr, allowed_date_delimiters);
}

bool tryParseDateTime64BestEffortStrict(DateTime64 &res, UInt32 scale, ReadBuffer &in, const DateLUTImpl &local_time_zone,
                                        const DateLUTImpl &utc_time_zone, const char *allowed_date_delimiters) {
  return parseDateTime64BestEffortImpl<bool, false, true>(res, scale, in, local_time_zone, utc_time_zone, allowed_date_delimiters);
}

bool tryParseDateTime64BestEffortUSStrict(DateTime64 &res, UInt32 scale, ReadBuffer &in, const DateLUTImpl &local_time_zone,
                                          const DateLUTImpl &utc_time_zone, const char *allowed_date_delimiters) {
