...
void LZMADeflatingWriteBuffer::initialize(int compression_level) {
  ...
  if (ret != LZMA_OK)
    throw Exception(ErrorCodes::LZMA_STREAM_ENCODER_FAILED, "lzma stream encoder init failed: error code: {} lzma version: {}", ret,
                    LZMA_VERSION_STRING);
}
...
void LZMADeflatingWriteBuffer::finalFlushBefore() {
  ...
  if (!compress_empty && lstr.total_out == 0) return;
  ...
  do {
    ...
    if (ret != LZMA_OK)
      throw Exception(ErrorCodes::LZMA_STREAM_ENCODER_FAILED, "lzma stream encoding failed: error code: {}; lzma version: {}", ret,
                      LZMA_VERSION_STRING);

  } while (lstr.avail_out == 0);
}

void LZMADeflatingWriteBuffer::finalFlushAfter() { lzma_end(&lstr); }
