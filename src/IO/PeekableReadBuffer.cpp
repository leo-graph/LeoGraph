...
bool PeekableReadBuffer::peekNext() {
  ...
  if (useSubbufferOnly()) {
    ...
    if (checkpoint) copy_from = *checkpoint;
    ...
    if (!bytes_to_copy) {
      ...
      if (checkpoint) checkpoint.emplace(pos);
...
void PeekableReadBuffer::checkStateCorrect() const {
  if (checkpoint) {
    if (checkpointInOwnMemory()) {
      if (!peeked_size) throw DB::Exception(ErrorCodes::LOGICAL_ERROR, "Checkpoint in empty own buffer");
      ...
      if (peeked_size) throw DB::Exception(ErrorCodes::LOGICAL_ERROR, "Own buffer is not empty");
      if (currentlyReadFromOwnMemory())
        throw DB::Exception(ErrorCodes::LOGICAL_ERROR, "Current position in own buffer before checkpoint in subbuffer");
      if (pos < *checkpoint) throw DB::Exception(ErrorCodes::LOGICAL_ERROR, "Current position in subbuffer before checkpoint in subbuffer");
    }
  } else {
    if (!currentlyReadFromOwnMemory() && peeked_size) throw DB::Exception(ErrorCodes::LOGICAL_ERROR, "Own buffer is not empty");
  }
  if (currentlyReadFromOwnMemory() && !peeked_size) throw DB::Exception(ErrorCodes::LOGICAL_ERROR, "Pos in empty own buffer");
}
...
void PeekableReadBuffer::resizeOwnMemoryIfNecessary(size_t bytes_to_append) {
  ...
  if (use_stack_memory) {
    ...
    if (sizeof(stack_memory) >= new_size) return;
    ...
    if (need_update_checkpoint) checkpoint.emplace(memory.data() + offset);
    if (need_update_pos) BufferBase::set(memory.data(), peeked_size, pos - stack_memory);
    ...
    if (bytes_to_append < offset && 2 * (peeked_size - offset) <= memory.size()) {
      ...
      if (need_update_checkpoint) *checkpoint -= offset;
      if (need_update_pos) pos -= offset;
    } else {
...
void PeekableReadBuffer::makeContinuousMemoryFromCheckpointToPos() {
  if (!checkpoint) throw DB::Exception(ErrorCodes::LOGICAL_ERROR, "There is no checkpoint");
  checkStateCorrect();

  if (!checkpointInOwnMemory() || currentlyReadFromOwnMemory()) return;  /// it's already continuous
...
PeekableReadBuffer::~PeekableReadBuffer() {
  if (!currentlyReadFromOwnMemory()) sub_buf->position() = pos;
}

bool PeekableReadBuffer::hasUnreadData() const { return peeked_size && pos != getMemoryData() + peeked_size; }

size_t PeekableReadBuffer::offsetFromCheckpointInOwnMemory() const { return peeked_size - (*checkpoint - getMemoryData()); }
