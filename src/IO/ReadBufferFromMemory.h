...
class ReadBufferFromMemory : public SeekableReadBuffer, private ReadBufferFromMemoryHelper<ReadBufferFromMemory> {
 public:
  ...
  explicit ReadBufferFromMemory(std::string_view str) : SeekableReadBuffer(const_cast<char *>(str.data()), str.size(), 0) {}

  off_t seek(off_t off, int whence) override { return seekImpl(off, whence); }

  off_t getPosition() override { return getPositionImpl(); }
...
class ReadBufferFromMemoryFileBase : public ReadBufferFromFileBase, private ReadBufferFromMemoryHelper<ReadBufferFromMemoryFileBase> {
 protected:
  ReadBufferFromMemoryFileBase(bool owns_memory, String file_name_, std::string_view data);
...
class ReadBufferFromOwnMemoryFile final : public ReadBufferFromMemoryFileBase {
 public:
  ...
  ReadBufferFromOwnMemoryFile(String file_name_, std::string_view data) : Base(true /*owns_memory*/, std::move(file_name_), data) {}
};
...
class ReadBufferFromOutsideMemoryFile final : public ReadBufferFromMemoryFileBase {
 public:
  ...
  ReadBufferFromOutsideMemoryFile(String file_name_, std::string_view data) : Base(false /*owns_memory*/, std::move(file_name_), data) {}
};
