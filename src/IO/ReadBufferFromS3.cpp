...
ReadBufferFromS3::ReadBufferFromS3(std::shared_ptr<const S3::Client> client_ptr_, const String &bucket_, const String &key_,
                                   const String &version_id_, const S3::S3RequestSettings &request_settings_, const ReadSettings &settings_,
                                   bool use_external_buffer_, size_t offset_, size_t read_until_position_, bool restricted_seek_,
                                   std::optional<size_t> file_size_, const S3CredentialsRefreshCallback &credentials_refresh_callback_)
...
bool ReadBufferFromS3::nextImpl() {
  ...
  if (impl) {
    ...
    if (impl->isResultReleased()) {
      if (read_until_position) {
        LOG_TRACE(log,
                  "Impl was released, but expected read range is not finished. "
                  "Current offset: {}, end offset: {}",
                  offset.load(), read_until_position.load());
      }
      stop_reason = fmt::format("Connection was released (read offset: {}/{}, release reason: {})", offset.load(),
                                read_until_position.load(), release_reason);
  ...
  for (size_t attempt = 1; !next_result; ++attempt) {
    ...
    try {
      ...
      if (!processException(getPosition(), attempt) || last_attempt) throw;
  ...
  if (!next_result) {
    ...
    stop_reason = fmt::format("EOF (read offset: {}/{}, expected file size: {}, restricted seek: {})", offset.load(),
                              read_until_position.load(), file_size_str, restricted_seek);
    ...
    chassert(!read_until_position, fmt::format("Cannot read all data. Key: {}, size: {}, expected size: {}, position: {}/{}", key,
                                               getObjectSizeFromS3(), file_size_str, offset.load(), read_until_position.load()));
    return false;
  }
  ...
  if (stream_eof || is_read_until_position) {
    release_reason = fmt::format("{} (read {}/{}, file size: {}, restricted seek: {})",
                                 impl->isStreamEof() ? "stream EOF" : "read until position reached", offset.load(),
                                 read_until_position.load(), file_size.has_value() ? toString(*file_size) : "Unknown", restricted_seek);
...
size_t ReadBufferFromS3::readBigAt(char *to, size_t n, size_t range_begin, const std::function<bool(size_t)> &progress_callback) const {
  ...
  for (size_t attempt = 1; n > 0; ++attempt) {
    ...
    try {
      ...
      if (cancelled) return initial_n - n + bytes_copied;

      /// Read remaining bytes after the end of the payload
      istr.ignore(INT64_MAX);
    } catch (...) {
      if (!processException(range_begin, attempt) || last_attempt) throw;
...
bool ReadBufferFromS3::processException(size_t read_offset, size_t attempt) const {
  ...
  LOG_DEBUG(log,
            ...
            "Attempt: {}/{}, Message: {}",
            bucket, key, version_id.empty() ? "Latest" : version_id, read_offset, attempt,
            request_settings[S3RequestSetting::max_single_read_retries].value, getCurrentExceptionMessage(/* with_stacktrace = */ false));
...
off_t ReadBufferFromS3::seek(off_t offset_, int whence) {
  if (offset_ == getPosition() && whence == SEEK_SET) return offset_;
  ...
  LOG_TEST(log, "Seek to {} (restricted seek: {}, impl: {}, working_buffer size: {})", offset_, restricted_seek, bool(impl),
           working_buffer.size());

  if (!restricted_seek) {
    if (!working_buffer.empty() && static_cast<size_t>(offset_) >= offset - working_buffer.size() && offset_ < offset) {
...
size_t ReadBufferFromS3::getObjectSizeFromS3() const { return S3::getObjectSize(*client_ptr, bucket, key, version_id); }

off_t ReadBufferFromS3::getPosition() { return offset - available(); }
...
bool ReadBufferFromS3::atEndOfRequestedRangeGuess() {
  if (!impl) return true;
  if (read_until_position) return getPosition() >= read_until_position;
  if (file_size) return getPosition() >= static_cast<off_t>(*file_size);
  return false;
}
...
Aws::S3::Model::GetObjectResult ReadBufferFromS3::sendRequest(size_t attempt, size_t range_begin,
                                                              std::optional<size_t> range_end_incl) const {
  S3::GetObjectRequest req;
  req.SetBucket(bucket);
  req.SetKey(key);
  if (!version_id.empty()) req.SetVersionId(version_id);

  S3::setClickhouseAttemptNumber(req, attempt);

  if (range_end_incl) {
    req.SetRange(fmt::format("bytes={}-{}", range_begin, *range_end_incl));
    LOG_TEST(log, "Read S3 object. Bucket: {}, Key: {}, Version: {}, Range: {}-{}", bucket, key, version_id.empty() ? "Latest" : version_id,
             range_begin, *range_end_incl);
  } else if (range_begin) {
    req.SetRange(fmt::format("bytes={}-", range_begin));
    LOG_TEST(log, "Read S3 object. Bucket: {}, Key: {}, Version: {}, Offset: {}", bucket, key, version_id.empty() ? "Latest" : version_id,
             range_begin);
  }

  ProfileEvents::increment(ProfileEvents::S3GetObject);
  if (client_ptr->isClientForDisk()) ProfileEvents::increment(ProfileEvents::DiskS3GetObject);
  ...
  if (outcome.IsSuccess()) return outcome.GetResultWithOwnership();
...
ObjectMetadata ReadBufferFromS3::getObjectMetadataFromTheLastRequest() const {
  if (!impl) throw Exception(ErrorCodes::NOT_INITIALIZED, "No S3 object metadata available because there were no successful requests");
