...
Poco::Timespan ConnectionTimeouts::saturate(Poco::Timespan timespan, Poco::Timespan limit) {
  if (limit.totalMicroseconds() == 0) return timespan;
...
class TimeoutsForFirstAttempt {
 private:
  ...
  static size_t getMethodIndex(const String& method) {
    ...
    if (it == known_methods.end()) return 0;
 ...
 public:
  static std::pair<Poco::Timespan, Poco::Timespan> getSendReceiveTimeout(const String& method, bool first_byte) {
    ...
    if (first_byte) return std::make_pair(Poco::Timespan(first_byte_ms[idx][0] * 1000), Poco::Timespan(first_byte_ms[idx][1] * 1000));

    return std::make_pair(Poco::Timespan(rest_bytes_ms[idx][0] * 1000), Poco::Timespan(rest_bytes_ms[idx][1] * 1000));
  }
  ...
  static std::pair<Poco::Timespan, Poco::Timespan> getConnectTimeouts(bool first_attempt) {
    if (first_attempt) {
      ...
      return std::make_pair(Poco::Timespan(CONNECT_TIMEOUT_FOR_FIRST_ATTEMPT_MS * 1000),
                            Poco::Timespan(CONNECT_TIMEOUT_FOR_FIRST_ATTEMPT_MS * 1000));
    }

    return std::make_pair(Poco::Timespan(CONNECT_TIMEOUT_FOR_N_ATTEMPT_MS * 1000), Poco::Timespan(CONNECT_TIMEOUT_FOR_N_ATTEMPT_MS * 1000));
  }
};

const TimeoutsForFirstAttempt::KnownMethodsArray TimeoutsForFirstAttempt::known_methods = {"GET", "POST", "DELETE", "PUT", "HEAD", "PATCH"};

ConnectionTimeouts ConnectionTimeouts::getAdaptiveTimeouts(const String& method, bool first_attempt, bool first_byte) const {
  if (!first_attempt) return *this;
