...
void WriteBufferFromFileDescriptor::nextImpl() {
  ...
  while (bytes_written != offset()) {
    ...
    if ((-1 == res || 0 == res) && errno != EINTR) {
      ...
      if (error_file_name.empty()) error_file_name = "(fd = " + toString(fd) + ")";
      ErrnoException::throwFromPath(ErrorCodes::CANNOT_WRITE_TO_FILE_DESCRIPTOR, error_file_name, "Cannot write to file {}",
                                    error_file_name);
    }
...
WriteBufferFromFileDescriptor::WriteBufferFromFileDescriptor(int fd_, size_t buf_size, char* existing_memory, ThrottlerPtr throttler_,
                                                             size_t alignment, std::string file_name_, bool use_adaptive_buffer_size_,
...
void WriteBufferFromFileDescriptor::sync() {
  ...
  if (-1 == res) ErrnoException::throwFromPath(ErrorCodes::CANNOT_FSYNC, getFileName(), "Cannot fsync {}", getFileName());
}
...
void WriteBufferFromFileDescriptor::truncate(off_t length)  // NOLINT
{
  ...
  if (-1 == res) ErrnoException::throwFromPath(ErrorCodes::CANNOT_TRUNCATE_FILE, getFileName(), "Cannot truncate file {}", getFileName());
}

off_t WriteBufferFromFileDescriptor::size() const {
  struct stat buf;
  int res = fstat(fd, &buf);
  if (-1 == res) ErrnoException::throwFromPath(ErrorCodes::CANNOT_FSTAT, getFileName(), "Cannot execute fstat {}", getFileName());
  return buf.st_size;
}

std::string WriteBufferFromFileDescriptor::getFileName() const {
  if (file_name.empty()) return "(fd = " + toString(fd) + ")";
