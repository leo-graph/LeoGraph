...
ParallelReadBuffer::ParallelReadBuffer(SeekableReadBuffer &input_, ThreadPoolCallbackRunnerUnsafe<void> schedule_,
                                       size_t max_working_readers_, size_t range_step_, size_t file_size_)
...
bool ParallelReadBuffer::addReaderToPool() {
  if (next_range_start >= file_size) return false;
...
void ParallelReadBuffer::addReaders() { while (read_workers.size() < max_working_readers && addReaderToPool()); }
...
off_t ParallelReadBuffer::seek(off_t offset, int whence) {
  ...
  if (offset < 0) throw Exception(ErrorCodes::SEEK_POSITION_OUT_OF_BOUND, "Seek position is out of bounds. Offset: {}", offset);
  ...
  const auto offset_is_in_range = [&](const auto &worker) {
    return static_cast<size_t>(offset) >= worker->start_offset &&
           static_cast<size_t>(offset) < worker->start_offset + worker->segment.size();
  };
  ...
  if (!read_workers.empty()) {
    ...
    while (true) {
      ...
      if (emergency_stop) handleEmergencyStop();

      if (w->bytes_produced > diff) {
        working_buffer = internal_buffer = Buffer(w->segment.data(), w->segment.data() + w->bytes_produced);
...
std::optional<size_t> ParallelReadBuffer::tryGetFileSize() { return file_size; }

off_t ParallelReadBuffer::getPosition() { return current_position - available(); }
...
bool ParallelReadBuffer::nextImpl() {
  while (true) {
    ...
    if (emergency_stop) handleEmergencyStop();  // throws
    ...
    if (w->bytes_produced > w->bytes_consumed) {
      ...
      working_buffer = internal_buffer = Buffer(w->segment.data() + w->bytes_consumed, w->segment.data() + w->bytes_produced);
...
void ParallelReadBuffer::readerThreadFunction(ReadWorkerPtr read_worker) {
  SCOPE_EXIT({
    if (active_working_readers.fetch_sub(1) == 1) active_working_readers.notify_all();
  });

  try {
    auto on_progress = [&](size_t bytes_read) -> bool {
      if (emergency_stop || read_worker->cancel) return true;

      std::lock_guard lock(read_worker->worker_mutex);
      if (bytes_read <= read_worker->bytes_produced) return false;

      bool need_notify = read_worker->bytes_produced == read_worker->bytes_consumed;
      read_worker->bytes_produced = bytes_read;
      if (need_notify) next_condvar.notify_all();
    ...
    if (!on_progress(r) && r < read_worker->segment.size())
      throw Exception(ErrorCodes::UNEXPECTED_END_OF_FILE, "Failed to read all the data from the reader at offset {}, got {}/{} bytes",
...
void ParallelReadBuffer::onBackgroundException() {
  ...
  if (!background_exception) background_exception = std::current_exception();
...
std::unique_ptr<ParallelReadBuffer> wrapInParallelReadBufferIfSupported(ReadBuffer &buf, ThreadPoolCallbackRunnerUnsafe<void> schedule,
                                                                        size_t max_working_readers, size_t range_step, size_t file_size) {
  auto *seekable = dynamic_cast<SeekableReadBuffer *>(&buf);
  if (!seekable || !seekable->supportsReadAt()) return nullptr;

  return std::make_unique<ParallelReadBuffer>(*seekable, schedule, max_working_readers, range_step, file_size);
}
