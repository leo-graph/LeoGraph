...
Lz4InflatingReadBuffer::Lz4InflatingReadBuffer(std::unique_ptr<ReadBuffer> in_, size_t buf_size, char *existing_memory, size_t alignment)
  ...
  if (LZ4F_isError(ret))
    throw Exception(ErrorCodes::LZ4_DECODER_FAILED, "LZ4 failed create decompression context LZ4F_dctx. LZ4F version: {}. Error: {}",
                    LZ4F_VERSION, LZ4F_getErrorName(ret));
}

Lz4InflatingReadBuffer::~Lz4InflatingReadBuffer() { LZ4F_freeDecompressionContext(dctx); }

bool Lz4InflatingReadBuffer::nextImpl() {
  while (true) {
    if (!in_eof) in_eof = in->eof();
    ...
    if (LZ4F_isError(ret))
      throw Exception(ErrorCodes::LZ4_DECODER_FAILED, "LZ4 decompression failed. LZ4F version: {}. Error: {}{}", LZ4F_VERSION,
                      LZ4F_getErrorName(ret), getExceptionEntryWithFileName(*in));
