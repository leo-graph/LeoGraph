...
bool LimitSeekableReadBuffer::nextImpl() {
  /// First let the nested buffer know the current position in the buffer (otherwise `in->eof()` or `in->seek()` below can work
  /// incorrectly).
  ...
  if (ptr - begin > offset_after_min) begin = ptr - offset_after_min;
  if (end - ptr > available_before_max) end = ptr + available_before_max;
...
off_t LimitSeekableReadBuffer::getPosition() {
  if (need_seek) return *need_seek - min_offset;
