...
struct ClientFake : DB::S3::Client {
  explicit ClientFake()
      : DB::S3::Client(1, DB::S3::ServerSideEncryptionKMSConfig(),
                       ...
                       DB::S3::ClientFactory::instance().createClientConfiguration(
                           "test_region", DB::RemoteHostFilter(), 1, DB::S3::PocoHTTPClientConfiguration::RetryStrategy{.max_retries = 0},
                           true, true, true, false, {},
                           /* request_throttler = */ {}, "http"),
                       Aws::Client::AWSAuthV4Signer::PayloadSigningPolicy::Never, DB::S3::ClientSettings()) {}
  ...
  void setGetObjectSuccess(const std::shared_ptr<CountedSession> &session, std::streambuf *sb) {
    ...
    getObjectImpl = [weak_session_ptr,
                     sb]([[maybe_unused]] const Aws::S3::Model::GetObjectRequest &request) -> Aws::S3::Model::GetObjectOutcome {
      ...
      Aws::AmazonWebServiceResult<Aws::Utils::Stream::ResponseStream> aws_result(std::move(response_stream),
  ...
  Aws::S3::Model::ListObjectsV2Outcome ListObjectsV2(const Aws::S3::Model::ListObjectsV2Request &request) const override {
    ...
    list_requests.emplace_back(ListRequest{.start_after = request.GetStartAfter(),
                                           .continuation_token = request.GetContinuationToken(),
                                           .start_after_has_been_set = request.StartAfterHasBeenSet()});

    if (listObjectsV2Impl) return (*listObjectsV2Impl)(request);
...
TEST_F(ReadBufferFromS3Test, IterateUsesStartAfter) {
  ...
  auto object_storage =
      std::make_shared<DB::S3ObjectStorage>(std::move(client), std::make_unique<DB::S3Settings>(), std::move(uri), cap, gen, disk_name);
