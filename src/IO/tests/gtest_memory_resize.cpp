...
class DummyAllocator {
 ...
 public:
  ...
  void free([[maybe_unused]] void *buf, size_t /*size*/) { assert(buf == dummy_address); }
...
TEST(MemoryResizeTest, BigInitAndSmallResizeOverflowWhenPadding) {
  {
    EXPECT_THROW_ERROR_CODE(
        { auto memory = Memory<DummyAllocator>(std::numeric_limits<size_t>::max()); }, Exception, ErrorCodes::ARGUMENT_OUT_OF_BOUND);
  }

  {
    EXPECT_THROW_ERROR_CODE(
        { auto memory = Memory<DummyAllocator>(std::numeric_limits<size_t>::max() - 1); }, Exception, ErrorCodes::ARGUMENT_OUT_OF_BOUND);
  }

  {
    EXPECT_THROW_ERROR_CODE(
        { auto memory = Memory<DummyAllocator>(std::numeric_limits<size_t>::max() - 10); }, Exception, ErrorCodes::ARGUMENT_OUT_OF_BOUND);
  }

#ifndef DEBUG_OR_SANITIZER_BUILD
  {
    EXPECT_THROW_ERROR_CODE(
        { auto memory = Memory<DummyAllocator>(std::numeric_limits<size_t>::max() - (PADDING_FOR_SIMD - 1)); }, Exception,
        ErrorCodes::LOGICAL_ERROR);
  }

  {
    EXPECT_THROW_ERROR_CODE(
        { auto memory = Memory<DummyAllocator>(0x8000000000000000ULL - (PADDING_FOR_SIMD - 1)); }, Exception, ErrorCodes::LOGICAL_ERROR);
  }
#endif
...
TEST(MemoryResizeTest, AlignmentWithRealAllocator) {
  {
    auto memory = Memory<>(0, 3);  // not a power of 2 but less than MALLOC_MIN_ALIGNMENT so user-defined alignment is ignored at Allocator
    ASSERT_EQ(memory.m_data, nullptr);
    ASSERT_EQ(memory.m_capacity, 0);
    ASSERT_EQ(memory.m_size, 0);
