...
std::ostream &dump_datetime(std::ostream &ostr, const DayNum &d) { return ostr << getTypeName(d) << "{" << d.toUnderType() << "}"; }

std::ostream &dump_datetime(std::ostream &ostr, const time_t &dt) { return ostr << getTypeName(dt) << "{" << dt << "}"; }
...
TEST_P(DateTimeToStringParamTestDayNum, writeDateText) { ASSERT_NO_FATAL_FAILURE(test(GetParam())); }

class DateTimeToStringParamTestTimeT : public DateTimeToStringParamTestBase<time_t> {};

TEST_P(DateTimeToStringParamTestTimeT, writeDateText) { ASSERT_NO_FATAL_FAILURE(test(GetParam())); }

class DateTimeToStringParamTestDateTime64 : public DateTimeToStringParamTestBase<DateTime64WithScale> {};

TEST_P(DateTimeToStringParamTestDateTime64, writeDateText) { ASSERT_NO_FATAL_FAILURE(test(GetParam())); }

class DateTimeToStringParamTestDateTime64TrimZeros : public DateTimeToStringParamTestBase<DateTime64WithScale, true> {};

TEST_P(DateTimeToStringParamTestDateTime64TrimZeros, writeDateText) { ASSERT_NO_FATAL_FAILURE(test(GetParam())); }
...
INSTANTIATE_TEST_SUITE_P(
    DateTimeToString, DateTimeToStringParamTestDayNum,
    ::testing::ValuesIn(std::initializer_list<DateTimeToStringParamTestCase<DayNum>>{
        {"Zero DayNum pointing to 1970-01-01", DayNum(0), "1970-01-01"},
        {"Non-Zero DayNum", DayNum(1), "1970-01-02"},
        {"Non-Zero DayNum", DayNum(10 * 365), "1979-12-30"},
        {"Negative DayNum value wraps as if it was UInt16 due to LUT limitations and to maintain compatibility with existing code.",
         DayNum(-10 * 365), "2139-06-10"},
    }));
...
INSTANTIATE_TEST_SUITE_P(DateTimeToString, DateTimeToStringParamTestTimeT,
                         ::testing::ValuesIn(std::initializer_list<DateTimeToStringParamTestCase<time_t>>{
                             {"Zero time_t pointing to 1970-01-01 00:00:00 in UTC", time_t(0), "1970-01-01 00:00:00"},
                             {"Non-Zero time_t is a valid date/time", time_t{NON_ZERO_TIME_T}, "1979-12-31 10:17:36"},
...
INSTANTIATE_TEST_SUITE_P(
    DateTimeToString, DateTimeToStringParamTestDateTime64,
    ::testing::ValuesIn(std::initializer_list<DateTimeToStringParamTestCase<DateTime64WithScale>>{
        /// Inside basic LUT boundaries
        {"Zero DateTime64 with scale 0 string representation matches one of zero time_t", DateTime64WithScale{0, 0}, "1970-01-01 00:00:00"},
        {"Zero DateTime64 with scale 3 string representation matches one of zero time_t with subsecond part", DateTime64WithScale{0, 3},
         "1970-01-01 00:00:00.000"},
        {"Non-Zero DateTime64 with scale 0", DateTime64WithScale{NON_ZERO_TIME_T, 0}, "1979-12-31 10:17:36"},
        {"Non-Zero DateTime64 with scale 3", DateTime64WithScale{NON_ZERO_TIME_T * 1000LL + 123, 3}, "1979-12-31 10:17:36.123"},
        //        {
        //            "Negative time_t value wraps around as if it was UInt32 due to LUT limitations and to maintain compatibility with
        //            existing code",
...
INSTANTIATE_TEST_SUITE_P(
    DateTimeToString, DateTimeToStringParamTestDateTime64TrimZeros,
    ::testing::ValuesIn(std::initializer_list<DateTimeToStringParamTestCase<DateTime64WithScale>>{
        /// Inside basic LUT boundaries
        {"Zero DateTime64 with scale 0", DateTime64WithScale{0, 0}, "1970-01-01 00:00:00"},
        {"Zero DateTime64 with scale 6, fractional is trimmed", DateTime64WithScale{0, 6}, "1970-01-01 00:00:00"},
        {"DateTime64 with scale 3, fractional is trimmed", DateTime64WithScale{NON_ZERO_TIME_T * 1000LL, 3}, "1979-12-31 10:17:36"},
        {"DateTime64 with scale 6, fractional is partially trimmed", DateTime64WithScale{120000, 6}, "1970-01-01 00:00:00.120"},
        {"DateTime64 with scale 6, fractional is kept", DateTime64WithScale{123456, 6}, "1970-01-01 00:00:00.123456"},
    }));
