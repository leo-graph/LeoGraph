...
class UploadHelper {
 public:
  UploadHelper(const std::shared_ptr<const S3::Client> &client_ptr_, const String &dest_bucket_, const String &dest_key_,
               const S3::S3RequestSettings &request_settings_, const std::optional<std::map<String, String>> &object_metadata_,
               ThreadPoolCallbackRunnerUnsafe<void> schedule_, BlobStorageLogWriterPtr blob_storage_log_, const LoggerPtr log_)
 ...
 protected:
  ...
  void fillCreateMultipartRequest(S3::CreateMultipartUploadRequest &request) {
    ...
    if (object_metadata.has_value()) request.SetMetadata(object_metadata.value());
  ...
  void createMultipartUpload() {
    ...
    if (client_ptr->isClientForDisk()) ProfileEvents::increment(ProfileEvents::DiskS3CreateMultipartUpload);
    ...
    if (blob_storage_log)
      blob_storage_log->addEvent(BlobStorageLogElement::EventType::MultiPartUploadCreate, dest_bucket, dest_key, /* local_path_ */ {},
                                 /* data_size */ 0, elapsed,
  ...
  void completeMultipartUpload() {
    LOG_TRACE(log, "Completing multipart upload. Bucket: {}, Key: {}, Upload_id: {}, Parts: {}", dest_bucket, dest_key, multipart_upload_id,
              multipart_tags.size());

    if (multipart_tags.empty()) throw Exception(ErrorCodes::S3_ERROR, "Failed to complete multipart upload. No parts have uploaded");
    ...
    for (size_t retries = 1;; ++retries) {
      ...
      if (client_ptr->isClientForDisk()) ProfileEvents::increment(ProfileEvents::DiskS3CompleteMultipartUpload);
      ...
      if (blob_storage_log)
        blob_storage_log->addEvent(BlobStorageLogElement::EventType::MultiPartUploadComplete, dest_bucket, dest_key, /* local_path_ */ {},
                                   /* data_size */ 0, elapsed,
                                   outcome.IsSuccess() ? 0 : static_cast<Int32>(outcome.GetError().GetErrorType()),
                                   outcome.IsSuccess() ? "" : outcome.GetError().GetMessage());

      if (outcome.IsSuccess()) {
        LOG_TRACE(log, "Multipart upload has completed. Bucket: {}, Key: {}, Upload_id: {}, Parts: {}", dest_bucket, dest_key,
                  multipart_upload_id, multipart_tags.size());
        break;
      }
      ...
      if ((outcome.GetError().GetErrorType() == Aws::S3::S3Errors::NO_SUCH_KEY) && (retries < max_retries)) {
        ...
        LOG_INFO(log, "Multipart upload failed with NO_SUCH_KEY error for Bucket: {}, Key: {}, Upload_id: {}, Parts: {}, will retry",
                 dest_bucket, dest_key, multipart_upload_id, multipart_tags.size());
        continue;  /// will retry
      }
      ProfileEvents::increment(ProfileEvents::WriteBufferFromS3RequestsErrors, 1);
      throw S3Exception(outcome.GetError().GetErrorType(), "Message: {}, Key: {}, Bucket: {}, Tags: {}", outcome.GetError().GetMessage(),
                        dest_key, dest_bucket, fmt::join(multipart_tags.begin(), multipart_tags.end(), " "));
    }
  }
  ...
  void abortMultipartUpload() {
    ...
    if (blob_storage_log)
      blob_storage_log->addEvent(BlobStorageLogElement::EventType::MultiPartUploadAbort, dest_bucket, dest_key, /* local_path_ */ {},
                                 /* data_size */ 0, elapsed,
                                 outcome.IsSuccess() ? 0 : static_cast<Int32>(outcome.GetError().GetErrorType()),
                                 outcome.IsSuccess() ? "" : outcome.GetError().GetMessage());
  }
  ...
  void performMultipartUpload(size_t start_offset, size_t size) {
    ...
    try {
      for (size_t part_number = 1; position < end_position; ++part_number) {
        ...
        LOG_TRACE(log, "Writing part #{} of {}. Bucket: {}, Key: {}, Upload_id: {}, Size: {}", part_number, num_parts, dest_bucket,
                  dest_key, multipart_upload_id, part_size);
  ...
  void calculatePartSize(size_t total_size) {
    if (!total_size) throw Exception(ErrorCodes::LOGICAL_ERROR, "Chosen multipart upload for an empty file. This must not happen");
    ...
    if (!max_part_number) throw Exception(ErrorCodes::INVALID_CONFIG_PARAMETER, "max_part_number must not be 0");
    if (!min_upload_part_size) throw Exception(ErrorCodes::INVALID_CONFIG_PARAMETER, "min_upload_part_size must not be 0");
  ...
  void processUploadTask(UploadPartTask &task, String &part_tag) {
    ...
    try {
      ...
      LOG_TRACE(log, "Finished writing part #{}. Bucket: {}, Key: {}, Upload_id: {}, Etag: {}, Finished parts: {} of {}", task.part_number,
                dest_bucket, dest_key, multipart_upload_id, tag, finished_count, num_parts);
    } catch (Exception &e) {
  ...
  virtual std::unique_ptr<Aws::AmazonWebServiceRequest> makeUploadPartRequest(size_t part_number, size_t part_offset,
                                                                              size_t part_size) const = 0;
...
class CopyDataToFileHelper : public UploadHelper {
 public:
  CopyDataToFileHelper(const CreateReadBuffer &create_read_buffer_, size_t offset_, size_t size_,
                       const std::shared_ptr<const S3::Client> &client_ptr_, const String &dest_bucket_, const String &dest_key_,
                       const S3::S3RequestSettings &request_settings_, const std::optional<std::map<String, String>> &object_metadata_,
                       ThreadPoolCallbackRunnerUnsafe<void> schedule_, BlobStorageLogWriterPtr blob_storage_log_)
      : UploadHelper(client_ptr_, dest_bucket_, dest_key_, request_settings_, object_metadata_, schedule_, blob_storage_log_,
  ...
  void performCopy() {
    ...
    if (request_settings[S3RequestSetting::check_objects_after_upload]) checkObjectAfterUpload();
  }
 ...
 private:
  ...
  void processPutRequest(S3::PutObjectRequest &request) {
    ...
    for (size_t retries = 1;; ++retries) {
      ...
      if (client_ptr->isClientForDisk()) ProfileEvents::increment(ProfileEvents::DiskS3PutObject);
      ...
      if (blob_storage_log)
        blob_storage_log->addEvent(BlobStorageLogElement::EventType::Upload, dest_bucket, dest_key, /* local_path_ */ {}, size, elapsed,
                                   outcome.IsSuccess() ? 0 : static_cast<Int32>(outcome.GetError().GetErrorType()),
                                   outcome.IsSuccess() ? "" : outcome.GetError().GetMessage());
      ...
      if (outcome.IsSuccess()) {
        ...
        LOG_TRACE(log, "Single part upload has completed. Bucket: {}, Key: {}, Object size: {}", dest_bucket, dest_key, object_size);
        break;
      }

      if (outcome.GetError().GetExceptionName() == "EntityTooLarge" || outcome.GetError().GetExceptionName() == "InvalidRequest") {
        // Can't come here with MinIO, MinIO allows single part upload for large objects.
        LOG_INFO(log, "Single part upload failed with error {} for Bucket: {}, Key: {}, Object size: {}, will retry with multipart upload",
                 outcome.GetError().GetExceptionName(), dest_bucket, dest_key, size);
        performMultipartUpload();
        break;
      }
      ...
      if ((outcome.GetError().GetErrorType() == Aws::S3::S3Errors::NO_SUCH_KEY) && (retries < max_retries)) {
        /// For unknown reason, at least MinIO can respond with NO_SUCH_KEY for put requests
        LOG_INFO(log, "Single part upload failed with NO_SUCH_KEY error for Bucket: {}, Key: {}, Object size: {}, will retry", dest_bucket,
                 dest_key, request.GetContentLength());
        continue;  /// will retry
      }
      ProfileEvents::increment(ProfileEvents::WriteBufferFromS3RequestsErrors, 1);
      throw S3Exception(outcome.GetError().GetErrorType(), "Message: {}, Key: {}, Bucket: {}, Object size: {}",
                        outcome.GetError().GetMessage(), dest_key, dest_bucket, request.GetContentLength());
    }
  }

  void performMultipartUpload() { UploadHelper::performMultipartUpload(offset, size); }

  std::unique_ptr<Aws::AmazonWebServiceRequest> makeUploadPartRequest(size_t part_number, size_t part_offset,
                                                                      size_t part_size) const override {
  ...
  String processUploadPartRequest(Aws::AmazonWebServiceRequest &request) override {
    ...
    if (client_ptr->isClientForDisk()) ProfileEvents::increment(ProfileEvents::DiskS3UploadPart);
    ...
    if (blob_storage_log)
      blob_storage_log->addEvent(BlobStorageLogElement::EventType::MultiPartUploadWrite, dest_bucket, dest_key, /* local_path_ */ {}, size,
                                 elapsed, outcome.IsSuccess() ? 0 : static_cast<Int32>(outcome.GetError().GetErrorType()),
                                 outcome.IsSuccess() ? "" : outcome.GetError().GetMessage());
...
class CopyFileHelper : public UploadHelper {
 public:
  CopyFileHelper(const std::shared_ptr<const S3::Client> &client_ptr_, const String &src_bucket_, const String &src_key_,
                 size_t src_offset_, size_t src_size_, const String &dest_bucket_, const String &dest_key_,
                 const S3::S3RequestSettings &request_settings_, const ReadSettings &read_settings_,
                 const std::optional<std::map<String, String>> &object_metadata_, ThreadPoolCallbackRunnerUnsafe<void> schedule_,
                 BlobStorageLogWriterPtr blob_storage_log_, std::function<void()> fallback_method_)
      : UploadHelper(client_ptr_, dest_bucket_, dest_key_, request_settings_, object_metadata_, schedule_, blob_storage_log_,
 ...
 private:
  ...
  void processCopyRequest(S3::CopyObjectRequest &request) {
    ...
    for (size_t retries = 1;; ++retries) {
      ...
      if (client_ptr->isClientForDisk()) ProfileEvents::increment(ProfileEvents::DiskS3CopyObject);

      auto outcome = client_ptr->CopyObject(request);
      if (outcome.IsSuccess()) {
        LOG_TRACE(log, "Single operation copy has completed. Bucket: {}, Key: {}, Object size: {}", dest_bucket, dest_key, size);
        break;
      }

      if (outcome.GetError().GetExceptionName() == "EntityTooLarge" || outcome.GetError().GetExceptionName() == "InvalidRequest" ||
          outcome.GetError().GetExceptionName() == "InvalidArgument" || outcome.GetError().GetExceptionName() == "AccessDenied" ||
      ...
      if ((outcome.GetError().GetErrorType() == Aws::S3::S3Errors::NO_SUCH_KEY) && (retries < max_retries)) {
        ...
        /// For unknown reason, at least MinIO can respond with NO_SUCH_KEY for put requests
        LOG_INFO(log, "Single operation copy failed with NO_SUCH_KEY error for Bucket: {}, Key: {}, Object size: {}, will retry",
                 dest_bucket, dest_key, size);
        continue;  /// will retry
      }

      throw S3Exception(outcome.GetError().GetErrorType(), "Message: {}, Key: {}, Bucket: {}, Object size: {}",
                        outcome.GetError().GetMessage(), dest_key, dest_bucket, size);
    }
  }
  ...
  void performMultipartUploadCopy() {
    try {
      ...
      if (e.getS3ErrorCode() != Aws::S3::S3Errors::ACCESS_DENIED) throw;
  ...
  String processUploadPartRequest(Aws::AmazonWebServiceRequest &request) override {
    ...
    if (client_ptr->isClientForDisk()) ProfileEvents::increment(ProfileEvents::DiskS3UploadPartCopy);
...
void copyDataToS3File(const std::function<std::unique_ptr<SeekableReadBuffer>()> &create_read_buffer, size_t offset, size_t size,
                      const std::shared_ptr<const S3::Client> &dest_s3_client, const String &dest_bucket, const String &dest_key,
                      const S3::S3RequestSettings &settings, BlobStorageLogWriterPtr blob_storage_log,
                      ThreadPoolCallbackRunnerUnsafe<void> schedule, const std::optional<std::map<String, String>> &object_metadata) {
  CopyDataToFileHelper helper{create_read_buffer, offset,          size,     dest_s3_client,  dest_bucket, dest_key,
                              settings,           object_metadata, schedule, blob_storage_log};
  helper.performCopy();
}

void copyS3File(std::shared_ptr<const S3::Client> src_s3_client, const String &src_bucket, const String &src_key, size_t src_offset,
                size_t src_size, std::shared_ptr<const S3::Client> dest_s3_client, const String &dest_bucket, const String &dest_key,
                const S3::S3RequestSettings &settings, const ReadSettings &read_settings, BlobStorageLogWriterPtr blob_storage_log,
                ThreadPoolCallbackRunnerUnsafe<void> schedule, const CreateReadBuffer &fallback_file_reader,
                const std::optional<std::map<String, String>> &object_metadata) {
  if (!dest_s3_client) dest_s3_client = src_s3_client;

  std::function<void()> fallback_method = [&] mutable {
    copyDataToS3File(fallback_file_reader, src_offset, src_size, dest_s3_client, dest_bucket, dest_key, settings, blob_storage_log,
                     schedule, object_metadata);
  };
  ...
  CopyFileHelper helper{src_s3_client,
                        src_bucket,
                        src_key,
                        src_offset,
                        src_size,
