...
Aws::Http::HeaderValueCollection CopyObjectRequest::GetRequestSpecificHeaders() const {
  ...
  if (api_mode != ApiMode::GCS) return headers;
  ...
  for (auto& [header, value] : new_meta_headers) headers.emplace(std::move(header), std::move(value));
...
void HeadObjectRequest::SetAdditionalCustomHeaderValue(const Aws::String& headerName, const Aws::String& headerValue) {
  ...
  if (headerName != "x-amz-server-side-encryption") Model::HeadObjectRequest::SetAdditionalCustomHeaderValue(headerName, headerValue);
}
...
Aws::String ComposeObjectRequest::SerializePayload() const {
  if (component_names.empty()) return {};
...
const Aws::String& ComposeObjectRequest::GetBucket() const { return bucket; }

bool ComposeObjectRequest::BucketHasBeenSet() const { return !bucket.empty(); }

void ComposeObjectRequest::SetBucket(const Aws::String& value) { bucket = value; }

void ComposeObjectRequest::SetBucket(Aws::String&& value) { bucket = std::move(value); }

void ComposeObjectRequest::SetBucket(const char* value) { bucket.assign(value); }

const Aws::String& ComposeObjectRequest::GetKey() const { return key; }

bool ComposeObjectRequest::KeyHasBeenSet() const { return !key.empty(); }

void ComposeObjectRequest::SetKey(const Aws::String& value) { key = value; }

void ComposeObjectRequest::SetKey(Aws::String&& value) { key = std::move(value); }

void ComposeObjectRequest::SetKey(const char* value) { key.assign(value); }

void ComposeObjectRequest::SetComponentNames(std::vector<Aws::String> component_names_) { component_names = std::move(component_names_); }

void ComposeObjectRequest::SetContentType(Aws::String value) { content_type = std::move(value); }
...
size_t getAttemptFromInfo(const Aws::String& request_info) {
  ...
  if (key_begin == Aws::String::npos) return 1;

  auto val_begin = key_begin + key.size();
  auto val_end = request_info.find(';', val_begin);
  if (val_end == Aws::String::npos) val_end = request_info.size();

  if (val_begin == val_end) return 1;
