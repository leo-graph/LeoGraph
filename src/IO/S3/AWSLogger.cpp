...
AWSLogger::AWSLogger(bool enable_s3_requests_logging_) : enable_s3_requests_logging(enable_s3_requests_logging_) {
  for (auto [tag, name] : S3_LOGGER_TAG_NAMES) tag_loggers[tag] = getLogger(name);

  default_logger = tag_loggers[S3_LOGGER_TAG_NAMES[0][0]];
}

Aws::Utils::Logging::LogLevel AWSLogger::GetLogLevel() const {
  if (enable_s3_requests_logging) return Aws::Utils::Logging::LogLevel::Trace;
...
bool is404Muted(const char* message) {
  ...
  if (!Expect404ResponseScope::is404Expected()) return false;

  static const char* prefix_str = "HTTP response code: ";
  static const size_t prefix_len = strlen(prefix_str);

  if (!startsWith(message, prefix_str)) return false;
  ...
  for (size_t i = 0; i < code_len; ++i)
    if (!code_str[i]) return false;

  UInt64 code = 0;
  if (!tryParse<UInt64>(code, code_str, code_len)) return false;
...
{
  if (is404Muted(format_str)) return;
...
void AWSLogger::LogStream(Aws::Utils::Logging::LogLevel log_level, const char* tag, const Aws::OStringStream& message_stream) {
  if (is404Muted(message_stream.str().c_str())) return;
