...
Client::RetryStrategy::RetryStrategy(const PocoHTTPClientConfiguration::RetryStrategy &config_)
    : config(config_), log(getLogger("S3ClientRetryStrategy")) {
...
bool Client::RetryStrategy::ShouldRetry(const Aws::Client::AWSError<Aws::Client::CoreErrors> &error, long attemptedRetries) const {
  if (error.GetResponseCode() == Aws::Http::HttpResponseCode::MOVED_PERMANENTLY) return false;

  if (attemptedRetries >= config.max_retries) return false;

  if (CurrentThread::isInitialized() && CurrentThread::get().isQueryCanceled()) return false;

  /// It does not make sense to retry when GCS suggest to use Rewrite
  if (useGCSRewrite(error)) return false;

  return error.ShouldRetry();
}

bool Client::RetryStrategy::useGCSRewrite(const Aws::Client::AWSError<Aws::Client::CoreErrors> &error) {
  return error.GetResponseCode() == Aws::Http::HttpResponseCode::GATEWAY_TIMEOUT && error.GetExceptionName() == "InternalError" &&
         error.GetMessage().contains("use the Rewrite method in the JSON API");
}

/// NOLINTNEXTLINE(google-runtime-int)
long Client::RetryStrategy::CalculateDelayBeforeNextRetry(const Aws::Client::AWSError<Aws::Client::CoreErrors> &,
  ...
  if (config.jitter_factor > 0) {
    ...
    res = static_cast<std::uint64_t>(
        std::min(jitter * static_cast<double>(config.initial_delay_ms) * static_cast<double>(backoff_limited_pow),
                 static_cast<double>(config.max_delay_ms)));
  } else
...
/// NOLINTNEXTLINE(google-runtime-int)
long Client::RetryStrategy::GetMaxAttempts() const { return config.max_retries + 1; }
...
void Client::RetryStrategy::RequestBookkeeping(const Aws::Client::HttpResponseOutcome &httpResponseOutcome) {
  if (!httpResponseOutcome.IsSuccess()) {
    ...
    if (error.ShouldRetry())
      LOG_TRACE(log, "Attempt {}/{} failed with retryable error: {}, {}", httpResponseOutcome.GetRetryCount() + 1, GetMaxAttempts(),
                static_cast<size_t>(error.GetResponseCode()), error.GetMessage());
  }
}
...
void Client::RetryStrategy::RequestBookkeeping(const Aws::Client::HttpResponseOutcome &httpResponseOutcome,
  ...
  if (httpResponseOutcome.IsSuccess())
    LOG_TRACE(log, "Attempt {}/{} succeeded with response code {}, last error: {}, {}", httpResponseOutcome.GetRetryCount() + 1,
              GetMaxAttempts(), static_cast<size_t>(httpResponseOutcome.GetResult()->GetResponseCode()),
              static_cast<size_t>(lastError.GetResponseCode()), lastError.GetMessage());
...
void addAdditionalAMZHeadersToCanonicalHeadersList(Aws::AmazonWebServiceRequest &request, const HTTPHeaderEntries &extra_headers) {
...
std::unique_ptr<Client> Client::create(size_t max_redirects_, ServerSideEncryptionKMSConfig sse_kms_config_,
...
std::unique_ptr<Client> Client::clone() const { return cloneWithConfigurationOverride(this->client_configuration); }
...
ProviderType deduceProviderType(const std::string &url) {
  if (url.contains(".amazonaws.com")) return ProviderType::AWS;

  if (url.contains("storage.googleapis.com")) return ProviderType::GCS;
...
Client::Client(size_t max_redirects_, ServerSideEncryptionKMSConfig sse_kms_config_,
               const std::shared_ptr<Aws::Auth::AWSCredentialsProvider> &credentials_provider_,
               const PocoHTTPClientConfiguration &client_configuration_, Aws::Client::AWSAuthV4Signer::PayloadSigningPolicy sign_payloads_,
  ...
  if (provider_type == ProviderType::GCS) {
    ...
    if (credentials_provider) {
      ...
      if (credentials.IsEmpty()) api_mode = ApiMode::GCS;
    }
  }
...
Client::Client(const Client &other, const PocoHTTPClientConfiguration &client_configuration_)
...
Aws::Auth::AWSCredentials Client::getCredentials() const { return credentials_provider->GetAWSCredentials(); }
...
bool Client::checkIfWrongRegionDefined(const std::string &bucket, const Aws::S3::S3Error &error, std::string &region) const {
  ...
  if (error.GetResponseCode() == Aws::Http::HttpResponseCode::BAD_REQUEST && error.GetExceptionName() == "AuthorizationHeaderMalformed") {
    ...
    if (region.empty()) region = getRegionForBucket(bucket, /*force_detect*/ true);

    assert(!explicit_region.empty());
    if (region == explicit_region) return false;
...
void Client::setKMSHeaders(RequestType &request) const {
  ...
  if (sse_kms_config.key_id) {
    ...
    if (!sse_kms_config.key_id->empty()) request.SetSSEKMSKeyId(*sse_kms_config.key_id);
    if (sse_kms_config.encryption_context) request.SetSSEKMSEncryptionContext(*sse_kms_config.encryption_context);
    if (sse_kms_config.bucket_key_enabled) request.SetBucketKeyEnabled(*sse_kms_config.bucket_key_enabled);
  }
}
...
Model::HeadObjectOutcome Client::HeadObject(HeadObjectRequest &request) const {
  ...
  if (isS3ExpressBucket()) request.setIsS3ExpressBucket();

  addAdditionalAMZHeadersToCanonicalHeadersList(request, client_configuration.extra_headers);

  if (auto region = getRegionForBucket(bucket); !region.empty()) {
    if (!detect_region) LOG_INFO(log, "Using region override {} for bucket {}", region, bucket);
    request.overrideRegion(std::move(region));
  }

  if (auto uri = getURIForBucket(bucket); uri.has_value()) request.overrideURI(std::move(*uri));
  ...
  if (error.GetResponseCode() != Aws::Http::HttpResponseCode::MOVED_PERMANENTLY) return result;
  ...
  if (!bucket_uri) {
    if (auto maybe_error = updateURIForBucketForHead(bucket); maybe_error.has_value()) return *maybe_error;
  ...
  return processRequestResult(HeadObject(static_cast<const Model::HeadObjectRequest &>(request)));
}
...
Model::GetObjectTaggingOutcome Client::GetObjectTagging(GetObjectTaggingRequest &request) const {
  return processRequestResult(doRequest(request, [this](const Model::GetObjectTaggingRequest &req) { return GetObjectTagging(req); }));
}

Model::ListObjectsV2Outcome Client::ListObjectsV2(ListObjectsV2Request &request) const {
  return doRequestWithRetryNetworkErrors</*IsReadMethod*/ true>(request,
                                                                [this](Model::ListObjectsV2Request &req) { return ListObjectsV2(req); });
}

Model::ListObjectsOutcome Client::ListObjects(ListObjectsRequest &request) const {
  return doRequestWithRetryNetworkErrors</*IsReadMethod*/ true>(request,
                                                                [this](Model::ListObjectsRequest &req) { return ListObjects(req); });
}

Model::GetObjectOutcome Client::GetObject(GetObjectRequest &request) const {
  return processRequestResult(doRequest(request, [this](Model::GetObjectRequest &req) { return GetObject(req); }));
}
...
Model::CompleteMultipartUploadOutcome Client::CompleteMultipartUpload(CompleteMultipartUploadRequest &request) const {
  ...
  if (!outcome.IsSuccess() && outcome.GetError().GetErrorType() == Aws::S3::S3Errors::NO_SUCH_UPLOAD) {
    auto check_request = HeadObjectRequest().WithBucket(bucket).WithKey(key);
...
Model::CopyObjectOutcome Client::CopyObject(CopyObjectRequest &request) const {
  return doRequestWithRetryNetworkErrors</*IsReadMethod*/ false>(request,
                                                                 [this](Model::CopyObjectRequest &req) { return CopyObject(req); });
}

Model::PutObjectOutcome Client::PutObject(PutObjectRequest &request) const {
  return doRequestWithRetryNetworkErrors</*IsReadMethod*/ false>(request, [this](Model::PutObjectRequest &req) { return PutObject(req); });
}
...
Model::UploadPartOutcome Client::UploadPart(UploadPartRequest &request) const {
  return doRequestWithRetryNetworkErrors</*IsReadMethod*/ false>(request,
                                                                 [this](Model::UploadPartRequest &req) { return UploadPart(req); });
}

Model::UploadPartCopyOutcome Client::UploadPartCopy(UploadPartCopyRequest &request) const {
  return doRequestWithRetryNetworkErrors</*IsReadMethod*/ false>(request,
                                                                 [this](Model::UploadPartCopyRequest &req) { return UploadPartCopy(req); });
}

Model::DeleteObjectOutcome Client::DeleteObject(DeleteObjectRequest &request) const {
  return doRequestWithRetryNetworkErrors</*IsReadMethod*/ false>(request, [this](Model::DeleteObjectRequest &req) {
    Expect404ResponseScope scope;
    return DeleteObject(req);
  });
}

Model::DeleteObjectsOutcome Client::DeleteObjects(DeleteObjectsRequest &request) const {
  return doRequestWithRetryNetworkErrors</*IsReadMethod*/ false>(request, [this](Model::DeleteObjectsRequest &req) {
    Expect404ResponseScope scope;
    return DeleteObjects(req);
  });
}
...
Client::ComposeObjectOutcome Client::ComposeObject(ComposeObjectRequest &request) const {
  auto request_fn = [this](ComposeObjectRequest &req) {
    ...
    AWS_OPERATION_CHECK_PTR(endpoint_provider, ComposeObject, Aws::Client::CoreErrors,
    ...
    if (!req.BucketHasBeenSet()) {
      ...
      return ComposeObjectOutcome(Aws::Client::AWSError<Aws::S3::S3Errors>(Aws::S3::S3Errors::MISSING_PARAMETER, "MISSING_PARAMETER",
    ...
    AWS_OPERATION_CHECK_SUCCESS(endpointResolutionOutcome, ComposeObject, Aws::Client::CoreErrors,
                                Aws::Client::CoreErrors::ENDPOINT_RESOLUTION_FAILURE, endpointResolutionOutcome.GetError().GetMessage());
  ...
  return doRequestWithRetryNetworkErrors</*IsReadMethod*/ false>(request, request_fn);
}

template <typename RequestType, typename RequestFn>
std::invoke_result_t<RequestFn, RequestType &> Client::doRequest(RequestType &request, RequestFn request_fn) const {
  addAdditionalAMZHeadersToCanonicalHeadersList(request, client_configuration.extra_headers);
  const auto &bucket = request.GetBucket();
  request.setApiMode(api_mode);
  ...
  SCOPE_EXIT(if (found_new_endpoint) {
  ...
  for (size_t attempt = 0; attempt <= max_redirects; ++attempt) {
    ...
    if (error.GetResponseCode() != Aws::Http::HttpResponseCode::MOVED_PERMANENTLY && !is_illegal_constraint_exception) return result;
...
std::invoke_result_t<RequestFn, RequestType &> Client::doRequestWithRetryNetworkErrors(RequestType &request, RequestFn request_fn) const {
  ...
  auto with_retries = [this, request_fn_ = std::move(request_fn)](RequestType &request_) {
    ...
    for (attempt_no = 1; attempt_no <= max_attempts; ++attempt_no) {
      ...
      if (attempt_no > 1) {
        ...
        if (isClientForDisk())
          incrementProfileEvents<IsReadMethod>(ProfileEvents::DiskS3ReadRequestRetryableErrors,
                                               ProfileEvents::DiskS3WriteRequestRetryableErrors);
      ...
      try {
        ...
        if (!client_configuration.retryStrategy->ShouldRetry(outcome.GetError(), /*attemptedRetries*/ -1)) break;
      } catch (Poco::Net::NetException &) {
        /// This includes "connection reset", "malformed message", and possibly other exceptions.
        if (!net_exception_handler()) break;
...
RequestResult Client::processRequestResult(RequestResult &&outcome) const {
  if (outcome.IsSuccess() || !isClientForDisk()) return std::forward<RequestResult>(outcome);
  ...
  String enriched_message =
      fmt::format("{} {}", outcome.GetError().GetMessage(),
...
void Client::slowDownAfterRetryableError() const {
  ...
  for (;;) {
    ...
    if (current_time_ms >= next_time_ms) {
      if (next_time_ms != 0) LOG_TRACE(log, "Retry time has passed; proceeding without delay");
      break;
    }
...
void Client::logConfiguration() const {
  if (client_configuration.for_disk_s3) {
    LOG_TRACE(log, "S3 client for disk '{}' initialized with s3_retry_attempts: {}", client_configuration.opt_disk_name.value_or(""),
              client_configuration.retry_strategy.max_retries);
    LOG_TRACE(log, "S3 client for disk '{}': slowing down threads on retryable errors is {}",
    ...
    LOG_TRACE(log, "S3 client: slowing down threads on retryable errors is {}",
              client_configuration.s3_slow_all_threads_after_retryable_error ? "enabled" : "disabled");
  }
}

bool Client::supportsMultiPartCopy() const { return provider_type != ProviderType::GCS; }
...
std::string Client::getGCSOAuthToken() const {
  if (provider_type != ProviderType::GCS) return "";
...
std::string Client::getRegionForBucket(const std::string &bucket, bool force_detect) const {
  std::lock_guard lock(cache->region_cache_mutex);
  if (auto it = cache->region_for_bucket_cache.find(bucket); it != cache->region_for_bucket_cache.end()) return it->second;

  if (!force_detect && !detect_region) return "";
  ...
  if (outcome.IsSuccess()) {
    ...
    if (auto it = headers.find(region_header); it != headers.end()) region = it->second;
  }
...
std::optional<S3::URI> Client::getURIFromError(const Aws::S3::S3Error &error) const {
  ...
  if (!resolved_endpoint.IsSuccess()) return std::nullopt;
...
std::optional<S3::URI> Client::getURIForBucket(const std::string &bucket) const {
  ...
  if (auto it = cache->uri_for_bucket_cache.find(bucket); it != cache->uri_for_bucket_cache.end()) return it->second;
...
void Client::updateURIForBucket(const std::string &bucket, S3::URI new_uri) const {
  ...
  if (auto it = cache->uri_for_bucket_cache.find(bucket); it != cache->uri_for_bucket_cache.end()) {
    if (it->second.uri == new_uri.uri) return;
...
void ClientCacheRegistry::registerClient(const std::shared_ptr<ClientCache> &client_cache) {
  ...
  if (!inserted) throw Exception(ErrorCodes::LOGICAL_ERROR, "Same S3 client registered twice");
}
...
std::unique_ptr<S3::Client> ClientFactory::create(  // NOLINT
    const PocoHTTPClientConfiguration &cfg_, ClientSettings client_settings, const String &access_key_id, const String &secret_access_key,
    const String &server_side_encryption_customer_key_base64, ServerSideEncryptionKMSConfig sse_kms_config, HTTPHeaderEntries headers,
    CredentialsConfiguration credentials_configuration, const String &session_token) {
  ...
  if (client_configuration.endpointOverride.empty()) client_settings.use_virtual_addressing = true;

  return Client::create(client_configuration.s3_max_redirects, std::move(sse_kms_config), credentials_provider,
...
PocoHTTPClientConfiguration ClientFactory::createClientConfiguration(  // NOLINT
    const String &force_region, const RemoteHostFilter &remote_host_filter, unsigned int s3_max_redirects,
    const PocoHTTPClientConfiguration::RetryStrategy &retry_strategy, bool s3_slow_all_threads_after_network_error,
    bool s3_slow_all_threads_after_retryable_error, bool enable_s3_requests_logging, bool for_disk_s3,
    std::optional<std::string> opt_disk_name, const HTTPRequestThrottler &request_throttler, const String &protocol) {
  auto context = Context::getGlobalContextInstance();
  chassert(context);
  auto proxy_configuration_resolver =
      ProxyConfigurationResolverProvider::get(ProxyConfiguration::protocolFromString(protocol), context->getConfigRef());
  ...
  auto config = PocoHTTPClientConfiguration(
      per_request_configuration, force_region, remote_host_filter, s3_max_redirects, retry_strategy,
      s3_slow_all_threads_after_network_error, s3_slow_all_threads_after_retryable_error, enable_s3_requests_logging, for_disk_s3,
      opt_disk_name, context->getGlobalContext()->getSettingsRef()[Setting::s3_use_adaptive_timeouts], request_throttler, error_report);
