...
class Client : private Aws::S3::S3Client {
 public:
  ...
  static std::unique_ptr<Client> create(size_t max_redirects_, ServerSideEncryptionKMSConfig sse_kms_config_,
  ...
  class RetryStrategy : public Aws::Client::RetryStrategy {
   public:
    ...
    void RequestBookkeeping(const Aws::Client::HttpResponseOutcome &httpResponseOutcome,
    ...
    ///
    ///     AWSError 'InternalError': Copy spanning locations and/or storage classes could not complete within 30 seconds. Please use the
    ///     Rewrite method in the JSON API (https://cloud.google.com/storage/docs/json_api/v1/objects/rewrite) instead.
    ///
  ...
  bool isClientForDisk() const { return client_configuration.for_disk_s3; }
 ...
 protected:
  // visible for testing
  Client(size_t max_redirects_, ServerSideEncryptionKMSConfig sse_kms_config_,
         const std::shared_ptr<Aws::Auth::AWSCredentialsProvider> &credentials_provider_,
         const PocoHTTPClientConfiguration &client_configuration, Aws::Client::AWSAuthV4Signer::PayloadSigningPolicy sign_payloads,
         const ClientSettings &client_settings_);

 private:
  Client(const Client &other, const PocoHTTPClientConfiguration &client_configuration);
  ...
  std::invoke_result_t<RequestFn, RequestType &> doRequest(RequestType &request, RequestFn request_fn) const;

  template <bool IsReadMethod, typename RequestType, typename RequestFn>
  std::invoke_result_t<RequestFn, RequestType &> doRequestWithRetryNetworkErrors(RequestType &request, RequestFn request_fn) const;
...
class ClientFactory {
 public:
  ...
  std::unique_ptr<S3::Client> create(const PocoHTTPClientConfiguration &cfg, ClientSettings client_settings, const String &access_key_id,
                                     const String &secret_access_key, const String &server_side_encryption_customer_key_base64,
                                     ServerSideEncryptionKMSConfig sse_kms_config, HTTPHeaderEntries headers,
                                     CredentialsConfiguration credentials_configuration, const String &session_token = "");

  PocoHTTPClientConfiguration createClientConfiguration(const String &force_region, const RemoteHostFilter &remote_host_filter,
                                                        unsigned int s3_max_redirects,
                                                        const PocoHTTPClientConfiguration::RetryStrategy &retry_strategy,
                                                        bool s3_slow_all_threads_after_network_error,
                                                        bool s3_slow_all_threads_after_retryable_error, bool enable_s3_requests_logging,
                                                        bool for_disk_s3, std::optional<std::string> opt_disk_name,
                                                        const HTTPRequestThrottler &request_throttler, const String &protocol = "https");
