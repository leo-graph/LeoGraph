...
PocoHTTPClientConfiguration::PocoHTTPClientConfiguration(std::function<ProxyConfiguration()> per_request_configuration_,
                                                         const String &force_region_, const RemoteHostFilter &remote_host_filter_,
                                                         unsigned int s3_max_redirects_, RetryStrategy retry_strategy_,
                                                         bool s3_slow_all_threads_after_network_error_,
                                                         bool s3_slow_all_threads_after_retryable_error_, bool enable_s3_requests_logging_,
                                                         bool for_disk_s3_, std::optional<std::string> opt_disk_name_,
                                                         bool s3_use_adaptive_timeouts_, const HTTPRequestThrottler &request_throttler_,
...
void PocoHTTPClientConfiguration::updateSchemeAndRegion() {
  if (!endpointOverride.empty()) {
    ...
    if (uri.getScheme() == "http") scheme = Aws::Http::Scheme::HTTP;
...
std::shared_ptr<Aws::Http::HttpResponse> PocoHTTPClient::MakeRequest(const std::shared_ptr<Aws::Http::HttpRequest> &request,
...
void PocoHTTPClient::addMetric(const Aws::Http::HttpRequest &request, S3MetricType type, ProfileEvents::Count amount) const {
  ...
  static const ProfileEvents::Event
      disk_s3_events_map[static_cast<size_t>(S3MetricType::EnumSize)][static_cast<size_t>(S3MetricKind::EnumSize)] = {
  ...
  if (for_disk_s3) ProfileEvents::increment(disk_s3_events_map[static_cast<unsigned int>(type)][static_cast<unsigned int>(kind)], amount);
}
...
void PocoHTTPClient::observeLatency(const Aws::Http::HttpRequest &request, S3LatencyType type, HistogramMetrics::Value latency) const {
  ...
  const String attempt_label = [](const S3LatencyType t) {
    switch (t) {
      case S3LatencyType::FirstByteAttempt1:
        return "1";
      case S3LatencyType::FirstByteAttempt2:
        return "2";
      case S3LatencyType::FirstByteAttemptN:
        return "N";
      default:
        return "UNKNOWN";
    }
  ...
  HistogramMetrics::observe(HistogramMetrics::S3FirstByte, first_byte_label_values, latency);

  if (for_disk_s3) {
    HistogramMetrics::observe(HistogramMetrics::DiskS3FirstByte, first_byte_label_values, latency);
  }
}

ConnectionTimeouts PocoHTTPClient::getTimeouts(const String &method, bool first_attempt, bool first_byte) const {
  if (!s3_use_adaptive_timeouts) return timeouts;

  return timeouts.getAdaptiveTimeouts(method, first_attempt, first_byte);
}

void PocoHTTPClient::makeRequestInternal(Aws::Http::HttpRequest &request, std::shared_ptr<PocoHTTPResponse> &response,
                                         Aws::Utils::RateLimits::RateLimiterInterface *readLimiter,
                                         Aws::Utils::RateLimits::RateLimiterInterface *writeLimiter) const {
...
void PocoHTTPClient::makeRequestInternalImpl(Aws::Http::HttpRequest &request, std::shared_ptr<PocoHTTPResponse> &response,
  ...
  if (!first_attempt)
    LOG_DEBUG(log, "Retrying S3 request to: {}, aws sdk attempt: {}, clickhouse attempt: {}, kind: {}", uri, sdk_attempt, ch_attempt,
              getMetricKind(request) == S3MetricKind::Read ? "Read" : "Write");
  else  // if (enable_s3_requests_logging)
    LOG_TEST(log, "Make S3 request to: {}, aws sdk attempt: {}, clickhouse attempt: {}, kind: {}", uri, sdk_attempt, ch_attempt,
             getMetricKind(request) == S3MetricKind::Read ? "Read" : "Write");
  ...
  try {
    ...
    if (per_request_configuration) proxy_configuration = per_request_configuration();
    ...
    for (size_t attempt = 0; attempt <= s3_max_redirects; ++attempt) {
      ...
      auto session =
          makeHTTPSession(group, target_uri, getTimeouts(method, first_attempt, /*first_byte*/ true), proxy_configuration, &connect_time);
      ...
      if (request.GetContentBody()) {
        if (enable_s3_requests_logging) LOG_TEST(log, "Writing request body.");
        ...
        if (enable_s3_requests_logging) LOG_TEST(log, "Written {} bytes to request body", size);
      }

      setTimeouts(*session, getTimeouts(method, first_attempt, /*first_byte*/ false));

      if (enable_s3_requests_logging) LOG_TEST(log, "Receiving response...");
      ...
      if (status_code >= SUCCESS_RESPONSE_MIN && status_code <= SUCCESS_RESPONSE_MAX) {
        if (enable_s3_requests_logging) LOG_TEST(log, "Response status: {}, {}", status_code, poco_response.getReason());
      ...
      if (enable_s3_requests_logging) {
        ...
        for (const auto &[header_name, header_value] : poco_response) response->AddHeader(header_name, header_value);
      }
      ...
      if (status_code >= SUCCESS_RESPONSE_MIN && status_code <= SUCCESS_RESPONSE_MAX && checkRequestCanReturn2xxAndErrorInBody(request)) {
        ...
        std::string response_string((std::istreambuf_iterator<char>(response_body_stream)), std::istreambuf_iterator<char>());

        /// Just trim string so it will not be so long
        LOG_TRACE(log, "Got dangerous response with successful code {}, checking its body: '{}'", status_code,
                  response_string.substr(0, 300));
        const static std::string_view needle = "<Error>";
        if (auto it = std::search(response_string.begin(), response_string.end(), std::default_searcher(needle.begin(), needle.end()));
            it != response_string.end()) {
          LOG_WARNING(log,
                      "Response for the request contains an <Error> tag in the body, will treat it as an internal server error (code 500)");
          response->SetResponseCode(Aws::Http::HttpResponseCode::INTERNAL_SERVER_ERROR);

          addMetric(request, S3MetricType::Errors);
          if (error_report) error_report(proxy_configuration);
        }
        ...
        if (status_code == 429 || status_code == 503) {
          ...
          addMetric(request, S3MetricType::Errors);
          if (status_code >= 500 && error_report) error_report(proxy_configuration);
        }
    ...
    LOG_DEBUG(log, "Failed to make request to: {}: will be retried, Poco::Net::NetException: {}", uri,
              getCurrentExceptionMessage(/* with_stacktrace */ true));
    exception_handler(Aws::Client::CoreErrors::NETWORK_CONNECTION);
  } catch (Poco::IOException &) {
    LOG_DEBUG(log, "Failed to make request to: {}: will be retried, Poco::IOException: {}", uri,
              getCurrentExceptionMessage(/* with_stacktrace */ true));
    exception_handler(Aws::Client::CoreErrors::NETWORK_CONNECTION);
  } catch (Poco::TimeoutException &) {
    LOG_DEBUG(log, "Failed to make request to: {}: will be retried, Poco::TimeoutException: {}", uri,
              getCurrentExceptionMessage(/* with_stacktrace */ true));
    exception_handler(Aws::Client::CoreErrors::NETWORK_CONNECTION);
  } catch (const NetException &e) {
    bool dns_error = e.code() == ErrorCodes::DNS_ERROR;
    LOG_DEBUG(log, "Failed to make request to: {}: {}DB::NetException: {}", uri, dns_error ? "the error about DNS, " : "will be retried, ",
              getCurrentExceptionMessage(/* with_stacktrace */ true));
    exception_handler(e.code() == ErrorCodes::DNS_ERROR ? Aws::Client::CoreErrors::ENDPOINT_RESOLUTION_FAILURE
...
String getStringOrDefault(const String &str, const String &default_str) { return str.empty() ? default_str : str; }
...
void PocoHTTPClientGCPOAuth::makeRequestInternal(Aws::Http::HttpRequest &request, std::shared_ptr<PocoHTTPResponse> &response,
                                                 Aws::Utils::RateLimits::RateLimiterInterface *readLimiter,
                                                 Aws::Utils::RateLimits::RateLimiterInterface *writeLimiter) const {
  {
    std::lock_guard lock(mutex);
    if (!bearer_token || std::chrono::system_clock::now() > bearer_token->is_valid_to) bearer_token = requestBearerToken();
...
PocoHTTPClientGCPOAuth::BearerToken PocoHTTPClientGCPOAuth::requestBearerToken() const {
  ...
  if (enable_s3_requests_logging) LOG_TEST(log, "Make request to: {}", url.toString());
  ...
  if (enable_s3_requests_logging) LOG_TEST(log, "Received token in response: {}", token_json_raw);
  ...
  if (token_type != "Bearer")
    throw Exception(ErrorCodes::AUTHENTICATION_FAILED, "Unexpected structure of response. Expected Bearer token, got {}", token_type);

  return {.token = object->getValue<String>("access_token"),
