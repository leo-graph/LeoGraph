...
URI::URI(const std::string &uri_, bool allow_archive_path_syntax, bool keep_presigned_query_parameters) {
  ...
  for (const auto &[qk, qv] : original_uri.getQueryParameters()) {
    if (qk == "versionId" || qk == "AWSAccessKeyId" || qk == "Signature" || qk == "Expires" || qk.starts_with("X-Amz-") ||
        qk == "GoogleAccessId" || qk.starts_with("X-Goog-")) {
  ...
  if (!keep_presigned_query_parameters && looks_like_presigned) looks_like_presigned = false;
  ...
  if (context) {
    ...
    if (config->has("url_scheme_mappers")) {
      ...
      for (const std::string &config_key : config_keys) mapper[config_key] = config->getString("url_scheme_mappers." + config_key + ".to");
    } else {
    ...
    if (!mapper.empty()) URIConverter::modifyURI(uri, mapper);
  }

  storage_name = "S3";

  if (uri.getHost().empty()) throw Exception(ErrorCodes::BAD_ARGUMENTS, "Host is empty in S3 URI.");
  ...
  if (!is_using_aws_private_link_interface &&
    ...
    is_virtual_hosted_style = false;
    endpoint = uri.getScheme() + "://" + uri.getAuthority();
    if (!uri.getPath().empty()) key = uri.getPath().substr(1);
  }
...
void URI::validateKey(const String &key, const Poco::URI &uri) {
  auto onError = [&]() {
    throw Exception(ErrorCodes::BAD_ARGUMENTS, "Invalid S3 key: {}{}", quoteString(key), !uri.empty() ? " (" + uri.toString() + ")" : "");
  };
