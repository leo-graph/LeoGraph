...
class MockRequestHandler : public Poco::Net::HTTPRequestHandler {
 ...
 public:
  explicit MockRequestHandler(Poco::Net::MessageHeader &last_request_header_) : last_request_header(last_request_header_) {}
...
class HTTPRequestHandlerFactory : public Poco::Net::HTTPRequestHandlerFactory {
 ...
 public:
  explicit HTTPRequestHandlerFactory(Poco::Net::MessageHeader &last_request_header_) : last_request_header(last_request_header_) {}
...
class TestPocoHTTPServer {
 ...
 public:
  ...
  std::string getUrl() { return "http://" + server_socket->address().toString(); }
...
class MockStsRequestHandler : public Poco::Net::HTTPRequestHandler {
 public:
  explicit MockStsRequestHandler(std::optional<StsRequestInfo> &last_request_info_, std::string role_access_key_,
                                 std::string role_secret_key_)
      : last_request_info(last_request_info_), role_access_key(std::move(role_access_key_)), role_secret_key(std::move(role_secret_key_)) {}
...
<AssumeRoleResult>
    <Credentials>
        ...
        <SessionToken>session_token</SessionToken>
                                         ...
                                         role_access_key, role_secret_key);
    out << result_xml;
    out.flush();
  }
...
class StsHTTPRequestHandlerFactory : public Poco::Net::HTTPRequestHandlerFactory {
 ...
 public:
  explicit StsHTTPRequestHandlerFactory(std::optional<StsRequestInfo> &last_request_info_, std::string role_access_key_,
                                        std::string role_secret_key_)
      : last_request_info(last_request_info_), role_access_key(std::move(role_access_key_)), role_secret_key(std::move(role_secret_key_)) {}
...
class TestPocoHTTPStsServer {
 ...
 public:
  ...
  void resetLastRequest() { last_request_info.reset(); }

  bool hasLastRequest() const { return last_request_info.has_value(); }

  const Poco::Net::MessageHeader &getLastRequestHeader() const { return last_request_info->headers; }

  const auto &getLastQueryParams() const { return last_request_info->query_params; }
};
