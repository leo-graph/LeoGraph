...
void deleteFileFromS3(const std::shared_ptr<const S3::Client>& s3_client, const String& bucket, const String& key, bool if_exists,
                      BlobStorageLogWriterPtr blob_storage_log, const String& local_path_for_blob_storage_log,
                      size_t file_size_for_blob_storage_log, std::optional<ProfileEvents::Event> profile_event) {
  S3::DeleteObjectRequest request;
  request.SetBucket(bucket);
  request.SetKey(key);

  ProfileEvents::increment(ProfileEvents::S3DeleteObjects);
  if (profile_event && *profile_event != ProfileEvents::S3DeleteObjects) ProfileEvents::increment(*profile_event);
  ...
  if (blob_storage_log) {
    ...
    blob_storage_log->addEvent(BlobStorageLogElement::EventType::Delete, bucket, key, local_path_for_blob_storage_log,
                               file_size_for_blob_storage_log, elapsed,
  ...
  if (outcome.IsSuccess()) {
    ...
    throw S3Exception(err.GetErrorType(), "{} (Code: {}) while removing object with path {} from S3", err.GetMessage(),
                      static_cast<size_t>(err.GetErrorType()), key);
  }
}

void deleteFilesFromS3(const std::shared_ptr<const S3::Client>& s3_client, const String& bucket, const Strings& keys, bool if_exists,
                       S3Capabilities& s3_capabilities, size_t batch_size, BlobStorageLogWriterPtr blob_storage_log,
                       const Strings& local_paths_for_blob_storage_log, const std::vector<size_t>& file_sizes_for_blob_storage_log,
  ...
  if (keys.empty()) return;  /// Nothing to delete.
  ...
  if (try_batch_delete) {
    ...
    while (current_position < keys.size()) {
      ...
      for (; current_position < keys.size() && current_chunk.size() < batch_size; ++current_position) {
        ...
        if (!comma_separated_keys.empty()) comma_separated_keys += ", ";
      ...
      if (blob_storage_log) {
        ...
        for (size_t i = first_position; i < current_position; ++i) {
          const String& local_path_for_blob_storage_log =
              (i < local_paths_for_blob_storage_log.size()) ? local_paths_for_blob_storage_log[i] : empty_string;
          ...
          blob_storage_log->addEvent(BlobStorageLogElement::EventType::Delete, bucket, keys[i], local_path_for_blob_storage_log,
                                     file_size_for_blob_storage_log, elapsed / (current_position - first_position),
                                     outcome.IsSuccess() ? 0 : static_cast<Int32>(outcome.GetError().GetErrorType()),
                                     outcome.IsSuccess() ? "" : outcome.GetError().GetMessage(), time_now);
        }
      } else {
      ...
      if (outcome.IsSuccess()) {
        ...
        if (errors.empty()) {
          ...
          for (const auto& err : errors) {
            ...
            if (if_exists && S3::isNotFoundError(error_type)) {
              if (not_found_keys.empty()) not_found_keys += ", ";
              not_found_keys += err.GetKey();
            } else if (!other_error) {
              other_error = std::make_exception_ptr(S3Exception{error_type, "{} (Code: {}) while removing object with path {} from S3",
          ...
          if (!removed_keys.empty()) {
            ...
            for (const auto& key : removed_keys) {
              if (!removed_keys_comma_separated.empty()) removed_keys_comma_separated += ", ";
        ...
        if ((err.GetExceptionName() == "InvalidRequest") || (err.GetExceptionName() == "InvalidArgument") ||
          ...
          LOG_TRACE(log, "DeleteObjects is not supported: {} (Code: {}). Retrying with plain DeleteObject.", err.GetMessage(),
                    static_cast<size_t>(err.GetErrorType()));
          /// Multiple threads can call deleteFilesFromS3() with a reference to the same `s3_capabilities`,
          /// and the following line doesn't cause a race because `s3_capabilities` is protected with mutex.
        ...
        if (if_exists && S3::isNotFoundError(err.GetErrorType())) {
          ...
          throw S3Exception(err.GetErrorType(), "{} (Code: {}) while removing objects with paths [{}] from S3", err.GetMessage(),
                            static_cast<size_t>(err.GetErrorType()), comma_separated_keys);
        }
      }
    }

    if (!need_retry_with_plain_delete_object) return;
  }
  ...
  for (size_t i = 0; i != keys.size(); ++i) {
    ...
    deleteFileFromS3(s3_client, bucket, keys[i], if_exists, blob_storage_log, local_path_for_blob_storage_log,
                     file_size_for_blob_storage_log, profile_event);
  }
}
