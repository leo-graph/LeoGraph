...
inline void setPartChecksum(Model::CompletedPart& part, const std::string& checksum) { part.SetChecksumCRC32(checksum); }

inline void setRequestChecksum(Model::UploadPartRequest& req, const std::string& checksum) { req.SetChecksumCRC32(checksum); }
...
class ExtendedRequest : public BaseRequest {
 public:
  Aws::Endpoint::EndpointParameters GetEndpointContextParams() const override {
    ...
    if (!region_override.empty()) params.emplace_back("Region", region_override);
  ...
  Aws::String GetChecksumAlgorithmName() const override {
    ...
    ///
    ///   [1]:
    ///   https://github.com/aws/aws-sdk-cpp/blob/b0ee1c0d336dbb371c34358b68fba6c56aae2c92/src/aws-cpp-sdk-core/source/client/AWSClient.cpp#L783-L839
    if (!is_s3express_bucket && !checksum) return "";
  ...
  std::string getRegionOverride() const { return region_override; }

  void overrideRegion(std::string region) const { region_override = std::move(region); }

  void overrideURI(S3::URI uri) const { uri_override = std::move(uri); }

  const auto& getURIOverride() const { return uri_override; }

  void setApiMode(ApiMode api_mode_) const { api_mode = api_mode_; }
