...
S3Capabilities::S3Capabilities(const S3Capabilities& src) : S3Capabilities(src.isBatchDeleteSupported(), src.support_proxy) {}
...
S3Capabilities getCapabilitiesFromConfig(const Poco::Util::AbstractConfiguration& config, const std::string& config_prefix) {
  ...
  if (config.has(config_prefix + ".support_batch_delete")) support_batch_delete = config.getBool(config_prefix + ".support_batch_delete");
...
S3Capabilities S3Capabilities::deserialize(ReadBuffer& in) {
  ...
  if (support_batch_deletes_int != 2) support_batch_deletes = support_batch_deletes_int;
