...
class CredentialsProviderCache : boost::noncopyable {
  using CredentialsProviderKey =
      std::variant<AWSInstanceProfileCredentialsProvider::CacheKey, AwsAuthSTSAssumeRoleWebIdentityCredentialsProvider::CacheKey,
 ...
 public:
  ...
  explicit CredentialsProviderCache(size_t max_credentials_) : max_credentials(max_credentials_) {}
  ...
  CredentialsProviderPtr getOrSet(T&& cache_key, std::function<CredentialsProviderPtr()> creator) {
    ...
    if (current_max_credentials == 0) return creator();

    CredentialsProviderKey key = std::forward<T>(cache_key);
    std::lock_guard lock(mutex);
    if (auto it = cached_credentials.find(key); it == cached_credentials.end() || it->second == credentials_lru.end()) {
      LOG_TEST(getLogger("CredentialsProviderCache"), "Total cached credentials size: {}, adding new credentials provider to cache",
               cached_credentials.size());

      if (it != cached_credentials.end()) cached_credentials.erase(it);
      ...
      LOG_TEST(getLogger("CredentialsProviderCache"), "Total credentials size: {}, reusing credentials provider",
               cached_credentials.size());
...
void setCredentialsProviderCacheMaxSize(size_t cache_size) { CredentialsProviderCache::instance().setSize(cache_size); }
...
Aws::String AWSEC2MetadataClient::getDefaultCredentials() const {
  ...
  if (trimmed_credentials_string.empty()) return {};

  std::vector<String> security_credentials = Aws::Utils::StringUtils::Split(trimmed_credentials_string, '\n');

  LOG_DEBUG(logger, "Calling EC2MetadataService resource, {} returned credential string {}.", EC2_SECURITY_CREDENTIALS_RESOURCE,
...
Aws::String AWSEC2MetadataClient::getDefaultCredentialsSecurely() const {
  ...
  if (response_code == Aws::Http::HttpResponseCode::BAD_REQUEST || response_code == Aws::Http::HttpResponseCode::REQUEST_NOT_MADE) {
  ...
  LOG_DEBUG(logger, "Calling EC2MetadataService resource, {} with token returned profile string {}.", EC2_SECURITY_CREDENTIALS_RESOURCE,
  ...
  std::shared_ptr<Aws::Http::HttpRequest> credentials_request(
      Aws::Http::CreateHttpRequest(ss.str(), Aws::Http::HttpMethod::HTTP_GET, Aws::Utils::Stream::DefaultResponseStreamFactoryMethod));
...
std::pair<Aws::String, Aws::Http::HttpResponseCode> AWSEC2MetadataClient::getEC2MetadataToken(const std::string& user_agent_string) const {
  ...
  std::shared_ptr<Aws::Http::HttpRequest> token_request(
      Aws::Http::CreateHttpRequest(ss.str(), Aws::Http::HttpMethod::HTTP_PUT, Aws::Utils::Stream::DefaultResponseStreamFactoryMethod));
...
Aws::String AWSEC2MetadataClient::getCurrentRegion() const { return Aws::Region::AWS_GLOBAL; }
    ...
    if (ec2_metadata_service_endpoint_mode.empty()) {
      ...
      if (ec2_metadata_service_endpoint_mode.length() == 4) {
        if (Aws::Utils::StringUtils::CaselessCompare(ec2_metadata_service_endpoint_mode.c_str(), "ipv4")) {
          ...
          LOG_ERROR(logger, "AWS_EC2_METADATA_SERVICE_ENDPOINT_MODE can only be set to ipv4 or ipv6, received: {}",
...
String getGCPAvailabilityZoneOrException() {
  ...
  if (zone_info.size() != 4)
    throw DB::Exception(ErrorCodes::GCP_ERROR,
                        "Invalid format of GCP zone information, expect projects/<project-number>/zones/<zone-value>");
  return zone_info[3];
}
...
AWSEC2InstanceProfileConfigLoader::AWSEC2InstanceProfileConfigLoader(const std::shared_ptr<AWSEC2MetadataClient>& client_,
                                                                     bool use_secure_pull_)
    : client(client_), use_secure_pull(use_secure_pull_), logger(getLogger("AWSEC2InstanceProfileConfigLoader")) {}
...
AWSInstanceProfileCredentialsProvider::AWSInstanceProfileCredentialsProvider(
    const std::shared_ptr<AWSEC2InstanceProfileConfigLoader>& config_loader)
...
std::shared_ptr<Aws::Auth::AWSCredentialsProvider> AWSInstanceProfileCredentialsProvider::create(
  ...
  return CredentialsProviderCache::instance().getOrSet(AWSInstanceProfileCredentialsProvider::CacheKey{endpoint, use_secure_pull}, [&] {
...
std::shared_ptr<Aws::Auth::AWSCredentialsProvider> AwsAuthSTSAssumeRoleWebIdentityCredentialsProvider::create(
  ...
  // region source is not enforced, but we need it to construct sts endpoint, if we can't find from environment, we should check if it's set
  // in config file.
  ...
  return CredentialsProviderCache::instance().getOrSet(
      AwsAuthSTSAssumeRoleWebIdentityCredentialsProvider::CacheKey{role_arn, token_file, cache_on_session_name ? session_name : ""}, [&] {
        return std::make_shared<AwsAuthSTSAssumeRoleWebIdentityCredentialsProvider>(aws_client_configuration, expiration_window_seconds_,
                                                                                    std::move(role_arn), std::move(token_file),
                                                                                    std::move(session_name), std::move(tmp_region));
      });
}

AwsAuthSTSAssumeRoleWebIdentityCredentialsProvider::AwsAuthSTSAssumeRoleWebIdentityCredentialsProvider(
    DB::S3::PocoHTTPClientConfiguration& aws_client_configuration, uint64_t expiration_window_seconds_, Aws::String role_arn_,
    Aws::String token_file_, Aws::String session_name_, String tmp_region)
  ...
  aws_client_configuration.retryStrategy =
      std::make_shared<Aws::Client::SpecifiedRetryableErrorsRetryStrategy>(retryable_errors, /* maxRetries = */ 3);
...
Aws::Auth::AWSCredentials AwsAuthSTSAssumeRoleWebIdentityCredentialsProvider::GetAWSCredentials() {
  Aws::Utils::Threading::ReaderLockGuard guard(m_reloadLock);
  if (!IsSetNeedRefresh() && !areCredentialsEmptyOrExpired(credentials, expiration_window_seconds)) return credentials;

  guard.UpgradeToWriterLock();
  if (!IsSetNeedRefresh() &&
      !areCredentialsEmptyOrExpired(credentials, expiration_window_seconds))  // double-checked lock to avoid refreshing twice
    return credentials;
...
void AwsAuthSTSAssumeRoleWebIdentityCredentialsProvider::Reload() {
  ...
  LOG_INFO(logger, "Got {}credentials from STS", result.creds.IsEmpty() ? "empty " : ((result.creds == credentials) ? "same " : ""));

  credentials = result.creds;
}

SSOCredentialsProvider::SSOCredentialsProvider(DB::S3::PocoHTTPClientConfiguration aws_client_configuration_,
...
void SSOCredentialsProvider::Reload() {
  ...
  const auto access_token = [&] {
    ...
    ss_token << Aws::FileSystem::PATH_DELIM << "sso" << Aws::FileSystem::PATH_DELIM << "cache" << Aws::FileSystem::PATH_DELIM
             << hashed_start_url << ".json";
  ...
  LOG_INFO(logger, "Got {}credentials with AWS_ACCESS_KEY: {}",
           result.creds.IsEmpty() ? "empty " : ((credentials == result.creds) ? "same " : ""), result.creds.GetAWSAccessKeyId());
...
void SSOCredentialsProvider::refreshIfExpired() {
  Aws::Utils::Threading::ReaderLockGuard guard(m_reloadLock);
  if (!IsSetNeedRefresh() && !areCredentialsEmptyOrExpired(credentials, expiration_window_seconds)) return;
...
Aws::String SSOCredentialsProvider::loadAccessTokenFile(const Aws::String& sso_access_token_path) {
  ...
  if (input_file) {
    ...
    if (tmp_access_token.empty() || !expiration.WasParseSuccessful()) {
      LOG_TRACE(logger, "Token cache file failed because {}{}", (tmp_access_token.empty() ? "AccessToken was empty " : ""),
...
S3CredentialsProviderChain::S3CredentialsProviderChain(const DB::S3::PocoHTTPClientConfiguration& configuration,
  ...
  if (credentials_configuration.no_sign_request || configuration.http_client == "gcp_oauth") return;
  ...
  if (credentials_configuration.use_environment_credentials) {
    ...
    /// quite verbose even if nobody configured them. So we use our provider first and only after it use default providers.
    {
      DB::S3::PocoHTTPClientConfiguration aws_client_configuration = DB::S3::ClientFactory::instance().createClientConfiguration(
          configuration.region, configuration.remote_host_filter, configuration.s3_max_redirects, configuration.retry_strategy,
          configuration.s3_slow_all_threads_after_network_error, configuration.s3_slow_all_threads_after_retryable_error,
          configuration.enable_s3_requests_logging, configuration.for_disk_s3, configuration.opt_disk_name,
          configuration.request_throttler);
      AddProvider(AwsAuthSTSAssumeRoleWebIdentityCredentialsProvider::create(
...

    {
      DB::S3::PocoHTTPClientConfiguration aws_client_configuration = DB::S3::ClientFactory::instance().createClientConfiguration(
          ...
          configuration.enable_s3_requests_logging, configuration.for_disk_s3, configuration.opt_disk_name,
          configuration.request_throttler);
      AddProvider(std::make_shared<SSOCredentialsProvider>(std::move(aws_client_configuration),
    ...
    if (!relative_uri.empty()) {
      ...
      LOG_INFO(logger, "Added ECS metadata service credentials provider with relative path: [{}] to the provider chain.", relative_uri);
      ...
      AddProvider(std::make_shared<Aws::Auth::GeneralHTTPCredentialsProvider>(relative_uri, absolute_uri, token, token_path));

      /// DO NOT log the value of the authorization token for security purposes.
      LOG_INFO(logger, "Added ECS credentials provider with URI: [{}] to the provider chain with a{} authorization token.", absolute_uri,
               token.empty() ? "n empty" : " non-empty");
    } else if (Aws::Utils::StringUtils::ToLower(ec2_metadata_disabled.c_str()) != "true") {
      DB::S3::PocoHTTPClientConfiguration aws_client_configuration = DB::S3::ClientFactory::instance().createClientConfiguration(
          configuration.region, configuration.remote_host_filter, configuration.s3_max_redirects, configuration.retry_strategy,
          configuration.s3_slow_all_threads_after_network_error, configuration.s3_slow_all_threads_after_retryable_error,
          configuration.enable_s3_requests_logging, configuration.for_disk_s3, configuration.opt_disk_name, configuration.request_throttler,
      ...
      AddProvider(
          AWSInstanceProfileCredentialsProvider::create(aws_client_configuration, !credentials_configuration.use_insecure_imds_request));
...
AssumeRoleResult::AssumeRoleResult(Aws::AmazonWebServiceResult<Aws::Utils::Xml::XmlDocument> result) {
  ...
  const auto get_credential_value_from_node = [&](auto& dest, const std::string& node_name) {
    if (auto node = credentials_node.FirstChild(node_name); !node.IsNull())
...
AWSAssumeRoleClient::AWSAssumeRoleClient(const std::shared_ptr<Aws::Auth::AWSCredentialsProvider>& credentials_provider,
                                         const Aws::Client::ClientConfiguration& client_configuration,
                                         const std::string& sts_endpoint_override)
    : Aws::Client::AWSXMLClient(client_configuration,
                                std::make_shared<Aws::Auth::DefaultAuthSignerProvider>(
                                    credentials_provider, "sts", Aws::Region::ComputeSignerRegion(client_configuration.region),
  ...
  if (client_configuration.region != "aws-global") endpoint_str += client_configuration.region + ".";
...
std::shared_ptr<Aws::Auth::AWSCredentialsProvider> AwsAuthSTSAssumeRoleCredentialsProvider::create(
    std::string role_arn_, std::string session_name_, uint64_t expiration_window_seconds_,
    std::shared_ptr<Aws::Auth::AWSCredentialsProvider> credentials_provider,
    const DB::S3::PocoHTTPClientConfiguration& client_configuration, const std::string& sts_endpoint_override) {
  ...
  return CredentialsProviderCache::instance().getOrSet(
      AwsAuthSTSAssumeRoleCredentialsProvider::CacheKey{role_arn_, session_name, client->getEndpoint().GetURL(),
                                                        credentials_provider->GetAWSCredentials()},
      [&] {
        return std::make_shared<AwsAuthSTSAssumeRoleCredentialsProvider>(std::move(role_arn_), std::move(session_name),
                                                                         expiration_window_seconds_, std::move(client));
      });
}

AwsAuthSTSAssumeRoleCredentialsProvider::AwsAuthSTSAssumeRoleCredentialsProvider(std::string role_arn_, std::string session_name_,
                                                                                 uint64_t expiration_window_seconds_,
...
Aws::Auth::AWSCredentials AwsAuthSTSAssumeRoleCredentialsProvider::GetAWSCredentials() {
  Aws::Utils::Threading::ReaderLockGuard guard(m_reloadLock);
  if (!areCredentialsEmptyOrExpired(credentials, expiration_window_seconds)) return credentials;
...
std::shared_ptr<Aws::Auth::AWSCredentialsProvider> getCredentialsProvider(const DB::S3::PocoHTTPClientConfiguration& configuration,
                                                                          const Aws::Auth::AWSCredentials& credentials,
                                                                          const CredentialsConfiguration& credentials_configuration) {
  std::shared_ptr<Aws::Auth::AWSCredentialsProvider> credentials_provider;
  if (credentials_configuration.no_sign_request || configuration.http_client == "gcp_oauth") {
    ...
    credentials_provider = std::make_shared<S3CredentialsProviderChain>(configuration, credentials, credentials_configuration);
  }

  if (!credentials_configuration.role_arn.empty()) {
    credentials_provider = AwsAuthSTSAssumeRoleCredentialsProvider::create(
        credentials_configuration.role_arn, credentials_configuration.role_session_name,
        credentials_configuration.expiration_window_seconds, std::move(credentials_provider), configuration,
