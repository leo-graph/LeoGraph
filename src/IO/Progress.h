...
struct WriteProgress {
  ...
  WriteProgress(UInt64 written_rows_, UInt64 written_bytes_) : written_rows(written_rows_), written_bytes(written_bytes_) {}
};
...
struct FileProgress {
  ...
  explicit FileProgress(UInt64 read_bytes_, UInt64 total_bytes_to_read_ = 0)
      : read_bytes(read_bytes_), total_bytes_to_read(total_bytes_to_read_) {}
};
...
struct Progress {
  ...
  Progress(Progress&& other) noexcept { *this = std::move(other); }
};
