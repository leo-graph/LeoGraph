...
ReturnType readFloatTextPreciseImpl(T &x, ReadBuffer &buf) {
  ...
  if (likely(!buf.eof() && (buf_from_memory || buf.position() + MAX_LENGTH <= buf.buffer().end()))) {
    ...
    if (unlikely(res.ec != std::errc())) {
      if constexpr (throw_exception)
        throw Exception(ErrorCodes::CANNOT_PARSE_NUMBER, "Cannot read floating point value here: {}",
    ...
    while (!buf.eof() && num_copied_chars < MAX_LENGTH) {
      char c = *buf.position();
      if (!(isNumericASCII(c) || c == '-' || c == '+' || c == '.' || c == 'e' || c == 'E')) break;
    ...
    if (unlikely(res.ec != std::errc() || res.ptr - tmp_buf != num_copied_chars)) {
      if constexpr (throw_exception)
        throw Exception(ErrorCodes::CANNOT_PARSE_NUMBER, "Cannot read floating point value here: {}", String(tmp_buf, num_copied_chars));
      else
        return ReturnType(false);
    }
