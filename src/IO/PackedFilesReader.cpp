...
PackedFilesReader::PackedFilesReader(DiskPtr disk_, const String& data_file_name_, const ReadSettings& read_settings_)
...
PackedFilesReader::PackedFilesReader(DiskPtr disk_, const String& data_file_name_, const PackedFilesIO::Index& index_)
...
PackedFilesIO::Index PackedFilesReader::readIndex(ReadBuffer& in) {
  ...
  if (version > PackedFilesIO::VERSION)
    throw Exception(ErrorCodes::UNKNOWN_FORMAT_VERSION, "Unknown format ({}) of packed data", std::to_string(version));
  ...
  for (size_t i = 0; i < num_files; ++i) {
    ...
    if (!index.emplace(file_name, offset).second) throw Exception(ErrorCodes::CORRUPTED_DATA, "Duplicated file: {}", file_name);
  }
  return index;
}

bool PackedFilesReader::exists(const std::string& file_name) const { return index.contains(file_name); }

size_t PackedFilesReader::getFileSize(const String& file_name) const {
  if (auto it = index.find(file_name); it != index.end()) return it->second.size;
...
Names PackedFilesReader::getFileNames() const {
  Names res;
  for (const auto& [name, _] : index) res.emplace_back(name);
  return res;
}

static ReadSettings patchSettings(ReadSettings settings) {
  settings.direct_io_threshold = 0;
  if (settings.local_fs_method == LocalFSReadMethod::mmap) settings.local_fs_method = LocalFSReadMethod::pread;
  return settings;
}

std::unique_ptr<ReadBufferFromFileBase> PackedFilesReader::readFile(const String& file_name, const ReadSettings& settings,
                                                                    std::optional<size_t> read_hint) const {
  auto it = index.find(file_name);
  if (it == index.end()) throw Exception(ErrorCodes::FILE_DOESNT_EXIST, "File {} does not exist", file_name);
