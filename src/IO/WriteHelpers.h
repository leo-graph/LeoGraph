...
inline void writeChar(char x, WriteBuffer &buf) { buf.write(x); }
...
inline void writeStringBinary(const char *s, WriteBuffer &buf) { writeStringBinary(std::string_view{s}, buf); }

template <typename T, typename Alloc = std::allocator<T>>
void writeVectorBinary(const std::vector<T, Alloc> &v, WriteBuffer &buf) {
  writeVarUInt(v.size(), buf);

  for (typename std::vector<T>::const_iterator it = v.begin(); it != v.end(); ++it) writeBinary(*it, buf);
}

inline void writeBoolText(bool x, WriteBuffer &buf) { writeChar(x ? '1' : '0', buf); }
...
inline void writeString(const char *data, size_t size, WriteBuffer &buf) { buf.write(data, size); }

inline void writeString(std::string_view ref, WriteBuffer &buf) { writeString(ref.data(), ref.size(), buf); }
  ...
  for (const char *it = begin; it != end; ++it) {
    switch (*it) {
      ...
      case '\\':
        ...
        break;
      case '/':
        if (settings.json.escape_forward_slashes) writeChar('\\', buf);
        writeChar('/', buf);
        break;
      case '"':
        writeChar('\\', buf);
      ...
      default:
        ...
        if (c <= 0x1F) {
          ...
          if (it[2] == '\xA8') writeCString("\\u2028", buf);
          if (it[2] == '\xA9') writeCString("\\u2029", buf);
    ...
    if (next_pos == end) {
      ...
      switch (*pos) {
        ...
        case '\0':
          ...
          break;
        case '\\':
          if constexpr (escape_backslash_with_backslash) writeChar('\\', buf);
          writeChar('\\', buf);
          break;
        default:
          writeChar(*pos, buf);
...
void writeJSONNumber(T x, WriteBuffer &ostr, const FormatSettings &settings) {
  ...
  const bool need_quote = (is_integer<T> && (sizeof(T) >= 8) && settings.json.quote_64bit_integers) ||
                          ...
                          (is_floating_point<T> && (sizeof(T) >= 8) && settings.json.quote_64bit_floats);

  if (need_quote) writeChar('"', ostr);
...
inline void writeEscapedString(const char *str, size_t size, WriteBuffer &buf) { writeAnyEscapedString<'\''>(str, str + size, buf); }

inline void writeEscapedString(std::string_view ref, WriteBuffer &buf) { writeEscapedString(ref.data(), ref.size(), buf); }
...
inline void writeMarkdownEscapedString(std::string_view ref, WriteBuffer &buf) { writeMarkdownEscapedString(ref.data(), ref.size(), buf); }
...
inline void writeQuotedString(const String &s, WriteBuffer &buf) { writeAnyQuotedString<'\''>(s, buf); }
...
inline void writeDoubleQuotedString(const String &s, WriteBuffer &buf) { writeAnyQuotedString<'"'>(s, buf); }

inline void writeDoubleQuotedString(std::string_view s, WriteBuffer &buf) { writeAnyQuotedString<'"'>(s, buf); }

/// Outputs a string in backquotes.
inline void writeBackQuotedString(std::string_view s, WriteBuffer &buf) { writeAnyQuotedString<'`'>(s, buf); }
...
inline void writeDateTime64FractionalText(typename DecimalType::NativeType fractional, UInt32 scale, WriteBuffer &buf) {
  ...
  for (Int32 pos = scale - 1; pos >= 0 && fractional; --pos, fractional /= DateTime64(10)) data[pos] += fractional % DateTime64(10);
...
template <char date_delimeter = '-', char time_delimeter = ':', char between_date_time_delimiter = ' ',
          char fractional_time_delimiter = '.', bool cut_trailing_zeros_align_to_groups_of_thousands = false>
...
inline void writeDateTimeTextCutTrailingZerosAlignToGroupOfThousands(DateTime64 datetime64, UInt32 scale, WriteBuffer &buf,
...
inline void writeTime64FractionalText(typename DecimalType::NativeType fractional, UInt32 scale, WriteBuffer &buf) {
  ...
  for (Int32 pos = scale - 1; pos >= 0 && abs_fractional; --pos, abs_fractional /= 10) data[pos] += abs_fractional % 10;
...
template <char delimiter1 = ':', char delimiter2 = ':', char fractional_delimiter = '.',
...
inline void writeTime64Text(const Time64 &time64, UInt32 scale, WriteBuffer &buf) {
  ...
  if (is_negative) local_time.negative(true);  // Keep negative sign even if whole part is 0
  writeTimeText<delimiter1>(local_time, buf);

  if (scale > 0) {
    buf.write(fractional_delimiter);
    writeTime64FractionalText<Time64, cut_trailing_zeros_align_to_groups_of_thousands>(components.fractional, scale, buf);
  }
}
...
inline void writeTimeText(const LocalTime &local_time, WriteBuffer &buf) {
  if (local_time.negative()) buf.write("-", 1);
...
inline void writeTimeText(const LocalTime &local_time, WriteBuffer &buf) { writeTimeText<':'>(local_time, buf); }
...
inline void writeTimeTextCutTrailingZerosAlignToGroupOfThousands(Time64 time64, UInt32 scale, WriteBuffer &buf,
                                                                 [[maybe_unused]] const DateLUTImpl &time_zone = DateLUT::instance()) {
  writeTime64Text<':', '.', true>(time64, scale, buf);
}
...
inline void writeText(const DayNum &x, WriteBuffer &buf, const DateLUTImpl &time_zone = DateLUT::instance()) {
...
template <typename T>
void writeDecimalFractional(const T &x, UInt32 scale, WriteBuffer &ostr, bool trailing_zeros, bool fixed_fractional_length,
...
template <typename T>
void writeText(Decimal<T> x, UInt32 scale, WriteBuffer &ostr, bool trailing_zeros = false, bool fixed_fractional_length = false,
...
void writeBinary(const std::vector<T, Alloc> &x, WriteBuffer &buf) {
  ...
  for (size_t i = 0; i < size; ++i) writeBinary(x[i], buf);
}
