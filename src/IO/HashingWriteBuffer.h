...
class IHashingBuffer : public BufferWithOwnMemory<Buffer> {
 public:
  ...
  uint128 getHash() {
    if (block_pos) return CityHash_v1_0_2::CityHash128WithSeed(BufferWithOwnMemory<Buffer>::memory.data(), block_pos, state);
    return state;
  }

  void append(DB::BufferBase::Position data) { state = CityHash_v1_0_2::CityHash128WithSeed(data, block_size, state); }
...
class HashingWriteBuffer : public IHashingBuffer<WriteBuffer> {
 ...
 public:
  explicit HashingWriteBuffer(WriteBuffer& out_, size_t block_size_ = DBMS_DEFAULT_HASHING_BLOCK_SIZE)
