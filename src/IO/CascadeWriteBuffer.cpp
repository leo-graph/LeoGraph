...
void CascadeWriteBuffer::finalizeImpl() {
  ...
  if (curr_buffer) curr_buffer->position() = position();
...
WriteBuffer *CascadeWriteBuffer::setNextBuffer() {
  ...
  if (!res) throw Exception(ErrorCodes::CANNOT_CREATE_IO_BUFFER, "Required WriteBuffer is not created");

  /// Check that returned buffer isn't empty
  if (!res->hasPendingData()) res->next();
