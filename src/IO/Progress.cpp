...
UInt64 getApproxTotalRowsToRead(UInt64 read_rows, UInt64 read_bytes, UInt64 total_bytes_to_read) {
  if (!read_rows || !read_bytes) return 0;
...
bool Progress::empty() const {
  return read_rows == 0 && read_bytes == 0 && written_rows == 0 && written_bytes == 0 && total_rows_to_read == 0 && result_rows == 0 &&
         result_bytes == 0;
...
void ProgressValues::writeJSON(WriteBuffer& out, bool write_zero_values) const {
  ...
  auto write = [&](const char* name, UInt64 value) {
    if (!value && !write_zero_values) return;
    if (has_value) writeChar(',', out);
...
void Progress::writeJSON(WriteBuffer& out, DisplayMode mode) const { getValues().writeJSON(out, mode == DisplayMode::Verbose); }

void Progress::incrementElapsedNs(UInt64 elapsed_ns_) { elapsed_ns.fetch_add(elapsed_ns_, std::memory_order_relaxed); }
