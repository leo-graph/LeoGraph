...
LZMAInflatingReadBuffer::LZMAInflatingReadBuffer(std::unique_ptr<ReadBuffer> in_, size_t buf_size, char *existing_memory, size_t alignment)
  ...
  if (ret != LZMA_OK)
    throw Exception(ErrorCodes::LZMA_STREAM_DECODER_FAILED, "lzma_stream_decoder initialization failed: error code: {}; lzma version: {}",
                    ret, LZMA_VERSION_STRING);
}

LZMAInflatingReadBuffer::~LZMAInflatingReadBuffer() { lzma_end(&lstr); }
...
bool LZMAInflatingReadBuffer::nextImpl() {
  ...
  if (ret == LZMA_STREAM_END) {
    ...
    throw Exception(ErrorCodes::LZMA_STREAM_DECODER_FAILED,
                    "lzma decoder finished, but input stream has not exceeded: error code: {}; lzma version: {}{}", ret,
                    LZMA_VERSION_STRING, getExceptionEntryWithFileName(*in));
  }

  if (ret != LZMA_OK)
    throw Exception(ErrorCodes::LZMA_STREAM_DECODER_FAILED, "lzma_stream_decoder failed: error code: error code {}; lzma version: {}{}",
                    ret, LZMA_VERSION_STRING, getExceptionEntryWithFileName(*in));
