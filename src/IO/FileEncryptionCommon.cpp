...
const EVP_CIPHER *getCipher(Algorithm algorithm) {
  switch (algorithm) {
    case Algorithm::AES_128_CTR:
      return EVP_aes_128_ctr();
    case Algorithm::AES_192_CTR:
      return EVP_aes_192_ctr();
    case Algorithm::AES_256_CTR:
...
void checkKeySize(const EVP_CIPHER *evp_cipher, size_t key_size) {
  if (!key_size) throw Exception(ErrorCodes::BAD_ARGUMENTS, "Encryption key must not be empty");
  size_t expected_key_size = static_cast<size_t>(EVP_CIPHER_key_length(evp_cipher));
  if (key_size != expected_key_size)
    throw Exception(ErrorCodes::BAD_ARGUMENTS, "Got an encryption key with unexpected size {}, the size should be {}", key_size,
                    expected_key_size);
}
...
void checkInitVectorSize(const EVP_CIPHER *evp_cipher) {
  ...
  if (InitVector::kSize != expected_iv_length)
    throw Exception(ErrorCodes::DATA_ENCRYPTION_ERROR, "Got an initialization vector with unexpected size {}, the size should be {}",
                    InitVector::kSize, expected_iv_length);
}
...
size_t encryptFinal(EVP_CIPHER_CTX *evp_ctx, WriteBuffer &out) {
  ...
  __msan_unpoison(ciphertext, ciphertext_size);  /// OpenSSL uses assembly which evades msans analysis
  if (ciphertext_size) out.write(reinterpret_cast<const char *>(ciphertext), ciphertext_size);
  return ciphertext_size;
}
...
Algorithm parseAlgorithmFromString(const String &str) {
  if (boost::iequals(str, "aes_128_ctr")) return Algorithm::AES_128_CTR;
  if (boost::iequals(str, "aes_192_ctr")) return Algorithm::AES_192_CTR;
  if (boost::iequals(str, "aes_256_ctr")) return Algorithm::AES_256_CTR;
  throw Exception(ErrorCodes::BAD_ARGUMENTS,
                  "Encryption algorithm '{}' is not supported, specify one of the following: aes_128_ctr, aes_192_ctr, aes_256_ctr", str);
}
...
void InitVector::read(ReadBuffer &in) { readBinaryBigEndian(counter, in); }

void InitVector::write(WriteBuffer &out) const { writeBinaryBigEndian(counter, out); }
...
Encryptor::Encryptor(Algorithm algorithm_, const String &key_, const InitVector &iv_)
    : key(key_), init_vector(iv_), evp_cipher(getCipher(algorithm_)) {
...
void Encryptor::encrypt(const char *data, size_t size, WriteBuffer &out) {
  ...
  if (EVP_EncryptInit_ex(evp_ctx, nullptr, nullptr, reinterpret_cast<const uint8_t *>(key.c_str()),
                         reinterpret_cast<const uint8_t *>(current_iv.c_str())) != 1)
    throw Exception(DB::ErrorCodes::OPENSSL_ERROR, "EVP_EncryptInit_ex failed: {}", getOpenSSLErrors());
...
void Encryptor::decrypt(const char *data, size_t size, char *out) {
  ...
  if (EVP_DecryptInit_ex(evp_ctx, nullptr, nullptr, reinterpret_cast<const uint8_t *>(key.c_str()),
                         reinterpret_cast<const uint8_t *>(current_iv.c_str())) != 1)
    throw Exception(DB::ErrorCodes::OPENSSL_ERROR, "EVP_DecryptInit_ex failed: {}", getOpenSSLErrors());
...
void Header::read(ReadBuffer &in) {
  ...
  if (version == 0x0100ULL) {
    ...
    /// So now if we read the version as little-endian and it's 256 that means two things: the version is actually 1 and the whole header is
    /// in big endian.
  ...
  if (std::endian::native != endian) algorithm_u16 = std::byteswap(algorithm_u16);
  ...
  if (version < 2) {
    ...
    if (std::endian::native != endian) key_id = std::byteswap(key_id);
...
void Header::write(WriteBuffer &out) const {
  ...
  constexpr size_t reserved_size =
      kSize - kHeaderSignature.length() - sizeof(version) - sizeof(algorithm_u16) - sizeof(key_fingerprint) - InitVector::kSize;
