...
#define INITIALIZE_SETTING_EXTERN(TYPE, NAME, DEFAULT, DESCRIPTION, FLAGS, ...) \
  S3RequestSettings##TYPE NAME = &S3RequestSettingsImpl ::NAME;
...
namespace {
bool setValueFromConfig(const Poco::Util::AbstractConfiguration& config, const std::string& path,
                        typename S3RequestSettingsImpl::SettingFieldRef& field) {
...
S3RequestSettings::S3RequestSettings(const Poco::Util::AbstractConfiguration& config, const DB::Settings& settings,
                                     const std::string& config_prefix, const std::string& setting_name_prefix, bool validate_settings)
    : S3RequestSettings() {
  for (auto& field : impl->allMutable()) {
    ...
    if (!updated) {
      ...
      if (settings.has(setting_name) && settings.isChanged(setting_name)) field.setValue(settings.get(setting_name));
    }
  }
...
void S3RequestSettings::updateFromSettings(const DB::Settings& settings, bool if_changed, bool validate_settings) {
  ...
  normalizeSettings();
  if (validate_settings) validateUploadSettings();
}

void S3RequestSettings::updateIfChanged(const S3RequestSettings& settings) {
  for (auto& setting : settings.impl->all()) {
    if (setting.isValueChanged()) impl->set(setting.getName(), setting.getValue());
  }
}

void S3RequestSettings::validateUploadSettings() {
  if (!impl->max_part_number) throw Exception(ErrorCodes::INVALID_SETTING_VALUE, "Setting max_part_number cannot be zero");

  if (!impl->strict_upload_part_size) {
    if (!impl->min_upload_part_size)
      throw Exception(ErrorCodes::INVALID_SETTING_VALUE, "Setting min_upload_part_size ({}) cannot be zero",
    ...
    if (!impl->upload_part_size_multiply_factor)
      throw Exception(ErrorCodes::INVALID_SETTING_VALUE, "Setting upload_part_size_multiply_factor cannot be zero");

    if (!impl->upload_part_size_multiply_parts_count_threshold)
      throw Exception(ErrorCodes::INVALID_SETTING_VALUE, "Setting upload_part_size_multiply_parts_count_threshold cannot be zero");
...
void S3RequestSettings::finishInit(const DB::Settings& settings, bool validate_settings) {
  ...
  if (UInt64 max_get_rps = (*this)[S3RequestSetting::max_get_rps].changed ? (*this)[S3RequestSetting::max_get_rps].value
    ...
    size_t max_get_burst =
        (*this)[S3RequestSetting::max_get_burst].changed ? (*this)[S3RequestSetting::max_get_burst].value : default_max_get_burst;

    request_throttler.get_throttler = std::make_shared<Throttler>(max_get_rps, max_get_burst, ProfileEvents::S3GetRequestThrottlerCount,
  ...
  if (UInt64 max_put_rps = (*this)[S3RequestSetting::max_put_rps].changed ? (*this)[S3RequestSetting::max_put_rps].value
    ...
    size_t max_put_burst =
        (*this)[S3RequestSetting::max_put_burst].changed ? (*this)[S3RequestSetting::max_put_burst].value : default_max_put_burst;

    request_throttler.put_throttler = std::make_shared<Throttler>(max_put_rps, max_put_burst, ProfileEvents::S3PutRequestThrottlerCount,
...
void S3RequestSettings::serialize(WriteBuffer& out, ContextPtr) const { impl->writeChangedBinary(out); }
