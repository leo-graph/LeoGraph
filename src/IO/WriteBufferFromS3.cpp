...
WriteBufferFromS3::WriteBufferFromS3(std::shared_ptr<const S3::Client> client_ptr_, const String& bucket_, const String& key_,
                                     size_t buf_size_, const S3::S3RequestSettings& request_settings_, BlobStorageLogWriterPtr blob_log_,
                                     std::optional<std::map<String, String>> object_metadata_,
                                     ThreadPoolCallbackRunnerUnsafe<void> schedule_, const WriteSettings& write_settings_)
    : WriteBufferFromFileBase(std::min(buf_size_, static_cast<size_t>(DBMS_DEFAULT_BUFFER_SIZE)), nullptr, 0),
      ...
      task_tracker(std::make_unique<TaskTracker>(std::move(schedule_), request_settings[S3RequestSetting::max_inflight_parts_for_one_file],
                                                 limited_log)),
...
void WriteBufferFromS3::nextImpl() {
  ...
  if (!multipart_upload_id.empty() || detached_part_data.size() > 1) writeMultipartUpload();

  allocateBuffer();
}

void WriteBufferFromS3::preFinalize() {
  if (is_prefinalized) return;
  ...
  if (multipart_upload_id.empty() && detached_part_data.size() <= 1) {
    if (detached_part_data.empty() ||
        detached_part_data.front().data_size <= request_settings[S3RequestSetting::max_single_part_upload_size])
...
void WriteBufferFromS3::finalizeImpl() {
  ...
  if (!is_prefinalized) preFinalize();
  ...
  if (request_settings[S3RequestSetting::check_objects_after_upload]) {
    ...
    if (actual_size != total_size)
      throw Exception(ErrorCodes::S3_ERROR,
                      "Object {} from bucket {} has unexpected size {} after upload, expected size {}, it's a bug in S3 or S3 API.", key,
                      bucket, actual_size, total_size);
  }
}
...
String WriteBufferFromS3::getVerboseLogDetails() const {
  ...
  if (!multipart_upload_id.empty())
    multipart_upload_details = fmt::format(", upload id {}, upload has finished {}", multipart_upload_id, multipart_upload_finished);

  return fmt::format(
      "Details: bucket {}, key {}, total size {}, count {}, hidden_size {}, offset {}, with pool: {}, prefinalized: {}, finalized: {}{}",
      bucket, key, total_size, count(), hidden_size, offset(), task_tracker->isAsync(), is_prefinalized, finalized,
      multipart_upload_details);
}

String WriteBufferFromS3::getShortLogDetails() const {
  String multipart_upload_details;
  if (!multipart_upload_id.empty()) multipart_upload_details = fmt::format(", upload id {}", multipart_upload_id);

  return fmt::format("Details: bucket {}, key {}{}", bucket, key, multipart_upload_details);
}
...
void WriteBufferFromS3::reallocateFirstBuffer() {
  ...
  if (buffer_allocation_policy->getBufferNumber() > 1 || available() > 0) return;

  const size_t max_first_buffer = buffer_allocation_policy->getBufferSize();
  if (memory.size() == max_first_buffer) return;
...
void WriteBufferFromS3::createMultipartUpload() {
  ...
  if (object_metadata.has_value()) req.SetMetadata(object_metadata.value());
...
void WriteBufferFromS3::abortMultipartUpload() {
  ...
  if (client_ptr->isClientForDisk()) ProfileEvents::increment(ProfileEvents::DiskS3AbortMultipartUpload);
...
void WriteBufferFromS3::writePart(WriteBufferFromS3::PartData&& data) {
  ...
  if (multipart_upload_id.empty())
    throw Exception(ErrorCodes::LOGICAL_ERROR,
                    "Unable to write a part without multipart_upload_id, details: WriteBufferFromS3 created for bucket {}, key {}", bucket,
                    key);
  ...
  if (part_number > request_settings[S3RequestSetting::max_part_number]) {
    throw Exception(
        ...
        request_settings[S3RequestSetting::max_part_number].value, count(), request_settings[S3RequestSetting::min_upload_part_size].value,
        request_settings[S3RequestSetting::max_upload_part_size].value,
        request_settings[S3RequestSetting::upload_part_size_multiply_factor].value,
        request_settings[S3RequestSetting::upload_part_size_multiply_parts_count_threshold].value,
  ...
  auto upload_worker = [&, worker_data, part_number]() {
    ...
    LOG_TEST(limited_log, "Write part succeeded {}, part size {}, part number {}, etag {}", getShortLogDetails(), data_size, part_number,
             part_tag);
  };
...
void WriteBufferFromS3::completeMultipartUpload() {
  ...
  if (multipart_tags.empty()) throw Exception(ErrorCodes::LOGICAL_ERROR, "Failed to complete multipart upload. No parts have uploaded");

  for (size_t i = 0; i < multipart_tags.size(); ++i) {
    const auto tag = multipart_tags.at(i);
    if (tag.empty()) throw Exception(ErrorCodes::LOGICAL_ERROR, "Failed to complete multipart upload. Part {} haven't been uploaded.", i);
  }
  ...
  if (!write_settings.object_storage_write_if_none_match.empty()) req.SetIfNoneMatch(write_settings.object_storage_write_if_none_match);

  if (!write_settings.object_storage_write_if_match.empty()) req.SetIfMatch(write_settings.object_storage_write_if_match);

  Aws::S3::Model::CompletedMultipartUpload multipart_upload;
  for (size_t i = 0; i < multipart_tags.size(); ++i) {
    Aws::S3::Model::CompletedPart part;
    part.WithETag(multipart_tags[i]).WithPartNumber(static_cast<int>(i + 1));
    if (!multipart_checksums.empty()) S3::RequestChecksum::setPartChecksum(part, multipart_checksums.at(i));
  ...
  for (size_t i = 0; i < max_retry; ++i) {
    ...
    if (outcome.GetError().GetErrorType() == Aws::S3::S3Errors::NO_SUCH_KEY) {
      ...
      LOG_INFO(log, "Multipart upload failed with NO_SUCH_KEY error, will retry. {}, Parts: {}", getVerboseLogDetails(),
               multipart_tags.size());
    } else {
      throw S3Exception(outcome.GetError().GetErrorType(), "Message: {}, Key: {}, Bucket: {}, Tags: {}", outcome.GetError().GetMessage(),
                        key, bucket, fmt::join(multipart_tags.begin(), multipart_tags.end(), " "));
    }
  }

  throw S3Exception(Aws::S3::S3Errors::NO_SUCH_KEY,
                    "Message: Multipart upload failed with NO_SUCH_KEY error, retries {}, Key: {}, Bucket: {}", max_retry, key, bucket);
}
  ...
  auto upload_worker = [&, worker_data]() {
    ...
    for (size_t i = 0; i < max_retry; ++i) {
      ...
      if (outcome.GetError().GetErrorType() == Aws::S3::S3Errors::NO_SUCH_KEY) {
        ...
        LOG_ERROR(log, "S3Exception name {}, Message: {}, bucket {}, key {}, object size {}", outcome.GetError().GetExceptionName(),
                  outcome.GetError().GetMessage(), bucket, key, content_length);
        throw S3Exception(outcome.GetError().GetErrorType(), "Message: {}, bucket {}, key {}, object size {}",
                          outcome.GetError().GetMessage(), bucket, key, content_length);
      }
    }

    throw S3Exception(Aws::S3::S3Errors::NO_SUCH_KEY,
                      "Message: Single part upload failed with NO_SUCH_KEY error, retries {}, Key: {}, Bucket: {}", max_retry, key, bucket);
  };
