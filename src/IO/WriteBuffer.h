...
class WriteBuffer : public BufferBase {
 ...
 private:
  ...
  bool isStackUnwinding() const { return exception_level < std::uncaught_exceptions(); }
...
class AutoCanceledWriteBuffer final : public Base {
 ...
 public:
  ...
  ~AutoCanceledWriteBuffer() override {
    if (!this->finalized && !this->canceled) this->cancel();
  }
};
}  // namespace DB
