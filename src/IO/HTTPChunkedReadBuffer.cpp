...
size_t HTTPChunkedReadBuffer::readChunkHeader() {
  ...
  if (!isHexDigit(*in->position())) throw Exception(ErrorCodes::BAD_REQUEST_PARAMETER, "Unexpected data instead of HTTP chunk header");
...
void HTTPChunkedReadBuffer::readChunkFooter() { assertString("\r\n", *in); }

bool HTTPChunkedReadBuffer::nextImpl() {
  if (!in) return false;

  /// The footer of previous chunk.
  if (count()) readChunkFooter();
