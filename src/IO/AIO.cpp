...
int io_setup(unsigned nr, aio_context_t* ctxp) { return static_cast<int>(syscall(__NR_io_setup, nr, ctxp)); }

int io_destroy(aio_context_t ctx) { return static_cast<int>(syscall(__NR_io_destroy, ctx)); }
...
AIOContext::AIOContext(unsigned int nr_events) {
  ctx = 0;
  if (io_setup(nr_events, &ctx) < 0) throw DB::ErrnoException(DB::ErrorCodes::CANNOT_IOSETUP, "io_setup failed");
}
...
AIOContext::AIOContext(AIOContext&& rhs) noexcept { *this = std::move(rhs); }
...
int io_setup(void) { return kqueue(); }

int io_destroy(int ctx) { return close(ctx); }
...
AIOContext::AIOContext(unsigned int) {
  ctx = io_setup();
  if (ctx < 0) throw DB::ErrnoException(DB::ErrorCodes::CANNOT_IOSETUP, "io_setup failed");
}

AIOContext::~AIOContext() { io_destroy(ctx); }
