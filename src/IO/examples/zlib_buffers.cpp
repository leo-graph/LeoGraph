...
int main(int, char **) try {
  ...
  {
    auto buf =
        std::make_unique<DB::WriteBufferFromFile>("test_zlib_buffers.gz", DB::DBMS_DEFAULT_BUFFER_SIZE, O_WRONLY | O_CREAT | O_TRUNC);
    ...
    std::cout << "Writing done. Elapsed: " << stopwatch.elapsedSeconds() << " s."
              << ", " << (static_cast<double>(deflating_buf.count()) / stopwatch.elapsedSeconds() / 1000000) << " MB/s" << std::endl;
  }

  {
    ...
    for (size_t i = 0; i < n; ++i) {
      ...
      if (x != i) throw DB::Exception(0, "Failed!, read: {}, expected: {}", x, i);
    }
    stopwatch.stop();
    std::cout << "Reading done. Elapsed: " << stopwatch.elapsedSeconds() << " s."
              << ", " << (static_cast<double>(inflating_buf.count()) / stopwatch.elapsedSeconds() / 1000000) << " MB/s" << std::endl;
  }
