...
int main(int, char **) {
  {
    ...
    if (readAndPrint(in)) std::cout << "readAndPrint from ReadBufferFromString failed" << std::endl;
  }
  ...
  if (readAndPrint(*in)) std::cout << "readAndPrint from ReadBufferFromOwnString failed" << std::endl;
