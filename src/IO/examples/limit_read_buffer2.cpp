...
int main(int, char **) try {
...

  {
    ...
    if (limit_in.count() != 1) throw Exception(ErrorCodes::LOGICAL_ERROR, "Failed!, incorrect count(): {}", limit_in.count());

    if (in.count() != limit_in.count())
      throw Exception(ErrorCodes::LOGICAL_ERROR, "Failed!, incorrect underlying buffer's count(): {}", in.count());
    if (src != dst) throw Exception(ErrorCodes::LOGICAL_ERROR, "Failed!, incorrect destination value, read: {}, expected: {}", dst, src);
  }
  {
    std::string src = "abc";
    ReadBuffer in(src.data(), src.size(), 0);
    ...
    if (dst != "b") throw Exception(ErrorCodes::LOGICAL_ERROR, "Failed!, Incorrect destination value: {}, expected 'b'", dst);

    char y;
    readChar(y, in);
    if (y != 'c') throw Exception(ErrorCodes::LOGICAL_ERROR, "Failed!, Read incorrect value from underlying buffer: {}, expected 'c'", y);
    while (!in.eof()) in.ignore();
    if (in.count() != 3)
