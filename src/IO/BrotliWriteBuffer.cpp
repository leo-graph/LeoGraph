...
BrotliWriteBuffer::BrotliStateWrapper::BrotliStateWrapper() : state(BrotliEncoderCreateInstance(nullptr, nullptr, nullptr)) {}

BrotliWriteBuffer::BrotliStateWrapper::~BrotliStateWrapper() { BrotliEncoderDestroyInstance(state); }
...
void BrotliWriteBuffer::nextImpl() {
  ...
  try {
    do {
      ...
      int result =
          BrotliEncoderCompressStream(brotli->state, BROTLI_OPERATION_PROCESS, &in_available, &in_data, &out_capacity, &out_data, nullptr);
...
void BrotliWriteBuffer::finalFlushBefore() {
  ...
  if (!compress_empty && total_in == 0) return;
  ...
  while (true) {
    ...
    int result =
        BrotliEncoderCompressStream(brotli->state, BROTLI_OPERATION_FINISH, &in_available, &in_data, &out_capacity, &out_data, nullptr);
