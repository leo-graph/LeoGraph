...
void MySQLPacketPayloadWriteBuffer::setWorkingBuffer() {
  ...
  if (payload_length - bytes_written == 0) {
    /// Finished writing packet. Due to an implementation of WriteBuffer, working_buffer cannot be empty. Further write attempts will throw
    /// Exception.
    eof = true;
...
void MySQLPacketPayloadWriteBuffer::nextImpl() {
  ...
  if (eof) throw Exception(ErrorCodes::CANNOT_WRITE_AFTER_END_OF_BUFFER, "Cannot write after end of buffer.");

  out.position() += written;
  bytes_written += written;

  /// Packets of size greater than MAX_PACKET_LENGTH are split into few packets of size MAX_PACKET_LENGTH and las packet of size <
  /// MAX_PACKET_LENGTH.
  if (bytes_written == payload_length && (total_left > 0 || payload_length == MAX_PACKET_LENGTH)) startNewPacket();
