...
inline void readStringBinary(std::string &s, ReadBuffer &buf, size_t max_string_size = DEFAULT_MAX_STRING_SIZE) {
  ...
  if (size > max_string_size) throw Exception(ErrorCodes::TOO_LARGE_STRING_SIZE, "Too large string size.");
...
inline void readIPv6Binary(IPv6 &ip, ReadBuffer &buf) {
  ...
  if (size != sizeof(IPv6::UnderlyingType))
    throw Exception(ErrorCodes::SIZE_OF_FIXED_STRING_DOESNT_MATCH, "Size of the string {} doesn't match size of binary IPv6 {}", size,
...
inline bool checkChar(char c, ReadBuffer &buf) {
  char a;
  if (!buf.peek(a) || a != c) return false;
  buf.ignore();
  return true;
}
...
inline bool checkCharCaseInsensitive(char c, ReadBuffer &buf) {
  char a;
  if (!buf.peek(a) || !equalsCaseInsensitive(a, c)) return false;
  buf.ignore();
  return true;
}

inline void assertString(const String &s, ReadBuffer &buf) { assertString(s.c_str(), buf); }

bool checkString(const char *s, ReadBuffer &buf);
inline bool checkString(const String &s, ReadBuffer &buf) { return checkString(s.c_str(), buf); }

bool checkStringCaseInsensitive(const char *s, ReadBuffer &buf);
inline bool checkStringCaseInsensitive(const String &s, ReadBuffer &buf) { return checkStringCaseInsensitive(s.c_str(), buf); }

void assertStringCaseInsensitive(const char *s, ReadBuffer &buf);
inline void assertStringCaseInsensitive(const String &s, ReadBuffer &buf) { assertStringCaseInsensitive(s.c_str(), buf); }
 ...
 *     - If settings.custom_delimiter is not specified, it is read until next settings.delimiter, either until end of line (CR or LF) or
 * until end of stream;
...
inline ReturnType readDateTextImpl(LocalDate &date, ReadBuffer &buf, const char *allowed_delimiters = nullptr) {
  ...
  if (!buf.eof() && buf.position() + 10 <= buf.buffer().end()) {
    ...
    if (!isNumericASCII(pos[0]) || !isNumericASCII(pos[1]) || !isNumericASCII(pos[2]) || !isNumericASCII(pos[3])) return error();
    ...
    if (isNumericASCII(pos[-1])) {
      /// YYYYMMDD
      if (!isNumericASCII(pos[0]) || !isNumericASCII(pos[1]) || !isNumericASCII(pos[2])) return error();

      month = (pos[-1] - '0') * 10 + (pos[0] - '0');
      day = (pos[1] - '0') * 10 + (pos[2] - '0');
      pos += 3;
    } else {
      if (!isSymbolIn(pos[-1], allowed_delimiters)) return error();

      if (!isNumericASCII(pos[0])) return error();
      ...
      if (isNumericASCII(pos[-1]) || !isNumericASCII(pos[0])) return error();
...
inline ReturnType readDateTextImpl(DayNum &date, ReadBuffer &buf, const DateLUTImpl &date_lut, const char *allowed_delimiters = nullptr,
  ...
  if constexpr (throw_exception) {
    ...
    if (!readDateTextImpl<ReturnType>(local_date, buf, allowed_delimiters)) return false;
    ...
    if (saturate_on_overflow) {
      ...
      if (!tryToConvertToDayNum(date, *ret)) return false;
    }
...
inline ReturnType readDateTextImpl(ExtendedDayNum &date, ReadBuffer &buf, const DateLUTImpl &date_lut,
                                   const char *allowed_delimiters = nullptr) {
  static constexpr bool throw_exception = std::is_same_v<ReturnType, void>;
  ...
  /// When the parameter is out of rule or out of range, Date32 uses 1925-01-01 as the default value
  /// (-DateLUT::instance().getDayNumOffsetEpoch(), -16436) and Date uses 1970-01-01.
  date = makeDayNum(date_lut, local_date.year(), local_date.month(), local_date.day(), -static_cast<Int32>(getDayNumOffsetEpoch()));
  return ReturnType(true);
}

inline void readDateText(LocalDate &date, ReadBuffer &buf) { readDateTextImpl<void>(date, buf); }
...
inline bool tryReadDateText(DayNum &date, ReadBuffer &buf, const DateLUTImpl &time_zone = DateLUT::instance(),
                            const char *allowed_delimiters = nullptr, bool saturate_on_overflow = true) {
...
inline void readUUIDText(UUID &uuid, ReadBuffer &buf) { readUUIDTextImpl<void>(uuid, buf); }

inline bool tryReadUUIDText(UUID &uuid, ReadBuffer &buf) { return readUUIDTextImpl<bool>(uuid, buf); }
...
inline void readIPv4Text(IPv4 &ip, ReadBuffer &buf) { readIPv4TextImpl<void>(ip, buf); }

inline bool tryReadIPv4Text(IPv4 &ip, ReadBuffer &buf) { return readIPv4TextImpl<bool>(ip, buf); }
...
inline void readIPv6Text(IPv6 &ip, ReadBuffer &buf) { readIPv6TextImpl<void>(ip, buf); }

inline bool tryReadIPv6Text(IPv6 &ip, ReadBuffer &buf) { return readIPv6TextImpl<bool>(ip, buf); }
...
ReturnType readDateTimeTextFallback(time_t &datetime, ReadBuffer &buf, const DateLUTImpl &date_lut,
                                    const char *allowed_date_delimiters = nullptr, const char *allowed_time_delimiters = nullptr,
                                    bool saturate_on_overflow = true);

template <typename ReturnType = void, bool t64_mode = false>
ReturnType readTimeTextFallback(time_t &time, ReadBuffer &buf, const DateLUTImpl &date_lut, const char *allowed_date_delimiters = nullptr,
  ...
  if (optimistic_path_for_date_time_input) {
    if (s[4] < '0' || s[4] > '9') {
      if constexpr (!throw_exception) {
        ...
        if (!isSymbolIn(s[4], allowed_date_delimiters) || !isSymbolIn(s[7], allowed_date_delimiters)) return ReturnType(false);
      }
      ...
      if (dt_long) {
        if constexpr (!throw_exception) {
          ...
          if (!isSymbolIn(s[13], allowed_time_delimiters) || !isSymbolIn(s[16], allowed_time_delimiters)) return ReturnType(false);
        }
...
inline ReturnType readTimeTextImpl(time_t &time, ReadBuffer &buf, const DateLUTImpl &date_lut,
  ...
  if (!allowed_time_delimiters) allowed_time_delimiters = ":";
  ...
  if (available_bytes > 0)  // Always try optimistic path if we have some bytes
  {
    ...
    if (available_bytes >= 9 && isNumericASCII(s[0]) && isNumericASCII(s[1]) && isNumericASCII(s[2]) &&
        isSymbolIn(s[3], allowed_time_delimiters) && isNumericASCII(s[4]) && isNumericASCII(s[5]) &&
        isSymbolIn(s[6], allowed_time_delimiters) && isNumericASCII(s[7]) && isNumericASCII(s[8])) {
    ...
    // HH:MM:SS
    else if (available_bytes >= 8 && isNumericASCII(s[0]) && isNumericASCII(s[1]) && isSymbolIn(s[2], allowed_time_delimiters) &&
             isNumericASCII(s[3]) && isNumericASCII(s[4]) && isSymbolIn(s[5], allowed_time_delimiters) && isNumericASCII(s[6]) &&
             isNumericASCII(s[7])) {
      hour = (s[0] - '0') * 10 + (s[1] - '0');
    ...
    // H:MM:SS
    else if (available_bytes >= 7 && isNumericASCII(s[0]) && isSymbolIn(s[1], allowed_time_delimiters) && isNumericASCII(s[2]) &&
             isNumericASCII(s[3]) && isSymbolIn(s[4], allowed_time_delimiters) && isNumericASCII(s[5]) && isNumericASCII(s[6])) {
      hour = (s[0] - '0');
    ...
    // MM:SS
    else if (available_bytes >= 5 && isNumericASCII(s[0]) && isNumericASCII(s[1]) && isSymbolIn(s[2], allowed_time_delimiters) &&
    ...
    // M:SS
    else if (available_bytes >= 4 && isNumericASCII(s[0]) && isSymbolIn(s[1], allowed_time_delimiters) && isNumericASCII(s[2]) &&
             isNumericASCII(s[3])) {
      hour = 0;
      minute = s[0] - '0';
...
inline ReturnType readDateTimeTextImpl(DateTime64 &datetime64, UInt32 scale, ReadBuffer &buf, const DateLUTImpl &date_lut,
  ...
  if (!buf.eof() && *buf.position() == '.') {
    ...
    while (!buf.eof() && isNumericASCII(*buf.position())) ++buf.position();
  ...
  if constexpr (std::is_same_v<ReturnType, void>) {
    ...
    if (is_ok) datetime64 *= negative_fraction_multiplier;
  }
...
template <typename ReturnType>
inline ReturnType readTimeTextImpl(Time64 &time64, UInt32 scale, ReadBuffer &buf, const DateLUTImpl &date_lut,
                                   const char *allowed_date_delimiters = nullptr, const char *allowed_time_delimiters = nullptr) {
  static constexpr bool throw_exception = std::is_same_v<ReturnType, void>;
  ...
  if constexpr (throw_exception) {
    ...
    parse_success = ok;
    if (!ok && (buf.eof() || *buf.position() != '.')) return ReturnType(false);
  }
  ...
  if (!buf.eof() && *buf.position() == '.') {
    ...
    if (is_negative_timestamp && components.whole == 0 && components.fractional != 0) negative_fraction_multiplier = -1;
  }
  ...
  if constexpr (std::is_same_v<ReturnType, void>) {
    ...
    if (is_ok) time64 *= negative_fraction_multiplier;
  }
...
inline bool tryReadTimeText(time_t &time, ReadBuffer &buf, const DateLUTImpl &time_zone = DateLUT::instance(),
                            const char *allowed_date_delimiters = nullptr, const char *allowed_time_delimiters = nullptr) {
...
inline bool tryReadTime64Text(Time64 &time64, UInt32 scale, ReadBuffer &buf, const DateLUTImpl &date_lut = DateLUT::instance(),
                              const char *allowed_date_delimiters = nullptr, const char *allowed_time_delimiters = nullptr) {
...
inline void readDateTimeText(LocalDateTime &datetime, ReadBuffer &buf) {
  ...
  if (buf.eof() || !(*buf.position() == ' ' || *buf.position() == 'T')) return;
...
inline void readTime64Text(Decimal64 &time64, UInt32 scale, ReadBuffer &buf) { readTimeTextImpl<void>(time64, scale, buf); }
...
inline void readBinary(UUID &x, ReadBuffer &buf) { readUUIDBinary(x, buf); }
...
inline bool tryReadText(is_integer auto &x, ReadBuffer &buf) { return tryReadIntText(x, buf); }

inline bool tryReadText(is_floating_point auto &x, ReadBuffer &buf) { return tryReadFloatText(x, buf); }
...
inline void readText(T &x, ReadBuffer &buf) {
  readFloatText(x, buf);
}
...
inline bool tryReadText(DayNum &x, ReadBuffer &buf, const DateLUTImpl &time_zone = DateLUT::instance()) {
  return tryReadDateText(x, buf, time_zone);
}
...
inline bool tryReadText(LocalDateTime &x, ReadBuffer &buf) {
  time_t time;
  if (!tryReadDateTimeText(time, buf)) return false;
...
template <typename T>
  requires is_arithmetic_v<T>
inline void readQuoted(T &x, ReadBuffer &buf) {
  readText(x, buf);
}

template <typename T>
  requires is_arithmetic_v<T>
inline void readQuoted(T &x, ReadBuffer &buf, const DateLUTImpl &time_zone) {
...
template <typename T>
  requires is_arithmetic_v<T>
inline void readDoubleQuoted(T &x, ReadBuffer &buf) {
  readText(x, buf);
}

template <typename T>
  requires is_arithmetic_v<T>
inline void readDoubleQuoted(T &x, ReadBuffer &buf, const DateLUTImpl &time_zone) {
  readText(x, buf, time_zone);
}
...
inline ReturnType readCSVSimple(T &x, ReadBuffer &buf) {
  ...
  if (maybe_quote == '\'' || maybe_quote == '\"') ++buf.position();
...
void readBinary(std::vector<T, Alloc> &x, ReadBuffer &buf) {
  ...
  if (size > DEFAULT_MAX_STRING_SIZE) throw Poco::Exception("Too large vector size.");

  x.resize(size);
  for (size_t i = 0; i < size; ++i) readBinary(x[i], buf);
}
...
inline void skipWhitespaceIfAny(ReadBuffer &buf, bool one_line = false) {
  if (!one_line)
    while (!buf.eof() && isWhitespaceASCII(*buf.position())) ++buf.position();
  else
    while (!buf.eof() && isWhitespaceASCIIOneLine(*buf.position())) ++buf.position();
}
...
inline bool tryParse(T &res, const char *data, size_t size) {
  ReadBufferFromMemory buf(data, size);
  if (!tryReadText(res, buf)) return false;
  return buf.eof();
}
