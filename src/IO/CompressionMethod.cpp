...
CompressionMethod chooseHTTPCompressionMethod(const std::string& list) {
  ...
  if (list.contains("zstd")) return CompressionMethod::Zstd;
  if (list.contains("br")) return CompressionMethod::Brotli;
  if (list.contains("lz4")) return CompressionMethod::Lz4;
  if (list.contains("snappy")) return CompressionMethod::Snappy;
  if (list.contains("gzip")) return CompressionMethod::Gzip;
  if (list.contains("deflate")) return CompressionMethod::Zlib;
  if (list.contains("xz")) return CompressionMethod::Xz;
  if (list.contains("bz2")) return CompressionMethod::Bzip2;
...
CompressionMethod chooseCompressionMethod(const std::string& path, const std::string& hint) {
  ...
  if (hint.empty() || hint == "auto") {
    ...
    if (pos != std::string::npos) file_extension = path.substr(pos + 1, std::string::npos);
  }
  ...
  if (method_str == "gzip" || method_str == "gz") return CompressionMethod::Gzip;
  if (method_str == "deflate") return CompressionMethod::Zlib;
  if (method_str == "brotli" || method_str == "br") return CompressionMethod::Brotli;
  if (method_str == "lzma" || method_str == "xz") return CompressionMethod::Xz;
  if (method_str == "zstd" || method_str == "zst") return CompressionMethod::Zstd;
  if (method_str == "lz4") return CompressionMethod::Lz4;
  if (method_str == "bz2") return CompressionMethod::Bzip2;
  if (method_str == "snappy") return CompressionMethod::Snappy;
  if (hint.empty() || hint == "auto" || hint == "none") return CompressionMethod::None;

  throw Exception(ErrorCodes::NOT_IMPLEMENTED,
                  "Unknown compression method '{}'. "
                  "Only 'auto', 'none', 'gzip', 'deflate', 'br', 'xz', 'zstd', 'lz4', 'bz2', 'snappy' are supported as compression methods",
                  hint);
}
...
static std::unique_ptr<CompressedReadBufferWrapper> createCompressedWrapper(std::unique_ptr<ReadBuffer> nested, CompressionMethod method,
...
#if USE_BZIP2
  if (method == CompressionMethod::Bzip2) return std::make_unique<Bzip2ReadBuffer>(std::move(nested), buf_size, existing_memory, alignment);
#endif
#if USE_SNAPPY
...
std::unique_ptr<ReadBuffer> wrapReadBufferWithCompressionMethod(std::unique_ptr<ReadBuffer> nested, CompressionMethod method,
                                                                int zstd_window_log_max, size_t buf_size, char* existing_memory,
                                                                size_t alignment) {
  if (method == CompressionMethod::None) return nested;
...
std::unique_ptr<WriteBuffer> createWriteCompressedWrapper(WriteBufferT&& nested, CompressionMethod method, int level, int zstd_window_log,
                                                          size_t buf_size, char* existing_memory, size_t alignment, bool compress_empty) {
  if (method == DB::CompressionMethod::Gzip || method == CompressionMethod::Zlib)
    return std::make_unique<ZlibDeflatingWriteBuffer>(std::forward<WriteBufferT>(nested), method, level, buf_size, existing_memory,
                                                      alignment, compress_empty);
  ...
  if (method == CompressionMethod::Zstd)
    return std::make_unique<ZstdDeflatingWriteBuffer>(std::forward<WriteBufferT>(nested), level, zstd_window_log, buf_size, existing_memory,
                                                      alignment, compress_empty);
...
#endif
#if USE_SNAPPY
  if (method == CompressionMethod::Snappy) throw Exception(ErrorCodes::NOT_IMPLEMENTED, "Unsupported compression method");
#endif
...
std::unique_ptr<WriteBuffer> wrapWriteBufferWithCompressionMethod(std::unique_ptr<WriteBuffer> nested, CompressionMethod method, int level,
                                                                  int zstd_window_log, size_t buf_size, char* existing_memory,
                                                                  size_t alignment, bool compress_empty) {
  if (method == CompressionMethod::None) return nested;
...
std::unique_ptr<WriteBuffer> wrapWriteBufferWithCompressionMethod(WriteBuffer* nested, CompressionMethod method, int level,
                                                                  int zstd_window_log, size_t buf_size, char* existing_memory,
                                                                  size_t alignment, bool compress_empty) {
