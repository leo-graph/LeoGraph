...
bool isRetriableError(const Poco::Net::HTTPResponse::HTTPStatus http_status) noexcept {
  static constexpr std::array non_retriable_errors{
      Poco::Net::HTTPResponse::HTTPStatus::HTTP_BAD_REQUEST,     Poco::Net::HTTPResponse::HTTPStatus::HTTP_UNAUTHORIZED,
      Poco::Net::HTTPResponse::HTTPStatus::HTTP_NOT_FOUND,       Poco::Net::HTTPResponse::HTTPStatus::HTTP_FORBIDDEN,
      Poco::Net::HTTPResponse::HTTPStatus::HTTP_NOT_IMPLEMENTED, Poco::Net::HTTPResponse::HTTPStatus::HTTP_METHOD_NOT_ALLOWED};

  return std::all_of(non_retriable_errors.begin(), non_retriable_errors.end(), [&](const auto status) { return http_status != status; });
}
...
Poco::URI getUriAfterRedirect(const Poco::URI &prev_uri, Poco::Net::HTTPResponse &response, bool enable_url_encoding) {
  ...
  if (!location_uri.isRelative()) return location_uri;
...
class ReadBufferFromSessionResponse : public DB::ReadBufferFromIStream {
 ...
 public:
  ReadBufferFromSessionResponse(DB::HTTPSessionPtr &&session_, std::istream &rstr, size_t size)
      : ReadBufferFromIStream(rstr, size), session(std::move(session_)) {}
};
...
size_t ReadWriteBufferFromHTTP::getOffset() const { return read_range.begin.value_or(0) + offset_from_begin_pos; }
...
void ReadWriteBufferFromHTTP::prepareRequest(Poco::Net::HTTPRequest &request, std::optional<HTTPRange> range) const {
  ...
  for (const auto &[header, value] : http_header_entries) request.set(header, value);
  ...
  if (!credentials.getUsername().empty()) credentials.authenticate(request);
}
...
bool ReadWriteBufferFromHTTP::supportsReadAt() {
  if (!file_info) file_info = getFileInfo();
...
String ReadWriteBufferFromHTTP::getFileName() const { return initial_uri.toString(); }

void ReadWriteBufferFromHTTP::getHeadResponse(Poco::Net::HTTPResponse &response) {
  doWithRetries([&]() { callWithRedirects(response, Poco::Net::HTTPRequest::HTTP_HEAD, {}); },
                /*on_retry=*/nullptr,
                /*mute_logging=*/true);
}

ReadWriteBufferFromHTTP::ReadWriteBufferFromHTTP(const HTTPConnectionGroupType &connection_group_, const Poco::URI &uri_,
                                                 const std::string &method_, ProxyConfiguration proxy_config_, ReadSettings read_settings_,
                                                 ConnectionTimeouts timeouts_, const Poco::Net::HTTPBasicCredentials &credentials_,
                                                 const RemoteHostFilter *remote_host_filter_, size_t buffer_size_, size_t max_redirects_,
                                                 bool enable_url_encoding_, OutStreamCallback out_stream_callback_,
                                                 bool use_external_buffer_, bool http_skip_not_found_url_,
                                                 HTTPHeaderEntries http_header_entries_, bool delay_initialization,
                                                 std::optional<HTTPFileInfo> file_info_)
    : SeekableReadBuffer(nullptr, 0),
      connection_group(connection_group_),
      initial_uri(uri_),
      method(!method_.empty()       ? method_
             : out_stream_callback_ ? Poco::Net::HTTPRequest::HTTP_POST
  ...
  if (current_uri.getPath().empty()) current_uri.setPath("/");
  ...
  if (read_settings.http_max_tries <= 0 || read_settings.http_retry_initial_backoff_ms <= 0 ||
    ...
    throw Exception(ErrorCodes::BAD_ARGUMENTS,
                    ...
                    read_settings.http_max_tries, read_settings.http_retry_initial_backoff_ms, read_settings.http_retry_max_backoff_ms);
...
ReadWriteBufferFromHTTP::CallResult ReadWriteBufferFromHTTP::callImpl(Poco::Net::HTTPResponse &response, const std::string &method_,
                                                                      const std::optional<HTTPRange> &range, bool allow_redirects) const {
  if (remote_host_filter) remote_host_filter->checkURL(current_uri);
  ...
  if (out_stream_callback) out_stream_callback(stream_out);
...
ReadWriteBufferFromHTTP::CallResult ReadWriteBufferFromHTTP::callWithRedirects(Poco::Net::HTTPResponse &response, const String &method_,
  ...
  while (isRedirect(response.getStatus())) {
    ...
    if (redirects > max_redirects)
      throw Exception(ErrorCodes::TOO_MANY_REDIRECTS,
                      ...
                      " Redirects are restricted to prevent possible attack when a malicious server redirects to an internal resource, "
                      "bypassing the authentication or firewall.",
...
void ReadWriteBufferFromHTTP::doWithRetries(std::function<void()> &&callable, std::function<void()> on_retry, bool mute_logging) const {
  ...
  for (size_t attempt = 1; attempt <= read_settings.http_max_tries; ++attempt) {
    ...
    try {
      ...
      if (!isRetriableError(e.getHTTPStatus())) is_retriable = false;
      ...
      if (e.code() == POCO_EMFILE) is_retriable = false;
    ...
    if (last_attempt || !is_retriable) {
      if (!mute_logging)
        LOG_DEBUG(log,
                  ...
                  current_uri.toString() == initial_uri.toString() ? String() : fmt::format(" redirect to '{}'", current_uri.toString()),
                  error_message, attempt, read_settings.http_max_tries);
      ...
      if (!mute_logging)
        LOG_TRACE(log,
                  ...
                  initial_uri.toString(),
                  current_uri.toString() == initial_uri.toString() ? String() : fmt::format(" redirect to '{}'", current_uri.toString()),
                  error_message, attempt + 1, read_settings.http_max_tries, milliseconds_to_wait, read_settings.http_retry_max_backoff_ms);
...
std::unique_ptr<ReadBuffer> ReadWriteBufferFromHTTP::initialize() {
  ...
  if (withPartialContent()) range = HTTPRange{getOffset(), read_range.end};
  ...
  if (range.has_value() && response.getStatus() != Poco::Net::HTTPResponse::HTTPStatus::HTTP_PARTIAL_CONTENT) {
    ...
    if (getOffset() != 0) {
      ...
      if (response.getStatus() == Poco::Net::HTTPResponse::HTTPStatus::HTTP_OK) {
        String explanation =
            fmt::format("Cannot read with range: [{}, {}] (response status: {}, reason: {}), will retry", getOffset(),
                        read_range.end ? toString(*read_range.end) : "-", toString(response.getStatus()), response.getReason());

        /// it is retriable error
        throw HTTPException(ErrorCodes::HTTP_RANGE_NOT_SATISFIABLE, current_uri.toString(),
                            Poco::Net::HTTPResponse::HTTP_REQUESTED_RANGE_NOT_SATISFIABLE, response.getReason(), explanation);
      }
      throw Exception(ErrorCodes::HTTP_RANGE_NOT_SATISFIABLE, "Cannot read with range: [{}, {}] (response status: {}, reason: {})",
                      getOffset(), read_range.end ? toString(*read_range.end) : "-", toString(response.getStatus()), response.getReason());
    }
    if (read_range.end) {
  ...
  if (!read_range.end && response.hasContentLength()) file_info = parseFileInfo(response, range.has_value() ? getOffset() : 0);

  return std::move(result).transformToReadBuffer(use_external_buffer ? 0 : buffer_size);
}

bool ReadWriteBufferFromHTTP::nextImpl() {
  if (next_callback) next_callback(count());
      ...
      [&]() {
        ...
        if (response.getStatus() != Poco::Net::HTTPResponse::HTTPStatus::HTTP_PARTIAL_CONTENT &&
          ...
          String explanation = fmt::format(
              ...
              initial_uri.toString(), *range.begin, *range.end, toString(response.getStatus()), response.getReason());
...
off_t ReadWriteBufferFromHTTP::getPosition() { return getOffset() - available(); }

off_t ReadWriteBufferFromHTTP::seek(off_t offset_, int whence) {
  if (whence != SEEK_SET) throw Exception(ErrorCodes::CANNOT_SEEK_THROUGH_FILE, "Only SEEK_SET mode is allowed.");

  if (offset_ < 0) throw Exception(ErrorCodes::SEEK_POSITION_OUT_OF_BOUND, "Seek position is out of bounds. Offset: {}", offset_);
  ...
  if (impl) {
    ...
    if (!atEndOfRequestedRangeGuess()) ProfileEvents::increment(ProfileEvents::ReadBufferSeekCancelConnection);
...
void ReadWriteBufferFromHTTP::setReadUntilPosition(size_t until) {
  ...
  if (read_range.end && *read_range.end + 1 == until) return;
...
bool ReadWriteBufferFromHTTP::atEndOfRequestedRangeGuess() {
  if (!impl) return true;
  if (read_range.end) return getPosition() > static_cast<off_t>(*read_range.end);
  if (file_info && file_info->file_size) return getPosition() >= static_cast<off_t>(*file_info->file_size);
  return false;
}

std::string ReadWriteBufferFromHTTP::getResponseCookie(const std::string &name, const std::string &def) const {
  for (const auto &cookie : cookies)
    if (cookie.getName() == name) return cookie.getValue();
  return def;
}
...
const std::string &ReadWriteBufferFromHTTP::getCompressionMethod() const { return content_encoding; }
...
ReadWriteBufferFromHTTP::HTTPFileInfo ReadWriteBufferFromHTTP::getFileInfo() {
  ...
  if (!read_settings.http_make_head_request) return HTTPFileInfo{};
  ...
  try {
    ...
    if (e.getHTTPStatus() >= 400 && e.getHTTPStatus() <= 499 && e.getHTTPStatus() != Poco::Net::HTTPResponse::HTTP_TOO_MANY_REQUESTS &&
...
ReadWriteBufferFromHTTP::HTTPFileInfo ReadWriteBufferFromHTTP::parseFileInfo(const Poco::Net::HTTPResponse &response,
                                                                             size_t requested_range_begin) {
  HTTPFileInfo res;
  ...
  if (response.has("Last-Modified")) {
    ...
    if (end == date_str.data() + date_str.size()) res.last_modified = timegm(&info);
  }
...
ReadWriteBufferFromHTTPPtr BuilderRWBufferFromHTTP::create(const Poco::Net::HTTPBasicCredentials &credentials_) {
  ...
  std::unique_ptr<ReadWriteBufferFromHTTP> ptr(
      new ReadWriteBufferFromHTTP(connection_group, uri, method, proxy_configuration, read_settings, timeouts, credentials_,
                                  remote_host_filter, buffer_size, max_redirects, enable_url_encoding, out_stream_callback,
                                  use_external_buffer, http_skip_not_found_url, http_header_entries, delay_initialization,
