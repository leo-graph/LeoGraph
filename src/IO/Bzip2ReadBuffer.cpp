...
class Bzip2ReadBuffer::Bzip2StateWrapper {
 public:
  Bzip2StateWrapper() {
    ...
    if (ret != BZ_OK) throw Exception(ErrorCodes::BZIP2_STREAM_DECODER_FAILED, "bzip2 stream encoder init failed: error code: {}", ret);
  }

  ~Bzip2StateWrapper() { BZ2_bzDecompressEnd(&stream); }
  ...
  void reinitialize() {
    ...
    if (ret != BZ_OK)
      throw Exception(ErrorCodes::BZIP2_STREAM_DECODER_FAILED, "bzip2 stream encoder reinit decompress end failed: error code: {}", ret);

    memset(&stream, 0, sizeof(bz->stream));

    ret = BZ2_bzDecompressInit(&stream, 0, 0);

    if (ret != BZ_OK) throw Exception(ErrorCodes::BZIP2_STREAM_DECODER_FAILED, "bzip2 stream encoder reinit failed: error code: {}", ret);
...
bool Bzip2ReadBuffer::nextImpl() {
  ...
  if (ret != BZ_OK)
    throw Exception(ErrorCodes::BZIP2_STREAM_DECODER_FAILED, "bzip2 stream decoder failed: error code: {}{}", ret,
                    getExceptionEntryWithFileName(*in));

  if (in->eof()) {
    eof_flag = true;
    throw Exception(ErrorCodes::UNEXPECTED_END_OF_FILE, "Unexpected end of bzip2 archive{}", getExceptionEntryWithFileName(*in));
  }
