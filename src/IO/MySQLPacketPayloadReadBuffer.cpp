...
MySQLPacketPayloadReadBuffer::MySQLPacketPayloadReadBuffer(ReadBuffer &in_, uint8_t &sequence_id_)
    : ReadBuffer(in_.position(), 0),
      in(in_),
      sequence_id(sequence_id_)  // not in.buffer().begin(), because working buffer may include previous packet
{}
...
bool MySQLPacketPayloadReadBuffer::nextImpl() {
  if (!has_read_header || (payload_length == MAX_PACKET_LENGTH && offset == payload_length)) {
    ...
    if (payload_length > MAX_PACKET_LENGTH)
      throw Exception(ErrorCodes::UNKNOWN_PACKET_FROM_CLIENT, "Received packet with payload larger than max_packet_size: {}",
                      payload_length);

    size_t packet_sequence_id = 0;
    in.readStrict(reinterpret_cast<char &>(packet_sequence_id));
    if (packet_sequence_id != sequence_id)
      throw Exception(ErrorCodes::UNKNOWN_PACKET_FROM_CLIENT, "Received packet with wrong sequence-id: {}. Expected: {}.",
  ...
  if (!in.hasPendingData()) return false;
