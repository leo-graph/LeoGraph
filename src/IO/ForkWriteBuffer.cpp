...
ForkWriteBuffer::ForkWriteBuffer(WriteBufferPtrs&& sources_) : WriteBuffer(nullptr, 0), sources(std::move(sources_)) {
  if (sources.empty()) {
...
void ForkWriteBuffer::preFinalize() {
  ...
  for (const WriteBufferPtr& buffer : sources) buffer->preFinalize();
}

void ForkWriteBuffer::finalizeImpl() {
  preFinalize();
  for (const WriteBufferPtr& buffer : sources) buffer->finalize();
}

void ForkWriteBuffer::cancelImpl() noexcept {
  WriteBuffer::cancelImpl();
  for (const WriteBufferPtr& buffer : sources) buffer->cancel();
}
