...
ReadBufferFromMemoryFileBase::ReadBufferFromMemoryFileBase(bool owns_memory, String file_name_, std::string_view data)
    : ReadBufferFromFileBase(data.size() /*buf_size*/, owns_memory ? nullptr : const_cast<char *>(data.data()) /*existing_memory*/,
                             0 /*alignment*/, data.size() /*file_size*/),
      file_name(std::move(file_name_)) {
  chassert(data.size() == internal_buffer.size());

  if (owns_memory) std::memcpy(internal_buffer.begin(), data.data(), data.size());
