...
class ReadBufferFromFile : public ReadBufferFromFileDescriptor {
 ...
 public:
  explicit ReadBufferFromFile(const std::string& file_name_, size_t buf_size = DBMS_DEFAULT_BUFFER_SIZE, int flags = -1,
                              char* existing_memory = nullptr, size_t alignment = 0, std::optional<size_t> file_size_ = std::nullopt,
  ...
  explicit ReadBufferFromFile(
      ...
      const std::string& original_file_name = {}, size_t buf_size = DBMS_DEFAULT_BUFFER_SIZE, char* existing_memory = nullptr,
      size_t alignment = 0, std::optional<size_t> file_size_ = std::nullopt, ThrottlerPtr throttler = {});
...
class ReadBufferFromFilePRead : public ReadBufferFromFile {
 public:
  explicit ReadBufferFromFilePRead(const std::string& file_name_, size_t buf_size = DBMS_DEFAULT_BUFFER_SIZE, int flags = -1,
                                   char* existing_memory = nullptr, size_t alignment = 0, std::optional<size_t> file_size_ = std::nullopt)
...
class ReadBufferFromFilePReadWithDescriptorsCache : public ReadBufferFromFileDescriptorPRead {
 ...
 public:
  explicit ReadBufferFromFilePReadWithDescriptorsCache(const std::string& file_name_, size_t buf_size = DBMS_DEFAULT_BUFFER_SIZE,
                                                       int flags = -1, char* existing_memory = nullptr, size_t alignment = 0,
                                                       std::optional<size_t> file_size_ = std::nullopt, ThrottlerPtr throttler_ = {})
