...
void OpenedFile::open() const {
  ...
  if (-1 == fd)
    DB::ErrnoException::throwFromPath(errno == ENOENT ? ErrorCodes::FILE_DOESNT_EXIST : ErrorCodes::CANNOT_OPEN_FILE, file_name,
...
std::string OpenedFile::getFileName() const { return file_name; }
