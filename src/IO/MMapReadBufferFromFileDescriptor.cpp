...
MMapReadBufferFromFileDescriptor::MMapReadBufferFromFileDescriptor(int fd, size_t offset) : mapped(fd, offset) { init(); }

void MMapReadBufferFromFileDescriptor::finish() { mapped.finish(); }

std::string MMapReadBufferFromFileDescriptor::getFileName() const { return "(fd = " + toString(mapped.getFD()) + ")"; }

int MMapReadBufferFromFileDescriptor::getFD() const { return mapped.getFD(); }

off_t MMapReadBufferFromFileDescriptor::getPosition() { return count(); }
...
off_t MMapReadBufferFromFileDescriptor::seek(off_t offset, int whence) {
  ...
  if (new_pos < 0 || new_pos > off_t(working_buffer.size()))
    throw Exception(ErrorCodes::CANNOT_SEEK_THROUGH_FILE, "Cannot seek through file {} because seek position ({}) is out of bounds [0, {}]",
...
std::optional<size_t> MMapReadBufferFromFileDescriptor::tryGetFileSize() { return getSizeFromFileDescriptor(getFD(), getFileName()); }

size_t MMapReadBufferFromFileDescriptor::readBigAt(char *to, size_t n, size_t offset, const std::function<bool(size_t)> &) const {
  if (offset >= mapped.getLength()) return 0;
