...
ReadBufferFromEncryptedFile::ReadBufferFromEncryptedFile(const String& file_name_, size_t buffer_size_,
                                                         std::unique_ptr<ReadBufferFromFileBase> in_, const String& key_,
                                                         const FileEncryption::Header& header_, size_t offset_)
...
std::optional<size_t> ReadBufferFromEncryptedFile::tryGetFileSize() {
  ...
  if (!file_size || (*file_size < FileEncryption::Header::kSize)) return {};
...
off_t ReadBufferFromEncryptedFile::seek(off_t off, int whence) {
  ...
  off_t new_pos;
  if (whence == SEEK_SET) {
    if (off < 0) throw Exception(ErrorCodes::ARGUMENT_OUT_OF_BOUND, "SEEK_SET underflow: off = {}", off);
    new_pos = off;
  } else if (whence == SEEK_CUR) {
    if (off < 0 && -off > old_pos) throw Exception(ErrorCodes::ARGUMENT_OUT_OF_BOUND, "SEEK_CUR shift out of bounds");
    new_pos = old_pos + off;
  } else
    throw Exception(ErrorCodes::ARGUMENT_OUT_OF_BOUND, "ReadBufferFromFileEncrypted::seek expects SEEK_SET or SEEK_CUR as whence");

  if (read_until_position && new_pos > *read_until_position) new_pos = *read_until_position;
...
off_t ReadBufferFromEncryptedFile::getPosition() { return offset - available(); }

void ReadBufferFromEncryptedFile::setReadUntilPosition(size_t position) {
  if (read_until_position == position) return;

  if (static_cast<off_t>(position) < getPosition()) {
    throw Exception(ErrorCodes::LOGICAL_ERROR,
                    "Attempt to set read_until_position to {} before already read data (position: {}) while reading {}", position,
...
bool ReadBufferFromEncryptedFile::nextImpl() {
  ...
  if (need_seek || need_set_read_until_position) performSeekAndSetReadUntilPosition();
  ...
  if (read_until_position) {
    if (offset > *read_until_position) {
      throw Exception(ErrorCodes::LOGICAL_ERROR, "Attempt to read after `read_until_position` ({}) (position: {}) while reading {}",
...
void ReadBufferFromEncryptedFile::performSeekAndSetReadUntilPosition() {
  ...
  chassert(in->getPosition() == static_cast<off_t>(offset + FileEncryption::Header::kSize),
           "ReadBufferFromEncryptedFile invariant violation: inner buffer position must match encrypted offset");
}

void ReadBufferFromEncryptedFile::prefetch(Priority priority) { in->prefetch(priority); }
