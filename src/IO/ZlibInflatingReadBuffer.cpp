...
ZlibInflatingReadBuffer::ZlibInflatingReadBuffer(std::unique_ptr<ReadBuffer> in_, CompressionMethod compression_method, size_t buf_size,
  ...
  if (buf_size > max_buffer_size)
    throw Exception(ErrorCodes::ARGUMENT_OUT_OF_BOUND,
                    "Zlib does not support decompression with buffer size greater than {}, got buffer size: {}", max_buffer_size, buf_size);
  ...
  if (rc != Z_OK) throw Exception(ErrorCodes::ZLIB_INFLATE_FAILED, "inflateInit2 failed: {}; zlib version: {}.", zError(rc), ZLIB_VERSION);
}

ZlibInflatingReadBuffer::~ZlibInflatingReadBuffer() { inflateEnd(&zstr); }
...
bool ZlibInflatingReadBuffer::nextImpl() {
  ...
  do {
    ...
    if (!zstr.avail_in) {
      ...
      zstr.avail_in = static_cast<BufferSizeType>(
          std::min(static_cast<UInt64>(in->buffer().end() - in->position()), static_cast<UInt64>(max_buffer_size)));
    }
    ...
    if (rc == Z_STREAM_END) {
      ...
      if (rc != Z_OK)
        throw Exception(ErrorCodes::ZLIB_INFLATE_FAILED, "inflateReset failed: {}{}", zError(rc), getExceptionEntryWithFileName(*in));
      if (working_buffer.empty()) continue;
    ...
    if (rc != Z_OK)
      throw Exception(ErrorCodes::ZLIB_INFLATE_FAILED, "inflate failed: {}{}", zError(rc), getExceptionEntryWithFileName(*in));
