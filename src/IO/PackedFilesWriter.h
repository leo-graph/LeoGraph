...
class PackedFilesWriter {
 public:
  ...
  struct MetadataChange {
    ...
    MetadataChange(Type type_, const String from_, const String& to_) : type(type_), from(from_), to(to_) {}
  ...
  class FakeWriteBufferFromFile : public WriteBufferFromFileBase {
   public:
    ...
    FakeWriteBufferFromFile(const String& file_name_, std::shared_ptr<Data> data_)
        : WriteBufferFromFileBase(0, nullptr, 0), file_name(file_name_), data(data_), impl(std::make_unique<Impl>(data->chars)) {
      swap(*impl);
    }

    ~FakeWriteBufferFromFile() override { swap(*impl); }
   ...
   private:
    struct SwapGuard {
      explicit SwapGuard(FakeWriteBufferFromFile& buf_) : buf(buf_) { buf.swap(*buf.impl); }

      ~SwapGuard() { buf.swap(*buf.impl); }
