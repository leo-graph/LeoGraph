...
class WriteBufferFromFile : public WriteBufferFromFileDescriptor {
 ...
 public:
  explicit WriteBufferFromFile(const std::string& file_name_, size_t buf_size = DBMS_DEFAULT_BUFFER_SIZE, int flags = -1,
                               ThrottlerPtr throttler_ = {}, mode_t mode = 0666, char* existing_memory = nullptr, size_t alignment = 0,
  ...
  explicit WriteBufferFromFile(
      ...
      const std::string& original_file_name = {}, size_t buf_size = DBMS_DEFAULT_BUFFER_SIZE, ThrottlerPtr throttler_ = {},
      char* existing_memory = nullptr, size_t alignment = 0, bool use_adaptive_buffer_size_ = false,
      size_t adaptive_buffer_initial_size = DBMS_DEFAULT_INITIAL_ADAPTIVE_BUFFER_SIZE);
