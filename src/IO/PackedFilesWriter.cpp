...
std::unique_ptr<WriteBufferFromFileBase> PackedFilesWriter::writeFile(const String &file_name, const WriteSettings &settings) {
  if (!write_settings && settings != WriteSettings{}) write_settings = settings;

  return writeFile(file_name);
}

std::unique_ptr<WriteBufferFromFileBase> PackedFilesWriter::writeFile(const String &file_name) {
  auto [it, inserted] = written_files.try_emplace(file_name);

  if (!inserted) throw Exception(ErrorCodes::FILE_ALREADY_EXISTS, "File {} already exists", file_name);
...
static UInt64 getLengthOfSerializedString(const String &str) { return getLengthOfVarUInt(str.size()) + str.size(); }
...
void PackedFilesWriter::applyMoveFile(MetadataChange &change, Map &index_map) {
  if (change.type == MetadataChange::MOVE && index_map.contains(change.to))
    throw Exception(ErrorCodes::FILE_ALREADY_EXISTS, "Cannot move file from {} to {}. File {} already exists", change.from, change.to,
                    change.to);
  ...
  for (auto &change : metadata_changes) {
    if (!change.is_applied) {
      if (change.type == MetadataChange::MOVE || change.type == MetadataChange::REPLACE)
        throw Exception(ErrorCodes::FILE_DOESNT_EXIST, "Cannot move file from {} to {}. File {} does not exist", change.from, change.to,
                        change.from);

      if (change.type == MetadataChange::REMOVE)
        throw Exception(ErrorCodes::FILE_DOESNT_EXIST, "Cannot remove file {}. File does not exist", change.from);
    }
  }
