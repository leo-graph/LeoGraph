...
class WriteBufferFromPocoSocket : public BufferWithOwnMemory<WriteBuffer> {
 public:
  ...
  explicit WriteBufferFromPocoSocket(Poco::Net::Socket &socket_, const ProfileEvents::Event &write_event_,
 ...
 protected:
  ...
  void socketSendStr(const std::string &str) { socketSendBytes(str.data(), str.size()); }
  void socketSendStr(const char *ptr) { socketSendBytes(ptr, strlen(ptr)); }
