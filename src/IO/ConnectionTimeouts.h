...
#define DEFINE_BUILDER_FOR_MEMBER(member, setter_func)                                                                           \
  inline ConnectionTimeouts& ConnectionTimeouts::setter_func(size_t seconds) { return setter_func(Poco::Timespan(seconds, 0)); } \
...
#define SATURATE_MEMBER(member, setter_func) .setter_func(saturate(member, limit))

  return ConnectionTimeouts(*this) APPLY_FOR_ALL_CONNECTION_TIMEOUT_MEMBERS(SATURATE_MEMBER);
