...
ConcatReadBufferFromFile::ConcatReadBufferFromFile(std::string file_name_) : ReadBufferFromFileBase(), file_name(std::move(file_name_)) {}

ConcatReadBufferFromFile::ConcatReadBufferFromFile(std::string file_name_, std::unique_ptr<SeekableReadBuffer> buf1, size_t size1,
                                                   std::unique_ptr<SeekableReadBuffer> buf2, size_t size2)
    : ReadBufferFromFileBase(), file_name(std::move(file_name_)) {
  appendBuffer(std::move(buf1), size1);
  appendBuffer(std::move(buf2), size2);
}

ConcatReadBufferFromFile::ConcatReadBufferFromFile(std::string file_name_, SeekableReadBuffer& buf1, size_t size1, SeekableReadBuffer& buf2,
                                                   size_t size2)
    : ReadBufferFromFileBase(), file_name(std::move(file_name_)) {
...
void ConcatReadBufferFromFile::appendBuffer(SeekableReadBuffer& buffer, size_t size) { appendBuffer(&buffer, false, size); }
...
bool ConcatReadBufferFromFile::nextImpl() {
  if (current < buffers.size()) {
    ...
    while ((current < buffers.size()) && buffers[current].in->eof()) {
      ...
      if (current < buffers.size()) buffers[current].in->seek(0, SEEK_SET);
    }
  }
...
off_t ConcatReadBufferFromFile::getPosition() {
  ...
  if (current < buffers.size()) current_pos += buffers[current].in->getPosition() + offset();
  return current_pos;
}
...
off_t ConcatReadBufferFromFile::seek(off_t off, int whence) {
  ...
  if (new_position < 0) throw Exception(ErrorCodes::ARGUMENT_OUT_OF_BOUND, "SEEK_SET underflow: off = {}", off);
  if (static_cast<UInt64>(new_position) > total_size) throw Exception(ErrorCodes::ARGUMENT_OUT_OF_BOUND, "SEEK_CUR shift out of bounds");
  ...
  while (new_position < static_cast<off_t>(current_start_pos)) current_start_pos -= buffers[--current].size;

  while (new_position >= static_cast<off_t>(current_start_pos + buffers[current].size)) current_start_pos += buffers[current++].size;
