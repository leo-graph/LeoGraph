...
void copyDataImpl(ReadBuffer& from, WriteBuffer& to, bool check_bytes, size_t bytes, const std::atomic<int>* is_cancelled,
  ...
  while (bytes > 0 && !from.eof()) {
    ...
    if (throttler) throttler->throttle(count);
  }
...
void copyDataImpl(ReadBuffer& from, WriteBuffer& to, bool check_bytes, size_t bytes, std::function<void()> cancellation_hook,
  ...
  if (check_bytes && bytes > 0) throw Exception(ErrorCodes::ATTEMPT_TO_READ_AFTER_EOF, "Attempt to read after EOF.");
}

}  // namespace

void copyData(ReadBuffer& from, WriteBuffer& to) { copyDataImpl(from, to, false, std::numeric_limits<size_t>::max(), nullptr, nullptr); }
...
void copyData(ReadBuffer& from, WriteBuffer& to, size_t bytes) { copyDataImpl(from, to, true, bytes, nullptr, nullptr); }
...
void copyDataMaxBytes(ReadBuffer& from, WriteBuffer& to, size_t max_bytes) {
  ...
  if (!from.eof()) throw Exception(ErrorCodes::CANNOT_READ_ALL_DATA, "Cannot read all data, max readable size reached.");
}
