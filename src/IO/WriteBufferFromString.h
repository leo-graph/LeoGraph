...
class WriteBufferFromString final : public WriteBufferFromVectorImpl<std::string> {
 ...
 public:
  explicit WriteBufferFromString(std::string& vector_) : Base(vector_) {}

  WriteBufferFromString(std::string& vector_, AppendModeTag tag_) : Base(vector_, tag_) {}
...
class WriteBufferFromOwnString : public detail::StringHolder, public WriteBufferFromVectorImpl<std::string> {
 ...
 public:
  WriteBufferFromOwnString() : Base(value) {}
  ~WriteBufferFromOwnString() override;

  std::string_view stringView() const {
    return isFinalized() ? std::string_view(value) : std::string_view(value.data(), pos - value.data());
  }
