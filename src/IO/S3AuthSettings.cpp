...
bool setValueFromConfig(const Poco::Util::AbstractConfiguration& config, const std::string& path,
                        typename S3AuthSettingsImpl::SettingFieldRef& field) {
...
S3AuthSettings::S3AuthSettings(const Poco::Util::AbstractConfiguration& config, const DB::Settings& settings,
  ...
  for (const auto& key : keys) {
    if (startsWith(key, "user")) users.insert(config.getString(config_prefix + "." + key));
  }
}
...
bool S3AuthSettings::operator==(const S3AuthSettings& right) {
  if (headers != right.headers) return false;

  if (access_headers != right.access_headers) return false;

  if (users != right.users) return false;

  if (server_side_encryption_kms_config != right.server_side_encryption_kms_config) return false;
...
void S3AuthSettings::updateIfChanged(const S3AuthSettings& settings) {
  ...
  if (!settings.headers.empty()) headers = settings.headers;

  if (!settings.access_headers.empty()) access_headers = settings.access_headers;

  if (!settings.users.empty()) users.insert(settings.users.begin(), settings.users.end());
...
HTTPHeaderEntries S3AuthSettings::getHeaders() const {
  ...
  if (access_headers.empty() || !auth_settings_is_default) return headers;
