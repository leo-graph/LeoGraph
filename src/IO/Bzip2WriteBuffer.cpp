...
Bzip2WriteBuffer::~Bzip2WriteBuffer() { BZ2_bzCompressEnd(&stream); }
...
void Bzip2WriteBuffer::nextImpl() {
  ...
  try {
    do {
      ...
      if (ret != BZ_RUN_OK) throw Exception(ErrorCodes::BZIP2_STREAM_ENCODER_FAILED, "bzip2 stream encoder failed: error code: {}", ret);
...
void Bzip2WriteBuffer::finalFlushBefore() {
  ...
  do {
    ...
    if (ret != BZ_FINISH_OK) throw Exception(ErrorCodes::BZIP2_STREAM_ENCODER_FAILED, "bzip2 stream encoder failed: error code: {}", ret);
  } while (true);
}
