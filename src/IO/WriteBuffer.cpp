...
void WriteBuffer::write(const char* from, size_t n) {
  if (finalized) throw Exception{ErrorCodes::LOGICAL_ERROR, "Cannot write to finalized buffer"};

  if (canceled) throw Exception{ErrorCodes::CANNOT_WRITE_AFTER_BUFFER_CANCELED, "Cannot write to canceled buffer"};
...
void WriteBuffer::write(char x) {
  ...
  if (canceled) throw Exception{ErrorCodes::LOGICAL_ERROR, "Cannot write to canceled buffer"};
...
void WriteBuffer::finalize() {
  ...
  if (canceled) throw Exception(ErrorCodes::LOGICAL_ERROR, "Cannot finalize buffer after cancellation.");
...
void WriteBuffer::nextImpl() { throw Exception(ErrorCodes::CANNOT_WRITE_AFTER_END_OF_BUFFER, "Cannot write after end of buffer."); }
}  // namespace DB
