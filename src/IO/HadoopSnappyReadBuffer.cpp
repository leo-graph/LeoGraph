...
inline bool HadoopSnappyDecoder::checkBufferLength(int max) const { return buffer_length >= 0 && buffer_length < max; }

inline bool HadoopSnappyDecoder::checkAvailIn(size_t avail_in, int min) { return avail_in >= static_cast<size_t>(min); }
...
inline HadoopSnappyDecoder::Status HadoopSnappyDecoder::readLength(size_t *avail_in, const char **next_in, int *length) {
  ...
  if (!checkBufferLength(4)) return Status::INVALID_INPUT;
...
inline HadoopSnappyDecoder::Status HadoopSnappyDecoder::readCompressedData(size_t *avail_in, const char **next_in, size_t *avail_out,
                                                                           char **next_out) {
  if (!checkBufferLength(compressed_length)) return Status::INVALID_INPUT;
...
HadoopSnappyDecoder::Status HadoopSnappyDecoder::readBlock(size_t *avail_in, const char **next_in, size_t *avail_out, char **next_out) {
  if (*avail_in == 0) {
    if (buffer_length == 0 && block_length < 0 && compressed_length < 0) return Status::OK;
    return Status::NEEDS_MORE_INPUT;
  }
