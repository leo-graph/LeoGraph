...
CachedInMemoryReadBufferFromFile::CachedInMemoryReadBufferFromFile(PageCacheKey cache_key_, PageCachePtr cache_,
...
String CachedInMemoryReadBufferFromFile::getFileName() const { return cache_key.path; }

String CachedInMemoryReadBufferFromFile::getInfoForLog() { return "CachedInMemoryReadBufferFromFile(" + in->getInfoForLog() + ")"; }
...
off_t CachedInMemoryReadBufferFromFile::getPosition() { return file_offset_of_buffer_end - available(); }

size_t CachedInMemoryReadBufferFromFile::getFileOffsetOfBufferEnd() const { return file_offset_of_buffer_end; }
...
void CachedInMemoryReadBufferFromFile::setReadUntilEnd() { setReadUntilPosition(file_size.value()); }

bool CachedInMemoryReadBufferFromFile::nextImpl() {
  chassert(read_until_position <= file_size.value());
  if (file_offset_of_buffer_end >= read_until_position) return false;

  size_t block_size = settings.page_cache_block_size;

  if (chunk != nullptr) {
    chassert(chunk->key.hash() == cache_key.hash());
    if (file_offset_of_buffer_end < cache_key.offset || file_offset_of_buffer_end >= cache_key.offset + block_size) chunk.reset();
  }
  ...
  if (chunk == nullptr) {
    ...
    chunk = cache->getOrSet(
        cache_key, settings.read_from_page_cache_if_exists_otherwise_bypass_cache, settings.page_cache_inject_eviction, [&](auto cell) {
          ...
          while (pos < cache_key.size) {
            ...
            if (pos == 0) {
              ...
              if (inner_read_until_position < cache_key.offset + cache_key.size || inner_read_until_position > lookahead_block_end) {
                ...
                } while (temp_key.offset < lookahead_block_end && !cache->contains(temp_key, settings.page_cache_inject_eviction));
            ...
            if (in->eof())
              throw Exception(ErrorCodes::UNEXPECTED_END_OF_FILE, "File {} ended after {} bytes, but we expected {}", getFileName(),
                              cache_key.offset + pos, file_size.value());
            ...
            chassert(n);
            if (in->position() != piece_start) memmove(piece_start, in->position(), n);
            in->position() += n;
            pos += n;
          }
  ...
  working_buffer = Buffer(chunk->data(), chunk->data() + std::min(chunk->size(), read_until_position - cache_key.offset));
