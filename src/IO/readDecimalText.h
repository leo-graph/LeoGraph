...
inline bool readDigits(ReadBuffer& buf, T& x, uint32_t& digits, int32_t& exponent, bool digits_only = false) {
  ...
  if (buf.eof()) {
    if constexpr (_throw_on_error) throwReadAfterEOF();
    return false;
  }
  ...
  while (!buf.eof() && !stop) {
    ...
    switch (byte) {
      ...
      case '9': {
        ...
        digits += places;
        if (after_point) exponent -= places;
      ...
      default:
        if (digits_only) {
          if constexpr (_throw_on_error) throw Exception(ErrorCodes::CANNOT_PARSE_NUMBER, "Unexpected symbol while reading decimal");
          return false;
        }
        stop = true;
        continue;
...
inline ReturnType readDecimalText(ReadBuffer& buf, T& x, uint32_t precision, uint32_t& scale, bool digits_only = false) {
  ...
  if (!throw_exception && !ok) return ReturnType(false);
...
inline void readCSVDecimalText(ReadBuffer& buf, T& x, uint32_t precision, uint32_t& scale) {
  ...
  if (maybe_quote == '\'' || maybe_quote == '\"') assertChar(maybe_quote, buf);
}
...
inline bool tryReadCSVDecimalText(ReadBuffer& buf, T& x, uint32_t precision, uint32_t& scale) {
  ...
  if (!tryReadDecimalText(buf, x, precision, scale)) return false;

  if ((maybe_quote == '\'' || maybe_quote == '\"') && !checkChar(maybe_quote, buf)) return false;
