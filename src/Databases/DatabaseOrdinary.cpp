...
DatabaseOrdinary::DatabaseOrdinary(const String &name_, const String &metadata_path_, ContextPtr context_,
    ...
    : DatabaseOrdinary(name_, metadata_path_, DatabaseCatalog::getDataDirPath(name_) / "", "DatabaseOrdinary (" + name_ + ")", context_,
                       database_metadata_disk_settings_) {}

DatabaseOrdinary::DatabaseOrdinary(const String &name_, const String &metadata_path_, const String &data_path_, const String &logger,
                                   ContextPtr context_, DatabaseMetadataDiskSettings database_metadata_disk_settings_)
...
static void checkReplicaPathExists(ASTCreateQuery &create_query, ContextPtr local_context) {
  ...
  if (local_context->getZooKeeper()->exists(zookeeper_path))
    throw Exception(ErrorCodes::UNEXPECTED_NODE_IN_ZOOKEEPER,
...
void DatabaseOrdinary::convertMergeTreeToReplicatedIfNeeded(ASTPtr ast, const QualifiedTableName &qualified_name, const String &file_name) {
  ...
  if (!create_query.storage || !create_query.storage->engine->name.ends_with("MergeTree") ||
      create_query.storage->engine->name.starts_with("Replicated") || create_query.storage->engine->name.starts_with("Shared"))
    return;
  ...
  if (!checking_disk->existsFile(convert_to_replicated_flag_path)) return;
  ...
  LOG_INFO(log,
           "Engine of table {} is set to replicated in metadata. Not removing {} flag until table is loaded and metadata in zookeeper is "
           "restored.",
           backQuote(qualified_name.getFullName()), CONVERT_TO_REPLICATED_FLAG_NAME);
}
...
void DatabaseOrdinary::loadTablesMetadata(ContextPtr local_context, ParsedTablesMetadata &metadata, bool is_startup) {
  ...
  auto process_metadata = [&metadata, is_startup, local_context, db_disk, this](const String &file_name) {
    ...
    try {
      auto ast = parseQueryFromMetadata(log, local_context, db_disk, full_path.string(), /*throw_on_error*/ true, /*remove_empty*/ false);
      if (ast) {
        ...
        if (db_disk->existsFile(full_path.string() + detached_suffix)) {
          ...
          snapshot_detached_tables.emplace(detached_table_name,
                                           SnapshotDetachedTable{.database = create_query->getDatabase(),
...
void DatabaseOrdinary::loadTableFromMetadata(ContextMutablePtr local_context, const String &file_path, const QualifiedTableName &name,
                                             const ASTPtr &ast, LoadingStrictnessLevel mode) {
  ...
  while (true) {
    try {
      auto [table_name, table] = createTableFromAST(query, name.database, getTableDataPath(query), local_context, mode);

      attachTable(local_context, table_name, table, getTableDataPath(query));
      return;
    } catch (Coordination::Exception &e) {
      e.addMessage("Cannot attach table " + backQuote(name.database) + "." + backQuote(query.getTable()) + " from metadata file " +
                   file_path + " from query " + query.formatForErrorMessage());
...
bool DatabaseOrdinary::shouldLazyLoad(const ASTCreateQuery &query, LoadingStrictnessLevel mode) const {
  if (!database_metadata_disk_settings[DatabaseMetadataDiskSetting::lazy_load_tables]) return false;

  if (query.is_ordinary_view || query.is_materialized_view || query.is_dictionary || query.isParameterizedView() || query.is_window_view)
    return false;

  /// Already handled by `StorageTableFunctionProxy`.
  if (query.as_table_function) return false;

  if (mode == LoadingStrictnessLevel::FORCE_RESTORE) return false;

  return true;
}

void DatabaseOrdinary::loadTableLazy(ContextMutablePtr local_context, const QualifiedTableName &name, const ASTPtr &ast,
  ...
  auto get_nested = [query_str = ast->formatWithSecretsMultiLine(), db_name = name.database, table_data_path,
                     global_context = local_context->getGlobalContext(), mode]() -> StoragePtr {
    auto load_context = Context::createCopy(global_context);
    ParserCreateQuery parser;
    ASTPtr parsed_ast = parseQuery(parser, query_str.data(), query_str.data() + query_str.size(), "lazy load", 0,
    ...
    auto [_, table] = createTableFromAST(create_query, db_name, table_data_path, load_context, mode);
    return table;
  };

  auto proxy = std::make_shared<StorageTableProxy>(table_id, std::move(get_nested), std::move(columns));

  attachTable(local_context, query.getTable(), proxy, table_data_path);
}

LoadTaskPtr DatabaseOrdinary::loadTableFromMetadataAsync(AsyncLoader &async_loader, LoadJobSet load_after, ContextMutablePtr local_context,
                                                         const String &file_path, const QualifiedTableName &name, const ASTPtr &ast,
                                                         LoadingStrictnessLevel mode) {
  TransactionLog::increaseAsyncTablesLoadingJobNumber();
  std::scoped_lock lock(mutex);
  auto job = makeLoadJob(std::move(load_after), TablesLoaderBackgroundLoadPoolId, fmt::format("load table {}", name.getFullName()),
...
void DatabaseOrdinary::restoreMetadataAfterConvertingToReplicated(StoragePtr table, const QualifiedTableName &name) {
  ...
  LOG_INFO(log, "Removing convert to replicated flag for {}.", backQuote(name.getFullName()));

  auto has_metadata = rmt->hasMetadataInZooKeeper();
  if (!has_metadata.has_value()) {
    LOG_WARNING(log,
                "No connection to ZooKeeper, can't restore metadata for {} in ZooKeeper after conversion. Run SYSTEM RESTORE REPLICA while "
                "connected to ZooKeeper.",
                backQuote(name.getFullName()));
  } else if (*has_metadata) {
    LOG_INFO(log, "Table {} already has metatada in ZooKeeper.", backQuote(name.getFullName()));
  } else {
    rmt->restoreMetadataInZooKeeper(/* zookeeper_retries_info = */ {}, false);
    LOG_INFO(log, "Metadata in ZooKeeper for {} is restored.", backQuote(name.getFullName()));
  }
}

LoadTaskPtr DatabaseOrdinary::startupTableAsync(AsyncLoader &async_loader, LoadJobSet startup_after, const QualifiedTableName &name,
                                                LoadingStrictnessLevel /*mode*/) {
  std::scoped_lock lock(mutex);
  ...
  auto job = makeLoadJob(std::move(startup_after), TablesLoaderBackgroundStartupPoolId, fmt::format("startup table {}", name.getFullName()),
                         [this, name](AsyncLoader &, const LoadJobPtr &) {
                           if (auto table = tryGetTableNoWait(name.table)) {
                             /// Since startup() method can use physical paths on disk we don't allow any exclusive actions (rename, drop so
                             /// on) until startup finished.
                             auto table_lock_holder =
                                 table->lockForShare(RWLockImpl::NO_QUERY, getContext()->getSettingsRef()[Setting::lock_acquire_timeout]);
                             table->startup();
...
LoadTaskPtr DatabaseOrdinary::startupDatabaseAsync(AsyncLoader &async_loader, LoadJobSet startup_after, LoadingStrictnessLevel /*mode*/) {
  auto job = makeLoadJob(std::move(startup_after), TablesLoaderBackgroundStartupPoolId,
                         fmt::format("startup Ordinary database {}", getDatabaseName()), ignoreDependencyFailure,
...
void DatabaseOrdinary::waitTableStarted(const String &name) const {
  ...
  {
    std::scoped_lock lock(mutex);
    if (auto it = startup_table.find(name); it != startup_table.end()) task = it->second;
  }
...
DatabaseTablesIteratorPtr DatabaseOrdinary::getTablesIterator(ContextPtr local_context,
                                                              const DatabaseOnDisk::FilterByNameFunction &filter_by_table_name,
  ...
  if (!skip_not_loaded) {
    ...
    {
      std::lock_guard lock(mutex);
      if (!filter_by_table_name) tasks_to_wait.reserve(startup_table.size());
      for (const auto &[table_name, task] : startup_table)
        if (!filter_by_table_name || filter_by_table_name(table_name)) tasks_to_wait.emplace_back(task);
    }
...
Strings DatabaseOrdinary::getAllTableNames(ContextPtr) const {
  ...
  {
    std::lock_guard lock(mutex);
    for (const auto &[table_name, _] : tables) unique_names.emplace(table_name);
    // Not yet loaded table are not listed in `tables`, so we have to add table names from tasks
    for (const auto &[table_name, _] : startup_table) unique_names.emplace(table_name);
  }
...
void DatabaseOrdinary::alterTable(ContextPtr local_context, const StorageID &table_id, const StorageInMemoryMetadata &metadata,
  ...
  if (table_id.uuid != UUIDHelpers::Nil && create_query.uuid != table_id.uuid)
    throw Exception(ErrorCodes::UNKNOWN_TABLE, "Cannot alter table {}: metadata file {} has different UUID", table_id.getNameForLogs(),
                    table_metadata_path);
  ...
  auto ref_dependencies = getDependenciesFromCreateQuery(local_context->getGlobalContext(), table_id.getQualifiedName(), ast,
                                                         local_context->getCurrentDatabase());
  ...
  DatabaseCatalog::instance().checkTableCanBeAddedWithNoCyclicDependencies(table_id.getQualifiedName(), ref_dependencies.dependencies,
  ...
  DatabaseCatalog::instance().updateDependencies(
      table_id, ref_dependencies.dependencies, loading_dependencies,
