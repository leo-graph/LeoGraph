...
ZooKeeperPtr DatabaseReplicated::getZooKeeper() const { return getContext()->getDefaultOrAuxiliaryZooKeeper(zookeeper_name); }
...
static inline std::tuple<String, UInt16, UUID> parseHostID(const String &content) {
  ...
  if (pos == std::string::npos || pos + 1 >= content.size()) throw Exception(ErrorCodes::SYNTAX_ERROR, "Invalid host ID '{}'", content);

  auto [address, port] = Cluster::Address::fromString(content.substr(0, pos));
  UUID db_uuid;
  if (!tryParse(db_uuid, content.substr(pos + 1))) throw Exception(ErrorCodes::SYNTAX_ERROR, "Invalid host ID '{}'", content);
...
String normalizeZooKeeperPath(const String &path) noexcept {
  ...
  if (!normalized_path.starts_with("/")) normalized_path = '/' + normalized_path;

  if (normalized_path.ends_with("/")) normalized_path.pop_back();

  return normalized_path;
}
}  // namespace

DatabaseReplicated::DatabaseReplicated(const String &name_, const String &metadata_path_, UUID uuid, const String &zookeeper_name_,
                                       const String &zookeeper_path_, const String &shard_name_, const String &replica_name_,
                                       DatabaseReplicatedSettings db_settings_, ContextPtr context_)
  ...
  if (!replica_group_name.empty() && database_name.starts_with(DatabaseReplicated::ALL_GROUPS_CLUSTER_PREFIX)) {
    auto constexpr message_format_string =
        "There's a Replicated database with a name starting from '{}', "
        "and replica_group_name is configured. It may cause collisions in cluster names.";
    context_->addOrUpdateWarningMessage(Context::WarningType::REPLICATED_DB_WITH_ALL_GROUPS_CLUSTER_PREFIX,
                                        PreformattedMessage::create(message_format_string, ALL_GROUPS_CLUSTER_PREFIX));
  }
}

String DatabaseReplicated::getFullReplicaName(const String &shard, const String &replica) { return shard + '|' + replica; }
...
void DatabaseReplicated::getStatus(ReplicatedStatus &response, const bool with_zk_fields) const {
  ...
  if (!with_zk_fields || response.is_session_expired) return;
  ...
  try {
    ...
    if (get_result[1].error == Coordination::Error::ZNONODE) throw zkutil::KeeperException(get_result[1].error);
...
String DatabaseReplicated::getFullReplicaName() const { return getFullReplicaName(shard_name, replica_name); }
...
ClusterPtr DatabaseReplicated::tryGetAllGroupsCluster() const {
  std::lock_guard lock{mutex};
  if (replica_group_name.empty()) return nullptr;

  if (cluster_all_groups) return cluster_all_groups;

  /// Database is probably not created or not initialized yet, it's ok to return nullptr
  if (is_readonly) return cluster_all_groups;
...
ClusterPtr DatabaseReplicated::getClusterImpl(bool all_groups) const {
  ...
  while (++iteration <= max_retries) {
    ...
    if (unfiltered_hosts.empty())
      throw Exception(ErrorCodes::NO_ACTIVE_REPLICAS,
                      "No replicas of database {} found. "
                      "It's possible if the first replica is not fully created yet "
                      "or if the last replica was just dropped or due to logical error",
                      zookeeper_path);
    ...
    if (all_groups) {
      ...
      for (const auto &host : unfiltered_hosts) paths.push_back(zookeeper_path + "/replicas/" + host + "/replica_group");

      auto replica_groups = zookeeper->tryGet(paths);

      for (size_t i = 0; i < paths.size(); ++i) {
        if (replica_groups[i].data == replica_group_name) hosts.push_back(unfiltered_hosts[i]);
      }
    }
    ...
    for (const auto &host : hosts) host_paths.emplace_back(zookeeper_path + "/replicas/" + host);
  ...
  if (shards.empty()) throw Exception(ErrorCodes::ALL_CONNECTION_TRIES_FAILED, "No active replicas");
  ...
  if (all_groups) cluster_name = ALL_GROUPS_CLUSTER_PREFIX + cluster_name;

  ClusterConnectionParameters params{cluster_auth_info.cluster_username,
...
ReplicasInfo DatabaseReplicated::tryGetReplicasInfo(const ClusterPtr &cluster_) const {
  ...
  try {
    ...
    if (max_log_ptr_zk.error != Coordination::Error::ZOK) throw Coordination::Exception(max_log_ptr_zk.error);
    ...
    for (size_t shard_index = 0; shard_index < shards_info.size(); ++shard_index) {
      for (const auto &replica : addresses_with_failover[shard_index]) {
        ...
        UInt64 recovery_time = 0;
        {
          ...
          if (replica.is_local && ddl_worker) recovery_time = ddl_worker->getCurrentInitializationDurationMs();
        }
        ...
        replicas_info[global_replica_index] = ReplicaInfo{
            ...
            .replication_lag = replica_log_ptr.error != Coordination::Error::ZNONODE
                                   ? std::optional(max_log_ptr - parse<UInt32>(replica_log_ptr.data))
                                   : std::nullopt,
...
void DatabaseReplicated::tryConnectToZooKeeperAndInitDatabase(LoadingStrictnessLevel mode) {
  try {
    ...
    if (mode < LoadingStrictnessLevel::FORCE_ATTACH) throw;
...
void DatabaseReplicated::initDatabaseReplica(const ZooKeeperPtr &current_zookeeper, LoadingStrictnessLevel mode) {
  ...
  if (replica_exists_in_zk) {
    if (replica_host_id == DROPPED_MARK && !is_create_query) {
      LOG_WARNING(log,
                  "Database {} exists locally, but marked dropped in ZooKeeper ({}). "
                  "Will not try to start it up",
                  getDatabaseName(), replica_path);
    ...
    if (!current_zookeeper->tryGet(replica_path + "/replica_group", zk_replica_group_name)) {
      ...
      if (!replica_group_name.empty()) createEmptyLogEntry(current_zookeeper);
  ...
  if (is_create_query) {
    ...
    LOG_WARNING(log,
                "Database {} exists locally, but its replica does not exist in ZooKeeper ({}). "
                "Assuming it was dropped, will not try to start it up",
                getDatabaseName(), replica_path);
    is_probably_dropped = true;
    return;
  }
...
bool DatabaseReplicated::createDatabaseNodesInZooKeeper(const zkutil::ZooKeeperPtr &current_zookeeper) {
  ...
  if (res == Coordination::Error::ZOK) return true;           /// Created new database (it's the first replica)
  if (res == Coordination::Error::ZNODEEXISTS) return false;  /// Database exists, we will add new replica
...
bool DatabaseReplicated::looksLikeReplicatedDatabasePath(const ZooKeeperPtr &current_zookeeper, const String &path) {
  ...
  if (!current_zookeeper->tryGet(path, maybe_database_mark, &stat)) return false;
  if (maybe_database_mark.starts_with(REPLICATED_DATABASE_MARK)) return true;
  if (!maybe_database_mark.empty()) return false;
  ...
  Coordination::Responses responses;
  auto res = current_zookeeper->tryMulti(ops, responses);
  if (res == Coordination::Error::ZOK) return true;
...
bool DatabaseReplicated::waitForReplicaToProcessAllEntries(UInt64 timeout_ms, SyncReplicaMode mode) {
  ...
  {
    std::lock_guard lock{ddl_worker_mutex};
    if (!ddl_worker || is_probably_dropped) return false;
  }

  if (mode == SyncReplicaMode::DEFAULT) return ddl_worker->waitForReplicaToProcessAllEntries(timeout_ms);
  ...
  while (true) {
    ...
    if (ddl_worker->waitForReplicaToProcessAllEntries(timeout_ms - elapsed_ms)) return true;
...
void DatabaseReplicated::createReplicaNodesInZooKeeper(const zkutil::ZooKeeperPtr &current_zookeeper) {
  if (!looksLikeReplicatedDatabasePath(current_zookeeper, zookeeper_path))
    throw Exception(ErrorCodes::BAD_ARGUMENTS,
                    "Cannot add new database replica: provided path {} "
                    "already contains some data and it does not look like Replicated database path.",
                    zookeeper_path);
  ...
  if (nodes_exist) {
    ...
    for (size_t i = 0; i != expected_data.size(); ++i) {
      if (check_responses[i].data != expected_data[i]) {
        throw Exception(ErrorCodes::REPLICA_ALREADY_EXISTS, "Replica node {} in ZooKeeper already exists and contains unexpected value: {}",
...
LoadTaskPtr DatabaseReplicated::startupDatabaseAsync(AsyncLoader &async_loader, LoadJobSet startup_after, LoadingStrictnessLevel mode) {
  ...
  auto job = makeLoadJob(base->goals(), TablesLoaderBackgroundStartupPoolId,
                         fmt::format("startup Replicated database {}", getDatabaseName()), [this](AsyncLoader &, const LoadJobPtr &) {
                           auto component_guard = Coordination::setCurrentComponent("DatabaseReplicated::startupDatabaseAsync");
                           UInt64 digest = 0;
                           {
                             std::lock_guard lock{mutex};
                             for (const auto &table : tables) digest += getMetadataHash(table.first);
...
void DatabaseReplicated::waitDatabaseStarted() const {
  ...
  if (task) waitLoad(currentPoolOr(TablesLoaderForegroundPoolId), task);
}
...
void DatabaseReplicated::restoreDatabaseNodesInKeeper(const ZooKeeperPtr &zookeeper) {
  ...
  if (code == Coordination::Error::ZOK) return;
...
ASTPtr DatabaseReplicated::tryGetCreateOrAttachTableQuery(const String &name, ContextPtr local_context) const {
  ...
  create.attach = false;
  if (create.is_materialized_view_with_inner_table()) create.attach = true;
  if (create.storage && create.storage->engine && (create.storage->engine->name == "TimeSeries")) create.attach = true;
  return res;
}
...
void DatabaseReplicated::tryCompareLocalAndZooKeeperTablesAndDumpDiffForDebugOnly(const ContextPtr &local_context) const {
  ...
  if (table_name_to_metadata_in_zk.size() != table_names_local.size()) {
    LOG_ERROR(log, "Amount of tables in coordinator {} differs from number of tables locally {}", table_name_to_metadata_in_zk.size(),
              table_names_local.size());
  }
  ...
  for (const auto &table_name : table_names_local) {
    ...
    if (zk_metadata_it != table_name_to_metadata_in_zk.end())
      zk_ast_ptr = parseQueryFromMetadataInZooKeeper(getContext(),
  ...
  for (const auto &[table_name, table_metadata] : table_name_to_metadata_in_zk) {
    if (!checked_tables.contains(table_name)) {
      auto zk_ast_ptr = parseQueryFromMetadataInZooKeeper(getContext(),
                                                          /*database_name_=*/getDatabaseName(),
                                                          /*zookeeper_path_=*/zookeeper_path,
                                                          /*node_name=*/table_name, table_metadata);
...
void DatabaseReplicated::checkTableEngine(const ASTCreateQuery &query, ASTStorage &storage, ContextPtr query_context) const {
  bool replicated_table = storage.engine && (startsWith(storage.engine->name, "Replicated") || startsWith(storage.engine->name, "Shared"));
  if (!replicated_table || !storage.engine->arguments) return;
  ...
  if (!arg1 || !arg2 || arg1->value.getType() != Field::Types::String || arg2->value.getType() != Field::Types::String) return;
  ...
  bool enable_functional_tests_helper =
      getContext()->getConfigRef().has("_functional_tests_helper_database_replicated_replace_args_macros");

  if (maybe_shard_macros && maybe_replica_macros) return;

  if (enable_functional_tests_helper && !has_expression_argument) {
    if (maybe_path.empty() || maybe_path.back() != '/') maybe_path += '/';
...
void DatabaseReplicated::assertDigestWithProbability(const ContextPtr &local_context) const {
  ...
  if (thread_local_rng() % 16) return;

  if (!checkDigestValid(local_context)) throw Exception(ErrorCodes::LOGICAL_ERROR, "Digest does not match");
#else
...
bool DatabaseReplicated::checkDigestValid(const ContextPtr &local_context) const {
  ...
  if (!local_context->getZooKeeperMetadataTransaction() && (!ddl_worker || !ddl_worker->isCurrentlyActive())) return true;
  ...
  if (tables_being_restarted.load() > 0) return true;

  UInt64 local_digest = 0;
  {
    std::lock_guard lock{mutex};
    for (const auto &table : tables) local_digest += getMetadataHash(table.first);
  }
  ...
  if (local_context->isInternalSubquery()) return true;
...
void DatabaseReplicated::checkQueryValid(const ASTPtr &query, ContextPtr query_context) const {
  ...
  if (auto *ddl_query = dynamic_cast<ASTQueryWithTableAndOutput *>(query.get())) {
    if (ddl_query->getDatabase() != getDatabaseName()) throw Exception(ErrorCodes::UNKNOWN_DATABASE, "Database was renamed");
    ddl_query->database.reset();

    if (auto *create = query->as<ASTCreateQuery>()) {
      if (create->storage) checkTableEngine(*create, *create->storage, query_context);
...
BlockIO DatabaseReplicated::tryEnqueueReplicatedDDL(const ASTPtr &query, ContextPtr query_context, QueryFlags flags,
  ...
  if (is_readonly) throw Exception(ErrorCodes::NO_ZOOKEEPER, "Database is in readonly mode, because it cannot connect to ZooKeeper");
  ...
  for (size_t i = 0; i < paths.size(); ++i) {
    if (replica_groups[i].data == replica_group_name) hosts_to_wait.push_back(unfiltered_hosts[i]);
  }

  return getQueryStatus(zookeeper_name, node_path, fs::path(zookeeper_path) / "replicas", query_context, hosts_to_wait,
...
static UUID getTableUUIDIfReplicated(const String &metadata, ContextPtr context) {
  ...
  if (!(looks_like_replicated || looks_like_shared) || !looks_like_merge_tree) return UUIDHelpers::Nil;
  ...
  if (!create.storage || !create.storage->engine) return UUIDHelpers::Nil;
...
void DatabaseReplicated::recoverLostReplica(const ZooKeeperPtr &current_zookeeper, UInt32 our_log_ptr, UInt32 &max_log_ptr) {
  ...
  for (const auto &zk_table : table_name_to_metadata) {
    ...
    if (zk_replicated_id != UUIDHelpers::Nil) zk_replicated_id_to_name.emplace(zk_replicated_id, zk_table.first);
  }
  ...
  if (static_cast<double>(total_tables) * db_settings[DatabaseReplicatedSetting::max_broken_tables_ratio] <
    ...
    throw Exception(ErrorCodes::DATABASE_REPLICATION_FAILED, "Too many tables to recreate: {} of {}", tables_to_detach.size(),
                    total_tables);
  ...
  if (!drop_broken_tables && !tables_to_detach.empty()) {
    ...
    query_context->setCurrentQueryId({});
    {
      ...
      if (!CurrentThread::getGroup()) query_scope = CurrentThread::QueryScope::create(query_context);
  ...
  for (const auto &table_name : tables_to_detach) {
    ...
    auto move_table_to_database = [&](const String &broken_table_name, const String &to_database_name) {
      ...
      LOG_DEBUG(log, "Will RENAME TABLE {} TO {}.{}", backQuoteIfNeed(broken_table_name), backQuoteIfNeed(to_database_name),
                backQuoteIfNeed(to_name));
      ...
      DatabaseAtomic::renameTable(make_query_context(), broken_table_name, *to_db_ptr, to_name, /* exchange */ false,
                                  /* dictionary */ false);
  ...
  if (!tables_to_detach.empty())
    LOG_WARNING(log, "Cleaned {} outdated objects: dropped {} dictionaries and {} tables, moved {} tables", tables_to_detach.size(),
                dropped_dictionaries, dropped_tables.size() - dropped_dictionaries, moved_tables);
  ...
  for (auto &[_, intermediate, to] : replicated_tables_to_rename) rename_table(intermediate, to);

  LOG_DEBUG(log, "Renames completed successfully");

  for (const auto &id : dropped_tables) DatabaseCatalog::instance().waitTableFinallyDropped(id);
  ...
  for (const auto &[table_name, create_table_query] : table_name_to_metadata) {
    ...
    auto query_ast = parseQueryFromMetadataInZooKeeper(getContext(),
                                                       /*database_name_=*/getDatabaseName(),
                                                       /*zookeeper_path_=*/zookeeper_path,
                                                       /*node_name=*/table_name,
                                                       /*query=*/create_table_query);
    tables_dependencies.addDependencies(qualified_name, getDependenciesFromCreateQuery(getContext()->getGlobalContext(), qualified_name,
                                                                                       query_ast, getContext()->getCurrentDatabase())
                                                            .dependencies);
  }
  ...
  auto allow_concurrent_table_creation =
      getContext()->getServerSettings()[ServerSetting::max_database_replicated_create_table_thread_pool_size] != 1;
      ...
      auto task = [this, &table_id, &table_name_to_metadata, &make_query_context]() {
        ...
        if (metadata_it == table_name_to_metadata.end()) {
          ...
          LOG_WARNING(log,
                      ...
                      table_id.getNameForLogs(), getDatabaseName());
          return;
        }
        ...
        if (isTableExist(table_name, getContext())) {
          assert(create_query_string == readMetadataFile(table_name) ||
                 getTableUUIDIfReplicated(create_query_string, getContext()) != UUIDHelpers::Nil);
          return;
        }
...
std::map<String, String> DatabaseReplicated::getConsistentMetadataSnapshotImpl(const ZooKeeperPtr &zookeeper,
                                                                               const FilterByNameFunction &filter_by_table_name,
                                                                               size_t max_retries, UInt32 &max_log_ptr) const {
  std::map<String, String> table_name_to_metadata;

  if (zookeeper->isFeatureEnabled(KeeperFeatureFlag::FILTERED_LIST) && zookeeper->isFeatureEnabled(KeeperFeatureFlag::MULTI_READ) &&
      zookeeper->isFeatureEnabled(KeeperFeatureFlag::LIST_WITH_STAT_AND_DATA) && !filter_by_table_name) {
    auto paths = {zookeeper_path + "/metadata", zookeeper_path};
  ...
  while (++iteration <= max_retries) {
    ...
    for (const auto &table : escaped_table_names) paths_to_fetch.push_back(metadata_path + "/" + table);
    ...
    if (current_metadata_path_stat.czxid != prev_metadata_path_stat.czxid) {
      LOG_DEBUG(log, "Database zookeeper path was recreated. Created time before {}, current {}", prev_metadata_path_stat.czxid,
    ...
    for (size_t i = 0; i < escaped_table_names.size(); ++i) {
      ...
      if (res.error != Coordination::Error::ZOK) break;
    ...
    if (new_max_log_ptr == max_log_ptr && escaped_table_names.size() == table_name_to_metadata.size()) break;
...
ASTPtr DatabaseReplicated::parseQueryFromMetadata(ContextPtr context_, const String &database_name_, const String &table_name,
                                                  const String &query, const String &description) {
  ParserCreateQuery parser;
  auto ast = parseQuery(parser, query, description, 0, context_->getSettingsRef()[Setting::max_parser_depth],
  ...
  if (create.select && create.isView()) ApplyWithSubqueryVisitor(context_).visit(*create.select);

  return ast;
}
ASTPtr DatabaseReplicated::parseQueryFromMetadataInZooKeeper(ContextPtr context_, const String &database_name_,
                                                             const String &zookeeper_path_, const String &node_name, const String &query) {
...
void DatabaseReplicated::dropReplica(DatabaseReplicated *database, const String &zookeeper_name, const String &database_zookeeper_path,
                                     const String &shard, const String &replica, bool throw_if_noop) {
  ...
  if (!db_path_exists && !throw_if_noop) return;
  ...
  if (!zookeeper->exists(database_replica_path)) {
    if (!throw_if_noop) return;
    throw Exception(ErrorCodes::BAD_ARGUMENTS, "Replica {} does not exist (database path: {})", full_replica_name, database_zookeeper_path);
  }

  if (zookeeper->exists(database_replica_path + "/active"))
    throw Exception(ErrorCodes::BAD_ARGUMENTS, "Replica {} is active, cannot drop it (database path: {})", full_replica_name,
  ...
  if (database) database->createEmptyLogEntry(zookeeper);
...
void DatabaseReplicated::restoreDatabaseInKeeper(ContextPtr) {
  ...
  auto zookeeper = getZooKeeper();
  if (!zookeeper) throw Exception(ErrorCodes::NO_ZOOKEEPER, "No ZooKeeper");

  try {
    restoreDatabaseNodesInKeeper(zookeeper);
  } catch (const zkutil::KeeperMultiException &e) {
    if (Coordination::Error::ZNODEEXISTS != e.code) throw;
...
void DatabaseReplicated::stopReplication() {
  ...
  std::lock_guard lock{ddl_worker_mutex};
  if (ddl_worker) ddl_worker->shutdown();
}
...
void DatabaseReplicated::dropTable(ContextPtr local_context, const String &table_name, bool sync) {
  ...
  assert(!ddl_worker || !ddl_worker->isCurrentlyActive() || txn || startsWith(table_name, ".inner_id.") ||
         startsWith(table_name, ".tmp.inner_id."));
  ...
  auto table = tryGetTable(table_name, getContext());
  if (!table) throw Exception(ErrorCodes::LOGICAL_ERROR, "Table {} doesn't exist", table_name);
  if (table->getName() == "MaterializedView" || table->getName() == "WindowView" || table->getName() == "SharedSet" ||
      table->getName() == "SharedJoin") {
...
void DatabaseReplicated::renameTable(ContextPtr local_context, const String &table_name, IDatabase &to_database,
  ...
  if (table_name == to_table_name) throw Exception(ErrorCodes::INCORRECT_QUERY, "Cannot rename table to itself");
  if (!isTableExist(table_name, local_context)) throw Exception(ErrorCodes::UNKNOWN_TABLE, "Table {} does not exist", table_name);
  ...
  String statement_to;
  if (exchange) statement_to = readMetadataFile(to_table_name);
  ...
  if (txn->isInitialQuery()) {
    ...
    if (!txn->isCreateOrReplaceQuery()) txn->addOp(zkutil::makeRemoveRequest(metadata_zk_path, stat.version));
  ...
  if (txn && !is_recovering) txn->addOp(zkutil::makeSetRequest(replica_path + "/digest", toString(new_digest), -1));
...
void DatabaseReplicated::commitCreateTable(const ASTCreateQuery &query, const StoragePtr &table, const String &table_metadata_tmp_path,
                                           const String &table_metadata_path, ContextPtr query_context) {
...
void DatabaseReplicated::commitAlterTable(const StorageID &table_id, const String &table_metadata_tmp_path,
                                          const String &table_metadata_path, const String &statement, ContextPtr query_context) {
  auto txn = query_context->getZooKeeperMetadataTransaction();
...
void DatabaseReplicated::removeDetachedPermanentlyFlag(ContextPtr local_context, const String &table_name,
                                                       const String &table_metadata_path, bool attach) {
  waitDatabaseStarted();
...
std::vector<std::pair<ASTPtr, StoragePtr>> DatabaseReplicated::getTablesForBackup(const FilterByNameFunction &filter,
  ...
  for (const auto &[table_name, metadata] : snapshot) {
    ParserCreateQuery parser;
    auto create_table_query = parseQuery(parser, metadata, 0, getContext()->getSettingsRef()[Setting::max_parser_depth],
    ...
    StoragePtr storage;
    if (create.uuid != UUIDHelpers::Nil) storage = DatabaseCatalog::instance().tryGetByUUID(create.uuid).second;
...
void DatabaseReplicated::createTableRestoredFromBackup(const ASTPtr &create_table_query, ContextMutablePtr local_context,
                                                       std::shared_ptr<IRestoreCoordination> restore_coordination, UInt64 timeout_ms) {
  ...
  while (!isTableExist(table_name, local_context)) {
    ...
    if (elapsed > timeout)
      throw Exception(ErrorCodes::CANNOT_RESTORE_TABLE, "Couldn't restore table {}.{} on other node or sync it (elapsed {})",
...
bool DatabaseReplicated::shouldReplicateQuery(const ContextPtr &query_context, const ASTPtr &query_ptr) const {
  if (query_context->getClientInfo().is_replicated_database_internal) return false;
  ...
  const auto has_many_shards = [&]() {
    ...
    return !current_cluster ||  /// Couldn't get the cluster, so we don't know how many shards there are.
  ...
  if (const auto *alter = query_ptr->as<const ASTAlterQuery>()) {
    ...
    if (is_keeper_map_table(query_ptr) && !alter->isCommentAlter()) return false;

    if (has_many_shards() || !is_replicated_table(query_ptr)) return true;

    try {
      /// Metadata alter should go through database
      for (const auto &child : alter->command_list->children)
        if (AlterCommand::parse(child->as<ASTAlterCommand>())) return true;
  ...
  if (const auto *drop = query_ptr->as<const ASTDropQuery>()) {
    if (drop->table.get()) return drop->kind != ASTDropQuery::Truncate || !is_keeper_map_table(query_ptr);

    return false;
  }

  if (query_ptr->as<const ASTDeleteQuery>() || query_ptr->as<const ASTUpdateQuery>()) {
    if (is_keeper_map_table(query_ptr)) return false;
  ...
  auto create_fn = [](const DatabaseFactory::Arguments &args) {
    ...
    if (!args.create_query.attach) {
      ...
      if (engine->arguments->children.empty())
        engine->arguments->children.emplace_back(
            make_intrusive<ASTLiteral>(settings[DatabaseReplicatedSetting::default_replica_path].value));
      if (engine->arguments->children.size() == 1)
        engine->arguments->children.emplace_back(
            make_intrusive<ASTLiteral>(settings[DatabaseReplicatedSetting::default_replica_shard_name].value));
      if (engine->arguments->children.size() == 2)
        engine->arguments->children.emplace_back(
            make_intrusive<ASTLiteral>(settings[DatabaseReplicatedSetting::default_replica_name].value));
    }
    ...
    for (auto &engine_arg : arguments) engine_arg = evaluateConstantExpressionOrIdentifierAsLiteral(engine_arg, args.context);
    ...
    if (allow_uuid_macro) info.table_id.uuid = args.uuid;
    ...
    if (engine_define->settings) database_replicated_settings.loadFromQuery(*engine_define);

    return std::make_shared<DatabaseReplicated>(args.database_name, args.metadata_path, args.uuid, zookeeper_name, zookeeper_path,
                                                shard_name, replica_name, std::move(database_replicated_settings), args.context);
  };
  factory.registerDatabase("Replicated", create_fn, {.supports_arguments = true, .supports_settings = true});
}

BlockIO DatabaseReplicated::getQueryStatus(const String &zookeeper_name, const String &node_path, const String &replicas_path,
                                           ContextPtr context_, const Strings &hosts_to_wait, DDLGuardPtr &&database_guard) {
  BlockIO io;
  if (context_->getSettingsRef()[Setting::distributed_ddl_task_timeout] == 0) return io;
