...
class DiskBackupConfiguration : public Poco::Util::LayeredConfiguration {
 public:
  DiskBackupConfiguration() : Poco::Util::LayeredConfiguration() {}
};
...
void updateCreateQueryWithDatabaseBackupStoragePolicy(ASTCreateQuery *create_query, const DatabaseBackup::Configuration &config,
                                                      ContextPtr) {
  ...
  if (storage->engine->arguments) {
    ...
    for (; i < storage->engine->arguments->children.size(); ++i) args->children.push_back(storage->engine->arguments->children[i]->clone());
  }
...
DatabaseBackup::DatabaseBackup(const String &name_, const String &metadata_path_, const Configuration &config_, ContextPtr context_)
    : DatabaseOrdinary(name_, metadata_path_, DatabaseCatalog::getDataDirPath(name_) / "", "DatabaseBackup(" + name_ + ")", context_),
      config(config_) {}

void DatabaseBackup::createTable(ContextPtr, const String &, const StoragePtr &, const ASTPtr &) {
...
void DatabaseBackup::dropTable(ContextPtr, const String &, bool) {
  throw Exception(ErrorCodes::UNSUPPORTED_METHOD, "DROP TABLE is not supported for Backup database");
}

void DatabaseBackup::renameTable(ContextPtr, const String &, IDatabase &, const String &, bool, bool) {
...
void DatabaseBackup::loadTablesMetadata(ContextPtr local_context, ParsedTablesMetadata &metadata, bool) {
  if (!backup) throw Exception(ErrorCodes::LOGICAL_ERROR, "Backup is not initialized");
  ...
  auto process_metadata_file = [&metadata, &current_database_name, local_context, this](const String &file_name) {
    ...
    try {
      ...
      if (!create_query->storage->engine->name.ends_with("MergeTree")) {
        LOG_INFO(log, "Skipping table {} with engine {}. Only MergeTree engine is supported.", create_query->getTable(),
                 create_query->storage->engine->name);
        return;
      }
  ...
  ThreadPool pool(CurrentMetrics::DatabaseBackupThreads, CurrentMetrics::DatabaseBackupThreadsActive,
                  CurrentMetrics::DatabaseBackupThreadsScheduled);
    ...
    for (auto it = metadata_files.begin(); it < metadata_files.end(); std::advance(it, batch_size)) {
      ...
      runner.enqueueAndKeepTrack(
          [batch, &process_metadata_file]() mutable {
            for (const auto &file : batch) process_metadata_file(file);
          },
  ...
  LOG_INFO(log, "Metadata processed, database {} has {} tables and {} dictionaries in total.", current_database_name, tables_in_database,
...
ASTPtr DatabaseBackup::getCreateQueryFromMetadata(const String &table_name, bool throw_on_error) const {
  ...
  if (it == table_name_to_create_query.end()) {
    if (throw_on_error) throw Exception(ErrorCodes::CANNOT_GET_CREATE_TABLE_QUERY, "Table {} does not exist", table_name);
...
ASTPtr DatabaseBackup::getCreateDatabaseQueryImpl() const {
  ...
  const String query = fmt::format("CREATE DATABASE {} ENGINE = Backup({}, {})", backQuoteIfNeed(database_name),
                                   quoteString(config.database_name), quoteString(config.backup_info.toString()));

  ParserCreateQuery parser;
  ASTPtr ast = parseQuery(parser, query.data(), query.data() + query.size(), "", 0, settings[Setting::max_parser_depth],
...
void registerDatabaseBackup(DatabaseFactory &factory) {
  auto create_fn = [](const DatabaseFactory::Arguments &args) {
    ...
    ASTs engine_args;
    if (engine->arguments) engine_args = engine->arguments->children;
