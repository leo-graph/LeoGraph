...
void cckMetadataPathForOrdinary(const ASTCreateQuery& create, const String& metadata_path) {
  ...
  if (!default_db_disk->isSymlinkSupported()) return;

  const String& engine_name = create.storage->engine->name;
  const String& database_name = create.getDatabase();

  if (engine_name != "Ordinary") return;

  if (!default_db_disk->isSymlink(metadata_path)) return;

  String target_path = default_db_disk->readSymlink(metadata_path);
  fs::path path_to_remove = metadata_path;
  if (path_to_remove.filename().empty()) path_to_remove = path_to_remove.parent_path();
  ...
  throw Exception(ErrorCodes::CANNOT_CREATE_DATABASE,
                  ...
                  metadata_path, database_name, target_path, quoteString(path_to_remove.string()), quoteString(target_path),
                  quoteString(path_to_remove.string()));
}
...
void DatabaseFactory::validate(const ASTCreateQuery& create_query) const {
  ...
  bool has_unexpected_element =
      storage->engine->parameters || storage->partition_by || storage->primary_key || storage->order_by || storage->sample_by;
...
DatabasePtr DatabaseFactory::get(const ASTCreateQuery& create, const String& metadata_path, ContextPtr context) {
  ...
  if (impl && create.comment) impl->setDatabaseComment(create.comment->as<ASTLiteral>()->value.safeGet<String>());
...
DatabasePtr DatabaseFactory::getImpl(const ASTCreateQuery& create, const String& metadata_path, ContextPtr context) {
  ...
  if (storage->engine->arguments) has_engine_args = true;

  ASTs empty_engine_args;
  Arguments arguments{.engine_name = engine_name,
