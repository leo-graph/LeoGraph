...
DatabaseReplicatedDDLWorker::DatabaseReplicatedDDLWorker(DatabaseReplicated *db, ContextPtr context_)
    : DDLWorker(
          /* pool_size */ 1, db->zookeeper_path + "/log", db->zookeeper_path + "/replicas", context_, nullptr, {}, db->zookeeper_name,
          fmt::format("DDLWorker({})", db->getDatabaseName())),
      database(db) {
  LOG_INFO(
      log, "allow_skipping_old_temporary_tables_ddls_of_refreshable_materialized_views {}",
...
bool DatabaseReplicatedDDLWorker::initializeMainThread() {
  ...
  while (!stop_flag) {
    try {
      ...
      if (database->is_readonly) database->tryConnectToZooKeeperAndInitDatabase(LoadingStrictnessLevel::ATTACH);
      ...
      if (database->db_settings[DatabaseReplicatedSetting::max_retries_before_automatic_recovery] &&
        ...
        LOG_WARNING(log,
                    "Database got stuck at processing task {}: it failed {} times in a row with the same error. "
...
void DatabaseReplicatedDDLWorker::initializeReplication() {
  ...
  LOG_TRACE(log, "Trying to initialize replication: our_log_ptr={}, max_log_ptr={}, local_digest={}, zk_digest={}", our_log_ptr,
            max_log_ptr, local_digest, digest);
  ...
  if (is_new_replica || lost_according_to_log_ptr || lost_according_to_digest) {
    if (!is_new_replica)
      LOG_WARNING(log, "Replica seems to be lost: our_log_ptr={}, max_log_ptr={}, local_digest={}, zk_digest={}", our_log_ptr, max_log_ptr,
                  local_digest, digest);
  ...
  if (is_new_replica || first_initialization) {
    ...
    unsynced_after_recovery =
        max_log_ptr + database->db_settings[DatabaseReplicatedSetting::max_replication_lag_to_enqueue] <= new_max_log_ptr;
    LOG_INFO(log, "Finishing replica initialization, our_log_ptr={}, max_log_ptr={}, unsynced_after_recovery={}", max_log_ptr,
             new_max_log_ptr, unsynced_after_recovery.load());
    if (unsynced_after_recovery) active_id += DatabaseReplicated::REPLICA_UNSYNCED_MARKER;
  } else {
    /// For lost_according_to_digest and ordinary connection loss we don't mark replica as "unsynced"
    /// because there's much lower chance of huge replication lag, and also we want to avoid metadata consistency issues for existing
    /// replicas when we don't wait for such replicas due to distributed_ddl_output_mode = '*_only_active'.
...
void DatabaseReplicatedDDLWorker::scheduleTasks(bool reinitialized) {
  ...
  if (need_update_cached_cluster) {
    ...
    if (!database->replica_group_name.empty()) database->setCluster(database->getClusterImpl(/*all_groups*/ true), /*all_groups*/ true);
...
bool DatabaseReplicatedDDLWorker::waitForReplicaToProcessAllEntries(UInt64 timeout_ms) {
  ...
  if (our_log_ptr == max_log_ptr) return true;
  ...
  if (new_log == toString(max_log_ptr)) return true;

  return event_ptr->tryWait(timeout_ms);
}

String DatabaseReplicatedDDLWorker::enqueueQueryImpl(const ZooKeeperPtr &zookeeper, DDLLogEntry &entry, DatabaseReplicated *const database,
                                                     bool committed, Coordination::Requests additional_checks) {
  ...
  while (--iters) {
    ...
    if (res[0]->error != Coordination::Error::ZNODEEXISTS) zkutil::KeeperMultiException::check(code, ops, res);
...
String DatabaseReplicatedDDLWorker::tryEnqueueAndExecuteEntry(DDLLogEntry &entry, ContextPtr query_context, bool internal_query) {
  ...
  if (our_log_ptr + database->db_settings[DatabaseReplicatedSetting::max_replication_lag_to_enqueue] < max_log_ptr)
    throw Exception(ErrorCodes::NOT_A_LEADER,
                    "Cannot enqueue query on this replica, "
                    "because it has replication lag of {} queries. Try other replica.",
                    max_log_ptr - our_log_ptr);
  ...
  LOG_DEBUG(log, "Waiting for worker thread to process all entries before {} (timeout: {}s{})", entry_name, timeout,
            cancellation.stop_possible() ? ", cancellable" : "");

  {
    ...
    if (!processed)
      throw Exception(ErrorCodes::UNFINISHED,
                      "Timeout: Cannot enqueue query on this replica, "
  ...
  if (!task->was_executed) {
    throw Exception(ErrorCodes::LOGICAL_ERROR, "Entry {} was executed, but was not committed: code {}: {}", task->entry_name,
...
static bool getRMVCoordinationInfo(LoggerPtr log, const ZooKeeperPtr &zookeeper, UUID parent_uuid, Coordination::Stat &stats,
                                   RefreshTask::CoordinationZnode &coordination_znode) {
  if (parent_uuid == UUIDHelpers::Nil) return false;
  ...
  if (!refresh || refresh->append) return false;
  const auto *mv = dynamic_cast<const StorageMaterializedView *>(storage.get());
  if (!mv) return false;

  const auto coordination_path = mv->getCoordinationPath();

  if (!coordination_path.has_value() || (*coordination_path).empty()) return false;
  try {
    String data;
    if (!zookeeper->tryGet(*coordination_path, data, &stats)) return false;
...
bool DatabaseReplicatedDDLWorker::shouldSkipCreatingRMVTempTable(const ZooKeeperPtr &zookeeper, UUID parent_uuid, UUID create_uuid,
                                                                 int64_t ddl_log_ctime) {
  if (create_uuid == UUIDHelpers::Nil) return false;

  Coordination::Stat stats;
  RefreshTask::CoordinationZnode coordination_znode;

  if (!getRMVCoordinationInfo(log, zookeeper, parent_uuid, stats, coordination_znode)) return false;

  LOG_TEST(log, "MV {}, coordination info: {}", parent_uuid, coordination_znode.toString());
  if (coordination_znode.last_success_table_uuid == create_uuid) return false;
...
bool DatabaseReplicatedDDLWorker::shouldSkipRenamingRMVTempTable(const ZooKeeperPtr &zookeeper, UUID parent_uuid,
                                                                 const QualifiedTableName &rename_from_table) {
  Coordination::Stat stats;
  RefreshTask::CoordinationZnode coordination_znode;
...
DDLTaskPtr DatabaseReplicatedDDLWorker::initAndCheckTask(const String &entry_name, String &out_reason, const ZooKeeperPtr &zookeeper,
  ...
  if (task->is_initial_query) {
    ...
    assert(zookeeper->exists(fs::path(entry_path) / "committed") ==
           (zookeeper->get(task->getFinishedNodePath()) == ExecutionStatus(0).serializeText()));
  ...
  if (database->db_settings[DatabaseReplicatedSetting::allow_skipping_old_temporary_tables_ddls_of_refreshable_materialized_views] &&
    ...
    // 3. Rename (with exchange) the temporary table to the target table name. The target table name can be the inner table of the view, or
    // the table specified in the view create query.
    // 4. Drop the temporary table after exchanging. Note that the temporary table after exchanging is not the table created in step 1. It
    // is the one exchanged in step 3.
    ...
    // Once the creating DDL is skipped, there is no table to rename in the renaming DDL, so the renaming DDL is skipped if the table name
    // does not exist. For the dropping DDLs, as the query contains `IF EXISTS`, we don't need to skip.
    ...
    if (const auto *create_query = task->query->as<ASTCreateQuery>()) {
      if (create_query->uuid != UUIDHelpers::Nil &&
        ...
        LOG_INFO(log, "Skip DDL {} as creating the old temp table of RMV '{}', query {}", entry_name, parent_table_uuid,
                 create_query->formatForLogging());
        out_reason =
            fmt::format("Creating the old temp table '{}' of Refreshable Materialized View '{}'", create_query->uuid, parent_table_uuid);
        return {};
      }
      ...
      if (rename_query->getElements().size() == 1) {
        ...
        if (shouldSkipRenamingRMVTempTable(zookeeper, *task->entry.parent_table_uuid, from_table)) {
          LOG_INFO(log, "Skip DDL {} as renaming the old temp table of RMV '{}', query {}", entry_name, parent_table_uuid,
                   rename_query->formatForLogging());

          out_reason = fmt::format("Renaming the old temp table '{}' of Refreshable Materialized View '{}'", from_table.getFullName(),
                                   parent_table_uuid);
          return {};
        }
      }
...
void DatabaseReplicatedDDLWorker::initializeLogPointer(const String &processed_entry_name) { updateMaxDDLEntryID(processed_entry_name); }
