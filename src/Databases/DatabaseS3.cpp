...
static const std::unordered_set<std::string_view> optional_configuration_keys = {"url", "access_key_id", "secret_access_key",
                                                                                 "no_sign_request"};
...
DatabaseS3::DatabaseS3(const String &name_, const Configuration &config_, ContextPtr context_)
    : IDatabase(name_), WithContext(context_->getGlobalContext()), config(config_), log(getLogger("DatabaseS3(" + name_ + ")")) {}
...
void DatabaseS3::addTable(const std::string &table_name, StoragePtr table_storage) const {
  ...
  if (!inserted)
    throw Exception(ErrorCodes::LOGICAL_ERROR, "Table with name `{}` already exists in database `{}` (engine {})", table_name,
                    getDatabaseName(), getEngineName());
}

std::string DatabaseS3::getFullUrl(const std::string &name) const {
  if (!config.url_prefix.empty()) return fs::path(config.url_prefix) / name;
...
bool DatabaseS3::isTableExist(const String &name, ContextPtr context_) const {
  std::lock_guard lock(mutex);
  if (loaded_tables.contains(name)) return true;
...
StoragePtr DatabaseS3::getTableImpl(const String &name, ContextPtr context_) const {
  /// Check if the table exists in the loaded tables map.
  {
    ...
    if (it != loaded_tables.end()) return it->second;
  }
  ...
  if (!table_function) return nullptr;

  /// TableFunctionS3 throws exceptions, if table cannot be created.
  auto table_storage =
      table_function->execute(function, context_, name, /*cached_columns_=*/{}, /*use_global_context=*/false, /*is_insert_query=*/true);
  if (table_storage) addTable(name, table_storage);
...
StoragePtr DatabaseS3::getTable(const String &name, ContextPtr context_) const {
  ...
  if (auto storage = getTableImpl(name, context_)) return storage;
...
StoragePtr DatabaseS3::tryGetTable(const String &name, ContextPtr context_) const {
  try {
    ...
    if (e.code() == ErrorCodes::BAD_ARGUMENTS || e.code() == ErrorCodes::S3_ERROR || e.code() == ErrorCodes::FILE_DOESNT_EXIST ||
...
DatabaseS3::Configuration DatabaseS3::parseArguments(ASTs engine_args, ContextPtr context_) {
  ...
  if (auto named_collection = tryGetNamedCollectionWithOverrides(engine_args, context_)) {
    ...
    if (!key_id.empty()) result.access_key_id = key_id;

    if (!secret_key.empty()) result.secret_access_key = secret_key;
  } else {
    ...
    for (auto &arg : engine_args) arg = evaluateConstantExpressionOrIdentifierAsLiteral(arg, context_);

    if (engine_args.size() > 3) throw Exception::createRuntime(ErrorCodes::NUMBER_OF_ARGUMENTS_DOESNT_MATCH, error_message.c_str());

    if (engine_args.empty()) return result;
