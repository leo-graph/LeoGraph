...
LoadTaskPtrs TablesLoader::loadTablesAsync(LoadJobSet load_after) {
  ...
  if (databases_to_load.empty()) return {};
  ...
  LOG_INFO(log, "Parsed metadata of {} tables in {} databases in {} sec", metadata.parsed_tables.size(), databases_to_load.size(),
  ...
  for (const auto& table_id : all_loading_dependencies.getTablesSortedByDependency()) {
    ...
    auto task =
        databases[table_name.database]->loadTableFromMetadataAsync(async_loader, getGoals(load_dependency_tasks, load_after), load_context,
                                                                   path_and_query.path, table_name, path_and_query.ast, strictness_mode);
...
LoadTaskPtrs TablesLoader::startupTablesAsync(LoadJobSet startup_after) {
  ...
  for (const auto& table_id : mv_to_dependencies.getTables()) {
    ...
    for (const auto& storage_id : storage_id_vector) all_startup_dependencies.addDependency(storage_id, table_id);
  }
  for (const auto& table_id : mv_from_dependencies.getTables()) {
    ...
    for (const auto& storage_id : storage_id_vector) all_startup_dependencies.addDependency(table_id, storage_id);
  }
  ...
  for (const auto& table_id : all_startup_dependencies.getTablesSortedByDependency()) {
    ...
    auto task = databases[table_name.database]->startupTableAsync(
        async_loader, joinJobs(load_table[table_id.getFullTableName()]->goals(), getGoals(startup_mv_dependency_tasks, startup_after)),
        table_name, strictness_mode);
  ...
  for (auto& database_name : databases_to_load) {
    auto task = databases[database_name]->startupDatabaseAsync(async_loader, getGoals(startup_database[database_name], startup_after),
                                                               strictness_mode);
    result.push_back(task);
  }
...
void TablesLoader::buildDependencyGraph() {
  for (const auto& [table_name, table_metadata] : metadata.parsed_tables) {
    auto new_ref_dependencies =
        getDependenciesFromCreateQuery(global_context, table_name, table_metadata.ast, global_context->getCurrentDatabase(),
    ...
    if (!new_ref_dependencies.dependencies.empty()) referential_dependencies.addDependencies(table_name, new_ref_dependencies.dependencies);
...
void TablesLoader::removeUnresolvableDependencies() {
  auto need_exclude_dependency = [this](const StorageID& table_id) {
    ...
    if (metadata.parsed_tables.contains(table_id.getQualifiedName())) return false;
    ...
    if (DatabaseCatalog::instance().isTableExist(table_id, global_context)) {
      ...
      LOG_WARNING(log,
                  ...
                  fmt::join(all_loading_dependencies.getDependents(table_id), ", "), table_id);
    } else {
      ...
      LOG_WARNING(log, "Tables {} depend on {}, but seems like that does not exist. Will ignore it and try to load existing tables",
                  fmt::join(all_loading_dependencies.getDependents(table_id), ", "), table_id);
    }
    ...
    if (num_dependencies || !num_dependents)
      throw Exception(ErrorCodes::LOGICAL_ERROR,
                      "Table {} does not have dependencies and dependent tables as it expected to."
                      "It's a bug",
                      table_id);
