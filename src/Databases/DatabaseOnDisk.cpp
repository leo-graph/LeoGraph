...
std::pair<String, StoragePtr> createTableFromAST(ASTCreateQuery ast_create_query, const String &database_name,
                                                 const String &table_data_path_relative, ContextMutablePtr context,
                                                 LoadingStrictnessLevel mode) {
  ast_create_query.attach = true;
  ast_create_query.setDatabase(database_name);

  if (ast_create_query.select && ast_create_query.isView()) ApplyWithSubqueryVisitor(context).visit(*ast_create_query.select);
  ...
  if (ast_create_query.is_dictionary) has_columns = false;
  if (ast_create_query.isParameterizedView()) has_columns = false;
    ...
    if (!ast_create_query.columns_list || !ast_create_query.columns_list->columns) {
      if (!ast_create_query.storage || !ast_create_query.storage->engine)
        throw Exception(ErrorCodes::LOGICAL_ERROR,
                        "Invalid storage definition in metadata file: "
...
String getObjectDefinitionFromCreateQuery(const ASTPtr &query) {
  ...
  if (!create) throw Exception(ErrorCodes::LOGICAL_ERROR, "Query '{}' is not CREATE query", query->formatForErrorMessage());

  /// Clean the query from temporary flags.
  cleanupObjectDefinitionFromTemporaryFlags(*create);

  if (!create->is_dictionary) create->attach = true;

  /// We remove everything that is not needed for ATTACH from the query.
  assert(!create->isTemporary());
  create->database.reset();

  if (create->uuid != UUIDHelpers::Nil) create->setTable(TABLE_WITH_UUID_NAME_PLACEHOLDER);
...
DatabaseOnDisk::DatabaseOnDisk(const String &name, const String &metadata_path_, const String &data_path_, const String &logger,
...
void DatabaseOnDisk::createTable(ContextPtr local_context, const String &table_name, const StoragePtr &table, const ASTPtr &query) {
  ...
  if (!create.attach) checkMetadataFilenameAvailability(table_name);
  ...
  if (create.attach && db_disk->existsFileOrDirectory(table_metadata_path)) {
    ...
    if (create.uuid != create_detached.uuid)
      throw Exception(ErrorCodes::TABLE_ALREADY_EXISTS,
                      ...
                      backQuote(getDatabaseName()), backQuote(table_name), backQuote(getDatabaseName()), backQuote(table_name));
  }
...
void DatabaseOnDisk::commitCreateTable(const ASTCreateQuery &query, const StoragePtr &table, const String &table_metadata_tmp_path,
...
void DatabaseOnDisk::dropTable(ContextPtr local_context, const String &table_name, bool /*sync*/) {
  ...
  if (table_data_path_relative.empty()) throw Exception(ErrorCodes::LOGICAL_ERROR, "Path is empty");
  ...
  try {
    ...
    if (table) attachTable(local_context, table_name, table, table_data_path_relative);
    if (renamed) db_disk->replaceFile(table_metadata_path_drop, table_metadata_path);
    throw;
  }

  for (const auto &[disk_name, disk] : getContext()->getDisksMap()) {
    if (disk->isReadOnly() || !disk->existsDirectory(table_data_path_relative)) continue;

    LOG_INFO(log, "Removing data directory from disk {} with path {} for dropped table {} ", disk_name, table_data_path_relative,
             table_name);
...
void DatabaseOnDisk::checkMetadataFilenameAvailabilityUnlocked(const String &to_table_name) const {
  ...
  if (db_disk->existsFile(table_metadata_path)) {
    ...
    if (db_disk->existsFile(detached_permanently_flag))
      throw Exception(ErrorCodes::TABLE_ALREADY_EXISTS, "Table {}.{} already exists (detached permanently)", backQuote(database_name),
                      backQuote(to_table_name));
    else
      throw Exception(ErrorCodes::TABLE_ALREADY_EXISTS, "Table {}.{} already exists (detached)", backQuote(database_name),
                      backQuote(to_table_name));
  }
}

void DatabaseOnDisk::renameTable(ContextPtr local_context, const String &table_name, IDatabase &to_database, const String &to_table_name,
                                 bool exchange, bool dictionary) {
  if (exchange) throw Exception(ErrorCodes::NOT_IMPLEMENTED, "Tables can be exchanged only in Atomic databases");
  ...
  try {
    ...
    if (from_ordinary_to_atomic) create.uuid = UUIDHelpers::generateV4();
    if (from_atomic_to_ordinary) std::swap(create.uuid, prev_uuid);

    if (auto *target_db = dynamic_cast<DatabaseOnDisk *>(&to_database)) target_db->checkMetadataFilenameAvailability(to_table_name);
  ...
  if (from_atomic_to_ordinary) {
    ...
    if (table->storesDataOnDisk()) atomic_db.tryRemoveSymlink(table_name);
...
ASTPtr DatabaseOnDisk::getCreateTableQueryImpl(const String &table_name, ContextPtr, bool throw_on_error) const {
  ...
  if (has_table) is_system_storage = storage->isSystemStorage();
  ...
  try {
    ...
    if (!is_system_storage && throw_on_error) throw;
  }
  if (!ast && is_system_storage) ast = getCreateQueryFromStorage(table_name, storage, throw_on_error);
  return ast;
}
...
ASTPtr DatabaseOnDisk::getCreateDatabaseQueryImpl() const {
  ...
  if (!ast) {
    ...
    ast = parseQuery(parser, query.data(), query.data() + query.size(), "", 0, settings[Setting::max_parser_depth],
...
void DatabaseOnDisk::drop(ContextPtr local_context) {
  ...
  if (local_context->getSettingsRef()[Setting::force_remove_data_recursively_on_drop]) {
    ...
    try {
      ...
      if (e.code() != ErrorCodes::CANNOT_RMDIR) throw;
      throw Exception(ErrorCodes::DATABASE_NOT_EMPTY,
                      "Cannot drop: {}. "
...
time_t DatabaseOnDisk::getObjectMetadataModificationTime(const String &object_name) const {
  ...
  if (!db_disk->existsFileOrDirectory(table_metadata_path)) return static_cast<time_t>(0);
...
void DatabaseOnDisk::iterateMetadataFiles(const IteratingFunction &process_metadata_file) const {
  auto db_disk = getDisk();
  if (!db_disk->existsDirectory(metadata_path)) return;
  ...
  for (const auto it = db_disk->iterateDirectory(metadata_path); it->isValid(); it->next()) {
    ...
    if (!file_name.empty() && file_name.at(0) == '.') continue;

    /// There are .sql.bak files - skip them.
    if (endsWith(file_name, ".sql.bak")) continue;

    /// Permanently detached table flag
    if (endsWith(file_name, ".sql.detached")) continue;
    ...
    if (endsWith(file_name, ".sql.tmp_drop")) {
      ...
      throw Exception(ErrorCodes::INCORRECT_FILE_NAME, "Incorrect file extension: {} in metadata directory {}", file_name,
                      getMetadataPath());
  }
  ...
  ThreadPool pool(CurrentMetrics::DatabaseOnDiskThreads, CurrentMetrics::DatabaseOnDiskThreadsActive,
                  CurrentMetrics::DatabaseOnDiskThreadsScheduled);
  ...
  for (auto it = metadata_files.begin(); it < metadata_files.end(); std::advance(it, batch_size)) {
    ...
    pool.scheduleOrThrow(
        ...
        Priority{}, getContext()->getSettingsRef()[Setting::lock_acquire_timeout].totalMicroseconds());
  }
  pool.wait();
}

ASTPtr DatabaseOnDisk::parseQueryFromMetadata(LoggerPtr logger, ContextPtr local_context, DiskPtr disk, const String &metadata_file_path,
                                              bool throw_on_error /*= true*/, bool remove_empty /*= false*/) {
  ...
  if (!disk->existsFile(metadata_file_path)) {
    if (!throw_on_error) return nullptr;
  ...
  if (remove_empty && query.empty()) {
    if (logger) LOG_ERROR(logger, "File {} is empty. Removing.", metadata_file_path);
...
ASTPtr DatabaseOnDisk::parseQueryFromMetadata(LoggerPtr logger, ContextPtr local_context, const String &metadata_file_path,
                                              const String &query, bool throw_on_error /*= true*/) {
  const auto &settings = local_context->getSettingsRef();
  ParserCreateQuery parser;
  const char *pos = query.data();
  std::string error_message;
  auto ast = tryParseQuery(parser, pos, pos + query.size(), error_message,
                           /* hilite = */ false, "in file " + metadata_file_path,
                           /* allow_multi_statements = */ false, 0, settings[Setting::max_parser_depth],
                           settings[Setting::max_parser_backtracks], true);

  if (!ast && throw_on_error) throw Exception::createDeprecated(error_message, ErrorCodes::SYNTAX_ERROR);
  ...
  if (create.table && create.uuid != UUIDHelpers::Nil) {
    ...
    if (create.getTable() != TABLE_WITH_UUID_NAME_PLACEHOLDER && logger)
      LOG_WARNING(logger, "File {} contains both UUID and table name. Will use name `{}` instead of `{}`", metadata_file_path, table_name,
                  create.getTable());
...
ASTPtr DatabaseOnDisk::getCreateQueryFromStorage(const String &table_name, const StoragePtr &storage, bool throw_on_error) const {
  ...
  if (metadata_ptr == nullptr) {
    if (throw_on_error)
      throw Exception(ErrorCodes::CANNOT_GET_CREATE_TABLE_QUERY, "Cannot get metadata of {}.{}", backQuote(getDatabaseName()),
  ...
  auto create_table_query =
      DB::getCreateQueryFromStorage(storage, ast_storage, false, static_cast<unsigned>(settings[Setting::max_parser_depth]),
                                    static_cast<unsigned>(settings[Setting::max_parser_backtracks]), throw_on_error);
...
void DatabaseOnDisk::modifySettingsMetadata(const SettingsChanges &settings_changes, ContextPtr) {
  ...
  if (settings) {
    ...
    for (const auto &change : settings_changes) {
      auto it = std::find_if(storage_settings.begin(), storage_settings.end(), [&](const auto &prev) { return prev.name == change.name; });
