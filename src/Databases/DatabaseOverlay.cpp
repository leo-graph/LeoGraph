...
bool DatabaseOverlay::isTableExist(const String &table_name, ContextPtr context_) const {
  for (const auto &db : databases) {
    if (db->isTableExist(table_name, context_)) return true;
  }
  return false;
}
...
void DatabaseOverlay::createTable(ContextPtr context_, const String &table_name, const StoragePtr &table, const ASTPtr &query) {
  ...
  throw Exception(ErrorCodes::LOGICAL_ERROR, "There are no databases for CREATE TABLE `{}` query in database `{}` (engine {})", table_name,
                  getDatabaseName(), getEngineName());
}
...
void DatabaseOverlay::dropTable(ContextPtr context_, const String &table_name, bool sync) {
  ...
  throw Exception(ErrorCodes::LOGICAL_ERROR, "There are no databases for DROP TABLE `{}` query in database `{}` (engine {})", table_name,
                  getDatabaseName(), getEngineName());
}

void DatabaseOverlay::attachTable(ContextPtr context_, const String &table_name, const StoragePtr &table,
  ...
  throw Exception(ErrorCodes::LOGICAL_ERROR, "There are no databases for ATTACH TABLE `{}` query in database `{}` (engine {})", table_name,
                  getDatabaseName(), getEngineName());
}
...
StoragePtr DatabaseOverlay::detachTable(ContextPtr context_, const String &table_name) {
  StoragePtr result = nullptr;
  for (auto &db : databases) {
    if (db->isTableExist(table_name, context_)) return db->detachTable(context_, table_name);
  }
  throw Exception(ErrorCodes::LOGICAL_ERROR, "There are no databases for DETACH TABLE `{}` query in database `{}` (engine {})", table_name,
                  getDatabaseName(), getEngineName());
}

void DatabaseOverlay::renameTable(ContextPtr current_context, const String &name, IDatabase &to_database, const String &to_name,
    ...
    if (db->isTableExist(name, current_context)) {
      if (DatabaseOverlay *to_overlay_database = typeid_cast<DatabaseOverlay *>(&to_database)) {
        ...
        if (to_overlay_database->databases.empty())
          throw Exception(ErrorCodes::BAD_ARGUMENTS, "The destination Overlay database {} does not have any members",
...
ASTPtr DatabaseOverlay::getCreateTableQueryImpl(const String &name, ContextPtr context_, bool throw_on_error) const {
  ...
  if (!result && throw_on_error)
    throw Exception(ErrorCodes::CANNOT_GET_CREATE_TABLE_QUERY, "There is no metadata of table `{}` in database `{}` (engine {})", name,
                    getDatabaseName(), getEngineName());
  return result;
}
...
UUID DatabaseOverlay::getUUID() const {
  ...
  for (const auto &db : databases) {
    result = db->getUUID();
    if (result != UUIDHelpers::Nil) break;
  }
  return result;
}
...
void DatabaseOverlay::drop(ContextPtr context_) {
  for (auto &db : databases) db->drop(context_);
}
...
void DatabaseOverlay::alterTable(ContextPtr local_context, const StorageID &table_id, const StorageInMemoryMetadata &metadata,
  ...
  throw Exception(ErrorCodes::LOGICAL_ERROR, "There are no databases for ALTER TABLE `{}` query in database `{}` (engine {})",
                  table_id.table_name, getDatabaseName(), getEngineName());
}

std::vector<std::pair<ASTPtr, StoragePtr>> DatabaseOverlay::getTablesForBackup(const FilterByNameFunction &filter,
...
void DatabaseOverlay::createTableRestoredFromBackup(const ASTPtr &create_table_query, ContextMutablePtr local_context,
...
void DatabaseOverlay::shutdown() {
  for (auto &db : databases) db->shutdown();
}

DatabaseTablesIteratorPtr DatabaseOverlay::getTablesIterator(ContextPtr context_, const FilterByNameFunction &filter_by_table_name,
...
bool DatabaseOverlay::isExternal() const {
  for (const auto &db : databases)
    if (!db->isExternal()) return false;
  return true;
}

void DatabaseOverlay::loadStoredObjects(ContextMutablePtr local_context, LoadingStrictnessLevel mode) {
  for (auto &db : databases)
    if (!db->isReadOnly()) db->loadStoredObjects(local_context, mode);
}

bool DatabaseOverlay::supportsLoadingInTopologicalOrder() const {
  for (const auto &db : databases)
    if (db->supportsLoadingInTopologicalOrder()) return true;
  return false;
}

void DatabaseOverlay::beforeLoadingMetadata(ContextMutablePtr local_context, LoadingStrictnessLevel mode) {
  for (auto &db : databases)
    if (!db->isReadOnly()) db->beforeLoadingMetadata(local_context, mode);
}

void DatabaseOverlay::loadTablesMetadata(ContextPtr local_context, ParsedTablesMetadata &metadata, bool is_startup) {
  for (auto &db : databases)
    if (!db->isReadOnly()) db->loadTablesMetadata(local_context, metadata, is_startup);
}

void DatabaseOverlay::loadTableFromMetadata(ContextMutablePtr local_context, const String &file_path, const QualifiedTableName &name,
                                            const ASTPtr &ast, LoadingStrictnessLevel mode) {
  for (auto &db : databases) {
    if (db->isReadOnly()) continue;
  ...
  throw Exception(ErrorCodes::LOGICAL_ERROR,
                  "There are no databases capable of loading table `{}` from path `{}` in database `{}` (engine {})", name.table, file_path,
                  getDatabaseName(), getEngineName());
}

LoadTaskPtr DatabaseOverlay::loadTableFromMetadataAsync(AsyncLoader &async_loader, LoadJobSet load_after, ContextMutablePtr local_context,
...
LoadTaskPtr DatabaseOverlay::startupTableAsync(AsyncLoader &async_loader, LoadJobSet startup_after, const QualifiedTableName &name,
                                               LoadingStrictnessLevel mode) {
  for (auto &db : databases) {
    if (db->isReadOnly()) continue;
  ...
  throw Exception(ErrorCodes::LOGICAL_ERROR, "There are no databases capable of starting up table `{}` in database `{}` (engine {})",
                  name.table, getDatabaseName(), getEngineName());
}

LoadTaskPtr DatabaseOverlay::startupDatabaseAsync(AsyncLoader &async_loader, LoadJobSet startup_after, LoadingStrictnessLevel mode) {
  for (auto &db : databases) {
    if (db->isReadOnly()) continue;
  ...
  throw Exception(ErrorCodes::LOGICAL_ERROR, "There are no databases capable of starting up asynchronously in database `{}` (engine {})",
                  getDatabaseName(), getEngineName());
}
...
void DatabaseOverlay::waitTableStarted(const String &name) const {
  ...
  throw Exception(ErrorCodes::LOGICAL_ERROR,
                  "There are no databases capable of waiting for table startup `{}` in database `{}` (engine {})", name, getDatabaseName(),
                  getEngineName());
}
...
void DatabaseOverlay::waitDatabaseStarted() const {
  ...
  throw Exception(ErrorCodes::LOGICAL_ERROR, "There are no databases capable of waiting for startup in database `{}` (engine {})",
                  getDatabaseName(), getEngineName());
}
...
void DatabaseOverlay::stopLoading() {
  ...
  throw Exception(ErrorCodes::LOGICAL_ERROR, "There are no databases capable of stop loading in database `{}` (engine {})",
                  getDatabaseName(), getEngineName());
}
