...
#define LIST_OF_DATABASE_METADATA_DISK_SETTINGS(DECLARE, DECLARE_WITH_ALIAS)                                                                                 \
  ...
  DECLARE(                                                                                                                                                   \
      Bool, lazy_load_tables, false,                                                                                                                         \
      R"(If enabled, tables are not loaded during database startup. Instead, a lightweight proxy is created and the real table is loaded on first access.)", \
      0)
...
void DatabaseMetadataDiskSettingsImpl::loadFromQuery(ASTStorage &storage_def, ContextPtr context, bool is_attach) {
  if (!storage_def.settings) return;
...
DatabaseMetadataDiskSettings::DatabaseMetadataDiskSettings(const DatabaseMetadataDiskSettings &settings)
    : impl(std::make_unique<DatabaseMetadataDiskSettingsImpl>(*settings.impl)) {}
