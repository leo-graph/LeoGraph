...
DatabasePostgreSQL::DatabasePostgreSQL(ContextPtr context_, const String &metadata_path_, const ASTStorage *database_engine_define_,
                                       const String &dbname_, const StoragePostgreSQL::Configuration &configuration_,
                                       postgres::PoolWithFailoverPtr pool_, bool cache_tables_, UUID uuid)
  ...
  cleaner_task =
      getContext()->getSchedulePool().createTask(StorageID::createEmpty(), "PostgreSQLCleanerTask", [this] { removeOutdatedTables(); });
  cleaner_task->deactivate();
}

String DatabasePostgreSQL::getTableNameForLogs(const String &table_name) const {
  if (configuration.schema.empty()) return fmt::format("{}.{}", configuration.database, table_name);
...
String DatabasePostgreSQL::formatTableName(const String &table_name, bool quoted) const {
  if (configuration.schema.empty()) return quoted ? doubleQuoteString(table_name) : table_name;
...
bool DatabasePostgreSQL::empty() const {
  ...
  for (const auto &table_name : tables_list)
    if (!detached_or_dropped.contains(table_name)) return false;

  return true;
}

DatabaseTablesIteratorPtr DatabasePostgreSQL::getTablesIterator(ContextPtr local_context,
  ...
  try {
    ...
    for (const auto &table_name : table_names)
      if (!detached_or_dropped.contains(table_name)) tables[table_name] = fetchTable(table_name, local_context, true);
  } catch (...) {
...
StoragePtr DatabasePostgreSQL::tryGetTable(const String &table_name, ContextPtr local_context) const {
  ...
  if (!detached_or_dropped.contains(table_name)) return fetchTable(table_name, local_context, false);
...
StoragePtr DatabasePostgreSQL::fetchTable(const String &table_name, ContextPtr context_, bool table_checked) const {
  if (!cache_tables || !cached_tables.contains(table_name)) {
    if (!table_checked && !checkPostgresTable(table_name)) return StoragePtr{};

    auto connection_holder = pool->get();
    auto columns_info = fetchPostgreSQLTableStructure(connection_holder->get(), table_name, configuration.schema).physical_columns;

    if (!columns_info) return StoragePtr{};

    auto storage = std::make_shared<StoragePostgreSQL>(StorageID(database_name, table_name), pool, table_name,
...
void DatabasePostgreSQL::attachTable(ContextPtr /* context_ */, const String &table_name, const StoragePtr &storage, const String &) {
  ...
  if (!checkPostgresTable(table_name))
    throw Exception(ErrorCodes::UNKNOWN_TABLE, "Cannot attach PostgreSQL table {} because it does not exist in PostgreSQL (database: {})",
                    getTableNameForLogs(table_name), database_name);

  if (!detached_or_dropped.contains(table_name))
    throw Exception(ErrorCodes::TABLE_ALREADY_EXISTS, "Cannot attach PostgreSQL table {} because it already exists (database: {})",
                    getTableNameForLogs(table_name), database_name);

  if (cache_tables) cached_tables[table_name] = storage;
...
StoragePtr DatabasePostgreSQL::detachTable(ContextPtr /* context_ */, const String &table_name) {
  ...
  if (detached_or_dropped.contains(table_name))
    throw Exception(ErrorCodes::TABLE_IS_DROPPED, "Cannot detach table {}. It is already dropped/detached",
                    getTableNameForLogs(table_name));

  if (!checkPostgresTable(table_name))
    throw Exception(ErrorCodes::UNKNOWN_TABLE, "Cannot detach table {}, because it does not exist", getTableNameForLogs(table_name));

  if (cache_tables) cached_tables.erase(table_name);
...
void DatabasePostgreSQL::createTable(ContextPtr local_context, const String &table_name, const StoragePtr &storage,
                                     const ASTPtr &create_query) {
  const auto &create = create_query->as<ASTCreateQuery>();

  if (!create->attach) throw Exception(ErrorCodes::NOT_IMPLEMENTED, "PostgreSQL database engine does not support create table");
...
void DatabasePostgreSQL::dropTable(ContextPtr, const String &table_name, bool /* sync */) {
  if (!persistent) throw Exception(ErrorCodes::NOT_IMPLEMENTED, "DROP TABLE is not supported for non-persistent MySQL database");
...
void DatabasePostgreSQL::shutdown() { cleaner_task->deactivate(); }
...
ASTPtr DatabasePostgreSQL::getCreateDatabaseQueryImpl() const {
  ...
  if (!comment.empty()) create_query->set(create_query->comment, make_intrusive<ASTLiteral>(comment));
...
ASTPtr DatabasePostgreSQL::getCreateTableQueryImpl(const String &table_name, ContextPtr local_context, bool throw_on_error) const {
  ...
  if (!storage) {
    if (throw_on_error) throw Exception(ErrorCodes::UNKNOWN_TABLE, "PostgreSQL table {} does not exist", getTableNameForLogs(table_name));
  ...
  if (typeid_cast<ASTIdentifier *>(storage_engine_arguments->children[0].get())) {
    storage_engine_arguments->children.push_back(
        makeASTOperator("equals", make_intrusive<ASTIdentifier>("table"), make_intrusive<ASTLiteral>(table_id.table_name)));
  } else {
    /// Remove extra engine argument (`schema` and `use_table_cache`)
    if (storage_engine_arguments->children.size() >= 5) storage_engine_arguments->children.resize(4);

    /// Add table_name to engine arguments.
    if (storage_engine_arguments->children.size() >= 2)
      storage_engine_arguments->children.insert(storage_engine_arguments->children.begin() + 2,
...
ASTPtr DatabasePostgreSQL::getColumnDeclaration(const DataTypePtr &data_type) const {
  ...
  if (which.isDateTime64()) return makeASTDataType("DateTime64", make_intrusive<ASTLiteral>(static_cast<UInt32>(6)));

  if (which.isDecimal())
    return makeASTDataType("Decimal", make_intrusive<ASTLiteral>(getDecimalPrecision(*data_type)),
                           make_intrusive<ASTLiteral>(getDecimalScale(*data_type)));
...
void registerDatabasePostgreSQL(DatabaseFactory &factory) {
  auto create_fn = [](const DatabaseFactory::Arguments &args) {
    ...
    if (!engine->arguments) throw Exception(ErrorCodes::BAD_ARGUMENTS, "Engine `PostgreSQL` must have arguments");
    ...
    if (auto named_collection = tryGetNamedCollectionWithOverrides(engine_args, args.context)) {
      ...
      if (!is_deprecated_syntax && engine_args.size() >= 6) use_table_cache = safeGetLiteralValue<UInt8>(engine_args[5], engine_name);
    }
    ...
    return std::make_shared<DatabasePostgreSQL>(args.context, args.metadata_path, engine_define, args.database_name, configuration, pool,
                                                use_table_cache, args.uuid);
  };
