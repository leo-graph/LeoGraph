...
class DDLDependencyVisitorData {
 ...
 public:
  DDLDependencyVisitorData(const ContextPtr &global_context_, const QualifiedTableName &table_name_, const ASTPtr &ast_,
                           const String &current_database_, bool can_throw_, bool validate_current_database_)
      : create_query(ast_),
        table_name(table_name_),
        default_database(global_context_->getCurrentDatabase()),
        current_database(current_database_),
        global_context(global_context_),
        can_throw(can_throw_),
        validate_current_database(validate_current_database_) {}
  ...
  std::optional<StorageID> getMvToDependency() { return mv_to_dependency; }
  std::optional<StorageID> getMvFromDependency() { return mv_from_dependency; }
 ...
 private:
  ...
  void visitCreateQuery(const ASTCreateQuery &create) {
      ...
      for (const auto &target : create.targets->targets) {
        if (target.kind == ViewTarget::Kind::To) {
          ...
          if (!table_id.table_name.empty()) {
            ...
            if (mv_to_dependency->database_name.empty()) mv_to_dependency->database_name = current_database;
        ...
        if (mv_to_dependency && mv_to_dependency->database_name.empty()) mv_to_dependency->database_name = current_database;
      }
    }
    ...
    if (!as_table.table.empty()) {
      /// AS table_name
      if (as_table.database.empty()) as_table.database = current_database;
    ...
    if (create.select) {
      if (create.isView()) {
        if (create.is_materialized_view) {
          ...
          auto select_query = SelectQueryDescription::getSelectQueryFromASTForMatView(
              select_copy, create.refresh_strategy != nullptr /*refresheable*/, mv_db_context);
  ...
  void visitDictionaryDef(const ASTDictionary &dictionary) {
    if (!dictionary.source || dictionary.source->name != "clickhouse" || !dictionary.source->elements) return;
    ...
    if (!info->table_name.table.empty()) {
      ...
      if (info->table_name.database.empty()) info->table_name.database = table_name.database;
  ...
  void visitTableExpression(const ASTTableExpression &expr) {
    if (!expr.database_and_table_name) return;
    ...
    if (qualified_name.table.empty()) return;
  ...
  void visitTableEngine(const ASTFunction &table_engine) {
    ...
    if (table_engine.name == "Dictionary") addQualifiedNameFromArgument(table_engine, 0);

    /// Buffer('db_name', 'dest_table_name')
    if (table_engine.name == "Buffer") addDatabaseAndTableNameFromArguments(table_engine, 0, 1);

    /// Distributed(cluster_name, db_name, table_name, ...)
    if (table_engine.name == "Distributed") visitDistributedTableEngine(table_engine);
  ...
  void visitDistributedTableEngine(const ASTFunction &table_engine) {
    ...
    if (auto cluster_name = tryGetClusterNameFromArgument(table_engine, 0)) {
      ...
      if (cluster && cluster->getLocalShardCount()) has_local_replicas = true;
    }

    if (has_local_replicas) addDatabaseAndTableNameFromArguments(table_engine, 1, 2);
  }
  ...
  void visitRemoteFunction(const ASTFunction &function, bool is_cluster_function) {
    ...
    if (is_cluster_function) {
      if (auto cluster_name = tryGetClusterNameFromArgument(function, 0)) {
        if (auto cluster = global_context->tryGetCluster(*cluster_name)) {
          if (cluster->getLocalShardCount()) has_local_replicas = true;
        }
      }
    } else {
    ...
    if (has_local_replicas && !table_function) {
      ...
      if (!maybe_qualified_name) return;
  ...
  std::optional<String> tryGetStringFromArgument(const ASTFunction &function, size_t arg_idx, bool evaluate = true) const {
    ...
    if (arg_idx >= args.size()) return {};

    const auto &arg = args[arg_idx];

    if (const auto *id = arg->as<ASTIdentifier>()) return id->name();

    if (const auto *literal = arg->as<ASTLiteral>()) {
      if (literal->value.getType() == Field::Types::String) return literal->value.safeGet<String>();
    }
  ...
  std::optional<QualifiedTableName> tryGetQualifiedNameFromArgument(const ASTFunction &function, size_t arg_idx, bool evaluate = true,
                                                                    bool apply_current_database = true) const {
    if (!function.arguments) return {};
    ...
    if (const auto *identifier = dynamic_cast<const ASTIdentifier *>(arg.get())) {
      ...
      if (!qualified_name_as_string) return {};
    ...
    if (qualified_name.database.empty() && apply_current_database) qualified_name.database = current_database;
  ...
  void addQualifiedNameFromArgumentUsingTableDatabase(const ASTFunction &function, size_t arg_idx, bool evaluate = true) {
    if (auto qualified_name = tryGetQualifiedNameFromArgument(function, arg_idx, evaluate, /* apply_current_database= */ false)) {
      if (qualified_name->database.empty()) qualified_name->database = table_name.database;
      dependencies.emplace(std::move(qualified_name).value());
    }
  }

  /// Returns a database name and a table name extracted from two separate arguments.
  std::optional<QualifiedTableName> tryGetDatabaseAndTableNameFromArguments(const ASTFunction &function, size_t database_arg_idx,
  ...
  std::optional<String> tryGetClusterNameFromArgument(const ASTFunction &function, size_t arg_idx) const {
    ...
    if (cluster_name) return cluster_name;
...
void tryVisitNestedSelect(const String &query, DDLDependencyVisitorData &data) {
  try {
    ...
    String description = fmt::format("Query for ClickHouse dictionary {}.{}", backQuoteIfNeed(data.table_name.database),
                                     backQuoteIfNeed(data.table_name.table));
    String fixed_query = removeWhereConditionPlaceholder(query);
    const Settings &settings = data.global_context->getSettingsRef();
    ASTPtr select = parseQuery(parser, fixed_query, description, settings[Setting::max_query_size], settings[Setting::max_parser_depth],
...
CreateQueryDependencies getDependenciesFromCreateQuery(const ContextPtr &global_global_context, const QualifiedTableName &table_name,
                                                       const ASTPtr &ast, const String &current_database, bool can_throw,
                                                       bool validate_current_database) {
...
TableNamesSet getDependenciesFromDictionaryNestedSelectQuery(const ContextPtr &global_context, const QualifiedTableName &table_name,
                                                             const ASTPtr &ast, const String &select_query, const String &current_database,
                                                             bool can_throw) {
