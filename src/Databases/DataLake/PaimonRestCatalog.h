...
struct PaimonToken {
  ...
  explicit PaimonToken(const String &bearer_token_) : token_provider("bearer"), bearer_token(bearer_token_) {}

  PaimonToken(const String &access_key_id_, const String &access_key_secret_)
      : token_provider("dlf"), dlf_access_key_id(access_key_id_), dlf_access_key_secret(access_key_secret_) {}
};

class PaimonRestCatalog final : public ICatalog, private DB::WithContext {
 public:
  explicit PaimonRestCatalog(const String &warehouse_, const String &base_url_, const PaimonToken &token_, const String &region,
 ...
 private:
  ...
  // void listDatabases();
  void createAuthHeaders(DB::HTTPHeaderEntries &current_headers, const String &resource_path,
                         const std::unordered_map<String, String> &query_params, const String &method,
                         const std::optional<String> &data = std::nullopt) const;

  DB::ReadWriteBufferFromHTTPPtr createReadBuffer(const String &endpoint, const String &method,
  ...
  Poco::JSON::Object::Ptr requestRest(const String &endpoint, const String &method, const Poco::URI::QueryParameters &params = {},
                                      const DB::HTTPHeaderEntries &headers = {}) const;
};
}  // namespace DataLake
#endif
