...
std::pair<Poco::Dynamic::Var, std::string> UnityCatalog::getJSONRequest(const std::string &route,
...
std::pair<Poco::Dynamic::Var, std::string> UnityCatalog::postJSONRequest(const std::string &route,
                                                                         std::function<void(std::ostream &)> out_stream_callaback) const {
  const auto &context = getContext();
  return makeHTTPRequestAndReadJSON(base_url / route, context, credentials, {}, {auth_header}, Poco::Net::HTTPRequest::HTTP_POST,
                                    out_stream_callaback);
}

bool UnityCatalog::empty() const {
  auto all_schemas = getSchemas("");
  for (const auto &schema : all_schemas) {
    if (!getTablesForSchema(schema, 1).empty()) return false;
  }
...
void UnityCatalog::getTableMetadata(const std::string &namespace_name, const std::string &table_name, TableMetadata &result) const {
...
bool UnityCatalog::tryGetTableMetadata(const std::string &schema_name, const std::string &table_name, TableMetadata &result) const {
  ...
  try {
    ...
    if (hasValueAndItsNotNone("name", object) && object->get("name").extract<String>() == table_name) {
      ...
      if (result.requiresLocation()) {
        if (hasValueAndItsNotNone("storage_location", object)) {
          ...
          result.setTableIsNotReadable(
              fmt::format("Cannot read table `{}` because it doesn't have storage location. "
      ...
      if (has_securable_kind && !READABLE_TABLES.contains(object->get("securable_kind").extract<String>())) {
        result.setTableIsNotReadable(
            fmt::format("Cannot read table `{}` because it has unsupported securable_kind: '{}'. "
      ...
      if (has_data_source_format && object->get("data_source_format").extract<String>() != READABLE_DATA_SOURCE_FORMAT) {
        result.setTableIsNotReadable(fmt::format(
            "Cannot read table `{}` because it has unsupported data_source_format '{}'. "
      ...
      if (!has_data_source_format && !has_securable_kind) {
        result.setTableIsNotReadable(
            fmt::format("Cannot read table `{}` because it has no information about data_source_format or securable_kind. "
                        "It means that it's unreadable with Unity catalog in ClickHouse",
                        full_table_name));
      }
      ...
      if (result.requiresSchema()) {
        ...
        try {
          ...
          /// Non-delta tables can have very weird datatypes in schemas like
          /// https://docs.databricks.com/aws/en/sql/language-manual/data-types/null-type We still don't know how to read them so we can
          /// ignore absence of schema and return weird output for SHOW CREATE TABLE.
          if (!result.isDefaultReadableTable()) {
            LOG_DEBUG(log,
                      "Cannot read table `{}` because of schema parsing exception `{}`, but it is not delta table, so we ignore this error",
      ...
      if (result.isDefaultReadableTable() && result.requiresCredentials()) getCredentials(object->get("table_id"), result);
...
bool UnityCatalog::existsTable(const std::string &schema_name, const std::string &table_name) const {
  ...
  try {
    ...
    const Poco::JSON::Object::Ptr &object = json.extract<Poco::JSON::Object::Ptr>();
    if (hasValueAndItsNotNone("name", object) && object->get("name").extract<String>() == table_name) return true;
    return false;
  } catch (DB::Exception &e) {
    e.addMessage("while parsing JSON: " + json_str);
    throw;
...
DB::Names UnityCatalog::getTablesForSchema(const std::string &schema, size_t limit) const {
  ...
  do {
    ...
    try {
      ...
      if (!hasValueAndItsNotNone("tables", object)) return tables;

      auto tables_object = object->get("tables").extract<Poco::JSON::Array::Ptr>();
      if (!tables_object) throw DB::Exception(DB::ErrorCodes::LOGICAL_ERROR, "Cannot parse result");
      ...
      if (hasValueAndItsNotNone("next_page_token", object)) {
        ...
        if (continuation_token.empty()) break;

        if (params.size() == 4) params.pop_back();
...
DataLake::ICatalog::Namespaces UnityCatalog::getSchemas(const std::string &base_prefix, size_t limit) const {
  ...
  do {
    ...
    try {
      ...
      if (!schemas_object) throw DB::Exception(DB::ErrorCodes::LOGICAL_ERROR, "Cannot parse result");
      ...
      for (size_t i = 0; i < schemas_object->size(); ++i) {
        ...
        if (schema_name.schema_name.starts_with(base_prefix)) schemas.push_back(schema_name.schema_name);

        if (limit && schemas.size() > limit) break;
      }
      ...
      if (hasValueAndItsNotNone("next_page_token", object)) {
        ...
        if (params.size() == 2) params.pop_back();
...
UnityCatalog::UnityCatalog(const std::string &catalog_, const std::string &base_url_, const std::string &catalog_credential_,
