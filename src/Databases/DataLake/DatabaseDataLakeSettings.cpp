...
#define DATABASE_ICEBERG_RELATED_SETTINGS(DECLARE, ALIAS)                                                                                  \
  ...
  DECLARE(String, google_project_id, "",                                                                                                   \
          "Google Cloud project ID for BigLake. Required for BigLake catalog. Used in x-goog-user-project header. If not set and "         \
          "google_adc_quota_project_id is provided, it latter will be used",                                                               \
          0)                                                                                                                               \
  DECLARE(String, google_service_account, "",                                                                                              \
          "Google Cloud service account email for metadata service authentication. Default: 'default'. Only used when ADC credentials "    \
          "are not provided",                                                                                                              \
          0)                                                                                                                               \
  DECLARE(String, google_metadata_service, "",                                                                                             \
          "Google Cloud metadata service endpoint for token retrieval. Default: 'metadata.google.internal'. Only used when ADC "           \
          "credentials are not provided",                                                                                                  \
          0)                                                                                                                               \
  DECLARE(                                                                                                                                 \
      String, google_adc_client_id, "",                                                                                                    \
      "Google Application Default Credentials client_id for BigLake. Required if using ADC authentication instead of metadata service", 0) \
  DECLARE(String, google_adc_client_secret, "",                                                                                            \
          "Google Application Default Credentials client_secret for BigLake. Required if using ADC authentication instead of metadata "    \
          "service",                                                                                                                       \
          0)                                                                                                                               \
  DECLARE(String, google_adc_refresh_token, "",                                                                                            \
          "Google Application Default Credentials refresh_token for BigLake. Required if using ADC authentication instead of metadata "    \
          "service",                                                                                                                       \
          0)                                                                                                                               \
  DECLARE(String, google_adc_quota_project_id, "",                                                                                         \
          "Google Application Default Credentials quota_project_id for BigLake. Optional, used if google_project_id is not set", 0)        \
  DECLARE(String, google_adc_credentials_file, "",                                                                                         \
          "Path to JSON file containing Google Application Default Credentials. If set, credentials will be loaded from this file. File "  \
          "should contain: type, client_id, client_secret, refresh_token, and optionally quota_project_id",                                \
          0)                                                                                                                               \
...
void DatabaseDataLakeSettings::applyChanges(const SettingsChanges& changes) { impl->applyChanges(changes); }
    ...
    try {
      for (const auto& change : storage_def.settings->changes) {
        if (!is_attach && change.name.starts_with("iceberg_"))
          throw Exception(ErrorCodes::BAD_ARGUMENTS, "The setting {} is used for storage. Please use the setting without `iceberg_` prefix",
                          change.name);
      }
      impl->applyChanges(storage_def.settings->changes);
    } catch (Exception& e) {
      if (e.code() == ErrorCodes::UNKNOWN_SETTING) e.addMessage("for database engine " + storage_def.engine->name);
      throw;
    }
  }
}
...
SettingsChanges DatabaseDataLakeSettings::allChanged() const {
  ...
  for (const auto& setting : impl->allChanged()) changes.emplace_back(setting.getName(), setting.getValue());
  return changes;
}
