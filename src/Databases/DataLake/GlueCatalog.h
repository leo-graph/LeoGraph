...
class GlueCatalog final : public ICatalog, private DB::WithContext {
 public:
  GlueCatalog(const String& endpoint, DB::ContextPtr context_, const CatalogSettings& settings_, DB::ASTPtr table_engine_definition_);
  ...
  void getTableMetadata(const std::string& database_name, const std::string& table_name, TableMetadata& result) const override;

  bool tryGetTableMetadata(const std::string& database_name, const std::string& table_name, TableMetadata& result) const override;
  ...
  DB::DatabaseDataLakeCatalogType getCatalogType() const override { return DB::DatabaseDataLakeCatalogType::GLUE; }
 ...
 private:
  ...
  /// The Glue catalog does not store detailed information about the types of timestamp columns, such as whether the column is timestamp or
  /// timestamptz. This method allows to clarify the actual type of the timestamp column.
