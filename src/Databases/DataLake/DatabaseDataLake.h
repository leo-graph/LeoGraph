...
class DatabaseDataLake final : public IDatabase, WithContext {
 public:
  explicit DatabaseDataLake(const std::string &database_name_, const std::string &url_, const DatabaseDataLakeSettings &settings_,
  ...
  /// skip_not_loaded flag ignores all non-iceberg tables
  std::vector<LightWeightTableDetails> getLightweightTablesIterator(ContextPtr context, const FilterByNameFunction &filter_by_table_name,
                                                                    bool skip_not_loaded) const override;
  ...
  void dropTable(  /// NOLINT
      ContextPtr context_, const String &name, bool /*sync*/) override;
 ...
 private:
  ...
  std::shared_ptr<StorageObjectStorageConfiguration> getConfiguration(DatabaseDataLakeStorageType type,
