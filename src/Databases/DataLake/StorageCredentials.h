...
class S3Credentials final : public IStorageCredentials {
 public:
  ...
  void addCredentialsToEngineArgs(DB::ASTs& engine_args) const override {
    if (engine_args.size() != 1) throw DB::Exception(DB::ErrorCodes::BAD_ARGUMENTS, "Storage credentials specified in AST already");
    ...
    if (!session_token.empty()) engine_args.push_back(DB::make_intrusive<DB::ASTLiteral>(session_token));
  }

  const String& getAccessKeyId() const { return access_key_id; }

  const String& getSecretAccessKey() const { return secret_access_key; }

  const String& getSessionToken() const { return session_token; }
...
class AzureCredentials final : public IStorageCredentials {
 public:
  explicit AzureCredentials(const std::string& sas_token_) : sas_token(sas_token_) {}
