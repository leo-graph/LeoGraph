...
DB::HTTPHeaderEntry parseAuthHeader(const std::string &auth_header) {
  ...
  if (pos == std::string::npos) throw DB::Exception(DB::ErrorCodes::BAD_ARGUMENTS, "Unexpected format of auth header");

  return DB::HTTPHeaderEntry(auth_header.substr(0, pos), auth_header.substr(pos + 1));
}

std::string correctAPIURI(const std::string &uri) {
  if (uri.ends_with("v1")) return uri;
...
std::string RestCatalog::Config::toString() const {
  ...
  if (!prefix.empty()) wb << "prefix: " << prefix.string() << ", ";

  if (!default_base_location.empty()) wb << "default_base_location: " << default_base_location << ", ";

  return wb.str();
}

RestCatalog::RestCatalog(const std::string &warehouse_, const std::string &base_url_, const std::string &catalog_credential_,
                         const std::string &auth_scope_, const std::string &auth_header_, const std::string &oauth_server_uri_,
                         bool oauth_server_use_request_body_, DB::ContextPtr context_)
    : ICatalog(warehouse_),
...
RestCatalog::RestCatalog(const std::string &warehouse_, const std::string &base_url_, const std::string &auth_scope_,
                         const std::string &oauth_server_uri_, bool oauth_server_use_request_body_, DB::ContextPtr context_)
    : ICatalog(warehouse_),
      DB::WithContext(context_),
      base_url(correctAPIURI(base_url_)),
      log(getLogger("RestCatalog(" + warehouse_ + ")")),
...
void RestCatalog::parseCatalogConfigurationSettings(const Poco::JSON::Object::Ptr &object, Config &result) {
  ...
  if (object->has("prefix")) result.prefix = object->get("prefix").extract<String>();

  if (object->has("default-base-location")) result.default_base_location = object->get("default-base-location").extract<String>();
}
...
OneLakeCatalog::OneLakeCatalog(const std::string &warehouse_, const std::string &base_url_, const std::string &onelake_tenant_id,
                               const std::string &onelake_client_id, const std::string &onelake_client_secret,
                               const std::string &auth_scope_, const std::string &oauth_server_uri_, bool oauth_server_use_request_body_,
...
AccessToken RestCatalog::retrieveAccessToken() const {
  ...
  if (oauth_server_uri.empty() && !oauth_server_use_request_body) {
    ...
    body = fmt::format("grant_type=client_credentials&scope={}&client_id={}&client_secret={}", encoded_auth_scope, encoded_client_id,
...
BigLakeCatalog::BigLakeCatalog(const std::string &warehouse_, const std::string &base_url_, const std::string &google_project_id_,
                               const std::string &google_service_account_, const std::string &google_metadata_service_,
                               const std::string &google_adc_client_id_, const std::string &google_adc_client_secret_,
                               const std::string &google_adc_refresh_token_, const std::string &google_adc_quota_project_id_,
                               DB::ContextPtr context_)
...
AccessToken BigLakeCatalog::retrieveGoogleCloudAccessTokenFromRefreshToken() const {
  ...
  if (google_adc_client_id.empty() || google_adc_client_secret.empty() || google_adc_refresh_token.empty()) {
    throw DB::Exception(DB::ErrorCodes::BAD_ARGUMENTS, "Invalid ADC credentials: client_id, client_secret, and refresh_token are required");
  }
  ...
  String body = fmt::format("grant_type=refresh_token&client_id={}&client_secret={}&refresh_token={}", encoded_client_id,
                            encoded_client_secret, encoded_refresh_token);
  ...
  if (!session) throw DB::Exception(DB::ErrorCodes::BAD_ARGUMENTS, "Session for big lake catalog is not initialized");
  ...
  if (response.getStatus() != Poco::Net::HTTPResponse::HTTP_OK) {
    throw DB::Exception(DB::ErrorCodes::BAD_ARGUMENTS, "Failed to request Google Cloud access token using refresh token: {} (status: {})",
                        response.getReason(), static_cast<int>(response.getStatus()));
  }
  ...
  if (token_type != "Bearer") {
    throw DB::Exception(DB::ErrorCodes::BAD_ARGUMENTS, "Unexpected token type in Google Cloud response. Expected Bearer token, got {}",
                        token_type);
  }
...
AccessToken BigLakeCatalog::retrieveGoogleCloudAccessToken() const {
  ...
  if (!session) throw DB::Exception(DB::ErrorCodes::BAD_ARGUMENTS, "Can not create HTTP session");
  ...
  if (response.getStatus() != Poco::Net::HTTPResponse::HTTP_OK) {
    throw DB::Exception(DB::ErrorCodes::BAD_ARGUMENTS, "Failed to request Google Cloud bearer token from metadata service: {} (status: {})",
                        response.getReason(), static_cast<int>(response.getStatus()));
  }
...
std::optional<StorageType> RestCatalog::getStorageType() const {
  if (config.default_base_location.empty()) return std::nullopt;
  return parseStorageTypeFromLocation(config.default_base_location);
}

DB::ReadWriteBufferFromHTTPPtr RestCatalog::createReadBuffer(const std::string &endpoint, const Poco::URI::QueryParameters &params,
  ...
  if (!params.empty()) url.setQueryParameters(params);
  ...
  try {
    ...
    if (update_token_if_expired && (status == Poco::Net::HTTPResponse::HTTPStatus::HTTP_UNAUTHORIZED ||
...
DB::Names RestCatalog::getTables() const {
...

  {
    ...
    auto execute_for_each_namespace = [&](const std::string &current_namespace) {
      runner.enqueueAndKeepTrack([=, &tables, &mutex, this] {
...
void RestCatalog::getNamespacesRecursive(const std::string &base_namespace, Namespaces &result, StopCondition stop_condition,
  ...
  for (const auto &current_namespace : namespaces) {
    ...
    if (stop_condition && stop_condition(current_namespace)) break;

    if (func) func(current_namespace);
...
Poco::URI::QueryParameters RestCatalog::createParentNamespaceParams(const std::string &base_namespace) const {
  ...
  for (const auto &part : parts) {
    ...
    if (!parent_param.empty()) parent_param += static_cast<char>(0x1F);
    parent_param += part;
  }
...
RestCatalog::Namespaces RestCatalog::getNamespaces(const std::string &base_namespace) const {
  Poco::URI::QueryParameters params;
  if (!base_namespace.empty()) params = createParentNamespaceParams(base_namespace);
  ...
  try {
    ...
    std::string message = fmt::format("Received error while fetching list of namespaces from iceberg catalog `{}`. ", warehouse);
    ...
    message += fmt::format("Code: {}, status: {}, message: {}", e.code(), e.getHTTPStatus(), e.displayText());
...
RestCatalog::Namespaces RestCatalog::parseNamespaces(DB::ReadBuffer &buf, const std::string &base_namespace) const {
  ...
  try {
    ...
    if (!namespaces_object) throw DB::Exception(DB::ErrorCodes::LOGICAL_ERROR, "Cannot parse result");
    ...
    for (size_t i = 0; i < namespaces_object->size(); ++i) {
      ...
      if (getCatalogType() == DB::DatabaseDataLakeCatalogType::ICEBERG_BIGLAKE && !base_namespace.empty() &&
          current_namespace == base_namespace) {
        continue;
      }
...
DB::Names RestCatalog::parseTables(DB::ReadBuffer &buf, const std::string &base_namespace, size_t limit) const {
  ...
  try {
    ...
    if (!identifiers_object) throw DB::Exception(DB::ErrorCodes::LOGICAL_ERROR, "Cannot parse result");
    ...
    for (size_t i = 0; i < identifiers_object->size(); ++i) {
      ...
      if (limit && tables.size() >= limit) break;
    }
...
#  endif
  ...
  if (!metadata_object) throw DB::Exception(DB::ErrorCodes::LOGICAL_ERROR, "Cannot parse result");
  ...
  if (result.isDefaultReadableTable() && result.requiresCredentials() && object->has("config")) {
    ...
    if (!config_object) throw DB::Exception(DB::ErrorCodes::LOGICAL_ERROR, "Cannot parse config result");
    auto [parsed_credentials, parsed_endpoint] = getCredentialsAndEndpoint(config_object, location);
    if (parsed_credentials) result.setStorageCredentials(parsed_credentials);
    if (!parsed_endpoint.empty()) result.setEndpoint(parsed_endpoint);
  }
...
void RestCatalog::sendRequest(const String &endpoint, Poco::JSON::Object::Ptr request_body, const String &method,
                              bool ignore_result) const {
  std::ostringstream oss;  // STYLE_CHECK_ALLOW_STD_STRING_STREAM
  if (request_body) request_body->stringify(oss);
...
void RestCatalog::createTable(const String &namespace_name, const String &table_name, const String & /*new_metadata_path*/,
...
bool RestCatalog::updateMetadata(const String &namespace_name, const String &table_name, const String & /*new_metadata_path*/,
...
std::pair<std::shared_ptr<IStorageCredentials>, String> RestCatalog::getCredentialsAndEndpoint(Poco::JSON::Object::Ptr object,
  ...
  switch (storage_type) {
    case StorageType::S3: {
      ...
      if (object->has(access_key_id_str)) access_key_id = object->get(access_key_id_str).extract<String>();
      if (object->has(secret_access_key_str)) secret_access_key = object->get(secret_access_key_str).extract<String>();
      if (object->has(session_token_str)) session_token = object->get(session_token_str).extract<String>();
      if (object->has(storage_endpoint_str)) storage_endpoint = object->get(storage_endpoint_str).extract<String>();
