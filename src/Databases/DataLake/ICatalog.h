...
class TableMetadata {
 public:
  ...
  TableMetadata &withDataLakeSpecificProperties() {
    with_datalake_specific_metadata = true;
    return *this;
  }
...
class ICatalog {
 public:
  ...
  virtual bool existsTable(const std::string &namespace_naem, const std::string &table_name) const = 0;
  ...
  virtual void createTable(const String &namespace_name, const String &table_name, const String &new_metadata_path,
                           Poco::JSON::Object::Ptr metadata_content) const;

  /// Updates metadata in catalog.
  virtual bool updateMetadata(const String &namespace_name, const String &table_name, const String &new_metadata_path,
  ...
  virtual CredentialsRefreshCallback getCredentialsConfigurationCallback(const DB::StorageID & /*storage_id*/) { return std::nullopt; }
