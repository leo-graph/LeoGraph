...
void TableMetadata::setLocation(const std::string& location_) {
  if (!with_location) throw DB::Exception(DB::ErrorCodes::LOGICAL_ERROR, "Data location was not requested");
  ...
  if (pos == std::string::npos) throw DB::Exception(DB::ErrorCodes::NOT_IMPLEMENTED, "Unexpected location format: {}", location_);
  ...
  if (pos_to_path == std::string::npos) throw DB::Exception(DB::ErrorCodes::NOT_IMPLEMENTED, "Unexpected location format: {}", location_);
  ...
  if (at_pos != std::string::npos) {
    ...
    LOG_TEST(getLogger("TableMetadata"), "Parsed Azure location - container: {}, account: {}, path: {}", bucket, azure_account_with_suffix,
             path);
  } else {
    /// Standard format (S3, GCS, etc.)
    bucket = bucket_part;
    LOG_TEST(getLogger("TableMetadata"), "Parsed location without path: {}, path: {}", location_without_path, path);
  }
}
...
std::string TableMetadata::getLocation() const {
  ...
  if (!endpoint.empty()) return constructLocation(endpoint);
...
std::string TableMetadata::getLocationWithEndpoint(const std::string& endpoint_) const {
  ...
  if (endpoint_.empty()) throw DB::Exception(DB::ErrorCodes::LOGICAL_ERROR, "Passed endpoint is empty");
...
std::string TableMetadata::constructLocation(const std::string& endpoint_) const {
  ...
  if (!azure_account_with_suffix.empty()) {
    ...
    if (location.ends_with(bucket)) return std::filesystem::path(location) / path / "";
...
void TableMetadata::setEndpoint(const std::string& endpoint_) { endpoint = endpoint_; }

void TableMetadata::setSchema(const DB::NamesAndTypesList& schema_) {
  if (!with_schema) throw DB::Exception(DB::ErrorCodes::LOGICAL_ERROR, "Data schema was not requested");
...
void TableMetadata::setStorageCredentials(std::shared_ptr<IStorageCredentials> credentials_) {
  if (!with_storage_credentials) throw DB::Exception(DB::ErrorCodes::LOGICAL_ERROR, "Storage credentials were not requested");

  storage_credentials = std::move(credentials_);
}

std::shared_ptr<IStorageCredentials> TableMetadata::getStorageCredentials() const {
  if (!with_storage_credentials) throw DB::Exception(DB::ErrorCodes::LOGICAL_ERROR, "Data schema was not requested");
...
std::optional<DataLakeSpecificProperties> TableMetadata::getDataLakeSpecificProperties() const { return data_lake_specific_metadata; }
...
bool TableMetadata::hasLocation() const { return !location_without_path.empty(); }
bool TableMetadata::hasSchema() const { return !schema.empty(); }
bool TableMetadata::hasStorageCredentials() const { return storage_credentials != nullptr; }
...
std::string TableMetadata::getMetadataLocation(const std::string& iceberg_metadata_file_location) const {
  ...
  if (!metadata_location.empty()) {
    ...
    if (metadata_location.starts_with(storage_type_str)) metadata_location = metadata_location.substr(storage_type_str.size());
...
void ICatalog::createTable(const String& /*namespace_name*/, const String& /*table_name*/, const String& /*new_metadata_path*/,
...
bool ICatalog::updateMetadata(const String& /*namespace_name*/, const String& /*table_name*/, const String& /*new_metadata_path*/,
