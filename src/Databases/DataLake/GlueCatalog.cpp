...
GlueCatalog::GlueCatalog(const String& endpoint, DB::ContextPtr context_, const CatalogSettings& settings_,
  ...
  if (global_settings.isChanged("s3_max_redirects")) s3_max_redirects = static_cast<int>(global_settings[DB::Setting::s3_max_redirects]);

  int s3_retry_attempts = static_cast<int>(server_settings[DB::ServerSetting::s3_retry_attempts]);
  // just for compatibility with old setting
  if (global_settings.isChanged("s3_retry_attempts")) s3_retry_attempts = static_cast<int>(global_settings[DB::Setting::s3_retry_attempts]);
  ...
  DB::S3::PocoHTTPClientConfiguration poco_config = DB::S3::ClientFactory::instance().createClientConfiguration(
      region, getContext()->getRemoteHostFilter(), s3_max_redirects,
      DB::S3::PocoHTTPClientConfiguration::RetryStrategy{.max_retries = static_cast<unsigned>(s3_retry_attempts)},
      s3_slow_all_threads_after_network_error, s3_slow_all_threads_after_retryable_error, enable_s3_requests_logging,
      /* for_disk_s3 = */ false,
      /* opt_disk_name = */ {},
      /* request_throttler = */ {});
  ...
  if (!endpoint.empty()) {
    ...
    if (uri.getScheme() == "http") poco_config.scheme = Aws::Http::Scheme::HTTP;
  } else {
    LOG_TRACE(log, "Creating AWS glue client with credentials empty {}, region '{}', endpoint '{}'", credentials.IsEmpty(), region,
              endpoint);
  }
...
DataLake::ICatalog::Namespaces GlueCatalog::getDatabases(const std::string& prefix, size_t limit) const {
  ...
  if (limit != 0) request.SetMaxResults(static_cast<int>(limit));
  ...
  do {
    ...
    if (outcome.IsSuccess()) {
      ...
      for (const auto& db : dbs) {
        ...
        if (!db_name.starts_with(prefix)) continue;
  ...
  do {
    ...
    if (outcome.IsSuccess()) {
      ...
      for (const auto& table : tables) {
        ...
        if (table.GetStorageDescriptor().GetColumns().empty()) continue;
...
bool GlueCatalog::tryGetTableMetadata(const std::string& database_name, const std::string& table_name, TableMetadata& result) const {
  Aws::Glue::Model::GetTableRequest request;
  request.SetDatabaseName(database_name);
  request.SetName(table_name);
  ...
  if (outcome.IsSuccess()) {
    ...
    if (result.requiresLocation()) result.setLocation(table_outcome.GetStorageDescriptor().GetLocation());

    std::string table_type;
    if (table_outcome.GetParameters().contains("table_type")) table_type = table_outcome.GetParameters().at("table_type");
    ...
    if (Poco::toUpper(table_type) != "ICEBERG") {
      ...
      result.setTableIsNotReadable(fmt::format(
          "Cannot read table `{}` because it has {}. "
    ...
    if (result.requiresCredentials()) setCredentials(result);
    ...
    auto setup_specific_properties = [&] {
      ...
      if (table_params.contains("metadata_location")) {
        result.setDataLakeSpecificProperties(
            DataLakeSpecificProperties{.iceberg_metadata_file_location = table_params.at("metadata_location")});
        ...
        if (!location_with_slash.ends_with('/')) location_with_slash += '/';
        ...
        if (resolved_metadata_path.empty()) {
          result.setTableIsNotReadable(
              fmt::format("Could not determine metadata_location of table `{}`. ", database_name + "." + table_name));
        } else {
          result.setDataLakeSpecificProperties(DataLakeSpecificProperties{.iceberg_metadata_file_location = resolved_metadata_path});
        }
      } else {
        result.setTableIsNotReadable(
            fmt::format("Cannot read table `{}` because it has no metadata_location and Location. "
    ...
    if (result.requiresSchema()) {
      ...
      for (const auto& column : columns) {
        ...
        if (column_type == "timestamp") {
          if (!result.requiresDataLakeSpecificProperties()) setup_specific_properties();
          if (classifyTimestampTZ(column.GetName(), result)) column_type = "timestamptz";
        }
    ...
    if (outcome.GetError().GetErrorType() == Aws::Glue::GlueErrors::ENTITY_NOT_FOUND) return false;  // Table does not exist

    throw DB::Exception(DB::ErrorCodes::DATALAKE_DATABASE_ERROR, "Exception calling GetTable for table {}: {}",
...
void GlueCatalog::getTableMetadata(const std::string& database_name, const std::string& table_name, TableMetadata& result) const {
  if (!tryGetTableMetadata(database_name, table_name, result)) {
    throw DB::Exception(DB::ErrorCodes::DATALAKE_DATABASE_ERROR, "Table {} does not exist in Glue catalog",
                        database_name + "." + table_name);
  }
}
...
void GlueCatalog::setCredentials(TableMetadata& metadata) const {
  ...
  if (storage_type == StorageType::S3) {
    ...
    auto s3_creds =
        std::make_shared<S3Credentials>(credentials.GetAWSAccessKeyId(), credentials.GetAWSSecretKey(), credentials.GetSessionToken());
    metadata.setStorageCredentials(s3_creds);
  } else {
    throw DB::Exception(DB::ErrorCodes::DATALAKE_DATABASE_ERROR, "Glue catalog support S3 backend for data storage only");
  }
}
...
bool GlueCatalog::empty() const {
  ...
  for (const auto& db : all_databases) {
    if (!getTablesForDatabase(db, /* limit = */ 1).empty()) return false;
  }
  return true;
}
...
bool GlueCatalog::classifyTimestampTZ(const String& column_name, const TableMetadata& table_metadata) const {
  ...
  if (!table_specific_properties.has_value())
    throw DB::Exception(DB::ErrorCodes::BAD_ARGUMENTS, "Failed to read table metadata, reason why table is unreadable: {}",
                        table_metadata.getReasonWhyTableIsUnreadable());
...
GlueCatalog::ObjectStorageWithPath GlueCatalog::createObjectStorageForEarlyTableAccess(const String& s3_location,
  ...
  if (args.size() == 1) {
    if (table_metadata.hasStorageCredentials()) {
      ...
      auto credentials = credentials_provider->GetAWSCredentials();
      DataLake::S3Credentials(credentials.GetAWSAccessKeyId(), credentials.GetAWSSecretKey(), credentials.GetSessionToken())
          .addCredentialsToEngineArgs(args);
    }
  }
  ...
  if (table_path.ends_with('/')) table_path = table_path.substr(0, table_path.size() - 1);
...
String GlueCatalog::resolveMetadataPathFromTableLocation(const String& table_location, const TableMetadata& table_metadata) const {
  ...
  try {
    auto [metadata_version, metadata_path, compression_method] = DB::Iceberg::getLatestOrExplicitMetadataFileAndVersion(
        object_storage, table_path, *storage_settings, nullptr, getContext(), log.get(), std::nullopt);
...
void GlueCatalog::createTable(const String& namespace_name, const String& table_name, const String& new_metadata_path,
  ...
  if (!response.IsSuccess())
    throw DB::Exception(DB::ErrorCodes::DATALAKE_DATABASE_ERROR, "Can not create metadata in glue catalog: {}",
                        response.GetError().GetMessage());
}

bool GlueCatalog::updateMetadata(const String& namespace_name, const String& table_name, const String& new_metadata_path,
  ...
  if (!response.IsSuccess())
    throw DB::Exception(DB::ErrorCodes::DATALAKE_DATABASE_ERROR, "Can not update metadata in glue catalog {}",
                        response.GetError().GetMessage());
...
void GlueCatalog::dropTable(const String& namespace_name, const String& table_name) const {
  ...
  if (!response.IsSuccess())
    throw DB::Exception(DB::ErrorCodes::DATALAKE_DATABASE_ERROR, "Can not delete table from glue catalog: {}",
                        response.GetError().GetMessage());
}
