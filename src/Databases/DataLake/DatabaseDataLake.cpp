...
DatabaseDataLake::DatabaseDataLake(const std::string &database_name_, const std::string &url_, const DatabaseDataLakeSettings &settings_,
                                   ASTPtr database_engine_definition_, ASTPtr table_engine_definition_, UUID uuid)
  ...
  if (settings[DatabaseDataLakeSetting::catalog_type].value == DB::DatabaseDataLakeCatalogType::GLUE) {
    if (settings[DatabaseDataLakeSetting::region].value.empty())
      throw Exception(ErrorCodes::BAD_ARGUMENTS,
                      "`region` setting cannot be empty for Glue Catalog. "
...
std::shared_ptr<DataLake::ICatalog> DatabaseDataLake::getCatalog() const {
  ...
  if (catalog_impl) return catalog_impl;
  ...
  switch (settings[DatabaseDataLakeSetting::catalog_type].value) {
    case DB::DatabaseDataLakeCatalogType::ICEBERG_REST: {
      catalog_impl = std::make_shared<DataLake::RestCatalog>(
          settings[DatabaseDataLakeSetting::warehouse].value, url, settings[DatabaseDataLakeSetting::catalog_credential].value,
          settings[DatabaseDataLakeSetting::auth_scope].value, settings[DatabaseDataLakeSetting::auth_header],
          settings[DatabaseDataLakeSetting::oauth_server_uri].value, settings[DatabaseDataLakeSetting::oauth_server_use_request_body].value,
          Context::getGlobalContextInstance());
      break;
    }
    case DB::DatabaseDataLakeCatalogType::ICEBERG_ONELAKE: {
      catalog_impl = std::make_shared<DataLake::OneLakeCatalog>(
          settings[DatabaseDataLakeSetting::warehouse].value, url, settings[DatabaseDataLakeSetting::onelake_tenant_id].value,
          settings[DatabaseDataLakeSetting::onelake_client_id].value, settings[DatabaseDataLakeSetting::onelake_client_secret].value,
          settings[DatabaseDataLakeSetting::auth_scope].value, settings[DatabaseDataLakeSetting::oauth_server_uri].value,
          settings[DatabaseDataLakeSetting::oauth_server_use_request_body].value, Context::getGlobalContextInstance());
      break;
    }
    case DB::DatabaseDataLakeCatalogType::ICEBERG_BIGLAKE: {
      ...
      if (settings[DatabaseDataLakeSetting::google_adc_credentials_file].changed) {
        try {
          ...
          if (object->has("type")) {
            ...
            if (type != "authorized_user") {
              throw DB::Exception(DB::ErrorCodes::BAD_ARGUMENTS,
                                  "Unsupported credentials type '{}' in Google ADC credentials file. Expected 'authorized_user'", type);
            }
          }

          if (google_adc_client_id.empty() && object->has("client_id")) google_adc_client_id = object->get("client_id").extract<String>();
          ...
          if (google_project_id.empty() && object->has("project_id")) google_project_id = object->get("project_id").extract<String>();
        } catch (const DB::Exception &e) {
          throw DB::Exception(DB::ErrorCodes::BAD_ARGUMENTS, "Failed to load Google ADC credentials from file '{}': {}",
                              settings[DatabaseDataLakeSetting::google_adc_credentials_file].value, e.message());
        }
      }

      catalog_impl = std::make_shared<DataLake::BigLakeCatalog>(settings[DatabaseDataLakeSetting::warehouse].value, url, google_project_id,
                                                                google_service_account, google_metadata_service, google_adc_client_id,
                                                                google_adc_client_secret, google_adc_refresh_token,
                                                                google_adc_quota_project_id, Context::getGlobalContextInstance());
      break;
    }
    case DB::DatabaseDataLakeCatalogType::UNITY: {
      catalog_impl = std::make_shared<DataLake::UnityCatalog>(settings[DatabaseDataLakeSetting::warehouse].value, url,
    ...
    case DB::DatabaseDataLakeCatalogType::GLUE: {
      catalog_impl =
          std::make_shared<DataLake::GlueCatalog>(url, Context::getGlobalContextInstance(), catalog_parameters, table_engine_definition);
      break;
    }
    case DB::DatabaseDataLakeCatalogType::ICEBERG_HIVE: {
#  if USE_HIVE
      catalog_impl = std::make_shared<DataLake::HiveCatalog>(settings[DatabaseDataLakeSetting::warehouse].value, url,
                                                             Context::getGlobalContextInstance());
      break;
#  else
      throw Exception(ErrorCodes::BAD_ARGUMENTS,
                      "Cannot use 'hive' database engine: ClickHouse was compiled without USE_HIVE built option");
#  endif
    }
    ...
    case DB::DatabaseDataLakeCatalogType::PAIMON_REST: {
      if (!settings[DatabaseDataLakeSetting::catalog_credential].value.empty()) {
        catalog_impl = std::make_shared<DataLake::PaimonRestCatalog>(
            settings[DatabaseDataLakeSetting::warehouse].value, url,
            DataLake::PaimonToken(settings[DatabaseDataLakeSetting::catalog_credential].value),
            settings[DatabaseDataLakeSetting::region].value, Context::getGlobalContextInstance());
        ...
        catalog_impl = std::make_shared<DataLake::PaimonRestCatalog>(
            settings[DatabaseDataLakeSetting::warehouse].value, url,
            DataLake::PaimonToken(settings[DatabaseDataLakeSetting::dlf_access_key_id].value,
                                  settings[DatabaseDataLakeSetting::dlf_access_key_secret].value),
            settings[DatabaseDataLakeSetting::region].value, Context::getGlobalContextInstance());
      } else {
        throw Exception(ErrorCodes::BAD_ARGUMENTS,
                        "Paimon catalog requires either catalog_credential or (dlf_access_key_id, dlf_access_key_secret and region)");
      }
      break;
    }
  }
...
std::shared_ptr<StorageObjectStorageConfiguration> DatabaseDataLake::getConfiguration(DatabaseDataLakeStorageType type,
...
#  endif
        default:
          throw Exception(ErrorCodes::BAD_ARGUMENTS, "Server does not contain support for storage type {} for Iceberg OneLake catalog",
                          type);
      }
    }
...
#  if !USE_AWS_S3 || !USE_AZURE_BLOB_STORAGE || !USE_HDFS
        default:
          throw Exception(ErrorCodes::BAD_ARGUMENTS, "Server does not contain support for storage type {} for Iceberg Rest catalog", type);
#  endif
      }
    }
...
#  endif
        ...
        default:
          throw Exception(ErrorCodes::BAD_ARGUMENTS, "Server does not contain support for storage type {} for Unity catalog", type);
      }
    }
...
#  endif
        ...
        default:
          throw Exception(ErrorCodes::BAD_ARGUMENTS, "Server does not contain support for storage type {} for Glue catalog", type);
      }
    }
...
std::string DatabaseDataLake::getStorageEndpointForTable(const DataLake::TableMetadata &table_metadata) const {
  ...
  if (endpoint_from_settings.empty()) return table_metadata.getLocation();
  return table_metadata.getLocationWithEndpoint(endpoint_from_settings);
}

bool DatabaseDataLake::empty() const { return getCatalog()->empty(); }
...
StoragePtr DatabaseDataLake::tryGetTableImpl(const String &name, ContextPtr context_, bool lightweight, bool ignore_if_not_iceberg) const {
  ...
  fiu_do_on(FailPoints::lightweight_show_tables, { std::this_thread::sleep_for(std::chrono::seconds(10)); });

  fiu_do_on(FailPoints::datalake_try_get_table_return_nullptr, { return nullptr; });

  const bool with_vended_credentials = settings[DatabaseDataLakeSetting::vended_credentials].value;
  if (with_vended_credentials) table_metadata = table_metadata.withStorageCredentials();

  auto [namespace_name, table_name] = DataLake::parseTableName(name);

  if (!catalog->tryGetTableMetadata(namespace_name, table_name, table_metadata)) return nullptr;
  if (ignore_if_not_iceberg && !table_metadata.isDefaultReadableTable()) return nullptr;
  ...
  if (table_metadata.hasLocation()) {
    ...
    if (table_endpoint.starts_with(DataLake::FILE_PATH_PREFIX)) table_endpoint = table_endpoint.substr(DataLake::FILE_PATH_PREFIX.length());
    if (args.empty())
  ...
  if (args.size() == 1) {
    std::array<DatabaseDataLakeCatalogType, 3> vended_credentials_catalogs = {DatabaseDataLakeCatalogType::ICEBERG_ONELAKE,
                                                                              DatabaseDataLakeCatalogType::ICEBERG_BIGLAKE,
                                                                              DatabaseDataLakeCatalogType::PAIMON_REST};
    if (table_metadata.hasStorageCredentials()) {
      ...
      if (storage_credentials) {
        ...
        LOG_DEBUG(log, "Has no credentials");
               ...
               std::find(vended_credentials_catalogs.begin(), vended_credentials_catalogs.end(), catalog->getCatalogType()) ==
                   vended_credentials_catalogs.end()) {
      throw Exception(ErrorCodes::BAD_ARGUMENTS,
  ...
  if (storage_type_from_catalog.has_value()) {
    ...
    if (table_metadata.hasLocation() || !lightweight) storage_type = table_metadata.getStorageType();
  }

  auto storage_settings = std::make_shared<DataLakeStorageSettings>();
  storage_settings->loadFromSettingsChanges(settings.allChanged());

  if (auto table_specific_properties = table_metadata.getDataLakeSpecificProperties(); table_specific_properties.has_value()) {
...
#  if USE_AZURE_BLOB_STORAGE
    ...
    if (!azure_configuration) throw Exception(ErrorCodes::LOGICAL_ERROR, "Configuration is not azure type for one lake catalog");
    auto rest_catalog = std::static_pointer_cast<DataLake::OneLakeCatalog>(catalog);
    if (!rest_catalog) throw Exception(ErrorCodes::LOGICAL_ERROR, "Catalog is not equals to one lake");
    azure_configuration->setInitializationAsOneLake(rest_catalog->getClientId(), rest_catalog->getClientSecret(),
...
#  endif
  ...
  bool can_use_distributed_iterator = context_->getClientInfo().collaborate_with_initiator && can_use_parallel_replicas;

  return std::make_shared<StorageObjectStorage>(
      configuration,
      configuration->createObjectStorage(context_copy, /* is_readonly */ false,
                                         catalog->getCredentialsConfigurationCallback(StorageID(getDatabaseName(), name))),
      context_copy, StorageID(getDatabaseName(), name),
      /* columns */ columns,
      /* constraints */ ConstraintsDescription{},
      /* comment */ "", getFormatSettings(context_copy), LoadingStrictnessLevel::CREATE, getCatalog(),
      /* if_not_exists*/ true,
...
void DatabaseDataLake::dropTable(  /// NOLINT
    ContextPtr context_, const String &name, bool /*sync*/) {
...
DatabaseTablesIteratorPtr DatabaseDataLake::getTablesIterator(ContextPtr context_, const FilterByNameFunction &filter_by_table_name,
  ...
  for (const auto &table_name : iceberg_tables) {
    if (filter_by_table_name && !filter_by_table_name(table_name)) continue;

    try {
      promises.emplace_back(std::make_shared<std::promise<StoragePtr>>());
      futures.emplace_back(promises.back()->get_future());

      pool.scheduleOrThrow([this, table_name, skip_not_loaded, context_, promise = promises.back()]() mutable {
        ...
        try {
          ...
          if (context_->getSettingsRef()[Setting::database_datalake_require_metadata_access]) {
            ...
            auto enhanced_message = fmt::format(
                ...
                error_code, table_name, error_message);
            promise->set_exception(std::make_exception_ptr(Exception::createRuntime(error_code, enhanced_message)));
            return;
          } else
...
std::vector<LightWeightTableDetails> DatabaseDataLake::getLightweightTablesIterator(ContextPtr /*context_*/,
...
ASTPtr DatabaseDataLake::getCreateTableQueryImpl(const String &name, ContextPtr /* context_ */, bool throw_on_error) const {
  auto catalog = getCatalog();
  ...
  if (!catalog->tryGetTableMetadata(namespace_name, table_name, table_metadata)) {
    if (throw_on_error) throw Exception(ErrorCodes::CANNOT_GET_CREATE_TABLE_QUERY, "Table `{}` doesn't exist", name);
    return {};
  }
  ...
  if (!table_metadata.isDefaultReadableTable()) storage->engine->name = DataLake::FAKE_TABLE_ENGINE_NAME_FOR_UNREADABLE_TABLES;
  ...
  if (table_metadata.isDefaultReadableTable()) {
    ...
    LOG_DEBUG(log, "Table endpoint {}", table_endpoint);
    if (storage_engine_arguments->children.empty()) storage_engine_arguments->children.emplace_back();
...
void registerDatabaseDataLake(DatabaseFactory &factory) {
  auto create_fn = [](const DatabaseFactory::Arguments &args) {
    ...
    if (database_engine_define->settings) database_settings.loadFromQuery(*database_engine_define, args.create_query.attach);
    ...
    for (auto &engine_arg : engine_args) engine_arg = evaluateConstantExpressionOrIdentifierAsLiteral(engine_arg, args.context);

    std::string url;
    if (!engine_args.empty()) url = engine_args[0]->as<ASTLiteral>()->value.safeGet<String>();
    ...
    switch (catalog_type) {
      ...
      case DatabaseDataLakeCatalogType::ICEBERG_REST:
      case DatabaseDataLakeCatalogType::ICEBERG_BIGLAKE: {
        if (!args.create_query.attach && !args.context->getSettingsRef()[Setting::allow_experimental_database_iceberg]) {
      ...
      case DatabaseDataLakeCatalogType::GLUE: {
        if (!args.create_query.attach && !args.context->getSettingsRef()[Setting::allow_experimental_database_glue_catalog]) {
          throw Exception(ErrorCodes::SUPPORT_IS_DISABLED,
      ...
      case DatabaseDataLakeCatalogType::UNITY: {
        if (!args.create_query.attach && !args.context->getSettingsRef()[Setting::allow_experimental_database_unity_catalog]) {
          throw Exception(ErrorCodes::SUPPORT_IS_DISABLED,
      ...
      case DatabaseDataLakeCatalogType::ICEBERG_HIVE: {
        if (!args.create_query.attach && !args.context->getSettingsRef()[Setting::allow_experimental_database_hms_catalog]) {
          throw Exception(ErrorCodes::SUPPORT_IS_DISABLED,
      ...
      case DatabaseDataLakeCatalogType::PAIMON_REST: {
        if (!args.create_query.attach && !args.context->getSettingsRef()[Setting::allow_experimental_database_paimon_rest_catalog]) {
          throw Exception(ErrorCodes::SUPPORT_IS_DISABLED,
    ...
    return std::make_shared<DatabaseDataLake>(args.database_name, url, database_settings, database_engine_define->clone(),
                                              std::move(engine_for_tables), args.uuid);
  };
