...
std::pair<String, Int32> testParseHostPort(const String& url) {
  ...
  if (colon_pos == String::npos) throw DB::Exception(DB::ErrorCodes::DATALAKE_DATABASE_ERROR, "Invalid URL format: missing port number");
...
TEST_F(HiveCatalogUrlParsingTest, MissingProtocolSeparator) {
  EXPECT_THROW({ testParseHostPort("thrift:hive-metastore:9083"); }, DB::Exception);

  EXPECT_THROW({ testParseHostPort("hive-metastore:9083"); }, DB::Exception);
}

TEST_F(HiveCatalogUrlParsingTest, MissingPort) {
  EXPECT_THROW({ testParseHostPort("thrift://hive-metastore"); }, DB::Exception);

  EXPECT_THROW({ testParseHostPort("thrift://hive-metastore/"); }, DB::Exception);
}

TEST_F(HiveCatalogUrlParsingTest, InvalidPortFormats) {
  EXPECT_THROW({ testParseHostPort("thrift://hive-metastore:abc"); }, DB::Exception);

  EXPECT_THROW({ testParseHostPort("thrift://hive-metastore:"); }, DB::Exception);

  EXPECT_THROW({ testParseHostPort("thrift://hive-metastore:123abc"); }, DB::Exception);
}

TEST_F(HiveCatalogUrlParsingTest, PortRangeValidation) {
  EXPECT_THROW({ testParseHostPort("thrift://hive-metastore:0"); }, DB::Exception);
  ...
  EXPECT_THROW({ testParseHostPort("thrift://hive-metastore:65536"); }, DB::Exception);

  EXPECT_THROW({ testParseHostPort("thrift://hive-metastore:999999"); }, DB::Exception);
}

TEST_F(HiveCatalogUrlParsingTest, EdgeCases) {
  EXPECT_THROW({ testParseHostPort(""); }, DB::Exception);

  EXPECT_THROW({ testParseHostPort("thrift://"); }, DB::Exception);
