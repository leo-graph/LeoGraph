...
TEST_F(AzureAbfssParsingTest, TableMetadataSetLocationInvalidFormat) {
  ...
  EXPECT_THROW({ metadata.setLocation("invalid-location-without-protocol"); }, DB::Exception);
}
...
TEST_F(AzureAbfssParsingTest, TableMetadataSetLocationMissingPath) {
  ...
  EXPECT_THROW({ metadata.setLocation("abfss://container@account.dfs.core.windows.net"); }, DB::Exception);
}
