...
DB::DataTypePtr getType(const String& type_name, bool nullable, const String& prefix) {
  ...
  if (name.starts_with("map<") && name.ends_with(">")) {
    ...
    if (args.size() != 2) throw DB::Exception(DB::ErrorCodes::DATALAKE_DATABASE_ERROR, "Invalid data type {}", type_name);
  ...
  if (name.starts_with("struct<") && name.ends_with(">")) {
    ...
    for (const auto& arg : args) {
      ...
      if (colon == String::npos) throw DB::Exception(DB::ErrorCodes::DATALAKE_DATABASE_ERROR, "Invalid data type {}", type_name);
...
std::pair<std::string, std::string> parseTableName(const std::string& name) {
  auto pos = name.rfind('.');
  if (pos == std::string::npos) throw DB::Exception(DB::ErrorCodes::BAD_ARGUMENTS, "Table cannot have empty namespace: {}", name);
