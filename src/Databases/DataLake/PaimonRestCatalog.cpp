...
const std::vector<String> PaimonRestCatalog::SIGNED_HEADERS = {
    Poco::toLower(String(DLF_CONTENT_MD5_HEADER_KEY)),   Poco::toLower(String(DLF_CONTENT_TYPE_KEY)),
    Poco::toLower(String(DLF_CONTENT_SHA56_HEADER_KEY)), Poco::toLower(String(DLF_DATE_HEADER_KEY)),
    Poco::toLower(String(DLF_AUTH_VERSION_HEADER_KEY)),  Poco::toLower(String(DLF_SECURITY_TOKEN_HEADER_KEY))};

PaimonRestCatalog::PaimonRestCatalog(const String &warehouse_, const String &base_url_, const PaimonToken &token_, const String &region_,
...
void PaimonRestCatalog::loadConfig() {
  ...
  auto set_config = [this](const Poco::JSON::Object::Ptr &object_) {
    if (!object_) return;
    if (object_->has("prefix")) this->prefix = object_->get("prefix").extract<String>();
  ...
  if (prefix.empty()) throw DB::Exception(DB::ErrorCodes::BAD_ARGUMENTS, "Could not found prefix in catalog configuration.");
  if (warehouse.empty()) {
...
void PaimonRestCatalog::createAuthHeaders(DB::HTTPHeaderEntries &current_headers, const String &resource_path,
                                          const std::unordered_map<String, String> &query_params, const String &method,
  ...
  if (token->token_provider == "bearer") {
    ...
    auto get_authorization = [&headers_map, &query_params, &resource_path, &method, get_or_default, this](String date,
      ...
      String string_to_sign =
          fmt::to_string(fmt::join({String(SIGNATURE_ALGORITHM), date_time, fmt::format("{}/{}/{}/{}", date, region, PRODUCT, REQUEST_TYPE),
...
DB::ReadWriteBufferFromHTTPPtr PaimonRestCatalog::createReadBuffer(const String &endpoint, const String &method,
...
void PaimonRestCatalog::forEachDatabase(DB::Strings &databases, StopCondition stop_condition, ExecuteFunc execute_func) const {
  auto json_ptr = requestRest(std::filesystem::path(API_VERSION) / prefix / DATABASES_ENDPOINT, "GET",
...
void PaimonRestCatalog::forEachTables(const String &database, DB::Names &tables, StopCondition stop_condition,
                                      ExecuteFunc execute_func) const {
  auto json_ptr = requestRest(std::filesystem::path(API_VERSION) / prefix / DATABASES_ENDPOINT / database / TABLES_ENDPOINT, "GET",
                              {{"maxResults", fmt::to_string(LIST_MAX_RESULTS)}});
...
bool PaimonRestCatalog::existsTable(const String &database_name, const String &table_name) const {
  try {
    createReadBuffer(std::filesystem::path(API_VERSION) / prefix / DATABASES_ENDPOINT / database_name / TABLES_ENDPOINT / table_name,
                     "GET");
...
bool PaimonRestCatalog::tryGetTableMetadata(const String &database_name, const String &table_name, TableMetadata &result) const {
  try {
    auto table_json_ptr =
        requestRest(std::filesystem::path(API_VERSION) / prefix / DATABASES_ENDPOINT / database_name / TABLES_ENDPOINT / table_name, "GET");
    ...
    if (result.isDefaultReadableTable() && result.requiresCredentials()) {
      auto table_token_json_ptr = requestRest(std::filesystem::path(API_VERSION) / prefix / DATABASES_ENDPOINT / database_name /
                                                  TABLES_ENDPOINT / table_name / TABLE_TOKEN_ENDPOINT,
                                              "GET");
      ...
      switch (table_storage_type) {
        case StorageType::S3: {
          ...
          static constexpr std::array<String, 3> secret_access_key_strs = {"fs.s3a.secret-key", "fs.s3a.secret.key",
...
Poco::JSON::Object::Ptr PaimonRestCatalog::requestRest(const String &endpoint, const String &method,
                                                       const Poco::URI::QueryParameters &params,
                                                       const DB::HTTPHeaderEntries &headers) const {
