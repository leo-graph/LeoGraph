...
DB::Names HiveCatalog::getTables() const {
  ...
  for (const auto& db : databases) {
    ...
    for (const auto& table : current_tables) result.push_back(db + "." + table);
  }
  return result;
}
...
bool HiveCatalog::existsTable(const std::string& namespace_name, const std::string& table_name) const {
  ...
  if (table.tableName.empty()) return false;
...
std::optional<StorageType> HiveCatalog::getStorageType() const { return std::nullopt; }
