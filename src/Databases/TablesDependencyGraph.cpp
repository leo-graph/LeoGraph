...
TablesDependencyGraph::TablesDependencyGraph(const String &name_for_logging_) : name_for_logging(name_for_logging_) {}

TablesDependencyGraph::TablesDependencyGraph(const TablesDependencyGraph &src) : TablesDependencyGraph(src.name_for_logging) {
  *this = src;
}

TablesDependencyGraph::TablesDependencyGraph(TablesDependencyGraph &&src) noexcept : TablesDependencyGraph(src.name_for_logging) {
  *this = std::move(src);
}
...
bool TablesDependencyGraph::empty() const { return nodes.empty(); }

size_t TablesDependencyGraph::getNumberOfTables() const { return nodes.size(); }
...
void TablesDependencyGraph::addDependency(const StorageID &table_id, const StorageID &dependency) {
  ...
  if (!inserted) return;  /// Not inserted because we already had this dependency.
...
void TablesDependencyGraph::addDependencies(const StorageID &table_id, const std::vector<StorageID> &dependencies) {
  ...
  for (const auto &dependency : dependencies) new_dependency_nodes.emplace(addOrUpdateNode(dependency));

  if (table_node->dependencies == new_dependency_nodes) return;
  ...
  if (!old_dependencies.empty()) {
    LOG_WARNING(getLogger(), "Replacing outdated dependencies ({}) of {} with: {}", fmt::join(old_dependencies, ", "), table_id,
...
void TablesDependencyGraph::addDependencies(const StorageID &table_id, const TableNamesSet &dependencies) {
  ...
  for (const auto &dependency : dependencies) converted_dependencies.emplace_back(StorageID{dependency});
...
bool TablesDependencyGraph::removeDependency(const StorageID &table_id, const StorageID &dependency, bool remove_isolated_tables) {
  ...
  if (!dependency_node) return false;

  auto dependency_it = table_node->dependencies.find(dependency_node);
  if (dependency_it == table_node->dependencies.end()) return false;  /// No such dependency, nothing to remove.
  ...
  if (remove_isolated_tables && dependency_node->dependencies.empty() && dependency_node->dependents.empty()) {
    ...
    if (table_node == dependency_node) table_node_removed = true;
  }
...
TablesDependencyGraph::Node *TablesDependencyGraph::findNode(const StorageID &table_id) const {
  ...
  if (table_id.hasUUID()) {
    ...
    if (it != nodes_by_uuid.end()) return it->second;  /// Found by UUID.
  }
...
TablesDependencyGraph::Node *TablesDependencyGraph::addOrUpdateNode(const StorageID &table_id) {
  ...
  if (node) {
    ...
    if (!table_id.table_name.empty() &&
        ((table_id.table_name != node->storage_id.table_name) || (table_id.database_name != node->storage_id.database_name))) {
      auto it = nodes_by_database_and_table_names.find(table_id);
      if (it != nodes_by_database_and_table_names.end()) {
...
size_t TablesDependencyGraph::removeTablesIf(const std::function<bool(const StorageID &)> &function) {
  ...
  if (num_removed) setNeedRecalculateLevels();
...
std::vector<StorageID> TablesDependencyGraph::getTables() const {
  ...
  for (const auto &node : nodes) res.emplace_back(node->storage_id);
  return res;
}

void TablesDependencyGraph::mergeWith(const TablesDependencyGraph &other) {
  for (const auto &other_node : other.nodes) addDependencies(other_node->storage_id, TablesDependencyGraph::getDependencies(*other_node));
}
...
std::vector<StorageID> TablesDependencyGraph::getDependencies(const Node &node) {
  ...
  for (const auto *dependency_node : node.dependencies) res.emplace_back(dependency_node->storage_id);
  return res;
}
...
std::vector<StorageID> TablesDependencyGraph::getDependents(const Node &node) {
  ...
  for (const auto *dependent_node : node.dependents) res.emplace_back(dependent_node->storage_id);
  return res;
}
...
void TablesDependencyGraph::checkNoCyclicDependencies() const {
  if (hasCyclicDependencies()) {
    throw Exception(ErrorCodes::INFINITE_LOOP, "{}: Tables {} have cyclic dependencies: {}", name_for_logging,
                    fmt::join(getTablesWithCyclicDependencies(), ", "), describeCyclicDependencies());
  }
}
...
std::vector<StorageID> TablesDependencyGraph::getTablesWithCyclicDependencies() const {
  ...
  for (const auto *node : getNodesSortedByLevel() | boost::adaptors::reversed) {
    if (node->level != CYCLIC_LEVEL) break;
...
String TablesDependencyGraph::describeCyclicDependencies() const {
  ...
  for (const auto *node : getNodesSortedByLevel() | boost::adaptors::reversed) {
    ...
    for (const auto *dependency_node : node->dependencies) {
      if (dependency_node->level != CYCLIC_LEVEL) continue;
      if (need_comma) res += ", ";
      need_comma = true;
    ...
    for (size_t i = begin; i != end; ++i) {
      ...
      for (const Node *dependent_node : current_node->dependents) {
        if (!dependent_node->num_dependencies_to_count)
          throw Exception(ErrorCodes::LOGICAL_ERROR, "{}: Trying to decrement 0 dependencies counter for {}. It's a bug", name_for_logging,
                          dependent_node->storage_id);
...
std::vector<std::vector<StorageID>> TablesDependencyGraph::getTablesSplitByDependencyLevel() const {
  ...
  if (sorted_nodes.empty()) return tables_split_by_level;
...
LoggerPtr TablesDependencyGraph::getLogger() const {
  if (!logger) logger = ::getLogger(name_for_logging);
  return logger;
}
