...
class DatabaseReplicated : public DatabaseAtomic {
 public:
  ...
  DatabaseReplicated(const String &name_, const String &metadata_path_, UUID uuid, const String &zookeeper_name_,
                     const String &zookeeper_path_, const String &shard_name_, const String &replica_name_,
                     DatabaseReplicatedSettings db_settings_, ContextPtr context);

  ~DatabaseReplicated() override;

  String getEngineName() const override { return "Replicated"; }

  /// If current query is initial, then the following methods add metadata updating ZooKeeper operations to current
  /// ZooKeeperMetadataTransaction.
  void dropTable(ContextPtr, const String &table_name, bool sync) override;
  void renameTable(ContextPtr context, const String &table_name, IDatabase &to_database, const String &to_table_name, bool exchange,
  ...
  void createTableRestoredFromBackup(const ASTPtr &create_table_query, ContextMutablePtr local_context,
                                     std::shared_ptr<IRestoreCoordination> restore_coordination, UInt64 timeout_ms) override;

  bool shouldReplicateQuery(const ContextPtr &query_context, const ASTPtr &query_ptr) const override;

  static void dropReplica(DatabaseReplicated *database, const String &zookeeper_name, const String &database_zookeeper_path,
  ...
  static ASTPtr parseQueryFromMetadataInZooKeeper(ContextPtr context_, const String &database_name_, const String &zookeeper_path_,
                                                  const String &node_name, const String &query);

  friend struct DatabaseReplicatedTask;
  friend class DatabaseReplicatedDDLWorker;

 protected:
  void commitCreateTable(const ASTCreateQuery &query, const StoragePtr &table, const String &table_metadata_tmp_path,
                         const String &table_metadata_path, ContextPtr query_context) override;
  void commitAlterTable(const StorageID &table_id, const String &table_metadata_tmp_path, const String &table_metadata_path,
 ...
 private:
  ...
  std::map<String, String> getConsistentMetadataSnapshotImpl(const ZooKeeperPtr &zookeeper,
                                                             const FilterByNameFunction &filter_by_table_name, size_t max_retries,
                                                             UInt32 &max_log_ptr) const;

  static ASTPtr parseQueryFromMetadata(ContextPtr context_, const String &database_name_, const String &table_name, const String &query,
  ...
  void assertDigestInTransactionOrInline(const ContextPtr &local_context, const ZooKeeperMetadataTransactionPtr &txn)
      TSA_REQUIRES(metadata_mutex);
  ...
  static BlockIO getQueryStatus(const String &zookeeper_name, const String &node_path, const String &replicas_path, ContextPtr context,
                                const Strings &hosts_to_wait, DDLGuardPtr &&database_guard);
