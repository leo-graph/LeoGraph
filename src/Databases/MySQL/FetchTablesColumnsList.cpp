...
std::map<String, ColumnsDescription> fetchTablesColumnsList(mysqlxx::PoolWithFailover& pool, const String& database_name,
                                                            const std::vector<String>& tables_name, const Settings& settings,
  ...
  Block tables_columns_sample_block{
      {std::make_shared<DataTypeString>(), "table_name"},     {std::make_shared<DataTypeString>(), "column_name"},
      {std::make_shared<DataTypeString>(), "column_type"},    {std::make_shared<DataTypeUInt8>(), "is_nullable"},
      {std::make_shared<DataTypeUInt8>(), "is_unsigned"},     {std::make_shared<DataTypeUInt64>(), "length"},
      {std::make_shared<DataTypeUInt64>(), "precision"},      {std::make_shared<DataTypeUInt64>(), "scale"},
  ...
  if (!database_name.empty()) query << " TABLE_SCHEMA = " << quote << database_name << " AND ";
  ...
  while (executor.pull(block)) {
    ...
    for (size_t i = 0; i < rows; ++i) {
      ...
      ColumnDescription column_description(
          ...
          convertMySQLDataType(type_support, column_type_col[i].safeGet<String>(),
                               settings[Setting::external_table_functions_use_nulls] && is_nullable_col[i].safeGet<UInt64>(),
                               is_unsigned_col[i].safeGet<UInt64>(), char_max_length_col[i].safeGet<UInt64>(),
                               precision_col[i].safeGet<UInt64>(), scale_col[i].safeGet<UInt64>()));
