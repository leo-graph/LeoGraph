...
DatabaseMySQL::DatabaseMySQL(ContextPtr context_, const String &database_name_, const String &metadata_path_,
                             const ASTStorage *database_engine_define_, const String &database_name_in_mysql_,
                             std::unique_ptr<MySQLSettings> settings_, mysqlxx::PoolWithFailover &&pool, bool attach, UUID uuid)
...
bool DatabaseMySQL::empty() const {
  ...
  if (local_tables_cache.empty()) return true;

  for (const auto &[table_name, storage_info] : local_tables_cache)
    if (!remove_or_detach_tables.contains(table_name)) return false;
...
bool DatabaseMySQL::isTableExist(const String &name, ContextPtr local_context) const { return bool(tryGetTable(name, local_context)); }
...
ASTPtr DatabaseMySQL::getCreateTableQueryImpl(const String &table_name, ContextPtr local_context, bool throw_on_error) const {
  ...
  try {
    ...
    if (throw_on_error) {
      throw Exception(ErrorCodes::CANNOT_GET_CREATE_TABLE_QUERY,
                      "Received error while fetching table structure for table {} from MySQL: {}", backQuote(table_name),
  ...
  if (!local_tables_cache.contains(table_name)) {
    if (throw_on_error) throw Exception(ErrorCodes::UNKNOWN_TABLE, "MySQL table {}.{} doesn't exist.", database_name_in_mysql, table_name);
    return nullptr;
  }
  ...
  auto create_table_query =
      DB::getCreateQueryFromStorage(storage, table_storage_define, true, static_cast<unsigned>(settings[Setting::max_parser_depth]),
...
void DatabaseMySQL::fetchLatestTablesStructureIntoCache(const std::map<String, UInt64> &tables_modification_time,
  ...
  for (const auto &table_and_columns : tables_and_columns) {
    ...
    local_tables_cache[table_name] = std::make_pair(
        ...
        std::make_shared<StorageMySQL>(StorageID(database_name, table_name), std::move(mysql_pool), database_name_in_mysql, table_name,
                                       /* replace_query_ */ false,
                                       /* on_duplicate_clause = */ "", ColumnsDescription{columns_name_and_type}, ConstraintsDescription{},
                                       String{}, getContext(), MySQLSettings{}));
  }
}
...
std::map<String, ColumnsDescription> DatabaseMySQL::fetchTablesColumnsList(const std::vector<String> &tables_name,
                                                                           ContextPtr local_context) const {
  const auto &settings = local_context->getSettingsRef();

  return DB::fetchTablesColumnsList(mysql_pool, database_name_in_mysql, tables_name, settings,
...
void DatabaseMySQL::shutdown() {
  ...
  for (const auto &[table_name, modify_time_and_storage] : tables_snapshot) modify_time_and_storage.second->flushAndShutdown();
...
void DatabaseMySQL::attachTable(ContextPtr /* context_ */, const String &table_name, const StoragePtr &storage, const String &) {
  ...
  if (!local_tables_cache.contains(table_name))
    throw Exception(ErrorCodes::UNKNOWN_TABLE, "Cannot attach table {}.{} because it does not exist.", backQuoteIfNeed(database_name),
...
String DatabaseMySQL::getMetadataPath() const { return metadata_path; }
...
void DatabaseMySQL::detachTablePermanently(ContextPtr, const String &table_name) {
  if (!persistent) throw Exception(ErrorCodes::NOT_IMPLEMENTED, "DETACH TABLE is not supported for non-persistent MySQL database");
...
void DatabaseMySQL::createTable(ContextPtr local_context, const String &table_name, const StoragePtr &storage, const ASTPtr &create_query) {
  if (!persistent) throw Exception(ErrorCodes::NOT_IMPLEMENTED, "MySQL database engine does not support CREATE or ATTACH TABLE queries");
...
void registerDatabaseMySQL(DatabaseFactory &factory) {
  auto create_fn = [](const DatabaseFactory::Arguments &args) {
    ...
    if (engine_define->settings) mysql_settings->loadFromQuery(*engine_define);

    auto mysql_pool = createMySQLPoolWithFailover(configuration, *mysql_settings);

    try {
      return make_shared<DatabaseMySQL>(args.context, args.database_name, args.metadata_path, engine_define, configuration.database,
                                        std::move(mysql_settings), std::move(mysql_pool), args.create_query.attach, args.uuid);
    } catch (...) {
