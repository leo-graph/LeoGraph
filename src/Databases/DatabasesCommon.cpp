...
void validateCreateQuery(const ASTCreateQuery &query, ContextPtr context) {
  ...
  ASTPtr new_query_raw =
      parseQuery(parser, serialized_query.data(), serialized_query.data() + serialized_query.size(), "after altering table ", 0,
  ...
  if (!new_query.columns_list || !new_query.columns_list->columns) return;

  const auto &columns = *new_query.columns_list;
  /// Do some basic sanity checks. We cannot do the same strict checks as on create, because context might not have the same settings if it
  /// is not called directly from an alter query. SECONDARY_CREATE should check most of the important things.
  ...
  for (const auto &ast : columns.columns->children) {
    ...
    if (!col_decl.getDefaultExpression()) continue;

    /// If no column description for the name, let's skip the validation of default expressions, but let's log the fact that something went
    /// wrong
  ...
  if (default_expr_info.expr_list) validateColumnsDefaultsAndGetSampleBlock(default_expr_info.expr_list, columns_desc.getAll(), context);
  ...
  if (columns.projections) {
    for (const auto &child : columns.projections->children) ProjectionDescription::getProjectionFromAST(child, columns_desc, context);
  }
  if (!new_query.storage) return;
  ...
  if (storage.order_by) primary_key = KeyDescription::getKeyFromAST(storage.order_by->ptr(), columns_desc, context);
  if (storage.primary_key) primary_key = KeyDescription::getKeyFromAST(storage.primary_key->ptr(), columns_desc, context);
  if (storage.partition_by) KeyDescription::getKeyFromAST(storage.partition_by->ptr(), columns_desc, context);
  if (storage.sample_by) KeyDescription::getKeyFromAST(storage.sample_by->ptr(), columns_desc, context);
...
void applyMetadataChangesToCreateQuery(const ASTPtr &query, const StorageInMemoryMetadata &metadata, ContextPtr context,
  ...
  if (ast_create_query.as_table_function && !has_structure)
    throw Exception(ErrorCodes::NOT_IMPLEMENTED,
                    "Cannot alter table {} because it was created AS table function"
  ...
  if (metadata.sql_security_type) {
    ...
    if (metadata.definer) new_sql_security->definer = make_intrusive<ASTUserNameWithHost>(*metadata.definer);
  ...
  if (ast_create_query.storage) {
    ...
    if (is_extended_storage_def) {
      if (metadata.sorting_key.definition_ast) storage_ast.set(storage_ast.order_by, metadata.sorting_key.definition_ast);

      if (metadata.primary_key.definition_ast) storage_ast.set(storage_ast.primary_key, metadata.primary_key.definition_ast);
      ...
      if (metadata.settings_changes) storage_ast.set(storage_ast.settings, metadata.settings_changes);
    } else if (metadata.settings_changes) {
      auto &settings_changes = metadata.settings_changes->as<ASTSetQuery &>().changes;
      if (!settings_changes.empty()) storage_ast.set(storage_ast.settings, metadata.settings_changes);
    }
  }
  ...
  if (validate_new_create_query) validateCreateQuery(ast_create_query, context);
}

ASTPtr getCreateQueryFromStorage(const StoragePtr &storage, const ASTPtr &ast_storage, bool only_ordinary, uint32_t max_parser_depth,
                                 uint32_t max_parser_backtracks, bool throw_on_error) {
  auto table_id = storage->getStorageID();
  auto metadata_ptr = storage->getInMemoryMetadataPtr();
  if (metadata_ptr == nullptr) {
    if (throw_on_error)
      throw Exception(ErrorCodes::CANNOT_GET_CREATE_TABLE_QUERY, "Cannot get metadata of {}.{}", backQuote(table_id.database_name),
    ...
    for (const auto &column_name_and_type : columns) {
      ...
      /// parser typename
      {
        ...
        if (!parser.parse(pos, ast_type, expected)) {
          if (throw_on_error)
            throw Exception(ErrorCodes::LOGICAL_ERROR, "Cannot parse metadata of {}.{}", backQuote(table_id.database_name),
                            backQuote(table_id.table_name));
          return nullptr;
        }
...
void cleanupObjectDefinitionFromTemporaryFlags(ASTCreateQuery &query) {
  ...
  if (!query.isView()) query.select = nullptr;
...
void writeMetadataFile(std::shared_ptr<IDisk> disk, const String &file_path, std::string_view content, bool fsync_metadata) {
  ...
  out->next();
  if (fsync_metadata) out->sync();
  out->finalize();
  out.reset();
}

void DatabaseWithAltersOnDiskBase::alterDatabaseComment(const AlterCommand &command, ContextPtr query_context [[maybe_unused]]) {
  if (!command.comment) throw Exception(ErrorCodes::BAD_ARGUMENTS, "Unable to obtain database comment from query");
  ...
  try {
#if CLICKHOUSE_CLOUD
    bool managed_by_shared_catalog =
        SharedDatabaseCatalog::initialized() && SharedDatabaseCatalog::isDatabaseEngineSupported(getEngineName());
    if (managed_by_shared_catalog && !SharedDatabaseCatalog::isInitialQuery(query_context)) return;
#endif
...
DatabaseWithOwnTablesBase::DatabaseWithOwnTablesBase(const String &name_, const String &logger, ContextPtr context_)
    : DatabaseWithAltersOnDiskBase(name_), WithContext(context_->getGlobalContext()), log(getLogger(logger)) {}
...
DatabaseTablesIteratorPtr DatabaseWithOwnTablesBase::getTablesIterator(ContextPtr, const FilterByNameFunction &filter_by_table_name,
  ...
  std::lock_guard lock(mutex);
  if (!filter_by_table_name) return std::make_unique<DatabaseTablesSnapshotIterator>(tables, database_name);

  Tables filtered_tables;
  for (const auto &[table_name, storage] : tables)
    if (filter_by_table_name(table_name)) filtered_tables.emplace(table_name, storage);
...
DatabaseDetachedTablesSnapshotIteratorPtr DatabaseWithOwnTablesBase::getDetachedTablesIterator(
  ...
  std::lock_guard lock(mutex);
  if (!filter_by_table_name) return std::make_unique<DatabaseDetachedTablesSnapshotIterator>(snapshot_detached_tables);
...
StoragePtr DatabaseWithOwnTablesBase::detachTableUnlocked(const String &table_name) {
  ...
  snapshot_detached_tables.emplace(table_name, SnapshotDetachedTable{.database = it->second->getStorageID().getDatabaseName(),
                                                                     .table = table_name,
  ...
  if (!table_storage->isSystemStorage() && !DatabaseCatalog::isPredefinedDatabase(database_name)) {
    ...
    for (auto metric : getAttachedCountersForStorage(table_storage)) CurrentMetrics::sub(metric);
  }
...
void DatabaseWithOwnTablesBase::attachTableUnlocked(const String &table_name, const StoragePtr &table) {
  ...
  if (table_id.database_name != database_name)
    throw Exception(ErrorCodes::UNKNOWN_DATABASE, "Database was renamed to `{}`, cannot create table in `{}`", database_name,
  ...
  if (!tables.emplace(table_name, table).second) {
    if (table_id.hasUUID()) DatabaseCatalog::instance().removeUUIDMapping(table_id.uuid);
  ...
  if (!table->isSystemStorage() && !DatabaseCatalog::isPredefinedDatabase(database_name)) {
    ...
    for (auto metric : getAttachedCountersForStorage(table)) CurrentMetrics::add(metric);
  }
}
...
std::vector<std::pair<ASTPtr, StoragePtr>> DatabaseWithOwnTablesBase::getTablesForBackup(const FilterByNameFunction &filter,
  ...
  for (auto it = getTablesIterator(local_context, filter, /*skip_not_loaded=*/false); it->isValid(); it->next()) {
    ...
    if (!storage) continue;  /// Probably the table has been just dropped.

    auto create_table_query = tryGetCreateTableQuery(it->name(), local_context);
    if (!create_table_query) {
      LOG_WARNING(log, "Couldn't get a create query for table {}.{}", backQuoteIfNeed(getDatabaseName()), backQuoteIfNeed(it->name()));
      continue;
    }
    ...
    if (create->getTable() != it->name()) {
      ...
      LOG_WARNING(log, "Got a create query with unexpected name {} for table {}.{}", backQuoteIfNeed(create->getTable()),
...
void DatabaseWithOwnTablesBase::createTableRestoredFromBackup(const ASTPtr &create_table_query, ContextMutablePtr local_context,
                                                              std::shared_ptr<IRestoreCoordination>, UInt64) {
