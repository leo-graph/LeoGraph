...
bool DDLMatcherBase::needChildVisit(const ASTPtr &node, const ASTPtr &child) {
  if (node->as<ASTStorage>()) return false;

  if (auto *create = node->as<ASTCreateQuery>()) {
    if (child.get() == create->select) return false;
  }
...
ssize_t DDLMatcherBase::getPositionOfTableNameArgumentToEvaluate(const ASTFunction &function) {
  if (functionIsJoinGet(function.name) || functionIsDictGet(function.name)) return 0;
...
ssize_t DDLMatcherBase::getPositionOfTableNameArgumentToVisit(const ASTFunction &function) {
  ...
  if (0 <= maybe_res) return maybe_res;

  if (functionIsInOrGlobalInOperator(function.name)) {
    if (function.children.empty()) return -1;

    const auto *args = function.children[0]->as<ASTExpressionList>();
    if (!args || args->children.size() != 2) return -1;

    if (args->children[1]->as<ASTFunction>()) return -1;
...
void DDLLoadingDependencyVisitor::visit(const ASTFunction &function, Data &data) {
  ...
  if (table_name_arg_idx < 0) return;
...
void DDLLoadingDependencyVisitor::visit(const ASTFunctionWithKeyValueArguments &dict_source, Data &data) {
  if (dict_source.name != "clickhouse") return;
  if (!dict_source.elements) return;
  ...
  if (!info->table_name.table.empty()) {
    ...
    if (info->table_name.database.empty()) info->table_name.database = data.table_name.database;
    ...
    auto select_query_dependencies = getDependenciesFromDictionaryNestedSelectQuery(data.global_context, data.table_name, data.create_query,
                                                                                    info->query, data.default_database, data.can_throw);
...
void DDLLoadingDependencyVisitor::visit(const ASTStorage &storage, Data &data) {
  if (storage.ttl_table) {
    auto ttl_dependencies =
        getDependenciesFromCreateQuery(data.global_context, data.table_name, storage.ttl_table->ptr(), data.default_database);
...
void DDLLoadingDependencyVisitor::extractTableNameFromArgument(const ASTFunction &function, Data &data, size_t arg_idx) {
  ...
  if (const auto *function_arg = arg->as<ASTFunction>()) {
    if (!functionIsJoinGet(function_arg->name) && !functionIsDictGet(function_arg->name)) return;
    ...
    if (name->value.getType() != Field::Types::String) return;
