...
static DataTypePtr convertSQLiteDataType(String type) {
  ...
  /// The SQLite columns get the INTEGER affinity if the type name contains "int". This means variable-length integers up to 8 bytes. The
  /// bit width is not really enforced even in a STRICT table, so in general we should treat these columns as Int64. Besides that, we allow
  /// some common fixed-width int specifiers for applications to select a particular width, even though it's not enforced in any way by
  /// SQLite itself. Docs: https://www.sqlite.org/datatype3.html The most insane quote from there: Note that a declared type of "FLOATING
  /// POINT" would give INTEGER affinity, not REAL affinity, due to the "INT" at the end of "POINT".
...
std::shared_ptr<NamesAndTypesList> fetchSQLiteTableStructure(sqlite3 *connection, const String &sqlite_table_name) {
  ...
  auto callback_get_data = [](void *res, int col_num, char **data_by_col, char **col_names) -> int {
    ...
    if (is_nullable) name_and_type.type = std::make_shared<DataTypeNullable>(name_and_type.type);
  ...
  if (status != SQLITE_OK) {
    ...
    throw Exception(ErrorCodes::SQLITE_ENGINE_ERROR, "Failed to fetch SQLite data. Status: {}. Message: {}", status, err_msg);
  }
