...
StoragePtr createStorageDictionary(const String& database_name, const ExternalLoader::LoadResult& load_result, ContextPtr context) {
  try {
    if (!load_result.config) return nullptr;
    ...
    return std::make_shared<StorageDictionary>(StorageID(database_name, load_result.name), load_result.name, dictionary_structure, comment,
                                               StorageDictionary::Location::DictionaryDatabase, context);
  } catch (Exception& e) {
    throw Exception(e.code(), "Error while loading dictionary '{}.{}': {}", database_name, load_result.name, e.displayText());
  }
}
}  // namespace

DatabaseDictionary::DatabaseDictionary(const String& name_, ContextPtr context_)
    : IDatabase(name_), WithContext(context_->getGlobalContext()), log(getLogger("DatabaseDictionary(" + database_name + ")")) {}
...
Tables DatabaseDictionary::listTables(const FilterByNameFunction& filter_by_name) const {
  ...
  for (auto& load_result : load_results) {
    ...
    if (storage) tables.emplace(storage->getStorageID().table_name, storage);
  }
  return tables;
}
...
bool DatabaseDictionary::empty() const { return !getContext()->getExternalDictionariesLoader().hasObjects(); }
...
ASTPtr DatabaseDictionary::getCreateTableQueryImpl(const String& table_name, ContextPtr, bool throw_on_error) const {
  String query;
  {
    ...
    auto names_and_types =
        StorageDictionary::getNamesAndTypes(ExternalDictionariesLoader::getDictionaryStructure(*load_result.config), false);
...
ASTPtr DatabaseDictionary::getCreateDatabaseQueryImpl() const {
  String query;
  {
    ...
    if (!comment.empty()) buffer << " COMMENT " << backQuote(comment);
  }
  const auto& settings = getContext()->getSettingsRef();
  ParserCreateQuery parser;
  return parseQuery(parser, query.data(), query.data() + query.size(), "", 0, settings[Setting::max_parser_depth],
...
void registerDatabaseDictionary(DatabaseFactory& factory) {
  auto create_fn = [](const DatabaseFactory::Arguments& args) { return make_shared<DatabaseDictionary>(args.database_name, args.context); };
