...
DatabaseAtomic::DatabaseAtomic(String name_, String metadata_path_, UUID uuid, const String &logger_name, ContextPtr context_,
    ...
    : DatabaseOrdinary(name_, metadata_path_, DatabaseCatalog::getStoreDirPath() / "", logger_name, context_,
...
void DatabaseAtomic::createDirectoriesUnlocked() {
  ...
  if (db_disk->isSymlinkSupported()) db_disk->createDirectories(path_to_table_symlinks);
...
void DatabaseAtomic::drop(ContextPtr) {
  ...
  if (!db_disk->isReadOnly()) db_disk->removeRecursive(getMetadataPath());
}

void DatabaseAtomic::attachTable(ContextPtr /* context_ */, const String &name, const StoragePtr &table,
                                 const String &relative_table_path) {
...
void DatabaseAtomic::dropTableImpl(ContextPtr local_context, const String &table_name, bool sync) {
  ...
  auto db_disk = getDisk();
  {
    ...
    if (txn && !local_context->isInternalSubquery()) txn->commit();  /// Commit point (a sort of) for Replicated database
  ...
  if (table->storesDataOnDisk()) tryRemoveSymlink(table_name);
...
void DatabaseAtomic::renameTable(ContextPtr local_context, const String &table_name, IDatabase &to_database, const String &to_table_name,
                                 bool exchange, bool dictionary) TSA_NO_THREAD_SAFETY_ANALYSIS  /// TSA does not support conditional locking
{
  ...
  if (exchange && !supportsAtomicRename(&message))
    throw Exception(ErrorCodes::NOT_IMPLEMENTED,
                    "RENAME EXCHANGE is not supported because exchanging files is not supported by the OS ({})", message);
  ...
  if (!inside_database) other_db.createDirectories();
  ...
  auto detach = [](DatabaseAtomic &db, const String &table_name_, bool has_symlink) TSA_REQUIRES(db.mutex) {
    ...
    if (it != db.table_name_to_path.end()) table_data_path_saved = it->second;
    assert(!table_data_path_saved.empty());
    db.tables.erase(table_name_);
    db.table_name_to_path.erase(table_name_);
    if (has_symlink) db.tryRemoveSymlink(table_name_);
  ...
  if (inside_database && table_name == to_table_name) return;
  ...
  if (!exchange) other_db.checkMetadataFilenameAvailabilityUnlocked(to_table_name);
  ...
  if (exchange) other_table_data_path = detach(other_db, to_table_name, other_table->storesDataOnDisk());

  table->renameInMemory(new_table_id);
  if (exchange) other_table->renameInMemory(other_table_new_id);

  if (!inside_database) {
    DatabaseCatalog::instance().updateUUIDMapping(old_table_id.uuid, other_db.shared_from_this(), table);
    if (exchange) DatabaseCatalog::instance().updateUUIDMapping(other_table->getStorageID().uuid, shared_from_this(), other_table);
  }

  attach(other_db, to_table_name, table_data_path, table);
  if (exchange) attach(*this, table_name, other_table_data_path, other_table);
}

void DatabaseAtomic::commitCreateTable(const ASTCreateQuery &query, const StoragePtr &table, const String &table_metadata_tmp_path,
  ...
  try {
    ...
    if (txn && !query_context->isInternalSubquery()) txn->commit();  /// Commit point (a sort of) for Replicated database
  ...
  if (table->storesDataOnDisk()) tryCreateSymlink(table);
}
...
void DatabaseAtomic::commitAlterTable(const StorageID &table_id, const String &table_metadata_tmp_path, const String &table_metadata_path,
  ...
  SCOPE_EXIT({
    if (check_file_exists) db_disk->removeFileIfExists(table_metadata_tmp_path);
  });

  std::lock_guard lock{mutex};
  auto actual_table_id = getTableUnlocked(table_id.table_name)->getStorageID();

  if (table_id.uuid != actual_table_id.uuid) throw Exception(ErrorCodes::CANNOT_ASSIGN_ALTER, "Cannot alter table because it was renamed");
  ...
  if (!check_file_exists) db_disk->replaceFile(table_metadata_tmp_path, table_metadata_path);
}
...
DatabaseAtomic::DetachedTables DatabaseAtomic::cleanupDetachedTables() {
  DetachedTables not_in_use;
  if (detached_tables.empty()) return not_in_use;
...
DatabaseTablesIteratorPtr DatabaseAtomic::getTablesIterator(ContextPtr local_context,
                                                            const IDatabase::FilterByNameFunction &filter_by_table_name,
...
UUID DatabaseAtomic::tryGetTableUUID(const String &table_name) const {
  if (auto table = tryGetTable(table_name, getContext())) return table->getStorageID().uuid;
...
void DatabaseAtomic::beforeLoadingMetadata(ContextMutablePtr /*context*/, LoadingStrictnessLevel mode) {
  ...
  if (mode < LoadingStrictnessLevel::FORCE_RESTORE) return;

  if (!db_disk->isSymlinkSupported()) return;
  ...
  if (!db_disk->existsFileOrDirectory(path_to_table_symlinks)) return;
  ...
  for (const auto it = db_disk->iterateDirectory(path_to_table_symlinks); it->isValid(); it->next()) {
    ...
    if (table_path.filename().empty()) table_path = table_path.parent_path();
    if (!db_disk->isSymlink(table_path)) {
      throw Exception(ErrorCodes::ABORTED, "'{}' is not a symlink. Atomic database should contains only symlinks.",
...
LoadTaskPtr DatabaseAtomic::startupDatabaseAsync(AsyncLoader &async_loader, LoadJobSet startup_after, LoadingStrictnessLevel mode) {
  ...
  auto job = makeLoadJob(base->goals(), TablesLoaderBackgroundStartupPoolId, fmt::format("startup Atomic database {}", getDatabaseName()),
                         [this, mode, db_disk](AsyncLoader &, const LoadJobPtr &) {
                           ...
                           for (const auto &table : table_names) {
                             ...
                             if (table_ptr) {
                               if (table_ptr->storesDataOnDisk()) tryCreateSymlink(table_ptr, true);
                             } else
...
void DatabaseAtomic::waitDatabaseStarted() const {
  ...
  if (task) waitLoad(currentPoolOr(TablesLoaderForegroundPoolId), task, false);
}
...
void DatabaseAtomic::tryCreateSymlink(const StoragePtr &table, bool if_data_path_exist) {
  ...
  if (table->getDataPaths().empty()) return;
  ...
  try {
    ...
    LOG_DEBUG(log, "Trying to create a symlink for table {}, data_path {}, link {}", table->getStorageID().getNameForLogs(),
              table_data_path, link);

    /// If it already points where needed.
    if (db_disk->equivalentNoThrow(table_data_path, link)) return;

    if (if_data_path_exist && !db_disk->existsFileOrDirectory(data_path)) return;
...
void DatabaseAtomic::tryCreateMetadataSymlink() {
  ...
  if (db_disk->existsFileOrDirectory(path_to_metadata_symlink)) {
    ...
    try {
      ...
      if (db_disk->isSymlinkNoThrow(path_to_metadata_symlink)) db_disk->removeFileIfExists(path_to_metadata_symlink);

      LOG_DEBUG(log, "Creating directory symlink, path_to_metadata_symlink: {}, metadata_path: {}", path_to_metadata_symlink,
                metadata_path);
...
void DatabaseAtomic::renameDatabase(ContextPtr query_context, const String &new_name) {
  ...
  try {
    auto db_disk = getDisk();
    if (db_disk->isSymlinkSupported()) db_disk->removeFileIfExists(path_to_metadata_symlink);
  } catch (...) {
    LOG_WARNING(log, getCurrentExceptionMessageAndPattern(/* with_stacktrace */ true));
  }
  ...
  {
    {
      ...
      for (auto &table : tables) table_names.push_back(table.first);
...
void DatabaseAtomic::waitDetachedTableNotInUse(const UUID &uuid) {
  ...
  while (true) {
    DetachedTables not_in_use;
    {
      std::lock_guard lock{mutex};
      not_in_use = cleanupDetachedTables();
      if (!detached_tables.contains(uuid)) return;
    }
...
void registerDatabaseAtomic(DatabaseFactory &factory) {
  auto create_fn = [](const DatabaseFactory::Arguments &args) {
    ...
    return make_shared<DatabaseAtomic>(args.database_name, args.metadata_path, args.uuid, args.context, database_metadata_disk_settings);
  };
