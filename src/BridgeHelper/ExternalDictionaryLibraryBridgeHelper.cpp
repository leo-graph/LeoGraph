...
ExternalDictionaryLibraryBridgeHelper::ExternalDictionaryLibraryBridgeHelper(ContextPtr context_, const Block& sample_block_,
...
bool ExternalDictionaryLibraryBridgeHelper::bridgeHandShake() {
  ...
  if (dictionary_id_exists && !library_initialized)
    throw Exception(ErrorCodes::LOGICAL_ERROR, "Library was not initialized, but bridge responded to already have dictionary id: {}",
                    dictionary_id);
...
QueryPipeline ExternalDictionaryLibraryBridgeHelper::loadKeys(const Block& requested_block) {
  ...
  ReadWriteBufferFromHTTP::OutStreamCallback out_stream_callback = [requested_block, this](std::ostream& os) {
    ...
    auto output_format =
        getContext()->getOutputFormat(ExternalDictionaryLibraryBridgeHelper::DEFAULT_FORMAT, out_buffer, requested_block.cloneEmpty());
...
bool ExternalDictionaryLibraryBridgeHelper::executeRequest(const Poco::URI& uri,
                                                           ReadWriteBufferFromHTTP::OutStreamCallback out_stream_callback) const {
...
QueryPipeline ExternalDictionaryLibraryBridgeHelper::loadBase(const Poco::URI& uri,
  ...
  auto source = FormatFactory::instance().getInput(ExternalDictionaryLibraryBridgeHelper::DEFAULT_FORMAT, *read_buf_ptr, sample_block,
                                                   getContext(), DEFAULT_BLOCK_SIZE);
