...
Strings BackupCoordinationReplicatedAccess::getFilePaths(const String& access_zk_path, AccessEntityType access_entity_type,
                                                         const String& host_id) const {
  auto it = file_paths_by_zk_path.find(std::make_pair(access_zk_path, access_entity_type));
  if (it == file_paths_by_zk_path.end()) return {};

  const auto& file_paths = it->second;
  if ((file_paths.host_to_store_access != host_id) || file_paths.file_paths.empty()) return {};
  ...
  for (const auto& file_path : file_paths.file_paths) res.emplace_back(fs::path{file_path}.replace_filename(filename));
