...
BackupMetadataFinder::BackupMetadataFinder(const RestoreSettings& restore_settings_, const BackupPtr& backup_,
...
void BackupMetadataFinder::waitFutures(bool throw_if_error) {
  ...
  for (;;) {
    ...
    if (futures_to_wait.empty()) break;
...
void BackupMetadataFinder::findRootPathsInBackup() {
  ...
  LOG_TRACE(log, "Will use paths in backup: {}",
            boost::algorithm::join(root_paths_in_backup | boost::adaptors::transformed([](const std::filesystem::path& path) -> String {
                                     return doubleQuoteString(String{path});
                                   }),
                                   ", "));
}

void BackupMetadataFinder::findTableInBackup(const QualifiedTableName& table_name_in_backup, bool skip_if_inner_table,
                                             const std::optional<ASTs>& partitions) {
  schedule([this, table_name_in_backup, skip_if_inner_table,
            partitions]() { findTableInBackupImpl(table_name_in_backup, skip_if_inner_table, partitions); },
           ThreadName::RESTORE_FIND_TABLE);
}

void BackupMetadataFinder::findTableInBackupImpl(const QualifiedTableName& table_name_in_backup, bool skip_if_inner_table,
                                                 const std::optional<ASTs>& partitions) {
  ...
  for (const auto& root_path_in_backup : root_paths_in_backup) {
    ...
    if (is_temporary_table) {
      try_metadata_path = root_path_in_backup / "temporary_tables" / "metadata" / (escapeForFileName(table_name_in_backup.table) + ".sql");
    } else {
  ...
  if (!metadata_path)
    throw Exception(ErrorCodes::BACKUP_ENTRY_NOT_FOUND, "{} not found in backup",
  ...
  if (skip_if_inner_table && BackupUtils::isInnerTable(table_name)) return;
  ...
  if (auto it = table_infos.find(table_name); it != table_infos.end()) {
    ...
    if (table_info.create_table_query && (table_info.create_table_query_str != create_table_query_str)) {
      throw Exception(ErrorCodes::CANNOT_RESTORE_TABLE, "Extracted two different create queries for the same {}: {} and {}",
                      tableNameWithTypeToString(table_name.database, table_name.table, false), table_info.create_table_query_str,
  ...
  if (partitions) {
    if (!res_table_info.partitions) res_table_info.partitions.emplace();
    insertAtEnd(*res_table_info.partitions, *partitions);
  }
}

void BackupMetadataFinder::findDatabaseInBackup(const String& database_name_in_backup,
                                                const std::set<DatabaseAndTableName>& except_table_names) {
  schedule([this, database_name_in_backup, except_table_names]() { findDatabaseInBackupImpl(database_name_in_backup, except_table_names); },
           ThreadName::RESTORE_FIND_TABLE);
}

void BackupMetadataFinder::findDatabaseInBackupImpl(const String& database_name_in_backup,
                                                    const std::set<DatabaseAndTableName>& except_table_names) {
  std::optional<fs::path> metadata_path;
  ...
  for (const auto& root_path_in_backup : root_paths_in_backup) {
    ...
    if (!metadata_path && !try_metadata_path.empty() && backup->fileExists(try_metadata_path)) metadata_path = try_metadata_path;

    Strings file_names = backup->listFiles(try_tables_metadata_path, /*recursive*/ false);
    for (const String& file_name : file_names) {
      if (!file_name.ends_with(".sql")) continue;
  ...
  if (metadata_path) {
    ...
    if (database_info.create_database_query && (database_info.create_database_query_str != create_database_query_str)) {
      throw Exception(ErrorCodes::CANNOT_RESTORE_DATABASE, "Extracted two different create queries for the same database {}: {} and {}",
                      backQuoteIfNeed(database_name), database_info.create_database_query_str, create_database_query_str);
    }
  ...
  for (const String& table_name_in_backup : table_names_in_backup) {
    if (except_table_names.contains({database_name_in_backup, table_name_in_backup})) continue;

    findTableInBackup({database_name_in_backup, table_name_in_backup}, /* skip_if_inner_table= */ true, /* partitions= */ {});
  }
}

void BackupMetadataFinder::findEverythingInBackup(const std::set<String>& except_database_names,
                                                  const std::set<DatabaseAndTableName>& except_table_names) {
  ...
  for (const auto& root_path_in_backup : root_paths_in_backup) {
    ...
    for (String& file_name : file_names) {
      if (file_name.ends_with(".sql")) file_name.resize(file_name.length() - strlen(".sql"));
  ...
  for (const String& database_name_in_backup : database_names_in_backup) {
    if (except_database_names.contains(database_name_in_backup)) continue;
