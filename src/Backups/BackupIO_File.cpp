...
bool BackupReaderFile::fileExists(const String &file_name) { return fs::exists(root_path / file_name); }

UInt64 BackupReaderFile::getFileSize(const String &file_name) { return fs::file_size(root_path / file_name); }
...
void BackupReaderFile::copyFileToDisk(const String &path_in_backup, size_t file_size, bool encrypted_in_backup, DiskPtr destination_disk,
...
bool BackupWriterFile::fileExists(const String &file_name) { return fs::exists(root_path / file_name); }

UInt64 BackupWriterFile::getFileSize(const String &file_name) { return fs::file_size(root_path / file_name); }
...
void BackupWriterFile::removeFile(const String &file_name) { (void)fs::remove(root_path / file_name); }
...
void BackupWriterFile::removeEmptyDirectoriesImpl(const fs::path &current_dir) {
  ...
  if (!fs::is_directory(current_dir)) return;
  ...
  for (const auto &it : std::filesystem::directory_iterator{current_dir}) removeEmptyDirectoriesImpl(it.path());

  if (fs::is_empty(current_dir)) (void)fs::remove(current_dir);
}

void BackupWriterFile::copyFileFromDisk(const String &path_in_backup, DiskPtr src_disk, const String &src_path, bool copy_encrypted,
