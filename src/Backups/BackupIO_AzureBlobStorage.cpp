...
BackupReaderAzureBlobStorage::BackupReaderAzureBlobStorage(const AzureBlobStorage::ConnectionParams &connection_params_,
                                                           const String &blob_path_, bool allow_azure_native_copy,
                                                           const ReadSettings &read_settings_, const WriteSettings &write_settings_,
    ...
    : BackupReaderDefault(read_settings_, write_settings_, getLogger("BackupReaderAzureBlobStorage")),
      data_source_description{DataSourceType::ObjectStorage,
                              ...
                              MetadataStorageType::None,
                              connection_params_.getConnectionURL(),
                              false,
                              false,
                              ""},
  ...
  auto settings_ptr =
      AzureBlobStorage::getRequestSettingsForBackup(context_, connection_params.endpoint.storage_account_url, allow_azure_native_copy);

  object_storage = std::make_unique<AzureObjectStorage>("BackupReaderAzureBlobStorage", connection_params.auth_method,
                                                        std::move(client_ptr), std::move(settings_ptr), connection_params,
                                                        connection_params.getContainer(), connection_params.getConnectionURL(),
                                                        /*common_key_prefix*/ "");
...
std::unique_ptr<ReadBufferFromFileBase> BackupReaderAzureBlobStorage::readFile(const String &file_name) {
  String key = fs::path(blob_path) / file_name;
  return std::make_unique<ReadBufferFromAzureBlobStorage>(client, key, read_settings, settings->max_single_read_retries,
...
void BackupReaderAzureBlobStorage::copyFileToDisk(const String &path_in_backup, size_t file_size, bool encrypted_in_backup,
  ...
  LOG_TRACE(log, "Source description {}, destination description {}", data_source_description.description,
            destination_data_source_description.description);
  if (destination_data_source_description.object_storage_type == ObjectStorageType::Azure &&
    ...
    auto write_blob_function = [&](const Strings &dst_blob_path, WriteMode mode, const std::optional<ObjectAttributes> &) -> size_t {
      ...
      copyAzureBlobStorageFile(client, destination_disk->getObjectStorage()->getAzureBlobStorageClient(), connection_params.getContainer(),
                               fs::path(blob_path) / path_in_backup, 0, file_size,
                               /* dest_container */ dst_blob_path[1],
                               /* dest_path */ dst_blob_path[0], settings, read_settings, std::optional<ObjectAttributes>(),
...
BackupWriterAzureBlobStorage::BackupWriterAzureBlobStorage(const AzureBlobStorage::ConnectionParams &connection_params_,
                                                           const String &blob_path_, bool allow_azure_native_copy,
                                                           const ReadSettings &read_settings_, const WriteSettings &write_settings_,
                                                           const ContextPtr &context_, bool attempt_to_create_container)
    : BackupWriterDefault(read_settings_, write_settings_, getLogger("BackupWriterAzureBlobStorage")),
      ...
      connection_params(connection_params_),
      blob_path(blob_path_) {
  if (!attempt_to_create_container) connection_params.endpoint.container_already_exists = true;

  auto client_ptr = AzureBlobStorage::getContainerClient(connection_params, /*readonly=*/false);
  auto settings_ptr =
      AzureBlobStorage::getRequestSettingsForBackup(context_, connection_params.endpoint.storage_account_url, allow_azure_native_copy);

  object_storage = std::make_unique<AzureObjectStorage>("BackupWriterAzureBlobStorage", connection_params.auth_method,
                                                        std::move(client_ptr), std::move(settings_ptr), connection_params,
                                                        connection_params.getContainer(), connection_params.getConnectionURL(),
                                                        /*common_key_prefix*/ "");
...
void BackupWriterAzureBlobStorage::copyFileFromDisk(const String &path_in_backup, DiskPtr src_disk, const String &src_path,
  ...
  LOG_TRACE(log, "Source description {}, destination description {}", source_data_source_description.description,
  ...
  if (source_data_source_description.object_storage_type == ObjectStorageType::Azure &&
    ...
    if (auto src_blob_path = src_disk->getBlobPath(src_path); src_blob_path.size() == 2) {
      ...
      copyAzureBlobStorageFile(src_disk->getObjectStorage()->getAzureBlobStorageClient(), client,
                               /* src_container */ src_blob_path[1],
                               /* src_path */ src_blob_path[0], start_pos, length, connection_params.getContainer(),
                               fs::path(blob_path) / path_in_backup, settings, read_settings, std::optional<ObjectAttributes>(),
...
void BackupWriterAzureBlobStorage::copyFile(const String &destination, const String &source, size_t size) {
  ...
  copyAzureBlobStorageFile(client, client, connection_params.getContainer(), fs::path(blob_path) / source, 0, size,
                           /* dest_container */ connection_params.getContainer(),
                           /* dest_path */ destination, settings, read_settings, std::optional<ObjectAttributes>(),
                           threadPoolCallbackRunnerUnsafe<void>(getBackupsIOThreadPool().get(), ThreadName::AZURE_BACKUP_WRITER));
}

void BackupWriterAzureBlobStorage::copyDataToFile(const String &path_in_backup, const CreateReadBufferFunction &create_read_buffer,
                                                  UInt64 start_pos, UInt64 length) {
  copyDataToAzureBlobStorageFile(create_read_buffer, start_pos, length, client, connection_params.getContainer(),
                                 fs::path(blob_path) / path_in_backup, settings,
                                 threadPoolCallbackRunnerUnsafe<void>(getBackupsIOThreadPool().get(), ThreadName::AZURE_BACKUP_WRITER));
}
...
std::unique_ptr<WriteBuffer> BackupWriterAzureBlobStorage::writeFile(const String &file_name) {
  ...
  return std::make_unique<WriteBufferFromAzureBlobStorage>(
      client, key, DBMS_DEFAULT_BUFFER_SIZE, write_settings, settings, connection_params.getContainer(),
      /* blob_log */ nullptr, threadPoolCallbackRunnerUnsafe<void>(getBackupsIOThreadPool().get(), ThreadName::AZURE_BACKUP_WRITER));
}
...
void BackupWriterAzureBlobStorage::removeFiles(const Strings &file_names) {
  ...
  for (const auto &file_name : file_names) objects.emplace_back(fs::path(blob_path) / file_name);
