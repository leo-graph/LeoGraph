...
class BackupImpl : public IBackup {
 public:
  ...
  /// RESTORE
  BackupImpl(BackupFactory::CreateParams params_, const ArchiveParams &archive_params_, std::shared_ptr<IBackupReader> reader_,
             SnapshotReaderCreator lightweight_snapshot_reader_creator_ = {});

  /// BACKUP
  BackupImpl(BackupFactory::CreateParams params_, const ArchiveParams &archive_params_, std::shared_ptr<IBackupWriter> writer_);

  /// UNLOCK
  BackupImpl(const BackupInfo &backup_info_, const ArchiveParams &archive_params_, std::shared_ptr<IBackupReader> reader_,
  ...
  size_t copyFileToDisk(const String &file_name, DiskPtr destination_disk, const String &destination_path,
                        WriteMode write_mode) const override;
 ...
 private:
...
#if CLICKHOUSE_CLOUD
  size_t copyFileToDiskByObjectKey(const String &object_key, DiskPtr destination_disk, const String &destination_path,
  ...
  std::unique_ptr<ReadBufferFromFileBase> readFileImpl(const String &file_name, const SizeAndChecksum &size_and_checksum,
  ...
  std::map<String /* file_name */, SizeAndChecksum> file_names
      TSA_GUARDED_BY(mutex);  /// Should be ordered alphabetically, see listFiles(). For empty files we assume checksum = 0.
