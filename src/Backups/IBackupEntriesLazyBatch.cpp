...
class IBackupEntriesLazyBatch::BackupEntryFromBatch : public IBackupEntry {
 public:
  ...
  std::unique_ptr<SeekableReadBuffer> getReadBuffer(const ReadSettings& read_settings) const override {
    return getInternalBackupEntry()->getReadBuffer(read_settings);
  }
  ...
  std::optional<UInt128> getPartialChecksum(UInt64 limit, const ReadSettings& read_settings) const override {
    return getInternalBackupEntry()->getPartialChecksum(limit, read_settings);
  }
...
void IBackupEntriesLazyBatch::generateIfNecessary() {
  ...
  if (entries.size() != getSize()) throw Exception(ErrorCodes::LOGICAL_ERROR, "Backup entries were generated incorrectly");

  for (size_t i = 0; i != entries.size(); ++i) {
    if (entries[i].first != getName(i)) throw Exception(ErrorCodes::LOGICAL_ERROR, "Backup entries were generated incorrectly");
  }
