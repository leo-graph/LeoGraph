 ...
 public:
  ...
  void addPartInfo(MergeTreePartInfo&& new_part_info, const std::shared_ptr<const String>& replica_name) {
    ...
    while (last_it != parts.end()) {
      ...
      if (part.info.min_block > new_max_block) break;
      if (!new_part_info.contains(part.info)) {
        throw Exception(ErrorCodes::CANNOT_BACKUP_TABLE, "Intersected parts detected: {} on replica {} and {} on replica {}",
                        part.info.getPartNameForLogs(), *part.replica_name, new_part_info.getPartNameForLogs(), *replica_name);
      }
      ++last_it;
    }
  ...
  bool isCoveredByAnotherPart(const MergeTreePartInfo& part_info) const {
    ...
    if (partition_it == partitions.end()) return false;
...
void BackupCoordinationReplicatedTables::addPartNames(PartNamesForTableReplica&& part_names) {
  ...
  if (prepared) throw Exception(ErrorCodes::LOGICAL_ERROR, "addPartNames() must not be called after preparing");

  auto& table_info = table_infos[table_zk_path];
  table_info.table_name_for_logs = table_name_for_logs;

  if (!table_info.covered_parts_finder) table_info.covered_parts_finder = std::make_unique<CoveredPartsFinder>(table_name_for_logs);
  ...
  for (const auto& part_name_and_checksum : part_names_and_checksums) {
    ...
    if (it == table_info.replicas_by_part_name.end()) {
      ...
      if (other.checksum != checksum) {
        ...
        throw Exception(ErrorCodes::CANNOT_BACKUP_TABLE,
                        ...
                        table_name_for_logs, replica_name, part_name, other_replica_name, getHexUIntLowercase(checksum), replica_name,
                        getHexUIntLowercase(other.checksum), other_replica_name);
      }
    }
    ...
    replica_names.insert(std::upper_bound(replica_names.begin(), replica_names.end(), replica_name_ptr, LessReplicaName{}),
                         replica_name_ptr);
  }
}
...
Strings BackupCoordinationReplicatedTables::getPartNames(const String& table_zk_path, const String& replica_name) const {
  ...
  if (it2 == part_names_by_replica_name.end()) return {};
...
void BackupCoordinationReplicatedTables::addMutations(MutationsForTableReplica&& mutations_for_table_replica) {
  ...
  if (prepared) throw Exception(ErrorCodes::LOGICAL_ERROR, "addMutations() must not be called after preparing");

  auto& table_info = table_infos[table_zk_path];
  table_info.table_name_for_logs = table_name_for_logs;
  for (const auto& [mutation_id, mutation_entry] : mutations) table_info.mutations.emplace(mutation_id, mutation_entry);
...
std::vector<MutationInfo> BackupCoordinationReplicatedTables::getMutations(const String& table_zk_path, const String& replica_name) const {
  prepare();
  ...
  if (table_info.replica_name_to_store_mutations != replica_name) return {};

  std::vector<MutationInfo> res;
  res.reserve(table_info.mutations.size());
  for (const auto& [mutation_id, mutation_entry] : table_info.mutations) res.emplace_back(MutationInfo{mutation_id, mutation_entry});
  return res;
}
...
void BackupCoordinationReplicatedTables::prepare() const {
  ...
  for (const auto& table_info : table_infos | boost::adaptors::map_values) {
    try {
      ...
      for (const auto& [part_name, part_replicas] : table_info.replicas_by_part_name) {
        if (table_info.covered_parts_finder->isCoveredByAnotherPart(part_name)) continue;
