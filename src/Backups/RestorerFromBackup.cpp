...
bool isSystemAccessTableName(const QualifiedTableName &table_name) {
  if (table_name.database != DatabaseCatalog::SYSTEM_DATABASE) return false;
...
RestorerFromBackup::RestorerFromBackup(const ASTBackupQuery::Elements &restore_query_elements_, const RestoreSettings &restore_settings_,
                                       std::shared_ptr<IRestoreCoordination> restore_coordination_, const BackupPtr &backup_,
                                       const ContextMutablePtr &context_, const ContextPtr &query_context_, ThreadPool &thread_pool_,
...
void RestorerFromBackup::run(Mode mode_) {
  ...
  if (!current_stage.empty()) throw Exception(ErrorCodes::LOGICAL_ERROR, "Already restoring");
  ...
  if (mode == Mode::CHECK_ACCESS_ONLY) return;
...
void RestorerFromBackup::schedule(std::function<void()> &&task_, ThreadName thread_name_) {
  ...
  auto future = scheduleFromThreadPoolUnsafe<void>(
      [this, task = std::move(task_)]() mutable {
        ...
        if (after_task_callback) after_task_callback();
      },
      thread_pool, thread_name_);
...
void RestorerFromBackup::loadSystemAccessTables() {
  if (restore_settings.structure_only) return;
  ...
  for (const auto &[table_name, table_info] : table_infos) {
    if (isSystemAccessTableName(table_name)) {
      if (!access_restorer) access_restorer = std::make_unique<AccessRestorerFromBackup>(backup, restore_settings);
      access_restorer->addDataPath(table_info.data_path_in_backup);
    }
  }

  if (access_restorer) access_restorer->loadFromBackup();
}
...
void RestorerFromBackup::checkAccessForObjectsFoundInBackup() const {
...

  {
    ...
    for (const auto &[database_name, database_info] : database_infos) {
      if (database_info.is_predefined_database) continue;

      AccessFlags flags;

      if (restore_settings.create_database != RestoreDatabaseCreationMode::kMustExist) flags |= AccessType::CREATE_DATABASE;

      if (!flags) flags = AccessType::SHOW_DATABASES;
    ...
    for (const auto &[table_name, table_info] : table_infos) {
      if (table_info.is_predefined_table) {
        if (isSystemFunctionsTableName(table_name)) {
          ...
          if (!restore_settings.structure_only && table_info.has_data) required_access.emplace_back(AccessType::CREATE_FUNCTION);
        }
    ...
    if (access_restorer) insertAtEnd(required_access, access_restorer->getRequiredAccess());
  }
  ...
  if (current_user_access_rights->contains(required_access_rights)) return;
...
void RestorerFromBackup::createAndCheckDatabases() {
  ...
  for (const auto &database_name : database_names) createAndCheckDatabase(database_name);
...
void RestorerFromBackup::createDatabase(const String &database_name) const {
  try {
    if (restore_settings.create_database == RestoreDatabaseCreationMode::kMustExist) return;
    ...
    boost::intrusive_ptr<ASTCreateQuery> create_database_query;
    {
      ...
      if (database_info.is_predefined_database) return;
    ...
    if (shared_catalog && (engine_name == "Replicated" || engine_name == "Atomic")) {
      ...
      auto engine = makeASTFunction("Replicated", make_intrusive<ASTLiteral>("/clickhouse/databases/{uuid}"),
                                    make_intrusive<ASTLiteral>("{shard}"), make_intrusive<ASTLiteral>("{replica}"));
    ...
    if (shared_catalog && SharedDatabaseCatalog::instance().shouldRestoreDatabase(create_database_query)) {
      SharedDatabaseCatalog::instance().createDatabaseRestoredFromBackup(
          database_name, create_database_query, create_query_context, restore_coordination,
    ...
    if (!restore_settings.allow_different_database_def && !is_predefined_database && !shared_migration) {
      ...
      if (!BackupUtils::compareRestoredDatabaseDef(*existing_database_def, *database_def_from_backup, context->getGlobalContext())) {
        throw Exception(ErrorCodes::CANNOT_RESTORE_DATABASE,
                        ...
                        existing_database_def->formatForErrorMessage(), database_def_from_backup->formatForErrorMessage());
      }
    }
  } catch (Exception &e) {
...
void RestorerFromBackup::removeUnresolvedDependencies() {
  ...
  auto need_exclude_dependency = [&](const StorageID &table_id) TSA_REQUIRES(mutex) -> bool {
    ...
    if (table_infos.contains(table_id.getQualifiedName())) return false;
    ...
    if (!DatabaseCatalog::instance().isTableExist(table_id, context)) {
      LOG_WARNING(log,
                  ...
                  fmt::join(tables_dependencies.getDependents(table_id), ", "), table_id, table_id);
    }
    ...
    if (num_dependencies || !num_dependents)
      throw Exception(ErrorCodes::LOGICAL_ERROR,
                      "Table {} in backup doesn't have dependencies and dependent tables as it expected to. It's a bug", table_id);
  ...
  if (tables_dependencies.hasCyclicDependencies()) {
    LOG_WARNING(log, "Tables {} in backup have cyclic dependencies: {}. Will try to ignore that and restore tables",
                fmt::join(tables_dependencies.getTablesWithCyclicDependencies(), ", "), tables_dependencies.describeCyclicDependencies());
  }
}
...
void RestorerFromBackup::createAndCheckTables() {
  ...
  for (const auto &table_ids : tables_to_create) createAndCheckTablesWithSameDependencyLevel(table_ids);
}

void RestorerFromBackup::createAndCheckTablesWithSameDependencyLevel(const std::vector<StorageID> &table_ids) {
  for (const auto &table_id : table_ids) createAndCheckTable(table_id.getQualifiedName());
  waitFutures();
}

void RestorerFromBackup::createAndCheckTable(const QualifiedTableName &table_name) {
  schedule([this, table_name]() { createAndCheckTableImpl(table_name); }, ThreadName::RESTORE_MAKE_TABLE);
}
...
void RestorerFromBackup::createTable(const QualifiedTableName &table_name) {
  try {
    if (restore_settings.create_table == RestoreTableCreationMode::kMustExist) return;
...

    {
      ...
      if (table_info.is_predefined_table) return;
    ...
    if (query_context->getSettingsRef()[Setting::restore_replicated_merge_tree_to_shared_merge_tree]) {
      ...
      if (storage != nullptr && storage->engine != nullptr)
        ...
        if (storage != nullptr && storage->engine != nullptr) boost::replace_first(storage->engine->name, "Replicated", "Shared");
      }
    }

    LOG_TRACE(log, "Creating {}: {}", BackupMetadataFinder::tableNameWithTypeToString(table_name.database, table_name.table, false),
    ...
    if (!database) {
      ...
      if (!table_info.database) table_info.database = database;
    }
    ...
    database->createTableRestoredFromBackup(create_table_query, create_query_context, restore_coordination,
                                            std::chrono::duration_cast<std::chrono::milliseconds>(create_table_timeout).count());
  } catch (Exception &e) {
...
void RestorerFromBackup::checkTable(const QualifiedTableName &table_name) {
  try {
    ...
    if (!database) database = DatabaseCatalog::instance().getDatabase(table_name.database);
    ...
    if (!restore_settings.allow_different_table_def && !is_predefined_table &&
      ...
      if (!BackupUtils::compareRestoredTableDef(*existing_table_def, *table_def_from_backup, context->getGlobalContext())) {
        throw Exception(ErrorCodes::CANNOT_RESTORE_TABLE,
                        ...
                        "comparing to its definition in the backup: {}",
                        existing_table_def->formatForErrorMessage(), table_def_from_backup->formatForErrorMessage());
      }
    }
  } catch (Exception &e) {
...
void RestorerFromBackup::insertDataToTables() {
  ...
  for (const auto &table_name : table_names) insertDataToTable(table_name);
...
void RestorerFromBackup::insertDataToTable(const QualifiedTableName &table_name) {
  ...
  schedule([this, table_name, storage, data_path_in_backup,
            partitions]() { insertDataToTableImpl(table_name, storage, data_path_in_backup, partitions); },
           ThreadName::RESTORE_TABLE_DATA);
}

void RestorerFromBackup::insertDataToTableImpl(const QualifiedTableName &table_name, StoragePtr storage, const String &data_path_in_backup,
                                               const std::optional<ASTs> &partitions) {
  try {
    if (partitions && !storage->supportsBackupPartition()) {
      throw Exception(ErrorCodes::CANNOT_RESTORE_TABLE, "Table engine {} doesn't support partitions", storage->getName());
    }
...
void RestorerFromBackup::runDataRestoreTasks() {
  ...
  for (;;) {
    ...
    if (tasks_to_run.empty()) break;

    for (auto &task : tasks_to_run) schedule(std::move(task), ThreadName::RESTORE_TABLE_TASK);
...
void RestorerFromBackup::finalizeTables() {
  std::vector<StoragePtr> tables;
  {
    ...
    for (const auto &[_, info] : table_infos) tables.push_back(info.storage);
  }

  for (const auto &storage : tables) storage->finalizeRestoreFromBackup();
}
