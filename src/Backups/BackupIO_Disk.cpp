...
BackupReaderDisk::BackupReaderDisk(const DiskPtr& disk_, const String& root_path_, const ReadSettings& read_settings_,
...
bool BackupReaderDisk::fileExists(const String& file_name) { return disk->existsFile(root_path / file_name); }

UInt64 BackupReaderDisk::getFileSize(const String& file_name) { return disk->getFileSize(root_path / file_name); }
...
void BackupReaderDisk::copyFileToDisk(const String& path_in_backup, size_t file_size, bool encrypted_in_backup, DiskPtr destination_disk,
  ...
  bool has_throttling =
      disk->isRemote() ? static_cast<bool>(read_settings.remote_throttler) : static_cast<bool>(read_settings.local_throttler);
...
BackupWriterDisk::BackupWriterDisk(const DiskPtr& disk_, const String& root_path_, const ReadSettings& read_settings_,
                                   const WriteSettings& write_settings_)
...
bool BackupWriterDisk::fileExists(const String& file_name) { return disk->existsFile(root_path / file_name); }

UInt64 BackupWriterDisk::getFileSize(const String& file_name) { return disk->getFileSize(root_path / file_name); }
...
void BackupWriterDisk::removeFile(const String& file_name) { disk->removeFileIfExists(root_path / file_name); }
...
void BackupWriterDisk::removeEmptyDirectoriesImpl(const fs::path& current_dir) {
  ...
  if (!disk->existsDirectory(current_dir)) return;
  ...
  for (auto it = disk->iterateDirectory(current_dir); it->isValid(); it->next()) removeEmptyDirectoriesImpl(current_dir / it->name());

  if (disk->isDirectoryEmpty(current_dir)) disk->removeDirectory(current_dir);
}

void BackupWriterDisk::copyFileFromDisk(const String& path_in_backup, DiskPtr src_disk, const String& src_path, bool copy_encrypted,
  ...
  bool has_throttling =
      src_disk->isRemote() ? static_cast<bool>(read_settings.remote_throttler) : static_cast<bool>(read_settings.local_throttler);
