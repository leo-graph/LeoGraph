...
class BackupCoordinationStageSync {
 public:
  ...
  BackupCoordinationStageSync(
      ...
      const Strings&
          all_hosts_,  /// all the hosts (including the initiator and the current host) performing the BACKUP/RESTORE ON CLUSTER command
      bool allow_concurrency_,  /// whether it's allowed to have concurrent backups or restores.
      BackupConcurrencyCounters& concurrency_counters_, const WithRetries& with_retries_, ThreadPoolCallbackRunnerUnsafe<void> schedule_,
 ...
 private:
  ...
  /// Atomically creates both 'start' and 'alive' nodes and also checks that there is no concurrent backup or restore if `allow_concurrency`
  /// is false.
  ...
  bool checkIfOtherHostsFinish(const String& reason, std::optional<std::chrono::milliseconds> timeout, bool time_is_out,
                               bool throw_if_error) const TSA_REQUIRES(mutex);
