...
namespace {
bool isFinishedSuccessfully(BackupStatus status) { return (status == BackupStatus::BACKUP_CREATED) || (status == BackupStatus::RESTORED); }

bool isFailed(BackupStatus status) { return (status == BackupStatus::BACKUP_FAILED) || (status == BackupStatus::RESTORE_FAILED); }

bool isCancelled(BackupStatus status) { return (status == BackupStatus::BACKUP_CANCELLED) || (status == BackupStatus::RESTORE_CANCELLED); }

bool isFailedOrCancelled(BackupStatus status) { return isFailed(status) || isCancelled(status); }

bool isFinalStatus(BackupStatus status) { return isFinishedSuccessfully(status) || isFailedOrCancelled(status); }

bool isBackupStatus(BackupStatus status) {
  return (status == BackupStatus::CREATING_BACKUP) || (status == BackupStatus::BACKUP_CREATED) || (status == BackupStatus::BACKUP_FAILED) ||
         (status == BackupStatus::BACKUP_CANCELLED);
}

BackupStatus getBackupStatusFromCurrentException() {
  if (getCurrentExceptionCode() == ErrorCodes::QUERY_WAS_CANCELLED ||
      getCurrentExceptionCode() == ErrorCodes::QUERY_WAS_CANCELLED_BY_CLIENT)
...
size_t getNumActiveBackupsChange(BackupStatus status) { return status == BackupStatus::CREATING_BACKUP; }

/// Used to change num_active_restores.
size_t getNumActiveRestoresChange(BackupStatus status) { return status == BackupStatus::RESTORING; }
...
class BackupsWorker::ThreadPools {
 public:
  ...
  ThreadPool &getThreadPool(ThreadPoolId thread_pool_id) {
    ...
    if (it != thread_pools.end()) return *it->second;
    ...
    auto thread_pool = std::make_unique<ThreadPool>(metric_threads, metric_active_threads, metric_scheduled_threads, max_threads,
                                                    max_free_threads, queue_size);
  ...
  void wait() {
    ...
    for (auto thread_pool_id : wait_sequence) {
      ThreadPool *thread_pool = nullptr;
      {
        std::lock_guard lock{mutex};
        auto it = thread_pools.find(thread_pool_id);
        if (it != thread_pools.end()) thread_pool = it->second.get();
      }
      if (thread_pool) thread_pool->wait();
    }
  }
...
ThreadPool &BackupsWorker::getThreadPool(ThreadPoolId thread_pool_id) { return thread_pools->getThreadPool(thread_pool_id); }
...
std::pair<OperationID, BackupStatus> BackupsWorker::start(const ASTPtr &backup_or_restore_query, ContextMutablePtr context) {
  ...
  if (backup_query.kind == ASTBackupQuery::Kind::BACKUP) return startMakingBackup(backup_or_restore_query, context);
...
struct BackupsWorker::BackupStarter {
  ...
  ContextPtr query_context;  /// We have to keep `query_context` until the end of the operation because a pointer to it is stored inside the
                             /// ThreadGroup we're using.
  ...
  BackupStarter(BackupsWorker &backups_worker_, const ASTPtr &query_, const ContextPtr &context_)
    ...
    if (!backup_settings.backup_uuid) backup_settings.backup_uuid = UUIDHelpers::generateV4();
    ...
    if (is_internal_backup) backup_id += "-internal-" + backup_settings.host_id;
    ...
    if (!backup_settings.data_file_name_prefix_length)
      backup_settings.data_file_name_prefix_length = backup_context->getConfigRef().getUInt64("backups.data_file_name_prefix_length", 3);
    ...
    if (process_list_element) process_list_element_holder = process_list_element->getProcessListEntry();
    ...
    if (!query_context->getSettingsRef()[Setting::s3_disable_checksum] && backup_info.backup_engine_name == "S3" &&
  ...
  std::pair<bool, BackupStatus> addInfo() {
    ...
    if (backup_settings.base_backup_info) base_backup_name = backup_settings.base_backup_info->toStringForLogging();

    auto process_list_element = backup_context->getProcessListElement();

    return backups_worker.addInfo(backup_id, backup_name_for_logging, base_backup_name, backup_context->getCurrentQueryId(),
                                  is_internal_backup, process_list_element, BackupStatus::CREATING_BACKUP);
  }
  ...
  void doBackup() {
    ...
    backups_worker.doBackup(backup, backup_query, backup_id, backup_settings, backup_coordination, backup_context, on_cluster, cluster);

    if (!is_internal_backup) backup_coordination->waitOtherHostsFinish(/* throw_if_error = */ true);
  ...
  void onException() {
    ...
    tryLogCurrentException(backups_worker.log, fmt::format("Failed to make {} {}", (is_internal_backup ? "internal backup" : "backup"),
    ...
    bool should_remove_files_in_backup =
        backup && backup_is_corrupted && backups_worker.remove_backup_files_after_failure && backup_coordination &&
        backup_coordination->isErrorSet() &&
        (backup_coordination->isBackupQuerySentToOtherHosts() ? backup_coordination->allHostsFinished() : backup_coordination->finished());

    if (should_remove_files_in_backup) backup->tryRemoveAllFiles();

    backup.reset();

    /// It's fine to remove the coordination info if the current host is the last host which was working on this backup.
    bool should_cleanup_coordination =
        backup_coordination && backup_coordination->isErrorSet() &&
        (backup_coordination->isBackupQuerySentToOtherHosts() ? backup_coordination->allHostsFinished() : backup_coordination->finished());

    if (should_cleanup_coordination) backup_coordination->cleanup(/* throw_if_error = */ false);
...
std::pair<BackupOperationID, BackupStatus> BackupsWorker::startMakingBackup(const ASTPtr &query, const ContextPtr &context) {
  ...
  if (!info_added) return {starter->backup_id, current_status};

  try {
    auto thread_pool_id =
        starter->is_internal_backup ? ThreadPoolId::ASYNC_BACKGROUND_INTERNAL_BACKUP : ThreadPoolId::ASYNC_BACKGROUND_BACKUP;
...
BackupMutablePtr BackupsWorker::openBackupForWriting(const BackupInfo &backup_info, const BackupSettings &backup_settings,
                                                     std::shared_ptr<IBackupCoordination> backup_coordination,
...
void BackupsWorker::doBackup(BackupMutablePtr backup, const boost::intrusive_ptr<ASTBackupQuery> &backup_query,
                             const OperationID &backup_id, const BackupSettings &backup_settings,
                             std::shared_ptr<IBackupCoordination> backup_coordination, ContextMutablePtr context, bool on_cluster,
  ...
  if (on_cluster && !is_internal_backup) {
    ...
    sendQueryToOtherHosts(*backup_query, cluster, backup_settings.shard_num, backup_settings.replica_num, context, required_access,
                          backup_coordination->getOnClusterInitializationKeeperRetriesInfo());
    ...
    {
      BackupEntriesCollector backup_entries_collector(backup_query->elements, backup_settings, backup_id, backup_coordination,
    ...
    writeBackupEntries(backup, std::move(backup_entries), backup_id, backup_coordination, is_internal_backup,
...
void BackupsWorker::buildFileInfosForBackupEntries(const BackupPtr &backup, const BackupEntries &backup_entries,
                                                   const ReadSettings &read_settings,
                                                   std::shared_ptr<IBackupCoordination> backup_coordination,
                                                   QueryStatusPtr process_list_element) {
  backup_coordination->setStage(Stage::BUILDING_FILE_INFOS, "", /* sync = */ true);
  backup_coordination->addFileInfos(::DB::buildFileInfosForBackupEntries(backup_entries, backup->getBaseBackup(), read_settings,
                                                                         getThreadPool(ThreadPoolId::BACKUP), process_list_element));
}

void BackupsWorker::writeBackupEntries(BackupMutablePtr backup, BackupEntries &&backup_entries, const OperationID &backup_id,
                                       std::shared_ptr<IBackupCoordination> backup_coordination, bool is_internal_backup,
                                       QueryStatusPtr process_list_element) {
  ...
  if (file_infos.size() != backup_entries.size()) {
    throw Exception(ErrorCodes::LOGICAL_ERROR, "Number of file infos ({}) doesn't match the number of backup entries ({})",
                    file_infos.size(), backup_entries.size());
  }
  ...
  for (size_t i = 0; i != backup_entries.size(); ++i) {
    ...
    auto job = [&failed, &process_list_element, &backup, &file_info, &entry, this, is_internal_backup, &backup_id]() {
      ...
      try {
        ...
        // Update metadata
        if (!is_internal_backup) {
          setNumFilesAndSize(backup_id, backup->getNumFiles(), backup->getTotalSize(), backup->getNumEntries(),
                             backup->getUncompressedSize(), backup->getCompressedSize(), 0, 0);
        }
      } catch (...) {
        failed = true;
        throw;
...
struct BackupsWorker::RestoreStarter {
  ...
  RestoreStarter(BackupsWorker &backups_worker_, const ASTPtr &query_, const ContextPtr &context_)
    ...
    if (!restore_settings.restore_uuid) restore_settings.restore_uuid = UUIDHelpers::generateV4();

    /// `restore_id` will be used as a key to the `infos` map, so it should be unique.
    if (is_internal_restore)
      restore_id =
          "internal-" + toString(UUIDHelpers::generateV4());  /// Always generate `restore_id` for internal restore to avoid collision if
                                                              /// both internal and non-internal restores are on the same host
  ...
  std::pair<bool, BackupStatus> addInfo() {
    String base_backup_name;
    if (restore_settings.base_backup_info) base_backup_name = restore_settings.base_backup_info->toStringForLogging();

    auto process_list_element = restore_context->getProcessListElement();

    return backups_worker.addInfo(restore_id, backup_name_for_logging, base_backup_name, restore_context->getCurrentQueryId(),
                                  is_internal_restore, process_list_element, BackupStatus::RESTORING);
  }
  ...
  void doRestore() {
    ...
    if (!is_internal_restore) restore_coordination->waitOtherHostsFinish(/* throw_if_error = */ true);
  ...
  void onException() {
    ...
    tryLogCurrentException(backups_worker.log, fmt::format("Failed to restore from {} {}",
    ...
    bool should_cleanup_coordination = restore_coordination && restore_coordination->isErrorSet() &&
                                       (restore_coordination->isRestoreQuerySentToOtherHosts() ? restore_coordination->allHostsFinished()
                                                                                               : restore_coordination->finished());

    if (should_cleanup_coordination) restore_coordination->cleanup(/* throw_if_error = */ false);
...
std::pair<BackupOperationID, BackupStatus> BackupsWorker::startRestoring(const ASTPtr &query, ContextMutablePtr context) {
  ...
  auto [info_added, current_status] = starter->addInfo();
  if (!info_added) return {starter->restore_id, current_status};

  try {
    auto thread_pool_id =
        starter->is_internal_restore ? ThreadPoolId::ASYNC_BACKGROUND_INTERNAL_RESTORE : ThreadPoolId::ASYNC_BACKGROUND_RESTORE;
...
BackupPtr BackupsWorker::openBackupForReading(const BackupInfo &backup_info, const RestoreSettings &restore_settings,
                                              const ContextPtr &context) const {
...
void BackupsWorker::doRestore(const boost::intrusive_ptr<ASTBackupQuery> &restore_query, const OperationID &restore_id,
                              const BackupInfo &backup_info, RestoreSettings restore_settings,
                              std::shared_ptr<IRestoreCoordination> restore_coordination, ContextMutablePtr context,
                              const ContextPtr &query_context, bool on_cluster, const ClusterPtr &cluster) {
  ...
  if (on_cluster && !is_internal_restore) {
    ...
    for (const auto *address : addresses) {
      ...
      for (auto &element : restore_elements) element.setCurrentDatabase(addr_database);
      RestorerFromBackup dummy_restorer{
          restore_elements, restore_settings, nullptr, backup, context, query_context, getThreadPool(ThreadPoolId::RESTORE), {}};
  ...
  if (on_cluster && !is_internal_restore) {
    ...
    sendQueryToOtherHosts(*restore_query, cluster, restore_settings.shard_num, restore_settings.replica_num, context, {},
                          restore_coordination->getOnClusterInitializationKeeperRetriesInfo());
    ...
    auto after_task_callback = [&] {
      maybeSleepForTesting();
      setNumFilesAndSize(restore_id, backup->getNumFiles(), backup->getTotalSize(), backup->getNumEntries(), backup->getUncompressedSize(),
                         backup->getCompressedSize(), backup->getNumReadFiles(), backup->getNumReadBytes());
    };

    /// Restore from the backup.
    RestorerFromBackup restorer{restore_query->elements,
                                restore_settings,
                                restore_coordination,
                                backup,
                                context,
                                query_context,
                                getThreadPool(ThreadPoolId::RESTORE),
                                after_task_callback};
    restorer.run(RestorerFromBackup::RESTORE);
  }
}

void BackupsWorker::sendQueryToOtherHosts(const ASTBackupQuery &backup_or_restore_query, const ClusterPtr &cluster, size_t only_shard_num,
                                          size_t only_replica_num, ContextMutablePtr context, const AccessRightsElements &access_to_check,
...
std::shared_ptr<IBackupCoordination> BackupsWorker::makeBackupCoordination(bool on_cluster, const BackupSettings &backup_settings,
                                                                           const ContextPtr &context) const {
  if (!on_cluster) {
  ...
  auto thread_pool_id =
      is_internal_backup ? ThreadPoolId::ON_CLUSTER_COORDINATION_INTERNAL_BACKUP : ThreadPoolId::ON_CLUSTER_COORDINATION_BACKUP;
  ...
  return std::make_shared<BackupCoordinationOnCluster>(backup_settings, root_zk_path, get_zookeeper, keeper_settings, current_host,
                                                       all_hosts, allow_concurrent_backups, *concurrency_counters, schedule,
                                                       context->getProcessListElement());
}

std::shared_ptr<IRestoreCoordination> BackupsWorker::makeRestoreCoordination(bool on_cluster, const RestoreSettings &restore_settings,
                                                                             const ContextPtr &context) const {
  if (!on_cluster) {
  ...
  auto thread_pool_id =
      is_internal_restore ? ThreadPoolId::ON_CLUSTER_COORDINATION_INTERNAL_RESTORE : ThreadPoolId::ON_CLUSTER_COORDINATION_RESTORE;
  auto thread_name = is_internal_restore ? ThreadName::RESTORE_COORDINATION_INTERNAL : ThreadName::RESTORE_COORDINATION;
  auto schedule = threadPoolCallbackRunnerUnsafe<void>(thread_pools->getThreadPool(thread_pool_id), thread_name);

  return std::make_shared<RestoreCoordinationOnCluster>(*restore_settings.restore_uuid, root_zk_path, get_zookeeper, keeper_settings,
                                                        current_host, all_hosts, allow_concurrent_restores, *concurrency_counters, schedule,
                                                        context->getProcessListElement());
}
...
std::pair<bool, BackupStatus> BackupsWorker::addInfo(const OperationID &id, const String &name, const String &base_backup_name,
  ...
  if (process_list_element) {
    info.profile_counters =
        process_list_element->getInfo(/* get_thread_list= */ false, /* get_profile_events= */ true, /* get_settings= */ false)
            .profile_counters;
    if (!is_final_status) extended_info.process_list_element = process_list_element;
  }

  if (is_final_status) info.end_time_us = info.start_time_us;
  ...
  if (it != infos.end()) {
    ...
    throw Exception(ErrorCodes::BAD_ARGUMENTS, "Cannot start {} with ID {} because there is another {} with the same ID",
                    isBackupStatus(status) ? "backup" : "restore", quoteString(id), isBackupStatus(current_status) ? "backup" : "restore");
  }

  if (backup_log) backup_log->add(BackupLogElement{info});
...
void BackupsWorker::setStatus(const String &id, BackupStatus status, bool throw_if_error) {
  ...
  auto it = infos.find(id);
  if (it == infos.end()) {
    if (throw_if_error) throw Exception(ErrorCodes::LOGICAL_ERROR, "Unknown backup ID {}", id);
    return;
  }
  ...
  if (extended_info.process_list_element) {
    info.profile_counters =
        extended_info.process_list_element->getInfo(/* get_thread_list= */ false, /* get_profile_events= */ true, /* get_settings= */ false)
            .profile_counters;
    if (is_final_status) extended_info.process_list_element = nullptr;
  }

  if (is_final_status) info.end_time_us = timeInMicroseconds(std::chrono::system_clock::now());
  ...
  if (status != old_status) status_changed.notify_all();
}
...

{
  ...
  std::lock_guard lock{infos_mutex};
  auto it = infos.find(id);
  if (it == infos.end()) throw Exception(ErrorCodes::LOGICAL_ERROR, "Unknown backup ID {}", id);
...
void BackupsWorker::maybeSleepForTesting() const {
  if (test_inject_sleep) sleepForSeconds(1);
}
...
BackupStatus BackupsWorker::wait(const OperationID &backup_or_restore_id, bool rethrow_exception) {
  ...
  status_changed.wait(lock, [&] {
    ...
    if (it == infos.end()) throw Exception(ErrorCodes::LOGICAL_ERROR, "Unknown backup ID {}", backup_or_restore_id);
    const auto &info = it->second.info;
    current_status = info.status;
    if (rethrow_exception && isFailedOrCancelled(current_status)) std::rethrow_exception(info.exception);
    if (isFinalStatus(current_status)) return true;
...
void BackupsWorker::waitAll() {
  std::vector<OperationID> current_operations;
  {
    ...
    for (const auto &[id, extended_info] : infos)
      if (!isFinalStatus(extended_info.info.status)) current_operations.push_back(id);
  }

  if (current_operations.empty()) return;

  LOG_INFO(log, "Waiting for running backups and restores to finish");

  for (const auto &id : current_operations) wait(id, /* rethrow_exception= */ false);
...
BackupStatus BackupsWorker::cancel(const BackupOperationID &backup_or_restore_id, bool wait_) {
...

  {
    ...
    current_status = info.status;
    if (isFinalStatus(current_status) || !extended_info.process_list_element) return current_status;
  ...
  if (!wait_) return current_status;
...
void BackupsWorker::cancelAll(bool wait_) {
  ...
  for (const auto &id : current_operations) cancel(id, /* wait= */ false);
...
std::vector<BackupOperationInfo> BackupsWorker::getAllInfos() const {
  ...
  for (const auto &extended_info : infos | boost::adaptors::map_values) {
    const auto &info = extended_info.info;
    if (!info.internal) res_infos.push_back(info);
  }
  return res_infos;
}
