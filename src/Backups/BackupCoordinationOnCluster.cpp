...
size_t BackupCoordinationOnCluster::findCurrentHostIndex(const String& current_host, const Strings& all_hosts) {
  ...
  if (it == all_hosts.end()) return all_hosts.size();
  return it - all_hosts.begin();
}

BackupCoordinationOnCluster::BackupCoordinationOnCluster(const BackupSettings& backup_settings_, const String& root_zookeeper_path_,
                                                         zkutil::GetZooKeeper get_zookeeper_, const BackupKeeperSettings& keeper_settings_,
                                                         const String& current_host_, const Strings& all_hosts_,
                                                         bool allow_concurrent_backup_, BackupConcurrencyCounters& concurrency_counters_,
    ...
    : backup_uuid(*backup_settings_.backup_uuid),
      ...
      with_retries(log, get_zookeeper_, keeper_settings, process_list_element_,
                   [root_zookeeper_path_](Coordination::ZooKeeperWithFaultInjection::Ptr zk) { zk->sync(root_zookeeper_path_); }),
      cleaner(/* is_restore = */ false, zookeeper_path, with_retries, log),
      stage_sync(/* is_restore = */ false, fs::path{zookeeper_path} / "stage", current_host, all_hosts, allow_concurrent_backup_,
                 concurrency_counters_, with_retries, schedule_, process_list_element_, log) {
  /// If the current host isn't the initiator then there are other hosts working on this backup (at least the initiator itself).
  if (current_host != kInitiator) setBackupQueryIsSentToOtherHosts();
}
...
void BackupCoordinationOnCluster::setBackupQueryIsSentToOtherHosts() { stage_sync.setQueryIsSentToOtherHosts(); }

bool BackupCoordinationOnCluster::isBackupQuerySentToOtherHosts() const { return stage_sync.isQuerySentToOtherHosts(); }
...
bool BackupCoordinationOnCluster::isErrorSet() const { return stage_sync.isErrorSet(); }

void BackupCoordinationOnCluster::waitOtherHostsFinish(bool throw_if_error) const { stage_sync.waitOtherHostsFinish(throw_if_error); }

void BackupCoordinationOnCluster::finish(bool throw_if_error) { stage_sync.finish(throw_if_error); }

bool BackupCoordinationOnCluster::finished() const { return stage_sync.finished(); }

bool BackupCoordinationOnCluster::allHostsFinished() const { return stage_sync.allHostsFinished(); }

void BackupCoordinationOnCluster::cleanup(bool throw_if_error) { cleaner.cleanup(throw_if_error); }
...
ZooKeeperRetriesInfo BackupCoordinationOnCluster::getOnClusterInitializationKeeperRetriesInfo() const {
  return ZooKeeperRetriesInfo{keeper_settings.max_retries_while_initializing,
                              ...
                              static_cast<UInt64>(keeper_settings.retry_max_backoff_ms.count()), process_list_element};
}
...
void BackupCoordinationOnCluster::serializeToMultipleZooKeeperNodes(const String& path, const String& value, const String& logging_name) {
  ...
  if (!max_part_size) max_part_size = value.size();
...
void BackupCoordinationOnCluster::addReplicatedPartNames(const String& table_zk_path, const String& table_name_for_logs,
                                                         const String& replica_name,
                                                         const std::vector<PartNameAndChecksum>& part_names_and_checksums) {
  {
    std::lock_guard lock{replicated_tables_mutex};
    if (replicated_tables) throw Exception(ErrorCodes::LOGICAL_ERROR, "addReplicatedPartNames() must not be called after preparing");
  }
...
void BackupCoordinationOnCluster::addReplicatedMutations(const String& table_zk_path, const String& table_name_for_logs,
                                                         const String& replica_name, const std::vector<MutationInfo>& mutations) {
  {
    std::lock_guard lock{replicated_tables_mutex};
    if (replicated_tables) throw Exception(ErrorCodes::LOGICAL_ERROR, "addReplicatedMutations() must not be called after preparing");
  }
...
std::vector<IBackupCoordination::MutationInfo> BackupCoordinationOnCluster::getReplicatedMutations(const String& table_zk_path,
...
void BackupCoordinationOnCluster::addReplicatedDataPath(const String& table_zk_path, const String& data_path) {
  {
    std::lock_guard lock{replicated_tables_mutex};
    if (replicated_tables) throw Exception(ErrorCodes::LOGICAL_ERROR, "addReplicatedDataPath() must not be called after preparing");
  }
...
void BackupCoordinationOnCluster::prepareReplicatedTables() const {
    ...
    holder.retries_ctl.retryLoop([&, &zk = holder.faulty_zookeeper]() {
      ...
      for (const String& escaped_table_zk_path : zk->getChildren(path)) {
        ...
        for (const String& escaped_replica_name : zk->getChildren(path2)) {
          ...
          mutations_for_replicated_tables.push_back({table_zk_path, mutations.table_name_for_logs, replica_name, mutations.mutations});
        }
      }
    });
  }
  ...
  for (auto& part_names : part_names_for_replicated_tables) replicated_tables->addPartNames(std::move(part_names));
  for (auto& mutations : mutations_for_replicated_tables) replicated_tables->addMutations(std::move(mutations));
  for (auto& data_paths : data_paths_for_replicated_tables) replicated_tables->addDataPath(std::move(data_paths));
}

void BackupCoordinationOnCluster::addReplicatedAccessFilePath(const String& access_zk_path, AccessEntityType access_entity_type,
                                                              const String& file_path) {
  {
    std::lock_guard lock{replicated_access_mutex};
    if (replicated_access) throw Exception(ErrorCodes::LOGICAL_ERROR, "addReplicatedAccessFilePath() must not be called after preparing");
  }
...
void BackupCoordinationOnCluster::prepareReplicatedAccess() const {
  ...
  for (auto& file_path : file_path_for_access_entities) replicated_access->addFilePath(std::move(file_path));
}

void BackupCoordinationOnCluster::addReplicatedSQLObjectsDir(const String& loader_zk_path, UserDefinedSQLObjectType object_type,
...
void BackupCoordinationOnCluster::prepareReplicatedSQLObjects() const {
  if (replicated_sql_objects) return;
  ...
  for (auto& directory : directories_for_sql_objects) replicated_sql_objects->addDirectory(std::move(directory));
}

void BackupCoordinationOnCluster::addKeeperMapTable(const String& table_zookeeper_root_path, const String& table_id,
                                                    const String& data_path_in_backup) {
  {
    std::lock_guard lock{keeper_map_tables_mutex};
    if (keeper_map_tables) throw Exception(ErrorCodes::LOGICAL_ERROR, "addKeeperMapTable() must not be called after preparing");
  }
...
void BackupCoordinationOnCluster::prepareKeeperMapTables() const {
  ...
  holder.retries_ctl.retryLoop([&, &zk = holder.faulty_zookeeper]() {
    ...
    for (auto& table : tables) table = tables_path / table;
    ...
    for (size_t i = 0; i < tables_info.size(); ++i) {
      ...
      keeper_map_table_infos.emplace_back(std::move(data[0]),
                                          BackupCoordinationKeeperMapTables::KeeperMapTableInfo{.table_id = fs::path(tables[i]).filename(),
                                                                                                .data_path_in_backup = std::move(data[1])});
    }
  });
...
void BackupCoordinationOnCluster::addFileInfos(BackupFileInfos&& file_infos_) {
  {
    ...
    if (file_infos) throw Exception(ErrorCodes::LOGICAL_ERROR, "addFileInfos() must not be called after preparing");
  }
...
bool BackupCoordinationOnCluster::startWritingFile(size_t data_file_index) {
  {
    ...
    if (writing_files.contains(data_file_index)) return false;
  }
  ...
  if (!host_is_assigned) return false;  /// Other host is writing this file.
