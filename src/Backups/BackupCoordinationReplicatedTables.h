...
class BackupCoordinationReplicatedTables {
 ...
 private:
  ...
  struct TableInfo {
    String table_name_for_logs;
    std::map<String /* part_name */, PartReplicas>
        replicas_by_part_name;  /// Should be ordered because we need this map to be in the same order on every replica.
  ...
  std::map<String /* table_zk_path */, TableInfo>
      table_infos;  /// Should be ordered because we need this map to be in the same order on every replica.
