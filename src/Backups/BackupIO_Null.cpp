...
std::unique_ptr<WriteBuffer> BackupWriterNull::writeFile(const String& /* file_name */) { return std::make_unique<NullWriteBuffer>(); }

void BackupWriterNull::copyDataToFile(const String& /* path_in_backup */, const CreateReadBufferFunction& /* create_read_buffer */,
                                      UInt64 /* start_pos */, UInt64 /* length */) {
  /// no op
}

void BackupWriterNull::copyFileFromDisk(const String& /* path_in_backup */, DiskPtr /* src_disk */, const String& /* src_path */,
...
bool BackupWriterNull::fileExists(const String& /* file_name */) { return false; }
...
bool BackupWriterNull::fileContentsEqual(const String& file_name, const String& /* expected_file_contents */,
  ...
  if (fs::path{file_name}.filename() == ".lock") return true;  /// To pass the check for the ".lock" file in BackupImpl::checkLockFile().
...
void registerBackupEngineNull(BackupFactory& factory) {
  auto creator_fn = [](const BackupFactory::CreateParams& params) -> std::unique_ptr<IBackup> {
    ...
    if (!args.empty()) throw Exception(ErrorCodes::NUMBER_OF_ARGUMENTS_DOESNT_MATCH, "Backup engine 'Null' doesn't take arguments");

    if (params.open_mode == IBackup::OpenMode::READ)
      throw Exception(ErrorCodes::BACKUP_NOT_FOUND, "Backup {} not found (cannot restore from Null)",
                      params.backup_info.toStringForLogging());
