...
const std::vector<std::pair<String, Int8>>& getBackupStatusEnumValues() {
  static const std::vector<std::pair<String, Int8>> values = [] {
    ...
    for (auto status : collections::range(BackupStatus::MAX)) res.emplace_back(toString(status), static_cast<Int8>(status));
    return res;
  }();
  return values;
}
...
const std::vector<std::pair<String, Int8>>& getSnapshotLockStatusEnumValues() {
  static const std::vector<std::pair<String, Int8>> values = [] {
    ...
    res.emplace_back(magic_enum::enum_name(SnapshotLockStatus::SNAPSHOT_UNLOCKING),
                     static_cast<Int8>(SnapshotLockStatus::SNAPSHOT_UNLOCKING));
    return res;
  }();
  return values;
}
