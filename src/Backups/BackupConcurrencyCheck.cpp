...
BackupConcurrencyCheck::BackupConcurrencyCheck(bool is_restore_, bool on_cluster_, const String& zookeeper_path_, bool allow_concurrency_,
  ...
  if (!allow_concurrency_) {
    ...
    if (is_restore) {
      ...
      if (on_cluster) {
        if (!counters.on_cluster_restores.contains(zookeeper_path)) ++num_on_cluster_restores;
      } else {
        ++num_local_restores;
      }
      ...
      if (on_cluster) {
        if (!counters.on_cluster_backups.contains(zookeeper_path)) ++num_on_cluster_backups;
      } else {
        ++num_local_backups;
      }
    ...
    if (found_concurrent_operation) throwConcurrentOperationNotAllowed(is_restore);
  }
  ...
  if (on_cluster) {
    if (is_restore) {
      ...
      if (it != counters.on_cluster_restores.end()) {
        if (!--it->second) counters.on_cluster_restores.erase(it);
      }
    } else {
      auto it = counters.on_cluster_backups.find(zookeeper_path);
      if (it != counters.on_cluster_backups.end()) {
        if (!--it->second) counters.on_cluster_backups.erase(it);
      }
    }
  } else {
    if (is_restore)
...
void BackupConcurrencyCheck::throwConcurrentOperationNotAllowed(bool is_restore) {
  throw Exception(ErrorCodes::CONCURRENT_ACCESS_NOT_SUPPORTED, "Concurrent {} are not allowed, turn on setting '{}'",
                  is_restore ? "restores" : "backups", is_restore ? "allow_concurrent_restores" : "allow_concurrent_backups");
}
