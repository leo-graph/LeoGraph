...
WithRetries::WithRetries(LoggerPtr log_, zkutil::GetZooKeeper get_zookeeper_, const BackupKeeperSettings& settings_,
                         QueryStatusPtr process_list_element_, RenewerCallback callback_)
    : log(log_), get_zookeeper(get_zookeeper_), settings(settings_), process_list_element(process_list_element_), callback(callback_) {}
...
WithRetries::RetriesControlHolder::RetriesControlHolder(const WithRetries* parent, const String& name, Kind kind)
    : info((kind == kInitialization)  ? parent->settings.max_retries_while_initializing
           ...
           parent->settings.retry_initial_backoff_ms.count(), parent->settings.retry_max_backoff_ms.count(),
...
void WithRetries::renewZooKeeper(FaultyKeeper my_faulty_zookeeper) const {
  ...
  if (!zookeeper || zookeeper->expired()) {
    ...
    if (callback) callback(my_faulty_zookeeper);
  } else {
    my_faulty_zookeeper->setKeeper(zookeeper);
  }
}

const BackupKeeperSettings& WithRetries::getKeeperSettings() const { return settings; }
...
WithRetries::FaultyKeeper WithRetries::getFaultyZooKeeper() const {
  ...
  auto faulty_zookeeper = ZooKeeperWithFaultInjection::createInstance(settings.fault_injection_probability, settings.fault_injection_seed,
                                                                      current_zookeeper, log->name(), log);
