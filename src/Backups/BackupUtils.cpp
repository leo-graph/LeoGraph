...
bool compareRestoredTableDef(const IAST &restored_table_create_query, const IAST &create_query_from_backup,
...
bool compareRestoredDatabaseDef(const IAST &restored_database_create_query, const IAST &create_query_from_backup,
                                const ContextPtr &global_context) {
  return compareRestoredTableDef(restored_database_create_query, create_query_from_backup, global_context);
}

bool isInnerTable(const QualifiedTableName &table_name) { return isInnerTable(table_name.database, table_name.table); }
...
bool isInnerTable(const String & /* database_name */, const String &table_name) {
  ...
  return table_name.starts_with(".inner.") || table_name.starts_with(".inner_id.") || table_name.starts_with(".tmp.inner.") ||
         table_name.starts_with(".tmp.inner_id.");
}
...
bool isTargetForReplaceRefreshableMaterializedView(const StorageID &storage_id, const ContextPtr &context) {
  ...
  auto is_rmv_targeting_table = [&](const StorageID &mv_candidate, const StorageID &target_id) -> bool {
    ...
    if (!table || table->getName() != "MaterializedView") return false;
  ...
  return std::any_of(dependents.begin(), dependents.end(),
                     [&](const auto &dependent) { return is_rmv_targeting_table(dependent, storage_id); });
}
}  // namespace DB::BackupUtils
