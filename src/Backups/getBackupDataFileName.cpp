...
std::string getBackupDataFileName(const BackupFileInfo& file_info, BackupDataFileNameGeneratorType data_file_name_generator,
  ...
  switch (data_file_name_generator) {
    ...
    case BackupDataFileNameGeneratorType::Checksum: {
      ...
      if (prefix_length == 0 || prefix_length >= checksum_hex.size()) return checksum_hex;
