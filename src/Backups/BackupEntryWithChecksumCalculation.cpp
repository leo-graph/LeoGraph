...
UInt128 combineChecksums(UInt128 checksum1, UInt128 second_checksum) {
  ...
  return CityHash_v1_0_2::CityHash128WithSeed(reinterpret_cast<const char *>(&checksum1), sizeof(checksum1),
                                              {second_checksum.items[0], second_checksum.items[1]});
}
}  // namespace
...
UInt128 BackupEntryWithChecksumCalculation::getChecksum(const ReadSettings &read_settings) const {
  {
    ...
    if (calculated_checksum) return *calculated_checksum;
  }
...
std::optional<UInt128> BackupEntryWithChecksumCalculation::getPartialChecksum(UInt64 limit, const ReadSettings &read_settings) const {
  UInt64 size = getSize();
  if (limit >= size) return getChecksum(read_settings);
  ...
  if (has_calculated_full_checksum) return calculateChecksum(limit, read_settings);
...
bool BackupEntryWithChecksumCalculation::hasPrecalculatedChecksum() const { return getPrecalculatedChecksum().has_value(); }
...
std::pair<std::optional<UInt128>, std::optional<UInt128>> BackupEntryWithChecksumCalculation::getPrecalculatedChecksumIfFull(
    UInt64 limit, std::optional<UInt64> second_limit) const {
  UInt64 size = getSize();
  limit = std::min(limit, size);
  if (second_limit) second_limit = std::min(*second_limit, size);

  std::optional<UInt128> checksum;
  if (limit == size) checksum = getPrecalculatedChecksum().value();

  std::optional<UInt128> second_checksum;
  if (second_limit == size) second_checksum = getPrecalculatedChecksum().value();

  return {checksum, second_checksum};
}

std::pair<std::optional<UInt128>, std::optional<UInt128>> BackupEntryWithChecksumCalculation::combinePrecalculatedChecksumWithEncryptionIV(
    UInt64 limit, std::optional<UInt64> second_limit) const {
  auto [checksum, second_checksum] = getPrecalculatedChecksumIfFull(limit, second_limit);

  UInt128 iv;
  if (checksum || second_checksum) iv = getDisk()->getEncryptedFileIV(getFilePath());

  if (checksum) checksum = combineChecksums(*checksum, iv);

  if (second_checksum) second_checksum = combineChecksums(*second_checksum, iv);
...
std::pair<std::optional<UInt128>, std::optional<UInt128>> BackupEntryWithChecksumCalculation::calculateChecksumFromReading(
  ...
  auto calculate_hash = [&](UInt64 limit_) -> std::optional<UInt128> {
    if ((limit_ != size) && !isPartialChecksumAllowed()) return {};
    if (limit_ == offset) return current_hash;
    ...
    if (offset != limit_) {
      ...
      throw Exception(ErrorCodes::UNEXPECTED_END_OF_FILE,
                      "Size of file {} decreased ({} -> {}) unexpectedly while calculating its checksum", getFilePath(), size, offset);
    }
    if ((limit_ == size) && !isPartialChecksumAllowed() && !hashing_read_buffer->eof()) {
      ...
      throw Exception(ErrorCodes::EXPECTED_END_OF_FILE, "Size of immutable file {} increased unexpectedly while calculating its checksum",
                      getFilePath());
    }
  ...
  std::optional<UInt128> second_checksum;
  if (second_limit) second_checksum = calculate_hash(*second_limit);
...
std::pair<std::optional<UInt128>, std::optional<UInt128>> BackupEntryWithChecksumCalculation::calculateChecksumFromRemotePath(
  ...
  auto calculate_hash = [&](UInt64 limit_) -> std::optional<UInt128> {
    if ((limit_ != size) && !isPartialChecksumAllowed()) return {};
    if (offset == limit_) return current_hash;
    if (!stored_objects) stored_objects = getDisk()->getStorageObjects(getFilePath());
    offset = 0;
    UInt128 hash = 0;
    size_t index = 0;
    while (offset != limit_) {
      if (index == stored_objects->size()) {
        ...
        /// or append-only.
        throw Exception(ErrorCodes::UNEXPECTED_END_OF_FILE,
                        "Size of file {} decreased ({} -> {}) unexpectedly while calculating its checksum", getFilePath(), size,
                        getTotalSize(*stored_objects));
      }
      ...
      if (bytes_count) {
        ...
        hash = CityHash_v1_0_2::CityHash128WithSeed(reinterpret_cast<const char *>(&bytes_count), sizeof(bytes_count),
