...
std::optional<SizeAndChecksum> getInfoAboutFileFromBaseBackupIfExists(const BackupPtr& base_backup, const std::string& file_path) {
  if (base_backup && base_backup->fileExists(file_path)) return base_backup->getFileSizeAndChecksum(file_path);
...
CheckBackupResult checkBaseBackupForFile(const SizeAndChecksum& base_backup_info, const BackupFileInfo& new_entry_info) {
  ...
  if ((new_entry_info.size < base_backup_info.first) || !base_backup_info.first) return CheckBackupResult::HasNothing;

  if (base_backup_info.first == new_entry_info.size) return CheckBackupResult::HasFull;
...
ChecksumsForNewEntry calculateNewEntryChecksumsIfNeeded(const BackupEntryPtr& entry, UInt64 prefix_size,
...
String BackupFileInfo::describe() const {
  ...
  if (!reference_target.empty()) result += fmt::format("reference_target: {};\n", reference_target);
  return result;
}

BackupFileInfo buildFileInfoForBackupEntry(const String& file_name, const BackupEntryPtr& backup_entry, const BackupPtr& base_backup,
                                           const ReadSettings& read_settings, LoggerPtr log) {
  ...
  if (!log) log = getLogger("FileInfoFromBackupEntry");
  ...
  if (base_backup && !info.base_size) LOG_TRACE(log, "Nothing found for file {} in base backup", adjusted_path);

  return info;
}

BackupFileInfos buildFileInfosForBackupEntries(const BackupEntries& backup_entries, const BackupPtr& base_backup,
                                               const ReadSettings& read_settings, ThreadPool& thread_pool,
