...
BackupCoordinationCleaner::BackupCoordinationCleaner(bool is_restore_, const String& zookeeper_path_, const WithRetries& with_retries_,
                                                     LoggerPtr log_)
...
void BackupCoordinationCleaner::cleanupImpl(bool throw_if_error, WithRetries::Kind retries_kind) {
  ...
  try {
    ...
    LOG_TRACE(log, "Caught exception while removing nodes from ZooKeeper for this {}: {}", is_restore ? "restore" : "backup",
