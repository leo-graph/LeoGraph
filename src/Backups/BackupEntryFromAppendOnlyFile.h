...
class BackupEntryFromAppendOnlyFile : public BackupEntryWithChecksumCalculation {
 public:
  ...
  BackupEntryFromAppendOnlyFile(const DiskPtr& disk_, const String& file_path_, bool copy_encrypted_ = false,
                                const std::optional<UInt64>& file_size_ = {}, bool allow_checksum_from_remote_path_ = true);
