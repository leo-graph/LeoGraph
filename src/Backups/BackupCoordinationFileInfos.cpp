...
void BackupCoordinationFileInfos::addFileInfos(BackupFileInfos &&file_infos_, const String &host_id_) {
  if (prepared) throw Exception(ErrorCodes::LOGICAL_ERROR, "addFileInfos() must not be called after preparing");
...
BackupFileInfos BackupCoordinationFileInfos::getFileInfosForAllHosts() const {
  ...
  for (const auto *file_info : file_infos_for_all_hosts) res.emplace_back(*file_info);
  return res;
}
...
void BackupCoordinationFileInfos::prepare() const {
  ...
  for (const auto &[_, infos] : file_infos) total_num_infos += infos.size();

  file_infos_for_all_hosts.reserve(total_num_infos);
  for (auto &[_, infos] : file_infos)
    for (auto &info : infos) file_infos_for_all_hosts.emplace_back(&info);
  ...
  auto adjacent_it =
      std::adjacent_find(file_infos_for_all_hosts.begin(), file_infos_for_all_hosts.end(), BackupFileInfo::EqualByFileName{});
  if (adjacent_it != file_infos_for_all_hosts.end()) {
    throw Exception(ErrorCodes::BACKUP_ENTRY_ALREADY_EXISTS, "Entry {} added multiple times to backup",
                    quoteString((*adjacent_it)->file_name));
  }
  ...
  const auto handle_unresolved_references = [&](const auto &try_resolve_reference) {
    for (auto *reference : unresolved_references) {
      if (!try_resolve_reference(*reference))
        throw DB::Exception(ErrorCodes::LOGICAL_ERROR, "Couldn't resolve reference {} with target {}", reference->file_name,
  ...
  if (config.plain_backup) {
    const auto try_resolve_reference = [&](BackupFileInfo &reference) {
      ...
      if (it == file_name_to_info.end()) return false;
    ...
    for (size_t i = 0; i != file_infos_for_all_hosts.size(); ++i) {
      ...
      if (!info.reference_target.empty()) {
        if (!try_resolve_reference(info)) unresolved_references.push_back(&info);
