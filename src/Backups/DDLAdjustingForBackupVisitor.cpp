...
void visitStorageReplicatedTableEngine(ASTStorage &storage, const DDLAdjustingForBackupVisitor::Data &data) {
  ...
  if (!engine_args_ast) return;

  auto &engine_args = engine_args_ast->children;
  if (engine_args.size() < 2) return;
  ...
  if (zookeeper_path_ast && (zookeeper_path_ast->value.getType() == Field::Types::String) && replica_name_ast &&
      (replica_name_ast->value.getType() == Field::Types::String)) {
...
void visitCreateQuery(ASTCreateQuery &create, const DDLAdjustingForBackupVisitor::Data &data) {
  if (create.storage) visitStorage(*create.storage, data);
}
}  // namespace

bool DDLAdjustingForBackupVisitor::needChildVisit(const ASTPtr &, const ASTPtr &) { return false; }

void DDLAdjustingForBackupVisitor::visit(ASTPtr ast, const Data &data) {
  if (auto *create = ast->as<ASTCreateQuery>()) visitCreateQuery(*create, data);
}
