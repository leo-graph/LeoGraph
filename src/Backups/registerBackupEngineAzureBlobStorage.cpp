...
void registerBackupEngineAzureBlobStorage(BackupFactory& factory) {
  auto creator_fn = []([[maybe_unused]] BackupFactory::CreateParams params) -> std::unique_ptr<IBackup> {
    ...
    if (auto collection = params.backup_info.getNamedCollection(params.context)) {
      ...
      connection_params =
          getAzureConnectionParams(connection_url, container_name, get_optional("account_name"), get_optional("account_key"),
                                   get_optional("client_id"), get_optional("tenant_id"), params.context);

      if (args.size() > 1)
        throw Exception(ErrorCodes::NUMBER_OF_ARGUMENTS_DOESNT_MATCH,
                        "Backup AzureBlobStorage requires 1 or 2 arguments: named_collection, [filename]");

      if (args.size() == 1) blob_path = args[0].safeGet<String>();
    } else {
      if (args.size() == 3) {
        ...
        connection_params = getAzureConnectionParams(connection_url, container_name, std::nullopt, std::nullopt, std::nullopt, std::nullopt,
                                                     params.context);
      } else if (args.size() == 5) {
        auto connection_url = args[0].safeGet<String>();
        auto container_name = args[1].safeGet<String>();
        ...
        connection_params =
            getAzureConnectionParams(connection_url, container_name, account_name, account_key, std::nullopt, std::nullopt, params.context);
      } else {
        throw Exception(
    ...
    if (hasRegisteredArchiveFileExtension(blob_path)) {
      ...
      if (!params.password.empty()) throw Exception(ErrorCodes::BAD_ARGUMENTS, "Password is not applicable, backup cannot be encrypted");
    }

    if (params.open_mode == IBackup::OpenMode::UNLOCK) {
      auto reader = std::make_shared<BackupReaderAzureBlobStorage>(connection_params, blob_path, params.allow_azure_native_copy,
                                                                   params.read_settings, params.write_settings, params.context);

      auto lightweight_snapshot_writer =
          std::make_shared<BackupWriterAzureBlobStorage>(connection_params, "", params.allow_azure_native_copy, params.read_settings,
                                                         params.write_settings, params.context, params.azure_attempt_to_create_container);

      return std::make_unique<BackupImpl>(params.backup_info, archive_params, reader, lightweight_snapshot_writer);
    }
    ...
    if (params.open_mode == IBackup::OpenMode::READ) {
      ...
      auto snapshot_reader_creator = [&](const String& endpoint, const String& container_name) {
        ...
        return std::make_shared<BackupReaderAzureBlobStorage>(connection_params, "", params.allow_azure_native_copy, params.read_settings,
                                                              params.write_settings, params.context);
      };
    ...
    auto writer =
        std::make_shared<BackupWriterAzureBlobStorage>(connection_params, blob_path, params.allow_azure_native_copy, params.read_settings,
                                                       params.write_settings, params.context, params.azure_attempt_to_create_container);
