...
void registerBackupEngineS3(BackupFactory& factory) {
  auto creator_fn = []([[maybe_unused]] const BackupFactory::CreateParams& params) -> std::unique_ptr<IBackup> {
    ...
    if (auto collection = params.backup_info.getNamedCollection(params.context)) {
      ...
      if (collection->has("filename")) s3_uri = std::filesystem::path(s3_uri) / collection->get<String>("filename");
      ...
      if (args.size() == 1) s3_uri = std::filesystem::path(s3_uri) / args[0].safeGet<String>();
    } else {
    ...
    if (params.open_mode == IBackup::OpenMode::UNLOCK) {
      auto reader = std::make_shared<BackupReaderS3>(S3::URI{s3_uri}, access_key_id, secret_access_key, role_arn, role_session_name,
                                                     params.allow_s3_native_copy, params.read_settings, params.write_settings,
                                                     params.context, params.is_internal_backup);
      ...
      auto lightweight_snapshot_writer = std::make_shared<BackupWriterS3>(
          uri_for_lightweight, access_key_id, secret_access_key, role_arn, role_session_name, params.allow_s3_native_copy,
          params.s3_storage_class, params.read_settings, params.write_settings, params.context, params.is_internal_backup);
      ...
      auto snapshot_reader_creator = [&](const String& s3_uri_, const String& s3_bucket_) {
        ...
        /// We set the prefix to "" because in meta file, object key is absolute path.
        uri_for_lightweight.key = "";
        return std::make_shared<BackupReaderS3>(uri_for_lightweight, access_key_id, secret_access_key, role_arn, role_session_name,
                                                params.allow_s3_native_copy, params.read_settings, params.write_settings, params.context,
      ...
      auto writer = std::make_shared<BackupWriterS3>(
          S3::URI{s3_uri}, access_key_id, secret_access_key, std::move(role_arn), std::move(role_session_name), params.allow_s3_native_copy,
          params.s3_storage_class, params.read_settings, params.write_settings, params.context, params.is_internal_backup);
