...
class BackupMetadataFinder : private boost::noncopyable {
 ...
 protected:
  BackupMetadataFinder(const RestoreSettings& restore_settings_, const BackupPtr& backup_, const ContextMutablePtr& context_,
                       const ContextPtr& query_context_, ThreadPool& thread_pool_);
  ...
  void findTableInBackupImpl(const QualifiedTableName& table_name_in_backup, bool skip_if_inner_table,
