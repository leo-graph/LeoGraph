...
String tableNameWithTypeToString(const String &database_name, const String &table_name, bool first_upper) {
  ...
  if (first_upper) str[0] = static_cast<char>(std::toupper(str[0]));
  return str;
}
...
std::chrono::milliseconds getSleepTimeAfterInconsistencyError(int attempt_no, unsigned int attempts_before_sleep,
                                                              std::chrono::milliseconds min_sleep, std::chrono::milliseconds max_sleep) {
  attempts_before_sleep = std::max(attempts_before_sleep, 1U);
  if (((attempt_no + 1) % attempts_before_sleep) != 0) return std::chrono::milliseconds::zero();  /// no sleep
...
BackupEntriesCollector::BackupEntriesCollector(const ASTBackupQuery::Elements &backup_query_elements_,
                                               const BackupSettings &backup_settings_, const String &backup_id_,
                                               std::shared_ptr<IBackupCoordination> backup_coordination_,
                                               const ReadSettings &read_settings_, const ContextPtr &context_, ThreadPool &threadpool_)
    : backup_query_elements(backup_query_elements_),
      ...
      zookeeper_retries_info(context->getSettingsRef()[Setting::backup_restore_keeper_max_retries],
...
BackupEntries BackupEntriesCollector::run() {
  ...
  if (!current_stage.empty()) throw Exception(ErrorCodes::LOGICAL_ERROR, "Already making backup entries");

  /// Find other hosts working along with us to execute this ON CLUSTER query.
  all_hosts = BackupSettings::Util::filterHostIDs(backup_settings.cluster_host_ids, backup_settings.shard_num, backup_settings.replica_num);
  ...
  /// "shards/<shard_num>/replicas/<replica_num>/".
...
void BackupEntriesCollector::gatherMetadataAndCheckConsistency() {
  ...
  /// 2) it tries to collect it again, and compares the results from the first and the second collecting; if they match, it's done;
  /// otherwise 3) it sleeps 100 millisecond and tries to collect again, then compares the results from the second and the third collecting;
  /// if they match, it's done; otherwise 4) it tries to collect again, then compares the results from the third and the fourth collecting;
  /// if they match, it's done; otherwise 5) it sleeps 200 milliseconds and tries to collect again, then compares the results from the
  /// fourth and the fifth collecting; if they match, it's done;
  /// ...
  ...
  for (int attempt_no = 0;; ++attempt_no) {
    ...
    if (inconsistency_error && (std::chrono::steady_clock::now() > collect_metadata_end_time)) inconsistency_error->rethrow();

    /// Rethrow or just log the inconsistency error.
    if (!need_another_attempt) break;

    /// It's going to be another attempt, we need to sleep a bit.
    if (inconsistency_error) {
      auto sleep_time = getSleepTimeAfterInconsistencyError(attempt_no, attempts_to_collect_metadata_before_sleep,
                                                            min_sleep_before_next_attempt_to_collect_metadata,
                                                            max_sleep_before_next_attempt_to_collect_metadata);
      if (sleep_time.count()) {
...
void BackupEntriesCollector::tryGatherMetadataAndCompareWithPrevious(int attempt_no, std::optional<Exception> &inconsistency_error,
                                                                     bool &need_another_attempt) {
  try {
    ...
    if (e.code() != ErrorCodes::INCONSISTENT_METADATA_FOR_BACKUP) throw;

    /// If gatherDatabasesMetadata() or gatherTablesMetadata() threw a INCONSISTENT_METADATA_FOR_BACKUP error then the metadata is not
    /// consistent by itself, for example a CREATE QUERY could contain a wrong table name if the table has been just renamed. We need
    /// another attempt in that case.
    inconsistency_error = e;
    need_another_attempt = true;
    return;
  }

  if (!compare_collected_metadata) return;
  ...
  if (!compareWithPrevious(mismatch_description)) {
    ...
    if (attempt_no >= 1) inconsistency_error = Exception{ErrorCodes::INCONSISTENT_METADATA_FOR_BACKUP, "{}", mismatch_description};
    need_another_attempt = true;
  }
}

void BackupEntriesCollector::syncMetadataGatheringWithOtherHosts(int attempt_no, std::optional<Exception> &inconsistency_error,
                                                                 bool &need_another_attempt) {
  ...
  for (size_t i = 0; i != count; ++i) {
    ...
    if ((other_host_result != "consistent") && (other_host_result != "need_another_attempt")) {
      ...
      inconsistency_error =
          Exception{ErrorCodes::INCONSISTENT_METADATA_FOR_BACKUP, "Found inconsistency on host {}: {}", other_host, other_host_result};
      need_another_attempt = true;
      return;
    }
  }
  ...
  for (const auto &element : backup_query_elements) {
    switch (element.type) {
      case ASTBackupQuery::ElementType::TABLE: {
        gatherDatabaseMetadata(element.database_name,
                               /* throw_if_database_not_found= */ true,
                               /* backup_create_database_query= */ false, element.table_name,
                               /* throw_if_table_not_found= */ true, element.partitions,
                               /* all_tables= */ false,
                               /* except_table_names= */ {});
        break;
      }
      ...
      case ASTBackupQuery::ElementType::TEMPORARY_TABLE: {
        gatherDatabaseMetadata(DatabaseCatalog::TEMPORARY_DATABASE,
                               /* throw_if_database_not_found= */ true,
                               /* backup_create_database_query= */ false, element.table_name,
                               /* throw_if_table_not_found= */ true, element.partitions,
                               /* all_tables= */ false,
      ...
      case ASTBackupQuery::ElementType::ALL: {
        for (const auto &[database_name, database] :
             DatabaseCatalog::instance().getDatabases(GetDatabasesOptions{.with_datalake_catalogs = true})) {
          if (!element.except_databases.contains(database_name)) {
            gatherDatabaseMetadata(database_name,
...
void BackupEntriesCollector::gatherDatabaseMetadata(const String &database_name, bool throw_if_database_not_found,
                                                    bool backup_create_database_query, const std::optional<String> &table_name,
                                                    bool throw_if_table_not_found, const std::optional<ASTs> &partitions, bool all_tables,
                                                    const std::set<DatabaseAndTableName> &except_table_names) {
  checkIsQueryCancelled();

#if CLICKHOUSE_CLOUD
  if (database_name == SharedDatabaseCatalog::INTERNAL_DATABASE_TO_DROP) return;
#endif
  ...
  if (backup_create_database_query && !database_info.create_database_query && (database_name != DatabaseCatalog::TEMPORARY_DATABASE)) {
    ...
    try {
      ...
      if (throw_if_database_not_found) throw;
    ...
    if (create->getDatabase() != database_name) {
      ...
      LOG_WARNING(log, "Got a create query with unexpected name {} for database {}", backQuoteIfNeed(create->getDatabase()),
  ...
  if (table_name) {
    ...
    if (throw_if_table_not_found) table_params.throw_if_table_not_found = true;
    if (partitions) {
  ...
  if (all_tables) {
    ...
    for (const auto &except_table_name : except_table_names)
      if (except_table_name.first == database_name) database_info.except_table_names.emplace(except_table_name.second);
  }
}
  ...
  for (const auto &[database_name, database_info] : database_infos) {
    ...
    for (const auto &db_table : db_tables) {
      ...
      if (table_name_in_backup.database == DatabaseCatalog::TEMPORARY_DATABASE) {
        ...
        metadata_path_in_backup = root_path_in_backup / "metadata" / escapeForFileName(table_name_in_backup.database) /
      ...
      if (res_table_info.should_backup_data) {
        ...
        if (it != database_info.tables.end()) {
          ...
          if (partitions && storage && !storage->supportsBackupPartition()) {
            throw Exception(ErrorCodes::CANNOT_BACKUP_TABLE, "Table engine {} doesn't support partitions, cannot backup {}",
                            storage->getName(), tableNameWithTypeToString(database_name, table_name, false));
          }
...
std::vector<std::pair<ASTPtr, StoragePtr>> BackupEntriesCollector::findTablesInDatabase(const String &database_name) const {
  ...
  auto filter_by_table_name = [&](const String &table_name) {
    if (BackupUtils::isInnerTable(database_name, table_name)) return false;

    if (database_info.tables.contains(table_name)) return true;

    if (database_info.all_tables) return !database_info.except_table_names.contains(table_name);
  ...
  for (auto &db_table : db_tables) {
    ...
    if (database_name == DatabaseCatalog::TEMPORARY_DATABASE) {
      if (!create->isTemporary()) {
        throw Exception(ErrorCodes::LOGICAL_ERROR, "Got a non-temporary create query for {}",
      ...
      if (create->getDatabase() != database_name) {
        ...
        LOG_WARNING(log, "Got a create query with unexpected database name {} for {}", backQuoteIfNeed(create->getDatabase()),
                    tableNameWithTypeToString(database_name, create->getTable(), false));
...
bool BackupEntriesCollector::compareWithPrevious(String &mismatch_description) {
  ...
  for (const auto &[database_name, database_info] : database_infos)
    databases_metadata.emplace_back(
        database_name, database_info.create_database_query ? database_info.create_database_query->formatWithSecretsOneLine() : "");
  ...
  enum class MismatchType : uint8_t { ADDED, REMOVED, CHANGED, NONE };
  ...
  auto find_mismatch = [](const auto &metadata, const auto &previous_metadata)
    ...
    if ((mismatch != metadata.end()) && (p_mismatch != previous_metadata.end())) {
      if (mismatch->first == p_mismatch->first) return {MismatchType::CHANGED, mismatch->first};
      if (mismatch->first < p_mismatch->first) return {MismatchType::ADDED, mismatch->first};
...
void BackupEntriesCollector::makeBackupEntriesForDatabasesDefs() {
  for (const auto &[database_name, database_info] : database_infos) {
    if (!database_info.create_database_query) continue;  /// We store CREATE DATABASE queries only if there was BACKUP DATABASE specified.
    ...
    backup_entries.emplace_back(metadata_path_in_backup,
                                std::make_shared<BackupEntryFromMemory>(new_create_query->formatWithSecretsOneLine()));
  }
}
...
void BackupEntriesCollector::makeBackupEntriesForTablesDefs() {
  for (auto &[table_name, table_info] : table_infos) {
    ...
    table_info.replicated_table_zk_path =
        extractZooKeeperPathFromReplicatedTableDef(new_create_query->as<const ASTCreateQuery &>(), context);
    adjustCreateQueryForBackup(new_create_query, context->getGlobalContext());
    renameDatabaseAndTableNameInCreateQuery(new_create_query, renaming_map, context->getGlobalContext());
...
void BackupEntriesCollector::makeBackupEntriesForTablesData() {
  ...
  for (const auto &it : table_infos) {
    runner.enqueueAndKeepTrack([&]() { makeBackupEntriesForTableData(it.first); });
  }
...
void BackupEntriesCollector::makeBackupEntriesForTableData(const QualifiedTableName &table_name) {
  ...
  if (!table_info.should_backup_data) return;
...
bool BackupEntriesCollector::shouldBackupTableData(const QualifiedTableName &table_name, const StoragePtr &storage) const {
  if (backup_settings.structure_only) return false;
...
void BackupEntriesCollector::addBackupEntryUnlocked(const String &file_name, BackupEntryPtr backup_entry) {
  if (current_stage == Stage::WRITING_BACKUP) throw Exception(ErrorCodes::LOGICAL_ERROR, "Adding backup entries is not allowed");
...
void BackupEntriesCollector::addBackupEntries(const BackupEntries &backup_entries_) {
  std::lock_guard lock(mutex);
  if (current_stage == Stage::WRITING_BACKUP) throw Exception(ErrorCodes::LOGICAL_ERROR, "Adding of backup entries is not allowed");
...
void BackupEntriesCollector::addPostTask(std::function<void()> task) {
  std::lock_guard lock(mutex);
  if (current_stage == Stage::WRITING_BACKUP) throw Exception(ErrorCodes::LOGICAL_ERROR, "Adding of post tasks is not allowed");
...
std::unordered_map<UUID, AccessEntityPtr> BackupEntriesCollector::getAllAccessEntities() {
  ...
  if (!all_access_entities) {
    ...
    for (const auto &[id, entity] : entities_with_ids) {
      if (entity->isBackupAllowed()) all_access_entities->emplace(id, entity);
    }
  }
