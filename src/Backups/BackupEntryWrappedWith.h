...
class BackupEntryWrappedWith : public IBackupEntry {
 public:
  ...
  std::unique_ptr<SeekableReadBuffer> getReadBuffer(const ReadSettings& read_settings) const override {
    return entry->getReadBuffer(read_settings);
  }
  ...
  std::optional<UInt128> getPartialChecksum(UInt64 limit, const ReadSettings& read_settings) const override {
    return entry->getPartialChecksum(limit, read_settings);
  }
...
void wrapBackupEntriesWith(std::vector<std::pair<String, BackupEntryPtr>>& backup_entries, const T& custom_value) {
  for (auto& [_, backup_entry] : backup_entries) backup_entry = wrapBackupEntryWith(std::move(backup_entry), custom_value);
}
