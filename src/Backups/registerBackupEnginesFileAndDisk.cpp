...
void checkPath(const String& disk_name, const DiskPtr& disk, fs::path& path) {
  ...
  if (!path_ok)
    throw Exception(ErrorCodes::BAD_ARGUMENTS, "Path '{}' to backup must be inside the specified disk '{}'", quoteString(path.c_str()),
                    quoteString(disk_name));
}
...
void checkPath(fs::path& path, const Poco::Util::AbstractConfiguration& config, const fs::path& data_dir) {
  path = path.lexically_normal();
  if (path.empty()) throw Exception(ErrorCodes::BAD_ARGUMENTS, "Path to backup must not be empty");
  ...
  if (path.is_relative()) {
    ...
    if (first_allowed_path.is_relative()) first_allowed_path = data_dir / first_allowed_path;
  ...
  while (true) {
    ...
    if (allowed_path.is_relative()) allowed_path = data_dir / allowed_path;
