...
struct SettingFieldRestoreTableCreationMode {
  ...
  explicit SettingFieldRestoreTableCreationMode(const Field &field) {
    if (field.getType() == Field::Types::String) {
      ...
      if (boost::iequals(str, "if not exists") || boost::iequals(str, "if-not-exists") || boost::iequals(str, "create if not exists") ||
          boost::iequals(str, "create-if-not-exists")) {
  ...
  explicit operator Field() const {
    switch (value) {
      case RestoreTableCreationMode::kCreate:
        return Field{true};
      case RestoreTableCreationMode::kMustExist:
        return Field{false};
      case RestoreTableCreationMode::kCreateIfNotExists:
        return Field{"if-not-exists"};
    }
...
struct SettingFieldRestoreAccessCreationMode {
  ...
  explicit operator Field() const {
    switch (value) {
      case RestoreAccessCreationMode::kCreate:
        return Field{true};
      case RestoreAccessCreationMode::kCreateIfNotExists:
        return Field{"if-not-exists"};
      case RestoreAccessCreationMode::kReplace:
...
#define GET_RESTORE_SETTINGS_FROM_QUERY(TYPE, NAME)     \
  ...
  if (query.cluster_host_ids) res.cluster_host_ids = BackupSettings::Util::clusterHostIDsFromAST(*query.cluster_host_ids);
