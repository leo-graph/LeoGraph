...
bool BackupCoordinationStageSync::HostInfo::operator!=(const HostInfo &other) const { return !(*this == other); }
...
void BackupCoordinationStageSync::State::merge(const State &other) {
  ...
  for (const auto &[host, other_host_info] : other.hosts) {
    ...
    if (other_host_info.finished) host_info.finished = true;
  }
}
...
BackupCoordinationStageSync::BackupCoordinationStageSync(bool is_restore_, const String &zookeeper_path_, const String &current_host_,
                                                         const Strings &all_hosts_, bool allow_concurrency_,
                                                         BackupConcurrencyCounters &concurrency_counters_, const WithRetries &with_retries_,
                                                         ThreadPoolCallbackRunnerUnsafe<void> schedule_,
                                                         QueryStatusPtr process_list_element_, LoggerPtr log_)
...
BackupCoordinationStageSync::~BackupCoordinationStageSync() {
  ...
  if (!finished()) finish(/* throw_if_error = */ false);
...
void BackupCoordinationStageSync::initializeState() {
  ...
  if (!state.hosts.contains(current_host)) throw Exception(ErrorCodes::LOGICAL_ERROR, "List of hosts must contain the current host");

  if (!state.hosts.contains(String{kInitiator})) throw Exception(ErrorCodes::LOGICAL_ERROR, "List of hosts must contain the initiator");
...
void BackupCoordinationStageSync::createStartAndAliveNodesAndCheckConcurrency() {
  auto holder =
      with_retries.createRetriesControlHolder("BackupCoordinationStageSync::createStartAndAliveNodes", WithRetries::kInitialization);
  holder.retries_ctl.retryLoop([&, &zookeeper = holder.faulty_zookeeper]() {
    with_retries.renewZooKeeper(zookeeper);
...
void BackupCoordinationStageSync::createStartAndAliveNodesAndCheckConcurrency(Coordination::ZooKeeperWithFaultInjection::Ptr zookeeper) {
  ...
  for (size_t attempt_no = 1; attempt_no <= max_attempts_after_bad_version; ++attempt_no) {
    ...
    if (code == Coordination::Error::ZOK) {
      LOG_INFO(log, "Created start node #{} in ZooKeeper for {} (coordination version: {})", num_hosts.value_or(0) + 1, current_host_desc,
               static_cast<int>(kCurrentVersion));
      return;
    }
    ...
    if ((operation_node_pos < responses.size()) && (responses[operation_node_pos]->error == Coordination::Error::ZNODEEXISTS)) {
      show_error_before_next_attempt(fmt::format("Node {} already exists", operation_zookeeper_path));
      /// needs another attempt
    } else if ((zookeeper_path_pos < responses.size()) && (responses[zookeeper_path_pos]->error == Coordination::Error::ZNODEEXISTS)) {
...
void BackupCoordinationStageSync::checkConcurrency(Coordination::ZooKeeperWithFaultInjection::Ptr zookeeper) {
  ...
  if (code == Coordination::Error::ZNONODE) return;
    ...
    if (found_operation.starts_with(is_restore ? "restore-" : "backup-") && (found_operation != operation_node_name)) {
      ...
      if (code == Coordination::Error::ZOK) {
        for (const String &stage : stages) {
          if (stage.starts_with("alive")) BackupConcurrencyCheck::throwConcurrentOperationNotAllowed(is_restore);
        }
      }
    }
  }
...
void BackupCoordinationStageSync::stopWatchingThread() {
  {
    std::lock_guard lock{mutex};
    if (should_stop_watching_thread) return;
    should_stop_watching_thread = true;

    /// Wake up waiting threads.
    if (zk_nodes_changed) zk_nodes_changed->set();
    state_changed.notify_all();
  }

  if (watching_thread_future.valid()) watching_thread_future.wait();
...
void BackupCoordinationStageSync::createAliveNode(Coordination::ZooKeeperWithFaultInjection::Ptr zookeeper) {
  if (zookeeper->exists(alive_node_path)) return;
...
void BackupCoordinationStageSync::readCurrentState(Coordination::ZooKeeperWithFaultInjection::Ptr zookeeper) {
  ...
  std::sort(new_zk_nodes.begin(),
            new_zk_nodes.end());  /// Sorting is necessary because we compare the list of zk nodes with its previous versions.
...

  {
    ...
    if (!removed_zk_nodes.empty())
      LOG_TRACE(log, "Detected that some zookeeper nodes disappeared from the stage folder: {}",
                boost::algorithm::join(removed_zk_nodes, ", "));

    zk_nodes = new_zk_nodes;
    new_state = state;
    for (auto &[_, host_info] : new_state.hosts) host_info.connected = false;
  }

  auto get_host_info = [&](const String &host) -> HostInfo * {
    auto it = new_state.hosts.find(host);
    if (it == new_state.hosts.end()) return nullptr;
    return &it->second;
  };
  ...
  for (const auto &zk_node : new_zk_nodes) {
    if (zk_node == "error") {
      if (!new_state.host_with_error) {
        ...
        if (exception) new_state.addErrorInfo(exception, host);
      }
    } else if (zk_node.starts_with("finished|")) {
      String host = zk_node.substr(strlen("finished|"));
      if (auto *host_info = get_host_info(host)) host_info->finished = true;
  ...
  if (was_state_changed) state_changed.notify_all();
}
...
int BackupCoordinationStageSync::parseStartNode(const String &start_node_contents, const String &host) const {
  ...
  if (start_node_contents.empty()) {
    ...
    throw Exception(ErrorCodes::FAILED_TO_SYNC_BACKUP_OR_RESTORE, "Coordination version {} used by {} is not supported",
                    start_node_contents, getHostDesc(host));
  }

  if (version < kCurrentVersion) LOG_WARNING(log, "Coordination version {} used by {} is outdated", version, getHostDesc(host));
  return version;
}

void BackupCoordinationStageSync::checkIfQueryCancelled() {
  if (process_list_element->checkTimeLimitSoft()) return;  /// Not cancelled.
  state_changed.notify_all();
}
...
void BackupCoordinationStageSync::cancelQueryIfError() {
  ...
  {
    std::lock_guard lock{mutex};
    if (state.host_with_error) exception = state.hosts.at(*state.host_with_error).exception;
  }
...
void BackupCoordinationStageSync::cancelQueryIfDisconnectedTooLong() {
  ...
  {
    std::lock_guard lock{mutex};
    if (state.host_with_error || ((failure_after_host_disconnected_for_seconds.count() == 0))) return;
    ...
    for (auto &[host, host_info] : state.hosts) {
      if (!host_info.connected && !host_info.finished && (host != current_host)) {
        auto disconnected_duration =
            std::chrono::duration_cast<std::chrono::seconds>(monotonic_now - host_info.last_connection_time_monotonic);
        if (disconnected_duration > failure_after_host_disconnected_for_seconds) {
          ...
          String message = fmt::format(
              "The 'alive' node hasn't been updated in ZooKeeper for {} for {} "
              "which is more than the specified timeout {}. Last time the 'alive' node was detected at {}",
              getHostDesc(host), disconnected_duration, failure_after_host_disconnected_for_seconds, host_info.last_connection_time);
          LOG_WARNING(log, "Lost connection to {}: {}", getHostDesc(host), message);
          exception = std::make_exception_ptr(
              Exception{ErrorCodes::FAILED_TO_SYNC_BACKUP_OR_RESTORE, "Lost connection to {}: {}", getHostDesc(host), message});
          break;
        }
...
void BackupCoordinationStageSync::setStage(const String &stage, const String &stage_result) {
  ...
  {
    std::lock_guard lock{mutex};
    if (state.hosts.at(current_host).stages.contains(stage)) return;  /// Already set.
  }
...
void BackupCoordinationStageSync::createStageNode(const String &stage, const String &stage_result,
                                                  Coordination::ZooKeeperWithFaultInjection::Ptr zookeeper) {
  if (isErrorSet()) rethrowSetError();
...
Strings BackupCoordinationStageSync::waitHostsReachStage(const Strings &hosts, const String &stage_to_wait) const {
  ...
  auto check_if_hosts_reach_stage = [&]() TSA_NO_THREAD_SAFETY_ANALYSIS { return checkIfHostsReachStage(hosts, stage_to_wait, results); };
...
bool BackupCoordinationStageSync::checkIfHostsReachStage(const Strings &hosts, const String &stage_to_wait, Strings &results) const {
  ...
  if (state.host_with_error) std::rethrow_exception(state.hosts.at(*state.host_with_error).exception);

  for (size_t i = 0; i != hosts.size(); ++i) {
    const String &host = hosts[i];
    auto it = state.hosts.find(host);
    if (it == state.hosts.end())
      throw Exception(ErrorCodes::LOGICAL_ERROR, "waitHostsReachStage() was called for unexpected {}, all hosts are {}", getHostDesc(host),
                      getHostsDesc(all_hosts));
    ...
    if (host_info.finished) {
      ...
      throw Exception(ErrorCodes::FAILED_TO_SYNC_BACKUP_OR_RESTORE, "{} finished without coming to stage {}", getHostDesc(host),
                      stage_to_wait);
    }

    if (should_stop_watching_thread)
      throw Exception(ErrorCodes::LOGICAL_ERROR, "waitHostsReachStage() can't wait for stage {} after the watching thread stopped",
                      stage_to_wait);
...
void BackupCoordinationStageSync::finish(bool throw_if_error) {
  ...
  if (throw_if_error) retries_kind = WithRetries::kErrorHandling;
...
void BackupCoordinationStageSync::finishImpl(bool throw_if_error, WithRetries::Kind retries_kind) {
  {
    ...
    if (failed_to_set_error) {
      ...
      LOG_INFO(log, "Skipping creating the finish node for {} because there was an error which we were unable to send to other hosts",
               current_host_desc);
      return;
    }
  ...
  try {
    ...
    LOG_TRACE(log, "Caught exception while creating the 'finish' node for {}: {}", current_host_desc,
              getCurrentExceptionMessage(/* with_stacktrace= */ false, /* check_embedded_stacktrace= */ true));

    if (throw_if_error) throw;
  }
}
...
void BackupCoordinationStageSync::createFinishNodeAndRemoveAliveNode(Coordination::ZooKeeperWithFaultInjection::Ptr zookeeper,
  ...
  for (size_t attempt_no = 1; attempt_no <= max_attempts_after_bad_version; ++attempt_no) {
    ...
    if (finished()) {
      ...
      /// That can happen if previous attempt failed because of a connection loss but was in fact successful.
      LOG_INFO(log, "The 'finish' node in ZooKeeper for {} already exists. Hosts {} haven't finished yet", current_host_desc,
               getHostsDesc(getUnfinishedOtherHosts()));
      return;
    }
    ...
    if (num_hosts) {
      ...
      requests.emplace_back(
          zkutil::makeSetRequest(num_hosts_node_path, toString(start_node_exists ? (*num_hosts - 1) : *num_hosts), num_hosts_version));
    }
    ...
    if (code == Coordination::Error::ZOK) {
      LOG_INFO(log, "Created the 'finish' node in ZooKeeper for {}. Hosts {} haven't finished yet", current_host_desc,
               getHostsDesc(getUnfinishedOtherHosts()));
      std::lock_guard lock{mutex};
      state.hosts.at(current_host).finished = true;
      return;
    ...
    auto get_node_failed_check_for_non_existence = [&]() -> String {
      for (const auto &[pos, node_path] : non_existent_node_pos) {
        if ((pos < responses.size()) && (responses[pos]->error == Coordination::Error::ZNODEEXISTS)) return node_path;
      }
      return "";
    };
    ...
    auto show_error_before_next_attempt = [&](const String &message) {
      ...
      LOG_TRACE(log, "{} (attempt #{}){}", message, attempt_no, will_try_again ? ", will try again" : "");
      if (!will_try_again) {
        throw Exception(ErrorCodes::FAILED_TO_SYNC_BACKUP_OR_RESTORE, "Couldn't create the 'finish' node for {} after {} attempts: {}",
                        current_host_desc, attempt_no, message);
      }
    };

    if ((alive_node_pos < responses.size()) && (responses[alive_node_pos]->error == Coordination::Error::ZNONODE)) {
      show_error_before_next_attempt(fmt::format("Node {} doesn't exist", alive_node_path));
      /// needs another attempt
    } else if ((num_hosts_node_pos < responses.size()) && (responses[num_hosts_node_pos]->error == Coordination::Error::ZBADVERSION)) {
      show_error_before_next_attempt(fmt::format("The version of node {} changed", num_hosts_node_path));
      num_hosts.reset();  /// needs to reread 'num_hosts' again
...
void BackupCoordinationStageSync::waitOtherHostsFinish(bool throw_if_error) const {
  ...
  if (!throw_if_error) {
    if (finish_timeout_after_error.count() != 0) timeout = finish_timeout_after_error;
...
void BackupCoordinationStageSync::waitOtherHostsFinishImpl(const String &reason, std::optional<std::chrono::seconds> timeout,
  ...
  if (timeout) {
    if (!state_changed.wait_for(lock.getUnderlyingLock(), *timeout,
    ...
    state_changed.wait(lock.getUnderlyingLock(),
                       [&] TSA_REQUIRES(mutex) { return checkIfOtherHostsFinish(reason, timeout, false, throw_if_error); });
  }
}

bool BackupCoordinationStageSync::checkIfOtherHostsFinish(const String &reason, std::optional<std::chrono::milliseconds> timeout,
                                                          bool time_is_out, bool throw_if_error) const {
  if (throw_if_error) {
    process_list_element->checkTimeLimit();
    if (state.host_with_error) std::rethrow_exception(state.hosts.at(*state.host_with_error).exception);
  }
  ...
  for (const auto &[host, host_info] : state.hosts) {
    if ((host == current_host) || host_info.finished) continue;
    ...
    if (time_is_out) {
      if (throw_if_error) {
        throw Exception(ErrorCodes::FAILED_TO_SYNC_BACKUP_OR_RESTORE, "Waited longer than timeout {} for {} to finish{}{}", *timeout,
                        getHostDesc(host), reason_text, host_status);
      }
      LOG_INFO(log, "Waited longer than timeout {} for {} to finish{}{}", *timeout, getHostDesc(host), reason_text, host_status);
      return true;  /// stop waiting
    }
    ...
    if (should_stop_watching_thread) {
      ...
      if (throw_if_error)
        throw Exception(ErrorCodes::LOGICAL_ERROR,
                        "waitOtherHostsFinish() can't wait for other hosts to finish after the watching thread stopped");
      return true;  /// stop waiting
    }
...
bool BackupCoordinationStageSync::finishedNoLock() const { return state.hosts.at(current_host).finished; }
...
bool BackupCoordinationStageSync::otherHostsFinishedNoLock() const {
  for (const auto &[host, host_info] : state.hosts) {
    if (!host_info.finished && (host != current_host)) return false;
  }
  return true;
}

bool BackupCoordinationStageSync::allHostsFinishedNoLock() const { return finishedNoLock() && otherHostsFinishedNoLock(); }
...
Strings BackupCoordinationStageSync::getUnfinishedHostsNoLock() const {
  if (allHostsFinishedNoLock()) return {};

  Strings res;
  res.reserve(all_hosts.size());
  for (const auto &[host, host_info] : state.hosts) {
    if (!host_info.finished) res.emplace_back(host);
  }
  return res;
}
...
Strings BackupCoordinationStageSync::getUnfinishedOtherHostsNoLock() const {
  if (otherHostsFinishedNoLock()) return {};

  Strings res;
  res.reserve(all_hosts.size() - 1);
  for (const auto &[host, host_info] : state.hosts) {
    if (!host_info.finished && (host != current_host)) res.emplace_back(host);
  }
  return res;
}
...
void BackupCoordinationStageSync::setError(const Exception &exception, bool throw_if_error) {
  ...
  try {
    ...
    LOG_TRACE(log, "Caught exception while creating the error node for this {}: {}", is_restore ? "restore" : "backup",
              getCurrentExceptionMessage(/* with_stacktrace= */ false, /* check_embedded_stacktrace= */ true));
...
std::pair<std::exception_ptr, String> BackupCoordinationStageSync::parseErrorNode(const String &error_node_contents) const {
  ...
  if (std::find(all_hosts.begin(), all_hosts.end(), host) == all_hosts.end()) return {};
