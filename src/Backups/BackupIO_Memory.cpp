...
BackupReaderMemory::BackupReaderMemory(std::shared_ptr<const BackupInMemory> backup_in_memory_, const ReadSettings& read_settings_,
...
bool BackupReaderMemory::fileExists(const String& file_name) { return backup_in_memory->fileExists(file_name); }

UInt64 BackupReaderMemory::getFileSize(const String& file_name) { return backup_in_memory->getFileSize(file_name); }
...
BackupWriterMemory::BackupWriterMemory(std::shared_ptr<BackupInMemory> backup_in_memory_, const ReadSettings& read_settings_,
                                       const WriteSettings& write_settings_)
    : BackupWriterDefault(read_settings_, write_settings_, getLogger("BackupWriterMemory")), backup_in_memory(backup_in_memory_) {}

bool BackupWriterMemory::fileExists(const String& file_name) { return backup_in_memory->fileExists(file_name); }

UInt64 BackupWriterMemory::getFileSize(const String& file_name) { return backup_in_memory->getFileSize(file_name); }

std::unique_ptr<WriteBuffer> BackupWriterMemory::writeFile(const String& file_name) { return backup_in_memory->writeFile(file_name); }
...
void BackupWriterMemory::removeFile(const String& file_name) { backup_in_memory->removeFile(file_name); }
...
void BackupWriterMemory::removeEmptyDirectories() {
  if (backup_in_memory->isEmpty()) backup_in_memory->drop();
}
