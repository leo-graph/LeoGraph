...
RestoreCoordinationOnCluster::RestoreCoordinationOnCluster(
    const UUID& restore_uuid_, const String& root_zookeeper_path_, zkutil::GetZooKeeper get_zookeeper_,
    const BackupKeeperSettings& keeper_settings_, const String& current_host_, const Strings& all_hosts_, bool allow_concurrent_restore_,
    BackupConcurrencyCounters& concurrency_counters_, ThreadPoolCallbackRunnerUnsafe<void> schedule_, QueryStatusPtr process_list_element_)
    : root_zookeeper_path(root_zookeeper_path_),
      ...
      stage_sync(/* is_restore = */ true, fs::path{zookeeper_path} / "stage", current_host, all_hosts, allow_concurrent_restore_,
  ...
  if (current_host != kInitiator) setRestoreQueryIsSentToOtherHosts();
}
...
void RestoreCoordinationOnCluster::setRestoreQueryIsSentToOtherHosts() { stage_sync.setQueryIsSentToOtherHosts(); }

bool RestoreCoordinationOnCluster::isRestoreQuerySentToOtherHosts() const { return stage_sync.isQuerySentToOtherHosts(); }
...
bool RestoreCoordinationOnCluster::isErrorSet() const { return stage_sync.isErrorSet(); }

void RestoreCoordinationOnCluster::waitOtherHostsFinish(bool throw_if_error) const { stage_sync.waitOtherHostsFinish(throw_if_error); }

void RestoreCoordinationOnCluster::finish(bool throw_if_error) { stage_sync.finish(throw_if_error); }

bool RestoreCoordinationOnCluster::finished() const { return stage_sync.finished(); }

bool RestoreCoordinationOnCluster::allHostsFinished() const { return stage_sync.allHostsFinished(); }

void RestoreCoordinationOnCluster::cleanup(bool throw_if_error) { cleaner.cleanup(throw_if_error); }
