...
class BackupEntriesTest : public ::testing::Test {
 protected:
  ...
  static String getChecksum(const BackupEntryPtr& backup_entry) { return getHexUIntUppercase(backup_entry->getChecksum({})); }
  ...
  static String getPartialChecksum(const BackupEntryPtr& backup_entry, UInt64 prefix_length) {
    ...
    if (!partial_checksum) return String{NO_CHECKSUM};
...
TEST_F(BackupEntriesTest, BackupEntryFromImmutableFile) {
  ...
  auto precalculated_entry =
      std::make_shared<BackupEntryFromImmutableFile>(local_disk, "a.txt", false, PRECALCULATED_SIZE, PRECALCULATED_CHECKSUM_UINT128);
...
TEST_F(BackupEntriesTest, DecryptedEntriesFromEncryptedDisk) {
...

  {
    ...
    BackupEntryPtr entries[] = {std::make_shared<BackupEntryFromImmutableFile>(encrypted_disk, "empty.txt"),
  ...
  {
    auto precalculated_entry =
        std::make_shared<BackupEntryFromImmutableFile>(encrypted_disk, "a.txt", false, PRECALCULATED_SIZE, PRECALCULATED_CHECKSUM_UINT128);
    EXPECT_EQ(precalculated_entry->getSize(), PRECALCULATED_SIZE);
    EXPECT_EQ(getChecksum(precalculated_entry), PRECALCULATED_CHECKSUM);
    EXPECT_EQ(getPartialChecksum(precalculated_entry, 0), NO_CHECKSUM);
    EXPECT_EQ(getPartialChecksum(precalculated_entry, 1), NO_CHECKSUM);
...
TEST_F(BackupEntriesTest, EncryptedEntriesFromEncryptedDisk) {
  ...
  {
    auto precalculated_entry = std::make_shared<BackupEntryFromImmutableFile>(encrypted_disk, "a.txt", /* copy_encrypted= */ true,
                                                                              PRECALCULATED_SIZE, PRECALCULATED_CHECKSUM_UINT128);
