...
class BackupInMemory::WriteBufferToBackupInMemory : public WriteBufferFromOwnString {
 public:
  WriteBufferToBackupInMemory(std::shared_ptr<BackupInMemory> backup_, const String& file_name_) : backup(backup_), file_name(file_name_) {}
...
void BackupInMemory::drop() {
  if (auto holder_ptr = holder.lock()) holder_ptr->dropBackup(backup_name);
}
