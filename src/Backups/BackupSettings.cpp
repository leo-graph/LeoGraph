...
#define GET_BACKUP_SETTINGS_FROM_QUERY(TYPE, NAME)      \
  ...
  if (query.base_backup_name) res.base_backup_info = BackupInfo::fromAST(*query.base_backup_name);

  if (query.cluster_host_ids) res.cluster_host_ids = Util::clusterHostIDsFromAST(*query.cluster_host_ids);
...
bool BackupSettings::isAsync(const ASTBackupQuery &query) {
  if (query.settings) {
    ...
    if (field) return field->safeGet<bool>();
  }
...
#define COPY_BACKUP_SETTINGS_TO_QUERY(TYPE, NAME) \
  if ((NAME) != default_settings.NAME) query_settings->changes.emplace_back(#NAME, static_cast<Field>(SettingField##TYPE{NAME}));
  ...
  if (query_settings->changes.empty()) query_settings = nullptr;
...
ASTPtr BackupSettings::Util::clusterHostIDsToAST(const std::vector<Strings> &cluster_host_ids) {
  if (cluster_host_ids.empty()) return nullptr;
  ...
  for (size_t i = 0; i != cluster_host_ids.size(); ++i) {
    ...
    for (size_t j = 0; j != shard.size(); ++j) res_shard[j] = Field{shard[j]};
...
std::pair<size_t, size_t> BackupSettings::Util::findShardNumAndReplicaNum(const std::vector<Strings> &cluster_host_ids,
                                                                          const String &host_id) {
  for (size_t i = 0; i != cluster_host_ids.size(); ++i) {
    for (size_t j = 0; j != cluster_host_ids[i].size(); ++j)
      if (cluster_host_ids[i][j] == host_id) return {i + 1, j + 1};
  }
...
Strings BackupSettings::Util::filterHostIDs(const std::vector<Strings> &cluster_host_ids, size_t only_shard_num, size_t only_replica_num) {
  ...
  auto collect_replicas = [&](size_t shard_index) {
    ...
    if (only_replica_num) {
      if (only_replica_num <= shard.size()) collected_host_ids.push_back(shard[only_replica_num - 1]);
    } else {
      for (size_t replica_index = 1; replica_index <= shard.size(); ++replica_index) collected_host_ids.push_back(shard[replica_index - 1]);
    }
  };

  if (only_shard_num) {
    if (only_shard_num <= cluster_host_ids.size()) collect_replicas(only_shard_num);
  } else {
    for (size_t shard_index = 1; shard_index <= cluster_host_ids.size(); ++shard_index) collect_replicas(shard_index);
  }
