...
BackupEntryFromAppendOnlyFile::BackupEntryFromAppendOnlyFile(const DiskPtr& disk_, const String& file_path_, bool copy_encrypted_,
                                                             const std::optional<UInt64>& file_size_, bool allow_checksum_from_remote_path_)
    : disk(disk_),
...
BackupEntryFromAppendOnlyFile::BackupEntryFromAppendOnlyFile(TemporaryDataBufferPtr tmp_file_)
    : tmp_file(std::move(tmp_file_)), file_path(tmp_file->describeFilePath()), size(tmp_file->getStat().compressed_size) {}
