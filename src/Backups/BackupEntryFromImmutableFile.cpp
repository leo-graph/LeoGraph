...
BackupEntryFromImmutableFile::BackupEntryFromImmutableFile(const DiskPtr& disk_, const String& file_path_, bool copy_encrypted_,
                                                           const std::optional<UInt64>& file_size_, const std::optional<UInt128>& checksum_,
...
std::unique_ptr<SeekableReadBuffer> BackupEntryFromImmutableFile::getReadBuffer(const ReadSettings& read_settings) const {
  if (copy_encrypted) return disk->readEncryptedFile(file_path, read_settings);
...
UInt64 BackupEntryFromImmutableFile::getSize() const {
  {
    ...
    if (calculated_size) return *calculated_size;
  }
...
UInt64 BackupEntryFromImmutableFile::calculateSize() const {
  if (copy_encrypted) return passed_file_size ? disk->getEncryptedFileSize(*passed_file_size) : disk->getEncryptedFileSize(file_path);

  if (passed_file_size) return *passed_file_size;
