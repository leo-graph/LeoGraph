...
uint64_t DynamicDelay::getCurrentDelay() const { return static_cast<uint64_t>(delay); }
...
void DynamicDelay::up() { delay = calibrateDelay(delay * factor_up, min_delay, max_delay); }

void DynamicDelay::lower() { delay = calibrateDelay(delay / factor_lower, min_delay, max_delay); }

void DynamicDelay::rotateToMin() { delay = min_delay; }

void DynamicDelay::rotateToMax() { delay = max_delay; }
