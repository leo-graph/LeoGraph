...
enum class WeekModeFlag : UInt8 { MONDAY_FIRST = 1, YEAR = 2, FIRST_WEEKDAY = 4, NEWYEAR_DAY = 8 };
using YearWeek = std::pair<UInt16, UInt8>;

/// Modes for toDayOfWeek() function.
enum class WeekDayMode : uint8_t { WeekStartsMonday1 = 0, WeekStartsMonday0 = 1, WeekStartsSunday0 = 2, WeekStartsSunday1 = 3 };
...
class DateLUTImpl {
 private:
  ...
  static LUTIndex normalizeLUTIndex(UInt32 index) {
    if (index >= DATE_LUT_SIZE) return LUTIndex(DATE_LUT_SIZE - 1);
    return LUTIndex{index};
  }

  static LUTIndex normalizeLUTIndex(Int64 index) {
    if (unlikely(index < 0)) return LUTIndex(0);
    if (index >= DATE_LUT_SIZE) return LUTIndex(DATE_LUT_SIZE - 1);
 ...
 private:
  ...
  LUTIndex findIndex(Time t) const {
    ...
    if (unlikely(t < 0)) --guess;

    if (guess < 0) return LUTIndex(0);
    if (guess >= DATE_LUT_SIZE) return LUTIndex(DATE_LUT_SIZE - 1);

    /// UTC offset is from -12 to +14 in all known time zones. This requires checking only three indices.

    if (t >= lut[guess].date) {
      if (guess + 1 >= DATE_LUT_SIZE || t < lut[guess + 1].date) return LUTIndex(static_cast<unsigned>(guess));
  ...
  static LUTIndex toLUTIndex(DayNum d) { return normalizeLUTIndex(d + daynum_offset_epoch); }

  static LUTIndex toLUTIndex(ExtendedDayNum d) { return normalizeLUTIndex(static_cast<Int64>(d) + daynum_offset_epoch); }

  LUTIndex toLUTIndex(Time t) const { return findIndex(t); }

  static LUTIndex toLUTIndex(LUTIndex i) { return i; }
 ...
 public:
  ...
  static ExtendedDayNum toDayNum(ExtendedDayNum d) { return d; }
  ...
  Time toFirstDayOfYear(Time t) const { return lut[years_lut[lut[findIndex(t)].year - DATE_LUT_MIN_YEAR]].date; }
  ...
  UInt8 daysInMonth(Int16 year, UInt8 month) const {
    ...
    if (unlikely(idx >= DATE_LUT_YEARS)) return 31;  /// Implementation specific behaviour on overflow.
   ...
   */
  Time toDateAndShift(Time t, Int32 days) const { return lut[findIndex(t) + days].date; }
  ...
  Time toTime(Time t) const {
    ...
    if (res >= lut[index].time_at_offset_change()) res += lut[index].amount_of_offset_change();
  ...
  unsigned toHour(Time t) const {
    ...
    if (time >= lut[index].time_at_offset_change()) time += lut[index].amount_of_offset_change();
  ...
  unsigned toMinute(Time t) const {
    if (t >= 0 && offset_is_whole_number_of_hours_during_epoch) return (t / 60) % 60;
  ...
  template <typename DateOrTime>
  DateOrTime toStartOfMinute(DateOrTime t) const {
    return toStartOfMinuteInterval(t, 1);
  }
  template <typename DateOrTime>
  DateOrTime toStartOfFiveMinutes(DateOrTime t) const {
    return toStartOfMinuteInterval(t, 5);
  }
  template <typename DateOrTime>
  DateOrTime toStartOfFifteenMinutes(DateOrTime t) const {
    return toStartOfMinuteInterval(t, 15);
  }
  template <typename DateOrTime>
  DateOrTime toStartOfTenMinutes(DateOrTime t) const {
    return toStartOfMinuteInterval(t, 10);
  }
  template <typename DateOrTime>
  DateOrTime toStartOfHour(DateOrTime t) const {
  ...
  template <typename DateOrTime>
  UInt8 toMonth(DateOrTime v) const {
    return lut[toLUTIndex(v)].month;
  }

  template <typename DateOrTime>
  UInt8 toQuarter(DateOrTime v) const {
    return (lut[toLUTIndex(v)].month - 1) / 3 + 1;
  }

  template <typename DateOrTime>
  Int16 toYear(DateOrTime v) const {
    return lut[toLUTIndex(v)].year;
  }

  template <typename DateOrTime>
  Int16 toYearSinceEpoch(DateOrTime v) const {
    return lut[toLUTIndex(v)].year - 1970;
  }
  ...
  UInt8 toDayOfWeek(DateOrTime v, UInt8 week_day_mode) const {
    ...
    if (start_from_sunday) res = res % 7 + 1;
    if (zero_based) --res;
  ...
  LUTIndex toFirstDayNumOfISOYearIndex(DateOrTime v) const {
    ...
    return LUTIndex{first_day_of_week_of_year <= 4 ? first_day_of_year + (1 - first_day_of_week_of_year)
  ...
  Time toFirstDayOfISOYear(Time t) const { return lut[toFirstDayNumOfISOYearIndex(t)].date; }
  ...
  */
  ...
  YearWeek toYearWeek(DateOrTime v, UInt8 week_mode) const {
    ...
    if (toMonth(i) == 1 && toDayOfMonth(i) <= static_cast<UInt32>(7 - weekday)) {
      if (!week_year_mode && ((first_weekday_mode && weekday != 0) || (!first_weekday_mode && weekday >= 4))) return yw;
      week_year_mode = true;
      --yw.first;
  ...
  YearWeek toYearWeekOfNewyearMode(DateOrTime v, bool monday_first_mode) const {
    ...
    if (monday_first_mode) {
      ...
      if (toDayOfWeek(first_day) != 7) first_day = ExtendedDayNum(first_day - toDayOfWeek(first_day));
      if (toDayOfWeek(i) != 7) this_day = ExtendedDayNum(i - toDayOfWeek(i));
    }
  ...
  UInt8 check_week_mode(UInt8 mode) const  /// NOLINT
  {
    ...
    if (!(week_format & static_cast<UInt8>(WeekModeFlag::MONDAY_FIRST))) week_format ^= static_cast<UInt8>(WeekModeFlag::FIRST_WEEKDAY);
    return week_format;
  }
  ...
  {
    const LUTIndex i = toLUTIndex(v);
    if (!sunday_first_day_of_week) return toDayOfWeek(i) - 1;
  ...
  ALWAYS_INLINE Time toRelativeHourNum(Time t) const {
    if (t >= 0 && offset_is_whole_number_of_hours_during_epoch) return t / 3600;
  ...
  auto toStartOfYearInterval(DateOrTime v, UInt64 years) const {
    if (years == 1) return toFirstDayNumOfYear(v);
    ...
    if (unlikely(year < DATE_LUT_MIN_YEAR)) year = DATE_LUT_MIN_YEAR;
  ...
  auto toStartOfQuarterInterval(Date d, UInt64 quarters) const {
    if (quarters == 1) return toFirstDayNumOfQuarter(d);
  ...
  auto toStartOfMonthInterval(Date d, UInt64 months) const {
    if (months == 1) return toFirstDayNumOfMonth(d);
  ...
  auto toStartOfWeekInterval(Date d, UInt64 weeks) const {
    if (weeks == 1) return toFirstDayNumOfWeek(d);
    UInt64 days = weeks * 7;
  ...
  DateOrTime toStartOfHourInterval(DateOrTime t, UInt64 hours) const {
    if (hours == 1) return toStartOfHour(t);
  ...
  DateOrTime toStartOfSecondInterval(DateOrTime t, UInt64 seconds) const {
    if (seconds == 1) return t;
    if (seconds % 60 == 0) return toStartOfMinuteInterval(t, seconds / 60);

    return static_cast<DateOrTime>(roundDown(t, seconds));
  }

  LUTIndex makeLUTIndex(Int16 year, UInt8 month, UInt8 day_of_month) const {
    if (unlikely(year < DATE_LUT_MIN_YEAR || month < 1 || month > 12 || day_of_month < 1 || day_of_month > 31)) return LUTIndex(0);

    if (unlikely(year > DATE_LUT_MAX_YEAR)) return LUTIndex(DATE_LUT_SIZE - 1);
  ...
  std::optional<LUTIndex> tryToMakeLUTIndex(Int16 year, UInt8 month, UInt8 day_of_month) const {
    ...
    if (index >= DATE_LUT_SIZE) return std::nullopt;
  ...
  std::optional<ExtendedDayNum> tryToMakeDayNum(Int16 year, UInt8 month, UInt8 day_of_month) const {
    if (unlikely(year < DATE_LUT_MIN_YEAR || month < 1 || month > 12 || day_of_month < 1 || day_of_month > 31)) return std::nullopt;
  ...
  Time makeDate(Int16 year, UInt8 month, UInt8 day_of_month) const { return lut[makeLUTIndex(year, month, day_of_month)].date; }
  ...
  Time makeDateTime(Int16 year, UInt8 month, UInt8 day_of_month, UInt8 hour, UInt8 minute, UInt8 second) const {
    ...
    if (time_offset >= lut[index].time_at_offset_change()) time_offset -= lut[index].amount_of_offset_change();
  ...
  Time makeTime(int64_t hour, UInt8 minute, UInt8 second) const {
    ...
    if (time_offset >= lut[1].time_at_offset_change()) time_offset -= lut[0].amount_of_offset_change();

    return time_offset;
  }

  template <typename DateOrTime>
  const Values &getValues(DateOrTime v) const {
    return lut[toLUTIndex(v)];
  }
  ...
  DateTimeComponents toDateTimeComponents(Time t) const {
    ...
    Time time = t - values.date;
    if (time >= values.time_at_offset_change()) time += values.amount_of_offset_change();
    ...
    /// In case time was changed backwards at the start of next day, we will repeat the hour 23.
    if (unlikely(res.time.hour > 23)) res.time.hour = 23;
  ...
  TimeComponents toTimeComponents(Time t) const {
    ...
    if (unlikely(t > 3599999)) t = 3599999;
  ...
  UInt64 toNumYYYYMMDDhhmmss(Time t) const {
    ...
    return components.time.second + components.time.minute * 100 + components.time.hour * 10000 + UInt64(components.date.day) * 1000000 +
           UInt64(components.date.month) * 100000000 + UInt64(components.date.year) * 10000000000;
  }

  Time YYYYMMDDhhmmssToTime(UInt64 num) const  /// NOLINT
  {
    return makeDateTime(static_cast<Int16>(num / 10000000000), num / 100000000 % 100, num / 1000000 % 100, num / 10000 % 100,
                        num / 100 % 100, num % 100);
  }
  ...
  NO_SANITIZE_UNDEFINED Time addDays(Time t, Int64 delta) const {
    ...
    if (time >= lut[new_index].time_at_offset_change()) time -= lut[new_index].amount_of_offset_change();

    return lut[new_index].date + time;
  }

  NO_SANITIZE_UNDEFINED Time addWeeks(Time t, Int64 delta) const { return addDays(t, delta * 7); }
  ...
  Time NO_SANITIZE_UNDEFINED addMonths(DateTime t, Int64 delta) const {
    ...
    if (time >= lut[result_day].time_at_offset_change()) time -= lut[result_day].amount_of_offset_change();
  ...
  LUTIndex NO_SANITIZE_UNDEFINED addYearsIndex(DateOrTime v, Int64 delta) const {
    ...
    if (unlikely(day_of_month == 29 && month == 2)) day_of_month = saturateDayOfMonth(static_cast<Int16>(year), month, day_of_month);
