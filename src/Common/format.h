...
static inline UInt64 parseNumber(const String& description, UInt64 l, UInt64 r, UInt64 argument_number) {
  ...
  for (UInt64 pos = l; pos < r; ++pos) {
    if (!isNumericASCII(description[pos])) throw Exception(ErrorCodes::BAD_ARGUMENTS, "Not a number in curly braces at position {}", pos);
...
static inline void init(const String& pattern, size_t argument_number, const std::vector<std::optional<String>>& constant_strings,
                        IndexPositions& index_positions, std::vector<String>& substrings) {
  ...
  auto double_brace_removal = [](String& str) {
    ...
    std::erase_if(str, [&i, &should_delete, &str](char) {
  ...
  for (size_t i = 0; i < pattern.size(); ++i) {
    if (pattern[i] == '{') {
      ...
      if (is_open_curly) throw Exception(ErrorCodes::BAD_ARGUMENTS, "Two open curly braces without close one at position {}", i);
      ...
      if (!is_open_curly) throw Exception(ErrorCodes::BAD_ARGUMENTS, "Closed curly brace without open one at position {}", i);
  ...
  if (is_open_curly) throw Exception(ErrorCodes::BAD_ARGUMENTS, "Last open curly brace is not closed");
