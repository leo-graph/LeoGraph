...
std::optional<UInt64> getMostStrictAvailableSystemMemory() {
  ...
  if (query_memory_tracker = DB::CurrentThread::getMemoryTracker(); !query_memory_tracker) return {};
  /// query-level memory tracker
  if (query_memory_tracker = query_memory_tracker->getParent(); !query_memory_tracker) return {};
  ...
  if (available == std::numeric_limits<Int64>::max()) return {};
  return available;
}
...
std::optional<UInt64> getCurrentQueryHardLimit() {
  ...
  if (hard_limit == std::numeric_limits<Int64>::max()) return {};
  return hard_limit;
}
...
Int64 getCurrentQueryMemoryUsage() {
  ...
  if (!thread_memory_tracker || thread_memory_tracker->level != VariableContext::Thread) return 0;

  auto* query_process_memory_tracker = thread_memory_tracker->getParent();
  if (!query_process_memory_tracker || query_process_memory_tracker->level != VariableContext::Process) return 0;
