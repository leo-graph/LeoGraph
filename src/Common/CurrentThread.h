...
class CurrentThread {
 public:
  ...
  static void attachInternalTextLogsQueue(const std::shared_ptr<InternalTextLogsQueue>& logs_queue, LogsLevel client_logs_level);
  ...
  struct IOSchedulingScope : private boost::noncopyable {
    ...
    ~IOSchedulingScope() {
      if (read_resource_attached) detachReadResource();
      if (write_resource_attached) detachWriteResource();
    }
  ...
  struct ReadThrottlingScope : private boost::noncopyable {
    ...
    ~ReadThrottlingScope() {
      if (read_throttler_attached) detachReadThrottler();
    }
  ...
  struct WriteThrottlingScope : private boost::noncopyable {
    ...
    ~WriteThrottlingScope() {
      if (write_throttler_attached) detachWriteThrottler();
    }
