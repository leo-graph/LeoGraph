...
void AsynchronousMetrics::openCgroupv2MetricFile(const std::string &filename, std::optional<ReadBufferFromFilePRead> &out) {
  if (auto path = getCgroupsV2PathContainingFile(filename)) openFileIfExists((path.value() + filename).c_str(), out);
};

#endif

AsynchronousMetrics::AsynchronousMetrics(unsigned update_period_seconds, const ProtocolServerMetricsFunc &protocol_server_metrics_func_,
                                         bool update_jemalloc_epoch_, bool update_rss_, const ContextPtr &context_)
  ...
  /// CGroups v1
  if (!cgroupmem_limit_in_bytes) openFileIfExists("/sys/fs/cgroup/memory/memory.limit_in_bytes", cgroupmem_limit_in_bytes);
  if (!cgroupcpu_max) {
  ...
  if (!cgroupcpu_stat) openFileIfExists("/sys/fs/cgroup/cpuacct/cpuacct.stat", cgroupcpuacct_stat);

  try {
    const auto [cgroup_path, version] = ICgroupsReader::getCgroupsPath();
    LOG_INFO(getLogger("AsynchronousMetrics"), "Will use cgroup reader from '{}' (cgroups version: {})", cgroup_path,
...
void AsynchronousMetrics::openSensors() TSA_REQUIRES(data_mutex) {
  ...
  for (size_t thermal_device_index = 0;; ++thermal_device_index) {
    std::unique_ptr<ReadBufferFromFilePRead> file =
        openFileIfExists(fmt::format("/sys/class/thermal/thermal_zone{}/temp", thermal_device_index));
    if (!file) {
      /// Sometimes indices are from zero sometimes from one.
      if (thermal_device_index == 0) continue;
      break;
    }
    ...
    try {
      ...
      LOG_WARNING(getLogger("AsynchronousMetrics"), "Thermal monitor '{}' exists but could not be read: {}.", thermal_device_index,
...
void AsynchronousMetrics::openBlockDevices() TSA_REQUIRES(data_mutex) {
  ...
  if (!std::filesystem::exists("/sys/block")) return;

  block_devices_rescan_delay.restart();
  block_devs.clear();

  for (const auto &device_dir :
       std::filesystem::directory_iterator("/sys/block", std::filesystem::directory_options::skip_permission_denied)) {
    String device_name = device_dir.path().filename();

    /// We are not interested in loopback devices.
    if (device_name.starts_with("loop")) continue;
...
void AsynchronousMetrics::openEDAC() TSA_REQUIRES(data_mutex) {
  ...
  for (size_t edac_index = 0;; ++edac_index) {
    ...
    if (!edac_correctable_file_exists && !edac_uncorrectable_file_exists) {
      if (edac_index == 0) continue;
      break;
    }

    edac.emplace_back();

    if (edac_correctable_file_exists) edac.back().first = openFileIfExists(edac_correctable_file);
    if (edac_uncorrectable_file_exists) edac.back().second = openFileIfExists(edac_uncorrectable_file);
  }
}
  ...
  for (size_t hwmon_index = 0;; ++hwmon_index) {
    ...
    for (size_t sensor_index = 0;; ++sensor_index) {
      ...
      try {
        ...
        LOG_WARNING(getLogger("AsynchronousMetrics"), "Hardware monitor '{}', sensor '{}' exists but could not be read: {}.", hwmon_name,
                    sensor_index, errnoToString(e.getErrno()));
        continue;
      }
...
AsynchronousMetrics::~AsynchronousMetrics() { stop(); }
...
void AsynchronousMetrics::run() {
  ...
  while (true) {
...

    {
      ...
      if (wait_cond.wait_until(lock, next_update_time, [this] TSA_REQUIRES(thread_mutex) { return quit; })) {
        break;
      }
    }
...
Value saveJemallocMetricImpl(AsynchronousMetricValues &values, const std::string &jemalloc_full_name,
...
template <typename Value>
Value saveJemallocMetric(AsynchronousMetricValues &values, const std::string &metric_name) {
  return saveJemallocMetricImpl<Value>(values, fmt::format("stats.{}", metric_name), fmt::format("jemalloc.{}", metric_name));
}

template <typename Value>
Value saveAllArenasMetric(AsynchronousMetricValues &values, const std::string &metric_name) {
  return saveJemallocMetricImpl<Value>(values, fmt::format("stats.arenas.{}.{}", MALLCTL_ARENAS_ALL, metric_name),
                                       fmt::format("jemalloc.arenas.all.{}", metric_name));
}

template <typename Value>
Value saveJemallocProf(AsynchronousMetricValues &values, const std::string &metric_name) {
  return saveJemallocMetricImpl<Value>(values, fmt::format("prof.{}", metric_name), fmt::format("jemalloc.prof.{}", metric_name));
}
...
AsynchronousMetrics::ProcStatValuesCPU AsynchronousMetrics::ProcStatValuesCPU::operator-(
    const AsynchronousMetrics::ProcStatValuesCPU &other) const {
...
AsynchronousMetrics::ProcStatValuesOther AsynchronousMetrics::ProcStatValuesOther::operator-(
    const AsynchronousMetrics::ProcStatValuesOther &other) const {
...
AsynchronousMetrics::BlockDeviceStatValues AsynchronousMetrics::BlockDeviceStatValues::operator-(
    const AsynchronousMetrics::BlockDeviceStatValues &other) const {
...
AsynchronousMetrics::NetworkInterfaceStatValues AsynchronousMetrics::NetworkInterfaceStatValues::operator-(
    const AsynchronousMetrics::NetworkInterfaceStatValues &other) const {
...
void AsynchronousMetrics::applyCgroupCPUMetricsUpdate(AsynchronousMetricValues &new_values, const ProcStatValuesCPU &delta_values,
                                                      double multiplier) {
  ...
  new_values["CGroupSystemTime"] = {static_cast<double>(delta_values.system) * multiplier,
                                    "The ratio of time the CPU core was running OS kernel (system) code."};
}

void AsynchronousMetrics::applyCPUMetricsUpdate(AsynchronousMetricValues &new_values, const std::string &cpu_suffix,
                                                const ProcStatValuesCPU &delta_values, double multiplier) {
...
void AsynchronousMetrics::applyCgroupNormalizedCPUMetricsUpdate(AsynchronousMetricValues &new_values, double num_cpus_to_normalize,
                                                                const ProcStatValuesCPU &delta_values_all_cpus, double multiplier) {
...
void AsynchronousMetrics::applyNormalizedCPUMetricsUpdate(AsynchronousMetricValues &new_values, double num_cpus_to_normalize,
                                                          const ProcStatValuesCPU &delta_values_all_cpus, double multiplier) {
  chassert(num_cpus_to_normalize);
...
void readPressureFile(AsynchronousMetricValues &new_values, const std::string &type, ReadBufferFromFilePRead &in,
  ...
  while (!in.eof()) {
    ...
    if (!first_run) {
      ...
      new_values[metric_key] = AsynchronousMetricValue(
          delta,
...
const AsynchronousMetricValue *AsynchronousMetrics::getAsynchronousMetricValue(const AsynchronousMetricValues &values,
                                                                               std::string_view name) {
  if (auto it = values.find(String{name}); it != values.end()) return &it->second;
  return nullptr;
}
...
void AsynchronousMetrics::processWarningForMutationStats(const AsynchronousMetricValues &new_values) const {
  ...
  if (!num_pending_mutations) return;
  ...
  const auto *num_pending_mutations_over_execution_time =
      getAsynchronousMetricValue(new_values, "NumberOfPendingMutationsOverExecutionTime");
  if (!num_pending_mutations_over_execution_time) return;
...
void AsynchronousMetrics::processWarningForMemoryOverload(const AsynchronousMetricValues &new_values) const {
  ...
  if (user_space_cache_bytes > 0.0) memory_resident = std::max(0.0, memory_resident - user_space_cache_bytes);
  ...
  const auto warning_message = PreformattedMessage::create("High ClickHouse memory usage: {} of {} used ({}%) for at least {} second(s)",
                                                           formatReadableSizeWithDecimalSuffix(memory_resident),
                                                           formatReadableSizeWithDecimalSuffix(memory_total), usage_percent, min_duration);

  if (ratio >= mem_warn_ratio) {
    if (!mem_overload_started) mem_overload_started = now;
    ...
    if (ratio <= mem_clear_ratio) context->removeWarningMessage(Context::WarningType::SERVER_MEMORY_OVERLOAD);
  }
}
...
void AsynchronousMetrics::processWarningForCPUOverload(const AsynchronousMetricValues &new_values) const {
  ...
  const auto warning_message = PreformattedMessage::create("High CPU usage: {}% busy (>= {}%) for at least {} second(s)", busy_percent,
                                                           threshold_percent, min_duration);

  if (busy_time >= cpu_warn_ratio) {
    if (!cpu_overload_started) cpu_overload_started = now;
    ...
    if (busy_time <= cpu_clear_ratio) context->removeWarningMessage(Context::WarningType::SERVER_CPU_OVERLOAD);
  }
}
...
void AsynchronousMetrics::update(TimePoint update_time, bool force_update) {
  ...
  new_values["Jitter"] = {
      static_cast<double>(std::chrono::duration_cast<std::chrono::nanoseconds>(current_time - update_time).count()) / 1e9,
      "The difference in time the thread for calculation of the asynchronous metrics was scheduled to wake up and the time it was in fact, "
      "woken up."
...
#if defined(OS_LINUX) || defined(OS_FREEBSD)
  {
    ...
    new_values["MemoryVirtual"] = {data.virt,
                                   ...
                                   " The size of the virtual address space is usually much greater than the physical memory consumption, "
                                   "and should not be used as an estimate for the memory consumption."
                                   " The large values of this metric are totally normal, and makes only technical sense."};
    new_values["MemoryResident"] = {data.resident, "The amount of physical memory used by the server process, in bytes."};

    UInt64 page_cache_bytes = 0;
    if (context && context->getPageCache()) page_cache_bytes = context->getPageCache()->sizeInBytes();

    UInt64 resident_without_page_cache = (data.resident > page_cache_bytes) ? (data.resident - page_cache_bytes) : 0;
...
#  endif
    ...
    new_values["MemoryDataAndStack"] = {
        data.data_and_stack,
        "The amount of virtual memory mapped for the use of stack and for the allocated memory, in bytes."
        " It is unspecified whether it includes the per-thread stacks and most of the allocated memory, that is allocated with the 'mmap' "
        "system call."
        " This metric exists only for completeness reasons. I recommend to use the `MemoryResident` metric for monitoring."};

    if (update_rss) MemoryTracker::updateRSS(data.resident);
  }
  ...
  new_values["TrackedMemory"] = {total_memory_tracker.get(),
...
#  define LOAD_AVERAGE_DOCUMENTATION                                                                                               \
    ...
    " if the system is overloaded, and many processes are ready to run but waiting for CPU or IO."
      ...
      new_values["LoadAverage5"] = {
          loadavg5, "The whole system load, averaged with exponential smoothing over 5 minutes." LOAD_AVERAGE_DOCUMENTATION};
      new_values["LoadAverage15"] = {
          loadavg15, "The whole system load, averaged with exponential smoothing over 15 minutes." LOAD_AVERAGE_DOCUMENTATION};
      new_values["OSThreadsRunnable"] = {threads_runnable, "The total number of 'runnable' threads, as the OS kernel scheduler seeing it."};
      new_values["OSThreadsTotal"] = {threads_total, "The total number of threads, as the OS kernel scheduler seeing it."};
    } catch (...) {
      tryLogCurrentException(__PRETTY_FUNCTION__);

      /// A slight improvement for the rare case when ClickHouse is run inside LXC and LXCFS is used.
      /// The LXCFS has an issue: sometimes it returns an error "Transport endpoint is not connected" on reading from the file inside
      /// `/proc`. This error was correctly logged into ClickHouse's server log, but it was a source of annoyance to some users. We
      /// additionally workaround this issue by reopening a file.
      openFileIfExists("/proc/loadavg", loadavg);
    }
  }
  ...
  if (cgroupcpu_max) {
    try {
      ...
      if (quota > 0 && period > 0) max_cpu_cgroups = static_cast<double>(quota) / static_cast<double>(period);
    } catch (...) {
      tryLogCurrentException(__PRETTY_FUNCTION__);
    }
    ...
    try {
      ...
      if (!first_run) {
        auto get_clock_ticks = [&]() {
          if (auto hz = sysconf(_SC_CLK_TCK); hz != -1) return hz;
        ...
        const double multiplier =
            1.0 / cgroup_version_specific_divisor /
        ...
        if (max_cpu_cgroups > 0) applyCgroupNormalizedCPUMetricsUpdate(new_values, max_cpu_cgroups, delta_values, multiplier);
      }
  ...
  if (proc_stat) {
    try {
      ...
      if (-1 == hz) throw ErrnoException(ErrorCodes::CANNOT_SYSCONF, "Cannot call 'sysconf' to obtain system HZ");

      double multiplier =
          1.0 / static_cast<double>(hz) /
          (static_cast<double>(std::chrono::duration_cast<std::chrono::nanoseconds>(time_since_previous_update).count()) / 1e9);
      double num_cpus = 0;
      ...
      while (!proc_stat->eof()) {
        ...
        if (name.starts_with("cpu")) {
          ...
          if (!cpu_num_str.empty()) {
            ...
            if (proc_stat_values_per_cpu.size() <= cpu_num) proc_stat_values_per_cpu.resize(cpu_num + 1);
          }
      ...
      if (!first_run) {
        ...
        new_values["OSInterrupts"] = {delta_values.interrupts,
                                      "The number of interrupts on the host machine. This is a system-wide metric, it includes all the "
                                      "processes on the host machine, not just clickhouse-server."};
        new_values["OSContextSwitches"] = {
            delta_values.context_switches,
            "The number of context switches that the system underwent on the host machine. This is a system-wide metric, it includes all "
            "the processes on the host machine, not just clickhouse-server."};
        new_values["OSProcessesCreated"] = {delta_values.processes_created,
                                            "The number of processes created. This is a system-wide metric, it includes all the processes "
                                            "on the host machine, not just clickhouse-server."};
  ...
  if (cgroupmem_limit_in_bytes && cgroupmem_reader) {
    try {
      ...
      new_values["CGroupMemoryTotal"] = {
          limit, "The total amount of memory in cgroup, in bytes. If stated zero, the limit is the same as OSMemoryTotal."};
    ...
    try {
      ...
      while (!meminfo->eof()) {
        ...
        if (name == "MemTotal:") {
          ...
          new_values["OSMemoryFreeWithoutCached"] = {
              bytes,
          ...
          new_values["OSMemoryAvailable"] = {
              bytes,
              "The amount of memory available to be used by programs, in bytes. This is very similar to the `OSMemoryFreePlusCached` "
              "metric."
              " This is a system-wide metric, it includes all the processes on the host machine, not just clickhouse-server."};
        } else if (name == "Buffers:") {
          new_values["OSMemoryBuffers"] = {
              bytes,
              "The amount of memory used by OS kernel buffers, in bytes. This should be typically small, and large values may indicate a "
              "misconfiguration of the OS."
              " This is a system-wide metric, it includes all the processes on the host machine, not just clickhouse-server."};
        } else if (name == "Cached:") {
          free_plus_cached_bytes += bytes;
          new_values["OSMemoryCached"] = {
              bytes,
              "The amount of memory used by the OS page cache, in bytes. Typically, almost all available memory is used by the OS page "
              "cache - high values of this metric are normal and expected."
              " This is a system-wide metric, it includes all the processes on the host machine, not just clickhouse-server."};
        } else if (name == "SwapCached:") {
          new_values["OSMemorySwapCached"] = {
              bytes,
              "The amount of memory in swap that was also loaded in RAM. Swap should be disabled on production systems. If the value of "
              "this metric is large, it indicates a misconfiguration."
              " This is a system-wide metric, it includes all the processes on the host machine, not just clickhouse-server."};
        }
      ...
      new_values["OSMemoryFreePlusCached"] = {
          free_plus_cached_bytes,
          "The amount of free memory plus OS page cache memory on the host system, in bytes. This memory is available to be used by "
          "programs. The value should be very similar to `OSMemoryAvailable`."
          " This is a system-wide metric, it includes all the processes on the host machine, not just clickhouse-server."};
    } catch (...) {
      tryLogCurrentException(__PRETTY_FUNCTION__);
      openFileIfExists("/proc/meminfo", meminfo);
    ...
    try {
      ...
      while (!cpuinfo->eof()) {
        ...
        if (s.starts_with(PROCESSOR)) {
          ...
          while (core_id_start < std::ssize(s) && !std::isdigit(s[core_id_start])) ++core_id_start;
          ...
          if (auto colon = s.find_first_of(':')) {
            ...
            new_values[fmt::format("CPUFrequencyMHz_{}", core_id)] = {
                mhz,
                "The current frequency of the CPU, in MHz. Most of the modern CPUs adjust the frequency dynamically for power saving and "
                "Turbo Boosting."};
          }
        }
      }
  ...
  if (file_nr) {
    try {
      ...
      new_values["OSOpenFiles"] = {
          open_files,
          "The total number of opened files on the host machine."
          " This is a system-wide metric, it includes all the processes on the host machine, not just clickhouse-server."};
    } catch (...) {
      tryLogCurrentException(__PRETTY_FUNCTION__);
      openFileIfExists("/proc/sys/fs/file-nr", file_nr);
  ...
  if (block_devices_rescan_delay.elapsedSeconds() >= 300) openBlockDevices();
  ...
  try {
    for (auto &[name, device] : block_devs) {
      ...
      try {
        ...
        LOG_DEBUG(log, "Cannot read statistics about the block device '{}': {}.", name, errnoToString(e.getErrno()));
        continue;
      }
...
#  define BLOCK_DEVICE_EXPLANATION                                                                                  \
    ...
    " Source: `/sys/block`. See https://www.kernel.org/doc/Documentation/block/stat.txt"
      ...
      new_values["BlockReadOps_" + name] = {delta_values.read_ios,
                                            "Number of read operations requested from the block device." BLOCK_DEVICE_EXPLANATION};
      new_values["BlockWriteOps_" + name] = {delta_values.write_ios,
                                             "Number of write operations requested from the block device." BLOCK_DEVICE_EXPLANATION};
      new_values["BlockDiscardOps_" + name] = {
          delta_values.discard_ops,
          "Number of discard operations requested from the block device. These operations are relevant for SSD."
          " Discard operations are not used by ClickHouse, but can be used by other processes on the system." BLOCK_DEVICE_EXPLANATION};

      new_values["BlockReadMerges_" + name] = {
          delta_values.read_merges,
          "Number of read operations requested from the block device and merged together by the OS IO scheduler." BLOCK_DEVICE_EXPLANATION};
      new_values["BlockWriteMerges_" + name] = {delta_values.write_merges,
                                                "Number of write operations requested from the block device and merged together by the OS "
                                                "IO scheduler." BLOCK_DEVICE_EXPLANATION};
      new_values["BlockDiscardMerges_" + name] = {
          delta_values.discard_merges,
          "Number of discard operations requested from the block device and merged together by the OS IO scheduler."
          " These operations are relevant for SSD. Discard operations are not used by ClickHouse, but can be used by other processes on "
          "the system." BLOCK_DEVICE_EXPLANATION};

      new_values["BlockReadBytes_" + name] = {delta_values.read_sectors * sector_size,
                                              "Number of bytes read from the block device."
                                              " It can be lower than the number of bytes read from the filesystem due to the usage of the "
                                              "OS page cache, that saves IO." BLOCK_DEVICE_EXPLANATION};
      new_values["BlockWriteBytes_" + name] = {
          delta_values.write_sectors * sector_size,
          ...
          " A write to the block device may happen later than the corresponding write to the filesystem due to write-through "
          "caching." BLOCK_DEVICE_EXPLANATION};
      new_values["BlockDiscardBytes_" + name] = {delta_values.discard_sectors * sector_size,
                                                 "Number of discarded bytes on the block device."
                                                 " These operations are relevant for SSD. Discard operations are not used by ClickHouse, "
                                                 "but can be used by other processes on the system." BLOCK_DEVICE_EXPLANATION};

      new_values["BlockReadTime_" + name] = {static_cast<double>(delta_values.read_ticks) * time_multiplier,
                                             "Time in seconds spend in read operations requested from the block device, summed across all "
                                             "the operations." BLOCK_DEVICE_EXPLANATION};
      new_values["BlockWriteTime_" + name] = {static_cast<double>(delta_values.write_ticks) * time_multiplier,
                                              "Time in seconds spend in write operations requested from the block device, summed across "
                                              "all the operations." BLOCK_DEVICE_EXPLANATION};
      new_values["BlockDiscardTime_" + name] = {
          static_cast<double>(delta_values.discard_ticks) * time_multiplier,
          "Time in seconds spend in discard operations requested from the block device, summed across all the operations."
          " These operations are relevant for SSD. Discard operations are not used by ClickHouse, but can be used by other processes on "
          "the system." BLOCK_DEVICE_EXPLANATION};

      new_values["BlockInFlightOps_" + name] = {
          delta_values.in_flight_ios,
          "This value counts the number of I/O requests that have been issued to"
          " the device driver but have not yet completed. It does not include IO"
          " requests that are in the queue but not yet issued to the device driver." BLOCK_DEVICE_EXPLANATION};
      new_values["BlockActiveTime_" + name] = {static_cast<double>(delta_values.io_ticks) * time_multiplier,
                                               "Time in seconds the block device had the IO requests queued." BLOCK_DEVICE_EXPLANATION};
      new_values["BlockQueueTime_" + name] = {static_cast<double>(delta_values.time_in_queue) * time_multiplier,
                                              ...
                                              " number of requests waiting." BLOCK_DEVICE_EXPLANATION};

      if (delta_values.in_flight_ios) {
        /// TODO Check if these values are meaningful.

        new_values["BlockActiveTimePerOp_" + name] = {
            static_cast<double>(delta_values.io_ticks) * time_multiplier / static_cast<double>(delta_values.in_flight_ios),
            "Similar to the `BlockActiveTime` metrics, but the value is divided to the number of IO operations to count the per-operation "
            "time."};
        new_values["BlockQueueTimePerOp_" + name] = {
            static_cast<double>(delta_values.time_in_queue) * time_multiplier / static_cast<double>(delta_values.in_flight_ios),
            "Similar to the `BlockQueueTime` metrics, but the value is divided to the number of IO operations to count the per-operation "
            "time."};
      }
    }
  } catch (...) {
  ...
  if (net_dev) {
    try {
      ...
      while (!net_dev->eof()) {
        ...
        if (!first_run) {
          new_values["NetworkReceiveBytes_" + interface_name] = {
              delta_values.recv_bytes,
              " Number of bytes received via the network interface."
              " This is a system-wide metric, it includes all the processes on the host machine, not just clickhouse-server."};
          new_values["NetworkReceivePackets_" + interface_name] = {
              delta_values.recv_packets,
              " Number of network packets received via the network interface."
              " This is a system-wide metric, it includes all the processes on the host machine, not just clickhouse-server."};
          new_values["NetworkReceiveErrors_" + interface_name] = {
              delta_values.recv_errors,
              " Number of times error happened receiving via the network interface."
              " This is a system-wide metric, it includes all the processes on the host machine, not just clickhouse-server."};
          new_values["NetworkReceiveDrop_" + interface_name] = {
              delta_values.recv_drop,
              " Number of bytes a packet was dropped while received via the network interface."
              " This is a system-wide metric, it includes all the processes on the host machine, not just clickhouse-server."};

          new_values["NetworkSendBytes_" + interface_name] = {
              delta_values.send_bytes,
              " Number of bytes sent via the network interface."
              " This is a system-wide metric, it includes all the processes on the host machine, not just clickhouse-server."};
          new_values["NetworkSendPackets_" + interface_name] = {
              delta_values.send_packets,
              " Number of network packets sent via the network interface."
              " This is a system-wide metric, it includes all the processes on the host machine, not just clickhouse-server."};
          new_values["NetworkSendErrors_" + interface_name] = {
              delta_values.send_errors,
              " Number of times error (e.g. TCP retransmit) happened while sending via the network interface."
              " This is a system-wide metric, it includes all the processes on the host machine, not just clickhouse-server."};
          new_values["NetworkSendDrop_" + interface_name] = {
              delta_values.send_drop,
  ...
  if (net_tcp || net_tcp6) {
    ...
    auto process_net = [&](const char *path, auto &file) {
      try {
        ...
        while (!file->eof()) {
          ...
          skipWhitespaceIfAny(*file, true);
          if (auto pos = remote_address_and_port.find(':'); pos != std::string::npos) remote_address_and_port.resize(pos);
    ...
    if (net_tcp) process_net("/proc/net/tcp", net_tcp);

    if (net_tcp6) process_net("/proc/net/tcp6", net_tcp6);
    ...
    auto process_socket_state = [&](UInt8 state, const char *description) {
      if (state < 16 && sockets_by_state[state])
        new_values[fmt::format("NetworkTCPSockets_{}", description)] = {
            sockets_by_state[state], "Total number of network sockets in the specific state on the server across TCPv4 and TCPv6."};
    };
    ...
    new_values["NetworkTCPUnrecoveredRetransmits"] = {
    ...
    new_values["NetworkTCPSocketRemoteAddresses"] = {
        remote_addresses.size(), "Total number of unique remote addresses of network sockets used on the server across TCPv4 and TCPv6."};
  }
  ...
  if (vm_max_map_count) {
    try {
      ...
      new_values["VMMaxMapCount"] = {max_map_count,
  ...
  if (vm_maps) {
    try {
      ...
      while (!vm_maps->eof()) {
        ...
        if (!vm_maps->hasPendingData()) continue;
  ...
  if (process_status) {
    try {
      ...
      if (signal_queue_limit == 0) LOG_WARNING(getLogger("AsynchronousMetrics"), "Cannot find SigQ in /proc/self/status");
      new_values["ProcessSignalQueueSize"] = {signal_queue_size, "Size of signal queue (pending signals, timers for query profiling)"};
      new_values["ProcessSignalQueueLimit"] = {
          signal_queue_limit,
  ...
  try {
    for (size_t i = 0, size = thermal.size(); i < size; ++i) {
      ...
      new_values[fmt::format("Temperature{}", i)] = {
          static_cast<double>(temperature) * 0.001,
    ...
    for (const auto &[hwmon_name, sensors] : hwmon_devices) {
      for (const auto &[sensor_name, sensor_file] : sensors) {
        ...
        try {
          ...
          LOG_DEBUG(log, "Hardware monitor '{}', sensor '{}' exists but could not be read: {}.", hwmon_name, sensor_name,
                    errnoToString(e.getErrno()));
          continue;
        }
        ...
        if (sensor_name.empty())
          new_values[fmt::format("Temperature_{}", hwmon_name)] = {static_cast<double>(temperature) * 0.001,
                                                                   "The temperature reported by the corresponding hardware monitor in ℃. A "
                                                                   "sensor can return an unrealistic value. Source: `/sys/class/hwmon`"};
        else
          new_values[fmt::format("Temperature_{}_{}", hwmon_name, sensor_name)] = {
              static_cast<double>(temperature) * 0.001,
              "The temperature reported by the corresponding hardware monitor and the corresponding sensor in ℃. A sensor can return an "
              "unrealistic value. Source: `/sys/class/hwmon`"};
      }
    }
  } catch (...) {
    if (errno != ENODATA)  /// Ok for thermal sensors.
  ...
  try {
    for (size_t i = 0, size = edac.size(); i < size; ++i) {
      ...
      if (edac[i].first) {
        ...
        uint64_t errors = 0;
        readText(errors, in);
        new_values[fmt::format("EDAC{}_Correctable", i)] = {
            errors,
            ...
            " because in presence of a high number of corrected errors, a number of silent errors may happen as well, leading to data "
            "corruption."
      ...
      if (edac[i].second) {
        ...
        uint64_t errors = 0;
        readText(errors, in);
        new_values[fmt::format("EDAC{}_Uncorrectable", i)] = {
            errors,
...

  {
    auto threads_get_metric_name_doc = [](const String &name) -> std::pair<const char *, const char *> {
      static std::map<String, std::pair<const char *, const char *>> metric_map = {
          ...
          {"prometheus.port",
           {"PrometheusThreads",
            "Number of threads in the server of the Prometheus endpoint. Note: prometheus endpoints can be also used via the usual "
            "HTTP/HTTPs ports."}},
      ...
      if (it == metric_map.end()) return {nullptr, nullptr};
      return it->second;
    };
      ...
      static std::map<String, std::pair<const char *, const char *>> metric_map = {
          ...
          {"prometheus.port",
           {"PrometheusRejectedConnections",
            "Number of rejected connections for the Prometheus endpoint. Note: prometheus endpoints can be also used via the usual "
            "HTTP/HTTPs ports."}},
  ...
  new_values["OSCPUOverload"] = {
      ProfileEvents::global_counters.getCPUOverload(context->getServerSettings()[ServerSetting::os_cpu_busy_time_threshold],
                                                    /*reset*/ true),
      "Relative CPU deficit, calculated as: how many threads are waiting for CPU relative to the number of threads, using CPU. If it is "
      "greater than zero, the server would benefit from more CPU. If it is significantly greater than zero, the server could become "
      "unresponsive. The metric is accumulated between the updates of asynchronous metrics."};
  ...
  new_values["AsynchronousMetricsCalculationTimeSpent"] = {
