...
inline Value get(Metric metric) { return values[metric].load(std::memory_order_relaxed); }
...
inline void max(Metric metric, Value value) { __atomic_fetch_max(reinterpret_cast<Value *>(&values[metric]), value, __ATOMIC_RELAXED); }
...
class Increment {
 ...
 public:
  explicit Increment(Metric metric, Value amount_ = 1) : Increment(&values[metric], amount_) {
  ...
  ~Increment() {
    if (what) what->fetch_sub(amount, std::memory_order_relaxed);
  }

  Increment(Increment &&old) noexcept { *this = std::move(old); }
