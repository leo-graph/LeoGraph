...
void FieldVisitorEncodeBinary::operator()(const Array &x, WriteBuffer &buf) const {
  ...
  for (size_t i = 0; i < size; ++i) Field::dispatch([&buf](const auto &value) { FieldVisitorEncodeBinary()(value, buf); }, x[i]);
}
...
T decodeArrayLikeField(ReadBuffer &buf) {
  ...
  T value;
  for (size_t i = 0; i != size; ++i) value.push_back(decodeField(buf));
  return value;
}
