...
inline UInt64 clock_gettime_ns(clockid_t clock_type = STOPWATCH_DEFAULT_CLOCK) {
  struct timespec ts;
  if (0 != clock_gettime(clock_type, &ts)) throw std::system_error(std::error_code(errno, std::system_category()));
...
#ifdef CLOCK_MONOTONIC_RAW
  if (likely(clock_type == CLOCK_MONOTONIC_RAW)) return clock_gettime_ns(clock_type);
#endif

  UInt64 current_time = clock_gettime_ns(clock_type);
  if (likely(prev_time <= current_time)) return current_time;
...
class AtomicStopwatch {
 public:
  ...
  bool compareAndRestart(double seconds) {
    ...
    while (true) {
      if (current_ns < current_start_ns + threshold) return false;

      if (start_ns.compare_exchange_weak(current_start_ns, current_ns)) return true;
    }
  }
  ...
  Lock compareAndRestartDeferred(double seconds) {
    ...
    while (true) {
      if ((current_start_ns & 0x8000000000000000ULL)) return {};

      if (current_ns < current_start_ns + threshold) return {};

      if (start_ns.compare_exchange_weak(current_start_ns, current_ns | 0x8000000000000000ULL)) return Lock(this);
    }
  }
