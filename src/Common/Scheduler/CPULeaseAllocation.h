...
class CPULeaseAllocation final : public ISlotAllocation {
 ...
 public:
  CPULeaseAllocation(SlotCount max_threads_, ResourceLink master_link_, ResourceLink worker_link_, CPULeaseSettings settings = {});
 ...
 private:
  ...
  struct Threads {
    explicit Threads(size_t max_threads_) : leased(max_threads_), preempted(max_threads_), wake(max_threads_) {}
  ...
  /// Also, the described logic is required to avoid "bursts" when all quanta for all threads are exhausted simultaneously, and the query is
  /// stuck because it no longer has preallocated resources. So, decoupling threads from resource requests is necessary to provide a stable,
  /// burst-less flow of requests from the query to the scheduler. NOTE: The number of running threads may not be equal to the number of
  /// requests in "consuming" state, because we allow a thread to run while corresponding request is enqueued, but not yet granted (see
  /// renew() method).
  class RequestChain {
   public:
