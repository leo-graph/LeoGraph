...
AcquiredCPUSlot::AcquiredCPUSlot(SlotAllocationPtr &&allocation_, CPUSlotRequest *request_, size_t slot_id_)
    : IAcquiredSlot(slot_id_),
      ...
      request(request_),
      acquired_slot_increment(request ? CurrentMetrics::ConcurrencyControlAcquired
...
CPUSlotsAllocation::CPUSlotsAllocation(SlotCount master_slots_, SlotCount worker_slots_, ResourceLink master_link_,
    ...
    : master_slots(master_slots_),
      ...
      requests(total_slots -
               noncompeting_slots)  // NOTE: it should not be reallocated after initialization because AcquiredCPUSlot holds raw pointer
      ,
      current_request(requests.empty() ? nullptr : &requests.front()) {
  for (CPUSlotRequest &request : requests) request.allocation = this;
...
CPUSlotsAllocation::~CPUSlotsAllocation() {
  if (master_link || worker_link) {
    ...
    for (size_t index = last_acquire_index; granted > 0; --granted, ++index) requests[index].finish();
  }
}
...
[[nodiscard]] AcquiredSlotPtr CPUSlotsAllocation::tryAcquire() {
  ...
  while (value) {
    if (granted.compare_exchange_strong(value, value - 1)) {
      ...
      return AcquiredSlotPtr(
          new AcquiredCPUSlot(shared_from_this(), &requests[index], last_slot_id.fetch_add(1, std::memory_order_relaxed)));
    }
  }
...
[[nodiscard]] AcquiredSlotPtr CPUSlotsAllocation::acquire() {
  while (true) {
    if (auto result = tryAcquire()) return result;
    std::unique_lock lock{schedule_mutex};
    waiters++;
...
void CPUSlotsAllocation::grant() {
  ...
  if (allocated < total_slots) {
    // TODO(serxa): we should not request more slots if we already have at least 2 granted and not acquired slots to avoid holding
    // unnecessary slots
    if (ISchedulerQueue *queue = getCurrentQueue(lock))  // competing slot - use scheduler
    {
      queue->enqueueRequest(current_request);
