...
class SchedulerRoot final : public ISchedulerNode {
 private:
  struct Resource {
    ...
    explicit Resource(const SchedulerNodePtr &root_) : root(root_) { root->info.parent.ptr = this; }

    // Get pointer stored by ctor in info
    static Resource *get(SchedulerNodeInfo &info) { return reinterpret_cast<Resource *>(info.parent.ptr); }
  };

 public:
  explicit SchedulerRoot() : ISchedulerNode(&events) {}

  ~SchedulerRoot() override {
    stop();
    while (!children.empty()) removeChild(children.begin()->first);
  }

  /// Runs separate scheduler thread
  void start(ThreadName name) {
    if (!scheduler.joinable()) scheduler = ThreadFromGlobalPool([this, name] { schedulerThread(name); });
  }
      ...
      if (graceful) {
        ...
        while (has_work) {
          ...
          if (request)
            ...
            has_work = false;
          while (events.forceProcess()) has_work = true;
        }
      }
    }
  }
  ...
  bool equals(ISchedulerNode *other) override {
    ...
    if (auto *_ = dynamic_cast<SchedulerRoot *>(other)) return true;
    return false;
  }
  ...
  void attachChild(const SchedulerNodePtr &child) override {
    ...
    if (auto [it, inserted] = children.emplace(child.get(), child); !inserted)
      throw Exception(ErrorCodes::INVALID_SCHEDULER_NODE, "Can't add the same scheduler node twice");
  ...
  bool isActive() override { return current != nullptr; }

  size_t activeChildren() override { return 0; }

  void activateChild(ISchedulerNode *child) override { activate(Resource::get(child->info)); }
 ...
 private:
  ...
  void deactivate(Resource *value) {
    if (value->next == nullptr) return;  // Already deactivated
