...
struct SchedulerNodeInfo {
  ...
  void setWeight(double value) {
    if (value <= 0 || !isfinite(value))
      throw Exception(ErrorCodes::INVALID_SCHEDULER_NODE, "Zero, negative and non-finite node weights are not allowed: {}", value);
    weight = value;
  }

  void setQueueSize(Int64 value) {
    if (value <= 0)
      throw Exception(ErrorCodes::INVALID_SCHEDULER_NODE, "Workload setting `max_waiting_queries` value must be positive, got: {}", value);
    queue_size = value;
  }

  void setPriority(Int64 value) { priority.value = value; }
...
class ISchedulerNode : public boost::intrusive::list_base_hook<>, private boost::noncopyable {
 public:
  explicit ISchedulerNode(EventQueue *event_queue_, const Poco::Util::AbstractConfiguration &config = emptyConfig(),
                          const String &config_prefix = {})
      : event_queue(event_queue_), info(config, config_prefix) {}

  ISchedulerNode(EventQueue *event_queue_, const SchedulerNodeInfo &info_) : event_queue(event_queue_), info(info_) {}
  ...
  virtual bool equals(ISchedulerNode *other) { return info.equals(other->info); }
...
class EventQueue {
 public:
  ...
  struct Event {
    ...
    Event(EventId event_id_, Task &&task_) : event_id(event_id_), task(std::move(task_)) {}
  };
  ...
  struct Postponed {
    ...
    Postponed(TimePoint key_, EventId event_id_, Task &&task_)
        : key(key_), event_id(event_id_), task(std::make_unique<Task>(std::move(task_))) {}
  ...
  [[nodiscard]] EventId postpone(TimePoint until, Task &&task) {
    ...
    if (postponed.empty() || until < postponed.front().key) pending.notify_one();
  ...
  bool cancelPostponed(EventId postponed_event_id) {
    if (postponed_event_id == not_postponed) return false;
    std::unique_lock lock{mutex};
  ...
  void enqueue(Task &&task) {
    ...
    if (was_empty) pending.notify_one();
  }
  ...
  void cancelActivation(ISchedulerNode *node) {
    std::unique_lock lock{mutex};
    if (node->is_linked()) activations.erase(activations.iterator_to(*node));
  ...
  bool tryProcess() {
    ...
    if (postponed.empty()) return false;
  ...
  TimePoint now() {
    ...
    if (likely(result == TimePoint())) return std::chrono::system_clock::now();
    return result;
  }
 ...
 private:
  void wait(std::unique_lock<std::mutex> &lock) { pending.wait(lock); }
  ...
  void processActivation(std::unique_lock<std::mutex> &&lock) {
    ...
    if (node->parent) node->parent->activateChild(node);
  }
 ...
 public:
  ...
  struct SchedulerThread {
    ...
    ~SchedulerThread() { current_thread_queue = nullptr; }
...
inline void ISchedulerNode::setParent(ISchedulerNode *parent_) {
  ...
  if (parent == nullptr) event_queue->cancelActivation(this);
}
