...
#if 0
#  define LOG_EVENT(X)                                                                                                                  \
    LOG_TRACE(log, "{}:{} ({}) allocated={} granted={} running={} L:{} P:{} <{}/{}> e:{}", lease_id, settings.workload, #X, allocated,  \
              granted, threads.running_count, formatBitset(threads.leased), formatBitset(threads.preempted), consumed_ns, requested_ns, \
              requests.hasEnqueued())
namespace
{
...
CPULeaseAllocation::Lease::Lease(CPULeaseAllocationPtr &&parent_, size_t slot_id_) : ISlotLease(slot_id_), parent(std::move(parent_)) {}
...
CPULeaseAllocation::RequestChain::RequestChain(CPULeaseAllocation *lease, size_t max_threads_, ResourceLink master_link_,
  ...
  for (Request &request : requests) request.lease = lease;
}

void CPULeaseAllocation::RequestChain::finish() {
  tail->finish();
  if (tail->is_master_slot) request_master_slot = true;
  ++tail;
  if (tail == requests.end()) tail = requests.begin();
}

void CPULeaseAllocation::RequestChain::granted() {
  ++head;
  if (head == requests.end()) head = requests.begin();
}
...
bool CPULeaseAllocation::RequestChain::enqueue(ResourceCost cost, ResourceCost requested_ns_) {
  ...
  if (auto *queue = head->is_master_slot ? master_link.queue : worker_link.queue) {
    ...
    // We do not use enqueueRequestUsingBudget() because it redistributes resource between requests in the queue (which might be from
    // different queries). Instead we do budgeting for every query independently for better fairness
...
void CPULeaseAllocation::RequestChain::scheduled() {
  ...
  if (wait_cancel) cancel_cv.notify_one();
}

CPULeaseAllocation::CPULeaseAllocation(SlotCount max_threads_, ResourceLink master_link_, ResourceLink worker_link_,
                                       CPULeaseSettings settings_)
  ...
  if (!schedule(lock)) grantImpl(lock);
}

CPULeaseAllocation::~CPULeaseAllocation() { free(); }
...
[[nodiscard]] AcquiredSlotPtr CPULeaseAllocation::tryAcquire() {
  if (!acquirable.load(std::memory_order_relaxed)) return {};  // shortcut to avoid unnecessary mutex locking

  std::unique_lock lock{mutex};
  if (exception)
    throw Exception(ErrorCodes::RESOURCE_ACCESS_DENIED, "CPU Resource request failed: {}",
                    getExceptionMessage(exception, /* with_stacktrace = */ false));
  if (granted > 0) return acquireImpl(lock);
  return {};
}

[[nodiscard]] AcquiredSlotPtr CPULeaseAllocation::acquire() {
  std::unique_lock lock{mutex};
  if (threads.leased.count() == max_threads) return {};  // Max number of threads already acquired
...
size_t CPULeaseAllocation::upscale() {
  ...
  if (granted <= 0 && !exception) acquirable.store(false, std::memory_order_relaxed);
...
void CPULeaseAllocation::downscale(size_t thread_num, bool shutdown_) {
  if (!shutdown_) ProfileEvents::increment(ProfileEvents::ConcurrencyControlDownscales);
  ...
  if (threads.preempted[thread_num])
    ...
    if (threads.last_running == thread_num) {
      while (threads.last_running-- > 0) {
        if (threads.leased[threads.last_running] && !threads.preempted[threads.last_running]) break;
      }
    }

    // We have stopped a running thread that held an acquired slot, which becomes granted
    ++granted;
    if (granted > 0 && !shutdown) acquirable.store(true, std::memory_order_relaxed);
  }
  LOG_EVENT(D);
}
...
void CPULeaseAllocation::failed(const std::exception_ptr &ptr) {
  ...
  for (auto &cv : threads.wake) cv.notify_one();
...
void CPULeaseAllocation::grantImpl(std::unique_lock<std::mutex> &lock) {
  ...
  // TODO(serxa): we should release granted but not acquired slots after some timeout, to avoid unnecessary overprovisioning, but this
  // requires modification of the PipelineExecutor as well
}
...
bool CPULeaseAllocation::renew(Lease &lease) {
  ...
  if (delta_ns < settings.report_ns) return true;  // Not enough time passed to report
  ...
  if (granted + static_cast<Int64>(requests.hasEnqueued()) < 0 || consumed_ns >= requested_ns) {
    ...
    if (thread_num == threads.last_running) {
      ...
      if (settings.on_resume) settings.on_resume(thread_num);
...
bool CPULeaseAllocation::waitForGrant(std::unique_lock<std::mutex> &lock, size_t thread_num) {
  ...
  auto predicate = [this, thread_num] { return !threads.preempted[thread_num] || exception || shutdown; };
...
bool CPULeaseAllocation::schedule(std::unique_lock<std::mutex> &) {
  if (allocated == max_threads || shutdown) return true;
...
void CPULeaseAllocation::release(Lease &lease) {
  ...
  try {
    ...
    if (e.code() != ErrorCodes::INVALID_SCHEDULER_NODE) throw;
  }
