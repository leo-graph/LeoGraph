...
WorkloadEntityConfigStorage::WorkloadEntityConfigStorage(const ContextPtr &global_context_) : WorkloadEntityStorageBase(global_context_) {}
...
WorkloadEntityConfigStorage::OperationResult WorkloadEntityConfigStorage::storeEntityImpl(const ContextPtr &, WorkloadEntityType,
                                                                                          const String &, ASTPtr, bool, bool,
                                                                                          const Settings &) {
  // Config storage is read-only - entities come from config, not SQL. This function should not be called
  return OperationResult::Failed;
}

WorkloadEntityConfigStorage::OperationResult WorkloadEntityConfigStorage::removeEntityImpl(const ContextPtr &, WorkloadEntityType,
                                                                                           const String &, bool) {
  // Config storage is read-only - entities come from config, not SQL. This function should not be called
  return OperationResult::Failed;
}
