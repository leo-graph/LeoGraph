...
WorkloadEntityKeeperStorage::WorkloadEntityKeeperStorage(const ContextPtr &global_context_, const String &zookeeper_path_,
                                                         std::unique_ptr<IWorkloadEntityStorage> next_storage_)
    : WorkloadEntityStorageBase(global_context_, std::move(next_storage_)),
      ...
      zookeeper_watch(std::make_shared<Coordination::WatchCallback>([my_watch = watch](const Coordination::WatchResponse &response) {
  ...
  if (zookeeper_path.back() == '/') zookeeper_path.pop_back();

  /// If zookeeper chroot prefix is used, path should start with '/', because chroot concatenates without it.
  if (zookeeper_path.front() != '/') zookeeper_path = "/" + zookeeper_path;
}

WorkloadEntityKeeperStorage::~WorkloadEntityKeeperStorage() { SCOPE_EXIT_SAFE(stopWatchingThread()); }

void WorkloadEntityKeeperStorage::startWatchingThread() {
  if (!watching_flag.exchange(true)) watching_thread = ThreadFromGlobalPool(&WorkloadEntityKeeperStorage::processWatchQueue, this);
}
...
void WorkloadEntityKeeperStorage::loadEntities(const Poco::Util::AbstractConfiguration &config) {
  ...
  /// loadEntities() is called at startup and on configuration reload, so it's better not to stop here on no connection to ZooKeeper or any
  /// other error. However the watching thread must be started anyway in case the connection will be established later.
  try {
    auto lock = getLock();
  ...
  while (watching_flag) {
    try {
...

      {
        ...
        if (!watch->cv.wait_for(lock, std::chrono::seconds(10), [&] { return !watching_flag || handled != watch->triggered; })) continue;
...
void WorkloadEntityKeeperStorage::stopWatching() { stopWatchingThread(); }
...
WorkloadEntityStorageBase::OperationResult WorkloadEntityKeeperStorage::storeEntityImpl(
    const ContextPtr & /*current_context*/, WorkloadEntityType entity_type, const String &entity_name, ASTPtr create_entity_query,
    bool /*throw_if_exists*/, bool /*replace_if_exists*/, const Settings &) {
...
WorkloadEntityStorageBase::OperationResult WorkloadEntityKeeperStorage::removeEntityImpl(const ContextPtr & /*current_context*/,
