...
WorkloadEntityType getEntityType(const ASTPtr &ptr) {
  if (auto *res = typeid_cast<ASTCreateWorkloadQuery *>(ptr.get()); res) return WorkloadEntityType::Workload;
  if (auto *res = typeid_cast<ASTCreateResourceQuery *>(ptr.get()); res) return WorkloadEntityType::Resource;
  chassert(false);
...
bool entityEquals(const ASTPtr &lhs, const ASTPtr &rhs) {
  ...
  if (auto *a = typeid_cast<ASTCreateResourceQuery *>(lhs.get())) {
    if (auto *b = typeid_cast<ASTCreateResourceQuery *>(rhs.get()))
      return std::forward_as_tuple(a->getResourceName(), a->operations) == std::forward_as_tuple(b->getResourceName(), b->operations);
  }
  return false;
}
...
void forEachReference(const ASTPtr &source_entity,
  ...
  if (auto *res = typeid_cast<ASTCreateWorkloadQuery *>(source_entity.get())) {
    ...
    if (!parent.empty()) func(parent, res->getWorkloadName(), ReferenceType::Parent);
    ...
    for (const auto &[name, value, resource] : res->changes) {
      if (!resource.empty()) resources.insert(resource);
    }
    for (const String &resource : resources) func(resource, res->getWorkloadName(), ReferenceType::ForResource);
  }
...
void topologicallySortedWorkloadsImpl(const String &name, const ASTPtr &ast, const std::unordered_map<String, ASTPtr> &workloads,
                                      std::unordered_set<String> &visited, std::vector<std::pair<String, ASTPtr>> &sorted_workloads) {
...
std::vector<std::pair<String, ASTPtr>> topologicallySortedWorkloads(const std::unordered_map<String, ASTPtr> &workloads) {
  ...
  for (const auto &[name, ast] : workloads) topologicallySortedWorkloadsImpl(name, ast, workloads, visited, sorted_workloads);
  return sorted_workloads;
}

/// Helper for recursive DFS
void topologicallySortedDependenciesImpl(const String &name, const std::unordered_map<String, std::unordered_set<String>> &dependencies,
                                         std::unordered_set<String> &visited, std::vector<String> &result) {
  if (visited.contains(name)) return;
  visited.insert(name);

  if (auto it = dependencies.find(name); it != dependencies.end()) {
    for (const String &dep : it->second) topologicallySortedDependenciesImpl(dep, dependencies, visited, result);
  }
...
std::vector<String> topologicallySortedDependencies(const std::unordered_map<String, std::unordered_set<String>> &dependencies) {
  ...
  for (const auto &[name, _] : dependencies) topologicallySortedDependenciesImpl(name, dependencies, visited, result);
...
std::vector<EntityChange> topologicallySortedChanges(const std::vector<EntityChange> &changes) {
  ...
  for (const auto &change : changes) change_by_name[change.name] = &change;
  ...
  for (const auto &change : changes) {
    if (change.before) {
      forEachReference(change.before,
                       [&](const String &target, const String &source, ReferenceType) { old_sources[target].insert(source); });
    }
    if (change.after) {
      forEachReference(change.after,
                       [&](const String &target, const String &source, ReferenceType) { new_targets[source].insert(target); });
    }
  }
  ...
  std::unordered_map<String, std::unordered_set<String>>
      dependencies;  // Key is entity name. Value is set of names of entity that should be changed first.
  for (const auto &change : changes) {
    ...
    for (const auto &event : change.toEvents()) {
      if (!event.entity)  // DROP
      {
        ...
        for (const String &source : old_sources[event.name]) {
          if (change_by_name.contains(source)) dependencies[event.name].insert(source);
        }
      } else  // CREATE || CREATE OR REPLACE
      {
        ...
        for (const String &target : new_targets[event.name]) {
          if (auto it = change_by_name.find(target); it != change_by_name.end()) {
            ...
            if (!target_change.before) dependencies[event.name].insert(target);
          }
        }
      }
    }
  ...
  for (const String &name : topologicallySortedDependencies(dependencies)) result.push_back(*change_by_name[name]);
...
{
  if (next_storage) {
    subscription = next_storage->getAllEntitiesAndSubscribe([this](const std::vector<Event> &other_tx) {
      ...
      for (const auto &other_event : other_tx) {
        ...
        if (other_event.entity)
          ...
          if (auto it = local_entities.find(change.name); it != local_entities.end()) change.after = it->second;
        }
...
ASTPtr WorkloadEntityStorageBase::get(const String &entity_name) const {
  if (auto result = tryGet(entity_name)) return result;
  throw Exception(ErrorCodes::BAD_ARGUMENTS, "The workload entity name '{}' is not saved", entity_name);
}
...
bool WorkloadEntityStorageBase::has(const String &entity_name) const { return tryGet(entity_name) != nullptr; }
...
void WorkloadEntityStorageBase::loadEntities(const Poco::Util::AbstractConfiguration &config) {
  if (next_storage) next_storage->loadEntities(config);
}

bool WorkloadEntityStorageBase::storeEntity(const ContextPtr &current_context, WorkloadEntityType entity_type, const String &entity_name,
                                            ASTPtr create_entity_query, bool throw_if_exists, bool replace_if_exists,
                                            const Settings &settings) {
  if (entity_name.empty()) throw Exception(ErrorCodes::BAD_ARGUMENTS, "Workload entity name should not be empty.");
    ...
    if (old_entity) {
      ...
      if (other_entities.contains(entity_name))
        throw Exception(ErrorCodes::BAD_ARGUMENTS,
                        "It is not allowed to replace workload entity '{}' that is stored in read-only {} storage", entity_name,
                        next_storage->getName());
    }
    ...
    if (workload) {
      if (!workload->hasParent()) {
        if (!root_name.empty() && root_name != workload->getWorkloadName())
          throw Exception(ErrorCodes::BAD_ARGUMENTS, "The second root is not allowed. You should probably add 'PARENT {}' clause.",
                          root_name);
      }
      ...
      for (const auto &operation : resource->operations) {
        if (operation.mode == ResourceAccessMode::MasterThread) {
          if (!master_thread_resource.empty() && master_thread_resource != resource->getResourceName())
            throw Exception(ErrorCodes::BAD_ARGUMENTS, "The second resource for MASTER THREAD is not allowed. Current resource name: '{}'.",
                            master_thread_resource);
        }
        if (operation.mode == ResourceAccessMode::WorkerThread) {
          if (!worker_thread_resource.empty() && worker_thread_resource != resource->getResourceName())
            throw Exception(ErrorCodes::BAD_ARGUMENTS, "The second resource for WORKER THREAD is not allowed. Current resource name: '{}'.",
                            worker_thread_resource);
        }
        if (operation.mode == ResourceAccessMode::Query) {
          if (!query_resource.empty() && query_resource != resource->getResourceName())
            throw Exception(ErrorCodes::BAD_ARGUMENTS, "The second resource for QUERY is not allowed. Current resource name: '{}'.",
                            query_resource);
        }
      }
    }

    forEachReference(create_entity_query, [this, workload](const String &target, const String &source, ReferenceType type) {
      if (auto it = entities.find(target); it == entities.end())
        throw Exception(ErrorCodes::BAD_ARGUMENTS, "Workload entity '{}' references another workload entity '{}' that doesn't exist",
                        source, target);
      ...
      if (isIndirectlyReferenced(source, target)) throw Exception(ErrorCodes::BAD_ARGUMENTS, "Workload entity cycles are not allowed");
    });

    auto result =
        storeEntityImpl(current_context, entity_type, entity_name, create_entity_query, throw_if_exists, replace_if_exists, settings);

    if (result == OperationResult::Retry) continue;  // Entities were updated, we need to rerun all the validations
...
bool WorkloadEntityStorageBase::removeEntity(const ContextPtr &current_context, WorkloadEntityType entity_type, const String &entity_name,
  ...
  while (true) {
    ...
    if (auto reference_it = references.find(entity_name); reference_it != references.end()) {
      String names;
      for (const String &name : reference_it->second) names += " " + name;
      throw Exception(ErrorCodes::BAD_ARGUMENTS, "Workload entity '{}' cannot be dropped. It is referenced by:{}", entity_name, names);
    }

    if (other_entities.contains(entity_name))
      throw Exception(ErrorCodes::BAD_ARGUMENTS, "It is not allowed to remove workload entity '{}' that is stored in read-only {} storage",
                      entity_name, next_storage->getName());

    auto result = removeEntityImpl(current_context, entity_type, entity_name, throw_if_not_exists);
...
void WorkloadEntityStorageBase::unlockAndNotify(std::unique_lock<std::recursive_mutex> &lock, const std::vector<Event> &tx) {
  if (tx.empty()) return;
...
std::unique_lock<std::recursive_mutex> WorkloadEntityStorageBase::getLock() const { return std::unique_lock{mutex}; }
...
void WorkloadEntityStorageBase::setLocalEntities(const std::vector<std::pair<String, ASTPtr>> &raw_new_entities) {
  ...
  for (const auto &[entity_name, entity] : local_new_entities) merged_new_entities[entity_name] = entity;
  for (const auto &[entity_name, entity] : other_entities) merged_new_entities[entity_name] = entity;  // override local entity if exists
  ...
  for (const auto &[entity_name, entity] : entities) {
    if (auto it = merged_new_entities.find(entity_name); it != merged_new_entities.end()) {
      if (!entityEquals(entity, it->second)) {
        changes.emplace_back(entity_name, entity,
                             it->second);  // Update entities that are present in both `merged_new_entities` and `entities`
...
void WorkloadEntityStorageBase::applyEvent(std::unique_lock<std::recursive_mutex> &, const Event &event) {
  if (event.entity)  // CREATE || CREATE OR REPLACE
  {
    ...
    if (workload && !workload->hasParent()) root_name = workload->getWorkloadName();

    // Update resource names
    if (resource) {
      for (const auto &operation : resource->operations) {
        if (operation.mode == ResourceAccessMode::MasterThread) master_thread_resource = resource->getResourceName();
        if (operation.mode == ResourceAccessMode::WorkerThread) worker_thread_resource = resource->getResourceName();
        if (operation.mode == ResourceAccessMode::Query) query_resource = resource->getResourceName();
      }
    }

    // Remove references of a replaced entity (only for CREATE OR REPLACE)
    if (auto it = entities.find(event.name); it != entities.end()) removeReferences(it->second);
  ...
  } else  // DROP
  {
    ...
    if (event.name == root_name) root_name.clear();

    if (event.name == master_thread_resource) master_thread_resource.clear();

    if (event.name == worker_thread_resource) worker_thread_resource.clear();

    if (event.name == query_resource) query_resource.clear();
...
void WorkloadEntityStorageBase::insertReferences(const ASTPtr &entity) {
  if (!entity) return;
  forEachReference(entity, [this](const String &target, const String &source, ReferenceType) { references[target].insert(source); });
}

void WorkloadEntityStorageBase::removeReferences(const ASTPtr &entity) {
  if (!entity) return;
  forEachReference(entity, [this](const String &target, const String &source, ReferenceType) {
    references[target].erase(source);
    if (references[target].empty()) references.erase(target);
  });
}

std::vector<WorkloadEntityStorageBase::Event> WorkloadEntityStorageBase::orderEntities(
    const std::unordered_map<String, ASTPtr> &all_entities, std::optional<Event> change) {
  ...
  for (const auto &[entity_name, ast] : all_entities) {
    if (typeid_cast<ASTCreateWorkloadQuery *>(ast.get())) {
      if (change && change->name == entity_name) continue;  // Skip this workload if it is removed or updated
      workloads.emplace(entity_name, ast);
    } else if (typeid_cast<ASTCreateResourceQuery *>(ast.get())) {
      if (change && change->name == entity_name) continue;  // Skip this resource if it is removed or updated
...
std::vector<std::pair<String, ASTPtr>> WorkloadEntityStorageBase::parseEntitiesFromString(const String &data, LoggerPtr log) {
  ...
  while (pos < end) {
    queries.emplace_back(
        parseQueryAndMovePosition(parser, pos, end, "", true, 0, DBMS_DEFAULT_MAX_PARSER_DEPTH, DBMS_DEFAULT_MAX_PARSER_BACKTRACKS));
    while (isWhitespaceASCII(*pos) || *pos == ';') ++pos;
  }
