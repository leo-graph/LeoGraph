...
WorkloadEntityDiskStorage::WorkloadEntityDiskStorage(const ContextPtr& global_context_, const String& dir_path_,
    ...
    : WorkloadEntityStorageBase(global_context_, std::move(next_storage_)), dir_path{makeDirectoryPathCanonical(dir_path_)} {
...
ASTPtr WorkloadEntityDiskStorage::tryLoadEntity(WorkloadEntityType entity_type, const String& entity_name, const String& path,
  ...
  try {
    ...
    auto parse = [&](auto parser) {
      return parseQuery(parser, entity_create_query.data(), entity_create_query.data() + entity_create_query.size(), "", 0,
    ...
    switch (entity_type) {
      case WorkloadEntityType::Workload:
        return parse(ParserCreateWorkloadQuery());
      case WorkloadEntityType::Resource:
        return parse(ParserCreateResourceQuery());
  ...
  if (!std::filesystem::exists(dir_path)) {
    ...
    for (Poco::DirectoryIterator it(dir_path); it != dir_end; ++it) {
      ...
      if (file_name.starts_with(workload_prefix) && file_name.ends_with(sql_suffix)) {
        String name =
            unescapeForFileName(file_name.substr(workload_prefix.size(), file_name.size() - workload_prefix.size() - sql_suffix.size()));

        if (name.empty()) continue;

        ASTPtr ast = tryLoadEntity(WorkloadEntityType::Workload, name, dir_path + it.name(), /* check_file_exists= */ false);
        if (ast) entities_name_and_queries.emplace_back(name, ast);
      }

      if (file_name.starts_with(resource_prefix) && file_name.ends_with(sql_suffix)) {
        String name =
            unescapeForFileName(file_name.substr(resource_prefix.size(), file_name.size() - resource_prefix.size() - sql_suffix.size()));
...
WorkloadEntityStorageBase::OperationResult WorkloadEntityDiskStorage::storeEntityImpl(const ContextPtr& /*current_context*/,
                                                                                      WorkloadEntityType entity_type,
                                                                                      const String& entity_name, ASTPtr create_entity_query,
...
WorkloadEntityStorageBase::OperationResult WorkloadEntityDiskStorage::removeEntityImpl(const ContextPtr& /*current_context*/,
                                                                                       WorkloadEntityType entity_type,
