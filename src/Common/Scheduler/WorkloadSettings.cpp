...
bool WorkloadSettings::hasThrottler() const {
  switch (unit) {
    case CostUnit::IOByte:
      return max_bytes_per_second != 0;
    case CostUnit::CPUNanosecond:
      return max_cpus != 0;
    case CostUnit::QuerySlot:
      return max_queries_per_second != 0;
  }
}
...
Float64 WorkloadSettings::getThrottlerMaxSpeed() const {
  switch (unit) {
    ...
    case CostUnit::CPUNanosecond:
      return max_cpus * 1'000'000'000;  // Convert CPU count to CPU * nanoseconds / sec
    case CostUnit::QuerySlot:
...
Float64 WorkloadSettings::getThrottlerMaxBurst() const {
  switch (unit) {
    case CostUnit::IOByte:
      return max_burst_bytes;
    case CostUnit::CPUNanosecond:
      return max_burst_cpu_seconds * 1'000'000'000;  // Convert seconds to nanoseconds
    case CostUnit::QuerySlot:
      return max_burst_queries;
  }
}
...
bool WorkloadSettings::hasSemaphore() const {
  switch (unit) {
    case CostUnit::IOByte:
      return max_io_requests != unlimited || max_bytes_inflight != unlimited;
    case CostUnit::CPUNanosecond:
      return max_concurrent_threads != unlimited;
    case CostUnit::QuerySlot:
      return max_concurrent_queries != unlimited;
  }
}
...
Int64 WorkloadSettings::getQueueSize() const { return max_waiting_queries; }

void WorkloadSettings::initFromChanges(CostUnit unit_, const ASTCreateWorkloadQuery::SettingsChanges& changes, const String& resource_name,
                                       bool throw_on_unknown_setting) {
  // Set resource unit
  unit = unit_;
  ...
  struct {
    ...
    static Float64 getNotNegativeFloat64(const String& name, const Field& field) {
      {
        UInt64 val;
        if (field.tryGet(val)) return static_cast<Float64>(val);  // We dont mind slight loss of precision
      }

      {
        Int64 val;
        if (field.tryGet(val)) {
          if (val < 0) throw Exception(ErrorCodes::BAD_ARGUMENTS, "Unexpected negative Int64 value for workload setting '{}'", name);
    ...
    static Int64 getNotNegativeInt64(const String& name, const Field& field) {
      {
        ...
        if (field.tryGet(val)) {
          ...
          if (val > static_cast<UInt64>(std::numeric_limits<Int64>::max())) val = std::numeric_limits<Int64>::max();
  ...
  // Regular values are used for all resources, while specific values are from setting having 'FOR resource' clause and has priority over
  // regular
  auto get_value = []<typename T>(const std::optional<T>& specific_value, const std::optional<T>& regular_value, T default_value,
                                  T zero_value = T()) {
    if (specific_value) return *specific_value == T() ? zero_value : *specific_value;
    if (regular_value) return *regular_value == T() ? zero_value : *regular_value;
    return default_value;
  };

  // Validate that we could use values read in a scheduler node
  {
    SchedulerNodeInfo validating_node(get_value(specific.weight, regular.weight, weight),
  ...
  if (specific.max_cpus || regular.max_cpus || specific.max_cpu_share || regular.max_cpu_share) {
    ...
    if (exact_limit > 0) limit = exact_limit;
    if (share_limit > 0) {
      Float64 value = share_limit * getNumberOfCPUCoresToUse();
      if (value > 0 && (limit == 0 || value < limit)) limit = value;
    }
    max_cpus = limit;
  }
  ...
  // Zero setting value means unlimited number of threads.
  {
    ...
    if (exact_number > 0 && exact_number < limit) limit = exact_number;
    if (ratio_to_cores > 0) {
      Int64 value = static_cast<Int64>(ratio_to_cores * getNumberOfCPUCoresToUse());
      if (value > 0 && value < limit) limit = value;
    }
