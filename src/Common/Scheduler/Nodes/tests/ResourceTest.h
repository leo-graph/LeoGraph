...
struct ResourceTestBase {
  ResourceTestBase() {
    [[maybe_unused]] static bool typesRegistered = [] {
...
class ResourceTestClass : public ResourceTestBase {
  struct Request : public ResourceRequest {
    ...
    Request(ResourceTestClass *test_, ResourceCost cost_, const String &name_) : ResourceRequest(cost_), test(test_), name(name_) {}
 ...
 public:
  ~ResourceTestClass() {
    if (root_node) dequeue();  // Just to avoid any leaks of `Request` object
  }
  ...
  UnifiedSchedulerNodePtr createUnifiedNode(const String &basename, const UnifiedSchedulerNodePtr &parent,
                                            const WorkloadSettings &settings = {}) {
  ...
  void updateUnifiedNode(const UnifiedSchedulerNodePtr &node, const UnifiedSchedulerNodePtr &old_parent,
                         const UnifiedSchedulerNodePtr &new_parent, const WorkloadSettings &new_settings) {
    EXPECT_TRUE((old_parent && new_parent) || (!old_parent && !new_parent));  // changing root node is not supported
    bool detached = false;
    if (UnifiedSchedulerNode::updateRequiresDetach(old_parent ? old_parent->basename : "", new_parent ? new_parent->basename : "",
                                                   node->getSettings(), new_settings)) {
      if (old_parent) old_parent->detachUnifiedChild(node);
      detached = true;
    }

    node->updateSchedulingSettings(new_settings);

    if (detached && new_parent) new_parent->attachUnifiedChild(node);
  }
  ...
  void enqueue(const String &path, const std::vector<ResourceCost> &costs) {
    ...
    if (node) enqueueImpl(dynamic_cast<ISchedulerQueue *>(node), costs);
  }
  ...
  void enqueueImpl(ISchedulerQueue *queue, const std::vector<ResourceCost> &costs, const String &name = {}) {
    ...
    if (!queue) return;             // to make clang-analyzer-core.NonNullParamChecker happy
    for (ResourceCost cost : costs) queue->enqueueRequest(new Request(this, cost, name.empty() ? queue->basename : name));
...
struct ResourceTestManager : public ResourceTestBase {
  ...
  struct Guard : public ResourceGuard {
    ...
    Guard(ResourceTestManager &t_, ResourceLink link_, ResourceCost cost_)
        : ResourceGuard(ResourceGuard::Metrics::getIOWrite(), link_, cost_, Lock::Defer), t(t_), cost(cost_) {
      t.onEnqueue(link);
      waitExecute();
    }
  ...
  struct ResourceQueueHash {
    size_t operator()(const ResourceLink &link) const { return std::hash<ISchedulerQueue *>()(link.queue); }
  };

  std::mutex link_data_mutex;
  std::unordered_map<ResourceLink, TItem, ResourceQueueHash> link_data;

  explicit ResourceTestManager(size_t thread_count = 1) : manager(new TManager), busy_period(thread_count) {}

  enum DoNotInitManagerEnum { DoNotInitManager };

  explicit ResourceTestManager(size_t thread_count, DoNotInitManagerEnum) : busy_period(thread_count) {}

  ~ResourceTestManager() { wait(); }
  ...
  void wait() {
    while (true) {
      ...
      if (threads_to_join.empty()) break;
  ...
  // Use exactly two threads for each queue to avoid queue being deactivated (happens with 1 thread) and reordering (happens with >2
  // threads): while the first request is executing, the second request is in queue - holding it active. use onEnqueue() and onExecute()
  // functions for this purpose.
  void onEnqueue(ResourceLink link) { getLinkData(link).enqueued.fetch_add(1, std::memory_order_relaxed); }
