...
TEST(SchedulerThrottlerConstraint, PeekAndAvgLimits) {
  ...
  for (int i = 0; i < total_cost / req_cost; i++) t.enqueue("/avg/A", {req_cost});

  double consumed = 0;
  for (int seconds = 0; seconds < 100; seconds++) {
    t.process(start + std::chrono::seconds(seconds));
    double arrival_curve =
        std::min(100.0 + 50.0 * static_cast<double>(seconds), 5000.0 + 10.0 * static_cast<double>(seconds)) + static_cast<double>(req_cost);
