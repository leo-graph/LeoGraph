...
class FakeSchedulerNode : public ISchedulerNode {
 public:
  explicit FakeSchedulerNode(String &log_, EventQueue *event_queue_, const Poco::Util::AbstractConfiguration &config = emptyConfig(),
                             const String &config_prefix = {})
      : ISchedulerNode(event_queue_, config, config_prefix), log(log_) {}
  ...
  void attachChild(const SchedulerNodePtr &child) override { log += " +" + child->basename; }

  void removeChild(ISchedulerNode *child) override { log += " -" + child->basename; }

  ISchedulerNode *getChild(const String & /* child_name */) override { return nullptr; }

  void activateChild(ISchedulerNode *child) override { log += " A" + child->basename; }
...
struct QueueTest {
  ...
  QueueTest() : root_node(log, &event_queue) {}
  ...
  void process(EventQueue::TimePoint now, const String &expected_log, size_t limit = size_t(-1)) {
    ...
    for (; limit > 0; limit--) {
      if (!event_queue.tryProcess()) break;
    }
    EXPECT_EQ(log, expected_log);
    log.clear();
  }

  void activate(const SchedulerNodePtr &node) { event_queue.enqueueActivation(node.get()); }
  ...
  void cancel(EventId event_id) { event_queue.cancelPostponed(event_id); }
};
...
TEST(SchedulerEventQueue, ActivationCancelRace) {
  ...
  for (int i = 0; i < iterations; ++i) {
    ...
    struct RaceDetectorNode : public FakeSchedulerNode {
      ...
      RaceDetectorNode(String &log_, EventQueue *eq, std::atomic<bool> &r, std::atomic<bool> &a, std::atomic<bool> &ia)
    ...
    std::thread processor([&] {
      EventQueue::SchedulerThread scheduler_thread(&event_queue);
      while (!stop.load(std::memory_order_relaxed)) event_queue.tryProcess();
    ...
    for (int j = 0; j < 50; ++j) {
      ...
      if (in_activate_child.load(std::memory_order_acquire)) race_detected.store(true, std::memory_order_release);
    ...
    EXPECT_FALSE(race_detected.load()) << "activateChild() was still in progress after cancelActivation() returned";
  }
}
