...
#  include <iostream>
#  define DBG_PRINT(...)                                                                                        \
    std::cout << fmt::format("\033[01;3{}m[{}] {} {} {}\033[00m {}:{}\n", 1 + getThreadId() % 8, getThreadId(), \
                             reinterpret_cast<void *>(this), fmt::format(__VA_ARGS__), __PRETTY_FUNCTION__, __FILE__, __LINE__)
#else
...
class WorkloadEntityTestStorage : public WorkloadEntityStorageBase {
 public:
  WorkloadEntityTestStorage() : WorkloadEntityStorageBase(Context::getGlobalContextInstance()) {}

  std::string_view getName() const override { return "test"; }

  void loadEntities(const Poco::Util::AbstractConfiguration &config) override { WorkloadEntityStorageBase::loadEntities(config); }
  ...
  void executeQuery(const String &query) {
    ...
    auto parse = [&](IParser &parser) {
      ...
      return tryParseQuery(parser, end, query.data() + query.size(), error, false, "", false, 0, DBMS_DEFAULT_MAX_PARSER_DEPTH,
                           DBMS_DEFAULT_MAX_PARSER_BACKTRACKS, true);
    };
    ...
    if (ASTPtr create_workload = parse(create_workload_p)) {
      ...
      storeEntity(nullptr, WorkloadEntityType::Workload, workload_name, create_workload, throw_if_exists, replace_if_exists, {});
      ...
      storeEntity(nullptr, WorkloadEntityType::Resource, resource_name, create_resource, throw_if_exists, replace_if_exists, {});
      ...
      removeEntity(nullptr, WorkloadEntityType::Workload, parsed.workload_name, throw_if_not_exists);
    } else if (ASTPtr drop_resource = parse(drop_resource_p)) {
      auto &parsed = drop_resource->as<ASTDropResourceQuery &>();
      bool throw_if_not_exists = !parsed.if_exists;
      removeEntity(nullptr, WorkloadEntityType::Resource, parsed.resource_name, throw_if_not_exists);
    } else
      throw Exception(ErrorCodes::LOGICAL_ERROR, "Invalid query in WorkloadEntityTestStorage: {}", query);
  }

 private:
  WorkloadEntityStorageBase::OperationResult storeEntityImpl(const ContextPtr &current_context, WorkloadEntityType entity_type,
                                                             const String &entity_name, ASTPtr create_entity_query, bool throw_if_exists,
                                                             bool replace_if_exists, const Settings &settings) override {
    UNUSED(current_context, entity_type, entity_name, create_entity_query, throw_if_exists, replace_if_exists, settings);
    return OperationResult::Ok;
  }

  WorkloadEntityStorageBase::OperationResult removeEntityImpl(const ContextPtr &current_context, WorkloadEntityType entity_type,
                                                              const String &entity_name, bool throw_if_not_exists) override {
...
struct ResourceTest : ResourceTestManager<WorkloadResourceManager> {
  ...
  explicit ResourceTest(size_t thread_count = 1) : ResourceTestManager(thread_count, DoNotInitManager) {
    manager = std::make_shared<WorkloadResourceManager>(storage);
  }

  void query(const String &query_str) { storage.executeQuery(query_str); }

  template <class Func>
  ThreadFromGlobalPool *async(Func func) {
    std::scoped_lock lock{threads_mutex};
    threads.emplace_back([func2 = std::move(func)] mutable { func2(); });
    return &threads.back();
  }
  ...
  void async(ThreadPool &pool, Func func) {
    pool.scheduleOrThrowOnError([func2 = std::move(func)] mutable { func2(); });
  }
...
class ThreadMetrics {
 public:
  ...
  ThreadMetrics(ThreadMetrics &&other) noexcept
      : last_update_ns(other.last_update_ns), consumed(other.consumed.load(std::memory_order_relaxed)) {}
  ...
  UInt64 takeConsumed() { return consumed.exchange(0, std::memory_order_relaxed); }
...
class ThreadMetricsGroup : public boost::noncopyable {
 public:
  explicit ThreadMetricsGroup(size_t size = 0) : metrics(size) {}

  void resize(size_t size) { metrics.resize(size); }

  void start(size_t thread_num) { metrics[thread_num].start(); }

  void update(size_t thread_num) { metrics[thread_num].update(); }

  UInt64 takeConsumed() {
    UInt64 total_consumed = 0;
    for (auto &metric : metrics) total_consumed += metric.takeConsumed();
...
class ThreadMetricsTester {
 public:
  static double now() { return static_cast<double>(clock_gettime_ns()); }
  ...
  ThreadMetricsTester &expectShare(ThreadMetricsGroup *group, double share) {
    assertions.emplace_back(Assertion{.group = group, .share = share});
    return *this;
  }
  ...
  UInt64 burstUpperBound(double burst_sec, size_t max_concurrent_threads, size_t max_concurrent_queries, size_t quantum_ns) const {
    ...
    return static_cast<UInt64>(burst_sec * 1'000'000'000 + static_cast<double>(max_concurrent_threads + max_concurrent_queries) *
                                                               (static_cast<double>(quantum_ns) * 1.1));
  }

  /// Waits for share of group to stabilize on given value
  ThreadMetricsTester &expectThrottling(double speed, double burst_sec, size_t max_concurrent_threads, size_t max_concurrent_queries,
                                        size_t quantum_ns) {
    max_speed = speed;
  ...
  /// Waits for share of group to stabilize on given value
  /// WARNING: do not add the same group to both expectShare and expectThrottling
  ThreadMetricsTester &expectThrottling(ThreadMetricsGroup *group, double speed, double burst_sec, size_t max_concurrent_threads,
                                        size_t max_concurrent_queries, size_t quantum_ns, bool exclude_from_total = false) {
  ...
  void check() {
    ...
    double start_ns = now();
    for (auto &assertion : assertions) assertion.init(start_ns);
    ...
    while (!passed && now() < start_ns + runtime_ns) {
      ...
      for (auto &assertion : assertions) total_consumed_delta += assertion.process(now_ns);
      ...
      if (consumed_cpus < assume_cpus * 0.9)  // Allow some margin of error
      {
        ...
        for (auto &assertion : assertions) {
          if (assertion.share > 0.0) {
            ...
            if (std::abs(actual_share - assertion.share) / assertion.share > 0.1) {
              passed = false;
              DBG_PRINT("Assertion failed: expected share {:.2f}, actual share {:.2f}, total cpus {:.2f}", assertion.share, actual_share,
                        consumed_cpus);
            } else {
              DBG_PRINT("Assertion passed: expected share {:.2f}, actual share {:.2f}, total cpus {:.2f}", assertion.share, actual_share,
                        consumed_cpus);
            }
          }
          if (assertion.max_speed > 0.0) {
            ...
            if (static_cast<double>(assertion.consumed_integral_ns) > allowed_consumption_ns) {
              DBG_PRINT("Assertion failed: expected less than {:.2f} ms, actual {:.2f} ms", allowed_consumption_ns / 1'000'000.0,
                        static_cast<double>(assertion.consumed_integral_ns) / 1'000'000.0);
              GTEST_FAIL();
            } else {
              DBG_PRINT("Assertion passed: expected less than {:.2f} ms, actual {:.2f} ms", allowed_consumption_ns / 1'000'000.0,
                        static_cast<double>(assertion.consumed_integral_ns) / 1'000'000.0);
            }
          }
        }
        ...
        if (max_speed > 0.0) {
          ...
          if (static_cast<double>(total_consumed_integral_ns) > allowed_consumption_ns) {
            DBG_PRINT("Assertion failed: expected less than {:.2f} ms, actual {:.2f} ms", allowed_consumption_ns / 1'000'000.0,
                      static_cast<double>(total_consumed_integral_ns) / 1'000'000.0);
            GTEST_FAIL();
          } else {
            DBG_PRINT("Assertion passed: expected less than {:.2f} ms, actual {:.2f} ms", allowed_consumption_ns / 1'000'000.0,
                      static_cast<double>(total_consumed_integral_ns) / 1'000'000.0);
...
struct TestQuery {
  ...
  explicit TestQuery(ResourceTest &t_) : t(t_) {}

  ~TestQuery() {
    finish();
    if (master_thread && master_thread->joinable()) master_thread->join();
  }

  AcquiredSlotPtr trySpawn() {
    {
      std::unique_lock lock{mutex};
      if (query_is_finished) return {};
    }
  ...
  bool controlConcurrency(ISlotLease *cpu_lease) {
    ...
    while (auto slot = trySpawn()) {
      t.async(*pool, [this, my_slot = std::move(slot)] mutable { threadFunc(std::move(my_slot)); });
    }
    ...
    while (true) {
      {
        ...
        if (slots && slots->isRequesting()) return;
      }
  ...
  size_t onThreadStart(ISlotLease *cpu_lease) {
    ...
    if (cpu_lease) {
      ...
      size_t thread_num = 0;
      while (thread_num < max_threads && threads[thread_num].is_active) thread_num++;
  ...
  bool doWork(size_t thread_num) {
    {
      ...
      std::unique_lock lock{mutex};
      if (work_left > 0) work_left--;
    }

    auto predicate = [=, this] {
      return query_is_finished || work_left == 0 ||
             (thread_num <
                  threads_finished  // When finish not the last thread make sure query will have at least 1 active thread afterwards
  ...
  void threadFunc(AcquiredSlotPtr self_slot) {
    ...
    if (cpu_lease) cpu_lease->startConsumption();
    metrics.start(thread_num);

    DB::setThreadName(DB::ThreadName::TEST_SCHEDULER);
    while (true) {
      if (!controlConcurrency(cpu_lease)) break;
      if (!doWork(thread_num)) break;
    }
  }
  ...
  void start(AllocationType type, String workload, SlotCount max_threads_, UInt64 runtime_us = UInt64(-1)) {
    ...
    if (runtime_us != UInt64(-1)) work_left = runtime_us / us_per_work;
    threads.resize(max_threads);
    metrics.resize(max_threads);
    pool = std::make_unique<ThreadPool>(CurrentMetrics::QueryPipelineExecutorThreads, CurrentMetrics::QueryPipelineExecutorThreadsActive,
                                        CurrentMetrics::QueryPipelineExecutorThreadsScheduled, max_threads);
...
TEST(SchedulerWorkloadResourceManager, CPUSchedulingRoundRobin) {
  ...
  for (int iteration = 0; iteration < 4; ++iteration) {
    ...
    for (int query = 0; query < 2; query++) queries.push_back(std::make_shared<TestQuery>(t));

    auto ensure = [&](size_t q0_threads, size_t q1_threads) {
      if (q0_threads > 0) queries[0]->waitStartedThreads(q0_threads);
      if (q1_threads > 0) queries[1]->waitStartedThreads(q1_threads);
    };

    auto finish = [&](size_t q0_threads, size_t q1_threads) {
      if (q0_threads > 0) queries[0]->finishThread(q0_threads);
      if (q1_threads > 0) queries[1]->finishThread(q1_threads);
    };
...
auto getAcquired() { return CurrentMetrics::get(CurrentMetrics::ConcurrencyControlAcquired); }

auto getPreempted() { return CurrentMetrics::get(CurrentMetrics::ConcurrencyControlPreempted); }

struct EventCounter {
  ProfileEvents::Event event;
  size_t initial_value;

  explicit EventCounter(ProfileEvents::Event event_) : event(event_) { initial_value = getValue(); }

  size_t count() const { return getValue() - initial_value; }

 private:
  size_t getValue() const { return ProfileEvents::global_counters[event].load(std::memory_order_relaxed); }
};
...
TEST(SchedulerWorkloadResourceManager, PreemptiveCPUSchedulingFairness) {
  ...
  queries[0]->start(TestQuery::AllocateLeaseNoDownscale, "A", 8);
  queries[1]->start(TestQuery::AllocateLeaseNoDownscale, "B", 8);
  ThreadMetricsTester().expectShare(&queries[0]->metrics, 0.5).expectShare(&queries[1]->metrics, 0.5).check();
  DBG_PRINT("--- Stop ---");
...
TEST(SchedulerWorkloadResourceManager, PreemptiveCPUSchedulingFairnessIsQueryNumberIndependent) {
  ...
  std::vector<TestQueryPtr> queries;
  for (int query = 0; query < 6; query++) queries.push_back(std::make_shared<TestQuery>(t));
  queries[0]->start(TestQuery::AllocateLease, "A", 8);
...
TEST(SchedulerWorkloadResourceManager, PreemptiveCPUSchedulingWeights) {
  ...
  queries[0]->start(TestQuery::AllocateLeaseNoDownscale, "A", 8);
  queries[1]->start(TestQuery::AllocateLeaseNoDownscale, "B", 8);
  ThreadMetricsTester().expectShare(&queries[0]->metrics, 0.25).expectShare(&queries[1]->metrics, 0.75).check();
  DBG_PRINT("--- Stop ---");
...
TEST(SchedulerWorkloadResourceManager, PreemptiveCPUSchedulingMaxMinFairness) {
  ...
  DBG_PRINT("--- Start ---");
  std::vector<TestQueryPtr> queries;
  for (int query = 0; query < 4; query++) queries.push_back(std::make_shared<TestQuery>(t));
...
TEST(SchedulerWorkloadResourceManager, PreemptiveCPUSchedulingThrottlingAndFairness) {
  ...
  for (int iteration = 0; iteration < 3; ++iteration) {
    DBG_PRINT("--- Start #{} ---", iteration);
    std::vector<TestQueryPtr> queries;
    for (int query = 0; query < 3; query++) queries.push_back(std::make_shared<TestQuery>(t));
