...
struct ResourceTest : public ResourceTestBase {
  ...
  ResourceTest() { scheduler.start(ThreadName::TEST_SCHEDULER); }

  ~ResourceTest() { scheduler.stop(true); }
...
struct ResourceHolder {
  ...
  explicit ResourceHolder(ResourceTest &t_) : t(t_) {}

  ~ResourceHolder() { unregisterResource(); }
...
struct MyRequest : public ResourceRequest {
  ...
  explicit MyRequest(ResourceCost cost_, std::function<void()> on_execute_) : ResourceRequest(cost_), on_execute(on_execute_) {}

  void execute() override {
    if (on_execute) on_execute();
  }

  void failed(const std::exception_ptr &) override { FAIL(); }
};
...
TEST(SchedulerRoot, Cancel) {
  ...
  std::thread consumer2([&] {
    MyRequest request(1, [&] { FAIL() << "This request must be canceled, but instead executes"; });
    sync.arrive_and_wait();  // (A) wait for request of consumer1 to be inside execute, so that constraint is in violated state and our
                             // request will not be executed immediately
