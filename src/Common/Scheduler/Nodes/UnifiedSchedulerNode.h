...
class UnifiedSchedulerNode final : public ISchedulerNode {
 private:
  ...
  static void reparent(const SchedulerNodePtr &node, const SchedulerNodePtr &new_parent) { reparent(node, new_parent.get()); }

  /// Helper function for managing a parent of a node
  static void reparent(const SchedulerNodePtr &node, ISchedulerNode *new_parent) {
    chassert(node);
    chassert(new_parent);
    if (new_parent == node->parent) return;
  ...
  struct FairnessBranch {
    ...
    [[nodiscard]] SchedulerNodePtr attachUnifiedChild(EventQueue *event_queue_, const UnifiedSchedulerNodePtr &child) {
      if (auto [it, inserted] = children.emplace(child->basename, child); !inserted)
        throw Exception(ErrorCodes::INVALID_SCHEDULER_NODE, "Can't add another child with the same path: {}", it->second->getPath());
      ...
      if (children.size() == 2) {
        ...
        for (auto &[_, node] : children) reparent(node, root);
    ...
    [[nodiscard]] SchedulerNodePtr detachUnifiedChild(EventQueue *, const UnifiedSchedulerNodePtr &child) {
      ...
      if (it == children.end()) return {};  // unknown child
  ...
  struct ChildrenBranch {
    ...
    [[nodiscard]] SchedulerNodePtr attachUnifiedChild(EventQueue *event_queue_, const UnifiedSchedulerNodePtr &child) {
      ...
      if (!new_branch) {
        ...
        if (branches.size() == 2) {
          ...
          root->basename = "prio";
          for (auto &[_, branch] : branches) reparent(branch.getRoot(), root);
          return root;  // New root has been created
    ...
    [[nodiscard]] SchedulerNodePtr detachUnifiedChild(EventQueue *event_queue_, const UnifiedSchedulerNodePtr &child) {
      ...
      if (it == branches.end()) return {};  // unknown child
  ...
  struct QueueOrChildrenBranch {
    ...
    /// Returns root node if it has been changed to a different node, otherwise returns null.
    [[nodiscard]] SchedulerNodePtr detachUnifiedChild(EventQueue *event_queue_, const UnifiedSchedulerNodePtr &child) {
      if (queue) return {};  // No-op, it already has no children
  ...
  struct ConstraintsBranch {
    ...
    [[nodiscard]] SchedulerNodePtr initialize(EventQueue *event_queue_, const WorkloadSettings &settings_) {
      ...
      if (settings.hasSemaphore()) {
        semaphore = std::make_shared<SemaphoreConstraint>(event_queue_, SchedulerNodeInfo{}, settings.getSemaphoreMaxRequests(),
                                                          settings.getSemaphoreMaxCost());
        semaphore->basename = "semaphore";
        reparent(node, semaphore);
        node = semaphore;
      }
      if (settings.hasThrottler()) {
        throttler = std::make_shared<ThrottlerConstraint>(event_queue_, SchedulerNodeInfo{}, settings.getThrottlerMaxSpeed(),
                                                          settings.getThrottlerMaxBurst());
    ...
    [[nodiscard]] SchedulerNodePtr updateSchedulingSettings(EventQueue *event_queue_, const WorkloadSettings &new_settings) {
      ...
      {
        semaphore = std::make_shared<SemaphoreConstraint>(event_queue_, SchedulerNodeInfo{}, new_settings.getSemaphoreMaxRequests(),
                                                          new_settings.getSemaphoreMaxCost());
        semaphore->basename = "semaphore";
        reparent(node, semaphore);
        node = semaphore;
      ...
      {
        static_cast<SemaphoreConstraint &>(*semaphore)
            .updateConstraints(semaphore, new_settings.getSemaphoreMaxRequests(), new_settings.getSemaphoreMaxCost());
        node = semaphore;
      }

      if (!settings.hasThrottler() && new_settings.hasThrottler())  // Add throttler
      {
        throttler = std::make_shared<ThrottlerConstraint>(event_queue_, SchedulerNodeInfo{}, new_settings.getThrottlerMaxSpeed(),
                                                          new_settings.getThrottlerMaxBurst());
        throttler->basename = "throttler";
        reparent(node, throttler);
        node = throttler;
      ...
      {
        static_cast<ThrottlerConstraint &>(*throttler)
            .updateConstraints(new_settings.getThrottlerMaxSpeed(), new_settings.getThrottlerMaxBurst());
        node = throttler;
      }
 ...
 public:
  ...
  ~UnifiedSchedulerNode() override {
    ...
    if (immediate_child) removeChild(immediate_child.get());
  }
  ...
  void attachUnifiedChild(const UnifiedSchedulerNodePtr &child) {
    if (auto new_child = impl.attachUnifiedChild(event_queue, child)) reparent(new_child, this);
  }
  ...
  void detachUnifiedChild(const UnifiedSchedulerNodePtr &child) {
    if (auto new_child = impl.detachUnifiedChild(event_queue, child)) reparent(new_child, this);
  }

  static bool updateRequiresDetach(const String &old_parent, const String &new_parent, const WorkloadSettings &old_settings,
  ...
  void updateSchedulingSettings(const WorkloadSettings &new_settings) {
    ...
    if (auto new_child = impl.updateSchedulingSettings(event_queue, new_settings)) reparent(new_child, this);
  }

  const WorkloadSettings &getSettings() const { return impl.settings; }

  /// Returns the queue to be used for resource requests or `nullptr` if it has unified children
  std::shared_ptr<ISchedulerQueue> getQueue() const { return static_pointer_cast<ISchedulerQueue>(impl.branch.queue); }
  ...
  void addRawPointerNodes(std::vector<SchedulerNodePtr> &nodes) {
    ...
    if (impl.semaphore) nodes.push_back(impl.semaphore);
    if (impl.branch.queue) nodes.push_back(impl.branch.queue);
    for (auto &[_0, branch] : impl.branch.branch.branches) {
      for (auto &[_1, child] : branch.children) child->addRawPointerNodes(nodes);
    }
  }

  bool hasUnifiedChildren() const { return impl.branch.queue == nullptr; }
  ...
  void forEachSchedulerNode(std::function<void(ISchedulerNode *)> visitor) {
    visitor(this);
    if (impl.throttler) visitor(impl.throttler.get());
    if (impl.semaphore) visitor(impl.semaphore.get());
    if (impl.branch.queue) visitor(impl.branch.queue.get());
 ...
 protected:  // Hide all the ISchedulerNode interface methods as an implementation details
  ...
  void attachChild(const SchedulerNodePtr &child_) override {
    ...
    if (immediate_child->isActive()) activateChild(immediate_child.get());
  }
