...
String getEntityName(const ASTPtr &ast) {
  if (auto *create = typeid_cast<ASTCreateWorkloadQuery *>(ast.get())) return create->getWorkloadName();
  if (auto *create = typeid_cast<ASTCreateResourceQuery *>(ast.get())) return create->getResourceName();
...
CostUnit getResourceUnit(const ASTPtr &ast) {
  ...
  if (auto *create = typeid_cast<ASTCreateResourceQuery *>(ast.get())) return create->unit;
...
WorkloadResourceManager::Resource::Resource(const ASTPtr &resource_entity_)
    : resource_entity(resource_entity_), resource_name(getEntityName(resource_entity)), unit(getResourceUnit(resource_entity_)) {
  scheduler.start(ThreadName::WORKLOAD_RESOURCE_MANAGER);
}

WorkloadResourceManager::Resource::~Resource() { scheduler.stop(); }

void WorkloadResourceManager::Resource::createNode(const NodeInfo &info) {
  if (info.name.empty()) throw Exception(ErrorCodes::LOGICAL_ERROR, "Workload must have a name in resource '{}'", resource_name);

  if (info.name == info.parent)
    throw Exception(ErrorCodes::LOGICAL_ERROR, "Self-referencing workload '{}' is not allowed in resource '{}'", info.name, resource_name);

  if (node_for_workload.contains(info.name))
    throw Exception(ErrorCodes::LOGICAL_ERROR, "Node for creating workload '{}' already exist in resource '{}'", info.name, resource_name);

  if (!info.parent.empty() && !node_for_workload.contains(info.parent))
    throw Exception(ErrorCodes::LOGICAL_ERROR, "Parent node '{}' for creating workload '{}' does not exist in resource '{}'", info.parent,
...
void WorkloadResourceManager::Resource::deleteNode(const NodeInfo &info) {
  if (!node_for_workload.contains(info.name))
    throw Exception(ErrorCodes::LOGICAL_ERROR, "Node for removing workload '{}' does not exist in resource '{}'", info.name, resource_name);

  if (!info.parent.empty() && !node_for_workload.contains(info.parent))
    throw Exception(ErrorCodes::LOGICAL_ERROR, "Parent node '{}' for removing workload '{}' does not exist in resource '{}'", info.parent,
                    info.name, resource_name);

  auto node = node_for_workload[info.name];

  if (node->hasUnifiedChildren())
    throw Exception(ErrorCodes::LOGICAL_ERROR, "Removing workload '{}' with children in resource '{}'", info.name, resource_name);
...
void WorkloadResourceManager::Resource::updateNode(const NodeInfo &old_info, const NodeInfo &new_info) {
  if (old_info.name != new_info.name)
    throw Exception(ErrorCodes::LOGICAL_ERROR, "Updating a name of workload '{}' to '{}' is not allowed in resource '{}'", old_info.name,
                    new_info.name, resource_name);

  if (old_info.parent != new_info.parent && (old_info.parent.empty() || new_info.parent.empty()))
    throw Exception(ErrorCodes::LOGICAL_ERROR, "Workload '{}' invalid update of parent from '{}' to '{}' in resource '{}'", old_info.name,
                    old_info.parent, new_info.parent, resource_name);

  if (!node_for_workload.contains(old_info.name))
    throw Exception(ErrorCodes::LOGICAL_ERROR, "Node for updating workload '{}' does not exist in resource '{}'", old_info.name,
                    resource_name);
  ...
  executeInSchedulerThread([&, this] {
    ...
    if (UnifiedSchedulerNode::updateRequiresDetach(old_info.parent, new_info.parent, old_info.settings, new_info.settings)) {
      if (!old_info.parent.empty()) node_for_workload[old_info.parent]->detachUnifiedChild(node);
      detached = true;
    }
    ...
    if (detached) {
      if (!new_info.parent.empty()) node_for_workload[new_info.parent]->attachUnifiedChild(node);
    }
    updateCurrentVersion();
  });
}
...
void WorkloadResourceManager::Resource::updateCurrentVersion() {
  ...
  if (root_node) root_node->addRawPointerNodes(current_version->nodes);
...
WorkloadResourceManager::Workload::Workload(WorkloadResourceManager *resource_manager_, const ASTPtr &workload_entity_)
    : resource_manager(resource_manager_), workload_entity(workload_entity_) {
  try {
...
void WorkloadResourceManager::Workload::updateWorkload(const ASTPtr &new_entity) {
  try {
    for (auto &[resource_name, resource] : resource_manager->resources)
      resource->updateNode(NodeInfo(resource->getUnit(), workload_entity, resource_name),
                           NodeInfo(resource->getUnit(), new_entity, resource_name));
...
WorkloadResourceManager::WorkloadResourceManager(IWorkloadEntityStorage &storage_)
    : storage(storage_), log{getLogger("WorkloadResourceManager")} {
  subscription = storage.getAllEntitiesAndSubscribe([this](const std::vector<IWorkloadEntityStorage::Event> &events) {
...
WorkloadResourceManager::Classifier::Classifier(const ClassifierSettings &settings_) : settings(settings_) {}
...
void WorkloadResourceManager::Classifier::attach(const ResourcePtr &resource, const VersionPtr &version, UnifiedSchedulerNode *node) {
  ...
  if (node) {
    ...
    if (queue) link = ResourceLink{.queue = queue.get()};
...
ClassifierPtr WorkloadResourceManager::acquire(const String &workload_name, const ClassifierSettings &settings) {
  ...
  std::vector<std::future<void>> futures;
  {
    ...
    for (auto &[resource_name, resource] : resources) futures.emplace_back(resource->attachClassifier(*classifier, workload_name));
  }
...
void WorkloadResourceManager::Resource::forEachResourceNode(IResourceManager::VisitorFunc &visitor) {
  executeInSchedulerThread([&, this] {
    for (auto &[path, node] : node_for_workload) {
      node->forEachSchedulerNode(
          [&](ISchedulerNode *scheduler_node) { visitor(resource_name, scheduler_node->getPath(), scheduler_node); });
    }
  });
}
...
void WorkloadResourceManager::forEachNode(IResourceManager::VisitorFunc visitor) {
  ...
  for (auto &[resource_name, resource] : resources_copy) resource->forEachResourceNode(visitor);
}

void WorkloadResourceManager::topologicallySortedWorkloadsImpl(Workload *workload, std::unordered_set<Workload *> &visited,
                                                               std::vector<Workload *> &sorted_workloads) {
  if (visited.contains(workload)) return;
...
std::vector<WorkloadResourceManager::Workload *> WorkloadResourceManager::topologicallySortedWorkloads() {
  ...
  for (auto &[workload_name, workload] : workloads) topologicallySortedWorkloadsImpl(workload.get(), visited, sorted_workloads);
