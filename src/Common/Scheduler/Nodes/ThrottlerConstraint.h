...
class ThrottlerConstraint final : public ISchedulerConstraint {
 public:
  ...
  explicit ThrottlerConstraint(EventQueue *event_queue_, const Poco::Util::AbstractConfiguration &config = emptyConfig(),
  ...
  ~ThrottlerConstraint() override {
    ...
    if (child) removeChild(child.get());
  }
  ...
  bool equals(ISchedulerNode *other) override {
    if (!ISchedulerNode::equals(other)) return false;
    if (auto *o = dynamic_cast<ThrottlerConstraint *>(other)) return max_speed == o->max_speed && max_burst == o->max_burst;
    return false;
  }
  ...
  void attachChild(const std::shared_ptr<ISchedulerNode> &child_) override {
    ...
    if (child->isActive()) activateChild(child.get());
  }
  ...
  ISchedulerNode *getChild(const String &child_name) override {
    if (child->basename == child_name) return child.get();
    return nullptr;
  }
  ...
  void activateChild(ISchedulerNode *child_) override {
    if (child_ == child.get())
      if (!std::exchange(child_active, true) && satisfied() && parent) parent->activateChild(this);
  }
  ...
  void updateConstraints(double new_max_speed, double new_max_burst) {
    ...
    if (!was_active && active() && parent) parent->activateChild(this);
  }
  ...
  bool isSatisfied() override { return satisfied(); }
  ...
  std::chrono::nanoseconds getThrottlingDuration() const { return throttling_duration; }

  std::pair<double, double> getParams() const { return {max_speed, max_burst}; }
 ...
 private:
  ...
  void updateBucket(ResourceCost use = 0, bool do_not_postpone = false) {
    ...
    if (max_speed > 0.0) {
      ...
      tokens -= static_cast<double>(
          use);  // This is done outside min() to avoid passing large requests w/o token consumption after long idle period
      ...
      if (!do_not_postpone && tokens < 0.0) {
        ...
        if (postponed == EventQueue::not_postponed) {
          postponed = event_queue->postpone(std::chrono::time_point_cast<EventQueue::Duration>(now + delay_ns), [this] { onPostponed(); });
  ...
  bool satisfied() const { return tokens >= 0.0; }

  bool active() const { return satisfied() && child_active; }
