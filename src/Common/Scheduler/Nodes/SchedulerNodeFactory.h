...
class SchedulerNodeFactory : private boost::noncopyable {
 public:
  ...
  SchedulerNodePtr get(const String& name, EventQueue* event_queue, const Poco::Util::AbstractConfiguration& config,
                       const String& config_prefix) {
    std::lock_guard lock{mutex};
    if (auto iter = methods.find(name); iter != methods.end()) return iter->second(event_queue, config, config_prefix);
 ...
 private:
  std::mutex mutex;
  using Method = std::function<SchedulerNodePtr(EventQueue* event_queue, const Poco::Util::AbstractConfiguration& config,
                                                const String& config_prefix)>;
