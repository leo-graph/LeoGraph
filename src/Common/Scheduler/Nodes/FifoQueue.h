...
class FifoQueue final : public ISchedulerQueue {
 public:
  ...
  FifoQueue(EventQueue *event_queue_, const SchedulerNodeInfo &info_) : ISchedulerQueue(event_queue_, info_) {}

  ~FifoQueue() override { purgeQueue(); }
  ...
  bool equals(ISchedulerNode *other) override {
    ...
    if (auto *_ = dynamic_cast<FifoQueue *>(other)) return true;
    return false;
  }

  void enqueueRequest(ResourceRequest *request) override {
    std::lock_guard lock(mutex);
    if (is_not_usable) throw Exception(ErrorCodes::INVALID_SCHEDULER_NODE, "Scheduler queue is about to be destructed");

    if (requests.size() >= static_cast<size_t>(info.queue_size))
      throw Exception(ErrorCodes::SERVER_OVERLOADED, "Workload limit `max_waiting_queries` has been reached: {} of {}", requests.size(),
                      info.queue_size);
    queue_cost += request->cost;
    bool was_empty = requests.empty();
    requests.push_back(*request);
    if (was_empty) scheduleActivation();
  }

  std::pair<ResourceRequest *, bool> dequeueRequest() override {
    std::lock_guard lock(mutex);
    if (requests.empty()) return {nullptr, false};
  ...
  bool cancelRequest(ResourceRequest *request) override {
    std::lock_guard lock(mutex);
    if (is_not_usable) return false;  // Any request should already be failed or executed
  ...
  void purgeQueue() override {
    ...
    for (ResourceRequest *request : requests_to_fail) request->failed(exception);
  }
