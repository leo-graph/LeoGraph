...
CustomResourceManager::State::State(EventQueue *event_queue, const Poco::Util::AbstractConfiguration &config) : classifiers(config) {
...
CustomResourceManager::State::Resource::Resource(const String &name, EventQueue *event_queue,
  ...
  for (const auto &key : keys) {
    if (!startsWith(key, "node")) continue;
    ...
    if (path[0] != '/') throw Exception(ErrorCodes::INVALID_SCHEDULER_NODE, "Path must start with '/' for resource '{}'", name);
  ...
  if (!has_root) throw Exception(ErrorCodes::INVALID_SCHEDULER_NODE, "undefined root node path '/' for resource '{}'", name);
}
...
CustomResourceManager::State::Resource::~Resource() {
  ...
  if (attached_to != nullptr) {
    ...
    attached_to->event_queue->enqueue([my_scheduler = attached_to, root] { my_scheduler->removeChild(root); });
  }
}

CustomResourceManager::State::Node::Node(const String &name, EventQueue *event_queue, const Poco::Util::AbstractConfiguration &config,
...
bool CustomResourceManager::State::Resource::equals(const CustomResourceManager::State::Resource &o) const {
  ...
  for (const auto &[path, o_node] : o.nodes) {
    ...
    if (!iter->second.equals(o_node)) return false;
  }
...
CustomResourceManager::Classifier::Classifier(const ClassifierSettings &settings_, const CustomResourceManager::StatePtr &state_,
    ...
    if (auto resource_iter = state->resources.find(resource_name); resource_iter != state->resources.end()) {
      ...
      if (auto node_iter = resource->nodes.find(path); node_iter != resource->nodes.end()) {
        if (auto *queue = dynamic_cast<ISchedulerQueue *>(node_iter->second.ptr.get()))
          ...
          throw Exception(ErrorCodes::RESOURCE_NOT_FOUND, "Unable to access non-queue node at path '{}' for resource '{}'", path,
                          resource_name);
      } else
...
bool CustomResourceManager::Classifier::has(const String &resource_name) { return resources.contains(resource_name); }

ResourceLink CustomResourceManager::Classifier::get(const String &resource_name) {
  if (auto iter = resources.find(resource_name); iter != resources.end()) return iter->second;
...
CustomResourceManager::CustomResourceManager() : state(new State()) { scheduler.start(ThreadName::CUSTOM_RESOURCE_MANAGER); }
...
void CustomResourceManager::updateConfiguration(const Poco::Util::AbstractConfiguration &config) {
  ...
  for (auto &resource : resources_to_attach) {
    ...
    scheduler.event_queue->enqueue([this, root] { scheduler.attachChild(root); });
  }
...
void CustomResourceManager::forEachNode(IResourceManager::VisitorFunc visitor) {
  ...
  scheduler.event_queue->enqueue([state_ref, visitor, &promise] {
    for (auto &[name, resource] : state_ref->resources)
      for (auto &[path, node] : resource->nodes) visitor(name, path, node.ptr.get());
    promise.set_value();
  });
