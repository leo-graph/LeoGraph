...
class FairPolicy final : public ISchedulerNode {
  ...
  struct Item {
    ...
    bool operator<(const Item &rhs) const noexcept { return vruntime > rhs.vruntime; }
  };

 public:
  explicit FairPolicy(EventQueue *event_queue_, const Poco::Util::AbstractConfiguration &config = emptyConfig(),
                      const String &config_prefix = {})
      : ISchedulerNode(event_queue_, config, config_prefix) {}

  FairPolicy(EventQueue *event_queue_, const SchedulerNodeInfo &info_) : ISchedulerNode(event_queue_, info_) {}

  ~FairPolicy() override {
    // We need to clear `parent` in all children to avoid dangling references
    while (!children.empty()) removeChild(children.begin()->second.get());
  }
  ...
  bool equals(ISchedulerNode *other) override {
    ...
    if (auto *_ = dynamic_cast<FairPolicy *>(other)) return true;
    return false;
  }
  ...
  void attachChild(const SchedulerNodePtr &child) override {
    ...
    if (child->isActive()) activateChildImpl(items.size() - 1);
  }
  ...
  ISchedulerNode *getChild(const String &child_name) override {
    if (auto iter = children.find(child_name); iter != children.end()) return iter->second.get();
    return nullptr;
  }
  ...
  std::pair<ResourceRequest *, bool> dequeueRequest() override {
    ...
    while (true) {
      if (heap_size == 0) return {nullptr, false};
      ...
      if (heap_size == 0) {
        ...
        if (last_reset_ns + 1000000000 < ns) {
          last_reset_ns = ns;
          for (Item &item : items) item.vruntime = 0;
          max_vruntime = 0;
        }
  ...
  bool isActive() override { return heap_size > 0; }

  size_t activeChildren() override { return heap_size; }
  ...
  // For introspection
  double getSystemVRuntime() const { return system_vruntime; }

  std::optional<double> getChildVRuntime(ISchedulerNode *child) const {
    for (const auto &item : items) {
      if (child == item.child) return item.vruntime;
    }
    return std::nullopt;
  }
 ...
 private:
  void activateChildImpl(size_t inactive_idx) {
    ...
    if (activate_parent && parent) parent->activateChild(this);
  }
