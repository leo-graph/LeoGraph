...
class PriorityPolicy final : public ISchedulerNode {
 ...
 public:
  explicit PriorityPolicy(EventQueue *event_queue_, const Poco::Util::AbstractConfiguration &config = emptyConfig(),
                          const String &config_prefix = {})
      : ISchedulerNode(event_queue_, config, config_prefix) {}

  explicit PriorityPolicy(EventQueue *event_queue_, const SchedulerNodeInfo &node_info) : ISchedulerNode(event_queue_, node_info) {}
  ...
  bool equals(ISchedulerNode *other) override {
    ...
    if (auto *_ = dynamic_cast<PriorityPolicy *>(other)) return true;
    return false;
  }
  ...
  std::pair<ResourceRequest *, bool> dequeueRequest() override {
    ...
    while (true) {
      if (items.empty()) return {nullptr, false};
  ...
  bool isActive() override { return !items.empty(); }

  size_t activeChildren() override { return items.size(); }
