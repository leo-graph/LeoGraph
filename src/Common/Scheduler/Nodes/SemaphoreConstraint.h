...
class SemaphoreConstraint final : public ISchedulerConstraint {
 ...
 public:
  explicit SemaphoreConstraint(EventQueue *event_queue_, const Poco::Util::AbstractConfiguration &config = emptyConfig(),
  ...
  SemaphoreConstraint(EventQueue *event_queue_, const SchedulerNodeInfo &info_, Int64 max_requests_, Int64 max_cost_)
      : ISchedulerConstraint(event_queue_, info_), max_requests(max_requests_), max_cost(max_cost_) {}
  ...
  bool equals(ISchedulerNode *other) override {
    ...
    if (auto *o = dynamic_cast<SemaphoreConstraint *>(other)) return max_requests == o->max_requests && max_cost == o->max_cost;
    return false;
  }
  ...
  void finishRequest(ResourceRequest *request) override {
    ...
    if (should_activate) scheduleActivation();
  }
  ...
  void updateConstraints(const SchedulerNodePtr &self, Int64 new_max_requests, UInt64 new_max_cost) {
    ...
    if (parent) {
      ...
      if (!was_active && active()) parent->activateChild(this);
 ...
 private:
  bool satisfied() const { return requests < max_requests && cost < max_cost; }
