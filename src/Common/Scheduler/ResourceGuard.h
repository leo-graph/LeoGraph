 ...
 public:
  ...
  struct Metrics {
    ...
    static const Metrics* getIORead() {
      static Metrics metrics{.requests = ProfileEvents::SchedulerIOReadRequests,
    ...
    static const Metrics* getIOWrite() {
      static Metrics metrics{.requests = ProfileEvents::SchedulerIOWriteRequests,
  ...
  class Request : public ResourceRequest {
   public:
    ...
    void wait() {
      ...
      if (exception)
        throw Exception(ErrorCodes::RESOURCE_ACCESS_DENIED, "Resource request failed: {}",
    ...
    void finish(ResourceCost real_cost_, ResourceLink link_) {
      ...
      state = Finished;
      if (estimated_cost != real_cost_) link_.queue->adjustBudget(estimated_cost, real_cost_);
  ...
  explicit ResourceGuard(const Metrics* metrics, ResourceLink link_, ResourceCost cost = 1,
                         ResourceGuard::Lock type = ResourceGuard::Lock::Default)
      : link(link_), request(Request::local(metrics)) {
    if (cost == 0)
      link.reset();  // Ignore zero-cost requests
    else if (link) {
      request.enqueue(cost, link);
      if (type == Lock::Default) request.wait();
    }
  }
  ...
  void lock() {
    if (link) request.wait();
  }

  void consume(ResourceCost cost) { real_cost += cost; }
