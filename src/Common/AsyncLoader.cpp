...
AsyncLoader::Pool::Pool(const AsyncLoader::PoolInitializer &init)
    : name(init.name),
      ...
      thread_pool(std::make_unique<ThreadPool>(
          init.metric_threads, init.metric_active_threads, init.metric_scheduled_threads,
...
AsyncLoader::Pool::Pool(Pool &&o) noexcept
    : name(o.name),
      ...
      workers(o.workers),
      suspended_workers(o.suspended_workers.load())  // All these constructors are needed because std::atomic is neither copy-constructible,
                                                     // nor move-constructible. We never move pools after init, so it is safe.
      ,
...
void cancelOnDependencyFailure(const LoadJobPtr &self, const LoadJobPtr &dependency, std::exception_ptr &cancel) {
  cancel = std::make_exception_ptr(Exception(ErrorCodes::ASYNC_LOAD_CANCELED, "Load job '{}' -> {}", self->name,
...
size_t LoadJob::executionPool() const { return execution_pool_id; }

size_t LoadJob::pool() const { return pool_id; }
...
LoadTask::LoadTask(AsyncLoader &loader_, LoadJobSet &&jobs_, LoadJobSet &&goal_jobs_)
    : loader(loader_), jobs(std::move(jobs_)), goal_jobs(std::move(goal_jobs_)) {}

LoadTask::~LoadTask() { remove(); }
...
void LoadTask::schedule() { loader.schedule(*this); }
...
AsyncLoader::AsyncLoader(std::vector<PoolInitializer> pool_initializers, bool log_failures_, bool log_progress_, bool log_events_)
  ...
  for (auto &&init : pool_initializers) pools.push_back(Pool(init));
}
...
AsyncLoader::~AsyncLoader() {
  ...
  // To make sure we check for all pending jobs to be finished.
  {
    ...
    if (!scheduled_jobs.empty() || !finished_jobs.empty()) {
      ...
      for (const auto &[job, _] : scheduled_jobs) scheduled.push_back(job->name);
      for (const auto &job : finished_jobs) finished.push_back(job->name);
      LOG_ERROR(log, "Bug. Destruction with pending ({}) and finished ({}) load jobs.", fmt::join(scheduled, ", "),
                fmt::join(finished, ", "));
      abort();
    }
  }
...
void AsyncLoader::wait() {
  ...
  while (!scheduled_jobs.empty() || hasWorker(lock)) {
    lock.unlock();
    for (auto &p : pools) p.thread_pool->wait();
    lock.lock();
    ...
    if (!scheduled_jobs.empty() && !hasWorker(lock)) {
      ...
      for (const auto &[job, _] : scheduled_jobs) names.push_back(job->name);
...
void AsyncLoader::shutdown() {
...

  {
    ...
    for (const auto &[job, _] : scheduled_jobs) jobs.insert(job);
  }
...
void AsyncLoader::schedule(const LoadJobSet &jobs_to_schedule) {
  ...
  LoadJobSet jobs;
  for (const auto &job : jobs_to_schedule) gatherNotScheduled(job, jobs, lock);

  if (jobs.empty()) return;

  if (shutdown_requested) throw Exception(ErrorCodes::ASYNC_LOAD_CANCELED, "AsyncLoader was shut down");

  // Ensure scheduled_jobs graph will have no cycles. The only way to get a cycle is to add a cycle, assuming old jobs cannot reference new
  // ones.
  ...
  for (const auto &job : jobs) {
    ...
    NOEXCEPT_SCOPE({
      ...
      if (log_events) LOG_DEBUG(log, "Schedule load job '{}' into {}", job->name, getPoolName(job->pool()));
    });
  }
  ...
  for (const auto &job : jobs) {
    ...
    if (!info.isBlocked()) enqueue(info, job, lock);
  }
  ...
  for (const auto &job : jobs) {
    if (auto info = scheduled_jobs.find(job); info != scheduled_jobs.end()) {
      for (const auto &dep : job->dependencies) {
        if (scheduled_jobs.contains(dep)) continue;  // Skip dependencies on scheduled jobs (already processed in pass 3)
        LoadStatus dep_status = dep->status();
        if (dep_status == LoadStatus::OK) continue;  // Dependency on already successfully finished job -- it's okay.
        ...
        if (dep_status == LoadStatus::FAILED || dep_status == LoadStatus::CANCELED) {
          ...
          NOEXCEPT_SCOPE({
            ALLOW_ALLOCATIONS_IN_SCOPE;
            if (job->dependency_failure) job->dependency_failure(job, dep, cancel);
          });
          if (cancel) {
...
void AsyncLoader::gatherNotScheduled(const LoadJobPtr &job, LoadJobSet &jobs, std::unique_lock<std::mutex> &lock) {
  if (job->status() == LoadStatus::PENDING && !scheduled_jobs.contains(job) && !jobs.contains(job)) {
    jobs.insert(job);
    for (const auto &dep : job->dependencies) gatherNotScheduled(dep, jobs, lock);
  }
}
...
void AsyncLoader::wait(const LoadJobPtr &job, bool no_throw) {
  ...
  if (!no_throw && job->load_exception)
    throw Exception(ErrorCodes::ASYNC_LOAD_WAIT_FAILED, "Waited job failed: {}",
                    getExceptionMessage(job->load_exception, /* with_stacktrace = */ false));
}
...
void AsyncLoader::remove(const LoadJobSet &jobs) {
  ...
  for (const auto &job : jobs) {
    if (auto info = scheduled_jobs.find(job); info != scheduled_jobs.end()) {
      if (info->second.isExecuting()) continue;  // Skip executing jobs on the first pass
      std::exception_ptr e;
      NOEXCEPT_SCOPE({
        ALLOW_ALLOCATIONS_IN_SCOPE;
  ...
  for (const auto &job : jobs) {
    ...
    if (old_jobs >= erased && job->finishTime() != LoadJob::TimePoint{} && job->finishTime() < busy_period_start_time) old_jobs -= erased;
  }
}

void AsyncLoader::setMaxThreads(size_t pool, size_t value) {
  if (value == 0) value = getNumberOfCPUCoresToUse();
  std::unique_lock lock{mutex};
  auto &p = pools[pool];
  ...
  for (size_t i = 0; canSpawnWorker(p, lock) && i < p.ready_queue.size(); i++) spawn(p, lock);
}
...
std::vector<AsyncLoader::JobState> AsyncLoader::getJobStates() const {
  ...
  for (const auto &job : finished_jobs) states.emplace(job->name, JobState{.job = job});
  lock.unlock();
  std::vector<JobState> result;
  for (auto &&[_, state] : states) result.emplace_back(std::move(state));
  return result;
}

size_t AsyncLoader::suspendedWorkersCount(size_t pool_id) { return pools[pool_id].suspended_workers.load(); }
...
String AsyncLoader::checkCycle(const LoadJobPtr &job, LoadJobSet &left, LoadJobSet &visited, std::unique_lock<std::mutex> &lock) {
  if (!left.contains(job)) return {};  // Do not consider external dependencies and already processed jobs
...
void AsyncLoader::finish(const LoadJobPtr &job, LoadStatus status, std::exception_ptr reason, std::unique_lock<std::mutex> &lock) {
  ...
  for (const auto &dpt : dependent) {
    ...
    if (dpt_info == scheduled_jobs.end()) continue;
    dpt_info->second.dependencies_left--;
    if (!dpt_info->second.isBlocked()) enqueue(dpt_info->second, dpt, lock);

    if (status != LoadStatus::OK) {
      std::exception_ptr cancel;
      NOEXCEPT_SCOPE({
        ALLOW_ALLOCATIONS_IN_SCOPE;
        if (dpt->dependency_failure) dpt->dependency_failure(dpt, job, cancel);
      });
      // Recurse into dependent job if it should be canceled
      if (cancel) finish(dpt, LoadStatus::CANCELED, cancel, lock);
    }
  }
  ...
  if (status != LoadStatus::OK) {
    for (const auto &dep : job->dependencies) {
      if (auto dep_info = scheduled_jobs.find(dep); dep_info != scheduled_jobs.end()) dep_info->second.dependent_jobs.erase(job);
    }
  }
...
void AsyncLoader::prioritize(const LoadJobPtr &job, size_t new_pool_id, std::unique_lock<std::mutex> &lock) {
  ...
  if (old_pool.priority <= new_pool.priority) return;  // Never lower priority or change pool leaving the same priority
  ...
  if (auto info = scheduled_jobs.find(job); info != scheduled_jobs.end()) {
    ...
    if (UInt64 ready_seqno = info->second.ready_seqno) {
      ...
      if (canSpawnWorker(new_pool, lock)) spawn(new_pool, lock);
    }
  }
  ...
  for (const auto &dep : job->dependencies) prioritize(dep, new_pool_id, lock);
}
...
void AsyncLoader::enqueue(Info &info, const LoadJobPtr &job, std::unique_lock<std::mutex> &lock) {
  ...
  if (canSpawnWorker(pool, lock)) spawn(pool, lock);
}
...
size_t currentPoolOr(size_t pool) { return current_load_job ? current_load_job->executionPool() : pool; }

bool static detectWaitDependentDeadlock(const LoadJobPtr &waited) {
  if (waited.get() == current_load_job) return true;
  for (const auto &dep : waited->dependencies) {
    if (detectWaitDependentDeadlock(dep)) return true;
  }
  return false;
}
...
void AsyncLoader::wait(std::unique_lock<std::mutex> &job_lock, const LoadJobPtr &job) {
  ...
  if (current_load_job && job->load_status == LoadStatus::PENDING) {
    ...
    if (worker_pool == job->pool_id) {
      ...
      if (job->load_status != LoadStatus::PENDING) return;  // Job is already done, no wait required
      ...
      if (worker_pool == job->pool_id) {
        ...
        suspended_lock = [&pool] {
          chassert(pool.suspended_workers.load());
          pool.suspended_workers.fetch_sub(1);
        };
        if (canSpawnWorker(pool, lock)) spawn(pool, lock);
      }
    }
  ...
  if (log_events) LOG_DEBUG(log, "Wait load job '{}' in {}", job->name, getPoolName(job->pool_id));

  if (job->on_waiters_increment) job->on_waiters_increment(job);
  ...
  if (job->on_waiters_decrement) job->on_waiters_decrement(job);
...
bool AsyncLoader::canSpawnWorker(Pool &pool, std::unique_lock<std::mutex> &) {
  // TODO(serxa): optimization: we should not spawn new worker on the first enqueue during `finish()` because current worker will take this
  // job.
  return is_running && !pool.ready_queue.empty() && pool.workers < pool.max_threads + pool.suspended_workers.load() &&
...
void AsyncLoader::setCurrentPriority(std::unique_lock<std::mutex> &, std::optional<Priority> priority) {
  if (log_events && current_priority != priority) {
    NOEXCEPT_SCOPE({
      ALLOW_ALLOCATIONS_IN_SCOPE;
      LOG_DEBUG(log, "Change current priority: {} -> {}", current_priority ? std::to_string(*current_priority) : "none",
...
void AsyncLoader::updateCurrentPriorityAndSpawn(std::unique_lock<std::mutex> &lock) {
  ...
  for (Pool &pool : pools) {
    if (pool.isActive() && (!priority || *priority > pool.priority)) priority = pool.priority;
  }
  setCurrentPriority(lock, priority);

  // Spawn workers in all pools with current priority
  for (Pool &pool : pools) {
    for (size_t i = 0; canSpawnWorker(pool, lock) && i < pool.ready_queue.size(); i++) spawn(pool, lock);
  }
}
...
void AsyncLoader::spawn(Pool &pool, std::unique_lock<std::mutex> &lock) {
  ...
  NOEXCEPT_SCOPE({
    ALLOW_ALLOCATIONS_IN_SCOPE;
    if (log_events) LOG_DEBUG(log, "Spawn loader worker #{} in {}", pool.workers, pool.name);
  ...
  while (true) {
...

    {
      ...
      if (!canWorkerLive(pool, lock)) {
        ...
        if (--pool.workers == 0) updateCurrentPriorityAndSpawn(lock);  // It will spawn lower priority workers if needed
        return;
      }
    ...
    try {
      ...
      NOEXCEPT_SCOPE({
        if (log_failures) tryLogCurrentException(__PRETTY_FUNCTION__);
        exception_from_job = std::make_exception_ptr(Exception(ErrorCodes::ASYNC_LOAD_FAILED, "Load job '{}' failed: {}", job->name,
