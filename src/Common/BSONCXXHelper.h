...
static bsoncxx::types::bson_value::value fieldAsBSONValue(const Field &field, const DataTypePtr &type, const bool is_oid) {
  if (field.isNull()) return bsoncxx::types::b_null{};

  auto type_id = type->getTypeId();
  if (type->isNullable()) type_id = typeid_cast<const DataTypeNullable *>(type.get())->getNestedType()->getTypeId();

  switch (type_id) {
    case TypeIndex::Nothing:
      return bsoncxx::types::b_null{};
    case TypeIndex::String: {
      if (is_oid) return bsoncxx::oid(field.safeGet<String>());
      return bsoncxx::types::b_string{field.safeGet<String>()};
    }
    case TypeIndex::UInt8: {
      if (isBool(type)) return bsoncxx::types::b_bool{field.safeGet<UInt8>() != 0};
    ...
    case TypeIndex::UUID: {
      ...
      return bsoncxx::types::bson_value::value(reinterpret_cast<const uint8_t *>(&uuid_numbers[0]), 16, bsoncxx::binary_sub_type::k_uuid);
    }
    case TypeIndex::Tuple: {
      auto arr = bsoncxx::v_noabi::builder::basic::array();
      const auto *tuple_type = typeid_cast<const DataTypeTuple *>(type.get());

      if (!tuple_type) throw Exception(ErrorCodes::TYPE_MISMATCH, "Expected Tuple type, got {}.", type->getPrettyName());
      ...
      if (elem_types.size() != tuple_value.size())
        throw Exception(ErrorCodes::TYPE_MISMATCH, "Tuple size mismatch, expected {} elements, got {} in field.", elem_types.size(),
                        tuple_value.size());

      for (size_t i = 0; i < elem_types.size(); ++i) arr.append(fieldAsBSONValue(tuple_value[i], elem_types[i], is_oid));
      return arr.view();
    }
    case TypeIndex::Array: {
      ...
      if (!array_type) throw Exception(ErrorCodes::TYPE_MISMATCH, "Expected Array type, got {}.", type->getPrettyName());

      const auto &elem_type = array_type->getNestedType();
      for (const auto &elem : field.safeGet<Array>()) arr.append(fieldAsBSONValue(elem, elem_type, is_oid));
      return arr.view();
    }
    default:
...
static JSONBuilder::ItemPtr BSONElementAsJSON(const bsoncxx::types::bson_value::view &value) {
  switch (value.type()) {
    ...
    case bsoncxx::type::k_document: {
      ...
      for (const auto &elem : value.get_document().value) doc->add(std::string(elem.key()), BSONElementAsJSON(elem.get_value()));
      return doc;
    }
    case bsoncxx::type::k_array: {
      auto arr = std::make_unique<JSONBuilder::JSONArray>();
      for (const auto &elem : value.get_array().value) arr->add(BSONElementAsJSON(elem.get_value()));
      return arr;
    }
...
static std::string BSONElementAsString(const bsoncxx::types::bson_value::view &value,
  ...
  switch (value.type()) {
    ...
    // MongoDB's documents and arrays may not have strict types or be nested, so the most optimal solution is store their JSON
    // representations. bsoncxx::to_json function will return something like "'number': {'$numberInt': '321'}", this why we have to use own
    // implementation.
    case bsoncxx::type::k_document:
    case bsoncxx::type::k_array:
    case bsoncxx::type::k_regex:
...
static Array BSONArrayAsArray(size_t dimensions, const bsoncxx::types::b_array &array, const DataTypePtr &type, const Field &default_value,
                              const std::string &name, const JSONBuilder::FormatSettings &json_format_settings) {
  auto arr = Array();
  if (dimensions > 0) {
    --dimensions;
