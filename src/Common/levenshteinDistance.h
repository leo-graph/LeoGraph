...
size_t levenshteinDistance(std::span<const Element> lhs, std::span<const Element> rhs) {
  ...
  for (size_t j = 1; j <= m; ++j) {
    ...
    for (size_t i = 1; i <= n; ++i) {
      size_t old = v0[i];
      v0[i] = std::min(prev + (lhs[j - 1] != rhs[i - 1]), std::min(v0[i - 1], v0[i]) + 1);
      prev = old;
    }
  }
  return v0[n];
...
Weight levenshteinDistanceWeighted(std::span<const Element> lhs, std::span<const Element> rhs, std::span<const Weight> lhs_weights,
                                   std::span<const Weight> rhs_weights) {
  if (lhs.size() != lhs_weights.size() || rhs.size() != rhs_weights.size())
    throw Exception(ErrorCodes::SIZES_OF_ARRAYS_DONT_MATCH, "Sizes of arrays don't match: {} != {} or {} != {}", lhs.size(),
                    lhs_weights.size(), rhs.size(), rhs_weights.size());

  auto sum_span = [](const std::span<const Weight>& span) -> Weight { return std::accumulate(span.begin(), span.end(), Weight{}); };
  ...
  for (size_t j = 1; j <= n; ++j) {
    ...
    for (size_t i = 1; i <= m; ++i) {
      ...
      row[i] = static_cast<Weight>(std::min({row[i] + rhs_weights[j - 1],                              // deletion
