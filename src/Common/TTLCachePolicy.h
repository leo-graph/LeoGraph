...
class PerUserTTLCachePolicyUserQuota : public ICachePolicyUserQuota {
 public:
  ...
  bool approveWrite(const UUID &user_id, size_t entry_size_in_bytes) const override {
    ...
    if (it_actual != actual.end()) actual_for_user = it_actual->second;

    auto it_quota = quotas.find(user_id);
    Resources quota_for_user{.size_in_bytes = std::numeric_limits<size_t>::max(),
                             .num_items = std::numeric_limits<size_t>::max()};  /// if no user is found, the default is no threshold
    if (it_quota != quotas.end()) quota_for_user = it_quota->second;

    /// Special case: A quota configured as 0 means no threshold
    if (quota_for_user.size_in_bytes == 0) quota_for_user.size_in_bytes = std::numeric_limits<UInt64>::max();
    if (quota_for_user.num_items == 0) quota_for_user.num_items = std::numeric_limits<UInt64>::max();

    /// Check size quota
    if (actual_for_user.size_in_bytes + entry_size_in_bytes > quota_for_user.size_in_bytes) return false;

    /// Check items quota
    if (actual_for_user.num_items + 1 > quota_for_user.num_items) return false;

    return true;
  }

  void clear() override { actual.clear(); }
...
/// The cache size (in bytes and number of entries) can be changed at runtime. It is expected to set both sizes explicitly after
/// construction.
...
class TTLCachePolicy : public ICachePolicy<Key, Mapped, HashFunction, WeightFunction> {
 public:
  ...
  explicit TTLCachePolicy(CurrentMetrics::Metric size_in_bytes_metric_, CurrentMetrics::Metric count_metric_,
                          CachePolicyUserQuotaPtr quotas_)
  ...
  ~TTLCachePolicy() override { clearImpl(); }

  size_t sizeInBytes() const override { return size_in_bytes; }

  size_t count() const override { return cache.size(); }
  ...
  void clear() override { clearImpl(); }

  void remove(const Key &key) override {
    auto it = cache.find(key);
    if (it == cache.end()) return;
    size_t sz = weight_function(*it->second);
    if (it->first.user_id.has_value()) Base::user_quotas->decreaseActual(*it->first.user_id, sz);
    cache.erase(it);
    size_in_bytes -= sz;
  ...
  bool contains(const Key &key) const override { return cache.count(key) != 0; }
  ...
  void set(const Key &key, const MappedPtr &mapped) override {
    ...
    auto sufficient_space_in_cache_for_user = [&]() {
      if (key.user_id.has_value()) return Base::user_quotas->approveWrite(*key.user_id, entry_size_in_bytes);
      return true;
    };
    ...
    if (sufficient_space_in_cache() && sufficient_space_in_cache_for_user()) {
      ...
      if (key.user_id.has_value()) Base::user_quotas->increaseActual(*key.user_id, entry_size_in_bytes);
    }
  ...
  std::vector<KeyMapped> dump() const override {
    ...
    for (const auto &[key, mapped] : cache) res.push_back({key, mapped});
    return res;
  }
