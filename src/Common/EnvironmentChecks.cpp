...
enum class InstructionFail : uint8_t { NONE = 0, SSE3 = 1, SSSE3 = 2, SSE4_1 = 3, SSE4_2 = 4, POPCNT = 5, AVX = 6, AVX2 = 7, AVX512 = 8 };
...
[[noreturn]] void sigIllCheckHandler(int, siginfo_t *, void *) { siglongjmp(jmpbuf, 1); }
...
#define writeError(data)                                              \
  do {                                                                \
    ...
    if (!writeRetry(STDERR_FILENO, data, sizeof(data) - 1)) _Exit(1); \
  } while (false)
...
void checkRequiredInstructions() {
  ...
  auto signal = SIGILL;
  if (sigemptyset(&sa.sa_mask) != 0 || sigaddset(&sa.sa_mask, signal) != 0 || sigaction(signal, &sa, &sa_old) != 0) {
  ...
  if (sigsetjmp(jmpbuf, 1)) {
    ...
    if (!std::apply(writeRetry, instructionFailToString(fail))) _Exit(1);
...
struct Checker {
  Checker() { checkRequiredInstructions(); }
} checker
#ifndef OS_DARWIN
