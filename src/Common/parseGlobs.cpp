...
bool containsRangeGlob(const std::string& input) { return RE2::PartialMatch(input, Regexps::instance().range_regex); }

bool containsOnlyEnumGlobs(const std::string& input) { return input.find_first_of("*?") == String::npos && !containsRangeGlob(input); }
...
/* Transforms string from grep-wildcard-syntax ("{N..M}", "{a,b,c}" as in remote table function and "*", "?") to perl-regexp for using re2
 * library for matching with such steps: 1) search intervals like {0..9} and enums like {abc,xyz,qwe} in {}, replace them by regexp with
 * pipe (expr1|expr2|expr3), 2) search and replace "*" and "?". Before each search need to escape symbols that we would not search.
 *
...
std::string makeRegexpPatternFromGlobs(const std::string& initial_str_with_globs) {
  ...
  while (true) {
    ...
    /// it is a range.
    if (did_match_range && did_match_enum && matched_range.data() == matched_enum.data()) did_match_enum = false;
    ...
    if (did_match_range && (!did_match_enum || matched_range.data() < matched_enum.data())) {
      ...
      if (leading_zeros) oss_for_replacing << std::setfill('0') << std::setw(static_cast<int>(output_width));
...
void expandSelectorGlobImpl(const std::string& path, std::vector<std::string>& for_match_paths_expanded) {
  ...
  if (!definitely_no_selector_globs) {
    ...
    auto is_this_enum_of_one_char =
        left_bracket_pos != std::string::npos && right_bracket_pos != std::string::npos && (right_bracket_pos - left_bracket_pos) == 2;
  ...
  for (auto it = path.begin(); it != path.end(); ++it) {
    if (*it == '{') {
      if (opened) throw Exception(ErrorCodes::BAD_ARGUMENTS, "Unexpected '{{' found in path '{}' at position {}.", path, it - path.begin());
      anchor_positions.push_back(std::distance(path.begin(), it));
      opened = true;
    } else if (*it == '}') {
      if (!opened)
        throw Exception(ErrorCodes::BAD_ARGUMENTS, "Unexpected '}}' found in path '{}' at position {}.", path, it - path.begin());
      anchor_positions.push_back(std::distance(path.begin(), it));
      closed = true;
      break;
    } else if (*it == ',') {
      if (!opened)
        throw Exception(ErrorCodes::BAD_ARGUMENTS, "Unexpected ',' found in path '{}' at position {}.", path,
                        std::distance(path.begin(), it));
      anchor_positions.push_back(std::distance(path.begin(), it));
    }
  }
  if (!opened || !closed) throw Exception(ErrorCodes::BAD_ARGUMENTS, "Invalid {{}} glob in path {}.", path);
  ...
  for (size_t i = 1; i < anchor_positions.size(); ++i) {
    std::string current_selection = path.substr(anchor_positions[i - 1] + 1, (anchor_positions[i] - anchor_positions[i - 1] - 1));
