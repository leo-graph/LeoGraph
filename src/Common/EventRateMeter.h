...
class EventRateMeter {
 public:
  explicit EventRateMeter(double now, double period_, size_t heating_ = 0) : period(period_), max_interval(period * 10), heating(heating_) {
    reset(now);
  }
  ...
  void add(double now, double count) {
    ...
    if (count != 0.0 && data_points++ <= heating) reset(events.time, data_points);
  ...
  double rate(double now) {
    add(now, 0);
    if (unlikely(now <= start)) return 0;
