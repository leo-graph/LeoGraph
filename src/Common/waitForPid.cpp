...
enum PollPidResult { RESTART, FAILED };
...
static int syscall_pidfd_open(pid_t pid) { return static_cast<int>(syscall(SYS_pidfd_open, pid, 0)); }
...
static PollPidResult pollPid(pid_t pid, int timeout_in_ms) {
  ...
  if (supportsPidFdOpen()) {
    ...
    if (pid_fd < 0) {
      if (errno == ESRCH) return PollPidResult::RESTART;
    ...
    if (pid_fd < 0) {
      if (errno == ENOENT) return PollPidResult::RESTART;
  ...
  if (ready <= 0) return PollPidResult::FAILED;
...
static PollPidResult pollPid(pid_t pid, int timeout_in_ms) {
  int kq = kqueue();
  if (kq == -1) return PollPidResult::FAILED;
...
static PollPidResult pollPid(pid_t pid, int timeout_in_ms) {
  ...
  if (hdl == NULL) {
    if (perr == G_NOPROC) return PollPidResult::RESTART;
    return PollPidResult::FAILED;
  }

  rc = Pstopstatus(hdl, PCWSTOP, timeout_in_ms);
  if (rc < 0 && errno == ENOENT) result = PollPidResult::RESTART;
  if (rc == 0) {
    int state = Pstate(hdl);
    if (state == PS_DEAD || state == PS_UNDEAD) result = PollPidResult::RESTART;
  }
...
bool waitForPid(pid_t pid, size_t timeout_in_seconds) {
  ...
  while (timeout_in_ms > 0) {
    int waitpid_res = HANDLE_EINTR(waitpid(pid, &status, WNOHANG));
    bool process_terminated_normally = (waitpid_res == pid);
    if (process_terminated_normally) return true;

    if (waitpid_res != 0) return false;

    watch.restart();

    PollPidResult result = pollPid(pid, timeout_in_ms);

    if (result == PollPidResult::FAILED) return false;
