...
#  define SAFE_CHECK(cond, ...)                                                  \
    do {                                                                         \
      if (!(cond)) throw Exception(ErrorCodes::CANNOT_PARSE_DWARF, __VA_ARGS__); \
    } while (false)
...
uint64_t readOffset(std::string_view &sp, bool is64_bit) { return is64_bit ? read<uint64_t>(sp) : read<uint32_t>(sp); }
...
std::string_view Dwarf::getSection(const char *name) const {
  ...
  if (!elf_section) return {};

  if (elf_section->header.flags & SectionHeaderFlag::COMPRESSED) return {};
...
Dwarf::Attribute Dwarf::readAttribute(const CompilationUnit &cu, const Die &die, AttributeSpec spec, std::string_view &info) const {
...
bool Dwarf::findDebugInfoOffset(uintptr_t address, std::string_view aranges, uint64_t &offset) {
  ...
  while (aranges_section.next(chunk)) {
    ...
    for (;;) {
      ...
      if (address >= start && address < start + length) return true;
    }
  }
  return false;
}
...
std::optional<std::pair<std::optional<Dwarf::CompilationUnit>, uint64_t>> Dwarf::getReferenceAttribute(const CompilationUnit &cu,
                                                                                                       const Die &die,
  ...
  switch (form) {
    ...
    case DW_FORM_ref_sig8:
      ...
      throw Exception(ErrorCodes::CANNOT_PARSE_DWARF, "Type signatures are not supported (DIE at 0x{:x}, attr 0x{:x}).", die.offset,
                      attr_name);

    case DW_FORM_ref_sup4:
    case DW_FORM_ref_sup8:
      throw Exception(ErrorCodes::CANNOT_PARSE_DWARF, "Supplementary object files are not supported (DIE at 0x{:x}, attr 0x{:x}).",
                      die.offset, attr_name);

    default:
      throw Exception(ErrorCodes::CANNOT_PARSE_DWARF, "Unexpected form of attribute 0x{:x}: 0x{:x} (DIE at 0x{:x}).", attr_name, form,
                      die.offset);
  }
}
 ...
 */
bool Dwarf::findLocation(uintptr_t address, const LocationInfoMode mode, CompilationUnit &cu, LocationInfo &info,
                         std::vector<SymbolizedFrame> &inline_frames, bool assume_in_cu_range) const {
  Die die = getDieAtOffset(cu, cu.first_die);
...
void Dwarf::findSubProgramDieForAddress(const CompilationUnit &cu, const Die &die, uint64_t address, std::optional<uint64_t> base_addr_cu,
 ...
 */
void Dwarf::findInlinedSubroutineDieForAddress(const CompilationUnit &cu, const Die &die, const LineNumberVM &line_vm, uint64_t address,
                                               std::optional<uint64_t> base_addr_cu, std::vector<CallLocation> &locations,
  ...
  forEachChild(cu, die, [&](const Die &child_die) {
    ...
    auto get_function_name = [&](const CompilationUnit &srcu, uint64_t die_offset) {
      ...
      if (def.has_value()) {
        ...
        if (def_cu.has_value()) spec_cu = std::move(def_cu.value());
...
bool Dwarf::isAddrInRangeList(const CompilationUnit &cu, uint64_t address, std::optional<uint64_t> base_addr, size_t offset,
...
Dwarf::LineNumberVM::LineNumberVM(std::string_view data, std::string_view compilationDirectory, std::string_view debugStr,
                                  std::string_view debugLineStr)
    : compilationDirectory_(compilationDirectory), debugStr_(debugStr), debugLineStr_(debugLineStr) {
...
LineNumberAttribute static readLineNumberAttribute(bool is64_bit, std::string_view &format, std::string_view &entries,
