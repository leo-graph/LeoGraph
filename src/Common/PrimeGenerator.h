...
class OddPrimesCache {
 public:
  ...
  OddPrimesCache() : current_limit(7), composite_bits(1, 0), primes{3, 5, 7} {
#ifndef NDEBUG
  ...
  void ensureUpTo(UInt64 limit) {
    ...
    if ((limit_odd & 1) == 0) --limit_odd;

    if (limit_odd > std::numeric_limits<UInt32>::max())
      throw Exception(ErrorCodes::LOGICAL_ERROR, "OddPrimesCache supports limit up to {}, got {}",
                      static_cast<UInt64>(std::numeric_limits<UInt32>::max()) + 1, limit);

    /// We already have enough primes cached
    if (limit_odd <= current_limit) return;

    const UInt64 sqrt_limit = integerSqrt(limit_odd);

    /// Ensure primes up to sqrt(limit_odd) exist before sieving the extension
    if (sqrt_limit >= 3 && current_limit < sqrt_limit) ensureUpTo(sqrt_limit);
    ...
    for (UInt64 prime : primes) {
      ...
      if ((prime_sq & 1) == 0) prime_sq += prime;

      const UInt64 multiple_start_index = (prime_sq - 3) >> 1;

      /// Mark odd multiples of prime as composite
      for (UInt64 idx = multiple_start_index; idx < new_n_odds; idx += prime) setBit(composite_bits, idx);
    }

    for (UInt64 i = old_n_odds; i < new_n_odds; ++i)
      if (!getBit(composite_bits, i)) primes.push_back(static_cast<UInt32>(3 + (i << 1)));
...
class SegmentedOddSieve {
  ...
  std::optional<UInt64> next() {
    while (true) {
      ...
      if (!segment_sieved || next_candidate_idx >= segment_odd_count) {
        if (!sieveNextSegment()) return std::nullopt;
      }
 ...
 private:
  ...
  bool sieveNextSegment() {
    ...
    if (bounded) {
      if (range_high_odd == 0) return false;
    }
    ...
    if (bounded && segment_begin > range_high_odd) return false;
    ...
    if (unlikely(segment_end < segment_begin)) segment_end = MAX_UINT64;  /// clamp on overflow (MAX_UINT64 is odd)
    ...
    if (tail_bits != 0) segment_composite_bits[segment_word_count - 1] |= (~0ULL) << tail_bits;
  ...
  void sieveSegment() {
    ...
    for (size_t i = 0; i < active_primes; ++i) {
      ...
      if (unlikely(prime_sq == NO_MULTIPLE)) continue;

      chassert((prime_sq & 1) == 1);
      chassert(prime_sq >= segment_begin);

      if (likely(segment_end <= MAX_UINT64 - step)) {
        for (; prime_sq <= segment_end; prime_sq += step) markCompositeInSegment(prime_sq);
      } else {
...
class RangeSegmentedSievePrimeGenerator {
 public:
  void setRange(UInt64 low, UInt64 high) {
    if (low > high) throw Exception(ErrorCodes::LOGICAL_ERROR, "Invalid range for RangeSegmentedSievePrimeGenerator: [{}, {}]", low, high);
    ...
    if ((range_low_odd & 1) == 0) ++range_low_odd;

    /// Make high odd
    UInt64 range_high_odd = high;
    if ((range_high_odd & 1) == 0) --range_high_odd;
