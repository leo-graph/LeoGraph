...
This part of executable binary is loaded in memory and accessible via 'dl_iterate_phdr', 'dladdr' and 'backtrace_symbols' functions from
libc. ClickHouse versions prior to 19.13 has used just these symbol names to symbolize stack traces and stack traces may be incomplete due
to lack of symbols with internal linkage. But because ClickHouse is linked with most of the symbols exported (-rdynamic flag) it can still
provide good enough stack traces.
...
If you have debug info (you build ClickHouse by yourself or install clickhouse-common-static-dbg package), you will get source file names
and line numbers. Otherwise you will get only symbol names. If your binary contains symbol table in section headers (the default, unless
stripped), you will get all symbol names. Otherwise you will get only exported symbols from program headers.
...
void collectSymbolsFromProgramHeaders(DynamicLinkingProgramHeaderInfo *info, std::vector<SymbolIndex::Symbol> &symbols) {
  ...
  for (size_t header_index = 0; header_index < info->phnum; ++header_index) {
    ...
    if (info->phdr[header_index].type != ProgramHeaderType::DYNAMIC) continue;
    ...
    auto correct_address = [](uint64_t base, uint64_t ptr) { return ptr > base ? ptr : base + ptr; };
    ...
    size_t sym_cnt = 0;
    {
      for (const auto *it = dyn_begin;; ++it) {
        ...
        if (it->tag == DynamicTableTag::Null) break;

        if (it->tag != DynamicTableTag::GNU_HASH) continue;
        ...
        for (uint32_t i = 0; i < hash[0]; ++i) sym_cnt = std::max<size_t>(sym_cnt, buckets[i]);
    ...
    for (const auto *it = dyn_begin; it->tag != DynamicTableTag::Null; ++it) {
      ...
      if (it->tag == DynamicTableTag::SYMTAB) {
        ...
        for (uint32_t sym_index = 0; sym_index < uint32_t(sym_cnt); ++sym_index) {
          ...
          symbol.offset_begin = reinterpret_cast<const void *>(elf_sym[sym_index].value);
          symbol.offset_end = reinterpret_cast<const void *>(elf_sym[sym_index].value + elf_sym[sym_index].size);
          symbol.name = sym_name;

          /// We are not interested in empty symbols.
          if (elf_sym[sym_index].size) symbols.push_back(symbol);
        }
...
String getBuildIDFromProgramHeaders(DynamicLinkingProgramHeaderInfo *info) {
  ...
  for (size_t header_index = 0; header_index < info->phnum; ++header_index) {
    ...
    if (phdr.type != ProgramHeaderType::NOTE) continue;
...
void collectSymbolsFromELFSymbolTable(const Elf &elf, const Elf::Section &symbol_table, const Elf::Section &string_table,
  ...
  for (; symbol_table_entry < symbol_table_end; ++symbol_table_entry) {
    if (!symbol_table_entry->name || !symbol_table_entry->value || strings + symbol_table_entry->name >= elf.end()) continue;
    ...
    SymbolIndex::Symbol symbol;
    symbol.offset_begin = reinterpret_cast<const void *>(symbol_table_entry->value);
    symbol.offset_end = reinterpret_cast<const void *>(symbol_table_entry->value + symbol_table_entry->size);
    symbol.name = symbol_name;

    if (symbol_table_entry->size) symbols.push_back(symbol);
  }
}

bool searchAndCollectSymbolsFromELFSymbolTable(const Elf &elf, SectionHeaderType section_header_type, const char *string_table_name,
                                               std::vector<SymbolIndex::Symbol> &symbols) {
...
void collectSymbolsFromELF(DynamicLinkingProgramHeaderInfo *info, std::vector<SymbolIndex::Symbol> &symbols,
                           std::vector<SymbolIndex::Object> &objects, String &self_build_id) {
  String object_name;
  String build_id;
  ...
  if (object_name.empty()) {
    ...
    if (build_id.empty()) build_id = Elf(object_name).getBuildID();

    if (self_build_id.empty()) self_build_id = build_id;
  }
#endif
  ...
  if (build_id != file_build_id) {
    ...
    if (object_name != canonical_path) {
      ...
      if (build_id != file_build_id) return;
    } else
      return;
  }
...
const SymbolIndex::Symbol *find(const void *offset, const std::vector<SymbolIndex::Symbol> &vec) {
  ...
  if (offset >= it->offset_begin && offset < it->offset_end) return &*it;
  return nullptr;
}
...
const SymbolIndex::Object *find(const void *address, const std::vector<SymbolIndex::Object> &vec) {
  ...
  if (address >= it->address_begin && address < it->address_end) return &*it;
  return nullptr;
}
...
void SymbolIndex::load() {
  ...
  data.symbols.erase(
      std::unique(data.symbols.begin(), data.symbols.end(),
                  [](const Symbol &a, const Symbol &b) { return a.offset_begin == b.offset_begin && a.offset_end == b.offset_end; }),
      data.symbols.end());
}
...
const SymbolIndex::Symbol *SymbolIndex::findSymbol(const void *address) const {
  ...
  if (object) {
    ...
    offset = reinterpret_cast<const void *>(reinterpret_cast<uintptr_t>(address) - reinterpret_cast<uintptr_t>(object->address_begin));
  }
...
const SymbolIndex::Object *SymbolIndex::findObject(const void *address) const { return find(address, data.objects); }
...
const SymbolIndex &SymbolIndex::instance() {
  ...
  /// nice error:
  ///
  ///   __cxa_guard_acquire detected recursive initialization: do you have a function-local static variable whose initialization depends on
  ///   that function
  ///
