...
String mapTypesToTypesWithLinks(const std::vector<std::string>& types, const FunctionDocumentation::Syntax& syntax) {
  ...
  for (auto type : types) {
    ...
    if (type.starts_with("const ")) type = type.substr(6);  // Remove "const " prefix
    ...
    if (type == "NULL")
    ...
    else if (type.starts_with("Int") || type.starts_with("UInt") ||
             type.starts_with("(U)Int"))  /// "Int8", "Int16", ... || "UInt8", "UInt16", ... || "(U)Int*", "(U)Int8", "(U)Int16", ...
      ...
      result += "`](/sql-reference/data-types/aggregatefunction)";
                 ...
                 "SimpleAggregateFunction"))  /// "SimpleAggregateFunction(agg_func, T)", "SimpleAggregateFunction(any, UInt8)", ...
...
String argumentsOrParametersAsString(const Type& arguments_or_parameters, const FunctionDocumentation::Syntax& syntax) {
  ...
  for (const auto& [name, description_, types] : arguments_or_parameters) {
    ...
    if (!types.empty()) result += mapTypesToTypesWithLinks(types, syntax);
  }
  return result;
}

String FunctionDocumentation::argumentsAsString() const { return argumentsOrParametersAsString(arguments, syntax); }

String FunctionDocumentation::parametersAsString() const { return argumentsOrParametersAsString(parameters, syntax); }
...
String FunctionDocumentation::syntaxAsString() const {
  ...
  if (syntax.ends_with(";")) throw Exception(ErrorCodes::LOGICAL_ERROR, "Syntax field must not end with ';': {}", syntax);
...
String FunctionDocumentation::returnedValueAsString() const {
  ...
  /// We assume that if 'type' is empty(), 'description' already ends with a type definition. This is a reasonable assumption to be
  /// able to handle special cases which cannot be represented by the type mapping in mapTypesToTypesWithLinks.
  if (!returned_value.types.empty()) result += mapTypesToTypesWithLinks(returned_value.types, syntax);
...
String FunctionDocumentation::categoryAsString() const {
  static const std::unordered_map<Category, std::string> category_to_string = {
      {Category::Unknown,
       ""},  /// Default enum value for default-constructed FunctionDocumentation objects. Be consistent with other default fields (empty).
...
FunctionDocumentation FunctionDocumentation::INTERNAL_FUNCTION_DOCS = {
    "", "", {}, {}, {"", {}}, {}, FunctionDocumentation::VERSION_UNKNOWN, FunctionDocumentation::Category::Internal};
