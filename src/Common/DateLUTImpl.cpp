...
DateLUTImpl::DateLUTImpl(std::string_view time_zone_) : time_zone(time_zone_) {
  /// DateLUT should not be initialized in global constructors for the following reasons:
  /// 1. It is too heavy.
  if (&inside_main) assert(inside_main);
  ...
  do {
    ...
    if (date >= next_transition_date) {
      ...
      if (cctz_time_zone.next_transition(start_of_day_time_point_if_no_transitions - std::chrono::seconds(1), &transition) &&
          (cctz::civil_day(transition.from) == date || cctz::civil_day(transition.to) == date) && transition.from != transition.to) {
  ...
  for (unsigned day = 0; day < DATE_LUT_SIZE; ++day) {
    ...
    if (values.day_of_month == 1) {
      if (values.month == 1) years_lut[values.year - DATE_LUT_MIN_YEAR] = day;
  ...
  /// Fill saturated LUT.
  {
    ...
    for (; day >= 0; --day) lut_saturated[day] = lut_saturated[day + 1];
  }
}
...
std::unique_ptr<cctz::ZoneInfoSource> custom_factory(
    const std::string& name, const std::function<std::unique_ptr<cctz::ZoneInfoSource>(const std::string& name)>& fallback) {
  std::string_view tz_file = getTimeZone(name.data());

  if (!tz_file.empty()) return std::make_unique<Source>(tz_file.data(), tz_file.size());
