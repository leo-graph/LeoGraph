...
class SensitiveDataMasker::MaskingRule {
 ...
 public:
  ...
  MaskingRule(const std::string& name_, const std::string& regexp_string_, const std::string& replacement_string_, bool throw_on_match_)
      : name(name_),
...
void SensitiveDataMasker::setInstance(std::unique_ptr<SensitiveDataMasker>&& sensitive_data_masker_) {
  if (!sensitive_data_masker_) throw Exception(ErrorCodes::LOGICAL_ERROR, "The 'sensitive_data_masker' is not set");
...
SensitiveDataMasker::MaskerMultiVersion::Version SensitiveDataMasker::getInstance() { return sensitive_data_masker.get(); }
...
SensitiveDataMasker::SensitiveDataMasker(const Poco::Util::AbstractConfiguration& config, const std::string& config_prefix) {
  ...
  for (const auto& rule : keys) {
    ...
    if (startsWith(rule, "rule")) {
      ...
      if (!used_names.insert(rule_name).second) {
        throw Exception(ErrorCodes::INVALID_CONFIG_PARAMETER, "query_masking_rules configuration contains more than one rule named '{}'.",
                        rule_name);
      }
  ...
  if (rules_count > 0) LOG_INFO(logger, "{} query masking rules loaded.", rules_count);
}

void SensitiveDataMasker::addMaskingRule(const std::string& name, const std::string& regexp_string, const std::string& replacement_string,
                                         bool throw_on_match) {
...
size_t SensitiveDataMasker::wipeSensitiveDataThrow(std::string& data) const {
  size_t matches = 0;
  for (const auto& rule : all_masking_rules) matches += rule->applyThrow(data);
  return matches;
}

size_t SensitiveDataMasker::wipeSensitiveData(std::string& data) const {
  size_t matches = 0;
  for (const auto& rule : all_masking_rules) matches += rule->applyNoThrow(data);

  if (matches) ProfileEvents::increment(ProfileEvents::QueryMaskingRulesMatch, matches);
...
size_t SensitiveDataMasker::rulesCount() const { return all_masking_rules.size(); }
...
std::string wipeSensitiveDataAndCutToLength(std::string str, size_t max_length, bool wipe_sensitive) {
  ...
  if (max_length && (length > max_length)) {
    ...
    if (max_length < max_extra_msg_len) return "(removed " + std::to_string(length) + " characters)";
