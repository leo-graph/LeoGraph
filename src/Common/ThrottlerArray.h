...
class ThrottlerArray : public IThrottler {
 public:
  ...
  explicit ThrottlerArray(ThrottlerArgs&&... args) : throttlers{std::forward<ThrottlerArgs>(args)...} {
  ...
  bool isThrottling() const override {
    for (const auto& throttler : throttlers) {
      if (throttler->isThrottling()) return true;
    }
    return false;
  }
  ...
  Int64 getAvailable() override {
    ...
    for (const auto& throttler : throttlers) min_available = std::min(min_available, throttler->getAvailable());
  ...
  UInt64 getMaxBurst() const override {
    ...
    for (const auto& throttler : throttlers) min_burst = std::min(min_burst, throttler->getMaxBurst());
    return min_burst;
  }
