 ...
 public:
  ...
  Int64 add(Int64 delta, Callback&& locked_callback, bool create_if_need = false) {
    ...
    if (file_doesnt_exists && !create_if_need) {
      throw Poco::Exception("File " + path +
                            " does not exist. "
    ...
    if (-1 == fd) DB::ErrnoException::throwFromPath(DB::ErrorCodes::CANNOT_OPEN_FILE, path, "Cannot open file {}", path);

    try {
      int flock_ret = flock(fd, LOCK_EX);
      if (-1 == flock_ret) DB::ErrnoException::throwFromPath(DB::ErrorCodes::CANNOT_OPEN_FILE, path, "Cannot lock file {}", path);
    ...
    try {
      ...
      if (file_exists) {
        ...
        try {
          ...
          if (e.code() != DB::ErrorCodes::CANNOT_READ_ALL_DATA && e.code() != DB::ErrorCodes::ATTEMPT_TO_READ_AFTER_EOF) throw;
        }
      }
