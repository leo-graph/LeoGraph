...
void ThreadPoolCallbackRunnerFast::initThreadPool(ThreadPool &pool_, size_t max_threads_, ThreadName thread_name_,
...
ThreadPoolCallbackRunnerFast::ThreadPoolCallbackRunnerFast(Mode mode_) : mode(mode_) { chassert(mode != Mode::ThreadPool); }

ThreadPoolCallbackRunnerFast::~ThreadPoolCallbackRunnerFast() { shutdown(); }
...
void ThreadPoolCallbackRunnerFast::shutdown() {
  ...
  if (mode == Mode::ThreadPool) chassert(active_tasks.load() == queue.size());
}
...
void ThreadPoolCallbackRunnerFast::operator()(std::function<void()> f) {
  if (mode == Mode::Disabled) throw Exception(ErrorCodes::LOGICAL_ERROR, "Thread pool runner is not initialized");
  ...
  if (mode == Mode::ThreadPool) {
    ...
    if (prev_size < max_threads) futexWake(&queue_size, 1);
#else
    queue_cv.notify_one();
#endif
  }
...
void ThreadPoolCallbackRunnerFast::bulkSchedule(std::vector<std::function<void()>> fs) {
  ...
  if (mode == Mode::ThreadPool) {
    ...
    if (prev_size < max_threads) futexWake(&queue_size, static_cast<int>(n));
#else
    if (n < 4)
      for (size_t i = 0; i < n; ++i) queue_cv.notify_one();
    else
      queue_cv.notify_all();
#endif
  }
...
void ThreadPoolCallbackRunnerFast::threadFunction() {
  ...
  while (true) {
    ...
    while (true) {
      ...
      } else if (queue_size.compare_exchange_weak(x, x - 1, std::memory_order_acquire, std::memory_order_relaxed))
        break;
    }
#endif
    ...
    std::function<void()> f;
    {
...
#else
      timed_out =
          !queue_cv.wait_for(lock, std::chrono::nanoseconds(THREAD_IDLE_TIMEOUT_NS), [&] { return shutdown_requested || !queue.empty(); });
#endif
      ...
      if (shutdown_requested || timed_out) {
        ...
        threads -= 1;
        if (threads == 0) shutdown_cv.notify_all();
...
bool ShutdownHelper::shutdown_requested() { return val.load(std::memory_order_relaxed) >= SHUTDOWN_START; }
...
bool ShutdownHelper::begin_shutdown() {
  ...
  if (already_called) n = val.fetch_sub(SHUTDOWN_START) - SHUTDOWN_START;
  if (n == SHUTDOWN_START) {
    val.fetch_add(SHUTDOWN_END);
    {
      std::unique_lock lock(mutex);
