...
StatusFile::FillFunction StatusFile::write_pid = [](WriteBuffer& out) { out << getpid(); };
...
StatusFile::StatusFile(std::string path_, FillFunction fill) : path(std::move(path_)) {
  ...
  if (-1 == fd) ErrnoException::throwFromPath(ErrorCodes::CANNOT_OPEN_FILE, path, "Cannot open file {}", path);

  try {
    int flock_ret = flock(fd, LOCK_EX | LOCK_NB);
    if (-1 == flock_ret) {
      if (errno == EWOULDBLOCK)
        throw Exception(ErrorCodes::CANNOT_OPEN_FILE, "Cannot lock file {}. Another server instance in same directory is already running.",
                        path);
      ErrnoException::throwFromPath(ErrorCodes::CANNOT_OPEN_FILE, path, "Cannot lock file {}", path);
    }

    if (0 != ftruncate(fd, 0)) ErrnoException::throwFromPath(ErrorCodes::CANNOT_TRUNCATE_FILE, path, "Cannot ftruncate file {}", path);
...
StatusFile::~StatusFile() {
  if (0 != close(fd)) LOG_ERROR(getLogger("StatusFile"), "Cannot close file {}, {}", path, errnoToString());

  if (0 != unlink(path.c_str())) LOG_ERROR(getLogger("StatusFile"), "Cannot unlink file {}, {}", path, errnoToString());
}
