...
struct OvercommitRatio {
  OvercommitRatio(Int64 committed_, Int64 soft_limit_) : committed(committed_), soft_limit(soft_limit_) {}
  ...
  friend bool operator<(OvercommitRatio const &lhs, OvercommitRatio const &rhs) noexcept {
    ...
    // (a / b < c / d) <=> (a * d < c * b)
    return (lhs_committed * rhs_soft_limit) < (rhs_committed * lhs_soft_limit) || (lhs_soft_limit == 0 && rhs_soft_limit > 0) ||
