...
SSHKey::SSHKey(const SSHKey& other) { key = ssh_key_dup(other.key); }
...
bool SSHKey::operator==(const SSHKey& other) const { return isEqual(other); }
...
bool SSHKey::verifySignature(std::string_view signature, std::string_view original) const {
  ...
  if (rc != SSH_OK) {
    if (verify_key != nullptr) ssh_key_free(verify_key);
    return false;
  }
...
bool SSHKey::isPrivate() const { return ssh_key_is_private(key); }

bool SSHKey::isPublic() const { return ssh_key_is_public(key); }
...
String SSHKey::getKeyType() const { return ssh_key_type_to_char(ssh_key_type(key)); }

void SSHKey::setNeedsDeallocation(bool needs_deallocation_) { needs_deallocation = needs_deallocation_; }

SSHKey::~SSHKey() {
  if (needs_deallocation) ssh_key_free(key);
}
