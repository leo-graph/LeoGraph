...
std::string_view getName(ErrorCode error_code) {
  if (error_code < 0 || error_code > END) return std::string_view();
...
size_t increment(ErrorCode error_code, bool remote, const std::string& message, const std::string& format_string,
...
void ErrorPairHolder::extendedMessage(bool remote, size_t error_index, const std::string& new_message) {
  ...
  if ((error.count == error_index + 1) && new_message.starts_with(error.message)) error.message = new_message;
}
