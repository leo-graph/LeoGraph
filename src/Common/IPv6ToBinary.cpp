...
static constexpr RawMaskArrayT generateBitMask(size_t prefix) {
  RawMaskArrayT arr{0};
  if (prefix >= arr.size() * 8) prefix = arr.size() * 8;
  size_t i = 0;
  for (; prefix >= 8; ++i, prefix -= 8) arr[i] = 0xff;
  if (prefix > 0) arr[i++] = static_cast<uint8_t>(~(0xff >> prefix));
  while (i < arr.size()) arr[i++] = 0x00;
  return arr;
}
...
static constexpr std::array<RawMaskArrayT, masksCount> generateBitMasks() {
  ...
  for (size_t i = 0; i < masksCount; ++i) arr[i] = generateBitMask<RawMaskArrayT>(i);
  return arr;
}
...
bool matchIPv6Subnet(const uint8_t *addr, const uint8_t *cidr_addr, UInt8 prefix) {
  uint16_t mask = static_cast<uint16_t>(_mm_movemask_epi8(_mm_cmpeq_epi8(_mm_loadu_si128(reinterpret_cast<const __m128i *>(addr)),
  ...
  if (mask) {
    ...
    if (prefix / 8 != offset) return prefix / 8 < offset;
...
bool matchIPv6Subnet(const uint8_t *addr, const uint8_t *cidr_addr, UInt8 prefix) {
  ...
  for (; prefix >= 8; ++i, prefix -= 8) {
    if (addr[i] != cidr_addr[i]) return false;
  }
