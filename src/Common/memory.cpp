...
#endif
  {
    if (mprotect(addr, len, PROT_NONE)) throw DB::ErrnoException(DB::ErrorCodes::SYSTEM_ERROR, "Cannot mprotect(PROT_NONE)");
  }
}
...
#ifdef OS_LINUX
  if (supports_guard_pages) {
    if (madvise(addr, len, MADV_GUARD_REMOVE)) throw DB::ErrnoException(DB::ErrorCodes::SYSTEM_ERROR, "Cannot madvise(MADV_GUARD_INSTALL)");
  } else
#endif
  {
    if (mprotect(addr, len, PROT_READ | PROT_WRITE)) throw DB::ErrnoException(DB::ErrorCodes::SYSTEM_ERROR, "Cannot mprotect(PROT_NONE)");
  }
}
