...
__attribute__((__noinline__)) void remapToHugeStep2(void *begin, size_t size, void *scratch) {
  ...
  if (-1 == mmap_res) syscall_func(SYS_exit, 1);
...
__attribute__((__noinline__)) void remapToHugeStep1(void *begin, size_t size) {
  ...
  void *scratch = mmap(nullptr, size, PROT_READ | PROT_WRITE | PROT_EXEC, MAP_PRIVATE | MAP_ANONYMOUS, -1,
                       0);  // NOLINT(clang-analyzer-security.MmapWriteExec)
  if (MAP_FAILED == scratch) throw ErrnoException(ErrorCodes::CANNOT_ALLOCATE_MEMORY, "Cannot mmap {} bytes", size);
