 ...
 * See
 * https://unix.stackexchange.com/questions/212894/whats-the-right-format-for-the-http-proxy-environment-variable-caps-or-no-ca/212972#212972
 * */
...
EnvironmentProxyConfigurationResolver::EnvironmentProxyConfigurationResolver(Protocol request_protocol_,
...
const char* getProxyHost(DB::ProxyConfiguration::Protocol protocol) {
   ...
   * aside from tests and a very early call during startup:
   * https://github.com/ClickHouse/ClickHouse/blob/master/src/Daemon/BaseDaemon.cpp#L791
   * */
  switch (protocol) {
...
ProxyConfiguration buildProxyConfiguration(ProxyConfiguration::Protocol request_protocol, const Poco::URI& uri,
  ...
  const bool use_tunneling_for_https_requests_over_http_proxy = ProxyConfiguration::useTunneling(
      request_protocol, ProxyConfiguration::protocolFromString(scheme), disable_tunneling_for_https_requests_over_http_proxy);
...
ProxyConfiguration EnvironmentProxyConfigurationResolver::resolve() {
  ...
  return buildProxyConfiguration(request_protocol, request_protocol == Protocol::HTTP ? http_proxy_uri : https_proxy_uri,
                                 poco_no_proxy_hosts, disable_tunneling_for_https_requests_over_http_proxy);
}
