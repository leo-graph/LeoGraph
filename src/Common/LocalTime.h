...
class LocalTime {
 private:
  ...
  void init(const char *s, size_t length) {
    if (length < 7) throw std::runtime_error("Cannot parse LocalTime: " + std::string(s, length));
    ...
    if (s_copy[3] == ':' && s[6] == ':')  /// Case 1
    {
      ...
      m_hour = 0;
      for (int i = 0; i < 3; ++i) {
        if (s_copy[i] < '0' || s_copy[i] > '9') throw std::runtime_error("Invalid hour digit in LocalTime: " + std::string(s, length));
    ...
    } else if (s_copy[1] == ':' && s_copy[4] == ':')  /// Case 3
    {
      ...
      if (s_copy[0] < '0' || s_copy[0] > '9') throw std::runtime_error("Invalid hour digit in LocalTime: " + std::string(s, length));
  ...
  inline int64_t totalSeconds() const {
    ...
    int64_t total = static_cast<int64_t>(m_hour) * 3600 + static_cast<int64_t>(m_minute) * 60 + static_cast<int64_t>(m_second);
 ...
 public:
   ...
   * @param time_zone Time zone information.
   */
  explicit LocalTime(time_t time, const DateLUTImpl &time_zone = DateLUT::instance()) { init(time, time_zone); }
  ...
  explicit LocalTime(const std::string &s) {
    if (s.size() < 9) throw std::runtime_error("Cannot parse LocalTime: " + s);
   ...
   */
  LocalTime(const char *data, size_t length) { init(data, length); }
  ...
  bool operator<(const LocalTime &other) const { return totalSeconds() < other.totalSeconds(); }
