...
std::vector<typename Container::const_iterator> pickWeightedRandom(const Container& data, GetWeightFunction get_weight, size_t count) {
  ...
  ::partial_sort(keys.begin(), keys.begin() + count, keys.end(), [](const auto& lhs, const auto& rhs) { return lhs.second > rhs.second; });

  std::vector<Iterator> selected;
  selected.reserve(count);

  for (size_t i = 0; i < count; ++i) selected.push_back(keys[i].first);
