...
class Fiber {
 private:
  ...
  template <typename T>
  friend class FiberLocal;

 public:
  template <typename StackAlloc, typename Fn>
  Fiber(StackAlloc &&salloc, Fn &&fn)
 ...
 private:
  ...
  struct RoutineImpl {
    struct SuspendCallback {
      ...
      void operator()() { impl = std::move(impl).resume(); }
    };
  ...
  DataPtr &getLocalData(void *key) { return local_data[key]; }

  Impl &&release() { return std::move(impl); }
