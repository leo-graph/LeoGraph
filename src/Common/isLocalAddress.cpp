...
struct NetworkInterfaces : public boost::noncopyable {
  ifaddrs *ifaddr;
  NetworkInterfaces() {
    if (getifaddrs(&ifaddr) == -1) throw ErrnoException(ErrorCodes::SYSTEM_ERROR, "Cannot getifaddrs");
  }
  ...
  bool hasAddress(const Poco::Net::IPAddress &address) const {
    ...
    for (iface = ifaddr; iface != nullptr; iface = iface->ifa_next) {
       ...
       */
      if (interface_address->length() == address.length() && 0 == memcmp(interface_address->addr(), address.addr(), address.length()))
        return true;
    }
    return false;
  }

  ~NetworkInterfaces() { freeifaddrs(ifaddr); }
};
...
bool isLocalAddress(const Poco::Net::IPAddress &address) {
  ...
  if (address.isLoopback()) {
    if (address.family() == Poco::Net::AddressFamily::IPv4) {
      ...
      if (digits[0] == 127 && digits[1] <= 1 && digits[2] <= 1 && digits[3] <= 1) {
        return true;
      }
...
size_t getHostNamePrefixDistance(const std::string &local_hostname, const std::string &host) {
  ...
  for (size_t i = 0; i < std::min(local_hostname.length(), host.length()); ++i)
    if (local_hostname[i] != host[i]) ++hostname_difference;
