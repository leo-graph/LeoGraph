...
int32_t readFrom(const std::filesystem::path& filename, int default_value) {
  ...
  if (!infile.is_open()) return default_value;
  int idata;
  if (infile >> idata) return idata;
...
uint32_t getCGroupLimitedCPUCores(unsigned default_cpu_count) {
  ...
  if (cgroupsV2Enabled()) {
    ...
    if (cgroup_path.empty()) return default_cpu_count;
...
unsigned physical_concurrency() try {
...
unsigned getNumberOfCPUCoresToUseImpl() {
  ...
  ///
  /// On really big machines, SMT is detrimental to performance (+ ~5% overhead in ClickBench). On such machines, we limit ourself to the
  /// physical cores. Few cores indicate it is a small machine, runs in a VM or is a limited cloud instance --> it is reasonable to use all
  /// the cores.
  if (cores >= 64) cores = physical_concurrency();
#endif
