...
KeyPair::KeyPair(EVP_PKEY *key_) : key(key_) {}

KeyPair::operator EVP_PKEY *() const { return key; }
...
KeyPair KeyPair::fromFile(const std::string &path, const std::string &password) {
  ...
  if (!file) throw Exception(ErrorCodes::OPENSSL_ERROR, "BIO_new_file failed: {}", getOpenSSLErrors());
...
KeyPair KeyPair::fromPEMString(const std::string_view &pem, const std::string &password) {
  ...
  if (!pem_bio) throw Exception(ErrorCodes::OPENSSL_ERROR, "BIO_new_mem_buf failed: {}", getOpenSSLErrors());
...
KeyPair KeyPair::fromBIO(BIO_ptr bio, const std::string &password) {
  ...
  if (!key) throw Exception(ErrorCodes::OPENSSL_ERROR, "Failed to load key: {}", getOpenSSLErrors());
...
KeyPair KeyPair::fromBuffer(const std::string &buffer, const std::string &password) {
  ...
  /// Try to load a private key.
  {
    ...
    if (!bio_buffer) throw Exception(ErrorCodes::OPENSSL_ERROR, "BIO_new_mem_buf failed: {}", getOpenSSLErrors());
  ...
  if (!key) {
    ...
    if (!bio_buffer) throw Exception(ErrorCodes::OPENSSL_ERROR, "BIO_new_file failed: {}", getOpenSSLErrors());

    key = PEM_read_bio_PUBKEY(bio_buffer.get(), nullptr, nullptr, nullptr);
  }

  if (!key) throw Exception(ErrorCodes::OPENSSL_ERROR, "Failed to load key from a file: {}", getOpenSSLErrors());
...
KeyPair KeyPair::generateRSA(uint32_t bits, uint32_t exponent) {
  ...
  if (!ctx) throw Exception(ErrorCodes::OPENSSL_ERROR, "EVP_PKEY_CTX_new_id failed: {}", getOpenSSLErrors());
  ...
  if (!bn || !BN_set_word(bn, exponent)) throw Exception(ErrorCodes::OPENSSL_ERROR, "BN_set_word failed: {}", getOpenSSLErrors());
...
std::string KeyPair::publicKey() const {
  ...
  if (!bio) throw Exception(ErrorCodes::OPENSSL_ERROR, "BIO_new failed: {}", getOpenSSLErrors());
