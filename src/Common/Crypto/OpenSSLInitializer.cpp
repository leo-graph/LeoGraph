...
OpenSSLInitializer::OpenSSLInitializer() { initialize(); }
...
#  endif
  ...
  if (!initialize_done) {
    ...
    if (!default_provider) throw std::runtime_error(fmt::format("Failed to load OpenSSL 'default' provider. {}", getOpenSSLErrors()));

    legacy_provider = OSSL_PROVIDER_load(nullptr, "legacy");
    if (!legacy_provider) throw std::runtime_error(fmt::format("Failed to load OpenSSL 'legacy' provider. {}", getOpenSSLErrors()));
  }
#endif
}
...
OpenSSLInitializer::~OpenSSLInitializer() { cleanup(); }
