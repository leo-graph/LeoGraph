...
X509Certificate::X509Certificate(X509 *cert_) : certificate(cert_) {}

X509Certificate::operator X509 *() const { return certificate; }
...
X509Certificate::X509Certificate(const std::string &path) {
  ...
  if (!bio) throw Exception(ErrorCodes::OPENSSL_ERROR, "BIO_new_file failed: {}", getOpenSSLErrors());

  certificate = PEM_read_bio_X509(bio.get(), nullptr, nullptr, nullptr);
  if (!certificate) throw Exception(ErrorCodes::OPENSSL_ERROR, "PEM_read_bio_X509 failed for file {}: {}", path, getOpenSSLErrors());
}
...
X509Certificate::List readCertificatesFromBIO(const BIO_ptr &bio, const std::string &source_description) {
  ...
  while (true) {
    ...
    if (!cert) {
      ...
      throw Exception(ErrorCodes::OPENSSL_ERROR, "PEM_read_bio_X509 failed: c:{}, f:{}, e:{}", certs.size(), source_description,
                      getOpenSSLErrors());
    }
  ...
  if (certs.empty()) throw Exception(ErrorCodes::OPENSSL_ERROR, "No certificates found in {}", source_description);
...
std::string X509Certificate::issuerName(uint nid) const {
  if (X509_NAME *issuer = X509_get_issuer_name(certificate)) {
    char buffer[X509Certificate::NAME_BUFFER_SIZE];
    if (X509_NAME_get_text_by_NID(issuer, nid, buffer, sizeof(buffer)) >= 0) return std::string(buffer);
  }
  return {};
}
...
std::string X509Certificate::subjectName(uint nid) const {
  if (X509_NAME *subj = X509_get_subject_name(certificate)) {
    char buffer[X509Certificate::NAME_BUFFER_SIZE];
    if (X509_NAME_get_text_by_NID(subj, nid, buffer, sizeof(buffer)) >= 0) return std::string(buffer);
  }
  return {};
}

std::string X509Certificate::commonName() const { return subjectName(NID_commonName); }
...
const X509Certificate::Subjects::container &X509Certificate::Subjects::at(Type type_) const { return subjects[static_cast<size_t>(type_)]; }

bool X509Certificate::Subjects::empty() {
  for (auto &subject_list : subjects) {
    if (!subject_list.empty()) return false;
  }
  return true;
}
...
void X509Certificate::Subjects::insert(Type type_, String &&subject) { subjects[static_cast<size_t>(type_)].insert(std::move(subject)); }

X509Certificate::Subjects::Type X509Certificate::Subjects::parseSubjectType(const String &type_) {
  if (type_ == "CN") return X509Certificate::Subjects::Type::CN;
  if (type_ == "SAN") return X509Certificate::Subjects::Type::SAN;
...
bool X509Certificate::Subjects::operator==(const X509Certificate::Subjects &rhs) const {
  for (X509Certificate::Subjects::Type type : {X509Certificate::Subjects::Type::CN, X509Certificate::Subjects::Type::SAN}) {
    if (this->at(type) != rhs.at(type)) return false;
  }
...
X509Certificate::Subjects X509Certificate::extractAllSubjects() {
  ...
  if (!commonName().empty()) subjects.insert(Subjects::Type::CN, commonName());
  ...
  GeneralNamesPtr cert_names(static_cast<STACK_OF(GENERAL_NAME) *>(X509_get_ext_d2i(certificate, NID_subject_alt_name, nullptr, nullptr)),
                             general_names_deleter);

  if (!cert_names) return subjects;
