...
PageCache::PageCache(std::chrono::milliseconds history_window_, const String &cache_policy, double size_ratio, size_t min_size_in_bytes_,
                     size_t max_size_in_bytes_, double free_memory_ratio_, size_t num_shards)
  ...
  for (size_t i = 0; i < num_shards; ++i) {
    shards.push_back(std::make_unique<Shard>(cache_policy, CurrentMetrics::PageCacheBytes, CurrentMetrics::PageCacheCells, bytes_per_shard,
                                             Base::NO_MAX_COUNT, size_ratio));
  }
}

PageCache::MappedPtr PageCache::getOrSet(const PageCacheKey &key, bool detached_if_missing, bool inject_eviction,
                                         std::function<void(const MappedPtr &)> load) {
  ...
  if (inject_eviction && thread_local_rng() % 10 == 0) shard.remove(key_hash);
...
PageCache::MappedPtr PageCache::get(const PageCacheKey &key, bool inject_eviction) {
  ...
  if (inject_eviction && thread_local_rng() % 10 == 0) return nullptr;
  ...
  if (result) ProfileEvents::increment(ProfileEvents::PageCacheHits);
...
bool PageCache::contains(const PageCacheKey &key, bool inject_eviction) const {
  ...
  if (inject_eviction && thread_local_rng() % 10 == 0) return false;
  Key key_hash = key.hash();
...
void PageCache::clear() {
  MemoryTrackerBlockerInThread blocker(VariableContext::Global);
  for (const auto &shard : shards) shard->clear();
}

size_t PageCache::sizeInBytes() const {
  MemoryTrackerBlockerInThread blocker(VariableContext::Global);
  size_t sum = 0;
  for (const auto &shard : shards) sum += shard->sizeInBytes();
  return sum;
}
...
size_t PageCache::count() const {
  MemoryTrackerBlockerInThread blocker(VariableContext::Global);
  size_t sum = 0;
  for (const auto &shard : shards) sum += shard->count();
  return sum;
}
...
size_t PageCache::maxSizeInBytes() const {
  MemoryTrackerBlockerInThread blocker(VariableContext::Global);
  size_t sum = 0;
  for (const auto &shard : shards) sum += shard->maxSizeInBytes();
  return sum;
}
...
PageCacheCell::PageCacheCell(PageCacheKey key_, bool temporary) : key(std::move(key_)), m_size(key.size), m_temporary(temporary) {
  ...
  if (!m_temporary) blocker.emplace();
