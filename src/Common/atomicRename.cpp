...
static std::optional<std::string> supportsAtomicRenameImpl() {
  ...
  if (linux_version >= renameat2_minimal_version) return std::nullopt;
...
static bool renameat2(const std::string &old_path, const std::string &new_path, int flags) {
  if (!supportsAtomicRename()) return false;
  ...
  /// unsigned int flags
  if (0 == syscall(__NR_renameat2, AT_FDCWD, old_path.c_str(), AT_FDCWD, new_path.c_str(), flags)) return true;
  ...
  if (errno == EEXIST)
    throw ErrnoException(ErrorCodes::ATOMIC_RENAME_FAIL, "Cannot rename {} to {} because the second path already exists", old_path,
                         new_path);
  if (errno == ENOENT)
    throw ErrnoException(ErrorCodes::ATOMIC_RENAME_FAIL, "Paths cannot be exchanged because {} or {} does not exist", old_path, new_path);
...
bool supportsAtomicRename(std::string *out_message) {
  ...
  if (out_message) *out_message = error.value();
  return false;
}
...
static bool renameat2(const std::string &old_path, const std::string &new_path, int flags) {
  ...
  if (0 == (*fun)(old_path.c_str(), new_path.c_str(), flags)) return true;
  int errnum = errno;

  if (errnum == ENOTSUP || errnum == EINVAL) return false;
  if (errnum == EEXIST)
    throw ErrnoException(ErrorCodes::ATOMIC_RENAME_FAIL, "Cannot rename {} to {} because the second path already exists", old_path,
                         new_path);
  if (errnum == ENOENT)
...
static std::optional<std::string> supportsAtomicRenameImpl() {
  ...
  if (fun != nullptr) return std::nullopt;
...
static bool renameat2(const std::string &, const std::string &, int) { return false; }

bool supportsAtomicRename(std::string *out_message) {
  if (out_message) *out_message = "only Linux and macOS are supported";
  return false;
}
...
static void renameNoReplaceFallback(const std::string &old_path, const std::string &new_path) {
  /// NOTE it's unsafe
  if (fs::exists(new_path)) throw Exception(ErrorCodes::FILE_ALREADY_EXISTS, "File {} exists", new_path);
...
void renameNoReplace(const std::string &old_path, const std::string &new_path) {
  if (!renameat2(old_path, new_path, RENAME_NOREPLACE)) renameNoReplaceFallback(old_path, new_path);
}

void renameExchange(const std::string &old_path, const std::string &new_path) {
  if (!renameat2(old_path, new_path, RENAME_EXCHANGE)) renameExchangeFallback(old_path, new_path);
}
