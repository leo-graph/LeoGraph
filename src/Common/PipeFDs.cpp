...
void LazyPipeFDs::open() {
  for (int& fd : fds_rw)
    if (fd >= 0) throw Exception(ErrorCodes::LOGICAL_ERROR, "Pipe is already opened");

#ifndef OS_DARWIN
  if (0 != pipe2(fds_rw, O_CLOEXEC)) throw ErrnoException(ErrorCodes::CANNOT_PIPE, "Cannot create pipe");
#else
  if (0 != pipe(fds_rw)) throw ErrnoException(ErrorCodes::CANNOT_PIPE, "Cannot create pipe");
...
void LazyPipeFDs::close() {
  fiu_do_on(FailPoints::lazy_pipe_fds_fail_close, { throw Exception(ErrorCodes::CANNOT_PIPE, "Manually triggered exception on close"); });

  for (int& fd : fds_rw) {
    if (fd < 0) continue;
    if (0 != ::close(fd)) throw ErrnoException(ErrorCodes::CANNOT_PIPE, "Cannot close pipe");
    fd = -1;
  }
}

PipeFDs::PipeFDs() { open(); }
...
void LazyPipeFDs::setNonBlockingWrite() {
  ...
  if (-1 == flags) throw ErrnoException(ErrorCodes::CANNOT_FCNTL, "Cannot get file status flags of pipe");
