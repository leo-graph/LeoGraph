...
static void formatReadable(double size, DB::WriteBuffer& out, int precision, const char** units, size_t units_size, double delimiter) {
  size_t i = 0;
  for (; i + 1 < units_size && fabs(size) >= delimiter; ++i) size /= delimiter;
  ...
  if (!result) result = converter.ToShortest(size, &builder);

  if (!result) throw DB::Exception(DB::ErrorCodes::CANNOT_PRINT_FLOAT_OR_DOUBLE_NUMBER, "Cannot print float or double number");
