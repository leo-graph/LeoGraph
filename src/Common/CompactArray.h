...
class CompactArray final {
 public:
  ...
  UInt8 ALWAYS_INLINE operator[](BucketIndex bucket_index) const {
    ...
    if (locus.index_l == locus.index_r) return locus.read(bitset[locus.index_l]);
...
class CompactArray<BucketIndex, content_width, bucket_count>::Reader final {
 public:
  explicit Reader(ReadBuffer &in_) : in(in_) {}
  ...
  std::pair<BucketIndex, UInt8> get() const {
    if ((current_bucket_index == 0) || is_eof) throw Exception(ErrorCodes::NO_AVAILABLE_DATA, "No available data.");

    if (fits_in_byte) return std::make_pair(current_bucket_index - 1, locus.read(value_l));
...
class CompactArray<BucketIndex, content_width, bucket_count>::Locus final {
 ...
 public:
  ...
  {
    if (content_l == content_r) return read(*content_l);
 ...
 private:
  ...
  UInt8 ALWAYS_INLINE read(UInt8 value_l, UInt8 value_r) const {
    /// The cell overlaps two bytes.
    return static_cast<UInt8>(((value_l >> offset_l) & ((1 << (8 - offset_l)) - 1)) |
