...
bool enoughSpaceInDirectory(const std::string &path, size_t data_size) {
  ...
  while (filepath.has_parent_path() && !fs::exists(filepath)) filepath = filepath.parent_path();
...
#if defined(OS_LINUX)
  struct stat sb;
  if (lstat(path.c_str(), &sb)) DB::ErrnoException::throwFromPath(DB::ErrorCodes::CANNOT_STAT, path, "Cannot lstat {}", path);
...
BlockDeviceType getBlockDeviceType([[maybe_unused]] const String &device_id) {
  ...
  try {
    ...
    if (!std::filesystem::exists(path)) return BlockDeviceType::UNKNOWN;
...
std::filesystem::path getMountPoint(std::filesystem::path absolute_path) {
  if (absolute_path.is_relative()) throw Exception(ErrorCodes::LOGICAL_ERROR, "Path is relative. It's a bug.");
  ...
  while (absolute_path.has_relative_path()) {
    ...
    if (device_id != parent_device_id) return absolute_path;
...
String getFilesystemName([[maybe_unused]] const String &mount_point) {
  ...
  if (!mounted_filesystems) throw DB::Exception(ErrorCodes::SYSTEM_ERROR, "Cannot open /etc/mtab to get name of filesystem");
  mntent fs_info;
...
size_t getSizeFromFileDescriptor(int fd, const String &file_name) {
  ...
  if (-1 == res) {
    DB::ErrnoException::throwFromPath(DB::ErrorCodes::CANNOT_FSTAT, file_name, "Cannot execute fstat{}",
                                      file_name.empty() ? "" : " file: " + file_name);
  }
  return buf.st_size;
}
...
std::optional<size_t> tryGetSizeFromFilePath(const String &path) {
  ...
  if (ec == std::errc::operation_not_supported) return std::nullopt;
...
bool exists(const std::string &path) { return faccessat(AT_FDCWD, path.c_str(), F_OK, AT_EACCESS) == 0; }
...
time_t getChangeTime(const std::string &path) {
  struct stat st;
  if (stat(path.c_str(), &st) == 0) return st.st_ctime;
  std::error_code m_ec(errno, std::generic_category());
  throw fs::filesystem_error("Cannot check change time for file", path, m_ec);
}

Poco::Timestamp getModificationTimestamp(const std::string &path) { return Poco::Timestamp::fromEpochTime(getModificationTime(path)); }
...
bool isSymlink(const fs::path &path) {
  ...
  if (path.filename().empty()) return fs::is_symlink(path.parent_path());  /// STYLE_CHECK_ALLOW_STD_FS_SYMLINK
  return fs::is_symlink(path);                                             /// STYLE_CHECK_ALLOW_STD_FS_SYMLINK
}

bool isSymlinkNoThrow(const fs::path &path) {
  std::error_code dummy;
  if (path.filename().empty()) return fs::is_symlink(path.parent_path(), dummy);  /// STYLE_CHECK_ALLOW_STD_FS_SYMLINK
...
fs::path readSymlink(const fs::path &path) {
  ...
  if (path.filename().empty()) return fs::read_symlink(path.parent_path());  /// STYLE_CHECK_ALLOW_STD_FS_SYMLINK
