...
std::string_view toString(ThreadName name) {
  switch (name) {
    case ThreadName::UNKNOWN:
      return as_string_view("Unknown");

#define THREAD_NAME_ACTION(NAME, STR) \
  case ThreadName::NAME:              \
    return as_string_view(STR);
      THREAD_NAME_VALUES(THREAD_NAME_ACTION)
#undef THREAD_NAME_ACTION
  }
}
...
void setThreadName(ThreadName name) {
  ...
  if (thread_name_view.empty()) throw DB::Exception(DB::ErrorCodes::LOGICAL_ERROR, "Thread name cannot be empty");
...
ThreadName getThreadName() {
  if (thread_name != ThreadName::UNKNOWN) return thread_name;
