...
bool CurrentThread::isInitialized() { return current_thread; }
...
std::shared_ptr<InternalTextLogsQueue> CurrentThread::getInternalTextLogsQueue() {
  ...
  if (unlikely(!current_thread)) return nullptr;
...
void CurrentThread::attachInternalTextLogsQueue(const std::shared_ptr<InternalTextLogsQueue>& logs_queue, LogsLevel client_logs_level) {
  if (unlikely(!current_thread)) return;
...
MemoryTracker* CurrentThread::getUserMemoryTracker() {
  ...
  while (tracker && tracker->level != VariableContext::User) tracker = tracker->getParent();
