...
class ProfilingScopedWriteRWLock {
 public:
  ProfilingScopedWriteRWLock(SharedMutex& rwl_, ProfileEvents::Event event) : scoped_write_lock(rwl_) {
...
class ProfilingScopedReadRWLock {
 public:
  ProfilingScopedReadRWLock(SharedMutex& rwl, ProfileEvents::Event event) : scoped_read_lock(rwl) {
    ProfileEvents::increment(event, watch.elapsed());
  }
