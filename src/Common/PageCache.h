...
struct PageCacheWeightFunction {
  size_t operator()(const PageCacheCell &x) const { return x.capacity(); }
};
...
class PageCache {
 ...
 public:
  ...
  PageCache(std::chrono::milliseconds history_window_, const String &cache_policy, double size_ratio, size_t min_size_in_bytes_,
