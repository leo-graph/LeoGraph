...
struct X509ExtensionStackDeleter {
  void operator()(X509_EXTENSIONS *ptr) const {
    OPENSSL_sk_pop_free(reinterpret_cast<OPENSSL_STACK *>(ptr), &X509ExtensionStackDeleter::freeX509Extension);
  }

  static void freeX509Extension(void *ptr) { X509_EXTENSION_free(static_cast<X509_EXTENSION *>(ptr)); }
};
