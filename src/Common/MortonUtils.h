...
void intervalBinaryPartition(UInt64 first, UInt64 last, F&& callback) {
  ...
  if (split < last) intervalBinaryPartition(split + 1, last, callback);
}
...
void hyperrectangleBinaryPartitionImpl(std::array<std::pair<UInt64, UInt64>, N> hyperrectangle, F&& callback) {
  intervalBinaryPartition(hyperrectangle[start_idx].first, hyperrectangle[start_idx].second, [&](UInt64 a, UInt64 b) mutable {
...
void hyperrectangleBinaryPartition(std::array<std::pair<UInt64, UInt64>, N> hyperrectangle, F&& callback) {
 ...
 */
template <size_t N, typename F>
void hyperrectangleToPossibleMortonIntervals(std::array<std::pair<UInt64, UInt64>, N> hyperrectangle, F&& callback) {
  ...
  hyperrectangleBinaryPartition<N>(hyperrectangle, [&](auto part) {
    ...
    for (size_t i = 0; i < N; ++i)
      if (part[i].second != part[i].first) suffix_size = std::max<size_t>(suffix_size, 1 + bitScanReverse(part[i].second ^ part[i].first));
    ...
    while (result_bit_idx < 64) {
      for (size_t i = 0; i < N; ++i) {
        ...
        ++result_bit_idx;
        if (!(result_bit_idx < 64)) break;
      }
      ++source_bit_idx;
    }

    if (found_intervals.insert({first, last}).second) callback(first, last);
  });
}
