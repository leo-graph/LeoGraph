...
#  if defined(OS_FREEBSD) || defined(OS_SUNOS)
  ...
  if (0 != pthread_attr_get_np(pthread_self(), &attr)) throw ErrnoException(ErrorCodes::CANNOT_PTHREAD_ATTR, "Cannot pthread_attr_get_np");
#  else
...
#  ifdef USE_MUSL
  ...
  if (static_cast<pid_t>(getThreadId()) == getpid()) {
    ::rlimit rlimit{};
    if (::getrlimit(RLIMIT_STACK, &rlimit)) return 0;
...
__attribute__((__weak__)) void checkStackSize() {
  ...
  if (Fiber::getCurrentFiber()) return;

  if (!stack_address) max_stack_size = getStackSize(&stack_address);

  /// The check is impossible.
  if (!max_stack_size) return;
  ...
  if (stack_size > max_stack_size_allowed) {
    throw Exception(ErrorCodes::TOO_DEEP_RECURSION,
                    "Stack size too large. Stack address: {}, frame address: {}, stack size: {}, maximum stack size: {}", stack_address,
                    frame_address, stack_size, max_stack_size);
  }
}
