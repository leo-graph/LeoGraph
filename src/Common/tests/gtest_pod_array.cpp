...
TEST(Common, PODArrayBasicMove) {
  ...
  using Array = PODArray<UInt64, initial_bytes, AllocatorWithStackMemory<Allocator<false>, initial_bytes>>;
...
TEST(Common, PODArrayInsert) {
  ...
  }
  {
    ...
    for (size_t i = 0; i < 120; ++i) values.emplace_back(i);
    ...
    for (size_t i = 0; i < 240; ++i) values_to_insert.emplace_back(i);
...
TEST(Common, PODArrayInsertFromItself) {
  {
    ...
    for (size_t i = 0; i < 3; ++i) array.insertFromItself(array.begin(), array.end());
...
TEST(Common, PODInsertElementSizeNotMultipleOfLeftPadding) {
  ...
  for (size_t test = 0; test < items_to_insert_size; ++test) arr1_initially_empty.emplace_back();

  EXPECT_EQ(arr1_initially_empty.size(), items_to_insert_size);

  PaddedPODArray<ItemWith24Size> arr2_initially_nonempty;

  for (size_t test = 0; test < items_to_insert_size; ++test) arr2_initially_nonempty.emplace_back();
...
TEST(Common, PODErase) {
  ...
  }
  {
    ...
    for (size_t i = 0; i < 10; ++i) actual.emplace_back(static_cast<UInt64>(i));
