...
class Events {
 public:
  ...
  void add(String&& event, std::chrono::milliseconds correction = std::chrono::milliseconds::zero()) {
    String timepoint =
        std::to_string(std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::steady_clock::now() - start_time).count());
    if (timepoint.length() < 5) timepoint.insert(0, 5 - timepoint.length(), ' ');
    if (correction.count()) std::this_thread::sleep_for(correction);
  ...
  void check(const Strings& expected_events) {
    ...
    for (size_t i = 0; i != events.size(); ++i) EXPECT_EQ(events[i], (i < expected_events.size() ? expected_events[i] : ""));
  }
...
TEST(Common, RWLock1) {
  ...
  for (auto pool_size : pool_sizes) {
    for (int round = 0; round < 10; ++round) {
      ...
      for (size_t thread = 0; thread < pool_size; ++thread) threads.emplace_back([=]() { func(pool_size, round); });
...
TEST(Common, RWLockDeadlock) {
  ...
  std::thread t1([&]() {
    ...
    try {
      ...
      if (!holder2) {
        throw Exception(ErrorCodes::DEADLOCK_AVOIDED, "Locking attempt timed out! Possible deadlock avoided. Client should retry.");
      }
    } catch (const Exception& e) {
      if (e.code() != ErrorCodes::DEADLOCK_AVOIDED) throw;
    }
  });
...
TEST(Common, RWLockPerfTestReaders) {
  ...
  for (auto pool_size : pool_sizes) {
    ...
    std::list<std::thread> threads;
    for (size_t thread = 0; thread < pool_size; ++thread) threads.emplace_back(func);
...
TEST(Common, RWLockWriteLockTimeoutDuringRead) {
  ...
  events.check({"Locking ra", "Locked ra", "Locking wc", "Failed to lock wc", "Unlocking ra", "Unlocked ra", "Locking wd", "Locked wd",
                "Unlocking wd", "Unlocked wd"});
}
...
TEST(Common, RWLockWriteLockTimeoutDuringTwoReads) {
  ...
  events.check({"Locking ra", "Locked ra", "Locking wc", "Locking rb", "Failed to lock wc", "Locked rb", "Unlocking ra", "Unlocked ra",
                "Unlocking rb", "Unlocked rb", "Locking wd", "Locked wd", "Unlocking wd", "Unlocked wd"});
}
...
TEST(Common, RWLockWriteLockTimeoutDuringWriteWithWaitingRead) {
  ...
  events.check({"Locking wa", "Locked wa", "Locking wb", "Locking rc", "Failed to lock wb", "Failed to lock rc", "Unlocking wa",
                "Unlocked wa", "Locking wd", "Locked wd", "Unlocking wd", "Unlocked wd"});
}
