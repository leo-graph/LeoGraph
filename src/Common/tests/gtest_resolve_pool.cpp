...
void sleep_until(auto time_point) { std::this_thread::sleep_until(time_point); }

void sleep_for(auto duration) { std::this_thread::sleep_for(duration); }

size_t toMilliseconds(auto duration) { return std::chrono::duration_cast<std::chrono::milliseconds>(duration).count(); }
...
class ResolvePoolTest : public testing::Test {
 protected:
  ResolvePoolTest() { DB::HostResolversPool::instance().dropCache(); }
...
TEST_F(ResolvePoolTest, BannedForConsiquenceFail) {
  ...
  if (now() > start_at + 2 * history - epsilon) return;
