...
struct AsyncLoaderTest {
  ...
  explicit AsyncLoaderTest(size_t max_threads = 1) : AsyncLoaderTest({{.max_threads = max_threads, .priority = {}}}) {}
  ...
  std::vector<AsyncLoader::PoolInitializer> getPoolInitializers(std::vector<Initializer> initializers) {
    ...
    for (auto &desc : initializers) {
      result.push_back({.name = fmt::format("Pool{}", pool_id),
  ...
  void randomSleepUs(UInt64 min_us, UInt64 max_us, int probability_percent) {
    if (randomInt(0, 99) < probability_percent) std::this_thread::sleep_for(std::chrono::microseconds(randomInt(min_us, max_us)));
  }
  ...
  LoadJobSet randomJobSet(int job_count, int dep_probability_percent, JobFunc job_func, std::string_view name_prefix = "job") {
    ...
    for (int j = 0; j < job_count; j++) {
      LoadJobSet deps;
      for (int d = 0; d < j; d++) {
        if (randomInt(0, 99) < dep_probability_percent) deps.insert(jobs[d]);
      }
  ...
  template <typename JobFunc>
  LoadJobSet randomJobSet(int job_count, int dep_probability_percent, const std::vector<LoadJobPtr> &external_deps, JobFunc job_func,
                          std::string_view name_prefix = "job") {
    std::vector<LoadJobPtr> jobs;
    jobs.reserve(job_count);
    for (int j = 0; j < job_count; j++) {
      LoadJobSet deps;
  ...
  LoadJobSet chainJobSet(int job_count, JobFunc job_func, std::string_view name_prefix = "job") {
    ...
    for (int j = 1; j < job_count; j++) jobs.push_back(makeLoadJob({jobs[j - 1]}, fmt::format("{}{}", name_prefix, j), job_func));
    return {jobs.begin(), jobs.end()};
  }
...
TEST(AsyncLoader, Smoke) {
  ...
  auto job_func = [&](AsyncLoader &, const LoadJobPtr &self) {
    jobs_done++;
    if (self->pool() == low_priority_pool) low_priority_jobs_done++;
  };
...
TEST(AsyncLoader, CycleDetection) {
  ...
  try {
    ...
    for (int i = 0; i < std::size(present); i++) ASSERT_EQ(e.message().find(fmt::format("job{}", i)) != String::npos, present[i]);
  }
...
TEST(AsyncLoader, CancelExecutingJob) {
  ...
  while (job->waitersCount() == 0) std::this_thread::yield();
...
TEST(AsyncLoader, CancelExecutingTask) {
  ...
  for (int iteration = 0; iteration < 10; iteration++) {
    ...
    for (int i = 0; i < 100; i++) task1_jobs.push_back(makeLoadJob({blocker_job}, "job_to_cancel", job_to_cancel_func));
    ...
    while (blocker_job->waitersCount() == 0) std::this_thread::yield();
    ...
    for (const auto &job : task1_jobs) {
      if (job != blocker_job) ASSERT_EQ(job->status(), LoadStatus::CANCELED);
    }
  }
}
...
TEST(AsyncLoader, JobFailure) {
  ...
  auto job_func = [&](AsyncLoader &, const LoadJobPtr &) { throw std::runtime_error(error_message); };
...
TEST(AsyncLoader, ScheduleJobWithFailedDependencies) {
  ...
  auto failed_job_func = [&](AsyncLoader &, const LoadJobPtr &) { throw Exception(ErrorCodes::ASYNC_LOAD_FAILED, "{}", error_message); };
...
TEST(AsyncLoader, IgnoreDependencyFailure) {
  ...
  auto failed_job_func = [&](AsyncLoader &, const LoadJobPtr &) { throw Exception(ErrorCodes::ASYNC_LOAD_FAILED, "{}", error_message); };
  auto dependent_job_func = [&](AsyncLoader &, const LoadJobPtr &) { success.store(true); };

  auto failed_job = makeLoadJob({}, "failed_job", failed_job_func);
  auto dependent_job = makeLoadJob({failed_job}, "dependent_job", ignoreDependencyFailure, dependent_job_func);
...
TEST(AsyncLoader, CustomDependencyFailure) {
  ...
  auto evil_dep_func = [&](AsyncLoader &, const LoadJobPtr &) { throw Exception(ErrorCodes::ASYNC_LOAD_FAILED, "{}", error_message); };
  auto good_dep_func = [&](AsyncLoader &, const LoadJobPtr &) { good_count++; };
  ...
  auto fail_after_two = [&](const LoadJobPtr &self, const LoadJobPtr &, std::exception_ptr &cancel) {
    if (++error_count == 2)
      cancel = std::make_exception_ptr(
          Exception(ErrorCodes::ASYNC_LOAD_CANCELED, "Load job '{}' canceled: too many dependencies have failed", self->name));
  };
  ...
  auto collect_job = makeLoadJob({evil_dep1, evil_dep2, evil_dep3, good_dep1, good_dep2, good_dep3, late_dep1, late_dep2, late_dep3},
...
TEST(AsyncLoader, WaitersLimit) {
  ...
  auto waiters_inc = [&](const LoadJobPtr &) {
    ...
    while (true) {
      if (value >= waiters_limit) throw Exception(ErrorCodes::ASYNC_LOAD_FAILED, "Too many waiters: {}", value);
      if (waiters_total.compare_exchange_strong(value, value + 1)) break;
    }
  };
  auto waiters_dec = [&](const LoadJobPtr &) { waiters_total.fetch_sub(1); };
  ...
  for (int i = 0; i < 10; i++) waiters.emplace_back(waiter);

  while (failure.load() != 5) std::this_thread::yield();
...
TEST(AsyncLoader, TestConcurrency) {
  ...
  for (int concurrency = 1; concurrency <= 10; concurrency++) {
    ...
    for (int i = 0; i < concurrency; i++) tasks.push_back(t.schedule(t.chainJobSet(5, job_func)));
    t.loader.wait();
...
TEST(AsyncLoader, StaticPriorities) {
  ...
  auto job_func = [&](AsyncLoader &, const LoadJobPtr &self) { schedule += fmt::format("{}{}", self->name, self->pool()); };
...
TEST(AsyncLoader, SimplePrioritization) {
  ...
  auto job_func_B_tester = [&](AsyncLoader &, const LoadJobPtr &) { ASSERT_EQ(executed++, 2); };

  auto job_func_C_boosted = [&](AsyncLoader &, const LoadJobPtr &) { ASSERT_EQ(executed++, 1); };
    ...
    auto job_func = [&](AsyncLoader &, const LoadJobPtr &self) {
      ...
      if (prioritize && self->name == "C") {
        for (const auto &state : t.loader.getJobStates()) {
          if (state.job->name == "D") ready_seqno_D = state.ready_seqno;
          if (state.job->name == "E") ready_seqno_E = state.ready_seqno;
        }
...
TEST(AsyncLoader, JobPrioritizedWhileWaited) {
  ...
  auto running_job_func = [&](AsyncLoader &, const LoadJobPtr &) { sync.arrive_and_wait(); };

  auto dependent_job_func = [&](AsyncLoader &, const LoadJobPtr &) {};

  auto waiting_job_func = [&](AsyncLoader &loader, const LoadJobPtr &) { loader.wait(job_to_wait); };
  ...
  while (job_to_wait->waitersCount() == 0) std::this_thread::yield();
...
TEST(AsyncLoader, RandomIndependentTasks) {
  ...
  auto job_func = [&](AsyncLoader &, const LoadJobPtr &self) {
    for (const auto &dep : self->dependencies) ASSERT_EQ(dep->status(), LoadStatus::OK);
...
TEST(AsyncLoader, RandomDependentTasks) {
  ...
  while (tasks_left-- > 0) {
    ...
    if (tasks.size() > 100) tasks.erase(tasks.begin() + t.randomInt<size_t>(0, tasks.size() - 1));
  }
...
TEST(AsyncLoader, SetMaxThreads) {
  ...
  for (int max_threads : max_threads_values) syncs.push_back(std::make_unique<std::barrier<>>(max_threads + 1));
  ...
  std::vector<LoadTaskPtr> tasks;
  tasks.reserve(1000);
  for (int i = 0; i < 1000; i++) tasks.push_back(t.schedule({makeLoadJob({}, "job", job_func)}));
  ...
  while (sync_index < syncs.size()) {
    ...
    syncs[idx]->arrive_and_wait();  // (A)
    sync_index++;
    if (sync_index < syncs.size()) t.loader.setMaxThreads(/* pool = */ 0, max_threads_values[sync_index]);
    syncs[idx]->arrive_and_wait();  // (B) this sync point is required to allow `executing` value to go back down to zero after we change
                                    // number of workers
  }
  t.loader.wait();
}
...
TEST(AsyncLoader, SubJobs) {
  ...
  class MyComponent : boost::noncopyable {
   public:
    MyComponent(AsyncLoader &loader_, int jobs) : loader(loader_), jobs_left(jobs) {}

    [[nodiscard]] LoadTaskPtr loadAsync() {
      auto job_func = [this](AsyncLoader &, const LoadJobPtr &) {
        auto sub_job_func = [this](AsyncLoader &, const LoadJobPtr &) { --jobs_left; };
        LoadJobSet jobs;
        for (size_t j = 0; j < jobs_left; j++) jobs.insert(makeLoadJob({}, fmt::format("sub job {}", j), sub_job_func));
    ...
    bool isLoaded() const { return jobs_left == 0; }
  ...
  for (double jobs_per_thread : std::array{0.5, 1.0, 2.0}) {
    for (size_t threads = 1; threads <= 32; threads *= 2) {
      ...
      waitLoad(tasks);
      for (const auto &component : components) ASSERT_TRUE(component.isLoaded());
    }
  }
}
...
TEST(AsyncLoader, RecursiveJob) {
  ...
  class MyComponent : boost::noncopyable {
   public:
    ...
    [[nodiscard]] LoadTaskPtr loadAsync() { return load_task = loadAsyncImpl(jobs_left); }

    bool isLoaded() const { return jobs_left == 0; }

   private:
    [[nodiscard]] LoadTaskPtr loadAsyncImpl(int id) {
      auto job_func = [this](AsyncLoader &, const LoadJobPtr &self) { jobFunction(self); };
    ...
    void jobFunction(const LoadJobPtr &self) {
      int next = --jobs_left;
      if (next > 0) waitLoad(self->pool(), loadAsyncImpl(next));
    }
