...
GTEST_TEST(WideInteger, Conversions) {
  ...
  ASSERT_EQ(Float64(UInt256(1) * 1000000000 * 1000000000 * 1000000000 * 1000000000 * 1000000000 * 1000000000 * 1000000000 * 1000000000),
            1e72);
...
static T divide(T& numerator, T&& denominator) {
  if (!denominator) throwError("Division by zero");
...
GTEST_TEST(WideInteger, FromDouble) {
  ...
  if (normalized_exponent >= 128) {
    ...
    if (normalized_exponent > 0) res += Int128(1) << normalized_exponent;
  }
