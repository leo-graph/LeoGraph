...
TEST(RemoteProxyConfigurationResolver, HTTPOverHTTP) {
  ...
  auto remote_server_configuration =
      RemoteProxyConfigurationResolver::RemoteServerConfiguration{Poco::URI("not_important"), "http", 80, std::chrono::seconds{10}};

  RemoteProxyConfigurationResolver resolver(remote_server_configuration, ProxyConfiguration::Protocol::HTTP, "",
...
TEST(RemoteProxyConfigurationResolver, HTTPSOverHTTPS) {
  const char *proxy_server_mock = "proxy1";
  auto remote_server_configuration =
      RemoteProxyConfigurationResolver::RemoteServerConfiguration{Poco::URI("not_important"), "https", 443, std::chrono::seconds{10}};

  RemoteProxyConfigurationResolver resolver(remote_server_configuration, ProxyConfiguration::Protocol::HTTPS, "",
                                            std::make_shared<RemoteProxyHostFetcherMock>(proxy_server_mock));
...
TEST(RemoteProxyConfigurationResolver, SimpleCacheTest) {
  ...
  auto cache_ttl = 5u;
  auto remote_server_configuration =
      RemoteProxyConfigurationResolver::RemoteServerConfiguration{Poco::URI("not_important"), "http", 80, std::chrono::seconds{cache_ttl}};

  auto fetcher_mock = std::make_shared<RemoteProxyHostFetcherMock>(proxy_server_mock);

  RemoteProxyConfigurationResolver resolver(remote_server_configuration, ProxyConfiguration::Protocol::HTTP, "", fetcher_mock);
