...
TEST(SLRUCache, set) {
  ...
  auto slru_cache = SimpleCacheBase("SLRU", CurrentMetrics::MarkCacheBytes, CurrentMetrics::MarkCacheFiles, /*max_size_in_bytes=*/10,
                                    /*max_count=*/0, /*size_ratio*/ 0.5);
...
TEST(SLRUCache, update) {
  using SimpleCacheBase = DB::CacheBase<int, int>;
  auto slru_cache =
      SimpleCacheBase("SLRU", CurrentMetrics::end(), CurrentMetrics::end(), /*max_size_in_bytes=*/10, /*max_count=*/0, /*size_ratio*/ 0.5);
  slru_cache.set(1, std::make_shared<int>(2));
...
TEST(SLRUCache, removeFromProtected) {
  using SimpleCacheBase = DB::CacheBase<int, int>;
  auto slru_cache =
      SimpleCacheBase("SLRU", CurrentMetrics::end(), CurrentMetrics::end(), /*max_size_in_bytes=*/2, /*max_count=*/0, /*size_ratio*/ 0.5);
  slru_cache.set(1, std::make_shared<int>(2));
  slru_cache.set(1, std::make_shared<int>(3));
...
TEST(SLRUCache, evictOnElements) {
  ...
  auto slru_cache = SimpleCacheBase(CurrentMetrics::MarkCacheBytes, CurrentMetrics::MarkCacheFiles, /*max_size_in_bytes=*/10,
                                    /*max_count=*/1, /*size_ratio*/ 0.5);
...
TEST(SLRUCache, MaxCount) {
  ...
  for (size_t max_count = 1; max_count < 1024; max_count *= 2) {
    ...
    for (int i = 0; i < 10; ++i) {
      ...
      ASSERT_NE(value, nullptr) << "max_count = " << max_count << ", i = " << i;
      ASSERT_EQ(*value, 5) << "max_count = " << max_count << ", i = " << i;
      ASSERT_EQ(slru_cache.count(), std::min(static_cast<size_t>(i + 1), max_count)) << "max_count = " << max_count << ", i = " << i;
    }
  }
}
