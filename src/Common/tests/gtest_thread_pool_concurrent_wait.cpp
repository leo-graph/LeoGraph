...
TEST(ThreadPool, ConcurrentWait) {
  auto worker = [] {
    for (size_t i = 0; i < 100000000; ++i) __asm__ volatile("nop");
  };
  ...
  for (size_t i = 0; i < num_jobs; ++i) pool.scheduleOrThrowOnError(worker);
  ...
  for (size_t i = 0; i < num_waiting_threads; ++i) waiting_pool.scheduleOrThrowOnError([&pool] { pool.wait(); });
