...
struct OvercommitTrackerForTest : BaseTracker {
  ...
  explicit OvercommitTrackerForTest(Ts&&... args) : BaseTracker(std::move(args)...) {}

  void setCandidate(MemoryTracker* candidate) { tracker = candidate; }

 protected:
  void pickQueryToExcludeImpl() override { BaseTracker::picked_tracker = tracker; }
...
void free_not_continue_test(T& overcommit_tracker) {
  ...
  for (auto& tracker : trackers) tracker.setOvercommitWaitingTime(WAIT_TIME);
  ...
  for (size_t i = 0; i < THREADS; ++i) {
    threads.push_back(std::thread([&, i]() {
      if (overcommit_tracker.needToStopQuery(&trackers[i], 100) != OvercommitResult::MEMORY_FREED) ++need_to_stop;
    }));
  }
...
void query_stop_not_continue_test(T& overcommit_tracker) {
  ...
  auto thread = std::thread([&]() {
    if (overcommit_tracker.needToStopQuery(&another, 100) != OvercommitResult::MEMORY_FREED) ++need_to_stop;
  });
  std::this_thread::sleep_for(1000ms);
