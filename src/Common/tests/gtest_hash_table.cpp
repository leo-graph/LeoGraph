...
std::set<std::string> convertToSet(const HashTable& table) {
  ...
  for (auto v : table) result.emplace(toString(v.getValue()));
...
TEST_P(TwoLevelHashSetFixture, WriteAsSingleLevel) {
...

  {
    ...
    for (const auto& elem : hash_set_content) two_level.insert(elem);
    ...
    for (const auto& elem : hash_set_content) EXPECT_NE(single_level.find(elem), nullptr);
  }
}

INSTANTIATE_TEST_SUITE_P(TwoLevelHashSetTests, TwoLevelHashSetFixture,
                         ::testing::Values(HashSetContent{}, getVectorWithNumbersUpToN(1), getVectorWithNumbersUpToN(100),
                                           getVectorWithNumbersUpToN(1000), getVectorWithNumbersUpToN(10000),
                                           getVectorWithNumbersUpToN(100000), getVectorWithNumbersUpToN(1000000)));
...
TEST_P(TwoLevelHashSetMergingFixture, MergeWithSingleLevelSet) {
  ...
  TwoLevelHashSet<Key, HashCRC32<Key>> two_level;
  for (size_t elem = 1; elem < set_size + 1; ++elem) two_level.insert(elem);
  if (two_level_has_zero) two_level.insert(0);

  HashSet<Key, HashCRC32<Key>> single_level;
  for (size_t elem = 42; elem < set_size + 42; ++elem) single_level.insert(elem);
  if (single_level_has_zero) single_level.insert(0);
  ...
  for (size_t elem = 1; elem < set_size + 1; ++elem) ASSERT_NE(two_level.find(elem), nullptr);

  for (size_t elem = 42; elem < set_size + 42; ++elem) ASSERT_NE(two_level.find(elem), nullptr);
}

INSTANTIATE_TEST_SUITE_P(TwoLevelHashSetMergingTests, TwoLevelHashSetMergingFixture,
                         ::testing::Values(std::make_tuple(0, false, false), std::make_tuple(0, false, true),
                                           std::make_tuple(0, true, false), std::make_tuple(0, true, true),

                                           std::make_tuple(1, false, false), std::make_tuple(1, false, true),
                                           std::make_tuple(1, true, false), std::make_tuple(1, true, true),

                                           std::make_tuple(10, false, false), std::make_tuple(10, false, true),
                                           std::make_tuple(10, true, false), std::make_tuple(10, true, true)));
