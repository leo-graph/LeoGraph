...
cctz::civil_day YYYYMMDDToDay(unsigned value) {
  return cctz::civil_day(value / 10000,          // year
...
std::vector<const char*> allTimezones(bool with_weird_offsets = true) {
  ...
  while (*timezone_name) {
    ...
    if (!weird_offsets || with_weird_offsets) result.push_back(*timezone_name);
    ++timezone_name;
  }
...
TEST_P(DateLUTWithTimeZone, VaidateTimeComponentsAroundEpoch) {
  ...
  for (time_t i = -856147870; i < 86400 * 10000; i += 11 * 13 * 17 * 19) {
    ...
    if (current_failures.total >= max_failures_per_tz) break;
  }
}
...
INSTANTIATE_TEST_SUITE_P(ExoticTimezones, DateLUTWithTimeZone,
...
INSTANTIATE_TEST_SUITE_P(AllTimeZones, DateLUTWithTimeZone, ::testing::ValuesIn(allTimezones()));
...
TEST_P(DateLUTWithTimeZoneAndTimeRange, InRange) {
  ...
  for (time_t expected_time_t = start; expected_time_t < stop; expected_time_t += step) {
    ...
    if (timestamp_current_day_pre % 900 || timestamp_current_day_post % 900) continue;
    ...
    /// To large transition.
    if (has_transition && std::abs(transition.from - transition.to) > 3600 * 3) {
      /*std::cerr << "Skipping " << timezone_name << " " << tz_time
          << " because of unsupported timezone transition from " << transition.from << " to " << transition.to
          << " (it is too large)\n";*/
      continue;
    ...
    if (countFailures(*test_info->result()).total >= max_failures_per_case) break;
  }
}

INSTANTIATE_TEST_SUITE_P(AllTimezones_Year2010, DateLUTWithTimeZoneAndTimeRange,
                         ::testing::Combine(::testing::ValuesIn(allTimezones()),
...
INSTANTIATE_TEST_SUITE_P(AllTimezones_Year1970_WHOLE, DateLUTWithTimeZoneAndTimeRange,
                         ::testing::Combine(::testing::ValuesIn(allTimezones(false)),
                                            ::testing::ValuesIn(std::initializer_list<TimeRangeParam>{
                                                // Values from tests/date_lut3.cpp
                                                {YYYYMMDDToDay(19700101), YYYYMMDDToDay(19701231), 10 * 3191 /*53m 11s*/},
                                            })));

INSTANTIATE_TEST_SUITE_P(AllTimezones_Year2010_WHOLE, DateLUTWithTimeZoneAndTimeRange,
                         ::testing::Combine(::testing::ValuesIn(allTimezones(false)),
                                            ::testing::ValuesIn(std::initializer_list<TimeRangeParam>{
                                                // Values from tests/date_lut3.cpp
...
INSTANTIATE_TEST_SUITE_P(AllTimezones_Year2020_WHOLE, DateLUTWithTimeZoneAndTimeRange,
                         ::testing::Combine(::testing::ValuesIn(allTimezones()),
                                            ::testing::ValuesIn(std::initializer_list<TimeRangeParam>{
                                                // Values from tests/date_lut3.cpp
...
INSTANTIATE_TEST_SUITE_P(AllTimezones_PreEpoch, DateLUTWithTimeZoneAndTimeRange,
                         ::testing::Combine(::testing::ValuesIn(allTimezones(false)),
                                            ::testing::ValuesIn(std::initializer_list<TimeRangeParam>{
...
INSTANTIATE_TEST_SUITE_P(
    AllTimezones_Year1970, DateLUTWithTimeZoneAndTimeRange,
    ::testing::Combine(::testing::ValuesIn(allTimezones(false)),
                       ::testing::ValuesIn(std::initializer_list<TimeRangeParam>{
