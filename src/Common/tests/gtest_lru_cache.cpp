...
TEST(LRUCache, set) {
  ...
  auto lru_cache = SimpleCacheBase("LRU", CurrentMetrics::MarkCacheBytes, CurrentMetrics::MarkCacheFiles, /*max_size_in_bytes*/ 10,
                                   /*max_count*/ 10, /*size_ratio*/ 0.5);
...
TEST(LRUCache, update) {
  using SimpleCacheBase = DB::CacheBase<int, int>;
  auto lru_cache =
      SimpleCacheBase("LRU", CurrentMetrics::end(), CurrentMetrics::end(), /*max_size_in_bytes*/ 10, /*max_count*/ 10, /*size_ratio*/ 0.5);
  lru_cache.set(1, std::make_shared<int>(2));
...
TEST(LRUCache, evictOnSize) {
  ...
  auto lru_cache =
      SimpleCacheBase("LRU", CurrentMetrics::end(), CurrentMetrics::end(), /*max_size_in_bytes*/ 20, /*max_count*/ 3, /*size_ratio*/ 0.5);
