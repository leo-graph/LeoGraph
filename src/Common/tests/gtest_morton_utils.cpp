...
GTEST_TEST(MortonUtils, Intervals) {
  {
    ...
    intervalBinaryPartition(6, 13, [&](UInt64 first, UInt64 last) { res << first << ", " << last << "; "; });
    ASSERT_EQ(res.str(), "6, 7; 8, 11; 12, 13; ");
  }

  {
    std::stringstream res;
    intervalBinaryPartition(15, 31, [&](UInt64 first, UInt64 last) { res << first << ", " << last << "; "; });
    ASSERT_EQ(res.str(), "15, 15; 16, 31; ");
  }

  {
    std::stringstream res;
    intervalBinaryPartition(15, 16, [&](UInt64 first, UInt64 last) { res << first << ", " << last << "; "; });
    ASSERT_EQ(res.str(), "15, 15; 16, 16; ");
  }

  {
    std::stringstream res;
    intervalBinaryPartition(191, 769, [&](UInt64 first, UInt64 last) { res << first << ", " << last << "; "; });
...

  {
    ...
    hyperrectangleBinaryPartition<2>(input, [&](auto hyperrectangle) {
      res << "[" << hyperrectangle[0].first << ", " << hyperrectangle[0].second << "] x [" << hyperrectangle[1].first << ", "
          << hyperrectangle[1].second << "]; ";
    });

    ASSERT_EQ(res.str(),
              "[6, 7] x [15, 15]; [6, 7] x [16, 31]; [8, 11] x [15, 15]; [8, 11] x [16, 31]; [12, 13] x [15, 15]; [12, 13] x [16, 31]; ");
  }
...

  {
    ...
    ASSERT_EQ(
        res.str(),
        "[30, 31] x [12, 13]; [28, 31] x [14, 15]; [0, 7] x [16, 23]; [8, 11] x [16, 19]; [12, 15] x [16, 17]; [12, 12] x [18, 18]; ");
  }

  {
    ...
    std::stringstream res;
    hyperrectangleToPossibleMortonIntervals<2>(input, [&](UInt64 first, UInt64 last) { res << first << ", " << last << "; "; });
