...
TEST(ThreadPool, GlobalFull1) {
  ...
  for (size_t i = 0; i < capacity; ++i) pool.scheduleOrThrowOnError(func);
...
TEST(ThreadPool, GlobalFull2) {
  ...
  ThreadPool pool(CurrentMetrics::LocalThread, CurrentMetrics::LocalThreadActive, CurrentMetrics::LocalThreadScheduled, capacity, 0,
                  capacity);
  for (size_t i = 0; i < capacity; ++i) pool.scheduleOrThrowOnError(func);
  ...
  for (size_t i = 0; i < capacity; ++i) another_pool.scheduleOrThrowOnError([&] { ++counter; });
