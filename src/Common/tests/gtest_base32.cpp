...
TYPED_TEST(BaseCoderTest, EncodeDecode) {
  ...
  for (const auto &test : test_data_t::encode_tests) {
    ...
    size_t const encoded_result_len = coder_t::encodeBase32(reinterpret_cast<const UInt8 *>(test.input.data()), input_len, encode_output);
...
TYPED_TEST(BaseCoderTest, LargeTest) {
  ...
  EXPECT_EQ(0, memcmp(large_input.data(), decoded.data(), large_size)) << " for input (as hex) " << to_hex(large_input.data(), large_size);
}
