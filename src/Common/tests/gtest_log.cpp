...
class AlwaysFailingChannel : public Poco::StreamChannel {
 public:
  explicit AlwaysFailingChannel(std::ostream &str) : Poco::StreamChannel(str) {}
...
std::vector<std::string> getLogFileNames(const std::string &log_dir) {
  ...
  for (const auto &entry : std::filesystem::directory_iterator(log_dir)) log_file_names.emplace_back(entry.path().filename());
...
std::string readLogFile(const std::string &path, bool skip_comments = true) {
  ...
  while (input) {
    ...
    if (!line.empty() && (!skip_comments || !line.starts_with("#"))) file_contents.append(line).append("\n");
  }
