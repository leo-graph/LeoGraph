...
class MockRequestHandler : public Poco::Net::HTTPRequestHandler {
 public:
  explicit MockRequestHandler(SafeHandler<RequestOptions>::Ptr options_) : options(options_) {}
  ...
  void handleRequest(Poco::Net::HTTPServerRequest &request, Poco::Net::HTTPServerResponse &response) override {
    ...
    if (params.slowdown_receive > 0) sleepForSeconds(params.slowdown_receive);
...
class HTTPRequestHandlerFactory : public Poco::Net::HTTPRequestHandlerFactory {
 public:
  explicit HTTPRequestHandlerFactory(SafeHandler<RequestOptions>::Ptr options_) : options(options_) {}
...
class ConnectionPoolTest : public testing::Test {
 protected:
  ...
  std::string getServerUrl() const { return "http://" + server_data.server->socket().address().toString(); }

  void startServer() {
    server_data.reset();
    server_data.handler_factory = new HTTPRequestHandlerFactory(options);
    server_data.server = std::make_unique<Poco::Net::HTTPServer>(server_data.handler_factory, server_data.port);

    server_data.server->start();
  }

  Poco::Net::HTTPServer &getServer() const { return *server_data.server; }
...
void wait_until(std::function<bool()> pred) {
  while (!pred()) sleepForMilliseconds(10);
}
...
TEST_F(ConnectionPoolTest, ReceiveTimeout) {
...

  {
    ...
    auto connection = pool->getConnection(timeouts, nullptr);
    ASSERT_NO_THROW(echoRequest("Hello", *connection););
  }
...
TEST_F(ConnectionPoolTest, ReadWriteBufferFromHTTP) {
  ...
  auto uri = Poco::URI(getServerUrl());
  auto metrics =
      DB::HTTPConnectionPools::instance().getPool(DB::HTTPConnectionGroupType::HTTP, uri, DB::ProxyConfiguration{})->getMetrics();

  Poco::Net::HTTPBasicCredentials empty_creds;
  auto buf_from_http = DB::BuilderRWBufferFromHTTP(uri)
                           .withBypassProxy(true)
                           .withConnectionGroup(DB::HTTPConnectionGroupType::HTTP)
                           .withOutCallback([&](std::ostream &in) { in << message; })
                           .withDelayInit(false)
                           .create(empty_creds);
