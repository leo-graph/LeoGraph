...
std::set<IntervalType> intervalSetToSet(const IntervalSet<IntervalType>& interval_set) {
  ...
  for (const auto& interval : interval_set) result.insert(interval);
...
std::map<IntervalType, Value> intervalMapToMap(const IntervalMap<IntervalType, Value>& interval_map) {
  ...
  for (const auto& [interval, value] : interval_map) result.emplace(interval, value);
...
struct CollectIntervalsSetCallback {
  explicit CollectIntervalsSetCallback(std::set<IntervalType>& result_intervals_) : result_intervals(result_intervals_) {}
...
template <typename IntervalType>
std::set<IntervalType> intervalSetFindIntervals(const IntervalSet<IntervalType>& interval_set,
                                                typename IntervalType::IntervalStorageType point) {
  std::set<IntervalType> result;
...
struct CollectIntervalsMapCallback {
  explicit CollectIntervalsMapCallback(std::map<IntervalType, Value>& result_intervals_) : result_intervals(result_intervals_) {}
...
template <typename IntervalType, typename Value>
std::map<IntervalType, Value> intervalMapFindIntervals(const IntervalMap<IntervalType, Value>& interval_set,
                                                       typename IntervalType::IntervalStorageType point) {
  std::map<IntervalType, Value> result;
...
TEST(IntervalTree, IntervalSetIterators) {
  ...
  }
  {
    for (size_t intervals_size = 0; intervals_size < 120; ++intervals_size) {
      ...
      if (end != begin) actual.insert(*end);
