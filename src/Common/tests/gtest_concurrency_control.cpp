...
struct ConcurrencyControlTest {
  ...
  explicit ConcurrencyControlTest(SlotCount limit = UnlimitedSlots) { cc.setMaxConcurrency(limit); }
};
...
TEST(ConcurrencyControl, Unlimited) {
  ...
  while (auto slot = slots->tryAcquire()) acquired.emplace_back(std::move(slot));
...
TEST(ConcurrencyControl, Fifo) {
  ...
  for (int i = 0; i < count; i++) allocations.emplace_back(t.cc.allocate(0, 1));
...
TEST(ConcurrencyControl, Oversubscription) {
  ...
  for (int i = 0; i < 10; i++) allocations.emplace_back(t.cc.allocate(1, 2));
...
TEST(ConcurrencyControl, ReleaseUnacquiredSlots) {
  ...
  std::vector<AcquiredSlotPtr> acquired;
  while (auto slot = allocation->tryAcquire()) acquired.emplace_back(std::move(slot));
...
TEST(ConcurrencyControl, MultipleThreads) {
  ...
  auto run_query = [&](size_t max_threads) {
    ...
    for (size_t thread_num = 0;; thread_num++) {
      std::unique_lock lock{threads_mutex};
      if (thread_num >= threads.size()) break;
...
TEST(ConcurrencyControl, MaxMinFairPrioritizesMinimumAllocation) {
  ...
  while (auto slot = a1->tryAcquire()) a1_slots.emplace_back(std::move(slot));
  ...
  while (auto slot = a2->tryAcquire()) a2_slots.emplace_back(std::move(slot));
  while (auto slot = a3->tryAcquire()) a3_slots.emplace_back(std::move(slot));
