  ...
  void calculate(size_t k) {
    ...
    do {
      ...
      do {
        ...
        for (size_t i : subset) probability[i] += p;
...
std::unordered_map<size_t, size_t> calculateSelects(size_t k) {
  ...
  for (size_t iter = 0; iter < N; ++iter)
    for (auto it : pickWeightedRandom(objects, get_weight, k)) selects_count[it->first] += 1;
...
GTEST_TEST(WeightedRandomSampling, AES1) { ProbabilityValidator(1).validate(calculateSelects(1)); }

GTEST_TEST(WeightedRandomSampling, AES2) { ProbabilityValidator(2).validate(calculateSelects(2)); }

GTEST_TEST(WeightedRandomSampling, AES3) { ProbabilityValidator(3).validate(calculateSelects(3)); }

GTEST_TEST(WeightedRandomSampling, AES4) { ProbabilityValidator(4).validate(calculateSelects(4)); }

GTEST_TEST(WeightedRandomSampling, AES5) { ProbabilityValidator(5).validate(calculateSelects(5)); }
