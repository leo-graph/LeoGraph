...
class CollectionOfDerivedItems {
 ...
 private:
  struct Rec {
    ...
    bool operator<(const Rec& other) const { return type_idx < other.type_idx; }

    bool operator<(const std::type_index& value) const { return type_idx < value; }
 ...
 public:
  void swap(Self& other) noexcept { records.swap(other.records); }
  ...
  Self clone() const {
    ...
    for (const auto& rec : records) result.records.emplace_back(rec.type_idx, rec.ptr->clone());
    return result;
  }
  ...
  std::shared_ptr<T> get() const {
    static_assert(std::is_base_of_v<ItemBase, T>, "Template parameter must inherit items base class");
    auto it = getImpl(std::type_index(typeid(T)));
    if (it == records.cend()) return nullptr;
  ...
  std::shared_ptr<T> getSafe() const {
    static_assert(std::is_base_of_v<ItemBase, T>, "Template parameter must inherit items base class");
    auto it = getImpl(std::type_index(typeid(T)));
    if (it == records.cend()) throw Exception(ErrorCodes::LOGICAL_ERROR, "Item of type {} is not found in collection", typeid(T).name());
    auto cast = std::dynamic_pointer_cast<T>(it->ptr);
    chassert(cast);
    return cast;
  }
 ...
 private:
  ...
  typename Records::const_iterator getImpl(std::type_index type_idx) const {
    ...
    if (it == records.cend()) return records.cend();

    if (it->type_idx != type_idx) return records.cend();
