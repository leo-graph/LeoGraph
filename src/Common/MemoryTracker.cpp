...
void AllocationTrace::onAllocImpl(void *ptr, size_t size) const {
  if (sample_probability < 1 && !shouldTrackAllocation(sample_probability, ptr)) return;
...
MemoryTracker::~MemoryTracker() {
  ...
  if (this == &total_memory_tracker) DB::MainThreadStatus::reset();
...
void MemoryTracker::logPeakMemoryUsage() {
  ...
  if (peak_bytes < 128 * 1024) return;
  LOG_DEBUG(getLogger("MemoryTracker"), "{}{} memory usage: {}.", description ? std::string(description) : "",
            description ? " peak" : "Peak", ReadableSize(peak_bytes));
}

void MemoryTracker::logMemoryUsage(Int64 current) const {
  const auto *description = description_ptr.load(std::memory_order_relaxed);
  LOG_DEBUG(getLogger("MemoryTracker"), "{}{} memory usage: {}.", description ? std::string(description) : "",
            description ? " current" : "Current", ReadableSize(current));
}

void MemoryTracker::injectFault() const {
  if (!memoryTrackerCanThrow(level, true)) {
    LOG_WARNING(getLogger("MemoryTracker"), "Cannot inject fault at specific point. Uncaught exceptions: {}, stack trace:\n{}",
  ...
  const auto *description = description_ptr.load(std::memory_order_relaxed);
  throw DB::Exception(DB::ErrorCodes::MEMORY_LIMIT_EXCEEDED, "{}{}: fault injected (at specific point)", description ? description : "",
...
void incrementAllocationWithoutCheck(Int64 size) {
  ...
  if (MemoryTrackerDebugBlockerInThread::isBlocked()) return;
  ...
  if (size > threshold) {
    ...
    try {
      DB::TraceSender::send(DB::TraceType::MemoryAllocatedWithoutCheck, StackTrace(),
                            DB::TraceSender::Extras{
                                .size = size,
                                .memory_blocked_context = memory_blocked_context,
                            });
...
AllocationTrace MemoryTracker::allocImpl(Int64 size, bool throw_if_memory_exceeded, MemoryTracker *query_tracker,
  ...
  if (size < 0) throw DB::Exception(DB::ErrorCodes::LOGICAL_ERROR, "Negative size ({}) is passed to MemoryTracker. It is a bug.", size);

  if (_sample_probability < 0) _sample_probability = sample_probability;

  if (!isSizeOkForSampling(size)) _sample_probability = 0;
  ...
  if (MemoryTrackerBlockerInThread::isBlocked(level)) {
    if (level == VariableContext::Global) {
      ...
      if (metric_loaded != CurrentMetrics::end()) CurrentMetrics::add(metric_loaded, size);
    }
  ...
  if (unlikely(current_fault_probability > 0.0 && fault(thread_local_rng))) {
    if (memoryTrackerCanThrow(level, true) && throw_if_memory_exceeded) {
      ...
      const auto *description = description_ptr.load(std::memory_order_relaxed);
      throw DB::Exception(
          DB::ErrorCodes::MEMORY_LIMIT_EXCEEDED, "{}{}: fault injected. Would use {} (attempt to allocate chunk of {}), maximum: {}",
          description ? description : "", description ? " memory tracker" : "Memory tracker", formatReadableSizeWithBinarySuffix(will_be),
          formatReadableSizeWithBinarySuffix(size), formatReadableSizeWithBinarySuffix(current_hard_limit));
    }
  ...
  if (unlikely(current_hard_limit &&
               (will_be > current_hard_limit || (level == VariableContext::Global && will_be_rss > current_hard_limit)))) {
#if USE_JEMALLOC
    if (level == VariableContext::Global && jemalloc_flush_profile_on_memory_exceeded) {
      ...
      if (DB::Jemalloc::getValue<bool>("prof.active")) {
        ...
        if (!flush_prefix) {
          if (throw_if_memory_exceeded) LOG_WARNING(getLogger("MemoryTracker"), "Cannot flush memory profile, empty prefix");
    ...
    if (memoryTrackerCanThrow(level, false) && throw_if_memory_exceeded) {
      ...
      if (overcommit_result == OvercommitResult::NONE && ((overcommit_tracker_ptr = overcommit_tracker.load(std::memory_order_relaxed))) &&
          query_tracker != nullptr)
      ...
      if (overcommit_result != OvercommitResult::MEMORY_FREED) {
        bool overcommit_result_ignore = overcommit_result == OvercommitResult::NONE || overcommit_result == OvercommitResult::DISABLED;
        /// Revert
        amount.fetch_sub(size, std::memory_order_relaxed);
        rss.fetch_sub(size, std::memory_order_relaxed);
        ...
        throw DB::Exception(DB::ErrorCodes::MEMORY_LIMIT_EXCEEDED,
                            ...
                            "{}{}",
                            description ? description : "", description ? " memory limit" : "Memory limit",
                            formatReadableSizeWithBinarySuffix(will_be), formatReadableSizeWithBinarySuffix(size),
                            (level == VariableContext::Global)
                                ? fmt::format(", current RSS: {}", formatReadableSizeWithBinarySuffix(rss.load(std::memory_order_relaxed)))
                                : "",
                            (level == VariableContext::Global && page_cache_ptr)
                                ? ", userspace page cache " + formatReadableSizeWithBinarySuffix(page_cache_ptr->sizeInBytes())
                                : "",
  ...
  auto metric_loaded = metric.load(std::memory_order_relaxed);
  if (metric_loaded != CurrentMetrics::end() && size) CurrentMetrics::add(metric_loaded, size);
...
void MemoryTracker::adjustOnBackgroundTaskEnd(const MemoryTracker *child) {
  ...
  auto metric_loaded = metric.load(std::memory_order_relaxed);
  if (metric_loaded != CurrentMetrics::end()) CurrentMetrics::sub(metric_loaded, background_memory_consumption);
}
...
#  endif
      ...
      if (DB::Jemalloc::getValue<bool>("prof.active")) {
        ...
        auto *flush_prefix = DB::Jemalloc::getValue<char *>("opt.prof_prefix");
        if (!flush_prefix) {
          if (log_memory_usage) LOG_WARNING(getLogger("MemoryTracker"), "Cannot flush memory profile, empty prefix");
        } else {
          ...
          if (static_cast<uint64_t>(will_be) > (previous_peak + jemalloc_flush_profile_interval_bytes) &&
            ...
            auto flushed_profile = DB::Jemalloc::flushProfile(flush_prefix);
            if (log_memory_usage) {
              LOG_INFO(getLogger("MemoryTracker"), "Flushed memory profile to {} after updating peak to {}", flushed_profile,
...
AllocationTrace MemoryTracker::free(Int64 size, double _sample_probability) {
  ...
  if (MemoryTrackerBlockerInThread::isBlocked(level)) {
    if (level == VariableContext::Global) {
      ...
      rss.fetch_sub(size, std::memory_order_relaxed);
      auto metric_loaded = metric.load(std::memory_order_relaxed);
      if (metric_loaded != CurrentMetrics::end()) CurrentMetrics::sub(metric_loaded, size);
    }

    /// Since the MemoryTrackerBlockerInThread should respect the level, we should go to the next parent.
    if (auto *loaded_next = parent.load(std::memory_order_relaxed)) return loaded_next->free(size, _sample_probability);
  ...
  auto metric_loaded = metric.load(std::memory_order_relaxed);
  if (metric_loaded != CurrentMetrics::end()) CurrentMetrics::sub(metric_loaded, accounted_size);
...
OvercommitRatio MemoryTracker::getOvercommitRatio(Int64 limit) { return {amount.load(std::memory_order_relaxed), limit}; }

void MemoryTracker::setOvercommitWaitingTime(UInt64 wait_time) { max_wait_time.store(wait_time * 1us, std::memory_order_relaxed); }
...
void MemoryTracker::updateRSS(Int64 rss_) { total_memory_tracker.rss.store(rss_, std::memory_order_relaxed); }

void MemoryTracker::updateAllocated(Int64 allocated_, bool log_change) {
  Int64 new_amount = allocated_;
  if (log_change)
    LOG_INFO(getLogger("MemoryTracker"), "Correcting the value of global memory tracker from {} to {}",
             ReadableSize(total_memory_tracker.amount.load(std::memory_order_relaxed)), ReadableSize(allocated_));
  ...
  if (metric_loaded != CurrentMetrics::end()) CurrentMetrics::set(metric_loaded, new_amount);

  bool log_memory_usage = true;
  total_memory_tracker.updatePeak(new_amount, log_memory_usage);
}

void MemoryTracker::setSoftLimit(Int64 value) { soft_limit.store(value, std::memory_order_relaxed); }
...
double MemoryTracker::getSampleProbability(UInt64 size) {
  if (sample_probability >= 0) {
    if (!isSizeOkForSampling(size)) return 0;
    return sample_probability;
  }

  if (auto *loaded_next = parent.load(std::memory_order_relaxed)) return loaded_next->getSampleProbability(size);
...
void MemoryTracker::setParent(MemoryTracker *elem) {
  ...
  if (level == VariableContext::Thread && DB::current_thread) DB::current_thread->flushUntrackedMemory();
  ...
  do {
    ...
    if (next == &total_memory_tracker) found_total_memory_tracker = true;
  } while ((next = next->parent.load(std::memory_order_relaxed)));
  if (!found_total_memory_tracker) throw DB::Exception(DB::ErrorCodes::LOGICAL_ERROR, "Cannot found total MemoryTracker. Bug in nesting.");
#endif
}
