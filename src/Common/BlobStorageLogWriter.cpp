...
void BlobStorageLogWriter::addEvent(BlobStorageLogElement::EventType event_type, const String& bucket, const String& remote_path,
                                    const String& local_path_, size_t data_size, size_t elapsed_microseconds, Int32 error_code,
                                    const String& error_message, BlobStorageLogElement::EvenTime time_now) {
  if (!log) {
  ...
  if (!time_now.time_since_epoch().count()) time_now = std::chrono::system_clock::now();
...
BlobStorageLogWriterPtr BlobStorageLogWriter::create(const String& disk_name) {
  if (auto blob_storage_log = Context::getGlobalContextInstance()->getBlobStorageLog()) {
    ...
    if (CurrentThread::isInitialized() && CurrentThread::get().getQueryContext()) log_writer->query_id = CurrentThread::getQueryId();
