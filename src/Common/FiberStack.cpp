...
FiberStack::FiberStack(size_t stack_size_) : stack_size(stack_size_), page_size(getPageSize()) {}
...
boost::context::stack_context FiberStack::allocate() const {
  ...
  if (!data) throw DB::ErrnoException(DB::ErrorCodes::CANNOT_ALLOCATE_MEMORY, "Cannot allocate FiberStack");
...
void FiberStack::deallocate(boost::context::stack_context& sctx) const {
  ...
  if constexpr (guardPagesEnabled()) memoryGuardRemove(data, page_size);
