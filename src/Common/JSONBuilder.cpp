...
static bool isArrayOrMap(const IItem &item) { return typeid_cast<const JSONArray *>(&item) || typeid_cast<const JSONMap *>(&item); }

static bool isSimpleArray(const std::vector<ItemPtr> &values) {
  for (const auto &value : values)
    if (isArrayOrMap(*value)) return false;

  return true;
}

void JSONString::format(const FormatSettings &settings, FormatContext &context) { writeJSONString(value, context.out, settings.settings); }

void JSONBool::format(const FormatSettings &, FormatContext &context) { writeString(value ? "true" : "false", context.out); }
...
void JSONArray::format(const FormatSettings &settings, FormatContext &context) {
  ...
  for (const auto &value : values) {
    if (!first) writeChar(',', context.out);
...
void JSONMap::format(const FormatSettings &settings, FormatContext &context) {
  ...
  for (const auto &value : values) {
    ...
    writeChar(':', context.out);
    if (!settings.solid) writeChar(' ', context.out);
...
void JSONNull::format(const FormatSettings &, FormatContext &context) { writeString("null", context.out); }
