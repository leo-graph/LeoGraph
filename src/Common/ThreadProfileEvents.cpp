...
bool TasksStatsCounters::checkIfAvailable() { return findBestAvailableProvider() != MetricsProvider::None; }

std::unique_ptr<TasksStatsCounters> TasksStatsCounters::create(UInt64 tid) {
  std::unique_ptr<TasksStatsCounters> instance;
  if (checkIfAvailable()) instance.reset(new TasksStatsCounters(tid, findBestAvailableProvider()));
  return instance;
}
...
TasksStatsCounters::MetricsProvider TasksStatsCounters::findBestAvailableProvider() {
  ...
  static std::optional<MetricsProvider> provider = []() -> MetricsProvider {
...
void TasksStatsCounters::incrementProfileEvents(const ::taskstats &prev, const ::taskstats &curr, ProfileEvents::Counters &profile_events) {
  profile_events.increment(ProfileEvents::OSCPUWaitMicroseconds, safeDiff(prev.cpu_delay_total, curr.cpu_delay_total) / 1000U);
  profile_events.increment(ProfileEvents::OSIOWaitMicroseconds, safeDiff(prev.blkio_delay_total, curr.blkio_delay_total) / 1000U);
...
#  define SOFTWARE_EVENT(PERF_NAME, LOCAL_NAME)                                                                                \
    PerfEventInfo {                                                                                                            \
      .event_type = perf_type_id::PERF_TYPE_SOFTWARE, .event_config = (PERF_NAME), .profile_event = ProfileEvents::LOCAL_NAME, \
      .settings_name = #LOCAL_NAME                                                                                             \
    }

#  define HARDWARE_EVENT(PERF_NAME, LOCAL_NAME)                                                                                \
    PerfEventInfo {                                                                                                            \
      .event_type = perf_type_id::PERF_TYPE_HARDWARE, .event_config = (PERF_NAME), .profile_event = ProfileEvents::LOCAL_NAME, \
      .settings_name = #LOCAL_NAME                                                                                             \
    }
...
#  define CACHE_EVENT(PERF_NAME, LOCAL_NAME, TYPE)                                                                  \
    PerfEventInfo {                                                                                                 \
      ...
      .event_config = (PERF_NAME) | (PERF_COUNT_HW_CACHE_OP_READ << 8) | (PERF_COUNT_HW_CACHE_RESULT_##TYPE << 16), \
      .profile_event = ProfileEvents::LOCAL_NAME, .settings_name = #LOCAL_NAME                                      \
    }
...
static void enablePerfEvent(int event_fd) {
  if (ioctl(event_fd, PERF_EVENT_IOC_ENABLE, 0)) {
    LOG_WARNING(getLogger("PerfEvents"), "Can't enable perf event with file descriptor {}: '{}' ({})", event_fd, errnoToString(), errno);
  }
}

static void disablePerfEvent(int event_fd) {
  if (ioctl(event_fd, PERF_EVENT_IOC_DISABLE, 0)) {
    LOG_WARNING(getLogger("PerfEvents"), "Can't disable perf event with file descriptor {}: '{}' ({})", event_fd, errnoToString(), errno);
  }
}

static void releasePerfEvent(int event_fd) {
  if (close(event_fd)) {
    LOG_WARNING(getLogger("PerfEvents"), "Can't close perf event file descriptor {}: {} ({})", event_fd, errnoToString(), errno);
  }
}

static bool validatePerfEventDescriptor(int &fd) {
  if (fcntl(fd, F_GETFL) != -1) return true;

  if (errno == EBADF) {
    LOG_WARNING(getLogger("PerfEvents"), "Event descriptor {} was closed from the outside; reopening", fd);
  } else {
    LOG_WARNING(getLogger("PerfEvents"), "Error while checking availability of event descriptor {}: {} ({})", fd, errnoToString(), errno);
...
bool PerfEventsCounters::processThreadLocalChanges(const std::string &needed_events_list) {
  ...
  for (size_t i = 0; i < NUMBER_OF_RAW_EVENTS; ++i) old_state[i] = thread_events_descriptors_holder.descriptors[i] != -1;

  bool new_state[NUMBER_OF_RAW_EVENTS];
  std::fill_n(new_state, NUMBER_OF_RAW_EVENTS, false);
  for (size_t opened_index : valid_event_indices) new_state[opened_index] = true;
  ...
  for (size_t i = 0; i < NUMBER_OF_RAW_EVENTS; ++i) {
    ...
    if (old_one == new_one) {
      if (old_one && !validatePerfEventDescriptor(thread_events_descriptors_holder.descriptors[i])) {
  ...
  for (size_t i : events_to_open) {
    ...
    if (fd == -1 && errno != ENOENT) {
      ...
      LOG_WARNING(getLogger("PerfEvents"),
                  ...
                  "'{}' ({})",
                  event_info.settings_name, event_info.event_type, event_info.event_config, errnoToString(), errno);
    }
  }
...
std::vector<size_t> PerfEventsCounters::eventIndicesFromString(const std::string &events_list) {
  ...
  for (auto &event_name : event_names) {
    ...
    if (entry != event_name_to_index.end()) {
      ...
      LOG_ERROR(getLogger("PerfEvents"), "Unknown perf event name '{}' specified in settings", event_name);
    }
  }
...
void PerfEventsCounters::initializeProfileEvents(const std::string &events_list) {
  if (!processThreadLocalChanges(events_list)) return;
...
void PerfEventsCounters::finalizeProfileEvents(ProfileEvents::Counters &profile_events) {
  ...
  for (size_t i = 0; i < NUMBER_OF_RAW_EVENTS; ++i) {
    ...
    if (bytes_read != bytes_to_read) {
      LOG_WARNING(getLogger("PerfEvents"), "Can't read event value from file descriptor {}: '{}' ({})", fd, errnoToString(), errno);
      current_values[i] = {};
    }
  }
  ...
  for (size_t i = 0; i < NUMBER_OF_RAW_EVENTS; ++i) {
    ...
    const auto scaled_value = static_cast<Float64>(current_value.value - previous_value.value) * static_cast<Float64>(enabled) /
                              std::max(1., static_cast<Float64>(running));
  ...
  if (min_enabled_time != UInt64(-1)) {
    profile_events.increment(ProfileEvents::PerfMinEnabledTime, min_enabled_time);
    profile_events.increment(ProfileEvents::PerfMinEnabledRunningTime, running_time_for_min_enabled_time);
  }
...
void PerfEventsCounters::closeEventDescriptors() { thread_events_descriptors_holder.releaseResources(); }

PerfDescriptorsHolder::PerfDescriptorsHolder() {
  for (int &descriptor : descriptors) descriptor = -1;
}

PerfDescriptorsHolder::~PerfDescriptorsHolder() { releaseResources(); }
