...
HostResolver::WeakPtr HostResolver::getWeakFromThis() { return weak_from_this(); }

HostResolver::HostResolver(String host_, Poco::Timespan history_)
    : HostResolver([](const String& host_to_resolve) { return DNSResolver::instance().resolveHostAllInOriginOrder(host_to_resolve); },
                   host_, history_) {}

HostResolver::HostResolver(ResolveFunction&& resolve_function_, String host_, Poco::Timespan history_)
    : host(std::move(host_)),
      history(history_),
...
HostResolver::~HostResolver() {
  ...
  auto banned_count = 0;
  for (const auto& rec : records) banned_count += rec.failed;
...
void HostResolver::Entry::setFail() {
  ...
  if (auto lock = pool.lock()) lock->setFail(address);
}

HostResolver::Entry::~Entry() {
  if (!fail) {
    if (auto lock = pool.lock()) lock->setSuccess(address);
  }
}
...
void HostResolver::update() {
  ...
  if (next_gen.empty()) throw NetException(ErrorCodes::DNS_ERROR, "no endpoints resolved for host {}", host);
...
void HostResolver::updateWeights() {
  ...
  if (getTotalWeight() == 0 && !records.empty()) {
    ...
    for (auto& rec : records) rec.failed = false;
...
void HostResolver::setSuccess(const Poco::Net::IPAddress& address) {
  ...
  if (old_weight != new_weight) updateWeights();
}
...
void HostResolver::setFail(const Poco::Net::IPAddress& address) {
...

  {
    ...
    if (it->setFail(now)) CurrentMetrics::add(metrics.banned_count);
  }
...
HostResolver::Records::iterator HostResolver::find(const Poco::Net::IPAddress& addr) TSA_REQUIRES(mutex) {
  auto it = std::lower_bound(records.begin(), records.end(), addr,
                             [](const Record& rec, const Poco::Net::IPAddress& value) { return rec.address < value; });

  if (it != records.end() && it->address != addr) return records.end();
...
void HostResolver::updateImpl(Poco::Timestamp now, std::vector<Poco::Net::IPAddress>& next_gen) TSA_REQUIRES(mutex) {
  ...
  while (it_before != records.end() || it_next != next_gen.end()) {
    if (it_next == next_gen.end() || (it_before != records.end() && it_before->address < *it_next)) {
      if (it_before->resolve_time >= last_effective_resolve)
        ...
        if (it_before->failed) CurrentMetrics::sub(metrics.banned_count);
      }
      ++it_before;
  ...
  for (auto& record : merged) {
    if (!record.failed || !record.consecutive_fail_count) continue;
  ...
  if (records.empty()) throw NetException(ErrorCodes::DNS_ERROR, "no endpoints resolved for host {}", host);
...
HostResolver::Ptr HostResolversPool::getResolver(const String& host) {
  ...
  if (it != host_pools.end()) return it->second;
