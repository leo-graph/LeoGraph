...
class FindResultImpl : public FindResultImplBase, public FindResultImplOffsetBase<need_offset> {
 ...
 public:
  FindResultImpl()
      : FindResultImplBase(false),
        FindResultImplOffsetBase<need_offset>(
            0)  // NOLINT(clang-analyzer-optin.cplusplus.UninitializedObject)  intentionally allow uninitialized value here
  {}
...
class HashMethodBase {
 public:
  ...
  ALWAYS_INLINE bool hasOnlyOneValueSinceLastReset() const {
    if constexpr (consecutive_keys_optimization) return cache.hasOnlyOneValue();
    return false;
  }

  ALWAYS_INLINE UInt64 getCacheMissesSinceLastReset() const {
    if constexpr (consecutive_keys_optimization) return cache.misses;
    return 0;
  }
 ...
 protected:
  ...
  explicit HashMethodBase(const IColumn *column = nullptr) {
    ...
    if constexpr (nullable) null_map = &checkAndGetColumn<ColumnNullable>(*column).getNullMapColumn();
  }
  ...
  ALWAYS_INLINE EmplaceResult emplaceImpl(KeyHolder &key_holder, Data &data) {
    ...
    if constexpr (has_mapped) cached = &it->getMapped();
    ...
    if constexpr (consecutive_keys_optimization) {
      ...
      if constexpr (nullable) cache.is_null = false;
  ...
  ALWAYS_INLINE FindResult findKeyImpl(Key key, Data &data) {
    ...
    if constexpr (consecutive_keys_optimization) {
      ...
      if constexpr (has_mapped) {
        ...
        if (it) cache.value.second = it->getMapped();
      } else {
        cache.value = key;
      }
    }
    ...
    size_t offset = 0;
    if constexpr (FindResult::has_offset) offset = it ? data.offsetInternal(it) : 0;
