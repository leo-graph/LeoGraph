...
LoggerPtr getLogger(const std::string& name) { return Poco::Logger::getShared(name); }
...
LoggerRawPtr getRawLogger(const std::string& name) { return &Poco::Logger::get(name); }
...
bool hasLogger(const std::string& name) { return Poco::Logger::has(name); }
