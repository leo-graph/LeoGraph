...
GetPriorityForLoadBalancing::Func GetPriorityForLoadBalancing::getPriorityFunc(LoadBalancing load_balance, size_t offset,
  ...
  switch (load_balance) {
    ...
    case LoadBalancing::ROUND_ROBIN:
      ...
      get_priority = [pool_size, local_last_used](size_t i) {
        ...
        if (i < local_last_used) priority = pool_size - 1 - (local_last_used - i);
        if (i > local_last_used) priority = i - local_last_used - 1;
