...
class SpanAttribute {
 private:
  String key;
  std::variant<String, int64_t, uint64_t, double, int, bool> value;

 public:
  template <typename V>
    requires(!std::is_constructible_v<String, std::decay_t<V>>)
  SpanAttribute(std::string_view k, V v) : key(k), value(std::forward<V>(v)) {}

  template <typename V>
    requires std::is_constructible_v<String, std::decay_t<V>>
  SpanAttribute(std::string_view k, V&& v) : key(k), value(String(std::forward<V>(v))) {}
  ...
  String getValue() const {
    return std::visit(
        []<typename T>(T&& v) {
          if constexpr (std::is_convertible_v<std::decay_t<T>, String>) return v;
          return toString(v);
        },
        value);
  }
...
enum class SpanKind : uint8_t {
  ...
  /// Indicates that the span describes the initiators of an asynchronous request. This parent span will often end before the corresponding
  /// child CONSUMER span, possibly even before the child span starts. In messaging scenarios with batching, tracing individual messages
  /// requires a new PRODUCER span per message to be created.
  PRODUCER = 3,
...
struct Span {
  ...
  bool isTraceEnabled() const { return trace_id != UUID(); }

  void start() {
    start_time_us = std::chrono::duration_cast<std::chrono::microseconds>(std::chrono::steady_clock::now().time_since_epoch()).count();
  }

  void finish() {
    finish_time_us = std::chrono::duration_cast<std::chrono::microseconds>(std::chrono::steady_clock::now().time_since_epoch()).count();
  }
...
///
/// Although it's SAFE to construct this object multiple times on one same thread, it should be created at the beginning of one thread
/// execution.
struct TracingContextHolder {
  ...
  TracingContextHolder(std::string_view _operation_name, const TracingContext& _parent_trace_context,
      ...
      : TracingContextHolder(_operation_name, _parent_trace_context, nullptr, _log) {}
  ...
  TracingContextHolder(std::string_view _operation_name, const TracingContextOnThread& _parent_thread_trace_context)
      : TracingContextHolder(_operation_name, _parent_thread_trace_context, nullptr, _parent_thread_trace_context.span_log) {}

  /// For servers like HTTP/TCP/GRPC to initialize tracing context on thread that process requests from clients
  TracingContextHolder(std::string_view _operation_name, TracingContext _parent_trace_context, const Settings& _settings,
                       const std::weak_ptr<OpenTelemetrySpanLog>& _log)
      : TracingContextHolder(_operation_name, _parent_trace_context, &_settings, _log) {}

  TracingContextHolder(std::string_view _operation_name, TracingContext _parent_trace_context, const Settings* settings_ptr,
                       const std::weak_ptr<OpenTelemetrySpanLog>& _log);
...
struct SpanHolder : public Span {
  explicit SpanHolder(std::string_view _operation_name, SpanKind _kind = SpanKind::INTERNAL, bool create_trace_if_not_exists = false);

  SpanHolder(std::string_view _operation_name, SpanKind _kind, std::vector<SpanAttribute> _attributes,
