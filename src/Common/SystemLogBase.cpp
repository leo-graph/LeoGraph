...

{
  ...
  if (settings.turn_off_logger) log->setLevel(0);
}
...
void SystemLogQueue<LogElement>::push(LogElement&& element) {
  ...
  /// - MemoryTrackerBlockerInThread here because this allocation should not be take into account in the query scope (since it will be freed
  /// outside of it)
  ...
  if (buffer_size_rows_flush_threshold_exceeded)
    LOG_INFO(log, "Queue is half full for system log '{}'. buffer_size_rows_flush_threshold {}", demangle(typeid(*this).name()),
             settings.buffer_size_rows_flush_threshold);
}
...
void SystemLogQueue<LogElement>::notifyFlushUnlocked(Index expected_flushed_index, bool should_prepare_tables_anyway) {
  if (should_prepare_tables_anyway) requested_prepare_tables = std::max(requested_prepare_tables, expected_flushed_index);
...
void SystemLogQueue<LogElement>::waitFlush(SystemLogQueue<LogElement>::Index expected_flushed_index, bool should_prepare_tables_anyway) {
  ...
  // there is no obligation to call notifyFlush before waitFlush, than we have to be sure that flush_event has been triggered before we wait
  // the result
  notifyFlushUnlocked(expected_flushed_index, should_prepare_tables_anyway);

  // prepared_tables starts from -1, so we need to wait for prepared_tables >= 0 when expected_flushed_index == 0 to make sure the table is
  // created In theory it should be possible to wait only for prepared_tables, but:
  ...
  if (!result) {
    throw Exception(ErrorCodes::TIMEOUT_EXCEEDED, "Timeout exceeded ({} s) while flushing system log '{}'.", toString(timeout_seconds),
...
typename SystemLogQueue<LogElement>::PopResult SystemLogQueue<LogElement>::pop() {
...

  {
    ...
    if (is_shutdown) return PopResult{.is_shutdown = true};
    ...
    if (!queue.empty()) result.logs.swap(queue);
    ...
    if (queue_size > queue.capacity()) queue.reserve(std::max(settings.reserved_size_rows, queue_size));
  }

  if (prev_ignored_logs)
    LOG_ERROR(log, "Queue had been full at {}, accepted {} logs, ignored {} logs.", result.last_log_index - result.logs.size(),
              result.logs.size(), prev_ignored_logs);
...
template <typename LogElement>
SystemLogBase<LogElement>::SystemLogBase(const SystemLogQueueSettings& settings_, std::shared_ptr<SystemLogQueue<LogElement>> queue_)
...
void SystemLogBase<LogElement>::stopFlushThread() {
  {
    ...
    if (!saving_thread || !saving_thread->joinable()) return;
