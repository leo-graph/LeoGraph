...
class InterruptListener {
 ...
 public:
  ...
  void block() {
    if (!active) {
      if (pthread_sigmask(SIG_BLOCK, &sig_set, nullptr)) throw ErrnoException(ErrorCodes::CANNOT_BLOCK_SIGNAL, "Cannot block signal");
  ...
  void unblock() {
    if (active) {
      if (pthread_sigmask(SIG_UNBLOCK, &sig_set, nullptr)) throw ErrnoException(ErrorCodes::CANNOT_UNBLOCK_SIGNAL, "Cannot unblock signal");
