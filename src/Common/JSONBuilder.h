...
class JSONNumber : public IItem {
 public:
  explicit JSONNumber(T value_) : value(value_) { static_assert(std::is_arithmetic_v<T>, "JSONNumber support only numeric types"); }

  void format(const FormatSettings& settings, FormatContext& context) override { writeJSONNumber(value, context.out, settings.settings); }
...
class JSONArray : public IItem {
 public:
  ...
  void add(T value) {
    add(std::make_unique<JSONNumber<T>>(value));
  }
...
class JSONMap : public IItem {
 ...
 public:
  ...
  void add(std::string key, T value) {
    add(std::move(key), std::make_unique<JSONNumber<T>>(value));
  }
