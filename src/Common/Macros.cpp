...
Macros::Macros(std::map<String, String, std::less<>> map) { macros = std::move(map); }

String Macros::expand(const String& s, MacroExpansionInfo& info) const {
  ...
  if (info.level && s.size() > 65536) throw Exception(ErrorCodes::SYNTAX_ERROR, "Too long string while expanding macros");

  if (info.level >= 10) throw Exception(ErrorCodes::SYNTAX_ERROR, "Too deep recursion while expanding macros: '{}'", s);

  /// If config file contains explicit special macro, then we do not expand it in this mode.
  if (!enable_special_macros && info.expand_special_macros_only) return s;
  ...
  while (true) {
    ...
    if (end == String::npos) throw Exception(ErrorCodes::SYNTAX_ERROR, "Unbalanced {{ and }} in string with macros: '{}'", s);
    ...
    if (it != macros.end() && !info.expand_special_macros_only) {
      ...
      if (info.table_id.uuid == UUIDHelpers::Nil) {
        if (info.expand_for_database)
          throw Exception(ErrorCodes::BAD_ARGUMENTS,
                          "Macro 'uuid' in engine arguments is only supported when the UUID is explicitly specified "
      ...
      if (info.level) throw Exception(ErrorCodes::SYNTAX_ERROR, "Macro 'uuid' should not be inside another macro");
      ...
      if (info.expand_special_macros_only) res += '{';
      res += macro_name;
      if (info.expand_special_macros_only) res += '}';
      info.has_unknown = true;
    } else
      throw Exception(ErrorCodes::NO_ELEMENTS_IN_CONFIG,
                      "No macro '{}' in config while processing substitutions in "
                      "'{}' at '{}' or macro is not supported here",
                      macro_name, s, toString(begin));

    pos = end + 1;
  }

  ++info.level;
  if (info.expand_special_macros_only) return res;

  return expand(res, info);
}

String Macros::getValue(std::string_view key) const {
  if (auto it = macros.find(key); it != macros.end()) return it->second;
