...
class BucketCountQuantile {
 ...
 public:
  BucketCountQuantile(int64_t min_value_, int64_t max_value_, int64_t buckets_count_)
    ...
    for (int64_t i = 0; i < 4 * buckets_count; ++i) buckets[i].store(0);
  }
...
std::tuple<double, BucketCountQuantile *, std::atomic<bool> *> takeSharedQuantileData(CurrentMetrics::Metric metric) {
  ...
  std::call_once(initialize_metrics_shared_data, [&]() {
    ...
    buckets.emplace(std::piecewise_construct, std::forward_as_tuple(CurrentMetrics::SharedMergeTreeMaxActiveReplicas),
                    std::forward_as_tuple(0, 500, 501));
    buckets.emplace(std::piecewise_construct, std::forward_as_tuple(CurrentMetrics::SharedMergeTreeMaxInactiveReplicas),
                    std::forward_as_tuple(0, 500, 501));
    buckets.emplace(std::piecewise_construct, std::forward_as_tuple(CurrentMetrics::SharedMergeTreeMaxReplicas),
                    std::forward_as_tuple(0, 500, 501));
    buckets.emplace(std::piecewise_construct, std::forward_as_tuple(CurrentMetrics::SharedMergeTreeMinActiveReplicas),
                    std::forward_as_tuple(0, 500, 501));
    buckets.emplace(std::piecewise_construct, std::forward_as_tuple(CurrentMetrics::SharedMergeTreeMinInactiveReplicas),
                    std::forward_as_tuple(0, 500, 501));
    buckets.emplace(std::piecewise_construct, std::forward_as_tuple(CurrentMetrics::SharedMergeTreeMinReplicas),
                    std::forward_as_tuple(0, 500, 501));
...
GlobalSum::GlobalSum(CurrentMetrics::Metric destination_metric_) noexcept : destination_metric(destination_metric_) {}

GlobalSum::~GlobalSum() noexcept { set(0); }
...
void GlobalQuantile::updateBuckets(std::optional<CurrentMetrics::Value> prev_value,
                                   std::optional<CurrentMetrics::Value> new_value) noexcept {
  ...
  if (prev_value.has_value()) buckets->decrement(prev_value.value());

  if (new_value.has_value()) buckets->increment(new_value.value());
...
GlobalQuantile::GlobalQuantile(CurrentMetrics::Metric destination_metric_) noexcept : destination_metric(destination_metric_) {
...
GlobalQuantile::~GlobalQuantile() noexcept { updateBuckets(/*prev_value=*/accounted_value.load(), /*new_value=*/std::nullopt); }
