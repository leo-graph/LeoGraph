...
void call_default_signal_handler(int sig) {
  if (SIG_ERR == signal(sig, SIG_DFL)) throw ErrnoException(ErrorCodes::CANNOT_SET_SIGNAL_HANDLER, "Cannot set signal handler");

  if (0 != raise(sig)) throw ErrnoException(ErrorCodes::CANNOT_SEND_SIGNAL, "Cannot send signal");
}
...
static void signalHandler(int sig, siginfo_t *info, void *context) {
  if (asynchronous_stack_unwinding && sig == SIGSEGV) siglongjmp(asynchronous_stack_unwinding_signal_jump_buffer, 1);

  DENY_ALLOCATIONS_IN_SCOPE;
  auto saved_errno = errno;  /// We must restore previous value of errno in signal handler.

  if (sig != SIGTSTP) is_crashed.store(true, std::memory_order_relaxed);
  ...
  for (size_t i = 0; i < terminate_current_exception_trace_size; ++i) writePODBinary(terminate_current_exception_trace[i], out);
#endif
  out.finalize();
  ...
  if (sig != SIGTSTP)  /// This signal is used for debugging.
  {
    ...
    for (size_t i = 0; i < 300; ++i) {
      ...
      if (HandledSignals::instance().fatal_error_printed.test()) break;
...
[[noreturn]] void terminate_handler() {
  ...
  if (std::current_exception()) {
    ...
    try {
      ...
      for (size_t i = 0; i < terminate_current_exception_trace_size; ++i) terminate_current_exception_trace[i] = stack_trace_frames[i];
    } catch (...) {
    }  // NOLINT(bugprone-empty-catch) Ok: best-effort in terminate handler
  } else {
  ...
  if (log_message.size() > buf_size - 16) log_message.resize(buf_size - 16);
...
void HandledSignals::addSignalHandler(const std::vector<int> &signals, signal_function handler, bool register_signal) {
  ...
  for (auto signal : signals)
    if (sigaddset(&sa.sa_mask, signal)) throw Poco::Exception("Cannot set signal handler.");
#endif

  for (auto signal : signals)
    if (sigaction(signal, &sa, nullptr)) throw Poco::Exception("Cannot set signal handler.");

  if (register_signal) std::copy(signals.begin(), signals.end(), std::back_inserter(handled_signals));
}
...
#if defined(OS_DARWIN)
  sigemptyset(&sig_set);
  for (auto signal : signals) sigaddset(&sig_set, signal);
#else
  if (sigemptyset(&sig_set)) throw Poco::Exception("Cannot block signal.");

  for (auto signal : signals)
    if (sigaddset(&sig_set, signal)) throw Poco::Exception("Cannot block signal.");
#endif

  if (pthread_sigmask(SIG_BLOCK, &sig_set, nullptr)) throw Poco::Exception("Cannot block signal.");
}

SignalListener::SignalListener(BaseDaemon *daemon_, LoggerPtr log_) : daemon(daemon_), log(log_) {}
...
void SignalListener::run() {
  ...
  static_assert(signal_pipe_buf_size <= PIPE_BUF,
  ...
  while (!in.eof()) {
    ...
    if (sig == SIGHUP) {
      ...
      if (daemon) daemon->handleSignal(sig);
    } else if (sig == SIGCHLD) {
      pid_t child_pid = 0;
      ...
      for (size_t i = 0; i < exception_trace_size; ++i) readPODBinary(exception_trace[i], in);
#endif
...
void SignalListener::onTerminate(std::string_view message, UInt32 thread_num) const {
  ...
  LOG_FATAL(log, "(version {}{}, build id: {}, git hash: {}) (from thread {}) {}", VERSION_STRING, VERSION_OFFICIAL, build_id(), GIT_HASH,
            thread_num, message.substr(0, pos));
  ...
  while (pos != std::string_view::npos) {
    ...
    if (next_pos != std::string_view::npos) size = next_pos - pos;
...
void SignalListener::onFault(int sig, const siginfo_t &info, ucontext_t *context, const StackTrace &stack_trace,
                             const std::vector<FramePointers> &thread_frame_pointers, UInt32 thread_num, DB::ThreadStatus *thread_ptr,
                             const FramePointers &exception_trace, size_t exception_trace_size) const try {
  ...
  LOG_FATAL(log, "(version {}{}, build id: {}, git hash: {}, architecture: {}) (from thread {}) Received signal {} ({})", VERSION_STRING,
            VERSION_OFFICIAL, build_id(), GIT_HASH, Poco::Environment::osArchitecture(), thread_num, sig,
  ...
  if (query_id.empty()) {
    LOG_FATAL(log, "(version {}{}, build id: {}, git hash: {}) (from thread {}) (no query) Received signal {} ({})", VERSION_STRING,
              VERSION_OFFICIAL, build_id(), GIT_HASH, thread_num, signal_description, sig);
  } else {
    LOG_FATAL(log, "(version {}{}, build id: {}, git hash: {}) (from thread {}) (query_id: {}) (query: {}) Received signal {} ({})",
              VERSION_STRING, VERSION_OFFICIAL, build_id(), GIT_HASH, thread_num, query_id, query, signal_description, sig);
  }
  ...
  std::for_each(thread_frame_pointers.rbegin(), thread_frame_pointers.rend(), [this](const FramePointers &frame_pointers) {
    if (size_t size = std::ranges::find(frame_pointers, nullptr) - frame_pointers.begin()) {
      ...
      std::for_each_n(frame_pointers.begin(), size, [&bare_stacktrace](const void *ptr) {
        writeChar(' ', bare_stacktrace);
      ...
      StackTrace::toStringEveryLine(const_cast<void **>(frame_pointers.data()), 0, size,
                                    [this](std::string_view s) { LOG_FATAL(log, fmt::runtime(s)); });
    }
  });
  ...
  if (collectCrashLog) {
    ...
    collectCrashLog(sig, info.si_code, thread_num, query_id, query, stack_trace, fault_address, fault_access_type, si_code_description,
                    exception_trace, exception_trace_size);
  }

  if (daemon) daemon->flushTextLogs();
  Context::getGlobalContextInstance()->handleCrash();

  /// Send crash report to developers (if configured)
  if (daemon) {
    CrashWriter::onSignal(sig, std::string_view(error_message), stack_trace.getFramePointers(), stack_trace.getOffset(),
                          stack_trace.getSize());
  }
  ...
  if (std::string_view(VERSION_OFFICIAL).contains("official build")) {
    ...
    if (time(nullptr) - makeDate(DateLUT::instance(), static_cast<UInt8>(2000 + VERSION_MAJOR), static_cast<UInt8>(VERSION_MINOR), 1) <
        (365 + 30) * 86400) {
  ...
  if (thread_ptr) thread_ptr->onFatalError();
...
void HandledSignals::reset(bool close_pipe) {
  ...
  if (close_pipe) signal_pipe.close();
}
...
void HandledSignals::setupTerminateHandler() { std::set_terminate(terminate_handler); }
