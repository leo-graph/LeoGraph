...
DECLARE_DEFAULT_CODE(static bool isAllASCII(const UInt8 *data, size_t size) {
  UInt8 mask = 0;
  for (size_t i = 0; i < size; ++i) mask |= data[i];
...
DECLARE_X86_64_V2_SPECIFIC_CODE(
    ...
    static bool isAllASCII(const UInt8 *data, size_t size) {
      ...
      UInt8 tail_mask = 0;
      for (; i < size; i++) tail_mask |= data[i];

      mask |= (tail_mask & 0x80);
      return !mask;
    })

DECLARE_X86_64_V3_SPECIFIC_CODE(static bool isAllASCII(const UInt8 *data, size_t size) {
...
#if USE_MULTITARGET_CODE
  if (isArchSupported(DB::TargetArch::x86_64_v3)) return TargetSpecific::x86_64_v3::isAllASCII(data, size);
  if (isArchSupported(DB::TargetArch::x86_64_v2)) return TargetSpecific::x86_64_v2::isAllASCII(data, size);
#endif
...
std::tuple<String, bool> extractFixedPrefixFromLikePattern(std::string_view like_pattern, bool requires_perfect_prefix) {
  ...
  if (requires_perfect_prefix) return {"", false};
...
String firstStringThatIsGreaterThanAllStringsWithPrefix(const String &prefix) {
  ...
  while (!res.empty() && static_cast<UInt8>(res.back()) == std::numeric_limits<UInt8>::max()) res.pop_back();
