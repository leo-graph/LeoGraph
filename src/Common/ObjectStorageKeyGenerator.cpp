...
class GeneratorWithTemplate : public DB::IObjectStorageKeyGenerator {
 public:
  explicit GeneratorWithTemplate(std::string key_template_) : key_template(std::move(key_template_)), re_gen(key_template) {}

  DB::ObjectStorageKey generate() const override { return DB::ObjectStorageKey::createAsAbsolute(re_gen.generate()); }
