...
struct RUsageCounters {
  ...
  RUsageCounters(const ::rusage& rusage_, UInt64 real_time_) { set(rusage_, real_time_); }
...
#  endif  // OS_SUNOS
 ...
 private:
  static UInt64 getClockMonotonic() {
    struct timespec ts;
    if (0 != clock_gettime(CLOCK_MONOTONIC, &ts)) throw std::system_error(std::error_code(errno, std::system_category()));
