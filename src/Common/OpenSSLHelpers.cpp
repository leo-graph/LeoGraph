...
std::string encodeSHA256(std::string_view text) { return encodeSHA256(text.data(), text.size()); }
...
void encodeSHA256(std::string_view text, unsigned char *out) { encodeSHA256(text.data(), text.size(), out); }
...
void encodeSHA256(const void *text, size_t size, unsigned char *out) {
  ...
  if (!EVP_DigestInit(ctx.get(), EVP_sha256())) throw Exception(ErrorCodes::OPENSSL_ERROR, "EVP_DigestInit failed: {}", getOpenSSLErrors());
...
std::string rsaSHA256Sign(EVP_PKEY *pkey, const std::string &data) {
  ...
  if (!ctx) throw Exception(ErrorCodes::OPENSSL_ERROR, "{}", getOpenSSLErrors());

  if (EVP_DigestSignInit(ctx, nullptr, EVP_sha256(), nullptr, pkey) != 1)
    throw Exception(ErrorCodes::OPENSSL_ERROR, "{}", getOpenSSLErrors());

  if (EVP_DigestSignUpdate(ctx, data.data(), data.size()) != 1) throw Exception(ErrorCodes::OPENSSL_ERROR, "{}", getOpenSSLErrors());

  size_t signature_length = 0;
  if (EVP_DigestSignFinal(ctx, nullptr, &signature_length) != 1) throw Exception(ErrorCodes::OPENSSL_ERROR, "{}", getOpenSSLErrors());
...
bool genericSHA256Verify(EVP_PKEY *pkey, const std::string &data, const std::string &signature) {
  ...
  if (!ctx) throw DB::Exception(DB::ErrorCodes::OPENSSL_ERROR, "{}", getOpenSSLErrors());
  ...
  int result = EVP_DigestVerifyFinal(ctx, reinterpret_cast<const unsigned char *>(signature.data()), static_cast<int>(signature.size()));
...
std::vector<uint8_t> hmacSHA256(const std::vector<uint8_t> &key, const std::string &data) {
  ...
  if (!mac) throw Exception(ErrorCodes::OPENSSL_ERROR, "EVP_MAC_fetch failed: {}", getOpenSSLErrors());

  EVP_MAC_CTX_ptr ctx(EVP_MAC_CTX_new(mac.get()), &EVP_MAC_CTX_free);
  if (!ctx) throw Exception(ErrorCodes::OPENSSL_ERROR, "EVP_MAC_CTX_new failed: {}", getOpenSSLErrors());

  OSSL_PARAM params[] = {OSSL_PARAM_utf8_string(OSSL_MAC_PARAM_DIGEST, const_cast<char *>("SHA256"), 0), OSSL_PARAM_END};
...
std::vector<uint8_t> pbkdf2SHA256(std::string_view password, const std::vector<uint8_t> &salt, int iterations) {
  ...
  if (!kdf) throw Exception(ErrorCodes::OPENSSL_ERROR, "EVP_KDF_fetch failed");

  EVP_KDF_CTX_ptr ctx(EVP_KDF_CTX_new(kdf.get()), &EVP_KDF_CTX_free);
  if (!ctx) throw Exception(ErrorCodes::OPENSSL_ERROR, "EVP_KDF_CTX_new failed");
...
std::string generateCSR(const std::vector<std::string> domain_names, EVP_PKEY *key) {
  if (domain_names.empty()) throw Exception(ErrorCodes::BAD_ARGUMENTS, "No domain names provided");

  if (!key) throw Exception(ErrorCodes::BAD_ARGUMENTS, "No private key provided");

  /// Create X509 object and set subject name
  X509_REQ_ptr req(X509_REQ_new(), X509_REQ_free);
  if (!req) throw Exception(ErrorCodes::OPENSSL_ERROR, "Error creating X509_REQ: {}", getOpenSSLErrors());

  /// Owned by X509_REQ, does not need to be freed.
  X509_NAME *subject_name = X509_REQ_get_subject_name(req.get());
  if (!subject_name) throw Exception(ErrorCodes::OPENSSL_ERROR, "Error getting subject name from X509_REQ: {}", getOpenSSLErrors());
  ...
  for (const auto &domain_name : domain_names) {
    if (!domain_name.empty()) san_entries += "DNS:" + domain_name + ",";
  }
  if (!san_entries.empty() && san_entries.back() == ',') san_entries.pop_back();
  ...
  if (!OPENSSL_sk_push(reinterpret_cast<OPENSSL_STACK *>(extensions.get()), reinterpret_cast<void *>(san_extension)))
  ...
  if (csr_len == 0) throw Exception(ErrorCodes::OPENSSL_ERROR, "Failure in BIO_ctrl_pending: {}", getOpenSSLErrors());
