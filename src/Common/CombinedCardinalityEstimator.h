...
template <typename Key, typename HashContainer, UInt8 small_set_size_max, UInt8 medium_set_power2_max, UInt8 K,
          typename Hash = IntHash32<Key>, typename HashValueType = UInt32, typename BiasEstimator = TrivialBiasEstimator,
          HyperLogLogMode mode = HyperLogLogMode::FullFeatured, typename DenominatorType = double>
class CombinedCardinalityEstimator {
 public:
  using Self = CombinedCardinalityEstimator<Key, HashContainer, small_set_size_max, medium_set_power2_max, K, Hash, HashValueType,
                                            BiasEstimator, mode, DenominatorType>;
 ...
 public:
  ...
  UInt64 size() const {
    ...
    if (container_type == details::ContainerType::SMALL) return small.size();
    if (container_type == details::ContainerType::MEDIUM) return getContainer<Medium>().size();
    if (container_type == details::ContainerType::LARGE) return getContainer<Large>().size();
  ...
  void merge(const Self &rhs) {
    ...
    if (rhs.getContainerType() == details::ContainerType::SMALL) {
      for (const auto &x : rhs.small) insert(x.getValue());
    } else if (rhs.getContainerType() == details::ContainerType::MEDIUM) {
      for (const auto &x : rhs.getContainer<Medium>()) insert(x.getValue());
 ...
 private:
  void toMedium() {
    if (getContainerType() != details::ContainerType::SMALL) throw Poco::Exception("Internal error", ErrorCodes::LOGICAL_ERROR);

    auto tmp_medium = std::make_unique<Medium>();

    for (const auto &x : small) tmp_medium->insert(x.getValue());
  ...
  void toLarge() {
    ...
    if (container_type == details::ContainerType::SMALL) {
      for (const auto &x : small) tmp_large->insert(x.getValue());
    } else if (container_type == details::ContainerType::MEDIUM) {
      for (const auto &x : getContainer<Medium>()) tmp_large->insert(x.getValue());
  ...
  details::ContainerType getContainerType() const { return static_cast<details::ContainerType>(address & ~mask); }
