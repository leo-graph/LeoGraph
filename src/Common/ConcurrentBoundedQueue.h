...
class ConcurrentBoundedQueue {
  ...
  bool popImpl(T &x, std::optional<UInt64> timeout_milliseconds) {
    {
      ...
      if (is_finished && queue.empty()) return false;
 ...
 public:
  explicit ConcurrentBoundedQueue(size_t max_fill_) : max_fill(max_fill_) {}

  /// Returns false if queue is finished
  [[nodiscard]] bool pushFront(const T &x) { return emplaceImpl</* back= */ false>(/* timeout_milliseconds= */ std::nullopt, x); }

  /// Returns false if queue is finished
  [[nodiscard]] bool push(const T &x) { return emplace(x); }

  [[nodiscard]] bool push(T &&x) { return emplace(std::move(x)); }
  ...
  [[nodiscard]] bool tryPush(const T &x, UInt64 milliseconds = 0) { return emplaceImpl</* back= */ true>(milliseconds, x); }

  [[nodiscard]] bool tryPush(T &&x, UInt64 milliseconds = 0) { return emplaceImpl</* back= */ true>(milliseconds, std::move(x)); }
  ...
  [[nodiscard]] bool pop(T &x) { return popImpl</* front= */ true>(x, std::nullopt /*timeout in milliseconds*/); }

  /// Returns false if queue is (finished and empty) or (object was not popped during timeout)
  [[nodiscard]] bool tryPop(T &x, UInt64 milliseconds) { return popImpl</* front= */ true>(x, milliseconds); }
