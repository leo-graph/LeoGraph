...
class ConnectionGroup {
 public:
  ...
  void atConnectionCreate(std::string host, UInt16 port) {
    ...
    if (isHardLimitReached())
      throw Exception(ErrorCodes::HTTP_CONNECTION_LIMIT_REACHED,
                      "Cannot create new connection to {}:{}, hard limit {} for connections in group {} is reached", host, port,
                      limits.hard_limit, getType());
    ...
    if (total_connections_in_group >= limits.warning_limit && total_connections_in_group >= mute_warning_until) {
      ...
      LOG_WARNING(log, "Too many active sessions in group {}, count {}, warning limit {}, next warning at {}", type,
                  total_connections_in_group, limits.warning_limit, mute_warning_until);
    }
  }
 ...
 private:
  bool isHardLimitReached() const TSA_REQUIRES(mutex) { return limits.hard_limit > 0 && total_connections_in_group >= limits.hard_limit; }
...
struct ResourceGuardSessionDataHooks : public Poco::Net::IHTTPSessionDataHooks {
  ResourceGuardSessionDataHooks(ResourceLink link_, const ResourceGuard::Metrics *metrics, LoggerPtr log_, const String &method,
                                const String &uri)
      : link(link_), log(log_), http_request(method + " " + uri) {
  ...
  void atFinish(int bytes) override { request.finish(bytes, link); }

  void atFail() override { request.finish(0, link); }
...
class EndpointConnectionPool : public std::enable_shared_from_this<EndpointConnectionPool<Session>>, public IExtendedPool {
 private:
  ...
  class PooledConnection : public Session {
   public:
    ...
    void markAsExpired() { isExpired = true; }
    ...
    String getTarget() const {
      if (!Session::getProxyConfig().host.empty())
        return fmt::format("{}:{} over proxy {}", Session::getHost(), Session::getPort(), Session::getProxyConfig().host);
      return fmt::format("{}:{}", Session::getHost(), Session::getPort());
    }
    ...
    std::ostream &sendRequest(Poco::Net::HTTPRequest &request, UInt64 *connect_time, UInt64 *first_byte_time) override {
      ...
      if (ResourceLink link = CurrentThread::getReadResourceLink())
        Session::setReceiveDataHooks(std::make_shared<ResourceGuardSessionDataHooks>(link, ResourceGuard::Metrics::getIORead(), log,
                                                                                     request.getMethod(), request.getURI()));
      if (ResourceLink link = CurrentThread::getWriteResourceLink())
        Session::setSendDataHooks(std::make_shared<ResourceGuardSessionDataHooks>(link, ResourceGuard::Metrics::getIOWrite(), log,
                                                                                  request.getMethod(), request.getURI()));
      if (auto throttler = CurrentThread::getReadThrottler()) Session::setReceiveThrottler(throttler);
      if (auto throttler = CurrentThread::getWriteThrottler()) Session::setSendThrottler(throttler);
    ...
    ~PooledConnection() override {
      ...
      if (!isExpired)
        if (auto lock = pool.lock()) lock->atConnectionDestroy(*this);
   ...
   private:
    ...
    template <class... Args>
    explicit PooledConnection(EndpointConnectionPool::WeakPtr pool_, ConnectionGroup::Ptr group_,
                              IHTTPConnectionPoolForEndpoint::Metrics metrics_, Args &&...args)
        : Session(std::forward<Args>(args)...), pool(std::move(pool_)), group(group_), metrics(std::move(metrics_)) {
    ...
    void doConnect(UInt64 *connect_time) { Session::reconnect(connect_time); }

    bool isCompleted() const { return request_stream_completed && response_stream_completed; }
 ...
 public:
  ...
  ~EndpointConnectionPool() override { CurrentMetrics::sub(group->getMetrics().stored_count, stored_connections.size()); }

  String getTarget() const {
    if (!proxy_configuration.isEmpty()) return fmt::format("{} over proxy {}", host, proxy_configuration.host);
    return host;
  }
  ...
  const IHTTPConnectionPoolForEndpoint::Metrics &getMetrics() const override { return group->getMetrics(); }

  HTTPConnectionGroupType getGroupType() const override { return group->getType(); }
  ...
  size_t wipeExpiredImpl(std::vector<ConnectionPtr> &expired_connections) TSA_REQUIRES(mutex) {
    ...
    while (!stored_connections.empty()) {
      ...
      if (!isExpired(connection, isSoftLimitReached)) return stored_connections.size();
 ...
 private:
  EndpointConnectionPool(ConnectionGroup::Ptr group_, String host_, UInt16 port_, bool https_, ProxyConfiguration proxy_configuration_)
      : host(std::move(host_)), port(port_), https(https_), proxy_configuration(std::move(proxy_configuration_)), group(group_) {}
  ...
  bool isExpired(ConnectionPtr connection, bool isSoftLimitReached) TSA_REQUIRES(mutex) {
    if (isSoftLimitReached) return connection->isKeepAliveExpired(0.1);
...
struct EndpointPoolKey {
  ...
  bool operator==(const EndpointPoolKey &rhs) const {
    return std::tie(connection_group, target_host, target_port, is_target_https, proxy_config.host, proxy_config.port,
                    proxy_config.protocol, proxy_config.tunneling, proxy_config.original_request_protocol, proxy_config.no_proxy_hosts) ==
           std::tie(rhs.connection_group, rhs.target_host, rhs.target_port, rhs.is_target_https, rhs.proxy_config.host,
                    rhs.proxy_config.port, rhs.proxy_config.protocol, rhs.proxy_config.tunneling,
                    rhs.proxy_config.original_request_protocol, rhs.proxy_config.no_proxy_hosts);
  }
};
...
static IExtendedPool::Ptr createConnectionPool(ConnectionGroup::Ptr group, std::string host, UInt16 port, bool secure,
                                               ProxyConfiguration proxy_configuration) {
  if (secure) {
#if USE_SSL
    return EndpointConnectionPool<Poco::Net::HTTPSClientSession>::create(group, std::move(host), port, secure,
                                                                         std::move(proxy_configuration));
#else
    throw Exception(ErrorCodes::SUPPORT_IS_DISABLED, "HTTPS support is disabled, because ClickHouse was built without SSL library");
#endif
  } else {
    return EndpointConnectionPool<Poco::Net::HTTPClientSession>::create(group, std::move(host), port, secure,
                                                                        std::move(proxy_configuration));
  }
}
...
class HTTPConnectionPools::Impl {
 ...
 public:
  IHTTPConnectionPoolForEndpoint::Ptr getPool(HTTPConnectionGroupType type, const Poco::URI &uri,
                                              const ProxyConfiguration &proxy_configuration) {
    Poco::Timestamp now;
 ...
 protected:
  ...
  IExtendedPool::Ptr getPoolImpl(HTTPConnectionGroupType type, const Poco::URI &uri, const ProxyConfiguration &proxy_configuration)
    ...
    if (it != endpoints_pool.end()) return it->second;
  ...
  void wipeExpired() TSA_REQUIRES(mutex) {
    ...
    for (auto &[key, pool] : endpoints_pool) {
      ...
      if (left_connections == 0 && pool->getGroupType() != HTTPConnectionGroupType::DISK) keys_to_drop.push_back(key);
    }

    for (const auto &key : keys_to_drop) endpoints_pool.erase(key);
  }

  static bool useSecureConnection(const Poco::URI &uri, const ProxyConfiguration &proxy_configuration) {
    if (uri.getScheme() == "http") return false;

    if (uri.getScheme() != "https") throw Exception(ErrorCodes::UNSUPPORTED_URI_SCHEME, "Unsupported scheme in URI '{}'", uri.toString());
...
HTTPConnectionPools::HTTPConnectionPools() : impl(std::make_unique<HTTPConnectionPools::Impl>()) {}
...
void HTTPConnectionPools::setLimits(HTTPConnectionPools::Limits disk, HTTPConnectionPools::Limits storage,
                                    HTTPConnectionPools::Limits http) {
  impl->setLimits(std::move(disk), std::move(storage), std::move(http));
}

void HTTPConnectionPools::dropCache() { impl->dropCache(); }

IHTTPConnectionPoolForEndpoint::Ptr HTTPConnectionPools::getPool(HTTPConnectionGroupType type, const Poco::URI &uri,
                                                                 const ProxyConfiguration &proxy_configuration) {
