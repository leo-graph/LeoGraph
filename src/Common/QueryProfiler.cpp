...
void writeTraceInfo(TraceType trace_type, int /* sig */, siginfo_t *info, void *context) {
  ...
  if (stack_trace) TraceSender::send(trace_type, *stack_trace, {});
...
Timer::Timer() : log(getLogger("Timer")) {}
...
#  endif
    ...
    if (timer_create(clock_type, &sev, &local_timer_id)) {
      ...
      if (errno == 0)
        throw Exception(ErrorCodes::CANNOT_CREATE_TIMER,
                        "Failed to create thread timer. The function "
...
void Timer::set(UInt64 period) {
  /// Too high frequency can introduce infinite busy loop of signal handlers. We will limit maximum frequency (with 1000 signals per
  /// second).
...
void Timer::stop() {
  if (timer_id) {
    ...
    if (err) LOG_ERROR(log, "Failed to stop query profiler timer {}", errnoToString());
...
void Timer::cleanup() {
  if (timer_id) {
    ...
    if (err) LOG_ERROR(log, "Failed to delete query profiler timer {}", errnoToString());
...
QueryProfilerBase<ProfilerImpl>::QueryProfilerBase([[maybe_unused]] UInt64 thread_id, [[maybe_unused]] int clock_type,
                                                   [[maybe_unused]] UInt64 period, [[maybe_unused]] int pause_signal_)
...
QueryProfilerReal::QueryProfilerReal(UInt64 thread_id, UInt64 period) : QueryProfilerBase(thread_id, CLOCK_MONOTONIC, period, SIGUSR1) {}

void QueryProfilerReal::signalHandler(int sig, siginfo_t *info, void *context) {
  if (signal_handler_disarmed) return;
