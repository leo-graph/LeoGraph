 ...
 * This number is extremely good for compilers as it is numeric_limits<Uint16>::max() and there are optimizations with movzwl and other
...
template <typename Callback>
static bool putNGramUTF8CaseInsensitive(const UInt8 *pos, int offset, const UInt8 *begin, size_t size, Callback &&putNGramBase) {
    ...
    if (UTF8::isContinuationOctet(chars.c1)) {
      ...
      if (!u32) {
        ...
        if (l_u32 == u_u32) {
          ...
          if (length_l != length_r) return false;

          if (length_l < 2 || length_r < 2) return false;  /// Some part of the given ngram contains an invalid UTF-8 sequence.
      ...
      if (first_l_u32 == first_u_u32 && second_l_u32 == second_u_u32) {
        ...
        if (size_l != size_u) return false;

        if (size_l == 0 || size_u == 0) return false;  /// Some part of the given ngram contains an invalid UTF-8 sequence.
        ...
        if (size_first_l != size_first_u || size_second_l != size_second_u) return false;

        if (size_first_l <= seq_ngram_offset || size_first_u <= seq_ngram_offset || size_second_l == 0 || size_second_u == 0) return false;
...
class VolnitskyBase {
 ...
 public:
  ...
  VolnitskyBase(const char *const needle_, const size_t needle_size_, size_t haystack_size_hint = 0)
    ...
    if (fallback || fallback_searcher.getForceFallback()) return;
 ...
 protected:
  void putNGramBase(const VolnitskyTraits::Ngram ngram, const int offset) {
    ...
    while (hash[cell_num]) cell_num = (cell_num + 1) % VolnitskyTraits::hash_size;  /// Search for the next free cell.
 ...
 public:
    ...
    for (; last < size; ++last) {
      ...
      if (VolnitskyTraits::isFallbackNeedle(cur_needle_size)) {
        ...
        for (auto i = static_cast<int>(cur_needle_size - sizeof(VolnitskyTraits::Ngram)); i >= 0; --i) {
          VolnitskyTraits::putNGram<CaseSensitive, ASCII>(reinterpret_cast<const UInt8 *>(cur_needle_data) + i, i + 1,
                                                          reinterpret_cast<const UInt8 *>(cur_needle_data), cur_needle_size, callback);
        }
      }
  ...
  bool searchOne(const UInt8 *haystack, const UInt8 *haystack_end) const {
    ...
    for (size_t i = 0; i < fallback_size; ++i)
      if (fallback_searchers[fallback_needles[i]].search(haystack, haystack_end) != haystack_end) return true;
      ...
      for (; pos <= haystack_end - sizeof(VolnitskyTraits::Ngram); pos += step) {
        for (size_t cell_num = VolnitskyTraits::toNGram(pos) % VolnitskyTraits::hash_size; hash[cell_num].off;
          ...
          if (pos >= haystack + hash[cell_num].off - 1) {
            ...
            if (res + needles[ind].size() <= haystack_end && fallback_searchers[ind].compare(haystack, haystack_end, res)) return true;
          }
        }
      }
    }
