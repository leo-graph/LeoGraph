...
/// Pools are tried in the order consistent with lexicographical order of (error count, slowdown count, config priority, priority, random
/// number) tuples. Number of tries for a single pool is limited by max_tries parameter. The client can set nested pool priority by passing
/// a GetPriority functor.
///
...
class PoolWithFailoverBase : private boost::noncopyable {
 public:
  ...
  PoolWithFailoverBase(NestedPools nested_pools_, time_t decrease_error_period_, size_t max_error_cap_, LoggerPtr log_)
      : nested_pools(std::move(nested_pools_)),
        ...
        log(log_) {
    for (size_t i = 0; i < nested_pools.size(); ++i) shared_pool_states[i].config_priority = nested_pools[i]->getConfigPriority();
  }
  ...
  std::vector<TryResult> getMany(size_t min_entries, size_t max_entries, size_t max_tries, size_t max_ignored_errors,
                                 bool fallback_to_stale_replicas, bool skip_read_only_replicas, const TryGetEntryFunc& try_get_entry,
  ...
  TryResult getValidTryResult(const std::vector<TryResult>& results, bool skip_read_only_replicas) const {
    if (results.empty())
      throw DB::Exception(DB::ErrorCodes::ALL_CONNECTION_TRIES_FAILED,
                          "Cannot get any valid connection because all connection tries failed");

    auto result = results.front();
    if (isTryResultInvalid(result, skip_read_only_replicas))
      throw DB::Exception(DB::ErrorCodes::LOGICAL_ERROR,
                          "Got an invalid connection result: entry.isNull {}, is_usable {}, is_up_to_date {}, delay {}, is_readonly {}, "
                          "skip_read_only_replicas {}",
                          result.entry.isNull(), result.is_usable, result.is_up_to_date, result.delay, result.is_readonly,
                          skip_read_only_replicas);
 ...
 protected:
  ...
  Entry get(size_t max_ignored_errors, bool fallback_to_stale_replicas, const TryGetEntryFunc& try_get_entry,
...
template <typename TNestedPool>
std::vector<typename PoolWithFailoverBase<TNestedPool>::ShuffledPool> PoolWithFailoverBase<TNestedPool>::getShuffledPools(
  ...
  if (get_priority) {
    for (size_t i = 0; i < pool_states.size(); ++i) pool_states[i].priority = get_priority(i);
  }
  ...
  ::sort(shuffled_pools.begin(), shuffled_pools.end(), [use_slowdown_count](const ShuffledPool& lhs, const ShuffledPool& rhs) {
...
template <typename TNestedPool>
typename TNestedPool::Entry PoolWithFailoverBase<TNestedPool>::get(size_t max_ignored_errors, bool fallback_to_stale_replicas,
                                                                   const TryGetEntryFunc& try_get_entry,
                                                                   const GetPriorityFunc& get_priority) {
  std::vector<TryResult> results = getMany(
      /* min_entries= */ 1,
      /* max_entries= */ 1,
      /* max_tries= */ 1, max_ignored_errors, fallback_to_stale_replicas,
      /* skip_read_only_replicas= */ false, try_get_entry, get_priority);
  if (results.empty() || results[0].entry.isNull())
    throw DB::Exception(DB::ErrorCodes::LOGICAL_ERROR, "PoolWithFailoverBase::getMany() returned less than min_entries entries.");
...
std::vector<typename PoolWithFailoverBase<TNestedPool>::TryResult> PoolWithFailoverBase<TNestedPool>::getMany(
    size_t min_entries, size_t max_entries, size_t max_tries, size_t max_ignored_errors, bool fallback_to_stale_replicas,
    bool skip_read_only_replicas, const TryGetEntryFunc& try_get_entry, const GetPriorityFunc& get_priority) {
  ...
  SCOPE_EXIT({ updateSharedErrorCounts(shuffled_pools); });
  ...
  while (!finished) {
    for (size_t i = 0; i < shuffled_pools.size(); ++i) {
      ...
      if (max_tries && (shuffled_pool.error_count >= max_tries || !result.entry.isNull())) continue;

      std::string fail_message;
      result = try_get_entry(shuffled_pool.pool, fail_message);

      if (!fail_message.empty()) fail_messages += fail_message + '\n';
      ...
      if (!result.entry.isNull()) {
        ...
        if (result.is_usable) {
          if (skip_read_only_replicas && result.is_readonly)
            ...
            ++usable_count;
            if (result.is_up_to_date) ++up_to_date_count;
          }
        }
      } else {
  ...
  if (usable_count < min_entries)
    throw DB::NetException(DB::ErrorCodes::ALL_CONNECTION_TRIES_FAILED, "All connection tries failed. Log: \n\n{}\n", fail_messages);
  ...
  std::stable_sort(try_results.begin(), try_results.end(), [](const TryResult& left, const TryResult& right) {
    return std::forward_as_tuple(!left.is_up_to_date, left.delay) < std::forward_as_tuple(!right.is_up_to_date, right.delay);
  });
  ...
  if (fallback_to_stale_replicas) {
    ...
    if (try_results.size() < up_to_date_count)
      throw DB::Exception(DB::ErrorCodes::LOGICAL_ERROR, "Could not find enough connections for up-to-date results. Got: {}, needed: {}",
                          try_results.size(), up_to_date_count);
...
template <typename TNestedPool>
typename PoolWithFailoverBase<TNestedPool>::PoolStates PoolWithFailoverBase<TNestedPool>::updatePoolStates(size_t max_ignored_errors) {
  PoolStates result;
...

  {
    ...
    for (auto& state : shared_pool_states) state.randomize();
  ...
  for (auto& state : result) state.error_count = state.error_count > max_ignored_errors ? state.error_count - max_ignored_errors : 0;

  return result;
}

template <typename TNestedPool>
void PoolWithFailoverBase<TNestedPool>::updateErrorCounts(PoolWithFailoverBase<TNestedPool>::PoolStates& states,
                                                          time_t& last_decrease_time) const {
  ...
  if (last_decrease_time) {
    ...
    if (delta >= 0) {
      ...
      if (shift_amount) last_decrease_time = current_time;
