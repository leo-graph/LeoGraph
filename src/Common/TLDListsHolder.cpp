...
/// TLDList
TLDList::TLDList(size_t size) : tld_container(size), memory_pool(std::make_unique<Arena>()) {
...
TLDType TLDList::lookup(std::string_view host) const {
  if (auto it = tld_container.find(host); it != nullptr) return it->getMapped();
...
size_t TLDListsHolder::parseAndAddTldList(const std::string& name, const std::string& path) {
  ...
  if (!in.eof()) throw Exception(ErrorCodes::LOGICAL_ERROR, "Not all list had been read: {}", name);
...
const TLDList& TLDListsHolder::getTldList(const std::string& name) {
  ...
  if (it == tld_lists_map.end()) throw Exception(ErrorCodes::TLD_LIST_NOT_FOUND, "TLD list {} does not exist", name);
  return it->second;
}
