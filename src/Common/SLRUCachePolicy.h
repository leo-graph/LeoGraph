...
class SLRUCachePolicy : public ICachePolicy<Key, Mapped, HashFunction, WeightFunction> {
 public:
  ...
  SLRUCachePolicy(CurrentMetrics::Metric size_in_bytes_metric_, CurrentMetrics::Metric count_metric_, size_t max_size_in_bytes_,
                  size_t max_count_, double size_ratio_, OnRemoveEntryFunction on_remove_entry_function_)
  ...
  ~SLRUCachePolicy() override { clearImpl(); }
  ...
  void remove(const Key &key) override {
    ...
    if (cell.is_protected) current_protected_size -= cell.size;
  ...
  bool contains(const Key &key) const override { return cells.count(key) != 0; }
  ...
  std::vector<KeyMapped> dump() const override {
    ...
    for (const auto &[key, cell] : cells) res.push_back({key, cell.value});
    return res;
  }
 ...
 private:
  ...
  void removeOverflow(SLRUQueue &queue, size_t max_weight_size, size_t &current_weight_size, bool is_protected) {
    ...
    if (is_protected) {
      ...
      need_remove = [&]() {
        return ((max_count != 0 && cells.size() - probationary_queue.size() > max_count) || (current_weight_size > max_weight_size)) &&
               (queue_size > 0);
      };
    } else {
      need_remove = [&]() {
    ...
    while (need_remove()) {
      ...
      auto it = cells.find(key);
      if (it == cells.end()) std::terminate();  // Queue became inconsistent
      ...
      if (cell.is_protected) {
        ...
        if (on_remove_entry_function) on_remove_entry_function(cell.size, cell.value);
    ...
    if (current_size_in_bytes > (1ull << 63)) std::terminate();  // Queue became inconsistent
