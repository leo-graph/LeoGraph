...
struct ScopedDecrement {
  ...
  // Move constructor
  ScopedDecrement(ScopedDecrement&& other) noexcept : atomic_var(std::move(other.atomic_var)) { other.atomic_var.reset(); }
  ...
  explicit ScopedDecrement(std::atomic<int64_t>& var) : atomic_var(var) { atomic_var->get().fetch_sub(1, std::memory_order_relaxed); }

  ~ScopedDecrement() {
    if (atomic_var) atomic_var->get().fetch_add(1, std::memory_order_relaxed);
  }
};
}  // namespace
...
class JobWithPriority {
 public:
  ...
  JobWithPriority(Job job_, Priority priority_, CurrentMetrics::Metric metric,
                  const DB::OpenTelemetry::TracingContextOnThread& thread_trace_context_, bool capture_frame_pointers,
                  ScopedDecrement available_threads_decrement_)
      : job(job_),
        priority(priority_),
    ...
    if (!capture_frame_pointers) return;
  ...
  UInt64 elapsedMicroseconds() const { return job_create_time.elapsedMicroseconds(); }
};
...
ReturnType ThreadPoolImpl<Thread>::scheduleImpl(Job job, Priority priority, std::optional<uint64_t> wait_microseconds,
  ...
  auto on_error = [&](const std::string& reason) {
    if constexpr (std::is_same_v<ReturnType, void>) {
      ...
      throw DB::Exception(DB::ErrorCodes::CANNOT_SCHEDULE_TASK, "Cannot schedule a task: {} (threads={}, jobs={})", reason, threads.size(),
                          scheduled_jobs);
    } else
      return false;
  };
...

  {
    ...
    ProfileEvents::increment(std::is_same_v<Thread, std::thread> ? ProfileEvents::GlobalThreadPoolLockWaitMicroseconds
                                                                 : ProfileEvents::LocalThreadPoolLockWaitMicroseconds,
                             watch.elapsedMicroseconds());

    if (CannotAllocateThreadFaultInjector::injectFault()) return on_error("fault injected");
    ...
    if (shutdown) return on_error("shutdown");
    ...
    try {
      jobs.emplace(
          std::move(job), priority, metric_scheduled_jobs,
          ...
          DB::Exception::enable_job_stack_trace, std::move(available_threads_decrement));

      ++scheduled_jobs;

      if (adding_new_thread) (*thread_slot)->start(thread_slot);

    } catch (const std::exception&) {
      if (adding_new_thread) threads.pop_front();
...
void ThreadPoolImpl<Thread>::startNewThreadsNoLock() {
  ...
  while (threads.size() < std::min(scheduled_jobs, max_threads)) {
    ...
    if (!new_thread) break;  /// failed to start more threads
...
template <typename Thread>
void ThreadPoolImpl<Thread>::scheduleOrThrow(Job job, Priority priority, uint64_t wait_microseconds,
                                             bool propagate_opentelemetry_tracing_context) {
      ...
      thread_state(ThreadState::Preparing)  // Initial state is Preparing
{
  ...
  ProfileEvents::increment(std::is_same_v<Thread, std::thread> ? ProfileEvents::GlobalThreadPoolThreadCreationMicroseconds
                                                               : ProfileEvents::LocalThreadPoolThreadCreationMicroseconds,
                           watch2.elapsedMicroseconds());
  ProfileEvents::increment(std::is_same_v<Thread, std::thread> ? ProfileEvents::GlobalThreadPoolExpansions
                                                               : ProfileEvents::LocalThreadPoolExpansions);
...
ThreadPoolImpl<Thread>::ThreadFromThreadPool::~ThreadFromThreadPool() {
  ...
  ProfileEvents::increment(std::is_same_v<Thread, std::thread> ? ProfileEvents::GlobalThreadPoolShrinks
                                                               : ProfileEvents::LocalThreadPoolShrinks);
}
...
void ThreadPoolImpl<Thread>::ThreadFromThreadPool::worker() {
  ...
  if (thread_state.load(std::memory_order_relaxed) == ThreadState::Destructing) return;
  ...
  /// We'll run jobs in this worker while there are scheduled jobs and until some special event occurs (e.g. shutdown, or decreasing the
  /// number of max_threads). And if `max_free_threads > 0` we keep this number of threads even when there are no jobs for them currently.
  while (true) {
...

    {
      ...
      if (job_is_done) {
        ...
        if (exception_from_job) {
          if (!parent_pool.first_exception) parent_pool.first_exception = exception_from_job;
      ...
      if (parent_pool.jobs.empty() ||
        ...
        if (parent_pool.threads_remove_themselves) removeSelfFromPoolNoPoolLock();  // Detach and remove itself from the pool
      ...
      ProfileEvents::increment(std::is_same_v<Thread, std::thread> ? ProfileEvents::GlobalThreadPoolJobWaitTimeMicroseconds
                                                                   : ProfileEvents::LocalThreadPoolJobWaitTimeMicroseconds,
    ...
    if (DB::Exception::enable_job_stack_trace) DB::Exception::setThreadFramePointers(std::move(job_data->frame_pointers));
    ...
    try {
      ...
      if (final_thread != initial_thread || final_thread_group != initial_thread_group)
        throw DB::Exception(DB::ErrorCodes::LOGICAL_ERROR,
                            ...
                            initial_thread ? "non-nullptr" : "nullptr", final_thread ? "non-nullptr" : "nullptr",
                            initial_thread_group ? "master_thread_id " + std::to_string(initial_thread_group->master_thread_id) : "nullptr",
                            final_thread_group ? "master_thread_id " + std::to_string(final_thread_group->master_thread_id) : "nullptr");
#endif
...
GlobalThreadPool::GlobalThreadPool(size_t max_threads_, size_t max_free_threads_, size_t queue_size_, const bool shutdown_on_exception_,
                                   UInt64 global_profiler_real_time_period_ns_, UInt64 global_profiler_cpu_time_period_ns_)
    : FreeThreadPool(CurrentMetrics::GlobalThread, CurrentMetrics::GlobalThreadActive, CurrentMetrics::GlobalThreadScheduled, max_threads_,
                     max_free_threads_, queue_size_, shutdown_on_exception_),
...
void GlobalThreadPool::initialize(size_t max_threads, size_t max_free_threads, size_t queue_size,
                                  UInt64 global_profiler_real_time_period_ns, UInt64 global_profiler_cpu_time_period_ns) {
  if (the_instance) {
  ...
  the_instance.reset(new GlobalThreadPool(max_threads, max_free_threads, queue_size, false /*shutdown_on_exception*/,
                                          global_profiler_real_time_period_ns, global_profiler_cpu_time_period_ns));
}
...
bool CannotAllocateThreadFaultInjector::injectFault() {
  auto& ins = instance();
  if (!ins.enabled.load(std::memory_order_relaxed)) return false;

  if (ins.block_fault_injections) return false;
