...
ThreadFuzzer::ThreadFuzzer() {
  initConfiguration();
  if (isEffective()) started.store(true, std::memory_order_relaxed);
}
...
bool ThreadFuzzer::needsSetup() const {
  return cpu_time_period_us != 0 && (yield_probability > 0 || migrate_probability > 0 || (sleep_probability > 0 && sleep_time_us_max > 0));
}
...
#  define CHECK_WRAPPER_PARAMS(RET, NAME, ...)                                                \
    if (NAME##_before_yield_probability.load(std::memory_order_relaxed) > 0.0) return true;   \
    if (NAME##_before_migrate_probability.load(std::memory_order_relaxed) > 0.0) return true; \
    if (NAME##_before_sleep_probability.load(std::memory_order_relaxed) > 0.0) return true;   \
    if (NAME##_before_sleep_time_us_max.load(std::memory_order_relaxed) > 0.0) return true;   \
                                                                                              \
    if (NAME##_after_yield_probability.load(std::memory_order_relaxed) > 0.0) return true;    \
    if (NAME##_after_migrate_probability.load(std::memory_order_relaxed) > 0.0) return true;  \
    if (NAME##_after_sleep_probability.load(std::memory_order_relaxed) > 0.0) return true;    \
    if (NAME##_after_sleep_time_us_max.load(std::memory_order_relaxed) > 0.0) return true;
...
#  undef INIT_WRAPPER_PARAMS
  ...
  if (explicit_sleep_probability > 0 && sleep_time_us_max > 0) return true;

  if (explicit_memory_exception_probability > 0) return true;

  return false;
}

void ThreadFuzzer::stop() { started.store(false, std::memory_order_relaxed); }

void ThreadFuzzer::start() {
  if (!instance().isEffective()) return;
  started.store(true, std::memory_order_relaxed);
}

bool ThreadFuzzer::isStarted() { return started.load(std::memory_order_relaxed); }

static void injectionImpl(double yield_probability, double migrate_probability, double sleep_probability, double sleep_time_us_max) {
  DENY_ALLOCATIONS_IN_SCOPE;
  if (!ThreadFuzzer::isStarted()) return;

  if (yield_probability > 0 && std::bernoulli_distribution(yield_probability)(thread_local_rng)) {
    sched_yield();
  }

#if defined(OS_LINUX)
  int num_cpus_loaded = num_cpus.load(std::memory_order_relaxed);
  if (num_cpus_loaded > 0 && migrate_probability > 0 && std::bernoulli_distribution(migrate_probability)(thread_local_rng)) {
...
static ALWAYS_INLINE void injection(double yield_probability, double migrate_probability, double sleep_probability,
...
void ThreadFuzzer::maybeInjectMemoryLimitException() {
  auto& fuzzer = ThreadFuzzer::instance();
  if (fuzzer.explicit_memory_exception_probability <= 0.0) return;
  std::bernoulli_distribution fault(fuzzer.explicit_memory_exception_probability);
  if (fault(thread_local_rng)) CurrentMemoryTracker::injectFault();
}
...
#if defined(OS_LINUX)
  if (sigemptyset(&sa.sa_mask)) throw ErrnoException(ErrorCodes::CANNOT_MANIPULATE_SIGSET, "Failed to clean signal mask for thread fuzzer");
...
#  define INJECTION_BEFORE(NAME)                                                     \
    injectionImpl(NAME##_before_yield_probability.load(std::memory_order_relaxed),   \
...
#  define INJECTION_AFTER(NAME)                                                                                                           \
    injectionImpl(                                                                                                                        \
        NAME##_after_yield_probability.load(std::memory_order_relaxed), NAME##_after_migrate_probability.load(std::memory_order_relaxed), \
        NAME##_after_sleep_probability.load(std::memory_order_relaxed), NAME##_after_sleep_time_us_max.load(std::memory_order_relaxed));
...
#    define MAKE_WRAPPER_USING_DLSYM(RET, NAME, ...)                                       \
      ...
      extern "C" RET NAME(__VA_ARGS__) {                                                   \
        ...
        if (thread_fuzzer_enabled) INJECTION_BEFORE(NAME);                                 \
        if (unlikely(!real_##NAME)) {                                                      \
          real_##NAME = reinterpret_cast<RET (*)(__VA_ARGS__)>(getFunctionAddress(#NAME)); \
        }                                                                                  \
        auto&& ret{real_##NAME(arg)};                                                      \
        if (thread_fuzzer_enabled) INJECTION_AFTER(NAME);                                  \
        return ret;                                                                        \
      }
