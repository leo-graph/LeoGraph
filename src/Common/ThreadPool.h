...
class ThreadPoolImpl {
 public:
  ...
  class ThreadFromThreadPool {
   ...
   private:
    ...
    enum class ThreadState { Preparing, Running, Destructing };
  ...
  explicit ThreadPoolImpl(Metric metric_threads_, Metric metric_active_threads_, Metric metric_scheduled_jobs_, size_t max_threads_);

  /// queue_size - maximum number of running plus scheduled jobs. It can be greater than max_threads. Zero means unlimited.
  ThreadPoolImpl(Metric metric_threads_, Metric metric_active_threads_, Metric metric_scheduled_jobs_, size_t max_threads_,
                 size_t max_free_threads_, size_t queue_size_, bool shutdown_on_exception_ = true);
  ...
  void scheduleOrThrow(Job job, Priority priority = {}, uint64_t wait_microseconds = 0,
 ...
 private:
  ...
  ReturnType scheduleImpl(Job job, Priority priority, std::optional<uint64_t> wait_microseconds,
                          bool propagate_opentelemetry_tracing_context = true);

  /// Tries to start new threads if there are scheduled jobs and the limit `max_threads` is not reached. Must be called with the mutex
  /// locked.
...
class GlobalThreadPool : public FreeThreadPool, private boost::noncopyable {
  ...
  GlobalThreadPool(size_t max_threads_, size_t max_free_threads_, size_t queue_size_, bool shutdown_on_exception_,
 ...
 public:
  ...
  static void initialize(size_t max_threads = 10000, size_t max_free_threads = 1000, size_t queue_size = 10000,
                         UInt64 global_profiler_real_time_period_ns_ = 0, UInt64 global_profiler_cpu_time_period_ns_ = 0);
...
class ThreadFromGlobalPoolImpl : boost::noncopyable {
 public:
  ...
  explicit ThreadFromGlobalPoolImpl(Function&& func, Args&&... args) : state(std::make_shared<State>()) {
    ...
    GlobalThreadPool::instance().scheduleOrThrow(
        [my_state = state, global_profiler_real_time_period, global_profiler_cpu_time_period, my_func = std::forward<Function>(func),
         my_args = std::make_tuple(std::forward<Args>(args)...)]() mutable  /// mutable is needed to destroy capture
        {
          SCOPE_EXIT(my_state->thread_id = std::thread::id(); my_state->event.set(););
  ...
  void join() {
    ...
    if (state->thread_id == std::this_thread::get_id()) abort();
  ...
  bool joinable() const { return initialized(); }

  std::thread::id get_id() const { return state ? state->thread_id.load() : std::thread::id{}; }
...
/// Schedule jobs/tasks on global thread pool without implicit passing tracing context on current thread to underlying worker as parent
/// tracing context.
///
...
/// An alias of thread that execute jobs/tasks on global thread pool by implicit passing tracing context on current thread to underlying
/// worker as parent tracing context. If jobs/tasks are directly scheduled by using APIs of this class, you need to use this class or you
/// need to use class above.
...
/// Because ThreadFromGlobalPool schedules a job upon GlobalThreadPool, this means there will be two workers to schedule a job in
/// 'ThreadPool', one is at GlobalThreadPool level, the other is at ThreadPool level, so tracing context will be initialized on the same
/// thread twice.
///
...
///
/// To make sure the tracing context is correctly propagated, we explicitly disable context propagation(including initialization and
/// de-initialization) at underlying worker level.
///
