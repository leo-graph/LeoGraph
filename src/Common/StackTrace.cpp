...
bool shouldShowAddress(const void *addr) {
  ...
  if (reinterpret_cast<uintptr_t>(addr) < 4096) return true;

  return show_addresses.load(std::memory_order_relaxed);
}
}  // namespace

StackTrace::StackTrace() { tryCapture(); }

void StackTrace::setShowAddresses(bool show) { show_addresses.store(show, std::memory_order_relaxed); }
...
std::string signalToErrorMessage(int sig, const siginfo_t &info, [[maybe_unused]] const ucontext_t &context) {
  ...
  if (sig == SIGSEGV || sig == SIGBUS) {
    ...
    if (access.empty()) access = "<not available>";
    return fmt::format("Address: {}. Access: {}. {}", address, access, message);
  }

  if (sig == SIGTSTP) return "This is a signal used for debugging purposes by the user.";
...
std::optional<UInt64> getFaultAddress(int sig, const siginfo_t &info) {
  ...
  if (sig != SIGSEGV && sig != SIGBUS && sig != SIGILL && sig != SIGFPE) return std::nullopt;

  if (info.si_addr == nullptr) return 0;

  if (shouldShowAddress(info.si_addr)) return reinterpret_cast<UInt64>(info.si_addr);
...
void StackTrace::forEachFrame(const FramePointers &frame_pointers, size_t offset, size_t size, std::function<void(const Frame &)> callback,
                              bool fatal) {
  ...
  for (size_t i = offset; i < size; ++i) {
    ...
    if (const auto *symbol = symbol_index.findSymbol(current_frame.physical_addr)) current_frame.symbol = demangle(symbol->name);
...
StackTrace::StackTrace(FramePointers frame_pointers_, size_t size_, size_t offset_)
    : size(size_), offset(offset_), frame_pointers(std::move(frame_pointers_)) {}
...
String collapseDemangledNames(std::optional<std::string_view> file, String symbol_name) {
  ...
  if (file.has_value()) {
    ...
    if (file_copy.ends_with("functional")) return "?";
  }
...
static void toStringEveryLineImpl([[maybe_unused]] bool fatal, const StackTraceRefTriple &stack_trace,
                                  Fn<void(std::string_view)> auto &&callback) {
  if (stack_trace.size == 0) return callback("<Empty trace>");
  ...
  auto callback_wrapper = [&](const StackTrace::Frame &frame) {
    ...
    if (frame.file.has_value() && frame.line.has_value()) {
      ...
      if (frame.column.has_value() && *frame.column > 0) out << ':' << *frame.column;
      out << ": ";
    }
...
#else
  auto callback_wrapper = [&](const StackTrace::Frame &frame) {
    if (frame.virtual_addr && shouldShowAddress(frame.virtual_addr)) callback(fmt::format("{}. {}", frame_index++, frame.virtual_addr));
  };
#endif
...
void StackTrace::toStringEveryLine(const FramePointers &frame_pointers, std::function<void(std::string_view)> callback) {
  toStringEveryLineImpl(true, {frame_pointers, 0, static_cast<size_t>(std::ranges::find(frame_pointers, nullptr) - frame_pointers.begin())},
                        std::move(callback));
}
...
struct StackTraceCache : public StackTraceCacheBase {
  StackTraceCache() : StackTraceCacheBase() { can_use_cache = true; }

  ~StackTraceCache() { can_use_cache = false; }
};
...
static String toStringCached(const FramePointers &pointers, size_t offset, size_t size) {
  ...
  // Optimistic try for cache hit to avoid any contention whatsoever, should be the main hot code route
  {
    ...
    if (auto it = cache.find(key); it != cache.end()) cache_entry = it->second;
  }
...
std::string StackTrace::toString() const { return toStringCached(frame_pointers, offset, size); }
