...
class HostResolver : public std::enable_shared_from_this<HostResolver> {
  ...
  class Entry {
   ...
   private:
    ...
    Entry(HostResolver& pool_, Poco::Net::IPAddress address_)
        : pool(pool_.getWeakFromThis()), address(address_), resolved_host(address.toString()) {}
 ...
 protected:
  explicit HostResolver(String host_, Poco::Timespan history_ = Poco::Timespan(DEFAULT_RESOLVE_TIME_HISTORY_SECONDS, 0));

  using ResolveFunction = std::function<std::vector<Poco::Net::IPAddress>(const String& host)>;
  HostResolver(ResolveFunction&& resolve_function_, String host_, Poco::Timespan history_);
  ...
  struct Record {
    Record(Poco::Net::IPAddress address_, Poco::Timestamp resolve_time_) : address(address_), resolve_time(resolve_time_) {}
    ...
    bool operator<(const Record& r) const { return address < r.address; }
    ...
    size_t getWeight() const {
      ...
      if (usage > 10000) return 1;
      if (usage > 1000) return 5;
      if (usage > 100) return 8;
      return 10;
    }
    ...
    bool setFail(const Poco::Timestamp& now) {
      ...
      if (was_ok) {
        if (consecutive_fail_count < RECORD_CONSECTIVE_FAIL_COUNT_LIMIT) ++consecutive_fail_count;
      }
