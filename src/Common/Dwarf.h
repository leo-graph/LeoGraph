...
class Dwarf final {
 ...
 private:
  ...
  bool findLocation(uintptr_t address, LocationInfoMode mode, CompilationUnit& cu, LocationInfo& info,
   ...
   */
  void findSubProgramDieForAddress(const CompilationUnit& cu, const Die& die, uint64_t address, std::optional<uint64_t> base_addr_cu,
                                   Die& subprogram) const;

  // Interpreter for the line number bytecode VM
  class LineNumberVM {
   public:
    LineNumberVM(std::string_view data, std::string_view compilationDirectory, std::string_view debugStr, std::string_view debugLineStr);
   ...
   */
  void findInlinedSubroutineDieForAddress(const CompilationUnit& cu, const Die& die, const LineNumberVM& line_vm, uint64_t address,
  ...
  Attribute readAttribute(const CompilationUnit& cu, const Die& die, AttributeSpec spec, std::string_view& info) const;
  ...
  std::optional<std::pair<std::optional<CompilationUnit>, uint64_t>> getReferenceAttribute(const CompilationUnit& cu, const Die& die,
                                                                                           uint64_t attr_name) const;

  // Check if the given address is in the range list at the given offset in .debug_ranges.
  bool isAddrInRangeList(const CompilationUnit& cu, uint64_t address, std::optional<uint64_t> base_addr, size_t offset,
