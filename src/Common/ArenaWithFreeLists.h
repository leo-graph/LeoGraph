...
class ArenaWithFreeLists : private Allocator<false>, private boost::noncopyable {
 private:
  /// If the block is free, then the pointer to the next free block is stored at its beginning, or nullptr, if there are no more free
  /// blocks. If the block is used, then some data is stored in it.
  union Block {
    Block *next;
    char data[0];
  };
 ...
 public:
  ...
  char *alloc(const size_t size) {
    if (size > max_fixed_block_size) return static_cast<char *>(Allocator<false>::alloc(size));
...
class SynchronizedArenaWithFreeLists : private ArenaWithFreeLists {
 public:
  explicit SynchronizedArenaWithFreeLists(const size_t initial_size = 4096, const size_t growth_factor = 2,
                                          const size_t linear_growth_threshold = 128 * 1024 * 1024)
