...
void abortOnFailedAssertion(const String &description, std::string_view format_string, void *const *trace, size_t trace_offset,
                            size_t trace_size) {
  auto &logger = Poco::Logger::root();
  LOG_FATAL(&logger, "Logical error: '{}'.", description);
  if (!format_string.empty()) LOG_FATAL(&logger, "Format string: '{}'.", format_string);
  if (trace)
...
static size_t handle_error_code(const std::string &msg, std::string_view format_string, int code, bool remote,
                                const Exception::Trace &trace) {
...
Exception::MessageMasked::MessageMasked(const std::string &msg_, std::string format_string_)
  ...
  if (auto masker = SensitiveDataMasker::getInstance()) masker->wipeSensitiveData(msg);
}
...
Exception::Exception(const MessageMasked &msg_masked, int code, bool remote_) : Poco::Exception(msg_masked.msg, code), remote(remote_) {
  if (terminate_on_any_exception) std::_Exit(terminate_status_code);
...
Exception::Exception(MessageMasked &&msg_masked, int code, bool remote_)
    : Poco::Exception(std::move(msg_masked.msg), code), remote(remote_) {
  if (terminate_on_any_exception) std::_Exit(terminate_status_code);
  capture_thread_frame_pointers = getThreadFramePointers();
  message_format_string = msg_masked.format_string;
...
Exception::Exception(CreateFromPocoTag, const Poco::Exception &exc) : Poco::Exception(exc.displayText(), ErrorCodes::POCO_EXCEPTION) {
  if (terminate_on_any_exception) std::_Exit(terminate_status_code);
  capture_thread_frame_pointers = getThreadFramePointers();
...
static int getCodeForSTDException(const std::exception &exc) {
  ...
  if (name.starts_with("avro::")) return ErrorCodes::AVRO_EXCEPTION;
...
void Exception::addMessage(const MessageMasked &msg_masked) {
  ...
  if (error_index != static_cast<size_t>(-1)) ErrorCodes::extendedMessage(code(), remote, error_index, message());
}
...
Exception::ThreadFramePointers::ThreadFramePointers() { can_use_thread_frame_pointers = true; }

Exception::ThreadFramePointers::~ThreadFramePointers() { can_use_thread_frame_pointers = false; }

const Exception::ThreadFramePointersBase &Exception::getThreadFramePointers() {
  if (can_use_thread_frame_pointers) return thread_frame_pointers.frame_pointers;

  return dummy_frame_pointers;
}

void Exception::setThreadFramePointers(ThreadFramePointersBase frame_pointers) {
  if (can_use_thread_frame_pointers) thread_frame_pointers.frame_pointers = std::move(frame_pointers);
}

void Exception::clearThreadFramePointers() {
  if (can_use_thread_frame_pointers) thread_frame_pointers.frame_pointers.clear();
}

bool Exception::isErrorCodeImportant() const {
  const int error_code = code();
  return error_code == ErrorCodes::LOGICAL_ERROR || error_code == ErrorCodes::POTENTIALLY_BROKEN_DATA_PART ||
...
static void getNoSpaceLeftInfoMessage(std::filesystem::path path, String &msg) {
  ...
  while (!std::filesystem::exists(path) && path.has_relative_path()) path = path.parent_path();

  auto fs = getStatVFS(path);
  auto mount_point = getMountPoint(path).string();

  fmt::format_to(std::back_inserter(msg), "\nTotal space: {}\nAvailable space: {}\nTotal inodes: {}\nAvailable inodes: {}\nMount point: {}",
                 ReadableSize(fs.f_blocks * fs.f_frsize), ReadableSize(fs.f_bavail * fs.f_frsize), formatReadableQuantity(fs.f_files),
                 formatReadableQuantity(fs.f_favail), mount_point);
  ...
  try {
    ...
    UInt64 num_maps = 0;
    {
      ...
      while (!file.eof()) {
        ...
        if (!file.hasPendingData()) continue;
...
std::string getExtraExceptionInfo(const std::exception &e) {
  ...
  try {
    if (const auto *file_exception = dynamic_cast<const fs::filesystem_error *>(&e)) {
      ...
      else if (errno_exception->code() == ErrorCodes::CANNOT_ALLOCATE_MEMORY || errno_exception->code() == ErrorCodes::CANNOT_MREMAP)
...
PreformattedMessage getCurrentExceptionMessageAndPattern(bool with_stacktrace, bool check_embedded_stacktrace /*= false*/,
                                                         bool with_extra_info /*= true*/, bool with_version /*= true*/) {
  /// Explicitly block MEMORY_LIMIT_EXCEEDED
  LockMemoryExceptionInThread lock_memory_tracker(VariableContext::Global);
  ...
  try {
    ...
    if (with_version) stream << " (version " << VERSION_STRING << VERSION_OFFICIAL << ")";
    message_format_string = e.tryGetMessageFormatString();
    message_format_string_args = e.getMessageFormatStringArgs();
  } catch (const Poco::Exception &e) {
    try {
      stream << "Poco::Exception. Code: " << ErrorCodes::POCO_EXCEPTION << ", e.code() = " << e.code() << ", " << e.displayText()
             << (with_stacktrace
    ...
    }  // NOLINT(bugprone-empty-catch) Ok: best-effort exception formatting, must not throw
  } catch (const std::exception &e) {
    try {
      int status = 0;
      auto name = demangle(typeid(e).name(), status);

      if (status) name += " (demangling status: " + toString(status) + ")";
    ...
    if (debug_or_sanitizer_build || abort_on_logical_error.load(std::memory_order_relaxed)) {
      try {
        ...
        if (!with_stacktrace) stream << ", Stack trace:\n\n" << getExceptionStackTraceString(e);
...
PreformattedMessage getExceptionMessageAndPattern(const Exception &e, bool with_stacktrace, bool check_embedded_stacktrace) {
  ...
  try {
    ...
    if (!text.empty() && text.back() != '.') stream << '.';
...
ExecutionStatus ExecutionStatus::fromCurrentException(const std::string &start_of_message, bool with_stacktrace, bool with_version) {
  String msg = (start_of_message.empty() ? "" : (start_of_message + ": ")) +
               getCurrentExceptionMessage(with_stacktrace, /*check_embedded_stacktrace=*/true, /*with_extra_info=*/true, with_version);
