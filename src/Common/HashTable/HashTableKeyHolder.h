...
struct ArenaKeyHolder {
  ...
  ArenaKeyHolder(const std::string_view key_, Arena &pool_, std::unique_ptr<char[]> holder_ = {})
      : key(key_), pool(pool_), holder(std::move(holder_)) {}
};

}  // namespace DB

inline std::string_view &ALWAYS_INLINE keyHolderGetKey(DB::ArenaKeyHolder &holder) { return holder.key; }
...
inline std::string_view &ALWAYS_INLINE keyHolderGetKey(DB::SerializedKeyHolder &holder) { return holder.key; }
