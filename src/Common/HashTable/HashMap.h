...
template <typename Key, typename TMapped, typename Hash, typename TState, typename Pair>
struct tuple_element<0, HashMapCell<Key, TMapped, Hash, TState, Pair>> {
  using type = Key;
};

template <typename Key, typename TMapped, typename Hash, typename TState, typename Pair>
struct tuple_element<1, HashMapCell<Key, TMapped, Hash, TState, Pair>> {
  using type = TMapped;
};
}  // namespace std
...
template <typename Key, typename Cell, typename Hash = DefaultHash<Key>, typename Grower = HashTableGrowerWithPrecalculation<>,
 ...
 public:
  ...
  void forEachValue(Func &&func) {
    ...
    if (CouldPrefetchKey<Cell> && this->size() > DB::PrefetchingHelper::iterationsToMeasure() * 2) {
      ...
      for (; it != end; ++it) func(it->getKey(), it->getMapped());
    } else {
      for (auto &it : *this) func(it.getKey(), it.getMapped());
    }
  }
  ...
  typename Cell::Mapped &ALWAYS_INLINE operator[](const Key &x) {
     ...
     */
    if (inserted) new (reinterpret_cast<void *>(&it->getMapped())) typename Cell::Mapped();
  ...
  const typename Cell::Mapped &ALWAYS_INLINE at(const Key &x) const {
    if (auto it = this->find(x); it != this->end()) return it->getMapped();
...
template <typename Key, typename Mapped, typename Hash = DefaultHash<Key>, typename Grower = HashTableGrowerWithPrecalculation<>,
          typename Allocator = HashTableAllocator>
...
using HashMapWithStackMemory =
    HashMapTable<Key, HashMapCellWithSavedHash<Key, Mapped, Hash>, Hash, HashTableGrower<initial_size_degree>,
