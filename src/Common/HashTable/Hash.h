...
inline UInt64 intHashCRC32(T x, UInt64 updated_value) {
  ...
  // See std::hash<float>.
  if (x == static_cast<T>(0.0)) return intHashCRC32(0, updated_value);
...
inline size_t DefaultHash64(T key) {
  if constexpr (is_big_int_v<T> && sizeof(T) == 16) {
    ...
    return intHash64(static_cast<UInt64>(key.toUnderType()) ^ static_cast<UInt64>(key.toUnderType() >> 64));
  } else if constexpr (is_big_int_v<T> && sizeof(T) == 32) {
    return intHash64(static_cast<UInt64>(key) ^ static_cast<UInt64>(key >> 64) ^ static_cast<UInt64>(key >> 128) ^
...
struct DefaultHash<T> {
  size_t operator()(T key) const { return DefaultHash64<typename T::NativeType>(key.value); }
};
