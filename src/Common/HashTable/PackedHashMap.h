...
struct __attribute__((packed)) PackedPairNoInit {
  ...
  PackedPairNoInit(FirstValue&& first_, NoInitTag) : first(std::forward<FirstValue>(first_)) {}
...
bool check(const PackedPairNoInit<First, Second> p) {
  return p.first == First{};
}

template <typename First, typename Second, template <typename, typename> class PackedPairNoInit>
void set(PackedPairNoInit<First, Second>& p) {
  p.first = First{};
}
}  // namespace PackedZeroTraits
...
template <typename Key, typename TMapped, typename Hash, typename TState>
struct tuple_element<0, PackedHashMapCell<Key, TMapped, Hash, TState>> {
  using type = Key;
};

template <typename Key, typename TMapped, typename Hash, typename TState>
struct tuple_element<1, PackedHashMapCell<Key, TMapped, Hash, TState>> {
  using type = TMapped;
};
}  // namespace std
