...
class TwoLevelStringHashTable : private boost::noncopyable {
 ...
 public:
  ...
  explicit TwoLevelStringHashTable(const Source &src) {
    if (src.m0.hasZero()) impls[0].m0.setHasZero(*src.m0.zeroValue());
  ...
  ConstLookupResult ALWAYS_INLINE find(const Key x) const { return dispatch(*this, x, typename Impl::FindCallable{}); }
