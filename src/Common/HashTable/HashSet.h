...
template <typename Key, typename TCell, typename Hash = DefaultHash<Key>, typename Grower = HashTableGrowerWithPrecalculation<>,
...
class HashSetTable final : public HashTable<Key, TCell, Hash, Grower, Allocator> {
 public:
  ...
  void merge(const Self &rhs) {
    ...
    if (rhs.grower.bufSize() > Base::grower.bufSize()) Base::resize(rhs.size() + Base::size());
    ...
    for (size_t i = 0; i < rhs.grower.bufSize(); ++i)
      if (!rhs.buf[i].isZero(*this)) this->insert(rhs.buf[i]);
  }
};

template <typename Key,
          typename TCell,  /// Supposed to have no state (HashTableNoState)
          typename Hash = DefaultHash<Key>, typename Grower = TwoLevelHashTableGrower<>, typename Allocator = HashTableAllocator>
class TwoLevelHashSetTable final
 ...
 public:
  ...
  void merge(const HashSetTable<Key, Args...> &rhs) {
    for (auto it = rhs.begin(), end = rhs.end(); it != end; ++it) this->insert(*it);
  }

  void merge(const Self &rhs) {
    if (rhs.empty()) return;

    for (size_t i = 0; i < Base::NUM_BUCKETS; ++i) this->impls[i].merge(rhs.impls[i]);
  }
  ...
  void writeAsSingleLevel(DB::WriteBuffer &wb) const {
    ...
    for (size_t i = 0; i < Base::NUM_BUCKETS; ++i) {
      if (this->impls[i].hasZero()) {
        if (zero_written) throw DB::Exception(DB::ErrorCodes::LOGICAL_ERROR, "No more than one zero value expected");
    ...
    for (auto ptr = this->begin(); ptr != this->end(); ++ptr)
      if (!ptr.getPtr()->isZero(state)) ptr.getPtr()->write(wb);
  }
};
...
template <typename Key, typename Hash = DefaultHash<Key>, typename Grower = HashTableGrowerWithPrecalculation<>,
          typename Allocator = HashTableAllocator>
using HashSet = HashSetTable<Key, HashTableCell<Key, Hash>, Hash, Grower, Allocator>;

template <typename Key, typename Hash = DefaultHash<Key>, typename Grower = TwoLevelHashTableGrower<>,
          typename Allocator = HashTableAllocator>
...
using HashSetWithStackMemory = HashSet<Key, Hash, HashTableGrower<initial_size_degree>,
...
template <typename Key, typename Hash, size_t initial_size_degree>
using HashSetWithSavedHashWithStackMemory =
    HashSetWithSavedHash<Key, Hash, HashTableGrower<initial_size_degree>,
