...
template <typename Key, typename Mapped, typename Hash = DefaultHash<Key>, typename Cell = ClearableHashMapCell<Key, Mapped, Hash>,
          typename Grower = HashTableGrowerWithPrecalculation<>, typename Allocator = HashTableAllocator>
class ClearableHashMap : public HashTable<Key, Cell, Hash, Grower, Allocator> {
 public:
  Mapped& operator[](const Key& x) {
    ...
    if (inserted) new (&it->getMapped()) Mapped();
...
using ClearableHashMapWithStackMemory =
    ClearableHashMap<Key, Mapped, Hash, ClearableHashMapCell<Key, Mapped, Hash>, HashTableGrower<initial_size_degree>,
...
using ClearableHashMapWithStackMemoryAndSavedHash = ClearableHashMap<
    Key, Mapped, Hash, ClearableHashMapCellWithSavedHash<Key, Mapped, Hash>, HashTableGrower<initial_size_degree>,
