...
template <typename Key, typename Cell, typename Hash, typename Grower, typename Allocator,
          typename ImplTable = HashTable<Key, Cell, Hash, Grower, Allocator>, size_t BITS_FOR_BUCKET = 8>
                          ...
                          protected Hash  /// empty base optimization
{
 ...
 public:
  ...
  const_iterator iteratorAt(size_t bucket) const {
    if (bucket >= NUM_BUCKETS) return end();
  ...
  std::pair<LookupResult, bool> ALWAYS_INLINE insert(const Cell &cell) {
    ...
    if (res.second) res.first->setMapped(cell.getValue());
  ...
  void write(DB::WriteBuffer &wb) const {
    for (UInt32 i = 0; i < NUM_BUCKETS; ++i) impls[i].write(wb);
  }

  void writeText(DB::WriteBuffer &wb) const {
    for (UInt32 i = 0; i < NUM_BUCKETS; ++i) {
      if (i != 0) DB::writeChar(',', wb);
      impls[i].writeText(wb);
    }
  }

  void read(DB::ReadBuffer &rb) {
    for (UInt32 i = 0; i < NUM_BUCKETS; ++i) impls[i].read(rb);
  }

  void readText(DB::ReadBuffer &rb) {
    for (UInt32 i = 0; i < NUM_BUCKETS; ++i) {
      if (i != 0) DB::assertChar(',', rb);
      impls[i].readText(rb);
    }
  }
  ...
  size_t size() const {
    size_t res = 0;
    for (UInt32 i = 0; i < NUM_BUCKETS; ++i) res += impls[i].size();
  ...
  size_t getBufferSizeInBytes() const {
    size_t res = 0;
    for (UInt32 i = 0; i < NUM_BUCKETS; ++i) res += impls[i].getBufferSizeInBytes();
