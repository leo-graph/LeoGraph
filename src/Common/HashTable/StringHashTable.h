...
#endif
  size_t ALWAYS_INLINE operator()(std::string_view key) const { return StringViewHash()(key); }
};
...
struct StringHashTableEmpty {
 ...
 public:
  ...
  void clearHasZero() {
    has_zero = false;
    if (!std::is_trivially_destructible_v<Cell>) zeroValue()->~Cell();
  }
 ...
 public:
  ...
  struct FindCallable {
    ...
    auto ALWAYS_INLINE operator()(Submap &map, const SubmapKey &key, size_t hash) {
      ...
      if (!it) return decltype(&it->getMapped()){};
      return &it->getMapped();
    }
  };
