...
struct HashTableGrower {
  ...
  void increaseSize() { size_degree += size_degree >= max_size_degree ? 1 : 2; }
  ...
  void setBufSize(size_t buf_size_) { size_degree = static_cast<UInt8>(static_cast<size_t>(log2(buf_size_ - 1) + 1)); }
};

/** Determines the size of the hash table, and when and how much it should be resized.
 * This structure is aligned to cache line boundary and also occupies it all.
 * Precalculates some values to speed up lookups and insertion into the HashTable (and thus has bigger memory footprint than
 * HashTableGrower). This grower assume 0.5 load factor
 */
template <size_t initial_size_degree = 8>
...
struct ZeroValueStorage<true, Cell> {
 ...
 public:
  ...
  void clearHasZeroFlag() { has_zero = false; }
                  ...
                  public ZeroValueStorage<Cell::need_zero_value_storage, Cell>  /// empty base optimization
{
 ...
 protected:
  ...
  static size_t allocCheckOverflow(size_t buffer_size) {
    ...
    if (common::mulOverflow(buffer_size, sizeof(Cell), size))
      throw DB::Exception(DB::ErrorCodes::CANNOT_ALLOCATE_MEMORY,
  ...
  void resize(size_t for_num_elems = 0, size_t for_buf_size = 0) {
    ...
    if (!empty()) {
       ...
       *  or move to the left of the collision resolution chain, because the elements to the left of it have been moved to the new "right"
       * location.
       */
      size_t i = 0;
      for (; i < old_size; ++i)
        if (!buf[i].isZero(*this)) reinsert(buf[i], buf[i].getHash(*this));
      ...
      for (; i < new_size && !buf[i].isZero(*this); ++i) reinsert(buf[i], buf[i].getHash(*this));
    }
  ...
  class prefetching_iterator_base {
   ...
   public:
    ...
    Derived &operator++() {
      ...
      if (prefetched_count > 0) prefetched_count--;
    ...
    void prefetch() {
      ...
      if (prefetch_ptr < buf_end && prefetched_count < prefetch_ahead) [[likely]] {
        ...
        for (size_t i = 0; i < n; ++i) {
          while (prefetch_ptr < buf_end && prefetch_ptr->isZero(*container)) ++prefetch_ptr;
  ...
  class const_prefetching_iterator : public prefetching_iterator_base<const const_prefetching_iterator, true> {
   public:
    using prefetching_iterator_base<const const_prefetching_iterator, true>::prefetching_iterator_base;
  };

  class prefetching_iterator : public prefetching_iterator_base<prefetching_iterator, false> {
   public:
  ...
  class iterator_base  /// NOLINT
  {
   ...
   public:
    ...
    size_t getCollisionChainLength() const { return container->grower.place((ptr - container->buf) - container->grower.place(getHash())); }
 ...
 public:
  ...
  explicit HashTable(const Grower &grower_) : grower(grower_) {
    if (Cell::need_zero_value_storage) this->zeroValue()->setZero();
    alloc(grower);
  }
  ...
  class Reader final : private Cell::State {
   public:
    ...
    const value_type &get() const {
      if (!is_initialized || is_eof) throw DB::Exception(DB::ErrorCodes::NO_AVAILABLE_DATA, "No available data");
  ...
  auto begin() const {
    ...
    if (!buf) return end<prefetch>();

    if (this->hasZero()) return ConstIterator(this, this->zeroValue());
  ...
  auto begin() {
    ...
    if (this->hasZero()) return Iterator(this, this->zeroValue());
 ...
 protected:
  ...
  bool ALWAYS_INLINE emplaceIfZero(const Key &x, LookupResult &it, bool &inserted, size_t hash_value) {
    ...
    if constexpr (!Cell::need_zero_value_storage) return false;
  ...
  /// Only for non-zero keys. Find the right place, insert the key there, if it does not already exist. Set iterator to the cell in output
  /// parameter.
  template <typename KeyHolder>
 ...
 public:
  ...
  std::pair<LookupResult, bool> ALWAYS_INLINE insert(const value_type &x) {
    ...
    if (res.second) res.first->setMapped(x);
  ...
  void ALWAYS_INLINE emplace(KeyHolder &&key_holder, LookupResult &it, bool &inserted, size_t hash_value) {
    const auto &key = keyHolderGetKey(key_holder);
    if (!emplaceIfZero(key, it, inserted, hash_value)) emplaceNonZero(key_holder, it, inserted, hash_value);
  }
    ...
    requires Grower::performs_linear_probing_with_single_step
  {
    ...
    /// Key is not found
    if (buf[erased_key_position].isZero(*this)) return false;
    ...
    while (true) {
      ...
      if (buf[next_position].isZero(*this)) break;
      ...
      if (optimal_position == next_position) continue;
  ...
  bool ALWAYS_INLINE contains(const Key &x) const { return has(x); }
