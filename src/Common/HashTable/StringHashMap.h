...
template <typename TMapped>
struct StringHashMapCell<std::string_view, TMapped>
    : public HashMapCellWithSavedHash<std::string_view, TMapped, StringHashTableHash, HashTableNoState> {
...
struct StringHashMapSubMaps {
  ...
  using Ts =
      HashMapTable<std::string_view, StringHashMapCell<std::string_view, TMapped>, StringHashTableHash, StringHashTableGrower<>, Allocator>;
};
...
class StringHashMap : public StringHashTable<StringHashMapSubMaps<TMapped, Allocator>> {
 public:
  ...
  TMapped &ALWAYS_INLINE operator[](const Key &x) {
    ...
    if (inserted) new (&it->getMapped()) TMapped();
  ...
  void ALWAYS_INLINE forEachMapped(Func &&func) {
    if constexpr (std::is_same_v<decltype(func(this->m0.zeroValue()->getMapped())), bool>) {
      if (this->m0.size()) {
        if (!func(this->m0.zeroValue()->getMapped())) return;
      }
      for (auto &v : this->m1) {
      ...
      if (this->m0.size()) func(this->m0.zeroValue()->getMapped());
      for (auto &v : this->m1) func(v.getMapped());
      for (auto &v : this->m2) func(v.getMapped());
      for (auto &v : this->m3) func(v.getMapped());
      for (auto &v : this->ms) func(v.getMapped());
    }
  }
};
