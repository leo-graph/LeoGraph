...
class SmallTable : private boost::noncopyable, protected Cell::State {
   ...
   public:
    ...
    bool next() {
      if (!is_initialized) {
        ...
        if (size > capacity) throw DB::Exception(DB::ErrorCodes::INCORRECT_DATA, "Illegal size");
 ...
 public:
  ...
  std::pair<iterator, bool> ALWAYS_INLINE insert(const value_type &x) {
    ...
    if (res.second) res.first.ptr->setMapped(x);
  ...
  void write(DB::WriteBuffer &wb) const {
    ...
    for (size_t i = 0; i < m_size; ++i) buf[i].write(wb);
  }
  ...
  void read(DB::ReadBuffer &rb) {
    ...
    if (new_size > capacity) throw DB::Exception(DB::ErrorCodes::INCORRECT_DATA, "Illegal size");

    for (size_t i = 0; i < new_size; ++i) buf[i].read(rb);
  ...
  void clear() {
    if (!std::is_trivially_destructible_v<Cell>)
      for (iterator it = begin(); it != end(); ++it) it.ptr->~Cell();
