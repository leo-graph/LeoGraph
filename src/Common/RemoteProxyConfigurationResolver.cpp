...
std::string RemoteProxyHostFetcherImpl::fetch(const Poco::URI& endpoint, const ConnectionTimeouts& timeouts) {
  ...
  if (response.getStatus() != Poco::Net::HTTPResponse::HTTP_OK)
    throw HTTPException(ErrorCodes::RECEIVED_ERROR_FROM_REMOTE_IO_SERVER, endpoint.toString(), response.getStatus(), response.getReason(),
                        body);

  return body;
}

RemoteProxyConfigurationResolver::RemoteProxyConfigurationResolver(const RemoteServerConfiguration& remote_server_configuration_,
                                                                   Protocol request_protocol_, const std::string& no_proxy_hosts_,
    ...
    : ProxyConfigurationResolver(request_protocol_, disable_tunneling_for_https_requests_over_http_proxy_),
      remote_server_configuration(remote_server_configuration_),
      no_proxy_hosts(no_proxy_hosts_),
      fetcher(fetcher_) {}
...
ProxyConfiguration RemoteProxyConfigurationResolver::resolve() {
  ...
  if (cache_ttl.count() && cache_valid && now <= cache_timestamp + cache_ttl && now >= cache_timestamp) {
    LOG_DEBUG(logger, "Use cached proxy: {}://{}:{}. Tunneling: {}", cached_config.protocol, cached_config.host, cached_config.port,
  ...
  bool use_tunneling_for_https_requests_over_http_proxy =
      ProxyConfiguration::useTunneling(request_protocol, proxy_protocol, disable_tunneling_for_https_requests_over_http_proxy);
...
void RemoteProxyConfigurationResolver::errorReport(const ProxyConfiguration& config) {
  if (config.host.empty()) return;

  std::lock_guard lock(cache_mutex);

  if (!remote_server_configuration.cache_ttl_.count() || !cache_valid) return;

  if (std::tie(cached_config.protocol, cached_config.host, cached_config.port) != std::tie(config.protocol, config.host, config.port))
    return;
