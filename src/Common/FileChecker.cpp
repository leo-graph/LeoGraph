...
FileChecker::FileChecker(DiskPtr disk_, const String& file_info_path_) : disk(std::move(disk_)), log(getLogger("FileChecker")) {
...
void FileChecker::setPath(const String& file_info_path_) { files_info_path = file_info_path_; }

String FileChecker::getPath() const { return files_info_path; }
...
void FileChecker::update(const String& filename, size_t size) { map[filename] = size; }

void FileChecker::setEmpty(const String& full_file_path) { map[fileName(full_file_path)] = 0; }

size_t FileChecker::getFileSize(const String& full_file_path) const {
  auto it = map.find(fileName(full_file_path));
  if (it == map.end()) throw Exception(ErrorCodes::LOGICAL_ERROR, "File {} is not added to the file checker", full_file_path);
  return it->second;
}

size_t FileChecker::getTotalSize() const {
  size_t total_size = 0;
  for (auto size : map | boost::adaptors::map_values) total_size += size;
  return total_size;
}

FileChecker::DataValidationTasksPtr FileChecker::getDataValidationTasks() { return std::make_unique<DataValidationTasks>(map); }
...
std::optional<CheckResult> FileChecker::checkNextEntry(DataValidationTasksPtr& check_data_tasks) const {
  ...
  if (real_size != expected_size) {
    String failure_message =
        exists ? ("Size of " + path + " is wrong. Size is " + toString(real_size) + " but should be " + toString(expected_size))
...
void FileChecker::repair() {
  for (const auto& name_size : map) {
    ...
    if (real_size < expected_size)
      throw Exception(ErrorCodes::UNEXPECTED_END_OF_FILE, "Size of {} is less than expected. Size is {} but should be {}.", path, real_size,
                      expected_size);
...
void FileChecker::save(WriteBuffer& buffer) const {
  ...
  for (auto it = map.begin(); it != map.end(); ++it) {
    if (it != map.begin()) writeString(",", buffer);
...
void FileChecker::save() const {
...

  {
    std::unique_ptr<WriteBufferFromFileBase> out =
        disk ? disk->writeFile(tmp_files_info_path) : std::make_unique<WriteBufferFromFile>(tmp_files_info_path);
    save(*out);
    out->sync();
    out->finalize();
  }
...
void FileChecker::load() {
  ...
  if (!fileReallyExists(files_info_path)) return;

  std::unique_ptr<ReadBuffer> in =
      disk ? disk->readFile(files_info_path, getReadSettings()) : std::make_unique<ReadBufferFromFile>(files_info_path);
...
bool FileChecker::fileReallyExists(const String& path_) const { return disk ? disk->existsFile(path_) : fs::exists(path_); }

size_t FileChecker::getRealFileSize(const String& path_) const { return disk ? disk->getFileSize(path_) : fs::file_size(path_); }
