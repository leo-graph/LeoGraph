...
class RandomStringPrepareWalker : public Regexp::Walker<Regexp *> {
 private:
  ...
  class RegexpConcatFunction : public NodeFunction {
   public:
    RegexpConcatFunction(const Children &children_, const Generators &generators_) : children(getFuncs(children_, generators_)) {}
    ...
    size_t getRequiredSize() override {
      size_t total_size = 0;
      for (auto &child : children) total_size += child->getRequiredSize();
      return total_size;
    }
  ...
  class RegexpAlternateFunction : public NodeFunction {
   public:
    RegexpAlternateFunction(const Children &children_, const Generators &generators_) : children(getFuncs(children_, generators_)) {}
    ...
    size_t getRequiredSize() override {
      size_t total_size = 0;
      for (auto &child : children) total_size = std::max(total_size, child->getRequiredSize());
      return total_size;
    }
  ...
  class RegexpRepeatFunction : public NodeFunction {
   public:
    RegexpRepeatFunction(Regexp *re_, const Generators &generators_, int min_repeat_, int max_repeat_)
        : func(generators_.at(re_)), min_repeat(min_repeat_), max_repeat(max_repeat_) {}
    ...
    size_t getRequiredSize() override { return max_repeat * func->getRequiredSize(); }
  ...
  class RegexpCharClassFunction : public NodeFunction {
   ...
   public:
    explicit RegexpCharClassFunction(Regexp *re_) {
      CharClass *cc = re_->cc();
      chassert(cc);
      if (cc->empty()) throw DB::Exception(DB::ErrorCodes::BAD_ARGUMENTS, "kRegexpCharClass is empty");
    ...
    size_t operator()(char *out, size_t size) override {
      ...
      for (; it != char_ranges.end(); ++it) {
        ...
        if (count_down <= range_len) break;
        count_down -= range_len;
      }

      if (it == char_ranges.end())
        throw DB::Exception(DB::ErrorCodes::LOGICAL_ERROR, "Unable to choose the rune. Runes {}, ranges {}, chosen {}", char_count,
                            char_ranges.size(), chosen);
    ...
    size_t getRequiredSize() override { return UTFmax; }
  ...
  class RegexpLiteralStringFunction : public NodeFunction {
   public:
    ...
    size_t getRequiredSize() override { return literal_string.size(); }
  ...
  class RegexpLiteralFunction : public NodeFunction {
   public:
    ...
    size_t getRequiredSize() override { return literal.size(); }
  ...
  class ThrowExceptionFunction : public NodeFunction {
   public:
    explicit ThrowExceptionFunction(Regexp *re_) : operation(magic_enum::enum_name(re_->op())) {}

    size_t operator()(char *, size_t) override {
      throw DB::Exception(DB::ErrorCodes::BAD_ARGUMENTS,
                          "RandomStringPrepareWalker: regexp node '{}' is not supported for generating a random match", operation);
    }
 ...
 public:
  std::function<String()> getGenerator() {
    if (root == nullptr) throw DB::Exception(DB::ErrorCodes::LOGICAL_ERROR, "no root has been set");

    if (generators.empty()) throw DB::Exception(DB::ErrorCodes::LOGICAL_ERROR, "no generators");

    auto root_func = generators.at(root);
    auto required_buffer_size = root_func->getRequiredSize();
    auto generator_func = [=]() -> String {
 ...
 private:
  Children CopyChildrenArgs(Regexp **children, int nchild) {
    ...
    for (int i = 0; i < nchild; ++i) result.push_back(Copy(children[i]));
    return result;
  }
  ...
  Regexp *PostVisit(Regexp *re, Regexp * /*parent_arg*/, Regexp *pre_arg, Regexp **child_args, int nchild_args) override /*noexcept*/
  {
    switch (re->op()) {
...
void RandomStringGeneratorByRegexp::RegexpPtrDeleter::operator()(re2::Regexp *re) const noexcept { re->Decref(); }
...
RandomStringGeneratorByRegexp::RandomStringGeneratorByRegexp(const String &re_str) {
  ...
  if (!regexp) throw DB::Exception(DB::ErrorCodes::BAD_ARGUMENTS, "Error parsing regexp '{}': {}", re_str, status.Text());
...

  {
    ...
    if (!matched)
      throw DB::Exception(DB::ErrorCodes::BAD_ARGUMENTS, "Generator is unable to produce random string for regexp '{}': {}", re_str,
                          test_check);
  }
}
