...
auto connectionReestablisher(std::weak_ptr<Pool> pool, bool shareable) {
  return [weak_pool = pool, shareable](UInt64 interval_milliseconds) {
    ...
    if (!shared_pool) return false;

    if (shareable && shared_pool.use_count() == 2) return true;

    if (!shared_pool->isOnline()) {
      try {
        shared_pool->get();
        Poco::Util::Application::instance().logger().information("Reestablishing connection to " + shared_pool->getDescription() +
                                                                 " has succeeded.");
      } catch (const Poco::Exception &e) {
        if (interval_milliseconds >= 1000)
          Poco::Util::Application::instance().logger().warning("Reestablishing connection to " + shared_pool->getDescription() +
                                                               " has failed: " + e.displayText());
...
PoolWithFailover::PoolWithFailover(const Poco::Util::AbstractConfiguration &config_, const std::string &config_name_,
                                   const unsigned default_connections_, const unsigned max_connections_, const size_t max_tries_)
  ...
  if (config_.has(config_name_ + ".replica")) {
    ...
    for (auto &[_, replicas] : replicas_by_priority) {
      if (replicas.size() > 1) std::shuffle(replicas.begin(), replicas.end(), rnd_generator);
    }
  } else {
    replicas_by_priority[0].emplace_back(std::make_shared<Pool>(config_, config_name_, default_connections_, max_connections_));
    if (bg_reconnect)
      DB::ReplicasReconnector::instance().add(connectionReestablisher(std::weak_ptr(replicas_by_priority[0].back()), shareable));
  }
}

PoolWithFailover::PoolWithFailover(const std::string &config_name_, const unsigned default_connections_, const unsigned max_connections_,
                                   const size_t max_tries_)
    : PoolWithFailover{Poco::Util::Application::instance().config(), config_name_, default_connections_, max_connections_, max_tries_} {}

PoolWithFailover::PoolWithFailover(const std::string &database, const RemoteDescription &addresses, const std::string &user,
                                   const std::string &password, const std::string &ssl_ca, const std::string &ssl_cert,
                                   const std::string &ssl_key, unsigned default_connections_, unsigned max_connections_, size_t max_tries_,
                                   uint64_t wait_timeout_, size_t connect_timeout_, size_t rw_timeout_, bool bg_reconnect_)
    : max_tries(max_tries_), shareable(false), wait_timeout(wait_timeout_), bg_reconnect(bg_reconnect_) {
  /// Replicas have the same priority, but traversed replicas are moved to the end of the queue.
  for (const auto &[host, port] : addresses) {
    replicas_by_priority[0].emplace_back(std::make_shared<Pool>(database, host, user, password, port, ssl_ca, ssl_cert, ssl_key,
                                                                /* socket_ = */ "", connect_timeout_, rw_timeout_, default_connections_,
                                                                max_connections_));
    if (bg_reconnect)
      DB::ReplicasReconnector::instance().add(connectionReestablisher(std::weak_ptr(replicas_by_priority[0].back()), shareable));
  }
...
PoolWithFailover::PoolWithFailover(const PoolWithFailover &other)
    : max_tries{other.max_tries}, shareable{other.shareable}, wait_timeout(other.wait_timeout), bg_reconnect(other.bg_reconnect) {
  if (shareable) {
    ...
    for (const auto &priority_replicas : other.replicas_by_priority) {
      ...
      for (const auto &pool : priority_replicas.second) {
        ...
        if (bg_reconnect) DB::ReplicasReconnector::instance().add(connectionReestablisher(std::weak_ptr(replicas.back()), shareable));
      }
  ...
  for (size_t try_no = 0; try_no < max_tries; ++try_no) {
    ...
    for (auto &priority_replicas : replicas_by_priority) {
      ...
      for (size_t i = 0, size = replicas.size(); i < size; ++i) {
        ...
        if (bg_reconnect && !pool->isOnline()) continue;
