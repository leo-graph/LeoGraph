...
void Pool::Entry::incrementRefCount() {
  ...
  if (data->ref_count.fetch_add(1) == 0) mysql_thread_init();
}
...
Pool::Pool(const Poco::Util::AbstractConfiguration &cfg, const std::string &config_name, unsigned default_connections_,
           unsigned max_connections_, const char *parent_config_name_)
  ...
  if (parent_config_name_) {
    ...
    user = cfg.has(config_name + ".user") ? cfg.getString(config_name + ".user") : cfg.getString(parent_config_name + ".user");
    password =
        cfg.has(config_name + ".password") ? cfg.getString(config_name + ".password") : cfg.getString(parent_config_name + ".password");

    if (!cfg.has(config_name + ".port") && !cfg.has(config_name + ".socket") && !cfg.has(parent_config_name + ".port") &&
        !cfg.has(parent_config_name + ".socket"))
      throw Poco::Exception("mysqlxx::Pool configuration: expected port or socket");

    port = cfg.has(config_name + ".port") ? cfg.getInt(config_name + ".port") : cfg.getInt(parent_config_name + ".port", 0);
    socket = cfg.has(config_name + ".socket") ? cfg.getString(config_name + ".socket") : cfg.getString(parent_config_name + ".socket", "");
    ssl_ca = cfg.has(config_name + ".ssl_ca") ? cfg.getString(config_name + ".ssl_ca") : cfg.getString(parent_config_name + ".ssl_ca", "");
    ssl_cert =
        cfg.has(config_name + ".ssl_cert") ? cfg.getString(config_name + ".ssl_cert") : cfg.getString(parent_config_name + ".ssl_cert", "");
    ssl_key =
        cfg.has(config_name + ".ssl_key") ? cfg.getString(config_name + ".ssl_key") : cfg.getString(parent_config_name + ".ssl_key", "");
    ...
    enable_local_infile = cfg.getBool(config_name + ".enable_local_infile", MYSQLXX_DEFAULT_ENABLE_LOCAL_INFILE);

    opt_reconnect = cfg.getBool(config_name + ".opt_reconnect", MYSQLXX_DEFAULT_MYSQL_OPT_RECONNECT);
  }

  connect_timeout = cfg.getInt(config_name + ".connect_timeout", cfg.getInt("mysql_connect_timeout", MYSQLXX_DEFAULT_TIMEOUT));

  rw_timeout = cfg.getInt(config_name + ".rw_timeout", cfg.getInt("mysql_rw_timeout", MYSQLXX_DEFAULT_RW_TIMEOUT));
}

Pool::Pool(const std::string &db_, const std::string &server_, const std::string &user_, const std::string &password_, unsigned port_,
           const std::string &ssl_ca_, const std::string &ssl_cert_, const std::string &ssl_key_, const std::string &socket_,
           unsigned connect_timeout_, unsigned rw_timeout_, unsigned default_connections_, unsigned max_connections_,
           unsigned enable_local_infile_, bool opt_reconnect_)
  ...
  LOG_DEBUG(log,
            "Created MySQL Pool with settings: connect_timeout={}, read_write_timeout={}, default_connections_number={}, "
...
Pool::~Pool() {
  ...
  for (auto &connection : connections) delete static_cast<Connection *>(connection);
}
...
Pool::Entry Pool::get(uint64_t wait_timeout) {
  ...
  if (wait_timeout && wait_timeout != UINT64_MAX) deadline = clock_gettime_ns() + wait_timeout * 1'000'000'000;
  ...
  for (;;) {
    ...
    if (connections.size() < static_cast<size_t>(max_connections)) {
      ...
      if (conn) return Entry(conn, this);
    ...
    if (!wait_timeout) throw Poco::Exception("mysqlxx::Pool is full (wait is disabled, see connection_wait_timeout setting)");

    if (deadline && clock_gettime_ns() >= deadline) throw Poco::Exception("mysqlxx::Pool is full (connection_wait_timeout is exceeded)");
...
Pool::Entry Pool::tryGet() {
  ...
  if (connections.size() >= max_connections) throw Poco::Exception("mysqlxx::Pool is full");

  Connection *connection_ptr = allocConnection(true);
  if (connection_ptr) return {connection_ptr, this};
...
void Pool::Entry::forceConnected() const {
  if (data == nullptr) throw Poco::RuntimeException("Tried to access NULL database connection.");
  ...
  while (!tryForceConnected()) {
    ...
    LOG_DEBUG(pool->log, "Creating a new MySQL connection to {} with settings: connect_timeout={}, read_write_timeout={}",
    ...
    try {
      data->conn.connect(pool->db.c_str(), pool->server.c_str(), pool->user.c_str(), pool->password.c_str(), pool->port,
                         pool->socket.c_str(), pool->ssl_ca.c_str(), pool->ssl_cert.c_str(), pool->ssl_key.c_str(), pool->connect_timeout,
                         pool->rw_timeout, pool->enable_local_infile, pool->opt_reconnect);
...
bool Pool::Entry::tryForceConnected() const {
  ...
  if (data->conn.ping())  /// Attempts to reestablish lost connection
  {
    ...
    if (prev_connection_id != current_connection_id) {
      LOG_DEBUG(pool->log, "Entry(connection {}): Reconnected to MySQL server. Connection id changed: {} -> {}", current_connection_id,
...
void Pool::initialize() {
  if (!initialized) {
    ...
    for (unsigned i = 0; i < default_connections; ++i) allocConnection();
...
Pool::Connection *Pool::allocConnection(bool dont_throw_if_failed_first_time) {
  ...
  try {
    ...
    LOG_DEBUG(log, "Creating a new MySQL connection to {} with settings: connect_timeout={}, read_write_timeout={}", description,
              connect_timeout, rw_timeout);

    conn_ptr->conn.connect(db.c_str(), server.c_str(), user.c_str(), password.c_str(), port, socket.c_str(), ssl_ca.c_str(),
                           ssl_cert.c_str(), ssl_key.c_str(), connect_timeout, rw_timeout, enable_local_infile, opt_reconnect);
  } catch (mysqlxx::ConnectionFailed &e) {
    LOG_ERROR(log, "Failed to connect to MySQL ({}): {}", description, e.what());

    if (!online || (!was_successful && !dont_throw_if_failed_first_time) || e.errnum() == ER_ACCESS_DENIED_ERROR ||
        e.errnum() == ER_DBACCESS_DENIED_ERROR || e.errnum() == ER_BAD_DB_ERROR) {
      online = false;
      throw;
    }
    online = false;
