...
class Value {
 public:
  ...
  Int64 getInt() const { return getIntOrDateTime(); }
  ...
  LocalDateTime getDateTime() const { return LocalDateTime(data(), size()); }

  /// Get date (from value like '2011-01-01' or '2011-01-01 00:00:00').
  LocalDate getDate() const { return LocalDate(data(), size()); }
 ...
 private:
  ...
  Int64 getIntImpl() const { return readIntText(m_data, m_length); }

  Int64 getIntOrDateTime() const {
    if (unlikely(isNull())) throwException("Value is NULL");

    if (checkDateTime()) return getDateTimeImpl();
    return getIntImpl();
  }
...
inline unsigned char Value::get<unsigned char>() const {
  return static_cast<unsigned char>(getUInt());
}
template <>
inline char8_t Value::get<char8_t>() const {
  return static_cast<char8_t>(getUInt());
}
template <>
...
inline unsigned short Value::get<unsigned short>() const {
  return static_cast<unsigned short>(getUInt());
}  /// NOLINT
template <>
inline int Value::get<int>() const {
  return static_cast<int>(getInt());
}
template <>
inline unsigned int Value::get<unsigned int>() const {
  return static_cast<unsigned int>(getUInt());
}
template <>
inline long Value::get<long>() const {
  return static_cast<long>(getInt());
}  /// NOLINT
template <>
...
template <>
inline LocalDate Value::get<LocalDate>() const {
  return getDate();
}
template <>
inline LocalDateTime Value::get<LocalDateTime>() const {
  return getDateTime();
}
