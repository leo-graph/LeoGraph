...
class Pool final {
 ...
 public:
  ...
  class Entry {
   public:
    ...
    Entry(const Entry& src) : data(src.data), pool(src.pool) { incrementRefCount(); }
    ...
    Entry(Pool::Connection* conn, Pool* p) : data(conn), pool(p) { incrementRefCount(); }

    std::string getDescription() const {
      if (pool) return pool->getDescription();
      return "pool is null";
    }
  ...
  explicit Pool(const std::string& config_name, unsigned default_connections_ = MYSQLXX_POOL_DEFAULT_START_CONNECTIONS,
                unsigned max_connections_ = MYSQLXX_POOL_DEFAULT_MAX_CONNECTIONS, const char* parent_config_name_ = nullptr)
   ...
   */
  Pool(const std::string& db_, const std::string& server_, const std::string& user_, const std::string& password_, unsigned port_,
       const std::string& ssl_ca_ = "", const std::string& ssl_cert_ = "", const std::string& ssl_key_ = "",
       const std::string& socket_ = "", unsigned connect_timeout_ = MYSQLXX_DEFAULT_TIMEOUT,
       unsigned rw_timeout_ = MYSQLXX_DEFAULT_RW_TIMEOUT, unsigned default_connections_ = MYSQLXX_POOL_DEFAULT_START_CONNECTIONS,
  ...
  Pool(const Pool& other)
      : default_connections{other.default_connections},
        ...
        opt_reconnect(other.opt_reconnect) {}
 ...
 private:
  ...
  std::string getDescriptionImpl() const { return description; }
};
