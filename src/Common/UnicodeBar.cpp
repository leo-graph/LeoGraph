...
double getWidth(double x, double min, double max, double max_width) {
  if (isNaN(x) || isNaN(min) || isNaN(max)) return 0;

  if (x <= min) return 0;

  if (x >= max) return max_width;
...
static char* checkedCopy(const char* src, size_t src_size, char* dst, const char* dst_end) {
  if (dst + src_size > dst_end)
    throw DB::Exception(DB::ErrorCodes::LOGICAL_ERROR, "Not enough space in buffer for UnicodeBar::render, required: {}, got: {}", src_size,
                        dst_end - dst);
...
void render(double width, char* dst, const char* dst_end) {
  ...
  for (size_t i = 0; i < floor_width; ++i) dst = checkedCopy(FULL_BAR, UNICODE_BAR_CHAR_SIZE, dst, dst_end);

  size_t remainder = int_width % GRADES_IN_FULL_BAR;

  if (remainder) checkedCopy(&FRACTIONAL_BARS[(remainder - 1) * UNICODE_BAR_CHAR_SIZE], UNICODE_BAR_CHAR_SIZE, dst, dst_end);
}
