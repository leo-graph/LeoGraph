...
const size_t signal_pipe_buf_size = sizeof(int) + sizeof(siginfo_t) + sizeof(ucontext_t *) + sizeof(StackTrace) + sizeof(UInt64) +
                                    sizeof(UInt32) + sizeof(void *)
#if defined(OS_LINUX)
                                    + sizeof(UInt8) + sizeof(FramePointers)
#endif
    ;
...
__attribute__((__weak__)) void collectCrashLog(Int32 signal, Int32 signal_code, UInt64 thread_id, const String &query_id,
                                               const String &query, const StackTrace &stack_trace, std::optional<UInt64> fault_address,
                                               const String &fault_access_type, const String &signal_description,
                                               const FramePointers &current_exception_trace, size_t current_exception_trace_size);
...
class SignalListener : public Poco::Runnable {
 ...
 private:
  ...
  void onFault(int sig, const siginfo_t &info, ucontext_t *context, const StackTrace &stack_trace,
               const std::vector<FramePointers> &thread_frame_pointers, UInt32 thread_num, DB::ThreadStatus *thread_ptr,
               const FramePointers &exception_trace, size_t exception_trace_size) const;
};
