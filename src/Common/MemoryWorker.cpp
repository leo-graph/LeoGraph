...
uint64_t readMetricsFromStatFile(ReadBufferFromFile& buf, std::initializer_list<std::string_view> keys,
                                 std::initializer_list<std::string_view> optional_keys, bool* warnings_printed) {
  uint64_t sum = 0;
...
std::pair<std::string, ICgroupsReader::CgroupsVersion> ICgroupsReader::getCgroupsPath() {
  ...
  if (v2_path.has_value()) return {*v2_path, ICgroupsReader::CgroupsVersion::V2};

  auto v1_path = getCgroupsV1Path();
  if (v1_path.has_value()) return {*v1_path, ICgroupsReader::CgroupsVersion::V1};

  throw Exception(ErrorCodes::FILE_DOESNT_EXIST, "Cannot find cgroups v1 or v2 current memory file");
}

std::shared_ptr<ICgroupsReader> ICgroupsReader::createCgroupsReader(ICgroupsReader::CgroupsVersion version,
                                                                    const std::filesystem::path& cgroup_path) {
  if (version == CgroupsVersion::V2) return std::make_shared<CgroupsV2Reader>(cgroup_path);
...
std::string_view sourceToString(MemoryWorker::MemoryUsageSource source) {
  switch (source) {
    case MemoryWorker::MemoryUsageSource::Cgroups:
      return "Cgroups";
    case MemoryWorker::MemoryUsageSource::Jemalloc:
      return "Jemalloc";
    case MemoryWorker::MemoryUsageSource::None:
      return "None";
  }
}
...
MemoryWorker::MemoryWorker(MemoryWorkerConfig config, std::shared_ptr<PageCache> page_cache_)
  ...
  if (config.use_cgroup) {
    ...
    try {
      ...
      LOG_INFO(getLogger("CgroupsReader"), "Will create cgroup reader from '{}' (cgroups version: {})", cgroup_path,
      ...
      if (rss_update_period_ms == 0) rss_update_period_ms = cgroups_memory_usage_tick_ms;
  ...
  if (rss_update_period_ms == 0) rss_update_period_ms = jemalloc_memory_usage_tick_ms;
#endif
}

MemoryWorker::MemoryUsageSource MemoryWorker::getSource() { return source; }

void MemoryWorker::start() {
  if (source == MemoryUsageSource::None) return;

  const std::string purge_dirty_pages_info =
      purge_dirty_pages_threshold_ratio > 0 || purge_total_memory_threshold_ratio > 0
          ? fmt::format("enabled (total memory threshold ratio: {}, dirty pages threshold ratio: {}, page size: {})",
                        purge_total_memory_threshold_ratio, purge_dirty_pages_threshold_ratio, page_size)
          : "disabled";

  LOG_INFO(log, "Starting background memory thread with period of {}ms, using {} as source, purging dirty pages {}", rss_update_period_ms,
           sourceToString(source), purge_dirty_pages_info);
...
MemoryWorker::~MemoryWorker() {
  ...
  if (update_resident_memory_thread.joinable()) update_resident_memory_thread.join();

#if USE_JEMALLOC
  if (purge_dirty_pages_thread.joinable()) purge_dirty_pages_thread.join();
#endif
}
...
uint64_t MemoryWorker::getMemoryUsage(bool log_error) {
  switch (source) {
    case MemoryUsageSource::Cgroups: {
      if (cgroups_reader != nullptr) return cgroups_reader->readMemoryUsage();
      [[fallthrough]];
    }
    ...
    case MemoryUsageSource::None: {
      if (log_error) LOG_ERROR(log, "Trying to fetch memory usage while no memory source can be used");
      return 0;
    }
  }
}
...
void MemoryWorker::updateResidentMemoryThread() {
  ...
  while (true) {
    try {
      ...
      if (page_cache) page_cache->autoResize(std::max(resident, total_memory_tracker.get()), total_memory_tracker.getHardLimit());
      ...
      const bool needs_purge =
          ...
          (purge_dirty_pages_threshold_ratio > 0 && static_cast<double>(pdirty_mib.getValue() * page_size) > purge_dirty_pages_threshold);
      ...
      if (needs_purge) {
        ...
        if (decay_adjustment_period_ms.count() > 0) {
          if (!std::exchange(purging_dirty_pages, true)) {
          ...
          } else if ((getCurrentTimeMs() - purge_state_change_time_ms >= decay_adjustment_period_ms) &&
            ...
            notify_purge |=
                decay_state.compare_exchange_strong(expected, MemoryWorker::DecayState::DisableRequested, std::memory_order_relaxed);
          }
        }
        ...
        if (notify_purge) purge_dirty_pages_cv.notify_all();
...
void MemoryWorker::purgeDirtyPagesThread() {
  ...
  while (true) {
    try {
      ...
      purge_dirty_pages_cv.wait_for(purge_dirty_pages_lock, std::chrono::seconds(1), [&] {
        auto state = decay_state.load(std::memory_order_relaxed);
        return shutdown || purge_dirty_pages.load(std::memory_order_relaxed) || state == MemoryWorker::DecayState::DisableRequested ||
      ...
      if (current_state == MemoryWorker::DecayState::DisableRequested) {
        ...
        LOG_INFO(log,
                 ...
                 default_dirty_decay_ms, decay_adjustment_period_ms.count());
      ...
      if (!purge_dirty_pages.compare_exchange_strong(is_purge_enabled, false, std::memory_order_relaxed)) continue;
