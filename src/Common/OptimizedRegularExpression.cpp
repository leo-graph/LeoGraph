...
size_t shortest_literal_length(const Literals &literals) {
  ...
  for (const auto &lit : literals) shortest = std::min(shortest, lit.literal.size());
  return shortest;
}
...
const char *analyzeImpl(std::string_view regexp, const char *pos, Literal &required_substring, bool &is_trivial, bool &has_capture,
  ...
  auto finish_cur_alternatives = [&]() {
    if (cur_alternatives.empty()) return;
  ...
  auto finish_non_trivial_char = [&](bool create_new_substr = true) {
    is_trivial = false;
    if (create_new_substr) is_prefix = false;
    if (depth != 0) return;
    ...
    switch (*pos) {
      ...
      case '(':
        ...
        if (!in_square_braces) {
          ...
          auto is_flag_char = [](char x) { return x == '-' || x == 'i' || x == 'm' || x == 's' || x == 'U' || x == 'u'; };
          ...
          if ((pos + 1 < end && (pos[1] == '?' || pos[1] == '*')) || (pos + 2 < end && pos[1] == '{' && pos[2] == '0')) {
            finish_non_trivial_char();
          } else {
      ...
      case ']':
        if (!in_square_braces) goto ordinary;

        --depth;
        if (depth == 0) in_square_braces = false;
        finish_non_trivial_char();
        ++pos;
        break;
      ...
      case '}':
        if (!in_curly_braces) goto ordinary;
      ...
      default:
        if (depth == 0 && !in_curly_braces && !in_square_braces) {
          ...
          if (last_substring->first.empty()) last_substring->second = is_prefix;
...
namespace DB {
RegexpAnalysisResult OptimizedRegularExpression::analyze(std::string_view regexp_) try {
  ...
  for (auto &lit : alternative_literals) r.alternatives.push_back(std::move(lit.literal));
  return r;
} catch (...) {
  LOG_ERROR(getLogger("OptimizeRegularExpression"), "Analyze RegularExpression failed, got error: {}",
...
OptimizedRegularExpression::OptimizedRegularExpression(const std::string &regexp_, int options) {
  ...
  if (!is_trivial) {
    ...
    if (is_case_insensitive) regexp_options.set_case_sensitive(false);

    if (is_dot_nl) regexp_options.set_dot_nl(true);
    ...
    if (!is_no_capture) {
      ...
      if (number_of_subpatterns > MAX_SUBPATTERNS)
        throw DB::Exception(DB::ErrorCodes::CANNOT_COMPILE_REGEXP, "OptimizedRegularExpression: too many subpatterns in regexp: {}",
                            regexp_);
    }
  }
...
bool OptimizedRegularExpression::match(const char *subject, size_t subject_size) const {
  ...
  if (is_trivial) {
    if (required_substring.empty()) return true;

    if (is_case_insensitive) return haystack_end != case_insensitive_substring_searcher->search(haystack, subject_size);
    return haystack_end != case_sensitive_substring_searcher->search(haystack, subject_size);
  }

  if (!required_substring.empty()) {
    if (is_case_insensitive) {
      if (haystack_end == case_insensitive_substring_searcher->search(haystack, subject_size)) return false;
    } else {
      if (haystack_end == case_sensitive_substring_searcher->search(haystack, subject_size)) return false;
    }
  }
...
bool OptimizedRegularExpression::match(const char *subject, size_t subject_size, Match &match) const {
  ...
  if (is_trivial) {
    ...
    if (haystack_end == pos) return false;
  ...
  if (!re2::RE2::PartialMatch({subject, subject_size}, *re2, &piece)) return false;
