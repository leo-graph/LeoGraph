...
class FixedSizeBufferAllocationPolicy : public BufferAllocationPolicy {
 ...
 public:
  explicit FixedSizeBufferAllocationPolicy(const BufferAllocationPolicy::Settings& settings_) : buffer_size(settings_.strict_size) {
  ...
  void nextBuffer() override { ++buffer_number; }
};
...
class ExpBufferAllocationPolicy : public DB::BufferAllocationPolicy {
 ...
 public:
  ...
  void nextBuffer() override {
    ...
    if (2 == buffer_number) current_size = second_size;
...
BufferAllocationPolicyPtr BufferAllocationPolicy::create(BufferAllocationPolicy::Settings settings_) {
  if (settings_.strict_size > 0) return std::make_unique<FixedSizeBufferAllocationPolicy>(settings_);
