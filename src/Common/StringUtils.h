...
inline bool startsWith(const std::string &s, const std::string &prefix) { return impl::startsWith(s, prefix.data(), prefix.size()); }

inline bool endsWith(const std::string &s, const std::string &suffix) { return impl::endsWith(s, suffix.data(), suffix.size()); }

/// With GCC, strlen is evaluated compile time if we pass it a constant
/// string that is known at compile time.
inline bool startsWith(const std::string &s, const char *prefix) { return impl::startsWith(s, prefix, strlen(prefix)); }

inline bool endsWith(const std::string &s, const char *suffix) { return impl::endsWith(s, suffix, strlen(suffix)); }
...
inline bool isLowerAlphaASCII(char c) { return (c >= 'a' && c <= 'z'); }

inline bool isUpperAlphaASCII(char c) { return (c >= 'A' && c <= 'Z'); }

inline bool isAlphaASCII(char c) { return isLowerAlphaASCII(c) || isUpperAlphaASCII(c); }
...
inline bool isHexDigit(char c) { return isNumericASCII(c) || (c >= 'a' && c <= 'f') || (c >= 'A' && c <= 'F'); }

inline bool isAlphaNumericASCII(char c) { return isAlphaASCII(c) || isNumericASCII(c); }

inline bool isWordCharASCII(char c) { return isAlphaNumericASCII(c) || c == '_'; }

inline bool isValidIdentifierBegin(char c) { return isAlphaASCII(c) || c == '_'; }

inline bool isWhitespaceASCII(char c) { return c == ' ' || c == '\t' || c == '\n' || c == '\r' || c == '\f' || c == '\v'; }

/// Since |isWhiteSpaceASCII()| is used inside algorithms it's easier to implement another function than add extra argument.
inline bool isWhitespaceASCIIOneLine(char c) { return c == ' ' || c == '\t' || c == '\f' || c == '\v'; }

inline bool isControlASCII(char c) { return static_cast<unsigned char>(c) <= 31; }
...
inline bool isPunctuationASCII(char c) {
  uint8_t uc = c;
  return (uc >= 33 && uc <= 47) || (uc >= 58 && uc <= 64) || (uc >= 91 && uc <= 96) || (uc >= 123 && uc <= 125);
}

inline bool isNumberSeparator(bool is_start_of_block, bool is_hex, const char *pos, const char *end) {
  if (*pos != '_') return false;
  if (is_start_of_block && *pos == '_') return false;                                          // e.g. _123, 12e_3
  if (pos + 1 < end && !(is_hex ? isHexDigit(pos[1]) : isNumericASCII(pos[1]))) return false;  // e.g. 1__2, 1_., 1_e, 1_p, 1_;
  if (pos + 1 == end) return false;                                                            // e.g. 12_
  return true;
}

/// Works assuming isAlphaASCII.
inline char toLowerIfAlphaASCII(char c) { return c | 0x20; }

inline char toUpperIfAlphaASCII(char c) { return c & (~0x20); }

inline char alternateCaseIfAlphaASCII(char c) { return c ^ 0x20; }
      ...
      if (pos + 1 < end && upos[0] == 0xC2 && (upos[1] == 0x85 || upos[1] == 0xA0)) {
        pos += 2;
                 ...
                 ((upos[0] == 0xE1 && upos[1] == 0xA0 && upos[2] == 0x8E) ||
                  (upos[0] == 0xE2 && ((upos[1] == 0x80 && ((upos[2] >= 0x80 && upos[2] <= 0x8A) || (upos[2] >= 0xA8 && upos[2] <= 0xA9) ||
                                                            (upos[2] >= 0x8B && upos[2] <= 0x8D) || (upos[2] == 0xAF))) ||
                                       (upos[1] == 0x81 && (upos[2] == 0x9F || upos[2] == 0xA0)))) ||
                  (upos[0] == 0xE3 && upos[1] == 0x80 && upos[2] == 0x80) || (upos[0] == 0xEF && upos[1] == 0xBB && upos[2] == 0xBF))) {
        pos += 3;
      } else
        break;
    }
...
inline bool equalsCaseInsensitive(char a, char b) { return a == b || (isAlphaASCII(a) && alternateCaseIfAlphaASCII(a) == b); }
...
inline void trimLeft(std::string_view &str, char c = ' ') {
  while (str.starts_with(c)) str.remove_prefix(1);
}

inline void trimLeft(std::string &str, char c = ' ') { str.erase(0, str.find_first_not_of(c)); }

inline void trimRight(std::string_view &str, char c = ' ') {
  while (str.ends_with(c)) str.remove_suffix(1);
}

inline void trimRight(std::string &str, char c = ' ') { str.erase(str.find_last_not_of(c) + 1); }
...
constexpr bool containsGlobs(const std::string &str) { return str.find_first_of("*?{") != std::string::npos; }
...
inline bool isValidIdentifier(std::string_view str) {
  return !str.empty() && isValidIdentifierBegin(str[0]) &&
         ...
         && !(str.size() == strlen("null") && toLowerIfAlphaASCII(str[0]) == 'n' && toLowerIfAlphaASCII(str[1]) == 'u' &&
              toLowerIfAlphaASCII(str[2]) == 'l' && toLowerIfAlphaASCII(str[3]) == 'l');
}
