...
class MemoryTracker {
 ...
 public:
  ...
  Int64 getHardLimit() const { return hard_limit.load(std::memory_order_relaxed); }
  ...
  void setSampleProbability(double value) { sample_probability = value; }

  double getSampleProbability(UInt64 size);

  void setSampleMinAllocationSize(UInt64 value) { min_allocation_size_bytes = value; }

  void setJemallocFlushProfileInterval(UInt64 interval) { jemalloc_flush_profile_interval_bytes = interval; }

  void setJemallocFlushProfileOnMemoryExceeded(bool flush) { jemalloc_flush_profile_on_memory_exceeded = flush; }

  void setSampleMaxAllocationSize(UInt64 value) { max_allocation_size_bytes = value; }
  ...
  MemoryTracker *getParent() { return parent.load(std::memory_order_relaxed); }

  /// The memory consumption could be shown in realtime via CurrentMetrics counter
  void setMetric(CurrentMetrics::Metric metric_) { metric.store(metric_, std::memory_order_relaxed); }

  CurrentMetrics::Metric getMetric() { return metric.load(std::memory_order_relaxed); }
  ...
  std::chrono::microseconds getOvercommitWaitingTime() { return max_wait_time.load(std::memory_order_relaxed); }

  void setOvercommitWaitingTime(UInt64 wait_time);

  void setOvercommitTracker(OvercommitTracker *tracker) noexcept { overcommit_tracker.store(tracker, std::memory_order_relaxed); }

  void resetOvercommitTracker() noexcept { overcommit_tracker.store(nullptr, std::memory_order_relaxed); }

  void setPageCache(DB::PageCache *cache) noexcept { page_cache.store(cache); }

  void resetPageCache() noexcept { page_cache.store(nullptr); }
