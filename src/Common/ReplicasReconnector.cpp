...
std::unique_ptr<ReplicasReconnector> ReplicasReconnector::init(ContextPtr context) {
  if (instance_ptr) throw Exception(ErrorCodes::LOGICAL_ERROR, "Replicas reconnector is already initialized.");
...
ReplicasReconnector& ReplicasReconnector::instance() {
  if (!instance_ptr) throw Exception(ErrorCodes::NOT_INITIALIZED, "Replicas reconnector is not initialized.");
...
void ReplicasReconnector::run() {
  auto interval_milliseconds =
      Context::getGlobalContextInstance()->getServerSettings()[ServerSetting::dictionary_background_reconnect_interval];
