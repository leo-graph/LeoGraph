...
class FieldVisitorScale : public StaticVisitor<void> {
 ...
 public:
  ...
  template <typename T>
  void operator()(DecimalField<T> &x) const {
    x = DecimalField<T>(x.getValue() * T(rhs), x.getScale());
  }
  void operator()(DecimalField<Decimal32> &x) const {
    x = DecimalField<Decimal32>(x.getValue() * Decimal32(static_cast<int32_t>(rhs)), x.getScale());
  }
