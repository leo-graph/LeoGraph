...
std::string_view IntervalKind::toString() const { return magic_enum::enum_name(kind); }
...
Int32 IntervalKind::toAvgSeconds() const {
  switch (kind) {
    ...
    case IntervalKind::Kind::Month:
      return 2629746;  /// Exactly 1/12 of a year.
    case IntervalKind::Kind::Quarter:
      return 7889238;  /// Exactly 1/4 of a year.
    case IntervalKind::Kind::Year:
      return 31556952;  /// The average length of a Gregorian year is equal to 365.2425 days
  }
}
...
IntervalKind IntervalKind::fromAvgSeconds(Int64 num_seconds) {
  if (num_seconds) {
    if (!(num_seconds % 31556952)) return IntervalKind::Kind::Year;
    if (!(num_seconds % 7889238)) return IntervalKind::Kind::Quarter;
    if (!(num_seconds % 2629746)) return IntervalKind::Kind::Month;
    if (!(num_seconds % 604800)) return IntervalKind::Kind::Week;
    if (!(num_seconds % 86400)) return IntervalKind::Kind::Day;
    if (!(num_seconds % 3600)) return IntervalKind::Kind::Hour;
    if (!(num_seconds % 60)) return IntervalKind::Kind::Minute;
  }
  ...
  switch (kind) {
    ...
    case IntervalKind::Kind::Week:
      ...
      throw Exception(ErrorCodes::SYNTAX_ERROR,
                      "The syntax 'EXTRACT(WEEK FROM date)' is not supported, cannot extract the number of a week");
    case IntervalKind::Kind::Month:
      return "toMonth";
    case IntervalKind::Kind::Quarter:
      return "toQuarter";
