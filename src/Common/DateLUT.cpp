...
Poco::DigestEngine::Digest calcSHA1(const std::string& path) {
  ...
  if (!stream) throw Poco::Exception("Error while opening file: '" + path + "'.");
  ...
  if (!stream.eof()) throw Poco::Exception("Error while reading file: '" + path + "'.");
...
std::string determineDefaultTimeZone() {
  ...
  const char* tzdir_env_var =
      std::getenv("TZDIR");  // NOLINT(concurrency-mt-unsafe) // ok, because it does not run concurrently with other getenv calls
  fs::path tz_database_path = tzdir_env_var ? tzdir_env_var : "/usr/share/zoneinfo/";

  fs::path tz_file_path;
  std::string error_prefix;
  const char* tz_env_var =
      std::getenv("TZ");  // NOLINT(concurrency-mt-unsafe) // ok, because it does not run concurrently with other getenv calls
  ...
  if (tz_env_var) {
    ...
    if (!fs::exists(tz_file_path)) return "UTC";
    ...
    if (FS::isSymlink(tz_file_path)) {
      ...
      if (tz_file_path.is_relative()) tz_file_path = (fs::path("/etc/") / tz_file_path).lexically_normal();
    }
  }
  ...
  try {
    ...
    {
      if (!tz_file_path.is_absolute()) tz_file_path = tz_database_path / tz_file_path;
    ...
    for (auto candidate_it = begin; candidate_it != end; ++candidate_it) {
      ...
      if (!fs::is_regular_file(*candidate_it) || path.filename() == "localtime") continue;
...
const DateLUTImpl& DateLUT::instance() {
  ...
  if (DB::CurrentThread::isInitialized()) {
    ...
    if (query_context) timezone_from_context.emplace(query_context->getSettingsRef()[DB::Setting::session_timezone]);
  }
  ...
  if (!timezone_from_context.has_value()) {
    ...
    if (global_context) timezone_from_context.emplace(global_context->getSettingsRef()[DB::Setting::session_timezone]);
  }

  if (timezone_from_context.has_value() && !timezone_from_context->empty()) return date_lut.getImplementation(*timezone_from_context);
...
const DateLUTImpl& DateLUT::getImplementation(std::string_view time_zone) const {
  ...
  if (!it->second) it->second = std::unique_ptr<DateLUTImpl>(new DateLUTImpl(time_zone));
...
std::optional<Int64> tryToMakeDateTime(const DateLUTImpl& date_lut, Int16 year, UInt8 month, UInt8 day_of_month, UInt8 hour, UInt8 minute,
                                       UInt8 second) {
  static_assert(std::same_as<Int64, DateLUTImpl::Time>);
  return date_lut.tryToMakeDateTime(year, month, day_of_month, hour, minute, second);
}

const std::string& getDateLUTTimeZone(const DateLUTImpl& date_lut) { return date_lut.getTimeZone(); }

UInt32 getDayNumOffsetEpoch() { return DateLUTImpl::getDayNumOffsetEpoch(); }
