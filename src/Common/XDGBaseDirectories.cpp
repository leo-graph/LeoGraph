...
fs::path getPathFromEnvOrDefault(const char* env_var_name, const char* path_prefix) {
  ...
  if (xdg_config_home) return fs::path(xdg_config_home) / APP_NAME;

  auto* home_path = getenv(ENV_HOME);  // NOLINT(concurrency-mt-unsafe)
  if (home_path) return fs::path(home_path) / path_prefix / APP_NAME;

  return "";
}
}  // namespace

fs::path XDGBaseDirectories::getConfigurationHome() { return getPathFromEnvOrDefault(ENV_XDG_CONFIG_HOME, CONFIG_PATH_PREFIX); }

fs::path XDGBaseDirectories::getDataHome() { return getPathFromEnvOrDefault(ENV_XDG_DATA_HOME, DATA_PATH_PREFIX); }

fs::path XDGBaseDirectories::getStateHome() { return getPathFromEnvOrDefault(ENV_XDG_STATE_HOME, STATE_PATH_PREFIX); }

fs::path XDGBaseDirectories::getCacheHome() { return getPathFromEnvOrDefault(ENV_XDG_CACHE_HOME, CACHE_PATH_PREFIX); }
