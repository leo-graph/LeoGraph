...
struct LogLUT {
  LogLUT() {
    log_table[0] = 0.0;
    for (size_t i = 1; i <= M; ++i) log_table[i] = log(static_cast<double>(i));
  }

  double getLog(size_t x) const {
    if (x <= M) return log_table[x];
...
class __attribute__((__packed__)) Denominator<precision, max_rank, HashValueType, DenominatorType, denominator_mode> {
 ...
 public:
  ...
  void update(UInt8 rank) { denominator += static_cast<T>(1.0) / (1ULL << rank); }
...
class __attribute__((__packed__)) Denominator<precision, max_rank, HashValueType, DenominatorType, denominator_mode> {
 public:
  ...
  void update(UInt8 rank) { ++rank_count[rank]; }
...
/// of Algorithms).
template <UInt8 precision, typename Key = UInt64, typename Hash = IntHash32<Key>, typename HashValueType = UInt32,
          typename DenominatorType = double, typename BiasEstimator = TrivialBiasEstimator,
          HyperLogLogMode mode = HyperLogLogMode::FullFeatured, DenominatorMode denominator_mode = DenominatorMode::StableIfBig>
class HyperLogLogCounter : private Hash {
 ...
 public:
  ...
  void merge(const HyperLogLogCounter &rhs) {
    ...
    for (HashValueType bucket = 0; bucket < bucket_count; ++bucket) update(bucket, rhs_rank_store[bucket]);
  }
  ...
  static void skip(DB::ReadBuffer &in) { in.ignore(sizeof(RankStore) + sizeof(DenominatorCalculatorType) + sizeof(ZerosCounterType)); }
 ...
 private:
  ...
  UInt8 calculateRank(HashValueType val) const {
    if (unlikely(val == 0)) return max_rank;

    auto zeros_plus_one = details::TrailingZerosCounter<HashValueType>::apply(val) + 1;

    if (unlikely(zeros_plus_one) > max_rank) return max_rank;
  ...
  HashValueType getHash(Value key) const {
    ...
    return static_cast<HashValueType>(Hash::operator()(static_cast<Key>(key)));
  }
  ...
  double fixRawEstimate(double raw_estimate) const {
    if ((mode == HyperLogLogMode::Raw) || ((mode == HyperLogLogMode::BiasCorrected) && BiasEstimator::isTrivial())) return raw_estimate;
    if (mode == HyperLogLogMode::LinearCounting) return applyLinearCorrection(raw_estimate);
    if ((mode == HyperLogLogMode::BiasCorrected) && !BiasEstimator::isTrivial()) return applyBiasCorrection(raw_estimate);
  ...
  double applyCorrection(double raw_estimate) const {
    ...
    if (BiasEstimator::isTrivial()) {
      ...
      if (linear_estimate < BiasEstimator::getThreshold()) fixed_estimate = linear_estimate;
    }
...
template <UInt8 precision, typename Key, typename Hash, typename HashValueType, typename DenominatorType, typename BiasEstimator,
          HyperLogLogMode mode, DenominatorMode denominator_mode>
details::LogLUT<precision>
    HyperLogLogCounter<precision, Key, Hash, HashValueType, DenominatorType, BiasEstimator, mode, denominator_mode>::log_lut;

/// Lightweight implementation of expression's denominator is used in Metrage.
/// Serialization format must not be changed.
using HLL12 = HyperLogLogCounter<12, UInt64, IntHash32<UInt64>, UInt32, double, TrivialBiasEstimator, HyperLogLogMode::FullFeatured,
