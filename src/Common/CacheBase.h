...
class CacheBase {
 ...
 public:
  ...
  explicit CacheBase(CurrentMetrics::Metric size_in_bytes_metric, CurrentMetrics::Metric count_metric, size_t max_size_in_bytes,
                     size_t max_count = NO_MAX_COUNT, double size_ratio = DEFAULT_SIZE_RATIO)
      : CacheBase("SLRU", size_in_bytes_metric, count_metric, max_size_in_bytes, max_count, size_ratio) {}

  /// Use this ctor if the user should be able to configure the cache policy and cache sizes via settings. Supports only general-purpose
  /// policies LRU and SLRU.
  explicit CacheBase(std::string_view cache_policy_name, CurrentMetrics::Metric size_in_bytes_metric, CurrentMetrics::Metric count_metric,
                     size_t max_size_in_bytes, size_t max_count, double size_ratio) {
    auto on_remove_entry_function = [this](size_t weight_loss, const MappedPtr &mapped_ptr) { onEntryRemoval(weight_loss, mapped_ptr); };
    ...
    if (cache_policy_name == "LRU") {
      ...
      cache_policy =
          std::make_unique<LRUPolicy>(size_in_bytes_metric, count_metric, max_size_in_bytes, max_count, on_remove_entry_function);
    } else if (cache_policy_name == "SLRU") {
      using SLRUPolicy = SLRUCachePolicy<TKey, TMapped, HashFunction, WeightFunction>;
      cache_policy = std::make_unique<SLRUPolicy>(size_in_bytes_metric, count_metric, max_size_in_bytes, max_count, size_ratio,
                                                  on_remove_entry_function);
    } else
  ...
  std::pair<MappedPtr, bool> getOrSet(const Key &key, LoadFunc &&load_func) {
    InsertTokenHolder token_holder;
    {
      ...
      if (!token) token = std::make_shared<InsertToken>(*this);
    ...
    if (!token->cleaned_up) token_holder.cleanup(token_lock, cache_lock);
 ...
 private:
  ...
  struct InsertTokenHolder {
    ...
    ~InsertTokenHolder() {
      ...
      if (token->cleaned_up) return;

      std::lock_guard cache_lock(token->cache.mutex);

      --token->refcount;
      if (token->refcount == 0) cleanup(token_lock, cache_lock);
    }
  };
