...
void StudentTTest::add(size_t distribution, double value) {
  if (distribution > 1) throw std::logic_error("Distribution number for Student's T-Test must be eigther 0 or 1");
...
std::pair<bool, std::string> StudentTTest::compareAndReport(size_t confidence_level_index) const {
  ...
  if (data[0].size == 0 || data[1].size == 0) return {true, ""};
  ...
  double pooled_standard_deviation =
      sqrt((static_cast<double>(data[0].size - 1) * data[0].var() + static_cast<double>(data[1].size - 1) * data[1].var()) /
           static_cast<double>(degrees_of_freedom));

  double t_statistic = pooled_standard_deviation * sqrt(1.0 / static_cast<double>(data[0].size) + 1.0 / static_cast<double>(data[1].size));
  ...
  if (mean_difference > mean_confidence_interval &&
      (mean_difference - mean_confidence_interval >
       0.0001))  /// difference must be more than 0.0001, to take into account connection latency.
  {
