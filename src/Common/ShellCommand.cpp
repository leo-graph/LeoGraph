...
ShellCommand::ShellCommand(pid_t pid_, int &in_fd_, int &out_fd_, int &err_fd_, const ShellCommand::Config &config_)
    : in(in_fd_), out(out_fd_), err(err_fd_), pid(pid_), config(config_) {}

LoggerPtr ShellCommand::getLogger() { return ::getLogger("ShellCommand"); }

ShellCommand::~ShellCommand() {
  if (do_not_terminate) return;

  if (wait_called) return;
  ...
  if (config.terminate_in_destructor_strategy.terminate_in_destructor) {
    ...
    if (process_terminated_normally) return;

    LOG_TRACE(getLogger(), "Will kill shell command pid {} with signal {}", pid,
              config.terminate_in_destructor_strategy.termination_signal);

    int retcode = kill(pid, config.terminate_in_destructor_strategy.termination_signal);
    if (retcode != 0) LOG_WARNING(getLogger(), "Cannot kill shell command pid {}, error: '{}'", pid, errnoToString());
  } else {
    try {
      tryWait();
    } catch (...) {
...
bool ShellCommand::tryWaitProcessWithTimeout(size_t timeout_in_seconds) {
  ...
  for (auto &[_, fd] : write_fds) fd.close();

  for (auto &[_, fd] : read_fds) fd.close();
...
std::unique_ptr<ShellCommand> ShellCommand::executeImpl(const char *filename, char *const argv[], const Config &config) {
  ...
  if (!real_vfork) throw ErrnoException(ErrorCodes::CANNOT_DLSYM, "Cannot find symbol vfork in myself");
  ...
  for (size_t i = 0; i < config.read_fds.size(); ++i) read_pipe_fds.emplace_back(std::make_unique<PipeFDs>());

  for (size_t i = 0; i < config.write_fds.size(); ++i) write_pipe_fds.emplace_back(std::make_unique<PipeFDs>());

  pid_t pid = reinterpret_cast<pid_t (*)()>(real_vfork)();

  if (pid == -1) throw ErrnoException(ErrorCodes::CANNOT_FORK, "Cannot vfork");
  ...
  if (0 == pid) {
    ...
    if (STDIN_FILENO != dup2(pipe_stdin.fds_rw[0], STDIN_FILENO)) _exit(static_cast<int>(ReturnCodes::CANNOT_DUP_STDIN));

    if (!config.pipe_stdin_only) {
      if (STDOUT_FILENO != dup2(pipe_stdout.fds_rw[1], STDOUT_FILENO)) _exit(static_cast<int>(ReturnCodes::CANNOT_DUP_STDOUT));

      if (STDERR_FILENO != dup2(pipe_stderr.fds_rw[1], STDERR_FILENO)) _exit(static_cast<int>(ReturnCodes::CANNOT_DUP_STDERR));
    }
    ...
    for (size_t i = 0; i < config.read_fds.size(); ++i) {
      ...
      if (fd != dup2(fds.fds_rw[1], fd)) _exit(static_cast<int>(ReturnCodes::CANNOT_DUP_READ_DESCRIPTOR));
    }
    ...
    for (size_t i = 0; i < config.write_fds.size(); ++i) {
      ...
      if (fd != dup2(fds.fds_rw[0], fd)) _exit(static_cast<int>(ReturnCodes::CANNOT_DUP_WRITE_DESCRIPTOR));
    }
  ...
  std::unique_ptr<ShellCommand> res(new ShellCommand(pid, pipe_stdin.fds_rw[1], pipe_stdout.fds_rw[0], pipe_stderr.fds_rw[0], config));
  ...
  LOG_TRACE(getLogger(), "Started shell command '{}' with pid {} and file descriptors: out {}, err {}", filename, pid, res->out.getFD(),
            res->err.getFD());
...
std::unique_ptr<ShellCommand> ShellCommand::execute(const ShellCommand::Config &config) {
  ...
  for (const auto &argument : config.arguments) config_copy.arguments.emplace_back(argument);
...
std::unique_ptr<ShellCommand> ShellCommand::executeDirect(const ShellCommand::Config &config) {
  ...
  for (const auto &arg : arguments) argv_sum_size += arg.size() + 1;
...
int ShellCommand::tryWait() { return tryWaitImpl(true).retcode; }
...
ShellCommand::tryWaitResult ShellCommand::tryWaitImpl(bool blocking) {
  ...
  while (waitpid_retcode < 0) {
    ...
    if (errno != EINTR) throw ErrnoException(ErrorCodes::CANNOT_WAITPID, "Cannot waitpid");
  }
