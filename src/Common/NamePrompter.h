...
class NamePrompter {
 public:
  ...
  static std::vector<String> getHints(const String &name, const std::vector<String> &prompting_strings) {
    ...
    for (size_t i = 0; i < prompting_strings.size(); ++i) appendToQueue(i, name, queue, prompting_strings);
 ...
 private:
  static void appendToQueue(size_t ind, const String &name, DistanceIndexQueue &queue, const std::vector<String> &prompting_strings) {
    ...
    if (min_possible_edit_distance > 0 && name.size() / min_possible_edit_distance < 3) return;
    ...
    if (prompt.size() <= name.size() + mistake_factor && prompt.size() + mistake_factor >= name.size()) {
      ...
      if (distance <= mistake_factor) {
        ...
        if (queue.size() > MaxNumHints) queue.pop();
      }
    }
  }
...
class IHints {
 public:
  ...
  std::vector<String> getHints(const String &name) const { return prompter.getHints(name, getAllRegisteredNames()); }
  ...
  String getHintsMessage(const String &name) const { return getHintsErrorMessageSuffix(getHints(name)); }
