...
constexpr bool memcpy_can_be_used_for_assignment = std::is_same_v<T, U> || (std::is_integral_v<T> && std::is_integral_v<U> &&
                                                                            sizeof(T) == sizeof(U));  /// NOLINT(misc-redundant-expression)
...
inline size_t byte_size(size_t num_elements, size_t element_size) {
  size_t amount;
  if (__builtin_mul_overflow(num_elements, element_size, &amount)) throw_alloc_error();
  return amount;
}
...
inline size_t minimum_memory_for_elements(size_t num_elements, size_t element_size, size_t pad_left, size_t pad_right) {
  size_t amount;
  if (__builtin_add_overflow(byte_size(num_elements, element_size), pad_left + pad_right, &amount)) throw_alloc_error();
  return amount;
}
                     ...
                     private TAllocator  /// empty base optimization
{
 ...
 protected:
  ...
  NO_INLINE void reallocPowerOfTwoElements(size_t n, TAllocatorParams &&...allocator_params) {
    realloc(roundUpToPowerOfTwoOrZero(PODArrayDetails::minimum_memory_for_elements(n, ELEMENT_SIZE, pad_left, pad_right)),
            std::forward<TAllocatorParams>(allocator_params)...);
  }
  ...
  void protectImpl(int prot) {
    ...
    char *left_rounded_up = reinterpret_cast<char *>((reinterpret_cast<intptr_t>(c_start) - pad_left + PROTECT_PAGE_SIZE - 1) /
 ...
 public:
  ...
  ALWAYS_INLINE void reserve(size_t n, TAllocatorParams &&...allocator_params) {
    if (n > capacity()) reallocPowerOfTwoElements(n, std::forward<TAllocatorParams>(allocator_params)...);
  }
  ...
  {
    if (n > capacity())
      realloc(PODArrayDetails::minimum_memory_for_elements(n, ELEMENT_SIZE, pad_left, pad_right),
              std::forward<TAllocatorParams>(allocator_params)...);
  }
  ...
  void shrink_to_fit(TAllocatorParams &&...allocator_params) {
    realloc(PODArrayDetails::minimum_memory_for_elements(size(), ELEMENT_SIZE, pad_left, pad_right),
            std::forward<TAllocatorParams>(allocator_params)...);
  }
  ...
  void push_back_raw(const void *ptr, TAllocatorParams &&...allocator_params)  /// NOLINT
  {
    ...
    if (unlikely(required_capacity > capacity())) reserve(required_capacity, std::forward<TAllocatorParams>(allocator_params)...);
...
class PODArray : public PODArrayBase<sizeof(T), initial_bytes, TAllocator, pad_right_, pad_left_> {
 ...
 public:
  ...
  PODArray(PODArray &&other) noexcept { this->swap(other); }
  ...
  {
    if (unlikely(this->c_end + sizeof(T) > this->c_end_of_storage)) this->reserveForNextSize();
  ...
  void insert(iterator it, It1 from_begin, It2 from_end) {
    ...
    if (unlikely(bytes_to_move)) memmove(this->c_end + bytes_to_copy - bytes_to_move, this->c_end - bytes_to_move, bytes_to_move);
  ...
  void assign(It1 from_begin, It2 from_end, TAllocatorParams &&...allocator_params) {
    ...
    if (required_capacity > this->capacity()) this->reserve_exact(required_capacity, std::forward<TAllocatorParams>(allocator_params)...);

    size_t bytes_to_copy = PODArrayDetails::byte_size(required_capacity, sizeof(T));
    if (bytes_to_copy) memcpy(this->c_start, reinterpret_cast<const void *>(&*from_begin), bytes_to_copy);
...
void swap(PODArray<T, initial_bytes, TAllocator, pad_right_, pad_left_> &lhs,
          PODArray<T, initial_bytes, TAllocator, pad_right_, pad_left_> &rhs)  /// NOLINT
{
  lhs.swap(rhs);
}
