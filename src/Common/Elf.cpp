...
Elf::Elf(const char *data, size_t size, const std::string &path_) { init(data, size, path_); }
...
void Elf::init(const char *data, size_t size, const std::string &path_) {
  ...
  auto section_names_strtab = findSection(
      [&](const Section &section, size_t idx) { return section.header.type == SectionHeaderType::STRTAB && header->shstrndx == idx; });
...
Elf::Section::Section(const ElfSectionHeader &header_, const Elf &elf_) : header(header_), elf(elf_) {}
...
bool Elf::iterateSections(std::function<bool(const Section &section, size_t idx)> &&pred) const {
  for (size_t idx = 0; idx < header->shnum; ++idx) {
    ...
    if (section.header.offset + section.header.size > elf_size) continue;

    if (pred(section, idx)) return true;
  }
  return false;
}
...
String Elf::getBuildID() const {
  ...
  for (size_t idx = 0; idx < header->phnum; ++idx) {
    ...
    if (phdr.type == ProgramHeaderType::NOTE) return getBuildID(mapped + phdr.offset, phdr.filesz);
  }
...
String Elf::getStoredBinaryHash() const {
  if (auto section = findSectionByName(".clickhouse.hash")) return {section->begin(), section->end()};
  return {};
}

const char *Elf::Section::name() const {
  if (!elf.section_names) throw Exception(ErrorCodes::CANNOT_PARSE_ELF, "Section names are not initialized");

  /// TODO buffer overflow is possible, we may need to check strlen.
  return elf.section_names + header.name;
}

const char *Elf::Section::begin() const { return elf.mapped + header.offset; }

const char *Elf::Section::end() const { return begin() + size(); }

size_t Elf::Section::size() const { return header.size; }
