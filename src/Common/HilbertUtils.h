...
void segmentBinaryPartition(UInt64 start, UInt64 finish, UInt8 current_bits, F&& callback) {
  ...
  auto construct_range = [next_bits, history](UInt64 chunk) {
    return HilbertDetails::Segment{.begin = history + (chunk << next_bits), .end = history + ((chunk + 1) << next_bits) - 1};
  };
...
inline std::array<std::pair<UInt64, UInt64>, 2> createRangeFromCorners(UInt64 x1, UInt64 y1, UInt64 x2, UInt64 y2) {
  ...
  if (contains_minimum_vertice) {
    ...
    return {std::pair<UInt64, UInt64>{x_min, x_min + range_size}, std::pair<UInt64, UInt64>{y_min, y_min + range_size}};
  }
...
void hilbertIntervalToHyperrectangles2D(UInt64 first, UInt64 last, F&& callback) {
  ...
  segmentBinaryPartition(first, last, static_cast<UInt8>(64 - even_equal_bits_count), [&](HilbertDetails::Segment range) {
    ...
    std::array<std::pair<UInt64, UInt64>, 2> unpacked_range =
        createRangeFromCorners(std::get<0>(interval1), std::get<1>(interval1), std::get<0>(interval2), std::get<1>(interval2));
