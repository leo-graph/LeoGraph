...
bool isTunnelingDisabledForHTTPSRequestsOverHTTPProxy(const Poco::Util::AbstractConfiguration& configuration) {
  return configuration.getBool("proxy.disable_tunneling_for_https_requests_over_http_proxy", false);
}

std::shared_ptr<ProxyConfigurationResolver> getRemoteResolver(ProxyConfiguration::Protocol request_protocol, const String& config_prefix,
  ...
  auto server_configuration =
      RemoteProxyConfigurationResolver::RemoteServerConfiguration{endpoint, proxy_scheme, proxy_port, std::chrono::seconds{cache_ttl}};

  return std::make_shared<RemoteProxyConfigurationResolver>(
      server_configuration, request_protocol, buildPocoNonProxyHosts(getNoProxyHosts(configuration)),
      std::make_shared<RemoteProxyHostFetcherImpl>(), isTunnelingDisabledForHTTPSRequestsOverHTTPProxy(configuration));
}
...
auto extractURIList(const String& config_prefix, const Poco::Util::AbstractConfiguration& configuration) {
  ...
  for (const auto& key : keys) {
    if (startsWith(key, "uri")) {
      ...
      if (proxy_uri.getHost().empty()) throw Exception(ErrorCodes::BAD_ARGUMENTS, "Empty host in proxy uri: {}", proxy_uri.toString());
...
std::shared_ptr<ProxyConfigurationResolver> getListResolver(ProxyConfiguration::Protocol request_protocol, const String& config_prefix,
                                                            const Poco::Util::AbstractConfiguration& configuration) {
  auto uris = extractURIList(config_prefix, configuration);

  return uris.empty() ? nullptr
                      : std::make_shared<ProxyListConfigurationResolver>(uris, request_protocol,
 ...
 */
std::optional<std::string> getProtocolPrefix(ProxyConfiguration::Protocol request_protocol, const String& config_prefix,
                                             const Poco::Util::AbstractConfiguration& configuration) {
...
std::optional<std::string> calculatePrefixBasedOnSettingsSyntax(bool new_syntax, ProxyConfiguration::Protocol request_protocol,
                                                                const String& config_prefix,
                                                                const Poco::Util::AbstractConfiguration& configuration) {
  if (!configuration.has(config_prefix)) return std::nullopt;

  if (new_syntax) return getProtocolPrefix(request_protocol, config_prefix, configuration);
...
std::shared_ptr<ProxyConfigurationResolver> ProxyConfigurationResolverProvider::get(Protocol request_protocol) {
  ...
  if (!context) {
    return std::make_shared<EnvironmentProxyConfigurationResolver>(request_protocol, false);
  }
...
std::shared_ptr<ProxyConfigurationResolver> ProxyConfigurationResolverProvider::get(
    Protocol request_protocol, const Poco::Util::AbstractConfiguration& configuration) {
  if (auto resolver = getFromSettings(true, request_protocol, "proxy", configuration)) return resolver;

  return std::make_shared<EnvironmentProxyConfigurationResolver>(request_protocol,
                                                                 isTunnelingDisabledForHTTPSRequestsOverHTTPProxy(configuration));
}

std::shared_ptr<ProxyConfigurationResolver> ProxyConfigurationResolverProvider::getFromSettings(
    bool new_syntax, Protocol request_protocol, const String& config_prefix, const Poco::Util::AbstractConfiguration& configuration) {
...
std::shared_ptr<ProxyConfigurationResolver> ProxyConfigurationResolverProvider::getFromOldSettingsFormat(
    Protocol request_protocol, const String& config_prefix, const Poco::Util::AbstractConfiguration& configuration) {
   ...
   * */
  if (auto resolver =
          ProxyConfigurationResolverProvider::getFromSettings(false, request_protocol, config_prefix + ".proxy", configuration)) {
    return resolver;
  }
