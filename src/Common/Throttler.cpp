...
Throttler::Throttler(size_t max_speed_, const ThrottlerPtr& parent_, ProfileEvents::Event event_amount_,
...
Throttler::Throttler(size_t max_speed_, ProfileEvents::Event event_amount_, ProfileEvents::Event event_sleep_us_)
...
bool Throttler::throttle(size_t amount, size_t max_block_ns) {
  ...
  if (event_amount != ProfileEvents::end()) ProfileEvents::increment(event_amount, amount);

  if (limit && count_value > limit)
    throw Exception::createDeprecated(limit_exceeded_exception_message + std::string(" Maximum: ") + toString(limit),
  ...
  if (block) {
    ...
    if (event_sleep_us != ProfileEvents::end()) timer2.emplace(profile_events.timer(event_sleep_us));
    ...
    UInt64 block_ns = block_ns_double >= static_cast<double>(std::numeric_limits<UInt64>::max()) ? std::numeric_limits<UInt64>::max()
  ...
  if (parent) parent_block = parent->throttle(amount, max_block_ns);
...
void Throttler::throttleImpl(size_t amount, size_t& count_value, double& tokens_value, size_t& max_speed_value) {
  ...
  if (max_speed) {
    ...
    tokens = std::min<double>(tokens + static_cast<double>(max_speed) * delta_seconds - static_cast<double>(amount),
                              static_cast<double>(max_burst));
  }
  count += amount;
  count_value = count;
  tokens_value = tokens;
...
bool Throttler::isThrottling() const { return block_count.load(std::memory_order_relaxed) > 0 || (parent && parent->isThrottling()); }
