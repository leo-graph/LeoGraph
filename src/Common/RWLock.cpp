...
class RWLockImpl::LockHolderImpl {
 ...
 private:
  ...
  bool bindWith(RWLock&& parent_, GroupsContainer::iterator it_group_) noexcept {
    if (bound || parent_ == nullptr) return false;
    it_group = it_group_;
 ...
 */
RWLockImpl::LockHolder RWLockImpl::getLock(RWLockImpl::Type type, const String& query_id, const std::chrono::milliseconds& lock_timeout_ms,
                                           bool throw_in_fast_path) {
  ...
  auto finalize_metrics = [type, &watch]() {
    ...
    ProfileEvents::increment((type == Read) ? ProfileEvents::RWLockReadersWaitMilliseconds : ProfileEvents::RWLockWritersWaitMilliseconds,
  ...
  if (type == Type::Write) {
    ...
    if (!can_use_last_group) readers_queue.emplace_back(type);  /// SM1: may throw (nothing to roll back)
  }
  ...
  if (rdlock_owner == readers_queue.end() && wrlock_owner == writers_queue.end()) {
    ...
    if (lock_deadline_tp == std::chrono::time_point<std::chrono::steady_clock>::max()) {
      ...
      if (!wait_result)  /// Wait timed out!
      {
        ...
        if ((rdlock_owner != readers_queue.end()) && writers_queue.empty()) grantOwnershipToAllReaders();
        return nullptr;
      }
    }
  }
  ...
  if (request_has_query_id) {
    try {
      ...
      if (!emplace_res.second) ++emplace_res.first->second;         /// SM3: nothrow
    } catch (...) {
      ...
      if (it_group->requests == 0) dropOwnerGroupAndPassOwnership(it_group);  /// Rollback(SM1): nothrow
...
void RWLockImpl::grantOwnershipToAllReaders() noexcept {
  if (rdlock_owner != readers_queue.end()) {
    ...
    for (;;) {
      if (!rdlock_owner->ownership) ++num_new_owners;
      grantOwnership(rdlock_owner);
      if (std::next(rdlock_owner) == readers_queue.end()) break;
      ++rdlock_owner;
    }
...
String RWLockImpl::getOwnerQueryIdsDescription() const {
  ...
  for (const auto& [query_id, num_owners] : map) {
    if (need_comma) out << ", ";
    out << query_id;
    if (num_owners != 1) out << " (" << num_owners << ")";
    need_comma = true;
  }
  return out.str();
}
