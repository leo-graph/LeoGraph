...
class StringSearcher<true, ASCII> final : public StringSearcherBase {
 ...
 public:
  StringSearcher(const UInt8 *needle_, size_t needle_size)
      : needle(reinterpret_cast<sz_cptr_t>(needle_)), needle_end(needle + needle_size) {}
 ...
 public:
  StringSearcher(const UInt8 *needle_, size_t needle_size_)
    ...
    if (*needle < 0x80u) {
      ...
      if (!first_u32) {
        ...
        if (length_l != length_u) force_fallback = true;
      }
    ...
    for (size_t i = 0; i < N;) {
      ...
      if (cache_actual_len < N) cache_valid_len += src_len;
  ...
  ALWAYS_INLINE bool compareTrivial(const UInt8 *haystack_pos, const UInt8 *const haystack_end, const uint8_t *needle_pos) const {
    while (haystack_pos < haystack_end && needle_pos < needle_end) {
      ...
      if (!haystack_code_point || !needle_code_point) break;

      /// Not equals case insensitive.
      if (Poco::Unicode::toLower(*haystack_code_point) != Poco::Unicode::toLower(*needle_code_point)) break;
  ...
  ALWAYS_INLINE bool compare(const UInt8 * /*haystack*/, const UInt8 *haystack_end, const UInt8 *pos) const {
    ...
    if (isPageSafe(pos) && !force_fallback) {
      ...
      if (0xffff == cachemask) {
        if (mask == cachemask) {
          if (compareTrivial(pos, haystack_end, needle)) return true;
        }
      } else if ((mask & cachemask) == cachemask) {
        if (compareTrivial(pos, haystack_end, needle)) return true;
      }
    ...
    if (*pos == l || *pos == u) {
      ...
      if (compareTrivial(pos, haystack_end, needle_pos)) return true;
    }
  ...
  const UInt8 *search(const UInt8 *haystack, const UInt8 *const haystack_end) const {
    ...
    while (haystack < haystack_end) {
        ...
        if (haystack + N <= haystack_end && isPageSafe(haystack)) {
          ...
          if (0xffff == cachemask) {
            if (mask_offset_both == cachemask) {
              if (compareTrivial(haystack, haystack_end, needle)) return haystack;
            }
      ...
      if (*haystack == l || *haystack == u) {
        ...
        if (compareTrivial(haystack_pos, haystack_end, needle_pos)) return haystack;
      }
...
struct StdLibASCIIStringSearcher {
  ...
  const CharT *search(const CharT *haystack_start, const CharT *const haystack_end) const {
    if constexpr (CaseInsensitive)
      return std::search(haystack_start, haystack_end, needle_start, needle_end,
                         [](char c1, char c2) { return std::toupper(c1) == std::toupper(c2); });
    else
      return std::search(haystack_start, haystack_end, needle_start, needle_end, [](char c1, char c2) { return c1 == c2; });
  }
