...
struct ThreadStack {
  ThreadStack() {
    ...
    if (!data) throw ErrnoException(ErrorCodes::CANNOT_ALLOCATE_MEMORY, "Cannot allocate ThreadStack");
  ...
  ~ThreadStack() {
    if constexpr (guardPagesEnabled()) memoryGuardRemove(data, getPageSize());
  ...
  static size_t getSize() {
    ...
    if constexpr (guardPagesEnabled()) size += getPageSize();
...
ThreadStatus::ThreadStatus() : thread_id(getThreadId()) {
...
void ThreadStatus::clearQueryId() noexcept { query_id.clear(); }

const String& ThreadStatus::getQueryId() const { return query_id; }

ContextPtr ThreadStatus::getQueryContext() const { return query_context.lock(); }

ContextPtr ThreadStatus::getGlobalContext() const { return global_context.lock(); }
...
void ThreadStatus::attachInternalTextLogsQueue(const InternalTextLogsQueuePtr& logs_queue, LogsLevel logs_level) {
  local_data.logs_queue_ptr = logs_queue;
  local_data.client_logs_level = logs_level;

  if (thread_group) thread_group->attachInternalTextLogsQueue(logs_queue, logs_level);
}

InternalTextLogsQueuePtr ThreadStatus::getInternalTextLogsQueue() const { return local_data.logs_queue_ptr.lock(); }

InternalProfileEventsQueuePtr ThreadStatus::getInternalProfileEventsQueue() const { return local_data.profile_queue_ptr.lock(); }

const String& ThreadStatus::getQueryForLog() const { return local_data.query_for_logs; }

LogsLevel ThreadStatus::getClientLogsLevel() const { return local_data.client_logs_level; }

void ThreadStatus::flushUntrackedMemory() {
  if (untracked_memory == 0) return;
...
bool ThreadStatus::isQueryCanceled() const {
  if (!thread_group) return false;

  if (local_data.query_is_canceled_predicate) return local_data.query_is_canceled_predicate();
  return false;
}

size_t ThreadStatus::getNextPlanStepIndex() const { return local_data.plan_step_index->fetch_add(1); }

size_t ThreadStatus::getNextPipelineProcessorIndex() const { return local_data.pipeline_processor_index->fetch_add(1); }
...
void ThreadStatus::updatePerformanceCounters() {
  try {
    ...
    if (taskstats) taskstats->updateCounters(counters);
  } catch (...) {
...
void ThreadStatus::updatePerformanceCountersIfNeeded() {
  if (last_rusage->thread_id == 0) return;  // Performance counters are not initialized, so there is no need to update them
...
void ThreadStatus::onFatalError() {
  if (fatal_error_callback) fatal_error_callback();
}
