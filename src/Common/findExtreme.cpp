...
struct NativeComparatorT {
  using Type = Comparator;
};
template <underlying_has_find_extreme_implementation T>
struct NativeComparatorT<MinComparator<T>> {
  using Type = MinComparator<NativeType<T>>;
};
template <underlying_has_find_extreme_implementation T>
struct NativeComparatorT<MaxComparator<T>> {
  using Type = MaxComparator<NativeType<T>>;
};
template <class Comparator>
using NativeComparator = typename NativeComparatorT<Comparator>::Type;
}  // namespace detail
...
MULTITARGET_FUNCTION_X86_V3(
    MULTITARGET_FUNCTION_HEADER(
        ...
        static std::optional<T>
            NO_INLINE),
    findExtremeImpl,
    MULTITARGET_FUNCTION_BODY((const T *__restrict ptr, const UInt8 *__restrict condition_map [[maybe_unused]], size_t row_begin,
                               size_t row_end)  /// NOLINT
                              {
                                size_t count = row_end - row_begin;
                                ptr += row_begin;
                                if constexpr (!add_all_elements) condition_map += row_begin;
                                ...
                                if constexpr (is_floating_point<T>) {
                                  ...
                                  if (i < unrolled_end) {
                                    ...
                                    for (size_t unroll_it = 0; unroll_it < unroll_block; unroll_it++) partial_min[unroll_it] = ret;
                                  ...
                                  for (; i < count; i++) {
                                    if (add_all_elements || !condition_map[i] == add_if_cond_zero) ret = ComparatorClass::cmp(ret, ptr[i]);
                                  }
                                  return ret;
                                } else {
...
static std::optional<T> findExtreme(const T *__restrict ptr, const UInt8 *__restrict condition_map [[maybe_unused]], size_t start,
                                    size_t end) {
...
static std::optional<T> findExtreme(const T *__restrict ptr, const UInt8 *__restrict condition_map [[maybe_unused]], size_t start,
  ...
  auto ret = findExtreme<U, detail::NativeComparator<ComparatorClass>, add_all_elements, add_if_cond_zero>(reinterpret_cast<const U *>(ptr),
                                                                                                           condition_map, start, end);

  if (ret.has_value()) return T(*ret);
  return std::nullopt;
}
...
std::optional<size_t> findExtremeMinIndex(const T *__restrict ptr, size_t start, size_t end) {
  ...
  if constexpr (is_floating_point<T>) {
    ...
    if (value == ptr[start]) return {start};
    for (size_t i = end - 1; i > start; i--)
      if (ptr[i] == *opt) return {i};
  }
  return std::nullopt;
}
...
#define INSTANTIATION(T)                                                                                                                \
  ...
  template std::optional<T> findExtremeMinNotNull(const T *__restrict ptr, const UInt8 *__restrict condition_map, size_t start,         \
                                                  size_t end);                                                                          \
  ...
  template std::optional<T> findExtremeMaxNotNull(const T *__restrict ptr, const UInt8 *__restrict condition_map, size_t start,         \
                                                  size_t end);                                                                          \
