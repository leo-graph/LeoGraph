...
void LogFrequencyLimiterImpl::log(Poco::Message&& msg) {
  ...
  if (need_cleanup && thread_local_rng() % 100 == 0) cleanup();
  ...
  if (skipped_similar_messages) msg.appendText(fmt::format(" (skipped {} similar messages)", skipped_similar_messages));

  if (auto* channel = logger->getChannel()) channel->log(std::move(msg));
}
...
LogSeriesLimiter::LogSeriesLimiter(LoggerPtr logger_, size_t allowed_count_, time_t interval_s_) : logger(std::move(logger_)) {
  if (allowed_count_ == 0) {
    accepted = false;
    return;
  }
  ...
  if (last_time + interval_s_ <= now) {
    debug_message = fmt::format(" (LogSeriesLimiter: on interval from {} to {} accepted series {} / {} for the logger {})",
                                DateLUT::instance().timeToString(last_time), DateLUT::instance().timeToString(now), accepted_count,
                                total_count, logger->name());
...
void LogSeriesLimiter::log(Poco::Message&& message) {
  ...
  if (auto* channel = logger->getChannel()) channel->log(std::move(message));
}
