...
CalendarTimeInterval::Intervals CalendarTimeInterval::toIntervals() const {
  ...
  greedy(seconds, {{IntervalKind::Kind::Week, 3600 * 24 * 7},
                   {IntervalKind::Kind::Day, 3600 * 24},
                   {IntervalKind::Kind::Hour, 3600},
                   {IntervalKind::Kind::Minute, 60},
                   {IntervalKind::Kind::Second, 1}});
  if (res.empty()) res.emplace_back(IntervalKind::Kind::Second, 0);
  return res;
}

UInt64 CalendarTimeInterval::minSeconds() const { return 3600 * 24 * (months / 12 * 365 + months % 12 * 28) + seconds; }

UInt64 CalendarTimeInterval::maxSeconds() const { return 3600 * 24 * (months / 12 * 366 + months % 12 * 31) + seconds; }
...
void CalendarTimeInterval::assertPositive() const {
  if (!seconds && !months) throw Exception(ErrorCodes::BAD_ARGUMENTS, "Interval must be positive");
}
...
static std::chrono::sys_seconds startOfAbsoluteMonth(Int64 absolute_month) {
  ...
  std::chrono::year_month_day ymd(std::chrono::year(int(year + 1970)), std::chrono::month(unsigned(month + 1)), std::chrono::day(1));
...
std::chrono::sys_seconds CalendarTimeInterval::floor(std::chrono::system_clock::time_point tp) const {
  ...
  if (months) return startOfAbsoluteMonth(toAbsoluteMonth(tp) / months * months);
...
bool CalendarTimeInterval::operator!=(const CalendarTimeInterval& rhs) const { return !(*this == rhs); }
