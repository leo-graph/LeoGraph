...
bool ProgressIndication::updateProgress(const Progress &value) { return progress.incrementPiecewiseAtomically(value); }
...
ProgressIndication::MemoryUsage ProgressIndication::getMemoryUsage() const {
  ...
  return std::accumulate(hosts_data.cbegin(), hosts_data.cend(), MemoryUsage{}, [](MemoryUsage const &acc, auto const &host_data) {
    UInt64 host_usage = host_data.second.memory_usage;
    return MemoryUsage{.total = acc.total + host_usage,
                       .max = std::max(acc.max, host_usage),
                       .peak = std::max(acc.peak, host_data.second.peak_memory_usage)};
  });
}
...
void ProgressIndication::writeFinalProgress() {
  ...
  if (progress.read_rows < 1000) return;
  ...
  if (elapsed_ns)
    output_stream << " ("
                  << formatReadableQuantity(static_cast<double>(progress.read_rows.load()) * 1000000000.0 / static_cast<double>(elapsed_ns))
                  << " rows/s., "
                  << formatReadableSizeWithDecimalSuffix(static_cast<double>(progress.read_bytes.load()) * 1000000000.0 /
...
void ProgressIndication::writeProgress(WriteBufferFromFileDescriptor &message, std::unique_lock<std::mutex> &) {
  ...
  static const char *indicators[8] = {
      "\033[1;30m→\033[0m", "\033[1;31m↘\033[0m", "\033[1;32m↓\033[0m", "\033[1;33m↙\033[0m",
      "\033[1;34m←\033[0m", "\033[1;35m↖\033[0m", "\033[1;36m↑\033[0m", "\033[1m↗\033[0m",
  };
  ...
  message << formatReadableQuantity(progress.read_rows.load()) << " rows, "
          << formatReadableSizeWithDecimalSuffix(progress.read_bytes.load());
  ...
  if (cpu_usage > 0 || memory_usage > 0) {
    ...
    if (memory_usage > 0) profiling_msg_builder << ", " << formatReadableSizeWithDecimalSuffix(memory_usage) << " RAM";
    if (max_host_usage < memory_usage) profiling_msg_builder << ", " << formatReadableSizeWithDecimalSuffix(max_host_usage) << " max/host";
  ...
  if (progress.total_rows_to_read || progress.total_bytes_to_read) {
    ...
    if (elapsed_ns > 500000000) {
      ...
      if (current_count * 2 < max_count) show_progress_bar = true;
      ...
      if (show_progress_bar) {
        ...
        if (width_of_progress_bar <= 1 + 2 * static_cast<int64_t>(profiling_msg.size())) profiling_msg.clear();

        if (width_of_progress_bar > 0) {
          double bar_width = UnicodeBar::getWidth(static_cast<double>(current_count), 0, static_cast<double>(max_count),
                                                  static_cast<double>(width_of_progress_bar));
          ...
          if (profiling_msg.empty()) {
            message << "\033[0;32m" << bar << "\033[0m" << std::string(width_of_progress_bar - bar_width_in_terminal, ' ');
          } else {
            ...
            if (render_profiling_msg_at_left) {
              ...
              message << "\033[0;32m" << bar << "\033[0m"
                      << std::string(width_of_progress_bar - bar_width_in_terminal - profiling_msg.size(), ' ') << "\033[2m"
                      << profiling_msg << "\033[0m";
            }
          }
        }
      }
    ...
    if (remaining_space >= static_cast<int64_t>(profiling_msg.size())) {
      if (remaining_space > static_cast<int64_t>(profiling_msg.size())) message << std::string(remaining_space - profiling_msg.size(), ' ');
