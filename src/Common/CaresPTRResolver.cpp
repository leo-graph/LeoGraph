...
static void socket_state_change_callback(void *data, ares_socket_t socket_fd, int readable, int writable) {
  ...
  if (idx >= state->poll_nfds) {
    ...
    if (state->poll_nfds > state->poll_fds_alloc) {
      ...
      {
        auto *new_poll_fds = static_cast<struct pollfd *>(std::realloc(state->poll_fds, sizeof(*state->poll_fds) * state->poll_nfds));

        if (!new_poll_fds) throw std::bad_alloc();

        state->poll_fds = new_poll_fds;
      }

      {
        auto *new_ares_fds = static_cast<ares_fd_events_t *>(std::realloc(state->ares_fds, sizeof(*state->ares_fds) * state->poll_nfds));

        if (!new_ares_fds) throw std::bad_alloc();
...
struct AresChannelRAII {
  ...
  dnsstate_t &getDNSState() { return dns_state; }
...
CaresPTRResolver::CaresPTRResolver(CaresPTRResolver::provider_token) {
   ...
   * to grpc. As discussed in https://github.com/ClickHouse/ClickHouse/pull/37827#discussion_r919189085, c-ares should be adapted to be
   * atomic
   *
