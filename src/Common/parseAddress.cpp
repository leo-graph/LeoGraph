...
std::pair<std::string, UInt16> parseAddress(const std::string& str, UInt16 default_port) {
  if (str.empty()) throw Exception(ErrorCodes::BAD_ARGUMENTS, "Empty address passed to function parseAddress");
  ...
  if (port != end) {
    if (*port != ':') throw Exception(ErrorCodes::BAD_ARGUMENTS, "Illegal port prefix passed to function parseAddress: {}", port);
    ...
    if (!tryReadText(port_number, port_buf) || !port_buf.eof()) {
      throw Exception(ErrorCodes::BAD_ARGUMENTS, "Illegal port passed to function parseAddress: {}", port);
    }
