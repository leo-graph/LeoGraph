...
struct RapidJSONParser {
    ...
    ALWAYS_INLINE ElementType type() const {
      switch (ptr->GetType()) {
        case rapidjson::kNumberType:
          return ptr->IsDouble() ? ElementType::DOUBLE : (ptr->IsUint64() ? ElementType::UINT64 : ElementType::INT64);
  ...
  class Object {
   public:
    ...
    bool find(std::string_view key_, Element& result) const {
      ...
      if (it == ptr->MemberEnd()) return false;
  ...
  bool parse(std::string_view json, Element& result) {
    ...
    if (document.HasParseError() || (ms.Tell() != json.size())) return false;
    result = document;
    return true;
  }
...
inline ALWAYS_INLINE RapidJSONParser::Array RapidJSONParser::Element::getArray() const { return *ptr; }

inline ALWAYS_INLINE RapidJSONParser::Object RapidJSONParser::Element::getObject() const { return *ptr; }
