...
void SharedMutex::lock() {
  /// Try fast acquire
  UInt64 value = 0;
  if (likely(state.compare_exchange_strong(value, writers))) return;
  ...
  /// Drain readers
  value |= writers;
  while (value & readers) futexWaitLowerFetch(state, value);
}
...
void SharedMutex::unlock() {
  state.store(0);
  if (waiters) futexWakeUpperAll(state);
}
...
bool SharedMutex::try_lock_shared() {
  UInt64 value = state.load();
  while (true) {
    if (value & writers) return false;
    if (state.compare_exchange_strong(value, value + 1)) break;
...
void SharedMutex::unlock_shared() {
  ...
  if (value == writers) futexWakeLowerOne(state);  // Wake writer
}
