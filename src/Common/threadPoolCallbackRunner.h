...
ThreadPoolCallbackRunnerUnsafe<Result, Callback> threadPoolCallbackRunnerUnsafe(ThreadPool &pool, ThreadName thread_name) {
  return [my_pool = &pool, thread_group = CurrentThread::getGroup(), thread_name](Callback &&callback,
                                                                                  Priority priority) mutable -> std::future<Result> {
    auto task =
        std::make_shared<std::packaged_task<Result()>>([thread_group, thread_name, my_callback = std::move(callback)]() mutable -> Result {
...
class ThreadPoolCallbackRunnerLocal final {
 ...
 private:
  ...
  static void executeCallback(std::promise<FunctionResult> &promise, Function &&callback, ThreadGroupPtr thread_group,
                              ThreadName thread_name) {
 ...
 public:
  ThreadPoolCallbackRunnerLocal(PoolT &pool_, ThreadName thread_name_) : pool(pool_), thread_name(thread_name_) {}
  ...
  [[nodiscard]] std::shared_ptr<Task> enqueueAndGiveOwnership(std::reference_wrapper<Fn> callback, Priority priority = {},
  ...
  /// You are responsible for handling it from now on, checking its status and so on. You must implement your own waitForAllToFinish*
  /// equivalent You must ensure that all returned tasks are waited upon (i.e., their futures are completed) before the ThreadPool is
  /// destroyed. Otherwise, the task's lambda may reference a destroyed pool state, leading to undefined behavior.
  [[nodiscard]] std::shared_ptr<Task> enqueueAndGiveOwnership(Callback &&callback, Priority priority = {},
  ...
  static void waitForAllToFinishAndRethrowFirstError(std::vector<std::shared_ptr<Task>> &tasks) {
    ...
    for (auto &task : tasks) {
      ...
      if (task->future.valid()) task->future.get();
    }
...
class ThreadPoolCallbackRunnerFast {
 public:
  ...
  void initManual() { mode = Mode::Manual; }
