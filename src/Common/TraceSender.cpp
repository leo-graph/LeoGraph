...
void TraceSender::send(TraceType trace_type, const StackTrace &stack_trace, Extras extras) {
   ...
   */
  if (unlikely(inside_send)) return;
  inside_send = true;
  ...
  static_assert(buf_size <= PIPE_BUF,
  ...
  if (CurrentThread::isInitialized()) {
    ...
    if (query_id.size() > QUERY_ID_MAX_LEN) query_id.remove_suffix(query_id.size() - QUERY_ID_MAX_LEN);
  ...
  for (size_t i = stack_trace_offset; i < stack_trace_size; ++i) writePODBinary(stack_trace.getFramePointers()[i], out);
