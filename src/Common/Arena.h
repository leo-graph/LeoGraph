...
class Arena : private boost::noncopyable {
 private:
  ...
  struct alignas(16) MemoryChunk : private Allocator<false>  /// empty base optimization
  {
    ...
    MemoryChunk(MemoryChunk &&other) noexcept { *this = std::move(other); }
 ...
 public:
  ...
  char *alloc(size_t size) {
    used_bytes += size;
    if (unlikely(head.empty() || size > head.remaining())) addMemoryChunk(size);
  ...
  char *alignedAlloc(size_t size, size_t alignment) {
    used_bytes += size;
    if (unlikely(head.empty() || size > head.remaining())) addMemoryChunk(size + alignment);
