...
OvercommitResult OvercommitTracker::needToStopQuery(MemoryTracker* tracker, Int64 amount) {
  ...
  if (OvercommitTrackerBlockerInThread::isBlocked()) return OvercommitResult::NONE;
  ...
  if (max_wait_time == ZERO_MICROSEC) return OvercommitResult::DISABLED;
  ...
  bool timeout =
      !cv.wait_for(lk, max_wait_time, [this, id]() { return id < id_to_release || cancellation_state == QueryCancellationState::NONE; });
  ...
  if (allow_release && required_memory <= freed_memory && still_need) releaseThreads();
  ...
  if (required_memory == 0 && cancellation_state == QueryCancellationState::SELECTED) reset();
  if (timeout) return OvercommitResult::TIMEOUTED;
  if (still_need) return OvercommitResult::NOT_ENOUGH_FREED;
...
void OvercommitTracker::tryContinueQueryExecutionAfterFree(Int64 amount) {
  ...
  if (OvercommitTrackerBlockerInThread::isBlocked()) return;

  {
    std::shared_lock read_lock(overcommit_m);
    if (cancellation_state == QueryCancellationState::NONE) return;
  }

  std::lock_guard lk(overcommit_m);
  if (cancellation_state != QueryCancellationState::NONE) {
    freed_memory += amount;
    if (freed_memory >= required_memory) releaseThreads();
  }
}
...
void OvercommitTracker::onQueryStop(MemoryTracker* tracker) {
  ...
  {
    std::shared_lock read_lock(overcommit_m);
    if (picked_tracker != tracker) return;
  }
...
UserOvercommitTracker::UserOvercommitTracker(DB::ProcessList* process_list_, DB::ProcessListForUser* user_process_list_)
    : OvercommitTracker(process_list_), user_process_list(user_process_list_) {}
...
void UserOvercommitTracker::pickQueryToExcludeImpl() {
  ...
  for (auto const& query : queries) {
    if (query.second->isKilled()) continue;

    auto* memory_tracker = query.second->getMemoryTracker();
    if (!memory_tracker) continue;
...
GlobalOvercommitTracker::GlobalOvercommitTracker(DB::ProcessList* process_list_) : OvercommitTracker(process_list_) {}
