...
bool Span::addAttributeIfNotEmpty(std::string_view name, std::string_view value) noexcept {
  if (!this->isTraceEnabled() || name.empty() || value.empty()) return false;

  return addAttributeImpl(name, value);
}

bool Span::addAttribute(std::string_view name, std::function<String()> value_supplier) noexcept {
  if (!this->isTraceEnabled() || name.empty() || !value_supplier) return false;
...
bool Span::addAttribute(const Exception& e) noexcept {
  ...
  return addAttributeImpl("clickhouse.exception", getExceptionMessage(e, false)) && addAttributeImpl("clickhouse.exception_code", e.code());
}

bool Span::addAttribute(std::exception_ptr e) noexcept {
  if (!this->isTraceEnabled() || e == nullptr) return false;
...
bool Span::addAttribute(const ExecutionStatus& e) noexcept {
  ...
  return addAttributeImpl("clickhouse.exception", e.message) && addAttributeImpl("clickhouse.exception_code", e.code);
}

SpanHolder::SpanHolder(std::string_view _operation_name, SpanKind _kind, bool create_trace_if_not_exists) {
...
SpanHolder::~SpanHolder() { finish(std::chrono::system_clock::now()); }
...
String TracingContext::composeTraceparentHeader() const {
  ...
  // parent id.
  return fmt::format("00-{:016x}{:016x}-{:016x}-{:02x}", UUIDHelpers::getHighBytes(trace_id), UUIDHelpers::getLowBytes(trace_id), span_id,
...
UInt64 TracingContext::generateSpanId() { return thread_local_rng(); }

const TracingContextOnThread& CurrentContext() { return *current_trace_context; }
...
TracingContextHolder::TracingContextHolder(std::string_view _operation_name, TracingContext _parent_trace_context,
                                           const Settings* settings_ptr, const std::weak_ptr<OpenTelemetrySpanLog>& _span_log) {
  /// Use try-catch to make sure the ctor is exception safe.
  ...
  try {
    if (current_trace_context->isTraceEnabled()) {
      ...
      ///    1. bad design
      ///    2. right design but code changes so that original point where this object is constructing is not the new start execution of
      ///    current thread
      ///
