...
void StudentTTestOneSample::clear() { data.clear(); }

void StudentTTestOneSample::setPopulationMean(double mean) { data.population_mean = mean; }

void StudentTTestOneSample::add(double value) { data.add(value); }
...
std::pair<bool, std::string> StudentTTestOneSample::compareAndReport(size_t confidence_level_index) const {
  ...
  if (data.size == 1) return {true, "Insufficient data for t-test (need at least 2 observations)"};
