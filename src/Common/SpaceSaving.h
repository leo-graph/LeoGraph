...
struct SpaceSavingArena<std::string_view> {
  ...
  void free(std::string_view key) {
    if (key.data()) arena.free(const_cast<char *>(key.data()), key.size());
  }
...
class SpaceSaving {
 ...
 public:
  ...
  struct Counter {
    ...
    explicit Counter(const TKey &k, UInt64 c = 0, UInt64 e = 0, size_t h = 0) : key(k), hash(h), count(c), error(e) {}
  ...
  void merge(const Self &rhs) {
    ...
    if (alpha_map.size() == rhs.alpha_map.size()) {
      for (size_t i = 0; i < alpha_map.size(); i++) alpha_map[i] = std::min(alpha_map[i] + rhs.alpha_map[i], MAX_ALPHA_VALUE);
    }
  ...
  std::vector<Counter> topK(size_t k) const {
    ...
    for (auto &counter : counter_list) new_list.push_back(counter);

    size_t return_size = std::min(new_list.size(), k);
    ::partial_sort(new_list.begin(), new_list.begin() + return_size, new_list.end(), [](const auto &l, const auto &r) { return l > r; });
  ...
  void write(WriteBuffer &wb) const {
    ...
    for (size_t i = 0; i < return_size; i++) new_list[i].write(wb);

    writeVarUInt(alpha_map.size(), wb);
    for (auto alpha : alpha_map) writeVarUInt(alpha, wb);
  }
  ...
  void read(ReadBuffer &rb, size_t capacity) {
    ...
    readVarUInt(count, rb);
    if (count > capacity)
      throw DB::Exception(DB::ErrorCodes::TOO_LARGE_ARRAY_SIZE, "Found too large when reading counters (Passed: {}. Maximum: {})", count,
                          capacity);
  ...
  void readAlphaMap(ReadBuffer &rb) {
    ...
    if (alpha_size != expected_capacity)
      throw DB::Exception(DB::ErrorCodes::SIZES_OF_ARRAYS_DONT_MATCH, "Found incorrect alpha vector size (Passed: {}. Expected: {})",
                          alpha_size, expected_capacity);
 ...
 protected:
  ...
  ALWAYS_INLINE void truncateIfNeeded(bool force_counter_map_rebuild) {
    if (unlikely(counter_list.size() >= target_capacity)) {
      ::nth_element(counter_list.begin(), counter_list.begin() + capacity(), counter_list.end(),
    ...
    if (force_counter_map_rebuild) {
      ...
      counter_map.clear();
      for (size_t i = 0; i < counter_list.size(); ++i) counter_map.insertIfNotPresent(counter_list[i].key, counter_list[i].hash, i);
    }
  }

 private:
  void destroyElements() {
    for (auto &counter : counter_list) arena.free(counter.key);
  ...
  SpaceSaving &operator=(const SpaceSaving &rhs) {
    ...
    if constexpr (std::is_same_v<TKey, std::string_view>) {
      ...
      for (auto &counter : counter_list) counter.key = arena.emplace(counter.key);
    }
    truncateIfNeeded(true);
