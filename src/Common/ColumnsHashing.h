...
struct HashMethodSingleLowCardinalityColumn : public SingleColumnMethod {
  ...
  static const ColumnLowCardinality &getLowCardinalityColumn(const IColumn *column) {
    ...
    if (!low_cardinality_column)
      throw Exception(ErrorCodes::LOGICAL_ERROR,
                      "Invalid aggregation key type for HashMethodSingleLowCardinalityColumn method. "
                      "Excepted LowCardinality, got {}",
                      column->getName());
    return *low_cardinality_column;
  }

  HashMethodSingleLowCardinalityColumn(const ColumnRawPtrs &key_columns_low_cardinality, const Sizes &key_sizes,
                                       const HashMethodContextPtr &context)
      : Base({getLowCardinalityColumn(key_columns_low_cardinality[0]).getDictionary().getNestedNotNullableColumn().get()}, key_sizes,
             context) {
    const auto *column = &getLowCardinalityColumn(key_columns_low_cardinality[0]);

    if (!context) throw Exception(ErrorCodes::LOGICAL_ERROR, "Cache wasn't created for HashMethodSingleLowCardinalityColumn");
    ...
    if constexpr (has_mapped) mapped_cache.resize(key_columns[0]->size());
  ...
  ALWAYS_INLINE auto getKeyHolder(size_t row, Arena &pool) const { return Base::getKeyHolder(getIndexAt(row), pool); }
  ...
  ALWAYS_INLINE FindResult findKey(Data &data, size_t row_, Arena &pool) {
    ...
    if constexpr (has_mapped) {
      if (found) mapped_cache[row] = it->getMapped();
    }

    size_t offset = 0;

    if constexpr (FindResult::has_offset) offset = found ? data.offsetInternal(it) : 0;
  ...
  ALWAYS_INLINE size_t getHash(const Data &data, size_t row, Arena &pool) {
    row = getIndexAt(row);
    if (saved_hash) return saved_hash[row];
...
class HashMethodSerializedContext : public HashMethodContext {
 public:
  explicit HashMethodSerializedContext(const HashMethodContextSettings &settings_) : settings(settings_) {}
...
struct HashMethodSerialized
  ...
  HashMethodSerialized(const ColumnRawPtrs &key_columns_, const Sizes & /*key_sizes*/, const HashMethodContextPtr &context)
    ...
    if (!hash_serialized_context) {
      const auto &cached_val = *context;
      throw Exception(ErrorCodes::LOGICAL_ERROR, "Invalid type for HashMethodSerialized context: {}", demangle(typeid(cached_val).name()));
    }
    ...
    if constexpr (prealloc) {
      ...
      for (size_t i = 0; i < keys_size; ++i) key_columns[i]->collectSerializedValueSizes(row_sizes, null_maps[i], &serialization_settings);

      for (auto row_size : row_sizes) total_size += row_size;
...
extern template class CacheBase<ColumnsHashing::LowCardinalityDictionaryCache::DictionaryKey,
