...
class TSA_SCOPED_LOCKABLE SharedLockGuard {
 public:
  ...
  explicit SharedLockGuard(Mutex& mutex_, std::defer_lock_t) : shared_lock(mutex_, std::defer_lock), locked(false) {}
  ~SharedLockGuard() TSA_RELEASE() = default;

  SharedLockGuard(SharedLockGuard&& shared_lock_guard_) noexcept
      : shared_lock(std::move(shared_lock_guard_.shared_lock)), locked(shared_lock_guard_.locked) {
    shared_lock_guard_.locked = false;
  }

  void lock() TSA_ACQUIRE_SHARED() {
    if (locked) throw Exception(ErrorCodes::LOGICAL_ERROR, "Can't lock twice the same mutex");
  ...
  void unlock() TSA_RELEASE() {
    if (!locked) throw Exception(ErrorCodes::LOGICAL_ERROR, "Can't unlock the mutex without locking it first");
