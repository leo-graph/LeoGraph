...
MULTITARGET_FUNCTION_X86_V3(MULTITARGET_FUNCTION_HEADER(template <iota_supported_types T> static void NO_INLINE), iotaImpl,
                            MULTITARGET_FUNCTION_BODY((T * begin, size_t count, T first_value)  /// NOLINT
                                                      {
                                                        for (size_t i = 0; i < count; i++) *(begin + i) = static_cast<T>(first_value + i);
                                                      }))

template <iota_supported_types T>
void iota(T* begin, size_t count, T first_value) {
#if USE_MULTITARGET_CODE
  if (isArchSupported(TargetArch::x86_64_v3)) return iotaImpl_x86_64_v3(begin, count, first_value);
#endif
  return iotaImpl(begin, count, first_value);
}

MULTITARGET_FUNCTION_X86_V3(MULTITARGET_FUNCTION_HEADER(template <iota_supported_types T> static void NO_INLINE), iotaWithStepImpl,
                            MULTITARGET_FUNCTION_BODY((T * begin, size_t count, T first_value, T step)  /// NOLINT
                                                      {
...
#if USE_MULTITARGET_CODE
  if (isArchSupported(TargetArch::x86_64_v3)) return iotaWithStepImpl_x86_64_v3(begin, count, first_value, step);
#endif
