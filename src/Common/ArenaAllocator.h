...
class AlignedArenaAllocator {
 public:
  static void *alloc(size_t size, Arena *arena) { return arena->alignedAlloc(size, alignment); }
...
template <size_t REAL_ALLOCATION_THRESHOLD = 4096, typename TRealAllocator = Allocator<false>, typename TArenaAllocator = ArenaAllocator,
          size_t alignment = 0>
class MixedArenaAllocator : private TRealAllocator {
 public:
  ...
  void *realloc(void *buf, size_t old_size, size_t new_size, Arena *arena) {
    ...
    if (new_size < REAL_ALLOCATION_THRESHOLD) return TArenaAllocator::realloc(buf, old_size, new_size, arena);

    if (old_size >= REAL_ALLOCATION_THRESHOLD) return TRealAllocator::realloc(buf, old_size, new_size, alignment);
  ...
  void free(void *buf, size_t size) {
    if (size >= REAL_ALLOCATION_THRESHOLD) TRealAllocator::free(buf, size);
  }
...
using MixedAlignedArenaAllocator =
    MixedArenaAllocator<REAL_ALLOCATION_THRESHOLD, Allocator<false>, AlignedArenaAllocator<alignment>, alignment>;
