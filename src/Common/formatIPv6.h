...
inline bool parseIPv4(T &src, IsEOF eof, unsigned char *dst, int32_t first_octet = -1) {
  if (first_octet > 255) return false;
...
#if defined(__SSE4_1__)
  if (parseIPv4SSE(src, end - src, reinterpret_cast<uint32_t *>(dst))) return end;
  return nullptr;
#else
  if (parseIPv4(src, [&src, end]() { return src == end; }, dst)) return src;
  return nullptr;
#endif
}

/// returns true if whole buffer was parsed successfully
inline bool parseIPv4whole(const char *src, const char *end, unsigned char *dst) { return parseIPv4(src, end, dst) == end; }

/// returns pointer to the right after parsed sequence or null on failed parsing
inline const char *parseIPv4(const char *src, unsigned char *dst) {
  if (parseIPv4(src, []() { return false; }, dst)) return src;
  return nullptr;
}
...
inline bool parseIPv6(T &src, IsEOF eof, unsigned char *dst, int32_t first_block = -1) {
  ...
  while (!eof() && groups < 8) {
    ...
    if (*src == '.')  /// mixed IPv4 parsing
    {
      ...
      if constexpr (std::endian::native == std::endian::little) std::reverse(iter, iter + IPV4_BINARY_LENGTH);
...
inline const char *parseIPv6(const char *src, const char *end, unsigned char *dst) {
  if (parseIPv6(src, [&src, end]() { return src == end; }, dst)) return src;
  return nullptr;
}

/// returns true if whole buffer was parsed successfully
inline bool parseIPv6Whole(const char *src, const char *end, unsigned char *dst) { return parseIPv6(src, end, dst) == end; }

/// returns pointer to the right after parsed sequence or null on failed parsing
inline const char *parseIPv6(const char *src, unsigned char *dst) {
  if (parseIPv6(src, []() { return false; }, dst)) return src;
  return nullptr;
}
...
inline bool parseIPv6orIPv4(T &src, IsEOF eof, unsigned char *dst) {
  ...
  if (src == nullptr) return clear_dst();
  ...
  for (; !eof() && digits < 4; ++src, ++digits) {
    ...
    if (digits == 0 && *src == '0') leading_zero = true;

    UInt8 num = unhex(*src);
    if (num == 0xFF) return clear_dst();
    (val <<= 4) |= num;
  }
 ...
 *         > dst == "127.0.0.0"
 */
inline void formatIPv4(const unsigned char *src, size_t src_size, char *&dst, uint8_t mask_tail_octets = 0,
