...
class IntervalTree {
 public:
  ...
  ALWAYS_INLINE bool emplace(Interval interval) {
    ...
    if (unlikely(interval.left > interval.right)) return false;
  ...
  ALWAYS_INLINE bool emplace(Interval interval, Args &&...args) {
    ...
    sorted_intervals.emplace_back(std::piecewise_construct, std::forward_as_tuple(interval),
  ...
  iterator end() {
    ...
    if (likely(end_index < nodes.size())) last_interval_index = nodes[end_index].sorted_intervals_range_size;
  ...
  class Iterator {
   ...
   private:
    ...
    void iterateToNext() {
      ...
      if (current_interval_index < current_node.sorted_intervals_range_size) return;
 ...
 private:
  ...
  void findIntervalsImpl(IntervalStorageType point, IntervalCallback &&callback) const {
    ...
    while (true) {
      if (current_index >= nodes.size()) break;
      ...
      if (point < middle_element) {
        ...
        for (; start != end; ++start) {
          ...
          if (interval_left_sorted_asc.left > point) break;
        ...
        for (; start != end; ++start) {
          ...
          if (interval_right_sorted_desc.right < point) break;
  ...
  void findIntervalsNonConstructedImpl(IntervalStorageType point, IntervalCallback &&callback) const {
    for (auto &interval_with_value : sorted_intervals) {
      ...
      if (interval.contains(point)) callCallback(interval_with_value, callback);
    }
  }
  ...
  size_t findFirstIteratorNodeIndex() const {
    ...
    for (; result_index < nodes_size; ++result_index) {
      if (nodes[result_index].hasValue()) break;
    }

    if (unlikely(result_index == nodes_size)) result_index = 0;

    return result_index;
  }

  size_t findLastIteratorNodeIndex() const {
    if (unlikely(nodes.empty())) return 0;
  ...
  static void intervalsToPoints(const std::vector<IntervalWithValue> &intervals,
