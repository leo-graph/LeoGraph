...
ZooKeeperWithFaultInjection::ZooKeeperWithFaultInjection(zkutil::ZooKeeper::Ptr const &keeper_, double fault_injection_probability,
                                                         UInt64 fault_injection_seed, std::string name_, LoggerPtr logger_)
    : keeper(keeper_),
  ...
  if (keeper) {
    for (const auto &path_created : session_ephemeral_nodes) {
      try {
        ...
        if (logger) LOG_TRACE(logger, "Failed to delete ephemeral node ({}) during fault cleanup: {}", path_created, e.what());
      }
    }
  }
...
void ZooKeeperWithFaultInjection::multiResponseSaveEphemeralNodePaths(const Coordination::Requests &requests,
...
void ZooKeeperWithFaultInjection::injectFailureBeforeOperationThrow(const char *func_name, const String &path) {
  if (unlikely(!keeper)) {
    ...
    if (logger) LOG_TRACE(logger, "ZooKeeperWithFaultInjection called after fault: seed={}, func={} path={}", seed, func_name, path);
  ...
  if (unlikely(fault_policy) && fault_policy->beforeOperation()) {
    if (logger)
      LOG_TRACE(logger, "ZooKeeperWithFaultInjection call FAILED: seed={} func={} path={} code={} message={} ", seed, func_name, path,
                RandomFaultInjection::error_before_op, RandomFaultInjection::msg_before_op);
    resetKeeper();
...
void ZooKeeperWithFaultInjection::injectFailureAfterOperationThrow(const char *func_name, const String &path) {
  if (unlikely(fault_policy) && fault_policy->afterOperation()) {
    if (logger)
      LOG_TRACE(logger, "ZooKeeperWithFaultInjection call FAILED: seed={} func={} path={} code={} message={} ", seed, func_name, path,
                RandomFaultInjection::error_after_op, RandomFaultInjection::msg_after_op);
    resetKeeper();
...
std::invoke_result_t<Operation> ZooKeeperWithFaultInjection::executeWithFaultSync(const char *func_name, const std::string &path,
                                                                                  Operation operation) {
...
bool ZooKeeperWithFaultInjection::injectFailureBeforeOperationPromise(const char *func_name, Promise &promise, const String &path) {
  ...
  if (unlikely(fault_policy) && fault_policy->beforeOperation()) {
    if (logger)
      LOG_TRACE(logger, "ZooKeeperWithFaultInjection injected fault before operation: seed={} func={} path={}", seed, func_name, path);
    resetKeeper();
    promise->set_exception(std::make_exception_ptr(
...
Strings ZooKeeperWithFaultInjection::getChildren(const std::string &path, Coordination::Stat *stat, const Coordination::EventPtr &watch,
                                                 Coordination::ListRequestType list_request_type) {
  return executeWithFaultSync(__func__, path, [&]() { return keeper->getChildren(path, stat, watch, list_request_type); });
}

zkutil::ZooKeeper::MultiGetChildrenResponse ZooKeeperWithFaultInjection::getChildren(const std::vector<std::string> &paths,
                                                                                     Coordination::ListRequestType list_request_type) {
  return executeWithFaultSync(__func__, !paths.empty() ? paths.front() : "",
                              [&]() { return keeper->getChildren(paths, list_request_type); });
}

Coordination::Error ZooKeeperWithFaultInjection::tryGetChildren(const std::string &path, Strings &res, Coordination::Stat *stat,
...
zkutil::ZooKeeper::MultiTryGetChildrenResponse ZooKeeperWithFaultInjection::tryGetChildren(
    const std::vector<std::string> &paths, Coordination::ListRequestType list_request_type) {
  return executeWithFaultSync(__func__, !paths.empty() ? paths.front() : "",
                              [&]() { return keeper->tryGetChildren(paths, list_request_type); });
}

Strings ZooKeeperWithFaultInjection::getChildrenWatch(const std::string &path, Coordination::Stat *stat,
...
bool ZooKeeperWithFaultInjection::tryGet(const std::string &path, std::string &res, Coordination::Stat *stat,
                                         const Coordination::EventPtr &watch, Coordination::Error *code) {
...
std::string ZooKeeperWithFaultInjection::create(const std::string &path, const std::string &data, int32_t mode) {
  return executeWithFaultSync(__func__, path, [&]() {
...
Coordination::Responses ZooKeeperWithFaultInjection::multi(const Coordination::Requests &requests, bool check_session_valid) {
  return executeWithFaultSync(__func__, !requests.empty() ? requests.front()->getPath() : "", [&]() {
    auto responses = keeper->multi(requests, check_session_valid);
    if (unlikely(fault_policy)) multiResponseSaveEphemeralNodePaths(requests, responses);
    return responses;
  });
}
...
bool ZooKeeperWithFaultInjection::tryRemoveChildrenRecursive(const std::string &path, bool probably_flat,
                                                             zkutil::RemoveException keep_child) {
...
Coordination::Error ZooKeeperWithFaultInjection::trySet(const std::string &path, const std::string &data, int32_t version,
...
void ZooKeeperWithFaultInjection::deleteEphemeralNodeIfContentMatches(const std::string &path,
                                                                      const std::string &fast_delete_if_equal_value) {
  executeWithFaultSync(__func__, path, [&]() { keeper->deleteEphemeralNodeIfContentMatches(path, fast_delete_if_equal_value); });
}
...
Coordination::Error ZooKeeperWithFaultInjection::tryMulti(const Coordination::Requests &requests, Coordination::Responses &responses,
  ...
  return executeWithFaultSync(__func__, !requests.empty() ? requests.front()->getPath() : "", [&]() {
    ...
    if (unlikely(fault_policy) && code == Coordination::Error::ZOK) multiResponseSaveEphemeralNodePaths(requests, responses);
    return code;
  });
}

Coordination::Error ZooKeeperWithFaultInjection::tryMultiNoThrow(const Coordination::Requests &requests, Coordination::Responses &responses,
                                                                 bool check_session_valid) {
  try {
...
zkutil::ZooKeeper::FutureGet ZooKeeperWithFaultInjection::asyncTryGet(std::string path) {
  ...
  if (injectFailureBeforeOperationPromise(__func__, promise, path)) return future;

  const char *function_name = __func__;
  auto callback = [&, promise](const Coordination::GetResponse &response) mutable {
    if (injectFailureAfterOperationPromise(function_name, promise, path)) return;
...
zkutil::ZooKeeper::FutureMulti ZooKeeperWithFaultInjection::asyncTryMultiNoThrow(std::span<const Coordination::RequestPtr> ops) {
  ...
  for (const auto &op : ops) {
    ...
    if (create_req) chassert(!create_req->is_ephemeral);
  }
#endif
  ...
  auto callback = [&, promise](const Coordination::MultiResponse &response) mutable {
    if (unlikely(fault_policy) && fault_policy->afterOperation()) {
      if (logger)
        LOG_TRACE(logger, "ZooKeeperWithFaultInjection injected fault after operation: seed={} func={} path={}", seed, function_name, path);
      resetKeeper();
      Coordination::MultiResponse errors;
      for (size_t i = 0; i < request_size; i++) {
        auto r = std::make_shared<Coordination::ZooKeeperErrorResponse>();
