...
static void check(Coordination::Error code, const std::string &path) {
  if (code != Coordination::Error::ZOK) throw KeeperException::fromPath(code, path);
}
...
void ZooKeeper::init(ZooKeeperArgs args_, std::unique_ptr<Coordination::IKeeper> existing_impl) {
  ...
  if (existing_impl) {
    ...
    if (availability_zones.empty()) {
      ...
      if (args.availability_zone_autodetect) updateAvailabilityZones();
    }
    ...
    if (connected_to_suboptimal_node && may_benefit_from_reconnecting) {
      ...
      LOG_DEBUG(log,
                "Connected to a suboptimal ZooKeeper host ({}, index {})."
      ...
      auto reconnect_task_holder = DB::Context::getGlobalContextInstance()->getSchedulePool().createTask(
          DB::StorageID::createEmpty(), "ZKReconnect", [this, optimal_host = shuffled_hosts[0]]() {
            ...
            try {
              ...
              std::unique_ptr<Coordination::IKeeper> new_impl =
                  std::make_unique<Coordination::ZooKeeper>(node, args, zk_log, aggregated_zookeeper_log);
              ...
              LOG_WARNING(log, "Failed to connect to a more optimal ZooKeeper, will try again later: {}",
  ...
  if (!args.chroot.empty()) {
    ...
    if (!(code == Coordination::Error::ZOK || code == Coordination::Error::ZNONODE)) throw KeeperException::fromPath(code, "/");

    if (code == Coordination::Error::ZNONODE)
      throw KeeperException(Coordination::Error::ZNONODE, "ZooKeeper root doesn't exist. You should create root node {} before start.",
                            args.chroot);
  }
}

ZooKeeper::~ZooKeeper() {
  if (reconnect_task) (*reconnect_task)->deactivate();
}
...
ZooKeeper::ZooKeeper(const ZooKeeperArgs &args_, std::shared_ptr<DB::ZooKeeperLog> zk_log_,
                     std::shared_ptr<DB::AggregatedZooKeeperLog> aggregated_zookeeper_log_, Strings availability_zones_,
...
ShuffleHosts ZooKeeper::shuffleHosts() const {
  ...
  for (size_t i = 0; i < args.hosts.size(); ++i) {
    ...
    if (shuffle_host.secure) shuffle_host.host.erase(0, strlen("secure://"));
    ...
    if (get_priority) shuffle_host.priority = get_priority(i);
...
bool ZooKeeper::configChanged(const Poco::Util::AbstractConfiguration &config, const std::string &config_name) const {
  ...
  if (new_args.implementation == args.implementation && args.implementation == "testkeeper") return false;
...
Coordination::Error ZooKeeper::getChildrenImpl(const std::string &path, Strings &res, Coordination::Stat *stat,
                                               Coordination::WatchCallbackPtrOrEventPtr watch_callback,
                                               Coordination::ListRequestType list_request_type, bool with_stat, bool with_data) {
  ...
  if (code == Coordination::Error::ZOK) {
    res = response.names;
    if (stat) *stat = response.stat;
  }
  return code;
}

Strings ZooKeeper::getChildren(const std::string &path, Coordination::Stat *stat, const Coordination::EventPtr &watch,
                               Coordination::ListRequestType list_request_type, bool with_stat, bool with_data) {
  Strings res;
...
Strings ZooKeeper::getChildrenWatch(const std::string &path, Coordination::Stat *stat,
                                    Coordination::WatchCallbackPtrOrEventPtr watch_callback,
                                    Coordination::ListRequestType list_request_type, bool with_stat, bool with_data) {
  Strings res;
...
Coordination::Error ZooKeeper::tryGetChildren(const std::string &path, Strings &res, Coordination::Stat *stat,
                                              const Coordination::EventPtr &watch, Coordination::ListRequestType list_request_type,
                                              bool with_stat, bool with_data) {
  return tryGetChildrenWatch(path, res, stat, watch, list_request_type, with_stat, with_data);
}

Coordination::Error ZooKeeper::tryGetChildrenWatch(const std::string &path, Strings &res, Coordination::Stat *stat,
                                                   Coordination::WatchCallbackPtrOrEventPtr watch_callback,
                                                   Coordination::ListRequestType list_request_type, bool with_stat, bool with_data) {
  Coordination::Error code = getChildrenImpl(path, res, stat, watch_callback, list_request_type, with_stat, with_data);

  if (!(code == Coordination::Error::ZOK || code == Coordination::Error::ZNONODE)) throw KeeperException::fromPath(code, path);
...
Coordination::Error ZooKeeper::createImpl(const std::string &path, const std::string &data, int32_t mode, std::string &path_created) {
  ...
  maybeSetSpanStatus(maybe_span, code);
  if (code == Coordination::Error::ZOK) path_created = response.path_created;
  return code;
}
...
Coordination::Error ZooKeeper::tryCreate(const std::string &path, const std::string &data, int32_t mode, std::string &path_created) {
  ...
  if (code == Coordination::Error::ZNOTREADONLY && exists(path)) return Coordination::Error::ZNODEEXISTS;

  if (!(code == Coordination::Error::ZOK || code == Coordination::Error::ZNONODE || code == Coordination::Error::ZNODEEXISTS ||
...
void ZooKeeper::createIfNotExists(const std::string &path, const std::string &data) {
  ...
  if (code == Coordination::Error::ZOK || code == Coordination::Error::ZNODEEXISTS) return;
  if (code == Coordination::Error::ZNOTREADONLY && exists(path)) return;
  throw KeeperException::fromPath(code, path);
}
...
void ZooKeeper::createAncestors(const std::string &path) {
  ...
  if (isFeatureEnabled(DB::KeeperFeatureFlag::CREATE_IF_NOT_EXISTS)) {
    ...
    if (!failure_reason.empty()) throw KeeperException::fromMessage(code, failure_reason);
  ...
  if (last_pos == std::string::npos || last_pos == 0) return;
  ...
  while (true) {
    ...
    if (code == Coordination::Error::ZNONODE) {
      ...
      if (last_pos == std::string::npos || last_pos == 0) throw KeeperException::fromPath(code, path);
  ...
  for (const std::string &pending : pending_nodes | std::views::reverse) createIfNotExists(pending, data);
}
...
void ZooKeeper::remove(const std::string &path, int32_t version) { check(tryRemove(path, version), path); }
...
Coordination::Error ZooKeeper::tryRemove(const std::string &path, int32_t version) {
  ...
  if (!(code == Coordination::Error::ZOK || code == Coordination::Error::ZNONODE || code == Coordination::Error::ZBADVERSION ||
...
Coordination::Error ZooKeeper::existsImpl(const std::string &path, Coordination::Stat *stat,
  ...
  maybeSetSpanStatus(maybe_span, code);
  if (code == Coordination::Error::ZOK && stat) *stat = response.stat;
...
bool ZooKeeper::anyExists(const std::vector<std::string> &paths) {
  ...
  for (size_t i = 0; i < exists_multi_response.size(); ++i) {
    if (exists_multi_response[i].error == Coordination::Error::ZOK) return true;
  }
  return false;
}
...
Coordination::Error ZooKeeper::getImpl(const std::string &path, std::string &res, Coordination::Stat *stat,
                                       Coordination::WatchCallbackPtrOrEventPtr watch_callback) {
  std::optional<DB::OpenTelemetry::SpanHolder> maybe_span;
  if (sampleForOpenTelemetryTracing()) {
...
std::string ZooKeeper::get(const std::string &path, Coordination::Stat *stat, const Coordination::EventPtr &watch) {
  ...
  std::string res;
  if (tryGet(path, res, stat, watch, &code)) return res;
  throw KeeperException(code, "Can't get data for node '{}': node doesn't exist", path);
}

std::string ZooKeeper::getWatch(const std::string &path, Coordination::Stat *stat,
                                Coordination::WatchCallbackPtrOrEventPtr watch_callback) {
  Coordination::Error code = Coordination::Error::ZOK;
  std::string res;
  if (tryGetWatch(path, res, stat, watch_callback, &code)) return res;
  throw KeeperException(code, "Can't get data for node '{}': node doesn't exist", path);
}

bool ZooKeeper::tryGet(const std::string &path, std::string &res, Coordination::Stat *stat, const Coordination::EventPtr &watch,
                       Coordination::Error *return_code) {
  return tryGetWatch(path, res, stat, watch, return_code);
}

bool ZooKeeper::tryGetWatch(const std::string &path, std::string &res, Coordination::Stat *stat,
                            Coordination::WatchCallbackPtrOrEventPtr watch_callback, Coordination::Error *return_code) {
...
Coordination::Error ZooKeeper::setImpl(const std::string &path, const std::string &data, int32_t version, Coordination::Stat *stat) {
  std::optional<DB::OpenTelemetry::SpanHolder> maybe_span;
  if (sampleForOpenTelemetryTracing()) {
...
Coordination::Error ZooKeeper::trySet(const std::string &path, const std::string &data, int32_t version, Coordination::Stat *stat) {
...
std::pair<Coordination::Error, std::string> ZooKeeper::multiImpl(const Coordination::Requests &requests, Coordination::Responses &responses,
                                                                 bool check_session_valid) {
  if (requests.empty()) return {Coordination::Error::ZOK, ""};

  size_t total_size = 0;
  for (const auto &request : requests) total_size += request->bytesSize();
  ...
  if (future_result.wait_for(std::chrono::milliseconds(args.operation_timeout_ms)) != std::future_status::ready) {
    ...
    maybeSetSpanStatus(maybe_span, Coordination::Error::ZOPERATIONTIMEOUT);
    impl->finalize(fmt::format("Operation timeout on {} of {} requests. First ({}): {}", Coordination::OpNum::Multi, requests.size(),
                               demangle(typeid(request).name()), request.getPath()));
    return {Coordination::Error::ZOPERATIONTIMEOUT, ""};
  }
  ...
  if (check_session_valid) {
    ...
    chassert(code == Coordination::Error::ZOK || Coordination::isHardwareError(code) ||
             responses.back()->error != Coordination::Error::ZOK);
  }

  maybeSetSpanStatus(maybe_span, code);
  if (maybe_span && code != Coordination::Error::ZOK && !reason.empty()) maybe_span->status_message = reason;
...
Coordination::Error ZooKeeper::syncImpl(const std::string &path, std::string &returned_path) {
  ...
  maybeSetSpanStatus(maybe_span, code);
  if (code == Coordination::Error::ZOK) returned_path = std::move(response.path);
...
Coordination::Error ZooKeeper::trySync(const std::string &path, std::string &returned_path) { return syncImpl(path, returned_path); }
...
bool ZooKeeper::tryRemoveChildrenRecursive(const std::string &path, bool probably_flat, RemoveException keep_child) {
  Strings children;
  if (tryGetChildren(path, children) != Coordination::Error::ZOK) return false;
  ...
  while (!children.empty()) {
    ...
    for (size_t i = 0; i < MULTI_BATCH_SIZE && !children.empty(); ++i) {
      ...
      if (keep_child.path.empty() || keep_child.path != children.back()) [[likely]] {
        ...
        if (!probably_flat) tryRemoveChildrenRecursive(child_path);
    ...
    Coordination::Responses responses;
    if (tryMulti(ops, responses) == Coordination::Error::ZOK) continue;

    removed_as_expected = false;

    std::vector<zkutil::ZooKeeper::FutureRemove> futures;
    futures.reserve(batch.size());
    for (const std::string &child : batch) futures.push_back(asyncTryRemoveNoThrow(child, -1));

    for (size_t i = 0; i < batch.size(); ++i) {
      auto res = futures[i].get();
      if (res.error == Coordination::Error::ZOK) continue;
...
Coordination::Error ZooKeeper::tryRemoveRecursive(const std::string &path, uint32_t remove_nodes_limit) {
  ...
  if (!isFeatureEnabled(DB::KeeperFeatureFlag::REMOVE_RECURSIVE)) return fallback_method();
  ...
  auto callback = [promise](const Coordination::RemoveRecursiveResponse &response) mutable { promise->set_value(response); };
  ...
  if (code == Coordination::Error::ZNOTEMPTY) return fallback_method();
...
bool ZooKeeper::waitForDisappear(const std::string &path, const WaitCondition &condition) {
  ...
  auto watch = createWatchFromRawCallback("ZooKeeper::waitForDisappear::" + path, [&]() -> Coordination::WatchCallback {
    return [state](const Coordination::WatchResponse &response) {
      if (!state->code) {
        state->code = static_cast<int32_t>(response.error);
        if (!state->code) state->event_type = response.type;
        state->event.set();
      }
    };
  });
  ...
  do {
    ...
    if (!state->event.tryWait(1000)) continue;

    if (state->code == static_cast<int32_t>(Coordination::Error::ZNONODE)) return true;

    if (state->code) throw KeeperException::fromPath(static_cast<Coordination::Error>(state->code.load(std::memory_order_seq_cst)), path);

    if (state->event_type == Coordination::DELETED) return true;
...
void ZooKeeper::deleteEphemeralNodeIfContentMatches(const std::string &path, const std::string &fast_delete_if_equal_value) {
  deleteEphemeralNodeIfContentMatches(
      path, [&fast_delete_if_equal_value](const std::string &actual_content) { return actual_content == fast_delete_if_equal_value; });
}
...
void ZooKeeper::deleteEphemeralNodeIfContentMatches(const std::string &path, std::function<bool(const std::string &)> condition) {
  ...
  if (!tryGet(path, content, &stat, eph_node_disappeared)) return;

  if (condition(content)) {
    auto code = tryRemove(path, stat.version);
    if (code != Coordination::Error::ZOK && code != Coordination::Error::ZNONODE) throw Coordination::Exception::fromPath(code, path);
  } else {
...
Coordination::ReconfigResponse ZooKeeper::reconfig(const std::string &joining, const std::string &leaving, const std::string &new_members,
                                                   int32_t version) {
  std::optional<DB::OpenTelemetry::SpanHolder> maybe_span;
  if (sampleForOpenTelemetryTracing()) {
    maybe_span.emplace("zookeeper.reconfig", DB::OpenTelemetry::SpanKind::CLIENT, std::vector<DB::OpenTelemetry::SpanAttribute>{},
                       /*create_trace_if_not_exists=*/true);
    DB::OpenTelemetry::SetTraceFlagInCurrentContext(DB::OpenTelemetry::TRACE_FLAG_KEEPER_SPANS, true);
  }
...
ZooKeeperPtr ZooKeeper::startNewSession() const {
  ...
  auto res =
      std::shared_ptr<ZooKeeper>(new ZooKeeper(args_copy, zk_log, aggregated_zookeeper_log, availability_zones, std::move(optimal_impl)));
  res->initSession();
  return res;
}
...
bool ZooKeeper::expired() { return impl->isExpired(); }

bool ZooKeeper::isFeatureEnabled(DB::KeeperFeatureFlag feature_flag) const { return impl->isFeatureEnabled(feature_flag); }

Int64 ZooKeeper::getClientID() const { return impl->getSessionID(); }

Coordination::WatchCallbackPtrOrEventPtr ZooKeeper::createWatchFromRawCallback(const String &id,
                                                                               const Coordination::IKeeper::WatchCallbackCreator &creator) {
...
std::future<Coordination::CreateResponse> ZooKeeper::asyncTryCreateNoThrow(const std::string &path, const std::string &data, int32_t mode) {
  ...
  auto callback = [promise](const Coordination::CreateResponse &response) mutable { promise->set_value(response); };
...
std::future<Coordination::GetResponse> ZooKeeper::asyncTryGetNoThrow(const std::string &path,
                                                                     Coordination::WatchCallbackPtrOrEventPtr watch_callback) {
  auto promise = std::make_shared<std::promise<Coordination::GetResponse>>();
  auto future = promise->get_future();

  auto callback = [promise](const Coordination::GetResponse &response) mutable { promise->set_value(response); };
...
std::future<Coordination::ExistsResponse> ZooKeeper::asyncTryExistsNoThrow(const std::string &path,
                                                                           Coordination::WatchCallbackPtrOrEventPtr watch_callback) {
  auto promise = std::make_shared<std::promise<Coordination::ExistsResponse>>();
  auto future = promise->get_future();

  auto callback = [promise](const Coordination::ExistsResponse &response) mutable { promise->set_value(response); };
...
std::future<Coordination::SetResponse> ZooKeeper::asyncTrySetNoThrow(const std::string &path, const std::string &data, int32_t version) {
  ...
  auto callback = [promise](const Coordination::SetResponse &response) mutable { promise->set_value(response); };
...
std::future<Coordination::ListResponse> ZooKeeper::asyncGetChildren(const std::string &path,
                                                                    Coordination::ListRequestType list_request_type, bool with_stat,
                                                                    bool with_data) {
...
std::future<Coordination::ListResponse> ZooKeeper::asyncTryGetChildrenNoThrow(const std::string &path,
                                                                              Coordination::WatchCallbackPtrOrEventPtr watch_callback,
...
std::future<Coordination::ListResponse> ZooKeeper::asyncTryGetChildren(const std::string &path,
                                                                       Coordination::ListRequestType list_request_type, bool with_stat,
                                                                       bool with_data) {
  auto promise = std::make_shared<std::promise<Coordination::ListResponse>>();
  auto future = promise->get_future();
...
std::future<Coordination::RemoveResponse> ZooKeeper::asyncTryRemoveNoThrow(const std::string &path, int32_t version) {
  ...
  auto callback = [promise](const Coordination::RemoveResponse &response) mutable { promise->set_value(response); };
...
std::future<Coordination::MultiResponse> ZooKeeper::asyncTryMultiNoThrow(std::span<const Coordination::RequestPtr> ops) {
  ...
  auto callback = [promise](const Coordination::MultiResponse &response) mutable { promise->set_value(response); };
...
std::future<Coordination::SyncResponse> ZooKeeper::asyncTrySyncNoThrow(const std::string &path) {
  ...
  auto callback = [promise](const Coordination::SyncResponse &response) mutable { promise->set_value(response); };
...
ZooKeeper::FutureGetACL ZooKeeper::asyncTryGetACLNoThrow(const std::string &path) {
  ...
  auto callback = [promise](const Coordination::GetACLResponse &response) mutable { promise->set_value(response); };
...
std::future<Coordination::ReconfigResponse> ZooKeeper::asyncReconfig(const std::string &joining, const std::string &leaving,
                                                                     const std::string &new_members, int32_t version) {
...
void ZooKeeper::finalize(const String &reason) { impl->finalize(reason); }

void ZooKeeper::setServerCompletelyStarted() {
  if (auto *zk = dynamic_cast<Coordination::ZooKeeper *>(impl.get())) zk->setServerCompletelyStarted();
}

std::optional<int8_t> ZooKeeper::getConnectedHostIdx() const { return impl->getConnectedNodeIdx(); }

String ZooKeeper::getConnectedHostPort() const { return impl->getConnectedHostPort(); }

int64_t ZooKeeper::getConnectionXid() const { return impl->getConnectionXid(); }

String ZooKeeper::getConnectedHostAvailabilityZone() const {
  if (args.implementation != "zookeeper" || !impl) return "";
  std::optional<int8_t> idx = impl->getConnectedNodeIdx();
  if (!idx) return "";  /// session expired
...
size_t getFailedOpIndex(Coordination::Error exception_code, const Coordination::Responses &responses) {
  if (responses.empty()) throw DB::Exception(DB::ErrorCodes::LOGICAL_ERROR, "Responses for multi transaction is empty");

  for (size_t index = 0, size = responses.size(); index < size; ++index)
    if (responses[index]->error != Coordination::Error::ZOK) return index;

  if (!Coordination::isUserError(exception_code))
    throw DB::Exception(DB::ErrorCodes::LOGICAL_ERROR, "There are no failed OPs because '{}' is not valid response code for that",
                        exception_code);

  throw DB::Exception(DB::ErrorCodes::LOGICAL_ERROR, "There is no failed OpResult");
}

KeeperMultiException::KeeperMultiException(Coordination::Error exception_code, size_t failed_op_index_,
                                           const Coordination::Requests &requests_, const Coordination::Responses &responses_)
    : KeeperException(exception_code, "Transaction failed ({}): Op #{}, path", exception_code, failed_op_index_),
      requests(requests_),
      responses(responses_),
      failed_op_index(failed_op_index_) {
  addMessage(getPathForFirstFailedOp());
}

KeeperMultiException::KeeperMultiException(Coordination::Error exception_code, const Coordination::Requests &requests_,
                                           const Coordination::Responses &responses_)
    : KeeperMultiException(exception_code, getFailedOpIndex(exception_code, responses_), requests_, responses_) {}

std::string KeeperMultiException::getPathForFirstFailedOp() const { return requests[failed_op_index]->getPath(); }

void KeeperMultiException::check(Coordination::Error exception_code, const Coordination::Requests &requests,
                                 const Coordination::Responses &responses) {
  if (exception_code == Coordination::Error::ZOK) return;

  if (Coordination::isUserError(exception_code)) throw KeeperMultiException(exception_code, requests, responses);
...
Coordination::RequestPtr makeRemoveRecursiveRequest(const Client &client, const std::string &path, uint32_t remove_nodes_limit) {
  if (!client.isFeatureEnabled(DB::KeeperFeatureFlag::REMOVE_RECURSIVE))
    throw DB::Exception(DB::ErrorCodes::LOGICAL_ERROR,
                        "Trying to use RemoveRecursive request while Keeper doesn't support it or feature flag is disabled");

  return makeRemoveRecursiveRequest(path, remove_nodes_limit);
}

template Coordination::RequestPtr makeRemoveRecursiveRequest<zkutil::ZooKeeper>(const zkutil::ZooKeeper &client, const std::string &path,
                                                                                uint32_t remove_nodes_limit);
template Coordination::RequestPtr makeRemoveRecursiveRequest<DB::ZooKeeperWithFaultInjection>(const DB::ZooKeeperWithFaultInjection &client,
...
Coordination::RequestPtr makeCheckRequest(const std::string &path, int version, bool not_exists,
                                          std::optional<Coordination::Stat> stat_to_check) {
...
Coordination::RequestPtr makeListRequest(const std::string &path, Coordination::ListRequestType list_request_type, bool with_stat,
                                         bool with_data, Coordination::WatchCallbackPtrOrEventPtr watch) {
...
std::string normalizeZooKeeperPath(std::string zookeeper_path, bool check_starts_with_slash, LoggerPtr log) {
  if (!zookeeper_path.empty() && zookeeper_path.back() == '/') zookeeper_path.resize(zookeeper_path.size() - 1);
  ...
  if (!zookeeper_path.empty() && zookeeper_path.front() != '/') {
    ...
    if (log) LOG_WARNING(log, "ZooKeeper path ('{}') does not start with '/'. It will not be supported in future releases", zookeeper_path);
...
String extractZooKeeperName(const String &path) {
  if (path.empty()) throw DB::Exception(DB::ErrorCodes::BAD_ARGUMENTS, "ZooKeeper path should not be empty");
  if (path[0] == '/') return String(DEFAULT_ZOOKEEPER_NAME);
  auto pos = path.find(":/");
...
String extractZooKeeperPath(const String &path, bool check_starts_with_slash, LoggerPtr log) {
  if (path.empty()) throw DB::Exception(DB::ErrorCodes::BAD_ARGUMENTS, "ZooKeeper path should not be empty");
  if (path[0] == '/') return normalizeZooKeeperPath(path, check_starts_with_slash, log);
  auto pos = path.find(":/");
  if (pos != String::npos && pos < path.find('/')) {
...
bool hasZooKeeperConfig(const Poco::Util::AbstractConfiguration &config) {
  return config.has("zookeeper") || config.has("keeper") ||
         (config.has("keeper_server.raft_configuration") && config.getBool("keeper_server.use_cluster", true));
}

String getZooKeeperConfigName(const Poco::Util::AbstractConfiguration &config) {
  if (config.has("zookeeper")) return "zookeeper";

  if (config.has("keeper")) return "keeper";

  if (config.has("keeper_server.raft_configuration") && config.getBool("keeper_server.use_cluster", true)) return "keeper_server";
...
template void addCheckNotExistsRequest<zkutil::ZooKeeper>(Coordination::Requests &requests, const zkutil::ZooKeeper &client,
                                                          const std::string &path);
template void addCheckNotExistsRequest<DB::ZooKeeperWithFaultInjection>(Coordination::Requests &requests,
                                                                        const DB::ZooKeeperWithFaultInjection &client,
                                                                        const std::string &path);
