...
struct ZooKeeperOpentelemetrySpans {
  ...
  MaybeSpan read_wait_for_write{"keeper.read.wait_for_write", OpenTelemetry::SpanKind::INTERNAL,
                                HistogramMetrics::KeeperReadWaitForWriteTime};
  ...
  static void maybeInitialize(MaybeSpan& maybe_span, const std::optional<OpenTelemetry::TracingContext>& parent_context,
                              UInt64 start_time_us = now());

  template <typename MakeAttributes>
  static void maybeFinalize(MaybeSpan& maybe_span, MakeAttributes&& make_attributes,
                            OpenTelemetry::SpanStatus status = OpenTelemetry::SpanStatus::OK, const String& error_message = {},
  ...
  static void maybeFinalizeImpl(MaybeSpan& maybe_span, std::vector<OpenTelemetry::SpanAttribute> attributes = {},
                                OpenTelemetry::SpanStatus status = OpenTelemetry::SpanStatus::OK, const String& error_message = {},
                                UInt64 finish_time_us = now());
};
