...
WriteBuffer &ZooKeeper::getWriteBuffer() {
  if (compressed_out) return *compressed_out;
  return *out;
}

void ZooKeeper::flushWriteBuffer() {
  if (compressed_out) compressed_out->next();
  out->next();
}

void ZooKeeper::cancelWriteBuffer() noexcept {
  if (compressed_out) compressed_out->cancel();
  if (out) out->cancel();
}

ReadBuffer &ZooKeeper::getReadBuffer() {
  if (compressed_in) return *compressed_in;
  return *in;
}

static void removeRootPath(String &path, const String &chroot) {
  if (chroot.empty()) return;

  if (path.size() <= chroot.size()) throw Exception::fromMessage(Error::ZDATAINCONSISTENCY, "Received path is not longer than chroot");
...
ZooKeeper::ZooKeeper(const zkutil::ShuffleHosts &nodes, const zkutil::ZooKeeperArgs &args_, std::shared_ptr<ZooKeeperLog> zk_log_,
  ...
  if (!args.chroot.empty()) {
    if (args.chroot.back() == '/') args.chroot.pop_back();
  }
  ...
  if (args.enable_fault_injections_during_startup) setupFaultDistributions();
  ...
  if (!args.auth_scheme.empty()) sendAuth(args.auth_scheme, args.identity);
...
void ZooKeeper::connect(const zkutil::ShuffleHosts &nodes, Poco::Timespan connection_timeout) {
  if (nodes.empty()) throw Exception::fromMessage(Error::ZBADARGUMENTS, "No nodes passed to ZooKeeper constructor");
  ...
  for (const auto &node : nodes) {
    try {
      ...
      LOG_TRACE(log, "Adding ZooKeeper host {} ({}), az: {}, priority: {}", node.host, host_socket_addr.toString(), node.az_info,
                node.priority.value);
  ...
  if (resolved_count == 0) {
    ...
    if (dns_error)
      throw zkutil::KeeperException::fromMessage(Coordination::Error::ZCONNECTIONLOSS,
...
#else
          throw Poco::Exception("Communication with ZooKeeper over SSL is disabled because poco library was built without NetSSL support.");
#endif
        } else {
...
void ZooKeeper::receiveHandshake() {
  ...
  read(session_id);
  read(passwd);
  if (handshake_length == SERVER_HANDSHAKE_LENGTH_WITH_READONLY) read(read_only);
}
...
void ZooKeeper::sendAuth(const String &scheme, const String &data) {
  ...
  if (read_xid != AUTH_XID) throw Exception(Error::ZMARSHALLINGERROR, "Unexpected event received in reply to auth request: {}", read_xid);
  ...
  if (err != Error::ZOK)
    throw Exception(Error::ZMARSHALLINGERROR, "Error received in reply to auth request. Code: {}. Message: {}", static_cast<int32_t>(err),
                    err);
}
  ...
  try {
    while (!requests_queue.isFinished()) {
      ...
      if (next_heartbeat_time > now) {
        ...
        UInt64 max_wait =
            std::min(static_cast<UInt64>(std::chrono::duration_cast<std::chrono::milliseconds>(next_heartbeat_time - now).count()),
        ...
        if (requests_queue.tryPop(info, max_wait)) {
          ...
          ZooKeeperOpentelemetrySpans::maybeFinalize(info.request->spans.client_requests_queue, [&] {
            return std::vector<OpenTelemetry::SpanAttribute>{
                {"zookeeper_client.requests_queue.size", requests_queue.size()},
            };
          });

          if (info.watch) info.request->has_watch = true;

          if (info.request->add_root_path) info.request->addRootPath(args.chroot);
          ...
          if (info.request->xid == close_xid) break;
        }
      } else {
        /// Send heartbeat.
  ...
  try {
    ...
    while (!requests_queue.isFinished()) {
...

      {
        ...
        if (!operations.empty()) {
          ...
          auto earliest_operation_deadline =
              earliest_operation->request->create_ts + std::chrono::microseconds(static_cast<Int64>(args.operation_timeout_ms) * 1000);
      ...
      if (in->poll(max_wait_us)) {
        if (finalization_started.test()) break;
...
void ZooKeeper::receiveEvent() {
  ...
  if (xid == PING_XID) {
    if (err != Error::ZOK) throw Exception(Error::ZRUNTIMEINCONSISTENCY, "Received error in heartbeat response: {}", err);
    ...
    request_info.callback = [this](const Response &response_) {
      ...
      if (it == watches.end()) {
        ...
        for (const auto &event_or_callback : it->second) {
          if (event_or_callback) event_or_callback(watch_response);
        }
  ...
  } else {
    {
      ...
      if (it == operations.end()) throw Exception(Error::ZRUNTIMEINCONSISTENCY, "Received response for unknown xid {}", xid);
  ...
  try {
    if (!response) response = request_info.request->makeResponse();
    ...
    if (err != Error::ZOK) {
      ...
      if (!request_info.request || request_info.request->add_root_path) response->removeRootPath(args.chroot);
    }

    /// Just helper for watch callbacks update. The main logic is below.
    const auto update_watch_callbacks = [this](const Coordination::ZooKeeperRequestPtr &req, const Coordination::ResponsePtr &resp,
    ...
    try {
      if (request_info.callback) request_info.callback(*response);
...
void ZooKeeper::finalize(bool error_send, bool error_receive, const String &reason) {
  ...
  LOG_INFO(log, "Finalizing session {}. finalization_started: {}, queue_finished: {}, reason: '{}' {}", session_id, already_started,
           requests_queue.isFinished(), reason, StackTrace().toString());
  ...
  try {
    ...
    if (!error_receive) receive_thread.join();

    {
      ...
      for (auto &op : operations) {
        ...
        response->error = request_info.request->probably_sent ? Error::ZCONNECTIONLOSS : Error::ZSESSIONEXPIRED;
        response->xid = request_info.request->xid;

        chassert(request_info.request->create_ts != std::chrono::steady_clock::time_point{});
        UInt64 elapsed_microseconds =
            std::chrono::duration_cast<std::chrono::microseconds>(clock::now() - request_info.request->create_ts).count();
      ...
      if (info.callback) {
        ...
        if (response) {
          ...
          try {
            ...
            UInt64 elapsed_microseconds =
                std::chrono::duration_cast<std::chrono::microseconds>(clock::now() - info.request->create_ts).count();
...
void ZooKeeper::pushRequest(RequestInfo &&info) {
  try {
    ...
    if (!info.request->xid) {
      ...
      if (!use_xid_64) info.request->xid = static_cast<int32_t>(info.request->xid);

      if (info.request->xid == close_xid) throw Exception::fromMessage(Error::ZSESSIONEXPIRED, "xid equal to close_xid");
      if (info.request->xid < 0) throw Exception::fromMessage(Error::ZSESSIONEXPIRED, "XID overflow");

      if (auto *multi_request = dynamic_cast<ZooKeeperMultiRequest *>(info.request.get())) {
        for (auto &request : multi_request->requests) dynamic_cast<ZooKeeperRequest &>(*request).xid = multi_request->xid;
      }
    }

    maybeInjectSendFault();

    if (const auto &current_trace_context = OpenTelemetry::CurrentContext();
    ...
    if (!requests_queue.tryPush(std::move(info), args.operation_timeout_ms)) {
      if (requests_queue.isFinished()) throw Exception::fromMessage(Error::ZSESSIONEXPIRED, "Session expired");

      throw Exception(Error::ZOPERATIONTIMEOUT, "Cannot push request to queue within operation timeout of {} ms",
...
bool ZooKeeper::isFeatureEnabled(KeeperFeatureFlag feature_flag) const { return keeper_feature_flags.isEnabled(feature_flag); }
...
void ZooKeeper::executeGenericRequest(const ZooKeeperRequestPtr &request, ResponseCallback callback, WatchCallbackPtrOrEventPtr watch) {
  RequestInfo request_info;
...
void ZooKeeper::create(const String &path, const String &data, bool is_ephemeral, bool is_sequential, const ACLs &acls,
  ...
  if (!path_acls.empty()) {
    ...
    if (auto path_acls_it = path_acls.find(path); path_acls_it != path_acls.end()) final_acls.push_back(path_acls_it->second.acl);
...
void ZooKeeper::remove(const String &path, int32_t version, RemoveCallback callback) {
...
void ZooKeeper::removeRecursive(const String &path, uint32_t remove_nodes_limit, RemoveRecursiveCallback callback) {
  if (!isFeatureEnabled(KeeperFeatureFlag::REMOVE_RECURSIVE))
    throw Exception::fromMessage(Error::ZBADARGUMENTS,
                                 "RemoveRecursive request type cannot be used because it's not supported by the server");
...
void ZooKeeper::exists(const String &path, ExistsCallback callback, WatchCallbackPtrOrEventPtr watch) {
...
void ZooKeeper::get(const String &path, GetCallback callback, WatchCallbackPtrOrEventPtr watch) {
  ZooKeeperGetRequest request;
  request.path = path;
...
void ZooKeeper::set(const String &path, const String &data, int32_t version, SetCallback callback) {
...
void ZooKeeper::list(const String &path, ListRequestType list_request_type, ListCallback callback, WatchCallbackPtrOrEventPtr watch,
  ...
  if (with_stat || with_data) {
    ...
    if (list_request_type != ListRequestType::ALL)
      throw Exception::fromMessage(Error::ZBADARGUMENTS,
                                   "Filtered list request type cannot be used because it's not supported by the server");
  ...
  if (watch) request_info.watch = std::move(watch);
  request_info.request = std::move(request);
  pushRequest(std::move(request_info));
  ProfileEvents::increment(ProfileEvents::ZooKeeperList);
}

void ZooKeeper::check(const String &path, int32_t version, CheckCallback callback) {
...
void ZooKeeper::sync(const String &path, SyncCallback callback) {
...
void ZooKeeper::reconfig(std::string_view joining, std::string_view leaving, std::string_view new_members, int32_t version,
...
void ZooKeeper::multi(const Requests &requests, MultiCallback callback) { multi(std::span(requests), std::move(callback)); }
...
void ZooKeeper::multi(std::span<const RequestPtr> requests, MultiCallback callback) {
    ...
    for (const auto &generic_request : requests) {
      if (auto *create_request = dynamic_cast<CreateRequest *>(generic_request.get())) {
        const auto add_acl = [&](const auto &acl) {
          ...
          if (create_request->acls.empty()) create_request->acls = default_acls;

          // Append the path-specific ACL
          create_request->acls.push_back(acl);
        };

        if (auto path_acls_it = path_acls.find(create_request->path); path_acls_it != path_acls.end()) add_acl(path_acls_it->second.acl);
...
void ZooKeeper::close() {
  ...
  if (!requests_queue.tryPush(std::move(request_info), args.operation_timeout_ms))
    throw Exception(Error::ZOPERATIONTIMEOUT, "Cannot push close request to queue within operation timeout of {} ms",
                    args.operation_timeout_ms);
...
String ZooKeeper::getConnectedHostPort() const {
  ...
  if (idx) return args.hosts[*idx];
  return "";
}

int64_t ZooKeeper::getConnectionXid() const { return next_xid.load(); }
...
void ZooKeeper::logOperationIfNeeded(const ZooKeeperRequestPtr &request, const ZooKeeperResponsePtr &response, bool finalize,
  ...
  auto maybe_zk_log = getZooKeeperLog();
  if (!maybe_zk_log) return;

  if (elapsed_microseconds < maybe_zk_log->getDurationMicrosecondsThreshold()) return;

  ZooKeeperLogElement::Type log_type = ZooKeeperLogElement::UNKNOWN;
  Decimal64 event_time = std::chrono::duration_cast<std::chrono::microseconds>(std::chrono::system_clock::now().time_since_epoch()).count();
  LogElements elems;
  if (request) {
  ...
  if (finalize) log_type = ZooKeeperLogElement::FINALIZE;
...
void ZooKeeper::observeOperation(const ZooKeeperRequest *request, const Response *response, UInt64 elapsed_microseconds,
                                 StaticString component) {
  chassert(response);

  auto current_aggregated_zookeeper_log = getAggregatedZooKeeperLog();
  if (!current_aggregated_zookeeper_log) return;

  if (!request) {
    if (const auto *watch_response = dynamic_cast<const ZooKeeperWatchResponse *>(response)) {
      current_aggregated_zookeeper_log->observe(session_id, watch_response->tryGetOpNum(), watch_response->path, elapsed_microseconds,
                                                watch_response->error, component);
    } else {
  ...
  current_aggregated_zookeeper_log->observe(session_id, request->tryGetOpNum(), request->getPath(), elapsed_microseconds, response->error,
                                            component);

  const auto *multi_response = dynamic_cast<const ZooKeeperMultiResponse *>(response);

  if (!multi_response) return;

  const auto &multi_request = static_cast<const ZooKeeperMultiRequest &>(*request);
  chassert((request->getOpNum() == OpNum::Multi || request->getOpNum() == OpNum::MultiRead) &&
