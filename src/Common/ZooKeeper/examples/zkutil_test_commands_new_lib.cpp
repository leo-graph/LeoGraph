...
int main(int argc, char **argv) try {
  ...
  for (size_t i = 0; i < args.hosts.size(); ++i) {
    ...
    if (node.secure) host_string.erase(0, strlen("secure://"));
  ...
  zk.create("/test", "old", false, false, {}, [&](const CreateResponse &response) {
  ...
  zk.get(
      ...
      std::make_shared<Coordination::WatchCallback>([](const WatchResponse &response) {
        if (response.error != Coordination::Error::ZOK)
  ...
  zk.set("/test", "new", -1, [&](const SetResponse &response) {
    if (response.error != Coordination::Error::ZOK)
