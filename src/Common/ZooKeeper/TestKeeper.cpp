...
void TestKeeperRequest::processWatchesImpl(const String &path, TestKeeper::Watches &watches, TestKeeper::Watches &list_watches) {
...

  {
    ...
    if (it != list_watches.end()) {
      for (const auto &event_or_callback : it->second) {
        if (event_or_callback) event_or_callback(watch_list_response);
      }
  ...
  void processWatches(TestKeeper::Watches &node_watches, TestKeeper::Watches &list_watches) const override {
    ...
    auto add_deleted_watches = [&](TestKeeper::Watches &w) {
      for (const auto &[watch_path, _] : w)
        if (watch_path.starts_with(path)) deleted.emplace_back(watch_path, std::count(watch_path.begin(), watch_path.end(), '/'));
    };

    add_deleted_watches(node_watches);
    add_deleted_watches(list_watches);
    std::sort(deleted.begin(), deleted.end(), [](const auto &lhs, const auto &rhs) { return lhs.second < rhs.second; });

    for (const auto &[watch_path, _] : deleted) processWatchesImpl(watch_path, node_watches, list_watches);
  }
};
...
struct TestKeeperMultiRequest final : MultiRequest<RequestPtr>, TestKeeperRequest {
  ...
  void validateOrSpecifyRequestType(bool is_read) {
    if (!is_multi_read) is_multi_read = is_read;

    chassert(is_multi_read.value() == is_read);
  }

  explicit TestKeeperMultiRequest(const Requests &generic_requests) : TestKeeperMultiRequest(std::span(generic_requests)) {}
  ...
  explicit TestKeeperMultiRequest(std::span<const RequestPtr> generic_requests) {
    ...
    for (const auto &generic_request : generic_requests) {
      ...
      } else if (const auto *concrete_request_list_with_stat_and_data =
                     dynamic_cast<const ZooKeeperFilteredListWithStatsAndDataRequest *>(generic_request.get())) {
        validateOrSpecifyRequestType(/*is_read=*/true);
...
std::pair<ResponsePtr, Undo> TestKeeperCreateRequest::process(TestKeeper::Container &container, int64_t zxid) const {
  ...
  if (container.contains(path)) {
    ...
    if (it == container.end()) {
      ...
      if (is_sequential) path_created += fmt::format("{:0>10}", it->second.seq_num);
...
std::pair<ResponsePtr, Undo> TestKeeperRemoveRequest::process(TestKeeper::Container &container, int64_t zxid) const {
  ...
  if (it == container.end()) {
    if (!try_remove) response.error = Error::ZNONODE;
  } else if (version != -1 && version != it->second.stat.version) {
    if (!try_remove) response.error = Error::ZBADVERSION;
  } else if (it->second.stat.numChildren) {
    if (!try_remove) response.error = Error::ZNOTEMPTY;
  } else {
...
std::pair<ResponsePtr, Undo> TestKeeperRemoveRecursiveRequest::process(TestKeeper::Container &container, int64_t zxid) const {
  ...
  if (root_it == container.end()) return {std::make_shared<RemoveRecursiveResponse>(response), undo};
  ...
  undo = [&container, dead = std::move(removed_nodes), root_path = path]() {
    for (auto &&[child_path, child_node] : dead) container.emplace(child_path, child_node);
...
std::pair<ResponsePtr, Undo> TestKeeperListRequest::process(TestKeeper::Container &container, int64_t zxid) const {
  ...
  if (it == container.end()) {
    ...
    if (path_prefix.empty()) throw Exception::fromMessage(Error::ZSESSIONEXPIRED, "Logical error: path cannot be empty");

    if (path_prefix.back() != '/') path_prefix += '/';

    /// Fairly inefficient.
    for (auto child_it = container.upper_bound(path_prefix); child_it != container.end() && startsWith(child_it->first, path_prefix);
         ++child_it) {
      ...
      if (parentPath(child_it->first) == path) {
        ...
        if (with_data) response.data.emplace_back(child_it->second.data);

        if (with_stat) response.stats.emplace_back(child_it->second.stat);
      }
    }
...
std::pair<ResponsePtr, Undo> TestKeeperMultiRequest::processMultiRead(TestKeeper::Container &container, int64_t zxid) const {
  ...
  for (const auto &request : requests) {
    ...
    if (response.error == Error::ZOK && cur_response->error != Error::ZOK) response.error = cur_response->error;
  }
...
TestKeeper::TestKeeper(const zkutil::ZooKeeperArgs &args_) : args(args_) {
    ...
    while (!expired) {
      ...
      if (requests_queue.tryPop(info, max_wait)) {
        ...
        if (info.watch) {
          ...
          if (response->error == Error::ZOK) {
            auto &watches_type = dynamic_cast<const ListRequest *>(info.request.get()) ? list_watches : watches;
        ...
        if (response->error == Error::ZOK) info.request->processWatches(watches, list_watches);

        response->removeRootPath(args.chroot);
        if (info.callback) info.callback(*response);
      }
    }
  } catch (...) {
    tryLogCurrentException(__PRETTY_FUNCTION__);
...
void TestKeeper::pushRequest(RequestInfo &&request) {
  try {
    ...
    if (expired) throw Exception::fromMessage(Error::ZSESSIONEXPIRED, "Session expired");
...
void TestKeeper::create(const String &path, const String &data, bool is_ephemeral, bool is_sequential, const ACLs &,
...
void TestKeeper::remove(const String &path, int32_t version, RemoveCallback callback) {
...
void TestKeeper::removeRecursive(const String &path, uint32_t remove_nodes_limit, RemoveRecursiveCallback callback) {
...
void TestKeeper::exists(const String &path, ExistsCallback callback, WatchCallbackPtrOrEventPtr watch) {
...
void TestKeeper::get(const String &path, GetCallback callback, WatchCallbackPtrOrEventPtr watch) {
...
void TestKeeper::set(const String &path, const String &data, int32_t version, SetCallback callback) {
...
void TestKeeper::list(const String &path, ListRequestType list_request_type, ListCallback callback, WatchCallbackPtrOrEventPtr watch,
...
void TestKeeper::check(const String &path, int32_t version, CheckCallback callback) {
...
void TestKeeper::sync(const String &path, SyncCallback callback) {
...
void TestKeeper::reconfig(std::string_view joining, std::string_view leaving, std::string_view new_members, int32_t version,
  ...
  pushRequest({.request = std::make_shared<TestKeeperReconfigRequest>(std::move(req)),
               .callback = [callback](const Response &response) { callback(dynamic_cast<const ReconfigResponse &>(response)); },
               .watch = {},
               .time = {}});
}

void TestKeeper::multi(const Requests &requests, MultiCallback callback) { multi(std::span(requests), std::move(callback)); }

void TestKeeper::multi(std::span<const RequestPtr> requests, MultiCallback callback) {
