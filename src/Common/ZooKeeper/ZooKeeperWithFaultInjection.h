...
class ZooKeeperWithFaultInjection {
 ...
 public:
  ...
  ZooKeeperWithFaultInjection(zkutil::ZooKeeper::Ptr const& keeper_, double fault_injection_probability, UInt64 fault_injection_seed,
                              std::string name_, LoggerPtr logger_);

  explicit ZooKeeperWithFaultInjection(zkutil::ZooKeeper::Ptr const& keeper_) : keeper(keeper_) {}
  static ZooKeeperWithFaultInjection::Ptr createInstance(double fault_injection_probability, UInt64 fault_injection_seed,
                                                         zkutil::ZooKeeper::Ptr const& zookeeper, std::string name, LoggerPtr logger) {
    ...
    if (fault_injection_seed == 0) fault_injection_seed = randomSeed();

    if (fault_injection_probability > 0.0)
      return std::make_shared<ZooKeeperWithFaultInjection>(zookeeper, fault_injection_probability, fault_injection_seed, std::move(name),
                                                           logger);
  ...
  void forceFailureBeforeOperation() {
    if (!fault_policy) fault_policy = std::make_unique<RandomFaultInjection>(0, 0);
  ...
  bool tryRemoveChildrenRecursive(const std::string& path, bool probably_flat = false,
                                  zkutil::RemoveException keep_child = zkutil::RemoveException{});
