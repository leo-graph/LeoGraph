...
ZooKeeperLock::ZooKeeperLock(const ZooKeeperPtr& zookeeper_, const std::string& lock_prefix_, const std::string& lock_name_,
                             const std::string& lock_message_, bool throw_if_lost_)
...
bool ZooKeeperLock::isLocked(bool check_node) const {
  if (!was_locked || zookeeper->expired()) return false;

  if (!check_node) return true;
...
const std::string& ZooKeeperLock::getLockPath() const { return lock_path; }
...
void ZooKeeperLock::unlock() {
  ...
  if (result && stat.ephemeralOwner == zookeeper->getClientID()) {
    ...
    throw DB::Exception(DB::ErrorCodes::LOGICAL_ERROR, "Lock is lost, node does not exist. Path: {}, message: {}, our id: {}", lock_path,
                        lock_message, zookeeper->getClientID());
  else
...
std::unique_ptr<ZooKeeperLock> createSimpleZooKeeperLock(const ZooKeeperPtr& zookeeper, const String& lock_prefix, const String& lock_name,
                                                         const String& lock_message, bool throw_if_lost) {
