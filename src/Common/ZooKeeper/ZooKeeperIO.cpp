...
void write(OpNum x, WriteBuffer& out) { write(static_cast<int32_t>(x), out); }
...
void write(const Error& x, WriteBuffer& out) { write(static_cast<int32_t>(x), out); }

size_t size(OpNum x) { return size(static_cast<int32_t>(x)); }

size_t size(const std::string& s) { return size(static_cast<int32_t>(s.size())) + s.size(); }

size_t size(const ACL& acl) { return size(acl.permissions) + size(acl.scheme) + size(acl.id); }
...
size_t size(const Error& x) { return size(static_cast<int32_t>(x)); }
...
void read(std::string& s, ReadBuffer& in) {
  ...
  if (size < 0) throw Exception::fromMessage(Error::ZMARSHALLINGERROR, "Negative size while reading string from ZooKeeper");
  ...
  if (read_bytes != static_cast<size_t>(size))
    throw Exception(Error::ZMARSHALLINGERROR, "Buffer size read from Zookeeper is not big enough. Expected {}. Got {}", size, read_bytes);
}
