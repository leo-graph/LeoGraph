...
void write(const std::vector<T>& arr, WriteBuffer& out) {
  ...
  for (const auto& elem : arr) write(elem, out);
}
...
size_t size(const std::vector<T>& arr) {
  ...
  for (const auto& elem : arr) total_size += size(elem);
...
void read(std::array<char, N>& s, ReadBuffer& in) {
  int32_t size = 0;
  read(size, in);
  if (size != N) throw Exception::fromMessage(Error::ZMARSHALLINGERROR, "Unexpected array size while reading from ZooKeeper");
...
void read(std::vector<T>& arr, ReadBuffer& in) {
  int32_t size = 0;
  read(size, in);
  if (size < 0) throw Exception::fromMessage(Error::ZMARSHALLINGERROR, "Negative size while reading array from ZooKeeper");
  ...
  arr.resize(size);
  for (auto& elem : arr) read(elem, in);
}
