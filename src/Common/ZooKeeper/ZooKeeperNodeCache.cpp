...
ZooKeeperNodeCache::ZooKeeperNodeCache(GetZooKeeper get_zookeeper_)
    : get_zookeeper(std::move(get_zookeeper_)), context(std::make_shared<Context>()) {}
...
ZooKeeperNodeCache::ZNode ZooKeeperNodeCache::get(const std::string& path, Coordination::EventPtr caller_watch_event) {
  ...
  if (!zookeeper) throw DB::Exception(DB::ErrorCodes::NO_ZOOKEEPER, "Could not get znode: '{}'. ZooKeeper not configured.", path);

  for (const auto& invalidated_path : invalidated_paths) path_to_cached_znode.erase(invalidated_path);

  auto cache_it = path_to_cached_znode.find(path);
  if (cache_it != path_to_cached_znode.end()) return cache_it->second;
  ...
  auto wrapped_watch_callback = zookeeper->createWatchFromRawCallback("ZooKeeperNodeCache::" + path, [&]() -> Coordination::WatchCallback {
    ...
    return [weak_context, caller_watch_event](const Coordination::WatchResponse& response) {
      if (!(response.type != Coordination::SESSION || response.state == Coordination::EXPIRED_SESSION)) return;

      auto owned_context = weak_context.lock();
      if (!owned_context) return;
      ...
      if (changed && caller_watch_event) caller_watch_event->set();
    };
  });
...
void ZooKeeperNodeCache::sync() { get_zookeeper()->sync("/"); }
