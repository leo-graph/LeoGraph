...
struct RemoveException {
  explicit RemoveException(std::string_view path_ = "", bool remove_subtree_ = true) : path(path_), remove_subtree(remove_subtree_) {}
...
struct MultiReadResponses {
        ...
        [&]<typename TResponses>(TResponses &resp) -> ResponseType & {
          if constexpr (std::same_as<TResponses, RegularResponses>) {
            ...
            if constexpr (try_multi) {
              ...
              if (error != Coordination::Error::ZOK && error != Coordination::Error::ZNONODE) throw KeeperException(error);
            }
            return resp[index];
          } else {
            throw DB::Exception(DB::ErrorCodes::LOGICAL_ERROR, "No responses set for MultiRead");
  ...
  void waitForResponses() {
    if (auto *responses_with_futures = std::get_if<ResponsesWithFutures>(&responses)) responses_with_futures->waitForResponses();
  }
 ...
 private:
  ...
  struct ResponsesWithFutures {
    ResponsesWithFutures(FutureResponses future_responses_)
        : future_responses(std::move(future_responses_))  /// NOLINT(google-explicit-constructor)
    {
    ...
    ResponseType &operator[](size_t index) {
      if (cached_responses[index].has_value()) return *cached_responses[index];
    ...
    void waitForResponses() {
      for (size_t i = 0; i < size(); ++i) {
        if (!cached_responses[i].has_value()) cached_responses[i] = future_responses[i].get();
      }
    }
...
class ZooKeeper {
  ...
  ZooKeeper(const ZooKeeperArgs &args_, std::shared_ptr<DB::ZooKeeperLog> zk_log_,
 ...
 public:
  ...
  static Ptr create(ZooKeeperArgs args_, std::shared_ptr<DB::ZooKeeperLog> zk_log_,
  ...
  Coordination::WatchCallbackPtrOrEventPtr createWatchFromRawCallback(const String &id,
  ...
  MultiExistsResponse exists(const std::vector<std::string> &paths) { return exists(paths.begin(), paths.end()); }
  ...
  MultiGetResponse get(const std::vector<std::string> &paths) { return get(paths.begin(), paths.end()); }

  /// Doesn't not throw in the following cases:
  /// * The node doesn't exist. Returns false in this case.
  bool tryGet(const std::string &path, std::string &res, Coordination::Stat *stat = nullptr, const Coordination::EventPtr &watch = nullptr,
              Coordination::Error *code = nullptr);

  bool tryGetWatch(const std::string &path, std::string &res, Coordination::Stat *stat,
                   Coordination::WatchCallbackPtrOrEventPtr watch_callback, Coordination::Error *code = nullptr);
  ...
  MultiTryGetResponse tryGet(const std::vector<std::string> &paths) { return tryGet(paths.begin(), paths.end()); }
  ...
  Strings getChildren(const std::string &path, Coordination::Stat *stat = nullptr, const Coordination::EventPtr &watch = nullptr,
                      Coordination::ListRequestType list_request_type = Coordination::ListRequestType::ALL, bool with_stat = false,
                      bool with_data = false);

  Strings getChildrenWatch(const std::string &path, Coordination::Stat *stat, Coordination::WatchCallbackPtrOrEventPtr watch_callback,
                           Coordination::ListRequestType list_request_type = Coordination::ListRequestType::ALL, bool with_stat = false,
                           bool with_data = false);
  ...
  template <typename TIter>
  MultiGetChildrenResponse getChildren(TIter start, TIter end,
                                       Coordination::ListRequestType list_request_type = Coordination::ListRequestType::ALL,
                                       bool with_stat = false, bool with_data = false) {
    return multiRead<Coordination::ListResponse, false>(
        start, end,
        [list_request_type, with_stat, with_data](const auto &path) {
          return zkutil::makeListRequest(path, list_request_type, with_stat, with_data);
        },
        [&](const auto &path) { return asyncGetChildren(path, list_request_type, with_stat, with_data); });
  }

  MultiGetChildrenResponse getChildren(const std::vector<std::string> &paths,
                                       Coordination::ListRequestType list_request_type = Coordination::ListRequestType::ALL,
                                       bool with_stat = false, bool with_data = false) {
  ...
  /// Doesn't not throw in the following cases:
  /// * The node doesn't exist.
  Coordination::Error tryGetChildren(const std::string &path, Strings &res, Coordination::Stat *stat = nullptr,
                                     const Coordination::EventPtr &watch = nullptr,
                                     Coordination::ListRequestType list_request_type = Coordination::ListRequestType::ALL,
                                     bool with_stat = false, bool with_data = false);

  Coordination::Error tryGetChildrenWatch(const std::string &path, Strings &res, Coordination::Stat *stat,
                                          Coordination::WatchCallbackPtrOrEventPtr watch_callback,
                                          Coordination::ListRequestType list_request_type = Coordination::ListRequestType::ALL,
                                          bool with_stat = false, bool with_data = false);

  template <typename TIter>
  MultiTryGetChildrenResponse tryGetChildren(TIter start, TIter end,
                                             Coordination::ListRequestType list_request_type = Coordination::ListRequestType::ALL,
                                             bool with_stat = false, bool with_data = false) {
  ...
  MultiTryGetChildrenResponse tryGetChildren(const std::vector<std::string> &paths,
                                             Coordination::ListRequestType list_request_type = Coordination::ListRequestType::ALL,
                                             bool with_stat = false, bool with_data = false) {
  ...
  /// Doesn't not throw in the following cases:
  /// * The node doesn't exist. Returns false in this case.
  bool tryGetACL(const std::string &path, Coordination::ACLs &res, Coordination::Stat *stat = nullptr, Coordination::Error *code = nullptr);
  ...
  Coordination::Error tryMulti(const Coordination::Requests &requests, Coordination::Responses &responses,
                               bool check_session_valid = false);
  ...
  Coordination::ReconfigResponse reconfig(const std::string &joining, const std::string &leaving, const std::string &new_members,
                                          int32_t version = -1);
  ...
  FutureGetChildren asyncGetChildren(const std::string &path,
                                     ...
                                     bool with_stat = false, bool with_data = false);
  /// Like the previous one but don't throw any exceptions on future.get()
  FutureGetChildren asyncTryGetChildrenNoThrow(const std::string &path, Coordination::WatchCallbackPtrOrEventPtr watch_callback = {},
                                               Coordination::ListRequestType list_request_type = Coordination::ListRequestType::ALL,
                                               bool with_stat = false, bool with_data = false);
  ...
  /// Doesn't throw in the following cases:
  /// * The node doesn't exist
  FutureGetChildren asyncTryGetChildren(const std::string &path,
                                        Coordination::ListRequestType list_request_type = Coordination::ListRequestType::ALL,
                                        bool with_stat = false, bool with_data = false);

  using FutureReconfig = std::future<Coordination::ReconfigResponse>;
  FutureReconfig asyncReconfig(const std::string &joining, const std::string &leaving, const std::string &new_members,
                               int32_t version = -1);
 ...
 private:
  ...
  Coordination::Error getImpl(const std::string &path, std::string &res, Coordination::Stat *stat,
                              Coordination::WatchCallbackPtrOrEventPtr watch_callback);
  Coordination::Error setImpl(const std::string &path, const std::string &data, int32_t version, Coordination::Stat *stat);
  Coordination::Error getChildrenImpl(const std::string &path, Strings &res, Coordination::Stat *stat,
                                      Coordination::WatchCallbackPtrOrEventPtr watch_callback,
  ...
  std::pair<Coordination::Error, std::string> multiImpl(const Coordination::Requests &requests, Coordination::Responses &responses,
  ...
  bool sampleForOpenTelemetryTracing() const {
    ...
    return static_cast<int32_t>((impl->getSessionID() ^ getConnectionXid()) & 1023) <
           static_cast<int32_t>(opentelemetry_start_keeper_trace_probability * 1024.0f);
  }
  ...
  MultiReadResponses<TResponse, try_multi> multiRead(TIter start, TIter end, RequestFactory request_factory,
                                                     AsyncFunction<TResponse> async_fun) {
    if (isFeatureEnabled(DB::KeeperFeatureFlag::MULTI_READ)) {
      Coordination::Requests requests;
      for (auto it = start; it != end; ++it) requests.push_back(request_factory(*it));
    ...
    if (responses_size == 0) return MultiReadResponses<TResponse, try_multi>(std::move(future_responses));

    future_responses.reserve(responses_size);

    for (auto it = start; it != end; ++it) future_responses.push_back(async_fun(*it));
...
class EphemeralNodeHolder {
 public:
  ...
  EphemeralNodeHolder(const std::string &path_, ZooKeeper &zookeeper_, bool create, bool try_create, bool sequential,
    ...
    if (create) {
      ...
      need_remove = created =
          Coordination::Error::ZOK == zookeeper.tryCreate(path, data, sequential ? CreateMode::EphemeralSequential : CreateMode::Ephemeral);
    }
  }
  ...
  void setAlreadyRemoved() { need_remove = false; }
