...
ZooKeeperArgs::ZooKeeperArgs(const Poco::Util::AbstractConfiguration& config, const String& config_name) {
  ...
  if (!chroot.empty()) {
    if (chroot.front() != '/')
      throw KeeperException(Coordination::Error::ZBADARGUMENTS, "Root path in config file should start with '/', but got {}", chroot);
    if (chroot.back() == '/') chroot.pop_back();
  }
...
void ZooKeeperArgs::initFromKeeperServerSection(const Poco::Util::AbstractConfiguration& config) {
  ...
  if (auto key = std::string{config_name} + ".tcp_port_secure"; config.has(key)) {
  ...
  bool secure{false};
  std::string tcp_port;
  if (auto tcp_port_secure_key = std::string{config_name} + ".tcp_port_secure"; config.has(tcp_port_secure_key)) {
    secure = true;
    tcp_port = config.getString(tcp_port_secure_key);
  } else if (auto tcp_port_key = std::string{config_name} + ".tcp_port"; config.has(tcp_port_key)) {
  ...
  if (auto coordination_key = std::string{config_name} + ".coordination_settings"; config.has(coordination_key)) {
    if (auto operation_timeout_key = coordination_key + ".operation_timeout_ms"; config.has(operation_timeout_key))
      operation_timeout_ms = config.getInt(operation_timeout_key);

    if (auto session_timeout_key = coordination_key + ".session_timeout_ms"; config.has(session_timeout_key))
      session_timeout_ms = config.getInt(session_timeout_key);

    if (auto use_xid_64_key = coordination_key + ".use_xid_64"; config.has(use_xid_64_key)) use_xid_64 = config.getBool(use_xid_64_key);
  }
  ...
  for (const auto& key : keys) {
    if (startsWith(key, "server")) {
      hosts.push_back((secure ? "secure://" : "") + config.getString(raft_configuration_key + "." + key + ".hostname") + ":" + tcp_port);
      availability_zones.push_back(config.getString(raft_configuration_key + "." + key + ".availability_zone", ""));
    }
  }

  static constexpr std::array load_balancing_keys{".zookeeper_load_balancing", ".keeper_load_balancing"};

  for (const auto* load_balancing_key : load_balancing_keys) {
    if (auto load_balancing_config = std::string{config_name} + load_balancing_key; config.has(load_balancing_config)) {
      ...
      if (!load_balancing) throw DB::Exception(DB::ErrorCodes::BAD_ARGUMENTS, "Unknown load balancing: {}", load_balancing_str);
  ...
  if (prefer_local_availability_zone) client_availability_zone = DB::PlacementInfo::PlacementInfo::instance().getAvailabilityZone();
}
...
void ZooKeeperArgs::initFromKeeperSection(const Poco::Util::AbstractConfiguration& config, const std::string& config_name) {
  ...
  for (const auto& key : keys) {
    if (key.starts_with("node")) {
      hosts.push_back((config.getBool(config_name + "." + key + ".secure", false) ? "secure://" : "") +
      ...
      if (!identity.empty()) auth_scheme = "digest";
    } else if (key == "root") {
      ...
      if (password.size() > Coordination::PASSWORD_LENGTH)
        throw KeeperException(Coordination::Error::ZBADARGUMENTS, "Password cannot be longer than {} characters, specified {}",
  ...
  if (load_balancing) get_priority_load_balancing = DB::GetPriorityForLoadBalancing(*load_balancing, thread_local_rng() % hosts.size());
