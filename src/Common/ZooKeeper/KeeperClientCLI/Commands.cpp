...
bool LSCommand::parse(IParser::Pos &pos, boost::intrusive_ptr<ASTKeeperQuery> &node, Expected &expected) const {
  String path;
  if (!parseKeeperPath(pos, expected, path)) return true;
...
void LSCommand::execute(const ASTKeeperQuery *query, KeeperClientBase *client) const {
  ...
  for (const auto &child : children) {
    if (std::exchange(need_space, true)) client->cout << " ";
...
bool SetCommand::parse(IParser::Pos &pos, boost::intrusive_ptr<ASTKeeperQuery> &node, Expected &expected) const {
  String path;
  if (!parseKeeperPath(pos, expected, path)) return false;
  node->args.push_back(std::move(path));

  String arg;
  if (!parseKeeperArg(pos, expected, arg)) return false;
  node->args.push_back(std::move(arg));

  ASTPtr version;
  if (ParserNumber{}.parse(pos, version, expected)) node->args.push_back(version->as<ASTLiteral &>().value);
...
void CreateCommand::execute(const ASTKeeperQuery *query, KeeperClientBase *client) const {
  client->zookeeper->create(client->getAbsolutePath(query->args[0].safeGet<String>()), query->args[1].safeGet<String>(),
...
bool TouchCommand::parse(IParser::Pos &pos, boost::intrusive_ptr<ASTKeeperQuery> &node, Expected &expected) const {
  String arg;
  if (!parseKeeperPath(pos, expected, arg)) return false;
  node->args.push_back(std::move(arg));
...
struct TraversalTask : public std::enable_shared_from_this<TraversalTask<UserCtx>> {
 ...
 public:
  TraversalTask(const fs::path &path_, TraversalTaskPtr parent_) : path(path_), parent(parent_) {}
  ...
  void onStart(Ctx &ctx) {
    ...
    ctx.in_flight_list_requests.push_back(
        [task = this->shared_from_this(), list_request](Ctx &ctx_) mutable { task->onGetChildren(ctx_, list_request->get()); });
  }
...
bool FindSuperNodes::parse(IParser::Pos &pos, boost::intrusive_ptr<ASTKeeperQuery> &node, Expected &expected) const {
  ASTPtr threshold;
  if (!ParserUnsignedInteger{}.parse(pos, threshold, expected)) return false;
  ...
  String path;
  if (!parseKeeperPath(pos, expected, path)) path = ".";
...
bool DeleteStaleBackups::parse(IParser::Pos & /* pos */, boost::intrusive_ptr<ASTKeeperQuery> & /* node */,
                               Expected & /* expected */) const {
  return true;
}

void DeleteStaleBackups::execute(const ASTKeeperQuery * /* query */, KeeperClientBase *client) const {
  client->askConfirmation("You are going to delete all inactive backups in /clickhouse/backups.", [client] {
...
void FindBigFamily::execute(const ASTKeeperQuery *query, KeeperClientBase *client) const {
  ...
  struct {
    ...
    void onFinishChildrenTraversal(const fs::path &path, Int64 nodes_in_subtree) { result.emplace_back(nodes_in_subtree, path.string()); }
  } ctx;
...
void RMCommand::execute(const ASTKeeperQuery *query, KeeperClientBase *client) const {
  Int32 version{-1};
  if (query->args.size() == 2) version = static_cast<Int32>(query->args[1].safeGet<Int32>());
...
void RMRCommand::execute(const ASTKeeperQuery *query, KeeperClientBase *client) const {
  ...
  client->askConfirmation("You are going to recursively delete path " + path, [client, path, remove_nodes_limit] {
    client->zookeeper->removeRecursive(path, static_cast<UInt32>(remove_nodes_limit));
  });
}
...
void HelpCommand::execute(const ASTKeeperQuery * /* query */, KeeperClientBase *client) const {
  for (const auto &pair : KeeperClientBase::commands) client->cout << pair.second->generateHelpString() << "\n";
}
...
bool FourLetterWordCommand::parse(IParser::Pos &pos, boost::intrusive_ptr<ASTKeeperQuery> &node, Expected &expected) const {
  ...
  if (pos->type != TokenType::BareWord) return false;
...
void GetAllChildrenNumberCommand::execute(const ASTKeeperQuery *query, KeeperClientBase *client) const {
  ...
  while (!queue.empty()) {
    ...
    for (auto &child : children) child = next_path / child;
...
class CPMVOperation {
 ...
 public:
  ...
  void perform() {
    ...
    if (remove_src) ops.push_back(zkutil::makeRemoveRequest(src, src_stat.version));
    ...
    switch (code) {
      ...
      case Coordination::Error::ZBADVERSION: {
        ...
        if (isTryLimitReached()) zkutil::KeeperMultiException::check(code, ops, responses);
...
class CPMVROperation {
  ...
  std::vector<TreeNode> collectSubtree() {
    ...
    while (!queue.empty()) {
      ...
      for (const auto &child : client->zookeeper->getChildren(next_path)) queue.emplace(fs::path(next_path) / child);
  ...
  void perform() {
    ...
    switch (code) {
      ...
      case Coordination::Error::ZNONODE: {
        ...
        if (isTryLimitReached() || error.getPathForFirstFailedOp() == src) error.rethrow();
...
bool CPCommand::parse(IParser::Pos &pos, boost::intrusive_ptr<ASTKeeperQuery> &node, [[maybe_unused]] Expected &expected) const {
  String src_path;
  if (!parseKeeperPath(pos, expected, src_path)) return false;
  node->args.push_back(std::move(src_path));

  String to_path;
  if (!parseKeeperPath(pos, expected, to_path)) return false;
...
void CPCommand::execute(const ASTKeeperQuery *query, KeeperClientBase *client) const {
  ...
  while (!operation.isTryLimitReached() && !operation.isCompleted()) operation.perform();
}
...
bool GetAclCommand::parse(IParser::Pos &pos, boost::intrusive_ptr<ASTKeeperQuery> &node, Expected &expected) const {
  String src_path;
  if (!parseKeeperPath(pos, expected, src_path)) return true;
    ...
    if (acl.permissions == Coordination::ACL::All) {
      ...
      for (const auto &[perm, name] : perms_map) {
        if (acl.permissions & perm) {
          if (!first) client->cout << "|";
          client->cout << name;
          first = false;
        }
      }
      if (first) client->cout << "NONE";
    }
    client->cout << "\n";
  }
}
