...
class TouchCommand : public IKeeperClientCommand {
  ...
  String getHelpMessage() const override {
    return "{} <path> -- Creates new node with an empty string as value. Doesn't throw an exception if the node already exists";
  }
};
...
class DeleteStaleBackups : public IKeeperClientCommand {
  ...
  String getHelpMessage() const override { return "{} -- Deletes ClickHouse nodes used for backups that are now inactive"; }
};
...
class RMRCommand : public IKeeperClientCommand {
  ...
  String getHelpMessage() const override {
    return "{} <path> [limit] -- Recursively deletes path if the subtree size is smaller than the limit. Confirmation required (default "
           "limit = 100)";
  }
};
...
class ReconfigCommand : public IKeeperClientCommand {
  ...
  String getHelpMessage() const override {
    return "{} <add|remove|set> \"<arg>\" [version] -- Reconfigure Keeper cluster. See "
           "https://clickhouse.com/docs/en/guides/sre/keeper/clickhouse-keeper#reconfiguration";
  }
};
...
class GetDirectChildrenNumberCommand : public IKeeperClientCommand {
  ...
  String getHelpMessage() const override { return "{} [path] -- Get numbers of direct children nodes under a specific path"; }
};
...
class GetAllChildrenNumberCommand : public IKeeperClientCommand {
  ...
  String getHelpMessage() const override { return "{} [path] -- Get all numbers of children nodes under a specific path"; }
};
...
class CPCommand : public IKeeperClientCommand {
  ...
  String getHelpMessage() const override { return "{} <src> <dest> -- Copies 'src' node to 'dest' path."; }
};
...
class CPRCommand : public IKeeperClientCommand {
  ...
  String getHelpMessage() const override { return "{} <src> <dest> -- Copies 'src' node subtree to 'dest' path."; }
};
...
class MVCommand : public IKeeperClientCommand {
  ...
  String getHelpMessage() const override { return "{} <src> <dest> -- Moves 'src' node to the 'dest' path."; }
};
...
class MVRCommand : public IKeeperClientCommand {
  ...
  String getHelpMessage() const override { return "{} <src> <dest> -- Moves 'src' node subtree to 'dest' path."; }
};
...
class GetAclCommand : public IKeeperClientCommand {
  ...
  String getHelpMessage() const override { return "{} [path] -- Get ACL for specified path."; }
};
