...
bool parseKeeperArg(IParser::Pos& pos, Expected& expected, String& result) {
  if (pos->type == TokenType::QuotedIdentifier || pos->type == TokenType::StringLiteral) {
    if (!parseIdentifierOrStringLiteral(pos, expected, result)) return false;
...
bool KeeperParser::parseImpl(Pos& pos, ASTPtr& node, Expected& expected) {
  ...
  for (const auto& pair : KeeperClientBase::commands) expected.add(pos, pair.first.data());

  for (const auto& four_letter_word_command : four_letter_word_commands) expected.add(pos, four_letter_word_command.data());
  ...
  if (!command->parse(pos, query, expected)) return false;
