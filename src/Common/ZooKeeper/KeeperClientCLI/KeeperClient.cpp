...
fs::path KeeperClientBase::getAbsolutePath(const String& relative) const {
  ...
  if (result.ends_with('/') && result.size() > 1) result.pop_back();
...
void KeeperClientBase::loadCommands(std::vector<Command>&& new_commands) {
  ...
  for (const auto& command : four_letter_word_commands) registered_commands_and_four_letter_words.push_back(command);
...
KeeperClientBase::KeeperClientBase(std::ostream& cout_, std::ostream& cerr_) : cout(cout_), cerr(cerr_) {
  loadCommands({
