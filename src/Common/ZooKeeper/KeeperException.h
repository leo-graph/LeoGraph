...
class Exception : public DB::Exception {
 private:
  ...
  Exception(T &&message, Error code_)
      : DB::Exception(message, DB::ErrorCodes::KEEPER_EXCEPTION, /* format_string= */ "", /* remote_= */ false), code(code_) {
 ...
 public:
  ...
  Exception(Error code_, FormatStringHelper<Args...> fmt, Args &&...args)
      : DB::Exception(DB::ErrorCodes::KEEPER_EXCEPTION, std::move(fmt), std::forward<Args>(args)...), code(code_) {
    incrementErrorMetrics(code);
  }

  static Exception createDeprecated(const std::string &msg, Error code_) { return Exception(msg, code_, 0); }
...
class KeeperMultiException : public KeeperException {
 ...
 private:
  KeeperMultiException(Coordination::Error code, size_t failed_op_index_, const Coordination::Requests &requests_,
