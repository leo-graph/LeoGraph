...
struct ZooKeeperRetriesInfo {
  ...
  ZooKeeperRetriesInfo(UInt64 max_retries_, UInt64 initial_backoff_ms_, UInt64 max_backoff_ms_, QueryStatusPtr query_status_)
      : max_retries(max_retries_),
        initial_backoff_ms(std::min(initial_backoff_ms_, max_backoff_ms_)),
...
class ZooKeeperRetriesControl {
 public:
  ...
  void setUserError(std::exception_ptr exception, int code, const std::string& message) {
    if (logger) LOG_TRACE(logger, "ZooKeeperRetriesControl: {}: setUserError: error={} message={}", name, code, message);

    if (iteration_succeeded) total_failures++;
  ...
  void setUserError(const Exception& exception) { setUserError(std::make_exception_ptr(exception), exception.code(), exception.message()); }

  void setKeeperError(std::exception_ptr exception, Coordination::Error code, std::string message) {
    if (logger) LOG_TRACE(logger, "ZooKeeperRetriesControl: {}: setKeeperError: error={} message={}", name, code, message);
 ...
 private:
  ...
  bool canTry() {
    ...
    if (iteration_succeeded) {
      if (logger && total_failures > 0)
        LOG_DEBUG(logger, "ZooKeeperRetriesControl: {}: succeeded after: Iterations={} Total keeper failures={}/{}", name,
                  current_iteration, total_failures, retries_info.max_retries);
      return false;
    }
    ...
    if (retries_info.query_status) retries_info.query_status->checkTimeLimit();
  ...
  void throwIfError() const {
    if (user_error.exception) std::rethrow_exception(user_error.exception);

    if (keeper_error.exception) std::rethrow_exception(keeper_error.exception);
  }

  void logLastError(const std::string_view& header) {
    if (!logger) return;
    if (user_error.code == ErrorCodes::OK) {
      LOG_DEBUG(logger, "ZooKeeperRetriesControl: {}: {}: retry_count={}/{} timeout={}ms error={} message={}", name, header,
                current_iteration, retries_info.max_retries, current_backoff_ms, keeper_error.code, keeper_error.message);
    } else {
      LOG_DEBUG(logger, "ZooKeeperRetriesControl: {}: {}: retry_count={}/{} timeout={}ms error={} message={}", name, header,
                current_iteration, retries_info.max_retries, current_backoff_ms, user_error.code, user_error.message);
    }
  }
