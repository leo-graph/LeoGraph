...
std::shared_ptr<KeeperDispatcher> getKeeperDispatcher() {
  if (const auto global_context = Context::getGlobalContextInstance()) return global_context->tryGetKeeperDispatcher();
  return nullptr;
}
#endif
}  // namespace

void ZooKeeperOpentelemetrySpans::maybeInitialize(MaybeSpan& maybe_span, const std::optional<OpenTelemetry::TracingContext>& parent_context,
...
void ZooKeeperOpentelemetrySpans::maybeFinalizeImpl(MaybeSpan& maybe_span, std::vector<OpenTelemetry::SpanAttribute> attributes,
                                                    OpenTelemetry::SpanStatus status, const String& error_message, UInt64 finish_time_us) {
  chassert(maybe_span.start_time_us != 0);

  maybe_span.histogram.observe((finish_time_us - maybe_span.start_time_us) / 1000);

  if (!maybe_span.span) return;
  ...
  if (keeper_dispatcher) attributes.emplace_back("raft.role", keeper_dispatcher->getRoleString());
#endif
