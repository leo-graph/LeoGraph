...
struct ACL {
  ...
  bool operator<(const ACL &other) const {
    return std::tuple(permissions, scheme, id) < std::tuple(other.permissions, other.scheme, other.id);
  }
...
/// Valid errors sent from the server about database state (like "no node"). Logical and authentication errors (like "bad arguments") are
/// not here.
...
struct WatchCallbackPtrOrEventPtr {
 ...
 public:
  ...
  explicit operator bool() const { return static_cast<bool>(event) || static_cast<bool>(callback); }

  bool operator==(const WatchCallbackPtrOrEventPtr &rhs) const { return std::tie(callback, event) == std::tie(rhs.callback, rhs.event); }
...
struct CheckWatchRequest : virtual Request {
  enum class CheckWatchType : int32_t { CHILDREN = 1, DATA = 2, ANY = 3, PERSISTENT = 4, PERSISTENT_RECURSIVE = 5 };
...
struct RemoveWatchRequest : virtual Request {
  String path;
  enum class WatchType : int32_t { CHILDREN = 1, DATA = 2, PERSISTENT = 4, PERSISTENTRECURSIVE = 5, ANY = 3 } type;
...
struct SetWatchesRequest : virtual Request {
 ...
 protected:
  static size_t pathesSize(const std::vector<String> &paths) {
    ...
    for (const auto &elem : paths) result += sizeof(Int32) + elem.size();
    return result;
  }
};
...
struct CreateRequest : virtual Request {
  ...
  size_t bytesSize() const override {
    return path.size() + data.size() + sizeof(is_ephemeral) + sizeof(is_sequential) + acls.size() * sizeof(ACL);
  }
};
...
enum class ListRequestType : uint8_t { ALL, PERSISTENT_ONLY, EPHEMERAL_ONLY };
...
struct ListResponse : virtual Response {
  ...
  size_t bytesSize() const override {
    ...
    for (const auto &name : names) size += name.size();
    size += stats.size() * sizeof(Stat);
    for (const auto &child_data : data) size += child_data.size();
    return size;
  }
};
...
struct ReconfigRequest : virtual Request {
  ...
  size_t bytesSize() const final { return joining.size() + leaving.size() + new_members.size() + sizeof(version); }
};
...
struct MultiRequest : virtual Request {
  ...
  void addRootPath(const String &root_path) override {
    for (auto &request : requests) request->addRootPath(root_path);
  }
  ...
  size_t bytesSize() const override {
    size_t size = 0;
    for (const auto &request : requests) size += request->bytesSize();
    return size;
  }
};
...
struct MultiResponse : virtual Response {
  ...
  size_t bytesSize() const override {
    size_t size = 0;
    for (const auto &response : responses) size += response->bytesSize();
    return size;
  }
};
...
/// For watches.
enum State { EXPIRED_SESSION = -112, AUTH_FAILED = -113, CONNECTING = 1, ASSOCIATING = 2, CONNECTED = 3, READONLY = 5, NOTCONNECTED = 999 };

enum Event { CREATED = 1, DELETED = 2, CHANGED = 3, CHILD = 4, SESSION = -1, NOTWATCHING = -2 };
...
class IKeeper {
 public:
  ...
  virtual void create(const String &path, const String &data, bool is_ephemeral, bool is_sequential, const ACLs &acls,
                      CreateCallback callback) = 0;

  virtual void remove(const String &path, int32_t version, RemoveCallback callback) = 0;

  virtual void removeRecursive(const String &path, uint32_t remove_nodes_limit, RemoveRecursiveCallback callback) = 0;

  virtual void exists(const String &path, ExistsCallback callback, WatchCallbackPtrOrEventPtr watch) = 0;

  virtual void get(const String &path, GetCallback callback, WatchCallbackPtrOrEventPtr watch) = 0;

  virtual void set(const String &path, const String &data, int32_t version, SetCallback callback) = 0;

  virtual void list(const String &path, ListRequestType list_request_type, ListCallback callback, WatchCallbackPtrOrEventPtr watch,
                    bool with_stat, bool with_data) = 0;

  virtual void check(const String &path, int32_t version, CheckCallback callback) = 0;

  virtual void sync(const String &path, SyncCallback callback) = 0;

  virtual void reconfig(std::string_view joining, std::string_view leaving, std::string_view new_members, int32_t version,
                        ReconfigCallback callback) = 0;

  virtual void multi(std::span<const RequestPtr> requests, MultiCallback callback) = 0;
...
template <>
struct fmt::formatter<Coordination::Error> : fmt::formatter<std::string_view> {
...
struct std::hash<Coordination::WatchCallbackPtrOrEventPtr> {
  size_t operator()(const Coordination::WatchCallbackPtrOrEventPtr &self) const { return self.hash(); }
};
