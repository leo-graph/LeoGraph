...
class EventNotifier {
 public:
  struct Handler {
    Handler(EventNotifier& parent_, size_t event_id_, size_t callback_id_)
        : parent(parent_), event_id(event_id_), callback_id(callback_id_) {}
  ...
  void notify(EventType event) {
    ...
    for (const auto& identifier : callback_table[DefaultHash64(event)]) storage[identifier]();
  }
