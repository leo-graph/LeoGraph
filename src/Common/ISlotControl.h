...
class IAcquiredSlot : public std::enable_shared_from_this<IAcquiredSlot>, boost::noncopyable {
 public:
  ...
  explicit IAcquiredSlot(size_t slot_id_) : slot_id(slot_id_) {}
...
class ISlotLease : public IAcquiredSlot {
 public:
  explicit ISlotLease(size_t slot_id_) : IAcquiredSlot(slot_id_) {}
...
class GrantedAllocation : public ISlotAllocation {
 public:
  explicit GrantedAllocation(SlotCount granted_) : total(granted_), granted(granted_) {}

  [[nodiscard]] AcquiredSlotPtr tryAcquire() override {
    SlotCount value = granted.load();
    while (value) {
      if (granted.compare_exchange_strong(value, value - 1)) return std::make_shared<IAcquiredSlot>(total - value);
    }
    return {};
  }
