...
class Exception : public Poco::Exception {
 public:
  ...
  Exception() {
    if (terminate_on_any_exception) std::terminate();
  ...
  Exception(const Exception &o) : Poco::Exception(o) {
    remote = o.remote;
  ...
  static void initializeThreadFramePointers() { [[maybe_unused]] auto &v = thread_frame_pointers; }
 ...
 protected:
  ...
  Exception(const std::string &msg, int code, std::string format_string = "", bool remote_ = false)
      : Exception(MessageMasked(msg, std::move(format_string)), code, remote_) {}
  Exception(std::string &&msg, int code, std::string format_string = "", bool remote_ = false)
      : Exception(MessageMasked(std::move(msg), std::move(format_string)), code, remote_) {}
 ...
 public:
  ...
  static Exception createDeprecated(const std::string &msg, int code, bool remote_ = false) { return Exception(msg, code, "", remote_); }
...
[[noreturn]] void abortOnFailedAssertion(const String &description, std::string_view format_string, void *const *trace, size_t trace_offset,
                                         size_t trace_size);
...
struct ExecutionStatus {
  ...
  explicit ExecutionStatus(int return_code, const std::string &exception_message = "") : code(return_code), message(exception_message) {}
...
/// Return copy of a current exception if it is a Poco::Exception (DB::Exception), since this exception is mutable, and returning reference
/// is unsafe. And a reference otherwise.
