...
class NamedCollection {
 public:
  ...
  template <typename T>
  T getAny(const std::initializer_list<Key>& keys) const;

  template <typename T>
  T getAnyOrDefault(const std::initializer_list<Key>& keys, const T& default_value) const;
 ...
 protected:
  ...
  NamedCollection(ImplPtr pimpl_, const std::string& collection_name, bool is_mutable_);
...
class NamedCollectionFromConfig final : public NamedCollection {
 public:
  static MutableNamedCollectionPtr create(const Poco::Util::AbstractConfiguration& config, const std::string& collection_name,
 ...
 private:
  NamedCollectionFromConfig(const Poco::Util::AbstractConfiguration& config, const std::string& collection_name,
                            const std::string& collection_path, const Keys& keys);
};
