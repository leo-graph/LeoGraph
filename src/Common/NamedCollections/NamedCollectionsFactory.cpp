...
NamedCollectionFactory::~NamedCollectionFactory() { shutdown(); }

void NamedCollectionFactory::shutdown() {
  shutdown_called = true;
  if (update_task) update_task->deactivate();
...
MutableNamedCollectionPtr NamedCollectionFactory::tryGet(const std::string &collection_name, std::lock_guard<std::mutex> &) const {
  auto it = loaded_named_collections.find(collection_name);
  if (it == loaded_named_collections.end()) return nullptr;
  return it->second;
}

MutableNamedCollectionPtr NamedCollectionFactory::getMutable(const std::string &collection_name, std::lock_guard<std::mutex> &lock) const {
  auto collection = tryGet(collection_name, lock);
  if (!collection) {
    throw Exception(ErrorCodes::NAMED_COLLECTION_DOESNT_EXIST, "There is no named collection `{}`", collection_name);
  }
  if (!collection->isMutable()) {
    throw Exception(ErrorCodes::NAMED_COLLECTION_IS_IMMUTABLE,
...
void NamedCollectionFactory::add(const std::string &collection_name, MutableNamedCollectionPtr collection, std::lock_guard<std::mutex> &) {
  auto [it, inserted] = loaded_named_collections.emplace(collection_name, collection);
  if (!inserted) {
    throw Exception(ErrorCodes::NAMED_COLLECTION_ALREADY_EXISTS, "A named collection `{}` already exists", collection_name);
  }
}

void NamedCollectionFactory::add(NamedCollectionsMap collections, std::lock_guard<std::mutex> &lock) {
  for (const auto &[collection_name, collection] : collections) add(collection_name, collection, lock);
}
...
bool NamedCollectionFactory::removeIfExists(const std::string &collection_name, std::lock_guard<std::mutex> &lock) {
  auto collection = tryGet(collection_name, lock);
...
void NamedCollectionFactory::removeById(NamedCollection::SourceId id, std::lock_guard<std::mutex> &) {
  std::erase_if(loaded_named_collections, [&](const auto &value) { return value.second->getSourceId() == id; });
}
...
MutableNamedCollectionPtr getCollection(const Poco::Util::AbstractConfiguration &config, const std::string &collection_name) {
  ...
  const bool collection_is_empty = enumerate_result.size() == 1 && *enumerate_result.begin() == collection_prefix;

  std::set<std::string, std::less<>> keys;
  if (!collection_is_empty) {
    /// Skip collection prefix and add +1 to avoid '.' in the beginning.
    for (const auto &path : enumerate_result) keys.emplace(path.substr(collection_prefix.size() + 1));
  }

  return NamedCollectionFromConfig::create(config, collection_name, collection_prefix, keys);
}
...
bool NamedCollectionFactory::loadIfNot(std::lock_guard<std::mutex> &lock) {
  ...
  if (metadata_storage->isReplicated()) {
    update_task =
        context->getSchedulePool().createTask(StorageID::createEmpty(), "NamedCollectionsMetadataStorage", [this] { updateFunc(); });
...
void NamedCollectionFactory::createFromSQL(const ASTCreateNamedCollectionQuery &query) {
  ...
  if (exists(query.collection_name, lock)) {
    if (query.if_not_exists) return;

    throw Exception(ErrorCodes::NAMED_COLLECTION_ALREADY_EXISTS, "A named collection `{}` already exists", query.collection_name);
  }
...
void NamedCollectionFactory::removeFromSQL(const ASTDropNamedCollectionQuery &query) {
  ...
  if (!exists(query.collection_name, lock)) {
    ...
    throw Exception(ErrorCodes::NAMED_COLLECTION_DOESNT_EXIST, "Cannot remove collection `{}`, because it doesn't exist",
...
void NamedCollectionFactory::updateFromSQL(const ASTAlterNamedCollectionQuery &query) {
  ...
  if (!exists(collection_name, lock)) {
    ...
    throw Exception(ErrorCodes::NAMED_COLLECTION_DOESNT_EXIST, "Cannot update collection `{}`, because it doesn't exist", collection_name);
  }
  auto updated_collection_ptr = metadata_storage->update(query);

  auto it = loaded_named_collections.find(collection_name);
  if (it == loaded_named_collections.end()) {
    throw Exception(ErrorCodes::LOGICAL_ERROR, "The named collection {} unexpectedly does not exist.", collection_name);
  }
...
void NamedCollectionFactory::removeDependencies(const StorageID &table_id) {
  ...
  if (table_id.hasUUID()) {
    ...
    for (auto it = range.first; it != range.second; ++it) {
      if (it->table_id.uuid == UUIDHelpers::Nil) to_erase.push_back(it);
    }

    for (auto it : to_erase) idx.erase(it);
  }
}
...
void NamedCollectionFactory::renameDependencies(const StorageID &from_table_id, const StorageID &to_table_id) {
  ...
  if (from_table_id.hasUUID() || to_table_id.hasUUID()) return;
  ...
  for (auto it : to_erase) name_idx.erase(it);

  for (const auto &name : collection_names) dependencies.emplace(name, to_table_id);
}
...
std::vector<StorageID> NamedCollectionFactory::getDependents(const String &collection_name) const {
  ...
  for (auto it = range.first; it != range.second; ++it) result.push_back(it->table_id);
