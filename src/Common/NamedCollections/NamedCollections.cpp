...
class NamedCollection::Impl {
 ...
 public:
  static ImplPtr create(const Poco::Util::AbstractConfiguration &config, const std::string &collection_name,
                        const std::string &collection_path, const Keys &keys) {
    auto collection_config = NamedCollectionConfiguration::createEmptyConfiguration(collection_name);
    for (const auto &key : keys) Configuration::copyConfigValue<String>(config, collection_path + '.' + key, *collection_config, key);

    return std::unique_ptr<Impl>(new Impl(collection_config, keys));
  }

  bool has(const Key &key) const { return Configuration::hasConfigValue(*config, key); }
  ...
  template <typename T>
  T getOrDefault(const Key &key, const T &default_value) const {
  ...
  bool isOverridable(const Key &key, const bool default_value) {
    ...
    if (is_overridable) return *is_overridable;
    return default_value;
  }

  ImplPtr createCopy(const std::string &collection_name_) const { return create(*config, collection_name_, "", keys); }
  ...
  Keys getKeys(ssize_t depth, const std::string &prefix) const {
    ...
    if (prefix.empty()) {
      ...
      if (!Configuration::hasConfigValue(*config, prefix)) return {};
  ...
  Keys::const_iterator begin() const { return keys.begin(); }

  Keys::const_iterator end() const { return keys.end(); }
  ...
  std::string dumpStructure() const {
    ...
    for (const auto &key : keys) {
      ...
      while (it != key_parts.end() && prev_key_parts_it != prev_key_parts.end() && *it == *prev_key_parts_it) {
        ++it;
        ++prev_key_parts_it;
        ++tab_cnt;
      }
...
NamedCollection::NamedCollection(ImplPtr pimpl_, const std::string &collection_name_, const bool is_mutable_)
    : pimpl(std::move(pimpl_)), collection_name(collection_name_), is_mutable(is_mutable_) {}
...
bool NamedCollection::hasAny(const std::initializer_list<Key> &keys) const {
  std::lock_guard lock(mutex);
  for (const auto &key : keys)
    if (pimpl->has(key)) return true;
  return false;
}

template <typename T>
T NamedCollection::get(const Key &key) const {
  std::lock_guard lock(mutex);
  return pimpl->get<T>(key);
}

template <typename T>
T NamedCollection::getOrDefault(const Key &key, const T &default_value) const {
  std::lock_guard lock(mutex);
  return pimpl->getOrDefault<T>(key, default_value);
}

template <typename T>
T NamedCollection::getAny(const std::initializer_list<Key> &keys) const {
  std::lock_guard lock(mutex);
  for (const auto &key : keys) {
    if (pimpl->has(key)) return pimpl->get<T>(key);
  }
  throw Exception(ErrorCodes::BAD_ARGUMENTS, "No such keys: {}", fmt::join(keys, ", "));
}

template <typename T>
T NamedCollection::getAnyOrDefault(const std::initializer_list<Key> &keys, const T &default_value) const {
  std::lock_guard lock(mutex);
  for (const auto &key : keys) {
    if (pimpl->has(key)) return pimpl->get<T>(key);
  }
...
void NamedCollection::assertMutable() const {
  if (!is_mutable) {
    throw Exception(ErrorCodes::NAMED_COLLECTION_IS_IMMUTABLE, "Cannot change named collection because it is immutable");
  }
}
...
MutableNamedCollectionPtr NamedCollection::duplicate() const {
  ...
  return std::unique_ptr<NamedCollection>(new NamedCollection(std::move(impl), collection_name, true));
}
...
template <bool Locked>
NamedCollection::const_iterator NamedCollection::begin() const {
  std::unique_lock lock(mutex, std::defer_lock);
  if constexpr (!Locked) lock.lock();
  return pimpl->begin();
}
...
template <bool Locked>
NamedCollection::const_iterator NamedCollection::end() const {
  std::unique_lock lock(mutex, std::defer_lock);
  if constexpr (!Locked) lock.lock();
  return pimpl->end();
}
...
std::unique_lock<std::mutex> NamedCollection::lock() { return std::unique_lock(mutex); }

void NamedCollection::update(const ASTAlterNamedCollectionQuery & /*query*/) {
  throw Exception(ErrorCodes::NOT_IMPLEMENTED, "update() not implemented for NamedCollection base class.");
}

NamedCollectionFromConfig::NamedCollectionFromConfig(const Poco::Util::AbstractConfiguration &config_, const std::string &collection_name_,
                                                     const std::string &collection_path_, const Keys &keys_)
    : NamedCollection(Impl::create(config_, collection_name_, collection_path_, keys_), collection_name_, /* is_mutable */ false) {}

MutableNamedCollectionPtr NamedCollectionFromConfig::create(const Poco::Util::AbstractConfiguration &config_,
                                                            const std::string &collection_name_, const std::string &collection_path_,
                                                            const Keys &keys_) {
  return std::unique_ptr<NamedCollection>(new NamedCollectionFromConfig(config_, collection_name_, collection_path_, keys_));
}
...
NamedCollectionFromSQL::NamedCollectionFromSQL(const ASTCreateNamedCollectionQuery &query_)
    : NamedCollection(nullptr, query_.collection_name, true), create_query_ptr(query_.clone()->as<ASTCreateNamedCollectionQuery &>()) {
  const auto config =
      NamedCollectionConfiguration::createConfiguration(collection_name, create_query_ptr.changes, create_query_ptr.overridability);

  std::set<std::string, std::less<>> keys;
  for (const auto &[name, _] : create_query_ptr.changes) keys.insert(name);
...
void NamedCollectionFromSQL::update(const ASTAlterNamedCollectionQuery &alter_query) {
  ...
  for (const auto &[name, value] : alter_query.changes) {
    ...
    if (!inserted) {
      throw Exception(ErrorCodes::BAD_ARGUMENTS, "Value with key `{}` is used twice in the SET query (collection name: {})", name,
                      alter_query.collection_name);
    }
  }

  for (const auto &[name, value] : create_query_ptr.changes) result_changes_map.emplace(name, value);

  std::unordered_map<std::string, bool> result_overridability_map;
  for (const auto &[name, value] : alter_query.overridability) result_overridability_map.emplace(name, value);
  for (const auto &[name, value] : create_query_ptr.overridability) result_overridability_map.emplace(name, value);

  for (const auto &delete_key : alter_query.delete_keys) {
    auto it = result_changes_map.find(delete_key);
    if (it == result_changes_map.end()) {
      throw Exception(ErrorCodes::BAD_ARGUMENTS, "Cannot delete key `{}` because it does not exist in collection", delete_key);
    }

    result_changes_map.erase(it);
    auto it_override = result_overridability_map.find(delete_key);
    if (it_override != result_overridability_map.end()) result_overridability_map.erase(it_override);
  }

  create_query_ptr.changes.clear();
  for (const auto &[name, value] : result_changes_map) create_query_ptr.changes.emplace_back(name, value);
  ...
  for (const auto &key : alter_query.delete_keys) remove<true>(key);
}
...
template String NamedCollection::getAnyOrDefault<String>(const std::initializer_list<NamedCollection::Key> &key,
                                                         const String &default_value) const;
template UInt64 NamedCollection::getAnyOrDefault<UInt64>(const std::initializer_list<NamedCollection::Key> &key,
                                                         const UInt64 &default_value) const;
template Int64 NamedCollection::getAnyOrDefault<Int64>(const std::initializer_list<NamedCollection::Key> &key,
                                                       const Int64 &default_value) const;
template Float64 NamedCollection::getAnyOrDefault<Float64>(const std::initializer_list<NamedCollection::Key> &key,
                                                           const Float64 &default_value) const;
template bool NamedCollection::getAnyOrDefault<bool>(const std::initializer_list<NamedCollection::Key> &key,
