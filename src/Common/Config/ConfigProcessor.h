...
class ConfigProcessor {
 public:
  ...
  explicit ConfigProcessor(const std::string& path, bool throw_on_bad_incl = false, bool log_to_console = false,
                           const Substitutions& substitutions = Substitutions(), bool throw_on_bad_include_from = true);
  ...
  XMLDocumentPtr processConfig(bool* has_zk_includes = nullptr, zkutil::ZooKeeperNodeCache* zk_node_cache = nullptr,
                               const Coordination::EventPtr& zk_changed_event = nullptr, bool is_config_changed = true);

  static void processIncludes(XMLDocumentPtr& config, const Substitutions& substitutions, const std::string& include_from_path,
                              bool throw_on_bad_incl, Poco::XML::DOMParser& dom_parser, const LoggerPtr& log,
                              std::unordered_set<std::string>* contributing_zk_paths = {},
                              std::vector<std::string>* contributing_files = {}, zkutil::ZooKeeperNodeCache* zk_node_cache = {},
  ...
  LoadedConfig loadConfigWithZooKeeperIncludes(zkutil::ZooKeeperNodeCache* zk_node_cache, const Coordination::EventPtr& zk_changed_event,
                                               bool fallback_to_preprocessed = false, bool is_config_changed = true);
 ...
 private:
  ...
  static bool hasNodeWithNameAndChildNodeWithAttribute(Poco::XML::Node* config_root, const std::string& node_name,
                                                       const std::string& attribute_name);
  /// Determine if there is a node in loaded_config with a given node_name which has a descendant with a given attribute
  static bool hasNodeWithNameAndChildNodeWithAttribute(LoadedConfig& loaded_config, const std::string& node_name,
                                                       const std::string& attribute_name);
#endif
  ...
  static void doIncludesRecursive(XMLDocumentPtr config, XMLDocumentPtr include_from, const Substitutions& substitutions,
                                  bool throw_on_bad_incl, Poco::XML::DOMParser& dom_parser, const LoggerPtr& log, Poco::XML::Node* node,
