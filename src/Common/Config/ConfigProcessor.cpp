...
bool ConfigProcessor::isPreprocessedFile(const std::string &path) { return endsWith(fs::path(path).stem(), PREPROCESSED_SUFFIX); }

ConfigProcessor::ConfigProcessor(const std::string &path_, bool throw_on_bad_incl_, bool log_to_console,
                                 const Substitutions &substitutions_, bool throw_on_bad_include_from_)
    : path(path_),
...
void ConfigProcessor::registerEmbeddedConfig(std::string name, std::string_view content) { embedded_configs[name] = content; }
...
static ElementIdentifier getElementIdentifier(Node *element) {
  ...
  for (const Node *node = attrs->item(0); node; node = node->nextSibling()) {
    ...
    if (name == "replace" || name == "remove" || subst_name_pos != ConfigProcessor::SUBSTITUTION_ATTRS.end()) continue;
...
static Node *getRootNode(Document *document) { return XMLUtils::getRootNode(document); }

static size_t firstNonWhitespacePos(const std::string &s) { return s.find_first_not_of(" \t\n\r"); }

static bool allWhitespace(const std::string &s) { return firstNonWhitespacePos(s) == std::string::npos; }
...
static void deleteAttributesRecursive(Node *root) {
  ...
  for (Node *child = children->item(0); child; child = child->nextSibling()) {
    if (child->nodeType() == Node::ELEMENT_NODE) {
      ...
      if (child_element.hasAttribute("replace")) child_element.removeAttribute("replace");
...
bool ConfigProcessor::hasNodeWithAttribute(Poco::XML::Node *config_root, const std::string &attribute_name) {
  for (Node *node = config_root->firstChild(); node != nullptr; node = node->nextSibling()) {
    ...
    if (dynamic_cast<Element &>(*node).hasAttribute(attribute_name) || hasNodeWithAttribute(node, attribute_name)) return true;
  }
  return false;
}

bool ConfigProcessor::hasNodeWithNameAndChildNodeWithAttribute(Poco::XML::Node *config_root, const std::string &node_name,
                                                               const std::string &attribute_name) {
  for (Node *node = config_root->firstChild(); node != nullptr; node = node->nextSibling()) {
    if (node->nodeType() != Node::ELEMENT_NODE) continue;

    if ((node_name == node->nodeName() && hasNodeWithAttribute(node, attribute_name)) ||
        hasNodeWithNameAndChildNodeWithAttribute(node, node_name, attribute_name))
      return true;
  }
  return false;
}

bool ConfigProcessor::hasNodeWithNameAndChildNodeWithAttribute(LoadedConfig &loaded_config, const std::string &node_name,
                                                               const std::string &attribute_name) {
...
void ConfigProcessor::mergeRecursive(XMLDocumentPtr config, Node *config_root, const Node *with_root) {
  ...
  for (Node *with_node = with_nodes->item(0); with_node; with_node = next_with_node) {
    ...
    if (with_node->nodeType() == Node::ELEMENT_NODE) {
      ...
      if (remove && replace) throw Poco::Exception("both remove and replace attributes set for element <" + with_node->nodeName() + ">");
      ...
      if (it != config_element_by_id.end()) {
        ...
        if (remove) {
          ...
          if (source_has_value)
            for (const auto &attr_name : SUBSTITUTION_ATTRS) config_element.removeAttribute(attr_name);
...
bool ConfigProcessor::merge(XMLDocumentPtr config, XMLDocumentPtr with) {
  ...
  if (config_root_node_name != merged_root_node_name && !((config_root_node_name == "yandex" || config_root_node_name == "clickhouse") &&
                                                          (merged_root_node_name == "yandex" || merged_root_node_name == "clickhouse"))) {
    if (config_root_node_name != "clickhouse" && config_root_node_name != "yandex") return false;

    throw Poco::Exception(
        "Root element doesn't have the corresponding root element as the config file."
        " It must be <" +
        config_root->nodeName() + ">");
  }
...
void ConfigProcessor::doIncludesRecursive(XMLDocumentPtr config, XMLDocumentPtr include_from, const Substitutions &substitutions,
                                          bool throw_on_bad_incl, Poco::XML::DOMParser &dom_parser, const LoggerPtr &log, Node *node,
                                          zkutil::ZooKeeperNodeCache *zk_node_cache, const Coordination::EventPtr &zk_changed_event,
  ...
  if (node->nodeType() == Node::TEXT_NODE) {
    for (const auto &substitution : substitutions) {
      ...
      if (replace_occurred) node->setNodeValue(value);
    }
  }
  ...
  if (node->nodeName() == "include") {
    if (node->hasChildNodes()) throw Poco::Exception("<include> element must have no children");
    if (substs_count == 0) throw Poco::Exception("No substitution attributes set for element <include>, must have exactly one");
  }
  ...
  auto process_include = [&](const Node *include_attr, const std::function<const Node *(const std::string &)> &get_node,
    ...
    if (!node_to_include) {
      if (attributes->getNamedItem("optional"))
        ...
        if (node->nodeName() == "include") node->parentNode()->removeChild(node);
      ...
      if (node->nodeName() == "include") {
        ...
        for (const auto &attr_name : SUBSTITUTION_ATTRS) element.removeAttribute(attr_name);

        if (replace) {
          while (Node *child = node->firstChild()) node->removeChild(child);
  ...
  if (attr_nodes["from_zk"])  /// we have zookeeper subst
  {
    ...
    if (node->hasChildNodes() && !replace)
      throw Poco::Exception("Element <" + node->nodeName() +
                            "> has value and does not have 'replace' attribute, can't process from_zk substitution");

    if (contributing_zk_paths) contributing_zk_paths->insert(attr_nodes["from_zk"]->getNodeValue());
    ...
    if (zk_node_cache) {
      ...
      auto get_zk_node = [&](const std::string &name) -> const Node * {
        ...
        if (!znode.exists) return nullptr;
  ...
  if (attr_nodes["from_env"])  /// we have env subst
  {
    ...
    auto get_env_node = [&](const std::string &name) -> const Node * {
      const char *env_val = std::getenv(
          name.c_str());  // NOLINT(concurrency-mt-unsafe) // this is safe on Linux glibc/Musl, but potentially not safe on other platforms
      if (env_val == nullptr) return nullptr;
  ...
  if (included_something)
    doIncludesRecursive(config, include_from, substitutions, throw_on_bad_incl, dom_parser, log, node, zk_node_cache, zk_changed_event,
    ...
    for (Node *child = children->item(0); child; child = next_child) {
      next_child = child->nextSibling();
      doIncludesRecursive(config, include_from, substitutions, throw_on_bad_incl, dom_parser, log, child, zk_node_cache, zk_changed_event,
                          contributing_zk_paths);
    }
  }
}
...
ConfigProcessor::Files ConfigProcessor::getConfigMergeFiles(const std::string &config_path) {
  ...
  for (const std::string &merge_dir_name : merge_dirs) {
    if (!fs::exists(merge_dir_name) || !fs::is_directory(merge_dir_name)) continue;
    ...
    for (fs::directory_iterator it(merge_dir_name); it != fs::directory_iterator(); ++it) {
      ...
      if (fs::is_regular_file(path) && (extension == ".xml" || extension == ".conf" || extension == ".yaml" || extension == ".yml") &&
...
XMLDocumentPtr ConfigProcessor::parseConfig(const std::string &config_path, Poco::XML::DOMParser &dom_parser) {
  ...
  if (extension == ".xml") return dom_parser.parse(config_path);
  if (extension == ".yaml" || extension == ".yml") return YAMLParser::parse(config_path);

  /// Suppose non regular file parsed as XML, such as pipe: /dev/fd/X (regardless it has .xml extension or not)
  if (!fs::is_regular_file(config_path)) return dom_parser.parse(config_path);

  /// If the regular file begins with < it might be XML, otherwise it might be YAML.
  bool maybe_xml = false;
  {
    std::ifstream file(config_path);
    if (!file.is_open()) throw Exception(ErrorCodes::CANNOT_LOAD_CONFIG, "Unknown format of '{}' config", config_path);
  ...
  if (maybe_xml) return dom_parser.parse(config_path);
  return YAMLParser::parse(config_path);
}

XMLDocumentPtr ConfigProcessor::processConfig(bool *has_zk_includes, zkutil::ZooKeeperNodeCache *zk_node_cache,
                                              const Coordination::EventPtr &zk_changed_event, bool is_config_changed) {
  if (is_config_changed) LOG_DEBUG(log, "Processing configuration file '{}'.", path);
  ...
  if (fs::exists(path)) {
    ...
    if (auto it = embedded_configs.find(path); it != embedded_configs.end()) {
      if (is_config_changed) LOG_DEBUG(log, "There is no file '{}', will use embedded config.", path);
  ...
  for (auto &merge_file : getConfigMergeFiles(path)) {
    try {
      if (is_config_changed) LOG_DEBUG(log, "Merging configuration file '{}'.", merge_file);

      XMLDocumentPtr with;
      with = parseConfig(merge_file, dom_parser);
      if (!merge(config, with)) {
        LOG_DEBUG(log,
                  "Merging bypassed - configuration file '{}' "
  ...
  try {
    ...
    if (node) {
      ...
      doIncludesRecursive(config, nullptr, substitutions, throw_on_bad_incl, dom_parser, log, node, zk_node_cache, zk_changed_event,
                          &contributing_zk_paths);

      include_from_path = node->innerText();
    } else {
      std::string default_path = "/etc/metrika.xml";
      if (fs::exists(default_path)) include_from_path = default_path;
    }
    ...
    if (!throw_on_bad_include_from && !fs::exists(include_from_path)) {
      ...
      processIncludes(config, substitutions, include_from_path, throw_on_bad_incl, dom_parser, log, &contributing_zk_paths,
                      &contributing_files, zk_node_cache, zk_changed_event);
    }
  } catch (Exception &e) {
  ...
  if (has_zk_includes) *has_zk_includes = !contributing_zk_paths.empty();
  ...
  if (zk_node_cache && !contributing_zk_paths.empty()) {
    ...
    for (const std::string &contributing_zk_path : contributing_zk_paths) comment << "\n       " << contributing_zk_path;
  }
...
void ConfigProcessor::processIncludes(XMLDocumentPtr &config, const Substitutions &substitutions, const std::string &include_from_path,
                                      bool throw_on_bad_incl, Poco::XML::DOMParser &dom_parser, const LoggerPtr &log,
                                      std::unordered_set<std::string> *contributing_zk_paths, std::vector<std::string> *contributing_files,
                                      zkutil::ZooKeeperNodeCache *zk_node_cache, const Coordination::EventPtr &zk_changed_event) {
  ...
  if (!include_from_path.empty()) {
    ...
    if (contributing_files) contributing_files->push_back(include_from_path);
  }

  doIncludesRecursive(config, include_from, substitutions, throw_on_bad_incl, dom_parser, log, getRootNode(config.get()), zk_node_cache,
                      zk_changed_event, contributing_zk_paths);
}
...
ConfigProcessor::LoadedConfig ConfigProcessor::loadConfigWithZooKeeperIncludes(zkutil::ZooKeeperNodeCache *zk_node_cache,
                                                                               const Coordination::EventPtr &zk_changed_event,
                                                                               bool fallback_to_preprocessed, bool is_config_changed) {
  ...
  try {
    ...
    if (zk_node_cache) zk_node_cache->sync();
    config_xml = processConfig(&has_zk_includes, zk_node_cache, zk_changed_event, is_config_changed);
    processed_successfully = true;
  } catch (const Poco::Exception &ex) {
    if (!fallback_to_preprocessed) throw;

    const auto *zk_exception = dynamic_cast<const Coordination::Exception *>(ex.nested());
    if (!zk_exception) throw;

    LOG_WARNING(log, "Error while processing from_zk config includes: {}. Config will be loaded from preprocessed file: {}",
                zk_exception->message(), preprocessed_path);
...
void ConfigProcessor::savePreprocessedConfig(LoadedConfig &loaded_config, std::string preprocessed_dir
  ...
  try {
    if (preprocessed_path.empty()) {
      ...
      if (new_path.starts_with(main_config_path)) new_path.erase(0, main_config_path.size());
      ...
      if (!preprocessed_path_parent.empty()) fs::create_directories(preprocessed_path_parent);
    }
    DOMWriter writer;
    writer.setNewLine("\n");
    writer.setIndent("    ");
...
#if USE_SSL
  /* Some callers (e.g. during very early server startup) will not have access to Zookeeper. Such callers can specify
  `skip_zk_encryption_keys = false` (but they will not be able to decrypt encrypted elements). If there are no `encryption_codecs` tags with
  `from_zk` attributes, we can decrypt anyways.
  ...
  encrypted_by="AES_128_GCM_SIV">96260000000B0000000000E8FE3C087CED2205A5071078B29FD5C3B97F824911DED3217E980C</max_table_size_to_drop>
  </clickhouse>
...
void ConfigProcessor::setConfigPath(const std::string &config_path) {
  ...
  if (!main_config_path.ends_with('/')) main_config_path += '/';
}
