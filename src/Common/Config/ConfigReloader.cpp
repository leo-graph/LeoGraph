...
ConfigReloader::ConfigReloader(std::string_view config_path_, const std::vector<std::string>& extra_paths_,
                               const std::string& preprocessed_dir_, std::unique_ptr<zkutil::ZooKeeperNodeCache>&& zk_node_cache_,
                               const Coordination::EventPtr& zk_changed_event_, Updater&& updater_)
  ...
  if (config.has_value())
    reload_interval =
        std::chrono::milliseconds(config->configuration->getInt64("config_reload_interval_ms", DEFAULT_RELOAD_INTERVAL.count()));
  else
...
std::optional<ConfigProcessor::LoadedConfig> ConfigReloader::reloadIfNewer(bool force, bool throw_on_error, bool fallback_to_preprocessed,
  ...
  if (force || need_reload_from_zk || is_config_changed) {
    ...
    try {
      ...
      if (loaded_config.has_zk_includes)
        loaded_config = config_processor.loadConfigWithZooKeeperIncludes(zk_node_cache.get(), zk_changed_event, fallback_to_preprocessed,
                                                                         is_config_changed);
    } catch (const Coordination::Exception& e) {
      if (Coordination::isHardwareError(e.code)) need_reload_from_zk = true;
      ...
      if (throw_on_error) exc->rethrow();

      LOG_ERROR(log, "ZooKeeper error when loading config from '{}': {}", config_path,
                getExceptionMessageForLogging(*exc, /*with_stacktrace=*/false, /*check_embedded_stacktrace=*/false));
      return std::nullopt;
    } catch (...) {
      const auto message = getCurrentExceptionMessageAndPattern(/*with_stacktrace=*/true);
      auto exc = std::make_unique<Exception>(message, ErrorCodes::CANNOT_LOAD_CONFIG);
      ...
      LOG_ERROR(log, "Error loading config from '{}': {}", config_path,
                getExceptionMessageForLogging(*exc, /*with_stacktrace=*/false, /*check_embedded_stacktrace=*/false));
      return std::nullopt;
    }
    ...
    try {
      ...
      LOG_ERROR(log, "Error updating configuration from '{}': {}", config_path,
                getExceptionMessageForLogging(*exc, /*with_stacktrace=*/false, /*check_embedded_stacktrace=*/false));
      return std::nullopt;
    }
...
struct ConfigReloader::FileWithTimestamp {
  ...
  explicit FileWithTimestamp(const std::string& path_) : path(path_), modification_time(fs::last_write_time(path_)) {}
...
void ConfigReloader::FilesChangesTracker::addIfExists(const std::string& path_to_add) {
  if (!path_to_add.empty() && fs::exists(path_to_add)) files.emplace(path_to_add);
}
...
ConfigReloader::FilesChangesTracker ConfigReloader::getNewFileList() const {
  ...
  for (const std::string& path : extra_paths) file_list.addIfExists(path);

  for (const auto& merge_path : ConfigProcessor::getConfigMergeFiles(config_path)) file_list.addIfExists(merge_path);
