...
class ConfigReloader {
 public:
  ...
  ConfigReloader(std::string_view path_, const std::vector<std::string>& extra_paths_, const std::string& preprocessed_dir,
                 std::unique_ptr<zkutil::ZooKeeperNodeCache>&& zk_node_cache_, const Coordination::EventPtr& zk_changed_event,
                 Updater&& updater);
  ...
  void reload() {
    reloadIfNewer(/* force */ true, /* throw_on_error */ true, /* fallback_to_preprocessed */ false, /* initial_loading = */ false);
  }

 private:
  void run();

  std::optional<ConfigProcessor::LoadedConfig> reloadIfNewer(bool force, bool throw_on_error, bool fallback_to_preprocessed,
