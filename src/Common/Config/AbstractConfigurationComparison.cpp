...
String concatKeyAndSubKey(const String& key, const String& subkey) {
  ...
  String result = key;
  if (!result.empty() && !subkey.empty() && subkey[0] != '[') result += '.';
  result += subkey;
  return result;
}
...
bool isSameConfigurationImpl(const Poco::Util::AbstractConfiguration& left, const String& left_key,
  ...
  if (&left == &right && left_key == right_key) return true;
  ...
  if (ignore_keys) {
    ...
    for (const auto& ignore_key : *ignore_keys) chassert(!ignore_key.contains('.'));
#endif
  }

  /// Check that the right configuration has the same set of subkeys as the left configuration.
  if (left_subkeys.size() != right_subkeys.size()) return false;
  ...
  for (const auto& subkey : right_subkeys) {
    if (!left_subkeys_set.contains(subkey)) return false;
...
bool isSameConfigurationWithMultipleKeys(const Poco::Util::AbstractConfiguration& left, const Poco::Util::AbstractConfiguration& right,
  ...
  if (left_multiple_keys.size() != right_multiple_keys.size()) return false;

  for (auto& key : left_multiple_keys)
    if (!isSameConfiguration(left, right, concatKeyAndSubKey(root, key))) return false;
