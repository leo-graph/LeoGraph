...
class PoolBase : private boost::noncopyable {
 ...
 public:
  ...
  class Entry {
   public:
     ...
     */
    void expire() { data->data.is_expired = true; }

    bool isNull() const { return data == nullptr; }

    PoolBase* getPool() const {
      if (!data) throw DB::Exception(DB::ErrorCodes::LOGICAL_ERROR, "Attempt to get pool from uninitialized entry");
  ...
  void reserve(size_t count) {
    ...
    while (items.size() < count) items.emplace_back(std::make_shared<PooledObject>(allocObject(), *this));
  }
 ...
 protected:
  ...
  PoolBase(unsigned max_items_, LoggerPtr log_) : max_items(max_items_), log(log_) { items.reserve(max_items); }
