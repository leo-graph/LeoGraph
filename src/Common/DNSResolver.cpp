...
void splitHostAndPort(const std::string& host_and_port, std::string& out_host, UInt16& out_port) {
  ...
  {
    ++it;
    while (it != end && *it != ']') out_host += *it++;
    if (it == end) throw Exception(ErrorCodes::BAD_ARGUMENTS, "Malformed IPv6 address");
    ++it;
...
DNSResolver::IPAddresses resolveIPAddressImpl(const std::string& host) {
  ...
  if (host.starts_with('[')) {
    ...
    if (Poco::Net::IPAddress::tryParse(host.substr(1, host.size() - 2), ip)) return DNSResolver::IPAddresses(1, ip);
  } else {
    if (Poco::Net::IPAddress::tryParse(host, ip)) return DNSResolver::IPAddresses(1, ip);
  }
...
Poco::Net::IPAddress pickAddress(const DNSResolver::IPAddresses& addresses) { return addresses.front(); }
...
struct DNSResolver::Impl {
  ...
  CacheBase<Poco::Net::IPAddress, std::unordered_set<std::string>> cache_address{CurrentMetrics::DNSAddressesCacheBytes,
                                                                                 CurrentMetrics::DNSAddressesCacheSize, 1024};
  ...
  void performAddressFiltering(DNSResolver::IPAddresses& addresses) const {
    ...
    if (!dns_resolve_ipv4 && !dns_resolve_ipv6) {
      throw Exception(
          ...
          "DNS can't resolve any address, because dns_allow_resolve_names_to_ipv6 and dns_allow_resolve_names_to_ipv4 both are disabled");
    }
...
DNSResolver::IPAddresses DNSResolver::resolveIPAddressWithCache(const std::string& host) {
  auto [result, _] = impl->cache_host.getOrSet(host, [&host, this]() {
    return std::make_shared<DNSResolver::CacheEntry>(getResolvedIPAddressesWithFiltering(host), std::chrono::system_clock::now());
  });
  return result->addresses;
}

std::unordered_set<String> DNSResolver::reverseResolveWithCache(const Poco::Net::IPAddress& address) {
  auto [result, _] = impl->cache_address.getOrSet(
      address, [&address]() { return std::make_shared<std::unordered_set<String>>(reverseResolveImpl(address)); });
  return *result;
}
...
bool DNSResolver::getFilterIPv4() const { return addressFilter->settings.get()->dns_allow_resolve_names_to_ipv4; }

bool DNSResolver::getFilterIPv6() const { return addressFilter->settings.get()->dns_allow_resolve_names_to_ipv6; }

DNSResolver::IPAddresses DNSResolver::resolveHostAllInOriginOrder(const std::string& host) {
  if (impl->disable_cache) return getResolvedIPAddressesWithFiltering(host);
...
Poco::Net::SocketAddress DNSResolver::resolveAddress(const std::string& host_and_port) {
  ...
  if (impl->disable_cache) return Poco::Net::SocketAddress(pickAddress(getResolvedIPAddressesWithFiltering(host)), port);
...
std::vector<Poco::Net::SocketAddress> DNSResolver::resolveAddressList(const std::string& host, UInt16 port) {
  ...
  for (auto& ip_addresse : ip_addresses) socket_addresses.emplace_back(ip_addresse, port);

  return socket_addresses;
}

std::unordered_set<String> DNSResolver::reverseResolve(const Poco::Net::IPAddress& address) {
  if (impl->disable_cache) return reverseResolveImpl(address);
...
void DNSResolver::removeHostFromCache(const std::string& host) { impl->cache_host.remove(host); }

void DNSResolver::setDisableCacheFlag(bool is_disabled) { impl->disable_cache = is_disabled; }
...
String DNSResolver::getHostName() {
  if (impl->disable_cache) return Poco::Net::DNS::hostName();

  std::lock_guard lock(impl->drop_mutex);

  if (!impl->host_name.has_value()) impl->host_name.emplace(Poco::Net::DNS::hostName());
...
void DNSResolver::updateCacheImpl(UpdateF&& update_func,  // NOLINT(cppcoreguidelines-missing-std-forward)
                                  ElemsT&& elems,         // NOLINT(cppcoreguidelines-missing-std-forward)
                                  UInt32 max_consecutive_failures, FormatStringHelper<String> notfound_log_msg,
  ...
  for (auto it = elems.begin(); it != elems.end(); it++) {
    try {
      ...
      if (!lost_elems.empty()) lost_elems += ", ";
      lost_elems += cacheElemToString(it->first);
      if (max_consecutive_failures) {
        it->second++;
        if (it->second >= max_consecutive_failures) elements_to_drop.emplace_back(it);
      }
    } catch (...) {
      tryLogCurrentException(log, __PRETTY_FUNCTION__);
    }
  ...
  if (!lost_elems.empty()) LOG_INFO(log, notfound_log_msg.format(std::move(lost_elems)));
  if (elements_to_drop.size()) {
    String deleted_elements;
    for (auto it : elements_to_drop) {
      if (!deleted_elements.empty()) deleted_elements += ", ";
...
void DNSResolver::updateCache(UInt32 max_consecutive_failures) {
...

  {
    ...
    for (const auto& host : impl->new_hosts) impl->known_hosts.insert(host);
    impl->new_hosts.clear();

    for (const auto& address : impl->new_addresses) impl->known_addresses.insert(address);
    impl->new_addresses.clear();
  }
  ...
  updateCacheImpl(&DNSResolver::updateHost, impl->known_hosts, max_consecutive_failures, "Cached hosts not found: {}",
                  "Cached hosts dropped: {}");
  updateCacheImpl(&DNSResolver::updateAddress, impl->known_addresses, max_consecutive_failures, "Cached addresses not found: {}",
