...
void test(size_t n, const char *name, F &&kernel) {
  ...
  std::cerr << std::fixed << std::setprecision(2) << n << " ops in " << watch.elapsedSeconds() << " sec., "
...
int main(int argc, char **argv) {
  ...
  test(n, "pthread_create, pthread_join each iteration", [] {
    pthread_t thread;
    if (pthread_create(&thread, nullptr, g, nullptr)) throw DB::ErrnoException(DB::ErrorCodes::PTHREAD_ERROR, "Cannot create thread");
    if (pthread_join(thread, nullptr)) throw DB::ErrnoException(DB::ErrorCodes::PTHREAD_ERROR, "Cannot join thread");
  });
