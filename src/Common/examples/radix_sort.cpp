...
static void NO_INLINE sort1(Key* data, size_t size) { std::sort(data, data + size); }

static void NO_INLINE sort2(Key* data, size_t size) { radixSortLSD(data, size); }
...
static void NO_INLINE sort4(Key* data, size_t size) { radixSortMSD(data, size, size); }

static void NO_INLINE sort5(Key* data, size_t size) { pdqsort(data, data + size); }

static void NO_INLINE sort6(Key* data, size_t size, size_t limit) { std::partial_sort(data, data + limit, data + size); }
...
static void NO_INLINE sort8(Key* data, size_t size, size_t limit) { radixSortMSD(data, size, limit); }
...
int main(int argc, char** argv) {
  ...
  if (argc == 4) limit = DB::parse<size_t>(argv[3]);
  ...
  if (n <= 100) {
    std::cerr << std::endl;
    for (const auto& elem : data) std::cerr << elem << ' ';
    std::cerr << std::endl;
  }

  {
    ...
    watch.stop();
    double elapsed = watch.elapsedSeconds();
    std::cerr << "Sorted in " << elapsed << " (" << static_cast<double>(n) / elapsed << " elem/sec., "
              << static_cast<double>(n) * sizeof(Key) / elapsed / 1048576 << " MB/sec.)" << std::endl;
  }
...

  {
    ...
    double elapsed = watch.elapsedSeconds();
    if (!ok)
      std::cerr << "Checked in " << elapsed << " (" << static_cast<double>(limit) / elapsed << " elem/sec., "
                << static_cast<double>(limit) * sizeof(Key) / elapsed / 1048576 << " MB/sec.)" << std::endl
