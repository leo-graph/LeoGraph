...
static std::string createTmpPath(const std::string &filename) {
  ...
  if (dir == nullptr) throw std::runtime_error("Could not create directory");
...
struct Test {
  static void perform() {
    ...
    try {
      ...
      for (size_t i = 0; i < bucket_count; ++i) store[i] = Generator::execute(i, width);
...

      {
        ...
        while (reader.next()) {
          ...
          if (data.second != store[data.first]) throw std::runtime_error("Found discrepancy");
        }
      }
    ...
    if (ok) std::cout << "Test width=" << width << " bucket_count=" << bucket_count << " passed\n";
  }
};
...
struct Generator1 {
  static UInt8 execute(size_t, size_t width) { return static_cast<UInt8>((1 << width) - 1); }
};

struct Generator2 {
  static UInt8 execute(size_t i, size_t width) { return static_cast<UInt8>((i >> 1) & ((1 << width) - 1)); }
};

struct Generator3 {
  static UInt8 execute(size_t i, size_t width) { return static_cast<UInt8>((i * 17 + 31) % (1ULL << width)); }
};
