 ...
 *
 * for i in {1..1000}; do ./sip_hash_perf < titles.tsv 2>&1 | grep Processed | grep -oP '\d+\.\d+ rows/sec'; done | awk '{ if ($1 > x) { x =
 * $1; print x } }'
 */
...
int main(int, char **) {
...

  {
    ...
    std::cerr << "Read " << data.size() << " rows, " << (static_cast<double>(in.count()) / 1048576.0) << " MiB "
              << " in " << seconds << " sec., " << (static_cast<double>(data.size()) / seconds) << " rows/sec., "
...

  {
    ...
    double seconds = watch.elapsedSeconds();
    std::cerr << "Processed " << data.size() << " rows, " << (static_cast<double>(in.count()) / 1048576.0) << " MiB "
              << " in " << seconds << " sec., " << (static_cast<double>(data.size()) / seconds) << " rows/sec., "
              << (static_cast<double>(in.count()) / 1048576.0 / seconds) << " MiB/sec. "
              << "(res = " << res << ")\n";
  }
