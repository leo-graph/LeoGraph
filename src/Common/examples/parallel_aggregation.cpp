...
/// $file ----------------------\n"; for method in 22 506 507; do ./src/Common/examples/parallel_aggregation 90000000 64 $method <
/// src/Common/examples/${file}.bin; done; done

using ThreadFromGlobalPoolSimple =
    ThreadFromGlobalPoolImpl</* propagate_opentelemetry_context= */ false, /* global_trace_collector_allowed= */ false>;
...
class HashMapWithSmallLocks {
 ...
 public:
  HashMapWithSmallLocks() : size_degree(DEFAULT_SIZE_DEGREE) { buf.resize(1ULL << size_degree); }

  explicit HashMapWithSmallLocks(size_t size_degree_) : size_degree(size_degree_) { buf.resize(1ULL << size_degree); }
  ...
  bool tryEmplace(const Key& key, Cell*& found, bool& inserted) {
    ...
    for (size_t i = 0; i < (1ULL << size_degree); ++i) {
      ...
      if (!cell.lock.tryLock()) return false;  /// Couldn't get lock, caller should handle locally
...
class RobinHoodHashMap {
 ...
 public:
  RobinHoodHashMap() : size_degree(INITIAL_SIZE_DEGREE) { buf.resize(1ULL << size_degree); }

  explicit RobinHoodHashMap(size_t initial_size_degree) : size_degree(initial_size_degree) { buf.resize(1ULL << size_degree); }
  ...
  Cell* find(const Key& key) {
    ...
    while (dist <= MAX_PROBE_DISTANCE + m_size)  /// Safety bound
    {
      ...
      if (cell.probe_distance < dist && dist <= MAX_PROBE_DISTANCE) return nullptr;  /// Key would have been here if it existed
  ...
  class iterator {
   ...
   public:
    iterator(Cell* p, Cell* e) : ptr(p), end_ptr(e) {
      while (ptr < end_ptr && ptr->isEmpty()) ++ptr;
    }
    iterator& operator++() {
      ++ptr;
      while (ptr < end_ptr && ptr->isEmpty()) ++ptr;
 ...
 public:
  ...
  void resize() {
    ...
    for (size_t i = 0; i < old_capacity; ++i) {
      if (old_ctrl[i] >= 0)  /// Not empty or deleted
      {
        ...
        if (inserted) result->value = old_slots[i].value;
      }
    }
  }
  ...
  Slot* find(const Key& key) {
    ...
    while (probes < capacity) {
      ...
      if (matchEmpty(group)) return nullptr;
  ...
  void emplace(const Key& key, Slot*& result, bool& inserted) {
    ...
    while (probes < capacity) {
      ...
      if (empty_mask) {
        ...
        if (idx < GROUP_SIZE) ctrl[capacity + idx] = h2;
        slots[idx].key = key;
  ...
  class iterator {
   ...
   public:
    iterator(const int8_t* c, const int8_t* ce, Slot* s, size_t /* cap */) : ctrl_ptr(c), ctrl_end(ce), slot_base(s), idx(0) {
    ...
    iterator& operator++() {
      ++idx;
      while (ctrl_ptr + idx < ctrl_end && ctrl_ptr[idx] < 0) ++idx;
      return *this;
    }
    bool operator!=(const iterator& other) const { return (ctrl_ptr + idx) != (other.ctrl_ptr + other.idx); }
  ...
  iterator begin() { return iterator(ctrl.data(), ctrl.data() + capacity, slots.data(), capacity); }
  iterator end() { return iterator(ctrl.data() + capacity); }
};
#endif
...
class TwoLevelRobinHoodHashMap {
 ...
 public:
  TwoLevelRobinHoodHashMap() {
    ...
    for (size_t i = 0; i < NUM_BUCKETS; ++i) impls.emplace_back(BUCKET_SIZE_DEGREE);
  }
  ...
  size_t size() const {
    size_t result = 0;
    for (size_t i = 0; i < NUM_BUCKETS; ++i) result += impls[i].size();
    return result;
  }
...
void aggregate1(Map& map, Source::const_iterator begin, Source::const_iterator end) {
  for (auto it = begin; it != end; ++it) ++map[*it];
}
...
void aggregate1Prefetch(Map& map, Source::const_iterator begin, Source::const_iterator end) {
  ...
  for (size_t i = 0; i < PREFETCH_LOOKAHEAD && it + i < end; ++i) map.prefetch(*(it + i));
...
void aggregate1BatchPrefetch(Map& map, Source::const_iterator begin, Source::const_iterator end) {
  ...
  while (it + BATCH_SIZE <= end) {
    ...
    for (size_t i = 0; i < BATCH_SIZE; ++i) map.prefetch(*(it + i));

    /// Then process all elements in the batch
    for (size_t i = 0; i < BATCH_SIZE; ++i) ++map[*(it + i)];
...
void merge2Prefetch(MapTwoLevel* maps, size_t num_threads, size_t bucket) {
  ...
  for (size_t i = 1; i < num_threads; ++i) {
    ...
    /// Prefetch initial elements
    auto prefetch_it = it;
    for (size_t j = 0; j < PREFETCH_LOOKAHEAD && prefetch_it != end; ++j, ++prefetch_it) dst_map.prefetch(prefetch_it->getKey());
    ...
    /// Process remaining elements
    for (; it != end; ++it) dst_map[it->getKey()] += it->getMapped();
  }
}
...
void aggregate33(Map& local_map, Map& global_map, Mutex& mutex, Source::const_iterator begin, Source::const_iterator end) {
  ...
  for (auto it = begin; it != end; ++it) {
    ...
    if (inserted && local_map.size() == threshold) {
      ...
      for (auto& value_type : local_map) global_map[value_type.getKey()] += value_type.getMapped();
...
void aggregate4(Map& local_map, MapTwoLevel& global_map, Mutex* mutexes, Source::const_iterator begin, Source::const_iterator end) {
  ...
  while (it != end) {
    ...
    if (local_map.size() < threshold) {
      for (; it != block_end; ++it) ++local_map[*it];
    } else {
      for (; it != block_end; ++it) {
        auto* found = local_map.find(*it);
...
void aggregate6(MapSmallLocks& global_map, Source::const_iterator begin, Source::const_iterator end) {
  for (auto it = begin; it != end; ++it) global_map.increment(*it);
}
...
int main(int argc, char** argv) {
...

  {
    ...
    watch.stop();
    std::cerr << "Vector. Size: " << n << ", elapsed: " << watch.elapsedSeconds() << " (" << static_cast<double>(n) / watch.elapsedSeconds()
              << " elem/sec.)" << std::endl
              << std::endl;
  }
  ...
  if (!method || method == 1) {
    ...
    for (size_t i = 0; i < num_threads; ++i)
      pool.scheduleOrThrowOnError([&, i] {
        aggregate1(maps[i], data.begin() + (data.size() * i) / num_threads, data.begin() + (data.size() * (i + 1)) / num_threads);
      });
    ...
    std::cerr << "Aggregated in " << time_aggregated << " (" << static_cast<double>(n) / time_aggregated << " elem/sec.)" << std::endl;
    ...
    for (size_t i = 1; i < num_threads; ++i)
      for (auto it = maps[i].begin(); it != maps[i].end(); ++it) maps[0][it->getKey()] += it->getMapped();

    watch.stop();
    double time_merged = watch.elapsedSeconds();
    std::cerr << "Merged in " << time_merged << " (" << static_cast<double>(size_before_merge) / time_merged << " elem/sec.)" << std::endl;

    double time_total = time_aggregated + time_merged;
    std::cerr << "Total in \033[1m" << time_total << "\033[0m"
              << " (" << static_cast<double>(n) / time_total << " elem/sec.)" << std::endl;
  ...
  if (!method || method == 12) {
    ...
    for (size_t i = 0; i < num_threads; ++i)
      pool.scheduleOrThrowOnError([&, i] {
        aggregate12(maps[i], data.begin() + (data.size() * i) / num_threads, data.begin() + (data.size() * (i + 1)) / num_threads);
      });
  ...
  if (!method || method == 2) {
    ...
    for (size_t i = 0; i < num_threads; ++i)
      pool.scheduleOrThrowOnError([&, i] {
        aggregate2(maps[i], data.begin() + (data.size() * i) / num_threads, data.begin() + (data.size() * (i + 1)) / num_threads);
      });
    ...
    for (unsigned i = 0; i < MapTwoLevel::NUM_BUCKETS; ++i) pool.scheduleOrThrowOnError([&, i] { merge2(maps.data(), num_threads, i); });
  ...
  if (!method || method == 22) {
    ...
    for (size_t i = 0; i < num_threads; ++i)
      pool.scheduleOrThrowOnError([&, i] {
        aggregate22(maps[i], data.begin() + (data.size() * i) / num_threads, data.begin() + (data.size() * (i + 1)) / num_threads);
      });
  ...
  if (!method || method == 3) {
    ...
    for (size_t i = 0; i < num_threads; ++i)
      pool.scheduleOrThrowOnError([&, i] {
        aggregate3(local_maps[i], global_map, mutex, data.begin() + (data.size() * i) / num_threads,
                   data.begin() + (data.size() * (i + 1)) / num_threads);
      });
    ...
    for (size_t i = 0; i < num_threads; ++i)
      for (auto it = local_maps[i].begin(); it != local_maps[i].end(); ++it) global_map[it->getKey()] += it->getMapped();
  ...
  if (!method || method == 33) {
    ...
    for (size_t i = 0; i < num_threads; ++i)
      pool.scheduleOrThrowOnError([&, i] {
        aggregate33(local_maps[i], global_map, mutex, data.begin() + (data.size() * i) / num_threads,
                    data.begin() + (data.size() * (i + 1)) / num_threads);
      });
  ...
  if (!method || method == 4) {
    ...
    for (size_t i = 0; i < num_threads; ++i)
      pool.scheduleOrThrowOnError([&, i] {
        aggregate4(local_maps[i], global_map, mutexes.data(), data.begin() + (data.size() * i) / num_threads,
                   data.begin() + (data.size() * (i + 1)) / num_threads);
      });
  ...
  if (!method || method == 100) {
    ...
    for (size_t i = 0; i < num_threads; ++i)
      pool.scheduleOrThrowOnError([&, i] {
        aggregate1Prefetch(maps[i], data.begin() + (data.size() * i) / num_threads, data.begin() + (data.size() * (i + 1)) / num_threads);
      });
  ...
  if (!method || method == 101) {
    ...
    for (size_t i = 0; i < num_threads; ++i)
      pool.scheduleOrThrowOnError([&, i] {
        aggregate1BatchPrefetch(maps[i], data.begin() + (data.size() * i) / num_threads,
                                data.begin() + (data.size() * (i + 1)) / num_threads);
      });
  ...
  if (!method || method == 200) {
    ...
    for (size_t i = 0; i < num_threads; ++i)
      pool.scheduleOrThrowOnError([&, i] {
        aggregate2Prefetch(maps[i], data.begin() + (data.size() * i) / num_threads, data.begin() + (data.size() * (i + 1)) / num_threads);
      });
  ...
  if (!method || method == 201) {
    ...
    for (size_t i = 0; i < num_threads; ++i)
      pool.scheduleOrThrowOnError([&, i] {
        aggregate2BatchPrefetch(maps[i], data.begin() + (data.size() * i) / num_threads,
                                data.begin() + (data.size() * (i + 1)) / num_threads);
      });
  ...
  if (!method || method == 203) {
    ...
    for (size_t i = 0; i < num_threads; ++i)
      pool.scheduleOrThrowOnError([&, i] {
        aggregate2PrefetchSeq(maps[i], data.begin() + (data.size() * i) / num_threads,
                              data.begin() + (data.size() * (i + 1)) / num_threads);
      });
  ...
  if (!method || method == 5) {
    ...
    for (size_t i = 0; i < num_threads; ++i)
      pool.scheduleOrThrowOnError([&, i] {
        aggregate5(local_maps[i], global_map, data.begin() + (data.size() * i) / num_threads,
                   data.begin() + (data.size() * (i + 1)) / num_threads);
      });
  ...
  if (!method || method == 6) {
    ...
    for (size_t i = 0; i < num_threads; ++i)
      pool.scheduleOrThrowOnError([&, i] {
        aggregate6(global_map, data.begin() + (data.size() * i) / num_threads, data.begin() + (data.size() * (i + 1)) / num_threads);
      });
  ...
  if (!method || method == 7) {
    ...
    for (size_t i = 0; i < num_threads; ++i)
      pool.scheduleOrThrowOnError([&, i] {
        aggregate7(local_maps[i], global_map, data.begin() + (data.size() * i) / num_threads,
                   data.begin() + (data.size() * (i + 1)) / num_threads);
      });
  ...
  if (!method || method == 300) {
    ...
    for (size_t i = 0; i < num_threads; ++i)
      pool.scheduleOrThrowOnError([&, i] {
        aggregateRobinHood(maps[i], data.begin() + (data.size() * i) / num_threads, data.begin() + (data.size() * (i + 1)) / num_threads);
      });
  ...
  if (!method || method == 301) {
    ...
    for (size_t i = 0; i < num_threads; ++i)
      pool.scheduleOrThrowOnError([&, i] {
        aggregateRobinHoodPrefetch(maps[i], data.begin() + (data.size() * i) / num_threads,
                                   data.begin() + (data.size() * (i + 1)) / num_threads);
      });
  ...
  if (!method || method == 400) {
    ...
    for (size_t i = 0; i < num_threads; ++i)
      pool.scheduleOrThrowOnError([&, i] {
        aggregateSwiss(maps[i], data.begin() + (data.size() * i) / num_threads, data.begin() + (data.size() * (i + 1)) / num_threads);
      });
  ...
  if (!method || method == 401) {
    ...
    for (size_t i = 0; i < num_threads; ++i)
      pool.scheduleOrThrowOnError([&, i] {
        aggregateSwissPrefetch(maps[i], data.begin() + (data.size() * i) / num_threads,
                               data.begin() + (data.size() * (i + 1)) / num_threads);
      });
  ...
  if (!method || method == 302) {
    ...
    for (size_t i = 0; i < num_threads; ++i)
      pool.scheduleOrThrowOnError([&, i] {
        aggregateTwoLevelRobinHood(maps[i], data.begin() + (data.size() * i) / num_threads,
                                   data.begin() + (data.size() * (i + 1)) / num_threads);
      });
  ...
  if (!method || method == 303) {
    ...
    for (size_t i = 0; i < num_threads; ++i)
      pool.scheduleOrThrowOnError([&, i] {
        aggregateTwoLevelRobinHoodPrefetch(maps[i], data.begin() + (data.size() * i) / num_threads,
                                           data.begin() + (data.size() * (i + 1)) / num_threads);
      });
  ...
  if (!method || method == 402) {
    ...
    for (size_t i = 0; i < num_threads; ++i)
      pool.scheduleOrThrowOnError([&, i] {
        aggregateTwoLevelSwiss(maps[i], data.begin() + (data.size() * i) / num_threads,
                               data.begin() + (data.size() * (i + 1)) / num_threads);
      });
  ...
  if (!method || method == 403) {
    ...
    for (size_t i = 0; i < num_threads; ++i)
      pool.scheduleOrThrowOnError([&, i] {
        aggregateTwoLevelSwissPrefetch(maps[i], data.begin() + (data.size() * i) / num_threads,
                                       data.begin() + (data.size() * (i + 1)) / num_threads);
      });
  ...
  if (!method || method == 500) {
    ...
    for (auto& p : partitions) p.reserve(data.size() / num_threads * 2);  /// Reserve with some slack
    ...
    std::cerr << "Partitioned in " << time_partitioned << " (" << static_cast<double>(n) / time_partitioned << " elem/sec.)" << std::endl;

    std::cerr << "Partition sizes: ";
    for (size_t i = 0; i < num_threads; ++i) std::cerr << (i == 0 ? "" : ", ") << partitions[i].size();
    std::cerr << std::endl;
    ...
    for (size_t i = 0; i < num_threads; ++i)
      pool.scheduleOrThrowOnError([&, i] {
        auto& map = maps[i];
        for (const auto& key : partitions[i]) ++map[key];
      });
    ...
    size_t total_size = 0;
    for (size_t i = 0; i < num_threads; ++i) total_size += maps[i].size();
  ...
  if (!method || method == 501) {
    ...
    for (auto& tp : thread_partitions) {
      ...
      for (auto& p : tp) p.reserve(data.size() / (num_threads * num_threads) * 2);
    }
    ...
    for (size_t i = 0; i < num_threads; ++i)
      pool.scheduleOrThrowOnError([&, i] {
        ...
        for (size_t t = 0; t < num_threads; ++t) {
          for (const auto& key : thread_partitions[t][i]) ++map[key];
        }
      });
  ...
  if (!method || method == 502) {
    ...
    for (size_t i = 0; i < num_threads; ++i)
      pool.scheduleOrThrowOnError([&, i] {
        ...
        size_t total = 0;
        for (size_t t = 0; t < num_threads; ++t) total += thread_partitions[t][i].size();
        ...
        for (size_t j = 0; j < PREFETCH_LOOKAHEAD && it + j < end; ++j) map.prefetch(*(it + j));
  ...
  if (!method || method == 503) {
    ...
    for (size_t i = 0; i < num_threads; ++i) {
      ...
      for (size_t p = 0; p < num_threads; ++p) thread_partitions[i][p].reserve(histograms[i][p]);
    }
  ...
  if (!method || method == 504) {
    ...
    struct LocalMap {
      ...
      bool tryInsertOrIncrement(Key key, size_t hash) {
        ...
        for (size_t probe = 0; probe < 16; ++probe)  /// Limited probing
        {
          ...
          if (!entry.occupied) {
            if (size >= LOCAL_MAP_MAX_SIZE) return false;  /// Table full, defer this key
            entry.key = key;
            entry.value = 1;
            entry.occupied = true;
            ++size;
    ...
    for (auto& d : deferred) {
      ...
      for (auto& p : d) p.reserve(data.size() / (num_threads * num_threads));
    }
    ...
    for (size_t i = 0; i < num_threads; ++i) {
      ...
      for (size_t p = 0; p < num_threads; ++p) total_deferred += deferred[i][p].size();
    }

    std::cerr << "Phase 1 (local + defer) in " << time_phase1 << " (" << static_cast<double>(n) / time_phase1 << " elem/sec.)"
              << " local_keys=" << total_local << " deferred_keys=" << total_deferred << std::endl;
    ...
    for (size_t i = 0; i < num_threads; ++i)
      pool.scheduleOrThrowOnError([&, i] {
        ...
        for (size_t t = 0; t < num_threads; ++t) {
          for (const auto& key : deferred[t][i]) ++map[key];
        }
      });
    ...
    std::cerr << "Phase 2 (aggregate deferred) in " << time_phase2 << " (" << static_cast<double>(total_deferred) / time_phase2
              << " elem/sec.)" << std::endl;
    ...
    std::cerr << "Phase 3 (merge local to partitioned) in " << time_phase3 << " (" << static_cast<double>(total_local) / time_phase3
              << " elem/sec.)" << std::endl;

    size_t total_size = 0;
    for (size_t i = 0; i < num_threads; ++i) total_size += partitioned_maps[i].size();
    ...
    struct LocalMap {
      ...
      bool tryInsertOrIncrement(Key key, size_t hash) {
        ...
        for (size_t probe = 0; probe < 16; ++probe) {
          auto& entry = entries[pos];
          if (!entry.occupied) {
            if (size >= LOCAL_MAP_MAX_SIZE) return false;
            entry.key = key;
            entry.value = 1;
            entry.occupied = true;
            ++size;
      ...
      for (size_t t = 0; t < num_threads; ++t) {
        ...
        for (const auto& entry : local_map.entries) {
          if (entry.occupied) {
            UInt32 part_hash = static_cast<UInt32>(intHashCRC32(entry.key));
            size_t partition = (static_cast<UInt64>(part_hash) * num_threads) >> 32;
            if (partition == p) partitioned_maps[p][entry.key] += entry.value;
          }
        }
      }
    }
  ...
  if (!method || method == 506) {
    ...
    for (auto& lp : local_partitioned) lp.resize(num_threads);
    ...
    std::cerr << "Phase 1 (local + defer + partition) in " << time_phase1 << " (" << static_cast<double>(n) / time_phase1 << " elem/sec.)"
              << " local_keys=" << total_local << " deferred_keys=" << total_deferred << std::endl;
    ...
    for (size_t i = 0; i < num_threads; ++i)
      pool.scheduleOrThrowOnError([&, i] {
        ...
        for (size_t t = 0; t < num_threads; ++t) {
          for (const auto& [key, value] : local_partitioned[t][i]) map[key] += value;
        }
      });
    ...
    watch.stop();
    double time_phase2 = watch.elapsedSeconds();
    std::cerr << "Phase 2+3 (aggregate all) in " << time_phase2 << " (" << static_cast<double>(total_deferred + total_local) / time_phase2
              << " elem/sec.)" << std::endl;
  ...
  if (!method || method == 508) {
    ...
    std::vector<std::vector<std::vector<Key>>> deferred(num_threads);
    for (auto& d : deferred) d.resize(num_threads);
    ...
    for (size_t block = 0; block < num_blocks; ++block) {
      ...
      for (size_t i = 0; i < num_threads; ++i)
        pool.scheduleOrThrowOnError([&, i, block_start, block_end] {
          ...
          for (auto& d : my_deferred) d.clear();
    ...
    std::cerr << "Processed " << total_blocks << " blocks in " << time_blocks << " (" << static_cast<double>(n) / time_blocks
              << " elem/sec.)" << std::endl;

    /// Count local map entries
    size_t total_local = 0;
    for (size_t i = 0; i < num_threads; ++i) total_local += local_maps[i].size;
    ...
    std::cerr << "Final merge in " << time_merge << " (" << static_cast<double>(total_local) / time_merge << " elem/sec.)" << std::endl;
  ...
  if (!method || method == 509) {
      ...
      for (size_t i = 0; i < num_threads; ++i)
        pool.scheduleOrThrowOnError([&, i] {
          ...
          for (size_t t = 0; t < num_threads; ++t) {
            ...
            for (size_t j = 0; j < PREFETCH_LOOKAHEAD && j < sz; ++j) map.prefetch(keys[j]);

            for (size_t j = 0; j < sz; ++j) {
              if (j + PREFETCH_LOOKAHEAD < sz) map.prefetch(keys[j + PREFETCH_LOOKAHEAD]);
              ++map[keys[j]];
            }
          }
        });
    ...
    std::cerr << "Processed " << num_blocks << " blocks in " << time_blocks << " (" << static_cast<double>(n) / time_blocks << " elem/sec.)"
              << std::endl;
  ...
  if (!method || method == 510) {
      ...
      pool.scheduleOrThrowOnError([&, i] {
        ...
        for (size_t idx = start; idx < end; ++idx) {
          ...
          if (!local_map.tryInsertOrIncrement(key, hash)) {
            ...
            if (deferred_buffer[partition].size() >= BLOCK_SIZE) {
              ...
              for (const auto& k : deferred_buffer[partition]) ++map[k];
        ...
        for (size_t p = 0; p < num_threads; ++p) {
          if (!deferred_buffer[p].empty()) {
            ...
            for (const auto& k : deferred_buffer[p]) ++map[k];
          }
        }
      });
    ...
    std::cerr << "Main phase in " << time_main << " (" << static_cast<double>(n) / time_main << " elem/sec.)" << std::endl;
  ...
  if (!method || method == 512) {
    ...
    for (auto& row : deferred) row.resize(num_threads);
      ...
      pool.scheduleOrThrowOnError([&, i] {
        ...
        for (size_t idx = start; idx < end; ++idx) {
          ...
          if (!local_map.tryInsertOrIncrement(key, hash)) {
            ...
            if (local_buffer[partition].size() >= BUFFER_FLUSH_SIZE) {
              ...
              my_deferred[partition].keys.insert(my_deferred[partition].keys.end(), local_buffer[partition].begin(),
        ...
        for (size_t p = 0; p < num_threads; ++p) {
          if (!local_buffer[p].empty()) {
            ...
            my_deferred[p].keys.insert(my_deferred[p].keys.end(), local_buffer[p].begin(), local_buffer[p].end());
          }
        }
      });
    ...
    std::cerr << "Production phase in " << time_produce << " (" << static_cast<double>(n) / time_produce << " elem/sec.)" << std::endl;
        ...
        while (buffers_processed < num_threads) {
          for (size_t t = 0; t < num_threads; ++t) {
            ...
            if (buf.mutex->try_lock()) {
              for (const auto& key : buf.keys) ++map[key];
              buf.keys.clear();
              buf.mutex->unlock();
              processed[t] = true;
              ++buffers_processed;
    ...
    std::cerr << "Consume phase in " << time_consume << " (" << static_cast<double>(n) / time_consume << " elem/sec.)" << std::endl;
  ...
  if (!method || method == 513) {
    ...
    for (size_t i = 0; i < num_threads; ++i)
      pool.scheduleOrThrowOnError([&, i] {
        ...
        for (size_t idx = start; idx < end; ++idx) {
          ...
          if ((idx & 0xFFF) == 0)  /// Every 4K elements
          {
            for (size_t attempts = 0; attempts < 2; ++attempts) {
              ...
              if (buf.mutex->try_lock()) {
                if (!buf.keys.empty()) {
                  for (const auto& k : buf.keys) ++map[k];
                  buf.keys.clear();
                }
                buf.mutex->unlock();
              }
        ...
        while (producers_done.load() < num_threads) std::this_thread::yield();
    ...
    for (size_t i = 0; i < num_threads; ++i)
      pool.scheduleOrThrowOnError([&, i] {
        ...
        while (buffers_processed < num_threads) {
          ...
          if (batch_count == 0) std::this_thread::yield();
        }
      });
  ...
  if (!method || method == 515) {
    ...
    for (size_t i = 0; i < num_threads; ++i)
      pool.scheduleOrThrowOnError([&, i] {
        ...
        while (threads_done.load() < num_threads) std::this_thread::yield();
