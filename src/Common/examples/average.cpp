...
struct State {
  ...
  void addBatch(const Float *ptr, size_t size) {
    ...
    while (ptr < unrolled_end) {
      for (size_t i = 0; i < unroll_count; ++i) partial_sums[i] += ptr[i];
      ptr += unroll_count;
    }

    for (size_t i = 0; i < unroll_count; ++i) sum += partial_sums[i];
  ...
  Float result() const { return sum / static_cast<Float>(count); }
...
Float NO_INLINE baseline_baseline(const PODArray<UInt8> &keys, const PODArray<Float> &values) {
  ...
  for (size_t i = 0; i < size; ++i) {
    ...
    if (unlikely(!place)) place = new (arena.alloc<State>()) State();
...
using FixedHashMapWithCalculatedSize =
    FixedHashMap<Key, Mapped, FixedHashMapCell<Key, Mapped>, FixedHashTableCalculatedSize<FixedHashMapCell<Key, Mapped>>>;
...
Float NO_INLINE init_out_of_the_loop(const PODArray<UInt8> &keys, const PODArray<Float> &values) {
  ...
  for (size_t i = 0; i < 256; ++i) map[static_cast<UInt8>(i)] = new (arena.alloc<State>()) State();
...
Float NO_INLINE simple_lookup_table_embedded_states(const PODArray<UInt8> &keys, const PODArray<Float> &values) {
  ...
  size_t size = keys.size();
  for (size_t i = 0; i < size; ++i) map[keys[i]].add(values[i]);
...
Float NO_INLINE unrolled(const PODArray<UInt8> &keys, const PODArray<Float> &values) {
  ...
  for (; i < size_unrolled; i += UNROLL_COUNT) {
    ...
    for (size_t j = 0; j < UNROLL_COUNT; ++j) places[j]->add(values[i + j]);
  }
...
Float NO_INLINE embedded_states_unrolled(const PODArray<UInt8> &keys, const PODArray<Float> &values) {
  ...
  for (; i < size_unrolled; i += UNROLL_COUNT) {
    StatePtr places[UNROLL_COUNT];
    for (size_t j = 0; j < UNROLL_COUNT; ++j) places[j] = &map[keys[i + j]];
...
Float NO_INLINE microsort(const PODArray<UInt8> &keys, const PODArray<Float> &values) {
  ...
  for (const UInt8 *elem = keys.data(); elem < keys.data() + unrolled_size; elem += UNROLL_COUNT)
    for (size_t i = 0; i < UNROLL_COUNT; ++i) ++count[i * HISTOGRAM_SIZE + elem[i]];

  for (const UInt8 *elem = keys.data() + unrolled_size; elem < keys.data() + size; ++elem) ++count[*elem];

  for (size_t i = 0; i < HISTOGRAM_SIZE; ++i)
    for (size_t j = 1; j < UNROLL_COUNT; ++j) count[i] += count[j * HISTOGRAM_SIZE + i];
  ...
  for (size_t i = 1; i < HISTOGRAM_SIZE; ++i) positions[i] = positions[i - 1] + count[i - 1];

  for (UInt32 i = 0; i < size; ++i) *positions[keys[i]]++ = i;

  /// Update states.

  UInt32 *idx = indices.data();
  for (size_t i = 0; i < HISTOGRAM_SIZE; ++i)
    for (; idx < positions[i]; ++idx) map[i].add(values[*idx]);
...
Float NO_INLINE buffered(const PODArray<UInt8> &keys, const PODArray<Float> &values) {
  ...
  for (size_t i = 0; i < 256; ++i) ptrs[i] = &buffers[i * BUF_SIZE];
  ...
  for (size_t i = 0; i < 256; ++i) map[i].addBatch<4>(&buffers[i * BUF_SIZE], ptrs[i] - &buffers[i * BUF_SIZE]);
...
Float NO_INLINE really_unrolled(const PODArray<UInt8> &keys, const PODArray<Float> &values) {
  ...
  size_t size_unrolled = size / UNROLL_COUNT * UNROLL_COUNT;
  for (; i < size_unrolled; i += UNROLL_COUNT)
    for (size_t j = 0; j < UNROLL_COUNT; ++j) map[256 * j + keys[i + j]].add(values[i + j]);

  for (size_t key = 0; key < 256; ++key)
    for (size_t j = 1; j < UNROLL_COUNT; ++j) map[key].merge(map[256 * j + key]);

  for (; i < size; ++i) map[keys[i]].add(values[i]);
...
struct State4 {
  ...
  Float result() const { return (sum[0] + sum[1] + sum[2] + sum[3]) / static_cast<Float>(count[0] + count[1] + count[2] + count[3]); }
};
...
int main(int argc, char **argv) {
  ...
  switch (variant) {
    case 0:
      res = baseline(keys, values);
      break;
    case 1:
      res = implicit_zero(keys, values);
      break;
    case 2:
      res = calculated_size(keys, values);
      break;
    case 3:
      res = implicit_zero_and_calculated_size(keys, values);
      break;
    case 4:
      res = init_out_of_the_loop(keys, values);
      break;
    case 5:
      res = embedded_states(keys, values);
      break;
    case 6:
      res = simple_lookup_table(keys, values);
      break;
    case 7:
      res = simple_lookup_table_embedded_states(keys, values);
      break;
    case 8:
      res = microsort<1>(keys, values);
      break;
    case 9:
      res = baseline_baseline(keys, values);
      break;
    case 10:
      res = buffered(keys, values);
      break;
    case 11:
      res = really_unrolled<1>(keys, values);
      break;
    case 12:
      res = another_unrolled_x4(keys, values);
      break;

    case 32:
      res = unrolled<2>(keys, values);
      break;
    case 34:
      res = unrolled<4>(keys, values);
      break;
    case 36:
      res = unrolled<6>(keys, values);
      break;
    case 38:
      res = unrolled<8>(keys, values);
      break;
    case 316:
      res = unrolled<16>(keys, values);
      break;

    case 52:
      res = embedded_states_unrolled<2>(keys, values);
      break;
    case 54:
      res = embedded_states_unrolled<4>(keys, values);
      break;
    case 56:
      res = embedded_states_unrolled<6>(keys, values);
      break;
    case 58:
      res = embedded_states_unrolled<8>(keys, values);
      break;
    case 516:
      res = embedded_states_unrolled<16>(keys, values);
      break;

    case 62:
      res = simple_lookup_table_unrolled<2>(keys, values);
      break;
    case 64:
      res = simple_lookup_table_unrolled<4>(keys, values);
      break;
    case 66:
      res = simple_lookup_table_unrolled<6>(keys, values);
      break;
    case 68:
      res = simple_lookup_table_unrolled<8>(keys, values);
      break;
    case 616:
      res = simple_lookup_table_unrolled<16>(keys, values);
      break;

    case 72:
      res = simple_lookup_table_embedded_states_unrolled<2>(keys, values);
      break;
    case 74:
      res = simple_lookup_table_embedded_states_unrolled<4>(keys, values);
      break;
    case 76:
      res = simple_lookup_table_embedded_states_unrolled<6>(keys, values);
      break;
    case 78:
      res = simple_lookup_table_embedded_states_unrolled<8>(keys, values);
      break;
    case 716:
      res = simple_lookup_table_embedded_states_unrolled<16>(keys, values);
      break;

    case 82:
      res = microsort<2>(keys, values);
      break;
    case 84:
      res = microsort<4>(keys, values);
      break;
    case 86:
      res = microsort<6>(keys, values);
      break;
    case 88:
      res = microsort<8>(keys, values);
      break;
    case 816:
      res = microsort<16>(keys, values);
      break;

    case 112:
      res = really_unrolled<2>(keys, values);
      break;
    case 114:
      res = really_unrolled<4>(keys, values);
      break;
    case 116:
      res = really_unrolled<5>(keys, values);
      break;
    case 118:
      res = really_unrolled<8>(keys, values);
      break;
    case 1116:
      res = really_unrolled<16>(keys, values);
      break;
