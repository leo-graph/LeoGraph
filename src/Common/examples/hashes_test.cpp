...
int main(int, char **) {
...

  {
    ...
    watch.stop();
    rows = strings.size();
    std::cerr << std::fixed << std::setprecision(2) << "Read " << rows << " rows, " << static_cast<double>(bytes) / 1000000.0 << " MB"
              << ", elapsed: " << watch.elapsedSeconds() << " (" << static_cast<double>(rows) / watch.elapsedSeconds() << " rows/sec., "
              << static_cast<double>(bytes) / 1000000.0 / watch.elapsedSeconds() << " MB/sec.)" << std::endl;
  }
...

  {
    ...
    std::cerr << std::fixed << std::setprecision(2) << "CityHash64 (check = " << check << ")"
              << ", elapsed: " << watch.elapsedSeconds() << " (" << static_cast<double>(rows) / watch.elapsedSeconds() << " rows/sec., "
              << static_cast<double>(bytes) / 1000000.0 / watch.elapsedSeconds() << " MB/sec.)" << std::endl;
  }
...

  {
    ...
    std::cerr << std::fixed << std::setprecision(2) << "SipHash, stream (check = " << check << ")"
              << ", elapsed: " << watch.elapsedSeconds() << " (" << static_cast<double>(rows) / watch.elapsedSeconds() << " rows/sec., "
              << static_cast<double>(bytes) / 1000000.0 / watch.elapsedSeconds() << " MB/sec.)" << std::endl;
  }
...
#if USE_SSL
  {
    ...
    std::cerr << std::fixed << std::setprecision(2) << "MD5 (check = " << check << ")"
              << ", elapsed: " << watch.elapsedSeconds() << " (" << static_cast<double>(rows) / watch.elapsedSeconds() << " rows/sec., "
              << static_cast<double>(bytes) / 1000000.0 / watch.elapsedSeconds() << " MB/sec.)" << std::endl;
  }
#endif
