 ...
 */
template <typename TKey, typename TMapped, typename WeightFunction = TrivialLRUResourceCacheWeightFunction<TMapped>,
          typename ReleaseFunction = TrivialLRUResourceCacheReleaseFunction<TMapped>, typename HashFunction = std::hash<TKey>>
 ...
 private:
  ...
  MappedPtr getImpl(const Key &key, LoadFunc &&load_func) {
    InsertTokenHolder token_holder;
    {
      ...
      if (it != cells.end()) {
        ...
        if (it->second.reference_count > 0) return nullptr;
    ...
    if (!token->value) token->value = load_func();
    ...
    if (!token->cleaned_up) token_holder.cleanup(token_lock, lock);
  ...
  void releaseInsertToken(const Key &key) {
    ...
    if (it != insert_tokens.end()) {
      it->second.reference_count -= 1;
      if (it->second.reference_count == 0) insert_tokens.erase(it);
    }
  }
