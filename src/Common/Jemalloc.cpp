...
void setBackgroundThreads(bool enabled) { setValue("background_thread", enabled); }

void setMaxBackgroundThreads(size_t max_threads) { setValue("max_background_threads", max_threads); }

void setProfileSamplingRate(size_t lg_prof_sample) {
  size_t current = getValue<size_t>("prof.lg_sample");
  if (current == lg_prof_sample) return;
...
void jemallocAllocationTracker(const void *ptr, size_t /*size*/, void **backtrace, unsigned backtrace_length, size_t usize) {
  ...
  if (!collect_local_profiles_in_trace_log && !collect_global_profiles_in_trace_log) return;
  ...
  try {
    ...
    TraceSender::send(TraceType::JemallocSample, StackTrace(std::move(frame_pointers), stacktrace_size),
                      TraceSender::Extras{
...
void jemallocDeallocationTracker(const void *ptr, unsigned usize) {
  ...
  try {
    TraceSender::send(TraceType::JemallocSample, StackTrace(),
                      TraceSender::Extras{
                          .size = -static_cast<Int64>(usize),
                          .ptr = const_cast<void *>(ptr),
                          .memory_blocked_context = MemoryTrackerBlockerInThread::getLevel(),
...
void setCollectLocalProfileSamplesInTraceLog(bool value) { collect_local_profiles_in_trace_log = value; }
...
void setup(bool enable_global_profiler, bool enable_background_threads, size_t max_background_threads_num,
  ...
  if (max_background_threads_num) setValue("max_background_threads", max_background_threads_num);

  if (profiler_sampling_rate != default_profiler_sampling_rate) setProfileSamplingRate(profiler_sampling_rate);
...
void verifySetup(bool enable_global_profiler, bool enable_background_threads, size_t max_background_threads_num,
  ...
  auto log_warning = [](std::string_view setting) {
    ...
    LOG_WARNING(&Poco::Logger::get("Jemalloc"), "Jemalloc settings mismatch: `{}` differs between BaseDaemon and server settings", setting);
  };

  if (getThreadProfileInitMib().getValue() != enable_global_profiler) log_warning(config_enable_global_profiler);
  if (getValue<bool>("background_thread") != enable_background_threads) log_warning(config_enable_background_threads);
...
std::string_view getLastFlushProfileForThread() { return last_flush_profile; }
