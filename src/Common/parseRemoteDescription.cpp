...
static void append(std::vector<String>& to, const std::vector<String>& what, size_t max_addresses) {
  ...
  for (const auto& elem_to : to)
    for (const auto& elem_what : what) res.push_back(elem_to + elem_what);
...
static bool parseNumber(const String& description, size_t l, size_t r, size_t& res) {
  ...
  for (size_t pos = l; pos < r; ++pos) {
    if (!isNumericASCII(description[pos])) return false;
    res = res * 10 + description[pos] - '0';
    if (static_cast<double>(res) > 1e15) return false;
  }
  return true;
}
    ...
    if (description[i] == '{') {
      ...
      if (last_dot != -1) {
        ...
        if (description[last_dot - 1] != '.')
          throw Exception(ErrorCodes::BAD_ARGUMENTS, "Table function '{}': incorrect argument in braces (only one dot): {}", func_name,
        ...
        if (!parseNumber(description, i + 1, last_dot - 1, left))
          throw Exception(ErrorCodes::BAD_ARGUMENTS,
                          ...
                          func_name, description.substr(i, m - i + 1));
        ...
        if (last_dot - 1 - (i + 1) == m - (last_dot + 1)) add_leading_zeroes = true;
        for (size_t id = left; id <= right; ++id) {
          String id_str = toString<UInt64>(id);
          if (add_leading_zeroes) {
            while (id_str.size() < len) id_str = "0" + id_str;
          }
