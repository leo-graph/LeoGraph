...
bool madviseSupportsMadvPopulateWrite() {
  ...
  if (!is_supported) LOG_TRACE(getLogger("Allocator"), "Disabled page pre-faulting (kernel is too old).");
  return is_supported;
}
...
#if defined(MADV_POPULATE_WRITE)
  if (len_ < POPULATE_THRESHOLD) return;

  if (unlikely(!is_supported_by_kernel)) return;
...
void *allocNoTrack(size_t size, size_t alignment) {
  ...
  if (likely(alignment <= MALLOC_MIN_ALIGNMENT)) {
    ...
    if (unlikely(nullptr == buf))
      throw DB::ErrnoException(DB::ErrorCodes::CANNOT_ALLOCATE_MEMORY, "Allocator: Cannot malloc {}.",
    ...
    if (unlikely(0 != res)) {
      ...
      DB::ErrnoException::throwWithErrno(DB::ErrorCodes::CANNOT_ALLOCATE_MEMORY, res, "Cannot allocate memory (posix_memalign) {}.",
                                         ReadableSize(size));
    }
  ...
  if constexpr (populate) prefaultPages(buf, size);

  return buf;
}

void freeNoTrack(void *buf) { __real_free(buf); }
...
/// Constant is chosen almost arbitrarily, what I observed is 128KB is too small, 1MB is almost indistinguishable from 64MB and 1GB is too
/// large.
...
void *Allocator<clear_memory_, populate>::realloc(void *buf, size_t old_size, size_t new_size, size_t alignment) {
  ...
  if (likely(alignment <= MALLOC_MIN_ALIGNMENT)) {
    ...
    if (unlikely(nullptr == new_buf)) {
      ...
      throw DB::ErrnoException(DB::ErrorCodes::CANNOT_ALLOCATE_MEMORY, "Allocator: Cannot realloc from {} to {}", ReadableSize(old_size),
    ...
    if constexpr (clear_memory)
      if (new_size > old_size) memset(reinterpret_cast<char *>(buf) + old_size, 0, new_size - old_size);
  } else {
  ...
  if constexpr (populate) prefaultPages(buf, new_size);
