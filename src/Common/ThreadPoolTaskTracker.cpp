...
void TaskTracker::add(Callback&& func) {
  ...
  Callback func_with_notification = [&, my_func = std::move(func),
                                     my_pre_allocated_finished = std::move(pre_allocated_finished)]() mutable {
    SCOPE_EXIT({
...
bool TaskTracker::isAsync() const { return is_async; }
