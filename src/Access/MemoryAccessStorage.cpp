...
std::optional<UUID> MemoryAccessStorage::findImpl(AccessEntityType type, const String& name) const {
  ...
  if (it == entries_by_name.end()) return {};
...
std::vector<UUID> MemoryAccessStorage::findAllImpl(AccessEntityType type) const {
  ...
  for (const auto& [id, entry] : entries_by_id)
    if (entry.entity->isTypeOf(type)) result.emplace_back(id);
  return result;
}
...
bool MemoryAccessStorage::insertNoLock(const UUID& id, const AccessEntityPtr& new_entity, bool replace_if_exists, bool throw_if_exists,
                                       UUID* conflicting_id, bool notify) {
  ...
  UUID id_by_name;
  if (name_collision) id_by_name = it_by_name->second->id;

  if (name_collision && !replace_if_exists) {
    if (throw_if_exists) {
      throwNameCollisionCannotInsert(type, name, getStorageName());
    } else {
      if (conflicting_id) *conflicting_id = id_by_name;
      return false;
    }
  }
  ...
  if (id_collision) {
    ...
    if (existing_entry.entity->getType() == new_entity->getType()) {
      if (*existing_entry.entity != *new_entity) {
        ...
        if (notify) changes_notifier.onEntityUpdated(id, new_entity);
      }
      return true;
    }
  ...
  if (notify) changes_notifier.onEntityAdded(id, new_entity);
  return true;
}
...
bool MemoryAccessStorage::removeNoLock(const UUID& id, bool throw_if_not_exists, bool notify) {
  ...
  if (notify) changes_notifier.onEntityRemoved(removed_id, type);
  return true;
}
...
bool MemoryAccessStorage::updateNoLock(const UUID& id, const UpdateFunc& update_func, bool throw_if_not_exists, bool notify) {
  ...
  if (!new_entity->isTypeOf(old_entity->getType())) throwBadCast(id, new_entity->getType(), new_entity->getName(), old_entity->getType());

  if (*new_entity == *old_entity) return true;
...
void MemoryAccessStorage::removeAllExceptNoLock(const boost::container::flat_set<UUID>& ids_to_keep) {
  for (auto it = entries_by_id.begin(); it != entries_by_id.end();) {
    ...
    if (!ids_to_keep.contains(id)) removeNoLock(id, /* throw_if_not_exists */ true);  // NOLINT
  }
}
...
void MemoryAccessStorage::setAll(const std::vector<AccessEntityPtr>& all_entities) {
  ...
  for (const auto& entity : all_entities) entities_with_ids.emplace_back(generateRandomID(), entity);
...
void MemoryAccessStorage::setAll(const std::vector<std::pair<UUID, AccessEntityPtr>>& all_entities, bool notify) {
  ...
  for (const auto& [id, _] : entities_without_conflicts) ids_to_keep.insert(id);
  removeAllExceptNoLock(ids_to_keep);
