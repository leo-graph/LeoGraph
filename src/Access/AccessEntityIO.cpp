...
static AccessEntityPtr deserializeAccessEntityImpl(const String& definition) {
  ...
  for (const auto& query : queries) {
    if (auto* create_user_query = query->as<ASTCreateUserQuery>()) {
      if (res) throw Exception(ErrorCodes::INCORRECT_ACCESS_ENTITY_DEFINITION, "Two access entities attached in the same file");
      res = user = std::make_unique<User>();
      InterpreterCreateUserQuery::updateUserFromQuery(*user, *create_user_query, /* allow_no_password = */ true,
                                                      /* allow_plaintext_password = */ true,
                                                      /* max_number_of_authentication_methods = zero is unlimited*/ 0);
      ...
      if (!user && !role) throw Exception(ErrorCodes::INCORRECT_ACCESS_ENTITY_DEFINITION, "A user or role should be attached before grant");
      if (user)
  ...
  if (!res) throw Exception(ErrorCodes::INCORRECT_ACCESS_ENTITY_DEFINITION, "No access entities attached");

  return res;
}

AccessEntityPtr deserializeAccessEntity(const String& definition, const String& file_path) {
  if (file_path.empty()) return deserializeAccessEntityImpl(definition);
