...
namespace {
String outputID(const UUID &id) { return "ID(" + toString(id) + ")"; }
}  // namespace

std::vector<UUID> IAccessStorage::findAll(AccessEntityType type) const { return findAllImpl(type); }

std::optional<UUID> IAccessStorage::find(AccessEntityType type, const String &name) const { return findImpl(type, name); }
...
std::vector<UUID> IAccessStorage::getIDs(AccessEntityType type, const Strings &names) const {
  std::vector<UUID> ids;
  ids.reserve(names.size());
  for (const String &name : names) ids.push_back(getID(type, name));
  return ids;
}

String IAccessStorage::readName(const UUID &id) const { return readNameWithType(id).first; }
...
std::optional<String> IAccessStorage::readName(const UUID &id, bool throw_if_not_exists) const {
  if (auto name_and_type = readNameWithType(id, throw_if_not_exists)) return name_and_type->first;
  return std::nullopt;
}
...
Strings IAccessStorage::readNames(const std::vector<UUID> &ids, bool throw_if_not_exists) const {
  ...
  for (const auto &id : ids) {
    if (auto name = readName(id, throw_if_not_exists)) res.emplace_back(std::move(name).value());
  }
  return res;
}

std::optional<String> IAccessStorage::tryReadName(const UUID &id) const { return readName(id, /* throw_if_not_exists = */ false); }

Strings IAccessStorage::tryReadNames(const std::vector<UUID> &ids) const { return readNames(ids, /* throw_if_not_exists = */ false); }
...
std::optional<std::pair<String, AccessEntityType>> IAccessStorage::readNameWithTypeImpl(const UUID &id, bool throw_if_not_exists) const {
  if (auto entity = read(id, throw_if_not_exists)) return std::make_pair(entity->getName(), entity->getType());
  return std::nullopt;
}
...
std::vector<std::pair<UUID, AccessEntityPtr>> IAccessStorage::readAllWithIDs(AccessEntityType type) const {
  ...
  for (const auto &id : findAll(type)) {
    if (auto entity = tryRead(id)) entities.emplace_back(id, entity);
  }
  return entities;
}
...
std::optional<UUID> IAccessStorage::insert(const AccessEntityPtr &entity, bool replace_if_exists, bool throw_if_exists,
  ...
  if (insert(id, entity, replace_if_exists, throw_if_exists, conflicting_id)) return id;
...
std::vector<UUID> IAccessStorage::insert(const std::vector<AccessEntityPtr> &multiple_entities, bool replace_if_exists,
                                         bool throw_if_exists) {
  return insert(multiple_entities, /* ids = */ {}, replace_if_exists, throw_if_exists);
}

std::vector<UUID> IAccessStorage::insert(const std::vector<AccessEntityPtr> &multiple_entities, const std::vector<UUID> &ids,
                                         bool replace_if_exists, bool throw_if_exists) {
  assert(ids.empty() || (multiple_entities.size() == ids.size()));

  if (multiple_entities.empty()) return {};
  ...
  if (multiple_entities.size() == 1) {
    ...
    if (insert(id, multiple_entities[0], replace_if_exists, throw_if_exists)) return {id};
    return {};
  }
  ...
  try {
    ...
    if (!successfully_inserted.empty()) {
      ...
      for (const auto &entity : successfully_inserted) {
        if (!successfully_inserted_str.empty()) successfully_inserted_str += ", ";
        successfully_inserted_str += entity->formatTypeWithName();
      }
      e.addMessage("After successfully inserting {}/{}: {}", successfully_inserted.size(), multiple_entities.size(),
...
bool IAccessStorage::insertImpl(const UUID &, const AccessEntityPtr &entity, bool, bool, UUID *) {
  if (isReadOnly()) throwReadonlyCannotInsert(entity->getType(), entity->getName());
  throw Exception(ErrorCodes::NOT_IMPLEMENTED, "insertImpl is not implemented in {}", getStorageType());
}

bool IAccessStorage::remove(const UUID &id, bool throw_if_not_exists) { return removeImpl(id, throw_if_not_exists); }

std::vector<UUID> IAccessStorage::remove(const std::vector<UUID> &ids, bool throw_if_not_exists) {
  if (ids.empty()) return {};
  if (ids.size() == 1) return remove(ids[0], throw_if_not_exists) ? ids : std::vector<UUID>{};
  ...
  try {
    ...
    for (const auto &id : ids) {
      if (isReadOnly(id))
        ...
        if (remove(id, throw_if_not_exists)) {
          ...
          if (name) removed_names.push_back(std::move(name).value());
        }
      }
    }
    ...
    if (!removed_names.empty()) {
      ...
      for (const auto &name : removed_names) {
        if (!removed_names_str.empty()) removed_names_str += ", ";
...
bool IAccessStorage::tryRemove(const UUID &id) { return remove(id, /* throw_if_not_exists = */ false); }

std::vector<UUID> IAccessStorage::tryRemove(const std::vector<UUID> &ids) { return remove(ids, /* throw_if_not_exists = */ false); }
...
std::vector<UUID> IAccessStorage::update(const std::vector<UUID> &ids, const UpdateFunc &update_func, bool throw_if_not_exists) {
  if (ids.empty()) return {};
  if (ids.size() == 1) return update(ids[0], update_func, throw_if_not_exists) ? ids : std::vector<UUID>{};
  ...
  try {
    ...
    for (const auto &id : ids) {
      if (isReadOnly(id))
        ...
        if (update(id, update_func, throw_if_not_exists)) {
          ...
          if (name) names_of_updated.push_back(std::move(name).value());
        }
      }
    }
    ...
    if (!names_of_updated.empty()) {
      ...
      for (const auto &name : names_of_updated) {
        if (!names_of_updated_str.empty()) names_of_updated_str += ", ";
...
AuthResult IAccessStorage::authenticate(const Credentials &credentials, const Poco::Net::IPAddress &address,
  ...
  return *authenticateImpl(credentials, address, external_authenticators, client_info, /* throw_if_user_not_exists = */ true,
                           allow_no_password, allow_plaintext_password);
}

std::optional<AuthResult> IAccessStorage::authenticate(const Credentials &credentials, const Poco::Net::IPAddress &address,
                                                       const ExternalAuthenticators &external_authenticators, const ClientInfo &client_info,
                                                       bool throw_if_user_not_exists, bool allow_no_password,
                                                       bool allow_plaintext_password) const {
  return authenticateImpl(credentials, address, external_authenticators, client_info, throw_if_user_not_exists, allow_no_password,
                          allow_plaintext_password);
}

Authentication::CredentialsCheckResult areCredentialsValid(const std::string &user_name, const AuthenticationData &authentication_method,
                                                           const Credentials &credentials,
                                                           const ExternalAuthenticators &external_authenticators,
                                                           const ClientInfo &client_info, SettingsChanges &settings);

std::optional<AuthResult> IAccessStorage::authenticateImpl(const Credentials &credentials, const Poco::Net::IPAddress &address,
                                                           const ExternalAuthenticators &external_authenticators,
                                                           const ClientInfo &client_info, bool throw_if_user_not_exists,
                                                           bool allow_no_password, bool allow_plaintext_password) const {
  if (auto id = find<User>(credentials.getUserName())) {
    if (auto user = tryRead<User>(*id)) {
      ...
      if (!isAddressAllowed(*user, address)) throwAddressNotAllowed(address);
      ...
      for (const auto &auth_method : user->authentication_methods) {
        ...
        auto cred_check_result =
            areCredentialsValid(user->getName(), auth_method, credentials, external_authenticators, client_info, auth_result.settings);
        if (cred_check_result == Authentication::CredentialsCheckResult::Success) {
          auth_result.authentication_data = auth_method;
          return auth_result;
        }
        if (cred_check_result == Authentication::CredentialsCheckResult::NeedSecondFactor) need_second_factor = true;
      }

      if (skipped_not_allowed_authentication_methods) {
        LOG_INFO(getLogger(),
                 "Skipped the check for not allowed authentication methods,"
                 "check allow_no_password and allow_plaintext_password settings in the server configuration");
      }

      if (need_second_factor) throw Exception(ErrorCodes::REQUIRED_SECOND_FACTOR, "Authentication requires second factor");
...
Authentication::CredentialsCheckResult areCredentialsValid(const std::string &user_name, const AuthenticationData &authentication_method,
                                                           ...
                                                           const ExternalAuthenticators &external_authenticators,
                                                           const ClientInfo &client_info, SettingsChanges &settings) {
  if (!credentials.isReady()) return Authentication::CredentialsCheckResult::Fail;

  if (credentials.getUserName() != user_name) return Authentication::CredentialsCheckResult::Fail;

  auto valid_until = authentication_method.getValidUntil();
  if (valid_until) {
    const time_t now = std::chrono::system_clock::to_time_t(std::chrono::system_clock::now());

    if (now > valid_until) return Authentication::CredentialsCheckResult::Fail;
  }
...
void IAccessStorage::backup(BackupEntriesCollector &backup_entries_collector, const String &data_path_in_backup,
                            AccessEntityType type) const {
  if (!isBackupAllowed()) throwBackupNotAllowed();

  auto entities_ids = findAll(type);
  if (entities_ids.empty()) return;

  auto backup_entry_with_path =
      makeBackupEntryForAccessEntities(entities_ids, backup_entries_collector.getAllAccessEntities(),
                                       backup_entries_collector.getBackupSettings().write_access_entities_dependents, data_path_in_backup);
  ...
  if (isReplicated()) {
    ...
    backup_entries_collector.addPostTask(
        [backup_entry = backup_entry_with_path.second, replication_id, type, &backup_entries_collector, backup_coordination] {
...
void IAccessStorage::restoreFromBackup(RestorerFromBackup &restorer, const String &data_path_in_backup) {
  if (!isRestoreAllowed()) throwRestoreNotAllowed();

  if (isReplicated()) {
    auto restore_coordination = restorer.getRestoreCoordination();
    if (!restore_coordination->acquireReplicatedAccessStorage(getReplicationID())) return;
  }

  restorer.addDataRestoreTask([this, &restorer, data_path_in_backup] {
...
void IAccessStorage::clearConflictsInEntitiesList(std::vector<std::pair<UUID, AccessEntityPtr>> &entities, LoggerPtr log_) {
  ...
  if (positions_to_remove.empty()) return;
  ...
  for (size_t pos : positions_to_remove) {
    LOG_WARNING(log_, "Skipping {} (id={}) due to conflicts with other access entities", entities[pos].second->formatTypeWithName(),
...
LoggerPtr IAccessStorage::getLogger() const {
  callOnce(log_initialized, [&] { log = ::getLogger("Access(" + storage_name + ")"); });
...
void IAccessStorage::throwBadCast(const UUID &id, AccessEntityType type, const String &name, AccessEntityType required_type) {
  throw Exception(ErrorCodes::LOGICAL_ERROR, "{}: {} expected to be of type {}", outputID(id), formatEntityTypeWithName(type, name),
                  toString(required_type));
}

void IAccessStorage::throwIDCollisionCannotInsert(const UUID &id, AccessEntityType type, const String &name, AccessEntityType existing_type,
                                                  const String &existing_name, const String &storage_name) {
  throw Exception(ErrorCodes::ACCESS_ENTITY_ALREADY_EXISTS, "{}: cannot insert because the {} is already used by {} in {}",
                  formatEntityTypeWithName(type, name), outputID(id), formatEntityTypeWithName(existing_type, existing_name),
                  backQuote(storage_name));
}
...
void IAccessStorage::throwNameCollisionCannotRename(AccessEntityType type, const String &old_name, const String &new_name,
                                                    const String &storage_name) {
  throw Exception(ErrorCodes::ACCESS_ENTITY_ALREADY_EXISTS, "{}: cannot rename to {} because {} already exists in {}",
                  formatEntityTypeWithName(type, old_name), backQuote(new_name), formatEntityTypeWithName(type, new_name),
                  backQuote(storage_name));
}
