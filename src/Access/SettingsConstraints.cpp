...
struct SettingSourceRestrictions {
  ...
  constexpr SettingSourceRestrictions(std::initializer_list<SettingSource> allowed_sources_) {
    for (auto allowed_source : allowed_sources_) setSourceAllowed(allowed_source, true);
  }
...
SettingSourceRestrictions getSettingSourceRestrictions(std::string_view name) {
  ...
  if (settingConstraintIter != SETTINGS_SOURCE_RESTRICTIONS.end()) return settingConstraintIter->second;
...
void SettingsConstraints::set(const String &full_name, const Field &min_value, const Field &max_value,
                              const std::vector<Field> &disallowed_values, SettingConstraintWritability writability) {
  std::string resolved_name{resolveSettingName(full_name)};

  auto &constraint = constraints[resolved_name];

  if (full_name != resolved_name) settings_alias_cache[full_name] = resolved_name;

  if (!min_value.isNull()) constraint.min_value = settingCastValueUtil(resolved_name, min_value);
  if (!max_value.isNull()) constraint.max_value = settingCastValueUtil(resolved_name, max_value);
...
void SettingsConstraints::get(const Settings &current_settings, std::string_view short_name, Field &min_value, Field &max_value,
                              std::vector<Field> &disallowed_values, SettingConstraintWritability &writability) const {
...
void SettingsConstraints::get(const MergeTreeSettings &, std::string_view short_name, Field &min_value, Field &max_value,
                              std::vector<Field> &disallowed_values, SettingConstraintWritability &writability) const {
...
void SettingsConstraints::merge(const SettingsConstraints &other) {
  if (access_control->doesSettingsConstraintsReplacePrevious()) {
    ...
    for (const auto &[other_name, other_constraint] : other.constraints) {
      ...
      if (!other_constraint.min_value.isNull()) constraint.min_value = other_constraint.min_value;
      if (!other_constraint.max_value.isNull()) constraint.max_value = other_constraint.max_value;
      if (!other_constraint.disallowed_values.empty()) constraint.disallowed_values = other_constraint.disallowed_values;
      if (other_constraint.writability == SettingConstraintWritability::CONST)
        constraint.writability =
            SettingConstraintWritability::CONST;  // NOTE: In this mode <readonly/> flag cannot be overridden to be false
    }
  }
...
void SettingsConstraints::check(const Settings &current_settings, const AlterSettingsProfileElements &profile_elements,
...
void SettingsConstraints::check(const Settings &current_settings, const SettingsProfileElements &profile_elements,
                                SettingSource source) const {
  for (const auto &element : profile_elements) {
    if (SettingsProfileElements::isAllowBackupSetting(element.setting_name)) continue;
    ...
    if (element.writability) new_value = *element.writability;

    auto setting_name = Settings::resolveName(element.setting_name);
    auto it = constraints.find(setting_name);
    if (it != constraints.end()) old_value = it->second.writability;
...
void SettingsConstraints::check(const Settings &current_settings, const SettingsChanges &changes, SettingSource source) const {
  for (const auto &change : changes) check(current_settings, change, source);
}

void SettingsConstraints::check(const Settings &current_settings, SettingsChanges &changes, SettingSource source) const {
  std::erase_if(changes, [&](SettingChange &change) -> bool { return !checkImpl(current_settings, change, THROW_ON_VIOLATION, source); });
}
...
void SettingsConstraints::check(const MergeTreeSettings &current_settings, const SettingsChanges &changes) const {
  for (const auto &change : changes) check(current_settings, change);
}

void SettingsConstraints::clamp(const Settings &current_settings, SettingsChanges &changes, SettingSource source) const {
  std::erase_if(changes, [&](SettingChange &change) -> bool { return !checkImpl(current_settings, change, CLAMP_ON_VIOLATION, source); });
}
...
bool getNewValueToCheck(const SettingsT &current_settings, SettingChange &change, Field &new_value, bool throw_on_failure) {
  ...
  if (has_current_value && change.value == current_value) return false;
  ...
  /// Setting isn't checked if value has not changed.
  if (has_current_value && new_value == current_value) return false;

  return true;
}

bool SettingsConstraints::checkImpl(const Settings &current_settings, SettingChange &change, ReactionOnViolation reaction,
                                    SettingSource source) const {
  std::string_view setting_name = Settings::resolveName(change.name);

  if (setting_name == "profile") return true;
  ...
  Field new_value;
  if (!getNewValueToCheck(current_settings, change, new_value, reaction == THROW_ON_VIOLATION)) return false;
...
bool SettingsConstraints::Checker::check(SettingChange &change, const Field &new_value, ReactionOnViolation reaction,
                                         SettingSource source) const {
  if (!explain.text.empty()) {
    if (reaction == THROW_ON_VIOLATION) throw Exception(explain, code);
    return false;
  }
  ...
  auto less_or_cannot_compare = [=](const Field &left, const Field &right) {
    if (reaction == THROW_ON_VIOLATION) return accurateLess(left, right);
    try {
  ...
  auto equals_or_cannot_compare = [=](const Field &left, const Field &right) {
    if (reaction == THROW_ON_VIOLATION) return accurateEquals(left, right);
    try {
  ...
  if (!min_value.isNull() && !max_value.isNull() && less_or_cannot_compare(max_value, min_value)) {
    if (reaction == THROW_ON_VIOLATION)
      throw Exception(ErrorCodes::SETTING_CONSTRAINT_VIOLATION, "The maximum ({}) value is less than the minimum ({}) value for setting {}",
                      max_value, min_value, setting_name);
    return false;
  }

  if (!min_value.isNull() && less_or_cannot_compare(new_value, min_value)) {
    if (reaction == THROW_ON_VIOLATION) {
      throw Exception(ErrorCodes::SETTING_CONSTRAINT_VIOLATION, "Setting {} shouldn't be less than {}", setting_name,
  ...
  if (!max_value.isNull() && less_or_cannot_compare(max_value, new_value)) {
    if (reaction == THROW_ON_VIOLATION) {
      throw Exception(ErrorCodes::SETTING_CONSTRAINT_VIOLATION, "Setting {} shouldn't be greater than {}", setting_name,
  ...
  for (const auto &value : disallowed_values) {
    ...
    if (equals)
      throw Exception(ErrorCodes::SETTING_CONSTRAINT_VIOLATION, "Setting {} shouldn't be {}", setting_name,
...
std::string_view SettingsConstraints::resolveSettingNameWithCache(std::string_view name) const {
  if (auto it = settings_alias_cache.find(name); it != settings_alias_cache.end()) return it->second;
  return name;
}
...
SettingsConstraints::Checker SettingsConstraints::getChecker(const Settings &current_settings, std::string_view setting_name) const {
  ...
  if (access_control) {
    ...
    if (!allowed_experimental || !allowed_beta) {
      ...
      if (setting_tier == SettingsTierType::BETA && !allowed_beta)
        return Checker(PreformattedMessage::create(
  ...
  if (current_settings[Setting::readonly] == 1) {
    if (it == constraints.end() || it->second.writability != SettingConstraintWritability::CHANGEABLE_IN_READONLY)
      return Checker(PreformattedMessage::create("Cannot modify '{}' setting in readonly mode", setting_name), ErrorCodes::READONLY);
  } else  // For both readonly=0 and readonly=2
  {
    if (it == constraints.end()) return Checker(Settings::resolveName);  // Allowed
  }
...
SettingsConstraints::Checker SettingsConstraints::getMergeTreeChecker(std::string_view short_name) const {
  ...
  if (it == constraints.end()) return Checker(MergeTreeSettings::resolveName);  // Allowed
...
bool SettingsConstraints::Constraint::operator==(const Constraint &other) const {
  return writability == other.writability && min_value == other.min_value && max_value == other.max_value &&
         disallowed_values == other.disallowed_values;
}

bool operator==(const SettingsConstraints &left, const SettingsConstraints &right) { return left.constraints == right.constraints; }
}  // namespace DB
