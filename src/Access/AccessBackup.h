...
void restoreAccessEntitiesFromBackup(IAccessStorage& access_storage, const AccessEntitiesToRestore& entities_to_restore,
...
class AccessRestorerFromBackup {
 ...
 private:
  ...
  struct EntityInfo {
    ...
    /// For example, if we're going to restore the following user: `CREATE USER user1 DEFAULT ROLE role1, role2 SETTINGS PROFILE profile1,
    /// profile2` then `restore=true` for `user1` and `is_dependency=true` for `role1`, `role2`, `profile1`, `profile2`. Flags `restore` and
    /// `is_dependency` both can be set at the same time.
