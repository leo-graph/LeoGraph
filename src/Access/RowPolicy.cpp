...
void RowPolicy::setName(const String &) { throw Exception(ErrorCodes::NOT_IMPLEMENTED, "RowPolicy::setName is not implemented"); }

bool RowPolicy::equal(const IAccessEntity &other) const {
  if (!IAccessEntity::equal(other)) return false;
...
std::vector<UUID> RowPolicy::findDependencies() const { return to_roles.findDependencies(); }

bool RowPolicy::hasDependencies(const std::unordered_set<UUID> &ids) const { return to_roles.hasDependencies(ids); }

void RowPolicy::replaceDependencies(const std::unordered_map<UUID, UUID> &old_to_new_ids) { to_roles.replaceDependencies(old_to_new_ids); }

void RowPolicy::copyDependenciesFrom(const IAccessEntity &src, const std::unordered_set<UUID> &ids) {
  if (getType() != src.getType()) return;
  const auto &src_policy = typeid_cast<const RowPolicy &>(src);
  to_roles.copyDependenciesFrom(src_policy.to_roles, ids);
}

void RowPolicy::removeDependencies(const std::unordered_set<UUID> &ids) { to_roles.removeDependencies(ids); }

void RowPolicy::clearAllExceptDependencies() {
  for (auto &filter : filters) filter = {};
}
