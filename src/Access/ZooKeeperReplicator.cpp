...
String makeWatchIdFromId(const DB::UUID &id) { return "ZooKeeperReplicator::" + toString(id); }
...
ZooKeeperReplicator::ZooKeeperReplicator(const String &storage_name_, const String &zookeeper_path_, zkutil::GetZooKeeper get_zookeeper_,
                                         AccessChangesNotifier &changes_notifier_, MemoryAccessStorage &memory_storage_,
    ...
    : storage_name(storage_name_),
      ...
      watch_entities_list(
          std::make_shared<Coordination::WatchCallback>([my_watched_queue = watched_queue](const Coordination::WatchResponse &) {
...
void ZooKeeperReplicator::shutdown() { stopWatchingThread(); }

void ZooKeeperReplicator::startWatchingThread() {
  bool prev_watching_flag = watching.exchange(true);
  if (!prev_watching_flag) watching_thread = std::make_unique<ThreadFromGlobalPool>(&ZooKeeperReplicator::runWatchingThread, this);
}
...
void ZooKeeperReplicator::stopWatchingThread() {
  ...
  if (prev_watching_flag) {
    ...
    if (watching_thread && watching_thread->joinable()) watching_thread->join();
  }
}
...
bool ZooKeeperReplicator::insertEntity(const UUID &id, const AccessEntityPtr &new_entity, bool replace_if_exists, bool throw_if_exists,
                                       UUID *conflicting_id) {
  ...
  bool ok = false;
  retryOnZooKeeperUserError(1000,
                            [&] { ok = insertZooKeeper(zookeeper, id, new_entity, replace_if_exists, throw_if_exists, conflicting_id); });
...
bool ZooKeeperReplicator::insertZooKeeper(const zkutil::ZooKeeperPtr &zookeeper, const UUID &id, const AccessEntityPtr &new_entity,
                                          bool replace_if_exists, bool throw_if_exists, UUID *conflicting_id) {
  const String &name = new_entity->getName();
  ...
  if (res == Coordination::Error::ZNODEEXISTS) {
    if (!replace_if_exists) {
      if (responses[0]->error == Coordination::Error::ZNODEEXISTS) {
        ...
        if (throw_if_exists) {
          ...
          if (conflicting_id) *conflicting_id = id;
          return false;
        }
    ...
    if (responses[0]->error == Coordination::Error::ZNODEEXISTS) {
      ...
      const String existing_name_path =
          zookeeper_path + "/" + existing_entity_type_info.unique_char + "/" + escapeForFileName(existing_entity_name);

      LOG_DEBUG(&Poco::Logger::get(storage_name), "Removing existing entity with name {} and path {}", existing_entity_name,
                existing_name_path);
      if (existing_name_path != name_path) replace_ops.emplace_back(zkutil::makeRemoveRequest(existing_name_path, -1));
    ...
    if (responses[1]->error == Coordination::Error::ZNODEEXISTS) {
      ...
      LOG_DEBUG(&Poco::Logger::get(storage_name), "Removing existing entity with uuid {} and path {}", existing_entity_uuid,
                existing_entity_path);
      if (existing_entity_path != entity_path) replace_ops.emplace_back(zkutil::makeRemoveRequest(existing_entity_path, -1));
...
bool ZooKeeperReplicator::updateZooKeeper(const zkutil::ZooKeeperPtr &zookeeper, const UUID &id,
                                          const IAccessStorage::UpdateFunc &update_func, bool throw_if_not_exists) {
  const String entity_uuid = toString(id);
  const String entity_path = zookeeper_path + "/uuid/" + entity_uuid;
  ...
  if (!new_entity->isTypeOf(old_entity->getType()))
    throw Exception(ErrorCodes::LOGICAL_ERROR, "{}: {} expected to be of type {}", toString(id),
                    formatEntityNameWithType(new_entity->getType(), new_entity->getName()), toString(old_entity->getType()));
...
void ZooKeeperReplicator::initZooKeeperWithRetries(size_t max_retries) {
  for (size_t attempt = 0; attempt < max_retries; ++attempt) {
    try {
      ...
      if (!need_another_attempt) throw;
    }
  }
}

void ZooKeeperReplicator::initZooKeeperIfNeeded() { getZooKeeper(); }
...
zkutil::ZooKeeperPtr ZooKeeperReplicator::getZooKeeperNoLock() {
  if (!cached_zookeeper || cached_zookeeper->expired()) {
    ...
    if (!zookeeper) throw Exception(ErrorCodes::NO_ZOOKEEPER, "Can't have Replicated access without ZooKeeper");
...
bool ZooKeeperReplicator::refresh() {
  UUID id;
  if (!watched_queue->tryPop(id, /* timeout_ms: */ 10000)) return false;
...
void ZooKeeperReplicator::refreshEntities(const zkutil::ZooKeeperPtr &zookeeper, bool all) {
  ...
  for (const String &entity_uuid_str : entity_uuid_strs) entity_uuids.emplace_back(parseUUID(entity_uuid_str));
  ...
  if (all) {
    ...
    for (const auto &uuid : entity_uuids) {
      if (auto entity = tryReadEntityFromZooKeeper(zookeeper, uuid)) entities.emplace_back(uuid, entity);
    }
    ...
    for (const auto &uuid : entity_uuids) {
      if (!memory_storage.exists(uuid)) refreshEntityNoLock(zookeeper, uuid);
    }
  }
...
AccessEntityPtr ZooKeeperReplicator::tryReadEntityFromZooKeeper(const zkutil::ZooKeeperPtr &zookeeper, const UUID &id) const {
  auto watch = zookeeper->createWatchFromRawCallback(makeWatchIdFromId(id), [&]() -> Coordination::WatchCallback {
    return [my_watched_queue = watched_queue, id](const Coordination::WatchResponse &response) {
      if (response.type == Coordination::Event::CHANGED) [[maybe_unused]]
        bool push_result = my_watched_queue->push(id);
    };
  });
  ...
  try {
    ...
    if (throw_on_invalid_entities) throw;
