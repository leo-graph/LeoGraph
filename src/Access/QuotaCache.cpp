...
String QuotaCache::QuotaInfo::calculateKey(const EnabledQuota& enabled, bool throw_if_client_key_empty) const {
  ...
  switch (quota->key_type) {
    ...
    case QuotaKeyType::CLIENT_KEY: {
      if (!params.client_key.empty()) return params.client_key;

      if (throw_if_client_key_empty)
        throw Exception(ErrorCodes::QUOTA_REQUIRES_CLIENT_KEY, "Quota {} (for user {}) requires a client supplied key.", quota->getName(),
                        params.user_name);
...
boost::shared_ptr<const EnabledQuota::Intervals> QuotaCache::QuotaInfo::getOrBuildIntervals(const String& key) {
  ...
  if (it != key_to_intervals.end()) return it->second;
  return rebuildIntervals(key, std::chrono::system_clock::now());
}

void QuotaCache::QuotaInfo::rebuildAllIntervals() {
  if (key_to_intervals.empty()) return;
  auto current_time = std::chrono::system_clock::now();
  for (const String& key : key_to_intervals | boost::adaptors::map_keys) rebuildIntervals(key, current_time);
}

boost::shared_ptr<const EnabledQuota::Intervals> QuotaCache::QuotaInfo::rebuildIntervals(
    const String& key, std::chrono::system_clock::time_point current_time) {
  ...
  for (const auto& limits : quota->all_limits) {
    ...
    for (auto quota_type : collections::range(QuotaType::MAX)) {
      ...
      if (limits.max[quota_type_i]) interval.max[quota_type_i] = *limits.max[quota_type_i];
  ...
  if (it == key_to_intervals.end()) {
    ...
    for (auto& new_interval : new_intervals->intervals) {
      ...
      if ((lower_bound == old_intervals.end()) || (lower_bound->duration != new_interval.duration)) continue;
...
QuotaCache::QuotaCache(const AccessControl& access_control_) : access_control(access_control_) {}

QuotaCache::~QuotaCache() = default;

std::shared_ptr<const EnabledQuota> QuotaCache::getEnabledQuota(const UUID& user_id, const String& user_name,
...
void QuotaCache::ensureAllQuotasRead() {
  ...
  if (all_quotas_read) return;
  all_quotas_read = true;

  subscription = access_control.subscribeForChanges<Quota>([&](const UUID& id, const AccessEntityPtr& entity) {
    if (entity)
  ...
  for (const UUID& quota_id : access_control.findAll<Quota>()) {
    ...
    if (quota) all_quotas.emplace(quota_id, QuotaInfo(quota, quota_id));
  }
}
...
void QuotaCache::quotaAddedOrChanged(const UUID& quota_id, const std::shared_ptr<const Quota>& new_quota) {
  ...
  if (it == all_quotas.end()) {
    ...
    if (it->second.quota == new_quota) return;
  }
...
std::vector<QuotaUsage> QuotaCache::getAllQuotasUsage() const {
  ...
  for (const auto& info : all_quotas | boost::adaptors::map_values) {
    for (const auto& intervals : info.key_to_intervals | boost::adaptors::map_values) {
      ...
      if (usage) all_usage.push_back(std::move(usage).value());
    }
  }
  return all_usage;
}
