...
bool User::equal(const IAccessEntity &other) const {
  ...
  return (authentication_methods == other_user.authentication_methods) && (allowed_client_hosts == other_user.allowed_client_hosts) &&
...
void User::setName(const String &name_) {
  ...
  if (name_.empty()) throw Exception(ErrorCodes::BAD_ARGUMENTS, "User name is empty");
  if (name_ == EncodedUserInfo::USER_INTERSERVER_MARKER) throw Exception(ErrorCodes::BAD_ARGUMENTS, "User name '{}' is reserved", name_);
...
bool User::hasDependencies(const std::unordered_set<UUID> &ids) const {
  return default_roles.hasDependencies(ids) || granted_roles.hasDependencies(ids) || grantees.hasDependencies(ids) ||
         settings.hasDependencies(ids);
}
