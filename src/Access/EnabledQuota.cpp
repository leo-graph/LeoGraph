...
struct EnabledQuota::Impl {
  [[noreturn]] static void throwQuotaExceed(const String& user_name, const String& quota_name, QuotaType quota_type, QuotaValue used,
                                            QuotaValue max, std::chrono::seconds duration,
                                            std::chrono::system_clock::time_point end_of_interval) {
    const auto& type_info = QuotaTypeInfo::get(quota_type);
    throw Exception(ErrorCodes::QUOTA_EXCEEDED,
                    "Quota for user {} for {} has been exceeded: {}/{}. "
                    "Interval will end at {}. Name of quota template: {}",
                    backQuote(user_name), to_string(duration), type_info.valueToStringWithName(used), type_info.valueToString(max),
                    to_string(end_of_interval), backQuote(quota_name));
  }

  static void used(const String& user_name, const Intervals& intervals, QuotaType quota_type, QuotaValue value,
                   std::chrono::system_clock::time_point current_time, bool check_exceeded) {
    ...
    for (const auto& interval : intervals.intervals) {
      ...
      if (used > max) {
        ...
        if (counters_were_reset) used = (interval.used[quota_type_i] += value);
  ...
  static void checkExceeded(const String& user_name, const Intervals& intervals, QuotaType quota_type,
  ...
  static void checkExceeded(const String& user_name, const Intervals& intervals, std::chrono::system_clock::time_point current_time) {
    for (auto quota_type : collections::range(QuotaType::MAX)) checkExceeded(user_name, intervals, quota_type, current_time);
  }
  ...
  static void resetQuotaValue(const Intervals& intervals, QuotaType quota_type, QuotaValue value,
                              std::chrono::system_clock::time_point current_time) {
...
EnabledQuota::Interval::Interval(std::chrono::seconds duration_, bool randomize_interval_,
  ...
  if (randomize_interval_) initial_end += Impl::randomDuration(duration_);
...
std::chrono::system_clock::time_point EnabledQuota::Interval::getEndOfInterval(std::chrono::system_clock::time_point current_time,
                                                                               bool& counters_were_reset) const {
...
std::optional<QuotaUsage> EnabledQuota::Intervals::getUsage(std::chrono::system_clock::time_point current_time) const {
  ...
  for (const auto& in : intervals) {
    ...
    for (auto quota_type : collections::range(QuotaType::MAX)) {
      auto quota_type_i = static_cast<size_t>(quota_type);
      if (in.max[quota_type_i]) out.max[quota_type_i] = in.max[quota_type_i];
...
void EnabledQuota::used(QuotaType quota_type, QuotaValue value, bool check_exceeded) const { used({quota_type, value}, check_exceeded); }
...
void EnabledQuota::used(const std::pair<QuotaType, QuotaValue>& usage1, const std::pair<QuotaType, QuotaValue>& usage2,
...
void EnabledQuota::used(const std::pair<QuotaType, QuotaValue>& usage1, const std::pair<QuotaType, QuotaValue>& usage2,
                        const std::pair<QuotaType, QuotaValue>& usage3, bool check_exceeded) const {
  if (empty) return;
  auto loaded = intervals.load();
  auto current_time = std::chrono::system_clock::now();
  Impl::used(getUserName(), *loaded, usage1.first, usage1.second, current_time, check_exceeded);
...
void EnabledQuota::used(const std::vector<std::pair<QuotaType, QuotaValue>>& usages, bool check_exceeded) const {
  ...
  auto loaded = intervals.load();
  auto current_time = std::chrono::system_clock::now();
  for (const auto& usage : usages) Impl::used(getUserName(), *loaded, usage.first, usage.second, current_time, check_exceeded);
}
