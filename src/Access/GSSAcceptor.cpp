...
GSSAcceptorContext::GSSAcceptorContext(const GSSAcceptorContext::Params &params_) : params(params_) {}

GSSAcceptorContext::~GSSAcceptorContext() { resetHandles(); }
...
bool GSSAcceptorContext::isFailed() const { return is_failed; }
...
String extractSpecificStatusMessages(OM_uint32 status_code, int status_type, const gss_OID &mech_type) {
  ...
  do {
    ...
    SCOPE_EXIT({
      ...
      [[maybe_unused]] OM_uint32 major_status = gss_release_buffer(&minor_status, &status_string_buf);
    });

    OM_uint32 minor_status = 0;
    [[maybe_unused]] OM_uint32 major_status =
        gss_display_status(&minor_status, status_code, status_type, mech_type, &message_context, &status_string_buf);
...
String extractStatusMessages(OM_uint32 major_status_code, OM_uint32 minor_status_code, const gss_OID &mech_type) {
  ...
  if (!gss_messages.empty()) messages += "Majors: " + gss_messages;
...
std::pair<String, String> extractNameAndRealm(const gss_name_t &name) {
  ...
  SCOPE_EXIT({
    OM_uint32 minor_status = 0;
    [[maybe_unused]] OM_uint32 major_status = gss_release_buffer(&minor_status, &name_buf);
  });

  OM_uint32 minor_status = 0;
  [[maybe_unused]] OM_uint32 major_status = gss_display_name(&minor_status, name, &name_buf, nullptr);
...
bool equalMechanisms(const String &left_str, const gss_OID &right_oid) {
  ...
  SCOPE_EXIT({
    if (left_oid != GSS_C_NO_OID) {
      OM_uint32 minor_status = 0;
      [[maybe_unused]] OM_uint32 major_status = gss_release_oid(&minor_status, &left_oid);
      left_oid = GSS_C_NO_OID;
    }
  });

  OM_uint32 minor_status = 0;
  OM_uint32 major_status = gss_str_to_oid(&minor_status, &left_buf, &left_oid);

  if (GSS_ERROR(major_status)) return false;
...
void GSSAcceptorContext::resetHandles() noexcept {
  ...
  if (acceptor_credentials_handle != GSS_C_NO_CREDENTIAL) {
    OM_uint32 minor_status = 0;
    [[maybe_unused]] OM_uint32 major_status = gss_release_cred(&minor_status, &acceptor_credentials_handle);
    acceptor_credentials_handle = GSS_C_NO_CREDENTIAL;
  }

  if (context_handle != GSS_C_NO_CONTEXT) {
    OM_uint32 minor_status = 0;
    [[maybe_unused]] OM_uint32 major_status = gss_delete_sec_context(&minor_status, &context_handle, GSS_C_NO_BUFFER);
...
void GSSAcceptorContext::initHandles() {
  ...
  if (!params.keytab.empty()) {
    if (!std::filesystem::exists(params.keytab)) throw Exception(ErrorCodes::BAD_ARGUMENTS, "Keytab file not found");
  ...
  if (!params.principal.empty()) {
    if (!params.realm.empty()) throw Exception(ErrorCodes::BAD_ARGUMENTS, "Realm and principal name cannot be specified simultaneously");
    ...
    SCOPE_EXIT({
      if (acceptor_name != GSS_C_NO_NAME) {
        OM_uint32 minor_status = 0;
        [[maybe_unused]] OM_uint32 major_status = gss_release_name(&minor_status, &acceptor_name);
        acceptor_name = GSS_C_NO_NAME;
      }
    });

    OM_uint32 minor_status = 0;
    OM_uint32 major_status = gss_import_name(&minor_status, &acceptor_name_buf, GSS_C_NT_HOSTBASED_SERVICE, &acceptor_name);
    ...
    minor_status = 0;
    major_status = gss_acquire_cred(&minor_status, acceptor_name, GSS_C_INDEFINITE, GSS_C_NO_OID_SET, GSS_C_ACCEPT,
                                    &acceptor_credentials_handle, nullptr, nullptr);
...
String GSSAcceptorContext::processToken(const String &input_token, LoggerPtr log) {
  ...
  try {
    if (is_ready || is_failed || context_handle == GSS_C_NO_CONTEXT) reset();
    ...
    SCOPE_EXIT({
      if (initiator_name != GSS_C_NO_NAME) {
        OM_uint32 minor_status = 0;
        [[maybe_unused]] OM_uint32 major_status = gss_release_name(&minor_status, &initiator_name);
        initiator_name = GSS_C_NO_NAME;
      }

      OM_uint32 minor_status = 0;
      [[maybe_unused]] OM_uint32 major_status = gss_release_buffer(&minor_status, &output_token_buf);
    });

    OM_uint32 minor_status = 0;
    OM_uint32 major_status =
        gss_accept_sec_context(&minor_status, &context_handle, acceptor_credentials_handle, &input_token_buf, GSS_C_NO_CHANNEL_BINDINGS,
                               &initiator_name, &mech_type, &output_token_buf, &flags, nullptr, nullptr);
    ...
    if (major_status == GSS_S_COMPLETE) {
      ...
      if (flags & GSS_C_ANON_FLAG)
        throw Exception(ErrorCodes::KERBEROS_ERROR,
                        "gss_accept_sec_context() succeeded, but: the initiator does not wish to be authenticated");
