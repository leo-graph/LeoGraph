...
AuthenticationData::Digest AuthenticationData::Util::encodeScramSHA256(std::string_view password [[maybe_unused]],
                                                                       std::string_view salt [[maybe_unused]]) {
#if USE_SSL
  std::vector<uint8_t> salt_digest;
  for (auto elem : base64Decode(String(salt))) salt_digest.push_back(elem);
  auto salted_password = pbkdf2SHA256(password, salt_digest, 4096);
  return salted_password;
#else
  throw Exception(ErrorCodes::SUPPORT_IS_DISABLED,
                  "SCRAM SHA256 passwords support is disabled, because ClickHouse was built without SSL library");
#endif
}
...
AuthenticationData::Digest AuthenticationData::Util::encodeBcrypt(std::string_view text [[maybe_unused]], int workfactor [[maybe_unused]]) {
#if USE_BCRYPT
  if (text.size() > 72) throw Exception(ErrorCodes::BAD_ARGUMENTS, "bcrypt does not support passwords with a length of more than 72 bytes");
  ...
  if (ret != 0) throw Exception(ErrorCodes::LOGICAL_ERROR, "BCrypt library failed: bcrypt_gensalt returned {}", ret);

  ret = bcrypt_hashpw(text.data(), salt, reinterpret_cast<char *>(hash.data()));  /// NOLINT(bugprone-suspicious-stringview-data-usage)
  if (ret != 0) throw Exception(ErrorCodes::LOGICAL_ERROR, "BCrypt library failed: bcrypt_hashpw returned {}", ret);
...
bool AuthenticationData::Util::checkPasswordBcrypt(std::string_view password [[maybe_unused]],
                                                   const Digest &password_bcrypt [[maybe_unused]]) {
#if USE_BCRYPT
  ...
  static auto bcrypt_cache = SimpleCacheBase("LRU", CurrentMetrics::BcryptCacheBytes, CurrentMetrics::BcryptCacheSize,
                                             /*max_size_in_bytes*/ 1024, /*max_count*/ 1024, /*size_ratio*/ 0.5);
  ...
  auto cache_key = fmt::format("{}{}", std::string_view{reinterpret_cast<const char *>(password_digest.data()), password_digest.size()},
  ...
  auto [result, _] = bcrypt_cache.getOrSet(cache_key, [&] -> std::shared_ptr<bool> {
    int ret = bcrypt_checkpw(password.data(),
                             reinterpret_cast<const char *>(password_bcrypt.data()));  /// NOLINT(bugprone-suspicious-stringview-data-usage)
    ...
    if (ret == -1) throw Exception(ErrorCodes::AUTHENTICATION_FAILED, "Internal failure decoding Bcrypt hash");
...
bool operator==(const AuthenticationData &lhs, const AuthenticationData &rhs) {
  return (lhs.type == rhs.type) && (lhs.password_hash == rhs.password_hash) && (lhs.ldap_server_name == rhs.ldap_server_name) &&
         (lhs.kerberos_realm == rhs.kerberos_realm)
#if USE_SSL
...
#endif
         && (lhs.http_auth_scheme == rhs.http_auth_scheme) && (lhs.http_auth_server_name == rhs.http_auth_server_name) &&
...
void AuthenticationData::setPasswordBcrypt(const String &password_, int workfactor_, std::optional<OneTimePasswordSecret> second_factor,
                                           bool validate) {
...
String AuthenticationData::getPassword() const {
  if (type == AuthenticationType::PLAINTEXT_PASSWORD) return String(password_hash.data(), password_hash.data() + password_hash.size());
  ...
  switch (type) {
    ...
    case AuthenticationType::BCRYPT_PASSWORD: {
      ...
      if (validate) {
        ...
        if (ret == -1) throw Exception(ErrorCodes::BAD_ARGUMENTS, "Could not decode the provided hash with 'bcrypt_hash'");
      }
#endif
...
String AuthenticationData::getSalt() const { return salt; }
...
boost::intrusive_ptr<ASTAuthenticationData> AuthenticationData::toAST() const {
  ...
  switch (auth_type) {
    ...
    case AuthenticationType::SHA256_PASSWORD: {
      ...
      if (!getSalt().empty()) node->children.push_back(make_intrusive<ASTLiteral>(getSalt()));
      break;
    }
    case AuthenticationType::SCRAM_SHA256_PASSWORD: {
      node->contains_hash = true;
    ...
    case AuthenticationType::KERBEROS: {
      ...
      if (!realm.empty()) node->children.push_back(make_intrusive<ASTLiteral>(realm));
    ...
    case AuthenticationType::SSL_CERTIFICATE: {
      ...
      for (const auto &name : getSSLCertificateSubjects().at(cert_subject_type)) node->children.push_back(make_intrusive<ASTLiteral>(name));
    ...
    case AuthenticationType::SSH_KEY: {
#if USE_SSH
      for (const auto &key : getSSHKeys()) node->children.push_back(make_intrusive<ASTPublicSSHKey>(key.getBase64(), key.getKeyType()));
...
AuthenticationData AuthenticationData::fromAST(const ASTAuthenticationData &query, ContextPtr context, bool validate) {
  ...
  size_t args_size = query.children.size();
  ASTs args(args_size);
  for (size_t i = 0; i < args_size; ++i) args[i] = evaluateConstantExpressionAsLiteral(query.children[i], context);

  if (query.contains_password) {
    if (!query.type && !context) throw Exception(ErrorCodes::LOGICAL_ERROR, "Cannot get default password type without context");

    if (validate && !context) throw Exception(ErrorCodes::LOGICAL_ERROR, "Cannot check password complexity rules without context");
    ...
    if (validate) context->getAccessControl().checkPasswordComplexityRules(value);
    ...
    if (query.type == AuthenticationType::SHA256_PASSWORD) {
      ...
      uint8_t key[32];
      if (RAND_bytes(key, sizeof(key)) != 1) throw Exception(ErrorCodes::OPENSSL_ERROR, "RAND_bytes failed: {}", getOpenSSLErrors());
  ...
  if (query.contains_hash) {
    ...
    if ((query.type == AuthenticationType::SHA256_PASSWORD || query.type == AuthenticationType::SCRAM_SHA256_PASSWORD) && args_size == 2) {
    ...
    if (args_size > 1) scheme = parseHTTPAuthenticationScheme(checkAndGetLiteralArgument<String>(args[1], "scheme"));
