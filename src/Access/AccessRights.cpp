...
struct ProtoElement {
  ...
  friend bool operator<(const ProtoElement &left, const ProtoElement &right) {
    ...
    if (left.full_name.size() != right.full_name.size()) return left.full_name.size() < right.full_name.size();
...
class ProtoElements : public std::vector<ProtoElement> {
 ...
 private:
  size_t countElementsWithDifferenceInColumnOnly(size_t start) const {
    ...
    if ((start_element.full_name.size() != 3) || (start == size() - 1)) return 1;
  ...
  void appendResultWithElementsWithDifferenceInColumnOnly(size_t start, size_t count, AccessRightsElements &res) const {
    ...
    while (pbegin < pend) {
      while (pbegin < pend && !(pbegin->access_flags - handled_flags)) ++pbegin;

      while (pbegin < pend && !((pend - 1)->access_flags - handled_flags)) --pend;
      ...
      for (const auto *element = pbegin + 1; element != pend; ++element) {
        if (auto new_common_flags = (element->access_flags - handled_flags) & common_flags) common_flags = new_common_flags;
      }
      ...
      for (const auto *element = pbegin; element != pend; ++element) {
        if (((element->access_flags - handled_flags) & common_flags) == common_flags) back.columns.emplace_back(element->full_name[2]);
      }
...
AccessFlags getAllGrantableFlags(Level level) {
  switch (level) {
    case GLOBAL_LEVEL:
      return AccessFlags::allFlagsGrantableOnGlobalLevel();
    case DATABASE_LEVEL:
      return AccessFlags::allFlagsGrantableOnDatabaseLevel() | AccessFlags::allFlagsGrantableOnGlobalWithParameterLevel();
    case TABLE_LEVEL:
      return AccessFlags::allFlagsGrantableOnTableLevel() | AccessFlags::allSourceFlags() |
             AccessFlags::allFlagsGrantableOnGlobalWithParameterLevel();
    case COLUMN_LEVEL:
      return AccessFlags::allFlagsGrantableOnColumnLevel() | AccessFlags::allFlagsGrantableOnGlobalWithParameterLevel();
  }
  chassert(false);
}
}  // namespace
...
struct AccessRights::Node {
 public:
  ...
  class Iterator {
   public:
    ...
    const Node &operator*() const noexcept { return *current; }

    const Node *operator->() const noexcept { return current; }

    Iterator &operator++() noexcept {
      if (!root.children || root.children->empty()) return *this;

      if (iters.empty()) iters.emplace_back(root.children->begin(), root.children->end());

      while (!iters.empty()) {
        auto &[begin, end] = iters.back();
        if (begin == end) {
          iters.pop_back();
          if (!iters.empty()) ++iters.back().first;
          continue;
        }
    ...
    String getPath() const {
      String res;
      for (const auto &it : iters) res += it.first->node_name;
      return res;
    }
  ...
  AccessFlags min_flags_with_children;  /// min_flags_with_children = access & child[0].min_flags_with_children & ... &
                                        /// child[N-1].min_flags_with_children
  AccessFlags max_flags_with_children;  /// max_flags_with_children = access | child[0].max_flags_with_children | ... |
                                        /// child[N-1].max_flags_with_children
  ...
  void grant(const AccessFlags &flags_) {
    if constexpr (wildcard) wildcard_grant = true;
  ...
  bool isGranted(const AccessFlags &flags_, std::string_view name, const Args &...subnames) const {
    ...
    if (!flags_to_check) return true;
    if (!max_flags_with_children.contains(flags_to_check)) return false;
  ...
  bool isGranted(const AccessFlags &flags_, const std::vector<StringT> &names) const {
    ...
    for (const auto &name : names) {
      ...
      if (!child.isGranted<wildcard>(flags_to_check, name)) return false;
    }
    return true;
  }

  friend bool operator==(const Node &left, const Node &right) {
    if (left.node_name != right.node_name) return false;

    if (left.wildcard_grant != right.wildcard_grant) return false;

    if (left.flags != right.flags) return false;

    if (!left.children) return !right.children;

    if (!right.children) return false;
  ...
  bool contains(const Node &other) const {
    ...
    if (!flags.contains(other.flags)) return false;
    ...
    if (other.children) {
      for (const auto &other_child : *other.children) {
        ...
        if (!this_child.contains(other_child)) return false;
      }
    }
  ...
  void modifyFlags(const ModifyFlagsFunction &function, bool grant_option, bool &flags_added, bool &flags_removed) {
    ...
    if (flags_added || flags_removed) optimizeTree();
  }
  ...
  void dumpTree(WriteBuffer &buffer, const String &title, size_t depth = 0) const {
    ...
    if (children) {
      for (auto &child : *children) child.dumpTree(buffer, title, depth + 1);
    }
  }
  ...
  std::vector<Filter> getFilters(std::string_view parameter) {
    ...
    for (auto it = node.begin(); it != node.end(); ++it) res.emplace_back(it->flags, it.getPath());
 ...
 private:
  ...
  AccessFlags getChildAllGrantableFlags() const {
    return ::DB::getAllGrantableFlags(static_cast<Level>(level == Level::MAX ? level : (level + 1)));
  }
  ...
  Node &getLeaf(std::string_view path, Level level_ = Level::GLOBAL_LEVEL, bool return_parent_node = false) {
    if (path.empty() && return_parent_node) return *this;

    if (!children) children = std::make_shared<Container>();

    auto find_possible_prefix = [path](const Node &n) {
      if (path.empty()) return n.isLeaf();
    ...
    if (auto it = std::find_if(children->begin(), children->end(), find_possible_prefix); it != children->end()) {
      ...
      if (right == child.node_name.end()) return child.getLeaf(path.substr(child.node_name.size()), level_, return_parent_node);
    ...
    /// Child is a leaf.
    if (path.empty() || return_parent_node) return child;
  ...
  Node tryGetLeaf(std::string_view path, Level level_ = Level::GLOBAL_LEVEL, bool return_parent_node = false) const {
    ...
    if (path.empty() && return_parent_node) return this->makeLightCopy();
    ...
    if (children) {
      if (auto it = std::find_if(children->begin(), children->end(), find_possible_prefix); it != children->end()) {
        if (path.empty()) return it->makeLightCopy();

        auto child = it->makeLightCopy();
        const auto &[left, right] = std::mismatch(path.begin(), path.end(), child.node_name.begin(), child.node_name.end());

        if (right == child.node_name.end()) return child.tryGetLeaf(path.substr(child.node_name.size()), level_, return_parent_node);
    ...
    if (path.empty() || return_parent_node) return createChildNode(path, level_);
  ...
  bool canMergeChild(const Node &child) const { return ((flags & child.getAllGrantableFlags()) == child.flags); }

  bool canEraseChild(const Node &child) const { return canMergeChild(child) && !child.children; }

  void addGrantsRec(const AccessFlags &flags_) {
    if (auto flags_to_add = flags_ & getAllGrantableFlags()) {
      flags |= flags_to_add;
      if (children) {
        for (auto &child : *children) child.addGrantsRec(flags_to_add);
      }
    }
  }
  ...
  void removeGrantsRec(const AccessFlags &flags_) {
    flags &= ~flags_;
    if (children) {
      for (auto &child : *children) child.removeGrantsRec(flags_);
    }
  }

  /// Dumps GRANT/REVOKE elements from the tree.
  static void getElementsRec(ProtoElements &res, boost::container::small_vector<String, 3> full_name, const Node &node,
                             const AccessFlags &parent_flags, String path) {
    ...
    if (node.isLeaf() || node.wildcard_grant) {
      ...
      if (revokes) res.push_back(ProtoElement{revokes, new_full_name, false, true, node.wildcard_grant});

      if (grants) res.push_back(ProtoElement{grants, new_full_name, false, false, node.wildcard_grant});
    }
  ...
  static void getElementsRec(ProtoElements &res, boost::container::small_vector<String, 3> full_name, Node *node,
                             const AccessFlags &parent_flags, Node *node_go, const AccessFlags &parent_flags_go, String path) {
    Node *target_node = node;
    if (!target_node) target_node = node_go;

    auto grantable_flags =
        target_node ? ::DB::getAllGrantableFlags(target_node->level) : ::DB::getAllGrantableFlags(static_cast<Level>(full_name.size()));
    ...
    if (target_node && (target_node->isLeaf() || target_node->wildcard_grant)) {
      ...
      if (node && revokes) res.push_back(ProtoElement{revokes, new_full_name, false, true, node->wildcard_grant});

      if (node_go && revokes_go) res.push_back(ProtoElement{revokes_go, new_full_name, true, true, node_go->wildcard_grant});

      if (node && grants) res.push_back(ProtoElement{grants, new_full_name, false, false, node->wildcard_grant});

      if (node_go && grants_go) res.push_back(ProtoElement{grants_go, new_full_name, true, false, node_go->wildcard_grant});
    }
    ...
    if (node && node->children) {
      for (auto &child : *node->children) {
        ...
        if (node_go) child_node_go = &node_go->getLeaf(child.node_name, child.level, !child.isLeaf());
      ...
      for (auto &child : *node_go->children) {
        if (node && node->children) {
          auto starts_with = [&child](const Node &n) {
            if (child.isLeaf()) return n.isLeaf();
  ...
  void optimizeChildren() {
    ...
    if (children && children->empty()) children = nullptr;
  }

  /// Optimizes and compresses radix tree.
  void optimizeTree() {
    if (children) {
      for (auto &child : *children) child.optimizeTree();
  ...
  void optimizePath(std::string_view path) {
    if (children) {
      for (auto &child : *children) {
        if (path.empty() && child.isLeaf()) break;

        if (!child.isLeaf() && path.starts_with(child.node_name)) {
          if (!path.empty()) child.optimizePath(path.substr(child.node_name.size()));
  ...
  void calculateMinMaxFlags() {
    ...
    if (level != Level::COLUMN_LEVEL && isLeaf()) add_flags = getAllGrantableFlags() - getChildAllGrantableFlags();
  ...
  void modifyFlagsRec(const ModifyFlagsFunction &function, bool grant_option, bool &flags_added, bool &flags_removed) {
    ...
    auto new_flags = function(flags, min_flags_with_children, max_flags_with_children, level, grant_option, isLeaf() || wildcard_grant);
...
AccessRights::AccessRights(const AccessRights &src) { *this = src; }
...
AccessRights::AccessRights(const AccessFlags &access) { grant(access); }

AccessRights::AccessRights(const AccessRightsElement &element) { grant(element); }
...
bool AccessRights::isEmpty() const { return !root && !root_with_grant_option; }
...
void AccessRights::grantImpl(const AccessFlags &flags, const Args &...args) {
  auto helper = [&](std::unique_ptr<Node> &root_node) {
    if (!root_node) root_node = std::make_unique<Node>();
    root_node->grant<wildcard>(flags, args...);
    if (!root_node->flags && !root_node->children) root_node = nullptr;
  };
  helper(root);

  if constexpr (with_grant_option) helper(root_with_grant_option);
}
...
void AccessRights::grantImpl(const AccessRightsElement &element) {
  if (element.is_partial_revoke) throw Exception(ErrorCodes::BAD_ARGUMENTS, "A partial revoke should be revoked, not granted");
  if (element.wildcard) {
...
void AccessRights::grantImpl(const AccessRightsElements &elements) {
  for (const auto &element : elements) grantImpl<with_grant_option, wildcard>(element);
}
...
void AccessRights::grant(const AccessFlags &flags, std::string_view database, std::string_view table) {
  grantImpl<false, false>(flags, database, table);
}
void AccessRights::grant(const AccessFlags &flags, std::string_view database, std::string_view table, std::string_view column) {
  grantImpl<false, false>(flags, database, table, column);
}
void AccessRights::grant(const AccessFlags &flags, std::string_view database, std::string_view table,
                         const std::vector<std::string_view> &columns) {
  grantImpl<false, false>(flags, database, table, columns);
}
void AccessRights::grant(const AccessFlags &flags, std::string_view database, std::string_view table, const Strings &columns) {
  grantImpl<false, false>(flags, database, table, columns);
}
...
void AccessRights::grantWildcard(const AccessFlags &flags, std::string_view database, std::string_view table) {
  grantImpl<false, true>(flags, database, table);
}
void AccessRights::grantWildcard(const AccessFlags &flags, std::string_view database, std::string_view table, std::string_view column) {
  grantImpl<false, true>(flags, database, table, column);
}
void AccessRights::grantWildcard(const AccessFlags &flags, std::string_view database, std::string_view table,
                                 const std::vector<std::string_view> &columns) {
  grantImpl<false, true>(flags, database, table, columns);
}
void AccessRights::grantWildcard(const AccessFlags &flags, std::string_view database, std::string_view table, const Strings &columns) {
  grantImpl<false, true>(flags, database, table, columns);
}
...
void AccessRights::grantWithGrantOption(const AccessFlags &flags, std::string_view database, std::string_view table) {
  grantImpl<true, false>(flags, database, table);
}
void AccessRights::grantWithGrantOption(const AccessFlags &flags, std::string_view database, std::string_view table,
                                        std::string_view column) {
  grantImpl<true, false>(flags, database, table, column);
}
void AccessRights::grantWithGrantOption(const AccessFlags &flags, std::string_view database, std::string_view table,
                                        const std::vector<std::string_view> &columns) {
  grantImpl<true, false>(flags, database, table, columns);
}
void AccessRights::grantWithGrantOption(const AccessFlags &flags, std::string_view database, std::string_view table,
...
void AccessRights::grantWildcardWithGrantOption(const AccessFlags &flags, std::string_view database) {
  grantImpl<true, true>(flags, database);
}
void AccessRights::grantWildcardWithGrantOption(const AccessFlags &flags, std::string_view database, std::string_view table) {
  grantImpl<true, true>(flags, database, table);
}
void AccessRights::grantWildcardWithGrantOption(const AccessFlags &flags, std::string_view database, std::string_view table,
                                                std::string_view column) {
  grantImpl<true, true>(flags, database, table, column);
}
void AccessRights::grantWildcardWithGrantOption(const AccessFlags &flags, std::string_view database, std::string_view table,
                                                const std::vector<std::string_view> &columns) {
  grantImpl<true, true>(flags, database, table, columns);
}
void AccessRights::grantWildcardWithGrantOption(const AccessFlags &flags, std::string_view database, std::string_view table,
                                                const Strings &columns) {
  grantImpl<true, true>(flags, database, table, columns);
...
void AccessRights::revokeImpl(const AccessFlags &flags, const Args &...args) {
  ...
  if constexpr (!grant_option) helper(root);
}
...
void AccessRights::revokeImpl(const AccessRightsElements &elements) {
  for (const auto &element : elements) revokeImpl<grant_option, wildcard>(element);
}
...
void AccessRights::revoke(const AccessFlags &flags, std::string_view database, std::string_view table) {
  revokeImpl<false, false>(flags, database, table);
}
void AccessRights::revoke(const AccessFlags &flags, std::string_view database, std::string_view table, std::string_view column) {
  revokeImpl<false, false>(flags, database, table, column);
}
void AccessRights::revoke(const AccessFlags &flags, std::string_view database, std::string_view table,
                          const std::vector<std::string_view> &columns) {
  revokeImpl<false, false>(flags, database, table, columns);
}
void AccessRights::revoke(const AccessFlags &flags, std::string_view database, std::string_view table, const Strings &columns) {
  revokeImpl<false, false>(flags, database, table, columns);
}
...
void AccessRights::revokeWildcard(const AccessFlags &flags, std::string_view database, std::string_view table) {
  revokeImpl<false, true>(flags, database, table);
}
void AccessRights::revokeWildcard(const AccessFlags &flags, std::string_view database, std::string_view table, std::string_view column) {
  revokeImpl<false, true>(flags, database, table, column);
}
void AccessRights::revokeWildcard(const AccessFlags &flags, std::string_view database, std::string_view table,
                                  const std::vector<std::string_view> &columns) {
  revokeImpl<false, true>(flags, database, table, columns);
}
void AccessRights::revokeWildcard(const AccessFlags &flags, std::string_view database, std::string_view table, const Strings &columns) {
  revokeImpl<false, true>(flags, database, table, columns);
}
...
void AccessRights::revokeGrantOption(const AccessFlags &flags, std::string_view database, std::string_view table) {
  revokeImpl<true, false>(flags, database, table);
}
void AccessRights::revokeGrantOption(const AccessFlags &flags, std::string_view database, std::string_view table, std::string_view column) {
  revokeImpl<true, false>(flags, database, table, column);
}
void AccessRights::revokeGrantOption(const AccessFlags &flags, std::string_view database, std::string_view table,
                                     const std::vector<std::string_view> &columns) {
  revokeImpl<true, false>(flags, database, table, columns);
}
void AccessRights::revokeGrantOption(const AccessFlags &flags, std::string_view database, std::string_view table, const Strings &columns) {
  revokeImpl<true, false>(flags, database, table, columns);
}
...
void AccessRights::revokeWildcardGrantOption(const AccessFlags &flags, std::string_view database) {
  revokeImpl<true, true>(flags, database);
}
void AccessRights::revokeWildcardGrantOption(const AccessFlags &flags, std::string_view database, std::string_view table) {
  revokeImpl<true, true>(flags, database, table);
}
void AccessRights::revokeWildcardGrantOption(const AccessFlags &flags, std::string_view database, std::string_view table,
                                             std::string_view column) {
  revokeImpl<true, true>(flags, database, table, column);
}
void AccessRights::revokeWildcardGrantOption(const AccessFlags &flags, std::string_view database, std::string_view table,
                                             const std::vector<std::string_view> &columns) {
  revokeImpl<true, true>(flags, database, table, columns);
}
void AccessRights::revokeWildcardGrantOption(const AccessFlags &flags, std::string_view database, std::string_view table,
                                             const Strings &columns) {
  revokeImpl<true, true>(flags, database, table, columns);
...
AccessRightsElements AccessRights::getElements() const {
  if (!root) return {};
  if (!root_with_grant_option) return root->getElements().getResult();
  return Node::getElements(*root, *root_with_grant_option).getResult();
}

String AccessRights::toString() const { return getElements().toString(); }

AccessRights AccessRights::getGrantableRights() const {
  AccessRights result;
  for (auto &element : getElements()) {
    if (!element.grant_option && !element.is_partial_revoke) continue;
...
bool AccessRights::isGrantedImpl(const AccessFlags &flags, const Args &...args) const {
  auto helper = [&](const std::unique_ptr<Node> &root_node) -> bool {
    if (!root_node) return flags.isEmpty();
...
bool AccessRights::containsImpl(const AccessRights &other) const {
  auto helper = [&](const std::unique_ptr<Node> &root_node, const std::unique_ptr<Node> &other_root_node) -> bool {
    if (!root_node) return !other_root_node;
    if (!other_root_node) return true;
...
bool AccessRights::isGrantedImplHelper(const AccessRightsElement &element) const {
  assert(!element.grant_option || grant_option);
  if (element.isGlobalWithParameter()) {
    if (element.anyParameter()) return isGrantedImpl<grant_option, wildcard>(element.access_flags);

    return isGrantedImpl<grant_option, wildcard>(element.access_flags, element.parameter);
  }
  if (element.anyDatabase()) return isGrantedImpl<grant_option, wildcard>(element.access_flags);
  if (element.anyTable()) return isGrantedImpl<grant_option, wildcard>(element.access_flags, element.database);
  if (element.anyColumn()) return isGrantedImpl<grant_option, wildcard>(element.access_flags, element.database, element.table);
...
bool AccessRights::isGrantedImpl(const AccessRightsElement &element) const {
  if (element.wildcard) {
    if (element.grant_option) return isGrantedImplHelper<true, true>(element);

    return isGrantedImplHelper<grant_option, true>(element);
  }

  if (element.grant_option) return isGrantedImplHelper<true, wildcard>(element);
...
bool AccessRights::isGrantedImpl(const AccessRightsElements &elements) const {
  for (const auto &element : elements)
    if (!isGrantedImpl<grant_option, wildcard>(element)) return false;
  return true;
}

bool AccessRights::isGranted(const AccessFlags &flags) const { return isGrantedImpl<false, false>(flags); }
bool AccessRights::isGranted(const AccessFlags &flags, std::string_view database) const {
  return isGrantedImpl<false, false>(flags, database);
}
bool AccessRights::isGranted(const AccessFlags &flags, std::string_view database, std::string_view table) const {
  return isGrantedImpl<false, false>(flags, database, table);
}
bool AccessRights::isGranted(const AccessFlags &flags, std::string_view database, std::string_view table, std::string_view column) const {
  return isGrantedImpl<false, false>(flags, database, table, column);
}
bool AccessRights::isGranted(const AccessFlags &flags, std::string_view database, std::string_view table,
                             const std::vector<std::string_view> &columns) const {
  return isGrantedImpl<false, false>(flags, database, table, columns);
}
bool AccessRights::isGranted(const AccessFlags &flags, std::string_view database, std::string_view table, const Strings &columns) const {
  return isGrantedImpl<false, false>(flags, database, table, columns);
}
...
bool AccessRights::isGrantedWildcard(const AccessFlags &flags, std::string_view database) const {
  return isGrantedImpl<false, true>(flags, database);
}
bool AccessRights::isGrantedWildcard(const AccessFlags &flags, std::string_view database, std::string_view table) const {
  return isGrantedImpl<false, true>(flags, database, table);
}
bool AccessRights::isGrantedWildcard(const AccessFlags &flags, std::string_view database, std::string_view table,
                                     std::string_view column) const {
  return isGrantedImpl<false, true>(flags, database, table, column);
}
bool AccessRights::isGrantedWildcard(const AccessFlags &flags, std::string_view database, std::string_view table,
                                     const std::vector<std::string_view> &columns) const {
  return isGrantedImpl<false, true>(flags, database, table, columns);
}
bool AccessRights::isGrantedWildcard(const AccessFlags &flags, std::string_view database, std::string_view table,
...
bool AccessRights::hasGrantOption(const AccessFlags &flags, std::string_view database) const {
  return isGrantedImpl<true, false>(flags, database);
}
bool AccessRights::hasGrantOption(const AccessFlags &flags, std::string_view database, std::string_view table) const {
  return isGrantedImpl<true, false>(flags, database, table);
}
bool AccessRights::hasGrantOption(const AccessFlags &flags, std::string_view database, std::string_view table,
                                  std::string_view column) const {
  return isGrantedImpl<true, false>(flags, database, table, column);
}
bool AccessRights::hasGrantOption(const AccessFlags &flags, std::string_view database, std::string_view table,
                                  const std::vector<std::string_view> &columns) const {
  return isGrantedImpl<true, false>(flags, database, table, columns);
}
bool AccessRights::hasGrantOption(const AccessFlags &flags, std::string_view database, std::string_view table,
                                  const Strings &columns) const {
  return isGrantedImpl<true, false>(flags, database, table, columns);
...
bool AccessRights::hasGrantOptionWildcard(const AccessFlags &flags, std::string_view database) const {
  return isGrantedImpl<true, true>(flags, database);
}
bool AccessRights::hasGrantOptionWildcard(const AccessFlags &flags, std::string_view database, std::string_view table) const {
  return isGrantedImpl<true, true>(flags, database, table);
}
bool AccessRights::hasGrantOptionWildcard(const AccessFlags &flags, std::string_view database, std::string_view table,
                                          std::string_view column) const {
  return isGrantedImpl<true, true>(flags, database, table, column);
}
bool AccessRights::hasGrantOptionWildcard(const AccessFlags &flags, std::string_view database, std::string_view table,
                                          const std::vector<std::string_view> &columns) const {
  return isGrantedImpl<true, true>(flags, database, table, columns);
}
bool AccessRights::hasGrantOptionWildcard(const AccessFlags &flags, std::string_view database, std::string_view table,
                                          const Strings &columns) const {
  return isGrantedImpl<true, true>(flags, database, table, columns);
...
void AccessRights::makeUnion(const AccessRights &other) {
  auto helper = [](std::unique_ptr<Node> &root_node, const std::unique_ptr<Node> &other_root_node) {
    if (!root_node) {
      if (other_root_node) root_node = std::make_unique<Node>(*other_root_node);
      return;
    }
    if (other_root_node) {
...
void AccessRights::makeDifference(const AccessRights &other) {
  auto helper = [](std::unique_ptr<Node> &root_node, const std::unique_ptr<Node> &other_root_node) {
    if (!root_node || !other_root_node) return;
...
std::vector<AccessRights::Filter> AccessRights::getFilters(std::string_view parameter) const {
  if (root) return root->getFilters(parameter);
...
void AccessRights::modifyFlags(const ModifyFlagsFunction &function) {
  ...
  if (flags_removed && root_with_grant_option) root_with_grant_option->makeIntersection(*root);
...
void AccessRights::dumpTree(WriteBuffer &buffer) const {
  if (root) {
    ...
    if (root_with_grant_option) root->dumpTree(buffer, "go");
  }
}

std::vector<String> AccessRights::dumpNodes() const {
  std::vector<String> result;
  for (auto it = root->begin(); it != root->end(); ++it) result.push_back(it.getPath());
