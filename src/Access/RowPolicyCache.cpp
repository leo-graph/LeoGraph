...
class FiltersMixer {
 public:
  ...
  ASTPtr getResult(bool users_without_row_policies_can_read_rows) && {
    ...
    ASTPtr result;
    if (!restrictions.empty()) result = makeASTForLogicalAnd(std::move(restrictions));

    if (result) {
      bool value;
      if (tryGetLiteralBool(result.get(), value) && value) result = nullptr;  /// The condition is always true, no need to check it.
    }
...
void RowPolicyCache::PolicyInfo::setPolicy(const RowPolicyPtr& policy_) {
  ...
  for (auto filter_type : collections::range(0, RowPolicyFilterType::MAX)) {
    ...
    try {
      ...
      tryLogCurrentException(getLogger("RowPolicy"), String("Could not parse the condition ") + toString(filter_type) + " of row policy " +
                                                         backQuote(policy->getName()));
    }
  }
}

RowPolicyCache::RowPolicyCache(const AccessControl& access_control_) : access_control(access_control_) {}

RowPolicyCache::~RowPolicyCache() = default;

std::shared_ptr<const EnabledRowPolicies> RowPolicyCache::getEnabledRowPolicies(const UUID& user_id,
...
void RowPolicyCache::ensureAllRowPoliciesRead() {
  ...
  if (all_policies_read) return;
  all_policies_read = true;

  subscription = access_control.subscribeForChanges<RowPolicy>([&](const UUID& id, const AccessEntityPtr& entity) {
    if (entity)
...
void RowPolicyCache::rowPolicyAddedOrChanged(const UUID& policy_id, const RowPolicyPtr& new_policy) {
  ...
  if (it == all_policies.end()) {
    ...
    if (it->second.policy == new_policy) return;
  }
    ...
    if (info.isForDatabase()) {
      ...
      for (auto filter_type : collections::range(0, RowPolicyFilterType::MAX)) {
        ...
        if (info.parsed_filters[filter_type_i]) {
          MixedFiltersKey key{info.database_and_table_name->first, info.database_and_table_name->second, filter_type};
    ...
    if (!info.isForDatabase()) {
      ...
      for (auto filter_type : collections::range(0, RowPolicyFilterType::MAX)) {
        ...
        if (info.parsed_filters[filter_type_i]) {
          ...
          if (table_it == table_mixers.end()) {  /// no exact match - create new mixer
