...
class DiskAccessStorage : public IAccessStorage {
 public:
  ...
  DiskAccessStorage(const String& storage_name_, const String& directory_path_, AccessChangesNotifier& changes_notifier_, bool readonly_,
                    bool allow_backup_);
 ...
 private:
  ...
  bool insertNoLock(const UUID& id, const AccessEntityPtr& new_entity, bool replace_if_exists, bool throw_if_exists, UUID* conflicting_id,
                    bool write_on_disk) TSA_REQUIRES(mutex);
  ...
  struct EntityOnDisk : public IAccessEntity {
   ...
   protected:
    bool equal(const IAccessEntity& other) const override { return IAccessEntity::equal(other) && typeid(other) == typeid(EntityOnDisk); }
