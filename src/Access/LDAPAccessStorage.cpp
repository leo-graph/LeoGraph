...
LDAPAccessStorage::LDAPAccessStorage(const String &storage_name_, AccessControl &access_control_,
...
void LDAPAccessStorage::setConfiguration(const Poco::Util::AbstractConfiguration &config, const String &prefix) {
  ...
  // TODO: switch to passing config as a ConfigurationView and remove this extra prefix once a version of Poco with proper implementation is
  // available.
  ...
  if (!has_server) throw Exception(ErrorCodes::BAD_ARGUMENTS, "Missing 'server' field for LDAP user directory");

  const auto ldap_server_name_cfg = config.getString(prefix_str + "server");
  if (ldap_server_name_cfg.empty()) throw Exception(ErrorCodes::BAD_ARGUMENTS, "Empty 'server' field for LDAP user directory");
  ...
  role_change_subscription = access_control.subscribeForChanges<Role>(
      [this](const UUID &id, const AccessEntityPtr &entity) { this->processRoleChange(id, entity); });
}
  ...
  if (common_role_names.contains(role_name)) {
    ...
    if (it != users_per_roles.end()) {
      ...
      for (const auto &user_name : user_names) {
        if (const auto user_id = memory_storage.find<User>(user_name)) user_ids.emplace_back(*user_id);
      }
    }
  }
...
void LDAPAccessStorage::assignRolesNoLock(User &user, const LDAPClient::SearchResultsList &external_roles) const {
  ...
  auto grant_role = [this, &user_name, &granted_roles](const String &role_name, const bool common) {
    ...
    if (it != granted_role_ids.end()) {
      ...
      LOG_WARNING(getLogger(), "Unable to grant {} role '{}' to user '{}': role not found", (common ? "common" : "mapped"), role_name,
                  user_name);
    }
  };
  ...
  for (const auto &old_role_name : old_role_names) {
    if (local_role_names.contains(old_role_name)) continue;

    const auto rit = users_per_roles.find(old_role_name);
    if (rit == users_per_roles.end()) continue;

    auto &user_names = rit->second;
    user_names.erase(user_name);

    if (!user_names.empty()) continue;

    users_per_roles.erase(rit);

    if (common_role_names.contains(old_role_name)) continue;

    const auto iit = granted_role_ids.find(old_role_name);
    if (iit == granted_role_ids.end()) continue;
...
void LDAPAccessStorage::updateAssignedRolesNoLock(const UUID &id, const String &user_name,
                                                  const LDAPClient::SearchResultsList &external_roles) const {
  ...
  if (it != users_external_roles.end() && it->second == external_roles) return;
...
std::set<String> LDAPAccessStorage::mapExternalRolesNoLock(const LDAPClient::SearchResultsList &external_roles) const {
  ...
  if (external_roles.empty()) return role_names;

  if (external_roles.size() != role_search_params.size()) throw Exception(ErrorCodes::BAD_ARGUMENTS, "Unable to map external roles");
...
bool LDAPAccessStorage::areLDAPCredentialsValidNoLock(const User &user, const Credentials &credentials,
  ...
  if (credentials.getUserName() != user.getName()) return false;

  if (typeid_cast<const AlwaysAllowCredentials *>(&credentials)) return true;
...
const char *LDAPAccessStorage::getStorageType() const { return STORAGE_TYPE; }
...
String LDAPAccessStorage::getStorageParamsJSON() const {
  ...
  for (const auto &role_mapping : role_search_params) {
    ...
    switch (role_mapping.scope) {
      case LDAPClient::SearchParams::Scope::BASE:
        scope = "base";
        break;
      case LDAPClient::SearchParams::Scope::ONE_LEVEL:
        scope = "one_level";
        break;
      case LDAPClient::SearchParams::Scope::SUBTREE:
        scope = "subtree";
        break;
      case LDAPClient::SearchParams::Scope::CHILDREN:
        scope = "children";
        break;
    }
...
std::optional<AuthResult> LDAPAccessStorage::authenticateImpl(const Credentials &credentials, const Poco::Net::IPAddress &address,
                                                              const ExternalAuthenticators &external_authenticators,
  ...
  if (id)
    return AuthResult{
        .user_id = *id, .authentication_data = AuthenticationData(AuthenticationType::LDAP), .user_name = credentials.getUserName()};
  return std::nullopt;
}
