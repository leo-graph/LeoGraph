...
String UsersConfigAccessStorage::getStorageParamsJSON() const {
  ...
  if (!path.empty()) json.set("path", path);
...
bool UsersConfigAccessStorage::isPathEqual(const String& path_) const { return getPath() == path_; }
...
void UsersConfigAccessStorage::parseFromConfig(const Poco::Util::AbstractConfiguration& config) {
  try {
    ...
    for (const auto& entity : parser.parseQuotas(config)) all_entities.emplace_back(UsersConfigParser::generateID(*entity), entity);

    for (const auto& entity : parser.parseRowPolicies(config)) all_entities.emplace_back(UsersConfigParser::generateID(*entity), entity);
...
void UsersConfigAccessStorage::load(const String& users_config_path, const String& include_from_path, const String& preprocessed_dir,
  ...
  config_reloader = std::make_unique<ConfigReloader>(
      users_config_path, std::vector{{include_from_path}}, preprocessed_dir, std::move(zk_node_cache), std::make_shared<Poco::Event>(),
...
void UsersConfigAccessStorage::startPeriodicReloading() {
  std::lock_guard lock{load_mutex};
  if (config_reloader) config_reloader->start();
}

void UsersConfigAccessStorage::stopPeriodicReloading() {
  std::lock_guard lock{load_mutex};
  if (config_reloader) config_reloader->stop();
}

void UsersConfigAccessStorage::reload(ReloadMode /* reload_mode */) {
  std::lock_guard lock{load_mutex};
  if (config_reloader) config_reloader->reload();
}
...
std::vector<UUID> UsersConfigAccessStorage::findAllImpl(AccessEntityType type) const { return memory_storage.findAll(type); }

bool UsersConfigAccessStorage::exists(const UUID& id) const { return memory_storage.exists(id); }
...
std::optional<std::pair<String, AccessEntityType>> UsersConfigAccessStorage::readNameWithTypeImpl(const UUID& id,
