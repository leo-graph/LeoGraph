...
class AccessTypeToStringConverter {
 public:
  ...
  std::string_view convert(AccessType type) const { return access_type_to_string_mapping[static_cast<size_t>(type)]; }

 private:
  AccessTypeToStringConverter() {
#define ACCESS_TYPE_TO_STRING_CONVERTER_ADD_TO_MAPPING(name, aliases, node_type, parent_group_name) addToMapping(AccessType::name, #name);
...
std::string_view toString(AccessType type) { return AccessTypeToStringConverter::instance().convert(type); }
