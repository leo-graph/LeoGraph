...
IPAddress toIPv6(const IPAddress &ip) {
  ...
  // ::ffff:127.XX.XX.XX -> ::1
  if ((v6 & IPAddress(104, IPAddress::IPv6)) == IPAddress("::ffff:127.0.0.0")) v6 = IPAddress{"::1"};

  return v6;
}

IPSubnet toIPv6(const IPSubnet &subnet) { return IPSubnet(toIPv6(subnet.getPrefix()), subnet.getMask()); }
...
bool isAddressOfHost(const IPAddress &address, const String &host) {
  ...
  for (const auto &addr : host_addresses) {
    if (addr.family() == IPAddress::Family::IPv4 && addr_v6 == toIPv6(addr)) return true;
    if (addr.family() == IPAddress::Family::IPv6 && addr_v6 == addr) return true;
  }
...
std::vector<IPAddress> getAddressesOfLocalhostImpl() {
  ...
  SCOPE_EXIT({
    if (ifa_begin) freeifaddrs(ifa_begin);
  });

  int err = getifaddrs(&ifa_begin);
  if (err) return {IPAddress{"::1"}};
...
std::unordered_set<String> getHostsByAddress(const IPAddress &address) {
  ...
  if (hosts.empty()) throw Exception(ErrorCodes::DNS_ERROR, "{} could not be resolved", address.toString());
...
void parseLikePattern(const String &pattern, std::optional<IPSubnet> &subnet, std::optional<String> &name,
...
void AllowedClientHosts::IPSubnet::set(const IPAddress &prefix_, const IPAddress &mask_) {
  ...
  if (prefix.family() != mask.family()) {
    if (prefix.family() == IPAddress::IPv4) prefix = IPAddress("::ffff:" + prefix.toString());

    if (mask.family() == IPAddress::IPv4) mask = IPAddress(96, IPAddress::IPv6) | IPAddress("::ffff:" + mask.toString());
  }
...
void AllowedClientHosts::IPSubnet::set(const IPAddress &address) { set(address, address.length() * 8); }
...
String AllowedClientHosts::IPSubnet::toString() const {
  ...
  if (isMaskAllBitsOne()) return prefix.toString();
  if (IPAddress{prefix_length, mask.family()} == mask) return fs::path(prefix.toString()) / std::to_string(prefix_length);
  return fs::path(prefix.toString()) / mask.toString();
}

bool AllowedClientHosts::IPSubnet::isMaskAllBitsOne() const { return mask == IPAddress(mask.length() * 8, mask.family()); }
...
void AllowedClientHosts::addLocalHost() { local_host = true; }

void AllowedClientHosts::removeLocalHost() { local_host = false; }
...
void AllowedClientHosts::add(const AllowedClientHosts &other) {
  ...
  if (other.containsLocalHost()) addLocalHost();
  for (const IPAddress &address : other.getAddresses()) addAddress(address);
  for (const IPSubnet &subnet : other.getSubnets()) addSubnet(subnet);
  for (const String &name : other.getNames()) addName(name);
  for (const String &name_regexp : other.getNameRegexps()) addNameRegexp(name_regexp);
  for (const String &like_pattern : other.getLikePatterns()) addLikePattern(like_pattern);
}
...
void AllowedClientHosts::remove(const AllowedClientHosts &other) {
  ...
  if (other.containsLocalHost()) removeLocalHost();
  for (const IPAddress &address : other.getAddresses()) removeAddress(address);
  for (const IPSubnet &subnet : other.getSubnets()) removeSubnet(subnet);
  for (const String &name : other.getNames()) removeName(name);
  for (const String &name_regexp : other.getNameRegexps()) removeNameRegexp(name_regexp);
  for (const String &like_pattern : other.getLikePatterns()) removeLikePattern(like_pattern);
}
...
bool AllowedClientHosts::contains(const IPAddress &client_address) const {
  ...
  auto check_name = [&](const String &name_) {
    ...
    try {
      ...
      LOG_WARNING(getLogger("AddressPatterns"), "Failed to check if the allowed client hosts contain address {}. {}, code = {}",
  ...
  for (const String &name_regexp : name_regexps)
    if (check_name_regexp(name_regexp)) return true;
  ...
  auto check_like_pattern = [&](const String &pattern) {
    ...
    if (subnet) return check_subnet(*subnet);
    if (name) return check_name(*name);
    if (name_regexp) return check_name_regexp(*name_regexp);
    return false;
  };

  for (const String &like_pattern : like_patterns)
    if (check_like_pattern(like_pattern)) return true;
