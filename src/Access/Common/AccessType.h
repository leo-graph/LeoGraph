  ...
  class EnumHolder##NAME {                                                                                      \
   public:                                                                                                      \
    ...
    const std::map<String, NAME>& getAliasesMap() const { return aliases; }                                     \
                                                                                                                \
   private:                                                                                                     \
    EnumHolder##NAME() { M(ACCESS_TYPE_OBJECT_ADD_TO_MAPPING) }                                                 \
                                                                                                                \
    ...
    std::map<String, NAME> aliases;                                                                             \
                                                                                                                ...
                                                                                                                \
  inline auto toString##NAME(NAME type) { return EnumHolder##NAME::instance().toString(type); }                 \
                                                                                                                \
  inline auto fromString##NAME(const String& name) { return EnumHolder##NAME::instance().fromString(name); }    \
  inline auto unify##NAME(const String& name) { return toString##NAME(fromString##NAME(name)); }                \
                                                                                                                \
  inline auto& getAliasesMap##NAME() { return EnumHolder##NAME::instance().getAliasesMap(); }                   \
                                                                                                                \
  inline auto validate##NAME(const String& name) { return EnumHolder##NAME::instance().validate(name); }
...
#define APPLY_FOR_ACCESS_TYPES(M)                                                                                                          \
  ...
  M(ALTER_SETTINGS, "ALTER SETTING, ALTER MODIFY SETTING, MODIFY SETTING, RESET SETTING", TABLE,                                           \
  ...
  M(ALTER_EXECUTE, "ALTER TABLE EXECUTE", TABLE, ALTER_TABLE)                                                                              \
                                                                                                                                           \
  M(ALTER_DATABASE_SETTINGS, "ALTER DATABASE SETTING, ALTER MODIFY DATABASE SETTING, MODIFY DATABASE SETTING", DATABASE,                   \
    ALTER_DATABASE)                                                       /* allows to execute ALTER MODIFY SETTING */                     \
  ...
  M(ALLOW_SQL_SECURITY_NONE, "CREATE SQL SECURITY NONE, ALLOW SQL SECURITY NONE, SQL SECURITY NONE, SECURITY NONE", GLOBAL,                \
    ACCESS_MANAGEMENT)                                                                                                                     \
  ...
  M(SYSTEM_DROP_CONNECTIONS_CACHE, "SYSTEM CLEAR CONNECTIONS CACHE, SYSTEM DROP CONNECTIONS CACHE, DROP CONNECTIONS CACHE", GLOBAL,        \
    SYSTEM_DROP_CACHE)                                                                                                                     \
  ...
  M(SYSTEM_DROP_ICEBERG_METADATA_CACHE, "SYSTEM CLEAR ICEBERG_METADATA_CACHE, SYSTEM DROP ICEBERG_METADATA_CACHE", GLOBAL,                 \
    SYSTEM_DROP_CACHE)                                                                                                                     \
  M(SYSTEM_DROP_PARQUET_METADATA_CACHE, "SYSTEM DROP PARQUET_METADATA_CACHE", GLOBAL, SYSTEM_DROP_CACHE)                                   \
  M(SYSTEM_PREWARM_PRIMARY_INDEX_CACHE, "SYSTEM PREWARM PRIMARY INDEX, PREWARM PRIMARY INDEX CACHE, PREWARM PRIMARY INDEX", GLOBAL,        \
    SYSTEM_DROP_CACHE)                                                                                                                     \
  M(SYSTEM_DROP_PRIMARY_INDEX_CACHE,                                                                                                       \
    "SYSTEM CLEAR PRIMARY INDEX CACHE, SYSTEM DROP PRIMARY INDEX, DROP PRIMARY INDEX CACHE, DROP PRIMARY INDEX", GLOBAL,                   \
    SYSTEM_DROP_CACHE)                                                                                                                     \
  M(SYSTEM_DROP_UNCOMPRESSED_CACHE,                                                                                                        \
    "SYSTEM CLEAR UNCOMPRESSED CACHE, SYSTEM DROP UNCOMPRESSED, DROP UNCOMPRESSED CACHE, DROP UNCOMPRESSED", GLOBAL, SYSTEM_DROP_CACHE)    \
  M(SYSTEM_DROP_VECTOR_SIMILARITY_INDEX_CACHE,                                                                                             \
    "SYSTEM CLEAR VECTOR SIMILARITY INDEX CACHE, SYSTEM DROP VECTOR SIMILARITY INDEX CACHE, DROP VECTOR SIMILARITY INDEX CACHE", GLOBAL,   \
    SYSTEM_DROP_CACHE)                                                                                                                     \
  M(SYSTEM_DROP_TEXT_INDEX_TOKENS_CACHE,                                                                                                   \
    "SYSTEM CLEAR TEXT INDEX TOKENS CACHE, SYSTEM DROP TEXT INDEX TOKENS CACHE, DROP TEXT INDEX TOKENS CACHE", GLOBAL, SYSTEM_DROP_CACHE)  \
  M(SYSTEM_DROP_TEXT_INDEX_HEADER_CACHE,                                                                                                   \
    "SYSTEM CLEAR TEXT INDEX HEADER CACHE, SYSTEM DROP TEXT INDEX HEADER CACHE, DROP TEXT INDEX HEADER CACHE", GLOBAL, SYSTEM_DROP_CACHE)  \
  M(SYSTEM_DROP_TEXT_INDEX_POSTINGS_CACHE,                                                                                                 \
    "SYSTEM CLEAR TEXT INDEX POSTINGS CACHE, SYSTEM DROP TEXT INDEX POSTINGS CACHE, DROP TEXT INDEX POSTINGS CACHE", GLOBAL,               \
    SYSTEM_DROP_CACHE)                                                                                                                     \
  M(SYSTEM_DROP_TEXT_INDEX_CACHES, "SYSTEM CLEAR TEXT INDEX CACHES, SYSTEM DROP TEXT INDEX CACHES, DROP TEXT INDEX CACHES", GLOBAL,        \
    SYSTEM_DROP_CACHE)                                                                                                                     \
  M(SYSTEM_DROP_MMAP_CACHE, "SYSTEM CLEAR MMAP CACHE, SYSTEM DROP MMAP, DROP MMAP CACHE, DROP MMAP", GLOBAL, SYSTEM_DROP_CACHE)            \
  M(SYSTEM_DROP_QUERY_CONDITION_CACHE,                                                                                                     \
    "SYSTEM CLEAR QUERY CONDITION CACHE, SYSTEM DROP QUERY CONDITION, DROP QUERY CONDITION CACHE, DROP QUERY CONDITION", GLOBAL,           \
    SYSTEM_DROP_CACHE)                                                                                                                     \
  ...
  M(SYSTEM_DROP_COMPILED_EXPRESSION_CACHE,                                                                                                 \
    "SYSTEM CLEAR COMPILED EXPRESSION CACHE, SYSTEM DROP COMPILED EXPRESSION, DROP COMPILED EXPRESSION CACHE, DROP COMPILED EXPRESSIONS",  \
    GLOBAL, SYSTEM_DROP_CACHE)                                                                                                             \
  M(SYSTEM_DROP_FILESYSTEM_CACHE, "SYSTEM CLEAR FILESYSTEM CACHE, SYSTEM DROP FILESYSTEM CACHE, DROP FILESYSTEM CACHE", GLOBAL,            \
    SYSTEM_DROP_CACHE)                                                                                                                     \
  M(SYSTEM_DROP_DISTRIBUTED_CACHE, "SYSTEM CLEAR DISTRIBUTED CACHE, SYSTEM DROP DISTRIBUTED CACHE, DROP DISTRIBUTED CACHE", GLOBAL,        \
    SYSTEM_DROP_CACHE)                                                                                                                     \
  ...
  M(SYSTEM_DROP_FORMAT_SCHEMA_CACHE, "SYSTEM CLEAR FORMAT SCHEMA CACHE, SYSTEM DROP FORMAT SCHEMA CACHE, DROP FORMAT SCHEMA CACHE",        \
    GLOBAL, SYSTEM_DROP_CACHE)                                                                                                             \
  ...
  M(SYSTEM_RELOAD_EMBEDDED_DICTIONARIES, "RELOAD EMBEDDED DICTIONARIES", GLOBAL,                                                           \
  ...
  M(SYSTEM_VIEWS,                                                                                                                          \
    "SYSTEM REFRESH VIEW, SYSTEM START VIEWS, SYSTEM STOP VIEWS, SYSTEM START VIEW, SYSTEM STOP VIEW, SYSTEM CANCEL VIEW, REFRESH VIEW, "  \
    "START VIEWS, STOP VIEWS, START VIEW, STOP VIEW, CANCEL VIEW",                                                                         \
    VIEW, SYSTEM)                                                                                                                          \
  M(SYSTEM_DISTRIBUTED_SENDS,                                                                                                              \
    "SYSTEM STOP DISTRIBUTED SENDS, SYSTEM START DISTRIBUTED SENDS, STOP DISTRIBUTED SENDS, START DISTRIBUTED SENDS", TABLE, SYSTEM_SENDS) \
  M(SYSTEM_REPLICATED_SENDS, "SYSTEM STOP REPLICATED SENDS, SYSTEM START REPLICATED SENDS, STOP REPLICATED SENDS, START REPLICATED SENDS", \
    TABLE, SYSTEM_SENDS)                                                                                                                   \
  M(SYSTEM_SENDS, "SYSTEM STOP SENDS, SYSTEM START SENDS, STOP SENDS, START SENDS", GROUP, SYSTEM)                                         \
  M(SYSTEM_REPLICATION_QUEUES,                                                                                                             \
    "SYSTEM STOP REPLICATION QUEUES, SYSTEM START REPLICATION QUEUES, STOP REPLICATION QUEUES, START REPLICATION QUEUES", TABLE, SYSTEM)   \
  M(SYSTEM_VIRTUAL_PARTS_UPDATE,                                                                                                           \
    "SYSTEM STOP VIRTUAL PARTS UPDATE, SYSTEM START VIRTUAL PARTS UPDATE, STOP VIRTUAL PARTS UPDATE, START VIRTUAL PARTS UPDATE", TABLE,   \
    SYSTEM)                                                                                                                                \
  M(SYSTEM_REDUCE_BLOCKING_PARTS,                                                                                                          \
    "SYSTEM STOP REDUCE BLOCKING PARTS, SYSTEM START REDUCE BLOCKING PARTS, STOP REDUCE BLOCKING PARTS, START REDUCE BLOCKING PARTS",      \
    TABLE, SYSTEM)                                                                                                                         \
  ...
  M(SYSTEM_THREAD_FUZZER, "SYSTEM START THREAD FUZZER, SYSTEM STOP THREAD FUZZER, START THREAD FUZZER, STOP THREAD FUZZER", GLOBAL,        \
    SYSTEM)                                                                                                                                \
  ...
  M(SYSTEM_JEMALLOC,                                                                                                                       \
    "SYSTEM JEMALLOC PURGE, SYSTEM JEMALLOC ENABLE PROFILE, SYSTEM JEMALLOC DISABLE PROFILE, SYSTEM JEMALLOC FLUSH PROFILE", GLOBAL,       \
    SYSTEM)                                                                                                                                \
  ...
  M(SYSTEM, "", GROUP, ALL) /* allows to execute SYSTEM {SHUTDOWN|RELOAD CONFIG|...} */                                                    \
                                                                                                                                           \
  M(dictGet, "dictHas, dictGetHierarchy, dictIsIn", DICTIONARY,                                                                            \
    ALL)                                            /* allows to execute functions dictGet(), dictHas(), dictGetHierarchy(), dictIsIn() */ \
  M(displaySecretsInShowAndSelect, "", GLOBAL, ALL) /* allows to show plaintext secrets in SELECT and SHOW queries.                        \
                                                       display_secrets_in_show_and_select format and server settings must be turned on */  \
                                                                                                                                           \
...
#define DECLARE_ACCESS_TYPE_ENUM_CONST(name, aliases, node_type, parent_group_name) name,
