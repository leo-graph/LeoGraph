...
String toString(AccessEntityType type) { return AccessEntityTypeInfo::get(type).name; }
...
const AccessEntityTypeInfo& AccessEntityTypeInfo::get(AccessEntityType type_) {
  static constexpr auto make_info = [](const char* raw_name_, const char* plural_raw_name_, char unique_char_, int not_found_error_code_) {
    ...
    return AccessEntityTypeInfo{raw_name_,
                                plural_raw_name_,
                                std::move(init_names[0]),
                                std::move(init_aliases[0]),
                                std::move(init_names[1]),
                                std::move(init_aliases[1]),
                                std::move(init_name_for_output_with_entity_name),
                                unique_char_,
                                not_found_error_code_};
  };
...
AccessEntityType AccessEntityTypeInfo::parseType(const String& name_) {
  for (auto type : collections::range(AccessEntityType::MAX)) {
    ...
    if (boost::iequals(info.name, name_)) return type;
  }
