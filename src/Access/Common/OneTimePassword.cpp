...
String normalizeOneTimePasswordSecret(const String& secret) {
  ...
  for (const auto* p = b32_alphabet; *p; p++) table[*p] = CHAR_IS_VALID;
  for (const auto* p = b32_lower_alphabet; *p; p++) table[*p] = CHAR_IS_LOWER;

  String result = secret;
  size_t i = 0;
  size_t n = 0;
  for (; i < secret.size(); ++i) {
    if (secret[i] == ' ' || secret[i] == '=') continue;
    ...
    if (table[idx] == CHAR_IS_VALID) result[n] = secret[i];
    if (table[idx] == CHAR_IS_LOWER) result[n] = static_cast<char>(std::toupper(secret[i]));
    ++n;
  }
  result.resize(n);

  if (result.empty()) throw Exception(ErrorCodes::BAD_ARGUMENTS, "Empty secret for one-time password");
  return result;
}

static std::string_view toString(OneTimePasswordParams::Algorithm algorithm) {
  switch (algorithm) {
    case OneTimePasswordParams::Algorithm::SHA1:
      return "SHA1";
    case OneTimePasswordParams::Algorithm::SHA256:
      return "SHA256";
    case OneTimePasswordParams::Algorithm::SHA512:
      return "SHA512";
  }
...
static OneTimePasswordParams::Algorithm hashingAlgorithmFromString(const String& algorithm_name) {
  for (auto alg :
       {OneTimePasswordParams::Algorithm::SHA1, OneTimePasswordParams::Algorithm::SHA256, OneTimePasswordParams::Algorithm::SHA512}) {
    if (Poco::toUpper(algorithm_name) == toString(alg)) return alg;
  }
...
OneTimePasswordParams::OneTimePasswordParams(Int32 num_digits_, Int32 period_, const String& algorithm_name_) {
  if (num_digits_) num_digits = num_digits_;
  if (period_) period = period_;
  if (!algorithm_name_.empty()) algorithm = hashingAlgorithmFromString(algorithm_name_);
  ...
  if (period <= 0 || 120 < period) throw Exception(ErrorCodes::BAD_ARGUMENTS, "Invalid period for one-time password: {}", period);
}
...
String getOneTimePasswordSecretLink(const OneTimePasswordSecret& secret) {
  ...
  return fmt::format("otpauth://totp/ClickHouse?issuer={}&secret={}&digits={}&period={}&algorithm={}", issuer_name, secret.key,
                     secret.params.num_digits, secret.params.period, toString(secret.params.algorithm));
}
...
String getOneTimePassword(const String& secret [[maybe_unused]], const OneTimePasswordParams& config [[maybe_unused]],
                          UInt64 current_time [[maybe_unused]]) {
#if USE_SSL && USE_LIBCOTP
  ...
  cotp_error_t error;
  auto result =
      std::unique_ptr<char, CStringDeleter>(get_totp_at(secret.c_str(), current_time, config.num_digits, config.period, sha_algo, &error));
  ...
  throw Exception(ErrorCodes::SUPPORT_IS_DISABLED,
                  "One-time password support is disabled, because ClickHouse was built without openssl library");
#endif
}

bool checkOneTimePassword(std::string_view password, const OneTimePasswordSecret& secret) {
  if (password.size() != static_cast<size_t>(secret.params.num_digits) || !std::all_of(password.begin(), password.end(), ::isdigit))
    return false;

  auto current_time = static_cast<UInt64>(std::time(nullptr));
  for (int delta : {0, -1, 1}) {
    if (password == getOneTimePassword(secret.key, secret.params, current_time + delta * secret.params.period)) return true;
  }
  return false;
}
