...
class Helper {
 public:
  ...
  Flags accessTypeToFlags(AccessType type) const { return access_type_to_flags_mapping[static_cast<size_t>(type)]; }
  ...
  Flags keywordToFlags(std::string_view keyword) const {
    ...
    if (it == keyword_to_flags_map.end()) {
      ...
      if (it == keyword_to_flags_map.end()) throw Exception(ErrorCodes::UNKNOWN_ACCESS_TYPE, "Unknown access type: {}", String(keyword));
    }
    return it->second;
  }
  ...
  Flags keywordsToFlags(const std::vector<std::string_view> &keywords) const {
    Flags res;
    for (const auto &keyword : keywords) res |= keywordToFlags(keyword);
    return res;
  }
 ...
 private:
  ...
  static Strings splitAliases(std::string_view str) {
    ...
    for (auto &alias : aliases) boost::trim(alias);
    return aliases;
  }

  static void makeNode(AccessType access_type, std::string_view name, std::string_view aliases, NodeType node_type,
                       std::string_view parent_group_name, std::unordered_map<std::string_view, Node *> &nodes,
                       std::unordered_map<std::string_view, NodePtr> &owned_nodes, size_t &next_flag) {
    NodePtr node;
    ...
    if (it != owned_nodes.end()) {
      ...
      if (nodes.contains(keyword)) throw Exception(ErrorCodes::LOGICAL_ERROR, "{} declared twice", keyword);
    ...
    if (node_type != GROUP) node->setFlag(next_flag++);
...
#undef MAKE_ACCESS_FLAGS_NODE
    ...
    if (!owned_nodes.contains("NONE")) throw Exception(ErrorCodes::LOGICAL_ERROR, "'NONE' not declared");
    if (!owned_nodes.contains("ALL")) throw Exception(ErrorCodes::LOGICAL_ERROR, "'ALL' not declared");
    ...
    if (!owned_nodes.empty()) {
      ...
      if (unused_node.node_type == UNKNOWN) throw Exception(ErrorCodes::LOGICAL_ERROR, "Parent group '{}' not found", unused_node.keyword);
  ...
  void makeKeywordToFlagsMap(Node *start_node = nullptr) {
    ...
    for (auto &child : start_node->children) makeKeywordToFlagsMap(child.get());
  }
  ...
  void makeAccessTypeToFlagsMapping(Node *start_node = nullptr) {
    ...
    for (auto &child : start_node->children) makeAccessTypeToFlagsMapping(child.get());
  }
  ...
  void collectAllFlags(const Node *start_node = nullptr) {
    ...
    for (const auto &child : start_node->children) collectAllFlags(child.get());
    ...
    all_flags_grantable_on_global_with_parameter_level = all_flags_for_target[NAMED_COLLECTION] | all_flags_for_target[USER_NAME] |
                                                         all_flags_for_target[TABLE_ENGINE] | all_flags_for_target[DEFINER] |
                                                         all_flags_for_target[SOURCE];
  ...
  static void flagsToAccessTypesRec(const Flags &flags_, std::vector<AccessType> &access_types, const Node &start_node) {
    ...
    if (matching_flags.any()) {
      if (matching_flags == start_node.flags) {
        ...
        for (const auto &child : start_node.children) flagsToAccessTypesRec(flags_, access_types, *child);
      }
    }
  }
  ...
  static void flagsToKeywordsRec(const Flags &flags_, std::vector<std::string_view> &keywords, const Node &start_node) {
    ...
    if (matching_flags.any()) {
      if (matching_flags == start_node.flags) {
        ...
        for (const auto &child : start_node.children) flagsToKeywordsRec(flags_, keywords, *child);
      }
    }
  }
...
bool AccessFlags::isGlobalWithParameter() const { return getParameterType() != AccessFlags::NONE; }
...
std::unordered_map<AccessFlags::ParameterType, AccessFlags> AccessFlags::splitIntoParameterTypes() const {
  ...
  if (named_collection_flags) result.emplace(ParameterType::NAMED_COLLECTION, named_collection_flags);

  auto user_flags = AccessFlags::allUserNameFlags() & *this;
  if (user_flags) result.emplace(ParameterType::USER_NAME, user_flags);

  auto definer_flags = AccessFlags::allDefinerFlags() & *this;
  if (definer_flags) result.emplace(ParameterType::DEFINER, definer_flags);

  auto table_engine_flags = AccessFlags::allTableEngineFlags() & *this;
  if (table_engine_flags) result.emplace(ParameterType::TABLE_ENGINE, table_engine_flags);

  auto source_flags = AccessFlags::allSourceFlags() & *this;
  if (source_flags) result.emplace(ParameterType::SOURCE, source_flags);

  auto other_flags = (~named_collection_flags & ~user_flags & ~definer_flags & ~table_engine_flags & ~source_flags) & *this;
  if (other_flags) result.emplace(ParameterType::NONE, other_flags);

  return result;
}

AccessFlags::ParameterType AccessFlags::getParameterType() const {
  if (isEmpty() || !AccessFlags::allGlobalWithParameterFlags().contains(*this)) return AccessFlags::NONE;

  /// All flags refer to NAMED COLLECTION access type.
  if (AccessFlags::allNamedCollectionFlags().contains(*this)) return AccessFlags::NAMED_COLLECTION;

  if (AccessFlags::allUserNameFlags().contains(*this)) return AccessFlags::USER_NAME;

  if (AccessFlags::allDefinerFlags().contains(*this)) return AccessFlags::DEFINER;

  /// All flags refer to TABLE ENGINE access type.
  if (AccessFlags::allTableEngineFlags().contains(*this)) return AccessFlags::TABLE_ENGINE;

  /// All flags refer to SOURCE access type.
  if (AccessFlags::allSourceFlags().contains(*this)) return AccessFlags::SOURCE;
...
AccessFlags AccessFlags::allFlagsGrantableOnGlobalWithParameterLevel() {
