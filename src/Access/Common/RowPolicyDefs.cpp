...
String toString(RowPolicyFilterType type) { return RowPolicyFilterTypeInfo::get(type).raw_name; }
...
const RowPolicyFilterTypeInfo& RowPolicyFilterTypeInfo::get(RowPolicyFilterType type_) {
  ...
  switch (type_) {
    case RowPolicyFilterType::SELECT_FILTER: {
      static const auto info = make_info("SELECT_FILTER", "Expression which is used for filtering in SELECT queries.");
      return info;
    }
