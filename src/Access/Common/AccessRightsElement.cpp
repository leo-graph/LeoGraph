...
String toStringImpl(const AccessRightsElement& element, bool with_options) {
  ...
  if (with_options) formatOptions(element.grant_option, element.is_partial_revoke, result);
  return result;
}

String toStringImpl(const AccessRightsElements& elements, bool with_options) {
  if (elements.empty()) return with_options ? "GRANT USAGE ON *.*" : "USAGE ON *.*";
  ...
  for (size_t i = 0; i != elements.size(); ++i) {
    ...
    if (!next_element_uses_same_table_and_options) {
      ...
      if (with_options) formatOptions(element.grant_option, element.is_partial_revoke, part);
      if (result.empty())
...
void AccessRightsElement::formatColumnNames(WriteBuffer& buffer) const {
  ...
  for (const auto& column : columns) {
    if (std::exchange(need_comma, true)) buffer << ", ";
...
void AccessRightsElement::formatFilter(WriteBuffer& buffer) const { buffer << "(" << backQuoteIfNeed(filter) << ")"; }
...
void AccessRightsElement::formatONClause(WriteBuffer& buffer) const {
  ...
  if (isGlobalWithParameter()) {
    ...
    if (access_flags.getParameterType() == AccessFlags::USER_NAME && !is_enabled_user_name_access_type) {
      if (!anyParameter())
      ...
      if (anyParameter())
        ...
        if (wildcard)
          ...
          if (hasFilter() && is_enabled_read_write_grants) formatFilter(buffer);
        }
      }
    }
  } else if (anyDatabase())
    ...
    if (!database.empty()) buffer << backQuoteIfNeed(database) << ".";

    buffer << backQuoteIfNeed(table);

    if (columns.empty() && wildcard) buffer << "*";
...
AccessRightsElement::AccessRightsElement(AccessFlags access_flags_, std::string_view database_, std::string_view table_,
    ...
    : access_flags(access_flags_), database(database_), table(table_), columns({String{column_}}) {}

AccessRightsElement::AccessRightsElement(AccessFlags access_flags_, std::string_view database_, std::string_view table_,
                                         const std::vector<std::string_view>& columns_)
    : access_flags(access_flags_), database(database_), table(table_) {
  columns.resize(columns_.size());
  for (size_t i = 0; i != columns_.size(); ++i) columns[i] = String{columns_[i]};
}

AccessRightsElement::AccessRightsElement(AccessFlags access_flags_, std::string_view database_, std::string_view table_,
                                         const Strings& columns_)
    : access_flags(access_flags_), database(database_), table(table_), columns(columns_) {}
...
void AccessRightsElement::throwIfNotGrantable() const {
  ...
  if (!anyColumn()) throw Exception(ErrorCodes::INVALID_GRANT, "{} cannot be granted on the column level", access_flags.toString());
  if (!anyTable()) throw Exception(ErrorCodes::INVALID_GRANT, "{} cannot be granted on the table level", access_flags.toString());
  if (!anyDatabase()) throw Exception(ErrorCodes::INVALID_GRANT, "{} cannot be granted on the database level", access_flags.toString());
  if (!anyParameter())
...
void AccessRightsElement::eraseNotGrantable() { access_flags = getGrantableFlags(); }

void AccessRightsElement::replaceEmptyDatabase(const String& current_database) {
  if (isEmptyDatabase()) database = current_database;
}

void AccessRightsElement::replaceDeprecated() {
  if (!access_flags) return;

  if (access_flags.toAccessTypes().size() != 1)
    throw Exception(ErrorCodes::LOGICAL_ERROR, "replaceDeprecated() was called on an access element with multiple access flags: {}",
...
void AccessRightsElement::makeBackwardCompatible() {
  static const std::unordered_map<std::string, AccessType> string_to_accessType = {
      {"FILE", AccessType::FILE},         {"URL", AccessType::URL},     {"REMOTE", AccessType::REMOTE},     {"MONGO", AccessType::MONGO},
      {"REDIS", AccessType::REDIS},       {"MYSQL", AccessType::MYSQL}, {"POSTGRES", AccessType::POSTGRES}, {"SQLITE", AccessType::SQLITE},
      {"ODBC", AccessType::ODBC},         {"JDBC", AccessType::JDBC},   {"HDFS", AccessType::HDFS},         {"S3", AccessType::S3},
      {"HIVE", AccessType::HIVE},         {"AZURE", AccessType::AZURE}, {"KAFKA", AccessType::KAFKA},       {"NATS", AccessType::NATS},
...
bool AccessRightsElements::empty() const {
  return std::all_of(begin(), end(), [](const AccessRightsElement& e) { return e.empty(); });
}

bool AccessRightsElements::sameDatabaseAndTableAndParameter() const {
  return (size() < 2) || std::all_of(std::next(begin()), end(),
                                     [this](const AccessRightsElement& e) { return e.sameDatabaseAndTableAndParameter(front()); });
}

bool AccessRightsElements::sameDatabaseAndTable() const {
  return (size() < 2) ||
         std::all_of(std::next(begin()), end(), [this](const AccessRightsElement& e) { return e.sameDatabaseAndTable(front()); });
}
...
void AccessRightsElements::throwIfNotGrantable() const {
  for (const auto& element : *this) element.throwIfNotGrantable();
}
...
void AccessRightsElements::replaceDeprecated() {
  for (auto& element : *this) element.replaceDeprecated();
}

void AccessRightsElements::replaceEmptyDatabase(const String& current_database) {
  for (auto& element : *this) element.replaceEmptyDatabase(current_database);
}
...
void AccessRightsElements::formatElementsWithoutOptions(WriteBuffer& buffer) const {
  ...
  for (size_t i = 0; i != size(); ++i) {
    ...
    auto keywords = element.access_flags.toKeywords();
    if (keywords.empty() || (!element.anyColumn() && element.columns.empty())) continue;

    for (const auto& keyword : keywords) {
      if (!std::exchange(no_output, false)) buffer << ", ";

      buffer << keyword;
      if (!element.anyColumn()) element.formatColumnNames(buffer);
    }
