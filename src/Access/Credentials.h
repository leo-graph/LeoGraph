...
class AlwaysAllowCredentials : public Credentials {
 public:
...
#if USE_SSL
class SSLCertificateCredentials : public Credentials {
 public:
...
class ScramSHA256Credentials : public Credentials {
 public:
  ...
  const String& getClientProof() const { return client_proof; }

  const String& getAuthMessage() const { return auth_message; }
...
class SSHPTYCredentials : public Credentials {
 public:
  explicit SSHPTYCredentials(const String& user_name_, const SSHKey& key_) : Credentials(user_name_), key(key_) { is_ready = true; }
