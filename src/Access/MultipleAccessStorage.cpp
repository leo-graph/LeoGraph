...
void MultipleAccessStorage::addStorage(const StoragePtr &new_storage) {
  std::lock_guard lock{mutex};
  if (boost::range::find(*nested_storages, new_storage) != nested_storages->end()) return;
...
void MultipleAccessStorage::removeStorage(const StoragePtr &storage_to_remove) {
  ...
  if (it == nested_storages->end()) return;
...
std::vector<StoragePtr> MultipleAccessStorage::getStorages() { return *getStoragesPtr(); }
...
std::shared_ptr<const Storages> MultipleAccessStorage::getStoragesPtr() { return getStoragesInternal(); }
...
bool MultipleAccessStorage::exists(const UUID &id) const { return findStorage(id) != nullptr; }
...
StoragePtr MultipleAccessStorage::findStorage(const UUID &id) {
  ...
  if (from_cache && from_cache->exists(id)) return from_cache;
...
StoragePtr MultipleAccessStorage::findStorageByName(const String &storage_name) {
  auto storages = getStoragesInternal();
  for (const auto &storage : *storages) {
    if (storage->getStorageName() == storage_name) return storage;
  }
...
StoragePtr MultipleAccessStorage::findExcludingStorage(AccessEntityType type, const String &name,
                                                       DB::MultipleAccessStorage::StoragePtr exclude) const {
  auto storages = getStoragesInternal();
  for (const auto &storage : *storages) {
    if (storage == exclude) continue;

    if (storage->find(type, name)) return storage;
  }

  return nullptr;
}

void MultipleAccessStorage::moveAccessEntities(const std::vector<UUID> &ids, const String &source_storage_name,
                                               const String &destination_storage_name) {
  ...
  try {
    ...
    for (const auto &entity : to_move) {
      if (std::exchange(need_comma, true)) message += ", ";
    ...
    if (need_rollback) source_storage->insert(to_move, ids);
...
AccessEntityPtr MultipleAccessStorage::readImpl(const UUID &id, bool throw_if_not_exists) const {
  if (auto storage = findStorage(id)) return storage->read(id, throw_if_not_exists);
...
std::optional<std::pair<String, AccessEntityType>> MultipleAccessStorage::readNameWithTypeImpl(const UUID &id,
                                                                                               bool throw_if_not_exists) const {
  if (auto storage = findStorage(id)) return storage->readNameWithType(id, throw_if_not_exists);
...
bool MultipleAccessStorage::isReadOnly(const UUID &id) const {
  auto storage = findStorage(id);
  if (storage) return storage->isReadOnly(id);
  return false;
}
...
bool MultipleAccessStorage::isEphemeral() const {
  auto storages = getStoragesInternal();
  for (const auto &storage : *storages) {
    if (!storage->isEphemeral()) return false;
  }
  return true;
}
...
bool MultipleAccessStorage::isEphemeral(const UUID &id) const {
  auto storage = findStorage(id);
  if (storage) return storage->isEphemeral(id);
  return false;
}

void MultipleAccessStorage::startPeriodicReloading() {
  auto storages = getStoragesInternal();
  for (const auto &storage : *storages) storage->startPeriodicReloading();
}

void MultipleAccessStorage::stopPeriodicReloading() {
  auto storages = getStoragesInternal();
  for (const auto &storage : *storages) storage->stopPeriodicReloading();
}

void MultipleAccessStorage::reload(ReloadMode reload_mode) {
  auto storages = getStoragesInternal();
  for (const auto &storage : *storages) storage->reload(reload_mode);
}
...
bool MultipleAccessStorage::insertImpl(const UUID &id, const AccessEntityPtr &entity, bool replace_if_exists, bool throw_if_exists,
  ...
  if (!storage_for_insertion) {
    throw Exception(ErrorCodes::ACCESS_STORAGE_FOR_INSERTION_NOT_FOUND,
                    "Could not insert {} because there is no writeable access storage in {}", entity->formatTypeWithName(),
                    getStorageName());
  }
...
bool MultipleAccessStorage::removeImpl(const UUID &id, bool throw_if_not_exists) {
  if (auto storage = findStorage(id)) return storage->remove(id, throw_if_not_exists);
  ...
  if ((storages->size() > 1) && (storages->front() != storage_for_updating)) {
    if (auto old_entity = storage_for_updating->tryRead(id)) {
      ...
      if (new_entity->getName() != old_entity->getName()) {
        for (const auto &storage : *storages) {
          if (storage == storage_for_updating) break;
          if (storage->find(new_entity->getType(), new_entity->getName())) {
            throw Exception(ErrorCodes::ACCESS_ENTITY_ALREADY_EXISTS, "{}: cannot rename to {} because {} already exists in {}",
                            old_entity->formatTypeWithName(), backQuote(new_entity->getName()), new_entity->formatTypeWithName(),
...
std::optional<AuthResult> MultipleAccessStorage::authenticateImpl(const Credentials &credentials, const Poco::Net::IPAddress &address,
  ...
  for (size_t i = 0; i != storages->size(); ++i) {
    ...
    auto auth_result = storage->authenticate(credentials, address, external_authenticators, client_info,
                                             (throw_if_user_not_exists && is_last_storage), allow_no_password, allow_plaintext_password);
    if (auth_result) {
      std::lock_guard lock{mutex};
...
bool MultipleAccessStorage::isBackupAllowed() const {
  auto storages = getStoragesInternal();
  for (const auto &storage : *storages) {
    if (storage->isBackupAllowed()) return true;
  }
  return false;
}
...
bool MultipleAccessStorage::isRestoreAllowed() const {
  auto storages = getStoragesInternal();
  for (const auto &storage : *storages) {
    if (storage->isRestoreAllowed()) return true;
  }
  return false;
}

void MultipleAccessStorage::backup(BackupEntriesCollector &backup_entries_collector, const String &data_path_in_backup,
  ...
  if (!allowed) throwBackupNotAllowed();
}
...
bool MultipleAccessStorage::containsStorage(std::string_view storage_type) const {
  ...
  for (const auto &storage : *storages) {
    if (storage->getStorageType() == storage_type) return true;
  }
  return false;
}
}  // namespace DB
