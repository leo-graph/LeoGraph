...
struct AccessEntitiesInBackup {
  ...
  BackupEntryPtr toBackupEntry() const {
    ...
    if (!dependents.empty()) {
      if (!dependencies.empty()) writeText("\n", buf);
      ...
      while (!buf->eof()) {
        ...
        if (reading_dependencies) {
          ...
          while (!buf->eof()) {
            ...
            if (!queries.empty()) queries.append("\n");
          ...
          if (type != entity->getType())
            throw Exception(ErrorCodes::CANNOT_RESTORE_TABLE, "Unexpected type {} is specified for {}",
                            AccessEntityTypeInfo::get(type).name, entity->formatTypeWithName());
...
std::pair<String, BackupEntryPtr> makeBackupEntryForAccessEntities(const std::vector<UUID> &entities_ids,
                                                                   const std::unordered_map<UUID, AccessEntityPtr> &all_entities,
                                                                   bool write_dependents, const String &data_path_in_backup) {
  AccessEntitiesInBackup ab;

  std::unordered_set<UUID> entities_ids_set;
  for (const auto &id : entities_ids) entities_ids_set.emplace(id);
...
AccessRestorerFromBackup::AccessRestorerFromBackup(const BackupPtr &backup_, const RestoreSettings &restore_settings_)
    : backup(backup_),
...
void AccessRestorerFromBackup::addDataPath(const String &data_path_in_backup) {
  if (loaded) throw Exception(ErrorCodes::LOGICAL_ERROR, "Access entities already loaded");
  ...
  for (size_t data_path_index = 0; data_path_index != data_paths_in_backup.size(); ++data_path_index) {
    ...
    for (const String &filename : filenames) {
      ...
      for (const auto &[id, entity] : ab.dependents) {
        ...
        EntityInfo &entity_info = it->second;
        if (!entity_info.restore) entity_info.entity = entity;
      }
    }
  }
...
AccessRightsElements AccessRestorerFromBackup::getRequiredAccess() const {
  if (!loaded) throw Exception(ErrorCodes::LOGICAL_ERROR, "Access entities not loaded");

  AccessRightsElements res;
  for (const auto &[id, entity_info] : entity_infos) {
    if (!entity_info.restore) continue;
    ...
    switch (entity_type) {
      case User::TYPE: {
        ...
        if (!restore_with_current_grants) {
          ...
          for (auto &element : elements) {
            if (!element.is_partial_revoke) element.grant_option = true;
            res.emplace_back(element);
          }
        }
        if (!user.granted_roles.isEmpty()) res.emplace_back(AccessType::ROLE_ADMIN);
        break;
      }
      ...
      case Role::TYPE: {
        ...
        if (!role.granted_roles.isEmpty()) res.emplace_back(AccessType::ROLE_ADMIN);
        break;
      }
...
void AccessRestorerFromBackup::generateRandomIDsAndResolveDependencies(const AccessControl &access_control) {
  ...
  for (const auto &[id, entity_info] : entity_infos) {
    if (entity_info.restore) ids_to_restore.insert(id);
...
AccessEntitiesToRestore AccessRestorerFromBackup::getEntitiesToRestore(const String &data_path_in_backup) const {
  if (!ids_assigned) throw Exception(ErrorCodes::LOGICAL_ERROR, "IDs not assigned");

  if (data_path_in_backup != data_path_with_entities_to_restore) return {};
  ...
  for (const auto &[id, entity_info] : entity_infos) {
    if (entity_info.restore) res.new_entities.emplace_back(*entity_info.new_id, entity_info.entity);

    if (entity_info.is_dependent) res.dependents.emplace_back(AccessEntitiesToRestore::Dependent{*entity_info.new_id, entity_info.entity});
  }

  return res;
}

void restoreAccessEntitiesFromBackup(IAccessStorage &destination_access_storage, const AccessEntitiesToRestore &entities_to_restore,
                                     const RestoreSettings &restore_settings) {
  if (entities_to_restore.new_entities.empty()) return;  /// Nothing to restore.
  ...
  for (const auto &[id, entity] : entities_to_restore.new_entities) {
    ...
    if (destination_access_storage.insert(id, entity, replace_if_exists, throw_if_exists, &existing_id)) {
      ...
      LOG_TRACE(log, "{}: Not added because already exists with UUID {}", AccessEntityTypeInfo::get(type).formatEntityNameWithType(name),
                existing_id);
      new_to_existing_ids[id] = existing_id;
      if (update_dependents) additional_dependents.emplace_back(AccessEntitiesToRestore::Dependent{existing_id, entity});
    }
  }
  ...
  if (!new_to_existing_ids.empty()) {
    ...
    auto update_func = [&](const AccessEntityPtr &entity, const UUID &) -> AccessEntityPtr {
      if (!entity->hasDependencies(new_ids)) return entity;
  ...
  auto do_update_dependents = [&](const AccessEntitiesToRestore::Dependents &dependents) {
    ...
    auto update_func = [&](const AccessEntityPtr &entity, const UUID &existing_id) -> AccessEntityPtr {
      ...
      if (!source.hasDependencies(restored_ids)) return entity;
