...
class SettingsProfilesCache {
 public:
  ...
  std::shared_ptr<const EnabledSettings> getEnabledSettings(const UUID& user_id, const SettingsProfileElements& settings_from_user_,
 ...
 private:
  ...
  void substituteProfiles(SettingsProfileElements& elements, std::vector<UUID>& profiles, std::vector<UUID>& substituted_profiles,
