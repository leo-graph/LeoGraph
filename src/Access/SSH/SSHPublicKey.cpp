...
SSHPublicKey::SSHPublicKey(KeyPtr key_, bool own)
    : key(key_, own ? &deleter : &disabledDeleter) {  // disable deleter if class is constructed without ownership
  if (!key) throw DB::Exception(DB::ErrorCodes::LOGICAL_ERROR, "No ssh_key provided in explicit constructor");
}
...
SSHPublicKey::SSHPublicKey(const SSHPublicKey& other) : key(ssh_key_dup(other.key.get()), &deleter) {
  if (!key) throw DB::Exception(DB::ErrorCodes::SSH_EXCEPTION, "Failed to duplicate ssh_key");
}

SSHPublicKey& SSHPublicKey::operator=(const SSHPublicKey& other) {
  if (this != &other) {
    KeyPtr new_key = ssh_key_dup(other.key.get());
    if (!new_key) throw DB::Exception(DB::ErrorCodes::SSH_EXCEPTION, "Failed to duplicate ssh_key");
...
bool SSHPublicKey::operator==(const SSHPublicKey& other) const { return isEqual(other); }
...
SSHPublicKey SSHPublicKey::createFromBase64(const String& base64, const String& key_type) {
  ...
  if (rc != SSH_OK)
    throw DB::Exception(DB::ErrorCodes::SSH_EXCEPTION, "Failed importing public key from base64 format. Key: {}; Type: {}", base64,
                        key_type);
...
SSHPublicKey SSHPublicKey::createNonOwning(KeyPtr key) { return SSHPublicKey(key, false); }
...
String SSHPublicKey::getBase64Representation() const {
  ...
  if (rc != SSH_OK) throw DB::Exception(DB::ErrorCodes::SSH_EXCEPTION, "Failed to export public key to base64");
...
void SSHPublicKey::deleter(KeyPtr key) { ssh_key_free(key); }
