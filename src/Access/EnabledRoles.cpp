...
void EnabledRoles::setRolesInfo(const std::shared_ptr<const EnabledRolesInfo>& info_, scope_guard* notifications) {
  {
    std::lock_guard lock{info_mutex};
    if (info && info_ && *info == *info_) return;
  ...
  if (notifications) {
    ...
    notifications->join(scope_guard([my_info = info, my_handlers_to_notify = std::move(handlers_to_notify)] {
      for (const auto& handler : my_handlers_to_notify) handler(my_info);
    }));
  }
}
