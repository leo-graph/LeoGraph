...
ContextAccessParams::ContextAccessParams(std::optional<UUID> user_id_, bool full_access_, bool use_default_roles_,
                                         const std::shared_ptr<const std::vector<UUID>>& current_roles_,
                                         const std::shared_ptr<const std::vector<UUID>>& external_roles_, const Settings& settings_,
                                         const String& current_database_, const ClientInfo& client_info_,
...
String ContextAccessParams::toString() const {
  ...
  if (user_id) out << separator() << "user_id = " << *user_id;
  if (full_access) out << separator() << "full_access = " << full_access;
  if (use_default_roles) out << separator() << "use_default_roles = " << use_default_roles;
  ...
  if (readonly) out << separator() << "readonly = " << readonly;
  if (allow_ddl) out << separator() << "allow_ddl = " << allow_ddl;
  if (allow_introspection) out << separator() << "allow_introspection = " << allow_introspection;
  if (!current_database.empty()) out << separator() << "current_database = " << current_database;
  out << separator() << "interface = " << magic_enum::enum_name(interface);
  if (http_method != ClientInfo::HTTPMethod::UNKNOWN) out << separator() << "http_method = " << magic_enum::enum_name(http_method);
  if (!address->isWildcard()) out << separator() << "address = " << address->toString();
  if (!forwarded_address.empty()) out << separator() << "forwarded_address = " << forwarded_address;
  if (!quota_key.empty()) out << separator() << "quota_key = " << quota_key;
  if (initial_user_id) out << separator() << "initial_user_id = " << *initial_user_id;
  return out.str();
}
...
#define CONTEXT_ACCESS_PARAMS_EQUALS(name) \
  if (!check_equals(left.name, right.name)) return false;
...
#define CONTEXT_ACCESS_PARAMS_LESS(name) \
  if (auto cmp = check_less(left.name, right.name); cmp != 0) return cmp < 0;
