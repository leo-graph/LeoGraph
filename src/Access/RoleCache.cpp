...
namespace DB {
namespace {
void collectRoles(EnabledRolesInfo &roles_info, boost::container::flat_set<UUID> &skip_ids,
                  Fn<RolePtr(const UUID &)> auto &&get_role_function, const UUID &role_id, bool is_current_role, bool with_admin_option) {
  if (roles_info.enabled_roles.count(role_id)) {
    if (is_current_role) roles_info.current_roles.emplace(role_id);
    if (with_admin_option) roles_info.enabled_roles_with_admin_option.emplace(role_id);
    return;
  }

  if (skip_ids.count(role_id)) return;
...
std::shared_ptr<const EnabledRoles> RoleCache::getEnabledRoles(const std::vector<UUID> &roles,
                                                               const std::vector<UUID> &roles_with_admin_option) {
  ...
  if (it != enabled_roles_by_params.end()) {
    if (auto enabled_roles = it->second.enabled_roles.lock()) return enabled_roles;
...
void RoleCache::collectEnabledRoles(EnabledRoles &enabled_roles, SubscriptionsOnRoles &subscriptions_on_roles, scope_guard *notifications) {
  ...
  /// We need to collect and keep not only enabled roles but also subscriptions for them to be able to recalculate EnabledRolesInfo when
  /// some of the roles change.
  SubscriptionsOnRoles new_subscriptions_on_roles;
  new_subscriptions_on_roles.reserve(subscriptions_on_roles.size());

  auto get_role_function = [this, &new_subscriptions_on_roles](const UUID &id)
                               TSA_NO_THREAD_SAFETY_ANALYSIS { return getRole(id, new_subscriptions_on_roles); };
