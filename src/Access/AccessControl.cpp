...
namespace {
void checkForUsersNotInMainConfig(const Poco::Util::AbstractConfiguration& config, const std::string& config_path,
                                  const std::string& users_config_path, LoggerPtr log) {
  if (config.getBool("skip_check_for_incorrect_settings", false)) return;
 ...
 public:
  ...
  std::shared_ptr<const ContextAccess> getContextAccess(const ContextAccessParams& params) {
    if (auto cached_access = cache.get(params)) {
      ...
      if (!(*cached_access)->getUserID() || (*cached_access)->tryGetUser()) return *cached_access;
    }
...
class AccessControl::CustomSettingsPrefixes {
 public:
  ...
  bool isSettingNameAllowed(std::string_view setting_name) const {
    if (settingIsBuiltin(setting_name)) return true;

    std::lock_guard lock{mutex};
    for (const auto& prefix : registered_prefixes) {
      if (setting_name.starts_with(prefix)) return true;
    }
  ...
  void checkSettingNameIsAllowed(std::string_view setting_name) const {
    if (isSettingNameAllowed(setting_name)) return;
 ...
 public:
    ...
    if (config_.has("password_complexity")) {
      ...
      for (const auto& key : password_complexity) {
        if (key == "rule" || key.starts_with("rule[")) {
          ...
          if (!matcher->ok())
            throw Exception(ErrorCodes::CANNOT_COMPILE_REGEXP, "Password complexity pattern {} cannot be compiled: {}", pattern,
                            matcher->error());
  ...
  void setPasswordComplexityRules(const std::vector<std::pair<String, String>>& rules_) {
    ...
    for (const auto& [original_pattern, exception_message] : rules_) {
      ...
      if (!matcher->ok())
        throw Exception(ErrorCodes::CANNOT_COMPILE_REGEXP, "Password complexity pattern {} cannot be compiled: {}", original_pattern,
                        matcher->error());
  ...
  void checkPasswordComplexityRules(const String& password_) const {
    ...
    for (const auto& rule : rules) {
      if (!RE2::PartialMatch(password_, *rule.matcher)) {
        ...
        if (!exception_text.empty()) exception_text += ", ";

        exception_text += rule.exception_message;
      }
    }

    if (failed) throw Exception(ErrorCodes::BAD_ARGUMENTS, "Invalid password. The password should: {}", exception_text);
  }
  ...
  std::vector<std::pair<String, String>> getPasswordComplexityRules() {
    ...
    for (const auto& rule : rules) result.push_back({rule.original_pattern, rule.exception_message});
...
void AccessControl::setupFromMainConfig(const Poco::Util::AbstractConfiguration& config_, const String& config_path_,
  ...
  if (config_.has("custom_settings_prefixes")) setCustomSettingsPrefixes(config_.getString("custom_settings_prefixes"));
  ...
  setEnabledUsersWithoutRowPoliciesCanReadRows(
      config_.getBool("access_control_improvements.users_without_row_policies_can_read_rows", true));
  ...
  setSelectFromInformationSchemaRequiresGrant(
      config_.getBool("access_control_improvements.select_from_information_schema_requires_grant", true));
  ...
  setImpersonateUserAllowed(
      config_.getBool("access_control_improvements.allow_impersonate_user", config_.getBool("allow_impersonate_user", true)));
  ...
  setThrowOnInvalidReplicatedAccessEntities(
      config_.getBool("access_control_improvements.throw_on_invalid_replicated_access_entities", false));
...
void AccessControl::addUsersConfigStorage(const String& storage_name_, const Poco::Util::AbstractConfiguration& users_config_,
  ...
  LOG_DEBUG(getLogger(), "Added {} access storage '{}', path: {}", String(new_storage->getStorageType()), new_storage->getStorageName(),
            new_storage->getPath());
}
...
void AccessControl::addUsersConfigStorage(const String& storage_name_, const String& users_config_path_, const String& include_from_path_,
                                          const String& preprocessed_dir_, const zkutil::GetZooKeeper& get_zookeeper_function_,
                                          bool allow_backup_) {
  auto storages = getStoragesPtr();
  for (const auto& storage : *storages) {
    if (auto users_config_storage = typeid_cast<std::shared_ptr<UsersConfigAccessStorage>>(storage)) {
      if (users_config_storage->isPathEqual(users_config_path_)) return;
    }
  }
  auto new_storage = std::make_shared<UsersConfigAccessStorage>(storage_name_, *this, allow_backup_);
...
void AccessControl::addReplicatedStorage(const String& storage_name_, const String& zookeeper_path_,
                                         const zkutil::GetZooKeeper& get_zookeeper_function_, bool allow_backup_) {
  auto storages = getStoragesPtr();
  for (const auto& storage : *storages) {
    if (auto replicated_storage = typeid_cast<std::shared_ptr<ReplicatedAccessStorage>>(storage)) return;
  }
  auto new_storage = std::make_shared<ReplicatedAccessStorage>(storage_name_, zookeeper_path_, get_zookeeper_function_, *changes_notifier,
                                                               allow_backup_, throw_on_invalid_replicated_access_entities);
  addStorage(new_storage);
  ...
  for (const auto& storage : *storages) {
    if (auto disk_storage = typeid_cast<std::shared_ptr<DiskAccessStorage>>(storage)) {
      if (disk_storage->isPathEqual(directory_)) {
        if (readonly_) disk_storage->setReadOnly(readonly_);
        return;
      }
    }
  }
...
void AccessControl::addMemoryStorage(const String& storage_name_, bool allow_backup_) {
  auto storages = getStoragesPtr();
  for (const auto& storage : *storages) {
    if (auto memory_storage = typeid_cast<std::shared_ptr<MemoryAccessStorage>>(storage)) return;
  }
...
void AccessControl::addLDAPStorage(const String& storage_name_, const Poco::Util::AbstractConfiguration& config_, const String& prefix_) {
  ...
  addStorage(new_storage);
  LOG_DEBUG(getLogger(), "Added {} access storage '{}', LDAP server name: {}", String(new_storage->getStorageType()),
            new_storage->getStorageName(), new_storage->getLDAPServerName());
}

void AccessControl::addStoragesFromUserDirectoriesConfig(const Poco::Util::AbstractConfiguration& config, const String& key,
                                                         const String& config_dir, const String& dbms_dir, const String& include_from_path,
  ...
  for (const String& key_in_user_directories : keys_in_user_directories) {
    ...
    if (size_t bracket_pos = type.find('['); bracket_pos != String::npos) type.resize(bracket_pos);
    ...
    if (type == MemoryAccessStorage::STORAGE_TYPE) {
      ...
      if (std::filesystem::path{path}.is_relative() && std::filesystem::exists(config_dir + path)) path = config_dir + path;
...
void AccessControl::addStoragesFromMainConfig(const Poco::Util::AbstractConfiguration& config, const String& config_path,
                                              const zkutil::GetZooKeeper& get_zookeeper_function) {
  ...
  if (users_config_path.empty()) {
    if (!has_user_directories) users_config_path = config_path;
  } else if (std::filesystem::path{users_config_path}.is_relative() && std::filesystem::exists(config_dir + users_config_path))
    users_config_path = config_dir + users_config_path;

  if (!users_config_path.empty()) {
    if (users_config_path != config_path) checkForUsersNotInMainConfig(config, config_path, users_config_path, getLogger());

    addUsersConfigStorage(UsersConfigAccessStorage::STORAGE_TYPE, users_config_path, include_from_path, dbms_dir, get_zookeeper_function,
                          /* allow_backup= */ false);
  }
...
bool AccessControl::removeImpl(const UUID& id, bool throw_if_not_exists) {
  ...
  if (removed) changes_notifier->sendNotifications();
  return removed;
}
...
bool AccessControl::updateImpl(const UUID& id, const UpdateFunc& update_func, bool throw_if_not_exists) {
  ...
  if (updated) changes_notifier->sendNotifications();
  return updated;
}

AccessChangesNotifier& AccessControl::getChangesNotifier() { return *changes_notifier; }

AuthResult AccessControl::authenticate(const Credentials& credentials, const Poco::Net::IPAddress& address,
  ...
  try {
    ...
    if (authentication_quota) authentication_quota->reset(QuotaType::FAILED_SEQUENTIAL_AUTHENTICATIONS);
    ...
    if (credentials.getUserName().empty() || credentials.getUserName() == "default") {
      if (credentials.allowInteractiveBasicAuthenticationInTheBrowser()) error_code = ErrorCodes::REQUIRED_PASSWORD;
...
See also /etc/clickhouse-server/users.xml on the server where ClickHouse is installed.
    ...
    if (getCurrentExceptionCode() == ErrorCodes::REQUIRED_SECOND_FACTOR) error_code = ErrorCodes::REQUIRED_SECOND_FACTOR;

    /// We use the same message for all authentication failures because we don't want to give away any unnecessary information for security
    /// reasons. Only the log ((*), above) will show the exact reason. Note that (*) logs at information level instead of the default error
    /// level as authentication failures are not an unusual event.
    throw Exception(
        PreformattedMessage{message.str(), "{}: Authentication failed: password is incorrect, or there is no user with such name",
...
void AccessControl::setCustomSettingsPrefixes(const Strings& prefixes) { custom_settings_prefixes->registerPrefixes(prefixes); }
...
bool AccessControl::isImplicitNoPasswordAllowed() const { return allow_implicit_no_password; }

void AccessControl::setNoPasswordAllowed(bool allow_no_password_) { allow_no_password = allow_no_password_; }

bool AccessControl::isNoPasswordAllowed() const { return allow_no_password; }

void AccessControl::setPlaintextPasswordAllowed(bool allow_plaintext_password_) { allow_plaintext_password = allow_plaintext_password_; }

bool AccessControl::isPlaintextPasswordAllowed() const { return allow_plaintext_password; }
...
AuthenticationType AccessControl::getDefaultPasswordType() const { return default_password_type; }
...
void AccessControl::checkPasswordComplexityRules(const String& password_) const { password_rules->checkPasswordComplexityRules(password_); }
...
int AccessControl::getBcryptWorkfactor() const { return bcrypt_workfactor; }
...
bool AccessControl::isEnabledUserNameAccessType() const { return enable_user_name_access_type; }

void AccessControl::setEnableReadWriteGrants(bool enable_read_write_grants_) { enable_read_write_grants = enable_read_write_grants_; }

bool AccessControl::isEnabledReadWriteGrants() const { return enable_read_write_grants; }
...
std::shared_ptr<const EnabledRoles> AccessControl::getEnabledRoles(const std::vector<UUID>& current_roles,
                                                                   const std::vector<UUID>& current_roles_with_admin_option) const {
  return role_cache->getEnabledRoles(current_roles, current_roles_with_admin_option);
}

std::shared_ptr<const EnabledRolesInfo> AccessControl::getEnabledRolesInfo(const std::vector<UUID>& current_roles,
                                                                           const std::vector<UUID>& current_roles_with_admin_option) const {
  return getEnabledRoles(current_roles, current_roles_with_admin_option)->getRolesInfo();
}

std::shared_ptr<const EnabledRowPolicies> AccessControl::getEnabledRowPolicies(
    const UUID& user_id, const boost::container::flat_set<UUID>& enabled_roles) const {
...
std::shared_ptr<const EnabledQuota> AccessControl::getEnabledQuota(const UUID& user_id, const String& user_name,
                                                                   const boost::container::flat_set<UUID>& enabled_roles,
                                                                   const std::shared_ptr<Poco::Net::IPAddress>& address,
                                                                   const String& forwarded_address, const String& custom_quota_key) const {
  return quota_cache->getEnabledQuota(user_id, user_name, enabled_roles, address, forwarded_address, custom_quota_key, true);
}

std::shared_ptr<const EnabledQuota> AccessControl::getAuthenticationQuota(const String& user_name, const Poco::Net::IPAddress& address,
  ...
  if (user_id && (user = tryRead<User>(*user_id))) {
    ...
    return quota_cache->getEnabledQuota(*user_id, user->getName(), roles_info->enabled_roles,
                                        std::make_shared<Poco::Net::IPAddress>(address), forwarded_address, quota_key,
                                        throw_if_client_key_empty);
  }
  return nullptr;
}

std::vector<QuotaUsage> AccessControl::getAllQuotasUsage() const { return quota_cache->getAllQuotasUsage(); }

std::shared_ptr<const EnabledSettings> AccessControl::getEnabledSettings(const UUID& user_id,
...
std::shared_ptr<const SettingsProfilesInfo> AccessControl::getEnabledSettingsInfo(
    const UUID& user_id, const SettingsProfileElements& settings_from_user, const boost::container::flat_set<UUID>& enabled_roles,
    const SettingsProfileElements& settings_from_enabled_roles) const {
...
const ExternalAuthenticators& AccessControl::getExternalAuthenticators() const { return *external_authenticators; }

void AccessControl::allowAllSettings() { custom_settings_prefixes->registerPrefixes({""}); }
...
UInt32 AccessControl::getAllowTierSettings() const {
  if (allow_experimental_tier_settings) return 0;
  if (allow_beta_tier_settings) return 1;
  return 2;
}

bool AccessControl::getAllowExperimentalTierSettings() const { return allow_experimental_tier_settings; }

bool AccessControl::getAllowBetaTierSettings() const { return allow_beta_tier_settings; }
}  // namespace DB
