...
void LDAPClient::Params::updateHash(SipHash &hash) const {
  ...
  if (user_dn_detection) user_dn_detection->updateHash(hash);
}

LDAPClient::LDAPClient(const Params &params_) : params(params_) {}

LDAPClient::~LDAPClient() { closeConnection(); }
...
auto replacePlaceholders(const String &src, const std::vector<std::pair<String, String>> &pairs) {
  ...
  for (const auto &pair : pairs) {
    ...
    for (auto pos = dest.find(placeholder); pos != std::string::npos; pos = dest.find(placeholder, pos)) {
...
bool LDAPClient::openConnection() {
...

  {
    ...
    if (!uri) throw Exception(ErrorCodes::LDAP_ERROR, "ldap_url_desc2str() failed");

    SCOPE_EXIT({ ldap_memfree(uri); });

    handleError(ldap_initialize(&handle, uri));
    if (!handle) throw Exception(ErrorCodes::LDAP_ERROR, "ldap_initialize() failed");
  }

  {
    int value = 0;
    switch (params.protocol_version) {
      case LDAPClient::Params::ProtocolVersion::V2:
        value = LDAP_VERSION2;
        break;
      case LDAPClient::Params::ProtocolVersion::V3:
        value = LDAP_VERSION3;
        break;
    }
...
#  ifdef LDAP_OPT_REFERRALS
  handleError(ldap_set_option(handle, LDAP_OPT_REFERRALS, params.follow_referrals ? LDAP_OPT_ON : LDAP_OPT_OFF));
#  endif
...
#  ifdef LDAP_OPT_X_TLS_PROTOCOL_MIN
  {
    ...
    switch (params.tls_minimum_protocol_version) {
      case LDAPClient::Params::TLSProtocolVersion::SSL2:
        value = LDAP_OPT_X_TLS_PROTOCOL_SSL2;
        break;
      case LDAPClient::Params::TLSProtocolVersion::SSL3:
        value = LDAP_OPT_X_TLS_PROTOCOL_SSL3;
        break;
      case LDAPClient::Params::TLSProtocolVersion::TLS1_0:
        value = LDAP_OPT_X_TLS_PROTOCOL_TLS1_0;
        break;
      case LDAPClient::Params::TLSProtocolVersion::TLS1_1:
        value = LDAP_OPT_X_TLS_PROTOCOL_TLS1_1;
        break;
      case LDAPClient::Params::TLSProtocolVersion::TLS1_2:
        value = LDAP_OPT_X_TLS_PROTOCOL_TLS1_2;
        break;
    }
...
#  ifdef LDAP_OPT_X_TLS_REQUIRE_CERT
  {
    ...
    switch (params.tls_require_cert) {
      case LDAPClient::Params::TLSRequireCert::NEVER:
        value = LDAP_OPT_X_TLS_NEVER;
        break;
      case LDAPClient::Params::TLSRequireCert::ALLOW:
        value = LDAP_OPT_X_TLS_ALLOW;
        break;
      case LDAPClient::Params::TLSRequireCert::TRY:
        value = LDAP_OPT_X_TLS_TRY;
        break;
      case LDAPClient::Params::TLSRequireCert::DEMAND:
...
#  ifdef LDAP_OPT_X_TLS_CERTFILE
  if (!params.tls_cert_file.empty()) handleError(ldap_set_option(handle, LDAP_OPT_X_TLS_CERTFILE, params.tls_cert_file.c_str()));
#  endif

#  ifdef LDAP_OPT_X_TLS_KEYFILE
  if (!params.tls_key_file.empty()) handleError(ldap_set_option(handle, LDAP_OPT_X_TLS_KEYFILE, params.tls_key_file.c_str()));
#  endif

#  ifdef LDAP_OPT_X_TLS_CACERTFILE
  if (!params.tls_ca_cert_file.empty()) handleError(ldap_set_option(handle, LDAP_OPT_X_TLS_CACERTFILE, params.tls_ca_cert_file.c_str()));
#  endif

#  ifdef LDAP_OPT_X_TLS_CACERTDIR
  if (!params.tls_ca_cert_dir.empty()) handleError(ldap_set_option(handle, LDAP_OPT_X_TLS_CACERTDIR, params.tls_ca_cert_dir.c_str()));
#  endif

#  ifdef LDAP_OPT_X_TLS_CIPHER_SUITE
  if (!params.tls_cipher_suite.empty()) handleError(ldap_set_option(handle, LDAP_OPT_X_TLS_CIPHER_SUITE, params.tls_cipher_suite.c_str()));
#  endif
...
#  endif
  ...
  if (params.enable_tls == LDAPClient::Params::TLSEnable::YES_STARTTLS) handleError(ldap_start_tls_s(handle, nullptr, nullptr));
  ...
  switch (params.sasl_mechanism) {
    case LDAPClient::Params::SASLMechanism::SIMPLE: {
...

      {
        ...
        if (rc == LDAP_INVALID_CREDENTIALS) return false;
      ...
      if (params.user_dn_detection) {
        ...
        if (user_dn_search_results.empty()) throw Exception(ErrorCodes::LDAP_ERROR, "Failed to detect user DN: empty search results");
...
LDAPClient::SearchResults LDAPClient::search(const SearchParams &search_params) {
  ...
  switch (search_params.scope) {
    case SearchParams::Scope::BASE:
      scope = LDAP_SCOPE_BASE;
      break;
    case SearchParams::Scope::ONE_LEVEL:
      scope = LDAP_SCOPE_ONELEVEL;
      break;
    case SearchParams::Scope::SUBTREE:
      scope = LDAP_SCOPE_SUBTREE;
      break;
    case SearchParams::Scope::CHILDREN:
      scope = LDAP_SCOPE_CHILDREN;
      break;
  }

  const auto final_base_dn = replacePlaceholders(
      search_params.base_dn, {{"{user_name}", final_user_name}, {"{bind_dn}", final_bind_dn}, {"{user_dn}", final_user_dn}});

  const auto final_search_filter = replacePlaceholders(search_params.search_filter, {{"{user_name}", escapeForFilter(final_user_name)},
  ...
  handleError(ldap_search_ext_s(handle, final_base_dn.c_str(), scope, final_search_filter.c_str(), attrs, 0, nullptr, nullptr, &timeout,
                                params.search_limit, &msgs));

  for (auto *msg = ldap_first_message(handle, msgs); msg != nullptr; msg = ldap_next_message(handle, msg)) {
    switch (ldap_msgtype(msg))  // NOLINT(bugprone-switch-missing-default-case)
    {
      case LDAP_RES_SEARCH_ENTRY: {
        ...
        if (boost::iequals("dn", search_params.attribute)) {
          ...
          if (bv.bv_val && bv.bv_len > 0) result.emplace(bv.bv_val, bv.bv_len);
        }
        ...
        for (auto *attr = ldap_first_attribute(handle, msg, &ber); attr != nullptr; attr = ldap_next_attribute(handle, msg, ber)) {
          SCOPE_EXIT({
            ldap_memfree(attr);
            attr = nullptr;
          });
          ...
          if (search_params.attribute.empty() || boost::iequals(attr, search_params.attribute)) {
            ...
            if (vals) {
              ...
              for (size_t i = 0; vals[i]; ++i) {
                if (vals[i]->bv_val && vals[i]->bv_len > 0) result.emplace(vals[i]->bv_val, vals[i]->bv_len);
              }
            }
          }
        }
        ...
        if (referrals) {
          ...
          for (size_t i = 0; referrals[i]; ++i) {
            if (params.follow_referrals)
              LOG_TRACE(getLogger("LDAPClient"), "Received LDAP search reference: {} (library referral chasing enabled)", referrals[i]);
            else
              LOG_TRACE(getLogger("LDAPClient"), "Received LDAP search reference but not following it: {}", referrals[i]);
          }
        }
...
bool LDAPSimpleAuthClient::authenticate(const RoleSearchParamsList *role_search_params, SearchResultsList *role_search_results) {
  if (params.user.empty()) throw Exception(ErrorCodes::BAD_ARGUMENTS, "LDAP authentication of a user with empty name is not allowed");

  if (!role_search_params != !role_search_results) throw Exception(ErrorCodes::BAD_ARGUMENTS, "Cannot return LDAP search results");

  // Silently reject authentication attempt if the password is empty as if it didn't match.
  if (params.password.empty()) return false;
