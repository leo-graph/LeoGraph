...
ReplicatedAccessStorage::ReplicatedAccessStorage(const String& storage_name_, const String& zookeeper_path_,
                                                 zkutil::GetZooKeeper get_zookeeper_, AccessChangesNotifier& changes_notifier_,
                                                 bool allow_backup_, bool throw_on_invalid_entities_)
  ...
  if (zookeeper_path_.empty()) throw Exception(ErrorCodes::BAD_ARGUMENTS, "ZooKeeper path must be non-empty");
}
...
void ReplicatedAccessStorage::shutdown() { replicator.shutdown(); }
...
void ReplicatedAccessStorage::reload(ReloadMode reload_mode) { replicator.reload(reload_mode == ReloadMode::ALL); }
...
std::vector<UUID> ReplicatedAccessStorage::findAllImpl(AccessEntityType type) const { return replicator.findAllEntities(type); }

bool ReplicatedAccessStorage::exists(const UUID& id) const { return replicator.exists(id); }
