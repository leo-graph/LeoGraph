...
SettingsProfileElement::SettingsProfileElement(const ASTSettingsProfileElement& ast) { init(ast, nullptr); }
...
void SettingsProfileElement::init(const ASTSettingsProfileElement& ast, const AccessControl* access_control) {
  auto name_to_id = [id_mode{ast.id_mode}, access_control](const String& name_) -> UUID {
    if (id_mode) return parse<UUID>(name_);
    assert(access_control);
    return access_control->getID<SettingsProfile>(name_);  /// NOLINT(clang-analyzer-core.CallAndMessage)
  };

  if (!ast.parent_profile.empty()) parent_profile = name_to_id(ast.parent_profile);
  ...
  if (!ast.setting_name.empty()) {
    ...
    if (access_control) {
      ...
      if (!SettingsProfileElements::isAllowBackupSetting(setting_name)) access_control->checkSettingNameIsAllowed(setting_name);
      /// Check if a CHANGEABLE_IN_READONLY is allowed.
      if (ast.writability == SettingConstraintWritability::CHANGEABLE_IN_READONLY &&
          !access_control->doesSettingsConstraintsReplacePrevious())
        throw Exception(ErrorCodes::NOT_IMPLEMENTED,
                        "CHANGEABLE_IN_READONLY for {} "
                        "is not allowed unless settings_constraints_replace_previous is enabled",
                        setting_name);
    }
    ...
    if (value) value = Settings::castValueUtil(setting_name, *value);
    if (min_value) min_value = Settings::castValueUtil(setting_name, *min_value);
    if (max_value) max_value = Settings::castValueUtil(setting_name, *max_value);
    for (auto& allowed_value : disallowed_values) value = Settings::castValueUtil(setting_name, allowed_value);
  }
}
...
boost::intrusive_ptr<ASTSettingsProfileElement> SettingsProfileElement::toAST() const {
  ...
  if (parent_profile) ast->parent_profile = ::DB::toString(*parent_profile);
...
boost::intrusive_ptr<ASTSettingsProfileElement> SettingsProfileElement::toASTWithNames(const AccessControl& access_control) const {
  ...
  if (parent_profile) {
    ...
    if (parent_profile_name) ast->parent_profile = *parent_profile_name;
  }
...
SettingsProfileElements::SettingsProfileElements(const ASTSettingsProfileElements& ast, bool normalize_) {
  for (const auto& ast_element : ast.elements) emplace_back(*ast_element);
...
SettingsProfileElements::SettingsProfileElements(const ASTSettingsProfileElements& ast, const AccessControl& access_control,
                                                 bool normalize_) {
  for (const auto& ast_element : ast.elements) emplace_back(*ast_element, access_control);
  if (normalize_) normalize();
}
...
boost::intrusive_ptr<ASTSettingsProfileElements> SettingsProfileElements::toAST() const {
  ...
  for (const auto& element : *this) {
    ...
    if (!element_ast->empty()) res->elements.push_back(element_ast);
  }
  return res;
}
...
std::vector<UUID> SettingsProfileElements::findDependencies() const {
  std::vector<UUID> res;
  for (const auto& element : *this) {
    if (element.parent_profile) res.push_back(*element.parent_profile);
  }
  return res;
}
...
bool SettingsProfileElements::hasDependencies(const std::unordered_set<UUID>& ids) const {
  std::vector<UUID> res;
  for (const auto& element : *this) {
    if (element.parent_profile && ids.contains(*element.parent_profile)) return true;
  }
  return false;
}
...
void SettingsProfileElements::removeDependencies(const std::unordered_set<UUID>& ids) {
  std::erase_if(*this,
                [&](const SettingsProfileElement& element) { return element.parent_profile && ids.contains(*element.parent_profile); });
}

void SettingsProfileElements::removeSettingsKeepProfiles() {
  for (auto& element : *this) element.setting_name.clear();
...
Settings SettingsProfileElements::toSettings() const {
  ...
  for (const auto& elem : *this) {
    if (!elem.setting_name.empty() && !isAllowBackupSetting(elem.setting_name) && elem.value) res.set(elem.setting_name, *elem.value);
  }
  return res;
}
...
SettingsChanges SettingsProfileElements::toSettingsChanges() const {
  ...
  for (const auto& elem : *this) {
    if (!elem.setting_name.empty() && !isAllowBackupSetting(elem.setting_name)) {
      if (elem.value) res.push_back({elem.setting_name, *elem.value});
    }
  }
  return res;
}
...
SettingsConstraints SettingsProfileElements::toSettingsConstraints(const AccessControl& access_control) const {
  ...
  for (const auto& elem : *this)
    if (!elem.setting_name.empty() && elem.isConstraint() && !isAllowBackupSetting(elem.setting_name))
      res.set(elem.setting_name, elem.min_value ? *elem.min_value : Field{}, elem.max_value ? *elem.max_value : Field{},
              elem.disallowed_values, elem.writability ? *elem.writability : SettingConstraintWritability::WRITABLE);
  return res;
}
...
std::vector<UUID> SettingsProfileElements::toProfileIDs() const {
  std::vector<UUID> res;
  for (const auto& elem : *this) {
    if (elem.parent_profile) res.push_back(*elem.parent_profile);
  }
...
void SettingsProfileElements::normalize() {
  ...
  auto profiles_end = std::stable_partition(
      begin(), end(), [](const SettingsProfileElement& element) { return static_cast<bool>(element.parent_profile); });
  ...
  /// if pr_A and pr_B give different values to same settings.
  {
    ...
    while (it != profiles_begin) {
      --it;
      auto& element = *it;
      if (element.parent_profile && !profile_ids.emplace(*element.parent_profile).second) element.parent_profile.reset();
    }
  }
  ...
  /// We keep the first position of any used setting, and merge settings with the same name to that first element.
  {
    ...
    for (auto it = settings_begin; it != settings_end; ++it) {
      ...
      if (it != first) {
        ...
        if (element.value) first_element.value = element.value;
        if (element.min_value) first_element.min_value = element.min_value;
        if (element.max_value) first_element.max_value = element.max_value;
        if (!element.disallowed_values.empty()) first_element.disallowed_values = element.disallowed_values;
        if (element.writability) first_element.writability = element.writability;
...
bool SettingsProfileElements::isBackupAllowed() const {
  for (const auto& setting : *this) {
    if (isAllowBackupSetting(setting.setting_name) && setting.value) return static_cast<bool>(SettingFieldBool{*setting.value});
  }
  return true;
}
...
AlterSettingsProfileElements::AlterSettingsProfileElements(const ASTAlterSettingsProfileElements& ast) {
  ...
  if (ast.modify_settings) modify_settings = SettingsProfileElements{*ast.modify_settings, /* normalize= */ false};

  if (ast.drop_settings) drop_settings = SettingsProfileElements{*ast.drop_settings, /* normalize= */ false};
}
...
AlterSettingsProfileElements::AlterSettingsProfileElements(const ASTAlterSettingsProfileElements& ast,
  ...
  if (ast.add_settings)
    add_settings = SettingsProfileElements{*ast.add_settings, access_control,
                                           /* normalize= */ false};  /// For "ALTER" the normalization is unnecessary.

  if (ast.modify_settings) modify_settings = SettingsProfileElements{*ast.modify_settings, access_control, /* normalize= */ false};

  if (ast.drop_settings) drop_settings = SettingsProfileElements{*ast.drop_settings, access_control, /* normalize= */ false};
}
...
void SettingsProfileElements::applyChanges(const AlterSettingsProfileElements& changes) {
  ...
  auto apply_drop_setting = [&](const String& setting_name) {
    for (auto& element : *this) {
      if (element.setting_name == setting_name) element.setting_name.clear();
    }
  };

  auto apply_drop_profile = [&](const UUID& profile_id) {
    for (auto& element : *this) {
      if (element.parent_profile == profile_id) element.parent_profile.reset();
    }
  };

  for (const auto& drop : changes.drop_settings) {
    if (drop.parent_profile) apply_drop_profile(*drop.parent_profile);
    if (!drop.setting_name.empty()) apply_drop_setting(drop.setting_name);
  }
  ...
  for (const auto& add : changes.add_settings) {
    if (add.parent_profile) apply_add_profile(*add.parent_profile);
    if (!add.setting_name.empty()) apply_add_setting(add);
  }
  ...
  for (const auto& modify : changes.modify_settings) {
    ...
    if (!modify.setting_name.empty()) apply_modify_setting(modify);
  }
