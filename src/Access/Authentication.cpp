...
bool checkPasswordDoubleSHA1MySQL(std::string_view scramble, std::string_view scrambled_password, const Digest &password_double_sha1) {
  ...
  if ((scramble.size() < scramble_length) || (scramble.size() > scramble_length + 1) ||
      ((scramble.size() == scramble_length + 1) && (scramble[scramble_length] != 0)) || (scrambled_password.size() != sha1_size) ||
      (password_double_sha1.size() != sha1_size))
    return false;
  ...
  for (size_t i = 0; i < sha1_size; ++i) calculated_password_sha1[i] = static_cast<UInt8>(scrambled_password[i] ^ digest[i]);
...
bool checkSshSignature(const std::vector<SSHKey> &keys, std::string_view signature, std::string_view original) {
  for (const auto &key : keys)
    if (key.isPublic() && key.verifySignature(signature, original)) return true;
  return false;
}
...
bool checkKerberosAuthentication(const GSSAcceptorContext *gss_acceptor_context, const AuthenticationData &authentication_method,
...
std::string computeScramSHA256ClientProof(const std::vector<uint8_t> &salted_password [[maybe_unused]],
                                          const std::string &auth_message [[maybe_unused]]) {
#if USE_SSL
  ...
  for (size_t i = 0; i < client_key.size(); ++i) client_proof[i] = client_key[i] ^ client_signature[i];
...
bool checkScramSHA256Authentication(const ScramSHA256Credentials *scram_sha256_credentials,
  ...
  if (computed_client_proof.size() != client_proof.size()) return false;

  for (size_t i = 0; i < computed_client_proof.size(); ++i) {
    if (static_cast<UInt8>(computed_client_proof[i]) != static_cast<UInt8>(client_proof[i])) return false;
  }
  return true;
}

bool checkMySQLAuthentication(const MySQLNative41Credentials *mysql_credentials, const AuthenticationData &authentication_method) {
  switch (authentication_method.getType()) {
    case AuthenticationType::PLAINTEXT_PASSWORD:
      return checkPasswordPlainTextMySQL(mysql_credentials->getScramble(), mysql_credentials->getScrambledPassword(),
                                         authentication_method.getPasswordHashBinary());
    case AuthenticationType::DOUBLE_SHA1_PASSWORD:
      return checkPasswordDoubleSHA1MySQL(mysql_credentials->getScramble(), mysql_credentials->getScrambledPassword(),
                                          authentication_method.getPasswordHashBinary());
    default:
      return false;
  }
...
/// pAssw0rd+123456
std::pair<std::string_view, std::string_view> splitOneTimePasswordAndPassword(std::string_view password_with_otp,
                                                                              const std::optional<OneTimePasswordSecret> &otp_secret) {
  if (!otp_secret) return {password_with_otp, ""};
  auto num_digits = otp_secret->params.num_digits;
  if (password_with_otp.size() <= static_cast<size_t>(num_digits)) return {password_with_otp, ""};
  size_t separator_pos = password_with_otp.size() - num_digits - 1;
  if (password_with_otp[separator_pos] != '+') return {password_with_otp, ""};
...
Authentication::CredentialsCheckResult checkBasicAuthentication(const BasicCredentials *basic_credentials,
                                                                const AuthenticationData &authentication_method,
                                                                const ExternalAuthenticators &external_authenticators,
  ...
  switch (authentication_method.getType()) {
    ...
    case AuthenticationType::HTTP: {
      ...
      if (authentication_method.getHTTPAuthenticationScheme() == HTTPAuthenticationScheme::BASIC) {
        return external_authenticators.checkHTTPBasicCredentials(authentication_method.getHTTPAuthenticationServerName(),
                                                                 *basic_credentials, client_info, settings)
                   ? on_success
                   : Authentication::CredentialsCheckResult::Fail;
      }
      break;
...
#if USE_SSL
bool checkSSLCertificateAuthentication(const SSLCertificateCredentials *ssl_certificate_credentials,
                                       const AuthenticationData &authentication_method) {
  ...
  for (X509Certificate::Subjects::Type type : {X509Certificate::Subjects::Type::CN, X509Certificate::Subjects::Type::SAN}) {
    for (const auto &subject : authentication_method.getSSLCertificateSubjects().at(type)) {
      if (ssl_certificate_credentials->getSSLCertificateSubjects().at(type).contains(subject)) return true;
      ...
      if (subject.contains('*')) {
        ...
        for (const auto &certificate_subject : ssl_certificate_credentials->getSSLCertificateSubjects().at(type)) {
          ...
          if (matches_wildcard && matches_slashes) return true;
        }
      }
    }
  }
...
#if USE_SSH
bool checkSshAuthentication(const SshCredentials *ssh_credentials, const AuthenticationData &authentication_method) {
 ...
 */
bool checkSSHLoginAuthentication(const SSHPTYCredentials *ssh_login_credentials, const AuthenticationData &authentication_method) {
  return AuthenticationType::SSH_KEY == authentication_method.getType() &&
...
Authentication::CredentialsCheckResult Authentication::areCredentialsValid(const Credentials &credentials,
                                                                           const AuthenticationData &authentication_method,
                                                                           const ExternalAuthenticators &external_authenticators,
                                                                           const ClientInfo &client_info, SettingsChanges &settings) {
  if (!credentials.isReady()) return CredentialsCheckResult::Fail;
  ...
  if (const auto *gss_acceptor_context = typeid_cast<const GSSAcceptorContext *>(&credentials)) {
    return checkKerberosAuthentication(gss_acceptor_context, authentication_method, external_authenticators)
               ? CredentialsCheckResult::Success
               : CredentialsCheckResult::Fail;
  }

  if (const auto *mysql_credentials = typeid_cast<const MySQLNative41Credentials *>(&credentials)) {
    return checkMySQLAuthentication(mysql_credentials, authentication_method) ? CredentialsCheckResult::Success
                                                                              : CredentialsCheckResult::Fail;
  }
  ...
  if (const auto *scram_shh256_credentials = typeid_cast<const ScramSHA256Credentials *>(&credentials)) {
    return checkScramSHA256Authentication(scram_shh256_credentials, authentication_method) ? CredentialsCheckResult::Success
                                                                                           : CredentialsCheckResult::Fail;
  }

#if USE_SSL
  if (const auto *ssl_certificate_credentials = typeid_cast<const SSLCertificateCredentials *>(&credentials)) {
    return checkSSLCertificateAuthentication(ssl_certificate_credentials, authentication_method) ? CredentialsCheckResult::Success
                                                                                                 : CredentialsCheckResult::Fail;
  }
#endif
  ...
  if (const auto *ssh_credentials = typeid_cast<const SshCredentials *>(&credentials)) {
    return checkSshAuthentication(ssh_credentials, authentication_method) ? CredentialsCheckResult::Success : CredentialsCheckResult::Fail;
  }

  if (const auto *ssh_login_credentials = typeid_cast<const SSHPTYCredentials *>(&credentials)) {
    return checkSSHLoginAuthentication(ssh_login_credentials, authentication_method) ? CredentialsCheckResult::Success
                                                                                     : CredentialsCheckResult::Fail;
  }
#endif
