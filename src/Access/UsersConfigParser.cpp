...
namespace {
template <typename T>
void parseGrant(T &entity, const String &string_query, const std::unordered_set<UUID> &role_ids_from_users_config,
                const AccessControl &access_control, LoggerPtr log) {
  ParserGrantQuery parser;
  parser.setParseWithoutGrantees();

  String error_message;
  const char *pos = string_query.data();
  auto ast = tryParseQuery(parser, pos, pos + string_query.size(), error_message, false, "", false, 0, DBMS_DEFAULT_MAX_PARSER_DEPTH,
                           DBMS_DEFAULT_MAX_PARSER_BACKTRACKS, true);

  if (!ast) throw Exception(ErrorCodes::BAD_ARGUMENTS, "Failed to parse grant query. Error: {}", error_message);

  auto &query = ast->as<ASTGrantQuery &>();

  if (query.roles && query.is_revoke) throw Exception(ErrorCodes::BAD_ARGUMENTS, "Roles can't be revoked in config file");

  if (!query.cluster.empty()) throw Exception(ErrorCodes::BAD_ARGUMENTS, "Can't grant on cluster using config file");

  if (query.grantees) throw Exception(ErrorCodes::BAD_ARGUMENTS, "You can't specify grantees in query using config file");
...
UserPtr parseUser(const Poco::Util::AbstractConfiguration &config, String user_name, const std::unordered_set<UUID> &allowed_profile_ids,
                  const std::unordered_set<UUID> &role_ids_from_users_config, const AccessControl &access_control, bool allow_no_password,
                  bool allow_plaintext_password, LoggerPtr log) {
  ...
  if (num_password_fields > 1)
    throw Exception(ErrorCodes::BAD_ARGUMENTS,
                    "More than one field of 'password', 'password_sha256_hex', "
  ...
  if (num_password_fields < 1)
    throw Exception(ErrorCodes::BAD_ARGUMENTS,
                    "Either 'password' or 'password_sha256_hex' "
                    "or 'password_double_sha1_hex' or 'no_password' or 'ldap' or 'kerberos "
                    "or 'ssl_certificates' or 'ssh_keys' or 'http_authentication' must be specified for user {}.",
                    user_name);

  std::optional<OneTimePasswordSecret> otp_secret;
  if (config.has(user_config + ".time_based_one_time_password")) {
    bool is_password_based = has_no_password || has_password_plaintext || has_password_sha256_hex || has_password_double_sha1_hex ||
                             has_scram_password_sha256_hex;
    if (!is_password_based)
      throw Exception(ErrorCodes::BAD_ARGUMENTS, "One-time password can be used only with password based authentication for user {}.",
                      user_name);

    String otp_secret_key = config.getString(user_config + ".time_based_one_time_password.secret");
    OneTimePasswordParams otp_config(config.getInt(user_config + ".time_based_one_time_password.digits", {}),
  ...
  if (has_no_password && otp_secret) {
    ...
    user->authentication_methods.back().setPasswordHashHex(config.getString(user_config + ".password_scram_sha256_hex"), otp_secret,
                                                           validate);
  } else if (has_password_double_sha1_hex) {
    user->authentication_methods.emplace_back(AuthenticationType::DOUBLE_SHA1_PASSWORD);
    user->authentication_methods.back().setPasswordHashHex(config.getString(user_config + ".password_double_sha1_hex"), otp_secret,
                                                           validate);
  } else if (has_ldap) {
    ...
    if (ldap_server_name.empty()) throw Exception(ErrorCodes::BAD_ARGUMENTS, "LDAP server name cannot be empty for user {}.", user_name);
    ...
    for (const String &key : keys) {
      if (key.starts_with("common_name")) {
        ...
        String value = config.getString(certificates_config + "." + key);
        if (value.empty()) throw Exception(ErrorCodes::BAD_ARGUMENTS, "Expected ssl_certificates.subject_alt_name to not be empty");
    ...
    throw Exception(ErrorCodes::SUPPORT_IS_DISABLED,
                    "SSL certificates support is disabled, because ClickHouse was built without SSL library");
#endif
  ...
  for (const auto &authentication_method : user->authentication_methods) {
    ...
    if (((auth_type == AuthenticationType::NO_PASSWORD) && !allow_no_password) ||
      ...
      throw Exception(ErrorCodes::BAD_ARGUMENTS,
                      "Authentication type {} is not allowed, check the setting allow_{} in the server configuration", toString(auth_type),
  ...
  bool named_collection_control =
      config.getBool(user_config + ".named_collection_control", false) || config.getBool(user_config + ".named_collection_admin", false);
  ...
  if (grant_queries) {
    for (const auto &string_query : *grant_queries) parseGrant(*user, string_query, role_ids_from_users_config, access_control, log);
  } else {
    ...
    if (databases) {
      ...
      for (const String &database : *databases) user->access.grantWithGrantOption(AccessFlags::allFlags(), database);
    }
...
RolePtr parseRole(const Poco::Util::AbstractConfiguration &config, const String &role_name,
                  const std::unordered_set<UUID> &role_ids_from_users_config, const AccessControl &access_control, LoggerPtr log) {
...
QuotaPtr parseQuota(const Poco::Util::AbstractConfiguration &config, const String &quota_name, const std::vector<UUID> &user_ids) {
  ...
  for (const String &interval_key : interval_keys) {
    if (!startsWith(interval_key, "interval")) continue;
    ...
    for (auto quota_type : collections::range(QuotaType::MAX)) {
      ...
      if (value != "0") limits.max[static_cast<size_t>(quota_type)] = type_info.stringToValue(value);
    }
  }
...
SettingsProfileElements parseSettingsConstraints(const Poco::Util::AbstractConfiguration &config, const String &path_to_constraints,
  ...
  for (const String &setting_name : keys) {
    ...
    for (const String &constraint_type : constraint_types) {
      ...
      /// </constraints>
        ...
        profile_element.disallowed_values.push_back(
            settingStringToValueUtil(setting_name, config.getString(path_to_name + "." + constraint_type)));
        ...
        if (access_control.doesSettingsConstraintsReplacePrevious())
          ...
          throw Exception(ErrorCodes::NOT_IMPLEMENTED,
                          "Setting changeable_in_readonly for {} is not allowed "
                          "unless settings_constraints_replace_previous is enabled",
                          setting_name);
      } else
    ...
    if (writability_count > 1)
      throw Exception(ErrorCodes::NOT_IMPLEMENTED,
                      "Not more than one constraint writability specifier "
                      "(const/readonly/changeable_in_readonly) is allowed for {}",
                      setting_name);
...
std::shared_ptr<SettingsProfile> parseSettingsProfile(const Poco::Util::AbstractConfiguration &config, const String &profile_name,
...
UsersConfigParser::UsersConfigParser(AccessControl &access_control_)
    : access_control(access_control_), log(getLogger("UsersConfigParser")) {}
...
std::vector<AccessEntityPtr> UsersConfigParser::parseUsers(const Poco::Util::AbstractConfiguration &config,
  ...
  for (const auto &user_name : user_names) {
    try {
      users.push_back(parseUser(config, user_name, allowed_profile_ids, role_ids_from_users_config, access_control, no_password_allowed,
                                plaintext_password_allowed, log));
    } catch (Exception &e) {
...
std::vector<AccessEntityPtr> UsersConfigParser::parseRoles(const Poco::Util::AbstractConfiguration &config,
                                                           const std::unordered_set<UUID> &role_ids_from_users_config) const {
...
std::vector<AccessEntityPtr> UsersConfigParser::parseRowPolicies(const Poco::Util::AbstractConfiguration &config) const {
  ...
  std::map<std::pair<String /* database */, String /* table */>, std::unordered_map<String /* user */, String /* filter */>>
      all_filters_map;
  ...
  for (auto &[database_and_table_name, user_to_filters] : all_filters_map) {
    ...
    for (const String &user_name : user_names) {
      ...
      if (it != user_to_filters.end()) {
        ...
        if (users_without_row_policies_can_read_rows) continue;
        filter = "1";
      }
...
std::vector<AccessEntityPtr> UsersConfigParser::parseSettingsProfiles(const Poco::Util::AbstractConfiguration &config,
...
std::unordered_set<UUID> UsersConfigParser::getAllowedIDs(const Poco::Util::AbstractConfiguration &config, const String &configuration_key,
                                                          AccessEntityType type) {
  Poco::Util::AbstractConfiguration::Keys keys;
  config.keys(configuration_key, keys);
  std::unordered_set<UUID> ids;
  for (const auto &key : keys) ids.emplace(generateID(type, key));
  return ids;
}
