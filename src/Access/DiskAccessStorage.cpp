...
void writeEntityFile(const String& file_path, const IAccessEntity& entity) {
  ...
  SCOPE_EXIT({
    if (!succeeded) (void)std::filesystem::remove(tmp_file_path);
  });
...
String makeDirectoryPathCanonical(const String& directory_path) {
  ...
  if (canonical_directory_path.has_filename()) canonical_directory_path += std::filesystem::path::preferred_separator;
  return canonical_directory_path;
}

/// Calculates the path to a file named <id>.sql for saving an access entity.
String getEntityFilePath(const String& directory_path, const UUID& id) { return directory_path + toString(id) + ".sql"; }
...
String getNeedRebuildListsMarkFilePath(const String& directory_path) { return directory_path + "need_rebuild_lists.mark"; }

bool tryParseUUID(const String& str, UUID& id) { return tryParse(id, str); }
}  // namespace

DiskAccessStorage::DiskAccessStorage(const String& storage_name_, const String& directory_path_, AccessChangesNotifier& changes_notifier_,
                                     bool readonly_, bool allow_backup_)
  ...
  if (!std::filesystem::exists(directory_path) || !std::filesystem::is_directory(directory_path) || create_dir_error_code)
    throw Exception(ErrorCodes::DIRECTORY_DOESNT_EXIST, "Couldn't create directory {} reason: '{}'", directory_path,
  ...
  if (!should_rebuild_lists) {
    if (!readLists()) should_rebuild_lists = true;
  }
...
String DiskAccessStorage::getStorageParamsJSON() const {
  ...
  if (readonly_loaded) json.set("readonly", Poco::Dynamic::Var{true});
...
bool DiskAccessStorage::readLists() {
  ...
  for (auto type : collections::range(AccessEntityType::MAX)) {
    ...
    try {
      for (auto& [id, name] : readListFile(file_path)) ids_entities.emplace_back(id, std::make_shared<EntityOnDisk>(std::move(name), type));
    } catch (...) {
...
void DiskAccessStorage::writeLists() {
  ...
  if (types_of_lists_to_write.empty()) return;
  ...
  for (const auto& type : types_of_lists_to_write) {
    ...
    try {
      ...
      for (const auto& [id, entity] : all_entities) id_name_pairs.emplace_back(id, entity->getName());
...
void DiskAccessStorage::scheduleWriteLists(AccessEntityType type) {
  ...
  if (lists_writing_thread && lists_writing_thread->joinable()) lists_writing_thread->join();
...
void DiskAccessStorage::reloadAllAndRebuildLists() {
  ...
  for (const auto& directory_entry : std::filesystem::directory_iterator(directory_path)) {
    ...
    if (path.extension() != ".sql") continue;

    UUID id;
    if (!tryParseUUID(path.stem(), id)) continue;
  ...
  for (auto type : collections::range(AccessEntityType::MAX)) types_of_lists_to_write.insert(type);
...
void DiskAccessStorage::reload(ReloadMode reload_mode) {
  if (reload_mode != ReloadMode::ALL) return;
...
AccessEntityPtr DiskAccessStorage::readImpl(const UUID& id, bool throw_if_not_exists) const {
  ...
  if (isNotLoadedFromDisk(entity)) entity = readAccessEntityFromDisk(id);
...
bool DiskAccessStorage::insertNoLock(const UUID& id, const AccessEntityPtr& new_entity, bool replace_if_exists, bool throw_if_exists,
                                     UUID* conflicting_id, bool write_on_disk) {
  /// Check that we can insert.
  if (readonly) throwReadonlyCannotInsert(new_entity->getType(), new_entity->getName());
  ...
  /// Do insertion.
  if (!memory_storage.insert(id, new_entity, replace_if_exists, throw_if_exists, conflicting_id)) return false;
...
bool DiskAccessStorage::removeNoLock(const UUID& id, bool throw_if_not_exists, bool write_on_disk) {
  ...
  if (readonly) throwReadonlyCannotRemove(type, entity->getName());
...
bool DiskAccessStorage::updateNoLock(const UUID& id, const UpdateFunc& update_func, bool throw_if_not_exists, bool write_on_disk) {
  ...
  if (readonly) throwReadonlyCannotUpdate(old_entity->getType(), old_entity->getName());
  ...
  if (!memory_storage.update(id, update_func, throw_if_not_exists)) return false;

  AccessEntityPtr new_entity = memory_storage.read(id, throw_if_not_exists);
  if (write_on_disk) {
    if (old_entity->getName() != new_entity->getName()) scheduleWriteLists(new_entity->getType());
    writeAccessEntityToDisk(id, *new_entity);
  }
...
void DiskAccessStorage::deleteAccessEntityOnDisk(const UUID& id) const {
  ...
  if (!std::filesystem::remove(file_path)) throw Exception(ErrorCodes::FILE_DOESNT_EXIST, "Couldn't delete {}", file_path);
}
