...
class LDAPClient {
 public:
  struct SearchParams {
    enum class Scope : uint8_t { BASE, ONE_LEVEL, SUBTREE, CHILDREN };
  ...
  struct RoleSearchParams : public SearchParams {
    String prefix;
  ...
  struct Params {
    enum class ProtocolVersion : uint8_t { V2, V3 };

    enum class TLSEnable : uint8_t { NO, YES_STARTTLS, YES };

    enum class TLSProtocolVersion : uint8_t { SSL2, SSL3, TLS1_0, TLS1_1, TLS1_2 };

    enum class TLSRequireCert : uint8_t { NEVER, ALLOW, TRY, DEMAND };

    enum class SASLMechanism : uint8_t { UNKNOWN, SIMPLE };
...
class LDAPSimpleAuthClient : private LDAPClient {
 public:
