...
RolesOrUsersSet::RolesOrUsersSet(AllTag) { all = true; }

RolesOrUsersSet::RolesOrUsersSet(const UUID& id) { add(id); }

RolesOrUsersSet::RolesOrUsersSet(const std::vector<UUID>& ids_) { add(ids_); }

RolesOrUsersSet::RolesOrUsersSet(const ASTRolesOrUsersSet& ast) { init(ast, nullptr); }
...
RolesOrUsersSet::RolesOrUsersSet(const ASTRolesOrUsersSet& ast, const AccessControl& access_control) { init(ast, &access_control); }

RolesOrUsersSet::RolesOrUsersSet(const ASTRolesOrUsersSet& ast, const AccessControl& access_control,
                                 const std::optional<UUID>& current_user_id) {
...
void RolesOrUsersSet::init(const ASTRolesOrUsersSet& ast, const AccessControl* access_control, const std::optional<UUID>& current_user_id) {
  ...
  auto name_to_id = [&ast, access_control](const String& name) -> UUID {
    if (ast.id_mode) return parse<UUID>(name);
  ...
  if (!ast.names.empty() && !all) {
    ...
    for (const String& name : ast.names) ids.insert(name_to_id(name));
  }
  ...
  if (!ast.except_names.empty()) {
    ...
    for (const String& name : ast.except_names) except_ids.insert(name_to_id(name));
  }
  ...
  for (const UUID& id : except_ids) ids.erase(id);
}
...
boost::intrusive_ptr<ASTRolesOrUsersSet> RolesOrUsersSet::toAST() const {
  ...
  if (!ids.empty() && !all) {
    ...
    for (const UUID& id : ids) ast->names.emplace_back(::DB::toString(id));
    ::sort(ast->names.begin(), ast->names.end());
  }

  if (!except_ids.empty()) {
    ast->except_names.reserve(except_ids.size());
    for (const UUID& except_id : except_ids) ast->except_names.emplace_back(::DB::toString(except_id));
...
boost::intrusive_ptr<ASTRolesOrUsersSet> RolesOrUsersSet::toASTWithNames(const AccessControl& access_control) const {
  ...
  if (!ids.empty() && !all) {
    ...
    for (const UUID& id : ids) {
      ...
      if (name) ast->names.emplace_back(std::move(*name));
    }
    ::sort(ast->names.begin(), ast->names.end());
  }
  ...
  if (!except_ids.empty()) {
    ...
    for (const UUID& except_id : except_ids) {
      ...
      if (except_name) ast->except_names.emplace_back(std::move(*except_name));
    }
    ::sort(ast->except_names.begin(), ast->except_names.end());
  }
...
bool RolesOrUsersSet::empty() const { return ids.empty() && !all; }
...
void RolesOrUsersSet::add(const UUID& id) {
  if (!all) ids.insert(id);
  except_ids.erase(id);
}

void RolesOrUsersSet::add(const std::vector<UUID>& ids_) {
  if (!all) ids.insert(ids_.begin(), ids_.end());
  for (const auto& id : ids_) except_ids.erase(id);
}

bool RolesOrUsersSet::match(const UUID& id) const { return (all || ids.contains(id)) && !except_ids.contains(id); }
...
bool RolesOrUsersSet::match(const UUID& user_id, const boost::container::flat_set<UUID>& enabled_roles) const {
  if (!all && !ids.contains(user_id)) {
    bool found_enabled_role =
        std::any_of(enabled_roles.begin(), enabled_roles.end(), [this](const UUID& enabled_role) { return ids.contains(enabled_role); });
    if (!found_enabled_role) return false;
  }

  if (except_ids.contains(user_id)) return false;

  bool in_except_list = std::any_of(enabled_roles.begin(), enabled_roles.end(),
                                    [this](const UUID& enabled_role) { return except_ids.contains(enabled_role); });
  return !in_except_list;
}

std::vector<UUID> RolesOrUsersSet::getMatchingIDs() const {
  if (all) throw Exception(ErrorCodes::LOGICAL_ERROR, "getAllMatchingIDs() can't get ALL ids without access_control");
  std::vector<UUID> res;
...
std::vector<UUID> RolesOrUsersSet::getMatchingIDs(const AccessControl& access_control) const {
  if (!all) return getMatchingIDs();

  std::vector<UUID> res;
  for (const UUID& id : access_control.findAll<User>()) {
    if (match(id)) res.push_back(id);
  }
...
bool RolesOrUsersSet::hasDependencies(const std::unordered_set<UUID>& dependencies_ids) const {
  for (const auto& id : ids) {
    if (dependencies_ids.contains(id)) return true;
  }
...
void RolesOrUsersSet::copyDependenciesFrom(const RolesOrUsersSet& src, const std::unordered_set<UUID>& dependencies_ids) {
  if (all != src.all) return;

  for (const auto& id : src.ids) {
    if (dependencies_ids.contains(id)) ids.emplace(id);
  }

  for (const auto& id : src.except_ids) {
    if (dependencies_ids.contains(id)) except_ids.emplace(id);
  }
}
