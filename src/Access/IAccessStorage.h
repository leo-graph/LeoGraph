...
class IAccessStorage : public boost::noncopyable {
 public:
  ...
  std::optional<UUID> find(const String &name) const {
    return find(EntityClassT::TYPE, name);
  }

  std::vector<UUID> find(AccessEntityType type, const Strings &names) const;

  template <typename EntityClassT>
  std::vector<UUID> find(const Strings &names) const {
    return find(EntityClassT::TYPE, names);
  }
  ...
  template <typename EntityClassT>
  UUID getID(const String &name) const {
    return getID(EntityClassT::TYPE, name);
  }

  std::vector<UUID> getIDs(AccessEntityType type, const Strings &names) const;

  template <typename EntityClassT>
  std::vector<UUID> getIDs(const Strings &names) const {
    return getIDs(EntityClassT::TYPE, names);
  }
  ...
  std::vector<UUID> insert(const std::vector<AccessEntityPtr> &multiple_entities, bool replace_if_exists = false,
                           bool throw_if_exists = true);
  std::vector<UUID> insert(const std::vector<AccessEntityPtr> &multiple_entities, const std::vector<UUID> &ids,
                           bool replace_if_exists = false, bool throw_if_exists = true);
  ...
  AuthResult authenticate(const Credentials &credentials, const Poco::Net::IPAddress &address,
                          const ExternalAuthenticators &external_authenticators, const ClientInfo &client_info, bool allow_no_password,
                          bool allow_plaintext_password) const;
  std::optional<AuthResult> authenticate(const Credentials &credentials, const Poco::Net::IPAddress &address,
  ...
  [[noreturn]] static void throwIDCollisionCannotInsert(const UUID &id, AccessEntityType type, const String &name,
                                                        AccessEntityType existing_type, const String &existing_name,
  ...
  [[noreturn]] static void throwNameCollisionCannotRename(AccessEntityType type, const String &old_name, const String &new_name,
                                                          const String &storage_name);
 ...
 protected:
  ...
  virtual std::optional<AuthResult> authenticateImpl(const Credentials &credentials, const Poco::Net::IPAddress &address,
                                                     const ExternalAuthenticators &external_authenticators, const ClientInfo &client_info,
  ...
  static String formatEntityTypeWithName(AccessEntityType type, const String &name) {
...
std::shared_ptr<const EntityClassT> IAccessStorage::read(const UUID &id, bool throw_if_not_exists) const {
  ...
  if constexpr (std::is_same_v<EntityClassT, IAccessEntity>)
    ...
    if (auto ptr = typeid_cast<std::shared_ptr<const EntityClassT>>(entity)) return ptr;
...
std::shared_ptr<const EntityClassT> IAccessStorage::read(const String &name, bool throw_if_not_exists) const {
  if (auto id = find<EntityClassT>(name)) return read<EntityClassT>(*id, throw_if_not_exists);
...
std::vector<AccessEntityPtr> IAccessStorage::read(const std::vector<UUID> &ids, bool throw_if_not_exists) const {
  ...
  for (const auto &id : ids) result.push_back(read<EntityClassT>(id, throw_if_not_exists));
...
std::vector<std::pair<UUID, std::shared_ptr<const EntityClassT>>> IAccessStorage::readAllWithIDs() const {
  ...
  for (const auto &id : findAll<EntityClassT>()) {
    if (auto entity = tryRead<EntityClassT>(id)) entities.emplace_back(id, entity);
  }
  return entities;
}
