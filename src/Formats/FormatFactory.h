...
class FormatFactory final : private boost::noncopyable, public IHints<2> {
 public:
   ...
   */
  using FileSegmentationEngine =
      std::function<std::pair<bool, size_t>(ReadBuffer &buf, DB::Memory<Allocator<false>> &memory, size_t min_bytes, size_t max_rows)>;

  using FileSegmentationEngineCreator = std::function<FileSegmentationEngine(const FormatSettings &settings)>;
 ...
 private:
  ...
  using InputCreator = std::function<InputFormatPtr(ReadBuffer &buf, const Block &header, const RowInputFormatParams &params,
  ...
  using RandomAccessInputCreator = std::function<InputFormatPtr(
      ReadBuffer &buf, const Block &header, const FormatSettings &settings, const ReadSettings &read_settings, bool is_remote_fs,
  ...
  using OutputCreator = std::function<OutputFormatPtr(WriteBuffer &buf, const Block &sample, const FormatSettings &settings,
  ...
  InputFormatPtr getInputImpl(const String &name, ReadBuffer &buf, const Block &sample, const ContextPtr &context, UInt64 max_block_size,
                              const std::optional<RelativePathWithMetadata> &object_with_metadata,
                              const std::optional<FormatSettings> &format_settings, FormatParserSharedResourcesPtr parser_shared_resources,
                              FormatFilterInfoPtr format_filter_info, bool is_remote_fs, CompressionMethod compression,
                              bool need_only_count, const std::optional<UInt64> &max_block_size_bytes,
 ...
 public:
  ...
  InputFormatPtr getInput(const String &name, ReadBuffer &buf, const Block &sample, const ContextPtr &context, UInt64 max_block_size,
                          ...
                          CompressionMethod compression = CompressionMethod::None, bool need_only_count = false,
  ...
  InputFormatPtr getInputWithMetadata(const String &name, ReadBuffer &buf, const Block &sample, const ContextPtr &context,
  ...
  OutputFormatPtr getOutputFormatParallelIfPossible(const String &name, WriteBuffer &buf, const Block &sample, const ContextPtr &context,
                                                    const std::optional<FormatSettings> &format_settings = std::nullopt,
                                                    FormatFilterInfoPtr format_filter_info = nullptr) const;

  OutputFormatPtr getOutputFormat(const String &name, WriteBuffer &buf, const Block &sample, const ContextPtr &context,
  ...
  SchemaReaderPtr getSchemaReader(const String &name, ReadBuffer &buf, const ContextPtr &context, const RelativePathWithMetadata &metadata,
                                  const std::optional<FormatSettings> &format_settings = std::nullopt) const;

  SchemaReaderPtr getSchemaReader(const String &name, ReadBuffer &buf, const ContextPtr &context,
                                  const std::optional<FormatSettings> &format_settings = std::nullopt) const;

  ExternalSchemaReaderPtr getExternalSchemaReader(const String &name, const ContextPtr &context,
                                                  const std::optional<FormatSettings> &format_settings = std::nullopt) const;
  ...
  bool checkIfFormatSupportAppend(const String &name, const ContextPtr &context,
  ...
  bool checkIfFormatSupportsSubsetOfColumns(const String &name, const ContextPtr &context,
                                            const std::optional<FormatSettings> &format_settings_ = std::nullopt) const;

  void registerPrewhereSupportChecker(const String &name, PrewhereSupportChecker prewhere_support_checker);
  bool checkIfFormatSupportsPrewhere(const String &name, const ContextPtr &context,
                                     const std::optional<FormatSettings> &format_settings_ = std::nullopt) const;
  ...
  void registerAdditionalInfoForSchemaCacheGetter(const String &name,
                                                  AdditionalInfoForSchemaCacheGetter additional_info_for_schema_cache_getter);
  String getAdditionalInfoForSchemaCache(const String &name, const ContextPtr &context,
                                         const std::optional<FormatSettings> &format_settings_ = std::nullopt);
 ...
 private:
  ...
  std::unique_ptr<ReadBuffer> wrapReadBufferIfNeeded(ReadBuffer &buf, CompressionMethod compression, const Creators &creators,
                                                     const FormatSettings &format_settings, const Settings &settings, bool is_remote_fs,
