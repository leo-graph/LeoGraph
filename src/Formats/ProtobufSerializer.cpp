...
struct ColumnNameWithProtobufFieldNameComparator {
  static bool equals(char c1, char c2) { return convertChar(c1) == convertChar(c2); }
  ...
  static bool less(std::string_view s1, std::string_view s2) {
    return std::lexicographical_compare(s1.begin(), s1.end(), s2.begin(), s2.end(),
                                        [](char c1, char c2) { return convertChar(c1) < convertChar(c2); });
  }
...
bool isGoogleWrapperField(const FieldDescriptor &field_descriptor) {
  ...
  if (message_descriptor == nullptr) return false;
  return isGoogleWrapperMessage(*message_descriptor);
}

bool isGoogleWrapperField(const FieldDescriptor *field_descriptor) {
  if (field_descriptor == nullptr) return false;
...
bool shouldSkipZeroOrEmpty(const FieldDescriptor &field_descriptor, bool google_wrappers_special_treatment = false) {
  if (!field_descriptor.is_optional()) return false;
  if (field_descriptor.containing_type()->options().map_entry()) return false;
  if (google_wrappers_special_treatment && isGoogleWrapperField(field_descriptor)) return false;
...
bool shouldPackRepeated(const FieldDescriptor &field_descriptor) {
  if (!field_descriptor.is_repeated()) return false;
...
class ProtobufSerializerSingleValue : public ProtobufSerializer {
 protected:
  ProtobufSerializerSingleValue(std::string_view column_name_, const FieldDescriptor &field_descriptor_,
  ...
  void setColumns(const ColumnPtr *columns, [[maybe_unused]] size_t num_columns) override {
    if (num_columns != 1) wrongNumberOfColumns(num_columns, "1");
    column = columns[0];
  }
  ...
  void writeInt(NumberType value) {
    ...
    if (cast != 0 || !skip_zero_or_empty) writer->writeInt(field_tag, cast);
  }

  template <typename NumberType>
  void writeSInt(NumberType value) {
    auto cast = castNumber<Int64>(value);
    if (cast != 0 || !skip_zero_or_empty) writer->writeSInt(field_tag, cast);
  }

  template <typename NumberType>
  void writeUInt(NumberType value) {
    auto cast = castNumber<UInt64>(value);
    if (cast != 0 || !skip_zero_or_empty) writer->writeUInt(field_tag, cast);
  }

  template <typename FieldType, typename NumberType>
  void writeFixed(NumberType value) {
    auto cast = castNumber<FieldType>(value);
    if (cast != 0 || !skip_zero_or_empty) writer->writeFixed(field_tag, cast);
  }
  ...
  void writeStr(std::string_view str) {
    if (!str.empty() || !skip_zero_or_empty) writer->writeString(field_tag, str);
  }
  ...
  DestType castNumber(SrcType value) const {
    if constexpr (std::is_same_v<DestType, SrcType>) return value;
    DestType result;
    try {
  ...
  [[noreturn]] void incompatibleColumnType(std::string_view column_type) const {
    throw Exception(ErrorCodes::DATA_TYPE_INCOMPATIBLE_WITH_PROTOBUF_FIELD,
                    ...
                    quoteString(column_name), column_type, quoteString(field_descriptor.full_name()), field_descriptor.type_name());
  }
  ...
  [[noreturn]] void cannotConvertValue(std::string_view src_value, std::string_view src_type_name, std::string_view dest_type_name) const {
    throw Exception(ErrorCodes::PROTOBUF_BAD_CAST, "Could not convert value '{}' from type {} to type {} while {} field {} {} column {}",
                    String{src_value}, String{src_type_name}, String{dest_type_name}, (reader ? "reading" : "writing"),
                    quoteString(field_descriptor.name()), (reader ? "for inserting into" : "extracted from"), quoteString(column_name));
  }
...
class ProtobufSerializerNumber : public ProtobufSerializerSingleValue {
 public:
  ...
  ProtobufSerializerNumber(std::string_view column_name_, const FieldDescriptor &field_descriptor_,
                           const ProtobufReaderOrWriter &reader_or_writer_)
  ...
  void insertDefaults(size_t row_num) override {
    auto &column_vector = assert_cast<ColumnType &>(column->assumeMutableRef());
    if (row_num < column_vector.size()) return;
 ...
 private:
    ...
    switch (field_typeid) {
      ...
      case FieldTypeId::TYPE_BOOL: {
        ...
        read_function = [this]() -> NumberType {
          UInt64 u64 = readUInt();
          if (u64 < 2) return castNumber<NumberType>(u64);
      ...
      case FieldTypeId::TYPE_ENUM: {
        if (is_floating_point<NumberType>) incompatibleColumnType(TypeName<NumberType>);
  ...
  NumberType getDefaultNumber() {
    if (!default_number) default_number = default_function();
  ...
  void checkProtobufEnumValue(int value) const {
    ...
    if (!enum_value_descriptor) cannotConvertValue(toString(value), TypeName<NumberType>, field_descriptor.type_name());
  }
...
class ProtobufSerializerString : public ProtobufSerializerSingleValue {
 public:
  ...
  ProtobufSerializerString(std::string_view column_name_, const std::shared_ptr<const DataTypeFixedString> &fixed_string_data_type_,
  ...
  ProtobufSerializerString(std::string_view column_name_, const google::protobuf::FieldDescriptor &field_descriptor_,
                           const ProtobufReaderOrWriter &reader_or_writer_)
      : ProtobufSerializerSingleValue(column_name_, field_descriptor_, reader_or_writer_) {
  ...
  void readRow(size_t row_num) override {
    ...
    if constexpr (is_fixed_string) {
      ...
      if (row_num < old_size) {
        if (row_num != old_size - 1) throw Exception(ErrorCodes::LOGICAL_ERROR, "Cannot replace a string in the middle of ColumnString");
  ...
  void describeTree(WriteBuffer &out, size_t indent) const override {
    writeIndent(out, indent) << "ProtobufSerializerString<" << (is_fixed_string ? "fixed" : "") << ">: column " << quoteString(column_name)
                             << " -> field " << quoteString(field_descriptor.full_name()) << " (" << field_descriptor.type_name() << ")\n";
  }
 ...
 private:
  void setFunctions() {
    switch (field_typeid) {
      ...
      case FieldTypeId::TYPE_BOOL: {
        ...
        default_function = [this]() -> String { return field_descriptor.default_value_bool() ? "true" : "false"; };
        break;
      }
  ...
  const PaddedPODArray<UInt8> &getDefaultString() {
    if (!default_string) {
      ...
      if constexpr (is_fixed_string) SerializationFixedString::alignStringLength(n, arr, 0);
  ...
  int stringToProtobufEnumValue(std::string_view str) const {
    ...
    if (it == string_to_protobuf_enum_value_map.end()) cannotConvertValue(str, "String", field_descriptor.type_name());
    return it->second;
  }

  std::string_view protobufEnumValueToString(int value) const {
    const auto *enum_value_descriptor = field_descriptor.enum_type()->FindValueByNumber(value);
    if (!enum_value_descriptor) cannotConvertValue(toString(value), field_descriptor.type_name(), "String");
...
class ProtobufSerializerEnum : public ProtobufSerializerNumber<NumberType> {
 public:
  ...
  ProtobufSerializerEnum(std::string_view column_name_, const std::shared_ptr<const EnumDataType> &enum_data_type_,
 ...
 private:
  void setFunctions() {
    switch (this->field_typeid) {
      ...
      case FieldTypeId::TYPE_STRING:
      case FieldTypeId::TYPE_BYTES: {
        this->write_function = [this](NumberType value) { writeStr(enumDataTypeValueToString(value)); };
      ...
      case FieldTypeId::TYPE_ENUM: {
        ...
        this->default_function = [this]() -> NumberType {
          return protobufEnumValueToEnumDataTypeValue(this->field_descriptor.default_value_enum()->number());
        };
        break;
      }
  ...
  void prepareEnumMapping() {
    ...
    if (!not_found_by_name_values.empty()) {
      ...
      for (NumberType value : not_found_by_name_values) {
        if (enum_descriptor.FindValueByNumber(value)) add_to_mapping(value, value);
      }
    }

    size_t num_mapped_values =
        this->writer ? enum_data_type_value_to_protobuf_enum_value_map.size() : protobuf_enum_value_to_enum_data_type_value_map.size();

    if (!num_mapped_values && !enum_data_type->getValues().empty() && enum_descriptor.value_count()) {
      throw Exception(ErrorCodes::DATA_TYPE_INCOMPATIBLE_WITH_PROTOBUF_FIELD,
                      "Couldn't find mapping between data type {} and the enum {} in the protobuf schema", enum_data_type->getName(),
                      quoteString(enum_descriptor.full_name()));
    }
  }
  ...
  [[noreturn]] void cannotConvertValue(std::string_view src_value, std::string_view src_type_name, std::string_view dest_type_name) const {
    ProtobufSerializerSingleValue::cannotConvertValue(src_value, src_type_name, dest_type_name);
  }
...
class ProtobufSerializerDecimal : public ProtobufSerializerSingleValue {
 public:
  ...
  ProtobufSerializerDecimal(std::string_view column_name_, const DataTypeDecimalBase<DecimalType> &decimal_data_type_,
                            const FieldDescriptor &field_descriptor_, const ProtobufReaderOrWriter &reader_or_writer_)
      : ProtobufSerializerSingleValue(column_name_, field_descriptor_, reader_or_writer_),
  ...
  void insertDefaults(size_t row_num) override {
    auto &column_decimal = assert_cast<ColumnType &>(column->assumeMutableRef());
    if (row_num < column_decimal.size()) return;
 ...
 private:
      ...
      case FieldTypeId::TYPE_BOOL: {
        if (std::is_same_v<DecimalType, DateTime64>)
          ...
          write_function = [this](const DecimalType &decimal) {
            if (decimal.value == 0)
              writeInt(0);
            else if (DecimalComparison<DecimalType, int, EqualsOp>::compare(decimal, 1, scale, 0, /* check overflow */ true))
              writeInt(1);
            else {
          ...
          read_function = [this]() -> DecimalType {
            UInt64 u64 = readUInt();
            if (u64 < 2) return numberToDecimal(static_cast<UInt64>(u64 != 0));
            cannotConvertValue(toString(u64), field_descriptor.type_name(), TypeName<DecimalType>);
          };

          default_function = [this]() -> DecimalType { return numberToDecimal(static_cast<Int64>(field_descriptor.default_value_bool())); };
        }
        break;
      }
  ...
  DecimalType getDefaultDecimal() {
    if (!default_decimal) default_decimal = default_function();
...
class ProtobufSerializerDate : public ProtobufSerializerNumber<UInt16> {
 public:
  ProtobufSerializerDate(std::string_view column_name_, const FieldDescriptor &field_descriptor_,
                         const ProtobufReaderOrWriter &reader_or_writer_)
...
class ProtobufSerializerDate32 : public ProtobufSerializerNumber<Int32> {
 public:
  ProtobufSerializerDate32(std::string_view column_name_, const FieldDescriptor &field_descriptor_,
                           const ProtobufReaderOrWriter &reader_or_writer_)
...
class ProtobufSerializerIPv4 : public ProtobufSerializerNumber<IPv4> {
 public:
  ProtobufSerializerIPv4(std::string_view column_name_, const FieldDescriptor &field_descriptor_,
                         const ProtobufReaderOrWriter &reader_or_writer_)
...
class ProtobufSerializerDateTime : public ProtobufSerializerNumber<UInt32> {
 public:
  ProtobufSerializerDateTime(std::string_view column_name_, const DataTypeDateTime &type, const FieldDescriptor &field_descriptor_,
                             const ProtobufReaderOrWriter &reader_or_writer_)
      : ProtobufSerializerNumber<UInt32>(column_name_, field_descriptor_, reader_or_writer_), date_lut(type.getTimeZone()) {
    setFunctions();
  }
...
class ProtobufSerializerUUID : public ProtobufSerializerSingleValue {
 public:
  ProtobufSerializerUUID(std::string_view column_name_, const google::protobuf::FieldDescriptor &field_descriptor_,
                         const ProtobufReaderOrWriter &reader_or_writer_)
      : ProtobufSerializerSingleValue(column_name_, field_descriptor_, reader_or_writer_) {
    setFunctions();
  }
 ...
 private:
  void setFunctions() {
    if ((field_typeid != FieldTypeId::TYPE_STRING) && (field_typeid != FieldTypeId::TYPE_BYTES)) incompatibleColumnType("UUID");
...
class ProtobufSerializerIPv6 : public ProtobufSerializerSingleValue {
 public:
  ProtobufSerializerIPv6(std::string_view column_name_, const google::protobuf::FieldDescriptor &field_descriptor_,
                         const ProtobufReaderOrWriter &reader_or_writer_)
      : ProtobufSerializerSingleValue(column_name_, field_descriptor_, reader_or_writer_) {
    setFunctions();
  }
 ...
 private:
  void setFunctions() {
    if ((field_typeid != FieldTypeId::TYPE_STRING) && (field_typeid != FieldTypeId::TYPE_BYTES)) incompatibleColumnType(TypeName<IPv6>);
...
class ProtobufSerializerAggregateFunction : public ProtobufSerializerSingleValue {
 public:
  ProtobufSerializerAggregateFunction(std::string_view column_name_,
  ...
  void insertDefaults(size_t row_num) override {
    auto &column_af = assert_cast<ColumnAggregateFunction &>(column->assumeMutableRef());
    if (row_num < column_af.size()) return;
...
class ProtobufSerializerOneOf : public ProtobufSerializer {
 public:
  explicit ProtobufSerializerOneOf(std::unique_ptr<ProtobufSerializer> nested_serializer_, std::string_view oneof_column_name_,
                                   size_t presence_column_idx_, int field_tag_)
  ...
  void setColumns(const MutableColumnPtr *columns, size_t num_columns) override {
    ...
    for (size_t i : collections::range(num_columns)) cols.push_back(columns[i]->getPtr());
  ...
  void readRow(size_t row_num) override {
    if (presence_column) {
      if (row_num < presence_column->size())
        throw Exception(ErrorCodes::BAD_ARGUMENTS, "Invalid protobuf data: OneOf has more than one value to track via column `{}`",
                        oneof_column_name);
  ...
  void insertDefaults(size_t row_num) override {
    if (row_num >= presence_column->size()) presence_column->insert(0);
...
class ProtobufSerializerNullable : public ProtobufSerializer {
 public:
  ...
  void writeRow(size_t row_num) override {
    ...
    if (!null_map[row_num]) nested_serializer->writeRow(row_num);
  }
  ...
  void readRow(size_t row_num) override {
    ...
    if (row_num < old_size) {
      ...
      if (new_size != old_size + 1) throw Exception(ErrorCodes::LOGICAL_ERROR, "Size of ColumnNullable is unexpected");
      try {
  ...
  void insertDefaults(size_t row_num) override {
    auto &column_nullable = assert_cast<ColumnNullable &>(column->assumeMutableRef());
    if (row_num < column_nullable.size()) return;
...
class ProtobufSerializerLowCardinality : public ProtobufSerializer {
 public:
  ...
  void insertDefaults(size_t row_num) override {
    auto &column_lc = assert_cast<ColumnLowCardinality &>(column->assumeMutableRef());
    if (row_num < column_lc.size()) return;
...
class ProtobufSerializerArray : public ProtobufSerializer {
 public:
  ...
  void writeRow(size_t row_num) override {
    ...
    for (size_t i : collections::range(offsets[row_num - 1], offsets[row_num])) element_serializer->writeRow(i);
  }
  ...
  void readRow(size_t row_num) override {
    ...
    if (row_num + 1 < old_size) throw Exception(ErrorCodes::LOGICAL_ERROR, "Cannot replace an element in the middle of ColumnArray");
    ...
    try {
      ...
      if (data_size != old_data_size + 1) throw Exception(ErrorCodes::LOGICAL_ERROR, "Size of ColumnArray is unexpected");
      ...
      if (data_column->size() > old_data_size) data_column->assumeMutableRef().popBack(data_column->size() - old_data_size);
      if (offsets.size() > old_size) column_array.getOffsetsColumn().popBack(offsets.size() - old_size);
      throw;
    }
  }
  ...
  void insertDefaults(size_t row_num) override {
    auto &column_array = assert_cast<ColumnArray &>(column->assumeMutableRef());
    if (row_num < column_array.size()) return;
...
class ProtobufSerializerTupleAsArray : public ProtobufSerializer {
 public:
  ProtobufSerializerTupleAsArray(std::string_view column_name_, const std::shared_ptr<const DataTypeTuple> &tuple_data_type_,
  ...
  void writeRow(size_t row_num) override {
    for (size_t i : collections::range(tuple_size)) element_serializers[i]->writeRow(row_num);
  }
  ...
  void readRow(size_t row_num) override {
    ...
    if (row_num >= old_size) current_element_index = 0;

    insertDefaults(row_num);

    if (current_element_index >= tuple_size) {
      throw Exception(ErrorCodes::PROTOBUF_BAD_CAST,
                      "Column {}: More than {} elements was read from the field {} to fit in the data type {}", quoteString(column_name),
                      tuple_size, quoteString(field_descriptor.full_name()), tuple_data_type->getName());
    }
  ...
  void insertDefaults(size_t row_num) override {
    ...
    try {
      for (size_t i : collections::range(tuple_size)) element_serializers[i]->insertDefaults(row_num);
    } catch (...) {
      for (size_t i : collections::range(tuple_size)) {
        auto element_column = column_tuple.getColumnPtr(i)->assumeMutable();
        if (element_column->size() > old_size) element_column->popBack(element_column->size() - old_size);
      }
      throw;
    }
  }
  ...
  void describeTree(WriteBuffer &out, size_t indent) const override {
    writeIndent(out, indent) << "ProtobufSerializerTupleAsArray: column " << quoteString(column_name) << " (" << tuple_data_type->getName()
                             << ") -> field " << quoteString(field_descriptor.full_name()) << " (" << field_descriptor.type_name()
                             << ") ->\n";
    for (const auto &element_serializer : element_serializers) element_serializer->describeTree(out, indent + 1);
  }
...
class ProtobufSerializerMessage : public ProtobufSerializer {
 public:
  ...
  ProtobufSerializerMessage(std::vector<FieldDesc> &&field_descs_, const FieldDescriptor *parent_field_descriptor_,
                            bool with_length_delimiter_, bool google_wrappers_special_treatment_,
      ...
      : parent_field_descriptor(parent_field_descriptor_),
        ...
        should_skip_if_empty(parent_field_descriptor ? shouldSkipZeroOrEmpty(*parent_field_descriptor, google_wrappers_special_treatment_)
                                                     : false),
        reader(reader_or_writer_.reader),
        writer(reader_or_writer_.writer) {
    ...
    for (size_t i : collections::range(field_infos.size())) field_index_by_field_tag.emplace(field_infos[i].field_tag, i);
  }

  void setHasEnvelopeAsParent() { has_envelope_as_parent = true; }

  void setColumns(const ColumnPtr *columns_, size_t num_columns_) override {
    if (!num_columns_) wrongNumberOfColumns(num_columns_, ">0");
    ...
    if (reader || (google_wrappers_special_treatment && isGoogleWrapperField(parent_field_descriptor))) {
      ...
      for (size_t i : collections::range(num_columns_)) mutable_columns[i] = columns_[i]->assumeMutable();

      std::vector<UInt8> column_is_missing;
      column_is_missing.resize(num_columns_, true);
      for (const FieldInfo &info : field_infos)
        for (size_t i : info.column_indices) column_is_missing[i] = false;
  ...
  void setColumns(const MutableColumnPtr *columns_, size_t num_columns_) override {
    ...
    for (size_t i : collections::range(num_columns_)) cols.push_back(columns_[i]->getPtr());
    setColumns(cols.data(), cols.size());
  }
  ...
  void writeRow(size_t row_num) override {
    ...
    for (const FieldInfo &info : field_infos) {
      if (info.should_pack_repeated) writer->startRepeatedPack();
      info.field_serializer->writeRow(row_num);
      if (info.should_pack_repeated) writer->endRepeatedPack(info.field_tag, true);
    }
    ...
    if (!field_infos.empty()) {
      ...
      try {
        ...
        while (reader->readFieldNumber(field_tag)) {
          ...
          if (field_index == static_cast<size_t>(-1)) continue;
        ...
        for (auto &info : field_infos) {
          if (!info.field_descriptor) continue;
        ...
        for (auto &column : mutable_columns) {
          if (column->size() > old_size) column->popBack(column->size() - old_size);
        }
        throw;
      }
    }
  ...
  void describeTree(WriteBuffer &out, size_t indent) const override {
    ...
    for (const auto &field_info : field_infos) num_columns += field_info.column_indices.size();
 ...
 private:
  size_t findFieldIndexByFieldTag(int field_tag) {
    while (true) {
      if (field_tag == last_field_tag) return last_field_index;
      if (field_tag < last_field_tag) break;
      if (++last_field_index >= field_infos.size()) break;
  ...
  void addDefaultsToMissingColumns(size_t row_num) {
    if (has_missing_columns) missing_columns_filler->addDefaults(mutable_columns, row_num);
  }
  ...
  bool isNullableGoogleWrapper() { return isGoogleWrapperField(parent_field_descriptor) && mutable_columns[0].get()->isNullable(); }

  struct FieldInfo {
    FieldInfo(std::vector<size_t> &&column_indices_, const FieldDescriptor *field_descriptor_,
...
class ProtobufSerializerEnvelope : public ProtobufSerializer {
 public:
  ProtobufSerializerEnvelope(std::unique_ptr<ProtobufSerializer> &&serializer_, const ProtobufReaderOrWriter &reader_or_writer_)
  ...
  void setColumns(const ColumnPtr *columns_, size_t num_columns_) override { serializer->setColumns(columns_, num_columns_); }

  void setColumns(const MutableColumnPtr *columns_, size_t num_columns_) override { serializer->setColumns(columns_, num_columns_); }
  ...
  void finalizeWrite() override {
    ...
    if (!first_call_of_write_row) writer->endMessage(/*with_length_delimiter = */ true);
  }
  ...
  void readRow(size_t row_num) override {
    ...
    int field_tag;
    if (!reader->readFieldNumber(field_tag)) throw Exception(ErrorCodes::PROTOBUF_BAD_CAST, "Unexpected end of ProtobufList message");
...
class ProtobufSerializerTupleAsNestedMessage : public ProtobufSerializer {
 public:
  ...
  void setColumns(const ColumnPtr *columns, [[maybe_unused]] size_t num_columns) override {
    ...
    for (size_t i : collections::range(tuple_size)) element_columns.emplace_back(column_tuple.getColumnPtr(i));
...
class ProtobufSerializerFlattenedNestedAsArrayOfNestedMessages : public ProtobufSerializer {
 public:
  explicit ProtobufSerializerFlattenedNestedAsArrayOfNestedMessages(const std::vector<std::string_view> &column_names_,
    ...
    for (const auto &column_name : column_names_) column_names.emplace_back(column_name);
  }

  void setColumns(const ColumnPtr *columns, size_t num_columns) override {
    if (!num_columns) wrongNumberOfColumns(num_columns, ">0");
    data_columns.clear();
    ...
    for (size_t i : collections::range(num_columns)) {
      ...
      if (std::binary_search(offset_columns.begin(), offset_columns.end(), offset_column)) continue;
  ...
  void writeRow(size_t row_num) override {
    ...
    for (size_t i : collections::range(start_offset, end_offset)) message_serializer->writeRow(i);
  }
  ...
  void readRow(size_t row_num) override {
    ...
    try {
      ...
      for (auto &offset_column : offset_columns) {
        if (offset_column->size() > old_size) offset_column->assumeMutableRef().popBack(offset_column->size() - old_size);
      }
      throw;
    }
  }
...
class ProtobufSerializerBuilder {
 public:
  ...
  std::unique_ptr<ProtobufSerializer> buildMessageSerializer(const Strings &column_names, const DataTypes &data_types,
    ...
    auto message_serializer = buildMessageSerializerImpl(
        /* num_columns = */ column_names.size(), column_names.data(), data_types.data(), message_descriptor, with_length_delimiter,
        google_wrappers_special_treatment, oneof_presence,
        /* parent_field_descriptor = */ nullptr, used_column_indices,
...
#  endif
 ...
 private:
  ...
  static Strings getFieldNames(const MessageDescriptor &message_descriptor) {
    ...
    for (int i : collections::range(message_descriptor.field_count())) field_names.emplace_back(message_descriptor.field(i)->name());
    return field_names;
  }
  ...
  static bool columnNameStartsWithFieldName(std::string_view column_name, const FieldDescriptor &field_descriptor,
                                            std::string_view &suffix) {
    ...
    if (column_name.length() == matching_length) return true;
    if ((column_name.length() < matching_length + 2) || !field_descriptor.message_type()) return false;
    char first_char_after_matching = column_name[matching_length];
    if (!ColumnNameWithProtobufFieldNameComparator::equals(first_char_after_matching, '.')) return false;
  ...
  static bool findFieldsByColumnName(
      std::string_view column_name, const MessageDescriptor &message_descriptor,
    ...
    for (int i : collections::range(message_descriptor.field_count())) {
      ...
      if (columnNameEqualsToFieldName(column_name, field_descriptor)) {
        std::string_view suffix =
            google_wrappers_special_treatment && isGoogleWrapperField(field_descriptor) ? googleWrapperColumnName(field_descriptor) : "";
        out_field_descriptors_with_suffixes.emplace_back(&field_descriptor, suffix);
        break;
      }
    }

    if (!out_field_descriptors_with_suffixes.empty()) return true;  /// We have an exact match, no need to compare prefixes.
  ...
  static void transformColumnIndices(std::vector<size_t> &column_indices, const std::vector<size_t> &outer_indices) {
    for (size_t &idx : column_indices) idx = outer_indices[idx];
  }
  ...
  std::unique_ptr<ProtobufSerializer> buildMessageSerializerImpl(size_t num_columns, const String *column_names,
                                                                 const DataTypePtr *data_types, const MessageDescriptor &message_descriptor,
                                                                 bool with_length_delimiter, bool google_wrappers_special_treatment,
                                                                 bool oneof_presence, const FieldDescriptor *parent_field_descriptor,
                                                                 std::vector<size_t> &used_column_indices,
    ...
    for (size_t i = 0; i != num_columns; ++i) column_names_sv.emplace_back(column_names[i]);

    return buildMessageSerializerImpl(num_columns, column_names_sv.data(), data_types, message_descriptor, with_length_delimiter,
                                      google_wrappers_special_treatment, oneof_presence, parent_field_descriptor, used_column_indices,
                                      columns_are_reordered_outside, check_nested_while_filling_missing_columns);
  }

  std::unique_ptr<ProtobufSerializer> buildMessageSerializerImpl(size_t num_columns, const std::string_view *column_names,
                                                                 const DataTypePtr *data_types, const MessageDescriptor &message_descriptor,
                                                                 bool with_length_delimiter, bool google_wrappers_special_treatment,
                                                                 bool oneof_presence, const FieldDescriptor *parent_field_descriptor,
                                                                 std::vector<size_t> &used_column_indices,
    ...
    auto add_field_serializer = [&](std::string_view column_name_, std::vector<size_t> &&column_indices_,
                                    const FieldDescriptor &field_descriptor_, std::unique_ptr<ProtobufSerializer> field_serializer_) {
      ...
      if (it != field_descriptors_in_use.end()) {
        throw Exception(ErrorCodes::MULTIPLE_COLUMNS_SERIALIZED_TO_SAME_PROTOBUF_FIELD,
                        ...
                        backQuote(std::string_view{it->second}), backQuote(std::string_view{column_name_}),
      ...
      if (columns_are_reordered_outside && !oneof_presence) {
        for (auto &index : column_indices_to_pass_to_message_serializer) index = sequential_column_index++;
      }
    ...
    auto check_enum = [&throw_incompatible_oneof](const auto *data_type_enum, const OneofDescriptor *oneof_descriptor) {
      ...
      if (expected_size == oneof_descriptor->field_count() + 1) {
        ...
        for (const auto &elem : data_type_enum->getValues()) enum_values_sorted.insert(elem.second);
        ...
        if (oneof_values_sorted == enum_values_sorted) return;
      }

      throw_incompatible_oneof(oneof_descriptor->name());
    };

    auto check_int_type_suitable_for_oneof_presence = [&throw_incompatible_oneof](TypeIndex data_type_id, std::string_view oneof_name) {
      if (!leastDecimalPrecisionFor(data_type_id)) throw_incompatible_oneof(oneof_name);
    };

    auto maybe_add_oneof_wrapper = [&](std::unique_ptr<ProtobufSerializer> &serializer_ptr_ref, const OneofDescriptor *oneof_descriptor,
                                       int field_tag, size_t num_columns_, const std::string_view *column_names_,
                                       const DataTypePtr *data_types_, std::vector<size_t> &used_columns_for_field) {
        ...
        for (size_t idx : collections::range(num_columns_)) {
          ...
          if (ColumnNameWithProtobufFieldNameComparator::equals(name, expected_name)) {
            ...
            serializer_ptr_ref = std::make_unique<ProtobufSerializerOneOf>(std::move(serializer_ptr_ref), oneof_descriptor->name(),
                                                                           used_columns_for_field.size(), field_tag);
    ...
    for (size_t column_idx : collections::range(num_columns)) {
      if (used_column_indices_sorted.contains(column_idx)) continue;
      ...
      if ((field_descriptors_with_suffixes.size() == 1) && field_descriptors_with_suffixes[0].second.empty()) {
        ...
        if (field_serializer) {
          ...
          maybe_add_oneof_wrapper(field_serializer, field_descriptor.containing_oneof(), field_descriptor.number(), num_columns,
                                  column_names, data_types, idxs);
      ...
      for (const auto &[field_descriptor, suffix] : field_descriptors_with_suffixes) {
        ...
        for (size_t j : collections::range(column_idx + 1, num_columns)) {
          if (used_column_indices_sorted.contains(j)) continue;
          std::string_view other_suffix;
          if (!columnNameStartsWithFieldName(column_names[j], *field_descriptor, other_suffix)) continue;
        ...
        for (size_t j : nested_column_indices) nested_data_types.push_back(data_types[j]);
        ...
        auto attempt_build_serializer = [&](const DataTypes &passed_nested_data_types) {
          std::unique_ptr<ProtobufSerializer> message_serializer = buildMessageSerializerImpl(
              nested_column_names.size(), nested_column_names.data(), passed_nested_data_types.data(), *field_descriptor->message_type(),
              /* with_length_delimiter = */ false, google_wrappers_special_treatment, oneof_presence, field_descriptor,
          ...
          maybe_add_oneof_wrapper(message_serializer, field_descriptor->containing_oneof(), field_descriptor->number(), num_columns,
                                  column_names, data_types, nested_column_indices);
        ...
        auto attempt_unwrap_and_build_array_serializer = [&]() {
          ...
          if (auto serializer = attempt_build_serializer(unwrapped_nested_data_types)) {
            ...
            for (const size_t i : used_column_indices_in_nested) column_names_used.emplace_back(nested_column_names[i]);
        ...
        if (repeated_field_matching_nested_columns_are_all_arrays) {
          if (attempt_unwrap_and_build_array_serializer()) break;
        }
        ...
        try {
          ...
          if ((e.code() != ErrorCodes::PROTOBUF_FIELD_NOT_REPEATED) || !repeated_field_matching_nested_columns_have_some_arrays) throw;
        }
    ...
    if (field_descs.empty()) return nullptr;
    ...
    return std::make_unique<ProtobufSerializerMessage>(std::move(field_descs), parent_field_descriptor, with_length_delimiter,
                                                       google_wrappers_special_treatment, std::move(missing_columns_filler),
                                                       reader_or_writer);
  }

  /// Builds a serializer for one-to-one match:
  /// one column is serialized as one field in the protobuf message.
  std::unique_ptr<ProtobufSerializer> buildFieldSerializer(std::string_view column_name, const DataTypePtr &data_type,
                                                           const FieldDescriptor &field_descriptor, bool allow_repeat,
                                                           bool google_wrappers_special_treatment, bool oneof_presence) {
    auto serializer_ptr =
        buildFieldSerializerImpl(column_name, data_type, field_descriptor, allow_repeat, google_wrappers_special_treatment, oneof_presence);
    return serializer_ptr;
  }

  /// Builds a serializer for one-to-one match:
  /// one column is serialized as one field in the protobuf message.
  std::unique_ptr<ProtobufSerializer> buildFieldSerializerImpl(std::string_view column_name, const DataTypePtr &data_type,
                                                               const FieldDescriptor &field_descriptor, bool allow_repeat,
                                                               bool google_wrappers_special_treatment, bool oneof_presence) {
    auto data_type_id = data_type->getTypeId();
    switch (data_type_id) {
      case TypeIndex::UInt8:
        return std::make_unique<ProtobufSerializerNumber<UInt8>>(column_name, field_descriptor, reader_or_writer);
      case TypeIndex::UInt16:
        return std::make_unique<ProtobufSerializerNumber<UInt16>>(column_name, field_descriptor, reader_or_writer);
      case TypeIndex::UInt32:
        return std::make_unique<ProtobufSerializerNumber<UInt32>>(column_name, field_descriptor, reader_or_writer);
      case TypeIndex::UInt64:
        return std::make_unique<ProtobufSerializerNumber<UInt64>>(column_name, field_descriptor, reader_or_writer);
      case TypeIndex::UInt128:
        return std::make_unique<ProtobufSerializerNumber<UInt128>>(column_name, field_descriptor, reader_or_writer);
      case TypeIndex::UInt256:
        return std::make_unique<ProtobufSerializerNumber<UInt256>>(column_name, field_descriptor, reader_or_writer);
      case TypeIndex::Int8:
        return std::make_unique<ProtobufSerializerNumber<Int8>>(column_name, field_descriptor, reader_or_writer);
      case TypeIndex::Int16:
        return std::make_unique<ProtobufSerializerNumber<Int16>>(column_name, field_descriptor, reader_or_writer);
      case TypeIndex::Date32:
        return std::make_unique<ProtobufSerializerDate32>(column_name, field_descriptor, reader_or_writer);
      case TypeIndex::Int32:
        return std::make_unique<ProtobufSerializerNumber<Int32>>(column_name, field_descriptor, reader_or_writer);
      case TypeIndex::Int64:
        return std::make_unique<ProtobufSerializerNumber<Int64>>(column_name, field_descriptor, reader_or_writer);
      case TypeIndex::Int128:
        return std::make_unique<ProtobufSerializerNumber<Int128>>(column_name, field_descriptor, reader_or_writer);
      case TypeIndex::Int256:
        return std::make_unique<ProtobufSerializerNumber<Int256>>(column_name, field_descriptor, reader_or_writer);
      case TypeIndex::Float32:
        return std::make_unique<ProtobufSerializerNumber<Float32>>(column_name, field_descriptor, reader_or_writer);
      case TypeIndex::Float64:
        return std::make_unique<ProtobufSerializerNumber<Float64>>(column_name, field_descriptor, reader_or_writer);
      case TypeIndex::Date:
        return std::make_unique<ProtobufSerializerDate>(column_name, field_descriptor, reader_or_writer);
      case TypeIndex::DateTime:
        return std::make_unique<ProtobufSerializerDateTime>(column_name, assert_cast<const DataTypeDateTime &>(*data_type),
                                                            field_descriptor, reader_or_writer);
      case TypeIndex::DateTime64:
        return std::make_unique<ProtobufSerializerDateTime64>(column_name, assert_cast<const DataTypeDateTime64 &>(*data_type),
                                                              field_descriptor, reader_or_writer);
      case TypeIndex::String:
        return std::make_unique<ProtobufSerializerString<false>>(column_name, field_descriptor, reader_or_writer);
      case TypeIndex::FixedString:
        return std::make_unique<ProtobufSerializerString<true>>(
            column_name, typeid_cast<std::shared_ptr<const DataTypeFixedString>>(data_type), field_descriptor, reader_or_writer);
      case TypeIndex::Enum8:
        return std::make_unique<ProtobufSerializerEnum<Int8>>(column_name, typeid_cast<std::shared_ptr<const DataTypeEnum8>>(data_type),
                                                              field_descriptor, reader_or_writer);
      case TypeIndex::Enum16:
        return std::make_unique<ProtobufSerializerEnum<Int16>>(column_name, typeid_cast<std::shared_ptr<const DataTypeEnum16>>(data_type),
                                                               field_descriptor, reader_or_writer);
      case TypeIndex::Decimal32:
        return std::make_unique<ProtobufSerializerDecimal<Decimal32>>(
            column_name, assert_cast<const DataTypeDecimal<Decimal32> &>(*data_type), field_descriptor, reader_or_writer);
      case TypeIndex::Decimal64:
        return std::make_unique<ProtobufSerializerDecimal<Decimal64>>(
            column_name, assert_cast<const DataTypeDecimal<Decimal64> &>(*data_type), field_descriptor, reader_or_writer);
      case TypeIndex::Decimal128:
        return std::make_unique<ProtobufSerializerDecimal<Decimal128>>(
            column_name, assert_cast<const DataTypeDecimal<Decimal128> &>(*data_type), field_descriptor, reader_or_writer);
      case TypeIndex::Decimal256:
        return std::make_unique<ProtobufSerializerDecimal<Decimal256>>(
            column_name, assert_cast<const DataTypeDecimal<Decimal256> &>(*data_type), field_descriptor, reader_or_writer);
      case TypeIndex::UUID:
        return std::make_unique<ProtobufSerializerUUID>(column_name, field_descriptor, reader_or_writer);
      case TypeIndex::IPv4:
        return std::make_unique<ProtobufSerializerIPv4>(column_name, field_descriptor, reader_or_writer);
      case TypeIndex::IPv6:
        return std::make_unique<ProtobufSerializerIPv6>(column_name, field_descriptor, reader_or_writer);
      case TypeIndex::Interval:
        return std::make_unique<ProtobufSerializerInterval>(column_name, field_descriptor, reader_or_writer);
      case TypeIndex::AggregateFunction:
        return std::make_unique<ProtobufSerializerAggregateFunction>(
            column_name, typeid_cast<std::shared_ptr<const DataTypeAggregateFunction>>(data_type), field_descriptor, reader_or_writer);
      ...
      case TypeIndex::Nullable: {
        ...
        auto nested_serializer = buildFieldSerializer(column_name, nullable_data_type.getNestedType(), field_descriptor, allow_repeat,
                                                      google_wrappers_special_treatment, oneof_presence);
        if (!nested_serializer) return nullptr;
      ...
      case TypeIndex::LowCardinality: {
        ...
        auto nested_serializer = buildFieldSerializer(column_name, low_cardinality_data_type.getDictionaryType(), field_descriptor,
                                                      allow_repeat, google_wrappers_special_treatment, oneof_presence);
        if (!nested_serializer) return nullptr;
      ...
      case TypeIndex::Array: {
        ...
        if (!allow_repeat) {
          ...
          if (field_descriptor.message_type() && field_descriptor.message_type()->field_count() == 1) {
            ...
            std::vector<size_t> used_column_indices;
            auto message_serializer =
                buildMessageSerializerImpl(1, column_names.data(), data_types.data(), *field_descriptor.message_type(),
                                           /* with_length_delimiter = */ false, google_wrappers_special_treatment, oneof_presence,
                                           &field_descriptor, used_column_indices,
                                           /* columns_are_reordered_outside = */ false,
                                           /* check_nested_while_filling_missing_columns = */ false);

            if (!message_serializer) return nullptr;
        ...
        auto nested_serializer =
            buildFieldSerializer(column_name, array_data_type.getNestedType(), field_descriptor,
      ...
      case TypeIndex::Tuple: {
        ...
        if (const auto *message_type = field_descriptor.message_type()) {
          ...
          std::vector<size_t> used_column_indices;
          auto message_serializer =
              buildMessageSerializerImpl(size_of_tuple, element_names.data(), tuple_data_type.getElements().data(), *message_type,
                                         /* with_length_delimiter = */ false, google_wrappers_special_treatment, oneof_presence,
                                         &field_descriptor, used_column_indices,
                                         /* columns_are_reordered_outside = */ false,
                                         /* check_nested_while_filling_missing_columns = */ false);
        ...
        if (!allow_repeat && (size_of_tuple > 1)) throwFieldNotRepeated(field_descriptor, column_name);
        ...
        for (const auto &nested_data_type : tuple_data_type.getElements()) {
          auto nested_serializer = buildFieldSerializer(
              column_name, nested_data_type, field_descriptor,
              /* allow_repeat = */ false,  // We do our repeating now, so for nested type we forget about the repeating.
              google_wrappers_special_treatment, oneof_presence);

          if (!nested_serializer) break;
          nested_serializers.push_back(std::move(nested_serializer));
        }

        if (nested_serializers.size() != size_of_tuple) return nullptr;

        return std::make_unique<ProtobufSerializerTupleAsArray>(column_name, typeid_cast<std::shared_ptr<const DataTypeTuple>>(data_type),
                                                                field_descriptor, std::move(nested_serializers));
      }
...
std::optional<NameAndTypePair> getNameAndDataTypeFromField(const google::protobuf::FieldDescriptor *field_descriptor,
                                                           bool skip_unsupported_fields, bool allow_repeat);
...
std::optional<NameAndTypePair> getNameAndDataTypeFromFieldRecursive(
  ...
  if (allow_repeat && field_descriptor->is_map()) {
    ...
    auto name_and_type = getNameAndDataTypeFromFieldRecursive(field_descriptor, skip_unsupported_fields, false, unresolved_descriptors);
    if (!name_and_type) return std::nullopt;
  ...
  switch (field_descriptor->type()) {
    ...
    case FieldTypeId::TYPE_ENUM: {
      ...
      if (enum_descriptor->value_count() == 0) {
        if (skip_unsupported_fields) return std::nullopt;
        throw Exception(ErrorCodes::BAD_ARGUMENTS, "Empty enum field");
      }
      int max_abs = std::abs(enum_descriptor->value(0)->number());
      for (int i = 1; i != enum_descriptor->value_count(); ++i) max_abs = std::max(std::abs(enum_descriptor->value(i)->number()), max_abs);
      if (max_abs < 128) return NameAndTypePair{std::string(field_descriptor->name()), getEnumDataType<Int8>(enum_descriptor)};
      if (max_abs < 32768) return NameAndTypePair{std::string(field_descriptor->name()), getEnumDataType<Int16>(enum_descriptor)};
    ...
    case FieldTypeId::TYPE_MESSAGE: {
      ...
      if (message_descriptor->field_count() == 1) {
        ...
        if (auto p = unresolved_descriptors.emplace(nested_field_descriptor); !p.second) {
          if (skip_unsupported_fields) return std::nullopt;
          throw Exception(ErrorCodes::BAD_ARGUMENTS, "ClickHouse doesn't support type recursion ({})",
                          nested_field_descriptor->full_name());
        }

        auto nested_name_and_type =
            getNameAndDataTypeFromFieldRecursive(nested_field_descriptor, skip_unsupported_fields, true, unresolved_descriptors);
        if (!nested_name_and_type) return std::nullopt;
      ...
      for (int i = 0; i != message_descriptor->field_count(); ++i) {
        if (auto p = unresolved_descriptors.emplace(message_descriptor->field(i)); !p.second) {
          if (skip_unsupported_fields) continue;
          throw Exception(ErrorCodes::BAD_ARGUMENTS, "ClickHouse doesn't support type recursion ({})",
                          message_descriptor->field(i)->full_name());
        }
        auto nested_name_and_type =
            getNameAndDataTypeFromFieldRecursive(message_descriptor->field(i), skip_unsupported_fields, true, unresolved_descriptors);
        if (!nested_name_and_type) continue;
      ...
      if (nested_types.empty()) return std::nullopt;

      return NameAndTypePair{std::string(field_descriptor->name()),
...
std::optional<NameAndTypePair> getNameAndDataTypeFromField(const google::protobuf::FieldDescriptor *field_descriptor,
                                                           bool skip_unsupported_fields, bool allow_repeat = true) {
...
std::unique_ptr<ProtobufSerializer> ProtobufSerializer::create(const Strings &column_names, const DataTypes &data_types,
                                                               std::vector<size_t> &missing_column_indices,
                                                               const ProtobufSchemas::DescriptorHolder &descriptor,
                                                               bool with_length_delimiter, bool with_envelope, bool flatten_google_wrappers,
                                                               bool oneof_presence, ProtobufReader &reader) {
  return ProtobufSerializerBuilder(reader).buildMessageSerializer(column_names, data_types, missing_column_indices,
                                                                  *descriptor.message_descriptor, with_length_delimiter, with_envelope,
                                                                  flatten_google_wrappers, oneof_presence);
}

std::unique_ptr<ProtobufSerializer> ProtobufSerializer::create(const Strings &column_names, const DataTypes &data_types,
                                                               const ProtobufSchemas::DescriptorHolder &descriptor,
                                                               bool with_length_delimiter, bool with_envelope,
                                                               bool defaults_for_nullable_google_wrappers, ProtobufWriter &writer) {
  std::vector<size_t> missing_column_indices;
  return ProtobufSerializerBuilder(writer).buildMessageSerializer(column_names, data_types, missing_column_indices,
                                                                  *descriptor.message_descriptor, with_length_delimiter, with_envelope,
                                                                  defaults_for_nullable_google_wrappers, false);
}
...
NamesAndTypesList protobufSchemaToCHSchema(const google::protobuf::Descriptor *message_descriptor, bool skip_unsupported_fields,
  ...
  if (schema.empty())
    throw Exception(ErrorCodes::BAD_ARGUMENTS,
                    "Cannot convert Protobuf schema to ClickHouse table schema, all fields have unsupported types");
  if (!oneofs.empty()) schema.splice(schema.end(), oneofs);
  return schema;
}
}  // namespace DB
#endif
