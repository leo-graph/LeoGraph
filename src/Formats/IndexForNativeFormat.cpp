...
IndexOfBlockForNativeFormat IndexOfBlockForNativeFormat::extractIndexForColumns(const NameSet& required_columns) const {
  if (num_columns < required_columns.size()) throw Exception(ErrorCodes::INCORRECT_INDEX, "Index contain less than required columns");

  IndexOfBlockForNativeFormat res;
  for (size_t i = 0; i < num_columns; ++i) {
    const auto& column = columns[i];
    if (required_columns.contains(column.name)) res.columns.push_back(column);
  }
  ...
  if (res.columns.size() > required_columns.size()) throw Exception(ErrorCodes::INCORRECT_INDEX, "Index contain duplicate columns");
...
void IndexForNativeFormat::write(WriteBuffer& ostr) const {
  for (const auto& block : blocks) block.write(ostr);
}
...
IndexForNativeFormat IndexForNativeFormat::extractIndexForColumns(const NameSet& required_columns) const {
  ...
  for (const auto& block : blocks) res.blocks.emplace_back(block.extractIndexForColumns(required_columns));
  return res;
}
