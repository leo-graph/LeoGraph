...
enum WireType { VARINT = 0, BITS64 = 1, LENGTH_DELIMITED = 2, GROUP_START = 3, GROUP_END = 4, BITS32 = 5 };

UInt8 *writeFieldNumber(UInt32 field_number, WireType wire_type, UInt8 *ptr) { return writeVarint((field_number << 3) | wire_type, ptr); }

void writeFieldNumber(UInt32 field_number, WireType wire_type, PODArray<UInt8> &buf) { writeVarint((field_number << 3) | wire_type, buf); }
}  // namespace

ProtobufWriter::ProtobufWriter(WriteBuffer &out_) : out(out_) {}
...
void ProtobufWriter::endMessage(bool with_length_delimiter) {
  ...
  for (const auto &piece : pieces)
    if (piece.end > piece.start) out.write(reinterpret_cast<char *>(&buffer[piece.start]), piece.end - piece.start);
  buffer.clear();
  pieces.clear();
  num_bytes_skipped = 0;
...
void ProtobufWriter::writeInt(int field_number, Int64 value) { writeUInt(field_number, static_cast<UInt64>(value)); }

void ProtobufWriter::writeSInt(int field_number, Int64 value) { writeUInt(field_number, encodeZigZag(value)); }
