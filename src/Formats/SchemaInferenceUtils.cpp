 ...
 public:
  ...
  struct PathHash {
    size_t operator()(const std::vector<String> &path) const {
      ...
      for (const auto &part : path) hash.update(part);
      return hash.get64();
    }
  };
  ...
  bool equals(const IDataType &rhs) const override {
    ...
    if (rhs.getTypeId() != getTypeId()) return false;

    const auto &rhs_paths = assert_cast<const DataTypeJSONPaths &>(rhs).paths;
    if (paths.size() != rhs_paths.size()) return false;

    for (const auto &[path, type] : paths) {
      auto it = rhs_paths.find(path);
      if (it == rhs_paths.end() || !it->second->equals(*type)) return false;
    }
    ...
    for (const auto &[rhs_path, rhs_type] : rhs.paths) {
      ...
      if (!inserted) {
        ...
        if (!type->equals(*rhs_type)) {
          ...
          if (!type->equals(*rhs_type_copy)) return false;
        }
      }
    }
 ...
 private:
  struct PathNode {
    ...
    DataTypePtr getType(bool use_string_type_for_ambiguous_paths) const {
      if (nodes.empty()) return leaf_type;
      ...
      if (leaf_type && !isNothing(removeNullable(leaf_type)) && !nodes.empty()) {
        if (use_string_type_for_ambiguous_paths) return std::make_shared<DataTypeString>();
...
void updateTypeIndexes(DataTypes &data_types, TypeIndexesSet &type_indexes) {
  type_indexes.clear();
  for (const auto &type : data_types) type_indexes.insert(type->getTypeId());
}
...
void transformNothingSimpleTypes(DataTypes &data_types, TypeIndexesSet &type_indexes) {
  ...
  if (!type_indexes.contains(TypeIndex::Nothing) || type_indexes.size() <= 1) return;
  ...
  for (auto &type : data_types) {
    if (isNothing(type)) type = not_nothing_type;
  }
...
void transformIntegers(DataTypes &data_types, TypeIndexesSet &type_indexes, JSONInferenceInfo *json_info) {
  if (!type_indexes.contains(TypeIndex::Int64) || !type_indexes.contains(TypeIndex::UInt64)) return;
  ...
  for (auto &type : data_types) {
    if (WhichDataType(type).isInt64()) {
      ...
      if (!is_negative) type = std::make_shared<DataTypeUInt64>();
    }
  }

  if (!have_negative_integers) type_indexes.erase(TypeIndex::Int64);
}
...
void transformIntegersAndFloatsToFloats(DataTypes &data_types, TypeIndexesSet &type_indexes, JSONInferenceInfo *json_info) {
  ...
  if (!have_integers || !have_floats) return;
...
void transformVariant(DataTypes &data_types, TypeIndexesSet &type_indexes) {
  if (checkIfTypesAreEqual(data_types)) return;
  ...
  for (auto &type : data_types) type = variant_type;
...
void transformDatesAndDateTimes(DataTypes &data_types, TypeIndexesSet &type_indexes) {
  ...
  bool all_dates_or_datetimes = (type_indexes.size() == (static_cast<size_t>(have_dates) + static_cast<size_t>(have_datetimes) +
                                                         static_cast<size_t>(have_datetimes64)));

  if (!all_dates_or_datetimes && (have_dates || have_datetimes || have_datetimes64)) {
    for (auto &type : data_types) {
      if (isDate(type) || isDateTime(type) || isDateTime64(type)) type = std::make_shared<DataTypeString>();
    }
...
void transformJSONNumbersBackToString(DataTypes &data_types, const FormatSettings &settings, TypeIndexesSet &type_indexes,
  ...
  if (!have_strings || !have_numbers) return;
...
void transformBoolsAndNumbersToNumbers(DataTypes &data_types, TypeIndexesSet &type_indexes) {
  ...
  if (!have_bools || (!have_signed_integers && !have_unsigned_integers && !have_floats)) return;
...
void transformJSONBoolsAndStringsToString(DataTypes &data_types, TypeIndexesSet &type_indexes) {
  if (!type_indexes.contains(TypeIndex::String) || !type_indexes.contains(TypeIndex::UInt8)) return;

  for (auto &type : data_types) {
    if (isBool(type)) type = std::make_shared<DataTypeString>();
  }
...
void transformNothingComplexTypes(DataTypes &data_types, TypeIndexesSet &type_indexes) {
  ...
  if (!have_nothing || !not_nothing_type) return;

  for (auto &type : data_types) {
    if (isNothing(removeNullable(type))) type = not_nothing_type;
  }
...
void transformNullableTypes(DataTypes &data_types, TypeIndexesSet &type_indexes, const FormatSettings &settings) {
  if (!type_indexes.contains(TypeIndex::Nullable)) return;

  for (auto &type : data_types) {
    if (canBeInsideNullableBySchemaSettings(type, settings)) type = makeNullable(type);
  }
...
void transformTuplesWithEqualNestedTypesToArrays(DataTypes &data_types, TypeIndexesSet &type_indexes) {
  if (!type_indexes.contains(TypeIndex::Tuple)) return;

  bool remove_tuple_index = true;
  for (auto &type : data_types) {
    if (isTuple(type)) {
      const auto *tuple_type = assert_cast<const DataTypeTuple *>(type.get());
      if (tuple_type->hasExplicitNames()) return;
  ...
  if (remove_tuple_index) type_indexes.erase(TypeIndex::Tuple);
}
...
void transformJSONTuplesAndArraysToArrays(DataTypes &data_types, const FormatSettings &settings, TypeIndexesSet &type_indexes,
                                          JSONInferenceInfo *json_info) {
  if (!type_indexes.contains(TypeIndex::Tuple)) return;
  ...
  if (!have_arrays && !tuple_sizes_are_equal) return;
  ...
  for (auto &type : data_types) {
    if (isArray(type))
      ...
      for (const auto &element : elements) nested_types.push_back(element);
    }
  }
  ...
  if (checkIfTypesAreEqual(nested_types)) {
    for (auto &type : data_types) {
      if (isArray(type) || isTuple(type)) type = std::make_shared<DataTypeArray>(nested_types.back());
    }
...
void transformUnnamedTuplesAndArraysToArrayOfDynamic(DataTypes &data_types, TypeIndexesSet &type_indexes) {
  ...
  for (const auto &type : data_types) {
    if (const auto *tuple_type = typeid_cast<const DataTypeTuple *>(type.get()); tuple_type && tuple_type->hasExplicitNames()) return;
  }

  /// Next, change all Arrays and Tuples to Array(Dynamic).
  for (auto &type : data_types) {
    if (isArray(type) || isTuple(type)) type = std::make_shared<DataTypeArray>(std::make_shared<DataTypeDynamic>());
  }
...
void transformMapsAndStringsToStrings(DataTypes &data_types, TypeIndexesSet &type_indexes) {
  ...
  if (!type_indexes.contains(TypeIndex::Map) || !type_indexes.contains(TypeIndex::String)) return;

  for (auto &type : data_types) {
    if (isMap(type)) type = std::make_shared<DataTypeString>();
  }
...
void mergeJSONPaths(DataTypes &data_types, TypeIndexesSet &type_indexes, const FormatSettings &settings, JSONInferenceInfo *json_info) {
  if (!type_indexes.contains(TypeIndex::JSONPaths)) return;

  std::shared_ptr<DataTypeJSONPaths> merged_type = std::make_shared<DataTypeJSONPaths>();
  auto transform_func = [&](DataTypePtr &type1, DataTypePtr &type2) {
    transformInferredJSONTypesIfNeeded(type1, type2, settings, json_info);
  };
  for (auto &type : data_types) {
    if (const auto *json_type = typeid_cast<const DataTypeJSONPaths *>(type.get())) merged_type->merge(*json_type, transform_func);
  }

  for (auto &type : data_types) {
    if (type->getTypeId() == TypeIndex::JSONPaths) type = merged_type;
  }
}
...
void mergeNamedTuples(DataTypes &data_types, TypeIndexesSet &type_indexes, const FormatSettings &settings, JSONInferenceInfo *json_info) {
  ...
  for (auto &type : data_types) {
    ...
    if (tuple_type && tuple_type->hasExplicitNames()) {
      ...
      for (size_t i = 0; i != elements.size(); ++i) {
        if (!names_to_types.contains(names[i])) element_names.push_back(names[i]);
  ...
  for (const auto &name : element_names) {
    ...
    if (!checkIfTypesAreEqual(types)) return;
  ...
  for (auto &type : data_types) {
    const auto *tuple_type = typeid_cast<const DataTypeTuple *>(type.get());
    if (tuple_type && tuple_type->hasExplicitNames()) type = result_tuple;
  }
}
...
void transformInferredTypesIfNeededImpl(DataTypes &types, const FormatSettings &settings, JSONInferenceInfo *json_info) {
  auto transform_simple_types = [&](DataTypes &data_types, TypeIndexesSet &type_indexes) {
    ...
    if (settings.try_infer_dates || settings.try_infer_datetimes) transformDatesAndDateTimes(data_types, type_indexes);

    if constexpr (!is_json) {
      if (settings.try_infer_variant) transformVariant(data_types, type_indexes);
      return;
    }
    ...
    if (settings.json.read_bools_as_numbers) transformBoolsAndNumbersToNumbers(data_types, type_indexes);

    /// Convert Bool to String if needed.
    if (settings.json.read_bools_as_strings) transformJSONBoolsAndStringsToString(data_types, type_indexes);

    if (settings.json.try_infer_objects_as_tuples) mergeJSONPaths(data_types, type_indexes, settings, json_info);
  ...
  auto transform_complex_types = [&](DataTypes &data_types, TypeIndexesSet &type_indexes) {
    ...
    if (settings.json.read_objects_as_strings) transformMapsAndStringsToStrings(data_types, type_indexes);

    if (json_info && json_info->allow_merging_named_tuples) mergeNamedTuples(data_types, type_indexes, settings, json_info);
...
template <bool is_json>
DataTypePtr tryInferDataTypeForSingleFieldImpl(ReadBuffer &buf, const FormatSettings &settings, JSONInferenceInfo *json_info,
                                               size_t depth = 1);
...
bool tryInferDate(std::string_view field, DayNum &date) {
  ...
  if (field.size() < 8 || field.size() > 10) return false;
  ...
  if (std::all_of(field.begin(), field.end(), isNumericASCII)) return false;
...
bool fastCheckForInvalidDateTimeOrDateTime64(std::string_view field) {
  ...
  if (field.empty() || field.size() > 50) return true;

  /// Check that we have at least one digit, don't infer datetime form strings like "Apr"/"May"/etc.
  if (!std::any_of(field.begin(), field.end(), isNumericASCII)) return true;
  ...
  if (std::all_of(field.begin(), field.end(), isNumericASCII)) return true;
  ...
  if (tryReadFloatText(tmp_float, buf) && buf.eof()) return true;

  return false;
}

bool tryInferDateTime(std::string_view field, time_t &date_time, const FormatSettings &settings,
                      const DateLUTImpl &time_zone = DateLUT::instance(), const DateLUTImpl &utc_time_zone = DateLUT::instance("UTC")) {
  ReadBufferFromString buf(field);
  switch (settings.date_time_input_format) {
    case FormatSettings::DateTimeInputFormat::Basic:
      if (tryReadDateTimeText(date_time, buf, time_zone, /*allowed_date_delimiters=*/"-/:", /*allowed_time_delimiters=*/":",
...
bool tryInferDateTime64(std::string_view field, DateTime64 &date_time, const FormatSettings &settings,
                        const DateLUTImpl &time_zone = DateLUT::instance(), const DateLUTImpl &utc_time_zone = DateLUT::instance("UTC")) {
  ReadBufferFromString buf(field);
  switch (settings.date_time_input_format) {
    case FormatSettings::DateTimeInputFormat::Basic:
      return tryReadDateTime64Text(date_time, 9, buf, time_zone, /*allowed_date_delimiters=*/"-/:", /*allowed_time_delimiters=*/":",
                                   /*saturate_on_overflow=*/false) &&
             buf.eof();
    case FormatSettings::DateTimeInputFormat::BestEffort:
...
DataTypePtr tryInferDateTimeOrDateTime64(std::string_view field, const FormatSettings &settings) {
  if (fastCheckForInvalidDateTimeOrDateTime64(field)) return nullptr;

  if (!settings.try_infer_datetimes_only_datetime64) {
    time_t tmp;
    if (tryInferDateTime(field, tmp, settings)) return std::make_shared<DataTypeDateTime>();
  }

  DateTime64 tmp;
  if (tryInferDateTime64(field, tmp, settings)) return std::make_shared<DataTypeDateTime64>(9);
...
DataTypePtr tryInferArray(ReadBuffer &buf, const FormatSettings &settings, JSONInferenceInfo *json_info, size_t depth) {
  ...
  while (!buf.eof() && *buf.position() != ']') {
    if (!first) {
      ...
      if (!checkChar(',', buf)) return nullptr;
      skipWhitespaceIfAny(buf);
    } else
      first = false;
  ...
  if (have_invalid_nested_type) return nullptr;

  /// Empty array has type Array(Nothing)
  if (nested_types.empty()) return std::make_shared<DataTypeArray>(std::make_shared<DataTypeNothing>());

  if (checkIfTypesAreEqual(nested_types)) return std::make_shared<DataTypeArray>(std::move(nested_types.back()));
  ...
  if constexpr (is_json) {
    ...
    for (const auto &type : nested_types) {
      if (!checkIfTypeIsComplete(type)) return std::make_shared<DataTypeTuple>(nested_types);
    }
    ...
    if (checkIfTypesAreEqual(nested_types_copy)) return std::make_shared<DataTypeArray>(nested_types_copy.back());
    return std::make_shared<DataTypeTuple>(nested_types);
  } else {
    transformInferredTypesIfNeededImpl<is_json>(nested_types, settings);
    if (checkIfTypesAreEqual(nested_types)) return std::make_shared<DataTypeArray>(nested_types.back());
...
DataTypePtr tryInferTuple(ReadBuffer &buf, const FormatSettings &settings, JSONInferenceInfo *json_info, size_t depth) {
  ...
  /// Nested data is invalid.
  if (have_invalid_nested_type || nested_types.empty()) return nullptr;
...
bool tryReadFloat(Float64 &value, ReadBuffer &buf, const FormatSettings &settings, bool &has_fractional) {
  if (is_json || settings.try_infer_exponent_floats) return tryReadFloatTextExt(value, buf, has_fractional);
...
DataTypePtr tryInferNumber(ReadBuffer &buf, const FormatSettings &settings, JSONInferenceInfo *json_info) {
  ...
  if (settings.try_infer_integers) {
    ...
    if (auto * /*string_buf*/ _ = dynamic_cast<ReadBufferFromString *>(&buf)) {
      ...
      if (tryReadFloat<is_json>(tmp_float, buf, settings, has_fractional) && has_fractional) return std::make_shared<DataTypeFloat64>();

      Int64 tmp_int;
      buf.position() = number_start;
      if (tryReadIntText(tmp_int, buf)) {
        auto type = std::make_shared<DataTypeInt64>();
        if (json_info && tmp_int < 0) json_info->negative_integers.insert(type.get());
        return type;
      }
      ...
      UInt64 tmp_uint;
      buf.position() = number_start;
      if (tryReadIntText(tmp_uint, buf)) return std::make_shared<DataTypeUInt64>();
    ...
    /// In case of Int64 overflow we can try to infer UInt64.
    UInt64 tmp_uint;
    if (tryReadIntText(tmp_uint, peekable_buf)) return std::make_shared<DataTypeUInt64>();
...
DataTypePtr tryInferNumberFromStringImpl(std::string_view field, const FormatSettings &settings,
                                         JSONInferenceInfo *json_inference_info = nullptr) {
  ReadBufferFromString buf(field);
  ...
  if (settings.try_infer_integers) {
    ...
    if (tryReadIntText(tmp_int, buf) && buf.eof()) {
      auto type = std::make_shared<DataTypeInt64>();
      if (json_inference_info && tmp_int < 0) json_inference_info->negative_integers.insert(type.get());
      return type;
    }
    ...
    UInt64 tmp_uint;
    if (tryReadIntText(tmp_uint, buf) && buf.eof()) return std::make_shared<DataTypeUInt64>();
  }
  ...
  Float64 tmp;
  bool has_fractional;
  if (tryReadFloat<is_json>(tmp, buf, settings, has_fractional) && buf.eof()) return std::make_shared<DataTypeFloat64>();
...
DataTypePtr tryInferString(ReadBuffer &buf, const FormatSettings &settings, JSONInferenceInfo *json_info) {
  ...
  if constexpr (is_json) {
    if (json_info->is_object_key) return std::make_shared<DataTypeString>();
  }

  if (auto type = tryInferDateOrDateTimeFromString(field, settings)) return type;
...
bool tryReadJSONObject(ReadBuffer &buf, const FormatSettings &settings, DataTypeJSONPaths::Paths &paths, const std::vector<String> &path,
                       JSONInferenceInfo *json_info, size_t depth) {
  if (depth > settings.max_parser_depth)
    throw Exception(ErrorCodes::TOO_DEEP_RECURSION, "Maximum parse depth ({}) exceeded. Consider raising max_parser_depth setting.",
  ...
  while (!buf.eof() && *buf.position() != '}') {
    if (!first) {
      if (!checkChar(',', buf)) return false;
      skipWhitespaceIfAny(buf);
    } else
      first = false;

    String key;
    if (!tryReadJSONStringInto(key, buf, settings.json)) return false;
    ...
    if (!buf.eof() && *buf.position() == '{') {
      if (!tryReadJSONObject(buf, settings, paths, current_path, json_info, depth + 1)) return false;
    } else {
  ...
  if (first && !path.empty()) paths[path] = std::make_shared<DataTypeNothing>();
  return true;
}
...
DataTypePtr tryInferJSONPaths(ReadBuffer &buf, const FormatSettings &settings, JSONInferenceInfo *json_info, size_t depth) {
  ...
  if (!tryReadJSONObject(buf, settings, paths, {}, json_info, depth)) return nullptr;
...
DataTypePtr tryInferMapOrObject(ReadBuffer &buf, const FormatSettings &settings, JSONInferenceInfo *json_info, size_t depth) {
  ...
  if constexpr (is_json) {
    if (settings.json.read_objects_as_strings) return std::make_shared<DataTypeString>();

    transformInferredTypesIfNeededImpl<is_json>(value_types, settings, json_info);
    if (!checkIfTypesAreEqual(value_types)) return nullptr;

    return std::make_shared<DataTypeMap>(key_types.back(), value_types.back());
  }

  if (!checkIfTypesAreEqual(key_types)) transformInferredTypesIfNeededImpl<is_json>(key_types, settings);
  if (!checkIfTypesAreEqual(value_types)) transformInferredTypesIfNeededImpl<is_json>(value_types, settings);

  if (!checkIfTypesAreEqual(key_types) || !checkIfTypesAreEqual(value_types)) return nullptr;

  auto key_type = removeNullable(key_types.back());
  if (!DataTypeMap::isValidKeyType(key_type)) return nullptr;
...
DataTypePtr tryInferDataTypeForSingleFieldImpl(ReadBuffer &buf, const FormatSettings &settings, JSONInferenceInfo *json_info,
  ...
  /// Array [field1, field2, ...]
  if (*buf.position() == '[') return tryInferArray<is_json>(buf, settings, json_info, depth);

  /// Tuple (field1, field2, ...), if format is not JSON
  if constexpr (!is_json) {
    if (*buf.position() == '(') return tryInferTuple(buf, settings, json_info, depth);
  }

  /// Map/Object for JSON { key1 : value1, key2 : value2, ...}
  if (*buf.position() == '{') {
    if constexpr (is_json) {
      if (settings.json.try_infer_objects_as_tuples) return tryInferJSONPaths(buf, settings, json_info, depth);
    }
  ...
  /// String
  char quote = is_json ? '"' : '\'';
  if (*buf.position() == quote) return tryInferString<is_json>(buf, settings, json_info);

  /// Bool
  if (checkStringCaseInsensitive("true", buf) || checkStringCaseInsensitive("false", buf)) return DataTypeFactory::instance().get("Bool");

  /// Null or NaN
  if (checkCharCaseInsensitive('n', buf)) {
    if (checkStringCaseInsensitive("ull", buf)) {
      if (settings.schema_inference_make_columns_nullable == 0) return std::make_shared<DataTypeNothing>();
      return makeNullable(std::make_shared<DataTypeNothing>());
    }
    if (checkStringCaseInsensitive("an", buf)) return std::make_shared<DataTypeFloat64>();
  }
...
bool canBeInsideNullableBySchemaSettings(const DataTypePtr &type, const FormatSettings &settings) {
  if (isTuple(type) && !settings.schema_inference_allow_nullable_tuple_type) return false;
...
bool checkIfTypesAreEqual(const DataTypes &types) {
  ...
  for (size_t i = 1; i < types.size(); ++i) {
    if (!types[0]->equals(*types[i])) return false;
  }
  return true;
}
...
void transformFinalInferredJSONTypeIfNeededImpl(DataTypePtr &data_type, const FormatSettings &settings, JSONInferenceInfo *json_info,
                                                bool remain_nothing_types = false) {
  if (!data_type) return;
  ...
  if (const auto *map_type = typeid_cast<const DataTypeMap *>(data_type.get())) {
    ...
    if (isNothing(key_type) && settings.json.infer_incomplete_types_as_strings) key_type = std::make_shared<DataTypeString>();
    ...
    /// If we couldn't infer common type for array elements, use Array(Dynamic) or keep it as unnamed Tuple.
      ...
      if (checkIfTypesAreEqual(nested_types_copy)) {
        ...
        if (!remain_nothing_types) {
          for (auto &nested_type : nested_types) transformFinalInferredJSONTypeIfNeededImpl(nested_type, settings, json_info);
        }
...
DataTypePtr tryInferDateOrDateTimeFromString(std::string_view field, const FormatSettings &settings) {
  if (settings.try_infer_dates) {
    DayNum tmp;
    if (tryInferDate(field, tmp)) return std::make_shared<DataTypeDate>();
  }

  if (settings.try_infer_datetimes) {
    if (auto type = tryInferDateTimeOrDateTime64(field, settings)) return type;
  }

  return nullptr;
}

bool tryInferDateFromString(std::string_view field, DayNum &date) { return tryInferDate(field, date); }
...
bool tryInferDateTimeFromString(std::string_view field, time_t &date_time, const FormatSettings &settings, const DateLUTImpl &time_zone,
  ...
  if (fastCheckForInvalidDateTimeOrDateTime64(field)) return false;
...
static DataTypePtr adjustNullableRecursively(DataTypePtr type, bool make_nullable, const FormatSettings &settings) {
  ...
  if (which.isNullable()) return make_nullable ? type : removeNullable(type);

  /// Leave named compound types unchanged.
  /// E.g. don't turn `Point` into `Tuple(Nullable(Float64), Nullable(Float64))`.
  if (type->hasCustomName()) return makeNullableSafe(type);
  ...
  if (which.isObject() && !settings.schema_inference_make_json_columns_nullable) return type;
...
NamesAndTypesList getNamesAndRecursivelyNullableTypes(const Block &header, const FormatSettings &settings) {
  ...
  for (auto &[name, type] : header.getNamesAndTypesList()) result.emplace_back(name, makeNullableRecursively(type, settings));
  return result;
}
...
bool checkIfTypeIsComplete(const DataTypePtr &type) {
  ...
  if (which.isNothing()) return false;

  if (which.isNullable()) return checkIfTypeIsComplete(assert_cast<const DataTypeNullable *>(type.get())->getNestedType());

  if (which.isArray()) return checkIfTypeIsComplete(assert_cast<const DataTypeArray *>(type.get())->getNestedType());

  if (which.isTuple()) {
    const auto *tuple_type = assert_cast<const DataTypeTuple *>(type.get());
    for (const auto &element : tuple_type->getElements()) {
      if (!checkIfTypeIsComplete(element)) return false;
    }
    return true;
  }
  ...
  if (which.isMap()) {
    const auto *map_type = assert_cast<const DataTypeMap *>(type.get());
    if (!checkIfTypeIsComplete(map_type->getKeyType())) return false;
