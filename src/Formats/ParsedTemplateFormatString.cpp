...
void ParsedTemplateFormatString::parse(const String& format_string, const ColumnIdxGetter& idx_by_name, bool allow_indexes) {
  enum ParserState { Delimiter, Column, Format };
    ...
    switch (state) {
      case Delimiter:
        if (*pos == '$') {
          ...
          if (*pos == '{') {
            ...
            throwInvalidFormat("At pos " + std::to_string(pos - format_string.c_str()) + ": Expected '{' or '$' after '$'" + ", got \"" +
                                   std::string(pos, std::min(end - pos, 16l)) + "\"",
                               columnsCount());
        }
        break;
      ...
      case Column:
        ...
        if (*pos == ':')
          ...
          throwInvalidFormat("At pos " + std::to_string(pos - format_string.c_str()) + ": Expected ':' or '}' after column name \"" +
                                 column_names.back() + "\"" + ", got \"" + std::string(pos, std::min(end - pos, 16l)) + "\"",
                             columnsCount());
  ...
  if (state != Delimiter) throwInvalidFormat("Unbalanced parentheses", columnsCount());
  delimiters.back().append(token_begin, pos - token_begin);
}

size_t ParsedTemplateFormatString::columnsCount() const { return format_idx_to_column_idx.size(); }
...
const char* ParsedTemplateFormatString::readMayBeQuotedColumnNameInto(const char* pos, size_t size, String& s) {
  ...
  if (*pos == '"')
    ...
    size_t name_size = 1;
    while (name_size < size && isWordCharASCII(*(pos + name_size))) ++name_size;
...
void ParsedTemplateFormatString::throwInvalidFormat(const String& message, size_t column) const {
  throw Exception(ErrorCodes::INVALID_TEMPLATE_FORMAT,
                  "Invalid format string for Template: {} (near column {}). "
