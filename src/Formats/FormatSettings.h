...
struct FormatSettings {
  ...
  enum class DateTimeInputFormat : uint8_t {
    Basic,       /// Default format for fast parsing: YYYY-MM-DD hh:mm:ss (ISO-8601 without fractional part and timezone) or NNNNNNNNNN unix
                 /// timestamp.
  ...
  enum class DateTimeOutputFormat : uint8_t { Simple, ISO, UnixTimestamp };

  enum class EscapingRule : uint8_t { None, Escaped, Quoted, CSV, JSON, XML, Raw };
  ...
  enum class IntervalOutputFormat : uint8_t { Kusto, Numeric };

  IntervalOutputFormat interval_output_format = IntervalOutputFormat::Numeric;

  enum class DateTimeOverflowBehavior : uint8_t { Ignore, Throw, Saturate };
  ...
  enum class ArrowCompression : uint8_t { NONE, LZ4_FRAME, ZSTD };
