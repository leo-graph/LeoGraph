...
#define DECLARE_FORMAT_EXTERN(TYPE, NAME, DEFAULT, DESCRIPTION, FLAGS, ...) extern Settings##TYPE NAME;
...
bool FormatFactory::exists(const String &name) const { return dict.contains(boost::to_lower_copy(name)); }

const FormatFactory::Creators &FormatFactory::getCreators(const String &name) const {
  auto it = dict.find(boost::to_lower_copy(name));
  if (dict.end() != it) return it->second;
...
FormatSettings getFormatSettings(const ContextPtr &context, const Settings &settings) {
  ...
  format_settings.date_time_64_output_format_cut_trailing_zeros_align_to_groups_of_thousands =
      settings[Setting::date_time_64_output_format_cut_trailing_zeros_align_to_groups_of_thousands];
  ...
  format_settings.json.use_string_type_for_ambiguous_paths_in_named_tuples_inference_from_objects =
      settings[Setting::input_format_json_use_string_type_for_ambiguous_paths_in_named_tuples_inference_from_objects];
  format_settings.json.defaults_for_missing_elements_in_named_tuple =
      settings[Setting::input_format_json_defaults_for_missing_elements_in_named_tuple];
  ...
  format_settings.json.compact_allow_variable_number_of_columns =
      settings[Setting::input_format_json_compact_allow_variable_number_of_columns];
  ...
  format_settings.json.max_dynamic_subcolumns_in_json_type_parsing =
      settings[Setting::max_dynamic_subcolumns_in_json_type_parsing].valueOrNullopt();
  format_settings.json.type_json_allow_duplicated_key_with_literal_and_nested_object =
      settings[Setting::type_json_allow_duplicated_key_with_literal_and_nested_object];
  format_settings.json.type_json_use_partial_match_to_skip_paths_by_regexp =
      settings[Setting::type_json_use_partial_match_to_skip_paths_by_regexp];
  format_settings.json.pretty_print = settings[Setting::output_format_json_pretty_print];
  format_settings.json.infer_array_of_dynamic_from_array_of_different_values =
      settings[Setting::input_format_json_infer_array_of_dynamic_from_array_of_different_types];
  ...
  format_settings.parquet.skip_columns_with_unsupported_types_in_schema_inference =
      settings[Setting::input_format_parquet_skip_columns_with_unsupported_types_in_schema_inference];
  ...
  format_settings.parquet.output_fixed_string_as_fixed_byte_array =
      settings[Setting::output_format_parquet_fixed_string_as_fixed_byte_array];
  ...
  format_settings.pretty.charset = settings[Setting::output_format_pretty_grid_charset].toString() == "ASCII"
  ...
  format_settings.pretty.max_column_name_width_min_chars_to_cut =
      settings[Setting::output_format_pretty_max_column_name_width_min_chars_to_cut];
  format_settings.pretty.max_value_width = settings[Setting::output_format_pretty_max_value_width];
  format_settings.pretty.max_value_width_apply_for_single_value =
      settings[Setting::output_format_pretty_max_value_width_apply_for_single_value];
  ...
  format_settings.pretty.display_footer_column_names_min_rows =
      settings[Setting::output_format_pretty_display_footer_column_names_min_rows];
  ...
  format_settings.pretty.fallback_to_vertical_max_rows_per_chunk =
      settings[Setting::output_format_pretty_fallback_to_vertical_max_rows_per_chunk];
  format_settings.pretty.fallback_to_vertical_min_table_width =
      settings[Setting::output_format_pretty_fallback_to_vertical_min_table_width];
  ...
  format_settings.protobuf.skip_fields_with_unsupported_types_in_schema_inference =
      settings[Setting::input_format_protobuf_skip_fields_with_unsupported_types_in_schema_inference];
  ...
  format_settings.schema.is_server =
      context->hasGlobalContext() && (context->getGlobalContext()->getApplicationType() == Context::ApplicationType::SERVER);
  ...
  format_settings.orc.skip_columns_with_unsupported_types_in_schema_inference =
      settings[Setting::input_format_orc_skip_columns_with_unsupported_types_in_schema_inference];
  ...
  format_settings.capn_proto.skip_fields_with_unsupported_types_in_schema_inference =
      settings[Setting::input_format_capn_proto_skip_fields_with_unsupported_types_in_schema_inference];
  ...
  format_settings.native.use_flattened_dynamic_and_json_serialization =
      settings[Setting::output_format_native_use_flattened_dynamic_and_json_serialization];
  ...
  if (format_settings.schema.is_server) {
    ...
    if (!avro_schema_registry_url.empty()) context->getRemoteHostFilter().checkURL(avro_schema_registry_url);
  }
...
InputFormatPtr FormatFactory::getInputImpl(const String &name, ReadBuffer &_buf, const Block &sample, const ContextPtr &context,
                                           UInt64 max_block_size, const std::optional<RelativePathWithMetadata> &object_with_metadata,
                                           const std::optional<FormatSettings> &_format_settings,
                                           FormatParserSharedResourcesPtr parser_shared_resources, FormatFilterInfoPtr format_filter_info,
                                           bool is_remote_fs, CompressionMethod compression, bool need_only_count,
  ...
  if (!parser_shared_resources)
    parser_shared_resources = std::make_shared<FormatParserSharedResources>(settings,
                                                                            /*num_streams_=*/1);

  if (format_settings.max_block_wait_ms > 0 && !format_settings.connection_handling)
    throw Exception(ErrorCodes::BAD_ARGUMENTS,
                    "Setting 'input_format_max_block_wait_ms' requires 'input_format_connection_handling' to be enabled");
  ...
  bool parallel_parsing = max_parsing_threads > 1 && settings[Setting::input_format_parallel_parsing] &&
                          ...
                          !(creators.random_access_input_creator || creators.random_access_input_creator_with_metadata) && !need_only_count;
  ...
  if (format_settings.connection_handling) parallel_parsing = false;
  ...
  if (parallel_parsing) {
    ...
    if (non_trivial_prefix_and_suffix_checker && non_trivial_prefix_and_suffix_checker(buf)) parallel_parsing = false;
  }
  ...
  if (parallel_parsing) {
    ...
    auto parser_creator = [input_getter, sample, row_input_format_params, format_settings](ReadBuffer &input) -> InputFormatPtr {
      return input_getter(input, sample, row_input_format_params, format_settings);
    };

    /// TODO: Try using parser_shared_resources->parsing_runner instead of creating a ThreadPool in
    ///       ParallelParsingInputFormat.
    ParallelParsingInputFormat::Params params{buf,
                                              sample,
                                              parser_creator,
                                              creators.file_segmentation_engine_creator,
                                              name,
  ...
  // 2. Prefer to use metadata-aware creator if we have object metadata
    ...
    format = creators.random_access_input_creator_with_metadata(buf, sample, format_settings, context->getReadSettings(), is_remote_fs,
  ...
  // 3. Use the normal random access creator for formats that need to jump around in the file
    ...
    format = creators.random_access_input_creator(buf, sample, format_settings, context->getReadSettings(), is_remote_fs,
  ...
  if (owned_buf) format->addBuffer(std::move(owned_buf));
  ...
  if (auto *values = typeid_cast<ValuesBlockInputFormat *>(format.get())) values->setContext(context);

  return format;
}

InputFormatPtr FormatFactory::getInput(const String &name, ReadBuffer &buf, const Block &sample, const ContextPtr &context,
                                       UInt64 max_block_size, const std::optional<FormatSettings> &format_settings,
                                       FormatParserSharedResourcesPtr parser_shared_resources, FormatFilterInfoPtr format_filter_info,
                                       bool is_remote_fs, CompressionMethod compression, bool need_only_count,
                                       const std::optional<UInt64> &max_block_size_bytes, const std::optional<UInt64> &min_block_size_rows,
                                       const std::optional<UInt64> &min_block_size_bytes) const {
  return getInputImpl(name, buf, sample, context, max_block_size, std::nullopt, format_settings, parser_shared_resources,
                      format_filter_info, is_remote_fs, compression, need_only_count, max_block_size_bytes, min_block_size_rows,
                      min_block_size_bytes);
}

// Overload with metadata
InputFormatPtr FormatFactory::getInputWithMetadata(
    const String &name, ReadBuffer &buf, const Block &sample, const ContextPtr &context, UInt64 max_block_size,
    const std::optional<RelativePathWithMetadata> &object_with_metadata, const std::optional<FormatSettings> &format_settings,
    FormatParserSharedResourcesPtr parser_shared_resources, FormatFilterInfoPtr format_filter_info, bool is_remote_fs,
    CompressionMethod compression, bool need_only_count, const std::optional<UInt64> &max_block_size_bytes,
    const std::optional<UInt64> &min_block_size_rows, const std::optional<UInt64> &min_block_size_bytes) const {
  chassert(object_with_metadata.has_value());
  return getInputImpl(name, buf, sample, context, max_block_size, object_with_metadata, format_settings, parser_shared_resources,
                      format_filter_info, is_remote_fs, compression, need_only_count, max_block_size_bytes, min_block_size_rows,
                      min_block_size_bytes);
}

std::unique_ptr<ReadBuffer> FormatFactory::wrapReadBufferIfNeeded(ReadBuffer &buf, CompressionMethod compression, const Creators &creators,
                                                                  const FormatSettings &format_settings, const Settings &settings,
                                                                  bool is_remote_fs,
  ...
  if (creators.random_access_input_creator) parallel_read &= compression != CompressionMethod::None;
  size_t file_size = 0;
  ...
  if (parallel_read) {
    try {
      ...
      parallel_read = false;
      LOG_TRACE(getLogger("FormatFactory"),
  ...
  if (parallel_read) {
    LOG_TRACE(getLogger("FormatFactory"), "Using ParallelReadBuffer with {} workers with chunks of {} bytes", max_download_threads,
    ...
    res = wrapInParallelReadBufferIfSupported(buf, threadPoolCallbackRunnerUnsafe<void>(getIOThreadPool().get(), ThreadName::PARALLEL_READ),
                                              max_download_threads, settings[Setting::max_download_buffer_size], file_size);
  }

  if (compression != CompressionMethod::None) {
    if (!res) res = wrapReadBufferReference(buf);
...
static void addExistingProgressToOutputFormat(OutputFormatPtr format, const ContextPtr &context) {
  ...
  if (element_id) {
    ...
    if (!read_progress.empty()) format->setProgress(std::move(read_progress));
...
OutputFormatPtr FormatFactory::getOutputFormatParallelIfPossible(const String &name, WriteBuffer &buf, const Block &sample,
                                                                 const ContextPtr &context,
                                                                 const std::optional<FormatSettings> &_format_settings,
                                                                 FormatFilterInfoPtr format_filter_info) const {
  const auto &output_getter = getCreators(name).output_creator;
  if (!output_getter) throw Exception(ErrorCodes::FORMAT_IS_NOT_SUITABLE_FOR_OUTPUT, "Format {} is not suitable for output", name);
  ...
  if (settings[Setting::output_format_parallel_formatting] && getCreators(name).supports_parallel_formatting &&
    ...
    ParallelFormattingOutputFormat::Params builder{buf, std::make_shared<const Block>(sample), formatter_creator,
...
OutputFormatPtr FormatFactory::getOutputFormat(const String &name, WriteBuffer &buf, const Block &sample, const ContextPtr &context,
                                               const std::optional<FormatSettings> &_format_settings,
                                               FormatFilterInfoPtr format_filter_info) const {
  const auto &output_getter = getCreators(name).output_creator;
  if (!output_getter) throw Exception(ErrorCodes::FORMAT_IS_NOT_SUITABLE_FOR_OUTPUT, "Format {} is not suitable for output", name);
  ...
  if (auto *mysql = typeid_cast<MySQLOutputFormat *>(format.get())) mysql->setContext(context);
...
SchemaReaderPtr FormatFactory::getSchemaReader(const String &name, ReadBuffer &buf, const ContextPtr &context,
  ...
  if (schema_reader->needContext()) schema_reader->setContext(context);
  return schema_reader;
}

ExternalSchemaReaderPtr FormatFactory::getExternalSchemaReader(const String &name, const ContextPtr &context,
                                                               const std::optional<FormatSettings> &_format_settings) const {
...
void FormatFactory::registerNonTrivialPrefixAndSuffixChecker(const String &name,
...
void FormatFactory::registerAppendSupportChecker(const String &name, AppendSupportChecker append_support_checker) {
  ...
  if (target) throw Exception(ErrorCodes::LOGICAL_ERROR, "FormatFactory: Suffix checker {} is already registered", name);
...
bool FormatFactory::checkIfFormatSupportAppend(const String &name, const ContextPtr &context,
...
void FormatFactory::registerOutputFormat(const String &name, OutputCreator output_creator) {
  ...
  if (target) throw Exception(ErrorCodes::LOGICAL_ERROR, "FormatFactory: Output format {} is already registered", name);
...
std::optional<String> FormatFactory::tryGetFormatFromFileName(String file_name) {
  if (file_name == "stdin") return tryGetFormatFromFileDescriptor(STDIN_FILENO);
  ...
  if (it == file_extension_formats.end()) return std::nullopt;

  return it->second;
}

String FormatFactory::getFormatFromFileName(String file_name) {
  if (auto format = tryGetFormatFromFileName(file_name)) return *format;
...
std::optional<String> FormatFactory::tryGetFormatFromFileDescriptor(int fd) {
  ...
  if (file_name) return tryGetFormatFromFileName(*file_name);

  return std::nullopt;
}

String FormatFactory::getFormatFromFileDescriptor(int fd) {
  if (auto format = tryGetFormatFromFileDescriptor(fd)) return *format;
...
void FormatFactory::registerFileSegmentationEngine(const String &name, FileSegmentationEngine file_segmentation_engine) {
  ...
  if (target) throw Exception(ErrorCodes::LOGICAL_ERROR, "FormatFactory: File segmentation engine {} is already registered", name);
  auto creator = [file_segmentation_engine](const FormatSettings &) { return file_segmentation_engine; };
  target = std::move(creator);
}

void FormatFactory::registerFileSegmentationEngineCreator(const String &name,
                                                          FileSegmentationEngineCreator file_segmentation_engine_creator) {
  auto &target = getOrCreateCreators(name).file_segmentation_engine_creator;
  if (target) throw Exception(ErrorCodes::LOGICAL_ERROR, "FormatFactory: File segmentation engine creator {} is already registered", name);
...
void FormatFactory::registerSchemaReader(const String &name, SchemaReaderCreator schema_reader_creator) {
  ...
  if (target) throw Exception(ErrorCodes::LOGICAL_ERROR, "FormatFactory: Schema reader {} is already registered", name);
...
void FormatFactory::registerSubsetOfColumnsSupportChecker(const String &name,
...
void FormatFactory::markOutputFormatPrefersLargeBlocks(const String &name) {
  ...
  if (target) throw Exception(ErrorCodes::LOGICAL_ERROR, "FormatFactory: Format {} is already marked as preferring large blocks", name);
  target = true;
}

void FormatFactory::markOutputFormatNotTTYFriendly(const String &name) {
  auto &target = getOrCreateCreators(name).is_tty_friendly;
  if (!target) throw Exception(ErrorCodes::LOGICAL_ERROR, "FormatFactory: Format {} is already marked as non-TTY-friendly", name);
  target = false;
}
...
bool FormatFactory::checkIfFormatSupportsSubsetOfColumns(const String &name, const ContextPtr &context,
                                                         const std::optional<FormatSettings> &format_settings_) const {
...
bool FormatFactory::checkIfFormatSupportsPrewhere(const String &name, const ContextPtr &context,
                                                  const std::optional<FormatSettings> &format_settings_) const {
  const auto &target = getCreators(name);
  auto format_settings = format_settings_ ? *format_settings_ : getFormatSettings(context);
...
void FormatFactory::registerAdditionalInfoForSchemaCacheGetter(const String &name,
...
String FormatFactory::getAdditionalInfoForSchemaCache(const String &name, const ContextPtr &context,
                                                      const std::optional<FormatSettings> &format_settings_) {
  const auto &additional_info_getter = getCreators(name).additional_info_for_schema_cache_getter;
  if (!additional_info_getter) return "";
...
bool FormatFactory::isInputFormat(const String &name) const {
  auto it = dict.find(boost::to_lower_copy(name));
  return it != dict.end() &&
         (it->second.input_creator || it->second.random_access_input_creator || it->second.random_access_input_creator_with_metadata);
}
...
bool FormatFactory::checkParallelizeOutputAfterReading(const String &name, const ContextPtr &context) const {
  ...
  if (format_name == "parquet" && context->getSettingsRef()[Setting::input_format_parquet_preserve_order]) return false;
...
void FormatFactory::checkFormatName(const String &name) const {
  auto it = dict.find(boost::to_lower_copy(name));
  if (it == dict.end()) throw Exception(ErrorCodes::UNKNOWN_FORMAT, "Unknown format {}", name);
}
...
std::vector<String> FormatFactory::getAllRegisteredNames() const { return KnownFormatNames::instance().getAllRegisteredNames(); }
}  // namespace DB
