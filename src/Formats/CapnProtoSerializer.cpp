...
std::optional<capnp::StructSchema::Field> findFieldByName(const capnp::StructSchema &struct_schema, const String &name) {
  ...
  for (auto field : fields) {
    ...
    if (boost::to_lower_copy(name) == boost::to_lower_copy(field_name)) return field;
  }
  return std::nullopt;
}

[[noreturn]] void throwCannotConvert(const DataTypePtr &type, const String &name, const capnp::Type &capnp_type) {
  throw Exception(ErrorCodes::CAPN_PROTO_BAD_CAST, "Cannot convert ClickHouse column \"{}\" with type {} to CapnProto type {}", name,
                  type->getName(), getCapnProtoFullTypeName(capnp_type));
}
...
struct ListBuilder : public FieldBuilder {
  explicit ListBuilder(capnp::DynamicValue::Builder builder, UInt32 elements_size)
      : impl(builder.as<capnp::DynamicList>()), nested_builders(elements_size) {}
...
struct StructBuilder : public FieldBuilder {
  explicit StructBuilder(capnp::DynamicStruct::Builder struct_builder, size_t fields_size)
      : impl(std::move(struct_builder)), field_builders(fields_size) {}
...
std::unique_ptr<StructBuilder> initStructBuilder(ParentBuilder &parent_builder, UInt32 offset_or_index,
                                                 const capnp::_::StructSize &struct_size, size_t elements,
  ...
  if constexpr (std::is_same_v<ParentBuilder, capnp::DynamicStruct::Builder>)
    builder_impl =
        capnp::DynamicStruct::Builder(schema, parent_builder.getBuilderImpl().getPointerField(offset_or_index).initStruct(struct_size));
  else
...
class ICapnProtoSerializer {
 public:
  ...
  virtual void writeRow(const ColumnPtr &column,
                        std::unique_ptr<FieldBuilder> &builder,  /// Maybe unused for simple types, needed to initialize structs and lists.
                        capnp::DynamicStruct::Builder &parent_struct_builder, UInt32 slot_offset, size_t row_num) = 0;

  /// Write row as list element.
  virtual void writeRow(const ColumnPtr &column,
                        std::unique_ptr<FieldBuilder> &builder,  /// Maybe unused for simple types, needed to initialize structs and lists.
                        capnp::DynamicList::Builder &parent_list_builder, UInt32 array_index, size_t row_num) = 0;
...
class CapnProtoIntegerSerializer : public ICapnProtoSerializer {
 public:
  void writeRow(const ColumnPtr &column, std::unique_ptr<FieldBuilder> &, capnp::DynamicStruct::Builder &parent_struct_builder,
                UInt32 slot_offset, size_t row_num) override {
    parent_struct_builder.getBuilderImpl().setDataField<CapnProtoNumericType>(slot_offset, getValue(column, row_num));
  }

  void writeRow(const ColumnPtr &column, std::unique_ptr<FieldBuilder> &, capnp::DynamicList::Builder &parent_list_builder,
                UInt32 array_index, size_t row_num) override {
...
std::unique_ptr<ICapnProtoSerializer> createIntegerSerializer(const DataTypePtr &data_type, const String &column_name,
...
std::unique_ptr<ICapnProtoSerializer> createFloatSerializer(const DataTypePtr &data_type, const String &column_name,
                                                            const capnp::Type &capnp_type) {
  switch (capnp_type.which()) {
...
class CapnProtoEnumSerializer : public ICapnProtoSerializer {
 public:
  CapnProtoEnumSerializer(const DataTypePtr &data_type_, const String &column_name, const capnp::Type &capnp_type,
                          const FormatSettings::CapnProtoEnumComparingMode enum_comparing_mode_)
      : data_type(data_type_), enum_comparing_mode(enum_comparing_mode_) {
    if (!capnp_type.isEnum()) throwCannotConvert(data_type, column_name, capnp_type);
    ...
    if (enum_comparing_mode == FormatSettings::CapnProtoEnumComparingMode::BY_VALUES) {
      ...
      for (auto enumerant : enumerants) capn_enum_values.insert(enumerant.getOrdinal());
      ...
      for (auto &[name, value] : all_values) ch_name_to_value[to_lower ? boost::algorithm::to_lower_copy(name) : name] = value;
 ...
 private:
  UInt16 getValue(const ColumnPtr &column, size_t row_num) {
    if (!capnp_enum_is_superset)
      throw Exception(ErrorCodes::CAPN_PROTO_BAD_CAST,
                      "Cannot convert ClickHouse enum to CapnProto enum: CapnProto enum values/names is not a superset of ClickHouse enum "
                      "values/names");

    EnumType enum_value = assert_cast<const ColumnVector<EnumType> &>(*column).getElement(row_num);
    if (enum_comparing_mode == FormatSettings::CapnProtoEnumComparingMode::BY_VALUES) return static_cast<UInt16>(enum_value);
    auto it = ch_to_capnp_values.find(enum_value);
    if (it == ch_to_capnp_values.end()) throw Exception(ErrorCodes::INCORRECT_DATA, "Unexpected value {} in ClickHouse enum", enum_value);
  ...
  void insertValue(IColumn &column, UInt16 capnp_enum_value) {
    if (!ch_enum_is_superset)
      throw Exception(ErrorCodes::CAPN_PROTO_BAD_CAST,
                      "Cannot convert CapnProto enum to ClickHouse enum: ClickHouse enum values/names is not a superset of CapnProto enum "
                      "values/names");
...
class CapnProtoDateSerializer : public ICapnProtoSerializer {
 public:
  CapnProtoDateSerializer(const DataTypePtr &data_type, const String &column_name, const capnp::Type &capnp_type) {
    if (!capnp_type.isUInt16()) throwCannotConvert(data_type, column_name, capnp_type);
  }
 ...
 private:
  UInt16 getValue(const ColumnPtr &column, size_t row_num) { return assert_cast<const ColumnDate &>(*column).getElement(row_num); }

  void insertValue(IColumn &column, UInt16 value) { assert_cast<ColumnDate &>(column).insertValue(value); }
};
...
class CapnProtoDate32Serializer : public ICapnProtoSerializer {
 public:
  CapnProtoDate32Serializer(const DataTypePtr &data_type, const String &column_name, const capnp::Type &capnp_type) {
    if (!capnp_type.isInt32()) throwCannotConvert(data_type, column_name, capnp_type);
  }
 ...
 private:
  Int32 getValue(const ColumnPtr &column, size_t row_num) { return assert_cast<const ColumnDate32 &>(*column).getElement(row_num); }

  void insertValue(IColumn &column, Int32 value) { assert_cast<ColumnDate32 &>(column).insertValue(value); }
};
...
class CapnProtoDateTimeSerializer : public ICapnProtoSerializer {
 public:
  CapnProtoDateTimeSerializer(const DataTypePtr &data_type, const String &column_name, const capnp::Type &capnp_type) {
    if (!capnp_type.isUInt32()) throwCannotConvert(data_type, column_name, capnp_type);
  }
 ...
 private:
  UInt32 getValue(const ColumnPtr &column, size_t row_num) { return assert_cast<const ColumnDateTime &>(*column).getElement(row_num); }

  void insertValue(IColumn &column, UInt32 value) { assert_cast<ColumnDateTime &>(column).insertValue(value); }
};
...
class CapnProtoDateTime64Serializer : public ICapnProtoSerializer {
 public:
  CapnProtoDateTime64Serializer(const DataTypePtr &data_type, const String &column_name, const capnp::Type &capnp_type) {
    if (!capnp_type.isInt64()) throwCannotConvert(data_type, column_name, capnp_type);
  }
 ...
 private:
  Int64 getValue(const ColumnPtr &column, size_t row_num) { return assert_cast<const ColumnDateTime64 &>(*column).getElement(row_num); }

  void insertValue(IColumn &column, Int64 value) { assert_cast<ColumnDateTime64 &>(column).insertValue(value); }
};
...
class CapnProtoDecimalSerializer : public ICapnProtoSerializer {
 ...
 private:
  ...
  void insertValue(IColumn &column, NativeType value) { assert_cast<ColumnDecimal<DecimalType> &>(column).insertValue(value); }
};
...
class CapnProtoIPv4Serializer : public ICapnProtoSerializer {
 ...
 private:
  UInt32 getValue(const ColumnPtr &column, size_t row_num) { return assert_cast<const ColumnIPv4 &>(*column).getElement(row_num); }

  void insertValue(IColumn &column, UInt32 value) { assert_cast<ColumnIPv4 &>(column).insertValue(IPv4(value)); }
};
...
class CapnProtoFixedSizeRawDataSerializer : public ICapnProtoSerializer {
 ...
 public:
  CapnProtoFixedSizeRawDataSerializer(const DataTypePtr &data_type_, const String &column_name, const capnp::Type &capnp_type)
      : data_type(data_type_) {
    if (!capnp_type.isData()) throwCannotConvert(data_type, column_name, capnp_type);
  }
  ...
  void writeRow(const ColumnPtr &column, std::unique_ptr<FieldBuilder> &, capnp::DynamicList::Builder &parent_struct_builder,
                UInt32 array_index, size_t row_num) override {
...
class CapnProtoStringSerializer : public ICapnProtoSerializer {
 public:
  CapnProtoStringSerializer(const DataTypePtr &data_type, const String &column_name, const capnp::Type &capnp_type) {
    if (!capnp_type.isData() && !capnp_type.isText()) throwCannotConvert(data_type, column_name, capnp_type);
  }
 ...
 private:
  ...
  void insertData(IColumn &column, Reader data) { column.insertData(reinterpret_cast<const char *>(data.begin()), data.size()); }
};
...
class CapnProtoFixedStringSerializer : public ICapnProtoSerializer {
 private:
 public:
  CapnProtoFixedStringSerializer(const DataTypePtr &data_type, const String &column_name, const capnp::Type &capnp_type_)
      : capnp_type(capnp_type_) {
    if (!capnp_type.isData() && !capnp_type.isText()) throwCannotConvert(data_type, column_name, capnp_type);
  }
 ...
 private:
  ...
  Reader getData(const ColumnPtr &column, size_t row_num) {
    ...
    if constexpr (std::is_same_v<CapnpType, capnp::Data>) {
      ...
      if (data.back() == 0) return Reader(data.data(), data.size());
  ...
  void insertData(IColumn &column, Reader data) {
    ...
    if (data.size() > fixed_string_column.getN())
      throw Exception(ErrorCodes::INCORRECT_DATA, "Cannot read data with size {} to FixedString with size {}", data.size(),
                      fixed_string_column.getN());
...
std::unique_ptr<ICapnProtoSerializer> createSerializer(const DataTypePtr &type, const String &name, const capnp::Type &capnp_type,
                                                       const FormatSettings::CapnProto &settings);

class CapnProtoLowCardinalitySerializer : public ICapnProtoSerializer {
 public:
  CapnProtoLowCardinalitySerializer(const DataTypePtr &data_type, const String &column_name, const capnp::Type &capnp_type,
                                    const FormatSettings::CapnProto &settings) {
    nested_serializer =
        createSerializer(assert_cast<const DataTypeLowCardinality &>(*data_type).getDictionaryType(), column_name, capnp_type, settings);
  }

  void writeRow(const ColumnPtr &column, std::unique_ptr<FieldBuilder> &field_builder, capnp::DynamicStruct::Builder &parent_struct_builder,
                UInt32 slot_offset, size_t row_num) override {
    writeRowImpl(column, field_builder, parent_struct_builder, slot_offset, row_num);
  }

  void writeRow(const ColumnPtr &column, std::unique_ptr<FieldBuilder> &field_builder, capnp::DynamicList::Builder &parent_list_builder,
                UInt32 array_index, size_t row_num) override {
 ...
 private:
  template <typename ParentBuilder>
  void writeRowImpl(const ColumnPtr &column, std::unique_ptr<FieldBuilder> &field_builder, ParentBuilder &parent_builder,
                    UInt32 offset_or_index, size_t row_num) {
...
class CapnProtoNullableSerializer : public ICapnProtoSerializer {
 public:
  CapnProtoNullableSerializer(const DataTypePtr &data_type, const String &column_name, const capnp::Type &capnp_type,
                              const FormatSettings::CapnProto &settings) {
    if (!capnp_type.isStruct())
      throw Exception(ErrorCodes::CAPN_PROTO_BAD_CAST,
                      "Cannot convert column \"{}\": Nullable can be represented only as a named union of type Void and nested type, got "
                      "CapnProto type {}",
                      column_name, getCapnProtoFullTypeName(capnp_type));
  ...
  void readRow(IColumn &column, const capnp::DynamicStruct::Reader &parent_struct_reader, UInt32 slot_offset) override {
    auto struct_reader =
        capnp::DynamicStruct::Reader(struct_schema, parent_struct_reader.getReaderImpl().getPointerField(slot_offset).getStruct(nullptr));
 ...
 private:
  ...
  void writeRowImpl(const ColumnPtr &column, std::unique_ptr<FieldBuilder> &field_builder, ParentBuilder &parent_builder,
                    UInt32 offset_or_index, size_t row_num) {
    if (!field_builder) field_builder = initStructBuilder(parent_builder, offset_or_index, struct_size, 1, struct_schema);
...
class CapnProtoArraySerializer : public ICapnProtoSerializer {
 public:
  CapnProtoArraySerializer(const DataTypePtr &data_type, const String &column_name, const capnp::Type &capnp_type,
                           const FormatSettings::CapnProto &settings) {
    if (!capnp_type.isList()) throwCannotConvert(data_type, column_name, capnp_type);
  ...
  void readRow(IColumn &column, const capnp::DynamicStruct::Reader &parent_struct_reader, UInt32 slot_offset) override {
    auto list_reader = capnp::DynamicList::Reader(
        list_schema, parent_struct_reader.getReaderImpl().getPointerField(slot_offset).getList(element_size, nullptr));
    readRowImpl(column, list_reader);
  }

  void readRow(IColumn &column, const capnp::DynamicList::Reader &parent_list_reader, UInt32 array_index) override {
    auto list_reader = capnp::DynamicList::Reader(
        list_schema, parent_list_reader.getReaderImpl().getPointerElement(array_index).getList(element_size, nullptr));
    readRowImpl(column, list_reader);
  }
 ...
 private:
  ...
  void writeRowImpl(const ColumnPtr &column, std::unique_ptr<FieldBuilder> &field_builder, ParentBuilder &parent_builder,
    ...
    if (!field_builder)
      field_builder = std::make_unique<ListBuilder>(
          capnp::DynamicList::Builder(list_schema, initListBuilder(parent_builder, offset_or_index, size)), size);
  ...
  void readRowImpl(IColumn &column, const capnp::DynamicList::Reader &list_reader) {
    ...
    auto read_array = [&]() {
      for (UInt32 i = 0; i != size; ++i) nested_serializer->readRow(nested_column, list_reader, i);
...
class CapnProtoMapSerializer : public ICapnProtoSerializer {
 public:
  CapnProtoMapSerializer(const DataTypePtr &data_type, const String &column_name, const capnp::Type &capnp_type,
                         const FormatSettings::CapnProto &settings) {
    ...
    if (!capnp_type.isStruct()) throwCannotConvert(data_type, column_name, capnp_type);
    ...
    if (checkIfStructContainsUnnamedUnion(struct_schema))
      throw Exception(ErrorCodes::CAPN_PROTO_BAD_CAST,
                      "Cannot convert ClickHouse column \"{}\" with type {} to CapnProto Struct with unnamed union {}", column_name,
                      data_type->getName(), getCapnProtoFullTypeName(capnp_type));
    ...
    if (!field_type.isList())
      throw Exception(ErrorCodes::CAPN_PROTO_BAD_CAST,
                      ...
                      column_name, getCapnProtoFullTypeName(field_type));
    ...
    if (!list_element_type.isStruct())
      throw Exception(
          ...
          column_name, getCapnProtoFullTypeName(list_element_type));

    auto key_value_struct = list_element_type.asStruct();
    if (checkIfStructContainsUnnamedUnion(key_value_struct))
      throw Exception(ErrorCodes::CAPN_PROTO_BAD_CAST,
                      "Cannot convert ClickHouse column \"{}\": struct that represents Map entries is unnamed union: {}", column_name,
...
class CapnProtoStructureSerializer : public ICapnProtoSerializer {
 public:
  CapnProtoStructureSerializer(const DataTypes &data_types, const Names &names, const capnp::StructSchema &schema,
                               const FormatSettings::CapnProto &settings)
  ...
  CapnProtoStructureSerializer(const DataTypePtr &data_type, const String &column_name, const capnp::Type &capnp_type,
                               const FormatSettings::CapnProto &settings) {
    if (!capnp_type.isStruct()) throwCannotConvert(data_type, column_name, capnp_type);
    ...
    if (checkIfStructIsNamedUnion(struct_schema) || checkIfStructContainsUnnamedUnion(struct_schema))
      throw Exception(ErrorCodes::CAPN_PROTO_BAD_CAST,
                      ...
                      column_name, data_type->getName(), getCapnProtoFullTypeName(capnp_type));
    ...
    if (!has_explicit_names) {
      if (nested_types.size() != structure_fields.size())
        throw Exception(
            ...
            column_name, data_type->getName(), getCapnProtoFullTypeName(capnp_type), nested_types.size(), structure_fields.size());
      nested_names.reserve(structure_fields.size());
      for (auto field : structure_fields) nested_names.push_back(field.getProto().getName());
    } else {
  ...
  void writeRow(const Columns &columns, StructBuilder &struct_builder, size_t row_num) {
    for (size_t i = 0; i != columns.size(); ++i)
      fields_serializers[i]->writeRow(columns[i], struct_builder.field_builders[fields_indexes[i]], struct_builder.impl, fields_offsets[i],
                                      row_num);
  }
  ...
  void readRow(MutableColumns &columns, const capnp::DynamicStruct::Reader &reader) {
    for (size_t i = 0; i != columns.size(); ++i) fields_serializers[i]->readRow(*columns[i], reader, fields_offsets[i]);
  }
 ...
 private:
  void initialize(const DataTypes &data_types, const Names &names, const FormatSettings::CapnProto &settings) {
    ...
    for (size_t i = 0; i != data_types.size(); ++i) {
      ...
      if (!field) throw Exception(ErrorCodes::THERE_IS_NO_COLUMN, "Capnproto schema doesn't contain field with name {}", field_name);
  ...
  void writeRowImpl(const ColumnPtr &column, std::unique_ptr<FieldBuilder> &field_builder, ParentBuilder &parent_builder,
                    UInt32 offset_or_index, size_t row_num) {
    if (!field_builder) field_builder = initStructBuilder(parent_builder, offset_or_index, struct_size, fields_count, struct_schema);

    auto &struct_builder = assert_cast<StructBuilder &>(*field_builder);
    if (const auto *tuple_column = typeid_cast<const ColumnTuple *>(column.get())) {
      const auto &columns = tuple_column->getColumns();
      for (size_t i = 0; i != columns.size(); ++i)
        fields_serializers[i]->writeRow(columns[i], struct_builder.field_builders[fields_indexes[i]], struct_builder.impl,
                                        fields_offsets[i], row_num);
    } else {
      fields_serializers[0]->writeRow(column, struct_builder.field_builders[fields_indexes[0]], struct_builder.impl, fields_offsets[0],
                                      row_num);
    }
  }
...
std::unique_ptr<ICapnProtoSerializer> createSerializer(const DataTypePtr &type, const String &name, const capnp::Type &capnp_type,
  ...
  if (!nested_name.empty() && !capnp_type.isList()) {
    if (!capnp_type.isStruct()) throw Exception(ErrorCodes::CAPN_PROTO_BAD_CAST, "Field {} is not a struct", field_name);
  ...
  switch (type->getTypeId()) {
    ...
    case TypeIndex::UInt8:
      if (isBool(type)) return createIntegerSerializer<UInt8, true>(type, name, capnp_type);
    ...
    case TypeIndex::String:
      if (capnp_type.isData()) return std::make_unique<CapnProtoStringSerializer<capnp::Data>>(type, name, capnp_type);
      return std::make_unique<CapnProtoStringSerializer<capnp::Text>>(type, name, capnp_type);
    case TypeIndex::FixedString:
      if (capnp_type.isData()) return std::make_unique<CapnProtoFixedStringSerializer<capnp::Data>>(type, name, capnp_type);
...
class CapnProtoSerializer::Impl {
 public:
  ...
  void readRow(MutableColumns &columns, capnp::DynamicStruct::Reader &reader) { struct_serializer->readRow(columns, reader); }
...
CapnProtoSerializer::CapnProtoSerializer(const DataTypes &data_types, const Names &names, const capnp::StructSchema &schema,
                                         const FormatSettings::CapnProto &settings)
