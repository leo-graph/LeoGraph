...
void writeReadableNumberTip(WriteBuffer &out, const IColumn &column, size_t row, const FormatSettings &settings, bool color,
  ...
  if (threshold && isFinite(value) &&
    ...
    if (tip_width <= max_width) {
      if (color) writeCString(GRAY_COLOR, out);
      writeCString(" -- ", out);
      writeString(output, out);
      if (color) writeCString(RESET_COLOR, out);
    }
  }
}
...
String highlightDigitGroups(String source) {
  ...
  if (!is_regular_number || num_digits_before_decimal <= 4) return source;
...
String highlightTrailingSpaces(String source) {
  ...
  if (last_significant) {
    ...
    if (highlight_start_pos >= source.size()) return source;
  }

  return source.substr(0, highlight_start_pos) + RED_COLOR + UNDERSCORE + source.substr(highlight_start_pos, std::string::npos) +
         RESET_COLOR;
}
...
std::pair<String, size_t> truncateName(String name, size_t cut_to, size_t hysteresis, bool ascii) {
  ...
  if (!cut_to || length <= cut_to + hysteresis || isValidIdentifier(name)) return {name, length};
  ...
  size_t suffix_bytes_begin =
      UTF8::computeBytesBeforeWidth(reinterpret_cast<const UInt8 *>(name.data()), name.size(), 0, suffix_chars_begin);
