...
class NativeWriter {
 public:
   ...
   */
  NativeWriter(WriteBuffer& ostr_, UInt64 client_revision_, SharedHeader header_,
               std::optional<FormatSettings> format_settings_ = std::nullopt, bool remove_low_cardinality_ = false,
  ...
  static std::tuple<SerializationPtr, SerializationInfoPtr, ColumnPtr> getSerializationAndColumn(UInt64 client_revision,
                                                                                                 const ColumnWithTypeAndName& column);

  static void writeData(const ISerialization& serialization, const ColumnPtr& column, WriteBuffer& ostr,
                        const std::optional<FormatSettings>& format_settings, UInt64 offset, UInt64 limit, UInt64 client_revision);
