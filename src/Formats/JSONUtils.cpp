...
static std::pair<bool, size_t> fileSegmentationEngineJSONEachRowImpl(ReadBuffer &in, DB::Memory<> &memory, size_t min_bytes,
  ...
  if (max_rows && (max_rows < min_rows)) max_rows = min_rows;
  ...
  while (loadAtPosition(in, memory, pos) && need_more_data) {
    ...
    if (min_bytes != 0 && current_object_size > 10 * min_bytes)
      throw Exception(
          ...
          in.count(), min_bytes, current_object_size);

    if (quotes) {
      pos = find_first_symbols<'\\', '"'>(pos, in.buffer().end());

      if (pos > in.buffer().end()) throw Exception(ErrorCodes::LOGICAL_ERROR, "Position in buffer is out of bounds. There must be a bug.");
      if (pos == in.buffer().end()) continue;
...
std::pair<bool, size_t> fileSegmentationEngineJSONCompactEachRow(ReadBuffer &in, DB::Memory<> &memory, size_t min_bytes, size_t min_rows,
                                                                 size_t max_rows) {
...
void skipRowForJSONEachRowImpl(ReadBuffer &in) {
  ...
  while (!in.eof()) {
    if (quotes) {
      ...
      if (in.position() == in.buffer().end()) continue;
...
void skipRowForJSONEachRow(ReadBuffer &in) { skipRowForJSONEachRowImpl<'{', '}'>(in); }

void skipRowForJSONCompactEachRow(ReadBuffer &in) { skipRowForJSONEachRowImpl<'[', ']'>(in); }

NamesAndTypesList readRowAndGetNamesAndDataTypesForJSONEachRow(ReadBuffer &in, const FormatSettings &settings,
  ...
  if (in.eof()) throw Exception(ErrorCodes::INCORRECT_DATA, "Unexpected EOF while reading JSON object");
...
DataTypes readRowAndGetDataTypesForJSONCompactEachRow(ReadBuffer &in, const FormatSettings &settings, JSONInferenceInfo *inference_info) {
  ...
  if (in.eof()) throw Exception(ErrorCodes::INCORRECT_DATA, "Unexpected EOF while reading JSON array");
...
bool readField(ReadBuffer &in, IColumn &column, const DataTypePtr &type, const SerializationPtr &serialization, const String &column_name,
               const FormatSettings &format_settings, bool yield_strings) {
  try {
    ...
    if (yield_strings) {
      ...
      if (as_nullable) return SerializationNullable::deserializeNullAsDefaultOrNestedWholeText(column, buf, format_settings, serialization);
    ...
    auto deserialize = [as_nullable, &format_settings, &serialization](IColumn &column_, ReadBuffer &buf) -> bool {
      if (as_nullable) return SerializationNullable::deserializeNullAsDefaultOrNestedTextJSON(column_, buf, format_settings, serialization);
...
void writeObjectStart(WriteBuffer &out, size_t indent, const char *title) {
  if (title) writeTitle(title, out, indent, "\n");
  writeChar('\t', indent, out);
  writeCString("{\n", out);
}
...
void writeCompactObjectEnd(WriteBuffer &out) { writeChar('}', out); }
...
void writeFieldFromColumn(const IColumn &column, const ISerialization &serialization, size_t row_num, bool yield_strings,
                          const FormatSettings &settings, WriteBuffer &out, const std::optional<String> &name, size_t indent,
                          const char *title_after_delimiter, bool pretty_json) {
  if (name.has_value()) {
    if (pretty_json) {
...
void writeColumns(const Columns &columns, const Names &names, const Serializations &serializations, size_t row_num, bool yield_strings,
                  const FormatSettings &settings, WriteBuffer &out, size_t indent) {
  for (size_t i = 0; i < columns.size(); ++i) {
    if (i != 0) writeFieldDelimiter(out);
    writeFieldFromColumn(*columns[i], *serializations[i], row_num, yield_strings, settings, out, names[i], indent);
  }
}

void writeCompactColumns(const Columns &columns, const Serializations &serializations, size_t row_num, bool yield_strings,
                         const FormatSettings &settings, WriteBuffer &out) {
  for (size_t i = 0; i < columns.size(); ++i) {
    if (i != 0) writeFieldCompactDelimiter(out);
...
void writeMetadata(const Names &names, const DataTypes &types, const FormatSettings &settings, WriteBuffer &out) {
  ...
  for (size_t i = 0; i < names.size(); ++i) {
    ...
    if (i + 1 < names.size()) writeFieldDelimiter(out);
  }

  writeArrayEnd(out, 1);
}

void writeAdditionalInfo(size_t rows, size_t rows_before_limit, bool applied_limit, size_t rows_before_aggregation,
                         bool applied_aggregation, const Stopwatch &watch, const Progress &progress, bool write_statistics,
                         WriteBuffer &out) {
...
bool tryReadStringField(ReadBuffer &in, String &value, const FormatSettings::JSON &settings) {
  skipWhitespaceIfAny(in);
  if (!tryReadJSONStringInto(value, in, settings)) return false;
  skipWhitespaceIfAny(in);
  return true;
}
...
NameAndTypePair readObjectWithNameAndType(ReadBuffer &in, const FormatSettings::JSON &settings) {
  ...
  if (first_field_name == "name" && second_field_name == "type")
    ...
    throw Exception(ErrorCodes::INCORRECT_DATA,
                    R"(Expected two fields "name" and "type" with column name and type, found fields "{}" and "{}")", first_field_name,
                    second_field_name);
  skipObjectEnd(in);
...
bool tryReadObjectWithNameAndType(ReadBuffer &in, NameAndTypePair &name_and_type, const FormatSettings::JSON &settings) {
  if (!checkAndSkipObjectStart(in)) return false;

  std::pair<String, String> first_field_and_value;
  if (!tryReadStringFieldNameAndValue(in, first_field_and_value, settings)) return false;

  if (!checkAndSkipComma(in)) return false;

  std::pair<String, String> second_field_and_value;
  if (!tryReadStringFieldNameAndValue(in, second_field_and_value, settings)) return false;
...
bool tryReadMetadata(ReadBuffer &in, NamesAndTypesList &names_and_types, const FormatSettings::JSON &settings) {
  String field_name;
  if (!tryReadFieldName(in, field_name, settings) || field_name != "meta") return false;

  if (!checkAndSkipArrayStart(in)) return false;
  ...
  while (!checkAndSkipArrayEnd(in)) {
    ...
    NameAndTypePair name_and_type;
    if (!tryReadObjectWithNameAndType(in, name_and_type, settings)) return false;
...
void validateMetadataByHeader(const NamesAndTypesList &names_and_types_from_metadata, const Block &header) {
  for (const auto &[name, type] : names_and_types_from_metadata) {
    if (!header.has(name)) continue;

    auto header_type = header.getByName(name).type;
    if (!type->equals(*header_type))
      throw Exception(ErrorCodes::INCORRECT_DATA, "Type {} of column '{}' from metadata is not the same as type in header {}",
...
bool skipUntilFieldInObject(ReadBuffer &in, const String &desired_field_name, const FormatSettings::JSON &settings) {
  while (!checkAndSkipObjectEnd(in)) {
    ...
    if (field_name == desired_field_name) return true;
  }
...
ReturnType deserializeEmpyStringAsDefaultOrNested(IColumn &column, ReadBuffer &istr,
                                                  const NestedDeserialize<ReturnType> &deserialize_nested) {
  ...
  if (istr.eof() || *istr.position() != EMPTY_STRING[0]) return deserialize_nested(column, istr);
  ...
  if (istr.available() >= EMPTY_STRING_LENGTH) {
    ...
    auto check_for_empty_string = [](ReadBuffer &buf) -> bool {
      ...
      if (checkString(EMPTY_STRING, buf)) return true;
      buf.position() = pos;
      return false;
    };
  ...
  auto check_for_empty_string = [](ReadBuffer &buf) -> bool {
    ...
    if (checkString(EMPTY_STRING, peekable_buf)) return true;
  ...
  auto deserialize_nested_with_check = [&deserialize_nested](IColumn &column_, ReadBuffer &buf) -> ReturnType {
    ...
    if (unlikely(peekable_buf.hasUnreadData()))
      throw Exception(ErrorCodes::LOGICAL_ERROR, "Incorrect state while parsing JSON: PeekableReadBuffer has unread data in own memory: {}",
                      String(peekable_buf.position(), peekable_buf.available()));
...
template void deserializeEmpyStringAsDefaultOrNested<void, true>(IColumn &column, ReadBuffer &istr,
                                                                 const NestedDeserialize<void> &deserialize_nested);
template bool deserializeEmpyStringAsDefaultOrNested<bool, true>(IColumn &column, ReadBuffer &istr,
                                                                 const NestedDeserialize<bool> &deserialize_nested);
template bool deserializeEmpyStringAsDefaultOrNested<bool, false>(IColumn &column, ReadBuffer &istr,
                                                                  const NestedDeserialize<bool> &deserialize_nested);
}  // namespace JSONUtils
