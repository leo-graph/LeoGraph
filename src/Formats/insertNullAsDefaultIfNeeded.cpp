...
bool insertNullAsDefaultIfNeeded(ColumnWithTypeAndName &input_column, const ColumnWithTypeAndName &header_column, size_t column_i,
  ...
  if (isArray(input_column.type) && isArray(header_column.type)) {
    ...
    if (!insertNullAsDefaultIfNeeded(nested_input_column, nested_header_column, 0, nullptr)) return false;
  ...
  if (isTuple(input_column.type) && isTuple(header_column.type)) {
    ...
    if (tuple_input_type.getElements().size() != tuple_header_type.getElements().size()) return false;
    ...
    input_column.type = has_explicit_names ? std::make_shared<DataTypeTuple>(std::move(nested_input_types), explicit_names)
  ...
  if (!isNullableOrLowCardinalityNullable(input_column.type) || isNullableOrLowCardinalityNullable(header_column.type)) return false;

  if (block_missing_values) {
    for (size_t i = 0; i < input_column.column->size(); ++i) {
      if (input_column.column->isNullAt(i)) block_missing_values->setBit(column_i, i);
    }
  }
