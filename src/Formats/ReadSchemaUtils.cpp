...
static std::optional<NamesAndTypesList> getOrderedColumnsList(const NamesAndTypesList &columns_list, const Names &columns_order_hint) {
  if (columns_list.size() != columns_order_hint.size()) return {};

  std::unordered_map<String, DataTypePtr> available_columns;
  for (const auto &[name, type] : columns_list) available_columns.emplace(name, type);

  NamesAndTypesList res;
  for (const auto &name : columns_order_hint) {
    auto it = available_columns.find(name);
    if (it == available_columns.end()) return {};
...
static const std::vector<String> &getFormatsOrderForDetection() {
  static const std::vector<String> formats_order = {
      "Parquet",     "ORC",    "Arrow", "ArrowStream",       "Avro",        "AvroConfluent",      "Npy",         "Native", "BSONEachRow",
      "JSONCompact", "Values", "TSKV",  "JSONObjectEachRow", "JSONColumns", "JSONCompactColumns", "JSONCompact", "JSON",
  };
...
std::pair<ColumnsDescription, String> readSchemaFromFormatImpl(std::optional<String> format_name,
                                                               const std::optional<FormatSettings> &format_settings,
                                                               IReadBufferIterator &read_buffer_iterator, const ContextPtr &context) try {
  ...
  if (format_name && mode == SchemaInferenceMode::UNION &&
      !format_factory.checkIfFormatSupportsSubsetOfColumns(*format_name, context, format_settings)) {
    ...
    if (format_name->ends_with("WithNames") || format_name->ends_with("WithNamesAndTypes"))
      additional_message =
          " (formats -WithNames(AndTypes) support reading subset of columns only when setting input_format_with_names_use_header is "
          "enabled)";

    throw Exception(ErrorCodes::BAD_ARGUMENTS,
                    "UNION schema inference mode is not supported for format {}, because it doesn't support reading subset of columns{}",
                    *format_name, additional_message);
  }
  ...
  if (format_name && format_factory.checkIfFormatHasExternalSchemaReader(*format_name)) {
    ...
    try {
      ...
      e.addMessage(fmt::format("The table structure cannot be extracted from a {} format file. You can specify the structure manually",
                               *format_name));
      throw;
    }
  }
  ...
  if (!format_name || format_factory.checkIfFormatHasSchemaReader(*format_name)) {
    ...
    while (true) {
      ...
      try {
        ...
        if (iterator_data.cached_columns) {
          ...
          if (!format_name) throw Exception(ErrorCodes::LOGICAL_ERROR, "Schema from cache was returned, but format name is unknown");

          if (mode == SchemaInferenceMode::DEFAULT) return {*iterator_data.cached_columns, *format_name};

          schemas_for_union_mode.emplace_back(iterator_data.cached_columns->getAll(), read_buffer_iterator.getLastFilePath());
          continue;
        }

        if (!iterator_data.buf) break;
        ...
        if (mode == SchemaInferenceMode::UNION) {
          if (!format_name)
            throw Exception(
                ...
                "The data format cannot be detected by the contents of the files: the file is empty. You can specify the format manually");

          throw Exception(ErrorCodes::CANNOT_EXTRACT_TABLE_STRUCTURE, "{}. You can specify the structure manually", exception_message);
        }

        if (!exception_messages.empty()) exception_messages += "\n";
      ...
      if (format_name) {
        if (!format_factory.checkIfFormatHasSchemaReader(*format_name)) {
          throw Exception(ErrorCodes::BAD_ARGUMENTS,
                          "{} file format doesn't support schema inference. You must specify the structure manually", *format_name);
        }
        ...
        try {
          ...
          if (num_rows) read_buffer_iterator.setNumRowsToLastFile(*num_rows);

          if (!names_and_types.empty()) read_buffer_iterator.setSchemaToLastFile(ColumnsDescription(names_and_types));

          /// In default mode, we finish when schema is inferred successfully from any file.
          if (mode == SchemaInferenceMode::DEFAULT) break;
        ...
        for (const auto &format_to_detect : getFormatsOrderForDetection()) {
          try {
            ...
            schema_reader->setMaxRowsAndBytesToRead(max_rows_to_read, max_bytes_to_read);
            names_and_types = schema_reader->readSchema();
            if (names_and_types.empty()) continue;
        ...
        if (!format_name) {
          ...
          for (size_t i = 0; i != formats_set_to_detect.size(); ++i) {
            try {
              ...
              if (!tmp_names_and_types.empty()) format_to_schema[formats_set_to_detect[i]] = tmp_names_and_types;
          ...
          if (format_name) read_buffer_iterator.setFormatName(*format_name);
        }

        if (format_name) read_buffer_iterator.setSchemaToLastFile(ColumnsDescription(names_and_types));
        ...
        if (mode == SchemaInferenceMode::UNION) {
          ...
          if (!format_name)
            throw Exception(ErrorCodes::CANNOT_DETECT_FORMAT,
                            "The data format cannot be detected by the contents of the files. You can specify the format manually");

          schemas_for_union_mode.emplace_back(names_and_types, read_buffer_iterator.getLastFilePath());
        }

        if (format_name && mode == SchemaInferenceMode::DEFAULT) break;
      }
    }
    ...
    if (!format_factory.checkIfFormatHasSchemaReader(*format_name)) {
      throw Exception(ErrorCodes::BAD_ARGUMENTS, "{} file format doesn't support schema inference. You must specify the structure manually",
                      *format_name);
    }
    ...
    if (mode == SchemaInferenceMode::UNION) {
            ...
            if (!type->equals(*it->second)) {
              ...
              if (!it->second->equals(*new_type_copy))
                throw Exception(ErrorCodes::TYPE_MISMATCH,
                                ...
                                type->getName(), name, file_name.empty() ? "" : " in file " + file_name, it->second->getName());
            }
          }
        }
      }
      ...
      for (const auto &name : names_order) names_and_types.emplace_back(name, names_to_types[name]);
    }
    ...
    if (names_and_types.empty()) {
      if (iterations <= 1) {
        throw Exception(ErrorCodes::CANNOT_EXTRACT_TABLE_STRUCTURE,
                        "The table structure cannot be extracted from a {} format file. "
                        "Error:\n{}.\nYou can specify the structure manually",
                        *format_name, exception_messages);
      }
    ...
    if (!stateless_schema_reader->hasStrictOrderOfColumns() && !insertion_table.empty()) {
      ...
      if (ordered_list) names_and_types = *ordered_list;
    }

    /// Some formats like CSVWithNames can contain empty column names. We don't support empty column names and further processing can fail
    /// with an exception. Let's just remove columns with empty names from the structure.
  ...
  if (!file_path.empty()) e.addMessage(fmt::format("(in file/uri {})", file_path));
  throw;
}

ColumnsDescription readSchemaFromFormat(const String &format_name, const std::optional<FormatSettings> &format_settings,
                                        IReadBufferIterator &read_buffer_iterator, const ContextPtr &context) {
  return readSchemaFromFormatImpl(format_name, format_settings, read_buffer_iterator, context).first;
}

std::pair<ColumnsDescription, String> detectFormatAndReadSchema(const std::optional<FormatSettings> &format_settings,
                                                                IReadBufferIterator &read_buffer_iterator, const ContextPtr &context) {
  return readSchemaFromFormatImpl(std::nullopt, format_settings, read_buffer_iterator, context);
}

SchemaCache::Key getKeyForSchemaCache(const String &source, const String &format, const std::optional<FormatSettings> &format_settings,
...
SchemaCache::Keys getKeysForSchemaCache(const Strings &sources, const String &format, const std::optional<FormatSettings> &format_settings,
                                        const ContextPtr &context) {
