...
FormatFilterInfo::FormatFilterInfo(std::shared_ptr<const ActionsDAG> filter_actions_dag_, const ContextPtr& context_,
                                   ColumnMapperPtr column_mapper_, FilterDAGInfoPtr row_level_filter_, PrewhereInfoPtr prewhere_info_)
...
bool FormatFilterInfo::hasFilter() const { return filter_actions_dag != nullptr; }
...
void FormatFilterInfo::initKeyCondition(const Block& keys) {
  ...
  if (prewhere_info || row_level_filter) {
    ...
    if (row_level_filter) add_columns(row_level_filter->actions);
    if (prewhere_info) add_columns(prewhere_info->prewhere_actions);
  }

  ColumnsWithTypeAndName columns = keys.getColumnsWithTypeAndName();
  for (const auto& col : additional_columns) columns.push_back(col);
  Names names;
  ...
  key_condition = std::make_shared<const KeyCondition>(inverted_dag, ctx, names, std::make_shared<ExpressionActions>(ActionsDAG(columns)));
}
