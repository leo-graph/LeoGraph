...
String getFormatSchemaDefaultFileExtension(const String &format) {
  ...
  if (format == "CapnProto") return "capnp";
  return "";
}
}  // namespace

FormatSchemaInfo::FormatSchemaInfo(const String &format_schema_source, const String &format_schema,
                                   const String &format_schema_message_name, const String &format, bool require_message, bool is_server,
                                   const String &format_schema_path, bool generated_)
    : message_name(format_schema_message_name), generated(generated_), log(getLogger("FormatSchemaInfo")) {
  if (format_schema_source.empty()) throw Exception(ErrorCodes::BAD_ARGUMENTS, "The format_schema_source setting should be set");
  ...
  throw Exception(ErrorCodes::BAD_ARGUMENTS, "Invalid format_schema_source setting. Possible value: '{}', '{}', '{}', actual '{}'",
                  FormatSettings::FORMAT_SCHEMA_SOURCE_FILE, FormatSettings::FORMAT_SCHEMA_SOURCE_STRING,
                  FormatSettings::FORMAT_SCHEMA_SOURCE_QUERY, format_schema_source);
}

FormatSchemaInfo::FormatSchemaInfo(const FormatSettings &settings, const String &format, bool require_message)
    : FormatSchemaInfo(settings.schema.format_schema_source, settings.schema.format_schema, settings.schema.format_schema_message_name,
                       format, require_message, settings.schema.is_server, settings.schema.format_schema_path) {}

void FormatSchemaInfo::handleSchemaFile(const String &format_schema, const String &format, bool require_message, bool is_server,
...
void FormatSchemaInfo::verifySchemaFileName(const String &format_schema, bool require_message, fs::path &path) {
  ...
  if (require_message) {
    ...
    if (message_name.empty()) message_name = format_schema_message_name;

    path = fs::path(format_schema_file_name);
    String filename = path.has_filename() ? path.filename() : path.parent_path().filename();
    if (filename.empty()) throw Exception(ErrorCodes::BAD_ARGUMENTS, "Invalid file name in 'format_schema' setting");
  } else {
    path = fs::path(format_schema_file_name);
...
void FormatSchemaInfo::handleSchemaSourceQuery(const String &format_schema, const String &format, bool is_server,
                                               const String &format_schema_path) {
  String default_file_extension = getFormatSchemaDefaultFileExtension(format);
...
String FormatSchemaInfo::querySchema(const String &query) {
  ...
  if (!current_query_context) current_query_context = Context::getGlobalContextInstance();
  ...
  while (executor.pull(block)) {
    if (result.has_value()) throw Exception(ErrorCodes::BAD_ARGUMENTS, "Expected the schema query result to have one row");
    if (block.columns() != 1) throw Exception(ErrorCodes::BAD_ARGUMENTS, "Expected the schema query result to have one column");
  ...
  if (!result.has_value()) throw Exception(ErrorCodes::BAD_ARGUMENTS, "Schema query result is empty");
...
void FormatSchemaInfo::storeSchemaOnDisk(const fs::path &file_path, const String &content) {
  ...
  try {
    ...
    if (!fs::exists(file_path)) throw;
  }
}

void FormatSchemaInfo::processSchemaFile(fs::path path, const String &default_file_extension, bool is_server,
                                         const String &format_schema_path) {
  ...
  if (path.is_absolute()) {
  ...
  } else if (path.has_parent_path() && !fs::weakly_canonical(default_schema_directory_path / path)
                                            .string()
    ...
    if (is_server)
      throw Exception(ErrorCodes::BAD_ARGUMENTS,
                      ...
                      format_schema_path, path.string(), format_schema_path);
...
MaybeAutogeneratedFormatSchemaInfo<SchemaGenerator>::MaybeAutogeneratedFormatSchemaInfo(const FormatSettings &settings,
                                                                                        const String &format, const Block &header,
                                                                                        bool use_autogenerated_schema, bool with_envelope) {
  ...
  if (!settings.schema.output_format_schema.empty()) {
    ...
    if (path.is_absolute()) {
      ...
      if (settings.schema.is_server)
        throw Exception(ErrorCodes::BAD_ARGUMENTS,
                        "Path in the 'format_schema' setting shouldn't go outside the 'format_schema_path' directory: {} ({} not in {})",
                        default_schema_directory_path.string(), path.string(), default_schema_directory_path.string());
      path = default_schema_directory_path / path;
    } else {
      path = default_schema_directory_path / path;
    }
    ...
    if (settings.schema.is_server) {
      tmp_file_path =
          Poco::TemporaryFile::tempName(default_schema_directory_path.string()) + '.' + getFormatSchemaDefaultFileExtension(format);
