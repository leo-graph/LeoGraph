...
NativeWriter::NativeWriter(WriteBuffer &ostr_, UInt64 client_revision_, SharedHeader header_,
                           std::optional<FormatSettings> format_settings_, bool remove_low_cardinality_, IndexForNativeFormat *index_,
...
void NativeWriter::flush() { ostr.next(); }

/*static*/ void NativeWriter::writeData(const ISerialization &serialization, const ColumnPtr &column, WriteBuffer &ostr,
                                        const std::optional<FormatSettings> &format_settings, UInt64 offset, UInt64 limit,
...
std::tuple<SerializationPtr, SerializationInfoPtr, ColumnPtr> NativeWriter::getSerializationAndColumn(UInt64 client_revision,
                                                                                                      const ColumnWithTypeAndName &column) {
  if (client_revision >= DBMS_MIN_REVISION_WITH_CUSTOM_SERIALIZATION) {
    ColumnPtr result_column = column.column;
    if (client_revision < DBMS_MIN_REVISION_WITH_REPLICATED_SERIALIZATION) result_column = result_column->convertToFullColumnIfReplicated();
    if (client_revision < DBMS_MIN_REVISION_WITH_SPARSE_SERIALIZATION) result_column = recursiveRemoveSparse(result_column);
    if (client_revision < DBMS_MIN_REVISION_WITH_NULLABLE_SPARSE_SERIALIZATION) {
      if (column.type->isNullable()) result_column = recursiveRemoveSparse(result_column);
    }
...
size_t NativeWriter::write(const Block &block) {
  ...
  if (client_revision > 0) block.info.write(ostr, client_revision);
  ...
  for (size_t i = 0; i < columns; ++i) {
    ...
    if (format_settings && format_settings->native.encode_types_in_binary_format) {
      ...
      if (client_revision < DBMS_MIN_REVISION_WITH_TIME_ZONE_PARAMETER_IN_DATETIME_DATA_TYPE && startsWith(type_name, "DateTime("))
        type_name = "DateTime";
    ...
    SerializationPtr serialization;
    {
      ...
      if (info) {
        ...
        if (info->hasCustomSerialization()) info->serialializeKindStackBinary(ostr);
      }
    }
  ...
  if (index) index->blocks.emplace_back(std::move(index_block));
