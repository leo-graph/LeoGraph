...
NativeReader::NativeReader(ReadBuffer &istr_, const Block &header_, UInt64 server_revision_, std::optional<FormatSettings> format_settings_,
...
NativeReader::NativeReader(ReadBuffer &istr_, UInt64 server_revision_, IndexForNativeFormat::Blocks::const_iterator index_block_it_,
                           IndexForNativeFormat::Blocks::const_iterator index_block_end_)
    : istr(istr_), server_revision(server_revision_), use_index(true), index_block_it(index_block_it_), index_block_end(index_block_end_) {
  ...
  if (index_block_it == index_block_end) return;
...
void NativeReader::readData(const ISerialization &serialization, ColumnPtr &column, ReadBuffer &istr, const FormatSettings *format_settings,
                            size_t rows, const NameAndTypePair *name_and_type, ValueSizeMap *avg_value_size_hints_) {
  ...
  if (column->size() != rows)
    throw Exception(ErrorCodes::CANNOT_READ_ALL_DATA, "Cannot read all data in NativeReader. Rows read: {}. Rows expected: {}",
                    column->size(), rows);
}

Block NativeReader::getHeader() const { return header; }
...
Block NativeReader::read() {
  ...
  if (use_index && index_block_it == index_block_end) return res;

  if (istr.eof()) {
    if (use_index) throw Exception(ErrorCodes::CANNOT_READ_ALL_DATA, "Input doesn't contain all data for index.");

    return res;
  }

  /// Additional information about the block.
  if (server_revision > 0) res.info.read(istr, server_revision);
  ...
  for (size_t i = 0; i < columns; ++i) {
    ...
    if (server_revision >= DBMS_MIN_REVISION_WITH_CUSTOM_SERIALIZATION) {
      ...
      if (has_custom) info->deserializeFromKindsBinary(istr);
    ...
    if (use_index) {
      ...
      if (index_column_it->name != column.name)
        throw Exception(ErrorCodes::INCORRECT_INDEX,
                        "Index points to a column with a wrong name ({} instead of {}): corrupted index or data", index_column_it->name,
                        column.name);
        ...
        if (!header_column.type->equals(*column.type)) {
          if (format_settings && format_settings->native.allow_types_conversion) {
            try {
              ...
              e.addMessage(fmt::format("while converting column \"{}\" from type {} to type {}", column.name, column.type->getName(),
    ...
    if (use_in_result) res.insert(std::move(column));

    if (use_index) ++index_column_it;
  }
  ...
  if (use_index) {
    ...
    ++index_block_it;
    if (index_block_it != index_block_end) index_column_it = index_block_it->columns.begin();
  }
  ...
  if (rows && !header.empty()) {
    ...
    for (size_t column_i = 0; column_i != header.columns(); ++column_i) {
      ...
      if (res.has(col.name)) {
        ...
        if (block_missing_values) block_missing_values->setBits(column_i, rows);
      }
    }
