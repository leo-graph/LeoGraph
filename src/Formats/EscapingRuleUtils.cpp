...
FormatSettings::EscapingRule stringToEscapingRule(const String& escaping_rule) {
  if (escaping_rule.empty()) return FormatSettings::EscapingRule::None;
  if (escaping_rule == "None") return FormatSettings::EscapingRule::None;
  if (escaping_rule == "Escaped") return FormatSettings::EscapingRule::Escaped;
  if (escaping_rule == "Quoted") return FormatSettings::EscapingRule::Quoted;
  if (escaping_rule == "CSV") return FormatSettings::EscapingRule::CSV;
  if (escaping_rule == "JSON") return FormatSettings::EscapingRule::JSON;
  if (escaping_rule == "XML") return FormatSettings::EscapingRule::XML;
  if (escaping_rule == "Raw") return FormatSettings::EscapingRule::Raw;
...
bool deserializeFieldByEscapingRule(const DataTypePtr& type, const SerializationPtr& serialization, IColumn& column, ReadBuffer& buf,
                                    FormatSettings::EscapingRule escaping_rule, const FormatSettings& format_settings) {
  bool read = true;
  ...
  switch (escaping_rule) {
    ...
    case FormatSettings::EscapingRule::Raw:
      ...
      break;
    default:
      throw Exception(ErrorCodes::BAD_ARGUMENTS, "Escaping rule {} is not suitable for deserialization",
                      escapingRuleToString(escaping_rule));
  }
  return read;
}

void serializeFieldByEscapingRule(const IColumn& column, const ISerialization& serialization, WriteBuffer& out, size_t row_num,
                                  FormatSettings::EscapingRule escaping_rule, const FormatSettings& format_settings) {
  switch (escaping_rule) {
    case FormatSettings::EscapingRule::Escaped:
...
void writeStringByEscapingRule(const String& value, WriteBuffer& out, FormatSettings::EscapingRule escaping_rule,
...
DataTypePtr tryInferDataTypeByEscapingRule(const String& field, const FormatSettings& format_settings,
                                           FormatSettings::EscapingRule escaping_rule, JSONInferenceInfo* json_info) {
  switch (escaping_rule) {
    case FormatSettings::EscapingRule::Quoted:
    ...
    case FormatSettings::EscapingRule::CSV: {
      if (!format_settings.csv.use_best_effort_in_schema_inference) return std::make_shared<DataTypeString>();

      if (field.empty()) return nullptr;

      if (field == format_settings.csv.null_representation) return makeNullable(std::make_shared<DataTypeNothing>());
      ...
      if (field.size() > 1 && ((field.front() == '\'' && field.back() == '\'') || (field.front() == '"' && field.back() == '"'))) {
        ...
        if (auto date_type = tryInferDateOrDateTimeFromString(data, format_settings)) return date_type;
        ...
        if (!type || (format_settings.csv.try_infer_strings_from_quoted_tuples && isTuple(type)) ||
            (!format_settings.csv.try_infer_numbers_from_strings && isNumber(type)) || isBool(type))
      ...
      if (auto number_type = tryInferNumberFromString(field, format_settings)) return number_type;

      if (auto date_type = tryInferDateOrDateTimeFromString(field, format_settings)) return date_type;
    ...
    case FormatSettings::EscapingRule::Raw:
      [[fallthrough]];
    case FormatSettings::EscapingRule::Escaped: {
      if (!format_settings.tsv.use_best_effort_in_schema_inference) return std::make_shared<DataTypeString>();

      if (field.empty()) return nullptr;

      if (field == format_settings.tsv.null_representation) return makeNullable(std::make_shared<DataTypeNothing>());
      ...
      if (field[0] == '0' && field.size() != 1) return std::make_shared<DataTypeString>();

      auto type = tryInferDataTypeForSingleField(field, format_settings);
      if (!type) return std::make_shared<DataTypeString>();
      return type;
    }
    default:
...
DataTypes tryInferDataTypesByEscapingRule(const std::vector<String>& fields, const FormatSettings& format_settings,
                                          FormatSettings::EscapingRule escaping_rule, JSONInferenceInfo* json_info) {
  DataTypes data_types;
  data_types.reserve(fields.size());
  for (const auto& field : fields) data_types.push_back(tryInferDataTypeByEscapingRule(field, format_settings, escaping_rule, json_info));
  return data_types;
}

void transformInferredTypesByEscapingRuleIfNeeded(DataTypePtr& first, DataTypePtr& second, const FormatSettings& settings,
                                                  FormatSettings::EscapingRule escaping_rule, JSONInferenceInfo* json_info) {
  switch (escaping_rule) {
    case FormatSettings::EscapingRule::JSON:
    ...
    case FormatSettings::EscapingRule::CSV:
      ...
      break;
    default:
      throw Exception(ErrorCodes::BAD_ARGUMENTS, "Cannot transform inferred types for value with {} escaping rule",
                      escapingRuleToString(escaping_rule));
  }
}
...
DataTypes getDefaultDataTypeForEscapingRules(const std::vector<FormatSettings::EscapingRule>& escaping_rules) {
  DataTypes data_types;
  for (const auto& rule : escaping_rules) data_types.push_back(getDefaultDataTypeForEscapingRule(rule));
  return data_types;
}

String getAdditionalFormatInfoForAllRowBasedFormats(const FormatSettings& settings) {
  return fmt::format(
      "schema_inference_hints={}, max_rows_to_read_for_schema_inference={}, max_bytes_to_read_for_schema_inference={}, "
      "schema_inference_make_columns_nullable={}, date_time_input_format={}, input_format_try_infer_variants={}",
      settings.schema_inference_hints, settings.max_rows_to_read_for_schema_inference, settings.max_bytes_to_read_for_schema_inference,
      settings.schema_inference_make_columns_nullable, settings.date_time_input_format, settings.try_infer_variant);
}
...
String getAdditionalFormatInfoByEscapingRule(const FormatSettings& settings, FormatSettings::EscapingRule escaping_rule) {
  ...
  result += fmt::format(", try_infer_integers={}, try_infer_dates={}, try_infer_datetimes={}, try_infer_datetimes_only_datetime64={}",
                        settings.try_infer_integers, settings.try_infer_dates, settings.try_infer_datetimes,
  ...
  switch (escaping_rule) {
    ...
    case FormatSettings::EscapingRule::Raw:
      result += fmt::format(
          ...
          settings.tsv.use_best_effort_in_schema_inference, settings.bool_true_representation, settings.bool_false_representation,
    ...
    case FormatSettings::EscapingRule::CSV:
      result += fmt::format(
          ...
          "try_infer_strings_from_quoted_tuples={}",
          settings.csv.use_best_effort_in_schema_inference, settings.bool_true_representation, settings.bool_false_representation,
          settings.csv.null_representation, settings.csv.delimiter, settings.csv.tuple_delimiter,
          settings.csv.try_infer_numbers_from_strings, settings.csv.try_infer_strings_from_quoted_tuples);
      break;
    case FormatSettings::EscapingRule::JSON:
      result += fmt::format(
          ...
          settings.json.try_infer_numbers_from_strings, settings.json.read_bools_as_numbers, settings.json.read_bools_as_strings,
          settings.json.read_objects_as_strings, settings.json.read_numbers_as_strings, settings.json.read_arrays_as_strings,
          settings.json.try_infer_objects_as_tuples, settings.json.infer_incomplete_types_as_strings,
...
void checkSupportedDelimiterAfterField(FormatSettings::EscapingRule escaping_rule, const String& delimiter, const DataTypePtr& type) {
  if (escaping_rule != FormatSettings::EscapingRule::Escaped) return;

  bool is_supported_delimiter_after_string = !delimiter.empty() && (delimiter.front() == '\t' || delimiter.front() == '\n');
  if (is_supported_delimiter_after_string) return;
  ...
  if (!type || isString(removeNullable(removeLowCardinality(type))))
    throw Exception(ErrorCodes::BAD_ARGUMENTS,
                    "'Escaped' serialization requires delimiter after String field to start with '\\t' or '\\n'");
}
