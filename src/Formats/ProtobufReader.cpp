...
ProtobufReader::ProtobufReader(ReadBuffer& in_) : in(&in_) {}
...
void ProtobufReader::endMessage(bool ignore_errors) {
  if (!current_message_level) return;
...
void ProtobufReader::startNestedMessage() {
  ...
  if ((cursor > field_end) && (field_end != END_OF_GROUP)) throwUnknownFormat();
...
bool ProtobufReader::readFieldNumber(int& field_number_) {
  ...
  if (unlikely(varint & (static_cast<UInt64>(0xFFFFFFFF) << 32))) throwUnknownFormat();
  ...
  switch (wire_type) {
    ...
    case GROUP_END: {
      if (current_message_end != END_OF_GROUP) throwUnknownFormat();
      current_message_end = cursor;
      return false;
    }
  }
...
Int64 ProtobufReader::readInt() { return static_cast<Int64>(readUInt()); }

Int64 ProtobufReader::readSInt() { return decodeZigZag(readUInt()); }

template <typename T>
T ProtobufReader::readFixed() {
  if (unlikely(cursor + static_cast<Int64>(sizeof(T)) > field_end)) throwUnknownFormat();
  T value;
  readBinary(&value, sizeof(T));
  if (cursor < field_end) next_field_number = field_number;
  return value;
}
...
void ProtobufReader::readString(String& str) {
  if (unlikely(cursor > field_end)) throwUnknownFormat();
...
void ProtobufReader::ignoreAll() { cursor += in->tryIgnore(std::numeric_limits<size_t>::max()); }

void ProtobufReader::moveCursorBackward(UInt64 num_bytes) {
  if (in->offset() < num_bytes) throwUnknownFormat();
...
#  define PROTOBUF_READER_READ_VARINT_BYTE(byteNo)                                                \
    do {                                                                                          \
      ...
      if constexpr ((byteNo) < 10) {                                                              \
        ...
        if (likely(!(c & 0x80))) return result;                                                   \
      } else {                                                                                    \
        if (likely(c == 1)) return result;                                                        \
      }                                                                                           \
      if constexpr ((byteNo) < 9) result &= ~(static_cast<UInt64>(0x80) << (7 * ((byteNo) - 1))); \
    } while (false)
...
#  define PROTOBUF_READER_IGNORE_VARINT_BYTE(byteNo) \
    do {                                             \
      ...
      ++cursor;                                      \
      if constexpr ((byteNo) < 10) {                 \
        if (likely(!(c & 0x80))) return;             \
      } else {                                       \
        if (likely(c == 1)) return;                  \
      }                                              \
    } while (false)
