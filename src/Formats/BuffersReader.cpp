...
BuffersReader::BuffersReader(ReadBuffer& istr_, const Block& header_, const FormatSettings& format_settings_)
    : istr(istr_), header(header_), format_settings(format_settings_) {
  chassert(header.columns() > 0);
}

Block BuffersReader::getHeader() const { return header; }

Block BuffersReader::read() {
  if (istr.eof()) return Block{};
  ...
  if (num_columns != header.columns())
    throw Exception(ErrorCodes::INCORRECT_DATA, "Buffers block has {} buffers, but header has {} columns", num_columns, header.columns());
  ...
  for (size_t i = 0; i < num_columns; ++i) {
    ...
    if (consumed != buffer_size)
      throw Exception(
          ...
          column.name, column.type->getName(), i, buffer_size, consumed);
