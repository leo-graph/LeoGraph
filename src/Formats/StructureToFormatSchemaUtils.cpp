...
void writeIndent(WriteBuffer& buf, size_t indent) { writeChar(' ', indent * 4, buf); }
...
String getSchemaMessageName(const String& column_name) {
  String result = column_name;
  if (!column_name.empty() && isalpha(column_name[0])) result[0] = static_cast<char>(toupper(column_name[0]));
  return result;
}
...
NamesAndTypesList collectNested(const NamesAndTypesList& names_and_types, bool allow_split_by_underscore, const String& format_name) {
  ...
  for (const auto& [name, type] : names_and_types) {
    ...
    if (isdigit(field_name[0]))
      throw Exception(ErrorCodes::BAD_ARGUMENTS, "Format {} doesn't support field names that starts with a digit: '{}'", format_name,
                      field_name);
  ...
  for (auto& [field_name, elements] : nested) elements = collectNested(elements, allow_split_by_underscore, format_name);
...
NamesAndTypesList getCollectedTupleElements(const DataTypeTuple& tuple_type, bool allow_split_by_underscore, const String& format_name) {
  ...
  if (tuple_type.hasExplicitNames()) {
    ...
    for (size_t i = 0; i != nested_types.size(); ++i) nested_names.push_back("e" + std::to_string(i + 1));
  }

  NamesAndTypesList result;
  for (size_t i = 0; i != nested_names.size(); ++i) result.emplace_back(nested_names[i], nested_types[i]);
