...
void writeBits(UInt64 *dest, size_t bit_offset, UInt64 value) {
  ...
  if (mod) dest[bit_offset / 64 + 1] |= value >> (64 - mod);
}
...
UInt64 readBits(const UInt64 *src, size_t bit_offset, size_t num_bits) {
  ...
  if (mod) value |= src[bit_offset / 64 + 1] << (64 - mod);
...
MarksInCompressedFile::MarksInCompressedFile(const PlainArray &marks)
  ...
  for (size_t block_idx = 0; block_idx < blocks.size(); ++block_idx) {
    ...
    block.bits_for_y = static_cast<UInt8>(sizeof(size_t) * 8 - getLeadingZeroBits((max_y - block.min_y) >> block.trailing_zero_bits_in_y));
  ...
  for (size_t idx = 0; idx < num_marks; ++idx) {
    ...
    writeBits(packed.data(), offset + block->bits_for_x,
...
size_t MarksInCompressedFile::approximateMemoryUsage() const { return sizeof(*this) + blocks.allocated_bytes() + packed.allocated_bytes(); }
