...
void jsonElementToString(const typename JSONParser::Element &element, WriteBuffer &buf, const FormatSettings &format_settings) {
  ...
  if (element.isArray()) {
    ...
    for (auto value : element.getArray()) {
      if (std::exchange(need_comma, true)) writeChar(',', buf);
...
bool tryGetNumericValueFromJSONElement(NumberType &value, const typename JSONParser::Element &element, bool convert_bool_to_number,
                                       bool allow_type_conversion, String &error) {
  switch (element.type()) {
    ...
    case ElementType::STRING: {
      if (!allow_type_conversion) return false;
      ...
      if constexpr (is_floating_point<NumberType>) {
        ...
        if (tryReadIntText(value, rb) && rb.eof()) break;
...
class NumericNode : public JSONExtractTreeNode<JSONParser> {
 public:
  ...
  bool insertResultToColumn(IColumn &column, const typename JSONParser::Element &element, const JSONExtractInsertSettings &insert_settings,
                            const FormatSettings &format_settings, String &error) const override {
    if (element.isNull()) {
    ...
    NumberType value;
    if (!tryGetNumericValueFromJSONElement<JSONParser, NumberType>(value, element, /*convert_bool_to_number=*/true,
                                                                   insert_settings.allow_type_conversion, error)) {
      if (error.empty())
        error = fmt::format("cannot read {} value from JSON element: {}", TypeName<NumberType>,
                            jsonElementToString<JSONParser>(element, format_settings));
      return false;
    }

    if (is_bool_type) value = static_cast<bool>(value);
...
class LowCardinalityNumericNode : public NumericNode<JSONParser, NumberType> {
 public:
  ...
  bool insertResultToColumn(IColumn &column, const typename JSONParser::Element &element, const JSONExtractInsertSettings &insert_settings,
    ...
    if (this->is_bool_type) value = static_cast<bool>(value);
...
class StringNode : public JSONExtractTreeNode<JSONParser> {
 public:
  bool insertResultToColumn(IColumn &column, const typename JSONParser::Element &element, const JSONExtractInsertSettings &insert_settings,
    ...
    if (!element.isString()) {
      if (!insert_settings.allow_type_conversion) return false;
...
class LowCardinalityFixedStringNode : public JSONExtractTreeNode<JSONParser> {
 public:
  explicit LowCardinalityFixedStringNode(bool is_nullable_, size_t fixed_length_)
      : is_nullable(is_nullable_), fixed_length(fixed_length_) {}
...
class UUIDNode : public JSONExtractTreeNode<JSONParser> {
 public:
  bool insertResultToColumn(IColumn &column, const typename JSONParser::Element &element, const JSONExtractInsertSettings &,
                            const FormatSettings &format_settings, String &error) const override {
  ...
  bool tryParse(time_t &value, std::string_view data, FormatSettings::DateTimeInputFormat date_time_input_format) const {
    ...
    switch (date_time_input_format) {
      case FormatSettings::DateTimeInputFormat::Basic:
        if (tryReadDateTimeText(value, buf, time_zone) && buf.eof()) return true;
        break;
      case FormatSettings::DateTimeInputFormat::BestEffort:
        if (tryParseDateTimeBestEffort(value, buf, time_zone, utc_time_zone) && buf.eof()) return true;
        break;
      case FormatSettings::DateTimeInputFormat::BestEffortUS:
        if (tryParseDateTimeBestEffortUS(value, buf, time_zone, utc_time_zone) && buf.eof()) return true;
        break;
    }
...
class TimeNode : public JSONExtractTreeNode<JSONParser> {
 public:
  ...
  bool tryParse(time_t &value, std::string_view data, FormatSettings::DateTimeInputFormat /*time_input_format*/) const {
    ...
    if (tryReadTimeText(value, buf, date_lut) && buf.eof()) return true;
...
class DateTime64Node : public JSONExtractTreeNode<JSONParser>, public TimezoneMixin {
 public:
  ...
  bool insertResultToColumn(IColumn &column, const typename JSONParser::Element &element, const JSONExtractInsertSettings &insert_settings,
    ...
    if (element.isString()) {
      ...
      switch (element.type()) {
        ...
        default:
          error =
              fmt::format("cannot read DateTime64 value from JSON element: {}", jsonElementToString<JSONParser>(element, format_settings));
          return false;
      }
    }
  ...
  bool tryParse(DateTime64 &value, std::string_view data, FormatSettings::DateTimeInputFormat date_time_input_format) const {
    ...
    switch (date_time_input_format) {
      case FormatSettings::DateTimeInputFormat::Basic:
        if (tryReadDateTime64Text(value, scale, buf, time_zone) && buf.eof()) return true;
        break;
      case FormatSettings::DateTimeInputFormat::BestEffort:
        if (tryParseDateTime64BestEffort(value, scale, buf, time_zone, utc_time_zone) && buf.eof()) return true;
        break;
      case FormatSettings::DateTimeInputFormat::BestEffortUS:
        if (tryParseDateTime64BestEffortUS(value, scale, buf, time_zone, utc_time_zone) && buf.eof()) return true;
        break;
    }
...
class Time64Node : public JSONExtractTreeNode<JSONParser> {
 public:
  ...
  bool tryParse(Time64 &value, std::string_view data, FormatSettings::DateTimeInputFormat /*time_input_format*/) const {
    ...
    if (tryReadTime64Text(value, scale, buf, date_lut) && buf.eof()) return true;
...
class EnumNode : public JSONExtractTreeNode<JSONParser> {
 public:
  explicit EnumNode(const std::vector<std::pair<String, Type>> &name_value_pairs_, Type default_value_)
      : name_value_pairs(name_value_pairs_), default_value(default_value_) {
...
class NullableNode : public JSONExtractTreeNode<JSONParser> {
 public:
  ...
  bool insertResultToColumn(IColumn &column, const typename JSONParser::Element &element, const JSONExtractInsertSettings &insert_settings,
    ...
    if (!nested->insertResultToColumn(col_null.getNestedColumn(), element, insert_settings, format_settings, error)) return false;
...
class LowCardinalityNode : public JSONExtractTreeNode<JSONParser> {
 public:
  ...
  bool insertResultToColumn(IColumn &column, const typename JSONParser::Element &element, const JSONExtractInsertSettings &insert_settings,
    ...
    if (!nested->insertResultToColumn(*tmp_nested, element, insert_settings, format_settings, error)) return false;
...
class TupleNode : public JSONExtractTreeNode<JSONParser> {
 public:
  TupleNode(std::vector<std::unique_ptr<JSONExtractTreeNode<JSONParser>>> nested_, const std::vector<String> &explicit_names_)
    ...
    for (size_t i = 0; i != explicit_names.size(); ++i) name_to_index_map.emplace(explicit_names[i], i);
  }
      ...
      for (size_t i = 0; i != tuple.tupleSize(); ++i) {
        ...
        if (col.size() != size) {
          if (col.size() > size)
            ...
            while (col.size() < size) col.insertDefault();
        }
      }
    };
      ...
      if (name_to_index_map.empty()) {
        ...
        for (const auto &[key, value] : object) {
          ...
          if (index != name_to_index_map.end()) {
            if (nested[index->second]->insertResultToColumn(tuple.getColumn(index->second), value, insert_settings, format_settings,
                                                            error)) {
              were_valid_elements = true;
            } else if (!insert_settings.insert_default_on_invalid_elements_in_complex_types) {
              set_size(old_size);
    ...
    for (size_t i : order) {
      ...
      if (variant_nodes[i]->insertResultToColumn(variant, element, insert_settings, format_settings, error)) {
        column_variant.getLocalDiscriminators().push_back(
            column_variant.localDiscriminatorByGlobal(static_cast<ColumnVariant::Discriminator>(i)));
...
class DynamicNode : public JSONExtractTreeNode<JSONParser> {
  ...
  bool insertResultToColumn(IColumn &column, const typename JSONParser::Element &element, const JSONExtractInsertSettings &insert_settings,
    ...
    if (insert_settings.try_existing_variants_in_dynamic_first) {
      ...
      if (variants_order_it == variants_order_cache.end())
        variants_order_it =
            variants_order_cache
                .emplace(variant_info.variant_name, SerializationVariant::getVariantsDeserializeTextOrder(
                                                        assert_cast<const DataTypeVariant &>(*variant_info.variant_type).getVariants()))
                .first;
      ...
      for (size_t i : variants_order_it->second) {
        if (i != shared_variant_discr) {
          ...
          if (it == json_extract_nodes_cache.end())
            it = json_extract_nodes_cache
                     .emplace(variant_info.variant_names[i], buildJSONExtractTree<JSONParser>(variant_types[i], "Dynamic inference"))
                     .first;

          if (it->second->insertResultToColumn(variant_column.getVariantByGlobalDiscriminator(i), element,
                                               insert_settings_with_no_type_conversion, format_settings, error)) {
            variant_column.getLocalDiscriminators().push_back(
                variant_column.localDiscriminatorByGlobal(static_cast<ColumnVariant::Discriminator>(i)));
    ...
    if (column_dynamic.addNewVariant(element_type, element_type_name)) {
      ...
      if (!it->second->insertResultToColumn(variant, element, insert_settings, format_settings, error)) return false;
    ...
    if (!node->insertResultToColumn(*tmp_variant_column, element, insert_settings, format_settings, error)) return false;
 ...
 private:
  DataTypePtr elementToDataTypeImpl(const typename JSONParser::Element &element, const FormatSettings &format_settings,
                                    JSONInferenceInfo &json_inference_info) const {
    switch (element.type()) {
      case ElementType::NULL_VALUE:
      ...
      case ElementType::INT64: {
        ...
        if (element.getInt64() < 0) json_inference_info.negative_integers.insert(type.get());
        return type;
      }
      case ElementType::UINT64:
      ...
      case ElementType::STRING: {
        ...
        if (auto type = tryInferDateOrDateTimeFromString(data, format_settings)) return type;
      ...
      case ElementType::ARRAY: {
        ...
        for (auto value : array) types.push_back(elementToDataTypeImpl(value, format_settings, json_inference_info));

        if (types.empty()) return std::make_shared<DataTypeArray>(std::make_shared<DataTypeNothing>());

        if (checkIfTypesAreEqual(types)) return std::make_shared<DataTypeArray>(types.back());
        ...
        for (const auto &type : types) {
          if (!checkIfTypeIsComplete(type)) return std::make_shared<DataTypeTuple>(types);
        }
        ...
        if (checkIfTypesAreEqual(types_copy)) return std::make_shared<DataTypeArray>(types_copy.back());
...
class ObjectJSONNode : public JSONExtractTreeNode<JSONParser> {
 public:
  ObjectJSONNode(const std::unordered_map<String, DataTypePtr> &typed_paths_types_,
                 std::unordered_map<String, std::unique_ptr<JSONExtractTreeNode<JSONParser>>> typed_path_nodes_,
                 const std::unordered_set<String> &paths_to_skip_, const std::vector<String> &path_regexps_to_skip_,
    ...
    for (const auto &regexp : path_regexps_to_skip_) path_regexps_to_skip.emplace_back(regexp);
  }
  ...
  bool insertResultToColumn(IColumn &column, const typename JSONParser::Element &element, const JSONExtractInsertSettings &insert_settings,
    ...
    for (const auto &[path, _] : paths_and_values_for_shared_data) new_paths_total_size += path.size();
    ...
    for (size_t i = 0; i != paths_and_values_for_shared_data.size(); ++i) {
      ...
      if (i != 0 && path == paths_and_values_for_shared_data[i - 1].first) {
        if (!format_settings.json.type_json_skip_duplicated_paths) {
          error = fmt::format(
              "Duplicate path found during parsing JSON object: {}. You can enable setting type_json_skip_duplicated_paths to skip "
              "duplicated paths during insert",
              path);
          SerializationObject::restoreColumnObject(column_object, prev_size);
          return false;
        }
    ...
    for (auto &[typed_path, typed_column] : column_object.getTypedPaths()) {
      if (typed_column->size() == prev_size) typed_paths_types.at(typed_path)->insertDefaultInto(*typed_column);
    }

    for (auto &[_, dynamic_column] : column_object.getDynamicPathsPtrs()) {
      if (dynamic_column->size() == prev_size) dynamic_column->insertDefault();
    }
 ...
 private:
  bool traverseAndInsert(ColumnObject &column_object, const typename JSONParser::Element &element, const String &current_path,
                         const JSONExtractInsertSettings &insert_settings, const FormatSettings &format_settings,
                         std::vector<std::pair<String, typename JSONParser::Element>> &paths_and_values_for_shared_data,
                         size_t current_size, String &error, bool is_root) const {
    if (shouldSkipPath(current_path, insert_settings)) return true;

    if (element.isObject() &&
        (!typed_path_nodes.contains(current_path) || (format_settings.json.type_json_allow_duplicated_key_with_literal_and_nested_object &&
                                                      hasTypedPathWithPrefix(current_path + ".")))) {
      std::unordered_map<std::string_view, std::unordered_set<JSONElementType>> visited_keys;
      for (auto [key, value] : element.getObject()) {
        String path = current_path;
        if (!is_root) path.append(".");
        ...
        if (it != visited_keys.end()) {
          if (format_settings.json.type_json_allow_duplicated_key_with_literal_and_nested_object) {
            ...
            if (it->second.contains(value_element_type)) {
              if (format_settings.json.type_json_skip_duplicated_paths) continue;
              error = fmt::format(
                  "Duplicate path found during parsing JSON object: {}. You can enable setting type_json_skip_duplicated_paths to skip "
                  "duplicated paths during insert",
                  path);
            ...
            error = fmt::format(
                "Duplicate path found during parsing JSON object: {}. You can enable setting type_json_skip_duplicated_paths to skip "
                "duplicated paths during insert or setting type_json_allow_duplicated_key_with_literal_and_nested_object to allow "
                "duplicated path with literal and nested object",
                path);
            return false;
          }
        } else {
        ...
        if (!traverseAndInsert(column_object, value, path, insert_settings, format_settings, paths_and_values_for_shared_data, current_size,
                               error, false))
          return false;
      }
    ...
    if (auto typed_it = typed_paths.find(current_path); typed_it != typed_paths.end()) {
      ...
      if (typed_it->second->size() > current_size) {
        if (!format_settings.json.type_json_skip_duplicated_paths) {
          error = fmt::format(
              "Duplicate path found during parsing JSON object: {}. You can enable setting type_json_skip_duplicated_paths to skip "
              "duplicated paths during insert or setting type_json_allow_duplicated_key_with_literal_and_nested_object to allow duplicated "
              "path with literal and nested object",
              current_path);
          return false;
        }
      } else if (!typed_path_nodes.at(current_path)
                      ->insertResultToColumn(*typed_it->second, element, insert_settings, format_settings, error)) {
  ...
  bool insertIntoDynamicPath(ColumnDynamic &column_dynamic, const typename JSONParser::Element &element,
                             const JSONExtractInsertSettings &insert_settings, const FormatSettings &format_settings, String &error) const {
    /// Check if element is NULL.
    if (element.isNull()) {
      column_dynamic.insertDefault();
      return true;
    ...
    switch (element.type()) {
      ...
      case ElementType::STRING: {
        ...
        if (auto it = variant_info.variant_name_to_discriminator.find("DateTime"); it != variant_info.variant_name_to_discriminator.end()) {
          time_t value;
          if (tryInferDateTimeFromString(data, value, format_settings, time_zone_for_schema_inference,
                                         utc_time_zone_for_schema_inference)) {
            insertValueIntoNumericVariant<ColumnDateTime, UInt32>(variant_info, variant_column, static_cast<UInt32>(value), "DateTime");
            return true;
          }
        }

        if (auto it = variant_info.variant_name_to_discriminator.find("DateTime64(9)");
            it != variant_info.variant_name_to_discriminator.end()) {
          DateTime64 value;
          if (tryInferDateTime64FromString(data, value, format_settings, time_zone_for_schema_inference,
                                           utc_time_zone_for_schema_inference)) {
  ...
  bool insertIntoSharedData(WriteBuffer &buf, const typename JSONParser::Element &element, const JSONExtractInsertSettings &insert_settings,
                            const FormatSettings &format_settings, String &error, MutableColumnPtr &tmp_dynamic_column) const {
    ...
    if (!tmp_dynamic_column) tmp_dynamic_column = ColumnDynamic::create();
  ...
  void insertValueIntoNumericVariant(const ColumnDynamic::VariantInfo &variant_info, ColumnVariant &variant_column, ElementType value,
  ...
  switch (type->getTypeId()) {
    ...
    case TypeIndex::LowCardinality: {
      ...
      switch (dictionary_type->getTypeId()) {
        ...
        case TypeIndex::FixedString:
          return std::make_unique<LowCardinalityFixedStringNode<JSONParser>>(
              is_nullable, assert_cast<const DataTypeFixedString &>(*dictionary_type).getN());
        case TypeIndex::UUID:
          return std::make_unique<LowCardinalityUUIDNode<JSONParser>>(is_nullable);
        default:
          return std::make_unique<LowCardinalityNode<JSONParser>>(
              is_nullable, buildJSONExtractTree<JSONParser>(dictionary_type, source_for_exception_message));
      }
    }
    case TypeIndex::Nullable:
      return std::make_unique<NullableNode<JSONParser>>(
          buildJSONExtractTree<JSONParser>(assert_cast<const DataTypeNullable &>(*type).getNestedType(), source_for_exception_message));
    case TypeIndex::Array:
      return std::make_unique<ArrayNode<JSONParser>>(
          buildJSONExtractTree<JSONParser>(assert_cast<const DataTypeArray &>(*type).getNestedType(), source_for_exception_message));
    ...
    case TypeIndex::Map: {
      ...
      if (!isString(removeLowCardinality(key_type)))
        throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT, "{} doesn't support the return type schema: {} with key type not String",
                        source_for_exception_message, type->getName());
    ...
    case TypeIndex::Variant: {
      ...
      for (const auto &variant : variants) variant_nodes.push_back(buildJSONExtractTree<JSONParser>(variant, source_for_exception_message));
      return std::make_unique<VariantNode<JSONParser>>(std::move(variant_nodes),
                                                       SerializationVariant::getVariantsDeserializeTextOrder(variants));
    }
    ...
    case TypeIndex::Object: {
      ...
      switch (object_type.getSchemaFormat()) {
        case DataTypeObject::SchemaFormat::JSON:
          return std::make_unique<ObjectJSONNode<JSONParser>>(typed_paths, std::move(typed_path_nodes), object_type.getPathsToSkip(),
                                                              object_type.getPathRegexpsToSkip(), object_type.getTypeOfNestedObjects());
      }
    }
    default:
      throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT, "{} doesn't support the return type schema: {}", source_for_exception_message,
                      type->getName());
  }
}

#if USE_SIMDJSON
template void jsonElementToString<SimdJSONParser>(const SimdJSONParser::Element &element, WriteBuffer &buf,
                                                  const FormatSettings &format_settings);
template std::unique_ptr<JSONExtractTreeNode<SimdJSONParser>> buildJSONExtractTree<SimdJSONParser>(
    const DataTypePtr &type, const char *source_for_exception_message);
#endif

#if USE_RAPIDJSON
template void jsonElementToString<RapidJSONParser>(const RapidJSONParser::Element &element, WriteBuffer &buf,
                                                   const FormatSettings &format_settings);
template std::unique_ptr<JSONExtractTreeNode<RapidJSONParser>> buildJSONExtractTree<RapidJSONParser>(
    const DataTypePtr &type, const char *source_for_exception_message);
template bool tryGetNumericValueFromJSONElement<RapidJSONParser, Float64>(Float64 &value, const RapidJSONParser::Element &element,
                                                                          bool convert_bool_to_number, bool allow_type_conversion,
                                                                          String &error);
#else
template void jsonElementToString<DummyJSONParser>(const DummyJSONParser::Element &element, WriteBuffer &buf,
                                                   const FormatSettings &format_settings);
template std::unique_ptr<JSONExtractTreeNode<DummyJSONParser>> buildJSONExtractTree<DummyJSONParser>(
    const DataTypePtr &type, const char *source_for_exception_message);
template bool tryGetNumericValueFromJSONElement<DummyJSONParser, Float64>(Float64 &value, const DummyJSONParser::Element &element,
                                                                          bool convert_bool_to_number, bool allow_type_conversion,
                                                                          String &error);
template bool tryGetNumericValueFromJSONElement<DummyJSONParser, Int64>(Int64 &value, const DummyJSONParser::Element &element,
                                                                        bool convert_bool_to_number, bool allow_type_conversion,
                                                                        String &error);
template bool tryGetNumericValueFromJSONElement<DummyJSONParser, UInt64>(UInt64 &value, const DummyJSONParser::Element &element,
                                                                         bool convert_bool_to_number, bool allow_type_conversion,
                                                                         String &error);
#endif
