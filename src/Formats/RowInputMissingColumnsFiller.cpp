...
RowInputMissingColumnsFiller::RowInputMissingColumnsFiller(const NamesAndTypesList &names_and_types_) : names_and_types(names_and_types_) {
...
void RowInputMissingColumnsFiller::setNestedGroups(std::unordered_map<std::string_view, std::vector<size_t>> &&nested_groups,
                                                   size_t num_columns) {
  if (!nested_groups.empty()) {
    column_infos.resize(num_columns);
    for (auto &nested_group : nested_groups | boost::adaptors::map_values) {
      if (nested_group.size() <= 1) continue;
      auto nested_group_shared = std::make_shared<std::vector<size_t>>(std::move(nested_group));
      for (size_t i : *nested_group_shared) column_infos[i].nested_group = nested_group_shared;
    }
  }
}
...
void RowInputMissingColumnsFiller::addDefaults(MutableColumns &columns, size_t row_num) const {
  for (size_t i = 0; i != columns.size(); ++i) {
    ...
    if (row_num < column_size) continue;  /// The column already has an element in this position, skipping.
    ...
    for (size_t j : nested_group) {
      ...
      if (row_num >= column_size_j) {
        if (row_num > column_size_j)
          throw Exception(ErrorCodes::ILLEGAL_COLUMN, "Wrong row_number {}, expected either {} or {}", row_num, column_size_j - 1,
                          column_size_j);
        ...
        for (size_t k = 0; k != size_of_array; ++k) data.insertDefault();
...
const NamesAndTypesList &RowInputMissingColumnsFiller::getNamesAndTypes() const { return names_and_types; }
