...
class TableFunctionExplain : public ITableFunction {
 ...
 private:
  StoragePtr executeImpl(const ASTPtr &ast_function, ContextPtr context, const String &table_name, ColumnsDescription cached_columns,
...
std::vector<size_t> TableFunctionExplain::skipAnalysisForArguments(const QueryTreeNodePtr &query_node_table_function,
  ...
  if (table_function_node_arguments_size == 3) return {2};
...
void TableFunctionExplain::parseArguments(const ASTPtr &ast_function, ContextPtr context) {
  ...
  if (!function || !function->arguments)
    throw Exception(ErrorCodes::BAD_ARGUMENTS, "Table function '{}' cannot be called directly, use `SELECT * FROM (EXPLAIN ...)` syntax",
                    getName());

  size_t num_args = function->arguments->children.size();
  if (num_args != 2 && num_args != 3)
    throw Exception(ErrorCodes::BAD_ARGUMENTS, "Table function '{}' requires 2 or 3 arguments, got {}", getName(), num_args);
  ...
  if (!kind_literal || kind_literal->value.getType() != Field::Types::String)
    throw Exception(ErrorCodes::BAD_ARGUMENTS, "Table function '{}' requires a String argument for EXPLAIN kind, got '{}'", getName(),
                    kind_arg->formatForErrorMessage());
  ...
  if (!settings_arg || settings_arg->value.getType() != Field::Types::String)
    throw Exception(ErrorCodes::BAD_ARGUMENTS, "Table function '{}' requires a serialized string settings argument, got '{}'", getName(),
                    function->arguments->children[1]->formatForErrorMessage());
  ...
  if (!settings_str.empty()) {
    ...
    ASTPtr settings_ast = parseQuery(settings_parser, settings_str, settings[Setting::max_query_size], settings[Setting::max_parser_depth],
  ...
  if (function->arguments->children.size() > 2) {
    ...
    if (!subquery)
      throw Exception(ErrorCodes::BAD_ARGUMENTS, "Table function '{}' requires a subquery argument, got '{}'", getName(),
                      subquery_arg->formatForErrorMessage());

    if (subquery->children.empty()) throw Exception(ErrorCodes::UNEXPECTED_AST_STRUCTURE, "A subquery AST element must have a child");

    const auto &query_arg = subquery->children[0];

    if (!query_arg->as<ASTSelectWithUnionQuery>())
      throw Exception(ErrorCodes::BAD_ARGUMENTS, "Table function '{}' requires a EXPLAIN's SELECT query argument, got '{}'", getName(),
                      query_arg->formatForErrorMessage());
...
ColumnsDescription TableFunctionExplain::getActualTableStructure(ContextPtr context, bool /*is_insert_query*/) const {
  Block sample_block = getInterpreter(context).getSampleBlock(
      query->as<ASTExplainQuery>()->getKind());  /// NOLINT(readability-static-accessed-through-instance)
  ColumnsDescription columns_description;
  for (const auto &column : sample_block.getColumnsWithTypeAndName()) columns_description.add(ColumnDescription(column.name, column.type));
  return columns_description;
}

Block executeMonoBlock(QueryPipeline &pipeline) {
  if (!pipeline.pulling()) throw Exception(ErrorCodes::LOGICAL_ERROR, "Expected pulling pipeline");
...
StoragePtr TableFunctionExplain::executeImpl(const ASTPtr & /*ast_function*/, ContextPtr context, const std::string &table_name,
...
InterpreterExplainQuery TableFunctionExplain::getInterpreter(ContextPtr context) const {
  if (!query) throw Exception(ErrorCodes::LOGICAL_ERROR, "Table function '{}' requires a explain query argument", getName());
