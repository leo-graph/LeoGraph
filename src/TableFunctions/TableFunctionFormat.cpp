...
void TableFunctionFormat::parseArguments(const ASTPtr& ast_function, ContextPtr context) {
  ...
  if (args_func.size() != 1) throw Exception(ErrorCodes::LOGICAL_ERROR, "Table function '{}' must have arguments", getName());

  ASTs& args = args_func.at(0)->children;

  if (args.empty() || args.size() > 3)
    throw Exception(ErrorCodes::NUMBER_OF_ARGUMENTS_DOESNT_MATCH,
                    "Table function '{}' requires from 1 to 3 arguments: [format, [structure]], data", getName());
  ...
  if (args.size() > 1) format = checkAndGetLiteralArgument<String>(args[0], "format");
  if (args.size() == 3) structure = checkAndGetLiteralArgument<String>(args[1], "structure");
}
...
ColumnsDescription TableFunctionFormat::getActualTableStructure(ContextPtr context, bool /*is_insert_query*/) const {
  if (structure == "auto") {
    ...
    if (format == "auto") return detectFormatAndReadSchema(std::nullopt, read_buffer_iterator, context).first;
...
Block TableFunctionFormat::parseData(const ColumnsDescription& columns, const String& format_name, const ContextPtr& context) const {
  ...
  if (columns.hasDefaults()) {
    builder.addSimpleTransform(
        [&](const SharedHeader& header) { return std::make_shared<AddingDefaultsTransform>(header, columns, *input_format, context); });
  }
  ...
  while (reader->pull(block)) blocks.push_back(std::move(block));
...
StoragePtr TableFunctionFormat::executeImpl(const ASTPtr& /*ast_function*/, ContextPtr context, const std::string& table_name,
...
const FunctionDocumentation format_table_function_documentation = {.description = R"(
