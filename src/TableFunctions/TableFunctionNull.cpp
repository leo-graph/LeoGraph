...
void TableFunctionNull::parseArguments(const ASTPtr& ast_function, ContextPtr context) {
  ...
  if (!arguments.empty() && arguments.size() != 1)
    throw Exception(ErrorCodes::NUMBER_OF_ARGUMENTS_DOESNT_MATCH, "Table function '{}' requires 'structure' argument or empty argument",
                    getName());
...
ColumnsDescription TableFunctionNull::getActualTableStructure(ContextPtr context, bool /*is_insert_query*/) const {
  if (structure != "auto") return parseColumnsListFromString(structure, context);
  return default_structure;
}

StoragePtr TableFunctionNull::executeImpl(const ASTPtr& /*ast_function*/, ContextPtr context, const std::string& table_name,
...
void registerTableFunctionNull(TableFunctionFactory& factory) { factory.registerFunction<TableFunctionNull>({}, {.allow_readonly = true}); }
