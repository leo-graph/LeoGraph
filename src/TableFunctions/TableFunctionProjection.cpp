...
void TableFunctionMergeTreeProjection::parseArguments(const ASTPtr& ast_function, ContextPtr context) {
  ...
  if (args.size() != 3)
    throw Exception(ErrorCodes::NUMBER_OF_ARGUMENTS_DOESNT_MATCH, "Table function '{}' must have 3 arguments, got: {}", getName(),
                    args.size());
...
ColumnsDescription TableFunctionMergeTreeProjection::getActualTableStructure(ContextPtr context, bool /*is_insert_query*/) const {
  ...
  if (!metadata_snapshot->getProjections().has(projection_name))
    throw Exception(ErrorCodes::NUMBER_OF_ARGUMENTS_DOESNT_MATCH, "There is no projection {} in table {}", projection_name,
                    source_table_id.getFullTableName());

  return metadata_snapshot->getProjections().get(projection_name).metadata->columns;
}

StoragePtr TableFunctionMergeTreeProjection::executeImpl(const ASTPtr& /*ast_function*/, ContextPtr context, const std::string& table_name,
  ...
  auto res = std::make_shared<StorageFromMergeTreeProjection>(std::move(storage_id), std::move(source_table), std::move(metadata_snapshot),
                                                              projection);
