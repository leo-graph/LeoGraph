...
void TableFunctionFactory::registerFunction(const std::string& name, Value value, Case case_sensitiveness) {
  ...
  if (case_sensitiveness == Case::Insensitive && !case_insensitive_table_functions.emplace(Poco::toLower(name), value).second)
...
TableFunctionPtr TableFunctionFactory::get(const ASTPtr& ast_function, ContextPtr context) const {
  ...
  if (!res) {
    ...
    if (!hints.empty())
      throw Exception(ErrorCodes::UNKNOWN_FUNCTION, "Unknown table function {}. Maybe you meant: {}", table_function->name,
                      toString(hints));
...
TableFunctionPtr TableFunctionFactory::tryGet(const std::string& name_param, ContextPtr) const {
  ...
  if (table_functions.end() != it) {
    ...
    if (case_insensitive_table_functions.end() != it) res = it->second.creator();
  }
...
bool TableFunctionFactory::isTableFunctionName(const std::string& name) const { return table_functions.contains(name); }
...
std::optional<FunctionDocumentation> TableFunctionFactory::tryGetDocumentationImpl(const String& name_param) const {
  ...
  if (found.creator) return found.documentation;
...
std::optional<TableFunctionProperties> TableFunctionFactory::tryGetPropertiesImpl(const String& name_param) const {
  ...
  if (found.creator) return found.properties;
