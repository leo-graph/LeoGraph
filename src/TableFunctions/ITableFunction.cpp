...
const char *ITableFunction::getNonClusteredStorageEngineName() const { throw Exception(ErrorCodes::LOGICAL_ERROR, "Not implemented"); }

std::optional<AccessTypeObjects::Source> ITableFunction::getSourceAccessObject() const {
  if (isClusterFunction()) return StorageFactory::instance().getSourceAccessObject(getNonClusteredStorageEngineName());
...
StoragePtr ITableFunction::execute(const ASTPtr &ast_function, ContextPtr context, const std::string &table_name,
  ...
  if (const auto access_object = getSourceAccessObject()) {
    ...
    if (is_insert_query) type_to_check = AccessType::WRITE;
  ...
  if (cached_columns.empty()) return executeImpl(ast_function, context, table_name, std::move(cached_columns), is_insert_query);
