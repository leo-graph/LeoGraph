...
void TableFunctionLoop::parseArguments(const ASTPtr &ast_function, ContextPtr context) {
  ...
  if (!args_func.arguments) throw Exception(ErrorCodes::NUMBER_OF_ARGUMENTS_DOESNT_MATCH, "Table function 'loop' must have arguments.");

  auto &args = args_func.arguments->children;
  if (args.empty()) throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT, "No arguments provided for table function 'loop'");
  ...
  if (args.size() == 1) {
    if (const auto *id = args[0]->as<ASTIdentifier>()) {
      ...
      throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT, "Expected identifier or function for argument 1 of function 'loop', got {}",
                      args[0]->getID());
    }
  }
  // loop(database, table)
...
ColumnsDescription TableFunctionLoop::getActualTableStructure(ContextPtr context, bool is_insert_query) const {
  ...
  if (database_name.empty()) database_name = context->getCurrentDatabase();

  auto database = DatabaseCatalog::instance().getDatabase(database_name);
  auto storage = database->tryGetTable(loop_table_name, context);
  if (!storage) throw Exception(ErrorCodes::UNKNOWN_TABLE, "Table '{}' not found in database '{}'", loop_table_name, database_name);

  return storage->getInMemoryMetadataPtr()->getColumns();
}

StoragePtr TableFunctionLoop::executeImpl(const ASTPtr & /*ast_function*/, ContextPtr context, const std::string &table_name,
  ...
  if (!inner_table_function_ast) {
    ...
    auto inner_table_function = TableFunctionFactory::instance().get(inner_table_function_ast, context);
    storage = inner_table_function->execute(inner_table_function_ast, context, table_name, std::move(cached_columns),
                                            /*use_global_context=*/false,
                                            /*is_insert_query=*/is_insert_query);
  }
  auto res = std::make_shared<StorageLoop>(StorageID(getDatabaseName(), table_name), storage,
...
void registerTableFunctionLoop(TableFunctionFactory &factory) {
  factory.registerFunction<TableFunctionLoop>(
      {.description = R"(The table function can be used to continuously output query results in an infinite loop.)",
       .examples{{"loop", "SELECT * FROM loop((numbers(3)) LIMIT 7",
                  "0"
                  "1"
                  "2"
                  "0"
