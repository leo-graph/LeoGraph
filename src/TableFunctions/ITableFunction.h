...
class ITableFunction : public std::enable_shared_from_this<ITableFunction> {
 public:
   ...
   */
  virtual std::vector<size_t> skipAnalysisForArguments(const QueryTreeNodePtr & /*query_node_table_function*/,
  ...
  StoragePtr execute(const ASTPtr &ast_function, ContextPtr context, const std::string &table_name, ColumnsDescription cached_columns_ = {},
                     bool use_global_context = false, bool is_insert_query = false) const;
 ...
 private:
  virtual StoragePtr executeImpl(const ASTPtr &ast_function, ContextPtr context, const std::string &table_name,
                                 ColumnsDescription cached_columns, bool is_insert_query) const = 0;
