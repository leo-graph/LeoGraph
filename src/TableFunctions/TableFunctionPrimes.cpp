...
StoragePtr TableFunctionPrimes::executeImpl(const ASTPtr& ast_function, ContextPtr context, const std::string& table_name,
  ...
  if (const auto* function = ast_function->as<ASTFunction>()) {
    ...
    if (arguments.size() >= 4)
      throw Exception(ErrorCodes::NUMBER_OF_ARGUMENTS_DOESNT_MATCH, "Table function '{}' cannot have more than three params", getName());
    ...
    if (!arguments.empty()) {
      ...
      if (!step) throw Exception(ErrorCodes::BAD_ARGUMENTS, "Table function {} requires step to be a positive number", getName());

      auto res =
          std::make_shared<StorageSystemPrimes>(StorageID(getDatabaseName(), table_name), std::string{"prime"}, length, offset, step);
  ...
  throw Exception(ErrorCodes::NUMBER_OF_ARGUMENTS_DOESNT_MATCH,
                  "Table function '{}' requires 'limit' or 'offset, length', or 'offset, length, step'.", getName());
}
...
void registerTableFunctionPrimes(TableFunctionFactory& factory) {
  factory.registerFunction<TableFunctionPrimes>(
      {
          .description =
              R"(Returns tables with a single UInt64 column `prime` containing prime numbers in ascending order, starting from 2.)",
          .syntax = "primes() | primes(N) | primes(N, M) | primes(N, M, S)",
          .arguments =
              {
                  {"N",
                   "If used as primes(N): number of primes to return. If used as primes(N, M[, S]): starting prime index (0-based).",
                   {"UInt64"}},
