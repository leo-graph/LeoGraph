...
std::vector<size_t> TableFunctionViewIfPermitted::skipAnalysisForArguments(const QueryTreeNodePtr &, ContextPtr) const { return {0}; }
...
void TableFunctionViewIfPermitted::parseArguments(const ASTPtr &ast_function, ContextPtr context) {
  ...
  if (!function || !function->arguments || (function->arguments->children.size() != 2))
    throw Exception(ErrorCodes::NUMBER_OF_ARGUMENTS_DOESNT_MATCH,
                    "Table function '{}' requires two arguments: a SELECT query and a table function", getName());

  const auto &arguments = function->arguments->children;
  auto *select = arguments[0]->as<ASTSelectWithUnionQuery>();
  if (!select) throw Exception(ErrorCodes::BAD_ARGUMENTS, "Table function '{}' requires a SELECT query as its first argument", getName());
...
StoragePtr TableFunctionViewIfPermitted::executeImpl(const ASTPtr & /* ast_function */, ContextPtr context, const std::string &table_name,
...
bool TableFunctionViewIfPermitted::isPermitted(const ContextPtr &context, const ColumnsDescription &else_columns) const {
  ...
  try {
    ...
    if (e.code() == ErrorCodes::ACCESS_DENIED) return false;
    throw;
  }
  ...
  if (columns != else_columns) {
    throw Exception(ErrorCodes::BAD_ARGUMENTS,
                    ...
                    getName(), columns.toString(true), else_columns.toString(true));
  }
