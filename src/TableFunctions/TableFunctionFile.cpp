...
void TableFunctionFile::parseFirstArguments(const ASTPtr& arg, const ContextPtr& context) {
  ...
  if (type == Field::Types::String) {
    ...
    fd = static_cast<int>((type == Field::Types::Int64) ? literal->value.safeGet<Int64>() : literal->value.safeGet<UInt64>());
    if (fd < 0) throw Exception(ErrorCodes::BAD_ARGUMENTS, "File descriptor must be non-negative");
  } else
...
std::optional<String> TableFunctionFile::tryGetFormatFromFirstArgument() {
  if (fd >= 0) return FormatFactory::instance().tryGetFormatFromFileDescriptor(fd);
...
StoragePtr TableFunctionFile::getStorage(const String& /*source*/, const String& format_, const ColumnsDescription& columns,
                                         ContextPtr global_context, const std::string& table_name, const std::string& compression_method_,
  ...
  if (fd >= 0) return std::make_shared<StorageFile>(fd, args);
...
ColumnsDescription TableFunctionFile::getActualTableStructure(ContextPtr context, bool /*is_insert_query*/) const {
  if (structure == "auto") {
    if (fd >= 0)
      throw Exception(ErrorCodes::BAD_ARGUMENTS, "Schema inference is not supported for table function '{}' with file descriptor",
                      getName());
    ...
    if (format == "auto")
      columns = StorageFile::getTableStructureAndFormatFromFile(file_source->paths, compression_method, std::nullopt, context,
                                                                file_source->archive_info)
                    .first;
    else
      columns = StorageFile::getTableStructureFromFile(format, file_source->paths, compression_method, std::nullopt, context,
                                                       file_source->archive_info);

    auto sample_path = file_source->paths.empty() ? String{} : file_source->paths.front();

    HivePartitioningUtils::setupHivePartitioningForFileURLLikeStorage(columns, sample_path,
...
void registerTableFunctionFile(TableFunctionFactory& factory) { factory.registerFunction<TableFunctionFile>({}); }
