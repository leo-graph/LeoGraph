...
[[noreturn]] void throwNoTablesMatchRegexp(const String &source_database_regexp, const String &source_table_regexp) {
  throw Exception(
      ...
      source_database_regexp, source_database_regexp, source_table_regexp);
}
...
std::vector<size_t> TableFunctionMerge::skipAnalysisForArguments(const QueryTreeNodePtr &query_node_table_function, ContextPtr) const {
  ...
  for (size_t i = 0; i < table_function_arguments_size; ++i) {
    ...
    if (function_node && function_node->getFunctionName() == "REGEXP") result.push_back(i);
  }
...
void TableFunctionMerge::parseArguments(const ASTPtr &ast_function, ContextPtr context) {
  ...
  if (args.size() == 1) {
    ...
    if (!is_regexp) args[0] = database_ast;
...
ColumnsDescription TableFunctionMerge::getActualTableStructure(ContextPtr context, bool /*is_insert_query*/) const {
  ...
  if (res.empty()) throwNoTablesMatchRegexp(source_database_name_or_regexp, source_table_regexp);

  return res;
}

StoragePtr TableFunctionMerge::executeImpl(const ASTPtr & /*ast_function*/, ContextPtr context, const std::string &table_name,
                                           ColumnsDescription /*cached_columns*/, bool /*is_insert_query*/) const {
  auto res = std::make_shared<StorageMerge>(StorageID(getDatabaseName(), table_name), ColumnsDescription{}, String{},
                                            source_database_name_or_regexp, database_is_regexp, source_table_regexp, context);
...
void registerTableFunctionMerge(TableFunctionFactory &factory) {
  factory.registerFunction<TableFunctionMerge>(
      {.description = "Creates a temporary Merge table. The structure will be derived from underlying tables by using a union of their "
                      "columns and by deriving common types.",
