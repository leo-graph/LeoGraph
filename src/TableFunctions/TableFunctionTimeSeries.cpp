...
void TableFunctionTimeSeriesTarget<target_kind>::parseArguments(const ASTPtr &ast_function, ContextPtr context) {
  ...
  if (!args_func.arguments) throw Exception(ErrorCodes::LOGICAL_ERROR, "Table function '{}' must have arguments.", name);
  ...
  if (args.size() == 1) {
    ...
    if (const auto *id = args[0]->as<ASTIdentifier>()) {
      if (auto table_id = id->createTable()) time_series_storage_id = table_id->getTableId();
    }
  }
...
template <ViewTarget::Kind target_kind>
StoragePtr TableFunctionTimeSeriesTarget<target_kind>::executeImpl(const ASTPtr & /* ast_function */, ContextPtr context,
                                                                   const String & /* table_name */, ColumnsDescription /* cached_columns */,
                                                                   bool /* is_insert_query */) const {
  return getTargetTable(context);
}

template <ViewTarget::Kind target_kind>
ColumnsDescription TableFunctionTimeSeriesTarget<target_kind>::getActualTableStructure(ContextPtr context,
                                                                                       bool /* is_insert_query */) const {
...
void registerTableFunctionTimeSeries(TableFunctionFactory &factory) {
  ...
  factory.registerFunction<TableFunctionTimeSeriesSelector>(
      {.description = R"(Reads time series from a specified TimeSeries table.)",
       .examples{{"timeSeriesSelector",
                  "SELECT * from timeSeriesSelector('mydb', 'time_series_table', 'http_requests{job=\"prometheus\"}', now() - INTERVAL 10 "
                  "MINUTES, now());",
                  ""}},
       .category = FunctionDocumentation::Category::TableFunction});
  factory.registerFunction<TableFunctionPrometheusQuery</* range = */ false>>(
      {.description = R"(Evaluates a prometheus query using data from a specified TimeSeries table.)",
       .examples{{"prometheusQuery",
                  "SELECT * from prometheusQuery('mydb', 'time_series_table', 'rate(http_requests_total[5m])[30m:1m]', now());", ""}},
       .category = FunctionDocumentation::Category::TableFunction});
  factory.registerFunction<TableFunctionPrometheusQuery</* range = */ true>>(
      {.description = R"(Evaluates a prometheus query using data from a specified TimeSeries table over a range of timestamps.)",
       .examples{{"prometheusQueryRange",
                  "SELECT * from prometheusQueryRange('mydb', 'time_series_table', 'http_requests{job=\"prometheus\"}', now() - INTERVAL "
                  "10 MINUTES, now(), INTERVAL 1 MINUTE);",
                  ""}},
       .category = FunctionDocumentation::Category::TableFunction});
}
