...
class TableFunctionObjectStorageCluster
    : public ITableFunctionCluster<TableFunctionObjectStorage<Definition, Configuration, is_data_lake>> {
 public:
 ...
 private:
  static ContextPtr getQueryOrGlobalContext() {
    if (auto query_context = CurrentThread::getQueryContext(); query_context != nullptr) return query_context;
...
#if USE_AVRO
using TableFunctionIcebergLocalCluster =
    TableFunctionObjectStorageCluster<IcebergLocalClusterDefinition, StorageLocalIcebergConfiguration, true>;
#endif
...
using TableFunctionIcebergAzureCluster =
    TableFunctionObjectStorageCluster<IcebergAzureClusterDefinition, StorageAzureIcebergConfiguration, true>;
#endif

#if USE_AVRO && USE_HDFS
using TableFunctionIcebergHDFSCluster =
    TableFunctionObjectStorageCluster<IcebergHDFSClusterDefinition, StorageHDFSIcebergConfiguration, true>;
#endif
...
#if USE_AVRO && USE_AZURE_BLOB_STORAGE
using TableFunctionPaimonAzureCluster =
    TableFunctionObjectStorageCluster<PaimonAzureClusterDefinition, StorageAzurePaimonConfiguration, true>;
#endif
...
using TableFunctionDeltaLakeS3Cluster =
    TableFunctionObjectStorageCluster<DeltaLakeS3ClusterDefinition, StorageS3DeltaLakeConfiguration, true>;
#endif

#if USE_PARQUET && USE_AZURE_BLOB_STORAGE && USE_DELTA_KERNEL_RS
using TableFunctionDeltaLakeAzureCluster =
    TableFunctionObjectStorageCluster<DeltaLakeAzureClusterDefinition, StorageAzureDeltaLakeConfiguration, true>;
#endif
