...
void TableFunctionRemote::parseArguments(const ASTPtr& ast_function, ContextPtr context) {
  ...
  if (args_func.size() != 1) throw Exception(help_message, ErrorCodes::NUMBER_OF_ARGUMENTS_DOESNT_MATCH);
  ...
  if (!is_cluster_function && (named_collection = tryGetNamedCollectionWithOverrides(args, context, false, &complex_args))) {
    validateNamedCollection<ValidateKeysMultiset<ExternalDatabaseEqualKeysSet>>(
        *named_collection, {"addresses_expr", "host", "hostname", "table"},
    ...
    if (cluster_description.empty() && named_collection->hasAny({"host", "hostname"}))
      cluster_description = named_collection->has("port") ? named_collection->getAny<String>({"host", "hostname"}) + ':' +
                                                                toString(named_collection->get<UInt64>("port"))
    ...
    auto get_string_literal = [](const IAST& node, String& res) {
      ...
      if (lit->value.getType() != Field::Types::String) return false;
    ...
    if (arg_num < args.size()) {
      ...
      if (function && TableFunctionFactory::instance().isTableFunctionName(function->name)) {
        ...
        if (qualified_name.database.empty()) {
          if (arg_num >= args.size()) {
            throw Exception(ErrorCodes::NUMBER_OF_ARGUMENTS_DOESNT_MATCH, "Table name was not found in function arguments. {}",
                            static_cast<const std::string>(help_message));
          }
  ...
  if (!cluster_name.empty()) {
    ...
    for (const auto& shard : shards) {
      ...
      if (replicas.empty()) throw Exception(ErrorCodes::BAD_ARGUMENTS, "Shard contains zero number of replicas");
      names.push_back(std::move(replicas));
    }

    if (names.empty()) throw Exception(ErrorCodes::BAD_ARGUMENTS, "Shard list is empty after parsing the first argument");
      ...
      for (const auto& host : hosts) {
        ...
        if (colon == String::npos)
          context->getRemoteHostFilter().checkHostAndPort(
              host, toString((secure ? (maybe_secure_port ? *maybe_secure_port : DBMS_DEFAULT_SECURE_PORT) : context->getTCPPort())));
        else
  ...
  if (!remote_table_function_ptr && table.empty()) throw Exception(ErrorCodes::BAD_ARGUMENTS, "The name of remote table cannot be empty");

  remote_table_id.database_name = database;
  remote_table_id.table_name = table;
}

StoragePtr TableFunctionRemote::executeImpl(const ASTPtr& /*ast_function*/, ContextPtr context, const std::string& table_name,
  ...
  if (cached_columns.empty()) cached_columns = getActualTableStructure(context, is_insert_query);
  ...
  StoragePtr res = std::make_shared<StorageDistributed>(
      StorageID(getDatabaseName(), table_name), cached_columns, ConstraintsDescription{}, String{}, remote_table_id.database_name,
      remote_table_id.table_name, String{}, context, sharding_key, String{}, String{}, DistributedSettings{},
      LoadingStrictnessLevel::CREATE, cluster, remote_table_function_ptr, !is_cluster_function);
...
TableFunctionRemote::TableFunctionRemote(const std::string& name_, bool secure_) : name{name_}, secure{secure_} {
  ...
  help_message = PreformattedMessage::create(
      ...
      "{}",
      name, is_cluster_function ? 0 : 1, is_cluster_function ? 4 : 6,
      is_cluster_function
          ? "[<cluster name or default if not specify>, <name of remote database>, <name of remote table>] [, sharding_key]"
...
void registerTableFunctionRemote(TableFunctionFactory& factory) {
  ...
  factory.registerFunction("remoteSecure",
                           {[]() -> TableFunctionPtr { return std::make_shared<TableFunctionRemote>("remote", /* secure = */ true); }, {}});
  factory.registerFunction(
      "cluster", {[]() -> TableFunctionPtr { return std::make_shared<TableFunctionRemote>("cluster"); }, {}, {.allow_readonly = true}});
  factory.registerFunction(
      "clusterAllReplicas",
      {[]() -> TableFunctionPtr { return std::make_shared<TableFunctionRemote>("clusterAllReplicas"); }, {}, {.allow_readonly = true}});
}
