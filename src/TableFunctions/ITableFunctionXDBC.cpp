...
class ITableFunctionXDBC : public ITableFunction {
  ...
  virtual BridgeHelperPtr createBridgeHelper(ContextPtr context, Poco::Timespan http_timeout_, const std::string &connection_string_,
...
class TableFunctionJDBC : public ITableFunctionXDBC {
 ...
 private:
  BridgeHelperPtr createBridgeHelper(ContextPtr context, Poco::Timespan http_timeout_, const std::string &connection_string_,
...
void ITableFunctionXDBC::parseArguments(const ASTPtr &ast_function, ContextPtr context) {
  ...
  if (!args_func.arguments) throw Exception(ErrorCodes::LOGICAL_ERROR, "Table function '{}' must have arguments.", getName());
  ...
  if (args.empty() || args.size() > 3)
    throw Exception(
        ...
        "Table function '{0}' requires 1, 2 or 3 arguments: {0}(named_collection) or {0}('DSN', table) or {0}('DSN', schema, table)",
        getName());
  ...
  if (auto named_collection = tryGetNamedCollectionWithOverrides(ast_function->children.at(0)->children, context)) {
    if (Poco::toLower(getName()) == "jdbc") {
      ...
      if (named_collection->has("external_table") && named_collection->has("table"))
        throw Exception(ErrorCodes::BAD_ARGUMENTS, "Table function '{0}' cannot have `external_table` and `table` arguments simultaneously",
                        getName());
      remote_table_name = named_collection->getAnyOrDefault<String>({"external_table", "table"}, "");
    } else {
      validateNamedCollection<>(*named_collection, {},
                                {"datasource", "connection_settings",  // Aliases
                                 "external_database", "external_table"});
    ...
    throw Exception(ErrorCodes::BAD_ARGUMENTS, "Table function '{0}' has 1 argument, it is expected to be named collection", getName());
  } else {
...
void ITableFunctionXDBC::startBridgeIfNot(ContextPtr context) const {
  if (!helper) {
    helper = createBridgeHelper(context, context->getSettingsRef()[Setting::http_receive_timeout].value, connection_string,
...
ColumnsDescription ITableFunctionXDBC::getActualTableStructure(ContextPtr context, bool /*is_insert_query*/) const {
  ...
  if (!schema_name.empty()) columns_info_uri.addQueryParameter("schema", schema_name);
...
StoragePtr ITableFunctionXDBC::executeImpl(const ASTPtr & /*ast_function*/, ContextPtr context, const std::string &table_name,
                                           ColumnsDescription /*cached_columns*/, bool is_insert_query) const {
  startBridgeIfNot(context);
  auto columns = getActualTableStructure(context, is_insert_query);
  auto result = std::make_shared<StorageXDBC>(StorageID(getDatabaseName(), table_name), schema_name, remote_table_name, columns,
                                              ConstraintsDescription{}, String{}, context, helper);
  result->startup();
  return result;
}
...
void registerTableFunctionJDBC(TableFunctionFactory &factory) { factory.registerFunction<TableFunctionJDBC>({}); }

void registerTableFunctionODBC(TableFunctionFactory &factory) { factory.registerFunction<TableFunctionODBC>({}); }
}  // namespace DB
