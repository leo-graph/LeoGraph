...
StoragePtr TableFunctionMongoDB::executeImpl(const ASTPtr & /*ast_function*/, ContextPtr context, const String &table_name,
                                             ColumnsDescription /*cached_columns*/, bool is_insert_query) const {
  auto columns = getActualTableStructure(context, is_insert_query);
  auto storage = std::make_shared<StorageMongoDB>(StorageID(getDatabaseName(), table_name), std::move(*configuration), columns,
...
void TableFunctionMongoDB::parseArguments(const ASTPtr &ast_function, ContextPtr context) {
  ...
  if (!func_args.arguments) throw Exception(ErrorCodes::BAD_ARGUMENTS, "Table function 'mongodb' must have arguments.");

  ASTs &args = func_args.arguments->children;

  if (auto named_collection = tryGetNamedCollectionWithOverrides(args, context)) {
    if (!named_collection->has("structure")) throw Exception(ErrorCodes::BAD_ARGUMENTS, "Required key (structure) is not specified.");
  ...
  if ((args.size() < 3 || args.size() > 4) && (args.size() < 6 || args.size() > 8))
    throw Exception(ErrorCodes::NUMBER_OF_ARGUMENTS_DOESNT_MATCH,
                    ...
                    "mongodb('host:port', database, collection, user, password, structure[, options[, oid_columns]]) or mongodb(uri, "
                    "collection, structure[, oid_columns]).",
                    getName());
...
std::pair<String, ASTPtr> getKeyValueMongoDBArgument(const ASTFunction *ast_func) {
  ...
  if (arg_name == "structure" || arg_name == "options") return std::make_pair(arg_name, function_args[1]);
...
void registerTableFunctionMongoDB(TableFunctionFactory &factory) {
  factory.registerFunction<TableFunctionMongoDB>(
      {.description = "Allows get data from MongoDB collection.",
       .examples =
           {
               {"Fetch collection by URI",
                "SELECT * FROM mongodb('mongodb://root:clickhouse@localhost:27017/database', 'example_collection', 'key UInt64, data "
                "String')",
                ""},
               {"Fetch collection over TLS",
                "SELECT * FROM mongodb('localhost:27017', 'database', 'example_collection', 'root', 'clickhouse', 'key UInt64, data "
                "String', 'tls=true')",
                ""},
               {"Fetch collection by named collection configuration with overrides",
                "SELECT * FROM mongodb(mongodb_creds, collection='example_collection', structure='key UInt64, data String')", ""},
           },
