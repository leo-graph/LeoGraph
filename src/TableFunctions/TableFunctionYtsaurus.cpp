...
StoragePtr TableFunctionYTsaurus::executeImpl(const ASTPtr & /*ast_function*/, ContextPtr context, const String &table_name,
  ...
  if (!context->getSettingsRef()[Setting::allow_experimental_ytsaurus_table_function])
    throw Exception(ErrorCodes::UNKNOWN_FUNCTION,
                    "Table function ytsaurus is experimental."
                    "Set `allow_experimental_ytsaurus_table_function` setting to enable it");

  auto columns = getActualTableStructure(context, is_insert_query);
  auto storage = std::make_shared<StorageYTsaurus>(StorageID(getDatabaseName(), table_name), std::move(*configuration), columns,
...
void TableFunctionYTsaurus::parseArguments(const ASTPtr &ast_function, ContextPtr context) {
  ...
  if (!func_args.arguments) throw Exception(ErrorCodes::BAD_ARGUMENTS, "Table function 'ytsaurus' must have arguments.");
