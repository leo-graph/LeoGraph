...
std::vector<String> extractParts(const ASTPtr &argument, const ContextPtr &context) {
  ...
  if (array) {
    if (const auto *literal = array->as<ASTLiteral>()) {
      ...
      for (const auto &element : literal->value.safeGet<Array>()) result.push_back(element.safeGet<String>());
      return result;
    }
...
class TableFunctionMergeTreeAnalyzeIndexes : public ITableFunction {
 public:
  explicit TableFunctionMergeTreeAnalyzeIndexes(bool resolve_by_uuid_) : resolve_by_uuid(resolve_by_uuid_) {}
...
std::vector<size_t> TableFunctionMergeTreeAnalyzeIndexes::skipAnalysisForArguments(const QueryTreeNodePtr & /* query_node_table_function */,
...
void TableFunctionMergeTreeAnalyzeIndexes::parseArguments(const ASTPtr &ast_function, ContextPtr context) {
  ...
  if (args_func.size() != 1) throw Exception(ErrorCodes::LOGICAL_ERROR, "Table function ({}) must have arguments.", quoteString(getName()));
...
void TableFunctionMergeTreeAnalyzeIndexes::parseArgumentsUUID(const ASTs &args_func, ContextPtr context) {
  ...
  if (args.empty() || args.size() > 5)
    throw Exception(
        ...
        "Table function '{}' must have from 1 to 3 or 5 arguments (UUID, condition[, parts_array], [, optimization, args_array]), got: {}",
        getName(), args.size());
  ...
  if (args.size() > 1) predicate = args[1]->clone();

  if (args.size() > 2) parts = extractParts(args[2], context);

  if (args.size() > 3) parseArgumentsForOptimizations(args, context);
...
void TableFunctionMergeTreeAnalyzeIndexes::parseArgumentsDatabaseTable(const ASTs &args_func, ContextPtr context) {
  ...
  if (args.size() < 2 || args.size() > 4)
    throw Exception(ErrorCodes::NUMBER_OF_ARGUMENTS_DOESNT_MATCH,
                    "Table function '{}' must have from 2 to 4 arguments (database, table, condition[, parts_array]), got: {}", getName(),
                    args.size());
  ...
  if (args.size() > 2) predicate = args[2]->clone();

  if (args.size() > 3) parts = extractParts(args[3], context);
  ...
  if (args.size() == 5) {
    ...
    if (optimization == "vector_search_index_analysis") {
      ...
      if (vector_search_args.size() != 6)
        throw Exception(ErrorCodes::NUMBER_OF_ARGUMENTS_DOESNT_MATCH, "vector_search_index_analysis requires 6 arguments");
...
StoragePtr TableFunctionMergeTreeAnalyzeIndexes::executeImpl(const ASTPtr & /*ast_function*/, ContextPtr context,
                                                             const std::string &table_name, ColumnsDescription /*cached_columns*/,
  ...
  if (source_table_id.hasUUID()) {
    ...
    if (!source_table) throw Exception(ErrorCodes::UNKNOWN_TABLE, "Table with UUID {} does not exist", source_table_id.uuid);
  } else
  ...
  auto res = std::make_shared<StorageMergeTreeAnalyzeIndexes>(std::move(storage_id), std::move(source_table), std::move(columns), parts,
                                                              predicate, vector_search_parameters);
  res->startup();
  return res;
}
...
void registerTableFunctionMergeTreeAnalyzeIndexes(TableFunctionFactory &factory) {
  factory.registerFunction(
      mergeTreeAnalyzeIndexFunctionName(/*resolve_by_uuid=*/false),
      TableFunctionFactoryData{
          ...
          {.description = "Internal function for index analysis",
           .examples = {{"mergeTreeAnalyzeIndexes",
                         "SELECT * FROM mergeTreeAnalyzeIndexes(currentDatabase(), mt_table, predicate[, ['part1', 'part2']])", ""}},
           .category = FunctionDocumentation::Category::TableFunction},
          {.allow_readonly = true}});

  factory.registerFunction(
      mergeTreeAnalyzeIndexFunctionName(/*resolve_by_uuid=*/true),
      TableFunctionFactoryData{
          []() { return std::make_shared<TableFunctionMergeTreeAnalyzeIndexes>(/* resolve_by_uuid_= */ true); },
          {.description = "Internal function for index analysis",
           .examples = {{"mergeTreeAnalyzeIndexes",
                         "SELECT * FROM mergeTreeAnalyzeIndexesUUID('table_uuid', predicate[, ['part1', 'part2']])", ""}},
           .category = FunctionDocumentation::Category::TableFunction},
          {.allow_readonly = true}});
}
