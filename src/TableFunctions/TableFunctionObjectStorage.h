...
class TableFunctionObjectStorage : public ITableFunction {
 public:
  ...
  using Settings = typename std::conditional_t<is_data_lake, DataLakeStorageSettings, StorageObjectStorageSettings>;
  ...
  static void updateStructureAndFormatArgumentsIfNeeded(ASTs& args, const String& structure, const String& format,
    ...
    if constexpr (is_data_lake) {
      ...
      if (configuration.format == "auto") configuration.format = "Parquet";  /// Default format of data lakes.
  ...
  void setPartitionBy(const ASTPtr& partition_by_) override { partition_by = partition_by_; }
