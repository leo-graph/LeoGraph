...
ColumnsDescription TableFunctionMergeTreeTextIndex::getActualTableStructure(ContextPtr, bool /*is_insert_query*/) const {
  return ColumnsDescription{{{"part_name", std::make_shared<DataTypeString>()},
...
StoragePtr TableFunctionMergeTreeTextIndex::executeImpl(const ASTPtr& /*ast_function*/, ContextPtr context, const std::string& table_name,
  ...
  if (index_desc.type != "text")
    throw Exception(ErrorCodes::BAD_ARGUMENTS, "Got index '{}' of type '{}', expected 'text'", source_index_name, index_desc.type);

  auto text_index = MergeTreeIndexFactory::instance().get(index_desc);
  auto columns = getActualTableStructure(context, is_insert_query);
  StorageID storage_id(getDatabaseName(), table_name);

  auto res = std::make_shared<StorageMergeTreeTextIndex>(std::move(storage_id), std::move(source_table_ptr), std::move(text_index),
                                                         std::move(columns));
