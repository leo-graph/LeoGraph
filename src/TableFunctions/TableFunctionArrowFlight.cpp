...
StoragePtr TableFunctionArrowFlight::executeImpl(const ASTPtr & /*ast_function*/, ContextPtr context, const String &table_name,
                                                 ColumnsDescription cached_columns, bool /*is_insert_query*/) const {
  return std::make_shared<StorageArrowFlight>(StorageID{"arrow_flight", table_name}, connection, config.dataset_name, cached_columns,
                                              ConstraintsDescription{}, context);
}
...
void TableFunctionArrowFlight::parseArguments(const ASTPtr &ast_function, ContextPtr context) {
  ...
  if (!func_args.arguments) throw Exception(ErrorCodes::BAD_ARGUMENTS, "Table function 'arrowFlight' must have arguments.");
