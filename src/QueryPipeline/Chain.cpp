...
static void checkSingleInput(const IProcessor& transform) {
  if (transform.getInputs().size() != 1)
    throw Exception(ErrorCodes::LOGICAL_ERROR, "Transform for chain should have single input, but {} has {} inputs", transform.getName(),
                    transform.getInputs().size());

  if (transform.getInputs().front().isConnected()) throw Exception(ErrorCodes::LOGICAL_ERROR, "Transform for chain has connected input");
}

static void checkSingleOutput(const IProcessor& transform) {
  if (transform.getOutputs().size() != 1)
    throw Exception(ErrorCodes::LOGICAL_ERROR, "Transform for chain should have single output, but {} has {} outputs", transform.getName(),
                    transform.getOutputs().size());

  if (transform.getOutputs().front().isConnected()) throw Exception(ErrorCodes::LOGICAL_ERROR, "Transform for chain has connected output");
}
...
static void checkInitialized(const std::list<ProcessorPtr>& processors) {
  if (processors.empty()) throw Exception(ErrorCodes::LOGICAL_ERROR, "Chain is not initialized");
}
  ...
  for (const auto& processor : processors) {
    for (const auto& input : processor->getInputs())
      if (&input != &getInputPort() && !input.isConnected())
        throw Exception(ErrorCodes::LOGICAL_ERROR, "Cannot initialize chain because there is a disconnected input for {}",
                        processor->getName());

    for (const auto& output : processor->getOutputs())
      if (&output != &getOutputPort() && !output.isConnected())
        throw Exception(ErrorCodes::LOGICAL_ERROR, "Cannot initialize chain because there is a disconnected output for {}",
                        processor->getName());
  }
}
...
void Chain::addSource(ProcessorPtr processor) {
  ...
  if (!processors.empty()) connect(processor->getOutputs().front(), getInputPort());
...
void Chain::addSink(ProcessorPtr processor) {
  ...
  if (!processors.empty()) connect(getOutputPort(), processor->getInputs().front());
...
Chain Chain::concat(Chain lhs, Chain rhs) {
  ...
  if (lhs.processors.empty()) return rhs;
...
const Block& Chain::getInputHeader() const { return getInputPort().getHeader(); }

const SharedHeader& Chain::getInputSharedHeader() const { return getInputPort().getSharedHeader(); }

const Block& Chain::getOutputHeader() const { return getOutputPort().getHeader(); }

const SharedHeader& Chain::getOutputSharedHeader() const { return getOutputPort().getSharedHeader(); }
