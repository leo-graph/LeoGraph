...
bool SizeLimits::check(UInt64 rows, UInt64 bytes, const char* what, int too_many_rows_exception_code,
                       int too_many_bytes_exception_code) const {
  if (overflow_mode == OverflowMode::THROW) {
    if (max_rows && rows > max_rows) {
      ProfileEvents::increment(ProfileEvents::OverflowThrow);
      throw Exception(too_many_rows_exception_code, "Limit for {} exceeded, max rows: {}, current rows: {}", what,
                      formatReadableQuantity(max_rows), formatReadableQuantity(rows));
    }

    if (max_bytes && bytes > max_bytes) {
      ProfileEvents::increment(ProfileEvents::OverflowThrow);
      throw Exception(too_many_bytes_exception_code, "Limit for {} exceeded, max bytes: {}, current bytes: {}", what,
                      ReadableSize(max_bytes), ReadableSize(bytes));
    }
...
bool SizeLimits::softCheck(UInt64 rows, UInt64 bytes) const {
  ...
  if ((max_rows && rows >= max_rows) || (max_bytes && bytes >= max_bytes)) {
...
static void checkAllowedOwerflowMode(OverflowMode mode, int code) {
  if (!(mode == OverflowMode::BREAK || mode == OverflowMode::THROW)) throw Exception(code, "Unexpected overflow mode {}", mode);
}
