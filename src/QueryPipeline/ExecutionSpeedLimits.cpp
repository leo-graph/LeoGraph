...
void ExecutionSpeedLimits::throttle(size_t read_rows, size_t read_bytes, size_t total_rows_to_read, UInt64 total_elapsed_microseconds,
                                    OverflowMode timeout_overflow_mode) const {
  if ((min_execution_rps != 0 || max_execution_rps != 0 || min_execution_bps != 0 || max_execution_bps != 0 ||
    ...
    if (elapsed_seconds > 0) {
      ...
      if (min_execution_rps && rows_per_second < static_cast<double>(min_execution_rps))
        throw Exception(ErrorCodes::TOO_SLOW, "Query is executing too slow: {} rows/sec., minimum: {}", rows_per_second, min_execution_rps);
      ...
      if (min_execution_bps && bytes_per_second < static_cast<double>(min_execution_bps))
        throw Exception(ErrorCodes::TOO_SLOW, "Query is executing too slow: {} bytes/sec., minimum: {}", bytes_per_second,
                        min_execution_bps);
      ...
      if (timeout_overflow_mode == OverflowMode::THROW && max_estimated_execution_time != 0 && total_rows_to_read && read_rows) {
        ...
        if (read_rows > rows_threshold || elapsed_seconds > time_threshold) {
          double estimated_execution_time_seconds =
              elapsed_seconds * (static_cast<double>(total_rows_to_read) / static_cast<double>(read_rows));

          if (estimated_execution_time_seconds > max_estimated_execution_time.totalSeconds())
            throw Exception(ErrorCodes::TOO_SLOW,
                            "Estimated query execution time ({:.5f} seconds) is too long. Maximum: {}. Estimated rows to process: {} ({} "
                            "read in {:.5f} seconds).",
                            estimated_execution_time_seconds, max_estimated_execution_time.totalSeconds(), total_rows_to_read, read_rows,
                            elapsed_seconds);
        }
      }
...
bool ExecutionSpeedLimits::checkTimeLimit(const UInt64 &elapsed_ns, OverflowMode overflow_mode) const {
  if (max_execution_time != 0) {
    if (elapsed_ns > static_cast<UInt64>(max_execution_time.totalMicroseconds()) * 1000)
      return handleOverflowMode(overflow_mode, ErrorCodes::TIMEOUT_EXCEEDED, "Timeout exceeded: elapsed {} ms, maximum: {} ms",
                                static_cast<double>(elapsed_ns) / 1000000ULL, max_execution_time.totalMilliseconds());
  }
