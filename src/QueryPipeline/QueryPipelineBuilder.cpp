...
void QueryPipelineBuilder::checkInitialized() {
  if (!initialized()) throw Exception(ErrorCodes::LOGICAL_ERROR, "QueryPipeline is uninitialized");
}

void QueryPipelineBuilder::checkInitializedAndNotCompleted() {
  checkInitialized();

  if (pipe.isCompleted()) throw Exception(ErrorCodes::LOGICAL_ERROR, "QueryPipeline is already completed");
}

static void checkSource(const ProcessorPtr &source, bool can_have_totals) {
  if (!source->getInputs().empty())
    throw Exception(ErrorCodes::LOGICAL_ERROR, "Source for query pipeline shouldn't have any input, but {} has {} inputs",
                    source->getName(), source->getInputs().size());

  if (source->getOutputs().empty())
    throw Exception(ErrorCodes::LOGICAL_ERROR, "Source for query pipeline should have single output, but {} doesn't have any",
                    source->getName());

  if (!can_have_totals && source->getOutputs().size() != 1)
    throw Exception(ErrorCodes::LOGICAL_ERROR, "Source for query pipeline should have single output, but {} has {} outputs",
                    source->getName(), source->getOutputs().size());

  if (source->getOutputs().size() > 2)
    throw Exception(ErrorCodes::LOGICAL_ERROR, "Source for query pipeline should have 1 or 2 output, but {} has {} outputs",
                    source->getName(), source->getOutputs().size());
}

void QueryPipelineBuilder::init(Pipe pipe_) {
  if (initialized()) throw Exception(ErrorCodes::LOGICAL_ERROR, "Pipeline has already been initialized");

  if (pipe_.empty()) throw Exception(ErrorCodes::LOGICAL_ERROR, "Can't initialize pipeline with empty pipe");
...
void QueryPipelineBuilder::addTotalsHavingTransform(ProcessorPtr transform) {
  ...
  if (pipe.getTotalsPort()) throw Exception(ErrorCodes::LOGICAL_ERROR, "Totals having transform was already added to pipeline");
...
void QueryPipelineBuilder::addExtremesTransform() {
  ...
  if (pipe.getExtremesPort()) pipe.dropExtremes();
...
QueryPipelineBuilder QueryPipelineBuilder::unitePipelines(std::vector<std::unique_ptr<QueryPipelineBuilder>> pipelines,
                                                          size_t max_threads_limit, Processors *collected_processors) {
  if (pipelines.empty()) throw Exception(ErrorCodes::LOGICAL_ERROR, "Cannot unite an empty set of pipelines");
...
QueryPipelineBuilderPtr QueryPipelineBuilder::mergePipelines(QueryPipelineBuilderPtr left, QueryPipelineBuilderPtr right,
                                                             ProcessorPtr transform, Processors *collected_processors) {
...
std::unique_ptr<QueryPipelineBuilder> QueryPipelineBuilder::joinPipelinesYShaped(std::unique_ptr<QueryPipelineBuilder> left,
                                                                                 std::unique_ptr<QueryPipelineBuilder> right, JoinPtr join,
                                                                                 SharedHeader &out_header, size_t max_block_size,
...
std::unique_ptr<QueryPipelineBuilder> QueryPipelineBuilder::joinPipelinesYShapedByShards(std::unique_ptr<QueryPipelineBuilder> left,
                                                                                         std::unique_ptr<QueryPipelineBuilder> right,
                                                                                         JoinPtr join, SharedHeader &out_header,
  ...
  if (left->getNumStreams() != right->getNumStreams())
    throw Exception(ErrorCodes::LOGICAL_ERROR, "Join by layers is supported only for pipelines equal number of output ports, got {} and {}",
                    left->getNumStreams(), right->getNumStreams());
  ...
  for (size_t i = 0; i < num_streams; ++i) {
    auto joining = std::make_shared<MergeJoinTransform>(
        join->clone(std::make_shared<TableJoin>(join->getTableJoin()), left->getSharedHeader(), right->getSharedHeader()), inputs,
        out_header, max_block_size);
    ...
    if (collected_processors) collected_processors->emplace_back(joining);
...
std::unique_ptr<QueryPipelineBuilder> QueryPipelineBuilder::joinPipelinesRightLeft(
    std::unique_ptr<QueryPipelineBuilder> left, std::unique_ptr<QueryPipelineBuilder> right, JoinPtr join, SharedHeader &output_header,
    size_t max_block_size, size_t min_block_size_rows, size_t min_block_size_bytes, size_t max_streams, bool keep_left_read_in_order,
    Processors *collected_processors) {
  left->checkInitializedAndNotCompleted();
  right->checkInitializedAndNotCompleted();
    ...
    auto concurrent_right_filling_transform = [&](OutputPortRawPtrs outports) {
      ...
      if (min_block_size_rows > 0 || min_block_size_bytes > 0) {
        for (auto &outport : outports) {
          auto squashing =
              std::make_shared<SimpleSquashingChunksTransform>(right->getSharedHeader(), min_block_size_rows, min_block_size_bytes);
  ...
  if (join->hasDelayedBlocks()) {
    ...
    if (collected_processors) collected_processors->emplace_back(delayed_root);
    left->pipe.processors->emplace_back(delayed_root);

    for (auto &outport : delayed_root->getOutputs()) delayed_root_output_ports.emplace_back(&outport);
  }
  ...
  auto joining_finish_counter = use_parallel_non_joined ? nullptr : std::make_shared<FinishCounter>(num_streams);
  ...
  for (size_t i = 0; i < num_streams; ++i) {
    ...
    auto joining =
        std::make_shared<JoiningTransform>(left_header, output_header, join, max_block_size, false, default_totals, joining_finish_counter);
    ...
    if (delayed_root) {
      ...
      auto delayed =
          std::make_shared<DelayedJoinedBlocksWorkerTransform>(output_header, [left_header, output_header, max_block_size, join]() {
      ...
      if (collected_processors) collected_processors->emplace_back(delayed);
      ...
      auto non_joined = std::make_shared<NonJoinedBlocksTransform>(output_header, join, *left_header, max_block_size, i, num_streams);

      joined_output_ports.push_back(&joining->getOutputs().front());
      joined_output_ports.push_back(&non_joined->getPort());

      if (collected_processors) collected_processors->emplace_back(non_joined);
  ...
  if (delayed_root || use_parallel_non_joined) {
    ...
    for (size_t i = 1; i < joined_output_ports.size(); i += 2) delayed_ports_numbers.push_back(i);

    auto delayed_processor = std::make_shared<DelayedPortsProcessor>(output_header, 2 * num_streams, delayed_ports_numbers);
    if (collected_processors) collected_processors->emplace_back(delayed_processor);
    ...
    for (OutputPort *port : joined_output_ports) connect(*port, *next_delayed_input++);
    left->pipe.output_ports.clear();
    for (OutputPort &port : delayed_processor->getOutputs()) left->pipe.output_ports.push_back(&port);
  ...
  if (step) step->appendExtraProcessors(processors);
...
std::unique_ptr<QueryPipelineBuilder> QueryPipelineBuilder::joinPipelinesByShards(std::unique_ptr<QueryPipelineBuilder> left,
                                                                                  std::unique_ptr<QueryPipelineBuilder> right, JoinPtr join,
                                                                                  SharedHeader &output_header, size_t max_block_size,
                                                                                  Processors *collected_processors) {
  left->checkInitializedAndNotCompleted();
  right->checkInitializedAndNotCompleted();
  ...
  if (!join->isCloneSupported()) throw Exception(ErrorCodes::LOGICAL_ERROR, "Clone is not supported for {}", join->getName());
  if (join->hasDelayedBlocks())
  ...
  if (right->getNumStreams() != num_streams)
    throw Exception(ErrorCodes::LOGICAL_ERROR,
                    "Join by layers expects the same number of ports for the left and the right pipeline. Actual: {} and {}.", num_streams,
                    right->getNumStreams());
  ...
  for (size_t i = 0; i < num_streams; ++i) {
    auto finish_counter = std::make_shared<FinishCounter>(1);
    auto joining = std::make_shared<JoiningTransform>(left_header, output_header, joins[i], max_block_size, false, false, finish_counter);
...
void QueryPipelineBuilder::addCreatingSetsTransform(SharedHeader res_header, SetAndKeyPtr set_and_key, const SizeLimits &limits,
                                                    PreparedSetsCachePtr prepared_sets_cache) {
  dropTotalsAndExtremes();
  resize(1);

  auto transform = std::make_shared<CreatingSetsTransform>(getSharedHeader(), res_header, std::move(set_and_key), limits,
...
void QueryPipelineBuilder::addPipelineBefore(QueryPipelineBuilder pipeline) {
  ...
  auto processor =
      std::make_shared<DelayedPortsProcessor>(getSharedHeader(), pipe.numOutputPorts() + num_extra_ports, delayed_streams, true);
...
void QueryPipelineBuilder::setProcessListElement(QueryStatusPtr elem) { process_list_element = elem; }

void QueryPipelineBuilder::setProgressCallback(ProgressCallback callback) { progress_callback = callback; }

PipelineExecutorPtr QueryPipelineBuilder::execute() {
  if (!isCompleted()) throw Exception(ErrorCodes::LOGICAL_ERROR, "Cannot execute pipeline because it is not completed");
...
void QueryPipelineBuilder::setCollectedProcessors(Processors *processors) { pipe.collected_processors = processors; }
...
QueryPipelineProcessorsCollector::~QueryPipelineProcessorsCollector() { pipeline.setCollectedProcessors(nullptr); }

Processors QueryPipelineProcessorsCollector::detachProcessors(size_t group) {
  for (auto &processor : processors) processor->setQueryPlanStep(step, group);
