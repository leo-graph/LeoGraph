...
static Block getBlockWithSize(const std::vector<std::string>& columns, size_t rows) {
  ...
  for (const auto& column : columns) {
    ...
    for (size_t j = 0; j < rows; ++j) column_ptr->getElement(j) = j;
...
[[maybe_unused]] static void testSplitResizeTransform(size_t instreams, size_t outstreams, size_t min_outstreams_per_resize_after_split,
                                                      bool strict) {
