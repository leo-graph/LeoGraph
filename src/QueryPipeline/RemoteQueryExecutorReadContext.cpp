...
RemoteQueryExecutorReadContext::RemoteQueryExecutorReadContext(RemoteQueryExecutor& executor_, bool suspend_when_query_sent_,
  ...
  if (-1 == pipe2(pipe_fd, O_NONBLOCK)) throw ErrnoException(ErrorCodes::CANNOT_OPEN_FILE, "Cannot create pipe");
...
bool RemoteQueryExecutorReadContext::checkBeforeTaskResume() { return !is_in_progress.load(std::memory_order_relaxed) || checkTimeout(); }
...
void RemoteQueryExecutorReadContext::Task::run(AsyncCallback async_callback, SuspendCallback suspend_callback) {
  ...
  if (read_context.suspend_when_query_sent) suspend_callback();

  if (read_context.executor.needToSkipUnavailableShard()) return;
...
void RemoteQueryExecutorReadContext::processAsyncEvent(int fd, Poco::Timespan socket_timeout, AsyncEventTimeoutType type,
...
bool RemoteQueryExecutorReadContext::checkTimeout(bool blocking) {
  ...
  for (size_t i = 0; i < num_events; ++i) {
    if (events[i].data.fd == connection_fd) is_socket_ready = true;
    if (events[i].data.fd == timer.getDescriptor()) is_timer_alarmed = true;
    if (events[i].data.fd == pipe_fd[0]) is_pipe_alarmed = true;
  }

  if (is_pipe_alarmed) return false;
...
void RemoteQueryExecutorReadContext::cancelBefore() {
  ...
  if (!is_timer_alarmed) {
    ...
    if (!is_query_sent) suspend_when_query_sent = true;
  ...
  while (-1 == write(pipe_fd[1], &buf, sizeof(buf))) {
    ...
    if (errno != EINTR) throw ErrnoException(ErrorCodes::CANNOT_READ_FROM_SOCKET, "Cannot write to pipe");
  }
}
