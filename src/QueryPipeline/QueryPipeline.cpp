...
QueryPipeline::QueryPipeline() : processors(std::make_shared<Processors>()) {}
...
static void checkInput(const InputPort &input, const ProcessorPtr &processor) {
  if (!input.isConnected())
    throw Exception(ErrorCodes::LOGICAL_ERROR, "Cannot create QueryPipeline because {} has disconnected input", processor->getName());
}

static void checkOutput(const OutputPort &output, const ProcessorPtr &processor, const Processors &processors = {}) {
  if (!output.isConnected()) {
    WriteBufferFromOwnString out;
    if (!processors.empty()) printPipeline(processors, out);

    throw Exception(ErrorCodes::LOGICAL_ERROR, "Cannot create QueryPipeline because {} {} has disconnected output: {}",
                    processor->getName(), processor->getDescription(), out.str());
  }
}

static void checkPulling(Processors &processors, OutputPort *output, OutputPort *totals, OutputPort *extremes) {
  if (!output || output->isConnected())
    throw Exception(ErrorCodes::LOGICAL_ERROR, "Cannot create pulling QueryPipeline because its output port is connected or null");

  if (totals && totals->isConnected())
    throw Exception(ErrorCodes::LOGICAL_ERROR, "Cannot create pulling QueryPipeline because its totals port is connected");

  if (extremes && extremes->isConnected())
    throw Exception(ErrorCodes::LOGICAL_ERROR, "Cannot create pulling QueryPipeline because its extremes port is connected");

  bool found_output = false;
  bool found_totals = false;
  bool found_extremes = false;
  for (const auto &processor : processors) {
    for (const auto &in : processor->getInputs()) checkInput(in, processor);
...
static void checkCompleted(Processors &processors) {
  for (const auto &processor : processors) {
    ...
    for (const auto &out : processor->getOutputs()) checkOutput(out, processor);
  }
}
...
static void initRowsBeforeLimit(IOutputFormat *output_format) {
  ...
  while (!queue.empty()) {
    ...
    if (auto *format = dynamic_cast<IOutputFormat *>(processor)) {
      ...
      if (visited.emplace(child_processor).second) queue.push({child_processor, limit_processor, limit_input_port});

      continue;
    }

    /// Skip CreatingSetsTransform
    if (typeid_cast<CreatingSetsTransform *>(processor)) continue;
    ...
    if (limit_processor == processor) {
      ...
      for (auto &child_port : processor->getInputs()) {
        ...
        if (visited.emplace(child_processor).second) queue.push({child_processor, limit_processor, i});
        ++i;
      }
    } else {
      for (auto &child_port : processor->getInputs()) {
  ...
  for (auto &&[limit, ports] : limit_candidates) {
    ...
    if (ports.size() < limit->getInputs().size()) {
      ...
      for (auto port : ports) limit->setInputPortHasCounter(port);
    }
  }

  if (!processors.empty()) {
    rows_before_limit_at_least = std::make_shared<RowsBeforeStepCounter>();
    for (auto &processor : processors) processor->setRowsBeforeLimitCounter(rows_before_limit_at_least);
    ...
    if (has_limit) rows_before_limit_at_least->add(0);
...
static void initRowsBeforeAggregation(std::shared_ptr<Processors> processors, IOutputFormat *output_format) {
  ...
  if (!processors->empty()) {
    ...
    if (has_aggregation) rows_before_aggregation->add(0);
    output_format->setRowsBeforeAggregationCounter(rows_before_aggregation);
  }
}

QueryPipeline::QueryPipeline(QueryPlanResourceHolder resources_, std::shared_ptr<Processors> processors_)
    : resources(std::move(resources_)), processors(std::move(processors_)) {
  checkCompleted(*processors);
}

QueryPipeline::QueryPipeline(QueryPlanResourceHolder resources_, std::shared_ptr<Processors> processors_, InputPort *input_)
    : resources(std::move(resources_)), processors(std::move(processors_)), input(input_) {
  if (!input || input->isConnected())
    throw Exception(ErrorCodes::LOGICAL_ERROR, "Cannot create pushing QueryPipeline because its input port is connected or null");
...
QueryPipeline::QueryPipeline(QueryPlanResourceHolder resources_, std::shared_ptr<Processors> processors_, OutputPort *output_,
                             OutputPort *totals_, OutputPort *extremes_)
    : resources(std::move(resources_)), processors(std::move(processors_)), output(output_), totals(totals_), extremes(extremes_) {
...
QueryPipeline::QueryPipeline(Chain chain)
  ...
  for (auto processor : chain.getProcessors()) processors->emplace_back(std::move(processor));
...
QueryPipeline::QueryPipeline(std::shared_ptr<IOutputFormat> format) : processors(std::make_shared<Processors>()) {
...
void QueryPipeline::complete(std::shared_ptr<ISink> sink) {
  if (!pulling()) throw Exception(ErrorCodes::LOGICAL_ERROR, "Pipeline must be pulling to be completed with sink");
...
void QueryPipeline::complete(Chain chain) {
  if (!pulling()) throw Exception(ErrorCodes::LOGICAL_ERROR, "Pipeline must be pulling to be completed with chain");
...
void QueryPipeline::complete(std::shared_ptr<SinkToStorage> sink) { complete(Chain(std::move(sink))); }

void QueryPipeline::complete(Pipe pipe) {
  if (!pushing()) throw Exception(ErrorCodes::LOGICAL_ERROR, "Pipeline must be pushing to be completed with pipe");
...
void QueryPipeline::complete(std::shared_ptr<IOutputFormat> format) {
  if (!pulling()) throw Exception(ErrorCodes::LOGICAL_ERROR, "Pipeline must be pulling to be completed with output format");
...
Block QueryPipeline::getHeader() const {
  if (input) return input->getHeader();
  if (output) return output->getHeader();
...
SharedHeader QueryPipeline::getSharedHeader() const {
  if (input) return input->getSharedHeader();
  if (output) return output->getSharedHeader();
  throw Exception(ErrorCodes::LOGICAL_ERROR, "Header is available only for pushing or pulling QueryPipeline");
}

void QueryPipeline::setProgressCallback(const ProgressCallback &callback) { progress_callback = callback; }
...
void QueryPipeline::setQuota(std::shared_ptr<const EnabledQuota> quota_) { quota = std::move(quota_); }

void QueryPipeline::setLimitsAndQuota(const StreamLocalLimits &limits, std::shared_ptr<const EnabledQuota> quota_) {
  if (!pulling()) throw Exception(ErrorCodes::LOGICAL_ERROR, "It is possible to set limits and quota only to pulling QueryPipeline");
...
void QueryPipeline::finalizeWriteInQueryResultCache() {
  ...
  if (it != processors->end()) dynamic_cast<StreamInQueryResultCacheTransform &>(**it).finalizeWriteInQueryResultCache();
}

void QueryPipeline::readFromQueryResultCache(std::unique_ptr<SourceFromChunks> source, std::unique_ptr<SourceFromChunks> source_totals,
...
void QueryPipeline::addStorageHolder(StoragePtr storage) { resources.storage_holders.emplace_back(std::move(storage)); }

void QueryPipeline::addCompletedPipeline(QueryPipeline &&other) {
  if (!other.completed()) throw Exception(ErrorCodes::LOGICAL_ERROR, "Cannot add not completed pipeline");

  resources.append(other.resources);
  processors->insert(processors->end(), std::make_move_iterator(other.processors->begin()),
                     std::make_move_iterator(other.processors->end()));
}
...
void QueryPipeline::convertStructureTo(const ColumnsWithTypeAndName &columns, const ContextPtr &context) {
  if (!pulling()) throw Exception(ErrorCodes::LOGICAL_ERROR, "Pipeline must be pulling to convert header");

  auto converting = ActionsDAG::makeConvertingActions(output->getHeader().getColumnsWithTypeAndName(), columns,
...
std::unique_ptr<ReadProgressCallback> QueryPipeline::getReadProgressCallback() const {
  ...
  if (!update_profile_events) callback->disableProfileEventUpdate();
