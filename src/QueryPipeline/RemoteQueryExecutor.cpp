...
RemoteQueryExecutor::RemoteQueryExecutor(const String &query_, SharedHeader header_, ContextPtr context_, const Scalars &scalars_,
                                         const Tables &external_tables_, QueryProcessingStage::Enum stage_,
                                         std::shared_ptr<const QueryPlan> query_plan_, std::optional<Extension> extension_,
    ...
    : header(header_),
      ...
      read_packet_type_separately(context->canUseParallelReplicasOnInitiator() &&
                                  !context->getSettingsRef()[Setting::use_hedged_requests]) {
...
RemoteQueryExecutor::RemoteQueryExecutor(ConnectionPoolPtr pool, const String &query_, SharedHeader header_, ContextPtr context_,
  ...
  create_connections = [this, pool, throttler, extension_, connection_pool_with_failover_](AsyncCallback) {
    ...
    if (!result.entry.isNull() && result.is_usable) {
      if (extension_ && extension_->parallel_reading_coordinator) ProfileEvents::increment(ProfileEvents::ParallelReplicasAvailableCount);
      ...
      if (result.entry.isNull()) {
        ...
        if (connection_pool_with_failover_) connection_pool_with_failover_->incrementErrorCount(pool);
      } else
    ...
    if (extension_ && extension_->replica_info) res->setReplicaInfo(*extension_->replica_info);

    return res;
  };
}

RemoteQueryExecutor::RemoteQueryExecutor(Connection &connection, const String &query_, SharedHeader header_, ContextPtr context_,
                                         ThrottlerPtr throttler, const Scalars &scalars_, const Tables &external_tables_,
                                         QueryProcessingStage::Enum stage_, std::optional<Extension> extension_)
...
RemoteQueryExecutor::RemoteQueryExecutor(std::vector<IConnectionPool::Entry> &&connections_, const String &query_, SharedHeader header_,
                                         ContextPtr context_, const ThrottlerPtr &throttler, const Scalars &scalars_,
                                         const Tables &external_tables_, QueryProcessingStage::Enum stage_,
                                         std::shared_ptr<const QueryPlan> query_plan_, std::optional<Extension> extension_)
...
RemoteQueryExecutor::RemoteQueryExecutor(const ConnectionPoolWithFailoverPtr &pool, const String &query_, SharedHeader header_,
                                         ContextPtr context_, const ThrottlerPtr &throttler, const Scalars &scalars_,
                                         const Tables &external_tables_, QueryProcessingStage::Enum stage_,
                                         std::shared_ptr<const QueryPlan> query_plan_, std::optional<Extension> extension_,
                                         GetPriorityForLoadBalancing::Func priority_func_)
  ...
  create_connections = [this, pool, throttler](AsyncCallback async_callback) -> std::unique_ptr<IConnections> {
    ...
    if (current_settings[Setting::use_hedged_requests]) {
      ...
      if (main_table) table_to_check = std::make_shared<QualifiedTableName>(main_table.getQualifiedName());

      auto res = std::make_unique<HedgedConnections>(pool, context, timeouts, throttler, pool_mode, table_to_check,
                                                     std::move(async_callback), priority_func);
      if (extension && extension->replica_info) res->setReplicaInfo(*extension->replica_info);
      return res;
    }
#endif
    ...
    if (extension && extension->parallel_reading_coordinator) skip_unavailable_endpoints = true;

    if (main_table) {
      auto try_results = pool->getManyChecked(timeouts, current_settings, pool_mode, main_table.getQualifiedName(),
                                              std::move(async_callback), skip_unavailable_endpoints, priority_func);
      connection_entries.reserve(try_results.size());
      for (auto &try_result : try_results) connection_entries.emplace_back(std::move(try_result.entry));
    } else {
      connection_entries =
          pool->getMany(timeouts, current_settings, pool_mode, std::move(async_callback), skip_unavailable_endpoints, priority_func);
    }
  ...
  for (const auto &elem : header) {
    ...
    if (elem.column && isColumnConst(*elem.column)) {
      ...
      if (block.rows() > 0 && block.has(elem.name)) {
        ...
        if (const auto *blob = typeid_cast<const ColumnBLOB *>(col.column.get())) col.column = blob->convertFrom();
...
void RemoteQueryExecutor::sendQueryUnlocked(ClientInfo::QueryKind query_kind, AsyncCallback async_callback) {
  if (sent_query || was_cancelled) return;
  ...
  if (extension) modified_client_info.collaborate_with_initiator = true;
  ...
  if (settings[Setting::enable_scalar_subquery_optimization]) sendScalars();

  if (query_plan) connections->sendQueryPlan(*query_plan);
...
int RemoteQueryExecutor::sendQueryAsync() {
  ...
  if (was_cancelled) return -1;

  if (!read_context)
    read_context = std::make_unique<ReadContext>(*this,
                                                 /*suspend_when_query_sent*/ true, read_packet_type_separately);
  ...
  if (read_context->isQuerySent()) return -1;
...
Block RemoteQueryExecutor::readBlock() {
  while (true) {
    ...
    if (res.getType() == ReadResult::Type::Data) return res.getBlock();
  }
}
...
RemoteQueryExecutor::ReadResult RemoteQueryExecutor::read() {
  if (!sent_query) {
    ...
    if (context->getSettingsRef()[Setting::skip_unavailable_shards] && (0 == connections->size())) return ReadResult(Block());
  }

  while (true) {
    LockAndBlocker lock(was_cancelled_mutex);
    if (was_cancelled) return ReadResult(Block());

    auto packet = connections->receivePacket();
    auto anything = processPacket(std::move(packet));

    if (anything.getType() == ReadResult::Type::Data || anything.getType() == ReadResult::Type::ParallelReplicasToken) return anything;

    if (got_duplicated_part_uuids) break;
  }
...
RemoteQueryExecutor::ReadResult RemoteQueryExecutor::readAsync() {
  ...
  if (!read_context || (resent_query && recreate_read_context)) {
    ...
    read_context = std::make_unique<ReadContext>(*this,
                                                 /*suspend_when_query_sent*/ false, read_packet_type_separately);
  ...
  while (true) {
    ...
    if (packet_in_progress) {
      ...
      if (!read_context->hasReadPacket() && !read_context->read()) return ReadResult(read_context->getFileDescriptor());
    ...
    if (read_context->isInProgress()) return ReadResult(read_context->getFileDescriptor());
...
RemoteQueryExecutor::ReadResult RemoteQueryExecutor::restartQueryWithoutDuplicatedUUIDs() {
  {
    ...
    if (resent_query) throw Exception(ErrorCodes::DUPLICATED_PART_UUIDS, "Found duplicate uuids while processing query");

    if (log) LOG_DEBUG(log, "Found duplicate UUIDs, will retry query without those parts");
...
RemoteQueryExecutor::ReadResult RemoteQueryExecutor::processPacket(Packet packet) {
  switch (packet.type) {
    ...
    case Protocol::Server::PartUUIDs:
      if (!setPartUUIDs(packet.part_uuids)) got_duplicated_part_uuids = true;
      break;
    case Protocol::Server::Data:
      ...
      if (!packet.block.empty() && (packet.block.rows() > 0)) return ReadResult(adaptBlockStructure(packet.block, *header));
    ...
    case Protocol::Server::ProfileInfo:
      ...
      if (profile_info_callback) profile_info_callback(packet.profile_info);
      break;

    case Protocol::Server::Totals:
      totals = packet.block;
      if (!totals.empty()) totals = adaptBlockStructure(totals, *header);
      break;

    case Protocol::Server::Extremes:
      extremes = packet.block;
      if (!extremes.empty()) extremes = adaptBlockStructure(packet.block, *header);
      break;

    case Protocol::Server::Log:
      /// Pass logs from remote server to client
      if (auto log_queue = CurrentThread::getInternalTextLogsQueue()) log_queue->pushBlock(std::move(packet.block));
      break;
    ...
    default:
      ...
      throw Exception(ErrorCodes::UNKNOWN_PACKET_FROM_SERVER, "Unknown packet {} from one of the following replicas: {}", packet.type,
...
void RemoteQueryExecutor::processReadTaskRequest() {
  if (!extension || !extension->task_iterator) throw Exception(ErrorCodes::LOGICAL_ERROR, "Distributed task iterator is not initialized");

  if (!extension->replica_info) throw Exception(ErrorCodes::LOGICAL_ERROR, "Replica info is not initialized");
...
void RemoteQueryExecutor::finish() {
   ...
   */
  if (!isQueryPending() || hasThrownException() || was_cancelled) return;
  ...
  if (!connections || !sent_query || finished) return;
...
void RemoteQueryExecutor::cancelUnlocked() {
  {
    ...
    for (auto &vec : external_tables_data)
      for (auto &elem : vec) elem->is_cancelled = true;
  }

  if (!isQueryPending() || hasThrownException()) return;

  tryCancel("Cancelling query");
}

void RemoteQueryExecutor::sendScalars() { connections->sendScalarsData(scalars); }
...

  {
    ...
    for (size_t i = 0; i < count; ++i) {
      ...
      for (const auto &table : external_tables) {
        ...
        if (!std::dynamic_pointer_cast<StorageMemory>(cur)) continue;
        ...
        data->creating_pipe_callback = [cur, limits, my_context = this->context]() {
          ...
          QueryProcessingStage::Enum read_from_table_stage =
              cur->getQueryProcessingStage(my_context, QueryProcessingStage::Complete, storage_snapshot, query_info);

          QueryPlan plan;
          cur->read(plan, metadata_snapshot->getColumns().getNamesOfPhysical(), storage_snapshot, query_info, my_context,
...
void RemoteQueryExecutor::tryCancel(const char *reason) {
  ...
  if (read_context) read_context->cancel();
  ...
  if (connections && sent_query && !finished) {
    ...
    if (log) LOG_TRACE(log, "({}) {}", connections->dumpAddresses(), reason);
  }
}

bool RemoteQueryExecutor::isQueryPending() const { return (sent_query || read_context) && !finished; }

bool RemoteQueryExecutor::hasThrownException() const { return got_exception_from_replica || got_unknown_packet_from_replica; }

void RemoteQueryExecutor::setProgressCallback(ProgressCallback callback) {
  LockAndBlocker guard(was_cancelled_mutex);
  progress_callback = std::move(callback);

  if (extension && extension->parallel_reading_coordinator) extension->parallel_reading_coordinator->setProgressCallback(progress_callback);
}
...
bool RemoteQueryExecutor::processParallelReplicaPacketIfAny() {
  ...
  if (!read_context->readPacketTypeSeparately()) return false;
  ...
  if (!read_context->hasReadTillPacketType() && !read_context->read()) return false;
  ...
  if (packet_type == Protocol::Server::MergeTreeReadTaskRequest || packet_type == Protocol::Server::MergeTreeAllRangesAnnouncement) {
    ...
    if (!read_context->hasReadPacket() && !read_context->read()) return false;
