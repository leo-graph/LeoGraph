...
static void checkSource(const IProcessor &source) {
  if (!source.getInputs().empty())
    throw Exception(ErrorCodes::LOGICAL_ERROR, "Source for pipe shouldn't have any input, but {} has {} inputs", source.getName(),
                    source.getInputs().size());

  if (source.getOutputs().size() != 1)
    throw Exception(ErrorCodes::LOGICAL_ERROR, "Source for pipe should have single output, but {} has {} outputs", source.getName(),
...
static OutputPort *uniteExtremes(const OutputPortRawPtrs &ports, SharedHeader header, Processors &processors) {
  ...
  for (const auto &port : ports) connect(*port, *(in++));
...
Pipe::Pipe(ProcessorPtr source, OutputPort *output, OutputPort *totals, OutputPort *extremes) : processors(std::make_shared<Processors>()) {
  if (!source->getInputs().empty())
    throw Exception(ErrorCodes::LOGICAL_ERROR, "Source for pipe shouldn't have any input, but {} has {} inputs", source->getName(),
                    source->getInputs().size());

  if (!output) throw Exception(ErrorCodes::LOGICAL_ERROR, "Cannot create Pipe from source because specified output port is nullptr");
  ...
  /// Check that ports belong to source and all ports from source were specified.
  {
    ...
    if (num_specified_ports != outputs.size())
      throw Exception(ErrorCodes::LOGICAL_ERROR, "Cannot create Pipe from source because it has {} output ports, but {} were specified",
                      outputs.size(), num_specified_ports);
  }
...
Pipe::Pipe(ProcessorPtr source) : processors(std::make_shared<Processors>()) {
  checkSource(*source);
...
Pipe::Pipe(std::shared_ptr<Processors> processors_) : processors(std::move(processors_)) {
  ...
  for (const auto &processor : *processors) set.emplace(processor.get());

  for (auto &processor : *processors) {
    for (const auto &port : processor->getInputs()) {
      if (!port.isConnected())
        throw Exception(ErrorCodes::LOGICAL_ERROR, "Cannot create Pipe because processor {} has disconnected input port",
                        processor->getName());
      ...
      if (!set.contains(connected_processor))
        throw Exception(ErrorCodes::LOGICAL_ERROR,
                        ...
                        processor->getName(), connected_processor->getName());
    }
  ...
  header = output_ports.front()->getSharedHeader();
  for (size_t i = 1; i < output_ports.size(); ++i) assertBlocksHaveEqualStructure(*header, output_ports[i]->getHeader(), "Pipe");

  max_parallel_streams = output_ports.size();

  if (collected_processors)
    for (const auto &processor : *processors) collected_processors->emplace_back(processor);
}
...
static Pipes removeEmptyPipes(Pipes pipes) {
  ...
  for (auto &pipe : pipes) {
    if (!pipe.empty()) res.emplace_back(std::move(pipe));
  }
  ...
  for (const auto &pipe : pipes) {
    ...
    for (size_t i = 0; i < res.columns(); ++i) {
      ...
      if (cur && common && !isColumnConst(*cur)) {
        if (const auto *column_const = typeid_cast<const ColumnConst *>(common.get())) common = column_const->getDataColumnPtr();
      }
    }
  }
...
Pipe Pipe::unitePipes(Pipes pipes) { return Pipe::unitePipes(std::move(pipes), nullptr, false); }
...
Pipe Pipe::unitePipes(Pipes pipes, Processors *collected_processors, bool allow_empty_header) {
  ...
  if (pipes.empty()) return res;

  if (pipes.size() == 1) return std::move(pipes[0]);
  ...
  for (auto &pipe : pipes) {
    if (!allow_empty_header || !pipe.header->empty()) assertCompatibleHeader(*pipe.header, *res.header, "Pipe::unitePipes");
    ...
    if (pipe.totals_port) totals.emplace_back(pipe.totals_port);

    if (pipe.extremes_port) extremes.emplace_back(pipe.extremes_port);
  }
...
void Pipe::addSource(ProcessorPtr source) {
  ...
  if (collected_processors) collected_processors->emplace_back(source);
...
void Pipe::addTotalsSource(ProcessorPtr source) {
  if (output_ports.empty()) throw Exception(ErrorCodes::LOGICAL_ERROR, "Cannot add totals source to empty Pipe");

  if (totals_port) throw Exception(ErrorCodes::LOGICAL_ERROR, "Totals source was already added to Pipe");
...
void Pipe::addExtremesSource(ProcessorPtr source) {
  if (output_ports.empty()) throw Exception(ErrorCodes::LOGICAL_ERROR, "Cannot add extremes source to empty Pipe");

  if (extremes_port) throw Exception(ErrorCodes::LOGICAL_ERROR, "Extremes source was already added to Pipe");
...
static void dropPort(OutputPort *&port, Processors &processors, Processors *collected_processors) {
  ...
  if (collected_processors) collected_processors->emplace_back(null_sink);

  processors.emplace_back(std::move(null_sink));
  port = nullptr;
}

void Pipe::dropTotals() { dropPort(totals_port, *processors, collected_processors); }

void Pipe::dropExtremes() { dropPort(extremes_port, *processors, collected_processors); }
...
void Pipe::addTransform(ProcessorPtr transform, OutputPort *totals, OutputPort *extremes) {
  addTransform(std::move(transform), static_cast<InputPort *>(nullptr), static_cast<InputPort *>(nullptr), totals, extremes);
}

void Pipe::addTransform(ProcessorPtr transform, InputPort *totals, InputPort *extremes) {
  addTransform(std::move(transform), totals, extremes, static_cast<OutputPort *>(nullptr), static_cast<OutputPort *>(nullptr));
}

void Pipe::addTransform(ProcessorPtr transform, InputPort *totals_in, InputPort *extremes_in, OutputPort *totals_out,
                        OutputPort *extremes_out) {
  if (output_ports.empty()) throw Exception(ErrorCodes::LOGICAL_ERROR, "Cannot add transform to empty Pipe");
  ...
  if (inputs.size() != expected_inputs)
    throw Exception(ErrorCodes::LOGICAL_ERROR, "Cannot add transform {} to Pipe because it has {} input ports, but {} expected",
                    transform->getName(), inputs.size(), expected_inputs);
  ...
  if (totals_in && !found_totals_in)
    throw Exception(ErrorCodes::LOGICAL_ERROR, "Cannot add transform {} to Pipes because specified totals port does not belong to it",
                    transform->getName());

  if (extremes_in && !found_extremes_in)
    throw Exception(ErrorCodes::LOGICAL_ERROR, "Cannot add transform {} to Pipes because specified extremes port does not belong to it",
                    transform->getName());
  ...
  for (auto &output : outputs) {
    if (&output != totals_out && &output != extremes_out) output_ports.emplace_back(&output);
  }

  header = output_ports.front()->getSharedHeader();
  for (size_t i = 1; i < output_ports.size(); ++i) assertBlocksHaveEqualStructure(*header, output_ports[i]->getHeader(), "Pipes");
  ...
  if (extremes_port) assertBlocksHaveEqualStructure(*header, extremes_port->getHeader(), "Pipes");

  if (collected_processors) collected_processors->emplace_back(transform);
...
void Pipe::addSimpleTransform(const ProcessorGetterSharedHeaderWithStreamKind &getter) {
  if (output_ports.empty()) throw Exception(ErrorCodes::LOGICAL_ERROR, "Cannot add simple transform to empty Pipe.");
  ...
  auto add_transform = [&](OutputPort *&port, StreamType stream_type) {
    ...
    if (transform) {
      if (transform->getInputs().size() != 1)
        throw Exception(ErrorCodes::LOGICAL_ERROR, "Processor for query pipeline transform should have single input, but {} has {} inputs",
                        transform->getName(), transform->getInputs().size());

      if (transform->getOutputs().size() != 1)
        throw Exception(ErrorCodes::LOGICAL_ERROR,
                        "Processor for query pipeline transform should have single output, but {} has {} outputs", transform->getName(),
  ...
  for (auto &port : output_ports) add_transform(port, StreamType::Main);
...
void Pipe::addSplitResizeTransform(size_t num_streams, size_t min_outstreams_per_resize_after_split, bool strict) {
  ...
  for (size_t i = 0, next_input = 0, next_output = 0; i < groups; ++i) {
    ...
    if (collected_processors) collected_processors->emplace_back(resize);
    processors->emplace_back(std::move(resize));
  }
...
void Pipe::resize(size_t num_streams, bool strict, UInt64 min_outstreams_per_resize_after_split) {
  if (output_ports.empty()) throw Exception(ErrorCodes::LOGICAL_ERROR, "Cannot resize an empty Pipe");
  ...
  if (numOutputPorts() == 1 && num_streams == 1) return;
  ...
  if (strict && num_streams == numOutputPorts()) return;
...
void Pipe::setSinks(const Pipe::ProcessorGetterSharedHeaderWithStreamKind &getter) {
  if (output_ports.empty()) throw Exception(ErrorCodes::LOGICAL_ERROR, "Cannot set sink to empty Pipe");
  ...
  auto add_transform = [&](OutputPort *&stream, Pipe::StreamType stream_type) {
    ...
    if (transform) {
      if (transform->getInputs().size() != 1)
        throw Exception(ErrorCodes::LOGICAL_ERROR, "Sink for query pipeline transform should have single input, but {} has {} inputs",
                        transform->getName(), transform->getInputs().size());

      if (!transform->getOutputs().empty())
        throw Exception(ErrorCodes::LOGICAL_ERROR, "Sink for query pipeline transform should have no outputs, but {} has {} outputs",
                        transform->getName(), transform->getOutputs().size());
    }

    if (!transform) transform = std::make_shared<NullSink>(stream->getSharedHeader());
...
void Pipe::transform(const Transformer &transformer, bool check_ports) {
  if (output_ports.empty()) throw Exception(ErrorCodes::LOGICAL_ERROR, "Cannot transform empty Pipe");

  auto new_processors = transformer(output_ports);

  /// Create hash table with new processors.
  std::unordered_set<const IProcessor *> set;
  for (const auto &processor : new_processors) set.emplace(processor.get());

  for (const auto &port : output_ports) {
    if (!check_ports) break;

    if (!port->isConnected())
      throw Exception(ErrorCodes::LOGICAL_ERROR, "Transformation of Pipe is not valid because output port ({})",
  ...
  for (const auto &processor : new_processors) {
    for (const auto &port : processor->getInputs()) {
      ...
      if (!port.isConnected())
        throw Exception(ErrorCodes::LOGICAL_ERROR, "Transformation of Pipe is not valid because processor {} has not connected input port",
                        processor->getName());
  ...
  if (totals_port) assertBlocksHaveEqualStructure(*header, totals_port->getHeader(), "Pipes");

  if (extremes_port) assertBlocksHaveEqualStructure(*header, extremes_port->getHeader(), "Pipes");

  if (collected_processors) {
    for (const auto &processor : new_processors) collected_processors->emplace_back(processor);
  }
