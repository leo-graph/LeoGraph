...
void printPipelineCompact(const Processors &processors, WriteBuffer &out, bool with_header) {
  ...
  for (const auto &processor : processors) {
    ...
    if (res.second) node.id = graph.size();
  }
  ...
  for (const auto &processor : processors) {
    ...
    for (auto &port : processor->getOutputs()) {
      ...
      if (!found) edge.emplace_back(EdgeData{header, 1});
    }
  }
  ...
  for (const auto &item : graph) steps_map[item.first.step].emplace_back(&item.second);
  ...
  for (const auto &item : steps_map) {
    ...
    for (const auto &node : item.second) {
      ...
      if (node->agents.size() > 1) out << " × " << node->agents.size();
  ...
  for (const auto &item : graph) {
    for (const auto &edge : item.second.edges) {
      for (const auto &data : edge.second) {
        ...
        if (data.count > 1) out << "× " << data.count;
