...
bool ReadProgressCallback::onProgress(uint64_t read_rows, uint64_t read_bytes, const StorageLimitsList& storage_limits) {
  ...
  if (process_list_elem) {
    if (!process_list_elem->updateProgressIn(value)) return false;
    ...
    for (const auto& limits : storage_limits) {
      ...
      if (limits.local_limits.mode == LimitsMode::LIMITS_TOTAL) {
        if (!limits.local_limits.size_limits.check(rows_to_check_limit, progress.read_bytes, "rows or bytes to read",
                                                   ErrorCodes::TOO_MANY_ROWS, ErrorCodes::TOO_MANY_BYTES)) {
          return false;
        }
      }

      if (!limits.leaf_limits.check(rows_to_check_limit, progress.read_bytes, "rows or bytes to read on leaf node",
                                    ErrorCodes::TOO_MANY_ROWS, ErrorCodes::TOO_MANY_BYTES)) {
        return false;
      }
    }
    ...
    for (const auto& limits : storage_limits)
      limits.local_limits.speed_limits.throttle(progress.read_rows, progress.read_bytes, total_rows, total_stopwatch.elapsedMicroseconds(),
                                                limits.local_limits.timeout_overflow_mode);

    if (quota) quota->used({QuotaType::READ_ROWS, value.read_rows}, {QuotaType::READ_BYTES, value.read_bytes});
  }
