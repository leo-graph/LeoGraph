...
Distribution getDistribution(size_t from, size_t to) {
  ...
  for (size_t i = 0; i < from; ++i) distribution[i] = i % to;
...
void narrowPipe(Pipe &pipe, size_t width) {
  ...
  pipe.transform([&](OutputPortRawPtrs ports) {
    for (size_t i = 0; i < size; ++i) partitions[distribution[i]].emplace_back(ports[i]);

    Processors concats;
    concats.reserve(width);

    for (size_t i = 0; i < width; ++i) {
      auto concat = std::make_shared<ConcatProcessor>(partitions[i].at(0)->getSharedHeader(), partitions[i].size());
      size_t next_port = 0;
