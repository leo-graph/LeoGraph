...
RemoteInserter::RemoteInserter(Connection &connection_, const ConnectionTimeouts &timeouts_, const String &query_,
                               const Settings &settings_, const ClientInfo &client_info_)
...
void RemoteInserter::initialize() {
  ...
  connection.sendQuery(timeouts, query, /* query_parameters */ {}, "", QueryProcessingStage::Complete, &settings, &modified_client_info,
  ...
  while (true) {
    ...
    if (Protocol::Server::Log == packet.type) {
      ...
      throw NetException(ErrorCodes::UNEXPECTED_PACKET_FROM_SERVER, "Unexpected packet from server (expected Data or Exception, got {})",
...
void RemoteInserter::onFinish() {
  ...
  while (true) {
    ...
    if (Protocol::Server::Exception == packet.type)
      packet.exception->rethrow();
    else if (Protocol::Server::Log == packet.type || Protocol::Server::Progress == packet.type ||
             Protocol::Server::ProfileEvents == packet.type || Protocol::Server::TimezoneUpdate == packet.type) {
      // Do nothing
    } else
