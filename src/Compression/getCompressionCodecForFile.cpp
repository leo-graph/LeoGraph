...
CompressionCodecPtr getCompressionCodecForFile(ReadBuffer& read_buffer, UInt32& size_compressed, UInt32& size_decompressed,
                                               bool skip_to_next_block) {
  ...
  if (method == static_cast<uint8_t>(CompressionMethodByte::Multiple)) {
    ...
    Codecs codecs;
    for (auto byte : codecs_bytes) codecs.push_back(CompressionCodecFactory::instance().get(byte));

    if (skip_to_next_block) read_buffer.ignore(size_compressed - (read_buffer.count() - starting_bytes));
