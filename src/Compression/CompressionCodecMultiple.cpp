...
CompressionCodecMultiple::CompressionCodecMultiple(Codecs codecs_) : codecs(codecs_) {
  ASTs arguments;
  for (const auto& codec : codecs) arguments.push_back(codec->getCodecDesc());
  /// Special case, codec doesn't have name and contain list of codecs.
  setCodecDescription("", arguments);
}

uint8_t CompressionCodecMultiple::getMethodByte() const { return static_cast<uint8_t>(CompressionMethodByte::Multiple); }

void CompressionCodecMultiple::updateHash(SipHash& hash) const {
  for (const auto& codec : codecs) codec->updateHash(hash);
}
...
UInt32 CompressionCodecMultiple::getMaxCompressedDataSize(UInt32 uncompressed_size) const {
  ...
  for (const auto& codec : codecs) compressed_size = codec->getCompressedReserveSize(compressed_size);
...
UInt32 CompressionCodecMultiple::doDecompressData(const char* source, UInt32 source_size, char* dest, UInt32 decompressed_size) const {
  if (source_size < 1 || !source[0]) throw Exception(decompression_error_code, "Wrong compression methods list");
  ...
  for (int idx = compression_methods_size - 1; idx >= 0; --idx) {
    ...
    if (compressed_buf.size() >= 1_GiB) throw Exception(decompression_error_code, "Too large compressed size: {}", compressed_buf.size());
    ...
    if (uncompressed_size >= 1_GiB) throw Exception(decompression_error_code, "Too large uncompressed size: {}", uncompressed_size);

    if (idx == 0 && uncompressed_size != decompressed_size)
      throw Exception(decompression_error_code, "Wrong final decompressed size in codec Multiple, got {}, expected {}", uncompressed_size,
                      decompressed_size);
...
std::vector<uint8_t> CompressionCodecMultiple::getCodecsBytesFromData(const char* source) {
  ...
  for (size_t i = 0; i < compression_methods_size; ++i) result.push_back(source[1 + i]);
...
bool CompressionCodecMultiple::isCompression() const {
  for (const auto& codec : codecs)
    if (codec->isCompression()) return true;
  return false;
}
