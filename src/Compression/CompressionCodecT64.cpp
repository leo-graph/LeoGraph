...
class CompressionCodecT64 : public ICompressionCodec {
 public:
  ...
  enum class Variant : uint8_t { Byte, Bit };
...
MagicNumber serializeTypeId(std::optional<TypeIndex> type_id) {
  if (!type_id) throw Exception(ErrorCodes::CANNOT_COMPRESS, "T64 codec doesn't support compression without information about column type");

  switch (*type_id) {
    case TypeIndex::UInt8:
      return MagicNumber::UInt8;
    case TypeIndex::UInt16:
      return MagicNumber::UInt16;
    case TypeIndex::UInt32:
      return MagicNumber::UInt32;
    case TypeIndex::UInt64:
      return MagicNumber::UInt64;
    case TypeIndex::Int8:
      return MagicNumber::Int8;
    case TypeIndex::Int16:
      return MagicNumber::Int16;
    case TypeIndex::Int32:
      return MagicNumber::Int32;
    case TypeIndex::Int64:
      return MagicNumber::Int64;
    case TypeIndex::Date:
      return MagicNumber::Date;
    case TypeIndex::Date32:
      return MagicNumber::Date32;
    case TypeIndex::Time:
      return MagicNumber::Time;
    case TypeIndex::Time64:
      return MagicNumber::Time64;
    case TypeIndex::DateTime:
      return MagicNumber::DateTime;
    case TypeIndex::DateTime64:
      return MagicNumber::DateTime64;
    case TypeIndex::Enum8:
      return MagicNumber::Enum8;
    case TypeIndex::Enum16:
      return MagicNumber::Enum16;
    case TypeIndex::Decimal32:
      return MagicNumber::Decimal32;
    case TypeIndex::Decimal64:
      return MagicNumber::Decimal64;
    case TypeIndex::IPv4:
      return MagicNumber::IPv4;
    default:
      break;
  }
...
TypeIndex deserializeTypeId(uint8_t serialized_type_id) {
  ...
  switch (magic) {
    case MagicNumber::UInt8:
      return TypeIndex::UInt8;
    case MagicNumber::UInt16:
      return TypeIndex::UInt16;
    case MagicNumber::UInt32:
      return TypeIndex::UInt32;
    case MagicNumber::UInt64:
      return TypeIndex::UInt64;
    case MagicNumber::Int8:
      return TypeIndex::Int8;
    case MagicNumber::Int16:
      return TypeIndex::Int16;
    case MagicNumber::Int32:
      return TypeIndex::Int32;
    case MagicNumber::Int64:
      return TypeIndex::Int64;
    case MagicNumber::Date:
      return TypeIndex::Date;
    case MagicNumber::Date32:
      return TypeIndex::Date32;
    case MagicNumber::DateTime:
      return TypeIndex::DateTime;
    case MagicNumber::DateTime64:
      return TypeIndex::DateTime64;
    case MagicNumber::Time:
      return TypeIndex::Time;
    case MagicNumber::Time64:
      return TypeIndex::Time64;
    case MagicNumber::Enum8:
      return TypeIndex::Enum8;
    case MagicNumber::Enum16:
      return TypeIndex::Enum16;
    case MagicNumber::Decimal32:
      return TypeIndex::Decimal32;
    case MagicNumber::Decimal64:
...
UInt8 codecId() { return static_cast<UInt8>(CompressionMethodByte::T64); }
...
TypeIndex typeIdx(const IDataType *data_type) {
  if (!data_type) return TypeIndex::Nothing;
...
void reverseTranspose64x8(UInt64 *src_dst) {
  ...
  for (UInt32 i = 0; i < 64; ++i) {
    dst8[i] = static_cast<UInt8>(((src_dst[0] >> i) & 0x1) | (((src_dst[1] >> i) & 0x1) << 1) | (((src_dst[2] >> i) & 0x1) << 2) |
...
void transposeBytes(T value, UInt64 *matrix, UInt32 col) {
  ...
  if constexpr (sizeof(T) > 1) matrix8[64 * 1 + col] = value8[1];
...
void reverseTransposeBytes(const UInt64 *matrix, UInt32 col, T &value) {
  ...
  if constexpr (sizeof(T) > 1) value |= static_cast<UInt32>(matrix8[64 * 1 + col]) << (8 * 1);
...
template <typename T>
void clear(T *buf) {
  for (UInt32 i = 0; i < 64; ++i) buf[i] = 0;
}

MULTITARGET_FUNCTION_X86_V4_V3(MULTITARGET_FUNCTION_HEADER(template <typename T, bool full> void), transposeImpl,
                               MULTITARGET_FUNCTION_BODY((const T *src, char *dst, UInt32 num_bits, UInt32 tail)  /// NOLINT
                                                         {
                                                           UInt32 full_bytes = num_bits / 8;
                                                           UInt32 part_bits = num_bits % 8;

                                                           UInt64 matrix[64] = {};
                                                           for (UInt32 col = 0; col < tail; ++col) transposeBytes(src[col], matrix, col);
...
MULTITARGET_FUNCTION_X86_V4_V3(MULTITARGET_FUNCTION_HEADER(template <typename T, bool full> void), reverseTransposeImpl,
                               MULTITARGET_FUNCTION_BODY((const char *src, T *buf, UInt32 num_bits, UInt32 tail)  /// NOLINT
                                                         {
                                                           UInt64 matrix[64] = {};
...
void restoreUpperBits(T *buf, T upper_min, T upper_max [[maybe_unused]], T sign_bit [[maybe_unused]], UInt32 tail = 64) {
  ...
  for (UInt32 col = 0; col < tail; ++col) buf[col] |= upper_min;
}

UInt32 getValuableBitsNumber(UInt64 min, UInt64 max) {
  UInt64 diff_bits = min ^ max;
  if (diff_bits) return 64 - std::countl_zero(diff_bits);
  return 0;
}

UInt32 getValuableBitsNumber(Int64 min, Int64 max) {
  if (min < 0 && max >= 0) {
    if (min + max >= 0) return getValuableBitsNumber(0ull, static_cast<UInt64>(max)) + 1;
...
UInt32 compressData(const char *src, UInt32 bytes_size, char *dst) {
  ...
  if (bytes_size == 0) return bytes_to_skip;
  ...
  if (!num_bits) return header_size + bytes_to_skip;
...
UInt32 decompressData(const char *src, UInt32 bytes_size, char *dst, UInt32 uncompressed_size) {
  ...
  if (uncompressed_size % sizeof(T) != 0)
    throw Exception(ErrorCodes::CANNOT_DECOMPRESS,
                    "Cannot decompress T64-encoded data, unexpected uncompressed size ({})"
                    " isn't a multiple of the data type size ({})",
                    uncompressed_size, sizeof(T));

  if (uncompressed_size == 0) return static_cast<UInt32>(dst - original_dst);
  ...
  if (!num_bits) {
    ...
    for (UInt32 i = 0; i < num_elements; ++i, dst += sizeof(T)) unalignedStore<T>(dst, min_value);
  ...
  if (expected != num_elements)
    throw Exception(ErrorCodes::CANNOT_DECOMPRESS,
                    "Cannot decompress, the number of elements in the compressed data ({})"
  ...
  if (num_bits < 64) upper_min = static_cast<T>(static_cast<UInt64>(min) >> num_bits << num_bits);
...
UInt32 compressData(const char *src, UInt32 src_size, char *dst, Variant variant) {
  if (variant == Variant::Bit) return compressData<T, true>(src, src_size, dst);
...
UInt32 CompressionCodecT64::doDecompressData(const char *src, UInt32 src_size, char *dst, UInt32 uncompressed_size) const {
  if (!src_size) throw Exception(ErrorCodes::CANNOT_DECOMPRESS, "Cannot decompress T64-encoded data");
...
uint8_t CompressionCodecT64::getMethodByte() const { return codecId(); }

CompressionCodecT64::CompressionCodecT64(std::optional<TypeIndex> type_idx_, Variant variant_) : type_idx(type_idx_), variant(variant_) {
...
void registerCodecT64(CompressionCodecFactory &factory) {
  auto reg_func = [&](const ASTPtr &arguments, const IDataType *type) -> CompressionCodecPtr {
    ...
    if (arguments && !arguments->children.empty()) {
      ...
      if (!literal) throw Exception(ErrorCodes::ILLEGAL_CODEC_PARAMETER, "Wrong modification for T64. Expected: 'bit', 'byte')");
