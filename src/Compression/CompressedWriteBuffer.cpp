...
CompressedWriteBuffer::CompressedWriteBuffer(WriteBuffer& out_, CompressionCodecPtr codec_, size_t buf_size, bool use_adaptive_buffer_size_,
  ...
  if (!codec) codec = CompressionCodecFactory::instance().getDefaultCodec();
}
...
void CompressedWriteBuffer::setCodec(CompressionCodecPtr codec_) {
  ...
  next();
  if (offset() != 0) throw Exception(ErrorCodes::LOGICAL_ERROR, "CompressedWriteBuffer: offset() is not zero");
