...
uint8_t CompressionCodecZSTD::getMethodByte() const { return static_cast<uint8_t>(CompressionMethodByte::ZSTD); }

void CompressionCodecZSTD::updateHash(SipHash& hash) const { getCodecDesc()->updateTreeHash(hash, /*ignore_aliases=*/true); }
...
void registerCodecZSTD(CompressionCodecFactory& factory) {
  ...
  factory.registerCompressionCodec("ZSTD", method_code, [&](const ASTPtr& arguments) -> CompressionCodecPtr {
    ...
    if (arguments && !arguments->children.empty()) {
      ...
      if (!literal) throw Exception(ErrorCodes::ILLEGAL_CODEC_PARAMETER, "ZSTD codec argument must be integer");

      level = static_cast<int>(literal->value.safeGet<UInt64>());
      if (level > ZSTD_maxCLevel()) {
        throw Exception(ErrorCodes::ILLEGAL_CODEC_PARAMETER, "ZSTD codec can't have level more than {}, given {}", ZSTD_maxCLevel(), level);
      }
      if (arguments->children.size() > 1) {
        const auto* window_literal = children[1]->as<ASTLiteral>();
        if (!window_literal) throw Exception(ErrorCodes::ILLEGAL_CODEC_PARAMETER, "ZSTD codec second argument must be integer");
        ...
        if (window_log != 0 && (window_log > window_log_bounds.upperBound || window_log < window_log_bounds.lowerBound))
          throw Exception(ErrorCodes::ILLEGAL_CODEC_PARAMETER, "ZSTD codec can't have window log more than {} and lower than {}, given {}",
                          toString(window_log_bounds.upperBound), toString(window_log_bounds.lowerBound), toString(window_log));
...
CompressionCodecPtr getCompressionCodecZSTD(int level) { return std::make_shared<CompressionCodecZSTD>(level); }
