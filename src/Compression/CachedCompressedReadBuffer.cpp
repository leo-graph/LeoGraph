...
void CachedCompressedReadBuffer::initInput() {
  if (!file_in) {
    ...
    if (profile_callback) file_in->setProfileCallback(profile_callback, clock_type);
  }
}
...
bool CachedCompressedReadBuffer::nextImpl() {
  ...
  if (owned_cell->data.size() == 0) return false;
  ...
  if (nextimpl_working_buffer_offset > working_buffer.size())
    throw Exception(ErrorCodes::SEEK_POSITION_OUT_OF_BOUND,
                    "Seek position is beyond the decompressed block (pos: "
                    "{}, block size: {})",
...
CachedCompressedReadBuffer::CachedCompressedReadBuffer(const std::string& path_,
                                                       std::function<std::unique_ptr<ReadBufferFromFileBase>()> file_in_creator_,
                                                       UncompressedCache* cache_, bool allow_different_codecs_)
...
void CachedCompressedReadBuffer::seek(size_t offset_in_compressed_file, size_t offset_in_decompressed_block) {
  ...
  if (!owned_cell && file_pos == offset_in_compressed_file &&
      ((!buffer().empty() && offset() == offset_in_decompressed_block) || nextimpl_working_buffer_offset == offset_in_decompressed_block))
    return;
