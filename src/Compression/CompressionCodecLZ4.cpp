...
class CompressionCodecLZ4HC : public CompressionCodecLZ4 {
 ...
 protected:
  ...
  std::string getDescription() const override {
    return "LZ4 High Compression algorithm with configurable level; slower but better compression than LZ4, but decompression is still "
           "fast.";
  }
...
CompressionCodecLZ4::CompressionCodecLZ4() { setCodecDescription("LZ4"); }

uint8_t CompressionCodecLZ4::getMethodByte() const { return static_cast<uint8_t>(CompressionMethodByte::LZ4); }

void CompressionCodecLZ4::updateHash(SipHash& hash) const { getCodecDesc()->updateTreeHash(hash, /*ignore_aliases=*/true); }

UInt32 CompressionCodecLZ4::getMaxCompressedDataSize(UInt32 uncompressed_size) const { return LZ4_COMPRESSBOUND(uncompressed_size); }
...
UInt32 CompressionCodecLZ4::doDecompressData(const char* source, UInt32 source_size, char* dest, UInt32 uncompressed_size) const {
  ...
  if (!success) throw Exception(ErrorCodes::CANNOT_DECOMPRESS, "Cannot decompress LZ4-encoded data");
...
UInt32 CompressionCodecLZ4HC::doCompressData(const char* source, UInt32 source_size, char* dest) const {
  ...
  if (!success) throw Exception(ErrorCodes::CANNOT_COMPRESS, "Cannot compress with LZ4 codec");
  ...
  factory.registerCompressionCodec("LZ4HC", {}, [&](const ASTPtr& arguments) -> CompressionCodecPtr {
    ...
    if (arguments && !arguments->children.empty()) {
      if (arguments->children.size() > 1)
        throw Exception(ErrorCodes::ILLEGAL_SYNTAX_FOR_CODEC_TYPE, "LZ4HC codec must have 1 parameter, given {}",
                        arguments->children.size());

      const auto children = arguments->children;
      const auto* literal = children[0]->as<ASTLiteral>();
      if (!literal) throw Exception(ErrorCodes::ILLEGAL_CODEC_PARAMETER, "LZ4HC codec argument must be integer");
...
CompressionCodecLZ4HC::CompressionCodecLZ4HC(int level_) : level(level_) {
  ASTs arguments;
  arguments.push_back(make_intrusive<ASTLiteral>(static_cast<UInt64>(level)));
  setCodecDescription("LZ4HC", arguments);
}

CompressionCodecPtr getCompressionCodecLZ4(int level) { return std::make_shared<CompressionCodecLZ4HC>(level); }
