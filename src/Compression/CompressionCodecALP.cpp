...
constexpr std::array<T, exponent_count> generatePowersOf10() {
  ...
  for (UInt64 i = 0, v = 1; i < exponent_count; ++i, v *= 10) arr[i] = inverse ? static_cast<T>(1) / static_cast<T>(v) : static_cast<T>(v);
  return arr;
}
...
struct ALPFloatUtils {
  static Int64 encodeValue(T value, UInt8 exponent, UInt8 fraction) {
    ...
    const bool invalid = std::isinf(value_enc) || std::isnan(value_enc) || value_enc < ALPFloatTraits<T>::LOWER ||
                         value_enc > ALPFloatTraits<T>::UPPER || (value_enc == static_cast<T>(0.0) && std::signbit(value_enc));

    if (unlikely(invalid)) return static_cast<Int64>(ALPFloatTraits<T>::UPPER);
...
class ALPCodecEncoder {
 public:
  ...
  UInt32 encode(const char *source, const UInt32 source_size, char *dest) {
    if (source_size % sizeof(T) != 0)
      throw Exception(ErrorCodes::CANNOT_COMPRESS, "Cannot compress with ALP codec, data size {} is not aligned to {}", source_size,
                      sizeof(T));
    ...
    if (float_count > 0) dest = encodeBlock(source, static_cast<UInt16>(float_count), dest);
 ...
 private:
  ...
  const char *encodeBlockToState(const char *source, const UInt16 float_count) {
    ...
    for (UInt32 i = 0; i < block.exceptions_count; ++i) block.encoded_floats[block.exceptions[i].index] = block.frame_of_reference;
  ...
  EncodingParams selectBlockParams(const char *source, const UInt32 float_count) {
    ...
    if (param_candidates.size() == 1) return param_candidates[0];
    ...
    for (UInt32 i = 0; i < sample_count; ++i) sample[i] = unalignedLoadLittleEndian<T>(&source[i * sample_step * sizeof(T)]);
    ...
    for (const auto &params : param_candidates) {
      ...
      if (estimated_size < best_size) {
        ...
        if (is_prev_worse) break;  // Early stop if two consecutive candidates are worse
        is_prev_worse = true;
      }
    }
  ...
  void estimateParamCandidates(const char *source, const UInt32 float_count) {
    ...
    const UInt32 sample_step =
        float_count > ALP_PARAMS_ESTIMATION_SAMPLES * ALP_PARAMS_ESTIMATION_SAMPLE_FLOATS
    ...
    for (UInt32 i = 0; i < ALP_PARAMS_ESTIMATION_SAMPLES; ++i) {
      ...
      if (sample_start_index >= float_count) break;
      ...
      for (UInt32 j = 0; j < sample_float_count; ++j, sample_pos += sizeof(T)) sample[j] = unalignedLoadLittleEndian<T>(sample_pos);
    ...
    for (const auto &[_, estimation] : estimations_map) estimations[estimation_count++] = estimation;
    std::sort(estimations.begin(), estimations.begin() + estimation_count, [](const Estimation &a, const Estimation &b) {
      if (a.occurred_times == b.occurred_times) {
        if (a.params.exponent == b.params.exponent) return a.params.fraction < b.params.fraction;
    ...
    for (UInt32 i = 0; i < estimation_count; ++i) param_candidates.push_back(estimations[i].params);
  }
  ...
  static UInt8 calculateEncodeBits(const Int64 min_value, const Int64 max_value) {
    if (unlikely(min_value > max_value)) return sizeof(Int64) * 8;  // Edge case when no values are encoded or overflow happened.
    ...
    if (unlikely(diff == 0)) return 0;  // Edge case when all values are the same.
 ...
 private:
  ...
  void decodeBlock(const char *&source, const char *source_end, char *&dest, const char *dest_end, const UInt16 float_count) {
    ...
    if (unlikely(exponent >= ALPFloatTraits<T>::EXPONENT_COUNT))
      throw Exception(ErrorCodes::CANNOT_DECOMPRESS, "Cannot decompress ALP-encoded data, invalid exponent value: {}, max allowed: {}",
    ...
    if (unlikely(fraction >= ALPFloatTraits<T>::EXPONENT_COUNT))
      throw Exception(ErrorCodes::CANNOT_DECOMPRESS, "Cannot decompress ALP-encoded data, invalid fraction value: {}, max allowed: {}",
    ...
    for (UInt16 i = 0; i < exception_count; ++i) {
      ...
      if (unlikely(exception_index >= float_count))
        throw Exception(ErrorCodes::CANNOT_DECOMPRESS,
                        "Cannot decompress ALP-encoded data, invalid exception index, index: {}, float count: {}", exception_index,
                        float_count);
  ...
  static void processUnencodedBlock(const char *&source, const char *source_end, char *&dest, const char *dest_end,
                                    const UInt16 float_count) {
    const size_t block_size = float_count * sizeof(T);
...
CompressionCodecALP::CompressionCodecALP(UInt8 float_width_) : float_width(float_width_) { setCodecDescription("ALP", {}); }

uint8_t CompressionCodecALP::getMethodByte() const { return static_cast<uint8_t>(CompressionMethodByte::ALP); }

void CompressionCodecALP::updateHash(SipHash &hash) const { getCodecDesc()->updateTreeHash(hash, /* ignore_aliases */ true); }

String CompressionCodecALP::getDescription() const { return "Adaptive Lossless floating-Point; suitable for time series data."; }
  ...
  *dest++ = float_width;
  ...
  switch (float_width) {
    ...
    case sizeof(Float64):
      ...
      break;
    default:
      throw Exception(ErrorCodes::CANNOT_COMPRESS, "Cannot compress with codec ALP, unsupported float width {}",
...
UInt32 CompressionCodecALP::doDecompressData(const char *source, UInt32 source_size, char *dest, UInt32 uncompressed_size) const {
  ...
  if (codec_version != ALP_CODEC_VERSION)
    throw Exception(ErrorCodes::CANNOT_DECOMPRESS, "Cannot decompress ALP-encoded data, unsupported codec version {}",
                    static_cast<UInt32>(codec_version));

  if (codec_variant != 0)
    throw Exception(ErrorCodes::CANNOT_DECOMPRESS, "Cannot decompress ALP-encoded data, unsupported codec variant {}",
  ...
  if (block_float_count != ALP_BLOCK_MAX_FLOAT_COUNT)
    throw Exception(ErrorCodes::CANNOT_DECOMPRESS, "Cannot decompress ALP-encoded data, supported block float count is {}, got {}",
  ...
  switch (data_float_width) {
    ...
    case sizeof(Float64):
      ...
      break;
    default:
      throw Exception(ErrorCodes::CANNOT_DECOMPRESS, "Cannot decompress ALP-encoded data, unsupported float width {}",
...
void registerCodecALP(CompressionCodecFactory &factory) {
  ...
  auto codec_builder = [&](const ASTPtr &arguments, const IDataType *column_type) -> CompressionCodecPtr {
    if (arguments && !arguments->children.empty())
      throw Exception(ErrorCodes::ILLEGAL_SYNTAX_FOR_CODEC_TYPE, "ALP codec must have 0 parameters, given {}", arguments->children.size());

    UInt8 float_width = sizeof(Float64);
    if (column_type) {
      if (!WhichDataType(column_type).isNativeFloat())
        throw Exception(ErrorCodes::BAD_ARGUMENTS, "Codec ALP is not applicable for {} because the data type is not Float*",
                        column_type->getName());
