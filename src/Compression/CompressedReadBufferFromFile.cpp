...
bool CompressedReadBufferFromFile::nextImpl() {
  try {
    ...
    if (!size_compressed) return false;
    ...
    if (nextimpl_working_buffer_offset > working_buffer.size())
      throw Exception(ErrorCodes::SEEK_POSITION_OUT_OF_BOUND,
                      "Required to move position beyond the decompressed block (pos: "
                      "{}, block size: {})",
                      nextimpl_working_buffer_offset, toString(working_buffer.size()));
...
void CompressedReadBufferFromFile::prefetch(Priority priority) { file_in.prefetch(priority); }
...
void CompressedReadBufferFromFile::seek(size_t offset_in_compressed_file, size_t offset_in_decompressed_block) {
  ...
  if (!size_compressed &&
      static_cast<size_t>(file_in.getPosition()) == offset_in_compressed_file &&  /// correct position in compressed file
  ...
  if (size_compressed && offset_in_compressed_file == file_in.getPosition() - size_compressed &&
...
off_t CompressedReadBufferFromFile::getPosition() const { return file_in.getPosition(); }
...
size_t CompressedReadBufferFromFile::readBig(char* to, size_t n) {
  try {
    ...
    if (pos < working_buffer.end()) bytes_read += read(to, std::min(static_cast<size_t>(working_buffer.end() - pos), n));
    ...
    while (bytes_read < n) {
      ...
      if (!new_size_compressed) return bytes_read;
