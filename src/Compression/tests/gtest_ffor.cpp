...
T generateRandomInput(T *in, UInt8 bits) {
  ...
  for (UInt16 i = 0; i < values; ++i) in[i] = in_dist(rng);
...
T generateMinMaxInput(T *in, UInt8 bits) {
  ...
  for (UInt16 i = 0; i < values; ++i) in[i] = i % 2 == 0 ? max : (i % 3 == 0 ? max : min);
...
void runPackUnpackCorrectnessTest(UInt8 bits, InputGeneratorFn<T, values> generator) {
  ...
  for (auto i = used_bytes; i < total_bytes; ++i) coded_bytes[i] = static_cast<char>(byte_dist(rng));

  // Decode
  Compression::FFOR::bitUnpack<values>(coded, decoded, bits, base);

  // Verify
  for (UInt16 i = 0; i < values; ++i) ASSERT_EQ(decoded[i], in[i]) << "bits=" << static_cast<UInt32>(bits) << " index=" << i;
}
...
TEST_P(FFOR16BitTest, RandomPackUnpackCorrectnessTest) { runPackUnpackCorrectnessTest<UInt16>(GetParam(), generateRandomInput<UInt16>); }

TEST_P(FFOR16BitTest, MinMaxPackUnpackCorrectnessTest) { runPackUnpackCorrectnessTest<UInt16>(GetParam(), generateMinMaxInput<UInt16>); }

TEST_P(FFOR32BitTest, RandomPackUnpackCorrectnessTest) { runPackUnpackCorrectnessTest<UInt32>(GetParam(), generateRandomInput<UInt32>); }

TEST_P(FFOR32BitTest, MinMaxPackUnpackCorrectnessTest) { runPackUnpackCorrectnessTest<UInt32>(GetParam(), generateMinMaxInput<UInt32>); }

TEST_P(FFOR64BitTest, RandomPackUnpackCorrectnessTest) { runPackUnpackCorrectnessTest<UInt64>(GetParam(), generateRandomInput<UInt64>); }

TEST_P(FFOR64BitTest, MinMaxPackUnpackCorrectnessTest) { runPackUnpackCorrectnessTest<UInt64>(GetParam(), generateMinMaxInput<UInt64>); }
