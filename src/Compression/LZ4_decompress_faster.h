...
struct PerformanceStatistics {
  struct Element {
    ...
    double adjustedCount() const { return count - NUM_INVOCATIONS_TO_THROW_OFF; }

    double mean() const { return sum / adjustedCount(); }
    ...
    double sigma() const { return mean() / adjustedCount(); }

    void update(double seconds, double bytes) {
      ++count;

      if (count > NUM_INVOCATIONS_TO_THROW_OFF) sum += seconds / bytes;
    }
    ...
    double sample(pcg64& stat_rng) const {
      ...
      if (adjustedCount() < NUM_INVOCATIONS_TO_THROW_OFF) return adjustedCount() - 1;
  ...
  size_t select(size_t max_method = NUM_ELEMENTS) {
    if (choose_method < 0) {
      ...
      for (size_t i = 0; i < max_method; ++i) samples[i] = choose_method == -1 ? data[i].sample(rng) : data[i].adjustedCount();
 ...
 */
bool decompress(const char* const source,  /// NOLINT
                char* const dest,          /// NOLINT
                size_t source_size, size_t dest_size, PerformanceStatistics& statistics);
