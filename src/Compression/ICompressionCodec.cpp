...
void ICompressionCodec::setCodecDescription(const String& codec_name, const ASTs& arguments) {
  ...
  if (codec_name.empty()) {
    ...
    for (const auto& argument : arguments) codec_desc->children.push_back(argument);
...
ASTPtr ICompressionCodec::getFullCodecDesc() const {
  if (full_codec_desc == nullptr) throw Exception(ErrorCodes::LOGICAL_ERROR, "Codec description is not prepared");
...
ASTPtr ICompressionCodec::getCodecDesc() const {
  ...
  if (arguments->children.size() == 1) return arguments->children[0];
...
UInt32 ICompressionCodec::decompress(const char* source, UInt32 source_size, char* dest) const {
  ...
  if (source_size < header_size)
    throw Exception(decompression_error_code,
                    ...
                    source_size, static_cast<size_t>(header_size));
  ...
  if (decompressed_size != final_decompressed_size)
    throw Exception(decompression_error_code,
                    ...
                    final_decompressed_size, decompressed_size, getCodecDesc()->formatForErrorMessage());
...
uint8_t ICompressionCodec::readMethod(const char* source) { return static_cast<uint8_t>(source[0]); }
