...
static void validateChecksum(char *data, size_t size, const Checksum expected_checksum, bool external_data) {
  ...
  if (expected_checksum == calculated_checksum) return;
  ...
  message << "Checksum doesn't match: corrupted data."
             " Reference: " +
                 getHexUIntLowercase(expected_checksum) + ". Actual: " + getHexUIntLowercase(calculated_checksum) +
                 ". Size of compressed block: " + toString(size);

  const char *message_hardware_failure =
      "This is most likely due to hardware failure. "
  ...
  auto flip_bit = [](char *buf, size_t pos) { buf[pos / 8] ^= 1 << pos % 8; };
  ...
  if (difference == 1) {
    ProfileEvents::increment(ProfileEvents::CompressedReadBufferChecksumDoesntMatchSingleBitMismatch);
    message << ". The mismatch is caused by single bit flip in checksum. " << message_hardware_failure;
    throw Exception::createDeprecated(message.str(), error_code);
  }
...
static void readHeaderAndGetCodecAndSize(const char *compressed_buffer, UInt8 header_size, CompressionCodecPtr &codec,
                                         size_t &size_decompressed, size_t &size_compressed_without_checksum, bool allow_different_codecs,
                                         bool external_data) {
  ...
  if (external_data) codec->setExternalDataFlag();
...
size_t CompressedReadBufferBase::readCompressedData(size_t &size_decompressed, size_t &size_compressed_without_checksum, bool always_copy) {
  ...
  if (!always_copy && compressed_in->available() >= size_header_plus_checksum) {
    ...
    readHeaderAndGetCodecAndSize(compressed_in->position() + sizeof(Checksum), header_size, codec, size_decompressed,
                                 size_compressed_without_checksum, allow_different_codecs, external_data);
    ...
    readHeaderAndGetCodecAndSize(own_compressed_buffer.data() + sizeof(Checksum), header_size, codec, size_decompressed,
                                 size_compressed_without_checksum, allow_different_codecs, external_data);
  }
  ...
  if (!disable_checksum) validateChecksum(compressed_buffer, size_compressed_without_checksum, checksum, external_data);
...
void CompressedReadBufferBase::decompress(BufferBase::Buffer &to, size_t size_decompressed, size_t size_compressed_without_checksum) {
  ...
  if (codec->isNone()) {
    ...
    if (size_compressed_without_checksum < header_size)
      throw Exception(
          external_data ? ErrorCodes::CANNOT_DECOMPRESS : ErrorCodes::CORRUPTED_DATA,
...
void CompressedReadBufferBase::addDiagnostics(Exception &e) const {
  ...
  if (auto *seekable_in = dynamic_cast<SeekableReadBuffer *>(compressed_in)) current_pos = seekable_in->tryGetPosition();
  UInt8 header_size = ICompressionCodec::getHeaderSize();
  String header_hex =
      own_compressed_buffer_header_init
  ...
  e.addMessage("While reading or decompressing {} (position: {}, typename: {}, compressed data header: {})",
               getFileNameFromReadBuffer(*compressed_in), (current_pos ? std::to_string(*current_pos) : "?"),
               demangle(typeid(*compressed_in).name()), header_hex);
}
