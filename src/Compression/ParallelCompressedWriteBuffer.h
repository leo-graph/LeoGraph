...
class ParallelCompressedWriteBuffer final : public WriteBuffer {
 public:
  explicit ParallelCompressedWriteBuffer(WriteBuffer& out_, CompressionCodecPtr codec_, size_t buf_size_, size_t num_threads_,
                                         ThreadPool& pool_);
 ...
 private:
  ...
  struct BufferPair {
    explicit BufferPair(size_t input_size) : uncompressed(input_size) {}
