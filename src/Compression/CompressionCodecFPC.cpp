...
uint8_t CompressionCodecFPC::getMethodByte() const { return static_cast<uint8_t>(CompressionMethodByte::FPC); }

void CompressionCodecFPC::updateHash(SipHash& hash) const { getCodecDesc()->updateTreeHash(hash, /*ignore_aliases=*/true); }

CompressionCodecFPC::CompressionCodecFPC(UInt8 float_width_, UInt8 compression_level_)
    : float_width(float_width_), compression_level(compression_level_) {
...
UInt32 CompressionCodecFPC::getMaxCompressedDataSize(UInt32 uncompressed_size) const {
  ...
  if (float_count % 2 != 0) ++float_count;
...
UInt8 getFloatByteWidth(const IDataType& column_type) {
  if (!WhichDataType(column_type).isNativeFloat())
    throw Exception(ErrorCodes::BAD_ARGUMENTS, "Codec 'FPC' is not applicable for {} because the data type is not Float*",
                    column_type.getName());
...
void registerCodecFPC(CompressionCodecFactory& factory) {
  ...
  auto codec_builder = [&](const ASTPtr& arguments, const IDataType* column_type) -> CompressionCodecPtr {
    UInt8 float_width = 4;
    if (column_type) float_width = getFloatByteWidth(*column_type);

    UInt8 level = CompressionCodecFPC::DEFAULT_COMPRESSION_LEVEL;
    if (arguments && !arguments->children.empty()) {
      if (arguments->children.size() > 2)
        throw Exception(ErrorCodes::ILLEGAL_SYNTAX_FOR_CODEC_TYPE, "Codec 'FPC' must have from 0 to 2 parameters, given {}",
      ...
      if (level < 1 || level > CompressionCodecFPC::MAX_COMPRESSION_LEVEL)
        throw Exception(ErrorCodes::ILLEGAL_CODEC_PARAMETER, "Codec 'FPC' level must be between {} and {}", 1,
                        static_cast<int>(CompressionCodecFPC::MAX_COMPRESSION_LEVEL));
...
class DfcmPredictor {
 public:
  explicit DfcmPredictor(size_t table_size) : table(table_size, 0), prev_value{0}, hash{0} {}
...
class FcmPredictor {
 public:
  explicit FcmPredictor(size_t table_size) : table(table_size, 0), hash{0} {}
...
class FPCOperation {
 ...
 public:
  FPCOperation(std::span<std::byte> destination, UInt8 compression_level)
      : dfcm_predictor(1u << compression_level),
        fcm_predictor(1u << compression_level),
        chunk{},
        result{destination},
        result_initial_pointer{result.data()} {}
  ...
  UInt32 decode(std::span<const std::byte> values, size_t decoded_size) && {
    size_t read_bytes = 0;

    std::span<TUInt> chunk_view(chunk);
    for (size_t i = 0; i < decoded_size; i += chunk_view.size_bytes()) {
      if (i + chunk_view.size_bytes() > decoded_size) chunk_view = chunk_view.first(ceilBytesToEvenValues(decoded_size - i));
 ...
 private:
  ...
  UInt32 decodeCompressedZeroByteCount(UInt32 encoded_size) {
    if constexpr (VALUE_SIZE == MAX_ZERO_BYTE_COUNT + 1) {
      if (encoded_size > 3) ++encoded_size;
    }
    return encoded_size;
  }
  ...
  size_t decodePair(std::span<const std::byte> bytes, TUInt& first, TUInt& second) {
    ...
    UInt32 zero_byte_count1 = decodeCompressedZeroByteCount(std::to_integer<UInt32>(bytes.front() >> 4) & MAX_ZERO_BYTE_COUNT);
    UInt32 zero_byte_count2 = decodeCompressedZeroByteCount(std::to_integer<UInt32>(bytes.front()) & MAX_ZERO_BYTE_COUNT);
    ...
    if (__builtin_add_overflow(tail_size1, tail_size2, &expected_size) || __builtin_add_overflow(expected_size, 1, &expected_size))
        [[unlikely]]
  ...
  /// Pointer to the start of the result buffer (used to calculate the final size)
  std::span<std::byte>::const_pointer result_initial_pointer{};
};
...
UInt32 CompressionCodecFPC::doDecompressData(const char* source, UInt32 source_size, char* dest, UInt32 uncompressed_size) const {
  ...
  switch (compressed_float_width) {
    case sizeof(Float64):
      return FPCOperation<UInt64>(destination, compressed_level).decode(src, uncompressed_size);
    case sizeof(Float32):
      return FPCOperation<UInt32>(destination, compressed_level).decode(src, uncompressed_size);
    default:
