...
class CompressionCodecGCD : public ICompressionCodec {
 ...
 protected:
  /// 1 byte (`gcd_bytes_size` value) + 1 byte (`bytes_to_skip` value) + `bytes_to_skip` bytes (trash) + `gcd_bytes_size` bytes (gcd value)
  /// + (`source_size` - `bytes_to_skip`) bytes (data)
  ...
  String getDescription() const override {
    return "Preprocessor. Greatest common divisor compression; divides values by a common divisor; effective for divisible integer "
           "sequences.";
  }
...
CompressionCodecGCD::CompressionCodecGCD(UInt8 gcd_bytes_size_) : gcd_bytes_size(gcd_bytes_size_) { setCodecDescription("GCD", {}); }

UInt32 CompressionCodecGCD::getMaxCompressedDataSize(UInt32 uncompressed_size) const {
  return uncompressed_size + gcd_bytes_size  // To store gcd
         + 2;                                // Values of `gcd_bytes_size` and `bytes_to_skip`
}

uint8_t CompressionCodecGCD::getMethodByte() const { return static_cast<uint8_t>(CompressionMethodByte::GCD); }

void CompressionCodecGCD::updateHash(SipHash& hash) const { getCodecDesc()->updateTreeHash(hash, /*ignore_aliases=*/true); }
...
void compressDataForType(const char* source, UInt32 source_size, char* dest) {
  if (source_size % sizeof(T) != 0)
    throw Exception(ErrorCodes::CANNOT_COMPRESS, "Cannot compress with GCD codec, data size {} is not aligned to {}", source_size,
                    sizeof(T));
...
UInt32 decompressDataForType(const char* source, UInt32 source_size, char* dest, UInt32 output_size) {
  ...
  if (source_size % sizeof(T) != 0)
    throw Exception(ErrorCodes::CANNOT_DECOMPRESS, "Cannot decompress GCD-encoded data, data size {} is not aligned to {}", source_size,
                    sizeof(T));

  if (source_size < sizeof(T))
    throw Exception(ErrorCodes::CANNOT_DECOMPRESS, "Cannot decompress GCD-encoded data, data size {} is less than {}", source_size,
                    sizeof(T));
  ...
  if (gcd_multiplier == 0 || gcd_multiplier == 1) {
    ...
    if (source_size - sizeof(T) != output_size) throw Exception(ErrorCodes::CANNOT_DECOMPRESS, "Cannot decompress GCD-encoded data");
...
UInt32 CompressionCodecGCD::doDecompressData(const char* source, UInt32 source_size, char* dest, UInt32 uncompressed_size) const {
  if (source_size < 2) throw Exception(ErrorCodes::CANNOT_DECOMPRESS, "Cannot decompress GCD-encoded data. File has wrong header");
...
UInt8 getGCDBytesSize(const IDataType* column_type) {
  ...
  if (!(which.isInt() || which.isUInt() || which.isDecimal() || which.isDateOrDate32() || which.isDateTime() || which.isDateTime64()))
    throw Exception(
        ...
        "Codec GCD cannot be applied to column {} because it can only be used with Int*, UInt*, Decimal*, Date* or DateTime* types.",
        column_type->getName());
...
void registerCodecGCD(CompressionCodecFactory& factory) {
  ...
  auto codec_builder = [&](const ASTPtr& arguments, const IDataType* column_type) -> CompressionCodecPtr {
    ...
    if (column_type) gcd_bytes_size = getGCDBytesSize(column_type);
...
CompressionCodecPtr getCompressionCodecGCD(UInt8 gcd_bytes_size) { return std::make_shared<CompressionCodecGCD>(gcd_bytes_size); }
