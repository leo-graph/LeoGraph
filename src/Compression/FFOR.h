 ...
 *
 * Important:
 *   * Original FastLanes implementation processes 1024 values only. Avoid using values other than 1024 at all costs as code likely won't
 * auto-vectorise!
...
consteval auto makeBitPackDispatchTable() {
  ...
  return [&]<UInt8... bits>(std::integer_sequence<UInt8, bits...>) {
    ...
    return std::array<fn, size>{
        +[](const T* __restrict in, T* __restrict out, const T base) { bitPack<T, bits, values>(in, out, base); }...};
...
consteval auto makeBitUnpackDispatchTable() {
  ...
  return [&]<UInt8... bits>(std::integer_sequence<UInt8, bits...>) {
    using fn = void (*)(const T* __restrict, T* __restrict, T);
    return std::array<fn, size>{
        +[](const T* __restrict in, T* __restrict out, const T base) { bitUnpack<T, bits, values>(in, out, base); }...};
  }(std::make_integer_sequence<UInt8, size>{});
}
}  // namespace Details
