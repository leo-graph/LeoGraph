...
EncryptionMethod toEncryptionMethod(const std::string &name) {
  if (name == "AES_128_GCM_SIV") return AES_128_GCM_SIV;
  if (name == "AES_256_GCM_SIV") return AES_256_GCM_SIV;
...
String getMethodName(EncryptionMethod Method) {
  if (Method == AES_128_GCM_SIV) return "AES_128_GCM_SIV";
  if (Method == AES_256_GCM_SIV) return "AES_256_GCM_SIV";
  return "";
}

/// Get method code (used for codec, to understand which one we are using)
uint8_t getMethodCode(EncryptionMethod Method) {
  if (Method == AES_128_GCM_SIV) return static_cast<uint8_t>(CompressionMethodByte::AES_128_GCM_SIV);
  if (Method == AES_256_GCM_SIV) return static_cast<uint8_t>(CompressionMethodByte::AES_256_GCM_SIV);
...
UInt64 methodKeySize(EncryptionMethod Method) {
  if (Method == AES_128_GCM_SIV) return 16;
  if (Method == AES_256_GCM_SIV) return 32;
  throw Exception(ErrorCodes::BAD_ARGUMENTS, "Unknown encryption method. Got {}", getMethodName(Method));
}
...
size_t encrypt(std::string_view plaintext, char *ciphertext_and_tag, EncryptionMethod method, const String &key, const String &nonce) {
  ...
  if (!ctx) throw Exception(ErrorCodes::OPENSSL_ERROR, "EVP_CIPHER_CTX_new failed: {}", getOpenSSLErrors());
  ...
  if (!cipher) throw Exception(ErrorCodes::OPENSSL_ERROR, "EVP_CIPHER_fetch failed: {}", getOpenSSLErrors());
  ...
  if (EVP_EncryptInit_ex(ctx.get(), nullptr, nullptr, reinterpret_cast<const uint8_t *>(key.data()),
                         reinterpret_cast<const uint8_t *>(nonce.data())) != 1)
    throw Exception(ErrorCodes::OPENSSL_ERROR, "EVP_EncryptInit_ex failed: {}", getOpenSSLErrors());

  if (EVP_EncryptUpdate(ctx.get(), reinterpret_cast<uint8_t *>(ciphertext_and_tag), &out_len,
                        reinterpret_cast<const uint8_t *>(plaintext.data()), static_cast<int32_t>(plaintext.size())) != 1)
  ...
  if (EVP_EncryptFinal_ex(ctx.get(), reinterpret_cast<uint8_t *>(ciphertext_and_tag) + out_len, reinterpret_cast<int32_t *>(&out_len)) != 1)
...
size_t decrypt(std::string_view ciphertext, char *plaintext, EncryptionMethod method, const String &key, const String &nonce) {
  ...
  if (EVP_DecryptInit_ex(ctx.get(), nullptr, nullptr, reinterpret_cast<const uint8_t *>(key.data()),
                         reinterpret_cast<const uint8_t *>(nonce.data())) != 1)
    throw Exception(ErrorCodes::OPENSSL_ERROR, "EVP_DecryptInit_ex failed: {}", getOpenSSLErrors());
  ...
  if (EVP_DecryptUpdate(ctx.get(), reinterpret_cast<uint8_t *>(plaintext), reinterpret_cast<int32_t *>(&out_len),
                        reinterpret_cast<const uint8_t *>(ciphertext.data()), static_cast<int32_t>(ciphertext.size()) - tag_size) != 1)
  ...
  if (EVP_DecryptFinal_ex(ctx.get(), reinterpret_cast<uint8_t *>(plaintext) + out_len, reinterpret_cast<int32_t *>(&out_len)) != 1)
  ...
  factory.registerCompressionCodec(getMethodName(Method), method_code, [&, Method](const ASTPtr &arguments) -> CompressionCodecPtr {
    if (arguments) {
      if (!arguments->children.empty())
        throw Exception(ErrorCodes::ILLEGAL_SYNTAX_FOR_CODEC_TYPE, "Codec {} must not have parameters, given {}", getMethodName(Method),
  ...
  if (nonce != empty_nonce) {
    *dest = 1;
    ...
    if (copied_symbols != nonce.size())
      throw Exception(ErrorCodes::INCORRECT_DATA, "Can't copy nonce into destination. Count of copied symbols {}, need to copy {}",
...
void CompressionCodecEncrypted::Configuration::loadImpl(const Poco::Util::AbstractConfiguration &config, const String &config_prefix,
                                                        EncryptionMethod method, std::unique_ptr<Params> &new_params) {
  // if method is not smaller than MAX_ENCRYPTION_METHOD it is incorrect
  if (method >= MAX_ENCRYPTION_METHOD) throw Exception(ErrorCodes::BAD_ARGUMENTS, "Wrong argument for loading configurations.");
  ...
  for (const std::string &config_key : config_keys) {
    ...
    if (key.size() != methodKeySize(method))
      throw Exception(ErrorCodes::BAD_ARGUMENTS, "Got an encryption key with unexpected size {}, the size should be {}", key.size(),
                      methodKeySize(method));
  ...
  if (new_params->keys_storage[method].empty()) throw Exception(ErrorCodes::BAD_ARGUMENTS, "No keys, an encryption needs keys to work");
  ...
  if (new_params->nonce[method].size() != actual_nonce_size && !new_params->nonce[method].empty())
    throw Exception(ErrorCodes::BAD_ARGUMENTS, "Got nonce with unexpected size {}, the size should be {}", new_params->nonce[method].size(),
                    actual_nonce_size);
}
...
void CompressionCodecEncrypted::Configuration::load(const Poco::Util::AbstractConfiguration &config, const String &config_prefix) {
  ...
  static constexpr std::pair<std::string_view, EncryptionMethod> config_encryption_methods[] = {{".aes_128_gcm_siv", AES_128_GCM_SIV},
  ...
  for (const auto &config_encryption_method : config_encryption_methods) {
    ...
    if (config.has(encryption_method_key)) {
      if (!new_params) new_params = std::make_unique<Params>();
      loadImpl(config, encryption_method_key, config_encryption_method.second, new_params);
    }
  }

  if (new_params) params.set(std::move(new_params));
}

void CompressionCodecEncrypted::Configuration::getCurrentKeyAndNonce(EncryptionMethod method, UInt64 &current_key_id, String &current_key,
                                                                     String &nonce) const {
  /// It parameters were not set, throw exception
  if (!params.get()) throw Exception(ErrorCodes::BAD_ARGUMENTS, "Empty params in CompressionCodecEncrypted configuration");
  ...
  if (current_params->keys_storage[method].contains(current_key_id))
    ...
    throw Exception(ErrorCodes::BAD_ARGUMENTS, "There is no current_key {} in config. Please, put it in config and reload.",
                    current_key_id);
  ...
  if (nonce.empty()) nonce = empty_nonce;
}
...
uint8_t CompressionCodecEncrypted::getMethodByte() const { return getMethodCode(encryption_method); }

void CompressionCodecEncrypted::updateHash(SipHash &hash) const { getCodecDesc()->updateTreeHash(hash, /*ignore_aliases=*/true); }
...
UInt32 CompressionCodecEncrypted::doCompressData(const char *source, UInt32 source_size, char *dest) const {
  ...
  if (out_len != source_size + tag_size)
    throw Exception(ErrorCodes::LOGICAL_ERROR, "Can't encrypt data, length after encryption {} is wrong, expected {}", out_len,
                    source_size + tag_size);
...
UInt32 CompressionCodecEncrypted::doDecompressData(const char *source, UInt32 source_size, char *dest, UInt32 uncompressed_size) const {
  ...
  if (ciphertext_size != uncompressed_size + tag_size)
    throw Exception(ErrorCodes::LOGICAL_ERROR, "Can't decrypt data, uncompressed_size {} is wrong, expected {}", uncompressed_size,
                    ciphertext_size - tag_size);

  size_t out_len = decrypt({ciphertext, ciphertext_size}, dest, encryption_method, key, nonce);
  if (out_len != ciphertext_size - tag_size)
    throw Exception(ErrorCodes::LOGICAL_ERROR, "Can't decrypt data, out length after decryption {} is wrong, expected {}", out_len,
                    ciphertext_size - tag_size);
...
/// Register codec in factory
void registerEncryptionCodec(CompressionCodecFactory &factory, EncryptionMethod Method) {
  auto throw_no_ssl = [](const ASTPtr &) -> CompressionCodecPtr {
    throw Exception(ErrorCodes::OPENSSL_ERROR, "Server was built without SSL support. Encryption is disabled.");
  };
  const auto method_code = getMethodCode(Method);  /// Codec need to know its code
...
bool CompressionCodecEncrypted::Configuration::tryLoad(const Poco::Util::AbstractConfiguration &config [[maybe_unused]],
                                                       const String &config_prefix [[maybe_unused]]) {
  return false;
}

/// if encryption is disabled, print warning about this.
void CompressionCodecEncrypted::Configuration::load(const Poco::Util::AbstractConfiguration &config [[maybe_unused]],
                                                    const String &config_prefix [[maybe_unused]]) {
