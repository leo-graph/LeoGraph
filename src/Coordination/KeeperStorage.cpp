...
bool fixupACL(const std::vector<Coordination::ACL> &request_acls, int64_t session_id, const UncommittedState &uncommitted_state,
              bool block_acl, std::vector<Coordination::ACL> &result_acls) {
  if (block_acl || request_acls.empty()) return true;

  bool valid_found = false;
  for (const auto &request_acl : request_acls) {
    if (request_acl.scheme == "auth") {
      uncommitted_state.forEachAuthInSession(session_id, [&](const KeeperStorageBase::AuthID &auth_id) {
        valid_found = true;
      ...
      if (request_acl.permissions != Coordination::ACL::All) result_acls.push_back(request_acl);
      valid_found = true;
    } else if (request_acl.scheme == "digest") {
      Coordination::ACL new_acl = request_acl;

      /// Bad auth
      if (std::count(new_acl.id.begin(), new_acl.id.end(), ':') != 1) return false;
...
void unregisterEphemeralPath(KeeperStorageBase::Ephemerals &ephemerals, int64_t session_id, const std::string &path,
  ...
  if (ephemerals_it->second.empty()) ephemerals.erase(ephemerals_it);
}

KeeperResponsesForSessions processWatchesImplBase(const String &path, KeeperStorageBase::Watches &watches,
                                                  KeeperStorageBase::Watches &list_watches,
                                                  KeeperStorageBase::SessionAndWatcher &sessions_and_watchers,
                                                  Coordination::Event event_type, bool should_delete) {
  ...
  if (watch_it != watches.end()) {
    ...
    if (should_delete) watches.erase(watch_it);
  }
  ...
  for (const auto &path_to_check : paths_to_check_for_list_watches) {
    ...
    if (watch_it != list_watches.end()) {
      ...
      if (should_delete) list_watches.erase(watch_it);
    }
  }
...
std::pair<KeeperResponsesForSessions, Int64> processWatchesImpl(const String &path, KeeperStorageBase &storage,
                                                                Coordination::Event event_type) {
  KeeperResponsesForSessions result;

  auto process_non_persistent_watches = processWatchesImplBase(path, storage.watches, storage.list_watches, storage.sessions_and_watchers,
                                                               event_type, /*should_delete=*/true);
  auto process_persistent_watches = processWatchesImplBase(path, storage.persistent_watches, storage.persistent_list_watches,
                                                           storage.sessions_and_watchers, event_type, /*should_delete=*/false);
  ...
  if (!storage.persistent_recursive_watches.empty()) {
    ...
    while (true) {
      ...
      if (watch_it != storage.persistent_recursive_watches.end()) {
        ...
        watch_list_response->state = Coordination::State::CONNECTED;
        for (auto watcher_session : watch_it->second) result.push_back(KeeperResponseForSession{watcher_session, watch_list_response});
      }
...
void KeeperRocksNodeInfo::copyStats(const Coordination::Stat &stat) { stats.copyStats(stat); }

void KeeperRocksNode::invalidateDigestCache() const {
  if (serialized) throw Exception(ErrorCodes::LOGICAL_ERROR, "We modify node after serialized it");
  cached_digest = 0;
}

UInt64 KeeperRocksNode::getDigest(std::string_view path) const {
  if (!cached_digest) cached_digest = calculateDigest(path, *this);
...
KeeperMemNode::KeeperMemNode(const KeeperMemNode &other) { *this = other; }
...
KeeperMemNode::KeeperMemNode(KeeperMemNode &&other) noexcept { *this = std::move(other); }

bool KeeperMemNode::empty() const { return stats.data_size == 0 && stats.mzxid == 0; }

void KeeperMemNode::copyStats(const Coordination::Stat &stat) { stats.copyStats(stat); }
...
uint64_t KeeperMemNode::sizeInBytes() const { return sizeof(KeeperMemNode) + children.size() * sizeof(std::string_view) + stats.data_size; }
...
void KeeperMemNode::addChild(std::string_view child_path) { children.insert(child_path); }

void KeeperMemNode::removeChild(std::string_view child_path) { children.erase(child_path); }

void KeeperMemNode::invalidateDigestCache() const { cached_digest = 0; }

UInt64 KeeperMemNode::getDigest(const std::string_view path) const {
  if (cached_digest == 0) cached_digest = calculateDigest(path, *this);
...
struct UpdateNodeStatDelta {
  ...
  explicit UpdateNodeStatDelta(const Node &node) : old_stats(node.stats), new_stats(node.stats) {}
...
using Operation = std::variant<CreateNodeDelta, RemoveNodeDelta, UpdateNodeStatDelta, UpdateNodeDataDelta, SetACLDelta, AddAuthDelta,
                               ErrorDelta, SubDeltaEnd, FailedMultiDelta, CloseSessionDelta>;
...
KeeperStorageBase::DeltaIterator KeeperStorageBase::DeltaRange::begin() const { return begin_it; }

KeeperStorageBase::DeltaIterator KeeperStorageBase::DeltaRange::end() const { return end_it; }

bool KeeperStorageBase::DeltaRange::empty() const { return begin_it == end_it; }

const KeeperStorageBase::Delta &KeeperStorageBase::DeltaRange::front() const { return *begin_it; }
...
KeeperStorage<Container>::KeeperStorage(int64_t tick_time_ms, const String &superdigest_, const KeeperContextPtr &keeper_context_,
                                        const bool initialize_system_nodes)
    : KeeperStorageBase(tick_time_ms, keeper_context_, superdigest_) {
  if constexpr (use_rocksdb) container.initialize(keeper_context);
  Node root_node;
  container.insert("/", root_node);
  if constexpr (!use_rocksdb) addDigest(root_node, "/");

  if (initialize_system_nodes) initializeSystemNodes();
}

template <typename Container>
void KeeperStorage<Container>::initializeSystemNodes() {
  if (initialized) throw Exception(ErrorCodes::LOGICAL_ERROR, "KeeperStorage system nodes initialized twice");
  ...
  if (container.find(keeper_system_path) == container.end()) {
    ...
    if constexpr (!use_rocksdb) removeDigest(current_root_it->value, "/");
    auto updated_root_it = container.updateValue("/", [](KeeperStorage::Node &node) {
    ...
    if constexpr (!use_rocksdb) addDigest(updated_root_it->value, "/");
  }
  ...
  for (const auto &[path, data] : keeper_context->getSystemNodesWithData()) {
    ...
    if constexpr (use_rocksdb)
      ...
      container.updateValue(Coordination::parentNodePath(path), [child_path](auto &parent) {
...
template <typename Container>
std::shared_ptr<typename Container::Node> KeeperStorage<Container>::UncommittedState::tryGetNodeFromStorage(
    std::string_view path, bool should_lock_storage) const {
  std::shared_lock lock(storage.storage_mutex, std::defer_lock);
  if (should_lock_storage) lock.lock();
...
void KeeperStorage<Container>::UncommittedState::UncommittedNode::materializeACL(const ACLMap &current_acl_map) {
  if (!acls.has_value()) acls.emplace(current_acl_map.convertNumber(node->acl_id));
}
  ...
  std::visit(
      [&]<typename DeltaType>(const DeltaType &operation) {
        ...
        if constexpr (std::same_as<DeltaType, CreateNodeDelta>) {
          ...
          if (digest && !zxid_to_nodes[delta.zxid].contains(node_it)) *digest -= node->getDigest(delta.path);
...
template <typename Container>
bool KeeperStorage<Container>::UncommittedState::hasACL(int64_t session_id, bool is_local,
                                                        std::function<bool(const AuthID &)> predicate) const {
  const auto check_auth = [&](const auto &auth_ids) {
    for (const auto &auth : auth_ids) {
      ...
      if (predicate(*auth_ptr)) return true;
    }
    return false;
  };

  const auto check_session = [&](const auto &session_and_auth_map) {
    if (auto auth_it = session_and_auth_map.find(session_id); auth_it != session_and_auth_map.end()) return check_auth(auth_it->second);
    return false;
  };
  ...
  if (closed_sessions_to_zxids.contains(session_id)) return false;

  // check if there are uncommitted
  if (check_session(session_and_auth)) return true;
...
UInt64 KeeperStorage<Container>::UncommittedState::updateNodesDigest(UInt64 current_digest, UInt64 zxid) const {
  if (!storage.keeper_context->digestEnabled()) return current_digest;

  auto nodes_it = zxid_to_nodes.find(zxid);
  if (nodes_it == zxid_to_nodes.end()) return current_digest;
...
void KeeperStorage<Container>::UncommittedState::cleanup(int64_t commit_zxid) {
  for (auto it = zxid_to_nodes.begin(); it != zxid_to_nodes.end(); it = zxid_to_nodes.erase(it)) {
    ...
    if (transaction_zxid > commit_zxid) break;

    for (const auto node_it : transaction_nodes) {
      node_it->second.applied_zxids.erase(transaction_zxid);
      if (node_it->second.applied_zxids.empty()) nodes.erase(node_it);
    }
  }
  ...
  std::list<Delta> rollback_deltas;
  {
    ...
    if (!deltas.empty() && deltas.back().zxid > rollback_zxid)
      throw DB::Exception(
          ...
          deltas.back().zxid, rollback_zxid);

    auto delta_it = deltas.rbegin();
    for (; delta_it != deltas.rend(); ++delta_it) {
      if (delta_it->zxid != rollback_zxid) break;
    }
...
void KeeperStorage<Container>::UncommittedState::rollback(std::list<Delta> rollback_deltas) {
  ...
  for (auto delta_it = rollback_deltas.rbegin(); delta_it != rollback_deltas.rend(); ++delta_it) {
    ...
    if (!delta.path.empty()) {
          ...
          [&]<typename DeltaType>(const DeltaType &operation) {
            if constexpr (std::same_as<DeltaType, CreateNodeDelta>) {
              if (operation.stat.ephemeralOwner != 0)
                unregisterEphemeralPath(storage.uncommitted_state.ephemerals, operation.stat.ephemeralOwner, delta.path,
      ...
      if (uncommitted_auth.back().second == add_auth->auth_id) {
        ...
        if (uncommitted_auth.empty()) session_and_auth.erase(add_auth->session_id);
      }
      ...
      chassert(erased == 1);
      if (close_zxids.empty()) closed_sessions_to_zxids.erase(close_session->session_id);
    }
  }
  ...
  const auto cleanup_uncommitted_nodes = [&](const auto zxid) {
    ...
    if (it == zxid_to_nodes.end()) return;
...
template <typename Container>
std::shared_ptr<typename Container::Node> KeeperStorage<Container>::UncommittedState::getNode(std::string_view path,
                                                                                              bool should_lock_storage) const {
  if (auto node_it = nodes.find(path); node_it != nodes.end()) return node_it->second.node;
...
void KeeperStorage<Container>::UncommittedState::forEachAuthInSession(int64_t session_id, std::function<void(const AuthID &)> func) const {
  ...
  const auto call_for_each_auth = [&](const auto &auth_ids) {
    for (const auto &auth : auth_ids) {
      ...
      if (!auth_ptr->scheme.empty()) func(*auth_ptr);
    }
  };

  // for uncommitted
  if (auto auth_it = session_and_auth.find(session_id); auth_it != session_and_auth.end()) call_for_each_auth(auth_it->second);
...
[[noreturn]] void onStorageInconsistency(std::string_view message) {
  LOG_ERROR(getLogger("KeeperStorage"),
...
int64_t KeeperStorageBase::getNextZXIDLocked() const {
  if (uncommitted_transactions.empty()) return zxid + 1;
...
void KeeperStorage<Container>::applyUncommittedState(KeeperStorage &other, int64_t last_log_idx) TSA_NO_THREAD_SAFETY_ANALYSIS {
  ...
  for (const auto &transaction : uncommitted_transactions) {
    if (transaction.log_idx == 0) throw DB::Exception(ErrorCodes::LOGICAL_ERROR, "Transaction has log idx equal to 0");

    if (transaction.log_idx <= last_log_idx) continue;
  ...
  for (const auto &uncommitted_delta : uncommitted_state.deltas) {
    if (!zxids_to_apply.contains(uncommitted_delta.zxid)) continue;
...
Coordination::Error KeeperStorage<Container>::commit(KeeperStorageBase::DeltaRange deltas) {
  ...
  for (const auto &delta : deltas) {
    auto result = std::visit(
        [&, &path = delta.path]<typename DeltaType>(const DeltaType &operation) -> Coordination::Error {
          if constexpr (std::same_as<DeltaType, CreateNodeDelta>) {
            ...
            if (node_it == container.end()) onStorageInconsistency("Node to be updated is missing");
            ...
            if constexpr (!use_rocksdb) {
              if (digest_on_commit) removeDigest(node_it->value, path);
            }
            ...
            auto updated_node = container.updateValue(path, [&](auto &node) {
              ...
              if constexpr (!use_rocksdb) node.invalidateDigestCache();
            });

            if constexpr (!use_rocksdb) {
              if (digest_on_commit) addDigest(updated_node->value, path);
            }

            return Coordination::Error::ZOK;
          } else if constexpr (std::same_as<DeltaType, RemoveNodeDelta>) {
            if (!removeNode(path, operation.version, digest_on_commit)) onStorageInconsistency("Failed to remove node");

            return Coordination::Error::ZOK;
          } else if constexpr (std::same_as<DeltaType, SetACLDelta>) {
            auto node_it = container.find(path);
            if (node_it == container.end()) onStorageInconsistency("Failed to set ACL because node is missing");
    ...
    if (result != Coordination::Error::ZOK) return result;
  }
...
template <typename Container>
bool KeeperStorage<Container>::createNode(const std::string &path, String data, const Coordination::Stat &stat,
                                          Coordination::ACLs node_acls, bool update_digest) {
  auto parent_path = Coordination::parentNodePath(path);
  auto node_it = container.find(parent_path);

  if (node_it == container.end()) return false;

  if (node_it->value.stats.isEphemeral()) return false;

  if (container.contains(path)) return false;
  ...
  if constexpr (use_rocksdb) {
    ...
    /// Take child path from key owned by map.
    auto child_path = Coordination::getBaseNodeName(map_key->getKey());
    container.updateValue(parent_path, [child_path](KeeperMemNode &parent) {
      parent.addChild(child_path);
      chassert(parent.stats.numChildren() == static_cast<int32_t>(parent.getChildren().size()));
    });

    if (update_digest) addDigest(map_key->getMapped()->value, map_key->getKey());
  }
...
bool KeeperStorage<Container>::removeNode(const std::string &path, int32_t version, bool update_digest) {
  ...
  if (version != -1 && version != node_it->value.stats.version) return false;
  ...
  if constexpr (use_rocksdb)
    ...
    container.updateValue(Coordination::parentNodePath(path), [child_basename = Coordination::getBaseNodeName(node_it->key)](
                                                                  KeeperMemNode &parent) { parent.removeChild(child_basename); });

    container.erase(path);

    if (update_digest) removeDigest(prev_node, path);
  }
...
Coordination::ACLs getNodeACLs(Storage &storage, std::string_view path, bool is_local) {
  if (is_local) {
    ...
    if (node_it == storage.container.end()) return {};
...
bool KeeperStorage<Container>::checkACL(std::string_view path, int32_t permission, int64_t session_id, bool is_local) {
  ...
  if (node_acls.empty()) return true;

  if (uncommitted_state.hasACL(session_id, is_local, [](const auto &auth_id) { return auth_id.scheme == "super"; })) return true;

  for (const auto &node_acl : node_acls) {
    if (node_acl.permissions & permission) {
      if (node_acl.scheme == "world" && node_acl.id == "anyone") return true;

      if (uncommitted_state.hasACL(session_id, is_local,
...
Coordination::ZooKeeperResponsePtr processLocal(const T &zk_request, Storage & /*storage*/, KeeperStorageBase::DeltaRange /*deltas*/,
                                                int64_t /*session_id*/) {
  throw Exception(ErrorCodes::LOGICAL_ERROR, "Local processing not supported for request with type {}", zk_request.getOpNum());
}

template <std::derived_from<Coordination::ZooKeeperRequest> T, typename Storage>
std::list<KeeperStorageBase::Delta> preprocess(const T & /*zk_request*/, Storage & /*storage*/, int64_t /*zxid*/, int64_t /*session_id*/,
                                               int64_t /*time*/, uint64_t * /*digest*/, const KeeperContext & /*keeper_context*/) {
  return {};
}

template <std::derived_from<Coordination::ZooKeeperRequest> T, typename Storage>
std::pair<KeeperResponsesForSessions, Int64> processWatches(const T & /*zk_request*/, KeeperStorageBase::DeltaRange /*deltas*/,
                                                            Storage & /*storage*/, int64_t /*session_id*/) {
  return {};
}
...
/// HEARTBEAT Request ///
template <typename Storage>
Coordination::ZooKeeperResponsePtr process(const Coordination::ZooKeeperHeartbeatRequest &zk_request, Storage &storage,
                                           KeeperStorageBase::DeltaRange deltas, int64_t /*session_id*/) {
...
/// SYNC Request ///
template <typename Storage>
Coordination::ZooKeeperResponsePtr process(const Coordination::ZooKeeperSyncRequest &zk_request, Storage & /* storage */,
                                           KeeperStorageBase::DeltaRange /* deltas */, int64_t /*session_id*/) {
...
template <typename Storage>
std::pair<KeeperResponsesForSessions, Int64> processWatches(const Coordination::ZooKeeperCreateRequest &zk_request,
                                                            KeeperStorageBase::DeltaRange /*deltas*/, Storage &storage,
                                                            int64_t /*session_id*/) {
  return processWatchesImpl(zk_request.getPath(), storage, Coordination::Event::CREATED);
}

template <typename Storage>
std::list<KeeperStorageBase::Delta> preprocess(const Coordination::ZooKeeperCreateRequest &zk_request, Storage &storage, int64_t zxid,
                                               int64_t session_id, int64_t time, uint64_t * /*digest*/,
  ...
  if (parent_node == nullptr) return {KeeperStorageBase::Delta{zxid, Coordination::Error::ZNONODE}};

  if (parent_node->stats.isEphemeral()) return {KeeperStorageBase::Delta{zxid, Coordination::Error::ZNOCHILDRENFOREPHEMERALS}};

  std::string path_created = zk_request.path;
  if (zk_request.is_sequential) {
    if (zk_request.not_exists) return {KeeperStorageBase::Delta{zxid, Coordination::Error::ZBADARGUMENTS}};
  ...
  if (Coordination::matchPath(path_created, keeper_system_path) != Coordination::PathMatchResult::NOT_MATCH) {
    auto error_msg = fmt::format("Trying to create a node inside the internal Keeper path ({}) which is not allowed. Path: {}",
                                 keeper_system_path, path_created);
  ...
  if (storage.uncommitted_state.getNode(path_created)) {
    if (zk_request.getOpNum() == Coordination::OpNum::CreateIfNotExists) return new_deltas;

    return {KeeperStorageBase::Delta{zxid, Coordination::Error::ZNODEEXISTS}};
  }

  if (Coordination::getBaseNodeName(path_created).empty()) return {KeeperStorageBase::Delta{zxid, Coordination::Error::ZBADARGUMENTS}};
  ...
  if (zk_request.is_ephemeral) storage.uncommitted_state.ephemerals[session_id].emplace(path_created);
  ...
  if (zxid > update_parent_delta.old_stats.pzxid) update_parent_delta.new_stats.pzxid = zxid;
  ...
  new_deltas.emplace_back(std::move(path_created), zxid, CreateNodeDelta{stat, std::move(node_acls), zk_request.data});

  return new_deltas;
}

template <typename Storage>
Coordination::ZooKeeperResponsePtr process(const Coordination::ZooKeeperCreateRequest &zk_request, Storage &storage,
                                           KeeperStorageBase::DeltaRange deltas, int64_t /*session_id*/) {
  ...
  auto create_delta_it = std::find_if(deltas.begin(), deltas.end(),
                                      [](const auto &delta) { return std::holds_alternative<CreateNodeDelta>(delta.operation); });
...
template <typename Storage>
std::list<KeeperStorageBase::Delta> preprocess(const Coordination::ZooKeeperGetRequest &zk_request, Storage &storage, int64_t zxid,
                                               int64_t /*session_id*/, int64_t /*time*/, uint64_t * /*digest*/,
                                               const KeeperContext & /*keeper_context*/) {
  ProfileEvents::increment(ProfileEvents::KeeperGetRequest);

  if (zk_request.path == Coordination::keeper_api_feature_flags_path || zk_request.path == Coordination::keeper_config_path ||
      zk_request.path == Coordination::keeper_availability_zone_path)
    return {};

  if (!storage.uncommitted_state.getNode(zk_request.path)) return {KeeperStorageBase::Delta{zxid, Coordination::Error::ZNONODE}};

  return {};
}

template <bool local, typename Storage>
Coordination::ZooKeeperResponsePtr processImpl(const Coordination::ZooKeeperGetRequest &zk_request, Storage &storage,
                                               KeeperStorageBase::DeltaRange deltas, int64_t /*session_id*/) {
  ...
  if (zk_request.path == Coordination::keeper_config_path) {
    response->data = serializeClusterConfig(storage.keeper_context->getDispatcher()->getStateMachine().getClusterConfig());
    response->error = Coordination::Error::ZOK;
    return response;
  }
...
template <typename Storage>
Coordination::ZooKeeperResponsePtr process(const Coordination::ZooKeeperGetRequest &zk_request, Storage &storage,
                                           KeeperStorageBase::DeltaRange deltas, int64_t session_id) {
  return processImpl<false>(zk_request, storage, std::move(deltas), session_id);
}

template <typename Storage>
Coordination::ZooKeeperResponsePtr processLocal(const Coordination::ZooKeeperGetRequest &zk_request, Storage &storage,
                                                KeeperStorageBase::DeltaRange deltas, int64_t session_id) {
  ProfileEvents::increment(ProfileEvents::KeeperGetRequest);
...
template <typename Storage>
std::pair<KeeperResponsesForSessions, Int64> processWatches(const Coordination::ZooKeeperRemoveRequest &zk_request,
                                                            KeeperStorageBase::DeltaRange /*deltas*/, Storage &storage,
                                                            int64_t /*session_id*/) {
...
template <typename Storage>
std::list<KeeperStorageBase::Delta> preprocess(const Coordination::ZooKeeperRemoveRequest &zk_request, Storage &storage, int64_t zxid,
                                               int64_t /* session_id */, int64_t /* time */, uint64_t * /* digest */,
                                               const KeeperContext &keeper_context) {
  ...
  if (parent_node) update_parent_delta.emplace(*parent_node);

  const auto add_parent_update_delta = [&] {
    if (!update_parent_delta) return;
  ...
  const auto update_parent_pzxid = [&]() {
    ...
    if (update_parent_delta->old_stats.pzxid < zxid) update_parent_delta->new_stats.pzxid = zxid;
  };

  auto node = storage.uncommitted_state.getNode(zk_request.path);

  if (!node) {
    if (zk_request.try_remove) return {};
  ...
  if (zk_request.restored_from_zookeeper_log) update_parent_pzxid();
  ...
  if (node->stats.isEphemeral()) {
    ...
    unregisterEphemeralPath(storage.uncommitted_state.ephemerals, node->stats.ephemeralOwner(), zk_request.path,
                            /*throw_if_missing=*/false);
  }
...
template <typename Storage>
Coordination::ZooKeeperResponsePtr process(const Coordination::ZooKeeperRemoveRequest &zk_request, Storage &storage,
                                           KeeperStorageBase::DeltaRange deltas, int64_t /*session_id*/) {
...
class ToDeleteTreeCollector {
  ...
  struct PathCmp {
    auto operator()(const std::string_view a, const std::string_view b) const {
 ...
 public:
  ...
  ToDeleteTreeCollector(Storage &storage_, int64_t zxid_, int64_t session_id_, uint32_t limit_)
      : storage(storage_), zxid(zxid_), session_id(session_id_), limit(limit_) {}

  CollectStatus collect(std::string_view root_path, const Storage::Node &root_node) {
    if (checkLimits(root_node)) return CollectStatus::LimitExceeded;
    ...
    for (auto current_delta_it = deltas.rbegin(); current_delta_it != deltas.rend(); ++current_delta_it) {
      ...
      if (!storage.checkACL(current_path, Coordination::ACL::Delete, session_id, /*is_local=*/false)) return CollectStatus::NoAuth;

      ProcessedUncommittedChildren processed_uncommitted_children;
      if (auto status = visitUncommitted(current_path, processed_uncommitted_children); status != CollectStatus::Ok) return status;

      if constexpr (Storage::use_rocksdb) {
        if (auto status = visitRocksDBNode(current_path, processed_uncommitted_children); status != CollectStatus::Ok) return status;
      } else {
        if (auto status = visitMemNode(current_path, processed_uncommitted_children); status != CollectStatus::Ok) return status;
      }
    }
  ...
  std::list<KeeperStorageBase::Delta> extractDeltas() { return std::move(deltas); }

 private:
  CollectStatus visitRocksDBNode(std::string_view current_path, const ProcessedUncommittedChildren &processed_uncommitted_children)
    requires Storage::use_rocksdb
  {
    ...
    for (auto &&[child_name, child_node] : children) {
      ...
      if (processed_uncommitted_children.contains(child_path)) continue;

      if (checkLimits(child_node)) return CollectStatus::LimitExceeded;
    ...
    requires(!Storage::use_rocksdb)
  {
    ...
    if (node_it == storage.container.end()) return CollectStatus::Ok;
  ...
  CollectStatus visitUncommitted(const std::string &path, ProcessedUncommittedChildren &processed_uncommitted_children) {
    for (auto nodes_it = uncommitted_children.upper_bound(path + "/");
         nodes_it != uncommitted_children.end() && Coordination::parentNodePath(nodes_it->first) == path; ++nodes_it) {
      ...
      if (checkLimits(*node_ptr)) return CollectStatus::LimitExceeded;
...
template <typename Storage>
std::pair<KeeperResponsesForSessions, Int64> processWatches(const Coordination::ZooKeeperRemoveRecursiveRequest & /*zk_request*/,
                                                            KeeperStorageBase::DeltaRange deltas, Storage &storage,
                                                            int64_t /*session_id*/) {
...
template <typename Storage>
std::list<KeeperStorageBase::Delta> preprocess(const Coordination::ZooKeeperRemoveRecursiveRequest &zk_request, Storage &storage,
                                               int64_t zxid, int64_t session_id, int64_t /* time */, uint64_t * /* digest */,
                                               const KeeperContext &keeper_context) {
  ProfileEvents::increment(ProfileEvents::KeeperRemoveRequest);
  ...
  for (const auto &delta : delete_deltas) {
    ...
    if (remove_delta && remove_delta->stat.ephemeralOwner()) {
      unregisterEphemeralPath(storage.uncommitted_state.ephemerals, remove_delta->stat.ephemeralOwner(), delta.path,
                              /*throw_if_missing=*/false);
    }
  }
...
template <typename Storage>
Coordination::ZooKeeperResponsePtr process(const Coordination::ZooKeeperRemoveRecursiveRequest &zk_request, Storage &storage,
                                           KeeperStorageBase::DeltaRange deltas, int64_t /*session_id*/) {
  Coordination::ZooKeeperResponsePtr response_ptr = zk_request.makeResponse();
...
/// EXISTS Request ///
template <typename Storage>
std::list<KeeperStorageBase::Delta> preprocess(const Coordination::ZooKeeperExistsRequest &zk_request, Storage &storage, int64_t zxid,
                                               int64_t /*session_id*/, int64_t /*time*/, uint64_t * /*digest*/,
                                               const KeeperContext & /*keeper_context*/) {
...
template <bool local, typename Storage>
Coordination::ZooKeeperResponsePtr processImpl(const Coordination::ZooKeeperExistsRequest &zk_request, Storage &storage,
                                               KeeperStorageBase::DeltaRange deltas, int64_t /*session_id*/) {
...
template <typename Storage>
Coordination::ZooKeeperResponsePtr process(const Coordination::ZooKeeperExistsRequest &zk_request, Storage &storage,
                                           KeeperStorageBase::DeltaRange deltas, int64_t session_id) {
  return processImpl<false>(zk_request, storage, std::move(deltas), session_id);
}

template <typename Storage>
Coordination::ZooKeeperResponsePtr processLocal(const Coordination::ZooKeeperExistsRequest &zk_request, Storage &storage,
                                                KeeperStorageBase::DeltaRange deltas, int64_t session_id) {
  ProfileEvents::increment(ProfileEvents::KeeperExistsRequest);
  return processImpl<true>(zk_request, storage, std::move(deltas), session_id);
}
...
template <typename Storage>
std::pair<KeeperResponsesForSessions, Int64> processWatches(const Coordination::ZooKeeperSetRequest &zk_request,
                                                            KeeperStorageBase::DeltaRange /*deltas*/, Storage &storage,
                                                            int64_t /*session_id*/) {
...
template <typename Storage>
std::list<KeeperStorageBase::Delta> preprocess(const Coordination::ZooKeeperSetRequest &zk_request, Storage &storage, int64_t zxid,
                                               int64_t /* session_id */, int64_t time, uint64_t * /* digest */,
                                               const KeeperContext &keeper_context) {
...
template <typename Storage>
Coordination::ZooKeeperResponsePtr process(const Coordination::ZooKeeperSetRequest &zk_request, Storage &storage,
                                           KeeperStorageBase::DeltaRange deltas, int64_t /*session_id*/) {
  auto &container = storage.container;
...
template <bool local, typename Storage>
Coordination::ZooKeeperResponsePtr processImpl(const Coordination::ZooKeeperCheckWatchRequest &zk_request, Storage &storage,
                                               KeeperStorageBase::DeltaRange deltas, int64_t /*session_id*/) {
  auto response = zk_request.makeResponse();
  ...
  if (!storage.containsWatch(zk_request.getPath(), zk_request.type)) response->error = Coordination::Error::ZNOWATCHER;
  return response;
}

template <typename Storage>
Coordination::ZooKeeperResponsePtr process(const Coordination::ZooKeeperCheckWatchRequest &zk_request, Storage &storage,
                                           KeeperStorageBase::DeltaRange deltas, int64_t session_id) {
  return processImpl<false>(zk_request, storage, std::move(deltas), session_id);
}

template <typename Storage>
Coordination::ZooKeeperResponsePtr processLocal(const Coordination::ZooKeeperCheckWatchRequest &zk_request, Storage &storage,
                                                KeeperStorageBase::DeltaRange deltas, int64_t session_id) {
...
template <bool local, typename Storage>
Coordination::ZooKeeperResponsePtr processImpl(const Coordination::ZooKeeperRemoveWatchRequest &zk_request, Storage &storage,
                                               KeeperStorageBase::DeltaRange deltas, int64_t session_id) {
  auto response = zk_request.makeResponse();
  if constexpr (!local) {
    if (const auto result = storage.commit(std::move(deltas)); result != Coordination::Error::ZOK) {
  ...
  if (!storage.removePersistentWatch(zk_request.path, zk_request.type, session_id)) response->error = Coordination::Error::ZNOWATCHER;

  return response;
}

template <typename Storage>
Coordination::ZooKeeperResponsePtr process(const Coordination::ZooKeeperRemoveWatchRequest &zk_request, Storage &storage,
                                           KeeperStorageBase::DeltaRange deltas, int64_t session_id) {
  return processImpl<false>(zk_request, storage, std::move(deltas), session_id);
}

template <typename Storage>
Coordination::ZooKeeperResponsePtr processLocal(const Coordination::ZooKeeperRemoveWatchRequest &zk_request, Storage &storage,
                                                KeeperStorageBase::DeltaRange deltas, int64_t session_id) {
...
template <bool local, typename Storage>
Coordination::ZooKeeperResponsePtr processImpl(const Coordination::ZooKeeperAddWatchRequest &zk_request, Storage &storage,
                                               KeeperStorageBase::DeltaRange deltas, int64_t session_id) {
  auto response = zk_request.makeResponse();
  if constexpr (!local) {
    if (const auto result = storage.commit(std::move(deltas)); result != Coordination::Error::ZOK) {
...
template <typename Storage>
Coordination::ZooKeeperResponsePtr process(const Coordination::ZooKeeperAddWatchRequest &zk_request, Storage &storage,
                                           KeeperStorageBase::DeltaRange deltas, int64_t session_id) {
  return processImpl<false>(zk_request, storage, std::move(deltas), session_id);
}

template <typename Storage>
Coordination::ZooKeeperResponsePtr processLocal(const Coordination::ZooKeeperAddWatchRequest &zk_request, Storage &storage,
                                                KeeperStorageBase::DeltaRange deltas, int64_t session_id) {
...
template <typename Storage>
std::pair<KeeperResponsesForSessions, Int64> processWatches(const Coordination::ZooKeeperSetWatchesRequest &zk_request,
                                                            KeeperStorageBase::DeltaRange /*deltas*/, Storage &storage,
                                                            int64_t session_id) {
  auto watch_responses =
      storage.setWatches(zk_request.zxid, zk_request.data_watches, zk_request.child_watches, zk_request.exist_watches, {}, {}, session_id);
  return {std::move(watch_responses), 0};
}

template <bool local, typename Storage>
Coordination::ZooKeeperResponsePtr processImpl(const Coordination::ZooKeeperSetWatchesRequest &zk_request, Storage &storage,
                                               KeeperStorageBase::DeltaRange deltas, int64_t /*session_id*/) {
...
template <typename Storage>
Coordination::ZooKeeperResponsePtr process(const Coordination::ZooKeeperSetWatchesRequest &zk_request, Storage &storage,
                                           KeeperStorageBase::DeltaRange deltas, int64_t session_id) {
  return processImpl<false>(zk_request, storage, std::move(deltas), session_id);
}

template <typename Storage>
Coordination::ZooKeeperResponsePtr processLocal(const Coordination::ZooKeeperSetWatchesRequest &zk_request, Storage &storage,
                                                KeeperStorageBase::DeltaRange deltas, int64_t session_id) {
...
template <typename Storage>
std::pair<KeeperResponsesForSessions, Int64> processWatches(const Coordination::ZooKeeperSetWatches2Request &zk_request,
                                                            KeeperStorageBase::DeltaRange /*deltas*/, Storage &storage,
                                                            int64_t session_id) {
  auto watch_responses = storage.setWatches(zk_request.zxid, zk_request.data_watches, zk_request.child_watches, zk_request.exist_watches,
                                            zk_request.persistent_watches, zk_request.persistent_recursive_watches, session_id);
  return {std::move(watch_responses), 0};
}

template <bool local, typename Storage>
Coordination::ZooKeeperResponsePtr processImpl(const Coordination::ZooKeeperSetWatches2Request &zk_request, Storage &storage,
                                               KeeperStorageBase::DeltaRange deltas, int64_t /*session_id*/) {
  auto response_ptr = zk_request.makeResponse();
  response_ptr->error = storage.commit(deltas);
  return response_ptr;
...
template <typename Storage>
Coordination::ZooKeeperResponsePtr process(const Coordination::ZooKeeperSetWatches2Request &zk_request, Storage &storage,
                                           KeeperStorageBase::DeltaRange deltas, int64_t session_id) {
  return processImpl<false>(zk_request, storage, std::move(deltas), session_id);
}

template <typename Storage>
Coordination::ZooKeeperResponsePtr processLocal(const Coordination::ZooKeeperSetWatches2Request &zk_request, Storage &storage,
                                                KeeperStorageBase::DeltaRange deltas, int64_t session_id) {
  ProfileEvents::increment(ProfileEvents::KeeperSetWatchesRequest);
  return processImpl<true>(zk_request, storage, std::move(deltas), session_id);
}
...
template <typename Storage>
std::list<KeeperStorageBase::Delta> preprocess(const Coordination::ZooKeeperListRequest &zk_request, Storage &storage, int64_t zxid,
                                               int64_t /*session_id*/, int64_t /*time*/, uint64_t * /*digest*/,
                                               const KeeperContext & /*keeper_context*/) {
...
template <bool local, typename Storage>
Coordination::ZooKeeperResponsePtr processImpl(const Coordination::ZooKeeperListRequest &zk_request, Storage &storage,
                                               KeeperStorageBase::DeltaRange deltas, int64_t /*session_id*/) {
  ...
  if (node_it == container.end()) {
    ...
    if (path_prefix.empty()) throw DB::Exception(ErrorCodes::LOGICAL_ERROR, "Path cannot be empty");
    ...
    const auto get_children = [&]() {
      ...
      if constexpr (Storage::use_rocksdb)
        return std::optional{container.getChildren(
            zk_request.path, list_request_type != Coordination::ListRequestType::ALL || with_stat || with_data, with_data)};
      else
    ...
    if (with_stat) response->stats.reserve(children->size());
    if (with_data) response->data.reserve(children->size());

#ifdef DEBUG_OR_SANITIZER_BUILD
    if (!zk_request.path.starts_with(keeper_system_path) && static_cast<size_t>(node_it->value.stats.numChildren()) != children->size()) {
      throw Exception(ErrorCodes::LOGICAL_ERROR, "Difference between numChildren ({}) and actual children size ({}) for '{}'",
                      node_it->value.stats.numChildren(), children->size(), zk_request.path);
    }
#endif
    ...
    const auto add_child = [&](const auto &child) {
      ...
      if constexpr (!Storage::use_rocksdb) {
        ...
        if (child_it == container.end()) onStorageInconsistency("Failed to find a child");
    ...
    for (const auto &child : *children) {
      if (Coordination::ListRequestType::ALL == list_request_type || add_child(child)) {
        if constexpr (Storage::use_rocksdb) {
          ...
          if (with_data) response->data.emplace_back(child.second.getData());
        } else {
          ...
          if (with_stat || with_data) {
            auto child_path = (std::filesystem::path(zk_request.path) / child).generic_string();
            auto child_it = container.find(child_path);
            if (child_it == container.end()) onStorageInconsistency("Failed to find a child for stats/data");
            ...
            if (with_data) response->data.emplace_back(child_it->value.getData());
          }
        }
      }
    }
...
template <typename Storage>
Coordination::ZooKeeperResponsePtr process(const Coordination::ZooKeeperListRequest &zk_request, Storage &storage,
                                           KeeperStorageBase::DeltaRange deltas, int64_t session_id) {
  return processImpl<false>(zk_request, storage, std::move(deltas), session_id);
}

template <typename Storage>
Coordination::ZooKeeperResponsePtr processLocal(const Coordination::ZooKeeperListRequest &zk_request, Storage &storage,
                                                KeeperStorageBase::DeltaRange deltas, int64_t session_id) {
  ProfileEvents::increment(ProfileEvents::KeeperListRequest);
  return processImpl<true>(zk_request, storage, std::move(deltas), session_id);
}
...
bool checkAuth(const Coordination::ZooKeeperCheckRequest &zk_request, Storage &storage, int64_t session_id, bool is_local) {
  auto path = zk_request.getPath();
  return storage.checkACL(zk_request.getOpNum() == Coordination::OpNum::CheckNotExists ? Coordination::parentNodePath(path) : path,
                          Coordination::ACL::Read, session_id, is_local);
}
...
template <typename Storage>
std::list<KeeperStorageBase::Delta> preprocess(const Coordination::ZooKeeperCheckRequest &zk_request, Storage &storage, int64_t zxid,
                                               int64_t /*session_id*/, int64_t /*time*/, uint64_t * /*digest*/,
                                               const KeeperContext & /*keeper_context*/) {
  ...
  if (zk_request.getOpNum() == Coordination::OpNum::CheckNotExists) {
    ...
    if (!node) return {KeeperStorageBase::Delta{zxid, Coordination::Error::ZNONODE}};
...
template <bool local, typename Storage>
Coordination::ZooKeeperResponsePtr processImpl(const Coordination::ZooKeeperCheckRequest &zk_request, Storage &storage,
                                               KeeperStorageBase::DeltaRange deltas, int64_t /*session_id*/) {
  ...
  if (zk_request.getOpNum() == Coordination::OpNum::CheckNotExists) {
    ...
    else if (zk_request.getOpNum() == Coordination::OpNum::CheckStat &&
             !checkNodeStat(node_it->value.stats, zk_request.stat_to_check.value()))
      on_error(Coordination::Error::ZBADVERSION);
    else
      response->error = Coordination::Error::ZOK;
  }
...
template <typename Storage>
Coordination::ZooKeeperResponsePtr process(const Coordination::ZooKeeperCheckRequest &zk_request, Storage &storage,
                                           KeeperStorageBase::DeltaRange deltas, int64_t session_id) {
  return processImpl<false>(zk_request, storage, std::move(deltas), session_id);
}

template <typename Storage>
Coordination::ZooKeeperResponsePtr processLocal(const Coordination::ZooKeeperCheckRequest &zk_request, Storage &storage,
                                                KeeperStorageBase::DeltaRange deltas, int64_t session_id) {
  ProfileEvents::increment(ProfileEvents::KeeperCheckRequest);
  return processImpl<true>(zk_request, storage, std::move(deltas), session_id);
}
...
bool checkAuth(const Coordination::ZooKeeperMultiRequest &zk_request, Storage &storage, int64_t session_id, bool is_local) {
  for (const auto &concrete_request : zk_request.requests) {
    if (!callOnConcreteRequestType(*concrete_request,
...
template <typename Storage>
std::list<KeeperStorageBase::Delta> preprocess(const Coordination::ZooKeeperMultiRequest &zk_request, Storage &storage, int64_t zxid,
                                               int64_t session_id, int64_t time, uint64_t *digest, const KeeperContext &keeper_context) {
  ...
  for (size_t i = 0; i < subrequests.size(); ++i) {
    auto new_subdeltas = callOnConcreteRequestType(*subrequests[i], [&](const auto &subrequest) {
  ...
  if (digest) *digest = current_digest;
...
KeeperStorageBase::DeltaRange extractSubdeltas(KeeperStorageBase::DeltaRange &deltas) {
  ...
  for (; it != deltas.end(); ++it) {
    if (std::holds_alternative<SubDeltaEnd>(it->operation)) break;
  }
...
template <typename Storage>
Coordination::ZooKeeperResponsePtr process(const Coordination::ZooKeeperMultiRequest &zk_request, Storage &storage,
                                           KeeperStorageBase::DeltaRange deltas, int64_t session_id) {
  ...
  if (const auto *failed_multi = std::get_if<FailedMultiDelta>(&deltas.front().operation)) {
    ...
    for (size_t i = 0; i < subrequests_count; ++i) response->responses.push_back(std::make_shared<Coordination::ZooKeeperErrorResponse>());
...
template <typename Storage>
Coordination::ZooKeeperResponsePtr processLocal(const Coordination::ZooKeeperMultiRequest &zk_request, Storage &storage,
                                                KeeperStorageBase::DeltaRange deltas, int64_t session_id) {
...
template <typename Storage>
std::pair<KeeperResponsesForSessions, Int64> processWatches(const Coordination::ZooKeeperMultiRequest &zk_request,
                                                            KeeperStorageBase::DeltaRange deltas, Storage &storage, int64_t session_id) {
  KeeperResponsesForSessions result;

  if (deltas.empty() || std::get_if<FailedMultiDelta>(&deltas.front().operation)) return {result, 0};

  const auto &subrequests = zk_request.requests;
  Int64 total_removed_watches = 0;
  for (const auto &generic_request : subrequests) {
    auto subdeltas = extractSubdeltas(deltas);
    auto responses = callOnConcreteRequestType(*generic_request, [&](const auto &subrequest) {
...
/// AUTH Request ///
template <typename Storage>
std::list<KeeperStorageBase::Delta> preprocess(const Coordination::ZooKeeperAuthRequest &zk_request, Storage &storage, int64_t zxid,
                                               int64_t session_id, int64_t /*time*/, uint64_t * /*digest*/,
                                               const KeeperContext & /*keeper_context*/) {
...
template <typename Storage>
Coordination::ZooKeeperResponsePtr process(const Coordination::ZooKeeperAuthRequest & /*zk_request*/, Storage &storage,
                                           KeeperStorageBase::DeltaRange deltas, int64_t /*session_id*/) {
  auto response = std::make_shared<Coordination::ZooKeeperAuthResponse>();

  if (const auto result = storage.commit(std::move(deltas)); result != Coordination::Error::ZOK) response->error = result;
...
/// CLOSE Request ///
template <typename Storage>
Coordination::ZooKeeperResponsePtr process(const Coordination::ZooKeeperCloseRequest & /* zk_request */, Storage &,
                                           KeeperStorageBase::DeltaRange /* deltas */, int64_t /*session_id*/) {
...
template <typename Storage>
std::list<KeeperStorageBase::Delta> preprocess(const Coordination::ZooKeeperSetACLRequest &zk_request, Storage &storage, int64_t zxid,
                                               int64_t session_id, int64_t /*time*/, uint64_t * /*digest*/,
                                               const KeeperContext &keeper_context) {
  if (Coordination::matchPath(zk_request.path, keeper_system_path) != Coordination::PathMatchResult::NOT_MATCH) {
    auto error_msg = fmt::format("Trying to update an internal Keeper path ({}) which is not allowed", zk_request.path);
  ...
  if (!uncommitted_state.getNode(zk_request.path)) return {KeeperStorageBase::Delta{zxid, Coordination::Error::ZNONODE}};
...
template <typename Storage>
Coordination::ZooKeeperResponsePtr process(const Coordination::ZooKeeperSetACLRequest &zk_request, Storage &storage,
                                           KeeperStorageBase::DeltaRange deltas, int64_t /*session_id*/) {
  ...
  if (node_it == storage.container.end()) onStorageInconsistency("Failed to set ACL because node is missing");
  node_it->value.setResponseStat(response->stat);
  response->error = Coordination::Error::ZOK;
...
template <typename Storage>
std::list<KeeperStorageBase::Delta> preprocess(const Coordination::ZooKeeperGetACLRequest &zk_request, Storage &storage, int64_t zxid,
                                               int64_t /*session_id*/, int64_t /*time*/, uint64_t * /*digest*/,
                                               const KeeperContext & /*keeper_context*/) {
  if (!storage.uncommitted_state.getNode(zk_request.path)) return {KeeperStorageBase::Delta{zxid, Coordination::Error::ZNONODE}};
...
template <bool local, typename Storage>
Coordination::ZooKeeperResponsePtr processImpl(const Coordination::ZooKeeperGetACLRequest &zk_request, Storage &storage,
                                               KeeperStorageBase::DeltaRange deltas, int64_t /*session_id*/) {
...
template <typename Storage>
Coordination::ZooKeeperResponsePtr process(const Coordination::ZooKeeperGetACLRequest &zk_request, Storage &storage,
                                           KeeperStorageBase::DeltaRange deltas, int64_t session_id) {
  return processImpl<false>(zk_request, storage, std::move(deltas), session_id);
}

template <typename Storage>
Coordination::ZooKeeperResponsePtr processLocal(const Coordination::ZooKeeperGetACLRequest &zk_request, Storage &storage,
                                                KeeperStorageBase::DeltaRange deltas, int64_t session_id) {
  return processImpl<true>(zk_request, storage, std::move(deltas), session_id);
}
/// GETACL Request ///
...
void KeeperStorageBase::finalize() {
  if (finalized) throw DB::Exception(ErrorCodes::LOGICAL_ERROR, "KeeperStorage already finalized");
...
bool KeeperStorageBase::isFinalized() const { return finalized; }

template <typename Container>
KeeperDigest KeeperStorage<Container>::preprocessRequest(const Coordination::ZooKeeperRequestPtr &zk_request, int64_t session_id,
                                                         int64_t time, int64_t new_last_zxid, bool check_acl,
                                                         std::optional<KeeperDigest> digest, int64_t log_idx) {
  Stopwatch watch;
  SCOPE_EXIT({
    watch.stop();
    ...
    if (elapsed_ms > keeper_context->getCoordinationSettings()[CoordinationSetting::log_slow_cpu_threshold_ms]) {
      LOG_INFO(getLogger("KeeperStorage"), "Preprocessing a request took too long ({}ms).\nRequest info: {}", elapsed_ms,
  ...
  if (!initialized) throw Exception(ErrorCodes::LOGICAL_ERROR, "KeeperStorage system nodes are not initialized");
...

  {
    ...
    if (uncommitted_transactions.empty()) {
      ...
      if (last_zxid != old_snapshot_zxid && new_last_zxid <= last_zxid)
        throw Exception(ErrorCodes::LOGICAL_ERROR, "Got new ZXID ({}) smaller or equal to current ZXID ({}). It's a bug", new_last_zxid,
                        last_zxid);
    } else {
  ...
  SCOPE_EXIT({ chassert(finalized, "Finalize not called before returning"); });

  if (zk_request->getOpNum() == Coordination::OpNum::Close)  /// Close request is special
  {
    std::unordered_map<std::string, UpdateNodeStatDelta, StringHashForHeterogeneousLookup,
                       StringHashForHeterogeneousLookup::transparent_key_equal>
        parent_updates;

    const auto process_ephemerals_for_session = [&](const auto &current_ephemerals, auto &processed_ephemeral_nodes,
                                                    bool check_processed_nodes) {
      ...
      if (session_ephemerals != current_ephemerals.end()) {
        for (const auto &ephemeral_path : session_ephemerals->second) {
          if (check_processed_nodes) {
            if (processed_ephemeral_nodes.contains(ephemeral_path)) continue;
          } else {
          ...
          if (!node || node->stats.ephemeralOwner() != session_id) continue;
          ...
          new_deltas.emplace_back(
              ephemeral_path, transaction->zxid,
              RemoveNodeDelta{
                  .stat = node->stats, .acls = uncommitted_state.getACLs(ephemeral_path), .data = std::string{node->getData()}});
        }
      }
    };
...
template <typename Container>
KeeperResponsesForSessions KeeperStorage<Container>::processRequest(const Coordination::ZooKeeperRequestPtr &zk_request, int64_t session_id,
                                                                    std::optional<int64_t> new_last_zxid, bool check_acl, bool is_local) {
  ...
  SCOPE_EXIT({
    ...
    if (elapsed_ms > keeper_context->getCoordinationSettings()[CoordinationSetting::log_slow_cpu_threshold_ms]) {
      LOG_INFO(getLogger("KeeperStorage"), "Processing a request took too long ({}ms).\nRequest info: {}", elapsed_ms,
               zk_request->toString(/*short_format=*/true));
    }
  ...
  uint64_t transaction_digest = 0;
  {
    ...
    if (new_last_zxid) {
      ...
      if (front_transaction.zxid != *new_last_zxid)
        throw Exception(ErrorCodes::LOGICAL_ERROR, "Trying to commit a ZXID {} while the next ZXID to commit is {}", *new_last_zxid,
  ...
  if (zk_request->getOpNum() == Coordination::OpNum::Close)  /// Close request is special
  {
...

    {
      ...
      commit(deltas_range);
      if (!keeper_context->digestEnabledOnCommit()) nodes_digest = transaction_digest;
    }
    {
      std::lock_guard lock(auth_mutex);
      auto auth_it = committed_session_and_auth.find(session_id);
      if (auth_it != committed_session_and_auth.end()) committed_session_and_auth.erase(auth_it);
    }
    ...
    const auto process_request = [&]<std::derived_from<Coordination::ZooKeeperRequest> T>(T &concrete_zk_request) {
      ...
      if (is_local) {
        ...
        if (check_acl && !checkAuth(concrete_zk_request, *this, session_id, true)) {
          ...
          const auto maybe_log_opentelemetery_span = [&](OpenTelemetry::SpanStatus status, const std::string &error_message) {
            ZooKeeperOpentelemetrySpans::maybeInitialize(concrete_zk_request.spans.read_process, concrete_zk_request.tracing_context,
                                                         start_time_us);
      ...
      const auto update_watches = [&](const Coordination::ZooKeeperRequestPtr &req, const Coordination::ResponsePtr &resp) {
        if (resp->error == Coordination::Error::ZOK) {
          static constexpr std::array list_requests{Coordination::OpNum::List, Coordination::OpNum::SimpleList,
                                                    Coordination::OpNum::FilteredList, Coordination::OpNum::FilteredListWithStatsAndData};
...

  {
    ...
    if (new_last_zxid) uncommitted_transactions.pop_front();
...
void KeeperStorage<Container>::rollbackRequest(int64_t rollback_zxid, bool allow_missing) TSA_NO_THREAD_SAFETY_ANALYSIS {
  if (allow_missing && (uncommitted_transactions.empty() || uncommitted_transactions.back().zxid < rollback_zxid)) return;

  if (uncommitted_transactions.empty() || uncommitted_transactions.back().zxid != rollback_zxid) {
    throw Exception(ErrorCodes::LOGICAL_ERROR, "Trying to rollback invalid ZXID ({}). It should be the last preprocessed.", rollback_zxid);
  }
...
KeeperDigest KeeperStorageBase::getNodesDigest(bool committed, bool lock_transaction_mutex) const TSA_NO_THREAD_SAFETY_ANALYSIS {
  if (!keeper_context->digestEnabled()) return {.version = KeeperDigestVersion::NO_DIGEST};
  ...
  if (lock_transaction_mutex) transaction_lock.lock();

  if (uncommitted_transactions.empty()) {
    if (lock_transaction_mutex) transaction_lock.unlock();
    std::shared_lock storage_lock(storage_mutex);
    return {KEEPER_CURRENT_DIGEST_VERSION, nodes_digest};
  }
...
std::vector<int64_t> KeeperStorageBase::getDeadSessions() const { return session_expiry_queue.getExpiredSessions(); }

SessionAndTimeout KeeperStorageBase::getActiveSessions() const { return session_and_timeout; }
...
void KeeperStorageBase::clearDeadWatches(int64_t session_id) {
  ...
  if (watches_it == sessions_and_watchers.end()) return;
  ...
  for (auto watch_it = watches_it->second.begin(); watch_it != watches_it->second.end();) {
    auto erase_session_from_map = [&](auto &watch_map, const auto watch_path) {
      ...
      if (watches_for_path.empty()) watch_map.erase(it);
  ...
  if (watches_it->second.empty()) sessions_and_watchers.erase(watches_it);
}
...
void KeeperStorageBase::dumpWatches(WriteBufferFromOwnString &buf) const {
  for (const auto &[session_id, watches_paths] : sessions_and_watchers) {
    ...
    for (const auto [path, watch_type] : watches_paths) buf << "\t" << path << "\n";
  }
}
...
void KeeperStorageBase::dumpWatchesByPath(WriteBufferFromOwnString &buf) const {
  auto write_watches = [&](const auto &watch_map) {
    for (const auto &[watch_path, sessions] : watch_map) {
      buf << watch_path << "\n";
      for (int64_t session_id : sessions) buf << "\t0x" << getHexUIntLowercase(session_id) << "\n";
    }
  };
...
const KeeperStorageStats &KeeperStorageBase::getStorageStats() const { return stats; }

uint64_t KeeperStorageBase::getTotalWatchesCount() const {
#ifndef NDEBUG
  size_t actual_watches_count = 0;
  for (const auto &[_, watches_for_session] : sessions_and_watchers) actual_watches_count += watches_for_session.size();

  chassert(actual_watches_count == total_watches_count,
           fmt::format("Actual watches count {} does not match total watches count {}", actual_watches_count, total_watches_count));
#endif

  return total_watches_count;
}

uint64_t KeeperStorageBase::getSessionWithEphemeralNodesCount() const { return committed_ephemerals.size(); }

uint64_t KeeperStorageBase::getSessionsWithWatchesCount() const { return sessions_and_watchers.size(); }

uint64_t KeeperStorageBase::getTotalEphemeralNodesCount() const { return committed_ephemeral_nodes; }
...
bool KeeperStorageBase::checkDigest(const KeeperDigest &first, const KeeperDigest &second) {
  if (first.version != second.version) return true;

  if (first.version == KeeperDigestVersion::NO_DIGEST) return true;
...
template <typename Container>
KeeperResponsesForSessions KeeperStorage<Container>::setWatches(int64_t last_zxid, const std::vector<String> &watches_paths,
...
bool KeeperStorageBase::removePersistentWatch(const String &path, Coordination::RemoveWatchRequest::WatchType type, int64_t session_id) {
  auto erase_watch_for_session = [&](auto &watch_map, WatchType watch_type) {
    auto watches_it = sessions_and_watchers.find(session_id);
    if (watches_it == sessions_and_watchers.end()) return false;
    ...
    chassert(erased);
    if (watches_for_path.empty()) watch_map.erase(watch_it);
