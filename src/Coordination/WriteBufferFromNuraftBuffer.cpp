...
void WriteBufferFromNuraftBuffer::nextImpl() {
  if (finalized) throw Exception(ErrorCodes::CANNOT_WRITE_AFTER_END_OF_BUFFER, "WriteBufferFromNuraftBuffer is finished");
  ...
  internal_buffer = Buffer(reinterpret_cast<Position>(buffer->data_begin() + pos_offset),
                           reinterpret_cast<Position>(buffer->data_begin() + buffer->size()));
