...
class LoggerWrapper : public nuraft::logger {
 ...
 public:
  LoggerWrapper(const std::string& name, LogsLevel level_) : log(getLogger(name)), level(level_) {
    log->setLevel(static_cast<int>(LEVELS.at(level)));
  }

  void put_details(int level_, const char* /* source_file */, const char* /* func_name */, size_t /* line_number */,
