...
namespace {
void moveSnapshotBetweenDisks(DiskPtr disk_from, const std::string &path_from, DiskPtr disk_to, const std::string &path_to,
                              const KeeperContextPtr &keeper_context) {
  moveFileBetweenDisks(std::move(disk_from), path_from, std::move(disk_to), path_to,
                       /*before_file_remove_op=*/{}, getLogger("KeeperSnapshotManager"), keeper_context);
}
...
void writeNode(const Node &node, SnapshotVersion version, WriteBuffer &out) {
  ...
  if (version < SnapshotVersion::V6) writeBinary(false, out);
  ...
  if (version < SnapshotVersion::V6) writeBinary(static_cast<int32_t>(node.stats.data_size), out);
  writeBinary(node.stats.numChildren(), out);
  writeBinary(node.stats.pzxid, out);

  writeBinary(node.stats.seqNum(), out);

  if (version >= SnapshotVersion::V4 && version <= SnapshotVersion::V5) writeBinary(node.sizeInBytes(), out);
}
...
void readNode(Node &node, ReadBuffer &in, SnapshotVersion version, ACLMap &acl_map, bool cleanup_acl) {
  ...
  if (version >= SnapshotVersion::V1) {
    ...
    if (!cleanup_acl) node.acl_id = acl_map.convertACLs(acls);
  }

  /// Some strange ACLID during deserialization from ZooKeeper
  if (node.acl_id == std::numeric_limits<uint64_t>::max()) node.acl_id = 0;
  ...
  if (ephemeral_owner != 0) node.stats.setEphemeralOwner(ephemeral_owner);
  ...
  if (ephemeral_owner == 0) node.stats.setNumChildren(num_children);

  readBinary(node.stats.pzxid, in);

  int32_t seq_num = 0;
  readBinary(seq_num, in);
  if (ephemeral_owner == 0) node.stats.setSeqNum(seq_num);
...
void KeeperStorageSnapshot<Storage>::serialize(const KeeperStorageSnapshot<Storage> &snapshot, WriteBuffer &out,
  ...
  for (auto it = snapshot.begin; counter < snapshot.snapshot_container_size; ++counter) {
    ...
    if (Coordination::matchPath(path, keeper_system_path) == Coordination::PathMatchResult::IS_CHILD) {
      if (counter == snapshot.snapshot_container_size - 1) break;
    ...
    if (node.stats.mzxid > snapshot.zxid) break;
  ...
  std::vector<std::pair<int64_t, int64_t>> sorted_session_and_timeout(snapshot.session_and_timeout.begin(),
                                                                      snapshot.session_and_timeout.end());
  ...
  for (const auto &[session_id, timeout] : sorted_session_and_timeout) {
    ...
    if (snapshot.session_and_auth.contains(session_id)) ids = snapshot.session_and_auth.at(session_id);
...
template <typename Storage>
void KeeperStorageSnapshot<Storage>::deserialize(SnapshotDeserializationResult<Storage> &deserialization_result, ReadBuffer &in,
                                                 KeeperContextPtr keeper_context, bool load_full_storage) TSA_NO_THREAD_SAFETY_ANALYSIS {
  uint8_t version;
  ...
  if (current_version >= SnapshotVersion::V1) {
    ...
    while (current_map_size < acls_map_size) {
      ...
      if (!keeper_context->shouldBlockACL()) storage.acl_map.addMapping(acl_id, acls);
      current_map_size++;
    }
  }
  ...
  if constexpr (!use_rocksdb) storage.container.reserve(snapshot_container_size);

  if (recalculate_digest) storage.nodes_digest = 0;

  auto batch_load_size = keeper_context->getCoordinationSettings()[CoordinationSetting::rocksdb_load_batch_size];
  if constexpr (use_rocksdb) storage.container.startLoading(batch_load_size);
  ...
  for (size_t nodes_read = 0; nodes_read < snapshot_container_size; ++nodes_read) {
    ...
    if constexpr (!use_rocksdb)
      if (!node.stats.isEphemeral() && node.stats.numChildren() > 0) node.getChildren().reserve(node.stats.numChildren());
    ...
    if (recalculate_digest) storage.nodes_digest += node.getDigest(path);
    ...
    for (const auto &itr : storage.container) {
      if (itr.key != "/") {
        if (itr.value.stats.numChildren() != static_cast<int32_t>(itr.value.getChildren().size())) {
          ...
          LOG_ERROR(getLogger("KeeperSnapshotManager"),
                    "Children counter in stat.numChildren {}"
                    " is different from actual children size {} for node {}",
                    itr.value.stats.numChildren(), itr.value.getChildren().size(), itr.key);
#else
          throw Exception(ErrorCodes::LOGICAL_ERROR,
                          "Children counter in stat.numChildren {}"
                          " is different from actual children size {} for node {}",
  ...
  while (current_session_size < active_sessions_size) {
    ...
    if (current_version >= SnapshotVersion::V1) {
      ...
      if (!ids.empty()) storage.committed_session_and_auth[active_session_id] = ids;
    }
...
template <typename Storage>
KeeperStorageSnapshot<Storage>::KeeperStorageSnapshot(Storage *storage_, const SnapshotMetadataPtr &snapshot_meta_,
                                                      const ClusterConfigPtr &cluster_config_)
    : storage(storage_),
...
template <typename Storage>
KeeperSnapshotManager<Storage>::KeeperSnapshotManager(size_t snapshots_to_keep_, const KeeperContextPtr &keeper_context_,
                                                      bool compress_snapshots_zstd_, const std::string &superdigest_,
  ...
  const auto load_snapshot_from_disk = [&](const auto &disk) {
    ...
    for (auto it = disk->iterateDirectory(""); it->isValid(); it->next()) {
      ...
      if (it->name().starts_with("snapshot_") && !clean_incomplete_file(it->path())) snapshot_files.push_back(it->path());
    }

    for (const auto &snapshot_file : snapshot_files) {
      if (clean_incomplete_file(fs::path(snapshot_file).filename())) continue;
      ...
      if (!inserted)
        LOG_WARNING(log, "Found another snapshots with last log idx {}, will use snapshot from disk {}", snapshot_up_to, disk->getName());
    }

    for (const auto &[name, path] : incomplete_files) disk->removeFile(path);

    if (snapshot_files.empty()) LOG_TRACE(log, "No snapshots were found on {}", disk->getName());

    read_disks.insert(disk);
  };

  for (const auto &disk : keeper_context->getOldSnapshotDisks()) load_snapshot_from_disk(disk);

  auto disk = getDisk();
  load_snapshot_from_disk(disk);

  auto latest_snapshot_disk = getLatestSnapshotDisk();
  if (latest_snapshot_disk != disk) load_snapshot_from_disk(latest_snapshot_disk);
...
template <typename Storage>
nuraft::ptr<nuraft::buffer> KeeperSnapshotManager<Storage>::serializeSnapshotToBuffer(
    const KeeperStorageSnapshot<Storage> &snapshot) const {
...
template <typename Storage>
SnapshotDeserializationResult<Storage> KeeperSnapshotManager<Storage>::deserializeSnapshotFromBuffer(nuraft::ptr<nuraft::buffer> buffer,
                                                                                                     bool load_full_storage) const {
...
void KeeperSnapshotManager<Storage>::removeOutdatedSnapshotsIfNeeded() {
  while (existing_snapshots.size() > snapshots_to_keep) removeSnapshot(existing_snapshots.begin()->first);
}
...
void KeeperSnapshotManager<Storage>::removeSnapshot(uint64_t log_idx) {
  ...
  if (itr == existing_snapshots.end()) throw Exception(ErrorCodes::UNKNOWN_SNAPSHOT, "Unknown snapshot with log index {}", log_idx);
...
size_t KeeperSnapshotManager<Storage>::getLatestSnapshotIndex() const {
  if (!existing_snapshots.empty()) return existing_snapshots.rbegin()->first;
  return 0;
}
...
SnapshotFileInfoPtr KeeperSnapshotManager<Storage>::getLatestSnapshotInfo() const {
  if (!existing_snapshots.empty()) {
    ...
    try {
      if (disk->existsFile(path)) return std::make_shared<SnapshotFileInfo>(path, disk);
    } catch (...) {
