...
struct ListNode {
  ...
  ListNode copyFromSnapshotNode() { return {key, value.copyFromSnapshotNode(), node_metadata}; }

  void setInactiveInMap() { node_metadata.active_in_map = false; }

  void setActiveInMap() { node_metadata.active_in_map = true; }

  bool isActiveInMap() { return node_metadata.active_in_map; }

  void setFreeKey() { node_metadata.free_key = true; }

  bool getFreeKey() { return node_metadata.free_key; }

  uint64_t getVersion() { return node_metadata.version; }

  void setVersion(uint64_t version) { node_metadata.version = version; }
};
...
class SnapshotableHashTable {
 private:
  struct GlobalArena {
    char *alloc(const size_t size) { return new char[size]; }

    void free(const char *ptr, size_t /*size*/) { delete[] ptr; }
  };
  ...
  void insertOrReplace(std::string_view key, V value, bool owns_key) {
    ...
    if (it == map.end()) {
      ...
      if (owns_key) arena.free(key.data(), key.size());
 ...
 public:
  ...
  void insertOrReplace(const std::string &key, V value) { insertOrReplace(key, std::move(value), /*owns_key*/ false); }

  struct KeyDeleter {
    void operator()(const char *key) {
      if (key) arena->free(key, size);
    }
  ...
  bool contains(const std::string &key) const { return map.find(key) != map.end(); }

  const_iterator updateValue(std::string_view key, ValueUpdater updater) {
    size_t hash_value = map.hash(key);
    auto it = map.find(key, hash_value);
    if (it == map.end()) throw Exception(ErrorCodes::LOGICAL_ERROR, "Could not find key: '{}'", key);
  ...
  void clearOutdatedNodes() {
    for (auto &itr : snapshot_invalid_iters) {
      if (itr->isActiveInMap()) throw Exception(ErrorCodes::LOGICAL_ERROR, "{} is not active in map", itr->key);
      updateDataSize(ERASE, itr->key.size(), 0, itr->value.sizeInBytes(), /*remove_old=*/true);
      if (itr->getFreeKey()) arena.free(const_cast<char *>(itr->key.data()), itr->key.size());
      list.erase(itr);
    }
  ...
  void clear() {
    clearOutdatedNodes();
    map.clear();
    for (auto itr = list.begin(); itr != list.end(); ++itr) arena.free(const_cast<char *>(itr->key.data()), itr->key.size());
    list.clear();
  ...
  void disableSnapshotMode() { snapshot_mode = false; }

  size_t size() const { return map.size(); }

  std::pair<size_t, size_t> snapshotSizeWithVersion() const { return std::make_pair(list.size(), current_version); }

  uint64_t getApproximateDataSize() const { return approximate_data_size; }
