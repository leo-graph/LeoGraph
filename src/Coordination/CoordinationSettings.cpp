...
#define LIST_OF_COORDINATION_SETTINGS(DECLARE, ALIAS)                                                                                      \
  ...
  DECLARE(Milliseconds, leadership_expiry_ms, 0,                                                                                           \
          "Duration after which a leader will expire if it fails to receive responses from peers. Set it lower or equal to "               \
          "election_timeout_lower_bound_ms to avoid multiple leaders.",                                                                    \
          0)                                                                                                                               \
  ...
  DECLARE(Milliseconds, sleep_before_leader_change_ms, 8000,                                                                               \
          "How much time we will wait before removing leader (so as leader could commit accepted but non-committed commands and they "     \
          "won't be lost -- leader removal is not synchronized with committing)",                                                          \
          0)                                                                                                                               \
  DECLARE(LogsLevel, raft_logs_level, LogsLevel::information,                                                                              \
          "Log internal RAFT logs into main server log level. Valid values: 'trace', 'debug', 'information', 'warning', 'error', "         \
          "'fatal', 'none'",                                                                                                               \
          0)                                                                                                                               \
  ...
  DECLARE(UInt64, max_requests_append_bytes_size, 10 * 1024 * 1024,                                                                        \
          "Max size in bytes of batch of requests that can be sent to replica in append request", 0)                                       \
  DECLARE(UInt64, max_flush_batch_size, 1000, "Max size of batch of requests that can be flushed together", 0)                             \
  DECLARE(UInt64, max_requests_quick_batch_size, 100,                                                                                      \
          "Max size of batch of requests to try to get before proceeding with RAFT. Keeper will not wait for requests but take only "      \
          "requests that are already in queue",                                                                                            \
          0)                                                                                                                               \
  ...
  DECLARE(UInt64, configuration_change_tries_count, 20,                                                                                    \
          "How many times we will try to apply configuration change (add/remove server) to the cluster", 0)                                \
  DECLARE(UInt64, max_log_file_size, 50 * 1024 * 1024,                                                                                     \
          "Max size of the Raft log file. If possible, each created log file will preallocate this amount of bytes on disk. Set to 0 to "  \
          "disable the limit",                                                                                                             \
          0)                                                                                                                               \
  DECLARE(UInt64, log_file_overallocate_size, 50 * 1024 * 1024,                                                                            \
          "If max_log_file_size is not set to 0, this value will be added to it for preallocating bytes on disk. If a log record is "      \
          "larger than this value, it could lead to uncaught out-of-space issues so a larger value is preferred",                          \
          0)                                                                                                                               \
  DECLARE(UInt64, min_request_size_for_cache, 50 * 1024,                                                                                   \
          "Minimal size of the request to cache the deserialization result. Caching can have negative effect on latency for smaller "      \
          "requests, set to 0 to disable",                                                                                                 \
          0)                                                                                                                               \
  DECLARE(                                                                                                                                 \
      UInt64, raft_limits_reconnect_limit, 50,                                                                                             \
      "If connection to a peer is silent longer than this limit * (multiplied by heartbeat interval), we re-establish the connection.", 0) \
  DECLARE(UInt64, raft_limits_response_limit, 20,                                                                                          \
          "Total wait time for a response is calculated by multiplying response_limit with heart_beat_interval_ms", 0)                     \
  DECLARE(Bool, async_replication, true,                                                                                                   \
          "Enable async replication. All write and read guarantees are preserved while better performance is achieved.", 0)                \
  ...
  DECLARE(UInt64, latest_logs_cache_entry_count_threshold, 200'000, "Maximum number of entries in in-memory cache of latest log entries.", \
          0)                                                                                                                               \
  DECLARE(UInt64, commit_logs_cache_size_threshold, 500_MiB,                                                                               \
          "Maximum total size of in-memory cache of log entries needed next for commit.", 0)                                               \
  DECLARE(UInt64, commit_logs_cache_entry_count_threshold, 100'000,                                                                        \
          "Maximum number of entries in in-memory cache of log entries needed next for commit.", 0)                                        \
  DECLARE(UInt64, disk_move_retries_wait_ms, 1000,                                                                                         \
          "How long to wait between retries after a failure which happened while a file was being moved between disks.", 0)                \
  DECLARE(UInt64, disk_move_retries_during_init, 100,                                                                                      \
          "The amount of retries after a failure which happened while a file was being moved between disks during initialization.", 0)     \
  DECLARE(UInt64, log_slow_total_threshold_ms, 5000, "Requests for which the total latency is larger than this settings will be logged",   \
          0)                                                                                                                               \
  DECLARE(UInt64, log_slow_cpu_threshold_ms, 100,                                                                                          \
          "Requests for which the CPU (preprocessing and processing) latency is larger than this settings will be logged", 0)              \
  DECLARE(UInt64, log_slow_connection_operation_threshold_ms, 1000,                                                                        \
          "Log message if a certain operation took too long inside a single connection", 0)                                                \
  DECLARE(Bool, use_xid_64, false, "Enable 64-bit XID. It is disabled by default because of backward compatibility", 0)                    \
  DECLARE(Bool, check_node_acl_on_remove, false,                                                                                           \
          "When trying to remove a node, check ACLs from both the node itself and the parent node. If disabled, default behaviour will "   \
          "be used where only ACL from the parent node is checked",                                                                        \
          0)
...
void CoordinationSettingsImpl::loadFromConfig(const String& config_elem, const Poco::Util::AbstractConfiguration& config) {
  ...
  try {
    for (const String& key : config_keys) set(key, config.getString(config_elem + "." + key));
  } catch (Exception& e) {
    if (e.code() == ErrorCodes::UNKNOWN_SETTING) e.addMessage("in Coordination settings config");
    throw;
  }
  ...
  if (!max_requests_append_size.changed) max_requests_append_size = max_requests_batch_size;
}
...
KeeperConfigurationAndSettingsPtr KeeperConfigurationAndSettings::loadFromConfig(const Poco::Util::AbstractConfiguration& config,
                                                                                 bool standalone_keeper_) {
  ...
  ret->four_letter_word_allow_list =
      config.getString("keeper_server.four_letter_word_allow_list",
                       config.getString("keeper_server.four_letter_word_white_list", DEFAULT_FOUR_LETTER_WORD_CMD));
