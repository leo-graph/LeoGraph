...
struct KeeperSnapshotManagerS3::S3Configuration {
  S3Configuration(S3::URI uri_, S3::S3AuthSettings auth_settings_, std::shared_ptr<const S3::Client> client_)
      : uri(std::move(uri_)), auth_settings(std::move(auth_settings_)), client(std::move(client_)) {}
...
KeeperSnapshotManagerS3::KeeperSnapshotManagerS3()
    : snapshots_s3_queue(std::numeric_limits<size_t>::max()), log(getLogger("KeeperSnapshotManagerS3")), uuid(UUIDHelpers::generateV4()) {}
...
void KeeperSnapshotManagerS3::updateS3Configuration(const Poco::Util::AbstractConfiguration& config,
  ...
  try {
    ...
    if (!config.has(config_prefix)) {
      ...
      if (snapshot_s3_client) LOG_INFO(log, "S3 configuration was removed");
    ...
    auto credentials =
        Aws::Auth::AWSCredentials(auth_settings[S3AuthSetting::access_key_id], auth_settings[S3AuthSetting::secret_access_key],
    ...
    S3::PocoHTTPClientConfiguration client_configuration = S3::ClientFactory::instance().createClientConfiguration(
        auth_settings[S3AuthSetting::region], RemoteHostFilter(), s3_max_redirects,
        S3::PocoHTTPClientConfiguration::RetryStrategy{.max_retries = s3_retry_attempts}, s3_slow_all_threads_after_network_error,
        s3_slow_all_threads_after_retryable_error, enable_s3_requests_logging,
        /* for_disk_s3 = */ false,
        /* opt_disk_name = */ {},
        /* request_throttler = */ {}, new_uri.uri.getScheme());
    ...
    auto client = S3::ClientFactory::instance().create(client_configuration, client_settings, credentials.GetAWSAccessKeyId(),
                                                       credentials.GetAWSSecretKey(),
                                                       auth_settings[S3AuthSetting::server_side_encryption_customer_key_base64],
                                                       auth_settings.server_side_encryption_kms_config, std::move(headers),
    ...
    auto new_client =
        std::make_shared<KeeperSnapshotManagerS3::S3Configuration>(std::move(new_uri), std::move(auth_settings), std::move(client));
...
void KeeperSnapshotManagerS3::uploadSnapshotImpl(const SnapshotFileInfo& snapshot_file_info) {
  ...
  try {
    ...
    if (s3_client == nullptr) return;

    S3::S3RequestSettings request_settings_1;

    const auto create_writer = [&](const auto& key) {
      /// blob_storage_log is not used for keeper
      return WriteBufferFromS3(s3_client->client, s3_client->uri.bucket, key, DBMS_DEFAULT_BUFFER_SIZE, request_settings_1,
                               /* blob_log */ {});
    };
    ...
    ReadBufferFromS3 lock_reader{s3_client->client, s3_client->uri.bucket, lock_file, "", request_settings_2, {}};
...
void KeeperSnapshotManagerS3::snapshotS3Thread() {
  ...
  while (!shutdown_called) {
    ...
    if (!snapshots_s3_queue.pop(snapshot_file_info)) break;
...
void KeeperSnapshotManagerS3::uploadSnapshot(const SnapshotFileInfoPtr& file_info, bool async_upload) {
  ...
  if (getSnapshotS3Client() == nullptr) return;

  if (async_upload) {
    if (!snapshots_s3_queue.push(file_info)) LOG_WARNING(log, "Failed to add snapshot {} to S3 queue", file_info->path);
...
void KeeperSnapshotManagerS3::shutdown() {
  ...
  try {
    ...
    if (snapshot_s3_thread.joinable()) snapshot_s3_thread.join();
  } catch (...) {
    tryLogCurrentException(__PRETTY_FUNCTION__);
  }
