...
auto getSslContextProvider(const Poco::Util::AbstractConfiguration &config, std::string_view key) {
  ...
  if (config.has(certificate_file_property)) params.certificateFile = config.getString(certificate_file_property);

  if (config.has(private_key_file_property)) params.privateKeyFile = config.getString(private_key_file_property);

  std::shared_ptr<CertificateReloader::Data> certificate_data;
  if (config.has(private_key_passphrase_property)) {
    certificate_data = std::make_shared<CertificateReloader::Data>(params.certificateFile, params.privateKeyFile,
                                                                   config.getString(private_key_passphrase_property));
    params.certificateFile.clear();
    params.privateKeyFile.clear();
  }

  if (config.has(root_ca_file_property)) params.caLocation = config.getString(root_ca_file_property);
  ...
  if (config.has(cipher_list_property)) params.cipherList = config.getString(cipher_list_property);

  String dh_params_file_property = fmt::format("openSSL.{}.dhParamsFile", key);
  if (config.has(dh_params_file_property)) params.dhParamsFile = config.getString(dh_params_file_property);
  ...
  return [params, disabled_protocols, prefer_server_cypher, cache_sessions, is_server = key == "server", certificate_data] {
    ...
    if (prefer_server_cypher) context.preferServerCiphers();

    if (cache_sessions) context.enableSessionCache();
    ...
    if (certificate_data) {
      ...
      if (auto err = SSL_CTX_use_PrivateKey(ssl_ctx, const_cast<EVP_PKEY *>(static_cast<const EVP_PKEY *>(certificate_data->key)));
          err != 1)
...
std::string checkAndGetSuperdigest(const String &user_and_digest) {
  if (user_and_digest.empty()) return "";
...
int32_t getValueOrMaxInt32AndLogWarning(uint64_t value, const std::string &name, LoggerPtr log) {
  if (value > std::numeric_limits<int32_t>::max()) {
    LOG_WARNING(log, "Got {} value for setting '{}' which is bigger than int32_t max value, lowering value to {}.", value, name,
...
KeeperServer::KeeperServer(const KeeperConfigurationAndSettingsPtr &configuration_and_settings_,
                           const Poco::Util::AbstractConfiguration &config, ResponsesQueue &responses_queue_,
                           SnapshotsQueue &snapshots_queue_, KeeperContextPtr keeper_context_, KeeperSnapshotManagerS3 &snapshot_manager_s3,
  ...
  if (coordination_settings[CoordinationSetting::experimental_use_rocksdb]) {
    state_machine = nuraft::cs_new<KeeperStateMachine<KeeperRocksStorage>>(
        responses_queue_, snapshots_queue_, keeper_context,
        config.getBool("keeper_server.upload_snapshot_on_exit", false) ? &snapshot_manager_s3 : nullptr, commit_callback,
  ...
  state_manager = nuraft::cs_new<KeeperStateManager>(server_id, "keeper_server", "state", config, keeper_context);
}
...
struct KeeperServer::KeeperRaftServer : public nuraft::raft_server {
  bool isClusterHealthy() {
    if (timer_from_init) {
      ...
      if (timer_from_init->elapsedMilliseconds() < expiry) return false;
  ...
  void setConfig(const nuraft::ptr<nuraft::cluster_config> &new_config) { set_config(new_config); }

  // Manually reconfigure the cluster
  // This should be used only for recovery
  void forceReconfigure(const nuraft::ptr<nuraft::cluster_config> &new_config) { reconfigure(new_config); }
  ...
  std::unique_lock<std::recursive_mutex> lockRaft() { return std::unique_lock(lock_); }

  std::unique_lock<std::mutex> lockCommit() { return std::unique_lock(commit_lock_); }

  bool isCommitInProgress() const { return sm_commit_exec_in_progress_; }

  void setServingRequest(bool value) { serving_req_ = value; }
  ...
  std::unordered_set<int32_t> getLearnerIds() {
    ...
    for (const auto &cfg : configs)
      if (cfg->is_learner()) learner_ids.insert(cfg->get_id());
    return learner_ids;
  }
  ...
  KeeperServer::RespondingCounts getRespondingCounts() {
    ...
    for (const auto &peer : peers) {
      if (peer.last_succ_resp_us_ > expiry_us) continue;
      ...
      if (learner_ids.contains(peer.id_)) {
        if (synced) ++counts.synced_non_voting_followers;
        continue;
      }

      ++counts.followers;
      if (synced) ++counts.synced_followers;
    }
...
void KeeperServer::enterRecoveryMode(nuraft::raft_params &params) {
  ...
  nuraft::ptr<nuraft::cluster_config> new_config =
      std::make_shared<nuraft::cluster_config>(0, latest_config ? latest_config->get_log_idx() : 0);
...
void KeeperServer::launchRaftServer(const Poco::Util::AbstractConfiguration &config, bool enable_ipv6) {
  ...
  params.heart_beat_interval_ = getValueOrMaxInt32AndLogWarning(
      coordination_settings[CoordinationSetting::heart_beat_interval_ms].totalMilliseconds(), "heart_beat_interval_ms", log);
  params.election_timeout_lower_bound_ =
      getValueOrMaxInt32AndLogWarning(coordination_settings[CoordinationSetting::election_timeout_lower_bound_ms].totalMilliseconds(),
                                      "election_timeout_lower_bound_ms", log);
  params.election_timeout_upper_bound_ =
      getValueOrMaxInt32AndLogWarning(coordination_settings[CoordinationSetting::election_timeout_upper_bound_ms].totalMilliseconds(),
                                      "election_timeout_upper_bound_ms", log);
  ...
  params.reserved_log_items_ =
      getValueOrMaxInt32AndLogWarning(coordination_settings[CoordinationSetting::reserved_log_items], "reserved_log_items", log);
  params.snapshot_distance_ =
      getValueOrMaxInt32AndLogWarning(coordination_settings[CoordinationSetting::snapshot_distance], "snapshot_distance", log);

  if (params.snapshot_distance_ < 10000)
    LOG_WARNING(log,
                "Very small snapshot_distance {} specified in coordination settings. "
                "It doesn't make sense to specify such small value, because it can lead to degraded performance and another issues.",
                params.snapshot_distance_);
  ...
  params.client_req_timeout_ = getValueOrMaxInt32AndLogWarning(
      coordination_settings[CoordinationSetting::operation_timeout_ms].totalMilliseconds(), "operation_timeout_ms", log);
  params.auto_forwarding_ = coordination_settings[CoordinationSetting::auto_forwarding];
  params.auto_forwarding_req_timeout_ =
      std::max<int32_t>(static_cast<int32_t>(coordination_settings[CoordinationSetting::operation_timeout_ms].totalMilliseconds() * 2),
                        std::numeric_limits<int32_t>::max());
  params.auto_forwarding_req_timeout_ = getValueOrMaxInt32AndLogWarning(
      coordination_settings[CoordinationSetting::operation_timeout_ms].totalMilliseconds() * 2, "operation_timeout_ms", log);
  params.max_append_size_ = getValueOrMaxInt32AndLogWarning(coordination_settings[CoordinationSetting::max_requests_append_size],
                                                            "max_requests_append_size", log);
  ...
  asio_opts.worker_start_ = [](uint32_t /*worker_id*/) { LockMemoryExceptionInThread::addUniqueLock(VariableContext::Global); };

  asio_opts.worker_stop_ = [](uint32_t /*worker_id*/) { LockMemoryExceptionInThread::removeUniqueLock(); };
  ...
  if (state_manager->isSecure()) {
    ...
    throw Exception(ErrorCodes::SUPPORT_IS_DISABLED,
                    "SSL support for NuRaft is disabled because ClickHouse was built without SSL support.");
#endif
  }
  if (is_recovering) enterRecoveryMode(params);
  ...
  nuraft::ptr<nuraft::logger> logger =
      nuraft::cs_new<LoggerWrapper>("RaftInstance", coordination_settings[CoordinationSetting::raft_logs_level]);
  ...
  if (listen_hosts.empty()) {
    ...
    if (!asio_listener) {
      ...
      if (!asio_listener)
        throw Exception(ErrorCodes::RAFT_ERROR, "Cannot create interserver listener on port {} after trying both IPv6 and IPv4.",
                        state_manager->getPort());
    }
    ...
    for (const auto &listen_host : listen_hosts) {
      ...
      if (asio_listener) asio_listeners.emplace_back(std::move(asio_listener));
    }
  }
  ...
  if (!raft_instance) throw Exception(ErrorCodes::RAFT_ERROR, "Cannot allocate RAFT instance");

  state_manager->getLogStore()->setRaftServer(raft_instance);

  nuraft::raft_server::limits raft_limits;
  raft_limits.reconnect_limit_ = getValueOrMaxInt32AndLogWarning(coordination_settings[CoordinationSetting::raft_limits_reconnect_limit],
                                                                 "raft_limits_reconnect_limit", log);
  raft_limits.response_limit_ =
      getValueOrMaxInt32AndLogWarning(coordination_settings[CoordinationSetting::raft_limits_response_limit], "response_limit", log);
...
void KeeperServer::startup(const Poco::Util::AbstractConfiguration &config, bool enable_ipv6) {
  ...
  if (state_machine->last_commit_index() >= last_log_idx_on_disk) keeper_context->setLocalLogsPreprocessed();

  loadLatestConfig();

  last_local_config =
      state_manager->parseServersConfiguration(config, true, coordination_settings[CoordinationSetting::async_replication]).cluster_config;
...
void KeeperServer::shutdownRaftServer() {
  ...
  if (create_snapshot_on_exit) raft_instance->create_snapshot();
  ...
  if (asio_service->get_active_workers() != 0) LOG_WARNING(log, "Failed to shutdown RAFT server in {} seconds", timeout);
}
...
void KeeperServer::putLocalReadRequest(const KeeperRequestForSession &request_for_session) {
  if (!request_for_session.request->isReadRequest()) throw Exception(ErrorCodes::LOGICAL_ERROR, "Cannot process non-read request locally");
...
RaftAppendResult KeeperServer::putRequestBatch(const KeeperRequestsForSessions &requests_for_sessions) {
  ...
  std::lock_guard lock{server_write_mutex};
  if (is_recovering) return nullptr;

  return raft_instance->append_entries(entries);
}

bool KeeperServer::isLeader() const { return raft_instance->is_leader(); }
...
bool KeeperServer::isFollower() const { return !isLeader() && !isObserver(); }

bool KeeperServer::isLeaderAlive() const { return raft_instance && raft_instance->is_leader_alive(); }
...
KeeperServer::RespondingCounts KeeperServer::getRespondingCounts() const { return raft_instance->getRespondingCounts(); }
...
nuraft::cb_func::ReturnCode KeeperServer::callbackFunc(nuraft::cb_func::Type type, nuraft::cb_func::Param *param) {
  if (is_recovering) {
    ...
    switch (type) {
      case nuraft::cb_func::HeartBeat: {
        if (raft_instance->isClusterHealthy()) finish_recovering();
        break;
      }
      case nuraft::cb_func::NewConfig: {
        ...
        if (log_idx == config->get_log_idx()) {
          ...
          // new configuration
          if (config->get_servers().size() == 1) finish_recovering();
        }
  ...
  if (!keeper_context->localLogsPreprocessed()) {
    const auto preprocess_logs = [&] {
      ...
      if (keeper_context->localLogsPreprocessed()) return;
      ...
      for (const auto &entry : *log_entries) {
        if (entry && entry->get_val_type() == nuraft::log_val_type::app_log) state_machine->pre_commit(idx, entry->get_buf());

        ++idx;
        ++preprocessed;

        if (preprocessed % 50000 == 0) LOG_TRACE(log, "Preprocessed {}/{} entries", preprocessed, log_entries->size());
      }
    ...
    switch (type) {
      ...
      case nuraft::cb_func::ProcessReq: {
        ...
        if (req.get_type() != nuraft::msg_type::append_entries_request) break;

        if (req.log_entries().empty()) break;
        ...
        if (state_machine->last_commit_index() >= last_log_idx_on_disk && !raft_instance->isCommitInProgress()) preprocess_logs();
      ...
      case nuraft::cb_func::GotAppendEntryReqFromLeader: {
        ...
        if (req.get_last_log_idx() < last_log_idx_on_disk) last_log_idx_on_disk = req.get_last_log_idx();
      ...
      case nuraft::cb_func::StateMachineExecution: {
        if (state_machine->last_commit_index() >= last_log_idx_on_disk) preprocess_logs();
        break;
      }
      default:
        break;
  ...
  const auto follower_preappend = [&](const auto &entry) {
    if (entry->get_val_type() != nuraft::app_log) return nuraft::cb_func::ReturnCode::Ok;
  ...
  if (initialized_flag) {
    switch (type) {
      ...
      case nuraft::cb_func::PreAppendLogLeader: {
        ...
        if (!digest_after_preprocessing) return nuraft::cb_func::ReturnCode::ReturnNull;
  ...
  if (next_index < last_commited || next_index - last_commited <= 1) commited_store = true;
  ...
  switch (type) {
    case nuraft::cb_func::BecomeLeader: {
      ...
      if (commited_store || initial_batch_committed) set_initialized();
      return nuraft::cb_func::ReturnCode::Ok;
    }
    ...
    case nuraft::cb_func::GotAppendEntryReqFromLeader: {
      if (param->leaderId != -1) {
        ...
        /// when it was leader.
        if (leader_index < our_index + keeper_context->getCoordinationSettings()[CoordinationSetting::fresh_log_gap]) set_initialized();
      }
      return nuraft::cb_func::ReturnCode::Ok;
    }
    ...
    case nuraft::cb_func::ReceivedMisbehavingMessage: {
      ...
      if (req.get_src() == server_id) {
        LOG_FATAL(
            ...
            server_id, req.get_src(), req.get_dst());
        std::terminate();
      }
...
std::vector<int64_t> KeeperServer::getDeadSessions() { return state_machine->getDeadSessions(); }

KeeperServer::ConfigUpdateState KeeperServer::applyConfigUpdate(const ClusterUpdateAction &action, bool last_command_was_leader_change) {
  using enum ConfigUpdateState;
  std::lock_guard _{server_write_mutex};

  if (const auto *add = std::get_if<AddRaftServer>(&action)) {
    if (raft_instance->get_srv_config(add->id) != nullptr) return Accepted;
  ...
  if (const auto *remove = std::get_if<RemoveRaftServer>(&action)) {
    ...
    if (remove->id == raft_instance->get_leader()) {
      if (!last_command_was_leader_change) return WaitBeforeChangingLeader;
    ...
    if (raft_instance->get_srv_config(remove->id) == nullptr) return Accepted;
  ...
  if (const auto *update = std::get_if<UpdateRaftServerPriority>(&action)) {
    ...
    if (ptr == nullptr) throw Exception(ErrorCodes::RAFT_ERROR, "Attempt to apply {} but server is not present in Raft", action);
    if (ptr->get_priority() == update->priority) return Accepted;
...
ClusterUpdateActions KeeperServer::getRaftConfigurationDiff(const Poco::Util::AbstractConfiguration &config) {
  ...
  if (!diff.empty()) {
    std::lock_guard lock{server_write_mutex};
    last_local_config =
        state_manager->parseServersConfiguration(config, true, coordination_settings[CoordinationSetting::async_replication])
            .cluster_config;
  }
...
void KeeperServer::applyConfigUpdateWithReconfigDisabled(const ClusterUpdateAction &action) {
  ...
  if (const auto *add = std::get_if<AddRaftServer>(&action)) {
    for (size_t i = 0; i < coordination_settings[CoordinationSetting::configuration_change_tries_count] && !is_recovering; ++i) {
      if (raft_instance->get_srv_config(add->id) != nullptr) return applied();  // NOLINT
      if (!isLeader()) return not_leader();                                     // NOLINT
      if (!raft_instance->add_srv(static_cast<nuraft::srv_config>(*add))->get_accepted()) backoff_on_refusal(i);
    }
    ...
    for (size_t i = 0; i < coordination_settings[CoordinationSetting::configuration_change_tries_count] && !is_recovering; ++i) {
      if (raft_instance->get_srv_config(remove->id) == nullptr) return applied();  // NOLINT
      if (!isLeader()) return not_leader();                                        // NOLINT
      if (!raft_instance->remove_srv(remove->id)->get_accepted()) backoff_on_refusal(i);
    }
  ...
  throw Exception(ErrorCodes::RAFT_ERROR, "Configuration change {} was not accepted by Raft after {} retries", action,
                  coordination_settings[CoordinationSetting::configuration_change_tries_count].value);
}
...
bool KeeperServer::waitForConfigUpdateWithReconfigDisabled(const ClusterUpdateAction &action) {
  ...
  auto applied = [&] {
    LOG_INFO(log, "Applied {}", action);
    return true;
  };
  auto became_leader = [&] {
    LOG_INFO(log, "Became leader, aborting");
    return false;
  };
  ...
  if (const auto *add = std::get_if<AddRaftServer>(&action)) {
    for (size_t i = 0; i < coordination_settings[CoordinationSetting::configuration_change_tries_count] && !is_recovering; ++i) {
      if (raft_instance->get_srv_config(add->id) != nullptr) return applied();
      if (isLeader()) return became_leader();
      backoff(i);
    }
  } else if (const auto *remove = std::get_if<RemoveRaftServer>(&action)) {
    for (size_t i = 0; i < coordination_settings[CoordinationSetting::configuration_change_tries_count] && !is_recovering; ++i) {
      if (raft_instance->get_srv_config(remove->id) == nullptr) return applied();
      if (isLeader()) return became_leader();
      backoff(i);
    }
...
bool KeeperServer::requestLeader() { return isLeader() || raft_instance->request_leadership(); }

int64_t KeeperServer::getLeaderID() const { return raft_instance->get_leader(); }

void KeeperServer::yieldLeadership() {
  if (isLeader()) raft_instance->yield_leadership();
}

void KeeperServer::recalculateStorageStats() { state_machine->recalculateStorageStats(); }
