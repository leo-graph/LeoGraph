...
nuraft::ptr<std::vector<nuraft::ptr<nuraft::log_entry>>> KeeperLogStore::log_entries_ext(uint64_t start, uint64_t end,
...
bool KeeperLogStore::flushChangelogAndShutdown() {
  std::lock_guard lock(changelog_lock);
  if (changelog.isInitialized()) changelog.flush();
  changelog.shutdown();
  return true;
}
