...
bool SessionExpiryQueue::remove(int64_t session_id) {
  ...
  if (session_it == session_to_expiration_time.end()) return false;

  auto set_it = expiry_to_sessions.find(session_it->second);
  if (set_it != expiry_to_sessions.end()) set_it->second.erase(session_id);

  /// No more sessions in this bucket
  if (set_it->second.empty()) expiry_to_sessions.erase(set_it);
...
void SessionExpiryQueue::addNewSessionOrUpdate(int64_t session_id, int64_t timeout_ms) {
  ...
  if (session_it != session_to_expiration_time.end()) {
    ...
    if (new_expiry_time == prev_expiry_time) return;
    ...
    if (prev_set_it != expiry_to_sessions.end()) prev_set_it->second.erase(session_id);

    /// No more sessions in this bucket
    if (prev_set_it->second.empty()) expiry_to_sessions.erase(prev_set_it);
  } else {
