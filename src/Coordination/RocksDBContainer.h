...
struct RocksDBContainer {
 ...
 private:
  ...
  struct MockNode {
    ...
    MockNode(size_t children_num, std::string_view data_, uint64_t acl_id_)
        : acl_id(acl_id_), children(std::vector<int>(children_num)), data(data_) {}
 ...
 public:
  ...
  struct const_iterator {
    ...
    explicit const_iterator(std::shared_ptr<rocksdb::Iterator> iter_) : iter(iter_) { updatePairFromIter(); }

    const KVPair& operator*() const { return *pair; }

    const KVPair* operator->() const { return pair.get(); }

    bool operator!=(const const_iterator& other) const { return !(*this == other); }

    bool operator==(const const_iterator& other) const {
      if (pair == nullptr && other == nullptr) return true;
      if (pair == nullptr || other == nullptr) return false;
      return pair->key == other->key && iter == other.iter;
    }

    bool operator==(std::nullptr_t) const { return iter == nullptr; }
  ...
  std::vector<std::pair<std::string, Node>> getChildren(std::string_view key_prefix, bool read_meta = true, bool read_data = false) {
    ...
    auto is_direct_child = [](rocksdb::Slice iter_key, rocksdb::Slice seek_key) {
      if (iter_key.size() <= 1) return false;
      size_t rslash_pos = 0;
      ...
      if (rslash_pos == 0 && seek_key.size() == 1) return true;
      return seek_key.compare(rocksdb::Slice(iter_key.data(), rslash_pos)) == 0;
    };
    for (iter->Seek(prefix); iter->Valid() && iter->key().starts_with(prefix); iter->Next()) {
      if (!is_direct_child(iter->key(), rocksdb::Slice(key_prefix))) continue;
      Node node;
      if (read_meta) {
        ReadBufferFromOwnString buffer(iter->value().ToStringView());
  ...
  bool contains(const std::string& path) {
    ...
    if (!status.ok())
      throw Exception(ErrorCodes::ROCKSDB_ERROR, "Got rocksdb error during executing contains. The error message is {}.",
                      status.ToString());
    return true;
  }
  ...
  const_iterator find(std::string_view key) {
    ...
    if (status.IsNotFound()) return end();
    if (!status.ok())
  ...
  void startLoading(size_t batch_size_) {
    ...
    if (rocksdb_ptr != nullptr) rocksdb_ptr->Close();
  ...
  void startBatch() { write_batch.emplace(); }

  void commitBatch() {
    if (!write_batch) return;
  ...
  std::pair<size_t, size_t> snapshotSizeWithVersion() const {
    if (!snapshot_mode) return std::make_pair(counter, current_version);
  ...
  const_iterator begin() const {
    rocksdb::ReadOptions read_options;
    read_options.total_order_seek = true;
    if (snapshot_mode) read_options.snapshot = snapshot;
  ...
  uint64_t getArenaDataSize() const { return 0; }
