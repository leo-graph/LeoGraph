...
IFourLetterCommand::IFourLetterCommand(KeeperDispatcher &keeper_dispatcher_) : keeper_dispatcher(keeper_dispatcher_) {}

int32_t IFourLetterCommand::code() { return toCode(name()); }
...
void FourLetterCommandFactory::checkInitialization() const {
  if (!initialized) throw Exception(ErrorCodes::LOGICAL_ERROR, "Four letter command not initialized");
}
...
bool FourLetterCommandFactory::isEnabled(int32_t code) const {
  checkInitialization();
  if (!allow_list.empty() && *allow_list.cbegin() == ALLOW_LIST_ALL) return true;
...
bool FourLetterCommandFactory::supportArguments(int32_t code) const {
  ...
  if (IFourLetterCommand::toName(code) == "rcfg") return true;
  return false;
}
...
String RuokCommand::run() { return "imok"; }
...
void print(StringBuffer &buf, const String &key, uint64_t value) { print(buf, key, toString(value)); }
...
String MonitorCommand::run() {
  if (!keeper_dispatcher.isServerActive()) return SERVER_NOT_ACTIVE_MSG;
...
String BriefWatchCommand::run() {
  ...
  StringBuffer buf;
  const auto &state_machine = keeper_dispatcher.getStateMachine();
  buf << state_machine.getSessionsWithWatchesCount() << " connections watching " << state_machine.getWatchedPathsCount() << " paths\n";
...
String IsReadOnlyCommand::run() {
  if (keeper_dispatcher.isObserver()) return "ro";
  return "rw";
}
...
String ApiVersionCommand::run() { return toString(static_cast<uint8_t>(KeeperApiVersion::WITH_MULTI_READ)); }
...
String JemallocFlushProfile::run() { return std::string{Jemalloc::flushProfile("/tmp/jemalloc_keeper")}; }
...
String ReconfigureCommand::run() { return "Reconfiguration command require single JSON argument"; }
