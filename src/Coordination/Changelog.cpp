...
void moveChangelogBetweenDisks(DiskPtr disk_from, ChangelogFileDescriptionPtr description, DiskPtr disk_to, const std::string &path_to,
                               const KeeperContextPtr &keeper_context) {
  auto path_from = description->path;
  moveFileBetweenDisks(
      disk_from, path_from, disk_to, path_to,
      [&] {
      ...
      getLogger("Changelog"), keeper_context);
}
...
Checksum computeRecordChecksum(const ChangelogRecord &record) {
  ...
  if (record.header.blob_size != 0) hash.update(reinterpret_cast<char *>(record.blob->data_begin()), record.blob->size());
  return hash.get64();
}
...
struct ChangelogFileOperation {
  explicit ChangelogFileOperation(ChangelogFileDescriptionPtr changelog_, ChangelogFileOperationVariant operation_)
      : changelog(std::move(changelog_)), operation(std::move(operation_)) {}
...
void ChangelogFileDescription::waitAllAsyncOperations() {
  for (const auto &op : file_operations) {
    if (auto op_locked = op.lock()) op_locked->done.wait(false);
  }

  file_operations.clear();
}

std::string Changelog::formatChangelogPath(const std::string &name_prefix, uint64_t from_index, uint64_t to_index,
...
class ChangelogWriter {
 ...
 public:
  ChangelogWriter(std::map<uint64_t, ChangelogFileDescriptionPtr> &existing_changelogs_, LogEntryStorage &entry_storage_,
                  KeeperContextPtr keeper_context_, LogFileSettings log_file_settings_, MoveChangelogCallback move_changelog_cb_)
  ...
  void setFile(ChangelogFileDescriptionPtr file_description, WriteMode mode) {
    ...
    try {
      if (mode == WriteMode::Append && file_description->expectedEntriesCountInLog() != log_file_settings.rotate_interval)
        LOG_TRACE(log, "Looks like rotate_logs_interval was changed, current {}, expected entries in last log {}",
                  log_file_settings.rotate_interval, file_description->expectedEntriesCountInLog());
      ...
      if (tryGetFileBaseBuffer() && prealloc_done) {
        ...
        if (current_file_description->deleted) {
          ...
          if (last_index_written && *last_index_written != current_file_description->to_log_index) {
            new_path = Changelog::formatChangelogPath(current_file_description->prefix, current_file_description->from_log_index,
                                                      *last_index_written, current_file_description->extension);
          }

          if (move_changelog_cb) move_changelog_cb(current_file_description, std::move(new_path), disk);
        }
      } else {
        cancelCurrentFile();
      }
  ...
  bool appendRecord(ChangelogRecord &&record) {
    ...
    if (record.header.index - getStartIndex() == current_file_description->expectedEntriesCountInLog()) {
      ...
      // we use count() that can be unreliable for more complex WriteBuffers, so we should be careful if we change the type of it in the
      // future
    ...
    if (record.header.blob_size != 0) write_buffer.write(reinterpret_cast<char *>(record.blob->data_begin()), record.blob->size());
    ...
    if (compressed_buffer) {
      ...
      unflushed_indices_with_log_location.emplace_back(
          record.header.index, LogLocation{.file_description = current_file_description,
  ...
  void flush() {
    ...
    if (file_buffer) {
      ...
      if (log_file_settings.force_sync) {
        ...
        if (!compressed_buffer) ProfileEvents::increment(ProfileEvents::KeeperChangelogFileSyncMicroseconds, watch.elapsedMicroseconds());
      } else
        file_buffer->next();
    }
  ...
  void rotate(uint64_t new_start_log_index) {
    ...
    if (log_file_settings.compress_logs) new_description->extension += "." + toContentEncodingName(CompressionMethod::Zstd);

    new_description->path =
        Changelog::formatChangelogPath(new_description->prefix, new_start_log_index,
                                       new_start_log_index + log_file_settings.rotate_interval - 1, new_description->extension);
 ...
 private:
  void finalizeCurrentFile() {
    ...
    if (compressed_buffer) compressed_buffer->finalize();

    flush();

    if (file_buf) file_buf->finalize();
    ...
    if (log_file_settings.max_size != 0 && file_buffer) {
      ...
      if (res != 0) LOG_WARNING(log, "Could not ftruncate file. Error: {}, errno: {}", errnoToString(), errno);
    }
  ...
  void cancelCurrentFile() {
    if (compressed_buffer) compressed_buffer->cancel();

    if (file_buf) file_buf->cancel();
  ...
  WriteBuffer &getBuffer() {
    ...
    if (compressed_buffer) return *compressed_buffer;

    if (file_buf) return *file_buf;
  ...
  WriteBufferFromFile &getFileBuffer() {
    ...
    if (!file_buffer) throw Exception(ErrorCodes::LOGICAL_ERROR, "Log writer wasn't initialized for any file");
  ...
  WriteBufferFromFile *tryGetFileBuffer() {
    if (compressed_buffer) return dynamic_cast<WriteBufferFromFile *>(compressed_buffer->getNestedBuffer());

    return dynamic_cast<WriteBufferFromFile *>(file_buf.get());
  }

  WriteBufferFromFileBase *tryGetFileBaseBuffer() {
    if (compressed_buffer) return dynamic_cast<WriteBufferFromFileBase *>(compressed_buffer->getNestedBuffer());
  ...
  void tryPreallocateForFile() {
    ...
    if (file_buffer) initial_file_size = getSizeFromFileDescriptor(file_buffer->getFD());
...
ChangelogRecord readChangelogRecord(ReadBuffer &read_buf, const std::string &filepath) {
  ...
  if (record.header.version > CURRENT_CHANGELOG_VERSION)
    throw Exception(ErrorCodes::UNKNOWN_FORMAT_VERSION, "Unsupported changelog version {} on path {}",
                    static_cast<uint8_t>(record.header.version), filepath);
  ...
  if (checksum != record_checksum) {
    throw Exception(ErrorCodes::CHECKSUM_DOESNT_MATCH, "Checksums doesn't match for log {} (version {}), index {}, blob_size {}", filepath,
                    record.header.version, record.header.index, record.header.blob_size);
  }
...
size_t logEntrySize(const LogEntryPtr &log_entry) { return log_entry->get_buf().size(); }

LogEntryPtr getLogEntry(const CacheEntry &cache_entry) {
  if (const auto *log_entry = std::get_if<LogEntryPtr>(&cache_entry)) return *log_entry;
  ...
  ChangelogReadResult readChangelog(LogEntryStorage &entry_storage, uint64_t start_log_index, LoggerPtr log) {
    ...
    try {
      while (!read_buf->eof()) {
        ...
        if (entry_storage.contains(record.header.index)) entry_storage.cleanAfter(record.header.index - 1);

        result.total_entries_read_from_log += 1;

        /// Read but skip this entry because our state is already more fresh
        if (record.header.index < start_log_index) continue;

        /// Create log entry for read data
        auto log_entry = logEntryFromRecord(record);
        if (result.first_read_index == 0) result.first_read_index = record.header.index;

        /// Put it into in memory structure
        entry_storage.addEntryWithLocation(record.header.index, log_entry,
                                           LogLocation{.file_description = changelog_description,
      ...
      if (ex.code() == ErrorCodes::UNKNOWN_FORMAT_VERSION) throw;
...
PrefetchedCacheEntry::PrefetchedCacheEntry() : log_entry(log_entry_resolver.get_future()) {}

const LogEntryPtr &PrefetchedCacheEntry::getLogEntry() const { return log_entry.get(); }

void PrefetchedCacheEntry::resolve(std::exception_ptr exception) { log_entry_resolver.set_exception(exception); }

void PrefetchedCacheEntry::resolve(LogEntryPtr log_entry_) { log_entry_resolver.set_value(std::move(log_entry_)); }
...
LogEntryStorage::~LogEntryStorage() { shutdown(); }
        ...
        prefetch_file_info.file_description->withLock([&] {
          ...
          for (size_t i = 0; i < count; ++i) {
            ...
            if (current_index != record.header.index)
              throw Exception(ErrorCodes::LOGICAL_ERROR, "Invalid index prefetched, expected {}, actual {}", current_index,
                              record.header.index);
...
void LogEntryStorage::startCommitLogsPrefetch(uint64_t last_committed_index) const {
  ...
  if (latest_logs_cache.hasUnlimitedSpace()) return;

  /// commit logs is not empty and it's not next log
  if (!commit_logs_cache.empty() && commit_logs_cache.max_index_in_cache != last_committed_index) return;

  if (logs_location.empty()) return;

  /// we are already prefetching some logs for commit
  if (current_prefetch_info && !current_prefetch_info->done) return;
...
LogEntryStorage::InMemoryCache::InMemoryCache(size_t size_threshold_, size_t count_threshold_)
    : size_threshold(size_threshold_), count_threshold(count_threshold_) {}
...
void LogEntryStorage::InMemoryCache::addEntry(uint64_t index, size_t size, CacheEntry log_entry) {
  ...
  if (!inserted) throw Exception(ErrorCodes::LOGICAL_ERROR, "Trying to insert log with index {} which is already present in cache", index);
...
CacheEntry *LogEntryStorage::InMemoryCache::getCacheEntry(uint64_t index) {
  if (!containsEntry(index)) return nullptr;

  auto it = cache.find(index);
  if (it == cache.end()) throw Exception(ErrorCodes::LOGICAL_ERROR, "Index {} missing from cache while it should be present", index);
...
PrefetchedCacheEntryPtr LogEntryStorage::InMemoryCache::getPrefetchedCacheEntry(uint64_t index) {
  ...
  if (cache_entry == nullptr) throw Exception(ErrorCodes::LOGICAL_ERROR, "Missing expected index {} in cache", index);
...
void LogEntryStorage::InMemoryCache::cleanUpTo(uint64_t index) {
  ...
  for (size_t i = min_index_in_cache; i < index; ++i) {
    ...
    if (it == cache.end()) throw Exception(ErrorCodes::LOGICAL_ERROR, "Log entry with index {} unexpectedly missing from cache", i);
...
void LogEntryStorage::InMemoryCache::cleanAfter(uint64_t index) {
  if (empty() || index >= max_index_in_cache) return;
...
bool LogEntryStorage::InMemoryCache::hasUnlimitedSpace() const { return size_threshold == 0 && count_threshold == 0; }

bool LogEntryStorage::InMemoryCache::empty() const { return cache.empty(); }

size_t LogEntryStorage::InMemoryCache::numberOfEntries() const { return cache.size(); }

bool LogEntryStorage::InMemoryCache::hasSpaceAvailable(size_t log_entry_size) const {
  if (hasUnlimitedSpace() || empty()) return true;

  if (size_threshold != 0 && cache_size + log_entry_size > size_threshold) return false;

  if (count_threshold != 0 && numberOfEntries() + 1 > count_threshold) return false;
...
bool LogEntryStorage::shouldMoveLogToCommitCache(uint64_t index, size_t log_entry_size) {
  ...
  if (commit_logs_cache.empty()) return keeper_context->lastCommittedIndex() + 1 == index;
...
void LogEntryStorage::updateTermInfoWithNewEntry(uint64_t index, uint64_t term) {
  if (!log_term_infos.empty() && log_term_infos.back().term == term) return;
...
void LogEntryStorage::addEntryWithLocation(uint64_t index, const LogEntryPtr &log_entry, LogLocation log_location) {
  ...
  while (!latest_logs_cache.hasSpaceAvailable(entry_size)) {
    ...
    size_t removed_entry_size = logEntrySize(getLogEntry(entry_handle.mapped()));
    {
      ...
      if (shouldMoveLogToCommitCache(entry_handle.key(), removed_entry_size)) commit_logs_cache.addEntry(std::move(entry_handle));
    }
  }
  ...
  if (logs_location.size() == 1) min_index_with_location = index;
...
void LogEntryStorage::cleanUpTo(uint64_t index) {
...

  {
    ...
    if (!unapplied_indices_with_log_locations.empty()) {
      auto last = std::ranges::lower_bound(unapplied_indices_with_log_locations, index, std::ranges::less{},
  ...
  if (current_prefetch_info && !current_prefetch_info->done) {
    ...
    std::lock_guard lock(commit_logs_cache_mutex);
    if (index > prefetch_from) commit_logs_cache.clear();
  ...
  if (first_log_index < index) first_log_entry = nullptr;
...
void LogEntryStorage::cleanAfter(uint64_t index) {
...

  {
    std::lock_guard lock(logs_location_mutex);
    if (!unapplied_indices_with_log_locations.empty()) {
      auto first = std::ranges::upper_bound(unapplied_indices_with_log_locations, index, std::ranges::less{},
                                            [](const auto &index_with_location) { return index_with_location.first; });
  ...
  while (!log_term_infos.empty() && log_term_infos.back().first_index > index) log_term_infos.pop_back();
}

bool LogEntryStorage::contains(uint64_t index) const { return logs_location.contains(index) || latest_logs_cache.containsEntry(index); }
...
LogEntryPtr LogEntryStorage::getEntry(uint64_t index) const {
  ...
  if (latest_config != nullptr && index == latest_config_index) return latest_config;

  if (first_log_entry != nullptr && index == first_log_index) return first_log_entry;
  ...
  if (auto it = logs_location.find(index); it != logs_location.end()) {
    it->second.file_description->withLock([&] {
      ...
      auto file = changelog_description->disk->readFile(changelog_description->path, getReadSettings());
      file->seek(position, SEEK_SET);
      LOG_TRACE(log, "Reading log entry at index {} from path {}, position {}, size {}", index, changelog_description->path, position,
                entry_size);
...
LogEntryPtr LogEntryStorage::getLatestConfigChange() const { return latest_config; }
...
uint64_t LogEntryStorage::termAt(uint64_t index) const {
  ...
  for (const auto [term, first_index] : log_term_infos) {
    if (index < first_index) return term_for_index;
...
void LogEntryStorage::addLogLocations(std::vector<std::pair<uint64_t, LogLocation>> &&indices_with_log_locations) {
  ...
  if (indices_with_log_locations.empty()) return;

  std::lock_guard lock(logs_location_mutex);
  unapplied_indices_with_log_locations.insert(unapplied_indices_with_log_locations.end(),
...
void LogEntryStorage::refreshCache() {
  ...
  for (auto &[index, log_location] : new_unapplied_indices_with_log_locations) {
    if (logs_location.empty()) min_index_with_location = index;
  ...
  while (latest_logs_cache.numberOfEntries() > 1 && latest_logs_cache.min_index_in_cache <= max_index_with_location &&
    ...
    if (shouldMoveLogToCommitCache(node.key(), log_entry_size)) commit_logs_cache.addEntry(std::move(node));
  }
}
...
LogEntriesPtr LogEntryStorage::getLogEntriesBetween(uint64_t start, uint64_t end, int64_t max_size_bytes) const {
  ...
  if (max_size_bytes == -1) return nuraft::cs_new<std::vector<nuraft::ptr<nuraft::log_entry>>>();
  ...
  const auto size_limit_reached = [&](int64_t entry_size) {
    if (max_size_bytes == 0) return false;

    bool limit_reached = total_size > 0 && total_size + entry_size > max_size_bytes;
    if (!limit_reached) total_size += entry_size;
  ...
  for (size_t i = start; i < end; ++i) {
    if (auto commit_cache_entry = commit_logs_cache.getEntry(i)) {
      ...
      if (size_limit_reached(static_cast<int64_t>(commit_cache_entry->get_buf().size()))) break;

      ret->push_back(std::move(commit_cache_entry));
    } else if (auto latest_cache_entry = latest_logs_cache.getEntry(i)) {
      flush_file();

      if (size_limit_reached(static_cast<int64_t>(latest_cache_entry->get_buf().size()))) break;
      ...
      if (size_limit_reached(static_cast<int64_t>(log_location.size_in_file))) break;
...
bool LogEntryStorage::isConfigLog(uint64_t index) const { return logs_with_config_changes.contains(index); }

size_t LogEntryStorage::empty() const { return logs_location.empty() && latest_logs_cache.empty(); }
...
size_t LogEntryStorage::size() const {
  ...
  if (!latest_logs_cache.empty()) max_index = latest_logs_cache.max_index_in_cache;

  return max_index - min_index + 1;
}

size_t LogEntryStorage::getFirstIndex() const {
  if (!logs_location.empty()) return min_index_with_location;

  if (!latest_logs_cache.empty()) return latest_logs_cache.min_index_in_cache;

  return 0;
}

void LogEntryStorage::shutdown() {
  if (std::exchange(is_shutdown, true)) return;

  if (!prefetch_queue.isFinished()) prefetch_queue.finish();
  ...
  if (commit_logs_prefetcher->joinable()) commit_logs_prefetcher->join();
}
...
ChangelogFileDescriptionPtr Changelog::getChangelogFileDescription(const std::filesystem::path &path) {
  ...
  if (first_dot == std::string::npos) throw Exception(ErrorCodes::LOGICAL_ERROR, "Invalid changelog file {}", path.generic_string());

  Strings filename_parts;
  boost::split(filename_parts, filename_with_extension_view.substr(0, first_dot), boost::is_any_of("_"));
  if (filename_parts.size() < 3) throw Exception(ErrorCodes::CORRUPTED_DATA, "Invalid changelog {}", path.generic_string());
...
void Changelog::spliceChangelog(ChangelogFileDescriptionPtr source_changelog, ChangelogFileDescriptionPtr destination_changelog) {
  ...
  LogFileSettings log_file_settings{.compress_logs =
                                        chooseCompressionMethod(destination_changelog->path, "auto") != CompressionMethod::None};
...
Changelog::Changelog(LoggerPtr log_, LogFileSettings log_file_settings, FlushSettings flush_settings_, KeeperContextPtr keeper_context_)
  ...
  try {
    ...
    const auto load_from_disk = [&](const auto &disk) {
      if (read_disks.contains(disk)) return;
      ...
      for (auto it = disk->iterateDirectory(""); it->isValid(); it->next()) {
        ...
        if (file_name == changelogs_detached_dir) continue;
        ...
        if (file_name.starts_with(DEFAULT_PREFIX)) {
          if (!clean_incomplete_file(it->path())) changelog_files.push_back(it->path());
        } else {
      ...
      for (const auto &changelog_file : changelog_files) {
        if (clean_incomplete_file(fs::path(changelog_file).filename())) continue;
      ...
      for (const auto &[name, path] : incomplete_files) disk->removeFile(path);

      read_disks.insert(disk);
    };

    /// Load all files from old disks
    for (const auto &disk : keeper_context->getOldLogDisks()) load_from_disk(disk);

    auto disk = getDisk();
    load_from_disk(disk);

    auto latest_log_disk = getLatestLogDisk();
    if (disk != latest_log_disk) load_from_disk(latest_log_disk);
    ...
    current_writer = std::make_unique<ChangelogWriter>(
        existing_changelogs, entry_storage, keeper_context, log_file_settings,
...
void Changelog::readChangelogAndInitWriter(uint64_t last_commited_log_index, uint64_t logs_to_keep) {
  ...
  for (const auto &[changelog_start_index, changelog_description_ptr] : existing_changelogs) {
    ...
    if (changelog_description.to_log_index >= start_to_read_from) {
      ...
      {
        LOG_INFO(log, "from log index: {}, to log index: {}, last committed log index: {}", changelog_description.from_log_index,
                 changelog_description.to_log_index, last_commited_log_index);
        ...
        if (changelog_description.from_log_index > last_commited_log_index &&
          ...
          throw Exception(
              ...
              last_commited_log_index, changelog_description.from_log_index);
        }
        ...
        if (changelog_description.from_log_index > start_to_read_from) {
          ...
          LOG_WARNING(log,
                      ...
                      "{}.",
                      start_to_read_from, changelog_description.from_log_index);
        }
        ...
        if (changelog_description.from_log_index <= last_commited_log_index) {
          LOG_INFO(
              ...
              last_read_index, changelog_description.from_log_index, last_commited_log_index, changelog_description.from_log_index);
          ...
          if (!last_log_read_result->error) {
            throw Exception(ErrorCodes::CORRUPTED_DATA,
                            ...
                            "is necessary for recovery but removing changelogs can lead to data loss.",
                            last_read_index, changelog_description.from_log_index);
          }
          break;
        }
      }
      ...
      if (last_log_read_result->last_read_index != 0) last_read_index = last_log_read_result->last_read_index;

      last_log_read_result->log_start_index = changelog_description.from_log_index;

      if (last_log_read_result->last_read_index != 0) max_log_id = last_log_read_result->last_read_index;
  ...
  if (remove_logs_before_index) removeAllLogFilesBefore(remove_logs_before_index);
  ...
  if (!current_writer->isFileSet()) current_writer->rotate(max_log_id + 1);
  ...
  for (const auto &[start_index, description] : existing_changelogs) {
    ...
    if (description->disk != disk) moveChangelogBetweenDisks(description->disk, description, disk, description->path, keeper_context);
  }
...
void Changelog::initWriter(ChangelogFileDescriptionPtr description) {
  if (description->expectedEntriesCountInLog() != rotate_interval)
    LOG_TRACE(log, "Looks like rotate_logs_interval was changed, current {}, expected entries in last log {}", rotate_interval,
  ...
  auto latest_log_disk = getLatestLogDisk();
  if (log_disk != latest_log_disk) moveChangelogBetweenDisks(log_disk, description, latest_log_disk, description->path, keeper_context);
...
std::string getCurrentTimestampFolder() {
  ...
  return fmt::format("{:02}{:02}{:02}T{:02}{:02}{:02}", timestamp.year(), timestamp.month(), timestamp.day(), timestamp.hour(),
                     timestamp.minute(), timestamp.second());
}

}  // namespace

DiskPtr Changelog::getDisk() const { return keeper_context->getLogDisk(); }

DiskPtr Changelog::getLatestLogDisk() const { return keeper_context->getLatestLogDisk(); }
...
void Changelog::removeAllLogsAfter(uint64_t remove_after_log_start_index) {
  ...
  if (start_to_remove_from_itr == existing_changelogs.end()) return;
...
void Changelog::removeAllLogFilesBefore(uint64_t remove_before_log_start_index) {
  ...
  if (end_to_remove_to_itr == existing_changelogs.begin()) return;
...
void Changelog::appendCompletionThread() {
  ...
  while (append_completion_queue.pop(append_ok)) {
    if (!append_ok) current_writer->finalize();
...
void Changelog::writeThread() {
  ...
  const auto notify_append_completion = [&] {
    ...
    if (!append_completion_queue.push(batch_append_ok)) LOG_WARNING(log, "Changelog is shut down");
  };
    ...
    while (true) {
      if (try_batch_flush) {
        ...
        if (!write_operations.tryPop(write_operation)) {
          ...
          if (!write_operations.pop(write_operation)) break;
        }
...
void Changelog::appendEntry(uint64_t index, const LogEntryPtr &log_entry) {
  if (!initialized) throw Exception(ErrorCodes::LOGICAL_ERROR, "Changelog must be initialized before appending records");

  entry_storage.addEntry(index, log_entry);
  max_log_id = index;

  if (!write_operations.push(AppendLog{index, log_entry})) LOG_WARNING(log, "Changelog is shut down");
}

void Changelog::writeAt(uint64_t index, const LogEntryPtr &log_entry) {
  if (!initialized) throw Exception(ErrorCodes::LOGICAL_ERROR, "Changelog must be initialized before writing records");
...
void Changelog::compact(uint64_t up_to_log_index) {
  if (!initialized) throw Exception(ErrorCodes::LOGICAL_ERROR, "Changelog must be initialized before compacting records");
  ...
  for (auto itr = existing_changelogs.begin(); itr != existing_changelogs.end();) {
    ...
    if (remove_all_logs || changelog_description.to_log_index <= up_to_log_index) {
      if (current_writer && changelog_description.from_log_index == current_writer->getStartIndex()) {
        LOG_INFO(log, "Trying to remove log {} which is current active log for write. Possibly this node recovers from snapshot", path);
        need_rotate = true;
      }
  ...
  if (need_rotate) current_writer->rotate(up_to_log_index + 1);

  LOG_INFO(log, "Compaction up to {} finished new min index {}, new max index {}", up_to_log_index, getStartIndex(), max_log_id);
}

uint64_t Changelog::getNextEntryIndex() const { return max_log_id + 1; }

uint64_t Changelog::getStartIndex() const { return entry_storage.empty() ? max_log_id + 1 : entry_storage.getFirstIndex(); }
...
LogEntryPtr Changelog::getLastEntry() const {
  ...
  if (entry == nullptr) return fake_entry;
...
LogEntryPtr Changelog::entryAt(uint64_t index) const { return entry_storage.getEntry(index); }

LogEntryPtr Changelog::getLatestConfigChange() const { return entry_storage.getLatestConfigChange(); }
...
nuraft::ptr<nuraft::buffer> Changelog::serializeEntriesToBuffer(uint64_t index, int32_t count) {
  ...
  for (uint64_t i = index; i < index + count; ++i) {
    auto entry = entry_storage.getEntry(i);
    if (entry == nullptr) throw Exception(ErrorCodes::LOGICAL_ERROR, "Don't have log entry {}", i);
...
bool Changelog::isConfigLog(uint64_t index) const { return entry_storage.isConfigLog(index); }

uint64_t Changelog::termAt(uint64_t index) const { return entry_storage.termAt(index); }
...
std::shared_ptr<bool> Changelog::flushAsync() {
  if (!initialized) throw Exception(ErrorCodes::LOGICAL_ERROR, "Changelog must be initialized before flushing records");
...
uint64_t Changelog::size() const { return entry_storage.size(); }

void Changelog::shutdown() {
  LOG_DEBUG(log, "Shutting down Changelog");
  if (!changelog_operation_queue.isFinished()) changelog_operation_queue.finish();

  if (background_changelog_operations_thread->joinable()) background_changelog_operations_thread->join();

  if (!write_operations.isFinished()) write_operations.finish();

  if (write_thread->joinable()) write_thread->join();

  if (!append_completion_queue.isFinished()) append_completion_queue.finish();

  if (append_completion_thread->joinable()) append_completion_thread->join();
...
void Changelog::modifyChangelogAsync(ChangelogFileOperationPtr changelog_operation) {
  if (!changelog_operation_queue.tryPush(changelog_operation, 60 * 1000)) {
    throw DB::Exception(ErrorCodes::SYSTEM_ERROR, "Background thread for changelog operations is stuck or not keeping up with operations");
  }
...
void Changelog::moveChangelogAsync(ChangelogFileDescriptionPtr changelog, std::string new_path, DiskPtr new_disk) {
  modifyChangelogAsync(std::make_shared<ChangelogFileOperation>(
...
bool Changelog::isInitialized() const { return initialized; }
