...
IKeeperStateMachine::IKeeperStateMachine(ResponsesQueue &responses_queue_, SnapshotsQueue &snapshots_queue_,
                                         const KeeperContextPtr &keeper_context_, KeeperSnapshotManagerS3 *snapshot_manager_s3_,
                                         CommitCallback commit_callback_, const std::string &superdigest_)
    : commit_callback(commit_callback_),
      ...
      read_pool(CurrentMetrics::KeeperAliveConnections, CurrentMetrics::KeeperAliveConnections, CurrentMetrics::KeeperAliveConnections, 100,
                10000, 10000),
...
KeeperStateMachine<Storage>::KeeperStateMachine(ResponsesQueue &responses_queue_, SnapshotsQueue &snapshots_queue_,
                                                // const CoordinationSettingsPtr & coordination_settings_,
                                                const KeeperContextPtr &keeper_context_, KeeperSnapshotManagerS3 *snapshot_manager_s3_,
                                                IKeeperStateMachine::CommitCallback commit_callback_, const std::string &superdigest_)
    : IKeeperStateMachine(responses_queue_, snapshots_queue_,
                          /// coordination_settings_,
                          keeper_context_, snapshot_manager_s3_, commit_callback_, superdigest_),
      snapshot_manager(keeper_context_->getCoordinationSettings()[CoordinationSetting::snapshots_to_keep], keeper_context_,
                       keeper_context_->getCoordinationSettings()[CoordinationSetting::compress_snapshots_with_zstd_format], superdigest_,
                       keeper_context_->getCoordinationSettings()[CoordinationSetting::dead_session_check_period_ms].totalMilliseconds()) {}

namespace {

bool isLocalDisk(const IDisk &disk) { return dynamic_cast<const DiskLocal *>(&disk) != nullptr; }
...
void KeeperStateMachine<Storage>::init() {
  ...
  if (has_snapshots) {
    try {
      ...
      if (isLocalDisk(*latest_snapshot_info->disk)) latest_snapshot_buf = nullptr;
      ...
      throw Exception(ErrorCodes::CORRUPTED_DATA,
                      ...
                      latest_log_index, getCurrentExceptionMessage(/* with_stacktrace= */ false, /* check_embedded_stacktrace= */ true));
    }
  }
  ...
  if (!storage)
    storage = std::make_unique<Storage>(
        keeper_context->getCoordinationSettings()[CoordinationSetting::dead_session_check_period_ms].totalMilliseconds(), superdigest,
        keeper_context);
}

namespace {

void assertDigest(const KeeperDigest &expected, const KeeperDigest &actual, const Coordination::ZooKeeperRequest &request, uint64_t log_idx,
                  uint64_t session_id, bool committing) {
  if (!KeeperStorageBase::checkDigest(expected, actual)) {
    LOG_FATAL(getLogger("KeeperStateMachine"),
              ...
              committing ? "committing" : "preprocessing", request.getOpNum(), log_idx, session_id, expected.value, actual.value,
              expected.version, request.toString());
    std::terminate();
  }
}
...
nuraft::ptr<nuraft::buffer> KeeperStateMachine<Storage>::pre_commit(uint64_t log_idx, nuraft::buffer &data) {
  ...
  /// all needed logs
  if (!keeper_context->localLogsPreprocessed()) return result;

  auto request_for_session = parseRequest(data, /*final=*/false);
  if (!request_for_session->zxid) request_for_session->zxid = log_idx;
  ...
  const auto maybe_log_opentelemetery_span = [&](OpenTelemetry::SpanStatus status, const std::string &error_message) {
    ZooKeeperOpentelemetrySpans::maybeInitialize(request_for_session->request->spans.pre_commit,
                                                 request_for_session->request->tracing_context, start_time_us);
...
nuraft::ptr<nuraft::buffer> IKeeperStateMachine::getZooKeeperLogEntry(const KeeperRequestForSession &request_for_session) {
  ...
  /// if new fields are added, update KeeperStateMachine::ZooKeeperLogSerializationVersion along with parseRequest function and PreAppendLog
  /// callback handler
  return write_buf.getBuffer();
}

std::shared_ptr<KeeperRequestForSession> IKeeperStateMachine::parseRequest(nuraft::buffer &data, bool final,
  ...
  if (length < 0) throw Exception(ErrorCodes::LOGICAL_ERROR, "Invalid request length: {}", length);
  ...
  if (request_end_position) *request_end_position = buffer.getPosition();
  ...
  if (serialization_version) *serialization_version = version;
  ...
  static constexpr std::array non_cacheable_xids{
      Coordination::WATCH_XID, Coordination::PING_XID, Coordination::AUTH_XID, Coordination::CLOSE_XID, Coordination::CLOSE_XID_64,
  };

  const bool should_cache = min_request_size_to_cache != 0 && request_for_session->session_id != -1 &&
                            data.size() >= min_request_size_to_cache &&
                            std::all_of(non_cacheable_xids.begin(), non_cacheable_xids.end(),
  ...
  if (tracing_context) request_for_session->request->tracing_context = std::move(tracing_context);
...
std::optional<KeeperDigest> KeeperStateMachine<Storage>::preprocess(const KeeperRequestForSession &request_for_session) {
  ...
  if (storage->isFinalized()) return std::nullopt;

  KeeperDigest digest_after_preprocessing;
  try {
    LockGuardWithStats<true> lock(storage_mutex);
    digest_after_preprocessing =
        storage->preprocessRequest(request_for_session.request, request_for_session.session_id, request_for_session.time,
                                   request_for_session.zxid, true /* check_acl */, request_for_session.digest, request_for_session.log_idx);
  } catch (...) {
    LOG_FATAL(log, "Failed to preprocess stored log at index {}: {}", request_for_session.log_idx,
  ...
  if (keeper_context->digestEnabled() && request_for_session.digest)
    assertDigest(*request_for_session.digest, digest_after_preprocessing, *request_for_session.request, request_for_session.log_idx,
                 request_for_session.session_id, false);
...
template <typename Storage>
KeeperResponseForSession KeeperStateMachine<Storage>::processReconfiguration(const KeeperRequestForSession &request_for_session) {
  ...
  if (!storage->checkACL(keeper_config_path, Coordination::ACL::Write, session_id, true)) return bad_request(ZNOAUTH);

  KeeperDispatcher &dispatcher = *keeper_context->getDispatcher();
  if (!dispatcher.reconfigEnabled()) return bad_request(ZUNIMPLEMENTED);
  if (request.version != -1) return bad_request(ZBADVERSION);
  ...
  if (!incremental_reconfig) return bad_request();
...
nuraft::ptr<nuraft::buffer> KeeperStateMachine<Storage>::commit(const uint64_t log_idx, nuraft::buffer &data) {
  ...
  if (!keeper_context->localLogsPreprocessed() && !preprocess(*request_for_session)) return nullptr;
  ...
  const auto maybe_log_opentelemetery_span = [&](OpenTelemetry::SpanStatus status, const std::string &error_message) {
    ZooKeeperOpentelemetrySpans::maybeInitialize(request_for_session->request->spans.commit, request_for_session->request->tracing_context,
                                                 start_time_us);
    ...
    if (op_num == Coordination::OpNum::SessionID) {
...

      {
        ...
        for (auto &response_for_session : responses_for_sessions) {
          if (response_for_session.response->xid != Coordination::WATCH_XID) response_for_session.request = request_for_session->request;
      ...
      if (keeper_context->digestEnabled() && request_for_session->digest)
        assertDigest(*request_for_session->digest, storage->getNodesDigest(true, /*lock_transaction_mutex=*/true),
                     *request_for_session->request, request_for_session->log_idx, request_for_session->session_id, true);
    }

    ProfileEvents::increment(ProfileEvents::KeeperCommits);

    if (commit_callback) commit_callback(log_idx, *request_for_session);
...
bool KeeperStateMachine<Storage>::apply_snapshot(nuraft::snapshot &s) {
  ...
  {  /// save snapshot into memory
    ...
    if (s.get_last_log_idx() < latest_snapshot_meta->get_last_log_idx()) {
      LOG_INFO(log, "A snapshot with a larger last log index ({}) was created, skipping applying this snapshot",
  ...
  {  /// deserialize and apply snapshot to storage
    ...
    storage->applyUncommittedState(*snapshot_deserialization_result.storage,
...
void KeeperStateMachine<Storage>::rollbackRequest(const KeeperRequestForSession &request_for_session, bool allow_missing) {
  if (request_for_session.request->getOpNum() == Coordination::OpNum::SessionID) return;
...
void KeeperStateMachine<Storage>::create_snapshot(nuraft::snapshot &s, nuraft::async_result<bool>::handler_type &when_done) {
        ...
        {  /// Read storage data without locks and create snapshot
          ...
          if (latest_snapshot_meta && snapshot->snapshot_meta->get_last_log_idx() <= latest_snapshot_meta->get_last_log_idx()) {
            LOG_INFO(log,
                     ...
                     "created",
                     snapshot->snapshot_meta->get_last_log_idx(), latest_snapshot_meta->get_last_log_idx());
          } else {
            ...
            if (isLocalDisk(*keeper_context->getLatestSnapshotDisk())) {
              ...
              auto snapshot_info =
                  snapshot_manager.serializeSnapshotBufferToDisk(*snapshot_buf, snapshot->snapshot_meta->get_last_log_idx());
              latest_snapshot_info = std::move(snapshot_info);
              latest_snapshot_buf = std::move(snapshot_buf);
            }

            ProfileEvents::increment(ProfileEvents::KeeperSnapshotCreations);
            LOG_DEBUG(log, "Created persistent snapshot {} with path {}", latest_snapshot_meta->get_last_log_idx(),
  ...
  if (!snapshots_queue.push(std::move(snapshot_task))) LOG_WARNING(log, "Cannot push snapshot task into queue");
}

template <typename Storage>
void KeeperStateMachine<Storage>::save_logical_snp_obj(nuraft::snapshot &s, uint64_t &obj_id, nuraft::buffer &data, bool /*is_first_obj*/,
                                                       bool /*is_last_obj*/) {
  ...
  /// we rely on the fact that the snapshot disk cannot be changed during runtime
  if (!isLocalDisk(*keeper_context->getSnapshotDisk())) cloned_buffer = nuraft::buffer::clone(data);
...
int IKeeperStateMachine::read_logical_snp_obj(nuraft::snapshot &s, void *& /*user_snp_ctx*/, uint64_t obj_id,
...
void KeeperStateMachine<Storage>::processReadRequest(const KeeperRequestForSession &request_for_session) {
  /// Pure local request, just process it with storage
  {
    ...
    std::lock_guard response_lock(process_and_responses_lock);
    auto responses = storage->processRequest(request_for_session.request, request_for_session.session_id, std::nullopt, true /*check_acl*/,
                                             true /*is_local*/);
    for (auto &response_for_session : responses) {
      ...
      if (response_for_session.request)
        ZooKeeperOpentelemetrySpans::maybeInitialize(response_for_session.request->spans.dispatcher_responses_queue,
                                                     response_for_session.request->tracing_context);
      if (!responses_queue.push(response_for_session))
        LOG_WARNING(log, "Failed to push response with session id {} to the queue, probably because of shutdown",
...
uint64_t KeeperStateMachine<Storage>::getLatestSnapshotSize() const {
  ...
  if (snapshot_info == nullptr || snapshot_info->disk == nullptr) return 0;
