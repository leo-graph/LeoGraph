...
SummingStateMachine::SummingStateMachine() : value(0), last_committed_idx(0) {}
...
bool SummingStateMachine::apply_snapshot(nuraft::snapshot& s) {
  ...
  if (entry == snapshots.end()) return false;
...
void SummingStateMachine::createSnapshotInternal(nuraft::snapshot& s) {
  ...
  for (ssize_t ii = 0; ii < num - MAX_SNAPSHOTS; ++ii) {
    if (entry == snapshots.end()) break;
    entry = snapshots.erase(entry);
  }
}

void SummingStateMachine::save_logical_snp_obj(nuraft::snapshot& s, uint64_t& obj_id, nuraft::buffer& data, bool /*is_first_obj*/,
                                               bool /*is_last_obj*/) {
  if (obj_id == 0) {
...
int SummingStateMachine::read_logical_snp_obj(nuraft::snapshot& s, void*& /*user_snp_ctx*/, uint64_t obj_id,
...
void SummingStateMachine::create_snapshot(nuraft::snapshot& s, nuraft::async_result<bool>::handler_type& when_done) {
  {
    std::lock_guard ll(snapshots_lock);
    createSnapshotInternal(s);
  }
