...
#  endif
  ...
  new_values["KeeperIsExceedingMemorySoftLimitHit"] = {is_exceeding_mem_soft_limit,
  ...
  /// new_values["KeeperLatestSnapshotSize"] = { latest_snapshot_size, "The uncompressed size in bytes of the latest snapshot created by
  /// ClickHouse Keeper." };
  ...
  new_values["KeeperLatestLogsCacheEntries"] = {keeper_log_info.latest_logs_cache_entries,
  ...
  new_values["KeeperCommitLogsCacheEntries"] = {keeper_log_info.commit_logs_cache_entries,
                                                "Number of entries stored in the in-memory cache for next logs to be committed"};
  new_values["KeeperCommitLogsCacheSize"] = {keeper_log_info.commit_logs_cache_size,
...
KeeperAsynchronousMetrics::KeeperAsynchronousMetrics(ContextPtr context_, unsigned update_period_seconds,
...
void KeeperAsynchronousMetrics::updateImpl(TimePoint /*update_time*/, TimePoint /*current_time*/, bool /*force_update*/, bool /*first_run*/,
