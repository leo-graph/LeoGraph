...
KeeperContext::KeeperContext(bool standalone_keeper_, CoordinationSettingsPtr coordination_settings_)
  ...
  static constexpr std::array enabled_by_default_feature_flags{
      KeeperFeatureFlag::FILTERED_LIST,    KeeperFeatureFlag::MULTI_READ,
      KeeperFeatureFlag::CHECK_NOT_EXISTS, KeeperFeatureFlag::CREATE_IF_NOT_EXISTS,
      KeeperFeatureFlag::REMOVE_RECURSIVE, KeeperFeatureFlag::MULTI_WATCHES,
      KeeperFeatureFlag::CHECK_STAT,       KeeperFeatureFlag::PERSISTENT_WATCHES,
      KeeperFeatureFlag::TRY_REMOVE,       KeeperFeatureFlag::LIST_WITH_STAT_AND_DATA,
  };

  for (const auto feature_flag : enabled_by_default_feature_flags) feature_flags.enableFeatureFlag(feature_flag);
...
static rocksdb::Options getRocksDBOptionsFromConfig(const Poco::Util::AbstractConfiguration &config) {
  ...
  if (config.has("keeper_server.rocksdb.options")) {
    ...
    if (!status.ok()) {
      throw Exception(ErrorCodes::ROCKSDB_ERROR, "Fail to merge rocksdb options from 'rocksdb.options' : {}", status.ToString());
    }
  }
...
KeeperContext::Storage KeeperContext::getRocksDBPathFromConfig(const Poco::Util::AbstractConfiguration &config) const {
  ...
  if (config.has("keeper_server.rocksdb_path")) return create_local_disk(config.getString("keeper_server.rocksdb_path"));

  if (config.has("keeper_server.storage_path"))
    return create_local_disk(std::filesystem::path{config.getString("keeper_server.storage_path")} / "rocksdb");

  if (standalone_keeper) return create_local_disk(std::filesystem::path{config.getString("path", KEEPER_DEFAULT_PATH)} / "rocksdb");
...
bool diskValidator(const Poco::Util::AbstractConfiguration &config, const std::string &disk_config_prefix, const std::string &) {
  ...
  if (std::all_of(supported_disk_types.begin(), supported_disk_types.end(),
...
void KeeperContext::initializeDisks(const Poco::Util::AbstractConfiguration &config) {
  ...
  const auto collect_old_disk_names = [&](const std::string_view key_prefix, std::vector<std::string> &disk_names) {
    ...
    for (const auto &key : disk_name_keys) {
      if (key.starts_with(key_prefix)) disk_names.push_back(config.getString(fmt::format("keeper_server.{}", key)));
    }
  };
...
KeeperContext::Phase KeeperContext::getServerState() const { return server_state; }

void KeeperContext::setServerState(KeeperContext::Phase server_state_) { server_state = server_state_; }

bool KeeperContext::ignoreSystemPathOnStartup() const { return ignore_system_path_on_startup; }

bool KeeperContext::digestEnabled() const { return digest_enabled; }

bool KeeperContext::digestEnabledOnCommit() const { return digest_enabled_on_commit; }

void KeeperContext::setDigestEnabled(bool digest_enabled_) { digest_enabled = digest_enabled_; }

DiskPtr KeeperContext::getDisk(const Storage &storage) const {
  if (const auto *storage_disk = std::get_if<DiskPtr>(&storage)) return *storage_disk;

  const auto &disk_name = std::get<std::string>(storage);
  return disk_selector->get(disk_name);
}

DiskPtr KeeperContext::getLogDisk() const { return getDisk(log_storage); }
...
std::vector<DiskPtr> KeeperContext::getOldLogDisks() const {
  ...
  for (const auto &disk_name : old_log_disk_names) old_log_disks.push_back(disk_selector->get(disk_name));

  return old_log_disks;
}

DiskPtr KeeperContext::getLatestLogDisk() const { return getDisk(latest_log_storage); }
...
DiskPtr KeeperContext::getLatestSnapshotDisk() const { return getDisk(latest_snapshot_storage); }

DiskPtr KeeperContext::getSnapshotDisk() const { return getDisk(snapshot_storage); }
...
std::vector<DiskPtr> KeeperContext::getOldSnapshotDisks() const {
  ...
  for (const auto &disk_name : old_snapshot_disk_names) old_snapshot_disks.push_back(disk_selector->get(disk_name));
...
DiskPtr KeeperContext::getStateFileDisk() const { return getDisk(state_file_storage); }

void KeeperContext::setStateFileDisk(DiskPtr disk) { state_file_storage = std::move(disk); }

const std::unordered_map<std::string, std::string> &KeeperContext::getSystemNodesWithData() const { return system_nodes_with_data; }

const KeeperFeatureFlags &KeeperContext::getFeatureFlags() const { return feature_flags; }
...
void KeeperContext::dumpConfiguration(WriteBufferFromOwnString &buf) const {
...

  {
    ...
    if (log_disk != latest_log_disk) dump_disk_info("latest_log_storage", *latest_log_disk);
  }
...
void KeeperContext::setRocksDBDisk(DiskPtr disk) { rocksdb_storage = std::move(disk); }
...
KeeperContext::Storage KeeperContext::getLogsPathFromConfig(const Poco::Util::AbstractConfiguration &config) const {
  ...
  if (config.has("keeper_server.log_storage_path")) return create_local_disk(config.getString("keeper_server.log_storage_path"));

  if (config.has("keeper_server.log_storage_disk")) return config.getString("keeper_server.log_storage_disk");

  if (config.has("keeper_server.storage_path"))
    return create_local_disk(std::filesystem::path{config.getString("keeper_server.storage_path")} / "logs");

  if (standalone_keeper) return create_local_disk(std::filesystem::path{config.getString("path", KEEPER_DEFAULT_PATH)} / "logs");
...
KeeperContext::Storage KeeperContext::getSnapshotsPathFromConfig(const Poco::Util::AbstractConfiguration &config) const {
  ...
  /// the most specialized path
  if (config.has("keeper_server.snapshot_storage_path")) return create_local_disk(config.getString("keeper_server.snapshot_storage_path"));

  if (config.has("keeper_server.snapshot_storage_disk")) return config.getString("keeper_server.snapshot_storage_disk");

  if (config.has("keeper_server.storage_path"))
    return create_local_disk(std::filesystem::path{config.getString("keeper_server.storage_path")} / "snapshots");

  if (standalone_keeper) return create_local_disk(std::filesystem::path{config.getString("path", KEEPER_DEFAULT_PATH)} / "snapshots");
...
KeeperContext::Storage KeeperContext::getStatePathFromConfig(const Poco::Util::AbstractConfiguration &config) const {
  ...
  if (config.has("keeper_server.state_storage_disk")) return config.getString("keeper_server.state_storage_disk");
  ...
  if (standalone_keeper) return create_local_disk(std::filesystem::path{config.getString("path", KEEPER_DEFAULT_PATH)});
...
bool KeeperContext::localLogsPreprocessed() const { return local_logs_preprocessed; }
...
const CoordinationSettings &KeeperContext::getCoordinationSettings() const { return *coordination_settings; }

int64_t KeeperContext::getPrecommitSleepMillisecondsForTesting() const { return precommit_sleep_ms_for_testing; }
...
bool KeeperContext::shouldBlockACL() const { return block_acl; }

void KeeperContext::setBlockACL(bool block_acl_) { block_acl = block_acl_; }
...
uint64_t KeeperContext::lastCommittedIndex() const { return last_committed_log_idx.load(std::memory_order_relaxed); }
...
void KeeperContext::setLastCommitIndex(uint64_t commit_index) {
  ...
  if (should_notify) last_committed_log_idx_cv.notify_all();
}
...
bool KeeperContext::waitCommittedUpto(uint64_t log_idx, uint64_t wait_timeout_ms) {
  ...
  bool success = last_committed_log_idx_cv.wait_for(lock, std::chrono::milliseconds(wait_timeout_ms),
...
bool KeeperContext::shouldLogRequests() const { return log_requests.load(std::memory_order_relaxed); }

void KeeperContext::setLogRequests(bool log_requests_) { log_requests.store(log_requests_, std::memory_order_relaxed); }
