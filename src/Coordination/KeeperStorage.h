...
struct NodeStats {
  ...
  bool isEphemeral() const { return is_ephemeral_and_ctime.is_ephemeral; }

  int64_t ephemeralOwner() const {
    if (isEphemeral()) return ephemeral_or_children_data.ephemeral_owner;
  ...
  void setSeqNum(int32_t seq_num) { ephemeral_or_children_data.children_info.seq_num = seq_num; }
  ...
  int64_t ctime() const { return is_ephemeral_and_ctime.ctime; }

  void setCtime(uint64_t ctime) { is_ephemeral_and_ctime.ctime = ctime; }
...
struct KeeperRocksNodeInfo {
  ...
  auto getChildren() const { return std::vector<int>(stats.numChildren()); }
...
struct KeeperRocksNode : public KeeperRocksNodeInfo {
  ...
  void reset() { serialized = false; }
  bool empty() const { return stats.data_size == 0 && stats.mzxid == 0; }
...
class KeeperStorageBase {
 public:
  ...
  using Watches = std::unordered_map<String /* path, relative of root_path */, SessionIDs, StringHashForHeterogeneousLookup,
...
class KeeperStorage : public KeeperStorageBase {
 public:
  ...
  struct UncommittedState {
    ...
    mutable std::unordered_map<std::string, UncommittedNode, StringHashForHeterogeneousLookup,
                               StringHashForHeterogeneousLookup::transparent_key_equal>
        nodes;

    using NodesIterator = decltype(nodes)::iterator;
    struct NodesIteratorHash {
      auto operator()(NodesIterator it) const { return std::hash<std::string_view>{}(it->first); }
    };
  ...
  bool createNode(const std::string &path, String data, const Coordination::Stat &stat, Coordination::ACLs node_acls, bool update_digest);
  ...
  KeeperResponsesForSessions processRequest(const Coordination::ZooKeeperRequestPtr &request, int64_t session_id,
                                            std::optional<int64_t> new_last_zxid, bool check_acl = true, bool is_local = false);
  KeeperDigest preprocessRequest(const Coordination::ZooKeeperRequestPtr &request, int64_t session_id, int64_t time, int64_t new_last_zxid,
                                 bool check_acl = true, std::optional<KeeperDigest> digest = std::nullopt, int64_t log_idx = 0);
  ...
  KeeperResponsesForSessions setWatches(int64_t last_zxid, const std::vector<String> &watches_paths,
                                        const std::vector<String> &list_watches_paths, const std::vector<String> &exist_watches_paths,
                                        const std::vector<String> &persistent_watches_paths,
                                        const std::vector<String> &persistent_recursive_watches_paths, int64_t session_id);
