...
TYPED_TEST(CoordinationTest, TestReapplyingDeltas) {
  ...
  const auto commit_initial_data = [&](auto &storage) {
    ...
    for (; zxid <= initial_zxid; ++zxid) process_create(storage, create_request, zxid);
  };
...
TYPED_TEST(CoordinationTest, TestRemoveRecursiveRequest) {
  ...
  const auto create = [&](const String &path, int create_mode) {
    ...
    create_request->is_sequential =
        create_mode == zkutil::CreateMode::PersistentSequential || create_mode == zkutil::CreateMode::EphemeralSequential;
...
TYPED_TEST(CoordinationTest, TestRemoveRecursiveInMultiRequest) {
  ...
  const auto is_multi_ok = [&](Coordination::ZooKeeperResponsePtr response) {
    ...
    for (const auto &op_response : multi_response.responses)
      if (op_response->error != Coordination::Error::ZOK) return false;
...
TYPED_TEST(CoordinationTest, TestUncommittedStateBasicCrud) {
  ...
  const auto get_committed_data = [&]() -> std::optional<String> {
    ...
    if (get_response.error != Error::ZOK) return std::nullopt;
...
TYPED_TEST(CoordinationTest, TestMultiWatches) {
  ...
  auto subscription =
      std::make_shared<Coordination::WatchCallback>([wait_event](const Coordination::WatchResponse &) { wait_event->set(); });
...
TYPED_TEST(CoordinationTest, TestCheckStat) {
  ...
  [[maybe_unused]] const auto run_check_request = [&](OpNum op, const std::string &path, int version, bool not_exists,
...

  {
    ...
    ASSERT_EQ(run_check_request(OpNum::CheckStat, "/A1", /*version=*/1, /*not_exists=*/false, /*stat_to_check=*/stat)->error,
              Error::ZBADVERSION);
