...
TYPED_TEST(CoordinationTest, TestStorageSnapshotMode) {
  ...
  }
  {
    ...
    for (size_t i = 0; i < 50; ++i) {
      if (i % 2 == 0) storage.container.erase(fmt::format("/hello_{}", i));
    }
...
TYPED_TEST(CoordinationTest, TestStorageSnapshotBroken) {
  ...
  /// Let's corrupt file
  DB::WriteBufferFromFile plain_buf("./snapshots/snapshot_50.bin" + this->extension, DB::DBMS_DEFAULT_BUFFER_SIZE,
...
TYPED_TEST(CoordinationTest, TestStorageSnapshotEqual) {
  ...
  for (size_t i = 0; i < 15; ++i) {
    ...
    for (size_t j = 0; j < 3333; ++j) storage.getSessionID(130 * j);
