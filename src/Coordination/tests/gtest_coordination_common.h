...
struct ChangelogDirTest {
  ...
  ~ChangelogDirTest() {
    if (fs::exists(path) && drop) fs::remove_all(path);
  }
};
...
#  endif
  ...
  void setRocksDBDirectory(const std::string &path) { keeper_context->setRocksDBDisk(std::make_shared<DB::DiskLocal>("RocksDisk", path)); }
...
void addNode(Storage &storage, const std::string &path, const std::string &data, int64_t ephemeral_owner = 0, uint64_t acl_id = 0) {
  ...
  Node node{};
  node.setData(data);
  if (ephemeral_owner) node.stats.setEphemeralOwner(ephemeral_owner);
  node.acl_id = acl_id;
  storage.container.insertOrReplace(path, node);
  auto child_it = storage.container.find(path);
  auto child_path = Coordination::getBaseNodeName(child_it->key);
  storage.container.updateValue(Coordination::parentNodePath(path), [&](auto &parent) {
...
#  if USE_ROCKSDB
                                      ,
                                      TestParam<DB::KeeperRocksStorage, true>, TestParam<DB::KeeperRocksStorage, false>
#  endif
                                      >;
...
nuraft::ptr<nuraft::log_entry> getLogEntryFromZKRequest(size_t term, int64_t session_id, int64_t zxid,
