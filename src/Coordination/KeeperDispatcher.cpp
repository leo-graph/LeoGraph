...
bool checkIfRequestIncreaseMem(const Coordination::ZooKeeperRequestPtr &request) {
  if (request->getOpNum() == Coordination::OpNum::Create || request->getOpNum() == Coordination::OpNum::Create2 ||
      request->getOpNum() == Coordination::OpNum::CreateIfNotExists || request->getOpNum() == Coordination::OpNum::Set) {
    return true;
  }
...
void KeeperDispatcher::requestThread() {
  ...
  while (!shutdown_called) {
    const auto handle_opentelemetery_spans = [this](const Coordination::ZooKeeperRequestPtr &request, int64_t session_id) {
      ZooKeeperOpentelemetrySpans::maybeFinalize(request->spans.dispatcher_requests_queue, [&] {
    ...
    try {
      if (requests_queue->tryPop(request, max_wait)) {
        ...
        if (configuration_and_settings->standalone_keeper && isExceedingMemorySoftLimit() && checkIfRequestIncreaseMem(request.request)) {
          ...
          LOG_WARNING(log,
                      "Processing requests refused because of max_memory_usage_soft_limit {}, the total allocated memory is {}, RSS is {}, "
                      "request type "
                      "is {}",
                      ReadableSize(mem_soft_limit), ReadableSize(total_memory_tracker.get()), ReadableSize(total_memory_tracker.getRSS()),
        ...
        if (prev_result)
          result_buf = forceWaitAndProcessResult(prev_result, prev_batch, /*clear_requests_on_success=*/!execute_requests_after_write);

        /// Process collected write requests batch
        if (!current_batch.empty()) {
          if (current_batch.size() == max_batch_size) ProfileEvents::increment(ProfileEvents::KeeperBatchMaxCount, 1);

          if (current_batch_bytes_size == max_batch_bytes_size) ProfileEvents::increment(ProfileEvents::KeeperBatchMaxTotalSize, 1);
        ...
        if (execute_requests_after_write) {
          ...
          if (result_buf) {
            ...
            if (!keeper_context->waitCommittedUpto(log_idx,
                                                   coordination_settings[CoordinationSetting::operation_timeout_ms].totalMilliseconds()))
        ...
        if (has_reconfig_request) server->getKeeperStateMachine()->reconfigure(request);
    ...
    if (responses_queue.tryPop(response_for_session, max_wait)) {
      ...
      if (response_was_sent && response_for_session.request) {
        ZooKeeperOpentelemetrySpans::maybeFinalize(
            ...
            OpenTelemetry::SpanStatus::OK, "", dequeue_time_us);
      }
    }
  }
}
...
void KeeperDispatcher::snapshotThread() {
  ...
  while (snapshots_queue.pop(task)) {
    try {
      ...
      if (!snapshot_file_info) continue;

      chassert(snapshot_file_info->disk != nullptr);
      if (isLeader()) snapshot_s3.uploadSnapshot(snapshot_file_info);
    } catch (...) {
      tryLogCurrentException(__PRETTY_FUNCTION__);
    }
  }
...
bool KeeperDispatcher::setResponse(int64_t session_id, const Coordination::ZooKeeperResponsePtr &response,
  ...
  } else  /// Normal response, just write to client
  {
    ...
    if (session_response_callback == session_to_response_callback.end()) return false;
...
bool KeeperDispatcher::putRequest(const Coordination::ZooKeeperRequestPtr &request, int64_t session_id, bool use_xid_64) {
  {
    ...
    std::lock_guard lock(session_to_response_callback_mutex);
    if (!session_to_response_callback.contains(session_id)) return false;
  }
  ...
  if (keeper_context->isShutdownCalled()) return false;
  ...
  if (request->getOpNum() == Coordination::OpNum::Close) {
    if (!requests_queue->push(std::move(request_info))) throw Exception(ErrorCodes::SYSTEM_ERROR, "Cannot push request to queue");
...
void KeeperDispatcher::initialize(const Poco::Util::AbstractConfiguration &config, bool standalone_keeper, bool start_async,
                                  const MultiVersion<Macros>::Version &macros) {
  ...
  keeper_context = std::make_shared<KeeperContext>(
      standalone_keeper, std::make_shared<CoordinationSettings>(configuration_and_settings->coordination_settings));

  keeper_context->initialize(config, this);

  requests_queue =
      std::make_unique<RequestsQueue>(configuration_and_settings->coordination_settings[CoordinationSetting::max_request_queue_size]);
  ...
  server = std::make_unique<KeeperServer>(
      configuration_and_settings, config, responses_queue, snapshots_queue, keeper_context, snapshot_s3,
          ...
          if (auto it = read_request_queue.find(request_for_session.session_id); it != read_request_queue.end()) {
            ...
            if (auto request_queue_it = xid_to_request_queue.find(request_for_session.request->xid);
              ...
              for (const auto &read_request : request_queue_it->second) {
                ...
                ZooKeeperOpentelemetrySpans::maybeFinalize(read_request.request->spans.read_wait_for_write, [&] {
                  return std::vector<OpenTelemetry::SpanAttribute>{
  ...
  update_configuration_thread = reconfigEnabled() ? ThreadFromGlobalPool([this] { clusterUpdateThread(); })
...
void KeeperDispatcher::shutdown() {
  try {
    {
      if (!keeper_context || !keeper_context->setShutdownCalled()) return;

      LOG_DEBUG(log, "Shutting down storage dispatcher");

      if (session_cleaner_thread.joinable()) session_cleaner_thread.join();
      ...
      if (responses_thread.joinable()) responses_thread.join();

      snapshots_queue.finish();
      if (snapshot_thread.joinable()) snapshot_thread.join();

      cluster_update_queue.finish();
      if (update_configuration_thread.joinable()) update_configuration_thread.join();
    }
    ...
    KeeperRequestsForSessions close_requests;
    {
      ...
      if (server && hasLeader()) {
        ...
        for (const auto &[session, response] : session_to_response_callback) {
          ...
          using namespace std::chrono;
          KeeperRequestForSession request_info{.session_id = session,
    ...
    if (server && !close_requests.empty()) {
      ...
      if (hasLeader()) {
        ...
        auto session_shutdown_timeout =
            configuration_and_settings->coordination_settings[CoordinationSetting::session_shutdown_timeout].totalMilliseconds();
        if (sessions_closing_done.wait_for(std::chrono::milliseconds(session_shutdown_timeout)) != std::future_status::ready)
          LOG_WARNING(log, "Failed to close sessions in {}ms. If they are not closed, they will be closed after session timeout.",
...
void KeeperDispatcher::forceRecovery() { server->forceRecovery(); }

KeeperDispatcher::~KeeperDispatcher() { shutdown(); }
...
void KeeperDispatcher::sessionCleanerTask() {
  ...
  while (true) {
    ...
    try {
      ...
      if (server->checkInit() && isLeader()) {
        ...
        for (int64_t dead_session : dead_sessions) {
          ...
          using namespace std::chrono;
          KeeperRequestForSession request_info{.session_id = dead_session,
                                               .time = duration_cast<milliseconds>(system_clock::now().time_since_epoch()).count(),
                                               .request = std::move(request),
                                               .digest = std::nullopt};
    ...
    auto time_to_sleep =
        configuration_and_settings->coordination_settings[CoordinationSetting::dead_session_check_period_ms].totalMilliseconds();
...
void KeeperDispatcher::addErrorResponses(const KeeperRequestsForSessions &requests_for_sessions, Coordination::Error error) {
  for (const auto &request_for_session : requests_for_sessions) {
    ...
    if (!responses_queue.push(DB::KeeperResponseForSession{request_for_session.session_id, response}))
      throw Exception(ErrorCodes::SYSTEM_ERROR, "Could not push error response xid {} zxid {} error message {} to responses queue",
                      response->xid, response->zxid, error);
  }
}

nuraft::ptr<nuraft::buffer> KeeperDispatcher::forceWaitAndProcessResult(RaftAppendResult &result,
                                                                        KeeperRequestsForSessions &requests_for_sessions,
                                                                        bool clear_requests_on_success) {
  ...
  if (!result_buf || clear_requests_on_success) requests_for_sessions.clear();
...
int64_t KeeperDispatcher::getSessionID(int64_t session_timeout_ms) {
  ...
  {
    std::lock_guard lock(session_to_response_callback_mutex);
    new_session_id_response_callback[request->internal_id] = [promise, internal_id = request->internal_id](
                                                                 const Coordination::ZooKeeperResponsePtr &response,
                                                                 Coordination::ZooKeeperRequestPtr /*request*/) {
      if (response->getOpNum() != Coordination::OpNum::SessionID) {
        promise->set_exception(std::make_exception_ptr(
            Exception(ErrorCodes::LOGICAL_ERROR, "Incorrect response of type {} instead of SessionID response", response->getOpNum())));
        return;
      }
      ...
      if (session_id_response.internal_id != internal_id) {
        promise->set_exception(
            std::make_exception_ptr(Exception(ErrorCodes::LOGICAL_ERROR, "Incorrect response with internal id {} instead of {}",
                                              session_id_response.internal_id, internal_id)));
        return;
      }
...
void KeeperDispatcher::clusterUpdateThread() {
  ...
  while (!shutdown_called) {
    ClusterUpdateAction action;
    if (!cluster_update_queue.pop(action)) return;
    ...
    if (const auto res = server->applyConfigUpdate(action, last_command_was_leader_change); res == Accepted)
      ...
      std::this_thread::sleep_for(
          last_command_was_leader_change
...
void KeeperDispatcher::pushClusterUpdates(ClusterUpdateActions &&actions) {
  ...
  for (auto &&action : actions) {
    if (!cluster_update_queue.push(std::move(action))) throw Exception(ErrorCodes::LOGICAL_ERROR, "Cannot push configuration update");
    LOG_DEBUG(log, "Processing config update {}: pushed", action);
  }
}

bool KeeperDispatcher::reconfigEnabled() const { return server->reconfigEnabled(); }

bool KeeperDispatcher::isServerActive() const { return checkInit() && hasLeader() && !server->isRecovering(); }
...
void KeeperDispatcher::updateConfiguration(const Poco::Util::AbstractConfiguration &config, const MultiVersion<Macros>::Version &macros) {
  ...
  if (!reconfigEnabled())
    for (auto &change : diff)
      if (!cluster_update_queue.push(change)) throw Exception(ErrorCodes::SYSTEM_ERROR, "Cannot push configuration update to queue");
...
uint64_t KeeperDispatcher::getLogDirSize() const {
  ...
  if (log_disk != latest_log_disk) size += getTotalSize(latest_log_disk);

  return size;
}

uint64_t KeeperDispatcher::getSnapDirSize() const { return getTotalSize(keeper_context->getSnapshotDisk()); }
...
void KeeperDispatcher::executeClusterUpdateActionAndWaitConfigChange(const ClusterUpdateAction &action,
                                                                     KeeperDispatcher::ConfigCheckCallback check_callback,
                                                                     size_t max_action_wait_time_ms, int64_t retry_count) {
  for (int64_t attempt = 0; attempt <= retry_count; ++attempt) {
    ...
    while (watch.elapsedMilliseconds() < max_action_wait_time_ms) {
      if (keeper_context->isShutdownCalled()) throw Exception(ErrorCodes::ABORTED, "Shutdown called, aborting configuration update");
    ...
    LOG_INFO(log, "Timeout exceeded waiting for configuration update {} to be applied, attempt {}/{}", action, attempt + 1,
             retry_count + 1);
  }
  throw Exception(ErrorCodes::TIMEOUT_EXCEEDED, "Timeout exceeded (with retries count {}) waiting for configuration update {} to happen",
                  action, retry_count);
}
...
void KeeperDispatcher::checkReconfigCommandPreconditions(Poco::JSON::Object::Ptr reconfig_command) {
  if (reconfig_command->has("preconditions")) {
    ...
    if (preconditions->has("members")) {
      ...
      for (unsigned int i = 0; i < members->size(); ++i) nodes.insert(members->getElement<int32_t>(i));

      auto servers_in_config = latest_config->get_servers();
      if (nodes.size() != servers_in_config.size())
        throw Exception(ErrorCodes::BAD_ARGUMENTS, "Precondition failed: expected members size {}, actual {}", nodes.size(),
    ...
    if (preconditions->has("leaders")) {
      ...
      for (unsigned int i = 0; i < leaders_array->size(); ++i) leaders.insert(leaders_array->getElement<int32_t>(i));

      if (!server->isLeaderAlive())
        throw Exception(ErrorCodes::BAD_ARGUMENTS, "Precondition failed: expected leader id {} but there is no leader currently",
                        fmt::join(leaders, ", "));

      if (!leaders.contains(static_cast<int32_t>(server->getLeaderID())))
        throw Exception(ErrorCodes::BAD_ARGUMENTS, "Precondition failed: expected leader id {} does not match actual leader id {}",
                        fmt::join(leaders, ", "), server->getLeaderID());
    }
  }
}
void KeeperDispatcher::checkReconfigCommandActions(Poco::JSON::Object::Ptr reconfig_command) {
  if (!reconfig_command->has("actions")) throw Exception(ErrorCodes::BAD_ARGUMENTS, "Reconfigure command must have 'actions' field");
  ...
  auto is_leader = [&state_model](int32_t server_id) -> bool {
    ...
    if (it == state_model.end()) return false;
    return it->second;
  };
    ...
    if (action_obj->has("remove_members")) {
      ...
      for (const auto &member_id_json : *remove_members) {
        ...
        if (member_id == server->getServerID()) {
          throw Exception(ErrorCodes::BAD_ARGUMENTS,
                          "Reconfigure command cannot remove current server id {} from cluster because it would make other servers ignore "
                          "all the commands from this one",
                          member_id);
        }
      ...
      for (const auto &member_id_json : *add_members) {
        ...
        if (state_model.contains(member_id)) {
          LOG_INFO(log, "Reconfigure command cannot add server id {} to cluster because it's already present, will do nothing", member_id);
        }
      ...
      for (const auto &priority_change : *set_priority_obj) {
        ...
        if (is_leader(member_id) && priority == 0) {
          throw Exception(ErrorCodes::BAD_ARGUMENTS,
                          "Reconfigure command cannot set priority 0 for server {}, because it can be potentially a leader", member_id);
        }
      }
    } else {
...
Poco::JSON::Object::Ptr KeeperDispatcher::reconfigureClusterFromReconfigureCommand(Poco::JSON::Object::Ptr reconfig_command) try {
  if (!server->isLeaderAlive()) throw Exception(ErrorCodes::BAD_ARGUMENTS, "Cannot reconfigure cluster because there is no active leader");
  ...
  size_t max_action_wait_time_ms =
      reconfig_command->has("max_action_wait_time_ms") ? reconfig_command->getValue<size_t>("max_action_wait_time_ms") : 60000;

  size_t max_total_wait_time_ms =
      reconfig_command->has("max_total_wait_time_ms") ? reconfig_command->getValue<size_t>("max_total_wait_time_ms") : 300000;
  ...
  for (const auto &action_json : *actions) {
    ...
    if (action_obj->has("retry")) retry_count = std::min(retry_count, action_obj->getValue<int64_t>("retry"));
    ...
    if (action_obj->has("remove_members")) {
      ...
      for (const auto &member_id_json : *remove_members) {
        ...
        auto remove_callback = [this, member_id](KeeperServer *server_) -> bool {
          ...
          if (config->get_server(member_id) == nullptr) {
            LOG_INFO(log, "Skip removing server id {} from cluster because it's not present in current configuration: {}", member_id,
                     serializeClusterConfig(config));
            return true;
          } else {
            LOG_INFO(log, "Waiting for removing server id {} from cluster configuration: {}", member_id, serializeClusterConfig(config));
            return false;
          }
        };
      ...
      for (const auto &member_id_json : *add_members) {
        ...
        auto add_callback = [this, member_id](KeeperServer *server_) -> bool {
          auto config = server_->getKeeperStateMachine()->getClusterConfig();
          if (config->get_server(member_id) != nullptr) {
            LOG_INFO(log, "Skip adding server id {} to cluster because it's already present in current configuration", member_id);
            return true;
          } else {
            LOG_INFO(log, "Waiting for adding server id {} to cluster configuration", member_id);
            return false;
          }
        };
      ...
      std::vector<int32_t> leader_ids;
      for (const auto &leader_id_json : *potential_laders) leader_ids.push_back(leader_id_json.convert<int32_t>());
      ...
      auto check_callback = [this, new_leader_id](KeeperServer *server_) -> bool {
        if (server_->getLeaderID() == new_leader_id) {
          LOG_INFO(log, "Leadership successfully transferred to server id {}", new_leader_id);
          return true;
        } else {
          LOG_INFO(log, "Waiting for leadership transfer to server id {}. Current leader id is {}", new_leader_id, server_->getLeaderID());
      ...
      for (const auto &priority_change : *set_priority_obj) {
        ...
        auto priority_callback = [this, member_id, priority](KeeperServer *server_) -> bool {
          ...
          if (server_in_config == nullptr) {
            LOG_INFO(log, "Cannot set priority for server id {} because it's not present in current configuration", member_id);
            return true;
          } else if (server_in_config->get_priority() == priority) {
            LOG_INFO(log, "Priority for server id {} successfully changed to {}", member_id, priority);
            return true;
          } else {
            LOG_INFO(log, "Waiting for setting priority {} for server id {} in cluster configuration, current {}", priority, member_id,
                     server_in_config->get_priority());
            return false;
          }
        };
