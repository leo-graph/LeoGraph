...
std::unordered_map<UInt64, std::string> getClientPorts(const Poco::Util::AbstractConfiguration &config) {
  ...
  for (const auto &config_port_name : config_port_names) {
    if (config.has(config_port_name)) ports[config.getUInt64(config_port_name)] = config_port_name;
  }
  return ports;
}
...
std::optional<AuthenticationData> getClientPasswordAuthentication(const Poco::Util::AbstractConfiguration &config) {
  ...
  for (const auto &[config_password_name, auth_type] : AUTH_TYPE_MAP) {
    if (config.has(config_password_name)) {
      if (data.has_value()) throw Exception(ErrorCodes::BAD_ARGUMENTS, "Only one authentication type is allowed in config");
      ...
      if (config_password_name.ends_with("client_password")) {
        ...
        if (password.length() > Coordination::PASSWORD_LENGTH)
          throw Exception(ErrorCodes::BAD_ARGUMENTS, "Password cannot be longer than {} characters, specified {}",
                          Coordination::PASSWORD_LENGTH, password.size());
...
KeeperStateManager::KeeperConfigurationWrapper KeeperStateManager::parseServersConfiguration(
    const Poco::Util::AbstractConfiguration &config, bool allow_without_us, bool enable_async_replication) const {
  ...
  for (const auto &server_key : keys) {
    if (!startsWith(server_key, "server")) continue;
    ...
    if (client_ports.contains(port)) {
      throw Exception(ErrorCodes::RAFT_ERROR,
                      "Raft configuration contains hostname '{}' with port '{}' which is equal to '{}' in server configuration", hostname,
                      port, client_ports[port]);
    }
    ...
    if (start_as_follower) result.servers_start_as_followers.insert(new_server_id);
...
KeeperStateManager::KeeperStateManager(int server_id_, const std::string &host, int port, KeeperContextPtr keeper_context_)
    : my_server_id(server_id_),
      secure(false),
      log_store(nuraft::cs_new<KeeperLogStore>(LogFileSettings{.force_sync = false, .compress_logs = false, .rotate_interval = 5000},
                                               FlushSettings{}, keeper_context_)),
...
KeeperStateManager::KeeperStateManager(int my_server_id_, const std::string &config_prefix_, const std::string &server_state_file_name_,
                                       const Poco::Util::AbstractConfiguration &config, KeeperContextPtr keeper_context_)
    : my_server_id(my_server_id_),
      secure(config.getBool(config_prefix_ + ".raft_configuration.secure", false)),
      config_prefix(config_prefix_),
      configuration_wrapper(
          parseServersConfiguration(config, false, keeper_context_->getCoordinationSettings()[CoordinationSetting::async_replication])),
      log_store(nuraft::cs_new<KeeperLogStore>(
          LogFileSettings{
              ...
              .latest_logs_cache_size_threshold =
                  keeper_context_->getCoordinationSettings()[CoordinationSetting::latest_logs_cache_size_threshold],
              .latest_logs_cache_entry_count_threshold =
                  keeper_context_->getCoordinationSettings()[CoordinationSetting::latest_logs_cache_entry_count_threshold],
              .commit_logs_cache_size_threshold =
                  keeper_context_->getCoordinationSettings()[CoordinationSetting::commit_logs_cache_size_threshold],
              .commit_logs_cache_entry_count_threshold =
                  keeper_context_->getCoordinationSettings()[CoordinationSetting::commit_logs_cache_entry_count_threshold],
          },
          FlushSettings{
...
ClusterConfigPtr KeeperStateManager::getLatestConfigFromLogStore() const {
  ...
  if (entry_with_change) return ClusterConfig::deserialize(entry_with_change->get_buf());
  return nullptr;
}
...
void KeeperStateManager::flushAndShutDownLogStore() { log_store->flushChangelogAndShutdown(); }
...
const String &KeeperStateManager::getOldServerStatePath() {
  static auto old_path = [this] { return server_state_file_name + "-OLD"; }();

  return old_path;
}

DiskPtr KeeperStateManager::getStateFileDisk() const { return keeper_context->getStateFileDisk(); }

namespace {
enum ServerStateVersion : uint8_t { V1 = 0 };
...
nuraft::ptr<nuraft::srv_state> KeeperStateManager::read_state() {
  ...
  const auto try_read_file = [&](const auto &path) -> nuraft::ptr<nuraft::srv_state> {
    try {
      ...
      if (content_size == 0) return nullptr;
...
ClusterUpdateActions KeeperStateManager::getRaftConfigurationDiff(const Poco::Util::AbstractConfiguration &config,
                                                                  const CoordinationSettings &coordination_settings) const {
  ...
  for (const auto &new_server : new_configuration_wrapper.cluster_config->get_servers()) new_ids[new_server->get_id()] = new_server;

  {
    std::lock_guard lock(configuration_wrapper_mutex);
    for (const auto &old_server : configuration_wrapper.cluster_config->get_servers()) old_ids[old_server->get_id()] = old_server;
  }
  ...
  for (const auto &[new_id, server_config] : new_ids) {
    ...
    if (old_server_it == old_ids.end())
      ...
      if (old_endpoint != server_config->get_endpoint()) {
        LOG_WARNING(getLogger("RaftConfiguration"),
  ...
  for (const auto &[old_id, server_config] : old_ids)
    if (!new_ids.contains(old_id)) result.emplace_back(RemoveRaftServer{old_id});
      ...
      for (const auto &new_server : new_configuration_wrapper.cluster_config->get_servers()) {
        if (old_server->get_id() == new_server->get_id()) {
          if (old_server->get_priority() != new_server->get_priority()) {
            result.emplace_back(UpdateRaftServerPriority{.id = new_server->get_id(), .priority = new_server->get_priority()});
          }
          break;
        }
      }
