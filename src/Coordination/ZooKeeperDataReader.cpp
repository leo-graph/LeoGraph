...
int64_t deserializeStorageData(Storage &storage, ReadBuffer &in, LoggerPtr log) TSA_NO_THREAD_SAFETY_ANALYSIS {
  ...
  while (path != "/") {
    ...
    if (!path.empty()) {
      if (ephemeral_owner == 0) node.stats.setSeqNum(node.stats.cversion);
    ...
    Coordination::read(path, in);
    count++;
    if (count % 1000 == 0) LOG_INFO(log, "Deserialized nodes from snapshot: {}", count);
  }

  for (const auto &itr : storage.container) {
    if (itr.key != "/") {
      auto parent_path = Coordination::parentNodePath(itr.key);
      storage.container.updateValue(parent_path, [my_path = itr.key](typename Storage::Node &value) {
...
void deserializeKeeperStorageFromSnapshot(Storage &storage, const std::string &snapshot_path, LoggerPtr log) TSA_NO_THREAD_SAFETY_ANALYSIS {
  ...
  if (zxid_from_nodes > zxid)
    LOG_WARNING(log,
                "ZooKeeper snapshot was in inconsistent (fuzzy) state. Will try to apply log. ZooKeeper create non fuzzy snapshot with "
                "restart. You can just restart ZooKeeper server and get consistent version.");
...
void deserializeKeeperStorageFromSnapshotsDir(Storage &storage, const std::string &path, LoggerPtr log) {
  ...
  for (const auto &p : fs::directory_iterator(path)) {
    ...
    if (!log_path.has_filename() || !startsWith(log_path.filename(), "snapshot.")) continue;
...
Coordination::ZooKeeperRequestPtr deserializeTxnImpl(ReadBuffer &in, bool subtxn, int64_t txn_length = 0) {
  ...
  if (subtxn) Coordination::read(sub_txn_length, in);

  bool empty_txn = !subtxn && txn_length == 32;  /// Possible for old-style CloseTxn's

  if (empty_txn && type != -11) throw Exception(ErrorCodes::NOT_IMPLEMENTED, "Empty non close-session transaction found");
  ...
  if (subtxn) {
    ...
    if (bytes_read < sub_txn_length) in.ignore(sub_txn_length - bytes_read);
  }
...
bool isErrorRequest(Coordination::ZooKeeperRequestPtr request) { return request == nullptr; }
...
bool hasErrorsInMultiRequest(Coordination::ZooKeeperRequestPtr request) {
  ...
  for (const auto &subrequest : dynamic_cast<Coordination::ZooKeeperMultiRequest *>(request.get())->requests)
    if (subrequest == nullptr) return true;
  return false;
}
...
bool deserializeTxn(Storage &storage, ReadBuffer &in, LoggerPtr /*log*/) TSA_NO_THREAD_SAFETY_ANALYSIS {
  ...
  if (bytes_read < txn_len) in.ignore(txn_len - bytes_read);

  /// We don't need to apply error requests
  if (isErrorRequest(request)) return true;
  ...
  if (zxid > storage.zxid) {
    ...
    if (request->getOpNum() == Coordination::OpNum::SessionID) {
      const Coordination::ZooKeeperSessionIDRequest &session_id_request =
          dynamic_cast<const Coordination::ZooKeeperSessionIDRequest &>(*request);
      storage.getSessionID(session_id_request.session_timeout_ms);
    } else {
      /// Skip failed multirequests
      if (request->getOpNum() == Coordination::OpNum::Multi && hasErrorsInMultiRequest(request)) return true;
...
void deserializeLogAndApplyToStorage(Storage &storage, const std::string &log_path, LoggerPtr log) {
  ...
  while (!reader.eof() && deserializeTxn(storage, reader, log)) {
    counter++;
    if (counter % 1000 == 0) LOG_INFO(log, "Deserialized txns log: {}", counter);

    int8_t forty_two;
    Coordination::read(forty_two, reader);
    if (forty_two != 0x42) throw Exception(ErrorCodes::NOT_IMPLEMENTED, "Forty two check byte ({}) is not equal 0x42", forty_two);
  }
...
void deserializeLogsAndApplyToStorage(Storage &storage, const std::string &path, LoggerPtr log) TSA_NO_THREAD_SAFETY_ANALYSIS {
  ...
  for (const auto &p : fs::directory_iterator(path)) {
    const auto &log_path = p.path();
    if (!log_path.has_filename() || !startsWith(log_path.filename(), "log.")) continue;
    int64_t zxid = getZxidFromName(log_path);
...
template void deserializeKeeperStorageFromSnapshot<KeeperMemoryStorage>(KeeperMemoryStorage &storage, const std::string &snapshot_path,
                                                                        LoggerPtr log);
template void deserializeKeeperStorageFromSnapshotsDir<KeeperMemoryStorage>(KeeperMemoryStorage &storage, const std::string &path,
                                                                            LoggerPtr log);
template void deserializeLogAndApplyToStorage<KeeperMemoryStorage>(KeeperMemoryStorage &storage, const std::string &log_path,
                                                                   LoggerPtr log);
