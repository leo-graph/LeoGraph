...
void moveFileBetweenDisks(DiskPtr disk_from, const std::string& path_from, DiskPtr disk_to, const std::string& path_to,
                          std::function<void()> before_file_remove_op, LoggerPtr logger, const KeeperContextPtr& keeper_context) {
  ...
  auto run_with_retries = [&](const auto& op, std::string_view operation_description) {
    ...
    do {
      try {
        ...
        tryLogCurrentException(
            logger, fmt::format("While moving file {} to disk {} and running '{}'", path_from, disk_to->getName(), operation_description));
    ...
    LOG_ERROR(logger, "Failed to run '{}' while moving file {} to disk {}", operation_description, path_from, disk_to->getName());
    return false;
  };
  ...
  if (!run_with_retries([&] { disk_from->copyFile(from_path, *disk_to, path_to, {}); }, "copying file")) return;

  if (!run_with_retries([&] { disk_to->removeFileIfExists(tmp_file_name); }, "removing temporary file")) return;

  if (before_file_remove_op) before_file_remove_op();

  if (!run_with_retries([&] { disk_from->removeFileIfExists(path_from); }, "removing file from source disk")) return;
}
}  // namespace DB
