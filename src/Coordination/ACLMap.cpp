...
bool ACLMap::ACLsComparator::operator()(const Coordination::ACLs& left, const Coordination::ACLs& right) const {
  ...
  for (size_t i = 0; i < left.size(); ++i) {
    if (left[i].permissions != right[i].permissions) return false;

    if (left[i].scheme != right[i].scheme) return false;
...
uint64_t ACLMap::convertACLs(const Coordination::ACLs& acls) {
  if (acls.empty()) return 0;

  std::lock_guard lock(map_mutex);
  if (acl_to_num.contains(acls)) return acl_to_num[acls];
...
Coordination::ACLs ACLMap::convertNumber(uint64_t acls_id) const {
  if (acls_id == 0) return Coordination::ACLs{};

  std::lock_guard lock(map_mutex);
  if (!num_to_acl.contains(acls_id)) throw Exception(ErrorCodes::LOGICAL_ERROR, "Unknown ACL id {}. It's a bug", acls_id);
...
void ACLMap::removeUsage(uint64_t acl_id) {
  std::lock_guard lock(map_mutex);
  if (!usage_counter.contains(acl_id)) return;
