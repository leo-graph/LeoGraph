...
class IKeeperStateMachine : public nuraft::state_machine {
 public:
  ...
  IKeeperStateMachine(ResponsesQueue& responses_queue_, SnapshotsQueue& snapshots_queue_, const KeeperContextPtr& keeper_context_,
                      KeeperSnapshotManagerS3* snapshot_manager_s3_, CommitCallback commit_callback_, const std::string& superdigest_);
  ...
  std::shared_ptr<KeeperRequestForSession> parseRequest(nuraft::buffer& data, bool final,
  ...
  int read_logical_snp_obj(nuraft::snapshot& s, void*& user_snp_ctx, uint64_t obj_id, nuraft::ptr<nuraft::buffer>& data_out,
 ...
 protected:
  ...
  virtual KeeperResponseForSession processReconfiguration(const KeeperRequestForSession& request_for_session) = 0;
};
...
class KeeperStateMachine : public IKeeperStateMachine {
 public:
  KeeperStateMachine(ResponsesQueue& responses_queue_, SnapshotsQueue& snapshots_queue_,
  ...
  // in a reasonable way.
  Storage& getStorageUnsafe() { return *storage; }
