...
RaftServerConfig::operator nuraft::srv_config() const noexcept { return {id, 0, endpoint, "", learner, priority}; }
...
std::optional<RaftServerConfig> RaftServerConfig::parse(std::string_view server) noexcept {
  ...
  if (!with_id_endpoint && !with_server_type && !with_priority) return std::nullopt;

  std::string_view id_str = parts[0];
  if (!id_str.starts_with("server.")) return std::nullopt;

  id_str = id_str.substr(7);
  if (auto eq_pos = id_str.find('='); std::string_view::npos != eq_pos) id_str = id_str.substr(0, eq_pos);

  Int32 id;
  if (!tryParse(id, id_str)) return std::nullopt;
  if (id < 0) return std::nullopt;

  const std::string_view endpoint = parts[1];
  const size_t port_delimiter = endpoint.find_last_of(':');
  if (port_delimiter == std::string::npos) return {};
  const std::string_view port = endpoint.substr(port_delimiter + 1);

  uint16_t port_tmp;
  if (!tryParse(port_tmp, port)) return std::nullopt;

  RaftServerConfig out{id, endpoint};

  if (with_id_endpoint) return out;

  if (parts[2] != "learner" && parts[2] != "participant") return std::nullopt;
  out.learner = parts[2] == "learner";
  if (with_server_type) return out;

  const std::string_view priority = parts[3];
  if (!tryParse(out.priority, priority)) return std::nullopt;
  if (out.priority < 0) return std::nullopt;
