...
class DiskEncryptedTransaction : public IDiskTransaction {
 public:
  static String wrappedPath(const String disk_path, const String& path) {
    ...
    if (!disk_path.empty() && path.starts_with(disk_path)) return path;
    return disk_path + path;
  }

  DiskEncryptedTransaction(DiskTransactionPtr delegate_transaction_, const std::string& disk_path_, DiskEncryptedSettings current_settings_,
                           IDisk* delegate_disk_)
    ...
    LOG_DEBUG(getLogger("DiskEncryptedTransaction"), "Creating DiskEncryptedTransaction for delegating disk {} at path {}",
              delegate_disk->getName(), disk_path);
  }

  void commit() override {
    delegate_transaction->commit();
    LOG_DEBUG(getLogger("DiskEncryptedTransaction"), "Commit DiskEncryptedTransaction for delegating disk {} at path {}",
              delegate_disk->getName(), disk_path);
  }

  void undo() noexcept override { delegate_transaction->undo(); }
  ...
  void removeSharedFiles(const RemoveBatchRequest& files, bool keep_all_batch_data,
    ...
    for (auto& file : wrapped_path_files) file.path = wrappedPath(file.path);
  ...
  std::unique_ptr<WriteBufferFromFileBase> writeEncryptedFile(const String& path, size_t buf_size, WriteMode mode,
 ...
 private:
  std::unique_ptr<WriteBufferFromFileBase> writeFileImpl(bool autocommit, const std::string& path, size_t buf_size, WriteMode mode,
                                                         const WriteSettings& settings);

  String wrappedPath(const String& path) const { return wrappedPath(disk_path, path); }
