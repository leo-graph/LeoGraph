...
struct ReservationConstraints {
  ...
  ReservationConstraints(UInt64 min_bytes_, Float32 min_ratio_) : min_bytes(min_bytes_), min_ratio(min_ratio_) {}
};
...
class IDisk : public Space {
 public:
  ...
  virtual void renameExchange(const std::string &, const std::string &) {
    throw Exception(ErrorCodes::NOT_IMPLEMENTED, "Method `renameExchange()` not implemented for disk: {}",
                    getDataSourceDescription().toString());
  }

  virtual bool renameExchangeIfSupported(const std::string &, const std::string &) {
    throw Exception(ErrorCodes::NOT_IMPLEMENTED, "Method `renameExchangeIfSupported()` not implemented for disk: {}",
                    getDataSourceDescription().toString());
  }
  ...
  virtual std::unique_ptr<ReadBufferFromFileBase> readFile(  /// NOLINT
      const String &path, const ReadSettings &settings, std::optional<size_t> read_hint = {}) const = 0;
  ...
  virtual std::unique_ptr<WriteBufferFromFileBase> writeFile(  /// NOLINT
      const String &path, size_t buf_size = DBMS_DEFAULT_BUFFER_SIZE, WriteMode mode = WriteMode::Rewrite,
  ...
  virtual void removeDirectoryIfExists(const String &) {
    throw Exception(ErrorCodes::NOT_IMPLEMENTED, "Method `removeDirectoryIfExists()` is not implemented for disk: {}",
                    getDataSourceDescription().toString());
  }
  ...
  virtual void removeSharedRecursive(const String &path, bool /* keep_all_shared_data */,
  ...
  using WriteBlobFunction =
      std::function<size_t(const Strings &blob_path, WriteMode mode, const std::optional<ObjectAttributes> &object_attributes)>;
  ...
  virtual std::unique_ptr<WriteBufferFromFileBase> writeEncryptedFile(const String &path, size_t buf_size, WriteMode mode,
  ...
  virtual NameSet getCacheLayersNames() const {
    throw Exception(ErrorCodes::NOT_IMPLEMENTED, "Method `getCacheLayersNames()` is not implemented for disk: {}",
                    getDataSourceDescription().toString());
  }
  ...
  virtual StoredObjects getStorageObjects(const String &) const {
    throw Exception(ErrorCodes::NOT_IMPLEMENTED, "Method `getStorageObjects()` not implemented for disk: {}",
                    getDataSourceDescription().toString());
  }

  virtual std::optional<StoredObjects> getStorageObjectsIfExist(const String &path) const {
    if (existsFile(path)) return getStorageObjects(path);
    return std::nullopt;
  }
  ...
  struct LocalPathWithObjectStoragePaths {
    ...
    LocalPathWithObjectStoragePaths(const std::string &local_path_, StoredObjects &&objects_)
        : local_path(local_path_), objects(std::move(objects_)) {}
  };
  ...
  virtual bool isSymlink(const String &) const {
    throw Exception(ErrorCodes::NOT_IMPLEMENTED, "Method isSymlink is not implemented for disk type: {}",
                    getDataSourceDescription().toString());
  }

  virtual bool isSymlinkNoThrow(const String &) const {
    throw Exception(ErrorCodes::NOT_IMPLEMENTED, "Method isSymlinkNothrow is not implemented for disk type: {}",
                    getDataSourceDescription().toString());
  }

  virtual void createDirectorySymlink(const String &, const String &) {
    throw Exception(ErrorCodes::NOT_IMPLEMENTED, "Method createDirectorySymlink is not implemented for disk type: {}",
                    getDataSourceDescription().toString());
  }

  virtual String readSymlink(const fs::path &) const {
    throw Exception(ErrorCodes::NOT_IMPLEMENTED, "Method readSymlink is not implemented for disk type: {}",
                    getDataSourceDescription().toString());
  }

  virtual bool equivalent(const String &, const String &) const {
    throw Exception(ErrorCodes::NOT_IMPLEMENTED, "Method equivalent is not implemented for disk type: {}",
                    getDataSourceDescription().toString());
  }
  ...
  virtual void applyNewSettings(const Poco::Util::AbstractConfiguration &config, ContextPtr context, const String &config_prefix,
                                const DisksMap &map);
  ...
  virtual MetadataStoragePtr getMetadataStorage() {
    throw Exception(ErrorCodes::NOT_IMPLEMENTED, "Method getMetadataStorage is not implemented for disk type: {}",
                    getDataSourceDescription().toString());
  }
  ...
  virtual ObjectStoragePtr getObjectStorage() {
    throw Exception(ErrorCodes::NOT_IMPLEMENTED, "Method getObjectStorage is not implemented for disk type: {}",
                    getDataSourceDescription().toString());
  }
  ...
  virtual std::shared_ptr<const S3::Client> getS3StorageClient() const {
    throw Exception(ErrorCodes::NOT_IMPLEMENTED, "Method getS3StorageClient is not implemented for disk type: {}",
                    getDataSourceDescription().toString());
  }
 ...
 protected:
  ...
  void copyThroughBuffers(const String &from_path, const std::shared_ptr<IDisk> &to_disk, const String &to_path,
                          const ReadSettings &read_settings, WriteSettings write_settings, const std::function<void()> &cancellation_hook);
...
inline String fullPath(const DiskPtr &disk, const String &path) { return fs::path(disk->getPath()) / path; }

/// Return parent path for the specified path.
inline String parentPath(const String &path) {
  if (path == "/") return "/";
  if (path.ends_with('/')) return fs::path(path).parent_path().parent_path() / "";
  return fs::path(path).parent_path() / "";
}

/// Return file name for the specified path.
inline String fileName(const String &path) { return fs::path(path).filename(); }

/// Return directory path for the specified path.
inline String directoryPath(const String &path) { return fs::path(path).parent_path() / ""; }
