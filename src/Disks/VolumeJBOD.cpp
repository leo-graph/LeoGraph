...
VolumeJBOD::VolumeJBOD(String name_, const Poco::Util::AbstractConfiguration& config, const String& config_prefix,
                       DiskSelectorPtr disk_selector)
    : IVolume(name_, config, config_prefix, disk_selector), disks_by_size(disks.begin(), disks.end()) {
  ...
  if (has_max_bytes) {
    ...
    if (ratio < 0) throw Exception(ErrorCodes::EXCESSIVE_ELEMENT_IN_CONFIG, "'max_data_part_size_ratio' have to be not less then 0.");
  ...
  if (max_data_part_size != 0 && max_data_part_size < MIN_PART_SIZE) {
    LOG_WARNING(logger, "Volume {} max_data_part_size is too low ({} < {})", backQuote(name), ReadableSize(max_data_part_size),
                ReadableSize(MIN_PART_SIZE));
  }
...
VolumeJBOD::VolumeJBOD(const VolumeJBOD& volume_jbod, const Poco::Util::AbstractConfiguration& config, const String& config_prefix,
                       DiskSelectorPtr disk_selector)
...
ReservationPtr VolumeJBOD::reserve(UInt64 bytes) { return reserveImpl(bytes, std::nullopt); }

ReservationPtr VolumeJBOD::reserve(UInt64 bytes, const ReservationConstraints& constraints) { return reserveImpl(bytes, constraints); }
...
ReservationPtr VolumeJBOD::reserveImpl(UInt64 bytes, const std::optional<ReservationConstraints>& constraints) {
  ...
  if (max_data_part_size != 0 && bytes > max_data_part_size) return {};

  auto try_reserve = [&bytes, &constraints](const DiskPtr& disk) -> ReservationPtr {
    if (disk->isReadOnly()) return {};

    return constraints.has_value() ? disk->reserve(bytes, *constraints) : disk->reserve(bytes);
  };
...
bool VolumeJBOD::areMergesAvoided() const {
  ...
  if (are_merges_avoided_user_override_value) return *are_merges_avoided_user_override_value;
  return are_merges_avoided;
}

void VolumeJBOD::setAvoidMergesUserOverride(bool avoid) { are_merges_avoided_user_override.store(avoid, std::memory_order_release); }
