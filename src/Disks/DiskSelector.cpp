...
MetadataStoragePtr unwrapEncryptedAndCacheLayers(DiskPtr disk) {
  if (disk->getDataSourceDescription().is_encrypted) return unwrapEncryptedAndCacheLayers(disk->getDelegateDiskIfExists());
...
void DiskSelector::assertInitialized() const {
  if (!is_initialized) throw Exception(ErrorCodes::LOGICAL_ERROR, "DiskSelector not initialized");
}
...
void DiskSelector::recordDisk(const std::string &disk_name, DiskPtr disk) {
  if (disk->isPlain() && !disk->isReadOnly() && !disk->isWriteOnce()) {
    for (const auto &[saved_disk_name, saved_disk] : disks) {
      if (!saved_disk->isPlain() || saved_disk->isReadOnly() || saved_disk->isWriteOnce()) continue;

      /// Same endpoint
      if (disk->getObjectStorage()->getDescription() != saved_disk->getObjectStorage()->getDescription()) continue;

      /// Same bucket
      if (disk->getObjectStorage()->getObjectsNamespace() != saved_disk->getObjectStorage()->getObjectsNamespace()) continue;

      LOG_TEST(getLogger("recordDisk"), "Validating plain disk: {}-{}-{}-{}-{} vs {}-{}-{}-{}-{}", disk_name, disk->isReadOnly(),
               disk->isWriteOnce(), disk->getObjectStorage()->getDescription(), disk->getObjectStorage()->getObjectsNamespace(),
               saved_disk_name, saved_disk->isReadOnly(), saved_disk->isWriteOnce(), saved_disk->getObjectStorage()->getDescription(),
               saved_disk->getObjectStorage()->getObjectsNamespace());
  ...
  if (!inserted) throw Exception(ErrorCodes::LOGICAL_ERROR, "Disk with name `{}` is already in disks map", disk_name);
}

void DiskSelector::initialize(const Poco::Util::AbstractConfiguration &config, const String &config_prefix, ContextPtr context,
                              DiskValidator disk_validator) try {
  ...
  for (const auto &disk_name : keys) {
    ...
    if (disk_name == DEFAULT_DISK_NAME) has_default_disk = true;

    if (disk_name == LOCAL_DISK_NAME) has_local_disk = true;

    const auto disk_config_prefix = config_prefix + "." + disk_name;

    if (disk_validator && !disk_validator(config, disk_config_prefix, disk_name)) continue;
    auto created_disk =
  ...
  for (const auto &[name, disk] : disks) disk->shutdown();
  throw;
}

DiskSelectorPtr DiskSelector::updateFromConfig(const Poco::Util::AbstractConfiguration &config, const String &config_prefix,
  ...
  for (const auto &disk_name : keys) {
    ...
    if (!result->getDisksMap().contains(disk_name)) {
      auto created_disk = factory.create(disk_name, config, disk_config_prefix, context, result->getDisksMap(), /*attach*/ false,
                                         /*custom_disk*/ false, skip_types);
      if (created_disk) {
  ...
  if (!old_disks_minus_new_disks.empty()) {
    ...
    for (const auto &[name, disk] : old_disks_minus_new_disks) {
      ...
      if (disk->isCustomDisk()) continue;

      if (num_disks_removed_from_config++ > 0) writeString(", ", warning);

      writeBackQuotedString(name, warning);
    }

    if (num_disks_removed_from_config > 0) {
      LOG_WARNING(getLogger("DiskSelector"), "{} disappeared from configuration, this change will be applied after restart of ClickHouse",
                  warning.str());
    }
  }
...
DiskPtr DiskSelector::tryGet(const String &name) const {
  ...
  if (it == disks.end()) return nullptr;
  return it->second;
}

DiskPtr DiskSelector::get(const String &name) const {
  auto disk = tryGet(name);
  if (!disk) throw Exception(ErrorCodes::UNKNOWN_DISK, "Unknown disk {}", name);
  return disk;
}
...
void DiskSelector::shutdown() {
  assertInitialized();
  for (auto &e : disks) e.second->shutdown();
}
