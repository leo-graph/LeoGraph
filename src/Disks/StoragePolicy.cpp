...
StoragePolicy::StoragePolicy(String name_, const Poco::Util::AbstractConfiguration& config, const String& config_prefix,
                             DiskSelectorPtr disks)
    : name(std::move(name_)), log(getLogger("StoragePolicy (" + name + ")")) {
  ...
  for (const auto& attr_name : keys) {
    if (!std::all_of(attr_name.begin(), attr_name.end(), isWordCharASCII))
      throw Exception(ErrorCodes::EXCESSIVE_ELEMENT_IN_CONFIG,
                      "Volume name can contain only alphanumeric and '_' in storage policy {} ({})", backQuote(name), attr_name);
    ...
    if (last_priority != std::numeric_limits<UInt64>::max() && !volume_priorities.insert(last_priority).second) {
      throw Exception(ErrorCodes::INVALID_CONFIG_PARAMETER, "volume_priority values must be unique across the policy");
    }
  }
  ...
  if (volumes.empty() && name == DEFAULT_STORAGE_POLICY_NAME) {
    auto default_volume = std::make_shared<VolumeJBOD>(DEFAULT_VOLUME_NAME, std::vector<DiskPtr>{disks->get(DEFAULT_DISK_NAME)},
                                                       /* max_data_part_size_= */ 0,
                                                       /* are_merges_avoided_= */ false,
                                                       /* perform_ttl_move_on_insert_= */ true, VolumeLoadBalancing::ROUND_ROBIN,
  ...
  if (move_factor > 1)
    throw Exception(ErrorCodes::LOGICAL_ERROR, "Disk move factor have to be in [0., 1.] interval, but set to {} in storage policy {}",
...
StoragePolicy::StoragePolicy(String name_, Volumes volumes_, double move_factor_)
    : volumes(std::move(volumes_)), name(std::move(name_)), move_factor(move_factor_), log(getLogger("StoragePolicy (" + name + ")")) {
  if (volumes.empty())
    throw Exception(ErrorCodes::NO_ELEMENTS_IN_CONFIG, "Storage policy {} must contain at least one Volume.", backQuote(name));
...
StoragePolicy::StoragePolicy(StoragePolicyPtr storage_policy, const Poco::Util::AbstractConfiguration& config, const String& config_prefix,
                             DiskSelectorPtr disks)
  ...
  for (auto& volume : volumes) {
    if (storage_policy->containsVolume(volume->getName())) {
      ...
      try {
        ...
        if (e.code() != ErrorCodes::NO_ELEMENTS_IN_CONFIG || storage_policy->getName() != DEFAULT_STORAGE_POLICY_NAME) throw;
...
bool StoragePolicy::isDefaultPolicy() const {
  ...
  if (getName() != DEFAULT_STORAGE_POLICY_NAME) return false;

  if (volumes.size() != 1) return false;

  if (volumes[0]->getName() != DEFAULT_VOLUME_NAME) return false;

  const auto& disks = volumes[0]->getDisks();
  if (disks.size() != 1) return false;

  if (disks[0]->getName() != DEFAULT_DISK_NAME) return false;
...
Disks StoragePolicy::getDisks() const {
  ...
  for (const auto& volume : volumes)
    for (const auto& disk : volume->getDisks()) res.push_back(disk);
  return res;
}
...
DiskPtr StoragePolicy::getAnyDisk() const {
  ...
  if (volumes.empty()) throw Exception(ErrorCodes::LOGICAL_ERROR, "Storage policy {} has no volumes. It's a bug.", backQuote(name));

  for (const auto& volume : volumes) {
    if (volume->getDisks().empty()) throw Exception(ErrorCodes::LOGICAL_ERROR, "Volume '{}' has no disks. It's a bug", volume->getName());
    for (const auto& disk : volume->getDisks()) {
      if (!disk->isBroken()) return disk;
    }
  }
...
DiskPtr StoragePolicy::tryGetDiskByName(const String& disk_name) const {
  for (auto&& volume : volumes)
    for (auto&& disk : volume->getDisks())
      if (disk->getName() == disk_name) return disk;
  return {};
}
...
UInt64 StoragePolicy::getMaxUnreservedFreeSpace() const {
  ...
  for (const auto& volume : volumes) {
    ...
    if (!volume_unreserved_space) return -1ULL;  /// There is at least one unlimited disk.

    if (!res || *volume_unreserved_space > *res) res = volume_unreserved_space;
  }
...
ReservationPtrOrError StoragePolicy::reserve(UInt64 bytes) const { return reserve(bytes, 0); }
...
ReservationPtr StoragePolicy::makeEmptyReservationOnLargestDisk() const {
  ...
  for (const auto& volume : volumes) {
    ...
    if (found_bottomless_disk) break;
  }

  if (!max_disk)
    throw Exception(ErrorCodes::NOT_ENOUGH_SPACE,
                    "There is no space on any disk in storage policy: {}. "
                    "It's likely all disks are broken",
                    name);
...
VolumePtr StoragePolicy::getVolume(size_t index) const {
  if (index < volume_index_by_volume_name.size()) return volumes[index];
...
VolumePtr StoragePolicy::tryGetVolumeByName(const String& volume_name) const {
  ...
  if (it == volume_index_by_volume_name.end()) return nullptr;
...
void StoragePolicy::checkCompatibleWith(const StoragePolicyPtr& new_storage_policy) const {
  ...
  if (!skip_volume_check) {
    ...
    for (const auto& volume : new_storage_policy->getVolumes()) new_volume_names.insert(volume->getName());

    for (const auto& volume : getVolumes()) {
      if (!new_volume_names.contains(volume->getName()))
        throw Exception(ErrorCodes::BAD_ARGUMENTS, "New storage policy {} shall contain volumes of the old storage policy {}",
                        backQuote(new_storage_policy->getName()), backQuote(name));

      std::unordered_set<String> new_disk_names;
      for (const auto& disk : new_storage_policy->getVolumeByName(volume->getName())->getDisks()) new_disk_names.insert(disk->getName());

      for (const auto& disk : volume->getDisks())
        if (!new_disk_names.contains(disk->getName()))
          throw Exception(ErrorCodes::BAD_ARGUMENTS, "New storage policy {} shall contain disks of the old storage policy {}",
                          backQuote(new_storage_policy->getName()), backQuote(name));
    }
  } else {
    std::unordered_set<String> new_disk_names;
    for (const auto& disk : new_storage_policy->getDisks()) new_disk_names.insert(disk->getName());
...
bool StoragePolicy::isCompatibleForPartitionOps(const StoragePolicyPtr& other) const {
  ...
  constexpr auto is_compatible = [](const auto& disk) -> bool {
    if (disk->isPlain()) return true;
    if (auto delegate = disk->getDelegateDiskIfExists()) return delegate->isPlain();
    return false;
  };
...
std::optional<size_t> StoragePolicy::tryGetVolumeIndexByDiskName(const String& disk_name) const {
  ...
  if (it != volume_index_by_disk_name.end()) return it->second;
  return {};
}
  ...
  for (size_t index = 0; index < volumes.size(); ++index) {
    ...
    if (volume_index_by_volume_name.contains(volume->getName()))
      throw Exception(ErrorCodes::EXCESSIVE_ELEMENT_IN_CONFIG,
                      ...
                      "is duplicated)",
                      backQuote(name), backQuote(volume->getName()));
    ...
    for (const auto& disk : volume->getDisks()) {
      ...
      if (volume_index_by_disk_name.contains(disk_name))
        throw Exception(ErrorCodes::EXCESSIVE_ELEMENT_IN_CONFIG,
                        ...
                        "is duplicated)",
                        backQuote(name), backQuote(disk_name));
...
bool StoragePolicy::hasAnyVolumeWithDisabledMerges() const {
  for (const auto& volume : volumes)
    if (volume->areMergesAvoided()) return true;
  return false;
}

bool StoragePolicy::containsVolume(const String& volume_name) const { return volume_index_by_volume_name.contains(volume_name); }

StoragePolicySelector::StoragePolicySelector(const Poco::Util::AbstractConfiguration& config, const String& config_prefix,
                                             DiskSelectorPtr disks) {
  Poco::Util::AbstractConfiguration::Keys keys;
  config.keys(config_prefix, keys);
  ...
  for (const auto& name : keys) {
    if (!std::all_of(name.begin(), name.end(), isWordCharASCII))
      throw Exception(ErrorCodes::EXCESSIVE_ELEMENT_IN_CONFIG, "Storage policy name can contain only alphanumeric and '_' ({})",
                      backQuote(name));
  ...
  if (!policies.contains(DEFAULT_STORAGE_POLICY_NAME)) {
    auto default_policy =
        std::make_shared<StoragePolicy>(DEFAULT_STORAGE_POLICY_NAME, config, config_prefix + "." + DEFAULT_STORAGE_POLICY_NAME, disks);
    policies.emplace(DEFAULT_STORAGE_POLICY_NAME, std::move(default_policy));
  }
}

StoragePolicySelectorPtr StoragePolicySelector::updateFromConfig(const Poco::Util::AbstractConfiguration& config,
                                                                 const String& config_prefix, DiskSelectorPtr disks,
                                                                 Strings& new_disks) const {
  ...
  for (const auto& [name, policy] : policies) {
    ...
    for (const auto& disk : result->policies[name]->getDisks()) disks_after_reload.insert(disk->getName());
  }

  std::set_difference(disks_after_reload.begin(), disks_after_reload.end(), disks_before_reload.begin(), disks_before_reload.end(),
...
StoragePolicyPtr StoragePolicySelector::tryGet(const String& name) const {
  ...
  if (it == policies.end()) return nullptr;
...
StoragePolicyPtr StoragePolicySelector::get(const String& name) const {
  ...
  if (!policy) throw Exception(ErrorCodes::UNKNOWN_POLICY, "Unknown storage policy {}", backQuote(name));
...
void StoragePolicySelector::add(StoragePolicyPtr storage_policy) {
  ...
  if (!inserted) throw Exception(ErrorCodes::LOGICAL_ERROR, "StoragePolicy is already present in StoragePolicySelector");
}
