...
DiskLocalCheckThread::DiskLocalCheckThread(DiskLocal* disk_, ContextPtr context_, UInt64 local_disk_check_period_ms)
    : WithContext(context_), disk(std::move(disk_)), check_period_ms(local_disk_check_period_ms), log(getLogger("DiskLocalCheckThread")) {
...
void DiskLocalCheckThread::startup() {
  ...
  LOG_INFO(log, "Disk check for disk {} started with period {}", disk->getName(),
           formatReadableTime(static_cast<double>(check_period_ms) * 1e6 /* ns */));
}

int diskStatusChange(bool old_val, bool new_val) { return static_cast<int>(new_val) - static_cast<int>(old_val); }
...
void DiskLocalCheckThread::run() {
  ...
  if (can_read) {
    if (disk->broken) LOG_INFO(log, "Disk {0} seems to be fine. It can be recovered using `SYSTEM RESTART DISK {0}`", disk->getName());
    retry = 0;
    if (can_write)
      disk->readonly = false;
    else {
