...
void getKeysFromConfig(const Poco::Util::AbstractConfiguration &config, const String &config_prefix, std::map<UInt64, Key> &out_keys_by_id,
                       std::vector<Key> &out_keys_without_id) {
  Strings config_keys;
  ...
  for (const std::string &config_key : config_keys) {
    ...
    if ((config_key == "key") || config_key.starts_with("key[")) {
      ...
      if (config.has(key_id_path)) key_id = config.getUInt64(key_id_path);
...
#  if CLICKHOUSE_CLOUD
    else if ((config_key == "key_aws") || config_key.starts_with("key_aws[") || (config_key == "key_gcp") ||
             config_key.starts_with("key_gcp[") || (config_key == "key_azure") || config_key.starts_with("key_azure[")) {
      String key_path = config_prefix + "." + config_key;
      String key_id_path = key_path + "[@id]";
      if (config.has(key_id_path)) key_id = config.getUInt64(key_id_path);
...
#  endif
  ...
  if (out_keys_by_id.empty() && out_keys_without_id.empty()) throw Exception(ErrorCodes::BAD_ARGUMENTS, "No encryption keys found");
...
String getCurrentKeyFromConfig(const Poco::Util::AbstractConfiguration &config, const String &config_prefix,
  ...
  auto check_current_key_found = [&](const String &current_key_) {
    for (const auto &[_, key] : keys_by_id) {
      if (key.plain == current_key_) return current_key_;

      if (std::string_view(key.encrypted.value_or("")) == std::string_view(current_key_)) return key.plain;
    }
  ...
  if (config.has(key_id_path)) {
    ...
    if (it == keys_by_id.end()) throw Exception(ErrorCodes::BAD_ARGUMENTS, "Not found a key with the current ID {}", current_key_id);
...
Algorithm getCurrentAlgorithmFromConfig(const Poco::Util::AbstractConfiguration &config, const String &config_prefix) {
  ...
  if (!config.has(path)) return DEFAULT_ENCRYPTION_ALGORITHM;
...
void getDiskAndPathFromConfig(const Poco::Util::AbstractConfiguration &config, const String &config_prefix, const DisksMap &map,
  ...
  if (disk_name.empty())
    throw Exception(ErrorCodes::BAD_ARGUMENTS, "Name of the wrapped disk must not be empty. Encrypted disk is a wrapper over another disk");

  auto disk_it = map.find(disk_name);
  if (disk_it == map.end())
    throw Exception(ErrorCodes::BAD_ARGUMENTS, "The wrapped disk must have been announced earlier. No disk with name {}", disk_name);
...
std::unique_ptr<const DiskEncryptedSettings> parseDiskEncryptedSettings(const String &disk_name,
                                                                        const Poco::Util::AbstractConfiguration &config,
                                                                        const String &config_prefix, const DisksMap &disk_map) {
  try {
...
DiskEncrypted::DiskEncrypted(const String &name_, const Poco::Util::AbstractConfiguration &config_, const String &config_prefix_,
                             const DisksMap &map_)
...
void DiskEncrypted::copyFile(const String &from_file_path, IDisk &to_disk, const String &to_file_path, const ReadSettings &read_settings,
...
std::unique_ptr<ReadBufferFromFileBase> DiskEncrypted::readFile(const String &path, const ReadSettings &settings,
                                                                std::optional<size_t> read_hint) const {
  if (read_hint && *read_hint > 0) read_hint = *read_hint + FileEncryption::Header::kSize;
...
size_t DiskEncrypted::getEncryptedFileSize(size_t unencrypted_size) const {
  if (unencrypted_size) return unencrypted_size + FileEncryption::Header::kSize;
  return 0;
}
...
std::unordered_map<String, String> DiskEncrypted::getSerializedMetadata(const std::vector<String> &paths) const {
  ...
  for (const auto &path : paths) wrapped_paths.emplace_back(wrappedPath(path));
  auto metadata = delegate->getSerializedMetadata(wrapped_paths);
  std::unordered_map<String, String> res;
  for (size_t i = 0; i != paths.size(); ++i) res.emplace(paths[i], metadata.at(wrapped_paths.at(i)));
  return res;
}

void DiskEncrypted::applyNewSettings(const Poco::Util::AbstractConfiguration &config, ContextPtr context, const String &config_prefix,
...
void registerDiskEncrypted(DiskFactory &factory, bool global_skip_access_check) {
  auto creator = [global_skip_access_check](const String &name, const Poco::Util::AbstractConfiguration &config,
                                            const String &config_prefix, ContextPtr, const DisksMap &map, bool, bool) -> DiskPtr {
