...
CachedOnDiskReadBufferFromFile::ReadInfo::ReadInfo(const FileCacheKey &cache_key_, const std::string &source_file_path_,
                                                   ImplementationBufferCreator impl_creator_, bool use_external_buffer_,
                                                   const ReadSettings &read_settings_, size_t read_until_position_)
...
CachedOnDiskReadBufferFromFile::CachedOnDiskReadBufferFromFile(const String &source_file_path_, const FileCache::Key &cache_key_,
                                                               FileCachePtr cache_, const FileCacheOriginInfo &origin_,
                                                               ImplementationBufferCreator implementation_buffer_creator_,
                                                               const ReadSettings &settings_, const String &query_id_, size_t file_size_,
                                                               bool allow_seeks_after_first_read_, bool use_external_buffer_,
    ...
    : ReadBufferFromFileBase(
      ...
      info(cache_key_, source_file_path_, implementation_buffer_creator_, use_external_buffer_, settings_,
           read_until_position_.value_or(file_size_)) {
  LOG_TEST(log,
           "Cache key: {}, source file path: {}, boundary alignment: {}, "
           "external buffer: {}, allow seeks after first read: {}, file size: {}",
           cache_key_.toString(), source_file_path_,
           settings_.filesystem_cache_boundary_alignment.has_value() ? DB::toString(settings_.filesystem_cache_boundary_alignment.value())
                                                                     : "None",
...
std::optional<size_t> CachedOnDiskReadBufferFromFile::tryGetFileSize() {
  if (file_size.has_value()) return file_size;
...
size_t CachedOnDiskReadBufferFromFile::getFileSize() {
  ...
  if (!object_size.has_value()) throw Exception(ErrorCodes::UNKNOWN_FILE_SIZE, "Cannot get file size for object {}", info.source_file_path);
  return object_size.value();
}

void CachedOnDiskReadBufferFromFile::appendFilesystemCacheLog(const FileSegment &file_segment,
                                                              CachedOnDiskReadBufferFromFile::ReadType type) {
  if (!cache_log) return;
...
bool CachedOnDiskReadBufferFromFile::nextFileSegmentsBatch() {
  ...
  if (info.settings.read_from_filesystem_cache_if_exists_otherwise_bypass_cache) {
    info.file_segments =
        cache->get(info.cache_key, file_offset_of_buffer_end, size, info.settings.filesystem_cache_segments_batch_size, origin.user_id);
  } else {
    const auto object_size = getFileSize();
    CreateFileSegmentSettings create_settings(FileSegmentKind::Regular);

    info.file_segments =
        cache->getOrSet(info.cache_key, file_offset_of_buffer_end, size, object_size, create_settings,
                        info.settings.filesystem_cache_segments_batch_size, origin, info.settings.filesystem_cache_boundary_alignment);
  }
...
void CachedOnDiskReadBufferFromFile::initialize() {
  if (initialized) throw Exception(ErrorCodes::LOGICAL_ERROR, "Caching buffer already initialized");
   ...
   */
  if (!nextFileSegmentsBatch()) throw Exception(ErrorCodes::LOGICAL_ERROR, "List of file segments cannot be empty");

  chassert(!info.file_segments->empty());

  LOG_TEST(log, "Having {} file segments to read: {}, current read range: [{}, {})", info.file_segments->size(),
           info.file_segments->toString(), file_offset_of_buffer_end, info.read_until_position);
...
std::shared_ptr<ReadBufferFromFileBase> getCacheReadBuffer(const FileSegment &file_segment, ReadInfo &info) {
  ...
  if (info.cache_file_reader) {
    ...
    if (info.cache_file_reader->getFileName() == path) return info.cache_file_reader;
  ...
  if (info.use_external_buffer) local_read_settings.local_fs_buffer_size = 0;
...
std::shared_ptr<ReadBufferFromFileBase> getRemoteReadBuffer(FileSegment &file_segment, size_t offset, ReadType read_type, ReadInfo &info) {
  ProfileEventTimeIncrement<Microseconds> watch(ProfileEvents::CachedReadBufferCreateBufferMicroseconds);
  ...
  switch (read_type) {
    ...
    case ReadType::REMOTE_FS_READ_BYPASS_CACHE: {
      if (info.remote_file_reader && offset == info.remote_file_reader->getFileOffsetOfBufferEnd()) return info.remote_file_reader;
...
String CachedOnDiskReadBufferFromFile::toString(ReadType type) { return String(magic_enum::enum_name(type)); }

CachedOnDiskReadBufferFromFile::ReadFromFileSegmentStatePtr CachedOnDiskReadBufferFromFile::createReadFromFileSegmentState(
    FileSegment &file_segment, size_t offset, ReadInfo &info_, LoggerPtr log) {
  ...
  if (info_.settings.read_from_filesystem_cache_if_exists_otherwise_bypass_cache) {
    if (download_state == FileSegment::State::DOWNLOADED) return create(ReadType::CACHED);

    LOG_TEST(log,
             "Bypassing cache because "
  ...
  while (true) {
    switch (download_state) {
      ...
      case FileSegment::State::PARTIALLY_DOWNLOADED_NO_CONTINUATION: {
        if (canStartFromCache(offset, file_segment)) return create(ReadType::CACHED);
...
CachedOnDiskReadBufferFromFile::ReadFromFileSegmentStatePtr CachedOnDiskReadBufferFromFile::prepareReadFromFileSegmentState(
    FileSegment &file_segment, size_t offset, ReadInfo &info, size_t file_size_, LoggerPtr log) {
  const auto range = file_segment.range();

  chassert(!file_segment.isDownloader(),
           fmt::format("!isDownloader() failed in prepareReadFromFileSegmentState: {}", getInfoForLog(nullptr, info, offset)));
  ...
  LOG_TEST(log, "Current read type: {}, read offset: {}/{}, impl file offset: {}, file segment: {}", toString(state->read_type), offset,
           file_size_, state->buf->getFileOffsetOfBufferEnd(), file_segment.getInfoForLog());
    ...
    case ReadType::CACHED: {
      ...
      if (file_size == 0 || range.left + file_size <= offset)
        throw Exception(ErrorCodes::LOGICAL_ERROR,
                        ...
                        file_size, range.left, range.left + file_size, offset);
#endif

      size_t seek_offset = offset - range.left;

      if (offset < range.left)
        throw Exception(ErrorCodes::LOGICAL_ERROR, "Invariant failed. Expected {} > {} (current offset > file segment's start offset)",
                        offset, range.left);
    ...
    case ReadType::REMOTE_FS_READ_AND_PUT_IN_CACHE: {
      ...
      if (current_write_offset != static_cast<size_t>(state->buf->getPosition())) {
        throw Exception(ErrorCodes::LOGICAL_ERROR,
                        ...
                        offset, current_write_offset, state->buf->getPosition(), state->buf->getFileOffsetOfBufferEnd(),
...
bool CachedOnDiskReadBufferFromFile::completeFileSegmentAndGetNext() {
  ...
  if (cache_log) appendFilesystemCacheLog(*current_file_segment, state->read_type);
  ...
  if (info.file_segments->empty() && !nextFileSegmentsBatch()) return false;
  ...
  LOG_TEST(log, "New segment range: {}, old range: {}", current_file_segment->range().toString(), completed_range.toString());
...
bool CachedOnDiskReadBufferFromFile::predownloadForFileSegment(FileSegment &file_segment, size_t offset, ReadFromFileSegmentState &state,
                                                               ReadInfo &info, LoggerPtr log) {
  ...
  if (state.bytes_to_predownload) {
    ...
    LOG_TEST(log, "Bytes to predownload: {}, caller_id: {}, buffer size: {}", state.bytes_to_predownload, FileSegment::getCallerId(),
    ...
    SCOPE_EXIT({ state.buf->set(initial_buffer, initial_buffer_size); });
    ...
    while (true) {
      ...
      if (!state.bytes_to_predownload || !has_more_data) {
        if (state.bytes_to_predownload)
          throw Exception(ErrorCodes::LOGICAL_ERROR,
                          ...
                          state.bytes_to_predownload, file_segment.range().toString(), file_segment.getCurrentWriteOffset(), offset,
                          state.buf->eof(), state.buf->internalBuffer().size());

        chassert(!state.buf->hasPendingData());
        auto result = state.buf->hasPendingData();
        if (result) {
          const auto write_offset = file_segment.getCurrentWriteOffset();
          if (write_offset != static_cast<size_t>(state.buf->getPosition()) || write_offset != offset) {
            throw Exception(ErrorCodes::LOGICAL_ERROR,
                            "Buffer's offsets mismatch after predownloading; download offset: {}, "
                            "cached buffer offset: {}, implementation buffer offset: {}, "
                            "file segment info: {}",
                            write_offset, offset, state.buf->getPosition(), file_segment.getInfoForLog());
          }
        }
      ...
      bool continue_predownload =
          file_segment.reserve(size, info.settings.filesystem_cache_reserve_space_wait_lock_timeout_milliseconds, failure_reason);
      ...
      if (continue_predownload) {
        ...
        continue_predownload = writeCache(state.buf->buffer().begin(), size, current_write_offset, file_segment, log);
...
void CachedOnDiskReadBufferFromFile::updateReadStateIfNeeded(FileSegment &file_segment, size_t offset, ReadFromFileSegmentStatePtr &state,
                                                             ReadInfo &info, size_t file_size_, LoggerPtr log) {
...
bool CachedOnDiskReadBufferFromFile::updateImplementationBufferIfNeeded() {
  ...
  if (file_offset_of_buffer_end > current_read_range.right) return completeFileSegmentAndGetNext();

  updateReadStateIfNeeded(file_segment, file_offset_of_buffer_end, state, info, getFileSize(), log);
  return true;
}

bool CachedOnDiskReadBufferFromFile::writeCache(char *data, size_t size, size_t offset, FileSegment &file_segment, LoggerPtr log) {
  Stopwatch watch(CLOCK_MONOTONIC);
...
bool CachedOnDiskReadBufferFromFile::nextImplStep() {
  ...
  if (file_offset_of_buffer_end == info.read_until_position) return false;
  ...
  if (!original_buffer_size) {
    throw Exception(ErrorCodes::LOGICAL_ERROR, "Internal buffer cannot be empty (use external buffer: {}; {})", use_external_buffer,
                    getInfoForLog());
  }
  ...
  SCOPE_EXIT({
    try {
      ...
      if (std::uncaught_exceptions() > 0) nextimpl_step_log_info = getInfoForLog();

      chassert(!internal_buffer.empty());
      chassert(internal_buffer.begin());

      if (info.file_segments->empty()) return;

      auto &file_segment = info.file_segments->front();

      if (file_segment.isDownloader()) {
        if (!implementation_buffer_can_be_reused) file_segment.resetRemoteFileReader();

        file_segment.completePartAndResetDownloader();
      }

      if (use_external_buffer && !internal_buffer.empty()) internal_buffer.resize(original_buffer_size);
  ...
  if (state && state->buf) {
    ...
    if (!can_read_further) return false;
  } else {
  ...
  if (use_external_buffer && state->read_type == ReadType::CACHED) {
    /// We allocate buffers not less than 1M so that s3 requests will not be too small. But the same buffers (members of
    /// AsynchronousReadIndirectBufferFromRemoteFS) are used for reading from files. Some of these readings are fairly small and their
    /// performance degrade when we use big buffers (up to ~20% for queries like Q23 from ClickBench).
  ...
  size_t size = 0;
  {
    SCOPE_EXIT({ state->buf->set(nullptr, 0); });

    size =
        readFromFileSegment(file_segment, file_offset_of_buffer_end, getFileSize(), *state, info, implementation_buffer_can_be_reused, log);
  ...
  if (size) file_offset_of_buffer_end += size;

  working_buffer = Buffer(internal_buffer.begin(), internal_buffer.begin() + size);

  if (file_segment.isDownloader()) file_segment.completePartAndResetDownloader();

  chassert(!file_segment.isDownloader(), "!isDownloader() failed in the end of nextImpl: " + getInfoForLog());

  if (file_offset_of_buffer_end > current_read_range.right) completeFileSegmentAndGetNext();

  return size;
}

size_t CachedOnDiskReadBufferFromFile::readFromFileSegment(FileSegment &file_segment, size_t offset, size_t file_size_,
                                                           ReadFromFileSegmentState &state, ReadInfo &info,
                                                           bool &implementation_buffer_can_be_reused, LoggerPtr log) {
  ...
  if (state.read_type != ReadType::CACHED) chassert(state.buf->getFileOffsetOfBufferEnd() == offset);
#endif

  auto do_download = state.read_type == ReadType::REMOTE_FS_READ_AND_PUT_IN_CACHE;
  if (do_download != file_segment.isDownloader()) {
    throw Exception(ErrorCodes::LOGICAL_ERROR, "Incorrect segment state. Having read type: {}, file segment info: {}",
  ...
  if (!size) {
    ...
    if (state.read_type == ReadType::CACHED) {
      ...
      if (cache_file_size == 0) {
        throw Exception(ErrorCodes::LOGICAL_ERROR, "Attempt to read from an empty cache file: {} (just before actual read)",
                        cache_file_size);
      }
    } else {
    ...
    if (state.buf->internalBuffer().empty()) {
      throw Exception(ErrorCodes::LOGICAL_ERROR, "Internal buffer cannot be empty (use external buffer: {}; {})", info.use_external_buffer,
  ...
  if (size) {
    ...
    if (do_download) {
      chassert(offset + size - 1 <= file_segment.range().right,
               fmt::format("Offset: {}, size: {}, file segment range: {}, impl offset: {}", offset, size, file_segment.range().toString(),
      ...
      if (success) {
        ...
        LOG_TRACE(log,
                  "No space left in cache to reserve {} bytes, reason: {}, "
                  "will continue without cache download",
                  size, failure_reason);
    ...
    if (info.file_segments->size() == 1) {
      ...
      LOG_TEST(log, "Remaining size to read: {}, read: {}. Resizing buffer to {}", remaining_size_to_read, size,
               state.buf->offset() + std::min(size, remaining_size_to_read));
    ...
    if (do_download && download_current_segment_succeeded) chassert(file_segment.getCurrentWriteOffset() >= offset + size);

    chassert(
        offset + size <= info.read_until_position,
        fmt::format("Expected {} <= {} (size: {}, read range: {}, hold file segments: {} ({}))", offset + size, info.read_until_position,
                    size, current_read_range.toString(), info.file_segments->size(), info.file_segments->toString(true)));
  }

  if (size) chassert(state.buf->available());
  ...
  if (size == 0 && offset + size < info.read_until_position) {
    ...
    throw Exception(ErrorCodes::LOGICAL_ERROR,
                    ...
                    offset + size, size, object_size ? std::to_string(*object_size) : "None", file_size_, info.read_until_position,
                    toString(state.read_type), impl_read_stop_reason ? *impl_read_stop_reason : "None", state.buf->buffer().size(),
                    state.buf->internalBuffer().size(), state.buf->available(), state.buf->getFileOffsetOfBufferEnd(),
                    impl_read_until_position ? std::to_string(*impl_read_until_position) : "None", state.buf->eof(), download_finished_time,
                    cache_file_size ? std::to_string(cache_file_size) : "None", cache_file_path, info.file_segments->size(),
                    file_segment.getInfoForLog());
  }

  // No necessary because of the SCOPE_EXIT above, but useful for logging below.
  LOG_TEST(log, "Read {} bytes (buffer size: {}). Read info: {}", size, state.buf->internalBuffer().size(),
           getInfoForLog(&state, info, offset + size));

  return size;
}

size_t CachedOnDiskReadBufferFromFile::readBigAt(char *to, size_t n, size_t range_begin,
                                                 const std::function<bool(size_t)> &progress_callback) const {
  ReadInfo current_info(info.cache_key, info.source_file_path, info.implementation_buffer_creator, info.use_external_buffer, info.settings,
                        /* read_until_position */ range_begin + n);

  if (info.settings.read_from_filesystem_cache_if_exists_otherwise_bypass_cache) {
    current_info.file_segments = cache->get(info.cache_key,
                                            /* offset */ range_begin,
                                            /* size */ n,
                                            /* batch_size */ 0, origin.user_id);
  } else {
    CreateFileSegmentSettings create_settings(FileSegmentKind::Regular);
    current_info.file_segments = cache->getOrSet(info.cache_key,
                                                 /* offset */ range_begin,
                                                 /* size */ n, file_size.value(), create_settings,
                                                 /* batch_size */ 0, origin);
  }

  if (current_info.file_segments->empty()) throw Exception(ErrorCodes::LOGICAL_ERROR, "No file segments");

  LOG_TEST(log, "ReadBigAt() at offset {} size {} file segments {}", range_begin, n, current_info.file_segments->size());
  ...
  SCOPE_EXIT({
    if (current_info.file_segments->empty()) return;
  ...
  while (!cancelled && read_bytes < n) {
    if (current_info.file_segments->empty()) {
      throw Exception(ErrorCodes::LOGICAL_ERROR, "Unexpected end of file segments. Offset: {}, read: {}/{}", offset, read_bytes, n);
    }
    ...
    if (current_state) {
      ...
      current_state = prepareReadFromFileSegmentState(file_segment, offset, current_info, object_size, log);
    }
    ...
    SCOPE_EXIT({ current_state->buf->set(nullptr, 0); });

    const auto size =
        readFromFileSegment(file_segment, offset, object_size, *current_state, current_info, implementation_buffer_can_be_reused, log);

    LOG_TEST(log, "ReadBigAt() read {} bytes at offset: {}. Total: {}/{}", size, offset, read_bytes + size, n);

    offset += size;
    read_bytes += size;

    chassert(read_bytes <= n);
    chassert(offset <= file_segment.range().right + 1,
             fmt::format("Offset: {}, size: {}, remaining size in file segment: {}, file segment: {}", offset, size,
                         remaining_size_in_file_segment, file_segment.getInfoForLog()));

    if (!size) {
      throw Exception(ErrorCodes::LOGICAL_ERROR, "Cannot read all data. Offset: {} (initial offset: {}), read bytes {}/{}", offset,
                      range_begin, read_bytes, n);
    }

    if (progress_callback) cancelled = progress_callback(size);
  }
...
off_t CachedOnDiskReadBufferFromFile::seek(off_t offset, int whence) {
  if (offset == getPosition() && whence == SEEK_SET) return offset;
  ...
  if (allow_seeks_after_first_read) {
    ...
    if (new_pos + (working_buffer.end() - pos) == file_offset_of_buffer_end) return new_pos;
...
size_t CachedOnDiskReadBufferFromFile::getRemainingSizeToRead() {
  ...
  if (!info.read_until_position) throw Exception(ErrorCodes::LOGICAL_ERROR, "Last position was not set");

  /// On this level should be guaranteed that read size is non-zero.
  if (file_offset_of_buffer_end > info.read_until_position)
    throw Exception(ErrorCodes::LOGICAL_ERROR, "Read boundaries mismatch. Expected {} < {}", file_offset_of_buffer_end,
...
void CachedOnDiskReadBufferFromFile::setReadUntilPosition(size_t position) {
  ...
  if (info.read_until_position == position) return;
...
void CachedOnDiskReadBufferFromFile::setReadUntilEnd() { setReadUntilPosition(getFileSize()); }

off_t CachedOnDiskReadBufferFromFile::getPosition() { return file_offset_of_buffer_end - available(); }

std::string CachedOnDiskReadBufferFromFile::getInfoForLog() { return getInfoForLog(state.get(), info, file_offset_of_buffer_end); }

std::string CachedOnDiskReadBufferFromFile::getInfoForLog(const ReadFromFileSegmentState *state, const ReadInfo &info, size_t offset) {
  ...
  if (!info.file_segments->empty()) wb << " (front: " << info.file_segments->front().getInfoForLog() << ")";

  return wb.str();
}

bool CachedOnDiskReadBufferFromFile::isSeekCheap() { return !initialized || (state && state->read_type == ReadType::CACHED); }
...
static bool isRangeContainedInSegments(size_t left, size_t right, const FileSegmentsHolderPtr &file_segments) {
  if (!FileSegment::Range{file_segments->front().range().left, file_segments->back().range().right}.contains({left, right}))
    throw Exception(ErrorCodes::LOGICAL_ERROR, "Requested range is not contained in the segments: left={}, right={}, file_segments={}",
                    left, right, file_segments->toString());
  ...
  return std::all_of(file_segments->begin(), end_of_intersection, [right](const auto &segment) {
...
bool CachedOnDiskReadBufferFromFile::isContentCached(size_t offset, size_t size) {
  ...
  if (info.file_segments->empty()) return false;
