...
WriteBufferFromAzureBlobStorage::WriteBufferFromAzureBlobStorage(AzureClientPtr blob_container_client_, const String &blob_path_,
                                                                 size_t buf_size_, const WriteSettings &write_settings_,
    ...
    : WriteBufferFromFileBase(std::min(buf_size_, static_cast<size_t>(DBMS_DEFAULT_BUFFER_SIZE)), nullptr, 0),
      ...
      max_unexpected_write_error_retries(write_settings_.is_initial_access_check ? settings_->max_unexpected_write_error_retries * 3
                                                                                 : settings_->max_unexpected_write_error_retries),
      blob_path(blob_path_),
      write_settings(write_settings_),
      blob_container_client(blob_container_client_),
      task_tracker(std::make_unique<TaskTracker>(std::move(schedule_), settings_->max_inflight_parts_for_one_file, limited_log)),
...
void WriteBufferFromAzureBlobStorage::execWithRetry(std::function<void(size_t)> func, size_t num_tries, size_t cost) {
  ...
  for (size_t i = 0; i < num_tries; ++i) {
    try {
      ResourceGuard rlock(ResourceGuard::Metrics::getIOWrite(), write_settings.io_scheduling.write_resource_link,
                          cost);  // Note that zero-cost requests are ignored
      func(i);
      rlock.unlock(cost);
      break;
      ...
      if (getCurrentExceptionCode() == ErrorCodes::CANNOT_ALLOCATE_MEMORY) throw;

      if (i == num_tries - 1) throw;
...
void WriteBufferFromAzureBlobStorage::preFinalize() {
  ...
  if (block_ids.empty()) {
    ...
    if (blob_container_client->IsClientForDisk()) ProfileEvents::increment(ProfileEvents::DiskAzureUpload);
    ...
    if (detached_part_data.size() == 1 && detached_part_data.front().data_size <= max_single_part_upload_size) {
      ...
      try {
        execWithRetry(
            [&](size_t retry_attempt) {
              ...
              block_blob_client.Upload(memory_stream, options,
                                       azure_context.WithValue(PocoAzureHTTPClient::getSDKContextKeyForBufferRetry(), retry_attempt));
            },
            max_unexpected_write_error_retries, part_data.data_size);
      } catch (const Azure::Core::RequestFailedException &e) {
        error_code = static_cast<Int32>(e.StatusCode);
        error_message = e.Message;
        if (blob_log)
          blob_log->addEvent(BlobStorageLogElement::EventType::Upload,
                             /* bucket */ container_for_logging,
                             /* remote_path */ blob_path,
                             /* local_path */ {},
                             /* data_size */ part_data.data_size, watch.elapsedMicroseconds(), error_code, error_message);
        throw;
      }
      ...
      if (blob_log)
        blob_log->addEvent(BlobStorageLogElement::EventType::Upload,
                           ...
                           /* remote_path */ blob_path,
                           /* local_path */ {},
                           /* data_size */ part_data.data_size, elapsed, error_code, error_message);
...
void WriteBufferFromAzureBlobStorage::finalizeImpl() {
  ...
  if (!block_ids.empty()) {
    ...
    if (blob_container_client->IsClientForDisk()) ProfileEvents::increment(ProfileEvents::DiskAzureCommitBlockList);
    ...
    try {
      execWithRetry(
          [&](size_t retry_attetmpt) {
            ...
            block_blob_client.CommitBlockList(
                block_ids, options, azure_context.WithValue(PocoAzureHTTPClient::getSDKContextKeyForBufferRetry(), retry_attetmpt));
          },
          max_unexpected_write_error_retries);
    } catch (const Azure::Core::RequestFailedException &e) {
      error_code = static_cast<Int32>(e.StatusCode);
      error_message = e.Message;
      if (blob_log)
        blob_log->addEvent(BlobStorageLogElement::EventType::MultiPartUploadComplete,
                           /* bucket */ container_for_logging,
                           /* remote_path */ blob_path,
                           /* local_path */ {},
                           /* data_size */ 0, watch.elapsedMicroseconds(), error_code, error_message);
    ...
    try {
      ...
      if (e.StatusCode == Azure::Core::Http::HttpStatusCode::NotFound)
        throw Exception(ErrorCodes::AZURE_BLOB_STORAGE_ERROR,
                        "Object {} not uploaded to azure blob storage, it's a bug in Azure Blob Storage or its API.", blob_path);
      throw;
    }
  }
}
...
void WriteBufferFromAzureBlobStorage::nextImpl() {
  ...
  if (detached_part_data.size() > 1) writeMultipartUpload();

  allocateBuffer();
}

void WriteBufferFromAzureBlobStorage::hidePartialData() {
  if (write_settings.remote_throttler) write_settings.remote_throttler->throttle(offset());
...
void WriteBufferFromAzureBlobStorage::writePart(WriteBufferFromAzureBlobStorage::PartData &&part_data) {
  ...
  auto upload_worker = [this, worker_data]() {
    ...
    if (blob_container_client->IsClientForDisk()) ProfileEvents::increment(ProfileEvents::DiskAzureStageBlock);
    ...
    try {
      execWithRetry(
          [&](size_t retry_attempt) {
            block_blob_client.StageBlock(data_block_id, memory_stream, Azure::Storage::Blobs::StageBlockOptions{},
                                         azure_context.WithValue(PocoAzureHTTPClient::getSDKContextKeyForBufferRetry(), retry_attempt));
          },
          max_unexpected_write_error_retries, data_size);
    } catch (const Azure::Core::RequestFailedException &e) {
      error_code = static_cast<Int32>(e.StatusCode);
      error_message = e.Message;
      if (blob_log)
        blob_log->addEvent(BlobStorageLogElement::EventType::MultiPartUploadWrite,
                           ...
                           /* remote_path */ blob_path,
                           /* local_path */ {},
                           /* data_size */ data_size, watch.elapsedMicroseconds(), error_code, error_message);
      throw;
    }
    auto elapsed = watch.elapsedMicroseconds();
    ...
    if (blob_log)
      blob_log->addEvent(BlobStorageLogElement::EventType::MultiPartUploadWrite,
                         ...
                         /* remote_path */ blob_path,
                         /* local_path */ {},
                         /* data_size */ data_size, elapsed, error_code, error_message);
  };
