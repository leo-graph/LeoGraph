...
struct AsyncReadIncrement : boost::noncopyable {
  explicit AsyncReadIncrement(std::shared_ptr<AsyncReadCounters> counters_) : counters(counters_) {
...
ThreadPoolRemoteFSReader::ThreadPoolRemoteFSReader(size_t pool_size, size_t queue_size_)
    : pool(std::make_unique<ThreadPool>(CurrentMetrics::ThreadPoolRemoteFSReaderThreads,
                                        ...
                                        CurrentMetrics::ThreadPoolRemoteFSReaderThreadsScheduled, pool_size, pool_size, queue_size_)) {}
...
std::future<IAsynchronousReader::Result> ThreadPoolRemoteFSReader::submit(Request request) {
...

  {
    ...
    if (!typeid_cast<CachedInMemoryReadBufferFromFile *>(&reader)) reader.set(request.buf, request.size);
  ...
  if (is_content_cached) {
    ...
    if (res.execution_watch)
      ProfileEvents::increment(ProfileEvents::ThreadpoolReaderSubmitReadSynchronouslyMicroseconds,
                               res.execution_watch->elapsedMicroseconds());
  ...
  return scheduleFromThreadPoolUnsafe<Result>([request, this]() -> Result { return execute(request, /*seek_performed=*/true); }, *pool,
                                              ThreadName::REMOTE_FS_READ_THREAD_POOL, request.priority);
}

IAsynchronousReader::Result ThreadPoolRemoteFSReader::execute(Request request) { return execute(request, /*seek_performed=*/false); }
...
IAsynchronousReader::Result ThreadPoolRemoteFSReader::execute(Request request, bool seek_performed) {
  ...
  if (!page_cache_reader && !request.buf) throw Exception(ErrorCodes::LOGICAL_ERROR, "Request buffer is invalid");

  if (!request.size) throw Exception(ErrorCodes::LOGICAL_ERROR, "Request buffer size cannot be zero");
...

  {
    ProfileEventTimeIncrement<Microseconds> elapsed(ProfileEvents::ThreadpoolReaderPrepareMicroseconds);
    if (!seek_performed) {
      if (!page_cache_reader) reader.set(request.buf, request.size);
      reader.seek(request.offset, SEEK_SET);
    }
...
void ThreadPoolRemoteFSReader::wait() { pool->wait(); }
