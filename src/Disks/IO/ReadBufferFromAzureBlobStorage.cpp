...
ReadBufferFromAzureBlobStorage::ReadBufferFromAzureBlobStorage(ContainerClientPtr blob_container_client_, const String &path_,
                                                               const ReadSettings &read_settings_, size_t max_single_read_retries_,
                                                               size_t max_single_download_retries_, bool use_external_buffer_,
                                                               bool restricted_seek_, size_t read_until_position_)
...
bool ReadBufferFromAzureBlobStorage::nextImpl() {
  if (read_until_position) {
    if (read_until_position == offset) return false;

    if (read_until_position < offset)
      throw Exception(ErrorCodes::LOGICAL_ERROR, "Attempt to read beyond right offset ({} > {})", offset, read_until_position - 1);
  }

  if (!initialized) initialize(/* attempt */ 0);
  ...
  for (size_t i = 0; i < max_single_read_retries; ++i) {
    try {
      ...
      if (read_settings.remote_throttler) read_settings.remote_throttler->throttle(bytes_read);
      break;
    } catch (const Azure::Core::RequestFailedException &e) {
      ProfileEvents::increment(ProfileEvents::ReadBufferFromAzureRequestsErrors);
      LOG_DEBUG(log, "Exception caught during Azure Read for file {} at attempt {}/{}: {}", path, i + 1, max_single_read_retries,
                e.Message);

      if (i + 1 == max_single_read_retries || !isRetryableAzureException(e)) throw;
      ...
      if (i + 1 == max_single_read_retries) throw;
...
off_t ReadBufferFromAzureBlobStorage::getPosition() { return offset - available(); }
...
void ReadBufferFromAzureBlobStorage::initialize(size_t attempt) {
  ...
  if (read_until_position != 0) length = {static_cast<int64_t>(read_until_position - offset)};
  ...
  if (!blob_client) blob_client = std::make_unique<Azure::Storage::Blobs::BlobClient>(blob_container_client->GetBlobClient(path));
  ...
  for (size_t i = 0; i < max_single_download_retries; ++i) {
    try {
      ...
      if (blob_container_client->IsClientForDisk()) ProfileEvents::increment(ProfileEvents::DiskAzureGetObject);
      ...
      ProfileEvents::increment(ProfileEvents::ReadBufferFromAzureRequestsErrors);
      LOG_DEBUG(log, "Exception caught during Azure Download for file {} at offset {} at attempt {}/{}: {}", path, offset, i + 1,
                max_single_download_retries, e.Message);

      if (i + 1 == max_single_download_retries || !isRetryableAzureException(e)) throw;
...
std::optional<size_t> ReadBufferFromAzureBlobStorage::tryGetFileSize() {
  ...
  if (!file_size) file_size = blob_client->GetProperties().Value.BlobSize;

  return file_size;
}

size_t ReadBufferFromAzureBlobStorage::readBigAt(char *to, size_t n, size_t range_begin,
  ...
  for (size_t i = 0; i < max_single_download_retries && n > 0; ++i) {
    ...
    try {
      ...
      Azure::Core::Context azure_context =
          Azure::Core::Context().WithValue(PocoAzureHTTPClient::getSDKContextKeyForBufferRetry(), size_t{0});
      ...
      if (read_settings.remote_throttler) read_settings.remote_throttler->throttle(bytes_copied);
    } catch (const Azure::Core::RequestFailedException &e) {
      ProfileEvents::increment(ProfileEvents::ReadBufferFromAzureRequestsErrors);
      LOG_DEBUG(log, "Exception caught during Azure Download for file {} at offset {} at attempt {}/{}: {}", path, offset, i + 1,
                max_single_download_retries, e.Message);
...
void ReadBufferFromAzureBlobStorage::setMetadataFromResponse(const Azure::Storage::Blobs::Models::DownloadBlobDetails &details,
  ...
  if (!details.Metadata.empty()) {
    for (const auto &[key, value] : details.Metadata) new_metadata.attributes[key] = value;
  }
