...
class RemoteFSFileDescriptor : public IAsynchronousReader::IFileDescriptor {
 public:
  ...
  explicit RemoteFSFileDescriptor(SeekableReadBuffer& reader_, std::shared_ptr<AsyncReadCounters> async_read_counters_)
      : reader(reader_), async_read_counters(async_read_counters_) {}
