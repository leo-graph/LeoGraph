...
ReadBufferFromRemoteFSGather::ReadBufferFromRemoteFSGather(ReadBufferCreator&& read_buffer_creator_, const StoredObjects& blobs_to_read_,
                                                           const ReadSettings& settings_, bool use_external_buffer_, size_t buffer_size)
  ...
  if (!blobs_to_read.empty()) current_object = blobs_to_read.front();
}
...
bool ReadBufferFromRemoteFSGather::nextImpl() {
  ...
  if (!current_buf) initialize();

  if (!current_buf) return false;

  if (readImpl()) return true;
...
bool ReadBufferFromRemoteFSGather::readImpl() {
  ...
  if (result) {
    ...
    chassert(blobs_to_read.size() != 1 || file_offset_of_buffer_end == current_buf->getFileOffsetOfBufferEnd(),
             fmt::format("offset: {}, buf offset: {}, available: {}, nextimpl offset: {}", file_offset_of_buffer_end,
                         current_buf->getFileOffsetOfBufferEnd(), current_buf->available(), nextimpl_working_buffer_offset));
  }
...
void ReadBufferFromRemoteFSGather::setReadUntilPosition(size_t position) {
  if (position == read_until_position) return;
  ...
  if (!use_external_buffer && position < file_offset_of_buffer_end) {
    ...
    if (available() >= file_offset_of_buffer_end - position) {
      ...
      throw Exception(ErrorCodes::LOGICAL_ERROR, "Attempt to set read until position before already read data ({} < {})", position,
                      getPosition());
    }
  }
...
off_t ReadBufferFromRemoteFSGather::seek(off_t offset, int whence) {
  ...
  if (use_external_buffer) {
    ...
    if (!working_buffer.empty() && static_cast<size_t>(offset) >= file_offset_of_buffer_end - working_buffer.size() &&
...
bool ReadBufferFromRemoteFSGather::isSeekCheap() { return !current_buf || current_buf->isSeekCheap(); }
  ...
  if (current_buf) {
    ...
    for (size_t i = 0; i < blobs_to_read.size(); ++i) {
      ...
      if (i == current_buf_idx) {
        if (offset + size <= blob.bytes_size) return current_buf->isContentCached(offset, size);
        return false;
      }
      if (offset < blob.bytes_size) return false;
