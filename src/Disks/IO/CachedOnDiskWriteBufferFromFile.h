...
class FileSegmentRangeWriter {
 public:
  FileSegmentRangeWriter(FileCache* cache_, const FileSegment::Key& key_, const FileCacheOriginInfo& origin_,
                         size_t reserve_space_lock_wait_timeout_milliseconds_, std::shared_ptr<FilesystemCacheLog> cache_log_,
                         const String& query_id_, const String& source_path_, bool is_distributed_cache_);
...
class CachedOnDiskWriteBufferFromFile final : public WriteBufferFromFileDecorator, public IFilesystemCacheWriteBuffer {
 public:
  CachedOnDiskWriteBufferFromFile(std::unique_ptr<WriteBuffer> impl_, FileCachePtr cache_, const String& source_path_,
                                  const FileCacheKey& key_, const String& query_id_, const WriteSettings& settings_,
                                  const FileCacheOriginInfo& origin_, std::shared_ptr<FilesystemCacheLog> cache_log_,
                                  bool is_distributed_cache_, FileSegmentKind file_segment_kind_ = FileSegmentKind::Regular);
