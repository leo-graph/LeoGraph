...
std::unique_ptr<ReadBufferFromFileBase> createReadBufferFromFileBase(const std::string &filename, const ReadSettings &settings,
                                                                     ...
                                                                     int flags, char *existing_memory, bool allow_userspace_page_cache) {
  if (file_size.has_value() && !*file_size) return std::make_unique<ReadBufferFromEmptyFile>();
  ...
  if (!existing_memory && settings.local_fs_method == LocalFSReadMethod::mmap && settings.mmap_threshold && settings.mmap_cache &&
  ...
  auto create = [&](size_t buffer_size, size_t buffer_alignment, int actual_flags) {
    ...
    if (settings.local_fs_method == LocalFSReadMethod::read) {
      res = std::make_unique<ReadBufferFromFile>(filename, use_page_cache ? 0 : buffer_size, actual_flags, existing_memory,
                                                 buffer_alignment, file_size, settings.local_throttler);
      ...
      res = std::make_unique<ReadBufferFromFilePReadWithDescriptorsCache>(
          filename, use_page_cache ? 0 : buffer_size, actual_flags, existing_memory, buffer_alignment, file_size, settings.local_throttler);
    } else if (settings.local_fs_method == LocalFSReadMethod::io_uring) {
#if USE_LIBURING
      use_page_cache = false;
      auto &reader = getIOUringReaderOrThrow();
      res = std::make_unique<AsynchronousReadBufferFromFileWithDescriptorsCache>(reader, settings.priority, filename, buffer_size,
                                                                                 actual_flags, existing_memory, buffer_alignment, file_size,
                                                                                 settings.local_throttler, get_prefetches_log());
#else
  ...
  if (flags == -1) flags = O_RDONLY | O_CLOEXEC;
  ...
  if (settings.direct_io_threshold && estimated_size >= settings.direct_io_threshold) {
    ...
    if (use_page_cache && settings.page_cache_block_size % min_alignment) {
      LOG_TEST(getLogger("createReadBufferFromFileBase"),
               "Not using userspace page cache because page cache block size is not divisible by direct IO alignment");
      use_page_cache = false;
    }
  ...
  if (!res) {
    ...
    if (read_hint.has_value() && *read_hint > 0 && *read_hint < buffer_size) buffer_size = *read_hint;
    if (file_size.has_value() && *file_size < buffer_size) buffer_size = *file_size;
