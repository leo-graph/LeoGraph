...
AsynchronousBoundedReadBuffer::AsynchronousBoundedReadBuffer(ImplPtr impl_, IAsynchronousReader &reader_, const ReadSettings &settings_,
                                                             size_t buffer_size_, size_t min_bytes_for_seek_,
  ...
  if (buffer_size == 0) throw Exception(ErrorCodes::BAD_ARGUMENTS, "Buffer size cannot be zero");
}

String AsynchronousBoundedReadBuffer::getInfoForLog() {
  if (prefetch_future.valid()) prefetch_future.wait();
  ...
  if (read_until_position) {
    ...
    if (file_offset_of_buffer_end > *read_until_position) {
      ...
      throw Exception(ErrorCodes::LOGICAL_ERROR, "Read beyond last offset ({} > {}): file size = {}, info: {}", file_offset_of_buffer_end,
                      *read_until_position, getFileSize(), impl->getInfoForLog());
    }
  }
...
void AsynchronousBoundedReadBuffer::prefetch(Priority priority) {
  ...
  if (!use_page_cache) prefetch_buffer.resize(buffer_size);
  ...
  if (!read_until_position || position != *read_until_position) {
    if (position < file_offset_of_buffer_end) {
      ...
      if (available() >= file_offset_of_buffer_end - position) {
        ...
        throw Exception(ErrorCodes::LOGICAL_ERROR, "Attempt to set read until position before already read data ({} < {}, info: {})",
                        position, getPosition(), impl->getInfoForLog());
      }
    }
...
void AsynchronousBoundedReadBuffer::appendToPrefetchLog(FilesystemPrefetchState state, int64_t size,
  ...
  if (prefetches_log) prefetches_log->add(std::move(elem));
}
...
bool AsynchronousBoundedReadBuffer::nextImpl() {
  ...
  if (prefetch_future.valid()) {
    ...
    /// Don't allocate any buffers if page cache is in use, the cache has its own buffers (PageCacheCell).
    if (!use_page_cache) memory.resize(buffer_size);
...
off_t AsynchronousBoundedReadBuffer::seek(off_t offset, int whence) {
  ...
  if (new_pos == static_cast<size_t>(getPosition())) return new_pos;
  ...
  if (read_until_position && new_pos > *read_until_position) {
    if (!impl->isSeekCheap()) ProfileEvents::increment(ProfileEvents::RemoteFSSeeksWithReset);
...
void AsynchronousBoundedReadBuffer::finalize() { resetPrefetch(FilesystemPrefetchState::UNNEEDED); }
...
size_t AsynchronousBoundedReadBuffer::readBigAt(char *to, size_t n, size_t range_begin,
