...
class CachedOnDiskReadBufferFromFile : public ReadBufferFromFileBase {
 public:
  ...
  CachedOnDiskReadBufferFromFile(const String& source_file_path_, const FileCacheKey& cache_key_, FileCachePtr cache_,
                                 const FileCacheOriginInfo& origin_, ImplementationBufferCreator implementation_buffer_creator_,
                                 const ReadSettings& settings_, const String& query_id_, size_t file_size_,
                                 bool allow_seeks_after_first_read_, bool use_external_buffer_, std::optional<size_t> read_until_position_,
  ...
  struct ReadInfo {
    ReadInfo(const FileCacheKey& cache_key_, const std::string& source_file_path_, ImplementationBufferCreator impl_creator_,
             bool use_external_buffer_, const ReadSettings& read_settings_, size_t read_until_position_);
 ...
 private:
  ...
  struct ReadFromFileSegmentState {
    ReadFromFileSegmentState(std::shared_ptr<ReadBufferFromFileBase> buf_, ReadType read_type_, size_t bytes_to_predownload_ = 0)
        : read_type(read_type_), buf(buf_), bytes_to_predownload(bytes_to_predownload_) {}
  ...
  static void updateReadStateIfNeeded(FileSegment& file_segment, size_t offset, ReadFromFileSegmentStatePtr& state, ReadInfo& info,
                                      size_t file_size_, LoggerPtr log);

  static bool canStartFromCache(size_t current_offset, const FileSegment& file_segment);

  static ReadFromFileSegmentStatePtr createReadFromFileSegmentState(FileSegment& file_segment, size_t offset, ReadInfo& info,
                                                                    LoggerPtr log);

  static ReadFromFileSegmentStatePtr prepareReadFromFileSegmentState(FileSegment& file_segment, size_t offset, ReadInfo& info,
                                                                     size_t file_size_, LoggerPtr log);

  static bool predownloadForFileSegment(FileSegment& file_segment, size_t offset, ReadFromFileSegmentState& state, ReadInfo& info,
                                        LoggerPtr log);

  static size_t readFromFileSegment(FileSegment& file_segment, size_t offset, size_t file_size_, ReadFromFileSegmentState& state,
                                    ReadInfo& info, bool& implementation_buffer_can_be_reused, LoggerPtr log);

  static bool writeCache(char* data, size_t size, size_t offset, FileSegment& file_segment, LoggerPtr log);

  static std::string getInfoForLog(const ReadFromFileSegmentState* state, const ReadInfo& info, size_t offset);
