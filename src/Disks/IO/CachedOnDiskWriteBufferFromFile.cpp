...
FileSegmentRangeWriter::FileSegmentRangeWriter(FileCache *cache_, const FileSegment::Key &key_, const FileCacheOriginInfo &origin_,
                                               size_t reserve_space_lock_wait_timeout_milliseconds_,
                                               std::shared_ptr<FilesystemCacheLog> cache_log_, const String &query_id_,
                                               const String &source_path_, bool is_distributed_cache_)
    : cache(cache_),
      key(key_),
      origin(origin_),
...
bool FileSegmentRangeWriter::write(char *data, size_t size, size_t offset, FileSegmentKind segment_kind) {
  ...
  if (expected_write_offset != offset) {
    throw Exception(ErrorCodes::LOGICAL_ERROR, "Cannot write file segment at offset {}, because expected write offset is: {}", offset,
                    expected_write_offset);
  }

  FileSegment *file_segment;

  if (!ignore_bytes && (!file_segments || file_segments->empty() || file_segments->front().isDownloaded())) {
  ...
  SCOPE_EXIT({
    if (!file_segments || file_segments->empty()) return;
    if (file_segments->front().isDownloader()) file_segments->front().completePartAndResetDownloader();
  });
  ...
  while (size > 0) {
    ...
    if (!file_segment->isDownloader()) {
      if (file_segment->getOrSetDownloader() != FileSegment::getCallerId()) {
        ...
        throw Exception(
            is_distributed_cache ? ErrorCodes::FILECACHE_CANNOT_WRITE_THROUGH_CACHE_WITH_CONCURRENT_READS : ErrorCodes::LOGICAL_ERROR,
    ...
    if (!reserved) {
      ...
      LOG_DEBUG(log, "Failed to reserve space in cache (size: {}, file segment info: {}", size, file_segment->getInfoForLog());
  ...
  size_t available_size = file_segment->range().size() - file_segment->getDownloadedSize();
  if (available_size == 0) completeFileSegment();
...
FileSegment &FileSegmentRangeWriter::allocateFileSegment(size_t offset, FileSegmentKind segment_kind) {
  ...
  if (is_distributed_cache) {
    file_segments = cache->getOrSet(key, offset, /* size */ cache->getMaxFileSegmentSize(),
    ...
    if (file_segment.getDownloadedSize() != 0) {
      LOG_TRACE(log,
                "File segment already exists and has downloaded size ({}) "
                "(write offset: {}, file segment range: {}), "
                "current write offset is {}. Will continue download offset",
                file_segment.getDownloadedSize(), file_segment.getCurrentWriteOffset(), file_segment.range().toString(), offset);
    }

    if (file_segment.getCurrentWriteOffset() > offset && file_segment.isBackgroundDownloadEnabled()) {
      LOG_TRACE(log,
      ...
      throw Exception(ErrorCodes::LOGICAL_ERROR, "Writing at offset {}, but covering file segment has write offset {} ({})", offset,
                      file_segment.getCurrentWriteOffset(), file_segment.getInfoForLog());
    }
  } else {
...
void FileSegmentRangeWriter::completeFileSegment() {
  ...
  if (file_segment.isDetached() || file_segment.isCompleted()) return;
...
CachedOnDiskWriteBufferFromFile::CachedOnDiskWriteBufferFromFile(std::unique_ptr<WriteBuffer> impl_, FileCachePtr cache_,
                                                                 const String &source_path_, const FileCache::Key &key_,
                                                                 const String &query_id_, const WriteSettings &settings_,
                                                                 const FileCacheOriginInfo &origin_,
                                                                 std::shared_ptr<FilesystemCacheLog> cache_log_, bool is_distributed_cache_,
  ...
  LOG_TEST(log, "Cache key: {}, source path: {}, is distributed cache: {}", key.toString(), source_path, is_distributed_cache);
}
...
void CachedOnDiskWriteBufferFromFile::cacheData(char *data, size_t size, bool throw_on_error) {
  if (cache_in_error_state_or_disabled) return;

  if (!cache_writer) {
    cache_writer = std::make_unique<FileSegmentRangeWriter>(cache.get(), key, origin, reserve_space_lock_wait_timeout_milliseconds,
                                                            cache_log, query_id, source_path, is_distributed_cache);

    if (cache_writer_start_position) cache_writer->jumpToPosition(cache_writer_start_position);
  }
