...
ThreadPoolReader::ThreadPoolReader(size_t pool_size, size_t queue_size_)
    : pool(std::make_unique<ThreadPool>(CurrentMetrics::ThreadPoolFSReaderThreads, CurrentMetrics::ThreadPoolFSReaderThreadsActive,
                                        CurrentMetrics::ThreadPoolFSReaderThreadsScheduled, pool_size, pool_size, queue_size_)) {}
  ...
  if (has_pread_nowait_support && !request.direct_io) {
    ...
    while (!bytes_read) {
...

      {
        ...
        res = syscall(SYS_preadv2, fd, &io_vec, 1,
    ...
    if (bytes_read) {
      ...
      promise.set_value(
          {.buf = request.buf, .size = bytes_read, .offset = request.ignore, .file_offset_of_buffer_end = request.offset + bytes_read});
      return future;
    }
  }
#endif
...
void ThreadPoolReader::wait() { pool->wait(); }
