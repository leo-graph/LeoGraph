...
ReadBufferFromWebServer::ReadBufferFromWebServer(const String& url_, ContextPtr context_, size_t file_size_, const ReadSettings& settings_,
                                                 bool use_external_buffer_, size_t read_until_position_)
...
std::unique_ptr<SeekableReadBuffer> ReadBufferFromWebServer::initialize() {
  ...
  if (read_until_position) res->setReadUntilPosition(read_until_position);
  if (offset) res->seek(offset, SEEK_SET);
...
bool ReadBufferFromWebServer::nextImpl() {
  ...
  if (result) offset += working_buffer.size();
...
off_t ReadBufferFromWebServer::getPosition() { return offset - available(); }
