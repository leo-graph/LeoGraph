...
WriteBufferWithFinalizeCallback::WriteBufferWithFinalizeCallback(std::unique_ptr<WriteBuffer> impl_, FinalizeCallback&& create_callback_,
                                                                 const String& remote_path_, bool create_blob_if_empty_)
...
void WriteBufferWithFinalizeCallback::preFinalize() {
  ...
  if (create_blob_if_empty || bytes_written > 0) WriteBufferFromFileDecorator::preFinalize();
}
...
void WriteBufferWithFinalizeCallback::finalizeImpl() {
  ...
  if (create_metadata_callback) create_metadata_callback(bytes_written);
}
