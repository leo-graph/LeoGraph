...
IOUringReader::IOUringReader(uint32_t entries_) : log(getLogger("IOUringReader")) {
  ...
  if (ret < 0) ErrnoException::throwWithErrno(ErrorCodes::IO_URING_INIT_FAILED, -ret, "Failed initializing io_uring");
...
std::future<IAsynchronousReader::Result> IOUringReader::submit(Request request) {
  ...
  auto enqueued_request = EnqueuedRequest{.promise = std::move(promise), .request = request, .resubmitting = false, .bytes_read = 0};
    ...
    if (ret > 0) {
      ...
      if (!success) {
        ...
        return makeFailedResult(
            Exception(ErrorCodes::LOGICAL_ERROR, "Tried enqueuing read request for {} that is already submitted", request_id));
      }
    ...
    if (ret > 0) {
      ...
      if (!in_flight_requests.contains(request_id))
        ...
        failPromise(
            ...
            Exception(ErrorCodes::LOGICAL_ERROR, "Tried enqueuing pending read request for {} that is already submitted", request_id));
    } else {
      ProfileEvents::increment(ProfileEvents::ReadBufferFromFileDescriptorReadFailed);
      failPromise(pending_request.promise, Exception(ErrorCodes::IO_URING_SUBMIT_ERROR, "Failed submitting SQE for pending request: {}",
...
void IOUringReader::monitorRing() {
  ...
  while (!cancelled.load(std::memory_order_relaxed)) {
    ...
    if (cqe->res == -EAGAIN || cqe->res == -EINTR) {
      ...
      if (ret <= 0) {
        failRequest(it, Exception(ErrorCodes::IO_URING_SUBMIT_ERROR, "Failed re-submitting SQE: {}",
                                  ret < 0 ? errnoToString(-ret) : "no SQE submitted"));
      }
    ...
    if (cqe->res < 0) {
      ...
      failRequest(
          it, Exception(ErrorCodes::CANNOT_READ_FROM_FILE_DESCRIPTOR, "Failed reading {} bytes at offset {} to address {} from fd {}: {}",
    ...
    if (bytes_read > 0 && total_bytes_read < enqueued.request.size) {
      ...
      ret = submitToRing(enqueued);
      if (ret <= 0) {
        failRequest(it, Exception(ErrorCodes::IO_URING_SUBMIT_ERROR, "Failed re-submitting SQE for short read: {}",
                                  ret < 0 ? errnoToString(-ret) : "no SQE submitted"));
      }
    } else {
      ProfileEvents::increment(ProfileEvents::AsynchronousReaderIgnoredBytes, enqueued.request.ignore);
      enqueued.promise.set_value(Result{.buf = enqueued.request.buf,
                                        .size = total_bytes_read,
                                        .offset = enqueued.request.ignore,
                                        .file_offset_of_buffer_end = enqueued.request.offset + total_bytes_read});
      finalizeRequest(it);
    }
