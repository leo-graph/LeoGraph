...
class DiskLocalReservation : public IReservation {
 public:
  DiskLocalReservation(const DiskLocalPtr& disk_, UInt64 size_, UInt64 unreserved_space_)
      : disk(disk_), size(size_), unreserved_space(unreserved_space_), metric_increment(CurrentMetrics::DiskSpaceReservedForMerge, size_) {}

  UInt64 getSize() const override { return size; }
  std::optional<UInt64> getUnreservedSpace() const override { return unreserved_space; }

  DiskPtr getDisk(size_t i) const override {
    if (i != 0) throw Exception(ErrorCodes::LOGICAL_ERROR, "Can't use i != 0 with single disk reservation. It's a bug");
    return disk;
  }
...
class DiskLocalDirectoryIterator final : public IDirectoryIterator {
 public:
  ...
  String path() const override {
    if (entry->is_directory()) return dir_path / entry->path().filename() / "";
...
ReservationPtr DiskLocal::reserve(UInt64 bytes) {
  ...
  if (!unreserved_space.has_value()) return {};
  return std::make_unique<DiskLocalReservation>(std::static_pointer_cast<DiskLocal>(shared_from_this()), bytes, unreserved_space.value());
}
...
std::optional<UInt64> DiskLocal::tryReserve(UInt64 bytes, const std::optional<ReservationConstraints>& constraints) {
  ...
  UInt64 unreserved_space =
      available_space ? *available_space - std::min(*available_space, reserved_bytes) : std::numeric_limits<UInt64>::max();
  ...
  if (constraints.has_value()) {
    ...
    if (available_space.has_value() && total_space.has_value()) {
      ...
      if (bytes > unreserved_space) return {};
      ...
      if (constraints->min_bytes > 0 && free_bytes_after < constraints->min_bytes) {
        LOG_TRACE(logger,
                  "Could not reserve {} ({} bytes) on disk {}. Free space after reservation {} bytes ({}) would be less than min_bytes {} "
                  "bytes ({})",
                  ReadableSize(bytes), bytes, backQuote(name), free_bytes_after, ReadableSize(free_bytes_after), constraints->min_bytes,
                  ReadableSize(constraints->min_bytes));
        return {};
      }
      ...
      if (constraints->min_ratio > 0.0) {
        ...
        if (free_bytes_after < min_bytes_from_ratio) {
          LOG_TRACE(logger,
                    "Could not reserve {} ({} bytes) on disk {}. Free space after reservation {} bytes ({}) would be less than min_ratio "
                    "requirement {} bytes ({}) (ratio: {}, total: {} bytes)",
                    ReadableSize(bytes), bytes, backQuote(name), free_bytes_after, ReadableSize(free_bytes_after), min_bytes_from_ratio,
                    ReadableSize(min_bytes_from_ratio), constraints->min_ratio, *total_space);
          return {};
        }
      }
    }
  ...
  if (unreserved_space >= bytes) {
    if (available_space) {
      LOG_TRACE(logger, "Reserved {} on local disk {}, having unreserved {}.", ReadableSize(bytes), backQuote(name),
                ReadableSize(unreserved_space));
    } else {
      LOG_TRACE(logger, "Reserved {} on local disk {}.", ReadableSize(bytes), backQuote(name));
    }
...
bool DiskLocal::existsFileOrDirectory(const String& path) const { return fs::exists(fs::path(disk_path) / path); }

bool DiskLocal::existsFile(const String& path) const { return fs::is_regular_file(fs::path(disk_path) / path); }

bool DiskLocal::existsDirectory(const String& path) const { return fs::is_directory(fs::path(disk_path) / path); }

size_t DiskLocal::getFileSize(const String& path) const { return fs::file_size(fs::path(disk_path) / path); }

void DiskLocal::createDirectory(const String& path) { fs::create_directory(fs::path(disk_path) / path); }

void DiskLocal::createDirectories(const String& path) { fs::create_directories(fs::path(disk_path) / path); }
...
DirectoryIteratorPtr DiskLocal::iterateDirectory(const String& path) const {
  ...
  if (!broken && fs::exists(meta_path) && fs::is_directory(meta_path)) return std::make_unique<DiskLocalDirectoryIterator>(disk_path, path);
...
std::unique_ptr<ReadBufferFromFileBase> DiskLocal::readFile(const String& path, const ReadSettings& settings,
                                                            std::optional<size_t> read_hint) const {
  return createReadBufferFromFileBase(fs::path(disk_path) / path, settings, read_hint, /*file_size*/ std::nullopt, /*flags*/ -1,
                                      /*existing_memory*/ nullptr, settings.use_page_cache_for_local_disks);
}
...
std::unique_ptr<WriteBufferFromFileBase> DiskLocal::writeFile(const String& path, size_t buf_size, WriteMode mode,
  ...
  return std::make_unique<WriteBufferFromFile>(fs::path(disk_path) / path, buf_size, flags, settings.local_throttler, 0666, nullptr, 0,
                                               settings.use_adaptive_write_buffer, settings.adaptive_write_buffer_initial_size);
}
...
void DiskLocal::removeFile(const String& path) {
  auto fs_path = fs::path(disk_path) / path;
  if (0 != unlink(fs_path.c_str())) ErrnoException::throwFromPath(ErrorCodes::CANNOT_UNLINK, fs_path, "Cannot unlink file {}", fs_path);
}

void DiskLocal::removeFileIfExists(const String& path) {
  auto fs_path = fs::path(disk_path) / path;
  if (0 != unlink(fs_path.c_str())) {
    if (errno != ENOENT) ErrnoException::throwFromPath(ErrorCodes::CANNOT_UNLINK, fs_path, "Cannot unlink file {}", fs_path);
  }
}

void DiskLocal::removeDirectory(const String& path) {
  auto fs_path = fs::path(disk_path) / path;
  if (0 != rmdir(fs_path.c_str())) ErrnoException::throwFromPath(ErrorCodes::CANNOT_RMDIR, fs_path, "Cannot remove directory {}", fs_path);
}

void DiskLocal::removeDirectoryIfExists(const String& path) {
  auto fs_path = fs::path(disk_path) / path;
  if (!existsDirectory(fs_path)) return;
  if (0 != rmdir(fs_path.c_str()))
    if (errno != ENOENT) ErrnoException::throwFromPath(ErrorCodes::CANNOT_RMDIR, fs_path, "Cannot remove directory {}", fs_path);
}

void DiskLocal::removeRecursive(const String& path) { (void)fs::remove_all(fs::path(disk_path) / path); }

void DiskLocal::listFiles(const String& path, std::vector<String>& file_names) const {
  file_names.clear();
  for (const auto& entry : fs::directory_iterator(fs::path(disk_path) / path)) file_names.emplace_back(entry.path().filename());
}
...
Poco::Timestamp DiskLocal::getLastModified(const String& path) const { return FS::getModificationTimestamp(fs::path(disk_path) / path); }

time_t DiskLocal::getLastChanged(const String& path) const { return FS::getChangeTime(fs::path(disk_path) / path); }
...
bool DiskLocal::isSymlink(const String& path) const { return FS::isSymlink(fs::path(disk_path) / path); }

bool DiskLocal::isSymlinkNoThrow(const String& path) const { return FS::isSymlinkNoThrow(fs::path(disk_path) / path); }
...
String DiskLocal::readSymlink(const fs::path& path) const { return FS::readSymlink(fs::path(disk_path) / path); }
...
void DiskLocal::truncateFile(const String& path, size_t size) {
  ...
  if (-1 == res) ErrnoException::throwFromPath(ErrorCodes::CANNOT_TRUNCATE_FILE, path, "Cannot truncate {}", path);
}

void DiskLocal::createFile(const String& path) { FS::createFile(fs::path(disk_path) / path); }

void DiskLocal::setReadOnly(const String& path) {
  fs::permissions(fs::path(disk_path) / path, fs::perms::owner_write | fs::perms::group_write | fs::perms::others_write,
                  fs::perm_options::remove);
}

bool inline isSameDiskType(const IDisk& one, const IDisk& another) { return typeid(one) == typeid(another); }
...
void DiskLocal::copyDirectoryContent(const String& from_dir, const std::shared_ptr<IDisk>& to_disk, const String& to_dir,
  ...
  if (isSameDiskType(*this, *to_disk) && !read_settings.local_throttler && !write_settings.local_throttler)
    fs::copy(fs::path(disk_path) / from_dir, fs::path(to_disk->getPath()) / to_dir,
             fs::copy_options::recursive | fs::copy_options::overwrite_existing);  /// Use more optimal way.
  else
...
void DiskLocal::applyNewSettings(const Poco::Util::AbstractConfiguration& config, ContextPtr context, const String& config_prefix,
  ...
  if (disk_path != new_disk_path) throw Exception(ErrorCodes::UNKNOWN_ELEMENT_IN_CONFIG, "Disk path can't be updated from config {}", name);

  if (keep_free_space_bytes != new_keep_free_space_bytes) keep_free_space_bytes = new_keep_free_space_bytes;
...
DiskLocal::DiskLocal(const String& name_, const String& path_, UInt64 keep_free_space_bytes_, ContextPtr context,
                     const Poco::Util::AbstractConfiguration& config, const String& config_prefix)
  ...
  if (local_disk_check_period_ms > 0) disk_checker = std::make_unique<DiskLocalCheckThread>(this, context, local_disk_check_period_ms);
}
...
DataSourceDescription DiskLocal::getDataSourceDescription() const { return data_source_description; }
...
void DiskLocal::shutdown() {
  if (disk_checker) disk_checker->shutdown();
}

std::optional<UInt32> DiskLocal::readDiskCheckerMagicNumber() const noexcept try {
...
bool DiskLocal::canRead() const noexcept try {
  if (FS::canRead(fs::path(disk_path) / disk_checker_path)) {
    auto magic_number = readDiskCheckerMagicNumber();
    if (magic_number && *magic_number == disk_checker_magic_number) return true;
  }
  return false;
} catch (...) {
...
bool DiskLocal::canWrite() noexcept try {
...
void DiskLocal::startupImpl() {
  ...
  if (disk_checker && disk_checker_can_check_read) disk_checker->startup();
}

struct stat DiskLocal::stat(const String& path) const {
  struct stat st;
  auto full_path = fs::path(disk_path) / path;
  if (::stat(full_path.string().c_str(), &st) == 0) return st;
...
void DiskLocal::chmod(const String& path, mode_t mode) {
  auto full_path = fs::path(disk_path) / path;
  if (::chmod(full_path.string().c_str(), mode) == 0) return;
...
void registerDiskLocal(DiskFactory& factory, bool global_skip_access_check) {
  auto creator = [global_skip_access_check](const String& name, const Poco::Util::AbstractConfiguration& config,
                                            const String& config_prefix, ContextPtr context, const DisksMap& map, bool, bool) -> DiskPtr {
    String path;
    ...
    std::shared_ptr<IDisk> disk = std::make_shared<DiskLocal>(name, path, keep_free_space_bytes, context, config, config_prefix);
