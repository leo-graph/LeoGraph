...
class DiskEncrypted : public IDisk {
 public:
  ...
  void copyDirectoryContent(const String& from_dir, const std::shared_ptr<IDisk>& to_disk, const String& to_dir,
                            const ReadSettings& read_settings, const WriteSettings& write_settings,
                            const std::function<void()>& cancellation_hook) override;

  void copyFile(const String& from_file_path, IDisk& to_disk, const String& to_file_path, const ReadSettings& read_settings,
...
#  endif
 ...
 private:
  String wrappedPath(const String& path) const { return DiskEncryptedTransaction::wrappedPath(disk_path, path); }
