...
using TransactionCommitOptionsVariant =
    std::variant<NoCommitOptions, MetaInKeeperCommitOptions<zkutil::ZooKeeper>, MetaInKeeperCommitOptions<ZooKeeperWithFaultInjection>>;

inline String getFirstFailedPath(Coordination::Error code, const Coordination::Responses& responses, size_t first_additional_request_idx,
                                 const Coordination::Requests& additional_ops) {
  size_t failed_op_index = zkutil::getFailedOpIndex(code, responses);
  if (failed_op_index < first_additional_request_idx) return "<in disk transaction>";
...
using TransactionCommitOutcomeVariant = std::variant<bool, MetaInKeeperCommitOutcome>;
...
inline bool mayRetryCommit(const TransactionCommitOptionsVariant& options, const TransactionCommitOutcomeVariant& outcome) {
  if (isSuccessfulOutcome(outcome)) return false;
...
inline MetaInKeeperCommitOutcome getKeeperOutcome(const TransactionCommitOutcomeVariant& outcome) {
  if (const auto* result = std::get_if<MetaInKeeperCommitOutcome>(&outcome); result) return *result;
