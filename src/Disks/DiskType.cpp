...
MetadataStorageType metadataTypeFromString(const String& type) {
  ...
  if (check_type == "local") return MetadataStorageType::Local;
  if (check_type == "plain") return MetadataStorageType::Plain;
  if (check_type == "plain_rewritable") return MetadataStorageType::PlainRewritable;
  if (check_type == "web") return MetadataStorageType::StaticWeb;
  if (check_type == "keeper") return MetadataStorageType::Keeper;
  if (check_type == "memory") return MetadataStorageType::Memory;
...
bool DataSourceDescription::operator==(const DataSourceDescription& other) const {
  return std::tie(type, object_storage_type, description, is_encrypted, zookeeper_name) ==
         std::tie(other.type, other.object_storage_type, other.description, other.is_encrypted, other.zookeeper_name);
}
...
bool DataSourceDescription::sameKind(const DataSourceDescription& other) const {
  ...
  return std::tie(type, object_storage_type, our_description) == std::tie(other.type, other.object_storage_type, other_description);
}
...
String DataSourceDescription::toString() const {
  return fmt::format("{} (description = '{}', is_encrypted = {}, is_cached = {}, zookeeper_name = '{}')", name(), description, is_encrypted,
                     is_cached, zookeeper_name);
}
