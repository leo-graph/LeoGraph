...
IDisk::IDisk(const String &name_, const Poco::Util::AbstractConfiguration &config, const String &config_prefix)
    : name(name_),
      copying_thread_pool(std::make_unique<ThreadPool>(CurrentMetrics::IDiskCopierThreads, CurrentMetrics::IDiskCopierThreadsActive,
...
IDisk::IDisk(const String &name_)
    : name(name_),
      copying_thread_pool(std::make_unique<ThreadPool>(CurrentMetrics::IDiskCopierThreads, CurrentMetrics::IDiskCopierThreadsActive,
                                                       CurrentMetrics::IDiskCopierThreadsScheduled, 16)) {}

IDisk::~IDisk() = default;

bool IDisk::isDirectoryEmpty(const String &path) const { return !iterateDirectory(path)->isValid(); }
...
void IDisk::copyFile(  /// NOLINT
  ...
  LOG_DEBUG(getLogger("IDisk"), "Copying from {} (path: {}) {} to {} (path: {}) {}.", getName(), getPath(), from_file_path,
            to_disk.getName(), to_disk.getPath(), to_file_path);
...
DiskTransactionPtr IDisk::createTransaction() { return std::make_shared<FakeDiskTransaction>(*this); }
...
void asyncCopy(IDisk &from_disk, String from_path, IDisk &to_disk, String to_path, ThreadPoolCallbackRunnerLocal<void> &runner,
               const ReadSettings &read_settings, const WriteSettings &write_settings, const std::function<void()> &cancellation_hook) {
  if (from_disk.existsFile(from_path)) {
    runner.enqueueAndKeepTrack([&from_disk, from_path, &to_disk, to_path, &read_settings, &write_settings, &cancellation_hook] {
      from_disk.copyFile(from_path, to_disk, to_path, read_settings, write_settings, cancellation_hook);
    });
  } else  /// Directory
  {
...
void IDisk::copyThroughBuffers(const String &from_path, const std::shared_ptr<IDisk> &to_disk, const String &to_path,
                               const ReadSettings &read_settings, WriteSettings write_settings,
...
void IDisk::truncateFile(const String &, size_t) {
  throw Exception(ErrorCodes::NOT_IMPLEMENTED, "Truncate operation is not implemented for disk of type {}",
                  getDataSourceDescription().type);
}

SyncGuardPtr IDisk::getDirectorySyncGuard(const String & /* path */) const { return nullptr; }

void IDisk::startup(bool skip_access_check) {
  if (!skip_access_check) {
    if (isReadOnly()) {
      LOG_DEBUG(getLogger("IDisk"), "Skip access check for disk {} (read-only disk).", getName());
    } else
      checkAccess();
  }
  startupImpl();
...
void IDisk::checkAccess() {
  ...
  if (server_uuid == DB::UUIDHelpers::Nil) throw Exception(ErrorCodes::LOGICAL_ERROR, "Server UUID is not initialized");
...
void IDisk::checkAccessImpl(const String &path) try {
  ...
  /// read
  {
    ...
    if (buf != payload) {
      throw Exception(ErrorCodes::CANNOT_READ_ALL_DATA, "Content of {}::{} does not matches after read ({} vs {})", name, path, buf,
                      payload);
    }
  }
  ...
  /// read with offset
  {
    ...
    if (buf != payload.substr(offset)) {
      throw Exception(ErrorCodes::CANNOT_READ_ALL_DATA, "Content of {}::{} does not matches after read with offset ({} vs {})", name, path,
...
bool IDisk::isCaseInsensitive() {
  ...
  if (is_case_sensitivity_checked) return is_case_insensitive;
...
void IDisk::applyNewSettings(const Poco::Util::AbstractConfiguration &config, ContextPtr /*context*/, const String &config_prefix,
                             const DisksMap & /*map*/) {
