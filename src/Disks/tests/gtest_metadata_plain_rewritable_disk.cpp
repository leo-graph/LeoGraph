...
class MetadataPlainRewritableDiskTest : public testing::Test {
 public:
  ...
  std::shared_ptr<IMetadataStorage> getMetadataStorage(const std::string& key_prefix) {
    ...
    if (!active_metadatas[key_prefix]) active_metadatas[key_prefix] = createMetadataStorage(key_prefix);
  ...
  void TearDown() override {
    for (const auto& [_, metadata] : active_metadatas) metadata->shutdown();
...
std::vector<std::string> listAllBlobs(std::string test) {
  if (!std::filesystem::exists(fmt::format("./{}", test))) return {};
  ...
  return sorted(std::filesystem::recursive_directory_iterator(fmt::format("./{}", test)) |
                ...
                std::views::transform([](const auto& file) { return file.path(); }) | std::ranges::to<std::vector<std::string>>());
}
...
TEST_F(MetadataPlainRewritableDiskTest, RemoveDirectoryRecursive) {
  ...
  EXPECT_EQ(listAllBlobs("RemoveDirectoryRecursive"),
...
TEST_F(MetadataPlainRewritableDiskTest, RemoveDirectoryRecursiveVirtualNodes) {
  ...
  EXPECT_FALSE(metadata->existsDirectory("root/A/B/C"));
  EXPECT_FALSE(metadata->existsDirectory("root/A/B/C/D"));
  EXPECT_EQ(listAllBlobs("RemoveDirectoryRecursiveVirtualNodes"),
            std::vector<std::string>({
...
TEST_F(MetadataPlainRewritableDiskTest, RemoveRoot) {
  ...
  EXPECT_EQ(listAllBlobs("RemoveRecursiveRoot"),
            std::vector<std::string>({"./RemoveRecursiveRoot/__root/A", "./RemoveRecursiveRoot/__root/B"}));
...
TEST_F(MetadataPlainRewritableDiskTest, CreateFiles) {
  ...
  /// Some rewrites
  {
    ...
    size_t size_2 =
        writeObject(object_storage, tx->generateObjectKeyForPath("/A/f1").serialize(), "Touch the untouchable, break the unbreakable");
    tx->createMetadataFile("/A/f1", {StoredObject("C", "f1", size_2)});
    size_t size_3 =
        writeObject(object_storage, tx->generateObjectKeyForPath("/A/f1").serialize(), "Just break the rule, then you see the truth");
...
TEST_F(MetadataPlainRewritableDiskTest, UncommittedMove) {
  ...
  EXPECT_EQ(listAllBlobs("UncommittedMove"),
            std::vector<std::string>({"./UncommittedMove/__meta/faefxnlkbtfqgxcbfqfjtztsocaqrnqn/prefix.path",
...
TEST_F(MetadataPlainRewritableDiskTest, UncommittedHardlink) {
  ...
  EXPECT_EQ(listAllBlobs("UncommittedHardlink"),
            std::vector<std::string>({"./UncommittedHardlink/__meta/faefxnlkbtfqgxcbfqfjtztsocaqrnqn/prefix.path",
...
TEST_F(MetadataPlainRewritableDiskTest, UncommittedDirectoryMoves) {
  ...
  EXPECT_EQ(listAllBlobs("UncommittedDirectoryMoves"),
            std::vector<std::string>({
...
TEST_F(MetadataPlainRewritableDiskTest, CreateDirectoryFromVirtualNode) {
  ...
  EXPECT_EQ(listAllBlobs("CreateDirectoryFromVirtualNode"),
            std::vector<std::string>({
...
TEST_F(MetadataPlainRewritableDiskTest, UnlinkUndoInCaseOfNetworkError) {
...

  {
    ...
    tx->createDirectory("/A");
    auto size_bytes =
        writeObject(object_storage, tx->generateObjectKeyForPath("/A/file").serialize(), "This is San Francisco, city of stile disco");
    tx->createMetadataFile("/A/file", {StoredObject("/A/file", "file", size_bytes)});

    tx->commit(DB::NoCommitOptions{});
  }

  EXPECT_EQ(readObject(object_storage, metadata->getStorageObjects("/A/file").front().remote_path),
            "This is San Francisco, city of stile disco");
