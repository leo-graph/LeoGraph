...
class DiskObjectStorageTest : public testing::Test {
 public:
  ...
  static void TearDownTestSuite() { DB::clearDiskRegistry(); }

  void SetUp() override { thread_local_rng.seed(42); }

  void TearDown() override {
    for (const auto& [_, disk] : initialized_disks) disk->shutdown();
  ...
  void waitBlobsCount(DB::DiskPtr disk, size_t needed_count) {
    for (size_t i = 0; i < 100; ++i) {
      ...
      if (children.size() == needed_count) return;
  ...
  DB::DiskPtr getDiskObjectStorage() {
    if (!initialized_disks.empty()) return initialized_disks.begin()->second;
    ...
    auto disk = factory.create(name, getContext().context->getConfigRef(), prefix, getContext().context, initialized_disks,
                               /*attach*/ false,
                               /*custom_disk*/ true,
                               /*skip_types*/ {});
