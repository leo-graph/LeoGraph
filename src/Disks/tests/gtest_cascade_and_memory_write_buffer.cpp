...
static std::string makeTestArray(size_t size) {
  ...
  for (size_t i = 0; i < res.size(); ++i) res[i] = static_cast<char>(i % 256);
  return res;
}
...
static void testCascadeBufferRedability(std::string data, CascadeWriteBuffer::WriteBufferPtrs &&arg1,
...
try {
  ...
  for (size_t s = 0; s < max_s; ++s) {
    testCascadeBufferRedability(
        makeTestArray(s), {std::make_shared<MemoryWriteBuffer>(s / 2, 1, 2.0), std::make_shared<MemoryWriteBuffer>(s - s / 2, 1, 2.0)}, {});
    ...
    testCascadeBufferRedability(makeTestArray(max_s), {std::make_shared<MemoryWriteBuffer>(s, 1, 2.0)},
...
static void checkHTTPHandlerCase(size_t input_size, size_t memory_buffer_size) {
...

  {
    ...
    CascadeWriteBuffer cascade({std::make_shared<MemoryWriteBuffer>(memory_buffer_size)}, {[res_buf](const WriteBufferPtr &prev_buf) {
...
TEST(CascadeWriteBuffer, HTTPHandlerCase) {
  ...
  for (size_t input_size : sizes) {
    for (size_t memory_buffer_size : sizes) {
      if (input_size > memory_buffer_size) checkHTTPHandlerCase(input_size, memory_buffer_size);
    }
  }
}
...
try {
  for (size_t s = 0; s < 4000000; s += 1000000) {
    testCascadeBufferRedability(makeTestArray(s), {}, {[=, this](auto) {
                                  return std::make_shared<WriteBufferFromTemporaryFile>(std::make_unique<TemporaryFileOnDisk>(disk));
                                }});
