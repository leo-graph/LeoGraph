...
class DiskEncryptedTest : public ::testing::Test {
 protected:
  ...
  String getDirectory() { return temp_dir->path() + "/"; }
  ...
  static String getAlphabetWithDigits() {
    String contents;
    for (char c = 'a'; c <= 'z'; ++c) contents += c;
    for (char c = '0'; c <= '9'; ++c) contents += c;
    return contents;
  }
...
TEST_F(DiskEncryptedTest, LocalBlobs) {
  ObjectStoragePtr object_storage =
      std::make_shared<LocalObjectStorage>(LocalObjectStorageSettings("test", fs::path{getDirectory()} / "local_blobs", false));
  ...
  auto local_blobs = std::make_shared<DiskObjectStorage>("local_blobs", std::move(cluster), std::move(metadata_storage),
                                                         std::move(object_storages), *config, "");
