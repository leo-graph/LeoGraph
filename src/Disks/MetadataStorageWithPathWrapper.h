...
class MetadataStorageWithPathWrapperTransaction final : public IMetadataTransaction {
 ...
 public:
  static String wrappedPath(const String& path_wrapper, const String& path) {
    ...
    if (!path_wrapper.empty() && path.starts_with(path_wrapper)) return path;
  ...
  MetadataStorageWithPathWrapperTransaction(MetadataTransactionPtr delegate_, const std::string& metadata_path_)
      : delegate(std::move(delegate_)), metadata_path(metadata_path_) {}

  ~MetadataStorageWithPathWrapperTransaction() override = default;

  void commit(const TransactionCommitOptionsVariant& options) final { delegate->commit(options); }
  ...
  void chmod(const String& path, mode_t mode) override { delegate->chmod(wrappedPath(path), mode); }

  void setReadOnly(const std::string& path) override { delegate->setReadOnly(wrappedPath(path)); }
  ...
  void createDirectory(const std::string& path) override { delegate->createDirectory(wrappedPath(path)); }

  void createDirectoryRecursive(const std::string& path) override { delegate->createDirectoryRecursive(wrappedPath(path)); }

  void removeDirectory(const std::string& path) override { delegate->removeDirectory(wrappedPath(path)); }
  ...
  void truncateFile(const std::string& src_path, size_t size) override { delegate->truncateFile(wrappedPath(src_path), size); }
  ...
  void recordBlobsReplication(const StoredObject& blob, const Locations& missing_locations) override {
    delegate->recordBlobsReplication(blob, missing_locations);
  }
...
class MetadataStorageWithPathWrapper final : public IMetadataStorage {
 private:
  ...
  Strings wrappedPaths(const Strings& paths) const {
    ...
    for (const auto& path : paths) wrapped_paths.push_back(wrappedPath(path));
 ...
 public:
  MetadataStorageWithPathWrapper(MetadataStoragePtr delegate_, const std::string& metadata_path_)
      : delegate(std::move(delegate_)), metadata_path(metadata_path_), metadata_absolute_path(wrappedPath(delegate->getPath())) {}

  MetadataTransactionPtr createTransaction() override {
    return std::make_shared<MetadataStorageWithPathWrapperTransaction>(delegate->createTransaction(), metadata_path);
  }

  const std::string& getPath() const override { return metadata_absolute_path; }
  ...
  bool existsFile(const std::string& path) const override { return delegate->existsFile(wrappedPath(path)); }

  bool existsDirectory(const std::string& path) const override { return delegate->existsDirectory(wrappedPath(path)); }

  bool existsFileOrDirectory(const std::string& path) const override { return delegate->existsFileOrDirectory(wrappedPath(path)); }

  uint64_t getFileSize(const String& path) const override { return delegate->getFileSize(wrappedPath(path)); }

  Poco::Timestamp getLastModified(const std::string& path) const override { return delegate->getLastModified(wrappedPath(path)); }

  time_t getLastChanged(const std::string& path) const override { return delegate->getLastChanged(wrappedPath(path)); }
  ...
  std::vector<std::string> listDirectory(const std::string& path) const override { return delegate->listDirectory(wrappedPath(path)); }

  DirectoryIteratorPtr iterateDirectory(const std::string& path) const override { return delegate->iterateDirectory(wrappedPath(path)); }

  std::string readFileToString(const std::string& path) const override { return delegate->readFileToString(wrappedPath(path)); }

  std::string readInlineDataToString(const std::string& path) const override { return delegate->readInlineDataToString(wrappedPath(path)); }

  std::unordered_map<String, String> getSerializedMetadata(const std::vector<String>& file_paths) const override {
    return delegate->getSerializedMetadata(wrappedPaths(file_paths));
  }

  uint32_t getHardlinkCount(const std::string& path) const override { return delegate->getHardlinkCount(wrappedPath(path)); }

  StoredObjects getStorageObjects(const std::string& path) const override { return delegate->getStorageObjects(wrappedPath(path)); }

  bool isReadOnly() const override { return delegate->isReadOnly(); }

  void updateCache(const std::vector<std::string>& paths, bool recursive, bool enforce_fresh,
                   std::string* serialized_cache_update_description) override {
  ...
  void invalidateCache(const std::string& path) override { delegate->invalidateCache(wrappedPath(path)); }

  void dropCache() override { delegate->dropCache(); }
  ...
  int64_t recordAsRemoved(const StoredObjects& blobs) override { return delegate->recordAsRemoved(blobs); }
  ...
  int64_t recordAsReplicated(const BlobsToReplicate& blobs) override { return delegate->recordAsReplicated(blobs); }
};
