...
DiskObjectStorageTransaction::DiskObjectStorageTransaction(ClusterConfigurationPtr cluster_, MetadataStoragePtr metadata_storage_,
...
MultipleDisksObjectStorageTransaction::MultipleDisksObjectStorageTransaction(ClusterConfigurationPtr source_cluster_,
...
void DiskObjectStorageTransaction::createDirectory(const std::string& path) {
  operations_to_execute.push_back([path](MetadataTransactionPtr tx) { tx->createDirectory(path); });
}

void DiskObjectStorageTransaction::createDirectories(const std::string& path) {
  operations_to_execute.push_back([path](MetadataTransactionPtr tx) { tx->createDirectoryRecursive(path); });
}

void DiskObjectStorageTransaction::moveDirectory(const std::string& from_path, const std::string& to_path) {
  operations_to_execute.push_back([from_path, to_path](MetadataTransactionPtr tx) { tx->moveDirectory(from_path, to_path); });
}

void DiskObjectStorageTransaction::moveFile(const String& from_path, const String& to_path) {
  operations_to_execute.push_back([from_path, to_path](MetadataTransactionPtr tx) { tx->moveFile(from_path, to_path); });
}

void DiskObjectStorageTransaction::truncateFile(const String& path, size_t size) {
  operations_to_execute.push_back([path, size](MetadataTransactionPtr tx) { tx->truncateFile(path, size); });
}

void DiskObjectStorageTransaction::replaceFile(const std::string& from_path, const std::string& to_path) {
  operations_to_execute.push_back([from_path, to_path](MetadataTransactionPtr tx) { tx->replaceFile(from_path, to_path); });
}

void DiskObjectStorageTransaction::removeFile(const std::string& path) {
  operations_to_execute.push_back(
      [path](MetadataTransactionPtr tx) { tx->unlinkFile(path, /*if_exists=*/false, /*should_remove_objects=*/true); });
}
...
void DiskObjectStorageTransaction::removeSharedRecursive(const std::string& path, bool keep_all_shared_data,
                                                         const NameSet& file_names_remove_metadata_only) {
  if (!keep_all_shared_data && file_names_remove_metadata_only.empty()) {
    operations_to_execute.push_back([path](MetadataTransactionPtr tx) { tx->removeRecursive(path, /*should_remove_objects=*/nullptr); });
  } else {
    operations_to_execute.push_back([path, keep_all_shared_data, file_names_remove_metadata_only](MetadataTransactionPtr tx) {
      tx->removeRecursive(
          path, /*should_remove_objects=*/[keep_all_shared_data, file_names_remove_metadata_only](const std::string& relative_path) {
...
void DiskObjectStorageTransaction::removeDirectory(const std::string& path) {
  operations_to_execute.push_back([path](MetadataTransactionPtr tx) { tx->removeDirectory(path); });
}
...
void DiskObjectStorageTransaction::removeFileIfExists(const std::string& path) {
  operations_to_execute.push_back(
      [path](MetadataTransactionPtr tx) { tx->unlinkFile(path, /*if_exists=*/true, /*should_remove_objects*/ true); });
}

void DiskObjectStorageTransaction::removeSharedFiles(const RemoveBatchRequest& files, bool keep_all_batch_data,
                                                     const NameSet& file_names_remove_metadata_only) {
  for (const auto& [path, if_exists] : files) {
    ...
    operations_to_execute.push_back(
        [path, if_exists, should_remove_objects](MetadataTransactionPtr tx) { tx->unlinkFile(path, if_exists, should_remove_objects); });
  }
}

std::unique_ptr<WriteBufferFromFileBase> DiskObjectStorageTransaction::writeFileWithAutoCommit(const std::string& path, size_t buf_size,
                                                                                               WriteMode mode,
                                                                                               const WriteSettings& settings) {
  return writeFileImpl(/*autocommit=*/true, path, buf_size, mode, settings);
}

std::unique_ptr<WriteBufferFromFileBase> DiskObjectStorageTransaction::writeFile(const std::string& path, size_t buf_size, WriteMode mode,
                                                                                 const WriteSettings& settings) {
  return writeFileImpl(/*autocommit=*/false, path, buf_size, mode, settings);
}

std::unique_ptr<WriteBufferFromFileBase> DiskObjectStorageTransaction::writeFileImpl(bool autocommit, const std::string& path,
                                                                                     size_t buf_size, WriteMode mode,
                                                                                     const WriteSettings& settings) {
  ...
  for (const auto& location : cluster->getEnabledLocations()) {
    ...
    if (location == cluster->getLocalLocation()) {
      ...
      writer = object_storage->writeObject(object,
                                           /// We always use mode Rewrite because we simulate append using metadata and different files
                                           WriteMode::Rewrite,
                                           /*attributes=*/std::nullopt, use_buffer_size, settings);
  ...
  /// tx->commit()
  const auto create_metadata_callback = [disk_tx = shared_from_this(), mode, object, autocommit,
                                         create_blob_if_empty](size_t count) mutable {
    object.bytes_size = count;

    auto missing_locations = disk_tx->cluster->findComplement(disk_tx->cluster->getEnabledLocations());
    disk_tx->operations_to_execute.push_back(
        [object, mode, create_blob_if_empty, blob_replication = std::move(missing_locations)](MetadataTransactionPtr tx) {
          if (mode == WriteMode::Rewrite) {
            if (object.bytes_size > 0 || create_blob_if_empty) {
              LOG_TEST(getLogger("DiskObjectStorageTransaction"), "Writing blob for path {}, key {}, size {}", object.local_path,
                       object.remote_path, object.bytes_size);
              tx->recordBlobsReplication(object, blob_replication);
              tx->createMetadataFile(object.local_path, {object});
            } else {
              LOG_TRACE(getLogger("DiskObjectStorageTransaction"), "Skipping writing empty blob for path {}, key {}", object.local_path,
                        object.remote_path);
              tx->createMetadataFile(object.local_path, {});
            }
          } else {
            if (object.bytes_size > 0 || create_blob_if_empty) tx->recordBlobsReplication(object, blob_replication);

            /// Even if not create_blob_if_empty and size is 0, we still need to add metadata just to make sure that a file gets created if
            /// this is the 1st append
            tx->addBlobToMetadata(object.local_path, object);
          }
        });

    if (autocommit) disk_tx->commit();
  };

  return std::make_unique<WriteBufferWithFinalizeCallback>(std::move(buffer_to_enabled_locations), std::move(create_metadata_callback),
                                                           object.remote_path, create_blob_if_empty);
}
...
void DiskObjectStorageTransaction::writeFileUsingBlobWritingFunction(const String& path, WriteMode mode,
  ...
  if (!objects_namespace.empty()) blob_path.emplace_back(objects_namespace);
...
void DiskObjectStorageTransaction::createHardLink(const std::string& src_path, const std::string& dst_path) {
  operations_to_execute.push_back([src_path, dst_path](MetadataTransactionPtr tx) { tx->createHardLink(src_path, dst_path); });
}

void DiskObjectStorageTransaction::setReadOnly(const std::string& path) {
  operations_to_execute.push_back([path](MetadataTransactionPtr tx) { tx->setReadOnly(path); });
}

void DiskObjectStorageTransaction::setLastModified(const std::string& path, const Poco::Timestamp& timestamp) {
  operations_to_execute.push_back([path, timestamp](MetadataTransactionPtr tx) { tx->setLastModified(path, timestamp); });
}

void DiskObjectStorageTransaction::chmod(const String& path, mode_t mode) {
  operations_to_execute.push_back([path, mode](MetadataTransactionPtr tx) { tx->chmod(path, mode); });
}

void DiskObjectStorageTransaction::createFile(const std::string& path) {
  if (metadata_storage->supportsEmptyFilesWithoutBlobs()) {
    operations_to_execute.push_back([path](MetadataTransactionPtr tx) { tx->createMetadataFile(path, /*objects=*/{}); });
  } else {
...
void DiskObjectStorageTransaction::copyFile(const std::string& from_file_path, const std::string& to_file_path,
  ...
  const auto blobs_to_create =
      blobs_to_copy | std::views::transform([&](const auto& from) {
        return StoredObject(metadata_transaction->generateObjectKeyForPath(to_file_path).serialize(), to_file_path, from.bytes_size);
      }) |
  ...
  for (const auto& location : locations_for_writing) {
    for (const auto [src_blob, dst_blob] : std::views::zip(blobs_to_copy, blobs_to_create)) {
      ...
      object_storages->takePointingTo(local_location)
          ->copyObjectToAnotherObjectStorage(src_blob, dst_blob, read_settings, write_settings, *object_storages->takePointingTo(location));
    }
  }

  operations_to_execute.push_back([blobs_to_create, missing_locations, to_file_path](MetadataTransactionPtr tx) {
    for (const auto& blob : blobs_to_create) tx->recordBlobsReplication(blob, missing_locations);

    tx->createMetadataFile(to_file_path, blobs_to_create);
  });
}

void MultipleDisksObjectStorageTransaction::copyFile(const std::string& from_file_path, const std::string& to_file_path,
                                                     const ReadSettings& read_settings, const WriteSettings& write_settings) {
  ...
  for (const auto& location : locations_for_writing) {
    for (const auto [src_blob, dst_blob] : std::views::zip(blobs_to_copy, blobs_to_create)) {
      written_blobs[location].push_back(dst_blob);
      source_object_storages->takePointingTo(source_local_location)
          ->copyObjectToAnotherObjectStorage(src_blob, dst_blob, read_settings, write_settings, *object_storages->takePointingTo(location));
    }
  }
...
TransactionCommitOutcomeVariant DiskObjectStorageTransaction::tryCommit(const TransactionCommitOptionsVariant& options) {
  for (size_t i = 0; i < operations_to_execute.size(); ++i) {
    try {
      ...
      if (needRollbackBlobs(options)) undo();
  ...
  if (std::get_if<MetaInKeeperCommitOutcome>(&outcome) == nullptr) outcome = metadata_transaction->tryCommit(options);
...
void DiskObjectStorageTransaction::undo() noexcept {
  try {
    for (const auto& [location, blobs] : written_blobs) object_storages->takePointingTo(location)->removeObjectsIfExist(blobs);
  } catch (...) {
