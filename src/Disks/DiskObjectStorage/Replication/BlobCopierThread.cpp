...
IMetadataStorage::BlobsToReplicate findBlobsToReplicate(size_t request_batch, const ClusterConfigurationPtr& cluster,
...
bool replicateBlob(const StoredObject& blob, const Location& from_location, const Location& to_location,
                   const ObjectStorageRouterPtr& object_storages, const ReadSettings& read_settings, const WriteSettings& write_settings,
...
std::vector<std::shared_ptr<ThreadPoolCallbackRunnerLocal<bool>::Task>> scheduleReplicationTasks(
    ThreadPoolCallbackRunnerLocal<bool>& replication_tasks_runner, const IMetadataStorage::BlobsToReplicate& blobs_to_replicate,
    const ObjectStorageRouterPtr& object_storages, const ReadSettings& read_settings, const WriteSettings& write_settings,
    const LoggerPtr& log) noexcept {
  ...
  try {
    for (const auto& [blob, from_location, to_location] : blobs_to_replicate)
      running_replications.push_back(replication_tasks_runner.enqueueAndGiveOwnership(
          [&blob, &from_location, &to_location, &object_storages, &read_settings, &write_settings, &log]() {
...
void recordBlobsReplication(IMetadataStorage::BlobsToReplicate replicated_blobs, const MetadataStoragePtr& metadata_storage,
                            const LoggerPtr& log) noexcept {
  try {
...
void replicateBlobs(IMetadataStorage::BlobsToReplicate blobs_to_replicate, ThreadPoolCallbackRunnerLocal<bool>& replication_tasks_runner,
                    const MetadataStoragePtr& metadata_storage, const ObjectStorageRouterPtr& object_storages,
                    const LoggerPtr& log) noexcept {
  if (blobs_to_replicate.empty()) return;
  ...
  auto replications =
      scheduleReplicationTasks(replication_tasks_runner, blobs_to_replicate, object_storages, read_settings, write_settings, log);
  ...
  for (const auto [replication, task] : std::views::zip(replications, blobs_to_replicate)) {
    ...
    if (is_replicated) replicated_blobs.emplace_back(blob, from_location, to_location);
  }

  recordBlobsReplication(std::move(replicated_blobs), metadata_storage, log);
}

void executeBlobsReplication(int64_t max_to_replicate, ThreadPoolCallbackRunnerLocal<bool>& replication_tasks_runner,
...
BlobCopierThread::BlobCopierThread(std::string disk_name_, ContextPtr context, ClusterConfigurationPtr cluster_,
    ...
    : disk_name(disk_name_),
      ...
      replication_tasks_pool(CurrentMetrics::BlobCopierThreads, CurrentMetrics::BlobCopierThreadsActive,
                             CurrentMetrics::BlobCopierThreadsScheduled, 0, 0, 0),
