...
IMetadataStorage::BlobsToRemove findBlobsToRemove(size_t request_batch, const ClusterConfigurationPtr& cluster,
                                                  const MetadataStoragePtr& metadata_storage, const LoggerPtr& log) noexcept {
  try {
...
std::vector<std::pair<Location, StoredObjects>> sliceIntoRemoveTasks(const IMetadataStorage::BlobsToRemove& blobs_to_remove) noexcept {
  ...
  for (const auto& [blob, locations_cleanup_list] : blobs_to_remove) {
    for (const auto& location : locations_cleanup_list) {
      ...
      if (incomplete_task_blobs.size() >= MAX_BLOBS_IN_TASK) tasks.emplace_back(location, std::exchange(incomplete_task_blobs, {}));
    }
  }

  for (auto& [location, blobs] : incomplete_tasks)
    if (!blobs.empty()) tasks.emplace_back(location, std::move(blobs));

  return tasks;
}

bool removeBlobsBatch(const Location& location, const StoredObjects& remove_batch, const ObjectStorageRouterPtr& object_storages,
...
std::vector<std::shared_ptr<ThreadPoolCallbackRunnerLocal<bool>::Task>> scheduleRemovalTasks(
    ThreadPoolCallbackRunnerLocal<bool>& remove_tasks_runner, const std::vector<std::pair<Location, StoredObjects>>& batches_to_remove,
    const ObjectStorageRouterPtr& object_storages, const LoggerPtr& log) noexcept {
  ...
  try {
    for (const auto& [location, remove_batch] : batches_to_remove)
      running_removals.push_back(remove_tasks_runner.enqueueAndGiveOwnership(
          [&location, &remove_batch, &object_storages, &log]() { return removeBlobsBatch(location, remove_batch, object_storages, log); }));
  } catch (...) {
    tryLogCurrentException(log);
  }
...
void recordBlobsRemoval(StoredObjects removed_blobs, const MetadataStoragePtr& metadata_storage, const LoggerPtr& log) noexcept {
  try {
...
void removeBlobs(IMetadataStorage::BlobsToRemove blobs_to_remove, ThreadPoolCallbackRunnerLocal<bool>& remove_tasks_runner,
                 const MetadataStoragePtr& metadata_storage, const ObjectStorageRouterPtr& object_storages, const LoggerPtr& log) noexcept {
  if (blobs_to_remove.empty()) return;
  ...
  for (auto [removal, task] : std::views::zip(removals, tasks)) {
    ...
    if (is_removed)
      for (const auto& removed_blob : remove_batch) blobs_to_remove[removed_blob].erase(location);
  }
  ...
  for (const auto& [blob, locations_cleanup_list] : blobs_to_remove)
    if (locations_cleanup_list.empty()) blobs_removed_from_all_locations.push_back(blob);

  recordBlobsRemoval(std::move(blobs_removed_from_all_locations), metadata_storage, log);
}

void executeBlobsCleanup(size_t max_to_remove, ThreadPoolCallbackRunnerLocal<bool>& remove_tasks_runner,
                         const ClusterConfigurationPtr& cluster, const MetadataStoragePtr& metadata,
                         const ObjectStorageRouterPtr& object_storages, const LoggerPtr& log) noexcept {
...
BlobKillerThread::BlobKillerThread(std::string disk_name, ContextPtr context, ClusterConfigurationPtr cluster_,
                                   MetadataStoragePtr metadata_storage_, ObjectStorageRouterPtr object_storages_)
    : cluster(std::move(cluster_)),
      metadata_storage(std::move(metadata_storage_)),
      object_storages(std::move(object_storages_)),
      log(getLogger(fmt::format("{}::BlobKillerThread", disk_name))),
      remove_tasks_pool(CurrentMetrics::BlobKillerThreads, CurrentMetrics::BlobKillerThreadsActive,
                        CurrentMetrics::BlobKillerThreadsScheduled, 0, 0, 0),
