...
ClusterConfiguration::ClusterConfiguration(std::unordered_map<Location, LocationInfo> locations_) : locations(std::move(locations_)) {
  LOG_DEBUG(getLogger("ClusterConfiguration"), "Cluster Configuration: {}",
            locations | std::views::keys | std::ranges::to<std::vector<std::string>>());
}
...
bool ClusterConfiguration::areAllLocationsEnabled(const LocationSet& to_check) const {
  ...
  for (const auto& location : to_check)
    if (!locations.at(location).enabled) return false;
...
bool ClusterConfiguration::isLocalLocationIn(const Locations& to_check) const {
  ...
  for (const auto& location : to_check)
    if (locations.at(location).local) return true;
...
Locations ClusterConfiguration::findComplement(LocationSet redacted, bool use_only_enabled) const {
  ...
  for (const auto& [location, info] : locations)
    if (!redacted.contains(location))
      if (!use_only_enabled || info.enabled) result.push_back(location);
...
Locations ClusterConfiguration::getEnabledLocations() const {
  ...
  Locations result;
  for (const auto& [location, info] : locations)
    if (info.enabled) result.push_back(location);
