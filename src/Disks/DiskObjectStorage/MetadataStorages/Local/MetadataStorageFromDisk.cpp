...
MetadataStorageFromDisk::MetadataStorageFromDisk(DiskPtr disk_, std::string compatible_key_prefix_,
                                                 ObjectStorageKeyGeneratorPtr key_generator_)
    : disk(disk_), compatible_key_prefix(std::move(compatible_key_prefix_)), key_generator(std::move(key_generator_)) {}

const std::string &MetadataStorageFromDisk::getPath() const { return disk->getPath(); }

bool MetadataStorageFromDisk::existsFile(const std::string &path) const { return disk->existsFile(path); }

bool MetadataStorageFromDisk::existsDirectory(const std::string &path) const { return disk->existsDirectory(path); }

bool MetadataStorageFromDisk::existsFileOrDirectory(const std::string &path) const { return disk->existsFileOrDirectory(path); }

Poco::Timestamp MetadataStorageFromDisk::getLastModified(const std::string &path) const { return disk->getLastModified(path); }

time_t MetadataStorageFromDisk::getLastChanged(const std::string &path) const { return disk->getLastChanged(path); }

uint64_t MetadataStorageFromDisk::getFileSize(const String &path) const { return getTotalSize(readMetadata(path)->objects); }
...
DirectoryIteratorPtr MetadataStorageFromDisk::iterateDirectory(const std::string &path) const { return disk->iterateDirectory(path); }
...
std::string MetadataStorageFromDisk::readInlineDataToString(const std::string &path) const { return readMetadata(path)->inline_data; }
...
MetadataTransactionPtr MetadataStorageFromDisk::createTransaction() { return std::make_shared<MetadataStorageFromDiskTransaction>(*this); }

bool MetadataStorageFromDisk::supportWritingWithAppend() const { return true; }

StoredObjects MetadataStorageFromDisk::getStorageObjects(const std::string &path) const { return readMetadata(path)->objects; }

uint32_t MetadataStorageFromDisk::getHardlinkCount(const std::string &path) const { return readMetadata(path)->ref_count; }
...
void MetadataStorageFromDiskTransaction::writeInlineDataToFile(const std::string &path, const std::string &data) {
  operations.addOperation(
      std::make_unique<WriteInlineDataOperation>(path, data, metadata_storage.compatible_key_prefix, *metadata_storage.getDisk()));
}
...
void MetadataStorageFromDiskTransaction::unlinkFile(const std::string &path, bool if_exists, bool should_remove_objects) {
  operations.addOperation(std::make_unique<UnlinkFileOperation>(
      path, if_exists, should_remove_objects, metadata_storage.compatible_key_prefix, *metadata_storage.getDisk(), objects_to_remove));
}
...
void MetadataStorageFromDiskTransaction::removeRecursive(const std::string &path,
  ...
  operations.addOperation(std::make_unique<RemoveRecursiveOperation>(path, should_remove_objects, metadata_storage.compatible_key_prefix,
                                                                     *metadata_storage.getDisk(), objects_to_remove));
}

void MetadataStorageFromDiskTransaction::createHardLink(const std::string &path_from, const std::string &path_to) {
  operations.addOperation(
      std::make_unique<CreateHardlinkOperation>(path_from, path_to, metadata_storage.compatible_key_prefix, *metadata_storage.disk));
}
...
void MetadataStorageFromDiskTransaction::replaceFile(const std::string &path_from, const std::string &path_to) {
  operations.addOperation(std::make_unique<ReplaceFileOperation>(path_from, path_to, metadata_storage.compatible_key_prefix,
                                                                 *metadata_storage.getDisk(), objects_to_remove));
}

void MetadataStorageFromDiskTransaction::setReadOnly(const std::string &path) {
  operations.addOperation(
      std::make_unique<SetReadonlyFileOperation>(path, metadata_storage.compatible_key_prefix, *metadata_storage.getDisk()));
}

void MetadataStorageFromDiskTransaction::createMetadataFile(const std::string &path, const StoredObjects &objects) {
  operations.addOperation(std::make_unique<RewriteFileOperation>(path, objects, metadata_storage.compatible_key_prefix,
                                                                 *metadata_storage.disk, objects_to_remove));
}
...
void MetadataStorageFromDiskTransaction::truncateFile(const std::string &src_path, size_t target_size) {
  operations.addOperation(std::make_unique<TruncateMetadataFileOperation>(src_path, target_size, metadata_storage.compatible_key_prefix,
                                                                          *metadata_storage.getDisk(), objects_to_remove));
}
...
StoredObjects MetadataStorageFromDiskTransaction::getSubmittedForRemovalBlobs() { return objects_to_remove; }
