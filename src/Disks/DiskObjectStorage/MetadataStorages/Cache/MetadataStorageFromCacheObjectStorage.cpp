...
MetadataStoragePtr MetadataStorageFromCacheObjectStorage::getUnderlying() const { return underlying; }
...
const std::string& MetadataStorageFromCacheObjectStorage::getPath() const { return underlying->getPath(); }

MetadataStorageType MetadataStorageFromCacheObjectStorage::getType() const { return underlying->getType(); }

std::string MetadataStorageFromCacheObjectStorage::getZooKeeperName() const { return underlying->getZooKeeperName(); }

std::string MetadataStorageFromCacheObjectStorage::getZooKeeperPath() const { return underlying->getZooKeeperPath(); }

bool MetadataStorageFromCacheObjectStorage::supportsEmptyFilesWithoutBlobs() const { return underlying->supportsEmptyFilesWithoutBlobs(); }

bool MetadataStorageFromCacheObjectStorage::areBlobPathsRandom() const { return underlying->areBlobPathsRandom(); }

bool MetadataStorageFromCacheObjectStorage::existsFile(const std::string& path) const { return underlying->existsFile(path); }

bool MetadataStorageFromCacheObjectStorage::existsDirectory(const std::string& path) const { return underlying->existsDirectory(path); }
...
uint64_t MetadataStorageFromCacheObjectStorage::getFileSize(const std::string& path) const { return underlying->getFileSize(path); }
...
time_t MetadataStorageFromCacheObjectStorage::getLastChanged(const std::string& path) const { return underlying->getLastChanged(path); }

bool MetadataStorageFromCacheObjectStorage::supportsChmod() const { return underlying->supportsChmod(); }

bool MetadataStorageFromCacheObjectStorage::supportsStat() const { return underlying->supportsStat(); }

struct stat MetadataStorageFromCacheObjectStorage::stat(const String& path) const { return underlying->stat(path); }
...
bool MetadataStorageFromCacheObjectStorage::isDirectoryEmpty(const std::string& path) const { return underlying->isDirectoryEmpty(path); }
...
void MetadataStorageFromCacheObjectStorage::startup() { underlying->startup(); }

void MetadataStorageFromCacheObjectStorage::shutdown() { underlying->shutdown(); }
...
std::unordered_map<std::string, std::string> MetadataStorageFromCacheObjectStorage::getSerializedMetadata(
    const std::vector<String>& file_paths) const {
...
bool MetadataStorageFromCacheObjectStorage::isReadOnly() const { return underlying->isReadOnly(); }

bool MetadataStorageFromCacheObjectStorage::isTransactional() const { return underlying->isTransactional(); }

bool MetadataStorageFromCacheObjectStorage::isPlain() const { return underlying->isPlain(); }

bool MetadataStorageFromCacheObjectStorage::isWriteOnce() const { return underlying->isWriteOnce(); }

bool MetadataStorageFromCacheObjectStorage::supportWritingWithAppend() const { return underlying->supportWritingWithAppend(); }

IMetadataStorage::BlobsToRemove MetadataStorageFromCacheObjectStorage::getBlobsToRemove(const ClusterConfigurationPtr& cluster,
                                                                                        int64_t max_count) {
  std::lock_guard guard(removed_objects_mutex);

  if (max_count == 0) max_count = std::numeric_limits<int64_t>::max();

  BlobsToRemove blobs_to_remove;
  for (const auto& blob : objects_to_remove | std::views::take(max_count)) blobs_to_remove[blob] = {cluster->getLocalLocation()};
...
int64_t MetadataStorageFromCacheObjectStorage::recordAsRemoved(const StoredObjects& blobs) {
  ...
  for (const auto& removed_blob : blobs) recorded_count += objects_to_remove.erase(removed_blob);

  return recorded_count;
}

IMetadataStorage::BlobsToReplicate MetadataStorageFromCacheObjectStorage::getBlobsToReplicate(const ClusterConfigurationPtr& cluster,
                                                                                              int64_t max_count) {
...
void MetadataStorageFromCacheObjectStorage::updateCache(const std::vector<std::string>& paths, bool recursive, bool enforce_fresh,
...
void MetadataStorageFromCacheObjectStorage::invalidateCache(const std::string& path) { underlying->invalidateCache(path); }

void MetadataStorageFromCacheObjectStorage::dropCache() { underlying->dropCache(); }

void MetadataStorageFromCacheObjectStorage::applyNewSettings(const Poco::Util::AbstractConfiguration& config,
                                                             const std::string& config_prefix, ContextPtr context) {
  underlying->applyNewSettings(config, config_prefix, context);
}

MetadataStorageFromCacheObjectStorageTransaction::MetadataStorageFromCacheObjectStorageTransaction(
    MetadataTransactionPtr underlying_, MetadataStorageFromCacheObjectStorage& metadata_storage_)
    : underlying(std::move(underlying_)), metadata_storage(metadata_storage_) {}
...
TransactionCommitOutcomeVariant MetadataStorageFromCacheObjectStorageTransaction::tryCommit(
...
bool MetadataStorageFromCacheObjectStorageTransaction::supportsChmod() const { return underlying->supportsChmod(); }

void MetadataStorageFromCacheObjectStorageTransaction::chmod(const String& path, mode_t mode) { underlying->chmod(path, mode); }

void MetadataStorageFromCacheObjectStorageTransaction::setReadOnly(const std::string& path) { underlying->setReadOnly(path); }
...
void MetadataStorageFromCacheObjectStorageTransaction::createDirectory(const std::string& path) { underlying->createDirectory(path); }
...
void MetadataStorageFromCacheObjectStorageTransaction::removeDirectory(const std::string& path) { underlying->removeDirectory(path); }

void MetadataStorageFromCacheObjectStorageTransaction::removeRecursive(const std::string& path,
...
void MetadataStorageFromCacheObjectStorageTransaction::recordBlobsReplication(const StoredObject& blob,
                                                                              const Locations& missing_locations) {
