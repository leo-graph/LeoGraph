...
MetadataStorageFromStaticFilesWebServer::MetadataStorageFromStaticFilesWebServer(const WebObjectStorage& object_storage_)
    : object_storage(object_storage_) {}

MetadataTransactionPtr MetadataStorageFromStaticFilesWebServer::createTransaction() { throwNotImplemented(); }
...
bool MetadataStorageFromStaticFilesWebServer::existsFileOrDirectory(const std::string& path) const { return object_storage.exists(path); }
...
std::optional<StoredObjects> MetadataStorageFromStaticFilesWebServer::getStorageObjectsIfExist(const std::string& path) const {
  ...
  if (auto file_info = object_storage.tryGetFileInfo(path)) return StoredObjects{StoredObject(remote_path, path, file_info->size)};
  return std::nullopt;
}
...
std::vector<std::string> MetadataStorageFromStaticFilesWebServer::listDirectory(const std::string& path) const {
  ...
  for (const auto& [file_path, _] : object_storage.files) {
    if (file_path.starts_with(path)) result.push_back(file_path);  /// It looks more like recursive listing, not sure it is right
  }
  return result;
}
...
DirectoryIteratorPtr MetadataStorageFromStaticFilesWebServer::iterateDirectory(const std::string& path) const {
  ...
  if (!existsDirectory(path)) return std::make_unique<StaticDirectoryIterator>(std::move(dir_file_paths));
