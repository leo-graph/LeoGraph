...
[[maybe_unused]] static bool isPathRelative(const std::filesystem::path& path) {
  for (const auto& step : path)
    if (step.string() == "..") return true;

  return false;
}

NormalizedPath NormalizedPath::parent_path() const { return NormalizedPath{std::filesystem::path::parent_path()}; }
...
NormalizedPath normalizePath(std::string path) {
  ...
  while (normalized_path.ends_with('/') || normalized_path.ends_with('.')) normalized_path.remove_suffix(1);

  while (normalized_path.starts_with('/') || normalized_path.starts_with('.')) normalized_path.remove_prefix(1);
