...
std::string getObjectKeyCompatiblePrefix(const ObjectStoragePtr &object_storage, const Poco::Util::AbstractConfiguration &config,
...
void MetadataStorageFactory::registerMetadataStorageType(const std::string &metadata_type, Creator creator) {
  if (!registry.emplace(metadata_type, creator).second) {
    throw Exception(ErrorCodes::LOGICAL_ERROR, "MetadataStorageFactory: the metadata type '{}' is not unique", metadata_type);
  }
}

std::string MetadataStorageFactory::getCompatibilityMetadataTypeHint(const ClusterConfigurationPtr &cluster,
...
std::string MetadataStorageFactory::getMetadataType(const Poco::Util::AbstractConfiguration &config, const std::string &config_prefix,
...
MetadataStoragePtr MetadataStorageFactory::create(const std::string &name, const Poco::Util::AbstractConfiguration &config,
                                                  const std::string &config_prefix, const ClusterConfigurationPtr &cluster,
                                                  const ObjectStorageRouterPtr &object_storages,
  ...
  if (it == registry.end()) {
    throw Exception(ErrorCodes::UNKNOWN_ELEMENT_IN_CONFIG, "MetadataStorageFactory: unknown metadata storage type: {}", type);
  }
...
void registerMetadataStorageFromDisk(MetadataStorageFactory &factory) {
  factory.registerMetadataStorageType(
      ...
      [](const std::string &name, const Poco::Util::AbstractConfiguration &config, const std::string &config_prefix,
         const ClusterConfigurationPtr &cluster, const ObjectStorageRouterPtr &object_storages) -> MetadataStoragePtr {
...
void registerMetadataStorageFromKeeper(MetadataStorageFactory &factory) {
  factory.registerMetadataStorageType(
      ...
      [](const std::string &name, const Poco::Util::AbstractConfiguration &config, const std::string &config_prefix,
        ...
        return std::make_shared<MetadataStorageFromKeeper>(name, zookeeper_name, keeper_prefix, key_compatibility_prefix, key_generator,
                                                           config, config_prefix, Context::getGlobalContextInstance());
      });
}
#endif
...
void registerPlainMetadataStorage(MetadataStorageFactory &factory) {
  factory.registerMetadataStorageType(
      ...
      [](const std::string & /* name */, const Poco::Util::AbstractConfiguration &config, const std::string &config_prefix,
        ...
        return std::make_shared<MetadataStorageFromPlainObjectStorage>(local_object_storage, key_compatibility_prefix,
                                                                       object_metadata_cache_size);
      });
}
...
void MetadataStorageFactory::clearRegistry() { registry.clear(); }
}  // namespace DB
