...
DiskObjectStorageMetadata::DiskObjectStorageMetadata(std::string compatible_key_prefix_, std::string metadata_file_path_)
    : compatible_key_prefix(std::move(compatible_key_prefix_)), metadata_file_path(std::move(metadata_file_path_)) {}
...
void DiskObjectStorageMetadata::deserialize(ReadBuffer& buf) {
  ...
  if (version < VERSION_ABSOLUTE_PATHS || version > VERSION_FULL_OBJECT_KEY)
    throw Exception(ErrorCodes::UNKNOWN_FORMAT, "Unknown metadata file version. Path: {}. Version: {}. Maximum expected version: {}",
    ...
    if (version == VERSION_ABSOLUTE_PATHS) {
      if (!remote_path.starts_with(compatible_key_prefix))
        throw Exception(ErrorCodes::UNKNOWN_FORMAT,
                        "Remote path in metadata does not correspond to root path. Path: {}, Root path: {}, Metadata path: {}", remote_path,
                        compatible_key_prefix, metadata_file_path);
...
void DiskObjectStorageMetadata::deserializeFromString(const std::string& data) try {
...
bool DiskObjectStorageMetadata::tryDeserializeFromString(const std::string& data) noexcept try {
  ReadBufferFromString buf(data);
  deserialize(buf);
  return true;
