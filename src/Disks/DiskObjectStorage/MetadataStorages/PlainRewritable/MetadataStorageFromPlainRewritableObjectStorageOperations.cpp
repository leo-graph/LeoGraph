...
MetadataStorageFromPlainObjectStorageCreateDirectoryOperation::MetadataStorageFromPlainObjectStorageCreateDirectoryOperation(
    bool recursive_, std::filesystem::path path_, std::string directory_remote_path_, std::shared_ptr<IObjectStorage> object_storage_,
    std::shared_ptr<InMemoryDirectoryTree> fs_tree_, std::shared_ptr<PlainRewritableLayout> layout_,
...
void MetadataStorageFromPlainObjectStorageCreateDirectoryOperation::execute() {
  ...
  if (fs_tree->existsFile(path)) throw Exception(ErrorCodes::CANNOT_CREATE_DIRECTORY, "File '{}' already exists", path.parent_path());
  ...
  if (exists_directory)
    LOG_TRACE(getLogger("MetadataStorageFromPlainObjectStorageCreateDirectoryOperation"),
              "Materializing virtual directory '{}' with remote path='{}'", path, metadata_object_key);
  else
    LOG_TRACE(getLogger("MetadataStorageFromPlainObjectStorageCreateDirectoryOperation"),
              "Creating metadata for directory '{}' with remote path='{}'", path, metadata_object_key);
...
void MetadataStorageFromPlainObjectStorageCreateDirectoryOperation::undo() {
  ...
  if (created_directory) fs_tree->unlinkTree(path);
...
MetadataStorageFromPlainObjectStorageMoveDirectoryOperation::MetadataStorageFromPlainObjectStorageMoveDirectoryOperation(
    std::filesystem::path path_from_, std::filesystem::path path_to_, std::shared_ptr<IObjectStorage> object_storage_,
    std::shared_ptr<InMemoryDirectoryTree> fs_tree_, std::shared_ptr<PlainRewritableLayout> layout_,
    std::shared_ptr<PlainRewritableMetrics> metrics_)
...
std::unique_ptr<WriteBufferFromFileBase> MetadataStorageFromPlainObjectStorageMoveDirectoryOperation::createWriteBuf(
  ...
  if (expected_content) {
    ...
    if (data != expected_content.value())
      throw Exception(ErrorCodes::INCORRECT_DATA, "Incorrect data for object key {}, expected {}, got {}", metadata_object_key,
                      expected_content.value(), data);
  }

  auto write_buf = object_storage->writeObject(metadata_object, WriteMode::Rewrite,
                                               /*object_attributes*/ std::nullopt,
                                               /*buf_size*/ 128,
                                               /*settings*/ getWriteSettings());
...
void MetadataStorageFromPlainObjectStorageMoveDirectoryOperation::rewriteSingleDirectory(const std::filesystem::path& from,
...
void MetadataStorageFromPlainObjectStorageMoveDirectoryOperation::execute() {
  ...
  for (const auto& [subdir, remote_info] : from_tree_info) {
    ...
    if (!remote_info.has_value()) {
      LOG_TRACE(getLogger("MetadataStorageFromPlainObjectStorageMoveDirectoryOperation"),
                "Directory '{}' is virtual will not update in object storage", sub_path_from);
      continue;
    }

    auto write_buf =
        createWriteBuf(remote_info.value(), /*expected_content*/ validate_content ? std::make_optional(sub_path_from) : std::nullopt);
...
void MetadataStorageFromPlainObjectStorageMoveDirectoryOperation::undo() {
  LOG_TRACE(getLogger("MetadataStorageFromPlainObjectStorageMoveDirectoryOperation"), "Reversing directory move from '{}' to '{}'",
            path_from, path_to);

  if (moved_in_memory) fs_tree->moveDirectory(path_to, path_from);

  for (const auto& [subdir, remote_info] : from_tree_info) {
    auto sub_path_to = path_to / subdir / "";
    auto sub_path_from = path_from / subdir / "";

    if (!changed_paths.contains(sub_path_from)) continue;
...
MetadataStorageFromPlainObjectStorageRemoveDirectoryOperation::MetadataStorageFromPlainObjectStorageRemoveDirectoryOperation(
    std::filesystem::path path_, std::shared_ptr<IObjectStorage> object_storage_, std::shared_ptr<InMemoryDirectoryTree> fs_tree_,
    std::shared_ptr<PlainRewritableLayout> layout_, std::shared_ptr<PlainRewritableMetrics> metrics_)
    : path(std::move(path_)),
      object_storage(std::move(object_storage_)),
      fs_tree(std::move(fs_tree_)),
      layout(std::move(layout_)),
...
MetadataStorageFromPlainObjectStorageWriteFileOperation::MetadataStorageFromPlainObjectStorageWriteFileOperation(
    std::string path_, StoredObject object_, std::shared_ptr<IObjectStorage> object_storage_,
    std::shared_ptr<InMemoryDirectoryTree> fs_tree_, std::shared_ptr<PlainRewritableLayout> layout_,
    std::shared_ptr<PlainRewritableMetrics> metrics_)
    : path(std::move(path_)),
...
void MetadataStorageFromPlainObjectStorageWriteFileOperation::execute() {
  LOG_TEST(getLogger("MetadataStorageFromPlainObjectStorageWriteFileOperation"), "Creating metadata for a file '{}', size: {}", path,
           object.bytes_size);
...
MetadataStorageFromPlainObjectStorageUnlinkMetadataFileOperation::MetadataStorageFromPlainObjectStorageUnlinkMetadataFileOperation(
    std::filesystem::path path_, bool if_exists_, std::shared_ptr<IObjectStorage> object_storage_,
    std::shared_ptr<InMemoryDirectoryTree> fs_tree_, std::shared_ptr<PlainRewritableLayout> layout_,
    std::shared_ptr<PlainRewritableMetrics> metrics_, StoredObjects& removed_objects_)
    : path(std::move(path_)),
...
void MetadataStorageFromPlainObjectStorageUnlinkMetadataFileOperation::execute() {
  LOG_TEST(getLogger("MetadataStorageFromPlainObjectStorageUnlinkMetadataFileOperation"), "Unlinking metadata for a write '{}'", path);
...
void MetadataStorageFromPlainObjectStorageUnlinkMetadataFileOperation::undo() {
  ...
  if (remove_finished) fs_tree->recordFile(path, std::move(file_remote_info.value()));
}
...
void MetadataStorageFromPlainObjectStorageUnlinkMetadataFileOperation::finalize() {
  ...
  if (copy_started) object_storage->removeObjectIfExists(StoredObject(remote_tmp_path));
}
...
void MetadataStorageFromPlainObjectStorageCopyFileOperation::undo() {
  if (!copy_attempted) return;

  LOG_WARNING(getLogger("MetadataStorageFromPlainObjectStorageCopyFileOperation"), "Removing file '{}' that was copied from '{}", path_to,
              path_from);

  object_storage->removeObjectIfExists(StoredObject(remote_path_to));

  if (fs_tree->existsFile(path_to)) fs_tree->removeFile(path_to);
}

MetadataStorageFromPlainObjectStorageMoveFileOperation::MetadataStorageFromPlainObjectStorageMoveFileOperation(
    bool replaceable_, std::filesystem::path path_from_, std::filesystem::path path_to_, std::shared_ptr<IObjectStorage> object_storage_,
    std::shared_ptr<InMemoryDirectoryTree> fs_tree_, std::shared_ptr<PlainRewritableLayout> layout_,
    std::shared_ptr<PlainRewritableMetrics> metrics_, StoredObjects& removed_objects_)
...
void MetadataStorageFromPlainObjectStorageMoveFileOperation::execute() {
  LOG_TEST(getLogger("MetadataStorageFromPlainObjectStorageMoveFileOperation"), "Moving file (replaceable = {}) from '{}' to '{}'",
           replaceable, path_from, path_to);
  ...
  if (fs_tree->existsFile(path_to)) {
    if (!replaceable) throw Exception(ErrorCodes::FILE_ALREADY_EXISTS, "Target file '{}' already exists", path_to);
    ...
    object_storage->copyObject(
        ...
        /*object_to=*/StoredObject(tmp_remote_path_to), read_settings, write_settings);
...

  {
    ...
    object_storage->copyObject(
        ...
        /*object_to=*/StoredObject(tmp_remote_path_from), read_settings, write_settings);
...
void MetadataStorageFromPlainObjectStorageMoveFileOperation::undo() {
  if (moved_file) fs_tree->recordFile(path_from, file_from_remote_info.value());

  const auto read_settings = getReadSettings();
  const auto write_settings = getWriteSettings();

  if (moved_file) {
    LOG_WARNING(getLogger("MetadataStorageFromPlainObjectStorageMoveFileOperation"),
                "Removing file '{}' that was moved (replaceable = {}) from '{}", path_to, replaceable, path_from);
  ...
  if (moved_existing_source_file) {
    object_storage->copyObject(
        ...
        /*object_to=*/StoredObject(remote_path_from), read_settings, write_settings);
  ...
  if (moved_existing_target_file) {
    object_storage->copyObject(
        ...
        /*object_to=*/StoredObject(remote_path_to), read_settings, write_settings);
...
void MetadataStorageFromPlainObjectStorageMoveFileOperation::finalize() {
  ...
  if (moved_existing_source_file) object_storage->removeObjectIfExists(StoredObject(tmp_remote_path_from));

  if (moved_existing_target_file) object_storage->removeObjectIfExists(StoredObject(tmp_remote_path_to));
}
...
MetadataStorageFromPlainObjectStorageRemoveRecursiveOperation::MetadataStorageFromPlainObjectStorageRemoveRecursiveOperation(
    std::filesystem::path path_, std::shared_ptr<IObjectStorage> object_storage_, std::shared_ptr<InMemoryDirectoryTree> fs_tree_,
    std::shared_ptr<PlainRewritableLayout> layout_, std::shared_ptr<PlainRewritableMetrics> metrics_, StoredObjects& removed_objects_)
    : path(std::move(path_)),
      object_storage(std::move(object_storage_)),
      fs_tree(std::move(fs_tree_)),
      layout(std::move(layout_)),
  ...
  move_to_tmp_op = std::make_unique<MetadataStorageFromPlainObjectStorageMoveDirectoryOperation>(path / "", tmp_path / "", object_storage,
                                                                                                 fs_tree, layout, metrics);
}
...
void MetadataStorageFromPlainObjectStorageRemoveRecursiveOperation::execute() {
  ...
  if (normalizePath(path).empty()) return;
...
void MetadataStorageFromPlainObjectStorageRemoveRecursiveOperation::finalize() {
  ...
  for (const auto& [subdir, remote_info] : fs_tree->getSubtreeRemoteInfo(tmp_path)) {
    ...
    for (const auto& child : fs_tree->listDirectory(subdir_path)) {
      ...
      if (!fs_tree->existsFile(file_path)) continue;
