...
struct InMemoryDirectoryTree::INode {
  ...
  explicit INode(std::shared_ptr<INode> parent_, std::string last_directory_name_)
      : parent(parent_), last_directory_name(std::move(last_directory_name_)) {}

  bool isVirtual() const { return !remote_info.has_value(); }

  bool isRoot() const { return parent.lock() == nullptr; }
};
...
void InMemoryDirectoryTree::traverseSubtree(const NormalizedPath &path, const ObserveFunction &observe) const {
  ...
  if (auto start_node = walk(path)) unvisited.emplace_back("", std::move(start_node));
  ...
  while (!unvisited.empty()) {
    ...
    for (const auto &[subdir, subnode] : node->subdirectories) unvisited.emplace_back(node_path / subdir, subnode);
  }
}
...
NormalizedPath InMemoryDirectoryTree::determineNodePath(std::shared_ptr<INode> node) const TSA_REQUIRES(mutex) {
  ...
  fs::path nodes_path;
  for (const auto &step : path_parts | std::views::reverse) nodes_path /= step;
...
std::shared_ptr<InMemoryDirectoryTree::INode> InMemoryDirectoryTree::trimDanglingVirtualPath(std::shared_ptr<INode> node) {
  while (!node->isRoot() && node->isVirtual()) {
    if (!node->subdirectories.empty()) break;
...
std::optional<std::pair<std::string, DirectoryRemoteInfo>> InMemoryDirectoryTree::lookupDirectoryIfNotChanged(
    const std::string &remote_path, const std::string &etag) const {
  std::lock_guard guard(mutex);
  const auto it = remote_path_to_inode.find(remote_path);
  if (it == remote_path_to_inode.end()) return std::nullopt;

  const auto &inode = it->second;
  if (!inode->remote_info.has_value()) return std::nullopt;

  if (inode->remote_info->etag != etag) return std::nullopt;

  return std::make_pair(determineNodePath(inode), inode->remote_info.value());
}

std::unordered_map<std::string, std::optional<DirectoryRemoteInfo>> InMemoryDirectoryTree::getSubtreeRemoteInfo(
...
std::optional<DirectoryRemoteInfo> InMemoryDirectoryTree::getDirectoryRemoteInfo(const std::string &path) const {
  ...
  if (inode->isVirtual()) return std::nullopt;
...
std::optional<FileRemoteInfo> InMemoryDirectoryTree::getFileRemoteInfo(const std::string &path) const {
  ...
  if (!directory_remote_info->files.contains(normalized_path.filename())) return std::nullopt;
...
void InMemoryDirectoryTree::recordDirectoryPath(const std::string &path, DirectoryRemoteInfo info) {
  ...
  if (!inode->isVirtual()) throw Exception(ErrorCodes::LOGICAL_ERROR, "Directory '{}' was already recorded", normalized_path.string());
...
void InMemoryDirectoryTree::recordFile(const std::string &path, FileRemoteInfo info) {
  std::lock_guard guard(mutex);
  const auto normalized_path = normalizePath(path);
  const auto inode = walk(normalized_path.parent_path());

  if (!inode) throw Exception(ErrorCodes::LOGICAL_ERROR, "Directory '{}' does not exist", normalized_path.string());

  if (inode->isVirtual()) throw Exception(ErrorCodes::LOGICAL_ERROR, "Creation of a file under the virtual directory is not possible");

  if (inode->subdirectories.contains(normalized_path.filename()))
    throw Exception(ErrorCodes::LOGICAL_ERROR, "There is a subdirectory '{}' under the path '{}'. Can't create file",
                    normalized_path.filename().string(), normalized_path.parent_path().string());
...
void InMemoryDirectoryTree::unlinkTree(const std::string &path) {
  ...
  if (inode->isRoot()) throw Exception(ErrorCodes::LOGICAL_ERROR, "Directory '{}' is root", normalized_path.string());

  traverseSubtree(normalized_path, [&](const std::string &, const std::shared_ptr<const INode> &node) TSA_REQUIRES(mutex) {
    if (!node->remote_info.has_value()) return;

    remote_path_to_inode.erase(node->remote_info->remote_path);
    remote_layout_files_count.sub(node->remote_info->files.size());
...
void InMemoryDirectoryTree::moveDirectory(const std::string &from, const std::string &to) {
  ...
  if (!inode_from) throw Exception(ErrorCodes::LOGICAL_ERROR, "Directory '{}' does not exist", normalized_from.string());

  if (inode_from->isRoot()) throw Exception(ErrorCodes::LOGICAL_ERROR, "Directory '{}' is root", normalized_from.string());
  ...
  if (inode_to_parent->subdirectories.contains(normalized_to.filename()))
    throw Exception(ErrorCodes::LOGICAL_ERROR, "There is a subdirectory '{}' under the path '{}', can't move",
                    normalized_to.filename().string(), normalized_to.parent_path().string());

  if (!inode_to_parent->isVirtual())
    if (inode_to_parent->remote_info->files.contains(normalized_to.filename()))
      throw Exception(ErrorCodes::LOGICAL_ERROR, "There is a file '{}' under the path '{}', can't move", normalized_to.filename().string(),
                      normalized_to.parent_path().string());
...
std::vector<std::string> InMemoryDirectoryTree::listDirectory(const std::string &path) const {
  ...
  if (!inode->isVirtual()) result.append_range(inode->remote_info->files | std::views::keys);
...
std::pair<bool, std::optional<DirectoryRemoteInfo>> InMemoryDirectoryTree::existsDirectory(const std::string &path) const {
  ...
  if (!inode) return {false, std::nullopt};

  if (inode->isVirtual()) return {true, std::nullopt};
...
bool InMemoryDirectoryTree::existsFile(const std::string &path) const {
  ...
  if (inode->isVirtual()) return false;

  return inode->remote_info->files.contains(normalized_path.filename());
}
...
void InMemoryDirectoryTree::removeFile(const std::string &path) {
  ...
  if (inode->isVirtual()) throw Exception(ErrorCodes::LOGICAL_ERROR, "Removal of a file under the virtual directory is not possible");

  if (!inode->remote_info->files.contains(normalized_path.filename()))
    throw Exception(ErrorCodes::LOGICAL_ERROR, "File '{}' does not exist", normalized_path.string());

  inode->remote_info->files.erase(normalized_path.filename());
