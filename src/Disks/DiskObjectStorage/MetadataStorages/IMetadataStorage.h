...
class IMetadataTransaction : private boost::noncopyable {
 public:
  ...
  virtual void writeStringToFile(const std::string& /* path */, const std::string& /* data */) { throwNotImplemented(); }

  /// Writes the data inline with the metadata
  virtual void writeInlineDataToFile(const std::string& /* path */, const std::string& /* data */) { throwNotImplemented(); }

  virtual void setLastModified(const std::string& /* path */, const Poco::Timestamp& /* timestamp */) { throwNotImplemented(); }

  virtual bool supportsChmod() const = 0;
  virtual void chmod(const String& /* path */, mode_t /* mode */) { throwNotImplemented(); }

  virtual void setReadOnly(const std::string& /* path */) { throwNotImplemented(); }

  virtual void unlinkFile(const std::string& /* path */, bool /* if_exists */, bool /* should_remove_objects */) { throwNotImplemented(); }

  virtual void createDirectory(const std::string& /* path */) { throwNotImplemented(); }

  virtual void createDirectoryRecursive(const std::string& /* path */) { throwNotImplemented(); }

  virtual void removeDirectory(const std::string& /* path */) { throwNotImplemented(); }
  ...
  virtual void createHardLink(const std::string& /* path_from */, const std::string& /* path_to */) { throwNotImplemented(); }

  virtual void moveFile(const std::string& /* path_from */, const std::string& /* path_to */) { throwNotImplemented(); }

  virtual void moveDirectory(const std::string& /* path_from */, const std::string& /* path_to */) { throwNotImplemented(); }

  virtual void replaceFile(const std::string& /* path_from */, const std::string& /* path_to */) { throwNotImplemented(); }
  ...
  virtual void recordBlobsReplication(const StoredObject& /*blob*/, const Locations& missing_locations) {
    if (!missing_locations.empty()) throwNotImplemented();
  }
  ...
  virtual void addBlobToMetadata(const std::string& /* path */, const StoredObject& /* object */) { throwNotImplemented(); }

  virtual void truncateFile(const std::string& /* path */, size_t /* size */) { throwNotImplemented(); }

  virtual ~IMetadataTransaction() = default;

 protected:
  [[noreturn]] static void throwNotImplemented() { throw Exception(ErrorCodes::NOT_IMPLEMENTED, "Operation is not implemented"); }
};
...
class IMetadataStorage : private boost::noncopyable {
 public:
  ...
  /// But if the metadata storage just relies on for example local FS to store data under logical path, then a file has to be created even
  /// if it's empty.
  ...
  virtual std::optional<uint64_t> getFileSizeIfExists(const std::string& path) const {
    if (existsFile(path)) return getFileSize(path);
    return std::nullopt;
  }
  ...
  virtual std::optional<Poco::Timestamp> getLastModifiedIfExists(const std::string& path) const {
    if (existsFileOrDirectory(path)) return getLastModified(path);
    return std::nullopt;
  }

  virtual time_t getLastChanged(const std::string& /* path */) const { throwNotImplemented(); }

  virtual bool supportsChmod() const = 0;

  virtual bool supportsStat() const = 0;
  virtual struct stat stat(const String& /* path */) const { throwNotImplemented(); }
  ...
  virtual bool isDirectoryEmpty(const std::string& path) const { return !iterateDirectory(path)->isValid(); }

  virtual uint32_t getHardlinkCount(const std::string& path) const = 0;

  /// Read metadata file to string from path
  virtual std::string readFileToString(const std::string& /* path */) const { throwNotImplemented(); }

  /// Read inline data for file to string from path
  virtual std::string readInlineDataToString(const std::string& /* path */) const { throwNotImplemented(); }
  ...
  virtual void applyNewSettings(const Poco::Util::AbstractConfiguration& /* config */, const std::string& /* config_prefix */,
                                ContextPtr /* context */) {}
