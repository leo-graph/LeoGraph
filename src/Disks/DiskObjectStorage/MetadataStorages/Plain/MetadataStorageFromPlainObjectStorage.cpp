...
MetadataStorageFromPlainObjectStorage::MetadataStorageFromPlainObjectStorage(ObjectStoragePtr object_storage_, String storage_path_prefix_,
                                                                             size_t object_metadata_cache_size)
    : object_storage(object_storage_),
      storage_path_prefix(std::move(storage_path_prefix_)),
      storage_path_full(fs::path(object_storage->getRootPrefix()) / storage_path_prefix) {
  if (object_metadata_cache_size)
    object_metadata_cache =
        std::make_shared<CacheBase<UInt128, ObjectMetadataEntry>>(CurrentMetrics::end(), CurrentMetrics::end(), object_metadata_cache_size);
}
...
bool MetadataStorageFromPlainObjectStorage::existsFile(const std::string &path) const {
  ...
  if (!object_storage->exists(object)) return false;
...
uint64_t MetadataStorageFromPlainObjectStorage::getFileSize(const String &path) const {
  if (auto res = getFileSizeIfExists(path)) return *res;
...
std::optional<uint64_t> MetadataStorageFromPlainObjectStorage::getFileSizeIfExists(const String &path) const {
  ...
  if (auto res = getObjectMetadataEntryWithCache(path)) return res->file_size;
  return std::nullopt;
}
...
std::optional<Poco::Timestamp> MetadataStorageFromPlainObjectStorage::getLastModifiedIfExists(const std::string &path) const {
  ...
  if (existsFileOrDirectory(path)) return Poco::Timestamp{};
  return std::nullopt;
}
...
std::vector<std::string> MetadataStorageFromPlainObjectStorage::listDirectory(const std::string &path) const {
  ...
  if (!absolute_key.empty() && !absolute_key.ends_with('/')) absolute_key += '/';
...
ObjectMetadataEntryPtr MetadataStorageFromPlainObjectStorage::getObjectMetadataEntryWithCache(const std::string &path) const {
  ...
  if (object_metadata_cache) {
    ...
    if (auto res = object_metadata_cache->get(hash128)) return res;
    if (auto mapped = get()) return object_metadata_cache->getOrSet(hash128, [&] { return mapped; }).first;
...
void MetadataStorageFromPlainObjectStorageTransaction::unlinkFile(const std::string &path, bool /*if_exists*/,
...
StoredObjects MetadataStorageFromPlainObjectStorageTransaction::getSubmittedForRemovalBlobs() { return objects_to_remove; }
