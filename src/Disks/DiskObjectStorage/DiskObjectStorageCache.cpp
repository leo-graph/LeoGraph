...
DiskObjectStoragePtr DiskObjectStorage::wrapWithCache(FileCachePtr cache, const FileCacheSettings& cache_settings,
  ...
  auto cache_disk = std::make_shared<DiskObjectStorage>(
      layer_name, std::make_shared<ClusterConfiguration>(cluster->getConfiguration()),
      std::make_shared<MetadataStorageFromCacheObjectStorage>(metadata_storage), std::make_shared<ObjectStorageRouter>(std::move(registry)),
      Context::getGlobalContextInstance()->getConfigRef(), "storage_configuration.disks." + layer_name, use_fake_transaction);
