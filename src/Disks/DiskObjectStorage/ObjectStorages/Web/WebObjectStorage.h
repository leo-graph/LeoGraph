...
class WebObjectStorage : public IObjectStorage, WithContext {
 ...
 protected:
  ...
  struct FileData {
    FileData(FileType type_, size_t size_, bool loaded_children_ = false) : type(type_), size(size_), loaded_children(loaded_children_) {}

    static FileDataPtr createFileInfo(size_t size_) { return std::make_shared<FileData>(FileType::File, size_, false); }
  ...
  struct Files : public std::map<String, FileDataPtr> {
    auto find(const String &path, bool is_file) const {
      if (is_file) return std::map<String, FileDataPtr>::find(path);
      return std::map<String, FileDataPtr>::find(path.ends_with("/") ? path : path + '/');
    }

    auto add(const String &path, FileDataPtr data) {
      if (data->type == FileType::Directory) return emplace(path.ends_with("/") ? path : path + '/', data);
 ...
 private:
  std::pair<WebObjectStorage::FileDataPtr, std::vector<std::filesystem::path>> loadFiles(const String &path,
