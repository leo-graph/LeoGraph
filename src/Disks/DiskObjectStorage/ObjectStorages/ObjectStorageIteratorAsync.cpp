...
IObjectStorageIteratorAsync::IObjectStorageIteratorAsync(CurrentMetrics::Metric threads_metric,
                                                         CurrentMetrics::Metric threads_active_metric,
                                                         CurrentMetrics::Metric threads_scheduled_metric, ThreadName thread_name)
...
void IObjectStorageIteratorAsync::nextBatch() {
  ...
  try {
    ...
    if (current_batch.empty()) {
      ...
      if (has_next_batch) outcome_future = scheduleBatch();
    }
  } catch (...) {
    has_next_batch = false;
    throw;
...
void IObjectStorageIteratorAsync::next() {
  ...
  if (current_batch_iterator == current_batch.end()) nextBatch();
}
...
size_t IObjectStorageIteratorAsync::getAccumulatedSize() const { return accumulated_size.load(std::memory_order_relaxed); }
