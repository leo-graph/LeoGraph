...
class StaticCredential : public Azure::Core::Credentials::TokenCredential {
 public:
  ...
  Azure::Core::Credentials::AccessToken GetToken(Azure::Core::Credentials::TokenRequestContext const &,
...
using AuthMethod =
    std::variant<ConnectionString, std::shared_ptr<Azure::Identity::ClientSecretCredential>,
                 std::shared_ptr<Azure::Storage::StorageSharedKeyCredential>, std::shared_ptr<Azure::Identity::WorkloadIdentityCredential>,
                 std::shared_ptr<Azure::Identity::ManagedIdentityCredential>, std::shared_ptr<AzureBlobStorage::StaticCredential>>;
}  // namespace DB::AzureBlobStorage
...
struct RelativePathWithMetadata {
  ...
  explicit RelativePathWithMetadata(String relative_path_, std::optional<ObjectMetadata> metadata_ = std::nullopt)
      : relative_path(std::move(relative_path_)), metadata(std::move(metadata_)) {}
...
struct ObjectKeyWithMetadata {
  ...
  ObjectKeyWithMetadata(ObjectStorageKey key_, ObjectMetadata metadata_) : key(std::move(key_)), metadata(std::move(metadata_)) {}
};
...
class IObjectStorage {
 public:
  ...
  virtual ObjectStorageIteratorPtr iterate(const std::string &path_prefix, size_t max_keys, bool with_tags,
  ...
  virtual std::unique_ptr<ReadBufferFromFileBase> readObject(  /// NOLINT
      const StoredObject &object, const ReadSettings &read_settings, std::optional<size_t> read_hint = {}) const = 0;
  ...
  virtual std::unique_ptr<WriteBufferFromFileBase> writeObject(  /// NOLINT
      const StoredObject &object, WriteMode mode, std::optional<ObjectAttributes> attributes = {},
      size_t buf_size = DBMS_DEFAULT_BUFFER_SIZE, const WriteSettings &write_settings = {}) = 0;
  ...
  virtual void copyObject(  /// NOLINT
      const StoredObject &object_from, const StoredObject &object_to, const ReadSettings &read_settings,
      const WriteSettings &write_settings, std::optional<ObjectAttributes> object_to_attributes = {}) = 0;
  ...
  virtual void copyObjectToAnotherObjectStorage(  /// NOLINT
      const StoredObject &object_from, const StoredObject &object_to, const ReadSettings &read_settings,
      const WriteSettings &write_settings, IObjectStorage &object_storage_to, std::optional<ObjectAttributes> object_to_attributes = {});
