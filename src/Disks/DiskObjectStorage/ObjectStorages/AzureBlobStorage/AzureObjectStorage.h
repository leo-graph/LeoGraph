...
class AzureObjectStorage : public IObjectStorage {
 public:
  ...
  AzureObjectStorage(const String& name_, AzureBlobStorage::AuthMethod auth_method, ClientPtr&& client_, SettingsPtr&& settings_,
                     const AzureBlobStorage::ConnectionParams& connection_params_, const String& object_namespace_,
                     const String& description_, const String& common_key_prefix_);

  void listObjects(const std::string& path, RelativePathsWithMetadata& children, size_t max_keys) const override;

  /// Sanitizer build may crash with max_keys=1; this looks like a false positive.
  ObjectStorageIteratorPtr iterate(const std::string& path_prefix, size_t max_keys, bool with_tags,
  ...
  std::unique_ptr<ReadBufferFromFileBase> readObject(  /// NOLINT
      const StoredObject& object, const ReadSettings& read_settings, std::optional<size_t> read_hint = {}) const override;

  SmallObjectDataWithMetadata readSmallObjectAndGetObjectMetadata(  /// NOLINT
      const StoredObject& object, const ReadSettings& read_settings, size_t max_size_bytes,
      std::optional<size_t> read_hint = {}) const override;

  /// Open the file for write and return WriteBufferFromFileBase object.
  std::unique_ptr<WriteBufferFromFileBase> writeObject(  /// NOLINT
      const StoredObject& object, WriteMode mode, std::optional<ObjectAttributes> attributes = {},
      size_t buf_size = DBMS_DEFAULT_BUFFER_SIZE, const WriteSettings& write_settings = {}) override;
  ...
  void copyObject(  /// NOLINT
      ...
      const WriteSettings& write_settings, std::optional<ObjectAttributes> object_to_attributes = {}) override;

  void shutdown() override {}

  void startup() override {}

  void applyNewSettings(const Poco::Util::AbstractConfiguration& config, const std::string& config_prefix, ContextPtr context,
  ...
  const AzureBlobStorage::ConnectionParams& getConnectionParameters() const { return connection_params; }

 private:
  void removeObjectImpl(const StoredObject& object, const std::shared_ptr<const AzureBlobStorage::ContainerClient>& client_ptr,
                        bool if_exists, BlobStorageLogWriterPtr blob_storage_log);
  ...
  /// We use source url without container and prefix as description, because in Azure there are no limitations for operations between
