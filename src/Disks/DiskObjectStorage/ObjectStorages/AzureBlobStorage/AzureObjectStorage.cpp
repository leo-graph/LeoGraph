...
class AzureIteratorAsync final : public IObjectStorageIteratorAsync {
 public:
  AzureIteratorAsync(const std::string &path_prefix, std::shared_ptr<const AzureBlobStorage::ContainerClient> client_, size_t max_list_size)
      : IObjectStorageIteratorAsync(CurrentMetrics::ObjectStorageAzureThreads, CurrentMetrics::ObjectStorageAzureThreadsActive,
                                    CurrentMetrics::ObjectStorageAzureThreadsScheduled, ThreadName::AZURE_LIST_POOL),
        client(client_) {
 ...
 private:
  bool getBatchAndCheckNext(RelativePathsWithMetadata &batch) override {
    ...
    if (client->IsClientForDisk()) ProfileEvents::increment(ProfileEvents::DiskAzureListObjects);
    ...
    for (const auto &blob : blobs_list) {
      batch.emplace_back(std::make_shared<RelativePathWithMetadata>(
          blob.Name,
          ObjectMetadata{static_cast<uint64_t>(blob.BlobSize),
    ...
    if (!blob_list_response.NextPageToken.HasValue() || blob_list_response.NextPageToken.Value().empty()) return false;
...
AzureObjectStorage::AzureObjectStorage(const String &name_, AzureBlobStorage::AuthMethod auth_method_, ClientPtr &&client_,
                                       SettingsPtr &&settings_, const AzureBlobStorage::ConnectionParams &connection_params_,
                                       const String &object_namespace_, const String &description_, const String &common_key_prefix_)
    : name(name_),
...
bool AzureObjectStorage::exists(const StoredObject &object) const {
  ...
  if (client_ptr->IsClientForDisk()) ProfileEvents::increment(ProfileEvents::DiskAzureGetProperties);
...
ObjectStorageIteratorPtr AzureObjectStorage::iterate(const std::string &path_prefix, size_t max_keys, bool,
...
void AzureObjectStorage::listObjects(const std::string &path, RelativePathsWithMetadata &children, size_t max_keys) const {
  ...
  for (auto blob_list_response = client_ptr->ListBlobs(options); blob_list_response.HasPage(); blob_list_response.MoveToNextPage()) {
    ProfileEvents::increment(ProfileEvents::AzureListObjects);
    if (client_ptr->IsClientForDisk()) ProfileEvents::increment(ProfileEvents::DiskAzureListObjects);
    ...
    if (max_keys && children.size() >= max_keys) break;
  }
}
...
std::unique_ptr<ReadBufferFromFileBase> AzureObjectStorage::readObject(  /// NOLINT
  ...
  return std::make_unique<ReadBufferFromAzureBlobStorage>(client.get(), object.remote_path, patchSettings(read_settings),
                                                          settings_ptr->max_single_read_retries, settings_ptr->max_single_download_retries,
...
std::unique_ptr<WriteBufferFromFileBase> AzureObjectStorage::writeObject(  /// NOLINT
    const StoredObject &object, WriteMode mode, std::optional<ObjectAttributes>, size_t buf_size, const WriteSettings &write_settings) {
  if (mode != WriteMode::Rewrite) throw Exception(ErrorCodes::UNSUPPORTED_METHOD, "Azure storage doesn't support append");
  ...
  return std::make_unique<WriteBufferFromAzureBlobStorage>(
      client.get(), object.remote_path,
      write_settings.use_adaptive_write_buffer ? write_settings.adaptive_write_buffer_initial_size : buf_size,
      patchSettings(write_settings), settings.get(), connection_params.getContainer(), std::move(blob_storage_log), std::move(scheduler));
}

void AzureObjectStorage::removeObjectImpl(const StoredObject &object,
                                          const std::shared_ptr<const AzureBlobStorage::ContainerClient> &client_ptr, bool if_exists,
                                          BlobStorageLogWriterPtr blob_storage_log) {
  ProfileEvents::increment(ProfileEvents::AzureDeleteObjects);
  if (client_ptr->IsClientForDisk()) ProfileEvents::increment(ProfileEvents::DiskAzureDeleteObjects);
  ...
  try {
    ...
    if (!if_exists && !delete_info.Value.Deleted)
      throw Exception(ErrorCodes::AZURE_BLOB_STORAGE_ERROR, "Failed to delete file (path: {}) in AzureBlob Storage, reason: {}", path,
    ...
    if (!if_exists) {
      if (blob_storage_log)
        blob_storage_log->addEvent(BlobStorageLogElement::EventType::Delete,
                                   ...
                                   /* remote_path */ path, object.local_path, object.bytes_size, watch.elapsedMicroseconds(), error_code,
                                   error_message);
      throw;
    }
    ...
    if (e.StatusCode == Azure::Core::Http::HttpStatusCode::NotFound) {
      ...
      if (blob_storage_log)
        blob_storage_log->addEvent(BlobStorageLogElement::EventType::Delete,
                                   /* bucket */ connection_params.getContainer(),
                                   /* remote_path */ path, object.local_path, object.bytes_size, elapsed, error_code, error_message);
      return;
    }
  ...
  if (blob_storage_log)
    blob_storage_log->addEvent(BlobStorageLogElement::EventType::Delete,
                               /* bucket */ connection_params.getContainer(),
                               /* remote_path */ path, object.local_path, object.bytes_size, elapsed, success ? 0 : error_code,
...
static void setAzureBlobTag(const std::shared_ptr<const AzureBlobStorage::ContainerClient> &client_ptr, const Strings &blob_names,
...
ObjectMetadata AzureObjectStorage::getObjectMetadata(const std::string &path, bool) const {
  ...
  if (!properties.Metadata.empty()) {
    ...
    for (const auto &[key, value] : properties.Metadata) result.attributes[key] = value;
  }
  result.last_modified = static_cast<std::chrono::system_clock::time_point>(properties.LastModified).time_since_epoch().count();
  return result;
}

std::optional<ObjectMetadata> AzureObjectStorage::tryGetObjectMetadata(const std::string &path, bool with_tags) const try {
...
void AzureObjectStorage::copyObject(  /// NOLINT
  ...
  if (client_ptr->IsClientForDisk()) ProfileEvents::increment(ProfileEvents::DiskAzureCopyObject);
  ...
  copyAzureBlobStorageFile(client_ptr, client_ptr, connection_params.getContainer(), object_from.remote_path, 0, object_metadata.size_bytes,
                           connection_params.getContainer(), object_to.remote_path, settings_ptr, read_settings, object_to_attributes,
                           scheduler);
}

void AzureObjectStorage::applyNewSettings(const Poco::Util::AbstractConfiguration &config, const std::string &config_prefix,
  ...
  if (!options.allow_client_change) return;
