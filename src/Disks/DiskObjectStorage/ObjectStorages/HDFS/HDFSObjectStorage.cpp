...
std::string HDFSObjectStorage::extractObjectKeyFromURL(const StoredObject &object) const {
  ...
  if (path.starts_with(url)) path = path.substr(url.size());
...
bool HDFSObjectStorage::exists(const StoredObject &object) const {
  ...
  if (path.starts_with(url_without_path)) path = path.substr(url_without_path.size());
...
std::unique_ptr<ReadBufferFromFileBase> HDFSObjectStorage::readObject(  /// NOLINT
  ...
  return std::make_unique<ReadBufferFromHDFS>(fs::path(url_without_path) / "", fs::path(data_directory) / path, config,
                                              patchSettings(read_settings),
                                              /* read_until_position */ 0, read_settings.remote_read_buffer_use_external_buffer);
}
...
std::unique_ptr<WriteBufferFromFileBase> HDFSObjectStorage::writeObject(  /// NOLINT
  ...
  return std::make_unique<WriteBufferFromHDFS>(url_without_path, fs::path(data_directory) / path, config, settings->replication,
                                               patchSettings(write_settings), buf_size,
                                               mode == WriteMode::Rewrite ? O_WRONLY : O_WRONLY | O_APPEND, std::move(blob_storage_log));
}
...
void HDFSObjectStorage::removeObject(const StoredObject &object) {
  ...
  if (blob_storage_log)
    blob_storage_log->addEvent(BlobStorageLogElement::EventType::Delete,
                               ...
                               /* data_size */ object.bytes_size, elapsed, error_code, error_message);

  if (res == -1) throw Exception(ErrorCodes::HDFS_ERROR, "HDFSDelete failed with path: {}", path);
}
...
void HDFSObjectStorage::removeObjectIfExists(const StoredObject &object) {
  initializeHDFSFS();
  if (exists(object)) removeObject(object);
}

void HDFSObjectStorage::removeObjectsIfExist(const StoredObjects &objects) {
  initializeHDFSFS();
  for (const auto &object : objects) removeObjectIfExists(object);
}
...
ObjectMetadata HDFSObjectStorage::getObjectMetadata(const std::string &path, bool) const {
  ...
  if (!metadata.has_value()) throw Exception(ErrorCodes::HDFS_ERROR, "{} does not exist. Error: {}", path, hdfsGetLastError());
...
void HDFSObjectStorage::listObjects(const std::string &path, RelativePathsWithMetadata &children, size_t max_keys) const {
  ...
  if (ls.file_info == nullptr && errno != ENOENT)  // NOLINT
  {
    ...
    if (ls.file_info == nullptr && errno != ENOENT)  // NOLINT
    {
      throw Exception(ErrorCodes::ACCESS_DENIED, "Cannot list directory {}: {}", path, String(hdfsGetLastError()));
    }
  }
  ...
  for (int i = 0; i < ls.length; ++i) {
    ...
    if (is_directory) {
      ...
      children.emplace_back(std::make_shared<RelativePathWithMetadata>(
          String(file_path),
          ObjectMetadata{
              static_cast<uint64_t>(ls.file_info[i].mSize), Poco::Timestamp::fromEpochTime(ls.file_info[i].mLastMod), "", {}, {}}));
    }
