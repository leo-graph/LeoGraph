...
struct HDFSObjectStorageSettings {
  HDFSObjectStorageSettings(int min_bytes_for_seek_, int replication_)
      : min_bytes_for_seek(min_bytes_for_seek_), replication(replication_) {}
...
class HDFSObjectStorage : public IObjectStorage, public HDFSErrorWrapper {
 public:
  ...
  HDFSObjectStorage(const String& hdfs_root_path_, SettingsPtr settings_, const Poco::Util::AbstractConfiguration& config_,
                    bool lazy_initialize, const String& disk_name_ = {})
    ...
    if (!lazy_initialize) initializeHDFSFS();
  }
