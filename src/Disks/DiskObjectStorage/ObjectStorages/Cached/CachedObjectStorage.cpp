...
CachedObjectStorage::CachedObjectStorage(ObjectStoragePtr object_storage_, FileCachePtr cache_, const FileCacheSettings& cache_settings_,
...
FileCache::Key CachedObjectStorage::getCacheKey(const std::string& path) const { return FileCacheKey::fromPath(path); }
...
void CachedObjectStorage::startup() { object_storage->startup(); }

bool CachedObjectStorage::exists(const StoredObject& object) const { return object_storage->exists(object); }

std::unique_ptr<ReadBufferFromFileBase> CachedObjectStorage::readObject(  /// NOLINT
    const StoredObject& object, const ReadSettings& read_settings, std::optional<size_t> read_hint) const {
  if (read_settings.enable_filesystem_cache) {
    if (cache->isInitialized()) {
      ...
      return std::make_unique<CachedOnDiskReadBufferFromFile>(
          object.remote_path, cache_key, cache, cache->getCommonOriginWithSegmentKeyType(object.local_path), read_buffer_creator,
          modified_read_settings, std::string(CurrentThread::getQueryId()), object.bytes_size,
          ...
          /* read_until_position */ std::nullopt, global_context->getFilesystemCacheLog());
    } else {
...
std::unique_ptr<WriteBufferFromFileBase> CachedObjectStorage::writeObject(  /// NOLINT
  ...
  if (cache_on_write && cache->isInitialized()) {
    ...
    return std::make_unique<CachedOnDiskWriteBufferFromFile>(
        std::move(implementation_buffer), cache, implementation_buffer->getFileName(), key,
        CurrentThread::isInitialized() && CurrentThread::get().getQueryContext() ? std::string(CurrentThread::getQueryId()) : "",
        modified_write_settings, cache->getCommonOriginWithSegmentKeyType(object.local_path),
...
void CachedObjectStorage::removeCacheIfExists(const std::string& path_key_for_cache) {
  if (path_key_for_cache.empty()) return;

  /// Add try catch?
  if (cache->isInitialized()) cache->removeKeyIfExists(getCacheKey(path_key_for_cache), FileCache::getCommonOrigin().user_id);
}

void CachedObjectStorage::removeObjectIfExists(const StoredObject& object) { removeCacheIfExists(object.remote_path); }

void CachedObjectStorage::removeObjectsIfExist(const StoredObjects& objects) {
  for (const auto& object : objects) removeCacheIfExists(object.remote_path);
}
...
void CachedObjectStorage::copyObjectToAnotherObjectStorage(  // NOLINT
  ...
  object_storage->copyObjectToAnotherObjectStorage(object_from, object_to, read_settings, write_settings, object_storage_to,
...
void CachedObjectStorage::shutdown() { object_storage->shutdown(); }

void CachedObjectStorage::applyNewSettings(const Poco::Util::AbstractConfiguration& config, const std::string& config_prefix,
...
String CachedObjectStorage::getObjectsNamespace() const { return object_storage->getObjectsNamespace(); }

ObjectStorageKeyGeneratorPtr CachedObjectStorage::createKeyGenerator() const { return object_storage->createKeyGenerator(); }
