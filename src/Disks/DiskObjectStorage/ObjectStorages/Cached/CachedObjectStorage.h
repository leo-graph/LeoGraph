...
class CachedObjectStorage final : public IObjectStorage {
 public:
  CachedObjectStorage(ObjectStoragePtr object_storage_, FileCachePtr cache_, const FileCacheSettings& cache_settings_,
                      const String& cache_config_name_);
  ...
  void copyObjectToAnotherObjectStorage(  /// NOLINT
      const StoredObject& object_from, const StoredObject& object_to, const ReadSettings& read_settings,
      const WriteSettings& write_settings, IObjectStorage& object_storage_to,
  ...
  AzureBlobStorage::AuthMethod getAzureBlobStorageAuthMethod() const override { return object_storage->getAzureBlobStorageAuthMethod(); }
#endif

#if USE_AWS_S3
  std::shared_ptr<const S3::Client> getS3StorageClient() override { return object_storage->getS3StorageClient(); }

  std::shared_ptr<const S3::Client> tryGetS3StorageClient() override { return object_storage->tryGetS3StorageClient(); }
#endif
