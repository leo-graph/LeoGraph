...
void throwIfError(const Aws::Utils::Outcome<Result, Error> &response) {
  if (!response.IsSuccess()) {
    ...
    throw S3Exception(
        fmt::format("{} (Code: {}, S3 exception: '{}')", err.GetMessage(), static_cast<size_t>(err.GetErrorType()), err.GetExceptionName()),
        err.GetErrorType());
  }
}
...
class S3IteratorAsync final : public IObjectStorageIteratorAsync {
 public:
  S3IteratorAsync(const std::string &bucket_, const std::string &path_prefix, std::shared_ptr<const S3::Client> client_,
                  size_t max_list_size, bool with_tags_, const std::optional<std::string> &start_after_)
      : IObjectStorageIteratorAsync(CurrentMetrics::ObjectStorageS3Threads, CurrentMetrics::ObjectStorageS3ThreadsActive,
                                    CurrentMetrics::ObjectStorageS3ThreadsScheduled, ThreadName::S3_LIST_POOL),
        client(client_),
    ...
    if (start_after_set) request->SetStartAfter(*start_after_);
  }
  ...
  bool getBatchAndCheckNext(RelativePathsWithMetadata &batch) override {
    ...
    if (outcome.IsSuccess()) {
      ...
      for (const auto &object : objects) {
        ObjectMetadata metadata{static_cast<uint64_t>(object.GetSize()),
                                Poco::Timestamp::fromEpochTime(object.GetLastModified().Seconds()),
                                object.GetETag(),
                                {},
                                {}};
        if (with_tags) metadata.tags = S3::getObjectTags(*client, request->GetBucket(), object.GetKey());
...
std::unique_ptr<ReadBufferFromFileBase> S3ObjectStorage::readObject(  /// NOLINT
    const StoredObject &object, const ReadSettings &read_settings, std::optional<size_t>) const {
  auto settings_ptr = s3_settings.get();
  return std::make_unique<ReadBufferFromS3>(client.get(), uri.bucket, object.remote_path, uri.version_id, settings_ptr->request_settings,
                                            patchSettings(read_settings), read_settings.remote_read_buffer_use_external_buffer,
                                            /* offset */ 0,
                                            /* read_until_position */ 0, read_settings.remote_read_buffer_restrict_seek,
...
SmallObjectDataWithMetadata S3ObjectStorage::readSmallObjectAndGetObjectMetadata(  /// NOLINT
    const StoredObject &object, const ReadSettings &read_settings, size_t max_size_bytes, std::optional<size_t> read_hint) const {
...
std::unique_ptr<WriteBufferFromFileBase> S3ObjectStorage::writeObject(  /// NOLINT
    ...
    std::optional<ObjectAttributes> attributes, size_t buf_size, const WriteSettings &write_settings) {
  WriteSettings disk_write_settings = IObjectStorage::patchSettings(write_settings);

  if (mode != WriteMode::Rewrite) throw Exception(ErrorCodes::BAD_ARGUMENTS, "S3 doesn't support append to files");
  ...
  if (auto query_context = CurrentThread::getQueryContext(); query_context && !query_context->isBackgroundContext()) {
  ...
  if (blob_storage_log) blob_storage_log->local_path = object.local_path;

  return std::make_unique<WriteBufferFromS3>(
      client.get(), uri.bucket, object.remote_path,
      write_settings.use_adaptive_write_buffer ? write_settings.adaptive_write_buffer_initial_size : buf_size, request_settings,
      std::move(blob_storage_log), attributes, std::move(scheduler), disk_write_settings);
}

ObjectStorageIteratorPtr S3ObjectStorage::iterate(const std::string &path_prefix, size_t max_keys, bool with_tags,
                                                  const std::optional<std::string> &start_after) const {
  auto settings_ptr = s3_settings.get();
  if (!max_keys) max_keys = settings_ptr->request_settings[S3RequestSetting::list_object_keys_size];
  ...
  do {
    ...
    for (const auto &object : objects)
      children.emplace_back(std::make_shared<RelativePathWithMetadata>(
          object.GetKey(), ObjectMetadata{static_cast<uint64_t>(object.GetSize()),
                                          Poco::Timestamp::fromEpochTime(object.GetLastModified().Seconds()),
                                          object.GetETag(),
                                          {},
                                          {}}));
...
void S3ObjectStorage::removeObjectImpl(const StoredObject &object, bool if_exists) {
  ...
  deleteFileFromS3(client.get(), uri.bucket, object.remote_path, if_exists, blob_storage_log, object.local_path, object.bytes_size,
...
void S3ObjectStorage::removeObjectsImpl(const StoredObjects &objects, bool if_exists) {
  ...
  deleteFilesFromS3(client.get(), uri.bucket, keys, if_exists, s3_capabilities,
                    settings_ptr->request_settings[S3RequestSetting::objects_chunk_size_to_delete], blob_storage_log,
                    local_paths_for_blob_storage_log, file_sizes_for_blob_storage_log, ProfileEvents::DiskS3DeleteObjects);
}

void S3ObjectStorage::removeObjectIfExists(const StoredObject &object) { removeObjectImpl(object, true); }

void S3ObjectStorage::removeObjectsIfExist(const StoredObjects &objects) { removeObjectsImpl(objects, true); }

void putObjectsTagOnS3(const std::shared_ptr<const S3::Client> &s3_client, const String &bucket, const Strings &object_keys,
                       const String &tag_key, const String &tag_value) {
  ...
  for (const String &object_key : object_keys) {
    ...
    if (!get_outcome.IsSuccess()) {
      ...
      throw S3Exception(err.GetErrorType(), "{} (Code: {}) while getting tagging of S3 object path {}", err.GetMessage(),
                        static_cast<size_t>(err.GetErrorType()), object_key);
    }
    const auto &get_result = get_outcome.GetResult();
    const Aws::Vector<Aws::S3::Model::Tag> &existing_tag_set = get_result.GetTagSet();
    const bool present = (std::find_if(existing_tag_set.begin(), existing_tag_set.end(), [&](const Aws::S3::Model::Tag &tag) {
    ...
    tag_set.push_back(Aws::S3::Model::Tag().WithKey(tag_key).WithValue(tag_value));
    ...
    if (put_outcome.IsSuccess()) {
      ...
      throw S3Exception(err.GetErrorType(), "{} (Code: {}) while putting tagging on S3 object {}", err.GetMessage(),
                        static_cast<size_t>(err.GetErrorType()), object_key);
    }
  }
}
...
std::optional<ObjectMetadata> S3ObjectStorage::tryGetObjectMetadata(const std::string &path, bool with_tags) const {
  ...
  if (object_info.size == 0 && object_info.last_modification_time == 0 && object_info.metadata.empty()) return {};
  ...
  try {
    ...
    if (credentials_refresh_callback) {
      ...
      if (new_client) {
        ...
        object_info =
            S3::getObjectInfo(*client.get(), uri.bucket, path, /*version_id=*/{}, /*with_metadata=*/true, /*with_tags=*/with_tags);
        updated = true;
      }
    }
    if (!updated) {
...
void S3ObjectStorage::copyObjectToAnotherObjectStorage(  // NOLINT
    const StoredObject &object_from, const StoredObject &object_to, const ReadSettings &read_settings, const WriteSettings &write_settings,
    IObjectStorage &object_storage_to, std::optional<ObjectAttributes> object_to_attributes) {
  /// Shortcut for S3
  if (auto *dest_s3 = dynamic_cast<S3ObjectStorage *>(&object_storage_to); dest_s3 != nullptr) {
    ...
    try {
      copyS3File(
          ...
          /*dest_key=*/object_to.remote_path, settings_ptr->request_settings, read_settings_to_use, BlobStorageLogWriter::create(disk_name),
          scheduler, [&, this] { return readObject(object_from, read_settings_to_use); }, object_to_attributes);
      return;
      ...
      LOG_WARNING(getLogger("S3ObjectStorage"), "S3-server-side copy object from the disk {} to the disk {} can not be performed: {}\n",
                  getName(), dest_s3->getName(), exc.what());
    }
  }

  IObjectStorage::copyObjectToAnotherObjectStorage(object_from, object_to, read_settings, write_settings, object_storage_to,
                                                   object_to_attributes);
}

void S3ObjectStorage::copyObject(  // NOLINT
    const StoredObject &object_from, const StoredObject &object_to, const ReadSettings &read_settings, const WriteSettings &,
...
void S3ObjectStorage::applyNewSettings(const Poco::Util::AbstractConfiguration &config, const std::string &config_prefix,
  ...
  settings_from_config->loadFromConfigForObjectStorage(config, config_prefix, context->getSettingsRef(), uri.uri.getScheme(),
...
ObjectStorageKeyGeneratorPtr S3ObjectStorage::createKeyGenerator() const {
  if (!key_generator) throw Exception(ErrorCodes::LOGICAL_ERROR, "Key generator is not set");

  return key_generator;
}

std::shared_ptr<const S3::Client> S3ObjectStorage::getS3StorageClient() { return client.get(); }

std::shared_ptr<const S3::Client> S3ObjectStorage::tryGetS3StorageClient() { return client.get(); }
}  // namespace DB
