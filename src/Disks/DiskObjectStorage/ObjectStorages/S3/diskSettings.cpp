...
std::unique_ptr<S3::Client> getClient(
    const std::string& endpoint, const S3Settings& settings, ContextPtr context, bool for_disk_s3, std::optional<std::string> opt_disk_name,
...

{
  ...
  if (!url.key.ends_with('/')) url.key.push_back('/');
  return getClient(url, settings, context, for_disk_s3, opt_disk_name, refresh_credentials_callback);
}

std::unique_ptr<S3::Client> getClient(
    const S3::URI& url, const S3Settings& settings, ContextPtr context, bool for_disk_s3, std::optional<std::string> opt_disk_name,
    std::optional<std::function<std::shared_ptr<DataLake::IStorageCredentials>()>> refresh_credentials_callback) {
  ...
  S3::PocoHTTPClientConfiguration client_configuration = S3::ClientFactory::instance().createClientConfiguration(
      auth_settings[S3AuthSetting::region], context->getRemoteHostFilter(), s3_max_redirects, retry_strategy,
      s3_slow_all_threads_after_network_error,
      /* s3_slow_all_threads_after_retryable_error = */ false, enable_s3_requests_logging, for_disk_s3, opt_disk_name,
      request_settings.request_throttler, url.uri.getScheme());
  ...
  auto credentials_configuration = S3::CredentialsConfiguration{auth_settings[S3AuthSetting::use_environment_credentials],
  ...
  return S3::ClientFactory::instance().create(client_configuration, client_settings, access_key_id, secret_access_key,
                                              auth_settings[S3AuthSetting::server_side_encryption_customer_key_base64],
                                              auth_settings.server_side_encryption_kms_config, auth_settings.getHeaders(),
                                              credentials_configuration, session_token);
}
