...
std::pair<FileCachePtr, FileCacheSettings> getCache(const Poco::Util::AbstractConfiguration &config, const std::string &config_prefix,
                                                    const ContextPtr &context, const std::string &cache_name, bool is_attach,
                                                    bool is_custom_disk) {
  FileCacheSettings file_cache_settings;
  auto predefined_configuration =
      config.has("cache_name") ? NamedCollectionFactory::instance().tryGet(config.getString("cache_name")) : nullptr;
  ...
  if (predefined_configuration)
    ...
    file_cache_settings.loadFromConfig(config, config_prefix, cache_path_prefix_if_relative,
                                       /* default_cache_path */ "");
  ...
  if (file_cache_settings.isPathRelativeInConfig()) {
    ...
    if (!is_attach && !pathStartsWith(file_cache_settings[FileCacheSetting::path].value, cache_path_prefix_if_relative)) {
      throw Exception(ErrorCodes::BAD_ARGUMENTS, "Filesystem cache relative path must lie inside `{}`, but have {}",
    ...
    if (!is_attach && !pathStartsWith(file_cache_settings[FileCacheSetting::path].value, cache_path_prefix_if_absolute)) {
      throw Exception(ErrorCodes::BAD_ARGUMENTS, "Filesystem cache absolute path must lie inside `{}`, but have {}",
                      cache_path_prefix_if_absolute, file_cache_settings[FileCacheSetting::path].value);
    }
  }

  auto cache = FileCacheFactory::instance().getOrCreate(cache_name, file_cache_settings, predefined_configuration ? "" : config_prefix);

  return std::pair(cache, file_cache_settings);
}

void registerDiskCache(DiskFactory &factory, bool /* global_skip_access_check */) {
  auto creator = [](const String &name, const Poco::Util::AbstractConfiguration &config, const String &config_prefix, ContextPtr context,
                    const DisksMap &map, bool attach, bool custom_disk) -> DiskPtr {
    auto disk_name = config.getString(config_prefix + ".disk", "");
    if (disk_name.empty()) throw Exception(ErrorCodes::BAD_ARGUMENTS, "Disk Cache requires `disk` field in config");
    ...
    if (!dynamic_cast<const DiskObjectStorage *>(disk.get()))
      throw Exception(ErrorCodes::BAD_ARGUMENTS,
                      "Cannot wrap disk `{}` with cache layer `{}`: cached disk is allowed only on top of object storage", disk_name, name);
    ...
    LOG_INFO(getLogger("DiskCache"), "Registered cached disk (`{}`) with structure: {}", name,
             assert_cast<DiskObjectStorage *>(cached_disk_object_storage.get())->getStructure());
