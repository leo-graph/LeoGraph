...
void registerDiskObjectStorage(DiskFactory& factory, bool global_skip_access_check) {
  ...
  auto creator = [global_skip_access_check](const String& name, const Poco::Util::AbstractConfiguration& config,
                                            const String& config_prefix, ContextPtr context, const DisksMap& /* map */, bool,
                                            bool) -> DiskPtr {
    ...
    if (config.has(config_prefix + ".locations")) {
      ...
      LOG_DEBUG(getLogger("registerDiskObjectStorage"), "Configuring DiskObjectStorage with multiple locations: [{}]",
                fmt::join(locations, ", "));
      ...
      for (const auto& location : locations) {
        ...
        const ObjectStoragePtr object_storage =
            ObjectStorageFactory::instance().create(fmt::format("{}.{}", name, location), config, object_storage_config_prefix, context,
    ...
    auto metadata_storage =
        MetadataStorageFactory::instance().create(name, config, config_prefix, cluster, object_storages, compatibility_metadata_type_hint);

    bool use_fake_transaction =
        config.getBool(config_prefix + ".use_fake_transaction", metadata_storage->getType() != MetadataStorageType::Keeper);
    DiskPtr disk = std::make_shared<DiskObjectStorage>(name, std::move(cluster), std::move(metadata_storage), std::move(object_storages),
                                                       config, config_prefix, use_fake_transaction);
