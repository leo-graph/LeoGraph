...
DiskTransactionPtr DiskObjectStorage::createTransaction() {
  if (use_fake_transaction) return std::make_shared<FakeDiskTransaction>(*this);
  return createObjectStorageTransaction();
}

ObjectStoragePtr DiskObjectStorage::getObjectStorage() { return object_storages->takePointingTo(cluster->getLocalLocation()); }
...
DiskTransactionPtr DiskObjectStorage::createObjectStorageTransactionToAnotherDisk(DiskObjectStorage &to_disk) {
  return std::make_shared<MultipleDisksObjectStorageTransaction>(cluster, metadata_storage, object_storages, to_disk.cluster,
                                                                 to_disk.metadata_storage, to_disk.object_storages);
}

DiskObjectStorage::DiskObjectStorage(const String &name_, ClusterConfigurationPtr cluster_, MetadataStoragePtr metadata_storage_,
                                     ObjectStorageRouterPtr object_storages_, const Poco::Util::AbstractConfiguration &config,
                                     const String &config_prefix, bool use_fake_transaction_)
    : IDisk(name_, config, config_prefix),
      ...
      blob_killer(
          std::make_shared<BlobKillerThread>(name, Context::getGlobalContextInstance(), cluster, metadata_storage, object_storages)),
      blob_copier(
      ...
      remove_shared_recursive_file_limit(
          config.getUInt64(config_prefix + ".remove_shared_recursive_file_limit", DEFAULT_REMOVE_SHARED_RECURSIVE_FILE_LIMIT)) {
  ...
  resource_changes_subscription = Context::getGlobalContextInstance()->getWorkloadEntityStorage().getAllEntitiesAndSubscribe(
      [this](const std::vector<IWorkloadEntityStorage::Event> &events) {
        ...
        // Current state
        if (!read_resource_name_from_sql.empty()) new_read_resource_name_from_sql.insert(read_resource_name_from_sql);
        if (!write_resource_name_from_sql.empty()) new_write_resource_name_from_sql.insert(write_resource_name_from_sql);
        if (!read_resource_name_from_sql_any.empty()) new_read_resource_name_from_sql_any.insert(read_resource_name_from_sql_any);
        if (!write_resource_name_from_sql_any.empty()) new_write_resource_name_from_sql_any.insert(write_resource_name_from_sql_any);
                ...
                if (!disk) {
                  switch (mode) {
                    case ResourceAccessMode::DiskRead:
                      new_read_resource_name_from_sql_any.insert(resource_name);
                      break;
                    case ResourceAccessMode::DiskWrite:
                      new_write_resource_name_from_sql_any.insert(resource_name);
                      break;
                    default:
                      break;
                  }
                  ...
                  switch (mode) {
                    case ResourceAccessMode::DiskRead:
                      new_read_resource_name_from_sql.insert(resource_name);
                      break;
                    case ResourceAccessMode::DiskWrite:
                      new_write_resource_name_from_sql.insert(resource_name);
                      break;
                    default:
                      break;
                  }
...
DiskObjectStorage::~DiskObjectStorage() { LOG_INFO(log, "Destroying disk {}", name); }
...
bool DiskObjectStorage::existsFile(const String &path) const { return metadata_storage->existsFile(path); }

bool DiskObjectStorage::existsDirectory(const String &path) const { return metadata_storage->existsDirectory(path); }

bool DiskObjectStorage::existsFileOrDirectory(const String &path) const { return metadata_storage->existsFileOrDirectory(path); }
...
size_t DiskObjectStorage::getFileSize(const String &path) const { return metadata_storage->getFileSize(path); }
...
void DiskObjectStorage::copyFile(  /// NOLINT
    const String &from_file_path, IDisk &to_disk, const String &to_file_path, const ReadSettings &read_settings,
    const WriteSettings &write_settings, const std::function<void()> &cancellation_hook) {
...
bool DiskObjectStorage::renameExchangeIfSupported(const std::string &, const std::string &) { return false; }
...
void DiskObjectStorage::removeSharedFiles(const RemoveBatchRequest &files, bool keep_all_batch_data,
...
UInt32 DiskObjectStorage::getRefCount(const String &path) const { return metadata_storage->getHardlinkCount(path); }
...
String DiskObjectStorage::getUniqueId(const String &path) const {
  ...
  if (!blobs_paths.empty()) id = blobs_paths[0].remote_path;
  return id;
}
...
void DiskObjectStorage::createDirectories(const String &path) {
  ...
  if (metadata_storage->isTransactional()) {
    while (!metadata_storage->existsFileOrDirectory(path)) {
      ...
      if (isSuccessfulOutcome(transaction->tryCommit(TransactionCommitOptionsVariant{}))) break;
    }
  } else {
    auto transaction = createObjectStorageTransaction();
    transaction->createDirectories(path);
...
DirectoryIteratorPtr DiskObjectStorage::iterateDirectory(const String &path) const { return metadata_storage->iterateDirectory(path); }

bool DiskObjectStorage::isDirectoryEmpty(const String &path) const { return metadata_storage->isDirectoryEmpty(path); }

void DiskObjectStorage::listFiles(const String &path, std::vector<String> &file_names) const {
  for (auto it = iterateDirectory(path); it->isValid(); it->next()) file_names.push_back(it->name());
}
...
Poco::Timestamp DiskObjectStorage::getLastModified(const String &path) const { return metadata_storage->getLastModified(path); }

time_t DiskObjectStorage::getLastChanged(const String &path) const { return metadata_storage->getLastChanged(path); }

struct stat DiskObjectStorage::stat(const String &path) const { return metadata_storage->stat(path); }
...
void DiskObjectStorage::shutdown() {
  ...
  for (const auto &[location, _] : cluster->getConfiguration()) object_storages->takePointingTo(location)->shutdown();
...
void DiskObjectStorage::startupImpl() {
  ...
  for (const auto &[location, _] : cluster->getConfiguration()) object_storages->takePointingTo(location)->startup();
  ...
  while (metadata_storage->hasUnreplicatedBlobs(cluster->getLocalLocation())) {
    ...
    if (auto elapsed = wait_round.elapsedMilliseconds(); elapsed < 1000) sleepForMilliseconds(1000 - elapsed);
  }
...
ReservationPtr DiskObjectStorage::reserve(UInt64 bytes) {
  if (!tryReserve(bytes, std::nullopt)) return {};
...
ReservationPtr DiskObjectStorage::reserve(UInt64 bytes, const ReservationConstraints &constraints) {
  if (!tryReserve(bytes, constraints)) return {};
...
bool DiskObjectStorage::tryReserve(UInt64 bytes, const std::optional<ReservationConstraints> &constraints) {
  ...
  if (constraints.has_value()) {
    ...
    if (available_space.has_value() && total_space.has_value()) {
      ...
      if (bytes > unreserved_space) return false;
  ...
  if (unreserved_space >= bytes) {
    LOG_TRACE(log, "Reserved {} on remote disk {}, having unreserved {}.", ReadableSize(bytes), backQuote(name),
...
void DiskObjectStorage::removeSharedRecursive(const String &path, bool keep_all_batch_data,
                                              const NameSet &file_names_remove_metadata_only) {
  auto transaction = createObjectStorageTransaction();
...
void DiskObjectStorage::removeSharedRecursiveWithLimit(const String &path, bool keep_all_batch_data,
                                                       const NameSet &file_names_remove_metadata_only) {
  ...
  auto remove = [&]() {
    auto transaction = createObjectStorageTransaction();
    if (!local_paths.empty()) transaction->removeSharedFiles(local_paths, keep_all_batch_data, file_names_remove_metadata_only);
    for (auto &directory : directories) transaction->removeDirectory(directory);
    transaction->commit();
    local_paths.clear();
    directories.clear();
  };
...
bool DiskObjectStorage::supportsCache() const { return object_storages->takePointingTo(cluster->getLocalLocation())->supportsCache(); }

bool DiskObjectStorage::isReadOnly() const { return object_storages->takePointingTo(cluster->getLocalLocation())->isReadOnly(); }

bool DiskObjectStorage::isPlain() const { return metadata_storage->isPlain(); }

bool DiskObjectStorage::isWriteOnce() const { return metadata_storage->isWriteOnce(); }
...
bool DiskObjectStorage::supportsHardLinks() const { return !metadata_storage->isWriteOnce() && !metadata_storage->isPlain(); }

template <class Settings>
static inline Settings updateIOSchedulingSettings(const Settings &settings, const String &read_resource_name,
                                                  const String &write_resource_name) {
  if (read_resource_name.empty() && write_resource_name.empty()) return settings;
...
std::unique_ptr<ReadBufferFromFileBase> DiskObjectStorage::readFile(const String &path, const ReadSettings &settings,
                                                                    std::optional<size_t> read_hint) const {
  const auto storage_objects = metadata_storage->getStorageObjects(path);
  auto global_context = Context::getGlobalContextInstance();

  if (storage_objects.empty()) return std::make_unique<ReadBufferFromEmptyFile>();
...
#if ENABLE_DISTRIBUTED_CACHE
  use_distributed_cache = enable_distributed_cache && DistributedCache::canUseDistributedCacheForRead(
                                                          read_settings, *object_storages->takePointingTo(cluster->getLocalLocation()));
#endif
  const bool use_async_buffer = read_settings.remote_fs_method == RemoteFSReadMethod::threadpool;
  const bool file_cache_enabled =
      object_storages->takePointingTo(cluster->getLocalLocation())->supportsCache() && read_settings.enable_filesystem_cache;
  const bool use_page_cache = read_settings.page_cache &&
  ...
  auto read_buffer_creator = [this, read_settings, read_hint](
  ...
  bool prefer_bigger_buffer_size = read_settings.filesystem_cache_prefer_bigger_buffer_size &&
                                   ...
                                   read_settings.enable_filesystem_cache && !use_distributed_cache;

  size_t buffer_size = prefer_bigger_buffer_size ? std::max<size_t>(settings.remote_fs_buffer_size, settings.prefetch_buffer_size)
                                                 : settings.remote_fs_buffer_size;

  size_t total_objects_size = getTotalSize(storage_objects);
  if (total_objects_size) buffer_size = std::min(buffer_size, total_objects_size);

  auto read_from_object_storage_gather = [=]() -> std::unique_ptr<ReadBufferFromFileBase> {
    return std::make_unique<ReadBufferFromRemoteFSGather>(std::move(read_buffer_creator), storage_objects, read_settings,
  ...
  if (use_distributed_cache) {
    impl = DistributedCache::readWithDistributedCache(
        path, storage_objects, read_settings, *object_storages->takePointingTo(cluster->getLocalLocation()),
        /*use_external_buffer*/ use_page_cache || use_async_buffer, read_from_object_storage_gather);
  }
#endif

  if (!impl) impl = read_from_object_storage_gather();
  ...
  if (use_page_cache) {
    ...
    if (!object_namespace.empty()) cache_path_prefix += object_namespace + "/";
    const auto cache_key = PageCacheKey{.path = cache_path_prefix + storage_objects.at(0).remote_path};

    impl = std::make_unique<CachedInMemoryReadBufferFromFile>(cache_key, read_settings.page_cache, std::move(impl), read_settings);
  }

  if (use_async_buffer) {
    auto &reader = global_context->getThreadPoolReader(FilesystemReaderType::ASYNCHRONOUS_REMOTE_FS_READER);
    const size_t min_bytes_for_seek =
        use_distributed_cache ? read_settings.distributed_cache_settings.min_bytes_for_seek : read_settings.remote_read_min_bytes_for_seek;

    return std::make_unique<AsynchronousBoundedReadBuffer>(std::move(impl), reader, read_settings, buffer_size, min_bytes_for_seek,
...
std::unique_ptr<ReadBufferFromFileBase> DiskObjectStorage::readFileIfExists(const String &path, const ReadSettings &settings,
                                                                            std::optional<size_t> read_hint) const {
...
std::unique_ptr<WriteBufferFromFileBase> DiskObjectStorage::writeFile(const String &path, size_t buf_size, WriteMode mode,
...
Strings DiskObjectStorage::getBlobPath(const String &path) const {
  ...
  for (const auto &object : objects) res.emplace_back(object.remote_path);
  String objects_namespace = object_storages->takePointingTo(cluster->getLocalLocation())->getObjectsNamespace();
  if (!objects_namespace.empty()) res.emplace_back(objects_namespace);
  return res;
}

bool DiskObjectStorage::areBlobPathsRandom() const { return metadata_storage->areBlobPathsRandom(); }
...
void DiskObjectStorage::applyNewSettings(const Poco::Util::AbstractConfiguration &config, ContextPtr context, const String &config_prefix,
                                         const DisksMap &map) {
  ...
  /// FIXME we cannot use config_prefix that was passed through arguments because the disk may be wrapped with cache and we need another
  /// name
  for (const auto &[location, info] : cluster->getConfiguration())
    object_storages->takePointingTo(location)->applyNewSettings(config, info.config_prefix, context,
                                                                IObjectStorage::ApplyNewSettingsOptions{.allow_client_change = true});

  {
    std::unique_lock lock(resource_mutex);
    if (String new_read_resource_name = config.getString(config_prefix + ".read_resource", "");
        new_read_resource_name != read_resource_name_from_config)
      read_resource_name_from_config = new_read_resource_name;
    if (String new_write_resource_name = config.getString(config_prefix + ".write_resource", "");
        new_write_resource_name != write_resource_name_from_config)
...
DiskPtr DiskObjectStorageReservation::getDisk(size_t i) const {
  if (i != 0) throw Exception(ErrorCodes::INCORRECT_DISK_INDEX, "Can't use i != 0 with single disk reservation");
  return disk;
}
