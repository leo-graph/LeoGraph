...
class DiskObjectStorage : public IDisk {
 ...
 public:
  DiskObjectStorage(const String& name_, ClusterConfigurationPtr cluster_, MetadataStoragePtr metadata_storage_,
                    ObjectStorageRouterPtr object_storages_, const Poco::Util::AbstractConfiguration& config, const String& config_prefix,
  ...
  bool supportParallelWrite() const override {
    return object_storages->takePointingTo(cluster->getLocalLocation())->supportParallelWrite();
  }
  ...
  void refresh(UInt64 not_sooner_than_milliseconds) override { metadata_storage->refresh(not_sooner_than_milliseconds); }

  ReservationPtr reserve(UInt64 bytes) override;

  ReservationPtr reserve(UInt64 bytes, const ReservationConstraints& constraints) override;

  std::unique_ptr<ReadBufferFromFileBase> readFile(const String& path, const ReadSettings& settings,
                                                   std::optional<size_t> read_hint) const override;

  std::unique_ptr<ReadBufferFromFileBase> readFileIfExists(const String& path, const ReadSettings& settings,
                                                           std::optional<size_t> read_hint) const override;

  std::unique_ptr<WriteBufferFromFileBase> writeFile(const String& path, size_t buf_size, WriteMode mode,
  ...
  void copyFile(  /// NOLINT
      ...
      const WriteSettings& write_settings = {}, const std::function<void()>& cancellation_hook = {}) override;

  void applyNewSettings(const Poco::Util::AbstractConfiguration& config, ContextPtr context, const String& config_prefix,
                        const DisksMap& map) override;
  ...
  String getStructure() const {
    return fmt::format("DiskObjectStorage-{}({})", getName(), object_storages->takePointingTo(cluster->getLocalLocation())->getName());
  }
