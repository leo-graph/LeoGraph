...
class DiskBackupDirectoryIterator final : public IDirectoryIterator {
 public:
  ...
  String path() const override { return fmt::format("{}/{}", dir_path, files[index]); }
...
DiskBackup::DiskBackup(std::shared_ptr<IBackup> backup_, PathPrefixReplacement path_prefix_replacement_,
...
std::optional<UInt64> DiskBackup::getTotalSpace() const { return backup->getTotalSize(); }

std::optional<UInt64> DiskBackup::getAvailableSpace() const { return 0; }

std::optional<UInt64> DiskBackup::getUnreservedSpace() const { return 0; }
...
std::unique_ptr<ReadBufferFromFileBase> DiskBackup::readFile(const String &path, const ReadSettings &, std::optional<size_t>) const {
  std::string replaced_path = replacePathPrefix(path);
...
Poco::Timestamp DiskBackup::getLastModified(const String &path) const {
...

  {
    ...
    if (it != last_modified.end()) return it->second;
  }

  return backup->getTimestamp();
}

time_t DiskBackup::getLastChanged(const String &) const { return backup->getTimestamp(); }
...
void DiskBackup::copyDirectoryContent(const String & /*from_dir*/, const std::shared_ptr<IDisk> & /*to_disk*/, const String & /*to_dir*/,
...
String DiskBackup::replacePathPrefix(const String &path) const {
  if (path.starts_with(path_prefix_replacement.from)) return path_prefix_replacement.to + path.substr(path_prefix_replacement.from.size());
