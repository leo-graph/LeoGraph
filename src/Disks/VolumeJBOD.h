...
class VolumeJBOD : public IVolume {
 public:
  VolumeJBOD(String name_, Disks disks_, UInt64 max_data_part_size_, bool are_merges_avoided_, bool perform_ttl_move_on_insert_,
             VolumeLoadBalancing load_balancing_, UInt64 least_used_ttl_ms_)
  ...
  VolumeJBOD(String name_, const Poco::Util::AbstractConfiguration& config, const String& config_prefix, DiskSelectorPtr disk_selector);

  VolumeJBOD(const VolumeJBOD& volume_jbod, const Poco::Util::AbstractConfiguration& config, const String& config_prefix,
 ...
 private:
  ...
  struct DiskWithSize {
    ...
    explicit DiskWithSize(DiskPtr disk_) : disk(disk_), free_size(disk->getUnreservedSpace()) {}

    bool operator<(const DiskWithSize& rhs) const {
      if (disk->isReadOnly()) return true;
      if (rhs.disk->isReadOnly()) return false;
