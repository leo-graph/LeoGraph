...
TEST(HierarchyDictionariesUtils, getHierarchy) {
  {
    ...
    auto get_parent_key_func = [&](auto key) {
      ...
      if (parent_key == hierarchy_null_value_key) return result;
    ...
    auto result = DB::detail::getHierarchy(keys, is_key_valid_func, get_parent_key_func);
...
TEST(HierarchyDictionariesUtils, getIsInHierarchy) {
  {
    ...
    PaddedPODArray<UInt8> actual = DB::detail::getIsInHierarchy(keys, keys_in, is_key_valid_func, get_parent_key_func);
...
TEST(HierarchyDictionariesUtils, getDescendants) {
  {
    ...
    {
      size_t keys_found;
      auto result = DB::detail::getDescendants(keys, *parent_to_child_index, DB::detail::GetAllDescendantsStrategy(), keys_found);
    ...
    }
    {
      size_t keys_found;
      auto result =
          DB::detail::getDescendants(keys, *parent_to_child_index, DB::detail::GetDescendantsAtSpecificLevelStrategy{1}, keys_found);
