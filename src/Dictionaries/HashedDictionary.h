...
class HashedDictionary final : public IDictionary {
  using DictionaryParallelLoaderType =
      HashedDictionaryParallelLoader<dictionary_key_type, HashedDictionary<dictionary_key_type, sparse, sharded>>;
 ...
 public:
  ...
  HashedDictionary(const StorageID &dict_id_, const DictionaryStructure &dict_struct_, DictionarySourcePtr source_ptr_,
                   const HashedDictionaryConfiguration &configuration_, BlockPtr update_field_loaded_block_ = nullptr);
  ...
  std::shared_ptr<IExternalLoadable> clone() const override {
    return std::make_shared<HashedDictionary<dictionary_key_type, sparse, sharded>>(getDictionaryID(), dict_struct, source_ptr->clone(),
                                                                                    configuration, update_field_loaded_block);
  }
  ...
  ColumnPtr getColumn(const std::string &attribute_name, const DataTypePtr &attribute_type, const Columns &key_columns,
                      const DataTypes &key_types, DefaultOrFilter default_or_filter) const override;

  ColumnUInt8::Ptr hasKeys(const Columns &key_columns, const DataTypes &key_types) const override;

  bool hasHierarchy() const override {
    return dictionary_key_type == DictionaryKeyType::Simple && dict_struct.hierarchical_attribute_index.has_value();
  }
  ...
  ColumnPtr getDescendants(ColumnPtr key_column, const DataTypePtr &key_type, size_t level,
 ...
 private:
  ...
  struct Attribute final {
    ...
    std::variant<CollectionsHolder<UInt8>, CollectionsHolder<UInt16>, CollectionsHolder<UInt32>, CollectionsHolder<UInt64>,
                 CollectionsHolder<UInt128>, CollectionsHolder<UInt256>, CollectionsHolder<Int8>, CollectionsHolder<Int16>,
                 CollectionsHolder<Int32>, CollectionsHolder<Int64>, CollectionsHolder<Int128>, CollectionsHolder<Int256>,
                 CollectionsHolder<Decimal32>, CollectionsHolder<Decimal64>, CollectionsHolder<Decimal128>, CollectionsHolder<Decimal256>,
                 CollectionsHolder<DateTime64>, CollectionsHolder<Float32>, CollectionsHolder<Float64>, CollectionsHolder<UUID>,
                 CollectionsHolder<IPv4>, CollectionsHolder<IPv6>, CollectionsHolder<std::string_view>, CollectionsHolder<Array>>
        containers;
  };
  ...
  void getItemsImpl(const Attribute &attribute, DictionaryKeysExtractor<dictionary_key_type> &keys_extractor, ValueSetter &&set_value,
                    DefaultValueExtractor &default_value_extractor) const;

  template <typename AttributeType, bool is_nullable, typename ValueSetter, typename NullAndDefaultSetter>
  void getItemsShortCircuitImpl(const Attribute &attribute, DictionaryKeysExtractor<dictionary_key_type> &keys_extractor,
                                ValueSetter &&set_value, NullAndDefaultSetter &&set_null_and_default, IColumn::Filter &default_mask) const;
...
template <DictionaryKeyType dictionary_key_type, bool sparse, bool sharded>
HashedDictionary<dictionary_key_type, sparse, sharded>::HashedDictionary(const StorageID &dict_id_, const DictionaryStructure &dict_struct_,
...
HashedDictionary<dictionary_key_type, sparse, sharded>::~HashedDictionary() {
  ...
  if constexpr (!sharded) return;

  size_t shards = std::max<size_t>(configuration.shards, 1);
  ThreadPool pool(CurrentMetrics::HashedDictionaryThreads, CurrentMetrics::HashedDictionaryThreadsActive,
                  CurrentMetrics::HashedDictionaryThreadsScheduled, shards);
  ...
  LOG_TRACE(log, "Destroying {} non empty hash tables for dictionary {} (using {} threads) ", hash_tables_count, dictionary_name,
            pool.getMaxThreads());
  pool.wait();
...
template <DictionaryKeyType dictionary_key_type, bool sparse, bool sharded>
ColumnPtr HashedDictionary<dictionary_key_type, sparse, sharded>::getColumn(const std::string &attribute_name,
                                                                            const DataTypePtr &attribute_type, const Columns &key_columns,
  ...
  if (dictionary_key_type == DictionaryKeyType::Complex) dict_struct.validateKeyTypes(key_types);
    ...
    if (is_short_circuit) {
      ...
      if constexpr (std::is_same_v<ValueType, Array>) {
        ...
        getItemsShortCircuitImpl<ValueType, false>(
            attribute, extractor, [&](const size_t, const Array &value) { out->insert(value); }, [&](size_t) { out->insertDefault(); },
            default_mask);
        ...
        if (is_attribute_nullable) {
          ...
          getItemsShortCircuitImpl<ValueType, false>(
              attribute, extractor, [&](size_t, std::string_view value) { out->insertData(value.data(), value.size()); },
              [&](size_t) { out->insertDefault(); }, default_mask);
      } else {
        ...
        if (is_attribute_nullable)
          getItemsShortCircuitImpl<ValueType, true>(
              ...
              [&](size_t row) { (*vec_null_map_to)[row] = true; }, default_mask);
        else
          getItemsShortCircuitImpl<ValueType, false>(
      ...
      if constexpr (std::is_same_v<ValueType, Array>) {
        ...
        getItemsImpl<ValueType, false>(
            attribute, extractor, [&](const size_t, const Array &value, bool) { out->insert(value); }, default_value_extractor);
      } else if constexpr (std::is_same_v<ValueType, std::string_view>) {
        auto *out = column.get();
        ...
        if (is_attribute_nullable)
          ...
          getItemsImpl<ValueType, false>(
              attribute, extractor, [&](size_t, std::string_view value, bool) { out->insertData(value.data(), value.size()); },
              default_value_extractor);
      } else {
        auto &out = column->getData();
        ...
        if (is_attribute_nullable)
          ...
          getItemsImpl<ValueType, false>(
              attribute, extractor, [&](size_t row, const auto value, bool) { out[row] = value; }, default_value_extractor);
      }
    }
  ...
  if (is_attribute_nullable) result = ColumnNullable::create(result, std::move(col_null_map_to));

  return result;
}

template <DictionaryKeyType dictionary_key_type, bool sparse, bool sharded>
ColumnUInt8::Ptr HashedDictionary<dictionary_key_type, sparse, sharded>::hasKeys(const Columns &key_columns,
...
template <DictionaryKeyType dictionary_key_type, bool sparse, bool sharded>
ColumnPtr HashedDictionary<dictionary_key_type, sparse, sharded>::getHierarchy(ColumnPtr key_column [[maybe_unused]],
  ...
  if constexpr (dictionary_key_type == DictionaryKeyType::Simple) {
    ...
    if (!dictionary_attribute.null_value.isNull()) null_value = dictionary_attribute.null_value.safeGet<UInt64>();
    ...
    auto get_parent_func = [&](auto &hierarchy_key) {
      ...
      if (null_value && *null_value == parent_key) return result;
...
template <DictionaryKeyType dictionary_key_type, bool sparse, bool sharded>
ColumnUInt8::Ptr HashedDictionary<dictionary_key_type, sparse, sharded>::isInHierarchy(ColumnPtr key_column [[maybe_unused]],
                                                                                       ColumnPtr in_key_column [[maybe_unused]],
                                                                                       const DataTypePtr &) const {
  if constexpr (dictionary_key_type == DictionaryKeyType::Simple) {
    if (key_column->isNullable()) key_column = assert_cast<const ColumnNullable *>(key_column.get())->getNestedColumnPtr();
...
DictionaryHierarchyParentToChildIndexPtr HashedDictionary<dictionary_key_type, sparse, sharded>::getHierarchicalIndex() const {
  if constexpr (dictionary_key_type == DictionaryKeyType::Simple) {
    if (hierarchical_index) return hierarchical_index;
    ...
    size_t size = 0;
    for (const auto &map : child_key_to_parent_key_maps) size += map.size();
...
void HashedDictionary<dictionary_key_type, sparse, sharded>::createAttributes() {
  ...
  for (const auto &dictionary_attribute : dict_struct.attributes) {
    auto type_call = [&, this](const auto &dictionary_attribute_type) {
      ...
      auto is_nullable_sets =
          dictionary_attribute.is_nullable ? std::make_optional<NullableSets>(configuration.shards) : std::optional<NullableSets>{};
      if constexpr (IsBuiltinHashTable<typename CollectionsHolder<ValueType>::value_type>) {
        ...
        for (size_t i = 0; i < configuration.shards; ++i) collections.emplace_back(grower);

        Attribute attribute{dictionary_attribute.underlying_type, std::move(is_nullable_sets), std::move(collections)};
        attributes.emplace_back(std::move(attribute));
      } else {
        Attribute attribute{dictionary_attribute.underlying_type, std::move(is_nullable_sets),
                            CollectionsHolder<ValueType>(configuration.shards)};
  ...
  if (unlikely(attributes.size()) == 0) {
    ...
    for (size_t i = 0; i < configuration.shards; ++i) no_attributes_containers.emplace_back(grower);
  }

  string_arenas.resize(configuration.shards);
  for (auto &arena : string_arenas) arena = std::make_unique<Arena>();
}
  ...
  if (!update_field_loaded_block || update_field_loaded_block->rows() == 0) {
    ...
    io.executeWithCallbacks([&]() {
      ...
      while (executor.pull(block)) {
        ...
        if (!update_field_loaded_block) update_field_loaded_block = std::make_shared<Block>(block.cloneEmpty());
    ...
    update_field_loaded_block = std::make_shared<Block>(
        mergeBlockWithPipe<dictionary_key_type>(dict_struct.getKeysSize(), *update_field_loaded_block, std::move(io)));
  }
...
template <DictionaryKeyType dictionary_key_type, bool sparse, bool sharded>
void HashedDictionary<dictionary_key_type, sparse, sharded>::blockToAttributes(const Block &block,
                                                                               DictionaryKeysArenaHolder<dictionary_key_type> &arena_holder,
                                                                               UInt64 shard) {
  ...
  for (size_t i = 0; i < skip_keys_size_offset; ++i) key_columns.emplace_back(block.safeGetByPosition(i).column);
  ...
  if (unlikely(attributes_size == 0)) {
    for (size_t key_index = 0; key_index < keys_size; ++key_index) {
      ...
      if constexpr (std::is_same_v<KeyType, std::string_view>) key = copyStringInArena(*string_arenas[shard], key);
...
template <DictionaryKeyType dictionary_key_type, bool sparse, bool sharded>
template <typename AttributeType, bool is_nullable, typename ValueSetter, typename DefaultValueExtractor>
void HashedDictionary<dictionary_key_type, sparse, sharded>::getItemsImpl(const Attribute &attribute,
...
void HashedDictionary<dictionary_key_type, sparse, sharded>::getItemsShortCircuitImpl(
    const Attribute &attribute, DictionaryKeysExtractor<dictionary_key_type> &keys_extractor, ValueSetter &&set_value,
    NullAndDefaultSetter &&set_null_and_default, IColumn::Filter &default_mask) const {
  const auto &attribute_containers = std::get<CollectionsHolder<AttributeType>>(attribute.containers);
  const size_t keys_size = keys_extractor.getKeysSize();
...
void HashedDictionary<dictionary_key_type, sparse, sharded>::loadData() {
  if (!source_ptr->hasUpdateField()) {
    ...
    if constexpr (sharded) parallel_loader.emplace(*this);
    ...
    if (parallel_loader) parallel_loader->finish();
  } else {
    updateData();
  }

  if (configuration.require_nonempty && 0 == element_count)
    throw Exception(ErrorCodes::DICTIONARY_IS_EMPTY, "{}: dictionary source is empty and 'require_nonempty' property is set.",
                    getFullName());
}

template <DictionaryKeyType dictionary_key_type, bool sparse, bool sharded>
void HashedDictionary<dictionary_key_type, sparse, sharded>::buildHierarchyParentToChildIndexIfNeeded() {
  if (!dict_struct.hierarchical_attribute_index) return;

  if (dict_struct.attributes[*dict_struct.hierarchical_attribute_index].bidirectional) hierarchical_index = getHierarchicalIndex();
}
...
void HashedDictionary<dictionary_key_type, sparse, sharded>::calculateBytesAllocated() {
  ...
  for (size_t attribute_index = 0; attribute_index < attributes_size; ++attribute_index) {
    ...
    if (attribute.is_nullable_sets.has_value()) {
      for (auto &is_nullable_set : *attribute.is_nullable_sets) bytes_allocated += is_nullable_set.getBufferSizeInBytes();
    }
  }
  ...
  if (update_field_loaded_block) bytes_allocated += update_field_loaded_block->allocatedBytes();
  ...
  for (const auto &arena : string_arenas) bytes_allocated += arena->allocatedBytes();
}

template <DictionaryKeyType dictionary_key_type, bool sparse, bool sharded>
Pipe HashedDictionary<dictionary_key_type, sparse, sharded>::read(const Names &column_names, size_t max_block_size,
  ...
  if (!attributes.empty()) {
    ...
    if (attribute.is_nullable_sets) {
      for (auto &is_nullable_set : *attribute.is_nullable_sets) {
        ...
        for (auto &node : is_nullable_set) keys.emplace_back(node.getKey());
      }
    }
  } else {
    for (const auto &container : no_attributes_containers) {
      ...
      for (const auto &key : container) keys.emplace_back(getSetKeyFromCell(key));
    }
  }
...
template <DictionaryKeyType dictionary_key_type, bool sparse, bool sharded>
template <typename GetContainersFunc>
void HashedDictionary<dictionary_key_type, sparse, sharded>::getAttributeContainers(size_t attribute_index,
                                                                                    GetContainersFunc &&get_containers_func) {
...
template <typename GetContainersFunc>
void HashedDictionary<dictionary_key_type, sparse, sharded>::getAttributeContainers(size_t attribute_index,
                                                                                    GetContainersFunc &&get_containers_func) const {
  const_cast<std::decay_t<decltype(*this)> *>(this)->getAttributeContainers(
      attribute_index, [&](auto &attribute_containers) { std::forward<GetContainersFunc>(get_containers_func)(attribute_containers); });
}
