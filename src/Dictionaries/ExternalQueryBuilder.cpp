...
ExternalQueryBuilder::ExternalQueryBuilder(const DictionaryStructure& dict_struct_, const std::string& db_, const std::string& schema_,
                                           const std::string& table_, const std::string& query_, const std::string& where_,
                                           IdentifierQuotingStyle quoting_style_)
    : dict_struct(dict_struct_), db(db_), schema(schema_), table(table_), query(query_), where(where_), quoting_style(quoting_style_) {
  if (table.empty() && query.empty()) throw Exception(ErrorCodes::UNSUPPORTED_METHOD, "Setting `table` or `query` must be non empty");
...
std::string ExternalQueryBuilder::composeLoadIdsQuery(const VectorWithMemoryTracking<UInt64>& ids) const {
  if (!dict_struct.id) throw Exception(ErrorCodes::UNSUPPORTED_METHOD, "Simple key required for method");
...
std::string ExternalQueryBuilder::composeLoadKeysQuery(const Columns& key_columns, const VectorWithMemoryTracking<size_t>& requested_rows,
                                                       LoadKeysMethod method, size_t partition_key_prefix) const {
  if (!dict_struct.key) throw Exception(ErrorCodes::UNSUPPORTED_METHOD, "Composite key required for method");
  ...
  if (query.empty()) {
    ...
    if (!where.empty()) {
      if (method != CASSANDRA_SEPARATE_PARTITION_KEY) writeString("(", out);
      writeString(where, out);
...
void ExternalQueryBuilder::composeKeyCondition(const Columns& key_columns, size_t row, WriteBuffer& out, size_t beg, size_t end) const {
  auto first = true;
  for (size_t i = beg; i < end; ++i) {
    if (!first) writeString(" AND ", out);
...
void ExternalQueryBuilder::composeKeysCondition(const Columns& key_columns, const VectorWithMemoryTracking<size_t>& requested_rows,
                                                LoadKeysMethod method, size_t partition_key_prefix, WriteBuffer& out) const {
  bool first = true;

  if (method == AND_OR_CHAIN) {
    first = true;
    for (const auto row : requested_rows) {
      if (!first) writeString(" OR ", out);
  ...
  } else /* if (method == CASSANDRA_SEPARATE_PARTITION_KEY) */
  {
    ...
    if (partition_key_prefix && partition_key_prefix < key_columns.size()) writeString(" AND ", out);
