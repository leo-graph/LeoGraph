...
class CacheDictionaryStorage final : public ICacheDictionaryStorage {
 ...
 public:
  ...
  explicit CacheDictionaryStorage(const DictionaryStructure &dictionary_structure, CacheDictionaryStorageConfiguration &configuration_)
      : configuration(configuration_), rnd_engine(randomSeed()) {
  ...
  String getName() const override {
    if (dictionary_key_type == DictionaryKeyType::Simple) return "Cache";
  ...
  size_t getBytesAllocated() const override {
    ...
    for (size_t attribute_index = 0; attribute_index < attributes_size; ++attribute_index) {
      getAttributeContainer(attribute_index,
                            [&](const auto &container) { attributes_size_in_bytes += container.capacity() * sizeof(container[0]); });
    }
 ...
 private:
  struct FetchedKey {
    FetchedKey(size_t element_index_, bool is_default_) : element_index(element_index_), is_default(is_default_) {}
  ...
  KeysStorageFetchResult fetchColumnsForKeysImpl(const PaddedPODArray<KeyType> &keys, const DictionaryStorageFetchRequest &fetch_request,
    ...
    for (size_t attribute_index = 0; attribute_index < fetch_request.attributesSize(); ++attribute_index) {
      if (!fetch_request.shouldFillResultColumnWithIndex(attribute_index)) continue;
      ...
      if (!default_mask) {
        ...
        if (unlikely(attribute.is_nullable)) {
          getItemsForFetchedKeys<Field>(
              attribute, fetched_columns_index, fetched_keys, [&](Field &value) { fetched_column.insert(value); }, default_value_provider);
        } else {
          auto type_call = [&](const auto &dictionary_attribute_type) {
            ...
            if constexpr (std::is_same_v<ValueType, Array>) {
              getItemsForFetchedKeys<ValueType>(
                  attribute, fetched_columns_index, fetched_keys, [&](Array &value) { fetched_column.insert(value); },
                  default_value_provider);
            } else if constexpr (std::is_same_v<ValueType, std::string_view>) {
              getItemsForFetchedKeys<ValueType>(
                  attribute, fetched_columns_index, fetched_keys,
                  [&](std::string_view value) { fetched_column.insertData(value.data(), value.size()); }, default_value_provider);
            } else {
              auto &data = column_typed.getData();

              getItemsForFetchedKeys<ValueType>(
                  attribute, fetched_columns_index, fetched_keys, [&](auto value) { data.push_back(static_cast<AttributeType>(value)); },
                  default_value_provider);
            }
          };
        ...
        if (unlikely(attribute.is_nullable)) {
          getItemsForFetchedKeysShortCircuit<Field>(
              attribute, fetched_columns_index, fetched_keys, [&](Field &value) { fetched_column.insert(value); }, *default_mask);
        } else {
          auto type_call = [&](const auto &dictionary_attribute_type) {
            using Type = std::decay_t<decltype(dictionary_attribute_type)>;
            using AttributeType = typename Type::AttributeType;
            ...
            if constexpr (std::is_same_v<ValueType, Array>) {
              getItemsForFetchedKeysShortCircuit<ValueType>(
                  attribute, fetched_columns_index, fetched_keys, [&](Array &value) { fetched_column.insert(value); }, *default_mask);
            } else if constexpr (std::is_same_v<ValueType, std::string_view>) {
              getItemsForFetchedKeysShortCircuit<ValueType>(
                  attribute, fetched_columns_index, fetched_keys,
                  [&](std::string_view value) { fetched_column.insertData(value.data(), value.size()); }, *default_mask);
            } else {
              auto &data = column_typed.getData();
  ...
  PaddedPODArray<KeyType> getCachedKeysImpl() const {
    ...
    for (auto &cell : cells) {
      if (cell.deadline == 0) continue;

      if (cell.is_default) continue;
  ...
  using ContainerType = std::conditional_t<std::is_same_v<ValueType, Field> || std::is_same_v<ValueType, Array>,
                                           VectorWithMemoryTracking<ValueType>, PaddedPODArray<ValueType>>;
  ...
  struct Attribute {
    ...
    std::variant<ContainerType<UInt8>, ContainerType<UInt16>, ContainerType<UInt32>, ContainerType<UInt64>, ContainerType<UInt128>,
                 ...
                 ContainerType<Decimal128>, ContainerType<Decimal256>, ContainerType<DateTime64>, ContainerType<Float32>,
                 ContainerType<Float64>, ContainerType<UUID>, ContainerType<IPv4>, ContainerType<IPv6>, ContainerType<std::string_view>,
                 ContainerType<Array>, ContainerType<Field>>
        attribute_container;
  };

  template <typename ValueType, typename ValueSetter>
  void getItemsForFetchedKeys(Attribute &attribute, size_t fetched_keys_size, PaddedPODArray<FetchedKey> &fetched_keys,
                              ValueSetter &&value_setter, const DefaultValueProvider &default_value_provider) {
    auto &container = std::get<ContainerType<ValueType>>(attribute.attribute_container);
  ...
  template <typename ValueType, typename ValueSetter>
  void getItemsForFetchedKeysShortCircuit(Attribute &attribute, size_t fetched_keys_size, PaddedPODArray<FetchedKey> &fetched_keys,
                                          ValueSetter &&value_setter, IColumn::Filter &default_mask) {
  ...
  KeyStateAndCellIndex getKeyStateAndCellIndex(const KeyType key, const time_t now) const {
    ...
    for (; place_value < place_value_end; ++place_value) {
      ...
      if (unlikely(now > cell.deadline + max_lifetime_seconds)) return std::make_pair(KeyState::not_found, cell_place_value);

      if (unlikely(now > cell.deadline)) return std::make_pair(KeyState::expired, cell_place_value);
  ...
  size_t getCellIndexForInsert(const KeyType &key) const {
    ...
    for (; place_value < place_value_end; ++place_value) {
      ...
      if (cell.deadline == 0) return cell_place_value;

      if (cell.key == key) return cell_place_value;
