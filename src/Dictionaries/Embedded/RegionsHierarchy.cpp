...
void RegionsHierarchy::reload() {
  ...
  if (!data_source->isModified()) return;
  ...
  while (regions_reader->readNext(region_entry)) {
    if (region_entry.id > max_region_id) {
      if (region_entry.id > max_size)
        throw Exception(ErrorCodes::INCORRECT_DATA, "Region id is too large: {}, should be not more than {}", region_entry.id, max_size);
  ...
  for (RegionID i = 0; i <= max_region_id; ++i) {
    if (types[i] == RegionType::City) new_city[i] = i;

    if (types[i] == RegionType::Area) new_area[i] = i;

    if (types[i] == RegionType::District) new_district[i] = i;

    if (types[i] == RegionType::Country) new_country[i] = i;
    ...
    while (true) {
      ...
      if (depth == std::numeric_limits<RegionDepth>::max())
        throw Exception(ErrorCodes::LOGICAL_ERROR, "Logical error in regions hierarchy: region {} possible is inside infinite loop",
                        current);
      ...
      if (current > max_region_id)
        throw Exception(ErrorCodes::LOGICAL_ERROR, "Logical error in regions hierarchy: region {} (specified as parent) doesn't exist",
                        current);

      if (types[current] == RegionType::City) new_city[i] = current;

      if (types[current] == RegionType::Area) new_area[i] = current;

      if (types[current] == RegionType::District) new_district[i] = current;

      if (types[current] == RegionType::Country) new_country[i] = current;

      if (types[current] == RegionType::Continent) {
        if (!new_continent[i]) new_continent[i] = current;
