...
bool RegionsHierarchyDataSource::isModified() const { return updates_tracker.isModified(); }
...
void RegionsHierarchiesDataProvider::discoverFilesWithCustomHierarchies() {
  ...
  for (fs::directory_iterator dir_it(dir_path); dir_it != dir_end; ++dir_it) {
    ...
    if (candidate_basename.starts_with(basename) && (candidate_basename.size() > basename.size() + 1) &&
...
VectorWithMemoryTracking<std::string> RegionsHierarchiesDataProvider::listCustomHierarchies() const {
  ...
  for (const auto& it : hierarchy_files) names.push_back(it.first);
  return names;
}
