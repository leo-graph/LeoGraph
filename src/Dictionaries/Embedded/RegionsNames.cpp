...
void RegionsNames::reload() {
  ...
  for (size_t language_id = 0; language_id < total_languages; ++language_id) {
    ...
    if (!names_source || !names_source->isModified()) continue;
    ...
    while (names_reader->readNext(name_entry)) {
      ...
      if (new_chars.capacity() < old_size + name_entry.name.length() + 1)
        throw Exception(ErrorCodes::LOGICAL_ERROR, "Logical error. Maybe size estimate of {} is wrong", names_source->getSourceName());
      ...
      if (name_entry.id > max_region_id) {
        ...
        if (name_entry.id > max_size)
          throw Exception(ErrorCodes::INCORRECT_DATA, "Region id is too large: {}, should be not more than {}", name_entry.id, max_size);
      }

      while (name_entry.id >= new_names_refs.size()) new_names_refs.resize(new_names_refs.size() * 2);
  ...
  for (size_t language_id = 0; language_id < total_languages; ++language_id) names_refs[language_id].resize(max_region_id + 1);
}
