...
void registerDictionarySourceYAMLRegExpTree(DictionarySourceFactory &factory) {
  auto create_table_source = [=]([[maybe_unused]] const String &name, [[maybe_unused]] const DictionaryStructure &dict_struct,
                                 [[maybe_unused]] const Poco::Util::AbstractConfiguration &config,
                                 [[maybe_unused]] const String &config_prefix, Block &, [[maybe_unused]] ContextPtr global_context,
                                 const String &, [[maybe_unused]] bool created_from_ddl) -> DictionarySourcePtr {
#if USE_YAML_CPP
    if (dict_struct.has_expressions) {
      throw Exception(ErrorCodes::SUPPORT_IS_DISABLED, "Dictionary source of type `{}` does not support attribute expressions",
                      kYAMLRegExpTree);
    }
    ...
    throw Exception(ErrorCodes::SUPPORT_IS_DISABLED,
                    "Dictionary source of type `{}` is disabled because ClickHouse was built without yaml_cpp support", kYAMLRegExpTree);
#endif
  };
...
void parseMatchList(UInt64 parent_id, UInt64 &id, const YAML::Node &node, ResultColumns &names_to_attributes, const String &key_name,
...
void parseMatchNode(UInt64 parent_id, UInt64 &id, const YAML::Node &node, ResultColumns &result, const String &key_name,
                    const DictionaryStructure &structure) {
  if (!node.IsMap()) {
  ...
  for (const auto &[key, node_] : match) {
    if (key == key_name) {
      if (!node_.IsScalar()) throw Exception(ErrorCodes::INVALID_CONFIG_PARAMETER, "`{}` should be a String", key_name);
...
Block parseYAMLAsRegExpTree(const YAML::Node &node, const String &key_name, const DictionaryStructure &structure) {
  ...
  columns.push_back(
      ColumnWithTypeAndName(std::move(result_cols.keys), DataTypePtr(new DataTypeArray(DataTypePtr(new DataTypeString()))), kKeys));
  columns.push_back(
      ColumnWithTypeAndName(std::move(result_cols.values), DataTypePtr(new DataTypeArray(DataTypePtr(new DataTypeString()))), kValues));

  return Block(std::move(columns));
}

YAMLRegExpTreeDictionarySource::YAMLRegExpTreeDictionarySource(const String &filepath_, const DictionaryStructure &dict_struct,
                                                               ContextPtr context_, bool created_from_ddl)
...
BlockIO YAMLRegExpTreeDictionarySource::loadAll() {
  ...
  BlockIO io;
  io.pipeline = QueryPipeline(
      std::make_shared<SourceFromSingleChunk>(std::make_shared<const Block>(parseYAMLAsRegExpTree(node, key_name, structure))));
  return io;
}
...
String YAMLRegExpTreeDictionarySource::toString() const { return fmt::format("{} with path: {}", kYAMLRegExpTree, filepath); }

Poco::Timestamp YAMLRegExpTreeDictionarySource::getLastModification() const { return FS::getModificationTimestamp(filepath); }
