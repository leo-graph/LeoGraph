...
class HashedDictionaryParallelLoader : public boost::noncopyable {
 public:
  explicit HashedDictionaryParallelLoader(DictionaryType& dictionary_)
      : dictionary(dictionary_),
        ...
        pool(CurrentMetrics::HashedDictionaryThreads, CurrentMetrics::HashedDictionaryThreadsActive,
    ...
    LOG_TRACE(dictionary.log, "Will load the {} dictionary using {} threads (with {} backlog and timeout {} sec)", dictionary_name, shards,
              backlog, loading_timeout.count());
    ...
    for (size_t shard = 0; shard < shards; ++shard) {
      ...
      try {
        pool.scheduleOrThrowOnError([this, shard, thread_group = CurrentThread::getGroup()] {
          ...
          SCOPE_EXIT_SAFE(LOG_TRACE(dictionary.log,
                                    "Finished worker for dictionary {} shard {}, processed {} blocks, {} rows, total time {}ms",
        ...
        for (size_t shard_to_finish = 0; shard_to_finish < shard; ++shard_to_finish) shards_queues[shard_to_finish]->clearAndFinish();
    ...
    for (size_t shard = 0; shard < shards; ++shard) {
      ...
      while (!shards_queues[shard]->tryPush(current_block, /* milliseconds= */ 100)) {
        ...
        if (loading_timeout.count() && std::chrono::milliseconds(total_loading_time.elapsedMilliseconds()) > loading_timeout) {
          stop_all_workers = true;
          pool.wait();
          throw Exception(ErrorCodes::TIMEOUT_EXCEEDED, "Timeout {} sec for dictionary {} loading is expired", loading_timeout.count(),
                          dictionary_name);
        }
      }
    }
  ...
  void finish() {
    for (auto& queue : shards_queues) queue->finish();
  ...
  ~HashedDictionaryParallelLoader() {
    try {
      for (auto& queue : shards_queues) queue->clearAndFinish();
 ...
 private:
  ...
  void threadWorker(size_t shard, WorkerStatistic& statistic) {
    ...
    while (true) {
      if (!shard_queue.tryPop(block, /* milliseconds= */ 100)) {
        ...
        if (shard_queue.isFinishedAndEmpty()) break;
  ...
  Blocks splitBlock(const IColumn::Selector& selector, const Block& block) {
    ...
    for (size_t shard = 0; shard < shards; ++shard) out_blocks[shard] = block.cloneEmpty();

    size_t columns = block.columns();
    for (size_t col = 0; col < columns; ++col) {
      MutableColumns split_columns = block.getByPosition(col).column->scatter(shards, selector);
      for (size_t shard = 0; shard < shards; ++shard) out_blocks[shard].getByPosition(col).column = std::move(split_columns[shard]);
    }
