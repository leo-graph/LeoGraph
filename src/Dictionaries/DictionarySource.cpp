...
class DictionarySource : public ISource {
 ...
 private:
  ...
  Chunk generate() override {
    ...
    if (!coordinator->getKeyColumnsNextRangeToRead(key_columns_to_read, data_columns)) return {};
    ...
    for (const auto& key_column_to_read : key_columns_to_read) {
      ...
      if (header->has(key_column_to_read.name)) name_to_column.emplace(key_column_to_read.name, key_column_to_read.column);
    }

    for (const auto& data_column : data_columns) {
      if (header->has(data_column.name)) name_to_column.emplace(data_column.name, data_column.column);
    }
    ...
    auto attributes_columns =
        read_columns_func(attributes_names_to_read, attributes_types_to_read, key_columns, key_types, attributes_default_values_columns);
...
bool DictionarySourceCoordinator::getKeyColumnsNextRangeToRead(ColumnsWithTypeAndName& key_columns, ColumnsWithTypeAndName& data_columns) {
  ...
  if (start >= keys_size) return false;
...
void DictionarySourceCoordinator::initialize(const Names& column_names) {
  ...
  for (const auto& column_name : column_names) {
    ...
    if (!column_with_type.type)
      throw Exception(ErrorCodes::NO_SUCH_COLUMN_IN_TABLE, "No such column name {} in dictionary {}", column_name,
...
ColumnsWithTypeAndName DictionarySourceCoordinator::cutColumns(const ColumnsWithTypeAndName& columns_with_type, size_t start,
                                                               size_t length) {
