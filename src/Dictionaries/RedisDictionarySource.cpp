...
void registerDictionarySourceRedis(DictionarySourceFactory& factory) {
  auto create_table_source = [=](const String& /*name*/, const DictionaryStructure& dict_struct,
                                 const Poco::Util::AbstractConfiguration& config, const String& config_prefix, Block& sample_block,
...
RedisDictionarySource::RedisDictionarySource(const DictionaryStructure& dict_struct_, const RedisConfiguration& configuration_,
  ...
  if (dict_struct.attributes.size() != 1)
    throw Exception(ErrorCodes::INVALID_CONFIG_PARAMETER, "Invalid number of non key columns for Redis source: {}, expected 1",
                    dict_struct.attributes.size());

  if (configuration.storage_type == RedisStorageType::HASH_MAP) {
    if (!dict_struct.key)
      throw Exception(ErrorCodes::INVALID_CONFIG_PARAMETER, "Redis source with storage type \'hash_map\' must have key");

    if (dict_struct.key->size() != 2)
      throw Exception(ErrorCodes::INVALID_CONFIG_PARAMETER, "Redis source with storage type \'hash_map\' requires 2 keys");
...
BlockIO RedisDictionarySource::loadAll() {
  ...
  if (all_keys.isNull()) {
    io.pipeline = QueryPipeline(
        std::make_shared<RedisSource>(std::move(connection), RedisArray{}, configuration.storage_type, sample_block, REDIS_MAX_BLOCK_SIZE));
    return io;
  }
  ...
  for (auto&& key : all_keys)
    if (key_type == connection->client->execute<String>(RedisCommand("TYPE").addRedisType(key))) keys.addRedisType(key);

  if (configuration.storage_type == RedisStorageType::HASH_MAP) {
    keys = *getRedisHashMapKeys(connection, keys);
  }

  io.pipeline = QueryPipeline(std::make_shared<RedisSource>(std::move(connection), std::move(keys), configuration.storage_type,
                                                            sample_block, REDIS_MAX_BLOCK_SIZE));
  return io;
}
...
BlockIO RedisDictionarySource::loadIds(const VectorWithMemoryTracking<UInt64>& ids) {
  ...
  for (UInt64 id : ids) keys << DB::toString(id);
...
String RedisDictionarySource::toString() const { return "Redis: " + configuration.host + ':' + DB::toString(configuration.port); }
