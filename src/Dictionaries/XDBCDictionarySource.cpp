...
namespace {
ExternalQueryBuilder makeExternalQueryBuilder(const DictionaryStructure& dict_struct_, const std::string& db_, const std::string& schema_,
                                              const std::string& table_, const std::string& query_, const std::string& where_,
                                              IXDBCBridgeHelper& bridge_) {
  QualifiedTableName qualified_name{schema_, table_};

  if (bridge_.isSchemaAllowed()) {
    if (qualified_name.database.empty()) qualified_name = QualifiedTableName::parseFromString(qualified_name.table);
  } else {
    if (!qualified_name.database.empty())
      throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT, "Dictionary source specifies a schema but schema is not supported by {}-driver",
                      bridge_.getName());
  }
...
XDBCDictionarySource::XDBCDictionarySource(const DictionaryStructure& dict_struct_, const Configuration& configuration_,
                                           const Block& sample_block_, ContextPtr context_, const BridgeHelperPtr bridge_)
    : WithContext(context_->getGlobalContext()),
      ...
      query_builder(makeExternalQueryBuilder(dict_struct, configuration.db, configuration.schema, configuration.table, configuration.query,
                                             configuration.where, *bridge_)),
  ...
  for (const auto& [name, value] : url_params) bridge_url.addQueryParameter(name, value);
}
...
bool XDBCDictionarySource::supportsSelectiveLoad() const { return true; }

bool XDBCDictionarySource::hasUpdateField() const { return !configuration.update_field.empty(); }

DictionarySourcePtr XDBCDictionarySource::clone() const { return std::make_shared<XDBCDictionarySource>(*this); }
...
std::string XDBCDictionarySource::doInvalidateQuery(const std::string& request) const {
  ...
  auto url_params = bridge_helper->getURLParams(max_block_size);
  for (const auto& [name, value] : url_params) invalidate_url.addQueryParameter(name, value);

  QueryPipeline pipeline = loadFromQuery(invalidate_url, invalidate_sample_block, request);
  return readInvalidateQuery(pipeline);
}

QueryPipeline XDBCDictionarySource::loadFromQuery(const Poco::URI& uri, const Block& required_sample_block,
...
void registerDictionarySourceXDBC(DictionarySourceFactory& factory) {
  auto create_table_source = [=](const String& /*name*/, const DictionaryStructure& dict_struct,
    ...
    BridgeHelperPtr bridge = std::make_shared<XDBCBridgeHelper<ODBCBridgeMixin>>(
        global_context, global_context->getSettingsRef()[Setting::http_receive_timeout],
...
void registerDictionarySourceJDBC(DictionarySourceFactory& factory) {
  auto create_table_source = [=](const String& /*name*/, const DictionaryStructure& /* dict_struct */,
                                 ...
                                 Block& /* sample_block */, ContextPtr /* global_context */, const std::string& /* default_database */,
