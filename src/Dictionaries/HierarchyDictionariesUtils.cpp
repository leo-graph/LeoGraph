 ...
 */
ChildToParentHierarchicalContext getChildToParentHierarchicalContext(const IDictionary *dictionary,
  ...
  if (!hierarchical_attribute.null_value.isNull()) null_value = hierarchical_attribute.null_value.safeGet<UInt64>();
  ...
  ColumnPtr key_not_in_storage_default_value_column =
      ColumnVector<UInt64>::create(initial_keys_to_request.size(), key_not_in_storage_value);
  if (hierarchical_attribute.is_nullable) key_not_in_storage_default_value_column = makeNullable(key_not_in_storage_default_value_column);
  ...
  if (hierarchical_attribute.is_nullable) context.child_key_parent_key_is_null = HashSet<UInt64>();
  ...
  while (!keys_to_request.empty()) {
    ...
    auto hierarchical_attribute_parent_key_column =
        dictionary->getColumn(hierarchical_attribute.name, hierarchical_attribute.type, {key_to_request_column}, {key_type},
    ...
    if (!parent_key_column_typed)
      throw Exception(ErrorCodes::UNSUPPORTED_METHOD, "Parent key column should be UInt64. Actual {}",
    ...
    for (size_t i = 0; i < keys_to_request_size; ++i) {
      ...
      if (parent_key == key_not_in_storage_value) continue;

      child_key_to_parent_key[child_key] = parent_key;

      if ((null_value && parent_key == *null_value) || already_requested_keys.find(parent_key) != nullptr) continue;
...
ColumnPtr getKeysHierarchyDefaultImplementation(const IDictionary *dictionary, ColumnPtr key_column, const DataTypePtr &key_type,
  ...
  if (!key_column_typed) throw Exception(ErrorCodes::UNSUPPORTED_METHOD, "Key column should be UInt64");
  ...
  auto get_parent_key_func = [&](auto &key) {
    ...
    if (it == nullptr) return result;

    UInt64 parent_key = it->getMapped();
    if (null_value && parent_key == *null_value) return result;
...
ColumnUInt8::Ptr getKeysIsInHierarchyDefaultImplementation(const IDictionary *dictionary, ColumnPtr key_column, ColumnPtr in_key_column,
  ...
  if (!in_key_column_typed) throw Exception(ErrorCodes::UNSUPPORTED_METHOD, "Key column should be UInt64");
