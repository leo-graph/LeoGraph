  ...
  auto create_dictionary_source = [](const String& /*name*/, const DictionaryStructure& dict_struct,
    ...
    if (!context->getSettingsRef()[Setting::allow_experimental_ytsaurus_dictionary_source])
      throw Exception(ErrorCodes::UNKNOWN_STORAGE,
                      "Dictionary source YTsaurus is experimental. "
    ...
    if (named_collection) {
      ...
      configuration =
          std::make_shared<YTsaurusStorageConfiguration>(StorageYTsaurus::processNamedCollectionResult(*named_collection, settings, true));
    } else {
...
YTsarususDictionarySource::YTsarususDictionarySource(ContextPtr context_, const DictionaryStructure& dict_struct_,
...
BlockIO YTsarususDictionarySource::loadAll() {
  BlockIO io;
  io.pipeline = QueryPipeline(YTsaurusSourceFactory::createPipe(client, configuration->cypress_path,
                                                                {
...
BlockIO YTsarususDictionarySource::loadIds(const VectorWithMemoryTracking<UInt64>& ids) {
  ...
  if (!supportsSelectiveLoad())
    throw Exception(ErrorCodes::UNSUPPORTED_METHOD,
                    "Can't make selective update of YTsaurus dictionary because data source doesn't supports lookups.");

  auto block = blockForIds(dict_struct, ids);

  BlockIO io;
  io.pipeline = QueryPipeline(YTsaurusSourceFactory::createPipe(
      client, configuration->cypress_path,
      {.settings = configuration->settings, .lookup_input_block = std::move(block), .check_types_allow_nullable = true}, sample_block,
      max_block_size
...
std::string YTsarususDictionarySource::toString() const { return fmt::format("YTsaurus: {}", configuration->cypress_path); }
#endif
