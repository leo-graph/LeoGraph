...
DirectDictionary<dictionary_key_type>::DirectDictionary(const StorageID &dict_id_, const DictionaryStructure &dict_struct_,
                                                        DictionarySourcePtr source_ptr_)
    : IDictionary(dict_id_), dict_struct(dict_struct_), source_ptr{std::move(source_ptr_)} {
...
template <DictionaryKeyType dictionary_key_type>
Columns DirectDictionary<dictionary_key_type>::getColumns(const Strings &attribute_names, const DataTypes &attribute_types,
  ...
  if constexpr (dictionary_key_type == DictionaryKeyType::Complex) dict_struct.validateKeyTypes(key_types);
  ...
  for (size_t attribute_index = 0; attribute_index < request.attributesSize(); ++attribute_index) {
    if (!request.shouldFillResultColumnWithIndex(attribute_index)) continue;
  ...
  io.executeWithCallbacks([&]() {
    ...
    while (executor.pull(block)) {
      ...
      for (size_t i = 0; i < dictionary_keys_size; ++i) block_key_columns.emplace_back(block.safeGetByPosition(i).column);
  ...
  LOG_DEBUG(getLogger("DirectDictionary"), "read {} blocks with {} rows from pipeline in {} ms", block_num, rows_num,
  ...
  for (size_t attribute_index = 0; attribute_index < result_columns.size(); ++attribute_index) {
    ...
    if (default_mask && !mask_filled) default_mask->resize(requested_keys_size);
    ...
    for (size_t requested_key_index = 0; requested_key_index < requested_keys_size; ++requested_key_index) {
      ...
      if (it) {
        ...
        if (default_mask && !mask_filled) (*default_mask)[requested_key_index] = 0;

        result_column->insert(value_to_insert);
      } else {
        if (default_mask) {
          if (!mask_filled) (*default_mask)[requested_key_index] = 1;
...
template <DictionaryKeyType dictionary_key_type>
ColumnPtr DirectDictionary<dictionary_key_type>::getColumn(const std::string &attribute_name, const DataTypePtr &attribute_type,
                                                           const Columns &key_columns, const DataTypes &key_types,
...
template <DictionaryKeyType dictionary_key_type>
ColumnUInt8::Ptr DirectDictionary<dictionary_key_type>::hasKeys(const Columns &key_columns,
...
template <DictionaryKeyType dictionary_key_type>
ColumnPtr DirectDictionary<dictionary_key_type>::getHierarchy(ColumnPtr key_column, const DataTypePtr &key_type) const {
...
template <DictionaryKeyType dictionary_key_type>
ColumnUInt8::Ptr DirectDictionary<dictionary_key_type>::isInHierarchy(ColumnPtr key_column, ColumnPtr in_key_column,
...
class SourceFromQueryPipeline : public ISource {
 public:
  explicit SourceFromQueryPipeline(QueryPipeline &pipeline_) : ISource(pipeline_.getSharedHeader()), executor(pipeline_) {
    pipeline_.setConcurrencyControl(false);
  }

  explicit SourceFromQueryPipeline(BlockIO io)
      : ISource(io.pipeline.getSharedHeader()), io_holder(std::move(io)), executor(io_holder->pipeline) {
...
Pipe getSourcePipe(QueryPipeline &pipeline, const bool use_async_executor) {
  if (use_async_executor) return Pipe(std::make_shared<SourceFromQueryPipeline<PullingAsyncPipelineExecutor>>(pipeline));
...
template <DictionaryKeyType dictionary_key_type>
Pipe DirectDictionary<dictionary_key_type>::read(const Names & /* column_names */, size_t /* max_block_size */,
...
namespace {
template <DictionaryKeyType dictionary_key_type>
DictionaryPtr createDirectDictionary(const std::string &full_name, const DictionaryStructure &dict_struct,
                                     const Poco::Util::AbstractConfiguration &config, const std::string &config_prefix,
                                     DictionarySourcePtr source_ptr, ContextPtr global_context, bool /* created_from_ddl */) {
  const auto *layout_name = dictionary_key_type == DictionaryKeyType::Simple ? "direct" : "complex_key_direct";

  if constexpr (dictionary_key_type == DictionaryKeyType::Simple) {
    if (dict_struct.key)
      throw Exception(ErrorCodes::UNSUPPORTED_METHOD, "'key' is not supported for dictionary of layout '{}'", layout_name);
  } else {
    if (dict_struct.id) throw Exception(ErrorCodes::UNSUPPORTED_METHOD, "'id' is not supported for dictionary of layout '{}'", layout_name);
  }
  ...
  if (config.has(config_prefix + ".lifetime.min") || config.has(config_prefix + ".lifetime.max"))
    throw Exception(ErrorCodes::BAD_ARGUMENTS, "'lifetime' parameter is redundant for the dictionary' of layout '{}'", layout_name);
