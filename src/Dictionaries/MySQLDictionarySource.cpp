...
static const ValidateKeysMultiset<ExternalDatabaseEqualKeysSet> dictionary_allowed_keys = {"host",
                                                                                           "port",
                                                                                           "user",
                                                                                           "password",
                                                                                           "db",
...
void registerDictionarySourceMysql(DictionarySourceFactory& factory) {
  auto create_table_source = [=](const String& /*name*/, [[maybe_unused]] const DictionaryStructure& dict_struct,
                                 [[maybe_unused]] const Poco::Util::AbstractConfiguration& config,
                                 [[maybe_unused]] const std::string& config_prefix, [[maybe_unused]] Block& sample_block,
                                 [[maybe_unused]] ContextPtr global_context, const std::string& /* default_database */,
                                 [[maybe_unused]] bool created_from_ddl) -> DictionarySourcePtr {
#if USE_MYSQL
    StreamSettings mysql_input_stream_settings(
        global_context->getSettingsRef(),
        config.getBool(config_prefix + ".mysql.close_connection", false) ||
            config.getBool(config_prefix + ".mysql.share_connection", false),
        false, config.getBool(config_prefix + ".mysql.fail_on_connection_loss", false) ? 1 : default_num_tries_on_connection_loss);
    ...
    if (named_collection) {
      ...
      for (const auto& name : setting_names) allowed_arguments.insert(name);
      ...
      for (auto& address : addresses) global_context->getRemoteHostFilter().checkHostAndPort(address.first, toString(address.second));
      ...
      pool = std::make_shared<mysqlxx::PoolWithFailover>(createMySQLPoolWithFailover(
          dictionary_configuration->db, addresses, named_collection->getAnyOrDefault<String>({"user", "username"}, ""),
          named_collection->getOrDefault<String>("password", ""), named_collection->getOrDefault<String>("ssl_ca", ""),
          named_collection->getOrDefault<String>("ssl_cert", ""), named_collection->getOrDefault<String>("ssl_key", ""), mysql_settings));
    } else {
      dictionary_configuration.emplace(MySQLDictionarySource::Configuration{
    ...
    return std::make_unique<MySQLDictionarySource>(dict_struct, *dictionary_configuration, std::move(pool), sample_block,
...
MySQLDictionarySource::MySQLDictionarySource(const DictionaryStructure& dict_struct_, const Configuration& configuration_,
                                             mysqlxx::PoolWithFailoverPtr pool_, const Block& sample_block_,
    ...
    : log(getLogger("MySQLDictionarySource")),
      ...
      query_builder(dict_struct, configuration.db, "", configuration.table, configuration.query, configuration.where,
...
QueryPipeline MySQLDictionarySource::loadFromQuery(const String& query) {
  return QueryPipeline(std::make_shared<MySQLWithFailoverSource>(pool, query, sample_block, settings));
}
...
bool MySQLDictionarySource::supportsSelectiveLoad() const { return true; }

bool MySQLDictionarySource::hasUpdateField() const { return !configuration.update_field.empty(); }

DictionarySourcePtr MySQLDictionarySource::clone() const { return std::make_shared<MySQLDictionarySource>(*this); }
