...
class IDictionary : public IExternalLoadable, public IKeyValueEntity {
 public:
  explicit IDictionary(const StorageID& dictionary_id_) : dictionary_id(dictionary_id_) {}
  ...
  std::string getQualifiedName() const {
    std::lock_guard lock{mutex};
    if (dictionary_id.database_name.empty()) return dictionary_id.table_name;
  ...
  std::string getDatabaseOrNoDatabaseTag() const {
    ...
    if (!dictionary_id.database_name.empty()) return dictionary_id.database_name;
  ...
  virtual void convertKeyColumns(Columns& key_columns, DataTypes& key_types) const {
    ...
    if (key_types_size != key_attributes_types_size)
      throw Exception(ErrorCodes::TYPE_MISMATCH, "Dictionary {} key lookup structure does not match, expected {}", getFullName(),
    ...
    for (size_t key_attribute_type_index = 0; key_attribute_type_index < key_attributes_types_size; ++key_attribute_type_index) {
      ...
      if (key_attribute_type->equals(*key_type)) continue;
  ...
  virtual ColumnPtr getColumn(const std::string& attribute_name [[maybe_unused]], const DataTypePtr& attribute_type [[maybe_unused]],
                              const Columns& key_columns [[maybe_unused]], const DataTypes& key_types [[maybe_unused]],
  ...
  virtual Columns getColumns(const Strings& attribute_names, const DataTypes& attribute_types, const Columns& key_columns,
                             const DataTypes& key_types, DefaultsOrFilter defaults_or_filter) const {
   ...
   */
  virtual ColumnPtr getColumnAllValues(const std::string& attribute_name [[maybe_unused]], const DataTypePtr& result_type [[maybe_unused]],
                                       const Columns& key_columns [[maybe_unused]], const DataTypes& key_types [[maybe_unused]],
                                       const ColumnPtr& default_values_column [[maybe_unused]], size_t limit [[maybe_unused]]) const {
    throw Exception(ErrorCodes::NOT_IMPLEMENTED, "Method getColumnAllValues is not supported for {} dictionary.",
   ...
   */
  virtual Columns getColumnsAllValues(const Strings& attribute_names, const DataTypes& result_types, const Columns& key_columns,
                                      const DataTypes& key_types, const Columns& default_values_columns, size_t limit) const {
    size_t attribute_names_size = attribute_names.size();
    ...
    for (size_t i = 0; i < attribute_names_size; ++i) {
      ...
      result.emplace_back(getColumnAllValues(attribute_name, result_type, key_columns, key_types, default_values_column, limit));
    }
   ...
   */
  virtual ColumnUInt8::Ptr hasKeys(const Columns& key_columns, const DataTypes& key_types) const = 0;

  virtual bool hasHierarchy() const { return false; }

  virtual ColumnPtr getHierarchy(ColumnPtr key_column [[maybe_unused]], const DataTypePtr& key_type [[maybe_unused]]) const {
    throw Exception(ErrorCodes::NOT_IMPLEMENTED, "Method getHierarchy is not supported for {} dictionary.",
                    getDictionaryID().getNameForLogs());
  }

  virtual ColumnUInt8::Ptr isInHierarchy(ColumnPtr key_column [[maybe_unused]], ColumnPtr in_key_column [[maybe_unused]],
                                         const DataTypePtr& key_type [[maybe_unused]]) const {
    throw Exception(ErrorCodes::NOT_IMPLEMENTED, "Method isInHierarchy is not supported for {} dictionary.",
                    getDictionaryID().getNameForLogs());
  }

  virtual DictionaryHierarchicalParentToChildIndexPtr getHierarchicalIndex() const {
    throw Exception(ErrorCodes::NOT_IMPLEMENTED, "Method getHierarchicalIndex is not supported for {} dictionary.",
                    getDictionaryID().getNameForLogs());
  }

  virtual size_t getHierarchicalIndexBytesAllocated() const { return 0; }

  virtual ColumnPtr getDescendants(ColumnPtr key_column [[maybe_unused]], const DataTypePtr& key_type [[maybe_unused]],
                                   size_t level [[maybe_unused]],
                                   DictionaryHierarchicalParentToChildIndexPtr parent_to_child_index [[maybe_unused]]) const {
    throw Exception(ErrorCodes::NOT_IMPLEMENTED, "Method getDescendants is not supported for {} dictionary.",
                    getDictionaryID().getNameForLogs());
  }
  ...
  std::shared_ptr<IDictionary> shared_from_this() { return std::static_pointer_cast<IDictionary>(IExternalLoadable::shared_from_this()); }
  ...
  Chunk getByKeys(const ColumnsWithTypeAndName& keys, const Names& result_names, PaddedPODArray<UInt8>& out_null_map,
                  IColumn::Offsets& /* out_offsets */) const override {
    if (keys.empty()) return Chunk(getSampleBlock(result_names).cloneEmpty().getColumns(), 0);
    ...
    if (!result_names.empty()) {
      for (const auto& attr_name : result_names) {
        if (!dictionary_structure.hasAttribute(attr_name)) continue;  /// skip keys
  ...
  Block getSampleBlock(const Names& result_names) const override {
    ...
    for (size_t i = 0; i < key_types.size(); ++i) sample_block.insert(ColumnWithTypeAndName(nullptr, key_types.at(i), key_names.at(i)));

    if (result_names.empty()) {
      for (const auto& attr : dictionary_structure.attributes) sample_block.insert(ColumnWithTypeAndName(nullptr, attr.type, attr.name));
    } else {
      for (const auto& attr_name : result_names) {
        if (!dictionary_structure.hasAttribute(attr_name)) continue;  /// skip keys
        const auto& attr = dictionary_structure.getAttribute(attr_name);
