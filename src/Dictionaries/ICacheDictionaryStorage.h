...
struct KeyState {
  ...
  KeyState(State state_, size_t fetched_column_index_) : state(state_), fetched_column_index(fetched_column_index_) {}
...
class ICacheDictionaryStorage {
 public:
  ...
  virtual SimpleKeysStorageFetchResult fetchColumnsForKeys(const PaddedPODArray<UInt64>& keys,
  ...
  /// Fetch columns for keys, this method is not write thread safe
  virtual ComplexKeysStorageFetchResult fetchColumnsForKeys(const PaddedPODArray<std::string_view>& keys,
