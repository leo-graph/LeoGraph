...
class IPolygonDictionary : public IDictionary {
 public:
   ...
   */
  enum class InputType : uint8_t { MultiPolygon, SimplePolygon };
  ...
  IPolygonDictionary(const StorageID &dict_id_, const DictionaryStructure &dict_struct_, DictionarySourcePtr source_ptr_,
                     DictionaryLifetime dict_lifetime_, Configuration configuration_);
 ...
 private:
  ...
  void getItemsImpl(const VectorWithMemoryTracking<IPolygonDictionary::Point> &requested_key_points, ValueGetter &&get_value,
                    ValueSetter &&set_value, DefaultValueExtractor &default_value_extractor) const;

  template <typename AttributeType, typename ValueGetter, typename ValueSetter>
  void getItemsShortCircuitImpl(const VectorWithMemoryTracking<IPolygonDictionary::Point> &requested_key_points, ValueGetter &&get_value,
