...
class CacheDictionaryUpdateUnit {
 public:
  ...
  explicit CacheDictionaryUpdateUnit(const Columns& key_columns_, const PaddedPODArray<KeyState>& key_index_to_state_from_storage_,
                                     const DictionaryStorageFetchRequest& request_, size_t keys_to_update_size_)
  ...
  CacheDictionaryUpdateUnit() : keys_to_update_size(0), alive_keys(CurrentMetrics::CacheDictionaryUpdateQueueKeys, 0) {}
...
class CacheDictionaryUpdateQueue {
 public:
  ...
  CacheDictionaryUpdateQueue(String dictionary_name_for_logs_, CacheDictionaryUpdateQueueConfiguration configuration_,
