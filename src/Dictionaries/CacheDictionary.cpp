...
CacheDictionary<dictionary_key_type>::CacheDictionary(const StorageID& dict_id_, const DictionaryStructure& dict_struct_,
                                                      DictionarySourcePtr source_ptr_, CacheDictionaryStoragePtr cache_storage_ptr_,
    ...
    : IDictionary(dict_id_),
      ...
      update_queue(dict_id_.getNameForLogs(), update_queue_configuration_,
                   [this](CacheDictionaryUpdateUnitPtr<dictionary_key_type> unit_to_update) { update(unit_to_update); }),
...
template <DictionaryKeyType dictionary_key_type>
ColumnPtr CacheDictionary<dictionary_key_type>::getColumn(const std::string& attribute_name, const DataTypePtr& attribute_type,
...
template <DictionaryKeyType dictionary_key_type>
Columns CacheDictionary<dictionary_key_type>::getColumns(const Strings& attribute_names, const DataTypes& attribute_types,
                                                         const Columns& key_columns, const DataTypes& key_types,
  ...
  DictionaryStorageFetchRequest request(
      dict_struct, attribute_names, attribute_types,
  ...
  if (is_short_circuit) default_mask = &std::get<RefFilter>(defaults_or_filter).get();
  ...
  if (not_found_keys_size == 0 && expired_keys_size == 0) {
    ...
    if (source_returns_fetched_columns_in_order_of_keys) return request.filterRequestedColumns(fetched_columns_from_storage);
  ...
  auto update_unit = std::make_shared<CacheDictionaryUpdateUnit<dictionary_key_type>>(key_columns, key_index_to_state_from_storage, request,
                                                                                      keys_to_update_size);
  ...
  MutableColumns aggregated_columns =
      aggregateColumns(keys, request, fetched_columns_from_storage, key_index_to_state_from_storage, fetched_columns_during_update,
                       requested_keys_to_fetched_columns_during_update_index, default_mask);
...
ColumnUInt8::Ptr CacheDictionary<dictionary_key_type>::hasKeys(const Columns& key_columns, const DataTypes& key_types) const {
  ...
  auto update_unit = std::make_shared<CacheDictionaryUpdateUnit<dictionary_key_type>>(
      key_columns, result_of_fetch_from_storage.key_index_to_state, request, keys_to_update_size);
  ...
  if (not_found_keys_size == 0 && expired_keys_size == 0) {
    ...
    if (result_of_fetch_from_storage.default_keys_size == 0) return ColumnUInt8::create(keys.size(), true);
  ...
  for (size_t key_index = 0; key_index < keys.size(); ++key_index) {
    ...
    bool valid_expired_key =
        allow_expired_keys_during_aggregation && result_of_fetch_from_storage.key_index_to_state[key_index].isExpired();
...
template <DictionaryKeyType dictionary_key_type>
ColumnPtr CacheDictionary<dictionary_key_type>::getHierarchy(ColumnPtr key_column [[maybe_unused]],
...
template <DictionaryKeyType dictionary_key_type>
ColumnUInt8::Ptr CacheDictionary<dictionary_key_type>::isInHierarchy(ColumnPtr key_column [[maybe_unused]],
...
template <DictionaryKeyType dictionary_key_type>
MutableColumns CacheDictionary<dictionary_key_type>::aggregateColumnsInOrderOfKeys(const PaddedPODArray<KeyType>& keys,
  ...
  for (size_t fetch_request_index = 0; fetch_request_index < request.attributesSize(); ++fetch_request_index) {
    if (!request.shouldFillResultColumnWithIndex(fetch_request_index)) continue;

    if (default_mask) default_mask->resize(keys.size());
...
MutableColumns CacheDictionary<dictionary_key_type>::aggregateColumns(
    const PaddedPODArray<KeyType>& keys, const DictionaryStorageFetchRequest& request, const MutableColumns& fetched_columns_from_storage,
    const PaddedPODArray<KeyState>& key_index_to_fetched_columns_from_storage_result, const MutableColumns& fetched_columns_during_update,
    const HashMap<KeyType, size_t>& found_keys_to_fetched_columns_during_update_index, IColumn::Filter* default_mask) const {
  /**
...
void CacheDictionary<dictionary_key_type>::update(CacheDictionaryUpdateUnitPtr<dictionary_key_type> update_unit_ptr) {
  ...
  if (now > backoff_end_time.load()) {
    try {
      ...
      for (auto& cell : not_found_keys) not_found_keys_in_source.emplace_back(cell.getKey());
      ...
      tryLogException(last_exception, log,
                      "Could not update cache dictionary '" + getDictionaryID().getNameForLogs() + "', next update is scheduled at " +
                          to_string(backoff_end_time.load()));
      try {
        std::rethrow_exception(last_exception);
      } catch (...) {
        throw DB::Exception(ErrorCodes::CACHE_DICTIONARY_UPDATE_FAIL, "Update failed for dictionary {} : {}",
    ...
    throw DB::Exception(ErrorCodes::CACHE_DICTIONARY_UPDATE_FAIL,
                        ...
                        "Try again later.",
                        getDictionaryID().getNameForLogs(), to_string(backoff_end_time.load()));
  }
}
