...
HashedArrayDictionary<dictionary_key_type, sharded>::HashedArrayDictionary(const StorageID &dict_id_,
...
template <DictionaryKeyType dictionary_key_type, bool sharded>
ColumnPtr HashedArrayDictionary<dictionary_key_type, sharded>::getColumn(const std::string &attribute_name,
...
template <DictionaryKeyType dictionary_key_type, bool sharded>
Columns HashedArrayDictionary<dictionary_key_type, sharded>::getColumns(const Strings &attribute_names, const DataTypes &attribute_types,
  ...
  if (attribute_names.size() > 1) {
    ...
    for (size_t key_index = 0; key_index < keys_size; ++key_index) {
      ...
      if (it == key_attribute_container.end()) {
        ...
        if (default_mask) (*default_mask)[key_index] = 1;
      } else {
        if constexpr (sharded)
        ...
        if (default_mask) (*default_mask)[key_index] = 0;
  ...
  for (size_t i = 0; i < attribute_names_size; ++i) {
    ...
    if (is_short_circuit) {
      if (attribute_names_size > 1)
        result_column = getAttributeColumn(attribute, dictionary_attribute, keys_size, *default_mask, key_index_to_element_index);
      else
        result_column = getAttributeColumn(attribute, dictionary_attribute, keys_size, *default_mask, extractor);
    } else {
...
template <DictionaryKeyType dictionary_key_type, bool sharded>
ColumnUInt8::Ptr HashedArrayDictionary<dictionary_key_type, sharded>::hasKeys(const Columns &key_columns,
...
template <DictionaryKeyType dictionary_key_type, bool sharded>
ColumnPtr HashedArrayDictionary<dictionary_key_type, sharded>::getHierarchy(ColumnPtr key_column [[maybe_unused]],
  ...
  if constexpr (dictionary_key_type == DictionaryKeyType::Simple) {
    ...
    auto get_parent_func = [&, this](auto &hierarchy_key) {
      ...
      if (it == key_attribute_container.end()) return result;

      size_t key_index = it->getMapped();

      if (unlikely(hierarchical_attribute.is_index_null) && (*hierarchical_attribute.is_index_null)[shard][key_index]) return result;
...
template <DictionaryKeyType dictionary_key_type, bool sharded>
ColumnUInt8::Ptr HashedArrayDictionary<dictionary_key_type, sharded>::isInHierarchy(ColumnPtr key_column [[maybe_unused]],
...
DictionaryHierarchicalParentToChildIndexPtr HashedArrayDictionary<dictionary_key_type, sharded>::getHierarchicalIndex() const {
  if constexpr (dictionary_key_type == DictionaryKeyType::Simple) {
    ...
    for (size_t shard = 0; shard < configuration.shards; ++shard) {
      ...
      for (auto &[key, value] : key_attribute.containers[shard]) index_to_key[value] = key;
      ...
      for (size_t i = 0; i < parent_keys_container_size; ++i) {
        if (unlikely(hierarchical_attribute.is_index_null) && (*hierarchical_attribute.is_index_null)[shard][i]) continue;

        const auto *it = index_to_key.find(i);
        if (it == index_to_key.end()) continue;
...
void HashedArrayDictionary<dictionary_key_type, sharded>::createAttributes() {
  ...
  for (const auto &dictionary_attribute : dict_struct.attributes) {
    auto type_call = [&, this](const auto &dictionary_attribute_type) {
      ...
      auto is_index_null = dictionary_attribute.is_nullable
                               ? std::make_optional<VectorWithMemoryTracking<typename Attribute::RowsMask>>(configuration.shards)
                               : std::nullopt;
      Attribute attribute{.containers = AttributeContainerShardsType<ValueType>(configuration.shards),
...
template <DictionaryKeyType dictionary_key_type, bool sharded>
void HashedArrayDictionary<dictionary_key_type, sharded>::blockToAttributes(const Block &block,
                                                                            DictionaryKeysArenaHolder<dictionary_key_type> &arena_holder,
                                                                            size_t shard) {
...
void HashedArrayDictionary<dictionary_key_type, sharded>::resize(size_t total_rows) {
  if (unlikely(!total_rows)) return;

  /// In multi shards configuration it is pointless.
  if constexpr (sharded) return;

  for (auto &container : key_attribute.containers) container.reserve(total_rows);
}

template <DictionaryKeyType dictionary_key_type, bool sharded>
template <typename KeysProvider>
ColumnPtr HashedArrayDictionary<dictionary_key_type, sharded>::getAttributeColumn(const Attribute &attribute,
                                                                                  const DictionaryAttribute &dictionary_attribute,
                                                                                  size_t keys_size, DefaultOrFilter default_or_filter,
    ...
    if (is_short_circuit) {
      ...
      if constexpr (std::is_same_v<ValueType, Array>) {
        ...
        if (is_attribute_nullable)
          ...
          getItemsShortCircuitImpl<ValueType, false>(
              attribute, keys_object, [&](size_t, std::string_view value, bool) { out->insertData(value.data(), value.size()); },
              default_mask);
      } else {
      ...
      if constexpr (std::is_same_v<ValueType, Array>) {
        ...
        getItemsImpl<ValueType, false>(
            attribute, keys_object, [&](const size_t, const Array &value, bool) { out->insert(value); }, default_value_extractor);
      } else if constexpr (std::is_same_v<ValueType, std::string_view>) {
        auto *out = column.get();
        ...
        if (is_attribute_nullable)
          ...
          getItemsImpl<ValueType, false>(
              attribute, keys_object, [&](size_t row, const auto value, bool) { out[row] = value; }, default_value_extractor);
      }
    }
...
void HashedArrayDictionary<dictionary_key_type, sharded>::getItemsImpl(const Attribute &attribute,
...
template <DictionaryKeyType dictionary_key_type, bool sharded>
template <typename AttributeType, bool is_nullable, typename ValueSetter>
void HashedArrayDictionary<dictionary_key_type, sharded>::getItemsShortCircuitImpl(const Attribute &attribute,
                                                                                   const KeyIndexToElementIndex &key_index_to_element_index,
                                                                                   ValueSetter &&set_value,
...
void HashedArrayDictionary<dictionary_key_type, sharded>::loadData() {
  if (!source_ptr->hasUpdateField()) {
    ...
    LOG_DEBUG(
        ...
        "sec",
        configuration.use_async_executor ? "asynchronous " : "", total_blocks, total_rows, dictionary_name,
        static_cast<double>(pull_time_microseconds) / 1000000.0, static_cast<double>(process_time_microseconds) / 1000000.0);
  } else {
    updateData();
  }
...
void HashedArrayDictionary<dictionary_key_type, sharded>::calculateBytesAllocated() {
  ...
  for (const auto &container : key_attribute.containers) bytes_allocated += container.size();
  ...
  for (auto &attribute : attributes) {
    ...
    if (attribute.is_index_null.has_value())
      for (const auto &container : attribute.is_index_null.value()) bytes_allocated += container.size();
  }
  ...
  if (attributes.size()) bucket_count /= attributes.size();
...
Pipe HashedArrayDictionary<dictionary_key_type, sharded>::read(const Names &column_names, size_t max_block_size, size_t num_streams) const {
  ...
  for (const auto &container : key_attribute.containers)
    for (auto &[key, _] : container) keys.emplace_back(key);
...
void registerDictionaryArrayHashed(DictionaryFactory &factory) {
  auto create_layout = [](const std::string &full_name, const DictionaryStructure &dict_struct,
                          const Poco::Util::AbstractConfiguration &config, const std::string &config_prefix, ContextPtr global_context,
                          DictionarySourcePtr source_ptr, DictionaryKeyType dictionary_key_type) -> DictionaryPtr {
    ...
    configuration.use_async_executor =
        clickhouse_source && clickhouse_source->isLocal() && settings[Setting::dictionary_use_async_executor];
    ...
    if (dictionary_key_type == DictionaryKeyType::Simple) {
      if (shards > 1)
        return std::make_unique<HashedArrayDictionary<DictionaryKeyType::Simple, true>>(dict_id, dict_struct, std::move(source_ptr),
                                                                                        configuration);
      return std::make_unique<HashedArrayDictionary<DictionaryKeyType::Simple, false>>(dict_id, dict_struct, std::move(source_ptr),
                                                                                       configuration);
    }

    if (shards > 1)
      return std::make_unique<HashedArrayDictionary<DictionaryKeyType::Complex, true>>(dict_id, dict_struct, std::move(source_ptr),
                                                                                       configuration);
    return std::make_unique<HashedArrayDictionary<DictionaryKeyType::Complex, false>>(dict_id, dict_struct, std::move(source_ptr),
                                                                                      configuration);
  };
