...
struct SSDCacheKey final {
  ...
  SSDCacheKey(KeyType key_, size_t size_, const char *data_) : key(key_), size(size_), data(data_) {}
...
class SSDCacheBlock final {
 ...
 public:
  ...
  explicit SSDCacheBlock(size_t block_size_) : block_size(block_size_) {}
  ...
  bool writeKey(const SSDCacheSimpleKey &cache_key, size_t &offset_in_block) {
    ...
    if (!enoughtPlaceToWriteKey(cache_key)) return false;
  ...
  size_t calculateCheckSum() const {
    size_t calculated_check_sum = static_cast<size_t>(
        CityHash_v1_0_2::CityHash64(block_data + block_header_check_sum_size, block_size - block_header_check_sum_size));
  ...
  void writeCheckSum() {
    size_t check_sum = static_cast<size_t>(
        CityHash_v1_0_2::CityHash64(block_data + block_header_check_sum_size, block_size - block_header_check_sum_size));
...
struct SSDCacheIndex {
  SSDCacheIndex(size_t block_index_, size_t offset_in_block_) : block_index(block_index_), offset_in_block(offset_in_block_) {}
...
class SSDCacheMemoryBuffer {
 public:
  ...
  bool writeKey(const SSDCacheKeyType &key, SSDCacheIndex &index) {
    if (current_block_index == partition_blocks_size) return false;
  ...
  char *getPlace(SSDCacheIndex index) const { return buffer.m_data + index.block_index * block_size + index.offset_in_block; }
...
class SSDCacheFileBuffer : private boost::noncopyable {
 ...
 public:
  ...
  explicit SSDCacheFileBuffer(const std::string &file_path_, size_t block_size_, size_t file_blocks_size_)
      : file_path(file_path_ + BIN_FILE_EXT), block_size(block_size_), file_blocks_size(file_blocks_size_) {
  ...
  void allocateSizeForNextPartition() {
    if (preallocateDiskSpace(file.fd, current_blocks_size * block_size, block_size * file_blocks_size) < 0)
      ErrnoException::throwFromPath(ErrorCodes::CANNOT_ALLOCATE_MEMORY, file_path, "Cannot preallocate space for the file {}", file_path);

    current_blocks_size += file_blocks_size;
  }

  bool writeBuffer(const char *buffer, size_t buffer_size_in_blocks) {
    if (current_block_index + buffer_size_in_blocks > current_blocks_size) return false;
...
#  endif
    ...
    while (io_submit(aio_context.ctx, 1, &write_request_ptr) < 0) {
      if (errno != EINTR) throw Exception(ErrorCodes::CANNOT_IO_SUBMIT, "Cannot submit request for asynchronous IO on file {}", file_path);
    }
    ...
    if (bytes_written != static_cast<decltype(bytes_written)>(block_size * buffer_size_in_blocks))
      throw Exception(ErrorCodes::AIO_WRITE_ERROR, "Not all data was written for asynchronous IO on file {}. returned: {}", file_path,
                      bytes_written);
...
#  if defined(OS_DARWIN)
    if (::fsync(file.fd) < 0) ErrnoException::throwFromPath(ErrorCodes::CANNOT_FSYNC, file_path, "Cannot fsync {}", file_path);
#  else
    if (::fdatasync(file.fd) < 0) ErrnoException::throwFromPath(ErrorCodes::CANNOT_FSYNC, file_path, "Cannot fdatasync {}", file_path);
#  endif
  ...
  bool readKeys(size_t block_start, size_t blocks_length, PaddedPODArray<KeyType> &out) const {
    if (block_start + blocks_length > current_blocks_size) return false;
...
#  endif
    ...
    while (io_submit(aio_context.ctx, 1, &request_ptr) != 1) {
      if (errno != EINTR) throw ErrnoException(ErrorCodes::CANNOT_IO_SUBMIT, "io_submit: Failed to submit a request for asynchronous IO");
    }

    while (io_getevents(aio_context.ctx, 1, 1, &event, nullptr) != 1) {
      if (errno != EINTR) throw ErrnoException(ErrorCodes::CANNOT_IO_GETEVENTS, "io_getevents: Failed to get an event for asynchronous IO");
    }

    auto read_bytes = eventResult(event);

    if (read_bytes != static_cast<ssize_t>(buffer_size_in_bytes))
      throw Exception(ErrorCodes::AIO_READ_ERROR, "GC: AIO failed to read file {}. Expected bytes {}. Actual bytes {}", file_path,
                      buffer_size_in_bytes, read_bytes);
  ...
  void fetchBlocks(size_t read_from_file_buffer_blocks_size, const PaddedPODArray<size_t> &blocks_to_fetch, FetchBlockFunc &&func) const {
    if (blocks_to_fetch.empty()) return;
...
#  endif
    ...
    while (to_pop < requests.size()) {
      ...
      while (to_pop < to_push &&
             (popped = io_getevents(aio_context.ctx, to_push - to_pop, to_push - to_pop, &events[to_pop], nullptr)) <= 0) {
        if (errno != EINTR)
      ...
      while (to_pop < requests.size() && processed[to_pop]) ++to_pop;

      /// add new io tasks
      const int new_tasks_count =
          static_cast<int>(std::min(read_from_file_buffer_blocks_size - (to_push - to_pop), requests.size() - to_push));
  ...
  void reset() { current_block_index = 0; }
...
class SSDCacheDictionaryStorage final : public ICacheDictionaryStorage {
 public:
  ...
  String getName() const override {
    if (dictionary_key_type == DictionaryKeyType::Simple) return "SSDCache";
  ...
  SimpleKeysStorageFetchResult fetchColumnsForKeys(const PaddedPODArray<UInt64> &keys, const DictionaryStorageFetchRequest &fetch_request,
  ...
  ComplexKeysStorageFetchResult fetchColumnsForKeys(const PaddedPODArray<std::string_view> &keys,
  ...
  double getLoadFactor() const override {
    ...
    if (partitions_size == configuration.max_partitions_count) return 1.0;
  ...
  size_t getBytesAllocated() const override {
    size_t memory_partitions_bytes_size =
        memory_buffer_partitions.size() * configuration.write_buffer_blocks_size * configuration.block_size;
 ...
 private:
  ...
  struct Cell {
    enum CellState { in_memory, on_disk, default_value };
  ...
  struct KeyToBlockOffset {
    KeyToBlockOffset(size_t key_index_, size_t offset_in_block_) : key_index(key_index_), offset_in_block(offset_in_block_) {}
  ...
  Result fetchColumnsForKeysImpl(const PaddedPODArray<KeyType> &keys, const DictionaryStorageFetchRequest &fetch_request,
    ...
    for (size_t attribute_size = 0; attribute_size < fetch_request.attributesSize(); ++attribute_size)
      if (fetch_request.shouldFillResultColumnWithIndex(attribute_size)) result.fetched_columns[attribute_size]->reserve(keys_size);
    ...
    for (size_t key_index = 0; key_index < keys_size; ++key_index) {
      ...
      switch (cell.state) {
        case Cell::in_memory: {
          result.key_index_to_state[key_index] = {key_state, default_mask ? fetched_columns_index_without_default : fetched_columns_index};
        ...
        case Cell::on_disk: {
          ...
          if (was_inserted) blocks_to_request.emplace_back(cell.index.block_index);
        ...
        case Cell::default_value: {
          ...
          if (!default_mask) insertDefaultValuesIntoColumns(result.fetched_columns, fetch_request, key_index);
          break;
        }
      }
    }
    ...
    file_buffer.fetchBlocks(configuration.read_buffer_blocks_size, blocks_to_request, [&](size_t block_index, char *block_data) {
      ...
      for (auto &key_in_block : keys_in_block) {
        ...
        result.key_index_to_state[key_in_block.key_index].setFetchedColumnIndex(default_mask ? fetched_columns_index_without_default
                                                                                             : fetched_columns_index);
  ...
  PaddedPODArray<KeyType> getCachedKeysImpl() const {
    ...
    for (auto &node : index) {
      ...
      if (cell.state == Cell::default_value) continue;
      ...
      if (write_into_memory_buffer_result) {
        ...
        if (started_reusing_old_partitions) {
          ...
          for (auto old_key : old_keys) {
            ...
            if (it) {
              ...
              if (old_key_cell.isOnDisk() && old_key_block >= block_index_in_file_before_write && old_key_block < file_read_end_block_index)
        ...
        if (flush_to_file_result) {
          ...
          for (Int64 i = keys_to_update_size - 1; i >= 0; --i) {
            ...
            if (!it || it->getMapped().state != Cell::in_memory) continue;

            /// If there were duplicated keys in memory buffer partition
            if (updated_keys.contains(it->getKey())) continue;
  ...
  void setCellDeadline(Cell &cell, TimePoint now) {
    if (configuration.lifetime.min_sec == 0 && configuration.lifetime.max_sec == 0) {
      auto deadline =
          std::chrono::time_point<std::chrono::system_clock>::max() - 2 * std::chrono::seconds(configuration.strict_max_lifetime_seconds);
  ...
  void eraseKeyFromIndex(KeyType key) {
    ...
    if constexpr (std::is_same_v<KeyType, std::string_view>) complex_key_arena.free(const_cast<char *>(key_copy.data()), key_copy.size());
  }
  ...
  using CacheMap = std::conditional_t<dictionary_key_type == DictionaryKeyType::Simple, SimpleKeyHashMap, ComplexKeyHashMap>;
