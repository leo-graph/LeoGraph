...
DictionarySourcePtr DictionarySourceFactory::create(const std::string& name, const Poco::Util::AbstractConfiguration& config,
                                                    const std::string& config_prefix, const DictionaryStructure& dict_struct,
  ...
  if (keys.empty() || keys.size() > 2)
    throw Exception(ErrorCodes::EXCESSIVE_ELEMENT_IN_CONFIG, "{}: element dictionary.source should have one or two child elements", name);
  ...
  throw Exception(ErrorCodes::UNKNOWN_ELEMENT_IN_CONFIG, "{}: unknown dictionary source type: {}", name, source_type);
}

void DictionarySourceFactory::checkSourceAvailable(const std::string& source_type, const std::string& dictionary_name,
                                                   const ContextPtr& /* context */) const {
  const auto found = registered_sources.find(source_type);
  if (found == registered_sources.end()) {
    throw Exception(ErrorCodes::UNKNOWN_ELEMENT_IN_CONFIG, "{}: unknown dictionary source type: {}", dictionary_name, source_type);
  }
}
