...
Block blockForKeys(const DictionaryStructure& dict_struct, const Columns& key_columns,
  ...
  for (size_t i = 0, size = key_columns.size(); i < size; ++i) {
    ...
    for (size_t idx : requested_rows) filter[idx] = true;
...
ContextMutablePtr copyContextAndApplySettingsFromDictionaryConfig(const ContextPtr& context,
...
static Block transformHeader(Block header, Block block_to_add) {
  for (Int64 i = static_cast<Int64>(block_to_add.columns() - 1); i >= 0; --i) header.insert(0, block_to_add.getByPosition(i).cloneEmpty());

  return header;
}

TransformWithAdditionalColumns::TransformWithAdditionalColumns(SharedHeader block_to_add_, SharedHeader header)
...
String TransformWithAdditionalColumns::getName() const { return "TransformWithAdditionalColumns"; }
