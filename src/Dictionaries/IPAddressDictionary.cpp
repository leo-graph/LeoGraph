...
struct IPRecord {
  ...
  IPRecord(const Poco::Net::IPAddress &addr_, UInt8 prefix_, size_t row_)
      : addr(addr_), prefix(prefix_), row(row_), isv6(addr.family() == Poco::Net::IPAddress::IPv6) {}

  const uint8_t *asIPv6Binary(uint8_t *buf) const {
    if (isv6) return reinterpret_cast<const uint8_t *>(addr.addr());
    memset(buf, 0, 10);
    buf[10] = '\xFF';
    buf[11] = '\xFF';
  ...
  UInt8 prefixIPv6() const { return isv6 ? prefix : prefix + 96; }
};
...
static std::pair<Poco::Net::IPAddress, UInt8> parseIPFromString(const std::string_view addr_str) {
  try {
    ...
    if (pos != std::string::npos) {
      ...
      if (ec != std::errc()) throw DB::Exception(ErrorCodes::CANNOT_PARSE_NUMBER, "Mask for IP address is not a valid number");
    ...
    throw DB::Exception(ErrorCodes::CANNOT_PARSE_INPUT_ASSERTION_FAILED, "Can't parse address \"{}\": {}", std::string(addr_str),
                        ex.what());
  }
}
...
static void validateKeyTypes(const DataTypes &key_types) {
  if (key_types.empty() || key_types.size() > 2) throw Exception(ErrorCodes::TYPE_MISMATCH, "Expected a single IP address or IP with mask");

  TypeIndex type_id = key_types[0]->getTypeId();
  const auto *key_string = typeid_cast<const DataTypeFixedString *>(key_types[0].get());

  if (type_id != TypeIndex::IPv4 && type_id != TypeIndex::UInt32 && type_id != TypeIndex::IPv6 &&
      !(key_string && key_string->getN() == IPV6_BINARY_LENGTH))
    throw Exception(ErrorCodes::TYPE_MISMATCH, "Key does not match, expected either IPv4 (or UInt32) or IPv6 (or FixedString(16))");

  if (key_types.size() > 1) {
    const auto *mask_col_type = typeid_cast<const DataTypeUInt8 *>(key_types[1].get());
    if (mask_col_type == nullptr) throw Exception(ErrorCodes::TYPE_MISMATCH, "Mask do not match, expected UInt8");
  }
}

template <typename T, typename Comp>
size_t sortAndUnique(VectorWithMemoryTracking<T> &vec, Comp comp) {
  ::sort(vec.begin(), vec.end(), [&](const auto &a, const auto &b) { return comp(a, b) < 0; });

  auto new_end = std::unique(vec.begin(), vec.end(), [&](const auto &a, const auto &b) { return comp(a, b) == 0; });
...
inline static UInt32 IPv4AsUInt32(const void *addr) { return Poco::ByteOrder::fromNetwork(*reinterpret_cast<const UInt32 *>(addr)); }

/// Convert mapped IPv6 to IPv4 if possible
inline static UInt32 mappedIPv4ToBinary(const uint8_t *addr, bool &success) {
  success = addr[0] == 0x0 && addr[1] == 0x0 && addr[2] == 0x0 && addr[3] == 0x0 && addr[4] == 0x0 && addr[5] == 0x0 && addr[6] == 0x0 &&
            addr[7] == 0x0 && addr[8] == 0x0 && addr[9] == 0x0 && addr[10] == 0xff && addr[11] == 0xff;
  if (!success) return 0;
...
IPAddressDictionary::IPAddressDictionary(const StorageID &dict_id_, const DictionaryStructure &dict_struct_,
                                         DictionarySourcePtr source_ptr_, IPAddressDictionary::Configuration configuration_)
...
ColumnPtr IPAddressDictionary::getColumn(const std::string &attribute_name, const DataTypePtr &attribute_type, const Columns &key_columns,
                                         const DataTypes &key_types, DefaultOrFilter default_or_filter) const {
  ...
  auto type_call = [&](const auto &dictionary_attribute_type) {
    ...
    if (is_short_circuit) {
      ...
      if constexpr (std::is_same_v<ValueType, Array>) {
        ...
        getItemsShortCircuitImpl<ValueType>(
            attribute, key_columns, [&](const size_t, std::string_view value) { out->insertData(value.data(), value.size()); },
            default_mask);
      } else {
      ...
      if constexpr (std::is_same_v<ValueType, Array>) {
        ...
        getItemsImpl<ValueType>(
            attribute, key_columns, [&](const size_t, const Array &value) { out->insert(value); }, default_value_extractor);
      } else if constexpr (std::is_same_v<ValueType, std::string_view>) {
        auto *out = column.get();
        ...
        getItemsImpl<ValueType>(
            attribute, key_columns, [&](const size_t row, const auto value) { return out[row] = value; }, default_value_extractor);
      }
    }
...
ColumnUInt8::Ptr IPAddressDictionary::hasKeys(const Columns &key_columns, const DataTypes &key_types) const {
  ...
  if (type_id == TypeIndex::IPv4 || type_id == TypeIndex::UInt32) {
    ...
    for (const auto i : collections::range(0, rows)) {
      ...
      if (addr.size() != IPV6_BINARY_LENGTH) throw Exception(ErrorCodes::TYPE_MISMATCH, "Expected key FixedString(16)");
  ...
  auto create_attributes_from_dictionary_attributes = [this](const VectorWithMemoryTracking<DictionaryAttribute> &dict_attrs) {
    ...
    for (const auto &attribute : dict_attrs) {
      if (attribute.is_nullable)
        throw Exception(ErrorCodes::UNSUPPORTED_METHOD, "{}: array or nullable attributes not supported for dictionary of type {}",
                        getFullName(), getTypeName());

      attribute_index_by_name.emplace(attribute.name, attributes.size());
      attributes.push_back(createAttributeWithType(attribute.underlying_type, attribute.null_value));

      if (attribute.hierarchical)
        throw Exception(ErrorCodes::TYPE_MISMATCH, "{}: hierarchical attributes not supported for dictionary of type {}", getFullName(),
                        getTypeName());
    }
  };

  create_attributes_from_dictionary_attributes(dict_struct.attributes);
  if (access_to_key_from_attributes) create_attributes_from_dictionary_attributes(*dict_struct.key);
}
...
void IPAddressDictionary::loadData() {
  ...
  bool has_ipv6 = false;
  {
    ...
    io.executeWithCallbacks([&]() {
      ...
      while (executor.pull(block)) {
        ...
        const auto attribute_column_ptrs = Columns{std::from_range_t{}, collections::range(0, dict_struct.attributes.size()) |
                                                                            std::views::transform([&](const size_t attribute_idx) {
  ...
  if (has_ipv6) {
    auto deleted_count = sortAndUnique(ip_records, [](const auto &record_a, const auto &record_b) {
      ...
      if (cmpres == 0) return compareTo(record_a.prefixIPv6(), record_b.prefixIPv6());
      return cmpres;
    });
    if (deleted_count > 0) LOG_TRACE(logger, "removing {} non-unique subnets from input", deleted_count);
    ...
    auto deleted_count = sortAndUnique(ip_records, [](const auto &record_a, const auto &record_b) {
      ...
      if (a == b) return compareTo(record_a.prefix, record_b.prefix);
...
void IPAddressDictionary::getItemsByTwoKeyColumnsImpl(const Attribute &attribute, const Columns &key_columns, ValueSetter &&set_value,
  ...
  if (const auto *ipv4_col = std::get_if<IPv4Container>(&ip_column)) {
    ...
    if (key_ip_column_ptr == nullptr) throw Exception(ErrorCodes::TYPE_MISMATCH, "Expected a UInt32 IP column");
    ...
    auto comp_v4 = [&](size_t elem, const IPv4Subnet &target) {
      ...
      if (addr == target.addr) return mask_column[elem] < target.prefix;
    ...
    for (const auto i : collections::range(0, rows)) {
      ...
      if (likely(found_it != range.end() && (*ipv4_col)[*found_it] == addr && mask_column[*found_it] == mask)) {
  ...
  for (const auto i : collections::range(0, rows)) {
    ...
    if (likely(found_it != range.end() && memequal16(getIPv6FromOffset(*ipv6_col, *found_it), target.addr) &&
...
size_t IPAddressDictionary::getItemsByTwoKeyColumnsShortCircuitImpl(const Attribute &attribute, const Columns &key_columns,
                                                                    ValueSetter &&set_value, IColumn::Filter &default_mask) const {
  const auto &first_column = key_columns.front();
  const size_t rows = first_column->size();
...
template <typename AttributeType, typename ValueSetter, typename DefaultValueExtractor>
void IPAddressDictionary::getItemsImpl(const Attribute &attribute, const Columns &key_columns, ValueSetter &&set_value,
                                       DefaultValueExtractor &default_value_extractor) const {
  const auto &first_column = key_columns.front();
  const size_t rows = first_column->size();

  if (unlikely(key_columns.size() == 2)) {
    getItemsByTwoKeyColumnsImpl<AttributeType>(attribute, key_columns, std::forward<ValueSetter>(set_value), default_value_extractor);
    query_count.fetch_add(rows, std::memory_order_relaxed);
    return;
  }
  ...
  if (type_id == TypeIndex::IPv4 || type_id == TypeIndex::UInt32) {
    ...
    for (const auto i : collections::range(0, rows)) {
      auto addr = first_column->getDataAt(i);
      if (addr.size() != IPV6_BINARY_LENGTH) throw Exception(ErrorCodes::TYPE_MISMATCH, "Expected key to be FixedString(16)");
      auto found = tryLookupIPv6(reinterpret_cast<const uint8_t *>(addr.data()));
      if (found != ipNotFound()) {
        set_value(i, vec[*found]);
        ++keys_found;
...
template <typename AttributeType, typename ValueSetter>
void IPAddressDictionary::getItemsShortCircuitImpl(const Attribute &attribute, const Columns &key_columns, ValueSetter &&set_value,
  ...
  if (unlikely(key_columns.size() == 2)) {
    keys_found =
        getItemsByTwoKeyColumnsShortCircuitImpl<AttributeType>(attribute, key_columns, std::forward<ValueSetter>(set_value), default_mask);
    query_count.fetch_add(rows, std::memory_order_relaxed);
    found_count.fetch_add(keys_found, std::memory_order_relaxed);
    return;
  }
...
static auto keyViewGetter() {
  return [](const Columns &columns, const VectorWithMemoryTracking<DictionaryAttribute> &dictonary_key_attributes) {
    ...
    return ColumnsWithTypeAndName{ColumnWithTypeAndName(std::move(column), std::make_shared<DataTypeString>(), key_attribute.name)};
  };
}
...
Pipe IPAddressDictionary::read(const Names &column_names, size_t max_block_size, size_t num_streams) const {
  ...
  ColumnsWithTypeAndName key_columns_with_type = {ColumnWithTypeAndName(key_columns.front(), key_type, ""),
  ...
  auto coordinator = std::make_shared<DictionarySourceCoordinator>(dictionary, column_names, std::move(key_columns_with_type),
                                                                   std::move(view_columns), max_block_size);
  auto result = coordinator->read(num_streams);

  return result;
}

IPAddressDictionary::RowIdxConstIter IPAddressDictionary::ipNotFound() const { return row_idx.end(); }
...
IPAddressDictionary::RowIdxConstIter IPAddressDictionary::tryLookupIPv6(const uint8_t *addr) const {
  if (std::get_if<IPv4Container>(&ip_column)) {
    ...
    if (!is_mapped) return ipNotFound();
...
IPAddressDictionary::RowIdxConstIter IPAddressDictionary::lookupIP(IPValueType target) const {
  if (row_idx.empty()) return ipNotFound();

  const auto *ipv4or6_col = std::get_if<IPContainerType>(&ip_column);
  if (ipv4or6_col == nullptr) return ipNotFound();
  ...
  if (found_it == range.begin()) return ipNotFound();

  --found_it;
  if constexpr (std::is_same_v<IPContainerType, IPv4Container>) {
    for (auto idx = *found_it;; idx = parent_subnet[idx]) {
      if (matchIPv4Subnet(target, (*ipv4or6_col)[idx], mask_column[idx])) return row_idx.begin() + idx;

      if (idx == parent_subnet[idx]) return ipNotFound();
    }
  } else {
    for (auto idx = *found_it;; idx = parent_subnet[idx]) {
      if (matchIPv6Subnet(target, getIPv6FromOffset(*ipv4or6_col, idx), mask_column[idx])) return row_idx.begin() + idx;
...
void registerDictionaryTrie(DictionaryFactory &factory) {
  auto create_layout = [=](const std::string &, const DictionaryStructure &dict_struct, const Poco::Util::AbstractConfiguration &config,
                           const std::string &config_prefix, DictionarySourcePtr source_ptr, ContextPtr global_context,
    ...
    if (!isString(key_type))
      throw Exception(ErrorCodes::BAD_ARGUMENTS, "Key attribute of ip_trie dictionary must be of type String, got {}", key_type->getName());
