...
class DictionaryHierarchicalParentToChildIndex {
 public:
  ...
  size_t getSizeInBytes() const { return parent_to_children_keys_range.getBufferSizeInBytes() + (keys.size() * sizeof(UInt64)); }
...
ElementsAndOffsets getHierarchy(const PaddedPODArray<UInt64>& keys, IsKeyValidFunc&& is_key_valid_func,
  ...
  for (size_t i = 0; i < hierarchy_keys_size; ++i) {
    ...
    while (true) {
      ...
      if (current_hierarchy_depth >= DBMS_HIERARCHICAL_DICTIONARY_MAX_DEPTH) break;
 ...
 */
template <typename IsKeyValidFunc, typename GetParentKeyFunc>
PaddedPODArray<UInt8> getIsInHierarchy(const PaddedPODArray<UInt64>& keys, const PaddedPODArray<UInt64>& in_keys,
                                       IsKeyValidFunc&& is_key_valid_func, GetParentKeyFunc&& get_parent_func) {
  assert(keys.size() == in_keys.size());

  PaddedPODArray<UInt8> result;
  result.resize_fill(keys.size());

  detail::ElementsAndOffsets hierarchy =
      detail::getHierarchy(keys, std::forward<IsKeyValidFunc>(is_key_valid_func), std::forward<GetParentKeyFunc>(get_parent_func));
...
struct GetAllDescendantsStrategy {
  size_t level = 0;
};
struct GetDescendantsAtSpecificLevelStrategy {
  size_t level = 0;
};
...
ElementsAndOffsets getDescendants(const PaddedPODArray<UInt64>& keys, const DictionaryHierarchicalParentToChildIndex& parent_to_child_index,
                                  Strategy strategy, size_t& valid_keys) {
  ...
  if constexpr (std::is_same_v<Strategy, GetAllDescendantsStrategy>) already_processed_keys_to_range.reserve(keys_size);
  ...
  for (size_t i = 0; i < keys_size; ++i) {
    ...
    while (!next_keys_to_process_stack.empty()) {
      ...
      if (!it || depth >= DBMS_HIERARCHICAL_DICTIONARY_MAX_DEPTH) continue;
      ...
      for (; children_range.start_index < children_range.end_index; ++children_range.start_index) {
        ...
        if constexpr (std::is_same_v<Strategy, GetAllDescendantsStrategy>) descendants.emplace_back(child_key);
...
ColumnPtr getKeysHierarchyArray(const PaddedPODArray<KeyType>& keys, IsKeyValidFunc&& is_key_valid_func,
                                GetParentKeyFunc&& get_parent_func) {
  auto elements_and_offsets =
      detail::getHierarchy(keys, std::forward<IsKeyValidFunc>(is_key_valid_func), std::forward<GetParentKeyFunc>(get_parent_func));
...
template <typename KeyType, typename IsKeyValidFunc, typename GetParentKeyFunc>
ColumnUInt8::Ptr getKeysIsInHierarchyColumn(const PaddedPODArray<KeyType>& hierarchy_keys, const PaddedPODArray<KeyType>& hierarchy_in_keys,
                                            IsKeyValidFunc&& is_key_valid_func, GetParentKeyFunc&& get_parent_func) {
  auto is_in_hierarchy_data = detail::getIsInHierarchy(hierarchy_keys, hierarchy_in_keys, std::forward<IsKeyValidFunc>(is_key_valid_func),
...
///
/// @param valid_keys - number of keys that are valid in parent_to_child map
ColumnPtr getKeysDescendantsArray(const PaddedPODArray<UInt64>& requested_keys,
                                  const DictionaryHierarchicalParentToChildIndex& parent_to_child_index, size_t level, size_t& valid_keys);
 ...
 */
ColumnUInt8::Ptr getKeysIsInHierarchyDefaultImplementation(const IDictionary* dictionary, ColumnPtr key_column, ColumnPtr in_key_column,
                                                           const DataTypePtr& key_type, size_t& valid_keys);
