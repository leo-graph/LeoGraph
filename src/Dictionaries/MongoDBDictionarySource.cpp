...
#if USE_MONGODB
  auto create_dictionary_source = [](const String& /*name*/, const DictionaryStructure& dict_struct,
                                     const Poco::Util::AbstractConfiguration& config, const std::string& root_config_prefix,
                                     Block& sample_block, ContextPtr context, const std::string& /* default_database */,
    ...
    if (auto named_collection = tryGetNamedCollectionWithOverrides(config, config_prefix, context)) {
      if (named_collection->has("uri")) {
        if (named_collection->has("options"))
          throw Exception(ErrorCodes::BAD_ARGUMENTS,
                          "The 'options' key should not be set when using 'uri', as connection options are already part of the URI");
        ...
        if (!user.empty()) auth_string = fmt::format("{}:{}@", user, escaped_password);
        configuration->uri = std::make_unique<mongocxx::uri>(
            fmt::format("mongodb://{}{}:{}/{}?{}", auth_string, named_collection->get<String>("host"),
                        named_collection->getOrDefault<String>("port", "27017"), named_collection->get<String>("db"),
      ...
      if (!uri_str.empty())
        ...
        if (!user.empty()) auth_string = fmt::format("{}:{}@", user, escaped_password);
        configuration->uri = std::make_unique<mongocxx::uri>(
            fmt::format("mongodb://{}{}:{}/{}?{}", auth_string, config.getString(config_prefix + ".host"),
                        config.getString(config_prefix + ".port", "27017"), config.getString(config_prefix + ".db"),
...
MongoDBDictionarySource::MongoDBDictionarySource(const DictionaryStructure& dict_struct_,
                                                 std::shared_ptr<MongoDBConfiguration> configuration_, SharedHeader sample_block_)
    : dict_struct{dict_struct_}, configuration{configuration_}, sample_block{sample_block_} {}
...
BlockIO MongoDBDictionarySource::loadAll() {
  BlockIO io;
  io.pipeline = QueryPipeline(std::make_shared<MongoDBSource>(*configuration->uri, configuration->collection, make_document(),
                                                              mongocxx::options::find(), sample_block, max_block_size));
  return io;
}

BlockIO MongoDBDictionarySource::loadIds(const VectorWithMemoryTracking<UInt64>& ids) {
  if (!dict_struct.id) throw Exception(ErrorCodes::UNSUPPORTED_METHOD, "'id' is required for selective loading");

  auto ids_array = array();
  for (const auto& id : ids) ids_array.append(static_cast<Int64>(id));

  BlockIO io;
  io.pipeline = QueryPipeline(std::make_shared<MongoDBSource>(
      *configuration->uri, configuration->collection, make_document(kvp(dict_struct.id->name, make_document(kvp("$in", ids_array)))),
      mongocxx::options::find(), sample_block, max_block_size));
  return io;
}

BlockIO MongoDBDictionarySource::loadKeys(const Columns& key_columns, const VectorWithMemoryTracking<size_t>& requested_rows) {
  if (!dict_struct.key) throw Exception(ErrorCodes::UNSUPPORTED_METHOD, "'key' is required for selective loading");
  ...
  BlockIO io;
  io.pipeline =
      QueryPipeline(std::make_shared<MongoDBSource>(*configuration->uri, configuration->collection, make_document(kvp("$or", keys)),
                                                    mongocxx::options::find(), sample_block, max_block_size));
  return io;
}

std::string MongoDBDictionarySource::toString() const { return fmt::format("MongoDB: {}", configuration->uri->to_string()); }
#endif
