...
RedisSource::RedisSource(RedisConnectionPtr connection_, const RedisArray &keys_, const RedisStorageType &storage_type_,
                         SharedHeader sample_block, size_t max_block_size_)
    : ISource(sample_block), connection(std::move(connection_)), keys(keys_), storage_type(storage_type_), max_block_size{max_block_size_} {
...
void insertValue(IColumn &column, const ValueType type, const Poco::Redis::BulkString &bulk_string) {
  if (bulk_string.isNull()) throw Exception(ErrorCodes::TYPE_MISMATCH, "Type mismatch, expected not Null String");
...
Chunk RedisSource::generate() {
  if (keys.isNull() || description.sample_block.rows() == 0 || cursor >= keys.size()) all_read = true;
  ...
  for (size_t i = 0; i < size; ++i) columns[i] = description.sample_block.getByPosition(i).column->cloneEmpty();
  ...
  if (storage_type == RedisStorageType::HASH_MAP) {
    ...
    for (; cursor < keys.size(); ++cursor) {
      ...
      if (keys_array.size() < 2) {
        throw Exception(ErrorCodes::LOGICAL_ERROR, "Too low keys in request to source: {}, expected 2 or more", keys_array.size());
      }

      if (num_rows + keys_array.size() - 1 > max_block_size) break;

      Poco::Redis::Command command_for_values("HMGET");
      for (const auto &elem : keys_array) command_for_values.addRedisType(elem);
      ...
      if (keys_array.size() != values.size() + 1)  // 'HMGET' primary_key secondary_keys
        throw Exception(ErrorCodes::NUMBER_OF_COLUMNS_DOESNT_MATCH, "Inconsistent sizes of keys and values in Redis request");
    ...
    for (size_t i = 0; i < need_values; ++i) command_for_values.add(keys.get<RedisBulkString>(cursor + i));

    auto values = connection->client->execute<RedisArray>(command_for_values);
    if (values.size() != need_values)
      throw Exception(ErrorCodes::INTERNAL_REDIS_ERROR, "Inconsistent sizes of keys and values in Redis request");
