...
constexpr bool isPodLayout() {
  if constexpr (std::is_same_v<V, UUID>) return false;
  if constexpr (std::is_same_v<V, DateTime64>) return false;
  if constexpr (std::is_same_v<V, Decimal32> || std::is_same_v<V, Decimal64> || std::is_same_v<V, Decimal128> ||
                std::is_same_v<V, Decimal256>)
    return false;
  if constexpr (std::is_same_v<V, std::string_view>) return false;
  if constexpr (std::is_same_v<V, IPv6> || std::is_same_v<V, IPv4>) return false;
  return true;
}
...
constexpr bool useSparseHashForHashedDictionary() {
  if constexpr (!isPodLayout<K>()) return true;
  if constexpr (!isPodLayout<V>()) return true;
...
class alignas(CH_CACHE_LINE_SIZE) HashTableGrowerWithPrecalculationAndMaxLoadFactor {
 ...
 public:
  ...
  explicit HashTableGrowerWithPrecalculationAndMaxLoadFactor(float max_load_factor_) : max_load_factor(max_load_factor_) {
  ...
  void set(size_t num_elems) {
    if (num_elems <= 1)
      ...
      if (static_cast<double>(1ULL << size_degree) * max_load_factor < static_cast<double>(num_elems)) ++size_degree;
    }
    increaseSizeDegree(0);
  }
...
struct HashedDictionarySparseMapType<dictionary_key_type, Key, Value, /* use_sparse_hash= */ true> {
  ...
  using Type =
      std::conditional_t<dictionary_key_type == DictionaryKeyType::Simple, google::sparse_hash_map<UInt64, Value, DefaultHash<Key>>,
...
struct HashedDictionaryMapType<dictionary_key_type, /* sparse= */ true, Key, Value>
    : public HashedDictionarySparseMapType<dictionary_key_type, Key, Value,
