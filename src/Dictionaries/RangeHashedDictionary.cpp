...
ColumnPtr RangeHashedDictionary<dictionary_key_type>::getColumn(const std::string& attribute_name, const DataTypePtr& attribute_type,
    ...
    if (is_short_circuit) {
      ...
      if constexpr (std::is_same_v<ValueType, Array>) {
        ...
        if (is_attribute_nullable)
          getItemsShortCircuitImpl<ValueType, true>(
              attribute, modified_key_columns,
          ...
          getItemsShortCircuitImpl<ValueType, false>(
              attribute, modified_key_columns, [&](size_t, std::string_view value, bool) { out->insertData(value.data(), value.size()); },
              default_mask);
      } else {
      ...
      if constexpr (std::is_same_v<ValueType, Array>) {
        ...
        getItemsImpl<ValueType, false>(
            attribute, modified_key_columns, [&](size_t, const Array& value, bool) { out->insert(value); }, default_value_extractor);
      } else if constexpr (std::is_same_v<ValueType, std::string_view>) {
        auto* out = column.get();
        ...
        if (is_attribute_nullable)
          ...
          getItemsImpl<ValueType, false>(
              attribute, modified_key_columns, [&](size_t row, const auto value, bool) { out[row] = value; }, default_value_extractor);
      }
    }
...
template ColumnPtr RangeHashedDictionary<DictionaryKeyType::Simple>::getColumn(const std::string& attribute_name,
                                                                               const DataTypePtr& attribute_type,
                                                                               const Columns& key_columns, const DataTypes& key_types,
                                                                               DefaultOrFilter default_or_filter) const;

template ColumnPtr RangeHashedDictionary<DictionaryKeyType::Complex>::getColumn(const std::string& attribute_name,
                                                                                const DataTypePtr& attribute_type,
                                                                                const Columns& key_columns, const DataTypes& key_types,
                                                                                DefaultOrFilter default_or_filter) const;
