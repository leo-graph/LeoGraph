...
IPolygonDictionary::IPolygonDictionary(const StorageID &dict_id_, const DictionaryStructure &dict_struct_, DictionarySourcePtr source_ptr_,
                                       const DictionaryLifetime dict_lifetime_, Configuration configuration_)
...
void IPolygonDictionary::convertKeyColumns(Columns &key_columns, DataTypes &key_types) const {
  ...
  for (size_t key_type_index = 0; key_type_index < key_types_size; ++key_type_index) {
    ...
    if (float_64_type->equals(*key_type)) continue;
...
ColumnPtr IPolygonDictionary::getColumn(const std::string &attribute_name, const DataTypePtr &attribute_type, const Columns &key_columns,
                                        const DataTypes &, DefaultOrFilter default_or_filter) const {
  ...
  if (unlikely(attribute.is_nullable)) {
    if (is_short_circuit) {
      getItemsShortCircuitImpl<Field>(
          requested_key_points, [&](size_t row) { return (*attribute_values_column)[row]; }, [&](Field &value) { result->insert(value); },
          default_mask.value());
    } else {
      getItemsImpl<Field>(
          requested_key_points, [&](size_t row) { return (*attribute_values_column)[row]; }, [&](Field &value) { result->insert(value); },
    ...
    auto type_call = [&](const auto &dictionary_attribute_type) {
      ...
      if (!attribute_values_column_typed) throw Exception(ErrorCodes::BAD_ARGUMENTS, "An attribute type should be same as dictionary type");
      ...
      if constexpr (std::is_same_v<ValueType, Array>) {
        if (is_short_circuit) {
          getItemsShortCircuitImpl<ValueType>(
              requested_key_points, [&](size_t row) { return (*attribute_values_column)[row].safeGet<Array>(); },
              [&](Array &value) { result_column_typed.insert(value); }, default_mask.value());
        } else {
          getItemsImpl<ValueType>(
              requested_key_points, [&](size_t row) { return (*attribute_values_column)[row].safeGet<Array>(); },
              [&](Array &value) { result_column_typed.insert(value); }, default_value_provider.value());
        }
      } else if constexpr (std::is_same_v<ValueType, std::string_view>) {
        if (is_short_circuit) {
          getItemsShortCircuitImpl<ValueType>(
              requested_key_points, [&](size_t row) { return attribute_values_column->getDataAt(row); },
              [&](std::string_view value) { result_column_typed.insertData(value.data(), value.size()); }, default_mask.value());
        } else {
          getItemsImpl<ValueType>(
              requested_key_points, [&](size_t row) { return attribute_values_column->getDataAt(row); },
              [&](std::string_view value) { result_column_typed.insertData(value.data(), value.size()); }, default_value_provider.value());
        }
      } else {
        ...
        if (is_short_circuit) {
          getItemsShortCircuitImpl<ValueType>(
              requested_key_points, [&](size_t row) { return attribute_data[row]; },
              [&](auto value) { result_data.emplace_back(static_cast<AttributeType>(value)); }, default_mask.value());
        } else {
          getItemsImpl<ValueType>(
              requested_key_points, [&](size_t row) { return attribute_data[row]; },
              [&](auto value) { result_data.emplace_back(static_cast<AttributeType>(value)); }, default_value_provider.value());
        }
      }
    };
...
void IPolygonDictionary::setup() {
  ...
  for (const auto &attribute : dict_struct.attributes) {
    ...
    if (attribute.hierarchical)
      throw Exception(ErrorCodes::TYPE_MISMATCH, "{}: hierarchical attributes not supported for dictionary of polygonal type",
...
void IPolygonDictionary::blockToAttributes(const DB::Block &block) {
  ...
  if (configuration.store_polygon_key_column) key_attribute_column->assumeMutable()->insertRangeFrom(*key_column, 0, key_column->size());
...
void IPolygonDictionary::loadData() {
  ...
  io.executeWithCallbacks([&]() {
    ...
    Block block;
    while (executor.pull(block)) blockToAttributes(block);
  });
  ...
  ::sort(polygon_ids.begin(), polygon_ids.end(),
         [&areas](const auto &lhs, const auto &rhs) { return areas[lhs.second] < areas[rhs.second]; });
...
void IPolygonDictionary::calculateBytesAllocated() {
  ...
  if (configuration.store_polygon_key_column) bytes_allocated += key_attribute_column->allocatedBytes();

  for (const auto &column : attributes_columns) bytes_allocated += column->allocatedBytes();

  for (auto &polygon : polygons) bytes_allocated += bg::num_points(polygon) * sizeof(Point);
}

VectorWithMemoryTracking<IPolygonDictionary::Point> IPolygonDictionary::extractPoints(const Columns &key_columns) {
  if (key_columns.size() != 2) throw Exception(ErrorCodes::BAD_ARGUMENTS, "Expected two columns of coordinates with type Float64");
  ...
  if (!column_x || !column_y) throw Exception(ErrorCodes::TYPE_MISMATCH, "Expected columns of Float64");
  ...
  for (size_t row = 0; row < rows; ++row) {
    ...
    if (isNaN(x) || isNaN(y)) throw Exception(ErrorCodes::BAD_ARGUMENTS, "PolygonDictionary input point component must not be NaN");

    if (std::isinf(x) || std::isinf(y))
      throw Exception(ErrorCodes::BAD_ARGUMENTS, "PolygonDictionary input point component must not be infinite");
...
void IPolygonDictionary::getItemsImpl(const VectorWithMemoryTracking<IPolygonDictionary::Point> &requested_key_points,
                                      ValueGetter &&get_value, ValueSetter &&set_value,
...
void IPolygonDictionary::getItemsShortCircuitImpl(const VectorWithMemoryTracking<IPolygonDictionary::Point> &requested_key_points,
                                                  ValueGetter &&get_value, ValueSetter &&set_value, IColumn::Filter &default_mask) const {
  size_t polygon_index = 0;
  size_t keys_found = 0;
...
struct Offset {
  ...
  Offset &operator++() {
    ++points_added;
    if (points_added <= ring_offsets[current_ring]) return *this;

    ++current_ring;
    if (current_ring < polygon_offsets[current_polygon]) return *this;

    ++current_polygon;
    if (current_polygon < multi_polygon_offsets[current_multi_polygon]) return *this;
  ...
  bool allRingsHaveAPositiveArea() {
    ...
    for (const auto offset : ring_offsets) {
      if (offset - prev_offset < 3) return false;
      prev_offset = offset;
    }
    return true;
  }
...
const IColumn *unrollMultiPolygons(const ColumnPtr &column, Offset &offset) {
  ...
  if (!ptr_multi_polygons) throw Exception(ErrorCodes::TYPE_MISMATCH, "Expected a column containing arrays of polygons");
  ...
  if (!ptr_polygons) throw Exception(ErrorCodes::TYPE_MISMATCH, "Expected a column containing arrays of rings when reading polygons");
  ...
  if (!ptr_rings) throw Exception(ErrorCodes::TYPE_MISMATCH, "Expected a column containing arrays of points when reading rings");
...
const IColumn *unrollSimplePolygons(const ColumnPtr &column, Offset &offset) {
  ...
  if (!ptr_polygons) throw Exception(ErrorCodes::TYPE_MISMATCH, "Expected a column containing arrays of points");
...
void handlePointsReprByArrays(const IColumn *column, Data &data, Offset &offset) {
  ...
  if (!ptr_coord) throw Exception(ErrorCodes::TYPE_MISMATCH, "Expected coordinates to be of type Float64");
  const auto &offsets = ptr_points->getOffsets();
  IColumn::Offset prev_offset = 0;
  for (size_t i = 0; i < offsets.size(); ++i) {
    if (offsets[i] - prev_offset != 2) throw Exception(ErrorCodes::BAD_ARGUMENTS, "All points should be two-dimensional");
    prev_offset = offsets[i];
    addNewPoint(static_cast<IPolygonDictionary::Coord>(ptr_coord->getElement(2 * i)),
                static_cast<IPolygonDictionary::Coord>(ptr_coord->getElement(2 * i + 1)), data, offset);
  }
}
...
void handlePointsReprByTuples(const IColumn *column, Data &data, Offset &offset) {
  ...
  if (!ptr_points) throw Exception(ErrorCodes::TYPE_MISMATCH, "Expected a column of tuples representing points");
  if (ptr_points->tupleSize() != 2) throw Exception(ErrorCodes::BAD_ARGUMENTS, "Points should be two-dimensional");
  ...
  for (size_t i = 0; i < column_x->size(); ++i) {
    addNewPoint(static_cast<IPolygonDictionary::Coord>(column_x->getElement(i)),
                static_cast<IPolygonDictionary::Coord>(column_y->getElement(i)), data, offset);
  }
}
...
void IPolygonDictionary::extractPolygons(const ColumnPtr &column) {
  ...
  if (!offset.allRingsHaveAPositiveArea())
    throw Exception(ErrorCodes::BAD_ARGUMENTS, "Every ring included in a polygon or excluded from it should contain at least 3 points");
