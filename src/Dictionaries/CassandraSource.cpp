...
CassandraSource::CassandraSource(const CassSessionShared &session_, const String &query_str, SharedHeader &sample_block,
...
void CassandraSource::assertTypes(const CassResultPtr &result) {
  ...
  for (size_t i = 0; i < column_count; ++i) {
    ...
    if (got != expected) {
      if (expected == CASS_VALUE_TYPE_TEXT && (got == CASS_VALUE_TYPE_ASCII || got == CASS_VALUE_TYPE_VARCHAR)) continue;

      const auto &column_name = description.sample_block.getColumnsWithTypeAndName()[i].name;
      throw Exception(ErrorCodes::TYPE_MISMATCH, "Type mismatch for column {} : expected Cassandra type {}", column_name, expected_text);
    }
  }
