...
static const ValidateKeysMultiset<ExternalDatabaseEqualKeysSet> dictionary_allowed_keys = {
    ...
    "table",        "schema",     "background_reconnect",
    "update_field", "update_lag", "invalidate_query",
    "query",        "where",      "name",
    "priority",     "sslmode"};
...
namespace {
ExternalQueryBuilder makeExternalQueryBuilder(const DictionaryStructure& dict_struct, const String& schema, const String& table,
                                              const String& query, const String& where) {
...
PostgreSQLDictionarySource::PostgreSQLDictionarySource(const DictionaryStructure& dict_struct_, const Configuration& configuration_,
                                                       postgres::PoolWithFailoverPtr pool_, SharedHeader sample_block_)
    : dict_struct(dict_struct_),
      ...
      query_builder(
          makeExternalQueryBuilder(dict_struct, configuration.schema, configuration.table, configuration.query, configuration.where)),
...
bool PostgreSQLDictionarySource::isModified() const {
  if (!configuration.invalidate_query.empty()) {
    ...
    if (response == invalidate_query_response) return false;
...
std::string PostgreSQLDictionarySource::doInvalidateQuery(const std::string& request) const {
  ...
  QueryPipeline pipeline(
      std::make_unique<PostgreSQLSource<>>(pool->get(), request, std::make_shared<const Block>(std::move(invalidate_sample_block)), 1));
  return readInvalidateQuery(pipeline);
}

bool PostgreSQLDictionarySource::hasUpdateField() const { return !configuration.update_field.empty(); }
...
bool PostgreSQLDictionarySource::supportsSelectiveLoad() const { return true; }

DictionarySourcePtr PostgreSQLDictionarySource::clone() const { return std::make_shared<PostgreSQLDictionarySource>(*this); }
...
static void validateConfigKeys(const Poco::Util::AbstractConfiguration& dict_config, const String& config_prefix) {
  Poco::Util::AbstractConfiguration::Keys config_keys;
  dict_config.keys(config_prefix, config_keys);
  for (const auto& config_key : config_keys) {
    if (dictionary_allowed_keys.contains(config_key) || startsWith(config_key, "replica")) continue;
...
void registerDictionarySourcePostgreSQL(DictionarySourceFactory& factory) {
  auto create_table_source = [=](const String& /*name*/, const DictionaryStructure& dict_struct,
                                 const Poco::Util::AbstractConfiguration& config, const std::string& config_prefix, Block& sample_block,
                                 ContextPtr context, const std::string& /* default_database */,
    ...
    if (named_collection) {
      ...
      common_configuration.database = config.getString(fmt::format("{}.database", settings_config_prefix),
                                                       config.getString(fmt::format("{}.db", settings_config_prefix), ""));
      ...
      if (config.has(settings_config_prefix + ".replica")) {
        ...
        for (const auto& config_key : config_keys) {
          if (config_key.starts_with("replica")) {
            ...
            if (replica_configuration.host.empty() || replica_configuration.port == 0 || replica_configuration.username.empty() ||
                replica_configuration.password.empty()) {
    ...
    if (created_from_ddl) {
      for (const auto& [_, replicas] : replicas_by_priority)
        for (const auto& replica : replicas) context->getRemoteHostFilter().checkHostAndPort(replica.host, toString(replica.port));
    }

    auto pool = std::make_shared<postgres::PoolWithFailover>(replicas_by_priority, settings[Setting::postgresql_connection_pool_size],
                                                             ...
                                                             settings[Setting::postgresql_connection_attempt_timeout], bg_reconnect);

    return std::make_unique<PostgreSQLDictionarySource>(dict_struct, dictionary_configuration.value(), pool,
