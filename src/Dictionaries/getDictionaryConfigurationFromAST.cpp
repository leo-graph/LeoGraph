...
String getAttributeExpression(const ASTDictionaryAttributeDeclaration *dict_attr) {
  if (!dict_attr->expression) return {};
 ...
 *  </lifetime>
 */
void buildLifetimeConfiguration(AutoPtr<Document> doc, AutoPtr<Element> root, const ASTDictionaryLifetime *lifetime) {
  if (!lifetime) return;

  if (lifetime->min_sec > lifetime->max_sec) {
    throw DB::Exception(ErrorCodes::BAD_ARGUMENTS, "{} parameter 'MIN' must be less than or equal to 'MAX'", lifetime->getID(0));
  }
 ...
 *  </layout>
 */
void buildLayoutConfiguration(AutoPtr<Document> doc, AutoPtr<Element> root, const ASTDictionarySettings *settings,
  ...
  for (const auto &param : layout->parameters->children) {
    ...
    if (!value_literal) {
      throw DB::Exception(ErrorCodes::BAD_ARGUMENTS, "Dictionary layout parameter value must be a literal, got '{}' instead",
                          pair->second->formatForErrorMessage());
    }

    const Field &value_field = value_literal->value;

    if (value_field.getType() != Field::Types::UInt64 && value_field.getType() != Field::Types::Float64 &&
    ...
    if (is_ssd_cache_layout) {
      if (value_field.getType() == Field::Types::UInt64 && value_field.safeGet<::UInt64>() == 0) {
        throw DB::Exception(ErrorCodes::BAD_ARGUMENTS, "{} parameter value should be positive number", layout->getID(0));
      }
    }
...
void buildRangeConfiguration(AutoPtr<Document> doc, AutoPtr<Element> root, const ASTDictionaryRange *range,
                             const AttributeNameToConfiguration &all_attrs) {
  ...
  if (range_min_attribute_it == all_attrs.end())
    throw Exception(ErrorCodes::INCORRECT_DICTIONARY_DEFINITION, "MIN {} attribute is not defined in the dictionary attributes",
                    range->min_attr_name);

  auto range_max_attribute_it = all_attrs.find(range->min_attr_name);
  if (range_max_attribute_it == all_attrs.end())
    throw Exception(ErrorCodes::INCORRECT_DICTIONARY_DEFINITION, "MAX {} attribute is not defined in the dictionary attributes",
                    range->max_attr_name);
...
void buildAttributeExpressionIfNeeded(AutoPtr<Document> doc, AutoPtr<Element> root, const ASTDictionaryAttributeDeclaration *dict_attr) {
  if (!dict_attr->expression) return;
 ...
 *  </attribute>
 */
void buildSingleAttribute(AutoPtr<Document> doc, AutoPtr<Element> root, const ASTDictionaryAttributeDeclaration *dict_attr) {
  ...
  if (dict_attr->default_value) null_value_str = convertFieldToString(dict_attr->default_value->as<ASTLiteral>()->value);
 ...
 *
 */
void buildPrimaryKeyConfiguration(AutoPtr<Document> doc, AutoPtr<Element> root, bool complex, const Names &key_names,
                                  const ASTExpressionList *dictionary_attributes) {
  const auto &children = dictionary_attributes->children;

  if (!complex) {
    if (key_names.size() != 1)
      throw Exception(ErrorCodes::INCORRECT_DICTIONARY_DEFINITION, "Primary key for simple dictionary must contain exactly one element");
    ...
    const auto it = std::find_if(children.begin(), children.end(), [&](const ASTPtr &node) {
    ...
    if (it == children.end()) {
      throw Exception(ErrorCodes::INCORRECT_DICTIONARY_DEFINITION, "Primary key field '{}' not found among attributes", identifier_name);
    }
    ...
    if (children.size() < key_names.size())
      throw Exception(ErrorCodes::INCORRECT_DICTIONARY_DEFINITION, "Primary key fields count is more, than dictionary attributes count.");
    ...
    for (const auto &key_name : key_names) {
      ...
      if (!found)
        throw Exception(ErrorCodes::INCORRECT_DICTIONARY_DEFINITION, "Primary key field '{}' not found among attributes.", key_name);
    }
  }
}
 ...
 */
AttributeNameToConfiguration buildDictionaryAttributesConfiguration(AutoPtr<Document> doc, AutoPtr<Element> root,
                                                                    const ASTExpressionList *dictionary_attributes,
  ...
  for (const auto &child : children) {
    ...
    if (!dict_attr->type) throw Exception(ErrorCodes::INCORRECT_DICTIONARY_DEFINITION, "Dictionary attribute must has type");
    ...
    if (std::find(key_columns.begin(), key_columns.end(), dict_attr->name) == key_columns.end()) buildSingleAttribute(doc, root, dict_attr);
  }
 ...
 */
void buildConfigurationFromFunctionWithKeyValueArguments(AutoPtr<Document> doc, AutoPtr<Element> root,
                                                         const ASTExpressionList *ast_expr_list, ContextPtr context) {
...
void buildSourceConfiguration(AutoPtr<Document> doc, AutoPtr<Element> root, const ASTFunctionWithKeyValueArguments *source,
                              const ASTDictionarySettings *settings, const String &dictionary_name, ContextPtr context) {
...
void checkPrimaryKey(const AttributeNameToConfiguration &all_attrs, const Names &key_attrs) {
  for (const auto &key_attr : key_attrs)
    if (!all_attrs.contains(key_attr)) throw Exception(ErrorCodes::INCORRECT_DICTIONARY_DEFINITION, "Unknown key attribute '{}'", key_attr);
}
...
void checkLifetime(const ASTCreateQuery &query) {
  if (query.dictionary->layout && query.dictionary->layout->layout_type == "direct") {
    if (query.dictionary->lifetime)
      throw Exception(ErrorCodes::BAD_ARGUMENTS, "'lifetime' parameter is redundant for the dictionary' of layout '{}'",
...
DictionaryConfigurationPtr getDictionaryConfigurationFromAST(const ASTCreateQuery &query, ContextPtr context,
  ...
  auto all_attr_names_and_types =
      buildDictionaryAttributesConfiguration(xml_document, structure_element, query.dictionary_attributes_list, pk_attrs);
  ...
  /// NOTE: the data type of Numeric key(simple layout) is UInt64, so if the type is not under UInt64, type casting will lead to precision
  /// loss.
  ...
  buildSourceConfiguration(xml_document, current_dictionary, query.dictionary->source, query.dictionary->dict_settings,
                           full_dictionary_name, context);
  buildLifetimeConfiguration(xml_document, current_dictionary, query.dictionary->lifetime);

  if (query.dictionary->range) buildRangeConfiguration(xml_document, structure_element, query.dictionary->range, all_attr_names_and_types);
...
std::optional<ClickHouseDictionarySourceInfo> getInfoIfClickHouseDictionarySource(DictionaryConfigurationPtr &config,
  ...
  try {
    if (isLocalAddress({host, port}, default_port)) info.is_local = true;
