...
DictionaryTypedSpecialAttribute makeDictionaryTypedSpecialAttribute(const Poco::Util::AbstractConfiguration& config,
                                                                    const std::string& config_prefix, const std::string& default_type) {
  auto name = config.getString(config_prefix + ".name", "");
  auto expression = config.getString(config_prefix + ".expression", "");

  if (name.empty() && !expression.empty()) throw Exception(ErrorCodes::BAD_ARGUMENTS, "Element {}.name is empty", config_prefix);
...
std::optional<AttributeUnderlyingType> tryGetAttributeUnderlyingType(TypeIndex index) {
  ...
  {
    case TypeIndex::Date:
      return AttributeUnderlyingType::UInt16;
    case TypeIndex::Date32:
      return AttributeUnderlyingType::Int32;
    case TypeIndex::DateTime:
      return AttributeUnderlyingType::UInt32;
    default:
      break;
  }
...
DictionaryStructure::DictionaryStructure(const Poco::Util::AbstractConfiguration& config, const std::string& config_prefix) {
  ...
  if (has_key && has_id) throw Exception(ErrorCodes::BAD_ARGUMENTS, "Only one of 'id' and 'key' should be specified");
  ...
  if (has_id) {
    ...
    if (key->empty()) throw Exception(ErrorCodes::BAD_ARGUMENTS, "Empty 'key' supplied");
  } else {
    throw Exception(ErrorCodes::BAD_ARGUMENTS, "Dictionary structure should specify either 'id' or 'key'");
  }

  if (id) {
    if (id->name.empty()) throw Exception(ErrorCodes::BAD_ARGUMENTS, "'id' cannot be empty");

    if (!id->expression.empty()) has_expressions = true;
  }
  ...
  for (size_t i = 0; i < attributes_size; ++i) {
    ...
    if (attribute.hierarchical) {
      if (id && attribute.underlying_type != AttributeUnderlyingType::UInt64)
        throw Exception(ErrorCodes::TYPE_MISMATCH, "Hierarchical attribute type for dictionary with simple key must be UInt64. Actual {}",
                        attribute.underlying_type);
      if (key) throw Exception(ErrorCodes::BAD_ARGUMENTS, "Dictionary with complex key does not support hierarchy");

      hierarchical_attribute_index = i;
    }
  }

  if (config.getBool(config_prefix + ".layout.ip_trie.access_to_key_from_attributes", false)) access_to_key_from_attributes = true;
}

DataTypes DictionaryStructure::getKeyTypes() const {
  if (id) return {std::make_shared<DataTypeUInt64>()};
  ...
  for (size_t i = 0; i < key_attributes_size; ++i) result.emplace_back(key_attributes[i].type);
...
void DictionaryStructure::validateKeyTypes(const DataTypes& key_types) const {
  ...
  for (size_t i = 0; i < key_types_size; ++i) {
    ...
    if (!expected_type->equals(*actual_type))
      throw Exception(ErrorCodes::TYPE_MISMATCH, "Key type for complex key at position {} does not match, expected {}, found {}", i,
                      expected_type->getName(), actual_type->getName());
  }
}
...
const DictionaryAttribute& DictionaryStructure::getAttribute(const std::string& attribute_name) const {
  ...
  if (it == attribute_name_to_index.end()) {
    if (!access_to_key_from_attributes) throw Exception(ErrorCodes::BAD_ARGUMENTS, "No such attribute '{}'", attribute_name);

    for (const auto& key_attribute : *key)
      if (key_attribute.name == attribute_name) return key_attribute;
...
const DictionaryAttribute& DictionaryStructure::getAttribute(const std::string& attribute_name, const DataTypePtr& type) const {
  ...
  if (!attribute.type->equals(*type))
    throw Exception(ErrorCodes::TYPE_MISMATCH, "Attribute type does not match, expected {}, found {}", attribute.type->getName(),
                    type->getName());
...
Strings DictionaryStructure::getKeysNames() const {
  ...
  for (const auto& key_attribute : key_attributes) keys_names.emplace_back(key_attribute.name);
...
static void checkAttributeKeys(const Poco::Util::AbstractConfiguration::Keys& keys) {
  ...
  for (const auto& key : keys) {
    if (!valid_keys.contains(key)) throw Exception(ErrorCodes::BAD_ARGUMENTS, "Unknown key '{}' inside attribute section", key);
  }
}

VectorWithMemoryTracking<DictionaryAttribute> DictionaryStructure::getAttributes(const Poco::Util::AbstractConfiguration& config,
  ...
  for (const auto& config_elem : config_elems) {
    if (!startsWith(config_elem, "attribute")) continue;
    ...
    if ((range_min && name == range_min->name) || (range_max && name == range_max->name)) continue;

    auto insert_result = attribute_names.insert(name);
    bool inserted = insert_result.second;

    if (!inserted)
      throw Exception(ErrorCodes::BAD_ARGUMENTS, "Dictionary attributes names must be unique. Attribute name {} is not unique", name);
    ...
    if (!underlying_type_opt)
      throw Exception(ErrorCodes::UNKNOWN_TYPE, "Unknown type {} for dictionary attribute", non_nullable_type->getName());

    const auto expression = config.getString(prefix + "expression", "");
    if (!expression.empty()) has_expressions = true;
    ...
    if (name.empty()) throw Exception(ErrorCodes::BAD_ARGUMENTS, "Properties 'name' and 'type' of an attribute cannot be empty");

    if (has_hierarchy && !hierarchy_allowed) throw Exception(ErrorCodes::BAD_ARGUMENTS, "Hierarchy not allowed in '{}'", prefix);

    if (has_hierarchy && hierarchical) throw Exception(ErrorCodes::BAD_ARGUMENTS, "Only one hierarchical attribute supported");
    ...
    res_attributes.emplace_back(DictionaryAttribute{.name = name,
                                                    .type = initial_type,
...
void DictionaryStructure::parseRangeConfiguration(const Poco::Util::AbstractConfiguration& config, const std::string& structure_prefix) {
  ...
  if (!range_min->type->equals(*range_max->type)) {
    throw Exception(ErrorCodes::BAD_ARGUMENTS,
                    ...
                    "'range_min' type: {},"
                    "'range_max' type: {}",
                    range_min->type->getName(), range_max->type->getName());
  }
  ...
  if (!range_min->expression.empty() || !range_max->expression.empty()) has_expressions = true;
}
