...
enum class RangeHashedDictionaryLookupStrategy : uint8_t { min, max };
...
class RangeHashedDictionary final : public IDictionary {
 public:
  ...
  RangeHashedDictionary(const StorageID &dict_id_, const DictionaryStructure &dict_struct_, DictionarySourcePtr source_ptr_,
                        DictionaryLifetime dict_lifetime_, RangeHashedDictionaryConfiguration configuration_,
  ...
  std::shared_ptr<IExternalLoadable> clone() const override {
    auto result = std::make_shared<RangeHashedDictionary>(getDictionaryID(), dict_struct, source_ptr->clone(), dict_lifetime, configuration,
 ...
 private:
  ...
  template <typename RangeStorageType>
  using KeyAttributeContainerType =
      std::conditional_t<dictionary_key_type == DictionaryKeyType::Simple,
  ...
  struct Attribute final {
    ...
    std::variant<AttributeContainerType<UInt8>, AttributeContainerType<UInt16>, AttributeContainerType<UInt32>,
                 AttributeContainerType<UInt64>, AttributeContainerType<UInt128>, AttributeContainerType<UInt256>,
                 AttributeContainerType<Int8>, AttributeContainerType<Int16>, AttributeContainerType<Int32>, AttributeContainerType<Int64>,
                 AttributeContainerType<Int128>, AttributeContainerType<Int256>, AttributeContainerType<Decimal32>,
                 AttributeContainerType<Decimal64>, AttributeContainerType<Decimal128>, AttributeContainerType<Decimal256>,
                 AttributeContainerType<DateTime64>, AttributeContainerType<Time64>, AttributeContainerType<Float32>,
                 AttributeContainerType<Float64>, AttributeContainerType<UUID>, AttributeContainerType<IPv4>, AttributeContainerType<IPv6>,
                 AttributeContainerType<std::string_view>, AttributeContainerType<Array>>
        container;
  ...
  using RangeStorageTypeContainer =
      std::variant<ContainerType<UInt8>, ContainerType<UInt16>, ContainerType<UInt32>, ContainerType<UInt64>, ContainerType<UInt128>,
                   ContainerType<UInt256>, ContainerType<Int8>, ContainerType<Int16>, ContainerType<Int32>, ContainerType<Int64>,
                   ContainerType<Int128>, ContainerType<Int256>, ContainerType<Decimal32>, ContainerType<Decimal64>,
                   ContainerType<Decimal128>, ContainerType<Decimal256>, ContainerType<DateTime64>, ContainerType<Time64>,
                   ContainerType<Float32>, ContainerType<Float64>, ContainerType<UUID>, ContainerType<IPv4>, ContainerType<IPv6>>;
  ...
  void getItemsImpl(const Attribute &attribute, const Columns &key_columns, ValueSetterFunc<ValueType> &&set_value,
                    DefaultValueExtractor &default_value_extractor) const;

  template <typename ValueType, bool is_nullable>
  void getItemsShortCircuitImpl(const Attribute &attribute, const Columns &key_columns, ValueSetterFunc<ValueType> &&set_value,
                                IColumn::Filter &default_mask) const;

  ColumnPtr getColumnInternal(const std::string &attribute_name, const DataTypePtr &result_type,
                              const PaddedPODArray<UInt64> &key_to_index) const;

  template <typename AttributeType, bool is_nullable, typename ValueSetter>
  void getItemsInternalImpl(const Attribute &attribute, const PaddedPODArray<UInt64> &key_to_index, ValueSetter &&set_value) const;
...
template <DictionaryKeyType dictionary_key_type>
RangeHashedDictionary<dictionary_key_type>::RangeHashedDictionary(const StorageID &dict_id_, const DictionaryStructure &dict_struct_,
                                                                  DictionarySourcePtr source_ptr_, DictionaryLifetime dict_lifetime_,
...
template <DictionaryKeyType dictionary_key_type>
ColumnPtr RangeHashedDictionary<dictionary_key_type>::getColumnInternal(const std::string &attribute_name, const DataTypePtr &result_type,
                                                                        const PaddedPODArray<UInt64> &key_to_index) const {
  ColumnPtr result;
  ...
  auto type_call = [&](const auto &dictionary_attribute_type) {
    ...
    if constexpr (std::is_same_v<ValueType, Array>) {
      ...
      getItemsInternalImpl<ValueType, false>(attribute, key_to_index, [&](size_t, const Array &value, bool) { out->insert(value); });
    } else if constexpr (std::is_same_v<ValueType, std::string_view>) {
      auto *out = column.get();

      if (is_attribute_nullable)
        getItemsInternalImpl<ValueType, true>(attribute, key_to_index, [&](size_t row, std::string_view value, bool is_null) {
          (*vec_null_map_to)[row] = is_null;
          out->insertData(value.data(), value.size());
        });
      else
        getItemsInternalImpl<ValueType, false>(attribute, key_to_index,
                                               [&](size_t, std::string_view value, bool) { out->insertData(value.data(), value.size()); });
    } else {
      auto &out = column->getData();

      if (is_attribute_nullable)
        getItemsInternalImpl<ValueType, true>(attribute, key_to_index, [&](size_t row, const auto value, bool is_null) {
          (*vec_null_map_to)[row] = is_null;
          out[row] = value;
        });
      else
        getItemsInternalImpl<ValueType, false>(attribute, key_to_index, [&](size_t row, const auto value, bool) { out[row] = value; });
    }
...
ColumnUInt8::Ptr RangeHashedDictionary<dictionary_key_type>::hasKeys(const Columns &key_columns, const DataTypes &key_types) const {
  ...
  impl::callOnRangeType(dict_struct.range_min->type, [&](const auto &types) {
    ...
    if (!range_column_typed)
      throw Exception(ErrorCodes::TYPE_MISMATCH, "Dictionary {} range column type should be equal to {}", getFullName(),
...
void RangeHashedDictionary<dictionary_key_type>::loadData() {
  ...
  impl::callOnRangeType(dict_struct.range_min->type, [&](const auto &types) {
    ...
    for (auto &[_, intervals] : key_attribute_container) intervals.build();
  });
...
void RangeHashedDictionary<dictionary_key_type>::calculateBytesAllocated() {
  impl::callOnRangeType(dict_struct.range_min->type, [&](const auto &types) {
    ...
    for (auto &[_, intervals] : key_attribute_container) bytes_allocated += intervals.getSizeInBytes();
  });
  ...
  for (const auto &attribute : attributes) {
    auto type_call = [&](const auto &dictionary_attribute_type) {
      ...
      if (attribute.is_value_nullable) bytes_allocated += (*attribute.is_value_nullable).size() * sizeof(bool);
    };
...
template <DictionaryKeyType dictionary_key_type>
typename RangeHashedDictionary<dictionary_key_type>::Attribute RangeHashedDictionary<dictionary_key_type>::createAttribute(
    const DictionaryAttribute &dictionary_attribute) {
  std::optional<VectorWithMemoryTracking<bool>> is_value_nullable;

  if (dictionary_attribute.is_nullable) is_value_nullable.emplace(VectorWithMemoryTracking<bool>());
...
template <DictionaryKeyType dictionary_key_type>
template <typename AttributeType, bool is_nullable, typename ValueSetter>
void RangeHashedDictionary<dictionary_key_type>::getItemsInternalImpl(const Attribute &attribute,
  ...
  for (size_t key_index = 0; key_index < keys_size; ++key_index) {
    ...
    if (unlikely(container_index >= container_size)) {
      throw Exception(ErrorCodes::LOGICAL_ERROR,
                      "Dictionary {} expected attribute container index {} must be less than attribute container size {}", getFullName(),
...
void RangeHashedDictionary<dictionary_key_type>::updateData() {
  ...
  if (!update_field_loaded_block || update_field_loaded_block->rows() == 0) {
    ...
    update_field_loaded_block = std::make_shared<Block>(mergeBlockWithPipe<DictionaryKeyType::Complex>(
        dict_struct.getKeysSize() + range_columns_size, *update_field_loaded_block, std::move(io)));
  }
...
void RangeHashedDictionary<dictionary_key_type>::blockToAttributes(const Block &block) {
  ...
  if (range_dictionary_attributes_size != block.columns()) {
    throw Exception(ErrorCodes::UNSUPPORTED_METHOD, "Block size mismatch. Actual {}. Expected {}", block_columns,
  ...
  for (size_t i = 0; i < dictionary_keys_size; ++i) key_columns.emplace_back(block.getByPosition(i).column);
  ...
  impl::callOnRangeType(dict_struct.range_min->type, [&](const auto &types) {
    ...
    if (!min_range_column_typed)
      throw Exception(ErrorCodes::TYPE_MISMATCH, "Dictionary {} range min column type should be equal to {}", getFullName(),
                      dict_struct.range_min->type->getName());

    const auto *max_range_column_typed = typeid_cast<const RangeColumnType *>(max_range_column);
    if (!max_range_column_typed)
      throw Exception(ErrorCodes::TYPE_MISMATCH, "Dictionary {} range max column type should be equal to {}", getFullName(),
    ...
    auto &key_attribute_container = std::get<KeyAttributeContainerType<RangeStorageType>>(key_attribute.container);
    auto &invalid_intervals_container =
        std::get<InvalidIntervalsContainerType<RangeStorageType>>(key_attribute.invalid_intervals_container);
    ...
    for (size_t key_index = 0; key_index < keys_size; ++key_index) {
      ...
      if constexpr (std::is_same_v<KeyType, std::string_view>) key = copyStringInArena(string_arena, key);
...
void RangeHashedDictionary<dictionary_key_type>::setAttributeValue(Attribute &attribute, const Field &value) {
  auto type_call = [&](const auto &dictionary_attribute_type) {
    ...
    if (unlikely(attribute.is_value_nullable.has_value())) {
      ...
      if (unlikely(value_is_null)) return;
    }
...
Pipe RangeHashedDictionary<dictionary_key_type>::read(const Names &column_names, size_t max_block_size, size_t num_streams) const {
  ...
  impl::callOnRangeType(dict_struct.range_min->type, [&](const auto &types) {
    ...
    const auto &invalid_intervals_container =
        std::get<InvalidIntervalsContainerType<RangeStorageType>>(key_attribute.invalid_intervals_container);
  ...
  auto range_min_column_with_type =
      ColumnWithTypeAndName{std::move(range_min_column), dict_struct.range_min->type, dict_struct.range_min->name};
  auto range_max_column_with_type =
      ColumnWithTypeAndName{std::move(range_max_column), dict_struct.range_max->type, dict_struct.range_max->name};
  ...
  DictionarySourceCoordinator::ReadColumnsFunc read_keys_func = [dictionary_copy = dictionary](
                                                                    const Strings &attribute_names, const DataTypes &result_types,
                                                                    const Columns &key_columns_, const DataTypes, const Columns &) {
    ...
    if (!key_to_index_column_)
      throw Exception(ErrorCodes::LOGICAL_ERROR, "Dictionary {} read expect indexes column with type UInt64",
  ...
  auto coordinator = std::make_shared<DictionarySourceCoordinator>(dictionary, column_names, std::move(key_columns),
                                                                   std::move(data_columns), max_block_size, std::move(read_keys_func));
