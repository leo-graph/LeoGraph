...
Field parseStringToField(const String &raw, const DataTypePtr data_type) try {
  ...
  throw Exception(ErrorCodes::INCORRECT_DICTIONARY_DEFINITION, "Cannot parse {} for data type {}, Reason is: {}", raw, data_type->getName(),
...
VectorWithMemoryTracking<StringPiece> createStringPieces(const String &value, int num_captures, const String &regex, LoggerPtr logger) {
    ...
    if ((value[i] == '\\' || value[i] == '$') && i + 1 < value.size()) {
      if (isNumericASCII(value[i + 1])) {
        ...
        if (ref_num >= num_captures)
          LOG_TRACE(logger, "Reference Id {} in set string is invalid, the regexp {} only has {} capturing groups", ref_num, regex,
                    num_captures - 1);
  ...
  if (!literal.empty()) result.push_back(StringPiece(literal));
  return result;
}

void RegExpTreeDictionary::calculateBytesAllocated() {
  for (const String &regex : simple_regexps) bytes_allocated += regex.size();
...
void RegExpTreeDictionary::initRegexNodes(Block &block) {
  ...
  for (size_t i = 0; i < size; i++) {
    ...
    if (regex_nodes.contains(id)) throw Exception(ErrorCodes::INCORRECT_DICTIONARY_DEFINITION, "There are duplicate id {}", id);

    if (id == 0) throw Exception(ErrorCodes::INCORRECT_DICTIONARY_DEFINITION, "There are invalid id {}", id);
    ...
    for (size_t j = 0; j < keys_size; j++) {
      ...
      if (structure.hasAttribute(name_)) {
        ...
        if (!string_pieces.empty()) {
          node->attributes[name_] =
              RegexTreeNode::AttributeValue{.field = values[j], .pieces = std::move(string_pieces), .original_value = value};
        } else {
    ...
    if (use_vectorscan) result = OptimizedRegularExpression::analyze(regex);
...
void RegExpTreeDictionary::loadData() {
  if (!source_ptr->hasUpdateField()) {
    ...
    if (simple_regexps.empty()) use_vectorscan = false;
    if (!use_vectorscan) return;
    ...
    if (flag_case_insensitive) flag_bits |= HS_FLAG_CASELESS;
    ...
    hs_error_t err = hs_compile_lit_multi(patterns.data(), flags.data(), ids.get(), lengths.data(), static_cast<unsigned>(patterns.size()),
                                          HS_MODE_BLOCK, nullptr, &db, &compile_error);
    origin_db.reset(db);
    if (err != HS_SUCCESS) {
      ...
      if (error->expression < 0) throw Exception::createRuntime(ErrorCodes::LOGICAL_ERROR, String(error->message));
      throw Exception(ErrorCodes::BAD_ARGUMENTS, "Pattern '{}' failed with error '{}'", patterns[error->expression],
    ...
    if (err != HS_SUCCESS) throw Exception(ErrorCodes::CANNOT_ALLOCATE_MEMORY, "Could not allocate scratch space for vectorscan");
#endif
...
RegExpTreeDictionary::RegExpTreeDictionary(const StorageID &id_, const DictionaryStructure &structure_, DictionarySourcePtr source_ptr_,
                                           Configuration configuration_, bool use_vectorscan_, bool flag_case_insensitive_,
                                           bool flag_dotall_)
    : IDictionary(id_),
...
bool RegExpTreeDictionary::setAttributes(UInt64 id, AttributeCollector &attributes_to_set, const String &data,
                                         ...
                                         const UnorderedMapWithMemoryTracking<String, ColumnPtr> &defaults, size_t key_index) const {
  if (visited_nodes.contains(id)) return attributes_to_set.attributesFull() == attributes.size();
  ...
  for (const auto &[name_, value] : node_attributes) {
    if (!attributes.contains(name_) || attributes_to_set.full(name_)) continue;
  ...
  if (parent_id > 0) setAttributes(parent_id, attributes_to_set, data, visited_nodes, attributes, defaults, key_index);

  /// if all attributes are full, we can stop walking the tree
  return attributes_to_set.attributesFull() == attributes.size();
}

bool RegExpTreeDictionary::setAttributesShortCircuit(UInt64 id, AttributeCollector &attributes_to_set, const String &data,
                                                     UnorderedSetWithMemoryTracking<UInt64> &visited_nodes,
                                                     const UnorderedMapWithMemoryTracking<String, const DictionaryAttribute &> &attributes,
  ...
  const auto &node_attributes = regex_nodes.at(id)->attributes;
  for (const auto &[name_, value] : node_attributes) {
    if (!attributes.contains(name_) || attributes_to_set.full(name_, defaults)) continue;
    ...
    if (value.containsBackRefs()) {
      ...
      if (use_default) {
        ...
        // - If collecting values, don't add anything. If we find no other matches for this attribute,
        //   then we'll set its value to the default Array value later.
        if (!attributes_to_set.collecting()) attributes_to_set.addDefault(name_, defaults);
      } else
  ...
  auto parent_id = regex_nodes.at(id)->parent_id;
  if (parent_id > 0) setAttributesShortCircuit(parent_id, attributes_to_set, data, visited_nodes, attributes, defaults);
...
struct MatchContext {
  ...
  MatchContext(const VectorWithMemoryTracking<UInt64> &regexp_ids_, const UnorderedMapWithMemoryTracking<UInt64, UInt64> &topology_order_,
               const char *data_, size_t length_, const MapWithMemoryTracking<UInt64, RegExpTreeDictionary::RegexTreeNodePtr> &regex_nodes_)
      : regexp_ids(regexp_ids_), topology_order(topology_order_), data(data_), length(length_), regex_nodes(regex_nodes_) {}

  [[maybe_unused]]
  void insertIdx(unsigned int idx) {
    UInt64 node_id = regexp_ids[idx - 1];
    ++pre_match_counter;
    if (!regex_nodes.at(node_id)->match(data, length)) return;
    ++match_counter;
  ...
  void sort() { std::sort(matched_idx_sorted_list.begin(), matched_idx_sorted_list.end()); }

  bool contains(UInt64 idx) const { return matched_idx_set.contains(idx); }
};

UnorderedMapWithMemoryTracking<String, ColumnPtr> RegExpTreeDictionary::match(
    const ColumnString::Chars &keys_data, const ColumnString::Offsets &keys_offsets,
    const UnorderedMapWithMemoryTracking<String, const DictionaryAttribute &> &attributes, DefaultMapOrFilter default_or_filter,
  ...
  for (size_t key_idx = 0; key_idx < input_rows_count; ++key_idx) {
    ...
    if (use_vectorscan) {
      ...
      auto on_match = [](unsigned int id, unsigned long long /* from */,  // NOLINT
      ...
      hs_error_t err = hs_scan(origin_db.get(), begin, static_cast<unsigned>(length), 0, smart_scratch.get(), on_match, &match_result);

      if (err != HS_SUCCESS) throw Exception(ErrorCodes::HYPERSCAN_CANNOT_SCAN_TEXT, "Failed to scan data with vectorscan");
    }
#endif
    ...
    auto is_valid = [&](UInt64 id) {
      while (id) {
        if (!match_result.contains(id)) return false;
    ...
    if (is_short_circuit) {
      ...
      for (auto item : match_result.matched_idx_sorted_list) {
        ...
        if (setAttributesShortCircuit(id, attributes_to_set, str, visited_nodes, attributes, &defaults)) break;
      }

      for (const auto &[name_, attr] : attributes) {
        if (attributes_to_set.contains(name_)) continue;
      ...
      for (auto item : match_result.matched_idx_sorted_list) {
        ...
        if (!is_valid(id)) continue;
        if (visited_nodes.contains(id)) continue;
        if (setAttributes(id, attributes_to_set, str, visited_nodes, attributes, default_map.value().get(), key_idx)) break;
      }

      for (const auto &[name_, attr] : attributes) {
        if (attributes_to_set.contains(name_)) continue;

        DefaultValueProvider default_value(collect_values_limit ? DataTypeArray(attr.type).getDefault() : attr.null_value,
                                           default_map.value().get().at(name_));
        columns[name_]->insert(default_value.getDefaultValue(key_idx));
      }

      /// insert to columns
      for (const auto &[name_, value] : attributes_to_set) columns[name_]->insert(value);
    }
  ...
  for (auto &[name_, mutable_ptr] : columns) result.emplace(name_, std::move(mutable_ptr));
...
Pipe RegExpTreeDictionary::read(const Names &, size_t max_block_size, size_t) const {
  ...
  for (;;) {
    ...
    block.insert(
        ColumnWithTypeAndName(std::move(col_values), std::make_shared<DataTypeArray>(std::make_shared<DataTypeString>()), kValues));
    result.push_back(std::move(block));
    if (it == regex_nodes.end()) break;
    block_size = 0;
  }
...
Columns RegExpTreeDictionary::getColumnsImpl(const Strings &attribute_names, const DataTypes &result_types, const Columns &key_columns,
                                             const DataTypes &key_types, DefaultsOrFilter defaults_or_filter,
                                             std::optional<size_t> collect_values_limit) const {
  ...
  for (size_t i = 0; i < attribute_names.size(); i++) {
    ...
    if (collect_values_limit) {
      if (!WhichDataType(attribute_type).isArray())
        throw Exception(ErrorCodes::LOGICAL_ERROR, "Expected Array result type for attribute `{}`, got `{}`", attribute_names[i],
  ...
  if (key_column == nullptr) throw Exception(ErrorCodes::TYPE_MISMATCH, "Expected a ColumnString column");

  const auto &columns_map =
      match(key_column->getChars(), key_column->getOffsets(), attributes,
            is_short_circuit ? std::get<RefFilter>(defaults_or_filter).get() /*default_mask*/ : DefaultMapOrFilter{defaults},
            collect_values_limit);

  Columns result;
  for (const String &name_ : attribute_names) result.push_back(columns_map.at(name_));

  return result;
}

void registerDictionaryRegExpTree(DictionaryFactory &factory) {
  auto create_layout = [=](const std::string & /*name*/, const DictionaryStructure &dict_struct,
                           const Poco::Util::AbstractConfiguration &config, const std::string &config_prefix,
                           DictionarySourcePtr source_ptr, ContextPtr global_context, bool) -> DictionaryPtr {
    ...
    return std::make_unique<RegExpTreeDictionary>(
        dict_id, dict_struct, std::move(source_ptr), configuration, context->getSettingsRef()[Setting::regexp_dict_allow_hyperscan],
        context->getSettingsRef()[Setting::regexp_dict_flag_case_insensitive], context->getSettingsRef()[Setting::regexp_dict_flag_dotall]);
  };
