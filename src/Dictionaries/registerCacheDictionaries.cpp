...
CacheDictionaryStorageConfiguration parseCacheStorageConfiguration(const Poco::Util::AbstractConfiguration &config, const String &full_name,
                                                                   const String &layout_type, const String &dictionary_layout_prefix,
                                                                   const DictionaryLifetime &dict_lifetime) {
  size_t size = config.getUInt64(dictionary_layout_prefix + ".size_in_cells");
  if (size == 0)
    throw Exception(ErrorCodes::TOO_SMALL_BUFFER_SIZE, "{}: dictionary of layout '{}' setting 'size_in_cells' must be greater than 0",
  ...
  CacheDictionaryStorageConfiguration storage_configuration{
      .max_size_in_cells = rounded_size, .strict_max_lifetime_seconds = strict_max_lifetime_seconds, .lifetime = dict_lifetime};
...
SSDCacheDictionaryStorageConfiguration parseSSDCacheStorageConfiguration(const Poco::Util::AbstractConfiguration &config,
                                                                         const String &full_name, const String &layout_type,
                                                                         const String &dictionary_layout_prefix,
                                                                         const DictionaryLifetime &dict_lifetime) {
  size_t strict_max_lifetime_seconds =
      config.getUInt64(dictionary_layout_prefix + ".strict_max_lifetime_seconds", static_cast<size_t>(dict_lifetime.max_sec));
  ...
  if (file_size % block_size != 0)
    throw Exception(ErrorCodes::BAD_ARGUMENTS, "{}: dictionary of layout '{}' setting 'file_size' must be a multiple of block_size",
                    full_name, layout_type);
  ...
  if (read_buffer_size % block_size != 0)
    throw Exception(ErrorCodes::BAD_ARGUMENTS, "{}: dictionary of layout '{}' setting 'read_buffer_size' must be a multiple of block_size",
                    full_name, layout_type);
  ...
  if (write_buffer_size % block_size != 0)
    throw Exception(ErrorCodes::BAD_ARGUMENTS, "{}: dictionary of layout '{}' setting 'write_buffer_size' must be a multiple of block_size",
                    full_name, layout_type);

  auto file_path = config.getString(dictionary_layout_prefix + ".path");
  if (file_path.empty())
    throw Exception(ErrorCodes::BAD_ARGUMENTS, "{}: dictionary of layout '{}' setting 'path' must be specified", full_name, layout_type);

  SSDCacheDictionaryStorageConfiguration configuration{.strict_max_lifetime_seconds = strict_max_lifetime_seconds,
...
CacheDictionaryUpdateQueueConfiguration parseCacheDictionaryUpdateQueueConfiguration(const Poco::Util::AbstractConfiguration &config,
                                                                                     const String &full_name, const String &layout_type,
                                                                                     const String &dictionary_layout_prefix) {
  ...
  if (max_update_queue_size == 0)
    throw Exception(ErrorCodes::TOO_SMALL_BUFFER_SIZE,
                    "{}: dictionary of layout '{}' setting 'max_update_queue_size' must be greater than 0", full_name, layout_type);

  size_t update_queue_push_timeout_milliseconds =
      config.getUInt64(dictionary_layout_prefix + ".update_queue_push_timeout_milliseconds", 10);
  ...
  if (max_threads_for_updates == 0)
    throw Exception(ErrorCodes::BAD_ARGUMENTS, "{}: dictionary of layout '{}' setting 'max_threads_for_updates' must be greater than 0",
                    full_name, layout_type);
...
DictionaryPtr createCacheDictionaryLayout(const String &full_name, const DictionaryStructure &dict_struct,
                                          const Poco::Util::AbstractConfiguration &config, const std::string &config_prefix,
                                          DictionarySourcePtr source_ptr, ContextPtr global_context [[maybe_unused]],
  ...
  if (require_nonempty)
    throw Exception(ErrorCodes::BAD_ARGUMENTS, "{}: cache dictionary of layout '{}' cannot have 'require_nonempty' attribute set",
                    full_name, layout_type);
  ...
  auto dictionary = std::make_unique<CacheDictionary<dictionary_key_type>>(dictionary_identifier, dict_struct, std::move(source_ptr),
                                                                           std::move(storage), update_queue_configuration, configuration);

  return dictionary;
}

void registerDictionaryCache(DictionaryFactory &factory) {
  auto create_simple_cache_layout = [=](const String &full_name, const DictionaryStructure &dict_struct,
                                        const Poco::Util::AbstractConfiguration &config, const std::string &config_prefix,
                                        DictionarySourcePtr source_ptr, ContextPtr global_context, bool created_from_ddl) -> DictionaryPtr {
    return createCacheDictionaryLayout<DictionaryKeyType::Simple, false /* ssd */>(full_name, dict_struct, config, config_prefix,
                                                                                   std::move(source_ptr), global_context, created_from_ddl);
  };

  factory.registerLayout("cache", create_simple_cache_layout, false);

  auto create_complex_key_cache_layout = [=](const std::string &full_name, const DictionaryStructure &dict_struct,
                                             const Poco::Util::AbstractConfiguration &config, const std::string &config_prefix,
                                             DictionarySourcePtr source_ptr, ContextPtr global_context,
                                             bool created_from_ddl) -> DictionaryPtr {
    return createCacheDictionaryLayout<DictionaryKeyType::Complex, false /* ssd */>(
  ...
  auto create_simple_ssd_cache_layout = [=](const std::string &full_name, const DictionaryStructure &dict_struct,
                                            const Poco::Util::AbstractConfiguration &config, const std::string &config_prefix,
                                            DictionarySourcePtr source_ptr, ContextPtr global_context,
                                            bool created_from_ddl) -> DictionaryPtr {
    return createCacheDictionaryLayout<DictionaryKeyType::Simple, true /* ssd */>(full_name, dict_struct, config, config_prefix,
                                                                                  std::move(source_ptr), global_context, created_from_ddl);
  };

  factory.registerLayout("ssd_cache", create_simple_ssd_cache_layout, false);

  auto create_complex_key_ssd_cache_layout = [=](const std::string &full_name, const DictionaryStructure &dict_struct,
                                                 const Poco::Util::AbstractConfiguration &config, const std::string &config_prefix,
                                                 DictionarySourcePtr source_ptr, ContextPtr global_context,
                                                 bool created_from_ddl) -> DictionaryPtr {
    return createCacheDictionaryLayout<DictionaryKeyType::Complex, true /* ssd */>(full_name, dict_struct, config, config_prefix,
                                                                                   std::move(source_ptr), global_context, created_from_ddl);
  };
