...
LibraryDictionarySource::LibraryDictionarySource(const DictionaryStructure& dict_struct_, const Poco::Util::AbstractConfiguration& config,
                                                 const std::string& config_prefix_, Block& sample_block_, ContextPtr context_,
...
LibraryDictionarySource::LibraryDictionarySource(const LibraryDictionarySource& other)
  ...
  bridge_helper = std::make_shared<ExternalDictionaryLibraryBridgeHelper>(context, description.sample_block, dictionary_id,
                                                                          other.bridge_helper->getLibraryData());
  if (!bridge_helper->cloneLibrary(other.dictionary_id)) throw Exception(ErrorCodes::EXTERNAL_LIBRARY_ERROR, "Failed to clone library");
}

bool LibraryDictionarySource::isModified() const { return bridge_helper->isModified(); }

bool LibraryDictionarySource::supportsSelectiveLoad() const { return bridge_helper->supportsSelectiveLoad(); }
...
DictionarySourcePtr LibraryDictionarySource::clone() const { return std::make_shared<LibraryDictionarySource>(*this); }

std::string LibraryDictionarySource::toString() const { return path; }
...
String LibraryDictionarySource::getLibrarySettingsString(const Poco::Util::AbstractConfiguration& config, const std::string& config_root) {
  ...
  for (const auto& key : config_keys) {
    ...
    if (bracket_pos != std::string::npos && bracket_pos > 0) key_name = key.substr(0, bracket_pos);
...
String LibraryDictionarySource::getDictAttributesString() {
  ...
  for (size_t i = 0; i < dict_struct.attributes.size(); ++i) attributes_names[i] = dict_struct.attributes[i].name;
  WriteBufferFromOwnString out;
...
void registerDictionarySourceLibrary(DictionarySourceFactory& factory) {
  auto create_table_source = [=](const String& /*name*/, const DictionaryStructure& dict_struct,
    ...
    return std::make_unique<LibraryDictionarySource>(dict_struct, config, config_prefix + ".library", sample_block, global_context,
                                                     created_from_ddl);
  };
