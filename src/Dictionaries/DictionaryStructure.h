...
struct DictionaryAttributeType {
  ...
  using AttributeType = TypeIndexToType<static_cast<TypeIndex>(static_cast<TypeIndexUnderlying>(type))>;
};
...
constexpr void callOnDictionaryAttributeType(AttributeUnderlyingType type, F&& func) {
  static_for<AttributeUnderlyingType>([type, my_func = std::forward<F>(func)](auto other) {
    if (type == other) my_func(DictionaryAttributeType<other>{});
  });
}
...
struct DictionaryStructure final {
 ...
 private:
  ...
  VectorWithMemoryTracking<DictionaryAttribute> getAttributes(const Poco::Util::AbstractConfiguration& config,
                                                              const std::string& config_prefix, bool complex_key_attributes);
