...
class DefaultValueProvider final {
 public:
  explicit DefaultValueProvider(Field default_value_, ColumnPtr default_values_column_ = nullptr)
      : default_value(std::move(default_value_)), default_values_column(default_values_column_) {}

  bool isConstant() const { return default_values_column == nullptr; }

  Field getDefaultValue(size_t row) const {
    if (default_values_column) return (*default_values_column)[row];
...
class DictionaryStorageFetchRequest {
 public:
  DictionaryStorageFetchRequest(const DictionaryStructure &structure, const Strings &attributes_to_fetch_names,
    ...
    for (size_t i = 0; i < attributes_to_fetch_size; ++i) attributes_to_fetch_name_to_index.emplace(attributes_to_fetch_names[i], i);
    ...
    for (size_t attribute_index = 0; attribute_index < attributes_size; ++attribute_index) {
      ...
      if (!attribute_to_fetch_result_type->equals(*dictionary_attribute.type))
        throw Exception(ErrorCodes::TYPE_MISMATCH, "Attribute {} type does not match, expected {}, found {}", dictionary_attribute.name,
                        attribute_to_fetch_result_type->getName(), dictionary_attribute.type->getName());
  ...
  ALWAYS_INLINE size_t attributesSize() const { return dictionary_attributes_names_and_types.size(); }
  ...
  ALWAYS_INLINE bool shouldFillResultColumnWithIndex(size_t attribute_index) const { return attributes_to_fetch_filter[attribute_index]; }

  const DataTypePtr &dataTypeAtIndex(size_t attribute_index) const { return dictionary_attributes_names_and_types[attribute_index].type; }
  ...
  MutableColumns makeAttributesResultColumns() const {
    ...
    for (const auto &name_and_type : dictionary_attributes_names_and_types) result.emplace_back(name_and_type.type->createColumn());
  ...
  Columns filterRequestedColumns(MutableColumns &fetched_mutable_columns) const {
    ...
    for (size_t attribute_index = 0; attribute_index < dictionary_attributes_size; ++attribute_index) {
      if (!shouldFillResultColumnWithIndex(attribute_index)) continue;
...
static inline void insertDefaultValuesIntoColumns(  /// NOLINT
    MutableColumns &columns, const DictionaryStorageFetchRequest &fetch_request, size_t row_index) {
  ...
  for (size_t column_index = 0; column_index < columns_size; ++column_index) {
    ...
    if (fetch_request.shouldFillResultColumnWithIndex(column_index)) column->insert(default_value_provider.getDefaultValue(row_index));
  }
}
...
static inline void deserializeAndInsertIntoColumns(  /// NOLINT
    MutableColumns &columns, const DictionaryStorageFetchRequest &fetch_request, ReadBuffer &in) {
  size_t columns_size = columns.size();
...
using DictionaryValueType = std::conditional_t<std::is_same_v<DictionaryAttributeType, String>, std::string_view, DictionaryAttributeType>;
...
class DictionaryAttributeColumnProvider {
 public:
  using ColumnType = std::conditional_t<
      std::is_same_v<DictionaryAttributeType, Array>, ColumnArray,
      std::conditional_t<std::is_same_v<DictionaryAttributeType, String>, ColumnString, ColumnVectorOrDecimal<DictionaryAttributeType>>>;
...
class DictionaryDefaultValueExtractor {
 ...
 public:
  ...
  explicit DictionaryDefaultValueExtractor(Field attribute_default_value, ColumnPtr default_values_column_) {
    if (default_values_column_ != nullptr && isColumnConst(*default_values_column_)) {
  ...
  DefaultValueType operator[](size_t row) {
    if (use_attribute_default_value) return static_cast<DefaultValueType>(default_value);
  ...
  bool isNullAt(size_t row) {
    if (default_value_is_null) return true;

    if (is_null_map) return is_null_map->getData()[row];
...
class DictionaryKeysExtractor {
 public:
  ...
  explicit DictionaryKeysExtractor(const Columns &key_columns_, Arena *complex_key_arena_)
      : key_columns(key_columns_), complex_key_arena(complex_key_arena_) {
    ...
    if constexpr (key_type == DictionaryKeyType::Simple) {
      ...
      if (!vector_col) throw Exception(ErrorCodes::TYPE_MISMATCH, "Column type mismatch for simple key expected UInt64");
    }
  ...
  size_t getCurrentKeyIndex() const { return current_key_index; }
  ...
  void rollbackCurrentKey() const {
    if constexpr (key_type == DictionaryKeyType::Complex) complex_key_arena->rollback(current_complex_key.size());
  }
  ...
  void reset() { current_key_index = 0; }
...
Block mergeBlockWithPipe(size_t key_columns_size, const Block &block_to_update, BlockIO &&io) {
  ...
  for (size_t i = 0; i < key_columns_size; ++i) saved_block_key_columns.emplace_back(block_to_update.safeGetByPosition(i).column);
  ...
  io.executeWithCallbacks([&]() {
    ...
    while (executor.pull(block)) {
      ...
      /// Split into keys columns and attribute columns
      for (size_t i = 0; i < key_columns_size; ++i) block_key_columns.emplace_back(block.safeGetByPosition(i).column);
...
/// TODO: Remove
template <typename T>
static const PaddedPODArray<T> &getColumnVectorData(const IDictionary *dictionary, const ColumnPtr column,
  ...
  if (!vector_col) {
    throw Exception(ErrorCodes::TYPE_MISMATCH, "{}: type mismatch: column has wrong type expected {}",
                    dictionary->getDictionaryID().getNameForLogs(), TypeName<T>);
  }
