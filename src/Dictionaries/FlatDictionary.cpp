...
FlatDictionary::FlatDictionary(const StorageID &dict_id_, const DictionaryStructure &dict_struct_, DictionarySourcePtr source_ptr_,
                               Configuration configuration_, BlockPtr update_field_loaded_block_)
...
ColumnPtr FlatDictionary::getColumn(const std::string &attribute_name, const DataTypePtr &attribute_type, const Columns &key_columns,
                                    const DataTypes &key_types [[maybe_unused]], DefaultOrFilter default_or_filter) const {
    ...
    if (is_short_circuit) {
      ...
      if constexpr (std::is_same_v<ValueType, Array>) {
        ...
        getItemsImpl<ValueType, false>(
            attribute, ids, [&](size_t, const Array &value, bool) { out->insert(value); }, default_value_extractor);
      } else if constexpr (std::is_same_v<ValueType, std::string_view>) {
        auto *out = column.get();
        ...
        if (is_attribute_nullable)
          ...
          getItemsImpl<ValueType, false>(
              attribute, ids, [&](size_t, std::string_view value, bool) { out->insertData(value.data(), value.size()); },
              default_value_extractor);
      } else {
        auto &out = column->getData();
        ...
        if (is_attribute_nullable)
          ...
          getItemsImpl<ValueType, false>(
              attribute, ids, [&](size_t row, const auto value, bool) { out[row] = value; }, default_value_extractor);
      }
    }
  ...
  if (attribute.is_nullable_set) result = ColumnNullable::create(result, std::move(col_null_map_to));
...
ColumnPtr FlatDictionary::getHierarchy(ColumnPtr key_column, const DataTypePtr &) const {
  ...
  auto get_parent_key_func = [&, this](auto &hierarchy_key) {
    ...
    if (!is_key_valid) return result;

    if (unlikely(hierarchical_attribute.is_nullable_set) && hierarchical_attribute.is_nullable_set->find(hierarchy_key)) return result;
...
ColumnUInt8::Ptr FlatDictionary::isInHierarchy(ColumnPtr key_column, ColumnPtr in_key_column, const DataTypePtr &) const {
  PaddedPODArray<UInt64> keys_backup_storage;
  const auto &keys = getColumnVectorData(this, key_column, keys_backup_storage);
...
DictionaryHierarchyParentToChildIndexPtr FlatDictionary::getHierarchicalIndex() const {
  if (hierarhical_index) return hierarhical_index;
  ...
  for (UInt64 child_key = 0; child_key < child_keys_size; ++child_key) {
    if (!loaded_keys[child_key]) continue;

    if (unlikely(hierarchical_attribute.is_nullable_set) && hierarchical_attribute.is_nullable_set->find(child_key)) continue;
...
ColumnPtr FlatDictionary::getDescendants(ColumnPtr key_column, const DataTypePtr &, size_t level,
...
void FlatDictionary::createAttributes() {
  ...
  for (const auto &attribute : dict_struct.attributes) attributes.push_back(createAttribute(attribute));
}
...
void FlatDictionary::blockToAttributes(const Block &block) {
  ...
  DictionaryKeysExtractor<DictionaryKeyType::Simple> keys_extractor(
      {keys_column}, arena_holder.getComplexKeyArena());  /// NOLINT(readability-static-accessed-through-instance)
  ...
  if (unlikely(attributes_size == 0)) {
    for (size_t i = 0; i < keys_size; ++i) {
      ...
      if (unlikely(key >= configuration.max_array_size))
        throw Exception(ErrorCodes::ARGUMENT_OUT_OF_BOUND, "{}: identifier should be less than {}", getFullName(),
  ...
  if (!update_field_loaded_block || update_field_loaded_block->rows() == 0) {
    ...
    io.executeWithCallbacks([&]() {
      ...
      while (executor.pull(block)) {
        ...
        if (!update_field_loaded_block) update_field_loaded_block = std::make_shared<DB::Block>(block.cloneEmpty());
    ...
    update_field_loaded_block = std::make_shared<Block>(
        mergeBlockWithPipe<DictionaryKeyType::Simple>(dict_struct.getKeysSize(), *update_field_loaded_block, std::move(io)));
  }

  if (update_field_loaded_block) blockToAttributes(*update_field_loaded_block.get());
}
...
void FlatDictionary::loadData() {
  ...
  for (size_t i = 0; i < loaded_keys_size; ++i) element_count += loaded_keys[i];
...
void FlatDictionary::buildHierarchyParentToChildIndexIfNeeded() {
  ...
  if (dict_struct.attributes[*dict_struct.hierarchical_attribute_index].bidirectional) hierarhical_index = getHierarchicalIndex();
}
...
void FlatDictionary::calculateBytesAllocated() {
  ...
  for (const auto &attribute : attributes) {
    ...
    if (attribute.is_nullable_set.has_value()) bytes_allocated = attribute.is_nullable_set->getBufferSizeInBytes();
  }
...
void FlatDictionary::getItemsImpl(const Attribute &attribute, const PaddedPODArray<UInt64> &keys, ValueSetter &&set_value,
...
void FlatDictionary::getItemsShortCircuitImpl(const Attribute &attribute, const PaddedPODArray<UInt64> &keys, ValueSetter &&set_value,
...
Pipe FlatDictionary::read(const Names &column_names, size_t max_block_size, size_t num_streams) const {
  ...
  for (size_t key_index = 0; key_index < keys_count; ++key_index)
    if (loaded_keys[key_index]) keys.push_back(key_index);
...
void registerDictionaryFlat(DictionaryFactory &factory) {
  auto create_layout = [=](const std::string &full_name, const DictionaryStructure &dict_struct,
                           ...
                           DictionarySourcePtr source_ptr, ContextPtr /* global_context */, bool /* created_from_ddl */) -> DictionaryPtr {
    if (dict_struct.key) throw Exception(ErrorCodes::UNSUPPORTED_METHOD, "'key' is not supported for dictionary of layout 'flat'");
