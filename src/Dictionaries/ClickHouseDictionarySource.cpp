...
ConnectionPoolWithFailoverPtr createPool(const ClickHouseDictionarySource::Configuration& configuration) {
  if (configuration.is_local) return nullptr;

  ConnectionPoolPtrs pools;
  pools.emplace_back(std::make_shared<ConnectionPool>(
      MAX_CONNECTIONS, configuration.host, configuration.port, configuration.db, configuration.user, configuration.password,
      configuration.proto_send_chunked, configuration.proto_recv_chunked, configuration.quota_key, "", /* cluster */
      "",                                                                                              /* cluster_secret */
      "ClickHouseDictionarySource", Protocol::Compression::Enable,
      configuration.secure ? Protocol::Secure::Enable : Protocol::Secure::Disable, "" /* bind_host */));
...
ClickHouseDictionarySource::ClickHouseDictionarySource(const DictionaryStructure& dict_struct_, const Configuration& configuration_,
                                                       const Block& sample_block_, ContextMutablePtr context_)
    : update_time{std::chrono::system_clock::from_time_t(0)},
      dict_struct{dict_struct_},
      configuration{configuration_},
      query_builder(std::make_shared<ExternalQueryBuilder>(dict_struct, configuration.db, "", configuration.table, configuration.query,
                                                           configuration.where, IdentifierQuotingStyle::Backticks)),
...
BlockIO ClickHouseDictionarySource::loadAll() { return createStreamForQuery(load_all_query); }
...
bool ClickHouseDictionarySource::isModified() const {
  if (!configuration.invalidate_query.empty()) {
    ...
    if (invalidate_query_response == response) return false;
    invalidate_query_response = response;
  }
  return true;
}

bool ClickHouseDictionarySource::hasUpdateField() const { return !configuration.update_field.empty(); }
...
BlockIO ClickHouseDictionarySource::createStreamForQuery(const String& query) {
  ...
  ASTPtr ast = parseQuery(parser, query_begin, query_end, "Query for ClickHouse dictionary", 0, DBMS_DEFAULT_MAX_PARSER_DEPTH,
  ...
  if (configuration.is_local) {
    ...
    if (!CurrentThread::getGroup()) io.query_scope = CurrentThread::QueryScope::create(context_copy);
...
std::string ClickHouseDictionarySource::doInvalidateQuery(const std::string& request) const {
  ...
  if (configuration.is_local) {
    ...
    if (!CurrentThread::getGroup()) query_scope = CurrentThread::QueryScope::create(context_copy);

    BlockIO io = executeQuery(request, context_copy, QueryFlags{.internal = true}).second;
    std::string result;
    io.executeWithCallbacks([&]() { result = readInvalidateQuery(io.pipeline); });
    return result;
  }
...
void registerDictionarySourceClickHouse(DictionarySourceFactory& factory) {
  auto create_table_source = [=](const String& /*name*/, const DictionaryStructure& dict_struct,
                                 ...
                                 ContextPtr global_context, const std::string& default_database,
    ...
    if (named_collection) {
      validateNamedCollection(
          *named_collection, {},
          ValidateKeysMultiset<ExternalDatabaseEqualKeysSet>{
              "secure", "host", "hostname", "port", "user", "username", "password", "proto_send_chunked", "proto_recv_chunked", "quota_key",
              "name", "db", "database", "table", "query", "where", "invalidate_query", "update_field", "update_lag"});
    ...
    if (configuration->is_local) {
      ...
      if (created_from_ddl) context->getRemoteHostFilter().checkHostAndPort(configuration->host, toString(configuration->port));
    }
