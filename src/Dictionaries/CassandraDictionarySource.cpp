...
void registerDictionarySourceCassandra(DictionarySourceFactory& factory) {
  auto create_table_source = [=]([[maybe_unused]] const String& name, [[maybe_unused]] const DictionaryStructure& dict_struct,
    ...
    global_context->getRemoteHostFilter().checkHostAndPort(config.getString(source_config_prefix + ".host"),
                                                           toString(config.getUInt(source_config_prefix + ".port", 0)));
...
CassandraDictionarySource::Configuration::Configuration(const Poco::Util::AbstractConfiguration& config, const String& config_prefix)
...
CassandraDictionarySource::CassandraDictionarySource(const DictionaryStructure& dict_struct_, const Configuration& configuration_,
  ...
  if (configuration.port) cassandraCheck(cass_cluster_set_port(cluster, configuration.port));
...
CassandraDictionarySource::CassandraDictionarySource(const DictionaryStructure& dict_struct_,
                                                     const Poco::Util::AbstractConfiguration& config, const String& config_prefix,
                                                     Block& sample_block_)
    : CassandraDictionarySource(dict_struct_, Configuration(config, config_prefix), sample_block_) {}

void CassandraDictionarySource::maybeAllowFiltering(String& query) const {
  if (!configuration.allow_filtering) return;
...
std::string CassandraDictionarySource::toString() const { return "Cassandra: " + configuration.db + '.' + configuration.table; }
...
BlockIO CassandraDictionarySource::loadKeys(const Columns& key_columns, const VectorWithMemoryTracking<size_t>& requested_rows) {
  if (requested_rows.empty()) throw Exception(ErrorCodes::LOGICAL_ERROR, "No rows requested");
  ...
  for (const auto& row : requested_rows) {
    ...
    for (size_t i = 0; i < configuration.partition_key_prefix; ++i) key_columns[i]->updateHashWithValue(row, partition_key);
    partitions[partition_key.get64()].push_back(row);
  }

  Pipes pipes;
  for (const auto& partition : partitions) {
    String query = query_builder.composeLoadKeysQuery(key_columns, partition.second, ExternalQueryBuilder::CASSANDRA_SEPARATE_PARTITION_KEY,
                                                      configuration.partition_key_prefix);
    maybeAllowFiltering(query);
