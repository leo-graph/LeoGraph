...
StorageInput::StorageInput(const StorageID &table_id, const ColumnsDescription &columns_) : IStorage(table_id) {
...
class ReadFromInput : public ISourceStep {
 public:
  ...
  ReadFromInput(SharedHeader sample_block, Pipe pipe_, StorageInput &storage_)
      : ISourceStep(std::move(sample_block)), pipe(std::move(pipe_)), storage(storage_) {}
...
void StorageInput::read(QueryPlan &query_plan, const Names &column_names, const StorageSnapshotPtr &storage_snapshot,
  ...
  auto reading = std::make_unique<ReadFromInput>(std::move(sample_block), std::move(input_source_pipe), *this);
...
void ReadFromInput::initializePipeline(QueryPipelineBuilder &pipeline, const BuildQueryPipelineSettings &) {
  ...
  if (!storage.was_pipe_initialized)
    throw Exception(ErrorCodes::INVALID_USAGE_OF_INPUT,
                    "Input stream is not initialized, input() must be used only in INSERT SELECT query");

  if (storage.was_pipe_used) throw Exception(ErrorCodes::LOGICAL_ERROR, "Trying to read from input() twice.");
