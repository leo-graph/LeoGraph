...
IStorage::IStorage(StorageID storage_id_, std::unique_ptr<StorageInMemoryMetadata> metadata_)
    : storage_id(std::move(storage_id_)), virtuals(std::make_unique<VirtualColumnsDescription>()) {
  if (metadata_)
...
RWLockImpl::LockHolder IStorage::tryLockTimed(const RWLock &rwlock, RWLockImpl::Type type, const String &query_id,
                                              const Poco::Timespan &acquire_timeout) const {
  auto lock_holder = rwlock->getLock(type, query_id, std::chrono::milliseconds(acquire_timeout.totalMilliseconds()));
  if (!lock_holder) {
    const String type_str = type == RWLockImpl::Type::Read ? "READ" : "WRITE";
    throw Exception(
        ErrorCodes::DEADLOCK_AVOIDED,
        "{} locking attempt on \"{}\" has timed out! ({}ms) Possible deadlock avoided. Client should retry. Owner query ids: {}", type_str,
        getStorageID(), acquire_timeout.totalMilliseconds(), rwlock->getOwnerQueryIdsDescription());
  }
  return lock_holder;
}
...
TableLockHolder IStorage::lockForShare(const String &query_id, const Poco::Timespan &acquire_timeout) {
  ...
  if (is_being_restarted)
    throw Exception(ErrorCodes::TABLE_IS_BEING_RESTARTED, "Table {}.{} is being restarted", table_id.database_name, table_id.table_name);
  return result;
}
...
std::optional<IStorage::AlterLockHolder> IStorage::tryLockForAlter(const Poco::Timespan &acquire_timeout) {
  ...
  if (!lock.try_lock_for(std::chrono::milliseconds(acquire_timeout.totalMilliseconds()))) return {};

  if (is_dropped || is_detached) throw Exception(ErrorCodes::TABLE_IS_DROPPED, "Table {} is dropped or detached", getStorageID());
...
Pipe IStorage::watch(const Names & /*column_names*/, const SelectQueryInfo & /*query_info*/, ContextPtr /*context*/,
...
Pipe IStorage::read(const Names & /*column_names*/, const StorageSnapshotPtr & /*storage_snapshot*/, SelectQueryInfo & /*query_info*/,
...
SinkToStoragePtr IStorage::write(const ASTPtr & /*query*/, const StorageMetadataPtr & /*metadata_snapshot*/, ContextPtr /*context*/,
                                 bool /*async_insert*/) {
  throw Exception(ErrorCodes::NOT_IMPLEMENTED, "Method write is not supported by storage {}", getName());
}

void IStorage::truncate(const ASTPtr & /*query*/, const StorageMetadataPtr & /* metadata_snapshot */, ContextPtr /* context */,
                        TableExclusiveLockHolder &) {
  throw Exception(ErrorCodes::NOT_IMPLEMENTED, "Truncate is not supported by storage {}", getName());
}

void IStorage::read(QueryPlan &query_plan, const Names &column_names, const StorageSnapshotPtr &storage_snapshot,
                    SelectQueryInfo &query_info, ContextPtr context, QueryProcessingStage::Enum processed_stage, size_t max_block_size,
                    size_t num_streams) {
  ...
  if (!should_not_resize && parallelize_output && parallelizeOutputAfterReading(context) && output_ports > 0 && output_ports < num_streams)
...
void IStorage::readFromPipe(QueryPlan &query_plan, Pipe pipe, const Names &column_names, const StorageSnapshotPtr &storage_snapshot,
                            SelectQueryInfo &query_info, ContextPtr context, std::shared_ptr<IStorage> storage_) {
  if (pipe.empty()) {
...
std::optional<QueryPipeline> IStorage::distributedWrite(const ASTInsertQuery & /*query*/, ContextPtr /*context*/) { return {}; }

Pipe IStorage::alterPartition(const StorageMetadataPtr & /* metadata_snapshot */, const PartitionCommands & /* commands */,
                              ContextPtr /* context */) {
...
void IStorage::checkAlterPartitionIsPossible(const PartitionCommands & /*commands*/, const StorageMetadataPtr & /*metadata_snapshot*/,
                                             const Settings & /*settings*/, ContextPtr /*context*/) const {
  throw Exception(ErrorCodes::NOT_IMPLEMENTED, "Table engine {} doesn't support partitioning", getName());
}

bool IStorage::optimize(const ASTPtr & /*query*/, const StorageMetadataPtr & /*metadata_snapshot*/, const ASTPtr & /*partition*/,
...
NameDependencies IStorage::getDependentViewsByColumn(ContextPtr context) const {
  ...
  for (const auto &view_id : view_ids) {
    ...
    if (view->getInMemoryMetadataPtr()->select.inner_query) {
      ...
      for (const auto &col_name : required_columns) name_deps[col_name].push_back(view_id.table_name);
    }
  }
  return name_deps;
}
...
bool IStorage::isStaticStorage() const {
  ...
  if (storage_policy) {
    for (const auto &disk : storage_policy->getDisks())
      if (!(disk->isReadOnly() || disk->isWriteOnce())) return false;
    return true;
  }
  return false;
}
...
std::optional<CheckResult> IStorage::checkDataNext(DataValidationTasksPtr & /* check_task_list */) { return {}; }
...
std::string FilterDAGInfo::dump() const {
  WriteBufferFromOwnString ss;
  ss << "FilterDAGInfo for column '" << column_name << "', do_remove_column " << do_remove_column << "\n";
