...
class StorageMerge final : public IStorage, WithContext {
 public:
  ...
  StorageMerge(const StorageID &table_id_, const ColumnsDescription &columns_, const String &comment,
               const String &source_database_name_or_regexp_, bool database_is_regexp_, const DBToTableSetMap &source_databases_and_tables_,
               ContextPtr context_);
  ...
  QueryProcessingStage::Enum getQueryProcessingStage(ContextPtr, QueryProcessingStage::Enum, const StorageSnapshotPtr &,
  ...
  static ColumnsDescription getColumnsDescriptionFromSourceTables(const ContextPtr &query_context,
                                                                  const String &source_database_name_or_regexp, bool database_is_regexp,
                                                                  const String &source_table_regexp, size_t max_tables_to_look);
 ...
 private:
  ...
  struct DatabaseNameOrRegexp {
    ...
    DatabaseNameOrRegexp(const String &source_database_name_or_regexp_, bool database_is_regexp_,
  ...
  template <typename F>
  static StoragePtr traverseTablesUntilImpl(const ContextPtr &query_context, const IStorage *ignore_self,
                                            const DatabaseNameOrRegexp &database_name_or_regexp, F &&predicate);

  /// Returns a unified column structure among multiple tables.
  static ColumnsDescription getColumnsDescriptionFromSourceTablesImpl(const ContextPtr &context,
...
class ReadFromMerge final : public SourceStepWithFilter {
 public:
  ...
  ReadFromMerge(const Names &column_names_, const SelectQueryInfo &query_info_, const StorageSnapshotPtr &storage_snapshot_,
                const ContextPtr &context_, SharedHeader common_header_, size_t max_block_size, size_t num_streams, StoragePtr storage,
 ...
 private:
  ...
  SelectQueryInfo getModifiedQueryInfo(const ContextMutablePtr &modified_context, const StorageWithLockAndName &storage_with_lock_and_name,
                                       const StorageSnapshotPtr &storage_snapshot, Names required_column_names,
                                       Names &column_names_as_aliases, bool &is_smallest_column_requested, Aliases &aliases) const;
  ...
  ChildPlan createPlanForTable(const StorageSnapshotPtr &storage_snapshot, SelectQueryInfo &query_info,
                               QueryProcessingStage::Enum processed_stage, UInt64 max_block_size,
                               const StorageWithLockAndName &storage_with_lock, const Names &real_column_names_read_from_the_source_table,
  ...
  void addVirtualColumns(ChildPlan &child, SelectQueryInfo &modified_query_info, QueryProcessingStage::Enum processed_stage,
                         const StorageWithLockAndName &storage_with_lock) const;

  QueryPipelineBuilderPtr buildPipeline(ChildPlan &child, QueryProcessingStage::Enum processed_stage) const;

  static void convertAndFilterSourceStream(const Block &header, SelectQueryInfo &modified_query_info, const StorageSnapshotPtr &snapshot,
                                           const Aliases &aliases, const RowPolicyDataOpt &row_policy_data_opt, ContextPtr context,
                                           ChildPlan &child, bool is_smallest_column_requested);

  StorageMerge::StorageListWithLocks getSelectedTables(ContextPtr query_context, bool filter_by_database_virtual_column,
