...
zkutil::ZooKeeperPtr StorageReplicatedMergeTree::getZooKeeper() const {
  ...
  if (!res) throw Exception(ErrorCodes::NO_ZOOKEEPER, "Cannot get ZooKeeper");
  return res;
}
...
StorageReplicatedMergeTree::StorageReplicatedMergeTree(const TableZnodeInfo &zookeeper_info_, LoadingStrictnessLevel mode,
                                                       const StorageID &table_id_, const String &relative_data_path_,
                                                       const StorageInMemoryMetadata &metadata_, ContextMutablePtr context_,
                                                       const String &date_column_name, const MergingParams &merging_params_,
                                                       std::unique_ptr<MergeTreeSettings> settings_, bool need_check_structure,
                                                       const ZooKeeperRetriesInfo &create_query_zookeeper_retries_info_)
    : MergeTreeData(table_id_, metadata_, context_, date_column_name, merging_params_, std::move(settings_),
                    true,  /// require_part_metadata
                    mode, [this](const std::string &name) { enqueuePartForCheck(name); }),
      ...
      replicated_fetches_throttler(std::make_shared<Throttler>((*getSettings())[MergeTreeSetting::max_replicated_fetches_network_bandwidth],
                                                               getContext()->getReplicatedFetchesThrottler())),
      replicated_sends_throttler(std::make_shared<Throttler>((*getSettings())[MergeTreeSetting::max_replicated_sends_network_bandwidth],
                                                             getContext()->getReplicatedSendsThrottler())) {
  ...
  queue_updating_task = getContext()->getSchedulePool().createTask(
      getStorageID(), getStorageID().getFullTableName() + " (StorageReplicatedMergeTree::queueUpdatingTask)",
      [this] { queueUpdatingTask(); });

  queue_updating_task->deactivate();

  mutations_updating_task = getContext()->getSchedulePool().createTask(
      getStorageID(), getStorageID().getFullTableName() + " (StorageReplicatedMergeTree::mutationsUpdatingTask)",
      [this] { mutationsUpdatingTask(); });

  mutations_updating_task->deactivate();

  merge_selecting_task = getContext()->getSchedulePool().createTask(
      getStorageID(), getStorageID().getFullTableName() + " (StorageReplicatedMergeTree::mergeSelectingTask)",
      [this] { mergeSelectingTask(); });
  ...
  mutations_finalizing_task = getContext()->getSchedulePool().createTask(
      getStorageID(), getStorageID().getFullTableName() + " (StorageReplicatedMergeTree::mutationsFinalizingTask)",
      [this] { mutationsFinalizingTask(); });
  ...
  if (LoadingStrictnessLevel::ATTACH <= mode) {
    try {
      ...
      if (current_zookeeper && current_zookeeper->exists(replica_path + "/flags/force_restore_data")) {
        ...
        LOG_WARNING(log, "Skipping the limits on severity of changes to data parts and columns (flag {}/flags/force_restore_data).",
                    replica_path);
        ...
        if (current_zookeeper->tryGetChildren(fs::path(replica_path) / "parts", parts_on_replica) == Coordination::Error::ZOK) {
          ...
          for (const auto &part : parts_on_replica) expected_parts_on_this_replica->insert(part);
        }
      }
    } catch (const Coordination::Exception &e) {
      if (!Coordination::isHardwareError(e.code)) throw;
  ...
  if (LoadingStrictnessLevel::ATTACH <= mode) {
    ...
    if ((*settings)[MergeTreeSetting::parts_to_delay_insert] &&
        max_parts_in_partition < (*settings)[MergeTreeSetting::parts_to_delay_insert]) {
      Float64 ratio =
          1.0 - static_cast<Float64>(max_parts_in_partition) / static_cast<Float64>((*settings)[MergeTreeSetting::parts_to_delay_insert]);
      merge_selecting_sleep_ms =
          static_cast<UInt64>(interpolateLinear(static_cast<double>((*settings)[MergeTreeSetting::merge_selecting_sleep_ms]),
                                                static_cast<double>((*settings)[MergeTreeSetting::max_merge_selecting_sleep_ms]), ratio));
    }
  }
  ...
  try {
    ...
    try {
      ...
      bool same_structure = checkTableStructure(zookeeper_path, metadata_snapshot, &metadata_version, need_check_structure,
                                                getCreateQueryZooKeeperRetriesInfo());
    ...
    if (!is_first_replica) createReplica(metadata_snapshot, getCreateQueryZooKeeperRetriesInfo());

    createNewZooKeeperNodes(getCreateQueryZooKeeperRetriesInfo());
    syncPinnedPartUUIDs(getCreateQueryZooKeeperRetriesInfo());

    if (!has_metadata_in_zookeeper.has_value() || *has_metadata_in_zookeeper) createTableSharedID(getCreateQueryZooKeeperRetriesInfo());
  } catch (...) {
...
void StorageReplicatedMergeTree::waitMutationToFinishOnReplicas(const Strings &replicas, const String &mutation_id) const {
  ...
  if (CurrentThread::isInitialized()) {
    ...
    if (query_context) process_list_element = query_context->getProcessListElement();
  }
  ...
  for (const String &replica : *all_required_replicas) {
    ...
    while (!partial_shutdown_called) {
      ...
      if (!zookeeper->exists(fs::path(zookeeper_path) / "replicas" / replica / "is_active")) {
        LOG_WARNING(log, "Replica {} is not active during mutation. Mutation will be done asynchronously when replica becomes active.",
                    replica);
      ...
      if (!zookeeper->tryGet(mutation_pointer, mutation_pointer_value, nullptr, wait_event)) {
        LOG_WARNING(log,
                    "Replica {} was removed during mutation. "
                    "Mutation will be done asynchronously when replica is restored.",
                    replica);
        inactive_replicas.emplace(replica);
        break;
      }
      if (mutation_pointer_value >= mutation_id)  /// Maybe we already processed more fresh mutation
      {
        ...
        LOG_TRACE(log, "Mutation {} is done because mutation pointer is {}, but state is not updated in memory, will wait", mutation_id,
      ...
      if (mutation_status && !mutation_status->latest_fail_reason.empty()) {
        LOG_DEBUG(log, "Mutation {} is done {} or failed {} (status: '{}')", mutation_id, mutation_status->is_done,
                  !mutation_status->latest_fail_reason.empty(), mutation_status->latest_fail_reason);

        /// In some cases latest_fail_reason may be retryable and there's a chance it will be cleared after the next attempt
        if (++retries_on_failed_mutation <= MAX_RETRIES_ON_FAILED_MUTATION) continue;
        ...
        throw Exception(ErrorCodes::LOGICAL_ERROR, "checkMutationStatus didn't throw when checking status of {}: {}", mutation_id,
                        mutation_status->latest_fail_reason);
      }
    }

    /// This replica inactive, don't check anything
    if (!inactive_replicas.empty() && inactive_replicas.contains(replica)) break;
...
void StorageReplicatedMergeTree::createNewZooKeeperNodesAttempt() const {
  ...
  if ((*settings)[MergeTreeSetting::allow_remote_fs_zero_copy_replication]) {
    ...
    futures.push_back(
        zookeeper->asyncTryCreateNoThrow(zookeeper_path + "/zero_copy_hdfs/shared", String(), zkutil::CreateMode::Persistent));
  }

  /// Part movement.
  futures.push_back(zookeeper->asyncTryCreateNoThrow(zookeeper_path + "/part_moves_shard", String(), zkutil::CreateMode::Persistent));
  futures.push_back(zookeeper->asyncTryCreateNoThrow(zookeeper_path + "/pinned_part_uuids", getPinnedPartUUIDs()->toString(),
                                                     zkutil::CreateMode::Persistent));
  /// For ALTER PARTITION with multi-leaders
  futures.push_back(
      zookeeper->asyncTryCreateNoThrow(zookeeper_path + "/alter_partition_version", String(), zkutil::CreateMode::Persistent));
  ...
  if (zookeeper->exists(zookeeper_path + "/temp")) {
    ...
    futures.push_back(zookeeper->asyncTryCreateNoThrow(zookeeper_path + "/temp/" + EphemeralLockInZooKeeper::LEGACY_LOCK_INSERT, String(),
                                                       zkutil::CreateMode::Persistent));
    futures.push_back(zookeeper->asyncTryCreateNoThrow(zookeeper_path + "/temp/" + EphemeralLockInZooKeeper::LEGACY_LOCK_OTHER, String(),
                                                       zkutil::CreateMode::Persistent));
  }
  ...
  futures.push_back(
      zookeeper->asyncTryCreateNoThrow(zookeeper_path + "/lightweight_updates/in_progress", String(), zkutil::CreateMode::Persistent));

  for (auto &future : futures) {
    auto res = future.get();
    if (res.error != Coordination::Error::ZOK && res.error != Coordination::Error::ZNODEEXISTS)
      throw Coordination::Exception(res.error, "Failed to create new nodes {} at {} with error {}", res.path_created, zookeeper_path,
                                    Coordination::errorMessage(res.error));
  }
}
...
bool StorageReplicatedMergeTree::createTableIfNotExistsAttempt(const StorageMetadataPtr &metadata_snapshot,
                                                               QueryStatusPtr process_list_element) const {
  auto zookeeper = getZooKeeper();
  ...
  for (size_t i = 0; i < 1000; ++i) {
    ...
    ops.emplace_back(zkutil::makeCreateRequest(zookeeper_path + "/blocks", "", zkutil::CreateMode::Persistent));
    ops.emplace_back(zkutil::makeCreateRequest(zookeeper_path + "/async_blocks", "", zkutil::CreateMode::Persistent));
    ops.emplace_back(zkutil::makeCreateRequest(zookeeper_path + "/deduplication_hashes", "", zkutil::CreateMode::Persistent));
    ops.emplace_back(zkutil::makeCreateRequest(zookeeper_path + "/block_numbers", "", zkutil::CreateMode::Persistent));
    ops.emplace_back(zkutil::makeCreateRequest(zookeeper_path + "/nonincrement_block_numbers", "",
                                               zkutil::CreateMode::Persistent));  /// /nonincrement_block_numbers dir is unused, but is
                                                                                  /// created nonetheless for backwards compatibility.
    ...
    ops.emplace_back(zkutil::makeCreateRequest(zookeeper_path + "/quorum", "", zkutil::CreateMode::Persistent));
    ops.emplace_back(zkutil::makeCreateRequest(zookeeper_path + "/quorum/last_part", "", zkutil::CreateMode::Persistent));
    ops.emplace_back(zkutil::makeCreateRequest(zookeeper_path + "/quorum/failed_parts", "", zkutil::CreateMode::Persistent));
    ops.emplace_back(zkutil::makeCreateRequest(zookeeper_path + "/mutations", "", zkutil::CreateMode::Persistent));
    ...
    ops.emplace_back(zkutil::makeCreateRequest(replica_path + "/metadata", metadata_str, zkutil::CreateMode::Persistent));
    ...
    ops.emplace_back(zkutil::makeCreateRequest(replica_path + "/min_unprocessed_insert_time", "", zkutil::CreateMode::Persistent));
    ops.emplace_back(zkutil::makeCreateRequest(replica_path + "/max_processed_insert_time", "", zkutil::CreateMode::Persistent));
    ops.emplace_back(zkutil::makeCreateRequest(replica_path + "/mutation_pointer", "", zkutil::CreateMode::Persistent));

    ops.emplace_back(zkutil::makeCreateRequest(
        replica_path + "/creator_info", toString(getStorageID().uuid) + "|" + toString(ServerUUID::get()), zkutil::CreateMode::Persistent));
...
void StorageReplicatedMergeTree::createReplicaAttempt(const StorageMetadataPtr &metadata_snapshot,
                                                      QueryStatusPtr process_list_element) const {
  auto zookeeper = getZooKeeper();
  ...
  std::vector<String> paths_exists = {replica_path + "/host",
  ...
  if (all_nodes_exist) {
    ...
    if (zk_host.empty() && zk_log_pointer.empty() && zk_queue.empty() && zk_parts.empty() && zk_flags.empty() &&
        (zk_is_lost == "0" || zk_is_lost == "1") && zk_metadata == local_metadata && zk_columns == local_columns &&
        zk_metadata_version == local_metadata_version && zk_min_unprocessed_insert_time.empty() && zk_max_processed_insert_time.empty() &&
        zk_mutation_pointer.empty() && zk_creator_info == creator_info) {
  ...
  do {
    ...
    ops.emplace_back(zkutil::makeCreateRequest(replica_path + "/metadata", local_metadata, zkutil::CreateMode::Persistent));
    ops.emplace_back(zkutil::makeCreateRequest(replica_path + "/columns", local_columns, zkutil::CreateMode::Persistent));
    ops.emplace_back(zkutil::makeCreateRequest(replica_path + "/metadata_version", local_metadata_version, zkutil::CreateMode::Persistent));
    ...
    ops.emplace_back(zkutil::makeCreateRequest(replica_path + "/creator_info", creator_info, zkutil::CreateMode::Persistent));
...
std::vector<String> StorageReplicatedMergeTree::getZookeeperZeroCopyLockPaths() const {
  ...
  for (const auto &disk : disks) {
    if (!disk->supportZeroCopyReplication()) continue;
...
void StorageReplicatedMergeTree::dropZookeeperZeroCopyLockPaths(zkutil::ZooKeeperPtr zookeeper, std::vector<String> zero_copy_locks_paths,
  ...
  for (const auto &zero_copy_locks_root : zero_copy_locks_paths) {
    ...
    if (code == Coordination::Error::ZNOTEMPTY) {
      LOG_WARNING(logger,
                  "Zero copy locks are not empty for {}. There are some lost locks inside."
                  "Removing them all.",
                  zero_copy_locks_root);
...
void StorageReplicatedMergeTree::drop() {
  ...
  if (maybe_has_metadata_in_zookeeper) {
    ...
    if (zookeeper->tryGet(lost_part_count_path, lost_part_count_str, &lost_part_count_stat)) {
      ...
      if (lost_part_count > 0) LOG_INFO(log, "Dropping table with non-zero lost_part_count equal to {}", lost_part_count);
    }
...
bool StorageReplicatedMergeTree::dropReplica(zkutil::ZooKeeperPtr zookeeper, const TableZnodeInfo &zookeeper_info, LoggerPtr logger,
...

  {
    ...
    if (table_settings && (*table_settings)[MergeTreeSetting::use_minimalistic_part_header_in_zookeeper]) flat_nodes.emplace_back("parts");
    for (const auto &node : flat_nodes) {
      bool removed_quickly = zookeeper->tryRemoveChildrenRecursive(fs::path(remote_replica_path) / node, /* probably flat */ true);
      if (!removed_quickly)
        LOG_WARNING(logger, "Failed to quickly remove node '{}' and its children, fell back to recursive removal (replica: {})", node,
                    remote_replica_path);
    }

    /// Then try to remove nodes that are known to have no children (and should always exist)
    Coordination::Requests ops;
    for (const auto &node : flat_nodes) ops.emplace_back(zkutil::makeRemoveRequest(remote_replica_path + "/" + node, -1));
    ...
    if (code != Coordination::Error::ZOK)
      LOG_WARNING(logger, "Cannot quickly remove nodes without children: {} (replica: {}). Will remove recursively.", code,
                  remote_replica_path);
    ...
    if (has_metadata_out) *has_metadata_out = false;

    if (zookeeper->tryRemove(remote_replica_path) != Coordination::Error::ZOK)
      LOG_ERROR(logger, "Replica was not completely removed from ZooKeeper, {} still exists and may contain some garbage.",
                remote_replica_path);
  }

  /// Check that `zookeeper_path` exists: it could have been deleted by another replica after execution of previous line.
  Strings replicas;
  if (Coordination::Error::ZOK != zookeeper->tryGetChildren(zookeeper_path + "/replicas", replicas) || !replicas.empty()) return false;
...
bool StorageReplicatedMergeTree::removeTableNodesFromZooKeeper(zkutil::ZooKeeperPtr zookeeper, const TableZnodeInfo &zookeeper_info2,
  ...
  static constexpr std::array flat_nodes = {"block_numbers",   "blocks", "async_blocks", "deduplication_hashes",
                                            "leader_election", "log",    "mutations",    "pinned_part_uuids"};
  ...
  for (const auto *node : flat_nodes) {
    ...
    if (!removed_quickly)
      LOG_WARNING(logger, "Failed to quickly remove node '{}' and its children, fell back to recursive removal (table: {})", node,
                  zookeeper_path);
  }

  /// Then try to remove nodes that are known to have no children (and should always exist)
  Coordination::Requests ops;
  for (const auto *node : flat_nodes) ops.emplace_back(zkutil::makeRemoveRequest(zookeeper_path + "/" + node, -1));
  ...
  if (code != Coordination::Error::ZOK)
    LOG_WARNING(logger, "Cannot quickly remove nodes without children: {} (table: {}). Will remove recursively.", code, zookeeper_path);
  ...
  for (const auto &child : children) {
    if (child != "dropped") zookeeper->tryRemoveRecursive(fs::path(zookeeper_path) / child);
  }
  ...
  if (code == Coordination::Error::ZNOTEMPTY) {
    ...
    try {
      ...
      LOG_WARNING(logger, "Failed to drop ancestor znodes {} - {} after dropping table: {}", zookeeper_info2.path_prefix_for_drop,
                  zookeeper_info2.path, getCurrentExceptionMessage(false));
    }
  }
 ...
 */
bool StorageReplicatedMergeTree::checkTableStructure(const String &zookeeper_prefix, const StorageMetadataPtr &metadata_snapshot,
...
bool StorageReplicatedMergeTree::checkTableStructureAttempt(const String &zookeeper_prefix, const StorageMetadataPtr &metadata_snapshot,
  ...
  auto metadata_from_zk = ReplicatedMergeTreeTableMetadata::parse(metadata_str);
  bool is_metadata_equal = old_metadata.checkEquals(metadata_from_zk, metadata_snapshot->getColumns(), getStorageID().getNameForLogs(),
                                                    getContext(), /*check_index_granularity*/ true, strict_check, log.load());

  if (metadata_version) *metadata_version = metadata_stat.version;

  Coordination::Stat columns_stat;
  auto columns_from_zk = ColumnsDescription::parse(zookeeper->get(fs::path(zookeeper_prefix) / "columns", &columns_stat));

  const ColumnsDescription &old_columns = metadata_snapshot->getColumns();
  if (columns_from_zk == old_columns && is_metadata_equal) return true;
...
void StorageReplicatedMergeTree::setTableStructure(const StorageID &table_id, const ContextPtr &local_context,
                                                   ...
                                                   const ReplicatedMergeTreeTableMetadata::Diff &metadata_diff,
  ...
  try {
    DatabaseCatalog::instance()
        ...
        ->alterTable(local_context, table_id, new_metadata, /*validate_new_create_query=*/false);
  } catch (...) {
...
static time_t tryGetPartCreateTime(zkutil::ZooKeeperPtr &zookeeper, const String &replica_path, const String &part_name) {
  ...
  String unused;
  if (zookeeper->tryGet(fs::path(replica_path) / "parts" / part_name, unused, &stat)) res = stat.ctime / 1000;

  return res;
}

void StorageReplicatedMergeTree::paranoidCheckForCoveredPartsInZooKeeperOnStart(const Strings &parts_in_zk,
                                                                                const Strings &parts_to_fetch) const {
  ...
  if (!paranoid_check_for_covered_parts) return;

  ActiveDataPartSet active_set(format_version);
  for (const auto &part_name : parts_in_zk) active_set.add(part_name);
  ...
  for (const auto &part_name : parts_in_zk) {
    ...
    if (part_name == covering_part) continue;

    bool found = false;
    for (const DiskPtr &disk : disks)
      if (disk->existsDirectory(fs::path(path) / part_name)) found = true;

    if (!found) found = std::find(parts_to_fetch.begin(), parts_to_fetch.end(), part_name) != parts_to_fetch.end();
    ...
    if (!found) {
      LOG_WARNING(log,
                  ...
                  part_name, getStorageID().getNameForLogs(), covering_part);
...
void StorageReplicatedMergeTree::checkParts(bool skip_sanity_checks) {
  if (checkPartsImpl(skip_sanity_checks)) return;
...
bool StorageReplicatedMergeTree::checkPartsImpl(bool skip_sanity_checks) {
  ...
  for (const String &missing_name : expected_parts)
    if (!getActiveContainingPart(missing_name)) parts_to_fetch.push_back(missing_name);
  ...
  if (!incomplete_list_of_outdated_parts) paranoidCheckForCoveredPartsInZooKeeperOnStart(expected_parts_vec, parts_to_fetch);
  ...
  for (const auto &load_state : unexpected_data_parts) {
    if (load_state.is_broken || load_state.part->rows_count || !load_state.uncovered) continue;

    set_of_empty_unexpected_parts.add(load_state.part->name);
  }
  if (auto empty_count = set_of_empty_unexpected_parts.size())
    LOG_WARNING(log, "Found {} empty unexpected parts (probably some dropped parts were not cleaned up before restart): [{}]", empty_count,
                fmt::join(set_of_empty_unexpected_parts.getParts(), ", "));
  ...
  for (const auto &load_state : unexpected_data_parts) {
    if (load_state.is_broken) continue;
    ...
    if (incomplete_list_of_outdated_parts) {
      LOG_INFO(log,
               "Outdated parts are not loaded yet, but we may need them to check if unexpected parts can be recovered. "
               "Need retry.");
      return false;
    }
  ...
  const UInt64 parts_to_fetch_blocks =
      std::accumulate(parts_to_fetch.cbegin(), parts_to_fetch.cend(), 0, [&](UInt64 acc, const String &part_name) {
        if (const auto part_info = MergeTreePartInfo::tryParsePartName(part_name, format_version)) return acc + part_info->getBlocksCount();
  ...
  for (const auto &part : parts) total_rows_on_filesystem += part->rows_count;
  /// We need to sum the rows count of all unexpected data parts;
  for (const auto &part : unexpected_data_parts) total_rows_on_filesystem += part.part->rows_count;

  const auto storage_settings_ptr = getSettings();
  bool insane =
      static_cast<double>(uncovered_unexpected_parts_rows) >
      static_cast<double>(total_rows_on_filesystem) * (*storage_settings_ptr)[MergeTreeSetting::replicated_max_ratio_of_wrong_parts];

  constexpr auto sanity_report_fmt =
  ...
  constexpr auto sanity_report_debug_fmt =
      "Uncovered unexpected parts: {}. Restorable unexpected parts: {}. Missing parts: {}. Covered unexpected parts: {}. Expected parts: "
      "{}.";

  if (insane && !skip_sanity_checks) {
    LOG_DEBUG(log, sanity_report_debug_fmt, fmt::join(uncovered_unexpected_parts, ", "), fmt::join(restorable_unexpected_parts, ", "),
              fmt::join(parts_to_fetch, ", "), fmt::join(covered_unexpected_parts, ", "), fmt::join(expected_parts, ", "));
    throw Exception(ErrorCodes::TOO_MANY_UNEXPECTED_DATA_PARTS, sanity_report_fmt, getStorageID().getNameForLogs(),
                    ...
                    uncovered_unexpected_parts.size(), uncovered_unexpected_parts_rows, unexpected_parts_nonnew,
                    unexpected_parts_nonnew_rows, parts_to_fetch.size(), parts_to_fetch_blocks, covered_unexpected_parts.size(),
  ...
  if (unexpected_parts_nonnew_rows > 0 || uncovered_unexpected_parts_rows > 0 || !restorable_unexpected_parts.empty()) {
    LOG_DEBUG(log, sanity_report_debug_fmt, fmt::join(uncovered_unexpected_parts, ", "), fmt::join(restorable_unexpected_parts, ", "),
              fmt::join(parts_to_fetch, ", "), fmt::join(covered_unexpected_parts, ", "), fmt::join(expected_parts, ", "));
    LOG_WARNING(log, sanity_report_fmt, getStorageID().getNameForLogs(), formatReadableQuantity(uncovered_unexpected_parts_rows),
                formatReadableQuantity(total_rows_on_filesystem), uncovered_unexpected_parts.size(), uncovered_unexpected_parts_rows,
                unexpected_parts_nonnew, unexpected_parts_nonnew_rows, parts_to_fetch.size(), parts_to_fetch_blocks,
                covered_unexpected_parts.size(), unexpected_parts_rows - uncovered_unexpected_parts_rows);
  } else {
  ...
  for (auto &part_state : unexpected_data_parts) part_state.part->renameToDetached("ignored", /* ignore_error= */ true);
...
bool StorageReplicatedMergeTree::checkPartChecksumsAndAddCommitOps(const ZooKeeperWithFaultInjectionPtr &zookeeper, const DataPartPtr &part,
                                                                   Coordination::Requests &ops, String part_name,
  ...
  for (const String &replica : replicas) {
    ...
    part_found = true;
    if (replica == replica_name) part_exists_on_our_replica = true;
      ...
      if (zookeeper->tryGet(fs::path(current_part_path) / "columns", columns_str) &&
        ...
        if (zookeeper->exists(current_part_path))
          throw Exception(ErrorCodes::LOGICAL_ERROR,
                          "Part {} has empty header and does not have columns and checksums. "
                          "Looks like a bug.",
                          current_part_path);
    ...
    if (replica_part_header.getColumnsHash() != local_part_header.getColumnsHash()) {
      ...
      throw Exception(ErrorCodes::CHECKSUM_DOESNT_MATCH,
                      "Part {} from {} has different columns hash "
                      "(it may rarely happen on race condition with KILL MUTATION).",
                      part_name, replica);
    }
  ...
  if (part_found) absent_replicas_paths.clear();
  ...
  if (!part_exists_on_our_replica) {
    ...
    if ((*storage_settings_ptr)[MergeTreeSetting::use_minimalistic_part_header_in_zookeeper]) {
      ...
      ops.emplace_back(
          zkutil::makeCreateRequest(fs::path(part_path) / "columns", part->getColumns().toString(), zkutil::CreateMode::Persistent));
      ops.emplace_back(zkutil::makeCreateRequest(fs::path(part_path) / "checksums", getChecksumsForZooKeeper(part->checksums),
                                                 zkutil::CreateMode::Persistent));
    }
  } else {
...
bool StorageReplicatedMergeTree::getOpsToCheckPartChecksumsAndCommit(const ZooKeeperWithFaultInjectionPtr &zookeeper,
                                                                     ...
                                                                     bool replace_zero_copy_lock, Coordination::Requests &ops,
...
MergeTreeData::MutableDataPartPtr StorageReplicatedMergeTree::attachPartHelperFoundValidPart(const LogEntry &entry,
                                                                                             PartsTemporaryRename &rename_parts) const {
  if (format_version != MERGE_TREE_DATA_MIN_FORMAT_VERSION_WITH_CUSTOM_PARTITIONING) return {};
  ...
  std::erase_if(detached_parts, [&partition_id](const DetachedPartInfo &detached_part_info) {
    return !detached_part_info.valid_name || !detached_part_info.prefix.empty() ||
  ...
  std::erase_if(detached_parts, [&](const DetachedPartInfo &detached_part_info) {
    ...
    auto part =
        getDataPartBuilder(entry.new_part_name, volume, fs::path(DETACHED_DIR_NAME) / detached_part_info.dir_name, getReadSettings())
    ...
    try {
      ...
      tryLogCurrentException(log, fmt::format("data race is possible when reading checksums.txt from detached part or part does not have a "
                                              "file checksums.txt, part {} is ignored",
                                              detached_part_info.dir_name));
      return true;
    }
  ...
  if (detached_parts.empty()) return {};

  for (auto &detached_part_info : detached_parts) {
    chassert(rename_parts.old_and_new_names.empty());
    rename_parts.addPart(detached_part_info.getPartNameV1(), detached_part_info.dir_name, "attaching_" + detached_part_info.dir_name,
                         detached_part_info.disk);

    try {
      rename_parts.tryRenameAll();
    } catch (...) {
      tryLogCurrentException(
          log, fmt::format("data race is possible when moving detached part, part {} is ignored", detached_part_info.dir_name));
      rename_parts.rollBackAll();
      continue;
    }

    const auto volume = std::make_shared<SingleDiskVolume>("volume_" + detached_part_info.dir_name, detached_part_info.disk);
    auto part = getDataPartBuilder(entry.new_part_name, volume,
                                   fs::path(rename_parts.source_dir) / rename_parts.old_and_new_names.front().new_dir, getReadSettings())
                    .withPartFormatFromDisk()
                    .build();
    ...
    try {
      ...
      if (isRetryableException(std::current_exception())) {
        tryLogCurrentException(
            log, fmt::format("unable to load part {}, however it does not look as broken, ignore it", detached_part_info.dir_name));
      } else {
...
bool StorageReplicatedMergeTree::executeLogEntry(LogEntry &entry) {
  fiu_do_on(FailPoints::replicated_queue_fail_next_entry, { entry.fault_injected = true; });
  fiu_do_on(FailPoints::replicated_queue_unfail_entries, { entry.fault_injected = false; });
  ...
  if (is_get_or_attach || entry.type == LogEntry::MERGE_PARTS || entry.type == LogEntry::MUTATE_PART) {
    ...
    if (!existing_part) existing_part = getActiveContainingPart(entry.new_part_name);
  ...
  if (entry.type == LogEntry::ATTACH_PART) {
    ...
    if (MutableDataPartPtr part = attachPartHelperFoundValidPart(entry, renamed_parts)) {
      ...
      writePartLog(PartLogElement::Type::NEW_PART, {}, 0 /** log entry is fake so we don't measure the time */, part->name, part,
                   {} /** log entry is fake so there are no initial parts */, nullptr, profile_events_scope.getSnapshot());

      return true;
    }

    LOG_TRACE(log, "Didn't find valid local part for {} ({}), will fetch it from other replica", entry.new_part_name,
...
bool StorageReplicatedMergeTree::executeFetch(LogEntry &entry, bool need_to_check_missing_part) {
  ...
  try {
    if (replica.empty()) {
       ...
       */
      if (entry.quorum) {
        if (entry.type != LogEntry::GET_PART) throw Exception(ErrorCodes::LOGICAL_ERROR, "Log entry with quorum but type is not GET_PART");

        LOG_DEBUG(log, "No active replica has part {} which needs to be written with quorum. Will try to mark that quorum as failed.",
                  entry.new_part_name);
        ...
        if (replica.empty()) {
          ...
          if (quorum_entry.part_name == entry.new_part_name) {
            ...
            ops.emplace_back(zkutil::makeCreateRequest(fs::path(zookeeper_path) / "quorum" / "failed_parts" / entry.new_part_name, "",
                                                       zkutil::CreateMode::Persistent));
            ...
            LOG_WARNING(log,
                        ...
                        "Or it is a bug.",
                        entry.new_part_name, quorum_entry.part_name, entry.quorum);
          }
        }
      }
      ...
      if (replica.empty()) {
        ...
        if (!need_to_check_missing_part) return false;
    ...
    try {
      ...
      if (!fetchPart(part_name, metadata_snapshot, zookeeper_info.zookeeper_name, source_replica_path,
                     /* to_detached= */ false, entry.quorum,
                     /* zookeeper_ */ nullptr,
      ...
      if (e.code() == ErrorCodes::RECEIVED_ERROR_TOO_MANY_REQUESTS) e.addMessage("Too busy replica. Will try later.");
      throw;
    }

    if (entry.type == LogEntry::MERGE_PARTS) ProfileEvents::increment(ProfileEvents::ReplicatedPartFetchesOfMerged);
  } catch (...) {
    ...
    try {
       ...
       */
      if (replica.empty()) enqueuePartForCheck(entry.new_part_name);
    } catch (...) {
...
MergeTreeData::MutableDataPartPtr StorageReplicatedMergeTree::executeFetchShared(const String &source_replica, const String &new_part_name,
                                                                                 const DiskPtr &disk, const String &path) {
...
static void paranoidCheckForCoveredPartsInZooKeeper(const ZooKeeperPtr &zookeeper, const String &replica_path,
  ...
  for (const auto &part_name : parts_remain) {
    ...
    if (part_info && dominated_info.contains(*part_info)) orphaned_parts.push_back(part_name);
  }

  if (orphaned_parts.empty()) return;

  throw Exception(ErrorCodes::LOGICAL_ERROR, "Part(s) [{}] in ZooKeeper of table {} are covered by DROP_RANGE {}",
                  fmt::join(orphaned_parts, ", "), storage.getStorageID().getNameForLogs(), covering_part_name);
}
  ...
  auto has_preactive_in_range = [&]() {
    ...
    for (auto it = begin; it != end; ++it) {
      if (drop_range.contains((*it)->info)) {
        LOG_TRACE(log, "Waiting for PreActive part {} to finish committing before DROP_RANGE", (*it)->name);
        return true;
      }
    }
    return false;
...
void StorageReplicatedMergeTree::executeDropRange(const LogEntry &entry) {
  ...
  PartsToRemoveFromZooKeeper parts_to_remove;
  {
    ...
    parts_to_remove = removePartsInRangeFromWorkingSetAndGetPartsToRemoveFromZooKeeper(NO_TRANSACTION_RAW, drop_range_info, data_parts_lock,
                                                                                       /*create_empty_part=*/true, clone_to_detached);
...
bool StorageReplicatedMergeTree::executeReplaceRange(LogEntry &entry) {
  ...
  LOG_DEBUG(log, "Executing log entry {} to replace parts range {} with {} parts from {}.{}", entry.znode_name,
            entry_replace.drop_range_part_name, entry_replace.new_part_names.size(), entry_replace.from_database, entry_replace.from_table);
  ...
  auto table_lock_holder_dst_table =
  ...
  for (size_t i = 0; i < entry_replace.new_part_names.size(); ++i) {
    all_parts.emplace_back(std::make_shared<PartDescription>(i, entry_replace.src_part_names.at(i), entry_replace.new_part_names.at(i),
                                                             entry_replace.part_names_checksums.at(i), format_version));
  }

  if (replace) waitForPreActivePartsInRange(drop_range);
  ...
  auto clone_data_parts_from_source_table = [&]() -> size_t {
    ...
    try {
      ...
      LOG_INFO(log, "Can't use {} as source table for REPLACE PARTITION command. Will fetch all parts. Reason: {}",
               source_table_id.getNameForLogs(), getCurrentExceptionMessage(false));
      return 0;
    }
    ...
    DataPartStates valid_states{MergeTreeDataPartState::PreActive, MergeTreeDataPartState::Active, MergeTreeDataPartState::Outdated};
    ...
    for (PartDescriptionPtr &part_desc : parts_to_add) {
      ...
      if (avoid_copy_local_part) {
        LOG_DEBUG(log, "Avoid copy local part {} from table {} because of zero-copy replication", part_desc->src_part_name,
  ...
  for (PartDescriptionPtr &part_desc : parts_to_add) {
    if (!adding_parts_active_set.getContainingPart(part_desc->new_part_info).empty()) continue;
    ...
    if (auto *replicated_src_table = typeid_cast<StorageReplicatedMergeTree *>(source_table.get())) {
      ...
      if (!src_replica.empty()) {
        LOG_DEBUG(log, "Found part {} on replica {} of source table, will not check part {} required for {}", part_desc->src_part_name,
                  src_replica, part_desc->new_part_name, entry.znode_name);
        need_check = false;
      }
    }
    ...
    throw Exception(ErrorCodes::NO_REPLICA_HAS_PART, "Not found part {} (or part covering it) neither source table neither remote replicas",
  ...
  Strings final_part_names;
  {
    ...
    for (const String &final_part_name : final_part_names) {
      ...
      if (!part_desc) throw Exception(ErrorCodes::LOGICAL_ERROR, "There is no final part {}. This is a bug", final_part_name);
  ...
  auto obtain_part = [&](PartDescriptionPtr &part_desc) {
    ...
    bool prefer_fetch_from_other_replica =
        !part_desc->replica.empty() && (*storage_settings_ptr)[MergeTreeSetting::allow_remote_fs_zero_copy_replication] &&
    ...
    if (part_desc->src_table_part && !prefer_fetch_from_other_replica) {
      ...
      auto [res_part, temporary_part_lock] = cloneAndLoadDataPart(
          part_desc->src_table_part, TMP_PREFIX_REPLACE_PARTITION_FROM + "clone_", part_desc->new_part_info, metadata_snapshot,
          clone_params, getContext()->getReadSettings(), getContext()->getWriteSettings(), true /*must_on_same_disk*/);
      ...
      if (interserver_scheme != address.scheme)
        throw Exception(ErrorCodes::LOGICAL_ERROR, "Interserver schemas are different '{}' != '{}', can't fetch part from {}",
                        interserver_scheme, address.scheme, address.host);

      auto [fetched_part, temporary_part_lock] = fetcher.fetchSelectedPart(
          metadata_snapshot, getContext(), part_desc->found_new_part_name, zookeeper_info.zookeeper_name, source_replica_path, address.host,
          address.replication_port, timeouts, credentials->getUser(), credentials->getPassword(), interserver_scheme,
          replicated_fetches_throttler, false, TMP_PREFIX_REPLACE_PARTITION_FROM + "fetch_");
  ...
  for (PartDescriptionPtr &part_desc : final_parts) obtain_part(part_desc);

  MutableDataPartsVector res_parts;
  for (PartDescriptionPtr &part_desc : final_parts) res_parts.emplace_back(part_desc->res_part);
  ...
  try {
    ...
    if (!ops.empty()) zookeeper->multi(ops);
...

    {
      ...
      if (replace) {
        ...
        LOG_TRACE(log, "Replacing {} parts {}with {} parts {}", parts_to_remove.size(), parts_to_remove_str, final_parts.size(),
                  boost::algorithm::join(final_part_names, ", "));
      }
    }

    PartLog::addNewParts(getContext(), PartLog::createPartLogEntries(res_parts, watch.elapsed(), profile_events_scope.getSnapshot()));
  } catch (...) {
    PartLog::addNewParts(getContext(), PartLog::createPartLogEntries(res_parts, watch.elapsed()),
                         ExecutionStatus::fromCurrentException("", true));

    for (const auto &res_part : res_parts) unlockSharedData(*res_part);
...
void StorageReplicatedMergeTree::executeClonePartFromShard(const LogEntry &entry) {
...

  {
    ...
    auto get_part = [&, address, timeouts, credentials, interserver_scheme]() {
      ...
      auto [fetched_part, lock] =
          fetcher.fetchSelectedPart(metadata_snapshot, getContext(), entry.new_part_name, zookeeper_info.zookeeper_name,
                                    source_replica_path, address.host, address.replication_port, timeouts, credentials->getUser(),
                                    credentials->getPassword(), interserver_scheme, replicated_fetches_throttler, true);
...
void StorageReplicatedMergeTree::cloneReplica(const String &source_replica, Coordination::Stat source_is_lost_stat,
                                              zkutil::ZooKeeperPtr &zookeeper) {
  ...
  while (true) {
    if (shutdown_called) throw Exception(ErrorCodes::ABORTED, "Cannot clone replica because shutdown called");
    ...
    if (rc == Coordination::Error::ZNODEEXISTS) {
      throw Exception(ErrorCodes::REPLICA_STATUS_CHANGED, "Can not clone replica, because the {} updated to new ClickHouse version",
                      source_replica);
    }
...

  {
    ...
    for (const String &entry_name : source_queue_names) queue_get_paths.push_back(fs::path(source_path) / "queue" / entry_name);
    ...
    for (size_t i = 0; i < source_queue_names.size(); ++i) {
      ...
      if (res.error == Coordination::Error::ZNONODE) continue;
      ...
      if (info.parsed_entry->type == LogEntry::DROP_RANGE || info.parsed_entry->type == LogEntry::DROP_PART) {
        ...
        if (maybe_covering_drop_range.empty()) get_part_set.add(info.parsed_entry->new_part_name);
      } else {
  ...
  for (const auto &active_part : source_replica_parts) get_part_set.add(active_part);
  ...
  if (test_delay) randomDelayForMaxMilliseconds(test_delay, log.load(), "cloneReplica: Before removing local parts");
  ...
  for (const auto &part : local_parts_in_zk) {
    ...
    if (get_part_set.getContainingPart(part) == part) continue;

    if (exact_part_names.contains(part)) continue;
...

  {
    ...
    if (is_lost_stat_new.version != source_is_lost_stat.version)
      throw Exception(ErrorCodes::REPLICA_STATUS_CHANGED,
                      "Cannot clone {}, because it suddenly become lost "
                      "or removed broken part from ZooKeeper",
                      source_replica);
  }
  ...
  for (const auto &part : local_active_parts) {
    if (get_part_set.getContainingPart(part->name) == part->name) continue;

    if (exact_part_names.contains(part->name)) continue;
  ...
  auto should_ignore_log_entry = [&drop_range_set, &get_part_set, this](std::unordered_set<String> &created_gets, const String &part_name,
                                                                        const String &log_msg_context) -> bool {
    ...
    if (covering_get_part_entry.empty()) return false;
  ...
  auto flush_create_requests = [&]() {
    if (create_requests.empty()) return;

    zookeeper->multi(create_requests);
    for (const auto &part_name : create_request_part_names) created_get_parts.insert(part_name);
  ...
  for (const String &name : active_parts) {
    if (should_ignore_log_entry(created_get_parts, name, "Not fetching")) continue;
    ...
    if (create_requests.size() >= batch_size) flush_create_requests();
  }
  ...
  for (const auto &entry_info : source_queue) {
    ...
    if (entry_info.parsed_entry && !entry_info.parsed_entry->new_part_name.empty()) {
      ...
      if (should_ignore_log_entry(created_get_parts, part_name, fmt::format("Not copying {} {}", entry_name, entry_type))) continue;

      if (entry_info.parsed_entry->type == LogEntry::GET_PART) created_get_parts.insert(part_name);
    }
...
void StorageReplicatedMergeTree::cloneMetadataIfNeeded(const String &source_replica, const String &source_path,
                                                       zkutil::ZooKeeperPtr &zookeeper) {
  ...
  if (metadata_snapshot->getMetadataVersion() == source_metadata_version) return;
  ...
  while (true) {
    if (shutdown_called) throw Exception(ErrorCodes::ABORTED, "Cannot clone metadata because shutdown called");
    ...
    if (code == Coordination::Error::ZOK) break;
  ...
  LOG_INFO(log, "Created an ALTER_METADATA entry {} to force metadata update after cloning replica from {}. Entry: {}", path_created,
           source_replica, dummy_alter.toString());
...
void StorageReplicatedMergeTree::cloneReplicaIfNeeded(zkutil::ZooKeeperPtr zookeeper) {
  ...
  if (zookeeper->tryGet(fs::path(replica_path) / "is_lost", res, &is_lost_stat)) {
    if (res == "0") return;
    if (is_lost_stat.version) is_new_replica = false;
  } else {
  ...
  for (const String &source_replica_name : replicas) {
    ...
    if (source_replica_name == replica_name) continue;
  ...
  for (const String &source_replica_name : replicas) {
    ...
    LOG_INFO(log, "Replica {} has log pointer '{}', approximate {} queue lag and {} queue size", source_replica_name, get_log_pointer.data,
             replica_queue_lag, replica_queue_size);
...
void StorageReplicatedMergeTree::queueUpdatingTask() {
  ...
  try {
    ...
    if (is_readonly) {
      ...
      if (shutdown_prepared_called) return;
...
void StorageReplicatedMergeTree::mutationsUpdatingTask() {
  try {
    ...
    if (e.code == Coordination::Error::ZSESSIONEXPIRED) return;
...
bool StorageReplicatedMergeTree::processQueueEntry(ReplicatedMergeTreeQueue::SelectedEntryPtr selected_entry) {
  ...
  return queue.processEntry([this] { return getZooKeeper(); }, entry,
                            [&](LogEntryPtr &entry_to_process) {
                              try {
                                return executeLogEntry(*entry_to_process);
                              } catch (const Exception &e) {
                                if (e.code() == ErrorCodes::NO_REPLICA_HAS_PART) {
                                  /// If no one has the right part, probably not all replicas work; We will not write to log with Error
                                  /// level.
                                ...
                                /** This exception will be written to the queue element, and it can be looked up using
                                 * `system.replication_queue` table. The thread that performs this action will sleep a few seconds after the
                                 * exception. See `queue.processEntry` function.
                                 */
                                throw;
                              } catch (...) {
                                tryLogCurrentException(log, __PRETTY_FUNCTION__);
...
bool StorageReplicatedMergeTree::scheduleDataProcessingJob(BackgroundJobsAssignee &assignee) {
  ...
  if (queue.actions_blocker.isCancelled()) return false;
  ...
  assignee.scheduleCommonTask(
      std::make_shared<ExecutableLambdaAdapter>([this, selected_entry]() mutable { return processQueueEntry(selected_entry); },
...
bool StorageReplicatedMergeTree::canExecuteFetch(const ReplicatedMergeTreeLogEntry &entry, String &disable_reason) const {
  ...
  if (busy_threads_in_pool >= replicated_fetches_pool_size) {
    disable_reason = fmt::format("Not executing fetch of part {} because {} fetches already executing, max {}.", entry.new_part_name,
                                 busy_threads_in_pool, replicated_fetches_pool_size);
    return false;
  }

  if (replicated_fetches_throttler->isThrottling()) {
    disable_reason = fmt::format(
        "Not executing fetch of part {} because fetches have already throttled by network settings "
        "<max_replicated_fetches_network_bandwidth> or <max_replicated_fetches_network_bandwidth_for_server>.",
        entry.new_part_name);
    return false;
  }

  if (entry.source_replica.empty()) {
    auto part = getPartIfExists(entry.new_part_name,
                                {MergeTreeDataPartState::Active, MergeTreeDataPartState::Outdated, MergeTreeDataPartState::Deleting});
    if (part && part->was_removed_as_broken) {
      disable_reason = fmt::format(
          "Not executing fetch of part {} because we still have broken part with that name. "
          "Waiting for the broken part to be removed first.",
          entry.new_part_name);
...
bool StorageReplicatedMergeTree::partIsAssignedToBackgroundOperation(const DataPartPtr &part) const { return queue.isVirtualPart(part); }
...
void StorageReplicatedMergeTree::mergeSelectingTask() {
  ...
  auto try_assign_merge = [&]() -> AttemptStatus {
    ...
    if (is_readonly) {
      /// Note that we need to check shutdown_prepared_called, not shutdown_called, since the table will be marked as readonly
      /// after calling StorageReplicatedMergeTree::flushAndPrepareForShutdown().
      if (shutdown_prepared_called) return AttemptStatus::CannotSelect;
      throw Exception(ErrorCodes::TABLE_IS_READ_ONLY, "Table is in readonly mode (replica path: {}), cannot assign new merges",
                      replica_path);
    }
    ...
    if (!canEnqueueBackgroundTask()) {
      ...
      LOG_TRACE(log,
                "Reached memory limit for the background tasks ({}), so won't select new parts to merge or mutate."
    ...
    if (merges_and_mutations_sum >= (*storage_settings_ptr)[MergeTreeSetting::max_replicated_merges_in_queue]) {
      LOG_TRACE(log,
                "Number of queued merges ({}) and part mutations ({})"
                " is greater than max_replicated_merges_in_queue ({}), so won't select new parts to merge or mutate.",
                merges_and_mutations_queued.merges, merges_and_mutations_queued.mutations,
    ...
    UInt64 max_source_part_bytes_for_mutation =
        CompactionStatistics::getMaxSourcePartBytesForMutation(*this, &max_source_part_bytes_for_mutation_log_comment);
    ...
    if ((*storage_settings.get())[MergeTreeSetting::assign_part_uuids]) future_merged_part->uuid = UUIDHelpers::generateV4();
    ...
    bool can_assign_merge = max_source_parts_bytes_for_merge > 0 && !merger_mutator.merges_blocker.isCancelled();
    ...
    if (can_assign_merge) {
      ...
      partitions_to_merge_in = merger_mutator.getPartitionsThatMayBeMerged(
          std::make_shared<ReplicatedMergeTreePartsCollector>(*this, local_merge_pred), local_merge_pred,
    ...
    if (can_assign_merge) {
      auto select_merge_result =
          merger_mutator.selectPartsToMerge(std::make_shared<ReplicatedMergeTreePartsCollector>(*this, merge_predicate), merge_predicate,
                                            MergeSelectorApplier(
                                                /*merge_constraints=*/{{max_source_parts_bytes_for_merge, max_result_part_rows}},
                                                /*merge_with_ttl_allowed=*/merge_with_ttl_allowed,
                                                /*aggressive_=*/false,
      ...
      if (select_merge_result.has_value()) {
        ...
        create_result =
            createLogEntryToMergeParts(zookeeper, future_merged_part->parts, future_merged_part->patch_parts, future_merged_part->name,
                                       future_merged_part->uuid, future_merged_part->part_format, deduplicate, deduplicate_by_columns,
                                       cleanup, nullptr, merge_predicate->getVersion(), future_merged_part->merge_type);

        if (create_result == CreateMergeEntryResult::Ok) return AttemptStatus::EntryCreated;
        if (create_result == CreateMergeEntryResult::LogUpdated) return AttemptStatus::NeedRetry;
      } else {
    ...
    if (max_source_part_bytes_for_mutation == 0 ||
        merges_and_mutations_queued.mutations >= (*storage_settings_ptr)[MergeTreeSetting::max_replicated_mutations_in_queue]) {
      if (max_source_part_bytes_for_mutation == 0)
        max_source_part_bytes_for_mutation_log_comment = " (" + max_source_part_bytes_for_mutation_log_comment + ")";

      LOG_TRACE(log,
                "Number of queued mutations ({}) is greater than max_replicated_mutations_in_queue ({})"
                " or there are not enough free threads for mutations{}, so won't select new parts to merge or mutate.",
                merges_and_mutations_queued.mutations, (*storage_settings_ptr)[MergeTreeSetting::max_replicated_mutations_in_queue].value,
    ...
    if (queue.countMutations() > 0) {
      ...
      if (!merge_predicate) merge_predicate = queue.getMergePredicate(zookeeper, PartitionIdsHint{});
      ...
      for (const auto &part : data_parts) {
        ...
        create_result =
            createLogEntryToMutatePart(*part, future_merged_part->uuid, expected->first, expected->second, merge_predicate->getVersion());
  ...
  merge_selecting_sleep_ms =
      std::clamp<UInt64>(static_cast<UInt64>(new_sleep_ms), (*storage_settings_ptr)[MergeTreeSetting::merge_selecting_sleep_ms],
...
void StorageReplicatedMergeTree::mutationsFinalizingTask() {
  ...
  try {
    ...
    if (is_readonly) {
      ...
      /// after calling StorageReplicatedMergeTree::flushAndPrepareForShutdown().
      if (shutdown_prepared_called) return;
      throw Exception(ErrorCodes::TABLE_IS_READ_ONLY, "Table is in readonly mode (replica path: {}), cannot finalize mutations",
                      replica_path);
    }
...
StorageReplicatedMergeTree::CreateMergeEntryResult StorageReplicatedMergeTree::createLogEntryToMergeParts(
    zkutil::ZooKeeperPtr &zookeeper, const DataPartsVector &parts, const DataPartsVector &patch_parts, const String &merged_name,
    const UUID &merged_part_uuid, const MergeTreeDataPartFormat &merged_part_format, bool deduplicate, const Names &deduplicate_by_columns,
    bool cleanup, ReplicatedMergeTreeLogEntryData *out_log_entry, int32_t log_version, MergeType merge_type) {
  Strings exists_paths;
  exists_paths.reserve(parts.size());
  for (const auto &part : parts) exists_paths.emplace_back(fs::path(replica_path) / "parts" / part->name);
  ...
  for (size_t i = 0; i < parts.size(); ++i) {
    ...
    if (exists_results[i].error == Coordination::Error::ZNONODE) {
      ...
      if (part->modification_time + MAX_AGE_OF_LOCAL_PART_THAT_WASNT_ADDED_TO_ZOOKEEPER < time(nullptr)) {
        LOG_WARNING(log,
                    "Part {} (that was selected for merge) with age {} seconds exists locally but not in ZooKeeper. Won't do merge with "
                    "that part and will check it.",
                    part->name, (time(nullptr) - part->modification_time));
        enqueuePartForCheck(part->name);
      }
    }
  }

  if (!all_in_zk) return CreateMergeEntryResult::MissingPart;
  ...
  for (const auto &part : parts) entry.source_parts.push_back(part->name);

  for (const auto &patch : patch_parts) entry.patch_parts.push_back(patch->name);

  Coordination::Requests ops;
  Coordination::Responses responses;

  ops.emplace_back(
      zkutil::makeCreateRequest(fs::path(zookeeper_path) / "log/log-", entry.toString(), zkutil::CreateMode::PersistentSequential));

  ops.emplace_back(zkutil::makeSetRequest(fs::path(zookeeper_path) / "log", "", log_version));  /// Check and update version.
  ...
  if (out_log_entry) *out_log_entry = entry;
...
StorageReplicatedMergeTree::CreateMergeEntryResult StorageReplicatedMergeTree::createLogEntryToMutatePart(
  ...
  if (!zookeeper->exists(fs::path(replica_path) / "parts" / part.name)) {
    if (part.modification_time + MAX_AGE_OF_LOCAL_PART_THAT_WASNT_ADDED_TO_ZOOKEEPER < time(nullptr)) {
      LOG_WARNING(log,
                  "Part {} (that was selected for mutation) with age {} seconds exists locally but not in ZooKeeper."
...
void StorageReplicatedMergeTree::removePartAndEnqueueFetch(const String &part_name, bool storage_init) {
  ...
  auto outdate_broken_part = [this, &broken_part]() {
    if (!broken_part) return;
    auto lock = lockParts();
    if (broken_part->getState() == DataPartState::Active) removePartsFromWorkingSet(NO_TRANSACTION_RAW, {broken_part}, true, lock);
    broken_part.reset();
    cleanup_thread.wakeup();
  };
  ...
  DataPartsVector partition_range;
  {
    ...
    partition_range = getDataPartsVectorInPartitionForInternalUsage({MergeTreeDataPartState::Active, MergeTreeDataPartState::Outdated},
                                                                    broken_part_info.getPartitionId(), parts_lock);
  }

  Strings detached_parts;
  for (const auto &part : partition_range) {
    if (!broken_part_info.contains(part->info)) continue;
  ...
  while (true) {
    if (shutdown_called || partial_shutdown_called) throw Exception(ErrorCodes::ABORTED, "Cannot remove part because shutdown called");
    ...
    if (!storage_init) {
      /// We use merge predicate + version check here, because DROP RANGE update log version and we are trying to avoid race with it. We
      /// must be sure, that our part was not dropped, otherwise we will have fetch entry, but no virtual part for it (DROP RANGE will
      /// remove it). So bad sequence is the following: 1) Create DROP PART in log for broken_part (for example because it's not only
      /// broken, but also empty) 1.5) Pull to log and start execution (removePartProducingOpsInRange) 2) Execute this method (create GET
      /// PART for broken part) 3) Finish DROP PART execution (remove part from virtual parts) 4) Now we get GET PART for broken part which
      /// is absent in virtual parts == bug
      ///
    ...
    ops.emplace_back(zkutil::makeCreateRequest(fs::path(replica_path) / "queue/queue-", log_entry->toString(),
...
void StorageReplicatedMergeTree::addLastSentPart(const MergeTreePartInfo &info) {
  {
    ...
    while (last_sent_parts.size() > LAST_SENT_PARS_WINDOW_SIZE) last_sent_parts.pop_front();
  }

  last_sent_parts_cv.notify_all();
}

void StorageReplicatedMergeTree::waitForUniquePartsToBeFetchedByOtherReplicas(
    StorageReplicatedMergeTree::ShutdownDeadline shutdown_deadline_) {
  ...
  if (!last_sent_parts_cv.wait_until(lock, shutdown_deadline_, wait_predicate))
    LOG_INFO(log, "Failed to wait for unique parts to be fetched in {} ms, {} parts can be left on this replica", wait_ms,
             unique_parts_set.size());
  else
    LOG_INFO(log, "Successfully waited all the parts");
}

std::set<MergeTreePartInfo> StorageReplicatedMergeTree::findReplicaUniqueParts(const String &replica_name_, const String &zookeeper_path_,
                                                                               MergeTreeDataFormatVersion format_version_,
                                                                               zkutil::ZooKeeper::Ptr zookeeper_, LoggerPtr log_) {
  ...
  if (!our_parts.empty() && our_unique_parts.empty()) LOG_TRACE(log_, "All parts found on replicas");
...
String StorageReplicatedMergeTree::findReplicaHavingCoveringPartImplLowLevel(LogEntry *entry, const String &part_name,
                                                                             String &found_part_name, bool active) {
  auto zookeeper = getZooKeeper();
  Strings replicas = zookeeper->getChildren(fs::path(zookeeper_path) / "replicas");
  ...
  for (const String &replica : replicas) {
    ...
    if (active && !zookeeper->exists(fs::path(zookeeper_path) / "replicas" / replica / "is_active")) continue;
...
bool StorageReplicatedMergeTree::findReplicaHavingCoveringPart(const String &part_name, bool active) {
  String dummy;
...
void StorageReplicatedMergeTree::updateQuorum(const String &part_name, bool is_parallel) {
  if (is_parallel && format_version < MERGE_TREE_DATA_MIN_FORMAT_VERSION_WITH_CUSTOM_PARTITIONING)
    throw Exception(ErrorCodes::NOT_IMPLEMENTED,
                    "Parallel quorum inserts are not compatible with the deprecated syntax of *MergeTree engines");

  auto zookeeper = getZooKeeper();

  /// Information on which replicas a part has been added, if the quorum has not yet been reached.
  String quorum_status_path = fs::path(zookeeper_path) / "quorum" / "status";
  if (is_parallel) quorum_status_path = fs::path(zookeeper_path) / "quorum" / "parallel" / part_name;
  ...
  while (zookeeper->tryGet(quorum_status_path, value, &stat)) {
    ...
    if (quorum_entry.part_name != part_name) {
      LOG_TRACE(log, "Quorum {}, already achieved for part {} current part {}", quorum_status_path, part_name, quorum_entry.part_name);
    ...
    if (quorum_entry.replicas.size() >= quorum_entry.required_number_of_replicas) {
      ...
      LOG_TRACE(log, "Got {} (of {} required) replicas confirmed quorum {}, going to remove node", quorum_entry.replicas.size(),
                quorum_entry.required_number_of_replicas, quorum_status_path);
      ...
      if (!is_parallel) {
        ...
        if (!old_added_parts.empty()) parts_with_quorum.fromString(old_added_parts);
    ...
    LOG_TRACE(log, "Quorum {} still not satisfied (have only {} of {} replicas), updating node", quorum_status_path,
              quorum_entry.replicas.size(), quorum_entry.required_number_of_replicas);
...
bool StorageReplicatedMergeTree::partIsLastQuorumPart(const MergeTreePartInfo &part_info) const {
  ...
  if (parts_with_quorum_str.empty()) return false;
  ...
  if (partition_it == parts_with_quorum.added_parts.end()) return false;
...
bool StorageReplicatedMergeTree::fetchPart(const String &part_name, const StorageMetadataPtr &metadata_snapshot,
                                           const String &source_zookeeper_name, const String &source_replica_path, bool to_detached,
                                           size_t quorum, zkutil::ZooKeeper::Ptr zookeeper_, bool try_fetch_shared) {
  ...
  if (!to_detached)
    table_lock_holder =
        lockForShare(RWLockImpl::NO_QUERY, (*settings_ptr)[MergeTreeSetting::lock_acquire_timeout_for_background_operations]);
  ...
  DataPartPtr part_to_clone;
  {
    ...
    if (source_part && !is_zero_copy_part(source_part)) {
      auto source_part_header = ReplicatedMergeTreePartHeader::fromColumnsAndChecksums(source_part->getColumns(), source_part->checksums);
      ...
      if (!part_znode.empty()) {
        ...
        if (zookeeper->tryGet(fs::path(part_path) / "columns", columns_str) &&
          ...
          LOG_INFO(log, "Not checking checksums of part {} with replica {}:{} because part was removed from ZooKeeper", part_name,
                   source_zookeeper_name, source_replica_path);
        }
      }
      ...
      /// be thrown.
      if (desired_part_header && source_part_header.getColumnsHash() == desired_part_header->getColumnsHash() &&
  ...
  if (part_to_clone) {
    get_part = [&, part_to_clone]() {
      ...
      auto [cloned_part, lock] =
          cloneAndLoadDataPart(part_to_clone, "tmp_clone_", part_info, metadata_snapshot, clone_params, getContext()->getReadSettings(),
                               getContext()->getWriteSettings(), true /*must_on_same_disk*/);
    ...
    get_part = [&, address, timeouts, credentials, interserver_scheme]() {
      ...
      auto [fetched_part, lock] =
          fetcher.fetchSelectedPart(metadata_snapshot, getContext(), part_name, source_zookeeper_name, source_replica_path, address.host,
                                    address.replication_port, timeouts, credentials->getUser(), credentials->getPassword(),
                                    interserver_scheme, replicated_fetches_throttler, to_detached, "", &tagger_ptr, try_fetch_shared);
      part_directory_lock = std::move(lock);
      return fetched_part;
    };
  }
  ...
  try {
    ...
    if (!to_detached) {
       ...
       * If you do not have time, in case of losing the session, when you restart the server - see the
       * `ReplicatedMergeTreeRestartingThread::updateQuorumIfWeHavePart` method.
       */
      if (quorum) {
      ...
      if (part_info.level != 0 || part_info.mutation != 0) {
        ...
        for (const String &quorum_part : quorum_parts) {
          ...
          if (part_info.contains(quorum_part_info)) updateQuorum(quorum_part, true);
        }
      }
      ...
      if (prewarm_caches.index_mark_cache) part->loadIndexMarksToCache(prewarm_caches.index_mark_cache.get());

      if (prewarm_caches.primary_index_cache) part->loadIndexToCache(*prewarm_caches.primary_index_cache);
    ...
    if (!to_detached) write_part_log(ExecutionStatus::fromCurrentException("", true));
  ...
  if (part_to_clone)
    ...
    LOG_DEBUG(log, "Fetched part {} from {}:{}{}", part_name, source_zookeeper_name, source_replica_path,
              to_detached ? " (to 'detached' directory)" : "");

  return true;
}

MergeTreeData::MutableDataPartPtr StorageReplicatedMergeTree::fetchExistsPart(const String &part_name,
                                                                              const StorageMetadataPtr &metadata_snapshot,
                                                                              const String &source_replica_path, DiskPtr replaced_disk,
  ...
  get_part = [&, address, timeouts, interserver_scheme, credentials]() {
    ...
    auto [fetched_part, lock] =
        fetcher.fetchSelectedPart(metadata_snapshot, getContext(), part_name, zookeeper_info.zookeeper_name, source_replica_path,
                                  address.host, address.replication_port, timeouts, credentials->getUser(), credentials->getPassword(),
                                  interserver_scheme, replicated_fetches_throttler, false, "", nullptr, true, replaced_disk);
    part_temp_directory_lock = std::move(lock);
    return fetched_part;
  };
...
void StorageReplicatedMergeTree::startupImpl(bool from_attach_thread, const ZooKeeperRetriesInfo &zookeeper_retries_info) {
  ...
  if (!has_metadata_in_zookeeper.has_value() || !*has_metadata_in_zookeeper) {
    if (!std::exchange(is_readonly_metric_set, true)) CurrentMetrics::add(CurrentMetrics::ReadonlyReplica);
  ...
  try {
    ...
    ///
    getContext()->getInterserverIOHandler().addEndpoint(data_parts_exchange_ptr->getId(getEndpointName()), data_parts_exchange_ptr);
    ...
    if (from_attach_thread) {
      ...
      while (!startup_event.tryWait(10 * 1000)) LOG_TRACE(log, "Waiting for RestartingThread to startup table");
    }
...
void StorageReplicatedMergeTree::flushAndPrepareForShutdown() {
  ...
  if (shutdown_prepared_called.exchange(true)) return;
  ...
  try {
    ...
    startup_event.set();
    shutdown_deadline.emplace(
        std::chrono::system_clock::now() +
        std::chrono::milliseconds((*settings_ptr)[MergeTreeSetting::wait_for_unique_parts_send_before_shutdown_ms].totalMilliseconds()));
  } catch (...) {
...
void StorageReplicatedMergeTree::shutdown(bool) {
  if (shutdown_called.exchange(true)) return;

  auto component_guard = Coordination::setCurrentComponent("StorageReplicatedMergeTree::shutdown");

  LOG_TRACE(log, "Shutdown started");

  if (refresh_parts_task) refresh_parts_task->deactivate();
  if (refresh_stats_task) refresh_stats_task->deactivate();

  flushAndPrepareForShutdown();

  if (!shutdown_deadline.has_value()) throw Exception(ErrorCodes::LOGICAL_ERROR, "Shutdown deadline is not set in shutdown");

  try {
    waitForUniquePartsToBeFetchedByOtherReplicas(*shutdown_deadline);
  } catch (const Exception &ex) {
    if (ex.code() == ErrorCodes::LOGICAL_ERROR) throw;
...
PartitionIdToMaxBlock StorageReplicatedMergeTree::getMaxAddedBlocks() const {
  ...
  if (zookeeper->tryGet(fs::path(zookeeper_path) / "quorum" / "last_part", added_parts_str)) {
    if (!added_parts_str.empty()) {
      ...
      for (const auto &max_block : part_with_quorum.getMaxInsertedBlocks()) max_added_blocks[max_block.first] = max_block.second;
    }
  }
  return max_added_blocks;
}

void StorageReplicatedMergeTree::read(QueryPlan &query_plan, const Names &column_names, const StorageSnapshotPtr &storage_snapshot,
                                      SelectQueryInfo &query_info, ContextPtr local_context, QueryProcessingStage::Enum processed_stage,
                                      const size_t max_block_size, const size_t num_streams) {
  ...
  if (local_context->canUseParallelReplicasCustomKey() && !settings[Setting::allow_experimental_analyzer] &&
    ...
    if (local_context->canUseParallelReplicasCustomKeyForCluster(*cluster)) {
      ...
      ClusterProxy::executeQueryWithParallelReplicasCustomKey(query_plan, getStorageID(), std::move(modified_query_info),
                                                              getInMemoryMetadataPtr()->getColumns(), storage_snapshot, processed_stage,
                                                              query_info.query, local_context);
      return;
    }
    LOG_WARNING(log,
...
void StorageReplicatedMergeTree::readLocalSequentialConsistencyImpl(QueryPlan &query_plan, const Names &column_names,
                                                                    const StorageSnapshotPtr &storage_snapshot, SelectQueryInfo &query_info,
                                                                    ContextPtr local_context, size_t max_block_size, size_t num_streams) {
  ...
  if (auto additional_blocks = local_context->getPartitionIdToMaxBlock(getStorageID().uuid)) {
    for (const auto &[partition_id, max_block] : *additional_blocks) {
      ...
      if (!inserted) it->second = std::min(it->second, max_block);
    }
  }

  auto plan = MergeTreeDataSelectExecutor(*this).read(column_names, storage_snapshot, query_info, local_context, max_block_size,
                                                      num_streams, std::move(max_added_blocks),
                                                      /*enable_parallel_reading=*/false);

  if (plan) query_plan = std::move(*plan);
}

void StorageReplicatedMergeTree::readParallelReplicasImpl(QueryPlan &query_plan, const Names & /*column_names*/,
                                                          SelectQueryInfo &query_info, ContextPtr local_context,
                                                          QueryProcessingStage::Enum processed_stage) {
  ClusterProxy::executeQueryWithParallelReplicas(query_plan, getStorageID(), processed_stage, query_info.query, local_context,
                                                 query_info.storage_limits);
}

void StorageReplicatedMergeTree::readLocalImpl(QueryPlan &query_plan, const Names &column_names, const StorageSnapshotPtr &storage_snapshot,
                                               SelectQueryInfo &query_info, ContextPtr local_context, const size_t max_block_size,
                                               const size_t num_streams) {
  const bool enable_parallel_reading = local_context->canUseParallelReplicasOnFollower();
  auto plan =
      MergeTreeDataSelectExecutor(*this).read(column_names, storage_snapshot, query_info, local_context, max_block_size, num_streams,
                                              local_context->getPartitionIdToMaxBlock(getStorageID().uuid), enable_parallel_reading);
...
void StorageReplicatedMergeTree::foreachActiveParts(Func &&func, bool select_sequential_consistency) const {
   ...
   */
  if (select_sequential_consistency) max_added_blocks = getMaxAddedBlocks();
  ...
  for (const auto &part : getDataPartsStateRange(DataPartState::Active, MergeTreePartInfo::Kind::Regular)) {
    ...
    if (max_added_blocks) {
      ...
      if (blocks_iterator == max_added_blocks->end() || part->info.max_block > blocks_iterator->second) continue;
    }
...
std::optional<UInt64> StorageReplicatedMergeTree::totalRowsByPartitionPredicate(const ActionsDAG &filter_actions_dag,
...
SinkToStoragePtr StorageReplicatedMergeTree::write(const ASTPtr & /*query*/, const StorageMetadataPtr &metadata_snapshot,
                                                   ContextPtr local_context, bool async_insert) {
  ...
  if (!initialization_done) throw Exception(ErrorCodes::NOT_INITIALIZED, "Table is not initialized yet");
  ...
  if (is_readonly && (!has_metadata_in_zookeeper.has_value() || false == has_metadata_in_zookeeper.value()))
    throw Exception(ErrorCodes::TABLE_IS_READ_ONLY,
                    "Table is in readonly mode since table metadata was not found in zookeeper: replica_path={}", replica_path);

  const auto storage_settings_ptr = getSettings();
  const Settings &query_settings = local_context->getSettingsRef();

  return std::make_shared<ReplicatedMergeTreeSink>(
      /* async_insert */ async_insert, *this, metadata_snapshot, query_settings[Setting::insert_quorum].valueOr(0),
      query_settings[Setting::insert_quorum_timeout].totalMilliseconds(), query_settings[Setting::max_partitions_per_insert_block],
      query_settings[Setting::insert_quorum_parallel], query_settings[Setting::insert_quorum].is_auto, local_context);
}

bool StorageReplicatedMergeTree::optimize(const ASTPtr &, const StorageMetadataPtr &, const ASTPtr &partition, bool final, bool deduplicate,
                                          const Names &deduplicate_by_columns, bool cleanup, ContextPtr query_context) {
  ...
  if (!is_leader) throw Exception(ErrorCodes::NOT_A_LEADER, "OPTIMIZE cannot be done on this replica because it is not a leader");
    ...
    for (; try_no < max_retries; ++try_no) {
      ...
      const auto select_merge = [&]() -> std::expected<MergeSelectorChoices, SelectMergeFailure> {
        if (partition_id.empty()) {
          ...
          return merger_mutator.selectPartsToMerge(parts_collector, merge_predicate,
                                                   MergeSelectorApplier(
                                                       /*merge_constraints=*/{{max_source_parts_bytes_for_merge, max_result_part_rows}},
          ...
          return merger_mutator.selectAllPartsToMergeWithinPartition(
              metadata_snapshot, parts_collector, merge_predicate, partition_id, final,
              query_context->getSettingsRef()[Setting::optimize_skip_merged_partitions]);
        }
      };

      const auto construct_future_part =
          [&](MergeSelectorChoices choices) -> std::expected<FutureMergedMutatedPartPtr, SelectMergeFailure> {
        ...
        if ((*storage_settings.get())[MergeTreeSetting::assign_part_uuids]) future_part->uuid = UUIDHelpers::generateV4();
      ...
      if (!select_merge_result.has_value()) {
        ...
        if (error.reason == SelectMergeFailure::Reason::NOTHING_TO_MERGE) return false;

        if (try_no + 1 < max_retries) {
          /// Here we trying to have a similar behaviour to ordinary MergeTree: if some merges are already in progress - let's wait for them
          /// to finish. This way `optimize final` won't just silently be a noop (if also `optimize_throw_if_noop=false`), but will wait for
          /// the active merges and repeat an attempt to schedule final merge. This guarantees are enough for tests, because there we have
          /// full control over insertions.
        ...
        if (!partition_id.empty()) error.explanation.text += fmt::format(" (in partition {})", partition_id);
      ...
      CreateMergeEntryResult create_result = createLogEntryToMergeParts(
          zookeeper, select_merge_result.value()->parts, select_merge_result.value()->patch_parts, select_merge_result.value()->name,
          select_merge_result.value()->uuid, select_merge_result.value()->part_format, deduplicate, deduplicate_by_columns, cleanup,
          &merge_entry, merge_predicate->getVersion(), select_merge_result.value()->merge_type);
      ...
      if (create_result == CreateMergeEntryResult::LogUpdated) continue;
  ...
  if (!partition && final) {
    ...
    for (const DataPartPtr &part : data_parts) partition_ids.emplace(part->info.getPartitionId());
    ...
    String partition_id;
    if (partition) partition_id = getPartitionIDFromQuery(partition, query_context);
    assigned = try_assign_merge(partition_id);
  }
  ...
  for (auto &merge_entry : merge_entries) waitForLogEntryToBeProcessedIfNecessary(merge_entry, query_context, watch_events);
...
bool StorageReplicatedMergeTree::executeMetadataAlter(const StorageReplicatedMergeTree::LogEntry &entry) {
  ...
  if (entry.alter_version < current_metadata->getMetadataVersion()) {
    ...
    LOG_WARNING(log,
                "Attempt to update metadata of version {} "
  ...
  if (is_in_replicated_database) {
    ...
    for (auto &op : requests) alter_txn->addOp(std::move(op));
    requests.clear();
...

  {
    ...
    auto metadata_diff = table_metadata.checkAndFindDiff(metadata_from_entry, getInMemoryMetadataPtr()->getColumns(),
...
PartitionBlockNumbersHolder StorageReplicatedMergeTree::allocateBlockNumbersInAffectedPartitions(
    const MutationCommands &commands, CommittingBlock::Op op, ContextPtr query_context, const zkutil::ZooKeeperPtr &zookeeper) const {
  ...
  if (mutation_affected_partition_ids.size() == 1) {
    ...
    if (!block_number_holder.isLocked()) return {};
  ...
  EphemeralLocksInAllPartitions lock_holder(fs::path(zookeeper_path) / "block_numbers", "block-", fs::path(zookeeper_path) / "temp",
                                            block_data, *zookeeper);

  PartitionBlockNumbersHolder::BlockNumbersType block_numbers;
  for (const auto &lock : lock_holder.getLocks()) {
    if (lock.partition_id.starts_with(MergeTreePartInfo::PATCH_PART_PREFIX)) continue;
...
void StorageReplicatedMergeTree::alter(const AlterCommands &commands, ContextPtr query_context, AlterLockHolder &table_lock_holder) {
  ...
  if (commands.isSettingsAlter()) {
    ...
    if (statistics_changed) setInMemoryMetadata(future_metadata);

    /// It is safe to ignore exceptions here as only settings are changed, which is not validated in `alterTable`
    DatabaseCatalog::instance()
        .getDatabase(table_id.database_name)
        ->alterTable(query_context, table_id, future_metadata, /*validate_new_create_query=*/true);
    return;
  }
  ...
  while (true) {
    if (shutdown_called || partial_shutdown_called) throw Exception(ErrorCodes::ABORTED, "Cannot assign alter because shutdown called");
    ...
    if (ast_to_str(future_metadata.sorting_key.definition_ast) != ast_to_str(current_metadata->sorting_key.definition_ast)) {
      ...
      /// in KeyDescription, but it contain version column for VersionedCollapsingMergeTree, which shouldn't be defined as a part of key
      /// definition AST. So the best compatible way is just to convert definition_ast to list and serialize it. In all other places
      /// key.expression_list_ast should be used.
    ...
    if (ast_to_str(future_metadata.sampling_key.definition_ast) != ast_to_str(current_metadata->sampling_key.definition_ast))
      future_metadata_in_zk.sampling_expression =
          extractKeyExpressionList(future_metadata.sampling_key.definition_ast)->formatWithSecretsOneLine();
    ...
    if (ast_to_str(future_metadata.partition_key.definition_ast) != ast_to_str(current_metadata->partition_key.definition_ast))
      future_metadata_in_zk.partition_key =
          extractKeyExpressionList(future_metadata.partition_key.definition_ast)->formatWithSecretsOneLine();
    ...
    if (new_indices_str != current_metadata->secondary_indices.explicitToString()) future_metadata_in_zk.skip_indices = new_indices_str;

    String new_projections_str = future_metadata.projections.toString();
    if (new_projections_str != current_metadata->projections.toString()) future_metadata_in_zk.projections = new_projections_str;

    String new_constraints_str = future_metadata.constraints.toString();
    if (new_constraints_str != current_metadata->constraints.toString()) future_metadata_in_zk.constraints = new_constraints_str;
    ...
    ops.emplace_back(
        zkutil::makeSetRequest(fs::path(zookeeper_path) / "metadata", new_metadata_str, current_metadata->getMetadataVersion()));
    ...
    if (settings_are_changed || comment_is_changed) {
      ...
      DatabaseCatalog::instance()
          .getDatabase(table_id.database_name)
          ->alterTable(query_context, table_id, metadata_copy, /*validate_new_create_query=*/true);
    }
    ...
    ops.emplace_back(zkutil::makeCreateRequest(fs::path(zookeeper_path) / "log/log-", alter_entry->toString(),
                                               zkutil::CreateMode::PersistentSequential));
    ...
    if (rc == Coordination::Error::ZOK) {
      if (have_mutation) {
        ...
        String alter_path = dynamic_cast<const Coordination::CreateResponse &>(*results[alter_path_idx]).path_created;
        alter_entry->znode_name = alter_path.substr(alter_path.find_last_of('/') + 1);
        LOG_DEBUG(log, "Created log entry {} to update table metadata to version {}", alter_entry->znode_name, alter_entry->alter_version);
      }
      break;
    }
    if (rc == Coordination::Error::ZBADVERSION) {
...
void StorageReplicatedMergeTree::restoreMetadataInZooKeeper(const ZooKeeperRetriesInfo &zookeeper_retries_info,
  ...
  if (!is_called_during_attach && !initialization_done) throw Exception(ErrorCodes::NOT_INITIALIZED, "Table is not initialized yet");

  if (!is_readonly) throw Exception(ErrorCodes::BAD_ARGUMENTS, "Replica must be readonly");
  ...
  if (getZooKeeper()->exists(replica_path))
    throw Exception(ErrorCodes::BAD_ARGUMENTS,
                    ...
                    "then use SYSTEM DROP REPLICA query first.",
                    replica_path);

  if (has_metadata_in_zookeeper.has_value() && *has_metadata_in_zookeeper)
    throw Exception(ErrorCodes::LOGICAL_ERROR,
                    "Replica has metadata in ZooKeeper: "
                    "it's either a bug or it's a result of manual intervention to ZooKeeper");

  if (are_restoring_replica.exchange(true)) throw Exception(ErrorCodes::CONCURRENT_ACCESS_NOT_SUPPORTED, "Replica restoration in progress");
  ...
  if (!is_first_replica) createReplica(metadata_snapshot, zookeeper_retries_info);
  ...
  if (max_parts_metadata_version > 0) {
    auto zookeeper = getZooKeeper();
    String metadata_str = zookeeper->get(zookeeper_path + "/metadata");
    for (int32_t i = 0; i < max_parts_metadata_version; ++i) zookeeper->set(zookeeper_path + "/metadata", metadata_str);
...
void StorageReplicatedMergeTree::dropPartNoWaitNoThrow(const String &part_name) {
  assertNotReadonly();
  if (!is_leader) throw Exception(ErrorCodes::NOT_A_LEADER, "DROP PART cannot be done on this replica because it is not a leader");
...
void StorageReplicatedMergeTree::dropPartition(const ASTPtr &partition, bool detach, ContextPtr query_context) {
  ...
  assertNotReadonly();
  if (!is_leader) throw Exception(ErrorCodes::NOT_A_LEADER, "DROP PARTITION cannot be done on this replica because it is not a leader");
  ...
  if (detach && (*settings)[MergeTreeSetting::disable_detach_partition_for_zero_copy_replication] &&
    ...
    for (const auto &disk : getDisks()) {
      if (disk->supportZeroCopyReplication()) throw Exception(ErrorCodes::SUPPORT_IS_DISABLED, "DETACH PARTITION queries are disabled.");
    }
  }
...
void StorageReplicatedMergeTree::dropPartitions(const zkutil::ZooKeeperPtr &zookeeper, const Strings &partition_ids, bool detach,
...
void StorageReplicatedMergeTree::truncate(const ASTPtr &, const StorageMetadataPtr &, ContextPtr query_context,
                                          TableExclusiveLockHolder &table_lock) {
  table_lock.release();  /// Truncate is done asynchronously.

  assertNotReadonly();
  if (!is_leader) throw Exception(ErrorCodes::NOT_A_LEADER, "TRUNCATE cannot be done on this replica because it is not a leader");
...
PartitionCommandsResultInfo StorageReplicatedMergeTree::attachPartition(const PartitionCommand &command,
                                                                        const StorageMetadataPtr &metadata_snapshot,
                                                                        ContextPtr query_context) {
  /// Allow ATTACH PARTITION on readonly replica when restoring it.
  if (!are_restoring_replica) assertNotReadonly();
  ...
  ReplicatedMergeTreeSink output(
      /* async_insert */ false, *this, metadata_snapshot,
      /* quorum */ 0,
      /* quorum_timeout_ms */ 0,
      /* max_parts_per_block */ 0,
      /* quorum_parallel */ false,
      /* majority_quorum */ false, query_context,
      /* is_attach */ true,
...
void StorageReplicatedMergeTree::checkTableCanBeRenamed(const StorageID &new_name) const {
  if (zookeeper_info.renaming_restrictions == RenamingRestrictions::ALLOW_ANY) return;
  ...
  if (zookeeper_info.renaming_restrictions == RenamingRestrictions::DO_NOT_ALLOW) {
    ...
    if (likely_converting_ordinary_to_atomic) {
      LOG_INFO(log,
               "Table {} should not be renamed, because zookeeper_path contains implicit 'database' or 'table' macro. "
...
bool StorageReplicatedMergeTree::existsNodeCached(const ZooKeeperWithFaultInjectionPtr &zookeeper, const std::string &path) const {
  {
    ...
    if (existing_nodes_cache.contains(path)) return true;
  }
...
EphemeralLockInZooKeeper StorageReplicatedMergeTree::allocateBlockNumber(const String &partition_id, const zkutil::ZooKeeperPtr &zookeeper,
  ...
  return allocateBlockNumber(partition_id, std::make_shared<ZooKeeperWithFaultInjection>(zookeeper), zookeeper_block_id_paths,
                             zookeeper_path_prefix, znode_data);
}

EphemeralLockInZooKeeper StorageReplicatedMergeTree::allocateBlockNumber(const String &partition_id,
  ...
  auto lock = createEphemeralLockInZooKeeper(fs::path(partition_path) / "block-", fs::path(zookeeper_table_path) / "temp", zookeeper,
                                             zookeeper_block_id_paths, znode_data);

  if (lock.isLocked()) LOG_TRACE(log, "Allocated block number {} in partition {}", lock.getNumber(), partition_id);

  return lock;
}

Strings StorageReplicatedMergeTree::tryWaitForAllReplicasToProcessLogEntry(const String &table_zookeeper_path,
...
void StorageReplicatedMergeTree::waitForAllReplicasToProcessLogEntry(const String &table_zookeeper_path,
                                                                     const ReplicatedMergeTreeLogEntryData &entry,
                                                                     Int64 wait_for_inactive_timeout, WatchEventByPath &watch_events,
                                                                     const String &error_context) {
  Strings unfinished_replicas =
      tryWaitForAllReplicasToProcessLogEntry(table_zookeeper_path, entry, wait_for_inactive_timeout, watch_events);
  if (unfinished_replicas.empty()) return;

  throw Exception(ErrorCodes::UNFINISHED,
                  "{}Timeout exceeded while waiting for replicas {} to process entry {}. "
                  "Probably some replicas are inactive",
                  error_context, fmt::join(unfinished_replicas, ", "), entry.znode_name);
}

void StorageReplicatedMergeTree::waitForLogEntryToBeProcessedIfNecessary(const ReplicatedMergeTreeLogEntryData &entry,
                                                                         ContextPtr query_context, WatchEventByPath &watch_events,
                                                                         const String &error_context) {
  ...
  if (query_context->getSettingsRef()[Setting::alter_sync] == 1) {
    ...
    if (!finished) {
      throw Exception(ErrorCodes::UNFINISHED,
                      "{}Log entry {} is not precessed on local replica, "
                      "most likely because the replica was shut down.",
                      error_context, entry.znode_name);
    }
...
bool StorageReplicatedMergeTree::tryWaitForReplicaToProcessLogEntry(const String &table_zookeeper_path, const String &replica,
                                                                    const ReplicatedMergeTreeLogEntryData &entry,
                                                                    Int64 wait_for_inactive_timeout, Coordination::EventPtr &watch_event) {
  ...
  if (startsWith(entry.znode_name, "log-")) {
    ...
    if (!pulled_to_queue) return false;
    ...
    for (const String &log_entry_name : log_entries) {
      ...
      if (!log_pointer.empty() && log_index < parse<UInt64>(log_pointer)) continue;
    ...
    if (found) {
      ...
      do {
        ...
        /// It can lead to query hung because table drop query can wait for some query (alter, optimize, etc) which called this method,
        /// but the query will never finish because the drop already shut down the table.
        if (!stop_waiting()) event->tryWait(event_wait_timeout_ms);
      } while (!stop_waiting());
...
void StorageReplicatedMergeTree::getStatus(ReplicatedStatus &res, bool with_zk_fields) {
  ...
  if (with_zk_fields && !res.is_session_expired) {
    try {
      ...
      for (const String &replica : all_replicas) paths.push_back(fs::path(zookeeper_path) / "replicas" / replica / "is_active");

      auto get_result = zookeeper->tryGet(paths);
      const auto &log_pointer_str = get_result[0].data;

      if (get_result[0].error == Coordination::Error::ZNONODE) throw zkutil::KeeperException(get_result[0].error);
...
void StorageReplicatedMergeTree::getReplicaDelays(time_t &out_absolute_delay, time_t &out_relative_delay) {
  ...
  if (out_absolute_delay < static_cast<time_t>((*storage_settings_ptr)[MergeTreeSetting::min_relative_delay_to_measure])) return;
  ...
  for (const auto &replica : replicas) {
    ...
    if (min_unprocessed_insert_time.error != Coordination::Error::ZOK) continue;
  ...
  if (have_replica_with_nothing_unprocessed)
    ...
    if (out_absolute_delay > min_replicas_delay) out_relative_delay = out_absolute_delay - min_replicas_delay;
  }
}

void StorageReplicatedMergeTree::fetchPartition(const ASTPtr &partition, const StorageMetadataPtr &metadata_snapshot, const String &from_,
                                                bool fetch_part, ContextPtr query_context) {
  ...
  if (from.empty()) throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT, "ZooKeeper path should not be empty");

  if ((*settings)[MergeTreeSetting::disable_fetch_partition_for_zero_copy_replication] &&
      (*settings)[MergeTreeSetting::allow_remote_fs_zero_copy_replication]) {
    for (const auto &disk : getDisks()) {
      if (disk->supportZeroCopyReplication()) throw Exception(ErrorCodes::SUPPORT_IS_DISABLED, "FETCH PARTITION queries are disabled.");
    }
  }

  zkutil::ZooKeeperPtr zookeeper = getContext()->getDefaultOrAuxiliaryZooKeeper(from_zookeeper_name);

  if (from.back() == '/') from.resize(from.size() - 1);

  if (fetch_part) {
    String part_name = partition->as<ASTLiteral &>().value.safeGet<String>();
    auto part_path = findReplicaHavingPart(part_name, from, zookeeper);

    if (part_path.empty()) throw Exception(ErrorCodes::NO_REPLICA_HAS_PART, "Part {} does not exist on any replica", part_name);
...

  {
    ...
    for (const String &replica : replicas)
      if (zookeeper->exists(fs::path(from) / "replicas" / replica / "is_active")) active_replicas.push_back(replica);

    if (active_replicas.empty()) throw Exception(ErrorCodes::NO_ACTIVE_REPLICAS, "No active replicas for shard {}", from_);
    ...
    for (const String &replica : active_replicas) {
      ...
      if (log_pointer > max_log_pointer || (log_pointer == max_log_pointer && queue_size < min_queue_size)) {
  ...
  if (best_replica.empty()) throw Exception(ErrorCodes::LOGICAL_ERROR, "Cannot choose best replica.");

  LOG_INFO(log, "Found {} replicas, {} of them are active. Selected {} to fetch from.", replicas.size(), active_replicas.size(),
           best_replica);
  ...
  do {
    if (try_no) LOG_INFO(log, "Some of parts ({}) are missing. Will try to fetch covering parts.", missing_parts.size());

    if (try_no >= query_context->getSettingsRef()[Setting::max_fetch_partition_retries_count])
      throw Exception(ErrorCodes::TOO_MANY_RETRIES_TO_FETCH_PARTS, "Too many retries to fetch parts from {}:{}", from_zookeeper_name,
                      best_replica_path);
    ...
    if (missing_parts.empty()) {
      ...
      if (parts_to_fetch.empty())
        throw Exception(ErrorCodes::PARTITION_DOESNT_EXIST, "Partition {} on {}:{} doesn't exist", partition_id, from_zookeeper_name,
                        best_replica_path);
    } else {
    ...
    for (const String &part : parts_to_fetch) {
      ...
      fetch_partition_runner.enqueueAndKeepTrack(
          [this, &part, &metadata_snapshot, &from_zookeeper_name, &best_replica_path, &zookeeper, &missing_parts_mutex, &missing_parts]() {
            bool fetched = false;

            try {
              fetched = fetchPart(part, metadata_snapshot, from_zookeeper_name, best_replica_path,
                                  /*to_detached=*/true,
                                  /*quorum=*/0, zookeeper,
                                  /*try_fetch_shared=*/false);
            } catch (const DB::Exception &e) {
              if (e.code() != ErrorCodes::RECEIVED_ERROR_FROM_REMOTE_IO_SERVER &&
                  e.code() != ErrorCodes::RECEIVED_ERROR_TOO_MANY_REQUESTS && e.code() != ErrorCodes::CANNOT_READ_ALL_DATA)
                throw;
...
void StorageReplicatedMergeTree::mutate(const MutationCommands &commands, ContextPtr query_context) {
  ...
  while (true) {
    if (shutdown_called || partial_shutdown_called) throw Exception(ErrorCodes::ABORTED, "Cannot assign mutation because shutdown called");
    ...
    if (auto txn = query_context->getZooKeeperMetadataTransaction()) txn->moveOpsTo(requests);
    ...
    if (rc == Coordination::Error::ZOK) {
      const String &path_created = dynamic_cast<const Coordination::CreateResponse *>(responses[1].get())->path_created;
      mutation_entry.znode_name = path_created.substr(path_created.find_last_of('/') + 1);
      LOG_TRACE(log, "Created mutation with ID {} (data versions: {})", mutation_entry.znode_name, mutation_entry.getBlockNumbersForLogs());
      break;
    }
    if (rc == Coordination::Error::ZBADVERSION) {
      /// Cannot retry automatically, because some zookeeper ops were lost on the first attempt. Will retry on DDLWorker-level.
...
std::vector<MergeTreeMutationStatus> StorageReplicatedMergeTree::getMutationsStatus() const { return queue.getMutationsStatus(); }
...
CancellationCode StorageReplicatedMergeTree::killMutation(const String &mutation_id) {
  ...
  if (!mutation_entry) return CancellationCode::NotFound;
...
bool StorageReplicatedMergeTree::haveCommittingOps(const CommittingBlocks &committing_blocks, PartitionIdToMaxBlockPtr partitions,
                                                   std::set<CommittingBlock::Op> ops) const {
  {
    ...
    for (const auto &[partition_id, _] : *partitions) {
      ...
      if (!committing_blocks.contains(partition_id) && !queue.current_parts.hasPartitionId(partition_id)) return true;
    }
  }
  ...
  for (const auto &[partition_id, blocks] : committing_blocks) {
    ...
    for (const auto block : blocks) {
      if (block.number >= max_block_number) break;

      if (block.op == CommittingBlock::Op::Unknown || ops.contains(block.op)) return true;
    }
  }
  ...
  for (const auto &part_name : currently_loading_part_names) {
    ...
    if (it != partitions->end() && info.getDataVersion() < it->second) return true;

    if (need_wait_updates && partitions->contains(info.getOriginalPartitionId())) return true;
  }

  return false;
}

void StorageReplicatedMergeTree::waitForCommittingOpsToFinish(zkutil::ZooKeeperPtr zookeeper, PartitionIdToMaxBlockPtr partitions,
                                                              std::set<CommittingBlock::Op> ops, size_t backoff_ms,
                                                              size_t sync_timeout_ms) {
  while (true) {
...
QueryPipeline StorageReplicatedMergeTree::updateLightweight(const MutationCommands &commands, ContextPtr query_context) {
  ...
  update_holder.partition_block_numbers =
      allocateBlockNumbersInAffectedPartitions(commands, CommittingBlock::Op::Update, query_context, zookeeper);
  ...
  auto sink = std::make_shared<ReplicatedMergeTreeSinkPatch>(*this, std::move(patch_metadata), std::move(update_holder), context_copy);
...
bool StorageReplicatedMergeTree::hasLightweightDeletedMask() const { return has_lightweight_delete_parts.load(std::memory_order_relaxed); }
...
size_t StorageReplicatedMergeTree::clearOldPartsAndRemoveFromZK() {
  ...
  if (parts.empty()) return total_parts_to_remove;
...
void StorageReplicatedMergeTree::clearOldPartsAndRemoveFromZKImpl(zkutil::ZooKeeperPtr zookeeper, DataPartsVector &&parts) {
  ...
  for (const auto &part : parts) {
    ...
    if (part->is_duplicate || part->is_unexpected_local_part) {
      LOG_WARNING(log, "Will not remove part {} from ZooKeeper (is_duplicate: {}, is_unexpected_local_part: {})", part->name,
                  part->is_duplicate, part->is_unexpected_local_part);
  ...
  try {
    ...
    for (const auto &part : parts_to_delete_completely) part_names_to_delete_completely.emplace_back(part->name);
...
void StorageReplicatedMergeTree::forcefullyRemoveBrokenOutdatedPartFromZooKeeperBeforeDetaching(const String &part_name) {
  auto component_guard =
      Coordination::setCurrentComponent("StorageReplicatedMergeTree::forcefullyRemoveBrokenOutdatedPartFromZooKeeperBeforeDetaching");
  ...
  ZooKeeperRetriesInfo retries_info{settings[Setting::keeper_max_retries], settings[Setting::keeper_retry_initial_backoff_ms],
                                    settings[Setting::keeper_retry_max_backoff_ms], nullptr};
  ...
  if (!part)
    throw Exception(ErrorCodes::LOGICAL_ERROR,
                    "Outdated part {} is broken and going to be detached, "
                    "but there's no active covering part, so we are not sure that it can be safely removed from ZooKeeper "
                    "(path: {})",
                    part_name, part_path);

  LOG_WARNING(log, "Outdated part {} is broken and going to be detached, removing it from ZooKeeper. The part is covered by {}", part_name,
              part->name);
...
void StorageReplicatedMergeTree::removePartsFromZooKeeperWithRetries(PartsToRemoveFromZooKeeper &parts, size_t max_retries) {
  ...
  for (const auto &part : parts) part_names_to_remove.emplace_back(part.getPartName());
...
void StorageReplicatedMergeTree::removePartsFromZooKeeperWithRetries(const Strings &part_names, size_t max_retries) {
  ...
  if (!parts_to_retry_set.empty())
    throw Exception(ErrorCodes::UNFINISHED, "Failed to remove {} parts from ZooKeeper after {} retries", parts_to_retry_set.size(),
                    num_tries);
}

void StorageReplicatedMergeTree::removePartsFromZooKeeper(zkutil::ZooKeeperPtr &zookeeper, const Strings &part_names,
                                                          NameSet *parts_should_be_retried) try {
  Strings exists_paths;
  ...
  for (size_t i = 0; i < remove_futures.size(); ++i) {
    ...
    if (response.error == Coordination::Error::ZOK) continue;

    if (response.error == Coordination::Error::ZNONODE) {
      LOG_DEBUG(log, "There is no part {} in ZooKeeper, it was only in filesystem", part_names[i]);
    } else {
      if (parts_should_be_retried) parts_should_be_retried->insert(part_names[i]);
  ...
  if (parts_should_be_retried) parts_should_be_retried->insert(part_names.begin(), part_names.end());
  throw;
}

void StorageReplicatedMergeTree::clearLockedBlockNumbersInPartition(zkutil::ZooKeeper &zookeeper, const String &partition_id,
  ...
  if (queries_in_progress.empty()) return;

  Strings paths_to_get;
  for (const auto &block : queries_in_progress) {
    if (!startsWith(block, "block-")) continue;
    Int64 block_number = parse<Int64>(block.substr(strlen("block-")));
    if (min_block_num <= block_number && block_number <= max_block_num) paths_to_get.push_back(partition_path / block);
  }
  ...
  for (size_t i = 0; i < paths_to_get.size(); ++i) {
    ...
    if (result.error == Coordination::Error::ZNONODE) continue;

    /// The query is not an insert (it does not have block_id)
    if (result.data.ends_with(EphemeralLockInZooKeeper::LEGACY_LOCK_OTHER)) continue;
    ...
    if (result.data.ends_with(EphemeralLockInZooKeeper::LEGACY_LOCK_INSERT)) {
      ...
      constexpr const char *old_version_warning =
          "Ephemeral lock {} (referencing {}) is created by a replica "
      ...
      constexpr const char *new_version_warning =
          "Ephemeral lock {} has unexpected content ({}), "
      ...
      const auto &stop_waiting = [this, &time_waiting, &timeout]() {
        return partial_shutdown_called || (timeout < Int64(time_waiting.elapsedSeconds()));
      };

      if (!zookeeper.waitForDisappear(paths_to_get[i], stop_waiting))
        throw Exception(ErrorCodes::TIMEOUT_EXCEEDED, "Path {} does not disappear, timed out after {} seconds", paths_to_get[i], timeout);
    }
  }
}

void StorageReplicatedMergeTree::getClearBlocksInPartitionOps(Coordination::Requests &ops, zkutil::ZooKeeper &zookeeper,
                                                              const String &partition_id, Int64 min_block_num, Int64 max_block_num) {
...
void StorageReplicatedMergeTree::getClearBlocksInPartitionOpsImpl(Coordination::Requests &ops, zkutil::ZooKeeper &zookeeper,
                                                                  const String &partition_id, Int64 min_block_num, Int64 max_block_num,
  ...
  for (const String &block_id : blocks)
    if (startsWith(block_id, partition_prefix)) paths_to_get.push_back(fs::path(zookeeper_path) / blocks_dir_name / block_id);

  LOG_DEBUG(log, "getClearBlocksInPartitionOpsImpl found {} blocks in partition {}, parts range: {} - {}", paths_to_get.size(),
            partition_id, min_block_num, max_block_num);
...
void StorageReplicatedMergeTree::clearBlocksInPartition(zkutil::ZooKeeper &zookeeper, const String &partition_id, Int64 min_block_num,
                                                        Int64 max_block_num) {
  ...
  if (code != Coordination::Error::ZOK) {
    for (size_t i = 0; i < delete_requests.size(); ++i)
      if (delete_responses[i]->error != Coordination::Error::ZOK)
        LOG_WARNING(log, "Error while deleting ZooKeeper path `{}`: {}, ignoring.", delete_requests[i]->getPath(),
                    delete_responses[i]->error);
  }

  async_block_ids_cache.truncate();
  deduplication_hashes_cache.truncate();

  LOG_TRACE(log, "Deleted {} deduplication block IDs in partition ID {} in range [{}, {}]", delete_requests.size(), partition_id,
...
void StorageReplicatedMergeTree::replacePartitionFrom(const StoragePtr &source_table, const ASTPtr &partition, bool replace,
  ...
  auto lock2 =
      source_table->lockForShare(query_context->getCurrentQueryId(), query_context->getSettingsRef()[Setting::lock_acquire_timeout]);
  ...
  if (partition->as<ASTPartition>()->all) {
    if (replace) throw DB::Exception(ErrorCodes::SUPPORT_IS_DISABLED, "Only support DROP/DETACH/ATTACH PARTITION ALL currently");
  ...
  for (const auto &partition_id : partitions) {
    entries[idx] = replacePartitionFromImpl(watch, profile_events_scope, metadata_snapshot, src_data, partition_id, zookeeper, replace,
                                            zero_copy_enabled,
                                            (*storage_settings_ptr)[MergeTreeSetting::always_use_copy_instead_of_hardlinks], query_context);
    ++idx;
  }
  ...
  WatchEventByPath watch_events;
  for (const auto &entry : entries) waitForLogEntryToBeProcessedIfNecessary(*entry, query_context, watch_events);
}

std::unique_ptr<ReplicatedMergeTreeLogEntryData> StorageReplicatedMergeTree::replacePartitionFromImpl(
    const Stopwatch &watch, ProfileEventsScope &profile_events_scope, const StorageMetadataPtr &metadata_snapshot,
    const MergeTreeData &src_data, const String &partition_id, const ZooKeeperPtr &zookeeper, bool replace, const bool &zero_copy_enabled,
    const bool &always_use_copy_instead_of_hardlinks, const ContextPtr &query_context) {
  ...
  if (auto query_txn = query_context->getZooKeeperMetadataTransaction())
    txn.emplace(query_txn->getZooKeeper(), query_txn->getDatabaseZooKeeperPath(), query_txn->isInitialQuery(),
  ...
  for (size_t retry = 0; retry < 1000; ++retry) {
    ...
    for (const auto &src_part : src_all_parts) {
      ...
      std::vector<std::string> block_id_path =
          (replace || is_duplicated_part)
              ? std::vector<std::string>()
              : std::vector<std::string>{(fs::path(zookeeper_path) / "blocks" / (partition_id + "_replace_from_" + hash_hex))};
      ...
      IDataPartStorage::ClonePartParams clone_params{
          .copy_instead_of_hardlink =
              always_use_copy_instead_of_hardlinks || (zero_copy_enabled && src_part->isStoredOnRemoteDiskWithZeroCopySupport()),
          .metadata_version_to_write = metadata_snapshot->getMetadataVersion()};
      if (replace) {
        /// Replace can only work on the same disk
        auto [dst_part, part_lock] =
            cloneAndLoadDataPart(src_part, TMP_PREFIX_REPLACE_PARTITION_FROM, dst_part_info, metadata_snapshot, clone_params,
                                 query_context->getReadSettings(), query_context->getWriteSettings(), true /*must_on_same_disk*/);
        ...
        auto [dst_part, part_lock] =
            cloneAndLoadDataPart(src_part, TMP_PREFIX_REPLACE_PARTITION_FROM, dst_part_info, metadata_snapshot, clone_params,
                                 query_context->getReadSettings(), query_context->getWriteSettings(), false /*must_on_same_disk*/);
        dst_parts.emplace_back(std::move(dst_part));
        dst_parts_locks.emplace_back(std::move(part_lock));
      }
    ...
    auto entry = std::make_unique<ReplicatedMergeTreeLogEntryData>();
    {
      ...
      for (const auto &part : src_parts) entry_replace.src_part_names.emplace_back(part->name);
      for (const auto &part : dst_parts) entry_replace.new_part_names.emplace_back(part->name);
      for (const String &checksum : part_checksums) entry_replace.part_names_checksums.emplace_back(checksum);
    ...
    try {
      ...
      {
        auto data_parts_lock = lockParts();
        for (auto &part : dst_parts) renameTempPartAndReplaceUnlocked(part, transaction, data_parts_lock, /*rename_in_transaction=*/true);
      }
      transaction.renameParts();

      for (const auto &dst_part : dst_parts) lockSharedData(*dst_part, false, /*hardlinked_files*/ {});
...

      {
        ...
        if (replace) {
          parts_holder =
              getDataPartsVectorInPartitionForInternalUsage(MergeTreeDataPartState::Active, drop_range.getPartitionId(), data_parts_lock);
      ...
      PartLog::addNewParts(getContext(), PartLog::createPartLogEntries(dst_parts, watch.elapsed()),
                           ExecutionStatus::fromCurrentException("", true));
      for (const auto &dst_part : dst_parts) unlockSharedData(*dst_part);
    ...
    for (auto &lock : ephemeral_locks) lock.assumeUnlocked();
...
void StorageReplicatedMergeTree::movePartitionToTable(const StoragePtr &dest_table, const ASTPtr &partition, ContextPtr query_context) {
  ...
  if (dest_table_storage->getStoragePolicy() != this->getStoragePolicy())
    throw Exception(
        ErrorCodes::UNKNOWN_POLICY, "Destination table {} should have the same storage policy of source table {}. {}: {}, {}: {}",
        dest_table_storage->getStorageID().getNameForLogs(), getStorageID().getNameForLogs(), getStorageID().getNameForLogs(),
        this->getStoragePolicy()->getName(), getStorageID().getNameForLogs(), dest_table_storage->getStoragePolicy()->getName());

  // Use the same back-pressure (delay/throw) logic as for INSERTs to be consistent and avoid possibility of exceeding part limits using
  // MOVE PARTITION queries
  ...
  for (size_t retry = 0; retry < 1000; ++retry) {
    ...
    if (covering_part)
      throw Exception(ErrorCodes::LOGICAL_ERROR, "Got part {} covering drop range {}, it's a bug", covering_part->name,
    ...
    if (version_check_code != Coordination::Error::ZOK)
      throw Exception(ErrorCodes::CANNOT_ASSIGN_ALTER,
                      "Cannot DROP PARTITION in {} after copying partition to {}, "
    ...
    for (const auto &src_part : src_all_parts) {
      ...
      auto [dst_part, dst_part_lock] = dest_table_storage->cloneAndLoadDataPart(
          src_part, TMP_PREFIX, dst_part_info, dest_metadata_snapshot, clone_params, query_context->getReadSettings(),
          query_context->getWriteSettings(), true /*must_on_same_disk*/);
    ...
    try {
      ...
      ops.emplace_back(zkutil::makeCreateRequest(fs::path(dest_table_storage->zookeeper_path) / "log/log-", entry.toString(),
                                                 zkutil::CreateMode::PersistentSequential));

      {
        ...
        for (const auto &dst_part : dst_parts) dest_table_storage->lockSharedData(*dst_part, false, /*hardlinked_files*/ {});

        Coordination::Error code = zookeeper->tryMulti(ops, op_results);
        if (code == Coordination::Error::ZBADVERSION) continue;
        zkutil::KeeperMultiException::check(code, ops, op_results);

        parts_holder =
            getDataPartsVectorInPartitionForInternalUsage(MergeTreeDataPartState::Active, drop_range.getPartitionId(), src_data_parts_lock);
      ...
      for (const auto &dst_part : dst_parts) dest_table_storage->unlockSharedData(*dst_part);
    ...
    ops_src.emplace_back(zkutil::makeCreateRequest(fs::path(zookeeper_path) / "log/log-", entry_delete.toString(),
                                                   zkutil::CreateMode::PersistentSequential));
    /// Just update version, because merges assignment relies on it
...
void StorageReplicatedMergeTree::movePartitionToShard(const ASTPtr &partition, bool move_part, const String &to,
                                                      ContextPtr /*query_context*/) {
  /// This is a lightweight operation that only optimistically checks if it could succeed and queues tasks.

  if (!move_part) throw Exception(ErrorCodes::NOT_IMPLEMENTED, "MOVE PARTITION TO SHARD is not supported, use MOVE PART instead");

  if (zkutil::normalizeZooKeeperPath(zookeeper_path, /* check_starts_with_slash */ true) ==
      zkutil::normalizeZooKeeperPath(to, /* check_starts_with_slash */ true))
  ...
  if (!part) throw Exception(ErrorCodes::NO_SUCH_DATA_PART, "Part {} not found locally", part_name);
  ...
  /// Extract it to a common method and re-use it before merging.
  {
    if (partIsLastQuorumPart(part->info)) {
      throw Exception(ErrorCodes::NOT_IMPLEMENTED, "Part {} is last inserted part with quorum in partition. Would not be able to drop",
                      part_name);
    }
...

  {
    ...
    checkTableStructure(to, getInMemoryMetadataPtr(), /* metadata_version = */ nullptr, /* strict_check = */ true,
  ...
  Coordination::Requests ops;
  ops.emplace_back(
      zkutil::makeCheckRequest(zookeeper_path + "/log", merge_predicate->getVersion()));  /// Make sure no new events were added to the log.
  ...
  ops.emplace_back(zkutil::makeCreateRequest(part_moves_between_shards_orchestrator.entries_znode_path + "/task-",
                                             part_move_entry.toString(), zkutil::CreateMode::PersistentSequential));
...
void StorageReplicatedMergeTree::getCommitPartOps(Coordination::Requests &ops, const DataPartPtr &part, const String &block_id_path) const {
...
void StorageReplicatedMergeTree::getCommitPartOps(Coordination::Requests &ops, const DataPartPtr &part,
  ...
  for (const String &block_id_path : block_id_paths) {
    ...
    ops.emplace_back(
        zkutil::makeCreateRequest(block_id_path,
  ...
  if ((*storage_settings_ptr)[MergeTreeSetting::use_minimalistic_part_header_in_zookeeper]) {
    ops.emplace_back(
        zkutil::makeCreateRequest(fs::path(replica_path) / "parts" / part->name,
                                  ReplicatedMergeTreePartHeader::fromColumnsAndChecksums(part->getColumns(), part->checksums).toString(),
                                  zkutil::CreateMode::Persistent));
  } else {
    ops.emplace_back(zkutil::makeCreateRequest(fs::path(replica_path) / "parts" / part->name, "", zkutil::CreateMode::Persistent));
    ops.emplace_back(zkutil::makeCreateRequest(fs::path(replica_path) / "parts" / part->name / "columns", part->getColumns().toString(),
                                               zkutil::CreateMode::Persistent));
    ops.emplace_back(zkutil::makeCreateRequest(fs::path(replica_path) / "parts" / part->name / "checksums",
                                               getChecksumsForZooKeeper(part->checksums), zkutil::CreateMode::Persistent));
  }
}
...
ActionLock StorageReplicatedMergeTree::getActionLock(StorageActionBlockType action_type) {
  if (action_type == ActionLocks::PartsMerge) return merger_mutator.merges_blocker.cancel();

  if (action_type == ActionLocks::PartsTTLMerge) return merger_mutator.ttl_merges_blocker.cancel();

  if (action_type == ActionLocks::PartsFetch) return fetcher.blocker.cancel();
  ...
  if (action_type == ActionLocks::ReplicationQueue) return queue.actions_blocker.cancel();

  if (action_type == ActionLocks::PartsMove) return parts_mover.moves_blocker.cancel();

  if (action_type == ActionLocks::PullReplicationLog) return queue.pull_log_blocker.cancel();

  if (action_type == ActionLocks::Cleanup) return cleanup_thread.getCleanupLock();

  return {};
}

void StorageReplicatedMergeTree::onActionLockRemove(StorageActionBlockType action_type) {
  if (action_type == ActionLocks::PartsMerge || action_type == ActionLocks::PartsTTLMerge || action_type == ActionLocks::PartsFetch ||
      action_type == ActionLocks::PartsSend || action_type == ActionLocks::ReplicationQueue)
    background_operations_assignee.trigger();
...
bool StorageReplicatedMergeTree::waitForProcessingQueue(UInt64 max_wait_milliseconds, SyncReplicaMode sync_mode,
  ...
  auto callback = [this, &target_entry_event, &wait_for_ids, &was_interrupted, sync_mode](size_t new_queue_size,
                                                                                          const String *removed_log_entry_id) {
    if (partial_shutdown_called) {
    ...
    if (sync_mode == SyncReplicaMode::STRICT) {
      ...
      if (new_queue_size == 0) target_entry_event.set();
      return;
    }

    if (removed_log_entry_id) wait_for_ids.erase(*removed_log_entry_id);

    if (wait_for_ids.empty()) target_entry_event.set();
  };

  const auto handler = queue.addSubscriber(std::move(callback), wait_for_ids, sync_mode, source_replicas);

  if (!target_entry_event.tryWait(max_wait_milliseconds)) return false;

  if (was_interrupted) throw Exception(ErrorCodes::ABORTED, "Shutdown is called for table");

  return true;
}

bool StorageReplicatedMergeTree::dropPartImpl(zkutil::ZooKeeperPtr &zookeeper, String part_name, LogEntry &entry, bool detach,
                                              bool throw_if_noop) {
  ...
  while (true) {
    if (shutdown_called || partial_shutdown_called) throw Exception(ErrorCodes::ABORTED, "Cannot drop part because shutdown called");
    ...
    if (!part) {
      if (throw_if_noop) throw Exception(ErrorCodes::NO_SUCH_DATA_PART, "Part {} not found locally, won't try to drop it.", part_name);
      return false;
    }

    if (merge_predicate->isGoingToBeDropped(part->info)) {
      if (throw_if_noop) throw Exception(ErrorCodes::PART_IS_TEMPORARILY_LOCKED, "Already has DROP RANGE for part {} in queue.", part_name);
    ...
    if (merge_predicate->partParticipatesInReplaceRange(part, out_reason)) {
      if (throw_if_noop) throw Exception(out_reason, ErrorCodes::PART_IS_TEMPORARILY_LOCKED);
      return false;
    }
    ...
    if (partIsInsertingWithParallelQuorum(part->info)) {
      if (throw_if_noop) throw Exception(ErrorCodes::NOT_IMPLEMENTED, "Part {} is inserting with parallel quorum. Cannot drop", part_name);
      return false;
    }

    if (detach && supportsLightweightUpdate()) {
      auto patch_parts = getPatchPartsVectorForPartition(part_info.getPartitionId());
      if (!assertNoPatchesForParts({part}, patch_parts, "DETACH PART " + part_name, throw_if_noop)) return false;
    }
    ...
    ops.emplace_back(zkutil::makeCheckRequest(fs::path(zookeeper_path) / "log",
                                              merge_predicate->getVersion()));  /// Make sure no new events were added to the log.
    ops.emplace_back(
        zkutil::makeCreateRequest(fs::path(zookeeper_path) / "log/log-", entry.toString(), zkutil::CreateMode::PersistentSequential));
    /// Just update version, because merges assignment relies on it
    ops.emplace_back(zkutil::makeSetRequest(fs::path(zookeeper_path) / "log", "", -1));
    ...
    if (rc == Coordination::Error::ZNONODE) {
      LOG_TRACE(log, "Other replica already removing same part {} or part deduplication node was removed by background thread. Retry.",
                part_name);
      continue;
    }
    zkutil::KeeperMultiException::check(rc, ops, responses);
...
bool StorageReplicatedMergeTree::addOpsToDropAllPartsInPartition(zkutil::ZooKeeper &zookeeper, const String &partition_id, bool detach,
  ...
  if (detach && supportsLightweightUpdate()) {
    ...
    if (!waitForProcessingQueue(sync_timeout_ms, SyncReplicaMode::LIGHTWEIGHT, {}))
      throw Exception(ErrorCodes::TIMEOUT_EXCEEDED, "Failed to sync replica with timeout {} to satisfy update_sequential_consistency",
                      sync_timeout_ms);
...
void StorageReplicatedMergeTree::dropAllPartsInPartitions(zkutil::ZooKeeper &zookeeper, const Strings &partition_ids,
                                                          std::vector<LogEntryPtr> &entries, ContextPtr query_context, bool detach) {
  ...
  for (size_t retry = 0; retry < 1000; ++retry) {
    ...
    if (auto txn = query_context->getZooKeeperMetadataTransaction()) txn->moveOpsTo(ops);

    Coordination::Responses responses;
    Coordination::Error code = zookeeper.tryMulti(ops, responses);

    if (code == Coordination::Error::ZOK) {
      for (auto &lock : delimiting_block_locks) lock.assumeUnlocked();
    } else if (code == Coordination::Error::ZBADVERSION) {
      /// Cannot retry automatically, because some zookeeper ops were lost on the first attempt. Will retry on DDLWorker-level.
      if (query_context->getZooKeeperMetadataTransaction())
        throw Exception(ErrorCodes::CANNOT_ASSIGN_ALTER,
...
void StorageReplicatedMergeTree::enqueuePartForCheck(const String &part_name, time_t delay_to_check_seconds) {
  ...
  if (queue.isGoingToBeDropped(MergeTreePartInfo::fromPartName(part_name, format_version), &covering_drop_range)) {
    LOG_WARNING(log, "Do not enqueue part {} for check because it's covered by drop range {} and going to be removed", part_name,
                covering_drop_range.getPartNameForLogs());
    return;
  }
...
IStorage::DataValidationTasksPtr StorageReplicatedMergeTree::getCheckTaskList(
  ...
  if (const auto *partition_opt = std::get_if<ASTPtr>(&check_task_filter)) {
    ...
    if (!part)
      throw Exception(ErrorCodes::NO_SUCH_DATA_PART, "No such data part '{}' to check in table '{}'", *part_name,
...
std::optional<CheckResult> StorageReplicatedMergeTree::checkDataNext(DataValidationTasksPtr &check_task_list) {
  ...
  if (shutdown_called || partial_shutdown_called) throw Exception(ErrorCodes::ABORTED, "Table shutdown was called");
...
bool StorageReplicatedMergeTree::canUseZeroCopyReplication() const {
  auto settings_ptr = getSettings();
  if (!(*settings_ptr)[MergeTreeSetting::allow_remote_fs_zero_copy_replication]) return false;

  auto disks = getStoragePolicy()->getDisks();
  for (const auto &disk : disks) {
    if (disk->supportZeroCopyReplication()) return true;
  }
  return false;
}
  ...
  for (auto disk_it = disks.rbegin(); disk_it != disks.rend(); ++disk_it) {
    ...
    if (disk_ptr->isBroken()) {
      {
        ...
        if (!last_broken_disks.insert(disk_ptr->getName()).second) continue;
      }

      LOG_INFO(log, "Scanning parts to recover on broken disk {} with path {}", disk_ptr->getName(), disk_ptr->getPath());

      if (!parts) parts = std::make_unique<DataPartsVector>(getDataPartsVectorForInternalUsage());

      for (auto &part : *parts) {
        if (part->getDataPartStorage().getDiskName() == disk_ptr->getName()) broken_part_callback(part->name);
      }
      continue;
    }

    {
      ...
      if (last_broken_disks.erase(disk_ptr->getName()) > 0)
        LOG_INFO(log, "Disk {} with path {} is recovered. Exclude it from last_broken_disks", disk_ptr->getName(), disk_ptr->getPath());
    }
  }
}
...
MutationCounters StorageReplicatedMergeTree::getMutationCounters() const { return queue.getMutationCounters(); }

void StorageReplicatedMergeTree::startBackgroundMovesIfNeeded() {
  if (areBackgroundMovesNeeded()) background_moves_assignee.start();
}
...
String StorageReplicatedMergeTree::getTableSharedID() const {
  ...
  if (has_metadata_in_zookeeper.value_or(true)) {
    ...
    if (table_shared_id == UUIDHelpers::Nil) createTableSharedID(/* zookeeper_retries_info = */ {});
  } else {
...
void StorageReplicatedMergeTree::createTableSharedIDAttempt() const {
  ...
  if (!zookeeper->tryGet(zookeeper_table_id_path, id)) {
    LOG_DEBUG(log, "Shared ID for table {} doesn't exist in ZooKeeper on path {}", getStorageID().getNameForLogs(),
              zookeeper_table_id_path);
    ...
    if (code == Coordination::Error::ZNODEEXISTS) {  /// Other replica create node early
...
zkutil::EphemeralNodeHolderPtr StorageReplicatedMergeTree::lockSharedDataTemporary(const String &part_name, const String &part_id,
                                                                                   const DiskPtr &disk) const {
  auto settings = getSettings();

  if (!disk || !disk->supportZeroCopyReplication() || !(*settings)[MergeTreeSetting::allow_remote_fs_zero_copy_replication]) return {};
  ...
  String zc_zookeeper_path = getZeroCopyPartPath(*getSettings(), disk->getDataSourceDescription().name(), getTableSharedID(), part_name,
                                                 zookeeper_path, getContext())[0];
  ...
  createZeroCopyLockNode(std::make_shared<ZooKeeperWithFaultInjection>(zookeeper), zookeeper_node, zkutil::CreateMode::Ephemeral, false);
...
void StorageReplicatedMergeTree::lockSharedData(const IMergeTreeDataPart &part, bool replace_existing_lock,
                                                std::optional<HardlinkedFiles> hardlinked_files) const {
  LOG_DEBUG(log, "Trying to create zero-copy lock for part {}", part.name);
  auto zookeeper = tryGetZooKeeper();
  if (zookeeper) lockSharedData(part, std::make_shared<ZooKeeperWithFaultInjection>(zookeeper), replace_existing_lock, hardlinked_files);
}

void StorageReplicatedMergeTree::getLockSharedDataOps(const IMergeTreeDataPart &part, const ZooKeeperWithFaultInjectionPtr &zookeeper,
                                                      bool replace_existing_lock, std::optional<HardlinkedFiles> hardlinked_files,
                                                      Coordination::Requests &requests) const {
  auto settings = getSettings();

  if (!(*settings)[MergeTreeSetting::allow_remote_fs_zero_copy_replication]) return;

  if (!part.getDataPartStorage().supportZeroCopyReplication()) return;

  if (zookeeper->isNull()) return;

  String id = part.getUniqueId();
  boost::replace_all(id, "/", "_");

  Strings zc_zookeeper_paths = getZeroCopyPartPath(*getSettings(), part.getDataPartStorage().getDiskType(), getTableSharedID(), part.name,
  ...
  if (hardlinked_files.has_value() && !hardlinked_files->hardlinks_from_source_part.empty()) {
    path_to_set_hardlinked_files =
        getZeroCopyPartPath(*getSettings(), part.getDataPartStorage().getDiskType(), hardlinked_files->source_table_shared_id,
  ...
  for (const auto &zc_zookeeper_path : zc_zookeeper_paths) {
    ...
    if (!path_to_set_hardlinked_files.empty() && !hardlinks.empty()) {
      LOG_DEBUG(log,
                "Locking shared node {} with hardlinks from the other shared node {}, "
                "hardlinks: [{}]",
                zookeeper_node, path_to_set_hardlinked_files, boost::algorithm::join(hardlinks, ","));
    }

    getZeroCopyLockNodeCreateOps(zookeeper, zookeeper_node, requests, zkutil::CreateMode::Persistent, replace_existing_lock,
                                 path_to_set_hardlinked_files, hardlinks);
  }
}

void StorageReplicatedMergeTree::lockSharedData(const IMergeTreeDataPart &part, const ZooKeeperWithFaultInjectionPtr &zookeeper,
                                                bool replace_existing_lock, std::optional<HardlinkedFiles> hardlinked_files) const {
  auto settings = getSettings();
  ...
  for (const auto &zc_zookeeper_path : zc_zookeeper_paths) {
    ...
    createZeroCopyLockNode(zookeeper, zookeeper_node, zkutil::CreateMode::Persistent, replace_existing_lock, path_to_set_hardlinked_files,
                           hardlinks);
...
std::pair<bool, NameSet> StorageReplicatedMergeTree::unlockSharedData(const IMergeTreeDataPart &part,
                                                                      const ZooKeeperWithFaultInjectionPtr &zookeeper) const {
  auto settings = getSettings();
  if (!(*settings)[MergeTreeSetting::allow_remote_fs_zero_copy_replication]) return std::make_pair(true, NameSet{});
  ...
  if (shared_id == toString(UUIDHelpers::Nil)) {
    if (zookeeper->exists(zookeeper_path)) {
      LOG_WARNING(log,
                  "Not removing shared data for part {} because replica does not have metadata in ZooKeeper, "
                  "but table path exist and other replicas may exist. It may leave some garbage on S3",
                  part.name);
  ...
  if (part.getDataPartStorage().existsFile(IMergeTreeDataPart::FILE_FOR_REFERENCES_CHECK)) {
    ...
    LOG_TRACE(log, "Part {} looks temporary, because {} file doesn't exists, blobs can be removed", part.name,
              IMergeTreeDataPart::FILE_FOR_REFERENCES_CHECK);
  ...
  if (part.getState() == MergeTreeDataPartState::Temporary && part.is_temp) {
    ...
    if (part.remove_tmp_policy == IMergeTreeDataPart::BlobsRemovalPolicyForTemporaryParts::REMOVE_BLOBS ||
      ...
      LOG_INFO(log,
               "Looks like CH knows the origin of that part. "
               "Part {} can be deleted without unlocking shared data in zookeeper. "
               "Part blobs {}.",
               part.name, can_remove_blobs ? "will be removed" : "have to be preserved");
      return std::make_pair(can_remove_blobs, NameSet{});
    }
  }

  if (part.rows_count == 0 &&
      part.remove_tmp_policy == IMergeTreeDataPart::BlobsRemovalPolicyForTemporaryParts::REMOVE_BLOBS_OF_NOT_TEMPORARY) {
    /// It's a non-replicated empty part that was created to avoid unexpected parts after DROP_RANGE
    LOG_INFO(log,
             "Looks like {} is a non-replicated empty part that was created to avoid unexpected parts after DROP_RANGE, "
  ...
  if (!has_metadata_in_zookeeper.has_value() && !zookeeper->exists(zookeeper_path)) return std::make_pair(true, NameSet{});

  return unlockSharedDataByID(part.getUniqueId(), shared_id, part.info, replica_name, part.getDataPartStorage().getDiskType(), zookeeper,
                              *getSettings(), log.load(), zookeeper_path, format_version, getContext());
}

namespace {

/// What is going on here?
/// Actually we need this code because of flaws in hardlinks tracking. When we create child part during mutation we can hardlink some files
/// from parent part, like all_0_0_0:
...
/// But sometimes we need an opposite. When we deleting all_0_0_0_1 it can be non replicated to other replicas, so we are the only owner of
/// this part. In this case when we will drop all_0_0_0_1 we will drop blobs for all_0_0_0. But it will lead to dataloss. For such case we
/// need to check that other replicas still need parent part.
std::pair<bool, NameSet> getParentLockedBlobs(const ZooKeeperWithFaultInjectionPtr &zookeeper_ptr,
                                              const std::string &zero_copy_part_path_prefix, const MergeTreePartInfo &part_info,
                                              MergeTreeDataFormatVersion format_version, LoggerPtr log) {
  NameSet files_not_to_remove;

  /// No mutations -- no hardlinks -- no issues
  if (part_info.mutation == 0) return {false, files_not_to_remove};
  ...
  for (const auto &[parent_candidate_info, part_candidate_info_str] : parts_infos | std::views::reverse) {
    if (parent_candidate_info == part_info) continue;
    ...
    if (part_info.isMutationChildOf(parent_candidate_info)) {
      ...
      Coordination::Error code;
      zookeeper_ptr->tryGet(fs::path(zero_copy_part_path_prefix) / part_candidate_info_str, files_not_to_remove_str, nullptr, nullptr,
                            &code);
      if (code != Coordination::Error::ZOK) {
      ...
      if (!files_not_to_remove_str.empty()) {
        ...
        LOG_TRACE(log, "Found files not to remove from parent part {}: [{}]", part_candidate_info_str,
                  fmt::join(files_not_to_remove, ", "));
      } else {
...
std::pair<bool, NameSet> StorageReplicatedMergeTree::unlockSharedDataByID(
    String part_id, const String &table_uuid, const MergeTreePartInfo &part_info, const String &replica_name_, const std::string &disk_type,
    const ZooKeeperWithFaultInjectionPtr &zookeeper_ptr, const MergeTreeSettings &settings, LoggerPtr logger,
    const String &zookeeper_path_old, MergeTreeDataFormatVersion data_format_version, const ContextPtr &local_context) {
  ...
  for (const auto &zc_zookeeper_path : zc_zookeeper_paths) {
    ...
    auto [has_parent, parent_not_to_remove] =
        getParentLockedBlobs(zookeeper_ptr, fs::path(zc_zookeeper_path).parent_path(), part_info, data_format_version, logger);

    files_not_to_remove.insert(parent_not_to_remove.begin(), parent_not_to_remove.end());

    LOG_TRACE(logger, "Removing zookeeper lock {} for part {} (files to keep: [{}])", zookeeper_part_replica_node, part_name,
              fmt::join(files_not_to_remove, ", "));
    ...
    if (auto ec = zookeeper_ptr->tryRemove(zookeeper_part_replica_node); ec != Coordination::Error::ZOK) {
      ...
      if (ec == Coordination::Error::ZNONODE) {
        LOG_INFO(logger, "Lock on path {} for part {}, which {} a parent, doesn't exist, refuse to remove blobs",
                 zookeeper_part_replica_node, part_name, (has_parent ? "has" : "doesn't have"));
        return {false, {}};
      } else {
    ...
    if (error_code == Coordination::Error::ZOK) {
      ...
      LOG_TRACE(logger, "Cannot remove last parent zookeeper lock {} for part {} with id {}, another replica locked part concurrently",
                zookeeper_part_uniq_node, part_name, part_id);
      part_has_no_more_locks = false;
      continue;
    } else if (error_code == Coordination::Error::ZNONODE) {
      LOG_TRACE(logger, "Node with parent zookeeper lock {} for part {} with id {} doesn't exist", zookeeper_part_uniq_node, part_name,
                part_id);
    } else {
    ...
    if (children.empty()) {
      ...
      if (error_code == Coordination::Error::ZOK) {
        ...
        LOG_TRACE(logger, "Cannot remove last parent zookeeper lock {} for part {}, another replica locked part concurrently",
                  zookeeper_part_uniq_node, part_name);
      } else if (error_code == Coordination::Error::ZNONODE) {
        ...
        LOG_WARNING(logger,
                    "Node with parent zookeeper lock {} for part {} doesn't exist (part was unlocked before), refuse to remove blobs",
                    zookeeper_part_uniq_node, part_name);
        return {false, {}};
      } else {
        throw zkutil::KeeperException::fromPath(error_code, zookeeper_part_uniq_node);
      ...
      if (part_info.mutation && !has_parent) part_has_no_more_locks = false;
...
MergeTreeData::MutableDataPartPtr StorageReplicatedMergeTree::tryToFetchIfShared(const IMergeTreeDataPart &part, const DiskPtr &disk,
                                                                                 const String &path) {
  const auto settings = getSettings();
  auto data_source_description = disk->getDataSourceDescription();
  if (!(disk->supportZeroCopyReplication() && (*settings)[MergeTreeSetting::allow_remote_fs_zero_copy_replication])) return nullptr;
  ...
  if (replica.empty()) return nullptr;

  return executeFetchShared(replica, part.name, disk, path);
}

String StorageReplicatedMergeTree::getSharedDataReplica(const IMergeTreeDataPart &part,
                                                        const DataSourceDescription &data_source_description) const {
  String best_replica;

  zkutil::ZooKeeperPtr zookeeper = tryGetZooKeeper();
  if (!zookeeper) return "";

  Strings zc_zookeeper_paths =
      getZeroCopyPartPath(*getSettings(), data_source_description.name(), getTableSharedID(), part.name, zookeeper_path, getContext());
  ...
  if (active_replicas.empty()) return "";
...
Strings StorageReplicatedMergeTree::getZeroCopyPartPath(const MergeTreeSettings &settings, const std::string &disk_type,
  ...
  String new_path =
      fs::path(local_context->getMacros()->expand(settings[MergeTreeSetting::remote_fs_zero_copy_zookeeper_path].toString())) / zero_copy /
      table_uuid / part_name;
  res.push_back(std::move(new_path));
  if (settings[MergeTreeSetting::remote_fs_zero_copy_path_compatible_mode] &&
      !zookeeper_path_old.empty()) {  /// Compatibility mode for cluster with old and new versions
...
void StorageReplicatedMergeTree::watchZeroCopyLock(const String &part_name, const DiskPtr &disk) {
  ...
  if (path) {
    ...
    auto watch =
        zookeeper->createWatchFromRawCallback(fmt::format("watchZeroCopyLock::{}", reinterpret_cast<void *>(flag.get())),
                                              [flag]() -> Coordination::WatchCallback { return [flag](const auto &) { *flag = false; }; });
...
std::optional<String> StorageReplicatedMergeTree::getZeroCopyPartPath(const String &part_name, const DiskPtr &disk) {
  if (!disk || !disk->supportZeroCopyReplication()) return std::nullopt;

  return getZeroCopyPartPath(*getSettings(), disk->getDataSourceDescription().name(), getTableSharedID(), part_name, zookeeper_path,
                             getContext())[0];
}
...
bool StorageReplicatedMergeTree::waitZeroCopyLockToDisappear(const ZeroCopyLock &lock, size_t milliseconds_to_wait) {
  ...
  if (partial_shutdown_called.load(std::memory_order_relaxed)) return true;

  auto lock_path = lock.lock->getLockPath();
  zkutil::ZooKeeperPtr zookeeper = tryGetZooKeeper();
  if (!zookeeper) return true;
...
std::optional<ZeroCopyLock> StorageReplicatedMergeTree::tryCreateZeroCopyExclusiveLock(const String &part_name, const DiskPtr &disk) {
  ...
  if (partial_shutdown_called.load(std::memory_order_relaxed) || is_readonly.load(std::memory_order_relaxed)) return std::nullopt;

  zkutil::ZooKeeperPtr zookeeper = tryGetZooKeeper();
  if (!zookeeper) return std::nullopt;
...
String StorageReplicatedMergeTree::findReplicaHavingPart(const String &part_name, const String &zookeeper_path_,
                                                         zkutil::ZooKeeper::Ptr zookeeper_ptr) {
...
bool StorageReplicatedMergeTree::checkIfDetachedPartExists(const String &part_name) {
  for (const auto &disk : getStoragePolicy()->getDisks()) {
    for (auto it = disk->iterateDirectory(fs::path(getRelativeDataPath()) / DETACHED_DIR_NAME); it->isValid(); it->next()) {
      if (it->name() == part_name) return true;
    }
  }
  return false;
}
...
bool StorageReplicatedMergeTree::checkIfDetachedPartitionExists(const String &partition_name) {
  for (const auto &disk : getStoragePolicy()->getDisks()) {
    for (auto it = disk->iterateDirectory(fs::path(getRelativeDataPath()) / DETACHED_DIR_NAME); it->isValid(); it->next()) {
      ...
      if (part_info && part_info->getPartitionId() == partition_name) return true;
    }
  }
  return false;
}
...
bool StorageReplicatedMergeTree::createEmptyPartInsteadOfLost(zkutil::ZooKeeperPtr zookeeper, const String &lost_part_name) {
  ...
  MergeTreePartition partition;
  {
    ...
    } else if (auto parsed_partition = MergeTreePartition::tryParseValueFromID(new_part_info.getPartitionId(),
                                                                               metadata_snapshot->getPartitionKey().sample_block)) {
      partition = MergeTreePartition(*parsed_partition);
    } else {
      LOG_WARNING(log,
                  "Empty part {} is not created instead of lost part because there are no parts in partition {} (it's empty), "
                  "resolve this manually using DROP/DETACH PARTITION.",
  ...
  try {
    ...
    if (!replaced_parts.empty()) {
      Strings part_names;
      for (const auto &part : replaced_parts) part_names.emplace_back(part->name);
      ...
      throw Exception(ErrorCodes::INCORRECT_DATA, "Tried to create empty part {}, but it replaces existing parts {}.", lost_part_name,
                      fmt::join(part_names, ", "));
    }
    ...
    while (true) {
      ...
      if (covering_virtual.empty()) {
        LOG_WARNING(log, "Will not create empty part instead of lost {}, because there's no covering part in replication queue",
                    lost_part_name);
        return false;
      }
      ...
      if (merge_predicate->isGoingToBeDropped(MergeTreePartInfo::fromPartName(lost_part_name, format_version), &drop_info)) {
        LOG_WARNING(log,
                    "Will not create empty part instead of lost {}, "
      ...
      for (const String &replica : replicas) {
        ...
        if (!zookeeper->exists(current_part_path)) {
          ...
          throw Exception(ErrorCodes::DUPLICATE_DATA_PART, "Part {} already exists on replica {} on path {}", lost_part_name, replica,
                          current_part_path);
        }
      }
      ...
      bool part_found =
          getOpsToCheckPartChecksumsAndCommit(std::make_shared<ZooKeeperWithFaultInjection>(zookeeper), new_data_part,
                                              /*hardlinked_files*/ {}, /*replace_zero_copy_lock*/ true, ops, num_check_ops_unused);
      if (part_found) throw Exception(ErrorCodes::DUPLICATE_DATA_PART, "Found part on another replica, probably it was already replaced");
...
void StorageReplicatedMergeTree::getZeroCopyLockNodeCreateOps(const ZooKeeperWithFaultInjectionPtr &zookeeper, const String &zookeeper_node,
                                                              Coordination::Requests &requests, int32_t mode, bool replace_existing_lock,
  ...
  if (mode == zkutil::CreateMode::Persistent) zookeeper->checkExistsAndGetCreateAncestorsOps(zookeeper_node, requests);
...
void StorageReplicatedMergeTree::createZeroCopyLockNode(const ZooKeeperWithFaultInjectionPtr &zookeeper, const String &zookeeper_node,
  ...
  auto is_ephemeral = [&](const String &node_path) -> bool {
    ...
    if (zookeeper->tryGet(node_path, dummy_res, &node_stat)) return node_stat.ephemeralOwner;
    return false;
  };
  ...
  for (int attempts = 5; attempts > 0; --attempts) {
    Coordination::Requests ops;
    Coordination::Responses responses;
    getZeroCopyLockNodeCreateOps(zookeeper, zookeeper_node, ops, mode, replace_existing_lock, path_to_set_hardlinked_files,
                                 hardlinked_files);
    ...
    if (mode == zkutil::CreateMode::Persistent) {
      if (error == Coordination::Error::ZNONODE) continue;
      ...
      if (error == Coordination::Error::ZNODEEXISTS) {
        size_t failed_op = zkutil::getFailedOpIndex(error, responses);
        if (ops[failed_op]->getPath() == zookeeper_node) {
          LOG_WARNING(getLogger("ZeroCopyLocks"),
  ...
  if (!created) {
    ...
    throw Exception(ErrorCodes::NOT_FOUND_NODE, "Cannot create {} zero copy lock {} because part was unlocked from zookeeper", mode_str,
                    zookeeper_node);
  }
}
...
bool StorageReplicatedMergeTree::removeSharedDetachedPart(DiskPtr disk, const String &path, const String &part_name,
                                                          const String &table_uuid, const String &detached_replica_name,
                                                          const String &detached_zookeeper_path, const ContextPtr &local_context,
  ...
  if (disk->existsFile(checksums)) {
    if (disk->getRefCount(checksums) == 0) {
      ...
      std::tie(can_remove, files_not_to_remove) = StorageReplicatedMergeTree::unlockSharedDataByID(
          id, table_uuid, part_info, detached_replica_name, disk->getDataSourceDescription().name(),
          std::make_shared<ZooKeeperWithFaultInjection>(zookeeper), local_context->getReplicatedMergeTreeSettings(),
          getLogger("StorageReplicatedMergeTree"), detached_zookeeper_path, MERGE_TREE_DATA_MIN_FORMAT_VERSION_WITH_CUSTOM_PARTITIONING,
          local_context);
...
void StorageReplicatedMergeTree::applyMetadataChangesToCreateQueryForBackup(const ASTPtr &create_query) const {
  ...
  try {
    ...
    auto metadata_diff =
        table_metadata.checkAndFindDiff(metadata_from_entry, current_metadata->getColumns(), getStorageID().getNameForLogs(), getContext());
    ...
    tryLogCurrentException(
        log, fmt::format("Failed to apply metadata changes to the create query of table {}", getStorageID().getNameForLogs()));
  }
}

void StorageReplicatedMergeTree::backupData(BackupEntriesCollector &backup_entries_collector, const String &data_path_in_backup,
  ...
  coordination->addReplicatedPartNames(zookeeper_info.full_path, getStorageID().getFullTableName(), getReplicaName(),
                                       part_names_with_hashes);
  ...
  /// Send a list of mutations to the coordination too (we need to find the mutations which are not finished for added part names).
  {
    ...
    Strings mutation_ids;
    {
      ...
      retries_ctl.retryLoop([&]() {
        if (!zookeeper || zookeeper->expired()) zookeeper = local_context->getZooKeeper();
    ...
    if (exists) {
      ...
      for (const auto &mutation_id : mutation_ids) {
        ...
        if (mutation_id_exists) mutation_infos.emplace_back(IBackupCoordination::MutationInfo{mutation_id, mutation});
      }
  ...
  auto post_collecting_task = [my_full_zookeeper_path = zookeeper_info.full_path, my_replica_name = getReplicaName(), coordination,
                               my_parts_backup_entries = std::move(parts_backup_entries), &backup_entries_collector]() {
    ...
    for (const auto &data_path : data_paths) data_paths_fs.push_back(data_path);
    ...
    for (const auto &part_backup_entries : my_parts_backup_entries) {
      if (part_names_set.contains(part_backup_entries.part_name)) {
        for (const auto &[relative_path, backup_entry] : part_backup_entries.backup_entries)
          for (const auto &data_path : data_paths_fs) backup_entries_collector.addBackupEntry(data_path / relative_path, backup_entry);
      }
    }
...
void StorageReplicatedMergeTree::restoreDataFromBackup(RestorerFromBackup &restorer, const String &data_path_in_backup,
                                                       const std::optional<ASTs> &partitions) {
  ...
  if (!restorer.isNonEmptyTableAllowed()) {
    ...
    if (empty) {
      ...
      if (status.queue.inserts_in_queue) empty = false;
    }
    auto backup = restorer.getBackup();
    if (!empty && backup->hasFiles(data_path_in_backup)) RestorerFromBackup::throwTableIsNotEmpty(getStorageID());
  }
...
void StorageReplicatedMergeTree::attachRestoredParts(MutableDataPartsVector &&parts) {
  ...
  auto sink = std::make_shared<ReplicatedMergeTreeSink>(
      /* async_insert */ false, *this, metadata_snapshot, /* quorum */ 0, /* quorum_timeout_ms */ 0, /* max_parts_per_block */ 0,
      /* quorum_parallel */ false,
      /* majority_quorum */ false, getContext(), /* is_attach */ true, /* allow_attach_while_readonly */ false);

  for (auto part : parts) sink->writeExistingPart(part);
}
