...
StoragePrometheusQuery::Configuration StoragePrometheusQuery::getConfiguration(ASTs& args, const ContextPtr& context, bool over_range) {
  ...
  if ((args.size() < min_num_args) || (args.size() > max_num_args)) {
    ...
    throw Exception(ErrorCodes::NUMBER_OF_ARGUMENTS_DOESNT_MATCH,
                    "Table function '{}' requires {}..{} arguments: {}([database, ] time_series_table, promql_query, {})", function_name,
  ...
  if (promql_query_field.getType() != Field::Types::String)
    throw Exception(ErrorCodes::BAD_ARGUMENTS, "Argument 'promql_query' must be a literal with type String, got {}",
                    promql_query_field.getType());
...
StoragePrometheusQuery::StoragePrometheusQuery(const StorageID& table_id_, const ColumnsDescription& columns_, const Configuration& config_)
    : IStorage{table_id_}, config(config_), log(getLogger("StoragePrometheusQuery")) {
  StorageInMemoryMetadata storage_metadata;
  storage_metadata.setColumns(columns_);
  setInMemoryMetadata(storage_metadata);
}

void StoragePrometheusQuery::read(QueryPlan& query_plan, const Names& column_names, const StorageSnapshotPtr& /* storage_snapshot */,
