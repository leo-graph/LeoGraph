 ...
 public:
  struct Columns {
    ...
    void addColumn(NameAndTypePair column) {
      if (column_names.contains(column.name)) return;
  ...
  const std::unordered_map<QueryTreeNodePtr, Columns> &getColumnSourceToColumns() const { return column_source_to_columns; }
...
class DistributedProductModeRewriteInJoinVisitor : public InDepthQueryTreeVisitorWithContext<DistributedProductModeRewriteInJoinVisitor> {
 public:
  ...
  explicit DistributedProductModeRewriteInJoinVisitor(const ContextPtr &context_) : Base(context_) {}
  ...
  const std::unordered_map<const IQueryTreeNode *, QueryTreeNodePtr> &getReplacementMap() const { return replacement_map; }

  const std::vector<InFunctionOrJoin> &getGlobalInOrJoinNodes() const { return global_in_or_join_nodes; }

  static bool needChildVisit(QueryTreeNodePtr &parent, QueryTreeNodePtr &child) {
    auto *function_node = parent->as<FunctionNode>();
    if (function_node && isNameOfGlobalInFunction(function_node->getFunctionName())) return false;

    auto *join_node = parent->as<JoinNode>();
    if (join_node && join_node->getLocality() == JoinLocality::Global && join_node->getRightTableExpression() == child) return false;
  ...
  void enterImpl(QueryTreeNodePtr &node) {
    ...
    if (node->getNodeType() == QueryTreeNodeType::TABLE) tryRewriteTableNodeIfNeeded(node);
  }
...
class ReplaceLongConstWithScalarVisitor : public InDepthQueryTreeVisitorWithContext<ReplaceLongConstWithScalarVisitor> {
 public:
  ...
  explicit ReplaceLongConstWithScalarVisitor(const ContextPtr &context, Int64 max_size_) : Base(context), max_size(max_size_) {}

  static bool needChildVisit(QueryTreeNodePtr &parent, QueryTreeNodePtr &child) {
    if (auto *function_node = parent->as<FunctionNode>()) {
      /// Do not traverse into `__getScalar` - it's already been processed.
      if (function_node->getFunctionName() == "__getScalar") return false;

      /// Do not visit parameters node.
      if (function_node->getParametersNode() == child) return false;
    }
    ...
    if (auto *query_node = parent->as<QueryNode>()) {
      ...
      if (query_node->hasLimit() && query_node->getLimit() == child) return false;
      if (query_node->hasOffset() && query_node->getOffset() == child) return false;
      if (query_node->hasLimitByLimit() && query_node->getLimitByLimit() == child) return false;
      if (query_node->hasLimitByOffset() && query_node->getLimitByOffset() == child) return false;
    }
  ...
  void enterImpl(QueryTreeNodePtr &node) {
    ...
    if (max_size > 0) {
      ...
      if (options.notFull(name_buf)) return;
    }
    ...
    auto scalar_query_hash_constant_node =
        std::make_shared<ConstantNode>(std::move(scalar_query_hash_string), std::make_shared<DataTypeString>());
...
void addDistinctRecursively(const QueryTreeNodePtr &node) {
  if (auto *query_node = node->as<QueryNode>()) {
    if (!query_node->isDistinct()) query_node->setIsDistinct(true);
    ...
    if (union_mode == SelectUnionMode::UNION_DEFAULT || union_mode == SelectUnionMode::UNION_ALL) {
      for (auto &child : union_node->getQueries().getNodes()) addDistinctRecursively(child);
    }
  }
}
 ...
 */
TableNodePtr executeSubqueryNode(const QueryTreeNodePtr &subquery_node, ContextMutablePtr &mutable_context, size_t subquery_depth) {
  ...
  if (!blocksHaveEqualStructure(sample_block_with_unique_names, *query_plan.getCurrentHeader())) {
    auto actions_dag = ActionsDAG::makeConvertingActions(query_plan.getCurrentHeader()->getColumnsWithTypeAndName(),
                                                         sample_block_with_unique_names.getColumnsWithTypeAndName(),
                                                         ActionsDAG::MatchColumnsMode::Position, context_copy);
  ...
  auto external_storage_holder = TemporaryTableHolder(mutable_context, ColumnsDescription(columns), ConstraintsDescription{},
  ...
  if (join_table_expression_node_type == QueryTreeNodeType::QUERY || join_table_expression_node_type == QueryTreeNodeType::UNION) {
    ...
    while (!nodes_to_visit.empty()) {
      ...
      for (const auto &child : current->getChildren())
        if (child) nodes_to_visit.push_back(child);
    }
    ...
    throw Exception(ErrorCodes::LOGICAL_ERROR, "Expected JOIN table expression to be table, table function, query or union node. Actual {}",
...
QueryTreeNodePtr buildQueryTreeForShard(const PlannerContextPtr &planner_context, QueryTreeNodePtr query_tree_to_modify,
  ...
  bool enable_add_distinct_to_in_subqueries =
      planner_context->getQueryContext()->getSettingsRef()[Setting::enable_add_distinct_to_in_subqueries];
  ...
  for (const auto &global_in_or_join_node : global_in_or_join_nodes) {
    if (auto *join_node = global_in_or_join_node.query_node->as<JoinNode>()) {
      ...
      auto temporary_table_expression_node =
          executeSubqueryNode(subquery_node, planner_context->getMutableQueryContext(), global_in_or_join_node.subquery_depth);
      ...
      for (const auto &child : join_table_expression->getChildren())
        if (child) descendants_to_map.push_back(child.get());
    ...
    if (auto *in_function_node = global_in_or_join_node.query_node->as<FunctionNode>()) {
      ...
      if (!temporary_table_expression_node) {
        ...
        if (enable_add_distinct_to_in_subqueries) addDistinctRecursively(subquery_to_execute);

        temporary_table_expression_node =
      ...
      throw Exception(ErrorCodes::LOGICAL_ERROR, "Expected global IN or JOIN query node. Actual {}",
                      global_in_or_join_node.query_node->formatASTForErrorMessage());
    }
  }

  if (!replacement_map.empty()) query_tree_to_modify = query_tree_to_modify->cloneAndReplace(replacement_map);
  ...
  if (auto *query_node = query_tree_to_modify->as<QueryNode>()) query_node->clearSettingsChanges();
 ...
 public:
  ...
  static bool allStoragesAreMergeTree(QueryTreeNodePtr &node) {
    ...
    for (const auto &storage : collect_storages.storages)
      if (!storage->isMergeTree()) return false;
  ...
  void enterImpl(QueryTreeNodePtr &node) {
    if (auto *join_node = node->as<JoinNode>()) {
      ...
      if (should_use_global_join) join_node->setLocality(JoinLocality::Global);
    }
  }

  static bool needChildVisit(QueryTreeNodePtr &parent, QueryTreeNodePtr &child) {
    auto *join_node = parent->as<JoinNode>();
    if (join_node && join_node->getRightTableExpression() == child) return false;
