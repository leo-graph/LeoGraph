...
bool MutationCommand::isBarrierCommand() const { return type == RENAME_COLUMN; }

bool MutationCommand::isPureMetadataCommand() const { return type == ALTER_WITHOUT_MUTATION; }

bool MutationCommand::isEmptyCommand() const { return type == EMPTY; }

bool MutationCommand::isDropOrRename() const {
  return type == Type::DROP_COLUMN || type == Type::DROP_INDEX || type == Type::DROP_PROJECTION || type == Type::DROP_STATISTICS ||
         type == Type::RENAME_COLUMN;
}

bool MutationCommand::affectsAllColumns() const { return type == DELETE || type == APPLY_DELETED_MASK || type == REWRITE_PARTS; }

std::optional<MutationCommand> MutationCommand::parse(const ASTAlterCommand &command, bool parse_alter_commands,
  ...
  if (command.type == ASTAlterCommand::UPDATE) {
    ...
    for (const ASTPtr &assignment_ast : command.update_assignments->children) {
      ...
      if (!insertion.second)
        throw Exception(ErrorCodes::MULTIPLE_ASSIGNMENTS_TO_COLUMN, "Multiple assignments in the single statement to column {}",
  ...
  if (parse_alter_commands && command.type == ASTAlterCommand::MODIFY_COLUMN && command.remove_property.empty() &&
    ...
    if (nullptr == ast_col_decl.getType())
      throw Exception(ErrorCodes::LOGICAL_ERROR, "MODIFY COLUMN mutation command doesn't specify type: {}",
                      command.formatForErrorMessage());
  ...
  if (parse_alter_commands && command.type == ASTAlterCommand::DROP_COLUMN) {
    ...
    res.column_name = getIdentifierName(command.column);
    if (command.partition) res.partition = command.partition->clone();
    if (command.clear_column) res.clear = true;
  ...
  if (parse_alter_commands && command.type == ASTAlterCommand::DROP_INDEX) {
    ...
    if (command.partition) res.partition = command.partition->clone();
    if (command.clear_index) res.clear = true;
    return res;
  }
  if (parse_alter_commands && command.type == ASTAlterCommand::DROP_STATISTICS) {
    res.type = MutationCommand::Type::DROP_STATISTICS;
    if (command.partition) res.partition = command.partition->clone();
    if (command.clear_statistics) res.clear = true;
    if (command.statistics_decl) res.statistics_columns = command.statistics_decl->as<ASTStatisticsDeclaration &>().getColumnNames();
    return res;
  }
  if (parse_alter_commands && command.type == ASTAlterCommand::DROP_PROJECTION) {
    ...
    if (command.partition) res.partition = command.partition->clone();
    if (command.clear_projection) res.clear = true;
    return res;
  }
...
boost::intrusive_ptr<ASTExpressionList> MutationCommands::ast(bool with_pure_metadata_commands) const {
  ...
  for (const MutationCommand &command : *this) {
    if (!command.isPureMetadataCommand() || with_pure_metadata_commands) res->children.push_back(command.ast->clone());
  }
  return res;
}
...
void MutationCommands::readText(ReadBuffer &in, bool with_pure_metadata_commands) {
  ...
  auto commands_ast = parseQuery(p_alter_commands, commands_str.data(), commands_str.data() + commands_str.length(),
                                 "mutation commands list", 0, DBMS_DEFAULT_MAX_PARSER_DEPTH, DBMS_DEFAULT_MAX_PARSER_BACKTRACKS);
...
bool MutationCommands::hasNonEmptyMutationCommands() const {
  for (const auto &command : *this) {
    if (!command.isEmptyCommand() && !command.isPureMetadataCommand()) return true;
  }
  return false;
}
...
bool MutationCommands::containBarrierCommand() const {
  for (const auto &command : *this) {
    if (command.isBarrierCommand()) return true;
  }
  return false;
}
...
NameSet MutationCommands::getAllUpdatedColumns() const {
  NameSet res;
  for (const auto &command : *this)
    for (const auto &[column_name, _] : command.column_to_update_expression) res.insert(column_name);
  return res;
}
