...
#define LIST_OF_MYSQL_SETTINGS(DECLARE, ALIAS)                                                                                             \
  DECLARE(UInt64, connection_pool_size, 16,                                                                                                \
          "Size of connection pool (if all connections are in use, the query will wait until some connection will be freed).", 0)          \
  DECLARE(UInt64, connection_max_tries, 3, "Number of retries for pool with failover", 0)                                                  \
  DECLARE(UInt64, connection_wait_timeout, 5,                                                                                              \
          "Timeout (in seconds) for waiting for free connection (in case of there is already connection_pool_size active connections), 0 " \
          "- do not wait.",                                                                                                                \
          0)                                                                                                                               \
  ...
  DECLARE(                                                                                                                                 \
      MySQLDataTypesSupport, mysql_datatypes_support_level, "decimal,datetime64,date2Date32",                                              \
      "Which MySQL types should be converted to corresponding ClickHouse types. All modern mappings (decimal, datetime64, date2Date32) "   \
      "are enabled by default. Can be set to any combination of 'decimal', 'datetime64', 'date2Date32', or 'date2String'.",                \
      0)
...
void MySQLSettings::loadFromQuery(const ASTSetQuery& settings_def) { impl->applyChanges(settings_def.changes); }
...
void MySQLSettings::loadFromQueryContext(ContextPtr context, ASTStorage& storage_def) {
  if (!context->hasQueryContext()) return;
  ...
  if (settings[Setting::mysql_datatypes_support_level].value != impl->mysql_datatypes_support_level.value) {
    ...
    if (changes.end() == std::find_if(changes.begin(), changes.end(), [](const SettingChange& c) { return c.name == setting_name; })) {
...
std::vector<std::string_view> MySQLSettings::getAllRegisteredNames() const {
  ...
  for (const auto& setting_field : impl->all()) all_settings.push_back(setting_field.getName());
  return all_settings;
}
...
bool MySQLSettings::hasBuiltin(std::string_view name) { return MySQLSettingsImpl::hasBuiltin(name); }
}  // namespace DB
