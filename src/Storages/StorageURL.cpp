...
static constexpr auto bad_arguments_error_message =
    "Storage URL requires 1-4 arguments: "
...
static const std::unordered_set<std::string_view> optional_configuration_keys = {
    "format", "compression", "compression_method", "structure",           "filename",
    "method", "http_method", "description",        "headers.header.name", "headers.header.value",
};

bool urlWithGlobs(const String &uri) { return (uri.contains('{') && uri.contains('}')) || uri.contains('|'); }
...
IStorageURLBase::IStorageURLBase(const String &uri_, const ContextPtr &context_, const StorageID &table_id_, const String &format_name_,
                                 const std::optional<FormatSettings> &format_settings_, const ColumnsDescription &columns_,
                                 const ConstraintsDescription &constraints_, const String &comment, const String &compression_method_,
                                 const HTTPHeaderEntries &headers_, const String &http_method_, ASTPtr partition_by_,
  ...
  if (format_name != "auto") FormatFactory::instance().checkFormatName(format_name);
  ...
  std::tie(hive_partition_columns_to_read_from_file_path, file_columns) = HivePartitioningUtils::setupHivePartitioningForFileURLLikeStorage(
      storage_columns, sample_path, columns_.empty(), format_settings, context_);

  auto virtual_columns_desc = VirtualColumnUtils::getVirtualsForFileLikeStorage(storage_metadata.columns, context_, format_settings,
  ...
  if (!storage_metadata.getColumns().has("_headers")) {
    virtual_columns_desc.addEphemeral(
        "_headers",
        std::make_shared<DataTypeMap>(std::make_shared<DataTypeLowCardinality>(std::make_shared<DataTypeString>()),
...
class StorageURLSource::DisclosedGlobIterator::Impl {
 public:
  Impl(const String &uri_, size_t max_addresses, const ActionsDAG::Node *predicate, const NamesAndTypesList &virtual_columns,
       const NamesAndTypesList &hive_columns, const ContextPtr &context) {
    uris = parseRemoteDescription(uri_, 0, uri_.size(), ',', max_addresses);

    std::optional<ActionsDAG> filter_dag;
    if (!uris.empty()) filter_dag = VirtualColumnUtils::createPathAndFileFilterDAG(predicate, virtual_columns, context, hive_columns);

    if (filter_dag) {
      std::vector<String> paths;
      paths.reserve(uris.size());
      for (const auto &uri : uris) paths.push_back(Poco::URI(uri).getPath());
  ...
  String next() {
    ...
    if (current_index >= uris.size()) return {};

    return uris[current_index];
  }

  size_t size() { return uris.size(); }
...
StorageURLSource::DisclosedGlobIterator::DisclosedGlobIterator(const String &uri, size_t max_addresses, const ActionsDAG::Node *predicate,
                                                               const NamesAndTypesList &virtual_columns,
                                                               const NamesAndTypesList &hive_columns, const ContextPtr &context)
    : pimpl(std::make_shared<StorageURLSource::DisclosedGlobIterator::Impl>(uri, max_addresses, predicate, virtual_columns, hive_columns,
                                                                            context)) {}

String StorageURLSource::DisclosedGlobIterator::next() { return pimpl->next(); }

size_t StorageURLSource::DisclosedGlobIterator::size() { return pimpl->size(); }
...
StorageURLSource::StorageURLSource(const ReadFromFormatInfo &info, std::shared_ptr<IteratorWrapper> uri_iterator_,
                                   const std::string &http_method, std::function<void(std::ostream &)> callback, const String &format_,
                                   const std::optional<FormatSettings> &format_settings_, String name_, const ContextPtr &context_,
                                   UInt64 max_block_size, const ConnectionTimeouts &timeouts, CompressionMethod compression_method,
                                   FormatParserSharedResourcesPtr parser_shared_resources_, FormatFilterInfoPtr format_filter_info_,
                                   const HTTPHeaderEntries &headers_, const URIParams &params, bool glob_url, bool need_only_count_)
  ...
  initialize = [=, this]() {
    ...
    do {
      ...
      if (current_uri_options.empty()) return false;

      auto first_option = current_uri_options.cbegin();
      uri_and_buf = getFirstAvailableURIAndReadBuffer(first_option, current_uri_options.end(), getContext(), params, http_method, callback,
                                                      timeouts, credentials, headers, glob_url, current_uri_options.size() == 1);
    ...
    if (num_rows_from_cache) {
      ...
      input_format = FormatFactory::instance().getInput(format, *read_buf, block_for_format, getContext(), max_block_size, format_settings,
                                                        parser_shared_resources, format_filter_info,
                                                        /* is_remote_ fs */ true, compression_method, need_only_count);
...
Chunk StorageURLSource::generate() {
  while (true) {
    ...
    if (reader->pull(chunk)) {
      ...
      size_t chunk_size = 0;
      if (input_format) chunk_size = input_format->getApproxBytesReadForChunk();
      ...
      if (!hive_partition_columns_to_read_from_file_path.empty()) {
        ...
        HivePartitioningUtils::addPartitionColumnsToChunk(chunk, hive_partition_columns_to_read_from_file_path, path);
      }

      VirtualColumnUtils::addRequestedFileLikeStorageVirtualsToChunk(chunk, requested_virtual_columns,
                                                                     {
      ...
      if (need_headers_virtual_column) {
        ...
        auto type = std::make_shared<DataTypeMap>(std::make_shared<DataTypeLowCardinality>(std::make_shared<DataTypeString>()),
                                                  std::make_shared<DataTypeLowCardinality>(std::make_shared<DataTypeString>()));
...
std::pair<Poco::URI, std::unique_ptr<ReadWriteBufferFromHTTP>> StorageURLSource::getFirstAvailableURIAndReadBuffer(
    std::vector<String>::const_iterator &option, const std::vector<String>::const_iterator &end, ContextPtr context_,
    const URIParams &params, const String &http_method, std::function<void(std::ostream &)> callback, const ConnectionTimeouts &timeouts,
    Poco::Net::HTTPBasicCredentials &credentials, const HTTPHeaderEntries &headers, bool glob_url, bool delay_initialization) {
  ...
  for (; option != end; ++option) {
    ...
    for (const auto &[param, value] : params) request_uri.addQueryParameter(param, value);
    ...
    try {
      ...
      if (first_exception_message.empty()) first_exception_message = getCurrentExceptionMessage(false);
  ...
  if (last_skipped_empty_res.second) return last_skipped_empty_res;
...
std::optional<size_t> StorageURLSource::tryGetNumRowsFromCache(const String &uri, std::optional<time_t> last_mod_time) {
  ...
  auto get_last_mod_time = [&]() -> std::optional<time_t> {
    ...
    if (!last_mod_time && !getContext()->getSettingsRef()[Setting::schema_inference_cache_require_modification_time_for_url]) return 0;
...
StorageURLSink::StorageURLSink(String uri_, String format_, const std::optional<FormatSettings> &format_settings_,
                               const Block &sample_block, const ContextPtr &context_, const ConnectionTimeouts &timeouts_,
                               const CompressionMethod &compression_method_, HTTPHeaderEntries headers_, String method)
...
void StorageURLSink::initBuffers() {
  ...
  write_buf = wrapWriteBufferWithCompressionMethod(std::move(write_buffer), compression_method,
...
class PartitionedStorageURLSink : public PartitionedSink {
 public:
  PartitionedStorageURLSink(std::shared_ptr<IPartitionStrategy> partition_strategy_, const String &uri_, const String &format_,
                            const std::optional<FormatSettings> &format_settings_, const Block &sample_block_, ContextPtr context_,
                            const ConnectionTimeouts &timeouts_, const CompressionMethod compression_method_,
  ...
  SinkPtr createSinkForPartition(const String &partition_id) override {
    ...
    return std::make_shared<StorageURLSink>(partition_path, format, format_settings, sample_block, context, timeouts, compression_method,
                                            headers, http_method);
  }
...
std::string IStorageURLBase::getReadMethod() const { return Poco::Net::HTTPRequest::HTTP_GET; }

std::vector<std::pair<std::string, std::string>> IStorageURLBase::getReadURIParams(
    const Names & /*column_names*/, const StorageSnapshotPtr & /*storage_snapshot*/, const SelectQueryInfo & /*query_info*/,
    const ContextPtr & /*context*/, QueryProcessingStage::Enum & /*processed_stage*/, size_t /*max_block_size*/) const {
  return {};
}

std::function<void(std::ostream &)> IStorageURLBase::getReadPOSTDataCallback(
    const Names & /*column_names*/, const ColumnsDescription & /* columns_description */, const SelectQueryInfo & /*query_info*/,
    const ContextPtr & /*context*/, QueryProcessingStage::Enum & /*processed_stage*/, size_t /*max_block_size*/) const {
  return nullptr;
}
...
class ReadBufferIterator : public IReadBufferIterator, WithContext {
 public:
  ReadBufferIterator(const std::vector<String> &urls_to_check_, std::optional<String> format_, const CompressionMethod &compression_method_,
                     const HTTPHeaderEntries &headers_, const std::optional<FormatSettings> &format_settings_, const ContextPtr &context_)
      : WithContext(context_),
        format(std::move(format_)),
        compression_method(compression_method_),
        headers(headers_),
  ...
  Data next() override {
    ...
    if (is_first) {
      ...
      if (getContext()->getSettingsRef()[Setting::schema_inference_mode] == SchemaInferenceMode::DEFAULT) {
        for (const auto &options : url_options_to_check) {
          if (auto cached_columns = tryGetColumnsFromCache(options)) return {nullptr, cached_columns, format};
        }
      }
    }
    ...
    do {
      ...
      uri_and_buf = StorageURLSource::getFirstAvailableURIAndReadBuffer(first_option, url_options_to_check[current_index].cend(),
                                                                        getContext(), {}, Poco::Net::HTTPRequest::HTTP_GET, {},
                                                                        getHTTPTimeouts(getContext()), credentials, headers, false, false);

      ++current_index;
    } while (getContext()->getSettingsRef()[Setting::engine_url_skip_empty_files] && uri_and_buf.second->eof());

    current_url_option = uri_and_buf.first.toString();
    return {wrapReadBufferWithCompressionMethod(std::move(uri_and_buf.second), compression_method,
                                                static_cast<int>(getContext()->getSettingsRef()[Setting::zstd_window_log_max])),
            std::nullopt, format};
  }

  void setNumRowsToLastFile(size_t num_rows) override {
    if (!getContext()->getSettingsRef()[Setting::schema_inference_use_cache_for_url]) return;
  ...
  std::unique_ptr<ReadBuffer> recreateLastReadBuffer() override {
    ...
    auto uri_and_buf = StorageURLSource::getFirstAvailableURIAndReadBuffer(
        first_option, url_options_to_check[current_index - 1].cend(), getContext(), {}, Poco::Net::HTTPRequest::HTTP_GET, {},
        getHTTPTimeouts(getContext()), credentials, headers, false, false);
 ...
 private:
  std::optional<ColumnsDescription> tryGetColumnsFromCache(const Strings &urls) {
    ...
    if (!context->getSettingsRef()[Setting::schema_inference_use_cache_for_url]) return std::nullopt;
...
std::pair<ColumnsDescription, String> IStorageURLBase::getTableStructureAndFormatFromDataImpl(
    std::optional<String> format, const String &uri, CompressionMethod compression_method, const HTTPHeaderEntries &headers,
...
ColumnsDescription IStorageURLBase::getTableStructureFromData(const String &format, const String &uri, CompressionMethod compression_method,
                                                              const HTTPHeaderEntries &headers,
                                                              const std::optional<FormatSettings> &format_settings,
...
std::pair<ColumnsDescription, String> IStorageURLBase::getTableStructureAndFormatFromData(
    const String &uri, CompressionMethod compression_method, const HTTPHeaderEntries &headers,
    const std::optional<FormatSettings> &format_settings, const ContextPtr &context) {
...
bool IStorageURLBase::supportsPrewhere() const { return supports_prewhere; }

bool IStorageURLBase::canMoveConditionsToPrewhere() const { return supports_prewhere; }
...
IStorage::ColumnSizeByName IStorageURLBase::getColumnSizes() const { return getInMemoryMetadataPtr()->getFakeColumnSizes(); }

bool IStorageURLBase::prefersLargeBlocks() const { return FormatFactory::instance().checkIfOutputFormatPrefersLargeBlocks(format_name); }
...
class ReadFromURL : public SourceStepWithFilter {
 public:
  ...
  ReadFromURL(const Names &column_names_, const SelectQueryInfo &query_info_, const StorageSnapshotPtr &storage_snapshot_,
              const ContextPtr &context_, Block sample_block, std::shared_ptr<IStorageURLBase> storage_, std::vector<String> *uri_options_,
              ReadFromFormatInfo info_, const bool need_only_count_, std::vector<std::pair<std::string, std::string>> read_uri_params_,
              std::function<void(std::ostream &)> read_post_data_callback_, size_t max_block_size_, size_t num_streams_)
      : SourceStepWithFilter(std::make_shared<const Block>(std::move(sample_block)), column_names_, query_info_, storage_snapshot_,
                             context_),
...
void IStorageURLBase::read(QueryPlan &query_plan, const Names &column_names, const StorageSnapshotPtr &storage_snapshot,
  ...
  auto read_from_format_info = prepareReadingFromFormat(
      column_names, storage_snapshot, local_context, supportsSubsetOfColumns(local_context),
  ...
  auto read_post_data_callback =
      getReadPOSTDataCallback(read_from_format_info.columns_description.getNamesOfPhysical(), read_from_format_info.columns_description,
  ...
  auto reading =
      std::make_unique<ReadFromURL>(column_names, query_info, storage_snapshot, local_context, read_from_format_info.source_header,
                                    std::move(this_ptr), nullptr, std::move(read_from_format_info), need_only_count, std::move(params),
                                    std::move(read_post_data_callback), max_block_size, num_streams);

  query_plan.addStep(std::move(reading));
}

void ReadFromURL::createIterator(const ActionsDAG::Node *predicate) {
  if (iterator_wrapper || is_empty_glob) return;

  if (uri_options) {
    iterator_wrapper = std::make_shared<StorageURLSource::IteratorWrapper>([&, done = false]() mutable {
      if (done) return StorageURLSource::FailoverOptions{};
      done = true;
      return *uri_options;
    });
  ...
  if (storage->distributed_processing) {
    iterator_wrapper =
        std::make_shared<StorageURLSource::IteratorWrapper>([callback = context->getClusterFunctionReadTaskCallback(), max_addresses]() {
          auto task = callback();
          if (!task || task->isEmpty()) return StorageURLSource::FailoverOptions{};
    ...
    auto glob_iterator = std::make_shared<StorageURLSource::DisclosedGlobIterator>(
        storage->uri, max_addresses, predicate, storage->getVirtualsList(), info.hive_partition_columns_to_read_from_file_path, context);
    ...
    iterator_wrapper = std::make_shared<StorageURLSource::IteratorWrapper>([glob_iterator, max_addresses]() {
      ...
      if (next_uri.empty()) return StorageURLSource::FailoverOptions{};
...
void ReadFromURL::initializePipeline(QueryPipelineBuilder &pipeline, const BuildQueryPipelineSettings &) {
  ...
  auto format_filter_info =
      std::make_shared<FormatFilterInfo>(filter_actions_dag, context, nullptr, query_info.row_level_filter, query_info.prewhere_info);

  for (size_t i = 0; i < num_streams; ++i) {
    auto source = std::make_shared<StorageURLSource>(
        info, iterator_wrapper, storage->getReadMethod(), read_post_data_callback, storage->format_name, storage->format_settings,
        storage->getName(), context, max_block_size, getHTTPTimeouts(context), storage->compression_method, parser_shared_resources,
        format_filter_info, storage->headers, read_uri_params, is_url_with_globs, need_only_count);

    pipes.emplace_back(std::move(source));
  }

  if (uri_options) std::shuffle(uri_options->begin(), uri_options->end(), thread_local_rng);
...
void StorageURLWithFailover::read(QueryPlan &query_plan, const Names &column_names, const StorageSnapshotPtr &storage_snapshot,
                                  SelectQueryInfo &query_info, ContextPtr local_context, QueryProcessingStage::Enum processed_stage,
                                  size_t max_block_size, size_t num_streams) {
  auto params = getReadURIParams(column_names, storage_snapshot, query_info, local_context, processed_stage, max_block_size);
  auto read_from_format_info = prepareReadingFromFormat(
  ...
  auto reading =
      std::make_unique<ReadFromURL>(column_names, query_info, storage_snapshot, local_context, read_from_format_info.source_header,
                                    std::move(this_ptr), &uri_options, std::move(read_from_format_info), need_only_count, std::move(params),
                                    std::move(read_post_data_callback), max_block_size, num_streams);
...
SinkToStoragePtr IStorageURLBase::write(const ASTPtr &query, const StorageMetadataPtr &metadata_snapshot, ContextPtr context,
  ...
  if (is_partitioned_implementation) {
    auto partition_strategy =
        PartitionStrategyFactory::get(PartitionStrategyFactory::StrategyType::WILDCARD, partition_by_ast,
                                      metadata_snapshot->getColumns().getAll(), context, format_name, urlWithGlobs(uri), has_wildcards,
                                      /* partition_columns_in_data_file */ true);

    return std::make_shared<PartitionedStorageURLSink>(partition_strategy, uri, format_name, format_settings,
                                                       metadata_snapshot->getSampleBlock(), context, getHTTPTimeouts(context),
                                                       compression_method, headers, http_method);
  }

  return std::make_shared<StorageURLSink>(uri, format_name, format_settings, metadata_snapshot->getSampleBlock(), context,
                                          getHTTPTimeouts(context), compression_method, headers, http_method);
}
...
std::optional<time_t> IStorageURLBase::tryGetLastModificationTime(const String &url, const HTTPHeaderEntries &headers,
...
StorageURL::StorageURL(const String &uri_, const StorageID &table_id_, const String &format_name_,
                       const std::optional<FormatSettings> &format_settings_, const ColumnsDescription &columns_,
                       const ConstraintsDescription &constraints_, const String &comment, const ContextPtr &context_,
                       const String &compression_method_, const HTTPHeaderEntries &headers_, const String &http_method_,
                       ASTPtr partition_by_, bool distributed_processing_)
    : IStorageURLBase(uri_, context_, table_id_, format_name_, format_settings_, columns_, constraints_, comment, compression_method_,
                      headers_, http_method_, partition_by_, distributed_processing_) {
  context_->getRemoteHostFilter().checkURL(Poco::URI(uri));
  context_->getHTTPHeaderFilter().checkAndNormalizeHeaders(headers);
}

StorageURLWithFailover::StorageURLWithFailover(const std::vector<String> &uri_options_, const StorageID &table_id_,
                                               const String &format_name_, const std::optional<FormatSettings> &format_settings_,
                                               const ColumnsDescription &columns_, const ConstraintsDescription &constraints_,
                                               const ContextPtr &context_, const String &compression_method_)
...
size_t StorageURL::evalArgsAndCollectHeaders(ASTs &url_function_args, HTTPHeaderEntries &header_entries, const ContextPtr &context,
  ...
  for (auto arg_it = url_function_args.begin(); arg_it != url_function_args.end(); ++arg_it) {
    ...
    if (headers_ast_function && headers_ast_function->name == "headers") {
      if (headers_it != url_function_args.end())
        throw Exception(ErrorCodes::BAD_ARGUMENTS, "URL table function can have only one key-value argument: headers=(). {}",
      ...
      for (auto &header_arg : headers_function_args) {
        ...
        if (header_args.size() != 2)
          throw Exception(ErrorCodes::BAD_ARGUMENTS, "Headers argument is incorrect: expected 2 arguments, got {}", header_args.size());
    ...
    if (headers_ast_function && headers_ast_function->name == "equals") continue;

    if (evaluate_arguments) (*arg_it) = evaluateConstantExpressionOrIdentifierAsLiteral((*arg_it), context);
  }

  if (headers_it == url_function_args.end()) return url_function_args.size();
...
void StorageURL::processNamedCollectionResult(Configuration &configuration, const NamedCollection &collection) {
  ...
  if (!configuration.http_method.empty() && configuration.http_method != Poco::Net::HTTPRequest::HTTP_POST &&
    ...
    throw Exception(ErrorCodes::BAD_ARGUMENTS, "HTTP method can be POST or PUT (current: {}). For insert default is POST, for select GET",
...
StorageURL::Configuration StorageURL::getConfiguration(ASTs &args, const ContextPtr &local_context, const StorageID *table_id) {
  ...
  if (auto named_collection = tryGetNamedCollectionWithOverrides(args, local_context, true, nullptr, table_id)) {
    ...
    if (count == 0 || count > 3) throw Exception(ErrorCodes::NUMBER_OF_ARGUMENTS_DOESNT_MATCH, bad_arguments_error_message);

    configuration.url = checkAndGetLiteralArgument<String>(args[0], "url");
    if (count > 1) configuration.format = checkAndGetLiteralArgument<String>(args[1], "format");
    if (count == 3) configuration.compression_method = checkAndGetLiteralArgument<String>(args[2], "compression_method");
  }
  ...
  for (const auto &[header, value] : configuration.headers) {
    if (header == "Range") throw Exception(ErrorCodes::BAD_ARGUMENTS, "Range headers are not allowed");
  }
...
void registerStorageURL(StorageFactory &factory) {
  factory.registerStorage("URL",
                          [](const StorageFactory::Arguments &args) {
                            ...
                            ASTPtr partition_by;
                            if (args.storage_def->partition_by) partition_by = args.storage_def->partition_by->clone();

                            return std::make_shared<StorageURL>(configuration.url, args.table_id, configuration.format, format_settings,
                                                                args.columns, args.constraints, args.comment, context,
                                                                configuration.compression_method, configuration.headers,
                                                                configuration.http_method, partition_by);
                          },
                          {
