...
VirtualColumnDescription::VirtualColumnDescription(String name_, DataTypePtr type_, ASTPtr codec_, String comment_, VirtualsKind kind_)
    : ColumnDescription(std::move(name_), std::move(type_), std::move(codec_), std::move(comment_)), kind(kind_) {}

void VirtualColumnsDescription::add(VirtualColumnDescription desc) {
  if (container.get<1>().contains(desc.name)) throw Exception(ErrorCodes::DUPLICATE_COLUMN, "Virtual column {} already exists", desc.name);
...
std::optional<ColumnDefault> VirtualColumnsDescription::getDefault(const String& column_name) const {
  ...
  if (it != container.get<1>().end() && it->default_desc.expression) return it->default_desc;
  return {};
}
...
NameAndTypePair VirtualColumnsDescription::get(const String& name, VirtualsKind kind) const {
  ...
  if (!column) throw Exception(ErrorCodes::NO_SUCH_COLUMN_IN_TABLE, "There is no virtual column {}", name);
  return *column;
}

const VirtualColumnDescription* VirtualColumnsDescription::tryGetDescription(const String& name, VirtualsKind kind) const {
  auto it = container.get<1>().find(name);
  if (it != container.get<1>().end() && (static_cast<UInt8>(it->kind) & static_cast<UInt8>(kind))) return &(*it);
  return nullptr;
}
...
Block VirtualColumnsDescription::getSampleBlock() const {
  Block result;
  for (const auto& desc : container) result.insert({desc.type->createColumn(), desc.type, desc.name});
  return result;
}

NamesAndTypesList VirtualColumnsDescription::getNamesAndTypesList() const {
  NamesAndTypesList result;
  for (const auto& desc : container) result.emplace_back(desc.name, desc.type);
  return result;
}
...
NamesAndTypesList VirtualColumnsDescription::getNamesAndTypesList(VirtualsKind kind) const {
  ...
  for (const auto& column : container)
    if (static_cast<UInt8>(column.kind) & static_cast<UInt8>(kind)) result.emplace_back(column.name, column.type);
  return result;
}
