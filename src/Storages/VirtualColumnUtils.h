...
///
/// Similar to filterBlockWithExpression(buildFilterExpression(splitFilterDagForAllowedInputs(...)))./// Similar to filterBlockWithQuery,
/// but uses ActionsDAG as a predicate. Basically it is filterBlockWithDAG(splitFilterDagForAllowedInputs). If
/// allow_filtering_with_partial_predicate is true, then the filtering will be done even if some part of the predicate cannot be evaluated
/// using the columns from the block.
void filterBlockWithPredicate(const ActionsDAG::Node *predicate, Block &block, ContextPtr context,
                              bool allow_filtering_with_partial_predicate = true);
...
std::optional<ActionsDAG> splitFilterDagForAllowedInputs(const ActionsDAG::Node *predicate, const Block *allowed_inputs,
                                                         const ContextPtr &context, bool allow_partial_result = true);
...
auto extractSingleValueFromBlock(const Block &block, const String &name) {
  ...
  for (size_t i = 0; i < rows; ++i) res.insert((*data.column)[i].safeGet<T>());
  return res;
}

NameSet getVirtualNamesForFileLikeStorage();
VirtualColumnsDescription getVirtualsForFileLikeStorage(
    ColumnsDescription &storage_columns, ContextPtr context, const std::optional<FormatSettings> &format_settings = std::nullopt,
    std::optional<PartitionStrategyFactory::StrategyType> partition_strategy = std::nullopt, const std::string &path = "");
...
template <typename T>
void filterByPathOrFile(std::vector<T> &sources, const std::vector<String> &paths, const ExpressionActionsPtr &actions,
  ...
  if (indexes.size() == sources.size()) return;

  std::vector<T> filtered_sources;
  filtered_sources.reserve(indexes.size());
  for (auto index : indexes) filtered_sources.emplace_back(std::move(sources[index]));
