...
FileLogConsumer::FileLogConsumer(StorageFileLog& storage_, size_t max_batch_size, size_t poll_timeout_, ContextPtr context_,
                                 size_t stream_number_, size_t max_streams_number_)
...
ReadBufferPtr FileLogConsumer::consume() {
  if (hasMorePolledRecords()) return getNextRecord();
...
FileLogConsumer::Records FileLogConsumer::pollBatch(size_t batch_size_) {
  ...
  if (new_records.size() == batch_size_ || stream_out) return new_records;
...
void FileLogConsumer::readNewRecords(FileLogConsumer::Records& new_records, size_t batch_size_) {
  ...
  for (size_t i = start; i < end; ++i) {
    ...
    if (file_ctx.status == StorageFileLog::FileStatus::NO_CHANGE) continue;

    auto& file_meta = StorageFileLog::findInMap(file_infos.meta_by_inode, file_ctx.inode);

    if (!file_ctx.reader) throw Exception(ErrorCodes::CANNOT_READ_ALL_DATA, "Ifstream for file {} is not initialized", file_meta.file_name);
    ...
    while (read_records_size < need_records_size) {
      ...
      if (static_cast<UInt64>(offset) >= file_meta.last_open_end) break;
...
ReadBufferPtr FileLogConsumer::getNextRecord() {
  if (!hasMorePolledRecords()) return nullptr;
