...
#define FILELOG_RELATED_SETTINGS(DECLARE, ALIAS)                                                                                          \
  ...
  DECLARE(MaxThreads, max_threads, 0,                                                                                                     \
          "Number of max threads to parse files, default is 0, which means the number will be max(1, physical_cpu_cores / 4)", 0)         \
  ...
  DECLARE(StreamingHandleErrorMode, handle_error_mode, StreamingHandleErrorMode::DEFAULT,                                                 \
          "How to handle errors for FileLog engine. Possible values: default (throw an exception after nats_skip_broken_messages broken " \
...
void FileLogSettings::loadFromQuery(ASTStorage& storage_def) {
  ...
  if (impl->poll_max_batch_size > max_sane_block_rows_size)
    throw Exception(ErrorCodes::INVALID_SETTING_VALUE, "Sanity check: 'poll_max_batch_size' value is too high ({})",
                    impl->poll_max_batch_size.value);
}

bool FileLogSettings::hasBuiltin(std::string_view name) { return FileLogSettingsImpl::hasBuiltin(name); }
}  // namespace DB
