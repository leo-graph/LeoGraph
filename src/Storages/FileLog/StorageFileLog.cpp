...
class ReadFromStorageFileLog final : public ReadFromStreamLikeEngine {
 public:
  ReadFromStorageFileLog(const Names &column_names_, StoragePtr storage_, const StorageSnapshotPtr &storage_snapshot_,
                         SelectQueryInfo &query_info, ContextPtr context_)
 ...
 private:
  Pipe makePipe() final {
    ...
    if (file_log.file_infos.file_names.empty()) {
      ...
      for (const auto &[name, type] : column_names_and_types) header.insert(ColumnWithTypeAndName(type, name));
    ...
    auto max_streams_number =
        std::min<UInt64>((*file_log.filelog_settings)[FileLogSetting::max_threads], file_log.file_infos.file_names.size());
    ...
    for (size_t stream_number = 0; stream_number < max_streams_number; ++stream_number) {
      pipes.emplace_back(std::make_shared<FileLogSource>(
          file_log, storage_snapshot, modified_context, column_names, file_log.getMaxBlockSize(), file_log.getPollTimeoutMillisecond(),
          stream_number, max_streams_number, (*file_log.filelog_settings)[FileLogSetting::handle_error_mode]));
    }
...
StorageFileLog::StorageFileLog(const StorageID &table_id_, ContextPtr context_, const ColumnsDescription &columns_, const String &path_,
  ...
  if (!fileOrSymlinkPathStartsWith(path, getContext()->getUserFilesPath())) {
    ...
    throw Exception(ErrorCodes::BAD_ARGUMENTS, "The absolute data path should be inside `user_files_path`({})",
  ...
  try {
    if (mode < LoadingStrictnessLevel::ATTACH) {
      if (disk->existsDirectory(metadata_base_path)) {
        throw Exception(ErrorCodes::TABLE_METADATA_ALREADY_EXISTS,
                        "Metadata files already exist by path: {}, remove them manually if it is intended", metadata_base_path);
      }
    ...
    if (path_is_directory) directory_watch = std::make_unique<FileLogDirectoryWatcher>(root_data_path, *this, getContext());
    ...
    if (mode <= LoadingStrictnessLevel::ATTACH) {
      if (created_metadata_directory) disk->removeRecursive(metadata_base_path);
      throw;
    }
...
void StorageFileLog::loadFiles() {
  ...
  if (file_infos.meta_by_inode.size() > file_infos.context_by_name.size()) {
    ...
    for (const auto &[inode, meta] : file_infos.meta_by_inode) {
      ...
      if (auto it = file_infos.context_by_name.find(meta.file_name); it != file_infos.context_by_name.end() && it->second.inode == inode)
...
void StorageFileLog::serialize() const {
  for (const auto &[inode, meta] : file_infos.meta_by_inode) serialize(inode, meta);
}
...
void StorageFileLog::deserialize() {
  if (!disk->existsDirectory(metadata_base_path)) return;
  ...
  for (const auto &file : files_to_remove) disk->removeFile(file);
}
...
void StorageFileLog::read(QueryPlan &query_plan, const Names &column_names, const StorageSnapshotPtr &storage_snapshot,
                          SelectQueryInfo &query_info, ContextPtr query_context, QueryProcessingStage::Enum /* processed_stage */,
...
void StorageFileLog::increaseStreams() { running_streams += 1; }
...
void StorageFileLog::startup() {
  if (task) task->holder->activateAndSchedule();
}
  ...
  for (const auto &file : file_infos.file_names) {
    ...
    if (file_ctx.status != FileStatus::NO_CHANGE) {
      ...
      if (meta.last_writen_position > static_cast<UInt64>(file_end)) {
        throw Exception(ErrorCodes::CANNOT_READ_ALL_DATA, "Last saved offsset for File {} is bigger than the file size ({} > {})", file,
                        meta.last_writen_position, std::streamoff{file_end});
      }
...
void StorageFileLog::closeFilesAndStoreMeta(size_t start, size_t end) {
  ...
  for (size_t i = start; i < end; ++i) {
    ...
    if (file_ctx.reader) {
      if (file_ctx.reader->is_open()) file_ctx.reader->close();
    }
...
void StorageFileLog::checkOffsetIsValid(const String &filename, UInt64 offset) const {
  ...
  if (metadata.last_writen_position > offset) {
    throw Exception(ErrorCodes::LOGICAL_ERROR,
                    "Last stored last_written_position in meta file {} is bigger than current last_written_pos ({} > {})", filename,
                    metadata.last_writen_position, offset);
  }
}
...
StorageFileLog::ReadMetadataResult StorageFileLog::readMetadata(const String &filename) const {
  ...
  if (!disk->existsFile(full_path)) {
    throw Exception(ErrorCodes::BAD_FILE_TYPE, "The file {} under {} is not a regular file", filename, metadata_base_path);
  }
  ...
  if (!tryReadIntText(inode, *in)) throw Exception(ErrorCodes::CANNOT_READ_ALL_DATA, "Read meta file {} failed (1)", full_path);

  if (!checkChar('\n', *in)) throw Exception(ErrorCodes::CANNOT_READ_ALL_DATA, "Read meta file {} failed (2)", full_path);

  if (!tryReadIntText(last_written_pos, *in)) throw Exception(ErrorCodes::CANNOT_READ_ALL_DATA, "Read meta file {} failed (3)", full_path);
...
size_t StorageFileLog::getPollMaxBatchSize() const {
  size_t batch_size = (*filelog_settings)[FileLogSetting::poll_max_batch_size].changed
                          ? (*filelog_settings)[FileLogSetting::poll_max_batch_size].value
                          : getContext()->getSettingsRef()[Setting::max_block_size].value;
  return std::min(batch_size, getMaxBlockSize());
}

size_t StorageFileLog::getPollTimeoutMillisecond() const {
  return (*filelog_settings)[FileLogSetting::poll_timeout_ms].changed
             ? (*filelog_settings)[FileLogSetting::poll_timeout_ms].totalMilliseconds()
  ...
  if (!task->stream_cancelled) {
    if (path_is_directory) {
      if (!getTableDependentCount() || reschedule)
        ...
        if (task->stream_cancelled) return;
...
bool StorageFileLog::streamToViews() {
  ...
  pipes.reserve(max_streams_number);
  for (size_t stream_number = 0; stream_number < max_streams_number; ++stream_number) {
    pipes.emplace_back(std::make_shared<FileLogSource>(*this, storage_snapshot, new_context, block_io.pipeline.getHeader().getNames(),
                                                       getPollMaxBatchSize(), getPollTimeoutMillisecond(), stream_number,
                                                       max_streams_number, (*filelog_settings)[FileLogSetting::handle_error_mode]));
  }
...
void registerStorageFileLog(StorageFactory &factory) {
  auto creator_fn = [](const StorageFactory::Arguments &args) {
    ...
    if ((*filelog_settings)[FileLogSetting::poll_max_batch_size].changed &&
        (*filelog_settings)[FileLogSetting::poll_max_batch_size].value < 1) {
    ...
    if ((*filelog_settings)[FileLogSetting::handle_error_mode].changed &&
        (*filelog_settings)[FileLogSetting::handle_error_mode].value == StreamingHandleErrorMode::DEAD_LETTER_QUEUE)
    ...
    return std::make_shared<StorageFileLog>(args.table_id, args.getContext(), args.columns, path, args.relative_data_path, format,
                                            std::move(filelog_settings), args.comment, args.mode);
  };

  factory.registerStorage("FileLog", creator_fn,
...
bool StorageFileLog::updateFileInfos() {
  if (file_infos.file_names.empty()) return false;
  ...
  if (error.has_error) LOG_ERROR(log, "Error happened during watching directory {}: {}", directory_watch->getPath(), error.error_msg);
  ...
  for (const auto &file_name : file_infos.file_names) {
    if (auto it = file_infos.context_by_name.find(file_name); it != file_infos.context_by_name.end()) {
      if (it->second.status == FileStatus::REMOVED) {
        ...
        if (auto meta = file_infos.meta_by_inode.find(inode); meta != file_infos.meta_by_inode.end() && meta->second.file_name == file_name)
          file_infos.meta_by_inode.erase(meta);

        if (std::filesystem::exists(getFullMetaPath(file_name))) (void)std::filesystem::remove(getFullMetaPath(file_name));
