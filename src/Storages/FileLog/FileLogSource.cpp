...
FileLogSource::FileLogSource(StorageFileLog& storage_, const StorageSnapshotPtr& storage_snapshot_, const ContextPtr& context_,
                             const Names& columns, size_t max_block_size_, size_t poll_time_out_, size_t stream_number_,
                             size_t max_streams_number_, StreamingHandleErrorMode handle_error_mode_)
...
Chunk FileLogSource::generate() {
  ...
  while (true) {
    ...
    if (!consumer->hasMorePolledRecords() && ((total_rows >= max_block_size) || watch.elapsedMilliseconds() > poll_time_out ||
