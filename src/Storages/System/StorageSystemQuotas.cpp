...
ColumnsDescription StorageSystemQuotas::getColumnsDescription() {
  return ColumnsDescription{
      ...
      {"storage", std::make_shared<DataTypeString>(),
       "Storage of quotas. Possible value: 'users.xml' if a quota configured in the users.xml file, 'disk' if a quota configured by an "
       "SQL-query."},
      {"keys", std::make_shared<DataTypeArray>(std::make_shared<DataTypeEnum8>(getKeyTypeEnumValues())),
       "Key specifies how the quota should be shared. If two connections use the same quota and key, they share the same amounts of "
       "resources. Values: "
       ...
       "['user_name', 'client_key'] — Connections with the same client_key share the same quota. If a key isn't provided by a client, the "
       "quota is tracked for `user_name`, "
       "['client_key', 'ip_address'] — Connections with the same client_key share the same quota. If a key isn't provided by a client, the "
       "quota is tracked for ip_address."},
      ...
      {"apply_to_list", std::make_shared<DataTypeArray>(std::make_shared<DataTypeString>()),
       "List of user names/roles that the quota should be applied to."},
      {"apply_to_except", std::make_shared<DataTypeArray>(std::make_shared<DataTypeString>()),
       "List of user names/roles that the quota should not apply to."}};
}
...
void StorageSystemQuotas::fillData(MutableColumns &res_columns, ContextPtr context, const ActionsDAG::Node *, std::vector<UInt8>) const {
  ...
  if (!access_control.doesSelectFromSystemDatabaseRequireGrant()) context->checkAccess(AccessType::SHOW_QUOTAS);
  ...
  auto add_row = [&](const String &name, const UUID &id, const String &storage_name, const std::vector<Quota::Limits> &all_limits,
                     QuotaKeyType key_type, const RolesOrUsersSet &apply_to) {
    ...
    if (key_type != QuotaKeyType::NONE) {
      const auto &type_info = QuotaKeyTypeInfo::get(key_type);
      for (auto base_type : type_info.base_types) column_key_types.push_back(static_cast<Int8>(base_type));
      if (type_info.base_types.empty()) column_key_types.push_back(static_cast<Int8>(key_type));
    }
    column_key_types_offsets.push_back(column_key_types.size());

    for (const auto &limits : all_limits) {
      column_durations.push_back(static_cast<UInt32>(std::chrono::duration_cast<std::chrono::seconds>(limits.duration).count()));
    }
    ...
    for (const auto &role_name : apply_to_ast->names) column_apply_to_list.insertData(role_name.data(), role_name.length());
    column_apply_to_list_offsets.push_back(column_apply_to_list.size());

    for (const auto &role_name : apply_to_ast->except_names) column_apply_to_except.insertData(role_name.data(), role_name.length());
...
void StorageSystemQuotas::backupData(BackupEntriesCollector &backup_entries_collector, const String &data_path_in_backup,
...
void StorageSystemQuotas::restoreDataFromBackup(RestorerFromBackup &restorer, const String &data_path_in_backup,
                                                const std::optional<ASTs> & /* partitions */) {
