...
StorageSystemReplicas::StorageSystemReplicas(const StorageID &table_id_) : IStorage(table_id_), pools(std::make_shared<TPools>(128)) {
  ColumnsDescription description = {
      ...
      {"is_leader", std::make_shared<DataTypeUInt8>(),
       "Whether the replica is the leader. Multiple replicas can be leaders at the same time. "
      ...
      {"is_readonly", std::make_shared<DataTypeUInt8>(),
       "Whether the replica is in read-only mode. This mode is turned on if the config does not have sections with ClickHouse Keeper, "
       "if an unknown error occurred when reinitializing sessions in ClickHouse Keeper, and during session reinitialization in ClickHouse "
       "Keeper."},
      {"readonly_start_time", std::make_shared<DataTypeNullable>(std::make_shared<DataTypeDateTime>()),
       "The timestamp when the replica transitioned into readonly mode. Null if the replica is not in readonly mode."},
      {"is_session_expired", std::make_shared<DataTypeUInt8>(),
       "Whether the session with ClickHouse Keeper has expired. Basically the same as `is_readonly`."},
      ...
      {"parts_to_check", std::make_shared<DataTypeUInt32>(),
       "The number of data parts in the queue for verification. A part is put in the verification queue if there is suspicion that it "
       "might be damaged."},
      {"zookeeper_name", std::make_shared<DataTypeString>(),
       "The name of the the [Zoo]Keeper cluster (possibly auxiliary one) where the table's metadata is stored"},
      ...
      {"columns_version", std::make_shared<DataTypeInt32>(),
       "Version number of the table structure. Indicates how many times ALTER was performed. "
      ...
      {"queue_size", std::make_shared<DataTypeUInt32>(),
       "Size of the queue for operations waiting to be performed. Operations include inserting blocks of data, merges, and certain other "
       "actions. It usually coincides with future_parts."},
      {"inserts_in_queue", std::make_shared<DataTypeUInt32>(),
       "Number of inserts of blocks of data that need to be made. Insertions are usually replicated fairly quickly. If this number is "
       "large, it means something is wrong."},
      ...
      {"queue_oldest_time", std::make_shared<DataTypeDateTime>(),
       "If `queue_size` greater than 0, shows when the oldest operation was added to the queue."},
      ...
      {"oldest_part_to_get", std::make_shared<DataTypeString>(),
       "The name of the part to fetch from other replicas obtained from the oldest GET_PARTS entry in the replication queue."},
      {"oldest_part_to_merge_to", std::make_shared<DataTypeString>(),
       "The result part name to merge to obtained from the oldest MERGE_PARTS entry in the replication queue."},
      {"oldest_part_to_mutate_to", std::make_shared<DataTypeString>(),
       "The result part name to mutate to obtained from the oldest MUTATE_PARTS entry in the replication queue."},
      ...
      {"log_pointer", std::make_shared<DataTypeUInt64>(),
       "Maximum entry number in the log of general activity that the replica copied to its execution queue, plus one. "
      ...
      {"active_replicas", std::make_shared<DataTypeUInt32>(),
      ...
      {"lost_part_count", std::make_shared<DataTypeUInt64>(),
       "The number of data parts lost in the table by all replicas in total since table creation. Value is persisted in ClickHouse Keeper "
       "and can only increase."},
      {"last_queue_update_exception", std::make_shared<DataTypeString>(),
       "When the queue contains broken entries. Especially important when ClickHouse breaks backward compatibility between versions and "
       "log entries written by newer versions aren't parseable by old versions."},
      ...
      {"replica_is_active", std::make_shared<DataTypeMap>(std::make_shared<DataTypeString>(), std::make_shared<DataTypeUInt8>()),
       "Map between replica name and is replica active."}};

  description.setAliases({
      {"readonly_duration", std::make_shared<DataTypeNullable>(std::make_shared<DataTypeUInt64>()),
       "if(isNull(readonly_start_time), NULL, now() - readonly_start_time)"},
  });
...
class ReadFromSystemReplicas : public SourceStepWithFilter {
 public:
  ...
  ReadFromSystemReplicas(const Names &column_names_, const SelectQueryInfo &query_info_, const StorageSnapshotPtr &storage_snapshot_,
                         const ContextPtr &context_, Block sample_block, std::map<String, std::map<String, StoragePtr>> replicated_tables_,
                         bool with_zk_fields_, size_t max_block_size_, std::shared_ptr<StorageSystemReplicas::TPools> pools_)
...
void ReadFromSystemReplicas::applyFilters(ActionDAGNodes added_filter_nodes) {
  ...
  if (filter_actions_dag) {
    ...
    if (dag) virtual_columns_filter = VirtualColumnUtils::buildFilterExpression(std::move(*dag), context);
  }
}

void StorageSystemReplicas::read(QueryPlan &query_plan, const Names &column_names, const StorageSnapshotPtr &storage_snapshot,
  ...
  for (const auto &db : DatabaseCatalog::instance().getDatabases(GetDatabasesOptions{.with_datalake_catalogs = false})) {
    ...
    for (auto iterator = db.second->getTablesIterator(context); iterator->isValid(); iterator->next()) {
      ...
      if (!dynamic_cast<const StorageReplicatedMergeTree *>(table.get())) continue;
      if (check_access_for_tables && !access->isGranted(AccessType::SHOW_TABLES, db.first, iterator->name())) continue;
  ...
  for (const auto &column_name : column_names) {
    if (column_name == "log_max_index" || column_name == "log_pointer" || column_name == "total_replicas" ||
        column_name == "active_replicas" || column_name == "lost_part_count" || column_name == "zookeeper_exception" ||
  ...
  auto reading = std::make_unique<ReadFromSystemReplicas>(column_names, query_info, storage_snapshot, std::move(context), std::move(header),
                                                          std::move(replicated_tables), with_zk_fields, max_block_size, pools);
...
class SystemReplicasSource : public ISource {
 public:
  SystemReplicasSource(SharedHeader header_, size_t max_block_size_, ColumnPtr col_database_, ColumnPtr col_table_, ColumnPtr col_uuid_,
                       ColumnPtr col_engine_, std::vector<TFuture> futures_, ContextPtr context_)
      : ISource(header_),
        max_block_size(max_block_size_),
...
void ReadFromSystemReplicas::initializePipeline(QueryPipelineBuilder &pipeline, const BuildQueryPipelineSettings &) {
  ...
  /// Determine what tables are needed by the conditions in the query.
  {
    ...
    if (virtual_columns_filter) VirtualColumnUtils::filterBlockWithExpression(virtual_columns_filter, filtered_block);
  ...
  for (size_t i = 0; i < tables_size; ++i) {
    ...
    auto &storage = replicated_tables[(*col_database)[i].safeGet<String>()][(*col_table)[i].safeGet<String>()];
  ...
  pipeline.init(Pipe(std::make_shared<SystemReplicasSource>(header, max_block_size, col_database, col_table, col_uuid, col_engine,
                                                            std::move(futures), context)));
}

Chunk SystemReplicasSource::generate() {
  if (i == futures.size()) return {};
  ...
  for (; i < futures.size(); ++i) {
    ...
    if (rows_added) {
      ...
      if (futures[i].wait_for(std::chrono::seconds(0)) != std::future_status::ready) break;

      if (max_block_size != 0) {
        size_t total_size = 0;
        for (const auto &column : res_columns) total_size += column->byteSize();
        /// If the block size exceeds the maximum, return the current block
        if (total_size >= max_block_size) break;
      }
    }
    ...
    try {
      ...
      if (e.code() == ErrorCodes::ABORTED) {
        tryLogCurrentException(
            logger, "Received the ABORTED error while trying to get the status of a storage, this is likely because it has been shut down");
        continue;
      }
      throw;
    }
