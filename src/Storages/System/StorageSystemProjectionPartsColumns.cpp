...
void StorageSystemProjectionPartsColumns::processNextStorage(ContextPtr, MutableColumns& columns, std::vector<UInt8>& columns_mask,
  ...
  for (size_t part_number = 0; part_number < all_parts.projection_parts.size(); ++part_number) {
    ...
    for (const auto& column : part->getColumns()) {
      ...
      if (columns_mask[src_index++]) columns[res_index++]->insert(UInt64(parent_part->info.getDataVersion()));
