    ...
    : StorageSystemPartsBase(
          ...
          ColumnsDescription{
              ...
              {"part_type", std::make_shared<DataTypeString>(),
               ...
               "Possible values: Wide — Each column is stored in a separate file in a filesystem, Compact — All columns are stored in one "
               "file in a filesystem."},
              {"active", std::make_shared<DataTypeUInt8>(),
               "Flag that indicates whether the data part is active. If a data part is active, it's used in a table. Otherwise, it's "
               "deleted. Inactive data parts remain after merging."},
              ...
              {"column_data_compressed_bytes", std::make_shared<DataTypeUInt64>(),
               "Total size of the compressed data in the column, in bytes."},
              ...
              {"column_ttl_min", std::make_shared<DataTypeNullable>(std::make_shared<DataTypeDateTime>()),
               "The minimum value of the calculated TTL expression of the column."},
              {"column_ttl_max", std::make_shared<DataTypeNullable>(std::make_shared<DataTypeDateTime>()),
               "The maximum value of the calculated TTL expression of the column."},
              {"statistics", std::make_shared<DataTypeArray>(std::make_shared<DataTypeString>()), "The statistics of the column."},
              {"estimates.min", std::make_shared<DataTypeNullable>(std::make_shared<DataTypeFloat64>()),
               "Estimated minimum value of the column."},
              {"estimates.max", std::make_shared<DataTypeNullable>(std::make_shared<DataTypeFloat64>()),
               "Estimated maximum value of the column."},
              {"estimates.cardinality", std::make_shared<DataTypeNullable>(std::make_shared<DataTypeUInt64>()),
               "Estimated cardinality of the column."},
              ...
              {"substreams", std::make_shared<DataTypeArray>(std::make_shared<DataTypeString>()),
               "Names of substreams to which column is serialized"},
              {"filenames", std::make_shared<DataTypeArray>(std::make_shared<DataTypeString>()),
               "Names of files for each substream of a column respectively"},
              ...
              {"subcolumns.serializations", std::make_shared<DataTypeArray>(std::make_shared<DataTypeString>()),
               "Kinds of serialization of subcolumns of a column"},
              {"subcolumns.bytes_on_disk", std::make_shared<DataTypeArray>(std::make_shared<DataTypeUInt64>()),
               "Sizes in bytes for each subcolumn"},
              {"subcolumns.data_compressed_bytes", std::make_shared<DataTypeArray>(std::make_shared<DataTypeUInt64>()),
               "Sizes of the compressed data for each subcolumn, in bytes"},
              {"subcolumns.data_uncompressed_bytes", std::make_shared<DataTypeArray>(std::make_shared<DataTypeUInt64>()),
               "Sizes of the decompressed data for each subcolumn, in bytes"},
              {"subcolumns.marks_bytes", std::make_shared<DataTypeArray>(std::make_shared<DataTypeUInt64>()),
               "Sizes of the marks for each subcolumn of a column, in bytes"},
          }) {}

void StorageSystemPartsColumns::processNextStorage(ContextPtr, MutableColumns& columns, std::vector<UInt8>& columns_mask,
  ...
  for (size_t part_number = 0; part_number < all_parts.size(); ++part_number) {
    ...
    auto find_estimate = [&](const auto& column_name) {
      if (!estimates.has_value()) estimates = part->getEstimates();
    ...
    for (const auto& column : part->getColumns()) {
      ...
      if (columns_mask[src_index++]) columns[res_index++]->insert(columns_size.data_compressed);
      if (columns_mask[src_index++]) columns[res_index++]->insert(columns_size.data_uncompressed);
      if (columns_mask[src_index++]) columns[res_index++]->insert(columns_size.marks);
      if (columns_mask[src_index++]) columns[res_index++]->insert(UInt64(part->modification_time));
      if (columns_mask[src_index++]) columns[res_index++]->insert(UInt64(part->remove_time.load(std::memory_order_relaxed)));

      if (columns_mask[src_index++]) columns[res_index++]->insert(UInt64(use_count));
      ...
      if (columns_mask[src_index++]) columns[res_index++]->insert(UInt64(part->info.getDataVersion()));
      if (columns_mask[src_index++]) columns[res_index++]->insert(index_size_in_bytes);
      if (columns_mask[src_index++]) columns[res_index++]->insert(index_size_in_allocated_bytes);
      ...
      if (columns_mask[src_index++]) columns[res_index++]->insert(column.name);
      if (columns_mask[src_index++]) columns[res_index++]->insert(column.type->getName());
      if (columns_mask[src_index++]) columns[res_index++]->insert(column_position);

      auto column_info_it = columns_info.find(column.name);
      if (column_info_it != columns_info.end()) {
        if (columns_mask[src_index++]) columns[res_index++]->insert(column_info_it->second.default_kind);
        if (columns_mask[src_index++]) columns[res_index++]->insert(column_info_it->second.default_expression);
      } else {
        if (columns_mask[src_index++]) columns[res_index++]->insertDefault();
        if (columns_mask[src_index++]) columns[res_index++]->insertDefault();
      }

      ColumnSize column_size = part->getColumnSize(column.name);
      if (columns_mask[src_index++]) columns[res_index++]->insert(column_size.data_compressed + column_size.marks);
      if (columns_mask[src_index++]) columns[res_index++]->insert(column_size.data_compressed);
      if (columns_mask[src_index++]) columns[res_index++]->insert(column_size.data_uncompressed);
      if (columns_mask[src_index++]) columns[res_index++]->insert(column_size.marks);
      if (columns_mask[src_index++]) {
      ...
      if (columns_mask[src_index++]) {
        ...
        if (estimate_it != estimates->end()) {
          Array types;
          for (const auto& type : estimate_it->second.types) types.push_back(toString(type));
      ...
      if (columns_mask[src_index++]) columns[res_index++]->insert(ISerialization::kindStackToString(serialization->getKindStack()));
      ...
      if (column.type->hasDynamicSubcolumns() && !part->getColumnsSubstreams().empty()) {
        ...
        serialization->enumerateStreams([&](const auto& subpath) {
          auto substream =
              ISerialization::getFileNameForStream(column.name, subpath, ISerialization::StreamFileNameSettings(*info.data->getSettings()));
          auto filename =
              IMergeTreeDataPart::getStreamNameForColumn(column.name, subpath, ".bin", part->checksums, info.data->getSettings());
      ...
      if (columns_mask[src_index++]) columns[res_index++]->insert(substreams);
      if (columns_mask[src_index++]) columns[res_index++]->insert(filenames);
      ...
      IDataType::forEachSubcolumn(
          [&](const auto& subpath, const auto& name, const auto& data) {
            ...
            if (isTuple(data.type) || isNested(data.type)) return;

            /// Skip ephemeral subcolumns that don't store any real data.
            if (ISerialization::isEphemeralSubcolumn(subpath, subpath.size())) return;
            ...
            auto stream_name =
                IMergeTreeDataPart::getStreamNameForColumn(subcolumn, subpath, ".bin", part->checksums, info.data->getSettings());
            if (stream_name) {
              ...
              if (mrk_checksum != part->checksums.files.end()) size.marks += mrk_checksum->second.file_size;
            }
      ...
      if (columns_mask[src_index++]) columns[res_index++]->insert(subcolumn_names);
      if (columns_mask[src_index++]) columns[res_index++]->insert(subcolumn_types);
      if (columns_mask[src_index++]) columns[res_index++]->insert(subcolumn_serializations);
      if (columns_mask[src_index++]) columns[res_index++]->insert(subcolumn_bytes_on_disk);
      if (columns_mask[src_index++]) columns[res_index++]->insert(subcolumn_data_compressed_bytes);
      if (columns_mask[src_index++]) columns[res_index++]->insert(subcolumn_data_uncompressed_bytes);
      if (columns_mask[src_index++]) columns[res_index++]->insert(subcolumn_marks_bytes);

      if (has_state_column) columns[res_index++]->insert(part->stateString());
    }
  }
}
