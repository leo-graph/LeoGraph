...
void signalHandler(int, siginfo_t *info, void *context) {
  ...
  if (info->si_pid != server_pid) return;

  /// Signal received too late.
  int notification_num = info->si_value.sival_int;
  if (notification_num != sequence_num.load(std::memory_order_acquire)) return;

  bool expected = false;
  if (!signal_latch.compare_exchange_strong(expected, true, std::memory_order_acquire)) return;
  ...
  if (!query_id.empty()) memcpy(query_id_data, query_id.data(), query_id_size);
...
bool wait(int timeout_ms) {
  while (true) {
    ...
    if (read_res == sizeof(notification_num)) {
      if (notification_num == sequence_num.load(std::memory_order_relaxed)) return true;
...
ThreadIdToName getFilteredThreadNames(const ActionsDAG::Node *predicate, ContextPtr context, const PaddedPODArray<UInt64> &thread_ids,
                                      LoggerPtr log) {
  ...
  for (UInt64 tid : thread_ids) {
    ...
    try {
      ...
      /// Ignore TOCTOU error
      if (e.code() == ErrorCodes::FILE_DOESNT_EXIST) continue;
      throw;
    }
...
bool isSignalBlocked(UInt64 tid, int signal) {
  ...
  try {
    ...
    while (!status.eof()) {
      ...
      if (!status.eof()) ++status.position();
      if (buffer.starts_with("SigBlk:")) break;
    }
    status.close();
    ...
    UInt64 sig_blk;
    if (parseHexNumber(line, sig_blk)) return sig_blk & signal;
  } catch (const Exception &e) {
    /// Ignore TOCTOU error
...
class StackTraceSource : public ISource {
 public:
  StackTraceSource(const Names &column_names, SharedHeader header_, std::shared_ptr<const ActionsDAG> filter_dag_, ContextPtr context_,
                   UInt64 max_block_size_, LoggerPtr log_)
      : ISource(header_),
        context(context_),
 ...
 protected:
  Chunk generate() override {
    ...
    if (thread_ids->empty()) return Chunk();
    ...
    if (read_thread_names) thread_names = getFilteredThreadNames(predicate, context, thread_ids_data, log);
...
class ReadFromSystemStackTrace : public SourceStepWithFilter {
 public:
  ...
  void initializePipeline(QueryPipelineBuilder &pipeline, const BuildQueryPipelineSettings &) override {
    Pipe pipe(std::make_shared<StackTraceSource>(column_names, getOutputHeader(), filter_actions_dag, context, max_block_size, log));
    pipeline.init(std::move(pipe));
  }

  ReadFromSystemStackTrace(const Names &column_names_, const SelectQueryInfo &query_info_, const StorageSnapshotPtr &storage_snapshot_,
                           const ContextPtr &context_, Block sample_block, size_t max_block_size_, LoggerPtr log_)
...
StorageSystemStackTrace::StorageSystemStackTrace(const StorageID &table_id_)
    : IStorage(table_id_), log(getLogger("StorageSystemStackTrace")) {
  ...
  storage_metadata.setColumns(ColumnsDescription({
      ...
      {"trace", std::make_shared<DataTypeArray>(std::make_shared<DataTypeUInt64>()),
       "The stacktrace of this thread. Basically just an array of addresses."},
  }));
  ...
  if (sigemptyset(&sa.sa_mask)) throw ErrnoException(ErrorCodes::CANNOT_MANIPULATE_SIGSET, "Cannot set signal handler");
...
void StorageSystemStackTrace::read(QueryPlan &query_plan, const Names &column_names, const StorageSnapshotPtr &storage_snapshot,
  ...
  auto reading =
      std::make_unique<ReadFromSystemStackTrace>(column_names, query_info, storage_snapshot, context, sample_block, max_block_size, log);
