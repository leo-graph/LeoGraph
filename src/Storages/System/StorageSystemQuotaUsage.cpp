...
ColumnsDescription StorageSystemQuotaUsage::getColumnsDescriptionImpl(bool add_column_is_current) {
  ColumnsDescription description{{"quota_name", std::make_shared<DataTypeString>(), "Quota name."},
                                 {"quota_key", std::make_shared<DataTypeString>(), "Key value."}};

  if (add_column_is_current) description.add({"is_current", std::make_shared<DataTypeUInt8>(), "Quota usage for current user."});

  description.add({"start_time", std::make_shared<DataTypeNullable>(std::make_shared<DataTypeDateTime>()),
                   "Start time for calculating resource consumption."});
  description.add({"end_time", std::make_shared<DataTypeNullable>(std::make_shared<DataTypeDateTime>()),
                   "End time for calculating resource consumption."});
  description.add({"duration", std::make_shared<DataTypeNullable>(std::make_shared<DataTypeUInt32>()),
...
void StorageSystemQuotaUsage::fillData(MutableColumns &res_columns, ContextPtr context, const ActionsDAG::Node *,
...
void StorageSystemQuotaUsage::fillDataImpl(MutableColumns &res_columns, ContextPtr context, bool add_column_is_current,
  ...
  if (add_column_is_current) column_is_current = &assert_cast<ColumnUInt8 &>(*res_columns[column_index++]).getData();
  ...
  if (add_column_is_current) {
    if (auto current_usage = context->getQuotaUsage()) current_quota_id = current_usage->quota_id;
  }
  ...
  auto add_row = [&](const String &quota_name, const UUID &quota_id, const String &quota_key, const QuotaUsage::Interval *interval) {
    ...
    if (add_column_is_current) column_is_current->push_back(quota_id == current_quota_id);
  ...
  auto add_rows = [&](const String &quota_name, const UUID &quota_id, const String &quota_key,
                      const std::vector<QuotaUsage::Interval> &intervals) {
    if (intervals.empty()) {
      add_row(quota_name, quota_id, quota_key, nullptr);
      return;
    }

    for (const auto &interval : intervals) add_row(quota_name, quota_id, quota_key, &interval);
  };

  for (const auto &usage : quotas_usage) add_rows(usage.quota_name, usage.quota_id, usage.quota_key, usage.intervals);
}
}  // namespace DB
