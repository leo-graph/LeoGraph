...
ColumnsDescription StorageSystemCertificates::getColumnsDescription() {
  return ColumnsDescription{
      ...
      {"serial_number", std::make_shared<DataTypeNullable>(std::make_shared<DataTypeString>()),
       "Serial Number of the certificate assigned by the issuer."},
      {"signature_algo", std::make_shared<DataTypeNullable>(std::make_shared<DataTypeString>()),
       "Signature Algorithm - an algorithm used by the issuer to sign this certificate."},
      {"issuer", std::make_shared<DataTypeNullable>(std::make_shared<DataTypeString>()),
       "Issuer - an unique identifier for the Certificate Authority issuing this certificate."},
      {"not_before", std::make_shared<DataTypeNullable>(std::make_shared<DataTypeString>()),
       "The beginning of the time window when this certificate is valid."},
      {"not_after", std::make_shared<DataTypeNullable>(std::make_shared<DataTypeString>()),
       "The end of the time window when this certificate is valid."},
      {"subject", std::make_shared<DataTypeNullable>(std::make_shared<DataTypeString>()),
       "Subject - identifies the owner of the public key."},
      {"pkey_algo", std::make_shared<DataTypeNullable>(std::make_shared<DataTypeString>()),
       "Public Key Algorithm defines the algorithm the public key can be used with."},
...
static void enumCertificates(const std::string &dir, bool def, MutableColumns &res_columns, const std::string &protocol) {
  ...
  for (auto const &dir_entry : std::filesystem::directory_iterator(p)) {
    if (!dir_entry.is_regular_file() || !RE2::FullMatch(dir_entry.path().filename().string(), cert_name)) continue;
...
void StorageSystemCertificates::fillData([[maybe_unused]] MutableColumns &res_columns, [[maybe_unused]] ContextPtr context,
  ...
  auto process_ca_paths = [&](const Poco::Net::Context::CAPaths &ca_paths, const std::string &protocol_name) {
    if (!ca_paths.caLocation.empty()) {
      ...
      if (afile.exists()) {
        if (afile.isDirectory()) {
          ...
          for (const auto &entry : dir_set) enumCertificates(entry, false, res_columns, protocol_name);
        } else {
          auto certs = X509Certificate::fromFile(afile.path());
          for (const auto &cert : certs) populateTable(cert, res_columns, afile.path(), false, protocol_name);
        }
      }
    }
    ...
    if (!ca_paths.caDefaultDir.empty()) {
      ...
      for (const auto &entry : dir_set) enumCertificates(entry, true, res_columns, protocol_name);
    }
    ...
    if (!ca_paths.caDefaultFile.empty()) {
      ...
      if (afile.exists()) {
        ...
        for (const auto &cert : certs) populateTable(cert, res_columns, ca_paths.caDefaultFile, true, protocol_name);
      }
    }
  };
