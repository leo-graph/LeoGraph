...
static std::vector<std::pair<String, Int8>> getStatusEnumsAndValues() {
  return std::vector<std::pair<String, Int8>>{
      {"Inactive", static_cast<Int8>(Status::INACTIVE)}, {"Active", static_cast<Int8>(Status::ACTIVE)},
      {"Finished", static_cast<Int8>(Status::FINISHED)}, {"Removing", static_cast<Int8>(Status::REMOVING)},
...
static String clusterNameFromDDLQuery(ContextPtr context, const DDLTask &task) {
  ...
  try {
    query = parseQuery(parser_query, begin, end, description, settings[Setting::max_query_size], settings[Setting::max_parser_depth],
  ...
  String cluster_name;
  if (const auto *query_on_cluster = dynamic_cast<const ASTQueryWithOnCluster *>(query.get())) cluster_name = query_on_cluster->cluster;

  return cluster_name;
}

static void fillCommonColumns(MutableColumns &res_columns, size_t &col, const DDLTask &task, const String &cluster_name,
...
static void fillStatusColumns(MutableColumns &res_columns, size_t &col, Coordination::GetResponse &finished_data,
                              UInt64 query_create_time_ms) {
...
void StorageSystemDDLWorkerQueue::fillData(MutableColumns &res_columns, ContextPtr context, const ActionsDAG::Node *,
  ...
  for (size_t i = 0; i < ddl_task_paths.size(); ++i) {
    ...
    if (active_hosts.error == Coordination::Error::ZOK) {
      for (const auto &host_id_str : active_hosts.names) {
        if (processed_hosts.contains(host_id_str)) continue;
    ...
    for (const auto &host_id : task.entry.hosts) {
      if (processed_hosts.contains(host_id.toString())) continue;
