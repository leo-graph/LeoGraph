...
ColumnsDescription StorageSystemDimensionalMetrics::getColumnsDescription() {
  ...
  description.setAliases({{"name", std::make_shared<DataTypeString>(), "metric"}});

  return description;
}

void StorageSystemDimensionalMetrics::fillData(MutableColumns &res_columns, ContextPtr, const ActionsDAG::Node *,
  ...
  factory.forEachFamily([&res_columns](const DimensionalMetrics::MetricFamily &family) {
    ...
    family.forEachMetric(
        [&res_columns, &family, &labels](const DimensionalMetrics::LabelValues &label_values, const DimensionalMetrics::Metric &metric) {
          Map labels_map;
