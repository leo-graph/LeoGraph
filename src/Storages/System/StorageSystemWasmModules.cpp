...
ColumnsDescription StorageSystemWasmModules::getColumnsDescription() {
  return ColumnsDescription{
      {"name", std::make_shared<DataTypeString>(), "Name of the WebAssembly module"},
...
class WasmModulesSink final : public SinkToStorage {
 public:
  WasmModulesSink(SharedHeader header_, WasmModuleManager &wasm_module_manager_)
      : SinkToStorage(std::move(header_)), wasm_module_manager(wasm_module_manager_) {}

  String getName() const override { return "WasmModulesSink"; }

  void consume(Chunk &chunk) override {
    const auto &columns = chunk.getColumns();
    if (columns.size() != 3) throw Exception(ErrorCodes::INCORRECT_NUMBER_OF_COLUMNS, "Expected 3 columns, got {}", columns.size());
...
StorageSystemWasmModules::StorageSystemWasmModules(const StorageID &table_id_, ColumnsDescription columns_description_,
                                                   WasmModuleManager &wasm_module_manager_)
    : IStorageSystemOneBlock(table_id_, std::move(columns_description_)), wasm_module_manager(wasm_module_manager_) {}

SinkToStoragePtr StorageSystemWasmModules::write(const ASTPtr &, const StorageMetadataPtr &, ContextPtr /* context */,
                                                 bool /*async_insert*/) {
...
void StorageSystemWasmModules::fillData(MutableColumns &res_columns, ContextPtr, const ActionsDAG::Node *,
  ...
  if (res_columns.size() != std::accumulate(columns_mask.begin(), columns_mask.end(), 0u))
    throw Exception(ErrorCodes::LOGICAL_ERROR, "Expected {} columns, got {}", std::accumulate(columns_mask.begin(), columns_mask.end(), 0u),
                    res_columns.size());

  for (const auto &[module_name, module_hash] : wasm_module_manager.getModulesList()) {
    size_t column_idx = 0;

    if (columns_mask[0]) res_columns[column_idx++]->insert(module_name);

    /// 'code' column is write-only, insert default value
    if (columns_mask[1]) res_columns[column_idx++]->insertDefault();

    if (columns_mask[2]) res_columns[column_idx++]->insert(module_hash);
  }
}
...
static std::optional<String> getModuleNameToDeleteFromAst(const MutationCommands &commands) {
  ...
  if (command.type != MutationCommand::DELETE || command.partition || !command.predicate) return {};

  const auto *func = command.predicate->as<ASTFunction>();
  if (!func || func->name != "equals" || func->arguments->children.size() != 2) return {};

  const auto &args = func->arguments->children;
  if (args[0]->as<ASTIdentifier>()->full_name != "name") return {};
...
void StorageSystemWasmModules::checkMutationIsPossible(const MutationCommands &commands, const Settings &) const {
  if (getModuleNameToDeleteFromAst(commands).has_value()) return;
  throw Exception(ErrorCodes::BAD_ARGUMENTS,
                  "Only deletion of a module by name is supported. "
                  "Use query `DELETE FROM {} WHERE name == 'module_name'`",
...
void StorageSystemWasmModules::mutate(const MutationCommands &commands, ContextPtr) {
  ...
  if (!module_name)
    throw Exception(ErrorCodes::BAD_ARGUMENTS, "Expected single DELETE command with WHERE clause, got {}",
                    commands.toString(/* with_pure_metadata_commands */ true));
