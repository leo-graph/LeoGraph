...
void StorageSystemNamedCollections::fillData(MutableColumns &res_columns, ContextPtr context, const ActionsDAG::Node *,
  ...
  for (const auto &[name, collection] : collections) {
    if (!access->isGranted(AccessType::SHOW_NAMED_COLLECTIONS, name)) continue;
