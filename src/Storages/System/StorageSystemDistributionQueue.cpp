...
std::string maskDataPath(const std::string &path) {
  ...
  if (!masked_path.ends_with('/')) throw Exception(ErrorCodes::LOGICAL_ERROR, "Invalid path format");
...
void StorageSystemDistributionQueue::fillData(MutableColumns &res_columns, ContextPtr context, const ActionsDAG::Node *predicate,
  ...
  for (const auto &db : DatabaseCatalog::instance().getDatabases(GetDatabasesOptions{.with_datalake_catalogs = false})) {
    ...
    for (auto iterator = db.second->getTablesIterator(context); iterator->isValid(); iterator->next()) {
      ...
      if (!dynamic_cast<const StorageDistributed *>(table.get())) continue;
