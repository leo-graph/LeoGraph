...
ColumnsDescription StorageSystemViewRefreshes::getColumnsDescription() {
  return ColumnsDescription{
      ...
      {"last_success_duration_ms", std::make_shared<DataTypeNullable>(std::make_shared<DataTypeUInt64>()),
       "How long the latest refresh took."},
      {"last_refresh_time", std::make_shared<DataTypeNullable>(std::make_shared<DataTypeDateTime>()),
       "Time when the latest refresh attempt finished (if known) or started (if unknown or still running). NULL if no refresh attempts "
       "happened since server startup or table creation."},
      {"last_refresh_replica", std::make_shared<DataTypeString>(),
       "If coordination is enabled, name of the replica that made the current (if running) or previous (if not running) refresh attempt."},
      {"next_refresh_time", std::make_shared<DataTypeNullable>(std::make_shared<DataTypeDateTime>()),
       "Time at which the next refresh is scheduled to start, if status = Scheduled."},
      ...
      {"retry", std::make_shared<DataTypeUInt64>(),
       "How many failed attempts there were so far, for the current refresh. Not available if status is `RunningOnAnotherReplica`."},
      {"progress", std::make_shared<DataTypeNullable>(std::make_shared<DataTypeFloat64>()),
       "Progress of the current running or most recently completed refresh at the given replica, between 0 and 1. NULL if status is "
       "`RunningOnAnotherReplica` or the refresh is not running."},
      {"read_rows", std::make_shared<DataTypeNullable>(std::make_shared<DataTypeUInt64>()),
       "Number of rows read by the current running or most recently completed refresh at the given replica. NULL if status is "
       "`RunningOnAnotherReplica`."},
      {"read_bytes", std::make_shared<DataTypeNullable>(std::make_shared<DataTypeUInt64>()),
       "Number of bytes read by the current running or most recently completed refresh at the given replica. NULL if status is "
       "`RunningOnAnotherReplica`"},
      {"total_rows", std::make_shared<DataTypeNullable>(std::make_shared<DataTypeUInt64>()),
       "Estimated total number of rows that need to be read by the current running or most recently completed refresh at the given "
       "replica. NULL if status is `RunningOnAnotherReplica`"},
      {"written_rows", std::make_shared<DataTypeNullable>(std::make_shared<DataTypeUInt64>()),
       "Number of rows written by the current running or most recently completed refresh at the given replica. NULL if status is "
       "`RunningOnAnotherReplica`"},
      {"written_bytes", std::make_shared<DataTypeNullable>(std::make_shared<DataTypeUInt64>()),
       "Number of bytes written by the current running or most recently completed refresh at the given replica. NULL if status is "
       "`RunningOnAnotherReplica`"},
  };
}

void StorageSystemViewRefreshes::fillData(MutableColumns &res_columns, ContextPtr context, const ActionsDAG::Node *,
  ...
  for (const RefreshTaskPtr &task : context->getRefreshSet().getTasks()) {
    ...
    if (refresh.refresh_running && retries) retries -= 1;
    ...
    if (refresh.znode.last_attempt_replica.empty() || refresh.replica_name == refresh.znode.last_attempt_replica) {
      res_columns[i++]->insert(static_cast<double>(refresh.progress.read_rows) /
