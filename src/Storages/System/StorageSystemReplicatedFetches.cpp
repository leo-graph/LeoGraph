...
ColumnsDescription StorageSystemReplicatedFetches::getColumnsDescription() {
  return ColumnsDescription{
      ...
      {"elapsed", std::make_shared<DataTypeFloat64>(),
       "The time elapsed (in seconds) since showing currently running background fetches started."},
      ...
      {"total_size_bytes_compressed", std::make_shared<DataTypeUInt64>(),
       "The total size (in bytes) of the compressed data in the result part."},
...
void StorageSystemReplicatedFetches::fillData(MutableColumns &res_columns, ContextPtr context, const ActionsDAG::Node *,
                                              std::vector<UInt8>) const {
  const auto access = context->getAccess();
  const bool check_access_for_tables = !access->isGranted(AccessType::SHOW_TABLES);

  for (const auto &fetch : context->getReplicatedFetchList().get()) {
    if (check_access_for_tables && !access->isGranted(AccessType::SHOW_TABLES, fetch.database, fetch.table)) continue;
