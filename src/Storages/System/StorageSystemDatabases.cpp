...
static String getEngineFull(const ContextPtr& ctx, const DatabasePtr& database) {
  ...
  while (true) {
    ...
    if (locked_database.get() == database.get()) break;

    /// Database was dropped
    if (name == database->getDatabaseName()) return {};
  ...
  if (!ast_create || !ast_create->storage) return {};
...
static ColumnPtr getFilteredDatabases(const Databases& databases, const ActionsDAG::Node* predicate, ContextPtr context) {
  ...
  Block block{ColumnWithTypeAndName(std::move(name_column), std::make_shared<DataTypeString>(), "name"),
...
void StorageSystemDatabases::fillData(MutableColumns& res_columns, ContextPtr context, const ActionsDAG::Node* predicate,
  ...
  for (size_t i = 0; i < filtered_databases_column->size(); ++i) {
    ...
    if (database_name == DatabaseCatalog::TEMPORARY_DATABASE)
      continue;  /// filter out the internal database for temporary tables in system.databases, asynchronous metric "NumberOfDatabases"
                 /// behaves the same way

    auto database_it = databases.find(database_name);
    if (database_it == databases.end()) throw Exception(ErrorCodes::UNKNOWN_DATABASE, "Database {} does not exist", database_name);
    const auto& database = database_it->second;

    size_t src_index = 0;
    size_t res_index = 0;
    if (columns_mask[src_index++]) res_columns[res_index++]->insert(database_name);
    if (columns_mask[src_index++]) res_columns[res_index++]->insert(database->getEngineName());
    if (columns_mask[src_index++]) res_columns[res_index++]->insert(context->getPath() + database->getDataPath());
    if (columns_mask[src_index++]) res_columns[res_index++]->insert(database->getMetadataPath());
    if (columns_mask[src_index++]) res_columns[res_index++]->insert(database->getUUID());
    if (columns_mask[src_index++]) res_columns[res_index++]->insert(getEngineFull(context, database));
    if (columns_mask[src_index++]) res_columns[res_index++]->insert(database->getDatabaseComment());
    if (columns_mask[src_index++]) res_columns[res_index++]->insert(database->isExternal());
  }
}
