...
namespace DB {
StorageSystemDataSkippingIndices::StorageSystemDataSkippingIndices(const StorageID &table_id_) : IStorage(table_id_) {
  auto creation_datatype = std::make_shared<DataTypeEnum8>(DataTypeEnum8::Values{
...
class DataSkippingIndicesSource : public ISource {
 public:
  DataSkippingIndicesSource(std::vector<UInt8> columns_mask_, SharedHeader header, UInt64 max_block_size_, ColumnPtr databases_,
                            ContextPtr context_)
      : ISource(header),
 ...
 protected:
  Chunk generate() override {
    if (database_idx >= databases->size()) return {};
    ...
    while (rows_count < max_block_size) {
      ...
      if (database_idx >= databases->size()) break;

      if (!tables_it || !tables_it->isValid()) tables_it = database->getTablesIterator(context);
      ...
      for (; rows_count < max_block_size && tables_it->isValid(); tables_it->next()) {
        ...
        for (const auto &index : indices) {
          ...
          // 'database' column
          if (column_mask[src_index++]) res_columns[res_index++]->insert(database_name);
          // 'table' column
          if (column_mask[src_index++]) res_columns[res_index++]->insert(table_name);
          // 'name' column
          if (column_mask[src_index++]) res_columns[res_index++]->insert(index.name);
          // 'type' column
          if (column_mask[src_index++]) res_columns[res_index++]->insert(index.type);
          // 'type_full' column
          ...
          /// 'creation' column
          if (column_mask[src_index++]) res_columns[res_index++]->insert(index.is_implicitly_created);

          // 'granularity' column
          if (column_mask[src_index++]) res_columns[res_index++]->insert(index.granularity);

          auto &secondary_index_size = secondary_index_sizes[index.name];

          // 'compressed bytes' column
          if (column_mask[src_index++]) res_columns[res_index++]->insert(secondary_index_size.data_compressed);

          // 'uncompressed bytes' column
          if (column_mask[src_index++]) res_columns[res_index++]->insert(secondary_index_size.data_uncompressed);

          /// 'marks_bytes' column
          if (column_mask[src_index++]) res_columns[res_index++]->insert(secondary_index_size.marks);
        }
      }
    }
...
class ReadFromSystemDataSkippingIndices : public SourceStepWithFilter {
 public:
  ...
  ReadFromSystemDataSkippingIndices(const Names &column_names_, const SelectQueryInfo &query_info_,
                                    const StorageSnapshotPtr &storage_snapshot_, const ContextPtr &context_, Block sample_block,
                                    std::shared_ptr<StorageSystemDataSkippingIndices> storage_, std::vector<UInt8> columns_mask_,
...
void StorageSystemDataSkippingIndices::read(QueryPlan &query_plan, const Names &column_names, const StorageSnapshotPtr &storage_snapshot,
                                            SelectQueryInfo &query_info, ContextPtr context,
  ...
  auto reading =
      std::make_unique<ReadFromSystemDataSkippingIndices>(column_names, query_info, storage_snapshot, std::move(context), std::move(header),
...
void ReadFromSystemDataSkippingIndices::initializePipeline(QueryPipelineBuilder &pipeline, const BuildQueryPipelineSettings &) {
  ...
  for (const auto &[database_name, database] : databases) {
    ...
    if (database->isExternal()) continue;
    column->insert(database_name);
  }

  /// Condition on "database" in a query acts like an index.
  Block block{ColumnWithTypeAndName(std::move(column), std::make_shared<DataTypeString>(), "database")};
  if (virtual_columns_filter) VirtualColumnUtils::filterBlockWithExpression(virtual_columns_filter, block);

  ColumnPtr &filtered_databases = block.getByPosition(0).column;
  pipeline.init(Pipe(std::make_shared<DataSkippingIndicesSource>(std::move(columns_mask), getOutputHeader(), max_block_size,
                                                                 std::move(filtered_databases), context)));
}
