...
ColumnsDescription StorageSystemReplicationQueue::getColumnsDescription() {
  return ColumnsDescription{
      ...
      {"required_quorum", std::make_shared<DataTypeUInt32>(),
       "The number of replicas waiting for the task to complete with confirmation of completion. This column is only relevant for the "
       "GET_PARTS task."},
      ...
      {"is_currently_executing", std::make_shared<DataTypeUInt8>(),
       "The flag indicates whether a specific task is being performed right now."},
...
void StorageSystemReplicationQueue::fillData(MutableColumns &res_columns, ContextPtr context, const ActionsDAG::Node *predicate,
  ...
  for (size_t i = 0, tables_size = col_database_to_filter->size(); i < tables_size; ++i) {
    ...
    for (size_t j = 0, queue_size = queue.size(); j < queue_size; ++j) {
      ...
      for (const auto &part_name : entry.source_parts) parts_to_merge.push_back(part_name);
