...
ColumnsDescription StorageSystemProcesses::getColumnsDescription() {
  ...
  auto description = ColumnsDescription{
      {"is_initial_query", std::make_shared<DataTypeUInt8>(),
       "Whether this query comes directly from user or was issues by ClickHouse server in a scope of distributed query execution."},

      {"user", std::make_shared<DataTypeString>(),
       "The user who made the query. Keep in mind that for distributed processing, queries are sent to remote servers under the default "
      ...
      {"address", DataTypeFactory::instance().get("IPv6"),
       "The IP address the query was made from. The same for distributed processing. To track where a distributed query was originally "
       "made from, look at system.processes on the query requestor server."},
      ...
      {"interface", std::make_shared<DataTypeUInt8>(),
       "The interface which was used to send the query. TCP = 1, HTTP = 2, GRPC = 3, MYSQL = 4, POSTGRESQL = 5, LOCAL = 6, TCP_INTERSERVER "
       "= 7."},
      ...
      {"distributed_depth", std::make_shared<DataTypeUInt64>(),
       "The number of times query was retransmitted between server nodes internally."},
      ...
      {"is_all_data_sent", std::make_shared<DataTypeUInt8>(),
       "Was all data sent to the client (in other words query had been finished on the server)."},
      {"read_rows", std::make_shared<DataTypeUInt64>(),
       "The number of rows read from the table. For distributed processing, on the requestor server, this is the total for all remote "
       "servers."},
      {"read_bytes", std::make_shared<DataTypeUInt64>(),
       "The number of uncompressed bytes read from the table. For distributed processing, on the requestor server, this is the total for "
       "all remote servers."},
      {"total_rows_approx", std::make_shared<DataTypeUInt64>(),
       "The approximation of the total number of rows that should be read. For distributed processing, on the requestor server, this is "
       "the total for all remote servers. It can be updated during request processing, when new sources to process become known."},
      ...
      {"thread_ids", std::make_shared<DataTypeArray>(std::make_shared<DataTypeUInt64>()),
       "The list of identifiers of all threads which participated in this query."},
...
void StorageSystemProcesses::fillData(MutableColumns &res_columns, ContextPtr context, const ActionsDAG::Node *, std::vector<UInt8>) const {
  ...
  for (const auto &process : info) {
...

    {
      ...
      for (const UInt64 thread_id : process.thread_ids) threads_array.emplace_back(thread_id);
