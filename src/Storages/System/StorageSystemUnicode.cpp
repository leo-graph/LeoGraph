...
constexpr UProperty int_properties[] = {UCHAR_BIDI_CLASS,
                                        UCHAR_BLOCK,
...
// String properties
constexpr UProperty string_properties[] = {UCHAR_AGE,
...
static DataTypePtr getEnumTypeForProperty(UProperty prop) {
  ...
  if (use_enum16) {
    if (enum16_values.empty()) return std::make_shared<DataTypeInt32>();
    return std::make_shared<DataTypeEnum16>(enum16_values);
  } else {
    if (enum8_values.empty()) return std::make_shared<DataTypeInt32>();
...
static void toUTF8(UChar32 code, IColumn &column, Poco::UTF8Encoding &encoding) {
  ...
  if (!res || res > 4) throw Exception(ErrorCodes::LOGICAL_ERROR, "Cannot convert code point {} to UTF-8", code);
...
static Block getFilteredCodePoints(const ActionsDAG::Node *predicate, ContextPtr context, Poco::UTF8Encoding &encoding) {
  ...
  if (U_FAILURE(status)) throw Exception(ErrorCodes::UNICODE_ERROR, "Cannot obtain the list of assigned code points");
  ...
  Block filter_block{ColumnWithTypeAndName(std::move(code_point_column), std::make_shared<DataTypeString>(), "code_point"),
...
void StorageSystemUnicode::fillData(MutableColumns &res_columns, ContextPtr context, const ActionsDAG::Node *predicate,
    ...
    if (columns_mask[mask_index++]) {
      ...
      if (string_prop == UCHAR_AGE) {
        ...
        for (UChar32 code : filtered_code_points) {
          ...
          if (U_FAILURE(err_code))
            throw Exception(ErrorCodes::UNICODE_ERROR, "Failed to convert to lowercase for code point {}: {}", code, u_errorName(err_code));
          icu::UnicodeString str(buffer);
          String ret;
          str.toUTF8String(ret);
          column.insert(ret);
        ...
        for (UChar32 code : filtered_code_points) {
          ...
          if (U_FAILURE(err_code))
            throw Exception(ErrorCodes::UNICODE_ERROR, "Failed to get character name for code point {}: {}", code, u_errorName(err_code));
        ...
        for (UChar32 code : filtered_code_points) {
          ...
          if (U_FAILURE(err_code))
            throw Exception(ErrorCodes::UNICODE_ERROR, "Failed to convert to uppercase for code point {}: {}", code, u_errorName(err_code));
          String ret;
          icu::UnicodeString str(buffer);
          str.toUTF8String(ret);
          column.insert(ret);
    ...
    if (columns_mask[mask_index++]) {
      if (other_prop == UCHAR_SCRIPT_EXTENSIONS) {
        ...
        for (UChar32 code : filtered_code_points) {
          ...
          if (U_FAILURE(err_code))
            throw Exception(ErrorCodes::UNICODE_ERROR, "Failed to get script extensions for code point {}: {}", code,
                            u_errorName(err_code));
          if (num_scripts < 0)
            throw Exception(ErrorCodes::UNICODE_ERROR, "Invalid number of scripts returned for code point {}: {}", code, num_scripts);

          Array arr;
          for (int32_t j = 0; j < num_scripts; ++j) {
            const char *script_name = uscript_getName(scx_val_array[j]);
            if (script_name == nullptr) {
              throw Exception(ErrorCodes::UNICODE_ERROR, "Failed to get script name for code point {}: {}", code, scx_val_array[j]);
            }
        ...
        for (UChar32 code : filtered_code_points) {
          ...
          if (U_FAILURE(err_code)) {
            throw Exception(ErrorCodes::UNICODE_ERROR, "Failed to get identifier types for code point {}: {}", code, u_errorName(err_code));
          }
          Array arr;
          for (int32_t i = 0; i < count; i++) {
            const char *type_name = u_getPropertyValueName(UCHAR_IDENTIFIER_TYPE, types[i], U_LONG_PROPERTY_NAME);
            if (type_name == nullptr) {
              throw Exception(ErrorCodes::UNICODE_ERROR, "Failed to get identifier type name for code point {}: {}", code, types[i]);
            }
