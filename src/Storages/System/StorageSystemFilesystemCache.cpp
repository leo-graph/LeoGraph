...
class SystemFilesystemCacheSource : public ISource, private WithContext {
 public:
  SystemFilesystemCacheSource(SharedHeader header_, UInt64 max_block_size_, ContextPtr context_)
      : ISource(header_), WithContext(context_), max_block_size(max_block_size_), origin(FileCache::getCommonOrigin()) {
 ...
 protected:
  Chunk generate() override {
    ...
    auto get_total_size = [&] -> size_t {
      return col_cache_name->byteSize() + col_cache_base_path->byteSize() + col_path->byteSize() + col_key->byteSize() +
             col_range_begin->byteSize() + col_range_end->byteSize() + col_size->byteSize() + col_state->byteSize() +
             col_finished_download_time->byteSize() + col_hits->byteSize() + col_references->byteSize() + col_downloaded_size->byteSize() +
             col_kind->byteSize() + col_unbound->byteSize() + col_user_id->byteSize() + col_file_origin->byteSize();
    };
    ...
    while (true) {
      if (num_rows && max_block_size && get_total_size() > max_block_size) break;

      if (current_cache == unique_caches.end()) break;
      ...
      if (!current_cache_iterator) current_cache_iterator = cache->getCacheIterator(origin.user_id);
    ...
    Columns columns{std::move(col_cache_name),  std::move(col_cache_base_path), std::move(col_path),
                    std::move(col_key),         std::move(col_range_begin),     std::move(col_range_end),
                    std::move(col_size),        std::move(col_state),           std::move(col_finished_download_time),
                    std::move(col_hits),        std::move(col_references),      std::move(col_downloaded_size),
                    std::move(col_kind),        std::move(col_unbound),         std::move(col_user_id),
                    std::move(col_file_origin), std::move(col_file_size)};
...
class ReadFromSystemFilesystemCache final : public SourceStepWithFilter {
 public:
  ReadFromSystemFilesystemCache(const Names &column_names_, const SelectQueryInfo &query_info_, const StorageSnapshotPtr &storage_snapshot_,
...
StorageSystemFilesystemCache::StorageSystemFilesystemCache(const StorageID &table_id_) : IStorage(table_id_) {
  StorageInMemoryMetadata storage_metadata;
  storage_metadata.setColumns(ColumnsDescription({
      {"cache_name", std::make_shared<DataTypeString>(),
       "Name of the cache object. Can be used in `SYSTEM DESCRIBE FILESYSTEM CACHE <name>`, `SYSTEM DROP FILESYSTEM CACHE <name>` "
       "commands"},
      {"cache_base_path", std::make_shared<DataTypeString>(),
       "Path to the base directory where all caches files (of a cache identidied by `cache_name`) are stored."},
      {"cache_path", std::make_shared<DataTypeString>(),
       "Path to a particular cache file, corresponding to a file segment in a source file"},
      ...
      {"file_segment_range_end", std::make_shared<DataTypeUInt64>(),
       "Offset corresponding to the (including) end of the file segment range"},
      ...
      {"references", std::make_shared<DataTypeUInt64>(),
       "Number of references to corresponding file segment. Value 1 means that nobody uses it at the moment (the only existing reference "
       "is in cache storage itself)"},
      {"downloaded_size", std::make_shared<DataTypeUInt64>(), "Downloaded size of the file segment"},
      {"kind", std::make_shared<DataTypeString>(),
       "File segment kind (used to distringuish between file segments added as a part of 'Temporary data in cache')"},
      ...
      {"segment_type", std::make_shared<DataTypeString>(),
       "Type of the segment. Used to separate data files(`.json`, `.txt` and etc) from data file(`.bin`, mark files)."},
      {"file_size", std::make_shared<DataTypeNullable>(std::make_shared<DataTypeUInt64>()),
       "File size of the file to which current file segment belongs"},
  }));
  setInMemoryMetadata(storage_metadata);
}

void StorageSystemFilesystemCache::read(QueryPlan &query_plan, const Names &column_names, const StorageSnapshotPtr &storage_snapshot,
  ...
  auto read_step =
      std::make_unique<ReadFromSystemFilesystemCache>(column_names, query_info, storage_snapshot, context, header, max_block_size);
