...
ColumnsDescription StorageSystemUsers::getColumnsDescription() {
  return ColumnsDescription{
      ...
      {"default_roles_all", std::make_shared<DataTypeUInt8>(), "Shows that all granted roles set for user by default."},
      ...
      {"grantees_any", std::make_shared<DataTypeUInt8>(),
       "The flag that indicates whether a user with any grant option can grant it to anyone."},
      {"grantees_list", std::make_shared<DataTypeArray>(std::make_shared<DataTypeString>()),
       "The list of users or roles to which this user is allowed to grant options to."},
      {"grantees_except", std::make_shared<DataTypeArray>(std::make_shared<DataTypeString>()),
       "The list of users or roles to which this user is forbidden from grant options to."},
...
void StorageSystemUsers::fillData(MutableColumns &res_columns, ContextPtr context, const ActionsDAG::Node *, std::vector<UInt8>) const {
  ...
  if (!access_control.doesSelectFromSystemDatabaseRequireGrant()) context->checkAccess(AccessType::SHOW_USERS);
  ...
  auto add_row = [&](const String &name, const UUID &id, const String &storage_name,
                     const std::vector<AuthenticationData> &authentication_methods, const AllowedClientHosts &allowed_hosts,
                     const RolesOrUsersSet &default_roles, const RolesOrUsersSet &grantees, const String default_database) {
    ...
    for (const auto &auth_data : authentication_methods) {
      ...
      if (auth_data.getType() == AuthenticationType::LDAP) {
        ...
        for (const String &subject : subjects.at(X509Certificate::Subjects::Type::CN)) common_names->add(subject);
        for (const String &subject : subjects.at(X509Certificate::Subjects::Type::SAN)) subject_alt_names->add(subject);

        if (common_names->size() > 0) auth_params_json.set("common_names", common_names);
        if (subject_alt_names->size() > 0) auth_params_json.set("subject_alt_names", subject_alt_names);
      }
#endif
    ...
    if (allowed_hosts.containsAnyHost()) {
      ...
      for (const auto &host_name : allowed_hosts.getNames()) column_host_names.insertData(host_name.data(), host_name.length());
    ...
    for (const auto &role_name : default_roles_ast->names) column_default_roles_list.insertData(role_name.data(), role_name.length());
    ...
    for (const auto &grantee_name : grantees_ast->names) column_grantees_list.insertData(grantee_name.data(), grantee_name.length());
    column_grantees_list_offsets.push_back(column_grantees_list.size());
    for (const auto &except_name : grantees_ast->except_names) column_grantees_except.insertData(except_name.data(), except_name.length());
  ...
  for (const auto &id : ids) {
    ...
    add_row(user->getName(), id, storage->getStorageName(), user->authentication_methods, user->allowed_client_hosts, user->default_roles,
            user->grantees, user->default_database);
  }
}

void StorageSystemUsers::backupData(BackupEntriesCollector &backup_entries_collector, const String &data_path_in_backup,
...
void StorageSystemUsers::restoreDataFromBackup(RestorerFromBackup &restorer, const String &data_path_in_backup,
                                               const std::optional<ASTs> & /* partitions */) {
