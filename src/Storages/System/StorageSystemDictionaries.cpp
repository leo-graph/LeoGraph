...
std::optional<DictionaryStructure> getDictionaryStructure(const ExternalLoader::LoadResult &load_result,
                                                          std::exception_ptr &last_exception) try {
...
ColumnsDescription StorageSystemDictionaries::getColumnsDescription() {
  return ColumnsDescription{
      {"database", std::make_shared<DataTypeString>(),
       "Name of the database containing the dictionary created by DDL query. Empty string for other dictionaries."},
      ...
      {"status", std::make_shared<DataTypeEnum8>(getExternalLoaderStatusEnumAllPossibleValues()),
       ...
       "LOADED_AND_RELOADING — Dictionary is loaded successfully, and is being reloaded right now (frequent reasons: SYSTEM RELOAD "
       "DICTIONARY query, timeout, dictionary config has changed), "
      ...
      {"key.types", std::make_shared<DataTypeArray>(std::make_shared<DataTypeString>()),
       "Corresponding array of key types provided by the dictionary."},
      ...
      {"query_count", std::make_shared<DataTypeUInt64>(),
       "Number of queries since the dictionary was loaded or since the last successful reboot."},
      {"hit_rate", std::make_shared<DataTypeFloat64>(),
       "For cache dictionaries, the percentage of uses for which the value was in the cache."},
      ...
      {"load_factor", std::make_shared<DataTypeFloat64>(),
       "Percentage filled in the dictionary (for a hashed dictionary, the percentage filled in the hash table)."},
      ...
      {"lifetime_min", std::make_shared<DataTypeUInt64>(),
       "Minimum lifetime of the dictionary in memory, after which ClickHouse tries to reload the dictionary (if invalidate_query is set, "
       "then only if it has changed). Set in seconds."},
      {"lifetime_max", std::make_shared<DataTypeUInt64>(),
       "Maximum lifetime of the dictionary in memory, after which ClickHouse tries to reload the dictionary (if invalidate_query is set, "
       "then only if it has changed). Set in seconds."},
      ...
      {"last_successful_update_time", std::make_shared<DataTypeDateTime>(),
       "End time for loading or updating the dictionary. Helps to monitor some troubles with dictionary sources and investigate the "
       "causes."},
      {"error_count", std::make_shared<DataTypeUInt64>(),
       "Number of errors since last successful loading. Helps to monitor some troubles with dictionary sources and investigate the "
       "causes."},
      {"loading_duration", std::make_shared<DataTypeFloat32>(), "Duration of a dictionary loading."},
      {"last_exception", std::make_shared<DataTypeString>(),
       "Text of the error that occurs when creating or reloading the dictionary if the dictionary couldn't be created."},
      {"comment", std::make_shared<DataTypeString>(), "Text of the comment to dictionary."}};
}

void StorageSystemDictionaries::fillData(MutableColumns &res_columns, ContextPtr context, const ActionsDAG::Node *,
  ...
  if (!check_access_for_dictionaries) return;
  ...
  for (const auto &load_result : external_dictionaries.getLoadResults()) {
    ...
    if (!access->isGranted(AccessType::SHOW_DICTIONARIES, db_or_tag, dict_id.table_name)) continue;
    ...
    if (dict_structure) {
      ...
      if (dict_structure->id)
        ...
        res_columns[i++]->insert(
            Array{std::from_range_t{}, *dict_structure->key | std::views::transform([](auto &attr) { return attr.type->getName(); })});

      res_columns[i++]->insert(
          Array{std::from_range_t{}, dict_structure->attributes | std::views::transform([](auto &attr) { return attr.name; })});
      res_columns[i++]->insert(
          Array{std::from_range_t{}, dict_structure->attributes | std::views::transform([](auto &attr) { return attr.type->getName(); })});
    } else {
    ...
    if (dict_ptr) {
      ...
      if (!last_exception) last_exception = dict_ptr->getLastException();
    } else {
