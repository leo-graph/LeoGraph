...
StoragesInfoStreamBase::StoragesInfoStreamBase(ContextPtr context)
    : query_id(context->getCurrentQueryId()),
      lock_timeout(std::chrono::milliseconds(context->getSettingsRef()[Setting::lock_acquire_timeout].totalMilliseconds())),
      next_row(0),
      rows(0) {}
...
bool StorageSystemPartsBase::hasStateColumn(const Names &column_names, const StorageSnapshotPtr &storage_snapshot) {
  ...
  if (!(has_state_column && real_column_names.empty())) storage_snapshot->check(real_column_names);

  return has_state_column;
}

MergeTreeData::DataPartsVector StoragesInfo::getParts(MergeTreeData::DataPartStateVector &state, bool has_state_column) const {
...
MergeTreeData::ProjectionPartsVector StoragesInfo::getProjectionParts(MergeTreeData::DataPartStateVector &state,
                                                                      bool has_state_column) const {
  if (data->getInMemoryMetadataPtr()->projections.empty()) return {};

  using State = MergeTreeData::DataPartState;
  if (need_inactive_parts) {
    /// If has_state_column is requested, return all states.
    if (!has_state_column) return data->getProjectionPartsVectorForInternalUsage({State::Active, State::Outdated}, &state);
...
StoragesInfoStream::StoragesInfoStream(std::optional<ActionsDAG> filter_by_database, std::optional<ActionsDAG> filter_by_other_columns,
                                       ContextPtr context)
...

  {
    ...
    for (const auto &database : databases) {
      ...
      if (!database.second->isExternal()) database_column_mut->insert(database.first);
    }
    block_to_filter.insert(ColumnWithTypeAndName(std::move(database_column_mut), std::make_shared<DataTypeString>(), database_column_name));

    /// Filter block_to_filter with column 'database'.
    if (filter_by_database)
      VirtualColumnUtils::filterBlockWithExpression(VirtualColumnUtils::buildFilterExpression(std::move(*filter_by_database), context),
                                                    block_to_filter);
    ...
    if (rows) {
      ...
      for (size_t i = 0; i < rows; ++i) {
        ...
        for (auto iterator = database->getTablesIterator(context); iterator->isValid(); iterator->next()) {
          ...
          if (!dynamic_cast<MergeTreeData *>(storage.get())) continue;

          if (check_access_for_tables && !access->isGranted(AccessType::SHOW_TABLES, database_name, table_name)) continue;
  ...
  block_to_filter.insert(
      ColumnWithTypeAndName(std::move(storage_uuid_column_mut), std::make_shared<DataTypeUUID>(), storage_uuid_column_name));

  if (rows) {
    /// Filter block_to_filter with columns 'database', 'table', 'engine', 'active'.
    if (filter_by_other_columns)
      VirtualColumnUtils::filterBlockWithExpression(VirtualColumnUtils::buildFilterExpression(std::move(*filter_by_other_columns), context),
                                                    block_to_filter);
    rows = block_to_filter.rows();
  }
...
class ReadFromSystemPartsBase : public SourceStepWithFilter {
 public:
  ...
  ReadFromSystemPartsBase(const Names &column_names_, const SelectQueryInfo &query_info_, const StorageSnapshotPtr &storage_snapshot_,
                          const ContextPtr &context_, SharedHeader sample_block, std::shared_ptr<StorageSystemPartsBase> storage_,
                          std::vector<UInt8> columns_mask_, bool has_state_column_);
...
ReadFromSystemPartsBase::ReadFromSystemPartsBase(const Names &column_names_, const SelectQueryInfo &query_info_,
                                                 const StorageSnapshotPtr &storage_snapshot_, const ContextPtr &context_,
                                                 SharedHeader sample_block, std::shared_ptr<StorageSystemPartsBase> storage_,
                                                 std::vector<UInt8> columns_mask_, bool has_state_column_)
...
void ReadFromSystemPartsBase::applyFilters(ActionDAGNodes added_filter_nodes) {
  ...
  if (filter_actions_dag) {
    ...
    if (filter_by_database) VirtualColumnUtils::buildSetsForDAG(*filter_by_database, context);
    ...
    if (filter_by_other_columns) VirtualColumnUtils::buildSetsForDAG(*filter_by_other_columns, context);
  }
}

void StorageSystemPartsBase::read(QueryPlan &query_plan, const Names &column_names, const StorageSnapshotPtr &storage_snapshot,
  ...
  if (has_state_column) header.insert(ColumnWithTypeAndName(std::make_shared<DataTypeString>(), "_state"));

  auto this_ptr = std::static_pointer_cast<StorageSystemPartsBase>(shared_from_this());

  auto reading = std::make_unique<ReadFromSystemPartsBase>(column_names, query_info, storage_snapshot, std::move(context),
                                                           std::make_shared<const Block>(std::move(header)), std::move(this_ptr),
                                                           std::move(columns_mask), has_state_column);
...
StorageSystemPartsBase::StorageSystemPartsBase(const StorageID &table_id_, ColumnsDescription &&columns) : IStorage(table_id_) {
  auto add_alias = [&](const String &alias_name, const String &column_name) {
    if (!columns.has(column_name)) return;
