...
class SystemDatabaseReplicasSource : public ISource {
 public:
  SystemDatabaseReplicasSource(SharedHeader header_, size_t max_block_size_, ColumnPtr col_database_, ContextPtr context_,
...
Chunk SystemDatabaseReplicasSource::generate() {
  ...
  for (; index < futures.size(); ++index) {
    ...
    if (rows_added) {
      if (futures[index].wait_for(std::chrono::seconds(0)) != std::future_status::ready) break;
...
class ReadFromSystemDatabaseReplicas : public SourceStepWithFilter {
 public:
  ReadFromSystemDatabaseReplicas(const Names &column_names_, const SelectQueryInfo &query_info_,
                                 const StorageSnapshotPtr &storage_snapshot_, const ContextPtr &context_, Block sample_block,
                                 std::map<String, DatabasePtr> replicated_databases_, size_t max_block_size_,
                                 std::shared_ptr<StorageSystemDatabaseReplicas::TPools> pools_, bool with_zk_fields_)
...
StorageSystemDatabaseReplicas::StorageSystemDatabaseReplicas(const StorageID &table_id_)
    : IStorage(table_id_), pools(std::make_shared<TPools>(DEFAULT_THREAD_COUNT)) {
  ColumnsDescription description = {
      ...
      {"log_ptr", std::make_shared<DataTypeInt32>(),
...
void StorageSystemDatabaseReplicas::read(QueryPlan &query_plan, const Names &column_names, const StorageSnapshotPtr &storage_snapshot,
  ...
  for (const auto &[db_name, db_data] : DatabaseCatalog::instance().getDatabases(GetDatabasesOptions{.with_datalake_catalogs = false})) {
    if (!dynamic_cast<const DatabaseReplicated *>(db_data.get())) continue;

    if (need_to_check_access_for_databases && !access->isGranted(AccessType::SHOW_DATABASES, db_name)) continue;
  ...
  auto reading =
      std::make_unique<ReadFromSystemDatabaseReplicas>(column_names, query_info, storage_snapshot, std::move(context), std::move(header),
                                                       std::move(replicated_databases), max_block_size, pools, with_zk_fields);
