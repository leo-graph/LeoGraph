...
StorageSystemUserDefinedFunctions::StorageSystemUserDefinedFunctions(const StorageID &storage_id_, ColumnsDescription columns_description_)
...
ColumnsDescription StorageSystemUserDefinedFunctions::getColumnsDescription() {
  return ColumnsDescription{
      ...
      {"name", std::make_shared<DataTypeString>(), "UDF name."},
      {"load_status", std::make_shared<DataTypeEnum8>(DataTypeEnum8::Values{{"Success", 0}, {"Failed", 1}}),
      ...
      {"loading_duration_ms", std::make_shared<DataTypeUInt64>(), "Time spent loading the UDF, in milliseconds."},

      // ===== UDF Configuration Fields (from XML config) =====
      {"type", std::make_shared<DataTypeEnum8>(DataTypeEnum8::Values{{"executable", 0}, {"executable_pool", 1}}),
       "UDF type: 'executable' (single process) or 'executable_pool' (process pool)."},
      {"command", std::make_shared<DataTypeString>(), "Script or command to execute for this UDF."},
      {"format", std::make_shared<DataTypeString>(), "Data format for I/O (e.g., 'TabSeparated', 'JSONEachRow')."},
      {"return_type", std::make_shared<DataTypeString>(), "Function return type (e.g., 'String', 'UInt64')."},
      {"return_name", std::make_shared<DataTypeString>(), "Optional return value identifier. Empty if not configured."},
      ...
      {"command_termination_timeout", std::make_shared<DataTypeUInt64>(), "Seconds before sending SIGTERM to command process."},
      {"command_read_timeout", std::make_shared<DataTypeUInt64>(), "Milliseconds for reading from command stdout."},
      {"command_write_timeout", std::make_shared<DataTypeUInt64>(), "Milliseconds for writing to command stdin."},
      {"pool_size", std::make_shared<DataTypeUInt64>(), "Number of command process instances. Only for 'executable_pool' type."},
      {"send_chunk_header", std::make_shared<DataTypeUInt8>(), "Whether to send row count before each data chunk (boolean)."},
      {"execute_direct", std::make_shared<DataTypeUInt8>(), "Whether to execute command directly (1) or via /bin/bash (0)."},
      {"lifetime", std::make_shared<DataTypeUInt64>(), "Reload interval in seconds. 0 means reload is disabled."},
      {"deterministic", std::make_shared<DataTypeUInt8>(), "Whether function returns the same result for the same arguments (boolean)."}};
}

void StorageSystemUserDefinedFunctions::fillData(MutableColumns &res_columns, ContextPtr context, const ActionsDAG::Node *,
  ...
  for (const auto &load_result : external_executable_functions_loader.getLoadResults()) {
    ...
    Int8 simplified_status =
        (load_result.status == ExternalLoaderStatus::LOADED || load_result.status == ExternalLoaderStatus::LOADED_AND_RELOADING) ? 0 : 1;
    ...
    res_columns[i++]->insert(std::chrono::duration_cast<std::chrono::milliseconds>(load_result.loading_duration).count());
    ...
    if (udf_ptr) {
      ...
      if (!config.command_arguments.empty()) {
        for (const auto &arg : config.command_arguments) full_command += " " + arg;
      }
      ...
      for (const auto &arg : config.arguments) argument_types_array.push_back(arg.type->getName());
      res_columns[i++]->insert(argument_types_array);

      Array argument_names_array;
      for (const auto &arg : config.arguments) argument_names_array.push_back(arg.name);
      ...
      for (size_t j = 0; j < config_fields_count; ++j) res_columns[i++]->insertDefault();
    }
  }
}
