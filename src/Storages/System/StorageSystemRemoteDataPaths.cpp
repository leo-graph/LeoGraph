...
class SystemRemoteDataPathsSource : public ISource {
 public:
  SystemRemoteDataPathsSource(const DisksMap &disks_, SharedHeader header_, UInt64 max_block_size_, ContextPtr context_)
      : ISource(header_), max_block_size(max_block_size_), context(std::move(context_)) {
    for (const auto &disk : disks_) {
      if (disk.second->isRemote()) disks.push_back(disk);
    }
 ...
 private:
  ...
  fs::path getCurrentPath() const {
    fs::path path;
    for (const auto &dir : paths_stack) path /= dir.names[dir.position].name;
    return path;
  }
  ...
  static bool skipPredicateForShadowDir(const String &local_path) {
    ...
    return (path.filename() == "revision.txt" && path.parent_path().has_parent_path() &&
...
class ReadFromSystemRemoteDataPaths final : public SourceStepWithFilter {
 public:
  ReadFromSystemRemoteDataPaths(DisksMap &&disks_, const Names &column_names_, const SelectQueryInfo &query_info_,
                                const StorageSnapshotPtr &storage_snapshot_, const ContextPtr &context_, const Block &header,
...
StorageSystemRemoteDataPaths::StorageSystemRemoteDataPaths(const StorageID &table_id_) : IStorage(table_id_) {
...
void StorageSystemRemoteDataPaths::read(QueryPlan &query_plan, const Names &column_names, const StorageSnapshotPtr &storage_snapshot,
  ...
  auto read_step = std::make_unique<ReadFromSystemRemoteDataPaths>(context->getDisksMap(), column_names, query_info, storage_snapshot,
...
bool SystemRemoteDataPathsSource::nextDisk() {
  while (current_disk < static_cast<ssize_t>(disks.size())) {
    ...
    if (current_disk >= static_cast<ssize_t>(disks.size())) break;
...
bool SystemRemoteDataPathsSource::isTablePath(const fs::path &path) {
  ...
  for (auto it = path.begin(); it != path.end(); ++it) components.push_back(it->string());

  return components.size() == 3 && components[0] == "store" && components[1].size() == 3  /// "364"
...
bool SystemRemoteDataPathsSource::nextFile() {
  while (true) {
    ...
    try {
      ...
      if (disk->existsFile(current_path)) return true;

      /// Files or directories can disappear due to concurrent operations
      if (!disk->existsFileOrDirectory(current_path)) continue;
      ...
      for (const auto &child : children) dir.names.push_back({child, skip_predicate});
      dir.position = -1;

      paths_stack.emplace_back(std::move(dir));
    } catch (const Exception &e) {
      /// Files or directories can disappear due to concurrent operations
      if (e.code() == ErrorCodes::FILE_DOESNT_EXIST || e.code() == ErrorCodes::DIRECTORY_DOESNT_EXIST) continue;
      ...
      if (e.code() == std::errc::permission_denied && isTablePath(current_path)) continue;
...
Chunk SystemRemoteDataPathsSource::generate() {
  ...
  if (current_disk >= static_cast<ssize_t>(disks.size())) return {};
  ...
  do {
    ...
    if (max_block_size > 0 && row_count > 0) {
      size_t total_size = col_disk_name->byteSize() + col_base_path->byteSize() + col_cache_base_path->byteSize() +
                          col_local_path->byteSize() + col_remote_path->byteSize() + col_size->byteSize() + col_namespace->byteSize() +
    ...
    const auto &skip_predicate = getCurrentSkipPredicate();
    if (skip_predicate && skip_predicate(local_path)) continue;

    FileCachePtr cache;

    if (disk->supportsCache()) cache = FileCacheFactory::instance().getByName(disk->getCacheName())->cache;
    ...
    try {
      ...
      if (e.code() == ErrorCodes::FILE_DOESNT_EXIST || e.code() == ErrorCodes::DIRECTORY_DOESNT_EXIST ||
          e.code() == ErrorCodes::ATTEMPT_TO_READ_AFTER_EOF || e.code() == ErrorCodes::CANNOT_READ_ALL_DATA)
        continue;
