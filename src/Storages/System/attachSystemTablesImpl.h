...
void attachImpl(ContextPtr context, IDatabase& system_database, const String& table_name, StringLiteral<CommentSize> comment,
                StorageArgs&&... args) {
  ...
  if (system_database.getUUID() == UUIDHelpers::Nil) {
    ...
    if constexpr (with_description)
      system_database.attachTable(
          context, table_name, std::make_shared<StorageT>(table_id, StorageT::getColumnsDescription(), std::forward<StorageArgs>(args)...),
          {});
    else
...
void attach(ContextPtr context, IDatabase& system_database, const String& table_name, StringLiteral<CommentSize> comment,
            StorageArgs&&... args) {
  attachImpl<StorageT, CommentSize, true>(context, system_database, table_name, comment, std::forward<StorageArgs>(args)...);
}

template <typename StorageT, int CommentSize, typename... StorageArgs>
void attachNoDescription(ContextPtr context, IDatabase& system_database, const String& table_name, StringLiteral<CommentSize> comment,
                         StorageArgs&&... args) {
