...
ColumnsDescription StorageSystemMutations::getColumnsDescription() {
  return ColumnsDescription{
      ...
      {"mutation_id", std::make_shared<DataTypeString>(),
       "The ID of the mutation. For replicated tables these IDs correspond to znode names in the "
       "<table_path_in_clickhouse_keeper>/mutations/ directory in ClickHouse Keeper. For non-replicated tables the IDs correspond to file "
       "names in the data directory of the table."},
      ...
      {"block_numbers.partition_id", std::make_shared<DataTypeArray>(std::make_shared<DataTypeString>()),
       "For mutations of replicated tables, the array contains the partitions' IDs (one record for each partition). For mutations of "
       "non-replicated tables the array is empty."},
      {"block_numbers.number", std::make_shared<DataTypeArray>(std::make_shared<DataTypeInt64>()),
       "For mutations of replicated tables, the array contains one record for each partition, with the block number that was acquired by "
       "the mutation. "
       ...
       "This means that for mutations of non-replicated tables, the column will contain one record with a single block number acquired by "
       "the mutation."},
      {"parts_in_progress_names", std::make_shared<DataTypeArray>(std::make_shared<DataTypeString>()),
       "An array of names of data parts that are currently being mutated."},
      {"parts_to_do_names", std::make_shared<DataTypeArray>(std::make_shared<DataTypeString>()),
       "An array of names of data parts that need to be mutated for the mutation to complete."},
      {"parts_to_do", std::make_shared<DataTypeInt64>(), "The number of data parts that need to be mutated for the mutation to complete."},
      {"parts_postpone_reasons", std::make_shared<DataTypeMap>(std::make_shared<DataTypeString>(), std::make_shared<DataTypeString>()),
       "A map of part names to reasons why they are postponed."},
      ...
      {"latest_fail_reason", std::make_shared<DataTypeString>(),
       "The exception message that caused the most recent part mutation failure."},
      {"latest_fail_error_code_name", std::make_shared<DataTypeString>(),
       "The error code of the exception that caused the most recent part mutation failure."},
  };
}
...
void StorageSystemMutations::fillData(MutableColumns &res_columns, ContextPtr context, const ActionsDAG::Node *predicate,
  ...
  for (const auto &db : DatabaseCatalog::instance().getDatabases(GetDatabasesOptions{.with_datalake_catalogs = false})) {
    ...
    for (auto iterator = db.second->getTablesIterator(context); iterator->isValid(); iterator->next()) {
      ...
      if (!dynamic_cast<const MergeTreeData *>(table.get())) continue;
  ...
  for (size_t i_storage = 0; i_storage < col_database->size(); ++i_storage) {
    ...
    std::vector<MergeTreeMutationStatus> statuses;
    {
      ...
      if (const auto *merge_tree = dynamic_cast<const MergeTreeData *>(storage)) statuses = merge_tree->getMutationsStatus();
    }
    ...
    for (const MergeTreeMutationStatus &status : statuses) {
      ...
      for (const String &part_name : status.parts_to_do_names) parts_to_do_names.emplace_back(part_name);

      Array parts_in_progress_names;
      parts_to_do_names.reserve(status.parts_in_progress_names.size());
      for (const String &part_name : status.parts_in_progress_names) parts_in_progress_names.emplace_back(part_name);
