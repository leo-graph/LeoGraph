...
ColumnsDescription StorageSystemZooKeeperInfo::getColumnsDescription() {
  ...
  return ColumnsDescription{
      ...
      /* 3 */
      {"index", std::make_shared<DataTypeNullable>(std::make_shared<DataTypeUInt8>()),
       "The index of the ZooKeeper node that ClickHouse connected to. The index is from ZooKeeper config. If not connected, this column is "
       "NULL."},
      ...
      /* 10 */
      {"packets_received", std::make_shared<DataTypeNullable>(std::make_shared<DataTypeUInt64>()), "The number of packets received."},
      /* 11 */ {"packets_sent", std::make_shared<DataTypeNullable>(std::make_shared<DataTypeUInt64>()), "The number of packets sent."},
      /* 12 */
      {"outstanding_requests", std::make_shared<DataTypeNullable>(std::make_shared<DataTypeUInt64>()),
      ...
      /* 18 */
      {"approximate_data_size", std::make_shared<DataTypeNullable>(std::make_shared<DataTypeUInt64>()), "The approximate data size."},
      /* 19 */
      {"followers", std::make_shared<DataTypeNullable>(std::make_shared<DataTypeUInt64>()),
       "The followers of the leader. This field is only exposed by the leader."},
      /* 20 */
      {"synced_followers", std::make_shared<DataTypeNullable>(std::make_shared<DataTypeUInt64>()),
       "The synced followers of the leader. This field is only exposed by the leader."},
      /* 21 */
      {"pending_syncs", std::make_shared<DataTypeNullable>(std::make_shared<DataTypeUInt64>()),
       "The pending syncs of the leader. This field is only exposed by the leader."},
      /* 22 */
      {"open_file_descriptor_count", std::make_shared<DataTypeNullable>(std::make_shared<DataTypeUInt64>()),
       "The open file descriptor count. Only available on Unix platforms."},
      /* 23 */
      {"max_file_descriptor_count", std::make_shared<DataTypeNullable>(std::make_shared<DataTypeUInt64>()),
       "The max file descriptor count. Only available on Unix platforms."},
      ...
      /// dirs command
      /* 28 */
      {"snapshot_dir_size", std::make_shared<DataTypeNullable>(std::make_shared<DataTypeUInt64>()),
       "The ZooKeeper snapshot directory size."},
      /* 29 */
      {"log_dir_size", std::make_shared<DataTypeNullable>(std::make_shared<DataTypeUInt64>()), "The ZooKeeper log directory size."},
      ...
      /* 34 */
      {"last_committed_idx", std::make_shared<DataTypeNullable>(std::make_shared<DataTypeUInt64>()), "The ZooKeeper last committed index."},
      /* 35 */
      {"leader_committed_log_idx", std::make_shared<DataTypeNullable>(std::make_shared<DataTypeUInt64>()),
       "The ZooKeeper leader committed log index."},
      /* 36 */
      {"target_committed_log_idx", std::make_shared<DataTypeNullable>(std::make_shared<DataTypeUInt64>()),
       "The ZooKeeper target committed log index."},
      /* 37 */
      {"last_snapshot_idx", std::make_shared<DataTypeNullable>(std::make_shared<DataTypeUInt64>()), "The ZooKeeper last snapshot index."},
  };
}
...
void StorageSystemZooKeeperInfo::fillData(MutableColumns &res_columns, ContextPtr context, const ActionsDAG::Node *,
    ...
    for (unsigned int index = 0; index < zk_args.hosts.size(); ++index) {
      ...
      if (mntr_output_expected.has_value()) {
        ...
        if (const auto it = mntr_responses_map.find("zk_followers"); it != mntr_responses_map.end()) {
          ...
          if (!followers_in_string.empty()) followers = parse<int>(followers_in_string);
        ...
        // /* 19 */ {"followers", std::make_shared<DataTypeUInt64>(), "The followers of the leader. This field is only exposed by the
        // leader."},
        if (const auto it = mntr_responses_map.find("zk_followers"); it != mntr_responses_map.end())
        ...
        // /* 20 */ {"synced_followers", std::make_shared<DataTypeUInt64>(), "The synced followers of the leader. This field is only exposed
        // by the leader."},
        // /* 21 */ {"pending_syncs", std::make_shared<DataTypeUInt64>(), "The pending syncs of the leader. This field is only exposed by
        // the leader."},
        ...
        // /* 22 */ {"open_file_descriptor_count", std::make_shared<DataTypeUInt64>(), "The open file descriptor count. Only available on
        // Unix platforms."},
        ...
        // /* 23 */ {"max_file_descriptor_count", std::make_shared<DataTypeUInt64>(), "The max file descriptor count. Only available on Unix
        // platforms."},
...
std::expected<String, String> StorageSystemZooKeeperInfo::sendFourLetterCommand(const String &host, UInt16 port,
