...
ColumnsDescription StorageSystemCompletions::getColumnsDescription() {
  auto description = ColumnsDescription{{"word", std::make_shared<DataTypeString>(), "Completion token."},
                                        {"context", std::make_shared<DataTypeString>(), "Token entity kind (e.g. table)."},
                                        {"belongs", std::make_shared<DataTypeNullable>(std::make_shared<DataTypeString>()),
                                         "Token for entity, this token belongs to (e.g. name of owning database)."}};
  return description;
}

void fillDataWithTableColumns(const String &database_name, const String &table_name, const StoragePtr &table, MutableColumns &res_columns,
  ...
  if (!table) return;  // table was dropped or deleted, while adding columns for previous table

  res_columns[0]->insert(table_name);
  res_columns[1]->insert(TABLE_CONTEXT);
  res_columns[2]->insert(database_name);

  if (!access->isGranted(AccessType::SHOW_COLUMNS) || !access->isGranted(AccessType::SHOW_COLUMNS, database_name, table_name)) return;

  auto table_lock = table->tryLockForShare(context->getCurrentQueryId(), context->getSettingsRef()[Setting::lock_acquire_timeout]);
  if (table_lock == nullptr) return;  // table was dropped while acquiring the lock
  ...
  for (const auto &column : columns) {
    if (!access->isGranted(AccessType::SHOW_COLUMNS, database_name, table_name, column.name)) continue;
...
void fillDataWithDatabasesTablesColumns(MutableColumns &res_columns, const ContextPtr &context) {
  ...
  for (const auto &[database_name, database_ptr] : databases) {
    if (!access->isGranted(AccessType::SHOW_DATABASES) || !access->isGranted(AccessType::SHOW_DATABASES, database_name)) continue;

    if (database_name == DatabaseCatalog::TEMPORARY_DATABASE) continue;  // skipping internal database temporary tables
...
void fillDataWithFunctions(MutableColumns &res_columns, const ContextPtr &context) {
  ...
  for (const auto &function_name : function_names) insert_function(function_name);
  ...
  for (const auto &function_name : aggregate_function_names) insert_function(function_name);
  const auto &user_defined_function_names = UserDefinedExecutableFunctionFactory::getRegisteredNames(context);
  for (const auto &function_name : user_defined_function_names) insert_function(function_name);
}
...
void fillDataWithAggregateFunctionCombinatorPair(MutableColumns &res_columns) {
  ...
  for (const auto &function_name : aggregate_functions) {
    for (const auto &[combinator_name, combinator] : aggregate_function_combinators) {
      if (combinator->isForInternalUsageOnly()) continue;
...
void fillDataWithDictionaries(MutableColumns &res_columns, const ContextPtr &context) {
  const auto &access = context->getAccess();
  if (!access->isGranted(AccessType::SHOW_DICTIONARIES)) return;
...
void StorageSystemCompletions::fillData(MutableColumns &res_columns, ContextPtr context, const ActionsDAG::Node *,
