...
class DetachedTablesBlockSource : public ISource {
 public:
  DetachedTablesBlockSource(std::vector<UInt8> columns_mask_, SharedHeader header_, UInt64 max_block_size_, ColumnPtr databases_,
                            ColumnPtr detached_tables_, ContextPtr context_)
  ...
  Chunk generate() override {
    ...
    for (; database_idx < databases->size(); ++database_idx) {
      ...
      for (; rows_count < max_block_size && detached_tables_it->isValid(); detached_tables_it->next()) {
        ...
        if (!detached_tables.contains(detached_table_name)) continue;

        if (need_to_check_access_for_tables && !access->isGranted(AccessType::SHOW_TABLES, database_name, detached_table_name)) continue;
      ...
      if (rows_count == max_block_size) {
        if (!detached_tables_it->isValid()) ++database_idx;
        break;
      }
    }

    if (databases->size() == database_idx) done = true;
 ...
 private:
  ...
  void fillResultColumnsByDetachedTableIterator(MutableColumns &result_columns) const {
    ...
    if (columns_mask[src_index++]) result_columns[res_index++]->insert(detached_tables_it->database());

    if (columns_mask[src_index++]) result_columns[res_index++]->insert(detached_tables_it->table());

    if (columns_mask[src_index++]) result_columns[res_index++]->insert(detached_tables_it->uuid());

    if (columns_mask[src_index++]) result_columns[res_index++]->insert(detached_tables_it->metadataPath());

    if (columns_mask[src_index++]) result_columns[res_index++]->insert(detached_tables_it->isPermanently());
  }
};
...
class ReadFromSystemDetachedTables : public SourceStepWithFilter {
 public:
  ...
  ReadFromSystemDetachedTables(const Names &column_names_, const SelectQueryInfo &query_info_, const StorageSnapshotPtr &storage_snapshot_,
...
void StorageSystemDetachedTables::read(QueryPlan &query_plan, const Names &column_names, const StorageSnapshotPtr &storage_snapshot,
  ...
  auto reading = std::make_unique<ReadFromSystemDetachedTables>(column_names, query_info, storage_snapshot, context, std::move(res_block),
                                                                std::move(columns_mask), max_block_size);

  query_plan.addStep(std::move(reading));
}

ReadFromSystemDetachedTables::ReadFromSystemDetachedTables(const Names &column_names_, const SelectQueryInfo &query_info_,
                                                           const StorageSnapshotPtr &storage_snapshot_, const ContextPtr &context_,
                                                           Block sample_block, std::vector<UInt8> columns_mask_, size_t max_block_size_)
...
void ReadFromSystemDetachedTables::initializePipeline(QueryPipelineBuilder &pipeline, const BuildQueryPipelineSettings &) {
  auto pipe =
      Pipe(std::make_shared<DetachedTablesBlockSource>(std::move(columns_mask), getOutputHeader(), max_block_size,
