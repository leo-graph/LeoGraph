...
class StatusRequestsPools final {
 public:
  explicit StatusRequestsPools(const size_t max_threads) : requests_without_zk_fields(max_threads), requests_with_zk_fields(max_threads) {}
...
class StatusRequestsPool final {
 ...
 public:
  explicit StatusRequestsPool(const size_t max_threads) : thread_pool(create_pool(max_threads)), log(getLogger("StatusRequestsPool")) {}
  ...
  void scheduleRequests(const UInt64 max_request_id, const QueryStatusPtr query_status) {
    while (true) {
      ...
      {
        std::lock_guard lock(mutex);
        if (requests_to_schedule.empty()) break;

        req = requests_to_schedule.front();

        if (req.reqiest_id > max_request_id) break;
      ...
      try {
        ...
        tryLogCurrentException(log, "Error scheduling get status task for " + get_holder_kind() + " " + get_holder_name(req.base_holder));
        req.promise->set_exception(std::current_exception());
        completeRequest(req.base_holder);
      }
    }
 ...
 private:
  ...
  static auto create_pool(const size_t max_threads) {
    if constexpr (std::is_same_v<TBaseHolder, IDatabase>) {
      return ThreadPool(CurrentMetrics::SystemDatabaseReplicasThreads, CurrentMetrics::SystemDatabaseReplicasThreadsActive,
                        CurrentMetrics::SystemDatabaseReplicasThreadsScheduled, max_threads);
    } else {
      return ThreadPool(CurrentMetrics::SystemReplicasThreads, CurrentMetrics::SystemReplicasThreadsActive,
                        CurrentMetrics::SystemReplicasThreadsScheduled, max_threads);
    }
  }
};
}  // namespace DB
