...
ColumnsDescription StorageSystemSettingsProfiles::getColumnsDescription() {
  return ColumnsDescription{
      ...
      {"storage", std::make_shared<DataTypeString>(),
       "Path to the storage of setting profiles. Configured in the `access_control_path` parameter."},
      {"num_elements", std::make_shared<DataTypeUInt64>(),
       "Number of elements for this profile in the `system.settings_profile_elements` table."},
...
void StorageSystemSettingsProfiles::fillData(MutableColumns &res_columns, ContextPtr context, const ActionsDAG::Node *,
                                             std::vector<UInt8>) const {
  /// If "select_from_system_db_requires_grant" is enabled the access rights were already checked in InterpreterSelectQuery.
  const auto &access_control = context->getAccessControl();
  if (!access_control.doesSelectFromSystemDatabaseRequireGrant()) context->checkAccess(AccessType::SHOW_SETTINGS_PROFILES);
  ...
  auto add_row = [&](const String &name, const UUID &id, const String &storage_name, const SettingsProfileElements &elements,
...
void StorageSystemSettingsProfiles::backupData(BackupEntriesCollector &backup_entries_collector, const String &data_path_in_backup,
...
void StorageSystemSettingsProfiles::restoreDataFromBackup(RestorerFromBackup &restorer, const String &data_path_in_backup,
                                                          const std::optional<ASTs> & /* partitions */) {
