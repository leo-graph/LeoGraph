...
ColumnsDescription StorageSystemScheduler::getColumnsDescription() {
  return ColumnsDescription{
      ...
      {"priority", std::make_shared<DataTypeInt64>(),
       "Priority of a node, used by a parent node of 'priority' type (Lower value means higher priority)."},
      ...
      {"vruntime", std::make_shared<DataTypeNullable>(std::make_shared<DataTypeFloat64>()),
       "For children of `fair` nodes only. Virtual runtime of a node used by SFQ algorithm to select the next child to process in a "
       "max-min fair manner."},
      ...
      {"inflight_requests", std::make_shared<DataTypeNullable>(std::make_shared<DataTypeInt64>()),
       "For `inflight_limit` nodes only. The number of resource requests dequeued from this node, that are currently in consumption "
       "state."},
...
void StorageSystemScheduler::fillData(MutableColumns &res_columns, ContextPtr context, const ActionsDAG::Node *, std::vector<UInt8>) const {
  context->getResourceManager()->forEachNode([&](const String &resource, const String &path, ISchedulerNode *node) {
    ...
    if (auto *parent = dynamic_cast<FairPolicy *>(node->parent)) {
      if (auto value = parent->getChildVRuntime(node)) vruntime = *value;
    }
    if (auto *ptr = dynamic_cast<FairPolicy *>(node)) system_vruntime = ptr->getSystemVRuntime();
    if (auto *ptr = dynamic_cast<FifoQueue *>(node)) std::tie(queue_length, queue_cost) = ptr->getQueueLengthAndCost();
    if (auto *ptr = dynamic_cast<ISchedulerQueue *>(node)) budget = ptr->getBudget();
    if (auto *ptr = dynamic_cast<ISchedulerConstraint *>(node)) is_satisfied = ptr->isSatisfied();
