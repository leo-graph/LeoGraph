...
StorageSystemColumns::StorageSystemColumns(const StorageID &table_id_) : IStorage(table_id_) {
  ...
  auto description = ColumnsDescription(
      {{"database", std::make_shared<DataTypeString>(), "Database name."},
       ...
       {"character_octet_length", std::make_shared<DataTypeNullable>(std::make_shared<DataTypeUInt64>()),
        "Maximum length in bytes for binary data, character data, or text data and images. In ClickHouse makes sense only for FixedString "
        "data type. Otherwise, the NULL value is returned."},
       ...
       {"numeric_precision_radix", std::make_shared<DataTypeNullable>(std::make_shared<DataTypeUInt64>()),
        "The base of the number system is the accuracy of approximate numeric data, exact numeric data, integer data or monetary data. In "
        "ClickHouse it's 2 for integer types and 10 for Decimal types. Otherwise, the NULL value is returned."},
       ...
       {"serialization_hint", std::make_shared<DataTypeNullable>(std::make_shared<DataTypeString>()),
  ...
  description.setAliases({{"column", std::make_shared<DataTypeString>(), "name"}});
...
class ColumnsSource : public ISource {
 public:
  ColumnsSource(std::vector<UInt8> columns_mask_, SharedHeader header_, UInt64 max_block_size_, ColumnPtr databases_, ColumnPtr tables_,
                Storages storages_, ContextPtr context)
      : ISource(header_),
 ...
 protected:
  Chunk generate() override {
    if (db_table_num >= total_tables) return {};
    ...
    while (rows_count < max_block_size && db_table_num < total_tables) {
...

      {
        ...
        if (auto hints = storage->tryGetSerializationHints()) serialization_hints = std::move(*hints);
        ...
        if (columns_mask[7] || columns_mask[8] || columns_mask[9]) {
          if (auto sizes = storage->tryGetColumnSizes()) column_sizes = std::move(*sizes);
        }

        if (columns_mask[11]) cols_required_for_partition_key = metadata_snapshot->getColumnsRequiredForPartitionKey();
        if (columns_mask[12]) cols_required_for_sorting_key = metadata_snapshot->getColumnsRequiredForSortingKey();
        if (columns_mask[13]) cols_required_for_primary_key = metadata_snapshot->getColumnsRequiredForPrimaryKey();
        if (columns_mask[14]) cols_required_for_sampling = metadata_snapshot->getColumnsRequiredForSampling();
      }
      ...
      bool need_to_check_access_for_columns =
      ...
      for (const auto &column : columns) {
        ...
        if (columns_mask[src_index++]) res_columns[res_index++]->insert(column.name);
        if (columns_mask[src_index++]) res_columns[res_index++]->insert(column.type->getName());
        if (columns_mask[src_index++]) res_columns[res_index++]->insert(position);

        if (column.default_desc.expression) {
          if (columns_mask[src_index++]) res_columns[res_index++]->insert(toString(column.default_desc.kind));
          if (columns_mask[src_index++]) res_columns[res_index++]->insert(column.default_desc.expression->formatForLogging());
        } else {
...

        {
          ...
          if (it == std::end(column_sizes)) {
            ...
            if (columns_mask[src_index++]) res_columns[res_index++]->insert(it->second.data_compressed);
            if (columns_mask[src_index++]) res_columns[res_index++]->insert(it->second.data_uncompressed);
            if (columns_mask[src_index++]) res_columns[res_index++]->insert(it->second.marks);
          }
        }

        if (columns_mask[src_index++]) res_columns[res_index++]->insert(column.comment);

        {
          ...
          if (columns_mask[src_index++]) res_columns[res_index++]->insert(find_in_vector(cols_required_for_partition_key));
          if (columns_mask[src_index++]) res_columns[res_index++]->insert(find_in_vector(cols_required_for_sorting_key));
          if (columns_mask[src_index++]) res_columns[res_index++]->insert(find_in_vector(cols_required_for_primary_key));
          if (columns_mask[src_index++]) res_columns[res_index++]->insert(find_in_vector(cols_required_for_sampling));
        }
...
class ReadFromSystemColumns : public SourceStepWithFilter {
 public:
  ...
  ReadFromSystemColumns(const Names &column_names_, const SelectQueryInfo &query_info_, const StorageSnapshotPtr &storage_snapshot_,
                        const ContextPtr &context_, Block sample_block, std::shared_ptr<StorageSystemColumns> storage_,
                        std::vector<UInt8> columns_mask_, size_t max_block_size_)
...
void ReadFromSystemColumns::applyFilters(ActionDAGNodes added_filter_nodes) {
  ...
  if (filter_actions_dag) {
    ...
    virtual_columns_filter =
        VirtualColumnUtils::splitFilterDagForAllowedInputs(filter_actions_dag->getOutputs().at(0), &block_to_filter, context);

    /// Must prepare sets here, initializePipeline() would be too late, see comment on FutureSetFromSubquery.
    if (virtual_columns_filter) VirtualColumnUtils::buildSetsForDAG(*virtual_columns_filter, context);
  }
}

void StorageSystemColumns::read(QueryPlan &query_plan, const Names &column_names, const StorageSnapshotPtr &storage_snapshot,
  ...
  auto reading = std::make_unique<ReadFromSystemColumns>(column_names, query_info, storage_snapshot, std::move(context), std::move(header),
                                                         std::move(this_ptr), std::move(columns_mask), max_block_size);
...
void ReadFromSystemColumns::initializePipeline(QueryPipelineBuilder &pipeline, const BuildQueryPipelineSettings &) {
...

  {
    ...
    if (context->hasSessionContext()) {
      ...
      if (!external_tables.empty()) database_column_mut->insertDefault();  /// Empty database for external tables.
    }
  ...
  pipes.emplace_back(std::make_shared<ColumnsSource>(std::move(columns_mask), std::move(header), max_block_size,
