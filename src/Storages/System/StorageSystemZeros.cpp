...
class ZerosSource : public ISource {
 public:
  ZerosSource(UInt64 block_size, UInt64 limit_, ZerosStatePtr state_) : ISource(createHeader()), limit(limit_), state(std::move(state_)) {
 ...
 protected:
  Chunk generate() override {
    ...
    if (limit) {
      ...
      if (generated_rows >= limit) return {};
...
Pipe StorageSystemZeros::read(const Names& column_names, const StorageSnapshotPtr& storage_snapshot, SelectQueryInfo& query_info,
  ...
  if (query_info.trivial_limit) query_limit = query_limit ? std::min(query_limit, query_info.trivial_limit) : query_info.trivial_limit;

  if (query_limit && query_limit < max_block_size) max_block_size = query_limit;
