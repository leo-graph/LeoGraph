...
ColumnsDescription StorageSystemFormats::getColumnsDescription() {
  return ColumnsDescription{
      ...
      {"supports_parallel_formatting", std::make_shared<DataTypeUInt8>(),
       "Flag that indicates whether the format supports parallel formatting."},
      {"is_tty_friendly", std::make_shared<DataTypeUInt8>(),
       "Flag that indicates whether the format usually displays fine in the terminal. For other formats, CLI will ask before output."},
      {"content_type", std::make_shared<DataTypeString>(),
       "HTTP Content-Type corresponding to the output format. May depend on the current format settings."},
      {"supports_random_access", std::make_shared<DataTypeUInt8>(),
       "Flag that indicates whether the format supports random access in the input."},
      {"has_schema_inference", std::make_shared<DataTypeUInt8>(),
       "The format can dynamically determine the schema from the data (either from embedded header/metadata or from the piece of data)."},
      {"has_external_schema", std::make_shared<DataTypeUInt8>(),
       "The format either has a fixed schema or accepts a predefined schema in its own format."},
      {"prefers_large_blocks", std::make_shared<DataTypeUInt8>(),
       "The format will write larger blocks into output and generate larger blocks on input."},
      {"supports_append", std::make_shared<DataTypeUInt8>(), "It's possible to append into a single file with this format."},
      {"supports_subsets_of_columns", std::make_shared<DataTypeUInt8>(),
       "The input format can recognize when certain columns are omitted."},
  };
}
...
void StorageSystemFormats::fillData(MutableColumns &res_columns, ContextPtr context, const ActionsDAG::Node *, std::vector<UInt8>) const {
  ...
  for (const auto &pair : formats) {
    ...
    bool has_input_format(creators.input_creator != nullptr || creators.random_access_input_creator != nullptr ||
                          creators.random_access_input_creator_with_metadata != nullptr);
    ...
    bool supports_random_access =
        creators.random_access_input_creator != nullptr || creators.random_access_input_creator_with_metadata != nullptr;
