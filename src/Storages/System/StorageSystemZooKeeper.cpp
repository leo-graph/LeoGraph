...
struct ZkNodeCache {
  ...
  void insert(const std::vector<String> &nodes, zkutil::ZooKeeperPtr zookeeper, const String &value_to_set, size_t index) {
    ...
    while (index < nodes.size() && nodes[index].empty()) ++index;
  ...
  void generateRequests(Coordination::Requests &requests) {
    ...
    for (const auto &[_, child] : children) child->generateRequests(requests);
  }
};
...
class ZooKeeperSink : public SinkToStorage {
 ...
 public:
  ZooKeeperSink(SharedHeader header, ContextPtr context_) : SinkToStorage(header), context(context_) {}
...
class ReadFromSystemZooKeeper final : public SourceStepWithFilter {
 public:
  ReadFromSystemZooKeeper(const Names &column_names_, const SelectQueryInfo &query_info_, const StorageSnapshotPtr &storage_snapshot_,
                          const ContextPtr &context_, const Block &header, UInt64 max_block_size_);
...
class SystemZooKeeperSource : public ISource {
 public:
  SystemZooKeeperSource(String &&zookeeper_name_, Paths &&paths_, SharedHeader header_, UInt64 max_block_size_, ContextPtr context_)
      : ISource(header_),
...
StorageSystemZooKeeper::StorageSystemZooKeeper(const StorageID &table_id_) : IStorage(table_id_) {
  StorageInMemoryMetadata storage_metadata;
  storage_metadata.setColumns(getColumnsDescription());
  setInMemoryMetadata(storage_metadata);
}

void StorageSystemZooKeeper::read(QueryPlan &query_plan, const Names &column_names, const StorageSnapshotPtr &storage_snapshot,
  ...
  auto read_step = std::make_unique<ReadFromSystemZooKeeper>(column_names, query_info, storage_snapshot, context, header, max_block_size);
  query_plan.addStep(std::move(read_step));
}

SinkToStoragePtr StorageSystemZooKeeper::write(const ASTPtr &, const StorageMetadataPtr &metadata, ContextPtr context,
                                               bool /*async_insert*/) {
...
ColumnsDescription StorageSystemZooKeeper::getColumnsDescription() {
  ...
  for (auto &name : description.getAllRegisteredNames()) {
    description.modify(name, [&](ColumnDescription &column) {
      ...
      if (column.name != "name" && column.name != "path" && column.name != "value" && column.name != "zookeeperName")
...
static String pathCorrected(const String &path) {
  ...
  if (path.empty() || path[0] != '/') path_corrected = '/';
  path_corrected += path;
  /// In all cases except the root, path must not end with a slash.
  if (path_corrected != "/" && path_corrected.back() == '/') path_corrected.resize(path_corrected.size() - 1);
  return path_corrected;
}

static bool isNameNode(const ActionsDAG::Node *node) {
  while (node->type == ActionsDAG::ActionType::ALIAS) node = node->children.at(0);
...
static void extractNameImpl(const ActionsDAG::Node &node, String &res, ContextPtr context) {
  if (node.type != ActionsDAG::ActionType::FUNCTION) return;

  auto function_name = node.function_base->getName();
  if (function_name == "and") {
    for (const auto *child : node.children) extractNameImpl(*child, res, context);
  ...
  if (function_name == "equals") {
    ...
    if (!value || !value->column) return;

    if (!isString(removeNullable(removeLowCardinality(value->result_type)))) return;

    if (value->column->size() != 1) return;
...
static void extractPathImpl(const ActionsDAG::Node &node, Paths &res, ContextPtr context, bool allow_unrestricted) {
  ...
  auto function_name = node.function_base->getName();
  if (function_name == "and") {
    for (const auto *child : node.children) extractPathImpl(*child, res, context, allow_unrestricted);
  ...
  if (function_name == "in") {
    if (!isPathNode(node.children.at(0))) return;
    ...
    if (!column_set) return;

    auto future_set = column_set->getData();
    if (!future_set) return;

    auto set = future_set->buildOrderedSetInplace(context);
    if (!set || !set->hasExplicitSetElements()) return;

    set->checkColumnsNumber(1);
    auto type = set->getElementsTypes()[0];
    if (!isString(removeNullable(removeLowCardinality(type)))) return;
...
static String extractName(const ActionsDAG::NodeRawConstPtrs &filter_nodes, ContextPtr context) {
  String res;
  for (const auto *node : filter_nodes) extractNameImpl(*node, res, context);

  if (res.empty()) return {String(zkutil::DEFAULT_ZOOKEEPER_NAME)};
...
static Paths extractPath(const ActionsDAG::NodeRawConstPtrs &filter_nodes, ContextPtr context, bool allow_unrestricted) {
  Paths res;
  for (const auto *node : filter_nodes) extractPathImpl(*node, res, context, allow_unrestricted);
  ...
  if (res.empty() && allow_unrestricted) res.insert({"/", ZkPathType::Recurse});
...
Chunk SystemZooKeeperSource::generate() {
  ...
  ZooKeeperRetriesInfo retries_seetings(settings[Setting::insert_keeper_max_retries],
                                        settings[Setting::insert_keeper_retry_initial_backoff_ms],
                                        settings[Setting::insert_keeper_retry_max_backoff_ms], query_status);

  /// Handles reconnects when needed
  auto get_zookeeper = [&]() {
    auto zookeeper = zookeepers.find(name);
    if (zookeeper == zookeepers.end() || zookeeper->second->expired()) {
      zookeepers[name] = ZooKeeperWithFaultInjection::createInstance(settings[Setting::insert_keeper_fault_injection_probability],
                                                                     settings[Setting::insert_keeper_fault_injection_seed],
                                                                     context->getDefaultOrAuxiliaryZooKeeper(name), "", nullptr);
    }
  ...
  while (!paths.empty()) {
    ...
    for (size_t list_task_idx = 0; list_task_idx < list_tasks.size(); ++list_task_idx) {
      ...
      if (list_result.error == Coordination::Error::ZNONODE) continue;
      ...
      if (task.path_part == "/") task.path_part.clear();

      if (!task.prefix.empty()) {
        // Remove nodes that do not match specified prefix
        std::erase_if(nodes, [&task](const String &node) { return !(task.path_part + '/' + node).starts_with(task.prefix); });
      }
    ...
    ZooKeeperRetriesControl("", nullptr, retries_seetings).retryLoop([&]() { get_responses = get_zookeeper()->tryGet(paths_to_get); });
    ...
    for (size_t i = 0, size = get_tasks.size(); i < size; ++i) {
      ...
      if (res.error == Coordination::Error::ZNONODE) continue;  /// Node was deleted meanwhile.
    ...
    for (size_t i = 0, size = get_tasks.size(); i < size; ++i) {
      ...
      if (auto [it, inserted] = visited.emplace(key); !inserted) continue;
...
ReadFromSystemZooKeeper::ReadFromSystemZooKeeper(const Names &column_names_, const SelectQueryInfo &query_info_,
                                                 const StorageSnapshotPtr &storage_snapshot_, const ContextPtr &context_,
                                                 const Block &header, UInt64 max_block_size_)
    : SourceStepWithFilter(std::make_shared<const Block>(header), column_names_, query_info_, storage_snapshot_, context_),
