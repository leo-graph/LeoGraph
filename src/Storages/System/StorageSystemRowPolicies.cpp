...
ColumnsDescription StorageSystemRowPolicies::getColumnsDescription() {
  ...
  for (auto filter_type : collections::range(RowPolicyFilterType::MAX)) {
    ...
    description.add(
        {filter_type_info.name, std::make_shared<DataTypeNullable>(std::make_shared<DataTypeString>()), filter_type_info.description});
  }
  ...
  description.add({"apply_to_all", std::make_shared<DataTypeUInt8>(), "Shows that the row policies set for all roles and/or users."});
...
void StorageSystemRowPolicies::fillData(MutableColumns &res_columns, ContextPtr context, const ActionsDAG::Node *,
                                        std::vector<UInt8>) const {
  /// If "select_from_system_db_requires_grant" is enabled the access rights were already checked in InterpreterSelectQuery.
  const auto &access_control = context->getAccessControl();
  if (!access_control.doesSelectFromSystemDatabaseRequireGrant()) context->checkAccess(AccessType::SHOW_ROW_POLICIES);
  ...
  for (auto filter_type : collections::range(RowPolicyFilterType::MAX)) {
    ...
    column_filter[filter_type_i] =
        &assert_cast<ColumnString &>(assert_cast<ColumnNullable &>(*res_columns[column_index]).getNestedColumn());
  ...
  auto add_row = [&](const String &name, const RowPolicyName &full_name, const UUID &id, const String &storage_name,
                     const std::array<String, static_cast<size_t>(RowPolicyFilterType::MAX)> &filters, bool is_restrictive,
  ...
  for (const auto &id : ids) {
    ...
    add_row(policy->getName(), policy->getFullName(), id, storage->getStorageName(), policy->filters, policy->isRestrictive(),
            policy->to_roles);
  }
}

void StorageSystemRowPolicies::backupData(BackupEntriesCollector &backup_entries_collector, const String &data_path_in_backup,
...
void StorageSystemRowPolicies::restoreDataFromBackup(RestorerFromBackup &restorer, const String &data_path_in_backup,
                                                     const std::optional<ASTs> & /* partitions */) {
