...
UInt64 getJeMallocValue(const char *name) {
  ...
  /// In this case variable 'size' will be changed from 8 to 4 and the 64 bit variable 'value' will hold the 32 bit actual value times 2^32
  /// on big-endian machines. We should right shift the value by 32 on big-endian machines(which is unnecessary on little-endian machines).
...
void fillJemallocBins(MutableColumns &) { LOG_INFO(getLogger("StorageSystemJemallocBins"), "jemalloc is not enabled"); }

#endif  // USE_JEMALLOC

StorageSystemJemallocBins::StorageSystemJemallocBins(const StorageID &table_id_) : IStorage(table_id_) {
...
Pipe StorageSystemJemallocBins::read(const Names &column_names, const StorageSnapshotPtr &storage_snapshot, SelectQueryInfo &,
