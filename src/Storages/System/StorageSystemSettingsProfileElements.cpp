...
const std::vector<std::pair<String, Int8>> &getSettingConstraintWritabilityEnumValues() {
  static const std::vector<std::pair<String, Int8>> values = [] {
    ...
    for (auto value : collections::range(SettingConstraintWritability::MAX)) res.emplace_back(toString(value), static_cast<Int8>(value));
    return res;
  }();
  return values;
}
...
ColumnsDescription StorageSystemSettingsProfileElements::getColumnsDescription() {
  return ColumnsDescription{
      ...
      {"writability", std::make_shared<DataTypeNullable>(std::make_shared<DataTypeEnum8>(getSettingConstraintWritabilityEnumValues())),
       "The property which shows whether a setting can be changed or not."},
...
void StorageSystemSettingsProfileElements::fillData(MutableColumns &res_columns, ContextPtr context, const ActionsDAG::Node *,
  ...
  auto add_rows_for_single_element = [&](const String &owner_name, AccessEntityType owner_type, const SettingsProfileElement &element,
                                         size_t &index) {
    ...
    if (inserted_setting_name || inserted_inherit_profile) {
      ...
      for (auto &res_column : res_columns) res_column->insertManyDefaults(new_num_rows - res_column->size());
    }
  };

  auto add_rows = [&](const String &owner_name, AccessEntityType owner_type, const SettingsProfileElements &elements) {
    size_t index = 0;
    for (const auto &element : elements) add_rows_for_single_element(owner_name, owner_type, element, index);
  };
