...
void StorageSystemQuotaLimits::fillData(MutableColumns &res_columns, ContextPtr context, const ActionsDAG::Node *,
  ...
  auto add_rows = [&](const String &quota_name, const std::vector<Quota::Limits> &all_limits) {
    for (const auto &limits : all_limits) add_row(quota_name, limits);
  };
