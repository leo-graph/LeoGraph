...
Field optionalTimeInMicroseconds(const TimePoint &time_point) {
  if (time_point == TimePoint{}) return {};
...
ColumnsDescription StorageSystemAsyncLoader::getColumnsDescription() {
  return ColumnsDescription{
      ...
      {"dependencies", std::make_shared<DataTypeArray>(std::make_shared<DataTypeUInt64>()),
       "List of IDs of jobs that should be done before this job."},
      ...
      {"status", std::make_shared<DataTypeEnum8>(getTypeEnumValues<LoadStatus>()),
       "Current load status of a job: PENDING: Load job is not started yet. OK: Load job executed and was successful. FAILED: Load job "
       "executed and failed. CANCELED: Load job is not going to be executed due to removal or dependency failure."},
      ...
      {"ready_seqno", std::make_shared<DataTypeNullable>(std::make_shared<DataTypeUInt64>()),
       "Not null for ready jobs. Worker pulls the next job to be executed from a ready queue of its pool. If there are multiple ready "
       "jobs, then job with the lowest value of `ready_seqno` is picked."},
      ...
      {"exception", std::make_shared<DataTypeNullable>(std::make_shared<DataTypeString>()),
       "Not null for failed and canceled jobs. Holds error message raised during query execution or error leading to cancelling of this "
       "job along with dependency failure chain of job names."},
      ...
      {"enqueue_time", std::make_shared<DataTypeNullable>(std::make_shared<DataTypeDateTime64>(TIME_SCALE)),
       "Time when job became ready and was enqueued into a ready queue of it's pool. Null if the job is not ready yet."},
      ...
      {"finish_time", std::make_shared<DataTypeNullable>(std::make_shared<DataTypeDateTime64>(TIME_SCALE)),
       "Time when job execution is finished. Null if the job is not finished yet."},
  };
}

void StorageSystemAsyncLoader::fillData(MutableColumns &res_columns, ContextPtr context, const ActionsDAG::Node *,
  ...
  for (const auto &state : async_loader.getJobStates()) {
    ...
    for (const auto &dep : state.job->dependencies) dependencies.emplace_back(dep->jobId());
    ...
    Field ready_seqno;
    if (state.ready_seqno) ready_seqno = state.ready_seqno;
