...
template <ObjectStorageType type>
void StorageSystemObjectStorageQueueSettings<type>::fillData(MutableColumns &res_columns, ContextPtr context, const ActionsDAG::Node *,
                                                             std::vector<UInt8>) const {
  auto add_table = [&](const DatabaseTablesIteratorPtr &it, StorageObjectStorageQueue &storage) {
    if (storage.getType() != type) return;
