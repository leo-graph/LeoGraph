...
class SourceState {
 ...
 public:
  explicit SourceState(StoragesInfoStream &&stream_) : stream(std::move(stream_)) {}
...
class DetachedPartsSource : public ISource {
 public:
  DetachedPartsSource(SharedHeader header_, std::shared_ptr<SourceState> state_, std::vector<UInt8> columns_mask_, UInt64 block_size_)
      : ISource(std::move(header_)), state(state_), columns_mask(std::move(columns_mask_)), block_size(block_size_) {}
 ...
 protected:
  ...
  Chunk generate() override {
    ...
    while (new_columns[0]->size() < block_size) {
      ...
      if (detached_parts.empty()) return nullWhenNoRows(std::move(new_columns));
 ...
 private:
  ...
  void calculatePartSizeOnDisk(size_t begin, std::vector<std::atomic<size_t>> &parts_sizes) {
    ...
    for (size_t i = 0; i < max_thread_to_run; ++i) {
      if (worker_state.next_task.load() >= worker_state.tasks.size()) break;
  ...
  void generateRows(MutableColumns &new_columns, size_t max_rows) {
    ...
    if (columns_mask[bytes_on_disk_col_idx]) calculatePartSizeOnDisk(begin, parts_sizes);
    ...
    for (auto p_id = begin; p_id < detached_parts.size(); ++p_id) {
      ...
      size_t src_index = 0;
      size_t res_index = 0;
      if (columns_mask[src_index++]) new_columns[res_index++]->insert(current_info.database);
      if (columns_mask[src_index++]) new_columns[res_index++]->insert(current_info.table);
      if (columns_mask[src_index++]) new_columns[res_index++]->insert(p.valid_name ? p.getPartitionId() : Field());
      if (columns_mask[src_index++]) new_columns[res_index++]->insert(p.dir_name);
      ...
      if (columns_mask[src_index++]) {
        ...
        try {
          modification_time =
              p.disk->getLastModified(fs::path(current_info.data->getRelativeDataPath()) / MergeTreeData::DETACHED_DIR_NAME / p.dir_name);
        } catch (const fs::filesystem_error &) {
          tryLogCurrentException(__PRETTY_FUNCTION__);
        }
        new_columns[res_index++]->insert(static_cast<UInt64>(modification_time.epochTime()));
      }
      if (columns_mask[src_index++]) new_columns[res_index++]->insert(p.disk->getName());
      if (columns_mask[src_index++])
        new_columns[res_index++]->insert(
            (fs::path(current_info.data->getFullPathOnDisk(p.disk)) / MergeTreeData::DETACHED_DIR_NAME / p.dir_name).string());
      if (columns_mask[src_index++]) new_columns[res_index++]->insert(p.valid_name ? p.prefix : Field());
      if (columns_mask[src_index++]) new_columns[res_index++]->insert(p.valid_name ? p.min_block : Field());
      if (columns_mask[src_index++]) new_columns[res_index++]->insert(p.valid_name ? p.max_block : Field());
      if (columns_mask[src_index++]) new_columns[res_index++]->insert(p.valid_name ? p.level : Field());
    }
...
StorageSystemDetachedParts::StorageSystemDetachedParts(const StorageID &table_id_) : IStorage(table_id_) {
  ...
  storage_metadata.setColumns(ColumnsDescription{{
      ...
      {"partition_id", std::make_shared<DataTypeNullable>(std::make_shared<DataTypeString>()),
       "The identifier of the partition this part belongs to."},
      ...
      {"modification_time", std::make_shared<DataTypeDateTime>(),
       "The time the directory with the data part was modified. This usually corresponds to the time when detach happened."},
...
class ReadFromSystemDetachedParts : public SourceStepWithFilter {
 public:
  ReadFromSystemDetachedParts(const Names &column_names_, const SelectQueryInfo &query_info_, const StorageSnapshotPtr &storage_snapshot_,
                              const ContextPtr &context_, Block sample_block, std::shared_ptr<StorageSystemDetachedParts> storage_,
                              std::vector<UInt8> columns_mask_, size_t max_block_size_, size_t num_streams_)
...
void ReadFromSystemDetachedParts::applyFilters(ActionDAGNodes added_filter_nodes) {
  ...
  if (filter_actions_dag) {
    ...
    if (filter) VirtualColumnUtils::buildSetsForDAG(*filter, context);
  }
}

void StorageSystemDetachedParts::read(QueryPlan &query_plan, const Names &column_names, const StorageSnapshotPtr &storage_snapshot,
  ...
  auto reading =
      std::make_unique<ReadFromSystemDetachedParts>(column_names, query_info, storage_snapshot, std::move(context), std::move(header),
                                                    std::move(this_ptr), std::move(columns_mask), max_block_size, num_streams);
