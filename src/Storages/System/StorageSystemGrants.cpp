...
ColumnsDescription StorageSystemGrants::getColumnsDescription() {
  return ColumnsDescription{
      ...
      {"access_type", std::make_shared<DataTypeEnum16>(StorageSystemPrivileges::getAccessTypeEnumValues()),
       "Access parameters for ClickHouse user account."},
      {"access_object", std::make_shared<DataTypeString>(),
       "Parameter for access_type. Contains: "
...
void StorageSystemGrants::fillData(MutableColumns &res_columns, ContextPtr context, const ActionsDAG::Node *, std::vector<UInt8>) const {
  ...
  if (!access_control.doesSelectFromSystemDatabaseRequireGrant()) context->checkAccess(AccessType::SHOW_USERS | AccessType::SHOW_ROLES);
  ...
  auto add_row = [&](const String &grantee_name, AccessEntityType grantee_type, AccessType access_type, const String &access_object,
                     const String *database, const String *table, const String *column, bool is_partial_revoke, bool grant_option) {
  ...
  auto add_rows = [&](const String &grantee_name, AccessEntityType grantee_type, const AccessRightsElements &elements) {
    for (const auto &element : elements) {
      auto access_types = element.access_flags.toAccessTypes();
      if (access_types.empty() || (!element.anyColumn() && element.columns.empty())) continue;
      ...
      if (element.anyColumn()) {
        for (const auto &access_type : access_types)
          add_row(grantee_name, grantee_type, access_type, element.parameter, database, table, nullptr, element.is_partial_revoke,
                  element.grant_option);
      } else {
        for (const auto &access_type : access_types)
          for (const auto &column : element.columns)
            add_row(grantee_name, grantee_type, access_type, element.parameter, database, table, &column, element.is_partial_revoke,
                    element.grant_option);
      }
    }
  };
