...
ColumnPtr getFilteredDatabases(const ActionsDAG::Node *predicate, ContextPtr context) {
  ...
  const auto databases = DatabaseCatalog::instance().getDatabases(
      GetDatabasesOptions{.with_datalake_catalogs = settings[Setting::show_data_lake_catalogs_in_system_tables]});
...
ColumnPtr getFilteredTables(const ActionsDAG::Node *predicate, const ColumnPtr &filtered_databases_column, ContextPtr context,
  ...
  Block sample{ColumnWithTypeAndName(nullptr, std::make_shared<DataTypeString>(), "name"),
  ...
  if (dag) {
    ...
    for (const auto *input : dag->getInputs()) {
      if (input->result_name == "engine") filter_by_engine = true;

      if (input->result_name == "uuid") filter_by_uuid = true;
    }

    if (filter_by_engine) engine_column = ColumnString::create();

    if (filter_by_uuid) uuid_column = ColumnUUID::create();
  }
    ...
    if (is_detached) {
      ...
      if (engine_column || uuid_column) {
        ...
        for (; table_it->isValid(); table_it->next()) {
          ...
          if (!table) continue;  /// Table was concurrently dropped and should be skipped
          table_column->insert(table_it->name());
          if (engine_column) engine_column->insert(table->getName());
          if (uuid_column) uuid_column->insert(table->getStorageID().uuid);
        }
      } else {
  ...
  if (engine_column) block.insert(ColumnWithTypeAndName(std::move(engine_column), std::make_shared<DataTypeString>(), "engine"));
  if (uuid_column) block.insert(ColumnWithTypeAndName(std::move(uuid_column), std::make_shared<DataTypeUUID>(), "uuid"));

  if (dag) VirtualColumnUtils::filterBlockWithExpression(VirtualColumnUtils::buildFilterExpression(std::move(*dag), context), block);
...
StorageSystemTables::StorageSystemTables(const StorageID &table_id_) : IStorage(table_id_) {
  ...
  auto description = ColumnsDescription{
      ...
      {"metadata_version", std::make_shared<DataTypeInt32>(),
      ...
      {"active_on_fly_data_mutations", std::make_shared<DataTypeUInt64>(),
       "Total number of active data mutations (UPDATEs and DELETEs) suitable for applying on the fly."},
      {"active_on_fly_alter_mutations", std::make_shared<DataTypeUInt64>(),
       "Total number of active alter mutations (MODIFY COLUMNs) suitable for applying on the fly."},
      {"active_on_fly_metadata_mutations", std::make_shared<DataTypeUInt64>(),
...
class TablesBlockSource : public ISource {
 public:
  TablesBlockSource(std::vector<UInt8> columns_mask_, SharedHeader header, UInt64 max_block_size_, ColumnPtr databases_, ColumnPtr tables_,
                    ContextPtr context_)
    ...
    tables.reserve(size);
    for (size_t idx = 0; idx < size; ++idx) tables.insert(std::string{tables_->getDataAt(idx)});
  }
 ...
 protected:
  NameToNameMap getSelectParamters(const StorageMetadataPtr &metadata_snapshot) {
    ...
    if (!inner_query || !inner_query->as<ASTSelectWithUnionQuery>()) return {};
  ...
  void fillParametralizedViewData(MutableColumns &columns, const StoragePtr &table, size_t &res_index) {
    if (table) {
      ...
      if (!query_parameters_array.empty()) {
        Array changes;
        for (const auto &[key, value] : query_parameters_array) changes.push_back(Tuple{key, value});
  ...
  size_t fillTableNamesOnly(MutableColumns &res_columns) {
    ...
    for (const auto &table_detail : table_details) {
      if (!tables.contains(table_detail.name)) continue;

      size_t src_index = 0;
      size_t res_index = 0;

      if (!access->isGranted(AccessType::SHOW_TABLES, database_name, table_detail.name)) continue;

      if (columns_mask[src_index++]) res_columns[res_index++]->insert(database_name);

      if (columns_mask[src_index++]) res_columns[res_index++]->insert(table_detail.name);
  ...
  Chunk generate() override {
    ...
    while (rows_count < max_block_size) {
      if (tables_it && !tables_it->isValid()) ++database_idx;
      ...
      if (database_idx >= databases->size()) {
        if (context->hasSessionContext()) {
          ...
          for (auto &table : external_tables) {
            ...
            // database
            if (columns_mask[src_index++]) res_columns[res_index++]->insertDefault();

            // name
            if (columns_mask[src_index++]) res_columns[res_index++]->insert(table.first);

            // uuid
            if (columns_mask[src_index++]) res_columns[res_index++]->insert(table.second->getStorageID().uuid);

            // engine
            if (columns_mask[src_index++]) res_columns[res_index++]->insert(table.second->getName());

            // is_temporary
            if (columns_mask[src_index++]) res_columns[res_index++]->insert(1u);
      ...
      bool needs_two_columns = (needed_columns.size() == 2 && ((needed_columns[0].name == "name" && needed_columns[1].name == "database") ||
      ...
      for (; rows_count < max_block_size && tables_it->isValid(); tables_it->next()) {
        ...
        if (!tables.contains(table_name)) continue;

        if (need_to_check_access_for_tables && !access->isGranted(AccessType::SHOW_TABLES, database_name, table_name)) continue;

        StoragePtr table = tables_it->table();
        if (!table) continue;  /// Table was concurrently dropped between iterator snapshot and table() call so we should skip it
        ...
        if (columns_mask[src_index++]) res_columns[res_index++]->insert(table_name);

        if (columns_mask[src_index++]) res_columns[res_index++]->insert(tables_it->uuid());
        ...
        if (columns_mask[src_index++]) res_columns[res_index++]->insert(0u);  // is_temporary
        ...
        if (columns_mask[src_index++]) {
          ...
          if (auto paths = table->tryGetDataPaths()) {
            ...
            for (const String &path : *paths) table_paths_array.push_back(path);
          }
        ...
        if (columns_mask[src_index++]) res_columns[res_index++]->insert(database->getObjectMetadataPath(table_name));

        if (columns_mask[src_index++])
          res_columns[res_index++]->insert(static_cast<UInt64>(database->getObjectMetadataModificationTime(table_name)));

        StorageMetadataPtr metadata_snapshot;
        if (table) metadata_snapshot = table->tryGetInMemoryMetadataPtr().value_or(nullptr);
...

        {
          ...
          if (columns_mask[src_index++]) res_columns[res_index++]->insert(views_database_name_array);

          if (columns_mask[src_index++]) res_columns[res_index++]->insert(views_table_name_array);
        }
        ...
        if (columns_mask[src_index] || columns_mask[src_index + 1] || columns_mask[src_index + 2]) {
          ...
          if (ast_create && !context->getSettingsRef()[Setting::show_table_uuid_in_table_create_query_if_not_nil]) {
            ...
            if (ast_create->targets) ast_create->targets->resetInnerUUIDs();
          }

          if (columns_mask[src_index++]) res_columns[res_index++]->insert(ast ? format({context, *ast}) : "");
          ...
          if (columns_mask[src_index++]) {
            ...
            if (ast_create && ast_create->storage) {
              ...
              if (startsWith(engine_full, extra_head)) engine_full = engine_full.substr(strlen(extra_head));
            }
          ...
          if (columns_mask[src_index++]) {
            String as_select;
            if (ast_create && ast_create->select) as_select = format({context, *ast_create->select});
        ...
        // parameterized view parameters
        if (columns_mask[src_index++]) fillParametralizedViewData(res_columns, table, res_index);
        ...
        if (table_merge_tree) {
          ...
          if (columns_mask[src_index++]) res_columns[res_index++]->insert(mutation_counters.num_data);
          if (columns_mask[src_index++]) res_columns[res_index++]->insert(mutation_counters.num_alter);
          if (columns_mask[src_index++]) res_columns[res_index++]->insert(mutation_counters.num_metadata);
        } else {
        ...
        if (columns_mask[src_index] || columns_mask[src_index + 1] || columns_mask[src_index + 2] || columns_mask[src_index + 3]) {
          ...
          if (columns_mask[src_index++]) res_columns[res_index++]->insert(dependencies_databases);
          if (columns_mask[src_index++]) res_columns[res_index++]->insert(dependencies_tables);

          if (columns_mask[src_index++]) res_columns[res_index++]->insert(dependents_databases);
          if (columns_mask[src_index++]) res_columns[res_index++]->insert(dependents_tables);
        } else {
          src_index += 4;
        }
...
class ReadFromSystemTables : public SourceStepWithFilter {
 public:
  ...
  ReadFromSystemTables(const Names &column_names_, const SelectQueryInfo &query_info_, const StorageSnapshotPtr &storage_snapshot_,
                       const ContextPtr &context_, Block sample_block, std::vector<UInt8> columns_mask_, size_t max_block_size_)
...
void StorageSystemTables::read(QueryPlan &query_plan, const Names &column_names, const StorageSnapshotPtr &storage_snapshot,
  ...
  auto reading = std::make_unique<ReadFromSystemTables>(column_names, query_info, storage_snapshot, context, std::move(res_block),
                                                        std::move(columns_mask), max_block_size);
...
void ReadFromSystemTables::initializePipeline(QueryPipelineBuilder &pipeline, const BuildQueryPipelineSettings &) {
  Pipe pipe(std::make_shared<TablesBlockSource>(std::move(columns_mask), getOutputHeader(), max_block_size,
                                                std::move(filtered_databases_column), std::move(filtered_tables_column), context));
