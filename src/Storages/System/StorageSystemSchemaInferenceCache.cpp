...
static String getSchemaString(const ColumnsDescription &columns) {
  ...
  for (auto it = names_and_types.begin(); it != names_and_types.end(); ++it) {
    if (it != names_and_types.begin()) writeCString(", ", buf);
...
ColumnsDescription StorageSystemSchemaInferenceCache::getColumnsDescription() {
  return ColumnsDescription{
      ...
      {"number_of_rows", std::make_shared<DataTypeNullable>(std::make_shared<DataTypeUInt64>()),
       "Number of rows in the file in given format. It's used for caching trivial count() from data files and for caching number of rows "
       "from the metadata during schema inference."},
...
void StorageSystemSchemaInferenceCache::fillData(MutableColumns &res_columns, ContextPtr context, const ActionsDAG::Node *,
