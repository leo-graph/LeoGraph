...
ColumnsDescription StorageSystemUserDirectories::getColumnsDescription() {
  return ColumnsDescription{
      ...
      {"precedence", std::make_shared<DataTypeUInt64>(),
       "The order in which this directory is declared in the config. The same order is used when ClickHouse tries to find a user or role."},
  };
}

void StorageSystemUserDirectories::fillData(MutableColumns &res_columns, ContextPtr context, const ActionsDAG::Node *,
