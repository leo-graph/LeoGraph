...
namespace DB {
StorageSystemProjections::StorageSystemProjections(const StorageID &table_id_) : IStorage(table_id_) {
  auto projection_type_datatype =
      std::make_shared<DataTypeEnum8>(DataTypeEnum8::Values{{"Normal", static_cast<UInt8>(ProjectionDescription::Type::Normal)},
...
class ProjectionsSource : public ISource {
 public:
  ProjectionsSource(std::vector<UInt8> columns_mask_, SharedHeader header, UInt64 max_block_size_, ColumnPtr databases_,
                    ContextPtr context_)
      : ISource(header),
    ...
    while (rows_count < max_block_size) {
      ...
      for (; rows_count < max_block_size && tables_it->isValid(); tables_it->next()) {
        ...
        for (const auto &projection : projections) {
          ...
          // 'name' column
          if (column_mask[src_index++]) res_columns[res_index++]->insert(projection.name);
          // 'type' column
          if (column_mask[src_index++]) res_columns[res_index++]->insert(projection.type);
          // 'sorting_key' column
...
class ReadFromSystemProjections : public SourceStepWithFilter {
 public:
  ...
  ReadFromSystemProjections(const Names &column_names_, const SelectQueryInfo &query_info_, const StorageSnapshotPtr &storage_snapshot_,
                            const ContextPtr &context_, Block sample_block, std::shared_ptr<StorageSystemProjections> storage_,
...
void StorageSystemProjections::read(QueryPlan &query_plan, const Names &column_names, const StorageSnapshotPtr &storage_snapshot,
  ...
  auto reading =
      std::make_unique<ReadFromSystemProjections>(column_names, query_info, storage_snapshot, std::move(context), std::move(header),
...
void ReadFromSystemProjections::initializePipeline(QueryPipelineBuilder &pipeline, const BuildQueryPipelineSettings &) {
  ...
  pipeline.init(Pipe(std::make_shared<ProjectionsSource>(std::move(columns_mask), getOutputHeader(), max_block_size,
