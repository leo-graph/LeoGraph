...
struct InputOrderInfo {
  ...
  InputOrderInfo(const SortDescription &sort_description_for_merging_, size_t used_prefix_of_sorting_key_size_, int direction_,
                 UInt64 limit_)
...
struct SelectQueryInfo {
  ...
  /// StorageMerge/StorageBuffer/StorageMaterializedView replace storage into query_tree. However, column types may be changed for inner
  /// table. So, resolved query tree might have incompatible types. StorageDistributed uses this query tree to calculate a header, throws if
  /// we use storage snapshot. To avoid this, we use initial_storage_snapshot.
