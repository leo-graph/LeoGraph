...
IndexDescription::IndexDescription(const IndexDescription& other)
  ...
  if (other.expression) expression = other.expression->clone();
}
...
IndexDescription IndexDescription::getIndexFromAST(const ASTPtr& definition_ast, const ColumnsDescription& columns,
                                                   bool is_implicitly_created, bool escape_filenames, ContextPtr context) {
  const auto* index_definition = definition_ast->as<ASTIndexDeclaration>();
  if (!index_definition) throw Exception(ErrorCodes::LOGICAL_ERROR, "Cannot create skip index from non ASTIndexDeclaration AST");

  if (index_definition->name.empty()) throw Exception(ErrorCodes::INCORRECT_QUERY, "Skip index must have name in definition.");

  auto index_type = index_definition->getType();
  if (!index_type) throw Exception(ErrorCodes::INCORRECT_QUERY, "TYPE is required for index");
  ...
  for (auto& elem : result.sample_block) {
    if (!elem.column) elem.column = elem.type->createColumn();
  ...
  if (index_type && index_type->arguments) result.arguments = index_type->arguments->clone();
...
void IndexDescription::initExpressionInfo(ASTPtr index_expression, const ColumnsDescription& columns, ContextPtr context) {
  ...
  if (expr_list == nullptr) throw Exception(ErrorCodes::LOGICAL_ERROR, "Expression is not set");
  ...
  TreeRewriterResultPtr syntax =
      TreeRewriter(context).analyze(expr_list, columns.get(GetColumnsOptions(GetColumnsOptions::AllPhysical).withSubcolumns()));
...
Field getFieldFromIndexArgumentAST(const ASTPtr& ast) {
  ...
  if (const auto* ast_literal = ast->as<ASTLiteral>()) return ast_literal->value;
  /// E.g. INDEX index_name column_name TYPE vector_similarity(index_name, column_name)
  if (const auto* ast_identifier = ast->as<ASTIdentifier>()) return Field(ast_identifier->name());
...
FieldVector getFieldsFromIndexArgumentsAST(const ASTPtr& arguments) {
  ...
  for (const auto& child : arguments->children) result.emplace_back(getFieldFromIndexArgumentAST(child));
  return result;
}
...
bool IndicesDescription::has(const String& name) const {
  for (const auto& index : *this)
    if (index.name == name) return true;
  return false;
}
...
bool IndicesDescription::hasType(const String& type) const {
  for (const auto& index : *this)
    if (index.type == type) return true;
  return false;
}
...
String IndicesDescription::explicitToString() const {
  ...
  for (const auto& index : *this) {
    if (!index.isImplicitlyCreated()) list.children.push_back(index.definition_ast);
  }
...
String IndicesDescription::allToString() const {
  ...
  ASTExpressionList list;
  for (const auto& index : *this) list.children.push_back(index.definition_ast);

  return list.formatWithSecretsOneLine();
}

IndicesDescription IndicesDescription::parse(const String& str, const ColumnsDescription& columns, bool escape_index_filenames,
                                             ContextPtr context) {
...
ExpressionActionsPtr IndicesDescription::getSingleExpressionForIndices(const ColumnsDescription& columns, ContextPtr context) const {
  ...
  for (const auto& index : *this)
    for (const auto& index_expr : index.expression_list_ast->children) combined_expr_list->children.push_back(index_expr->clone());

  auto syntax_result =
      TreeRewriter(context).analyze(combined_expr_list, columns.get(GetColumnsOptions(GetColumnsOptions::AllPhysical).withSubcolumns()));
...
ASTPtr createImplicitMinMaxIndexAST(const String& column_name) {
  ...
  auto index_ast = make_intrusive<ASTIndexDeclaration>(make_intrusive<ASTIdentifier>(column_name), index_type,
...
IndexDescription createImplicitMinMaxIndexDescription(const String& column_name, const ColumnsDescription& columns,
                                                      bool escape_index_filenames, ContextPtr context) {
