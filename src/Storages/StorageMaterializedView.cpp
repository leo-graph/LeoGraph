...
StorageMaterializedView::StorageMaterializedView(const StorageID &table_id_, ContextPtr local_context, const ASTCreateQuery &query,
                                                 const ColumnsDescription &columns_, LoadingStrictnessLevel mode, const String &comment,
                                                 bool is_restore_from_backup)
    : IStorage(table_id_), WithMutableContext(local_context->getGlobalContext()) {
  StorageInMemoryMetadata storage_metadata;
  storage_metadata.setColumns(columns_);

  if (query.sql_security) storage_metadata.setSQLSecurity(query.sql_security->as<ASTSQLSecurity &>());
  ...
  if (to_table_engine && to_table_engine->primary_key)
    storage_metadata.primary_key =
        KeyDescription::getKeyFromAST(to_table_engine->primary_key->ptr(), storage_metadata.columns, local_context->getGlobalContext());
  ...
  auto select =
      SelectQueryDescription::getSelectQueryFromASTForMatView(query.select->clone(), query.refresh_strategy != nullptr, mv_db_context);
  if (select.select_table_id) {
    ...
    if (max_materialized_views_count_for_table && select_table_dependent_views.size() >= max_materialized_views_count_for_table)
      throw Exception(ErrorCodes::TOO_MANY_MATERIALIZED_VIEWS, "Too many materialized views, maximum: {}",
                      max_materialized_views_count_for_table.value);
  }

  storage_metadata.setSelectQuery(select);
  if (!comment.empty()) storage_metadata.setComment(comment);
  if (query.refresh_strategy) storage_metadata.setRefresh(query.refresh_strategy->clone());

  setInMemoryMetadata(storage_metadata);

  bool point_to_itself_by_uuid = has_inner_table && to_inner_uuid != UUIDHelpers::Nil && to_inner_uuid == table_id_.uuid;
  bool point_to_itself_by_name =
      !has_inner_table && to_table_id.database_name == table_id_.database_name && to_table_id.table_name == table_id_.table_name;
  ...
  if (query.refresh_strategy) {
    ...
    if (is_replicated_db) {
      if (fixed_uuid) {
        ...
        RefreshSettings s;
        if (query.refresh_strategy->settings) s.applyChanges(query.refresh_strategy->settings->changes);
    ...
    if (mode < LoadingStrictnessLevel::ATTACH && !fixed_uuid) {
      ...
      if (has_inner_table) {
        ...
        if (storage && storage->engine) inner_engine = storage->engine->name;
      } else {
        if (auto task = getContext()->getRefreshSet().tryGetTaskForInnerTable(to_table_id))
          throw Exception(ErrorCodes::BAD_ARGUMENTS, "Table {} is already a target of another refreshable materialized view: {}",
                          to_table_id.getFullTableName(), task->getInfo().view_id.getFullTableName());

        StoragePtr inner_table = DatabaseCatalog::instance().tryGetTable(to_table_id, getContext());
        if (inner_table) inner_engine = inner_table->getName();
      }
      if (!inner_engine.empty()) {
        ...
        if (is_replicated_table && !is_replicated_db)
          throw Exception(
              ...
              "This combination doesn't work: refreshable materialized view, no APPEND, non-replicated database, replicated table. Each "
              "refresh would replace the replicated table locally, but other replicas wouldn't see it. Refusing to create");
        if (!is_replicated_table && refresh_coordinated)
          throw Exception(ErrorCodes::BAD_ARGUMENTS,
                          "This combination doesn't work: refreshable materialized view, no APPEND, replicated database, non-replicated "
                          "table. The refresh would be done on one replica, but the table would be replaced on other replicas too (with "
                          "empty tables). Refusing to create");
        ...
        /// e.g. if SELECTs are done using clusterAllReplicas(). (For the two disallowed cases above, clusterAllReplicas() wouldn't work
        /// reliably.)
      }
      ...
      if (!is_restore_from_backup && mode <= LoadingStrictnessLevel::CREATE) {
        ...
        refresh_context->checkAccess(AccessType::DROP_TABLE | AccessType::CREATE_TABLE | AccessType::SELECT | AccessType::INSERT,
                                     inner_db_name);
      }
    }

    refresher = RefreshTask::create(this, getContext(), *query.refresh_strategy, mode >= LoadingStrictnessLevel::ATTACH,
                                    refresh_coordinated, query.is_create_empty, is_restore_from_backup);
  }
  ...
  if (!fixed_uuid) {
    if (mode >= LoadingStrictnessLevel::ATTACH) {
      ...
      if (to_table_id.hasUUID())
        throw Exception(ErrorCodes::BAD_ARGUMENTS,
                        "explicit UUID is not allowed for target table of materialized view with REFRESH without APPEND");
    }
  }
  ...
  if (!has_inner_table) {
    ...
    if (storage_features.supports_skipping_indices) {
      if (query.columns_list->indices) new_columns_list->set(new_columns_list->indices, query.columns_list->indices->ptr());
      if (query.columns_list->constraints) new_columns_list->set(new_columns_list->constraints, query.columns_list->constraints->ptr());
      if (query.columns_list->primary_key) new_columns_list->set(new_columns_list->primary_key, query.columns_list->primary_key->ptr());
    ...
    if (storage_features.supports_projections) {
      if (query.columns_list->projections) new_columns_list->set(new_columns_list->projections, query.columns_list->projections->ptr());
    }

    manual_create_query->set(manual_create_query->columns_list, new_columns_list);

    if (to_table_engine) manual_create_query->set(manual_create_query->storage, to_table_engine);
...
QueryProcessingStage::Enum StorageMaterializedView::getQueryProcessingStage(ContextPtr local_context, QueryProcessingStage::Enum to_stage,
  ...
  return getTargetTable()->getQueryProcessingStage(local_context, to_stage,
                                                   getTargetTable()->getStorageSnapshot(target_metadata, local_context), query_info);
}
...
void StorageMaterializedView::read(QueryPlan &query_plan, const Names &column_names, const StorageSnapshotPtr &storage_snapshot,
  ...
  storage->read(query_plan, column_names, target_storage_snapshot, src_table_query_info, context, processed_stage, max_block_size,
                num_streams);
  ...
  if (query_plan.isInitialized()) {
    ...
    if (!blocksHaveEqualStructure(mv_header, target_header)) {
      auto converting_actions = ActionsDAG::makeConvertingActions(
          target_header.getColumnsWithTypeAndName(), mv_header.getColumnsWithTypeAndName(), ActionsDAG::MatchColumnsMode::Name, context);
...
SinkToStoragePtr StorageMaterializedView::write(const ASTPtr &query, const StorageMetadataPtr & /*metadata_snapshot*/,
...
void StorageMaterializedView::drop() {
  ...
  /// In this case, dropInnerTableIfAny will not find the inner table and will not drop it during executions of DROP query for the MV
  /// itself. DDLGuard does not protect from that, because RESTART REPLICA acquires DDLGuard for the inner table name, but DROP acquires
  /// DDLGuard for the name of MV. And we cannot acquire second DDLGuard for the inner name in DROP, because it may lead to
  /// lock-order-inversion (DDLGuards must be acquired in lexicographical order).
  dropInnerTableIfAny(/* sync */ false, getContext());

  if (refresher) refresher->drop(getContext());
}

void StorageMaterializedView::dropInnerTableIfAny(bool sync, ContextPtr local_context) {
  if (!has_inner_table) return;

  std::vector<StorageID> to_drop = {getTargetTableId()};
  if (!fixed_uuid) to_drop.push_back(StorageID(to_drop[0].getDatabaseName(), ".tmp" + to_drop[0].getTableName()));
  ...
  for (const StorageID &inner_table_id : to_drop) {
    ...
    if (table_exists)
      InterpreterDropQuery::executeDropQuery(ASTDropQuery::Kind::Drop, getContext(), local_context, inner_table_id, sync,
                                             /* ignore_sync_setting */ true, /*need_ddl_guard*/ false);
  }
}
...
void StorageMaterializedView::checkStatementCanBeForwarded() const {
  if (!has_inner_table)
    throw Exception(ErrorCodes::INCORRECT_QUERY,
                    ...
                    getTargetTableId().getNameForLogs());
}

bool StorageMaterializedView::optimize(const ASTPtr &query, const StorageMetadataPtr & /*metadata_snapshot*/, const ASTPtr &partition,
                                       bool final, bool deduplicate, const Names &deduplicate_by_columns, bool cleanup,
...
std::tuple<boost::intrusive_ptr<ASTInsertQuery>, CurrentThread::QueryScope> StorageMaterializedView::prepareRefresh(
    bool append, ContextMutablePtr refresh_context, std::optional<StorageID> &out_temp_table_id) const {
  ...
  if (!append) {
    ...
    if (create_query->targets) create_query->targets->resetInnerUUIDs();
  ...
  for (const String &name : header->getNames()) columns->children.push_back(make_intrusive<ASTIdentifier>(name));
...
void StorageMaterializedView::alter(const AlterCommands &params, ContextPtr local_context, AlterLockHolder &) {
  auto table_id = getStorageID();
  ...
  if (has_inner_table) {
    ...
    /// We need to copy the target table's columns (after checkTargetTableHasQueryOutputColumns() they can be still different - e.g. the
    /// data types of those columns can differ).
  ...
  if (old_metadata.sql_security_type == SQLSecurityType::DEFINER) instance.removeViewDependencies(table_id);

  if (new_metadata.sql_security_type == SQLSecurityType::DEFINER) instance.addViewDependency(*new_metadata.definer, table_id);

  setInMemoryMetadata(new_metadata);

  if (refresher) refresher->alterRefreshParams(new_metadata.refresh->as<const ASTRefreshStrategy &>());
}
...
void StorageMaterializedView::checkAlterIsPossible(const AlterCommands &commands, ContextPtr /*local_context*/) const {
  for (const auto &command : commands) {
    ...
    if (command.isCommentAlter()) continue;
    if (command.type == AlterCommand::MODIFY_QUERY) continue;
...
Pipe StorageMaterializedView::alterPartition(const StorageMetadataPtr &metadata_snapshot, const PartitionCommands &commands,
                                             ContextPtr local_context) {
  checkStatementCanBeForwarded();
  return getTargetTable()->alterPartition(metadata_snapshot, commands, local_context);
}

void StorageMaterializedView::checkAlterPartitionIsPossible(const PartitionCommands &commands, const StorageMetadataPtr &metadata_snapshot,
...
void StorageMaterializedView::renameInMemory(const StorageID &new_table_id) {
  ...
  if (refresher) refresher->rename(new_table_id, getTargetTableId());
}
...
void StorageMaterializedView::startup() {
  ...
  if (refresher) refresher->startup();
}

void StorageMaterializedView::flushAndPrepareForShutdown() {
  if (refresher) refresher->shutdown();
}
...
void StorageMaterializedView::shutdown(bool) {
  ...
  if (!select_query.select_table_id.empty()) DatabaseCatalog::instance().removeViewDependency(select_query.select_table_id, getStorageID());
}

bool StorageMaterializedView::canCreateOrDropOtherTables() const { return refresher && refresher->canCreateOrDropOtherTables(); }
...
Strings StorageMaterializedView::getDataPaths() const {
  if (auto table = tryGetTargetTable()) return table->getDataPaths();
  return {};
}
...
void StorageMaterializedView::backupData(BackupEntriesCollector &backup_entries_collector, const String &data_path_in_backup,
  ...
  if (hasInnerTable() && fixed_uuid) {
    if (auto table = tryGetTargetTable())
      ...
      LOG_WARNING(getLogger("StorageMaterializedView"), "Inner table does not exist, will not backup any data");
  }
}

void StorageMaterializedView::restoreDataFromBackup(RestorerFromBackup &restorer, const String &data_path_in_backup,
                                                    const std::optional<ASTs> &partitions) {
  if (hasInnerTable() && fixed_uuid) getTargetTable()->restoreDataFromBackup(restorer, data_path_in_backup, partitions);
}

void StorageMaterializedView::finalizeRestoreFromBackup() {
  if (refresher) refresher->finalizeRestoreFromBackup();
}

bool StorageMaterializedView::supportsBackupPartition() const {
  if (hasInnerTable() && fixed_uuid) return getTargetTable()->supportsBackupPartition();
  return false;
}

std::optional<UInt64> StorageMaterializedView::totalRows(ContextPtr query_context) const {
  if (hasInnerTable()) {
    if (auto table = tryGetTargetTable()) return table->totalRows(query_context);
  }
  return {};
}
...
std::optional<UInt64> StorageMaterializedView::totalBytes(ContextPtr query_context) const {
  if (hasInnerTable()) {
    if (auto table = tryGetTargetTable()) return table->totalBytes(query_context);
  }
  return {};
}
...
std::optional<UInt64> StorageMaterializedView::totalBytesUncompressed(const Settings &settings) const {
  if (hasInnerTable()) {
    if (auto table = tryGetTargetTable()) return table->totalBytesUncompressed(settings);
  }
  return {};
}

ActionLock StorageMaterializedView::getActionLock(StorageActionBlockType type) {
  if (type == ActionLocks::ViewRefresh && refresher) refresher->stop();
  if (has_inner_table) {
    if (auto target_table = tryGetTargetTable()) return target_table->getActionLock(type);
  }
  return ActionLock{};
}

bool StorageMaterializedView::isRemote() const {
  if (auto table = tryGetTargetTable()) return table->isRemote();
  return false;
}

void StorageMaterializedView::onActionLockRemove(StorageActionBlockType action_type) {
  if (action_type == ActionLocks::ViewRefresh && refresher) refresher->start();
}
...
StorageID StorageMaterializedView::getTargetTableId() const {
  ...
  if (!fixed_uuid) id.uuid = UUIDHelpers::Nil;
  return id;
}

void StorageMaterializedView::updateTargetTableId(std::optional<String> database_name, std::optional<String> table_name) {
  std::lock_guard guard(target_table_id_mutex);
  if (database_name) target_table_id.database_name = *std::move(database_name);
  if (table_name) target_table_id.table_name = *std::move(table_name);
}

String StorageMaterializedView::generateInnerTableName(const StorageID &view_id) {
  if (view_id.hasUUID()) return ".inner_id." + toString(view_id.uuid);
...
std::optional<NameSet> StorageMaterializedView::supportedPrewhereColumns() const {
  ...
  for (const auto &[name, type] : view_columns) {
    ...
    if (target_column && target_column->type->equals(*type)) supported_columns.insert(name);
  }
...
void registerStorageMaterializedView(StorageFactory &factory) {
  factory.registerStorage("MaterializedView", [](const StorageFactory::Arguments &args) {
    ...
    return std::make_shared<StorageMaterializedView>(args.table_id, args.getLocalContext(), args.query, args.columns, args.mode,
                                                     args.comment, args.is_restore_from_backup);
  });
}
