...
TimeSeriesInnerTablesCreator::TimeSeriesInnerTablesCreator(ContextPtr context_, StorageID time_series_storage_id_,
    ...
    case ViewTarget::Tags: {
      ...
      if (time_series_settings[TimeSeriesSetting::use_all_tags_column_to_generate_id]) {
        ...
        if (!all_tags_column.default_desc.expression) {
          ...
          all_tags_column.default_desc.expression =
              makeASTFunction("defaultValueOfTypeName", make_intrusive<ASTLiteral>(all_tags_column.type->getName()));
        }
      ...
      if (time_series_settings[TimeSeriesSetting::store_min_time_and_max_time]) {
        ...
        if (time_series_settings[TimeSeriesSetting::aggregate_min_time_and_max_time]) {
          ...
          auto custom_name =
              std::make_unique<DataTypeCustomSimpleAggregateFunction>(min_function, DataTypes{min_time_column.type}, Array{});
...
boost::intrusive_ptr<ASTCreateQuery> TimeSeriesInnerTablesCreator::getInnerTableCreateQuery(ViewTarget::Kind inner_table_kind,
  ...
  new_columns_list->set(new_columns_list->columns,
                        InterpreterCreateQuery::formatColumns(getInnerTableColumnsDescription(inner_table_kind)));
  manual_create_query->set(manual_create_query->columns_list, new_columns_list);

  if (inner_storage_def) manual_create_query->set(manual_create_query->storage, inner_storage_def->clone());

  return manual_create_query;
}

StorageID TimeSeriesInnerTablesCreator::createInnerTable(ViewTarget::Kind inner_table_kind, const UUID& inner_table_uuid,
                                                         const ASTStorage* inner_storage_def) const {
