...
void checkLabels(const google::protobuf::RepeatedPtrField<prometheus::Label> &labels) {
  ...
  for (size_t i = 0; i != static_cast<size_t>(labels.size()); ++i) {
    ...
    if (label_name.empty()) throw Exception(ErrorCodes::ILLEGAL_TIME_SERIES_TAGS, "Label name should not be empty");
    if (label_value.empty()) continue;  /// Empty label value is treated like the label doesn't exist.

    if (label_name == TimeSeriesTagNames::MetricName) metric_name_found = true;
...
const ColumnDescription &getInsertableColumnDescription(const ColumnsDescription &columns, const String &column_name,
                                                        const StorageID &time_series_storage_id) {
  const ColumnDescription *column = columns.tryGet(column_name);
  if (!column ||
      ((column->default_desc.kind != ColumnDefaultKind::Default) && (column->default_desc.kind != ColumnDefaultKind::Ephemeral))) {
    throw Exception(ErrorCodes::ILLEGAL_COLUMN, "{}: Column {} {}", time_series_storage_id.getNameForLogs(), column_name,
                    column ? "non-insertable" : "doesn't exist");
  }
  return *column;
}
...
IColumn &calculateId(const ContextPtr &context, const ColumnDescription &id_column_description, Block &tags_block) {
  ...
  auto adding_missing_defaults_dag =
      addMissingDefaults(pipe.getHeader(), header_with_id.getNamesAndTypesList(), ColumnsDescription{id_column_description}, context);
  ...
  auto convert_actions_dag =
      ActionsDAG::makeConvertingActions(pipe.getHeader().getColumnsWithTypeAndName(), header_with_id.getColumnsWithTypeAndName(),
  ...
  pipe.addSimpleTransform([&](const SharedHeader &stream_header) { return std::make_shared<ExpressionTransform>(stream_header, actions); });
  ...
  if (!id_column) id_column = id_type->createColumn();
...
DateTime64 scaleTimestamp(Int64 timestamp_ms, UInt32 scale) {
  if (scale == 3) return timestamp_ms;
  if (scale > 3) return timestamp_ms * DecimalUtils::scaleMultiplier<DateTime64>(scale - 3);
...
BlocksToInsert toBlocks(const google::protobuf::RepeatedPtrField<prometheus::TimeSeries> &time_series, const ContextPtr &context,
                        const StorageID &time_series_storage_id, const StorageInMemoryMetadata &time_series_storage_metadata,
                        const TimeSeriesSettings &time_series_settings) {
  size_t num_tags_rows = time_series.size();

  size_t num_data_rows = 0;
  for (const auto &element : time_series) num_data_rows += element.samples_size();

  if (!num_data_rows) return {};  /// Nothing to insert into target tables.
  ...
  for (size_t i = 0; i != static_cast<size_t>(time_series.size()); ++i) {
    ...
    if (!element.samples_size()) continue;
    ...
    if (time_series_settings[TimeSeriesSetting::use_all_tags_column_to_generate_id]) all_tags_offsets->push_back(all_tags_names->size());
    ...
    for (auto *column : columns_to_fill_in_tags_table) {
      if (column->size() == current_row_in_tags) column->insertDefault();
    }
...
std::string_view metricTypeToString(prometheus::MetricMetadata::MetricType metric_type) {
  ...
  switch (metric_type) {
    case prometheus::MetricMetadata::UNKNOWN:
      return "unknown"sv;
...
BlocksToInsert toBlocks(const google::protobuf::RepeatedPtrField<prometheus::MetricMetadata> &metrics_metadata,
  ...
  if (!num_rows) return {};  /// Nothing to insert into target tables.
...
void insertToTargetTables(BlocksToInsert &&blocks, StorageTimeSeries &time_series_storage, ContextPtr context, Poco::Logger *log) {
  ...
  for (auto &[table_kind, block] : blocks.blocks) {
    if (!block.empty()) {
      ...
      LOG_INFO(log, "{}: Inserting {} rows to the {} table", time_series_storage_id.getNameForLogs(), block.rows(), toString(table_kind));
      ...
      for (const auto &name : block.getNames()) columns_ast->children.emplace_back(make_intrusive<ASTIdentifier>(name));
...
void PrometheusRemoteWriteProtocol::writeTimeSeries(const google::protobuf::RepeatedPtrField<prometheus::TimeSeries> &time_series) {
  ...
  LOG_TRACE(log, "{}: Writing {} time series", time_series_storage_id.getNameForLogs(), time_series.size());
  ...
  LOG_TRACE(log, "{}: {} time series written", time_series_storage_id.getNameForLogs(), time_series.size());
}

void PrometheusRemoteWriteProtocol::writeMetricsMetadata(
    const google::protobuf::RepeatedPtrField<prometheus::MetricMetadata> &metrics_metadata) {
  auto time_series_storage_id = time_series_storage->getStorageID();

  LOG_TRACE(log, "{}: Writing {} metrics metadata", time_series_storage_id.getNameForLogs(), metrics_metadata.size());
  ...
  LOG_TRACE(log, "{}: {} metrics metadata written", time_series_storage_id.getNameForLogs(), metrics_metadata.size());
}
