...
ASTPtr buildSelectQueryForReadingTimeSeries(const StorageID &time_series_storage_id,
                                            const google::protobuf::RepeatedPtrField<prometheus::LabelMatcher> &label_matchers,
                                            Int64 min_time_ms, Int64 max_time_ms) {
...

  {
    ...
    select_list_exp->children.push_back(makeASTFunction("timeSeriesGroupArray",
...

  {
    ...
    table_exp->table_function = makeASTFunction("timeSeriesSelector", make_intrusive<ASTLiteral>(time_series_storage_id.getDatabaseName()),
...

  {
    ...
    group_by_list->children.push_back(makeASTFunction("timeSeriesIdToGroup", make_intrusive<ASTIdentifier>(TimeSeriesColumnNames::ID)));
...
void convertBlockToProtobuf(Block &&block, google::protobuf::RepeatedPtrField<prometheus::TimeSeries> &out_time_series) {
...
void PrometheusRemoteReadProtocol::readTimeSeries(google::protobuf::RepeatedPtrField<prometheus::TimeSeries> &out_time_series,
  ...
  ASTPtr select_query = buildSelectQueryForReadingTimeSeries(time_series_storage_id, label_matcher, start_timestamp_ms, end_timestamp_ms);

  LOG_TRACE(log, "{}: Executing query {}", time_series_storage_id.getNameForLogs(), select_query->formatForLogging());
  ...
  while (executor.pull(block)) {
    LOG_TRACE(log, "{}: Pulled block with {} columns and {} rows", time_series_storage_id.getNameForLogs(), block.columns(), block.rows());

    if (!block.empty()) convertBlockToProtobuf(std::move(block), out_time_series);
  }

  LOG_TRACE(log, "{}: {} time series read", time_series_storage_id.getNameForLogs(), out_time_series.size());
}
