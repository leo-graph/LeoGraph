...
SQLQueryPiece fromLiteral(const PQT::Scalar* scalar_node, ConverterContext& context) {
  ...
  if (node_range.empty()) return SQLQueryPiece{scalar_node, ResultType::SCALAR, StoreMethod::EMPTY};
...
SQLQueryPiece fromLiteral(const PQT::StringLiteral* string_node, ConverterContext& context) {
  ...
  if (node_range.empty()) return SQLQueryPiece{string_node, ResultType::STRING, StoreMethod::EMPTY};
