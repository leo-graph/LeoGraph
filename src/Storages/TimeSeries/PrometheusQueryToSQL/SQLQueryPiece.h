...
struct SQLQueryPiece {
  SQLQueryPiece(const Node* node_, ResultType type_, StoreMethod store_method_) : node(node_), type(type_), store_method(store_method_) {}
  ...
  /// If `store_method` is VECTOR_GRID then the SELECT query outputs two columns `group` (UInt64), `values`
  /// (Array(Nullable(scalar_data_type))). If `store_method` is RAW_DATA then the SELECT query outputs three columns `group` (UInt64),
  /// `timestamp` (timestamp_data_type), `value` (scalar_data_type). If `store_method` is CONST_SCALAR or CONST_STRING then the SELECT query
  /// is not used.
  ASTPtr select_query;
};
