...
SQLQueryPiece offsetEvaluationTime(const PQT::Offset* offset_node, SQLQueryPiece&& expression, DurationType offset_value,
  ...
  switch (expression.store_method) {
    ...
    case StoreMethod::RAW_DATA: {
      ...
      if (isDateTime64(context.timestamp_data_type)) {
        ...
        static const std::string_view to_interval_functions[] = {"toIntervalSecond", "toIntervalMillisecond", "toIntervalMicrosecond",
                                                                 "toIntervalNanosecond"};
        std::string_view to_interval_function = to_interval_functions[scale / 3];

        new_timestamp = makeASTFunction("plus", make_intrusive<ASTIdentifier>(ColumnNames::Timestamp),
...
SQLQueryPiece setEvaluationTime(const PQT::Offset* offset_node, SQLQueryPiece&& expression, ConverterContext& context) {
  ...
  if (expression.start_time != expression.end_time) {
    throw Exception(ErrorCodes::LOGICAL_ERROR, "Expression {} is expected to be calculated at a fixed evaluation time",
                    getPromQLText(expression, context));
  }

  auto node_range = context.node_range_getter.get(offset_node);
  if (node_range.empty()) return SQLQueryPiece{offset_node, offset_node->result_type, StoreMethod::EMPTY};
  ...
  switch (expression.store_method) {
    ...
    case StoreMethod::VECTOR_GRID: {
      ...
      auto new_values = makeASTFunction(
          "arrayResize", make_intrusive<ASTLiteral>(Array{}),
    ...
    case StoreMethod::RAW_DATA: {
      ...
      auto new_timestamp = makeASTFunction(
          "arrayJoin", makeASTFunction("timeSeriesRange", timeSeriesTimestampToAST(node_range.start_time, context.timestamp_data_type),
