...
[[noreturn]] void throwCannotFinalize(const SQLQueryPiece& result, const ConverterContext& context) {
  throw Exception(ErrorCodes::LOGICAL_ERROR,
                  ...
                  getPromQLText(result, context), result.type, result.store_method, toString(result.start_time, context.timestamp_scale),
                  toString(result.end_time, context.timestamp_scale), toString(result.step, context.timestamp_scale));
}
...
ASTPtr finalizeScalarAsSQL(SQLQueryPiece&& result, ConverterContext& context) {
  ...
  if (result.start_time != result.end_time) throwCannotFinalize(result, context);
...
ASTPtr finalizeInstantVectorAsSQL(SQLQueryPiece&& result, ConverterContext& context) {
  ...
  switch (result.store_method) {
    case StoreMethod::EMPTY: {
      ...
      String structure = fmt::format("{} Array(Tuple(String, String)), {} {}, {} {}", ColumnNames::Tags, ColumnNames::Timestamp,
                                     context.timestamp_data_type->getName(), ColumnNames::Value, context.scalar_data_type->getName());
    ...
    case StoreMethod::VECTOR_GRID: {
      ...
      value = timeSeriesScalarASTCast(
          makeASTFunction("assumeNotNull", makeASTFunction("arrayElement", make_intrusive<ASTIdentifier>(ColumnNames::Values),
      ...
      where = makeASTFunction(
          "isNotNull", makeASTFunction("arrayElement", make_intrusive<ASTIdentifier>(ColumnNames::Values), make_intrusive<ASTLiteral>(1u)));
      break;
    }
  ...
  if (!tags) {
    ...
    tags = makeASTFunction("materialize", makeASTFunction("CAST", make_intrusive<ASTLiteral>(Array{}),
                                                          make_intrusive<ASTLiteral>("Array(Tuple(String, String))")));
    tags->setAlias(ColumnNames::Tags);
  }
...
ASTPtr finalizeRangeVectorAsSQL(SQLQueryPiece&& result, ConverterContext& context) {
  ...
  switch (result.store_method) {
    case StoreMethod::EMPTY: {
      ...
      String structure = fmt::format("{} Array(Tuple(String, String)), {} Array(Tuple({}, {}))", ColumnNames::Tags, ColumnNames::TimeSeries,
                                     context.timestamp_data_type->getName(), context.scalar_data_type->getName());
    ...
    case StoreMethod::CONST_SCALAR: {
      ...
      values = makeASTFunction("arrayResize", make_intrusive<ASTLiteral>(Array{}),
    ...
    case StoreMethod::SCALAR_GRID: {
      ...
      values = makeASTFunction("CAST", make_intrusive<ASTIdentifier>(ColumnNames::Values),
    ...
    case StoreMethod::RAW_DATA: {
      ...
      time_series =
          makeASTFunction("timeSeriesGroupArray",
  ...
  if (!time_series) {
    ...
    chassert(values);
    time_series = makeASTFunction("timeSeriesFromGrid", timeSeriesTimestampToAST(result.start_time, context.timestamp_data_type),
                                  timeSeriesTimestampToAST(result.end_time, context.timestamp_data_type),
                                  timeSeriesDurationToAST(result.step, context.timestamp_data_type), std::move(values));
    time_series->setAlias(ColumnNames::TimeSeries);
  }
