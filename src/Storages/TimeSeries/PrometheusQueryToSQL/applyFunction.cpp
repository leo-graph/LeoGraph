...
SQLQueryPiece applyFunction(const PQT::Function* function_node, std::vector<SQLQueryPiece>&& arguments, ConverterContext& context) {
  ...
  if (isFunctionVector(function_name)) return applyFunctionVector(function_node, std::move(arguments), context);

  if (isFunctionScalar(function_name)) return applyFunctionScalar(function_node, std::move(arguments), context);

  if (isFunctionTime(function_name)) return fromFunctionTime(function_node, std::move(arguments), context);

  if (isDateTimeFunction(function_name)) return applyDateTimeFunction(function_node, std::move(arguments), context);

  if (isMathSimpleFunction(function_name)) return applyMathSimpleFunction(function_node, std::move(arguments), context);

  if (isFunctionPi(function_name)) return fromFunctionPi(function_node, std::move(arguments), context);

  if (isFunctionOverRange(function_name)) return applyFunctionOverRange(function_node, std::move(arguments), context);
