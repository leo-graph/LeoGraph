...
NodeEvaluationRangeGetter::NodeEvaluationRangeGetter(std::shared_ptr<const PrometheusQueryTree> promql_tree_,
  ...
  if (promql_tree->getTimestampScale() != timestamp_scale) {
    throw Exception(ErrorCodes::LOGICAL_ERROR, "Got two different timestamp scales: {} and {}", promql_tree->getTimestampScale(),
                    timestamp_scale);
  }
  ...
  if (settings_.use_current_time) {
    ...
    if (!settings_.start_time) throw Exception(ErrorCodes::BAD_ARGUMENTS, "start_time is not specified");
    if (!settings_.end_time) throw Exception(ErrorCodes::BAD_ARGUMENTS, "end_time is not specified");
    ...
    if (*settings_.start_time < *settings_.end_time) {
      if (!settings_.step) throw Exception(ErrorCodes::BAD_ARGUMENTS, "step is not specified");
      if (*settings_.step <= 0) throw Exception(ErrorCodes::BAD_ARGUMENTS, "step must be positive");
    }
...
void NodeEvaluationRangeGetter::visitChildren(const Node *node, const NodeEvaluationRange &range) {
  switch (node->node_type) {
    ...
    default: {
      for (const auto *child : node->children) visitNode(child, range);
    }
  }
}
...
void NodeEvaluationRangeGetter::propagateRangeToParents(const PQT::Node *node, Decimal64 range) {
  ...
  while (parent) {
    ...
    if (parent->result_type != ResultType::RANGE_VECTOR) break;
