...
void checkArgumentTypes(const PQT::Function* function_node, const std::vector<SQLQueryPiece>& arguments, const ConverterContext& context) {
  ...
  if (arguments.size() != 1) {
    throw Exception(ErrorCodes::CANNOT_EXECUTE_PROMQL_QUERY, "Function '{}' expects {} arguments, but was called with {} arguments",
  ...
  if (argument.type != ResultType::INSTANT_VECTOR) {
    throw Exception(ErrorCodes::CANNOT_EXECUTE_PROMQL_QUERY, "Function '{}' expects an argument of type {}, but expression {} has type {}",
                    function_name, ResultType::INSTANT_VECTOR, getPromQLText(argument, context), argument.type);
  }
}
      ...
      {"day_of_week",
       {
           ...
           [](ASTPtr t) -> ASTPtr { return makeASTFunction("toDayOfWeek", std::move(t), /* mode = */ make_intrusive<ASTLiteral>(2u)); },
       }},
      ...
      {"days_in_month",
       {
           ...
           [](ASTPtr t) -> ASTPtr {
             ...
             return makeASTFunction("plus",
                                    makeASTFunction("dateDiff", make_intrusive<ASTLiteral>("days"), makeASTFunction("toStartOfMonth", t),
...
bool isDateTimeFunction(std::string_view function_name) { return getImplInfo(function_name) != nullptr; }
    ...
    case StoreMethod::SINGLE_SCALAR: {
      ...
      ASTPtr new_value = timeSeriesScalarASTCast(
          (impl_info->transform_ast)(
              makeASTFunction("toDateTime64", std::move(current_value), make_intrusive<ASTLiteral>(0u), make_intrusive<ASTLiteral>("UTC"))),
    ...
    case StoreMethod::VECTOR_GRID: {
      ...
      builder.select_list.push_back(
          makeASTFunction("arrayMap",
                          makeASTFunction("lambda", makeASTFunction("tuple", make_intrusive<ASTIdentifier>("x")),
                                          timeSeriesScalarASTCast((impl_info->transform_ast)(makeASTFunction(
                                                                      "toDateTime64", make_intrusive<ASTIdentifier>("x"),
                                                                      make_intrusive<ASTLiteral>(0u), make_intrusive<ASTLiteral>("UTC"))),
                                                                  context.scalar_data_type)),
