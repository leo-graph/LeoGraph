...
void checkPrometheusQueryAllowsEvaluationRange(const PQT& promql_tree) {
  if ((promql_tree.getResultType() == ResultType::SCALAR) || (promql_tree.getResultType() == ResultType::INSTANT_VECTOR)) return;

  throw Exception(ErrorCodes::BAD_ARGUMENTS, "Invalid expression type {} for range query, must be {} or {}", promql_tree.getResultType(),
                  ResultType::SCALAR, ResultType::INSTANT_VECTOR);
}
}  // namespace
