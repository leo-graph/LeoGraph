...
ASTPtr SelectQueryBuilder::getSelectQuery() {
  ...
  if (having) select_query->setExpression(ASTSelectQuery::Expression::HAVING, std::move(having));
  ...
  if (where) select_query->setExpression(ASTSelectQuery::Expression::WHERE, std::move(where));

  if (limit) select_query->setExpression(ASTSelectQuery::Expression::LIMIT_LENGTH, make_intrusive<ASTLiteral>(*limit));
  ...
  if (!union_table.empty()) {
    ...
    for (const auto& select_exp : select_list) select_list_exp->children.push_back(select_exp->clone());
    select_query->setExpression(ASTSelectQuery::Expression::SELECT, std::move(select_list_exp));
