...
TimeSeriesColumnsValidator::TimeSeriesColumnsValidator(StorageID time_series_storage_id_,
    ...
    : time_series_storage_id(std::move(time_series_storage_id_)), time_series_settings(time_series_settings_) {}
...
void TimeSeriesColumnsValidator::validateTargetColumns(ViewTarget::Kind target_kind, const StorageID &target_table_id,
                                                       const ColumnsDescription &target_columns) const {
  try {
...
void TimeSeriesColumnsValidator::validateColumnForTimestamp(const ColumnDescription &column) const {
  if (!isDateTime64(removeNullable(column.type))) {
    throw Exception(ErrorCodes::INCOMPATIBLE_COLUMNS, "Column {} has illegal data type {}, expected DateTime64", column.name,
                    column.type->getName());
  }
}
...
void TimeSeriesColumnsValidator::validateColumnForValue(const ColumnDescription &column) const {
  if (!isFloat(removeNullable(column.type))) {
    throw Exception(ErrorCodes::INCOMPATIBLE_COLUMNS, "Column {} has illegal data type {}, expected Float32 or Float64", column.name,
                    column.type->getName());
  }
}

void TimeSeriesColumnsValidator::validateColumnForMetricName(const ColumnDescription &column) const { validateColumnForTagValue(column); }
...
void TimeSeriesColumnsValidator::validateColumnForTagsMap(const ColumnDescription &column) const {
  if (!isMap(column.type) || !isString(removeLowCardinality(typeid_cast<const DataTypeMap &>(*column.type).getKeyType())) ||
      !isString(removeLowCardinality(typeid_cast<const DataTypeMap &>(*column.type).getValueType()))) {
    throw Exception(ErrorCodes::INCOMPATIBLE_COLUMNS,
                    "Column {} has illegal data type {}, expected Map(String, String) or Map(LowCardinality(String), String)", column.name,
                    column.type->getName());
  }
}
...
void TimeSeriesColumnsValidator::validateColumnForType(const ColumnDescription &column) const { validateColumnForMetricFamilyName(column); }

void TimeSeriesColumnsValidator::validateColumnForUnit(const ColumnDescription &column) const { validateColumnForMetricFamilyName(column); }

void TimeSeriesColumnsValidator::validateColumnForHelp(const ColumnDescription &column) const { validateColumnForMetricFamilyName(column); }
