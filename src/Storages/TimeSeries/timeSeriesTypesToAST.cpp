...
boost::intrusive_ptr<ASTFunction> addTimeZone(boost::intrusive_ptr<ASTFunction>&& ast, String timezone) {
  if (!timezone.empty()) ast->arguments->children.push_back(make_intrusive<ASTLiteral>(std::move(timezone)));
  return ast;
}
}  // namespace
...
ASTPtr timeSeriesTimestampToAST(DateTime64 timestamp, const DataTypePtr& timestamp_data_type) {
  if (isDateTime64(timestamp_data_type)) {
    ...
    if (scale == 0) return timeSeriesTimestampASTCast(make_intrusive<ASTLiteral>(timestamp.value), timestamp_data_type);
...
ASTPtr timeSeriesTimestampASTCast(ASTPtr&& ast, const DataTypePtr& timestamp_data_type) {
  ...
  if (which_data_type.isDateTime64()) {
    auto scale = getDecimalScale(*timestamp_data_type);
    return addTimeZone(makeASTFunction("toDateTime64", std::move(ast), make_intrusive<ASTLiteral>(scale)),
    ...
    throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT, "Can't cast {} to the timestamp type {}", ast->formatForLogging(),
                    timestamp_data_type->getName());
  }
}
...
ASTPtr timeSeriesScalarASTCast(ASTPtr&& ast, const DataTypePtr& scalar_data_type) {
  ...
  if (which_data_type.isFloat64()) {
    ...
    throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT, "Can't cast {} to the scalar type {}", ast->formatForLogging(),
