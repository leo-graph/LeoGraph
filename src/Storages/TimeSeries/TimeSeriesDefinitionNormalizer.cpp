...
void TimeSeriesDefinitionNormalizer::normalize(ASTCreateQuery &create_query) const {
  ...
  if (as_create_query) addMissingInnerEnginesFromAsTable(create_query);

  addMissingInnerEngines(create_query);
}

void TimeSeriesDefinitionNormalizer::reorderColumns(ASTCreateQuery &create) const {
  if (!create.columns_list || !create.columns_list->columns) return;
  ...
  if (!columns_by_name.empty()) {
    throw Exception(ErrorCodes::INCOMPATIBLE_COLUMNS,
                    ...
                    "The TimeSeries table engine supports only a limited set of columns (id, timestamp, value, metric_name, tags, "
                    "metric_family_name, type, unit, help). "
...
void TimeSeriesDefinitionNormalizer::addMissingColumns(ASTCreateQuery &create) const {
  ...
  if (!create.columns_list->columns) create.columns_list->set(create.columns_list->columns, make_intrusive<ASTExpressionList>());
  auto &columns = create.columns_list->columns->children;
  ...
  auto make_nullable = [&](boost::intrusive_ptr<ASTDataType> type) {
    if (type->name == "Nullable") return type;
    return makeASTDataType("Nullable", type);
  };

  /// Add missing columns for the "data" table.
  if (!is_next_column_named(TimeSeriesColumnNames::ID)) make_new_column(TimeSeriesColumnNames::ID, get_uuid_type());

  if (!is_next_column_named(TimeSeriesColumnNames::Timestamp)) make_new_column(TimeSeriesColumnNames::Timestamp, get_datetime_type());
  ...
  if (!is_next_column_named(TimeSeriesColumnNames::Value)) make_new_column(TimeSeriesColumnNames::Value, get_float_type());
  ...
  for (const auto &tag_name_and_column_name : tags_to_columns) {
    const auto &tuple = tag_name_and_column_name.safeGet<Tuple>();
    const auto &column_name = tuple.at(1).safeGet<String>();
    if (!is_next_column_named(column_name)) make_new_column(column_name, get_string_type());
  }
  ...
  if (!is_next_column_named(TimeSeriesColumnNames::Type)) make_new_column(TimeSeriesColumnNames::Type, get_string_type());

  if (!is_next_column_named(TimeSeriesColumnNames::Unit)) make_new_column(TimeSeriesColumnNames::Unit, get_string_type());

  if (!is_next_column_named(TimeSeriesColumnNames::Help)) make_new_column(TimeSeriesColumnNames::Help, get_string_type());

  /// If the following fails that means the order in which columns are processed in this function doesn't match the order of columns in
  /// reorderColumns().
...
void TimeSeriesDefinitionNormalizer::addMissingInnerEnginesFromAsTable(ASTCreateQuery &create) const {
  ...
  for (auto target_kind : {ViewTarget::Data, ViewTarget::Tags, ViewTarget::Metrics}) {
    if (as_create_query->hasTargetTableID(target_kind)) {
      ...
      throw Exception(ErrorCodes::INCORRECT_QUERY, "Cannot CREATE a table AS {}.{} because it has external tables",
    ...
    if (!inner_table_engine) {
      ...
      if (inner_table_engine) create.setTargetInnerEngine(target_kind, boost::static_pointer_cast<ASTStorage>(inner_table_engine->clone()));
    }
  }
}
...
void TimeSeriesDefinitionNormalizer::addMissingInnerEngines(ASTCreateQuery &create) const {
  for (auto target_kind : {ViewTarget::Data, ViewTarget::Tags, ViewTarget::Metrics}) {
    if (create.hasTargetTableID(target_kind)) continue;  /// External target is set, inner engine is not needed.

    auto *inner_table_engine = create.getTargetInnerEngine(target_kind);
    if (inner_table_engine && inner_table_engine->engine) continue;  /// Engine is set already, skip it.
  ...
  switch (inner_table_kind) {
    case ViewTarget::Data: {
      ...
      if (!inner_storage_def.order_by && !inner_storage_def.primary_key && inner_storage_def.engine->name.ends_with("MergeTree")) {
        inner_storage_def.set(inner_storage_def.order_by, makeASTOperator("tuple", make_intrusive<ASTIdentifier>(TimeSeriesColumnNames::ID),
    ...
    case ViewTarget::Tags: {
      ...
      if (!inner_storage_def.order_by && !inner_storage_def.primary_key && inner_storage_def.engine->name.ends_with("MergeTree")) {
        inner_storage_def.set(inner_storage_def.primary_key, make_intrusive<ASTIdentifier>(TimeSeriesColumnNames::MetricName));
