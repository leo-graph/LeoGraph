...
class StorageFile final : public IStorage {
 public:
  ...
  struct FileSource {
    ...
    std::optional<String>
        format_from_filenames;                /// Set if we managed to figure out which file format is used from the names of the file(s).
  ...
  void truncate(const ASTPtr & /*query*/, const StorageMetadataPtr & /* metadata_snapshot */, ContextPtr /* context */,
  ...
  static ColumnsDescription getTableStructureFromFile(const String &format, const std::vector<String> &paths,
  ...
  static std::pair<ColumnsDescription, String> getTableStructureAndFormatFromFile(
      const std::vector<String> &paths, const String &compression_method, const std::optional<FormatSettings> &format_settings,
      const ContextPtr &context, const std::optional<ArchiveInfo> &archive_info = std::nullopt);
 ...
 private:
  std::pair<ColumnsDescription, String> getTableStructureAndFormatFromFileDescriptor(std::optional<String> format,
...
class StorageFileSource : public ISource, WithContext {
 public:
  class FilesIterator : WithContext {
   public:
    explicit FilesIterator(const Strings &files_, std::optional<StorageFile::ArchiveInfo> archive_info_, const ActionsDAG::Node *predicate,
                           const NamesAndTypesList &virtual_columns, const NamesAndTypesList &hive_columns, const ContextPtr &context_,
                           bool distributed_processing_ = false);

    String next();

    bool isReadFromArchive() const { return archive_info.has_value(); }

    bool validFileInArchive(const std::string &path) const { return archive_info->filter(path); }

    bool isSingleFileReadFromArchive() const { return archive_info->isSingleFileRead(); }
  ...
  StorageFileSource(const ReadFromFormatInfo &info, std::shared_ptr<StorageFile> storage_, const ContextPtr &context_,
                    UInt64 max_block_size_, FilesIteratorPtr files_iterator_, std::unique_ptr<ReadBuffer> read_buf_, bool need_only_count_,
