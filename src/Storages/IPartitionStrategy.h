 ...
 * Performs validation on required arguments by each strategy. Example: Partition strategy hive can not be used without a PARTITION BY
 * expression
 */
struct PartitionStrategyFactory {
  ...
  static std::shared_ptr<IPartitionStrategy> get(StrategyType strategy, ASTPtr partition_by, const NamesAndTypesList& partition_columns,
                                                 ContextPtr context, const std::string& file_format, bool globbed_path,
 ...
 * Path for writing appends partition key, snowflakeid as file name and file extension (e.g,
 * table_root/key1=value1/key2=value2/1933642830979268608.parquet).
 */
struct HiveStylePartitionStrategy : IPartitionStrategy {
  HiveStylePartitionStrategy(KeyDescription partition_key_description_, const Block& sample_block_, ContextPtr context_,
                             const std::string& file_format_, bool partition_columns_in_data_file_);
