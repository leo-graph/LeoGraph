...
class MemorySink : public SinkToStorage {
 public:
  MemorySink(StorageMemory &storage_, const StorageMetadataPtr &metadata_snapshot_, ContextPtr context)
  ...
  void consume(Chunk &chunk) override {
    ...
    if (storage.getMemorySettingsRef()[MemorySetting::compress]) {
      ...
      for (const auto &elem : block) compressed_block.insert({elem.column->compress(/*force_compression=*/true), elem.type, elem.name});
...
StorageMemory::StorageMemory(const StorageID &table_id_, ColumnsDescription columns_description_, ConstraintsDescription constraints_,
                             const String &comment, const MemorySettings &memory_settings_)
    : IStorage(table_id_), data(std::make_unique<const Blocks>()), memory_settings(std::make_unique<MemorySettings>(memory_settings_)) {
...
void StorageMemory::read(QueryPlan &query_plan, const Names &column_names, const StorageSnapshotPtr &storage_snapshot,
  ...
  query_plan.addStep(std::make_unique<ReadFromMemoryStorageStep>(column_names, query_info, storage_snapshot, context, shared_from_this(),
                                                                 num_streams, delay_read_for_global_subqueries));
}
...
void StorageMemory::mutate(const MutationCommands &commands, ContextPtr context) {
  ...
  while (executor.pull(block)) {
    if ((*memory_settings)[MemorySetting::compress])
      for (auto &elem : block) elem.column = elem.column->compress(/*force_compression=*/true);
...
void StorageMemory::truncate(const ASTPtr &, const StorageMetadataPtr &, ContextPtr, TableExclusiveLockHolder &) {
  data.set(std::make_unique<Blocks>());
  total_size_bytes.store(0, std::memory_order_relaxed);
  total_size_rows.store(0, std::memory_order_relaxed);
}
...
void StorageMemory::alter(const DB::AlterCommands &params, DB::ContextPtr context, DB::IStorage::AlterLockHolder & /*alter_lock_holder*/) {
  ...
  if (params.isSettingsAlter()) {
    ...
    if (!(*memory_settings)[MemorySetting::max_bytes_to_keep] ||
...
class MemoryBackup : public IBackupEntriesLazyBatch, boost::noncopyable {
 public:
  MemoryBackup(ContextPtr context_, const StorageMetadataPtr &metadata_snapshot_, const std::shared_ptr<const Blocks> blocks_,
               const String &data_path_in_backup, const TemporaryDataOnDiskScopePtr &tmp_data_, const ReadSettings &read_settings_)
      : context(context_), metadata_snapshot(metadata_snapshot_), blocks(blocks_), tmp_data(tmp_data_), read_settings(read_settings_) {
 ...
 private:
  size_t getSize() const override { return file_paths.size(); }

  const String &getName(size_t i) const override { return file_paths[i]; }
  ...
  BackupEntries generate() override {
    ...
    IndexForNativeFormat index;
    {
      ...
      NativeWriter block_out{data_out->getCompressedWriteBuffer(),
                             0,
                             std::make_shared<const Block>(metadata_snapshot->getSampleBlock()),
                             std::nullopt,
                             false,
                             &index};
      for (const auto &block : *blocks) block_out.write(block);
      data_out->finishWriting();
      backup_entries[data_bin_pos] = {file_paths[data_bin_pos], std::make_shared<BackupEntryFromAppendOnlyFile>(std::move(data_out))};
    }
    ...
    /// Writing index.mrk
    {
      ...
      backup_entries[index_mrk_pos] = {file_paths[index_mrk_pos],
                                       std::make_shared<BackupEntryFromAppendOnlyFile>(std::move(index_mrk_out))};
    }
...
void StorageMemory::backupData(BackupEntriesCollector &backup_entries_collector, const String &data_path_in_backup,
  ...
  backup_entries_collector.addBackupEntries(std::make_shared<MemoryBackup>(backup_entries_collector.getContext(), getInMemoryMetadataPtr(),
                                                                           data.get(), data_path_in_backup, tmp_data, read_settings)
                                                ->getBackupEntries());
}

void StorageMemory::restoreDataFromBackup(RestorerFromBackup &restorer, const String &data_path_in_backup,
                                          const std::optional<ASTs> & /* partitions */) {
  auto backup = restorer.getBackup();
  if (!backup->hasFiles(data_path_in_backup)) return;

  if (!restorer.isNonEmptyTableAllowed() && total_size_bytes) RestorerFromBackup::throwTableIsNotEmpty(getStorageID());

  restorer.addDataRestoreTask([storage = std::static_pointer_cast<StorageMemory>(shared_from_this()), backup, data_path_in_backup] {
...
void StorageMemory::restoreDataImpl(const BackupPtr &backup, const String &data_path_in_backup) {
  ...
  IndexForNativeFormat index;
  {
    ...
    try {
      ...
      if (e.code() == ErrorCodes::BACKUP_ENTRY_NOT_FOUND && file_size == 0) {
        LOG_ERROR(getLogger("StorageMemory"), "File '{}' with size 0 is listed in backup metadata but is not found in the backup, skipping",
                  index_file_path);
        return;
      } else
        throw;
  ...
  old_and_new_blocks.insert(old_and_new_blocks.end(), std::make_move_iterator(new_blocks.begin()),
...
std::optional<UInt64> StorageMemory::totalBytes(ContextPtr) const { return total_size_bytes.load(std::memory_order_relaxed); }

void registerStorageMemory(StorageFactory &factory) {
  factory.registerStorage("Memory",
                          [](const StorageFactory::Arguments &args) {
                            if (!args.engine_args.empty())
                              throw Exception(ErrorCodes::NUMBER_OF_ARGUMENTS_DOESNT_MATCH,
                                              "Engine {} doesn't support any arguments ({} given)", args.engine_name,
                                              args.engine_args.size());

                            bool has_settings = args.storage_def->settings;
                            MemorySettings settings;
                            if (has_settings) settings.loadFromQuery(*args.storage_def);
