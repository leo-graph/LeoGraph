...
MaterializedPostgreSQLConsumer::MaterializedPostgreSQLConsumer(ContextPtr context_, std::shared_ptr<postgres::Connection> connection_,
                                                               const std::string &replication_slot_name_,
                                                               const std::string &publication_name_, const std::string &start_lsn,
                                                               const size_t max_block_size_, bool schema_as_a_part_of_table_name_,
                                                               StorageInfos storages_info_, const String &name_for_logger)
  ...
  for (const auto &[table_name, storage_info] : storages_info_) storages.emplace(table_name, StorageData(storage_info, log));

  LOG_TRACE(log, "Starting replication. LSN: {} (last: {}), storages: {}", getLSNValue(current_lsn), getLSNValue(final_lsn),
            storages.size());
}
...
MaterializedPostgreSQLConsumer::StorageData::StorageData(const StorageInfo &storage_info, LoggerPtr log_)
  ...
  if (columns_attributes.size() + 2 != columns_num) {
    throw Exception(ErrorCodes::LOGICAL_ERROR, "Columns number mismatch. Attributes: {}, buffer: {}", columns_attributes.size(),
                    columns_num);
  }

  LOG_TRACE(log_, "Adding definition for table {}, structure: {}", storage_info.storage->getStorageID().getNameForLogs(),
            table_description.sample_block.dumpStructure());
}

MaterializedPostgreSQLConsumer::StorageData::Buffer::Buffer(ColumnsWithTypeAndName &&columns_,
                                                            const ExternalResultDescription &table_description_) {
  if (columns_.end() !=
      std::find_if(columns_.begin(), columns_.end(), [](const auto &col) { return col.name == "_sign" || col.name == "_version"; })) {
  ...
  for (const auto &col : columns_) {
    if (!table_description_.sample_block.has(col.name)) {
      throw Exception(ErrorCodes::LOGICAL_ERROR, "Having column {}, but no such column in table ({})", col.name,
                      table_description_.sample_block.dumpStructure());
    }

    const auto &actual_column = table_description_.sample_block.getByName(col.name);
    if (col.type != actual_column.type) {
      throw Exception(ErrorCodes::LOGICAL_ERROR, "Having column {} of type {}, but expected {}", col.name, col.type->getName(),
                      actual_column.type->getName());
    }
  }
  ...
  for (const auto &name : sample_block.getNames()) columns_ast.children.emplace_back(make_intrusive<ASTIdentifier>(name));
}

MaterializedPostgreSQLConsumer::StorageData::Buffer &MaterializedPostgreSQLConsumer::StorageData::getLastBuffer() {
  if (buffers.empty()) {
    throw Exception(ErrorCodes::LOGICAL_ERROR, "No data buffer for {}", storage->getStorageID().getNameForLogs());
  }
...
void MaterializedPostgreSQLConsumer::StorageData::addBuffer(BufferPtr buffer) { buffers.push_back(std::move(buffer)); }

void MaterializedPostgreSQLConsumer::StorageData::returnBuffer(BufferPtr buffer) { buffers.push_front(std::move(buffer)); }
...
void MaterializedPostgreSQLConsumer::insertValue(StorageData &storage_data, const std::string &value, size_t column_idx) {
  ...
  try {
    if (column_type_and_name.type->isNullable()) {
      ...
      insertPostgreSQLValue(column_nullable.getNestedColumn(), value, type_description.first, data_type.getNestedType(),
                            storage_data.array_info, column_idx_in_table);

      column_nullable.getNullMapData().emplace_back(false);
    } else {
      insertPostgreSQLValue(*column, value, type_description.first, column_type_and_name.type, storage_data.array_info,
                            column_idx_in_table);
    }
  } catch (const pqxx::conversion_error &e) {
    LOG_ERROR(log,
              "Conversion failed while inserting PostgreSQL value {}, "
              "will insert default value. Error: {}",
              value, e.what());
...
void MaterializedPostgreSQLConsumer::readTupleData(StorageData &storage_data, const char *message, size_t &pos, size_t size,
                                                   PostgreSQLQuery type, bool old_value) {
  ...
  auto process_column_value = [&](Int8 identifier, Int16 column_idx) {
    switch (identifier)  // NOLINT(bugprone-switch-missing-default-case)
    {
      ...
      case 't':  /// Text formatted value
      {
        ...
        String value;
        for (Int32 i = 0; i < col_len; ++i) value += static_cast<char>(readInt8(message, pos, size));
  ...
  for (Int16 column_idx = 0; column_idx < num_columns; ++column_idx) {
    try {
      ...
      LOG_ERROR(log, "Got error while receiving value for column {}, will insert default value. Error: {}", column_idx,
...
void MaterializedPostgreSQLConsumer::processReplicationMessage(const char *replication_message, size_t size) {
  ...
  switch (type) {
    ...
    case 'I':  // Insert
    {
      ...
      if (!isSyncAllowed(relation_id, table_name)) return;

      Int8 new_tuple = readInt8(replication_message, pos, size);
      auto &storage_data = storages.find(table_name)->second;

      if (new_tuple) readTupleData(storage_data, replication_message, pos, size, PostgreSQLQuery::INSERT);
    ...
    case 'U':  // Update
    {
      ...
      if (read_next) process_identifier(readInt8(replication_message, pos, size));
    ...
    case 'R':  // Relation
    {
      ...
      if (!relation_id_to_name.contains(relation_id)) relation_id_to_name[relation_id] = table_name;

      if (!isSyncAllowed(relation_id, relation_name)) return;
      ...
      if (replica_identity != 'd' && replica_identity != 'i') {
        LOG_WARNING(log, "Table has replica identity {} - not supported. A table must have a primary key or a replica identity index",
                    replica_identity);
        markTableAsSkipped(relation_id, table_name);
        return;
      }
      ...
      auto log_table_structure_changed = [&](const std::string &reason) {
        LOG_INFO(log,
                 "Table structure of the table {} changed ({}), "
      ...
      if (size_t(num_columns) < actual_columns_num) {
        ...
        std::set_difference(all_columns.begin(), all_columns.end(), received_columns.begin(), received_columns.end(),
                            std::back_inserter(absent_columns));

        for (const auto &name : absent_columns) {
          if (name == "_sign" || name == "_version") continue;
          ...
          if (!attributes_it->second.attr_def.empty()) continue;
    ...
    case 'T':  // Truncate
      break;
    default:
      throw Exception(ErrorCodes::LOGICAL_ERROR, "Unexpected byte1 value {} while parsing replication message", type);
  }
}
...
void MaterializedPostgreSQLConsumer::syncTables() {
    ...
    while (auto buffer = storage_data.popBuffer()) {
      ...
      try {
        if (result_rows.rows()) {
          ...
          auto input = std::make_shared<SourceFromSingleChunk>(std::make_shared<const Block>(result_rows.cloneEmpty()),
                                                               Chunk(result_rows.getColumns(), result_rows.rows()));
  ...
  LOG_DEBUG(log, "Table sync end for {} tables, last lsn: {} = {}, (attempted lsn {})", synced_tables, current_lsn,
            getLSNValue(current_lsn), getLSNValue(final_lsn));
...
bool MaterializedPostgreSQLConsumer::isSyncAllowed(Int32 relation_id, const String &relation_name) {
  if (deleted_tables.contains(relation_name)) return false;
  ...
  if (table_start_lsn.empty()) return false;
  ...
  /// table was reloaded.
  if (getLSNValue(current_lsn) >= getLSNValue(table_start_lsn)) {
    LOG_TRACE(log, "Synchronization is resumed for table: {} (start_lsn: {})", relation_id_to_name[relation_id], table_start_lsn);
...
void MaterializedPostgreSQLConsumer::addNested(const String &postgres_table_name, StorageInfo nested_storage_info,
                                               const String &table_start_lsn) {
  ...
  if (it != deleted_tables.end()) deleted_tables.erase(it);
...
void MaterializedPostgreSQLConsumer::updateNested(const String &table_name, StorageInfo nested_storage_info, Int32 table_id,
                                                  const String &table_start_lsn) {
...
void MaterializedPostgreSQLConsumer::removeNested(const String &postgres_table_name) {
  ...
  if (it != storages.end()) storages.erase(it);
  ...
  try {
    ...
    while (true) {
      ...
      try {
        ...
        if (e.code() == ErrorCodes::POSTGRESQL_REPLICATION_INTERNAL_ERROR) continue;
    ...
    if (!error_message.contains("out of relcache_callback_list slots")) tryLogCurrentException(__PRETTY_FUNCTION__);
