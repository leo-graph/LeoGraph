...
#  define LIST_OF_MATERIALIZED_POSTGRESQL_SETTINGS(DECLARE, ALIAS)                                                                        \
    ...
    DECLARE(Bool, materialized_postgresql_tables_list_with_schema, false,                                                                 \
            ...
            "then the first name is postgres schema and second is postgres table. This setting is needed to allow table names with dots", \
            0)                                                                                                                            \
    ...
    DECLARE(Bool, materialized_postgresql_use_unique_replication_consumer_identifier, false,                                              \
            "Should a unique consumer be registered for table replication", 0)
...
#  define INITIALIZE_SETTING_EXTERN(TYPE, NAME, DEFAULT, DESCRIPTION, FLAGS, ...) \
    MaterializedPostgreSQLSettings##TYPE NAME = &MaterializedPostgreSQLSettingsImpl ::NAME;
...
void MaterializedPostgreSQLSettings::applyChange(const SettingChange& change) { impl->applyChange(change); }

bool MaterializedPostgreSQLSettings::has(std::string_view name) const { return impl->has(name); }
...
bool MaterializedPostgreSQLSettings::hasBuiltin(std::string_view name) { return MaterializedPostgreSQLSettingsImpl::hasBuiltin(name); }
}  // namespace DB
