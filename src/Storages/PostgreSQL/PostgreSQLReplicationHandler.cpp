...
class TemporaryReplicationSlot {
 public:
  TemporaryReplicationSlot(PostgreSQLReplicationHandler *handler_, std::shared_ptr<pqxx::nontransaction> tx_, String &start_lsn,
...
String getPublicationName(const String &postgres_database, const String &postgres_table) {
  return fmt::format("{}_ch_publication",
...
void checkReplicationSlot(String name) {
  ...
  if (name.size() > replication_slot_name_max_size) throw Exception(ErrorCodes::LOGICAL_ERROR, "Too big replication slot size: {}", name);
}
...
String getReplicationSlotName(const String &postgres_database, const String &postgres_table, const String &clickhouse_uuid,
...
PostgreSQLReplicationHandler::PostgreSQLReplicationHandler(const String &postgres_database_, const String &postgres_table_,
                                                           const String &clickhouse_database_, const String &clickhouse_uuid_,
                                                           const postgres::ConnectionInfo &connection_info_, ContextPtr context_,
                                                           bool is_attach_, const MaterializedPostgreSQLSettings &replication_settings,
    ...
    : WithContext(context_->getGlobalContext()),
      ...
      schema_as_a_part_of_table_name(!schema_list.empty() ||
                                     replication_settings[MaterializedPostgreSQLSetting::materialized_postgresql_tables_list_with_schema]),
  ...
  startup_task = getContext()->getSchedulePool().createTask(StorageID::createEmpty(), "PostgreSQLReplicaStartup",
                                                            [this] { checkConnectionAndStart(); });
  consumer_task =
      getContext()->getSchedulePool().createTask(StorageID::createEmpty(), "PostgreSQLReplicaConsume", [this] { consumerFunc(); });
  cleanup_task =
      getContext()->getSchedulePool().createTask(StorageID::createEmpty(), "PostgreSQLReplicaCleanup", [this] { cleanupFunc(); });
}
...
std::pair<String, String> PostgreSQLReplicationHandler::getSchemaAndTableName(const String &table_name) const {
  ...
  if (!postgres_schema.empty()) return std::make_pair(postgres_schema, table_name);
...
String PostgreSQLReplicationHandler::doubleQuoteWithSchema(const String &table_name) const {
  ...
  if (schema.empty()) return doubleQuoteString(table);
...
void PostgreSQLReplicationHandler::startSynchronization(bool throw_on_error) {
  ...
  auto initial_sync = [&]() {
    ...
    for (const auto &[table_name, storage] : materialized_storages) {
      try {
        nested_storages.emplace(table_name,
                                loadFromSnapshot(*tmp_connection, snapshot_name, table_name, storage->as<StorageMaterializedPostgreSQL>()));
      } catch (Exception &e) {
        ...
        if (throw_on_error && !is_materialized_postgresql_database) throw;
      }
    }
  };
  ...
  /// Always drop replication slot if it is CREATE query and not ATTACH.
    ...
    if (!user_managed_slot) dropReplicationSlot(tx);
  ...
  /// and pass them to replication consumer.
    ...
    for (const auto &[table_name, storage] : materialized_storages) {
      ...
      try {
        ...
        if (!table_structure->physical_columns) throw Exception(ErrorCodes::LOGICAL_ERROR, "No columns");
  ...
  consumer = std::make_shared<MaterializedPostgreSQLConsumer>(
      getContext(), std::move(tmp_connection), replication_slot, publication_name, start_lsn, max_block_size,
      schema_as_a_part_of_table_name, nested_storages,
...
StorageInfo PostgreSQLReplicationHandler::loadFromSnapshot(postgres::Connection &connection, String &snapshot_name,
                                                           const String &table_name, StorageMaterializedPostgreSQL *materialized_storage) {
  ...
  if (!table_structure->physical_columns) throw Exception(ErrorCodes::LOGICAL_ERROR, "No table attributes");
  ...
  if (columns.empty())
    ...
    for (const auto &column : table_structure->physical_columns->columns) allowed_columns.push_back(doubleQuoteString(column.name));
  ...
  LOG_DEBUG(log, "Loaded table {}.{} (uuid: {})", nested_table_id.database_name, nested_table_id.table_name,
            toString(nested_table_id.uuid));
...
void PostgreSQLReplicationHandler::cleanupFunc() {
  try {
    ...
    execWithRetryAndFaultInjection(connection, [&](pqxx::nontransaction &tx) {
      if (isReplicationSlotExist(tx, last_committed_lsn, /* temporary */ true)) dropReplicationSlot(tx, /* temporary */ true);
    });
  } catch (...) {
    tryLogCurrentException(log);
  }

  if (!stop_synchronization) cleanup_task->scheduleAfter(CLEANUP_RESCHEDULE_MS);
}

PostgreSQLReplicationHandler::ConsumerPtr PostgreSQLReplicationHandler::getConsumer() {
  if (!consumer) throw Exception(ErrorCodes::LOGICAL_ERROR, "Consumer not initialized");
  return consumer;
}
...
void PostgreSQLReplicationHandler::createPublicationIfNeeded(pqxx::nontransaction &tx) {
  ...
  if (!is_attach && publication_exists) {
    ...
    LOG_WARNING(log, "Publication {} already exists, but it is a CREATE query, not ATTACH. Publication will be dropped",
  ...
  if (!is_attach || !publication_exists) {
    if (tables_list.empty()) {
      if (materialized_storages.empty()) throw Exception(ErrorCodes::BAD_ARGUMENTS, "No tables to replicate");
    ...
    if (tables_list.empty()) throw Exception(ErrorCodes::LOGICAL_ERROR, "No table found to be replicated");
...
bool PostgreSQLReplicationHandler::isReplicationSlotExist(pqxx::nontransaction &tx, String &start_lsn, bool temporary) {
  ...
  String query_str =
      fmt::format("SELECT active, restart_lsn, confirmed_flush_lsn FROM pg_replication_slots WHERE slot_name = '{}'", slot_name);
  pqxx::result result{tx.exec(query_str)};
  ...
  LOG_DEBUG(log, "Replication slot {} already exists (active: {}). Restart lsn position: {}, confirmed flush lsn: {}", slot_name,
            result[0][0].as<bool>(), result[0][1].as<std::string>(), start_lsn);

  return true;
}

void PostgreSQLReplicationHandler::createReplicationSlot(pqxx::nontransaction &tx, String &start_lsn, String &snapshot_name,
                                                         bool temporary) {
...
void PostgreSQLReplicationHandler::addTableToPublication(pqxx::nontransaction &ntx, const String &table_name) {
  std::string query_str =
      fmt::format("ALTER PUBLICATION {} ADD TABLE ONLY {}", doubleQuoteString(publication_name), doubleQuoteWithSchema(table_name));
  ntx.exec(query_str);
...
void PostgreSQLReplicationHandler::removeTableFromPublication(pqxx::nontransaction &ntx, const String &table_name) {
  try {
    std::string query_str =
        fmt::format("ALTER PUBLICATION {} DROP TABLE ONLY {}", doubleQuoteString(publication_name), doubleQuoteWithSchema(table_name));
    ntx.exec(query_str);
...
Strings PostgreSQLReplicationHandler::getTableAllowedColumns(const std::string &table_name) const {
  Strings result;
  if (tables_list.empty()) return result;

  size_t table_pos = 0;
  while (true) {
    table_pos = tables_list.find(table_name, table_pos + 1);
    if (table_pos == std::string::npos) return result;
    if (table_pos + table_name.length() + 1 > tables_list.length()) return result;
    if (tables_list[table_pos + table_name.length() + 1] == '(' || tables_list[table_pos + table_name.length() + 1] == ',' ||
  ...
  if (column_list.empty() || column_list[0] != '(') return result;
...
void PostgreSQLReplicationHandler::shutdownFinal() {
  try {
    ...
    if (user_managed_slot) return;

    connection.execWithRetry([&](pqxx::nontransaction &tx) {
      if (isReplicationSlotExist(tx, last_committed_lsn, /* temporary */ false)) dropReplicationSlot(tx, /* temporary */ false);
    });
  } catch (...) {
    LOG_ERROR(log, "Failed to drop replication slot: {}. It must be dropped manually. Error: {}", replication_slot,
...
std::set<String> PostgreSQLReplicationHandler::fetchRequiredTables() {
  ...
  if (!tables_list.empty()) {
    ...
    if (expected_tables.empty()) throw Exception(ErrorCodes::BAD_ARGUMENTS, "Cannot parse tables list: {}", tables_list);
  ...
  if (publication_exists_before_startup) {
    if (!is_attach) {
      ...
      if (tables_list.empty()) {
        LOG_WARNING(log, "Publication {} already exists and tables list is empty. Assuming publication is correct.",
                    doubleQuoteString(publication_name));
    ...
    if (!tables_list.empty()) {
      ...
      /// startSynchronization method.
      {
        ...
        for (const auto &table : result_tables) {
          if (!tables_string.empty()) tables_string += ", ";
          tables_string += table;
        }
  ...
  if (!tables_list.empty()) {
    ...
    if (parts.empty()) throw Exception(ErrorCodes::BAD_ARGUMENTS, "Empty list of tables");
...
template <typename T>
PostgreSQLTableStructurePtr PostgreSQLReplicationHandler::fetchTableStructure(T &tx, const std::string &table_name) const {
...
template PostgreSQLTableStructurePtr PostgreSQLReplicationHandler::fetchTableStructure(pqxx::ReadTransaction &tx,
                                                                                       const std::string &table_name) const;

template PostgreSQLTableStructurePtr PostgreSQLReplicationHandler::fetchTableStructure(pqxx::ReplicationTransaction &tx,
                                                                                       const std::string &table_name) const;

template PostgreSQLTableStructurePtr PostgreSQLReplicationHandler::fetchTableStructure(pqxx::nontransaction &tx,
                                                                                       const std::string &table_name) const;

void PostgreSQLReplicationHandler::addTableToReplication(StorageMaterializedPostgreSQL *materialized_storage,
  ...
  try {
...

    {
      ...
      if (isReplicationSlotExist(*tx, start_lsn, /* temporary */ true)) dropReplicationSlot(*tx, /* temporary */ true);

      TemporaryReplicationSlot temporary_slot(this, tx, start_lsn, snapshot_name);

      /// Protect against deadlock.
      auto nested = DatabaseCatalog::instance().tryGetTable(materialized_storage->getNestedStorageID(),
                                                            materialized_storage->getNestedTableContext());
      if (!nested) throw Exception(ErrorCodes::LOGICAL_ERROR, "Internal table was not created");
    ...
    throw Exception(ErrorCodes::POSTGRESQL_REPLICATION_INTERNAL_ERROR, "Failed to add table `{}` to replication. Info: {}",
                    postgres_table_name, error_message);
  }
  consumer_task->activateAndSchedule();
}
...
void PostgreSQLReplicationHandler::removeTableFromReplication(const String &postgres_table_name) {
  ...
  try {
    ...
    auto error_message = getCurrentExceptionMessage(false);
    throw Exception(ErrorCodes::POSTGRESQL_REPLICATION_INTERNAL_ERROR, "Failed to remove table `{}` from replication. Info: {}",
                    postgres_table_name, error_message);
  }
  consumer_task->activateAndSchedule();
}

void PostgreSQLReplicationHandler::execWithRetryAndFaultInjection(postgres::Connection &connection,
                                                                  const std::function<void(pqxx::nontransaction &)> &exec) const {
  if (fault_injection_probability > 0.) {
    std::bernoulli_distribution fault(fault_injection_probability);
    if (fault(thread_local_rng)) throw pqxx::broken_connection("Fault injected");
  }
