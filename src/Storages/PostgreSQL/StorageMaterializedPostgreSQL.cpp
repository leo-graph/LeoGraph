...
StorageMaterializedPostgreSQL::StorageMaterializedPostgreSQL(const StorageID &table_id_, LoadingStrictnessLevel mode,
                                                             const String &remote_database_name, const String &remote_table_name_,
  ...
  replication_handler = std::make_unique<PostgreSQLReplicationHandler>(remote_database_name, remote_table_name_, table_id_.database_name,
                                                                       toString(table_id_.uuid), connection_info, getContext(), is_attach,
...
String StorageMaterializedPostgreSQL::getNestedTableName() const {
  ...
  if (is_materialized_postgresql_database) return table_id.table_name;

  return toString(table_id.uuid) + NESTED_TABLE_SUFFIX;
}

StorageID StorageMaterializedPostgreSQL::getNestedStorageID() const {
  if (nested_table_id.has_value()) return nested_table_id.value();

  auto table_id = getStorageID();
  throw Exception(ErrorCodes::LOGICAL_ERROR, "No storageID found for inner table. ({})", table_id.getNameForLogs());
}
...
void StorageMaterializedPostgreSQL::createNestedIfNeeded(PostgreSQLTableStructurePtr table_structure,
  ...
  try {
    ...
    LOG_DEBUG(log, "Creating clickhouse table for postgresql table {} (ast: {})", table_id.getNameForLogs(),
              ast_create->formatForLogging());
...
void StorageMaterializedPostgreSQL::dropInnerTableIfAny(bool sync, ContextPtr local_context) {
  ...
  if (is_materialized_postgresql_database) return;
...
void StorageMaterializedPostgreSQL::read(QueryPlan &query_plan, const Names &column_names, const StorageSnapshotPtr & /*storage_snapshot*/,
  ...
  readFinalFromNestedStorage(query_plan, nested_table, column_names, query_info, context_, processed_stage, max_block_size, num_streams);
...
boost::intrusive_ptr<ASTColumnDeclaration> StorageMaterializedPostgreSQL::getMaterializedColumnsDeclaration(String name, String type,
...
boost::intrusive_ptr<ASTExpressionList> StorageMaterializedPostgreSQL::getColumnsExpressionList(
...
ASTPtr StorageMaterializedPostgreSQL::getCreateNestedTableQuery(PostgreSQLTableStructurePtr table_structure,
  ...
  if (is_materialized_postgresql_database) create_table_query->uuid = table_id.uuid;
  ...
  if (is_materialized_postgresql_database) {
    if (!table_structure && !table_override) {
      throw Exception(ErrorCodes::LOGICAL_ERROR, "No table structure returned for table {}.{}", table_id.database_name,
                      table_id.table_name);
    }

    if (!table_structure->physical_columns && (!table_override || !table_override->columns)) {
      throw Exception(ErrorCodes::LOGICAL_ERROR, "No columns returned for table {}.{}", table_id.database_name, table_id.table_name);
    }
    ...
    if (!table_structure->primary_key_columns && !table_structure->replica_identity_columns) {
      throw Exception(ErrorCodes::BAD_ARGUMENTS, "Table {}.{} has no primary key and no replica identity index", table_id.database_name,
                      table_id.table_name);
    }
    ...
    if (table_override && table_override->columns) {
      ...
      if (columns && columns->constraints) constraints = ConstraintsDescription(columns->constraints->children);
    } else {
    ...
    if (ordinary_columns_and_types.empty())
      throw Exception(ErrorCodes::LOGICAL_ERROR, "Table {}.{} has no columns", table_id.database_name, table_id.table_name);
    ...
    if (!primary_key_ast) throw Exception(ErrorCodes::BAD_ARGUMENTS, "Storage MaterializedPostgreSQL must have primary key");
...
void registerStorageMaterializedPostgreSQL(StorageFactory &factory) {
  auto creator_fn = [](const StorageFactory::Arguments &args) {
    ...
    if (args.mode <= LoadingStrictnessLevel::CREATE &&
      ...
      throw Exception(ErrorCodes::BAD_ARGUMENTS,
                      "MaterializedPostgreSQL is an experimental table engine."
    ...
    if (has_settings) postgresql_replication_settings->loadFromQuery(*args.storage_def);

    return std::make_shared<StorageMaterializedPostgreSQL>(args.table_id, args.mode, configuration.database, configuration.table,
                                                           connection_info, metadata, args.getContext(),
                                                           std::move(postgresql_replication_settings));
  };

  factory.registerStorage("MaterializedPostgreSQL", creator_fn,
