...
struct StorageInfo {
  ...
  StorageInfo(StoragePtr storage_, const PostgreSQLTableStructure::Attributes& attributes_) : storage(storage_), attributes(attributes_) {}
...
class MaterializedPostgreSQLConsumer {
 ...
 public:
  MaterializedPostgreSQLConsumer(ContextPtr context_, std::shared_ptr<postgres::Connection> connection_,
                                 const String& replication_slot_name_, const String& publication_name_, const String& start_lsn,
                                 size_t max_block_size_, bool schema_as_a_part_of_table_name_, StorageInfos storages_,
 ...
 private:
  ...
  enum class PostgreSQLQuery : uint8_t { INSERT, UPDATE, DELETE };

  void readTupleData(StorageData& storage_data, const char* message, size_t& pos, size_t size, PostgreSQLQuery type,
