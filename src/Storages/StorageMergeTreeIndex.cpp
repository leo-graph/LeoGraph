...
class MergeTreeIndexSource : public ISource, WithContext {
 public:
  MergeTreeIndexSource(SharedHeader header_, SharedHeader index_header_, SharedHeader minmax_header_,
                       MergeTreeData::DataPartsVector data_parts_, ContextPtr context_, bool with_marks_)
      : ISource(header_),
 ...
 protected:
  Chunk generate() override {
    if (part_index >= data_parts.size()) return {};
    ...
    if (with_marks && isCompactPart(part)) {
      marks_loader =
          createMarksLoader(part, MergeTreeDataPartCompact::DATA_FILE_NAME,
    ...
    for (size_t pos = 0; pos < num_columns; ++pos) {
      ...
      if (index_header->has(column_name)) {
        if (!index_ptr) index_ptr = part->getIndex();
        ...
        if (minmax_pos >= part->minmax_idx->hyperrectangle.size())
          throw Exception(ErrorCodes::CORRUPTED_DATA, "Part {} has broken minmax_idx: size = {} but {} has pos = {}", part->name,
                          part->minmax_idx->hyperrectangle.size(), column_name, minmax_pos);
        ...
        data.resize(num_rows);
        for (size_t i = 0; i < num_rows; ++i) data[i] = index_granularity->getMarkRows(i);
 ...
 private:
  std::shared_ptr<MergeTreeMarksLoader> createMarksLoader(const MergeTreeDataPartPtr &part, const String &prefix_name, size_t num_columns) {
    ...
    return std::make_shared<MergeTreeMarksLoader>(info_for_read, local_context->getMarkCache().get(),
                                                  info_for_read->getIndexGranularityInfo().getMarksFilePath(prefix_name),
                                                  info_for_read->getMarksCount(), info_for_read->getIndexGranularityInfo(),
                                                  /*save_marks_in_cache=*/false, local_context->getReadSettings(),
                                                  /*load_marks_threadpool=*/nullptr, num_columns);
  }

  ColumnPtr fillMarks(MergeTreeDataPartPtr part, std::shared_ptr<MergeTreeMarksLoader> marks_loader, const IDataType &data_type,
...
StorageMergeTreeIndex::StorageMergeTreeIndex(const StorageID &table_id_, const StoragePtr &source_table_, const ColumnsDescription &columns,
                                             bool with_marks_, bool with_minmax_)
    : IStorage(table_id_), source_table(source_table_), with_marks(with_marks_), with_minmax(with_minmax_) {
...
class ReadFromMergeTreeIndex : public SourceStepWithFilter {
 public:
  ...
  ReadFromMergeTreeIndex(const Names &column_names_, const SelectQueryInfo &query_info_, const StorageSnapshotPtr &storage_snapshot_,
                         const ContextPtr &context_, SharedHeader sample_block, std::shared_ptr<StorageMergeTreeIndex> storage_)
...
void StorageMergeTreeIndex::read(QueryPlan &query_plan, const Names &column_names, const StorageSnapshotPtr &storage_snapshot,
  ...
  auto reading = std::make_unique<ReadFromMergeTreeIndex>(column_names, query_info, storage_snapshot, std::move(context),
                                                          std::move(sample_block), std::move(this_ptr));
...
void ReadFromMergeTreeIndex::initializePipeline(QueryPipelineBuilder &pipeline, const BuildQueryPipelineSettings &) {
  ...
  LOG_DEBUG(log, "Reading index{}{} from {} parts of table {}", storage->with_marks ? " with marks" : "",
            storage->with_minmax ? " with minmax_idx" : "", filtered_parts.size(), storage->source_table->getStorageID().getNameForLogs());

  pipeline.init(Pipe(std::make_shared<MergeTreeIndexSource>(getOutputHeader(), storage->key_sample_block, storage->minmax_sample_block,
                                                            std::move(filtered_parts), context, storage->with_marks)));
}
