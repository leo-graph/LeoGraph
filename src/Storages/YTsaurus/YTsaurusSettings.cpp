...
#define LIST_OF_YTSAURUS_SETTINGS(DECLARE, ALIAS)                                                                                          \
  ...
  DECLARE(                                                                                                                                 \
      ...
      "Enable redirection to heavy proxies for heavy queries. See: https://ytsaurus.tech/docs/en/user-guide/proxy/http-reference#hosts",   \
      0)                                                                                                                                   \
  DECLARE(                                                                                                                                 \
      Bool, use_lock, true,                                                                                                                \
      "Lock table to do request, to get consistent snapshot. See: https://ytsaurus.tech/docs/en/user-guide/storage/transactions#locks", 0) \
  DECLARE(Milliseconds, transaction_timeout_ms, 150000, "Timeout for YTSaurus transaction.", 0)                                            \
  DECLARE(                                                                                                                                 \
      UInt64, min_rows_for_spawn_stream, 1000,                                                                                             \
      "Min number of rows to spawn the new stream. To use 8 streams the table must hold at least 8 * `min_rows_for_spawn_stream` rows.",   \
      0)                                                                                                                                   \
...
YTsaurusSettings::YTsaurusSettings(YTsaurusSettings&& settings) noexcept
    : impl(std::make_unique<YTsaurusSettingsImpl>(std::move(*settings.impl))) {}
...
void YTsaurusSettings::loadFromQuery(const ASTSetQuery& settings_def) { impl->applyChanges(settings_def.changes); }
...
void YTsaurusSettings::set(const std::string& name, const std::string& value) { impl->set(name, value); }

bool YTsaurusSettings::hasBuiltin(std::string_view name) { return YTsaurusSettingsImpl::hasBuiltin(name); }
