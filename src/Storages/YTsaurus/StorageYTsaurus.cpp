...
StorageYTsaurus::StorageYTsaurus(const StorageID& table_id_, YTsaurusStorageConfiguration configuration_,
...
Pipe StorageYTsaurus::read(const Names& column_names, const StorageSnapshotPtr& storage_snapshot, SelectQueryInfo& /*query_info*/,
...
YTsaurusStorageConfiguration StorageYTsaurus::processNamedCollectionResult(const NamedCollection& named_collection,
                                                                           const YTsaurusSettings& settings,
                                                                           bool is_for_dictionary = false) {
  ...
  if (is_for_dictionary) {
    ...
    if (!column_description.empty()) configuration.ytsaurus_columns_description = std::move(column_description);
  }
...
YTsaurusStorageConfiguration StorageYTsaurus::getConfiguration(ASTs engine_args, const YTsaurusSettings& settings, ContextPtr context,
  ...
  if (engine_args.size() == 3) {
    ...
    boost::split(configuration.http_proxy_urls, checkAndGetLiteralArgument<String>(engine_args[0], "http_proxy_urls"),
                 [](char c) { return c == '|'; });
...
void registerStorageYTsaurus(StorageFactory& factory) {
  factory.registerStorage("YTsaurus",
                          [](const StorageFactory::Arguments& args) {
                            if (args.mode <= LoadingStrictnessLevel::CREATE &&
                                !args.getLocalContext()->getSettingsRef()[Setting::allow_experimental_ytsaurus_table_engine])
                              throw Exception(ErrorCodes::UNKNOWN_STORAGE,
                                              "Table engine YTsaurus is experimental. "
                                              "Set `allow_experimental_ytsaurus_table_engine` setting to enable it");
                            return std::make_shared<StorageYTsaurus>(
                                args.table_id,
                                StorageYTsaurus::getConfiguration(args.engine_args, YTsaurusSettings::createFromQuery(*args.storage_def),
                                                                  args.getLocalContext(), &args.table_id),
