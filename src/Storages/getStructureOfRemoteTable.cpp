...
ColumnsDescription getStructureOfRemoteTableInShard(const Cluster &cluster, const Cluster::ShardInfo &shard_info, const StorageID &table_id,
                                                    ContextPtr context, const ASTPtr &table_func_ptr) {
  String query;
  ...
  if (!table_func_ptr) executor.setMainTable(table_id);
  ...
  for (Block current = executor.readBlock(); !current.empty(); current = executor.readBlock()) {
    ...
    for (size_t i = 0; i < size; ++i) {
      ...
      if (!kind_name.empty()) {
        ...
        column.default_desc.expression = parseQuery(expr_parser, expr_str.data(), expr_str.data() + expr_str.size(), "default expression",
...
ColumnsDescription getStructureOfRemoteTable(const Cluster &cluster, const StorageID &table_id, ContextPtr context,
  ...
  throw NetException(ErrorCodes::NO_REMOTE_SHARD_AVAILABLE, "All attempts to get table structure failed. Log: \n\n{}\n", fail_messages);
}
