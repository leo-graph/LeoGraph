...
void verifyTableId(const StorageID &table_id) {
  if (!table_id.hasUUID()) {
    ...
    throw Exception(ErrorCodes::BAD_ARGUMENTS,
                    ...
                    table_id.getDatabaseName(), database->getEngineName());
  }
}
 ...
 public:
  ...
  void consume(Chunk &chunk) override {
    ...
    for (size_t i = 0; i < rows; ++i) {
      ...
      if (version != -1) versions[key] = version;
  ...
  void finalize(bool strict) {
    ...
    ZooKeeperRetriesControl zk_retry{
        getName(), getLogger(getName()),
        ZooKeeperRetriesInfo{settings[Setting::insert_keeper_max_retries], settings[Setting::insert_keeper_retry_initial_backoff_ms],
                             settings[Setting::insert_keeper_retry_max_backoff_ms], context->getProcessListElement()}};
    ...
    zk_retry.retryLoop([&]() {
      ...
      for (const auto &[key, _] : new_values) key_paths.push_back(storage.fullPathForKey(key));

      zkutil::ZooKeeper::MultiExistsResponse results;

      if constexpr (!for_update) {
        if (!strict) results = zookeeper->exists(key_paths);
      }
      ...
      for (size_t i = 0; i < key_paths.size(); ++i) {
        ...
        if constexpr (for_update) {
          int32_t version = -1;
          if (strict) version = versions.at(key);
      ...
      if (new_keys_num != 0) {
        ...
        if (keys_limit != 0 && will_be > keys_limit)
          throw Exception(ErrorCodes::LIMIT_EXCEEDED,
                          ...
                          new_keys_num, keys_limit, will_be);
      }
...
class StorageKeeperMapSource : public ISource, WithContext {
 ...
 public:
  StorageKeeperMapSource(const StorageKeeperMap &storage_, SharedHeader header, size_t max_block_size_, KeyContainerPtr container_,
                         KeyContainerIter begin_, KeyContainerIter end_, bool with_version_column_, ContextPtr context_)
      : ISource(header),
  ...
  Chunk generate() override {
    ...
    if constexpr (std::same_as<KeyType, Field>) {
      ...
      for (auto &raw_key : raw_keys) raw_key = base64Encode(raw_key, /* url_encoding */ true);
...
StorageKeeperMap::StorageKeeperMap(ContextPtr context_, const StorageID &table_id, const StorageInMemoryMetadata &metadata, bool attach,
                                   std::string_view primary_key_, const std::string &zk_root_path_, UInt64 keys_limit_,
  ...
  out << "KeeperMap metadata format version: 1\n"
      << "columns: " << metadata.columns.toString(true) << "primary key: " << formattedAST(metadata.getPrimaryKey().expression_list_ast)
      << "\n";
  metadata_string = out.str();

  if (zk_root_path.empty()) throw Exception(ErrorCodes::BAD_ARGUMENTS, "zk_root_path should not be empty");
  if (!zk_root_path.starts_with('/')) throw Exception(ErrorCodes::BAD_ARGUMENTS, "zk_root_path should start with '/'");
  ...
  ZooKeeperRetriesControl zk_retry{
      ...
      ZooKeeperRetriesInfo{settings[Setting::keeper_max_retries], settings[Setting::keeper_retry_initial_backoff_ms],
                           settings[Setting::keeper_retry_max_backoff_ms], context_->getProcessListElement()}};
    ...
    zk_retry.retryLoop([&] {
      ...
      if (exists) {
        ...
        if (code != Coordination::Error::ZNONODE)
          throw zkutil::KeeperException(code, "Failed to create table on path {} because a table with same UUID already exists",
                                        zk_root_path);
      ...
      if (client->exists(zk_dropped_path)) {
        ...
        if (zk_retry.isRetry() && drop_lock_version != -1) {
          ...
          if (drop_lock_responses[0]->error == Coordination::Error::ZNONODE) {
            ...
            if (!dropTableData(client, metadata_drop_lock)) return;
            drop_finished = true;
          }
        }
      ...
      if (code != Coordination::Error::ZOK) zkutil::KeeperMultiException::check(code, create_requests, create_responses);
...
class ReadFromKeeperMap : public SourceStepWithFilter {
 public:
  ...
  ReadFromKeeperMap(const Names &column_names_, const SelectQueryInfo &query_info_, const StorageSnapshotPtr &storage_snapshot_,
                    const ContextPtr &context_, SharedHeader sample_block, const StorageKeeperMap &storage_, size_t max_block_size_,
                    size_t num_streams_, bool with_version_column_)
...
bool StorageKeeperMap::isMetadataStringEqual(const std::string &zk_metadata_string, const std::string &local_metadata_string,
                                             bool throw_on_error) const {
  if (zk_metadata_string == local_metadata_string) return true;
  ...
  if (!local_metadata_string.starts_with(metadata_format_version_prefix))
    throw Exception(ErrorCodes::CANNOT_PARSE_INPUT_ASSERTION_FAILED,
                    "Invalid KeeperMap metadata format version or columns definition in ZK: {}", zk_metadata_string);

  auto zk_pk_pos = zk_metadata_string.rfind(primary_key_header);
  if (zk_pk_pos == std::string::npos)
    throw Exception(ErrorCodes::CANNOT_PARSE_INPUT_ASSERTION_FAILED,
                    "Invalid KeeperMap metadata format version or primary key definition in ZK: {}", zk_metadata_string);

  auto local_pk_pos = local_metadata_string.rfind(primary_key_header);
  if (local_pk_pos == std::string::npos)
    throw Exception(ErrorCodes::CANNOT_PARSE_INPUT_ASSERTION_FAILED,
                    "Invalid local KeeperMap metadata format version or primary key definition: {}", local_metadata_string);

  auto local_columns = ColumnsDescription::parse(
      local_metadata_string.substr(metadata_format_version_prefix.size(), local_pk_pos - metadata_format_version_prefix.size()));
  auto zk_columns = ColumnsDescription::parse(
      zk_metadata_string.substr(metadata_format_version_prefix.size(), zk_pk_pos - metadata_format_version_prefix.size()));
  ...
  if (columns_equal && pk_equal) return true;

  if (throw_on_error) {
    throw Exception(ErrorCodes::BAD_ARGUMENTS,
                    "Path {} is already used but the stored {} definition doesn't match. Stored metadata: {}, local metadata: {}",
                    columns_equal ? "columns" : "primary key", zk_root_path, zk_metadata_string, local_metadata_string);
  }
...
void StorageKeeperMap::read(QueryPlan &query_plan, const Names &column_names, const StorageSnapshotPtr &storage_snapshot,
                            SelectQueryInfo &query_info, ContextPtr context_, QueryProcessingStage::Enum /*processed_stage*/,
  ...
  if (with_version_column)
    sample_block.insert({DataTypeInt32{}.createColumn(), std::make_shared<DataTypeInt32>(), std::string{version_column_name}});

  auto reading = std::make_unique<ReadFromKeeperMap>(column_names, query_info, storage_snapshot, context_,
                                                     std::make_shared<const Block>(std::move(sample_block)), *this, max_block_size,
                                                     num_streams, with_version_column);
...
void ReadFromKeeperMap::initializePipelineImpl(QueryPipelineBuilder &pipeline, KeyContainerPtr key_container) {
  ...
  for (size_t thread_idx = 0; thread_idx < num_threads; ++thread_idx) {
    ...
    pipes.emplace_back(std::make_shared<StorageKeeperMapSource<KeyContainer>>(storage, sample_block, max_block_size, key_container,
                                                                              key_container->begin() + begin, key_container->begin() + end,
                                                                              with_version_column, context));
  }
...
void StorageKeeperMap::truncate(const ASTPtr &, const StorageMetadataPtr &, ContextPtr local_context, TableExclusiveLockHolder &) {
  ...
  ZooKeeperRetriesControl zk_retry{
      getName(), getLogger(getName()),
      ZooKeeperRetriesInfo{settings[Setting::keeper_max_retries], settings[Setting::keeper_retry_initial_backoff_ms],
                           settings[Setting::keeper_retry_max_backoff_ms], local_context->getProcessListElement()}};
...
void StorageKeeperMap::dropTableFromZooKeeper(zkutil::ZooKeeperPtr zookeeper, String path_prefix_, String zk_root_path_, String uuid,
                                              LoggerPtr logger) {
  ...
  for (const auto &table : tables) {
    if (table.starts_with(uuid)) {
      ...
      if (code == Coordination::Error::ZNONODE)
        throw Exception(ErrorCodes::TABLE_WAS_NOT_DROPPED, "Table at {} is already started to be removed by another replica right now",
                        zk_root_path_);
    }
  }
  ...
  if (zookeeper->exists(zk_tables_path_to_remove)) ops.emplace_back(zkutil::makeRemoveRequest(zk_tables_path_to_remove, -1));
  ...
  if (code == Coordination::Error::ZNOTEMPTY) return;
  ...
  if (!dropTableData(zookeeper, drop_lock, zk_data_path_to_remove, zk_metadata_path_to_remove, zk_dropped_path_to_remove,
                     zk_dropped_lock_version_path, zk_root_path_, logger))
    throw Exception(ErrorCodes::TABLE_WAS_NOT_DROPPED, "Table at {} is already started to be removed by another replica right now",
                    zk_root_path_);
}
...
bool StorageKeeperMap::dropTableData(zkutil::ZooKeeperPtr zookeeper, const zkutil::EphemeralNodeHolder::Ptr &metadata_drop_lock) {
  return dropTableData(zookeeper, metadata_drop_lock, zk_data_path, zk_metadata_path, zk_dropped_path, zk_dropped_lock_version_path,
                       zk_root_path, log);
}

bool StorageKeeperMap::dropTableData(zkutil::ZooKeeperPtr zookeeper, const zkutil::EphemeralNodeHolder::Ptr &metadata_drop_lock,
                                     const String &zk_data_path_, const String &zk_metadata_path_, const String &zk_dropped_path_,
                                     const String &zk_dropped_lock_version_path_, const String &zk_root_path_, LoggerPtr logger) {
...
void StorageKeeperMap::drop() {
  ...
  if (current_table_status == TableStatus::UNKNOWN) {
    static constexpr auto error_msg =
        "Failed to activate table because of connection issues. It will be activated "
  ...
  if (current_table_status == TableStatus::INVALID_METADATA) return;
  ...
  if (auto code = client->tryRemove(zk_table_path); code == Coordination::Error::ZNOTEMPTY) {
    throw zkutil::KeeperException(code, "{} contains children which shouldn't happen. Please DETACH the table if you want to delete it",
                                  zk_table_path);
  }
  ...
  if (auto code = client->tryGetChildren(zk_tables_path, children); code != Coordination::Error::ZOK || !children.empty()) return;

  // used in private build
  bool do_not_drop_table_data_in_keeper = false;
  if (do_not_drop_table_data_in_keeper) return;
  ...
  if (code != Coordination::Error::ZOK) zkutil::KeeperMultiException::check(code, ops, responses);
...
class KeeperMapBackup : public IBackupEntriesLazyBatch, boost::noncopyable {
 public:
  KeeperMapBackup(const std::string &data_zookeeper_path_, const std::string &data_path_in_backup,
                  const TemporaryDataOnDiskScopePtr &tmp_data_, std::shared_ptr<WithRetries> with_retries_)
      : data_zookeeper_path(data_zookeeper_path_), tmp_data(tmp_data_), with_retries(std::move(with_retries_)) {
 ...
 private:
  ...
  BackupEntries generate() override {
    ...
    const auto write_rows = [&](std::span<std::string> keys) {
      ...
      for (const auto &key : keys) keys_full_path.push_back(data_zookeeper_path / key);
      ...
      for (size_t i = 0; i < keys.size(); ++i) {
        ...
        if (child_data.error != Coordination::Error::ZOK) continue;
...
void StorageKeeperMap::backupData(BackupEntriesCollector &backup_entries_collector, const String &data_path_in_backup,
  ...
  auto post_collecting_task = [coordination, &backup_entries_collector, my_data_path_in_backup = data_path_in_backup, this] {
    ...
    auto tmp_data =
        std::make_shared<TemporaryDataOnDiskScope>(backup_entries_collector.getContext()->getTempDataOnDisk(), tmp_data_settings);

    auto self = std::static_pointer_cast<StorageKeeperMap>(shared_from_this());
    auto with_retries = std::make_shared<WithRetries>(
        getLogger(fmt::format("StorageKeeperMapBackup ({})", getStorageID().getNameForLogs())), [self] { return self->getClient(); },
        BackupKeeperSettings(backup_entries_collector.getContext()), backup_entries_collector.getContext()->getProcessListElement());

    backup_entries_collector.addBackupEntries(
        std::make_shared<KeeperMapBackup>(this->zk_data_path, path_with_data, tmp_data, std::move(with_retries))->getBackupEntries());
  };

  backup_entries_collector.addPostTask(post_collecting_task);
}

void StorageKeeperMap::restoreDataFromBackup(RestorerFromBackup &restorer, const String &data_path_in_backup,
                                             const std::optional<ASTs> & /*partitions*/) {
  ...
  auto with_retries = std::make_shared<WithRetries>(
      getLogger(fmt::format("StorageKeeperMapRestore ({})", getStorageID().getNameForLogs())), [&] { return getClient(); },
      BackupKeeperSettings(restorer.getContext()), restorer.getContext()->getProcessListElement());
  ...
  if (!allow_non_empty_tables) {
    ...
    if (data_stats.numChildren != 0) RestorerFromBackup::throwTableIsNotEmpty(getStorageID());
  }

  /// only 1 table should restore data for a single path
  restorer.addDataRestoreTask(
      [storage = std::static_pointer_cast<StorageKeeperMap>(shared_from_this()), backup, data_path_in_backup, with_retries,
       allow_non_empty_tables] { storage->restoreDataImpl(backup, data_path_in_backup, with_retries, allow_non_empty_tables); });
}

void StorageKeeperMap::restoreDataImpl(const BackupPtr &backup, const String &data_path_in_backup,
                                       std::shared_ptr<WithRetries> with_retries, bool allow_non_empty_tables) {
  ...
  if (!create_requests.empty()) flush_create_requests();
}
...
const std::string &StorageKeeperMap::dataPath() const { return zk_data_path; }

std::string StorageKeeperMap::fullPathForKey(const std::string_view key) const { return fs::path(zk_data_path) / key; }

UInt64 StorageKeeperMap::keysLimit() const { return keys_limit; }

StorageKeeperMap::TableStatus StorageKeeperMap::getTableStatus(const ContextPtr &local_context) const {
  std::lock_guard lock{init_mutex};
  if (table_status != TableStatus::UNKNOWN) return table_status;
  ...
  [&] {
    try {
      ...
      if (!Coordination::isHardwareError(e.code)) table_status = TableStatus::INVALID_KEEPER_STRUCTURE;
    }
  }();
...
Chunk StorageKeeperMap::getByKeys(const ColumnsWithTypeAndName &keys, const Names &, PaddedPODArray<UInt8> &null_map,
                                  IColumn::Offsets & /* out_offsets */) const {
  if (keys.size() != 1) throw Exception(ErrorCodes::LOGICAL_ERROR, "StorageKeeperMap supports only one key, got: {}", keys.size());
...
Chunk StorageKeeperMap::getBySerializedKeys(const std::span<const std::string> keys, PaddedPODArray<UInt8> *null_map, bool with_version,
                                            const ContextPtr &local_context) const {
  Block sample_block = getInMemoryMetadataPtr()->getSampleBlock();
  MutableColumns columns = sample_block.cloneEmptyColumns();
  MutableColumnPtr version_column = nullptr;

  if (with_version) version_column = ColumnVector<Int32>::create();
  ...
  for (const auto &key : keys) full_key_paths.emplace_back(fullPathForKey(key));
  ...
  for (size_t i = 0; i < keys.size(); ++i) {
    ...
    if (code == Coordination::Error::ZOK) {
      ...
      if (version_column) version_column->insert(response.stat.version);
      ...
      if (null_map) {
        ...
        if (version_column) version_column->insert(-1);
      }
    } else {
  ...
  if (version_column) columns.push_back(std::move(version_column));
...
void StorageKeeperMap::checkTableCanBeRenamed(const StorageID &new_name) const { verifyTableId(new_name); }
...
void StorageKeeperMap::checkMutationIsPossible(const MutationCommands &commands, const Settings & /*settings*/) const {
  ...
  if (commands.size() > 1) throw Exception(ErrorCodes::BAD_ARGUMENTS, "Mutations cannot be combined for KeeperMap");
...
void StorageKeeperMap::mutate(const MutationCommands &commands, ContextPtr local_context) {
  ...
  if (commands.front().type == MutationCommand::Type::DELETE) {
    ...
    auto interpreter = std::make_unique<MutationsInterpreter>(storage_ptr, metadata_snapshot, commands, local_context, mutation_settings);
    ...
    while (executor.pull(block)) {
      ...
      if (status == Coordination::Error::ZOK) return;

      if (status != Coordination::Error::ZNONODE) throw zkutil::KeeperMultiException(status, delete_requests, responses);
  ...
  auto interpreter = std::make_unique<MutationsInterpreter>(storage_ptr, metadata_snapshot, commands, local_context, settings);
...
StoragePtr create(const StorageFactory::Arguments &args) {
  ...
  UInt64 keys_limit = 0;
  if (engine_args.size() > 1) keys_limit = checkAndGetLiteralArgument<UInt64>(engine_args[1], "keys_limit");
  ...
  if (!args.storage_def->primary_key) throw Exception(ErrorCodes::BAD_ARGUMENTS, "StorageKeeperMap requires one column in primary key");

  metadata.primary_key = KeyDescription::getKeyFromAST(args.storage_def->primary_key->ptr(), metadata.columns, args.getContext());
  auto primary_key_names = metadata.getColumnsRequiredForPrimaryKey();
  if (primary_key_names.size() != 1) throw Exception(ErrorCodes::BAD_ARGUMENTS, "StorageKeeperMap requires one column in primary key");

  // used in private build
  bool override_metadata = false;

  return std::make_shared<StorageKeeperMap>(args.getContext(), args.table_id, metadata, args.query.attach, primary_key_names[0],
                                            zk_root_path, keys_limit, override_metadata);
}

}  // namespace

void registerStorageKeeperMap(StorageFactory &factory) {
  factory.registerStorage("KeeperMap", create,
                          {
