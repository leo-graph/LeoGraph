...
std::string makeMetadataKey(const std::string& zookeeper_name, const std::string& zookeeper_path) {
  if (zookeeper_name == zkutil::DEFAULT_ZOOKEEPER_NAME) return zookeeper_path;
...
void StreamingStorageRegistry::registerTable(const StorageID& storage) {
  ...
  if (shutdown_called) throw Exception(ErrorCodes::LOGICAL_ERROR, "Shutdown was called");

  const bool inserted = storages.emplace(storage).second;
  if (!inserted) throw Exception(ErrorCodes::LOGICAL_ERROR, "Table with storage id {} already registered", storage.getNameForLogs());
...
void StreamingStorageRegistry::unregisterTable(const StorageID& storage, bool if_exists) {
  ...
  if (it == storages.end()) {
    ...
    throw Exception(ErrorCodes::LOGICAL_ERROR, "Table with storage id {} is not registered", storage.getNameForLogs());
  }
  storages.erase(it);
...
void StreamingStorageRegistry::shutdown() {
  ...
  ThreadPool pool(CurrentMetrics::ObjectStorageQueueShutdownThreads, CurrentMetrics::ObjectStorageQueueShutdownThreadsActive,
...
ObjectStorageQueueMetadataFactory::FilesMetadataPtr ObjectStorageQueueMetadataFactory::getOrCreate(const std::string& zookeeper_name,
...
void ObjectStorageQueueMetadataFactory::remove(const std::string& zookeeper_name, const std::string& zookeeper_path,
                                               const StorageID& storage_id, bool is_drop, bool keep_data_in_keeper) {
  std::lock_guard lock(mutex);
  const auto metadata_key = makeMetadataKey(zookeeper_name, zookeeper_path);
  auto it = metadata_by_path.find(metadata_key);
  ...
  *it->second.ref_count -= 1;
  ...
  LOG_TRACE(log, "Removed table {} (is drop: {}, keep data in keeper: {})", storage_id.getNameForLogs(), is_drop, keep_data_in_keeper);

  if (is_drop) {
    try {
      it->second.metadata->unregisterNonActive(storage_id,
  ...
  if (*it->second.ref_count == 0) metadata_by_path.erase(it);
}
