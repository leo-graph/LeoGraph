...
#define LIST_OF_EXECUTABLE_SETTINGS(DECLARE, ALIAS)                                                                \
  ...
  DECLARE(ExternalCommandStderrReaction, stderr_reaction, ExternalCommandStderrReaction::NONE,                     \
          "Reaction when external command outputs data to its stderr.", 0)                                         \
...
#define INITIALIZE_SETTING_EXTERN(TYPE, NAME, DEFAULT, DESCRIPTION, FLAGS, ...) \
  ExecutableSettings##TYPE NAME = &ExecutableSettingsImpl ::NAME;
...
ExecutableSettings::ExecutableSettings()
    : script_name({}), script_arguments({}), is_executable_pool(false), impl(std::make_unique<ExecutableSettingsImpl>()) {}
...
void ExecutableSettings::applyChanges(const SettingsChanges& changes) { impl->applyChanges(changes); }

bool ExecutableSettings::hasBuiltin(std::string_view name) { return ExecutableSettingsImpl::hasBuiltin(name); }
}  // namespace DB
