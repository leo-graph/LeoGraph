...
MessageQueueSink::MessageQueueSink(SharedHeader header, const String &format_name_, size_t max_rows_per_message_,
...
void MessageQueueSink::onStart() {
  LOG_TEST(getLogger("MessageQueueSink"), "Executing startup for MessageQueueSink");
...
void MessageQueueSink::onFinish() {
  if (format) format->finalize();
  if (buffer) buffer->finalize();
  if (producer) producer->finish();
}

void MessageQueueSink::onException(std::exception_ptr /* exception */) { onFinish(); }
  ...
  if (row_format) {
    ...
    while (row < chunk.getNumRows()) {
      ...
      for (; i < max_rows_per_message && row < chunk.getNumRows(); ++i, ++row) {
        if (i != 0) row_format->writeRowBetweenDelimiter();
