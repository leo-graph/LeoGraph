...
bool isUnordered(ObjectStorageQueueMode mode) { return mode == ObjectStorageQueueMode::UNORDERED; }
...
ObjectStorageQueueMetadata::ObjectStorageQueueMetadata(ObjectStorageType storage_type_, const std::string& zookeeper_name_,
                                                       const fs::path& zookeeper_path_,
                                                       const ObjectStorageQueueTableMetadata& table_metadata_,
                                                       size_t cleanup_interval_min_ms_, size_t cleanup_interval_max_ms_,
                                                       bool use_persistent_processing_nodes_,
                                                       size_t persistent_processing_nodes_ttl_seconds_, size_t keeper_multiread_batch_size_,
                                                       size_t metadata_cache_size_bytes_, size_t metadata_cache_size_elements_)
    : table_metadata(table_metadata_),
      ...
      log(getLogger(fmt::format("StorageObjectStorageQueue({}{})",
                                zookeeper_name_ == zkutil::DEFAULT_ZOOKEEPER_NAME ? "" : zookeeper_name_ + ":", zookeeper_path_.string()))),
      local_file_statuses(CurrentMetrics::ObjectStorageQueueMetadataCacheSizeBytes,
                          CurrentMetrics::ObjectStorageQueueMetadataCacheSizeElements, metadata_cache_size_bytes_,
  ...
  if (partitioning_mode == ObjectStorageQueuePartitioningMode::REGEX) {
    ...
    filename_parser =
        std::make_unique<ObjectStorageQueueFilenameParser>(table_metadata.partition_regex, table_metadata.partition_component);

    if (!filename_parser->isValid()) {
      throw Exception(ErrorCodes::BAD_ARGUMENTS, "Failed to initialize filename parser: {}", filename_parser->getError());
    }
  ...
  LOG_TRACE(log,
            "Mode: {}, buckets: {}, processing threads: {}, "
...
ObjectStorageQueueMetadata::~ObjectStorageQueueMetadata() { shutdown(); }
...
ZooKeeperWithFaultInjection::Ptr ObjectStorageQueueMetadata::getZooKeeper(LoggerPtr log, const String& zookeeper_name) {
  ...
  if (context->getSettingsRef()[Setting::s3queue_keeper_fault_injection_probability] != 0.0) {
    return ZooKeeperWithFaultInjection::createInstance(context->getSettingsRef()[Setting::s3queue_keeper_fault_injection_probability],
                                                       /* seed */ 0, zk_client, "S3Queue", log);
  }
...
ZooKeeperRetriesControl ObjectStorageQueueMetadata::getKeeperRetriesControl(LoggerPtr log) {
  ...
  return ZooKeeperRetriesControl{
      ...
      ZooKeeperRetriesInfo{settings[Setting::keeper_max_retries], settings[Setting::keeper_retry_initial_backoff_ms],
                           settings[Setting::keeper_retry_max_backoff_ms], context->getProcessListElement()}};
}

void ObjectStorageQueueMetadata::startup() {
  if (startup_called.exchange(true)) return;

  if (!cleanup_task && (cleanup_processed_files || cleanup_failed_files || cleanup_processing_files)) {
    cleanup_task = Context::getGlobalContextInstance()->getSchedulePool().createTask(
        StorageID::createEmpty(), "ObjectStorageQueueCleanupFunc", [this] { cleanupThreadFunc(); });

    cleanup_task->activate();
    cleanup_task->scheduleAfter(generateRescheduleInterval(cleanup_interval_min_ms, cleanup_interval_max_ms));
  }
  if (!update_registry_thread) update_registry_thread = std::make_unique<ThreadFromGlobalPool>([this]() { updateRegistryFunc(); });
}

void ObjectStorageQueueMetadata::shutdown() {
  shutdown_called = true;
  if (cleanup_task) cleanup_task->deactivate();
  if (update_registry_thread && update_registry_thread->joinable()) update_registry_thread->join();
}

ObjectStorageQueueMetadata::FileMetadataPtr ObjectStorageQueueMetadata::getFileMetadata(
    const std::string& path, ObjectStorageQueueOrderedFileMetadata::BucketInfoPtr bucket_info) {
  chassert(metadata_ref_count);
  auto [file_status, _] = local_file_statuses.getOrSet(
      getMetadataCacheKey(path), [&]() { return std::make_shared<ObjectStorageQueueIFileMetadata::FileStatus>(path); });
  switch (mode) {
    case ObjectStorageQueueMode::ORDERED:
      return std::make_shared<ObjectStorageQueueOrderedFileMetadata>(
          zookeeper_path, path, file_status, bucket_info, buckets_num, table_metadata.loading_retries, *metadata_ref_count,
          use_persistent_processing_nodes, zookeeper_name, bucketing_mode, partitioning_mode, filename_parser.get(), log);
    case ObjectStorageQueueMode::UNORDERED:
      return std::make_shared<ObjectStorageQueueUnorderedFileMetadata>(zookeeper_path, path, file_status, table_metadata.loading_retries,
                                                                       *metadata_ref_count, use_persistent_processing_nodes, zookeeper_name,
                                                                       log);
  }
}

bool ObjectStorageQueueMetadata::useBucketsForProcessing() const { return mode == ObjectStorageQueueMode::ORDERED && (buckets_num > 1); }
...
std::optional<std::string> ObjectStorageQueueMetadata::getStartAfterForListing() const {
  ...
  if (storage_type != ObjectStorageType::S3 || mode != ObjectStorageQueueMode::ORDERED ||
      partitioning_mode != ObjectStorageQueuePartitioningMode::NONE)
    return std::nullopt;

  const size_t buckets = std::max<size_t>(getBucketsNum(), 1);
  const auto last_processed_paths =
      ObjectStorageQueueOrderedFileMetadata::getLastProcessedPaths(zookeeper_path, buckets, partitioning_mode, zookeeper_name, log);
  ...
  if (last_processed_paths.size() != buckets) return std::nullopt;
  ...
  for (const auto& last : last_processed_paths) {
    ...
    if (!min_path || last < *min_path) min_path = last;
  }

  return min_path;
}

ObjectStorageQueueOrderedFileMetadata::BucketHolderPtr ObjectStorageQueueMetadata::tryAcquireBucket(const Bucket& bucket) {
  return ObjectStorageQueueOrderedFileMetadata::tryAcquireBucket(zookeeper_path, bucket, use_persistent_processing_nodes, zookeeper_name,
                                                                 log);
}
...
void ObjectStorageQueueMetadata::alterSettings(const SettingsChanges& changes, const ContextPtr& context) {
  ...
  if (is_initial_query) {
    ...
    for (size_t i = 0; i < num_tries; ++i) {
      alter_settings_lock =
          zkutil::EphemeralNodeHolder::tryCreate(alter_settings_lock_path, *zookeeper->getKeeper(), toString(getCurrentTime()));

      if (alter_settings_lock) break;

      if (i == num_tries - 1) throw Exception(ErrorCodes::TIMEOUT_EXCEEDED, "Failed to take alter setting lock after 5 seconds");
  ...
  for (const auto& change : changes) {
    ...
    if (!ObjectStorageQueueTableMetadata::isStoredInKeeper(change.name)) continue;

    if (change.name == "processing_threads_num") {
      const auto value = change.value.safeGet<UInt64>();
      if (table_metadata.processing_threads_num == value) {
        LOG_TRACE(log,
                  "Setting `processing_threads_num` already equals {}. "
                  "Will do nothing",
                  value);
        continue;
      }
      ...
      if (table_metadata.loading_retries == value) {
        LOG_TRACE(log,
                  "Setting `loading_retries` already equals {}. "
                  "Will do nothing",
                  value);
        continue;
      }
      ...
      if (table_metadata.after_processing == value) {
        LOG_TRACE(log,
                  "Setting `after_processing` already equals {}. "
                  "Will do nothing",
                  value);
        continue;
      }
      ...
      if (table_metadata.tracked_files_limit == value) {
        LOG_TRACE(log,
                  "Setting `tracked_files_limit` already equals {}. "
                  "Will do nothing",
                  value);
        continue;
      }
      ...
      if (table_metadata.tracked_files_ttl_sec == value) {
        LOG_TRACE(log,
                  "Setting `tracked_file_ttl_sec` already equals {}. "
                  "Will do nothing",
                  value);
        continue;
      }
      ...
      if (mode != ObjectStorageQueueMode::ORDERED) {
        throw Exception(ErrorCodes::SUPPORT_IS_DISABLED, "Changing `buckets` setting is allowed only for Ordered mode");
      }
  ...
  if (is_initial_query) zookeeper->set(table_metadata_path, new_metadata_str, stat.version);
...
void ObjectStorageQueueMetadata::migrateToBucketsInKeeper(size_t value) {
  ...
  ObjectStorageQueueOrderedFileMetadata::migrateToBuckets(zookeeper_path, value,
                                                          /* prev_value */ table_metadata.buckets, zookeeper_name);
  table_metadata.buckets = value;
  buckets_num = table_metadata.getBucketsNum();
}

ObjectStorageQueueTableMetadata ObjectStorageQueueMetadata::syncWithKeeper(const String& zookeeper_name, const fs::path& zookeeper_path,
                                                                           const ObjectStorageQueueSettings& settings,
                                                                           const ColumnsDescription& columns, const std::string& format,
                                                                           const ContextPtr& context, bool is_attach, LoggerPtr log) {
  ...
  if (settings[ObjectStorageQueueSetting::mode] == ObjectStorageQueueMode::ORDERED) {
    buckets_num = table_metadata.getBucketsNum();
    if (buckets_num == 0) throw Exception(ErrorCodes::LOGICAL_ERROR, "Cannot have zero values of `processing_threads_num` and `buckets`");
  ...
  for (size_t i = 0; i < 1000; ++i) {
    ...
    zk_retries.retryLoop([&] {
      ...
      if (!warned && settings_ref[Setting::cloud_mode] && table_metadata.getMode() == ObjectStorageQueueMode::ORDERED &&
          table_metadata.buckets <= 1 && table_metadata.processing_threads_num <= 1) {
        const std::string message =
            "Ordered mode in cloud without "
      ...
      if (!table_metadata.last_processed_path.empty()) {
        ...
        if (table_metadata.getPartitioningMode() == ObjectStorageQueuePartitioningMode::REGEX) {
          parser = std::make_unique<ObjectStorageQueueFilenameParser>(table_metadata.partition_regex, table_metadata.partition_component);
        }

        ObjectStorageQueueOrderedFileMetadata(
            zookeeper_path, table_metadata.last_processed_path,
            std::make_shared<ObjectStorageQueueIFileMetadata::FileStatus>(table_metadata.last_processed_path),
            /* bucket_info */ nullptr, buckets_num, table_metadata.loading_retries, noop,
            /* use_persistent_processing_nodes */ false,  /// Processing nodes will not be created.
            zookeeper_name, table_metadata.getBucketingMode(), table_metadata.getPartitioningMode(), parser.get(), log)
            .prepareProcessedAtStartRequests(requests);
      }
    ...
    if (code.has_value()) {
      ...
      if (*code != Coordination::Error::ZOK) zkutil::KeeperMultiException::check(*code, requests, responses);
    }
...
struct Info {
  ...
  static Info deserialize(std::string_view str) {
    ...
    buf >> info.hostname >> "\n";
    buf >> info.table_id >> "\n";
    if (info.version >= 1) buf >> info.server_uuid >> "\n";
    return info;
  }
};
}  // namespace
...
void ObjectStorageQueueMetadata::registerActive(const StorageID& storage_id) {
  ...
  getKeeperRetriesControl(log).retryLoop(
      [&] { code = getZooKeeper()->tryCreate(table_path, self.serialize(), zkutil::CreateMode::Ephemeral); });

  if (code != Coordination::Error::ZOK && code != Coordination::Error::ZNODEEXISTS) throw zkutil::KeeperException(code);
  ...
  for (size_t i = 0; i < max_tries; ++i) {
    ...
    zk_retries.retryLoop([&] {
      ...
      if (registry_exists) {
        ...
        requests.push_back(zkutil::makeCreateRequest(registry_path, self.serialize(), zkutil::CreateMode::Persistent));
...
Strings ObjectStorageQueueMetadata::getRegistered(bool active) {
  ...
  if (active) {
    ...
    if (code != Coordination::Error::ZOK && code != Coordination::Error::ZNONODE) throw zkutil::KeeperException(code);
  } else {
    std::string registry_str;
    zk_retries.retryLoop([&] { getZooKeeper()->tryGet(registry_path, registry_str); });
    if (!registry_str.empty()) splitInto<','>(registered, registry_str);
  }
  return registered;
}
...
void ObjectStorageQueueMetadata::unregisterActive(const StorageID& storage_id) {
  ...
  if (code == Coordination::Error::ZOK) {
    LOG_TRACE(log,
              "Table '{}' has been removed from the active registry "
              "(table path: {})",
    ...
    LOG_DEBUG(log,
              ...
              "(table path: {})",
              storage_id.getNameForLogs(), Coordination::errorMessage(code), table_path);
  }
}
...
void ObjectStorageQueueMetadata::unregisterNonActive(const StorageID& storage_id, bool remove_metadata_if_no_registered) {
  ...
  for (size_t i = 0; i < 1000; ++i) {
    ...
    try {
      ...
      for (const auto& elem : registered) {
        ...
        if (info == self)
          ...
          if (!new_registry_str.empty()) new_registry_str += ",";
    ...
    if (!responses.empty() && supports_remove_recursive &&
        code == Coordination::Error::ZNOTEMPTY)  /// potentiall we reached RemoveRecursive node limit, let's try without it
    {
    ...
    if (Coordination::isHardwareError(code) || code == Coordination::Error::ZBADVERSION) {
      is_retry = true;
      continue;
    }
 ...
 public:
  ...
  void rebuild(const NameSet& servers) {
    ...
    for (const auto& server : servers) {
      for (size_t i = 0; i < virtual_nodes_num; ++i) virtual_nodes.emplace(hash(server + DB::toString(i)), server);
...
std::string ObjectStorageQueueMetadata::getProcessorID(const StorageID& storage_id) { return toString(Info::create(storage_id).hash()); }
...
void ObjectStorageQueueMetadata::filterOutForProcessor(Strings& paths, const StorageID& storage_id) const {
  ...
  if (active_servers.empty() || !active_servers_hash_ring) return;
...
void ObjectStorageQueueMetadata::updateRegistry(const DB::Strings& registered_) {
  ...
  if (registered_set == active_servers) return;
  ...
  if (!active_servers_hash_ring) active_servers_hash_ring = std::make_shared<ServersHashRing>(1000, log);  /// TODO: Add a setting.
...
void ObjectStorageQueueMetadata::cleanupThreadFuncImpl() {
  ...
  auto ephemeral_node =
      zkutil::EphemeralNodeHolder::tryCreate(zookeeper_cleanup_lock_path, *zk_client->getKeeper(), toString(getCurrentTime()));
  ...
  if (cleanup_processing_files) cleanupPersistentProcessingNodes();

  if (table_metadata.hasTrackedFilesLimit()) {
    if (cleanup_processed_files) cleanupTrackedNodes(zookeeper_path / "processed", "processed");

    if (cleanup_failed_files) cleanupTrackedNodes(zookeeper_path / "failed", "failed");
  }

  LOG_TRACE(log, "Node limits check finished");
}

void ObjectStorageQueueMetadata::cleanupTrackedNodes(const std::string& nodes_path, std::string_view description) {
  LOG_TEST(log, "Checking {} nodes for tracking limits", description);

  Strings nodes;
  Coordination::Error code;
  auto zk_retries = getKeeperRetriesControl(log);
  zk_retries.retryLoop([&] { code = getZooKeeper()->tryGetChildren(nodes_path, nodes); });
  if (code != Coordination::Error::ZOK) {
  ...
  auto get_paths = [&] {
    ...
    zk_retries.resetFailures();
    zk_retries.retryLoop([&] { response = getZooKeeper()->tryGet(paths); });
  ...
  for (const auto& node : nodes) {
    ...
    if (paths.size() == keeper_multiread_batch_size) get_paths();
  }

  if (!paths.empty()) get_paths();
  ...
  LOG_TEST(log, "Checking node limits (max size: {}, max age: {}) for {}", table_metadata.tracked_files_limit.load(),
           table_metadata.tracked_files_ttl_sec.load(), get_nodes_str());
  ...
  size_t nodes_to_remove = check_nodes_limit && nodes_limit_exceeded ? nodes.size() - table_metadata.tracked_files_limit : 0;

  const auto remove_nodes = [&](bool node_limit) {
    zk_retries.retryLoop([&] { code = getZooKeeper()->tryMulti(remove_requests, remove_responses); });

    if (code == Coordination::Error::ZOK) {
      if (node_limit) nodes_to_remove -= remove_requests.size();
    } else {
      for (size_t i = 0; i < remove_requests.size(); ++i) {
        if (remove_responses[i]->error == Coordination::Error::ZOK) {
          if (node_limit) --nodes_to_remove;
          ...
          zk_retries.resetFailures();
          zk_retries.retryLoop([&] { code = getZooKeeper()->tryRemove(remove_requests[i]->getPath()); });
          if (code == Coordination::Error::ZOK) {
            if (node_limit) --nodes_to_remove;
          } else {
  ...
  for (const auto& node : sorted_nodes) {
    if (nodes_to_remove) {
      LOG_TRACE(log, "Removing node at path {} ({}) because max files limit is reached", node.metadata.file_path, node.zk_path);
      ...
      if (remove_requests.size() == keeper_multi_batch_size || remove_requests.size() == nodes_to_remove) remove_nodes(/*node_limit=*/true);
      ...
      if (node_age >= table_metadata.tracked_files_ttl_sec) {
        LOG_TRACE(log, "Removing node at path {} ({}) because file ttl is reached", node.metadata.file_path, node.zk_path);

        local_file_statuses.remove(getMetadataCacheKey(node.metadata.file_path));
        remove_requests.push_back(zkutil::makeRemoveRequest(node.zk_path, -1));
        if (remove_requests.size() == keeper_multi_batch_size) remove_nodes(/*node_limit=*/false);
  ...
  if (!remove_requests.empty()) remove_nodes(/*node_limit=*/false);
}

void ObjectStorageQueueMetadata::updateSettings(const SettingsChanges& changes) {
  for (const auto& change : changes) {
    if (change.name == "cleanup_interval_min_ms") cleanup_interval_min_ms = change.value.safeGet<UInt64>();
    if (change.name == "cleanup_interval_max_ms") cleanup_interval_max_ms = change.value.safeGet<UInt64>();
    if (change.name == "use_persistent_processing_nodes") use_persistent_processing_nodes = change.value.safeGet<bool>();
    if (change.name == "persistent_processing_node_ttl_seconds") persistent_processing_node_ttl_seconds = change.value.safeGet<UInt64>();
  }
}
...
void ObjectStorageQueueMetadata::cleanupPersistentProcessingNodes() {
  ...
  Coordination::Error code;
  zk_retries.retryLoop([&] { code = getZooKeeper()->tryGetChildren(zookeeper_persistent_processing_path, persistent_processing_nodes); });
  if (code != Coordination::Error::ZOK) {
    if (code == Coordination::Error::ZNONODE) {
  ...
  auto get_paths = [&] {
    zkutil::ZooKeeper::MultiTryGetResponse response;
    zk_retries.resetFailures();
    zk_retries.retryLoop([&] { response = getZooKeeper()->tryGet(get_batch); });
    ...
    for (size_t i = 0; i < response.size(); ++i) {
      ...
      LOG_TEST(log, "Node: {}, mtime: {}, ttl sec: {}, current time: {}", get_batch[i], response[i].stat.mtime,
               persistent_processing_node_ttl_seconds.load(), current_time);

      if (response[i].stat.mtime / 1000 + persistent_processing_node_ttl_seconds < current_time) nodes_to_remove.push_back(get_batch[i]);
    }
    get_batch.clear();
  };
  ...
  for (const auto& node : persistent_processing_nodes) {
    ...
    if (get_batch.size() == keeper_multiread_batch_size) get_paths();
  }
  ...
  if (!get_batch.empty()) get_paths();

  if (nodes_to_remove.empty()) {
    if (!persistent_processing_nodes.empty())
      LOG_TRACE(log,
                "No persistent processing nodes to remove, "
                "total persistent processing nodes: {}",
                persistent_processing_nodes.size());
    return;
  }

  for (const auto& node : nodes_to_remove) {
    zk_retries.resetFailures();
    zk_retries.retryLoop([&] { code = getZooKeeper()->tryRemove(node); });
    if (code != Coordination::Error::ZOK && code != Coordination::Error::ZNONODE) throw zkutil::KeeperException::fromPath(code, node);
  }
