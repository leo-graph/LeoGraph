...
#define OBJECT_STORAGE_QUEUE_RELATED_SETTINGS(DECLARE, ALIAS)                                                                              \
  DECLARE(ObjectStorageQueueMode, mode, ObjectStorageQueueMode::ORDERED,                                                                   \
          ...
          0)                                                                                                                               \
  DECLARE(ObjectStorageQueueAction, after_processing, ObjectStorageQueueAction::KEEP,                                                      \
          "Delete, keep, move or tag file after successful processing", 0)                                                                 \
  ...
  DECLARE(Bool, parallel_inserts, false,                                                                                                   \
          "By default processing_threads_num will produce one INSERT, but this limits the parallel execution, so better scalability "      \
          "enable this setting (note this will create not one INSERT per file, but one per max_process*_before_commit)",                   \
          0)                                                                                                                               \
  DECLARE(UInt32, enable_logging_to_queue_log, 1, "Enable logging to system table system.(s3/azure_)queue_log", 0)                         \
  DECLARE(String, last_processed_path, "",                                                                                                 \
          "For Ordered mode. Files that have lexicographically smaller file name are considered already processed", 0)                     \
  DECLARE(UInt64, tracked_files_limit, 1000, "For unordered mode. Max set size for tracking processed files in ZooKeeper", 0)              \
  DECLARE(UInt64, tracked_file_ttl_sec, 0,                                                                                                 \
          "Maximum number of seconds to store processed files in ZooKeeper node (store forever by default)", 0)                            \
  ...
  DECLARE(Bool, commit_on_select, false,                                                                                                   \
          "Whether SELECT query from queue table (not materialized view, but direct select from a queue table) needs to commit data and "  \
          "apply after_processing action. See also profile level setting stream_like_engine_allow_direct_select, which needs to be "       \
          "enabled if you want to use direct SELECT queries",                                                                              \
          0)                                                                                                                               \
  DECLARE(Bool, deduplication_v2, true,                                                                                                    \
          "Deduplicate blocks in dependent materialized views using user token set as object_etag:chunk_offset", 0)                        \
  ...
  DECLARE(UInt64, min_insert_block_size_rows_for_materialized_views, 0,                                                                    \
          "Override for profile setting min_insert_block_size_rows_for_materialized_views", 0)                                             \
  DECLARE(UInt64, min_insert_block_size_bytes_for_materialized_views, 0,                                                                   \
          "Override for profile setting min_insert_block_size_bytes_for_materialized_views", 0)                                            \
  ...
  DECLARE(UInt64, max_processed_files_before_commit, 100,                                                                                  \
          "Number of files which can be processed before being committed to keeper (in case of parallel_inserts=true, works on a "         \
          "per-thread basis)",                                                                                                             \
          0)                                                                                                                               \
  DECLARE(UInt64, max_processed_rows_before_commit, 0,                                                                                     \
          "Number of rows which can be processed before being committed to keeper (in case of parallel_inserts=true, works on a "          \
          "per-thread basis)",                                                                                                             \
          0)                                                                                                                               \
  DECLARE(UInt64, max_processed_bytes_before_commit, 0,                                                                                    \
          "Number of bytes which can be processed before being committed to keeper (in case of parallel_inserts=true, works on a "         \
          "per-thread basis)",                                                                                                             \
          0)                                                                                                                               \
  DECLARE(UInt64, max_processing_time_sec_before_commit, 0,                                                                                \
          "Timeout in seconds after which to commit files committed to keeper (in case of parallel_inserts=true, works on a per-thread "   \
          "basis)",                                                                                                                        \
          0)                                                                                                                               \
  DECLARE(ObjectStorageQueuePartitioningMode, partitioning_mode, ObjectStorageQueuePartitioningMode::NONE,                                 \
          "Partitioning strategy: NONE (no partitioning, default), HIVE (path-based like date=2025-01-01/city=NY), or REGEX (extract "     \
          "from filename using partition_regex)",                                                                                          \
          0)                                                                                                                               \
  DECLARE(String, partition_regex, "",                                                                                                     \
          "Regex to extract named capture groups from filename. All named groups are captured. Use partition_component to specify which "  \
          "group is the partition key. Example: '(?P<hostname>[^_]+)_(?P<timestamp>[^_]+)_(?P<sequence>\\d+)'",                            \
          0)                                                                                                                               \
  DECLARE(String, partition_component, "",                                                                                                 \
          "Name of the capture group from partition_regex to use as partition key. Required when using partitioning_mode='regex'. "        \
          "Example: 'hostname'",                                                                                                           \
          0)                                                                                                                               \
  DECLARE(ObjectStorageQueueBucketingMode, bucketing_mode, ObjectStorageQueueBucketingMode::PATH,                                          \
          "Bucketing strategy for Ordered mode parallel processing: PATH (hash full file path, default), PARTITION (hash partition key, "  \
          "requires partitioning_mode != NONE)",                                                                                           \
          0)                                                                                                                               \
  ...
  DECLARE(Bool, use_hive_partitioning, false,                                                                                              \
          "DEPRECATED: Use partitioning_mode='hive' instead. Whether path contains hive partitioning and this engine should process such " \
          "files with a separate processed files tracking per partition.",                                                                 \
          0)
...
void ObjectStorageQueueSettings::dumpToSystemEngineSettingsColumns(MutableColumnsAndConstraints& params, const std::string& table_name,
  ...
  if (!settings_changes_ast) return;
  ...
  auto is_changed = [&](const std::string& setting_name) -> bool {
    return settings_changes.end() != std::find_if(settings_changes.begin(), settings_changes.end(),
...
void ObjectStorageQueueSettings::applyChanges(const SettingsChanges& changes) { impl->applyChanges(changes); }
  ...
  if (storage_def.settings) {
    try {
      ...
      for (auto& change : settings_changes) {
        if (auto maybe_new_name = adjustSettingName(change.name); maybe_new_name.has_value()) change.name = std::string{*maybe_new_name};
        ...
        if (change.name == "current_shard_num")
          ...
          if (!inserted) {
            if (is_attach) {
              LOG_WARNING(getLogger("StorageObjectStorageQueue"),
                          "Storage {} has a duplicated setting {}. "
                          "Will use the first declared setting's value",
                          storage_id.getNameForLogs(), change.name);
            } else {
              throw Exception(ErrorCodes::BAD_ARGUMENTS, "Setting {} is defined multiple times", change.name);
            }
          }
        }
      }

      for (const auto& setting : ignore_settings) settings_changes.removeSetting(setting);
...
Field ObjectStorageQueueSettings::get(const std::string& name) { return impl->get(name); }

bool ObjectStorageQueueSettings::hasBuiltin(std::string_view name) {
  if (auto maybe_new_name = adjustSettingName(name); maybe_new_name.has_value()) name = *maybe_new_name;
