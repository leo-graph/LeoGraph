...
StoragePtr createQueueStorage(const StorageFactory::Arguments& args) {
  ...
  if (engine_args.empty()) throw Exception(ErrorCodes::BAD_ARGUMENTS, "External data source must have arguments");
  ...
  if (!is_attach && args.storage_def->settings) {
    if (auto* path_setting = args.storage_def->settings->changes.tryGet("keeper_path")) {
      ...
      bool is_replicated_database =
          args.getLocalContext()->getClientInfo().query_kind == ClientInfo::QueryKind::SECONDARY_QUERY && database_engine == "Replicated";
  ...
  if ((*queue_settings)[ObjectStorageQueueSetting::use_hive_partitioning]) {
    if (!is_attach && !args.getLocalContext()->getSettingsRef()[Setting::allow_experimental_object_storage_queue_hive_partitioning]) {
  ...
  return std::make_shared<StorageObjectStorageQueue>(std::move(queue_settings), std::move(configuration), args.table_id, args.columns,
                                                     args.constraints, args.comment, args.getContext(), format_settings, args.storage_def,
                                                     args.mode,
...
void registerStorageS3Queue(StorageFactory& factory) {
  factory.registerStorage("S3Queue", [](const StorageFactory::Arguments& args) { return createQueueStorage<StorageS3Configuration>(args); },
                          {
...
void registerStorageAzureQueue(StorageFactory& factory) {
  factory.registerStorage("AzureQueue",
                          [](const StorageFactory::Arguments& args) { return createQueueStorage<StorageAzureConfiguration>(args); },
                          {
                              .supports_settings = true,
                              .supports_schema_inference = true,
