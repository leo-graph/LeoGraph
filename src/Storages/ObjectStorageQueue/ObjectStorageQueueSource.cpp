...
ObjectStorageQueueSource::ObjectStorageQueueObjectInfo::ObjectStorageQueueObjectInfo(
    ...
    : ObjectInfo(RelativePathWithMetadata{object_info.getPath(), object_info.getObjectMetadata()}), file_metadata(file_metadata_) {}

ObjectStorageQueueSource::FileIterator::FileIterator(std::shared_ptr<ObjectStorageQueueMetadata> metadata_,
                                                     ObjectStoragePtr object_storage_, StorageObjectStorageConfigurationPtr configuration_,
                                                     const StorageID &storage_id_, size_t list_objects_batch_size_,
                                                     const ActionsDAG::Node *predicate_, const NamesAndTypesList &virtual_columns_,
                                                     const NamesAndTypesList &hive_partition_columns_to_read_from_file_path_,
                                                     ContextPtr context_, LoggerPtr logger_, bool enable_hash_ring_filtering_,
                                                     bool file_deletion_on_processed_enabled_, std::atomic<bool> &shutdown_called_)
  ...
  object_storage_iterator =
      object_storage->iterate(reading_path.cutGlobs(configuration->supportsPartialPathPrefix()), list_objects_batch_size_,
                              /*with_tags=*/false, start_after);

  matcher = std::make_unique<re2::RE2>(makeRegexpPatternFromGlobs(globbed_key));
  if (!matcher->ok()) {
    throw Exception(ErrorCodes::CANNOT_COMPILE_REGEXP, "Cannot compile regex from glob ({}): {}", globbed_key, matcher->error());
  }
  ...
  if (use_buckets_for_processing) {
    if (!buckets_num) throw Exception(ErrorCodes::LOGICAL_ERROR, "Buckets number cannot be zero");

    for (size_t i = 0; i < buckets_num; ++i) keys_cache_per_bucket.emplace(i, std::make_unique<BucketInfo>());
  }
}
...
std::pair<ObjectInfoPtr, ObjectStorageQueueSource::FileMetadataPtr> ObjectStorageQueueSource::FileIterator::next() {
  std::lock_guard lock(next_mutex);

  bool current_batch_processed = object_infos.empty() || index >= object_infos.size();
  if (is_finished && current_batch_processed) {
    LOG_TEST(log, "is_finished: {}, index: {}, object_infos.size(): {}", is_finished, index, object_infos.size());
    return {};
  }
    ...
    while (new_batch.empty()) {
      ...
      if (filter_expr) {
        ...
        VirtualColumnUtils::filterByPathOrFile(new_batch, paths, filter_expr, virtual_columns,
                                               hive_partition_columns_to_read_from_file_path, getContext());
      ...
      if (!new_batch.empty() && enable_hash_ring_filtering && mode == ObjectStorageQueueMode::UNORDERED) {
        ...
        for (size_t i = 0; i < new_batch.size(); ++i) {
          file_metadatas[i] = metadata->getFileMetadata(new_batch[i]->getPath(),
          ...
          if (set_processing_result.has_value()) {
            ...
            if (processor_info.empty()) processor_info = current_processor_info;

            chassert(processor_info == current_processor_info, fmt::format("{} != {}", processor_info, current_processor_info));
          } else {
            new_batch[i] = nullptr;
          ...
          if (zk_retry.isRetry()) {
            ...
            if (is_multi_read_enabled) {
              ...
              for (size_t i = 0; i < processing_paths_responses.size(); ++i) {
                LOG_TEST(log, "Path {} has processor: {}, current processor: {}", processing_paths[i], processing_paths_responses[i].data,
                         processor_info);
        ...
        if (code == Coordination::Error::ZOK) {
          ...
          LOG_TRACE(log, "Failed to set files as processing in one request: {} ({})", code, requests[failed_idx]->getPath());
        ...
        if (num_successful_objects != new_batch.size()) {
          ...
          for (size_t i = 0; i < num_successful_objects; ++i, ++batch_i) {
            while (batch_i < new_batch.size() && !new_batch[batch_i]) ++batch_i;

            if (batch_i == new_batch.size()) {
              throw Exception(ErrorCodes::LOGICAL_ERROR, "Mismatch num_successful_objects ({}) is less than the number of valid objects",
                              num_successful_objects);
            }
      ...
      if (!new_batch.empty()) {
        ...
        for (const auto &file_metadata : file_metadatas) file_metadata->getFileStatus()->setGetObjectTime(get_object_time_ms);
      }
    }
  ...
  auto result = std::make_pair(object_infos[index], file_metadatas.empty() ? nullptr : file_metadatas[index]);

  if (result.second && result.first->getPath() != result.second->getPath()) {
    throw Exception(ErrorCodes::LOGICAL_ERROR, "Mismatch {} and {}", result.first->getPath(), result.second->getPath());
  }
...
void ObjectStorageQueueSource::FileIterator::filterProcessableFiles(ObjectInfos &objects) {
  ...
  for (const auto &object : objects) paths.push_back(object->getPath());

  if (enable_hash_ring_filtering && mode == ObjectStorageQueueMode::UNORDERED) metadata->filterOutForProcessor(paths, storage_id);
  ...
  if (mode == ObjectStorageQueueMode::UNORDERED)
    ...
    ObjectStorageQueueOrderedFileMetadata::filterOutProcessedAndFailed(paths, metadata->getPath(), metadata->getBucketsNum(),
                                                                       zookeeper_name, metadata->getBucketingMode(),
                                                                       metadata->getPartitioningMode(), metadata->getFilenameParser(), log);
  ...
  for (auto &object : objects) {
    if (paths_set.contains(object->getPath())) result.push_back(std::move(object));
  }
...
ObjectInfoPtr ObjectStorageQueueSource::FileIterator::next(size_t processor) {
  while (!shutdown_called) {
    ...
    chassert(use_buckets_for_processing == metadata->useBucketsForProcessing(),
    ...
    if (use_buckets_for_processing) {
      ...
      if (objects_to_retry.empty()) {
        ...
        if (!object_info) iterator_finished = true;
      } else {
    ...
    if (!file_metadata) {
      ...
      if (!file_metadata->trySetProcessing()) continue;
    }
...
void ObjectStorageQueueSource::FileIterator::returnForRetry(ObjectInfoPtr object_info, FileMetadataPtr file_metadata) {
  ...
  if (use_buckets_for_processing) {
    const auto bucket = ObjectStorageQueueMetadata::getBucketForPath(object_info->getPath(), buckets_num, metadata->getBucketingMode(),
                                                                     metadata->getPartitioningMode(), metadata->getFilenameParser());
    std::lock_guard lock(mutex);
...
void ObjectStorageQueueSource::FileIterator::releaseFinishedBuckets() {
  ...
  for (auto &[processor, holders] : bucket_holders) {
    ...
    for (auto it = holders->begin(); it != holders->end();) {
      ...
      if (cached_info != keys_cache_per_bucket.end()) cached_info->second->processor.reset();

      if (!buckets_str.empty()) buckets_str += ", ";
      buckets_str += toString(bucket);

      it = holders->erase(it);
    }
    LOG_TRACE(log,
              "Released {} bucket holders for processor {} "
              "(released buckets: {}, remaining holders: {}, remaining bucket: {})",
              released_holders, processor, buckets_str, holders->size(),
              holders->empty() ? "" : toString(holders->front()->getBucketInfo()->bucket));
  }
}

ObjectStorageQueueSource::BucketHolderPtr ObjectStorageQueueSource::FileIterator::tryAcquireBucket(size_t bucket, BucketInfo &bucket_info,
                                                                                                   BucketHolders &acquired_buckets,
                                                                                                   size_t processor) const {
  if (bucket_info.processor.has_value()) {
    throw Exception(ErrorCodes::LOGICAL_ERROR, "Bucket already has a processor: {} (attempted processor: {})",
...
std::string ObjectStorageQueueSource::FileIterator::bucketHoldersToString() const {
  ...
  for (const auto &[processor, bucket_holder] : bucket_holders) {
    if (!processors_infos.empty()) processors_infos += ", ";

    processors_infos += fmt::format("processor {} -> {} buckets ", processor, bucket_holder->size());
    if (!bucket_holder->empty()) {
      processors_infos += "(";
      for (const auto &bucket : *bucket_holder) processors_infos += toString(bucket->getBucket()) + " ";
      processors_infos += ")";
    }
  }
...
ObjectStorageQueueSource::FileIterator::NextKeyFromBucket ObjectStorageQueueSource::FileIterator::getNextKeyFromAcquiredBucket(
    size_t processor) {
  std::shared_ptr<BucketHolders> acquired_buckets = [this, processor]() TSA_REQUIRES(mutex) {
    auto it = bucket_holders.find(processor);
    if (it == bucket_holders.end()) it = bucket_holders.emplace(processor, std::make_shared<BucketHolders>()).first;
    return it->second;
  }();

  BucketHolderPtr current_bucket_holder =
      acquired_buckets->empty() || acquired_buckets->back()->isFinished() ? nullptr : acquired_buckets->back();
  ...
  LOG_TEST(log, "Current processor: {}, acquired bucket: {}", processor,
  ...
  while (!shutdown_called) {
      ...
      if (it != keys_cache_per_bucket.end()) {
        ...
        if (!bucket_processor.has_value()) {
          ...
          throw Exception(ErrorCodes::LOGICAL_ERROR,
                          ...
                          processor, bucket_processor.has_value() ? toString(bucket_processor.value()) : "None", bucket,
                          current_bucket_holder->getBucketInfo()->toString(), processor_info.value_or("None"), bucketHoldersToString());
        }
    ...
    if (!current_bucket_holder) {
      ...
      for (auto it = keys_cache_per_bucket.begin(); it != keys_cache_per_bucket.end();) {
        ...
        LOG_TEST(log, "Bucket: {}, cached keys: {}, processor: {}", bucket, bucket_info->keys.size(),
                 bucket_info->processor.has_value() ? toString(bucket_info->processor.value()) : "None");

        if (bucket_info->processor.has_value()) {
          LOG_TEST(log, "Bucket {} is already locked for processing by {} (keys: {})", bucket, bucket_info->processor.value(),
    ...
    while (true) {
      ...
      if (bucket_it == keys_cache_per_bucket.end()) {
        throw Exception(ErrorCodes::LOGICAL_ERROR, "Bucket {} not found in keys cache (buckets keys cache size: {}, expected buckets: {})",
                        bucket, keys_cache_per_bucket.size(), metadata->getBucketsNum());
      }
      BucketInfo &bucket_info = *bucket_it->second;

      LOG_TEST(log, "Found next file: {}, bucket: {}, current bucket: {}, cached_keys: {}", object_info->getFileName(), bucket,
               current_bucket_holder ? toString(current_bucket_holder->getBucket()) : "None", bucket_info.keys.size());
    ...
    if (keys_cache_per_bucket.end() ==
        std::find_if(keys_cache_per_bucket.begin(), keys_cache_per_bucket.end(), [](const auto &v) { return !v.second->keys.empty(); })) {
      break;
    }
  }
  return {};
...
ObjectStorageQueueSource::ObjectStorageQueueSource(
    String name_, size_t processor_id_, std::shared_ptr<FileIterator> file_iterator_, StorageObjectStorageConfigurationPtr configuration_,
    ObjectStoragePtr object_storage_, ProcessingProgressPtr progress_, const ReadFromFormatInfo &read_from_format_info_,
    const std::optional<FormatSettings> &format_settings_, FormatParserSharedResourcesPtr parser_shared_resources_,
    const CommitSettings &commit_settings_, const AfterProcessingSettings &after_processing_settings_,
    std::shared_ptr<ObjectStorageQueueMetadata> files_metadata_, ContextPtr context_, size_t max_block_size_,
    const std::atomic<bool> &shutdown_called_, const std::atomic<bool> &table_is_being_dropped_,
    std::shared_ptr<ObjectStorageQueueLog> system_queue_log_, const StorageID &storage_id_, LoggerPtr log_, bool commit_once_processed_,
    ...
    : ISource(std::make_shared<const Block>(read_from_format_info_.source_header)),
      ...
      log(log_) {
  if (commit_once_processed) transaction_start_time = std::chrono::system_clock::to_time_t(std::chrono::system_clock::now());
}

String ObjectStorageQueueSource::getName() const { return name; }
...
Chunk ObjectStorageQueueSource::generate() {
  ...
  try {
    ...
    if (commit_once_processed) commit(false, getCurrentExceptionMessage(true));

    throw;
  }

  if (!chunk && commit_once_processed) commit(true);
  ...
  while (true) {
    if (isCancelled()) {
      ...
      if (processed_files.empty() || processed_files.back().state != FileState::Processing) {
        LOG_DEBUG(log,
                  ...
                  processed_files.size(), processed_files.empty() ? "None" : magic_enum::enum_name(processed_files.back().state));
      ...
      throw Exception(ErrorCodes::QUERY_WAS_CANCELLED, "Processing was cancelled (having unfinished file: {})", started_file->getPath());
    }
    ...
    if (shutdown_called) {
      ...
      if (table_is_being_dropped) {
        ...
        processed_files.back().state = FileState::Cancelled;
        /// Throw exception to avoid inserting half processed file to destination table.
        throw Exception(ErrorCodes::QUERY_WAS_CANCELLED, "Table is being dropped (having unfinished file: {})", started_file->getPath());
      }

      LOG_DEBUG(log,
                "Shutdown called, but file {} is partially processed ({} rows). "
    ...
    if (reader) {
      ...
      chassert(processed_files.back().metadata->getPath() == reader.getObjectInfo()->getPath(),
               fmt::format("Mismatch {} vs {}", processed_files.back().metadata->getPath(), reader.getObjectInfo()->getPath()));
      ...
      reader =
          StorageObjectStorageSource::createReader(processor_id, file_iterator, configuration, object_storage, read_from_format_info,
                                                   format_settings, context, nullptr, log, max_block_size, parser_shared_resources, nullptr,
      ...
      if (commit_settings.max_processed_files_before_commit) {
        ...
        if (old_processed_files >= commit_settings.max_processed_files_before_commit) {
          LOG_DEBUG(log,
                    "Number of max processed files before commit reached "
                    "(rows: {}, bytes: {}, files: {}, time: {})",
                    progress->processed_rows.load(), progress->processed_bytes.load(), progress->processed_files.load(),
                    progress->elapsed_time.elapsedSeconds());
    ...
    try {
      if (file_status->processed_rows > 0) {
        fiu_do_on(FailPoints::object_storage_queue_fail_in_the_middle_of_file, {
          throw Exception(ErrorCodes::UNKNOWN_EXCEPTION, "Failed to read file. Processed rows: {}", file_status->processed_rows.load());
        });
      }
      ...
      LOG_ERROR(log, "Got an error while pulling chunk: {}. Will set file {} as failed (processed rows: {})", message, path,
                file_status->processed_rows.load());
    ...
    if (result) {
      ...
      LOG_TEST(log, "Read {} rows from file {} (file offset: {}, deduplication token for chunk: {})", chunk.getNumRows(), path, row_offset,
               dedup_token);
      ...
      VirtualColumnUtils::addRequestedFileLikeStorageVirtualsToChunk(
          ...
          {.path = path, .size = object_metadata->size_bytes, .last_modified = object_metadata->last_modified}, getContext());
    ...
    LOG_DEBUG(log, "Processed file {}. Total processed files: {}, processed rows: {}, processed bytes: {}", path,
              progress->processed_files.load(), progress->processed_rows.load(), progress->processed_bytes.load());
    ...
    if (commit_settings.max_processed_rows_before_commit && progress->processed_rows >= commit_settings.max_processed_rows_before_commit) {
      LOG_DEBUG(log,
                "Number of max processed rows before commit reached "
                "(rows: {}, bytes: {}, files: {}, time: {})",
                progress->processed_rows.load(), progress->processed_bytes.load(), progress->processed_files.load(),
                progress->elapsed_time.elapsedSeconds());
      break;
...
void ObjectStorageQueueSource::prepareCommitRequests(Coordination::Requests &requests, bool insert_succeeded,
                                                     StoredObjects &successful_files, PartitionLastProcessedFileInfoMap &file_map,
                                                     LastProcessedFileInfoMapPtr created_nodes, const std::string &exception_message,
                                                     int error_code) {
  if (processed_files.empty()) return;

  LOG_TEST(log, "Having {} files to set as {}", processed_files.size(), insert_succeeded ? "Processed" : "Failed");
  ...
  if (insert_succeeded && is_ordered_mode) {
    for (size_t i = 0; i < processed_files.size(); ++i) {
      ...
      if (processed_files[i].state != FileState::Processed) continue;

      auto [it, inserted] = last_processed_file_idx_per_bucket.emplace(bucket, i);
      if (!inserted && file_path > processed_files[it->second].metadata->getPath()) {
        it->second = i;
      }
    }
  }
  ...
  const bool reduce_retry_count = !(error_code == ErrorCodes::TOO_MANY_PARTS || error_code == ErrorCodes::TABLE_IS_BEING_RESTARTED ||
  ...
  if (!insert_succeeded && reduce_retry_count) {
    for (const auto &[file_state, file_metadata_, exception_during_read_] : processed_files)
      if (file_state == FileState::Processed) ++processed_count;
  }
  ...
  for (size_t i = 0; i < processed_files.size(); ++i) {
    ...
    switch (file_state) {
      case FileState::Processed: {
        ...
        if (insert_succeeded) {
          if (is_ordered_mode) {
            ...
            if (has_partitioning) file_metadata->preparePartitionProcessedMap(file_map);
          } else {
          ...
          file_metadata->prepareFailedRequests(requests, exception_message, reduce_retry_count && processed_count <= 1);
        }
        break;
      }
      ...
      case FileState::Processing: {
        chassert(exception_during_read.empty());
        if (insert_succeeded) {
          throw Exception(ErrorCodes::LOGICAL_ERROR, "Unexpected state {} of file {} while insert succeeded", file_state,
      ...
      case FileState::ErrorOnRead: {
        ...
        chassert(!exception_during_read.empty());
        file_metadata->prepareFailedRequests(requests, exception_during_read,
...
void ObjectStorageQueueSource::preparePartitionProcessedRequests(
    Coordination::Requests &requests, const PartitionLastProcessedFileInfoMap &last_processed_file_per_partition) {
  for (const auto &[partition_processed_path, last_processed_file_info] : last_processed_file_per_partition) {
    if (last_processed_file_info.exists) {
      requests.push_back(zkutil::makeSetRequest(partition_processed_path, last_processed_file_info.file_path, -1));
    } else {
      requests.push_back(
          zkutil::makeCreateRequest(partition_processed_path, last_processed_file_info.file_path, zkutil::CreateMode::Persistent));
    }
  }
}

void ObjectStorageQueueSource::finalizeCommit(bool insert_succeeded, UInt64 commit_id, time_t commit_time, time_t transaction_start_time_,
  ...
  for (const auto &[file_state, file_metadata, exception_during_read] : processed_files) {
    try {
      ...
      appendLogElement(file_metadata,
                       /* processed */ insert_succeeded && file_state == FileState::Processed, commit_id, commit_time,
...
void ObjectStorageQueueSource::commit(bool insert_succeeded, const std::string &exception_message) {
  ...
  prepareCommitRequests(requests, insert_succeeded, successful_objects, last_processed_file_per_partition,
                        /* created_nodes */ nullptr, exception_message);
  ...
  if (!successful_objects.empty() && files_metadata->getTableMetadata().after_processing != ObjectStorageQueueAction::KEEP) {
    auto postProcessor = ObjectStorageQueuePostProcessor(getContext(), configuration->getType(), object_storage, getName(),
                                                         files_metadata->getTableMetadata(), after_processing_settings);
  ...
  if (code != Coordination::Error::ZOK) throw zkutil::KeeperMultiException(code, requests, responses);
...
void ObjectStorageQueueSource::appendLogElement(const ObjectStorageQueueMetadata::FileMetadataPtr &file_metadata_, bool processed,
                                                UInt64 commit_id, time_t commit_time, time_t transaction_start_time_) {
  if (!system_queue_log) return;
  ...
  ObjectStorageQueueLogElement elem{};
  {
    elem = ObjectStorageQueueLogElement{
        ...
        .status = processed ? ObjectStorageQueueLogElement::ObjectStorageQueueStatus::Processed
                            : ObjectStorageQueueLogElement::ObjectStorageQueueStatus::Failed,
