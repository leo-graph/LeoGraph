...
class ObjectStorageQueueSource : public ISource, WithContext {
 public:
  ...
  struct ObjectStorageQueueObjectInfo : public ObjectInfo {
    ObjectStorageQueueObjectInfo(const ObjectInfo& object_info, FileMetadataPtr file_metadata_);
  ...
  class FileIterator : public IObjectIterator, private WithContext {
   public:
    FileIterator(std::shared_ptr<ObjectStorageQueueMetadata> metadata_, ObjectStoragePtr object_storage_,
                 StorageObjectStorageConfigurationPtr configuration_, const StorageID& storage_id_, size_t list_objects_batch_size_,
                 const ActionsDAG::Node* predicate_, const NamesAndTypesList& virtual_columns_,
                 const NamesAndTypesList& hive_partition_columns_to_read_from_file_path_, ContextPtr context_, LoggerPtr logger_,
                 bool enable_hash_ring_filtering_, bool file_deletion_on_processed_enabled_, std::atomic<bool>& shutdown_called_);
   ...
   private:
    ...
    BucketHolderPtr tryAcquireBucket(size_t bucket, BucketInfo& bucket_info, BucketHolders& acquired_buckets, size_t processor) const
        TSA_REQUIRES(mutex);
  };
  ...
  ObjectStorageQueueSource(String name_, size_t processor_id_, std::shared_ptr<FileIterator> file_iterator_,
                           StorageObjectStorageConfigurationPtr configuration_, ObjectStoragePtr object_storage_,
                           ProcessingProgressPtr progress_, const ReadFromFormatInfo& read_from_format_info_,
                           ...
                           std::shared_ptr<ObjectStorageQueueLog> system_queue_log_, const StorageID& storage_id_, LoggerPtr log_,
                           bool commit_once_processed_, bool add_deduplication_info_);
  ...
  void prepareCommitRequests(Coordination::Requests& requests, bool insert_succeeded, StoredObjects& successful_files,
                             PartitionLastProcessedFileInfoMap& file_map, LastProcessedFileInfoMapPtr created_nodes = nullptr,
                             const std::string& exception_message = {}, int error_code = 0);

  static void preparePartitionProcessedRequests(Coordination::Requests& requests,
  ...
  void finalizeCommit(bool insert_succeeded, UInt64 commit_id, time_t commit_time, time_t transaction_start_time_,
                      const std::string& exception_message = {});

 private:
  Chunk generateImpl();
  /// Log to system.s3(azure)_queue_log.
  void appendLogElement(const FileMetadataPtr& file_metadata_, bool processed, UInt64 commit_id, time_t commit_time,
  ...
  struct ProcessedFile {
    explicit ProcessedFile(FileMetadataPtr metadata_) : state(FileState::Processing), metadata(metadata_) {}
