...
class ObjectStorageQueueUnorderedFileMetadata : public ObjectStorageQueueIFileMetadata {
 public:
  ...
  explicit ObjectStorageQueueUnorderedFileMetadata(const std::filesystem::path& zk_path, const std::string& path_,
                                                   FileStatusPtr file_status_, size_t max_loading_retries_,
                                                   std::atomic<size_t>& metadata_ref_count_, bool use_persistent_processing_nodes_,
  ...
  static void filterOutProcessedAndFailed(std::vector<std::string>& paths, const std::filesystem::path& zk_path_,
                                          const std::string& zookeeper_name_, LoggerPtr log_);
