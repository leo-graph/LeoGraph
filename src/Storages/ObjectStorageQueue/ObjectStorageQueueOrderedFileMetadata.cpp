...
std::string getProcessedPathWithoutBucket(const std::filesystem::path& zk_path) { return zk_path / "processed"; }

bool useBucketsForProcessing(size_t buckets_num) { return buckets_num > 1; }
...
std::string getPartitionKey(const std::string& file_path, ObjectStorageQueuePartitioningMode partitioning_mode,
  ...
  switch (partitioning_mode) {
    ...
    case ObjectStorageQueuePartitioningMode::REGEX: {
      if (parser && parser->isValid()) {
        if (auto partition_key = parser->parse(file_path)) return *partition_key;
      }
      return "";
    }
...
std::string getProcessedPath(const std::filesystem::path& zk_path, const std::string& path, size_t buckets_num,
                             ObjectStorageQueueBucketingMode bucketing_mode, ObjectStorageQueuePartitioningMode partitioning_mode,
                             const ObjectStorageQueueFilenameParser* parser) {
...
std::vector<std::string> ObjectStorageQueueOrderedFileMetadata::getLastProcessedPaths(const std::filesystem::path& zk_path_,
                                                                                      size_t buckets_num_,
                                                                                      ObjectStorageQueuePartitioningMode partitioning_mode_,
                                                                                      const std::string& zookeeper_name_, LoggerPtr log_) {
  if (partitioning_mode_ != ObjectStorageQueuePartitioningMode::NONE)
    throw Exception(ErrorCodes::LOGICAL_ERROR, "Getting StartAfter does not support partitioning mode {}",
                    magic_enum::enum_name(partitioning_mode_));
  ...
  if (DB::useBucketsForProcessing(buckets_num_)) {
    ...
    for (size_t bucket = 0; bucket < buckets_num_; ++bucket) processed_node_paths.push_back(getProcessedPathWithBucket(zk_path_, bucket));
  } else {
  ...
  zk_retry.retryLoop([&] { responses = ObjectStorageQueueMetadata::getZooKeeper(log_, zookeeper_name_)->tryGet(processed_node_paths); });

  if (responses.size() != processed_node_paths.size()) {
    throw Exception(ErrorCodes::UNEXPECTED_ZOOKEEPER_ERROR, "Unexpected size of Keeper response, expected {}, got {}",
  ...
  for (size_t i = 0; i < responses.size(); ++i) {
    if (responses[i].error == Coordination::Error::ZNONODE || responses[i].data.empty()) continue;
    ...
    if (!metadata.file_path.empty()) result.emplace_back(std::move(metadata.file_path));
  }

  return result;
}

ObjectStorageQueueOrderedFileMetadata::BucketHolder::BucketHolder(const Bucket& bucket_, const std::string& bucket_lock_path_,
                                                                  const std::string& processor_info_, LoggerPtr log_,
                                                                  const std::string& zookeeper_name_)
    : bucket_info(std::make_shared<BucketInfo>(BucketInfo{
          .bucket = bucket_, .bucket_lock_path = bucket_lock_path_, .processor_info = processor_info_, .zookeeper_name = zookeeper_name_})),
      log(log_) {
...
bool ObjectStorageQueueOrderedFileMetadata::BucketHolder::checkBucketOwnership(std::shared_ptr<ZooKeeperWithFaultInjection> zk_client) {
  ...
  if (!processor_info.has_value()) return false;

  LOG_TEST(log, "Bucket lock node {} has owner: {}, current owner: {}", bucket_info->bucket_lock_path, processor_info.value(),
           bucket_info->processor_info);

  return processor_info.value() == bucket_info->processor_info;
}

std::optional<std::string> ObjectStorageQueueOrderedFileMetadata::BucketHolder::getProcessorInfo(
    std::shared_ptr<ZooKeeperWithFaultInjection> zk_client) {
  std::string data;
  /// No retries, because they must be done on a higher level.
  if (zk_client->tryGet(bucket_info->bucket_lock_path, data)) return data;
  return std::nullopt;
}
...
ObjectStorageQueueOrderedFileMetadata::BucketHolder::~BucketHolder() {
  if (!released) LOG_TEST(log, "Releasing bucket ({}) holder in destructor", bucket_info->bucket);
...
ObjectStorageQueueOrderedFileMetadata::ObjectStorageQueueOrderedFileMetadata(
    const std::filesystem::path& zk_path_, const std::string& path_, FileStatusPtr file_status_, BucketInfoPtr bucket_info_,
    size_t buckets_num_, size_t max_loading_retries_, std::atomic<size_t>& metadata_ref_count_, bool use_persistent_processing_nodes_,
    const std::string& zookeeper_name_, ObjectStorageQueueBucketingMode bucketing_mode_,
    ObjectStorageQueuePartitioningMode partitioning_mode_, const ObjectStorageQueueFilenameParser* parser_, LoggerPtr log_)
    : ObjectStorageQueueIFileMetadata(
          path_, zookeeper_name_,
          ...
          /* failed_node_path */ zk_path_ / "failed" / getNodeName(path_), file_status_, max_loading_retries_, metadata_ref_count_,
          use_persistent_processing_nodes_, log_),
  ...
  LOG_TEST(log,
           ...
           path, node_name, max_loading_retries, processed_node_path, processing_node_path, failed_node_path,
           magic_enum::enum_name(partitioning_mode));
}

bool ObjectStorageQueueOrderedFileMetadata::useBucketsForProcessing() const { return DB::useBucketsForProcessing(buckets_num); }
...
std::vector<std::string> ObjectStorageQueueOrderedFileMetadata::getMetadataPaths(size_t buckets_num) {
  if (DB::useBucketsForProcessing(buckets_num)) {
    ...
    for (size_t i = 0; i < buckets_num; ++i) paths.push_back("buckets/" + toString(i));
    return paths;
  }
...
bool ObjectStorageQueueOrderedFileMetadata::getMaxProcessedNode(NodeMetadata& result, Coordination::Stat* stat,
                                                                const std::string& processed_node_path_, LoggerPtr log_,
                                                                const std::string& zookeeper_name_) {
  std::string data;
  ...
  if (processed_node_exists) {
    if (!data.empty()) result = NodeMetadata::fromString(data);
    return true;
  }
  return false;
}

ObjectStorageQueueOrderedFileMetadata::ProcessingStateFromKeeper ObjectStorageQueueOrderedFileMetadata::getProcessingStateFromKeeper(
    Coordination::Stat* processed_node_stat, bool check_failed, LoggerPtr log_) {
  return getProcessingStateFromKeeper(
      processed_node_stat, processed_node_path, path,
      hasPartitioningMode(partitioning_mode)
          ...
          : std::nullopt,
      check_failed ? std::optional<std::string>(failed_node_path) : std::nullopt, log_, zookeeper_name);
}

ObjectStorageQueueOrderedFileMetadata::ProcessingStateFromKeeper ObjectStorageQueueOrderedFileMetadata::getProcessingStateFromKeeper(
    Coordination::Stat* processed_node_stat, const std::string& processed_node_path_, const std::string& file_path,
    std::optional<std::string> processed_node_hive_partitioning_path, std::optional<std::string> failed_node_path_, LoggerPtr log_,
    const std::string& zookeeper_name_) {
  ...
  if (failed_node_path_.has_value()) paths.push_back(*failed_node_path_);
  ...
  if (processed_node_hive_partitioning_path.has_value()) paths.push_back(*processed_node_hive_partitioning_path);

  zkutil::ZooKeeper::MultiTryGetResponse responses;
  auto zk_retry = ObjectStorageQueueMetadata::getKeeperRetriesControl(log_);
  zk_retry.retryLoop([&] { responses = ObjectStorageQueueMetadata::getZooKeeper(log_, zookeeper_name_)->tryGet(paths); });

  auto check_code = [](auto code, auto path) {
    if (!(code == Coordination::Error::ZOK || code == Coordination::Error::ZNONODE)) throw zkutil::KeeperException::fromPath(code, path);
  };

  if (responses.size() != paths.size()) {
    throw Exception(ErrorCodes::UNEXPECTED_ZOOKEEPER_ERROR, "Unexpected size of Keeper response, expected {}, got {}", paths.size(),
                    responses.size());
  }

  for (size_t i = 0; i < responses.size(); ++i) check_code(responses[i].error, paths[i]);

  bool is_failed = failed_node_path_.has_value() ? responses[1].error == Coordination::Error::ZOK : false;

  if (responses[0].data.empty()) return ProcessingStateFromKeeper(is_failed);

  NodeMetadata result = NodeMetadata::fromString(responses[0].data);
  if (processed_node_stat) *processed_node_stat = responses[0].stat;
...
ObjectStorageQueueOrderedFileMetadata::ProcessingStateFromKeeper::ProcessingStateFromKeeper(const std::string& path,
...
bool ObjectStorageQueueOrderedFileMetadata::getMaxProcessedFilesByHivePartition(
    std::unordered_map<std::string, std::string>& last_processed_path_per_hive_partition, const std::string& processed_node_path_,
    LoggerPtr log_, const std::string& zookeeper_name_) {
...
ObjectStorageQueueOrderedFileMetadata::Bucket ObjectStorageQueueOrderedFileMetadata::getBucketForPath(
    const std::string& path_, size_t buckets_num, ObjectStorageQueueBucketingMode bucketing_mode,
    ObjectStorageQueuePartitioningMode partitioning_mode, const ObjectStorageQueueFilenameParser* parser) {
...
ObjectStorageQueueOrderedFileMetadata::BucketHolderPtr ObjectStorageQueueOrderedFileMetadata::tryAcquireBucket(
    const std::filesystem::path& zk_path, const Bucket& bucket, bool /*use_persistent_processing_nodes_*/,
    const std::string& zookeeper_name_, LoggerPtr log_) {
  auto zk_retry = ObjectStorageQueueMetadata::getKeeperRetriesControl(log_);
  ...
  zk_retry.retryLoop([&] { bucket_exists = ObjectStorageQueueMetadata::getZooKeeper(log_, zookeeper_name_)->exists(bucket_path); });
  ...
  if (code == Coordination::Error::ZOK) {
    ...
    return std::make_shared<BucketHolder>(bucket, bucket_lock_path, processor_info, log_, zookeeper_name_);
  }

  if (code == Coordination::Error::ZNODEEXISTS) return nullptr;
...
std::pair<bool, ObjectStorageQueueIFileMetadata::FileStatus::State> ObjectStorageQueueOrderedFileMetadata::setProcessingImpl() {
  ...
  for (size_t i = 0; i < max_num_tries; ++i) {
    ...
    LOG_TEST(log, "Current max processed file is: {}. Processed node path is: {}",
             state.last_processed_path.has_value() ? *state.last_processed_path : "", processed_node_path);

    if (state.is_processed) return {false, FileStatus::State::Processed};
    ...
    requests.push_back(
        zkutil::makeCreateRequest(processing_node_path, processor_info,
    ...
    if (code == Coordination::Error::ZOK) return {true, FileStatus::State::None};
    ...
    if (has_request_failed(failed_path_doesnt_exist_idx)) return {false, FileStatus::State::Failed};

    if (has_request_failed(create_processing_path_idx)) return {false, FileStatus::State::Processing};
    ...
    LOG_DEBUG(log,
              "Retrying setProcessing because processing node id path is "
              "unexpectedly missing or was not created (error code: {})",
              code);
  }
...
void ObjectStorageQueueOrderedFileMetadata::doPrepareProcessedRequests(Coordination::Requests& requests,
                                                                       const std::string& processed_node_path_, bool ignore_if_exists,
  ...
  if (state.last_processed_path.has_value()) {
    if (state.is_processed) {
      LOG_TRACE(log, "File {} is already processed, current max processed file: {}", path, *state.last_processed_path);

      if (ignore_if_exists) return;

      throw Exception(ErrorCodes::LOGICAL_ERROR, "File ({}) is already processed, while expected it not to be (path: {})", path,
                      processed_node_path_);
    }

    LOG_TEST(log, "Current max processed file is: {}. Path {} hasn't been processed yet", state.last_processed_path.value(), path);
  }
  ...
  if (created_nodes) {
    if (auto it = created_nodes->find(processed_node_path_); it != created_nodes->end()) {
      if (it->second.file_path < path) {
        ...
        LOG_TEST(log, "Path {} was already created in this request pack, overridden with {}", processed_node_path_, path);

        it->second.file_path = path;
        requests[it->second.index] = std::move(request);
      }
    } else {
      created_nodes->emplace(processed_node_path_,
                             LastProcessedFileInfo({/* last_processed_path */ path, /* request_index */ requests.size()}));

      LOG_TEST(log, "Adding {} request for path {} (processed_node_path: {})", state.last_processed_path.has_value() ? "SET" : "CREATE",
               path, processed_node_path_);

      requests.push_back(std::move(request));
    }
  } else {
    LOG_TEST(log, "Adding {} request for path {} (processed_node_path: {})", state.last_processed_path.has_value() ? "SET" : "CREATE", path,
             processed_node_path_);

    requests.push_back(std::move(request));
  }

  if (created_processing_node) requests.push_back(zkutil::makeRemoveRequest(processing_node_path, -1));
}
...
void ObjectStorageQueueOrderedFileMetadata::preparePartitionProcessedMap(
    PartitionLastProcessedFileInfoMap& last_processed_file_per_partition) {
  if (!hasPartitioningMode(partitioning_mode)) return;

  const auto partition_processed_path =
      std::filesystem::path(processed_node_path) / getPartitionKey(node_metadata.file_path, partitioning_mode, parser);

  if (auto it = last_processed_file_per_partition.find(partition_processed_path); it != last_processed_file_per_partition.end()) {
    auto& last_processed_path = it->second.file_path;
    if (last_processed_path < node_metadata.file_path) last_processed_path = node_metadata.file_path;
  } else {
    auto zk_retry = ObjectStorageQueueMetadata::getKeeperRetriesControl(log);
    zk_retry.retryLoop([&] {
      bool processed_path_exists = ObjectStorageQueueMetadata::getZooKeeper(log, zookeeper_name)->exists(partition_processed_path);
      last_processed_file_per_partition.emplace(partition_processed_path,
                                                PartitionLastProcessedFileInfo({processed_path_exists,
...
void ObjectStorageQueueOrderedFileMetadata::migrateToBuckets(const std::string& zk_path, size_t value, size_t prev_value,
  ...
  for (size_t try_num = 0; try_num <= retries; ++try_num) {
    ...
    const auto old_processed_path = prev_value == 0 ? getProcessedPathWithoutBucket(zk_path) : getProcessedPathWithBucket(zk_path, 0);

    NodeMetadata processed_node;
    Coordination::Stat processed_node_stat;
    bool has_processed_node = ObjectStorageQueueOrderedFileMetadata::getMaxProcessedNode(processed_node, &processed_node_stat,
                                                                                         old_processed_path, log, zookeeper_name_);
    ...
    for (size_t i = 0; i < value; ++i) {
      ...
      requests.push_back(zkutil::makeCreateRequest(new_processed_path, processed_node.toString(), zkutil::CreateMode::Persistent));
    }

    responses.clear();
    code = zk_client->tryMulti(requests, responses);
    if (code == Coordination::Error::ZOK) {
      LOG_TRACE(log, "Successfully migrated metadata from {} to {} buckets", old_processed_path, value);
      return;
    }
    ...
    if (prev_value != 0
        ...
        && failed_idx == 2 && code == Coordination::Error::ZNODEEXISTS) {
      LOG_TRACE(log,
    ...
    throw Exception(ErrorCodes::LOGICAL_ERROR, "Unexpected error: {} (failed request id: {}, path: {})", code, failed_idx,
                    requests[failed_idx]->getPath());
  }

  throw zkutil::KeeperMultiException(code, requests, responses);
}

void ObjectStorageQueueOrderedFileMetadata::filterOutProcessedAndFailed(std::vector<std::string>& paths,
                                                                        const std::filesystem::path& zk_path_, size_t buckets_num,
                                                                        const std::string& zookeeper_name_,
                                                                        ObjectStorageQueueBucketingMode bucketing_mode,
                                                                        ObjectStorageQueuePartitioningMode partitioning_mode,
                                                                        const ObjectStorageQueueFilenameParser* parser, LoggerPtr log_) {
  ...
  for (size_t i = 0; i < buckets_num; ++i) {
    auto processed_node_path =
        use_buckets_for_processing ? getProcessedPathWithBucket(zk_path_, i) : getProcessedPathWithoutBucket(zk_path_);
    ...
    if (hasPartitioningMode(partitioning_mode)) {
      ...
      auto state = getProcessingStateFromKeeper(
          /* processed_node_stat */ {}, processed_node_path,
          /* file_path */ "", std::nullopt, std::nullopt, log_, zookeeper_name_);
      if (state.last_processed_path.has_value()) last_processed_file_map[i][""] = *state.last_processed_path;
    }
  }
  ...
  for (size_t i = 0; i < paths.size(); ++i) {
    ...
    if (!last_processed_file_map.empty()) {
      if (hasPartitioningMode(partitioning_mode)) {
        ...
        if (max_processed_file != last_processed_file_map[bucket].end() && path <= max_processed_file->second) {
