...
ObjectStorageQueueMode modeFromString(const std::string& mode) {
  if (mode == "ordered") return ObjectStorageQueueMode::ORDERED;
  if (mode == "unordered") return ObjectStorageQueueMode::UNORDERED;
  throw Exception(ErrorCodes::BAD_ARGUMENTS, "Unexpected ObjectStorageQueue mode: {}", mode);
}

void validateMode(const std::string& mode) {
  if (mode != "ordered" && mode != "unordered") throw Exception(ErrorCodes::BAD_ARGUMENTS, "Unexpected ObjectStorageQueue mode: {}", mode);
}
}  // namespace

ObjectStorageQueueTableMetadata::ObjectStorageQueueTableMetadata(const ObjectStorageQueueSettings& engine_settings,
                                                                 const ColumnsDescription& columns_, const std::string& format_)
    : format_name(format_),
      ...
      partitioning_mode(engine_settings[ObjectStorageQueueSetting::use_hive_partitioning]
                            ? "hive"
  ...
  if (partitioning_mode == "regex") {
    if (partition_regex.empty())
      throw Exception(ErrorCodes::BAD_ARGUMENTS, "partition_regex must be specified when using partitioning_mode='regex'");
    if (partition_component.empty())
      throw Exception(ErrorCodes::BAD_ARGUMENTS, "partition_component must be specified when using partitioning_mode='regex'");
  }

  if (bucketing_mode == "partition" && partitioning_mode == "none") {
    throw Exception(ErrorCodes::BAD_ARGUMENTS, "bucketing_mode='partition' requires partitioning_mode to be set to 'hive' or 'regex'");
  }
}
...
ObjectStorageQueueAction ObjectStorageQueueTableMetadata::actionFromString(const std::string& action) {
  if (action == "keep") return ObjectStorageQueueAction::KEEP;
  if (action == "delete") return ObjectStorageQueueAction::DELETE;
  if (action == "move") return ObjectStorageQueueAction::MOVE;
  if (action == "tag") return ObjectStorageQueueAction::TAG;
...
ObjectStorageQueueMode ObjectStorageQueueTableMetadata::getMode() const { return modeFromString(mode); }

ObjectStorageQueueBucketingMode ObjectStorageQueueTableMetadata::getBucketingMode() const {
  if (bucketing_mode == "path") return ObjectStorageQueueBucketingMode::PATH;
  if (bucketing_mode == "partition") return ObjectStorageQueueBucketingMode::PARTITION;
...
ObjectStorageQueuePartitioningMode ObjectStorageQueueTableMetadata::getPartitioningMode() const {
  if (partitioning_mode == "none") return ObjectStorageQueuePartitioningMode::NONE;
  if (partitioning_mode == "hive") return ObjectStorageQueuePartitioningMode::HIVE;
  if (partitioning_mode == "regex") return ObjectStorageQueuePartitioningMode::REGEX;
  throw Exception(ErrorCodes::BAD_ARGUMENTS, "Unexpected ObjectStorageQueue partitioning mode: {}", partitioning_mode);
}

template <typename T>
static auto getOrDefault(const Poco::JSON::Object::Ptr& json, const std::string& setting, const std::string& compatibility_prefix,
                         const T& default_value) {
  if (!compatibility_prefix.empty() && json->has(compatibility_prefix + setting)) return json->getValue<T>(compatibility_prefix + setting);

  if (json->has(setting)) return json->getValue<T>(setting);
...
void ObjectStorageQueueTableMetadata::adjustFromKeeper(const ObjectStorageQueueTableMetadata& from_zk) {
  if (processing_threads_num != from_zk.processing_threads_num) {
    ...
    const std::string message = fmt::format("Using `processing_threads_num` from keeper: {} (local: {})",
...
void ObjectStorageQueueTableMetadata::checkImmutableFieldsEquals(const ObjectStorageQueueTableMetadata& from_zk) const {
  if (after_processing != from_zk.after_processing)
    throw Exception(ErrorCodes::METADATA_MISMATCH,
                    ...
                    DB::toString(from_zk.after_processing.load()), DB::toString(after_processing.load()));
