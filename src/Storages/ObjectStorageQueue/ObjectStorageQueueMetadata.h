...
struct ObjectStorageQueueMetadataCacheWeightFunction {
  size_t operator()(const ObjectStorageQueueIFileMetadata::FileStatus& cell) const {
    return sizeof(cell) + sizeof(UInt128)  /// Cache key
           + cell.path.capacity() + cell.getException().capacity();
  }
};
...
class ObjectStorageQueueMetadata {
 public:
  ...
  using FileStatusesCache =
      CacheBase<UInt128, ObjectStorageQueueIFileMetadata::FileStatus, UInt128TrivialHash, ObjectStorageQueueMetadataCacheWeightFunction>;
  using Bucket = size_t;
  using Processor = std::string;

  ObjectStorageQueueMetadata(ObjectStorageType storage_type_, const std::string& zookeeper_name_, const fs::path& zookeeper_path_,
                             const ObjectStorageQueueTableMetadata& table_metadata_, size_t cleanup_interval_min_ms_,
                             size_t cleanup_interval_max_ms_, bool use_persistent_processing_nodes_,
  ...
  static ObjectStorageQueueTableMetadata syncWithKeeper(const String& zookeeper_name, const fs::path& zookeeper_path,
                                                        const ObjectStorageQueueSettings& settings, const ColumnsDescription& columns,
                                                        const std::string& format, const ContextPtr& context, bool is_attach,
                                                        LoggerPtr log);
  ...
  static Bucket getBucketForPath(const std::string& path, size_t buckets_num, ObjectStorageQueueBucketingMode bucketing_mode,
  ...
  void setMetadataRefCount(std::atomic<size_t>& ref_count_) {
    chassert(!metadata_ref_count);
    metadata_ref_count = &ref_count_;
  }
