...
namespace {
void validateSettings(ObjectStorageQueueSettings &queue_settings, bool is_attach) {
  ...
  if (queue_settings[ObjectStorageQueueSetting::cleanup_interval_min_ms] >
      queue_settings[ObjectStorageQueueSetting::cleanup_interval_max_ms]) {
  ...
  if (queue_settings[ObjectStorageQueueSetting::after_processing] == ObjectStorageQueueAction::MOVE) {
    ...
    if (move_uri || move_access_key_id || move_secret_access_key) {
      ...
      if (move_connection_string || move_container) {
        throw Exception(ErrorCodes::BAD_ARGUMENTS, "Unexpected Azure settings when S3 settings are provided:{}{}",
                        move_connection_string ? " move_connection_string" : "", move_container ? " move_container" : "");
      }
      ...
      throw Exception(ErrorCodes::BAD_ARGUMENTS, "Move of processed objects requires specifying target bucket/container or prefix");
    }
  }
...
std::shared_ptr<ObjectStorageQueueLog> getQueueLog(const ObjectStoragePtr &storage, const ContextPtr &context,
                                                   bool enable_logging_to_queue_log) {
  const auto &settings = context->getSettingsRef();
  switch (storage->getType()) {
    case DB::ObjectStorageType::S3: {
      if (enable_logging_to_queue_log || settings[Setting::s3queue_enable_logging_to_s3queue_log]) return context->getS3QueueLog();
      return nullptr;
    }
    case DB::ObjectStorageType::Azure: {
      if (enable_logging_to_queue_log) return context->getAzureQueueLog();
      return nullptr;
    }
    default:
...
StorageObjectStorageQueue::StorageObjectStorageQueue(std::unique_ptr<ObjectStorageQueueSettings> queue_settings_,
                                                     const StorageObjectStorageConfigurationPtr configuration_, const StorageID &table_id_,
                                                     const ColumnsDescription &columns_, const ConstraintsDescription &constraints_,
                                                     const String &comment, ContextPtr context_,
                                                     std::optional<FormatSettings> format_settings_, ASTStorage *engine_args,
                                                     LoadingStrictnessLevel mode, bool keep_data_in_keeper_)
    : IStorage(table_id_),
      ...
      min_insert_block_size_rows_for_materialized_views(
          (*queue_settings_)[ObjectStorageQueueSetting::min_insert_block_size_rows_for_materialized_views]),
      min_insert_block_size_bytes_for_materialized_views(
          (*queue_settings_)[ObjectStorageQueueSetting::min_insert_block_size_bytes_for_materialized_views]),
  ...
  if (use_hive_partitioning) {
    hive_partition_columns_to_read_from_file_path =
        HivePartitioningUtils::extractHivePartitionColumnsFromPath(columns, configuration->getRawPath().path, format_settings, context_);
  ...
  if (engine_args->settings) storage_metadata.settings_changes = engine_args->settings->ptr();
  setVirtuals(VirtualColumnUtils::getVirtualsForFileLikeStorage(storage_metadata.columns, context_));
  setInMemoryMetadata(storage_metadata);

  zk_path = chooseZooKeeperPath(getContext(), table_id_, context_->getSettingsRef(), *queue_settings_, UUIDHelpers::Nil, &zookeeper_name);
  LOG_INFO(log, "Using zookeeper path: {}", zk_path.string());

  auto table_metadata = ObjectStorageQueueMetadata::syncWithKeeper(zookeeper_name, zk_path, *queue_settings_, storage_metadata.getColumns(),
                                                                   configuration_->format, context_, is_attach, log);
  ...
  temp_metadata = std::make_unique<ObjectStorageQueueMetadata>(
      storage_type, zookeeper_name, zk_path, std::move(table_metadata),
      ...
      /* use_persistent_processing_nodes */ true, (*queue_settings_)[ObjectStorageQueueSetting::persistent_processing_node_ttl_seconds],
  ...
  size_t task_count = (*queue_settings_)[ObjectStorageQueueSetting::parallel_inserts]
                          ? (*queue_settings_)[ObjectStorageQueueSetting::processing_threads_num]
                          : 1;
...
void StorageObjectStorageQueue::startup() {
  ...
  files_metadata = ObjectStorageQueueMetadataFactory::instance().getOrCreate(zookeeper_name, zk_path, std::move(temp_metadata),
                                                                             getStorageID(), created_new_metadata);
  ...
  SCOPE_EXIT_SAFE({
    if (!startup_finished) {
      ...
      ObjectStorageQueueMetadataFactory::instance().remove(zookeeper_name, zk_path, getStorageID(),
  ...
  SCOPE_EXIT_SAFE({
    if (!startup_finished) StreamingStorageRegistry::instance().unregisterTable(getStorageID(), /* if_exists */ true);
  });

  fiu_do_on(FailPoints::object_storage_queue_fail_startup, { throw Exception(ErrorCodes::FAULT_INJECTED, "Failed to startup"); });

  /// Start background tasks.
  files_metadata->startup();
  for (auto &task : streaming_tasks) task->activateAndSchedule();
...
void StorageObjectStorageQueue::shutdown(bool is_drop) {
...

  {
    ...
    for (auto &task : streaming_tasks) task->deactivate();

    LOG_DEBUG(log, "Finished {} streaming tasks (took: {} ms)", streaming_tasks.size(), watch.elapsedMilliseconds());
  }
...
class ReadFromObjectStorageQueue : public SourceStepWithFilter {
 public:
  ...
  ReadFromObjectStorageQueue(const Names &column_names_, const SelectQueryInfo &query_info_, const StorageSnapshotPtr &storage_snapshot_,
                             const ContextPtr &context_, SharedHeader sample_block, ReadFromFormatInfo info_,
                             std::shared_ptr<StorageObjectStorageQueue> storage_, size_t max_block_size_, bool commit_once_processed_)
...
void StorageObjectStorageQueue::read(QueryPlan &query_plan, const Names &column_names, const StorageSnapshotPtr &storage_snapshot,
                                     SelectQueryInfo &query_info, ContextPtr local_context, QueryProcessingStage::Enum /*processed_stage*/,
                                     size_t max_block_size, size_t) {
  ...
  if (do_commit_on_select && getDependencies() > 0) {
    throw Exception(ErrorCodes::QUERY_NOT_ALLOWED, "Cannot read from {} with attached materialized views and commit_on_select=1",
                    getName());
  }
  ...
  auto read_from_format_info =
      prepareReadingFromFormat(column_names, storage_snapshot, local_context, supportsSubsetOfColumns(local_context));

  auto reading = std::make_unique<ReadFromObjectStorageQueue>(
      column_names, query_info, storage_snapshot, local_context, std::make_shared<const Block>(read_from_format_info.source_header),
      read_from_format_info, std::move(this_ptr), max_block_size, do_commit_on_select);
...
void ReadFromObjectStorageQueue::initializePipeline(QueryPipelineBuilder &pipeline, const BuildQueryPipelineSettings &) {
  ...
  for (size_t i = 0; i < processing_threads_num; ++i)
    pipes.emplace_back(storage->createSource(i /* processor_id */, info, parser_shared_resources, progress, iterator, max_block_size,
                                             context, commit_once_processed));
...
std::shared_ptr<ObjectStorageQueueSource> StorageObjectStorageQueue::createSource(
    size_t processor_id, const ReadFromFormatInfo &info, FormatParserSharedResourcesPtr parser_shared_resources,
    ProcessingProgressPtr progress_, std::shared_ptr<StorageObjectStorageQueue::FileIterator> file_iterator, size_t max_block_size,
    ContextPtr local_context, bool commit_once_processed, size_t max_processed_files_override) {
  ...
  if (max_processed_files_override) commit_settings_copy.max_processed_files_before_commit = max_processed_files_override;
  return std::make_shared<ObjectStorageQueueSource>(
      getName(), processor_id, file_iterator, configuration, object_storage, progress_, info, format_settings, parser_shared_resources,
      commit_settings_copy, after_processing_settings_copy, files_metadata, local_context, max_block_size, shutdown_called,
      table_is_being_dropped, getQueueLog(object_storage, local_context, enable_logging_to_queue_log), getStorageID(), log,
      commit_once_processed, add_deduplication_info);
}
...
size_t StorageObjectStorageQueue::getDependencies() const {
  ...
  for (const auto &view_id : view_ids) {
    ...
    if (materialized_view && !materialized_view->tryGetTargetTable()) return 0;
  }
    ...
    try {
      ...
      if (dependencies_count) {
        ...
        if (streamToViews(streaming_tasks_index)) {
          ...
          std::lock_guard lock(mutex);
          reschedule_processing_interval_ms =
              std::min<size_t>(polling_max_timeout_ms, reschedule_processing_interval_ms + polling_backoff_ms);
        }
...
bool StorageObjectStorageQueue::streamToViews(size_t streaming_tasks_index) {
  ...
  if (!table) throw Exception(ErrorCodes::LOGICAL_ERROR, "Engine table {} doesn't exist.", table_id.getNameForLogs());
  ...
  if (is_deduplication_v2) queue_context->setSetting("async_insert_deduplicate", 1);
  ...
  LOG_TEST(log, "Using {} processing threads (processing_threads_num: {}, parallel_inserts: {}, async deduplicate: {})", threads,
           processing_threads_num, parallel_inserts, is_deduplication_v2);
  ...
  while (!shutdown_called && !file_iterator->isFinished()) {
    ...
    InterpreterInsertQuery interpreter(insert, queue_context,
    ...
    auto read_from_format_info = prepareReadingFromFormat(
        block_io.pipeline.getHeader().getNames(), storage_snapshot, queue_context, supportsSubsetOfColumns(queue_context),
    ...
    auto parser_shared_resources = std::make_shared<FormatParserSharedResources>(queue_context->getSettingsRef(), /*num_streams_=*/threads);

    auto processing_progress = std::make_shared<ProcessingProgress>();
    for (size_t i = 0; i < threads; ++i) {
      size_t processor_id = i * (streaming_tasks_index + 1);
      auto source = createSource(processor_id, read_from_format_info, parser_shared_resources, processing_progress, file_iterator,
                                 DBMS_DEFAULT_BUFFER_SIZE, queue_context,
                                 /*commit_once_processed=*/false, effective_max_files);
    ...
    try {
      ...
      try {
        commit(
            /*insert_succeeded=*/false, rows, sources, transaction_start_time, getCurrentExceptionMessage(true), getCurrentExceptionCode());
        ...
        if (!current_max) current_max = processing_progress->processed_files.load();
    ...
    fiu_do_on(FailPoints::object_storage_queue_fail_after_insert, { throw Exception(ErrorCodes::FAULT_INJECTED, "Failed after insert"); });
...
void StorageObjectStorageQueue::postProcess(const StoredObjects &successful_objects) const {
  ...
  {
    std::lock_guard lock(mutex);
    post_processor.emplace(getContext(), type, object_storage, getName(), files_metadata->getTableMetadata(), after_processing_settings);
  }
...
void StorageObjectStorageQueue::commit(bool insert_succeeded, size_t inserted_rows,
                                       std::vector<std::shared_ptr<ObjectStorageQueueSource>> &sources, time_t transaction_start_time,
                                       const std::string &exception_message, int error_code) const {
  ...
  for (auto &source : sources) {
    source->prepareCommitRequests(requests, insert_succeeded, successful_objects, last_processed_file_per_partition, created_nodes,
  ...
  if (!successful_objects.empty() && files_metadata->getTableMetadata().after_processing != ObjectStorageQueueAction::KEEP) {
  ...
  zk_retry.retryLoop([&] {
    if (zk_retry.isRetry()) {
      LOG_TRACE(log, "Failed to commit processed files at try {}/{}, will retry", try_num,
                toString(settings[Setting::keeper_max_retries].value));
    }
    ++try_num;
  ...
  if (!code.has_value()) {
    throw Exception(ErrorCodes::KEEPER_EXCEPTION, "Failed to commit files with {} retries, last error message: {}",
  ...
  for (auto &source : sources) {
    try {
      source->finalizeCommit(insert_succeeded, commit_id, commit_time, transaction_start_time, exception_message);
    } catch (...) {
      tryLogCurrentException(log);
      if (!finalize_exception) finalize_exception = std::current_exception();
    }
  }
  if (finalize_exception) std::rethrow_exception(finalize_exception);

  LOG_DEBUG(log,
            "Successfully committed {} files with {} requests for {} sources with commit id {} "
            "(inserted rows: {}, files: {})",
            successful_objects.size(), requests.size(), sources.size(), commit_id, inserted_rows, collectRemotePaths(successful_objects));
}
...
static std::string normalizeSetting(const std::string &name) {
  ...
  if (name.starts_with("s3queue_")) return name.substr(std::strlen("s3queue_"));
  return name;
}

void checkNormalizedSetting(const std::string &name) {
  if (name.starts_with("s3queue_")) throw Exception(ErrorCodes::LOGICAL_ERROR, "Setting is not normalized: {}", name);
}
...
static AlterCommands normalizeAlterCommands(const AlterCommands &alter_commands) {
  ...
  for (auto &command : normalized_alter_commands) {
    for (auto &setting : command.settings_changes) setting.name = normalizeSetting(setting.name);

    std::set<std::string> settings_resets;
    for (const auto &setting : command.settings_resets) settings_resets.insert(normalizeSetting(setting));
...
void StorageObjectStorageQueue::checkAlterIsPossible(const AlterCommands &commands, ContextPtr local_context) const {
  ...
  if (old_metadata.settings_changes) {
    ...
    for (auto &setting : *old_settings) setting.name = normalizeSetting(setting.name);
  }
  ...
  if (!new_metadata.hasSettingsChanges()) throw Exception(ErrorCodes::LOGICAL_ERROR, "No settings changes");
  ...
  for (const auto &setting : new_settings) {
    ...
    if (old_settings) {
      auto it = std::find_if(old_settings->begin(), old_settings->end(),
    ...
    if (setting_changed) {
      ...
      if (!isSettingChangeable(setting.name, mode)) {
        throw Exception(ErrorCodes::SUPPORT_IS_DISABLED, "Changing setting {} is not allowed for {} mode of {}", setting.name,
                        magic_enum::enum_name(mode), getName());
      }
...
void StorageObjectStorageQueue::alter(const AlterCommands &commands, ContextPtr local_context, AlterLockHolder &) {
  ...
  if (commands.isSettingsAlter()) {
    ...
    if (old_settings) {
      auto get_names = [](const SettingsChanges &settings) {
        ...
        for (const auto &[name, _] : settings) {
          ...
          if (!inserted) throw Exception(ErrorCodes::BAD_ARGUMENTS, "Setting {} is duplicated", name);
        }
        return names;
      };
      ...
      std::set_difference(old_settings_set.begin(), old_settings_set.end(), new_settings_set.begin(), new_settings_set.end(),
                          std::inserter(reset_settings, reset_settings.begin()));

      if (!reset_settings.empty()) {
        LOG_TRACE(log, "Will reset settings: {} (old settings: {}, new_settings: {})", fmt::join(reset_settings, ", "),
                  fmt::join(old_settings_set, ", "), fmt::join(new_settings_set, ", "));

        ObjectStorageQueueSettings default_settings;
        for (const auto &name : reset_settings) new_settings.push_back(SettingChange(name, default_settings.get(name)));
      }
    }
    ...
    for (auto &setting : new_settings) {
      ...
      if (!inserted) throw Exception(ErrorCodes::BAD_ARGUMENTS, "Setting {} is duplicated", setting.name);
    ...
    LOG_TRACE(log, "New settings changes: {} (requires_detached_mv: {}, changed settings ({}):  {})",
              new_metadata.settings_changes->formatForLogging(), requires_detached_mv, changed_settings.size(),
              changed_settings.namesToString());
    ...
    if (requires_detached_mv) streaming_file_iterator.reset();
...
const ObjectStorageQueueTableMetadata &StorageObjectStorageQueue::getTableMetadata() const {
  if (!files_metadata) throw Exception(ErrorCodes::LOGICAL_ERROR, "Files metadata is empty");
  return files_metadata->getTableMetadata();
}

std::shared_ptr<StorageObjectStorageQueue::FileIterator> StorageObjectStorageQueue::createFileIterator(ContextPtr local_context,
  ...
  return std::make_shared<FileIterator>(files_metadata, object_storage, configuration, getStorageID(), list_objects_batch_size_copy,
                                        predicate, getVirtualsList(), hive_partition_columns_to_read_from_file_path, local_context, log,
                                        enable_hash_ring_filtering_copy, file_deletion_enabled, shutdown_called);
}
...
ObjectStorageQueueSettings StorageObjectStorageQueue::getSettings() const {
  ...
  if (!startup_finished) return settings;
  ...
  settings[ObjectStorageQueueSetting::persistent_processing_node_ttl_seconds] =
      static_cast<UInt32>(files_metadata->getPersistentProcessingNodeTTLSeconds());
...

  {
    ...
    settings[ObjectStorageQueueSetting::after_processing_move_access_key_id] =
        after_processing_settings.after_processing_move_access_key_id;
    settings[ObjectStorageQueueSetting::after_processing_move_secret_access_key] =
        after_processing_settings.after_processing_move_secret_access_key;
    settings[ObjectStorageQueueSetting::after_processing_move_connection_string] =
        after_processing_settings.after_processing_move_connection_string;
    ...
    settings[ObjectStorageQueueSetting::min_insert_block_size_rows_for_materialized_views] =
        min_insert_block_size_rows_for_materialized_views;
    settings[ObjectStorageQueueSetting::min_insert_block_size_bytes_for_materialized_views] =
        min_insert_block_size_bytes_for_materialized_views;
...
void StorageObjectStorageQueue::checkTableCanBeRenamed(const StorageID &new_name) const {
  ...
  if (move_between_databases && !can_be_moved_between_databases) {
    throw Exception(ErrorCodes::NOT_IMPLEMENTED,
                    ...
                    "Please set an explicit `keeper_path` to allow moving the table",
                    configuration->getEngineName());
  }
}

String StorageObjectStorageQueue::chooseZooKeeperPath(const ContextPtr &context_, const StorageID &table_id, const Settings &settings,
                                                      const ObjectStorageQueueSettings &queue_settings, UUID database_uuid,
  ...
  if (zk_path_prefix.empty()) zk_path_prefix = "/";
  ...
  if (queue_settings[ObjectStorageQueueSetting::keeper_path].changed) {
    ...
    if (database_uuid == UUIDHelpers::Nil) database_uuid = DatabaseCatalog::instance().getDatabase(table_id.database_name)->getUUID();
  ...
  if (result_zookeeper_name) *result_zookeeper_name = zkutil::extractZooKeeperName(result_zk_path);
