...
ObjectStorageQueueUnorderedFileMetadata::ObjectStorageQueueUnorderedFileMetadata(
    const std::filesystem::path& zk_path, const std::string& path_, FileStatusPtr file_status_, size_t max_loading_retries_,
    std::atomic<size_t>& metadata_ref_count_, bool use_persistent_processing_nodes_, const std::string& zookeeper_name_, LoggerPtr log_)
    : ObjectStorageQueueIFileMetadata(path_, zookeeper_name_,
                                      /* processing_node_path */ zk_path / "processing" / getNodeName(path_),
                                      /* processed_node_path */ zk_path / "processed" / getNodeName(path_),
                                      /* failed_node_path */ zk_path / "failed" / getNodeName(path_), file_status_, max_loading_retries_,
                                      metadata_ref_count_, use_persistent_processing_nodes_, log_) {
  LOG_TEST(log,
...
ObjectStorageQueueUnorderedFileMetadata::prepareProcessingRequestsImpl(Coordination::Requests& requests, const std::string& processing_id) {
  ...
  requests.push_back(zkutil::makeCreateRequest(processing_node_path, processor_info, zkutil::CreateMode::Persistent));
...
std::pair<bool, ObjectStorageQueueIFileMetadata::FileStatus::State> ObjectStorageQueueUnorderedFileMetadata::setProcessingImpl() {
  ...
  if (has_request_failed(result.processed_path_doesnt_exist_idx)) return {false, FileStatus::State::Processed};

  if (has_request_failed(result.failed_path_doesnt_exist_idx)) return {false, FileStatus::State::Failed};

  if (has_request_failed(result.create_processing_node_idx)) return {false, FileStatus::State::Processing};

  throw Exception(ErrorCodes::LOGICAL_ERROR, "Failed to set file processing, last error: {}", code);
}
...
void ObjectStorageQueueUnorderedFileMetadata::prepareProcessedRequestsImpl(Coordination::Requests& requests,
  ...
  requests.push_back(zkutil::makeCreateRequest(processed_node_path, node_metadata.toString(), zkutil::CreateMode::Persistent));
}

void ObjectStorageQueueUnorderedFileMetadata::filterOutProcessedAndFailed(std::vector<std::string>& paths,
  ...
  for (size_t i = 0; i < responses.size();) {
    ...
    if (responses[i].error == Coordination::Error::ZNONODE && responses[i + 1].error == Coordination::Error::ZNONODE) {
      result.push_back(std::move(paths[i / 2]));
    } else {
      LOG_TEST(log_, "Skipping file {}: {}", paths[i / 2], responses[i].error == Coordination::Error::ZOK ? "Processed" : "Failed");
    }
    i += 2;
  }
