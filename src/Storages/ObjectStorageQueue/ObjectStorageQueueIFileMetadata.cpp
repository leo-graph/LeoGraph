...
void ObjectStorageQueueIFileMetadata::FileStatus::setProcessingEndTime() { processing_end_time = now(); }

void ObjectStorageQueueIFileMetadata::FileStatus::setGetObjectTime(size_t elapsed_ms) { get_object_time_ms = elapsed_ms; }
...
void ObjectStorageQueueIFileMetadata::FileStatus::onFailed(const std::string& exception) {
  ...
  if (!processing_end_time) setProcessingEndTime();
  std::lock_guard lock(last_exception_mutex);
...
void ObjectStorageQueueIFileMetadata::FileStatus::updateState(State state_) { state = state_; }
...
ObjectStorageQueueIFileMetadata::ObjectStorageQueueIFileMetadata(const std::string& path_, const std::string& zookeeper_name_,
                                                                 const std::string& processing_node_path_,
                                                                 const std::string& processed_node_path_,
                                                                 const std::string& failed_node_path_, FileStatusPtr file_status_,
                                                                 size_t max_loading_retries_, std::atomic<size_t>& metadata_ref_count_,
                                                                 bool use_persistent_processing_nodes_, LoggerPtr log_)
    : path(path_),
...
ObjectStorageQueueIFileMetadata::~ObjectStorageQueueIFileMetadata() {
  ...
  if (created_processing_node) {
    ...
    LOG_TEST(log,
             "Removing processing node in destructor for file: {} "
             "(state: {}, exception: {})",
    ...
    try {
      ...
      if (Coordination::isHardwareError(code)) {
        LOG_WARNING(log,
                    "Keeper session expired and retries did not help. "
                    "Will rely on automatic processing node cleanup");
        return;
      }

      LOG_WARNING(log, "Unexpected error while removing processing node: {} (path: {})", code, processing_node_path);
...
ObjectStorageQueueIFileMetadata::NodeMetadata ObjectStorageQueueIFileMetadata::createNodeMetadata(const std::string& path,
...
std::string ObjectStorageQueueIFileMetadata::generateProcessingID() { return getRandomASCIIString(10); }

bool ObjectStorageQueueIFileMetadata::checkProcessingOwnership(std::shared_ptr<ZooKeeperWithFaultInjection> zk_client) {
  if (processor_info.empty()) throw Exception(ErrorCodes::LOGICAL_ERROR, "Processor info is not set");

  std::string data;
  /// No retries, because they must be done on a higher level.
  if (!zk_client->tryGet(processing_node_path, data)) return false;

  LOG_TEST(log, "Processing node {} has processor: {}, current processor: {}", processing_node_path, data, processor_info);
...
bool ObjectStorageQueueIFileMetadata::trySetProcessing() {
  ...
  if (state == FileStatus::State::Processing || state == FileStatus::State::Processed ||
    ...
    LOG_TEST(log, "File {} has non-processable state `{}` (retries: {}/{})", path, state, file_status->retries.load(), max_loading_retries);
    return false;
  }

  /// An optimization for local parallel processing.
  std::unique_lock processing_lock(file_status->processing_lock, std::defer_lock);
  if (!processing_lock.try_lock()) return {};
...
std::optional<ObjectStorageQueueIFileMetadata::SetProcessingResponseIndexes> ObjectStorageQueueIFileMetadata::prepareSetProcessingRequests(
  ...
  if (!processing_lock.try_lock()) {
    ...
    LOG_TEST(log,
             "File {} is being processed by another table on this server or"
             " another process insert thread in case parallel_insert = 1",
             path);
    return std::nullopt;
  }
...
void ObjectStorageQueueIFileMetadata::resetProcessing() {
  chassert(created_processing_node);
  SCOPE_EXIT({ created_processing_node = false; });

  auto state = file_status->state.load();
  if (state != FileStatus::State::Processing) throw Exception(ErrorCodes::LOGICAL_ERROR, "Cannot reset non-processing state: {}", state);

  SCOPE_EXIT({ (*file_status).reset(); });
  ...
  const auto failed_path = responses[0]->error != Coordination::Error::ZOK ? requests[0]->getPath() : requests[1]->getPath();

  throw Exception(ErrorCodes::LOGICAL_ERROR, "Failed to reset processing for file {}: (code: {}, path: {})", path, code, failed_path);
}
...
void ObjectStorageQueueIFileMetadata::prepareProcessedRequests(Coordination::Requests& requests,
  ...
  try {
    ...
    auto full_exception = fmt::format("Exception while setting file as failed: {}", getCurrentExceptionMessage(true));
...
void ObjectStorageQueueIFileMetadata::prepareFailedRequests(Coordination::Requests& requests, const std::string& exception_message,
  ...
  try {
    ...
    auto full_exception =
        fmt::format("First exception: {}, exception while setting file as failed: {}", exception_message, getCurrentExceptionMessage(true));
...
void ObjectStorageQueueIFileMetadata::finalizeProcessed() {
  ...
  ObjectStorageQueueMetadata::getKeeperRetriesControl(log).retryLoop([&] {
    auto zk_client = ObjectStorageQueueMetadata::getZooKeeper(log, zookeeper_name);
    chassert(!zk_client->exists(processing_node_path),
             fmt::format("Expected path {} not to exist while finalizing {}", processing_node_path, path));

    chassert(!zk_client->exists(failed_node_path),
...
void ObjectStorageQueueIFileMetadata::finalizeFailed(const std::string& exception_message) {
  ...
  ObjectStorageQueueMetadata::getKeeperRetriesControl(log).retryLoop([&] {
    ...
    chassert(zk_client->exists(failed_node_path) || zk_client->exists(failed_node_path + ".retriable"),
             fmt::format("Expected path {} to exist while finalizing {}", failed_node_path, path));
  });
#endif
}

void ObjectStorageQueueIFileMetadata::prepareFailedRequestsImpl(Coordination::Requests& requests, bool retriable) {
  if (!retriable) {
  ...
  if (node_metadata.retries >= max_loading_retries) {
    ...
    if (node_metadata.retries == 0) {
      ...
      requests.push_back(zkutil::makeCreateRequest(retrieable_failed_node_path, node_metadata.toString(), zkutil::CreateMode::Persistent));
    } else {
      /// Update retries count.
      requests.push_back(zkutil::makeSetRequest(retrieable_failed_node_path, node_metadata.toString(), retriable_failed_node_stat.version));
    }
  }
}
