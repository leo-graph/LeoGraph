    ...
    if (auto *t = ast->as<ASTFunction>()) {
      if (t->name == "tumble" || t->name == "hop") {
        ...
        if (!data.window_function) {
          if (data.check_duplicate_window) data.serialized_window_function = temp_node->formatWithSecretsOneLine();
          t->name = "windowID";
...
struct ReplaceWindowIdMatcher {
 public:
  ...
  static void visit(ASTPtr &ast, Data &data) {
    if (auto *t = ast->as<ASTFunction>()) {
      if (t->name == "windowID") t->name = data.window_name;
    }
  }
};
      ...
      if (const auto *t = node.arguments->children[0]->as<ASTFunction>(); t && t->name == "now") {
        if (!t->children.empty()) {
          ...
          if (!children.empty()) {
            ...
            if (timezone_ast) now_timezone = timezone_ast->value.safeGet<String>();
          }
        }
...
inline UInt32 now() { return static_cast<UInt32>(Poco::Timestamp().epochMicroseconds() / 1000000); }
...
class ToIdentifierMatcher {
 public:
  ...
  static void visit(ASTPtr &ast, Data &data) {
    if (const auto *t = ast->as<ASTFunction>()) visit(*t, ast, data);
    if (const auto *t = ast->as<ASTIdentifier>()) visit(*t, ast, data);
  }
 ...
 private:
  ...
  static void visit(const ASTIdentifier &node, ASTPtr &node_ptr, Data &data) {
    if (node.getColumnName() == data.window_id_alias) {
      if (auto identifier = boost::dynamic_pointer_cast<ASTIdentifier>(node_ptr)) identifier->setShortName(data.window_id_name);
    }
  }
};
...
void extractWindowArgument(const IAST *ast, IntervalKind::Kind &kind, Int64 &num_units, String err_msg) {
  ...
  if (!arg || !startsWith(arg->name, "toInterval") || !IntervalKind::tryParseString(Poco::toLower(arg->name.substr(10)), kind))
  ...
  if (!interval_unit || (interval_unit->value.getType() != Field::Types::String && interval_unit->value.getType() != Field::Types::UInt64))
  ...
  if (num_units <= 0) throw Exception(ErrorCodes::ARGUMENT_OUT_OF_BOUND, "Value for Interval argument must be positive.");
}
...
String generateInnerTableName(const StorageID &storage_id) {
  if (storage_id.hasUUID()) return ".inner." + toString(storage_id.uuid);
  return ".inner." + storage_id.getTableName();
}

String generateTargetTableName(const StorageID &storage_id) {
  if (storage_id.hasUUID()) return ".inner.target." + toString(storage_id.uuid);
...
static void extractDependentTable(ContextPtr context, ASTPtr &query, String &select_database_name, String &select_table_name) {
  ...
  if (!db_and_table && !subquery) return;
...
UInt32 StorageWindowView::getCleanupBound() {
  if (max_fired_watermark == 0) return 0;
  if (is_proctime) return max_fired_watermark;

  auto w_bound = max_fired_watermark;
  if (allowed_lateness) w_bound = addTime(w_bound, lateness_kind, -lateness_num_units, *time_zone);
  return getWindowLowerBound(w_bound);
}

ASTPtr StorageWindowView::getCleanupQuery() {
  ASTPtr function_less;
  function_less = makeASTOperator("less", make_intrusive<ASTIdentifier>(window_id_name), make_intrusive<ASTLiteral>(getCleanupBound()));
...
bool StorageWindowView::optimize(const ASTPtr &query, const StorageMetadataPtr & /*metadata_snapshot*/, const ASTPtr &partition, bool final,
                                 bool deduplicate, const Names &deduplicate_by_columns, bool cleanup, ContextPtr local_context) {
...
void StorageWindowView::alter(const AlterCommands &params, ContextPtr local_context, AlterLockHolder &) {
  ...
  SCOPE_EXIT({ modifying_query = false; });
  ...
  InterpreterDropQuery::executeDropQuery(ASTDropQuery::Kind::Drop, getContext(), local_context, inner_table_id, true);
  ...
  clean_cache_task =
      getContext()->getSchedulePool().createTask(getStorageID(), getStorageID().getFullTableName(), [this] { threadFuncCleanup(); });
  fire_task = getContext()->getSchedulePool().createTask(getStorageID(), getStorageID().getFullTableName(),
                                                         [this] { is_proctime ? threadFuncFireProc() : threadFuncFireEvent(); });
  ...
  if (has_inner_target_table) {
    ...
    const auto &select_query_output_columns =
        new_metadata.columns;  /// AlterCommands::alter() analyzed the query and assigned `new_metadata.columns` before.
    checkTargetTableHasQueryOutputColumns(target_table_metadata->columns, select_query_output_columns);
    /// We need to copy the target table's columns (after checkTargetTableHasQueryOutputColumns() they can be still different - e.g. in data
    /// types).
...
std::pair<BlocksPtr, Block> StorageWindowView::getNewBlocks(UInt32 watermark) {
  ...
  InterpreterSelectQuery fetch(inner_fetch_query, getContext(), inner_table, inner_table->getInMemoryMetadataPtr(),
  ...
  auto adding_column_actions = std::make_shared<ExpressionActions>(std::move(adding_column_dag), ExpressionActionsSettings(getContext()));
  builder.addSimpleTransform(
      [&](const SharedHeader &header) { return std::make_shared<ExpressionTransform>(header, adding_column_actions); });

  /// Removing window id column
  auto new_header = builder.getHeader();
  new_header.erase(window_id_name);
  auto convert_actions_dag =
      ActionsDAG::makeConvertingActions(builder.getHeader().getColumnsWithTypeAndName(), new_header.getColumnsWithTypeAndName(),
                                        ActionsDAG::MatchColumnsMode::Name, getContext());
  auto actions =
      std::make_shared<ExpressionActions>(std::move(convert_actions_dag), ExpressionActionsSettings(getContext(), CompileExpressions::yes));
  builder.addSimpleTransform(
      [&](const SharedHeader &stream_header) { return std::make_shared<ExpressionTransform>(stream_header, actions); });

  builder.addSimpleTransform([&](const SharedHeader &header) { return std::make_shared<AddingAggregatedChunkInfoTransform>(header); });
  ...
  InterpreterSelectQuery select(getFinalQuery(), getContext(), blocks_storage.getTable(),
                                blocks_storage.getTable()->getInMemoryMetadataPtr(), SelectQueryOptions(QueryProcessingStage::Complete));

  builder = select.buildQueryPipeline();

  builder.addSimpleTransform([&](const SharedHeader &current_header) {
    return std::make_shared<SquashingTransform>(current_header, getContext()->getSettingsRef()[Setting::min_insert_block_size_rows],
...
inline void StorageWindowView::fire(UInt32 watermark) {
  throwIfWindowViewIsDisabled();
  LOG_TRACE(log, "Watch streams number: {}, target table: {}", watch_streams.size(),
            target_table_id.empty() ? "None" : target_table_id.getNameForLogs());

  if (target_table_id.empty() && watch_streams.empty()) return;
  ...
  for (const auto &block : *blocks) {
    for (auto &watch_stream : watch_streams) {
      if (auto watch_stream_ptr = watch_stream.lock()) watch_stream_ptr->addBlock(block, watermark);
    }
  ...
  if (!target_table_id.empty()) {
    ...
    auto adding_missing_defaults_dag = addMissingDefaults(pipe.getHeader(), block_io.pipeline.getHeader().getNamesAndTypesList(),
                                                          getTargetTable()->getInMemoryMetadataPtr()->getColumns(), context,
    ...
    auto convert_actions_dag = ActionsDAG::makeConvertingActions(pipe.getHeader().getColumnsWithTypeAndName(),
                                                                 ...
                                                                 ActionsDAG::MatchColumnsMode::Position, context);
    auto actions =
        std::make_shared<ExpressionActions>(std::move(convert_actions_dag), ExpressionActionsSettings(context, CompileExpressions::yes));
...
ASTPtr StorageWindowView::getInnerTableCreateQuery(const ASTPtr &inner_query, const StorageID &inner_table_id_) {
  ...
  if (inner_table_engine) {
    ...
    if (storage.ttl_table)
      throw Exception(ErrorCodes::QUERY_IS_NOT_SUPPORTED_IN_WINDOW_VIEW, "TTL is not supported for inner table in Window View");

    new_storage->set(new_storage->engine, storage.engine->clone());

    if (endsWith(storage.engine->name, "MergeTree")) {
      if (storage.partition_by) new_storage->set(new_storage->partition_by, visit(storage.partition_by));
      if (storage.primary_key) new_storage->set(new_storage->primary_key, visit(storage.primary_key));
      if (storage.order_by) new_storage->set(new_storage->order_by, visit(storage.order_by));
      if (storage.sample_by) new_storage->set(new_storage->sample_by, visit(storage.sample_by));

      if (storage.settings) new_storage->set(new_storage->settings, storage.settings->clone());
    }
  } else {
...
#define CASE_WINDOW_KIND_ADD_TIME(KIND)                                                                                  \
  case IntervalKind::Kind::KIND: {                                                                                       \
    if (is_tumble) return ToStartOfTransform<IntervalKind::Kind::KIND>::execute(time_sec, window_num_units, *time_zone); \
...
void StorageWindowView::addFireSignal(std::set<UInt32> &signals) {
  ...
  for (const auto &signal : signals) fire_signal.push_back(signal);
  fire_task->schedule();
}
...
void StorageWindowView::threadFuncFireProc() {
  ...
  LOG_TRACE(log, "Start: {}, now: {}, next fire signal: {}, max watermark: {}", timestamp_start, timestamp_now, next_fire_signal,
            max_watermark);

  while (next_fire_signal <= timestamp_now) {
    try {
      if (max_watermark >= timestamp_start) fire(next_fire_signal);
    } catch (...) {
      tryLogCurrentException(__PRETTY_FUNCTION__);
    }
    ...
    if (slide_kind > IntervalKind::Kind::Day) slide_interval *= 86400;
    next_fire_signal += slide_interval;

    LOG_TRACE(log, "Start: {}, now: {}, next fire signal: {}, max watermark: {}, max fired watermark: {}, slide interval: {}",
              timestamp_start, timestamp_now, next_fire_signal, max_watermark, max_fired_watermark, slide_interval);
  }

  if (max_watermark >= timestamp_start) clean_cache_task->schedule();
  ...
  if (!shutdown_called) fire_task->scheduleAfter(next_fire_ms - std::min(next_fire_ms, timestamp_ms));
}
...
void StorageWindowView::read(QueryPlan &query_plan, const Names &column_names, const StorageSnapshotPtr &storage_snapshot,
  ...
  throwIfWindowViewIsDisabled(local_context);
  if (target_table_id.empty()) return;
  ...
  if (query_plan.isInitialized()) {
    ...
    if (!blocksHaveEqualStructure(*wv_header, *target_header)) {
      auto converting_actions =
          ActionsDAG::makeConvertingActions(target_header->getColumnsWithTypeAndName(), wv_header->getColumnsWithTypeAndName(),
                                            ActionsDAG::MatchColumnsMode::Name, local_context);
...
Pipe StorageWindowView::watch(const Names & /*column_names*/, const SelectQueryInfo &query_info, ContextPtr local_context,
                              QueryProcessingStage::Enum &processed_stage, size_t /*max_block_size*/, const size_t /*num_streams*/) {
  throwIfWindowViewIsDisabled(local_context);
  ...
  auto reader = std::make_shared<WindowViewSource>(std::static_pointer_cast<StorageWindowView>(shared_from_this()), query.is_watch_events,
                                                   window_view_timezone, has_limit, limit,
...
StorageWindowView::StorageWindowView(const StorageID &table_id_, ContextPtr context_, const ASTCreateQuery &query,
                                     const ColumnsDescription &columns_, const String &comment, LoadingStrictnessLevel mode)
    : IStorage(table_id_),
  ...
  if (context_->getSettingsRef()[Setting::allow_experimental_analyzer]) disabled_due_to_analyzer = true;

  if (mode <= LoadingStrictnessLevel::CREATE) throwIfWindowViewIsDisabled();

  if (!query.select) throw Exception(ErrorCodes::INCORRECT_QUERY, "SELECT query is not specified for {}", getName());
  ...
  if (query.select->list_of_selects->children.size() != 1)
    throw Exception(ErrorCodes::QUERY_IS_NOT_SUPPORTED_IN_WINDOW_VIEW, "UNION is not supported for {}", getName());
  ...
  if (auto *inner_storage = query.getTargetInnerEngine(ViewTarget::Inner)) inner_table_engine = inner_storage->clone();
  ...
  if (is_proctime) next_fire_signal = getWindowUpperBound(now());
  ...
  if (disabled_due_to_analyzer) return;
...
ASTPtr StorageWindowView::initInnerQuery(ASTSelectQuery query, ContextPtr context_) {
  ...
  if (is_time_column_func_now) window_id_name = func_now_data.window_id_name;
...
ASTPtr StorageWindowView::innerQueryParser(const ASTSelectQuery &query) {
  if (!query.groupBy()) throw Exception(ErrorCodes::INCORRECT_QUERY, "GROUP BY query is required for {}", getName());
  ...
  if (!query_info_data.is_tumble && !query_info_data.is_hop)
    throw Exception(ErrorCodes::INCORRECT_QUERY, "TIME WINDOW FUNCTION is not specified for {}", getName());
  ...
  if (arguments.size() < required_args)
    throw Exception(ErrorCodes::NUMBER_OF_ARGUMENTS_DOESNT_MATCH, "Too few arguments for function {}: expected at least {}, got {}",
                    window_function.name, required_args, arguments.size());
  extractWindowArgument(arguments.at(1).get(), window_kind, window_num_units,
                        "Illegal type of second argument of function " + window_function.name + " should be Interval");

  window_id_alias = window_function.alias;
  if (auto *node = arguments[0]->as<ASTIdentifier>()) timestamp_column_name = node->shortName();
  ...
  if (!is_tumble) {
    ...
    extractWindowArgument(arguments.at(2).get(), window_kind, window_num_units,
  ...
  if (arguments.size() > time_zone_arg_num) {
    ...
    if (!time_zone_ast || time_zone_ast->value.getType() != Field::Types::String)
      throw Exception(ErrorCodes::ILLEGAL_COLUMN, "Illegal column #{} of time zone argument of function, must be constant string",
                      time_zone_arg_num);
...
void StorageWindowView::eventTimeParser(const ASTCreateQuery &query) {
  ...
  if (query.is_watermark_strictly_ascending || query.is_watermark_ascending || query.is_watermark_bounded) {
    ...
    if (query.is_watermark_ascending) {
      ...
      extractWindowArgument(query.watermark_function, watermark_kind, watermark_num_units,
  ...
  if (query.allowed_lateness) {
    ...
    extractWindowArgument(query.lateness_function, lateness_kind, lateness_num_units,
                          "Illegal type ALLOWED_LATENESS function should be Interval");
  } else
    allowed_lateness = false;
}

void StorageWindowView::writeIntoWindowView(StorageWindowView &window_view, Block &&block, Chunk::ChunkInfoCollection &&chunk_infos,
                                            ContextPtr local_context) {
  window_view.throwIfWindowViewIsDisabled(local_context);
  while (window_view.modifying_query) std::this_thread::sleep_for(std::chrono::milliseconds(100));
  ...
  if (window_view.allowed_lateness && t_max_timestamp != 0) {
    ...
    if (window_view.is_watermark_bounded) {
      UInt32 watermark_lower_bound = addTime(t_max_watermark, window_view.slide_kind, -window_view.slide_num_units, *window_view.time_zone);
  ...
  {
    auto filter_function = makeASTOperator("greaterOrEquals", make_intrusive<ASTIdentifier>(window_view.timestamp_column_name),
    ...
    pipe.addSimpleTransform([&](const SharedHeader &header_) {
      return std::make_shared<FilterTransform>(header_, std::make_shared<ExpressionActions>(std::move(filter_expression)),
  ...
  if (window_view.is_proctime) {
    ...
    if (window_view.is_time_column_func_now) {
      ...
      auto adding_column_actions =
          std::make_shared<ExpressionActions>(std::move(adding_column_dag), ExpressionActionsSettings(local_context));

      pipe.addSimpleTransform(
          [&](const SharedHeader &stream_header) { return std::make_shared<ExpressionTransform>(stream_header, adding_column_actions); });
    }
  }
  ...
  InterpreterSelectQuery select_block(window_view.getMergeableQuery(), local_context, blocks_storage.getTable(),
                                      blocks_storage.getTable()->getInMemoryMetadataPtr(), QueryProcessingStage::WithMergeableState);

  builder = select_block.buildQueryPipeline();

  builder.addSimpleTransform([&](const SharedHeader &current_header) {
    return std::make_shared<SquashingTransform>(current_header, local_context->getSettingsRef()[Setting::min_insert_block_size_rows],
  ...
  if (!window_view.is_proctime) {
    ...
    if (window_view.is_watermark_bounded || window_view.allowed_lateness) {
      ...
      for (const auto &timestamp : timestamp_data) block_max_timestamp = std::max(timestamp, block_max_timestamp);
    }

    if (block_max_timestamp) window_view.updateMaxTimestamp(block_max_timestamp);
  }

  UInt32 lateness_upper_bound = 0;
  if (!window_view.is_proctime && window_view.allowed_lateness && t_max_fired_watermark) lateness_upper_bound = t_max_fired_watermark;
  ...
  if (!blocksHaveEqualStructure(builder.getHeader(), output->getHeader())) {
    auto convert_actions_dag =
        ActionsDAG::makeConvertingActions(builder.getHeader().getColumnsWithTypeAndName(), output->getHeader().getColumnsWithTypeAndName(),
                                          ActionsDAG::MatchColumnsMode::Name, local_context);
    auto convert_actions = std::make_shared<ExpressionActions>(std::move(convert_actions_dag),
                                                               ExpressionActionsSettings(local_context, CompileExpressions::yes));

    builder.addSimpleTransform(
        [&](const SharedHeader &header_) { return std::make_shared<ExpressionTransform>(header_, convert_actions); });
  }

  builder.addChain(Chain(std::move(output)));
  builder.setSinks([&](const SharedHeader &cur_header, Pipe::StreamType) { return std::make_shared<EmptySink>(cur_header); });
...
void StorageWindowView::startup() {
  ...
  if (is_proctime) fire_task->schedule();
}
...
void StorageWindowView::dropInnerTableIfAny(bool sync, ContextPtr local_context) {
  if (!std::exchange(has_inner_table, false)) return;

  try {
    InterpreterDropQuery::executeDropQuery(ASTDropQuery::Kind::Drop, getContext(), local_context, inner_table_id, sync);
...
SharedHeader StorageWindowView::getOutputHeader() const {
  ...
  if (output_header.empty()) {
    output_header =
        *InterpreterSelectQuery(select_query->clone(), getContext(), SelectQueryOptions(QueryProcessingStage::Complete)).getSampleBlock();
  }
  return std::make_shared<const Block>(output_header);
}

StoragePtr StorageWindowView::getSourceTable() const { return DatabaseCatalog::instance().getTable(select_table_id, getContext()); }

StoragePtr StorageWindowView::getInnerTable() const { return DatabaseCatalog::instance().getTable(inner_table_id, getContext()); }

StoragePtr StorageWindowView::getTargetTable() const { return DatabaseCatalog::instance().getTable(target_table_id, getContext()); }
...
void StorageWindowView::throwIfWindowViewIsDisabled(ContextPtr local_context) const {
  if (disabled_due_to_analyzer || (local_context && local_context->getSettingsRef()[Setting::allow_experimental_analyzer]))
    throw Exception(ErrorCodes::UNSUPPORTED_METHOD,
                    "Experimental WINDOW VIEW feature is not supported "
