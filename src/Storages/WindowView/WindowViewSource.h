...
class WindowViewSource : public ISource {
 public:
  WindowViewSource(std::shared_ptr<StorageWindowView> storage_, const bool is_events_, String window_view_timezone_, const bool has_limit_,
                   const UInt64 limit_, const UInt64 heartbeat_interval_sec_)
      : ISource(is_events_ ? std::make_shared<const Block>(Block({ColumnWithTypeAndName(
                                 ColumnUInt32::create(), std::make_shared<DataTypeDateTime>(window_view_timezone_), "watermark")}))
    ...
    if (is_events)
      header.insert(ColumnWithTypeAndName(ColumnUInt32::create(), std::make_shared<DataTypeDateTime>(window_view_timezone_), "watermark"));
    else
 ...
 protected:
  ...
  Chunk generate() override {
    ...
    if (block.empty()) return Chunk();
    if (is_events) {
      return Chunk({DataTypeDateTime(window_view_timezone).createColumnConst(block.rows(), watermark)->convertToFullColumnIfConst()},
                   block.rows());
    }
  ...
  std::pair<Block, UInt32> generateImpl() {
    if (has_limit && num_updates == static_cast<Int64>(limit)) return {Block(), 0};

    if (isCancelled() || storage->shutdown_called) return {Block(), 0};
