  ...
  if (auto named_collection = tryGetNamedCollectionWithOverrides(args, context_, true, nullptr, table_id)) {
    ...
    if (!(args.size() == 2 || args.size() == 4))
      throw Exception(ErrorCodes::NUMBER_OF_ARGUMENTS_DOESNT_MATCH,
                      "Storage ArrowFlight requires 2 or 4 parameters: "
                      "ArrowFlight('host:port', 'dataset' [, 'user', 'password']).");

    for (auto& arg : args) arg = evaluateConstantExpressionOrIdentifierAsLiteral(arg, context_);
...
StorageArrowFlight::Configuration StorageArrowFlight::processNamedCollectionResult(const NamedCollection& named_collection) {
  ...
  ValidateKeysMultiset<ExternalDatabaseEqualKeysSet> optional_arguments = {
      "host",     "hostname",   "dataset", "use_basic_authentication", "user", "username",
      "password", "enable_ssl", "ssl_ca",  "ssl_override_hostname"};
  ...
  if (!configuration.use_basic_authentication && is_username_set)
    throw Exception(ErrorCodes::BAD_ARGUMENTS,
                    "The 'username' is specified however the basic authentication is disabled in named collection");
...
StorageArrowFlight::StorageArrowFlight(const StorageID& table_id_, std::shared_ptr<ArrowFlightConnection> connection_,
                                       const String& dataset_name_, const ColumnsDescription& columns_,
                                       const ConstraintsDescription& constraints_, ContextPtr context_)
    : IStorage(table_id_),
...
ColumnsDescription StorageArrowFlight::getTableStructureFromData(std::shared_ptr<ArrowFlightConnection> connection_,
                                                                 const String& dataset_name_, ContextPtr context_) {
...
Pipe StorageArrowFlight::read(const Names& column_names, const StorageSnapshotPtr& storage_snapshot, SelectQueryInfo& /*query_info*/,
                              ContextPtr context_, QueryProcessingStage::Enum, size_t /*max_block_size*/, size_t /*num_streams*/) {
...
class ArrowFlightSink : public SinkToStorage {
 public:
  explicit ArrowFlightSink(const StorageMetadataPtr& metadata_snapshot_, std::shared_ptr<ArrowFlightConnection> connection_,
                           const String& dataset_name_, ContextPtr context_)
  ...
  void consume(Chunk& chunk) override {
    ...
    while (true) {
      ...
      if (!write_result.ok()) {
        throw Exception(ErrorCodes::ARROWFLIGHT_WRITE_ERROR, "Failed to write record batch to Arrow Flight server: {}",
    ...
    if (!close_result.ok()) {
      throw Exception(ErrorCodes::ARROWFLIGHT_WRITE_ERROR, "Failed to close writer after sending record batches: {}",
...
SinkToStoragePtr StorageArrowFlight::write(const ASTPtr& /* query */, const StorageMetadataPtr& metadata_snapshot, ContextPtr context_,
                                           bool /*async_write*/) {
...
void registerStorageArrowFlight(StorageFactory& factory) {
  factory.registerStorage("ArrowFlight",
                          [](const StorageFactory::Arguments& args) -> StoragePtr {
                            ...
                            return std::make_shared<StorageArrowFlight>(args.table_id, connection, config.dataset_name, args.columns,
                                                                        args.constraints, args.getContext());
                          },
                          {
