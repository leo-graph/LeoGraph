 ...
 */
void listFilesWithRegexpMatchingImpl(const std::string &path_for_ls, const std::string &for_match, size_t &total_bytes_to_read,
  ...
  if (first_glob_pos == std::string::npos) {
    try {
      ...
      absolute_path = absolute_path.lexically_normal();  /// ensure that the resulting path is normalized (e.g., removes any redundant
                                                         /// slashes or . and .. segments)
  ...
  if (!recursive) recursive = current_glob == "/**";

  const std::string prefix_without_globs = path_for_ls + for_match.substr(1, end_of_path_without_globs);

  if (!fs::exists(prefix_without_globs)) return;
  ...
  for (fs::directory_iterator it(prefix_without_globs, ec); it != end; it.increment(ec)) {
    ...
    if (!it->is_directory() && !looking_for_directory) {
      ...
      if (recursive) {
        ...
        /// Recursion depth is limited by pattern. '*' works only for depth = 1, for depth = 2 pattern path is '*/*'. So we do not need
        /// additional check.
        listFilesWithRegexpMatchingImpl(fs::path(full_path) / "", suffix_with_globs.substr(next_slash_after_glob_pos), total_bytes_to_read,
                                        result, false);
    }
  }
}

std::vector<std::string> listFilesWithRegexpMatching(const std::string &for_match, size_t &total_bytes_to_read) {
...
void checkCreationIsAllowed(ContextPtr context_global, const std::string &db_dir_path, const std::string &table_path,
  ...
  if (context_global->getApplicationType() != Context::ApplicationType::SERVER) return;
...
std::pair<String, String> splitToArchivePathAndPathInArchive(const String &source) {
  ...
  while (path_to_archive_view.ends_with(' ')) path_to_archive_view.remove_suffix(1);

  std::string_view filename_view = std::string_view{source}.substr(pos + 2);
  while (filename_view.starts_with(' ')) filename_view.remove_prefix(1);

  if (filename_view.empty() || path_to_archive_view.empty()) return {{}, source};
...
Strings getPathsList(const String &path_with_globs, const String &user_files_path, const ContextPtr &context, size_t &total_bytes_to_read) {
  ...
  if (fs_pattern.is_relative()) fs_pattern = user_files_absolute_path / fs_pattern;
  ...
  if (pattern.contains(PartitionedSink::PARTITION_ID_WILDCARD)) {
    ...
    if (!fs::is_directory(pattern)) {
      ...
      if (!error) total_bytes_to_read += size;
  ...
  for (const auto &path : paths) checkCreationIsAllowed(context, user_files_absolute_path, path, can_be_directory);
...
StorageFile::ArchiveInfo getArchiveInfo(const std::string &path_to_archive, const std::string &file_in_archive,
                                        const std::string &user_files_path, const ContextPtr &context, size_t &total_bytes_to_read) {
  ...
  if (file_in_archive.find_first_of("*?{") != std::string::npos) {
    ...
    if (!matcher->ok())
      throw Exception(ErrorCodes::CANNOT_COMPILE_REGEXP, "Cannot compile regex from glob ({}): {}", file_in_archive, matcher->error());
...
std::unique_ptr<ReadBuffer> selectReadBuffer(const String &current_path, bool use_table_fd, int table_fd, const struct stat &file_stat,
                                             ContextPtr context) {
  ...
  if (S_ISREG(file_stat.st_mode) && (read_method == LocalFSReadMethod::pread || read_method == LocalFSReadMethod::mmap)) {
    ...
    res = std::make_unique<AsynchronousReadBufferFromFileWithDescriptorsCache>(reader, Priority{}, current_path,
                                                                               context->getSettingsRef()[Setting::max_read_buffer_size]);
#else
...
struct stat getFileStat(const String &current_path, bool use_table_fd, int table_fd, const String &storage_name) {
  ...
  if (use_table_fd) {
    ...
    /// Check if file descriptor allows random reads (and reading it twice).
    if (0 != stat(current_path.c_str(), &file_stat)) throw ErrnoException(ErrorCodes::CANNOT_STAT, "Cannot stat file {}", current_path);
  }

  return file_stat;
}

std::unique_ptr<ReadBuffer> createReadBuffer(const String &current_path, const struct stat &file_stat, bool use_table_fd, int table_fd,
                                             const String &compression_method, ContextPtr context) {
...
struct ReadBufferFromFileIterator : public IReadBufferIterator, WithContext {
 public:
  ReadBufferFromFileIterator(const std::vector<String> &paths_, std::optional<String> format_, const String &compression_method_,
                             const std::optional<FormatSettings> &format_settings_, const ContextPtr &context_)
  ...
  Data next() override {
    ...
    if (is_first) {
      ...
      if (getContext()->getSettingsRef()[Setting::schema_inference_mode] == SchemaInferenceMode::DEFAULT) {
        if (auto cached_columns = tryGetColumnsFromCache(paths)) return {nullptr, cached_columns, format};
      }
    }
        ...
        if (is_first) {
          if (format)
            throw Exception(ErrorCodes::CANNOT_EXTRACT_TABLE_STRUCTURE,
                            "The table structure cannot be extracted from a {} format file, because all files are empty. You can specify "
                            "the format manually",
                            *format);

          throw Exception(ErrorCodes::CANNOT_DETECT_FORMAT,
                          "The data format cannot be detected by the contents of the files, because all files are empty. You can specify "
    ...
    if (getContext()->getSettingsRef()[Setting::schema_inference_mode] == SchemaInferenceMode::UNION) {
      if (auto cached_columns = tryGetColumnsFromCache({path})) return {nullptr, cached_columns, format};
    }
  ...
  void setNumRowsToLastFile(size_t num_rows) override {
    if (!getContext()->getSettingsRef()[Setting::use_cache_for_count_from_files]) return;
  ...
  void setSchemaToLastFile(const ColumnsDescription &columns) override {
    if (!getContext()->getSettingsRef()[Setting::schema_inference_use_cache_for_file]) return;
  ...
  String getLastFilePath() const override {
    if (current_index != 0) return paths[current_index - 1];
    return "";
  }

  void setFormatName(const String &format_name) override { format = format_name; }
 ...
 private:
  std::optional<ColumnsDescription> tryGetColumnsFromCache(const Strings &paths_) {
    ...
    if (!context->getSettingsRef()[Setting::schema_inference_use_cache_for_file]) return std::nullopt;
    ...
    for (const auto &path : paths_) {
      auto get_last_mod_time = [&]() -> std::optional<time_t> {
        if (0 != stat(path.c_str(), &file_stat)) return std::nullopt;
      ...
      if (format) {
        ...
        if (auto columns = schema_cache.tryGetColumns(cache_key, get_last_mod_time)) return columns;
      } else {
...
struct ReadBufferFromArchiveIterator : public IReadBufferIterator, WithContext {
 public:
  ReadBufferFromArchiveIterator(const StorageFile::ArchiveInfo &archive_info_, std::optional<String> format_,
                                const std::optional<FormatSettings> &format_settings_, const ContextPtr &context_)
      : WithContext(context_), archive_info(archive_info_), format(std::move(format_)), format_settings(format_settings_) {}
  ...
  Data next() override {
    ...
    while (true) {
      ...
      if (archive_info.isSingleFileRead()) {
        ...
        last_read_file_path =
            paths_for_schema_cache.emplace_back(fmt::format("{}::{}", archive_reader->getPath(), archive_info.path_in_archive));
        is_first = false;

        if (auto cached_schema = tryGetSchemaFromCache(archive, last_read_file_path)) return {nullptr, cached_schema, format};
      } else {
        if (last_read_buffer)
        ...
        if (!format) {
          ...
          /// Use this format only if we have a schema reader for it.
          if (format_from_file && FormatFactory::instance().checkIfFormatHasAnySchemaReader(*format_from_file)) format = format_from_file;
        }
  ...
  void setPreviousReadBuffer(std::unique_ptr<ReadBuffer> buffer) override {
    if (buffer) last_read_buffer = std::move(buffer);
  }
  ...
  String getLastFilePath() const override { return last_read_file_path; }
 ...
 private:
  std::optional<ColumnsDescription> tryGetSchemaFromCache(const std::string &archive_path, const std::string &full_path) {
    ...
    auto &schema_cache = StorageFile::getSchemaCache(context);
    auto get_last_mod_time = [&]() -> std::optional<time_t> {
      if (0 != stat(archive_path.c_str(), &file_stat)) return std::nullopt;
...
StorageFile::FileSource StorageFile::FileSource::parse(const String &source, const ContextPtr &context,
                                                       std::optional<bool> allow_archive_path_syntax) {
  String filename;
  String path_to_archive;

  if (!allow_archive_path_syntax) allow_archive_path_syntax = context->getSettingsRef()[Setting::allow_archive_path_syntax];
...
std::pair<ColumnsDescription, String> StorageFile::getTableStructureAndFormatFromFileDescriptor(std::optional<String> format,
...
std::pair<ColumnsDescription, String> StorageFile::getTableStructureAndFormatFromFileImpl(
    std::optional<String> format, const std::vector<String> &paths, const String &compression_method,
    const std::optional<FormatSettings> &format_settings, const ContextPtr &context, const std::optional<ArchiveInfo> &archive_info) {
  ...
  if (archive_info) {
    ...
    if (format) return {readSchemaFromFormat(*format, format_settings, read_buffer_iterator, context), *format};
...
ColumnsDescription StorageFile::getTableStructureFromFile(const DB::String &format, const std::vector<String> &paths,
...
std::pair<ColumnsDescription, String> StorageFile::getTableStructureAndFormatFromFile(const std::vector<String> &paths,
                                                                                      const DB::String &compression_method,
                                                                                      const std::optional<FormatSettings> &format_settings,
                                                                                      const ContextPtr &context,
                                                                                      const std::optional<ArchiveInfo> &archive_info) {
...
bool StorageFile::supportsPrewhere() const { return supports_prewhere; }
bool StorageFile::canMoveConditionsToPrewhere() const { return supports_prewhere; }
...
bool StorageFile::prefersLargeBlocks() const { return FormatFactory::instance().checkIfOutputFormatPrefersLargeBlocks(format_name); }
...
StorageFile::StorageFile(int table_fd_, CommonArguments args) : StorageFile(args) {
  ...
  if (-1 == res) throw ErrnoException(ErrorCodes::CANNOT_FSTAT, "Cannot execute fstat");
...
StorageFile::StorageFile(FileSource file_source_, CommonArguments args) : StorageFile(args) {
...
StorageFile::StorageFile(const std::string &relative_table_dir_path, CommonArguments args) : StorageFile(args) {
  if (relative_table_dir_path.empty()) throw Exception(ErrorCodes::INCORRECT_FILE_NAME, "Storage {} requires data path", getName());
  if (args.format_name == "Distributed")
    throw Exception(ErrorCodes::INCORRECT_FILE_NAME, "Distributed format is allowed only with explicit file path");
...
StorageFile::StorageFile(CommonArguments args)
  ...
  if (format_name != "Distributed" && format_name != "auto") FormatFactory::instance().checkFormatName(format_name);
...
void StorageFile::setStorageMetadata(CommonArguments args) {
    ...
    if (use_table_fd) {
      ...
      if (format_name == "auto")
        std::tie(columns, format_name) =
            getTableStructureAndFormatFromFile(paths, compression_method, format_settings, args.getContext(), archive_info);
      else
  ...
  std::tie(hive_partition_columns_to_read_from_file_path, std::ignore) = HivePartitioningUtils::setupHivePartitioningForFileURLLikeStorage(
      storage_columns, sample_path, args.columns.empty(), format_settings, args.getContext());

  setVirtuals(VirtualColumnUtils::getVirtualsForFileLikeStorage(storage_metadata.columns, args.getContext(), format_settings,
                                                                PartitionStrategyFactory::StrategyType::NONE, sample_path));
  setInMemoryMetadata(storage_metadata);

  supports_prewhere = format_name != "Distributed" &&
                      FormatFactory::instance().checkIfFormatSupportsPrewhere(format_name, args.getContext(), format_settings);
}
...
StorageFileSource::FilesIterator::FilesIterator(const Strings &files_, std::optional<StorageFile::ArchiveInfo> archive_info_,
                                                const ActionsDAG::Node *predicate, const NamesAndTypesList &virtual_columns,
                                                const NamesAndTypesList &hive_columns, const ContextPtr &context_,
...
String StorageFileSource::FilesIterator::next() {
  ...
  if (current_index >= fs.size()) return {};

  return fs[current_index];
}

const String &StorageFileSource::FilesIterator::getFileNameInArchive() {
  if (archive_info->path_in_archive.empty()) throw Exception(ErrorCodes::LOGICAL_ERROR, "Expected only 1 filename but it's empty");

  return archive_info->path_in_archive;
}

StorageFileSource::StorageFileSource(const ReadFromFormatInfo &info, std::shared_ptr<StorageFile> storage_, const ContextPtr &context_,
                                     UInt64 max_block_size_, FilesIteratorPtr files_iterator_, std::unique_ptr<ReadBuffer> read_buf_,
                                     bool need_only_count_, FormatParserSharedResourcesPtr parser_shared_resources_,
  ...
  if (!storage->use_table_fd) {
    ...
    if (!shared_lock) throw Exception(ErrorCodes::TIMEOUT_EXCEEDED, "Lock timeout exceeded");
    storage->readers_counter.fetch_add(1, std::memory_order_release);
  }
}

void StorageFileSource::beforeDestroy() {
  if (storage->file_renamer.isEmpty()) return;
  ...
  if (std::uncaught_exceptions() == 0 && cnt == 1 && !storage->was_renamed) {
    ...
    if (!exclusive_lock) return;
    if (storage->readers_counter.load(std::memory_order_acquire) != 0 || storage->was_renamed) return;
    ...
    for (auto &file_path_ref : storage->paths) {
      try {
        ...
        if (fs::exists(file_path)) throw Exception(ErrorCodes::FILE_ALREADY_EXISTS, "File {} already exists", file_path.string());
...
StorageFileSource::~StorageFileSource() { beforeDestroy(); }

bool StorageFileSource::tryGetCountFromCache(const struct stat &file_stat) {
  if (!getContext()->getSettingsRef()[Setting::use_cache_for_count_from_files]) return false;

  auto num_rows_from_cache = tryGetNumRowsFromCache(current_path, file_stat.st_mtime);
  if (!num_rows_from_cache) return false;
  ...
  auto const_chunk_generator =
      std::make_shared<ConstChunkGenerator>(std::make_shared<const Block>(block_for_format), *num_rows_from_cache, max_block_size);
  QueryPipelineBuilder builder;
  builder.init(Pipe(const_chunk_generator));
  builder.addSimpleTransform(
      [&](const SharedHeader &header) { return std::make_shared<ExtractColumnsTransform>(header, requested_columns); });
  ...
  while (!finished_generate) {
    ...
    if (!reader) {
      if (!storage->use_table_fd) {
        if (files_iterator->isReadFromArchive()) {
          if (files_iterator->isSingleFileReadFromArchive()) {
            ...
            if (getContext()->getSettingsRef()[Setting::engine_file_skip_empty_files] && file_stat.st_size == 0) continue;
            ...
            if (need_only_count && tryGetCountFromCache(file_stat)) continue;
            ...
            while (true) {
              if (file_enumerator == nullptr) {
                ...
                if (getContext()->getSettingsRef()[Setting::engine_file_skip_empty_files] && current_archive_stat.st_size == 0) continue;
            ...
            if (need_only_count && tryGetCountFromCache(current_archive_stat)) continue;
      ...
      if (!read_buf) {
        ...
        read_buf =
            createReadBuffer(current_path, file_stat, storage->use_table_fd, storage->table_fd, storage->compression_method, getContext());
      }
      ...
      input_format = FormatFactory::instance().getInput(storage->format_name, *read_buf, block_for_format, getContext(), max_block_size,
                                                        storage->format_settings, parser_shared_resources, format_filter_info,
                                                        /*is_remote_fs=*/false, CompressionMethod::None, need_only_count);

      input_format->setSerializationHints(serialization_hints);

      if (need_only_count) input_format->needOnlyCount();
    ...
    if (reader->pull(chunk)) {
      ...
      size_t chunk_size = 0;
      if (input_format && storage->format_name != "Distributed") chunk_size = input_format->getApproxBytesReadForChunk();
      ...
      if (!hive_partition_columns_to_read_from_file_path.empty()) {
        HivePartitioningUtils::addPartitionColumnsToChunk(chunk, hive_partition_columns_to_read_from_file_path, current_path);
      }
    ...
    if (storage->use_table_fd) finished_generate = true;
    ...
    if (files_iterator->isReadFromArchive() && !files_iterator->isSingleFileReadFromArchive()) {
      if (file_enumerator) {
        if (!file_enumerator->nextFile()) file_enumerator = nullptr;
      } else {
...
std::optional<size_t> StorageFileSource::tryGetNumRowsFromCache(const String &path, time_t last_mod_time) const {
  auto &schema_cache = StorageFile::getSchemaCache(getContext());
  auto key = getKeyForSchemaCache(path, storage->format_name, storage->format_settings, getContext());
  auto get_last_mod_time = [&]() -> std::optional<time_t> { return last_mod_time; };
...
class ReadFromFile : public SourceStepWithFilter {
 public:
  ...
  ReadFromFile(const Names &column_names_, const SelectQueryInfo &query_info_, const StorageSnapshotPtr &storage_snapshot_,
               const ContextPtr &context_, std::shared_ptr<StorageFile> storage_, ReadFromFormatInfo info_, const bool need_only_count_,
               size_t max_block_size_, size_t num_streams_)
...
void ReadFromFile::applyFilters(ActionDAGNodes added_filter_nodes) {
  ...
  if (filter_actions_dag) predicate = filter_actions_dag->getOutputs().at(0);
...
void StorageFile::read(QueryPlan &query_plan, const Names &column_names, const StorageSnapshotPtr &storage_snapshot,
  ...
  auto read_from_format_info = prepareReadingFromFormat(
      column_names, storage_snapshot, context, supportsSubsetOfColumns(context),
  ...
  bool need_only_count =
      (query_info.optimize_trivial_count || (read_from_format_info.requested_columns.empty() && !read_from_format_info.prewhere_info &&
                                             !read_from_format_info.row_level_filter)) &&
      context->getSettingsRef()[Setting::optimize_count_from_files];

  auto reading = std::make_unique<ReadFromFile>(column_names, query_info, storage_snapshot, context, std::move(this_ptr),
                                                std::move(read_from_format_info), need_only_count, max_block_size, num_streams);

  query_plan.addStep(std::move(reading));
}

void ReadFromFile::createIterator(const ActionsDAG::Node *predicate) {
  if (files_iterator) return;

  files_iterator = std::make_shared<StorageFileSource::FilesIterator>(
      storage->paths, storage->archive_info, predicate, storage->getVirtualsList(), info.hive_partition_columns_to_read_from_file_path,
      context, storage->distributed_processing);
}
...
void ReadFromFile::initializePipeline(QueryPipelineBuilder &pipeline, const BuildQueryPipelineSettings &) {
  ...
  if (max_num_streams > files_to_read) num_streams = files_to_read;
  ...
  if (progress_callback && !storage->archive_info) progress_callback(FileProgress(0, storage->total_bytes_to_read));
  ...
  auto format_filter_info =
      std::make_shared<FormatFilterInfo>(filter_actions_dag, ctx, nullptr, query_info.row_level_filter, query_info.prewhere_info);
  ...
  for (size_t i = 0; i < num_streams; ++i) {
    ...
    if (storage->has_peekable_read_buffer_from_fd.exchange(false)) read_buffer = std::move(storage->peekable_read_buffer_from_fd);

    auto source = std::make_shared<StorageFileSource>(info, storage, ctx, max_block_size, files_iterator, std::move(read_buffer),
                                                      need_only_count, parser_shared_resources, format_filter_info);
  ...
  if (pipe.empty()) pipe = Pipe(std::make_shared<NullSource>(std::make_shared<const Block>(info.source_header)));
...
class StorageFileSink final : public SinkToStorage, WithContext {
 public:
  StorageFileSink(const StorageMetadataPtr &metadata_snapshot_, const String &table_name_for_log_, int table_fd_, bool use_table_fd_,
                  std::string base_path_, std::string path_, const CompressionMethod compression_method_,
                  const std::optional<FormatSettings> &format_settings_, const String format_name_, const ContextPtr &context_, int flags_)
  ...
  StorageFileSink(const StorageMetadataPtr &metadata_snapshot_, const String &table_name_for_log_,
                  std::unique_lock<std::shared_timed_mutex> &&lock_, int table_fd_, bool use_table_fd_, std::string base_path_,
                  const std::string &path_, const CompressionMethod compression_method_,
                  const std::optional<FormatSettings> &format_settings_, const String format_name_, const ContextPtr &context_, int flags_)
      : SinkToStorage(std::make_shared<const Block>(metadata_snapshot_->getSampleBlock())),
        WithContext(context_),
        metadata_snapshot(metadata_snapshot_),
        ...
        flags(flags_),
        lock(std::move(lock_)) {
    if (!lock) throw Exception(ErrorCodes::TIMEOUT_EXCEEDED, "Lock timeout exceeded");
    initialize();
  }

  ~StorageFileSink() override {
    if (isCancelled()) cancelBuffers();
  }
  ...
  void initialize() {
    ...
    write_buf = wrapWriteBufferWithCompressionMethod(std::move(naked_buffer), compression_method,
    ...
    writer = FormatFactory::instance().getOutputFormatParallelIfPossible(format_name, *write_buf, metadata_snapshot->getSampleBlock(),
                                                                         getContext(), format_settings);

    if (do_not_write_prefix) writer->doNotWritePrefix();
  }
...
class PartitionedStorageFileSink : public PartitionedSink {
 public:
  PartitionedStorageFileSink(std::shared_ptr<IPartitionStrategy> partition_strategy_, const StorageMetadataPtr &metadata_snapshot_,
                             const String &table_name_for_log_, std::unique_lock<std::shared_timed_mutex> &&lock_, String base_path_,
                             String path_, const CompressionMethod compression_method_,
                             const std::optional<FormatSettings> &format_settings_, const String format_name_, ContextPtr context_,
                             int flags_)
  ...
  SinkPtr createSinkForPartition(const String &partition_id) override {
    ...
    return std::make_shared<StorageFileSink>(metadata_snapshot, table_name_for_log, -1,
                                             /* use_table_fd */ false, base_path, filepath, compression_method, format_settings,
                                             format_name, context, flags);
  }
...
SinkToStoragePtr StorageFile::write(const ASTPtr &query, const StorageMetadataPtr &metadata_snapshot, ContextPtr context,
                                    bool /*async_insert*/) {
  if (!use_table_fd && archive_info.has_value()) throw Exception(ErrorCodes::NOT_IMPLEMENTED, "Writing to archives is not supported");

  if (format_name == "Distributed") throw Exception(ErrorCodes::NOT_IMPLEMENTED, "Method write is not implemented for Distributed format");

  int flags = 0;

  if (context->getSettingsRef()[Setting::engine_file_truncate_on_insert]) flags |= O_TRUNC;
  ...
  if (is_partitioned_implementation) {
    if (path_for_partitioned_write.empty()) throw Exception(ErrorCodes::LOGICAL_ERROR, "Empty path for partitioned write");

    auto partition_strategy =
        PartitionStrategyFactory::get(PartitionStrategyFactory::StrategyType::WILDCARD, insert_query->partition_by,
                                      metadata_snapshot->getColumns().getAll(), context, format_name, is_path_with_globs, has_wildcards,
                                      /* partition_columns_in_data_file */ true);

    return std::make_shared<PartitionedStorageFileSink>(
        partition_strategy, metadata_snapshot, getStorageID().getNameForLogs(), std::unique_lock{rwlock, getLockTimeout(context)},
        base_path, path_for_partitioned_write, chooseCompressionMethod(path_for_partitioned_write, compression_method), format_settings,
        format_name, context, flags);
  }
  ...
  if (!paths.empty()) {
    if (is_path_with_globs)
      throw Exception(ErrorCodes::DATABASE_ACCESS_DENIED, "Table '{}' is in readonly mode because of globs in filepath",
    ...
    if (!context->getSettingsRef()[Setting::engine_file_truncate_on_insert] && !is_path_with_globs &&
      ...
      if (context->getSettingsRef()[Setting::engine_file_allow_create_multiple_files]) {
        ...
        throw Exception(ErrorCodes::CANNOT_APPEND_TO_FILE,
  ...
  return std::make_shared<StorageFileSink>(metadata_snapshot, getStorageID().getNameForLogs(),
                                           std::unique_lock{rwlock, getLockTimeout(context)}, table_fd, use_table_fd, base_path, path,
                                           chooseCompressionMethod(path, compression_method), format_settings, format_name, context, flags);
}

bool StorageFile::storesDataOnDisk() const { return is_db_table; }

Strings StorageFile::getDataPaths() const {
  if (paths.empty()) throw Exception(ErrorCodes::DATABASE_ACCESS_DENIED, "Table '{}' is in readonly mode", getStorageID().getNameForLogs());
  return paths;
}

void StorageFile::rename(const String &new_path_to_table_data, const StorageID &new_table_id) {
  if (!is_db_table)
    throw Exception(ErrorCodes::DATABASE_ACCESS_DENIED, "Can't rename table {} bounded to user-defined file (or FD)",
                    getStorageID().getNameForLogs());
...
void StorageFile::truncate(const ASTPtr & /*query*/, const StorageMetadataPtr & /* metadata_snapshot */, ContextPtr /* context */,
  ...
  factory.registerStorage(
      ...
      [](const StorageFactory::Arguments &factory_args) {
        ...
        StorageFile::CommonArguments storage_args{
            WithContext(context),     factory_args.table_id, {}, {}, {}, factory_args.columns,
            factory_args.constraints, factory_args.comment,  {},
        };
        ...
        if (auto opt_name = tryGetIdentifierName(engine_args_ast[1])) {
          if (*opt_name == "stdin")
            ...
            throw Exception(ErrorCodes::UNKNOWN_IDENTIFIER, "Unknown identifier '{}' in second arg of File storage constructor", *opt_name);
        ...
        if (!file_source) throw Exception(ErrorCodes::BAD_ARGUMENTS, "Second argument must be path or file descriptor");
