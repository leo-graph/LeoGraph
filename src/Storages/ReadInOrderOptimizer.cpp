...
ASTPtr getFixedPoint(const ASTPtr& ast, const ContextPtr& context) {
  ...
  if (!func || func->name != "equals") return nullptr;

  if (!func->arguments || func->arguments->children.size() != 2) return nullptr;

  const auto& lhs = func->arguments->children[0];
  const auto& rhs = func->arguments->children[1];

  if (!lhs->as<ASTLiteral>() && !rhs->as<ASTLiteral>()) return nullptr;
  ...
  while (const auto* arg_func = argument->as<ASTFunction>()) {
    if (!arg_func->arguments || arg_func->arguments->children.size() != 1) return nullptr;

    auto func_resolver = FunctionFactory::instance().tryGet(arg_func->name, context);
    if (!func_resolver || !func_resolver->isInjective({})) return nullptr;
...
NameSet getFixedSortingColumns(const ASTSelectQuery& query, const Names& sorting_key_columns, const ContextPtr& context) {
  ASTPtr condition;
  ...
  cnf->iterateGroups([&](const auto& group) {
    if (group.size() == 1 && !group.begin()->negative) {
      ...
      if (fixed_point) {
        ...
        if (sorting_key_columns_set.contains(column_name)) fixed_points.insert(column_name);
      }
    }
  });
...
MatchResult matchSortDescriptionAndKey(const ExpressionActions::Actions& actions, const SortColumnDescription& sort_column,
  ...
  if (sort_column.column_name == sorting_key_column) return result;
  ...
  for (const auto& action : actions) {
    ...
    found_function = true;
    if (action.node->children.size() != 1 || action.node->children.at(0)->result_name != sorting_key_column) return {};

    const auto& func = *action.node->function_base;
    if (!func.hasInformationAboutMonotonicity()) return {};

    auto monotonicity = func.getMonotonicityForRange(*func.getArgumentTypes().at(0), {}, {});
    if (!monotonicity.is_monotonic) return {};
    ...
    if (!monotonicity.is_strict) result.is_last_key = true;

    if (!monotonicity.is_positive) result.direction *= -1;
  }
...
ReadInOrderOptimizer::ReadInOrderOptimizer(const ASTSelectQuery& query_, const ManyExpressionActions& elements_actions_,
                                           const SortDescription& required_sort_description_, const TreeRewriterResultPtr& syntax_result)
    : elements_actions(elements_actions_), required_sort_description(required_sort_description_), query(query_) {
...
InputOrderInfoPtr ReadInOrderOptimizer::getInputOrderImpl(const StorageMetadataPtr& metadata_snapshot, const SortDescription& description,
                                                          const ManyExpressionActions& actions, const ContextPtr& context,
                                                          UInt64 limit) const {
  ...
  while (desc_pos < description.size() && key_pos < sorting_key_columns.size()) {
    if (forbidden_columns.contains(description[desc_pos].column_name)) break;
    ...
    if (read_direction == 0) read_direction = match.direction;
    ...
    if (match.is_last_key) break;
  }

  if (sort_description_for_merging.empty()) return {};
...
InputOrderInfoPtr ReadInOrderOptimizer::getInputOrder(const StorageMetadataPtr& metadata_snapshot, ContextPtr context, UInt64 limit) const {
  if (!metadata_snapshot->hasSortingKey()) return {};
  ...
  if (context->getSettingsRef()[Setting::optimize_respect_aliases] && !aliased_columns.empty()) {
    ...
    for (size_t i = 0; i < required_sort_description.size(); ++i) {
      if (!aliased_columns.contains(required_sort_description[i].column_name)) continue;
