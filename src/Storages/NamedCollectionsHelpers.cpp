...
std::optional<std::string> getCollectionName(ASTs asts) {
  if (asts.empty()) return std::nullopt;
...
std::optional<std::pair<std::string, std::variant<Field, ASTPtr>>> getKeyValueFromASTImpl(ASTPtr ast, bool fallback_to_ast_value,
                                                                                          ContextPtr context) {
  const auto *function = ast->as<ASTFunction>();
  if (!function || function->name != "equals") return std::nullopt;

  const auto *function_args_expr = assert_cast<const ASTExpressionList *>(function->arguments.get());
  const auto &function_args = function_args_expr->children;

  if (function_args.size() != 2) return std::nullopt;
  ...
  try {
    ...
    if (fallback_to_ast_value) return std::pair{key, function_args[1]};
    throw;
  }
...
std::map<String, Field> getParamsMapFromAST(ASTs asts, ContextPtr context) {
  ...
  for (const auto &ast : asts) {
    ...
    if (!inserted) throw Exception(ErrorCodes::BAD_ARGUMENTS, "Duplicated key '{}' in params", key);
  }

  return params;
}

MutableNamedCollectionPtr tryGetNamedCollectionWithOverrides(ASTs asts, ContextPtr context, bool throw_unknown_collection,
  ...
  if (!collection_name.has_value()) return nullptr;
  ...
  if (asts.size() == 1) {
    if (dependent_table_id) NamedCollectionFactory::instance().addDependency(*collection_name, *dependent_table_id);
  ...
  for (auto it = std::next(asts.begin()); it != asts.end(); ++it) {
    ...
    if (!value_override) {
      if (!(*it)->as<ASTFunction>()) throw Exception(ErrorCodes::BAD_ARGUMENTS, "Expected key-value argument or function");
      if (allow_override_by_default) continue;
...
MutableNamedCollectionPtr tryGetNamedCollectionWithOverrides(const Poco::Util::AbstractConfiguration &config,
  ...
  if (collection_name.empty()) return nullptr;
...
HTTPHeaderEntries getHeadersFromNamedCollection(const NamedCollection &collection) {
  ...
  for (const auto &key : keys) headers.emplace_back(collection.get<String>(key + ".name"), collection.get<String>(key + ".value"));
  return headers;
}
