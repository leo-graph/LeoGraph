...
static MergeTreeTransactionPtr tryGetTransactionForMutation(const MergeTreeMutationEntry &mutation, LoggerPtr log = nullptr) {
  ...
  if (mutation.tid.isPrehistoric()) return {};
...
StorageMergeTree::StorageMergeTree(const StorageID &table_id_, const String &relative_data_path_, const StorageInMemoryMetadata &metadata_,
                                   LoadingStrictnessLevel mode, ContextMutablePtr context_, const String &date_column_name,
                                   const MergingParams &merging_params_, std::unique_ptr<MergeTreeSettings> storage_settings_)
    : MergeTreeData(table_id_, metadata_, context_, date_column_name, merging_params_, std::move(storage_settings_),
...
void StorageMergeTree::flushAndPrepareForShutdown() {
  ...
  if (flush_called.exchange(true)) return;
...
void StorageMergeTree::shutdown(bool) {
  ...
  if (deduplication_log) deduplication_log->shutdown();
}

StorageMergeTree::~StorageMergeTree() { shutdown(false); }

void StorageMergeTree::read(QueryPlan &query_plan, const Names &column_names, const StorageSnapshotPtr &storage_snapshot,
...
std::optional<UInt64> StorageMergeTree::totalRows(ContextPtr) const { return getTotalActiveSizeInRows(); }

std::optional<UInt64> StorageMergeTree::totalRowsByPartitionPredicate(const ActionsDAG &filter_actions_dag,
...
std::optional<UInt64> StorageMergeTree::totalBytes(ContextPtr) const { return getTotalActiveSizeInBytes(); }

std::optional<UInt64> StorageMergeTree::totalBytesUncompressed(const Settings &) const {
  UInt64 res = 0;
  auto parts = getDataPartsForInternalUsage();
  for (const auto &part : parts) res += part->getBytesUncompressedOnDisk();
  return res;
}
...
void StorageMergeTree::alter(const AlterCommands &commands, ContextPtr local_context, AlterLockHolder &table_lock_holder) {
  ...
  if (!only_setting_changes) assertNotReadonly();
  ...
  auto maybe_mutation_commands =
      commands.getMutationCommands(new_metadata, query_settings[Setting::materialize_ttl_after_modify], local_context);
  if (!maybe_mutation_commands.empty()) delayMutationOrThrowIfNeeded(nullptr, local_context);
  ...
  if (!query_settings[Setting::allow_suspicious_primary_key]) MergeTreeData::verifySortingKey(new_metadata.sorting_key);

  /// This alter can be performed at new_metadata level only
  if (commands.isSettingsAlter()) {
    changeSettings(new_metadata.settings_changes, table_lock_holder);

    if (statistics_changed) setInMemoryMetadata(new_metadata);
    ...
    if (!maybe_mutation_commands.empty() && maybe_mutation_commands.containBarrierCommand()) {
      int64_t prev_mutation = 0;
      {
        ...
        if (it != current_mutations_by_version.rend()) prev_mutation = it->first;
      }
...

    {
      ...
      if (!maybe_mutation_commands.empty()) mutation_version = startMutation(maybe_mutation_commands, local_context);
    }

    if (!maybe_mutation_commands.empty() && query_settings[Setting::alter_sync] > 0) waitForMutation(mutation_version, false);
  }

  {
    ...
    if ((*old_storage_settings)[MergeTreeSetting::non_replicated_deduplication_window] !=
        (*new_storage_settings)[MergeTreeSetting::non_replicated_deduplication_window]) {
      ...
      if ((*new_storage_settings)[MergeTreeSetting::non_replicated_deduplication_window] != 0 &&
...
CurrentlyMergingPartsTagger::CurrentlyMergingPartsTagger(FutureMergedMutatedPartPtr future_part_, size_t total_size,
                                                         StorageMergeTree &storage_, const StorageMetadataPtr &metadata_snapshot,
                                                         bool is_mutation)
  ...
  if (is_mutation) {
    ...
    reserved_space = storage.balancedReservation(metadata_snapshot, total_size, max_volume_index, future_part->name, future_part->part_info,
                                                 future_part->parts, &tagger, &ttl_infos);
  ...
  if (!reserved_space) {
    if (is_mutation) throw Exception(ErrorCodes::NOT_ENOUGH_SPACE, "Not enough space for mutating part '{}'", future_part->parts[0]->name);
  ...
  if (is_mutation) storage.currently_mutating_part_future_versions[future_part->parts[0]] = future_part->part_info.mutation;
}
...
void CurrentlyMergingPartsTagger::finalize() {
  ...
  for (const auto &part : future_part->parts) {
    if (!storage.currently_merging_mutating_parts.contains(part)) std::terminate();
...
CurrentlyMergingPartsTagger::~CurrentlyMergingPartsTagger() {
  if (!finalized) {
    LOG_WARNING(getLogger("CurrentlyMergingPartsTagger"),
                "CurrentlyMergingPartsTagger was not finalized, the finalization will happen in the destructor");
    finalize();
  }
}
...
MergeMutateSelectedEntry::~MergeMutateSelectedEntry() {
  if (!finalized) {
    LOG_WARNING(getLogger("MergeMutateSelectedEntry"),
                "MergeMutateSelectedEntry was not finalized, the finalization will happen in the destructor");
    finalize();
  }
}
...
Int64 StorageMergeTree::startMutation(const MutationCommands &commands, ContextPtr query_context) {
  ...
  if (txn) txn->addMutation(shared_from_this(), mutation_id);

  {
    std::lock_guard lock(currently_processing_in_background_mutex);

    auto [it, inserted] = current_mutations_by_version.try_emplace(version, std::move(entry));
    if (!inserted) throw Exception(ErrorCodes::LOGICAL_ERROR, "Mutation {} already exists, it's a bug", version);
...
void StorageMergeTree::updateMutationEntriesErrors(FutureMergedMutatedPartPtr result_part, bool is_successful,
                                                   const String &exception_message, const String &error_code_name) {
  ...
  if (sources_data_version != result_data_version) {
    ...
    for (auto it = mutations_begin_it; it != mutations_end_it; ++it) {
      ...
      if (is_successful) {
        ...
        if (static_cast<UInt64>(result_part->part_info.mutation) == it->first) {
          mutation_backoff_policy.addPartMutationFailure(failed_part->name,
                                                         (*getSettings())[MergeTreeSetting::max_postpone_time_for_failed_mutations_ms]);
        }
      }
    }
  }
...
void StorageMergeTree::waitForMutation(Int64 version, const String &mutation_id, bool wait_for_another_mutation) {
  LOG_INFO(log, "Waiting mutation: {}", mutation_id);
  {
    auto check = [version, wait_for_another_mutation, this]() {
      if (shutdown_called) return true;
...
void StorageMergeTree::setMutationCSN(const String &mutation_id, CSN csn) {
  ...
  if (it == current_mutations_by_version.end()) throw Exception(ErrorCodes::LOGICAL_ERROR, "Cannot find mutation {}", mutation_id);
...
std::unique_ptr<PlainLightweightUpdateLock> StorageMergeTree::getLockForLightweightUpdate(const MutationCommands &commands,
...
QueryPipeline StorageMergeTree::updateLightweight(const MutationCommands &commands, ContextPtr query_context) {
  ...
  for (const auto &partition_id : all_partitions) {
    if (!partition_id.starts_with(MergeTreePartInfo::PATCH_PART_PREFIX)) partition_id_to_max_block->emplace(partition_id, block_number);
  }
...
bool StorageMergeTree::hasLightweightDeletedMask() const { return has_lightweight_delete_parts.load(std::memory_order_relaxed); }
...
std::optional<MergeTreeMutationStatus> StorageMergeTree::getIncompleteMutationsStatus(Int64 mutation_version,
                                                                                      std::set<String> *mutation_ids,
                                                                                      bool from_another_mutation) const {
  std::unique_lock lock(currently_processing_in_background_mutex);
  return getIncompleteMutationsStatusUnlocked(mutation_version, lock, mutation_ids, from_another_mutation);
}

std::optional<MergeTreeMutationStatus> StorageMergeTree::getIncompleteMutationsStatusUnlocked(Int64 mutation_version,
                                                                                              std::unique_lock<std::mutex> & /*lock*/,
                                                                                              std::set<String> *mutation_ids,
                                                                                              bool from_another_mutation) const {
  auto current_mutation_it = current_mutations_by_version.find(mutation_version);
  /// Killed
  if (current_mutation_it == current_mutations_by_version.end()) return {};
      ...
      if (!mutation_entry.latest_fail_reason.empty()) {
        ...
        if (mutation_ids) {
          ...
          for (auto it = mutations_begin_it; it != current_mutations_by_version.end(); ++it)
            ...
            if (it->second.latest_fail_reason == result.latest_fail_reason) mutation_ids->insert(it->second.file_name);
        }
...
std::map<std::string, MutationCommands> StorageMergeTree::getUnfinishedMutationCommands() const {
  ...
  for (const auto &part : data_parts) part_versions_with_names.emplace_back(PartVersionWithName{part->info.getDataVersion(), part->name});
  ...
  for (const auto &[mutation_version, entry] : current_mutations_by_version) {
    ...
    if (parts_to_do > 0) result.emplace(entry.file_name, *entry.commands);
  }
  return result;
}
...
std::vector<MergeTreeMutationStatus> StorageMergeTree::getMutationsStatus() const {
  ...
  for (const auto &kv : current_mutations_by_version) {
    ...
    for (size_t i = 0; i < parts_to_do; ++i) parts_to_do_names.push_back(part_versions_with_names[i].name);
    ...
    if (!parts_to_do_names.empty()) {
      for (const auto &[part_name, postpone_reason] : current_parts_postpone_reasons) {
        if (part_name == PostponeReasons::ALL_PARTS_KEY) {
          ...
          if (part_info.getDataVersion() < mutation_version) parts_postpone_reasons_map[part_name] = postpone_reason;
        }
      }
    }
...
CancellationCode StorageMergeTree::killMutation(const String &mutation_id) {
  ...
  if (!mutation_version) return CancellationCode::NotFound;
  ...
  std::optional<MergeTreeMutationEntry> to_kill;
  {
    ...
    if (it != current_mutations_by_version.end()) {
      if (!it->second.is_done) decrementMutationsCounters(mutation_counters, *it->second.commands);
  ...
  if (!to_kill) return CancellationCode::NotFound;
...
void StorageMergeTree::loadDeduplicationLog() {
  ...
  if ((*settings)[MergeTreeSetting::non_replicated_deduplication_window] != 0 &&
      format_version < MERGE_TREE_DATA_MIN_FORMAT_VERSION_WITH_CUSTOM_PARTITIONING)
    throw Exception(ErrorCodes::BAD_ARGUMENTS, "Deduplication for non-replicated MergeTree in old syntax is not supported");
      ...
      if (startsWith(it->name(), "mutation_")) {
        ...
        if (!entry.tid.isPrehistoric() && !entry.csn) {
          if (auto csn = TransactionLog::getCSN(entry.tid)) {
            ...
            LOG_DEBUG(log, "Mutation entry {} was created by transaction {}, but it was not committed. Removing mutation entry", it->name(),
                      entry.tid);
        ...
        if (!inserted) throw Exception(ErrorCodes::LOGICAL_ERROR, "Mutation {} already exists, it's a bug", block_number);
...
std::expected<MergeMutateSelectedEntryPtr, SelectMergeFailure> StorageMergeTree::selectPartsToMerge(
    const StorageMetadataPtr &metadata_snapshot, bool aggressive, const String &partition_id, bool final,
    TableLockHolder & /* table_lock_holder */, std::unique_lock<std::mutex> &lock, const MergeTreeTransactionPtr &txn,
  ...
  const auto is_background_memory_usage_ok = []() -> std::expected<void, PreformattedMessage> {
    if (canEnqueueBackgroundTask()) return {};
  ...
  const auto construct_future_part = [&](MergeSelectorChoices choices) -> std::expected<FutureMergedMutatedPartPtr, SelectMergeFailure> {
    ...
    auto future_part = [&]() {
      if (txn != nullptr) return constructFuturePart(*this, choice, {MergeTreeDataPartState::Active, MergeTreeDataPartState::Outdated});
    ...
    if ((*getSettings())[MergeTreeSetting::assign_part_uuids]) future_part->uuid = UUIDHelpers::generateV4();
  ...
  const auto select_without_hint = [&]() -> std::expected<FutureMergedMutatedPartPtr, SelectMergeFailure> {
    ...
    bool merge_with_ttl_allowed =
        getTotalMergesWithTTLInMergeList() < (*getSettings())[MergeTreeSetting::max_number_of_merges_with_ttl_in_pool];
    ...
    auto select_result =
        merger_mutator.selectPartsToMerge(parts_collector, merge_predicate,
  ...
  const auto select_in_partition = [&]() -> std::expected<FutureMergedMutatedPartPtr, SelectMergeFailure> {
    while (true) {
      ...
      auto select_result = merger_mutator.selectAllPartsToMergeWithinPartition(metadata_snapshot, parts_collector, merge_predicate,
                                                                               partition_id, final, optimize_skip_merged_partitions);
  ...
  const auto construct_merge_select_entry =
      [&](FutureMergedMutatedPartPtr future_part) -> std::expected<MergeMutateSelectedEntryPtr, SelectMergeFailure> {
    /// Account TTL merge here to avoid exceeding the max_number_of_merges_with_ttl_in_pool limit
    if (isTTLMergeType(future_part->merge_type)) getContext()->getMergeList().bookMergeWithTTL();
    ...
    try {
      ...
      if (isTTLMergeType(future_part->merge_type)) getContext()->getMergeList().cancelMergeWithTTL();
...
bool StorageMergeTree::merge(bool aggressive, const String &partition_id, bool final, bool deduplicate, const Names &deduplicate_by_columns,
                             bool cleanup, const MergeTreeTransactionPtr &txn, PreformattedMessage &out_disable_reason,
                             bool optimize_skip_merged_partitions) {
  ...
  auto merge_select_result = [&]() {
    ...
    return selectPartsToMerge(metadata_snapshot, aggressive, partition_id, final, table_lock_holder, lock, txn,
  ...
  if (merge_select_result.has_value()) {
    ...
    auto task = std::make_shared<MergePlainMergeTreeTask>(*this, metadata_snapshot, deduplicate, deduplicate_by_columns, cleanup,
                                                          merge_select_result.value(), table_lock_holder, f);
  ...
  if (error.reason == SelectMergeFailure::Reason::NOTHING_TO_MERGE) return true;
...
MergeMutateSelectedEntryPtr StorageMergeTree::selectPartsToMutate(
  ...
  if (current_mutations_by_version.empty()) return {};
  ...
  for (const auto &part : getDataPartsVectorForInternalUsage()) {
    if (currently_merging_mutating_parts.contains(part)) continue;
    ...
    if (merger_mutator.merges_blocker.isCancelledForPartition(part->info.getPartitionId())) continue;

    auto mutations_begin_it = current_mutations_by_version.upper_bound(part->info.getDataVersion());
    if (mutations_begin_it == mutations_end_it) continue;
    ...
    if (max_source_part_size < part->getBytesOnDisk()) {
      LOG_DEBUG(log, "Current max source part size for mutation is {} but part size {}. Will not mutate part {} yet", max_source_part_size,
                part->getBytesOnDisk(), part->name);
    ...
    if (!first_mutation_tid.isPrehistoric()) {
      ...
      if (!txn) {
        ...
        if (mutation_csn == Tx::UnknownCSN) {
          ...
          throw Exception(ErrorCodes::LOGICAL_ERROR,
                          "Cannot find transaction {} that has started mutation {} "
      ...
      if (!part->version.isVisible(Tx::MaxCommittedCSN, Tx::EmptyTID)) {
        LOG_DEBUG(log,
                  "Cannot mutate part {} because it's not visible (outdated, being created or removed "
    ...
    for (auto it = mutations_begin_it; it != mutations_end_it; ++it) {
      ...
      if (first_mutation_tid != it->second.tid) break;
      ...
      for (const auto &command : *it->second.commands) {
        if (command.type != MutationCommand::Type::DROP_COLUMN && command.type != MutationCommand::Type::DROP_INDEX &&
            command.type != MutationCommand::Type::DROP_PROJECTION && command.type != MutationCommand::Type::DROP_STATISTICS &&
      ...
      if (!commands_for_size_validation.empty()) {
        try {
          ...
          MutationsInterpreter interpreter(shared_from_this(), metadata_snapshot, commands_for_size_validation, fake_query_context,
                                           settings);
    ...
    if (!commands->empty()) {
      ...
      tagger = std::make_unique<CurrentlyMergingPartsTagger>(future_part, CompactionStatistics::estimateNeededDiskSpace({part}, false),
                                                             *this, metadata_snapshot, true);
...
UInt32 StorageMergeTree::getMaxLevelInBetween(const PartProperties &left, const PartProperties &right) const {
  ...
  for (auto it = begin++; it != end; ++it) {
    if (it == data_parts_by_info.end())
      throw Exception(ErrorCodes::LOGICAL_ERROR, "left and right parts in the wrong order, left part {}, right part {}. It's a bug",
...
bool StorageMergeTree::scheduleDataProcessingJob(BackgroundJobsAssignee &assignee) {
  if (shutdown_called) return false;
  ...
  bool has_mutations = false;
  {
    ...
    if (merger_mutator.merges_blocker.isCancelled()) return false;
    ...
    if (!merge_entry && !current_mutations_by_version.empty()) {
      ...
      if (!mutate_entry) LOG_TRACE(LogFrequencyLimiter(log.load(), 300), "Didn't start mutation: {}", out_reason.text);
    }
  ...
  auto is_cancelled = [&merges_blocker = merger_mutator.merges_blocker](const MergeMutateSelectedEntryPtr &entry) {
    if (entry->future_part) return merges_blocker.isCancelledForPartition(entry->future_part->part_info.getPartitionId());

    return merges_blocker.isCancelled();
  };

  if (merge_entry) {
    if (is_cancelled(merge_entry)) return false;

    bool cleanup = merge_entry->future_part->final && (*getSettings())[MergeTreeSetting::allow_experimental_replacing_merge_with_cleanup] &&
    ...
    auto task = std::make_shared<MergePlainMergeTreeTask>(*this, metadata_snapshot, /* deduplicate */ false, Names{}, cleanup, merge_entry,
                                                          shared_lock, common_assignee_trigger);
    ...
    if (!scheduled && isTTLMergeType(merge_entry->future_part->merge_type)) getContext()->getMergeList().cancelMergeWithTTL();

    fiu_do_on(FailPoints::storage_merge_tree_background_schedule_merge_fail, { scheduled = false; });

    return scheduled;
  }
  if (mutate_entry) {
    if (is_cancelled(mutate_entry)) return false;
    ...
    auto task =
        std::make_shared<MutatePlainMergeTreeTask>(*this, getInMemoryMetadataPtr(), mutate_entry, shared_lock, common_assignee_trigger);
...
UInt64 StorageMergeTree::getCurrentMutationVersion(UInt64 data_version,
                                                   std::unique_lock<std::mutex> & /*currently_processing_in_background_mutex_lock*/) const {
  auto it = current_mutations_by_version.upper_bound(data_version);
  if (it == current_mutations_by_version.begin()) return 0;
  --it;
  return it->first;
}

UInt64 StorageMergeTree::getNextMutationVersion(UInt64 data_version,
                                                std::unique_lock<std::mutex> & /* currently_processing_in_background_mutex_lock */) const {
  auto it = current_mutations_by_version.upper_bound(data_version);
  if (it == current_mutations_by_version.end()) return 0;
  return it->first;
}
...
size_t StorageMergeTree::clearOldMutations(bool truncate) {
  ...
  if (!truncate && !finished_mutations_to_keep) return 0;
  ...
  std::vector<MergeTreeMutationEntry> mutations_to_delete;
  {
    ...
    if (std::optional<Int64> min_version = getMinPartDataVersion()) end_it = current_mutations_by_version.upper_bound(*min_version);
    ...
    if (done_count <= finished_mutations_to_keep) return 0;
...
size_t StorageMergeTree::clearOldPartsFromFilesystem(bool force, bool with_pause_fail_point) {
  ...
  if (parts_to_remove.empty()) return 0;
  ...
  if (with_pause_fail_point) {
    ...
    // - after do operation like merge / optimize final (operations like drop part / drop partition / truncate do not fit here, they remove
    // old parts synchronously without timeout) All parts which are dropped in that operations are not removed until failpoint is released
    ...
    // if failpoint is enabled at that time, background thread could grab parts from those operations and remove them regardless enabled
    // failpoint
...
bool StorageMergeTree::optimize(const ASTPtr & /*query*/, const StorageMetadataPtr & /*metadata_snapshot*/, const ASTPtr &partition,
                                bool final, bool deduplicate, const Names &deduplicate_by_columns, bool cleanup, ContextPtr local_context) {
  assertNotReadonly();
  ...
  if (!partition && final) {
    ...
    for (const String &partition_id : partition_ids) {
      if (!merge(true, partition_id, true, deduplicate, deduplicate_by_columns, cleanup, txn, disable_reason,
    ...
    String partition_id;
    if (partition) partition_id = getPartitionIDFromQuery(partition, local_context);

    if (!merge(true, partition_id, final, deduplicate, deduplicate_by_columns, cleanup, txn, disable_reason,
               local_context->getSettingsRef()[Setting::optimize_skip_merged_partitions])) {
      constexpr auto message = "Cannot OPTIMIZE table: {}";
      LOG_INFO(log, message, disable_reason.text);
...
size_t countOccurrences(const StorageMergeTree::DataParts &haystack, const DataPartsVector &needle) {
  size_t total = 0;
  for (const auto &n : needle) total += haystack.count(n);

  return total;
}

auto getNameWithState(const auto &parts) {
  return std::views::transform(parts, [](const auto &p) { return p->getNameWithState(); });
}
...
ActionLock StorageMergeTree::stopMergesAndWaitForPartition(String partition_id) {
  ...
  /// Stop all merges and prevent new from starting, BUT unlike stopMergesAndWait(), only wait for the merges on small set of parts to
  /// finish.
  ...
  while (size_t still_merging = countOccurrences(currently_merging_mutating_parts, parts_to_wait)) {
    LOG_DEBUG(log,
              "StorageMergeTree::stopMergesAndWaitForPartition Waiting for currently running merges ({} {} parts are merging right now)",
...
MergeTreeDataPartPtr StorageMergeTree::outdatePart(MergeTreeTransaction *txn, const String &part_name, bool force,
  ...
  if (force) {
    ...
    if (!part) throw Exception(ErrorCodes::NO_SUCH_DATA_PART, "Part {} not found, won't try to drop it.", part_name);
  ...
  if (currently_merging_mutating_parts.contains(part)) return nullptr;
...
Strings getPartsNames(const FutureNewEmptyParts &parts) {
  Strings part_names;
  for (const auto &p : parts) part_names.push_back(p.part_name);
  return part_names;
}
...
std::pair<StorageMergeTree::MutableDataPartsVector, std::vector<scope_guard>> createEmptyDataParts(MergeTreeData &data,
                                                                                                   FutureNewEmptyParts &future_parts,
...
void StorageMergeTree::renameAndCommitEmptyParts(MutableDataPartsVector &new_parts, Transaction &transaction) {
    ...
    try {
      ...
      while (next_part_index < new_parts.size()) {
        ...
        if (covered_parts_by_one_part.size() > 1)
          LOG_INFO(log,
                   "Part {} covers {} parts instead of the expected 1."
      ...
      if (e.code() != ErrorCodes::PART_IS_TEMPORARILY_LOCKED) throw;

      if (Int64(watch.elapsedMilliseconds()) >= timeout_ms) throw;
  ...
  LOG_INFO(log, "Remove {} parts by covering them with empty {} parts. With txn {}.", covered_parts.size(), new_parts.size(),
           transaction.getTID());
  ...
  for (auto &part : covered_parts) part->remove_time.store(0, std::memory_order_relaxed);

  if (deduplication_log)
    for (const auto &part : covered_parts) deduplication_log->dropPart(part->info);
}
...
void StorageMergeTree::truncate(const ASTPtr &, const StorageMetadataPtr &, ContextPtr query_context, TableExclusiveLockHolder &) {
...

  {
    ...
    if (txn) {
      ...
      LOG_TEST(log, "Made {} empty parts in order to cover {} parts. Empty parts: {}, covered parts: {}. With txn {}", future_parts.size(),
               parts.size(), fmt::join(getPartsNames(future_parts), ", "), fmt::join(getPartsNames(parts), ", "), transaction.getTID());
      ...
      PartLog::addNewParts(query_context,
                           PartLog::createPartLogEntries(new_data_parts, watch.elapsed(), profile_events_scope.getSnapshot()));

      LOG_INFO(log, "Truncated table with {} parts by replacing them with new empty {} parts. With txn {}", parts.size(),
               future_parts.size(), transaction.getTID());
    }
  }
...
void StorageMergeTree::dropPart(const String &part_name, bool detach, ContextPtr query_context) {
  {
    ...
    auto txn = query_context->getCurrentTransaction();
    if (txn) {
      if (auto part = outdatePart(txn.get(), part_name, /*force=*/true)) dropPartsImpl({part}, detach);
    } else {
      MergeTreeData::Transaction transaction(*this, txn.get());
...

      {
        ...
        LOG_TEST(log, "Made {} empty parts in order to cover {} part. With txn {}", fmt::join(getPartsNames(future_parts), ", "),
                 fmt::join(getPartsNames({part}), ", "), transaction.getTID());
        ...
        LOG_INFO(log, "{} {} part by replacing it with new empty {} part. With txn {}", op, part->name, future_parts[0].part_name,
                 transaction.getTID());
      }
    }
  }
...
void StorageMergeTree::dropPartition(const ASTPtr &partition, bool detach, ContextPtr query_context) {
  {
    ...
    if (txn) {
      ...
      const auto *op = detach ? "Detached" : "Dropped";
      LOG_INFO(log, "{} partition with {} parts by replacing them with new empty {} parts. With txn {}", op, parts.size(),
               future_parts.size(), transaction.getTID());
    }
  }
...
void StorageMergeTree::dropPartsImpl(DataPartsVector &&parts_to_remove, bool detach) {
  ...
  if (deduplication_log) {
    for (const auto &part : parts_to_remove) deduplication_log->dropPart(part->info);
  }
...
PartitionCommandsResultInfo StorageMergeTree::attachPartition(const PartitionCommand &command, const StorageMetadataPtr &,
...
void StorageMergeTree::replacePartitionFrom(const StoragePtr &source_table, const ASTPtr &partition, bool replace,
                                            ContextPtr local_context) {
  assertNotReadonly();
  LOG_DEBUG(log, "StorageMergeTree::replacePartitionFrom\tsource_table: {}, replace: {}", source_table->getStorageID().getShortName(),
            replace);
  ...
  for (const DataPartPtr &src_part : src_parts) {
    if (is_all) partition_id = src_part->partition.getID(src_data);

    if (!canReplacePartition(src_part))
      throw Exception(ErrorCodes::BAD_ARGUMENTS, "Cannot replace partition '{}' because part '{}' has inconsistent granularity with table",
    ...
    if (replace) {
      ...
      auto [dst_part, part_lock] =
          cloneAndLoadDataPart(src_part, TMP_PREFIX, dst_part_info, my_metadata_snapshot, clone_params, local_context->getReadSettings(),
                               local_context->getWriteSettings(), !are_policies_partition_op_compatible /*must_on_same_disk*/);
      ...
      auto [dst_part, part_lock] =
          cloneAndLoadDataPart(src_part, TMP_PREFIX, dst_part_info, my_metadata_snapshot, clone_params, local_context->getReadSettings(),
                               local_context->getWriteSettings(), false /*must_on_same_disk*/);
      dst_parts.emplace_back(std::move(dst_part));
      dst_parts_locks.emplace_back(std::move(part_lock));
    }
  }
  ...
  if (!replace && dst_parts.empty()) return;
  ...
  try {
    {
      ...
      if (replace) removePartsInRangeFromWorkingSet(local_context->getCurrentTransaction().get(), drop_range, data_parts_lock);
    }
...
void StorageMergeTree::movePartitionToTable(const StoragePtr &dest_table, const ASTPtr &partition, ContextPtr local_context) {
  ...
  for (const DataPartPtr &src_part : src_parts) {
    if (!dest_table_storage->canReplacePartition(src_part))
      throw Exception(ErrorCodes::LOGICAL_ERROR, "Cannot move partition '{}' because part '{}' has inconsistent granularity with table",
                      partition_id, src_part->name);
    ...
    auto [dst_part, part_lock] = dest_table_storage->cloneAndLoadDataPart(
        src_part, TMP_PREFIX, dst_part_info, dest_metadata_snapshot, clone_params, local_context->getReadSettings(),
        local_context->getWriteSettings(), !are_policies_partition_op_compatible /*must_on_same_disk*/
    );
...
IStorage::DataValidationTasksPtr StorageMergeTree::getCheckTaskList(const std::variant<std::monostate, ASTPtr, String> &check_task_filter,
                                                                    ContextPtr local_context) {
    ...
    if (!part->getDataPartStorage().existsFile(checksums_path)) {
      try {
        auto calculated_checksums =
            checkDataPart(part, false, noop, /* is_cancelled */ [] { return false; }, /* throw_on_broken_projection */ true);
...
void StorageMergeTree::backupData(BackupEntriesCollector &backup_entries_collector, const String &data_path_in_backup,
  ...
  for (const auto &data_part : data_parts) min_data_version = std::min(min_data_version, data_part->info.getDataVersion() + 1);
...
MutationCommands StorageMergeTree::MutationsSnapshot::getOnFlyMutationCommandsForPart(const DataPartPtr &part) const {
  ...
  for (const auto &[mutation_version, commands] : mutations_by_version | std::views::reverse) {
    if (mutation_version <= part_data_version) break;
...
NameSet StorageMergeTree::MutationsSnapshot::getAllUpdatedColumns() const {
  ...
  if (!hasDataMutations()) return res;
...
MergeTreeData::MutationsSnapshotPtr StorageMergeTree::getMutationsSnapshot(const IMutationsSnapshot::Params &params) const {
  ...
  if (params.need_patch_parts) patch_parts = getPatchPartsVectorForInternalUsage();
...
PreparedSetsCachePtr StorageMergeTree::getPreparedSetsCache(Int64 mutation_id) {
  ...
  if (it != mutation_prepared_sets_cache.end()) {
    ...
    if (existing_set_cache) return existing_set_cache;
  }
...
void StorageMergeTree::assertNotReadonly() const {
  ...
  if ((*getSettings())[MergeTreeSetting::table_readonly]) throw Exception(ErrorCodes::TABLE_IS_READ_ONLY, "Table is in readonly mode");
}
...
void StorageMergeTree::waitForCommittingInsertsAndMutations(Int64 max_block_number, size_t timeout_ms) const {
  auto all_committed = [&] {
    for (const auto &block : committing_blocks) {
      ...
      if (block.op != CommittingBlock::Op::Update) return false;
    }
  ...
  if (!res)
    throw Exception(ErrorCodes::TIMEOUT_EXCEEDED, "Failed to wait ({} ms) for inserts and mutations to commit up to block number {}",
                    timeout_ms, max_block_number);
}
