...
HivePartitioningKeysAndValues parseHivePartitioningKeysAndValues(const String& path) {
  ...
  try {
    ...
    throw Exception(ErrorCodes::INCORRECT_DATA,
                    "Path '{}' to file with enabled hive-style partitioning contains duplicated partition key {} with different values, "
...
NamesAndTypesList extractHivePartitionColumnsFromPath(const ColumnsDescription& storage_columns, const std::string& sample_path,
  ...
  for (const auto& item : hive_map) {
    ...
    if (storage_columns.has(key)) {
      ...
      if (const auto type = tryInferDataTypeByEscapingRule(value, format_settings ? *format_settings : getFormatSettings(context),
                                                           FormatSettings::EscapingRule::Raw)) {
...
void addPartitionColumnsToChunk(Chunk& chunk, const NamesAndTypesList& hive_partition_columns_to_read_from_file_path,
  ...
  for (const auto& column : hive_partition_columns_to_read_from_file_path) {
    ...
    auto chunk_column = column.type->createColumnConst(chunk.getNumRows(), convertFieldToType(Field(it->second), *column.type))
                            ->convertToFullColumnIfConst();
    chunk.addColumn(std::move(chunk_column));
  }
}

void sanityCheckSchemaAndHivePartitionColumns(const NamesAndTypesList& hive_partition_columns_to_read_from_file_path,
                                              const ColumnsDescription& storage_columns, bool check_contained_in_schema) {
...
NamesAndTypesList extractPartitionColumnsFromPathAndEnrichStorageColumns(ColumnsDescription& storage_columns, const std::string& path,
...
HivePartitionColumnsWithFileColumnsPair setupHivePartitioningForObjectStorage(ColumnsDescription& columns,
  ...
  if (configuration->partition_strategy && configuration->partition_strategy_type == PartitionStrategyFactory::StrategyType::HIVE) {
    ...
    hive_partition_columns_to_read_from_file_path =
        extractPartitionColumnsFromPathAndEnrichStorageColumns(columns, sample_path, inferred_schema, format_settings, context);
