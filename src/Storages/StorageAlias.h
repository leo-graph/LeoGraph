...
class StorageAlias final : public IStorage, WithContext {
 public:
  ...
  StorageAlias(const StorageID& table_id_, ContextPtr context_, const String& target_database_, const String& target_table_);
  ...
  StoragePtr tryGetTargetTable() const {
    return DatabaseCatalog::instance().tryGetTable(StorageID(target_database, target_table), getContext());
  }
  ...
  SinkToStoragePtr write(const ASTPtr& query, const StorageMetadataPtr& metadata_snapshot, ContextPtr local_context,
  ...
  void checkAlterIsPossible(const AlterCommands& commands, ContextPtr local_context) const override {
    getTargetTable()->checkAlterIsPossible(commands, local_context);
  }
  ...
  bool optimize(const ASTPtr& query, const StorageMetadataPtr& metadata_snapshot, const ASTPtr& partition, bool final, bool deduplicate,
                const Names& deduplicate_by_columns, bool cleanup, ContextPtr local_context) override;

  /// Alter partition on target table
  Pipe alterPartition(const StorageMetadataPtr& metadata_snapshot, const PartitionCommands& commands, ContextPtr local_context) override;
  ...
  /// Mutate target table
  void mutate(const MutationCommands& commands, ContextPtr local_context) override;
  ...
  bool supportsTrivialCountOptimization(const StorageSnapshotPtr& storage_snapshot, ContextPtr query_context) const override {
    return getTargetTable()->supportsTrivialCountOptimization(storage_snapshot, query_context);
  }
  ...
  QueryProcessingStage::Enum getQueryProcessingStage(ContextPtr local_context, QueryProcessingStage::Enum to_stage,
  ...
  TableLockHolder lockForShare(const String& query_id, const std::chrono::milliseconds& acquire_timeout) const {
    return getTargetTable()->lockForShare(query_id, Poco::Timespan(acquire_timeout.count() * 1000));
  }
  ...
  std::optional<UInt64> totalBytesUncompressed(const Settings& settings) const override {
    return getTargetTable()->totalBytesUncompressed(settings);
  }
