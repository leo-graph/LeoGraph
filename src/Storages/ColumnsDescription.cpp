...
ColumnDescription::ColumnDescription(String name_, DataTypePtr type_) : name(std::move(name_)), type(std::move(type_)) {}
...
bool ColumnDescription::operator==(const ColumnDescription &other) const {
  ...
  return name == other.name && type->equals(*other.type) && default_desc == other.default_desc && statistics == other.statistics &&
         ast_to_str(codec) == ast_to_str(other.codec) && settings == other.settings && ast_to_str(ttl) == ast_to_str(other.ttl);
}
...
void ColumnDescription::readText(ReadBuffer &buf) {
  ...
  if (checkChar('\t', buf)) {
    ...
    if (auto *col_ast = ast->as<ASTColumnDeclaration>()) {
      ...
      if (auto col_comment = col_ast->getComment()) comment = col_comment->as<ASTLiteral &>().value.safeGet<String>();
      ...
      if (auto col_ttl = col_ast->getTTL()) ttl = col_ttl;

      if (auto col_settings = col_ast->getSettings()) settings = col_settings->as<ASTSetQuery &>().changes;
...
ColumnsDescription::ColumnsDescription(std::initializer_list<ColumnDescription> ordinary) {
  for (auto &&elem : ordinary) add(elem);
}

ColumnsDescription ColumnsDescription::fromNamesAndTypes(NamesAndTypes ordinary) {
  ColumnsDescription result;
  for (auto &elem : ordinary) result.add(ColumnDescription(std::move(elem.name), std::move(elem.type)));
  return result;
}

ColumnsDescription::ColumnsDescription(NamesAndTypesList ordinary, bool with_subcolumns) {
  for (auto &elem : ordinary) add(ColumnDescription(std::move(elem.name), std::move(elem.type)), String(), false, with_subcolumns);
}

ColumnsDescription::ColumnsDescription(NamesAndTypesList ordinary, NamesAndAliases aliases) {
  for (auto &elem : ordinary) add(ColumnDescription(std::move(elem.name), std::move(elem.type)));
...
void ColumnsDescription::setAliases(NamesAndAliases aliases) {
  for (auto &alias : aliases) {
    ...
    description.default_desc.expression = parseQuery(expression_parser, alias_expression_pos, alias_expression_end, "expression", 0,
...
static auto getNameRange(const ColumnsDescription::ColumnsContainer &columns, const String &name_without_dot) {
  ...
  for (auto it = columns.begin(); it != columns.end(); ++it) {
    if (it->name == name_without_dot) return std::make_pair(it, std::next(it));
  }

  auto begin = std::find_if(columns.begin(), columns.end(), [&](const auto &column) { return startsWith(column.name, name_with_dot); });

  if (begin == columns.end()) return std::make_pair(begin, begin);

  auto end = std::next(begin);
  for (; end != columns.end(); ++end) {
    if (!startsWith(end->name, name_with_dot)) break;
  }
...
void ColumnsDescription::add(ColumnDescription column, const String &after_column, bool first, bool add_subcolumns) {
  if (has(column.name))
    throw Exception(ErrorCodes::ILLEGAL_COLUMN, "Cannot add column {}: column with this name already exists", column.name);

  /// Normalize ASTs to be compatible with InterpreterCreateQuery.
  if (column.default_desc.expression) FunctionNameNormalizer::visit(column.default_desc.expression.get());
  if (column.ttl) FunctionNameNormalizer::visit(column.ttl.get());
  ...
  if (add_subcolumns && column.default_desc.kind != ColumnDefaultKind::Alias) addSubcolumns(column.name, column.type);
...
void ColumnsDescription::rename(const String &column_from, const String &column_to) {
  ...
  if (it == columns.get<1>().end()) {
    throw Exception(ErrorCodes::LOGICAL_ERROR, "Cannot find column {} in ColumnsDescription{}", column_from, getHintsMessage(column_from));
  }

  columns.get<1>().modify_key(it, [&column_to](String &old_name) { old_name = column_to; });
}
...
NamesAndTypesList ColumnsDescription::getOrdinary() const {
  ...
  for (const auto &col : columns)
    if (col.default_desc.kind == ColumnDefaultKind::Default) ret.emplace_back(col.name, col.type);
  return ret;
}
...
NamesAndTypesList ColumnsDescription::getMaterialized() const {
  NamesAndTypesList ret;
  for (const auto &col : columns)
    if (col.default_desc.kind == ColumnDefaultKind::Materialized) ret.emplace_back(col.name, col.type);
  return ret;
}
...
NamesAndTypesList ColumnsDescription::getAliases() const {
  NamesAndTypesList ret;
  for (const auto &col : columns)
    if (col.default_desc.kind == ColumnDefaultKind::Alias) ret.emplace_back(col.name, col.type);
  return ret;
}
...
NamesAndTypesList ColumnsDescription::getEphemeral() const {
  NamesAndTypesList ret;
  for (const auto &col : columns)
    if (col.default_desc.kind == ColumnDefaultKind::Ephemeral) ret.emplace_back(col.name, col.type);
  return ret;
}

NamesAndTypesList ColumnsDescription::getAll() const {
  NamesAndTypesList ret;
  for (const auto &col : columns) ret.emplace_back(col.name, col.type);
  return ret;
}
...
NamesAndTypesList ColumnsDescription::getNested(const String &column_name) const {
  ...
  for (auto &it = range.first; it != range.second; ++it) nested.emplace_back(it->name, it->type);
  return nested;
}
...
void ColumnsDescription::addSubcolumnsToList(NamesAndTypesList &source_list) const {
  ...
  for (const auto &col : source_list) {
    ...
    if (it != columns.get<1>().end() && it->default_desc.kind == ColumnDefaultKind::Ephemeral) continue;

    auto range = subcolumns.get<1>().equal_range(col.name);
    if (range.first != range.second) subcolumns_list.insert(subcolumns_list.end(), range.first, range.second);
  }
...
NamesAndTypesList ColumnsDescription::get(const GetColumnsOptions &options) const {
  ...
  if (options.with_subcolumns) addSubcolumnsToList(res);

  return res;
}

bool ColumnsDescription::has(const String &column_name) const { return columns.get<1>().find(column_name) != columns.get<1>().end(); }
...
bool ColumnsDescription::hasSubcolumn(GetColumnsOptions::Kind kind, const String &column_name) const {
  ...
  if (tryGetDynamicSubcolumn(column_name, kind)) return true;
...
const ColumnDescription &ColumnsDescription::get(const String &column_name) const {
  ...
  if (it == columns.get<1>().end()) throw Exception(ErrorCodes::NO_SUCH_COLUMN_IN_TABLE, "There is no column {} in table.", column_name);
...
NamesAndTypesList ColumnsDescription::getByNames(const GetColumnsOptions &options, const Names &names) const {
  NamesAndTypesList res;
  for (const auto &name : names) res.push_back(getColumn(options, name));
  return res;
}
...
std::optional<NameAndTypePair> ColumnsDescription::tryGetColumn(const GetColumnsOptions &options, const String &column_name) const {
  ...
  if (options.with_subcolumns) {
    ...
    if (jt != subcolumns.get<0>().end() &&
        (defaultKindToGetKind(columns.get<1>().find(jt->getNameInStorage())->default_desc.kind) & options.kind))
      return *jt;

    if (options.with_dynamic_subcolumns) {
      /// Check for dynamic subcolumns.
      if (auto dynamic_subcolumn = tryGetDynamicSubcolumn(column_name, options)) return dynamic_subcolumn;
    }
  }
...
NameAndTypePair ColumnsDescription::getColumn(const GetColumnsOptions &options, const String &column_name) const {
  ...
  if (!column) throw Exception(ErrorCodes::NO_SUCH_COLUMN_IN_TABLE, "There is no column {} in table.", column_name);
...
std::optional<const ColumnDescription> ColumnsDescription::tryGetColumnDescription(const GetColumnsOptions &options,
                                                                                   const String &column_name) const {
  auto it = columns.get<1>().find(column_name);
  if (it != columns.get<1>().end() && (defaultKindToGetKind(it->default_desc.kind) & options.kind)) return *it;
...
std::optional<const ColumnDescription> ColumnsDescription::tryGetColumnOrSubcolumnDescription(GetColumnsOptions::Kind kind,
                                                                                              const String &column_name) const {
...
NameAndTypePair ColumnsDescription::getColumnOrSubcolumn(GetColumnsOptions::Kind kind, const String &column_name) const {
  ...
  if (!column) throw Exception(ErrorCodes::NO_SUCH_COLUMN_IN_TABLE, "There is no column or subcolumn {} in table.", column_name);
...
NameAndTypePair ColumnsDescription::getPhysical(const String &column_name) const {
  ...
  if (!column) throw Exception(ErrorCodes::NO_SUCH_COLUMN_IN_TABLE, "There is no physical column {} in table.", column_name);
...
bool ColumnsDescription::hasColumnOrNested(GetColumnsOptions::Kind kind, const String &column_name) const {
  auto range = getNameRange(columns, column_name);
  return range.first != range.second && defaultKindToGetKind(range.first->default_desc.kind) & kind;
}

bool ColumnsDescription::hasDefaults() const {
  for (const auto &column : columns)
    if (column.default_desc.expression) return true;
  return false;
}

bool ColumnsDescription::hasOnlyOrdinary() const {
  for (const auto &column : columns)
    if (column.default_desc.kind != ColumnDefaultKind::Default) return false;
  return true;
}
...
ColumnDefaults ColumnsDescription::getDefaults() const {
  ColumnDefaults ret;
  for (const auto &column : columns)
    if (column.default_desc.expression) ret.emplace(column.name, column.default_desc);
...
std::optional<ColumnDefault> ColumnsDescription::getDefault(const String &column_name) const {
  auto it = columns.get<1>().find(column_name);
  if (it != columns.get<1>().end() && it->default_desc.expression) return it->default_desc;
...
ColumnsDescription::ColumnTTLs ColumnsDescription::getColumnTTLs() const {
  ColumnTTLs ret;
  for (const auto &column : columns)
    if (column.ttl) ret.emplace(column.name, column.ttl);
  return ret;
}
...
void ColumnsDescription::resetColumnTTLs() {
  ...
  for (const auto &col : columns) old_columns.emplace_back(col);
...
String ColumnsDescription::toString(bool include_comments) const {
  ...
  for (const ColumnDescription &column : columns) column.writeText(buf, ast_format_state, include_comments);
...
void ColumnsDescription::removeSubcolumns(const String &name_in_storage) {
  auto range = subcolumns.get<1>().equal_range(name_in_storage);
  if (range.first != range.second) subcolumns.get<1>().erase(range.first, range.second);
}
...
std::vector<String> ColumnsDescription::getAllRegisteredNames() const {
  ...
  for (const auto &column : columns) {
    if (!column.name.contains('.')) names.push_back(column.name);
  }
  return names;
}

std::optional<NameAndTypePair> ColumnsDescription::tryGetDynamicSubcolumn(const String &column_name,
                                                                          const GetColumnsOptions &options) const {
...
void getDefaultExpressionInfoInto(const ASTColumnDeclaration &col_decl, const DataTypePtr &data_type, DefaultExpressionsInfo &info) {
  ...
  if (!col_default_expression) return;
...
void assertNoMatcherNodes(const QueryTreeNodePtr &node, const String &context_description) {
  ...
  for (const auto &child : node->getChildren()) {
    if (child) assertNoMatcherNodes(child, context_description);
  }
}
...
void collectAliasDependenciesFromAST(const ASTPtr &node, const NameSet &candidate_names, NameSet &dependencies) {
  ...
  if (const auto *identifier = node->as<ASTIdentifier>()) {
    ...
    if (candidate_names.contains(column_name)) dependencies.insert(column_name);
    return;
  }

  for (const auto &child : node->children)
    if (child) collectAliasDependenciesFromAST(child, candidate_names, dependencies);
}
 ...
 */
void detectRecursiveDefaultCycles(const ASTPtr &expression_list, const NameSet &default_column_names) {
  if (!expression_list || default_column_names.empty()) return;

  const auto *list_node = expression_list->as<ASTExpressionList>();
  if (!list_node) throw Exception(ErrorCodes::LOGICAL_ERROR, "Default expression list node is expected to be a list node");
  ...
  for (const auto &child : list_node->children) {
    ...
    if (alias.empty()) alias = child->getColumnName();
  ...
  std::function<void(const String &)> dfs = [&](const String &vertex) {
    ...
    if (auto it = alias_to_expression.find(vertex); it != alias_to_expression.end()) {
      ...
      for (const auto &dependency : dependencies) dfs(dependency);
    }
  ...
  for (const auto &column_name : default_column_names) {
    if (!alias_names.contains(column_name)) continue;

    if (colors[column_name] != Color::Black) dfs(column_name);
  }
}

std::optional<Block> validateDefaultsWithAnalyzer(ASTPtr default_expr_list, const NamesAndTypesList &all_columns, ContextPtr context,
                                                  bool get_sample_block) {
  if (!default_expr_list || default_expr_list->children.empty()) {
    if (!get_sample_block) return {};
    return Block{};
  }
  ...
  for (const auto &column : all_columns) table_column_names.insert(column.name);
  ...
  for (const auto &child : default_expr_list->children) {
    ...
    String alias = child->tryGetAlias();
    if (alias.empty()) alias = child->getColumnName();
    if (!alias.empty() && table_column_names.contains(alias)) default_column_names.insert(alias);
  }
  ...
  auto [actions, _] = buildActionsDAGFromExpressionNode(expression_list, {}, planner_context, empty_correlated_columns_set);

  // Get all projected column names from actions dag and rename/alias each output result to match the original column names in the sample
  // output block.
  ...
  for (size_t i = 0; i != outputs.size(); ++i) rename_pairs.emplace_back(outputs[i]->result_name, projection_columns[i].name);
  ...
  Block result_block;
  for (const auto *output : outputs) result_block.insert(ColumnWithTypeAndName{output->column, output->result_type, output->result_name});

  return result_block;
}

std::optional<Block> validateColumnsDefaultsAndGetSampleBlockImpl(ASTPtr default_expr_list, const NamesAndTypesList &all_columns,
                                                                  ContextPtr context, bool get_sample_block) {
  if (!default_expr_list || default_expr_list->children.empty()) {
    if (!get_sample_block) return {};
    return Block{};
  }
  ...
  try {
    if (context->getSettingsRef()[Setting::allow_experimental_analyzer])
      ...
      auto syntax_analyzer_result =
          TreeRewriter(context).analyze(default_expr_list, all_columns, {}, {}, false, /* allow_self_aliases = */ false);
