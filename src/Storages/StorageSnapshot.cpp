...
StorageSnapshot::StorageSnapshot(const IStorage& storage_, StorageMetadataPtr metadata_)
    : storage(storage_), metadata(std::move(metadata_)), virtual_columns(storage_.getVirtualsPtr()) {}

StorageSnapshot::StorageSnapshot(const IStorage& storage_, StorageMetadataPtr metadata_, VirtualsDescriptionPtr virtual_columns_)
    : storage(storage_), metadata(std::move(metadata_)), virtual_columns(std::move(virtual_columns_)) {}

StorageSnapshot::StorageSnapshot(const IStorage& storage_, StorageMetadataPtr metadata_, DataPtr data_)
    : storage(storage_), metadata(std::move(metadata_)), virtual_columns(storage_.getVirtualsPtr()), data(std::move(data_)) {}

StorageSnapshot::StorageSnapshot(const IStorage& storage_, StorageMetadataPtr metadata_, VirtualsDescriptionPtr virtual_columns_,
                                 DataPtr data_)
    : storage(storage_), metadata(std::move(metadata_)), virtual_columns(std::move(virtual_columns_)), data(std::move(data_)) {}
...
std::optional<NameAndTypePair> StorageSnapshot::tryGetColumn(const GetColumnsOptions& options, const String& column_name) const {
  ...
  if (auto column = columns.tryGetColumn(options, column_name)) return column;

  if (options.virtuals_kind != VirtualsKind::None) {
    auto virtual_column = virtual_columns->tryGet(column_name, options.virtuals_kind);
    if (virtual_column) return NameAndTypePair{virtual_column->name, virtual_column->type};

    const auto& common_virtual_columns = IStorage::getCommonVirtuals();
    auto common_virtual_column = common_virtual_columns.tryGet(column_name, options.virtuals_kind);
    if (common_virtual_column) return NameAndTypePair{common_virtual_column->name, common_virtual_column->type};
  }
...
Block StorageSnapshot::getSampleBlockForColumns(const Names& column_names) const {
  ...
  for (const auto& column_name : column_names) {
    ...
    if (column) {
      ...
      throw Exception(ErrorCodes::NOT_FOUND_COLUMN_IN_BLOCK, "Column {} not found in table {}", backQuote(column_name),
...
ColumnsDescription StorageSnapshot::getDescriptionForColumns(const Names& column_names) const {
  ...
  for (const auto& name : column_names) {
    ...
    if (column) {
      ...
      throw Exception(ErrorCodes::NOT_FOUND_COLUMN_IN_BLOCK, "Column {} not found in table {}", backQuote(name),
                      storage.getStorageID().getNameForLogs());
    }
  }
...
void StorageSnapshot::check(const Names& column_names) const {
  ...
  if (column_names.empty()) {
    ...
    throw Exception(ErrorCodes::EMPTY_LIST_OF_COLUMNS_QUERIED, "Empty list of columns queried. There are columns: {}", list_of_columns);
  }
  ...
  for (const auto& name : column_names) {
    bool has_column = columns.hasColumnOrSubcolumn(GetColumnsOptions::AllPhysical, name) || virtual_columns->has(name) ||
                      common_virtual_columns.has(name);

    if (!has_column) {
      auto list_of_columns = listOfColumns(columns.get(options));
      throw Exception(ErrorCodes::NO_SUCH_COLUMN_IN_TABLE, "There is no column with name {} in table {}. There are columns: {}",
                      backQuote(name), storage.getStorageID().getNameForLogs(), list_of_columns);
    }

    if (unique_names.count(name)) throw Exception(ErrorCodes::COLUMN_QUERIED_MORE_THAN_ONCE, "Column {} queried more than once", name);
...
std::optional<ColumnDefault> StorageSnapshot::getDefault(const String& column_name) const {
  if (auto column_default = metadata->getColumns().getDefault(column_name)) return column_default;

  if (virtual_columns) return virtual_columns->getDefault(column_name);
