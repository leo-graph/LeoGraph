...
TableZnodeInfo TableZnodeInfo::resolve(const String& requested_path, const String& requested_replica_name, const StorageID& table_id,
  ...
  res.path = zkutil::extractZooKeeperPath(res.full_path, /* check_starts_with_slash */ mode <= LoadingStrictnessLevel::CREATE,
                                          getLogger(table_id.getNameForLogs()));
  ...
  if (expanded_uuid_in_path) {
    ...
    if (i == String::npos)
      ...
      throw Exception(ErrorCodes::LOGICAL_ERROR, "Can't find uuid in zookeeper path after expanding {{uuid}} macro: {} (uuid {})", res.path,
                      uuid_str);
    i += uuid_str.size();
    /// In case the path is "/foo/pika{uuid}chu/bar" (or "/foo/{uuid}{replica}/bar").
    while (i < res.path.size() && res.path[i] != '/') i += 1;
...
void TableZnodeInfo::dropAncestorZnodesIfNeeded(const zkutil::ZooKeeperPtr& zookeeper) const {
  ...
  if (path_prefix_for_drop.empty() || path_prefix_for_drop.size() == path.size()) return;
