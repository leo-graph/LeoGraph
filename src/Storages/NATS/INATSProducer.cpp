...
void INATSProducer::finishImpl() {
  try {
    if (connection) {
      if (connection->isConnected()) natsConnection_Flush(connection->getConnection());
...
void INATSProducer::produce(const String &message, size_t, const Columns &, size_t) {
  if (!payloads.push(message)) throw Exception(ErrorCodes::INVALID_STATE, "Could not push to payloads queue");
}
...
void INATSProducer::publish() {
  ...
  while (!payloads.empty()) {
    if (!connection->isConnected() || natsConnection_Buffered(connection->getConnection()) > MAX_BUFFERED) break;
    bool pop_result = payloads.pop(payload);

    if (!pop_result) throw Exception(ErrorCodes::LOGICAL_ERROR, "Could not pop payload");
    ...
    if (status != NATS_OK) {
      ...
      if (!payloads.pushFront(payload)) throw Exception(ErrorCodes::INVALID_STATE, "Could not push to payloads queue");
      break;
    }
  }
}

void INATSProducer::stopProducingTask() { payloads.finish(); }
