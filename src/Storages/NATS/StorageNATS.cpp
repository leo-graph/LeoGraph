...
StorageNATS::StorageNATS(const StorageID &table_id_, ContextPtr context_, const ColumnsDescription &columns_, const String &comment,
                         std::unique_ptr<NATSSettings> nats_settings_, LoadingStrictnessLevel mode)
    : IStorage(table_id_),
  ...
  configuration = {.url = getContext()->getMacros()->expand((*nats_settings)[NATSSetting::nats_url]),
                   ...
                   .credential_file = nats_credential_file.empty() ? getContext()->getConfigRef().getString("nats.credential_file", "")
  ...
  streaming_task =
      getContext()->getMessageBrokerSchedulePool().createTask(getStorageID(), "NATSStreamingTask", [this] { streamingToViewsFunc(); });
  streaming_task->deactivate();

  initialize_consumers_task = getContext()->getMessageBrokerSchedulePool().createTask(getStorageID(), "NATSInitializeConsumersTask",
                                                                                      [this] { initializeConsumersFunc(); });
  initialize_consumers_task->deactivate();
}
StorageNATS::~StorageNATS() { stopEventLoop(); }
...
void StorageNATS::createConsumersConnection() {
  if (consumers_connection) return;
...
void StorageNATS::createConsumers() {
  if (num_created_consumers != 0) return;
...
bool StorageNATS::subscribeConsumers() {
  ...
  if (are_consumers_initialized) consumers_ready.store(true);
...
void StorageNATS::unsubscribeConsumers() {
  std::lock_guard lock(consumers_mutex);
  for (auto &consumer : consumers) consumer->unsubscribe();
...
void StorageNATS::read(QueryPlan &query_plan, const Names &column_names, const StorageSnapshotPtr &storage_snapshot,
  ...
  if (mv_attached) throw Exception(ErrorCodes::QUERY_NOT_ALLOWED, "Cannot read from StorageNATS with attached materialized views");
  ...
  for (size_t i = 0; i < num_created_consumers; ++i) {
    ...
    auto converting_dag =
        ActionsDAG::makeConvertingActions(nats_source->getPort().getHeader().getColumnsWithTypeAndName(),
...
SinkToStoragePtr StorageNATS::write(const ASTPtr &, const StorageMetadataPtr &metadata_snapshot, ContextPtr local_context,
  ...
  if (pos != std::string::npos || subject.back() == '>') throw Exception(ErrorCodes::BAD_ARGUMENTS, "Can not publish to wildcard subject");

  if (!isSubjectInSubscriptions(subject)) throw Exception(ErrorCodes::BAD_ARGUMENTS, "Selected subject is not among engine subjects");
  ...
  return std::make_shared<MessageQueueSink>(std::make_shared<const Block>(metadata_snapshot->getSampleBlockNonMaterialized()),
                                            getFormatName(), max_rows, createProducer(std::move(subject)), getName(), modified_context);
}

void StorageNATS::startup() { initialize_consumers_task->activateAndSchedule(); }
...
void StorageNATS::shutdown(bool /* is_drop */) {
  ...
  try {
    if (drop_table) unsubscribeConsumers();

    if (consumers_connection) {
      if (consumers_connection->isConnected()) natsConnection_Flush(consumers_connection->getConnection());
...
INATSConsumerPtr StorageNATS::popConsumer() { return popConsumer(std::chrono::milliseconds::zero()); }
...
INATSConsumerPtr StorageNATS::createConsumer() {
  ...
  return std::make_shared<NATSJetStreamConsumer>(consumers_connection, std::move(stream_name), getConsumerName(), subjects, queue_name, log,
                                                 queue_size, shutdown_called);
}
...
bool StorageNATS::isSubjectInSubscriptions(const std::string &subject) {
  ...
  for (const auto &nats_subject : subjects) {
    ...
    if (!nats_subject_levels.empty() && nats_subject_levels.back() == ">") levels_to_check = nats_subject_levels.size() - 1;
    if (levels_to_check) {
      if (subject_levels.size() < levels_to_check) continue;
    } else {
      if (subject_levels.size() != nats_subject_levels.size()) continue;
      levels_to_check = nats_subject_levels.size();
    }

    bool is_same = true;
    for (size_t i = 0; i < levels_to_check; ++i) {
      if (nats_subject_levels[i] == "*") continue;
...
bool StorageNATS::streamToViews() {
  ...
  for (size_t i = 0; i < num_created_consumers; ++i) {
    auto source = std::make_shared<NATSSource>(*this, storage_snapshot, new_context, column_names, block_size,
                                               (*nats_settings)[NATSSetting::nats_handle_error_mode]);
  ...
  for (auto &source : sources) {
    if (source->queueEmpty()) ++queue_empty;
  }
...
String StorageNATS::getStreamName() const { return getContext()->getMacros()->expand((*nats_settings)[NATSSetting::nats_stream]); }
String StorageNATS::getConsumerName() const { return getContext()->getMacros()->expand((*nats_settings)[NATSSetting::nats_consumer_name]); }

void registerStorageNATS(StorageFactory &factory) {
  auto creator_fn = [](const StorageFactory::Arguments &args) {
    auto nats_settings = std::make_unique<NATSSettings>();
    if (auto named_collection =
            tryGetNamedCollectionWithOverrides(args.engine_args, args.getLocalContext(), true, nullptr, &args.table_id)) {
  ...
  factory.registerStorage("NATS", creator_fn,
