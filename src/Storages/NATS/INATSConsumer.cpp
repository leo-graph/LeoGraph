...
INATSConsumer::INATSConsumer(NATSConnectionPtr connection_, const std::vector<String> &subjects_, const String &subscribe_queue_name,
                             LoggerPtr log_, uint32_t queue_size_, const std::atomic<bool> &stopped_)
...
bool INATSConsumer::isSubscribed() const { return !subscriptions.empty(); }
...
void INATSConsumer::onMsg(natsConnection *, natsSubscription *, natsMsg *msg, void *consumer) {
  ...
  if (msg_length) {
    ...
    if (!nats_consumer->received.push(std::move(data))) throw Exception(ErrorCodes::INVALID_STATE, "Could not push to received queue");
  }
