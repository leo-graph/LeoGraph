...
NATSConnection::NATSConnection(const NATSConfiguration &configuration_, LoggerPtr log_, NATSOptionsPtr options_)
    : configuration(configuration_), log(std::move(log_)), options(std::move(options_)), connection(nullptr, &natsConnection_Destroy) {
  ...
  if (!configuration.token.empty()) natsOptions_SetToken(options.get(), configuration.token.c_str());
  ...
  const char *val = std::getenv("CLICKHOUSE_NATS_TLS_SECURE");  // NOLINT(concurrency-mt-unsafe) // this is safe on Linux glibc/Musl, but
                                                                // potentially not safe on other platforms
...
bool NATSConnection::isConnectedImpl(const Lock &) const {
  return connection &&
         (natsConnection_Status(connection.get()) == NATS_CONN_STATUS_CONNECTED || natsConnection_IsDraining(connection.get()));
}

bool NATSConnection::isDisconnectedImpl(const Lock &) const {
  return !connection ||
         (natsConnection_Status(connection.get()) == NATS_CONN_STATUS_DISCONNECTED || natsConnection_IsClosed(connection.get()));
}

bool NATSConnection::isClosedImpl(const Lock &) const { return !connection || natsConnection_IsClosed(connection.get()); }
...
void NATSConnection::connectImpl(const Lock &) {
  ...
  if (status != NATS_OK) {
    LOG_DEBUG(log, "New connection to {} failed. Nats status text: {}. Last error message: {}", connectionInfoForLog(),
...
void NATSConnection::disconnectImpl(const Lock &connection_lock) {
  if (isDisconnectedImpl(connection_lock)) return;
