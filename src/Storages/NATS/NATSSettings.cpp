...
#define NATS_RELATED_SETTINGS(DECLARE, ALIAS)                                                                                          \
  ...
  DECLARE(StreamingHandleErrorMode, nats_handle_error_mode, StreamingHandleErrorMode::DEFAULT,                                         \
          "How to handle errors for NATS engine. Possible values: default (throw an exception after nats_skip_broken_messages broken " \
...
SettingsChanges NATSSettings::getFormatSettings() const {
  ...
  for (const auto& setting : *impl) {
    ...
    if (!setting_name.starts_with("nats_")) values.emplace_back(setting_name, setting.getValue());
  }

  return values;
}

bool NATSSettings::hasBuiltin(std::string_view name) { return NATSSettingsImpl::hasBuiltin(name); }
}  // namespace DB
