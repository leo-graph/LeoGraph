...
NATSJetStreamConsumer::NATSJetStreamConsumer(NATSConnectionPtr connection, String stream_name_, String consumer_name_,
                                             const std::vector<String> &subjects, const String &subscribe_queue_name, LoggerPtr log,
                                             uint32_t queue_size, const std::atomic<bool> &stopped)
...
void NATSJetStreamConsumer::subscribe() {
  ...
  if (er != NATS_OK)
    throw Exception(ErrorCodes::CANNOT_CONNECT_NATS, "Failed to create NATS jet stream options for {}. Nats last error: {}",
  ...
  if (er != NATS_OK)
    throw Exception(ErrorCodes::CANNOT_CONNECT_NATS, "Failed to create NATS jet stream ctx for {}. Nats last error: {}",
                    getConnection()->connectionInfoForLog(), natsStatus_GetText(er));
  jet_stream_ctx.reset(new_jet_stream_ctx);

  er = jsSubOptions_Init(&subscribe_options);
  if (er != NATS_OK)
    throw Exception(ErrorCodes::CANNOT_CONNECT_NATS, "Failed to create NATS jet stream subscribe options for {}. Error: {}",
                    getConnection()->connectionInfoForLog(), natsStatus_GetText(er));

  subscribe_options.Stream = stream_name.c_str();
  subscribe_options.Consumer = consumer_name.c_str();

  if (!getQueueName().empty()) subscribe_options.Queue = getQueueName().c_str();
...
NATSSubscriptionPtr NATSJetStreamConsumer::subscribeToSubject(const String &subject) {
  ...
  auto status = js_PullSubscribeAsync(&subscription, jet_stream_ctx.get(), subject.c_str(), consumer_name.c_str(), onMsg,
                                      static_cast<void *>(this), &jet_stream_options, &subscribe_options, nullptr);
  if (status != NATS_OK)
    throw Exception(ErrorCodes::CANNOT_CONNECT_NATS, "Failed to subscribe consumer {} to subject {}. Error: {} {}",
                    static_cast<void *>(this), subject, natsStatus_GetText(status), nats_GetLastError(nullptr));
