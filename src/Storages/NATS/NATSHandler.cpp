...
NATSHandler::NATSHandler(LoggerPtr log_) : log(log_), loop_state(Loop::CREATED) {
...
std::future<NATSConnectionPtr> NATSHandler::createConnection(const NATSConfiguration &configuration) {
  ...
  post([this, &configuration, connect_promise = std::move(promise)]() {
    try {
      for (UInt64 i = 0; i < configuration.max_connect_tries; ++i) {
        ...
        if (!connection->connect()) {
          LOG_DEBUG(log, "Connect to {} attempt #{} failed, error: {}. Reconnecting...", connection->connectionInfoForLog(), i + 1,
...
NATSOptionsPtr NATSHandler::createOptions() {
  ...
  er = natsOptions_SetEventLoop(result.get(), static_cast<void *>(loop.getLoop()), natsLibuv_Attach, natsLibuv_Read, natsLibuv_Write,
                                natsLibuv_Detach);
  if (er) {
    throw Exception(ErrorCodes::CANNOT_CONNECT_NATS, "Can not set event loop. Nats error: {}", natsStatus_GetText(er));
  }
...
void NATSHandler::resetThreadLocalLoop() { uv_key_set(&uvLoopThreadKey, nullptr); }
