...
void checkExpressionDoesntContainSubqueries(const IAST& ast) {
  ...
  if (ast.as<ASTSubquery>()) throw Exception(ErrorCodes::BAD_ARGUMENTS, "Key expressions cannot contain subqueries");

  for (const auto& child : ast.children) checkExpressionDoesntContainSubqueries(*child);
}
