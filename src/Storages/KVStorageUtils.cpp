...
Field createTupleField(const std::vector<Field> &fields) {
  ...
  for (const auto &field : fields) tuple.push_back(field);
...
std::vector<Field> cartesianProduct(const std::vector<FieldVector> &column_values) {
  ...
  if (column_values.size() == 1) {
    ...
    for (const auto &val : column_values[0]) result.push_back(createTupleField({val}));
    return result;
  }
  ...
  while (true) {
    ...
    for (size_t i = 0; i < column_values.size(); ++i) tuple_values.push_back(column_values[i][indices[i]]);
    ...
    while (true) {
      ++indices[pos];
      if (indices[pos] < column_values[pos].size()) break;

      indices[pos] = 0;
      if (pos == 0) return result;  // All combinations generated
      --pos;
    }
  }
}
...
bool traverseDAGFilterSingleColumn(const std::string &primary_key, const DataTypePtr &primary_key_type, const ActionsDAG::Node *elem,
                                   const ContextPtr &context, FieldVectorPtr &res) {
  ...
  if (elem->type != ActionsDAG::ActionType::FUNCTION) return false;
  ...
  if (func_name == "and") {
    ...
    for (const auto *child : elem->children)
      if (traverseDAGFilterSingleColumn(primary_key, primary_key_type, child, context, res)) return true;
    return false;
  }
  if (func_name == "or") {
    // make sure every child has the key filter condition
    for (const auto *child : elem->children)
      if (!traverseDAGFilterSingleColumn(primary_key, primary_key_type, child, context, res)) return false;
    return true;
  }
  if (func_name == "equals") {
    if (elem->children.size() != 2) return false;

    const auto *key = elem->children.at(0);
    while (key->type == ActionsDAG::ActionType::ALIAS) key = key->children.at(0);

    if (key->type != ActionsDAG::ActionType::INPUT) return false;

    if (key->result_name != primary_key) return false;

    const auto *value = elem->children.at(1);
    if (value->type != ActionsDAG::ActionType::COLUMN) return false;

    auto converted_field = convertFieldToType((*value->column)[0], *primary_key_type);
    if (!converted_field.isNull()) res->push_back(converted_field);
    return true;
  }
  if (func_name == "in" || func_name == "globalIn") {
    ...
    if (const auto *col_const = typeid_cast<const ColumnConst *>(value_col)) value_col = &col_const->getDataColumn();

    const auto *col_set = typeid_cast<const ColumnSet *>(value_col);
    if (!col_set) return false;
    ...
    if (!set->hasExplicitSetElements()) return false;
    ...
    if (canBeSafelyCast(set_data_type_ptr, primary_key_type)) {
      ...
      for (size_t row = 0; row < set_column_ptr->size(); ++row) res->push_back(casted_set[row]);
      return true;
    } else {
...
bool traverseDAGFilter(const Names &primary_keys, const DataTypes &primary_key_types, const ActionsDAG::Node *elem,
                       const ContextPtr &context, FieldVectorPtr &res) {
  if (elem->type == ActionsDAG::ActionType::ALIAS)
  ...
  if (func_name == "equals") {
    ...
    // Skip aliases
    while (left->type == ActionsDAG::ActionType::ALIAS) left = left->children.at(0);
    ...
    if (left->type == ActionsDAG::ActionType::FUNCTION && left->function_base->getName() == "tuple") {
      ...
      if (left->children.size() != primary_keys.size()) return false;

      for (size_t i = 0; i < primary_keys.size(); ++i) {
        const auto *key_node = left->children.at(i);
        while (key_node->type == ActionsDAG::ActionType::ALIAS) key_node = key_node->children.at(0);

        if (key_node->type != ActionsDAG::ActionType::INPUT || key_node->result_name != primary_keys[i]) return false;
      }

      // Extract tuple value from right side
      if (right->type != ActionsDAG::ActionType::COLUMN) return false;

      const auto &value_field = (*right->column)[0];
      if (value_field.getType() != Field::Types::Tuple) return false;

      const auto &tuple_value = value_field.safeGet<Tuple>();
      if (tuple_value.size() != primary_keys.size()) return false;
  ...
  if (func_name == "in" || func_name == "globalIn") {
    ...
    if (left->type == ActionsDAG::ActionType::FUNCTION && left->function_base->getName() == "tuple") {
      ...
      auto set = future_set->get();
      if (!set || !set->hasExplicitSetElements()) return false;
      ...
      if (set_elements.empty()) return false;
      ...
      for (size_t row = 0; row < num_rows; ++row) {
        ...
        if (all_converted) res->push_back(createTupleField(tuple_values));
      }
  ...
  if (func_name == "and") {
    ...
    for (const auto *child : elem->children) {
      ...
      for (size_t i = 0; i < primary_keys.size(); ++i) {
        ...
        if (traverseDAGFilterSingleColumn(primary_keys[i], primary_key_types[i], child, context, child_res)) {
          if (!child_res->empty()) {
            for (const auto &val : *child_res) column_filters[primary_keys[i]].push_back(val);
          }
        }
      }
    }
    ...
    if (column_filters.size() != primary_keys.size()) return false;
    ...
    for (const auto &key_name : primary_keys) {
      ...
      if (it == column_filters.end() || it->second.empty()) return false;
      ordered_filters.push_back(it->second);
    }

    // Compute Cartesian product
    auto product = cartesianProduct(ordered_filters);
    for (const auto &tuple_field : product) res->push_back(tuple_field);
  ...
  if (func_name == "or") {
    for (const auto *child : elem->children) {
      if (!traverseDAGFilter(primary_keys, primary_key_types, child, context, res)) return false;
    }
    return true;
  }
...
std::pair<FieldVectorPtr, bool> getFilterKeys(const String &primary_key, const DataTypePtr &primary_key_type,
                                              const ActionsDAG *filter_actions_dag, const ContextPtr &context) {
  if (!filter_actions_dag) return {{}, true};
...
std::pair<FieldVectorPtr, bool> getFilterKeys(const Names &primary_keys, const DataTypes &primary_key_types,
                                              const ActionsDAG *filter_actions_dag, const ContextPtr &context) {
  if (primary_keys.size() == 1) return getFilterKeys(primary_keys.front(), primary_key_types.front(), filter_actions_dag, context);
...
std::vector<std::string> serializeKeysToRawString(FieldVector::const_iterator &it, FieldVector::const_iterator end,
                                                  DataTypePtr key_column_type, size_t max_block_size) {
...
std::vector<std::string> serializeKeysToRawString(FieldVector::const_iterator &it, FieldVector::const_iterator end,
                                                  const DataTypes &key_column_types, size_t max_block_size) {
  if (key_column_types.empty()) return {};
...
size_t getPrimaryKeyPos(const Block &header, const Names &primary_key) {
  if (primary_key.size() != 1) throw Exception(ErrorCodes::LOGICAL_ERROR, "Only one primary key is supported");
