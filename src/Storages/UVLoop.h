...
class UVLoop : public boost::noncopyable {
 public:
  UVLoop() : loop_ptr(new uv_loop_t()) {
    ...
    if (res != 0) throw Exception(ErrorCodes::SYSTEM_ERROR, "UVLoop could not initialize");
  }
  ...
  ~UVLoop() {
    if (loop_ptr) {
      ...
      if (res == UV_EBUSY) {
        ...
        if (res == UV_EBUSY) {
          LOG_ERROR(log, "Failed to close libuv loop (active requests/handles in the loop: {})", uv_loop_alive(loop_ptr.get()));
          chassert(false);
        }
      }
    }
 ...
 private:
  ...
  static void onUVWalkClosingCallback(uv_handle_t *handle, void *) {
    if (!uv_is_closing(handle)) uv_close(handle, onUVCloseCallback);
  }
