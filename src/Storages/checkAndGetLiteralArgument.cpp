...
T checkAndGetLiteralArgument(const ASTPtr &arg, const String &arg_name) {
  if (arg) {
    ...
    if (arg->as<ASTLiteral>()) return T(checkAndGetLiteralArgument<T>(*arg->as<ASTLiteral>(), arg_name));
  }

  throw Exception(ErrorCodes::BAD_ARGUMENTS, "Argument '{}' must be a literal, get {} (value: {})", arg_name, arg ? arg->getID() : "NULL",
...
T checkAndGetLiteralArgument(const ASTLiteral &arg, const String &arg_name) {
  ...
  if (requested_type != provided_type)
    throw Exception(ErrorCodes::BAD_ARGUMENTS, "Argument '{}' must be a literal with type {}, got {}", arg_name,
                    fieldTypeToString(requested_type), fieldTypeToString(provided_type));
...
bool checkAndGetLiteralArgument(const ASTLiteral &arg, const String &arg_name) {
  ...
  if (arg.value.getType() == Field::Types::Which::Bool) return arg.value.safeGet<bool>();
