...
void traverse(const ParserImpl::Element &e, std::shared_ptr<JSONNode> node) {
  ...
  if (e.isObject()) {
    ...
    if (!val.object) val.object = JSONNodeList{};
    ...
    if (!val.array) val.array = JSONNodeList{};
    ...
    if (!field) throw Exception(ErrorCodes::INCORRECT_DATA, "Failed to parse a fixed JSON value '{}'.", escapeString(e.getString()));
...
std::shared_ptr<JSONNode> parseJSON(const String &json) {
  ...
  if (!p.parse(json, document)) {
    ...
    throw Exception(ErrorCodes::INCORRECT_DATA, "Failed to parse JSON from the string '{}'{}.", escapeString(json.substr(0, max_length)),
...
String generateRandomStringValue(UInt64 min_length, UInt64 max_length, pcg64 &rnd) {
  ...
  String res;
  res.reserve(size);
  for (size_t i = 0; i < size; ++i) res.push_back(generateRandomStringValueCharacter(rnd));
...
String generateRandomKey(UInt64 min_length, UInt64 max_length, pcg64 &rnd) {
  ...
  String res;
  res.reserve(size);
  for (size_t i = 0; i < size; ++i) res.push_back(generateRandomKeyCharacter(rnd));
...
Field generateRandomFixedValue(const StorageFuzzJSON::Configuration &config, pcg64 &rnd) {
  ...
  static std::array<Field::Types::Which, 3> possible_types{Field::Types::Which::UInt64, Field::Types::Which::String,
                                                           Field::Types::Which::Bool};
...
String fuzzString(UInt64 min_length, UInt64 max_length, pcg64 &rnd, const String &source, std::function<char(pcg64 &)> charGen) {
  ...
  while (result.size() < min_length) result.push_back(charGen(rnd));
...
std::shared_ptr<JSONNode> generateRandomJSONNode(const StorageFuzzJSON::Configuration &config, pcg64 &rnd, bool with_key,
                                                 JSONValue::Type type) {
  auto node = std::make_shared<JSONNode>();

  if (with_key) node->key = generateRandomKey(config.min_key_length, config.max_key_length, rnd);
...
template <size_t n>
std::shared_ptr<JSONNode> generateRandomJSONNode(const StorageFuzzJSON::Configuration &config, pcg64 &rnd, bool with_key,
                                                 const std::array<JSONValue::Type, n> &possible_types) {
...
std::shared_ptr<JSONNode> generateRandomJSONNode(const StorageFuzzJSON::Configuration &config, pcg64 &rnd, bool with_key, size_t depth) {
  if (depth >= config.max_nesting_level) return generateRandomJSONNode(config, rnd, with_key, JSONValue::Type::Fixed);

  static constexpr std::array<JSONValue::Type, 3> possible_types = {JSONValue::Type::Fixed, JSONValue::Type::Array,
...
JSONNode &fuzzSingleJSONNode(JSONNode &n, const StorageFuzzJSON::Configuration &config, pcg64 &rnd, size_t depth, size_t &node_count) {
  ...
  if (n.key && (action & update_key)) n.key = fuzzJSONKey(config, rnd, *n.key);

  if ((action & update_value) == 0) return n;

  if (val.fixed)
    val.fixed = generateRandomFixedValue(config, rnd);
  else if (val.array && val.array->size() < config.max_array_size &&
           node_count + val.array->size() < StorageFuzzJSON::Configuration::value_number_limit) {
    if (val.array->empty())
    ...
    ++node_count;
  } else if (val.object && val.object->size() < config.max_object_size &&
             node_count + val.object->size() < StorageFuzzJSON::Configuration::value_number_limit) {
...
void fuzzJSONObject(const std::shared_ptr<JSONNode> &node, WriteBuffer &out, const StorageFuzzJSON::Configuration &config, pcg64 &rnd,
  ...
  if (should_fuzz) fuzzSingleJSONNode(*next_node, config, rnd, depth, node_count);
  ...
  if (val.fixed) {
    ...
    if (!val.array && !val.object) return;
    ...
    for (const auto &ptr : node_list) {
      if (node_count >= StorageFuzzJSON::Configuration::value_number_limit) break;

      WriteBufferFromOwnString child_out;
      if (!first) child_out << fuzzJSONStructure(config, rnd, ", ");
      first = false;

      fuzzJSONObject(ptr, child_out, config, rnd, depth + 1, node_count);
      // Should not exceed the maximum length of the output string.
      if (out.count() + child_out.count() >= config.max_output_length) break;
      out << child_out.str();
    }
...
class FuzzJSONSource : public ISource {
 public:
  FuzzJSONSource(UInt64 block_size_, SharedHeader block_header_, const StorageFuzzJSON::Configuration &config_,
                 std::shared_ptr<JSONNode> json_root_)
...
StorageFuzzJSON::StorageFuzzJSON(const StorageID &table_id_, const ColumnsDescription &columns_, const String &comment_,
...
Pipe StorageFuzzJSON::read(const Names &column_names, const StorageSnapshotPtr &storage_snapshot, SelectQueryInfo & /*query_info*/,
  ...
  for (UInt64 i = 0; i < num_streams; ++i)
    pipes.emplace_back(
        std::make_shared<FuzzJSONSource>(max_block_size, std::make_shared<const Block>(block_header), config, parseJSON(config.json_str)));

  return Pipe::unitePipes(std::move(pipes));
}

static constexpr std::array<std::string_view, 14> optional_configuration_keys = {
    "json_str",          "random_seed",    "reuse_output",    "malform_output",          "probability",    "max_output_length",
    "max_nesting_level", "max_array_size", "max_object_size", "max_string_value_length", "min_key_length", "max_key_length"};

void StorageFuzzJSON::processNamedCollectionResult(Configuration &configuration, const NamedCollection &collection) {
  validateNamedCollection(collection, std::unordered_set<std::string>(),
                          std::unordered_set<std::string>(optional_configuration_keys.begin(), optional_configuration_keys.end()));

  if (collection.has("json_str")) configuration.json_str = collection.get<String>("json_str");

  if (collection.has("random_seed")) configuration.random_seed = collection.get<UInt64>("random_seed");

  if (collection.has("reuse_output")) configuration.should_reuse_output = static_cast<bool>(collection.get<UInt64>("reuse_output"));

  if (collection.has("malform_output")) configuration.should_malform_output = static_cast<bool>(collection.get<UInt64>("malform_output"));
  ...
  if (collection.has("max_output_length")) {
    ...
    if (configuration.max_output_length < 2 || configuration.max_output_length > StorageFuzzJSON::Configuration::output_length_limit)
      throw Exception(ErrorCodes::BAD_ARGUMENTS, "The value of the 'max_output_length' argument must be within the interval [2, {}.]",
                      StorageFuzzJSON::Configuration::output_length_limit);
  }

  if (collection.has("max_nesting_level")) configuration.max_nesting_level = collection.get<UInt64>("max_nesting_level");

  if (collection.has("max_array_size")) configuration.max_array_size = collection.get<UInt64>("max_array_size");

  if (collection.has("max_object_size")) configuration.max_object_size = collection.get<UInt64>("max_object_size");
  ...
  if (collection.has("max_string_value_length")) {
    ...
    if (max_string_value_length > StorageFuzzJSON::Configuration::output_length_limit)
      throw Exception(ErrorCodes::BAD_ARGUMENTS, "The value of the 'max_string_value_length' argument must be at most {}.",
                      StorageFuzzJSON::Configuration::output_length_limit);
  ...
  if (collection.has("max_key_length")) {
    ...
    if (max_key_length > StorageFuzzJSON::Configuration::output_length_limit)
      throw Exception(ErrorCodes::BAD_ARGUMENTS, "The value of the 'max_key_length' argument must be less or equal than {}.",
                      StorageFuzzJSON::Configuration::output_length_limit);
  ...
  if (collection.has("min_key_length")) {
    ...
    if (min_key_length == 0) throw Exception(ErrorCodes::BAD_ARGUMENTS, "The value of the 'min_key_length' argument must be at least 1.");
...
StorageFuzzJSON::Configuration StorageFuzzJSON::getConfiguration(ASTs &engine_args, ContextPtr local_context, const StorageID *table_id) {
  ...
  if (auto named_collection = tryGetNamedCollectionWithOverrides(engine_args, local_context, true, nullptr, table_id)) {
    ...
    for (auto &engine_arg : engine_args) engine_arg = evaluateConstantExpressionOrIdentifierAsLiteral(engine_arg, local_context);
    ...
    if (engine_args.size() == 2) {
      ...
      if (!literal.value.isNull()) configuration.random_seed = checkAndGetLiteralArgument<UInt64>(literal, "random_seed");
    }
  }
...
void registerStorageFuzzJSON(StorageFactory &factory) {
  factory.registerStorage("FuzzJSON", [](const StorageFactory::Arguments &args) -> std::shared_ptr<StorageFuzzJSON> {
    ASTs &engine_args = args.engine_args;

    if (engine_args.empty()) throw Exception(ErrorCodes::NUMBER_OF_ARGUMENTS_DOESNT_MATCH, "Storage FuzzJSON must have arguments.");
