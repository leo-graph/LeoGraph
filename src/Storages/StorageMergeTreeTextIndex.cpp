...
class MergeTreeTextIndexSource : public ISource {
 public:
  MergeTreeTextIndexSource(SharedHeader header_, std::shared_ptr<MergeTreeData::DataPartsVector> data_parts_,
                           std::shared_ptr<std::atomic<size_t>> part_index_, MergeTreeIndexPtr index_ptr, ReadSettings read_settings_,
                           size_t max_block_size_, std::shared_ptr<const KeyCondition> token_key_condition_)
      : ISource(header_),
    ...
    while (total_rows < max_block_size) {
      ...
      for (size_t pos = 0; pos < num_columns; ++pos) {
        ...
        if (column_name == "token") {
          ...
          for (size_t i = 0; i < block_size; ++i) data.push_back(static_cast<UInt64>(dict_block->token_infos[i].cardinality));
 ...
 private:
  ...
  std::optional<DictionaryBlock> readNextDictionaryBlock() {
    while (true) {
      if (!dictionary_buf) {
        if (!advanceToNextPart()) return std::nullopt;
      }
  ...
  bool advanceToNextPart() {
    ...
    while (true) {
      ...
      if (idx >= data_parts->size()) return false;
      ...
      if (token_key_condition) {
        ...
        if (!actual_sparse_index_name) continue;
        ...
        if (sparse_index.empty()) continue;

        computeMatchingBlocks();
        if (matching_blocks.empty()) continue;
      }
  ...
  void computeMatchingBlocks() {
    ...
    for (size_t i = 0; i < num_blocks; ++i) {
      ...
      Range token_range =
          (i + 1 < num_blocks) ? Range(left_ref, true, FieldRef(&ref_columns, i + 1, 0), false) : Range::createLeftBounded(left_ref, true);

      Hyperrectangle hyperrectangle = {token_range};
      if (token_key_condition->checkInHyperrectangle(hyperrectangle, key_types).can_be_true) matching_blocks.push_back(i);
    }
  }
...
class ReadFromMergeTreeTextIndex : public SourceStepWithFilter {
 public:
  ...
  ReadFromMergeTreeTextIndex(const Names &column_names_, const SelectQueryInfo &query_info_, const StorageSnapshotPtr &storage_snapshot_,
                             const ContextPtr &context_, SharedHeader sample_block, std::shared_ptr<StorageMergeTreeTextIndex> storage_,
...
void ReadFromMergeTreeTextIndex::applyFilters(ActionDAGNodes added_filter_nodes) {
  ...
  if (filter_actions_dag) {
    ...
    if (!key_condition->alwaysUnknownOrTrue()) token_key_condition = std::move(key_condition);
  }
}
...
void ReadFromMergeTreeTextIndex::initializePipeline(QueryPipelineBuilder &pipeline, const BuildQueryPipelineSettings &) {
  ...
  for (size_t i = 0; i < actual_streams; ++i) {
    pipes.emplace_back(std::make_shared<MergeTreeTextIndexSource>(getOutputHeader(), filtered_parts_ptr, shared_part_index,
                                                                  storage->text_index, context->getReadSettings(), max_block_size,
                                                                  token_key_condition));
  }
...
StorageMergeTreeTextIndex::StorageMergeTreeTextIndex(const StorageID &table_id_, const StoragePtr &source_table_,
                                                     MergeTreeIndexPtr text_index_, const ColumnsDescription &columns)
    : IStorage(table_id_), source_table(source_table_), text_index(std::move(text_index_)) {
...
void StorageMergeTreeTextIndex::read(QueryPlan &query_plan, const Names &column_names, const StorageSnapshotPtr &storage_snapshot,
                                     SelectQueryInfo &query_info, ContextPtr context, QueryProcessingStage::Enum, size_t max_block_size,
                                     size_t num_streams) {
  ...
  auto row_policy_filter = context->getRowPolicyFilter(source_storage_id.getDatabaseName(), source_storage_id.getTableName(),
  ...
  auto reading = std::make_unique<ReadFromMergeTreeTextIndex>(column_names, query_info, storage_snapshot, std::move(context),
                                                              std::move(sample_block), std::move(this_ptr), max_block_size, num_streams);
