...
StorageID ReplicatedMergeMutateTaskBase::getStorageID() const { return storage.getStorageID(); }
...
bool ReplicatedMergeMutateTaskBase::executeStep() {
  ...
  if (need_to_save_exception && saved_exception) {
    ...
    if (log_entry->type == ReplicatedMergeTreeLogEntryData::MUTATE_PART) {
      ...
      Int64 result_data_version = MergeTreePartInfo::fromPartName(log_entry->new_part_name, storage.queue.format_version).getDataVersion();
      auto source_part_info = MergeTreePartInfo::fromPartName(log_entry->source_parts.at(0), storage.queue.format_version);
...
bool ReplicatedMergeMutateTaskBase::executeImpl() {
  ...
  if (merge_mutate_entry) switcher.emplace((*merge_mutate_entry)->thread_group, ThreadName::MERGE_MUTATE, /*allow_existing_group*/ true);
  ...
  auto execute_fetch = [&](bool need_to_check_missing_part) -> bool {
    try {
      if (storage.executeFetch(entry, need_to_check_missing_part)) return remove_processed_entry();
    } catch (...) {
  ...
  switch (state) {
    case State::NEED_PREPARE: {
      {
        ...
        if (res == CheckExistingPartResult::PART_EXISTS) return remove_processed_entry();
      }
      ...
      if (!prepare_result.prepared_successfully) return execute_fetch(prepare_result.need_to_check_missing_part_in_fetch);
    ...
    case State::NEED_FINALIZE: {
      if (!finalize(part_log_writer)) return execute_fetch(/* need_to_check_missing = */ true);
...
ReplicatedMergeMutateTaskBase::CheckExistingPartResult ReplicatedMergeMutateTaskBase::checkExistingPart() {
  ...
  if (!existing_part) existing_part = storage.getActiveContainingPart(entry.new_part_name);
...
void ReplicatedMergeMutateTaskBase::maybeSleepBeforeZeroCopyLock(uint64_t estimated_space_for_result) {
  ...
  uint64_t min_parts_size_sleep_no_scale =
      (*storage_settings_ptr)[MergeTreeSetting::zero_copy_merge_mutation_min_parts_size_sleep_no_scale_before_lock].value;
  uint64_t min_parts_size_sleep_log_scale =
      (*storage_settings_ptr)[MergeTreeSetting::zero_copy_merge_mutation_min_parts_size_sleep_before_lock].value;
  ...
  if (min_parts_size_sleep != 0) {
    ...
    /// Replicas can be slow because of different reasons like bigger latency for ZooKeeper or just slight step behind because of bigger
    /// queue. In this case faster replica can pick up all merges execution, especially large merges while other replicas can just idle. And
    /// even in this case the fast replica is not overloaded because amount of executing merges doesn't affect the ability to acquire locks
    /// for new merges.
    ///
    /// So here we trying to solve it with the simplest solution -- sleep random time up to 500ms if there's no scale.
    /// If log scale is used, then we will sleep depending on the part size. E.g. up to 500ms for 1GB part and up to 7 seconds for 300GB
    /// part. It can sound too much, but we are trying to acquire these locks in background tasks which can be scheduled each 5 seconds or
    /// so. Using no scale helps in a situation when we have a lot of merges small in size (e.g. 256KB), and we still want to balance the
    /// merges load by adding some sleep. But at the same time we don't want to ruin really big parts merges by introducing long sleeps.
    ...
    uint64_t time_to_sleep_milliseconds =
        std::min<uint64_t>(10000UL, std::uniform_int_distribution<uint64_t>(1, 1 + right_border_to_sleep_ms)(rng));

    LOG_INFO(log,
             "Size of {} is {} bytes (it's more than sleep threshold {} for {} scale) so will intentionally sleep for {} ms to allow other "
             "replicas to take this big {}",
             task_for_log, estimated_space_for_result, min_parts_size_sleep, (log_scale ? "log" : "no"), time_to_sleep_milliseconds,
             task_for_log);
