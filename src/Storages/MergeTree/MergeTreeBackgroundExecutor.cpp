...
MergeTreeBackgroundExecutor<Queue>::MergeTreeBackgroundExecutor(ThreadName name_, size_t threads_count_, size_t max_tasks_count_,
                                                                CurrentMetrics::Metric metric_, CurrentMetrics::Metric max_tasks_metric_,
                                                                ...
                                                                ProfileEvents::Event wait_profile_event_, std::string_view policy)
    : name(name_),
      threads_count(threads_count_),
      max_tasks_count(max_tasks_count_),
      metric(metric_),
      max_tasks_metric(max_tasks_metric_, max_tasks_count),
      pool(std::make_unique<ThreadPool>(CurrentMetrics::MergeTreeBackgroundExecutorThreads,
  ...
  if (!policy.empty()) pending.updatePolicy(policy);
  ...
  for (size_t number = 0; number < threads_count; ++number) pool->scheduleOrThrowOnError([this] { threadFunction(); });
}
...
void MergeTreeBackgroundExecutor<Queue>::increaseThreadsAndMaxTasksCount(size_t new_threads_count, size_t new_max_tasks_count) {
  ...
  if (new_threads_count < threads_count) {
    LOG_WARNING(log, "Loaded new threads count for {}Executor from top level config, but new value ({}) is not greater than current {}",
                name, new_threads_count, threads_count);
    return;
  }

  if (new_max_tasks_count < max_tasks_count.load(std::memory_order_relaxed)) {
    LOG_WARNING(log, "Loaded new max tasks count for {}Executor from top level config, but new value ({}) is not greater than current {}",
                name, new_max_tasks_count, max_tasks_count.load());
    return;
  }
  ...
  for (size_t number = threads_count; number < new_threads_count; ++number) pool->scheduleOrThrowOnError([this] { threadFunction(); });
...
bool MergeTreeBackgroundExecutor<Queue>::trySchedule(ExecutableTaskPtr task) {
  ...
  if (value.load() >= static_cast<int64_t>(max_tasks_count)) return false;
...
void MergeTreeBackgroundExecutor<Queue>::routine(TaskRuntimeDataPtr item) {
  ...
  auto release_task = [this, &erase_from_active](TaskRuntimeDataPtr &&item_) TSA_NO_THREAD_SAFETY_ANALYSIS {
    ...
    NOEXCEPT_SCOPE({
      ...
      if (elapsed_ms > THRESHOLD_MILLISECONDS) {
        LOG_WARNING(log, "Releasing background task runtime data took {} milliseconds, executor={}, storage={}, query_id={}", elapsed_ms,
                    name, captured_storage_id.value_or("unknown"), captured_query_id.value_or("unknown"));
      }
    });
  ...
  auto complete_task = [this, &release_task](TaskRuntimeDataPtr &&item_) {
    {
      ...
      if (watch_on_completed.elapsedMilliseconds() > 1000) {
        LOG_WARNING(
            log, "Execution of callback took {} ms in [{}], Stack trace (when copying this message, always include the lines below): \n {}",
  ...
  try {
    ...
    ALLOW_ALLOCATIONS_IN_SCOPE;
    if (item->task->printExecutionException()) printExceptionWithRespectToAbort(log, query_id);
