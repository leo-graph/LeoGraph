...
void BackupImpl(const DiskPtr& src_disk, const DiskPtr& dst_disk, IDiskTransaction* transaction, const String& source_path,
                const String& destination_path, const ReadSettings& read_settings, const WriteSettings& write_settings,
                bool make_source_readonly, size_t level, std::optional<size_t> max_level, bool copy_instead_of_hardlinks,
                const NameSet& files_to_copy_instead_of_hardlinks) {
  if (max_level && level > *max_level) return;

  if (level >= 1000) throw DB::Exception(DB::ErrorCodes::TOO_DEEP_RECURSION, "Too deep recursion");
  ...
  for (auto it = src_disk->iterateDirectory(source_path); it->isValid(); it->next()) {
    ...
    if (!src_disk->existsDirectory(source)) {
      ...
      BackupImpl(src_disk, dst_disk, transaction, source, destination, read_settings, write_settings, make_source_readonly, level + 1,
                 max_level, copy_instead_of_hardlinks, files_to_copy_instead_of_hardlinks);
    }
  }
}
...
class CleanupOnFail {
 public:
  explicit CleanupOnFail(std::function<void()>&& cleaner_) : cleaner(cleaner_) {}
...
void Backup(const DiskPtr& src_disk, const DiskPtr& dst_disk, const String& source_path, const String& destination_path,
            const ReadSettings& read_settings, const WriteSettings& write_settings, bool make_source_readonly,
  ...
  while (true) {
    try {
      if (disk_transaction) {
        BackupImpl(src_disk, dst_disk, disk_transaction.get(), source_path, destination_path, read_settings, write_settings,
                   make_source_readonly,
                   /* level= */ 0, max_level, copy_instead_of_hardlinks, files_to_copy_intead_of_hardlinks);
        ...
        if (dst_disk->supportZeroCopyReplication())
          ...
          cleaner = [dst_disk, destination_path]() {
            dst_disk->removeSharedRecursive(destination_path, /*keep_all_shared_data*/ true, {});
          };
        else
        ...
        BackupImpl(src_disk, dst_disk, disk_transaction.get(), source_path, destination_path, read_settings, write_settings,
                   make_source_readonly,
                   /* level= */ 0, max_level,
                   /* copy_instead_of_hardlinks= */ false, files_to_copy_intead_of_hardlinks);
        cleanup.success();
      }
    } catch (const DB::ErrnoException& e) {
      if (e.getErrno() != ENOENT) throw;

      ++try_no;
      if (try_no == max_tries) throw;
