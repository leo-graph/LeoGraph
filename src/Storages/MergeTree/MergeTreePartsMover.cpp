...
class LargestPartsWithRequiredSize {
 ...
 public:
  ...
  void add(MergeTreeData::DataPartPtr part) {
    ...
    if (!elems.empty() && (*elems.begin())->getBytesOnDisk() >= part->getBytesOnDisk()) return;
  ...
  MergeTreeData::DataPartsVector getAccumulatedParts() {
    ...
    for (const auto& elem : elems) res.push_back(elem);
    return res;
  }
...
bool MergeTreePartsMover::selectPartsForMove(MergeTreeMovingParts& parts_to_move, const AllowedMovingPredicate& can_move,
  ...
  if (data_parts.empty()) return false;
  ...
  if (need_to_move.empty() && !metadata_snapshot->hasAnyMoveTTL()) return false;

  for (const auto& part : data_parts) {
    String reason;
    /// Don't report message to log, because logging is excessive.
    if (!can_move(part, &reason)) continue;
    ...
    if (reservation)  /// Found reservation by TTL rule.
    {
      ...
      /// possibly to zero.
      if (to_insert != need_to_move.end()) to_insert->second.decreaseRequiredSizeAndRemoveRedundantParts(part->getBytesOnDisk());

      ++parts_to_move_by_ttl_rules;
      parts_to_move_total_size_bytes += static_cast<double>(part->getBytesOnDisk());
    } else {
      if (to_insert != need_to_move.end()) to_insert->second.add(part);
    }
  }
  ...
  if (!parts_to_move.empty()) {
    LOG_DEBUG(log, "Selected {} parts to move according to storage policy rules and {} parts according to TTL rules, {} total",
              parts_to_move_by_policy_rules, parts_to_move_by_ttl_rules, ReadableSize(parts_to_move_total_size_bytes));
    return true;
  }
  return false;
}

MergeTreePartsMover::TemporaryClonedPart MergeTreePartsMover::clonePart(const MergeTreeMoveEntry& moving_part,
                                                                        const ReadSettings& read_settings,
                                                                        const WriteSettings& write_settings) const {
  auto cancellation_hook = [&my_moves_blocker = moves_blocker]() {
    if (my_moves_blocker.isCancelled()) throw Exception(ErrorCodes::ABORTED, "Cancelled moving parts.");
  };
  cancellation_hook();
  ...
  if (disk->supportZeroCopyReplication() && (*settings)[MergeTreeSetting::allow_remote_fs_zero_copy_replication]) {
    ...
    if (disk->existsFile(path_to_clone + relative_path)) {
      ...
      if (data->allowRemoveStaleMovingParts())
        throw Exception(ErrorCodes::DIRECTORY_ALREADY_EXISTS, "Cannot clone part {} from '{}' to '{}': path '{}' already exists",
                        part->name, part->getDataPartStorage().getDiskName(), disk->getName(),
                        fullPath(disk, path_to_clone + relative_path));

      LOG_DEBUG(log, "Path {} already exists. Will remove it and clone again", fullPath(disk, path_to_clone + relative_path));
    ...
    if (zero_copy_part) {
      ...
      cloned_part_storage = part->getDataPartStorage().clonePart(path_to_clone, part->getDataPartStorage().getPartDirectory(), disk,
                                                                 read_settings, write_settings, log, cancellation_hook);
    }
  } else {
...
void MergeTreePartsMover::renameClonedPart(IMergeTreeDataPart& part) const try {
  part.is_temp = false;
...
void MergeTreePartsMover::swapClonedPart(TemporaryClonedPart& cloned_part) const {
  ...
  if (moves_blocker.isCancelled()) throw Exception(ErrorCodes::ABORTED, "Cancelled moving parts.");
  ...
  if (!active_part || active_part->name != cloned_part.part->name) {
    LOG_INFO(log,
             ...
             cloned_part.part->name, active_part ? active_part->name : "doesn't exist",
             cloned_part.part->getDataPartStorage().getFullPath(),
             data->allowRemoveStaleMovingParts()
                 ? "will be removed"
                 : "will remain intact (set <allow_remove_stale_moving_parts> in config.xml, exercise caution when using)");
    return;
  }
