...
class BackgroundProcessListEntry {
 ...
 public:
  ...
  BackgroundProcessListEntry(BackgroundProcessList<ListElement, Info> &list_, const typename container_t::iterator it_,
...
class BackgroundProcessList {
 protected:
  ...
  explicit BackgroundProcessList(const CurrentMetrics::Metric &metric_) : metric(metric_) {}
 ...
 public:
  ...
  info_container_t get() const {
    ...
    for (const auto &list_element : entries) res.emplace_back(list_element.getInfo());
    return res;
  }
