...
MergeTreeSink::MergeTreeSink(StorageMergeTree& storage_, StorageMetadataPtr metadata_snapshot_, size_t max_parts_per_block_,
                             ContextPtr context_)
    : SinkToStorage(std::make_shared<const Block>(metadata_snapshot_->getSampleBlock())),
      ...
      deduplicate((*storage.getSettings())[MergeTreeSetting::non_replicated_deduplication_window] > 0 &&
                  storage.getDeduplicationLog() != nullptr) {
...
void MergeTreeSink::consume(Chunk& chunk) {
  if (num_blocks_processed > 0) storage.delayInsertOrThrowIfNeeded(nullptr, context, false);
  ...
  for (auto& current_block : part_blocks) {
...

    {
      ...
      if (result.removed_rows > 0) {
        ...
        LOG_DEBUG(storage.log, "In partition {} self deduplication removed tokens {} out of {}, left rows {} in tokens {}, debug: {}",
    ...
    LOG_DEBUG(
        storage.log, "Wrote block with {} rows and deduplication blocks: {}, deduplication info: {}", current_block.block->rows(),
...
void MergeTreeSink::finishDelayedChunk() {
  ...
  for (auto& partition : delayed_chunk->partitions) {
    ...
    while (true) {
      ...
      auto result = partition.deduplication_info->deduplicateBlock(conflicts, partition.block_with_partition.partition_id, context);
      ...
      LOG_DEBUG(storage.log,
                ...
                retry_times, result.removed_rows, partition.deduplication_info->getRows(), result.removed_tokens,
      ...
      if (result.filtered_block->rows() == 0) {
        LOG_DEBUG(storage.log, "All rows are deduplicated for part with block IDs: {}, skipping the part commit.",
                  fmt::join(conflicts, ", "));
...
std::vector<std::string> MergeTreeSink::commitPart(MergeTreeMutableDataPartPtr& part,
  ...
  MergeTreeData::Transaction transaction(storage, context->getCurrentTransaction().get());
  {
    ...
    if (!deduplication_hashes.empty()) {
      ...
      if (!conflict_block_ids.empty()) return conflict_block_ids;
    }
