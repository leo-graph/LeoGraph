...
void replaceExpressionToIdentifier(ASTPtr& ast, const String& expression_name, const String& identifier_name) {
  ...
  for (auto& child : ast->children) replaceExpressionToIdentifier(child, expression_name, identifier_name);
}
...
ASTPtr convertASTForIndexColumn(const IndexDescription& index, const ASTPtr& expression_ast, bool replace_index_column) {
  ...
  if (expression_ast == nullptr) return nullptr;
  ...
  if (isArray(index.data_types.front())) {
    ...
    auto array_map_arg =
        replace_index_column ? make_intrusive<ASTIdentifier>(preprocessor_column_name) : index.expression_list_ast->children.front();
...
ActionsDAG createActionsDAGForPreprocessor(const NamesAndTypesList& source_columns, const String& source_name,
                                           const DataTypePtr& source_type, ASTPtr expression_ast) {
  if (expression_ast == nullptr) return ActionsDAG();
  ...
  if (outputs.size() != 1)
    throw Exception(ErrorCodes::INCORRECT_QUERY, "The preprocessor expression must return a single column. Got {} output columns",
                    outputs.size());

  if (outputs.front()->type != ActionsDAG::ActionType::FUNCTION)
    throw Exception(ErrorCodes::INCORRECT_QUERY, "The preprocessor expression must be a function. Got '{}' action type",
                    outputs.front()->type);

  if (outputs.front()->result_name == source_name)
    throw Exception(ErrorCodes::INCORRECT_QUERY, "The preprocessor must have at least one expression on top of the source column. Got '{}'",
                    outputs.front()->result_name);

  if (!outputs.front()->result_type->equals(*source_type))
    throw Exception(ErrorCodes::INCORRECT_QUERY,
                    "The preprocessor expression should return the same type as the source column. Got '{}', expected '{}'",
                    outputs.front()->result_type->getName(), source_type->getName());
  ...
  if (actions_dag.hasArrayJoin()) throw Exception(ErrorCodes::INCORRECT_QUERY, "The preprocessor expression must not contain arrayJoin");
...
MergeTreeIndexTextPreprocessor::MergeTreeIndexTextPreprocessor(ASTPtr expression_ast, const IndexDescription& index_description)
    : index_column_type(index_description.data_types.front())
      ...
      ,
      original_actions(createActionsDAGForPreprocessor(index_description.expression->getRequiredColumnsWithTypes(),
                                                       index_description.column_names.front(), index_column_type,
      ...
      ,
      actions_for_index_column(createActionsDAGForPreprocessor({{preprocessor_column_name, index_column_type}}, preprocessor_column_name,
                                                               index_column_type,
                                                               convertASTForIndexColumn(index_description, expression_ast, true)))
      /// Take constant string and execute preprocessor expression.
      ,
      actions_for_constant(createActionsDAGForPreprocessor({{preprocessor_column_name, std::make_shared<DataTypeString>()}},
                                                           preprocessor_column_name, std::make_shared<DataTypeString>(),
                                                           convertASTForConstant(index_description, expression_ast))) {}

std::pair<ColumnPtr, size_t> MergeTreeIndexTextPreprocessor::processColumn(const ColumnWithTypeAndName& column, size_t start_row,
                                                                           size_t n_rows) const {
  ColumnPtr index_column = column.column;
  if (actions_for_index_column.getActions().empty()) return {index_column, start_row};

  chassert(column.type->equals(*index_column_type));
  chassert(index_column->getDataType() == column.type->getTypeId());

  /// Only copy if needed
  if (start_row != 0 || n_rows != index_column->size()) index_column = index_column->cut(start_row, n_rows);
...
String MergeTreeIndexTextPreprocessor::processConstant(const String& input) const {
  if (actions_for_constant.getActions().empty()) return input;
