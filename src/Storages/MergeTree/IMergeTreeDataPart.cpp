...
void IMergeTreeDataPart::MinMaxIndex::load(const IMergeTreeDataPart &part) {
  ...
  for (size_t i = 0; i < minmax_idx_size; ++i) {
    ...
    // NULL_LAST
    if (min_val.isNull()) min_val = POSITIVE_INFINITY;
    if (max_val.isNull()) max_val = POSITIVE_INFINITY;
...
IMergeTreeDataPart::MinMaxIndex::WrittenFiles IMergeTreeDataPart::MinMaxIndex::store(StorageMetadataPtr metadata_snapshot,
...
IMergeTreeDataPart::MinMaxIndex::WrittenFiles IMergeTreeDataPart::MinMaxIndex::store(const Names &column_names, const DataTypes &data_types,
                                                                                     IDataPartStorage &part_storage,
                                                                                     Checksums &out_checksums,
                                                                                     const MergeTreeSettingsPtr &storage_settings) const {
  if (!initialized)
    throw Exception(ErrorCodes::LOGICAL_ERROR, "Attempt to store uninitialized MinMax index for part {}. This is a bug",
...
void IMergeTreeDataPart::MinMaxIndex::update(const Block &block, const Names &column_names) {
  ...
  if (!initialized) hyperrectangle.reserve(column_names.size());
  ...
  if (!initialized) {
    ...
    for (size_t i = 0; i < hyperrectangle.size(); ++i) {
      hyperrectangle[i].left =
          accurateLess(hyperrectangle[i].left, other.hyperrectangle[i].left) ? hyperrectangle[i].left : other.hyperrectangle[i].left;
      hyperrectangle[i].right =
          accurateLess(hyperrectangle[i].right, other.hyperrectangle[i].right) ? other.hyperrectangle[i].right : hyperrectangle[i].right;
    }
  }
}
...
String IMergeTreeDataPart::MinMaxIndex::getFileColumnName(const String &column_name, const Checksums &checksums_) {
  ...
  if (checksums_.files.contains("minmax_" + stream_name + ".idx")) return stream_name;

  auto hash = sipHash128String(stream_name);
  if (checksums_.files.contains("minmax_" + hash + ".idx")) return hash;
  return stream_name;
}
...
IMergeTreeDataPart::IMergeTreeDataPart(const MergeTreeData &storage_, const MergeTreeSettings &storage_settings, const String &name_,
                                       const MergeTreePartInfo &info_, const MutableDataPartStoragePtr &data_part_storage_, Type part_type_,
  ...
  DimensionalMetrics::add(DimensionalMetrics::MergeTreeParts, {stateToString(), part_type.toString(), std::to_string(isProjectionPart())});
...
IMergeTreeDataPart::~IMergeTreeDataPart() {
  ...
  DimensionalMetrics::sub(DimensionalMetrics::MergeTreeParts, {stateToString(), part_type.toString(), std::to_string(isProjectionPart())});
}
...
IMergeTreeDataPart::IndexPtr IMergeTreeDataPart::getIndex() const {
  ...
  if (auto index_cache = storage.getPrimaryIndexCache()) return loadIndexToCache(*index_cache);
...
void IMergeTreeDataPart::moveIndexToCache(PrimaryIndexCache &index_cache) {
  ...
  for (const auto &[_, projection] : projection_parts) projection->moveIndexToCache(index_cache);
}
...
void IMergeTreeDataPart::removeFromVectorIndexCache(VectorSimilarityIndexCache *vector_similarity_index_cache) const {
  if (!vector_similarity_index_cache) return;
...
void IMergeTreeDataPart::setIndex(Columns index_columns) {
  std::scoped_lock lock(index_mutex);
  if (index) throw Exception(ErrorCodes::LOGICAL_ERROR, "The index of data part can be set only once");
...
void IMergeTreeDataPart::setName(const String &new_name) {
  ...
  for (auto &proj_part : projection_parts) proj_part.second->parent_part_name = new_name;
}
...
std::optional<size_t> IMergeTreeDataPart::getColumnPosition(const String &column_name) const {
  ...
  if (it == column_name_to_position.end()) return {};
  return it->second;
}
...
std::string_view IMergeTreeDataPart::stateString(MergeTreeDataPartState state) { return magic_enum::enum_name(state); }
...
std::pair<DayNum, DayNum> IMergeTreeDataPart::getMinMaxDate() const {
  if (storage.minmax_idx_date_column_pos != -1 && minmax_idx->initialized && !info.isPatch()) {
    ...
    return {DayNum(static_cast<DayNum::UnderlyingType>(hyperrectangle.left.safeGet<UInt64>())),
...
void IMergeTreeDataPart::setColumns(const NamesAndTypesList &new_columns, const SerializationInfoByName &new_infos,
  ...
  for (const auto &column : columns) column_name_to_position.emplace(column.name, pos++);

  for (const auto &column : columns) {
    auto it = serialization_infos.find(column.name);
    auto serialization = it == serialization_infos.end() ? IDataType::getSerialization(column, serialization_infos.getSettings())
    ...
    IDataType::forEachSubcolumn(
        [&](const auto &, const auto &subname, const auto &subdata) {
          ...
          if (!column_name_to_position.contains(full_name)) serializations.emplace(full_name, subdata.serialization);
        },
...
String IMergeTreeDataPart::getProjectionName() const {
  if (!parent_part) return "";

  if (!temp_projection_block_number) return name;

  if (!is_temp)
    throw Exception(ErrorCodes::LOGICAL_ERROR, "Temporary block number {} is set but projection part {} is not temporary",
                    *temp_projection_block_number, name);

  String suffix = "_" + std::to_string(*temp_projection_block_number);

  if (!name.ends_with(suffix))
    throw Exception(ErrorCodes::LOGICAL_ERROR, "Temporary block number {} is set but projection part {} doesn't have it in name's suffix",
                    *temp_projection_block_number, name);
...
StorageMetadataPtr IMergeTreeDataPart::getMetadataSnapshot() const {
  if (info.isPatch()) return storage.getPatchPartMetadata(*columns_description, info.getPartitionId(), storage.getContext());

  auto metadata_snapshot = storage.getInMemoryMetadataPtr();
  if (!parent_part) return metadata_snapshot;
...
SerializationPtr IMergeTreeDataPart::getSerialization(const String &column_name) const {
  ...
  if (!serialization)
    throw Exception(ErrorCodes::NO_SUCH_COLUMN_IN_TABLE, "There is no column or subcolumn {} in part {}", column_name, name);
...
bool IMergeTreeDataPart::isMovingPart() const {
  ...
  if (part_directory_path.filename().empty()) part_directory_path = part_directory_path.parent_path();
...
void IMergeTreeDataPart::loadIndexMarksToCache(MarkCache *index_mark_cache) const {
  ...
  for (const auto &index_description : secondary_indices) {
    ...
    for (const auto &substream : index_format.substreams) {
      ...
      if (!stream_name) continue;

      loaders.emplace_back(std::make_unique<MergeTreeMarksLoader>(
          info_for_read, index_mark_cache, index_granularity_info.getMarksFilePath(*stream_name),
          index_granularity->getMarksCountForSkipIndex(index_description.granularity), index_granularity_info,
          /*save_marks_in_cache=*/true, read_settings,
  ...
  for (auto &loader : loaders) loader->loadMarks();
}
...
void IMergeTreeDataPart::removeIndexMarksFromCache(MarkCache *index_mark_cache) const {
  ...
  for (const auto &index_description : secondary_indices) {
    ...
    for (const auto &substream : index_format.substreams) {
      ...
      auto key =
          MarkCache::hash(getDataPartStorage().getDiskName() + ":" + (fs::path(getRelativePathOfActivePart()) / marks_file).string());
...
void IMergeTreeDataPart::clearCaches() {
  if (cleared_data_in_caches.exchange(true) || is_duplicate) return;
...
bool IMergeTreeDataPart::mayStoreDataInCaches() const {
  if (cleared_data_in_caches) return false;
...
void IMergeTreeDataPart::removeIfNeeded() {
  ...
  if (!is_temp && state != MergeTreeDataPartState::DeleteOnDestroy) return;
...
UInt64 IMergeTreeDataPart::getIndexSizeInBytes() const {
  ...
  UInt64 res = 0;
  for (const auto &column : *index) res += column->byteSize();
  return res;
}
...
UInt64 IMergeTreeDataPart::getIndexSizeInAllocatedBytes() const {
  ...
  UInt64 res = 0;
  for (const auto &column : *index) res += column->allocatedBytes();
  return res;
}

UInt64 IMergeTreeDataPart::getIndexGranularityBytes() const { return index_granularity->getBytesSize(); }

UInt64 IMergeTreeDataPart::getIndexGranularityAllocatedBytes() const { return index_granularity->getBytesAllocated(); }
...
void IMergeTreeDataPart::assertState(const std::initializer_list<MergeTreeDataPartState> &affordable_states) const {
  if (!checkState(affordable_states)) {
    ...
    if (!states_str.empty()) states_str.pop_back();

    throw Exception(ErrorCodes::NOT_FOUND_EXPECTED_DATA_PART, "Unexpected state of part {}. Expected: {}", getNameWithState(), states_str);
  }
}

UInt64 IMergeTreeDataPart::getMarksCount() const { return index_granularity->getMarksCount(); }

UInt64 IMergeTreeDataPart::getExistingBytesOnDisk() const {
  if ((*storage.getSettings())[MergeTreeSetting::exclude_deleted_rows_for_part_size_in_merge] && supportLightweightDeleteMutate() &&
      hasLightweightDelete() && existing_rows_count.has_value() && existing_rows_count.value() < rows_count && rows_count > 0)
...
size_t IMergeTreeDataPart::getFileSizeOrZero(const String &file_name) const {
  ...
  if (checksum == checksums.files.end()) return 0;
...
String IMergeTreeDataPart::getColumnNameWithMinimumCompressedSize(const NamesAndTypesList &available_columns) const {
  ...
  for (const auto &column : available_columns) {
    if (!hasColumnFiles(column)) continue;
...
PackedFilesReader *IMergeTreeDataPart::getStatisticsPackedReader() const {
  ...
  if (statistics_reader) return statistics_reader.get();

  const String filename = String(ColumnsStatistics::FILENAME);
  if (!checksums.has(filename)) return nullptr;
  ...
  if (!disk_storage) return nullptr;

  statistics_reader = std::make_unique<PackedFilesReader>(disk_storage->getDisk(), packed_file, storage.getContext()->getReadSettings());
...
static const ColumnDescription *getColumnForStatisticsFile(const String &filename, const ColumnsDescription &all_columns,
  ...
  if (!required_columns.empty() && !required_columns.contains(column_name)) return nullptr;
...
ColumnsStatistics IMergeTreeDataPart::loadStatisticsWide(const NameSet &required_columns) const {
  ...
  for (const auto &[filename, checksum] : checksums.files) {
    if (!filename.ends_with(STATS_FILE_SUFFIX) || !filename.starts_with(STATS_FILE_PREFIX)) continue;
...
ColumnsStatistics IMergeTreeDataPart::loadStatistics() const {
  if (auto *reader = getStatisticsPackedReader()) return loadStatisticsPacked(*reader, {});
...
ColumnsStatistics IMergeTreeDataPart::loadStatistics(const Names &required_columns) const {
  ...
  if (auto *reader = getStatisticsPackedReader()) return loadStatisticsPacked(*reader, required_columns_set);
...
Estimates IMergeTreeDataPart::getEstimates() const {
  ...
  if (estimates.has_value()) return *estimates;

  estimates = Estimates();
  auto statistics = loadStatistics();

  for (const auto &[column_name, stats] : statistics) estimates->emplace(column_name, stats->getEstimate());
...
void IMergeTreeDataPart::loadColumnsChecksumsIndexes(bool require_columns_checksums, bool check_consistency, bool load_metadata_version) {
  ...
  try {
    if (!isStoredOnReadonlyDisk()) loadUUID();
    ...
    if (!(*storage.getSettings())[MergeTreeSetting::primary_key_lazy_load]) index = loadIndex();
    ...
    if (!parent_part) {
      if (!isStoredOnReadonlyDisk()) loadTTLInfos();

      loadProjections(require_columns_checksums, check_consistency, has_broken_projections, false /* if_not_loaded */);
    }

    if (check_consistency && !has_broken_projections) checkConsistency(require_columns_checksums);
    ...
    if (!isRetryableException(std::current_exception())) {
      ...
      LOG_ERROR(storage.log, "Part {} is broken and needs manual correction. Reason: {}", getDataPartStorage().getFullPath(), message);
      ...
      if (Exception *e = current_exception_cast<Exception *>()) {
        ...
        if (!files_in_part.empty()) e->addMessage("Part contains files: {}", files_in_part);
        if (isEmpty()) e->addMessage("Part is empty");
      }
    }
...
MergeTreeDataPartBuilder IMergeTreeDataPart::getProjectionPartBuilder(const String &projection_name, ProjectionDescriptionRawPtr projection,
                                                                      bool is_temp_projection) {
...
void IMergeTreeDataPart::addProjectionPart(const String &projection_name, std::shared_ptr<IMergeTreeDataPart> &&projection_part) {
  ...
  if (projection_name != projection_part->name)
    throw Exception(ErrorCodes::LOGICAL_ERROR, "The name of projection part ({}) is inconsistent with the name of projection ({})",
                    projection_part->name, projection_name);

  projection_parts[projection_name] = std::move(projection_part);
}

void IMergeTreeDataPart::loadProjections(bool require_columns_checksums, bool check_consistency, bool &has_broken_projection,
                                         bool if_not_loaded, bool only_metadata) {
  auto metadata_snapshot = storage.getInMemoryMetadataPtr();
  for (const auto &projection : metadata_snapshot->projections) {
    ...
    if (getDataPartStorage().existsDirectory(path)) {
      if (hasProjection(projection.name)) {
        if (!if_not_loaded)
          throw Exception(ErrorCodes::LOGICAL_ERROR, "Projection part {} in part {} is already loaded. This is a bug", projection.name,
                          name);
      } else {
        ...
        try {
          ...
          LOG_WARNING(storage.log,
                      "Cannot load projection {}, "
                      "will consider it broken. Reason: {}",
                      projection.name, message);
...
void IMergeTreeDataPart::loadIndexGranularity() {
  throw Exception(ErrorCodes::NOT_IMPLEMENTED, "Method 'loadIndexGranularity' is not implemented for part with type {}",
                  getType().toString());
}
...
void IMergeTreeDataPart::optimizeIndexColumns(size_t marks_count, Columns &index_columns) const {
  ...
  if (info.isPatch()) return;

  size_t key_size = index_columns.size();
  Float64 ratio_to_drop_suffix_columns =
      (*storage.getSettings())[MergeTreeSetting::primary_key_ratio_of_unique_prefix_values_to_skip_suffix_columns];
  ...
  if (key_size > 1 && ratio_to_drop_suffix_columns > 0 && ratio_to_drop_suffix_columns < 1) {
    for (size_t j = 0; j < key_size - 1; ++j) {
      ...
      for (size_t i = 1; i < marks_count; ++i) {
        if (0 != index_columns[j]->compareAt(i, i - 1, *index_columns[j], 0)) ++num_changes;
      }
...
std::shared_ptr<IMergeTreeDataPart::Index> IMergeTreeDataPart::loadIndex() const {
  ...
  if (!key_size) return std::make_shared<Index>();
  ...
  for (size_t j = 0; j < key_size; ++j) key_serializations[j] = primary_key.data_types[j]->getDefaultSerialization();

  FormatSettings format_settings;
  for (size_t i = 0; i < marks_count; ++i) {
    for (size_t j = 0; j < key_size; ++j) key_serializations[j]->deserializeBinary(*loaded_index[j], *index_file, format_settings);
  }
  ...
  for (const auto &column : loaded_index) {
    ...
    if (column->size() != marks_count)
      throw Exception(ErrorCodes::CANNOT_READ_ALL_DATA,
                      ...
                      "{}, read: {})",
                      index_path, marks_count, column->size());
  }

  if (!index_file->eof()) throw Exception(ErrorCodes::EXPECTED_END_OF_FILE, "Index file {} is unexpectedly long", index_path);
...
NameSet IMergeTreeDataPart::getFileNamesWithoutChecksums() const {
  ...
  if (getDataPartStorage().existsFile(DEFAULT_COMPRESSION_CODEC_FILE_NAME)) result.emplace(DEFAULT_COMPRESSION_CODEC_FILE_NAME);

  if (getDataPartStorage().existsFile(TXN_VERSION_METADATA_FILE_NAME)) result.emplace(TXN_VERSION_METADATA_FILE_NAME);

  if (getDataPartStorage().existsFile(METADATA_VERSION_FILE_NAME)) result.emplace(METADATA_VERSION_FILE_NAME);

  if (getDataPartStorage().existsFile(COLUMNS_SUBSTREAMS_FILE_NAME)) result.emplace(COLUMNS_SUBSTREAMS_FILE_NAME);
  ...
  if (auto file_buf = readFileIfExists(DEFAULT_COMPRESSION_CODEC_FILE_NAME)) {
    ...
    if (!checkString("CODEC", buf)) {
      LOG_WARNING(storage.log,
                  ...
                  name, path, codec_line);
      default_codec = detectDefaultCompressionCodec();
    }

    try {
      ParserCodec codec_parser;
      auto codec_ast = parseQuery(codec_parser, codec_line.data() + buf.getPosition(), codec_line.data() + codec_line.length(),
                                  "codec parser", 0, DBMS_DEFAULT_MAX_PARSER_DEPTH, DBMS_DEFAULT_MAX_PARSER_BACKTRACKS);
      default_codec = CompressionCodecFactory::instance().get(codec_ast, {});
    } catch (const DB::Exception &ex) {
      LOG_WARNING(storage.log,
                  "Cannot parse default codec for part {} from file {}, content '{}', error '{}'. Default compression codec will be "
                  "deduced automatically, from data on disk.",
                  name, path, codec_line, ex.what());
      default_codec = detectDefaultCompressionCodec();
    }
  } else
    default_codec = detectDefaultCompressionCodec();
...
void IMergeTreeDataPart::writeChecksums(const MergeTreeDataPartChecksums &checksums_, const WriteSettings &settings) {
  writeMetadata("checksums.txt", settings, [&checksums_](auto &buffer) { checksums_.write(buffer); });
}

void IMergeTreeDataPart::writeColumns(const NamesAndTypesList &columns_, const WriteSettings &settings) {
  writeMetadata("columns.txt", settings, [&columns_](auto &buffer) { columns_.writeText(buffer); });
}
...
void IMergeTreeDataPart::writeVersionMetadata(const VersionMetadata &version_, bool fsync_part_dir) const {
  ...
  try {
    ...
    if (fsync_part_dir) sync_guard = data_part_storage.getDirectorySyncGuard();
...
void IMergeTreeDataPart::removeVersionMetadata() { getDataPartStorage().removeFileIfExists(TXN_VERSION_METADATA_FILE_NAME); }

void IMergeTreeDataPart::removeMetadataVersion() { getDataPartStorage().removeFileIfExists(METADATA_VERSION_FILE_NAME); }
...
CompressionCodecPtr IMergeTreeDataPart::detectDefaultCompressionCodec() const {
    ...
    if (column_size.data_compressed != 0 && !storage_columns.hasCompressionCodec(part_column.name)) {
      ...
      getSerialization(part_column.name)->enumerateStreams([&](const ISerialization::SubstreamPath &substream_path) {
        if (path_to_data_file.empty()) {
          ...
          if (getDataPartStorage().getFileSize(file_name) != 0) path_to_data_file = file_name;
        }
      });

      if (path_to_data_file.empty()) {
        LOG_WARNING(storage.log, "Part's {} column {} has non zero data compressed size, but all data files don't exist or empty", name,
                    backQuoteIfNeed(part_column.name));
        continue;
      }
  ...
  if (!result) result = CompressionCodecFactory::instance().getDefaultCodec();
...
void IMergeTreeDataPart::loadPartitionAndMinMaxIndex() {
  ...
  if (storage.format_version < MERGE_TREE_DATA_MIN_FORMAT_VERSION_WITH_CUSTOM_PARTITIONING && !parent_part) {
    ...
    if (!isEmpty()) minmax_idx->load(*this);
  }
...
void IMergeTreeDataPart::loadChecksums(bool require) {
  if (auto buf = readFileIfExists("checksums.txt")) {
    ...
    if (require) throw Exception(ErrorCodes::NO_FILE_IN_DATA_PART, "No checksums.txt in part {}", name);
    ...
    bool noop;
    checksums =
        checkDataPart(shared_from_this(), false, noop, /* is_cancelled */ [] { return false; }, /* throw_on_broken_projection */ false);
  ...
  if (index_granularity->empty()) {
    ...
    for (const auto &column : getColumns()) {
      ...
      if (column.type->isValueRepresentedByNumber() && !column.type->haveSubtypes() &&
        ...
        if (size.data_uncompressed == 0) continue;
        ...
        if (rows_in_column - index_granularity_without_last_mark > last_possibly_incomplete_mark_rows) {
          throw Exception(ErrorCodes::LOGICAL_ERROR,
                          ...
                          backQuote(column.name), rows_in_column - index_granularity_without_last_mark, name,
    ...
    for (const NameAndTypePair &column : columns) {
      ...
      if (!column_col->isFixedAndContiguous() || column_col->lowCardinality()) continue;

      size_t column_size = getColumnSize(column.name).data_uncompressed;
      if (!column_size) continue;

      size_t sizeof_field = column_col->sizeOfValueIfFixed();
      rows_count = column_size / sizeof_field;

      if (column_size % sizeof_field != 0) {
        throw Exception(ErrorCodes::LOGICAL_ERROR, "Uncompressed size of column {}({}) is not divisible by the size of value ({})",
...
void IMergeTreeDataPart::loadExistingRowsCount() {
  if (existing_rows_count.has_value()) return;
...
UInt64 IMergeTreeDataPart::readExistingRowsCount() {
  ...
  if (!total_mark) return rows_count;
  ...
  MergeTreeReaderPtr reader =
      createMergeTreeReader(part_info, cols, storage_snapshot_ptr, storage.getSettings(), MarkRanges{MarkRange(0, total_mark)},
                            /*virtual_fields=*/{},
                            /*uncompressed_cache=*/{}, storage.getContext()->getMarkCache().get(), nullptr,
                            MergeTreeReaderSettings::createFromSettings(), ValueSizeMap{}, ReadBufferFromFileBase::ProfileCallback{});
...
void IMergeTreeDataPart::loadColumns(bool require, bool load_metadata_version) {
  ...
  if (auto in = readFileIfExists("columns.txt")) {
    ...
    for (auto &column : loaded_columns) setVersionToAggregateFunctions(column.type, true);
  } else {
    /// We can get list of columns only from columns.txt in compact parts.
    if (require || part_type == Type::Compact || info.isPatch())
      throw Exception(ErrorCodes::NO_FILE_IN_DATA_PART, "No columns.txt in part {}, expected path {} on disk {}", name, path,
                      getDataPartStorage().getDiskName());

    auto metadata_snapshot = getMetadataSnapshot();
    /// If there is no file with a list of columns, write it down.
    for (const auto &column : metadata_snapshot->getColumns().getAllPhysical())
      if (getFileNameForColumn(column)) loaded_columns.push_back(column);

    if (loaded_columns.empty()) throw Exception(ErrorCodes::NO_FILE_IN_DATA_PART, "No columns in part {}", name);

    if (!is_readonly_storage) writeColumns(loaded_columns, {});
  }

  SerializationInfoByName infos({});
  if (auto in = readFileIfExists(SERIALIZATION_FILE_NAME)) infos = SerializationInfoByName::readJSON(loaded_columns, *in);
...
void IMergeTreeDataPart::loadColumnsSubstreams() {
  ...
  /// In Compact part with marks for substreams we must have substreams file. For other cases it's not mandatory.
    ...
    throw Exception(ErrorCodes::NO_FILE_IN_DATA_PART, "No columns_substreams.txt in part {}, expected path {} on drive {}", name,
                    fs::path(getDataPartStorage().getRelativePath()) / COLUMNS_SUBSTREAMS_FILE_NAME, getDataPartStorage().getDiskName());
  }
}
...
bool IMergeTreeDataPart::hasLightweightDelete() const { return columns.contains(RowExistsColumn::name); }
...
void IMergeTreeDataPart::assertHasVersionMetadata(MergeTreeTransaction *txn) const {
  ...
  if (version.creation_tid != expected_tid)
    throw Exception(ErrorCodes::LOGICAL_ERROR,
                    "CreationTID of part {} (table {}) is set to unexpected value {}, it's a bug. Current transaction: {}", name,
                    storage.getStorageID().getNameForLogs(), version.creation_tid, txn ? txn->dumpDescription() : "<none>");
...
void IMergeTreeDataPart::storeVersionMetadata(bool force) const {
  if (!wasInvolvedInTransaction() && !force) return;
  if (!storage.supportsTransactions()) throw Exception(ErrorCodes::LOGICAL_ERROR, "Storage does not support transaction. It is a bug");

  LOG_TEST(storage.log, "Writing version for {} (creation: {}, removal {}, creation csn {})", name, version.creation_tid,
           version.removal_tid, version.creation_csn.load());
...
void IMergeTreeDataPart::loadVersionMetadata() const try {
  ...
  auto remove_tmp_file = [&]() {
    ...
    LOG_WARNING(storage.log, "Found file {} that was last modified on {}, has size {} and the following content: {}", tmp_version_file_name,
                last_modified.epochTime(), content.size(), content);
  ...
  if (data_part_storage.existsFile(version_file_name)) {
    ...
    if (!isStoredOnReadonlyDisk() && data_part_storage.existsFile(tmp_version_file_name)) remove_tmp_file();
    return;
  }
  ...
  if (!isStoredOnReadonlyDisk()) remove_tmp_file();
} catch (Exception &e) {
...
bool IMergeTreeDataPart::wasInvolvedInTransaction() const {
  assert(!storage.data_parts_loading_finished || !version.creation_tid.isEmpty() ||
...
bool IMergeTreeDataPart::assertHasValidVersionMetadata() const {
  ...
  if (isProjectionPart()) return true;

  if (!wasInvolvedInTransaction()) return true;

  if (part_is_probably_removed_from_disk) return true;

  if (state == MergeTreeDataPartState::Temporary) return true;
  ...
  try {
    ...
    bool valid_creation_csn =
        version.creation_csn == file.creation_csn || version.creation_csn == Tx::RolledBackCSN || file.creation_csn == Tx::UnknownCSN;
    bool valid_removal_csn =
        version.removal_csn == file.removal_csn || version.removal_csn == Tx::PrehistoricCSN || file.removal_csn == Tx::UnknownCSN;
    bool valid_removal_tid_lock =
        (version.removal_tid.isEmpty() && version.removal_tid_lock == 0) || (version.removal_tid_lock == version.removal_tid.getHash());
    ...
    tryLogCurrentException(storage.log, fmt::format("File {} contains:\n{}\nexpected:\n{}\nlock: {}\nname: {}", version_file_name, content,
                                                    expected.str(), version.removal_tid_lock.load(), name));
    return false;
  }
}
...
void IMergeTreeDataPart::remove() {
  ...
  auto can_remove_callback = [this]() {
    ...
    if (!can_remove) LOG_TRACE(storage.log, "Blobs of part {} cannot be removed", name);
...
std::optional<String> IMergeTreeDataPart::getRelativePathForPrefix(const String &prefix, bool detached, bool broken) const {
  ...
  if (detached && parent_part) throw Exception(ErrorCodes::LOGICAL_ERROR, "Cannot detach projection");
...
std::optional<String> IMergeTreeDataPart::getRelativePathForDetachedPart(const String &prefix, bool broken) const {
  ...
  assert(prefix.empty() || std::find(DetachedPartInfo::DETACH_REASONS.begin(), DetachedPartInfo::DETACH_REASONS.end(), prefix) !=
                               DetachedPartInfo::DETACH_REASONS.end());
  if (auto path = getRelativePathForPrefix(prefix, /* detached */ true, broken)) return fs::path(MergeTreeData::DETACHED_DIR_NAME) / *path;
  return {};
}

String IMergeTreeDataPart::getRelativePathOfActivePart() const { return fs::path(getDataPartStorage().getFullRootPath()) / name / ""; }
...
DataPartStoragePtr IMergeTreeDataPart::makeCloneInDetached(const String &prefix, const StorageMetadataPtr & /*metadata_snapshot*/,
  ...
  if (!maybe_path_in_detached) return nullptr;
  ...
  IDataPartStorage::ClonePartParams params{
      .copy_instead_of_hardlink = isStoredOnRemoteDiskWithZeroCopySupport() && storage.supportsReplication() &&
                                  (*storage_settings)[MergeTreeSetting::allow_remote_fs_zero_copy_replication],
      .keep_metadata_version = true,
      .make_source_readonly = true,
      .external_transaction = disk_transaction};
  return getDataPartStorage().freeze(storage.relative_data_path, *maybe_path_in_detached,
...
MutableDataPartStoragePtr IMergeTreeDataPart::makeCloneOnDisk(const DiskPtr &disk, const String &directory_name,
  ...
  if (directory_name.empty()) throw Exception(ErrorCodes::LOGICAL_ERROR, "Can not clone data part {} to empty directory.", name);

  String path_to_clone = fs::path(storage.relative_data_path) / directory_name / "";
  return getDataPartStorage().clonePart(path_to_clone, getDataPartStorage().getPartDirectory(), disk, read_settings, write_settings,
                                        storage.log.load(), cancellation_hook);
}
...
IndexSize IMergeTreeDataPart::getIndexSizeFromFile() const {
  ...
  if (!pk.column_names.empty()) {
    ...
    if (bin_checksum == checksums.files.end()) bin_checksum = checksums.files.find("primary" + getIndexExtension(false));
...
void IMergeTreeDataPart::checkConsistencyBase() const {
  ...
  if (!checksums.empty()) {
    if (!pk.column_names.empty() &&
        (!checksums.files.contains("primary" + getIndexExtension(false)) && !checksums.files.contains("primary" + getIndexExtension(true))))
      throw Exception(ErrorCodes::NO_FILE_IN_DATA_PART, "No checksum for {} or {}", toString("primary" + getIndexExtension(false)),
                      toString("primary" + getIndexExtension(true)));

    if (storage.format_version >= MERGE_TREE_DATA_MIN_FORMAT_VERSION_WITH_CUSTOM_PARTITIONING) {
      if (!checksums.files.contains("count.txt")) throw Exception(ErrorCodes::NO_FILE_IN_DATA_PART, "No checksum for count.txt");
    ...
    for (const auto &[filename, checksum] : checksums.files) {
      try {
        ...
        if (!parent_part && filename.ends_with(".proj")) {
          ...
          if (hasProjection(projection_name)) markProjectionPartAsBroken(projection_name, ex.message(), ex.code());
        } else
          throw;
      }
    }
    ...
    auto check_file_not_empty = [this](const String &file_path) {
      ...
      if (!getDataPartStorage().existsFile(file_path) || (file_size = getDataPartStorage().getFileSize(file_path)) == 0)
        throw Exception(ErrorCodes::BAD_SIZE_OF_FILE_IN_DATA_PART, "Part {} is broken: {} is empty", getDataPartStorage().getFullPath(),
    ...
    if (storage.format_version >= MERGE_TREE_DATA_MIN_FORMAT_VERSION_WITH_CUSTOM_PARTITIONING) {
      ...
      if (metadata_snapshot->hasPartitionKey()) check_file_not_empty("partition.dat");

      if (!parent_part) {
        for (const String &col_name : MergeTreeData::getMinMaxColumnsNames(partition_key)) try {
...
void IMergeTreeDataPart::checkConsistency(bool require_part_metadata) const {
  try {
    ...
    const auto part_state = fmt::format("state: {}, is_unexpected_local_part: {}, is_frozen: {}, is_duplicate: {}", stateString(),
                                        is_unexpected_local_part, is_frozen.load(), is_duplicate, is_temp);

    const auto debug_info = fmt::format(
        "columns: {}, getMarkSizeInBytes: {}, getMarksCount: {}, index_granularity_info: [{}], index_granularity: [{}], "
        "part_state: [{}]",
        columns.toString(), index_granularity_info.getMarkSizeInBytes(columns.size()), index_granularity->getMarksCount(),
        index_granularity_info.describe(), index_granularity->describe(), part_state);
  ...
  for (auto &index_description : secondary_indices_descriptions) {
    ...
    for (const auto &index_substream : index_substreams) {
      ...
      if (actual_marks_file_name) {
        ...
        if (mrk_checksum != checksums.files.end()) substream_size.marks = mrk_checksum->second.file_size;
      }
...
ColumnSize IMergeTreeDataPart::getColumnSize(const String &column_name) const {
  ...
  if (!are_columns_and_secondary_indices_sizes_calculated && areChecksumsLoaded()) calculateColumnsAndSecondaryIndicesSizesOnDisk();

  /// For some types of parts columns_size maybe not calculated
  auto it = columns_sizes.find(column_name);
  if (it != columns_sizes.end()) return it->second;
...
ColumnSize IMergeTreeDataPart::getSubcolumnSize(const String &subcolumn_name) const {
  ...
  if (auto size = subcolumns_sizes_cache.get(subcolumn_name)) return *size;
...
IndexSize IMergeTreeDataPart::getSecondaryIndexSize(const String &secondary_index_name) const {
  std::unique_lock lock(columns_and_secondary_indices_sizes_mutex);
  if (!are_columns_and_secondary_indices_sizes_calculated) calculateColumnsAndSecondaryIndicesSizesOnDisk();

  auto it = secondary_index_sizes.find(secondary_index_name);
  if (it != secondary_index_sizes.end()) return it->second;
...
bool IMergeTreeDataPart::hasSecondaryIndex(const String &index_name, const StorageMetadataPtr &metadata) const {
  ...
  return getStreamNameOrHash(file_name, ".idx", checksums).has_value() || getStreamNameOrHash(file_name, ".idx2", checksums).has_value();
}

void IMergeTreeDataPart::accumulateColumnSizes(ColumnToSize &column_to_size) const {
  for (const auto &[column_name, size] : columns_sizes) column_to_size[column_name] = size.data_compressed;
}

bool IMergeTreeDataPart::checkAllTTLCalculated(const StorageMetadataPtr &metadata_snapshot) const {
  if (!metadata_snapshot->hasAnyTTL()) return false;

  if (metadata_snapshot->hasRowsTTL()) {
    if (isEmpty())  /// All rows were finally deleted and we don't store TTL
      return true;
    if (ttl_infos.table_ttl.min == 0) return false;
  }

  for (const auto &[column, desc] : metadata_snapshot->getColumnTTLs()) {
    /// Part has this column, but we don't calculated TTL for it
    if (!ttl_infos.columns_ttl.contains(column) && getColumns().contains(column)) return false;
  }

  for (const auto &move_desc : metadata_snapshot->getMoveTTLs()) {
    /// Move TTL is not calculated
    if (!ttl_infos.moves_ttl.contains(move_desc.result_column)) return false;
  }

  for (const auto &group_by_desc : metadata_snapshot->getGroupByTTLs()) {
    if (!ttl_infos.group_by_ttl.contains(group_by_desc.result_column)) return false;
  }

  for (const auto &rows_where_desc : metadata_snapshot->getRowsWhereTTLs()) {
    if (!ttl_infos.rows_where_ttl.contains(rows_where_desc.result_column)) return false;
  }

  return true;
}

String IMergeTreeDataPart::getUniqueId() const { return getDataPartStorage().getUniqueId(); }
...
std::optional<String> IMergeTreeDataPart::getStreamNameOrHash(const String &stream_name, const String &extension,
                                                              const Checksums &checksums_) {
  if (checksums_.files.contains(stream_name + extension)) return stream_name;

  auto hash = sipHash128String(stream_name);
  if (checksums_.files.contains(hash + extension)) return hash;
...
std::optional<String> IMergeTreeDataPart::getStreamNameOrHash(const String &stream_name, const String &extension,
  ...
  if (storage_.existsFile(stream_name + extension)) return stream_name;

  auto hash = sipHash128String(stream_name);
  if (storage_.existsFile(hash + extension)) return hash;

  return {};
}

std::optional<String> IMergeTreeDataPart::getStreamNameForColumn(const String &column_name,
                                                                 const ISerialization::SubstreamPath &substream_path,
                                                                 const String &extension, const Checksums &checksums_,
  ...
  if (auto result_stream_name = getStreamNameOrHash(stream_name, extension, checksums_)) return result_stream_name;
...
std::optional<String> IMergeTreeDataPart::getStreamNameForColumn(const NameAndTypePair &column,
                                                                 const ISerialization::SubstreamPath &substream_path,
                                                                 const String &extension, const Checksums &checksums_,
                                                                 const MergeTreeSettingsPtr &settings) {
  ISerialization::StreamFileNameSettings stream_file_name_settings(*settings);
...
std::optional<String> IMergeTreeDataPart::getStreamNameForColumn(const String &column_name,
                                                                 const ISerialization::SubstreamPath &substream_path,
                                                                 const String &extension, const IDataPartStorage &storage_,
                                                                 const MergeTreeSettingsPtr &settings) {
  ISerialization::StreamFileNameSettings stream_file_name_settings(*settings);
  auto stream_name = ISerialization::getFileNameForStream(column_name, substream_path, stream_file_name_settings);
  if (auto result_stream_name = getStreamNameOrHash(stream_name, extension, storage_)) return result_stream_name;
...
void IMergeTreeDataPart::markProjectionPartAsBroken(const String &projection_name, const String &message, int code) const {
  ...
  if (it == projection_parts.end()) throw Exception(ErrorCodes::LOGICAL_ERROR, "There is no projection part '{}'", projection_name);
  it->second->setBrokenReason(message, code);
}

bool IMergeTreeDataPart::hasBrokenProjection(const String &projection_name) const {
  auto it = projection_parts.find(projection_name);
  if (it == projection_parts.end()) return false;
...
ColumnPtr IMergeTreeDataPart::getColumnSample(const NameAndTypePair &column) const {
  ...
  if (!column.type->hasDynamicSubcolumns() || !total_mark) return column.type->createColumn();
  ...
  settings.use_prefixes_deserialization_thread_pool =
  ...
  MergeTreeReaderPtr reader =
      createMergeTreeReader(part_info, cols, storage_snapshot_ptr, storage.getSettings(), MarkRanges{MarkRange(0, total_mark)},
                            /*virtual_fields=*/{},
                            /*uncompressed_cache=*/{}, storage.getContext()->getMarkCache().get(), nullptr, settings, ValueSizeMap{},
...
bool isCompactPart(const MergeTreeDataPartPtr &data_part) { return (data_part && data_part->getType() == MergeTreeDataPartType::Compact); }

bool isWidePart(const MergeTreeDataPartPtr &data_part) { return (data_part && data_part->getType() == MergeTreeDataPartType::Wide); }

bool isCompressedFromIndexExtension(const String &index_extension) { return index_extension == getIndexExtension(true); }

Strings getPartsNames(const MergeTreeDataPartsVector &parts) {
  Strings part_names;
  for (const auto &p : parts) part_names.push_back(p->name);
  return part_names;
}
...
bool isCompressedFromFileName(const String &file_name) {
  ...
  return (MarkType::isMarkFileExtension(extension) && MarkType(extension).compressed) || isCompressedFromIndexExtension(extension);
}
...
std::unique_ptr<ReadBuffer> IMergeTreeDataPart::readFile(const String &file_name) const {
  ...
  if (isCompressedFromFileName(file_name)) return std::make_unique<CompressedReadBufferFromFile>(std::move(res));
