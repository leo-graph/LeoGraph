...
class MergeTreePartsMover {
 ...
 public:
  explicit MergeTreePartsMover(MergeTreeData *data_) : data(data_), log(getLogger("MergeTreePartsMover")) {}
  ...
  bool selectPartsForMove(MergeTreeMovingParts &parts_to_move, const AllowedMovingPredicate &can_move,
                          const std::lock_guard<std::mutex> &moving_parts_lock);

  /// Copies part to selected reservation in detached folder. Throws exception if part already exists.
  TemporaryClonedPart clonePart(const MergeTreeMoveEntry &moving_part, const ReadSettings &read_settings,
