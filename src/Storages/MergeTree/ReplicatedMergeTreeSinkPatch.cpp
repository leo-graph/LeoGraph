...
ReplicatedMergeTreeSinkPatch::ReplicatedMergeTreeSinkPatch(StorageReplicatedMergeTree& storage_, StorageMetadataPtr metadata_snapshot_,
                                                           LightweightUpdateHolderInKeeper update_holder_, ContextPtr context_)
    : ReplicatedMergeTreeSink(
          /*async_insert=*/false, storage_, metadata_snapshot_,
          /*quorum=*/0,
          /*quorum_timeout_ms=*/0,
          /*max_parts_per_block=*/0,
          /*quorum_parallel=*/false,
          /*majority_quorum=*/false, std::move(context_)),
...
void ReplicatedMergeTreeSinkPatch::finishDelayed(const ZooKeeperWithFaultInjectionPtr& zookeeper) {
  ...
  for (auto& partition : delayed_parts) {
    ...
    if (!part->info.isPatch()) throw Exception(ErrorCodes::LOGICAL_ERROR, "Expected patch part, got part with name: {}", part->name);
    ...
    try {
      ...
      if (!conflicts.empty()) throw Exception(ErrorCodes::LOGICAL_ERROR, "Patch part {} was deduplicated. It's a bug", part->name);
      ...
      PartLog::addNewPart(storage.getContext(), PartLog::PartLogEntry(part, partition.elapsed_ns, counters_snapshot),
                          deduplication_blocks_ids, ExecutionStatus(0));
      StorageReplicatedMergeTree::incrementInsertedPartsProfileEvent(part->getType());
    } catch (...) {
      auto counters_snapshot = std::make_shared<ProfileEvents::Counters::Snapshot>(partition.part_counters.getPartiallyAtomicSnapshot());
      PartLog::addNewPart(storage.getContext(), PartLog::PartLogEntry(part, partition.elapsed_ns, counters_snapshot),
                          deduplication_blocks_ids, ExecutionStatus::fromCurrentException(__PRETTY_FUNCTION__));
      throw;
    }
  }
...
UInt64 ReplicatedMergeTreeSinkPatch::getDataVersionInPartition(const String& partition_id) const {
  ...
  if (it == block_numbers.end()) throw Exception(ErrorCodes::LOGICAL_ERROR, "Not found block for partition id {}", original_partition_id);
