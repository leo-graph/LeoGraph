...
class MutatePlainMergeTreeTask : public IExecutableTask {
 public:
  MutatePlainMergeTreeTask(StorageMergeTree& storage_, StorageMetadataPtr metadata_snapshot_,
                           MergeMutateSelectedEntryPtr merge_mutate_entry_, TableLockHolder table_lock_holder_,
    ...
    for (auto& part : merge_mutate_entry->future_part->parts) priority.value += part->getBytesOnDisk();
  }
