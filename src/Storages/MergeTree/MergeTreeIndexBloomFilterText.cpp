...
MergeTreeIndexGranuleBloomFilterText::MergeTreeIndexGranuleBloomFilterText(const String &index_name_, size_t columns_number,
                                                                           const BloomFilterParameters &params_)
    : index_name(index_name_), params(params_), bloom_filters(columns_number, BloomFilter(params)), has_elems(false) {}

void MergeTreeIndexGranuleBloomFilterText::serializeBinary(WriteBuffer &ostr) const {
  if (empty()) throw Exception(ErrorCodes::LOGICAL_ERROR, "Attempt to write empty fulltext index {}.", backQuote(index_name));
...
MergeTreeIndexAggregatorBloomFilterText::MergeTreeIndexAggregatorBloomFilterText(const Names &index_columns_, const String &index_name_,
                                                                                 const BloomFilterParameters &params_,
    ...
    : index_columns(index_columns_),
      ...
      granule(std::make_shared<MergeTreeIndexGranuleBloomFilterText>(index_name, index_columns.size(), params)) {}

MergeTreeIndexGranulePtr MergeTreeIndexAggregatorBloomFilterText::getGranuleAndReset() {
  auto new_granule = std::make_shared<MergeTreeIndexGranuleBloomFilterText>(index_name, index_columns.size(), params);
...
MergeTreeConditionBloomFilterText::MergeTreeConditionBloomFilterText(const ActionsDAG::Node *predicate, ContextPtr context,
                                                                     const Block &index_sample_block, const BloomFilterParameters &params_,
  ...
  RPNBuilder<RPNElement> builder(predicate, context,
...
bool MergeTreeConditionBloomFilterText::alwaysUnknownOrTrue() const {
  return rpnEvaluatesAlwaysUnknownOrTrue(
      rpn, {RPNElement::FUNCTION_EQUALS, RPNElement::FUNCTION_NOT_EQUALS, RPNElement::FUNCTION_HAS, RPNElement::FUNCTION_IN,
            RPNElement::FUNCTION_NOT_IN, RPNElement::FUNCTION_MULTI_SEARCH, RPNElement::FUNCTION_MATCH, RPNElement::FUNCTION_HAS_ANY,
            RPNElement::FUNCTION_HAS_ALL, RPNElement::ALWAYS_FALSE});
}

/// Keep in-sync with MergeTreeIndexConditionGin::mayBeTrueOnGranuleInPart
bool MergeTreeConditionBloomFilterText::mayBeTrueOnGranule(
  ...
  if (!granule) throw Exception(ErrorCodes::LOGICAL_ERROR, "BloomFilter index condition got a granule with the wrong type.");
...
bool MergeTreeConditionBloomFilterText::extractAtomFromTree(const RPNBuilderTreeNode &node, RPNElement &out) {
  ...
  if (node.isFunction()) {
    ...
    } else if (function_name == "equals" || function_name == "notEquals" || function_name == "has" || function_name == "mapContains" ||
               function_name == "mapContainsKey" || function_name == "mapContainsKeyLike" || function_name == "mapContainsValue" ||
               function_name == "mapContainsValueLike" || function_name == "match" || function_name == "like" ||
               function_name == "notLike" || function_name.starts_with("hasToken") || function_name == "startsWith" ||
               function_name == "endsWith" || function_name == "multiSearchAny" || function_name == "hasAny" || function_name == "hasAll") {
      Field const_value;
      DataTypePtr const_type;

      if (right_argument.tryGetConstant(const_value, const_type)) {
        if (traverseTreeEquals(function_name, left_argument, const_type, const_value, out)) return true;
        ...
        if (traverseTreeEquals(function_name, right_argument, const_type, const_value, out)) return true;
      }
    }
  }
...
bool MergeTreeConditionBloomFilterText::traverseTreeEquals(const String &function_name, const RPNBuilderTreeNode &key_node,
    ...
    if (key_function_node_function_name == "arrayElement") {
      ...
      if (const auto map_keys_index = getKeyIndex(fmt::format("mapKeys({})", map_column_name))) {
        ...
        if (second_argument.tryGetConstant(const_value, const_type)) {
          ...
          if (!const_data_type.isStringOrFixedString() && !const_data_type.isArray()) return false;
        } else {
          return false;
        }
  ...
  if (!key_index && !map_key_index && !map_value_index) return false;
  ...
  if (function_name == "match") {
    ...
    if (result.required_substring.empty() && result.alternatives.empty()) return false;
    ...
    if (!result.alternatives.empty()) {
      ...
      tokenizer->substringToBloomFilter(result.required_substring.data(), result.required_substring.size(), *out.bloom_filter, false,
                                        false);
    }
...
bool MergeTreeConditionBloomFilterText::tryPrepareSetBloomFilter(const RPNBuilderTreeNode &left_argument,
                                                                 const RPNBuilderTreeNode &right_argument, RPNElement &out) {
  ...
  if (key_tuple_mapping.empty()) return false;
  ...
  for (const auto &prepared_set_data_type : prepared_set->getDataTypes()) {
    ...
    if (prepared_set_data_type_id != TypeIndex::String && prepared_set_data_type_id != TypeIndex::FixedString) return false;
  }
...
MergeTreeIndexConditionPtr MergeTreeIndexBloomFilterText::createIndexCondition(const ActionsDAG::Node *predicate,
...
MergeTreeIndexPtr bloomFilterIndexTextCreator(const IndexDescription &index) {
  static std::set<ITokenizer::Type> allowed_tokenizers = {ITokenizer::Type::Ngrams, ITokenizer::Type::SplitByNonAlpha,
  ...
  BloomFilterParameters params(args[first_bf_param_idx].safeGet<size_t>(), args[first_bf_param_idx + 1].safeGet<size_t>(),
...
void bloomFilterIndexTextValidator(const IndexDescription &index, bool /*attach*/) {
  for (const auto &index_data_type : index.data_types) {
    ...
    if (!data_type.isString() && !data_type.isFixedString() && !data_type.isIPv6())
      throw Exception(ErrorCodes::BAD_ARGUMENTS,
                      "Ngram and token bloom filter indexes can only be used with column types `String`, `FixedString`, "
                      "`LowCardinality(String)`, `LowCardinality(FixedString)`, `Array(String)` or `Array(FixedString)`");
  }
  ...
  if (index.type == NgramsTokenizer::getName()) {
    if (args.size() != 4) throw Exception(ErrorCodes::BAD_ARGUMENTS, "`ngrambf` index must have exactly 4 arguments");

    UInt64 ngram_length = args[0].safeGet<UInt64>();
    if (ngram_length < 1) throw Exception(ErrorCodes::BAD_ARGUMENTS, "ngram length must be at least 1");

    first_bf_param_idx = 1;
  } else if (index.type == SplitByNonAlphaTokenizer::getName()) {
    if (args.size() != 3) throw Exception(ErrorCodes::BAD_ARGUMENTS, "`tokenbf` index must have exactly 3 arguments");
