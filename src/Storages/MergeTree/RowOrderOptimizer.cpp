...
bool haveEqualSortingKeyValues(const Block& block, const SortDescription& sort_description, size_t left_row, size_t right_row) {
  for (const auto& sort_column : sort_description) {
    ...
    if (column.compareAt(left_row, right_row, column, 1) != 0) return false;
  }
  return true;
}
...
std::vector<size_t> getOtherColumnIndexes(const Block& block, const SortDescription& sort_description) {
  ...
  if (sorting_key_columns_count == 0) {
    ...
    std::set_difference(all_column_indexes.begin(), all_column_indexes.end(), sorted_column_indexes.begin(), sorted_column_indexes.end(),
...
/// Returns a set of equal row ranges (equivalence classes) with the same row values for all sorting key columns (usually primary key
/// columns.) Example with 2 PK columns, 2 other columns --> 3 equal ranges
///          pk1    pk2    c1    c2
///          ----------------------
///          1      1     a     b
///          1      1     b     e
...
EqualRanges getEqualRanges(const Block& block, const SortDescription& sort_description, const IColumn::Permutation& permutation) {
  ...
  if (sort_description.empty()) {
    ...
    for (size_t i = 0; i < rows;) {
      size_t j = i;
      while (j < rows && haveEqualSortingKeyValues(block, sort_description, permutation[i], permutation[j])) ++j;
...
std::vector<size_t> getCardinalitiesInPermutedRange(const Block& block, const std::vector<size_t>& other_column_indexes,
                                                    const IColumn::Permutation& permutation, const EqualRange& equal_range) {
...
void updatePermutationInEqualRange(const Block& block, const std::vector<size_t>& other_column_indexes, IColumn::Permutation& permutation,
                                   const EqualRange& equal_range, const std::vector<size_t>& cardinalities, const LoggerPtr& log) {
  ...
  for (size_t i : column_order) {
    ...
    column->updatePermutation(IColumn::PermutationSortDirection::Ascending, IColumn::PermutationSortStability::Stable, 0, 1, permutation,
                              ranges);
  }
...
void RowOrderOptimizer::optimize(const Block& block, const SortDescription& sort_description, IColumn::Permutation& permutation) {
  ...
  LOG_TRACE(log, "columns: {}, sorting key columns: {}, rows: {}, equal ranges: {}", block.columns(), sort_description.size(), block.rows(),
            equal_ranges.size());

  for (const auto& equal_range : equal_ranges) {
    if (equal_range.size() <= 1) continue;
