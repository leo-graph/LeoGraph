...
bool ReplicatedMergeTreeQueue::isGoingToBeDroppedImpl(const MergeTreePartInfo &part_info, MergeTreePartInfo *out_drop_range_info) const {
  ...
  if (!covering_virtual.empty()) {
    ...
    if (covering_virtual_info.isFakeDropRangePart()) {
      if (out_drop_range_info) *out_drop_range_info = covering_virtual_info;
      return true;
    }
  }
...
bool ReplicatedMergeTreeQueue::load(zkutil::ZooKeeperPtr zookeeper) {
...

  {
    ...
    {
      std::lock_guard lock(state_mutex);
      for (const LogEntryPtr &log_entry : queue) already_loaded_paths.insert(log_entry->znode_name);
    }

    Strings children = zookeeper->getChildren(queue_path);

    size_t removed_entries = std::erase_if(children, [&](const String &path) { return already_loaded_paths.count(path); });
    ...
    for (const String &child : children) paths.emplace_back(fs::path(queue_path) / child);
...
void ReplicatedMergeTreeQueue::createLogEntriesToFetchBrokenParts() {
  ...
  for (const auto &broken_part_name : broken_parts) storage.removePartAndEnqueueFetch(broken_part_name, /* storage_init = */ true);
...
bool ReplicatedMergeTreeQueue::isIntersectingWithDropReplaceIntent(const LogEntry &entry, const String &part_name, String &out_reason,
                                                                   std::unique_lock<std::mutex> & /*state_mutex lock*/) const {
  const auto part_info = MergeTreePartInfo::fromPartName(part_name, format_version);
  for (const auto &intent : drop_replace_range_intents) {
    if (!intent.isDisjoint(part_info)) {
      constexpr auto fmt_string =
          "Not executing {} of type {} for part {} (actual part {})"
          "because there is a drop or replace intent with part name {}.";
      LOG_INFO(LogToStr(out_reason, log), fmt_string, entry.znode_name, entry.type, entry.new_part_name, part_name,
...
bool ReplicatedMergeTreeQueue::isMergeOfPatchPartsBlocked(const LogEntry &entry, String &out_reason,
                                                          std::unique_lock<std::mutex> & /*state_mutex_lock*/) const {
  if (entry.type != LogEntry::MERGE_PARTS || !storage.supportsLightweightUpdate()) return false;

  auto new_part_info = MergeTreePartInfo::fromPartName(entry.new_part_name, format_version);

  if (!new_part_info.isPatch()) return false;

  auto source_parts_set = NameSet(entry.source_parts.begin(), entry.source_parts.end());

  for (const auto &merge_entry : queue) {
    if (merge_entry->type != LogEntry::MERGE_PARTS) continue;

    for (const auto &patch_part : merge_entry->patch_parts) {
      if (source_parts_set.contains(patch_part)) {
        constexpr auto fmt_string =
            "Not executing log entry {} for patch part {} because source patch part {} is scheduled to be applied in merge for {}.";
...
bool ReplicatedMergeTreeQueue::isDropOfPatchPartBlocked(const LogEntry &entry, String &out_reason,
                                                        std::unique_lock<std::mutex> & /*state_mutex_lock*/) const {
  if (entry.type != LogEntry::DROP_PART || !storage.supportsLightweightUpdate()) return false;

  auto dropped_info = MergeTreePartInfo::fromPartName(entry.new_part_name, format_version);

  if (!dropped_info.isPatch()) return false;

  auto original_partition_id = dropped_info.getOriginalPartitionId();

  for (const auto &merge_mutate_entry : queue) {
    if (merge_mutate_entry->type != LogEntry::MERGE_PARTS && merge_mutate_entry->type != LogEntry::MUTATE_PART) continue;
    ...
    if (new_part_info.getPartitionId() == original_partition_id && new_part_info.getDataVersion() > dropped_info.getDataVersion()) {
      constexpr auto fmt_string =
          "Not executing log entry {} for patch part {} because the merge or mutation (with result part {}) that triggered the drop of the "
          "patch part has not been executed yet.";
...
bool ReplicatedMergeTreeQueue::havePendingPatchPartsForMutation(const LogEntry &entry, String &out_reason,
                                                                const CommittingBlocks &committing_blocks,
                                                                std::unique_lock<std::mutex> & /*state_mutex_lock*/) const {
  if (entry.type != LogEntry::MUTATE_PART || !storage.supportsLightweightUpdate()) return false;
  ...
  if (it != committing_blocks.end()) {
    for (const auto &block : it->second) {
      if (block.number > data_version) break;

      if (block.op == CommittingBlock::Op::Update) {
        constexpr auto fmt_string =
            "Not executing log entry {} for part {} because lightweight update with data version {} is not committed yet";
  ...
  for (const auto &fetch_entry : queue) {
    if (fetch_entry->type != LogEntry::GET_PART) continue;
...
void ReplicatedMergeTreeQueue::insertUnlocked(const LogEntryPtr &entry, std::optional<time_t> &min_unprocessed_insert_time_changed,
  ...
  for (const String &virtual_part_name : entry_virtual_parts) {
    ...
    if (part_info.isFakeDropRangePart()) continue;
...
void ReplicatedMergeTreeQueue::updateStateOnQueueEntryRemoval(const LogEntryPtr &entry, bool is_successful,
                                                              std::optional<time_t> &min_unprocessed_insert_time_changed,
                                                              std::optional<time_t> &max_processed_insert_time_changed,
                                                              std::unique_lock<std::mutex> &state_lock) {
  auto entry_virtual_parts = entry->getVirtualPartNames(format_version);
  LOG_TEST(log, "Removing {} entry {} from queue with type {}", is_successful ? "successful" : "unsuccessful", entry->znode_name,
...
void ReplicatedMergeTreeQueue::removeCoveredPartsFromMutations(const String &part_name, bool remove_part, bool remove_covered_parts) {
  ...
  LOG_TEST(log, "Removing part {} from mutations (remove_part: {}, remove_covered_parts: {})", part_name, remove_part,
           remove_covered_parts);

  auto in_partition = mutations_by_partition.find(part_info.getPartitionId());
  if (in_partition == mutations_by_partition.end()) return;
  ...
  for (auto &it : in_partition->second) {
    ...
    if (status.parts_to_do.size() == 0) some_mutations_are_probably_done = true;
  ...
  if (some_mutations_are_probably_done) storage.mutations_finalizing_task->schedule();
}
...
void ReplicatedMergeTreeQueue::addPartInProgressToMutations(const String &part_name, const MergeTreePartInfo &part_info,
...
void ReplicatedMergeTreeQueue::removePartInProgressFromMutations(const String &part_name, const MergeTreePartInfo &part_info,
                                                                 const MergeTreePartInfo &new_part_info) {
...
void ReplicatedMergeTreeQueue::updateTimesInZooKeeper(zkutil::ZooKeeperPtr zookeeper,
                                                      std::optional<time_t> min_unprocessed_insert_time_changed,
  ...
  if (min_unprocessed_insert_time_changed)
    ops.emplace_back(
        zkutil::makeSetRequest(replica_path + "/min_unprocessed_insert_time", toString(*min_unprocessed_insert_time_changed), -1));

  if (max_processed_insert_time_changed)
    ops.emplace_back(zkutil::makeSetRequest(replica_path + "/max_processed_insert_time", toString(*max_processed_insert_time_changed), -1));
  ...
  if (!ops.empty()) {
    ...
    if (code != Coordination::Error::ZOK)
      LOG_ERROR(log,
                "Couldn't set value of nodes for insert times "
...
void ReplicatedMergeTreeQueue::removeProcessedEntry(zkutil::ZooKeeperPtr zookeeper, LogEntryPtr &entry) {
  ...
  /// First remove from memory then from ZooKeeper
  {
    ...
    if (entry->removed_by_other_entry) {
      ...
      for (Queue::iterator it = queue.end(); it != queue.begin();) {
        ...
        if (*it == entry) {
          found = true;
          updateStateOnQueueEntryRemoval(entry, /* is_successful = */ true, min_unprocessed_insert_time_changed,
                                         max_processed_insert_time_changed, lock);
  ...
  if (!found && need_remove_from_zk)
    throw Exception(ErrorCodes::LOGICAL_ERROR, "Can't find {} in the memory queue. It is a bug. Entry: {}", entry->znode_name,
                    entry->toString());
  notifySubscribers(queue_size, &(entry->znode_name));

  if (!need_remove_from_zk) return;
...
std::pair<int32_t, int32_t> ReplicatedMergeTreeQueue::pullLogsToQueue(zkutil::ZooKeeperPtr zookeeper,
  ...
  if (reason != LOAD && reason != FIX_METADATA_VERSION) {
    ...
    if (not_completely_initialized)
      throw Exception(ErrorCodes::LOGICAL_ERROR, "Tried to pull logs to queue (reason: {}) on readonly replica {}, it's a bug", reason,
                      storage.getStorageID().getNameForLogs());
  }

  if (pull_log_blocker.isCancelled()) throw Exception(ErrorCodes::ABORTED, "Log pulling is cancelled");
  ...
  if (!log_entries.empty()) {
    ...
    for (size_t entry_idx = 0, num_entries = log_entries.size(); entry_idx < num_entries;) {
      ...
      auto end = entry_idx + current_multi_batch_size >= log_entries.size() ? log_entries.end() : (begin + current_multi_batch_size);
      auto last = end - 1;
      ...
      if (!startsWith(last_entry, "log-"))
        throw Exception(ErrorCodes::UNEXPECTED_NODE_IN_ZOOKEEPER, "Error in zookeeper data: unexpected node {} in {}/log", last_entry,
                        zookeeper_path);
      ...
      for (size_t i = 0; i < get_num; ++i) {
        ...
        ops.emplace_back(
            zkutil::makeCreateRequest(fs::path(replica_path) / "queue/queue-", res.data, zkutil::CreateMode::PersistentSequential));
      ...
      ops.emplace_back(zkutil::makeSetRequest(fs::path(replica_path) / "log_pointer", toString(last_entry_index + 1), -1));

      if (min_unprocessed_insert_time_changed)
        ops.emplace_back(zkutil::makeSetRequest(fs::path(replica_path) / "min_unprocessed_insert_time",
                                                toString(*min_unprocessed_insert_time_changed), -1));
      ...
      try {
        ...
        for (size_t copied_entry_idx = 0, num_copied_entries = copied_entries.size(); copied_entry_idx < num_copied_entries;
             ++copied_entry_idx) {
...
PartitionQueueRepresentation getQueueRepresentation(const std::list<ReplicatedMergeTreeLogEntryPtr> &entries,
    ...
    switch (entry->type) {
      ...
      case LogEntryType::MUTATE_PART: {
        representations_by_znode[znode_name].produced_parts.push_back(
            MergeTreePartInfo::fromPartName(entry->new_part_name, format_version));
        break;
      }
...
/// After that we just update parts_to_do for each mutation when pulling entries into our queue (addPartToMutations,
/// removePartFromMutations).
ActiveDataPartSet getPartNamesToMutate(const ReplicatedMergeTreeMutationEntry &mutation, const ActiveDataPartSet &current_parts,
                                       const PartitionQueueRepresentation &queue_representation,
                                       MergeTreeDataFormatVersion format_version) {
  ...
  for (const auto &[partition_id, block_num] : mutation.block_numbers) {
    ...
    MergeTreePartInfo covering_part_info(partition_id, 0, block_num, MergeTreePartInfo::MAX_LEVEL, MergeTreePartInfo::MAX_BLOCK_NUMBER);
    ...
    for (const String &covered_part_name : current_parts.getPartsCoveredBy(covering_part_info)) {
      ...
      if (part_info.getDataVersion() < block_num) result.add(covered_part_name);
    }

    /// Traverse queue and update affected current_parts
    if (!queue_representation.contains(partition_id)) continue;
...
int32_t ReplicatedMergeTreeQueue::updateMutations(zkutil::ZooKeeperPtr zookeeper, Coordination::WatchCallbackPtr watch_callback) {
  ...
  bool some_active_mutations_were_killed = false;
  {
    ...
    for (auto it = mutations_by_znode.begin(); it != mutations_by_znode.end();) {
      ...
      if (!entries_in_zk_set.contains(entry.znode_name)) {
        ...
        for (const auto &partition_and_block_num : entry.block_numbers) {
          ...
          if (in_partition.empty()) mutations_by_partition.erase(partition_and_block_num.first);
        }
    ...
    for (const String &znode : entries_in_zk_set) {
      if (!mutations_by_znode.contains(znode)) entries_to_load.push_back(znode);
    }
  }

  if (some_active_mutations_were_killed) storage.background_operations_assignee.trigger();

  if (!entries_to_load.empty()) {
    LOG_INFO(log, "Loading {} mutation entries: {} - {}", toString(entries_to_load.size()), entries_to_load.front(),
             entries_to_load.back());

    std::vector<String> entry_paths;
    entry_paths.reserve(entries_to_load.size());

    for (const String &entry : entries_to_load) entry_paths.emplace_back(fs::path(zookeeper_path) / "mutations" / entry);
    ...
    for (size_t i = 0; i < entries_to_load.size(); ++i) {
      ...
      if (maybe_response.error != Coordination::Error::ZOK) {
        ...
        LOG_WARNING(log, "Cannot get mutation node {} ({}), probably it was concurrently removed", entries_to_load[i],
                    maybe_response.error);
        continue;
      }
    ...
    bool some_mutations_are_probably_done = false;
    {
      ...
      for (const ReplicatedMergeTreeMutationEntryPtr &entry : new_mutations) {
        ...
        LOG_TRACE(log, "Adding mutation {} for {} partitions (data versions: {})", entry->znode_name, entry->block_numbers.size(),
                  entry->getBlockNumbersForLogs());
        ...
        if (mutation.parts_to_do.size() == 0) some_mutations_are_probably_done = true;
...
ReplicatedMergeTreeMutationEntryPtr ReplicatedMergeTreeQueue::removeMutation(zkutil::ZooKeeperPtr zookeeper, const String &mutation_id) {
  std::lock_guard lock(update_mutations_mutex);

  auto rc = zookeeper->tryRemove(fs::path(zookeeper_path) / "mutations" / mutation_id);
  if (rc == Coordination::Error::ZOK) LOG_DEBUG(log, "Removed mutation {} from ZooKeeper.", mutation_id);
  ...
  bool mutation_was_active = false;
  {
    ...
    if (it == mutations_by_znode.end()) return nullptr;
  ...
  if (mutation_was_active) storage.background_operations_assignee.trigger();
...
ReplicatedMergeTreeQueue::StringSet ReplicatedMergeTreeQueue::moveSiblingPartsForMergeToEndOfQueue(const String &part_name) {
  ...
  for (Queue::iterator it = queue.begin(); it != queue.end(); ++it) {
    if ((*it)->type == LogEntry::MERGE_PARTS || (*it)->type == LogEntry::MUTATE_PART) {
      if (std::find((*it)->source_parts.begin(), (*it)->source_parts.end(), part_name) != (*it)->source_parts.end()) {
  ...
  if (!parts_for_merge.empty()) {
    ...
    for (Queue::iterator it = queue.begin(); it != queue.end();) {
      ...
      if (it0 == merge_entry) break;

      const auto t = (*it0)->type;

      if ((t == LogEntry::MERGE_PARTS || t == LogEntry::GET_PART || t == LogEntry::ATTACH_PART || t == LogEntry::MUTATE_PART) &&
...
bool ReplicatedMergeTreeQueue::checkReplaceRangeCanBeRemoved(const MergeTreePartInfo &part_info, LogEntryPtr entry_ptr,
                                                             const ReplicatedMergeTreeLogEntryData &current) const {
  if (entry_ptr->type != LogEntry::REPLACE_RANGE) return false;
  assert(entry_ptr->replace_range_entry);

  if (current.type != LogEntry::REPLACE_RANGE && current.type != LogEntry::DROP_RANGE && current.type != LogEntry::DROP_PART) return false;
  ...
  for (const String &new_part_name : entry_ptr->replace_range_entry->new_part_names) {
    if (part_info.contains(MergeTreePartInfo::fromPartName(new_part_name, format_version))) ++number_of_covered_parts;
  }
...
void ReplicatedMergeTreeQueue::removePartProducingOpsInRange(zkutil::ZooKeeperPtr zookeeper, const MergeTreePartInfo &part_info,
  ...
  [[maybe_unused]] bool called_from_alter_query_directly =
      covering_entry && covering_entry->replace_range_entry && covering_entry->replace_range_entry->columns_version < 0;
  ...
  for (Queue::iterator it = queue.begin(); it != queue.end();) {
    auto type = (*it)->type;
    bool is_simple_producing_op =
        type == LogEntry::GET_PART || type == LogEntry::ATTACH_PART || type == LogEntry::MERGE_PARTS || type == LogEntry::MUTATE_PART;
    bool simple_op_covered =
        is_simple_producing_op && part_info.contains(MergeTreePartInfo::fromPartName((*it)->new_part_name, format_version));
    ...
    if (simple_op_covered || replace_range_covered) {
      ...
      if ((*it)->currently_executing) to_wait.push_back(*it);
      ...
      updateStateOnQueueEntryRemoval(*it, /* is_successful = */ false, min_unprocessed_insert_time_changed,
                                     max_processed_insert_time_changed, lock);
...
void ReplicatedMergeTreeQueue::waitForCurrentlyExecutingOpsInRange(const MergeTreePartInfo &part_info) const {
  ...
  for (const auto &entry : queue) {
    if (!entry->currently_executing) continue;
...
bool ReplicatedMergeTreeQueue::isCoveredByFuturePartsImpl(const LogEntry &entry, const String &new_part_name, String &out_reason,
  ...
  if (entry_for_same_part_it != future_parts.end()) {
    ...
    constexpr auto fmt_string =
        "Not executing log entry {} of type {} for part {} (actual part {})"
  ...
  for (const auto &future_part_elem : future_parts) {
    ...
    if (future_part.isDisjoint(result_part)) continue;
    ...
    if (result_part.isFakeDropRangePart() && result_part.contains(future_part)) continue;
    ...
    if (covered_entries_to_wait) {
      if (entry.znode_name < future_part_elem.second->znode_name) {
        ...
        LOG_TRACE(LogToStr(out_reason, log), fmt_string, entry.znode_name, new_part_name, future_part_elem.first,
                  future_part_elem.second->znode_name);
        return true;
      }
...
bool ReplicatedMergeTreeQueue::addFuturePartIfNotCoveredByThem(const String &part_name, LogEntry &entry, String &reject_reason) {
  ...
  if (drop_parts.isAffectedByDropPart(part_name, reject_reason)) return false;

  std::vector<LogEntryPtr> covered_entries_to_wait;
  if (isCoveredByFuturePartsImpl(entry, part_name, reject_reason, lock, &covered_entries_to_wait)) return false;
...
UInt64 ReplicatedMergeTreeQueue::getPostponeTimeMsForEntry(const LogEntry &entry, const MergeTreeData &data) const {
  ...
  if (!postpone_time_upper_bound_ms) return postpone_time_upper_bound_ms;
...
bool ReplicatedMergeTreeQueue::shouldExecuteLogEntry(const LogEntry &entry, String &out_postpone_reason,
  ...
  if (auto postpone_time = getPostponeTimeMsForEntry(entry, data)) {
    constexpr auto fmt_string =
        "Not executing log entry {} of type {} "
  ...
  for (const String &new_part_name : entry.getVirtualPartNames(format_version)) {
    ...
    if (isIntersectingWithDropReplaceIntent(entry, new_part_name, out_postpone_reason, state_lock)) return false;
  }
  ...
  if (entry.type != LogEntry::DROP_RANGE && entry.type != LogEntry::DROP_PART) {
    ...
    if (drop_parts.isAffectedByDropPart(entry, out_postpone_reason)) return false;
  }
  ...
  if (!entry.new_part_name.empty()) {
    ...
    if (entry.type != LogEntry::DROP_PART && !new_part_info.isFakeDropRangePart() && isGoingToBeDroppedImpl(new_part_info, &drop_info)) {
      out_postpone_reason = fmt::format("Not executing {} because it produces part {} that is going to be dropped by {}", entry.znode_name,
                                        entry.new_part_name, drop_info.getPartNameForLogs());
      return false;
    }
  }

  /// Check that fetches pool is not overloaded
  if ((entry.type == LogEntry::GET_PART || entry.type == LogEntry::ATTACH_PART) && !storage.canExecuteFetch(entry, out_postpone_reason)) {
  ...
  if (entry.type == LogEntry::MERGE_PARTS || entry.type == LogEntry::MUTATE_PART) {
    ...
    for (const auto &name : entry.source_parts) {
      ...
      auto part =
          data.getPartIfExists(name, {MergeTreeDataPartState::PreActive, MergeTreeDataPartState::Active, MergeTreeDataPartState::Outdated});
      if (part) {
    ...
    if (havePendingPatchPartsForMutation(entry, out_postpone_reason, committing_blocks, state_lock)) return false;
    ...
    if (entry.type == LogEntry::MERGE_PARTS) {
      ...
      if (isMergeOfPatchPartsBlocked(entry, out_postpone_reason, state_lock)) return false;
    }
    ...
    if (!ignore_max_size && sum_parts_size_in_bytes > max_source_parts_size) {
      ...
      LOG_DEBUG(LogToStr(out_postpone_reason, LogFrequencyLimiter(log, 5)), fmt_string, entry.znode_name, entry.typeToString(),
                entry.new_part_name, ReadableSize(sum_parts_size_in_bytes), ReadableSize(max_source_parts_size));
  ...
  if (entry.type == LogEntry::ALTER_METADATA) {
    ...
    if (!database->canExecuteReplicatedMetadataAlter()) {
      LOG_TRACE(LogToStr(out_postpone_reason, log),
                "Cannot execute alter metadata {} with version {} "
                "because database {} cannot process metadata alters now",
                entry.znode_name, entry.alter_version, database_name);
      return false;
    }
  }
  ...
  if (auto drop_range = entry.getDropRange(format_version)) {
    ...
    for (const auto &info : currently_executing_drop_replace_ranges) {
      if (drop_range_info.isDisjoint(info)) continue;
      constexpr auto fmt_string =
          "Not executing log entry {} of type {} for part {} "
          "because another DROP_RANGE or REPLACE_RANGE entry with not disjoint range {} is currently executing.";
      LOG_TRACE(LogToStr(out_postpone_reason, log), fmt_string, entry.znode_name, entry.typeToString(), entry.new_part_name,
  ...
  if (entry.type == LogEntry::DROP_PART) {
    ...
    for (const auto &replace_entry : queue) {
      if (replace_entry->type != LogEntry::REPLACE_RANGE) continue;
      ...
      for (const auto &new_part_name : replace_entry->replace_range_entry->new_part_names) {
        ...
        if (!new_part_info.isDisjoint(drop_part_info)) {
          ...
          LOG_TRACE(LogToStr(out_postpone_reason, log), fmt_string, entry.znode_name, entry.typeToString(), entry.new_part_name,
                    replace_entry->znode_name);
          return false;
        }
      }
    }

    if (isDropOfPatchPartBlocked(entry, out_postpone_reason, state_lock)) return false;
  }
...
Int64 ReplicatedMergeTreeQueue::getCurrentMutationVersion(const String &partition_id, Int64 data_version) const {
  ...
  if (in_partition == mutations_by_partition.end()) return 0;

  auto it = in_partition->second.upper_bound(data_version);
  if (it == in_partition->second.begin()) return 0;

  --it;
  return it->first;
}

Int64 ReplicatedMergeTreeQueue::getNextMutationVersion(const String &partition_id, Int64 data_version) const {
  auto in_partition = mutations_by_partition.find(partition_id);
  if (in_partition == mutations_by_partition.end()) return 0;

  auto it = in_partition->second.upper_bound(data_version);
  if (it == in_partition->second.end()) return 0;

  return it->first;
}

ReplicatedMergeTreeQueue::CurrentlyExecuting::CurrentlyExecuting(const ReplicatedMergeTreeQueue::LogEntryPtr &entry_,
...
void ReplicatedMergeTreeQueue::CurrentlyExecuting::setActualPartName(ReplicatedMergeTreeQueue::LogEntry &entry,
                                                                     ...
                                                                     std::unique_lock<std::mutex> &state_lock,
                                                                     std::vector<LogEntryPtr> &covered_entries_to_wait) {
  if (actual_part_name.empty()) throw Exception(ErrorCodes::LOGICAL_ERROR, "Actual part name is empty");

  if (!entry.actual_new_part_name.empty())
    throw Exception(ErrorCodes::LOGICAL_ERROR, "Entry {} actual part isn't empty yet: '{}'. This is a bug.", entry.znode_name,
  ...
  if (actual_part_name == entry.new_part_name) return;

  if (!queue.future_parts.emplace(actual_part_name, entry.shared_from_this()).second)
    throw Exception(ErrorCodes::LOGICAL_ERROR,
                    "Attaching already existing future part {}. This is a bug. "
                    "It happened on attempt to execute {}: {}",
  ...
  for (LogEntryPtr &covered_entry : covered_entries_to_wait) {
    if (&entry == covered_entry.get()) continue;
...
ReplicatedMergeTreeQueue::CurrentlyExecuting::~CurrentlyExecuting() {
  ...
  for (const String &new_part_name : entry->getVirtualPartNames(queue.format_version)) erase_and_check(new_part_name);

  if (!entry->actual_new_part_name.empty()) erase_and_check(entry->actual_new_part_name);

  entry->actual_new_part_name.clear();

  for (const auto &actual_part : entry->replace_range_actual_new_part_names) erase_and_check(actual_part);

  entry->replace_range_actual_new_part_names.clear();
}

ReplicatedMergeTreeQueue::SelectedEntryPtr ReplicatedMergeTreeQueue::selectEntryToProcess(MergeTreeDataMergerMutator &merger_mutator,
                                                                                          MergeTreeData &data) {
  LogEntryPtr entry;
  ...
  if (entry) return std::make_shared<SelectedEntry>(entry, std::unique_ptr<CurrentlyExecuting>{new CurrentlyExecuting(entry, *this, lock)});
  return {};
}

bool ReplicatedMergeTreeQueue::processEntry(std::function<zkutil::ZooKeeperPtr()> get_zookeeper, LogEntryPtr &entry,
  ...
  try {
    ...
    if (func(entry)) removeProcessedEntry(get_zookeeper(), entry);
  } catch (...) {
...
ReplicatedMergeTreeQueue::OperationsInQueue ReplicatedMergeTreeQueue::countMergesAndPartMutations() const {
  ...
  for (const auto &entry : queue) {
    if (entry->type == ReplicatedMergeTreeLogEntry::MERGE_PARTS) {
      ++count_merges;
      if (isTTLMergeType(entry->merge_type)) ++count_merges_with_ttl;
...
ReplicatedMergeTreeQueue::MutationsSnapshot::MutationsSnapshot(Params params_, MutationCounters counters_,
                                                               MutationsByPartititon mutations_by_partition_, DataPartsVector patches_)
...
MutationCommands ReplicatedMergeTreeQueue::MutationsSnapshot::getOnFlyMutationCommandsForPart(
    const MergeTreeData::DataPartPtr &part) const {
  ...
  if (seen_all_data_mutations && seen_all_metadata_mutations) return {};
  ...
  for (const auto &[mutation_version, entry] : in_partition->second | std::views::reverse) {
    if (seen_all_data_mutations && seen_all_metadata_mutations) break;

    auto alter_version = entry->alter_version;

    if (alter_version != -1) {
      if (seen_all_metadata_mutations || alter_version > params.metadata_version) continue;
...
MergeTreeData::MutationsSnapshotPtr ReplicatedMergeTreeQueue::getMutationsSnapshot(const MutationsSnapshot::Params &params) const {
  ...
  if (params.need_patch_parts) patch_parts = storage.getPatchPartsVectorForInternalUsage();
  ...
  for (const auto &[partition_id, mutations] : mutations_by_partition) {
    if (partition_id.starts_with(MergeTreePartInfo::PATCH_PART_PREFIX)) continue;
    ...
    const int64_t max_mutation_version_to_include =
        MergeTreeData::IMutationsSnapshot::getMaxMutationVersionForPartition(params, partition_id);
    ...
    for (const auto &[mutation_version, status] : mutations | std::views::reverse) {
      ...
      if (mutation_version > max_mutation_version_to_include) continue;
...
MutationCommands ReplicatedMergeTreeQueue::getMutationCommands(const MergeTreeData::DataPartPtr &part, Int64 desired_mutation_version,
  ...
  if (in_partition == mutations_by_partition.end()) {
    LOG_WARNING(log, "There are no mutations for partition ID {} (trying to mutate part {} to {})", part->info.getPartitionId(), part->name,
  ...
  if (end == in_partition->second.end() || end->first != desired_mutation_version)
    LOG_WARNING(log, "Mutation with version {} not found in partition ID {} (trying to mutate part {})", desired_mutation_version,
                part->info.getPartitionId(), part->name);
  else
    ++end;
...
bool ReplicatedMergeTreeQueue::tryFinalizeMutations(zkutil::ZooKeeperPtr zookeeper) {
    ...
    for (auto &kv : mutations_by_znode) {
      ...
      if (znode <= mutation_pointer) {
        ...
        if (mutation.parts_to_do.size() != 0) {
          LOG_INFO(log,
                   "Seems like we jumped over mutation {} when downloaded part with bigger mutation number. "
                   "It's OK, tasks for rest parts will be skipped, but probably a lot of mutations "
                   "were executed concurrently on different replicas.",
                   znode);
  ...
  for (const auto &candidate : candidates)
    for (const auto &partitions : candidate->block_numbers)
      if (!candidate->checked_partitions_cache.contains(partitions.first)) partition_ids_hint.insert(partitions.first);
...
void ReplicatedMergeTreeQueue::getEntries(LogEntriesData &res) const {
  ...
  for (const auto &entry : queue) res.emplace_back(*entry);
}
...
std::optional<MergeTreeMutationStatus> ReplicatedMergeTreeQueue::getIncompleteMutationsStatus(const String &znode_name,
                                                                                              std::set<String> *mutation_ids) const {
  std::lock_guard lock(state_mutex);
  auto current_mutation_it = mutations_by_znode.find(znode_name);
  /// killed
  if (current_mutation_it == mutations_by_znode.end()) return {};
...
ReplicatedMergeTreeQueue::SubscriberHandler ReplicatedMergeTreeQueue::addSubscriber(ReplicatedMergeTreeQueue::SubscriberCallBack &&callback,
  ...
  if (sync_mode != SyncReplicaMode::PULL) {
    ...
    static constexpr std::array lightweight_entries = {LogEntry::GET_PART, LogEntry::ATTACH_PART, LogEntry::DROP_RANGE,
                                                       LogEntry::REPLACE_RANGE, LogEntry::DROP_PART};
    ...
    if (!src_replicas.empty()) {
      ...
      for (const auto &host : unfiltered_hosts) existing_replicas.emplace(host);
      ...
      for (const auto &requested_replica : src_replicas) {
        if (!existing_replicas.contains(requested_replica)) {
          throw Exception(ErrorCodes::BAD_ARGUMENTS,
                          ...
                          requested_replica, zookeeper_path, fmt::join(unfiltered_hosts, ", "));
        }
      }
    }
    ...
    for (const auto &entry : queue) {
      bool entry_matches = !lightweight_entries_only ||
...
void ReplicatedMergeTreeQueue::notifySubscribersOnPartialShutdown() {
  ...
  std::lock_guard lock_subscribers(subscribers_mutex);
  for (auto &subscriber_callback : subscribers) subscriber_callback(queue_size, nullptr);
}
...
void ReplicatedMergeTreeQueue::notifySubscribers(size_t new_queue_size, const String *removed_log_entry_id) {
  std::lock_guard lock_subscribers(subscribers_mutex);
  for (auto &subscriber_callback : subscribers) subscriber_callback(new_queue_size, removed_log_entry_id);
}
