...
void ReplicatedMergeTreeTableMetadata::read(ReadBuffer& in) {
  ...
  if (checkString("constraints: ", in)) in >> constraints >> "\n";
  ...
  if (merge_params_version >= REPLICATED_MERGE_TREE_METADATA_WITH_ALL_MERGE_PARAMETERS) {
    if (checkString("version column: ", in)) in >> version_column >> "\n";
...
static void handleTableMetadataMismatch(const std::string& table_name_for_error_message, std::string_view differs_in,
                                        const auto& stored_in_zk, const std::string& parsed_from_zk, const auto& local,
                                        bool strict_check = true, LoggerPtr logger = nullptr) {
  ...
  if (strict_check)
    throw Exception(ErrorCodes::METADATA_MISMATCH, "Metadata of table {} in ZooKeeper differs in {}. {}", table_name_for_error_message,
                    differs_in, metadata_string);

  if (logger)
    LOG_WARNING(logger, "Metadata of table {} in ZooKeeper differs in {}. {}", table_name_for_error_message, differs_in, metadata_string);
};

void ReplicatedMergeTreeTableMetadata::checkImmutableFieldsEquals(const ReplicatedMergeTreeTableMetadata& from_zk,
                                                                  const ColumnsDescription& columns,
                                                                  const std::string& table_name_for_error_message, ContextPtr context,
  ...
  if (check_index_granularity && index_granularity != from_zk.index_granularity)
    handleTableMetadataMismatch(table_name_for_error_message, "index granularity", DB::toString(from_zk.index_granularity), "",
                                DB::toString(index_granularity));

  if (merging_params_mode != from_zk.merging_params_mode)
    handleTableMetadataMismatch(table_name_for_error_message, "mode of merge operation", DB::toString(from_zk.merging_params_mode), "",
  ...
  if (merge_params_version >= REPLICATED_MERGE_TREE_METADATA_WITH_ALL_MERGE_PARAMETERS &&
      from_zk.merge_params_version >= REPLICATED_MERGE_TREE_METADATA_WITH_ALL_MERGE_PARAMETERS) {
  ...
  ///    in formatAST code.
  String parsed_zk_primary_key =
      formattedAST(KeyDescription::parse(from_zk.primary_key, columns, context, true).getOriginalExpressionList());
  ...
  if (data_format_version != from_zk.data_format_version)
    handleTableMetadataMismatch(table_name_for_error_message, "data format version",
                                DB::toString(from_zk.data_format_version.toUnderType()), "",
  ...
  if (partition_key != parsed_zk_partition_key)
    handleTableMetadataMismatch(table_name_for_error_message, "partition key expression", from_zk.partition_key, parsed_zk_partition_key,
                                partition_key);
}

bool ReplicatedMergeTreeTableMetadata::checkEquals(const ReplicatedMergeTreeTableMetadata& from_zk, const ColumnsDescription& columns,
                                                   const std::string& table_name_for_error_message, ContextPtr context,
                                                   bool check_index_granularity, bool strict_check, LoggerPtr logger) const {
  bool is_equal = true;
  checkImmutableFieldsEquals(from_zk, columns, table_name_for_error_message, context, check_index_granularity);

  String parsed_zk_sampling_expression =
      formattedAST(KeyDescription::parse(from_zk.sampling_expression, columns, context, false).definition_ast);
  if (sampling_expression != parsed_zk_sampling_expression) {
    handleTableMetadataMismatch(table_name_for_error_message, "sampling expression", from_zk.sampling_expression,
                                parsed_zk_sampling_expression, sampling_expression, strict_check, logger);
    is_equal = false;
  }

  String parsed_zk_sorting_key =
      formattedAST(extractKeyExpressionList(KeyDescription::parse(from_zk.sorting_key, columns, context, true).definition_ast));
  if (sorting_key != parsed_zk_sorting_key) {
    handleTableMetadataMismatch(table_name_for_error_message, "sorting key expression", from_zk.sorting_key, parsed_zk_sorting_key,
                                sorting_key, strict_check, logger);
    is_equal = false;
  }
  ...
  if (ttl_table != parsed_zk_ttl_table) {
    handleTableMetadataMismatch(table_name_for_error_message, "TTL", from_zk.ttl_table, parsed_zk_ttl_table, ttl_table, strict_check,
                                logger);
    is_equal = false;
  }

  constexpr bool escape_index_filenames = true;  /// It doesn't matter here, as we compare parsed strings
  String parsed_zk_skip_indices =
      IndicesDescription::parse(from_zk.skip_indices, columns, escape_index_filenames, context).explicitToString();
  if (skip_indices != parsed_zk_skip_indices) {
    String all_parsed_zk_skip_indices =
        IndicesDescription::parse(from_zk.skip_indices, columns, escape_index_filenames, context).allToString();
    ...
    if (skip_indices != all_parsed_zk_skip_indices) {
      handleTableMetadataMismatch(table_name_for_error_message, "skip indexes", from_zk.skip_indices, parsed_zk_skip_indices, skip_indices,
                                  strict_check, logger);
      is_equal = false;
    }
  }
  ...
  if (projections != parsed_zk_projections) {
    handleTableMetadataMismatch(table_name_for_error_message, "projections", from_zk.projections, parsed_zk_projections, projections,
                                strict_check, logger);
    is_equal = false;
  }
  ...
  if (constraints != parsed_zk_constraints) {
    handleTableMetadataMismatch(table_name_for_error_message, "constraints", from_zk.constraints, parsed_zk_constraints, constraints,
                                strict_check, logger);
    is_equal = false;
  }

  if (check_index_granularity && from_zk.index_granularity_bytes_found_in_zk &&
      index_granularity_bytes != from_zk.index_granularity_bytes) {
    handleTableMetadataMismatch(table_name_for_error_message, "index granularity bytes", from_zk.index_granularity_bytes, "",
                                index_granularity_bytes, strict_check, logger);
    is_equal = false;
  }
...
ReplicatedMergeTreeTableMetadata::Diff ReplicatedMergeTreeTableMetadata::checkAndFindDiff(const ReplicatedMergeTreeTableMetadata& from_zk,
                                                                                          const ColumnsDescription& columns,
...
StorageInMemoryMetadata ReplicatedMergeTreeTableMetadata::Diff::getNewMetadata(const ColumnsDescription& new_columns, ContextPtr context,
  ...
  if (!empty()) {
    ...
    if (sorting_key_changed) {
      ...
      if (new_metadata.primary_key.definition_ast == nullptr) {
        ...
        new_metadata.primary_key = KeyDescription::getKeyFromAST(old_sorting_key_ast, new_metadata.columns, context);
      }
    }
    ...
    if (skip_indices_changed)
      new_metadata.secondary_indices =
          IndicesDescription::parse(new_skip_indices, new_columns, new_metadata.escape_index_filenames, context);

    if (constraints_changed) new_metadata.constraints = ConstraintsDescription::parse(new_constraints);

    if (projections_changed) new_metadata.projections = ProjectionsDescription::parse(new_projections, new_columns, context);
    ...
    if (ttl_table_changed) {
      if (!new_ttl_table.empty()) {
        ...
        new_metadata.table_ttl =
            TTLTableDescription::getTTLForTableFromAST(ttl_for_table_ast, new_metadata.columns, context, new_metadata.primary_key,
  ...
  for (const auto& column : new_metadata.columns) new_metadata.addImplicitIndicesForColumn(column, context);

  if (!ttl_table_changed && new_metadata.table_ttl.definition_ast != nullptr)
    new_metadata.table_ttl =
        TTLTableDescription::getTTLForTableFromAST(new_metadata.table_ttl.definition_ast, new_metadata.columns, context,
                                                   new_metadata.primary_key, true /* allow_suspicious; because it is replication */);
