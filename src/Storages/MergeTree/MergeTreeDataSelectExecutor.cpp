...
MergeTreeDataSelectExecutor::MergeTreeDataSelectExecutor(const MergeTreeData &data_, ProjectionDescriptionRawPtr projection)
    : data(data_), data_settings(data.getSettings(projection)), log(getLogger(data.getLogName() + " (SelectExecutor)")) {}

size_t MergeTreeDataSelectExecutor::getApproximateTotalRowsToRead(const RangesInDataParts &parts,
                                                                  const StorageMetadataPtr &metadata_snapshot,
                                                                  const KeyCondition &key_condition, const Settings &settings,
                                                                  LoggerPtr log) {
  size_t rows_count = 0;
  ...
  for (const auto &part : parts) {
    ...
    for (const auto &range : part_ranges) rows_count += part.data_part->index_granularity->getRowsCountInRange(range);
  }
  UNUSED(exact_ranges);
...
static RelativeSize convertAbsoluteSampleSizeToRelative(const ASTSampleRatio::Rational &ratio, size_t approx_total_rows) {
  if (approx_total_rows == 0) return 1;

  auto absolute_sample_size = ratio.numerator / ratio.denominator;
  return std::min(RelativeSize(1), RelativeSize(absolute_sample_size) / RelativeSize(approx_total_rows));
}

QueryPlanPtr MergeTreeDataSelectExecutor::read(const Names &column_names_to_return, const StorageSnapshotPtr &storage_snapshot,
                                               const SelectQueryInfo &query_info, ContextPtr context, const UInt64 max_block_size,
                                               const size_t num_streams, PartitionIdToMaxBlockPtr max_block_numbers_to_read,
                                               bool enable_parallel_reading) const {
  const auto &snapshot_data = assert_cast<const MergeTreeData::SnapshotData &>(*storage_snapshot->data);

  auto step = readFromParts(snapshot_data.parts, snapshot_data.mutations_snapshot, column_names_to_return, storage_snapshot, query_info,
                            context, max_block_size, num_streams, max_block_numbers_to_read,
                            /*merge_tree_select_result_ptr=*/nullptr, enable_parallel_reading);

  auto plan = std::make_unique<QueryPlan>();
  if (step) plan->addStep(std::move(step));
  return plan;
}

MergeTreeDataSelectSamplingData MergeTreeDataSelectExecutor::getSampling(const SelectQueryInfo &select_query_info,
                                                                         NamesAndTypesList available_real_columns,
                                                                         const RangesInDataParts &parts, KeyCondition &key_condition,
  ...
  if (select_query_info.table_expression_modifiers) {
    ...
    if (select_sample_size) sample_size_ratio = select_sample_size->as<ASTSampleRatio &>().ratio;

    if (select_sample_offset) sample_offset_ratio = select_sample_offset->as<ASTSampleRatio &>().ratio;
  }

  if (sample_size_ratio) {
    relative_sample_size.assign(sample_size_ratio->numerator, sample_size_ratio->denominator);

    if (relative_sample_size < 0) throw Exception(ErrorCodes::ARGUMENT_OUT_OF_BOUND, "Negative sample size");

    relative_sample_offset = 0;
    if (sample_offset_ratio) relative_sample_offset.assign(sample_offset_ratio->numerator, sample_offset_ratio->denominator);

    if (relative_sample_offset < 0) throw Exception(ErrorCodes::ARGUMENT_OUT_OF_BOUND, "Negative sample offset");
    ...
    if (relative_sample_size == RelativeSize(1)) relative_sample_size = 0;
  ...
  const bool can_use_sampling_key_parallel_replicas = settings[Setting::allow_experimental_parallel_reading_from_replicas] > 0 &&
  ...
  if (can_use_sampling_key_parallel_replicas && settings[Setting::parallel_replicas_count] > 1 && !data.supportsSampling() &&
      settings[Setting::parallel_replica_offset] > 0) {
    LOG_DEBUG(log,
  ...
  sampling.use_sampling = relative_sample_size > 0 || (can_use_sampling_key_parallel_replicas &&
                                                       settings[Setting::parallel_replicas_count] > 1 && data.supportsSampling());
  bool no_data = false;  /// There is nothing left after sampling.

  if (sampling.use_sampling) {
    if (relative_sample_size != RelativeSize(0)) sampling.used_sample_factor = 1.0 / boost::rational_cast<Float64>(relative_sample_size);
    ...
    if (settings[Setting::parallel_replicas_count] > 1) {
      if (relative_sample_size == RelativeSize(0)) relative_sample_size = 1;
    ...
    if (relative_sample_offset >= RelativeSize(1)) no_data = true;
    ...
    if (upper_limit_rational < size_of_universum) has_upper_limit = true;

    if ((has_upper_limit && upper == 0) || (has_lower_limit && has_upper_limit && lower == upper)) no_data = true;
    ...
    if (no_data || (!has_lower_limit && !has_upper_limit)) {
      ...
      sampling.filter_expression =
          std::make_shared<const ActionsDAG>(ExpressionAnalyzer(sampling.filter_function, syntax_result, context).getActionsDAG(false));
    }
  }
...
void MergeTreeDataSelectExecutor::buildKeyConditionFromPartOffset(std::optional<KeyCondition> &part_offset_condition,
  ...
  Block sample = {ColumnWithTypeAndName(part_offset_type->createColumn(), part_offset_type, "_part_offset"),
  ...
  if (std::find(required_columns.begin(), required_columns.end(), "_part_offset") == required_columns.end()) return;

  part_offset_condition.emplace(
      KeyCondition{ActionsDAGWithInversionPushDown(dag->getOutputs().front(), context),
                   context,
                   sample.getNames(),
...
void MergeTreeDataSelectExecutor::buildKeyConditionFromTotalOffset(std::optional<KeyCondition> &total_offset_condition,
                                                                   const ActionsDAG::Node *predicate, ContextPtr context) {
  if (!predicate) return;
  ...
  const auto *node1 = &total_offset.addFunction(FunctionFactory::instance().get("plus", context), {part_offset, part_starting_offset}, {});
  const auto *node2 = &total_offset.addFunction(FunctionFactory::instance().get("plus", context), {part_starting_offset, part_offset}, {});
  ...
  if (dag->getInputs().size() != 1) return;

  auto required_columns = dag->getRequiredColumns();
  total_offset_condition.emplace(
      KeyCondition{ActionsDAGWithInversionPushDown(dag->getOutputs().front(), context),
                   context,
...
std::optional<std::unordered_set<String>> MergeTreeDataSelectExecutor::filterPartsByVirtualColumns(
    const StorageMetadataPtr &metadata_snapshot, const MergeTreeData &data, const RangesInDataParts &parts,
    const ActionsDAG::Node *predicate, ContextPtr context) {
  if (!predicate) return {};
  ...
  if (!has_virtual_column_input) {
    ...
    if (output_node.column) {
      ...
      if (filter_description.always_false) return std::unordered_set<String>{};
    }
    return {};
  }
...
RangesInDataParts MergeTreeDataSelectExecutor::filterPartsByPartition(
    const RangesInDataParts &parts, const std::optional<PartitionPruner> &partition_pruner,
    const std::optional<KeyCondition> &minmax_idx_condition, const std::optional<std::unordered_set<String>> &part_values,
    const StorageMetadataPtr &metadata_snapshot, const MergeTreeData &data, const ContextPtr &context,
    const PartitionIdToMaxBlock *max_block_numbers_to_read, LoggerPtr log, ReadFromMergeTree::IndexStats &index_stats) {
  ...
  if (query_context->getSettingsRef()[Setting::allow_experimental_query_deduplication])
    res = selectPartsToReadWithUUIDFilter(parts, part_values, data.getPinnedPartUUIDs(), minmax_idx_condition, minmax_columns_types,
                                          partition_pruner, max_block_numbers_to_read, query_context, part_filter_counters, log);
  else
    res = selectPartsToRead(parts, part_values, minmax_idx_condition, minmax_columns_types, partition_pruner, max_block_numbers_to_read,
                            part_filter_counters, query_status);

  index_stats.emplace_back(ReadFromMergeTree::IndexStat{.type = ReadFromMergeTree::IndexType::None,
  ...
  if (minmax_idx_condition) {
    ...
    index_stats.emplace_back(ReadFromMergeTree::IndexStat{.type = ReadFromMergeTree::IndexType::MinMax,
  ...
  if (partition_pruner) {
    ...
    index_stats.emplace_back(ReadFromMergeTree::IndexStat{.type = ReadFromMergeTree::IndexType::Partition,
                                                          .condition = std::move(description.condition),
                                                          .used_keys = std::move(description.used_keys),
...
RangesInDataParts MergeTreeDataSelectExecutor::filterPartsByPrimaryKeyAndSkipIndexes(IndexAnalysisContext &filter_context,
  ...
  if (use_skip_indexes && settings[Setting::force_data_skipping_indices].changed) {
    ...
    for (const auto &useful_index : skip_indexes.useful_indices) useful_indices_names.insert(useful_index.index->index.name);

    for (const auto &index_name : forced_indices) {
      if (!useful_indices_names.contains(index_name)) {
        throw Exception(ErrorCodes::INDEX_NOT_USED, "Index {} is not used and setting 'force_data_skipping_indices' contains it",
  ...
  const bool perform_top_k_optimization =
      top_k_filter_info && skip_indexes.skip_index_for_top_k_filtering && !top_k_filter_info->where_clause;
  const bool top_k_handle_ties = perform_top_k_optimization && (top_k_filter_info->num_sort_columns > 1 ||
  ...
  bool use_skip_indexes_for_disjunctions = use_skip_indexes_for_disjunctions_ && !use_skip_indexes_on_data_read_ &&
  ...
  auto is_index_supported_on_data_read = [&](const MergeTreeIndexPtr &index) -> bool {
    ...
    if (index->isVectorSimilarityIndex()) return false;
  ...
  /// Let's find what range to read from each part.
  {
    ...
    auto process_part = [&](size_t part_index) {
      ...
      if (metadata_snapshot->hasPrimaryKey() || part_offset_condition || total_offset_condition) {
        ...
        ranges.ranges = markRangesFromPKRange(ranges, metadata_snapshot, key_condition, part_offset_condition, total_offset_condition,
                                              find_exact_ranges ? &ranges.exact_ranges : nullptr, settings, log);
        ...
        if (ranges.ranges.empty()) pk_stat.parts_dropped.fetch_add(1, std::memory_order_relaxed);
      ...
      if (!skip_indexes.empty()) {
        ...
        auto can_use_index = [&](const MergeTreeIndexPtr &index) -> std::expected<void, PreformattedMessage> {
          if (all_updated_columns.empty()) return {};
          ...
          auto it = std::ranges::find_if(required_columns_list,
                                         [&](const auto &column) { return all_updated_columns.contains(column.getNameInStorage()); });

          if (it == required_columns_list.end()) return {};

          return std::unexpected(PreformattedMessage::create(
              "Index {} is not used for part {} because it depends on column `{}` which will be updated on the fly", index->index.name,
              ranges.data_part->name, it->getNameInStorage()));
        };
        ...
        if (use_skip_indexes_for_disjunctions)
          partial_eval_results.resize(
              ranges.data_part->index_granularity->getMarksCountWithoutFinal() * MAX_BITS_FOR_PARTIAL_DISJUNCTION_RESULT, true);

        for (size_t idx = 0; idx < num_indexes; ++idx) {
          if (ranges.ranges.empty()) break;
          ...
          if (settings[Setting::per_part_index_stats]) index_stat_idx += num_indexes * part_index;
          ...
          if (!is_index_supported_on_data_read(index_and_condition.index)) {
            std::tie(ranges.ranges, ranges.read_hints) = filterMarksUsingIndex(
                index_and_condition.index, index_and_condition.condition, key_condition_rpn_template, ranges.data_part, ranges.ranges,
                ranges.read_hints, reader_settings, mark_cache.get(), uncompressed_cache.get(), vector_similarity_index_cache.get(),
                use_skip_indexes_for_disjunctions, partial_eval_results, log);
          }
        ...
        if (use_skip_indexes_for_disjunctions && key_condition_rpn_template.has_value()) {
          ranges.ranges = mergePartialResultsForDisjunctions(ranges.data_part, ranges.ranges, key_condition_rpn_template.value(),
      ...
      if (perform_top_k_optimization) {
        ...
        auto min_max_granules =
            getMinMaxIndexGranules(ranges.data_part, skip_indexes.skip_index_for_top_k_filtering, ranges.ranges,
                                   top_k_filter_info->direction, false, /*access_by_mark*/
                                   reader_settings, mark_cache.get(), uncompressed_cache.get(), vector_similarity_index_cache.get());
    ...
    if (num_threads <= 1) {
      for (size_t part_index = 0; part_index < parts_with_ranges.size(); ++part_index) process_part(part_index);
    } else {
      /// Parallel loading and filtering of data parts.
      ThreadPool pool(CurrentMetrics::MergeTreeDataSelectExecutorThreads, CurrentMetrics::MergeTreeDataSelectExecutorThreadsActive,
                      CurrentMetrics::MergeTreeDataSelectExecutorThreadsScheduled, num_threads);
  ...
  if (perform_top_k_optimization) {
    ...
    MergeTreeIndexBulkGranulesMinMax::getTopKMarks(top_k_filter_info->direction, top_k_filter_info->limit_n,
                                                   skip_indexes.skip_index_for_top_k_filtering->index.granularity, top_k_handle_ties,
  ...
  if (metadata_snapshot->hasPrimaryKey()) {
    LOG_DEBUG(log, "PK index has dropped {}/{} granules, it took {}ms across {} threads.", pk_stat.granules_dropped.load(),
              pk_stat.total_granules.load(), pk_stat.elapsed_us.load() / 1000, num_threads);

    auto description = key_condition.getDescription();

    index_stats.emplace_back(ReadFromMergeTree::IndexStat{.type = ReadFromMergeTree::IndexType::PrimaryKey,
                                                          .condition = std::move(description.condition),
                                                          .used_keys = std::move(description.used_keys),
  ...
  for (size_t part_index = 0; part_index < part_stats_granularity; ++part_index) {
    for (size_t idx = 0; idx < skip_indexes.useful_indices.size(); ++idx) {
      ...
      LOG_DEBUG(log, "Index {} has dropped {}/{} granules, it took {}ms across {} threads.", backQuote(index_name),
                stat.granules_dropped.load(), stat.total_granules.load(), stat.elapsed_us.load() / 1000, num_threads);
      ...
      if (settings[Setting::per_part_index_stats]) {
        ...
        index_stats.emplace_back(ReadFromMergeTree::IndexStat{.type = ReadFromMergeTree::IndexType::Skip,
                                                              .name = index_name,
  ...
  if (perform_top_k_optimization) {
    ...
    LOG_DEBUG(log, "Skip index {} used for top N optimization, it took {}ms across {} threads.",
              skip_indexes.skip_index_for_top_k_filtering->index.name, top_k_elapsed_us.load() / 1000, num_threads);
  }
  /// Skip empty ranges.
  std::erase_if(parts_with_ranges, [&](const auto &part) {
...
MergeTreeDataSelectExecutor::RowLimits MergeTreeDataSelectExecutor::getRowLimits(const Settings &settings,
  ...
  if (settings[Setting::read_overflow_mode] == OverflowMode::THROW && settings[Setting::max_rows_to_read] && !query_info.input_order_info)
...
void MergeTreeDataSelectExecutor::filterPartsByQueryConditionCache(RangesInDataParts &parts_with_ranges,
                                                                   const SelectQueryInfo &select_query_info,
                                                                   const std::optional<VectorSearchParameters> &vector_search_parameters,
                                                                   const MergeTreeData::MutationsSnapshotPtr &mutations_snapshot,
                                                                   const ContextPtr &context, LoggerPtr log) {
  ...
  if (!settings[Setting::use_query_condition_cache] || !settings[Setting::allow_experimental_analyzer] ||
      ...
      || select_query_info.isFinal() || (mutations_snapshot->hasDataMutations() || mutations_snapshot->hasPatchParts()))
    return;
  ...
  auto drop_mark_ranges = [&](const ActionsDAG::Node *dag) {
    ...
    for (auto it = parts_with_ranges.begin(); it != parts_with_ranges.end();) {
      ...
      size_t min_marks_for_seek = roundRowsOrBytesToMarks(
          settings[Setting::merge_tree_min_rows_for_seek], settings[Setting::merge_tree_min_bytes_for_seek],
          part->index_granularity_info.fixed_index_granularity, part->index_granularity_info.index_granularity_bytes);
  ...
  if (const auto &prewhere_info = select_query_info.prewhere_info) {
    for (const auto *outputs : prewhere_info->prewhere_actions.getOutputs()) {
      if (outputs->result_name == prewhere_info->prewhere_column_name) {
        ...
        LOG_DEBUG(log, "Query condition cache has dropped {}/{} granules for PREWHERE condition {}.", stats.granules_dropped,
                  stats.total_granules, prewhere_info->prewhere_column_name);
        break;
      }
    }
  }
  ...
  if (const auto &filter_actions_dag = select_query_info.filter_actions_dag) {
    ...
    LOG_DEBUG(log, "Query condition cache has dropped {}/{} granules for WHERE condition {}.", stats.granules_dropped, stats.total_granules,
...
ReadFromMergeTree::AnalysisResultPtr MergeTreeDataSelectExecutor::estimateNumMarksToRead(
    RangesInDataParts parts, MergeTreeData::MutationsSnapshotPtr mutations_snapshot, const Names &column_names_to_return,
    const StorageMetadataPtr &metadata_snapshot, const SelectQueryInfo &query_info, ContextPtr context, size_t num_streams,
    PartitionIdToMaxBlockPtr max_block_numbers_to_read) const {
  size_t total_parts = parts.size();
  if (total_parts == 0) return std::make_shared<ReadFromMergeTree::AnalysisResult>();

  std::optional<ReadFromMergeTree::Indexes> indexes;
  return ReadFromMergeTree::selectRangesToRead(parts, mutations_snapshot, std::nullopt, std::nullopt, metadata_snapshot, query_info,
                                               context, num_streams, max_block_numbers_to_read, data, data_settings, column_names_to_return,
                                               log, indexes,
...
QueryPlanStepPtr MergeTreeDataSelectExecutor::readFromParts(
    RangesInDataPartsPtr parts, MergeTreeData::MutationsSnapshotPtr mutations_snapshot, const Names &column_names_to_return,
    const StorageSnapshotPtr &storage_snapshot, const SelectQueryInfo &query_info, ContextPtr context, const UInt64 max_block_size,
    const size_t num_streams, PartitionIdToMaxBlockPtr max_block_numbers_to_read,
    ReadFromMergeTree::AnalysisResultPtr merge_tree_select_result_ptr, bool enable_parallel_reading,
  ...
  if (merge_tree_select_result_ptr) {
    if (merge_tree_select_result_ptr->parts_with_ranges.empty()) return {};
  }
  ...
  return std::make_unique<ReadFromMergeTree>(parts, std::move(mutations_snapshot), column_names_to_return, data, data_settings, query_info,
                                             storage_snapshot, context, max_block_size, num_streams, max_block_numbers_to_read, log,
                                             merge_tree_select_result_ptr, enable_parallel_reading,
...
size_t MergeTreeDataSelectExecutor::roundRowsOrBytesToMarks(size_t rows_setting, size_t bytes_setting, size_t rows_granularity,
                                                            size_t bytes_granularity) {
  size_t res = (rows_setting + rows_granularity - 1) / rows_granularity;

  if (bytes_granularity == 0) return res;
...
size_t MergeTreeDataSelectExecutor::minMarksForConcurrentRead(size_t rows_setting, size_t bytes_setting, size_t rows_granularity,
                                                              size_t bytes_granularity, size_t min_marks, size_t max_marks) {
  size_t marks = 1;
...
MarkRanges MergeTreeDataSelectExecutor::markRangesFromPKRange(const RangesInDataPart &part_with_ranges,
                                                              const StorageMetadataPtr &metadata_snapshot,
                                                              const KeyCondition &key_condition,
                                                              const std::optional<KeyCondition> &part_offset_condition,
                                                              const std::optional<KeyCondition> &total_offset_condition,
                                                              MarkRanges *exact_ranges, const Settings &settings, LoggerPtr log) {
  const auto &part = part_with_ranges.data_part;
  MarkRanges res;

  size_t marks_count = part->index_granularity->getMarksCount();
  if (marks_count == 0) return res;
  ...
  if (!key_condition_useful && !part_offset_condition_useful && !total_offset_condition_useful) return part_with_ranges.ranges;
  ...
  if (key_condition.hasMonotonicFunctionsChain()) {
    create_field_ref = [index_columns](size_t row, size_t column, FieldRef &field) {
      ...
      // NULL_LAST
      if (field.isNull()) field = POSITIVE_INFINITY;
    };
  } else {
    create_field_ref = [index_columns](size_t row, size_t column, FieldRef &field) {
  ...
  DataTypes part_offset_types = {std::make_shared<DataTypeUInt64>(),
  ...
  auto check_in_range = [&](const MarkRange &range, BoolMask initial_mask = {}) {
    ...
    auto check_part_offset_condition = [&]() {
      ...
      return part_offset_condition->checkInRange(2, part_offset_left.data(), part_offset_right.data(), part_offset_types, initial_mask);
    };
    ...
    auto check_total_offset_condition = [&]() {
      ...
      return total_offset_condition->checkInRange(1, part_offset_left.data(), part_offset_right.data(), part_offset_types, initial_mask);
    };

    BoolMask result(true, false);

    if (key_condition_useful) result = result & check_key_condition();

    if (part_offset_condition_useful) result = result & check_part_offset_condition();

    if (total_offset_condition_useful) result = result & check_total_offset_condition();
  ...
  if (!key_condition_exact_range || !part_offset_condition_exact_range || !total_offset_condition_exact_range) {
    ...
    size_t min_marks_for_seek =
        roundRowsOrBytesToMarks(settings[Setting::merge_tree_min_rows_for_seek], settings[Setting::merge_tree_min_bytes_for_seek],
                                part->index_granularity_info.fixed_index_granularity, part->index_granularity_info.index_granularity_bytes);
    ...
    LOG_TRACE(log, "Used generic exclusion search {}over index for part {} with {} steps", exact_ranges ? "with exact ranges " : "",
              part_name, steps);
  } else {
    ...
    LOG_TRACE(log, "Found {} range {}in {} steps", res.empty() ? "empty" : "continuous", exact_ranges ? "with exact range " : "", steps);
  }
...
std::pair<MarkRanges, RangesInDataPartReadHints> MergeTreeDataSelectExecutor::filterMarksUsingIndex(
    MergeTreeIndexPtr index_helper, MergeTreeIndexConditionPtr condition, const std::optional<KeyCondition> &key_condition_rpn_template,
    MergeTreeData::DataPartPtr part, const MarkRanges &ranges, const RangesInDataPartReadHints &in_read_hints,
    const MergeTreeReaderSettings &reader_settings, MarkCache *mark_cache, UncompressedCache *uncompressed_cache,
    VectorSimilarityIndexCache *vector_similarity_index_cache, bool use_skip_indexes_for_disjunctions,
    PartialDisjunctionResult &partial_disjunction_result, LoggerPtr log) {
  ...
  const size_t min_marks_for_seek =
      roundRowsOrBytesToMarks(reader_settings.merge_tree_min_rows_for_seek, reader_settings.merge_tree_min_bytes_for_seek,
                              part->index_granularity_info.fixed_index_granularity, part->index_granularity_info.index_granularity_bytes);
  ...
  for (const auto &range : ranges) {
    MarkRange index_range(range.begin / skip_index_granularity, (range.end + skip_index_granularity - 1) / skip_index_granularity);
    index_ranges.push_back(index_range);
  }

  MergeTreeIndexReader reader(index_helper, part, part->index_granularity->getMarksCountForSkipIndex(skip_index_granularity), index_ranges,
                              mark_cache, uncompressed_cache, vector_similarity_index_cache, reader_settings);
  ...
  auto create_update_partial_disjunction_result_fn = [&](size_t range_begin) -> KeyCondition::UpdatePartialDisjunctionResultFn {
    if (use_skip_indexes_for_disjunctions && key_condition_rpn_template) {
      return [range_begin, &partial_disjunction_result](size_t position, bool element_result, bool is_unknown) {
        if (!is_unknown) partial_disjunction_result[(range_begin * MAX_BITS_FOR_PARTIAL_DISJUNCTION_RESULT) + position] = element_result;
      };
    }
    return nullptr;
  };
  ...
  if (index_helper->isTextIndex()) {
    ...
    if (filtered_granules.empty()) return {res, read_hints};
    ...
    for (size_t i = 0; i < ranges_size; ++i) {
      ...
      for (size_t index_mark = index_range.begin; index_mark < index_range.end; ++index_mark) {
        if (current_granule_num == *it) {
          MarkRange data_range(std::max(ranges[i].begin, index_mark * skip_index_granularity),
          ...
          ++it;
          if (it == filtered_granules.end()) break;
        }
    ...
    for (size_t i = 0; i < ranges_size; ++i) {
      ...
      for (size_t index_mark = index_range.begin; index_mark < index_range.end; ++index_mark) {
        ...
        if (index_helper->isVectorSimilarityIndex()) {
          ...
          if (has_duplicates) throw Exception(ErrorCodes::INCORRECT_DATA, "Usearch returned duplicate row numbers");
#endif
          if (!(read_hints.vector_search_results.value().distances.has_value())) read_hints = {};

          for (auto row : rows) {
            size_t num_marks = part->index_granularity->countMarksForRows(index_mark * skip_index_granularity, row);

            MarkRange data_range(std::max(ranges[i].begin, (index_mark * skip_index_granularity) + num_marks),
...
RangesInDataParts MergeTreeDataSelectExecutor::selectPartsToRead(const RangesInDataParts &parts,
                                                                 const std::optional<std::unordered_set<String>> &part_values,
                                                                 const std::optional<KeyCondition> &minmax_idx_condition,
                                                                 const DataTypes &minmax_columns_types,
                                                                 const std::optional<PartitionPruner> &partition_pruner,
                                                                 const PartitionIdToMaxBlock *max_block_numbers_to_read,
                                                                 PartFilterCounters &counters, QueryStatusPtr query_status) {
  ...
  for (const auto &prev_part : parts) {
    ...
    if (part_values && !part_values->contains(part->name)) continue;
...
RangesInDataParts MergeTreeDataSelectExecutor::selectPartsToReadWithUUIDFilter(
    const RangesInDataParts &parts, const std::optional<std::unordered_set<String>> &part_values,
    MergeTreeData::PinnedPartUUIDsPtr pinned_part_uuids, const std::optional<KeyCondition> &minmax_idx_condition,
    const DataTypes &minmax_columns_types, const std::optional<PartitionPruner> &partition_pruner,
    const PartitionIdToMaxBlock *max_block_numbers_to_read, ContextPtr query_context, PartFilterCounters &counters, LoggerPtr log) {
  ...
  auto select_parts = [&](const RangesInDataParts &in_parts, RangesInDataParts &selected_parts) -> bool {
    ...
    for (const auto &prev_part : in_parts) {
      ...
      if (part->uuid != UUIDHelpers::Nil && ignored_part_uuids->has(part->uuid)) continue;
      ...
      if (part->uuid != UUIDHelpers::Nil && pinned_part_uuids->contains(part->uuid)) {
        ...
        if (!result.second) throw Exception(ErrorCodes::LOGICAL_ERROR, "Found a part with the same UUID on the same replica.");
      }
...
MergeTreeIndexBulkGranulesMinMaxPtr MergeTreeDataSelectExecutor::getMinMaxIndexGranules(
    MergeTreeData::DataPartPtr part, MergeTreeIndexPtr skip_index_minmax, const MarkRanges &ranges, int direction, bool access_by_mark,
    const MergeTreeReaderSettings &reader_settings, MarkCache *mark_cache, UncompressedCache *uncompressed_cache,
  ...
  MergeTreeIndexReader reader(skip_index_minmax, part, part->index_granularity->getMarksCountForSkipIndex(skip_index_granularity),
                              index_ranges, mark_cache, uncompressed_cache, vector_similarity_index_cache, reader_settings);

  auto min_max_granules = std::make_shared<MergeTreeIndexBulkGranulesMinMax>(
      skip_index_minmax->index.name, skip_index_minmax->index.sample_block, skip_index_granularity, direction,
...
MarkRanges MergeTreeDataSelectExecutor::mergePartialResultsForDisjunctions(MergeTreeData::DataPartPtr part, const MarkRanges &ranges,
                                                                           const KeyCondition &rpn_template_for_eval_result,
                                                                           const PartialDisjunctionResult &partial_eval_results,
                                                                           MergeTreeReaderSettings reader_settings, LoggerPtr log) {
  MarkRanges res;

  auto rpn_template_for_eval_result_string = rpn_template_for_eval_result.toString();

  LOG_DEBUG(log, "Entered mergePartialResultsForDisjunctions for part {}, rpn = {}", part->name, rpn_template_for_eval_result_string);
    ...
    for (auto range_begin = range.begin; range_begin < range.end; range_begin++) {
      ...
      for (const auto &element : rpn) {
        if (element.function == KeyCondition::RPNElement::FUNCTION_UNKNOWN) {
          ...
          throw Exception(ErrorCodes::LOGICAL_ERROR, "Unexpected function type {} in mergePartialResultsForDisjunctions(), RPN = {}",
                          element.function, rpn_template_for_eval_result_string);

        position++;
      }

      if (rpn_stack.size() != 1)
        throw Exception(ErrorCodes::LOGICAL_ERROR, "Unexpected stack size {} in mergePartialResultsForDisjunctions(), RPN = {}",
                        rpn_stack.size(), rpn_template_for_eval_result_string);
