...
void fillRequiredColumns(const ActionsDAG::Node *node, std::unordered_map<const ActionsDAG::Node *, NodeInfo> &nodes_info) {
  if (nodes_info.contains(node)) return;
...
const ActionsDAG::Node &addClonedDAGToDAG(size_t step, const ActionsDAG::Node *original_dag_node, const ActionsDAGPtr &new_dag,
                                          OriginalToNewNodeMap &node_remap, NodeNameToLastUsedStepMap &node_to_step_map) {
  ...
  if (node_remap.contains(original_dag_node)) {
    ...
    if (node_ref.dag == new_dag.get()) return *node_ref.node;
    ...
    if (original_dag_node->type != ActionsDAG::ActionType::COLUMN) {
      ...
      if (node_ref.dag->getOutputs().at(0) != node_ref.node) node_ref.dag->getOutputs().push_back(node_ref.node);
...
const ActionsDAG::Node &addFunction(const ActionsDAGPtr &new_dag, const FunctionOverloadResolverPtr &function,
...
/// This is different from ActionsDAG::addCast() because it set the name equal to the original name effectively hiding the value before
/// cast, but it might be required for further steps with its original uncast type.
const ActionsDAG::Node &addCast(const ActionsDAGPtr &dag, const ActionsDAG::Node &node_to_cast, const DataTypePtr &to_type) {
  if (node_to_cast.result_type->equals(*to_type)) return node_to_cast;  /// NOLINT(bugprone-return-const-ref-from-parameter)
...
const ActionsDAG::Node &addAndTrue(const ActionsDAGPtr &dag, const ActionsDAG::Node &filter_node_to_normalize) {
...
bool tryBuildPrewhereSteps(PrewhereInfoPtr prewhere_info, const ExpressionActionsSettings &actions_settings, PrewhereExprInfo &prewhere,
  ...
  for (const auto *output : original_outputs) {
    ...
    if (node_remap.contains(output)) {
      ...
      if (std::ranges::find(new_outputs, new_node_info.node) == new_outputs.end()) new_outputs.push_back(new_node_info.node);
