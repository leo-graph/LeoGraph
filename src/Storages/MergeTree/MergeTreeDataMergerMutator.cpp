...
PartsRanges checkRanges(PartsRanges&& ranges) {
  ...
  for (const auto& range : ranges) chassert(!range.empty());
#endif
...
size_t calculatePartsCount(const PartsRanges& ranges) {
  ...
  for (const auto& range : ranges) count += range.size();
...
std::expected<void, PreformattedMessage> canMergeAllParts(const PartsRange& range, const MergePredicatePtr& merge_predicate) {
  for (size_t i = 1; i < range.size(); ++i) {
    ...
    if (auto can_merge_result = merge_predicate->canMergeParts(prev_part, current_part); !can_merge_result) return can_merge_result;
  }
...
String getBestPartitionToOptimizeEntire(size_t max_total_size_to_merge, const ContextPtr& context, const MergeTreeSettingsPtr& settings,
                                        const PartitionsStatistics& stats, const LoggerPtr& log) {
  if (!(*settings)[MergeTreeSetting::min_age_to_force_merge_on_partition_only]) return {};

  if (!(*settings)[MergeTreeSetting::min_age_to_force_merge_seconds]) return {};
  ...
  Int64 optimize_entire_partition_threshold =
      (*settings)[MergeTreeSetting::number_of_free_entries_in_pool_to_execute_optimize_entire_partition];
  ...
  const auto is_partition_invalid = [&](const PartitionStatistics& partition) {
    if (partition.part_count == 1) return true;

    if (!max_total_size_to_merge || !(*settings)[MergeTreeSetting::enable_max_bytes_limit_for_min_age_to_force_merge]) return false;

    return partition.total_size > max_total_size_to_merge;
  };

  auto best_partition_it = std::max_element(stats.begin(), stats.end(), [&](const auto& e1, const auto& e2) {
    // If one partition cannot be used for some reason (e.g. it has only single part, or it's size greater than limit), always select the
    // other partition.
    if (is_partition_invalid(e1.second)) return true;

    if (is_partition_invalid(e2.second)) return false;
  ...
  if (best_partition_min_age < (*settings)[MergeTreeSetting::min_age_to_force_merge_seconds] ||
...
CollectedPartsRanges grabAllPossibleRanges(const PartsCollectorPtr& parts_collector, const StorageMetadataPtr& metadata_snapshot,
                                           const StoragePolicyPtr& storage_policy, const time_t& current_time,
                                           const std::optional<PartitionIdsHint>& partitions_hint, LogSeriesLimiter& series_log) {
...
std::expected<PartsRange, PreformattedMessage> grabAllPartsInsidePartition(const PartsCollectorPtr& parts_collector,
                                                                           const StorageMetadataPtr& metadata_snapshot,
                                                                           const StoragePolicyPtr& storage_policy,
                                                                           const time_t& current_time, const std::string& partition_id) {
  ProfileEventTimeIncrement<Microseconds> watch(ProfileEvents::MergerMutatorsGetPartsForMergeElapsedMicroseconds);
  return parts_collector->grabAllPartsInsidePartition(metadata_snapshot, storage_policy, current_time, partition_id);
}

MergeSelectorChoices chooseMergesFrom(const MergeSelectorApplier& selector, const IMergePredicate& predicate, const PartsRanges& ranges,
                                      const PartitionsStatistics& partitions_stats, const StorageMetadataPtr& metadata_snapshot,
                                      const MergeTreeSettingsPtr& data_settings, const PartitionIdToTTLs& next_delete_times,
                                      const PartitionIdToTTLs& next_recompress_times, bool can_use_ttl_merges, time_t current_time,
                                      const LoggerPtr& log) {
  ProfileEventTimeIncrement<Microseconds> watch(ProfileEvents::MergerMutatorSelectPartsForMergeElapsedMicroseconds);

  auto choices = selector.chooseMergesFrom(ranges, partitions_stats, predicate, metadata_snapshot, data_settings, next_delete_times,
                                           next_recompress_times, can_use_ttl_merges, current_time);
  ...
  if (!choices.empty()) {
    ...
    for (size_t i = 0; i < choices.size(); ++i) {
      ...
      LOG_TRACE(log, "Merge #{} type {} with {} parts from {} to {} with {} patches", i, merge_type, range.size(), range.front().name,
                range.back().name, range_patches.size());
    }
  }
...
void MergeTreeDataMergerMutator::updateTTLMergeTimes(const MergeSelectorChoices& choices, const MergeTreeSettingsPtr& settings,
                                                     time_t current_time) {
  for (const auto& choice : choices) {
    ...
    switch (choice.merge_type) {
      ...
      case MergeType::TTLDelete: {
        ...
        LOG_TRACE(log,
                  "For table {} under database {}, the next scheduled execution time of TTLDelete task "
                  "for partition '{}' will be: '{}'.",
                  backQuote(storage.table_name), backQuote(storage.database_name), partition_id,
                  toString(next_delete_ttl_merge_times_by_partition[partition_id]));
        break;
      }
      case MergeType::TTLRecompress: {
        next_recompress_ttl_merge_times_by_partition[partition_id] =
            current_time + (*settings)[MergeTreeSetting::merge_with_recompression_ttl_timeout];
        break;
      }
    }
  }
...
PartitionIdsHint MergeTreeDataMergerMutator::getPartitionsThatMayBeMerged(const PartsCollectorPtr& parts_collector,
  ...
  if (collected.ranges.empty()) return {};
  ...
  for (const auto& [partition, ranges_in_partition] : ranges_by_partitions) {
    ...
    auto merge_choices = chooseMergesFrom(selector, *merge_predicate, ranges_in_partition, partitions_stats, metadata_snapshot, settings,
    ...
    if (!merge_choices.empty())
      ...
      LOG_TEST(log, "Nothing to merge in partition {} with max_merge_sizes = {} (looked up {} ranges)", partition_id,
               convertMergeConstraintsToString(selector.merge_constraints), ranges_in_partition.size());
  }

  if (auto best = getBestPartitionToOptimizeEntire(selector.merge_constraints[0].max_size_bytes, context, settings, partitions_stats, log);
      !best.empty())
...
std::expected<MergeSelectorChoices, SelectMergeFailure> MergeTreeDataMergerMutator::selectPartsToMerge(
    const PartsCollectorPtr& parts_collector, const MergePredicatePtr& merge_predicate, const MergeSelectorApplier& selector,
  ...
  const auto& partitions_stats = collected.partitions_stats;
  auto merge_choices = chooseMergesFrom(selector, *merge_predicate, ranges, partitions_stats, metadata_snapshot, settings,
                                        next_delete_ttl_merge_times_by_partition, next_recompress_ttl_merge_times_by_partition,
                                        can_use_ttl_merges, current_time, log);
  ...
  if (auto best = getBestPartitionToOptimizeEntire(selector.merge_constraints[0].max_size_bytes, context, settings, partitions_stats, log);
      !best.empty()) {
    return selectAllPartsToMergeWithinPartition(metadata_snapshot, parts_collector, merge_predicate,
                                                /*partition_id=*/best,
                                                /*final=*/true,
...
std::expected<MergeSelectorChoices, SelectMergeFailure> MergeTreeDataMergerMutator::selectAllPartsToMergeWithinPartition(
    const StorageMetadataPtr& metadata_snapshot, const PartsCollectorPtr& parts_collector, const MergePredicatePtr& merge_predicate,
  ...
  if (available_disk_space <= required_disk_space) {
    return std::unexpected(SelectMergeFailure{
        ...
        .explanation = PreformattedMessage::create(
            "Not enough free space to merge parts from {} to {}. Has {} free and unreserved, {} required now", parts.front().name,
            parts.back().name, ReadableSize(available_disk_space), ReadableSize(required_disk_space)),
    });
  }
  ...
  LOG_TRACE(log, "Selected {} parts from {} to {}. Will apply {} patch parts", parts.size(), parts.front().name, parts.back().name,
            patch_parts.size());
...
MergeTaskPtr MergeTreeDataMergerMutator::mergePartsToTemporaryPart(
    FutureMergedMutatedPartPtr future_part, StorageMetadataPtr metadata_snapshot, MergeList::Entry* merge_entry,
    std::unique_ptr<MergeListElement> projection_merge_list_element, TableLockHolder& holder, time_t time_of_merge, ContextPtr context,
    ReservationSharedPtr space_reservation, bool deduplicate, const Names& deduplicate_by_columns, bool cleanup,
    MergeTreeData::MergingParams merging_params, MergeTreeTransactionPtr txn, bool need_prefix, ProjectionDescriptionRawPtr projection,
    IMergeTreeDataPart* parent_part, const String& suffix) {
  ...
  return std::make_shared<MergeTask>(std::move(future_part), std::move(metadata_snapshot), merge_entry,
                                     std::move(projection_merge_list_element), time_of_merge, context, holder, space_reservation,
                                     deduplicate, deduplicate_by_columns, cleanup, std::move(merging_params), need_prefix, projection,
                                     parent_part, nullptr, suffix, std::move(txn), &data, this, &merges_blocker, &ttl_merges_blocker);
}

MutateTaskPtr MergeTreeDataMergerMutator::mutatePartToTemporaryPart(FutureMergedMutatedPartPtr future_part,
                                                                    StorageMetadataPtr metadata_snapshot, MutationCommandsConstPtr commands,
                                                                    MergeListEntry* merge_entry, time_t time_of_mutation,
                                                                    ContextPtr context, const MergeTreeTransactionPtr& txn,
                                                                    ReservationSharedPtr space_reservation, TableLockHolder& holder,
                                                                    bool need_prefix) {
  return std::make_shared<MutateTask>(future_part, metadata_snapshot, commands, merge_entry, time_of_mutation, context, space_reservation,
                                      holder, txn, data, *this, merges_blocker, need_prefix);
}

MergeTreeData::DataPartPtr MergeTreeDataMergerMutator::renameMergedTemporaryPart(MergeTreeData::MutableDataPartPtr& new_data_part,
  ...
  if (replaced_parts.size() != parts.size()) {
    ...
    LOG_WARNING(log,
                ...
                "Replaced parts:\n{}\n"
                "Parts:\n{}\n",
                new_data_part->name, replaced_parts.size(), parts.size(), fmt::join(getPartsNames(replaced_parts), "\n"),
                fmt::join(getPartsNames(parts), "\n"));
  } else {
    for (size_t i = 0; i < parts.size(); ++i)
      if (parts[i]->name != replaced_parts[i]->name)
        throw Exception(ErrorCodes::LOGICAL_ERROR, "Unexpected part removed when adding {}: {} instead of {}", new_data_part->name,
                        replaced_parts[i]->name, parts[i]->name);
  }
