...
MergedBlockOutputStream::MergedBlockOutputStream(const MergeTreeMutableDataPartPtr &data_part, MergeTreeSettingsPtr data_settings,
                                                 const StorageMetadataPtr &metadata_snapshot_, const NamesAndTypesList &columns_list_,
                                                 const MergeTreeIndices &skip_indices, CompressionCodecPtr default_codec_,
                                                 MergeTreeIndexGranularityPtr index_granularity_ptr, TransactionID tid,
                                                 size_t part_uncompressed_bytes, bool reset_columns_, bool blocks_are_granules_size,
                                                 const WriteSettings &write_settings_, WrittenOffsetSubstreams *written_offset_substreams)
    : IMergedBlockOutputStream(std::move(data_settings), data_part->getDataPartStoragePtr(), metadata_snapshot_, columns_list_,
                               reset_columns_),
  ...
  writer_settings = MergeTreeWriterSettings(data_part->storage.getContext()->getSettingsRef(), write_settings_, storage_settings, data_part,
                                            data_part->index_granularity_info.mark_type.adaptive,
                                            /* rewrite_primary_key = */ true, save_marks_in_cache, save_primary_index_in_memory,
  ...
  writer = createMergeTreeDataPartWriter(data_part->getType(), data_part->name, data_part->storage.getLogName(),
                                         data_part->getSerializations(), data_part_storage, data_part->index_granularity_info,
                                         storage_settings, columns_list, data_part->getColumnPositions(), metadata_snapshot,
                                         data_part->storage.getVirtualsPtr(), skip_indices, data_part->getMarksFileExtension(),
                                         default_codec, writer_settings, std::move(index_granularity_ptr), written_offset_substreams);
}

/// If data is pre-sorted.
void MergedBlockOutputStream::write(const Block &block) { writeImpl(block, nullptr); }
...
struct MergedBlockOutputStream::Finalizer::Impl {
  ...
  Impl(IMergeTreeDataPartWriter &writer_, MergeTreeData::MutableDataPartPtr part_, const NameSet &files_to_remove_after_finish_, bool sync_)
      : writer(writer_), part(std::move(part_)), files_to_remove_after_finish(files_to_remove_after_finish_), sync(sync_) {}
...
void MergedBlockOutputStream::Finalizer::Impl::finish() {
  ...
  for (const auto &file_name : files_to_remove_after_finish) part->getDataPartStorage().removeFile(file_name);
}
...
void MergedBlockOutputStream::finalizePart(const MergeTreeMutableDataPartPtr &new_part, const GatheredData &gathered_data, bool sync,
                                           const NamesAndTypesList *total_columns_list) {
  finalizePartAsync(new_part, gathered_data, sync, total_columns_list).finish();
}

void MergedBlockOutputStream::finalizeIndexGranularity() { writer->finalizeIndexGranularity(); }

MergedBlockOutputStream::Finalizer MergedBlockOutputStream::finalizePartAsync(const MergeTreeMutableDataPartPtr &new_part,
                                                                              const GatheredData &gathered_data, bool sync,
                                                                              const NamesAndTypesList *total_columns_list) {
  ...
  for (const auto &name : checksums_to_remove) checksums.files.erase(name);

  LOG_TRACE(getLogger("MergedBlockOutputStream"), "filled checksums {}", new_part->getNameWithState());

  for (const auto &[projection_name, projection_part] : new_part->getProjectionParts()) {
    checksums.addFile(projection_name + ".proj", projection_part->checksums.getTotalSizeOnDisk(),
  ...
  if (auto computed_index = writer->releaseIndexColumns()) new_part->setIndex(std::move(*computed_index));
  ...
  if (!new_part->existing_rows_count.has_value()) new_part->existing_rows_count = rows_count;
...
MergedBlockOutputStream::WrittenFiles MergedBlockOutputStream::finalizePartOnDisk(const MergeTreeMutableDataPartPtr &new_part,
  ...
  if (!new_part->isProjectionPart()) {
    if (new_part->uuid != UUIDHelpers::Nil) {
      write_hashed_file(IMergeTreeDataPart::UUID_FILE_NAME, [&](auto &buffer) { writeUUIDText(new_part->uuid, buffer); });
    }

    if (new_part->storage.format_version >= MERGE_TREE_DATA_MIN_FORMAT_VERSION_WITH_CUSTOM_PARTITIONING) {
      if (auto file =
              new_part->partition.store(metadata_snapshot, new_part->storage.getContext(), new_part->getDataPartStorage(), checksums)) {
      ...
      if (new_part->minmax_idx->initialized) {
        ...
        for (auto &file : files) written_files.emplace_back(std::move(file));
      } else if (rows_count) {
      ...
      if (!source_parts.empty()) {
        write_hashed_file(SourcePartsSetForPatch::FILENAME, [&](auto &buffer) { source_parts.writeBinary(buffer); });
      }
    }
  }

  write_hashed_file("count.txt", [&](auto &buffer) { writeIntText(rows_count, buffer); });

  if (!new_part->ttl_infos.empty()) {
    write_hashed_file("ttl.txt", [&](auto &buffer) { new_part->ttl_infos.write(buffer); });
  }

  const auto &serialization_infos = new_part->getSerializationInfos();
  if (serialization_infos.needsPersistence()) {
    write_hashed_file(IMergeTreeDataPart::SERIALIZATION_FILE_NAME, [&](auto &buffer) { serialization_infos.writeJSON(buffer); });
  }
  ...
  if (!statistics.empty()) {
    if (isFullPartStorage(new_part->getDataPartStorage())) {
      auto out = serializeStatisticsPacked(new_part->getDataPartStorage(), checksums, statistics, default_codec,
                                           writer_settings.query_write_settings);
      written_files.emplace_back(std::move(out));
    }
    /// Write statistics as separate compressed files in packed parts to avoid double buffering.
    else {
      auto files = serializeStatisticsWide(new_part->getDataPartStorage(), checksums, statistics, default_codec,
                                           writer_settings.query_write_settings);
      std::move(files.begin(), files.end(), std::back_inserter(written_files));
    }
  }

  write_plain_file("columns.txt", [&](auto &buffer) { new_part->getColumns().writeText(buffer); });
  ...
  auto columns_substreams =
      ColumnsSubstreams::merge(writer->getColumnsSubstreams(), gathered_data.columns_substreams, new_part->getColumns().getNames());

  if (!columns_substreams.empty()) {
    write_plain_file(IMergeTreeDataPart::COLUMNS_SUBSTREAMS_FILE_NAME, [&](auto &buffer) { columns_substreams.writeText(buffer); });

    new_part->setColumnsSubstreams(columns_substreams);
  }

  write_plain_file(IMergeTreeDataPart::METADATA_VERSION_FILE_NAME,
                   [&](auto &buffer) { writeIntText(new_part->getMetadataVersion(), buffer); });

  if (default_codec != nullptr) {
    write_plain_file(IMergeTreeDataPart::DEFAULT_COMPRESSION_CODEC_FILE_NAME,
                     [&](auto &buffer) { writeText(default_codec->getFullCodecDesc()->formatWithSecretsOneLine(), buffer); });
  } else {
    throw Exception(ErrorCodes::LOGICAL_ERROR, "Compression codec have to be specified for part on disk, empty for {}", new_part->name);
  }

  write_plain_file("checksums.txt", [&](auto &buffer) { checksums.write(buffer); });
...
void MergedBlockOutputStream::writeImpl(const Block &block, const IColumn::Permutation *permutation) {
  ...
  if (reset_columns) new_serialization_infos.add(block);
