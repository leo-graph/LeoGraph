...
MergeListElement::MergeListElement(const StorageID& table_id_, FutureMergedMutatedPartPtr future_part, const ContextPtr& context)
  ...
  if (result_part_name != result_part_info.getPartNameV1()) format_version = MERGE_TREE_DATA_OLD_FORMAT_VERSION;
  ...
  for (const auto& source_part : future_part->parts) {
    if (!is_fake_projection_part && !source_part->getParentPart()) {
      ...
      if (!result_part_info.contains(MergeTreePartInfo::fromPartName(source_part->name, format_version)))
        throw Exception(ErrorCodes::LOGICAL_ERROR, "Source part {} is not covered by result part {}", source_part->name,
                        result_part_info.getPartNameV1());
    }
...
MergeInfo MergeListElement::getInfo() const {
  ...
  for (const auto& source_part_name : source_part_names) res.source_part_names.emplace_back(source_part_name);

  for (const auto& source_part_path : source_part_paths) res.source_part_paths.emplace_back(source_part_path);

  return res;
}

const MemoryTracker& MergeListElement::getMemoryTracker() const { return thread_group->memory_tracker; }

MergeListElement::~MergeListElement() { background_memory_tracker.adjustOnBackgroundTaskEnd(&getMemoryTracker()); }
