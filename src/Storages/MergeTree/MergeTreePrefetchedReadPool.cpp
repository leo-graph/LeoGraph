...
MergeTreePrefetchedReadPool::ThreadTask::ThreadTask(InfoPtr read_info_, MarkRanges ranges_, std::vector<MarkRanges> patches_ranges_,
                                                    Priority priority_)
    : read_info(std::move(read_info_)), ranges(std::move(ranges_)), patches_ranges(std::move(patches_ranges_)), priority(priority_) {}

MergeTreePrefetchedReadPool::PrefetchedReaders::PrefetchedReaders(ThreadPool& pool, MergeTreeReadTask::Readers readers_, Priority priority_,
                                                                  MergeTreePrefetchedReadPool& read_prefetch)
    : is_valid(true), readers(std::move(readers_)), prefetch_runner(pool, ThreadName::PREFETCH_READER) {
...
MergeTreePrefetchedReadPool::MergeTreePrefetchedReadPool(RangesInDataParts&& parts_, MutationsSnapshotPtr mutations_snapshot_,
                                                         VirtualFields shared_virtual_fields_, const IndexReadTasks& index_read_tasks_,
                                                         const StorageSnapshotPtr& storage_snapshot_,
                                                         const FilterDAGInfoPtr& row_level_filter_, const PrewhereInfoPtr& prewhere_info_,
                                                         const ExpressionActionsSettings& actions_settings_,
                                                         const MergeTreeReaderSettings& reader_settings_, const Names& column_names_,
                                                         const PoolSettings& settings_, const MergeTreeReadTask::BlockSizeParams& params_,
                                                         const ContextPtr& context_, RuntimeDataflowStatisticsCacheUpdaterPtr updater_)
    : MergeTreeReadPoolBase(std::move(parts_), std::move(mutations_snapshot_), std::move(shared_virtual_fields_), index_read_tasks_,
      ...
      log(getLogger("MergeTreePrefetchedReadPool(" +
...
void MergeTreePrefetchedReadPool::createPrefetchedReadersForTask(ThreadTask& task) {
  if (task.isValidReadersFuture()) throw Exception(ErrorCodes::LOGICAL_ERROR, "Task already has a reader");
...
void MergeTreePrefetchedReadPool::startPrefetches() {
  ...
  if (prefetch_queue.empty()) return;

  [[maybe_unused]] TaskHolder prev;
  [[maybe_unused]] const Priority highest_priority{reader_settings.read_settings.priority.value + 1};
  /// Due to the difference in `estimated_memory_usage_for_single_prefetch` for different parts (column sizes might be different), it might
  /// happen that for the given thread, task number N won't fit in the prefetch memory limit, but the next task will. So the top of the
  /// queue may have a priority higher than `highest_priority`.
  ...
  while (!prefetch_queue.empty()) {
    ...
    if (prev.task) {
      ...
      if (prev.thread_id == top.thread_id) assert(prev.task->priority < top.task->priority);
    }
    prev = top;
#endif
    prefetch_queue.pop();
...
MergeTreeReadTaskPtr MergeTreePrefetchedReadPool::getTask(size_t task_idx, MergeTreeReadTask* previous_task) {
  ...
  if (per_thread_tasks.empty()) return nullptr;
  ...
  if (it == per_thread_tasks.end()) return stealTask(task_idx, previous_task);
  ...
  if (thread_tasks.empty()) per_thread_tasks.erase(it);
...
MergeTreeReadTaskPtr MergeTreePrefetchedReadPool::stealTask(size_t thread, MergeTreeReadTask* previous_task) {
  ...
  for (auto thread_tasks_it = per_thread_tasks.begin(); thread_tasks_it != per_thread_tasks.end(); ++thread_tasks_it) {
    ...
    auto task_it = std::find_if(thread_tasks.begin(), thread_tasks.end(), [](const auto& task) { return task->isValidReadersFuture(); });
    ...
    if (task_it == thread_tasks.end()) {
      ...
      if (non_prefetched_tasks_to_steal == per_thread_tasks.end() || non_prefetched_tasks_to_steal->second.size() < thread_tasks.size())
    ...
    else if (prefetched_tasks_to_steal == per_thread_tasks.end() || (*task_it)->priority < best_prefetched_task_priority) {
  ...
  if (prefetched_tasks_to_steal != per_thread_tasks.end()) {
    ...
    if (thread_tasks.empty()) per_thread_tasks.erase(prefetched_tasks_to_steal);
  ...
  if (non_prefetched_tasks_to_steal != per_thread_tasks.end()) {
    ...
    current_thread_tasks.insert(current_thread_tasks.end(), make_move_iterator(half_it), make_move_iterator(thread_tasks.end()));

    /// Erase them from the thread from which we steal.
    thread_tasks.resize(half);
    if (thread_tasks.empty()) per_thread_tasks.erase(non_prefetched_tasks_to_steal);

    auto thread_task = std::move(current_thread_tasks.front());
    current_thread_tasks.erase(current_thread_tasks.begin());
    if (current_thread_tasks.empty()) per_thread_tasks.erase(thread);
...
void MergeTreePrefetchedReadPool::fillPerPartStatistics() {
  ...
  for (size_t i = 0; i < parts_ranges.size(); ++i) {
    ...
    for (const auto& range : parts_ranges[i].ranges) part_stat.sum_marks += range.end - range.begin;
    ...
    for (const auto& column : read_info.task_columns.columns) update_stat_for_column(column.name);

    for (const auto& pre_columns : read_info.task_columns.pre_columns)
      for (const auto& column : pre_columns) update_stat_for_column(column.name);
  }
}
...
void MergeTreePrefetchedReadPool::fillPerThreadTasks(size_t threads, size_t sum_marks) {
  if (per_part_infos.empty()) return;

  const auto& context = getContext();
  const auto& settings = context->getSettingsRef();

  size_t total_size_approx = 0;
  for (const auto& part : per_part_statistics) total_size_approx += part.sum_marks * part.approx_size_of_mark;
  ...
  for (size_t i = 0; i < per_part_infos.size(); ++i) {
    ...
    if (settings[Setting::filesystem_prefetch_step_marks]) {
      ...
      part_stat.prefetch_step_marks =
          std::max<size_t>(1, static_cast<size_t>(std::round(static_cast<double>(settings[Setting::filesystem_prefetch_step_bytes]) /
                                                             static_cast<double>(part_stat.approx_size_of_mark))));
    }
    ...
    LOG_TRACE(log, "Part: {}, sum_marks: {}, approx mark size: {}, prefetch_step_bytes: {}, prefetch_step_marks: {}, (ranges: {})",
              getPartNameForLogging(parts_ranges[i].data_part), part_stat.sum_marks, part_stat.approx_size_of_mark,
              settings[Setting::filesystem_prefetch_step_bytes].value, part_stat.prefetch_step_marks, toString(parts_ranges[i].ranges));
  }

  const size_t min_marks_per_thread = (sum_marks - 1) / threads + 1;

  LOG_DEBUG(log, "Sum marks: {}, threads: {}, min_marks_per_thread: {}, prefetches limit: {}, total_size_approx: {}", sum_marks, threads,
            min_marks_per_thread, reader_settings.filesystem_prefetches_limit, total_size_approx);
  ...
  for (size_t i = 0, part_idx = 0; i < threads && part_idx < per_part_infos.size(); ++i) {
    ...
    while (need_marks > 0 && part_idx < per_part_infos.size()) {
      ...
      if (part_stat.sum_marks == marks_to_get_from_part) {
        ...
        if (part_stat.sum_marks < marks_to_get_from_part) {
          throw Exception(ErrorCodes::LOGICAL_ERROR, "Requested {} marks from part {}, but part has only {} marks", marks_to_get_from_part,
                          getPartNameForLogging(per_part_infos[part_idx]->data_part), part_stat.sum_marks);
        }
      ...
      if (allowed_memory_usage && (!allowed_prefetches_num.has_value() || allowed_prefetches_num.value() > 0)) {
        ...
        if (allow_prefetch) {
          ...
          if (allowed_prefetches_num.has_value()) *allowed_prefetches_num -= part_stat.required_readers_num;
        }
      }
      ...
      if (allow_prefetch) prefetch_queue.emplace(TaskHolder{thread_task.get(), i});
...
bool MergeTreePrefetchedReadPool::checkReadMethodAllowed(RemoteFSReadMethod method) { return method == RemoteFSReadMethod::threadpool; }
