...
ReplicatedMergeTreePartCheckThread::~ReplicatedMergeTreePartCheckThread() { stop(); }
...
void ReplicatedMergeTreePartCheckThread::enqueuePart(const String& name, time_t delay_to_check_seconds) {
  ...
  if (parts_set.contains(name)) return;
...
BackgroundSchedulePoolTaskHolder& ReplicatedMergeTreePartCheckThread::getTask() { return pausable_task.getTask(); }

BackgroundSchedulePoolPausableTask::PauseHolderPtr ReplicatedMergeTreePartCheckThread::temporaryPause() { return pausable_task.pause(); }
...
void ReplicatedMergeTreePartCheckThread::cancelRemovedPartsCheck(const MergeTreePartInfo& drop_range_info) {
  ...
  for (auto& part : parts_to_remove) removed_parts.emplace(std::move(part));
  size_t count = 0;
  ...
  for (const auto& elem : parts_queue) {
    ...
    if (is_removed != should_have_been_removed)
      throw Exception(ErrorCodes::LOGICAL_ERROR, "Inconsistent parts_queue: name={}, is_removed={}, should_have_been_removed={}", elem.name,
                      is_removed, should_have_been_removed);
    count += is_removed;
  }
  ...
  auto new_end = std::remove_if(parts_queue.begin(), parts_queue.end(),
                                [&removed_parts](const auto& elem) { return removed_parts.contains(elem.name); });

  parts_queue.erase(new_end, parts_queue.end());

  for (const auto& elem : removed_parts) parts_set.erase(elem);
}
...
bool ReplicatedMergeTreePartCheckThread::searchForMissingPartOnOtherReplicas(const String& part_name) const {
   ...
   *   - or, if the replica never comes to life, then the administrator will delete or create a new replica with the same address and see
  ...
  for (const String& replica : replicas) {
    ...
    for (const String& part_on_replica : parts) {
      ...
      if (part_info == part_on_replica_info) {
        ...
        if (replica_path == storage.replica_path) continue;
      ...
      if (part_info.contains(part_on_replica_info)) {
        ...
        if (part_on_replica_info.max_block == part_info.max_block) {
          ...
          if (parts_found.empty() || parts_found.back() != part_on_replica) parts_found.push_back(part_on_replica);
        }

        if (found_part_with_the_same_min_block && found_part_with_the_same_max_block) {
          LOG_INFO(log,
                   "Found parts with the same min block and with the same max block as the missing part {} on replica {}. "
...
std::pair<bool, MergeTreeDataPartPtr> ReplicatedMergeTreePartCheckThread::findLocalPart(const String& part_name) {
  ...
  if (!part) part = storage.getActiveContainingPart(part_name);
...
ReplicatedCheckResult ReplicatedMergeTreePartCheckThread::checkPartImpl(const String& part_name, bool throw_on_broken_projection) {
  ...
  if (exists_in_zookeeper && !part) {
    ...
    if (outdated) {
      ...
      auto message = PreformattedMessage::create(
          "Part {} is Outdated, will wait for cleanup thread to handle it "
          "and check again after {}s",
          part_name, result.recheck_after_seconds);
  ...
  auto zookeeper = storage.getZooKeeper();
  auto table_lock = storage.lockForShare(RWLockImpl::NO_QUERY,
  ...
  if (exists_in_zookeeper) {
    ...
    try {
      ...
      checkDataPart(
          part,
          /* require_checksums */ true, is_broken_projection, [this] { return need_stop.load(); }, throw_on_broken_projection);
      ...
      if (is_broken_projection && throw_on_broken_projection) {
        ...
        message = PreformattedMessage::create("Part `{}` has broken projections. It will be ignored. Broken projections info: {}",
    ...
    auto message = PreformattedMessage::create("Young part {} with age {} seconds hasn't been added to ZooKeeper yet. It's ok.", part_name,
                                               (current_time - part->modification_time));
...
CheckResult ReplicatedMergeTreePartCheckThread::checkPartAndFix(const String& part_name, std::optional<time_t>* recheck_after,
  ...
  switch (result.action) {
    case ReplicatedCheckResult::None:
      UNREACHABLE();
    case ReplicatedCheckResult::DoNothing:
      break;
    ...
    case ReplicatedCheckResult::TryFetchMissing: {
      ...
      if (result.exists_in_zookeeper) {
        ...
        if (result.part)
          LOG_WARNING(
              log, "Part {} exists in ZooKeeper and the local part was broken. Detaching it, removing from ZooKeeper and queueing a fetch.",
              part_name);
        else
...
bool ReplicatedMergeTreePartCheckThread::onPartIsLostForever(const String& part_name) {
  ...
  if (lost_part_info.level != 0 || lost_part_info.mutation != 0) {
    ...
    if (part_in_queue && !source_parts.empty()) {
      ...
      for (const String& source_part_name : source_parts) enqueuePart(source_part_name);
...
void ReplicatedMergeTreePartCheckThread::run() {
  ...
  try {
...

    {
      ...
      selected_it = std::ranges::find_if(parts_queue, [current_time](const auto& elem) { return elem.time <= current_time; });
      ...
      if (selected_it == parts_queue.end()) {
        ...
        auto next_it =
            std::min_element(parts_queue.begin(), parts_queue.end(), [](const auto& l, const auto& r) { return l.time < r.time; });
    ...
    if (Coordination::isHardwareError(e.code)) return;
