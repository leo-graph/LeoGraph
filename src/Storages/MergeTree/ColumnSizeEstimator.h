...
class ColumnSizeEstimator {
 ...
 public:
  ...
  ColumnSizeEstimator(ColumnToSize&& map_, const NamesAndTypesList& key_columns, const NamesAndTypesList& ordinary_columns)
    ...
    for (const auto& [name, _] : key_columns) sum_index_columns += map.at(name);

    for (const auto& [name, _] : ordinary_columns) sum_ordinary_columns += map.at(name);

    sum_total = std::max(static_cast<decltype(sum_index_columns)>(1), sum_index_columns + sum_ordinary_columns);
  }

  Float64 columnWeight(const String& column) const { return static_cast<Float64>(map.at(column)) / static_cast<Float64>(sum_total); }

  Float64 keyColumnsWeight() const { return static_cast<Float64>(sum_index_columns) / static_cast<Float64>(sum_total); }
};
}  // namespace DB
