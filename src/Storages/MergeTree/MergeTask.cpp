...
class BuildStatisticsTransform : public ISimpleTransform {
 public:
  BuildStatisticsTransform(SharedHeader header, ColumnsStatistics statistics_to_build_)
      : ISimpleTransform(header, header, false), statistics_to_build(std::move(statistics_to_build_)) {}
...
class BuildStatisticsStep : public ITransformingStep {
 public:
  BuildStatisticsStep(SharedHeader input_header_, std::shared_ptr<BuildStatisticsTransform> transform_)
      : ITransformingStep(input_header_, input_header_, getTraits()), transform(std::move(transform_)) {}

  void transformPipeline(QueryPipelineBuilder &pipeline, const BuildQueryPipelineSettings &) override { pipeline.addTransform(transform); }
...
class RowsSourcesTemporaryFile : public ITemporaryFileLookup {
 public:
  ...
  WriteBuffer &getTemporaryFileForWriting(const String &name) override {
    if (name != FILE_ID) throw Exception(ErrorCodes::LOGICAL_ERROR, "Unexpected temporary file name requested: {}", name);
  ...
  std::unique_ptr<ReadBuffer> getTemporaryFileForReading(const String &name) override {
    ...
    if (!finalized) throw Exception(ErrorCodes::LOGICAL_ERROR, "Temporary file is not finalized yet");

    /// tmp_disk might not create real file if no data was written to it.
    if (final_size == 0) return std::make_unique<ReadBufferFromEmptyFile>();
  ...
  size_t finalizeWriting() {
    if (!tmp_data_buffer) throw Exception(ErrorCodes::LOGICAL_ERROR, "Temporary file was not requested for writing");
...
static void addMissedColumnsToSerializationInfos(size_t num_rows_in_parts, const Names &part_columns,
                                                 const ColumnsDescription &storage_columns,
                                                 const SerializationInfo::Settings &info_settings, SerializationInfoByName &new_infos) {
  NameSet part_columns_set(part_columns.begin(), part_columns.end());

  for (const auto &column : storage_columns) {
    if (part_columns_set.contains(column.name)) continue;

    if (column.default_desc.kind != ColumnDefaultKind::Default) continue;

    if (column.default_desc.expression) continue;
...
static String getColumnNameInStorage(const String &column_name, const NameSet &storage_columns) {
  if (storage_columns.contains(column_name)) return column_name;
...
void MergeTask::ExecuteAndFinalizeHorizontalPart::extractMergingAndGatheringColumns(
  ...
  for (const auto &name : sort_key_columns_vec) key_columns.insert(getColumnNameInStorage(name, storage_columns));

  /// Force sign column for Collapsing mode and VersionedCollapsing mode
  if (!global_ctx->merging_params.sign_column.empty()) key_columns.emplace(global_ctx->merging_params.sign_column);

  /// Force is_deleted column for Replacing mode
  if (!global_ctx->merging_params.is_deleted_column.empty()) key_columns.emplace(global_ctx->merging_params.is_deleted_column);

  /// Force version column for Replacing mode and VersionedCollapsing mode
  if (!global_ctx->merging_params.version_column.empty()) key_columns.emplace(global_ctx->merging_params.version_column);
  ...
  if (key_columns.empty()) key_columns.emplace(global_ctx->storage_columns.front().name);
  ...
  for (const auto &index : skip_indexes) {
    ...
    if (index.type == "text") {
      ...
      if (index_columns.size() > 1) {
        for (const auto &index_column : index_columns) key_columns.insert(getColumnNameInStorage(index_column, storage_columns));
      }
  ...
  if (ctx->need_remove_expired_values && canVerticalTTLDelete(*global_ctx)) {
    auto add_ttl_expression_columns = [&](const TTLDescription &ttl_descr) {
      for (const auto &col : ttl_descr.expression_columns) key_columns.insert(getColumnNameInStorage(col.name, storage_columns));
      for (const auto &col : ttl_descr.where_expression_columns) key_columns.insert(getColumnNameInStorage(col.name, storage_columns));
    };

    if (global_ctx->metadata_snapshot->hasRowsTTL()) add_ttl_expression_columns(global_ctx->metadata_snapshot->getRowsTTL());

    for (const auto &where_ttl : global_ctx->metadata_snapshot->getRowsWhereTTLs()) add_ttl_expression_columns(where_ttl);
  }
  ...
  for (const auto &column : global_ctx->storage_columns) {
    if (key_columns.contains(column.name)) {
      ...
      if (it != global_ctx->skip_indexes_by_column.end()) {
        for (auto &index : it->second) global_ctx->merging_skip_indexes.push_back(std::move(index));
...
bool MergeTask::ExecuteAndFinalizeHorizontalPart::prepare() const {
  ...
  LOG_DEBUG(ctx->log, "Merging {} parts: from {} to {} into {} with storage {}", global_ctx->future_part->parts.size(),
            global_ctx->future_part->parts.front()->name, global_ctx->future_part->parts.back()->name,
            global_ctx->future_part->part_format.part_type.toString(), global_ctx->future_part->part_format.storage_type.toString());
  ...
  if (global_ctx->parent_part) {
    auto data_part_storage =
        global_ctx->parent_part->getDataPartStorage().getProjection(local_tmp_part_basename, /* use parent transaction */ false);
  ...
  ///    non-deterministic results across parts.
  {
    ...
    for (const auto &part : global_ctx->future_part->parts) {
      for (const auto &col : part->getColumns()) columns_present_in_parts.emplace(col.name);
    }
  ...
  if (!expired_columns.empty()) {
    ...
    auto filter_columns = [&](const NamesAndTypesList &input, NamesAndTypesList &expired_out) {
      ...
      for (const auto &column : input) {
        ...
        if (is_expired) expired_out.push_back(column);

        if (!is_expired || is_required_for_merge) result.push_back(column);
      }
  ...
  if (enabledBlockNumberColumn(global_ctx)) addGatheringColumn(global_ctx, BlockNumberColumn::name, BlockNumberColumn::type);

  if (enabledBlockOffsetColumn(global_ctx)) addGatheringColumn(global_ctx, BlockOffsetColumn::name, BlockOffsetColumn::type);
  ...
  if (!patch_parts.empty()) {
    LOG_DEBUG(ctx->log, "Will apply {} patches up to version {}", patch_parts.size(),
  ...
  if (global_ctx->merge_may_reduce_rows) {
    ...
    for (const auto &part : global_ctx->future_part->parts) {
      ...
      if (result_statistics.empty()) continue;
  ...
  for (const auto &part : global_ctx->future_part->parts) {
    if (!info_settings.isAlwaysDefault()) {
      ...
      addMissedColumnsToSerializationInfos(part->rows_count, part->getColumns().getNames(), global_ctx->metadata_snapshot->getColumns(),
                                           info_settings, part_infos);
  ...
  switch (global_ctx->chosen_merge_algorithm) {
    ...
    case MergeAlgorithm::Vertical: {
      ...
      for (const auto &part : global_ctx->future_part->parts) part->accumulateColumnSizes(local_merged_column_to_size);
  ...
  auto index_granularity_ptr =
      createMergeTreeIndexGranularity(ctx->sum_input_rows_upper_bound, ctx->sum_uncompressed_bytes_upper_bound, *merge_tree_settings,
                                      global_ctx->new_data_part->index_granularity_info, ctx->blocks_are_granules_size);

  global_ctx->to = std::make_shared<MergedBlockOutputStream>(
      global_ctx->new_data_part, merge_tree_settings, global_ctx->metadata_snapshot, global_ctx->merging_columns,
      MergeTreeIndexFactory::instance().getMany(global_ctx->merging_skip_indexes), global_ctx->compression_codec,
      std::move(index_granularity_ptr), global_ctx->txn ? global_ctx->txn->tid : Tx::PrehistoricTID,
      global_ctx->merge_list_element_ptr->total_size_bytes_compressed,
      /*reset_columns=*/true, ctx->blocks_are_granules_size, global_ctx->context->getWriteSettings(),
  ...
  ctx->is_cancelled = [merges_blocker = global_ctx->merges_blocker, ttl_merges_blocker = global_ctx->ttl_merges_blocker,
                       need_remove = ctx->need_remove_expired_values, merge_list_element = global_ctx->merge_list_element_ptr,
                       partition_id = global_ctx->future_part->part_info.getPartitionId()]() -> bool {
    return merges_blocker->isCancelledForPartition(partition_id) || (need_remove && ttl_merges_blocker->isCancelled()) ||
...
void MergeTask::addGatheringColumn(GlobalRuntimeContextPtr global_ctx, const String &name, const DataTypePtr &type) {
  if (global_ctx->storage_columns.contains(name)) return;
...
bool MergeTask::hasLightweightDelete(const FutureMergedMutatedPartPtr &future_part) {
  for (const auto &part : future_part->parts) {
    if (part->hasLightweightDelete()) return true;
  }

  for (const auto &patch_part : future_part->patch_parts) {
    if (patch_part->hasLightweightDelete()) return true;
  }
...
bool MergeTask::isVerticalLightweightDelete(const GlobalRuntimeContext &global_ctx) {
  if (global_ctx.merging_params.mode != MergeTreeData::MergingParams::Ordinary) return false;

  if (global_ctx.chosen_merge_algorithm != MergeAlgorithm::Vertical) return false;

  if (!(*global_ctx.data_settings)[MergeTreeSetting::vertical_merge_optimize_lightweight_delete]) return false;
...
bool MergeTask::canVerticalTTLDelete(const GlobalRuntimeContext &global_ctx) {
  ...
  if (!(*global_ctx.data_settings)[MergeTreeSetting::vertical_merge_optimize_ttl_delete]) return false;

  if (global_ctx.metadata_snapshot->hasAnyGroupByTTL()) return false;

  if (global_ctx.metadata_snapshot->hasAnyColumnTTL()) return false;

  if (hasLightweightDelete(global_ctx.future_part)) return false;
...
bool MergeTask::isVerticalTTLDelete(const GlobalRuntimeContext &global_ctx, const ExecuteAndFinalizeHorizontalPartRuntimeContext &ctx) {
  ...
  if (!ctx.need_remove_expired_values) return false;
...
void MergeTask::ExecuteAndFinalizeHorizontalPart::cancel() noexcept {
  if (ctx->merge_projection_parts_task_ptr) ctx->merge_projection_parts_task_ptr->cancel();
}
...
void MergeTask::ExecuteAndFinalizeHorizontalPart::prepareProjectionsToMergeAndRebuild() const {
  ...
  for (const auto &projection : projections) {
    ...
    bool some_source_column_expired = std::any_of(required_columns.begin(), required_columns.end(), [&](const String &name) {
      return global_ctx->new_data_part->expired_columns.contains(name);
    });
    ...
    for (const auto &part : global_ctx->future_part->parts) {
      ...
      if (it != part->getProjectionParts().end() && !it->second->is_broken) projection_parts.push_back(it->second);
    }
...
void MergeTask::ExecuteAndFinalizeHorizontalPart::calculateProjections(const Block &block, UInt64 starting_offset) const {
  for (size_t i = 0, size = global_ctx->projections_to_rebuild.size(); i < size; ++i) {
    ...
    if (squashed_chunk) {
      ...
      auto tmp_part = MergeTreeDataWriter::writeTempProjectionPart(*global_ctx->data, ctx->log, result, projection,
                                                                   global_ctx->new_data_part.get(), ++ctx->projection_block_num);
...
void MergeTask::ExecuteAndFinalizeHorizontalPart::finalizeProjections() const {
  for (size_t i = 0, size = global_ctx->projections_to_rebuild.size(); i < size; ++i) {
    ...
    if (squashed_chunk) {
      auto result = projection_squash_plan.getHeader()->cloneWithColumns(squashed_chunk.detachColumns());
      auto temp_part = MergeTreeDataWriter::writeTempProjectionPart(*global_ctx->data, ctx->log, result, projection,
                                                                    global_ctx->new_data_part.get(), ++ctx->projection_block_num);
  ...
  if (ctx->projection_parts_iterator != std::make_move_iterator(ctx->projection_parts.end())) constructTaskForProjectionPartsMerge();
}
...
void MergeTask::ExecuteAndFinalizeHorizontalPart::constructTaskForProjectionPartsMerge() const {
  ...
  ctx->merge_projection_parts_task_ptr = std::make_unique<MergeProjectionPartsTask>(
      name, std::move(parts), projection, ctx->projection_block_num, global_ctx->context, global_ctx->holder, global_ctx->mutator,
      global_ctx->merge_entry, global_ctx->time_of_merge, global_ctx->new_data_part, global_ctx->space_reservation);
}
...
bool MergeTask::ExecuteAndFinalizeHorizontalPart::executeMergeProjections() const {
  ...
  if (!ctx->merge_projection_parts_task_ptr) return false;

  if (ctx->merge_projection_parts_task_ptr->executeStep()) return true;

  ++ctx->projection_parts_iterator;

  if (ctx->projection_parts_iterator == std::make_move_iterator(ctx->projection_parts.end())) return false;
...
bool MergeTask::ExecuteAndFinalizeHorizontalPart::executeImpl() const {
  ...
  do {
    ...
    if (global_ctx->merge_may_reduce_rows) {
      global_ctx->new_data_part->minmax_idx->update(block,
...
class ColumnGathererStep : public ITransformingStep {
 public:
  ColumnGathererStep(const SharedHeader &input_header_, const String &rows_sources_temporary_file_name_, UInt64 merge_block_size_rows_,
                     UInt64 merge_block_size_bytes_, std::optional<size_t> max_dynamic_subcolumns_, bool is_result_sparse_)
  ...
  void transformPipeline(QueryPipelineBuilder &pipeline, const BuildQueryPipelineSettings &pipeline_settings) override {
    ...
    auto transform =
        std::make_unique<ColumnGathererTransform>(header, input_streams_count, std::move(rows_sources_read_buf), merge_block_size_rows,
                                                  merge_block_size_bytes, max_dynamic_subcolumns, is_result_sparse);
...
MergeTask::VerticalMergeRuntimeContext::PreparedColumnPipeline MergeTask::VerticalMergeStage::createPipelineForReadingOneColumn(
  ...
  for (size_t part_num = 0; part_num < global_ctx->future_part->parts.size(); ++part_num) {
    ...
    createReadFromPartStep(MergeTreeSequentialSourceType::Merge, *plan_for_part, *global_ctx->data, global_ctx->storage_snapshot,
                           RangesInDataPart(global_ctx->future_part->parts[part_num], nullptr, part_num, part_starting_offset),
                           global_ctx->alter_conversions[part_num], global_ctx->merged_part_offsets, Names{column_name},
                           global_ctx->input_rows_filtered, apply_deleted_mask,
                           /*filter=*/std::nullopt, ctx->read_with_direct_io, ctx->use_prefetch, global_ctx->context,
  ...
  /// Union of all parts streams
  {
    ...
    for (auto &plan : plans) input_headers.emplace_back(plan->getCurrentHeader());
  ...
  /// Add column gatherer step
  {
    ...
    bool is_result_sparse =
        ISerialization::hasKind(global_ctx->new_data_part->getSerialization(column_name)->getKindStack(), ISerialization::Kind::SPARSE);
    auto merge_step = std::make_unique<ColumnGathererStep>(merge_column_query_plan.getCurrentHeader(), RowsSourcesTemporaryFile::FILE_ID,
                                                           (*merge_tree_settings)[MergeTreeSetting::merge_max_block_size],
                                                           (*merge_tree_settings)[MergeTreeSetting::merge_max_block_size_bytes],
                                                           max_dynamic_subcolumns, is_result_sparse);
...
void MergeTask::VerticalMergeStage::prepareVerticalMergeForOneColumn() const {
  ...
  ctx->column_parts_pipeline.setProgressCallback(
  ...
  ctx->column_to = std::make_unique<MergedColumnOnlyOutputStream>(
      global_ctx->new_data_part, global_ctx->data_settings, global_ctx->metadata_snapshot, columns_list, column_pipepline.indexes_to_recalc,
      global_ctx->compression_codec, global_ctx->to->getIndexGranularity(),
      global_ctx->merge_list_element_ptr->total_size_bytes_uncompressed, &global_ctx->written_offset_substreams);
...
bool MergeTask::VerticalMergeStage::executeVerticalMergeForOneColumn() const {
  ...
  do {
    ...
    if (global_ctx->isCancelled() || !ctx->executor->pull(block)) return false;
...
void MergeTask::VerticalMergeStage::finalizeVerticalMergeForOneColumn() const {
  ...
  global_ctx->gathered_data.columns_substreams = ColumnsSubstreams::merge(global_ctx->gathered_data.columns_substreams, columns_substreams,
                                                                          global_ctx->new_data_part->getColumns().getNames());

  auto cached_marks = ctx->column_to->releaseCachedMarks();
  for (auto &[name, marks] : cached_marks) global_ctx->cached_marks.emplace(name, std::move(marks));

  auto cached_index_marks = ctx->column_to->releaseCachedIndexMarks();
  for (auto &[name, marks] : cached_index_marks) global_ctx->cached_index_marks.emplace(name, std::move(marks));
  ...
  global_ctx->merge_list_element_ptr->progress.store(ctx->progress_before + ctx->column_sizes->columnWeight(column_name),
...
bool MergeTask::VerticalMergeStage::finalizeVerticalMergeForAllColumns() const {
  for (auto &stream : ctx->delayed_streams) stream->finish(ctx->need_sync);
...
bool MergeTask::MergeProjectionsStage::prepareProjections() const {
  /// Print overall profiling info. NOTE: it may duplicates previous messages
  {
    ...
    LOG_DEBUG(ctx->log, "Merge sorted {} rows, containing {} columns ({} merged, {} gathered) in {} sec., {} rows/sec., {}/sec.",
              global_ctx->merge_list_element_ptr->rows_read.load(), global_ctx->storage_columns.size(), global_ctx->merging_columns.size(),
              global_ctx->gathering_columns.size(), elapsed_seconds,
  ...
  if (global_ctx->merged_part_offsets && !global_ctx->merged_part_offsets->isFinalized()) global_ctx->merged_part_offsets->flush();
  ...
  for (const auto &projection : global_ctx->projections_to_merge) {
    ...
    LOG_DEBUG(ctx->log, "Selected {} projection_parts from {} to {}", projection_parts.size(), projection_parts.front()->name,
    ...
    ctx->tasks_for_projections.emplace_back(std::make_shared<MergeTask>(
        projection_future_part, projection->metadata, global_ctx->merge_entry,
        std::make_unique<MergeListElement>((*global_ctx->merge_entry)->table_id, projection_future_part, global_ctx->context),
        global_ctx->time_of_merge, global_ctx->context, *global_ctx->holder, global_ctx->space_reservation, global_ctx->deduplicate,
        global_ctx->deduplicate_by_columns, global_ctx->cleanup, projection_merging_params, global_ctx->need_prefix, projection,
        global_ctx->new_data_part.get(), projection->with_parent_part_offset ? global_ctx->merged_part_offsets : nullptr, ".proj",
        NO_TRANSACTION_PTR, global_ctx->data, global_ctx->mutator, global_ctx->merges_blocker, global_ctx->ttl_merges_blocker));
  }

  /// merge projections with _part_offset first so that we can release offset mapping earlier.
  std::sort(ctx->tasks_for_projections.begin(), ctx->tasks_for_projections.end(),
...
bool MergeTask::MergeProjectionsStage::executeProjections() const {
  if (ctx->projections_iterator == ctx->tasks_for_projections.end()) return false;
  ...
  if ((*ctx->projections_iterator)->execute()) return true;
...
void MergeTask::VerticalMergeStage::cancel() noexcept {
  if (ctx->column_to) ctx->column_to->cancel();

  if (ctx->prepared_pipeline.has_value()) ctx->prepared_pipeline->pipeline.cancel();

  for (auto &stream : ctx->delayed_streams) stream->cancel();

  if (ctx->executor) ctx->executor->cancel();
}
...
void MergeTask::MergeTextIndexStage::cancel() noexcept {
  for (auto &task : ctx->merge_tasks) task->cancel();
}

std::vector<TextIndexSegment> MergeTask::MergeTextIndexStage::getTextIndexSegments(const String &part_name, const String &index_name,
                                                                                   size_t part_idx) const {
  ...
  for (const auto &transform : it->second) {
    if (transform->hasIndex(index_name)) return transform->getSegments(index_name, part_idx);
  }
...
bool MergeTask::MergeTextIndexStage::prepare() const {
  if (global_ctx->parent_part) return false;

  if (global_ctx->text_indexes_to_merge.empty()) return false;

  if (!global_ctx->merge_may_reduce_rows) {
    /// If merge text indexes without rebuilt, part offsets must be set.
    if (!global_ctx->merged_part_offsets) throw Exception(ErrorCodes::LOGICAL_ERROR, "Merged part offsets are not set");

    global_ctx->merged_part_offsets->flush();
  }

  if (global_ctx->temporary_text_index_storage) global_ctx->temporary_text_index_storage->commitTransaction();
  ...
  for (const auto &index : global_ctx->text_indexes_to_merge) {
    ...
    auto task =
        std::make_unique<MergeTextIndexesTask>(std::move(segments), global_ctx->new_data_part, index_ptr, global_ctx->merged_part_offsets,
                                               reader_settings, global_ctx->to->getWriterSettings());
...
bool MergeTask::MergeTextIndexStage::execute() const {
  ...
  if (ctx->merge_tasks.empty()) return false;
...
bool MergeTask::MergeTextIndexStage::finalize() const {
  ...
  if (global_ctx->merged_part_offsets && global_ctx->projections_to_merge.empty()) global_ctx->merged_part_offsets->clear();

  if (global_ctx->temporary_text_index_storage) global_ctx->temporary_text_index_storage->removeRecursive();
...
void MergeTask::MergeProjectionsStage::cancel() noexcept {
  for (auto &prj_task : ctx->tasks_for_projections) prj_task->cancel();
}
...
bool MergeTask::VerticalMergeStage::executeVerticalMergeForAllColumns() const {
  ...
  if (ctx->it_name_and_type == global_ctx->gathering_columns.end()) return false;
  ...
  switch (ctx->vertical_merge_one_column_state) {
    ...
    case VerticalMergeRuntimeContext::State::NEED_EXECUTE: {
      if (executeVerticalMergeForOneColumn()) return true;
...
bool MergeTask::execute() try {
  ...
  ++stages_iterator;
  if (stages_iterator == stages.end()) return false;
...
void MergeTask::cancel() noexcept {
  if (stages_iterator != stages.end()) (*stages_iterator)->cancel();

  if (global_ctx->merging_executor) global_ctx->merging_executor->cancel();

  global_ctx->merged_pipeline.cancel();

  if (global_ctx->to) global_ctx->to->cancel();

  if (global_ctx->new_data_part) global_ctx->new_data_part->removeIfNeeded();
}

/// Apply merge strategy (Ordinary, Collapsing, Aggregating, etc) to the stream
class MergePartsStep : public ITransformingStep {
 public:
  MergePartsStep(const SharedHeader &input_header_, const SortDescription &sort_description_,
                 const Names partition_and_sorting_required_columns_, const MergeTreeData::MergingParams &merging_params_,
                 const String &rows_sources_temporary_file_name_, const std::optional<String> &filter_column_name_,
                 UInt64 merge_block_size_rows_, UInt64 merge_block_size_bytes_, std::optional<size_t> max_dynamic_subcolumns_,
                 bool blocks_are_granules_size_, bool cleanup_, time_t time_of_merge_)
      : ITransformingStep(input_header_, input_header_, getTraits()),
    ...
    switch (merging_params.mode) {
      case MergeTreeData::MergingParams::Ordinary:
        merged_transform = std::make_shared<MergingSortedTransform>(
            ...
            /* limit_= */ 0,
            /* always_read_till_end_= */ false, rows_sources_write_buf, filter_column_name, blocks_are_granules_size);
        break;

      case MergeTreeData::MergingParams::Collapsing:
        merged_transform = std::make_shared<CollapsingSortedTransform>(
            header, input_streams_count, sort_description, merging_params.sign_column, false, merge_block_size_rows, merge_block_size_bytes,
            max_dynamic_subcolumns, rows_sources_write_buf, blocks_are_granules_size);
        break;

      case MergeTreeData::MergingParams::Summing:
        merged_transform = std::make_shared<SummingSortedTransform>(header, input_streams_count, sort_description,
                                                                    merging_params.columns_to_sum, partition_and_sorting_required_columns,
                                                                    merge_block_size_rows, merge_block_size_bytes, max_dynamic_subcolumns);
        break;
      ...
      case MergeTreeData::MergingParams::Replacing:
        merged_transform =
            std::make_shared<ReplacingSortedTransform>(header, input_streams_count, sort_description, merging_params.is_deleted_column,
                                                       merging_params.version_column, merge_block_size_rows, merge_block_size_bytes,
                                                       max_dynamic_subcolumns, rows_sources_write_buf, blocks_are_granules_size, cleanup);
        break;

      case MergeTreeData::MergingParams::Coalescing:
        merged_transform = std::make_shared<CoalescingSortedTransform>(
            header, input_streams_count, sort_description, merging_params.columns_to_sum, partition_and_sorting_required_columns,
            merge_block_size_rows, merge_block_size_bytes, max_dynamic_subcolumns);
        break;
      ...
      case MergeTreeData::MergingParams::VersionedCollapsing:
        merged_transform = std::make_shared<VersionedCollapsingTransform>(
            header, input_streams_count, sort_description, merging_params.sign_column, merge_block_size_rows, merge_block_size_bytes,
            max_dynamic_subcolumns, rows_sources_write_buf, blocks_are_granules_size);
        break;
    }
...
class TTLDeleteFilterStep : public ITransformingStep {
 public:
  TTLDeleteFilterStep(const SharedHeader &input_header_, const ContextPtr &context_, const StorageMetadataPtr &metadata_snapshot_,
                      const IMergeTreeDataPart::TTLInfos &old_ttl_infos_, time_t current_time_, bool force_)
  ...
  void transformPipeline(QueryPipelineBuilder &pipeline, const BuildQueryPipelineSettings &) override {
    pipeline.addSimpleTransform(
        [state = shared_state](const SharedHeader &header) { return std::make_shared<TTLDeleteFilterTransform>(header, state); });
  }
...
class TTLStep : public ITransformingStep {
 public:
  TTLStep(const SharedHeader &input_header_, const ContextPtr &context_, const MergeTreeData &storage_,
          const StorageMetadataPtr &metadata_snapshot_, const MergeTreeData::MutableDataPartPtr &data_part_,
          const NamesAndTypesList &expired_columns_, time_t current_time, bool force_)
      : ITransformingStep(input_header_, TTLTransform::addExpiredColumnsToBlock(input_header_, expired_columns_), getTraits()) {
    transform = std::make_shared<TTLTransform>(context_, input_header_, storage_, metadata_snapshot_, data_part_, expired_columns_,
                                               current_time, force_);
 ...
 public:
  ...
  void transformPipeline(QueryPipelineBuilder &pipeline, const BuildQueryPipelineSettings &) override {
    ...
    if (!isCompatibleHeader(*pipeline.getSharedHeader(), *getOutputHeader())) {
      auto converting_dag = ActionsDAG::makeConvertingActions(pipeline.getSharedHeader()->getColumnsWithTypeAndName(),
                                                              getOutputHeader()->getColumnsWithTypeAndName(),
                                                              ActionsDAG::MatchColumnsMode::Name, getContext());

      auto converting_actions = std::make_shared<ExpressionActions>(std::move(converting_dag));

      pipeline.addSimpleTransform(
          [&](const SharedHeader &header) { return std::make_shared<ExpressionTransform>(header, std::move(converting_actions)); });
    }
  }
...
void MergeTask::addSkipIndexesExpressionSteps(QueryPlan &plan, const IndicesDescription &indices_description,
                                              const GlobalRuntimeContextPtr &global_ctx) {
  auto indices_expression =
      indices_description.getSingleExpressionForIndices(global_ctx->metadata_snapshot->getColumns(), global_ctx->data->getContext());
...
void MergeTask::addBuildTextIndexesStep(QueryPlan &plan, const IMergeTreeDataPart &data_part, const GlobalRuntimeContextPtr &global_ctx) {
  if (global_ctx->text_indexes_to_merge.empty()) return;
  ...
  for (const auto &index : global_ctx->text_indexes_to_merge) {
    ...
    if (!read_any_required_column) continue;
  ...
  if (indexes_to_build.empty()) return;
  ...
  MergeTreeWriterSettings writer_settings(global_ctx->data->getContext()->getSettingsRef(), global_ctx->context->getWriteSettings(),
                                          global_ctx->data->getSettings(), global_ctx->new_data_part,
  ...
  auto transform = std::make_shared<BuildTextIndexTransform>(
      plan.getCurrentHeader(), "tmp_" + data_part.name, std::move(indexes_to_build), global_ctx->temporary_text_index_storage,
      std::move(writer_settings), global_ctx->compression_codec,
...
BuildStatisticsTransformPtr MergeTask::addBuildStatisticsStep(QueryPlan &plan, const IMergeTreeDataPart &data_part,
                                                              const GlobalRuntimeContextPtr &global_ctx) {
  auto it = global_ctx->statistics_to_build_by_part.find(data_part.name);
  if (it == global_ctx->statistics_to_build_by_part.end() || it->second.empty()) return nullptr;

  auto read_column_names = plan.getCurrentHeader()->getNameSet();
  ColumnsStatistics statistics_to_build;

  for (const auto &[column_name, column_stats] : it->second) {
    if (read_column_names.contains(column_name)) statistics_to_build.emplace(column_name, column_stats->cloneEmpty());
  }

  if (statistics_to_build.empty()) return nullptr;

  auto transform = std::make_shared<BuildStatisticsTransform>(plan.getCurrentHeader(), std::move(statistics_to_build));
...
void MergeTask::ExecuteAndFinalizeHorizontalPart::createMergedStream() const {
  ...
  for (size_t i = 0; i < global_ctx->future_part->parts.size(); ++i) {
    ...
    if (part->getMarksCount() == 0) LOG_TRACE(ctx->log, "Part {} is empty", part->name);

    createReadFromPartStep(MergeTreeSequentialSourceType::Merge, *plan_for_part, *global_ctx->data, global_ctx->storage_snapshot,
                           RangesInDataPart(part, nullptr, i, part_starting_offset), global_ctx->alter_conversions[i],
                           global_ctx->merged_part_offsets, merging_column_names, global_ctx->input_rows_filtered, apply_deleted_mask,
                           /*filter=*/std::nullopt, ctx->read_with_direct_io,
                           /*prefetch=*/false, global_ctx->context, ctx->log);
  ...
  if (global_ctx->metadata_snapshot->hasSortingKey()) {
    ...
    auto extracting_subcolumns_dag = createSubcolumnsExtractionActions(
        *merge_parts_query_plan.getCurrentHeader(), sorting_key_expression_dag.getRequiredColumnsNames(), global_ctx->data->getContext());
    auto calculate_sorting_key_expression_step =
        std::make_unique<ExpressionStep>(merge_parts_query_plan.getCurrentHeader(),
  ...
  if (global_ctx->vertical_ttl_delete) {
    auto ttl_filter_step =
        std::make_unique<TTLDeleteFilterStep>(merge_parts_query_plan.getCurrentHeader(), global_ctx->context, global_ctx->metadata_snapshot,
                                              global_ctx->new_data_part->ttl_infos, global_ctx->time_of_merge, ctx->force_ttl);
  ...
  /// Merge
  {
    ...
    auto merge_step = std::make_unique<MergePartsStep>(
        merge_parts_query_plan.getCurrentHeader(), sort_description, partition_and_sorting_required_columns, global_ctx->merging_params,
        (is_vertical_merge ? RowsSourcesTemporaryFile::FILE_ID : ""),  /// rows_sources' temporary file is used only for vertical merge
        filter_column_name, (*merge_tree_settings)[MergeTreeSetting::merge_max_block_size],
        (*merge_tree_settings)[MergeTreeSetting::merge_max_block_size_bytes], max_dynamic_subcolumns, is_vertical_merge, cleanup,
  ...
  if (global_ctx->deduplicate) {
    ...
    if (global_ctx->deduplicate_by_columns.empty()) {
      for (const auto &column : global_ctx->merging_columns) {
        if (virtuals.tryGet(column.name, VirtualsKind::Persistent)) continue;

        global_ctx->deduplicate_by_columns.emplace_back(column.name);
      }
    }

    auto deduplication_step = std::make_unique<DistinctStep>(merge_parts_query_plan.getCurrentHeader(), SizeLimits(), 0 /*limit_hint*/,
                                                             global_ctx->deduplicate_by_columns, false /*pre_distinct*/);
  ...
  if (ctx->need_remove_expired_values || !global_ctx->merging_columns_expired_by_ttl.empty()) {
    auto ttl_step = std::make_unique<TTLStep>(merge_parts_query_plan.getCurrentHeader(), global_ctx->context, *global_ctx->data,
                                              global_ctx->metadata_snapshot, global_ctx->new_data_part,
                                              global_ctx->merging_columns_expired_by_ttl, global_ctx->time_of_merge, ctx->force_ttl);
  ...
  if (!subqueries.empty()) addCreatingSetsStep(merge_parts_query_plan, std::move(subqueries), global_ctx->context);
  ...
  global_ctx->merged_pipeline.setProgressCallback(
...
MergeAlgorithm MergeTask::ExecuteAndFinalizeHorizontalPart::chooseMergeAlgorithm() const {
  ...
  if (global_ctx->deduplicate) return MergeAlgorithm::Horizontal;
  if ((*merge_tree_settings)[MergeTreeSetting::enable_vertical_merge_algorithm] == 0) return MergeAlgorithm::Horizontal;
  if (ctx->need_remove_expired_values) {
    if (!canVerticalTTLDelete(*global_ctx)) return MergeAlgorithm::Horizontal;
  }
  if (global_ctx->future_part->part_format.part_type != MergeTreeDataPartType::Wide) return MergeAlgorithm::Horizontal;
  if (global_ctx->future_part->part_format.storage_type != MergeTreeDataPartStorageType::Full) return MergeAlgorithm::Horizontal;
  if (global_ctx->cleanup) return MergeAlgorithm::Horizontal;

  if (!(*merge_tree_settings)[MergeTreeSetting::allow_vertical_merges_from_compact_to_wide_parts]) {
    for (const auto &part : global_ctx->future_part->parts) {
      if (!isWidePart(part)) return MergeAlgorithm::Horizontal;
    }
  }
  ...
  bool enough_ordinary_cols =
      global_ctx->gathering_columns.size() >= (*merge_tree_settings)[MergeTreeSetting::vertical_merge_algorithm_min_columns_to_activate];
