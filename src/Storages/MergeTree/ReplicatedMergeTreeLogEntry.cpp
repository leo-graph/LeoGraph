...
String ReplicatedMergeTreeLogEntryData::typeToString(Type type) {
  switch (type) {
    ...
    case ReplicatedMergeTreeLogEntryData::DROP_RANGE:
      return "DROP_RANGE";
    case ReplicatedMergeTreeLogEntryData::CLEAR_COLUMN:
      return "CLEAR_COLUMN";
    case ReplicatedMergeTreeLogEntryData::CLEAR_INDEX:
      return "CLEAR_INDEX";
    case ReplicatedMergeTreeLogEntryData::REPLACE_RANGE:
      return "REPLACE_RANGE";
    case ReplicatedMergeTreeLogEntryData::MUTATE_PART:
      return "MUTATE_PART";
    case ReplicatedMergeTreeLogEntryData::ALTER_METADATA:
      return "ALTER_METADATA";
    case ReplicatedMergeTreeLogEntryData::SYNC_PINNED_PART_UUIDS:
      return "SYNC_PINNED_PART_UUIDS";
    case ReplicatedMergeTreeLogEntryData::CLONE_PART_FROM_SHARD:
      return "CLONE_PART_FROM_SHARD";
    case ReplicatedMergeTreeLogEntryData::DROP_PART:
      return "DROP_PART";
    default:
...
void ReplicatedMergeTreeLogEntryData::writeText(WriteBuffer& out) const {
  ...
  if (!deduplicate_by_columns.empty()) format_version = std::max<UInt8>(format_version, FORMAT_WITH_DEDUPLICATE_BY_COLUMNS);
  ...
  if (new_part_uuid != UUIDHelpers::Nil) format_version = std::max<UInt8>(format_version, FORMAT_WITH_UUID);

  if (!log_entry_id.empty()) format_version = std::max<UInt8>(format_version, FORMAT_WITH_LOG_ENTRY_ID);
  ...
  switch (type) {
    ...
    case MERGE_PARTS:
      ...
      if (!patch_parts.empty()) {
        ...
        for (const auto& s : patch_parts) out << "\n" << s;
      }
      break;
  ...
  if (part_type != PartType::Wide && part_type != PartType::Unknown) out << "part_type: " << part_type.toString() << "\n";

  auto storage_type = new_part_format.storage_type;
  if (storage_type != StorageType::Full && storage_type != StorageType::Unknown) out << "storage_type: " << storage_type.toString() << "\n";
...
void ReplicatedMergeTreeLogEntryData::readText(ReadBuffer& in, MergeTreeDataFormatVersion partition_format_version) {
  ...
  if (format_version >= FORMAT_WITH_CREATE_TIME) {
    ...
    create_time = makeDateTime(DateLUT::serverTimezoneInstance(), create_time_dt.year(), create_time_dt.month(), create_time_dt.day(),
  ...
  if (format_version >= FORMAT_WITH_BLOCK_ID) {
    ...
    if (!block_hashes_string.empty()) splitInto<','>(deduplication_block_ids, block_hashes_string, true);
  }
  ...
  if (type_str == "get") {
    ...
    if (!drop_range_info.isFakeDropRangePart()) type = DROP_PART;
...
ReplicatedMergeTreeLogEntry::Ptr ReplicatedMergeTreeLogEntry::parse(const String& s, const Coordination::Stat& stat,
  ...
  if (!res->create_time) res->create_time = stat.ctime / 1000;

  return res;
}

std::optional<String> ReplicatedMergeTreeLogEntryData::getDropRange(MergeTreeDataFormatVersion format_version) const {
  if (type == DROP_RANGE) return new_part_name;

  if (type == DROP_PART) return new_part_name;
...
bool ReplicatedMergeTreeLogEntryData::isDropPart(MergeTreeDataFormatVersion) const { return type == DROP_PART; }
...
Strings ReplicatedMergeTreeLogEntryData::getVirtualPartNames(MergeTreeDataFormatVersion format_version) const {
  ...
  if (type == REPLACE_RANGE) {
    ...
    auto drop_range_info = MergeTreePartInfo::fromPartName(replace_range_entry->drop_range_part_name, format_version);
    if (auto drop_range = getDropRange(format_version)) res.emplace_back(*drop_range);
    return res;
  }
  ...
  if (type == DROP_PART) return {};

  /// Doesn't produce any part.
  if (type == SYNC_PINNED_PART_UUIDS) return {};

  /// Doesn't produce any part by itself.
  if (type == CLONE_PART_FROM_SHARD) return {};
