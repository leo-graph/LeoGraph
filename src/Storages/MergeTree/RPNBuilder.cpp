...
void appendColumnNameWithoutAlias(const ActionsDAG::Node &node, WriteBuffer &out, const ContextPtr &context, bool use_analyzer,
                                  bool legacy = false) {
  switch (node.type) {
...
const ActionsDAG::Node *getNodeWithoutAlias(const ActionsDAG::Node *node) {
  ...
  while (result->type == ActionsDAG::ActionType::ALIAS) result = result->children[0];
...
RPNBuilderTreeContext::RPNBuilderTreeContext(ContextPtr query_context_) : query_context(std::move(query_context_)) {}
...
std::string RPNBuilderTreeNode::getColumnNameWithModuloLegacy() const {
  ...
  return getColumnNameWithoutAlias(*dag_node, getTreeContext().getQueryContext(),
                                   getTreeContext().getSettings()[Setting::allow_experimental_analyzer], true /*legacy*/);
}
...
bool RPNBuilderTreeNode::isConstant() const {
  if (ast_node) {
    ...
    if (block_with_constants.has(column_name) && isColumnConst(*block_with_constants.getByName(column_name).column)) return true;
...
bool RPNBuilderTreeNode::isSubqueryOrSet() const {
  if (ast_node) {
    return typeid_cast<const ASTSubquery *>(ast_node) || typeid_cast<const ASTTableIdentifier *>(ast_node);
  }
...
ColumnWithTypeAndName RPNBuilderTreeNode::getConstantColumn() const {
  if (!isConstant()) throw Exception(ErrorCodes::LOGICAL_ERROR, "RPNBuilderTree node is not a constant");
...
bool RPNBuilderTreeNode::tryGetConstant(Field &output_value, DataTypePtr &output_type) const {
  if (ast_node) {
    ...
    if (const auto *literal = ast_node->as<ASTLiteral>()) {
      ...
      if (!block_with_constants.has(column_name)) column_name = "_dummy";
      ...
      if (!output_value.isNull()) output_type = removeNullable(output_type);
...
FutureSetPtr tryGetSetFromDAGNode(const ActionsDAG::Node *dag_node) {
  ...
  if (const auto *column_set = typeid_cast<const ColumnSet *>(column)) return column_set->getData();
...
FutureSetPtr RPNBuilderTreeNode::tryGetPreparedSet() const {
  ...
  if (ast_node && prepared_sets) {
    ...
    if (it != sets.end() && !it->second.empty()) return it->second.at(0);
...
RPNBuilderFunctionTreeNode RPNBuilderTreeNode::toFunctionNode() const {
  if (!isFunction()) throw Exception(ErrorCodes::LOGICAL_ERROR, "RPNBuilderTree node is not a function");

  if (ast_node) return RPNBuilderFunctionTreeNode(ast_node, tree_context);
...
std::optional<RPNBuilderFunctionTreeNode> RPNBuilderTreeNode::toFunctionNodeOrNull() const {
  ...
  if (ast_node) return RPNBuilderFunctionTreeNode(this->ast_node, tree_context);
  return RPNBuilderFunctionTreeNode(getNodeWithoutAlias(dag_node), tree_context);
}

std::string RPNBuilderFunctionTreeNode::getFunctionName() const {
  if (ast_node) return assert_cast<const ASTFunction *>(ast_node)->name;
  return dag_node->function_base->getName();
}

FunctionBasePtr RPNBuilderFunctionTreeNode::getFunctionBase() const { return ast_node ? nullptr : dag_node->function_base; }
...
RPNBuilderTreeNode RPNBuilderFunctionTreeNode::getArgumentAt(size_t index) const {
  ...
  if (index >= total_arguments)
    throw Exception(ErrorCodes::LOGICAL_ERROR, "RPNBuilderFunctionTreeNode has {} arguments, attempted to get argument at index {}",
...
RPNBuilder<RPNElement>::RPNBuilder(const ActionsDAG::Node *filter_actions_dag_node, ContextPtr query_context_,
...
void RPNBuilder<RPNElement>::traverseTree(const RPNBuilderTreeNode &node) {
  ...
  if (!extract_atom_from_tree_function(node, element)) element.function = RPNElement::FUNCTION_UNKNOWN;
...
bool RPNBuilder<RPNElement>::extractLogicalOperatorFromTree(const RPNBuilderFunctionTreeNode &function_node, RPNElement &out) {
  ...
  if (function_name == "not") {
    if (function_node.getArgumentsSize() != 1) return false;
