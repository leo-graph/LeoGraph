...
void buildScatterSelector(const ColumnRawPtrs &columns, PODArray<size_t> &partition_num_to_first_row, IColumn::Selector &selector,
                          size_t max_parts, ContextPtr context) {
  ...
  for (size_t i = 0; i < num_rows; ++i) {
    ...
    if (partitions_count > 1) selector[i] = it->getMapped();
  }
...
void updateTTLInfo(MergeTreeDataPartTTLInfo &ttl_info, const PaddedPODArray<TTLType> &ttl_data, const WhereColumnType *where_column,
...
void updateTTLInfo(MergeTreeDataPartTTLInfo &ttl_info, const IColumn &ttl_column, const WhereColumnType *where_column) {
  ...
  if (const ColumnUInt16 *column_date = typeid_cast<const ColumnUInt16 *>(&ttl_column)) {
    updateTTLInfo<with_where>(ttl_info, column_date->getData(), where_column,
                              [&date_lut](UInt16 val) { return date_lut.fromDayNum(DayNum(val)); });
  } else if (const ColumnUInt32 *column_date_time = typeid_cast<const ColumnUInt32 *>(&ttl_column)) {
    updateTTLInfo<with_where>(ttl_info, column_date_time->getData(), where_column, [](UInt32 val) { return val; });
  } else if (const ColumnInt32 *column_date_32 = typeid_cast<const ColumnInt32 *>(&ttl_column)) {
    updateTTLInfo<with_where>(ttl_info, column_date_32->getData(), where_column,
                              [&date_lut](Int32 val) { return date_lut.fromDayNum(ExtendedDayNum(val)); });
  } else if (const ColumnDateTime64 *column_date_time_64 = typeid_cast<const ColumnDateTime64 *>(&ttl_column)) {
    updateTTLInfo<with_where>(ttl_info, column_date_time_64->getData(), where_column,
                              [scale = column_date_time_64->getScale()](DateTime64 val) { return val / intExp10OfSize<Int64>(scale); });
  } else {
...
bool hasRowsInFilter(const ColumnType &where_column) {
  for (size_t i = 0; i < where_column.size(); ++i) {
    if (where_column.getBool(i)) return true;
  }
  return false;
}
...
void updateTTLInfoConst(MergeTreeDataPartTTLInfo &ttl_info, const ColumnConst &ttl_column, const IColumn *where_column) {
  if (where_column && !hasRowsInFilter(*where_column)) return;
...
void updateTTL(const ContextPtr context, const TTLDescription &ttl_entry, IMergeTreeDataPart::TTLInfos &ttl_infos,
               MergeTreeDataPartTTLInfo &ttl_info, const Block &block, bool update_part_min_max_ttls) {
  auto expr_and_set = ttl_entry.buildExpression(context);
  for (auto &subquery : expr_and_set.sets->getSubqueries()) subquery->buildSetInplace(context);
  ...
  if (ttl_entry.where_expression_ast) {
    ...
    for (auto &subquery : where_expr_and_set.sets->getSubqueries()) subquery->buildSetInplace(context);
    ...
    if (where_column) where_column = where_column->convertToFullColumnIfConst();
  }
  ...
  if (update_part_min_max_ttls) ttl_infos.updatePartMinMaxTTL(ttl_info.min, ttl_info.max);
}
...
void addSubcolumnsFromSortingKeyAndSkipIndicesExpression(const ExpressionActionsPtr &expr, Block &block) {
  ...
  for (const auto &required_column : expr->getRequiredColumns()) {
    if (!block.has(required_column)) block.insert(block.getSubcolumnByName(required_column));
  }
}
...
void MergeTreeTemporaryPart::finalize() {
  for (auto &stream : streams) stream.finalizer.finish();

  part->getDataPartStorage().precommitTransaction();
  for (const auto &[_, projection] : part->getProjectionParts()) projection->getDataPartStorage().precommitTransaction();
}
...
BlocksWithPartition MergeTreeDataWriter::splitBlockIntoParts(Block &&block, size_t max_parts, const StorageMetadataPtr &metadata_snapshot,
                                                             ContextPtr context) {
  ...
  for (const auto &element : partition_key_names_and_types) {
    ...
    if (!partition_columns.back()->hasEqualValues()) all_partition_columns_are_equal = false;
  }
  auto get_partition = [&](size_t row_num) {
    Row partition(partition_columns.size());
    for (size_t i = 0; i < partition_columns.size(); ++i) partition[i] = (*partition_columns[i])[row_num];
    return partition;
  };
  ...
  for (size_t col = 0; col < block.columns(); ++col) {
    ...
    for (size_t i = 0; i < partitions_count; ++i) result[i].block->getByPosition(col).column = std::move(scattered[i]);
  }

  for (auto &item : result) item.partition_id = item.partition.getID(metadata_snapshot->getPartitionKey().sample_block);

  return result;
}

Block MergeTreeDataWriter::mergeBlock(Block &&block, const StorageMetadataPtr &metadata_snapshot, SortDescription sort_description,
                                      IColumn::Permutation *&permutation, const MergeTreeData::MergingParams &merging_params) {
  ...
  auto get_merging_algorithm = [&]() -> std::shared_ptr<IMergingAlgorithm> {
    switch (merging_params.mode) {
      ...
      case MergeTreeData::MergingParams::Replacing:
        return std::make_shared<ReplacingSortedAlgorithm>(header, 1, sort_description, merging_params.is_deleted_column,
                                                          merging_params.version_column, block_size + 1, /*block_size_bytes=*/0,
                                                          /*max_dynamic_subcolumns=*/std::nullopt);
      case MergeTreeData::MergingParams::Collapsing:
        return std::make_shared<CollapsingSortedAlgorithm>(header, 1, sort_description, merging_params.sign_column, false, block_size + 1,
                                                           /*block_size_bytes=*/0, /*max_dynamic_subcolumns=*/std::nullopt,
                                                           getLogger("MergeTreeDataWriter"), /*out_row_sources_buf_=*/nullptr,
      ...
      case MergeTreeData::MergingParams::Summing: {
        ...
        return std::make_shared<SummingSortedAlgorithm>(header, 1, sort_description, merging_params.columns_to_sum, required_columns,
                                                        block_size + 1, /*block_size_bytes=*/0, /*max_dynamic_subcolumns=*/std::nullopt,
                                                        "sumWithOverflow", "sumMapWithOverflow", true, false);
      }
      ...
      case MergeTreeData::MergingParams::VersionedCollapsing:
        return std::make_shared<VersionedCollapsingAlgorithm>(header, 1, sort_description, merging_params.sign_column, block_size + 1,
                                                              /*block_size_bytes=*/0, /*max_dynamic_subcolumns=*/std::nullopt);
      case MergeTreeData::MergingParams::Graphite:
        return std::make_shared<GraphiteRollupSortedAlgorithm>(header, 1, sort_description, block_size + 1, /*block_size_bytes=*/0,
                                                               /*max_dynamic_subcolumns=*/std::nullopt, merging_params.graphite_params,
                                                               time(nullptr));
      case MergeTreeData::MergingParams::Coalescing: {
        ...
        return std::make_shared<SummingSortedAlgorithm>(header, 1, sort_description, merging_params.columns_to_sum, required_columns,
                                                        block_size + 1, /*block_size_bytes=*/0, /*max_dynamic_subcolumns=*/std::nullopt,
                                                        "last_value", "last_value", false, true);
      }
    }
  };

  auto merging_algorithm = get_merging_algorithm();
  if (!merging_algorithm) return *header;
  ...
  if (status.required_source != 0)
    throw Exception(ErrorCodes::LOGICAL_ERROR,
                    ...
                    status.chunk.getNumRows(), status.is_finished, status.required_source, merging_algorithm->getName());

  status = merging_algorithm->merge();

  /// Check that merge is finished.
  if (!status.is_finished) throw Exception(ErrorCodes::LOGICAL_ERROR, "Merge is not finished after the second merge.");
...
MergeTreeTemporaryPartPtr MergeTreeDataWriter::writeTempPart(BlockWithPartition &block, StorageMetadataPtr metadata_snapshot,
                                                             ContextPtr context) {
  auto partition_id = block.partition.getID(metadata_snapshot->getPartitionKey().sample_block);
  return writeTempPartImpl(block, std::move(metadata_snapshot), std::move(partition_id), /*source_parts_set=*/{}, std::move(context),
                           data.insert_increment.get());
}

MergeTreeTemporaryPartPtr MergeTreeDataWriter::writeTempPatchPart(BlockWithPartition &block, StorageMetadataPtr metadata_snapshot,
                                                                  String partition_id, SourcePartsSetForPatch source_parts_set,
                                                                  ContextPtr context) {
  return writeTempPartImpl(block, std::move(metadata_snapshot), std::move(partition_id), std::move(source_parts_set), std::move(context),
                           data.insert_increment.get());
}

MergeTreeTemporaryPartPtr MergeTreeDataWriter::writeTempPartImpl(BlockWithPartition &block_with_partition,
                                                                 StorageMetadataPtr metadata_snapshot, String partition_id,
                                                                 SourcePartsSetForPatch source_parts_set, ContextPtr context,
                                                                 UInt64 block_number) {
  ...
  bool optimize_on_insert = !isPatchPartitionId(partition_id) && global_settings[Setting::optimize_on_insert] &&
  ...
  if (!source_parts_set.empty()) new_part_info.mutation = source_parts_set.getMaxDataVersion();

  String part_name;
  if (data.format_version < MERGE_TREE_DATA_MIN_FORMAT_VERSION_WITH_CUSTOM_PARTITIONING) {
    DayNum min_date(
        static_cast<DayNum::UnderlyingType>(minmax_idx->hyperrectangle[data.minmax_idx_date_column_pos].left.safeGet<UInt64>()));
    DayNum max_date(
        static_cast<DayNum::UnderlyingType>(minmax_idx->hyperrectangle[data.minmax_idx_date_column_pos].right.safeGet<UInt64>()));
    ...
    if (min_month != max_month) throw Exception(ErrorCodes::LOGICAL_ERROR, "Part spans more than one month.");
  ...
  if (!temp_postfix.empty()) temp_prefix += temp_postfix + "_";
  ...
  if (global_settings[Setting::materialize_skip_indexes_on_insert]) {
    ...
    if (!exclude_indexes_string.empty()) {
      std::unordered_set<String> exclude_index_names = parseIdentifiersOrStringLiteralsToSet(exclude_indexes_string, global_settings);

      for (const auto &index : index_descriptions)
        if (!exclude_index_names.contains(index.name)) indices.emplace_back(MergeTreeIndexFactory::instance().get(index));
  ...
  if (block.rows() == 0) return temp_part;
  ...
  const UInt64 &min_bytes_to_perform_insert = (*data_settings)[MergeTreeSetting::min_free_disk_bytes_to_perform_insert].changed
  ...
  const Float32 &min_ratio_to_perform_insert = (*data_settings)[MergeTreeSetting::min_free_disk_ratio_to_perform_insert].changed
  ...
  if (!is_system_database && (min_bytes_to_perform_insert > 0 || min_ratio_to_perform_insert > 0.0)) {
    ...
    if (!reservation) {
      throw Exception(ErrorCodes::NOT_ENOUGH_SPACE,
                      ...
                      volume->getName(), min_bytes_to_perform_insert, min_ratio_to_perform_insert);
    }
  } else {
  ...
  if ((*data.storage_settings.get())[MergeTreeSetting::assign_part_uuids]) new_data_part->uuid = UUIDHelpers::generateV4();
  ...
  for (const auto &ttl_entry : recompression_ttl_entries)
    updateTTL(context, ttl_entry, new_data_part->ttl_infos, new_data_part->ttl_infos.recompression_ttl[ttl_entry.result_column], block,
              false);
  ...
  auto index_granularity_ptr =
      createMergeTreeIndexGranularity(block.rows(), block.bytes(), *data_settings, new_data_part->index_granularity_info,
  ...
  auto out = std::make_unique<MergedBlockOutputStream>(
      new_data_part, data_settings, metadata_snapshot, columns, indices, compression_codec, std::move(index_granularity_ptr),
      (data.supportsTransactions() && context->getCurrentTransaction()) ? context->getCurrentTransaction()->tid : Tx::PrehistoricTID,
      block.bytes(),
      /*reset_columns=*/false,
      /*blocks_are_granules_size=*/false, context->getWriteSettings(), static_cast<WrittenOffsetSubstreams *>(nullptr));
  ...
  for (const auto &projection : metadata_snapshot->getProjections()) {
    ...
    if (projection_block.rows()) {
      ...
      for (auto &stream : proj_temp_part->streams) temp_part->streams.emplace_back(std::move(stream));
    }
  }

  out->finalizeIndexGranularity();
  auto finalizer = out->finalizePartAsync(new_data_part, gathered_data, (*data_settings)[MergeTreeSetting::fsync_after_insert]);
...
MergeTreeTemporaryPartPtr MergeTreeDataWriter::writeProjectionPartImpl(const String &part_name, bool is_temp,
                                                                       IMergeTreeDataPart *parent_part, const MergeTreeData &data,
                                                                       LoggerPtr log, Block block, const ProjectionDescription &projection,
  ...
  if (is_temp) projection_part_storage->beginTransaction();
  ...
  auto out = std::make_unique<MergedBlockOutputStream>(
      new_data_part, data_settings, metadata_snapshot, columns, MergeTreeIndices{}, compression_codec, std::move(index_granularity_ptr),
      Tx::PrehistoricTID, block.bytes(),
      /*reset_columns=*/false,
      /*blocks_are_granules_size=*/false, data.getContext()->getWriteSettings(), static_cast<WrittenOffsetSubstreams *>(nullptr));
...
MergeTreeTemporaryPartPtr MergeTreeDataWriter::writeProjectionPart(const MergeTreeData &data, LoggerPtr log, Block block,
                                                                   const ProjectionDescription &projection, IMergeTreeDataPart *parent_part,
                                                                   bool merge_is_needed) {
  return writeProjectionPartImpl(projection.name, false /* is_temp */, parent_part, data, log, std::move(block), projection,
                                 merge_is_needed);
}

/// This is used for projection materialization process which may contain multiple stages of
/// projection part merges.
MergeTreeTemporaryPartPtr MergeTreeDataWriter::writeTempProjectionPart(const MergeTreeData &data, LoggerPtr log, Block block,
                                                                       const ProjectionDescription &projection,
                                                                       IMergeTreeDataPart *parent_part, size_t block_num) {
  auto part_name = fmt::format("{}_{}", projection.name, block_num);
  auto new_part =
      writeProjectionPartImpl(part_name, /*is_temp=*/true, parent_part, data, log, std::move(block), projection, /*merge_is_needed=*/true);
