...
static MutationCommand createCommandWithUpdatedColumns(const MutationCommand &command,
  ...
  if (command.predicate) res.predicate = command.predicate->clone();

  if (command.partition) res.partition = command.partition->clone();
  ...
  for (const auto &child : alter_ast.update_assignments->children) {
    ...
    if (res.column_to_update_expression.contains(assignment.column_name)) new_assignments->children.push_back(child->clone());
  }
...
static bool isLightweightDeleteCommand(const String &column_name, const ASTPtr &ast) {
  if (column_name != RowExistsColumn::name) return false;

  const auto *literal = ast->as<ASTLiteral>();
  if (!literal) return false;

  if (literal->value.getType() != Field::Types::UInt64) return false;
...
static MutationCommand createLightweightDeleteCommand(const MutationCommand &command) {
  ...
  if (command.partition) alter_command->partition = alter_command->children.emplace_back(command.partition->clone()).get();
  ...
  if (!mutation_command) throw Exception(ErrorCodes::LOGICAL_ERROR, "Failed to parse command {}", alter_command->formatForErrorMessage());

  return *mutation_command;
}

AlterConversions::AlterConversions(const MutationCommands &mutation_commands_, const PatchPartsForReader &patch_parts_,
                                   const ContextPtr &context) {
  for (const auto &command : mutation_commands_) addMutationCommand(command, context);

  for (const auto &patch : patch_parts_) addPatchPart(patch);

  /// Do not throw if there are no mutations or patches.
  if (number_of_alter_mutations > 1) {
    if (!mutation_commands.empty())
      throw Exception(ErrorCodes::NOT_IMPLEMENTED, "Applying mutations on-fly is not supported with more than one ALTER MODIFY");

    if (!patch_parts.empty())
      throw Exception(ErrorCodes::NOT_IMPLEMENTED, "Applying patch parts is not supported with more than one on-fly ALTER MODIFY");
  }
}

bool AlterConversions::hasLightweightDelete() const { return all_updated_columns.contains(RowExistsColumn::name); }
...
bool AlterConversions::isSupportedAlterMutation(MutationCommand::Type type) { return type == MutationCommand::READ_COLUMN; }

bool AlterConversions::isSupportedMetadataMutation(MutationCommand::Type type) {
  return type == MutationCommand::RENAME_COLUMN || type == MutationCommand::DROP_COLUMN;
}
...
void AlterConversions::addMutationCommand(const MutationCommand &command, const ContextPtr &context) {
  ...
  if (command.type == RENAME_COLUMN) {
    ...
    if (!chained) rename_map.emplace_back(RenamePair{command.rename_to, command.column_name});
    ...
    if (command.data_type) all_updated_columns.insert(command.column_name);
    ...
    if (result.nondeterministic_function_name)
      throw Exception(ErrorCodes::BAD_ARGUMENTS,
                      ...
                      *result.nondeterministic_function_name);

    for (const auto &[column, _] : command.column_to_update_expression) all_updated_columns.insert(column);
...
void AlterConversions::addPatchPart(PatchPartInfoForReader patch_part) {
  for (const auto &column : patch_part.part->getColumns()) {
    if (isPatchPartSystemColumn(column.name)) continue;
...
bool AlterConversions::columnHasNewName(const std::string &old_name) const {
  for (const auto &[new_name, prev_name] : rename_map) {
    if (old_name == prev_name) return true;
  }
...
std::string AlterConversions::getColumnNewName(const std::string &old_name) const {
  for (const auto &[new_name, prev_name] : rename_map) {
    if (old_name == prev_name) return new_name;
  }
...
bool AlterConversions::isColumnRenamed(const std::string &new_name) const {
  for (const auto &[name_to, name_from] : rename_map) {
    if (name_to == new_name) return true;
  }
  return false;
}
...
std::string AlterConversions::getColumnOldName(const std::string &new_name) const {
  for (const auto &[name_to, name_from] : rename_map) {
    if (name_to == new_name) return name_from;
  }
...
bool AlterConversions::isColumnDropped(const std::string &name) const {
  ...
  if (dropped_columns.contains(name)) return true;

  /// Check if the parent nested column was dropped (e.g. DROP COLUMN `n` should match `n.s`, `n.d`, etc.)
  auto nested_prefix_end = name.find('.');
  if (nested_prefix_end != std::string::npos && dropped_columns.contains(name.substr(0, nested_prefix_end))) return true;

  return false;
}

PrewhereExprSteps AlterConversions::getMutationSteps(const IMergeTreeDataPartInfoForReader &part_info,
                                                     const NamesAndTypesList &read_columns, const StorageMetadataPtr &metadata_snapshot,
  ...
  for (const auto &patch : patch_parts) {
    ...
    if (apply_deleted_mask && patch.part->hasLightweightDelete()) {
      ...
      has_column_in_patch = std::ranges::any_of(read_columns, [&](const auto &column) {
        if (isPatchPartSystemColumn(column.name)) return false;
...
std::vector<MutationActions> AlterConversions::getMutationActions(const IMergeTreeDataPartInfoForReader &part_info,
                                                                  const NamesAndTypesList &read_columns,
                                                                  const StorageMetadataPtr &metadata_snapshot,
                                                                  const ContextPtr &context) const {
  if (mutation_commands.empty()) return {};

  const auto *loaded_part_info = dynamic_cast<const LoadedMergeTreeDataPartInfoForReader *>(&part_info);
  if (!loaded_part_info) throw Exception(ErrorCodes::NOT_IMPLEMENTED, "Applying mutations on-fly is supported only for loaded data parts");
  ...
  for (const auto &column : read_columns) {
    auto name_in_storage = column.getNameInStorage();
    if (storage_read_columns_set.emplace(name_in_storage).second) storage_read_columns.emplace_back(name_in_storage);
  }
  ...
  if (filtered_commands.empty()) return {};
  ...
  MutationsInterpreter interpreter(const_cast<MergeTreeData &>(part->storage), part, alter_conversions, metadata_snapshot,
                                   std::move(filtered_commands), std::move(storage_read_columns), context, settings);

  return interpreter.getMutationActions();
}

void AlterConversions::addColumnsRequiredForMaterialized(Names &read_columns, NameSet &read_columns_set,
                                                         const StorageMetadataPtr &metadata_snapshot, const ContextPtr &context) const {
  ...
  for (const auto &column_name : read_columns_set) {
    ...
    if (default_desc && default_desc->kind == ColumnDefaultKind::Materialized) {
      ...
      for (const auto &dependency : syntax_result->requiredSourceColumns()) {
        if (all_updated_columns.contains(dependency)) required_source_columns.insert(dependency);
      }
    }
  }

  for (const auto &column_name : required_source_columns) {
    if (read_columns_set.emplace(column_name).second) read_columns.push_back(column_name);
  }
}
...
MutationCommands AlterConversions::filterMutationCommands(Names &read_columns, NameSet read_columns_set) const {
  ...
  for (const auto &command : mutation_commands | std::views::reverse) {
    ...
    for (const auto &column : source_columns) {
      if (read_columns_set.emplace(column).second) read_columns.push_back(column);
    }
  }
...
void MutationCounters::assertNotNegative() const {
  if (num_data < 0) throw Exception(ErrorCodes::LOGICAL_ERROR, "On-fly data mutations counter is negative ({})", num_data);

  if (num_alter < 0) throw Exception(ErrorCodes::LOGICAL_ERROR, "On-fly alter mutations counter is negative ({})", num_alter);

  if (num_metadata < 0) throw Exception(ErrorCodes::LOGICAL_ERROR, "On-fly metadata mutations counter is negative ({})", num_metadata);
}
