...
ReplicatedMergeTreeSink::ReplicatedMergeTreeSink(bool async_insert_, StorageReplicatedMergeTree &storage_,
                                                 const StorageMetadataPtr &metadata_snapshot_, size_t quorum_size,
                                                 size_t quorum_timeout_ms_, size_t max_parts_per_block_, bool quorum_parallel_,
                                                 bool majority_quorum, ContextPtr context_, bool is_attach_,
    ...
    : SinkToStorage(std::make_shared<const Block>(metadata_snapshot_->getSampleBlock())),
      ...
      deduplicate(async_insert_ ? (*storage.getSettings())[MergeTreeSetting::replicated_deduplication_window_for_async_inserts] != 0
  ...
  if (required_quorum_size == 1) required_quorum_size = 0;
  ...
  LOG_DEBUG(log,
            "Create ReplicatedMergeTreeSink {} async_insert={}, deduplicate={}, quorum_size={}, max_parts_per_block={}, "
            "quorum_parallel={}, is_attach={}",
            storage.getStorageID().getNameForLogs(), is_async_insert, deduplicate,
            required_quorum_size.has_value() ? toString(*required_quorum_size) : "majority", max_parts_per_block_, quorum_parallel_,
            is_attach_);
}

ReplicatedMergeTreeSink::~ReplicatedMergeTreeSink() {
  if (delayed_parts.empty()) return;
...
size_t ReplicatedMergeTreeSink::checkQuorumPrecondition(const ZooKeeperWithFaultInjectionPtr &zookeeper) {
  if (!isQuorumEnabled()) return 0;

  size_t replicas_number = 0;

  const auto &settings = context->getSettingsRef();
  ZooKeeperRetriesControl quorum_retries_ctl(
      "checkQuorumPrecondition", log,
      {settings[Setting::insert_keeper_max_retries], settings[Setting::insert_keeper_retry_initial_backoff_ms],
  ...
  quorum_retries_ctl.retryLoop([&]() {
    ...
    for (const auto &replica : replicas)
      if (replica != storage.replica_name) exists_paths.emplace_back(fs::path(storage.zookeeper_path) / "replicas" / replica / "is_active");
    ...
    if (!quorum_parallel && zookeeper->tryGet(quorum_info.status_path, quorum_status))
      throw Exception(ErrorCodes::UNSATISFIED_QUORUM_FOR_PREVIOUS_WRITE, "Quorum for previous write has not been satisfied yet. Status: {}",
                      quorum_status);
...
void ReplicatedMergeTreeSink::consume(Chunk &chunk) {
  ...
  if (num_blocks_processed > 0) storage.delayInsertOrThrowIfNeeded(&storage.partial_shutdown_event, context, false);
  ...
  BlocksWithPartition part_blocks =
      MergeTreeDataWriter::splitBlockIntoParts(std::move(block), max_parts_per_block, metadata_snapshot, context);
  ...
  for (auto &current_block : part_blocks) {
...

    {
      ...
      if (result.removed_rows > 0) {
        ...
        LOG_DEBUG(log, "In partition {} self deduplication removed tokens {} out of {}, left rows {} in tokens {}, debug: {}",
                  current_block.partition_id, result.removed_tokens, current_deduplication_info->getCount(), result.filtered_block->rows(),
                  result.deduplication_info->getCount(), result.deduplication_info->debug());
    ...
    if (!temp_part->part) continue;

    if (!support_parallel_write && temp_part->part->getDataPartStorage().supportParallelWrite()) support_parallel_write = true;
    ...
    LOG_DEBUG(
        log, "Wrote block with {} rows{} and deduplication blocks: {}, deduplication info: {}", current_block.block->rows(),
        quorumLogMessage(),
    ...
    for (const auto &stream : temp_part->streams) current_streams += stream.stream->getNumberOfOpenStreams();
...
void ReplicatedMergeTreeSink::finishDelayed(const ZooKeeperWithFaultInjectionPtr &zookeeper) {
  ...
  for (auto &partition : delayed_parts) {
    ...
    {
      Stopwatch watch;
      SCOPE_EXIT({ partition.elapsed_ns += watch.elapsed(); });
      ...
      while (true) {
        ...
        auto deduplication_hashes =
            partition.deduplication_info->getDeduplicationHashes(partition.block_with_partition.partition_id, deduplicate);
        ...
        if (isQuorumEnabled()) {
          if (conflicts.empty())
            ...
            for (const auto &hash : conflicts)
              if (hash.hasConflictPartName()) parts_to_wait_for_quorum.insert(hash.getConflictPartName());
          }
        }
        ...
        if (is_async_insert) storage.async_block_ids_cache.triggerCacheUpdate();

        {
          ProfileEventTimeIncrement<Microseconds> duplication_elapsed(ProfileEvents::DuplicationElapsedMicroseconds);

          LOG_DEBUG(log, "Found duplicate block IDs: {}, retry times {}", fmt::join(getDeduplicationBlockIds(conflicts), ", "),
                    retry_times);

          auto result = partition.deduplication_info->deduplicateBlock(getDeduplicationBlockIds(conflicts),
                                                                       partition.block_with_partition.partition_id, context);
          ...
          LOG_DEBUG(log,
                    "After filtering by collision, removed rows {}/{}, removed tokets {}/{} from origin block, after retry remaining rows: "
                    "{}, remaining tokens: {}, elapsed {} ms, new deduplication info debug: {}",
                    result.removed_rows, partition.deduplication_info->getRows(), result.removed_tokens,
                    partition.deduplication_info->getCount(), result.filtered_block->rows(), result.deduplication_info->getCount(),
                    duplication_elapsed.elapsed() / 1000, result.deduplication_info->debug());
        ...
        if (partition.block_with_partition.block->rows() == 0) {
          ...
          if (!is_async_insert) {
            ...
            LOG_INFO(log, "Block with ID {} {} as part {}; ignoring it.", block_id,
                     exists_locally ? "already exists locally" : "already exists on other replicas", actual_part_name);
          }
      ...
      if (isQuorumEnabled()) {
        for (const auto &part : parts_to_wait_for_quorum) resolveQuorum(zookeeper, part);
      }
    }

    // profile_events_scope has to be destroyed in the scope above
    auto counters_snapshot = std::make_shared<ProfileEvents::Counters::Snapshot>(partition.part_counters.getPartiallyAtomicSnapshot());
    PartLog::addNewPart(storage.getContext(), PartLog::PartLogEntry(partition.temp_part->part, partition.elapsed_ns, counters_snapshot),
...
bool ReplicatedMergeTreeSink::writeExistingPart(MergeTreeData::MutableDataPartPtr &part) {
  ...
  auto try_rollback_part_rename = [this, &part, &original_part_dir]() {
    if (original_part_dir == part->getDataPartStorage().getPartDirectory()) return;

    if (part->new_part_was_committed_to_zookeeper_after_rename_on_disk) return;
  ...
  if (deduplicate) {
    switch (insert_deduplication_version) {
      case InsertDeduplicationVersions::OLD_SEPARATE_HASHES:
        deduplication_hashes.emplace_back(
            DeduplicationHash::createSyncHash(part->checksums.getTotalChecksumUInt128(), part->info.getPartitionId()));
        break;
      case InsertDeduplicationVersions::COMPATIBLE_DOUBLE_HASHES:
        deduplication_hashes.emplace_back(
            DeduplicationHash::createSyncHash(part->checksums.getTotalChecksumUInt128(), part->info.getPartitionId()));
        deduplication_hashes.emplace_back(
            DeduplicationHash::createUnifiedHash(part->checksums.getTotalChecksumUInt128(), part->info.getPartitionId()));
        break;
  ...
  try {
    ...
    PartLog::addNewPart(storage.getContext(), PartLog::PartLogEntry(part, watch.elapsed(), profile_events_scope.getSnapshot()),
                        deduplication_ids, ExecutionStatus(error, error_message));
    return deduplicated;
  } catch (...) {
    try_rollback_part_rename();
    PartLog::addNewPart(storage.getContext(), PartLog::PartLogEntry(part, watch.elapsed(), profile_events_scope.getSnapshot()),
                        deduplication_ids, ExecutionStatus::fromCurrentException("", true));
    throw;
  }
}

std::vector<DeduplicationHash> ReplicatedMergeTreeSink::detectConflictsInAsyncBlockIDs(
  ...
  if (insert_deduplication_version != InsertDeduplicationVersions::NEW_UNIFIED_HASHES) {
    auto conflict_block_ids =
        storage.async_block_ids_cache.detectConflicts(deduplication_hashes, deduplication_async_inserts_cache_version);
...
struct CommitRetryContext {
  enum Stages { LOCK_AND_COMMIT, RESOLVE_CONFLICTS, FILTER_CONFLICTS_AND_RETRY, SUCCESS, ERROR };
...
std::vector<DeduplicationHash> ReplicatedMergeTreeSink::commitPart(const ZooKeeperWithFaultInjectionPtr &zookeeper,
  ...
  auto resolve_duplicate_stage = [&]() -> CommitRetryContext::Stages {
    ...
    for (size_t i = 0; i < retry_context.conflict_deduplication_hashes.size(); ++i) {
      ...
      if (resp.error == Coordination::Error::ZNONODE) continue;
  ...
  auto get_quorum_ops = [&](Coordination::Requests &ops) {
    ...
    if (isQuorumEnabled()) {
      ...
      ops.emplace_back(zkutil::makeCreateRequest(quorum_info.status_path, quorum_entry.toString(), zkutil::CreateMode::Persistent));

      /// Make sure that during the insertion time, the replica was not reinitialized or disabled (when the server is finished).
      ops.emplace_back(zkutil::makeCheckRequest(storage.replica_path + "/is_active", quorum_info.is_active_node_version));
      ...
      ops.emplace_back(zkutil::makeCheckRequest(storage.replica_path + "/host", quorum_info.host_node_version));
    }
  };
  ...
  auto get_logs_ops = [&](Coordination::Requests &ops) {
    ...
    ops.emplace_back(
        zkutil::makeCreateRequest(storage.zookeeper_path + "/log/log-", log_entry.toString(), zkutil::CreateMode::PersistentSequential));
  };

  auto sleep_before_commit_for_tests = [&]() {
    auto sleep_before_commit_local_part_in_replicated_table_ms =
        (*storage.getSettings())[MergeTreeSetting::sleep_before_commit_local_part_in_replicated_table_ms];
    if (sleep_before_commit_local_part_in_replicated_table_ms.totalMilliseconds()) {
      LOG_INFO(log, "committing part {}, triggered sleep_before_commit_local_part_in_replicated_table_ms {}", part->name,
               sleep_before_commit_local_part_in_replicated_table_ms.totalMilliseconds());
  ...
  auto commit_new_part_stage = [&]() -> CommitRetryContext::Stages {
    ...
    /// Save the current temporary path and name in case we need to revert the change to retry (ZK connection loss) or in case part is
    /// deduplicated.
    ...
    auto block_number_lock =
        storage.allocateBlockNumber(part->info.getPartitionId(), zookeeper, block_id_pathes, "", block_data);  /// 1 RTT
...

    {
      ...
      if (!conflict_path.empty()) {
        ...
        auto conflicted_hash_it =
            std::find_if(deduplication_hashes.begin(), deduplication_hashes.end(),
                         [&](const DeduplicationHash &hash) { return hash.getBlockId() == conflict_path.filename().string(); });
    ...
    if (Coordination::isHardwareError(multi_code)) {
      LOG_DEBUG(log, "Insert of part {} failed when committing to keeper (Reason: {}). Attempting to recover it", part->name, multi_code);
      ...
      new_retry_controller.retryLoop([&] {
        fiu_do_on(FailPoints::replicated_merge_tree_commit_zk_fail_when_recovering_from_hw_fault,
    ...
    if (multi_code == Coordination::Error::ZNODEEXISTS && !block_id_pathes.empty() &&
        std::ranges::contains(block_id_pathes, failed_op_path.string())) {
      /// Block with the same id have just appeared in table (or other replica), rollback the insertion.
      LOG_INFO(log, "Block with ID {} already exists (it was just appeared) for part {}. Ignore it.", failed_op_path, part->name);
      ...
      auto conflicted_hash_it = std::find_if(deduplication_hashes.begin(), deduplication_hashes.end(), [&](const DeduplicationHash &hash) {
    ...
    if (!Coordination::isUserError(multi_code))
      throw Exception(ErrorCodes::UNEXPECTED_ZOOKEEPER_ERROR, "Unexpected ZooKeeper error while adding block {} with ID '{}': {}",
                      block_number, fmt::join(deduplication_block_ids, ", "), multi_code);

    if (multi_code == Coordination::Error::ZNONODE && failed_op_idx == block_unlock_op_idx)
      throw Exception(ErrorCodes::QUERY_WAS_CANCELLED, "Insert query (for block {}) was canceled by concurrent ALTER PARTITION or TRUNCATE",
    ...
    throw Exception(ErrorCodes::UNEXPECTED_ZOOKEEPER_ERROR, "Unexpected logical error while adding block {} with ID '{}': {}, path {}",
                    block_number, fmt::join(deduplication_block_ids, ", "), multi_code, failed_op_path);
  };
    ...
    try {
      switch (retry_context.stage) {
        ...
        case CommitRetryContext::FILTER_CONFLICTS_AND_RETRY:
          throw Exception(ErrorCodes::LOGICAL_ERROR, "Operation is already has a result.");
        ...
        case CommitRetryContext::ERROR:
          throw Exception(ErrorCodes::LOGICAL_ERROR, "Operation is already in error state.");
      }
  ...
  retries_ctl.retryLoop([&]() {
    ...
    while (true) {
      ...
      if (retry_context.stage == CommitRetryContext::SUCCESS || retry_context.stage == CommitRetryContext::FILTER_CONFLICTS_AND_RETRY) {
        /// operation is done
        return;
      }
    }
...
void ReplicatedMergeTreeSink::waitForQuorum(const ZooKeeperWithFaultInjectionPtr &zookeeper, const std::string &part_name,
                                            const std::string &quorum_path, int is_active_node_version, int host_node_version) const {
  ...
  while (true) {
    ...
    if (!zookeeper->tryGet(quorum_path, value, nullptr, event)) break;
    ...
    if (quorum_entry.part_name != part_name) break;
...
size_t ReplicatedMergeTreeSink::getQuorumSize() const {
  ...
  if (required_quorum_size) return required_quorum_size.value();

  return quorum_replicas_num / 2 + 1;
}

bool ReplicatedMergeTreeSink::isQuorumEnabled() const { return !required_quorum_size.has_value() || required_quorum_size.value() > 1; }

void ReplicatedMergeTreeSink::resolveQuorum(const ZooKeeperWithFaultInjectionPtr &zookeeper, std::string actual_part_name) {
  if (actual_part_name.empty()) return;
  ...
  quorum_info.status_path = fs::path(storage.zookeeper_path) / "quorum" / "status";
  if (quorum_parallel) quorum_info.status_path = fs::path(storage.zookeeper_path) / "quorum" / "parallel" / actual_part_name;
  ...
  quorum_retries_ctl.actionAfterLastFailedRetry([&] {
    ...
    quorum_retries_ctl.setUserError(
        Exception(ErrorCodes::UNKNOWN_STATUS_OF_INSERT,
                  "Unknown quorum status. The data was inserted in the local replica but we could not verify quorum. Reason: {}",
  ...
  quorum_retries_ctl.retryLoop([&]() {
    zookeeper->setKeeper(storage.getZooKeeper());
    waitForQuorum(zookeeper, actual_part_name, quorum_info.status_path, quorum_info.is_active_node_version, quorum_info.host_node_version);
  });
}

ZooKeeperWithFaultInjectionPtr ReplicatedMergeTreeSink::createKeeper(String name) {
  const auto &settings = context->getSettingsRef();
  return ZooKeeperWithFaultInjection::createInstance(settings[Setting::insert_keeper_fault_injection_probability],
                                                     settings[Setting::insert_keeper_fault_injection_seed], storage.getZooKeeper(), name,
                                                     log);
}
}  // namespace DB
