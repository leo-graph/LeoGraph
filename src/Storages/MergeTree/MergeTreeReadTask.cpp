...
String MergeTreeReadTaskColumns::dump() const {
  ...
  for (size_t i = 0; i < pre_columns.size(); ++i) s << "STEP " << i << ":\n" << pre_columns[i].toString() << "\n";

  s << "MAIN:\n" << columns.toString() << "\n";

  for (size_t i = 0; i < patch_columns.size(); ++i) s << "PATCH " << i << ":\n" << patch_columns[i].toString() << "\n";
...
Names MergeTreeReadTaskColumns::getAllColumnNames() const {
  ...
  for (const auto& step_columns : pre_columns) {
    for (const auto& column : step_columns) res.push_back(column.name);
  }

  for (const auto& column : columns) res.push_back(column.name);

  return res;
}

void MergeTreeReadTaskColumns::moveAllColumnsFromPrewhere() {
  for (auto& step_columns : pre_columns) columns.splice(columns.end(), std::move(step_columns));
...
void MergeTreeReadTask::Readers::updateAllMarkRanges(const MarkRanges& ranges) {
  ...
  for (auto& reader : prewhere) reader->updateAllMarkRanges(ranges);
}

MergeTreeReadTask::MergeTreeReadTask(MergeTreeReadTaskInfoPtr info_, Readers readers_, MarkRanges mark_ranges_,
                                     std::vector<MarkRanges> patches_mark_ranges_, const BlockSizeParams& block_size_params_,
                                     MergeTreeBlockSizePredictorPtr size_predictor_, RuntimeDataflowStatisticsCacheUpdaterPtr updater_)
    : info(std::move(info_)),
  ...
  if (updater) {
    dataflow_cache_update_cb = [&](const ColumnsWithTypeAndName& columns, size_t read_bytes,
...
static const IndexReadTask* getIndexReadTaskForReadStep(const IndexReadTasks& index_read_tasks, const NamesAndTypesList& columns_to_read) {
  if (index_read_tasks.empty()) return nullptr;

  std::unordered_map<String, String> column_to_index;

  for (const auto& [index_name, index_task] : index_read_tasks) {
    for (const auto& column : index_task.columns) column_to_index[column.name] = index_name;
  }
  ...
  for (const auto& column : columns_to_read) {
    ...
    if (it == column_to_index.end()) {
      ...
      throw Exception(ErrorCodes::LOGICAL_ERROR, "Found columns for multiple indexes ({} and {}) in one read step", index_for_step,
                      it->second);
    }
  }
  ...
  if (!index_for_step.empty() && has_non_index_columns) return nullptr;

  return index_for_step.empty() ? nullptr : &index_read_tasks.at(index_for_step);
}

MergeTreeReadTask::Readers MergeTreeReadTask::createReaders(const MergeTreeReadTaskInfoPtr& read_info, const Extras& extras,
                                                            const MarkRanges& ranges, const std::vector<MarkRanges>& patches_ranges) {
  Readers new_readers;
  ...
  auto create_reader = [&](const NamesAndTypesList& columns_to_read, bool is_prewhere) {
    ...
    return createMergeTreeReader(part_info, columns_to_read, extras.storage_snapshot, read_info->data_part->storage.getSettings(), ranges,
                                 read_info->const_virtual_fields, extras.uncompressed_cache, extras.mark_cache,
                                 is_prewhere ? nullptr : read_info->deserialization_prefixes_cache.get(), extras.reader_settings,
                                 extras.value_size_map, extras.profile_callback);
  };
  ...
  if (is_vector_search) new_readers.main->data_part_info_for_read->setReadHints(read_info->read_hints, read_info->task_columns.columns);
  ...
  for (const auto& pre_columns_per_step : read_info->task_columns.pre_columns) {
    if (const auto* index_read_task = getIndexReadTaskForReadStep(read_info->index_read_tasks, pre_columns_per_step)) {
      ...
      new_readers.prewhere.push_back(
          createMergeTreeReaderIndex(new_readers.main.get(), index_read_task->index, pre_columns_per_step, can_skip_marks));
    } else {
      new_readers.prewhere.push_back(create_reader(pre_columns_per_step, true));
    }

    if (is_vector_search) new_readers.prewhere.back()->data_part_info_for_read->setReadHints(read_info->read_hints, pre_columns_per_step);
  }

  auto create_patch_reader = [&](size_t part_idx) {
    return createMergeTreeReader(read_info->patch_parts[part_idx].part, read_info->task_columns.patch_columns[part_idx],
                                 extras.storage_snapshot, read_info->data_part->storage.getSettings(), patches_ranges[part_idx],
                                 read_info->const_virtual_fields, extras.uncompressed_cache, extras.mark_cache,
                                 /*deserialization_prefixes_cache=*/nullptr, extras.reader_settings, extras.value_size_map,
                                 extras.profile_callback);
  };

  for (size_t i = 0; i < read_info->patch_parts.size(); ++i) {
    new_readers.patches.push_back(getPatchReader(read_info->patch_parts[i], create_patch_reader(i), extras.patch_join_cache));
  }
...
MergeTreeReadersChain MergeTreeReadTask::createReadersChain(const Readers& task_readers, const PrewhereExprInfo& prewhere_actions,
  ...
  if (prewhere_actions.steps.size() != task_readers.prewhere.size()) {
    throw Exception(ErrorCodes::LOGICAL_ERROR, "PREWHERE steps count mismatch, actions: {}, readers: {}", prewhere_actions.steps.size(),
                    task_readers.prewhere.size());
  }
  ...
  if (task_readers.prepared_index) {
    range_readers.emplace_back(task_readers.prepared_index.get(), Block{},
                               /*prewhere_info_=*/nullptr, read_steps_performance_counters.getCounterForIndexStep(),
                               /*main_reader_=*/false);
  }

  size_t counter_idx = 0;
  for (size_t i = 0; i < prewhere_actions.steps.size(); ++i) {
    range_readers.emplace_back(task_readers.prewhere[i].get(), range_readers.empty() ? Block{} : range_readers.back().getSampleBlock(),
                               prewhere_actions.steps[i].get(), read_steps_performance_counters.getCountersForStep(counter_idx++),
                               /*main_reader_=*/false);
  }

  if (!task_readers.main->getColumns().empty()) {
    range_readers.emplace_back(task_readers.main.get(), range_readers.empty() ? Block{} : range_readers.back().getSampleBlock(),
                               /*prewhere_info_=*/nullptr, read_steps_performance_counters.getCountersForStep(counter_idx),
                               /*main_reader_=*/true);
  }
...
void MergeTreeReadTask::initializeReadersChain(const PrewhereExprInfo& prewhere_actions, MergeTreeIndexBuildContextPtr index_build_context,
                                               LazyMaterializingRowsPtr lazy_materializing_rows,
                                               const ReadStepsPerformanceCounters& read_steps_performance_counters) {
  if (readers_chain.isInitialized()) throw Exception(ErrorCodes::LOGICAL_ERROR, "Range readers chain is already initialized");

  PrewhereExprInfo all_prewhere_actions;

  if (index_build_context || lazy_materializing_rows) initializeIndexReader(index_build_context, lazy_materializing_rows);

  for (const auto& step : info->mutation_steps) all_prewhere_actions.steps.push_back(step);

  for (const auto& step : prewhere_actions.steps) all_prewhere_actions.steps.push_back(step);

  readers_chain = createReadersChain(readers, all_prewhere_actions, read_steps_performance_counters);
}

void MergeTreeReadTask::initializeIndexReader(const MergeTreeIndexBuildContextPtr& index_build_context,
                                              const LazyMaterializingRowsPtr& lazy_materializing_rows) {
  ...
  if (index_build_context) index_read_result = index_build_context->getPreparedIndexReadResult(*this);
  ...
  if (index_read_result || lazy_materializing_rows) {
    bool can_read_incomplete_granules =
        readers.main->canReadIncompleteGranules() &&
        std::ranges::all_of(readers.prewhere, [](const auto& reader) { return reader->canReadIncompleteGranules(); });

    readers.prepared_index =
        std::make_unique<MergeTreeReaderIndex>(readers.main.get(), std::move(index_read_result), part_rows, can_read_incomplete_granules);
  }
}
...
UInt64 MergeTreeReadTask::estimateNumRows() const {
  ...
  if (!rows_to_read) return rows_to_read;
  ...
  if (unread_rows_in_current_granule >= rows_to_read) return rows_to_read;
...
MergeTreeReadTask::BlockAndProgress MergeTreeReadTask::read() {
  ...
  if (size_predictor) size_predictor->startBlock();
  ...
  if (read_result.num_rows == 0) read_result.columns.clear();
  ...
  if (size_predictor) {
    ...
    if (!read_result.columns.empty()) size_predictor->update(sample_block, read_result.columns, read_result.num_rows);
  }
  ...
  BlockAndProgress res = {.block = std::move(block),
