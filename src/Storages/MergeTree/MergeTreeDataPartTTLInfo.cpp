...
void MergeTreeDataPartTTLInfos::update(const MergeTreeDataPartTTLInfos& other_infos) {
  ...
  for (const auto& [name, ttl_info] : other_infos.recompression_ttl) recompression_ttl[name].update(ttl_info);

  for (const auto& [expression, ttl_info] : other_infos.moves_ttl) moves_ttl[expression].update(ttl_info);
...
void MergeTreeDataPartTTLInfos::read(ReadBuffer& in) {
  ...
  if (json.has("columns")) {
    ...
    for (auto col : columns)  // NOLINT
    {
      ...
      if (col.has("finished")) ttl_info.ttl_finished = col["finished"].getUInt();
  ...
  if (json.has("table")) {
    ...
    if (table.has("finished")) table_ttl.ttl_finished = table["finished"].getUInt();
  ...
  auto fill_ttl_info_map = [this](const JSON& json_part, TTLInfoMap& ttl_info_map, bool update_min_max) {
    for (auto elem : json_part)  // NOLINT
    {
      ...
      if (elem.has("finished")) ttl_info.ttl_finished = elem["finished"].getUInt();

      String expression = elem["expression"].getString();
      ttl_info_map.emplace(expression, ttl_info);

      if (update_min_max) updatePartMinMaxTTL(ttl_info.min, ttl_info.max);
    }
  };
...
void MergeTreeDataPartTTLInfos::write(WriteBuffer& out) const {
  ...
  if (!columns_ttl.empty()) {
    ...
    for (auto it = columns_ttl.begin(); it != columns_ttl.end(); ++it) {
      if (it != columns_ttl.begin()) writeString(",", out);
  ...
  if (table_ttl.min) {
    if (!columns_ttl.empty()) writeString(",", out);
  ...
  auto write_infos = [&out](const TTLInfoMap& infos, const String& type, bool is_first) {
    if (!is_first) writeString(",", out);

    writeDoubleQuotedString(type, out);
    writeString(":[", out);
    for (auto it = infos.begin(); it != infos.end(); ++it) {
      if (it != infos.begin()) writeString(",", out);
  ...
  if (!rows_where_ttl.empty()) write_infos(rows_where_ttl, "rows_where", is_first);
...
time_t MergeTreeDataPartTTLInfos::getMinimalMaxRecompressionTTL() const {
  ...
  for (const auto& [name, info] : recompression_ttl)
    if (info.max != 0) max = std::min(info.max, max);

  if (max == std::numeric_limits<time_t>::max()) return 0;
...
bool MergeTreeDataPartTTLInfos::hasAnyNonFinishedTTLs() const {
  ...
  if (!table_ttl.finished()) return true;

  if (has_non_finished_ttl(columns_ttl)) return true;

  if (has_non_finished_ttl(rows_where_ttl)) return true;

  if (has_non_finished_ttl(moves_ttl)) return true;

  if (has_non_finished_ttl(recompression_ttl)) return true;

  if (has_non_finished_ttl(group_by_ttl)) return true;
...
auto tryToFindTTLExpressionInMapByASTMatching(const TTLInfoMap& ttl_info_map, const std::string& result_column) {
  ...
  for (auto it = ttl_info_map.begin(); it != ttl_info_map.end(); ++it) {
    ...
    if (ast_candidate->getTreeHash(false) == ast_needle->getTreeHash(false)) return it;
  }
  return ttl_info_map.end();
}
}  // namespace

std::optional<TTLDescription> selectTTLDescriptionForTTLInfos(const TTLDescriptions& descriptions, const TTLInfoMap& ttl_info_map,
                                                              time_t current_time, bool use_max) {
  ...
  for (auto ttl_entry_it = descriptions.begin(); ttl_entry_it != descriptions.end(); ++ttl_entry_it) {
    ...
    if (ttl_info_it == ttl_info_map.end()) {
      ...
      if (ttl_info_it == ttl_info_map.end()) continue;
    }
    ...
    if (ttl_time <= current_time && best_ttl_time <= ttl_time) {
