...
namespace {
size_t roundDownToMultiple(size_t num, size_t multiple) { return (num / multiple) * multiple; }

size_t takeFromRange(const MarkRange &range, size_t min_number_of_marks, size_t &current_marks_amount,
...
void sortResponseRanges(RangesInDataPartsDescription &result) {
  ...
  for (auto &ranges_in_part : result) {
    if (new_result.empty() || new_result.back().info != ranges_in_part.info)
      new_result.push_back(RangesInDataPartDescription{.info = ranges_in_part.info, .projection_name = ranges_in_part.projection_name});

    new_result.back().ranges.insert(new_result.back().ranges.end(), std::make_move_iterator(ranges_in_part.ranges.begin()),
                                    std::make_move_iterator(ranges_in_part.ranges.end()));
    ranges_in_part.ranges.clear();
  }

  /// Sort ranges for each part
  for (auto &ranges_in_part : new_result) std::sort(ranges_in_part.ranges.begin(), ranges_in_part.ranges.end());
 ...
 public:
  ...
  static String toString(Stats stats) {
    ...
    for (size_t i = 0; i < stats.size(); ++i)
      stats_by_replica.push_back(
          fmt::format("replica {}{} - {{requests: {} marks: {} assigned_to_me: {} stolen_by_hash: {} stolen_unassigned: {}}}", i,
                      stats[i].is_unavailable ? " is unavailable" : "", stats[i].number_of_requests, stats[i].sum_marks,
                      stats[i].assigned_to_me, stats[i].stolen_by_hash, stats[i].stolen_unassigned));
  ...
  ImplInterface(size_t replicas_count_, CoordinationMode mode_)
      : stats{replicas_count_}, replicas_count(replicas_count_), mode(mode_), replica_status(replicas_count_) {}
  ...
  void handleInitialAllRangesAnnouncement(InitialAllRangesAnnouncement announcement) {
    if (++received_initial_requests > replicas_count)
      throw Exception(ErrorCodes::LOGICAL_ERROR, "Initiator received more initial requests than there are replicas: replica_num={}",
...
/// So the guarantees we provide here are basically the same as with single-node reading: we will read from parts as their were seen by some
/// node at the moment when query started.
///
/// Knowing that almost each part could be read by each node, we suppose ranges of each part to be available to all the replicas and thus
/// distribute them evenly between them (of course we still check if replica has access to the given part before scheduling a reading from
/// it).
///
...
class DefaultCoordinator : public ParallelReplicasReadingCoordinator::ImplInterface {
 public:
  DefaultCoordinator(size_t replicas_count_, CoordinationMode mode_)
      : ParallelReplicasReadingCoordinator::ImplInterface(replicas_count_, mode_), distribution_by_hash_queue(replicas_count_) {}
 ...
 private:
  ...
  ///     * otherwise put this segment into `distribution_by_hash_queue` for its stealer_by_hash if it's available and can read from it
  ///     * otherwise put this segment into `ranges_for_stealing_queue`
  /// 3. when we figuring out that some replica is unavailable we move all segments from its `distribution_by_hash_queue` to their stealers
  /// by hash or to `ranges_for_stealing_queue`
  /// 4. when we get the announcement from a replica we move all segments it cannot read to their stealers by hash or to
  /// `ranges_for_stealing_queue`
  ///
  /// So, segments always move in one direction down this path (possibly skipping some stops):
  /// `all_parts_to_read` -> `distribution_by_hash_queue[owner]` -> `distribution_by_hash_queue[stealer_by_hash]` ->
  /// `ranges_for_stealing_queue`
  ...
  ///     2. it was available in the beginning of execution (since we got announcement), so if it will become unavailable at some point -
  ///     query will be failed with exception.
  ///        this means that we can delegate reading of all leftover segments (i.e. segments that were not read by their owner or stealer by
  ///        hash) to this node
  ...
  /// When replica asks coordinator for a new portion of data to read, it traverses `all_parts_to_read` to find ranges relevant to this
  /// replica (by consistent hash). Many hashes are being calculated during this process and just to not loose this time we save the
  /// information about all these ranges observed along the way to what node they belong to. Ranges in this queue might belong to a part
  /// that the given replica cannot read from - the corresponding check happens later.
  ...
  /// For some ranges their owner and stealer (by consistent hash) cannot read from the given part at all. So this range have to be stolen
  /// anyway.
  /// TODO: consider making it bounded in size
  ...
  /// The second and all subsequent announcements needed only to understand if we can schedule reading from the given part to the given
  /// replica.
  ...
  void selectPartsAndRanges(size_t replica_num, ScanMode scan_mode, size_t min_number_of_marks, size_t &current_marks_amount,
                            RangesInDataPartsDescription &description);

  size_t computeConsistentHash(const std::string &part_name, size_t segment_begin, ScanMode scan_mode) const;

  void tryToTakeFromDistributionQueue(size_t replica_num, size_t min_number_of_marks, size_t &current_marks_amount,
                                      RangesInDataPartsDescription &description);

  void tryToStealFromQueues(size_t replica_num, ScanMode scan_mode, size_t min_number_of_marks, size_t &current_marks_amount,
                            RangesInDataPartsDescription &description);

  void tryToStealFromQueue(auto &queue,
                           ssize_t owner,  /// In case `queue` is `distribution_by_hash_queue[replica]`
                           size_t replica_num, ScanMode scan_mode, size_t min_number_of_marks, size_t &current_marks_amount,
                           RangesInDataPartsDescription &description);

  void processPartsFurther(size_t replica_num, ScanMode scan_mode, size_t min_number_of_marks, size_t &current_marks_amount,
                           RangesInDataPartsDescription &description);
...
void DefaultCoordinator::initializeReadingState(InitialAllRangesAnnouncement announcement) {
  ...
  std::ranges::sort(all_parts_to_read,
                    [](const Part &lhs, const Part &rhs) { return BiggerPartsFirst()(lhs.description, rhs.description); });
  state_initialized = true;
  source_replica_for_parts_snapshot = announcement.replica_num;

  mark_segment_size = announcement.mark_segment_size;
  if (mark_segment_size == 0) throw Exception(ErrorCodes::BAD_ARGUMENTS, "Zero value provided for `mark_segment_size`");
...
void DefaultCoordinator::markReplicaAsUnavailable(size_t replica_number) {
  ...
  for (const auto &segment : distribution_by_hash_queue[replica_number]) {
    if (segment.ranges.empty()) continue;
...
void DefaultCoordinator::updateQueryProgress() {
  ...
  if (progress_callback) {
    ...
    for (const auto &part : all_parts_to_read) total_rows_to_read += part.description.rows;
...
void DefaultCoordinator::doHandleInitialAllRangesAnnouncement(InitialAllRangesAnnouncement announcement) {
  ...
  if (received_initial_requests == 1) updateQueryProgress();
...
void DefaultCoordinator::tryToTakeFromDistributionQueue(size_t replica_num, size_t min_number_of_marks, size_t &current_marks_amount,
                                                        RangesInDataPartsDescription &description) {
  ...
  auto replica_can_read_part = [&](auto replica, const auto &part) {
    return part_visibility[part.getPartOrProjectionName()].contains(replica);
  };
  ...
  if (!result.ranges.empty()) description.push_back(result);
}

void DefaultCoordinator::tryToStealFromQueues(size_t replica_num, ScanMode scan_mode, size_t min_number_of_marks,
                                              size_t &current_marks_amount, RangesInDataPartsDescription &description) {
  auto steal_from_other_replicas = [&]() {
    ...
    std::ranges::sort(order,
                      [&](auto lhs, auto rhs) { return distribution_by_hash_queue[lhs].size() > distribution_by_hash_queue[rhs].size(); });

    for (auto replica : order)
      tryToStealFromQueue(distribution_by_hash_queue[replica], replica, replica_num, scan_mode, min_number_of_marks, current_marks_amount,
                          description);
  };
  ...
  if (scan_mode == ScanMode::TakeWhatsMineForStealing) {
    ...
    tryToStealFromQueue(ranges_for_stealing_queue, /*owner=*/-1, replica_num, scan_mode, min_number_of_marks, current_marks_amount,
                        description);
    ...
    if (replica_num == source_replica_for_parts_snapshot) steal_from_other_replicas();
  }
}

void DefaultCoordinator::tryToStealFromQueue(auto &queue, ssize_t owner, size_t replica_num, ScanMode scan_mode, size_t min_number_of_marks,
                                             size_t &current_marks_amount, RangesInDataPartsDescription &description) {
  auto replica_can_read_part = [&](auto replica, const auto &part) {
    return part_visibility[part.getPartOrProjectionName()].contains(replica);
  };
...
void DefaultCoordinator::processPartsFurther(size_t replica_num, ScanMode scan_mode, size_t min_number_of_marks,
                                             size_t &current_marks_amount, RangesInDataPartsDescription &description) {
  ...
  for (const auto &part : all_parts_to_read) {
    ...
    while (!part_ranges.empty() && current_marks_amount < min_number_of_marks) {
      ...
      for (size_t segment_begin = roundDownToMultiple(range.begin, mark_segment_size);
           segment_begin < range.end && current_marks_amount < min_number_of_marks; segment_begin += mark_segment_size) {
        const auto cur_segment = MarkRange{std::max(range.begin, segment_begin), std::min(range.end, segment_begin + mark_segment_size)};
    ...
    if (!result.ranges.empty()) description.push_back(std::move(result));
  }
}

void DefaultCoordinator::selectPartsAndRanges(size_t replica_num, ScanMode scan_mode, size_t min_number_of_marks,
                                              size_t &current_marks_amount, RangesInDataPartsDescription &description) {
...
void DefaultCoordinator::enqueueToStealerOrStealingQueue(const RangesInDataPartDescription &description, const MarkRange &segment) {
  auto &&range = RangesInDataPartDescription{.info = description.info, .ranges = {segment}, .projection_name = description.projection_name};
  const auto stealer_by_hash = computeConsistentHash(
      description.getPartOrProjectionName(), roundDownToMultiple(segment.begin, mark_segment_size), ScanMode::TakeWhatsMineForStealing);
...
ParallelReadResponse DefaultCoordinator::handleRequest(ParallelReadRequest request) {
  LOG_TRACE(log, "Handling request from replica {}, minimal marks size is {}, request count {}", request.replica_num,
            request.min_number_of_marks, stats[request.replica_num].number_of_requests);

  ParallelReadResponse response;
  size_t current_mark_size = 0;

  /// 1. Try to select ranges meant for this replica by consistent hash
  selectPartsAndRanges(request.replica_num, ScanMode::TakeWhatsMineByHash, request.min_number_of_marks, current_mark_size,
                       response.description);
  const size_t assigned_to_me = current_mark_size;

  /// 2. Try to steal but with caching again (with different key)
  selectPartsAndRanges(request.replica_num, ScanMode::TakeWhatsMineForStealing, request.min_number_of_marks, current_mark_size,
                       response.description);
  ...
  if (current_mark_size == 0)
    selectPartsAndRanges(request.replica_num, ScanMode::TakeEverythingAvailable, request.min_number_of_marks, current_mark_size,
                         response.description);
    ...
    if (++finished_replicas == replicas_count - unavailable_replicas_count) {
      ...
      for (const auto &part : all_parts_to_read)
        if (!part.description.ranges.empty())
          throw Exception(ErrorCodes::LOGICAL_ERROR, "Some segments were left unread for the part {}", part.description.describe());

      if (!ranges_for_stealing_queue.empty()) throw Exception(ErrorCodes::LOGICAL_ERROR, "Some orphaned segments were left unread");
  ...
  LOG_TRACE(log, "Going to respond to replica {} with {}; mine_marks={}, stolen_by_hash={}, stolen_rest={}", request.replica_num,
            response.describe(), assigned_to_me, stolen_by_hash, stolen_unassigned);
...
bool DefaultCoordinator::isReadingCompleted() const {
  for (const auto &part : all_parts_to_read) {
    auto &part_ranges = part.description.ranges;
    if (!part_ranges.empty()) return false;
  }

  for (const auto &r : distribution_by_hash_queue)
    if (!r.empty()) return false;

  if (!ranges_for_stealing_queue.empty()) return false;
...
class InOrderCoordinator : public ParallelReplicasReadingCoordinator::ImplInterface {
 public:
  ...
  ~InOrderCoordinator() override { LOG_TRACE(log, "Coordination done: {}", toString(stats)); }
...
void InOrderCoordinator::doHandleInitialAllRangesAnnouncement(InitialAllRangesAnnouncement announcement) {
  ...
  for (auto &&part : announcement.description) {
    ...
    if (covering_it != all_parts_to_read.end()) {
      ...
      if (is_covered_or_covering(*covering_it)) continue;
  ...
  {
    auto intersecting_part_it =
        std::adjacent_find(all_parts_to_read.begin(), all_parts_to_read.end(),
                           [](const Part &lhs, const Part &rhs) { return !lhs.description.info.isDisjoint(rhs.description.info); });

    if (intersecting_part_it != all_parts_to_read.end())
      throw Exception(ErrorCodes::LOGICAL_ERROR, "Parts {} and {} intersect", intersecting_part_it->description.info.getPartNameV1(),
                      std::next(intersecting_part_it)->description.info.getPartNameV1());
  }
#endif
...
ParallelReadResponse InOrderCoordinator::handleRequest(ParallelReadRequest request) {
  ...
  for (auto &part : response.description) {
    auto global_part_it = std::find_if(all_parts_to_read.begin(), all_parts_to_read.end(), [&part](const Part &other) {
      return other.description.info == part.info && other.description.projection_name == part.projection_name;
    });

    if (global_part_it == all_parts_to_read.end()) continue;

    if (global_part_it->replicas.empty())
      throw Exception(ErrorCodes::LOGICAL_ERROR, "Part {} requested by replica {} is not registered in working set",
                      part.info.getPartNameV1(), request.replica_num);

    if (!global_part_it->replicas.contains(request.replica_num)) continue;
  ...
  if (!overall_number_of_marks) response.finish = true;
...
void ParallelReplicasReadingCoordinator::handleInitialAllRangesAnnouncement(InitialAllRangesAnnouncement announcement) {
  ...
  fiu_do_on(FailPoints::parallel_replicas_check_read_mode_always, {
    if (pimpl && announcement.mode != pimpl->getCoordinationMode()) {
      throw Exception(ErrorCodes::LOGICAL_ERROR, "Replica {} decided to read in {} mode, not in {}. This is a bug",
                      announcement.replica_num, magic_enum::enum_name(announcement.mode),
                      magic_enum::enum_name(pimpl->getCoordinationMode()));
    }
  });

  if (is_reading_completed) return;

  std::lock_guard lock(mutex);

  if (!pimpl) initialize(announcement.mode);
  ...
  if (announcement.mode != pimpl->getCoordinationMode())
    throw Exception(ErrorCodes::LOGICAL_ERROR, "Replica {} decided to read in {} mode, not in {}. This is a bug", announcement.replica_num,
                    magic_enum::enum_name(announcement.mode), magic_enum::enum_name(pimpl->getCoordinationMode()));
...
ParallelReadResponse ParallelReplicasReadingCoordinator::handleRequest(ParallelReadRequest request) {
  if (request.min_number_of_marks == 0)
    throw Exception(ErrorCodes::BAD_ARGUMENTS, "Chosen number of marks to read is zero (likely because of weird interference of settings)");
  ...
  if (is_reading_completed) return response;
  ...
  bool reading_assignment_has_been_completed = false;
  {
    ...
    if (request.mode != pimpl->getCoordinationMode())
      throw Exception(ErrorCodes::LOGICAL_ERROR, "Replica {} decided to read in {} mode, not in {}. This is a bug", request.replica_num,
                      magic_enum::enum_name(request.mode), magic_enum::enum_name(pimpl->getCoordinationMode()));

    const auto replica_num = request.replica_num;

    if (pimpl->replica_status[replica_num].is_finished)
      throw Exception(ErrorCodes::LOGICAL_ERROR, "Got request from replica {} after ranges assignment has been completed for the replica",
                      request.replica_num);
    ...
    if (!response.finish) {
      ...
      if (replicas_used.insert(replica_num).second) ProfileEvents::increment(ProfileEvents::ParallelReplicasUsedCount);
    } else {
  ...
  if (reading_assignment_has_been_completed && read_completed_callback.has_value()) {
    if (replicas_count > replicas_to_exclude.size()) {
      ...
      if (!replicas_to_exclude.empty()) replicas = fmt::format("{}", fmt::join(replicas_to_exclude, ", "));

      LOG_DEBUG(getLogger("ParallelReplicasReadingCoordinator"),
                "All ranges for reading has been assigned to replicas. Cancelling execution for unused replicas. Used replicas: {}",
                replicas);

      if (pimpl && !pimpl->initializedWithEmptyRanges()) chassert(!replicas_used.empty());
...
void ParallelReplicasReadingCoordinator::initialize(CoordinationMode mode) {
  ...
  // progress_callback is not set when local plan is used for initiator
  if (progress_callback) pimpl->setProgressCallback(std::move(progress_callback));

  for (const auto replica : unavailable_nodes_registered_before_initialization) pimpl->markReplicaAsUnavailable(replica);
}
...
ParallelReplicasReadingCoordinator::ParallelReplicasReadingCoordinator(size_t replicas_count_) : replicas_count(replicas_count_) {
  LOG_DEBUG(getLogger("ParallelReplicasReadingCoordinator"), "Creating parallel replicas coordinator with replicas_count={}",
            replicas_count);
}
...
void ParallelReplicasReadingCoordinator::setProgressCallback(ProgressCallback callback) {
  ...
  if (pimpl) pimpl->setProgressCallback(std::move(progress_callback));
}
...
bool ParallelReplicasReadingCoordinator::isReadingCompleted() const {
  if (pimpl) return pimpl->isReadingCompleted();
