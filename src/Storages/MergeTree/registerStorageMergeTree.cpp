...
static Names extractColumnNames(const ASTPtr& node) {
  ...
  if (expr_func && expr_func->name == "tuple") {
    ...
    for (const auto& elem : elements) res.push_back(getIdentifierName(elem));
...
static ColumnsDescription getColumnsDescriptionFromZookeeper(const TableZnodeInfo& zookeeper_info, ContextMutablePtr context) {
  ...
  try {
    ...
    throw Exception{ErrorCodes::CANNOT_EXTRACT_TABLE_STRUCTURE,
                    "Cannot get replica structure from zookeeper, because cannot get zookeeper: {}. You must specify structure manually",
                    getCurrentExceptionMessage(false)};
  }

  if (!zookeeper->exists(zookeeper_info.path + "/replicas"))
    throw Exception{ErrorCodes::CANNOT_EXTRACT_TABLE_STRUCTURE,
                    "Cannot get replica structure, because there no other replicas in zookeeper. You must specify the structure manually"};
...
static bool isExtendedStorageDef(const ASTCreateQuery& query) {
  if (query.storage && query.storage->isExtendedStorageDefinition()) return true;
...
static void evaluateEngineArgs(ASTs& engine_args, const ContextPtr& context) {
  ...
  try {
    for (; arg_idx < engine_args.size(); ++arg_idx) {
      ...
      if (arg_func->name == "array" || arg_func->name == "tuple") continue;
      Field value = evaluateConstantExpression(arg, context).first;
      arg = make_intrusive<ASTLiteral>(value);
    }
  } catch (Exception& e) {
    throw Exception(ErrorCodes::BAD_ARGUMENTS, "Cannot evaluate engine argument {}: {} {}", arg_idx, e.message(), verbose_help_message);
  }
}

/// Returns whether this is a Replicated table engine?
static bool isReplicated(const String& engine_name) { return engine_name.starts_with("Replicated") && engine_name.ends_with("MergeTree"); }
...
static std::string_view getNamePart(const String& engine_name) {
  ...
  if (name_part.starts_with("Replicated")) name_part.remove_prefix(strlen("Replicated"));

  if (name_part.ends_with("MergeTree")) name_part.remove_suffix(strlen("MergeTree"));

  return name_part;
}

/// Extracts zookeeper path and replica name from the table engine's arguments.
/// The function can modify those arguments (that's why they're passed separately in `engine_args`) and also determines
/// RenamingRestrictions. The function assumes the table engine is Replicated.
static TableZnodeInfo extractZooKeeperPathAndReplicaNameFromEngineArgs(const ASTCreateQuery& query, const StorageID& table_id,
                                                                       const String& engine_name, ASTs& engine_args,
                                                                       LoadingStrictnessLevel mode, const ContextPtr& local_context) {
  ...
  auto expand_macro = [&](ASTLiteral* ast_zk_path, ASTLiteral* ast_replica_name, String zookeeper_path,
                          String replica_name) -> TableZnodeInfo {
  ...
  if (has_valid_arguments) {
    ...
    if (!query.attach && is_replicated_database &&
      ...
      LOG_WARNING(&Poco::Logger::get("registerStorageMergeTree"),
    ...
    if (!query.attach && is_replicated_database &&
      ...
      LOG_WARNING(&Poco::Logger::get("registerStorageMergeTree"),
                  ...
                  ast_zk_path->value.safeGet<String>(), ast_replica_name->value.safeGet<String>());
  ...
  if (is_extended_storage_def &&
      (arg_cnt == 0 || !engine_args[arg_num]->as<ASTLiteral>() || (arg_cnt == 1 && (getNamePart(engine_name) == "Graphite")))) {
    ...
    auto res = expand_macro(ast_zk_path, ast_replica_name, server_settings[ServerSetting::default_replica_path],
...
std::optional<String> extractZooKeeperPathFromReplicatedTableDef(const ASTCreateQuery& query, const ContextPtr& local_context) {
  if (!query.storage || !query.storage->engine) return {};

  const String& engine_name = query.storage->engine->name;
  if (!isReplicated(engine_name)) return {};

  StorageID table_id{query.getDatabase(), query.getTable(), query.uuid};

  ASTs engine_args;
  if (query.storage->engine->arguments) engine_args = query.storage->engine->arguments->children;
  for (auto& engine_arg : engine_args) engine_arg = engine_arg->clone();

  try {
    auto res = extractZooKeeperPathAndReplicaNameFromEngineArgs(query, table_id, engine_name, engine_args, LoadingStrictnessLevel::CREATE,
                                                                local_context);
...
static StoragePtr create(const StorageFactory::Arguments& args) {
   ...
   * For example: ENGINE = ReplicatedCollapsingMergeTree('/tables/mytable', 'rep02', EventDate, (CounterID, EventDate, intHash32(UniqID),
   * VisitID), 8192, Sign).
   *  - (for Summing, optional) a tuple of columns to be summed. If not specified, all numeric columns that are not included in the primary
   * key are used.
  ...
  if (name_part == "Collapsing")
    ...
    throw Exception(ErrorCodes::UNKNOWN_STORAGE, "Unknown storage {}", args.engine_name + verbose_help_message);
  ...
  if (replicated) {
    zookeeper_info = extractZooKeeperPathAndReplicaNameFromEngineArgs(args.query, args.table_id, args.engine_name, args.engine_args,
                                                                      args.mode, args.getLocalContext());
  ...
  if (merging_params.mode == MergeTreeData::MergingParams::Collapsing) {
    ...
    constexpr auto format_str =
        "Last parameter of GraphiteMergeTree must be the name (in single quotes) of the element "
    ...
    if (const auto* ast = engine_args[arg_cnt - 1]->as<ASTLiteral>()) {
      if (ast->value.getType() != Field::Types::String) throw Exception(ErrorCodes::BAD_ARGUMENTS, format_str, error_msg);
  ...
  if (is_extended_storage_def) {
    ...
    if (args.storage_def->partition_by) partition_by_key = args.storage_def->partition_by->ptr();
    ...
    metadata.sorting_key =
        KeyDescription::getSortingKeyFromAST(args.storage_def->order_by->ptr(), metadata.columns, context, merging_param_key_arg);
    ...
    if (args.storage_def->ttl_table) {
      metadata.table_ttl = TTLTableDescription::getTTLForTableFromAST(args.storage_def->ttl_table->ptr(), metadata.columns, context,
                                                                      metadata.primary_key, allow_suspicious_ttl);
    }
    ...
    if (args.query.columns_list && args.query.columns_list->projections) {
      for (auto& projection_ast : args.query.columns_list->projections->children) {
        try {
          ...
          if (args.mode < LoadingStrictnessLevel::FORCE_ATTACH) throw;
          tryLogCurrentException(__PRETTY_FUNCTION__,
                                 fmt::format("Cannot parse projection {} during server startup, skipping it. "
    ...
    if (args.query.columns_list && args.query.columns_list->constraints)
      for (auto& constraint : args.query.columns_list->constraints->children) constraints.push_back(constraint);
    ...
    /// before storage creation. After that storage will just copy this
    /// column if sorting key will be changed.
    metadata.sorting_key = KeyDescription::getSortingKeyFromAST(engine_args[arg_num], metadata.columns, context, merging_param_key_arg);
  ...
  if (args.mode <= LoadingStrictnessLevel::CREATE && !(*storage_settings)[MergeTreeSetting::allow_floating_point_partition_key]) {
    for (size_t i = 0; i < data_types.size(); ++i)
      if (isFloat(data_types[i]))
        throw Exception(ErrorCodes::BAD_ARGUMENTS, "Floating point partition key is not supported: {}",
  ...
  if (arg_num != arg_cnt) throw Exception(ErrorCodes::BAD_ARGUMENTS, "Wrong number of engine arguments.");

  if (replicated) {
    bool need_check_table_structure = true;
    if (auto txn = args.getLocalContext()->getZooKeeperMetadataTransaction()) need_check_table_structure = txn->isInitialQuery();

    ZooKeeperRetriesInfo create_query_zk_retries_info{
        local_settings[Setting::keeper_max_retries], local_settings[Setting::keeper_retry_initial_backoff_ms],
        local_settings[Setting::keeper_retry_max_backoff_ms], args.getLocalContext()->getProcessListElementSafe()};

    return std::make_shared<StorageReplicatedMergeTree>(zookeeper_info, args.mode, args.table_id, args.relative_data_path, metadata,
                                                        context, date_column_name, merging_params, std::move(storage_settings),
                                                        need_check_table_structure, create_query_zk_retries_info);
  }

  return std::make_shared<StorageMergeTree>(args.table_id, args.relative_data_path, metadata, args.mode, context, date_column_name,
                                            merging_params, std::move(storage_settings));
}
