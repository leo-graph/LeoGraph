...
class ReadStepsPerformanceCounters final {
 public:
  ReadStepPerformanceCountersPtr getCountersForStep(size_t step) const {
    ...
    if (step >= performance_counters.size()) performance_counters.resize(step + 1);
    if (!performance_counters[step]) performance_counters[step] = std::make_shared<ReadStepPerformanceCounters>();
    return performance_counters[step];
  }

  ReadStepPerformanceCountersPtr getCounterForIndexStep() const {
    std::lock_guard lock{mutex};
    if (!index_performance_counter) index_performance_counter = std::make_shared<ReadStepPerformanceCounters>();
...
class FilterWithCachedCount {
 ...
 public:
  ...
  explicit FilterWithCachedCount(const ColumnPtr& column_) : const_description(*column_) {
  ...
  size_t countBytesInFilter() const {
    if (cached_count_bytes == size_t(-1)) cached_count_bytes = DB::countBytesInFilter(*data);
...
class MergeTreeRangeReader {
 public:
  MergeTreeRangeReader(IMergeTreeReader* merge_tree_reader_, Block prev_reader_header_, const PrewhereExprStep* prewhere_info_,
                       ReadStepPerformanceCountersPtr performance_counters_, bool main_reader_);
 ...
 private:
  ...
  Block
      read_sample_block;  /// Block with columns that are actually read from disk + non-const virtual columns that are filled at this step.
