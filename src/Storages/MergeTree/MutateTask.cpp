...
static bool haveMutationsOfDynamicColumns(const MergeTreeData::DataPartPtr &data_part, const MutationCommands &commands) {
  for (const auto &command : commands) {
    if (!command.column_name.empty()) {
      ...
      if (column && column->type->hasDynamicSubcolumns()) return true;
    }
...
static UInt64 getExistingRowsCount(const Block &block) {
  ...
  for (UInt8 row_exists : row_exists_col->getData())
    if (row_exists) existing_count++;

  return existing_count;
}

static NameSet getRemovedStatistics(const StorageMetadataPtr &metadata_snapshot, const MutationCommand &command) {
  if (command.type != MutationCommand::Type::DROP_STATISTICS) return {};

  NameSet removed_stats;

  if (command.clear && command.statistics_columns.empty()) {
    for (const auto &column_desc : metadata_snapshot->getColumns()) removed_stats.insert(column_desc.name);
  } else {
    for (const auto &column_name : command.statistics_columns) removed_stats.insert(column_name);
  }
 ...
 */
static void splitAndModifyMutationCommands(MergeTreeData::DataPartPtr part, StorageMetadataPtr metadata_snapshot,
                                           AlterConversionsPtr alter_conversions, const MutationCommands &commands,
                                           MutationCommands &for_interpreter, MutationCommands &for_file_renames,
                                           bool suitable_for_ttl_optimization, LoggerPtr log) {
  ...
  if (haveMutationsOfDynamicColumns(part, commands) || !isWidePart(part) || !isFullPartStorage(part->getDataPartStorage())) {
    ...
    for (const auto &command : commands) {
      if (command.type == MutationCommand::Type::MATERIALIZE_COLUMN) {
        ...
        /// So we only mutate column if `command.column_name` is a default/materialized column or if the part does not have physical column
        /// file
        ...
        if (part_columns.has(command.column_name)) for_file_renames.push_back(command);
        ...
        if (has_column || command.read_for_patch) {
          ...
          mutated_columns.insert(command.column_name);
                 ...
                 command.type == MutationCommand::Type::MATERIALIZE_PROJECTION || command.type == MutationCommand::Type::MATERIALIZE_TTL ||
                 command.type == MutationCommand::Type::REWRITE_PARTS || command.type == MutationCommand::Type::DELETE ||
                 command.type == MutationCommand::Type::UPDATE || command.type == MutationCommand::Type::APPLY_DELETED_MASK ||
                 command.type == MutationCommand::Type::APPLY_PATCHES) {
        for_interpreter.push_back(command);
        for (const auto &[column_name, expr] : command.column_to_update_expression) mutated_columns.emplace(column_name);

        if (command.type == MutationCommand::Type::MATERIALIZE_TTL && suitable_for_ttl_optimization) {
          for (const auto &col : part_columns) {
            if (!mutated_columns.contains(col.name)) ignored_columns.emplace(col.name);
          }
        }
          ...
          for (const auto &projection : all_projections) {
            if (projection.name == command.projection_name) {
              for (const auto &column : projection.required_columns) {
                if (projection.with_parent_part_offset && column == "_part_offset") continue;
      ...
      } else if (bool has_column = part_columns.has(command.column_name), has_nested_column = part_columns.hasNested(command.column_name);
        ...
        if (command.type == MutationCommand::Type::DROP_COLUMN || command.type == MutationCommand::Type::RENAME_COLUMN) {
          if (has_nested_column) {
            ...
            assert(!nested.empty());
            for (const auto &nested_column : nested) mutated_columns.emplace(nested_column.name);
          } else
            mutated_columns.emplace(command.column_name);
    ...
    for (const auto &column : part_columns) {
      if (!mutated_columns.contains(column.name)) {
        if (!metadata_snapshot->getColumns().has(column.name) && !part->storage.getVirtualsPtr()->has(column.name) &&
            !ignored_columns.contains(column.name)) {
          ...
          if (part_metadata_version < table_metadata_version || allow_equal_versions) {
            LOG_WARNING(log,
                        "Ignoring column {} from part {} with metadata version {} because there is no such column "
                        "in table {} with metadata version {}. Assuming the column was dropped",
                        column.name, part->name, part_metadata_version, part->storage.getStorageID().getNameForLogs(),
          ...
          if (part->storage.supportsReplication())
            throw Exception(ErrorCodes::LOGICAL_ERROR,
                            "Part {} with metadata version {} contains column {} that is absent "
                            "in table {} with metadata version {}",
                            part->name, part_metadata_version, column.name, part->storage.getStorageID().getNameForLogs(),
                            table_metadata_version);
        }
    ...
    for (const auto &column_name : extra_columns_for_indices_and_projections) {
      if (mutated_columns.contains(column_name)) continue;

      if (column_name == "_part_offset") continue;

      auto data_type = metadata_snapshot->getColumns().getColumn(GetColumnsOptions::AllPhysical, column_name).type;

      for_interpreter.push_back(MutationCommand{
          .type = MutationCommand::Type::READ_COLUMN,
    ...
    for (const auto &command : commands) {
      if (command.type == MutationCommand::Type::MATERIALIZE_COLUMN) {
        ...
        if (part_columns.has(command.column_name)) for_file_renames.push_back(command);
                 ...
                 command.type == MutationCommand::Type::MATERIALIZE_PROJECTION || command.type == MutationCommand::Type::MATERIALIZE_TTL ||
                 command.type == MutationCommand::Type::REWRITE_PARTS || command.type == MutationCommand::Type::DELETE ||
                 command.type == MutationCommand::Type::APPLY_DELETED_MASK || command.type == MutationCommand::Type::APPLY_PATCHES) {
        for_interpreter.push_back(command);
      ...
      /// If we don't have this column in source part, we don't need to materialize it.
        ...
        if (command.type == MutationCommand::Type::RENAME_COLUMN) part_columns.rename(command.column_name, command.rename_to);
...
static void addRenamedColumnToColumnsSubstreams(ColumnsSubstreams &new_columns_substreams, const ColumnsSubstreams &old_columns_substreams,
                                                const String &new_name, const String &old_name, size_t old_position) {
...
static bool isDeletedMaskUpdated(const MutationCommand &command, const NameSet &storage_columns_set) {
  if (storage_columns_set.contains(RowExistsColumn::name)) return false;

  if (command.type == MutationCommand::READ_COLUMN) return command.read_for_patch && command.column_name == RowExistsColumn::name;
...
static std::tuple<NamesAndTypesList, SerializationInfoByName, ColumnsSubstreams> getColumnsForNewDataPart(
  ...
  for (const auto &[name, _] : storage_columns) storage_columns_set.insert(name);

  for (const auto &command : all_commands) {
    affects_all_columns |= command.affectsAllColumns();

    if (supports_lightweight_deletes) deleted_mask_updated |= isDeletedMaskUpdated(command, storage_columns_set);
    ...
    if (command.type == MutationCommand::DROP_COLUMN) removed_columns.insert(command.column_name);
  ...
  for (const auto &[name, type] : persistent_virtuals) {
    if (storage_columns_set.contains(name)) continue;
  ...
  for (const auto &[name, old_info] : serialization_infos) {
    ...
    if (!updated_header.has(new_name)) {
      if (isWidePart(source_part)) new_serialization_infos.emplace(new_name, old_info);
      continue;
    }
    ...
    if (settings.isAlwaysDefault() || !settings.canUseSparseSerialization(*new_type)) continue;
  ...
  for (const auto &it : source_columns) source_columns_name_to_type[it.name] = it.type;
  ...
  for (auto it = storage_columns.begin(); it != storage_columns.end();) {
    if (updated_header.has(it->name)) {
      ...
      if (updated_type != it->type) it->type = updated_type;
      ...
      if (source_col == source_columns_name_to_type.end()) {
        ...
        if (renamed_it != renamed_columns_to_from.end()) {
          ...
          if (source_col == source_columns_name_to_type.end())
            ...
            if (fill_columns_substreams)
              addRenamedColumnToColumnsSubstreams(new_columns_substreams, source_columns_substreams, it->name, source_col->first,
                                                  *source_part->getColumnPosition(source_col->first));
        ...
        if (!renamed_columns_to_from.contains(it->name) && (was_renamed || was_removed)) {
          ...
          if (was_removed) {  /// DROP COLUMN xxx, RENAME COLUMN yyy TO xxx
            ...
            if (fill_columns_substreams)
              addRenamedColumnToColumnsSubstreams(new_columns_substreams, source_columns_substreams, it->name, renamed_from,
                                                  *source_part->getColumnPosition(renamed_from));
          } else {
            /// Take a type from source part column.
            /// It may differ from column type in storage.
            it->type = source_col->second;
            if (fill_columns_substreams) {
              ...
              new_columns_substreams.addSubstreamsToLastColumn(
                  source_columns_substreams.getColumnSubstreams(*source_part->getColumnPosition(it->name)));
            }
          }
          ++it;
        }
...
static ExecuteTTLType shouldExecuteTTL(const StorageMetadataPtr &metadata_snapshot, const ColumnDependencies &dependencies) {
  if (!metadata_snapshot->hasAnyTTL()) return ExecuteTTLType::NONE;

  bool has_ttl_expression = false;

  for (const auto &dependency : dependencies) {
    if (dependency.kind == ColumnDependency::TTL_EXPRESSION) has_ttl_expression = true;

    if (dependency.kind == ColumnDependency::TTL_TARGET) return ExecuteTTLType::NORMAL;
  }
...
static std::set<ProjectionDescriptionRawPtr> getProjectionsToRecalculate(const MergeTreeDataPartPtr &source_part,
  ...
  for (const auto &projection : metadata_snapshot->getProjections()) {
    bool need_recalculate =
        ...
        (!is_full_part_storage && source_part->hasProjection(projection.name) && !source_part->hasBrokenProjection(projection.name));

    if (need_recalculate) projections_to_recalc.insert(&projection);
  }

  return projections_to_recalc;
}

static std::unordered_map<String, size_t> getStreamCounts(const MergeTreeDataPartPtr &data_part,
...
static NameSet collectFilesToSkip(const MergeTreeDataPartPtr &source_part, const MergeTreeDataPartPtr &new_part,
                                  const Block &updated_header, const std::set<MergeTreeIndexPtr> &indices_to_recalc,
                                  const std::set<MergeTreeIndexPtr> &indices_to_drop, const String &mrk_extension,
  ...
  for (const auto &index : indices_to_recalc) skip_index(index);
  for (const auto &index : indices_to_drop) skip_index(index);

  for (const auto &projection : projections_to_skip) files_to_skip.insert(projection->getDirectoryName());
...
static NameToNameVector collectFilesForRenames(StorageMetadataPtr metadata_snapshot, MergeTreeData::DataPartPtr source_part,
                                               MergeTreeData::DataPartPtr new_part, const MutationCommands &commands_for_renames,
                                               const NameSet &updated_columns_in_patches, const String &mrk_extension) {
  ...
  auto add_rename = [&rename_vector, &collected_names](const std::string &file_rename_from, const std::string &file_rename_to) {
    if (collected_names.emplace(file_rename_from).second) rename_vector.emplace_back(file_rename_from, file_rename_to);
  };
      ...
      for (const auto &substream : substreams) {
        for (const auto &extension : extensions) {
          ...
          if (actual_stream_name) {
            ...
            if (actual_mark_name) add_rename(*actual_mark_name + mrk_extension, "");
          }
        }
      }
    } else if (command.type == MutationCommand::Type::DROP_PROJECTION) {
      if (source_part->checksums.has(command.column_name + ".proj")) add_rename(command.column_name + ".proj", "");
      ...
      if (command.type == MutationCommand::Type::DROP_COLUMN) {
        ISerialization::StreamCallback callback = [&](const ISerialization::SubstreamPath &substream_path) {
          auto stream_name = IMergeTreeDataPart::getStreamNameForColumn(command.column_name, substream_path, ".bin", source_part->checksums,
                                                                        source_part->storage.getSettings());
        ...
        if (auto serialization = source_part->tryGetSerialization(command.column_name)) serialization->enumerateStreams(callback);
      } else if (command.type == MutationCommand::Type::RENAME_COLUMN) {
        /// Columns updated in patches should be rewritten by mutation.
        if (updated_columns_in_patches.contains(command.rename_to)) continue;
...
static void processStatisticsChanges(NameSet &files_to_skip, NameToNameVector &files_to_rename, ColumnsStatistics &all_statistics,
                                     const ColumnsStatistics &stats_to_recalc, const MutationCommands &commands_for_renames,
                                     const IMergeTreeDataPart &source_part, StorageMetadataPtr metadata_snapshot) {
  auto storage_settings = source_part.storage.getSettings();
  ...
  auto process_rename = [&](const String &from_name, const String &to_name) {
    ...
    if (it == all_statistics.end()) return;

    if (!to_name.empty()) all_statistics.emplace(to_name, it->second);
  ...
  for (const auto &command : commands_for_renames) {
    if (command.type == MutationCommand::Type::DROP_STATISTICS) {
      ...
      for (const auto &stats_name : removed_stats) process_rename(stats_name, "");
      ...
      if (!column_desc || column_desc->statistics.empty()) process_rename(command.column_name, "");
    }
  }

  if (!stats_to_recalc.empty()) {
    /// Create empty statistics for columns that are being recalculated.
    for (const auto &[stat_name, stat] : stats_to_recalc) all_statistics[stat_name] = stat->cloneEmpty();
  }
  ...
  if (isFullPartStorage(source_part.getDataPartStorage())) {
    ...
    for (const auto &[filename, _] : source_checksums.files) {
      if (filename.starts_with(STATS_FILE_PREFIX) && filename.ends_with(STATS_FILE_SUFFIX)) files_to_rename.emplace_back(filename, "");
    }
  }
}

/// Initialize and write to disk new part fields like checksums, columns, etc.
void finalizeMutatedPart(const MergeTreeDataPartPtr &source_part, MergeTreeData::MutableDataPartPtr new_data_part,
                         const IMergedBlockOutputStream::GatheredData &all_gathered_data, ExecuteTTLType execute_ttl_type,
                         const CompressionCodecPtr &codec, ContextPtr context, StorageMetadataPtr metadata_snapshot, bool sync) {
  ...
  if (serialization_infos.needsPersistence()) {
    auto out_serialization =
        new_data_part->getDataPartStorage().writeFile(IMergeTreeDataPart::SERIALIZATION_FILE_NAME, 4096, context->getWriteSettings());
  ...
  if (!statistics.empty()) {
    if (isFullPartStorage(new_data_part->getDataPartStorage())) {
      auto out = serializeStatisticsPacked(new_data_part->getDataPartStorage(), new_data_part->checksums, statistics, codec,
                                           context->getWriteSettings());
      written_files.push_back(std::move(out));
    }
    /// Write statistics as separate compressed files in packed parts to avoid double buffering.
    else {
      auto files = serializeStatisticsWide(new_data_part->getDataPartStorage(), new_data_part->checksums, statistics, codec,
                                           context->getWriteSettings());
  ...
  {
    auto out_comp = new_data_part->getDataPartStorage().writeFile(IMergeTreeDataPart::DEFAULT_COMPRESSION_CODEC_FILE_NAME, 4096,
                                                                  context->getWriteSettings());
  ...
  {
    auto out_metadata =
        new_data_part->getDataPartStorage().writeFile(IMergeTreeDataPart::METADATA_VERSION_FILE_NAME, 4096, context->getWriteSettings());
  ...
  if (!new_data_part->getColumnsSubstreams().empty()) {
    ...
    auto out_columns_substreams =
        new_data_part->getDataPartStorage().writeFile(IMergeTreeDataPart::COLUMNS_SUBSTREAMS_FILE_NAME, 4096, context->getWriteSettings());
  ...
  if (!new_data_part->storage.getPrimaryIndexCache()) new_data_part->setIndex(*source_part->getIndex());
...
struct MutationContext {
  ...
  bool checkOperationIsNotCanceled() const {
    if (new_data_part ? merges_blocker->isCancelledForPartition(new_data_part->info.getPartitionId())
                      : merges_blocker->isCancelled() || (*mutate_entry)->is_cancelled) {
...
class PartMergerWriter {
  ...
  bool execute() {
    switch (state) {
      ...
      case State::NEED_MUTATE_ORIGINAL_PART: {
        if (mutateOriginalPartAndPrepareProjections()) return true;

        state = State::NEED_EXECUTE_MERGE_SUBTASKS;
        return true;
      }
      case State::NEED_EXECUTE_MERGE_SUBTASKS: {
        if (iterateThroughAllMergeSubtasks()) return true;
 ...
 private:
  ...
  enum class State : uint8_t { NEED_PREPARE, NEED_MUTATE_ORIGINAL_PART, NEED_EXECUTE_MERGE_SUBTASKS, SUCCESS };
...
void PartMergerWriter::prepare() {
  ...
  for (size_t i = 0, size = ctx->projections_to_build.size(); i < size; ++i) {
    ...
    projection_squashes.emplace_back(std::make_shared<const Block>(ctx->updated_header), settings[Setting::min_insert_block_size_rows],
...
bool PartMergerWriter::mutateOriginalPartAndPrepareProjections() {
  ...
  do {
    ...
    if (!ctx->all_gathered_data.statistics.empty()) ctx->all_gathered_data.statistics.buildIfExists(cur_block);

    /// TODO: move this calculation to DELETE FROM mutation
    if (ctx->count_lightweight_deleted_rows) existing_rows_count += MutationHelpers::getExistingRowsCount(cur_block);
    ...
    for (size_t i = 0, size = ctx->projections_to_build.size(); i < size; ++i) {
...

      {
        ...
        if (block_to_squash.rows() == 0) continue;
      ...
      if (squashed_chunk) writeTempProjectionPart(i, std::move(squashed_chunk));
    }

    if (build_text_index_transform) build_text_index_transform->aggregate(cur_block);
...
void PartMergerWriter::createBuildTextIndexesTask() {
  ...
  build_text_index_transform =
      std::make_unique<BuildTextIndexTransform>(std::make_shared<Block>(ctx->updated_header),
                                                /*index_file_prefix=*/"tmp", std::move(text_indexes), temporary_text_index_storage,
                                                ctx->out->getWriterSettings(), ctx->compression_codec, ctx->mrk_extension);
}
...
void PartMergerWriter::writeTempProjectionPart(size_t projection_idx, Chunk chunk) {
  ...
  auto tmp_part = MergeTreeDataWriter::writeTempProjectionPart(*ctx->data, ctx->log, result, projection, ctx->new_data_part.get(),
...
void PartMergerWriter::finalizeTempProjectionsAndIndexes() {
  ...
  for (size_t i = 0, size = ctx->projections_to_build.size(); i < size; ++i) {
    auto squashed_chunk = Squashing::squash(projection_squashes[i].flush(), projection_squashes[i].getHeader());
    if (squashed_chunk) writeTempProjectionPart(i, std::move(squashed_chunk));
  }
  ...
  for (auto &&[projection_name, temporary_projection_parts] : projection_parts) {
    auto merge_task = std::make_unique<MergeProjectionPartsTask>(
        projection_name, std::move(temporary_projection_parts), projections.get(projection_name), projection_block_num, ctx->context,
        ctx->holder, ctx->mutator, ctx->mutate_entry, ctx->time_of_mutation, ctx->new_data_part, ctx->space_reservation);
  ...
  if (build_text_index_transform) {
    ...
    for (const auto &index : indexes) {
      ...
      auto merge_task =
          std::make_unique<MergeTextIndexesTask>(std::move(segments), ctx->new_data_part, index,
                                                 /*merged_part_offsets=*/nullptr, reader_settings, ctx->out->getWriterSettings());
...
bool PartMergerWriter::iterateThroughAllMergeSubtasks() {
  if (merge_subtasks.empty()) return false;

  auto &task = merge_subtasks.back();
  if (task->executeStep()) return true;
...
void PartMergerWriter::finalize() {
  if (ctx->count_lightweight_deleted_rows) ctx->new_data_part->existing_rows_count = existing_rows_count;

  if (temporary_text_index_storage) temporary_text_index_storage->removeRecursive();
}
  ...
  bool executeStep() override {
    switch (state) {
      ...
      case State::NEED_EXECUTE: {
        if (part_merger_writer_task->execute()) return true;
 ...
 private:
  void prepare() {
    ...
    for (const auto &idx : indices) {
      if (removed_indices.contains(idx.name)) continue;

      if (ctx->indices_to_drop_names.contains(idx.name)) continue;
    ...
    for (const auto &projection : projections) {
      if (removed_projections.contains(projection.name)) continue;

      bool need_recalculate = (ctx->materialized_projections.contains(projection.name) ||
    ...
    for (auto it = ctx->source_part->getDataPartStorage().iterate(); it->isValid(); it->next()) {
      if (!entries_to_hardlink.contains(it->name())) {
        ...
        ctx->new_data_part->getDataPartStorage().createHardLinkFrom(ctx->source_part->getDataPartStorage(), it->name(), it->name());
        ...
        for (auto p_it = projection_data_part_storage_src->iterate(); p_it->isValid(); p_it->next()) {
          projection_data_part_storage_dst->createHardLinkFrom(*projection_data_part_storage_src, p_it->name(), p_it->name());
    ...
    if (ctx->metadata_snapshot->hasPrimaryKey() || ctx->metadata_snapshot->hasSecondaryIndices()) {
      ...
      builder->addTransform(std::make_shared<ExpressionTransform>(builder->getSharedHeader(),
                                                                  std::make_shared<ExpressionActions>(std::move(indices_expression_dag))));
    ...
    if (ctx->execute_ttl_type == ExecuteTTLType::NORMAL) {
      auto transform =
          std::make_shared<TTLTransform>(ctx->context, builder->getSharedHeader(), *ctx->data, ctx->metadata_snapshot, ctx->new_data_part,
                                         NamesAndTypesList{} /*expired_columns*/, ctx->time_of_mutation, true);
      subqueries = transform->getSubqueries();
      builder->addTransform(std::move(transform));
    }

    if (ctx->execute_ttl_type == ExecuteTTLType::RECALCULATE) {
      auto transform = std::make_shared<TTLCalcTransform>(ctx->context, builder->getSharedHeader(), *ctx->data, ctx->metadata_snapshot,
                                                          ctx->new_data_part, ctx->time_of_mutation, true);
      subqueries = transform->getSubqueries();
      builder->addTransform(std::move(transform));
    }

    if (!subqueries.empty()) builder = addCreatingSetsTransform(std::move(builder), std::move(subqueries), ctx->context);
    ...
    if (!affects_all_columns && ctx->execute_ttl_type == ExecuteTTLType::NONE) {
      ...
      index_granularity_ptr = createMergeTreeIndexGranularity(ctx->source_part->rows_count, ctx->source_part->getBytesUncompressedOnDisk(),
                                                              *ctx->data->getSettings(), ctx->new_data_part->index_granularity_info,
    ...
    MutationHelpers::processStatisticsChanges(ctx->files_to_skip, ctx->files_to_rename, ctx->all_gathered_data.statistics,
                                              ctx->stats_to_recalc, ctx->for_file_renames, *ctx->source_part, ctx->metadata_snapshot);

    ctx->out = std::make_shared<MergedBlockOutputStream>(
        ctx->new_data_part, ctx->data->getSettings(), ctx->metadata_snapshot, ctx->new_data_part->getColumns(), skip_indices,
        ctx->compression_codec, std::move(index_granularity_ptr), ctx->txn ? ctx->txn->tid : Tx::PrehistoricTID,
        ctx->source_part->getBytesUncompressedOnDisk(),
        /*reset_columns=*/true,
        /*blocks_are_granules_size=*/false, ctx->context->getWriteSettings(), static_cast<WrittenOffsetSubstreams *>(nullptr));
...
class MutateSomePartColumnsTask : public IExecutableTask {
 public:
  ...
  bool executeStep() override {
    switch (state) {
      ...
      case State::NEED_EXECUTE: {
        if (part_merger_writer_task && part_merger_writer_task->execute()) return true;
 ...
 private:
  void prepare() {
    ...
    if (ctx->execute_ttl_type != ExecuteTTLType::NONE) ctx->files_to_skip.insert("ttl.txt");
    ...
    /// Create hardlinks for unchanged files
    for (auto it = ctx->source_part->getDataPartStorage().iterate(); it->isValid(); it->next()) {
      if (ctx->files_to_skip.contains(it->name())) continue;

      String file_name = it->name();

      auto rename_it = std::find_if(ctx->files_to_rename.begin(), ctx->files_to_rename.end(),
      ...
      if (it->isFile()) {
        if ((*settings)[MergeTreeSetting::always_use_copy_instead_of_hardlinks]) {
          ctx->new_data_part->getDataPartStorage().copyFileFrom(ctx->source_part->getDataPartStorage(), it->name(), destination);
        } else {
          ctx->new_data_part->getDataPartStorage().createHardLinkFrom(ctx->source_part->getDataPartStorage(), it->name(), destination);
      ...
      } else if (!endsWith(it->name(), ".tmp_proj"))  // ignore projection tmp merge dir
      {
        ...
        for (auto p_it = projection_data_part_storage_src->iterate(); p_it->isValid(); p_it->next()) {
          if ((*settings)[MergeTreeSetting::always_use_copy_instead_of_hardlinks]) {
            projection_data_part_storage_dst->copyFileFrom(*projection_data_part_storage_src, p_it->name(), p_it->name());
          } else {
            auto file_name_with_projection_prefix = fs::path(projection_data_part_storage_src->getPartDirectory()) / p_it->name();
    ...
    for (const auto &index : ctx->indices_to_drop) {
      ...
      for (const auto &index_substream : index_substreams) {
        ...
        auto actual_data_stream_name =
            IMergeTreeDataPart::getStreamNameOrHash(stream_name, index_substream.extension, ctx->source_part->checksums);
        if (actual_data_stream_name) ctx->new_data_part->checksums.remove(*actual_data_stream_name + index_substream.extension);

        auto actual_mark_stream_name =
            IMergeTreeDataPart::getStreamNameOrHash(stream_name, ctx->mrk_extension, ctx->source_part->checksums);
        if (actual_mark_stream_name) ctx->new_data_part->checksums.remove(*actual_mark_stream_name + ctx->mrk_extension);
      }
    }
    ...
    if (ctx->mutating_pipeline_builder.initialized()) {
      ...
      ctx->out = std::make_shared<MergedColumnOnlyOutputStream>(
          ctx->new_data_part, ctx->data->getSettings(), ctx->metadata_snapshot, ctx->updated_header.getNamesAndTypesList(),
          std::vector<MergeTreeIndexPtr>(ctx->indices_to_recalc.begin(), ctx->indices_to_recalc.end()), ctx->compression_codec,
          ctx->source_part->index_granularity, ctx->source_part->getBytesUncompressedOnDisk(),
      ...
      ctx->projections_to_build =
          std::vector<ProjectionDescriptionRawPtr>{ctx->projections_to_recalc.begin(), ctx->projections_to_recalc.end()};
  ...
  void finalize() {
    if (ctx->mutating_executor) {
      ...
      if (!new_columns_substreams.empty()) {
        ...
        new_columns_substreams = ColumnsSubstreams::merge(changed_columns_substreams, ctx->new_data_part->getColumnsSubstreams(),
                                                          ctx->new_data_part->getColumns().getNames());
    ...
    MutationHelpers::finalizeMutatedPart(ctx->source_part, ctx->new_data_part, ctx->all_gathered_data, ctx->execute_ttl_type,
                                         ctx->compression_codec, ctx->context, ctx->metadata_snapshot, ctx->need_sync);
  }
...
class ExecutableTaskDropTTLExpiredPartsDecorator : public IExecutableTask {
 public:
  explicit ExecutableTaskDropTTLExpiredPartsDecorator(std::unique_ptr<IExecutableTask> executable_task_, MutationContextPtr ctx_)
  ...
  bool executeStep() override {
    switch (state) {
      case State::NEED_EXECUTE: {
        if (executable_task->executeStep()) return true;

        if (isRowsMaxTTLExpired()) replacePartWithEmpty();
  ...
  void cancel() noexcept override { executable_task->cancel(); }
...
MutateTask::MutateTask(FutureMergedMutatedPartPtr future_part_, StorageMetadataPtr metadata_snapshot_, MutationCommandsConstPtr commands_,
                       MergeListEntry *mutate_entry_, time_t time_of_mutation_, ContextPtr context_,
                       ReservationSharedPtr space_reservation_, TableLockHolder &table_lock_holder_, const MergeTreeTransactionPtr &txn,
                       MergeTreeData &data_, MergeTreeDataMergerMutator &mutator_, PartitionActionBlocker &merges_blocker_,
                       bool need_prefix_)
...
void MutateTask::cancel() noexcept {
  ...
  if (ctx->new_data_part) ctx->new_data_part->removeIfNeeded();
}
...
static bool canSkipConversionToNullable(const MergeTreeDataPartPtr &part, const StorageMetadataPtr &metadata_snapshot,
                                        const MutationCommand &command) {
  if (command.type != MutationCommand::READ_COLUMN) return false;

  auto part_column = part->tryGetColumn(command.column_name);
  if (!part_column) return false;
  ...
  if (!to_nullable) return false;

  if (!part_column->type->equals(*to_nullable->getNestedType())) return false;

  auto serialization = part->getSerialization(command.column_name);
  if (serialization->getKindStack() != ISerialization::KindStack{ISerialization::Kind::DEFAULT}) return false;

  /// We need to rewrite statistics because they have different serialization with nullable type.
  const auto *column_desc = metadata_snapshot->getColumns().tryGet(command.column_name);
  if (!column_desc->statistics.empty()) return false;
...
static bool canSkipMutationCommandForPart(const MergeTreeDataPartPtr &part, const StorageMetadataPtr &metadata_snapshot,
                                          const MutationCommand &command, const ContextPtr &context) {
  if (command.partition) {
    auto command_partition_id = part->storage.getPartitionIDFromQuery(command.partition, context);
    if (part->info.getPartitionId() != command_partition_id) return true;
  }

  /// APPLY PATCHES command is handled separately.
  if (command.type == MutationCommand::APPLY_PATCHES) return true;

  if (command.type == MutationCommand::APPLY_DELETED_MASK && !part->hasLightweightDelete()) return true;

  if (canSkipConversionToNullable(part, metadata_snapshot, command)) return true;

  if (canSkipConversionToVariant(part, command)) return true;
  ...
  for (const auto &index : indices) {
    ...
    if (need_recalculate) {
      ...
      if (inserted) {
        ...
        for (const auto &expr : expr_list->children) indices_recalc_expr_list->children.push_back(expr->clone());
      }
    }
  }
...
bool MutateTask::prepare() {
  ...
  if (ctx->future_part->parts.size() != 1)
    throw Exception(ErrorCodes::LOGICAL_ERROR,
                    "Trying to mutate {} parts, not one. "
                    "This is a bug.",
                    ctx->future_part->parts.size());
  ...
  auto is_storage_touched =
      isStorageTouchedByMutations(ctx->source_part, mutations_snapshot, ctx->metadata_snapshot, ctx->commands_for_part, context_for_reading,
  ...
  if (!is_storage_touched.any_rows_affected) {
    ...
    /// In zero-copy replication checksums file path in s3 (blob path) is used for zero copy locks in ZooKeeper. If we will hardlink
    /// checksums file, we will have the same blob path and two different parts (source and new mutated part) will use the same locks in
    /// ZooKeeper. To avoid this we copy checksums.txt to generate new blob path. Example:
    ...
    bool copy_checksumns = ctx->data->supportsReplication() && (*settings_ptr)[MergeTreeSetting::allow_remote_fs_zero_copy_replication] &&
                           ctx->source_part->isStoredOnRemoteDiskWithZeroCopySupport();
    if (copy_checksumns) files_to_copy_instead_of_hardlinks.insert(IMergeTreeDataPart::FILE_FOR_REFERENCES_CHECK);

    LOG_TRACE(ctx->log, "Part {} doesn't change up to mutation version {}", ctx->source_part->name, ctx->future_part->part_info.mutation);
    std::string prefix;
    if (ctx->need_prefix) prefix = "tmp_clone_";
    ...
    {
      std::tie(part, lock) =
          ctx->data->cloneAndLoadDataPart(ctx->source_part, prefix, ctx->future_part->part_info, ctx->metadata_snapshot, clone_params,
  ...
  if (is_storage_touched.all_rows_affected) {
    bool has_only_delete_commands =
        std::ranges::all_of(ctx->commands_for_part, [](const auto &command) { return command.type == MutationCommand::DELETE; });

    if (has_only_delete_commands) {
      LOG_TRACE(ctx->log, "Part {} is fully deleted, creating empty part with mutation version {}", ctx->source_part->name,
                ctx->future_part->part_info.mutation);

      auto [empty_part, _] =
          ctx->data->createEmptyPart(ctx->future_part->part_info, ctx->source_part->partition, ctx->future_part->name, ctx->txn);
  ...
  bool suitable_for_ttl_optimization =
      ctx->metadata_snapshot->hasOnlyRowsTTL() && (*ctx->data->getSettings())[MergeTreeSetting::ttl_only_drop_parts];
  MutationHelpers::splitAndModifyMutationCommands(ctx->source_part, ctx->metadata_snapshot, alter_conversions, ctx->commands_for_part,
                                                  ctx->for_interpreter, ctx->for_file_renames, suitable_for_ttl_optimization, ctx->log);
  ...
  if (!ctx->for_interpreter.empty()) {
    ...
    ctx->interpreter = std::make_unique<MutationsInterpreter>(
        *ctx->data, ctx->source_part, alter_conversions, ctx->metadata_snapshot, ctx->for_interpreter,
    ...
    if (lightweight_delete_mode &&
      ...
      for (const auto &projection : ctx->metadata_snapshot->getProjections()) {
        if (!ctx->source_part->hasProjection(projection.name)) continue;
  ...
  std::string prefix;
  if (ctx->need_prefix) prefix = TEMP_DIRECTORY_PREFIX;
  ...
  auto [new_columns, new_infos, new_columns_substreams] =
      MutationHelpers::getColumnsForNewDataPart(ctx->source_part, ctx->updated_header, ctx->storage_columns,
  ...
  if (!new_columns_substreams.empty()) ctx->new_data_part->setColumnsSubstreams(new_columns_substreams);
  ...
  if (MutationHelpers::haveMutationsOfDynamicColumns(ctx->source_part, ctx->commands_for_part) || !isWidePart(ctx->source_part) ||
      !isFullPartStorage(ctx->source_part->getDataPartStorage()) || (ctx->interpreter && ctx->interpreter->isAffectingAllColumns())) {
    ...
    bool drop_expired_parts =
        suitable_for_ttl_optimization && !(*ctx->data->getSettings())[MergeTreeSetting::materialize_ttl_recalculate_only];
  ...
  } else  /// TODO: check that we modify only non-key columns in this case.
  {
    ...
    bool lightweight_delete_drops_projections = lightweight_mutation_projection_mode == LightweightMutationProjectionMode::DROP ||
    ...
    if (should_create_projections) {
      ctx->projections_to_recalc =
          MutationHelpers::getProjectionsToRecalculate(ctx->source_part, ctx->metadata_snapshot, ctx->materialized_projections);

      projections_to_skip.assign(ctx->projections_to_recalc.begin(), ctx->projections_to_recalc.end());
    } else {
      for (const auto &projection : ctx->metadata_snapshot->getProjections()) projections_to_skip.emplace_back(&projection);
    }
    ...
    ctx->files_to_skip =
        MutationHelpers::collectFilesToSkip(ctx->source_part, ctx->new_data_part, ctx->updated_header, all_indices_to_recalc,
                                            ctx->indices_to_drop, ctx->mrk_extension, projections_to_skip, updated_columns_in_patches);

    ctx->files_to_rename = MutationHelpers::collectFilesForRenames(ctx->metadata_snapshot, ctx->source_part, ctx->new_data_part,
                                                                   ctx->for_file_renames, updated_columns_in_patches, ctx->mrk_extension);
...
const HardlinkedFiles &MutateTask::getHardlinkedFiles() const { return ctx->hardlinked_files; }
