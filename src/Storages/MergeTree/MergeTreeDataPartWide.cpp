...
MergeTreeDataPartWide::MergeTreeDataPartWide(const MergeTreeData &storage_, const MergeTreeSettings &storage_settings, const String &name_,
                                             const MergeTreePartInfo &info_, const MutableDataPartStoragePtr &data_part_storage_,
                                             const IMergeTreeDataPart *parent_part_)
    : IMergeTreeDataPart(storage_, storage_settings, name_, info_, data_part_storage_, Type::Wide, parent_part_) {}

MergeTreeReaderPtr createMergeTreeReaderWide(const MergeTreeDataPartInfoForReaderPtr &read_info, const NamesAndTypesList &columns_to_read,
                                             const StorageSnapshotPtr &storage_snapshot, const MergeTreeSettingsPtr &storage_settings,
                                             const MarkRanges &mark_ranges, const VirtualFields &virtual_fields,
                                             UncompressedCache *uncompressed_cache, MarkCache *mark_cache,
                                             DeserializationPrefixesCache *deserialization_prefixes_cache,
                                             const MergeTreeReaderSettings &reader_settings, const ValueSizeMap &avg_value_size_hints,
                                             const ReadBufferFromFileBase::ProfileCallback &profile_callback) {
  return std::make_unique<MergeTreeReaderWide>(read_info, columns_to_read, virtual_fields, storage_snapshot, storage_settings,
                                               uncompressed_cache, mark_cache, deserialization_prefixes_cache, mark_ranges, reader_settings,
...
MergeTreeDataPartWriterPtr createMergeTreeDataPartWideWriter(
    ...
    const MergeTreeSettingsPtr &storage_settings_, const NamesAndTypesList &columns_list, const StorageMetadataPtr &metadata_snapshot,
    const VirtualsDescriptionPtr &virtual_columns, const std::vector<MergeTreeIndexPtr> &indices_to_recalc,
    const String &marks_file_extension_, const CompressionCodecPtr &default_codec_, const MergeTreeWriterSettings &writer_settings,
    MergeTreeIndexGranularityPtr computed_index_granularity, WrittenOffsetSubstreams *written_offset_substreams) {
  return std::make_unique<MergeTreeDataPartWriterWide>(data_part_name_, logger_name_, serializations_, data_part_storage_,
                                                       ...
                                                       virtual_columns, indices_to_recalc, marks_file_extension_, default_codec_,
                                                       writer_settings, std::move(computed_index_granularity), written_offset_substreams);
}
...
void MergeTreeDataPartWide::addStreamToColumnSize(const String &stream_name, ColumnSize &size) const {
  ...
  if (mrk_checksum != checksums.files.end()) size.marks += mrk_checksum->second.file_size;
}
...
ColumnSize MergeTreeDataPartWide::getColumnSizeImpl(const NameAndTypePair &column, std::unordered_set<String> *processed_substreams) const {
  ColumnSize size;
  if (checksums.empty()) return size;

  if (column.type->hasDynamicSubcolumns() && !columns_substreams.empty()) {
    auto column_position = getColumnPosition(column.name);
    if (!column_position) return size;
    ...
    getSerialization(column.name)
        ->enumerateStreams(
            [&](const ISerialization::SubstreamPath &substream_path) {
              auto stream_name =
                  IMergeTreeDataPart::getStreamNameForColumn(column, substream_path, DATA_FILE_EXTENSION, checksums, storage.getSettings());

              if (!stream_name) return;

              if (processed_substreams && !processed_substreams->insert(*stream_name).second) return;
...
ColumnSize MergeTreeDataPartWide::calculateSubcolumnSize(const String &subcolumn_name) const {
  ...
  for (const auto &stream : getListOfStreamsForColumn(getColumn(subcolumn_name))) addStreamToColumnSize(stream, size);

  return size;
}

void MergeTreeDataPartWide::loadIndexGranularityImpl(MergeTreeIndexGranularityPtr &index_granularity_ptr,
  ...
  if (!data_part_storage_.existsFile(marks_file_path))
    throw Exception(ErrorCodes::NO_FILE_IN_DATA_PART, "Marks file '{}' doesn't exist",
  ...
  if (!index_granularity_info_.mark_type.adaptive && !index_granularity_info_.mark_type.compressed) {
    ...
    if (!index_granularity_info_.mark_type.adaptive) {
      ...
      if (auto new_granularity_ptr = index_granularity_ptr->optimize()) index_granularity_ptr = std::move(new_granularity_ptr);
    }
  }
}

void MergeTreeDataPartWide::loadIndexGranularity() {
  if (columns.empty()) throw Exception(ErrorCodes::NO_FILE_IN_DATA_PART, "No columns in part {}", name);

  auto any_column_filename = getFileNameForColumn(columns.front());
  if (!any_column_filename)
    throw Exception(ErrorCodes::NO_FILE_IN_DATA_PART, "There are no files for column {} in part {}", columns.front().name,
...
void MergeTreeDataPartWide::loadMarksToCache(const Names &column_names, MarkCache *mark_cache) const {
  if (column_names.empty() || !mark_cache) return;
  ...
  for (const auto &column_name : column_names) {
    ...
    serialization->enumerateStreams([&](const auto &subpath) {
      ...
      loaders.emplace_back(std::make_unique<MergeTreeMarksLoader>(info_for_read, mark_cache,
                                                                  index_granularity_info.getMarksFilePath(*stream_name),
                                                                  index_granularity->getMarksCount(), index_granularity_info,
...
bool MergeTreeDataPartWide::isStoredOnRemoteDisk() const { return getDataPartStorage().isStoredOnRemoteDisk(); }

bool MergeTreeDataPartWide::isStoredOnReadonlyDisk() const { return getDataPartStorage().isReadonly(); }

bool MergeTreeDataPartWide::isStoredOnRemoteDiskWithZeroCopySupport() const { return getDataPartStorage().supportZeroCopyReplication(); }
...
void MergeTreeDataPartWide::doCheckConsistency(bool require_part_metadata) const {
  ...
  if (!checksums.empty()) {
    if (require_part_metadata) {
      for (const auto &name_type : columns) {
        getSerialization(name_type.name)->enumerateStreams([&](const ISerialization::SubstreamPath &substream_path) {
          ...
          if (!stream_name)
            throw Exception(ErrorCodes::NO_FILE_IN_DATA_PART, "No stream ({}{}) file checksum for column {} in part {}",
                            ISerialization::getFileNameForStream(name_type, substream_path,
                                                                 ISerialization::StreamFileNameSettings(*storage.getSettings())),
                            DATA_FILE_EXTENSION, name_type.name, getDataPartStorage().getFullPath());

          auto mrk_file_name = *stream_name + marks_file_extension;
          if (!checksums.files.contains(mrk_file_name))
            throw Exception(ErrorCodes::NO_FILE_IN_DATA_PART, "No {} file checksum for column {} in part {} ", mrk_file_name,
                            name_type.name, getDataPartStorage().getFullPath());
        });
      }
    }
  } else {
    ...
    for (const auto &name_type : columns) {
      getSerialization(name_type.name)->enumerateStreams([&](const ISerialization::SubstreamPath &substream_path) {
        auto stream_name =
            getStreamNameForColumn(name_type, substream_path, marks_file_extension, getDataPartStorage(), storage.getSettings());
        ...
        if (!marks_size)
          ...
          throw Exception(ErrorCodes::BAD_SIZE_OF_FILE_IN_DATA_PART, "Part {} is broken: marks have different sizes.",
                          getDataPartStorage().getFullPath());
      });
    }
  }
...
bool MergeTreeDataPartWide::hasColumnFiles(const NameAndTypePair &column) const {
  ...
  serialization->enumerateStreams([&](const auto &substream_path) {
    ...
    if (!stream_name || !checksums.files.contains(*stream_name + marks_file_extension)) res = false;
  });
...
void MergeTreeDataPartWide::calculateEachColumnSizes(ColumnSizeByName &each_columns_size, ColumnSize &total_size) const {
  ...
  for (const auto &column : columns) {
    ...
    if (rows_count != 0
        ...
        && size.data_uncompressed != 0 && column.type->isValueRepresentedByNumber() && !column.type->haveSubtypes() &&
...
std::vector<String> MergeTreeDataPartWide::getListOfStreamsForColumn(const NameAndTypePair &column) const {
  ...
  MergeTreeReaderPtr reader =
      createMergeTreeReaderWide(part_info, cols, storage_snapshot_ptr, storage.getSettings(), MarkRanges{MarkRange(0, getMarksCount())},
                                /*virtual_fields=*/{},
