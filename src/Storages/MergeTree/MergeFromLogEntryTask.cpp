...
MergeFromLogEntryTask::MergeFromLogEntryTask(ReplicatedMergeTreeQueue::SelectedEntryPtr selected_entry_,
                                             StorageReplicatedMergeTree& storage_,
                                             IExecutableTask::TaskResultCallback& task_result_callback_)
    : ReplicatedMergeMutateTaskBase(
          getLogger(storage_.getStorageID().getShortName() + "::" + selected_entry_->log_entry->new_part_name + " (MergeFromLogEntryTask)"),
...
ReplicatedMergeMutateTaskBase::PrepareResult MergeFromLogEntryTask::prepare() {
  ...
  fiu_do_on(FailPoints::rmt_merge_task_sleep_in_prepare, { std::this_thread::sleep_for(std::chrono::milliseconds(3000)); });
  ...
  auto part_log_writer = [this, stopwatch = *stopwatch_ptr](const ExecutionStatus& execution_status) {
    ...
    storage.writePartLog(PartLogElement::MERGE_PARTS, execution_status, stopwatch.elapsed(), entry.new_part_name, part, parts,
                         merge_mutate_entry.get(), std::move(profile_counters_snapshot));
  };
  ...
  if (entry.merge_type == MergeType::TTLRecompress &&
      (time(nullptr) - entry.create_time) <=
          (*storage_settings_ptr)[MergeTreeSetting::try_fetch_recompressed_part_timeout].totalSeconds() &&
    ...
    LOG_INFO(log,
             ...
             DateLUT::serverTimezoneInstance().timeToString(
                 entry.create_time + (*storage_settings_ptr)[MergeTreeSetting::try_fetch_recompressed_part_timeout].totalSeconds()),
  ...
  if (storage.merge_strategy_picker.shouldMergeOnSingleReplica(entry)) {
    ...
    if (replica_to_execute_merge) {
      LOG_DEBUG(log, "Prefer fetching part {} from replica {} due to execute_merges_on_single_replica_time_threshold", entry.new_part_name,
                replica_to_execute_merge.value());
  ...
  for (const String& source_part_name : entry.source_parts) {
    ...
    if (!source_part_or_covering) {
      ...
      LOG_DEBUG(log,
                "Don't have all parts (at least {} is missing) for merge {}; will try to fetch it instead. "
    ...
    if (source_part_or_covering->name != source_part_name) {
      ...
      if (!source_part_or_covering->info.contains(new_part_info)) throw Exception::createDeprecated(message, ErrorCodes::LOGICAL_ERROR);
  ...
  for (const auto& patch_part_name : entry.patch_parts) {
    ...
    if (!patch_part || patch_part->name != patch_part_name) {
      /// We do not have one of source parts locally, try to take some already merged part from someone.
      LOG_DEBUG(log,
                "Don't have all patch parts (at least {} is missing) for merge {}; "
                "will try to fetch part instead. Either pool for fetches is starving, see background_fetches_pool_size, or none of active "
                "replicas has it",
  ...
  if (entry.create_time + (*storage_settings_ptr)[MergeTreeSetting::prefer_fetch_merged_part_time_threshold].totalSeconds() <=
      time(nullptr)) {
    /// If entry is old enough, and have enough size, and part are exists in any replica,
    ///  then prefer fetching of merged part from replica.

    size_t sum_parts_bytes_on_disk = 0;
    for (const auto& item : parts) sum_parts_bytes_on_disk += item->getBytesOnDisk();

    if (sum_parts_bytes_on_disk >= (*storage_settings_ptr)[MergeTreeSetting::prefer_fetch_merged_part_size_threshold]) {
      String replica =
          storage.findReplicaHavingPart(entry.new_part_name, true);  /// NOTE excessive ZK requests for same data later, may remove.
      if (!replica.empty()) {
  ...
  table_lock_holder =
      storage.lockForShare(RWLockImpl::NO_QUERY, (*storage_settings_ptr)[MergeTreeSetting::lock_acquire_timeout_for_background_operations]);
  ...
  ReservationSharedPtr reserved_space =
      storage.balancedReservation(metadata_snapshot, estimated_space_for_merge, max_volume_index, future_merged_part->name,
                                  future_merged_part->part_info, future_merged_part->parts, &tagger, &ttl_infos);

  if (!reserved_space)
    reserved_space =
        storage.reserveSpacePreferringTTLRules(metadata_snapshot, estimated_space_for_merge, ttl_infos, time(nullptr), max_volume_index);

  future_merged_part->uuid = entry.new_part_uuid;
  future_merged_part->updatePath(storage, reserved_space.get());
  future_merged_part->merge_type = entry.merge_type;
  /// If a merge is a cleanup merge we need to mark the future part as final as cleanup merges can only be performed when merging all parts
  /// in a partition down to a single part.
  ...
  if ((*storage_settings_ptr)[MergeTreeSetting::allow_remote_fs_zero_copy_replication]) {
    if (auto disk = reserved_space->getDisk(); disk->supportZeroCopyReplication()) {
      ...
      if (!zero_copy_lock || !zero_copy_lock->isLocked()) {
        LOG_DEBUG(log, "Merge of part {} started by some other replica, will wait for it and fetch merged part. Number of tries {}",
                  entry.new_part_name, entry.num_tries);
      ...
      if (storage.findReplicaHavingCoveringPart(entry.new_part_name, /* active */ false)) {
        /// Why this if still needed? We can check for part in zookeeper, don't find it and sleep for any amount of time. During this sleep
        /// part will be actually committed from other replica and exclusive zero copy lock will be released. We will take the lock and
        ...
        ///
        /// NOTE: In case of mutation and hardlinks it can even lead to extremely rare dataloss (we will produce new part with the same
        /// hardlinks, don't fetch the same from other replica), so this check is important.
  ...
  /// Account TTL merge
  if (isTTLMergeType(future_merged_part->merge_type)) storage.getContext()->getMergeList().bookMergeWithTTL();
  ...
  /// Add merge to list
  merge_mutate_entry = storage.getContext()->getMergeList().insert(storage.getStorageID(), future_merged_part, task_context);

  storage.writePartLog(PartLogElement::MERGE_PARTS_START, {}, 0, entry.new_part_name, part, parts, merge_mutate_entry.get(), {});

  transaction_ptr = std::make_unique<MergeTreeData::Transaction>(storage, NO_TRANSACTION_RAW);

  merge_task = storage.merger_mutator.mergePartsToTemporaryPart(
      future_merged_part, metadata_snapshot, merge_mutate_entry.get(), {} /* projection_merge_list_element */, table_lock_holder,
      entry.create_time, storage.getContext(), reserved_space, entry.deduplicate, entry.deduplicate_by_columns, entry.cleanup,
      storage.merging_params, NO_TRANSACTION_PTR);

  /// Adjust priority
  for (auto& item : future_merged_part->parts) priority.value += item->getBytesOnDisk();
...
bool MergeFromLogEntryTask::finalize(ReplicatedMergeMutateTaskBase::PartLogWriter write_part_log) {
  ...
  try {
    ...
    if (MergeTreeDataPartChecksums::isBadChecksumsErrorCode(e.code())) {
      ...
      for (const auto& file : part->getFilesChecksums()) {
        files_with_size.push_back(
            fmt::format("{}: {} ({})", file.first, file.second.file_size, getHexUIntLowercase(file.second.file_hash)));
      }
  ...
  if (zero_copy_lock) zero_copy_lock->lock->unlock();
  ...
  if (prewarm_caches.mark_cache) addMarksToCache(*part, cached_marks, prewarm_caches.mark_cache.get());

  if (prewarm_caches.index_mark_cache) addMarksToCache(*part, cached_index_marks, prewarm_caches.index_mark_cache.get());

  /// Move index to cache and reset it here because we need
  /// a correct part name after rename for a key of cache entry.
  if (prewarm_caches.primary_index_cache) part->moveIndexToCache(*prewarm_caches.primary_index_cache);
