...
class MergeTreeReaderCompact : public IMergeTreeReader {
 public:
  MergeTreeReaderCompact(MergeTreeDataPartInfoForReaderPtr data_part_info_for_read_, NamesAndTypesList columns_,
 ...
 protected:
  ...
  void readData(size_t column_idx, ColumnPtr& column, size_t rows_to_read, size_t rows_offset, size_t from_mark,
                size_t column_size_before_reading, MergeTreeReaderStream& stream, std::unordered_map<String, ColumnPtr>& columns_cache,
                std::unordered_map<String, ColumnPtr>* columns_cache_for_subcolumns, ISerialization::SubstreamsCache* substreams_cache);

  virtual MergeTreeReaderStream& getStream(const NameAndTypePair& column) = 0;

  void readPrefix(size_t column_idx, size_t from_mark, MergeTreeReaderStream& stream,
 ...
 private:
  void readPrefix(const NameAndTypePair& name_and_type, const SerializationPtr& serialization,
