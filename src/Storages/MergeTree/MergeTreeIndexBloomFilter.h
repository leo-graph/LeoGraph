...
class MergeTreeIndexConditionBloomFilter final : public IMergeTreeIndexCondition, WithContext {
 public:
  ...
  bool mayBeTrueOnGranule(MergeTreeIndexGranulePtr granule,
                          const UpdatePartialDisjunctionResultFn &update_partial_result_disjuntion_fn) const override {
 ...
 private:
  ...
  bool mayBeTrueOnGranule(const MergeTreeIndexGranuleBloomFilter *granule,
                          const UpdatePartialDisjunctionResultFn &update_partial_result_disjuntion_fn) const;
  ...
  bool traverseTreeIn(const String &function_name, const RPNBuilderTreeNode &key_node, const ConstSetPtr &prepared_set,
                      const DataTypePtr &type, const ColumnPtr &column, RPNElement &out);

  bool traverseTreeEquals(const String &function_name, const RPNBuilderTreeNode &key_node, const DataTypePtr &value_type,
                          const Field &value_field, RPNElement &out, const RPNBuilderTreeNode *parent);
};
...
class MergeTreeIndexBloomFilter final : public IMergeTreeIndex {
 public:
  MergeTreeIndexBloomFilter(const IndexDescription &index_, size_t bits_per_row_, size_t hash_functions_);
