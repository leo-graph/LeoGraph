...
StorageID MutatePlainMergeTreeTask::getStorageID() const { return storage.getStorageID(); }
...
void MutatePlainMergeTreeTask::prepare() {
  ...
  storage.writePartLog(PartLogElement::MUTATE_PART_START, {}, 0, future_part->name, new_part, future_part->parts, merge_list_entry.get(),
                       {}, mutation_ids);

  write_part_log = [this, mutation_ids](const ExecutionStatus& execution_status) {
    auto profile_counters_snapshot = std::make_shared<ProfileEvents::Counters::Snapshot>(profile_counters.getPartiallyAtomicSnapshot());
    storage.writePartLog(PartLogElement::MUTATE_PART, execution_status, stopwatch->elapsed(), future_part->name, new_part,
                         future_part->parts, merge_list_entry.get(), std::move(profile_counters_snapshot), mutation_ids);
  };
  ...
  mutate_task = storage.merger_mutator.mutatePartToTemporaryPart(
      future_part, metadata_snapshot, merge_mutate_entry->commands, merge_list_entry.get(), time(nullptr), task_context,
      merge_mutate_entry->txn, merge_mutate_entry->tagger->reserved_space, table_lock_holder);
}

void MutatePlainMergeTreeTask::finish() {
  if (merge_mutate_entry) merge_mutate_entry->finalize();
}
...
bool MutatePlainMergeTreeTask::executeStep() {
  ...
  if (merge_list_entry) switcher.emplace((*merge_list_entry)->thread_group, ThreadName::MERGE_MUTATE, /*allow_existing_group*/ true);
  ...
  switch (state) {
    ...
    case State::NEED_EXECUTE: {
      try {
        if (mutate_task->execute()) return true;
        ...
        if (merge_mutate_entry->txn) merge_mutate_entry->txn->onException();
