...
struct MergeTreeSource::AsyncReadingState {
  ...
  ChunkAndProgress getResult() { return control->getResult(); }
  ...
  ~AsyncReadingState() {
    ...
    ///  ~EventFD()
    ///                                             event.write()
    if (control->stage == Stage::InProgress) control->event.read();
  }
...
#if defined(OS_LINUX)
  if (processor->getSettings().use_asynchronous_read_from_pool) async_reading_state = std::make_unique<AsyncReadingState>();
#endif
}

MergeTreeSource::~MergeTreeSource() = default;

std::string MergeTreeSource::getName() const { return processor->getName(); }

void MergeTreeSource::onCancel() noexcept { processor->cancel(); }

ISource::Status MergeTreeSource::prepare() {
#if defined(OS_LINUX)
  if (!async_reading_state) return ISource::prepare();
  ...
  if (async_reading_state && async_reading_state->getStage() == AsyncReadingState::Stage::InProgress) return ISource::Status::Async;
#endif
...
Chunk MergeTreeSource::processReadResult(ChunkAndProgress chunk) {
  if (chunk.num_read_rows || chunk.num_read_bytes) progress(chunk.num_read_rows, chunk.num_read_bytes);

  finished = chunk.is_finished;

  if (finished) processor->onFinish();
...
std::optional<Chunk> MergeTreeSource::tryGenerate() {
  ...
  if (async_reading_state) {
    if (async_reading_state->getStage() == AsyncReadingState::Stage::IsFinished) return processReadResult(async_reading_state->getResult());
...
#if defined(OS_LINUX)
int MergeTreeSource::schedule() { return async_reading_state->getFD(); }
#endif
