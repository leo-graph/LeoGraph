...
struct ReplicatedMergeTreeQuorumAddedParts {
  ...
  explicit ReplicatedMergeTreeQuorumAddedParts(const MergeTreeDataFormatVersion format_version_) : format_version(format_version_) {}
  ...
  void read(ReadBuffer& in) {
    if (checkString("version: ", in)) {
      ...
      if (version == 2) added_parts = readV2(in);
    } else
