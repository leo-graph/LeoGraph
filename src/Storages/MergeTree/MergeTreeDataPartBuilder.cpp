...
MergeTreeDataPartBuilder::MergeTreeDataPartBuilder(const MergeTreeData &data_, String name_, VolumePtr volume_, String root_path_,
                                                   String part_dir_, const ReadSettings &read_settings_)
    : data(data_),
      name(std::move(name_)),
...
MergeTreeDataPartBuilder::MergeTreeDataPartBuilder(const MergeTreeData &data_, String name_, MutableDataPartStoragePtr part_storage_,
                                                   const ReadSettings &read_settings_)
    : data(data_), name(std::move(name_)), part_storage(std::move(part_storage_)), read_settings(read_settings_) {}
...
std::shared_ptr<IMergeTreeDataPart> MergeTreeDataPartBuilder::build() {
  ...
  if (!part_type) throw Exception(ErrorCodes::LOGICAL_ERROR, "Cannot create part {}, because part type is not set", name);

  if (!part_storage) throw Exception(ErrorCodes::LOGICAL_ERROR, "Cannot create part {}, because part storage is not set", name);
  ...
  if (!data.canUsePolymorphicParts() && (part_type != PartType::Wide || part_storage_type != PartStorageType::Full)) {
  ...
  if (!part_info) part_info = MergeTreePartInfo::fromPartName(name, data.format_version);
  ...
  switch (part_type->getValue()) {
    ...
    default:
      throw Exception(ErrorCodes::UNKNOWN_PART_TYPE, "Unknown type of part {}", part_storage->getRelativePath());
  }
}

MutableDataPartStoragePtr MergeTreeDataPartBuilder::getPartStorageByType(MergeTreeDataPartStorageType storage_type_,
                                                                         const VolumePtr &volume_, const String &root_path_,
                                                                         const String &part_dir_,
                                                                         const ReadSettings &)  /// Unused here, but used in private repo.
{
  if (!volume_) throw Exception(ErrorCodes::LOGICAL_ERROR, "Cannot create part storage, because volume is not specified");
  ...
  switch (storage_type_.getValue()) {
    ...
    default:
      throw Exception(ErrorCodes::UNKNOWN_PART_TYPE, "Unknown type of storage for part {}", fs::path(root_path_) / part_dir_);
  }
}
...
MergeTreeDataPartBuilder &MergeTreeDataPartBuilder::withParentPart(const IMergeTreeDataPart *parent_part_) {
  if (parent_part_ && parent_part_->isProjectionPart()) throw Exception(ErrorCodes::LOGICAL_ERROR, "Parent part cannot be projection");
...
MergeTreeDataPartBuilder::PartStorageAndMarkType MergeTreeDataPartBuilder::getPartStorageAndMarkType(const VolumePtr &volume_,
...
MergeTreeDataPartBuilder &MergeTreeDataPartBuilder::withPartFormatFromDisk() {
  if (part_storage) return withPartFormatFromStorage();
