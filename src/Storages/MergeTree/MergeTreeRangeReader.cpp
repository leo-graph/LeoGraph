...
static bool canInplaceFilter(const ColumnPtr &column, const ColumnPtr &filter_column) {
  ...
  if (filter_column == column) return false;

  if (column->use_count() > 1) return false;

  bool can_inplace = true;
  column->forEachSubcolumn([&](const ColumnPtr &subcolumn) {
    if (!can_inplace) return;

    if (!canInplaceFilter(subcolumn, filter_column)) can_inplace = false;
  });
  ...
  for (auto &column : columns) {
    if (column) {
      if (column->size() != filter.size())
        throw Exception(ErrorCodes::LOGICAL_ERROR, "Size of column {} doesn't match size of filter {}", column->size(), filter.size());
...
void MergeTreeRangeReader::filterColumns(Columns &columns, const FilterWithCachedCount &filter) {
  if (filter.alwaysTrue()) return;

  if (filter.alwaysFalse()) {
    for (auto &col : columns)
      if (col) col = col->cloneEmpty();
...
size_t MergeTreeRangeReader::ReadResult::getLastMark(const MergeTreeRangeReader::ReadResult::RangesInfo &ranges) {
  ...
  for (const auto &mark_range : ranges) current_task_last_mark = std::max(current_task_last_mark, mark_range.range.end);
  return current_task_last_mark;
}

MergeTreeRangeReader::DelayedStream::DelayedStream(size_t from_mark, size_t current_task_last_mark_, IMergeTreeReader *merge_tree_reader_)
...
size_t MergeTreeRangeReader::DelayedStream::readRows(Columns &columns, size_t num_rows) {
  if (num_rows) {
    size_t rows_read = merge_tree_reader->readRows(current_mark, current_task_last_mark, continue_reading, num_rows, 0, columns);
    ...
    if (0 < rows_read && rows_read < num_rows) is_finished = true;
...
MergeTreeRangeReader::Stream::Stream(size_t from_mark, size_t to_mark, size_t current_task_last_mark, IMergeTreeReader *merge_tree_reader_)
  ...
  if (last_mark > marks_count)
    throw Exception(ErrorCodes::LOGICAL_ERROR, "Trying create stream to read to mark №{} but total marks count is {}", toString(last_mark),
                    toString(marks_count));
}

void MergeTreeRangeReader::Stream::checkNotFinished() const {
  if (isFinished()) throw Exception(ErrorCodes::BAD_ARGUMENTS, "Cannot read out of marks range.");
}
...
void MergeTreeRangeReader::Stream::toNextMark() {
  ...
  if (current_mark < total_marks_count)
    ...
    throw Exception(ErrorCodes::LOGICAL_ERROR, "Trying to read from mark {}, but total marks count {}", toString(current_mark),
                    toString(total_marks_count));
...
size_t MergeTreeRangeReader::Stream::read(Columns &columns, size_t num_rows, bool skip_remaining_rows_in_current_granule) {
  ...
  if (num_rows) {
    ...
    /// Start new granule.
    if (offset_after_current_mark == current_mark_index_granularity || skip_remaining_rows_in_current_granule) toNextMark();
...
void MergeTreeRangeReader::ReadResult::adjustLastGranule() {
  if (rows_per_granule.empty()) throw Exception(ErrorCodes::LOGICAL_ERROR, "Can't adjust last granule because no granules were added");
...
void MergeTreeRangeReader::ReadResult::shrink(Columns &old_columns, const NumRows &rows_per_granule_previous) const {
  for (auto &column : old_columns) {
    ...
    LOG_TEST(log, "ReadResult::shrink() column size: {} total_rows_per_granule: {}", column->size(), total_rows_per_granule);
    ...
    for (size_t j = 0, pos = 0; j < rows_per_granule_previous.size(); pos += rows_per_granule_previous[j++]) {
      if (rows_per_granule[j]) new_column->insertRangeFrom(*column, pos, rows_per_granule[j]);
    }
...
void MergeTreeRangeReader::ReadResult::checkInternalConsistency() const {
  ...
  if (final_filter.present() && final_filter.size() != total_rows_per_granule)
    throw Exception(ErrorCodes::LOGICAL_ERROR, "Final filter size {} doesn't match total_rows_per_granule {}", final_filter.size(),
  ...
  if (final_filter.present() && final_filter.countBytesInFilter() != num_rows && total_rows_per_granule != num_rows)
    throw Exception(ErrorCodes::LOGICAL_ERROR, "Number of rows {} doesn't match neither filter 1s count {} nor total_rows_per_granule {}",
  ...
  for (const auto &column : columns) {
    if (column) chassert(column->size() == num_rows);
  }
}

std::string MergeTreeRangeReader::ReadResult::dumpInfo() const {
  WriteBufferFromOwnString out;
  out << "num_rows: " << num_rows << ", columns: " << columns.size() << ", total_rows_per_granule: " << total_rows_per_granule;
  if (final_filter.present()) {
    out << ", filter size: " << final_filter.size() << ", filter 1s: " << final_filter.countBytesInFilter();
  } else {
    out << ", no filter";
  }
...
static ColumnPtr andFilters(ColumnPtr c1, ColumnPtr c2) {
  if (c1->size() != c2->size())
    throw Exception(ErrorCodes::LOGICAL_ERROR, "Size of filters don't match: {} and {}", c1->size(), c2->size());
  ...
  for (size_t i = 0; i < size; ++i) res_data[i] = (static_cast<UInt8>(!!c1_data[i]) & static_cast<UInt8>(!!c2_data[i]));
  return res;
}
...
void MergeTreeRangeReader::ReadResult::applyFilter(const FilterWithCachedCount &filter) {
  if (filter.size() != num_rows)
    throw Exception(ErrorCodes::LOGICAL_ERROR, "Filter size {} doesn't match number of rows {}", filter.size(), num_rows);
...
void MergeTreeRangeReader::ReadResult::optimize(const FilterWithCachedCount &current_filter, bool can_read_incomplete_granules,
  ...
  if (total_rows_per_granule == 0 || !filter.present()) return;
  ...
  SCOPE_EXIT(if (!std::uncaught_exceptions()) {
    checkInternalConsistency();
  ...
  if (2 * total_zero_rows_in_tails > filter.size()) {
        ...
        filter.countBytesInFilter() + total_zero_rows_in_tails == total_rows_per_granule_previous)  /// All zeros are in tails?
    {
      ...
      if (total_rows_per_granule != new_filter->size())
        throw Exception(ErrorCodes::LOGICAL_ERROR, "New filter size {} doesn't match number of rows to be read {}", new_filter->size(),
                        total_rows_per_granule);
...
size_t MergeTreeRangeReader::ReadResult::countZeroTails(const IColumn::Filter &filter_vec, NumRows &zero_tails,
                                                        bool can_read_incomplete_granules) const {
  zero_tails.resize(0);
  ...
  for (auto rows_to_read : rows_per_granule) {
    ...
    if (!can_read_incomplete_granules && zero_tail != rows_to_read) zero_tail = 0;
...
void MergeTreeRangeReader::ReadResult::collapseZeroTails(const IColumn::Filter &filter_vec, const NumRows &rows_per_granule_previous,
                                                         IColumn::Filter &new_filter_vec) const {
  const auto *filter_data = filter_vec.data();
...
DECLARE_X86_64_V4_SPECIFIC_CODE(size_t numZerosInTail(const UInt8 *begin, const UInt8 *end) {
  size_t count = 0;
  ...
  while (end - begin >= 64) {
    ...
    const auto *pos = end;
    UInt64 val =
        static_cast<UInt64>(_mm512_cmp_epi8_mask(_mm512_loadu_si512(reinterpret_cast<const __m512i *>(pos)), zero64, _MM_CMPINT_EQ));
    val = ~val;
    if (val == 0)
      count += 64;
    else {
...
DECLARE_X86_64_V3_SPECIFIC_CODE(size_t numZerosInTail(const UInt8 *begin, const UInt8 *end) {
  size_t count = 0;
  ...
  while (end - begin >= 64) {
    ...
    const auto *pos = end;
    UInt64 val =
        (static_cast<UInt64>(_mm256_movemask_epi8(_mm256_cmpeq_epi8(_mm256_loadu_si256(reinterpret_cast<const __m256i *>(pos)), zero32))) &
         0xffffffffu) |
        (static_cast<UInt64>(
             _mm256_movemask_epi8(_mm256_cmpeq_epi8(_mm256_loadu_si256(reinterpret_cast<const __m256i *>(pos + 32)), zero32)))
         << 32u);
...
size_t MergeTreeRangeReader::ReadResult::numZerosInTail(const UInt8 *begin, const UInt8 *end) {
  ...
  if (isArchSupported(TargetArch::x86_64_v4)) return TargetSpecific::x86_64_v4::numZerosInTail(begin, end);
  if (isArchSupported(TargetArch::x86_64_v3)) return TargetSpecific::x86_64_v3::numZerosInTail(begin, end);
#endif
  ...
  while (end - begin >= 64) {
    ...
    const auto *pos = end;
    UInt64 val =
        static_cast<UInt64>(_mm_movemask_epi8(_mm_cmpeq_epi8(_mm_loadu_si128(reinterpret_cast<const __m128i *>(pos)), zero16))) |
        (static_cast<UInt64>(_mm_movemask_epi8(_mm_cmpeq_epi8(_mm_loadu_si128(reinterpret_cast<const __m128i *>(pos + 16)), zero16)))
         << 16u) |
        (static_cast<UInt64>(_mm_movemask_epi8(_mm_cmpeq_epi8(_mm_loadu_si128(reinterpret_cast<const __m128i *>(pos + 32)), zero16)))
         << 32u) |
        (static_cast<UInt64>(_mm_movemask_epi8(_mm_cmpeq_epi8(_mm_loadu_si128(reinterpret_cast<const __m128i *>(pos + 48)), zero16)))
         << 48u);
    val = ~val;
    if (val == 0)
      count += 64;
...
MergeTreeRangeReader::MergeTreeRangeReader(IMergeTreeReader *merge_tree_reader_, Block prev_reader_header_,
                                           const PrewhereExprStep *prewhere_info_, ReadStepPerformanceCountersPtr performance_counters_,
                                           bool main_reader_)
    : merge_tree_reader(merge_tree_reader_),
      index_granularity(&(merge_tree_reader->data_part_info_for_read->getIndexGranularity())),
  ...
  if (prewhere_info) {
    ...
    if (step.actions) step.actions->execute(result_sample_block, true);

    if (step.remove_filter_column) result_sample_block.erase(step.filter_column_name);
  }
}

size_t MergeTreeRangeReader::numReadRowsInCurrentGranule() const { return stream.numReadRowsInCurrentGranule(); }

size_t MergeTreeRangeReader::numPendingRowsInCurrentGranule() const {
  auto pending_rows = stream.numPendingRowsInCurrentGranule();

  if (pending_rows) return pending_rows;
...
size_t MergeTreeRangeReader::currentMark() const { return stream.currentMark(); }
...
UInt64 MergeTreeRangeReader::Stream::lastPartOffset() const { return index_granularity->getMarkStartingRow(last_mark); }
...
bool MergeTreeRangeReader::isCurrentRangeFinished() const { return stream.isFinished(); }
...
static size_t getBytesInColumn(const IColumn &column) {
  if (const auto *col_str = typeid_cast<const ColumnString *>(&column)) {
    ...
    return getBytesInColumn(values) + offsets.size() * getLengthOfVarInt(offsets.getInt(offsets.size() - 1));
  } else {
...
static size_t getTotalBytesInColumns(const Columns &columns) {
  ...
  for (const auto &column : columns) {
    if (column) total_bytes += getBytesInColumn(*column);
  }
  return total_bytes;
}
...
MergeTreeRangeReader::ReadResult MergeTreeRangeReader::startReadingChain(size_t max_rows, MarkRanges &ranges) {
  ...
  if (!stream.isFinished()) current_mark = stream.current_mark;
  ...
  /// result.num_rows_read if the last granule in range also the last in part (so we have to adjust last granule).
  {
    ...
    while (space_left && (!stream.isFinished() || !ranges.empty())) {
      if (stream.isFinished()) {
        ...
        if (current_mark && *current_mark < stream.last_mark) result.addReadRange(MarkRange(*current_mark, stream.last_mark));
  ...
  if (current_mark && current_mark < last_mark) result.addReadRange(MarkRange{*current_mark, last_mark});

  /// Last granule may be incomplete.
  if (!result.rows_per_granule.empty()) result.adjustLastGranule();
...
void MergeTreeRangeReader::fillVirtualColumns(Columns &columns, ReadResult &result) {
  ...
  auto add_offset_column = [&](const auto &column_name) {
    ...
    if (columns[pos]) return;

    if (column_name == "_part_offset" || column_name == BlockOffsetColumn::name) {
      if (!part_offset_column) part_offset_column = createPartOffsetColumn(result);
  ...
  if (read_sample_block.has("_part_offset")) add_offset_column("_part_offset");

  /// Column _block_offset is the same as _part_offset if it's not persisted in part.
  if (read_sample_block.has(BlockOffsetColumn::name)) add_offset_column(BlockOffsetColumn::name);

  if (read_sample_block.has("_part_granule_offset")) add_offset_column("_part_granule_offset");
...
void MergeTreeRangeReader::fillDistanceColumnAndFilterForVectorSearch(Columns &columns, ReadResult & /*result*/,
                                                                      ColumnPtr &part_offsets_auto_column) {
  ...
  for (size_t i = 0; i < distances_from_index.size(); ++i) offset_to_distance[row_offsets_from_index[i]] = distances_from_index[i];
  ...
  for (size_t i = 0; i < part_offsets_auto_column->size(); ++i) {
    while (j < row_offsets_from_index.size() && offsets[i] > row_offsets_from_index[j]) j++;

    if (j >= row_offsets_from_index.size()) break;
...
Columns MergeTreeRangeReader::continueReadingChain(ReadResult &result, size_t &num_rows) {
  ...
  if (merge_tree_reader->getColumns().empty()) return columns;
  ...
  for (auto i : collections::range(0, size)) {
    if (next_range_to_start < started_ranges.size() && i == started_ranges[next_range_to_start].num_granules_read_before_start) {
  ...
  if (num_rows == 0) num_rows = result.total_rows_per_granule;

  if (num_rows != result.total_rows_per_granule)
    throw Exception(ErrorCodes::LOGICAL_ERROR, "RangeReader read {} rows, but {} expected.", num_rows, result.total_rows_per_granule);
...
static void checkCombinedFiltersSize(size_t bytes_in_first_filter, size_t second_filter_size) {
  if (bytes_in_first_filter != second_filter_size)
    throw Exception(ErrorCodes::LOGICAL_ERROR,
                    ...
                    bytes_in_first_filter, second_filter_size);
}

DECLARE_X86_ICELAKE_SPECIFIC_CODE(inline void combineFiltersImpl(UInt8 *first_begin, const UInt8 *first_end, const UInt8 *second_begin) {
 ...
 */
DECLARE_X86_64_V3_SPECIFIC_CODE(inline void combineFiltersImpl(UInt8 *first_begin, const UInt8 *first_end, const UInt8 *second_begin) {
...
static ColumnPtr combineFilters(ColumnPtr first, ColumnPtr second) {
  ...
  if (second_const_descr.always_true) return first;

  if (second_const_descr.always_false) return second->cloneResized(first->size());
...
void MergeTreeRangeReader::executePrewhereActionsAndFilterColumns(ReadResult &result, const Block &previous_header,
  ...
  if (!prewhere_info || prewhere_info->type == PrewhereExprStep::None) return;
  ...
  if (result.columns.size() < num_columns)
    throw Exception(ErrorCodes::LOGICAL_ERROR, "Invalid number of columns passed to MergeTreeRangeReader. Expected {}, got {}", num_columns,
                    result.columns.size());
...

  {
    ...
    if (prewhere_info->actions) {
      ...
      if (!dummy_column.empty()) block.erase(dummy_column);
    }
    ...
    if (!is_last_reader) {
      for (auto &col : additional_columns) {
        ...
        if (block.has(col.name)) continue;
  ...
  for (auto &col : block) result.columns.emplace_back(std::move(col.column));
  ...
  if (prewhere_info->type == PrewhereExprStep::Filter) {
    ...
    if (prewhere_info->remove_filter_column) result.columns.erase(result.columns.begin() + filter_column_pos);
    ...
    if (prewhere_info->need_filter && !result.filterWasApplied()) {
      ...
      FilterWithCachedCount filter_to_apply = current_filter.size() == result.total_rows_per_granule ? result.final_filter : current_filter;
...
std::string PrewhereExprInfo::dump() const {
  ...
  for (size_t i = 0; i < steps.size(); ++i) {
    s << "STEP " << i << ":\n"
      << "  ACTIONS: "
      << (steps[i]->actions ? (indent + boost::replace_all_copy(steps[i]->actions->dumpActions(), "\n", indent)) : "nullptr") << "\n"
...
std::string PrewhereExprInfo::dumpConditions() const {
  ...
  for (size_t i = 0; i < steps.size(); ++i) s << (i == 0 ? "\"" : ", \"") << steps[i]->filter_column_name << "\"";
