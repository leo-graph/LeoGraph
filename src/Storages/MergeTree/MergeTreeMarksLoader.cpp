...
MarkInCompressedFile MergeTreeMarksGetter::getMark(size_t row_index, size_t column_index) const {
  ...
  if (column_index >= num_columns_in_mark)
    throw Exception(ErrorCodes::LOGICAL_ERROR, "Column index: {} is out of range [0, {})", column_index, num_columns_in_mark);
#endif

  return marks->get(row_index * num_columns_in_mark + column_index);
}

MergeTreeMarksLoader::MergeTreeMarksLoader(MergeTreeDataPartInfoForReaderPtr data_part_reader_, MarkCache *mark_cache_,
                                           const String &mrk_path_, size_t marks_count_,
                                           const MergeTreeIndexGranularityInfo &index_granularity_info_, bool save_marks_in_cache_,
                                           const ReadSettings &read_settings_, ThreadPool *load_marks_threadpool_,
...
void MergeTreeMarksLoader::startAsyncLoad() {
  if (load_marks_threadpool) future = loadMarksAsync();
}
...
MergeTreeMarksGetterPtr MergeTreeMarksLoader::loadMarks() {
  ...
  if (marks) return std::make_unique<MergeTreeMarksGetter>(marks, num_columns_in_mark);
...
MarkCache::MappedPtr MergeTreeMarksLoader::loadMarksImpl() {
  ...
  if (file_size == 0 && marks_count != 0) {
    throw Exception(ErrorCodes::CORRUPTED_DATA, "Empty marks file '{}': {}, must be: {}", full_mark_path, file_size,
                    expected_uncompressed_size);
  }

  if (!index_granularity_info.mark_type.compressed && expected_uncompressed_size != file_size)
    throw Exception(ErrorCodes::CORRUPTED_DATA, "Bad size of marks file '{}': {}, must be: {}", full_mark_path, file_size,
                    expected_uncompressed_size);
  ...
  if (!index_granularity_info.mark_type.adaptive) {
    ...
    if (!reader->eof())
      throw Exception(ErrorCodes::CANNOT_READ_ALL_DATA, "Cannot read all marks from file {}, is eof: {}, buffer size: {}, file size: {}",
                      full_mark_path, reader->eof(), reader->buffer().size(), file_size);
  } else {
    for (size_t i = 0; i < marks_count; ++i) {
      ...
      size_t granularity;
      reader->readStrict(reinterpret_cast<char *>(plain_marks.data() + i * num_columns_in_mark),
                         num_columns_in_mark * sizeof(MarkInCompressedFile));
      readBinaryLittleEndian(granularity, *reader);
    }

    if (!reader->eof())
      throw Exception(ErrorCodes::CANNOT_READ_ALL_DATA, "Too many marks in file {}, marks expected {} (bytes size {})", full_mark_path,
                      marks_count, expected_uncompressed_size);
  }

  if constexpr (std::endian::native == std::endian::big) {
    std::ranges::for_each(plain_marks, [](auto &plain_mark) {
...
MarkCache::MappedPtr MergeTreeMarksLoader::loadMarksSync() {
  ...
  if (mark_cache) {
    ...
    if (save_marks_in_cache) {
      ...
      if (!loaded_marks) loaded_marks = loadMarksImpl();
    }
  } else {
  ...
  if (!loaded_marks) {
    throw Exception(ErrorCodes::LOGICAL_ERROR, "Failed to load marks: {}",
...
std::future<MarkCache::MappedPtr> MergeTreeMarksLoader::loadMarksAsync() {
  ...
  return scheduleFromThreadPoolUnsafe<MarkCache::MappedPtr>(
      ...
      *load_marks_threadpool, ThreadName::LOAD_MARKS);
}
...
void addMarksToCache(const IMergeTreeDataPart &part, const PlainMarksByName &cached_marks, MarkCache *mark_cache) {
  ...
  for (const auto &[stream_name, marks] : cached_marks) {
    ...
    auto key = MarkCache::hash(part.getDataPartStorage().getDiskName() + ":" +
                               (fs::path(part.getRelativePathOfActivePart()) / mark_path).string());
...
Names getColumnsToPrewarmMarks(const MergeTreeSettings &settings, const NamesAndTypesList &columns_list) {
  ...
  if (columns_str.empty()) return columns_list.getNames();
