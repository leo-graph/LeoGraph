...
size_t MergeTreeReadersChain::currentMark() const { return range_readers.empty() ? 0 : range_readers.back().currentMark(); }
...
static ColumnsWithTypeAndName toColumnsWithTypeAndName(const Columns& columns, const NamesAndTypes& on_disk_columns) {
  if (columns.size() != on_disk_columns.size())
    throw Exception(ErrorCodes::LOGICAL_ERROR,
                    "Number of columns doesn't match number of on-disk columns, columns size: {}, on_disk_columns size: {}", columns.size(),
  ...
  for (size_t i = 0; i < columns.size(); ++i) {
    ...
    if (columns[i]) res.emplace_back(columns[i], on_disk_columns[i].type, on_disk_columns[i].name);
  }
  return res;
}
...
MergeTreeReadersChain::ReadResult MergeTreeReadersChain::read(size_t max_rows, MarkRanges& ranges, std::vector<MarkRanges>& patch_ranges,
  ...
  if (max_rows == 0) throw Exception(ErrorCodes::LOGICAL_ERROR, "Expected at least 1 row to read, got 0.");

  ReadResult read_result{log};

  if (range_readers.empty()) return read_result;
  ...
  if (read_result.num_rows != 0) {
    ...
    if (dataflow_cache_update_cb)
      dataflow_cache_update_cb(toColumnsWithTypeAndName(read_result.columns, first_reader.getReader()->getColumnsToRead()),
                               read_result.num_bytes_read, should_continue_sampling);
  ...
  for (size_t i = 1; i < range_readers.size(); ++i) {
    ...
    if (read_result.num_rows == 0) continue;
    ...
    if (!columns.empty()) {
      ...
      if (num_read_rows == 0) num_read_rows = read_result.num_rows;
      ...
      if (dataflow_cache_update_cb) {
        ...
        dataflow_cache_update_cb(toColumnsWithTypeAndName(columns, range_readers[i].getReader()->getColumnsToRead()),
                                 read_result.num_bytes_read - num_bytes_read_so_far, should_continue_sampling);
      }
...
void MergeTreeReadersChain::executeActionsBeforePrewhere(ReadResult& result, Columns& read_columns, MergeTreeRangeReader& range_reader,
                                                         const Block& previous_header, size_t num_read_rows) const {
  ...
  if (result.total_rows_per_granule != num_read_rows) {
    ...
    if (result.num_rows != num_read_rows)
      throw Exception(ErrorCodes::LOGICAL_ERROR,
                      ...
                      result.total_rows_per_granule, result.num_rows, num_read_rows);
  ...
  auto apply_patches = [&](ColumnForPatch::Order order) {
    ...
    applyPatches(result_header, read_columns, result.patch_versions_block,
                 /*min_version=*/{}, patch_max_version, columns_for_patches, {order}, result.columns_for_patches);
  };
  ...
  if (should_evaluate_missing_defaults) {
    ...
    if (!previous_header.empty()) additional_columns = previous_header.cloneWithColumns(result.columns);

    for (const auto& col : result.additional_columns) additional_columns.insert(col);
...
void MergeTreeReadersChain::executePrewhereActions(MergeTreeRangeReader& reader, ReadResult& result, const Block& previous_header,
                                                   bool is_last_reader) {
...
void MergeTreeReadersChain::addPatchVirtuals(ReadResult& result, const Block& header) const {
  if (patch_readers.empty() || result.num_rows == 0) return;
...
void MergeTreeReadersChain::addPatchVirtuals(Block& to, const Block& from) const {
  ...
  for (const auto& column : system_columns) {
    ...
    if (!to.has(column.name) && from.has(column.name)) to.insert(from.getByName(column.name));
  }
}
...
ColumnsForPatches MergeTreeReadersChain::getColumnsForPatches(const Block& header, const Columns& columns) const {
  ...
  for (const auto& patch_reader : patch_readers) {
    ...
    for (const auto& column : block) {
      ...
      if (!patch_columns.hasColumnOrSubcolumn(GetColumnsOptions::All, column_name_in_patch)) continue;
...
void MergeTreeReadersChain::applyPatchesAfterReader(ReadResult& result, size_t reader_index) {
  if (patch_readers.empty() || result.num_rows == 0 || result.columns.empty()) return;
  ...
  if (reader_index + 1 != range_readers.size()) max_version = getMaxPatchVersionForStep(range_readers[reader_index + 1]);

  if (min_version == max_version) return;

  if (min_version.has_value()) {
    for (auto& column : result.patch_versions_block) column.column = ColumnUInt64::create(result.patch_versions_block.rows(), *min_version);
  }
  ...
  applyPatches(result_header, result.columns, result.patch_versions_block, min_version, max_version, columns_for_patches, suitable_orders,
               result.columns_for_patches);
}

void MergeTreeReadersChain::applyPatches(const Block& result_header, Columns& result_columns, Block& versions_block,
                                         std::optional<UInt64> min_version, std::optional<UInt64> max_version,
                                         const ColumnsForPatches& columns_for_patches,
                                         const std::set<ColumnForPatch::Order>& suitable_orders, const Block& additional_columns) const {
  if (patch_readers.empty() || result_columns.empty()) return;
  ...
  for (size_t i = 0; i < patch_readers.size(); ++i) {
    ...
    if (static_cast<UInt64>(patch.source_data_version) != source_data_version) {
      throw Exception(ErrorCodes::LOGICAL_ERROR, "Patches for one reader must have the same source data version. Got: {}, expected: {}",
                      patch.source_data_version, source_data_version);
    }

    if (min_version.has_value() && !patchHasHigherDataVersion(*patch.part, *min_version)) continue;

    if (max_version.has_value() && patchHasHigherDataVersion(*patch.part, *max_version)) continue;

    Names updated_columns;
    for (const auto& columns_for_patch : columns_for_patches[i]) {
      if (suitable_orders.contains(columns_for_patch.order)) updated_columns.push_back(columns_for_patch.column_name);
    }

    if (updated_columns.empty()) continue;
    ...
    for (const auto& patch_result : patch_results) {
      ...
      for (auto& patch_to_apply : patches) {
        if (!patch_to_apply->empty()) patches_to_apply[updated_columns].push_back(std::move(patch_to_apply));
      }
    }
  }

  if (min_version.has_value()) source_data_version = std::max(source_data_version, *min_version);
