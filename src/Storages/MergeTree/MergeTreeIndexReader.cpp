...
static std::unique_ptr<MergeTreeReaderStream> makeIndexReaderStream(const String& stream_name, const String& extension,
                                                                    MergeTreeData::DataPartPtr part, size_t marks_count,
                                                                    const MarkRanges& all_mark_ranges, MarkCache* mark_cache,
  ...
  auto marks_loader = std::make_shared<MergeTreeMarksLoader>(
      std::make_shared<LoadedMergeTreeDataPartInfoForReader>(part, std::make_shared<AlterConversions>()), mark_cache,
      part->index_granularity_info.getMarksFilePath(stream_name), marks_count, part->index_granularity_info, settings.save_marks_in_cache,
      settings.read_settings, load_marks_threadpool,
      /*num_columns_in_mark=*/1);

  marks_loader->startAsyncLoad();

  return std::make_unique<MergeTreeReaderStreamSingleColumn>(part->getDataPartStoragePtr(), stream_name, extension, marks_count,
...
MergeTreeIndexReader::MergeTreeIndexReader(MergeTreeIndexPtr index_, MergeTreeData::DataPartPtr part_, size_t marks_count_,
                                           const MarkRanges& all_mark_ranges_, MarkCache* mark_cache_,
                                           UncompressedCache* uncompressed_cache_,
                                           VectorSimilarityIndexCache* vector_similarity_index_cache_, MergeTreeReaderSettings settings_)
    : index(index_),
      part(std::move(part_)),
...
void MergeTreeIndexReader::initStreamIfNeeded() {
  ...
  for (const auto& substream : index_format.substreams) {
    ...
    auto stream = makeIndexReaderStream(stream_name, substream.extension, part, marks_count, all_mark_ranges, mark_cache,
                                        uncompressed_cache, patchSettings(settings, substream.type));
...
void MergeTreeIndexReader::read(size_t mark, const IMergeTreeIndexCondition* condition, MergeTreeIndexGranulePtr& granule) {
  auto load_func = [this, mark, condition](auto& res) {
    ...
    if (stream_mark != mark) {
      for (const auto& stream : stream_holders) stream->seekToMark(mark);
    }

    if (!res) res = index->createIndexGranule();
...
void MergeTreeIndexReader::read(size_t mark, size_t current_granule_num, MergeTreeIndexBulkGranulesPtr& granules) {
  if (granules == nullptr) granules = index->createIndexBulkGranules();

  initStreamIfNeeded();
  if (streams.size() != 1)
    throw Exception(ErrorCodes::LOGICAL_ERROR,
                    "Bulk filtering is not supported for indexes with multiple streams. Have {} streams for index {}", streams.size(),
                    index->getFileName());

  auto* stream = streams.at(MergeTreeIndexSubstream::Type::Regular);
  if (stream_mark != mark) stream->seekToMark(mark);
...
void MergeTreeIndexReader::adjustRightMark(size_t right_mark) {
  for (const auto& stream : stream_holders) stream->adjustRightMark(right_mark);
}
