...
MarkType::MarkType(bool adaptive_, bool compressed_, bool with_substreams_, MergeTreeDataPartType::Value part_type_)
  ...
  if (part_type == MergeTreeDataPartType::Unknown) throw Exception(ErrorCodes::LOGICAL_ERROR, "Unknown data part type");
}

bool MarkType::isMarkFileExtension(std::string_view extension) { return extension.contains("mrk"); }
...
std::optional<MarkType> MergeTreeIndexGranularityInfo::getMarksTypeFromFilesystem(const IDataPartStorage& data_part_storage) {
  if (data_part_storage.exists())
    for (auto it = data_part_storage.iterate(); it->isValid(); it->next())
      if (std::string ext = fs::path(it->name()).extension(); MarkType::isMarkFileExtension(ext)) return MarkType(ext);
  return {};
}

MergeTreeIndexGranularityInfo::MergeTreeIndexGranularityInfo(const MergeTreeData& storage, const MergeTreeSettings& storage_settings,
                                                             MergeTreeDataPartType type_)
    : MergeTreeIndexGranularityInfo(storage_settings,
                                    {storage.canUseAdaptiveGranularity(), storage_settings[MergeTreeSetting::compress_marks],
                                     storage_settings[MergeTreeSetting::write_marks_for_substreams_in_compact_parts], type_.getValue()}) {}
...
size_t MergeTreeIndexGranularityInfo::getMarkSizeInBytes(size_t columns_num) const {
  ...
  if (mark_type.part_type == MergeTreeDataPartType::Compact) return getAdaptiveMrkSizeCompact(columns_num);
  throw Exception(ErrorCodes::UNKNOWN_PART_TYPE, "Unknown part type");
}

std::string MergeTreeIndexGranularityInfo::describe() const {
  return fmt::format("mark_type: [{}], index_granularity_bytes: {}, fixed_index_granularity: {}", mark_type.describe(),
                     index_granularity_bytes, fixed_index_granularity);
}
