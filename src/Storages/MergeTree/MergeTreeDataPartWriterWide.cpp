...
Granules getGranulesToWrite(const MergeTreeIndexGranularity &index_granularity, size_t block_rows, size_t current_mark,
                            size_t rows_written_in_last_mark) {
  if (current_mark >= index_granularity.getMarksCount())
    throw Exception(ErrorCodes::LOGICAL_ERROR, "Request to get granules from mark {} but index granularity size is {}", current_mark,
...
MergeTreeDataPartWriterWide::MergeTreeDataPartWriterWide(
    const String &data_part_name_, const String &logger_name_, const SerializationByName &serializations_,
    MutableDataPartStoragePtr data_part_storage_, const MergeTreeIndexGranularityInfo &index_granularity_info_,
    const MergeTreeSettingsPtr &storage_settings_, const NamesAndTypesList &columns_list_, const StorageMetadataPtr &metadata_snapshot_,
    const VirtualsDescriptionPtr &virtual_columns_, const std::vector<MergeTreeIndexPtr> &indices_to_recalc_,
    const String &marks_file_extension_, const CompressionCodecPtr &default_codec_, const MergeTreeWriterSettings &settings_,
    MergeTreeIndexGranularityPtr index_granularity_, WrittenOffsetSubstreams *written_offset_substreams_)
    : MergeTreeDataPartWriterOnDisk(data_part_name_, logger_name_, serializations_, data_part_storage_, index_granularity_info_,
                                    storage_settings_, columns_list_, metadata_snapshot_, virtual_columns_, indices_to_recalc_,
                                    marks_file_extension_, default_codec_, settings_, std::move(index_granularity_),
...
void MergeTreeDataPartWriterWide::addStreams(const NameAndTypePair &name_and_type, const ASTPtr &effective_codec_desc) {
  ISerialization::StreamCallback callback = [&](const auto &substream_path) {
    assert(!substream_path.empty());

    /// Don't create streams for ephemeral subcolumns that don't store any real data.
    if (ISerialization::isEphemeralSubcolumn(substream_path, substream_path.size())) return;
    ...
    if (column_streams.contains(stream_name)) return;

    /// Don't write offsets more than one time for Nested type in case elements of nested had been written separately, i.e. via Vertical
    /// merge.
    if (written_offset_substreams) {
      bool is_offsets = !substream_path.empty() && substream_path.back().type == ISerialization::Substream::ArraySizes;
      if (is_offsets && written_offset_substreams->contains(stream_name)) return;
    }
    ...
    const auto column_desc = metadata_snapshot->columns.tryGetColumnDescription(GetColumnsOptions(GetColumnsOptions::AllPhysical),
                                                                                name_and_type.getNameInStorage());

    UInt64 max_compress_block_size = 0;
    if (column_desc)
      if (const auto *value = column_desc->settings.tryGet("max_compress_block_size")) max_compress_block_size = value->safeGet<UInt64>();
    if (!max_compress_block_size) max_compress_block_size = settings.max_compress_block_size;

    WriteSettings query_write_settings = settings.query_write_settings;
    query_write_settings.use_adaptive_write_buffer = (settings.min_columns_to_activate_adaptive_write_buffer &&
    ...
    column_streams[stream_name] = std::make_unique<MergeTreeWriterStream>(
        stream_name, data_part_storage, stream_name, DATA_FILE_EXTENSION, stream_name, marks_file_extension, compression_codec,
        max_compress_block_size, marks_compression_codec, settings.marks_compress_block_size, query_write_settings);
  ...
  auto data =
      ISerialization::SubstreamData(serialization).withType(name_and_type.type).withColumn(sample_column ? sample_column->column : nullptr);
  auto enumerate_settings = getEnumerateSettings(settings);
  serialization->enumerateStreams(enumerate_settings, callback, data);
}

const String &MergeTreeDataPartWriterWide::getStreamName(const NameAndTypePair &column,
                                                         const ISerialization::SubstreamPath &substream_path) const {
  auto full_stream_name =
      ISerialization::getFileNameForStream(column, substream_path, ISerialization::StreamFileNameSettings(*storage_settings));
  String stream_name = replaceFileNameToHashIfNeeded(full_stream_name, *storage_settings, data_part_storage.get());

  if (written_offset_substreams) {
    bool is_offsets = !substream_path.empty() && substream_path.back().type == ISerialization::Substream::ArraySizes;
    /// If it has been written already return an empty string placeholder, to avoid writing it again.
    if (is_offsets && written_offset_substreams->contains(stream_name)) return already_written_stream_holder;
  }

  auto it = full_name_to_stream_name.find(full_stream_name);
  if (it == full_name_to_stream_name.end()) throw Exception(ErrorCodes::LOGICAL_ERROR, "Stream {} not found", full_stream_name);
...
ISerialization::OutputStreamGetter MergeTreeDataPartWriterWide::createStreamGetter(const NameAndTypePair &column,
  ...
  return [&, this](const ISerialization::SubstreamPath &substream_path) -> WriteBuffer * {
    ...
    if (ISerialization::isEphemeralSubcolumn(substream_path, substream_path.size())) return nullptr;
    ...
    bool is_offsets = !substream_path.empty() && substream_path.back().type == ISerialization::Substream::ArraySizes;
    if (is_offsets && offset_substreams.contains(stream_name)) return nullptr;
...
void MergeTreeDataPartWriterWide::write(const Block &block, const IColumnPermutation *permutation) {
  ...
  if (settings.rewrite_primary_key) calculateAndSerializePrimaryIndex(primary_key_block, granules_to_write);
...
void MergeTreeDataPartWriterWide::writeSingleMark(const NameAndTypePair &name_and_type, const WrittenOffsetSubstreams &offset_substreams,
                                                  size_t number_of_rows) {
  StreamsWithMarks marks = getCurrentMarksForColumn(name_and_type, offset_substreams);
  for (const auto &mark : marks) flushMarkToFile(mark, number_of_rows);
}
...
void MergeTreeDataPartWriterWide::flushMarkToFile(const StreamNameAndMark &stream_with_mark, size_t rows_in_mark) {
  ...
  if (settings.can_use_adaptive_granularity) writeBinaryLittleEndian(rows_in_mark, marks_out);

  if (auto it = cached_marks.find(stream_with_mark.stream_name); it != cached_marks.end()) it->second->push_back(stream_with_mark.mark);
}
...
StreamsWithMarks MergeTreeDataPartWriterWide::getCurrentMarksForColumn(const NameAndTypePair &name_and_type,
  ...
  if (column_desc)
    if (const auto *value = column_desc->settings.tryGet("min_compress_block_size")) min_compress_block_size = value->safeGet<UInt64>();
  if (!min_compress_block_size) min_compress_block_size = settings.min_compress_block_size;
  ...
  auto callback = [&](const ISerialization::SubstreamPath &substream_path) {
    ...
    /// Don't write offsets more than one time for Nested type.
    bool is_offsets = !substream_path.empty() && substream_path.back().type == ISerialization::Substream::ArraySizes;
    if (is_offsets && offset_substreams.contains(stream_name)) return;

    auto &stream = *column_streams[stream_name];

    /// There could already be enough data to compress into the new block.
    if (stream.compressed_hashing.offset() >= min_compress_block_size) stream.compressed_hashing.next();
...
void MergeTreeDataPartWriterWide::writeSingleGranule(const NameAndTypePair &name_and_type, const IColumn &column,
                                                     const WrittenOffsetSubstreams &offset_substreams,
  ...
  serialization->serializeBinaryBulkWithMultipleStreams(column, granule.start_row, granule.rows_to_write, serialize_settings,
                                                        serialization_state);
...
ISerialization::SerializeBinaryBulkSettings MergeTreeDataPartWriterWide::getSerializationSettings() const {
  ...
  serialize_settings.object_and_dynamic_write_statistics =
      ISerialization::SerializeBinaryBulkSettings::ObjectAndDynamicStatisticsMode::SUFFIX;
  return serialize_settings;
}

/// Column must not be empty. (column.size() !== 0)
void MergeTreeDataPartWriterWide::writeColumn(const NameAndTypePair &name_and_type, const IColumn &column,
                                              WrittenOffsetSubstreams &offset_substreams, const Granules &granules) {
  if (granules.empty())
    throw Exception(ErrorCodes::LOGICAL_ERROR, "Empty granules for column {}, current mark {}", backQuoteIfNeed(name_and_type.name),
                    getCurrentMark());
  ...
  for (const auto &granule : granules) {
    ...
    writeSingleGranule(name_and_type, column, offset_substreams, it->second, serialize_settings, granule);
    ...
    if (granule.is_complete) {
      ...
      for (const auto &mark : marks_it->second) flushMarkToFile(mark, index_granularity->getMarkRows(granule.mark_number));
  ...
  auto callback = [&](const ISerialization::SubstreamPath &substream_path) {
    bool is_offsets = !substream_path.empty() && substream_path.back().type == ISerialization::Substream::ArraySizes;
    if (is_offsets) offset_substreams.insert(getStreamName(name_and_type, substream_path));
  };
  auto data = ISerialization::SubstreamData(serialization)
                  .withType(name_and_type.type)
                  .withColumn(block_sample.getByName(name_and_type.name).column);
...
void MergeTreeDataPartWriterWide::validateColumnOfFixedSize(const NameAndTypePair &name_type) {
  ...
  if (!type->isValueRepresentedByNumber() || type->haveSubtypes() ||
      serialization->getKindStack() != ISerialization::KindStack{ISerialization::Kind::DEFAULT})
  ...
  if (!getDataPartStorage().existsFile(mrk_path)) return;
  ...
  for (mark_num = 0; !mrk_in->eof(); ++mark_num) {
    if (mark_num > index_granularity->getMarksCount())
      throw Exception(ErrorCodes::LOGICAL_ERROR, "Incorrect number of marks in memory {}, on disk (at least) {}",
    ...
    if (must_be_last) {
      if (index_granularity_rows != 0)
        throw Exception(ErrorCodes::LOGICAL_ERROR, "We ran out of binary data but still have non empty mark #{} with rows number {}",
                        mark_num, index_granularity_rows);

      if (!mrk_in->eof()) throw Exception(ErrorCodes::LOGICAL_ERROR, "Mark #{} must be last, but we still have some to read", mark_num);
    ...
    if (index_granularity_rows != index_granularity->getMarkRows(mark_num)) {
      throw Exception(ErrorCodes::LOGICAL_ERROR,
                      ...
                      getDataPartStorage().getFullPath(), mark_num, offset_in_compressed_file, offset_in_decompressed_block,
                      index_granularity->getMarkRows(mark_num), index_granularity_rows, index_granularity->getMarksCount());
    }
    ...
    if (column->size() != index_granularity_rows) {
      if (must_be_last) {
        ...
        if (!settings.can_use_adaptive_granularity) break;
        ...
        if (!compute_granularity) continue;
      }

      throw Exception(ErrorCodes::LOGICAL_ERROR,
                      "Incorrect mark rows for mark #{} (compressed offset {}, decompressed offset {}), "
...
void MergeTreeDataPartWriterWide::finalizeIndexGranularity() {
  ...
  if (rows_written_in_last_mark > 0) {
    if (settings.can_use_adaptive_granularity && settings.blocks_are_granules_size)
      throw Exception(ErrorCodes::LOGICAL_ERROR,
                      ...
                      getCurrentMark(), index_granularity->getMarkRows(getCurrentMark()), rows_written_in_last_mark,
                      index_granularity->getMarksCount());
  ...
  bool write_final_mark = (with_final_mark && data_written);
  {
    ...
    for (size_t i = 0; i < columns_list.size(); ++i, ++it) {
      ...
      if (write_final_mark) writeFinalMark(*it, offset_substreams);
    }
  }
}
...
void MergeTreeDataPartWriterWide::fillChecksums(MergeTreeDataPartChecksums &checksums, NameSet &checksums_to_remove) {
  ...
  if (!columns_list.empty()) fillDataChecksums(checksums, checksums_to_remove);

  if (settings.rewrite_primary_key) fillPrimaryIndexChecksums(checksums);
...
void MergeTreeDataPartWriterWide::finish(bool sync) {
  // If we don't have anything to write, skip finalization.
  if (!columns_list.empty()) finishDataSerialization(sync);

  if (settings.rewrite_primary_key) finishPrimaryIndexSerialization(sync);

  finishSkipIndicesSerialization(sync);
}

void MergeTreeDataPartWriterWide::cancel() noexcept {
  for (auto &stream : column_streams) stream.second->cancel();
...
void MergeTreeDataPartWriterWide::writeFinalMark(const NameAndTypePair &name_and_type, WrittenOffsetSubstreams &offset_substreams) {
...
static void fillIndexGranularityImpl(MergeTreeIndexGranularity &index_granularity, size_t index_offset, size_t index_granularity_for_block,
...
void MergeTreeDataPartWriterWide::fillIndexGranularity(size_t index_granularity_for_block, size_t rows_in_block) {
  if (getCurrentMark() < index_granularity->getMarksCount() && getCurrentMark() != index_granularity->getMarksCount() - 1)
    throw Exception(ErrorCodes::LOGICAL_ERROR, "Trying to add marks, while current mark {}, but total marks {}", getCurrentMark(),
                    index_granularity->getMarksCount());

  size_t index_offset = 0;
  if (rows_written_in_last_mark != 0) index_offset = index_granularity->getLastMarkRows() - rows_written_in_last_mark;
...
void MergeTreeDataPartWriterWide::adjustLastMarkIfNeedAndFlushToDisk(size_t new_rows_in_last_mark) {
  ...
  if (rows_written_in_last_mark > new_rows_in_last_mark)
    throw Exception(ErrorCodes::LOGICAL_ERROR, "Tryin to make mark #{} smaller ({} rows) then it already has {}", getCurrentMark(),
                    new_rows_in_last_mark, rows_written_in_last_mark);
  ...
  if (compute_granularity && settings.can_use_adaptive_granularity) {
    if (getCurrentMark() != index_granularity->getMarksCount() - 1)
      throw Exception(ErrorCodes::LOGICAL_ERROR, "Non last mark {} (with {} rows) having rows offset {}, total marks {}", getCurrentMark(),
                      index_granularity->getMarkRows(getCurrentMark()), rows_written_in_last_mark, index_granularity->getMarksCount());
  ...
  if (last_non_written_marks.empty())
    throw Exception(ErrorCodes::LOGICAL_ERROR, "No saved marks for last mark {} having rows offset {}, total marks {}", getCurrentMark(),
                    rows_written_in_last_mark, index_granularity->getMarksCount());

  if (rows_written_in_last_mark == new_rows_in_last_mark) {
    for (const auto &[name, marks] : last_non_written_marks) {
      for (const auto &mark : marks) flushMarkToFile(mark, index_granularity->getMarkRows(getCurrentMark()));
    }
    ...
    if (compute_granularity && settings.can_use_adaptive_granularity) {
      ...
      for (size_t i = 0; i < skip_indices.size(); ++i) ++skip_index_accumulated_marks[i];
...
void MergeTreeDataPartWriterWide::initColumnsSubstreamsIfNeeded(const Block &block) {
  if (columns_substreams.getTotalSubstreams()) return;
