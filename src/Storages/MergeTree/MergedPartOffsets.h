...
class PackedPartOffsets {
 private:
  ...
  struct Page {
   public:
    ...
    explicit Page(UInt64 val) : num_vals(1), min_val(val), bits_per_val(0), compressed_data(nullptr) {}
    ...
    Page(const PODArray<UInt64> &vals, size_t bits_per_val_, UInt64 *compressed_data_)
        : num_vals(vals.size()), min_val(vals.front()), bits_per_val(bits_per_val_), compressed_data(compressed_data_) {
      size_t pos = 0;
      size_t offset = 0;
      ...
      for (size_t i = 1; i < num_vals; ++i) {
        ...
        if (offset == 0) {
          ...
          if (offset + bits_per_val > 64) compressed_data[pos + 1] = val >> (64 - offset);
        }
    ...
    UInt64 operator[](size_t i) const {
      ...
      if (offset + bits_per_val > 64) value |= compressed_data[pos + 1] << (64 - offset);
 ...
 public:
  ...
  void insert(UInt64 val) {
    if (current_page_values.size() >= PACKED_PAGE_SIZE) flush();
  ...
  void flush() {
    if (current_page_values.empty()) return;
    ...
    pages.emplace_back(current_page_values, bits_per_val,
...
class MergedPartOffsets {
 public:
  ...
  explicit MergedPartOffsets(size_t num_parts, MappingMode mode_ = MappingMode::Enabled)
      : mode(mode_), offset_maps(mode == MappingMode::Enabled ? num_parts : 0), finalized(mode == MappingMode::Disabled) {}
  ...
  void flush() {
    if (mode == MappingMode::Disabled) return;
    ...
    LOG_DEBUG(logger, "Holding {} merged _part_offset in memory with {} total allocated memory", num_rows,
