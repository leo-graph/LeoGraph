...
BackgroundJobsAssignee::BackgroundJobsAssignee(IBackgroundOperation& data_, const StorageID& storage_id_,
                                               BackgroundJobsAssignee::Type type_, ContextPtr global_context_)
    : WithContext(global_context_), type(type_), data(data_), storage_id(storage_id_), rng(randomSeed()), sleep_settings(getSettings()) {}
...
void BackgroundJobsAssignee::postpone() {
  ...
  size_t next_time_to_execute =
      static_cast<size_t>(1000 * (std::min(sleep_settings.task_sleep_seconds_when_no_work_max,
                                           sleep_settings.thread_sleep_seconds_if_nothing_to_do *
                                               std::pow(sleep_settings.task_sleep_seconds_when_no_work_multiplier, no_work_done_count)) +
                                  random_addition));
...
void BackgroundJobsAssignee::updateStorageID(const StorageID& new_id) { storage_id = new_id; }
...
void BackgroundJobsAssignee::threadFunc() try {
  bool succeed = false;
  switch (type) {
    case Type::DataProcessing:
