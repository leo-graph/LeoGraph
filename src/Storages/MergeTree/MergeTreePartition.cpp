...
class LegacyFieldVisitorHash : public StaticVisitor<> {
 ...
 public:
  ...
  void operator()(const UUID &x) const { operator()(x.toUnderType()); }
  void operator()(const IPv4 &x) const {
    UInt8 type = Field::Types::IPv4;
    hash.update(type);
    hash.update(x);
  }
  void operator()(const IPv6 &x) const { operator()(String(reinterpret_cast<const char *>(&x), 16)); }
  ...
  void operator()(const Array &x) const {
    ...
    for (const auto &elem : x) applyVisitor(*this, elem);
  }
...
String MergeTreePartition::getID(const Block &partition_key_sample) const {
  ...
  if (value.empty()) return "all";  /// It is tempting to use an empty string here. But that would break directory structure in ZK.
  ...
  if (are_all_integral) {
    ...
    for (size_t i = 0; i < value.size(); ++i) {
      ...
      if (typeid_cast<const DataTypeDate *>(partition_key_sample.getByPosition(i).type.get()))
        result += toString(
            DateLUT::serverTimezoneInstance().toNumYYYYMMDD(DayNum(static_cast<DayNum::UnderlyingType>(value[i].safeGet<UInt64>()))));
  ...
  for (const Field &field : value) applyVisitor(hashing_visitor, field);
...
std::optional<Row> MergeTreePartition::tryParseValueFromID(const String &partition_id, const Block &partition_key_sample) {
  ...
  for (size_t i = 0; i < num_keys; ++i) {
    ...
    if (i + 1 != num_keys) assertChar('-', buf);
  }
  ...
  if (expected_partition_id != partition_id)
    throw Exception(ErrorCodes::LOGICAL_ERROR, "Partition ID was parsed incorrectly: expected {}, got {}", expected_partition_id,
                    partition_id);

  return res;
}

void MergeTreePartition::serializeText(StorageMetadataPtr metadata_snapshot, WriteBuffer &out,
...
void MergeTreePartition::load(const IMergeTreeDataPart &part) {
  ...
  if (!metadata_snapshot->hasPartitionKey()) return;
...
std::unique_ptr<WriteBufferFromFileBase> MergeTreePartition::store(StorageMetadataPtr metadata_snapshot, ContextPtr storage_context,
...
std::unique_ptr<WriteBufferFromFileBase> MergeTreePartition::store(const Block &partition_key_sample, IDataPartStorage &data_part_storage,
                                                                   MergeTreeDataPartChecksums &checksums,
                                                                   const WriteSettings &settings) const {
  if (partition_key_sample.empty()) return nullptr;
...
NamesAndTypesList MergeTreePartition::executePartitionByExpression(const StorageMetadataPtr &metadata_snapshot, Block &block,
                                                                   ContextPtr context) {
...
KeyDescription MergeTreePartition::adjustPartitionKey(const StorageMetadataPtr &metadata_snapshot, ContextPtr context) {
  ...
  if (!partition_key.definition_ast) return partition_key;

  ASTPtr ast_copy = partition_key.definition_ast->clone();

  /// Implementation of modulo function was changed from 8bit result type to 16bit. For backward compatibility partition by expression is
  /// always calculated according to previous version - `moduloLegacy`.
