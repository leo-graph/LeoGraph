...
static Int64 findMinPosition(const NameSet &condition_table_columns, const NameToIndexMap &primary_key_positions) {
  ...
  for (const auto &column : condition_table_columns) {
    ...
    if (it != primary_key_positions.end()) min_position = std::min(min_position, static_cast<Int64>(it->second));
  }
...
static NameSet getTableColumns(const StorageSnapshotPtr &storage_snapshot, const Names &queried_columns) {
  ...
  for (const auto &column : columns_list) table_columns.insert(column.name);
  ...
  for (const auto &column : queried_columns) {
    if (storage_columns.hasSubcolumn(options.kind, column)) table_columns.insert(column);
  }

  return table_columns;
}

MergeTreeWhereOptimizer::MergeTreeWhereOptimizer(std::unordered_map<std::string, UInt64> column_sizes_,
                                                 const StorageSnapshotPtr &storage_snapshot, ConditionSelectivityEstimatorPtr estimator_,
                                                 const Names &queried_columns_, const std::optional<NameSet> &supported_columns_,
                                                 LoggerPtr log_)
    : estimator(estimator_),
      table_columns(getTableColumns(storage_snapshot, queried_columns_)),
      queried_columns{queried_columns_},
      supported_columns{supported_columns_},
      sorting_key_names{NameSet(storage_snapshot->metadata->getSortingKey().column_names.begin(),
                                storage_snapshot->metadata->getSortingKey().column_names.end())},
  ...
  for (const auto &name : queried_columns) {
    ...
    if (it != column_sizes.end()) total_size_of_queried_columns += it->second;
  }
}
...
void MergeTreeWhereOptimizer::optimize(SelectQueryInfo &select_query_info, const ContextPtr &context) const {
  ...
  if (!select.where() || select.prewhere()) return;

  auto block_with_constants =
      KeyCondition::getBlockWithConstants(select_query_info.query->clone(), select_query_info.syntax_analyzer_result, context);
  ...
  LOG_DEBUG(log, "MergeTreeWhereOptimizer: condition \"{}\" moved to PREWHERE",
...
MergeTreeWhereOptimizer::FilterActionsOptimizeResult MergeTreeWhereOptimizer::optimize(const ActionsDAG &filter_dag,
                                                                                       ...
                                                                                       const ContextPtr &context, bool is_final) {
  WhereOptimizerContext where_optimizer_context;
  where_optimizer_context.context = context;
  ...
  for (const auto &condition : optimize_result->prewhere_conditions) {
    ...
    if (prewhere_conditions.insert(condition_node).second) prewhere_conditions_list.push_back(condition_node);
  }

  return {.prewhere_nodes = std::move(prewhere_conditions),
          .prewhere_nodes_list = std::move(prewhere_conditions_list),
          .fully_moved_to_prewhere = optimize_result->where_conditions.empty()};
}

static void collectColumns(const RPNBuilderTreeNode &node, const RPNBuilderTreeNode *parent, const NameSet &columns_names,
                           NameSet &result_set, bool &has_invalid_column, bool &may_use_primary_index) {
  if (node.isConstant()) return;

  if (node.isSubqueryOrSet()) return;
  ...
  if (function_name == "indexHint") return;
  ...
  if (!KeyCondition::atom_map.contains(function_name)) {
    if (!parent) {
      ...
      if (arguments_size > 2 || arguments_size == 0 ||
          (function_base && (!function_base->isDeterministicInScopeOfQuery() || !function_base->hasInformationAboutMonotonicity()))) {
        may_use_primary_index = false;
      }
    }
  }
...
static bool isConditionGood(const RPNBuilderTreeNode &condition, const NameSet &columns_names) {
  if (!condition.isFunction()) return false;
   ...
   */
  if (function_node.getFunctionName() != "equals" || function_node.getArgumentsSize() != 2) return false;
  ...
  if (!constant_node->tryGetConstant(output_value, output_type)) return false;
...
void MergeTreeWhereOptimizer::analyzeImpl(Conditions &res, const RPNBuilderTreeNode &node,
                                          const WhereOptimizerContext &where_optimizer_context, std::set<Int64> &pk_positions) const {
  ...
  if (function_node_optional.has_value() && function_node_optional->getFunctionName() == "and") {
    ...
    /// Is it possible to use primary index for this condition? For conditions like `lower(country) = 'xx'`, may_use_primary_index will be
    /// false.`
    ...
    cond.viable = !has_invalid_column
    ...
    if (cond.viable) cond.good = isConditionGood(node, table_columns);
...
MergeTreeWhereOptimizer::Conditions MergeTreeWhereOptimizer::analyze(const RPNBuilderTreeNode &node,
  ...
  if (where_optimizer_context.move_primary_key_columns_to_end_of_prewhere) {
    ...
    for (auto pk_pos : pk_positions) {
      if (pk_pos != min_valid_pk_pos + 1) break;
      min_valid_pk_pos = pk_pos;
    }
    for (auto &cond : res) {
      if (cond.min_position_in_primary_key > min_valid_pk_pos) cond.min_position_in_primary_key = std::numeric_limits<Int64>::max() - 1;
    }
...
ASTPtr MergeTreeWhereOptimizer::reconstructAST(const Conditions &conditions) {
  ...
  if (conditions.size() == 1) return conditions.front().node.getASTNode()->clone();
  ...
  for (const auto &elem : conditions) function->arguments->children.push_back(elem.node.getASTNode()->clone());

  return function;
}

std::optional<MergeTreeWhereOptimizer::OptimizeResult> MergeTreeWhereOptimizer::optimizeImpl(
  ...
  size_t position = 0;
  for (const auto &condition : where_conditions) condition_positions[&condition] = position++;
  ...
  auto move_to_prewhere_conditions = [&](Conditions::iterator cond_it) {
    ...
    if (where_optimizer_context.allow_reorder_prewhere_conditions) {
      ...
      while (condition_positions[&(*prewhere_it)] < position && prewhere_it != prewhere_conditions.end()) ++prewhere_it;
  ...
  while (!where_conditions.empty()) {
    ...
    if (!where_optimizer_context.move_all_conditions_to_prewhere) {
      ...
      if (total_size_of_queried_columns > 0) {
        ...
        moved_enough =
            total_size_of_moved_conditions > 0 && (total_size_of_moved_conditions + it->columns_size) * 10 > total_size_of_queried_columns;
      } else {
        ...
        moved_enough =
            total_number_of_moved_columns > 0 && (total_number_of_moved_columns + it->table_columns.size()) * 4 > queried_columns.size();
      }
  ...
  /// Nothing was moved.
  if (prewhere_conditions.empty()) return {};
...
UInt64 MergeTreeWhereOptimizer::getColumnsSize(const NameSet &columns) const {
  ...
  for (const auto &column : columns)
    if (column_sizes.contains(column)) size += column_sizes.at(column);

  return size;
}

bool MergeTreeWhereOptimizer::columnsSupportPrewhere(const NameSet &columns) const {
  if (!supported_columns.has_value()) return true;

  for (const auto &column : columns)
    if (!supported_columns->contains(column)) return false;
...
bool MergeTreeWhereOptimizer::isExpressionOverSortingKey(const RPNBuilderTreeNode &node) const {
  if (node.isFunction()) {
    ...
    for (size_t i = 0; i < arguments_size; ++i) {
      ...
      if (argument.isConstant() || sorting_key_names.contains(argument_column_name)) continue;

      if (!isExpressionOverSortingKey(argument)) return false;
    }
...
bool MergeTreeWhereOptimizer::isSortingKey(const String &column_name) const { return sorting_key_names.contains(column_name); }

bool MergeTreeWhereOptimizer::isSubsetOfTableColumns(const NameSet &columns) const {
  for (const auto &column : columns)
    if (!table_columns.contains(column)) return false;
...
bool MergeTreeWhereOptimizer::cannotBeMoved(const RPNBuilderTreeNode &node, const WhereOptimizerContext &where_optimizer_context) const {
  if (node.isFunction()) {
    ...
    if (function_name == "arrayJoin") return true;

    size_t arguments_size = function_node.getArgumentsSize();
    for (size_t i = 0; i < arguments_size; ++i) {
      auto argument = function_node.getArgumentAt(i);
      if (cannotBeMoved(argument, where_optimizer_context)) return true;
    }
  } else {
    auto column_name = node.getColumnName();
...
NameSet MergeTreeWhereOptimizer::determineArrayJoinedNames(const ASTSelectQuery &select) {
  ...
  for (const auto &ast : array_join_expression_list->children) array_joined_names.emplace(ast->getAliasOrColumnName());
