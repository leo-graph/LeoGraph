...
MergeTreeDataPartCompact::MergeTreeDataPartCompact(const MergeTreeData& storage_, const MergeTreeSettings& storage_settings,
                                                   const String& name_, const MergeTreePartInfo& info_,
...
MergeTreeReaderPtr createMergeTreeReaderCompact(const MergeTreeDataPartInfoForReaderPtr& read_info,
  ...
  return std::make_unique<MergeTreeReaderCompactSingleBuffer>(
      read_info, columns_to_read, virtual_fields, storage_snapshot, storage_settings, uncompressed_cache, mark_cache,
      deserialization_prefixes_cache, mark_ranges, reader_settings, avg_value_size_hints, profile_callback, CLOCK_MONOTONIC_COARSE);
}
...
MergeTreeDataPartWriterPtr createMergeTreeDataPartCompactWriter(
    ...
    const MergeTreeSettingsPtr& storage_settings_, const NamesAndTypesList& columns_list, const ColumnPositions& column_positions,
    const StorageMetadataPtr& metadata_snapshot, const VirtualsDescriptionPtr& virtual_columns,
    const std::vector<MergeTreeIndexPtr>& indices_to_recalc, const String& marks_file_extension_, const CompressionCodecPtr& default_codec_,
    const MergeTreeWriterSettings& writer_settings, MergeTreeIndexGranularityPtr computed_index_granularity) {
  ...
  ordered_columns_list.sort(
      [&column_positions](const auto& lhs, const auto& rhs) { return column_positions.at(lhs.name) < column_positions.at(rhs.name); });

  return std::make_unique<MergeTreeDataPartWriterCompact>(data_part_name_, logger_name_, serializations_, data_part_storage_,
                                                          index_granularity_info_, storage_settings_, ordered_columns_list,
                                                          metadata_snapshot, virtual_columns, indices_to_recalc, marks_file_extension_,
...
void MergeTreeDataPartCompact::calculateEachColumnSizes(ColumnSizeByName& /*each_columns_size*/, ColumnSize& total_size) const {
  ...
  if (mrk_checksum != checksums.files.end()) total_size.marks += mrk_checksum->second.file_size;
}

void MergeTreeDataPartCompact::loadIndexGranularityImpl(MergeTreeIndexGranularityPtr& index_granularity_ptr,
                                                        const MergeTreeIndexGranularityInfo& index_granularity_info_,
                                                        size_t marks_per_granule, const IDataPartStorage& data_part_storage_,
...
void MergeTreeDataPartCompact::loadIndexGranularity() {
  ...
  loadIndexGranularityImpl(index_granularity, index_granularity_info,
                           index_granularity_info.mark_type.with_substreams ? columns_substreams.getTotalSubstreams() : columns.size(),
                           getDataPartStorage(), *storage.getSettings());
}
...
void MergeTreeDataPartCompact::loadMarksToCache(const Names& column_names, MarkCache* mark_cache) const {
  ...
  MergeTreeMarksLoader loader(info_for_read, mark_cache, index_granularity_info.getMarksFilePath(DATA_FILE_NAME),
                              index_granularity->getMarksCount(), index_granularity_info,
                              /*save_marks_in_cache=*/true, context->getReadSettings(),
...
bool MergeTreeDataPartCompact::hasColumnFiles(const NameAndTypePair& column) const {
  if (!getColumnPosition(column.getNameInStorage())) return false;
  ...
  if (!checksums.empty()) {
    ...
    if (require_part_metadata) {
      if (!checksums.files.contains(mrk_file_name))
        throw Exception(ErrorCodes::NO_FILE_IN_DATA_PART, "No marks file checksum for column in part {}",
                        getDataPartStorage().getFullPath());
      if (!checksums.files.contains(DATA_FILE_NAME_WITH_EXTENSION))
        throw Exception(ErrorCodes::NO_FILE_IN_DATA_PART, "No data file checksum for in part {}", getDataPartStorage().getFullPath());
    }
  } else {
    {
      /// count.txt should be present even in non custom-partitioned parts
      ...
      if (!getDataPartStorage().existsFile(file_path) || getDataPartStorage().getFileSize(file_path) == 0)
        throw Exception(ErrorCodes::BAD_SIZE_OF_FILE_IN_DATA_PART, "Part {} is broken: {} is empty", getDataPartStorage().getRelativePath(),
    ...
    if (getDataPartStorage().existsFile(mrk_file_name)) {
      ...
      if (!file_size)
        throw Exception(ErrorCodes::BAD_SIZE_OF_FILE_IN_DATA_PART, "Part {} is broken: {} is empty.",
      ...
      if (expected_file_size != file_size)
        throw Exception(ErrorCodes::BAD_SIZE_OF_FILE_IN_DATA_PART, "Part {} is broken: bad size of marks file '{}': {}, must be: {}",
                        getDataPartStorage().getRelativePath(), std::string(fs::path(getDataPartStorage().getFullPath()) / mrk_file_name),
...
bool MergeTreeDataPartCompact::isStoredOnRemoteDisk() const { return getDataPartStorage().isStoredOnRemoteDisk(); }

bool MergeTreeDataPartCompact::isStoredOnReadonlyDisk() const { return getDataPartStorage().isReadonly(); }

bool MergeTreeDataPartCompact::isStoredOnRemoteDiskWithZeroCopySupport() const { return getDataPartStorage().supportZeroCopyReplication(); }
