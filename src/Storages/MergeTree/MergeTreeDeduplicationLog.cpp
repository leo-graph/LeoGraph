...
MergeTreeDeduplicationLog::MergeTreeDeduplicationLog(const std::string &logs_dir_, size_t deduplication_window_,
                                                     const MergeTreeDataFormatVersion &format_version_, DiskPtr disk_)
    : logs_dir(logs_dir_),
  ...
  if (deduplication_window != 0 && !disk->existsDirectory(logs_dir)) disk->createDirectories(logs_dir);
}
...
void MergeTreeDeduplicationLog::load() {
  if (!disk->existsDirectory(logs_dir)) {
    if (auto *object_storage = dynamic_cast<DiskObjectStorage *>(disk.get())) {
      ...
      if (object_storage->getMetadataStorage()->getType() != MetadataStorageType::Plain) return;
    }
  }
  ...
  if (!existing_logs.empty()) current_log_number = existing_logs.rbegin()->first;
  ...
  if (deduplication_window != 0) {
    ...
    if (!current_writer) current_writer = disk->writeFile(existing_logs.rbegin()->second.path, DBMS_DEFAULT_BUFFER_SIZE, WriteMode::Append);
  }
}
...
void MergeTreeDeduplicationLog::rotate() {
  ...
  if (deduplication_window == 0) return;
  ...
  try {
    ...
    tryLogCurrentException(__PRETTY_FUNCTION__, "Error while writing MergeTree deduplication log on path " +
                                                    existing_logs[current_log_number].path +
                                                    ", lost recods: " + DB::toString(existing_logs[current_log_number].entries_count));
    if (current_writer) current_writer->cancel();
...
void MergeTreeDeduplicationLog::dropOutdatedLogs() {
  ...
  if (remove_from_value != 0) {
    ...
    for (auto itr = existing_logs.begin(); itr != existing_logs.end();) {
      ...
      if (remove_from_value == number) break;
    }
  }
}
...
std::vector<MergeTreeDeduplicationLog::AddPartResult> MergeTreeDeduplicationLog::addPart(const std::vector<std::string> &block_ids,
...
MergeTreeDeduplicationLog::~MergeTreeDeduplicationLog() { shutdown(); }
