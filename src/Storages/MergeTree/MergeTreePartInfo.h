...
struct MergeTreePartInfo {
 public:
  ...
  static Kind getKind(const String& partition_id) { return partition_id.starts_with(PATCH_PART_PREFIX) ? Kind::Patch : Kind::Regular; }
 ...
 public:
  ...
  bool contains(const MergeTreePartInfo& rhs) const {
    ...
    bool strictly_contains_block_range =
        (min_block == rhs.min_block && max_block == rhs.max_block) || level > rhs.level || level == MAX_LEVEL || level == LEGACY_MAX_LEVEL;

    return partition_id == rhs.getPartitionId()  /// Parts for different partitions are not merged
           && min_block <= rhs.min_block && max_block >= rhs.max_block && level >= rhs.level && mutation >= rhs.mutation &&
  ...
  bool isMutationChildOf(const MergeTreePartInfo& parent_candidate) const {
    return partition_id == parent_candidate.getPartitionId() && min_block == parent_candidate.min_block &&
           max_block == parent_candidate.max_block && level == parent_candidate.level && mutation >= parent_candidate.mutation;
  }

  /// Return part mutation version, if part wasn't mutated return zero
  Int64 getMutationVersion() const { return mutation; }

  /// True if parts do not intersect in any way.
  bool isDisjoint(const MergeTreePartInfo& rhs) const {
    return partition_id != rhs.getPartitionId() || min_block > rhs.max_block || max_block < rhs.min_block;
  }
  ...
  UInt64 getBlocksCount() const { return static_cast<UInt64>(max_block - min_block + 1); }
  ...
  static std::optional<MergeTreePartInfo> tryParsePartName(std::string_view part_name, MergeTreeDataFormatVersion format_version);
