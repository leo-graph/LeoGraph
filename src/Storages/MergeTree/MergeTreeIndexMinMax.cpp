...
MergeTreeIndexGranuleMinMax::MergeTreeIndexGranuleMinMax(const String &index_name_, const Block &index_sample_block_,
    ...
    : index_name(index_name_), index_sample_block(index_sample_block_), hyperrectangle(std::move(hyperrectangle_)) {
  for (size_t i = 0; i < index_sample_block.columns(); ++i) {
    const DataTypePtr &type = index_sample_block.getByPosition(i).type;
    serializations.push_back(type->getDefaultSerialization());
  }
...
void MergeTreeIndexGranuleMinMax::serializeBinary(WriteBuffer &ostr) const {
  if (empty()) throw Exception(ErrorCodes::LOGICAL_ERROR, "Attempt to write empty minmax index {}", backQuote(index_name));
...
void MergeTreeIndexAggregatorMinMax::update(const Block &block, size_t *pos, size_t limit) {
  ...
  for (size_t i = 0; i < index_sample_block.columns(); ++i) {
    ...
    if (hyperrectangle.size() <= i) {
      ...
      hyperrectangle[i].left = accurateLess(hyperrectangle[i].left, field_min) ? hyperrectangle[i].left : field_min;
      hyperrectangle[i].right = accurateLess(hyperrectangle[i].right, field_max) ? field_max : hyperrectangle[i].right;
    }
  }
...
MergeTreeIndexConditionMinMax::MergeTreeIndexConditionMinMax(const IndexDescription &index,
                                                             const ActionsDAGWithInversionPushDown &filter_dag, ContextPtr context)
    : index_data_types(index.data_types), condition(buildCondition(index, filter_dag, context)) {}

bool MergeTreeIndexConditionMinMax::alwaysUnknownOrTrue() const {
  return rpnEvaluatesAlwaysUnknownOrTrue(
      condition.getRPN(),
      {KeyCondition::RPNElement::FUNCTION_NOT_IN_RANGE, KeyCondition::RPNElement::FUNCTION_IN_RANGE,
       KeyCondition::RPNElement::FUNCTION_IN_SET, KeyCondition::RPNElement::FUNCTION_NOT_IN_SET,
       KeyCondition::RPNElement::FUNCTION_ARGS_IN_HYPERRECTANGLE, KeyCondition::RPNElement::FUNCTION_POINT_IN_POLYGON,
       KeyCondition::RPNElement::FUNCTION_IS_NULL, KeyCondition::RPNElement::FUNCTION_IS_NOT_NULL, KeyCondition::RPNElement::ALWAYS_FALSE});
}

bool MergeTreeIndexConditionMinMax::mayBeTrueOnGranule(MergeTreeIndexGranulePtr idx_granule,
...
std::string MergeTreeIndexConditionMinMax::getDescription() const { return condition.getDescription().condition; }
...
MergeTreeIndexConditionPtr MergeTreeIndexMinMax::createIndexCondition(const ActionsDAG::Node *predicate, ContextPtr context) const {
...
MergeTreeIndexFormat MergeTreeIndexMinMax::getDeserializedFormat(const MergeTreeDataPartChecksums &checksums,
...
MergeTreeIndexBulkGranulesMinMax::MergeTreeIndexBulkGranulesMinMax(const String &index_name_, const Block &index_sample_block_,
                                                                   size_t index_granularity_, int direction_, size_t size_hint_,
                                                                   size_t last_part_granule_, bool store_map_)
...
void MergeTreeIndexBulkGranulesMinMax::deserializeBinary(size_t granule_num, ReadBuffer &istr, MergeTreeIndexVersion /*version*/) {
  ...
  for (size_t i = 0; i < index_granularity; ++i) {
    ...
    if (part_granule_num >= last_part_granule) break;

    granules.emplace_back(MinMaxGranule{part_granule_num, value});
    if (store_map) granules_map.emplace(part_granule_num, granules.size() - 1);
  }
  empty = false;
}
...
template <bool handle_ties>
void MergeTreeIndexBulkGranulesMinMax::getTopKMarks(int direction, size_t n, size_t index_granularity,
  ...
  for (auto &part_ranges : result) std::sort(part_ranges.begin(), part_ranges.end());
}

void MergeTreeIndexBulkGranulesMinMax::getTopKMarks(int direction, size_t n, size_t index_granularity, bool handle_ties,
                                                    const std::vector<std::vector<MinMaxGranule>> &parts, std::vector<MarkRanges> &result) {
  if (handle_ties)
...
MergeTreeIndexPtr minmaxIndexCreator(const IndexDescription &index) { return std::make_shared<MergeTreeIndexMinMax>(index); }
