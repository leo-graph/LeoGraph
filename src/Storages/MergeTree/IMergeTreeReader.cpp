...
IMergeTreeReader::IMergeTreeReader(MergeTreeDataPartInfoForReaderPtr data_part_info_for_read_, const NamesAndTypesList &columns_,
                                   ...
                                   MarkCache *mark_cache_, const MarkRanges &all_mark_ranges_, const MergeTreeReaderSettings &settings_,
                                   const ValueSizeMap &avg_value_size_hints_)
    : data_part_info_for_read(data_part_info_for_read_),
      avg_value_size_hints(avg_value_size_hints_),
      part_columns(data_part_info_for_read->isWidePart() ? data_part_info_for_read->getColumnsDescriptionWithCollectedNested()
...
const ValueSizeMap &IMergeTreeReader::getAvgValueSizeHints() const { return avg_value_size_hints; }
...
void IMergeTreeReader::fillVirtualColumns(Columns &columns, size_t rows) const {
  ...
  for (size_t pos = 0; pos < columns.size(); ++pos, ++it) {
    if (columns[pos] || storage_columns.has(it->name)) continue;

    auto virtual_column = virtual_columns->tryGet(it->name);
    if (!virtual_column) continue;

    if (!it->type->equals(*virtual_column->type)) {
      throw Exception(ErrorCodes::LOGICAL_ERROR, "Data type for virtual column {} mismatched. Requested type: {}, virtual column type: {}",
    ...
    if (isTextIndexVirtualColumn(it->name)) continue;
...
void IMergeTreeReader::fillMissingColumns(Columns &res_columns, bool &should_evaluate_missing_defaults, size_t num_rows) const {
  try {
    ...
    if (!missing) {
      ...
      DB::fillMissingColumns(res_columns, num_rows, converted_requested_columns, Nested::convertToSubcolumns(available_columns),
                             partially_read_columns, storage_snapshot);
    ...
    e.addMessage("(while reading from part " + part_storage->getFullPath() + " located on disk " + part_storage->getDiskName() +
  ...
  try {
    ...
    for (size_t pos = 0; pos < num_columns; ++pos, ++it) {
      if (res_columns[pos]) {
        ...
        if (full_requested_columns_set.emplace(it->name).second) full_requested_columns.emplace_back(it->name, it->type);
    ...
    if (storage_snapshot->virtual_columns) {
      for (const auto &virtual_column : *storage_snapshot->virtual_columns) {
        if (virtual_column.default_desc.expression) combined_columns.add(virtual_column);
      }
    }

    auto dag = DB::evaluateMissingDefaults(additional_columns, full_requested_columns, combined_columns, context_copy);
...
bool IMergeTreeReader::isSubcolumnOffsetsOfNested(const String &name_in_storage, const String &subcolumn_name) const {
  ...
  if (!data_part_info_for_read->isWidePart() || subcolumn_name != "size0") return false;
...
std::pair<String, String> IMergeTreeReader::getStorageAndSubcolumnNameInPart(const NameAndTypePair &required_column) const {
  ...
  if (alter_conversions->isColumnRenamed(name_in_storage)) name_in_storage = alter_conversions->getColumnOldName(name_in_storage);
  ...
  if (isSubcolumnOffsetsOfNested(name_in_storage, subcolumn_name)) name_in_storage = Nested::splitName(name_in_storage).first;
...
void IMergeTreeReader::performRequiredConversions(Columns &res_columns) const {
  try {
    ...
    for (size_t pos = 0; pos < num_columns; ++pos, ++name_and_type)
      if (copy_block.has(name_and_type->name)) res_columns[pos] = std::move(copy_block.getByName(name_and_type->name).column);
  } catch (Exception &e) {
    /// Better diagnostics.
    const auto &part_storage = data_part_info_for_read->getDataPartStorage();
    e.addMessage("(while reading from part " + part_storage->getFullPath() + " located on disk " + part_storage->getDiskName() +
...
std::optional<IMergeTreeReader::ColumnForOffsets> IMergeTreeReader::findColumnForOffsets(const NameAndTypePair &required_column) const {
  auto get_offsets_streams = [](const auto &serialization, const auto &name_in_storage) {
    ...
    serialization->enumerateStreams([&](const auto &subpath) {
      if (subpath.empty() || subpath.back().type != ISerialization::Substream::ArraySizes) return;
  ...
  for (const auto &part_column : Nested::convertToSubcolumns(data_part_info_for_read->getColumns())) {
    ...
    if (name_in_storage != required_name_in_storage) continue;
    ...
    for (; i < required_offsets_streams.size(); ++i) {
      ...
      if (current_it == offsets_streams_map.end()) break;
      it = current_it;
    }
...
void IMergeTreeReader::checkNumberOfColumns(size_t num_columns_to_read) const {
  if (num_columns_to_read != getColumns().size())
    throw Exception(ErrorCodes::LOGICAL_ERROR,
                    ...
                    "Expected {}, got {}",
                    converted_requested_columns.size(), num_columns_to_read);
}
...
String IMergeTreeReader::getMessageForDiagnosticOfBrokenPart(size_t from_mark, size_t max_rows_to_read, size_t offset) const {
  ...
  return fmt::format(
      ...
      data_part_storage->getFullPath(), data_part_info_for_read->getTableName(), data_part_storage->getDiskName(),
      data_part_storage->getDiskType(), from_mark, max_rows_to_read, offset);
}

MergeTreeReaderPtr createMergeTreeReaderCompact(const MergeTreeDataPartInfoForReaderPtr &read_info,
                                                const NamesAndTypesList &columns_to_read, const StorageSnapshotPtr &storage_snapshot,
                                                const MergeTreeSettingsPtr &storage_settings, const MarkRanges &mark_ranges,
                                                const VirtualFields &virtual_fields, UncompressedCache *uncompressed_cache,
                                                MarkCache *mark_cache, DeserializationPrefixesCache *deserialization_prefixes_cache,
...
MergeTreeReaderPtr createMergeTreeReader(const MergeTreeDataPartInfoForReaderPtr &read_info, const NamesAndTypesList &columns_to_read,
                                         const StorageSnapshotPtr &storage_snapshot, const MergeTreeSettingsPtr &storage_settings,
                                         const MarkRanges &mark_ranges, const VirtualFields &virtual_fields,
                                         UncompressedCache *uncompressed_cache, MarkCache *mark_cache,
                                         DeserializationPrefixesCache *deserialization_prefixes_cache,
  ...
  if (read_info->isCompactPart())
    return createMergeTreeReaderCompact(read_info, columns_to_read, storage_snapshot, storage_settings, mark_ranges, virtual_fields,
                                        uncompressed_cache, mark_cache, deserialization_prefixes_cache, reader_settings,
                                        avg_value_size_hints, profile_callback);

  if (read_info->isWidePart())
    return createMergeTreeReaderWide(read_info, columns_to_read, storage_snapshot, storage_settings, mark_ranges, virtual_fields,
                                     uncompressed_cache, mark_cache, deserialization_prefixes_cache, reader_settings, avg_value_size_hints,
                                     profile_callback);

  throw Exception(ErrorCodes::LOGICAL_ERROR, "Unknown part type");
}

MergeTreeReaderPtr createMergeTreeReaderTextIndex(const IMergeTreeReader *main_reader, const MergeTreeIndexWithCondition &index,
                                                  const NamesAndTypesList &columns_to_read, bool can_skip_mark);

MergeTreeReaderPtr createMergeTreeReaderIndex(const IMergeTreeReader *main_reader, const MergeTreeIndexWithCondition &index,
                                              const NamesAndTypesList &columns_to_read, bool can_skip_mark) {
  if (index.index->index.type == "text") return createMergeTreeReaderTextIndex(main_reader, index, columns_to_read, can_skip_mark);
