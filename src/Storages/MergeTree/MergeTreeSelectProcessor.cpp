...
ParallelReadingExtension::ParallelReadingExtension(MergeTreeAllRangesCallback all_callback_, MergeTreeReadTaskCallback callback_,
                                                   size_t number_of_current_replica_, size_t total_nodes_count_)
  ...
  callback = TelemetryWrapper<MergeTreeReadTaskCallback>{std::move(callback_), ProfileEvents::ParallelReplicasReadRequestMicroseconds,
                                                         "ParallelReplicasReadRequest"};
}
...
std::optional<ParallelReadResponse> ParallelReadingExtension::sendReadRequest(CoordinationMode mode, size_t min_number_of_marks,
                                                                              const RangesInDataPartsDescription &description) const {
  return callback(ParallelReadRequest{mode, number_of_current_replica, min_number_of_marks, description});
}

MergeTreeIndexBuildContext::MergeTreeIndexBuildContext(RangesByIndex read_ranges_, ProjectionRangesByIndex projection_read_ranges_,
...
MergeTreeIndexReadResultPtr MergeTreeIndexBuildContext::getPreparedIndexReadResult(const MergeTreeReadTask &task) const {
  ...
  if (part_last_task) index_reader_pool->clear(task.getInfo().data_part);

  return index_read_result;
}

MergeTreeSelectProcessor::MergeTreeSelectProcessor(MergeTreeReadPoolPtr pool_, MergeTreeSelectAlgorithmPtr algorithm_,
    ...
    : pool(std::move(pool_)),
      ...
      prewhere_actions(getPrewhereActions(row_level_filter, prewhere_info, index_read_tasks_, actions_settings,
  ...
  if (has_prewhere_actions_steps) LOG_TEST(log, "PREWHERE condition was split into {} steps", prewhere_actions.steps.size());
...
PrewhereExprInfo MergeTreeSelectProcessor::getPrewhereActions(const FilterDAGInfoPtr &row_level_filter,
                                                              const PrewhereInfoPtr &prewhere_info, const IndexReadTasks &index_read_tasks,
  ...
  if (prewhere_info && (!enable_multiple_prewhere_read_steps ||
...
ChunkAndProgress MergeTreeSelectProcessor::readCurrentTask(MergeTreeReadTask &current_task,
                                                           IMergeTreeSelectAlgorithm &task_algorithm) const {
  if (!current_task.getReadersChain().isInitialized())
    current_task.initializeReadersChain(prewhere_actions, merge_tree_index_build_context, lazy_materializing_rows,
  ...
  if (res.row_count) {
    ...
    if (add_part_level) chunk.getChunkInfos().add(std::make_shared<MergeTreeReadInfo>(data_part->info.level));
    ...
    if (reader_settings.use_query_condition_cache) {
      ...
      chunk.getChunkInfos().add(std::make_shared<MarkRangesInfo>(data_part->storage.getStorageID().uuid, part_name,
                                                                 data_part->index_granularity->getMarksCount(),
                                                                 data_part->index_granularity->hasFinalMark(), res.read_mark_ranges));
    }
  ...
  if (reader_settings.use_query_condition_cache && prewhere_info) current_task.addPrewhereUnmatchedMarks(res.read_mark_ranges);
...
ChunkAndProgress MergeTreeSelectProcessor::read() {
  while (!is_cancelled) {
    try {
        ...
        if (reader_settings.use_query_condition_cache && task && prewhere_info) {
          for (const auto *output : prewhere_info->prewhere_actions.getOutputs()) {
            if (output->result_name == prewhere_info->prewhere_column_name) {
              if (!VirtualColumnUtils::isDeterministic(output)) continue;
              ...
              String part_name =
                  data_part->isProjectionPart() ? fmt::format("{}:{}", data_part->getParentPartName(), data_part->name) : data_part->name;
              query_condition_cache->write(data_part->storage.getStorageID().uuid, part_name, output->getHash(),
                                           prewhere_info->prewhere_actions.getNames()[0], task->getPrewhereUnmatchedMarks(),
      ...
      if (e.code() == ErrorCodes::QUERY_WAS_CANCELLED || e.code() == ErrorCodes::QUERY_WAS_CANCELLED_BY_CLIENT) break;
      throw;
    }
...
void MergeTreeSelectProcessor::cancel() noexcept {
  ...
  if (merge_tree_index_build_context) merge_tree_index_build_context->index_reader_pool->cancel();
}

Block MergeTreeSelectProcessor::transformHeader(Block block, const FilterDAGInfoPtr &row_level_filter,
...
static String dumpStatistics(const ReadStepsPerformanceCounters &counters) {
  ...
  if (index_counter) out << fmt::format("index step rows_read: {}, ", index_counter->rows_read.load());
