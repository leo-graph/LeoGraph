...
MergeTreeReadPool::MergeTreeReadPool(RangesInDataParts&& parts_, MutationsSnapshotPtr mutations_snapshot_,
...
MergeTreeReadTaskPtr MergeTreeReadPool::getTask(size_t task_idx, MergeTreeReadTask* previous_task) {
...

  {
    ...
    if (task_idx >= backoff_state.current_threads) return nullptr;

    if (remaining_thread_tasks.empty()) return nullptr;

    const auto tasks_remaining_for_this_thread = !threads_tasks[task_idx].sum_marks_in_parts.empty();
    if (!tasks_remaining_for_this_thread && pool_settings.do_not_steal_tasks) return nullptr;
    ...
    if (!tasks_remaining_for_this_thread) {
      ...
      } else  // Try steal tasks from the next thread
      {
        ...
        if (it == remaining_thread_tasks.end()) it = remaining_thread_tasks.begin();
        thread_idx = *it;
      }
    }
    ...
    if (marks_in_part > need_marks && marks_in_part - need_marks < min_marks_per_task / 2) need_marks = marks_in_part;
    ...
    if (marks_in_part <= need_marks) {
      ...
      if (thread_tasks.sum_marks_in_parts.empty()) remaining_thread_tasks.erase(thread_idx);
    } else {
      ...
      while (need_marks > 0 && !thread_task.ranges.empty()) {
        ...
        if (range.begin == range.end) thread_task.ranges.pop_front();
...
void MergeTreeReadPool::profileFeedback(ReadBufferFromFileBase::ProfileInfo info) {
  if (backoff_settings.min_read_latency_ms == 0 || pool_settings.do_not_steal_tasks) return;

  if (info.nanoseconds < backoff_settings.min_read_latency_ms * 1000000) return;

  std::lock_guard lock(mutex);

  if (backoff_state.current_threads <= backoff_settings.min_concurrency) return;

  size_t throughput = info.bytes_read * 1000000000 / info.nanoseconds;

  if (throughput >= backoff_settings.max_throughput) return;

  if (backoff_state.time_since_prev_event.elapsed() < backoff_settings.min_interval_between_events_ms * 1000000) return;

  backoff_state.time_since_prev_event.restart();
  ++backoff_state.num_events;

  ProfileEvents::increment(ProfileEvents::SlowRead);
  LOG_DEBUG(log, "Slow read, event №{}: read {} bytes in {} sec., {}/s.", backoff_state.num_events, info.bytes_read,
            static_cast<double>(info.nanoseconds) / 1e9, ReadableSize(throughput));

  if (backoff_state.num_events < backoff_settings.min_events) return;
...
void MergeTreeReadPool::fillPerThreadInfo(size_t threads, size_t sum_marks) {
  if (threads > 1000000ull) throw Exception(ErrorCodes::CANNOT_SCHEDULE_TASK, "Too many threads ({}) requested", threads);

  std::lock_guard lock(mutex);

  threads_tasks.resize(threads);
  if (parts_ranges.empty()) return;
  ...
  for (size_t i = 0; i < threads && !parts_queue.empty(); ++i) {
    ...
    while (need_marks > 0 && !parts_queue.empty()) {
      ...
      if (marks_in_part >= min_marks_per_task && need_marks < min_marks_per_task) need_marks = min_marks_per_task;

      /// Do not leave too few rows in part for next time.
      if (marks_in_part > need_marks && marks_in_part - need_marks < min_marks_per_task) need_marks = marks_in_part;
      ...
      if (marks_in_part <= need_marks) {
        ...
        if (current_parts.empty()) parts_queue.pop();
      } else {
        /// Loop through part ranges.
        while (need_marks > 0) {
          ...
          marks_in_part -= marks_to_get_from_range;
          need_marks -= marks_to_get_from_range;
          if (range.begin == range.end) part_with_ranges.ranges.pop_front();
        }
      }
      ...
      if (marks_in_ranges != 0) remaining_thread_tasks.insert(i);
    }
