...
struct MergeStageProgress {
  explicit MergeStageProgress(Float64 weight_) : is_first(true), weight(weight_) {}

  MergeStageProgress(Float64 initial_progress_, Float64 weight_) : initial_progress(initial_progress_), is_first(false), weight(weight_) {}
...
class MergeProgressCallback {
 public:
  ...
  MergeProgressCallback(MergeListElement* merge_list_element_ptr_, UInt64& watch_prev_elapsed_, MergeStageProgress& stage_,
  ...
  void operator()(const Progress& value) {
    ...
    if (stage.is_first) merge_list_element_ptr->rows_read += value.read_rows;
 ...
 private:
  ...
  void updateProfileEvents(const Progress& value, ProfileEvents::Event rows_event, ProfileEvents::Event bytes_event) const {
    ...
    if (stage.is_first) ProfileEvents::increment(rows_event, value.read_rows);
  }
};
