...
ReplicatedMergeTreeCleanupThread::ReplicatedMergeTreeCleanupThread(StorageReplicatedMergeTree& storage_)
    : IMergeTreeCleanupThread(storage_), storage(storage_) {}
...
Float32 ReplicatedMergeTreeCleanupThread::iterate() {
  ...
  {
    auto lock = storage.lockForShare(RWLockImpl::NO_QUERY,
    ...
    cleaned_part_like +=
        storage.clearOldTemporaryDirectories((*storage.getSettings())[MergeTreeSetting::temporary_directories_lifetime].totalSeconds());
  }
  ...
  if (storage.is_leader) {
    ...
    size_t deduplication_blocks = clearOldBlocks(storage.zookeeper_path, "deduplication_hashes", *zookeeper,
                                                 ...
                                                 cached_stats_for_insert_deduplication_hashes, log);

    size_t normal_blocks = clearOldBlocks(
        storage.zookeeper_path, "blocks", *zookeeper, (*storage_settings)[MergeTreeSetting::replicated_deduplication_window_seconds],
        (*storage_settings)[MergeTreeSetting::replicated_deduplication_window], cached_block_stats_for_sync_inserts, log);
    ...
    size_t async_blocks = clearOldBlocks(storage.zookeeper_path, "async_blocks", *zookeeper,
                                         ...
                                         cached_block_stats_for_async_inserts, log);

    /// Many async blocks are transformed into one ordinary block
    Float32 async_blocks_per_block =
        static_cast<Float32>((*storage_settings)[MergeTreeSetting::replicated_deduplication_window]) /
        static_cast<Float32>((*storage_settings)[MergeTreeSetting::replicated_deduplication_window_for_async_inserts] + 1);
    cleaned_blocks = (static_cast<Float32>(deduplication_blocks) + static_cast<Float32>(normal_blocks) +
                      static_cast<Float32>(async_blocks) * async_blocks_per_block) /
                     2;
...
size_t ReplicatedMergeTreeCleanupThread::clearOldLogs() {
  ...
  size_t min_replicated_logs_to_keep =
      static_cast<size_t>(static_cast<double>((*storage_settings)[MergeTreeSetting::min_replicated_logs_to_keep]) * ratio);

  if (static_cast<size_t>(children_count) < min_replicated_logs_to_keep) return 0;
  ...
  String min_saved_record_log_str = entries[entries.size() > (*storage_settings)[MergeTreeSetting::max_replicated_logs_to_keep]
  ...
  for (const String& replica : replicas) {
    ...
    if (!pointer.empty()) log_pointer = parse<UInt64>(pointer);
  ...
  if (!recovering_replicas.empty()) min_saved_log_pointer = std::min(min_saved_log_pointer, min_log_pointer_lost_candidate);

  /// We will not touch the last `min_replicated_logs_to_keep` records.
  entries.erase(entries.end() - std::min<UInt64>(entries.size(), (*storage_settings)[MergeTreeSetting::min_replicated_logs_to_keep]),
                entries.end());
  ...
  markLostReplicas(host_versions_lost_replicas, log_pointers_candidate_lost_replicas, replicas.size() - num_replicas_were_marked_is_lost,
                   zookeeper);
  ...
  for (; i < entries.size(); ++i) {
    ...
    if (ops.size() > 4 * zkutil::MULTI_BATCH_SIZE || i + 1 == entries.size()) {
      ...
      for (const auto& host_version : host_versions_lost_replicas)
        ops.emplace_back(
            zkutil::makeCheckRequest(storage.zookeeper_path + "/replicas/" + host_version.first + "/host", host_version.second));
  ...
  if (i != 0) LOG_DEBUG(log, "Removed {} old log entries: {} - {}", i, entries[0], entries[i - 1]);
...
void ReplicatedMergeTreeCleanupThread::markLostReplicas(const std::unordered_map<String, UInt32>& host_versions_lost_replicas,
  ...
  for (const auto& pair : log_pointers_candidate_lost_replicas) {
    ...
    ops.emplace_back(
        zkutil::makeCheckRequest(storage.zookeeper_path + "/replicas/" + replica + "/host", host_versions_lost_replicas.at(replica)));
  ...
  for (size_t i = 0; i < candidate_lost_replicas.size(); ++i) futures.emplace_back(zookeeper->asyncTryMultiNoThrow(requests[i]));
...
struct ReplicatedMergeTreeCleanupThread::NodeWithStat {
  ...
  NodeWithStat(String node_, Int64 ctime_, Int64 czxid_, Int32 version_)
      : node(std::move(node_)), ctime(ctime_), czxid(czxid_), version(version_) {}
...
size_t ReplicatedMergeTreeCleanupThread::clearOldBlocks(const String& zookeeper_path, const String& blocks_dir_name,
                                                        zkutil::ZooKeeper& zookeeper, UInt64 window_seconds, UInt64 window_size,
                                                        NodeCTimeAndVersionCache& cached_block_stats, LoggerPtr log_) {
  ...
  if (timed_blocks.empty()) return 0;

  /// Use ZooKeeper's first node (last according to time) timestamp as "current" time.
  Int64 current_time = timed_blocks.front().ctime;
  Int64 time_threshold = std::max(static_cast<Int64>(0), current_time - static_cast<Int64>(1000 * window_seconds));
  ...
  auto first_outdated_block_time_threshold =
      std::upper_bound(timed_blocks.begin(), timed_blocks.end(), block_threshold, NodeWithStat::greaterByTime);
  auto first_outdated_block = std::min(first_outdated_block_fixed_threshold, first_outdated_block_time_threshold);

  auto num_nodes_to_delete = timed_blocks.end() - first_outdated_block;
  if (!num_nodes_to_delete) return 0;

  auto last_outdated_block = timed_blocks.end() - 1;
  LOG_TRACE(log_, "Will clear {} old blocks from {} (ctime {}) to {} (ctime {})", num_nodes_to_delete, first_outdated_block->node,
            first_outdated_block->ctime, last_outdated_block->node, last_outdated_block->ctime);
...
void ReplicatedMergeTreeCleanupThread::getBlocksSortedByTime(const String& zookeeper_path, const String& blocks_dir_name,
                                                             zkutil::ZooKeeper& zookeeper, std::vector<NodeWithStat>& timed_blocks,
                                                             NodeCTimeAndVersionCache& cached_block_stats, LoggerPtr log_) {
  timed_blocks.clear();
  ...
  if (not_cached_blocks) {
    LOG_TRACE(log_, "Checking {} {} ({} are not cached) to clear old ones from ZooKeeper., path is {}/{}", stat.numChildren,
              blocks_dir_name, not_cached_blocks, zookeeper_path, blocks_dir_name);
  }
...
size_t ReplicatedMergeTreeCleanupThread::clearOldMutations() {
  auto storage_settings = storage.getSettings();
  if (!(*storage_settings)[MergeTreeSetting::finished_mutations_to_keep]) return 0;
  ...
  for (const String& replica : replicas) {
    ...
    if (pointer.empty()) return 0;  /// One replica hasn't done anything yet so we can't delete any mutations.
  ...
  if (entries.size() <= (*storage_settings)[MergeTreeSetting::finished_mutations_to_keep]) return 0;
  ...
  for (size_t i = 0; i < entries.size(); ++i) {
    ...
    if (ops.size() > 4 * zkutil::MULTI_BATCH_SIZE || i + 1 == entries.size()) {
      ...
      try {
        ...
        /// Another replica already deleted the same node concurrently.
        if (e.code == Coordination::Error::ZNONODE) break;
