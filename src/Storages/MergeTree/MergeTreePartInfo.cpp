...
MergeTreePartInfo MergeTreePartInfo::fromPartName(const String &part_name, MergeTreeDataFormatVersion format_version) {
  if (auto part_opt = tryParsePartName(part_name, format_version)) return *part_opt;
  throw Exception(ErrorCodes::BAD_DATA_PART_NAME, "Unexpected part name: {} for format version: {}", part_name,
...
void MergeTreePartInfo::validatePartitionID(const ASTPtr &partition_id_ast, MergeTreeDataFormatVersion format_version) {
  ...
  if (partition_id.empty()) throw Exception(ErrorCodes::INVALID_PARTITION_VALUE, "Partition id is empty");

  if (format_version < MERGE_TREE_DATA_MIN_FORMAT_VERSION_WITH_CUSTOM_PARTITIONING) {
    if (partition_id.size() != 6 || !std::all_of(partition_id.begin(), partition_id.end(), isNumericASCII))
      throw Exception(ErrorCodes::INVALID_PARTITION_VALUE, "Invalid partition format: {}. Partition should consist of 6 digits: YYYYMM",
                      partition_id);
  } else {
...
String MergeTreePartInfo::getOriginalPartitionId() const { return isPatch() ? getOriginalPartitionIdOfPatch(partition_id) : partition_id; }

std::optional<MergeTreePartInfo> MergeTreePartInfo::tryParsePartName(std::string_view part_name,
  ...
  if (format_version < MERGE_TREE_DATA_MIN_FORMAT_VERSION_WITH_CUSTOM_PARTITIONING) {
    ...
    if (!tryReadIntText(min_yyyymmdd, in) || !checkChar('_', in) || !tryReadIntText(max_yyyymmdd, in) || !checkChar('_', in)) {
      return std::nullopt;
    }
  ...
  /// Sanity check
  if (partition_id.empty()) return std::nullopt;
  ...
  if (!tryReadIntText(min_block_num, in) || !checkChar('_', in) || !tryReadIntText(max_block_num, in) || !checkChar('_', in) ||
      !tryReadIntText(level, in)) {
    return std::nullopt;
  }

  /// Sanity check
  if (min_block_num > max_block_num) return std::nullopt;

  if (!in.eof()) {
    if (!checkChar('_', in) || !tryReadIntText(mutation, in) || !in.eof()) {
      return std::nullopt;
    }
  }
...
void MergeTreePartInfo::parseMinMaxDatesFromPartName(const String &part_name, DayNum &min_date, DayNum &max_date) {
  ...
  if (!tryReadIntText(min_yyyymmdd, in) || !checkChar('_', in) || !tryReadIntText(max_yyyymmdd, in)) {
  ...
  if (min_month != max_month) throw Exception(ErrorCodes::BAD_DATA_PART_NAME, "Part name {} contains different months", part_name);
}
...
String MergeTreePartInfo::getPartNameAndCheckFormat(MergeTreeDataFormatVersion format_version) const {
  if (format_version == MERGE_TREE_DATA_MIN_FORMAT_VERSION_WITH_CUSTOM_PARTITIONING) return getPartNameV1();
  ...
  throw Exception(ErrorCodes::BAD_DATA_PART_NAME,
...
String MergeTreePartInfo::describe() const { return getPartNameV1(); }
...
bool MergeTreePartInfo::areAllBlockNumbersCovered(const MergeTreePartInfo &blocks_range, std::vector<MergeTreePartInfo> candidates) {
  ...
  if (candidates[0].min_block != blocks_range.min_block) return false;

  int64_t current_right_block = candidates[0].min_block - 1;

  for (const auto &candidate : candidates) {
    if (current_right_block + 1 != candidate.min_block) return false;

    current_right_block = candidate.max_block;
  }

  if (current_right_block != blocks_range.max_block) return false;

  return true;
}

DetachedPartInfo DetachedPartInfo::parseDetachedPartName(const DiskPtr &disk, std::string_view dir_name,
                                                         MergeTreeDataFormatVersion format_version) {
  ...
  const std::string_view part_name = dir_name.substr(first_separator + 1, dir_name.size() - first_separator - 1);
