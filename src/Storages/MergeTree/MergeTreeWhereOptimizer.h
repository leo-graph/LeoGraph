...
class MergeTreeWhereOptimizer : private boost::noncopyable {
 public:
  MergeTreeWhereOptimizer(std::unordered_map<std::string, UInt64> column_sizes_, const StorageSnapshotPtr &storage_snapshot,
                          ConditionSelectivityEstimatorPtr estimator_, const Names &queried_columns_,
                          const std::optional<NameSet> &supported_columns_, LoggerPtr log_);
  ...
  FilterActionsOptimizeResult optimize(const ActionsDAG &filter_dag, const std::string &filter_column_name, const ContextPtr &context,
                                       bool is_final);
 ...
 private:
  struct Condition {
    ...
    String toString() const {
      return fmt::format(
          ...
          node.getColumnName(), viable, good, min_position_in_primary_key, estimated_row_count, columns_size, table_columns.size());
    }
  ...
  void analyzeImpl(Conditions &res, const RPNBuilderTreeNode &node, const WhereOptimizerContext &where_optimizer_context,
                   std::set<Int64> &pk_positions) const;
