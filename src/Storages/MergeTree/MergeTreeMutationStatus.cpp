...
void checkMutationStatus(std::optional<MergeTreeMutationStatus>& status, const std::set<String>& mutation_ids) {
  if (mutation_ids.empty()) throw Exception(ErrorCodes::LOGICAL_ERROR, "Cannot check mutation status because no mutation ids provided");
  ...
  if (!status->is_done && !status->latest_fail_reason.empty()) {
    throw Exception(
        ...
        mutation_ids.size() > 1 ? "s" : "", boost::algorithm::join(mutation_ids, ", "), status->latest_failed_part,
