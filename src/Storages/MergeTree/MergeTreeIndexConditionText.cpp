...
TextSearchQuery::TextSearchQuery(String function_name_, TextSearchMode search_mode_, TextIndexDirectReadMode direct_read_mode_,
                                 std::vector<String> tokens_)
    : function_name(std::move(function_name_)), search_mode(search_mode_), direct_read_mode(direct_read_mode_), tokens(std::move(tokens_)) {
...
SipHash TextSearchQuery::getHash() const {
  ...
  for (const auto &token : tokens) hash.update(token);

  return hash;
}

MergeTreeIndexConditionText::MergeTreeIndexConditionText(const ActionsDAG::Node *predicate, ContextPtr context_,
                                                         const Block &index_sample_block, TokenizerPtr tokenizer_,
                                                         MergeTreeIndexTextPreprocessorPtr preprocessor_)
    : WithContext(context_), header(index_sample_block), tokenizer(tokenizer_), preprocessor(preprocessor_) {
  if (!predicate) {
  ...
  rpn = std::move(RPNBuilder<RPNElement>(predicate, context_, [&](const RPNBuilderTreeNode &node, RPNElement &out) {
  ...
  for (const auto &element : rpn) {
    ...
    if (requiresReadingAllTokens(element)) global_search_mode = TextSearchMode::Any;
  }
...
bool MergeTreeIndexConditionText::isSupportedFunction(const String &function_name) {
  return function_name == "hasToken" || function_name == "hasAnyTokens" || function_name == "hasAllTokens" || function_name == "equals" ||
         function_name == "notEquals" || function_name == "mapContainsKey" || function_name == "mapContainsKeyLike" ||
         function_name == "mapContainsValue" || function_name == "mapContainsValueLike" || function_name == "has" ||
         function_name == "like" || function_name == "notLike" || function_name == "hasTokenOrNull" || function_name == "startsWith" ||
         function_name == "endsWith" || function_name == "match";
}
...
TextIndexDirectReadMode MergeTreeIndexConditionText::getDirectReadMode(const String &function_name) const {
  ...
  if (function_name == "equals" || function_name == "has" || function_name == "mapContainsKey" || function_name == "mapContainsValue") {
  ...
  if (function_name == "like" || function_name == "startsWith" || function_name == "endsWith" || function_name == "mapContainsKeyLike" ||
...
TextSearchQueryPtr MergeTreeIndexConditionText::createTextSearchQuery(const ActionsDAG::Node &node) const {
  ...
  if (!traverseAtomNode(rpn_node, rpn_element)) return nullptr;

  if (rpn_element.text_search_queries.size() != 1) return nullptr;
...
std::optional<String> MergeTreeIndexConditionText::replaceToVirtualColumn(const TextSearchQuery &query, const String &index_name) {
  if (query.tokens.empty() && query.direct_read_mode == TextIndexDirectReadMode::Hint) return std::nullopt;

  auto query_hash = query.getHash();
  auto it = all_search_queries.find(query_hash.get128());

  if (it == all_search_queries.end()) return std::nullopt;
...
bool MergeTreeIndexConditionText::alwaysUnknownOrTrue() const {
  return rpnEvaluatesAlwaysUnknownOrTrue(
      rpn, {RPNElement::FUNCTION_EQUALS, RPNElement::FUNCTION_NOT_EQUALS, RPNElement::FUNCTION_HAS_ANY_TOKENS,
            RPNElement::FUNCTION_HAS_ALL_TOKENS, RPNElement::FUNCTION_IN, RPNElement::FUNCTION_NOT_IN, RPNElement::FUNCTION_MATCH});
}

bool MergeTreeIndexConditionText::mayBeTrueOnGranule(MergeTreeIndexGranulePtr idx_granule,
  ...
  if (!granule) throw Exception(ErrorCodes::LOGICAL_ERROR, "Text index condition got a granule with the wrong type.");
  ...
  for (const auto &element : rpn) {
    if (element.function == RPNElement::FUNCTION_UNKNOWN) {
      ...
      if (element.function == RPNElement::FUNCTION_NOT_EQUALS) rpn_stack.back() = !rpn_stack.back();
      ...
      rpn_stack.emplace_back(exists_in_granule, true);
      if (element.function == RPNElement::FUNCTION_NOT_IN) rpn_stack.back() = !rpn_stack.back();
...
bool MergeTreeIndexConditionText::traverseAtomNode(const RPNBuilderTreeNode &node, RPNElement &out) const {
  ...
  if (node.isFunction()) {
    ...
    if (traverseMapElementKeyNode(function, out)) return true;
    ...
    if (functionIsInOrGlobalInOperator(function_name)) {
      ...
      if (rhs_argument.tryGetConstant(const_value, const_type)) {
        if (traverseFunctionNode(function, lhs_argument, const_type, const_value, out)) return true;
      } else if (lhs_argument.tryGetConstant(const_value, const_type) && (function_name == "equals" || function_name == "notEquals")) {
        if (traverseFunctionNode(function, rhs_argument, const_type, const_value, out)) return true;
      }
    }
  }
...
bool MergeTreeIndexConditionText::traverseFunctionNode(const RPNBuilderFunctionTreeNode &function_node,
                                                       const RPNBuilderTreeNode &index_column_node, const DataTypePtr &value_type,
                                                       const Field &value_field, RPNElement &out) const {
  ...
  if (!has_index_column && !has_map_keys_column && !has_map_values_column) return false;

  auto value_data_type = WhichDataType(value_type);
  if (!value_data_type.isStringOrFixedString() && !value_data_type.isArray()) return false;

  if (has_map_keys_column || has_map_values_column) {
    if (!value_data_type.isStringOrFixedString()) return false;
    ...
    if (has_map_keys_column) {
      if (function_name == "mapContainsKey" || function_name == "has") return make_map_function(stringToTokens(value_field));
    ...
    if (has_map_values_column) {
      if (function_name == "mapContainsValue") return make_map_function(stringToTokens(value_field));
  ...
  if (function_name == "hasAnyTokens" || function_name == "hasAllTokens") {
    ...
    if (function_name == "hasAnyTokens") {
      ...
      out.text_search_queries.emplace_back(
          std::make_shared<TextSearchQuery>(function_name, TextSearchMode::Any, direct_read_mode, search_tokens));
    } else {
      out.function = RPNElement::FUNCTION_HAS_ALL_TOKENS;
      out.text_search_queries.emplace_back(
          std::make_shared<TextSearchQuery>(function_name, TextSearchMode::All, direct_read_mode, search_tokens));
    }
...
bool MergeTreeIndexConditionText::traverseMapElementKeyNode(const RPNBuilderFunctionTreeNode &function_node, RPNElement &out) const {
  ...
  if (required_columns.size() != 1 || outputs.size() != 1) return false;

  auto required_column = required_columns.front();
  auto output_column_name = outputs.front()->result_name;

  if (!isMap(required_column.type) || !header.has(fmt::format("mapKeys({})", required_column.name))) return false;
  ...
  while (!stack.empty()) {
    ...
    if (node->type == ActionsDAG::ActionType::FUNCTION && node->children.size() == 2 && node->function_base &&
        node->function_base->getName() == "arrayElement") {
      if (key_const_value.has_value()) return false;

      const auto &map_argument = node->children[0];
      const auto &const_key_argument = node->children[1];

      if (map_argument->type != ActionsDAG::ActionType::INPUT || map_argument->result_name != required_column.name) return false;
  ...
  if (result_column->getBool(0)) return false;

  auto tokens = stringToTokens(*key_const_value);
  out.function = RPNElement::FUNCTION_EQUALS;
  out.text_search_queries.emplace_back(
      std::make_shared<TextSearchQuery>("mapContainsKey", TextSearchMode::All, getHintOrNoneMode(), std::move(tokens)));
  return true;
}
...
bool MergeTreeIndexConditionText::traverseMapElementValueNode(const RPNBuilderTreeNode &index_column_node, const Field &const_value) const {
  ...
  if (!index_column_node.isFunction()) return false;

  const auto function = index_column_node.toFunctionNode();

  if (function.getArgumentsSize() != 2 || function.getFunctionName() != "arrayElement") return false;

  const auto column_name = function.getArgumentAt(0).getColumnName();

  if (const_value.getType() != Field::Types::String || const_value.safeGet<String>().empty()) return false;

  return header.has(fmt::format("mapValues({})", column_name));
}

bool MergeTreeIndexConditionText::tryPrepareSetForTextSearch(const RPNBuilderTreeNode &lhs, const RPNBuilderTreeNode &rhs,
                                                             const String &function_name, RPNElement &out) const {
  ...
  if (lhs.isFunction() && lhs.toFunctionNode().getFunctionName() == "tuple") {
    ...
    for (size_t i = 0; i < arguments_size; ++i) {
      if (header.has(function.getArgumentAt(i).getColumnName())) {
        ...
        if (set_key_position.has_value()) return false;

        set_key_position = i;
      }
    }
  } else {
    if (header.has(lhs.getColumnName())) set_key_position = 0;
  }
  ...
  if (!prepared_set || !prepared_set->hasExplicitSetElements()) return false;

  Columns columns = prepared_set->getSetElements();
  const auto &set_column = *columns[*set_key_position];

  if (!WhichDataType(set_column.getDataType()).isStringOrFixedString()) return false;
  ...
  for (size_t row = 0; row < total_row_count; ++row) {
    ...
    out.text_search_queries.emplace_back(
        std::make_shared<TextSearchQuery>(function_name, TextSearchMode::All, TextIndexDirectReadMode::None, std::move(tokens)));
  }

  return true;
}

bool isTextIndexVirtualColumn(const String &column_name) { return column_name.starts_with(TEXT_INDEX_VIRTUAL_COLUMN_PREFIX); }
