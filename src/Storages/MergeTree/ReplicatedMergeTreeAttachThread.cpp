...
ReplicatedMergeTreeAttachThread::~ReplicatedMergeTreeAttachThread() { shutdown(); }

void ReplicatedMergeTreeAttachThread::start() { task->activateAndSchedule(); }
...
void ReplicatedMergeTreeAttachThread::run() {
  ...
  try {
    ...
    if (needs_retry) {
      ...
      LOG_ERROR(log, "Initialization failed, table will remain readonly. Error: {}",
                getCurrentExceptionMessage(/* with_stacktrace */ true));
      storage.initialization_done = true;
    }

    if (!std::exchange(storage.is_readonly_metric_set, true)) CurrentMetrics::add(CurrentMetrics::ReadonlyReplica);
  }

  if (!first_try_done.exchange(true)) first_try_done.notify_one();
...
void ReplicatedMergeTreeAttachThread::checkHasReplicaMetadataInZooKeeper(const zkutil::ZooKeeperPtr &zookeeper,
...
void ReplicatedMergeTreeAttachThread::runImpl() {
  ...
  if (!host_node_exists) {
    LOG_WARNING(log,
                "Replica {} is dropped (but metadata is not completely removed from ZooKeeper), "
                "table will stay in readonly mode",
                replica_path);
    storage.has_metadata_in_zookeeper = false;
    return;
  }
  ...
  storage.checkTableStructure(replica_path, metadata_snapshot, /* metadata_version = */ nullptr, /* strict_check = */ true,
...
void ReplicatedMergeTreeAttachThread::setSkipSanityChecks(bool skip_sanity_checks_) { skip_sanity_checks = skip_sanity_checks_; }
