...
MergeTreeDataPartWriterOnDisk::MergeTreeDataPartWriterOnDisk(
    ...
    const VirtualsDescriptionPtr& virtual_columns_, const MergeTreeIndices& indices_to_recalc_, const String& marks_file_extension_,
    const CompressionCodecPtr& default_codec_, const MergeTreeWriterSettings& settings_, MergeTreeIndexGranularityPtr index_granularity_,
    WrittenOffsetSubstreams* written_offset_substreams_)
    : IMergeTreeDataPartWriter(data_part_name_, serializations_, data_part_storage_, index_granularity_info_, storage_settings_,
                               columns_list_, metadata_snapshot_, virtual_columns_, settings_, std::move(index_granularity_)),
  ...
  if (settings.rewrite_primary_key) initPrimaryIndex();

  initSkipIndices();
}

void MergeTreeDataPartWriterOnDisk::cancel() noexcept {
  if (index_file_stream) index_file_stream->cancel();
  if (index_file_hashing_stream) index_file_hashing_stream->cancel();
  if (index_compressor_stream) index_compressor_stream->cancel();
  if (index_source_hashing_stream) index_source_hashing_stream->cancel();

  for (auto& stream : skip_indices_streams_holders) stream->cancel();
}

size_t MergeTreeDataPartWriterOnDisk::computeIndexGranularity(const Block& block) const {
  return DB::computeIndexGranularity(block.rows(), block.bytes(), (*storage_settings)[MergeTreeSetting::index_granularity_bytes],
                                     (*storage_settings)[MergeTreeSetting::index_granularity], settings.blocks_are_granules_size,
...
void MergeTreeDataPartWriterOnDisk::initPrimaryIndex() {
  if (metadata_snapshot->hasPrimaryKey()) {
    ...
    if (compress_primary_key) {
      ...
      index_compressor_stream = std::make_unique<CompressedWriteBuffer>(*index_file_hashing_stream, primary_key_compression_codec,
                                                                        settings.primary_key_compress_block_size);
    ...
    for (const auto& type : primary_key_types) index_serializations.push_back(type->getDefaultSerialization());
  }
}

void MergeTreeDataPartWriterOnDisk::initSkipIndices() {
  if (skip_indices.empty()) return;
  ...
  for (const auto& skip_index : skip_indices) {
    ...
    for (const auto& index_substream : index_substreams) {
      ...
      if (settings.save_marks_in_cache) cached_index_marks.emplace(stream_name, std::make_unique<MarksInCompressedFile::PlainArray>());
    }
...
void MergeTreeDataPartWriterOnDisk::calculateAndSerializePrimaryIndexRow(const Block& index_block, size_t row) {
  ...
  for (size_t i = 0; i < index_block.columns(); ++i) {
    ...
    if (settings.save_primary_index_in_memory) index_columns[i]->insertFrom(*column, row);
  }
}
...
void MergeTreeDataPartWriterOnDisk::calculateAndSerializePrimaryIndex(const Block& primary_index_block, const Granules& granules_to_write) {
  if (!metadata_snapshot->hasPrimaryKey()) return;

  {
    ...
    for (const auto& granule : granules_to_write) {
      if (granule.mark_on_start) calculateAndSerializePrimaryIndexRow(primary_index_block, granule.start_row);
    }
  }

  /// Store block with last index row to write final mark at the end of column
  if (with_final_mark) last_index_block = primary_index_block;
}
    ...
    for (const auto& granule : granules_to_write) {
      ...
      if (skip_indices_aggregators[i]->empty() && granule.mark_on_start) {
        ...
        for (const auto& [type, stream] : index_streams) {
          ...
          if (stream->compressed_hashing.offset() >= settings.min_compress_block_size) stream->compressed_hashing.next();
          ...
          if (settings.can_use_adaptive_granularity) writeBinaryLittleEndian(1UL, marks_out);

          if (auto it = cached_index_marks.find(stream->escaped_column_name); it != cached_index_marks.end()) it->second->push_back(mark);
        }
      }
      ...
      if (granule.is_complete) ++skip_index_accumulated_marks[i];
...
void MergeTreeDataPartWriterOnDisk::fillPrimaryIndexChecksums(MergeTreeData::DataPart::Checksums& checksums) {
  ...
  if (write_final_mark && compute_granularity) index_granularity->appendMark(0);
...
void MergeTreeDataPartWriterOnDisk::finishPrimaryIndexSerialization(bool sync) {
  if (index_file_hashing_stream) {
    ...
    if (sync) index_file_stream->sync();
...
void MergeTreeDataPartWriterOnDisk::finishSkipIndicesSerialization(bool sync) {
  ...
  if (!skip_indices.empty() && log->is(Poco::Message::PRIO_DEBUG)) {
    ...
    if (skip_indices.size() > max_indices_to_show) {
      std::partial_sort(index_times.begin(), index_times.begin() + max_indices_to_show, index_times.end(),
    ...
    LOG_DEBUG(log, "Spent {} ms calculating {} skip indices for the part {}: [{}]", total_us / 1000, skip_indices.size(), data_part_name,
              indices_str);
  }
