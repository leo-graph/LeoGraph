...
MergeTreeData::MutableDataPartsVector MergeProjectionPartsTask::extractTemporaryParts() {
  ...
  for (auto&& [level, parts] : std::exchange(level_parts, {})) tmp_parts_to_remove.append_range(std::move(parts));
...
bool MergeProjectionPartsTask::executeStep() {
  ...
  if (selected_parts.empty()) {
    ...
    auto tmp_part_merge_task =
        mutator->mergePartsToTemporaryPart(projection_future_part, projection.metadata, merge_entry,
                                           std::make_unique<MergeListElement>((*merge_entry)->table_id, projection_future_part, context),
                                           *table_lock_holder, time_of_merge, context, space_reservation,
                                           false,  // TODO Do we need deduplicate for projections
                                           {},
                                           false,  // no cleanup
                                           projection_merging_params, NO_TRANSACTION_PTR,
                                           /* need_prefix */ true, &projection, new_data_part.get(), ".tmp_proj");
