...
class MergeTreeDataPartWriterWide : public MergeTreeDataPartWriterOnDisk {
 ...
 public:
  MergeTreeDataPartWriterWide(const String& data_part_name_, const String& logger_name_, const SerializationByName& serializations_,
                              MutableDataPartStoragePtr data_part_storage_, const MergeTreeIndexGranularityInfo& index_granularity_info_,
                              const MergeTreeSettingsPtr& storage_settings_, const NamesAndTypesList& columns_list,
                              const StorageMetadataPtr& metadata_snapshot, const VirtualsDescriptionPtr& virtual_columns_,
                              const std::vector<MergeTreeIndexPtr>& indices_to_recalc, const String& marks_file_extension,
                              const CompressionCodecPtr& default_codec, const MergeTreeWriterSettings& settings,
 ...
 private:
  ...
  void writeColumn(const NameAndTypePair& name_and_type, const IColumn& column, WrittenOffsetSubstreams& offset_substreams,
                   const Granules& granules);

  /// Write single granule of one column.
  void writeSingleGranule(const NameAndTypePair& name_and_type, const IColumn& column, const WrittenOffsetSubstreams& offset_substreams,
                          ISerialization::SerializeBinaryBulkStatePtr& serialization_state,
                          ISerialization::SerializeBinaryBulkSettings& serialize_settings, const Granule& granule);

  /// Take offsets from column and return as MarkInCompressed file with stream name
  StreamsWithMarks getCurrentMarksForColumn(const NameAndTypePair& name_and_type, const WrittenOffsetSubstreams& offset_substreams);
  ...
  void writeSingleMark(const NameAndTypePair& name_and_type, const WrittenOffsetSubstreams& offset_substreams, size_t number_of_rows);

  void writeFinalMark(const NameAndTypePair& name_and_type, WrittenOffsetSubstreams& offset_substreams);
