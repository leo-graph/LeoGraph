...
struct CurrentlyMergingPartsTagger {
  ...
  CurrentlyMergingPartsTagger(FutureMergedMutatedPartPtr future_part_, size_t total_size, StorageMergeTree& storage_,
                              const StorageMetadataPtr& metadata_snapshot, bool is_mutation);
...
struct MergeMutateSelectedEntry {
  ...
  MergeMutateSelectedEntry(FutureMergedMutatedPartPtr future_part_, CurrentlyMergingPartsTaggerPtr tagger_,
      ...
      : future_part(future_part_), tagger(std::move(tagger_)), commands(commands_), txn(txn_), mutation_ids(std::move(mutation_ids_)) {}
