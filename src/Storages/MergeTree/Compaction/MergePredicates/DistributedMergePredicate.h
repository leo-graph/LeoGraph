...
CommittingBlocks getCommittingBlocks(zkutil::ZooKeeperPtr& zookeeper, const std::string& zookeeper_path,
                                     std::optional<PartitionIdsHint>& partition_ids_hint, bool with_data);
...
class DistributedMergePredicate : public IMergePredicate {
 ...
 public:
    ...
    if (committing_blocks_ptr && left_max_block + 1 < right_min_block) {
      ...
      if (committing_blocks_ptr_in_partition != committing_blocks_ptr->end()) {
        ...
        if (block_it != block_numbers.end() && block_it->number < right_min_block)
          return std::unexpected(PreformattedMessage::create("Block number {} is still being inserted between parts {} and {}",
                                                             block_it->number, left.name, right.name));
      }
    }

    if (virtual_parts_ptr && left_max_block + 1 < right_min_block) {
      /// Fake part which will appear as merge result
      MergeTreePartInfo gap_part_info(left.info.getPartitionId(), left_max_block + 1, right_min_block - 1, MergeTreePartInfo::MAX_LEVEL,
      ...
      if (!covered.empty())
        return std::unexpected(
            PreformattedMessage::create("There are {} parts (from {} to {}) that are still not present or being processed by other "
                                        "background process on this replica between {} and {}",
                                        covered.size(), covered.front(), covered.back(), left.name, right.name));
    }

    if (mutations_state_ptr) {
      Int64 left_mutation_version =
          mutations_state_ptr->getCurrentMutationVersion(left.info.getOriginalPartitionId(), left.info.getDataVersion());

      Int64 right_mutation_version =
          mutations_state_ptr->getCurrentMutationVersion(left.info.getOriginalPartitionId(), right.info.getDataVersion());

      if (left_mutation_version != right_mutation_version)
        return std::unexpected(PreformattedMessage::create("Current mutation versions of parts {} and {} differ: {} and {} respectively",
                                                           left.name, right.name, left_mutation_version, right_mutation_version));
    }

    if (left.projection_names != right.projection_names)
      return std::unexpected(PreformattedMessage::create("Parts have different projection sets: {} in '{}' and {} in '{}'",
  ...
  std::expected<void, PreformattedMessage> canUsePartInMerges(const std::string& name, const MergeTreePartInfo& info) const {
    if (auto result = checkCanMergePartsPreconditions(name, info); !result) return result;
