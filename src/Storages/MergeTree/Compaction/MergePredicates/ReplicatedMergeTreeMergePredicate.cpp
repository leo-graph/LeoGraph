...
ReplicatedMergeTreeBaseMergePredicate::ReplicatedMergeTreeBaseMergePredicate(const ReplicatedMergeTreeQueue& queue_,
    ...
    : MergeCore(std::move(partition_ids_hint_)), queue(queue_) {}

std::expected<void, PreformattedMessage> ReplicatedMergeTreeBaseMergePredicate::canMergeParts(const PartProperties& left,
                                                                                              const PartProperties& right) const {
...
ReplicatedMergeTreeZooKeeperMergePredicate::ReplicatedMergeTreeZooKeeperMergePredicate(ReplicatedMergeTreeQueue& queue_,
  ...
  committing_blocks =
      std::make_shared<CommittingBlocks>(getCommittingBlocks(zookeeper, queue.zookeeper_path, partition_ids_hint, /*with_data=*/false));
...
bool ReplicatedMergeTreeZooKeeperMergePredicate::partParticipatesInReplaceRange(const MergeTreeData::DataPartPtr& part,
                                                                                PreformattedMessage& out_reason) const {
  /// FIXME: remove lock here
  std::lock_guard lock(queue.state_mutex);

  for (const auto& entry : queue.queue) {
    if (entry->type != ReplicatedMergeTreeLogEntry::REPLACE_RANGE) continue;

    for (const auto& part_name : entry->replace_range_entry->new_part_names) {
      if (part->info.isDisjoint(MergeTreePartInfo::fromPartName(part_name, queue.format_version))) continue;

      out_reason =
          PreformattedMessage::create("Part {} participates in REPLACE_RANGE {} ({})", part_name, entry->new_part_name, entry->znode_name);
      return true;
    }
  }
  return false;
...
std::optional<std::pair<Int64, int>> ReplicatedMergeTreeZooKeeperMergePredicate::getExpectedMutationVersion(
  ...
  if (queue.virtual_parts.getContainingPart(part->info) != part->name) return {};

  auto in_partition = queue.mutations_by_partition.find(part->info.getPartitionId());
  if (in_partition == queue.mutations_by_partition.end()) return {};
  ...
  if (auto it = patches_by_partition.find(part->info.getPartitionId()); it != patches_by_partition.end()) {
    for (const auto& patch_part : it->second) versions_of_patches.insert(patch_part.getDataVersion());
  }
  ...
  for (auto it = in_partition->second.begin(); it != in_partition->second.end(); ++it) {
    ...
    if (mutation_status->entry->commands.containBarrierCommand()) {
      ...
      if (max_version != mutation_version && max_version > current_version) break;

      /// This mutations is fresh, but it's barrier, let's execute only it
      if (mutation_version > current_version) barrier_found = true;
    }
    ...
    if (it != in_partition->second.begin()) {
      ...
      if (prev_version > current_version) {
        ...
        if (prev_pos != cur_pos) break;
      }
    }

    max_version = mutation_version;
    if (current_version < max_version) ++mutations_count;
    ...
    if (barrier_found == true) break;
  }

  if (current_version >= max_version) return {};

  LOG_TRACE(queue.log, "Will apply {} mutations and mutate part {} to version {} (the last version is {})", mutations_count, part->name,
            max_version, in_partition->second.rbegin()->first);

  return std::make_pair(max_version, alter_version);
}

bool ReplicatedMergeTreeZooKeeperMergePredicate::isMutationFinished(const std::string& znode_name,
                                                                    const std::map<String, int64_t>& block_numbers,
  ...
  for (const auto& kv : block_numbers) {
    ...
    if (checked_partitions_cache.contains(partition_id)) continue;
    ...
    if (partition_ids_hint && !partition_ids_hint->contains(partition_id)) {
      ...
      if (prev_virtual_parts && prev_virtual_parts->hasPartitionId(partition_id))
        throw Exception(ErrorCodes::LOGICAL_ERROR, "Committing blocks were not loaded for non-empty partition {}, it's a bug",
                        partition_id);
    ...
    if (partition_it != committing_blocks->end()) {
      size_t blocks_count = std::distance(partition_it->second.begin(), partition_it->second.lower_bound(block_num));
      if (blocks_count) {
        LOG_TRACE(queue.log, "Mutation {} is not done yet because in partition ID {} there are still {} uncommitted blocks.", znode_name,
                  partition_id, blocks_count);
        return false;
      }
    }
  ...
  if (auto it = queue.mutations_by_znode.find(znode_name); it != queue.mutations_by_znode.end()) {
    if (it->second.parts_to_do.size() == 0) return true;

    LOG_TRACE(queue.log, "Mutation {} is not done because some parts [{}] were just committed", znode_name,
              fmt::join(it->second.parts_to_do.getParts(), ", "));
    return false;
  }
...
bool ReplicatedMergeTreeZooKeeperMergePredicate::isGoingToBeDropped(const MergeTreePartInfo& new_drop_range_info,
