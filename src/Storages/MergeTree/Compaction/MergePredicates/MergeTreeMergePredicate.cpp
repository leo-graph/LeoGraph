...
static std::vector<MergeTreePartInfo> getPatchPartInfos(const StorageMergeTree& storage) {
  ...
  for (const auto& patch : patches_vector) patch_infos.push_back(patch->info);
...
std::expected<void, PreformattedMessage> MergeTreeMergePredicate::canMergeParts(const PartProperties& left,
...
std::expected<void, PreformattedMessage> MergeTreeMergePredicate::canUsePartInMerges(const MergeTreeDataPartPtr& part) const {
  ...
  if (min_update_block && part->info.getDataVersion() >= *min_update_block) {
    return std::unexpected(
        PreformattedMessage::create("Part {} has data version {}, but patch part with lower version {} is still being processed",
