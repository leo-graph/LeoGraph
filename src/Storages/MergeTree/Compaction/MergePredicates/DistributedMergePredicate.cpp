...
CommittingBlocks getCommittingBlocks(zkutil::ZooKeeperPtr& zookeeper, const std::string& zookeeper_path,
  ...
  if (!partition_ids_hint) {
    ...
    for (const auto& partition_id : *partition_ids_hint) {
      if (isPatchPartitionId(partition_id)) partitions_set.insert(getOriginalPartitionIdOfPatch(partition_id));
    }
  ...
  for (const String& partition : partitions) paths.push_back(fs::path(zookeeper_path) / "block_numbers" / partition);
  ...
  for (size_t i = 0; i < partitions.size(); ++i) {
    ...
    for (const String& entry : partition_block_numbers) {
      if (!startsWith(entry, "block-")) continue;
  ...
  if (with_data) {
    ...
    for (size_t i = 0; i < blocks_data.size(); ++i) {
      ...
      if (response.error != Coordination::Error::ZOK) throw Coordination::Exception::fromPath(response.error, block_data_paths[i]);
