...
bool sanityCheck(const PartsRanges& uncovered_ranges, RangesIterator range_it, PartsIterator range_begin, PartsIterator range_end) {
  if (!isInInterval(uncovered_ranges, range_it.base())) return false;

  if (!isInInterval(*range_it, range_begin.base())) return false;

  if (!isInSegment(*range_it, range_end.base())) return false;

  if (range_begin == range_end) return false;

  if (range_begin.base() >= range_end.base()) return false;
...
bool DisjointPartsRangesSet::BoundariesComparator::operator()(const PartsRangeBoundaries& lhs,
                                                              const PartsRangeBoundaries& rhs) const noexcept {
...
bool DisjointPartsRangesSet::isDisjoint(const PartsRangeBoundaries& boundaries, const SortedPartsRanges& sorted_ranges) {
  ...
  if (it != sorted_ranges.end())
    if (!isDisjoint(boundaries, *it)) return false;

  if (it != sorted_ranges.begin())
    if (!isDisjoint(boundaries, *std::prev(it))) return false;
...
bool DisjointPartsRangesSet::isCovered(const PartsIterator& part_it, const SortedPartsRanges& sorted_ranges) {
  ...
  if (it == sorted_ranges.begin()) return false;

  const auto& [range_begin, range_end] = *std::prev(it);
  return range_begin <= part_it && part_it < range_end;
}

DisjointPartsRangesSet::DisjointPartsRangesSet(const PartsRanges& uncovered_ranges_) : uncovered_ranges(uncovered_ranges_) {}
...
bool DisjointPartsRangesSet::isCovered(RangesIterator range_it, PartsIterator part_it) const {
  ...
  if (sorted_ranges_it == disjoint.end()) return false;
...
bool DisjointPartsRangesSet::addRangeIfPossible(RangesIterator range_it, PartsIterator range_begin, PartsIterator range_end) {
  ...
  if (!isDisjoint(range_boundaries, sorted_ranges)) return false;
