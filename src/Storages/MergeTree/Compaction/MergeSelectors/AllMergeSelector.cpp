...
PartsRanges AllMergeSelector::select(const PartsRanges& parts_ranges, const MergeConstraints& merge_constraints,
  ...
  for (auto it = parts_ranges.begin(); it != parts_ranges.end(); ++it) {
    ...
    if (range_filter && !range_filter(*it)) continue;
  ...
  if (min_partition_size && min_partition_rows && min_partition_size <= max_total_size_to_merge && min_partition_rows <= max_rows_in_part)
