...
static bool canIncludeToRange(size_t part_size, size_t part_rows, time_t part_ttl, time_t current_time, size_t usable_memory,
                              size_t usable_rows) {
...
class ITTLMergeSelector::MergeRangesConstructor {
  std::optional<PartsRange> buildRange(const CenterPosition& center_position, const MergeConstraint& constraint) {
    ...
    if (center->size > constraint.max_size_bytes) return std::nullopt;

    if (center->rows > constraint.max_size_rows) return std::nullopt;

    if (disjoint_set.isCovered(range, center)) return std::nullopt;

    size_t usable_memory = constraint.max_size_bytes - center->size;
    size_t usable_rows = constraint.max_size_rows - center->rows;
    size_t usable_parts =
        merge_selector.max_parts_to_merge_at_once ? merge_selector.max_parts_to_merge_at_once - 1 : std::numeric_limits<size_t>::max();
    ...
    if (range_filter && !range_filter({left, right})) return std::nullopt;

    if (!disjoint_set.addRangeIfPossible(range, left, right)) return std::nullopt;

    return PartsRange(left, right);
  }

 public:
  explicit MergeRangesConstructor(const ITTLMergeSelector& merge_selector_, const PartsRanges& parts_ranges,
  ...
  std::optional<PartsRange> buildMergeRange(const MergeConstraint& constraint) {
    constexpr static auto range_compare = [](const CenterPosition& lhs, const CenterPosition& rhs) { return lhs.ttl > rhs.ttl; };
    ...
    while (!centers.empty()) {
      ...
      if (auto range = buildRange(center, constraint)) return range;
    }
...
bool ITTLMergeSelector::needToPostponePartition(const std::string& partition_id) const {
  if (merge_due_times)
    if (auto it = merge_due_times->find(partition_id); it != merge_due_times->end()) return it->second > current_time;
...
std::vector<ITTLMergeSelector::CenterPosition> ITTLMergeSelector::findCenters(const PartsRanges& parts_ranges) const {
  ...
  for (auto range = parts_ranges.begin(); range != parts_ranges.end(); ++range) {
    ...
    if (needToPostponePartition(range_partition)) continue;

    for (auto part = range->begin(); part != range->end(); ++part) {
      if (!canConsiderPart(*part)) continue;

      time_t ttl = getTTLForPart(*part);
      if (!ttl || ttl > current_time) continue;
...
PartsIterator ITTLMergeSelector::findLeftRangeBorder(const CenterPosition& center_position, size_t& usable_memory, size_t& usable_rows,
                                                     size_t& usable_parts, DisjointPartsRangesSet& disjoint_set) const {
  PartsIterator left = center_position.center;

  while (left != center_position.range->begin()) {
    if (!usable_parts) break;

    auto next_to_check = std::prev(left);
    if (!canConsiderPart(*next_to_check)) break;

    if (disjoint_set.isCovered(center_position.range, next_to_check)) break;

    auto ttl = getTTLForPart(*next_to_check);
    if (!canIncludeToRange(next_to_check->size, next_to_check->rows, ttl, current_time, usable_memory, usable_rows)) break;
...
PartsIterator ITTLMergeSelector::findRightRangeBorder(const CenterPosition& center_position, size_t& usable_memory, size_t& usable_rows,
                                                      size_t& usable_parts, DisjointPartsRangesSet& disjoint_set) const {
  ...
  while (right != center_position.range->end()) {
    ...
    if (!canConsiderPart(*right)) break;

    if (disjoint_set.isCovered(center_position.range, right)) break;

    auto ttl = getTTLForPart(*right);
    if (!canIncludeToRange(right->size, right->rows, ttl, current_time, usable_memory, usable_rows)) break;
...
ITTLMergeSelector::ITTLMergeSelector(const PartitionIdToTTLs* merge_due_times_, time_t current_time_, size_t max_parts_to_merge_at_once_)
    : current_time(current_time_), merge_due_times(merge_due_times_), max_parts_to_merge_at_once(max_parts_to_merge_at_once_) {}

PartsRanges ITTLMergeSelector::select(const PartsRanges& parts_ranges, const MergeConstraints& merge_constraints,
...
time_t TTLPartDropMergeSelector::getTTLForPart(const PartProperties& part) const { return part.general_ttl_info->part_max_ttl; }

bool TTLPartDropMergeSelector::canConsiderPart(const PartProperties& part) const { return part.general_ttl_info.has_value(); }
...
time_t TTLRowDeleteMergeSelector::getTTLForPart(const PartProperties& part) const { return part.general_ttl_info->part_min_ttl; }

bool TTLRowDeleteMergeSelector::canConsiderPart(const PartProperties& part) const {
  if (part.is_in_volume_where_merges_avoid) return false;

  if (!part.general_ttl_info.has_value()) return false;
...
bool TTLRecompressMergeSelector::canConsiderPart(const PartProperties& part) const {
  ...
  if (!part.recompression_ttl_info.has_value()) return false;
