...
class Estimator {
 public:
  explicit Estimator(const PartsRanges& parts_ranges) : disjoint_set(parts_ranges) {}

  void consider(RangesIterator range_it, PartsIterator begin, PartsIterator end, size_t sum_size, size_t sum_rows, size_t size_prev_at_left,
                const SimpleMergeSelector::Settings& settings) {
    if (settings.enable_heuristic_to_remove_small_parts_at_right) {
      size_t size_delta = 0;
      size_t rows_delta = 0;
      while (end >= begin + 3 && static_cast<double>((end - 1)->size) <
                                     settings.heuristic_to_remove_small_parts_at_right_max_ratio * static_cast<double>(sum_size)) {
    ...
    double current_score =
        score(static_cast<double>(end - begin), static_cast<double>(sum_size), static_cast<double>(settings.size_fixed_cost_to_add));

    if (settings.enable_heuristic_to_align_parts &&
        static_cast<double>(size_prev_at_left) >
            static_cast<double>(sum_size) * settings.heuristic_to_align_parts_min_ratio_of_sum_size_to_prev_part) {
  ...
  std::optional<PartsRange> buildMergeRange(const MergeConstraint& constraint) {
    ...
    while (!ranges.empty()) {
      ...
      if (bytes <= constraint.max_size_bytes && rows <= constraint.max_size_rows &&
          disjoint_set.addRangeIfPossible(range_it, range_begin, range_end))
 ...
 */
bool allow(double sum_size, double max_size, double min_age, double partition_size, double min_size_to_lower_base_log,
           double max_size_to_lower_base_log, PartsIterator begin, PartsIterator end, const IMergeSelector::RangeFilter& range_filter,
           const SimpleMergeSelector::Settings& settings) {
  if (range_filter && !range_filter({begin, end})) return false;

  if (settings.min_age_to_force_merge && min_age >= static_cast<double>(settings.min_age_to_force_merge)) return true;

  const size_t size = end - begin;

  if (settings.min_parts_to_merge_at_once && size < settings.min_parts_to_merge_at_once) return false;
  ...
  double min_age_to_lower_base = interpolateLinear(static_cast<double>(settings.min_age_to_lower_base_at_min_size),
                                                   static_cast<double>(settings.min_age_to_lower_base_at_max_size), size_normalized);
  double max_age_to_lower_base = interpolateLinear(static_cast<double>(settings.max_age_to_lower_base_at_min_size),
                                                   static_cast<double>(settings.max_age_to_lower_base_at_max_size), size_normalized);
  /// Map age to 0..1
  ...
  double num_parts_normalized = mapPiecewiseLinearToUnit(partition_size, static_cast<double>(settings.min_parts_to_lower_base),
                                                         static_cast<double>(settings.max_parts_to_lower_base));
  ...
  return (sum_size + static_cast<double>(size) * static_cast<double>(settings.size_fixed_cost_to_add)) /
             (max_size + static_cast<double>(settings.size_fixed_cost_to_add)) >=
         lowered_base;
}
...
size_t calculateRangeWithStochasticSliding(size_t parts_count, size_t parts_threshold) {
  ...
  if (right_boundary > parts_count) right_boundary = 2 * parts_count - right_boundary;
...
void selectWithinPartsRange(RangesIterator range_it, const MergeConstraint& constraint, const IMergeSelector::RangeFilter& range_filter,
                            Estimator& estimator, const SimpleMergeSelector::Settings& settings, double min_size_to_lower_base_log,
                            double max_size_to_lower_base_log) {
  const PartsRange& parts = *range_it;
  size_t parts_count = parts.size();
  if (parts_count <= 1) return;
  ...
  if (settings.enable_heuristic_to_lower_max_parts_to_merge_at_once) {
    ...
    if (static_cast<double>(partition_stats.part_count) < settings.base) {
      ...
      max_parts_to_merge_at_once =
          static_cast<size_t>(settings.base + (static_cast<double>(max_parts_to_merge_at_once) - settings.base) *
                                                  (1.0 - std::pow((static_cast<double>(partition_stats.part_count) - settings.base) /
                                                                      (static_cast<double>(settings.parts_to_throw_insert) - settings.base),
                                                                  exponent)));
    }
  }
  ...
  for (; begin < parts_count; ++begin) {
    ...
    for (size_t end = begin + 2; end <= parts_count; ++end) {
      assert(end > begin);
      if (max_parts_to_merge_at_once && end - begin > max_parts_to_merge_at_once) break;
      ...
      if (sum_size > constraint.max_size_bytes) break;

      if (sum_rows > constraint.max_size_rows) break;

      auto range_begin = parts.begin() + begin;
      auto range_end = parts.begin() + end;

      if (allow(static_cast<double>(sum_size), static_cast<double>(max_size), static_cast<double>(min_age),
                static_cast<double>(parts_count), min_size_to_lower_base_log, max_size_to_lower_base_log, range_begin, range_end,
                range_filter, settings))
        estimator.consider(range_it, range_begin, range_end, sum_size, sum_rows, begin == 0 ? 0 : parts[begin - 1].size, settings);
    }
  }
}
...
PartsRanges SimpleMergeSelector::select(const PartsRanges& parts_ranges, const MergeConstraints& merge_constraints,
  ...
  for (auto range_it = parts_ranges.begin(); range_it != parts_ranges.end(); ++range_it)
    selectWithinPartsRange(range_it, merge_constraints[0], range_filter, estimator, settings, min_size_to_lower_base_log,
