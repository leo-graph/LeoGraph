...
static std::optional<MergeTreeDataPartsVector> findPartsInMemory(const MergeTreeData& data, const PartsRange& range,
                                                                 const MergeTreeData::DataPartStates& lookup_statuses) {
...
FutureMergedMutatedPartPtr constructFuturePart(const MergeTreeData& data, const MergeSelectorChoice& choice,
  ...
  if (!parts.has_value() || !patch_parts.has_value()) return nullptr;
