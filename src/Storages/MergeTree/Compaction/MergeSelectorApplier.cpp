...
MergeSelectorChoices pack(const ChooseContext& ctx, PartsRanges&& ranges, MergeType type) {
  ...
  for (auto& range : ranges) choices.push_back(create_choice(std::move(range), type));
...
MergeSelectorChoices tryChooseTTLMerge(const ChooseContext& ctx) {
  ...
  if (!ctx.merge_constraints.empty()) {
    ...
    std::vector<MergeConstraint> ttl_constraints(ctx.merge_constraints.size(),
...
SimpleMergeSelector::Settings fillSimpleSettings(const ChooseContext& ctx) {
  ...
  simple_merge_settings.enable_heuristic_to_remove_small_parts_at_right =
      ctx.merge_tree_settings[MergeTreeSetting::merge_selector_enable_heuristic_to_remove_small_parts_at_right];
  ...
  simple_merge_settings.enable_heuristic_to_lower_max_parts_to_merge_at_once =
      ctx.merge_tree_settings[MergeTreeSetting::merge_selector_enable_heuristic_to_lower_max_parts_to_merge_at_once];
  simple_merge_settings.heuristic_to_lower_max_parts_to_merge_at_once_exponent =
      ctx.merge_tree_settings[MergeTreeSetting::merge_selector_heuristic_to_lower_max_parts_to_merge_at_once_exponent];
  ...
  if (ctx.aggressive) simple_merge_settings.base = 1;
...
MergeSelectorApplier::MergeSelectorApplier(std::vector<MergeConstraint>&& merge_constraints_, bool merge_with_ttl_allowed_,
                                           bool aggressive_, IMergeSelector::RangeFilter range_filter_)
  ...
  chassert(
      std::ranges::is_sorted(merge_constraints, [](const auto& lhs, const auto& rhs) { return lhs.max_size_bytes > rhs.max_size_bytes; }),
      "Merge constraints must be sorted in desc order");
}

MergeSelectorChoices MergeSelectorApplier::chooseMergesFrom(const PartsRanges& ranges, const PartitionsStatistics& partitions_stats,
                                                            const IMergePredicate& predicate, const StorageMetadataPtr& metadata_snapshot,
                                                            const MergeTreeSettingsPtr& merge_tree_settings,
                                                            const PartitionIdToTTLs& next_delete_times,
                                                            const PartitionIdToTTLs& next_recompress_times, bool can_use_ttl_merges,
  ...
  if (metadata_snapshot->hasAnyTTL() && merge_with_ttl_allowed && can_use_ttl_merges)
    if (auto choices = tryChooseTTLMerge(ctx); !choices.empty()) return choices;
