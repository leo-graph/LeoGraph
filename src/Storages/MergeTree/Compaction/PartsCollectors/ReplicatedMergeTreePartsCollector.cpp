...
ReplicatedMergeTreePartsCollector::ReplicatedMergeTreePartsCollector(const StorageReplicatedMergeTree& storage_,
                                                                     ReplicatedMergeTreeMergePredicatePtr merge_pred_)
    : storage(storage_), merge_pred(std::move(merge_pred_)) {}

CollectedPartsRanges ReplicatedMergeTreePartsCollector::grabAllPossibleRanges(const StorageMetadataPtr& metadata_snapshot,
...
std::expected<PartsRange, PreformattedMessage> ReplicatedMergeTreePartsCollector::grabAllPartsInsidePartition(
  ...
  if (auto result = checkAllParts(parts, storage_policy, merge_pred); !result) return std::unexpected(std::move(result.error()));
