...
std::vector<std::vector<Part>> splitRangeByPredicate(std::vector<Part>&& parts, Predicate predicate, LogSeriesLimiter& series_log) {
  ...
  for (auto part_it = parts.begin(); part_it != parts.end();)
    if (auto next_range = build_next_range(part_it); !next_range.empty()) ranges.push_back(std::move(next_range));
...
std::expected<void, PreformattedMessage> checkAllPartsSatisfyPredicate(const std::vector<Part>& parts, Predicate predicate) {
  for (const auto& part : parts)
    if (auto result = predicate(part); !result) return std::unexpected(result.error());

  return {};
}

PartsRanges constructPartsRanges(std::vector<MergeTreeDataPartsVector>&& ranges, const StorageMetadataPtr& metadata_snapshot,
