...
MergeTreeDataPartsVector collectInitial(const MergeTreeData& data, const MergeTreeTransactionPtr& tx) {
  ...
  for (const auto& part : active_parts) active_parts_set.add(part->name);
  ...
  for (const auto& part : outdated_parts) {
    ...
    if (part->version.creation_csn == Tx::RolledBackCSN) continue;
    ...
    if (part->version.removal_csn != Tx::UnknownCSN) continue;
  ...
  std::merge(active_parts.begin(), active_parts.end(), outdated_parts.begin(), outdated_parts.end(), std::back_inserter(data_parts),
             MergeTreeData::LessDataPart());

  return data_parts;
}

auto constructPreconditionsPredicate(const StoragePolicyPtr& storage_policy, const MergeTreeTransactionPtr& tx,
...
std::vector<MergeTreeDataPartsVector> splitPartsByPreconditions(MergeTreeDataPartsVector&& parts, const StoragePolicyPtr& storage_policy,
...
std::expected<void, PreformattedMessage> checkAllParts(const MergeTreeDataPartsVector& parts, const StoragePolicyPtr& storage_policy,
                                                       const MergeTreeTransactionPtr& tx, const MergeTreeMergePredicatePtr& merge_pred) {
  return checkAllPartsSatisfyPredicate(parts, constructPreconditionsPredicate(storage_policy, tx, merge_pred));
}

}  // namespace

MergeTreePartsCollector::MergeTreePartsCollector(StorageMergeTree& storage_, MergeTreeTransactionPtr tx_,
                                                 MergeTreeMergePredicatePtr merge_pred_)
    : storage(storage_), tx(std::move(tx_)), merge_pred(std::move(merge_pred_)) {}

CollectedPartsRanges MergeTreePartsCollector::grabAllPossibleRanges(const StorageMetadataPtr& metadata_snapshot,
...
std::expected<PartsRange, PreformattedMessage> MergeTreePartsCollector::grabAllPartsInsidePartition(
    const StorageMetadataPtr& metadata_snapshot, const StoragePolicyPtr& storage_policy, const time_t& current_time,
    const std::string& partition_id) const {
  auto parts = filterByPartitions(collectInitial(storage, tx), PartitionIdsHint{partition_id});
  if (auto result = checkAllParts(parts, storage_policy, tx, merge_pred); !result) return std::unexpected(std::move(result.error()));
