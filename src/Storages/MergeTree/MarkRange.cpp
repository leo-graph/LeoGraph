...
MarkRange::MarkRange(size_t begin_, size_t end_) : begin(begin_), end(end_) { chassert(begin <= end); }

size_t MarkRange::getNumberOfMarks() const { return end - begin; }

bool MarkRange::operator==(const MarkRange& rhs) const { return begin == rhs.begin && end == rhs.end; }
...
bool MarkRange::operator<(const MarkRange& rhs) const {
  ...
  if (*this != rhs) {
    const bool is_intersection = (begin <= rhs.begin && rhs.begin < end) || (rhs.begin <= begin && begin < rhs.end);
...
size_t getLastMark(const MarkRanges& ranges) {
  ...
  for (const auto& mark_range : ranges) current_task_last_mark = std::max(current_task_last_mark, mark_range.end);
...
void assertSortedAndNonIntersecting(const MarkRanges& ranges) {
  ...
  if (ranges_copy != ranges)
    throw Exception(ErrorCodes::LOGICAL_ERROR, "Expected sorted and non intersecting ranges. Ranges: {}", toString(ranges));
}

size_t MarkRanges::getNumberOfMarks() const {
  size_t result = 0;
  for (const auto& mark : *this) result += mark.getNumberOfMarks();
  return result;
}
...
String MarkRanges::describe() const { return fmt::format("Size: {}, Data: {}", this->size(), fmt::join(*this, ",")); }
...
MarkRangesInfo::MarkRangesInfo(UUID table_uuid_, const String& part_name_, size_t marks_count_, bool has_final_mark_,
