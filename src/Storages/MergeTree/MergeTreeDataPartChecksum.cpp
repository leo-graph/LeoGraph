...
void MergeTreeDataPartChecksum::checkEqual(const MergeTreeDataPartChecksum& rhs, bool have_uncompressed, const String& name,
  ...
  if (is_compressed && have_uncompressed) {
    ...
    if (rhs.uncompressed_size != uncompressed_size) {
      throw Exception(ErrorCodes::BAD_SIZE_OF_FILE_IN_DATA_PART, "Unexpected uncompressed size of file {} in data part {} ({} vs {})", name,
                      part_name, uncompressed_size, rhs.uncompressed_size);
    }
    if (rhs.uncompressed_hash != uncompressed_hash) {
      throw Exception(ErrorCodes::CHECKSUM_DOESNT_MATCH, "Checksum mismatch for uncompressed file {} in data part {} ({} vs {})", name,
                      part_name, getHexUIntLowercase(uncompressed_hash), getHexUIntLowercase(rhs.uncompressed_hash));
    }
    return;
  }
  if (rhs.file_size != file_size) {
    throw Exception(ErrorCodes::BAD_SIZE_OF_FILE_IN_DATA_PART, "Unexpected size of file {} in data part {} ({} vs {})", name, part_name,
                    file_size, rhs.file_size);
  }
  if (rhs.file_hash != file_hash) {
    throw Exception(ErrorCodes::CHECKSUM_DOESNT_MATCH, "Checksum mismatch for file {} in data part {} ({} vs {})", name, part_name,
                    getHexUIntLowercase(file_hash), getHexUIntLowercase(rhs.file_hash));
  }
}
...
void MergeTreeDataPartChecksum::checkSize(const IDataPartStorage& storage, const String& name) const {
  ...
  if (storage.existsDirectory(name)) return;
...
void MergeTreeDataPartChecksums::checkEqual(const MergeTreeDataPartChecksums& rhs, bool have_uncompressed, const String& part_name) const {
  for (const auto& [name, _] : rhs.files)
    if (!files.contains(name)) throw Exception(ErrorCodes::UNEXPECTED_FILE_IN_DATA_PART, "Unexpected file {} in data part", name);

  for (const auto& [name, checksum] : files) {
    auto it = rhs.files.find(name);
    if (it == rhs.files.end()) throw Exception(ErrorCodes::NO_FILE_IN_DATA_PART, "No file {} in data part", name);
...
UInt64 MergeTreeDataPartChecksums::getTotalSizeOnDisk() const {
  UInt64 res = 0;
  for (const auto& [_, checksum] : files) res += checksum.file_size;
  return res;
}

UInt64 MergeTreeDataPartChecksums::getTotalSizeUncompressedOnDisk() const {
  UInt64 res = 0;
  for (const auto& [_, checksum] : files) res += checksum.uncompressed_size;
  return res;
}
...
void MergeTreeDataPartChecksums::addFile(const String& file_name, const Checksum& checksum) { files[file_name] = checksum; }
...
MergeTreeDataPartChecksums MergeTreeDataPartChecksums::deserializeFrom(const String& s) {
  ...
  if (!res.read(in)) throw Exception(ErrorCodes::FORMAT_VERSION_TOO_OLD, "Checksums format is too old");
  assertEOF(in);
  return res;
}

bool MergeTreeDataPartChecksums::isBadChecksumsErrorCode(int code) {
  return code == ErrorCodes::CHECKSUM_DOESNT_MATCH || code == ErrorCodes::BAD_SIZE_OF_FILE_IN_DATA_PART ||
         code == ErrorCodes::NO_FILE_IN_DATA_PART || code == ErrorCodes::UNEXPECTED_FILE_IN_DATA_PART;
}
...
String MergeTreeDataPartChecksums::getTotalChecksumHex() const { return getHexUIntUppercase(getTotalChecksumUInt128()); }
...
bool MinimalisticDataPartChecksums::deserialize(ReadBuffer& in) {
  ...
  if (format_version < MINIMAL_VERSION_WITH_MINIMALISTIC_CHECKSUMS) {
    ...
    if (!new_full_checksums.read(in, format_version)) return false;
...
String MinimalisticDataPartChecksums::getSerializedString(const MergeTreeDataPartChecksums& full_checksums, bool minimalistic) {
  if (!minimalistic) return full_checksums.getSerializedString();
...
void MinimalisticDataPartChecksums::checkEqual(const MergeTreeDataPartChecksums& rhs, bool check_uncompressed_hash_in_compressed_files,
                                               const String& part_name) const {
  if (full_checksums) full_checksums->checkEqual(rhs, check_uncompressed_hash_in_compressed_files, part_name);
...
void MinimalisticDataPartChecksums::checkEqualImpl(const MinimalisticDataPartChecksums& rhs,
                                                   bool check_uncompressed_hash_in_compressed_files) const {
  ...
  if (hash_of_uncompressed_files != rhs.hash_of_uncompressed_files) {
    errors.emplace_back(fmt::format("hash of uncompressed files doesn't match ({} vs {})", getHexUIntLowercase(hash_of_uncompressed_files),
  ...
  if (check_uncompressed_hash_in_compressed_files) {
    ...
    if (hash_of_all_files != rhs.hash_of_all_files) {
      errors.emplace_back(fmt::format("total hash of all files doesn't match ({} vs {})", getHexUIntLowercase(hash_of_all_files),
