...
MergeTreeDataPartWriterCompact::MergeTreeDataPartWriterCompact(
    ...
    : MergeTreeDataPartWriterOnDisk(data_part_name_, logger_name_, serializations_, data_part_storage_, index_granularity_info_,
      ...
      plain_file(getDataPartStorage().writeFile(MergeTreeDataPartCompact::DATA_FILE_NAME_WITH_EXTENSION, settings.max_compress_block_size,
                                                settings_.query_write_settings)),
      plain_hashing(*plain_file) {
  marks_file = getDataPartStorage().writeFile(MergeTreeDataPartCompact::DATA_FILE_NAME + marks_file_extension_, 4096,
                                              settings_.query_write_settings);

  marks_file_hashing = std::make_unique<HashingWriteBuffer>(*marks_file);

  if (index_granularity_info.mark_type.compressed) {
    marks_compressor = std::make_unique<CompressedWriteBuffer>(*marks_file_hashing,
...
void MergeTreeDataPartWriterCompact::addStreams(const NameAndTypePair &name_and_type, const ASTPtr &effective_codec_desc) {
  ISerialization::StreamCallback callback = [&](const auto &substream_path) {
    ...
    if (compressed_streams.contains(stream_name)) return;
    ...
    if (!stream) stream = std::make_shared<CompressedStream>(plain_hashing, compression_codec);
...
Granules getGranulesToWrite(const MergeTreeIndexGranularity &index_granularity, size_t block_rows, size_t current_mark, bool last_block) {
  ...
  while (current_row < block_rows) {
    ...
    if (rows_left_in_block < expected_rows_in_mark && !last_block) {
      ...
      throw Exception(ErrorCodes::LOGICAL_ERROR, "Required to write {} rows, but only {} rows was written for the non last granule",
                      expected_rows_in_mark, rows_left_in_block);
    }

    result.emplace_back(Granule{.start_row = current_row,
...
void writeColumnSingleGranule(const ColumnWithTypeAndName &column, const SerializationPtr &serialization,
                              ISerialization::OutputStreamGetter stream_getter, ISerialization::StreamMarkGetter stream_mark_getter,
                              size_t from_row, size_t number_of_rows, bool is_first_granule, const MergeTreeWriterSettings &settings) {
  ...
  if (is_first_granule)
    ...
    serialize_settings.object_and_dynamic_write_statistics =
        ISerialization::SerializeBinaryBulkSettings::ObjectAndDynamicStatisticsMode::PREFIX_EMPTY;
...
void MergeTreeDataPartWriterCompact::write(const Block &block, const IColumnPermutation *permutation) {
  ...
  if (header.empty()) header = result_block.cloneEmpty();
  ...
  if (columns_buffer.size() == 0 && result_block.rows() >= current_mark_rows) {
    ...
    if (rows_in_buffer >= current_mark_rows) flushed_block = header.cloneWithColumns(columns_buffer.releaseColumns());
  }
    ...
    for (size_t i = 0; i < columns_list.size(); ++i, ++name_and_type) {
      ...
      auto stream_getter = [&, this](const ISerialization::SubstreamPath &substream_path) -> WriteBuffer * {
        ...
        if (!prev_stream || index_granularity_info.mark_type.with_substreams) {
          ...
          if (!cached_marks.empty()) cached_marks.begin()->second->push_back(mark);
        }
      ...
      writeColumnSingleGranule(block.getByName(name_and_type->name), getSerialization(name_and_type->name), stream_getter,
                               stream_mark_getter, granule.start_row, granule.rows_to_write, !data_written, settings);
...
void MergeTreeDataPartWriterCompact::finalizeIndexGranularity() {
  ...
  if (index_granularity_info.mark_type.with_substreams && !data_written) {
    Block sample;
    for (const auto &[name, type] : columns_list) sample.insert(ColumnWithTypeAndName(type->createColumn(), type, name));
    initColumnsSubstreamsIfNeeded(sample);
  }

#ifndef NDEBUG
  /// Offsets should be 0, because compressed block is written for every granule.
  for (const auto &[_, stream] : streams_by_codec) assert(stream->hashing_buf.offset() == 0);
#endif
...
void MergeTreeDataPartWriterCompact::fillDataChecksums(MergeTreeDataPartChecksums &checksums) {
  ...
  if (marks_source_hashing) marks_source_hashing->finalize();
  if (marks_compressor) marks_compressor->finalize();
...
void MergeTreeDataPartWriterCompact::initColumnsSubstreamsIfNeeded(const Block &sample) {
  if (!index_granularity_info.mark_type.with_substreams || columns_substreams.getTotalSubstreams()) return;
  ...
  for (const auto &name_and_type : columns_list) {
    ...
    writeColumnSingleGranule(column, getSerialization(name_and_type.name), buffer_getter, mark_getter, column.column->size(), 0, false,
                             settings);
  }
}
...
void MergeTreeDataPartWriterCompact::addToChecksums(MergeTreeDataPartChecksums &checksums) {
  ...
  for (const auto &[_, stream] : streams_by_codec) {
    ...
    uncompressed_hash =
        CityHash_v1_0_2::CityHash128WithSeed(reinterpret_cast<const char *>(&stream_hash), sizeof(stream_hash), uncompressed_hash);
  }
...
size_t MergeTreeDataPartWriterCompact::ColumnsBuffer::size() const {
  if (accumulated_columns.empty()) return 0;
...
void MergeTreeDataPartWriterCompact::fillChecksums(MergeTreeDataPartChecksums &checksums, NameSet & /*checksums_to_remove*/) {
  ...
  if (!columns_list.empty()) fillDataChecksums(checksums);
...
void MergeTreeDataPartWriterCompact::cancel() noexcept {
  ...
  if (marks_source_hashing) marks_source_hashing->cancel();

  if (marks_compressor) marks_compressor->cancel();
