...
bool hasMaterializedTextIndex(const StorageSnapshotPtr &storage_snapshot, const IMergeTreeDataPartInfoForReader &data_part_info_for_reader,
                              const String &virtual_column_name) {
  if (!storage_snapshot->virtual_columns) return false;

  const auto *virtual_column = storage_snapshot->virtual_columns->tryGetDescription(virtual_column_name);
  if (!virtual_column) return false;
...
bool injectRequiredColumnsRecursively(const String &column_name, const StorageSnapshotPtr &storage_snapshot,
                                      const AlterConversionsPtr &alter_conversions,
                                      const IMergeTreeDataPartInfoForReader &data_part_info_for_reader, const GetColumnsOptions &options,
                                      Names &columns, NameSet &required_columns, NameSet &injected_columns) {
  ...
  if (!default_expression) return false;
  ...
  for (const auto &identifier : identifiers)
    result |= injectRequiredColumnsRecursively(identifier, storage_snapshot, alter_conversions, data_part_info_for_reader, options, columns,
                                               required_columns, injected_columns);
 ...
 */
NameSet injectRequiredColumns(const IMergeTreeDataPartInfoForReader &data_part_info_for_reader, const StorageSnapshotPtr &storage_snapshot,
                              bool with_subcolumns, Names &columns) {
  NameSet required_columns{std::begin(columns), std::end(columns)};
  NameSet injected_columns;

  bool have_at_least_one_physical_column = false;
  AlterConversionsPtr alter_conversions;
  if (!data_part_info_for_reader.isProjectionPart()) alter_conversions = data_part_info_for_reader.getAlterConversions();
  ...
  for (size_t i = 0; i < columns.size(); ++i) {
    ...
    have_at_least_one_physical_column |= injectRequiredColumnsRecursively(
        columns[i], storage_snapshot, alter_conversions, data_part_info_for_reader, options, columns, required_columns, injected_columns);
  }
  ...
  if (!have_at_least_one_physical_column) {
    ...
    for (const auto &column : part_columns) {
      if (storage_snapshot->tryGetColumn(options, column.name)) available_columns.push_back(column);
    }

    if (available_columns.empty()) available_columns = part_columns;
...
void MergeTreeBlockSizePredictor::initialize(const Block &sample_block, const Columns &columns, const Names &names, bool from_update) {
  ...
  if (!from_update) names_set.insert(names.begin(), names.end());
  ...
  for (size_t pos = 0; pos < num_columns; ++pos) {
    ...
    if (!from_update && !names_set.contains(column_name)) continue;

    /// At least PREWHERE filter column might be const.
    if (typeid_cast<const ColumnConst *>(column_data.get())) continue;
    ...
    if ((!column_from_part || !column_from_part->isSubcolumn()) && column_data->valuesHaveFixedSize()) {
      ...
      info.bytes_per_row_global =
          column_size.data_uncompressed
...
void MergeTreeBlockSizePredictor::startBlock() {
  block_size_bytes = 0;
  block_size_rows = 0;
  for (auto &info : dynamic_columns_infos) info.size_bytes = 0;
}
...
void addPatchPartsColumns(MergeTreeReadTaskColumns &result, const StorageSnapshotPtr &storage_snapshot, const GetColumnsOptions &options,
                          const PatchPartsForReader &patch_parts, const Names &all_columns_to_read, bool has_lightweight_delete) {
  if (patch_parts.empty()) return;
  ...
  for (size_t i = 0; i < patch_parts.size(); ++i) {
    ...
    for (const auto &column_name : all_columns_to_read) {
      ...
      if (!patch_part_columns.hasPhysical(column_name_in_patch)) continue;
...
MergeTreeReadTaskColumns getReadTaskColumns(const IMergeTreeDataPartInfoForReader &data_part_info_for_reader,
                                            const StorageSnapshotPtr &storage_snapshot, const Names &required_columns,
                                            const FilterDAGInfoPtr &row_level_filter, const PrewhereInfoPtr &prewhere_info,
                                            const PrewhereExprSteps &mutation_steps, const IndexReadTasks &index_read_tasks,
                                            const ExpressionActionsSettings &actions_settings,
                                            const MergeTreeReaderSettings &reader_settings, bool with_subcolumns) {
  ...
  auto add_step = [&](const PrewhereExprStep &step) {
    ...
    for (const auto &name : required_source_columns) {
      if (!columns_from_previous_steps.contains(name)) step_column_names.push_back(name);
    }
    ...
    for (const auto &name : step_column_names) {
      if (columns_from_previous_steps.emplace(name).second) columns_to_read_in_step.push_back(name);
    }

    /// Add results of the step to the list of already "known" columns so that we don't read or compute them again.
    if (step.actions) {
      for (const auto &name : step.actions->getActionsDAG().getNames()) columns_from_previous_steps.insert(name);
    }

    result.pre_columns.push_back(storage_snapshot->getColumnsByNames(options, columns_to_read_in_step));
  };

  for (const auto &step : mutation_steps) add_step(*step);
  ...
  if (prewhere_info || row_level_filter || !index_read_tasks.empty()) {
    auto prewhere_actions = MergeTreeSelectProcessor::getPrewhereActions(
        row_level_filter, prewhere_info, index_read_tasks, actions_settings, reader_settings.enable_multiple_prewhere_read_steps,
        reader_settings.force_short_circuit_execution);

    for (const auto &step : prewhere_actions.steps) add_step(*step);
  }

  /// Remove columns read in prewehere from the list of columns to read.
  Names post_column_names;
  for (const auto &name : column_to_read_after_prewhere) {
    if (!columns_from_previous_steps.contains(name)) post_column_names.push_back(name);
  }
...
MergeTreeReadTaskColumns getReadTaskColumnsForMerge(const IMergeTreeDataPartInfoForReader &data_part_info_for_reader,
                                                    const StorageSnapshotPtr &storage_snapshot, const Names &required_columns,
                                                    const PrewhereExprSteps &mutation_steps) {
  return getReadTaskColumns(data_part_info_for_reader, storage_snapshot, required_columns,
                            /*row_level_filter=*/nullptr,
                            /*prewhere_info=*/nullptr, mutation_steps,
                            /*index_read_tasks*/ {},
                            /*actions_settings=*/{},
