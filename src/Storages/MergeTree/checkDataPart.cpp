...
bool isNotEnoughMemoryErrorCode(int code) {
  ...
  return code == ErrorCodes::MEMORY_LIMIT_EXCEEDED || code == ErrorCodes::CANNOT_ALLOCATE_MEMORY || code == ErrorCodes::CANNOT_MUNMAP ||
...
bool isRetryableException(std::exception_ptr exception_ptr) {
  try {
    ...
    return e.getErrno() == EMFILE || e.getErrno() == ENOMEM || isNotEnoughMemoryErrorCode(e.code()) ||
           e.code() == ErrorCodes::NETWORK_ERROR || e.code() == ErrorCodes::SOCKET_TIMEOUT ||
           e.code() == ErrorCodes::CANNOT_SCHEDULE_TASK || e.code() == ErrorCodes::ABORTED;
    ...
    return isNotEnoughMemoryErrorCode(e.code()) || e.code() == ErrorCodes::NETWORK_ERROR || e.code() == ErrorCodes::SOCKET_TIMEOUT ||
           e.code() == ErrorCodes::CANNOT_SCHEDULE_TASK || e.code() == ErrorCodes::ABORTED ||
           e.code() == ErrorCodes::CANNOT_WRITE_TO_OSTREAM;
  } catch (const std::filesystem::filesystem_error &e) {
    return e.code() == std::errc::no_space_on_device || e.code() == std::errc::read_only_file_system ||
           e.code() == std::errc::too_many_files_open_in_system || e.code() == std::errc::operation_not_permitted ||
           e.code() == std::errc::device_or_resource_busy || e.code() == std::errc::permission_denied ||
           e.code() == std::errc::too_many_files_open || e.code() == std::errc::text_file_busy || e.code() == std::errc::timed_out ||
           e.code() == std::errc::not_enough_memory || e.code() == std::errc::not_supported || e.code() == std::errc::too_many_links ||
...
static IMergeTreeDataPart::Checksums checkDataPart(MergeTreeData::DataPartPtr data_part, const IDataPartStorage &data_part_storage,
                                                   const NamesAndTypesList &columns_list, const MergeTreeDataPartType &part_type,
                                                   const NameSet &files_without_checksums, const ReadSettings &read_settings,
                                                   bool require_checksums, std::function<bool()> is_cancelled, bool &is_broken_projection,
  ...
  auto checksum_compressed_file = [&read_settings](const IDataPartStorage &data_part_storage_, const String &file_path) {
    ...
    return IMergeTreeDataPart::Checksums::Checksum{compressed_hashing_buf.count(), compressed_hashing_buf.getHash(),
  ...
  if (data_part_storage.existsFile(IMergeTreeDataPart::SERIALIZATION_FILE_NAME)) {
    try {
      ...
      throw Exception(ErrorCodes::CORRUPTED_DATA, "Failed to load file {} of data part {}, with error {}",
                      IMergeTreeDataPart::SERIALIZATION_FILE_NAME, data_part->name, getCurrentExceptionMessage(true));
    }
  }

  auto get_serialization = [&serialization_infos](const auto &column) {
    auto it = serialization_infos.find(column.name);
    return it == serialization_infos.end() ? column.type->getSerialization(serialization_infos.getSettings())
  ...
  if (part_type == MergeTreeDataPartType::Compact) {
    ...
    for (const auto &column : columns_list) {
      get_serialization(column)->enumerateStreams(
          [&](const ISerialization::SubstreamPath &substream_path) {
            ...
            if (!stream_name)
              throw Exception(ErrorCodes::NO_FILE_IN_DATA_PART, "There is no file for column '{}' in data part '{}'", column.name,
                              data_part->name);
  ...
  /// Checksums from the rest files listed in checksums.txt. May be absent. If present, they are subsequently compared with the actual data
  /// checksums.
  ...
  for (const auto &[name, projection] : data_part->getProjectionParts()) {
    ...
    try {
      bool noop;
      projection_checksums = checkDataPart(projection, *data_part_storage.getProjection(projection_file), projection->getColumns(),
                                           projection->getType(), projection->getFileNamesWithoutChecksums(), read_settings,
                                           require_checksums, is_cancelled, noop, /* throw_on_broken_projection */ false);
    } catch (...) {
      ...
      if (throw_on_broken_projection) {
        if (!broken_projections_message.empty()) broken_projections_message += "\n";

        broken_projections_message +=
            fmt::format("Part `{}` has broken projection `{}` (error: {})", data_part->name, name, exception_message);
      }

      continue;
    }

    checksums_data.files[projection_file] =
        IMergeTreeDataPart::Checksums::Checksum(projection_checksums.getTotalSizeOnDisk(), projection_checksums.getTotalChecksumUInt128());
  ...
  if (throw_on_broken_projection) {
    ...
    if (require_checksums && !projections_on_disk.empty()) {
      throw Exception(ErrorCodes::UNEXPECTED_FILE_IN_DATA_PART, "Found unexpected projection directories: {}",
  ...
  if (require_checksums || !checksums_txt.files.empty()) checksums_txt.checkEqual(checksums_data, check_uncompressed, data_part->name);

  return checksums_data;
}

IMergeTreeDataPart::Checksums checkDataPart(MergeTreeData::DataPartPtr data_part, bool require_checksums, bool &is_broken_projection,
                                            std::function<bool()> is_cancelled, bool throw_on_broken_projection) {
  ...
  auto drop_cache_and_check = [&] {
    ...
    LOG_DEBUG(getLogger("checkDataPart"), "Will drop cache for data part {} and will check it once again", data_part->name);
    ...
    for (auto it = data_part_storage.iterate(); it->isValid(); it->next()) {
      ...
      if (!data_part_storage.existsDirectory(file_name)) {
        ...
        for (const auto &remote_path : remote_paths) cache.removePathIfExists(remote_path, FileCache::getCommonOrigin().user_id);
      }
    }
    ...
    try {
      return checkDataPart(data_part, data_part_storage, data_part->getColumns(), data_part->getType(),
                           data_part->getFileNamesWithoutChecksums(), read_settings, require_checksums, is_cancelled, is_broken_projection,
                           throw_on_broken_projection);
    } catch (...) {
      if (isRetryableException(std::current_exception())) {
        LOG_DEBUG(getLogger("checkDataPart"), "Got retriable error {} checking data part {}, will return empty", data_part->name,
  ...
  try {
    ReadSettings read_settings;
    return checkDataPart(data_part, data_part->getDataPartStorage(), data_part->getColumns(), data_part->getType(),
                         data_part->getFileNamesWithoutChecksums(), read_settings, require_checksums, is_cancelled, is_broken_projection,
                         throw_on_broken_projection);
  } catch (...) {
    if (isRetryableException(std::current_exception())) {
