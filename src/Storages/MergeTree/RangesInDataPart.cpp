...
void RangesInDataPartDescription::serialize(WriteBuffer& out, UInt64 parallel_protocol_version) const {
  ...
  if (parallel_protocol_version >= DBMS_PARALLEL_REPLICAS_MIN_VERSION_WITH_PROJECTION) writeBinary(projection_name, out);
}
...
String RangesInDataPartDescription::getPartOrProjectionName() const {
  if (projection_name.empty()) return info.getPartNameV1();
...
void RangesInDataPartDescription::deserialize(ReadBuffer& in, UInt64 parallel_protocol_version) {
  ...
  if (parallel_protocol_version >= DBMS_PARALLEL_REPLICAS_MIN_VERSION_WITH_PROJECTION) readBinary(projection_name, in);
}
...
void RangesInDataPartsDescription::serialize(WriteBuffer& out, UInt64 parallel_protocol_version) const {
  ...
  for (const auto& desc : *this) desc.serialize(out, parallel_protocol_version);
}

String RangesInDataPartsDescription::describe() const { return fmt::format("{} parts: [{}]", this->size(), fmt::join(*this, ", ")); }
...
void RangesInDataPartsDescription::deserialize(ReadBuffer& in, UInt64 parallel_protocol_version) {
  ...
  for (auto& desc : *this) desc.deserialize(in, parallel_protocol_version);
}

void RangesInDataPartsDescription::merge(const RangesInDataPartsDescription& other) {
  for (const auto& desc : other) this->emplace_back(desc);
}

RangesInDataPart::RangesInDataPart(const DataPartPtr& data_part_, const DataPartPtr& parent_part_, size_t part_index_in_query_,
                                   size_t part_starting_offset_in_query_, const MarkRanges& ranges_)
...
RangesInDataPart::RangesInDataPart(const DataPartPtr& data_part_, const DataPartPtr& parent_part_, size_t part_index_in_query_,
  ...
  if (total_marks_count) ranges.emplace_back(0, total_marks_count);
}
...
size_t RangesInDataPart::getMarksCount() const {
  size_t total = 0;
  for (const auto& range : ranges) total += range.end - range.begin;

  return total;
}

size_t RangesInDataPart::getRowsCount() const { return data_part->index_granularity->getRowsCountInRanges(ranges); }
...
RangesInDataPartsDescription RangesInDataParts::getDescriptions() const {
  ...
  for (const auto& part : *this) result.emplace_back(part.getDescription());
  return result;
}

size_t RangesInDataParts::getMarksCountAllParts() const {
  size_t result = 0;
  for (const auto& part : *this) result += part.getMarksCount();
  return result;
}

size_t RangesInDataParts::getRowsCountAllParts() const {
  size_t result = 0;
  for (const auto& part : *this) result += part.getRowsCount();
  return result;
}
