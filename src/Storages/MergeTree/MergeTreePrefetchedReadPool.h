...
class MergeTreePrefetchedReadPool : public MergeTreeReadPoolBase {
 public:
  MergeTreePrefetchedReadPool(RangesInDataParts&& parts_, MutationsSnapshotPtr mutations_snapshot_, VirtualFields shared_virtual_fields_,
 ...
 private:
  ...
  class PrefetchedReaders {
   public:
    PrefetchedReaders(ThreadPool& pool, MergeTreeReadTask::Readers readers_, Priority priority_,
  ...
  struct ThreadTask {
    ...
    ~ThreadTask() {
      if (readers_future && readers_future->valid()) readers_future->wait();
    }

    bool isValidReadersFuture() const { return readers_future && readers_future->valid(); }
  ...
  struct PrefetchIncrement : boost::noncopyable {
    explicit PrefetchIncrement(std::shared_ptr<AsyncReadCounters> counters_) : counters(counters_) {
