...
class MergeTask {
 public:
  ...
  MergeTask(FutureMergedMutatedPartPtr future_part_, StorageMetadataPtr metadata_snapshot_, MergeList::Entry* merge_entry_,
            std::unique_ptr<MergeListElement> projection_merge_list_element_, time_t time_of_merge_, ContextPtr context_,
            TableLockHolder& holder, ReservationSharedPtr space_reservation_, bool deduplicate_, Names deduplicate_by_columns_,
            bool cleanup_, MergeTreeData::MergingParams merging_params_, bool need_prefix, ProjectionDescriptionRawPtr projection_,
            IMergeTreeDataPart* parent_part_, MergedPartOffsetsPtr merged_part_offsets_, String suffix_, MergeTreeTransactionPtr txn,
  ...
  std::future<MergeTreeData::MutableDataPartPtr> getFuture() { return global_ctx->promise.get_future(); }

  MergeTreeData::MutableDataPartPtr getUnfinishedPart() {
    if (global_ctx) return global_ctx->new_data_part;
    return nullptr;
  }
 ...
 private:
  ...
  struct ExecuteAndFinalizeHorizontalPart : public IStage {
    ...
    const ExecuteAndFinalizeHorizontalPartSubtasks subtasks{&ExecuteAndFinalizeHorizontalPart::prepare,
  ...
  struct VerticalMergeRuntimeContext : public IStageRuntimeContext {
    ...
    enum class State : uint8_t { NEED_PREPARE, NEED_EXECUTE, NEED_FINISH };
  ...
  struct VerticalMergeStage : public IStage {
    ...
    const VerticalMergeStageSubtasks subtasks{&VerticalMergeStage::prepareVerticalMergeForAllColumns,
  ...
  struct MergeProjectionsStage : public IStage {
    ...
    const MergeProjectionsStageSubtasks subtasks{&MergeProjectionsStage::prepareProjections, &MergeProjectionsStage::executeProjections,
  ...
  const Stages stages{std::make_shared<ExecuteAndFinalizeHorizontalPart>(), std::make_shared<VerticalMergeStage>(),
                      std::make_shared<MergeTextIndexStage>(), std::make_shared<MergeProjectionsStage>()};
  ...
  static void addSkipIndexesExpressionSteps(QueryPlan& plan, const IndicesDescription& indices_description,
  ...
  static BuildStatisticsTransformPtr addBuildStatisticsStep(QueryPlan& plan, const IMergeTreeDataPart& data_part,
                                                            const GlobalRuntimeContextPtr& global_ctx);
};
