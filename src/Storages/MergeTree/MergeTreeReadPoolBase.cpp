...
MergeTreeReadPoolBase::MergeTreeReadPoolBase(RangesInDataParts&& parts_, MutationsSnapshotPtr mutations_snapshot_,
                                             VirtualFields shared_virtual_fields_, const IndexReadTasks& index_read_tasks_,
                                             const StorageSnapshotPtr& storage_snapshot_, const FilterDAGInfoPtr& row_level_filter_,
                                             const PrewhereInfoPtr& prewhere_info_, const ExpressionActionsSettings& actions_settings_,
                                             const MergeTreeReaderSettings& reader_settings_, const Names& column_names_,
                                             const PoolSettings& pool_settings_,
                                             const MergeTreeReadTask::BlockSizeParams& block_size_params_, const ContextPtr& context_)
...
MergeTreeReadPoolBase::MergeTreeReadPoolBase(MutationsSnapshotPtr mutations_snapshot_, const StorageSnapshotPtr& storage_snapshot_,
                                             const PrewhereInfoPtr& prewhere_info_, const ExpressionActionsSettings& actions_settings_,
                                             const MergeTreeReaderSettings& reader_settings_, const Names& column_names_,
                                             const PoolSettings& pool_settings_,
                                             const MergeTreeReadTask::BlockSizeParams& block_size_params_, const ContextPtr& context_)
...
static size_t getSizeOfColumns(const IMergeTreeDataPart& part, const Names& columns_to_read, const Settings& settings) {
  ...
  if (part.getType() == MergeTreeDataPartType::Compact) return part.getBytesOnDisk();
...
/// Columns from different prewhere steps are read independently, so it makes sense to use the heaviest set of columns among them as an
/// estimation.
static Names getHeaviestSetOfColumnsAmongPrewhereSteps(const IMergeTreeDataPart& part,
                                                       const std::vector<NamesAndTypesList>& prewhere_steps_columns,
                                                       const Settings& settings) {
  const auto it = std::ranges::max_element(prewhere_steps_columns, [&](const auto& lhs, const auto& rhs) {
...
calculateMinMarksPerTask(const RangesInDataPart& part, const Names& columns_to_read,
                         const std::vector<NamesAndTypesList>& prewhere_steps_columns,
                         const MergeTreeReadPoolBase::PoolSettings& pool_settings, const Settings& settings) {
  ...
  if (part_marks_count) {
    if (part.data_part->isStoredOnRemoteDisk()) {
      ...
      const auto heuristic_min_marks = std::min<size_t>(
          pool_settings.sum_marks / (pool_settings.threads * pool_settings.total_query_nodes) / 2, min_bytes_per_task / avg_mark_bytes);

      if (heuristic_min_marks > min_marks_per_task) {
        LOG_TEST(&Poco::Logger::get("MergeTreeReadPoolBase"), "Increasing min_marks_per_task from {} to {} based on columns size heuristic",
...
MergeTreeReadTaskInfo MergeTreeReadPoolBase::buildReadTaskInfo(const RangesInDataPart& part_with_ranges, const Settings& settings) const {
  ...
  if (read_task_info.data_part->isProjectionPart() && !read_task_info.parent_part) {
    throw Exception(ErrorCodes::LOGICAL_ERROR, "Did not find parent part {} for projection part {}",
                    read_task_info.data_part->getParentPartName(), read_task_info.data_part->getDataPartStorage().getFullPath());
  }
  ...
  bool has_lightweight_delete =
      read_task_info.data_part->hasLightweightDelete() || read_task_info.alter_conversions->hasLightweightDelete();
  ...
  read_task_info.task_columns = getReadTaskColumns(part_info, storage_snapshot, column_names, row_level_filter, prewhere_info,
                                                   read_task_info.mutation_steps, index_read_tasks, actions_settings, reader_settings,
  ...
  if (read_task_info.alter_conversions->hasPatches()) {
    ...
    read_task_info.patch_parts =
        read_task_info.alter_conversions->getPatchesForColumns(all_read_columns_list, reader_settings.apply_deleted_mask);

    addPatchPartsColumns(read_task_info.task_columns, storage_snapshot, options, read_task_info.patch_parts, all_read_columns,
  ...
  if (pool_settings.preferred_block_size_bytes > 0) {
    ...
    for (const auto& column_name : all_column_names) {
      if (auto column_in_part = read_task_info.data_part->tryGetColumn(column_name))
        sample_block_from_part.insert(
            ColumnWithTypeAndName(column_in_part->type->createColumn(), column_in_part->type, column_in_part->name));
    }

    read_task_info.shared_size_predictor = std::make_unique<MergeTreeBlockSizePredictor>(
        read_task_info.data_part, Names(all_column_names.begin(), all_column_names.end()), sample_block_from_part,
...
std::vector<size_t> MergeTreeReadPoolBase::getPerPartSumMarks() const {
  ...
  for (const auto& part_with_ranges : parts_ranges) {
    size_t sum_marks = 0;
    for (const auto& range : part_with_ranges.ranges) sum_marks += range.end - range.begin;
...
MergeTreeReadTaskPtr MergeTreeReadPoolBase::createTask(MergeTreeReadTaskInfoPtr read_info, MergeTreeReadTask::Readers task_readers,
                                                       MarkRanges ranges, std::vector<MarkRanges> patches_ranges,
  ...
  return std::make_unique<MergeTreeReadTask>(read_info, std::move(task_readers), std::move(ranges), std::move(patches_ranges),
                                             block_size_params, std::move(task_size_predictor), updater);
}

MergeTreeReadTaskPtr MergeTreeReadPoolBase::createTask(MergeTreeReadTaskInfoPtr read_info, MarkRanges ranges,
                                                       std::vector<MarkRanges> patches_ranges, MergeTreeReadTask* previous_task,
                                                       RuntimeDataflowStatisticsCacheUpdaterPtr updater) const {
