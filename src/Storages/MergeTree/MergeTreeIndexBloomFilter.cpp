...
MergeTreeIndexGranuleBloomFilter::MergeTreeIndexGranuleBloomFilter(size_t bits_per_row_, size_t hash_functions_,
  ...
  if (column_hashes_.empty()) throw Exception(ErrorCodes::LOGICAL_ERROR, "Granule_index_blocks empty or total_rows is zero.");

  size_t bloom_filter_max_size = 0;
  for (const auto &column_hash : column_hashes_) bloom_filter_max_size = std::max(bloom_filter_max_size, column_hash.size());
...
bool MergeTreeIndexGranuleBloomFilter::empty() const { return !total_rows; }

size_t MergeTreeIndexGranuleBloomFilter::memoryUsageBytes() const {
  size_t sum = 0;
  for (const auto &bloom_filter : bloom_filters) sum += bloom_filter->memoryUsageBytes();
  return sum;
}

void MergeTreeIndexGranuleBloomFilter::deserializeBinary(ReadBuffer &istr, MergeTreeIndexVersion version) {
  if (version != 1) throw Exception(ErrorCodes::LOGICAL_ERROR, "Unknown index version {}.", version);
...
void MergeTreeIndexGranuleBloomFilter::serializeBinary(WriteBuffer &ostr) const {
  if (empty()) throw Exception(ErrorCodes::LOGICAL_ERROR, "Attempt to write empty bloom filter index.");
...
void MergeTreeIndexGranuleBloomFilter::fillingBloomFilter(BloomFilterPtr &bf, const HashSet<UInt64> &hashes) const {
  for (const auto &bf_base_hash : hashes)
    for (size_t i = 0; i < hash_functions; ++i) bf->addHashWithSeed(bf_base_hash.getKey(), BloomFilterHash::bf_hash_seed[i]);
}

namespace {

ColumnWithTypeAndName getPreparedSetInfo(const ConstSetPtr &prepared_set) {
  if (prepared_set->getDataTypes().size() == 1) return {prepared_set->getSetElements()[0], prepared_set->getElementsTypes()[0], "dummy"};

  Columns set_elements;
  for (auto &set_element : prepared_set->getSetElements()) set_elements.emplace_back(set_element->convertToFullColumnIfConst());
...
bool hashMatchesFilter(const BloomFilterPtr &bloom_filter, UInt64 hash, size_t hash_functions) {
  return std::all_of(BloomFilterHash::bf_hash_seed, BloomFilterHash::bf_hash_seed + hash_functions,
                     [&](const auto &hash_seed) { return bloom_filter->findHashWithSeed(hash, hash_seed); });
}
...
bool maybeTrueOnBloomFilter(const IColumn *hash_column, const BloomFilterPtr &bloom_filter, size_t hash_functions, bool match_all) {
  ...
  if (!const_column && !non_const_column) throw Exception(ErrorCodes::LOGICAL_ERROR, "Hash column must be Const or UInt64.");

  if (const_column) {
    return hashMatchesFilter(bloom_filter, const_column->getValue<UInt64>(), hash_functions);
  }
  ...
  if (match_all) {
    return std::all_of(hashes.begin(), hashes.end(),
                       [&](const auto &hash_row) { return hashMatchesFilter(bloom_filter, hash_row, hash_functions); });
  }
...
MergeTreeIndexConditionBloomFilter::MergeTreeIndexConditionBloomFilter(const ActionsDAG::Node *predicate, ContextPtr context_,
                                                                       const Block &header_, size_t hash_functions_)
  ...
  RPNBuilder<RPNElement> builder(predicate, context_,
...
bool MergeTreeIndexConditionBloomFilter::alwaysUnknownOrTrue() const {
  return rpnEvaluatesAlwaysUnknownOrTrue(
      rpn, {RPNElement::FUNCTION_EQUALS, RPNElement::FUNCTION_NOT_EQUALS, RPNElement::FUNCTION_HAS, RPNElement::FUNCTION_HAS_ANY,
            RPNElement::FUNCTION_HAS_ALL, RPNElement::FUNCTION_IN, RPNElement::FUNCTION_NOT_IN});
}

bool MergeTreeIndexConditionBloomFilter::mayBeTrueOnGranule(
    const MergeTreeIndexGranuleBloomFilter *granule, const UpdatePartialDisjunctionResultFn &update_partial_result_disjuntion_fn) const {
    ...
    switch (element.function) {
      ...
      case RPNElement::FUNCTION_HAS_ALL: {
        ...
        for (size_t index = 0; match_rows && index < predicate.size(); ++index) {
          ...
          match_rows = maybeTrueOnBloomFilter(&*hash_column, filter, hash_functions, match_all);
        }
...
bool MergeTreeIndexConditionBloomFilter::traverseFunction(const RPNBuilderTreeNode &node, RPNElement &out,
  ...
  if (parent == nullptr) {
    ...
    for (size_t i = 0; i < arguments_size; ++i) {
      ...
      if (traverseFunction(argument, out, &node)) return true;
    }
  }

  if (arguments_size != 2) return false;
  ...
  if ((function_name == "indexOf") != (parent != nullptr)) return false;
    ...
    if (auto future_set = rhs_argument.tryGetPreparedSet(); future_set) {
      if (auto prepared_set = future_set->buildOrderedSetInplace(rhs_argument.getTreeContext().getQueryContext()); prepared_set) {
        if (prepared_set->hasExplicitSetElements()) {
          ...
          if (traverseTreeIn(function_name, lhs_argument, prepared_set, prepared_info.type, prepared_info.column, out)) return true;
        }
      }
    }
    return false;
  ...
  if (function_name == "equals" || function_name == "notEquals" || function_name == "has" || function_name == "mapContains" ||
      function_name == "mapContainsKey" || function_name == "mapContainsValue" || function_name == "indexOf" || function_name == "hasAny" ||
      function_name == "hasAll") {
    Field const_value;
    DataTypePtr const_type;

    if (rhs_argument.tryGetConstant(const_value, const_type)) {
      if (traverseTreeEquals(function_name, lhs_argument, const_type, const_value, out, parent)) return true;
      ...
      if (traverseTreeEquals(function_name, rhs_argument, const_type, const_value, out, parent)) return true;
    }
...
bool MergeTreeIndexConditionBloomFilter::traverseTreeIn(const String &function_name, const RPNBuilderTreeNode &key_node,
                                                        const ConstSetPtr &prepared_set, const DataTypePtr &type, const ColumnPtr &column,
                                                        RPNElement &out) {
  ...
  if (header.has(key_node_column_name)) {
    ...
    if (function_name == "in" || function_name == "globalIn") out.function = RPNElement::FUNCTION_IN;

    if (function_name == "notIn" || function_name == "globalNotIn") out.function = RPNElement::FUNCTION_NOT_IN;
  ...
  if (key_node.isFunction()) {
    ...
    if (which.isTuple() && key_node_function_name == "tuple") {
      ...
      for (size_t index = 0; index < key_node_function_arguments_size; ++index)
        match_with_subtype |=
            traverseTreeIn(function_name, key_node_function.getArgumentAt(index), nullptr, sub_data_types[index], sub_columns[index], out);
    ...
    if (key_node_function_name == "arrayElement") {
      ...
      if (set_contain_default_value) return false;
...
static ColumnPtr createColumnFromConstantArray(const Field &value_field, const DataTypePtr &actual_type) {
  if (value_field.getType() != Field::Types::Array) return nullptr;
  ...
  for (const auto &f : value_field.safeGet<Array>()) {
    ...
    if (converted.isNull()) return nullptr;
...
static bool indexOfCanUseBloomFilter(const RPNBuilderTreeNode *parent) {
  ...
  if (function_name == "equals"  /// notEquals is not applicable
    ...
    if (function_arguments_size != 2) return false;
...
bool MergeTreeIndexConditionBloomFilter::traverseTreeEquals(const String &function_name, const RPNBuilderTreeNode &key_node,
                                                            const DataTypePtr &value_type, const Field &value_field, RPNElement &out,
                                                            const RPNBuilderTreeNode *parent) {
    ...
    if (function_name == "has" || function_name == "indexOf") {
      if (array_type) {
        ...
        if (function_name == "has" || indexOfCanUseBloomFilter(parent)) {
          ...
          if (converted_field.isNull()) return false;
  ...
  if (function_name == "mapContainsValue" || function_name == "mapContainsKey" || function_name == "mapContains" ||
    ...
    if (function_name == "mapContainsValue") map_keys_index_column_name = fmt::format("mapValues({})", key_column_name);

    if (!header.has(map_keys_index_column_name)) return false;
  ...
  if (key_node.isFunction()) {
    ...
    if (which.isTuple() && key_node_function_name == "tuple") {
      ...
      if (tuple.size() != key_node_function_arguments_size) return false;
      ...
      for (size_t index = 0; index < tuple.size(); ++index)
        match_with_subtype |=
            traverseTreeEquals(function_name, key_node_function.getArgumentAt(index), subtypes[index], tuple[index], out, &key_node);
    ...
    if (key_node_function_name == "arrayElement" && (function_name == "equals" || function_name == "notEquals")) {
       ...
       */
      if (value_field == value_type->getDefault()) return false;
      ...
      if (header.has(map_keys_index_column_name)) {
        ...
        if (!second_argument.tryGetConstant(const_value, const_type)) return false;
      } else if (header.has(map_values_index_column_name)) {
...
MergeTreeIndexAggregatorBloomFilter::MergeTreeIndexAggregatorBloomFilter(size_t bits_per_row_, size_t hash_functions_,
                                                                         const Names &columns_name_)
...
bool MergeTreeIndexAggregatorBloomFilter::empty() const { return !total_rows; }
...
void MergeTreeIndexAggregatorBloomFilter::update(const Block &block, size_t *pos, size_t limit) {
  ...
  for (size_t column = 0; column < index_columns_name.size(); ++column) {
    ...
    for (const auto &hash : index_data) column_hashes[column].insert(hash);
  }
...
MergeTreeIndexBloomFilter::MergeTreeIndexBloomFilter(const IndexDescription &index_, size_t bits_per_row_, size_t hash_functions_)
    : IMergeTreeIndex(index_), bits_per_row(bits_per_row_), hash_functions(hash_functions_) {
  assert(bits_per_row != 0);
  assert(hash_functions != 0);
}
...
static void assertIndexColumnsType(const Block &header) {
  if (header.empty() || !header.columns()) throw Exception(ErrorCodes::INCORRECT_QUERY, "Index must have columns.");
...
MergeTreeIndexPtr bloomFilterIndexCreator(const IndexDescription &index) {
  ...
  return std::make_shared<MergeTreeIndexBloomFilter>(index, bits_per_row_and_size_of_hash_functions.first,
                                                     bits_per_row_and_size_of_hash_functions.second);
}
