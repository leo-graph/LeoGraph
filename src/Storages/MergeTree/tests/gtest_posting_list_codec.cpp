...
std::vector<uint32_t> generateZeroData(size_t count) { return std::vector<uint32_t>(count, 0); }
...
void verifyRoundTrip(const std::vector<uint32_t> &original, std::optional<uint32_t> bits = std::nullopt) {
  ...
  if (bits.has_value()) needed_bytes = Portable::bitpackingCompressedBytes(original.size(), use_bits);
  ...
  ASSERT_EQ(static_cast<size_t>(used_encode), needed_bytes) << "encode used bytes must equal calculateNeededBytesAndMaxBits().first";
  ASSERT_EQ(out_span.size(), buffer.size() - needed_bytes) << "encode must advance output span by used bytes";
  ...
  ASSERT_EQ(used_decode, needed_bytes) << "decode used bytes must equal encode used bytes";
  ASSERT_EQ(in_span.size(), 0u) << "decode must consume exactly used bytes from input span";
  ...
  if (use_bits > 0) ASSERT_EQ(decoded, original) << "roundtrip mismatch";
}
}  // namespace
...
TEST(PostingListCodecTest, MaxbitsLengthAllZeros) {
  for (size_t count : {1, 4, 7, 16, 100, 128, 129, 256}) {
    ...
    std::span<uint32_t> span(data);
    EXPECT_EQ(Portable::maxbitsLength(span), 0u) << "Failed for count=" << count;
  }
}
...
TEST(PostingListCodecTest, MaxbitsLengthPowersOfTwoMinusOne) {
  for (uint32_t bits = 1; bits <= 32; ++bits) {
    ...
    std::span<uint32_t> span(data);
    EXPECT_EQ(Portable::maxbitsLength(span), bits) << "Failed for bits=" << bits << ", value=" << value;
  }
}
...
TEST(PostingListCodecTest, MaxbitsLengthExactPowerOfTwo) {
  for (uint32_t bits = 1; bits <= 31; ++bits) {
    ...
    std::vector<uint32_t> data = {value};
    std::span<uint32_t> span(data);
    EXPECT_EQ(Portable::maxbitsLength(span), bits + 1) << "Failed for bits=" << bits << ", value=" << value;
  }
}
...
TEST(PostingListCodecTest, MaxbitsLengthTailHandling) {
  for (size_t count = 1; count <= 10; ++count) {
    ...
    data.back() = 128;
    std::span<uint32_t> span(data);
    EXPECT_EQ(Portable::maxbitsLength(span), 8u) << "Failed for count=" << count;
  }
}
...
TEST(PostingListCodecTest, CompressedBytesZeroBits) {
  for (size_t count : {0, 1, 100, 128, 1000}) {
    EXPECT_EQ(Portable::bitpackingCompressedBytes(count, 0), 0u) << "Failed for count=" << count;
  }
}

TEST(PostingListCodecTest, CompressedBytesZeroCount) {
  for (uint32_t bits = 0; bits <= 32; ++bits) {
    EXPECT_EQ(Portable::bitpackingCompressedBytes(0, bits), 0u) << "Failed for bits=" << bits;
  }
}
...
TEST(PostingListCodecTest, CompressedBytes32Bits) {
  for (size_t count : {1, 4, 10, 100, 128}) {
    ...
    EXPECT_EQ(Portable::bitpackingCompressedBytes(count, 32), expected) << "Failed for count=" << count;
  }
}
...
TEST(PostingListCodecTest, CompressedBytesSingleGroup) {
  for (uint32_t bits = 1; bits <= 31; ++bits) {
    ...
    EXPECT_EQ(Portable::bitpackingCompressedBytes(4, bits), expected) << "Failed for bits=" << bits;
  }
}
...
TEST(PostingListCodecTest, CompressedBytesMultipleGroups) {
  ...
  for (const auto &tc : cases) {
    ...
    size_t expected = words32 * 16;
    EXPECT_EQ(Portable::bitpackingCompressedBytes(tc.count, tc.bits), expected) << "Failed for count=" << tc.count << ", bits=" << tc.bits;
  }
}
...
TEST(PostingListCodecTest, CompressedBytesNonMultipleOf4) {
  for (size_t count = 1; count <= 20; ++count) {
    for (uint32_t bits : {1u, 8u, 16u, 31u}) {
      ...
      size_t words32 = (groups * bits + 31) / 32;
      size_t expected = words32 * 16;
      EXPECT_EQ(Portable::bitpackingCompressedBytes(count, bits), expected) << "Failed for count=" << count << ", bits=" << bits;
    }
  }
}
...
[[maybe_unused]] static std::vector<uint32_t> makeAllZeros(size_t n) { return std::vector<uint32_t>(n, 0u); }

[[maybe_unused]] static std::vector<uint32_t> makeAllMax(size_t n, uint32_t bit) { return std::vector<uint32_t>(n, maskForBit(bit)); }
...
TEST(PostingListCodecTest, SpecialPatterns) {
  ...
  const std::vector<Case> cases = {{0, 0},    {1, 0},    {128, 0},  {1, 32},   {3, 32},   {127, 32}, {128, 32}, {129, 32}, {127, 31},
                                   {128, 31}, {129, 31}, {127, 17}, {128, 17}, {129, 17}, {5, 1},    {5, 2},    {5, 3},    {5, 30}};
