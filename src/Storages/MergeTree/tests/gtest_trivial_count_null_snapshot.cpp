...
TEST(SupportsTrivialCountOptimization, NullMutationsSnapshot) {
  ...
  auto storage = std::make_shared<StorageMergeTree>(StorageID("test_db", "test_table"), "store/test_trivial_count/", metadata,
                                                    LoadingStrictnessLevel::ATTACH, context,
                                                    /*date_column_name=*/"", MergeTreeData::MergingParams{}, std::move(storage_settings));
