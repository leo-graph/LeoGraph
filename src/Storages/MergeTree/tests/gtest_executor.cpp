...
class FakeExecutableTask : public IExecutableTask {
 public:
  ...
  bool executeStep() override {
    ...
    if (choice == 0) throw TestException();
...
class LambdaExecutableTask : public IExecutableTask {
 public:
  explicit LambdaExecutableTask(const String &name_, size_t step_count_, StepFunc step_func_ = {}, Int64 priority_value = 0)
      : name(name_), step_count(step_count_), step_func(step_func_), priority{priority_value} {}

  bool executeStep() override {
    if (step_func) step_func(name, step_count);
    return --step_count;
  }
...
TEST(Executor, Simple) {
  auto executor = std::make_shared<DB::MergeTreeBackgroundExecutor<RoundRobinRuntimeQueue>>(
      ...
      1,    // threads
      100,  // max_tasks
      CurrentMetrics::BackgroundMergesAndMutationsPoolTask, CurrentMetrics::BackgroundMergesAndMutationsPoolSize,
      ProfileEvents::CommonBackgroundExecutorTaskExecuteStepMicroseconds, ProfileEvents::CommonBackgroundExecutorTaskCancelMicroseconds,
      ProfileEvents::CommonBackgroundExecutorTaskResetMicroseconds, ProfileEvents::CommonBackgroundExecutorWaitMicroseconds);
  ...
  auto task = [&](const String &name, size_t) {
    schedule += name;
    if (schedule.size() == expected_schedule.size()) barrier.arrive_and_wait();
  };
...
TEST(Executor, RemoveTasks) {
  ...
  auto executor = std::make_shared<DB::MergeTreeBackgroundExecutor<RoundRobinRuntimeQueue>>(
      ThreadName::TEST_SCHEDULER, tasks_kinds, tasks_kinds * batch, CurrentMetrics::BackgroundMergesAndMutationsPoolTask,
      CurrentMetrics::BackgroundMergesAndMutationsPoolSize, ProfileEvents::CommonBackgroundExecutorTaskExecuteStepMicroseconds,
  ...
  for (size_t i = 0; i < batch; ++i)
    for (size_t j = 0; j < tasks_kinds; ++j) ASSERT_TRUE(executor->trySchedule(std::make_shared<FakeExecutableTask>(std::to_string(j))));

  std::vector<std::thread> threads(batch);

  auto remover_routine = [&]() {
    for (size_t j = 0; j < tasks_kinds; ++j) executor->removeTasksCorrespondingToStorage({"test", std::to_string(j)});
  };

  for (auto &thread : threads) thread = std::thread(remover_routine);
...
TEST(Executor, RemoveTasksStress) {
  ...
  auto executor = std::make_shared<DB::MergeTreeBackgroundExecutor<RoundRobinRuntimeQueue>>(
      ThreadName::TEST_SCHEDULER, tasks_kinds, tasks_kinds * batch * (schedulers_count + removers_count),
      CurrentMetrics::BackgroundMergesAndMutationsPoolTask, CurrentMetrics::BackgroundMergesAndMutationsPoolSize,
      ProfileEvents::CommonBackgroundExecutorTaskExecuteStepMicroseconds, ProfileEvents::CommonBackgroundExecutorTaskCancelMicroseconds,
      ProfileEvents::CommonBackgroundExecutorTaskResetMicroseconds, ProfileEvents::CommonBackgroundExecutorWaitMicroseconds);
  ...
  auto scheduler_routine = [&]() {
    ...
    for (size_t i = 0; i < batch; ++i)
      for (size_t j = 0; j < tasks_kinds; ++j) executor->trySchedule(std::make_shared<FakeExecutableTask>(std::to_string(j)));
  };
  ...
  for (auto &scheduler : schedulers) scheduler = std::thread(scheduler_routine);

  std::vector<std::thread> removers(removers_count);
  for (auto &remover : removers) remover = std::thread(remover_routine);
...
TEST(Executor, UpdatePolicy) {
  ...
  auto task = [&](const String &name, size_t) {
    schedule += name;
    if (schedule.size() == 5) executor->updateSchedulingPolicy(PriorityRuntimeQueue::name);
    if (schedule.size() == 12) executor->updateSchedulingPolicy(RoundRobinRuntimeQueue::name);
    if (schedule.size() == expected_schedule.size()) barrier.arrive_and_wait();
  };
...
class SlowDestructorTask : public IExecutableTask {
 public:
  SlowDestructorTask(const String &name_, std::latch &destruction_started_, std::chrono::milliseconds delay_)
      : name(name_), destruction_started(destruction_started_), delay(delay_) {}
...
TEST(Executor, SlowTaskDestructionBlocksRemoveTasks) {
  ...
  executor->trySchedule(
      std::make_shared<SlowDestructorTask>("slow_storage", destruction_started, std::chrono::milliseconds(DESTRUCTION_DELAY_MS)));
  ...
  EXPECT_LT(elapsed_ms, 100) << "removeTasksCorrespondingToStorage for an unrelated storage was blocked for " << elapsed_ms
                             << " ms by slow task destruction holding the mutex";
