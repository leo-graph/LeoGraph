...
TEST(PackedPartOffsetsTest, VeryLargeValues) {
  ...
  std::vector<UInt64> values = {std::numeric_limits<UInt64>::max() - 1000, std::numeric_limits<UInt64>::max() - 800,
                                std::numeric_limits<UInt64>::max() - 600,  std::numeric_limits<UInt64>::max() - 400,
                                std::numeric_limits<UInt64>::max() - 200,  std::numeric_limits<UInt64>::max() - 1};
...
TEST(MergedPartOffsetsTest, ManyValues) {
  ...
  for (int i = 0; i < 5000; ++i) part_indices.push_back(0);
  for (int i = 0; i < 5000; ++i) part_indices.push_back(1);
  for (int i = 0; i < 5000; ++i) part_indices.push_back(2);
