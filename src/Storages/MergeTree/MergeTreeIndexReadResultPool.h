...
class MergeTreeSkipIndexReader {
 public:
  MergeTreeSkipIndexReader(UsefulSkipIndexes skip_indexes_, std::optional<KeyCondition> &key_condition_rpn_template_,
                           bool use_for_disjunctions_, MarkCachePtr mark_cache_, UncompressedCachePtr uncompressed_cache_,
                           VectorSimilarityIndexCachePtr vector_similarity_index_cache_, MergeTreeReaderSettings reader_settings_,
                           LoggerPtr log_);
...
class SingleProjectionIndexReader {
 public:
  SingleProjectionIndexReader(std::shared_ptr<MergeTreeReadPoolProjectionIndex> pool, PrewhereInfoPtr prewhere_info,
                              const ExpressionActionsSettings &actions_settings, const MergeTreeReaderSettings &reader_settings);
...
class MergeTreeIndexReadResultPool {
 public:
  MergeTreeIndexReadResultPool(MergeTreeSkipIndexReaderPtr skip_index_reader_, MergeTreeProjectionIndexReaderPtr projection_index_reader_);
  ...
  struct IndexReadResultEntry {
    ...
    IndexReadResultEntry()
        : promise(std::make_shared<std::promise<MergeTreeIndexReadResultPtr>>()), future(promise->get_future().share()) {}
  };
