...
void ColumnsSubstreams::addColumn(const String& column) {
  if (!columns_substreams.empty() && columns_substreams.back().second.empty())
    throw Exception(ErrorCodes::LOGICAL_ERROR, "Cannot add new column {} to ColumnsSubstreams: previous column {} has empty substreams",
                    column, columns_substreams.back().first);
...
void ColumnsSubstreams::addSubstreamToLastColumn(const String& substream) {
  ...
  if (column_position_to_substream_positions[columns_substreams.size() - 1].contains(substream)) return;
...
void ColumnsSubstreams::addSubstreamsToLastColumn(const std::vector<String>& substreams) {
  for (const auto& substream : substreams) addSubstreamToLastColumn(substream);
}
...
size_t ColumnsSubstreams::getSubstreamPosition(size_t column_position, const String& substream) const {
  if (column_position >= columns_substreams.size())
    throw Exception(ErrorCodes::LOGICAL_ERROR,
                    "Cannot get position for substream {}: column position {} is invalid, there are only {} columns", substream,
                    column_position, columns_substreams.size());
  ...
  if (it == column_position_to_substream_positions[column_position].end())
    throw Exception(ErrorCodes::LOGICAL_ERROR,
                    "Cannot get position for substream {}: column {} with position {} doesn't have such substream", substream,
                    columns_substreams[column_position].first, column_position);

  return it->second;
}

std::optional<size_t> ColumnsSubstreams::tryGetSubstreamPosition(size_t column_position, const String& substream) const {
  if (column_position >= columns_substreams.size()) return std::nullopt;

  auto it = column_position_to_substream_positions[column_position].find(substream);
  if (it == column_position_to_substream_positions[column_position].end()) return std::nullopt;

  return it->second;
}

size_t ColumnsSubstreams::getSubstreamPosition(size_t column_position, const NameAndTypePair& name_and_type,
  ...
  if (auto position = tryGetSubstreamPosition(column_position, substream)) return *position;
  ...
  throw Exception(ErrorCodes::LOGICAL_ERROR, "Cannot get position for substream {}: column {} with position {} doesn't have such substream",
                  substream, name_and_type.name, column_position);
}
...
std::optional<size_t> ColumnsSubstreams::tryGetSubstreamPosition(const String& substream) const {
  for (const auto& substream_to_position : column_position_to_substream_positions) {
    ...
    if (it != substream_to_position.end()) return it->second;
  }
...
size_t ColumnsSubstreams::getFirstSubstreamPosition(size_t column_position) const {
  if (column_position >= columns_substreams.size())
    throw Exception(ErrorCodes::LOGICAL_ERROR,
                    "Cannot get first substream position: column position {} is invalid, there are only {} columns", column_position,
                    columns_substreams.size());

  if (columns_substreams[column_position].second.empty())
    throw Exception(ErrorCodes::LOGICAL_ERROR, "Cannot get first substream position: column {} with position {} doesn't have substreams",
                    columns_substreams[column_position].first, column_position);
...
size_t ColumnsSubstreams::getLastSubstreamPosition(size_t column_position) const {
  if (column_position >= columns_substreams.size())
    throw Exception(ErrorCodes::LOGICAL_ERROR,
                    "Cannot get last substream position: column position {} is invalid, there are only {} columns", column_position,
                    columns_substreams.size());

  if (columns_substreams[column_position].second.empty())
    throw Exception(ErrorCodes::LOGICAL_ERROR, "Cannot get last substream position: column {} with position {} doesn't have substreams",
                    columns_substreams[column_position].first, column_position);
...
const std::vector<String>& ColumnsSubstreams::getColumnSubstreams(size_t column_position) const {
  if (column_position >= columns_substreams.size())
    throw Exception(ErrorCodes::LOGICAL_ERROR, "Cannot get substreams: column position {} is invalid, there are only {} columns",
                    column_position, columns_substreams.size());

  return columns_substreams[column_position].second;
}

ColumnsSubstreams ColumnsSubstreams::merge(const ColumnsSubstreams& left, const ColumnsSubstreams& right,
                                           const std::vector<String>& columns_order) {
  ...
  for (size_t i = 0; i != left.columns_substreams.size(); ++i) left_column_to_position[left.columns_substreams[i].first] = i;
  ...
  for (size_t i = 0; i != right.columns_substreams.size(); ++i) right_column_to_position[right.columns_substreams[i].first] = i;
...
void ColumnsSubstreams::validateColumns(const std::vector<String>& columns) const {
  if (columns.size() != columns_substreams.size())
    throw Exception(ErrorCodes::LOGICAL_ERROR, "Invalid columns substreams: expected {} columns, got {}", columns.size(),
                    columns_substreams.size());

  for (size_t i = 0; i != columns_substreams.size(); ++i) {
    if (columns_substreams[i].first != columns[i])
      throw Exception(ErrorCodes::LOGICAL_ERROR, "Unexpected column at position {} in columns substreams: expected {}, got {}", i,
                      columns[i], columns_substreams[i].first);
  }
}
