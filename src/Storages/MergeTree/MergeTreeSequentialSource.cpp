...
class MergeTreeSequentialSource : public ISource {
 public:
  MergeTreeSequentialSource(SharedHeader result_header, MergeTreeSequentialSourceType type, const MergeTreeData &storage_,
                            StorageSnapshotPtr storage_snapshot_, MergeTreeReadTaskInfoPtr read_task_info_,
                            std::optional<MarkRanges> mark_ranges_, bool read_with_direct_io_, bool prefetch);
...
MergeTreeSequentialSource::MergeTreeSequentialSource(SharedHeader result_header, MergeTreeSequentialSourceType type,
                                                     const MergeTreeData &storage_, StorageSnapshotPtr storage_snapshot_,
                                                     MergeTreeReadTaskInfoPtr read_task_info_, std::optional<MarkRanges> mark_ranges_,
                                                     bool read_with_direct_io_, bool prefetch)
  ...
  if (columns_to_read.size() == 1)
    ...
    LOG_DEBUG(log, "Reading {} marks from part {}, total {} rows starting from the beginning of the part", data_part->getMarksCount(),
              data_part->name, data_part->rows_count);
  ...
  if (!read_task_info->patch_parts.empty()) {
    ...
    patch_join_cache =
        std::make_shared<PatchJoinCache>(storage.getContext()->getSettingsRef()[Setting::apply_patch_parts_join_cache_buckets]);
  ...
  if (read_with_direct_io) read_settings.direct_io_threshold = 1;
  ...
  if (!readers.prewhere.empty()) throw Exception(ErrorCodes::LOGICAL_ERROR, "Sequential source doesn't support PREWHERE");

  if (prefetch && !data_part->isEmpty()) readers.main->prefetchBeginOfRange(Priority{});
...
void MergeTreeSequentialSource::updateRowsToRead(size_t mark_number) {
  ...
  if (mark_number < index_granularity->getMarksCountWithoutFinal()) current_rows_to_read = index_granularity->getMarkRows(mark_number);
}

Chunk MergeTreeSequentialSource::generate() try {
  const auto &index_granularity = read_task_info->data_part->index_granularity;
  ...
  if (!read_result.num_rows) return {};
  ...
  for (size_t i = 0; i < result_header.columns(); ++i) {
    ...
    if (read_task_info->merged_part_offsets && read_task_info->data_part->isProjectionPart() && name == "_parent_part_offset") {
      ...
      if (read_task_info->merged_part_offsets->isMappingEnabled()) {
        for (auto &offset : offset_data) offset = (*read_task_info->merged_part_offsets)[read_task_info->part_index_in_query, offset];
      } else {
        for (auto &offset : offset_data) offset += read_task_info->part_starting_offset_in_query;
      }
    }
  ...
  if (add_part_level) result.getChunkInfos().add(std::make_shared<MergeTreeReadInfo>(read_task_info->data_part->info.level));

  return result;
} catch (...) {
  /// Suspicion of the broken part. A part is added to the queue for verification.
  if (!isRetryableException(std::current_exception())) storage.reportBrokenPart(read_task_info->data_part);
  throw;
}
...
Pipe createMergeTreeSequentialSource(MergeTreeSequentialSourceType type, const MergeTreeData &storage,
                                     const StorageSnapshotPtr &storage_snapshot, RangesInDataPart data_part,
                                     AlterConversionsPtr alter_conversions, MergedPartOffsetsPtr merged_part_offsets, Names columns_to_read,
                                     std::optional<MarkRanges> mark_ranges, std::shared_ptr<std::atomic<size_t>> filtered_rows_count,
                                     bool apply_deleted_mask, bool read_with_direct_io, bool prefetch) {
  ...
  if (need_to_filter_deleted_rows) {
    if (!has_filter_column) columns_to_read.push_back(RowExistsColumn::name);
  ...
  if (info->alter_conversions->hasPatches()) {
    ...
    addPatchPartsColumns(info->task_columns, storage_snapshot, options, info->patch_parts, all_read_columns, need_to_filter_deleted_rows);
  }

  auto column_part_source = std::make_shared<MergeTreeSequentialSource>(
      std::move(result_header), type, storage, storage_snapshot, std::move(info), std::move(mark_ranges), read_with_direct_io, prefetch);
  ...
  if (need_to_filter_deleted_rows) {
    pipe.addSimpleTransform([filtered_rows_count, has_filter_column](const SharedHeader &header) {
      return std::make_shared<FilterTransform>(header, nullptr, RowExistsColumn::name, !has_filter_column, false, filtered_rows_count);
    });
  }
...
class ReadFromPart final : public ISourceStep {
 public:
  ReadFromPart(MergeTreeSequentialSourceType type_, const MergeTreeData &storage_, const StorageSnapshotPtr &storage_snapshot_,
               RangesInDataPart data_part_, AlterConversionsPtr alter_conversions_, MergedPartOffsetsPtr merged_part_offsets_,
               Names columns_to_read_, std::shared_ptr<std::atomic<size_t>> filtered_rows_count_, bool apply_deleted_mask_,
               std::optional<ActionsDAG> filter_, bool read_with_direct_io_, bool prefetch_, ContextPtr context_, LoggerPtr log_)
  ...
  void initializePipeline(QueryPipelineBuilder &pipeline, const BuildQueryPipelineSettings &) override {
    ...
    if (filter && metadata_snapshot->hasPrimaryKey()) {
      ...
      if (!key_condition.alwaysFalse())
        mark_ranges = MergeTreeDataSelectExecutor::markRangesFromPKRange(data_part, metadata_snapshot, key_condition,
                                                                         /*part_offset_condition=*/{},
                                                                         /*total_offset_condition=*/{},
                                                                         /*exact_ranges=*/nullptr, context->getSettingsRef(), log);
    ...
    auto source =
        createMergeTreeSequentialSource(type, storage, storage_snapshot, data_part, alter_conversions, merged_part_offsets, columns_to_read,
                                        std::move(mark_ranges), filtered_rows_count, apply_deleted_mask, read_with_direct_io, prefetch);
...
void createReadFromPartStep(MergeTreeSequentialSourceType type, QueryPlan &plan, const MergeTreeData &storage,
                            const StorageSnapshotPtr &storage_snapshot, RangesInDataPart data_part, AlterConversionsPtr alter_conversions,
                            MergedPartOffsetsPtr merged_part_offsets, Names columns_to_read,
                            std::shared_ptr<std::atomic<size_t>> filtered_rows_count, bool apply_deleted_mask,
                            std::optional<ActionsDAG> filter, bool read_with_direct_io, bool prefetch, ContextPtr context, LoggerPtr log) {
  auto reading =
      std::make_unique<ReadFromPart>(type, storage, storage_snapshot, std::move(data_part), std::move(alter_conversions),
                                     std::move(merged_part_offsets), std::move(columns_to_read), filtered_rows_count, apply_deleted_mask,
                                     std::move(filter), read_with_direct_io, prefetch, std::move(context), log);
