...
class MergedBlockOutputStream final : public IMergedBlockOutputStream {
 public:
  MergedBlockOutputStream(const MergeTreeMutableDataPartPtr &data_part, MergeTreeSettingsPtr data_settings,
                          ...
                          MergeTreeIndexGranularityPtr index_granularity_ptr, TransactionID tid, size_t part_uncompressed_bytes,
                          bool reset_columns_, bool blocks_are_granules_size, const WriteSettings &write_settings,
  ...
  Finalizer finalizePartAsync(const MergeTreeMutableDataPartPtr &new_part, const GatheredData &gathered_data, bool sync,
                              const NamesAndTypesList *total_columns_list = nullptr);

  void finalizePart(const MergeTreeMutableDataPartPtr &new_part, const GatheredData &gathered_data, bool sync,
                    const NamesAndTypesList *total_columns_list = nullptr);
 ...
 private:
  ...
  WrittenFiles finalizePartOnDisk(const MergeTreeMutableDataPartPtr &new_part, MergeTreeData::DataPart::Checksums &checksums,
