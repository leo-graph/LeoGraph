...
size_t MergeTreeReaderCompactSingleBuffer::readRows(size_t from_mark, size_t current_task_last_mark, bool continue_reading,
                                                    size_t max_rows_to_read, size_t rows_offset, Columns& res_columns) try {
  init();

  if (continue_reading) from_mark = next_mark;
  ...
  while (read_rows < max_rows_to_read) {
    ...
    for (size_t pos = 0; pos < num_columns; ++pos) {
      ...
      if (columns_to_read[pos].isSubcolumn() && has_substream_marks) continue;

      stream->adjustRightMark(current_task_last_mark);  /// Must go before seek.
      stream->seekToMarkAndColumn(
          from_mark, has_substream_marks ? columns_substreams.getFirstSubstreamPosition(*column_positions[pos]) : *column_positions[pos]);
      ...
      readData(pos, res_columns[pos], rows_to_read, rows_offset, from_mark, res_columns[pos]->size(), *stream, columns_cache,
               cache_for_subcolumns, nullptr);
    }
    ...
    if (has_substream_marks && has_subcolumns) {
      ...
      for (const auto& [column, subcolumns_order] : subcolumns_deserialization_order) {
        ...
        for (size_t pos : subcolumns_order) {
          if (!res_columns[pos]) continue;

          readData(pos, res_columns[pos], rows_to_read, rows_offset, from_mark, subcolumns_size_before_reading, *stream, columns_cache,
                   &columns_cache_for_subcolumns, &substreams_cache);
        }
      }
    }
...
void MergeTreeReaderCompactSingleBuffer::init() try {
  ...
  stream = std::make_unique<MergeTreeReaderStreamAllOfMultipleColumns>(
      ...
      MergeTreeDataPartCompact::DATA_FILE_EXTENSION, data_part_info_for_read->getMarksCount(), all_mark_ranges, stream_settings,
      uncompressed_cache, data_part_info_for_read->getFileSizeOrZero(MergeTreeDataPartCompact::DATA_FILE_NAME_WITH_EXTENSION), marks_loader,
