...
struct MovesListElement : private boost::noncopyable {
  ...
  MovesListElement(const StorageID& table_id_, const std::string& part_name_, const std::string& target_disk_name_,
                   const std::string& target_disk_path_, UInt64 part_size_);
...
class MovesList final : public BackgroundProcessList<MovesListElement, MoveInfo> {
 ...
 public:
  MovesList() : Parent(CurrentMetrics::Move) {}
};
