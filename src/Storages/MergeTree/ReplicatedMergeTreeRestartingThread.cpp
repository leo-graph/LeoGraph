...
static String generateActiveNodeIdentifier() { return Field(ServerUUID::get()).dump(); }
...
void ReplicatedMergeTreeRestartingThread::wakeup() { task->schedule(); }
...
bool ReplicatedMergeTreeRestartingThread::runImpl() {
  if (!storage.is_readonly && !storage.getZooKeeper()->expired()) return true;
  ...
  if (storage.getContext()->getServerSettings()[ServerSetting::insert_deduplication_version].value !=
      InsertDeduplicationVersions::OLD_SEPARATE_HASHES)
...
void ReplicatedMergeTreeRestartingThread::removeFailedQuorumParts() {
  ...
  if (zookeeper->tryGetChildren(storage.zookeeper_path + "/quorum/failed_parts", failed_parts) != Coordination::Error::ZOK) return;
...
void ReplicatedMergeTreeRestartingThread::updateQuorumIfWeHavePart() {
  ...
  if (zookeeper->tryGet(fs::path(storage.zookeeper_path) / "quorum" / "status", quorum_str)) {
    ...
    if (!quorum_entry.replicas.contains(storage.replica_name) && storage.getActiveContainingPart(quorum_entry.part_name)) {
  ...
  if (zookeeper->tryGetChildren(parallel_quorum_parts_path, part_names) == Coordination::Error::ZOK) {
    for (auto &part_name : part_names) {
      if (zookeeper->tryGet(fs::path(parallel_quorum_parts_path) / part_name, quorum_str)) {
        ReplicatedMergeTreeQuorumEntry quorum_entry(quorum_str);
        if (!quorum_entry.replicas.contains(storage.replica_name) && storage.getActiveContainingPart(part_name)) {
...
void ReplicatedMergeTreeRestartingThread::shutdown(bool part_of_full_shutdown) {
  ...
  if (part_of_full_shutdown) storage.startup_event.set();
...
void ReplicatedMergeTreeRestartingThread::setReadonly(bool on_shutdown) {
  ...
  if (became_readonly) {
    const UInt32 now = static_cast<UInt32>(std::chrono::system_clock::to_time_t(std::chrono::system_clock::now()));
  ...
  if (became_readonly && on_shutdown) return;
...
Int32 ReplicatedMergeTreeRestartingThread::fixReplicaMetadataVersionIfNeeded(zkutil::ZooKeeperPtr zookeeper) {
  ...
  for (size_t attempt = 0; attempt != num_attempts; ++attempt) {
    ...
    const bool replica_metadata_version_exists =
        zookeeper->tryGet(replica_path + "/metadata_version", replica_metadata_version_str, &replica_stat);
    if (!replica_metadata_version_exists) return -1;

    const Int32 metadata_version = parse<Int32>(replica_metadata_version_str);
    if (metadata_version != 0) return metadata_version;

    Coordination::Stat table_stat;
    zookeeper->get(fs::path(zookeeper_path) / "metadata", &table_stat);
    if (table_stat.version == 0) return metadata_version;
    ...
    if (code == Coordination::Error::ZBADVERSION) {
      LOG_WARNING(log,
                  "Cannot fix metadata_version because either metadata.version or metadata_version.version changed, attempts left = {}",
  ...
  /// If metadata.version changed during first attempt, on second attempt we will either get metadata_version != 0 and return the new
  /// metadata_version or we will get metadata_alters_in_queue != 0 and return 0. So either first or second attempt should return unless
  /// metadata_version was rewritten from 0 to 0 during the first attempt which is highly unlikely.
