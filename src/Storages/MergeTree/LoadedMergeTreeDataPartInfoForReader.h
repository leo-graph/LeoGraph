...
class LoadedMergeTreeDataPartInfoForReader final : public IMergeTreeDataPartInfoForReader {
 public:
  LoadedMergeTreeDataPartInfoForReader(MergeTreeData::DataPartPtr data_part_, AlterConversionsPtr alter_conversions_)
  ...
  Int64 getMinDataVersion() const override {
    return data_part->info.isPatch() ? data_part->getSourcePartsSet().getMinDataVersion() : data_part->info.getDataVersion();
  }

  Int64 getMaxDataVersion() const override {
    return data_part->info.isPatch() ? data_part->getSourcePartsSet().getMaxDataVersion() : data_part->info.getDataVersion();
  }
  ...
  const ColumnsDescription& getColumnsDescriptionWithCollectedNested() const override {
    return data_part->getColumnsDescriptionWithCollectedNested();
  }
  ...
  void setReadHints(const RangesInDataPartReadHints& read_hints_, const NamesAndTypesList& read_columns) override {
    if (read_columns.contains("_distance")) read_hints = read_hints_;
  }
