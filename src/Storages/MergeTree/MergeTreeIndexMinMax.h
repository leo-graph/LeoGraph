...
struct MergeTreeIndexGranuleMinMax final : public IMergeTreeIndexGranule {
  ...
  MergeTreeIndexGranuleMinMax(const String& index_name_, const Block& index_sample_block_, std::vector<Range>&& hyperrectangle_);
...
class MergeTreeIndexConditionMinMax final : public IMergeTreeIndexCondition {
 public:
  MergeTreeIndexConditionMinMax(const IndexDescription& index, const ActionsDAGWithInversionPushDown& filter_dag, ContextPtr context);
...
class MergeTreeIndexMinMax : public IMergeTreeIndex {
 public:
  explicit MergeTreeIndexMinMax(const IndexDescription& index_) : IMergeTreeIndex(index_) {}
  ...
  MergeTreeIndexFormat getDeserializedFormat(const MergeTreeDataPartChecksums& checksums,
                                             const std::string& path_prefix) const override;  /// NOLINT
};
...
struct MergeTreeIndexBulkGranulesMinMax final : public IMergeTreeIndexBulkGranules {
  ...
  explicit MergeTreeIndexBulkGranulesMinMax(const String& index_name_, const Block& index_sample_block_, size_t index_granularity_,
                                            int direction_, size_t size_hint_, size_t last_part_granule_, bool store_map_ = false);
 ...
 private:
  ...
  template <bool handle_ties>
  static void getTopKMarks(int direction, size_t n, size_t index_granularity, const std::vector<std::vector<MinMaxGranule>>& parts,
