...
UInt64 parseSequentialNodeNumber(const String &path, size_t prefix_size) {
  if (path.size() <= prefix_size)
    throw Exception(ErrorCodes::LOGICAL_ERROR,
                    "Path of the sequential node is shorter than the provided prefix size: path={}, prefix_size={}", path, prefix_size);
...
void warnIfBlockNumberIsHigh(UInt64 number, const String &path) {
  if (number > BLOCK_NUMBER_WARNING_THRESHOLD)
    LOG_WARNING(getLogger("EphemeralLockInZooKeeper"), "Block number {} is too high, this may lead to overflow. Path: {}", number, path);
}

}  // namespace

EphemeralLockInZooKeeper::EphemeralLockInZooKeeper(const String &path_prefix_, const ZooKeeperWithFaultInjectionPtr &zookeeper_,
                                                   const String &path_, UInt64 number_, const String &conflict_path_)
...
EphemeralLockInZooKeeper createEphemeralLockInZooKeeper(const String &path_prefix_, const String &temp_path,
  ...
  if (deduplication_paths.empty()) {
    ...
    for (const auto &single_dedup_path : deduplication_paths) zkutil::addCheckNotExistsRequest(ops, *zookeeper_, single_dedup_path);
    ...
    if (e == Coordination::Error::ZNODEEXISTS) {
      ...
      if (failed_idx < deduplication_path_ops_size) {
        ...
        LOG_DEBUG(getLogger("createEphemeralLockInZooKeeper"), "Deduplication path already exists: deduplication_path={}", failed_op_path);
    ...
    if (e != Coordination::Error::ZOK) {
      ...
      throw Exception(ErrorCodes::LOGICAL_ERROR, "Unable to handle error '{}' when acquiring ephemeral lock in ZK",
                      Coordination::errorMessage(e));
    }
...
void EphemeralLockInZooKeeper::checkCreated() const {
  if (!isLocked()) throw Exception(ErrorCodes::LOGICAL_ERROR, "EphemeralLock is not created");
}
...
EphemeralLockInZooKeeper::~EphemeralLockInZooKeeper() {
  ...
  try {
    ...
    if (Coordination::isHardwareError(e.code))
      LOG_DEBUG(getLogger("EphemeralLockInZooKeeper"), "ZooKeeper communication error during unlock: code={} message='{}'", e.code,
                e.message());
      ...
      LOG_DEBUG(getLogger("EphemeralLockInZooKeeper"), "ZooKeeper node was already deleted: code={} message={}", e.code, e.message());
    else
...
EphemeralLocksInAllPartitions::EphemeralLocksInAllPartitions(const String &block_numbers_path, const String &path_prefix,
                                                             const String &temp_path, const std::optional<String> &znode_data,
  ...
  while (true) {
    ...
    for (const auto &partition : partitions) {
      ...
      lock_ops.push_back(zkutil::makeCreateRequest(partition_path_prefix, holder_path, zkutil::CreateMode::EphemeralSequential));
    }
    ...
    if (rc != Coordination::Error::ZOK) throw Coordination::Exception(rc);
...
void EphemeralLocksInAllPartitions::assumeUnlocked() { zookeeper = nullptr; }

void EphemeralLocksInAllPartitions::getUnlockOps(Coordination::Requests &ops) const {
  if (!zookeeper) return;

  for (const auto &lock : locks) ops.emplace_back(zkutil::makeRemoveRequest(lock.path, -1));
}
...
void PartitionBlockNumbersHolder::assumeUnlocked() {
  if (multiple_partitions_holder) multiple_partitions_holder->assumeUnlocked();

  if (single_partition_holder) single_partition_holder->assumeUnlocked();
}

void PartitionBlockNumbersHolder::getUnlockOps(Coordination::Requests &ops) const {
  if (multiple_partitions_holder) multiple_partitions_holder->getUnlockOps(ops);

  if (single_partition_holder) single_partition_holder->getUnlockOp(ops);
}
