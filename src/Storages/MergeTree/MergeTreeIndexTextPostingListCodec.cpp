...
void PostingListCodecBitpackingImpl::insert(uint32_t row_id) {
  ...
  if (row_ids_in_current_segment == max_rowids_in_segment) flushCurrentSegment();
}
...
void PostingListCodecBitpackingImpl::encodeBlock(std::span<uint32_t> segment) {
  ...
  if (used_memory != needed_bytes_without_header || !compressed_data_span.empty()) {
    throw Exception(ErrorCodes::LOGICAL_ERROR,
                    ...
                    needed_bytes_without_header, max_bits, segment.size(), used_memory, compressed_data_span.size());
  }

  segment_descriptor.compressed_data_size = compressed_data.size() - segment_descriptor.compressed_data_offset;
}

void PostingListCodecBitpackingImpl::decodeBlock(std::span<const std::byte>& in, size_t count, uint32_t& prev_row_id,
                                                 std::vector<uint32_t>& current_segment) {
  chassert(count <= BLOCK_SIZE);
  if (in.empty()) throw Exception(ErrorCodes::CORRUPTED_DATA, "Corrupted data: expected at least {} bytes, but got {}", 1, in.size());

  uint8_t bits = readByte(in);
  if (bits > 32) throw Exception(ErrorCodes::CORRUPTED_DATA, "Corrupted data: expected bits <= 32, but got {}", bits);
  ...
  if (required_size != consumed_size)
    throw Exception(
        ...
        required_size, count, bits, consumed_size);
...
void PostingListCodecBitpacking::encode(const PostingList& postings, size_t max_rowids_in_segment, TokenPostingsInfo& info,
                                        WriteBuffer& out) const {
  ...
  if (!rowids_view.empty()) {
    for (auto rowid : rowids_view) impl.insert(rowid);
  }
