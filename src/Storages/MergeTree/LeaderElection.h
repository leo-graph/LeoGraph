...
inline void checkNoOldLeaders(LoggerPtr log, ZooKeeper& zookeeper, const String path) {
  ...
  while (num_tries--) {
    ...
    if (potential_leaders.empty()) {
      ...
      ops.emplace_back(
          makeCreateRequest(fs::path(path) / persistent_multiple_leaders, persistent_identifier, zkutil::CreateMode::Persistent));
    } else {
      ::sort(potential_leaders.begin(), potential_leaders.end());
      if (potential_leaders.front() == persistent_multiple_leaders) return;
