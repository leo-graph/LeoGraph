...
MergeTreeReaderCompact::MergeTreeReaderCompact(MergeTreeDataPartInfoForReaderPtr data_part_info_for_read_, NamesAndTypesList columns_,
                                               ...
                                               MarkRanges mark_ranges_, MergeTreeReaderSettings settings_,
                                               ValueSizeMap avg_value_size_hints_,
                                               const ReadBufferFromFileBase::ProfileCallback &profile_callback_, clockid_t clock_type_)
    : IMergeTreeReader(data_part_info_for_read_, columns_, virtual_fields_, storage_snapshot_, storage_settings_, uncompressed_cache_,
      ...
      marks_loader(std::make_shared<MergeTreeMarksLoader>(
          data_part_info_for_read_, mark_cache,
          data_part_info_for_read_->getIndexGranularityInfo().getMarksFilePath(MergeTreeDataPartCompact::DATA_FILE_NAME),
          data_part_info_for_read_->getMarksCount(), data_part_info_for_read_->getIndexGranularityInfo(), settings.save_marks_in_cache,
          settings.read_settings,
          settings_.load_marks_asynchronously ? &data_part_info_for_read_->getContext()->getLoadMarksThreadpool() : nullptr,
          data_part_info_for_read_->getIndexGranularityInfo().mark_type.with_substreams ? columns_substreams.getTotalSubstreams()
...
void MergeTreeReaderCompact::fillColumnPositions() {
  ...
  for (size_t i = 0; i < columns_num; ++i) {
    ...
    if (position.has_value() && isColumnDroppedByPendingMutation(i)) position.reset();
    ...
    if (position.has_value() && column_to_read.isSubcolumn()) {
      ...
      if (!storage_column_from_part.type->hasSubcolumn(subcolumn_name)) position.reset();
    }
    ...
    if (!column_positions[i]) findPositionForMissedNested(i);
...
NameAndTypePair MergeTreeReaderCompact::getColumnConvertedToSubcolumnOfNested(const NameAndTypePair &column) {
  if (!isArray(column.type)) return column;
  ...
  if (subcolumn_name.empty()) return column;
  ...
  return storage_columns_with_collected_nested->getColumnOrSubcolumn(GetColumnsOptions::All,
...
void MergeTreeReaderCompact::findPositionForMissedNested(size_t pos) {
  ...
  if (!is_array && !is_offsets_subcolumn) return;
...
static ColumnPtr getFullColumnFromCache(std::unordered_map<String, ColumnPtr> *columns_cache_for_subcolumns, const String &column_name) {
  if (!columns_cache_for_subcolumns) return nullptr;

  auto it = columns_cache_for_subcolumns->find(column_name);
  if (it == columns_cache_for_subcolumns->end()) return nullptr;

  return it->second;
}

void MergeTreeReaderCompact::readData(size_t column_idx, ColumnPtr &column, size_t rows_to_read, size_t rows_offset, size_t from_mark,
                                      size_t column_size_before_reading, MergeTreeReaderStream &stream,
  ...
  auto buffer_getter = [&](const ISerialization::SubstreamPath &substream_path) -> ReadBuffer * {
    if (needSkipStream(column_idx, substream_path)) return nullptr;

    if (seek_to_substream_mark) {
      size_t substream_position =
          columns_substreams.getSubstreamPosition(*column_positions[column_idx], name_and_type, substream_path, storage_settings);
  ...
  try {
    ...
    if (name_and_type.isSubcolumn()) {
      if (has_substream_marks) {
        ...
        serialization->deserializeBinaryBulkWithMultipleStreams(column, rows_offset, rows_to_read, deserialize_settings,
                                                                deserialize_binary_bulk_state_map_for_subcolumns[name], substreams_cache);
      } else {
        ...
        if (!temp_full_column) {
          ...
          serialization->deserializeBinaryBulkWithMultipleStreams(temp_full_column, rows_offset, rows_to_read, deserialize_settings,
                                                                  deserialize_binary_bulk_state_map_for_subcolumns[name_in_storage],
                                                                  substreams_cache);

          if (columns_cache_for_subcolumns) columns_cache_for_subcolumns->emplace(name_in_storage, temp_full_column);
        }
      ...
      const auto &serialization = serializations[column_idx];
      serialization->deserializeBinaryBulkWithMultipleStreams(column, rows_offset, rows_to_read, deserialize_settings,
                                                              deserialize_binary_bulk_state_map[name], substreams_cache);
    }
    ...
    if (read_rows_in_column != rows_to_read)
      throw Exception(ErrorCodes::CANNOT_READ_ALL_DATA, "Cannot read all data in MergeTreeReaderCompact. Rows read: {}. Rows expected: {}.",
...
void MergeTreeReaderCompact::readSubcolumnsPrefixes(size_t from_mark, size_t current_task_last_mark) {
  if (!has_subcolumns || !has_substream_marks) return;
  ...
  if (deserialization_prefixes_cache && settings.use_deserialization_prefixes_cache &&
      data_part_info_for_read->getDataPartStorage()->isStoredOnRemoteDisk())
...
void MergeTreeReaderCompact::initSubcolumnsDeserializationOrder() {
  if (!has_subcolumns || !has_substream_marks || !subcolumns_deserialization_order.empty()) return;
  ...
  for (const auto &[column, subcolumns_indexes] : column_to_subcolumns_indexes) {
    ...
    auto order = getSubcolumnsDeserializationOrder(column, subcolumns_data, columns_substreams.getColumnSubstreams(*pos),
                                                   enumerate_settings, ISerialization::StreamFileNameSettings(*storage_settings));
    deserialization_order.reserve(subcolumns_indexes.size());
    for (size_t i : order) deserialization_order.push_back(subcolumn_data_index_to_subcolumn_index[i]);
    /// Add indexes of non-existing subcolumns at the end.
    deserialization_order.insert(deserialization_order.end(), non_existing_subcolumns_indexes.begin(),
                                 non_existing_subcolumns_indexes.end());
  }
}

void MergeTreeReaderCompact::readPrefix(size_t column_idx, size_t from_mark, MergeTreeReaderStream &stream,
  ...
  if (columns_for_offsets[column_idx]) {
    ...
    auto buffer_getter = [&](const auto &) -> ReadBuffer * { return stream.getDataBuffer(); };
  ...
  auto buffer_getter = [&](const ISerialization::SubstreamPath &substream_path) -> ReadBuffer * {
    ...
    if (seek_to_substream_mark) {
      size_t substream_position =
          columns_substreams.getSubstreamPosition(*column_positions[column_idx], column, substream_path, storage_settings);
      stream.seekToMarkAndColumn(from_mark, substream_position);
    }
  ...
  if (column.isSubcolumn()) {
    if (has_substream_marks) {
      ...
      if (deserialize_binary_bulk_state_map_for_subcolumns.contains(name_in_storage)) return;
...
void MergeTreeReaderCompact::readPrefix(const NameAndTypePair &name_and_type, const SerializationPtr &serialization,
                                        ISerialization::DeserializeBinaryBulkStatePtr &state, const InputStreamGetter &buffer_getter,
                                        ISerialization::SubstreamsDeserializeStatesCache *cache) {
  try {
    ISerialization::DeserializeBinaryBulkSettings deserialize_settings;
    deserialize_settings.getter = buffer_getter;
...
void MergeTreeReaderCompact::createColumnsForReading(Columns &res_columns) const {
  for (size_t i = 0; i < columns_to_read.size(); ++i) {
    if (column_positions[i] && res_columns[i] == nullptr) res_columns[i] = columns_to_read[i].type->createColumn(*serializations[i]);
  }
}
...
bool MergeTreeReaderCompact::needSkipStream(size_t column_pos, const ISerialization::SubstreamPath &substream) const {
  ...
  if (!columns_for_offsets[column_pos]) return false;
