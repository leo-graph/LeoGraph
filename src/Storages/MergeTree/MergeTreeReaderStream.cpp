...
MergeTreeReaderStream::MergeTreeReaderStream(DataPartStoragePtr data_part_storage_, const String& path_prefix_,
                                             const String& data_file_extension_, size_t marks_count_, const MarkRanges& all_mark_ranges_,
                                             const MergeTreeReaderSettings& settings_, UncompressedCache* uncompressed_cache_,
...
void MergeTreeReaderStream::loadMarks() {
  if (!marks_getter) marks_getter = marks_loader->loadMarks();
}
...
void MergeTreeReaderStream::init() {
  ...
  if (sum_mark_range_bytes) estimated_sum_mark_range_bytes.emplace(sum_mark_range_bytes);
  ...
  if (settings.adjust_read_buffer_size && max_mark_range_bytes != 0) read_settings = read_settings.adjustBufferSize(max_mark_range_bytes);
  ...
  if (!settings.is_compressed) {
    auto buffer = data_part_storage->readFile(path_prefix + data_file_extension, read_settings, estimated_sum_mark_range_bytes);

    if (profile_callback) buffer->setProfileCallback(profile_callback, clock_type);
    ...
    auto buffer = std::make_unique<CachedCompressedReadBuffer>(
        ...
        [this, estimated_sum_mark_range_bytes, read_settings]() {
          ...
          return data_part_storage->readFile(path_prefix + data_file_extension, read_settings, estimated_sum_mark_range_bytes);
        },
        uncompressed_cache, settings.allow_different_codecs);

    if (profile_callback) buffer->setProfileCallback(profile_callback, clock_type);

    if (!settings.checksum_on_read) buffer->disableChecksumming();
    ...
    auto buffer = std::make_unique<CompressedReadBufferFromFile>(
        data_part_storage->readFile(path_prefix + data_file_extension, read_settings, estimated_sum_mark_range_bytes),
...
void MergeTreeReaderStream::seekToMarkAndColumn(size_t row_index, size_t column_position) {
  ...
  try {
    ...
    if (e.code() == ErrorCodes::ARGUMENT_OUT_OF_BOUND)
      e.addMessage("(while seeking to mark " + toString(row_index) + " of column " + path_prefix +
                   "; offsets are: " + toString(mark.offset_in_compressed_file) + " " + toString(mark.offset_in_decompressed_block) + ")");
...
void MergeTreeReaderStream::seekToMark(const MarkInCompressedFile& mark) {
  if (compressed_data_buffer) {
    ...
    if (mark.offset_in_decompressed_block != 0)
      throw Exception(ErrorCodes::LOGICAL_ERROR, "Cannot seek to offset in decompressed block ({}) for uncompressed data",
...
void MergeTreeReaderStream::seekToStart() {
  ...
  try {
    ...
    /// Better diagnostics.
    if (e.code() == ErrorCodes::ARGUMENT_OUT_OF_BOUND) e.addMessage("(while seeking to start of column " + path_prefix + ")");
...
void MergeTreeReaderStream::adjustRightMark(size_t right_mark) {
  ...
  if (!right_offset) {
    if (last_right_offset && *last_right_offset == 0) return;

    last_right_offset = 0;  // Zero value means the end of file.
    data_buffer->setReadUntilEnd();
  } else {
    if (last_right_offset && right_offset <= last_right_offset.value()) return;
...
size_t MergeTreeReaderStreamSingleColumn::getRightOffset(size_t right_mark) {
  ...
  if (marks_count == 0) return 0;

  chassert(right_mark <= marks_count);
  loadMarks();

  if (right_mark == 0) return marks_getter->getMark(right_mark, 0).offset_in_compressed_file;

  if (right_mark == marks_count) return file_size;
  ...
  if (settings.is_low_cardinality_dictionary) {
    ...
    if (it == indices.end()) return file_size;
  ...
  /// whole file size.
  if (settings.is_dynamic_or_object_structure) return file_size;
  ...
  if (it != indices.end()) return marks_getter->getMark(*it, 0).offset_in_compressed_file;
...
size_t MergeTreeReaderStreamSingleColumnWholePart::getRightOffset(size_t right_mark) {
  if (right_mark != marks_count) {
    throw Exception(ErrorCodes::LOGICAL_ERROR, "Expected one right mark: {}, got: {}", marks_count, right_mark);
  }
  return file_size;
}
...
std::pair<size_t, size_t> MergeTreeReaderStreamSingleColumnWholePart::estimateMarkRangeBytes(const MarkRanges& mark_ranges) {
  if (!mark_ranges.isOneRangeForWholePart(marks_count)) {
    throw Exception(ErrorCodes::LOGICAL_ERROR, "Expected one mark range that covers the whole part, got: {}", mark_ranges.describe());
  }
...
size_t MergeTreeReaderStreamMultipleColumns::getRightOffsetOneColumn(size_t right_mark_non_included, size_t column_position) {
  ...
  if (right_mark_non_included == 0) return marks_getter->getMark(right_mark_non_included, column_position).offset_in_compressed_file;
  ...
  for (size_t mark = right_mark_included + 1; mark < marks_count; ++mark) {
    ...
    if (current_mark.offset_in_compressed_file > right_mark_in_file.offset_in_compressed_file) {
      ...
      if (current_mark < next_stripe_right_mark_in_file) return current_mark.offset_in_compressed_file;
...
std::pair<size_t, size_t> MergeTreeReaderStreamMultipleColumns::estimateMarkRangeBytesOneColumn(const MarkRanges& mark_ranges,
...
MarkInCompressedFile MergeTreeReaderStreamMultipleColumns::getStartOfNextStripeMark(size_t row_index, size_t column_position) {
  ...
  if (marks_getter->getNumColumns() == 1) return MarkInCompressedFile{file_size, 0};
  ...
  if (column_position + 1 == marks_getter->getNumColumns()) {
    ...
    while (mark_index < marks_count && marks_getter->getMark(mark_index, 0) <= current_mark) ++mark_index;

    return mark_index == marks_count ? MarkInCompressedFile{file_size, 0} : marks_getter->getMark(mark_index, 0);
  }
  ...
  while (mark_index >= 0 && marks_getter->getMark(mark_index, column_position + 1) >= current_mark) --mark_index;
