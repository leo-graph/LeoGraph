...
CoordinationMode validateAndGet(uint8_t candidate) {
  if (candidate <= static_cast<uint8_t>(CoordinationMode::MAX)) return static_cast<CoordinationMode>(candidate);
...
ParallelReadRequest ParallelReadRequest::deserialize(ReadBuffer& in, UInt64 replica_protocol_version) {
  ...
  if (version < DBMS_MIN_SUPPORTED_PARALLEL_REPLICAS_PROTOCOL_VERSION)
    throw Exception(ErrorCodes::UNKNOWN_PROTOCOL, "Parallel replicas protocol version is too old. Got: {}, min supported version: {}",
                    version, DBMS_MIN_SUPPORTED_PARALLEL_REPLICAS_PROTOCOL_VERSION);
...
String ParallelReadResponse::describe() const { return fmt::format("{}. Finish: {}", description.describe(), finish); }
...
void InitialAllRangesAnnouncement::serialize(WriteBuffer& out, UInt64 initiator_protocol_version) const {
  ...
  description.serialize(out, initiator_protocol_version);
  writeIntBinary(replica_num, out);
  if (initiator_protocol_version >= DBMS_PARALLEL_REPLICAS_MIN_VERSION_WITH_MARK_SEGMENT_SIZE_FIELD) writeIntBinary(mark_segment_size, out);
}
...
InitialAllRangesAnnouncement InitialAllRangesAnnouncement::deserialize(ReadBuffer& in, UInt64 replica_protocol_version) {
  ...
  if (replica_protocol_version >= DBMS_PARALLEL_REPLICAS_MIN_VERSION_WITH_MARK_SEGMENT_SIZE_FIELD) readIntBinary(mark_segment_size, in);
