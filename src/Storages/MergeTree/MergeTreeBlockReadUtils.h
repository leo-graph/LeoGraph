...
struct MergeTreeBlockSizePredictor {
  MergeTreeBlockSizePredictor(const DataPartPtr& data_part_, const Names& columns, const Block& sample_block,
                              bool allow_subcolumns_sizes_calculation);
  ...
  size_t estimateNumRowsForMaxSizeColumn(size_t bytes_quota) const {
    ...
    /// The problem is that the conversion to double (`max_size_per_row` is double) can overflow size_t if `bytes_quota` is close to
    /// `SIZE_MAX` and an attempt to convert this value back to size_t will result in undefined behavior. ATST, if `bytes_quota` is close to
    /// `SIZE_MAX`, we can safely ignore it and assume that no limit was set.
    if (static_cast<double>(bytes_quota) >= static_cast<double>(std::numeric_limits<size_t>::max())) return 0;
  ...
  void updateFilteredRowsRation(size_t rows_was_read, size_t rows_was_filtered, double decay = calculateDecay()) {
    ...
    filtered_rows_ratio =
        current_ration < filtered_rows_ratio ? current_ration : alpha * filtered_rows_ratio + (1.0 - alpha) * current_ration;
  }
