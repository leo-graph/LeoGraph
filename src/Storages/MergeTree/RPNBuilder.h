...
class RPNBuilderTreeContext {
 public:
  ...
  /// Get query context
  const ContextPtr& getQueryContext() const { return query_context; }
   ...
   */
  const Block& getBlockWithConstants() const { return block_with_constants; }

  /** Get prepared sets.
   * Valid only for AST tree.
   */
  const PreparedSetsPtr& getPreparedSets() const { return prepared_sets; }
...
class RPNBuilderTreeNode {
 public:
  ...
  /// Get tree context
  const RPNBuilderTreeContext& getTreeContext() const { return tree_context; }

  /// Get tree context
  RPNBuilderTreeContext& getTreeContext() { return tree_context; }
...
class RPNBuilder {
 public:
  ...
  explicit RPNBuilder(const ActionsDAG::Node* filter_actions_dag_node, ContextPtr query_context_,
