...
MergeTreeIndexGranuleSet::MergeTreeIndexGranuleSet(const String &index_name_, const Block &index_sample_block_, size_t max_rows_)
    : index_name(index_name_), max_rows(max_rows_), block(index_sample_block_.cloneEmpty()) {
  size_t num_columns = block.columns();
  serializations.resize(num_columns);
  for (size_t i = 0; i < num_columns; ++i) serializations[i] = block.getByPosition(i).type->getDefaultSerialization();
}

MergeTreeIndexGranuleSet::MergeTreeIndexGranuleSet(const String &index_name_, const Block &index_sample_block_, size_t max_rows_,
                                                   MutableColumns &&mutable_columns_, std::vector<Range> &&set_hyperrectangle_)
...
void MergeTreeIndexGranuleSet::serializeBinary(WriteBuffer &ostr) const {
  if (empty()) throw Exception(ErrorCodes::LOGICAL_ERROR, "Attempt to write empty set index {}.", backQuote(index_name));
...
MergeTreeIndexBulkGranulesSet::MergeTreeIndexBulkGranulesSet(const Block &index_sample_block_)
    : block(index_sample_block_.cloneEmpty()), block_for_reading(index_sample_block_.cloneEmpty()) {
  size_t num_columns = block.columns();
  serializations.resize(num_columns);
  for (size_t i = 0; i < num_columns; ++i) serializations[i] = block.getByPosition(i).type->getDefaultSerialization();
...
void MergeTreeIndexBulkGranulesSet::deserializeBinary(size_t granule_num, ReadBuffer &istr, MergeTreeIndexVersion version) {
  ...
  for (size_t i = 0; i < rows_to_read; ++i) data.push_back(granule_num);
}
  ...
  if (has_new_data) {
    ...
    for (size_t i = 0; i < columns.size(); ++i) {
      ...
      if (set_hyperrectangle.size() <= i) {
        ...
        set_hyperrectangle[i].left = accurateLess(set_hyperrectangle[i].left, field_min) ? set_hyperrectangle[i].left : field_min;
        set_hyperrectangle[i].right = accurateLess(set_hyperrectangle[i].right, field_max) ? field_max : set_hyperrectangle[i].right;
      }
    }
  }
...
bool MergeTreeIndexAggregatorSet::buildFilter(Method &method, const ColumnRawPtrs &column_ptrs, IColumn::Filter &filter, size_t pos,
                                              size_t limit, ClearableSetVariants &variants) const {
  ...
  for (size_t i = 0; i < limit; ++i) {
    ...
    if (emplace_result.isInserted()) has_new_data = true;
...
MergeTreeIndexConditionSet::MergeTreeIndexConditionSet(size_t max_rows_, const ActionsDAGWithInversionPushDown &filter_dag,
                                                       ContextPtr context, const IndexDescription &index_description)
  ...
  for (const auto &name : index_description.sample_block.getNames())
    if (!key_columns.contains(name)) key_columns.insert(name);

  if (!filter_dag.predicate) return;

  std::vector<FutureSetPtr> sets_to_prepare;
  if (checkDAGUseless(*filter_dag.predicate, context, sets_to_prepare)) return;
...
bool MergeTreeIndexConditionSet::alwaysUnknownOrTrue() const { return isUseless(); }

bool MergeTreeIndexConditionSet::mayBeTrueOnGranule(MergeTreeIndexGranulePtr idx_granule,
                                                    const UpdatePartialDisjunctionResultFn &update_partial_disjunction_result_fn) const {
  if (isUseless()) return true;

  const MergeTreeIndexGranuleSet &granule = assert_cast<const MergeTreeIndexGranuleSet &>(*idx_granule);

  size_t size = granule.size();
  if (size == 0 || (max_rows != 0 && size > max_rows)) return true;
  ...
  for (size_t i = 0; i < size; ++i)
    ...
    /// The second part of the conditions checks if column is nullable, in that case there is no need to check null map since filter
    /// function possibly handles null values. Only in case the column is not nullable, check the null map.
    if ((column->get64(i) & 1) && (nullable || !column->isNullAt(i))) return true;

  return false;
}

std::string MergeTreeIndexConditionSet::getDescription() const { return condition.getDescription().condition; }

MergeTreeIndexConditionSet::FilteredGranules MergeTreeIndexConditionSet::getPossibleGranules(
  ...
  auto whole_range = [total_granules, min_granule = granules.min_granule] {
    ...
    for (size_t i = 0; i < total_granules; ++i) whole[i] = min_granule + i;
    return whole;
  };

  if (block_size == 0 || isUseless()) return whole_range();
  ...
  const auto &column =
      block.getByName(actions_output_column_name).column->convertToFullColumnIfConst()->convertToFullColumnIfLowCardinality();
  if (column->onlyNull()) return whole_range();
  ...
  if (!col_uint8) throw Exception(ErrorCodes::LOGICAL_ERROR, "ColumnUInt8 is expected as a Set index condition result");
    ...
    if (block_pos >= block_size || current_granule < current_granule_in_block) {
      ...
      for (bool passed = false; block_pos < block_size && current_granule_in_block == granule_nums[block_pos]; ++block_pos) {
        if (!passed) {
          ...
          if (passed) res.push_back(current_granule);
...
static const ActionsDAG::NodeRawConstPtrs &getArguments(const ActionsDAG::Node &node, ActionsDAG *result_dag_or_null,
                                                        ActionsDAG::NodeRawConstPtrs *storage) {
  chassert(node.type == ActionsDAG::ActionType::FUNCTION);
  if (node.function_base->getName() != "indexHint") return node.children;
  ...
  if (!result_dag_or_null) return index_hint.getActions().getOutputs();
...
const ActionsDAG::Node &MergeTreeIndexConditionSet::traverseDAG(
    const ActionsDAG::Node &node, ActionsDAG &result_dag, const ContextPtr &context,
    std::unordered_map<const ActionsDAG::Node *, const ActionsDAG::Node *> &node_to_result_node,
    std::unordered_map<String, const ActionsDAG::Node *> &key_column_inputs) const {
  auto result_node_it = node_to_result_node.find(&node);
  if (result_node_it != node_to_result_node.end()) return *result_node_it->second;
  ...
  if (const auto *operator_node_ptr = operatorFromDAG(node, result_dag, context, node_to_result_node, key_column_inputs)) {
    ...
    if (atom_node_ptr->type == ActionsDAG::ActionType::INPUT || atom_node_ptr->type == ActionsDAG::ActionType::FUNCTION ||
...
const ActionsDAG::Node *MergeTreeIndexConditionSet::atomFromDAG(
    const ActionsDAG::Node &node, ActionsDAG &result_dag, const ContextPtr &context,
    std::unordered_map<String, const ActionsDAG::Node *> &key_column_inputs) const {
  /// Function, literal or column

  const auto *node_to_check = &node;
  while (node_to_check->type == ActionsDAG::ActionType::ALIAS) node_to_check = node_to_check->children[0];

  if (node_to_check->column && (isColumnConst(*node_to_check->column) || WhichDataType(node.result_type).isSet())) return &node;
  ...
  if (key_columns.contains(column_name)) {
    ...
    if (it != key_column_inputs.end()) return it->second;

    const auto *result_node = node_to_check;

    if (node.type != ActionsDAG::ActionType::INPUT) result_node = &result_dag.addInput(column_name, node.result_type);

    key_column_inputs[column_name] = result_node;
    return result_node;
  }

  if (node.type != ActionsDAG::ActionType::FUNCTION) return nullptr;
...
const ActionsDAG::Node *MergeTreeIndexConditionSet::operatorFromDAG(
    const ActionsDAG::Node &node, ActionsDAG &result_dag, const ContextPtr &context,
    std::unordered_map<const ActionsDAG::Node *, const ActionsDAG::Node *> &node_to_result_node,
    std::unordered_map<String, const ActionsDAG::Node *> &key_column_inputs) const {
  ...
  if (node_to_check->column && (isColumnConst(*node_to_check->column) || WhichDataType(node.result_type).isSet())) return nullptr;

  if (node_to_check->type != ActionsDAG::ActionType::FUNCTION) return nullptr;
  ...
  if (function_name == "not") {
    if (arguments_size != 1) return nullptr;
...
bool MergeTreeIndexConditionSet::checkDAGUseless(const ActionsDAG::Node &node, const ContextPtr &context,
                                                 std::vector<FutureSetPtr> &sets_to_prepare, bool atomic) const {
  const auto *node_to_check = &node;
  while (node_to_check->type == ActionsDAG::ActionType::ALIAS) node_to_check = node_to_check->children[0];
  ...
  if (WhichDataType(node.result_type).isSet()) {
    if (auto set = tree_node.tryGetPreparedSet()) sets_to_prepare.push_back(set);
    return false;
  }
  ...
  if (node.type == ActionsDAG::ActionType::FUNCTION) {
    auto column_name = tree_node.getColumnName();
    if (key_columns.contains(column_name)) return false;
    ...
    if (function_name == "and" || function_name == "or" || function_name == "indexHint") {
      ...
      for (const auto &arg : arguments) {
        ...
        if (function_name == "or" && arg->column && isColumnConst(*arg->column) && !arg->column->getBool(0)) continue;
    ...
    if (function_name == "not") return checkDAGUseless(*arguments.at(0), context, sets_to_prepare, atomic);
...
MergeTreeIndexConditionPtr MergeTreeIndexSet::createIndexCondition(const ActionsDAG::Node *predicate, ContextPtr context) const {
