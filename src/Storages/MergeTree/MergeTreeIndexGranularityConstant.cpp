...
MergeTreeIndexGranularityConstant::MergeTreeIndexGranularityConstant(size_t constant_granularity_)
    : constant_granularity(constant_granularity_), last_mark_granularity(constant_granularity_) {}

MergeTreeIndexGranularityConstant::MergeTreeIndexGranularityConstant(size_t constant_granularity_, size_t last_mark_granularity_,
                                                                     size_t num_marks_without_final_, bool has_final_mark_)
...
size_t MergeTreeIndexGranularityConstant::getMarkRows(size_t mark_index) const {
  ...
  if (mark_index + 1 < num_marks_without_final) return constant_granularity;

  if (mark_index + 1 == num_marks_without_final) return last_mark_granularity;
...
size_t MergeTreeIndexGranularityConstant::getMarksCount() const { return num_marks_without_final + has_final_mark; }

size_t MergeTreeIndexGranularityConstant::getTotalRows() const {
  if (num_marks_without_final == 0) return 0;
...
void MergeTreeIndexGranularityConstant::appendMark(size_t rows_count) {
  if (has_final_mark) {
    ...
    throw Exception(ErrorCodes::LOGICAL_ERROR, "Cannot append mark with {} rows. Granularity is constant ({})", rows_count,
                    constant_granularity);
  } else {
...
void MergeTreeIndexGranularityConstant::adjustLastMark(size_t rows_count) {
  if (has_final_mark) {
    ...
    if (num_marks_without_final == 0) ++num_marks_without_final;
...
size_t MergeTreeIndexGranularityConstant::getMarkUpperBoundForRow(size_t row_index) const {
  ...
  if (row_index >= getTotalRows()) return getMarksCount();

  if (row_index >= num_rows_with_constant_granularity) return num_marks_without_final - 1;
...
void MergeTreeIndexGranularityConstant::fixFromRowsCount(size_t rows_count) {
  if (num_marks_without_final == 0) return;
  ...
  if (rows_count == 0) expected_data_marks = 0;
...
std::shared_ptr<MergeTreeIndexGranularityConstant> MergeTreeIndexGranularityConstant::fixedFromRowsCount(size_t rows_count) const {
  auto result = std::make_shared<MergeTreeIndexGranularityConstant>(constant_granularity, last_mark_granularity, num_marks_without_final,
                                                                    has_final_mark);
...
std::string MergeTreeIndexGranularityConstant::describe() const {
  return fmt::format("Constant(constant_granularity: {}, last_mark_granularity: {}, num_marks_without_final: {}, has_final_mark: {})",
