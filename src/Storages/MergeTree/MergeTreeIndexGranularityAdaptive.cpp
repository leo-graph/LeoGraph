...
size_t MergeTreeIndexGranularityAdaptive::getMarkRows(size_t mark_index) const {
  ...
  if (mark_index == 0) return marks_rows_partial_sums[0];
...
bool MergeTreeIndexGranularityAdaptive::hasFinalMark() const {
  if (marks_rows_partial_sums.empty()) return false;
  return getLastMarkRows() == 0;
}

size_t MergeTreeIndexGranularityAdaptive::getMarksCount() const { return marks_rows_partial_sums.size(); }

size_t MergeTreeIndexGranularityAdaptive::getTotalRows() const {
  if (marks_rows_partial_sums.empty()) return 0;
...
size_t MergeTreeIndexGranularityAdaptive::getRowsCountInRange(size_t begin, size_t end) const {
  ...
  size_t subtrahend = 0;
  if (begin != 0) subtrahend = marks_rows_partial_sums[begin - 1];
...
uint64_t MergeTreeIndexGranularityAdaptive::getBytesSize() const { return marks_rows_partial_sums.size() * sizeof(size_t); }

uint64_t MergeTreeIndexGranularityAdaptive::getBytesAllocated() const { return marks_rows_partial_sums.capacity() * sizeof(size_t); }
...
std::shared_ptr<MergeTreeIndexGranularity> MergeTreeIndexGranularityAdaptive::optimize() {
  ...
  if (marks_count == 0) return nullptr;
