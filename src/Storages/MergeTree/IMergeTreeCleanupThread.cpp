...
void IMergeTreeCleanupThread::start() { task->activateAndSchedule(); }

void IMergeTreeCleanupThread::wakeup() { task->schedule(); }

void IMergeTreeCleanupThread::stop() { task->deactivate(); }
...
void IMergeTreeCleanupThread::wakeupEarlierIfNeeded() {
  ...
  if (!(*storage_settings)[MergeTreeSetting::cleanup_thread_preferred_points_per_iteration]) return;
  ...
  if (number_of_outdated_objects < (*storage_settings)[MergeTreeSetting::cleanup_thread_preferred_points_per_iteration] * 2) return;
  ...
  if (!wakeup_check_timer.compareAndRestart(static_cast<double>((*storage_settings)[MergeTreeSetting::cleanup_delay_period]) / 4.0)) return;
  ...
  if (!prev_run_timestamp_ms || now_ms <= prev_run_timestamp_ms) return;

  /// Don't run it more often than cleanup_delay_period
  UInt64 seconds_passed = (now_ms - prev_run_timestamp_ms) / 1000;
  if (seconds_passed < (*storage_settings)[MergeTreeSetting::cleanup_delay_period]) return;
...
void IMergeTreeCleanupThread::run() {
  ...
  if (prev_timestamp && (*storage_settings)[MergeTreeSetting::cleanup_thread_preferred_points_per_iteration]) {
    ...
    sleep_ms = std::clamp(sleep_ms, (*storage_settings)[MergeTreeSetting::cleanup_delay_period] * 1000,
                          (*storage_settings)[MergeTreeSetting::max_cleanup_delay_period] * 1000);

    UInt64 interval_ms = now_ms - prev_timestamp;
    LOG_TRACE(log, "Scheduling next cleanup after {}ms (points: {}, interval: {}ms, ratio: {}, points per minute: {})", sleep_ms,
              cleanup_points, interval_ms, ratio, cleanup_points / static_cast<Float32>(interval_ms * 60'000));
  }
