...
MergeTreeIndexLegacyHypothesis::MergeTreeIndexLegacyHypothesis(const IndexDescription &index_) : IMergeTreeIndex(index_) {}
...
void legacyHypothesisIndexValidator(const IndexDescription &, bool attach) {
  if (!attach) throw Exception(ErrorCodes::ILLEGAL_INDEX, "Index of type 'hypothesis' is no longer supported. Please drop the index");
}
