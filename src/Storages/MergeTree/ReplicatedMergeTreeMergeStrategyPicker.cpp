...
ReplicatedMergeTreeMergeStrategyPicker::ReplicatedMergeTreeMergeStrategyPicker(StorageReplicatedMergeTree& storage_) : storage(storage_) {}
...
bool ReplicatedMergeTreeMergeStrategyPicker::isMergeFinishedByReplica(const String& replica, const ReplicatedMergeTreeLogEntryData& entry) {
  ...
  if (reference_timestamp == 0) reference_timestamp = entry.create_time;
...
bool ReplicatedMergeTreeMergeStrategyPicker::shouldMergeOnSingleReplica(const ReplicatedMergeTreeLogEntryData& entry) const {
  ...
  return (threshold > 0                                              /// feature turned on
...
std::optional<String> ReplicatedMergeTreeMergeStrategyPicker::pickReplicaToExecuteMerge(const ReplicatedMergeTreeLogEntryData& entry) {
  ...
  if (time(nullptr) - last_refresh_time > REFRESH_STATE_MAXIMUM_INTERVAL_SECONDS) refreshState();
  ...
  if (num_replicas == 0) return std::nullopt;

  auto replica_index = static_cast<int>(hash % num_replicas);

  if (replica_index == current_replica_index) return std::nullopt;
...
void ReplicatedMergeTreeMergeStrategyPicker::refreshState() {
  ...
  if (threshold_init == 0) remote_fs_execute_merges_on_single_replica_time_threshold = threshold_init;
  if (threshold == 0 && threshold_init == 0) return;
  ...
  if (current_replica_index_tmp < 0 || active_replicas_tmp.size() < 2) {
    if (execute_merges_on_single_replica_time_threshold > 0) {
      LOG_WARNING(storage.log,
                  "Can't find current replica in the active replicas list, or too few active replicas to use "
  ...
  LOG_DEBUG(storage.log, "Strategy picker state updated, current replica: {}, active replicas: [{}]", current_replica_index,
