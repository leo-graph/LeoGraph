...
static ASTPtr getCompressionCodecDeltaLZ4() {
  return makeASTFunction("CODEC", make_intrusive<ASTIdentifier>("Delta"), make_intrusive<ASTIdentifier>("LZ4"));
}
...
Field getFieldForConstVirtualColumn(const String& column_name, const IMergeTreeDataPart& part_or_projection) {
  ...
  if (column_name == RowExistsColumn::name) return 1ULL;

  if (column_name == BlockNumberColumn::name) return part.info.min_block;

  if (column_name == "_part") return part.name;

  if (column_name == "_part_uuid") return part.uuid;

  if (column_name == "_partition_id") return part.info.getPartitionId();

  if (column_name == PartDataVersionColumn::name) return part.info.getDataVersion();

  if (column_name == "_partition_value") return Tuple(part.partition.value.begin(), part.partition.value.end());

  if (column_name == "_disk_name") return part.getDataPartStorage().getDiskName();
