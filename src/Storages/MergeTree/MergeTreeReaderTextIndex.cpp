...
MergeTreeReaderTextIndex::MergeTreeReaderTextIndex(const IMergeTreeReader *main_reader_, MergeTreeIndexWithCondition index_,
                                                   NamesAndTypesList columns_, bool can_skip_mark_)
    : IMergeTreeReader(main_reader_->data_part_info_for_read, columns_,
                       /*virtual_fields=*/{}, main_reader_->storage_snapshot, main_reader_->storage_settings,
                       Context::getGlobalContextInstance()->getIndexUncompressedCache().get(),
                       Context::getGlobalContextInstance()->getIndexMarkCache().get(), main_reader_->all_mark_ranges,
  ...
  for (const auto &column : columns_) {
    if (!column.name.starts_with(TEXT_INDEX_VIRTUAL_COLUMN_PREFIX) || !WhichDataType(column.type).isUInt8()) {
      throw Exception(ErrorCodes::LOGICAL_ERROR, "Column {} with type {} should not be filled by text index reader", column.name,
                      column.type->getName());
    }
  }
  ...
  auto make_stream = [&](const auto &substream) {
    return makeTextIndexInputStream(data_part->getDataPartStoragePtr(), index.index->getFileName() + substream.suffix, substream.extension,
...
void MergeTreeReaderTextIndex::updateAllMarkRanges(const MarkRanges &ranges) {
  ...
  if (granule && !ranges.empty()) {
    ...
    if (row_begin != row_end) cleanupPostingsBlocks(RowsRange(row_begin, row_end - 1));
  }
}
...
MergeTreeDataPartPtr MergeTreeReaderTextIndex::getDataPart() const {
  ...
  if (!loaded_data_part) throw Exception(ErrorCodes::NOT_IMPLEMENTED, "Reading text index is supported only for loaded data parts");

  return loaded_data_part->getDataPart();
}

void MergeTreeReaderTextIndex::readGranule() {
  if (!is_prefetched) sparse_index_stream->seekToStart();
...
void MergeTreeReaderTextIndex::initializePostingStreams() {
  ...
  auto make_stream = [&] {
    auto stream = makeTextIndexInputStream(data_part->getDataPartStoragePtr(), index.index->getFileName() + substream.suffix,
                                           substream.extension, MergeTreeIndexReader::patchSettings(settings, substream.type));
  ...
  for (const auto &[token, token_info] : remaining_tokens) {
    if (granule_text.getPostingsForRareToken(token) || !useful_tokens.contains(token)) continue;
...
bool MergeTreeReaderTextIndex::canSkipMark(size_t mark, size_t) {
  ...
  if (!rows_range.has_value()) return true;
  ...
  if (may_be_true) may_be_true_granules.add(static_cast<UInt32>(mark));

  analyzed_granules.add(static_cast<UInt32>(mark));
  return can_skip_mark && !may_be_true;
}

size_t MergeTreeReaderTextIndex::readRows(size_t from_mark, size_t current_task_last_mark, bool continue_reading, size_t max_rows_to_read,
  ...
  if (auto rows_range = getRowsRangeForMark(from_mark - 1)) cleanupPostingsBlocks(*rows_range);
...
void MergeTreeReaderTextIndex::createEmptyColumns(Columns &columns) const {
  for (size_t i = 0; i < columns.size(); ++i) {
    if (columns[i] == nullptr) columns[i] = columns_to_read[i].type->createColumn(*serializations[i]);
  }
}

double MergeTreeReaderTextIndex::estimateCardinality(const TextSearchQuery &query, const TokenToPostingsInfosMap &remaining_tokens,
  ...
  switch (query.search_mode) {
    case TextSearchMode::All: {
      ...
      for (const auto &token : query.tokens) {
        ...
        if (it == remaining_tokens.end()) return 0;
...
PostingsMap MergeTreeReaderTextIndex::readPostingsIfNeeded(size_t mark) {
  auto rows_range = getRowsRangeForMark(mark);
  if (!rows_range.has_value()) return {};
  ...
  for (const auto &[token, token_info] : remaining_tokens) {
    ...
    if (token_postings.size() == 1) {
      ...
      for (const auto &posting : token_postings) *union_posting |= *posting;
...
std::vector<PostingListPtr> MergeTreeReaderTextIndex::readPostingsBlocksForToken(std::string_view token,
                                                                                 const TokenPostingsInfo &token_info,
                                                                                 const RowsRange &range) {
  auto &granule_text = assert_cast<MergeTreeIndexGranuleText &>(*granule);
  auto read_postings = granule_text.getPostingsForRareToken(token);

  if (read_postings) return {read_postings};
  ...
  for (const auto &block_idx : blocks_to_read) {
    ...
    if (inserted) {
      ...
      it->second = MergeTreeIndexGranuleText::readPostingsBlock(*postings_stream, *deserialization_state, token_info, block_idx,
                                                                postings_serialization, granule_text.getIndexIdForCaches());
    }
...
void MergeTreeReaderTextIndex::cleanupPostingsBlocks(const RowsRange &range) {
  ...
  for (const auto &[token, token_info] : remaining_tokens) {
    ...
    if (it == postings_blocks.end()) continue;

    for (size_t i = 0; i < token_info->ranges.size(); ++i) {
      if (!token_info->ranges[i].intersects(range)) it->second.erase(i);
    }
  }
}

/// Finds the union of the posting lists for range [granule_offset, granule_offset + num_rows)
void applyPostingsAny(IColumn &column, PostingsMap &postings_map, PaddedPODArray<UInt32> &indices, const std::vector<String> &search_tokens,
                      size_t column_offset, size_t row_offset, size_t num_rows) {
  ...
  for (const auto &token : search_tokens) {
    ...
    if (it == postings_map.end()) continue;
...
void applyPostingsAll(IColumn &column, PostingsMap &postings_map, PaddedPODArray<UInt32> &indices, const std::vector<String> &search_tokens,
                      size_t column_offset, size_t row_offset, size_t num_rows) {
  ...
  for (const PostingListPtr &posting : token_postings) {
    ...
    if (intersection_posting.cardinality() == 0) return;
  }
...
void MergeTreeReaderTextIndex::fillColumn(IColumn &column, const String &column_name, PostingsMap &postings, size_t row_offset,
                                          size_t num_rows) {
  auto &column_data = assert_cast<ColumnUInt8 &>(column).getData();
  const auto &condition_text = assert_cast<const MergeTreeIndexConditionText &>(*index.condition);
  ...
  if (postings.empty() || search_query->tokens.empty()) return;
