...
static_assert(MAX_CARDINALITY_FOR_EMBEDDED_POSTINGS <= MAX_CARDINALITY_FOR_RAW_POSTINGS,
              "MAX_CARDINALITY_FOR_EMBEDDED_POSTINGS must be less or equal to MAX_CARDINALITY_FOR_RAW_POSTINGS");
static_assert(PostingListBuilder::max_small_size <= MAX_CARDINALITY_FOR_RAW_POSTINGS,
              "max_small_size must be less than or equal to MAX_CARDINALITY_FOR_RAW_POSTINGS");
...
bool DictionaryBlockBase::empty() const { return !tokens || tokens->empty(); }

size_t DictionaryBlockBase::size() const { return tokens ? tokens->size() : 0; }
...
DictionaryBlock::DictionaryBlock(ColumnPtr tokens_, std::vector<TokenPostingsInfo> token_infos_, UInt64 tokens_format_)
    : DictionaryBlockBase(std::move(tokens_)), token_infos(std::move(token_infos_)), tokens_format(tokens_format_) {}

PostingsSerialization::PostingsSerialization(PostingListCodecPtr posting_list_codec_)
    : posting_list_codec(std::move(posting_list_codec_)), raw_postings_buffer(MAX_CARDINALITY_FOR_RAW_POSTINGS) {}
...
void PostingsSerialization::serialize(const PostingList &postings, TokenPostingsInfo &info, size_t posting_list_block_size,
                                      WriteBuffer &ostr) {
...
void PostingsSerialization::serialize(PostingListBuilder &postings, TokenPostingsInfo &info, size_t posting_list_block_size,
                                      WriteBuffer &ostr) {
  if (info.header & IsCompressed) {
    ...
    for (size_t i = 0; i < cardinality; ++i) writeVarUInt(array[i], ostr);
  }
}
...
PostingListPtr PostingsSerialization::deserialize(ReadBuffer &istr, UInt64 header, UInt64 cardinality) {
  if (header & IsCompressed) {
    ...
    if (cardinality > raw_postings_buffer.size()) raw_postings_buffer.resize(cardinality);

    for (size_t i = 0; i < cardinality; ++i) readVarUInt(raw_postings_buffer[i], istr);
...
std::vector<size_t> TokenPostingsInfo::getBlocksToRead(const RowsRange &range) const {
  ...
  for (size_t i = 0; i < ranges.size(); ++i) {
    if (ranges[i].intersects(range)) blocks.emplace_back(i);
  }
  return blocks;
}
...
MergeTreeIndexGranuleText::MergeTreeIndexGranuleText(MergeTreeIndexTextParams params_, PostingListCodecPtr posting_list_codec_)
    : params(std::move(params_)), postings_serialization(std::move(posting_list_codec_)) {}
...
ColumnPtr deserializeTokensFrontCoding(ReadBuffer &istr, size_t num_tokens) {
  ...
  if (num_tokens != 0) {
    ...
    /// Read the first token
    {
      ...
      if (offset > data.size()) data.resize_exact(roundUpToPowerOfTwoOrZero(std::max(offset, data.size() * 2)));
...
void MergeTreeIndexGranuleText::deserializeBinaryWithMultipleStreams(MergeTreeIndexInputStreams &streams,
...
void MergeTreeIndexGranuleText::analyzeDictionary(MergeTreeIndexReaderStream &header_stream, MergeTreeIndexReaderStream &dictionary_stream,
                                                  MergeTreeIndexDeserializationState &state) {
  auto tokens_to_read = fillTokensFromCache(state);
  if (tokens_to_read.empty()) return;

  auto sparse_index = loadSparseIndex(header_stream, state);
  if (sparse_index->empty()) return;
  ...
  for (const auto &[block_idx, needed_tokens] : blocks_to_read) {
    ...
    for (const auto &token : needed_tokens) {
      ...
      if (idx_in_block < num_tokens && block_tokens.getDataAt(idx_in_block) == token) matched_indices.emplace_back(idx_in_block);
    }
    ...
    auto infos = TextIndexSerialization::deserializeTokenInfos(*data_buffer, num_tokens, matched_indices, postings_serialization);
...
std::vector<String> MergeTreeIndexGranuleText::fillTokensFromCache(MergeTreeIndexDeserializationState &state) {
  ...
  for (const auto &token : all_search_tokens) keys.emplace_back(TextIndexTokensCache::hash(index_id_for_caches, token));
...
DictionarySparseIndexPtr MergeTreeIndexGranuleText::loadSparseIndex(MergeTreeIndexReaderStream &header_stream,
                                                                    MergeTreeIndexDeserializationState &state) {
...
PostingListPtr MergeTreeIndexGranuleText::readPostingsBlock(MergeTreeIndexReaderStream &stream, MergeTreeIndexDeserializationState &state,
                                                            const TokenPostingsInfo &token_info, size_t block_idx,
...
size_t MergeTreeIndexGranuleText::memoryUsageBytes() const {
  return sizeof(*this) + remaining_tokens.capacity() * sizeof(*remaining_tokens.begin()) +
...
bool MergeTreeIndexGranuleText::hasAnyQueryTokens(const TextSearchQuery &query) const {
  if (!current_range.has_value()) {
    return std::ranges::any_of(query.tokens, [this](const auto &token) { return remaining_tokens.contains(token); });
  }
  ...
  for (const auto &token : query.tokens) {
    ...
    if (it == remaining_tokens.end()) continue;

    bool has_any_range = std::ranges::any_of(it->second->ranges, [this](const auto &range) { return current_range->intersects(range); });

    if (!has_any_range) continue;
...
bool MergeTreeIndexGranuleText::hasAllQueryTokens(const TextSearchQuery &query) const {
  if (query.tokens.empty()) return false;
...
bool MergeTreeIndexGranuleText::hasAllQueryTokensOrEmpty(const TextSearchQuery &query) const {
  if (!current_range.has_value()) {
    return std::ranges::all_of(query.tokens, [this](const auto &token) { return remaining_tokens.contains(token); });
  }
  ...
  for (const auto &token : query.tokens) {
    auto it = remaining_tokens.find(token);
    if (it == remaining_tokens.end()) return false;

    bool has_any_range = std::ranges::any_of(it->second->ranges, [this](const auto &range) { return current_range->intersects(range); });

    if (!has_any_range) return false;
...
MergeTreeIndexGranuleTextWritable::MergeTreeIndexGranuleTextWritable(
    MergeTreeIndexTextParams params_, PostingListCodecPtr posting_list_codec_, SortedTokensAndPostings &&tokens_and_postings_,
    TokenToPostingsBuilderMap &&tokens_map_, std::list<PostingList> &&posting_lists_, std::unique_ptr<Arena> &&arena_)
...
size_t computeCommonPrefixLength(const std::string_view lhs, const std::string_view rhs) {
  ...
  while (common_prefix_length < max_length && lhs[common_prefix_length] == rhs[common_prefix_length]) ++common_prefix_length;
...
std::vector<roaring::api::roaring_bitmap_t> splitPostings(const PostingList &postings, size_t block_size) {
  ...
  if (begin_index < static_cast<size_t>(container.size)) result.emplace_back(create_bitmap_view(begin_index, container.size - begin_index));

  return result;
}

template <typename TokenGetter>
void serializeTokensImpl(const TokenGetter &token_getter, WriteBuffer &ostr, TextIndexSerialization::TokensFormat format,
...
TokenPostingsInfo TextIndexSerialization::serializePostings(PostingListBuilder &postings, MergeTreeIndexWriterStream &postings_stream,
  ...
  if (info.header & IsCompressed) {
    ...
    for (const auto &block : blocks) {
      if (roaring::api::roaring_bitmap_get_cardinality(&block) == 0) continue;
...
void TextIndexSerialization::serializeTokens(const ColumnString &tokens, WriteBuffer &ostr, TokensFormat format) {
  serializeTokensImpl([&](size_t i) { return tokens.getDataAt(i); }, ostr, format,
                      /*block_begin=*/0,
...
void TextIndexSerialization::serializeTokenInfo(WriteBuffer &ostr, const TokenPostingsInfo &token_info) {
  ...
  if (token_info.header & EmbeddedPostings) return;

  if (!(token_info.header & SingleBlock)) writeVarUInt(token_info.offsets.size(), ostr);
...
TokenPostingsInfo TextIndexSerialization::deserializeTokenInfo(ReadBuffer &istr, PostingsSerialization *postings_serialization) {
  ...
  if (info.header & EmbeddedPostings) {
    if (skip_postings) {
      chassert(info.header & RawPostings);
      for (size_t i = 0; i < info.cardinality; ++i) ignoreVarUInt(istr);
    } else {
    ...
    if (!(info.header & SingleBlock)) readVarUInt(num_postings_blocks, istr);
...
void TextIndexSerialization::skipTokenInfo(ReadBuffer &istr) {
  ...
  if (header & EmbeddedPostings) {
    ...
    for (size_t i = 0; i < cardinality; ++i) ignoreVarUInt(istr);
  } else {
    UInt64 num_postings_blocks = 1;

    if (!(header & SingleBlock)) readVarUInt(num_postings_blocks, istr);
...
std::vector<TokenPostingsInfo> TextIndexSerialization::deserializeTokenInfos(ReadBuffer &istr, size_t num_tokens,
  ...
  if (matched_indices.empty()) return result;
...
DictionaryBlock TextIndexSerialization::deserializeDictionaryBlock(ReadBuffer &istr, PostingsSerialization *postings_serialization) {
  ...
  for (size_t i = 0; i < num_tokens; ++i) token_infos.emplace_back(deserializeTokenInfo(istr, postings_serialization));

  return DictionaryBlock{std::move(tokens_column), std::move(token_infos), std::move(tokens_format)};
}

template <typename Stream>
DictionarySparseIndex serializeTokensAndPostings(const SortedTokensAndPostings &tokens_and_postings, Stream &dictionary_stream,
                                                 Stream &postings_stream, const MergeTreeIndexTextParams &params,
                                                 PostingsSerialization &postings_serialization) {
  ...
  for (size_t block_idx = 0; block_idx < num_blocks; ++block_idx) {
    ...
    serializeTokensImpl([&](size_t i) { return tokens_and_postings[i].first; }, dictionary_stream.compressed_hashing, tokens_format,
...
void MergeTreeIndexGranuleTextWritable::serializeBinaryWithMultipleStreams(MergeTreeIndexOutputStreams &streams) const {
  ...
  auto sparse_index_block =
      serializeTokensAndPostings(tokens_and_postings, *dictionary_stream, *postings_stream, params, postings_serialization);
...
size_t MergeTreeIndexGranuleTextWritable::memoryUsageBytes() const {
  ...
  for (const auto &plist : posting_lists) posting_lists_size += plist.getSizeInBytes();

  return sizeof(*this)
         /// can ignore the sizeof(PostingListBuilder) here since it is just references to tokens_map
         + tokens_and_postings.capacity() * sizeof(SortedTokensAndPostings::value_type) + tokens_map.getBufferSizeInBytes() +
         posting_lists_size + arena->allocatedBytes();
}

MergeTreeIndexTextGranuleBuilder::MergeTreeIndexTextGranuleBuilder(MergeTreeIndexTextParams params_, TokenizerPtr tokenizer_,
                                                                   PostingListCodecPtr posting_list_codec_)
    : params(std::move(params_)), tokenizer(tokenizer_), posting_list_codec(posting_list_codec_), arena(std::make_unique<Arena>()) {}

PostingListBuilder::PostingListBuilder(PostingList *posting_list)
    : large{posting_list, roaring::BulkContext()}, small_size(max_small_size) {}
...
void PostingListBuilder::add(UInt32 value, PostingListsHolder &postings_holder) {
  if (small_size < max_small_size) {
    if (small_size) {
      ...
      if (small[small_size - 1] == value) return;
    }
    ...
    if (small_size == max_small_size) {
      ...
      for (size_t i = 0; i < max_small_size; ++i) large.postings->addBulk(large.context, small_copy[i]);
    }
  } else {
...
void MergeTreeIndexTextGranuleBuilder::addDocument(std::string_view document) {
  forEachToken(*tokenizer, document.data(), document.size(), [&](const char *token_start, size_t token_length) {
    bool inserted;
...
std::unique_ptr<MergeTreeIndexGranuleTextWritable> MergeTreeIndexTextGranuleBuilder::build() {
  ...
  tokens_map.forEachValue([&](const auto &key, auto &mapped) { sorted_values.emplace_back(key, &mapped); });

  std::ranges::sort(sorted_values, [](const auto &lhs, const auto &rhs) { return lhs.first < rhs.first; });

  return std::make_unique<MergeTreeIndexGranuleTextWritable>(params, posting_list_codec, std::move(sorted_values), std::move(tokens_map),
                                                             std::move(posting_lists), std::move(arena));
}
...
MergeTreeIndexAggregatorText::MergeTreeIndexAggregatorText(String index_column_name_, MergeTreeIndexTextParams params_,
                                                           TokenizerPtr tokenizer_, PostingListCodecPtr posting_list_codec_,
  ...
  if (isArray(index_column.type)) {
    ...
    for (size_t i = offset; i < offset + rows_read; ++i) {
      for (size_t element_idx = column_offsets[i - 1]; element_idx < column_offsets[i]; ++element_idx) {
        if (data_is_nullable && column_data.isNullAt(element_idx)) continue;
...
MergeTreeIndexText::MergeTreeIndexText(const IndexDescription &index_, MergeTreeIndexTextParams params_,
                                       std::unique_ptr<ITokenizer> tokenizer_, std::unique_ptr<IPostingListCodec> posting_list_codec_)
...
MergeTreeIndexSubstreams MergeTreeIndexText::getSubstreams() const {
  return {{MergeTreeIndexSubstream::Type::Regular, "", ".idx"},
          {MergeTreeIndexSubstream::Type::TextIndexDictionary, ".dct", ".idx"},
          {MergeTreeIndexSubstream::Type::TextIndexPostings, ".pst", ".idx"}};
}

MergeTreeIndexFormat MergeTreeIndexText::getDeserializedFormat(const MergeTreeDataPartChecksums &checksums,
                                                               const std::string &path_prefix) const {
  if (indexFileExistsInChecksums(checksums, path_prefix, ".idx")) return {1, getSubstreams()};
...
MergeTreeIndexAggregatorPtr MergeTreeIndexText::createIndexAggregator() const {
  return std::make_shared<MergeTreeIndexAggregatorText>(index.column_names[0], params, tokenizer.get(), posting_list_codec.get(),
                                                        preprocessor);
}
...
Type castAs(const Field &field, std::string_view argument_name) {
  ...
  if (expected_type != field.getType()) {
    throw Exception(ErrorCodes::BAD_ARGUMENTS, "Text index argument '{}' expected to be {}, but got {}", argument_name,
                    fieldTypeToString(Field::TypeToEnum<Type>::value), field.getTypeName());
  }
...
ASTPtr extractASTOption(std::unordered_map<String, ASTPtr> &options, const String &option, bool is_required) {
  ...
  if (is_required) throw Exception(ErrorCodes::BAD_ARGUMENTS, "Text index argument '{}' is required", option);
...
std::pair<String, ASTPtr> parseNamedArgument(const ASTFunction *ast_equal_function) {
  ...
  if (!key_identifier)
    throw Exception(ErrorCodes::BAD_ARGUMENTS, "Text index argument must be a key-value pair. got {}",
...
MergeTreeIndexPtr textIndexCreator(const IndexDescription &index) {
  ...
  UInt64 dictionary_block_size =
      extractFieldOption<UInt64>(options, ARGUMENT_DICTIONARY_BLOCK_SIZE).value_or(DEFAULT_DICTIONARY_BLOCK_SIZE);
  ...
  UInt64 posting_list_block_size =
      extractFieldOption<UInt64>(options, ARGUMENT_POSTING_LIST_BLOCK_SIZE).value_or(DEFAULT_POSTING_LIST_BLOCK_SIZE);

  MergeTreeIndexTextParams index_params{dictionary_block_size, dictionary_block_frontcoding_compression, posting_list_block_size,
...
void textIndexValidator(const IndexDescription &index, bool /*attach*/) {
  ...
  if (dictionary_block_size == 0)
    throw Exception(ErrorCodes::BAD_ARGUMENTS, "Text index argument '{}' must be greater than 0, but got {}",
                    ARGUMENT_DICTIONARY_BLOCK_SIZE, dictionary_block_size);
  ...
  if (dictionary_block_use_fc_compression > 1)
    throw Exception(ErrorCodes::BAD_ARGUMENTS, "Text index argument '{}' must be 0 or 1, but got {}",
                    ARGUMENT_DICTIONARY_BLOCK_FRONTCODING_COMPRESSION, dictionary_block_use_fc_compression);

  UInt64 posting_list_block_size =
      extractFieldOption<UInt64>(options, ARGUMENT_POSTING_LIST_BLOCK_SIZE).value_or(DEFAULT_POSTING_LIST_BLOCK_SIZE);
  if (posting_list_block_size == 0)
    throw Exception(ErrorCodes::BAD_ARGUMENTS, "Text index argument '{}' must be greater than 0, but got {}",
                    ARGUMENT_POSTING_LIST_BLOCK_SIZE, posting_list_block_size);
  ...
  if (!data_type.isString() && !data_type.isFixedString()) {
    throw Exception(ErrorCodes::BAD_ARGUMENTS, "Text index must be created on columns of type `String` or `FixedString`");
  }
