...
class ReplicatedMergeTreeCleanupThread : public IMergeTreeCleanupThread {
 public:
  ...
  static size_t clearOldBlocks(const String& zookeeper_path, const String& blocks_dir_name, zkutil::ZooKeeper& zookeeper,
                               UInt64 window_seconds, UInt64 window_size, NodeCTimeAndVersionCache& cached_block_stats, LoggerPtr log_);
 ...
 private:
  ...
  void markLostReplicas(const std::unordered_map<String, UInt32>& host_versions_lost_replicas,
                        const std::unordered_map<String, String>& log_pointers_candidate_lost_replicas, size_t replicas_count,
  ...
  static void getBlocksSortedByTime(const String& zookeeper_path, const String& blocks_dir_name, zkutil::ZooKeeper& zookeeper,
                                    std::vector<NodeWithStat>& timed_blocks, NodeCTimeAndVersionCache& cached_block_stats, LoggerPtr log_);
