...
ActiveDataPartSet::ActiveDataPartSet(MergeTreeDataFormatVersion format_version_, const Strings& names) : format_version(format_version_) {
...
void ActiveDataPartSet::checkIntersectingParts(const MergeTreePartInfo& part_info) const {
    ...
    if (!part_info.contains(it->first)) {
      if (!part_info.isDisjoint(it->first))
        throw Exception(ErrorCodes::LOGICAL_ERROR,
                        "Part {} intersects previous part {}. It is a bug or a result of manual intervention in the ZooKeeper data.",
  ...
  if (it != part_info_to_name.end() && !part_info.isDisjoint(it->first))
    throw Exception(ErrorCodes::LOGICAL_ERROR,
                    "Part {} intersects next part {}. It is a bug or a result of manual intervention in the ZooKeeper data.",
                    part_info.getPartNameForLogs(), it->first.getPartNameForLogs());
}
...
ActiveDataPartSet::AddPartOutcome ActiveDataPartSet::addImpl(const MergeTreePartInfo& part_info, const String& name,
                                                             Strings* out_replaced_parts, String* out_reason) {
  /// TODO make it exception safe (out_replaced_parts->push_back(...) may throw)

  if (getContainingPartImpl(part_info) != part_info_to_name.end()) return AddPartOutcome::HasCovering;

  /// Parts contained in `part` are located contiguously in `part_info_to_name`, overlapping with the place where the part itself would be
  /// inserted.
  auto it = part_info_to_name.lower_bound(part_info);

  if (out_replaced_parts) out_replaced_parts->clear();
  ...
  while (it != part_info_to_name.begin()) {
    ...
    if (out_replaced_parts) out_replaced_parts->push_back(it->second);
    it = part_info_to_name.erase(it);
  }

  if (out_replaced_parts) std::reverse(out_replaced_parts->begin(), out_replaced_parts->end());
  ...
  if (it != part_info_to_name.end() && !part_info.isDisjoint(it->first)) {
    if (out_reason != nullptr)
      *out_reason = fmt::format(
          ...
          name, it->first.getPartNameForLogs());
...
String ActiveDataPartSet::getContainingPart(const MergeTreePartInfo& part_info) const {
  ...
  if (it != part_info_to_name.end()) return it->second;
  return {};
}
...
std::map<MergeTreePartInfo, String>::const_iterator ActiveDataPartSet::getContainingPartImpl(const MergeTreePartInfo& part_info) const {
  /// A part can only be covered/overlapped by the previous or next one in `part_info_to_name`.
  auto it = part_info_to_name.lower_bound(part_info);

  if (it != part_info_to_name.end()) {
    if (it->first.contains(part_info)) return it;
  }
...
std::vector<std::map<MergeTreePartInfo, String>::const_iterator> ActiveDataPartSet::getPartsCoveredByImpl(
    const MergeTreePartInfo& part_info) const {
  ...
  while (begin != part_info_to_name.begin()) {
    ...
    if (!part_info.contains(prev->first)) {
      if (prev->first.contains(part_info)) return {};
  ...
  while (end != part_info_to_name.end()) {
    if (!part_info.contains(end->first)) {
      if (end->first.contains(part_info)) return {};
      break;
    }
  ...
  for (auto it = begin; it != end; ++it) covered.push_back(it);
...
Strings ActiveDataPartSet::getPartsCoveredBy(const MergeTreePartInfo& part_info) const {
  Strings covered;
  for (const auto& it : getPartsCoveredByImpl(part_info)) covered.push_back(it->second);
  return covered;
}
...
std::vector<MergeTreePartInfo> ActiveDataPartSet::getPartInfosCoveredBy(const MergeTreePartInfo& part_info) const {
  ...
  for (const auto& it : getPartsCoveredByImpl(part_info)) covered.push_back(it->first);
  return covered;
}
...
Strings ActiveDataPartSet::getParts() const {
  ...
  for (const auto& kv : part_info_to_name) res.push_back(kv.second);
...
std::vector<MergeTreePartInfo> ActiveDataPartSet::getPartInfos() const {
  ...
  res.reserve(part_info_to_name.size());
  for (const auto& kv : part_info_to_name) res.push_back(kv.first);
...
std::vector<MergeTreePartInfo> ActiveDataPartSet::getPatchPartInfos() const {
  ...
  for (const auto& kv : part_info_to_name) {
    if (kv.first.isPatch()) res.push_back(kv.first);
  }

  return res;
}

bool ActiveDataPartSet::isEmpty() const { return part_info_to_name.empty(); }

bool ActiveDataPartSet::hasSome() const { return !isEmpty(); }

size_t ActiveDataPartSet::size() const { return part_info_to_name.size(); }
