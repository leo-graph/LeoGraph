...
void PartMovesBetweenShardsOrchestrator::run() {
  if (!(*storage.getSettings())[MergeTreeSetting::part_moves_between_shards_enable]) return;
...
void PartMovesBetweenShardsOrchestrator::start() { task->activateAndSchedule(); }

void PartMovesBetweenShardsOrchestrator::wakeup() { task->schedule(); }
...
bool PartMovesBetweenShardsOrchestrator::step() {
  if (!storage.is_leader) return false;
  ...
  /// Try find an entry to process and copy it.
  {
    ...
    for (auto const &entry : entries) {
      if (entry.state.value == EntryState::DONE || entry.state.value == EntryState::CANCELLED) continue;
  ...
  if (!entry_to_process.has_value()) return false;
  ...
  LOG_DEBUG(log, "stepEntry on task {} from state {} (rollback: {}), try: {}", entry_to_process->znode_name,
            entry_to_process->state.toString(), entry_to_process->rollback, entry_to_process->num_tries);
  ...
  switch (entry.state.value) {
    ...
    case EntryState::SYNC_SOURCE: {
      ...
      if (zk->tryGet(sync_source_log_entry_barrier_path, sync_source_log_entry_str, &sync_source_log_entry_stat)) {
        ...
        ops.emplace_back(zkutil::makeCreateRequest(zookeeper_path + "/log/log-", sync_source_log_entry.toString(),
    ...
    case EntryState::SYNC_DESTINATION: {
      ...
      if (zk->tryGet(sync_destination_log_entry_barrier_path, sync_destination_log_entry_str, &sync_destination_log_entry_stat)) {
        ...
        sync_destination_log_entry =
            *ReplicatedMergeTreeLogEntry::parse(sync_destination_log_entry_str, sync_destination_log_entry_stat, storage.format_version);
      } else {
        /// Log entry.
        Coordination::Requests ops;
        ops.emplace_back(zkutil::makeCheckRequest(entry.znode_path, entry.version));
        ...
        ops.emplace_back(zkutil::makeCreateRequest(entry.to_shard + "/log/log-", sync_destination_log_entry.toString(),
                                                   zkutil::CreateMode::PersistentSequential));
    ...
    case EntryState::DESTINATION_FETCH: {
      ...
      if (zk->tryGet(fetch_log_entry_barrier_path, fetch_log_entry_str, &fetch_log_entry_stat)) {
        ...
        ops.emplace_back(zkutil::makeSetRequest(entry.to_shard + "/log", "", -1));
        ops.emplace_back(
            zkutil::makeCreateRequest(entry.to_shard + "/log/log-", fetch_log_entry.toString(), zkutil::CreateMode::PersistentSequential));
    ...
    case EntryState::DESTINATION_ATTACH: {
      ...
      if (entry.rollback) {
        ...
        if (zk->tryGet(attach_rollback_log_entry_barrier_path, attach_rollback_log_entry_str, &attach_rollback_log_entry_stat)) {
          ...
          attach_rollback_log_entry =
              *ReplicatedMergeTreeLogEntry::parse(attach_rollback_log_entry_str, attach_rollback_log_entry_stat, storage.format_version);
        } else {
          ...
          storage.getClearBlocksInPartitionOps(ops, *zk, drop_part_info.getPartitionId(), drop_part_info.min_block,
                                               drop_part_info.max_block);
          ...
          ops.emplace_back(zkutil::makeCreateRequest(entry.to_shard + "/log/log-", attach_rollback_log_entry.toString(),
                                                     zkutil::CreateMode::PersistentSequential));
          ops.emplace_back(zkutil::makeSetRequest(entry.to_shard + "/log", "", -1));
          Coordination::Responses responses;
          Coordination::Error rc = zk->tryMulti(ops, responses);
      ...
      if (block_number_lock.isLocked()) {
        ...
        ops.emplace_back(zkutil::makeSetRequest(entry.to_shard + "/log", "", -1));
        ops.emplace_back(
            zkutil::makeCreateRequest(entry.to_shard + "/log/log-", log_entry.toString(), zkutil::CreateMode::PersistentSequential));
    ...
    case EntryState::SOURCE_DROP_PRE_DELAY: {
      ...
      std::this_thread::sleep_for(
          std::chrono::seconds((*storage.getSettings())[MergeTreeSetting::part_moves_between_shards_delay_seconds]));
      entry.state = EntryState::SOURCE_DROP;
      return entry;
    }

    case EntryState::SOURCE_DROP: {
      if (entry.rollback) throw Exception(ErrorCodes::LOGICAL_ERROR, "It is not possible to rollback from this state. This is a bug.");
      ...
      if (zk->tryGet(source_drop_log_entry_barrier_path, source_drop_log_entry_str, &source_drop_log_entry_stat)) {
        ...
        storage.getClearBlocksInPartitionOps(ops, *zk, source_drop_part_info.getPartitionId(), source_drop_part_info.min_block,
                                             source_drop_part_info.max_block);
        ...
        ops.emplace_back(zkutil::makeSetRequest(zookeeper_path + "/log", "", -1));
        ops.emplace_back(zkutil::makeCreateRequest(zookeeper_path + "/log/log-", source_drop_log_entry.toString(),
                                                   zkutil::CreateMode::PersistentSequential));
...
CancellationCode PartMovesBetweenShardsOrchestrator::killPartMoveToShard(const UUID &task_uuid) {
  while (true) {
    ...
    if (entry.state.value >= not_possible_to_rollback_after_state.value) {
      LOG_DEBUG(log, "Can't kill move part between shards entry {} ({}) after state {}. Current state: {}.", toString(entry.task_uuid),
                entry.znode_name, not_possible_to_rollback_after_state.toString(), entry.state.toString());
      return CancellationCode::CancelCannotBeSent;
    }

    LOG_TRACE(log, "Will try to mark move part between shards entry {} ({}) for rollback.", toString(entry.task_uuid), entry.znode_name);
...
PartMovesBetweenShardsOrchestrator::Entry PartMovesBetweenShardsOrchestrator::getEntryByUUID(const UUID &task_uuid) {
  ...
  std::lock_guard lock(state_mutex);
  for (auto const &entry : entries) {
    if (entry.task_uuid == task_uuid) return entry;
  }
