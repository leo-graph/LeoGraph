...
static String extractFixedPrefixFromRegularExpression(const String &regexp) {
  if (regexp.size() <= 1 || regexp[0] != '^') return {};
  ...
  while (pos < end) {
    switch (*pos) {
      ...
      case '?':
      case '*':
        if (!fixed_prefix.empty()) fixed_prefix.pop_back();
...
const KeyCondition::AtomMap KeyCondition::atom_map{
    ...
    {"empty",
     [](RPNElement &out, const Field &value) {
       if (value.getType() != Field::Types::String) return false;
    ...
    {"like",
     [](RPNElement &out, const Field &value) {
       ...
       if (!is_perfect) out.relaxed = true;

       String right_bound = firstStringThatIsGreaterThanAllStringsWithPrefix(prefix);

       out.function = RPNElement::FUNCTION_IN_RANGE;
       out.range = !right_bound.empty() ? Range(prefix, true, right_bound, false) : Range::createLeftBounded(prefix, true);
    ...
    {"startsWithUTF8",
     [](RPNElement &out, const Field &value) {
       ...
       if (is_ascii == false) return false;
    ...
    {"pointInPolygon", [](RPNElement &out, const Field &) {
...
static const std::set<std::string_view> no_relaxed_atom_functions = {"notLike",         "notIn",     "globalNotIn", "notNullIn",
                                                                     "globalNotNullIn", "notEquals", "notEmpty"};
...
static bool isTrivialCast(const ActionsDAG::Node &node) {
  ...
  if (!column_const) return false;

  Field field = column_const->getField();
  if (field.getType() != Field::Types::String) return false;
...
static const ActionsDAG::Node &cloneDAGWithInversionPushDown(
    const ActionsDAG::Node &node, ActionsDAG &inverted_dag,
    std::unordered_map<const ActionsDAG::Node *, const ActionsDAG::Node *> &inputs_mapping, const ContextPtr &context,
      ...
      if (name == "not") {
        ...
        if (const auto *adaptor = typeid_cast<const FunctionToFunctionBaseAdaptor *>(node.function_base.get())) {
          if (const auto *index_hint = typeid_cast<const FunctionIndexHint *>(adaptor->getFunction().get())) {
            ...
            for (auto &arg : children) arg = &cloneDAGWithInversionPushDown(*arg, inverted_dag, inputs_mapping, context, need_inversion);
          }
        }
        ...
        for (auto &arg : children) arg = &cloneDAGWithInversionPushDown(*arg, inverted_dag, inputs_mapping, context, false);
  ...
  if (!handled_inversion && need_inversion) res = &inverted_dag.addFunction(FunctionFactory::instance().get("not", context), {res}, "");
...
const std::unordered_map<String, KeyCondition::SpaceFillingCurveType> KeyCondition::space_filling_curve_name_to_type{
    {"mortonEncode", SpaceFillingCurveType::Morton}, {"hilbertEncode", SpaceFillingCurveType::Hilbert}};
 ...
 */
Block KeyCondition::getBlockWithConstants(const ASTPtr &query, const TreeRewriterResultPtr &syntax_analyzer_result, ContextPtr context) {
...
static NameSet getAllSubexpressionNames(const ExpressionActions &key_expr) {
  NameSet names;
  for (const auto &action : key_expr.getActions()) names.insert(action.node->result_name);
...
void KeyCondition::getAllSpaceFillingCurves(const BuildInfo &info) {
  ...
  for (const auto &action : info.key_expr->getActions()) {
    if (action.node->type == ActionsDAG::ActionType::FUNCTION && action.node->children.size() >= 2 &&
      ...
      if (2 == curve.arguments.size()) key_space_filling_curves.push_back(std::move(curve));
    }
  }
}
...
KeyCondition::KeyCondition(const ActionsDAGWithInversionPushDown &filter_dag, ContextPtr context, const Names &key_column_names_,
                           const ExpressionActionsPtr &key_expr_, bool single_point_, bool skip_analysis_)
    : num_key_columns(key_column_names_.size()),
      single_point(single_point_),
      date_time_overflow_behavior_ignore(context->getSettingsRef()[Setting::date_time_overflow_behavior] ==
  ...
  if (context->getSettingsRef()[Setting::analyze_index_with_space_filling_curves]) getAllSpaceFillingCurves(info);
  ...
  RPNBuilder<RPNElement> builder(filter_dag.predicate, context,
                                 [&](const RPNBuilderTreeNode &node, RPNElement &out) { return extractAtomFromTree(node, info, out); });
...
KeyCondition::KeyCondition(ThisIsPrivate, ColumnIndices key_columns_, size_t num_key_columns_, bool single_point_,
...
bool KeyCondition::addCondition(const String &column, const Range &range) {
  if (!key_columns.contains(column)) return false;
...
static FieldRef applyFunction(const FunctionBasePtr &func, const DataTypePtr &current_type, const FieldRef &field) {
  ...
  if (field.isExplicit()) return applyFunctionForField(func, current_type, field);
  ...
  for (size_t i = 0; i < result_idx; ++i) {
    if ((*columns)[i].name == result_name) result_idx = i;
  }
  ...
  if (result_idx == columns->size()) {
    ...
    (*columns)[result_idx].column =
        func->execute(args, (*columns)[result_idx].type, columns->front().column->size(), /* dry_run = */ false);
  }
...
bool applyFunctionChainToColumn(const ColumnPtr &in_column, const DataTypePtr &in_data_type, const std::vector<FunctionBasePtr> &functions,
                                ColumnPtr &out_column, DataTypePtr &out_data_type) {
  ...
  for (const auto &func : functions) {
    if (func->getArgumentTypes().empty()) return false;

    auto argument_type = removeLowCardinality(getArgumentTypeOfMonotonicFunction(*func));
    if (!canBeSafelyCast(result_type, argument_type)) return false;
    ...
    if (isDateTime64(arg_type_inner) || isTime64(arg_type_inner)) {
      ...
      UInt32 scale = isDateTime64(arg_type_inner) ? assert_cast<const DataTypeDateTime64 &>(*arg_type_inner).getScale()
      ...
      if (isDate(result_type_inner) && seconds >= static_cast<Int64>(DATE_LUT_MAX_DAY_NUM) * 86400) return false;
      if (isDateTime(result_type_inner) && seconds >= DATE_LUT_MAX) return false;
      if (isUInt32(result_type_inner) && seconds > static_cast<Int64>(std::numeric_limits<UInt32>::max())) return false;
      ...
      if (value < 0) return false;
      if (isDate(result_type_inner) && value > DATE_LUT_MAX_DAY_NUM) return false;
      if (isDateTime(result_type_inner) && value * 86400LL >= DATE_LUT_MAX) return false;
      if (isUInt32(result_type_inner) && value * 86400LL > static_cast<Int64>(std::numeric_limits<UInt32>::max())) return false;
    }
    ...
    if (isDate(result_type_inner)) {
      if (result_column->getUInt(0) >= DATE_LUT_MAX_DAY_NUM) return false;
    } else if (isDateTime(result_type_inner)) {
      if (result_column->get64(0) >= DATE_LUT_MAX) return false;
    }
  }
  out_column = result_column;
  out_data_type = result_type;
...
bool KeyCondition::isFunctionReallyMonotonic(const IFunctionBase &func, const IDataType &arg_type) const {
  if (date_time_overflow_behavior_ignore && func.getName() == "toDateTime") {
    ...
    if (const auto *lowcard_type = typeid_cast<const DataTypeLowCardinality *>(type)) type = lowcard_type->getDictionaryType().get();
    if (const auto *nullable_type = typeid_cast<const DataTypeNullable *>(type)) type = nullable_type->getNestedType().get();

    /// toDateTime(date) may overflow, breaking monotonicity.
    if (isDateOrDate32(type)) return false;
  }

  return true;
}

bool KeyCondition::canConstantBeWrappedByMonotonicFunctions(const RPNBuilderTreeNode &node, const BuildInfo &info,
                                                            size_t &out_key_column_num, DataTypePtr &out_key_column_type, Field &out_value,
                                                            DataTypePtr &out_type) {
  String expr_name = node.getColumnName();

  if (!info.key_subexpr_names.contains(expr_name)) return false;

  if (out_value.isNull()) return false;

  MonotonicFunctionsChain transform_functions;
  auto can_transform_constant = extractMonotonicFunctionsChainFromKey(
      node.getTreeContext().getQueryContext(), expr_name, info, out_key_column_num, out_key_column_type, transform_functions,
      [this](const IFunctionBase &func, const IDataType &type) {
        if (!func.hasInformationAboutMonotonicity()) return false;

        if (!isFunctionReallyMonotonic(func, type)) return false;
  ...
  if (!can_transform_constant) return false;

  auto const_column = out_type->createColumnConst(1, out_value);

  ColumnPtr transformed_const_column;
  DataTypePtr transformed_const_type;
  bool constant_transformed =
      applyFunctionChainToColumn(const_column, out_type, transform_functions, transformed_const_column, transformed_const_type);

  if (!constant_transformed) return false;
...
bool KeyCondition::extractDeterministicFunctionsDagFromKey(const String &expr_name, const BuildInfo &info, size_t &out_key_column_num,
                                                           DataTypePtr &out_key_column_type, DeterministicKeyTransformDag &out) const {
  ...
  for (const auto &node : dag.getNodes())
    if (node.result_name == expr_name) expr_nodes.push_back(&node);

  if (expr_nodes.empty()) return false;
  ...
  for (const auto *node : expr_nodes) expr_set.insert(node);

  std::unordered_map<const ActionsDAG::Node *, const ActionsDAG::Node *> projection_inputs;
  projection_inputs.reserve(expr_nodes.size());
  for (const auto *node : expr_nodes) projection_inputs.emplace(node, rename_node);
  ...
  auto can_fold_from_expr_only = [&](const ActionsDAG::Node *key_node) -> bool {
    ...
    while (!stack.empty()) {
      ...
      if (node->type == ActionsDAG::ActionType::INPUT) return false;

      if (node->type == ActionsDAG::ActionType::ARRAY_JOIN || node->type == ActionsDAG::ActionType::PLACEHOLDER) return false;
  ...
  for (const auto &key_node : dag.getNodes()) {
    ...
    if (it == key_columns.end()) continue;

    if (!can_fold_from_expr_only(&key_node)) continue;

    ActionsDAG sub = ActionsDAG::foldActionsByProjection(projection_inputs, {&key_node});

    const auto required = sub.getRequiredColumns();
    if (required.size() != 1 || required.front().name != expr_name) continue;

    if (sub.hasNonDeterministic() || sub.hasStatefulFunctions() || sub.hasArrayJoin()) continue;
...
bool applyDeterministicDagToColumn(const ColumnPtr &in_column, const DataTypePtr &in_type, const String &input_name,
                                   const DeterministicKeyTransformDag &dag, ColumnPtr &out_column, DataTypePtr &out_type) {
  ...
  auto cast_without_nulls = [&](ColumnPtr &column, DataTypePtr &type, const DataTypePtr &target_type) -> bool {
    ...
    if (!target_type->isNullable()) column = n.getNestedColumnPtr();
  ...
  if (!input_type->equals(*dag.input_type)) {
    ...
    auto try_apply_direct_cast_fast_path = [&]() -> bool {
      ...
      if (!cast_arg || cast_arg->type != ActionsDAG::ActionType::INPUT || cast_arg->result_name != input_name) return false;
      ...
      if (!input_type->equals(*cast_result_type) && !cast_without_nulls(out_column, out_type, cast_result_type)) return false;

      return finalize_output_column_and_type(out_column, out_type);
    };

    if (try_apply_direct_cast_fast_path()) return true;

    if (!cast_without_nulls(input_column, input_type, dag.input_type)) return false;
  }
...
bool KeyCondition::canConstantBeWrappedByDeterministicFunctions(const RPNBuilderTreeNode &node, const BuildInfo &info,
                                                                size_t &out_key_column_num, DataTypePtr &out_key_column_type,
                                                                Field &out_value, DataTypePtr &out_type, bool &out_is_injective) {
  ...
  if (!extractDeterministicFunctionsDagFromKey(expr_name, info, out_key_column_num, out_key_column_type, dag)) return false;
  ...
  DataTypePtr transformed_const_type;
  bool constant_transformed =
      applyDeterministicDagToColumn(const_column, out_type, expr_name, dag, transformed_const_column, transformed_const_type);
...
void KeyCondition::analyzeKeyExpressionForSetIndex(const RPNBuilderTreeNode &arg,
                                                   ...
                                                   DataTypes &data_types, size_t &args_count, const BuildInfo &info, bool &out_relaxed) {
  auto get_key_tuple_position_mapping = [&](const RPNBuilderTreeNode &node, size_t tuple_index) {
    ...
    if (isKeyPossiblyWrappedByMonotonicFunctions(node, info, index_mapping.key_index, key_space_filling_curve_argument_pos, data_type,
                                                 index_mapping.functions) &&
        !key_space_filling_curve_argument_pos)  /// We don't support the analysis of space-filling curves and IN set.
    {
      ...
      if (canSetValuesBeWrappedByDeterministicFunctions(node, info, index_mapping.key_index, data_type, set_transforming_dag,
                                                        is_injective)) {
        indexes_mapping.push_back(index_mapping);
        data_types.push_back(data_type);
        set_transforming_dags.push_back(std::move(set_transforming_dag));
        if (!is_injective) out_relaxed = true;
      }
    }
  };
  ...
  if (arg.isFunction()) {
    ...
    if (arg_tuple.getFunctionName() == "tuple" && arg_tuple.getArgumentsSize() > 1) {
      ...
      for (size_t i = 0; i < args_count; ++i) get_key_tuple_position_mapping(arg_tuple.getArgumentAt(i), i);
    } else {
...
bool tryPrepareSetColumnsForIndex(Columns &set_columns, DataTypes &set_types,
                                  const std::vector<std::optional<DeterministicKeyTransformDag>> &set_transforming_dags,
                                  const DataTypes &data_types,
                                  const std::vector<MergeTreeSetIndex::KeyTuplePositionMapping> &indexes_mapping, size_t args_count) {
  Columns new_columns;
  DataTypes new_types;
  ...
  for (size_t indexes_mapping_index = 0; indexes_mapping_index < indexes_mapping.size(); ++indexes_mapping_index) {
    ...
    if (set_transforming_dags[indexes_mapping_index].has_value()) {
      ...
      if (!applyDeterministicDagToColumn(set_column, set_element_type, set_transforming_dag.input_name, set_transforming_dag,
                                         transformed_set_column, transformed_set_type))
        return false;
    ...
    if (!key_column_type->canBeInsideNullable()) return false;
    ...
    if (isNullableOrLowCardinalityNullable(set_element_type)) {
      ...
      if (!set_column_nullable) return false;

      const NullMap &null_map_data = set_column_nullable->getNullMapData();
      if (!null_map_data.empty()) set_column_null_map = &null_map_data;
    ...
    if (!nullable_set_column_typed) return false;
    ...
    if (set_column_null_map) {
      ...
      for (size_t i = 0; i < nullable_set_column_null_map_size; ++i) filter[i] &= !nullable_set_column_null_map[i];
...
bool KeyCondition::tryPrepareSetIndexForIn(const RPNBuilderFunctionTreeNode &func, const BuildInfo &info, RPNElement &out) {
  ...
  analyzeKeyExpressionForSetIndex(left_arg, indexes_mapping, set_transforming_dags, data_types, left_args_count, info, out.relaxed);

  if (indexes_mapping.empty()) return false;

  const RPNBuilderTreeNode &right_arg = func.getArgumentAt(1);
  auto future_set = right_arg.tryGetPreparedSet();
  if (!future_set) return false;

  auto prepared_set = future_set->buildOrderedSetInplace(right_arg.getTreeContext().getQueryContext());
  if (!prepared_set) return false;

  /// The index can be prepared if the elements of the set were saved in advance.
  if (!prepared_set->hasExplicitSetElements()) return false;
  ...
  if (!tryPrepareSetColumnsForIndex(set_columns, set_types, set_transforming_dags, data_types, indexes_mapping, left_args_count))
    return false;
  ...
  for (const auto &index_mapping : adjusted_indexes_mapping) out.key_columns.push_back(index_mapping.key_index);
  ...
  if (adjusted_indexes_mapping.size() < set_types.size()) out.relaxed = true;

  if (out.set_index->size() > 1 || out.relaxed) relaxed = true;

  return true;
}

bool KeyCondition::tryPrepareSetIndexForHas(const RPNBuilderFunctionTreeNode &func, const BuildInfo &info, RPNElement &out) {
  ...
  analyzeKeyExpressionForSetIndex(key_arg, indexes_mapping, set_transforming_dags, data_types, key_args_count, info, out.relaxed);
  ...
  if (!tryPrepareSetColumnsForIndex(set_columns, set_types, set_transforming_dags, data_types, indexes_mapping, key_args_count))
    return false;
...
class FunctionWithOptionalConstArg : public IFunctionBase {
 public:
  ...
  ColumnPtr execute(const ColumnsWithTypeAndName &arguments, const DataTypePtr &result_type, size_t input_rows_count,
    ...
    if (kind == Kind::LEFT_CONST) {
      ...
      for (const auto &arg : arguments) new_arguments.push_back(arg);
  ...
  IFunctionBase::Monotonicity getMonotonicityForRange(const IDataType &type, const Field &left, const Field &right) const override {
    if (const auto *adaptor = typeid_cast<const FunctionToFunctionBaseAdaptor *>(func.get())) {
      if (dynamic_cast<FunctionDateOrDateTimeBase *>(adaptor->getFunction().get()) && kind == Kind::RIGHT_CONST) {
        ...
        if (type_ptr->isNullable()) type_ptr = static_cast<const DataTypeNullable &>(*type_ptr).getNestedType().get();
...
DataTypePtr getArgumentTypeOfMonotonicFunction(const IFunctionBase &func) {
  ...
  if (const auto *func_ptr = typeid_cast<const FunctionWithOptionalConstArg *>(&func)) {
    if (func_ptr->getKind() == FunctionWithOptionalConstArg::Kind::LEFT_CONST) return arg_types.at(1);
  }

  return arg_types.at(0);
}

bool KeyCondition::isKeyPossiblyWrappedByMonotonicFunctions(const RPNBuilderTreeNode &node, const BuildInfo &info,
  ...
  if (!isKeyPossiblyWrappedByMonotonicFunctionsImpl(node, info, out_key_column_num, out_argument_num_of_space_filling_curve,
                                                    key_column_type, chain_not_tested_for_monotonicity))
    return false;
  ...
  for (auto it = chain_not_tested_for_monotonicity.rbegin(); it != chain_not_tested_for_monotonicity.rend(); ++it) {
    ...
    if (function.getArgumentsSize() == 2) {
      ...
      if (const_arg.column->isNullAt(0)) return false;
    } else
      arguments.push_back({nullptr, key_column_type, ""});
    ...
    if (!assume_function_monotonicity && !isFunctionReallyMonotonic(*func, *key_column_type)) return false;
...
bool KeyCondition::isKeyPossiblyWrappedByMonotonicFunctionsImpl(const RPNBuilderTreeNode &node, const BuildInfo &info,
                                                                size_t &out_key_column_num,
                                                                std::optional<size_t> &out_argument_num_of_space_filling_curve,
  ...
  if (node.isFunction()) {
    ...
    if (arguments_size > 2 || arguments_size == 0) return false;
    ...
    if (arguments_size == 2) {
      if (function_node.getArgumentAt(0).isConstant()) {
        result =
            isKeyPossiblyWrappedByMonotonicFunctionsImpl(function_node.getArgumentAt(1), info, out_key_column_num,
                                                         out_argument_num_of_space_filling_curve, out_key_column_type, out_functions_chain);
      } else if (function_node.getArgumentAt(1).isConstant()) {
        result =
            isKeyPossiblyWrappedByMonotonicFunctionsImpl(function_node.getArgumentAt(0), info, out_key_column_num,
                                                         out_argument_num_of_space_filling_curve, out_key_column_type, out_functions_chain);
      }
    } else {
      result =
...
bool KeyCondition::extractMonotonicFunctionsChainFromKey(
    ContextPtr context, const String &expr_name, const BuildInfo &info, size_t &out_key_column_num, DataTypePtr &out_key_column_type,
    MonotonicFunctionsChain &out_functions_chain, std::function<bool(const IFunctionBase &, const IDataType &)> always_monotonic) const {
  const auto &sample_block = info.key_expr->getSampleBlock();
    ...
    if (it != key_columns.end()) {
      ...
      while (is_valid_chain) {
        ...
        if (cur_node->type == ActionsDAG::ActionType::FUNCTION && cur_node->children.size() <= 2) {
          ...
          for (const auto *arg : cur_node->children) {
            ...
            if (next_node) is_valid_chain = false;
      ...
      if (is_valid_chain) {
        while (!chain.empty()) {
          ...
          if (func->type != ActionsDAG::ActionType::FUNCTION) continue;
          ...
          if (date_time_parsing_functions.contains(func_name)) {
            ...
            if (has_string_argument && has_session_timezone) return false;
            ...
            if (has_string_argument) {
              ...
              for (const auto &type : func->function_base->getArgumentTypes()) new_args.push_back({nullptr, type, ""});
...
bool KeyCondition::canSetValuesBeWrappedByDeterministicFunctions(const RPNBuilderTreeNode &node, const BuildInfo &info,
                                                                 size_t &out_key_column_num, DataTypePtr &out_key_res_column_type,
  ...
  if (!extractDeterministicFunctionsDagFromKey(expr_name, info, out_key_column_num, out_key_res_column_type, out_transform)) return false;

  out_is_injective =
      isDeterministicTransformInjective(out_transform.actions->getActionsDAG(), out_transform.input_name, out_transform.output_name);
  return true;
}
...
KeyCondition::RPNElement::RPNElement() : polygon(std::make_shared<Polygon>()) {}

KeyCondition::RPNElement::RPNElement(Function function_) : function(function_), polygon(std::make_shared<Polygon>()) {}

KeyCondition::RPNElement::RPNElement(Function function_, std::vector<size_t> key_columns_)
    : function(function_), key_columns(std::move(key_columns_)), polygon(std::make_shared<Polygon>()) {}

KeyCondition::RPNElement::RPNElement(Function function_, std::vector<size_t> key_columns_, const Range &range_)
    : function(function_), range(range_), key_columns(std::move(key_columns_)), polygon(std::make_shared<Polygon>()) {}
 ...
 */
static bool tryRewriteFloatLiteralForIntKeyComparison(const DataTypePtr &key_type, Field &const_value, DataTypePtr &const_type,
                                                      std::string &func_name, KeyCondition::RPNElement &out) {
  if (const_value.isNull() || const_value.isNaN()) return false;

  /// `isNativeInteger` types: Int8, Int16, Int32, Int64, UInt8, UInt16, UInt32, UInt64.
  if (!isNativeInteger(key_type) || const_value.getType() != Field::Types::Float64) return false;
  ...
  const bool is_supported_operator = (func_name == "less" || func_name == "lessOrEquals" || func_name == "greater" ||
                                      func_name == "greaterOrEquals" || func_name == "equals" || func_name == "notEquals");

  if (!is_supported_operator) return false;
...
bool KeyCondition::extractAtomFromTree(const RPNBuilderTreeNode &node, const BuildInfo &info, RPNElement &out) {
  ...
  if (node.isFunction()) {
    ...
    if (atom_map.find(func_name) == std::end(atom_map)) return false;

    auto analyze_point_in_polygon = [&, this]() -> bool {
      /// pointInPolygon((x, y), [(0, 0), (8, 4), (5, 8), (0, 2)])
      if (func.getArgumentAt(0).tryGetConstant(const_value, const_type)) return false;
      if (!func.getArgumentAt(1).tryGetConstant(const_value, const_type)) return false;
      ...
      if (!func.getArgumentAt(0).isFunction()) return false;

      auto first_argument = func.getArgumentAt(0).toFunctionNode();
      if (first_argument.getArgumentsSize() != 2 || first_argument.getFunctionName() != "tuple") return false;

      for (size_t i = 0; i < 2; ++i) {
        auto name = first_argument.getArgumentAt(i).getColumnName();
        auto it = key_columns.find(name);
        if (it == key_columns.end()) return false;
      ...
      for (const auto &elem : const_value.safeGet<Array>()) {
        if (elem.getType() != Field::Types::Tuple) return false;

        const auto &elem_tuple = elem.safeGet<Tuple>();
        if (elem_tuple.size() != 2) return false;
    ...
    if (num_args == 1) {
      if (!(isKeyPossiblyWrappedByMonotonicFunctions(func.getArgumentAt(0), info, key_column_num, argument_num_of_space_filling_curve,
                                                     key_expr_type, chain)))
        return false;
      ...
      if (functionIsInOrGlobalInOperator(func_name)) {
        if (tryPrepareSetIndexForIn(func, info, out)) {
          ...
          if (valid_atom && out.relaxed) relaxed = true;
          return valid_atom;
        } else
          return false;
      }
      ...
      if (func_name == "has") {
        if (tryPrepareSetIndexForHas(func, info, out)) {
          ...
          if (out.function == RPNElement::ALWAYS_FALSE) return true;
      ...
      if (const_value.isNaN()) {
        ...
        if (key_arg.isNullable()) return false;
      ...
      if (isKeyPossiblyWrappedByMonotonicFunctions(key_arg, info, key_column_num, argument_num_of_space_filling_curve, key_expr_type, chain,
                                                   single_point)) {
      } else if (!no_relaxed_atom_functions.contains(func_name) &&
                 canConstantBeWrappedByMonotonicFunctions(key_arg, info, key_column_num, key_expr_type, const_value, const_type)) {
        condition_is_relaxed = true;
      } else if (func_name == "equals" || func_name == "notEquals") {
        bool is_injective = false;
        if (!canConstantBeWrappedByDeterministicFunctions(key_arg, info, key_column_num, key_expr_type, const_value, const_type,
                                                          is_injective))
          return false;
      ...
      if (key_arg_pos == 1) {
        if (func_name == "less")
          ...
          func_name = "greaterOrEquals";
        else if (func_name == "like" || func_name == "notLike" || func_name == "ilike" || func_name == "notILike" ||
      ...
      bool cast_not_needed = (isNativeInteger(key_expr_type_not_null) || isDateTimeOrDateTime64(key_expr_type_not_null)) &&
      ...
      if (!cast_not_needed && !key_expr_type_not_null->equals(*const_type)) {
        if (const_value.getType() == Field::Types::String) {
          ...
          const bool should_keep_original_string_constant = isStringOrFixedString(key_expr_type_not_null) &&
                                                            (func_name == "like" || func_name == "notLike" || func_name == "startsWith" ||
          ...
          if (!should_keep_original_string_constant) {
            ...
            if (const_value.isNull()) return false;
          ...
          const bool float_literal_was_rewritten_for_int_key =
              tryRewriteFloatLiteralForIntKeyComparison(key_expr_type_not_null, const_value, const_type, func_name, out);
          ...
          if (!float_literal_was_rewritten_for_int_key) {
            ...
            if (!const_type->equals(*common_type)) {
              ...
              if (!key_expr_type_not_null->equals(*common_type)) condition_is_relaxed = true;
            }
            if (!key_expr_type_not_null->equals(*common_type)) {
              ...
              if (!single_point && !func_cast->hasInformationAboutMonotonicity()) return false;
...
void KeyCondition::findHyperrectanglesForArgumentsOfSpaceFillingCurves() {
  ...
  auto num_arguments_of_a_curve = [&](size_t key_column_pos) {
    for (const auto &curve : key_space_filling_curves)
      if (curve.key_column_pos == key_column_pos) return curve.arguments.size();
    return 0uz;
  };
...
KeyCondition::Description KeyCondition::getDescription() const {
  ...
  auto combine = [](std::unique_ptr<Node> left, std::unique_ptr<Node> right, Node::Type type) {
    ...
    if (type == Node::Type::And) {
      /// false AND right
      if (left->type == Node::Type::False) return left;

      /// left AND false
      if (right->type == Node::Type::False) return right;

      /// true AND right
      if (left->type == Node::Type::True) return right;

      /// left AND true
      if (right->type == Node::Type::True) return left;
    }

    if (type == Node::Type::Or) {
      /// false OR right
      if (left->type == Node::Type::False) return right;

      /// left OR false
      if (right->type == Node::Type::False) return left;

      /// true OR right
      if (left->type == Node::Type::True) return left;

      /// left OR true
      if (right->type == Node::Type::True) return right;
    }

    return std::make_unique<Node>(Node{.type = type, .left = std::move(left), .right = std::move(right)});
  };
  ...
  if (rpn_stack.size() != 1) throw Exception(ErrorCodes::LOGICAL_ERROR, "Unexpected stack size in KeyCondition::getDescription");

  std::vector<String> key_names(num_key_columns);
  std::vector<bool> is_key_used(num_key_columns, false);

  for (const auto &key : key_columns) key_names[key.second] = key.first;
  ...
  describe = [&describe, &key_names, &is_key_used, &buf](const Node *node) {
    switch (node->type) {
      case Node::Type::Leaf: {
        for (size_t idx : node->element->key_columns) is_key_used[idx] = true;
  ...
  for (size_t i = 0; i < key_names.size(); ++i)
    if (is_key_used[i]) description.used_keys.emplace_back(key_names[i]);
 ...
 */
template <typename F>
static BoolMask forAnyHyperrectangle(size_t key_size, const FieldRef *left_keys, const FieldRef *right_keys, bool left_bounded,
                                     bool right_bounded,
                                     Hyperrectangle &hyperrectangle,  /// This argument is modified in-place for the callback
                                     const DataTypes &data_types, size_t prefix_size, BoolMask initial_mask, F &&callback) {
  if (!left_bounded && !right_bounded) return callback(hyperrectangle);
  ...
  if (prefix_size == key_size) return callback(hyperrectangle);
  ...
  if (left_bounded) {
    hyperrectangle[prefix_size] = Range(left_keys[prefix_size]);
    result = BoolMask::combine(result, forAnyHyperrectangle(key_size, left_keys, right_keys, true, false, hyperrectangle, data_types,
  ...
  if (right_bounded) {
    ...
    result = BoolMask::combine(result, forAnyHyperrectangle(key_size, left_keys, right_keys, false, true, hyperrectangle, data_types,
                                                            prefix_size + 1, initial_mask, callback));
  }
...
BoolMask KeyCondition::checkInRange(size_t used_key_size, const FieldRef *left_keys, const FieldRef *right_keys,
                                    const DataTypes &data_types, BoolMask initial_mask) const {
  ...
  return forAnyHyperrectangle(
      used_key_size, left_keys, right_keys, true, true, key_ranges, data_types, 0, initial_mask,
      [&](const Hyperrectangle &key_ranges_hyperrectangle) { return checkInHyperrectangle(key_ranges_hyperrectangle, data_types); });
}

std::optional<Range> KeyCondition::applyMonotonicFunctionsChainToRange(Range key_range, const MonotonicFunctionsChain &functions,
                                                                       DataTypePtr current_type, bool single_point) {
  for (const auto &func : functions) {
    ...
    if (!monotonicity.is_positive) key_range.invert();
  }
  return key_range;
}
...
bool KeyCondition::matchesExactContinuousRange() const {
  ...
  auto check_monotonicity_of_chain = [&field](const std::vector<FunctionBasePtr> &chain) -> std::pair<bool, bool> {
    ...
    for (const auto &func : chain) {
      if (!func || !func->hasInformationAboutMonotonicity()) return {false, false};

      auto arg_type = getArgumentTypeOfMonotonicFunction(*func);
      if (!arg_type) return {false, false};
      ...
      if (!all_always_monotonic && !all_strict) break;
    }
  ...
  for (const auto &element : rpn) {
    ...
    if (element.function == RPNElement::Function::FUNCTION_IN_SET && element.set_index && element.set_index->size() == 1) {
      for (const auto &mapping : element.set_index->getIndexesMapping()) {
        ...
        if (!is_chain_always_monotonic) return false;
        ...
        if (is_chain_strict)
          ...
          if (constraint != Constraint::POINT) constraint = Constraint::RANGE;
        }
      }
    ...
    if (element.function == RPNElement::Function::FUNCTION_IN_RANGE) {
      ...
      if (element.range.left == element.range.right) {
        ...
        /// So, POINT needs to be converted to a RANGE
        if (is_chain_strict) constraint = Constraint::POINT;
      }
...
bool KeyCondition::extractPlainRanges(Ranges &ranges) const {
  if (key_columns.size() != 1) return false;

  if (hasMonotonicFunctionsChain()) return false;
  ...
  for (const auto &element : rpn) {
    ...
    } else  /// atom relational expression or constants
    {
      if (element.function == RPNElement::FUNCTION_IN_RANGE) {
        ...
        if (element.set_index->hasMonotonicFunctionsChain()) return false;
        ...
        for (size_t i = 0; i < element.set_index->size(); i++) {
          ...
          if (pre) {
            ...
            /// skip blank range
            if (!(r.left > r.right || (r.left == r.right && !r.left_included && !r.right_included))) points_range.push_back(r);
          } else {
  ...
  if (rpn_stack.size() != 1) throw Exception(ErrorCodes::LOGICAL_ERROR, "Unexpected stack size in KeyCondition::extractPlainRanges");
...
Ranges KeyCondition::extractBounds() const {
  ...
  if (key_columns.size() != 1) return {Range::createWholeUniverseWithoutNull()};

  if (!has_filter) return {Range::createWholeUniverseWithoutNull()};
  ...
  for (const auto &[start, end] : topLevelConjunction()) {
    ...
    if (!one_conjunct.extractPlainRanges(conjunct_ranges)) continue;

    extracted_any = true;

    /// If any conjunct is unsatisfiable, the whole AND is unsatisfiable.
    if (conjunct_ranges.empty()) return {};

    bounds = bounds.intersectWith(PlainRanges(conjunct_ranges));

    /// Contradictory bounds between conjuncts (e.g. `x < 5 AND x > 10`).
    if (bounds.ranges.empty()) return {};
  }

  /// No useful information could be extracted from any conjunct.
  if (!extracted_any) return {Range::createWholeUniverseWithoutNull()};

  return std::move(bounds.ranges);
}

BoolMask KeyCondition::checkInHyperrectangle(const Hyperrectangle &hyperrectangle, const DataTypes &data_types,
  ...
  auto curve_type = [&](size_t key_column_pos) {
    for (const auto &curve : key_space_filling_curves)
      if (curve.key_column_pos == key_column_pos) return curve.type;
  ...
  for (const auto &element : rpn) {
    if (element.argument_num_of_space_filling_curve.has_value()) {
      ...
      if (key_column >= hyperrectangle.size()) {
        throw Exception(ErrorCodes::LOGICAL_ERROR, "Hyperrectangle size is {}, but requested element at position {} ({})",
      ...
      if (!element.monotonic_functions_chain.empty()) {
        std::optional<Range> new_range =
            applyMonotonicFunctionsChainToRange(key_range, element.monotonic_functions_chain, data_types[key_column], single_point);
      ...
      if (unlikely(key_range.left.isNaN() || key_range.right.isNaN())) contains = false;
      ...
      if (element.relaxed) rpn_stack.back().can_be_false = true;

      if (element.function == RPNElement::FUNCTION_NOT_IN_RANGE) rpn_stack.back() = !rpn_stack.back();
      ...
      if (key_range.left.getType() == Field::Types::UInt64 && key_range.right.getType() == Field::Types::UInt64) {
    ...
    } else if (element.function == RPNElement::FUNCTION_IS_NULL || element.function == RPNElement::FUNCTION_IS_NOT_NULL) {
      ...
      if (element.function == RPNElement::FUNCTION_IS_NULL) rpn_stack.back() = !rpn_stack.back();
    } else if (element.function == RPNElement::FUNCTION_IN_SET || element.function == RPNElement::FUNCTION_NOT_IN_SET) {
      if (!element.set_index) throw Exception(ErrorCodes::LOGICAL_ERROR, "Set for IN is not created yet");
      ...
      if (element.function == RPNElement::FUNCTION_NOT_IN_SET) rpn_stack.back() = !rpn_stack.back();
    } else if (element.function == RPNElement::FUNCTION_NOT) {
      assert(!rpn_stack.empty());
  ...
  if (rpn_stack.size() != 1) throw Exception(ErrorCodes::LOGICAL_ERROR, "Unexpected stack size in KeyCondition::checkInHyperrectangle");

  return rpn_stack[0];
}

void KeyCondition::prepareBloomFilterData(
    std::function<std::optional<uint64_t>(size_t column_idx, const Field &)> hash_one,
  ...
  for (auto &rpn_element : rpn) {
    ...
    if (rpn_element.function == RPNElement::FUNCTION_IN_RANGE || rpn_element.function == RPNElement::FUNCTION_NOT_IN_RANGE) {
    ...
    } else if (rpn_element.function == RPNElement::FUNCTION_IN_SET || rpn_element.function == RPNElement::FUNCTION_NOT_IN_SET) {
...
bool KeyCondition::mayBeTrueInRange(size_t used_key_size, const FieldRef *left_keys, const FieldRef *right_keys,
...
String KeyCondition::RPNElement::toString(const std::vector<String> &key_names) const {
  ...
  auto print_column_name = [&](size_t key_column) {
    if (argument_num_of_space_filling_curve) buf << "argument " << *argument_num_of_space_filling_curve << " of ";
  ...
  auto print_wrapped_columns = [&]() {
    ...
    for (size_t i = 0; i < key_columns.size(); ++i) {
      ...
      if (set_index) print_functions_chain_end(set_index->getIndexesMapping().at(i).functions);
    }
...
bool KeyCondition::alwaysUnknownOrTrue() const { return unknownOrAlwaysTrue(false); }

bool KeyCondition::anyUnknownOrAlwaysTrue() const { return unknownOrAlwaysTrue(true); }
...
bool KeyCondition::unknownOrAlwaysTrue(bool unknown_any) const {
  ...
  if (rpn_stack.size() != 1) throw Exception(ErrorCodes::LOGICAL_ERROR, "Unexpected stack size in KeyCondition::unknownOrAlwaysTrue");
...
bool KeyCondition::alwaysFalse() const {
  ...
  if (rpn_stack.size() != 1) throw Exception(ErrorCodes::LOGICAL_ERROR, "Unexpected stack size in KeyCondition::alwaysFalse");
...
bool KeyCondition::hasMonotonicFunctionsChain() const {
  for (const auto &element : rpn)
    if (!element.monotonic_functions_chain.empty() || (element.set_index && element.set_index->hasMonotonicFunctionsChain())) return true;
  return false;
}
  ...
  for (size_t i = 0; i < rpn.size(); ++i) {
    ...
    switch (elem.function) {
      ...
      case RPNElement::FUNCTION_AND: {
        ...
        if (left.conjuncts.size() < right.conjuncts.size()) std::swap(left.conjuncts, right.conjuncts);
...
void KeyCondition::extractSingleColumnConditions(std::vector<std::pair<size_t, std::shared_ptr<KeyCondition>>> &out_column_conditions,
                                                 std::shared_ptr<KeyCondition> *out_complex_condition) const {
  ...
  for (auto range : conjuncts) {
    ...
    for (size_t i = range.first; i < range.second; ++i) {
      ...
      if (element.key_columns.size() > 1) is_complex = true;
    ...
    if (!key_column.has_value()) is_complex = true;

    if (is_complex)
      complex_conjuncts.push_back(range);
    else {
      if (conjuncts_by_key_column.size() <= *key_column) conjuncts_by_key_column.resize(*key_column + 1);
  ...
  auto add_rpn_ranges = [](KeyCondition &target, const KeyCondition &source, const RPNRanges &ranges) {
    for (size_t j = 0; j < ranges.size(); ++j) {
      ...
      if (j > 0) target.rpn.emplace_back(RPNElement::FUNCTION_AND);
    }
  };

  if (!all_complex) {
    std::vector<const String *> key_column_names(num_key_columns);
    for (const auto &[name, index] : key_columns) key_column_names[index] = &name;
    ...
    for (size_t i = 0; i < conjuncts_by_key_column.size(); ++i) {
      ...
      auto condition = std::make_shared<KeyCondition>(ThisIsPrivate(), std::move(one_key_column), num_key_columns, single_point,
                                                      date_time_overflow_behavior_ignore, relaxed);
...
std::unordered_set<size_t> KeyCondition::getUsedColumns() const {
  ...
  for (const RPNElement &element : rpn) res.insert(element.key_columns.begin(), element.key_columns.end());
  return res;
}
