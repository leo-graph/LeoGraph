...
class AlterConversions : private boost::noncopyable {
 public:
  ...
  AlterConversions(const MutationCommands& mutation_commands_, const PatchPartsForReader& patch_parts_, const ContextPtr& context);
  ...
  PrewhereExprSteps getMutationSteps(const IMergeTreeDataPartInfoForReader& part_info, const NamesAndTypesList& read_columns,
 ...
 private:
  ...
  std::vector<MutationActions> getMutationActions(const IMergeTreeDataPartInfoForReader& part_info, const NamesAndTypesList& read_columns,
                                                  const StorageMetadataPtr& metadata_snapshot, const ContextPtr& context) const;

  /// Adds source columns of expressions of MATERIALIZED columns from @read_columns if any.
  void addColumnsRequiredForMaterialized(Names& read_columns, NameSet& read_columns_set, const StorageMetadataPtr& metadata_snapshot,
