  ...
  for (const auto &child : index_function->arguments->children) {
    ...
    if (!unique_index_expression_hashes.emplace(first_half_of_hash).second)
      throw Exception(ErrorCodes::BAD_ARGUMENTS,
                      "Primary key or secondary index contains a duplicate expression. To suppress this exception, rerun the command with "
                      "setting 'allow_suspicious_indices = 1'");
  }
}
...
static void checkSampleExpression(const StorageInMemoryMetadata &metadata, bool allow_sampling_expression_not_in_primary_key,
                                  bool check_sample_column_is_correct) {
  ...
  if (!check_sample_column_is_correct) return;
...
static bool hasColumnsWithDynamicSubcolumns(const Block &block) {
  for (const auto &column : block.getColumnsWithTypeAndName()) {
    if (column.type->hasDynamicSubcolumns()) return true;
  }

  return false;
}

void MergeTreeData::initializeDirectoriesAndFormatVersion(const std::string &relative_data_path_, bool attach,
                                                          const std::string &date_column_name, bool need_create_directories) {
  ...
  if ((*settings)[MergeTreeSetting::table_disk]) {
    ...
    if (relative_data_path.empty()) throw Exception(ErrorCodes::INCORRECT_FILE_NAME, "MergeTree storages require data path");
  }

  MergeTreeDataFormatVersion min_format_version(0);
  if (date_column_name.empty()) min_format_version = MERGE_TREE_DATA_MIN_FORMAT_VERSION_WITH_CUSTOM_PARTITIONING;
  ...
  for (const auto &disk : getDisks()) {
    ...
    if (auto buf = disk->readFileIfExists(format_version_path, getReadSettings())) {
      ...
      if (!buf->eof()) throw Exception(ErrorCodes::CORRUPTED_DATA, "Bad version file: {}", fullPath(disk, format_version_path));
      ...
      if (!read_format_version.has_value())
        ...
        throw Exception(ErrorCodes::CORRUPTED_DATA, "Version file on {} contains version {} expected version is {}.",
  ...
  if (!attach || !read_format_version) {
    ...
    for (const auto &disk : getStoragePolicy()->getDisks()) {
      ...
      if (!disk->isReadOnly() && !disk->isWriteOnce()) {
        ...
        buf->finalize();
        if (getContext()->getSettingsRef()[Setting::fsync_metadata]) buf->sync();
      }
...
DataPartsLock::DataPartsLock(SharedMutex &data_parts_mutex_, const MergeTreeData *data_)
    : wait_watch(Stopwatch(CLOCK_MONOTONIC)), lock(data_parts_mutex_), lock_watch(Stopwatch(CLOCK_MONOTONIC)), data(data_) {
...
DataPartsLock::~DataPartsLock() {
  ...
  if (lock_watch.has_value()) ProfileEvents::increment(ProfileEvents::PartsLockHoldMicroseconds, lock_watch->elapsedMicroseconds());
}

DataPartsSharedLock::DataPartsSharedLock(DB::SharedMutex &data_parts_mutex_)
    : wait_watch(Stopwatch(CLOCK_MONOTONIC)), lock(data_parts_mutex_), lock_watch(Stopwatch(CLOCK_MONOTONIC)) {
...
DataPartsSharedLock::~DataPartsSharedLock() {
  if (lock_watch.has_value()) ProfileEvents::increment(ProfileEvents::SharedPartsLockHoldMicroseconds, lock_watch->elapsedMicroseconds());
}

static Int64 extractVersion(const auto &versions, const String &partition_id, Int64 default_value) {
  if (!versions) return default_value;

  auto it = versions->find(partition_id);
  if (it == versions->end()) return default_value;
...
bool MergeTreeData::IMutationsSnapshot::needIncludeMutationToSnapshot(const Params &params, const MutationCommands &commands) {
  for (const auto &command : commands) {
    if (params.need_data_mutations && AlterConversions::isSupportedDataMutation(command.type)) return true;

    if (params.need_alter_mutations && AlterConversions::isSupportedAlterMutation(command.type)) return true;

    /// Metadata mutations must be included into the snapshot regardless of the parameters.
    if (AlterConversions::isSupportedMetadataMutation(command.type)) return true;
  }
...
NameSet MergeTreeData::MutationsSnapshotBase::getColumnsUpdatedInPatches() const {
  if (!params.need_patch_parts) return {};
  ...
  for (const auto &[_, patches] : patches_by_partition) {
    for (const auto &patch : patches) {
      ...
      for (const auto &column : columns) {
        if (!isPatchPartSystemColumn(column.name)) res.insert(column.name);
      }
    }
  }
  return res;
...
void MergeTreeData::MutationsSnapshotBase::addSupportedCommands(const MutationCommands &commands, UInt64 mutation_version,
                                                                MutationCommands &result_commands) const {
...
PatchParts MergeTreeData::MutationsSnapshotBase::getPatchesForPart(const DataPartPtr &part) const {
  ...
  if (in_partition == patches_by_partition.end()) return {};
...
MergeTreeData::MergeTreeData(const StorageID &table_id_, const StorageInMemoryMetadata &metadata_, ContextMutablePtr context_,
                             const String &date_column_name, const MergingParams &merging_params_,
                             std::unique_ptr<MergeTreeSettings> storage_settings_, bool require_part_metadata_, LoadingStrictnessLevel mode,
                             BrokenPartCallback broken_part_callback_)
    : WithMutableContext(context_->getGlobalContext()),
      IStorage(table_id_),
      format_version(date_column_name.empty() ? MERGE_TREE_DATA_MIN_FORMAT_VERSION_WITH_CUSTOM_PARTITIONING
                                              : MERGE_TREE_DATA_OLD_FORMAT_VERSION),
  ...
  if (!date_column_name.empty()) {
    try {
      ...
      if (minmax_idx_date_column_pos == -1) throw Exception(ErrorCodes::BAD_TYPE_OF_FIELD, "Could not find Date column");
    } catch (Exception &e) {
  ...
  if (metadata_.sampling_key.definition_ast != nullptr) {
    ...
    checkSampleExpression(metadata_,
                          !sanity_checks || (*settings)[MergeTreeSetting::compatibility_allow_sampling_expression_not_in_primary_key],
...
VirtualColumnsDescription MergeTreeData::createVirtuals(const StorageInMemoryMetadata &metadata) {
  ...
  desc.addEphemeral("_part_uuid", std::make_shared<DataTypeUUID>(),
                    "Unique part identifier (if enabled MergeTree setting assign_part_uuids)");
  ...
  desc.addEphemeral(PartDataVersionColumn::name, std::make_shared<DataTypeUInt64>(),
                    "Data version of part (either min block number or mutation version)");
...
VirtualColumnsDescription MergeTreeData::createProjectionVirtuals(const StorageInMemoryMetadata &metadata) {
  ...
  desc.addEphemeral("_part_uuid", std::make_shared<DataTypeUUID>(),
                    "Unique part identifier (if enabled MergeTree setting assign_part_uuids)");
  desc.addEphemeral("_partition_id", std::make_shared<DataTypeLowCardinality>(std::make_shared<DataTypeString>()), "Name of partition");
  desc.addEphemeral("_part_data_version", std::make_shared<DataTypeUInt64>(),
                    "Data version of part (either min block number or mutation version)");
  desc.addEphemeral("_disk_name", std::make_shared<DataTypeLowCardinality>(std::make_shared<DataTypeString>()), "Disk name");
...
std::map<std::string, DiskPtr> MergeTreeData::getDistinctDisksForParts(const DataPartsVector &parts_list) const {
  ...
  for (const auto &part : parts_list) {
    ...
    if (disk == nullptr) throw Exception(ErrorCodes::BAD_ARGUMENTS, "Couldn't find disk '{}' for part {}", disk_name, part->name);
...
ConditionSelectivityEstimatorPtr MergeTreeData::getConditionSelectivityEstimator(const RangesInDataParts &parts,
                                                                                 const Names &required_columns,
                                                                                 ContextPtr local_context) const {
  if (!local_context->getSettingsRef()[Setting::use_statistics]) return nullptr;

  if (parts.empty()) return {};

  {
    std::lock_guard<std::mutex> lock(stats_mutex);
    if (local_context->getSettingsRef()[Setting::use_statistics_cache] && cached_estimator) return cached_estimator;
  }
  ...
  for (const auto &part : parts) {
    try {
      ...
      for (const auto &[column_name, stat] : stats) estimator_builder.addStatistics(column_name, stat);
    } catch (...) {
...
bool MergeTreeData::supportsFinal() const {
  return merging_params.mode == MergingParams::Collapsing || merging_params.mode == MergingParams::Summing ||
         ...
         merging_params.mode == MergingParams::Coalescing || merging_params.mode == MergingParams::Graphite ||
         merging_params.mode == MergingParams::VersionedCollapsing;
}

static void checkKeyExpression(const ExpressionActions &expr, const Block &sample_block, const String &key_name, bool allow_nullable_key) {
  if (expr.hasArrayJoin()) throw Exception(ErrorCodes::ILLEGAL_COLUMN, "{} key cannot contain array joins", key_name);
...
void MergeTreeData::checkProperties(const StorageInMemoryMetadata &new_metadata, const StorageInMemoryMetadata &old_metadata, bool attach,
                                    bool allow_empty_sorting_key, bool allow_reverse_sorting_key, bool allow_nullable_key_,
                                    ContextPtr local_context) const {
  ...
  if (primary_key_size > sorting_key_size)
    throw Exception(ErrorCodes::BAD_ARGUMENTS,
                    "Primary key must be a prefix of the sorting key, but its length: "
  ...
  if (local_context) allow_suspicious_indices = local_context->getSettingsRef()[Setting::allow_suspicious_indices];
    ...
    if (i < primary_key_size) {
      ...
      if (pk_column != sorting_key_column)
        throw Exception(ErrorCodes::BAD_ARGUMENTS,
                        ...
                        "but the column in the position {} is {}",
                        i, sorting_key_column + ", not " + pk_column);
    }
  }
    ...
    for (const String &col : used_columns) {
      ...
      if (new_metadata.columns.getDefaults().contains(col))
        throw Exception(ErrorCodes::BAD_ARGUMENTS,
                        ...
                        "it to the sorting key is forbidden",
                        backQuoteIfNeed(col));
    }
  }
    ...
    for (const auto &index : new_metadata.secondary_indices) {
      try {
        if (!allow_suspicious_indices && !attach) {
          ...
          if (index_expression_ptr) checkSuspiciousIndices(index_expression_ptr);
        }
      ...
      if (index.type == TEXT_INDEX_NAME) {
        ...
        if (columns_with_text_indexes.contains(column))
          throw Exception(ErrorCodes::BAD_ARGUMENTS, "Column {} must not have more than one text index", backQuote(index.column_names[0]));
  ...
  ///     CREATE TABLE tab (`id` Int32, `vec` Array(Float32), INDEX idx vec TYPE  vector_similarity('hnsw', 'L2Distance') GRANULARITY
  ///     100000000) ENGINE = MergeTree ORDER BY id SETTINGS index_granularity_bytes = 0; INSERT INTO tab SELECT number, [toFloat32(number),
  ///     0.] FROM numbers(10000); WITH [1., 0.] AS reference_vec SELECT id, L2Distance(vec, reference_vec) FROM tab PREWHERE
  ///     toLowCardinality(10) ORDER BY L2Distance(vec, reference_vec) ASC LIMIT 100;
  ...
  if (!new_metadata.projections.empty()) {
    ...
    for (const auto &projection : new_metadata.projections) {
      ...
      if (projections_names.size() >= (*settings)[MergeTreeSetting::max_projections])
        throw Exception(ErrorCodes::LIMIT_EXCEEDED, "Maximum limit of {} projection(s) exceeded",
                        (*settings)[MergeTreeSetting::max_projections].value);
      ...
      checkProperties(*projection.metadata, *projection.metadata, attach, is_aggregate, allow_reverse_key, true /* allow_nullable_key */,
                      local_context);
    ...
    for (const auto &col : new_metadata.columns) {
      if (col.name == "_part_offset" || col.name == "_part_index" || col.name == "_parent_part_offset")
        throw Exception(ErrorCodes::BAD_ARGUMENTS,
                        ...
                        col.name, projection_with_parent_part_offset);
    }
  }

  for (const auto &col : new_metadata.columns) {
    if (!col.statistics.empty()) MergeTreeStatisticsFactory::instance().validate(col.statistics, col.type);
  }
...
void MergeTreeData::setProperties(const StorageInMemoryMetadata &new_metadata, const StorageInMemoryMetadata &old_metadata, bool attach,
  ...
  checkProperties(new_metadata, old_metadata, attach, false, allow_reverse_key, allow_nullable_key, local_context);
...
ExpressionActionsPtr getCombinedIndicesExpression(const KeyDescription &key, const MergeTreeIndices &indices,
                                                  const ColumnsDescription &columns, ContextPtr context) {
  ASTPtr combined_expr_list = key.expression_list_ast->clone();

  for (const auto &index : indices)
    for (const auto &index_expr : index->index.expression_list_ast->children) combined_expr_list->children.push_back(index_expr->clone());

  auto syntax_result = TreeRewriter(context).analyze(combined_expr_list,
                                                     columns.get(GetColumnsOptions(GetColumnsOptions::Kind::AllPhysical).withSubcolumns()));
...
ExpressionActionsPtr MergeTreeData::getMinMaxExpr(const KeyDescription &partition_key, const ExpressionActionsSettings &settings) {
  ...
  if (!partition_key.column_names.empty()) partition_key_columns = partition_key.expression->getRequiredColumnsWithTypes();
...
Names MergeTreeData::getMinMaxColumnsNames(const KeyDescription &partition_key) {
  if (!partition_key.column_names.empty()) return partition_key.expression->getRequiredColumns();
  return {};
}

DataTypes MergeTreeData::getMinMaxColumnsTypes(const KeyDescription &partition_key) {
  if (!partition_key.column_names.empty()) return partition_key.expression->getRequiredColumnsWithTypes().getTypes();
  return {};
}

ExpressionActionsPtr MergeTreeData::getPrimaryKeyAndSkipIndicesExpression(const StorageMetadataPtr &metadata_snapshot,
...
ExpressionActionsPtr MergeTreeData::getSortingKeyAndSkipIndicesExpression(const StorageMetadataPtr &metadata_snapshot,
                                                                          const MergeTreeIndices &indices) const {
...
void MergeTreeData::checkPartitionKeyAndInitMinMax(const KeyDescription &new_partition_key) {
  if (new_partition_key.expression_list_ast->children.empty()) return;
  ...
  if (!has_date_column) {
    for (size_t i = 0; i < minmax_idx_columns_types.size(); ++i) {
      if (isDateTime(minmax_idx_columns_types[i]) || isDateTime64(minmax_idx_columns_types[i])) {
...
void MergeTreeData::checkTTLExpressions(const StorageInMemoryMetadata &new_metadata, const StorageInMemoryMetadata &old_metadata) const {
  ...
  if (!new_column_ttls.empty()) {
    ...
    if (old_metadata.hasPartitionKey())
      for (const auto &col : old_metadata.getColumnsRequiredForPartitionKey()) columns_ttl_forbidden.insert(col);

    if (old_metadata.hasSortingKey())
      for (const auto &col : old_metadata.getColumnsRequiredForSortingKey()) columns_ttl_forbidden.insert(col);

    // Check new metadata keys (fix for issue #84442)
    if (new_metadata.hasPartitionKey())
      for (const auto &col : new_metadata.getColumnsRequiredForPartitionKey()) columns_ttl_forbidden.insert(col);

    if (new_metadata.hasSortingKey())
      for (const auto &col : new_metadata.getColumnsRequiredForSortingKey()) columns_ttl_forbidden.insert(col);

    for (const auto &[name, ttl_description] : new_column_ttls) {
      if (columns_ttl_forbidden.contains(name)) throw Exception(ErrorCodes::ILLEGAL_COLUMN, "Trying to set TTL for key column {}", name);
    }
  }
    ...
    for (const auto &move_ttl : new_table_ttl.move_ttl) {
      if (!move_ttl.if_exists && !getDestinationForMoveTTL(move_ttl)) {
        if (move_ttl.destination_type == DataDestinationType::DISK)
          throw Exception(ErrorCodes::BAD_TTL_EXPRESSION, "No such disk {} for given storage policy", backQuote(move_ttl.destination_name));

        throw Exception(ErrorCodes::BAD_TTL_EXPRESSION, "No such volume {} for given storage policy", backQuote(move_ttl.destination_name));
      }
    }
  }
}
...
void checkSpecialColumn(const std::string_view column_meta_name, const AlterCommand &command) {
  if (command.type == AlterCommand::DROP_COLUMN) {
    throw Exception(ErrorCodes::ALTER_OF_COLUMN_IS_FORBIDDEN, "Trying to ALTER DROP {} ({}) column", column_meta_name,
                    backQuoteIfNeed(command.column_name));
  } else if (command.type == AlterCommand::RENAME_COLUMN) {
    throw Exception(ErrorCodes::ALTER_OF_COLUMN_IS_FORBIDDEN, "Trying to ALTER RENAME {} ({}) column", column_meta_name,
                    backQuoteIfNeed(command.column_name));
  } else if (command.type == AlterCommand::MODIFY_COLUMN &&
             (command.default_kind == ColumnDefaultKind::Ephemeral || command.default_kind == ColumnDefaultKind::Alias)) {
    throw Exception(ErrorCodes::ALTER_OF_COLUMN_IS_FORBIDDEN, "Cannot modify {} column ({}) to {}", column_meta_name,
                    backQuoteIfNeed(command.column_name), toString(command.default_kind));
  }
};
...
void checkSpecialColumnWithDataType(const std::string_view column_meta_name, const AlterCommand &command) {
  ...
  if (command.type == AlterCommand::MODIFY_COLUMN) {
    if (!command.data_type) {
      throw Exception(ErrorCodes::ALTER_OF_COLUMN_IS_FORBIDDEN, "Trying to modify settings for column {} ({}) ", column_meta_name,
                      command.column_name);
    } else if (!typeid_cast<const TMustHaveDataType *>(command.data_type.get())) {
      throw Exception(ErrorCodes::ALTER_OF_COLUMN_IS_FORBIDDEN, "Cannot alter {} column ({}) to type {}, because it must have type {}",
                      column_meta_name, command.column_name, command.data_type->getName(), TypeName<TMustHaveDataType>);
    }
  }
};
...
void MergeTreeData::MergingParams::check(const MergeTreeSettings &settings, const StorageInMemoryMetadata &metadata) const {
  ...
  if (!is_deleted_column.empty() && mode != MergingParams::Replacing)
    throw Exception(ErrorCodes::LOGICAL_ERROR, "is_deleted column for MergeTree cannot be specified in modes except Replacing.");
      ...
      if (column.name == sign_column) {
        if (!typeid_cast<const DataTypeInt8 *>(column.type.get()))
          throw Exception(ErrorCodes::BAD_TYPE_OF_FIELD,
                          "Sign column ({}) for storage {} must have type Int8. "
                          "Provided column of type {}.",
                          sign_column, storage, column.type->getName());
        miss_column = false;
        break;
      }
    }
  ...
  /// Check that if the is_deleted column is needed, it exists and is of type UInt8. If exist, version column must be defined too but
  /// version checks are not done here.
    ...
    for (const auto &column : columns) {
      if (column.name == is_deleted_column) {
        if (!typeid_cast<const DataTypeUInt8 *>(column.type.get()))
          throw Exception(ErrorCodes::BAD_TYPE_OF_FIELD,
                          "is_deleted column ({}) for storage {} must have type UInt8. Provided column of type {}.", is_deleted_column,
                          storage, column.type->getName());
  ...
  if (mode == MergingParams::Collapsing) check_sign_column(false, "CollapsingMergeTree");
  ...
  if (mode == MergingParams::Summing || mode == MergingParams::Coalescing) {
    ...
    for (const auto &column_to_sum : columns_to_sum) {
      ...
      if (columns.end() == std::find_if(columns.begin(), columns.end(), check_column_to_sum_exists))
        throw Exception(ErrorCodes::NO_SUCH_COLUMN_IN_TABLE, "Column {} listed in columns to sum does not exist in table declaration.",
                        column_to_sum);
    }
    ...
    if (!allow_columns_in_partition_or_order_key && metadata.isPartitionKeyDefined()) {
      ...
      std::set_intersection(columns_to_sum_sorted.begin(), columns_to_sum_sorted.end(), partition_key_columns.begin(),
                            partition_key_columns.end(), std::back_inserter(names_intersection));

      if (!names_intersection.empty())
        throw Exception(ErrorCodes::BAD_ARGUMENTS,
                        "Columns: {} listed both in columns to sum and in partition key. "
                        "That is not allowed.",
                        boost::algorithm::join(names_intersection, ", "));
    }
    ...
    if (!allow_columns_in_partition_or_order_key && metadata.isSortingKeyDefined()) {
      ...
      Names names_intersection;
      std::set_intersection(columns_to_sum_sorted.begin(), columns_to_sum_sorted.end(), sorting_key_columns.begin(),
                            sorting_key_columns.end(), std::back_inserter(names_intersection));

      if (!names_intersection.empty())
        throw Exception(ErrorCodes::BAD_ARGUMENTS,
                        "Columns: {} listed both in columns to sum and in sorting key. "
                        "That is not allowed.",
                        boost::algorithm::join(names_intersection, ", "));
    }
  }
...
const Names MergeTreeData::virtuals_useful_for_filter = {
    "_part", "_partition_id", "_part_uuid", "_partition_value", "_part_data_version", "_disk_name"};
...
Block MergeTreeData::getHeaderWithVirtualsForFilter(const StorageMetadataPtr &metadata) const {
  ...
  for (const auto &name : virtuals_useful_for_filter) {
    ...
    if (auto column = virtuals_desc->tryGet(name)) header.insert({column->type->createColumn(), column->type, name});
  }

  return header;
}

Block MergeTreeData::getBlockWithVirtualsForFilter(const StorageMetadataPtr &metadata, const RangesInDataParts &parts,
                                                   bool ignore_empty) const {
  auto block = getHeaderWithVirtualsForFilter(metadata);

  for (const auto &part : parts) {
    if (ignore_empty && part.data_part->isEmpty()) continue;
...
std::optional<UInt64> MergeTreeData::totalRowsByPartitionPredicateImpl(const ActionsDAG &filter_actions_dag, ContextPtr local_context,
                                                                       const RangesInDataParts &parts) const {
  if (parts.empty()) return 0;

  auto metadata_snapshot = getInMemoryMetadataPtr();
  auto virtual_columns_block = getBlockWithVirtualsForFilter(metadata_snapshot, {parts[0]});

  auto filter_dag = VirtualColumnUtils::splitFilterDagForAllowedInputs(filter_actions_dag.getOutputs().at(0), nullptr, local_context,
                                                                       /*allow_partial_result=*/false);
  if (!filter_dag) return {};

  /// Generate valid expressions for filtering
  bool valid = true;
  for (const auto *input : filter_dag->getInputs())
    if (!virtual_columns_block.has(input->result_name)) valid = false;
  ...
  if (partition_pruner.isUseless() && !valid) return {};

  std::unordered_set<String> part_values;
  if (valid) {
    virtual_columns_block = getBlockWithVirtualsForFilter(metadata_snapshot, parts);
    VirtualColumnUtils::filterBlockWithExpression(VirtualColumnUtils::buildFilterExpression(std::move(*filter_dag), local_context),
                                                  virtual_columns_block);
    part_values = VirtualColumnUtils::extractSingleValueFromBlock<String>(virtual_columns_block, "_part");
    if (part_values.empty()) return 0;
  }
  // At this point, empty `part_values` means all parts.

  size_t res = 0;
  for (const auto &part : parts) {
    if ((part_values.empty() || part_values.contains(part.data_part->name)) && !partition_pruner.canBePruned(*part.data_part))
...
Int64 MergeTreeData::getMaxBlockNumber() const {
  ...
  for (const DataPartPtr &part : data_parts_by_info) max_block_num = std::max({max_block_num, part->info.max_block, part->info.mutation});
...
void MergeTreeData::PartLoadingTree::add(const MergeTreePartInfo &info, const String &name, const DiskPtr &disk) {
  ...
  if (!current_ptr) current_ptr = std::make_shared<Node>(MergeTreePartInfo{}, "", disk);
  ...
  auto has_transaction_metadata = [&](const String &part_name, const DiskPtr &part_disk) -> bool {
    if (relative_data_path.empty()) return false;
  ...
  while (true) {
      ...
      if (!prev_info.isDisjoint(info)) {
        if (has_transaction_metadata(name, disk) || has_transaction_metadata(prev->second->name, prev->second->disk)) {
          ...
          LOG_WARNING(getLogger("MergeTreeData"),
                      "Skipping part {} because it intersects part {}"
                      " and one of them has transaction metadata (likely a rolled-back transaction)",
                      name, prev->second->name);
          return;
        }
        throw Exception(ErrorCodes::LOGICAL_ERROR, "Part {} intersects previous part {}. It is a bug or a result of manual intervention",
                        name, prev->second->name);
      }
    }
    ...
    if (it != current->children.end()) {
      ...
      if (!next_info.isDisjoint(info)) {
        ...
        throw Exception(ErrorCodes::LOGICAL_ERROR, "Part {} intersects next part {}. It is a bug or a result of manual intervention", name,
                        it->second->name);
      }
    }
...
void MergeTreeData::PartLoadingTree::traverse(bool recursive, Func &&func) {
  std::function<void(const NodePtr &)> traverse_impl = [&](const auto &node) {
    func(node);
    if (recursive)
      for (const auto &[_, child] : node->children) traverse_impl(child);
  };

  for (const auto &elem : root_by_partition)
    for (const auto &[_, node] : elem.second->children) traverse_impl(node);
}

MergeTreeData::PartLoadingTree MergeTreeData::PartLoadingTree::build(PartLoadingInfos nodes, const String &relative_data_path_) {
  ...
  for (const auto &[info, name, disk] : nodes) tree.add(info, name, disk);
  return tree;
}

static std::optional<size_t> calculatePartSizeSafe(const MergeTreeData::DataPartPtr &part, const LoggerPtr &log) {
  try {
    return part->getDataPartStorage().calculateTotalSizeOnDisk();
  } catch (...) {
    tryLogCurrentException(
        log, fmt::format("while calculating part size {} on path {}", part->name, part->getDataPartStorage().getRelativePath()));
    return {};
  }
}
...
static void preparePartForRemoval(const MergeTreeMutableDataPartPtr &part) {
  ...
  if (creation_csn != Tx::RolledBackCSN && creation_csn != Tx::PrehistoricCSN && !part->version.isRemovalTIDLocked()) {
    ...
    throw Exception(ErrorCodes::LOGICAL_ERROR,
                    "Data part {} is Outdated and has creation TID {} and CSN {}, "
...
void MergeTreeData::loadUnexpectedDataPart(UnexpectedPartLoadState &state) {
  ...
  try {
    ...
    LOG_DEBUG(log, "Failed to load unexpected data part {} with exception: {}", part_name,
              getExceptionMessage(std::current_exception(), false));
    if (!state.part) {
...
MergeTreeData::LoadPartResult MergeTreeData::loadDataPart(const MergeTreePartInfo &part_info, const String &part_name,
                                                          const DiskPtr &part_disk_ptr, MergeTreeDataPartState to_state,
  ...
  try {
    ...
    if (isRetryableException(std::current_exception())) throw;
  ...
  if (res.part->wasInvolvedInTransaction()) {
    ...
    if (!version.removal_tid.isEmpty() && !version.removal_csn) {
      ...
      if (max) {
        ...
        LOG_TRACE(log, "Will fix version metadata of {} after unclean restart: clearing removal_tid={}", res.part->name,
                  version.removal_tid);
    ...
    if (version_updated) res.part->storeVersionMetadata(/* force */ true);
  ...
  if (res.part->hasLightweightDelete()) has_lightweight_delete_parts.store(true);

  LOG_TRACE(log, "Finished loading {} part {} on disk {}", magic_enum::enum_name(to_state), part_name, part_disk_ptr->getName());
  return res;
}

MergeTreeData::LoadPartResult MergeTreeData::loadDataPartWithRetries(const MergeTreePartInfo &part_info, const String &part_name,
                                                                     const DiskPtr &part_disk_ptr, MergeTreeDataPartState to_state,
                                                                     DB::SharedMutex &part_loading_mutex, size_t initial_backoff_ms,
                                                                     size_t max_backoff_ms, size_t max_tries) {
  auto handle_exception = [&, this](std::exception_ptr exception_ptr, size_t try_no) {
    if (try_no + 1 == max_tries) throw;

    LOG_DEBUG(log, "Failed to load data part {} at try {} with retryable error: {}. Will retry in {} ms", part_name, try_no,
              getExceptionMessage(exception_ptr, false), initial_backoff_ms);
  ...
  while (true) {
    ...
    runner.enqueueAndKeepTrack(
        [this, &part_loading_mutex, &part_select_mutex, &loaded_parts, &parts_to_load, part = std::move(current_part)]() {
          ...
          auto res = loadDataPartWithRetries(part->info, part->name, part->disk, DataPartState::Active, part_loading_mutex,
                                             loading_parts_initial_backoff_ms, loading_parts_max_backoff_ms, loading_parts_max_tries);
          ...
          if (!is_active_part && !part->children.empty()) {
            std::lock_guard lock{part_select_mutex};
            for (const auto &[_, node] : part->children) parts_to_load.push_back(node);
          }
...
bool MergeTreeData::isDiskEligibleForOrphanedPartsSearch(DiskPtr disk) const {
  ...
  bool is_disk_eligible = !disk->isBroken() && !disk->isCustomDisk() &&
                          (mode == SearchOrphanedPartsDisks::ANY || (mode == SearchOrphanedPartsDisks::LOCAL && !disk->isRemote()));
...
void MergeTreeData::loadDataParts(bool skip_sanity_checks, std::optional<std::unordered_set<std::string>> expected_parts) {
  ...
  if (!getStoragePolicy()->isDefaultPolicy() && !skip_sanity_checks && !(*settings)[MergeTreeSetting::disk].changed) {
    ...
    for (const auto &[disk_name, disk] : getContext()->getDisksMap()) {
      ...
      if (delegate && disk->getPath() == delegate->getPath()) defined_disk_names.insert(delegate->getName());
      ...
      if (!is_disk_defined && disk->existsDirectory(relative_data_path)) {
        for (const auto it = disk->iterateDirectory(relative_data_path); it->isValid(); it->next()) {
          ...
          throw Exception(ErrorCodes::UNKNOWN_DISK,
                          ...
                          it->name(), it->path(), disk_name, getStoragePolicy()->getName(), fmt::join(defined_disk_names, ", "),
                          fmt::join(skip_check_disks, ", "));
        }
      }
    }
  }
  ...
  for (size_t i = 0; i < disks.size(); ++i) {
    ...
    if (disk_ptr->isBroken()) continue;
    if (!disk_ptr->isReadOnly()) all_disks_are_readonly = false;
    ...
    runner.enqueueAndKeepTrack(
        [&expected_parts, &unexpected_disk_parts, &disk_parts, this, disk_ptr, component_name = Coordination::getCurrentComponent()]() {
          auto local_component_guard = Coordination::setCurrentComponent(component_name);
          for (auto it = disk_ptr->iterateDirectory(relative_data_path); it->isValid(); it->next()) {
            /// Skip temporary directories, file 'format_version.txt' and directory 'detached'.
            if (startsWith(it->name(), "tmp") || it->name() == MergeTreeData::FORMAT_VERSION_FILE_NAME || it->name() == DETACHED_DIR_NAME)
              continue;
  ...
  for (auto &disk_parts : parts_to_load_by_disk) std::move(disk_parts.begin(), disk_parts.end(), std::back_inserter(parts_to_load));
  ...
  loading_tree.traverse(/*recursive=*/false, [&](const auto &node) { active_parts.emplace_back(node); });
  ...
  if (num_parts > 0) {
    ...
    for (const auto &res : loaded_parts) {
      if (res.is_broken) {
        ...
        if (unexpected) {
          LOG_DEBUG(log,
                    "loadDataParts: Part {} is broken, but it's not expected to be in parts set, "
                    " will not count it as suspicious broken part",
                    res.part->name);
  ...
  if (suspicious_broken_unexpected_parts != 0)
    LOG_WARNING(log, "Found suspicious broken unexpected parts {} with total rows count {}", suspicious_broken_unexpected_parts,
  ...
  for (const auto &[info, name, disk] : unexpected_parts_to_load) {
    ...
    unexpected_unloaded_data_parts.push_back(
        {std::make_shared<PartLoadingTree::Node>(info, name, disk), uncovered, /*is_broken*/ false, /*part*/ nullptr});
  }
  ...
  if (!unexpected_unloaded_data_parts.empty()) {
    ...
    unexpected_data_parts_loading_task = getContext()->getSchedulePool().createTask(
        getStorageID(), "MergeTreeData::loadUnexpectedDataParts", [this, component_name = Coordination::getCurrentComponent()] {
          auto local_component_guard = Coordination::setCurrentComponent(component_name);
  ...
  loading_tree.traverse(/*recursive=*/true, [&](const auto &node) {
    if (!node->is_loaded) unloaded_parts.push_back(node);
  });
  ...
  if (!unloaded_parts.empty() && !all_disks_are_readonly && !is_table_readonly) {
    ...
    outdated_data_parts_loading_task = getContext()->getSchedulePool().createTask(
        getStorageID(), "MergeTreeData::loadOutdatedDataParts", [this, component_name = Coordination::getCurrentComponent()] {
          auto local_component_guard = Coordination::setCurrentComponent(component_name);
  ...
  if (all_disks_are_readonly && refresh_parts_interval && !refresh_parts_task) {
    refresh_parts_task =
        getContext()->getSchedulePool().createTask(getStorageID(), "MergeTreeData::refreshDataParts",
...
void MergeTreeData::startStatisticsCache() {
  ...
  if (refresh_statistics_seconds) {
    ...
    refresh_stats_task =
        getContext()->getSchedulePool().createTask(getStorageID(), "MergeTreeData::refreshStatistics",
...
void MergeTreeData::refreshDataParts(UInt64 interval_milliseconds) try {
  for (auto &disk : getStoragePolicy()->getDisks()) disk->refresh(interval_milliseconds);
...

  {
    ...
    loading_tree.traverse(/*recursive=*/false, [&, this](const auto &node) {
      if (auto it = data_parts_by_info.find(node->info); it == data_parts_by_info.end()) parts_to_add.emplace_back(node);
    });
  }
  ...
  for (const auto &my_part : parts_to_add) {
    auto res = loadDataPartWithRetries(my_part->info, my_part->name, my_part->disk, DataPartState::PreActive, data_parts_mutex,
                                       loading_parts_initial_backoff_ms, loading_parts_max_backoff_ms, loading_parts_max_tries);
  ...
  watch.stop();
  LOG_DEBUG(log, "Refreshing data parts (added {} items, removed {} items) took {} seconds", parts_to_add.size(), old_parts.size(),
...
void MergeTreeData::refreshStatistics(UInt64 interval_seconds) try {
  ...
  if (cached_estimator) {
    if (!cached_estimator->isStale(data_parts)) {
      ...
      if (interval_seconds) refresh_stats_task->scheduleAfter(interval_seconds * 1000);
      return;
    }
  }
...
void MergeTreeData::loadUnexpectedDataParts() try {
  {
    std::lock_guard lock(unexpected_data_parts_mutex);
    ...
    LOG_DEBUG(log, "Loading {} unexpected data parts", unexpected_data_parts.size());
  }
  ...
  for (auto &load_state : unexpected_data_parts) {
    ...
    runner.enqueueAndKeepTrack(
        [this, &load_state, replicated, component_name = Coordination::getCurrentComponent()]() {
          auto local_component_guard = Coordination::setCurrentComponent(component_name);
          loadUnexpectedDataPart(load_state);

          chassert(load_state.part);
          if (load_state.is_broken)
            load_state.part->renameToDetached("broken-on-start",
                                              /*ignore_error=*/replicated);  /// detached parts must not have '_' in prefixes
        },
        Priority{});
  }
  runner.waitForAllToFinishAndRethrowFirstError();
  ...
  LOG_FATAL(log,
            "Loading of unexpected parts failed. "
...
void MergeTreeData::loadOutdatedDataParts(bool is_async) try {
  {
    std::lock_guard lock(outdated_data_parts_mutex);
    ...
    LOG_DEBUG(log, "Loading {} outdated data parts {}", outdated_unloaded_data_parts.size(), is_async ? "asynchronously" : "synchronously");
  }

  std::this_thread::sleep_for(
      std::chrono::milliseconds(static_cast<size_t>((*getSettings())[MergeTreeSetting::sleep_before_loading_outdated_parts_ms])));
  ThreadFuzzer::maybeInjectSleep();
  ...
  while (true) {
...

    {
      ...
      if (is_async && outdated_data_parts_loading_canceled) {
        ...
        LOG_DEBUG(log,
                  ...
                  "Loaded {} parts, {} left unloaded",
                  num_loaded_parts.load(), outdated_unloaded_data_parts.size());
        return;
      }

      if (outdated_unloaded_data_parts.empty()) break;
    ...
    runner.enqueueAndKeepTrack(
        [this, my_part = part, &num_loaded_parts, replicated]() {
          auto blocker_for_runner_thread = CannotAllocateThreadFaultInjector::blockFaultInjections();

          auto res = loadDataPartWithRetries(my_part->info, my_part->name, my_part->disk, DataPartState::Outdated, data_parts_mutex,
                                             loading_parts_initial_backoff_ms, loading_parts_max_backoff_ms, loading_parts_max_tries);
  ...
  LOG_DEBUG(log, "Loaded {} outdated data parts {}", num_loaded_parts.load(), is_async ? "asynchronously" : "synchronously");
  ...
  LOG_FATAL(log,
            "Loading of outdated parts failed. "
            "Will terminate to avoid undefined behaviour due to inconsistent set of parts. "
            "Exception: {}",
            getCurrentExceptionMessage(true));
  std::terminate();
...
void MergeTreeData::waitForOutdatedPartsToBeLoaded() const TSA_NO_THREAD_SAFETY_ANALYSIS {
  ...
  /// If waiting is not required, do NOT log and do NOT enable/disable turbo mode to make `waitForOutdatedPartsToBeLoaded` a lightweight
  /// check
  {
    ...
    if (outdated_data_parts_loading_finished) return;
  }
  ...
  if (outdated_data_parts_loading_canceled) throw Exception(ErrorCodes::NOT_INITIALIZED, "Loading of outdated data parts was canceled");
...
void MergeTreeData::waitForUnexpectedPartsToBeLoaded() const TSA_NO_THREAD_SAFETY_ANALYSIS {
  ...
  /// If waiting is not required, do NOT log and do NOT enable/disable turbo mode to make `waitForUnexpectedPartsToBeLoaded` a lightweight
  /// check
  {
    ...
    if (unexpected_data_parts_loading_finished) return;
  }
  ...
  if (unexpected_data_parts_loading_canceled) throw Exception(ErrorCodes::NOT_INITIALIZED, "Loading of unexpected data parts was canceled");
...
void MergeTreeData::startOutdatedAndUnexpectedDataPartsLoadingTask() {
  if (outdated_data_parts_loading_task) outdated_data_parts_loading_task->activateAndSchedule();
  if (unexpected_data_parts_loading_task) unexpected_data_parts_loading_task->activateAndSchedule();
}
...
PrimaryIndexCachePtr MergeTreeData::getPrimaryIndexCache() const {
  ...
  if (!use_primary_key_cache || !primary_key_lazy_load) return nullptr;
...
MergeTreeData::CachesToPrewarm MergeTreeData::getCachesToPrewarm(size_t part_uncompressed_bytes) const {
  ...
  if (part_uncompressed_bytes && part_uncompressed_bytes < min_bytes_to_prewarm) return result;

  if ((*settings)[MergeTreeSetting::prewarm_primary_key_cache]) result.primary_index_cache = getPrimaryIndexCache();
...
void MergeTreeData::prewarmCaches(ThreadPool &pool, const CachesToPrewarm &caches) {
  ...
  std::sort(data_parts.begin(), data_parts.end(), [&to_tuple](const auto &lhs, const auto &rhs) { return to_tuple(lhs) > to_tuple(rhs); });
  ...
  for (const auto &part : data_parts) {
    ...
    if (caches.primary_index_cache && !part->isIndexLoaded() && enough_space(caches.primary_index_cache, index_ratio_to_prewarm)) {
      ...
      runner.enqueueAndKeepTrack(
          [&enough_space, &caches, index_ratio_to_prewarm, &part, current_component = Coordination::getCurrentComponent()] {
            ...
            if (enough_space(caches.primary_index_cache, index_ratio_to_prewarm)) part->loadIndexToCache(*caches.primary_index_cache);
          });
    ...
    if (caches.index_mark_cache && enough_space(caches.index_mark_cache, index_mark_ratio_to_prewarm)) {
      runner.enqueueAndKeepTrack(
          [&enough_space, &caches, index_mark_ratio_to_prewarm, &part, current_component = Coordination::getCurrentComponent()] {
            auto component_guard = Coordination::setCurrentComponent(current_component);
...
static bool isOldPartDirectory(const DiskPtr &disk, const String &directory_path, time_t threshold) {
  if (!disk->existsDirectory(directory_path) || disk->getLastModified(directory_path).epochTime() > threshold) return false;

  for (auto it = disk->iterateDirectory(directory_path); it->isValid(); it->next())
    if (disk->getLastModified(it->path()).epochTime() > threshold) return false;
...
size_t MergeTreeData::clearOldTemporaryDirectories(const String &root_path, size_t custom_directories_lifetime_seconds,
                                                   const NameSet &valid_prefixes) {
  ...
  for (const auto &disk : getDisks()) {
    ...
    if (!disk->existsDirectory(root_path)) continue;
    ...
    for (auto it = disk->iterateDirectory(root_path); it->isValid(); it->next()) {
      ...
      if (!start_with_valid_prefix) continue;
      ...
      try {
        if (isOldPartDirectory(disk, it->path(), deadline)) {
          ...
          if (temporary_parts.contains(basename)) {
            ...
            LOG_INFO(LogFrequencyLimiter(log.load(), 10),
                     "{} is in use (by merge/mutation/INSERT) (consider increasing temporary_directories_lifetime setting)", full_path);
            continue;
          }
          if (!disk->existsDirectory(it->path())) {
            /// We should recheck that the dir exists, otherwise we can get "No such file or directory"
            /// due to a race condition with "Renaming temporary part" (temporary part holder could be already released, so the check above
            /// is not enough)
            LOG_WARNING(log,
                        "Temporary directory {} suddenly disappeared while iterating, assuming it was concurrently renamed to persistent",
                        it->path());
            continue;
          }
...
MergeTreeData::MutableDataPartPtr MergeTreeData::asMutableDeletingPart(const DataPartPtr &part) {
  ...
  if (state != DataPartState::Deleting && state != DataPartState::DeleteOnDestroy)
    throw Exception(ErrorCodes::LOGICAL_ERROR, "Cannot remove part {}, because it has state: {}", part->name, magic_enum::enum_name(state));
...
MergeTreeData::DataPartsVector MergeTreeData::grabOldParts(bool force) {
  ...
  bool need_remove_parts_in_order =
      supportsReplication() && !isSharedStorage() && (*getSettings())[MergeTreeSetting::allow_remote_fs_zero_copy_replication];
  ...
  auto has_skipped_mutation_parent = [&skipped_parts, need_remove_parts_in_order](const DataPartPtr &part) {
    if (!need_remove_parts_in_order) return false;

    for (const auto &part_info : skipped_parts)
      if (part->info.isMutationChildOf(part_info)) return true;
...

  {
    ...
    for (auto it = outdated_parts_range.begin(); it != outdated_parts_range.end(); ++it) {
      ...
      bool reached_removal_time = part_remove_time <= time_now &&
                                  time_now - part_remove_time >= (*getSettings())[MergeTreeSetting::old_parts_lifetime].totalSeconds();
  ...
  if (!res.empty())
    LOG_TRACE(log, "Skipped {} old parts, found {} old parts to remove. Parts: [{}]", skipped_parts.size(), res.size(),
              fmt::join(getPartsNames(res), ", "));
...
void MergeTreeData::removePartsFinally(const MergeTreeData::DataPartsVector &parts, MergeTreeData::DataPartsVector *removed_parts) {
...

  {
    ...
    for (const auto &part : parts) {
      ...
      if (part->getState() == DataPartState::Temporary) continue;

      auto it = data_parts_by_info.find(part->info);
      if (it == data_parts_by_info.end()) throw Exception(ErrorCodes::LOGICAL_ERROR, "Deleting data part {} doesn't exist", part->name);
      ...
      if (removed_parts) removed_parts->push_back(part);
    }
  }
...
void MergeTreeData::clearPartsFromFilesystemImpl(const DataPartsVector &parts, bool throw_on_error, NameSet *parts_failed_to_delete) {
  ...
  auto get_failed_parts = [&part_names_succeed, &parts_failed_to_delete, &parts]() {
    if (part_names_succeed.size() == parts.size()) return;

    if (parts_failed_to_delete) {
      for (const auto &part : parts) {
        if (!part_names_succeed.contains(part->name)) parts_failed_to_delete->insert(part->name);
      }
    }
  };
...
void MergeTreeData::clearPartsFromFilesystemImplMaybeInParallel(const DataPartsVector &parts_to_remove, NameSet *part_names_succeed) {
  if (parts_to_remove.empty()) return;

  const auto settings = getSettings();

  auto remove_single_thread = [this, &parts_to_remove, part_names_succeed]() {
    LOG_DEBUG(log, "Removing {} parts from filesystem (serially): Parts: [{}]", parts_to_remove.size(), fmt::join(parts_to_remove, ", "));
    for (const DataPartPtr &part : parts_to_remove) {
      asMutableDeletingPart(part)->remove();
      if (part_names_succeed) part_names_succeed->insert(part->name);
    }
  };
  ...
  if ((*settings)[MergeTreeSetting::allow_remote_fs_zero_copy_replication] && dynamic_cast<StorageReplicatedMergeTree *>(this) != nullptr) {
    remove_parts_in_order = std::any_of(parts_to_remove.begin(), parts_to_remove.end(), [](const auto &data_part) {
      return data_part->isStoredOnRemoteDiskWithZeroCopySupport() && data_part->info.getMutationVersion() > 0;
    });
  }

  if (!remove_parts_in_order) {
    /// NOTE: Under heavy system load you may get "Cannot schedule a task" from ThreadPool.
    LOG_DEBUG(log, "Removing {} parts from filesystem (concurrently): Parts: [{}]", parts_to_remove.size(),
              fmt::join(parts_to_remove, ", "));
    ...
    for (const DataPartPtr &part : parts_to_remove) {
      ...
      runner.enqueueAndKeepTrack(
          [&part, &part_names_mutex, part_names_succeed, thread_group = CurrentThread::getGroup()] {
            asMutableDeletingPart(part)->remove();
  ...
  /// NOTE: Under heavy system load you may get "Cannot schedule a task" from ThreadPool.
  LOG_DEBUG(log, "Removing {} parts from filesystem (concurrently): Parts: [{}]", parts_to_remove.size(), fmt::join(parts_to_remove, ", "));
  ...
  auto split_into_independent_ranges = [this](const DataPartsVector &parts_to_remove_, size_t split_times) -> RemovalRanges {
    if (parts_to_remove_.empty()) return {};
    ...
    for (size_t i = 0; i < num_ranges; ++i) {
      ...
      for (const auto &part : parts_to_remove_)
        if (range.contains(part->info)) parts_in_range.push_back(part);
      sum_of_ranges += parts_in_range.size();
    }

    if (parts_to_remove_.size() != sum_of_ranges)
      throw Exception(ErrorCodes::LOGICAL_ERROR,
                      "Number of removed parts is not equal to number of parts in independent ranges "
                      "({} != {}), it's a bug",
                      parts_to_remove_.size(), sum_of_ranges);
  ...
  auto schedule_parts_removal = [this, &runner, &part_names_mutex, part_names_succeed](const MergeTreePartInfo &range,
  ...
  if (parts_to_remove.size() != sum_of_ranges + excluded_parts.size())
    throw Exception(ErrorCodes::LOGICAL_ERROR,
                    ...
                    "({} != {} + {}), it's a bug",
                    parts_to_remove.size(), sum_of_ranges, excluded_parts.size());
}
...
size_t MergeTreeData::clearPartsFromFilesystemAndRollbackIfError(const DataPartsVector &parts_to_delete, const String &parts_type) {
  ...
  if (!parts_failed_to_delete.empty()) {
    ...
    if (!rollback_parts.empty()) rollbackDeletingParts(rollback_parts);
...
size_t MergeTreeData::clearEmptyParts() {
  if (!(*getSettings())[MergeTreeSetting::remove_empty_parts]) return 0;
...
size_t MergeTreeData::clearUnusedPatchParts() {
  if (!supportsLightweightUpdate() || !(*getSettings())[MergeTreeSetting::remove_unused_patch_parts]) return 0;

  auto patch_parts = getPatchPartsVectorForInternalUsage();
  if (patch_parts.empty()) return 0;
  ...
  for (const auto &part : regular_parts) {
    ...
    if (!inserted) it->second = std::min(it->second, data_version);
  }
  ...
  for (const auto &patch : patch_parts) {
    ...
    if (it == min_data_version_by_partition.end() || max_data_version <= it->second) patch_names_to_drop.push_back(patch->name);
  }
...
void MergeTreeData::rename(const String &new_table_path, const StorageID &new_table_id) {
  ...
  if (!getStorageID().hasUUID()) getContext()->clearCaches();
...
void MergeTreeData::dropAllData() {
  ...
  if (skipped_parts > 0) LOG_WARNING(log, "dropAllData: {} parts had been skipped", skipped_parts);

  /// Tables in atomic databases have UUID and stored in persistent locations.
  /// No need to clear caches (that are keyed by filesystem path) because collision is not possible.
  if (!getStorageID().hasUUID()) getContext()->clearCaches();

  /// Removing of each data part before recursive removal of directory is to speed-up removal, because there will be less number of
  /// syscalls.
  ...
  try {
    ...
    for (const auto &part : all_parts) {
      if (!part_names_failed.contains(part->name)) data_parts_indexes.erase(part->info);
    }
  ...
  for (const auto &part : detached_parts) {
    ...
    try {
      bool keep_shared =
          removeDetachedPart(part.disk, fs::path(relative_data_path) / DETACHED_DIR_NAME / part.dir_name / "", part.dir_name);
      ...
      if (is_zero_copy && isRetryableException(std::current_exception())) throw;
  ...
  for (const auto &disk : getDisks()) {
    ...
    try {
      if (!isSharedStorage() && !disk->isDirectoryEmpty(relative_data_path) && supportsReplication() &&
          disk->supportZeroCopyReplication() && (*settings_ptr)[MergeTreeSetting::allow_remote_fs_zero_copy_replication]) {
        ...
        LOG_WARNING(log,
                    "dropAllData: Directory {} with table {} not empty (files [{}]) after removing parts. "
...
void MergeTreeData::dropIfEmpty() {
  ...
  if (!data_parts_by_info.empty()) return;
...
bool isSafeForKeyConversion(const IDataType *from, const IDataType *to) {
  if (from->getName() == to->getName()) return true;
  ...
  if (const auto *from_enum8 = typeid_cast<const DataTypeEnum8 *>(from)) {
    if (const auto *to_enum8 = typeid_cast<const DataTypeEnum8 *>(to)) return to_enum8->contains(*from_enum8);
    if (typeid_cast<const DataTypeInt8 *>(to)) return true;  // NOLINT
    return false;
  }

  if (const auto *from_enum16 = typeid_cast<const DataTypeEnum16 *>(from)) {
    if (const auto *to_enum16 = typeid_cast<const DataTypeEnum16 *>(to)) return to_enum16->contains(*from_enum16);
    if (typeid_cast<const DataTypeInt16 *>(to)) return true;  // NOLINT
    return false;
  }

  if (const auto *from_lc = typeid_cast<const DataTypeLowCardinality *>(from)) return from_lc->getDictionaryType()->equals(*to);

  if (const auto *to_lc = typeid_cast<const DataTypeLowCardinality *>(to)) return to_lc->getDictionaryType()->equals(*from);
...
void checkVersionColumnTypesConversion(const IDataType *old_type, const IDataType *new_type, const String column_name) {
  ...
  if (!new_type->canBeUsedAsVersion())
    throw Exception(ErrorCodes::ALTER_OF_COLUMN_IS_FORBIDDEN,
                    ...
                    "of an integer type or of type Date or DateTime",
                    backQuoteIfNeed(column_name), new_type->getName());
  ...
  if ((which_old_type.isInt() && !which_new_type.isInt()) || (which_old_type.isUInt() && !which_new_type.isUInt()) ||
      (which_old_type.isDate() && !which_new_type.isDate()) || (which_old_type.isDate32() && !which_new_type.isDate32()) ||
      (which_old_type.isDateTime() && !which_new_type.isDateTime()) || (which_old_type.isFloat() && !which_new_type.isFloat())) {
    throw Exception(ErrorCodes::ALTER_OF_COLUMN_IS_FORBIDDEN,
                    "Cannot alter version column {} from type {} to type {} "
...
void MergeTreeData::checkAlterIsPossible(const AlterCommands &commands, ContextPtr local_context) const {
  ...
  if (AlterCommands::hasTextIndex(new_metadata) && !settings[Setting::enable_full_text_index])
    throw Exception(ErrorCodes::SUPPORT_IS_DISABLED, "Text index feature is not enabled (turn on setting 'enable_full_text_index')");
  ...
  if (old_metadata.hasPartitionKey()) {
    ...
    for (const auto &action : partition_key_expr->getActions()) {
      for (const auto *child : action.node->children) columns_alter_type_forbidden.insert(child->result_name);
    }
  ...
  if (!merging_params.sign_column.empty()) columns_alter_type_forbidden.insert(merging_params.sign_column);
  ...
  for (const auto &index : old_metadata.getSecondaryIndices()) {
    if (!index.isImplicitlyCreated()) {
      for (const String &col : index.expression->getRequiredColumns()) columns_in_explicit_indices.emplace(col, index.name);
    }
  }

  std::unordered_map<String, String> columns_in_projections;
  for (const auto &projection : old_metadata.getProjections()) {
    for (const String &col : projection.getRequiredColumns()) columns_in_projections.emplace(col, projection.name);
  }

  NameSet dropped_columns;

  std::map<String, const IDataType *> old_types;
  for (const auto &column : old_metadata.getColumns().getAllPhysical()) old_types.emplace(column.name, column.type.get());
  ...
  for (const AlterCommand &command : commands) {
    ...
    if (command.column_name == merging_params.version_column) {
      ...
      if (command.type == AlterCommand::MODIFY_COLUMN) {
        ...
        if (new_type && it != old_types.end()) checkVersionColumnTypesConversion(it->second, new_type, command.column_name);
    ...
    } else if (merging_params.columns_to_sum.end() !=
               std::find(merging_params.columns_to_sum.begin(), merging_params.columns_to_sum.end(), command.column_name)) {
      checkSpecialColumn("columns to sum", command);
    }

    if (command.type == AlterCommand::MODIFY_QUERY)
      throw Exception(ErrorCodes::NOT_IMPLEMENTED, "ALTER MODIFY QUERY is not supported by MergeTree engines family");
    if (command.type == AlterCommand::MODIFY_REFRESH)
      throw Exception(ErrorCodes::NOT_IMPLEMENTED, "ALTER MODIFY REFRESH is not supported by MergeTree engines family");

    if (command.type == AlterCommand::MODIFY_SQL_SECURITY)
      throw Exception(ErrorCodes::NOT_IMPLEMENTED, "ALTER MODIFY SQL SECURITY is not supported by MergeTree engines family");
    ...
    if (command.type == AlterCommand::RENAME_COLUMN) {
      if (columns_in_keys.contains(command.column_name)) {
        throw Exception(ErrorCodes::ALTER_OF_COLUMN_IS_FORBIDDEN, "Trying to ALTER RENAME key {} column which is a part of key expression",
                        backQuoteIfNeed(command.column_name));
      }
      ...
      if (column_to_subcolumns_used_in_keys.contains(command.column_name)) {
        throw Exception(ErrorCodes::ALTER_OF_COLUMN_IS_FORBIDDEN,
                        ...
                        backQuoteIfNeed(command.column_name), boost::join(column_to_subcolumns_used_in_keys[command.column_name], ", "));
      }
      ...
      if (index_mode == AlterColumnSecondaryIndexMode::THROW) {
        if (auto it = columns_in_explicit_indices.find(command.column_name); it != columns_in_explicit_indices.end()) {
          throw Exception(ErrorCodes::ALTER_OF_COLUMN_IS_FORBIDDEN,
                          ...
                          backQuoteIfNeed(command.column_name), it->second);
        }
      }

      if (auto it = columns_in_projections.find(command.column_name); it != columns_in_projections.end()) {
        throw Exception(ErrorCodes::ALTER_OF_COLUMN_IS_FORBIDDEN, "Trying to ALTER RENAME {} column which is a part of projection {}",
                        backQuoteIfNeed(command.column_name), it->second);
      }
    } else if (command.type == AlterCommand::DROP_COLUMN) {
      if (columns_in_keys.contains(command.column_name)) {
        throw Exception(ErrorCodes::ALTER_OF_COLUMN_IS_FORBIDDEN, "Trying to ALTER DROP key {} column which is a part of key expression",
                        backQuoteIfNeed(command.column_name));
      }
      ...
      if (!command.clear) {
        ...
        if (!name_deps) name_deps = getDependentViewsByColumn(local_context);
      ...
      if (old_metadata.columns.has(command.column_name)) {
        ...
        for (const auto &nested_column : nested) dropped_columns.emplace(nested_column.name);
      }
      ...
      for (const auto &reset_setting : command.settings_resets) {
        if (!settings_from_storage->has(reset_setting))
          throw Exception(ErrorCodes::BAD_ARGUMENTS, "Cannot reset setting '{}' because it doesn't exist for MergeTree engines family",
                          reset_setting);
      }
      ...
      if (column_to_subcolumns_used_in_keys.contains(command.column_name)) {
        throw Exception(ErrorCodes::ALTER_OF_COLUMN_IS_FORBIDDEN,
                        "Trying to ALTER column {} whose subcolumns ({}) are part of key expression", backQuoteIfNeed(command.column_name),
        ...
        if (columns_alter_type_check_safe_for_partition.contains(command.column_name)) {
          ...
          if (!isSafeForKeyConversion(it->second, command.data_type.get()))
            throw Exception(ErrorCodes::ALTER_OF_COLUMN_IS_FORBIDDEN,
                            ...
                            "of partition key",
                            backQuoteIfNeed(command.column_name), it->second->getName(), command.data_type->getName());
        }
  ...
  if (old_metadata.hasSettingsChanges()) {
    ...
    for (const auto &changed_setting : new_changes) {
      ...
      if ((!current_value || *current_value != new_value) && MergeTreeSettings::isReadonlySetting(setting_name)) {
  ...
  for (const auto &part : getDataPartsVectorForInternalUsage()) {
    ...
    if (!at_least_one_column_rest) {
      std::string postfix;
      if (dropped_columns.size() > 1) postfix = "s";
      throw Exception(ErrorCodes::BAD_ARGUMENTS,
...
static bool hasTextIndexMaterialization(const MutationCommands &commands, StorageMetadataPtr metadata_snapshot) {
  ...
  if (secondary_indices.empty()) return false;

  for (const auto &command : commands) {
    if (command.type != MutationCommand::MATERIALIZE_INDEX) continue;

    auto it = std::ranges::find_if(secondary_indices, [&](const IndexDescription &index) { return index.name == command.index_name; });
    if (it == secondary_indices.end()) continue;

    if (it->type == "text" || it->type == "vector_similarity") return true;
  }
  return false;
}
  ...
  if (hasTextIndexMaterialization(commands, getInMemoryMetadataPtr())) {
    ...
    for (const auto &part : data_parts) {
      if (part->rows_count > std::numeric_limits<UInt32>::max()) {
        throw Exception(ErrorCodes::SUPPORT_IS_DISABLED,
                        "Cannot materialize text index in part {} with {} rows. Materialization of text index is not supported for parts "
                        "with more than {} rows",
...
MergeTreeDataPartFormat MergeTreeData::choosePartFormat(size_t bytes_uncompressed, size_t rows_count, UInt32 part_level,
  ...
  if (!canUsePolymorphicParts(*settings, out_reason)) return {PartType::Wide, PartStorageType::Full};
  ...
  if (satisfies((*settings)[MergeTreeSetting::min_bytes_for_wide_part], (*settings)[MergeTreeSetting::min_rows_for_wide_part],
                (*settings)[MergeTreeSetting::min_level_for_wide_part]))
    part_type = PartType::Compact;

  return {part_type, PartStorageType::Full};
}

MergeTreeDataPartBuilder MergeTreeData::getDataPartBuilder(const String &name, const VolumePtr &volume, const String &part_dir,
                                                           const ReadSettings &read_settings_) const {
  return MergeTreeDataPartBuilder(*this, name, volume, relative_data_path, part_dir, read_settings_);
}

void MergeTreeData::changeSettings(const ASTPtr &new_settings, AlterLockHolder & /* table_lock_holder */) {
  if (new_settings) {
    ...
    for (const auto &change : new_changes) {
      if (change.name == "disk" || change.name == "storage_policy") {
        ...
        if (new_storage_policy != old_storage_policy) {
          ...
          for (const auto &disk : new_storage_policy->getDisks()) all_diff_disk_names.insert(disk->getName());
          for (const auto &disk : old_storage_policy->getDisks()) all_diff_disk_names.erase(disk->getName());
    ...
    UInt64 has_refresh_statistics_interval_changed =
        (*storage_settings.get())[MergeTreeSetting::refresh_statistics_interval].totalSeconds() !=
        (*copy)[MergeTreeSetting::refresh_statistics_interval].totalSeconds();
    ...
    if (has_escape_index_filenames_changed) {
      ...
      for (auto &idx : new_metadata.secondary_indices) idx.escape_filenames = new_value;
    }

    setInMemoryMetadata(new_metadata);

    if (has_storage_policy_changed) startBackgroundMovesIfNeeded();
...
std::pair<String, bool> MergeTreeData::getNewImplicitStatisticsTypes(const StorageInMemoryMetadata &new_metadata,
                                                                     const MergeTreeSettings &old_settings) {
  Field new_statistics_types = new_metadata.getSettingChange("auto_statistics_types");

  if (new_statistics_types.isNull()) return std::make_pair(old_settings[MergeTreeSetting::auto_statistics_types], false);

  return std::make_pair(new_statistics_types.safeGet<String>(), true);
}

void MergeTreeData::PartsTemporaryRename::addPart(const String &part_name, const String &old_dir, const String &new_dir,
                                                  const DiskPtr &disk) {
...
void MergeTreeData::PartsTemporaryRename::tryRenameAll() {
  ...
  for (size_t i = 0; i < old_and_new_names.size(); ++i) {
    try {
      ...
      if (old_dir.empty() || new_dir.empty()) throw DB::Exception(ErrorCodes::LOGICAL_ERROR, "Empty part name. Most likely it's a bug.");
...
MergeTreeData::PartHierarchy MergeTreeData::getPartHierarchy(const MergeTreePartInfo &part_info, DataPartState state,
  ...
  if (begin != committed_parts_range.end() && (*begin)->info == part_info) ++begin;
...
MergeTreeData::DataPartsVector MergeTreeData::getCoveredOutdatedParts(const DataPartPtr &part, DataPartsLock &data_parts_lock) const {
...
MergeTreeData::DataPartsVector MergeTreeData::getActivePartsToReplace(const MergeTreePartInfo &new_part_info, const String &new_part_name,
  ...
  if (!hierarchy.intersected_parts.empty())
    throw Exception(ErrorCodes::LOGICAL_ERROR, "Part {} intersects part {}. It is a bug.", new_part_name,
                    hierarchy.intersected_parts.back()->getNameWithState());

  if (hierarchy.duplicate_part)
    throw Exception(ErrorCodes::LOGICAL_ERROR, "Unexpected duplicate part {}. It is a bug.", hierarchy.duplicate_part->getNameWithState());

  if (!hierarchy.covering_parts.empty()) out_covering_part = std::move(hierarchy.covering_parts.back());
  ...
  if (DataPartPtr existing_part_in_partition = getAnyPartInPartition(part->info.getPartitionId(), lock)) {
    if (part->partition.value != existing_part_in_partition->partition.value)
      throw Exception(ErrorCodes::CORRUPTED_DATA,
                      ...
                      "Existing part: {}, newly added part: {}",
                      existing_part_in_partition->name, part->name);
  }
}
...
void MergeTreeData::checkPartDuplicate(MutableDataPartPtr &part, Transaction &transaction, const DataPartsAnyLock & /*lock*/) const {
  ...
  if (it_duplicate != data_parts_by_info.end()) {
    ...
    if (transaction.txn) throw Exception(ErrorCodes::SERIALIZATION_ERROR, "Part {} already exists", (*it_duplicate)->getNameWithState());

    throw Exception(ErrorCodes::DUPLICATE_DATA_PART, "Part {} already exists", (*it_duplicate)->getNameWithState());
  }
}

void MergeTreeData::preparePartForCommit(MutableDataPartPtr &part, Transaction &out_transaction, DataPartsLock &, bool need_rename,
  ...
  if (need_rename && !rename_in_transaction) part->renameTo(part->name, true);
...
bool MergeTreeData::addTempPart(MutableDataPartPtr &part, Transaction &out_transaction, DataPartsLock &lock,
  ...
  if (part->hasLightweightDelete()) has_lightweight_delete_parts.store(true);
  ...
  if (out_covered_parts) {
    ...
    for (DataPartPtr &covered_part : covered_parts) out_covered_parts->emplace_back(std::move(covered_part));
  }

  return true;
}

bool MergeTreeData::renameTempPartAndReplaceImpl(MutableDataPartPtr &part, Transaction &out_transaction, DataPartsLock &lock,
                                                 DataPartsVector *out_covered_parts, bool rename_in_transaction) {
  LOG_TRACE(log, "Renaming temporary part {} to {} with tid {}.", part->getDataPartStorage().getPartDirectory(), part->name,
            out_transaction.getTID());
  ...
  if (!hierarchy.intersected_parts.empty()) {
    ...
    // So such operation may attempt to delete already outdated part. In this case, this outdated part is most likely covered by the other
    // part and intersection may occur. Part mayght be outdated due to merge|mutation|update|optimization operations.
    if (part->isEmpty() || (hierarchy.intersected_parts.size() == 1 && hierarchy.intersected_parts.back()->isEmpty())) {
      throw Exception(ErrorCodes::SERIALIZATION_ERROR,
                      "Part {} intersects part {}. One of them is empty part. "
    ...
    throw Exception(ErrorCodes::LOGICAL_ERROR, "Part {} intersects part {}. There are {} intersected parts. It is a bug.", part->name,
                    hierarchy.intersected_parts.back()->getNameWithState(), hierarchy.intersected_parts.size());
  }
...
bool MergeTreeData::renameTempPartAndReplaceUnlocked(MutableDataPartPtr &part, Transaction &out_transaction, DataPartsLock &lock,
                                                     bool rename_in_transaction) {
  return renameTempPartAndReplaceImpl(part, out_transaction, lock, /*out_covered_parts=*/nullptr, rename_in_transaction);
}

MergeTreeData::DataPartsVector MergeTreeData::renameTempPartAndReplace(MutableDataPartPtr &part, Transaction &out_transaction,
                                                                       bool rename_in_transaction) {
  auto lock = lockParts();
  return renameTempPartAndReplaceUnlocked(part, lock, out_transaction, rename_in_transaction);
}

MergeTreeData::DataPartsVector MergeTreeData::renameTempPartAndReplaceUnlocked(MutableDataPartPtr &part, DataPartsLock &lock,
                                                                               Transaction &out_transaction, bool rename_in_transaction) {
  DataPartsVector covered_parts;
  renameTempPartAndReplaceImpl(part, out_transaction, lock, &covered_parts, rename_in_transaction);
  return covered_parts;
}

bool MergeTreeData::renameTempPartAndAdd(MutableDataPartPtr &part, Transaction &out_transaction, DataPartsLock &lock,
                                         bool rename_in_transaction) {
  DataPartsVector covered_parts;

  if (!renameTempPartAndReplaceImpl(part, out_transaction, lock, &covered_parts, rename_in_transaction)) return false;

  if (!covered_parts.empty())
    throw Exception(ErrorCodes::LOGICAL_ERROR, "Added part {} covers {} existing part(s) (including {})", part->name, covered_parts.size(),
                    covered_parts[0]->name);

  return true;
}

void MergeTreeData::removePartsFromWorkingSet(MergeTreeTransaction *txn, const MergeTreeData::DataPartsVector &remove,
                                              bool clear_without_timeout, DataPartsLock &acquired_lock) {
  if (txn) transactions_enabled.store(true);

  auto remove_time = clear_without_timeout ? 0 : time(nullptr);

  for (const DataPartPtr &part : remove) {
    if (part->version.creation_csn != Tx::RolledBackCSN) MergeTreeTransaction::removeOldPart(shared_from_this(), part, txn);
    ...
    if (part->getState() != MergeTreeDataPartState::Outdated) modifyPartState(part, MergeTreeDataPartState::Outdated, acquired_lock);
  }
}
...
void MergeTreeData::removePartsFromWorkingSetImmediatelyAndSetTemporaryState(const DataPartsVector &remove, DataPartsLock &lock) {
  for (const auto &part : remove) {
    ...
    /// Erase immediately
    LOG_TEST(log, "removePartsFromWorkingSetImmediatelyAndSetTemporaryState: removing {} from data_parts_indexes",
             part->getNameWithState());
    data_parts_indexes.erase(it_part);
  }
}

void MergeTreeData::removePartsFromWorkingSet(MergeTreeTransaction *txn, const DataPartsVector &remove, bool clear_without_timeout) {
  auto lock = lockParts();
...
DataPartsVector MergeTreeData::grabActivePartsToRemoveForDropRange(MergeTreeTransaction *txn, const MergeTreePartInfo &drop_range,
                                                                   const DataPartsAnyLock &lock) {
  ...
  for (const DataPartPtr &part : partition_range) {
    ...
    if (is_drop_part && (part->info.min_block != drop_range.min_block || part->info.max_block != drop_range.max_block)) {
      ...
      if (is_covered_by_min_max_block) {
        LOG_TRACE(LogFrequencyLimiter(log.load(), 1), "Skipping drop range for part {} because covering part {} already exists",
                  drop_range.getPartNameForLogs(), part->name);
        return {};
      }
    }
    ...
    if (part->info.min_block < drop_range.min_block) {
      if (drop_range.min_block <= part->info.max_block) {
        ...
        throw Exception(ErrorCodes::LOGICAL_ERROR, "Unexpected merged part {} intersecting drop range {}", part->name,
                        drop_range.getPartNameForLogs());
      }
    ...
    /// Stop on new parts
    if (part->info.min_block > drop_range.max_block) break;
...
MergeTreeData::PartsToRemoveFromZooKeeper MergeTreeData::removePartsInRangeFromWorkingSetAndGetPartsToRemoveFromZooKeeper(
  ...
  auto inactive_parts_to_remove_immediately =
      getDataPartsVectorInPartitionForInternalUsage({DataPartState::Outdated, DataPartState::Deleting}, drop_range.getPartitionId(), lock);
  ...
  for (auto &part : parts_to_remove) parts_to_remove_from_zookeeper.emplace_back(std::move(part));

  for (auto &part : inactive_parts_to_remove_immediately) {
    if (!drop_range.contains(part->info)) continue;
...
void MergeTreeData::restoreAndActivatePart(const DataPartPtr &part) {
  auto lock = lockParts();
  if (part->getState() == DataPartState::Active) return;
...
void MergeTreeData::outdateUnexpectedPartAndCloneToDetached(const DataPartPtr &part_to_detach) {
  LOG_INFO(log, "Cloning part {} to unexpected_{} and making it obsolete.", part_to_detach->getDataPartStorage().getPartDirectory(),
           part_to_detach->name);
  ...
  if (part_to_detach->getState() == DataPartState::Active) removePartsFromWorkingSet(NO_TRANSACTION_RAW, {part_to_detach}, true, lock);
}

void MergeTreeData::forcefullyMovePartToDetachedAndRemoveFromMemory(const MergeTreeData::DataPartPtr &part_to_detach,
                                                                    const String &prefix) {
  if (prefix.empty())
    LOG_INFO(log, "Renaming {} to {} and forgetting it.", part_to_detach->getDataPartStorage().getPartDirectory(), part_to_detach->name);
  else
    LOG_INFO(log, "Renaming {} to {}_{} and forgetting it.", part_to_detach->getDataPartStorage().getPartDirectory(), prefix,
             part_to_detach->name);
    ...
    if (part->getState() != DataPartState::Temporary) {
      ...
      if (!((*it)->getState() == DataPartState::Outdated && isSharedPtrUnique(*it))) {
        if ((*it)->getState() != DataPartState::Outdated)
          LOG_WARNING(log,
                      "Cannot immediately remove part {} because it's not in Outdated state "
                      "usage counter {}",
                      part_name_with_state, it->use_count());

        if (!isSharedPtrUnique(*it))
          LOG_WARNING(log,
                      "Cannot immediately remove part {} because someone using it right now "
                      "usage counter {}",
                      part_name_with_state, it->use_count());
        return false;
      }
...
size_t MergeTreeData::getTotalActiveSizeInBytes() const { return total_active_size_bytes.load(); }

size_t MergeTreeData::getTotalActiveSizeInRows() const { return total_active_size_rows.load(); }

size_t MergeTreeData::getTotalUncompressedBytesInPatches() const { return total_uncompressed_bytes_in_patches.load(); }

size_t MergeTreeData::getActivePartsCount() const { return total_active_size_parts.load(); }

size_t MergeTreeData::getOutdatedPartsCount() const { return total_outdated_parts_count.load(); }
...
size_t MergeTreeData::getNumberOfOutdatedPartsWithExpiredRemovalTime() const {
  ...
  for (const auto &part : outdated_parts_range) {
    auto part_remove_time = part->remove_time.load(std::memory_order_relaxed);
    if (part_remove_time <= time_now &&
        time_now - part_remove_time >= (*getSettings())[MergeTreeSetting::old_parts_lifetime].totalSeconds() && isSharedPtrUnique(part))
      ++res;
  }
...
std::optional<Int64> MergeTreeData::getMinPartDataVersion() const {
  ...
  for (const auto &part : getDataPartsStateRange(DataPartState::Active)) {
    if (!result || *result > part->info.getDataVersion()) result = part->info.getDataVersion();
  }
...
void MergeTreeData::delayInsertOrThrowIfNeeded(Poco::Event *until, const ContextPtr &query_context, bool allow_throw) const {
  ...
  if (allow_throw && parts_count_in_total >= (*settings)[MergeTreeSetting::max_parts_in_total]) {
    ...
    throw Exception(ErrorCodes::TOO_MANY_PARTS,
                    "Too many parts ({}) in all partitions in total in table '{}'. This indicates wrong choice of partition key. The "
  ...
  size_t outdated_parts_over_threshold = 0;
  {
    ...
    if ((*settings)[MergeTreeSetting::inactive_parts_to_throw_insert] > 0 ||
        (*settings)[MergeTreeSetting::inactive_parts_to_delay_insert] > 0)
      outdated_parts_count_in_partition = getMaxOutdatedPartsCountForPartition();

    if (allow_throw && (*settings)[MergeTreeSetting::inactive_parts_to_throw_insert] > 0 &&
        outdated_parts_count_in_partition >= (*settings)[MergeTreeSetting::inactive_parts_to_throw_insert]) {
      ProfileEvents::increment(ProfileEvents::RejectedInserts);
      throw Exception(ErrorCodes::TOO_MANY_PARTS,
    ...
    if ((*settings)[MergeTreeSetting::inactive_parts_to_delay_insert] > 0 &&
        outdated_parts_count_in_partition >= (*settings)[MergeTreeSetting::inactive_parts_to_delay_insert])
  ...
  const auto active_parts_to_delay_insert = query_settings[Setting::parts_to_delay_insert]
                                                ? query_settings[Setting::parts_to_delay_insert]
                                                : (*settings)[MergeTreeSetting::parts_to_delay_insert];
  const auto active_parts_to_throw_insert = query_settings[Setting::parts_to_throw_insert]
                                                ? query_settings[Setting::parts_to_throw_insert]
                                                : (*settings)[MergeTreeSetting::parts_to_throw_insert];
  size_t active_parts_over_threshold = 0;

  {
    bool parts_are_large_enough_in_average = (*settings)[MergeTreeSetting::max_avg_part_size_for_too_many_parts] &&
                                             average_part_size > (*settings)[MergeTreeSetting::max_avg_part_size_for_too_many_parts];
    ...
    if (allow_throw && parts_count_in_partition >= active_parts_to_throw_insert && !parts_are_large_enough_in_average) {
      ...
      throw Exception(ErrorCodes::TOO_MANY_PARTS,
                      ...
                      parts_count_in_partition, ReadableSize(average_part_size), getLogName());
    }
    if (active_parts_to_delay_insert > 0 && parts_count_in_partition >= active_parts_to_delay_insert && !parts_are_large_enough_in_average)
  ...
  /// no need for delay
  if (!active_parts_over_threshold && !outdated_parts_over_threshold) return;
  ...
  UInt64 delay_milliseconds = 0;
  {
    ...
    if (use_active_parts_threshold) {
      ...
      if ((*settings)[MergeTreeSetting::inactive_parts_to_throw_insert] > 0)
        allowed_parts_over_threshold =
            (*settings)[MergeTreeSetting::inactive_parts_to_throw_insert] - (*settings)[MergeTreeSetting::inactive_parts_to_delay_insert];
    }

    const UInt64 max_delay_milliseconds =
        ((*settings)[MergeTreeSetting::max_delay_to_insert] > 0 ? (*settings)[MergeTreeSetting::max_delay_to_insert] * 1000 : 1000);
    if (allowed_parts_over_threshold == 0 || parts_over_threshold > allowed_parts_over_threshold) {
      ...
      delay_milliseconds =
          std::max(min_delay_milliseconds, static_cast<UInt64>(static_cast<double>(max_delay_milliseconds) * delay_factor));
    }
  }
  ...
  LOG_INFO(log, "Delaying inserting block by {} ms. because there are {} parts and their average size is {}", delay_milliseconds,
...
void MergeTreeData::delayMutationOrThrowIfNeeded(Poco::Event *until, const ContextPtr &query_context) const {
  ...
  size_t num_mutations_to_delay = query_settings[Setting::number_of_mutations_to_delay]
                                      ? query_settings[Setting::number_of_mutations_to_delay]
                                      : (*settings)[MergeTreeSetting::number_of_mutations_to_delay];

  size_t num_mutations_to_throw = query_settings[Setting::number_of_mutations_to_throw]
                                      ? query_settings[Setting::number_of_mutations_to_throw]
                                      : (*settings)[MergeTreeSetting::number_of_mutations_to_throw];

  if (!num_mutations_to_delay && !num_mutations_to_throw) return;
  ...
  if (num_mutations_to_throw && num_unfinished_mutations >= num_mutations_to_throw) {
    ...
    throw Exception(ErrorCodes::TOO_MANY_MUTATIONS, "Too many unfinished mutations ({}) in table {}", num_unfinished_mutations,
                    getLogName());
  }

  if (num_mutations_to_delay && num_unfinished_mutations >= num_mutations_to_delay) {
    if (!num_mutations_to_throw) num_mutations_to_throw = num_mutations_to_delay * 2;
    ...
    double delay_factor =
        std::min(static_cast<double>(mutations_over_threshold) / static_cast<double>(allowed_mutations_over_threshold), 1.0);
    size_t delay_milliseconds =
        static_cast<size_t>(interpolateLinear(static_cast<double>((*settings)[MergeTreeSetting::min_delay_to_mutate_ms]),
                                              static_cast<double>((*settings)[MergeTreeSetting::max_delay_to_mutate_ms]), delay_factor));
...
void MergeTreeData::throwLightweightUpdateIfNeeded(UInt64 added_uncompressed_bytes) const {
  ...
  if (max_uncompressed_bytes_in_patches == 0) return;
  ...
  if (new_bytes_in_patches >= max_uncompressed_bytes_in_patches) {
    ...
    throw Exception(ErrorCodes::TOO_LARGE_LIGHTWEIGHT_UPDATES,
                    ...
                    " The threshold can be modified with 'max_uncompressed_bytes_in_patches' setting in <merge_tree> element in config.xml "
                    "or with per-table setting."
                    " Also you can materialize current patches by 'ALTER TABLE ... APPLY PATCHES' mutation or consider using an 'ALTER "
                    "TABLE ... UPDATE' mutation for updating the data",
...
MergeTreeData::DataPartPtr MergeTreeData::getActiveContainingPart(const MergeTreePartInfo &part_info, MergeTreeData::DataPartState state,
  ...
  if (it != current_state_parts_range.end()) {
    if ((*it)->info == part_info) return *it;
    if ((*it)->info.contains(part_info)) return *it;
  }
...
MergeTreeData::DataPartsVector MergeTreeData::getVisibleDataPartsVectorInPartition(ContextPtr local_context,
...
MergeTreeData::DataPartsVector MergeTreeData::getVisibleDataPartsVectorInPartition(ContextPtr local_context, const String &partition_id,
                                                                                   const DataPartsAnyLock &lock) const {
  return getVisibleDataPartsVectorInPartition(local_context->getCurrentTransaction().get(), partition_id, lock);
}

MergeTreeData::DataPartsVector MergeTreeData::getVisibleDataPartsVectorInPartition(MergeTreeTransaction *txn, const String &partition_id,
                                                                                   const DataPartsAnyLock &lock) const {
  if (txn) {
    ...
    DataPartsVector res;
    {
      ...
      res.insert(res.end(), data_parts_by_state_and_info.lower_bound(outdated_parts),
...
MergeTreeData::DataPartsVector MergeTreeData::getDataPartsVectorInPartitionForInternalUsage(const DataPartStates &affordable_states,
                                                                                            const String &partition_id,
                                                                                            const DataPartsAnyLock & /* lock */) const {
  DataPartsVector res;
  for (const auto &state : affordable_states) {
    DataPartStateAndPartitionID state_with_partition{state, partition_id};
    res.insert(res.end(), data_parts_by_state_and_info.lower_bound(state_with_partition),
               data_parts_by_state_and_info.upper_bound(state_with_partition));
  }
  return res;
}

MergeTreeData::DataPartsVector MergeTreeData::getDataPartsVectorInPartitionForInternalUsage(const MergeTreeData::DataPartState &state,
                                                                                            const String &partition_id,
                                                                                            const DataPartsAnyLock & /* lock */) const {
  DataPartStateAndPartitionID state_with_partition{state, partition_id};

  return DataPartsVector(data_parts_by_state_and_info.lower_bound(state_with_partition),
                         data_parts_by_state_and_info.upper_bound(state_with_partition));
}

MergeTreeData::DataPartsVector MergeTreeData::getVisibleDataPartsVectorInPartitions(ContextPtr local_context,
                                                                                    const std::unordered_set<String> &partition_ids) const {
  ...
  DataPartsVector res;
  {
    ...
    for (const auto &partition_id : partition_ids) {
      DataPartStateAndPartitionID active_parts{MergeTreeDataPartState::Active, partition_id};
      insertAtEnd(res, DataPartsVector(data_parts_by_state_and_info.lower_bound(active_parts),
      ...
      if (txn) {
        ...
        insertAtEnd(res, DataPartsVector(data_parts_by_state_and_info.lower_bound(outdated_parts),
                                         data_parts_by_state_and_info.upper_bound(outdated_parts)));
      }
    }
  }

  if (txn) filterVisibleDataParts(res, txn->getSnapshot(), txn->tid);

  return res;
}

MergeTreeData::DataPartPtr MergeTreeData::getPartIfExists(const MergeTreePartInfo &part_info,
                                                          const MergeTreeData::DataPartStates &valid_states) const {
  auto lock = readLockParts();
...
MergeTreeData::DataPartPtr MergeTreeData::getPartIfExistsUnlocked(const String &part_name, const DataPartStates &valid_states,
                                                                  const DataPartsAnyLock &acquired_lock) const {
  return getPartIfExistsUnlocked(MergeTreePartInfo::fromPartName(part_name, format_version), valid_states, acquired_lock);
}

MergeTreeData::DataPartPtr MergeTreeData::getPartIfExistsUnlocked(const MergeTreePartInfo &part_info, const DataPartStates &valid_states,
                                                                  const DataPartsAnyLock & /* acquired_lock */) const {
  auto it = data_parts_by_info.find(part_info);
  if (it == data_parts_by_info.end()) return nullptr;

  for (auto state : valid_states)
    if ((*it)->getState() == state) return *it;
...
void MergeTreeData::calculateColumnAndSecondaryIndexSizesImpl(DataPartsLock & /*parts_lock*/) const {
  ...
  for (const auto &part : committed_parts_range) addPartContributionToColumnAndSecondaryIndexSizesUnlocked(part);

  are_columns_and_secondary_indices_sizes_calculated = true;
}

void MergeTreeData::calculateColumnAndSecondaryIndexSizesLazily(DataPartsSharedLock &parts_lock,
                                                                std::unique_lock<std::mutex> & /*sizes_lock*/) const {
  if (are_columns_and_secondary_indices_sizes_calculated) return;
  ...
  if (hasColumnsWithDynamicSubcolumns(getInMemoryMetadataPtr()->getSampleBlock())) {
    ...
    for (const auto &part : data_parts) addPartContributionToColumnAndSecondaryIndexSizesUnlocked(part);
  } else {
    for (const auto &part : committed_parts_range) addPartContributionToColumnAndSecondaryIndexSizesUnlocked(part);
  }
...
void MergeTreeData::removePartContributionToColumnAndSecondaryIndexSizes(const DataPartPtr &part) const {
  ...
  for (const auto &column : part->getColumns()) {
    ...
    auto log_subtract = [&](size_t &from, size_t value, const char *field) {
      if (value > from)
        LOG_ERROR(log, "Possibly incorrect column size subtraction: {} - {} = {}, column: {}, field: {}", from, value, from - value,
                  column.name, field);
  ...
  for (auto &[secondary_index_name, total_secondary_index_size] : secondary_index_sizes) {
    if (!part->hasSecondaryIndex(secondary_index_name, metadata_ptr)) continue;

    IndexSize part_secondary_index_size = part->getSecondaryIndexSize(secondary_index_name);

    auto log_subtract = [&](size_t &from, size_t value, const char *field) {
      if (value > from)
        LOG_ERROR(log, "Possibly incorrect index size subtraction: {} - {} = {}, index: {}, field: {}", from, value, from - value,
                  secondary_index_name, field);
...
void MergeTreeData::checkAlterPartitionIsPossible(const PartitionCommands &commands, const StorageMetadataPtr & /*metadata_snapshot*/,
  ...
  for (const auto &command : commands) {
    ...
    for (const auto &disk : disks) {
      ...
      switch (data_source_description.metadata_type) {
        ...
        case MetadataStorageType::PlainRewritable: {
          const static auto supported_commands = {
              PartitionCommand::DROP_PARTITION, PartitionCommand::DROP_DETACHED_PARTITION, PartitionCommand::ATTACH_PARTITION,
              PartitionCommand::MOVE_PARTITION, PartitionCommand::REPLACE_PARTITION,
          };
      ...
      if (!can_execute_alter_on_disk)
        throw Exception(ErrorCodes::SUPPORT_IS_DISABLED, "Partition operation ALTER TABLE {} is not supported for disk '{}'",
...
void MergeTreeData::checkPartitionCanBeDropped(const ASTPtr &partition, ContextPtr local_context) {
  if (!supportsReplication() && isStaticStorage()) return;
  ...
  for (const auto &part : parts_to_remove) partition_size += part->getBytesOnDisk();
  ...
  if (query_settings[Setting::max_partition_size_to_drop].changed) {
    getContext()->checkPartitionCanBeDropped(table_id.database_name, table_id.table_name, partition_size,
                                             query_settings[Setting::max_partition_size_to_drop]);
    return;
  }
...
void MergeTreeData::checkPartCanBeDropped(const String &part_name, ContextPtr local_context) {
  ...
  if (!part) throw Exception(ErrorCodes::NO_SUCH_DATA_PART, "No part {} in committed state", part_name);
...
void MergeTreeData::movePartitionToDisk(const ASTPtr &partition, const String &name, bool moving_part, ContextPtr local_context) {
  ...
  std::erase_if(parts, [&](auto part_ptr) { return part_ptr->getDataPartStorage().getDiskName() == disk->getName(); });

  if (parts.empty()) {
    if (moving_part) throw Exception(ErrorCodes::UNKNOWN_DISK, "Part '{}' is already on disk '{}'", partition_id, disk->getName());
  ...
  const auto &query_settings = local_context->getSettingsRef();
  std::future<MovePartsOutcome> moves_future =
      movePartsToSpace(moving_tagger, local_context->getReadSettings(), local_context->getWriteSettings(),
                       query_settings[Setting::alter_move_to_space_execute_async]);

  if (query_settings[Setting::alter_move_to_space_execute_async] &&
      moves_future.wait_for(std::chrono::seconds(0)) != std::future_status::ready) {
    return;
  }
  ...
  switch (moves_outcome) {
    ...
    case MovePartsOutcome::CannotScheduleMove:
      throw Exception(
          ...
          "Cannot schedule move, no free threads, try to wait until all in-progress move finish or increase <background_move_pool_size>");
...
void MergeTreeData::movePartitionToVolume(const ASTPtr &partition, const String &name, bool moving_part, ContextPtr local_context) {
  ...
  if (!volume) throw Exception(ErrorCodes::UNKNOWN_DISK, "Volume {} does not exist on policy {}", name, getStoragePolicy()->getName());

  if (parts.empty()) throw Exception(ErrorCodes::NO_SUCH_DATA_PART, "Nothing to move (check that the partition exists).");
  ...
  if (parts.empty()) {
    if (moving_part) throw Exception(ErrorCodes::UNKNOWN_DISK, "Part '{}' is already on volume '{}'", partition_id, volume->getName());
...
void MergeTreeData::movePartitionToTable(const PartitionCommand &command, ContextPtr query_context) {
  ...
  if (dest_storage->getStorageID() == this->getStorageID()) return;

  auto *dest_storage_merge_tree = dynamic_cast<MergeTreeData *>(dest_storage.get());
  if (!dest_storage_merge_tree)
    throw Exception(ErrorCodes::NOT_IMPLEMENTED, "Cannot move partition from table {} to table {} with storage {}",
...
void MergeTreeData::movePartitionToShard(const ASTPtr & /*partition*/, bool /*move_part*/, const String & /*to*/,
                                         ContextPtr /*query_context*/) {
  throw Exception(ErrorCodes::NOT_IMPLEMENTED, "MOVE PARTITION TO SHARD is not supported by storage {}", getName());
}

void MergeTreeData::fetchPartition(const ASTPtr & /*partition*/, const StorageMetadataPtr & /*metadata_snapshot*/, const String & /*from*/,
...
Pipe MergeTreeData::alterPartition(const StorageMetadataPtr &metadata_snapshot, const PartitionCommands &commands,
                                   ContextPtr query_context) {
  ...
  for (const PartitionCommand &command : commands) {
    ...
    switch (command.type) {
      ...
      case PartitionCommand::REPLACE_PARTITION: {
        if (command.replace) checkPartitionCanBeDropped(command.partition, query_context);
        ...
        if (!from_storage_merge_tree)
          throw Exception(ErrorCodes::NOT_IMPLEMENTED, "Cannot replace partition from table {} with storage {} to table {}",
    ...
    for (auto &command_result : current_command_results) command_result.command_type = command.typeToString();
    result.insert(result.end(), current_command_results.begin(), current_command_results.end());
  }

  if (query_context->getSettingsRef()[Setting::alter_partition_verbose_result]) return convertCommandsResultToSource(result);

  return {};
}

MergeTreeData::PartsBackupEntries MergeTreeData::backupParts(const DataPartsVector &data_parts, const String &data_path_in_backup,
                                                             const BackupSettings &backup_settings, const ContextPtr &local_context) {
  ...
  for (const auto &part : data_parts) {
    ...
    } else if (supportsReplication() && part->getDataPartStorage().supportZeroCopyReplication() &&
    ...
    part->getDataPartStorage().backup(part->checksums, part->getFileNamesWithoutChecksums(), data_path_in_backup, backup_settings,
                                      make_temporary_hard_links, backup_entries_from_part, &temp_dirs, false, false);

    auto backup_projection = [&](IDataPartStorage &storage, IMergeTreeDataPart &projection_part) {
      storage.backup(projection_part.checksums, projection_part.getFileNamesWithoutChecksums(), fs::path{data_path_in_backup} / part->name,
                     backup_settings, make_temporary_hard_links, backup_entries_from_part, &temp_dirs, projection_part.is_broken,
    ...
    for (const auto &[name, _] : part->checksums.files) {
      ...
      if (endsWith(name, proj_suffix) && !defined_projections.contains(projection_name)) {
        ...
        if (projection_storage->existsFile("checksums.txt")) {
          auto projection_part = const_cast<IMergeTreeDataPart &>(*part)
...
void MergeTreeData::restoreDataFromBackup(RestorerFromBackup &restorer, const String &data_path_in_backup,
                                          const std::optional<ASTs> &partitions) {
  auto backup = restorer.getBackup();
  if (!backup->hasFiles(data_path_in_backup)) return;
...
class MergeTreeData::RestoredPartsHolder {
 public:
  RestoredPartsHolder(const std::shared_ptr<MergeTreeData> &storage_, const BackupPtr &backup_) : storage(storage_), backup(backup_) {}
  ...
  String getTemporaryDirectory(const DiskPtr &disk, const String &part_name) {
    ...
    if (it == temp_part_dirs.end()) {
      auto temp_dir_deleter =
          std::make_unique<TemporaryFileOnDisk>(disk, fs::path{storage->getRelativeDataPath()} / ("tmp_restore_" + part_name + "-"));
      ...
      it = temp_part_dirs.emplace(part_name, std::make_pair(std::move(temp_dir_deleter), std::move(temp_dir_holder))).first;
    }
 ...
 private:
  void attachIfAllPartsRestored() {
    if (!num_parts || (parts.size() + num_broken_parts < num_parts)) return;
...
void MergeTreeData::restorePartsFromBackup(RestorerFromBackup &restorer, const String &data_path_in_backup,
                                           const std::optional<ASTs> &partitions) {
  std::optional<std::unordered_set<String>> partition_ids;
  if (partitions) partition_ids = getPartitionIDsFromQuery(*partitions, restorer.getContext());
  ...
  for (const String &part_name : part_names) {
    ...
    if (!part_info) {
      throw Exception(ErrorCodes::CANNOT_RESTORE_TABLE, "File name {} is not a part's name", String{data_path_in_backup_fs / part_name});
    }

    if (partition_ids && !partition_ids->contains(part_info->getPartitionId())) continue;

    restorer.addDataRestoreTask([storage = std::static_pointer_cast<MergeTreeData>(shared_from_this()), backup,
                                 part_path_in_backup = data_path_in_backup_fs / part_name, my_part_info = *part_info,
                                 restore_broken_parts_as_detached, restored_parts_holder] {
...
void MergeTreeData::restorePartFromBackup(std::shared_ptr<RestoredPartsHolder> restored_parts_holder, const MergeTreePartInfo &part_info,
                                          const String &part_path_in_backup, bool detach_if_broken) const {
  ...
  for (const String &filename : filenames) total_size_of_part += backup->getFileSize(part_path_in_backup_fs / filename);
  ...
  for (const String &filename : filenames) {
    ...
    if (separator_pos != String::npos) {
      ...
      if (subdirs.emplace(subdir).second) disk->createDirectories(temp_part_dir / subdir);
    }
...
MergeTreeData::MutableDataPartPtr MergeTreeData::loadPartRestoredFromBackup(const String &part_name, const DiskPtr &disk,
                                                                            const String &temp_part_dir, bool detach_if_broken) const {
...
String MergeTreeData::getPartitionIDFromQuery(const ASTPtr &ast, ContextPtr local_context, const DataPartsLock *acquired_lock) const {
  ...
  if (partition_ast.all) throw Exception(ErrorCodes::SUPPORT_IS_DISABLED, "Only Support DROP/DETACH/ATTACH PARTITION ALL currently");
  ...
  if (!partition_ast.fields_count.has_value()) {
    if (partition_value_ast->as<ASTLiteral>()) {
      ...
      throw Exception(ErrorCodes::INVALID_PARTITION_VALUE, "Expected literal or tuple for partition key, got {}",
  ...
  if (partition_ast_fields_count != fields_count)
    throw Exception(ErrorCodes::INVALID_PARTITION_VALUE, "Wrong number of fields in the partition expression: {}, must be: {}",
  ...
  if (fields_count == 0) {
    ...
    if (!args) throw Exception(ErrorCodes::BAD_ARGUMENTS, "Expected at least one argument in partition AST");
    bool empty_tuple = partition_value_ast->as<ASTFunction>()->arguments->children.empty();
    if (!empty_tuple) throw Exception(ErrorCodes::INVALID_PARTITION_VALUE, "Partition key is empty, expected 'tuple()' as partition key");
    ...
    if (partition_key_value.getType() != Field::Types::Tuple)
      throw Exception(ErrorCodes::INVALID_PARTITION_VALUE, "Expected tuple for complex partition key, got {}",
                      partition_key_value.getTypeName());

    const Tuple &tuple = partition_key_value.safeGet<Tuple>();
    if (tuple.size() != fields_count)
      throw Exception(ErrorCodes::LOGICAL_ERROR, "Wrong number of fields in the partition expression: {}, must be: {}", tuple.size(),
                      fields_count);
...
std::tuple<RangesInDataPartsPtr, MergeTreeData::DataPartsVectorPtr> MergeTreeData::getPossiblySharedVisibleDataPartsRanges(
    ContextPtr local_context) const {
  ...
  if (const auto *txn = local_context->getCurrentTransaction().get()) {
    ...
    return std::make_tuple(std::make_shared<const RangesInDataParts>(parts), std::make_shared<const DataPartsVector>(std::move(parts)));
  } else {
...
void MergeTreeData::filterVisibleDataParts(DataPartsVector &maybe_visible_parts, CSN snapshot_version, TransactionID current_tid) const {
  ...
  LOG_TEST(log, "Got {} parts (of {}) visible in snapshot {} (TID {}): {}", visible_size, total_size, snapshot_version, current_tid,
           fmt::join(getPartsNames(maybe_visible_parts), ", "));
}

std::unordered_set<String> MergeTreeData::getPartitionIDsFromQuery(const ASTs &asts, ContextPtr local_context) const {
  std::unordered_set<String> partition_ids;
  for (const auto &ast : asts) partition_ids.emplace(getPartitionIDFromQuery(ast, local_context));
  return partition_ids;
}

std::set<String> MergeTreeData::getPartitionIdsAffectedByCommands(const MutationCommands &commands, ContextPtr query_context) const {
  ...
  for (const auto &command : commands) {
    ...
    affected_partition_ids.insert(getPartitionIDFromQuery(command.partition, query_context));
  }
...
std::unordered_set<String> MergeTreeData::getAllPartitionIds() const {
  ...
  for (const auto &part : getDataPartsStateRange(DataPartState::Active)) {
    if (prev_id == part->info.getPartitionId()) continue;
...
MergeTreeData::DataPartsVector MergeTreeData::getDataPartsVectorForInternalUsage(const DataPartStates &affordable_states,
                                                                                 const DataPartsKinds &affordable_kinds,
                                                                                 const DataPartsAnyLock & /*lock*/,
  ...
  if (out_states != nullptr) {
    ...
    for (size_t i = 0; i < res.size(); ++i) (*out_states)[i] = res[i]->getState();
  }
...
DataPartsVector MergeTreeData::getDataPartsVectorForInternalUsage(const DataPartStates &affordable_states, const DataPartsAnyLock &lock,
                                                                  DataPartStateVector *out_states) const {
...
DataPartsVector MergeTreeData::getPatchPartsVectorForInternalUsage(const DataPartStates &affordable_states, const DataPartsLock &lock,
                                                                   DataPartStateVector *out_states) const {
  return getDataPartsVectorForInternalUsage(affordable_states, {DataPartKind::Patch}, lock, out_states);
}

DataPartsVector MergeTreeData::getPatchPartsVectorForInternalUsage(const DataPartStates &affordable_states,
                                                                   DataPartStateVector *out_states) const {
  auto lock = readLockParts();
  return getDataPartsVectorForInternalUsage(affordable_states, {DataPartKind::Patch}, lock, out_states);
}
...
DataPartsVector MergeTreeData::getPatchPartsVectorForPartition(const String &partition_id, const DataPartsAnyLock & /*lock*/) const {
  ...
  for (const auto &part : range) {
    if (isPatchForPartition(part->info, partition_id)) res.push_back(part);
  }
...
MergeTreeData::ProjectionPartsVector MergeTreeData::getProjectionPartsVectorForInternalUsage(const DataPartStates &affordable_states,
                                                                                             DataPartStateVector *out_states) const {
  auto lock = readLockParts();
  ...
  for (auto state : affordable_states) {
    ...
    for (const auto &part : range) {
      ...
      for (const auto &[_, projection_part] : part->getProjectionParts()) res.projection_parts.push_back(projection_part);
    }
  }

  if (out_states != nullptr) {
    out_states->resize(res.projection_parts.size());
    for (size_t i = 0; i < res.projection_parts.size(); ++i) (*out_states)[i] = res.projection_parts[i]->getParentPart()->getState();
  }
...
bool MergeTreeData::supportsLightweightDelete() const {
  auto lock = readLockParts();
  for (const auto &part : data_parts_by_info) {
    if (part->getState() == MergeTreeDataPartState::Outdated || part->getState() == MergeTreeDataPartState::Deleting) continue;

    if (!part->supportLightweightDeleteMutate()) return false;
  }
  return true;
}
...
bool MergeTreeData::hasProjection() const {
  ...
  for (const auto &part : data_parts_by_info) {
    ...
    if (part->hasProjection()) return true;
  }
  return false;
}

bool MergeTreeData::areAsynchronousInsertsEnabled() const { return (*getSettings())[MergeTreeSetting::async_insert]; }
...
MergeTreeData::ProjectionPartsVector MergeTreeData::getAllProjectionPartsVector(MergeTreeData::DataPartStateVector *out_states) const {
  ...
  for (const auto &part : data_parts_by_info) {
    res.data_parts.push_back(part);
    for (const auto &[p_name, projection_part] : part->getProjectionParts()) res.projection_parts.push_back(projection_part);
  }
...
DetachedPartsInfo MergeTreeData::getDetachedParts() const {
  ...
  for (const auto &disk : getDisks()) {
    ...
    if (disk->isReadOnly() || disk->isWriteOnce()) continue;
...
void MergeTreeData::validateDetachedPartName(const String &name) {
  if (name.empty()) throw DB::Exception(ErrorCodes::BAD_DATA_PART_NAME, "Empty part name");

  if (name.contains('/') || name == "." || name == "..")
    throw DB::Exception(ErrorCodes::INCORRECT_FILE_NAME, "Invalid part name '{}'", name);

  if (startsWith(name, "attaching_") || startsWith(name, "deleting_"))
    throw DB::Exception(ErrorCodes::BAD_DATA_PART_NAME,
                        "Cannot drop part {}: "
...
void MergeTreeData::dropDetached(const ASTPtr &partition, bool part, ContextPtr local_context) {
  ...
  if (part) {
    ...
    if (!all) partition_id = getPartitionIDFromQuery(partition, local_context);
    DetachedPartsInfo detached_parts = getDetachedParts();
    for (const auto &part_info : detached_parts)
      if (part_info.valid_name && (all || part_info.getPartitionId() == partition_id) && part_info.prefix != "attaching" &&
          part_info.prefix != "deleting")
...
void MergeTreeData::optimizeDryRun(const Names &part_names, const StorageMetadataPtr &metadata_snapshot, bool deduplicate,
                                   const Names &deduplicate_by_columns, bool cleanup, ContextPtr local_context) {
  if (part_names.empty()) throw Exception(ErrorCodes::BAD_ARGUMENTS, "OPTIMIZE DRY RUN requires at least one part name");
  ...
  for (const auto &part_info : part_infos) {
    ...
    if (!part) {
      throw Exception(ErrorCodes::NO_SUCH_DATA_PART, "Part {} not found in table {}", part_info.getPartNameForLogs(),
                      getStorageID().getNameForLogs());
    }

    if (!choice.range.empty() && part->info.getPartitionId() != choice.range.front().info.getPartitionId()) {
      throw Exception(ErrorCodes::BAD_ARGUMENTS,
                      "All parts for OPTIMIZE DRY RUN must belong to the same partition, but part {} belongs to partition {} while part {} "
  ...
  if ((*getSettings())[MergeTreeSetting::apply_patches_on_merge]) {
    ...
    if (!all_patch_parts.empty()) {
      ...
      for (const auto &patch : all_patch_parts) patch_infos.push_back(patch->info);
  ...
  if (!future_part)
    throw Exception(ErrorCodes::NO_SUCH_DATA_PART,
                    "Failed to construct future part for OPTIMIZE DRY RUN. Some of the source parts don't exist in the table");
  ...
  auto merge_list_entry = getContext()->getMergeList().insert(getStorageID(), future_part, task_context);

  auto merge_task =
      merger_mutator.mergePartsToTemporaryPart(future_part, metadata_snapshot, merge_list_entry.get(),
                                               /*projection_merge_list_element=*/{}, table_lock, time(nullptr), task_context, reservation,
                                               deduplicate, deduplicate_by_columns, cleanup, merging_params, nullptr /* txn */);

  auto new_part = executeHere(merge_task);

  /// This commits a transcation on disk, but doesn't commit the data part.
  if (new_part->getDataPartStorage().hasActiveTransaction()) new_part->getDataPartStorage().commitTransaction();

  LOG_INFO(log, "OPTIMIZE DRY RUN: successfully merged {} parts into temporary part {} ({} rows, {} bytes)", choice.range.size(),
           new_part->name, new_part->rows_count, new_part->getBytesOnDisk());
  ...
  if (local_context->getSettingsRef()[Setting::optimize_dry_run_check_part]) {
    ...
    checkDataPart(
        new_part,
        /*require_checksums=*/true, is_broken_projection,
...
MergeTreeData::MutableDataPartsVector MergeTreeData::tryLoadPartsToAttach(const PartitionCommand &command, ContextPtr local_context,
  ...
  if (command.part) {
    ...
    if (temporary_parts.contains(source_dir / part_directory)) {
      LOG_WARNING(log,
                  "Will not try to attach part {} (from directory {}) because its directory is temporary, "
                  "probably it's being detached right now",
                  part_name, part_directory);
      return {};
    }
    ...
    for (const auto &part_info : detached_parts) {
      if (temporary_parts.contains(String(DETACHED_DIR_NAME) + "/" + part_info.dir_name)) {
        LOG_WARNING(log,
                    "Will not try to attach part {} because its directory is temporary, "
                    "probably it's being detached right now",
                    part_info.dir_name);
        continue;
      }
    ...
    for (const auto &part_info : detached_parts) {
      ...
      if (containing_part.empty()) continue;
  ...
  for (const auto &[part_name, old_dir, new_dir, disk] : renamed_parts.old_and_new_names) {
    ...
    auto part = getDataPartBuilder(part_name, single_disk_volume, source_dir / new_dir, getReadSettings()).withPartFormatFromDisk().build();
...
bool MergeTreeData::assertNoPatchesForParts(const DataPartsVector &parts, const DataPartsVector &patches, std::string_view command,
                                            bool throw_on_error) const {
  if (parts.empty() || patches.empty()) return true;
  ...
  for (const auto &part : parts) {
    if (part->info.getPartitionId() != partition_id)
      throw Exception(ErrorCodes::LOGICAL_ERROR, "Parts belong to different partitions: {} and {}", part->info.getPartitionId(),
                      partition_id);
  }
  ...
  for (const auto &patch : patches) {
    if (patchHasHigherDataVersion(*patch, min_data_version)) ++num_patches;
  }

  if (num_patches > 0) {
    auto base_message = fmt::format("Cannot execute command \"{}\" because there are {} unapplied patch parts for partition {}", command,
                                    num_patches, partition_id);
...
ReservationPtr MergeTreeData::reserveSpacePreferringTTLRules(const StorageMetadataPtr &metadata_snapshot, UInt64 expected_size,
                                                             const IMergeTreeDataPart::TTLInfos &ttl_infos, time_t time_of_move,
                                                             size_t min_volume_index, bool is_insert, DiskPtr selected_disk) const {
  expected_size = std::max(RESERVATION_MIN_ESTIMATION_SIZE, expected_size);

  ReservationPtr reservation = tryReserveSpacePreferringTTLRules(metadata_snapshot, expected_size, ttl_infos, time_of_move,
                                                                 min_volume_index, is_insert, selected_disk);

  return checkAndReturnReservation(expected_size, std::move(reservation));
}

ReservationPtr MergeTreeData::tryReserveSpacePreferringTTLRules(const StorageMetadataPtr &metadata_snapshot, UInt64 expected_size,
                                                                const IMergeTreeDataPart::TTLInfos &ttl_infos, time_t time_of_move,
                                                                size_t min_volume_index, bool is_insert, DiskPtr selected_disk) const {
  expected_size = std::max(RESERVATION_MIN_ESTIMATION_SIZE, expected_size);
  ...
  if (move_ttl_entry) {
    ...
    if (!destination_ptr) {
      if (move_ttl_entry->destination_type == DataDestinationType::VOLUME && !move_ttl_entry->if_exists)
        LOG_WARNING(log, "Would like to reserve space on volume '{}' by TTL rule of table '{}' but volume was not found",
                    move_ttl_entry->destination_name, log.loadName());
      else if (move_ttl_entry->destination_type == DataDestinationType::DISK && !move_ttl_entry->if_exists)
        LOG_WARNING(log, "Would like to reserve space on disk '{}' by TTL rule of table '{}' but disk was not found",
                    move_ttl_entry->destination_name, log.loadName());
    } else if (is_insert && !perform_ttl_move_on_insert) {
      LOG_TRACE(log, "TTL move on insert to {} {} for table {} is disabled",
                (move_ttl_entry->destination_type == DataDestinationType::VOLUME ? "volume" : "disk"), move_ttl_entry->destination_name,
                log.loadName());
    } else {
  ...
  if (selected_disk) {
    LOG_TRACE(log, "Trying to reserve {} on the selected disk: {} (with type {})", ReadableSize(expected_size), selected_disk->getName(),
...
SpacePtr MergeTreeData::getDestinationForMoveTTL(const TTLDescription &move_ttl) const {
  ...
  if (move_ttl.destination_type == DataDestinationType::VOLUME) return policy->tryGetVolumeByName(move_ttl.destination_name);

  if (move_ttl.destination_type == DataDestinationType::DISK) return policy->tryGetDiskByName(move_ttl.destination_name);
...
bool MergeTreeData::shouldPerformTTLMoveOnInsert(const SpacePtr &move_destination) const {
  ...
  if (move_destination->isDisk()) {
    ...
    if (auto volume = getStoragePolicy()->tryGetVolumeByDiskName(disk->getName())) return volume->perform_ttl_move_on_insert;
  }
  return false;
}
...
bool MergeTreeData::isPartInTTLDestination(const TTLDescription &ttl, const IMergeTreeDataPart &part) const {
  ...
  if (ttl.destination_type == DataDestinationType::VOLUME) {
    for (const auto &disk : policy->getVolumeByName(ttl.destination_name)->getDisks())
      if (disk->getName() == part.getDataPartStorage().getDiskName()) return true;
  } else if (ttl.destination_type == DataDestinationType::DISK)
    return policy->getDiskByName(ttl.destination_name)->getName() == part.getDataPartStorage().getDiskName();
  return false;
}

CompressionCodecPtr MergeTreeData::getCompressionCodecForPart(size_t part_size_compressed, const IMergeTreeDataPart::TTLInfos &ttl_infos,
  ...
  if (best_ttl_entry) return CompressionCodecFactory::instance().get(best_ttl_entry->recompression_codec, {});

  return getContext()->chooseCompressionCodec(part_size_compressed,
                                              static_cast<double>(part_size_compressed) / static_cast<double>(getTotalActiveSizeInBytes()));
}

MergeTreeData::DataParts MergeTreeData::getDataParts(const DataPartStates &affordable_states,
...
MergeTreeData::DataPartPtr MergeTreeData::getAnyPartInPartition(const String &partition_id,
                                                                const DataPartsAnyLock & /*data_parts_lock*/) const {
  auto it = data_parts_by_state_and_info.lower_bound(DataPartStateAndPartitionID{DataPartState::Active, partition_id});

  if (it != data_parts_by_state_and_info.end() && (*it)->getState() == DataPartState::Active &&
      (*it)->info.getPartitionId() == partition_id)
    return *it;

  return nullptr;
}

MergeTreeData::Transaction::Transaction(MergeTreeData &data_, MergeTreeTransaction *txn_) : data(data_), txn(txn_) {
  if (txn) data.transactions_enabled.store(true);
}
...
void MergeTreeData::Transaction::rollbackPartsToTemporaryState() {
  if (!isEmpty()) {
    ...
    for (const auto &part : precommitted_parts) buf << " " << part->getDataPartStorage().getPartDirectory();
    buf << ".";
    LOG_DEBUG(data.log, "Undoing transaction.{}", buf.str());

    data.removePartsFromWorkingSetImmediatelyAndSetTemporaryState(DataPartsVector(precommitted_parts.begin(), precommitted_parts.end()));
  }
...
void MergeTreeData::Transaction::addPart(MutableDataPartPtr &part, bool need_rename) {
  ...
  if (need_rename) precommitted_parts_need_rename.insert(part);
}
...
void MergeTreeData::Transaction::rollback(DataPartsLock *acquired_lock) {
  if (!isEmpty()) {
    for (const auto &part : precommitted_parts) part->version.creation_csn.store(Tx::RolledBackCSN);
    ...
    WriteBufferFromOwnString buf;
    buf << "Removing parts:";
    for (const auto &part : non_detached_precommitted_parts) buf << " " << part->getDataPartStorage().getPartDirectory();
    buf << ".";
    if (!detached_precommitted_parts.empty()) {
      buf << " Rolling back parts state to temporary and removing from working set:";
      for (const auto &part : detached_precommitted_parts) buf << " " << part->getDataPartStorage().getPartDirectory();
      buf << ".";
    }
    ...
    if (data.data_parts_indexes.empty()) {
      ...
      if (!data.all_data_dropped) {
        Strings part_names;
        for (const auto &part : non_detached_precommitted_parts) part_names.emplace_back(part->name);
        throw Exception(ErrorCodes::LOGICAL_ERROR,
                        "There are some PreActive parts ({}) to rollback, "
                        "but data parts set is empty and table {} was not dropped. It's a bug",
                        fmt::join(part_names, ", "), data.getStorageID().getNameForLogs());
      }
    } else {
      data.removePartsFromWorkingSetImmediatelyAndSetTemporaryState(detached_precommitted_parts, lock);

      data.removePartsFromWorkingSet(txn, DataPartsVector(non_detached_precommitted_parts.begin(), non_detached_precommitted_parts.end()),
...
MergeTreeData::DataPartsVector MergeTreeData::Transaction::commit(DataPartsLock &acquired_parts_lock) {
  ...
  if (!isEmpty()) {
    if (!precommitted_parts_need_rename.empty()) throw Exception(ErrorCodes::LOGICAL_ERROR, "Parts had not been renamed");

    auto settings = data.getSettings();

    for (const auto &part : precommitted_parts)
      if (part->getDataPartStorage().hasActiveTransaction()) part->getDataPartStorage().commitTransaction();
    ...
    try {
      for (const auto &part : precommitted_parts) {
        ...
        if (txn) {
          ...
          LOG_TEST(data.log, "Got {} oudated parts covered by {} (TID {} CSN {}): {}", covered_outdated_parts.size(),
                   part->getNameWithState(), txn->tid, txn->getSnapshot(), fmt::join(getPartsNames(covered_outdated_parts), ", "));
        ...
        if (!covering_part) {
          ...
          if (!txn) locked_parts_to_cleanup.insert(locked_parts_to_cleanup.end(), covered_parts.begin(), covered_parts.end());
        }
...
bool MergeTreeData::isPrimaryOrMinMaxKeyColumnPossiblyWrappedInFunctions(const ASTPtr &node,
...
Block MergeTreeData::getMinMaxCountProjectionBlock(const StorageMetadataPtr &metadata_snapshot, const Names &required_columns,
                                                   const ActionsDAG *filter_dag, const RangesInDataParts &parts,
                                                   const PartitionIdToMaxBlock *max_block_numbers_to_read, ContextPtr query_context) const {
  ...
  if (!primary_key_max_column_name.empty()) need_primary_key_max_column = required_columns_set.contains(primary_key_max_column_name);
  ...
  if (has_virtual_column || filter_dag) {
    ...
    if (virtual_columns_block.rows() == 0) return {};
  }
  ...
  if (filter_dag) {
    if (metadata_snapshot->hasPartitionKey()) {
      ...
      minmax_idx_condition.emplace(inverted_dag, query_context, minmax_columns_names,
    ...
    VirtualColumnUtils::filterBlockWithPredicate(predicate, virtual_columns_block, query_context,
                                                 /*allow_filtering_with_partial_predicate =*/true);
  ...
  for (size_t row = 0, part_idx = 0; row < rows; ++row, ++part_idx) {
    if (part_name_column) {
      while (parts[part_idx].data_part->name != part_name_column->getDataAt(row)) ++part_idx;
    }
    ...
    if (max_block_numbers_to_read) {
      ...
      if (blocks_iterator == max_block_numbers_to_read->end() || part->info.max_block > blocks_iterator->second) continue;
    }
    ...
    if (partition_pruner) {
      if (partition_pruner->canBePruned(*part)) continue;
    }
    ...
    /// no final mark.
    if (need_primary_key_max_column && !part->index_granularity->hasFinalMark()) return {};

    real_parts.push_back(part);
    filter_column_data.back() = 1;
  }

  if (real_parts.empty()) return {};
  ...
  for (size_t i : metadata_snapshot->minmax_count_projection->partition_value_indices) {
    if (required_columns_set.contains(partition_minmax_count_column_names[pos]))
      for (const auto &part : real_parts) partition_minmax_count_columns[pos]->insert(part->partition.value[i]);
    ++pos;
  }
...
ActionDAGNodes MergeTreeData::getFiltersForPrimaryKeyAnalysis(const InterpreterSelectQuery &select) {
  ...
  if (before_where) filter_nodes.nodes.push_back(&before_where->dag.findInOutputs(where_column_name));

  return filter_nodes;
}

QueryProcessingStage::Enum MergeTreeData::getQueryProcessingStage(ContextPtr query_context, QueryProcessingStage::Enum to_stage,
                                                                  const StorageSnapshotPtr &, SelectQueryInfo &) const {
  ...
  if (!query_context->getSettingsRef()[Setting::allow_experimental_analyzer]) {
    ...
    if (query_context->canUseParallelReplicasCustomKey()) {
      if (query_context->getClientInfo().distributed_depth > 0) return QueryProcessingStage::FetchColumns;
    ...
    if (query_context->getClientInfo().collaborate_with_initiator) return QueryProcessingStage::Enum::FetchColumns;

    /// Parallel replicas
    if (query_context->canUseParallelReplicasOnInitiator() && to_stage >= QueryProcessingStage::WithMergeableState) {
      /// ReplicatedMergeTree
      if (supportsReplication()) return QueryProcessingStage::Enum::WithMergeableState;

      /// For non-replicated MergeTree we allow them only if parallel_replicas_for_non_replicated_merge_tree is enabled
      if (settings[Setting::parallel_replicas_for_non_replicated_merge_tree]) return QueryProcessingStage::Enum::WithMergeableState;
    }
  }
...
UInt64 MergeTreeData::estimateNumberOfRowsToRead(ContextPtr query_context, const StorageSnapshotPtr &storage_snapshot,
                                                 const SelectQueryInfo &query_info) const {
  const auto &snapshot_data = assert_cast<const MergeTreeData::SnapshotData &>(*storage_snapshot->data);

  MergeTreeDataSelectExecutor reader(*this);
  auto result_ptr = reader.estimateNumMarksToRead(*snapshot_data.parts, snapshot_data.mutations_snapshot,
                                                  storage_snapshot->metadata->getColumns().getAll().getNames(), storage_snapshot->metadata,
                                                  query_info, query_context, query_context->getSettingsRef()[Setting::max_threads]);

  UInt64 total_rows = result_ptr->selected_rows;
  if (query_info.trivial_limit > 0 && query_info.trivial_limit < total_rows) total_rows = query_info.trivial_limit;
  return total_rows;
}
...
void MergeTreeData::checkColumnFilenamesForCollision(const ColumnsDescription &columns, const MergeTreeSettings &settings,
                                                     bool throw_on_error) const {
  ...
  for (const auto &column : columns_list) {
    ...
    auto callback = [&](const auto &substream_path) {
      auto full_stream_name =
          ISerialization::getFileNameForStream(column, substream_path, ISerialization::StreamFileNameSettings(settings));
    ...
    for (const auto &[stream_name, full_stream_name] : column_streams) {
      ...
      if (!inserted) {
        ...
        auto message =
            fmt::format("Columns '{} {}' and '{} {}' have streams ({} and {}) with collision in file name {}", column.name,
                        column.type->getName(), other_column_name, other_type->getName(), full_stream_name, other_full_name, stream_name);

        if (settings[MergeTreeSetting::replace_long_file_name_to_hash])
          message +=
              ". It may be a collision between a filename for one column and a hash of filename for another column (see setting "
              "'replace_long_file_name_to_hash')";

        if (throw_on_error) throw Exception(ErrorCodes::BAD_ARGUMENTS, "{}", message);

        LOG_ERROR(log, "Table definition is incorrect. {}. It may lead to corruption of data or crashes. You need to resolve it manually",
                  message);
        return;
      }
    }
...
MergeTreeData &MergeTreeData::checkStructureAndGetMergeTreeData(IStorage &source_table, const StorageMetadataPtr &src_snapshot,
                                                                const StorageMetadataPtr &my_snapshot) const {
  MergeTreeData *src_data = dynamic_cast<MergeTreeData *>(&source_table);
  if (!src_data)
    throw Exception(ErrorCodes::NOT_IMPLEMENTED, "Table {} supports attachPartitionFrom only for MergeTree family of table engines. Got {}",
  ...
  if (format_version != src_data->format_version) throw Exception(ErrorCodes::BAD_ARGUMENTS, "Tables have different format_version");
  ...
  const auto check_definitions = [this](const auto &my_descriptions, const auto &src_descriptions) {
    ...
    if ((my_descriptions.size() < src_descriptions.size()) || (strict_match && my_descriptions.size() != src_descriptions.size()))
      return false;

    std::unordered_set<std::string> my_query_strings;
    for (const auto &description : my_descriptions) my_query_strings.insert(description.definition_ast->formatWithSecretsOneLine());

    for (const auto &src_description : src_descriptions)
      if (!my_query_strings.contains(src_description.definition_ast->formatWithSecretsOneLine())) return false;
...
MergeTreeData &MergeTreeData::checkStructureAndGetMergeTreeData(const StoragePtr &source_table, const StorageMetadataPtr &src_snapshot,
                                                                const StorageMetadataPtr &my_snapshot) const {
...
std::pair<MergeTreeData::MutableDataPartPtr, scope_guard> MergeTreeData::cloneAndLoadDataPart(
    ...
    const StorageMetadataPtr &metadata_snapshot, const IDataPartStorage::ClonePartParams &params, const ReadSettings &read_settings,
    const WriteSettings &write_settings, bool must_on_same_disk) {
  ...
  if (!on_same_disk && must_on_same_disk)
    throw Exception(ErrorCodes::BAD_ARGUMENTS, "Could not clone and load part {} because disk does not belong to storage policy",
  ...
  String with_copy;
  if (params.copy_instead_of_hardlink) with_copy = " (copying data)";

  std::shared_ptr<IDataPartStorage> dst_part_storage{};
  if (on_same_disk) {
    dst_part_storage = src_part_storage->freeze(relative_data_path, tmp_dst_part_name, read_settings, write_settings,
                                                /* save_metadata_callback= */ {}, params);
  } else {
    ...
    dst_part_storage =
        src_part_storage->freezeRemote(relative_data_path, tmp_dst_part_name,
                                       /* dst_disk = */ (*reservation_on_dst_or_error)->getDisk(), read_settings, write_settings,
                                       /* save_metadata_callback= */ {}, params);
  }
  ...
  if (params.metadata_version_to_write.has_value()) {
    ...
    if ((*getSettings())[MergeTreeSetting::fsync_after_insert]) out_metadata->sync();
  }

  LOG_DEBUG(log, "Clone{} part {} to {}{}", src_flushed_tmp_part ? " flushed" : "", src_part_storage->getFullPath(),
            std::string(fs::path(dst_part_storage->getFullRootPath()) / tmp_dst_part_name), with_copy);

  auto dst_data_part = MergeTreeDataPartBuilder(*this, dst_part_name, dst_part_storage, getReadSettings()).withPartFormatFromDisk().build();
...
String MergeTreeData::getFullPathOnDisk(const DiskPtr &disk) const { return fs::path(disk->getPath()) / relative_data_path; }

DiskPtr MergeTreeData::tryGetDiskForDetachedPart(const String &part_name) const {
  const auto disks = getStoragePolicy()->getDisks();

  for (const DiskPtr &disk : disks)
    if (disk->existsDirectory(fs::path(relative_data_path) / DETACHED_DIR_NAME / part_name)) return disk;

  return nullptr;
}

DiskPtr MergeTreeData::getDiskForDetachedPart(const String &part_name) const {
  if (auto disk = tryGetDiskForDetachedPart(part_name)) return disk;
...
Strings MergeTreeData::getDataPaths() const {
  Strings res;
  auto disks = getStoragePolicy()->getDisks();
  for (const auto &disk : disks) res.push_back(getFullPathOnDisk(disk));
  return res;
}
...
void MergeTreeData::reportBrokenPart(MergeTreeData::DataPartPtr data_part) const {
  ...
  if (data_part->getDataPartStorage().isBroken()) {
    ...
    LOG_WARNING(log, "Scanning parts to recover on broken disk {}@{}.", data_part->getDataPartStorage().getDiskName(),
                data_part->getDataPartStorage().getDiskPath());

    for (const auto &part : parts) {
      if (part->getDataPartStorage().getDiskName() == data_part->getDataPartStorage().getDiskName()) broken_part_callback(part->name);
    }
...
MergeTreeData::MatcherFn MergeTreeData::getPartitionMatcher(const ASTPtr &partition_ast, ContextPtr local_context) const {
  ...
  if (format_version < MERGE_TREE_DATA_MIN_FORMAT_VERSION_WITH_CUSTOM_PARTITIONING) {
    ...
    if (const auto *partition_lit = partition_ast->as<ASTPartition &>().value->as<ASTLiteral>()) {
      id = partition_lit->value.getType() == Field::Types::UInt64 ? toString(partition_lit->value.safeGet<UInt64>())
  ...
  return [prefixed, id](const String &partition_id) {
    if (prefixed) return startsWith(partition_id, id);

    return id == partition_id;
  };
}

PartitionCommandsResultInfo MergeTreeData::freezePartition(const ASTPtr &partition_ast, const String &with_name, ContextPtr local_context,
                                                           TableLockHolder &) {
  return freezePartitionsByMatcher(getPartitionMatcher(partition_ast, local_context), with_name, local_context);
}

PartitionCommandsResultInfo MergeTreeData::freezeAll(const String &with_name, ContextPtr local_context, TableLockHolder &) {
  return freezePartitionsByMatcher([](const String &) { return true; }, with_name, local_context);
}

PartitionCommandsResultInfo MergeTreeData::freezePartitionsByMatcher(MatcherFn matcher, const String &with_name, ContextPtr local_context) {
  auto settings = getSettings();
  ...
  ThreadPool pool(CurrentMetrics::FreezePartThreads, CurrentMetrics::FreezePartThreadsActive, CurrentMetrics::FreezePartThreadsScheduled,
  ...
  for (const auto &part : data_parts) {
    if (local_context->isCurrentQueryKilled()) return result;
    if (!matcher(part->info.getPartitionId())) continue;
    ...
    runner.enqueueAndKeepTrack(
        [this, &part, &backup_path, &backup_name, &local_context, &result, &result_mutex]() {
          ...
          IDataPartStorage::ClonePartParams params{.make_source_readonly = true};

          auto new_storage =
              data_part_storage->freeze(backup_part_path, part->getDataPartStorage().getPartDirectory(), local_context->getReadSettings(),
                                        local_context->getWriteSettings(), callback, params);
...
PartitionCommandsResultInfo MergeTreeData::unfreezePartition(const ASTPtr &partition, const String &backup_name, ContextPtr local_context,
                                                             TableLockHolder &) {
  return unfreezePartitionsByMatcher(getPartitionMatcher(partition, local_context), backup_name, local_context);
}

PartitionCommandsResultInfo MergeTreeData::unfreezeAll(const String &backup_name, ContextPtr local_context, TableLockHolder &) {
...
PartitionCommandsResultInfo MergeTreeData::unfreezePartitionsByMatcher(MatcherFn matcher, const String &backup_name,
                                                                       ContextPtr local_context) {
...
bool MergeTreeData::canReplacePartition(const DataPartPtr &src_part) const {
  ...
  if (!(*settings)[MergeTreeSetting::enable_mixed_granularity_parts] || (*settings)[MergeTreeSetting::index_granularity_bytes] == 0) {
    if (!canUseAdaptiveGranularity() && src_part->index_granularity_info.mark_type.adaptive) return false;
    if (canUseAdaptiveGranularity() && !src_part->index_granularity_info.mark_type.adaptive) return false;
  }
...
void MergeTreeData::checkTableCanBeDropped(ContextPtr query_context) const {
  ...
  if (query_settings[Setting::max_table_size_to_drop].changed) {
    getContext()->checkTableCanBeDropped(table_id.database_name, table_id.table_name, getTotalActiveSizeInBytes(),
                                         query_settings[Setting::max_table_size_to_drop]);
    return;
  }
...
void MergeTreeData::writePartLog(PartLogElement::Type type, const ExecutionStatus &execution_status, UInt64 elapsed_ns,
                                 const String &new_part_name, const DataPartPtr &result_part, const DataPartsVector &source_parts,
                                 const MergeListEntry *merge_entry, std::shared_ptr<ProfileEvents::Counters::Snapshot> profile_counters,
                                 const Strings &mutation_ids) try {
  auto table_id = getStorageID();
  ...
  if (part_log_elem.event_type == PartLogElement::MERGE_PARTS || part_log_elem.event_type == PartLogElement::MERGE_PARTS_START) {
    if (merge_entry) {
  ...
  for (const auto &source_part : source_parts) part_log_elem.source_part_names.push_back(source_part->name);
...
MergeTreeData::CurrentlyMovingPartsTagger::~CurrentlyMovingPartsTagger() {
  ...
  for (auto &moving_part : parts_to_move) {
    ...
    if (!data.currently_moving_parts.contains(moving_part.part)) std::terminate();
    data.currently_moving_parts.erase(moving_part.part);
  }
}

bool MergeTreeData::scheduleDataMovingJob(BackgroundJobsAssignee &assignee) {
  if (parts_mover.moves_blocker.isCancelled()) return false;

  auto moving_tagger = selectPartsForMove();
  if (moving_tagger->parts_to_move.empty()) return false;
...
bool MergeTreeData::areBackgroundMovesNeeded() const {
  ...
  if (policy->getVolumes().size() > 1) return true;

  return policy->getVolumes().size() == 1 && policy->getVolumes()[0]->getDisks().size() > 1;
}

std::future<MovePartsOutcome> MergeTreeData::movePartsToSpace(const CurrentlyMovingPartsTaggerPtr &moving_tagger,
                                                              const ReadSettings &read_settings, const WriteSettings &write_settings,
                                                              bool async) {
  ...
  if (async) {
    ...
    if (!is_scheduled) finish_move_promise->set_value(MovePartsOutcome::CannotScheduleMove);
  } else {
    auto outcome = moveParts(moving_tagger, read_settings, write_settings, /* wait_for_move_if_zero_copy= */ true);
    finish_move_promise->set_value(outcome);
  }
...
MergeTreeData::CurrentlyMovingPartsTaggerPtr MergeTreeData::checkPartsForMove(const DataPartsVector &parts, SpacePtr space) {
  ...
  for (const auto &part : parts) {
    ...
    if (!reservation) throw Exception(ErrorCodes::NOT_ENOUGH_SPACE, "Move is not possible. Not enough space on '{}'", space->getName());
    ...
    if (currently_moving_parts.contains(part) || partIsAssignedToBackgroundOperation(part))
      throw Exception(ErrorCodes::PART_IS_TEMPORARILY_LOCKED, "Cannot move part '{}' because it's participating in background process",
                      part->name);
...
MovePartsOutcome MergeTreeData::moveParts(const CurrentlyMovingPartsTaggerPtr &moving_tagger, const ReadSettings &read_settings,
                                          const WriteSettings &write_settings, bool wait_for_move_if_zero_copy) {
  ...
  for (const auto &moving_part : moving_tagger->parts_to_move) {
    ...
    auto write_part_log = [&](const ExecutionStatus &execution_status) {
      writePartLog(PartLogElement::Type::MOVE_PART, execution_status, stopwatch.elapsed(), moving_part.part->name, cloned_part.part,
                   {moving_part.part}, nullptr, profile_events_scope.getSnapshot());
    };

    // Register in global moves list (StorageSystemMoves)
    auto moves_list_entry =
        getContext()->getMovesList().insert(getStorageID(), moving_part.part->name, moving_part.reserved_space->getDisk()->getName(),
                                            moving_part.reserved_space->getDisk()->getPath(), moving_part.part->getBytesOnDisk());
    ...
    try {
      ...
      if (supportsReplication() && disk->supportZeroCopyReplication() &&
...
void MergeTreeData::checkDropOrRenameCommandDoesntAffectInProgressMutations(
  ...
  if (!command.isDropOrRename() || unfinished_mutations.empty()) return;

  auto throw_exception = [](const std::string &mutation_name, const std::string &action_name, const std::string &entity_name,
    ...
    throw Exception(ErrorCodes::BAD_ARGUMENTS,
                    ...
                    action_name, entity_name, backQuoteIfNeed(identifier_name), mutation_name, mutation_name);
  };
  ...
  for (const auto &[mutation_name, commands] : unfinished_mutations) {
    for (const MutationCommand &mutation_command : commands) {
      if (command.type == AlterCommand::DROP_INDEX && mutation_command.index_name == command.index_name) {
      ...
      } else if (command.type == AlterCommand::DROP_PROJECTION && mutation_command.projection_name == command.projection_name) {
        ...
        if (mutation_command.column_name == command.column_name) throw_exception(mutation_name, action, "column", command.column_name);
        ...
        if (mutation_command.predicate) {
          ...
          if (identifiers.contains(command.column_name)) throw_exception(mutation_name, action, "column", command.column_name);
        }

        for (const auto &[name, expr] : mutation_command.column_to_update_expression) {
          if (name == command.column_name) throw_exception(mutation_name, action, "column", command.column_name);
        ...
        for (const auto &stats_col1 : command.statistics_columns)
          for (const auto &stats_col2 : mutation_command.statistics_columns)
            if (stats_col1 == stats_col2) throw_exception(mutation_name, "drop", "statistics", stats_col1);
      }
    }
  }
}
...
AlterConversionsPtr MergeTreeData::getAlterConversionsForPart(const MergeTreeDataPartPtr &part, const MutationsSnapshotPtr &mutations,
...
StorageMetadataPtr MergeTreeData::getPatchPartMetadata(const ColumnsDescription &patch_part_desc, const String &patch_partition_id,
                                                       ContextPtr local_context) const {
  std::lock_guard lock(patch_parts_metadata_mutex);

  auto &metadata_snapshot = patch_parts_metadata_cache[patch_partition_id];
  if (!metadata_snapshot) metadata_snapshot = DB::getPatchPartMetadata(patch_part_desc, local_context);
...
std::expected<void, PreformattedMessage> MergeTreeData::supportsLightweightUpdate() const {
  if (format_version < MERGE_TREE_DATA_MIN_FORMAT_VERSION_WITH_CUSTOM_PARTITIONING)
    return std::unexpected(PreformattedMessage::create("Lightweight updates are supported only for tables with custom partitioning"));

  if (merging_params.mode == MergingParams::Summing || merging_params.mode == MergingParams::Aggregating ||
      merging_params.mode == MergingParams::Graphite)
    return std::unexpected(PreformattedMessage::create("Lightweight updates are not supported for tables with {}MergeTree engine",
  ...
  if (!(*data_settings)[MergeTreeSetting::enable_block_number_column])
    return std::unexpected(
        PreformattedMessage::create("Lightweight updates are supported only for tables with materialized _block_number column. "
  ...
  if (!(*data_settings)[MergeTreeSetting::enable_block_offset_column])
    return std::unexpected(
        PreformattedMessage::create("Lightweight updates are supported only for tables with materialized _block_offset column. "
...
QueryPipeline MergeTreeData::updateLightweightImpl(const MutationCommands &commands, ContextPtr query_context) {
  ...
  MutationsInterpreter interpreter(shared_from_this(), getInMemoryMetadataPtr(), commands_to_run, query_context, mutation_settings);
  ...
  pipeline_builder.addTransform(std::make_shared<SimpleSquashingChunksTransform>(
      pipeline_builder.getSharedHeader(), query_context->getSettingsRef()[Setting::min_insert_block_size_rows],
  ...
  pipeline_builder.addSimpleTransform(
      [&](const SharedHeader &header) -> ProcessorPtr { return std::make_shared<AddDeduplicationInfoTransform>(header); });

  return QueryPipelineBuilder::getPipeline(std::move(pipeline_builder));
}

size_t MergeTreeData::getTotalMergesWithTTLInMergeList() const { return getContext()->getMergeList().getMergesWithTTLCount(); }
...
bool MergeTreeData::insertQueryIdOrThrowNoLock(const String &query_id, size_t max_queries) const {
  if (query_id_set.contains(query_id)) return false;
  if (query_id_set.size() >= max_queries)
    throw Exception(ErrorCodes::TOO_MANY_SIMULTANEOUS_QUERIES, "Too many simultaneous queries for table {}. Maximum is: {}", log.loadName(),
                    max_queries);
...
ReservationPtr MergeTreeData::balancedReservation(const StorageMetadataPtr &metadata_snapshot, size_t part_size, size_t max_volume_index,
                                                  const String &part_name, const MergeTreePartInfo &part_info,
                                                  MergeTreeData::DataPartsVector covered_parts,
                                                  std::optional<CurrentlySubmergingEmergingTagger> *tagger_ptr,
                                                  const IMergeTreeDataPart::TTLInfos *ttl_infos, bool is_insert) {
  ...
  if (tagger_ptr && min_bytes_to_rebalance_partition_over_jbod > 0 && part_size >= min_bytes_to_rebalance_partition_over_jbod) {
    try {
      ...
      for (const auto &disk : disks) disk_occupation.emplace(disk->getName(), 0);
      ...
      for (const auto &part : currently_submerging_big_parts) {
        if (part_info.getPartitionId() == part->info.getPartitionId()) submerging_big_parts_from_partition.insert(part->name);
      }

      {
        ...
        std::erase_if(covered_parts, [min_bytes_to_rebalance_partition_over_jbod](const auto &part) {
        ...
        for (const auto &part : covered_parts) submerging_big_parts_from_partition.insert(part->name);
      ...
      for (const auto &[name, emerging_part] : currently_emerging_big_parts) {
        ...
        if (!committed_big_parts_from_partition.contains(name) && part_info.getPartitionId() == emerging_part.partition_id) {
          auto it = disk_occupation.find(emerging_part.disk_name);
          if (it != disk_occupation.end()) {
            it->second += emerging_part.estimate_bytes;
            disk_parts_for_logging[emerging_part.disk_name].push_back(formatReadableSizeWithBinarySuffix(emerging_part.estimate_bytes) +
                                                                      " (emerging)");
          } else {
            // Part is on different volume. Ignore it.
          }
      ...
      if (!candidates.empty()) {
        ...
        if (ttl_infos)
          reserved_space = tryReserveSpacePreferringTTLRules(metadata_snapshot, part_size, *ttl_infos, time(nullptr), max_volume_index,
                                                             is_insert, getStoragePolicy()->getDiskByName(selected_disk_name));
        else
...
static void updateSerializationHintsForPart(const DataPartPtr &part, const ColumnsDescription &storage_columns,
                                            SerializationInfoByName &hints, bool remove) {
  ...
  for (const auto &[name, info] : part->getSerializationInfos()) {
    ...
    if (part_columns.tryGetPhysical(name) != storage_columns.tryGetPhysical(name)) continue;
...
void MergeTreeData::resetSerializationHints(const DataPartsLock & /*lock*/) {
  ...
  for (const auto &part : range) updateSerializationHintsForPart(part, storage_columns, serialization_hints, false);
}

template <typename AddedParts, typename RemovedParts>
void MergeTreeData::updateSerializationHints(const AddedParts &added_parts, const RemovedParts &removed_parts,
                                             const DataPartsLock & /*lock*/) {
  const auto metadata_snapshot = getInMemoryMetadataPtr();
  const auto &storage_columns = metadata_snapshot->getColumns();

  for (const auto &part : added_parts) updateSerializationHintsForPart(part, storage_columns, serialization_hints, false);

  for (const auto &part : removed_parts) updateSerializationHintsForPart(part, storage_columns, serialization_hints, true);
}
...
bool MergeTreeData::supportsTrivialCountOptimization(const StorageSnapshotPtr &storage_snapshot, ContextPtr query_context) const {
  if (hasLightweightDeletedMask()) return false;

  const auto &settings = query_context->getSettingsRef();
  if (!storage_snapshot) return !settings[Setting::apply_mutations_on_fly] && !settings[Setting::apply_patch_parts];

  const auto &snapshot_data = assert_cast<const MergeTreeData::SnapshotData &>(*storage_snapshot->data);
  const auto &mutations_snapshot = snapshot_data.mutations_snapshot;

  if (!mutations_snapshot) return !settings[Setting::apply_mutations_on_fly] && !settings[Setting::apply_patch_parts];
...
Int64 MergeTreeData::getMinMetadataVersion(const DataPartsVector &parts) {
  ...
  for (const auto &part : parts) {
    ...
    if (version == -1 || part_version < version) version = part_version;
  }
  return version;
}
...
MergeTreeSettingsPtr MergeTreeData::getSettings(ProjectionDescriptionRawPtr projection) const {
  ...
  if (projection) {
    if ((projection->index_granularity && (*projection->index_granularity != (*data_settings)[MergeTreeSetting::index_granularity])) ||
      ...
      if (projection->index_granularity) (*new_data_settings)[MergeTreeSetting::index_granularity] = *projection->index_granularity;
...
{
  if (bypass_metadata_cache) return IStorage::getInMemoryMetadataPtr(bypass_metadata_cache);
  ...
  if (!query_metadata_cache) return IStorage::getInMemoryMetadataPtr(bypass_metadata_cache);
...
StorageSnapshotPtr MergeTreeData::createStorageSnapshot(const StorageMetadataPtr &metadata_snapshot, ContextPtr query_context,
                                                        bool without_data) const {
  if (without_data) return std::make_shared<StorageSnapshot>(*this, metadata_snapshot, std::make_unique<SnapshotData>());
...
StorageSnapshotPtr MergeTreeData::getStorageSnapshot(const StorageMetadataPtr &metadata_snapshot, ContextPtr query_context) const {
  ...
  auto query_metadata_cache = query_context->getQueryMetadataCache();
  if (!query_metadata_cache) return createStorageSnapshot(metadata_snapshot, query_context, false);
...
StorageSnapshotPtr MergeTreeData::getStorageSnapshotWithoutData(const StorageMetadataPtr &metadata_snapshot,
...
std::pair<MergeTreeData::MutableDataPartPtr, scope_guard> MergeTreeData::createEmptyPart(MergeTreePartInfo &new_part_info,
  ...
  if ((*settings)[MergeTreeSetting::assign_part_uuids]) new_data_part->uuid = UUIDHelpers::generateV4();
  ...
  if ((*getSettings())[MergeTreeSetting::fsync_part_directory]) sync_guard = new_data_part_storage->getDirectorySyncGuard();
  ...
  MergedBlockOutputStream out(new_data_part, getSettings(), metadata_snapshot, columns,
                              index_factory.getMany(metadata_snapshot->getSecondaryIndices()), compression_codec,
                              std::make_shared<MergeTreeIndexGranularityAdaptive>(), txn ? txn->tid : Tx::PrehistoricTID,
...
size_t MergeTreeData::unloadPrimaryKeysAndClearCachesOfOutdatedParts() {
...

  {
    ...
    for (const auto &part : parts_range) {
      ...
      if (!part->isIndexLoaded() && !part->mayStoreDataInCaches()) continue;
  ...
  if (!parts_to_clear.empty()) LOG_TRACE(log, "Unloaded primary key for outdated parts {}", fmt::join(getPartsNames(parts_to_clear), ", "));
...
void MergeTreeData::verifySortingKey(const KeyDescription &sorting_key) {
  ...
  for (const auto &data_type : sorting_key.data_types) {
    if (dynamic_cast<const DataTypeCustomSimpleAggregateFunction *>(data_type->getCustomName()))
      throw Exception(ErrorCodes::DATA_TYPE_CANNOT_BE_USED_IN_KEY, "Column with type {} is not allowed in key expression",
                      data_type->getCustomName()->getName());
  }
}

size_t MergeTreeData::NamesAndTypesListHash::operator()(const NamesAndTypesList &list) const noexcept {
  size_t hash = 0;
  for (const auto &name_type : list) boost::hash_combine(hash, StringViewHash{}(name_type.name));
  return hash;
}
...
MergeTreeData::ColumnsDescriptionCache MergeTreeData::getColumnsDescriptionForColumns(const NamesAndTypesList &columns) const {
  ...
  if (auto it = columns_descriptions_cache.find(columns); it != columns_descriptions_cache.end()) return it->second;
  ...
  if (*cache.with_collected_nested == *cache.original) cache.with_collected_nested = cache.original;
...
String replaceFileNameToHashIfNeeded(const String &file_name, const MergeTreeSettings &storage_settings,
                                     const IDataPartStorage *data_part_storage) {
  if ((data_part_storage && data_part_storage->isCaseInsensitive()) ||
      (storage_settings[MergeTreeSetting::replace_long_file_name_to_hash] &&
       file_name.size() > storage_settings[MergeTreeSetting::max_file_name_length]))
