...
class StorageFromMergeTreeDataPart final : public IStorage {
 public:
  ...
  StorageFromMergeTreeDataPart(const MergeTreeData::DataPartPtr& part_, const MergeTreeData::MutationsSnapshotPtr& mutations_snapshot_)
  ...
  String getPartitionIDFromQuery(const ASTPtr& ast, ContextPtr context) const { return storage.getPartitionIDFromQuery(ast, context); }

  bool materializeTTLRecalculateOnly() const;

  bool hasLightweightDeletedMask() const override { return !parts.empty() && parts.front().data_part->hasLightweightDelete(); }

  bool supportsLightweightDelete() const override { return !parts.empty() && parts.front().data_part->supportLightweightDeleteMutate(); }
