...
class IMergeTreeDataPart : public std::enable_shared_from_this<IMergeTreeDataPart>, public DataPartStorageHolder {
 public:
  ...
  IMergeTreeDataPart(const MergeTreeData& storage_, const MergeTreeSettings& storage_settings, const String& name_,
  ...
  /// otherwise, if the partition key includes dateTime column (also a common case), this function will return min and max values for that
  /// column.
  ...
  bool checkState(const std::initializer_list<MergeTreeDataPartState>& affordable_states) const {
    ...
    for (auto affordable_state : affordable_states) {
      if (current_state == affordable_state) return true;
    }
    return false;
  }
  ...
  struct MinMaxIndex {
   ...
   public:
    ...
    MinMaxIndex(DayNum min_date, DayNum max_date) : hyperrectangle(1, Range(min_date, true, max_date, true)), initialized(true) {}

    void load(const IMergeTreeDataPart& part);

    using WrittenFiles = std::vector<std::unique_ptr<WriteBufferFromFileBase>>;

    [[nodiscard]] WrittenFiles store(StorageMetadataPtr metadata_snapshot, IDataPartStorage& part_storage, Checksums& checksums,
                                     const MergeTreeSettingsPtr& storage_settings) const;
    [[nodiscard]] WrittenFiles store(const Names& column_names, const DataTypes& data_types, IDataPartStorage& part_storage,
                                     Checksums& checksums, const MergeTreeSettingsPtr& storage_settings) const;
  ...
  MutableDataPartStoragePtr makeCloneOnDisk(const DiskPtr& disk, const String& directory_name, const ReadSettings& read_settings,
                                            const WriteSettings& write_settings, const std::function<void()>& cancellation_hook) const;
  ...
  MergeTreeDataPartBuilder getProjectionPartBuilder(const String& projection_name, ProjectionDescriptionRawPtr projection,
                                                    bool is_temp_projection = false);
  ...
  static std::optional<String> getStreamNameOrHash(const String& name, const String& extension,
                                                   const IMergeTreeDataPart::Checksums& checksums);

  static std::optional<String> getStreamNameOrHash(const String& name, const String& extension, const IDataPartStorage& storage_);

  static std::optional<String> getStreamNameForColumn(const String& column_name, const ISerialization::SubstreamPath& substream_path,
                                                      const String& extension, const Checksums& checksums_,
                                                      const MergeTreeSettingsPtr& settings);

  static std::optional<String> getStreamNameForColumn(const NameAndTypePair& column, const ISerialization::SubstreamPath& substream_path,
                                                      const String& extension, const Checksums& checksums_,
                                                      const MergeTreeSettingsPtr& settings);
