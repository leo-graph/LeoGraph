...
struct MergeListElement : boost::noncopyable {
  ...
  MergeListElement(const StorageID& table_id_, FutureMergedMutatedPartPtr future_part, const ContextPtr& context);
...
class MergeList final : public BackgroundProcessList<MergeListElement, MergeInfo> {
 ...
 public:
  MergeList() : Parent(CurrentMetrics::Merge) {}
  ...
  void cancelPartMutations(const StorageID& table_id, const String& partition_id, Int64 mutation_version) {
    ...
    for (auto& merge_element : entries) {
      if ((partition_id.empty() || merge_element.partition_id == partition_id) && merge_element.table_id == table_id &&
          merge_element.source_data_version < mutation_version && merge_element.result_part_info.getDataVersion() >= mutation_version)
  ...
  void cancelInPartition(const StorageID& table_id, const String& partition_id, Int64 delimiting_block_number) {
    std::lock_guard lock{mutex};
    for (auto& merge_element : entries) {
      if (merge_element.table_id == table_id && merge_element.partition_id == partition_id &&
  ...
  void bookMergeWithTTL() { ++merges_with_ttl_counter; }

  void cancelMergeWithTTL() { --merges_with_ttl_counter; }

  size_t getMergesWithTTLCount() const { return merges_with_ttl_counter; }
};
