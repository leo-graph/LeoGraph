...
MergeTreeSkipIndexReader::MergeTreeSkipIndexReader(UsefulSkipIndexes skip_indexes_,
                                                   std::optional<KeyCondition> &key_condition_rpn_template_, bool use_for_disjunctions_,
                                                   MarkCachePtr mark_cache_, UncompressedCachePtr uncompressed_cache_,
                                                   VectorSimilarityIndexCachePtr vector_similarity_index_cache_,
                                                   MergeTreeReaderSettings reader_settings_, LoggerPtr log_)
...
SkipIndexReadResultPtr MergeTreeSkipIndexReader::read(const RangesInDataPart &part) {
  ...
  if (use_for_disjunctions)
    partial_eval_results.resize(part.data_part->index_granularity->getMarksCountWithoutFinal() *
                                    MergeTreeDataSelectExecutor::MAX_BITS_FOR_PARTIAL_DISJUNCTION_RESULT,
                                true);
  for (const auto &index_and_condition : skip_indexes.useful_indices) {
    ...
    ranges = MergeTreeDataSelectExecutor::filterMarksUsingIndex(
                 index_and_condition.index, index_and_condition.condition, key_condition_rpn_template, part.data_part, ranges,
                 part.read_hints, reader_settings, mark_cache.get(), uncompressed_cache.get(), vector_similarity_index_cache.get(),
                 use_for_disjunctions, partial_eval_results, log)
                 .first;
  ...
  if (use_for_disjunctions) {
    ranges = MergeTreeDataSelectExecutor::mergePartialResultsForDisjunctions(part.data_part, ranges, key_condition_rpn_template.value(),
                                                                             partial_eval_results, reader_settings, log);

    LOG_DEBUG(log, "Final set of granules after AND/OR processing : {} out of {} in part {}", ranges.getNumberOfMarks(), total_granules,
              part.data_part->name);
    total_granules = ranges.getNumberOfMarks();
  }
  ...
  for (const auto &range : ranges) {
    for (auto i = range.begin; i < range.end; ++i) (*res).granules_selected[i] = true;
  }
  ...
  if (skip_indexes.skip_index_for_top_k_filtering && skip_indexes.threshold_tracker) {
    res->min_max_index_for_top_k = MergeTreeDataSelectExecutor::getMinMaxIndexGranules(
        part.data_part, skip_indexes.skip_index_for_top_k_filtering, ranges, skip_indexes.threshold_tracker->getDirection(),
        true, /*access_by_mark*/
...
ProjectionIndexBitmap::ProjectionIndexBitmap(BitmapType bitmap_type) : type(bitmap_type) {
...
ProjectionIndexBitmap::~ProjectionIndexBitmap() {
  if (type == BitmapType::Bitmap32) {
    if (data.bitmap32) roaring_bitmap_free(data.bitmap32);
  } else {
    if (data.bitmap64) roaring64_bitmap_free(data.bitmap64);
  }
}

ProjectionIndexBitmapPtr ProjectionIndexBitmap::create32() { return std::make_shared<ProjectionIndexBitmap>(BitmapType::Bitmap32); }

ProjectionIndexBitmapPtr ProjectionIndexBitmap::create64() { return std::make_shared<ProjectionIndexBitmap>(BitmapType::Bitmap64); }
...
bool ProjectionIndexBitmap::contains(std::type_identity_t<Offset> value) {
  static_assert(std::is_same_v<Offset, UInt32> || std::is_same_v<Offset, UInt64>,
...
void ProjectionIndexBitmap::addBulk(const std::type_identity_t<Offset> *values, size_t size) {
  ...
  if constexpr (std::is_same_v<Offset, UInt32>) {
    ...
    for (size_t i = 0; i < size; ++i) roaring_bitmap_add_bulk(data.bitmap32, &context, values[i]);
  } else {
    chassert(type == BitmapType::Bitmap64);
    roaring::api::roaring64_bulk_context_t context{};
    for (size_t i = 0; i < size; ++i) roaring64_bitmap_add_bulk(data.bitmap64, &context, values[i]);
  }
}
...
bool ProjectionIndexBitmap::rangeAllZero(size_t begin, size_t end) const {
  if (type == BitmapType::Bitmap32) {
    ...
    if (!roaring_uint32_iterator_move_equalorlarger(&it, static_cast<UInt32>(begin))) return true;
    ...
    if (it == nullptr) throw Exception(ErrorCodes::MEMORY_LIMIT_EXCEEDED, "Failed to allocate roaring64 iterator");
...
bool ProjectionIndexBitmap::appendToFilter(PaddedPODArray<UInt8> &filter, size_t starting_row, size_t num_rows) const {
  ...
  if (type == BitmapType::Bitmap32) {
    roaring::api::roaring_uint32_iterator_t it;
    roaring_iterator_init(data.bitmap32, &it);
    if (!roaring_uint32_iterator_move_equalorlarger(&it, static_cast<UInt32>(starting_row))) return false;
    ...
    while (val < ending_row) {
      ...
      if (!roaring::api::roaring64_iterator_advance(it)) break;
...
SingleProjectionIndexReader::SingleProjectionIndexReader(std::shared_ptr<MergeTreeReadPoolProjectionIndex> pool,
                                                         PrewhereInfoPtr prewhere_info, const ExpressionActionsSettings &actions_settings,
                                                         const MergeTreeReaderSettings &reader_settings)
    : projection_index_read_pool(std::move(pool)),
      processor(std::make_unique<MergeTreeSelectProcessor>(std::static_pointer_cast<IMergeTreeReadPool>(projection_index_read_pool),
                                                           std::make_unique<MergeTreeProjectionIndexSelectAlgorithm>(),
                                                           nullptr /*row_level_filter*/, std::move(prewhere_info),
                                                           IndexReadTasks{} /*index_read_tasks*/, actions_settings, reader_settings)) {}
...
ProjectionIndexBitmapPtr SingleProjectionIndexReader::read(const RangesInDataPart &ranges) {
      ...
      if (chunk.chunk.getNumRows() > 0) {
        ...
        auto add_offsets = [&]<typename Offset>(Offset) {
          if (ranges.parent_ranges.isContiguousFullRange()) {
            for (auto offset : offsets.getData()) res->add<Offset>(static_cast<Offset>(offset));
          } else {
            for (auto offset : offsets.getData()) {
              if (ranges.parent_ranges.contains(offset)) res->add<Offset>(static_cast<Offset>(offset));
            }
          }
        };
  ...
  if (processor->is_cancelled) res = nullptr;

  return res;
}

void SingleProjectionIndexReader::cancel() noexcept { processor->cancel(); }
...
ProjectionIndexBitmapPtr MergeTreeProjectionIndexReader::read(const RangesInDataParts &projection_parts) {
  ...
  if (!bitmaps.empty()) {
    ...
    for (size_t i = 1; i < bitmaps.size(); ++i) projection_index_bitmap->intersectWith(*bitmaps[i]);
  }
...
void MergeTreeProjectionIndexReader::cancel() noexcept {
  for (auto &&[_, reader] : projection_index_readers) reader.cancel();
}
...
MergeTreeIndexReadResultPool::MergeTreeIndexReadResultPool(MergeTreeSkipIndexReaderPtr skip_index_reader_,
    ...
    : skip_index_reader(std::move(skip_index_reader_)), projection_index_reader(std::move(projection_index_reader_)) {
    ...
    try {
      ...
      if (projection_index_reader) {
        ...
        if (projection_index_res) {
          if (!res) res = std::make_shared<MergeTreeIndexReadResult>();
...
void MergeTreeIndexReadResultPool::cancel() noexcept {
  if (skip_index_reader) skip_index_reader->cancel();

  if (projection_index_reader) projection_index_reader->cancel();
}
