...
bool canApplyPatchInplace(const IColumn &column) {
  if (const auto *nullable = typeid_cast<const ColumnNullable *>(&column)) return nullable->getNestedColumn().isFixedAndContiguous();
...
IColumn::Versions &addDataVersionForColumn(Block &block, const String &column_name, UInt64 num_rows, UInt64 data_version) {
  ...
  if (block.has(data_version_name)) return getColumnUInt64Data(block, data_version_name);
...
struct CombinedPatchBuilder {
 public:
  explicit CombinedPatchBuilder(const PatchesToApply &patches_) : patches(patches_) { build(); }
 ...
 private:
  ...
  ALWAYS_INLINE UInt64 getResultRowIndex(UInt64 patch_idx, UInt64 row_idx) const { return patches[patch_idx]->result_row_indices[row_idx]; }

  ALWAYS_INLINE UInt64 getPatchRowIndex(UInt64 patch_idx, UInt64 row_idx) const { return patches[patch_idx]->patch_row_indices[row_idx]; }
...
void CombinedPatchBuilder::build() {
  ...
  for (size_t i = 0; i < patches.size(); ++i) {
    if (patches[i]->getNumRows() > 0) heap.push_back(i);
  }

  for (size_t i = 0; i < all_patch_blocks.size(); ++i) versions[i] = &getColumnUInt64Data(all_patch_blocks[i], PartDataVersionColumn::name);
  ...
  auto get_row_op = [&](UInt64 patch_idx, UInt64 row_idx) {
    ...
    if (dst_row_indices.empty()) return RowOp::Add;
    ...
    if (current_result_row != last_result_row) return RowOp::Add;
  ...
  auto greater = [&](UInt64 lhs, UInt64 rhs) { return getResultRowIndex(lhs, cursors[lhs]) > getResultRowIndex(rhs, cursors[rhs]); };
...
IColumn::Patch CombinedPatchBuilder::createPatchForColumn(const String &column_name, IColumn::Versions &dst_versions) const {
  ...
  for (const auto &patch_block : all_patch_blocks) {
    ...
    if (!patch_column) throw Exception(ErrorCodes::LOGICAL_ERROR, "Column {} has null data in patch block", column_name);
...
Block getUpdatedHeader(const PatchesToApply &patches, const NameSet &updated_columns) {
  ...
  for (const auto &patch : patches) {
    if (patch->patch_blocks.empty()) continue;
    ...
    for (const auto &column : patch->patch_blocks[0]) {
      ...
      if (!updated_columns.contains(column.name) || !column.column) header.erase(column.name);
    }

    headers.push_back(std::move(header));
  }

  for (size_t i = 1; i < headers.size(); ++i) assertCompatibleHeader(headers[i], headers[0], "patch parts");
  ...
  for (const auto &patch : patches) {
    ...
    if (patches.size() > 1) {
      for (const auto &column : patch->patch_blocks.front()) {
        if (!isPatchPartSystemColumn(column.name) && column.column && !canApplyPatchInplace(*column.column)) return false;
      }
    }
  }
...
void applyPatchesToBlockRaw(Block &result_block, Block &versions_block, const PatchesToApply &patches, const Block &updated_header,
                            UInt64 source_data_version) {
  if (patches.empty()) return;

  for (auto &result_column : result_block) {
    if (!updated_header.has(result_column.name)) continue;
...
void applyPatchesToBlockCombined(Block &result_block, Block &versions_block, const PatchesToApply &patches, const Block &updated_header,
                                 UInt64 source_data_version) {
  if (patches.empty()) return;
...
PatchToApplyPtr applyPatchMerge(const Block &result_block, const Block &patch_block, const PatchPartInfoForReader &patch) {
  if (patch.source_parts.size() != 1)
    throw Exception(ErrorCodes::LOGICAL_ERROR, "Applying patch parts with mode {} requires only one part, got: {}", patch.mode,
                    patch.source_parts.size());
  ...
  if (num_rows == 0 || patch_rows == 0) return patch_to_apply;
  ...
  auto [patch_begin, patch_end] =
      getPartNameOffsetRange(patch_name_column, patch_offset_data, patch.source_parts.front(), first_result_offset, last_result_offset);
...
PatchToApplyPtr applyPatchJoin(const Block &result_block, const PatchJoinCache::Entry &join_entry) {
  ...
  for (const auto &block : join_entry.blocks) {
    if (block->rows() != 0) patch_to_apply->patch_blocks.push_back(*block);
  }

  size_t num_rows = result_block.rows();
  if (num_rows == 0 || join_entry.hash_map.empty()) return patch_to_apply;
  ...
  for (size_t row = 0; row < num_rows; ++row) {
    if (result_block_number[row] < join_entry.min_block || result_block_number[row] > join_entry.max_block) continue;
...
#ifdef DEBUG_OR_SANITIZER_BUILD
    {
      ...
      if (it != last_offset_by_block_number.end() && it->second >= result_block_offset[row])
        throw Exception(ErrorCodes::LOGICAL_ERROR, "Block offsets ({}, {}) are not sorted within block number {}", it->second,
                        result_block_offset[row], result_block_number[row]);
...
void applyPatchesToBlock(Block &result_block, Block &versions_block, const PatchesToApply &patches, const Names &updated_columns,
                         UInt64 source_data_version) {
  if (patches.empty()) return;
