...
zkutil::EphemeralNodeHolderPtr getLockForSyncMode(const ContextPtr &context, const zkutil::ZooKeeperPtr &zookeeper,
  ...
  for (size_t i = 0; i < max_tries; ++i) {
    ...
    if (code != Coordination::Error::ZNODEEXISTS) throw zkutil::KeeperException::fromPath(code, lock_path);

    if (zookeeper->exists(lock_path, nullptr, lock_event)) lock_event->tryWait(lock_acquire_timeout);
  }

  throw Exception(ErrorCodes::TIMEOUT_EXCEEDED, "Failed to get lock with {} tries for lightwegiht update in sync mode", max_tries);
}

zkutil::EphemeralNodeHolderPtr getLockForAutoMode(const MutationCommands &commands, const ContextPtr &context,
  ...
  for (size_t num_try = 0; num_try < max_tries; ++num_try) {
    ProfileEvents::increment(ProfileEvents::PatchesAcquireLockTries);
    LOG_TRACE(getLogger("getLockForAutoMode"), "Trying to get lock (try: {}, path: {}) for lightweight update", num_try,
    ...
    for (const auto &id : in_progress_ids) multiget_paths.push_back(in_progress_path / id);
    ...
    for (size_t i = 0; i < contents.size(); ++i) {
      ...
      if (contents[i].error == Coordination::Error::ZNONODE) continue;

      if (contents[i].error != Coordination::Error::ZOK) throw zkutil::KeeperException::fromPath(contents[i].error, multiget_paths[i]);
    ...
    if (has_dependency_in_progress) {
      LOG_TRACE(getLogger("getLockForAutoMode"),
                "Columns required for lightweight update are being updated by another query, will try one more time");
...
bool UpdateAffectedColumns::hasConflict(const UpdateAffectedColumns &other) const {
  for (const auto &column : updated) {
    if (other.used.contains(column)) return true;
  }

  for (const auto &column : other.updated) {
    if (used.contains(column)) return true;
  }
...
void UpdateAffectedColumnsWithCounters::add(const UpdateAffectedColumns &other) {
  for (const auto &column : other.used) ++used[column];

  for (const auto &column : other.updated) ++updated[column];
}
...
void PlainLightweightUpdatesSync::lockColumns(const UpdateAffectedColumns &affected_columns, size_t timeout_ms) {
  ...
  if (!res) throw Exception(ErrorCodes::TIMEOUT_EXCEEDED, "Failed to get lock in {} ms for lightwegiht update with auto mode", timeout_ms);
...
PlainLightweightUpdateLock::~PlainLightweightUpdateLock() {
  if (lightweight_updates_sync) lightweight_updates_sync->releaseColumns(affected_columns);
}
...
zkutil::EphemeralNodeHolderPtr getLockForLightweightUpdateInKeeper(const MutationCommands &commands, const ContextPtr &context,
  ...
  if (parallel_mode == UpdateParallelMode::SYNC) return getLockForSyncMode(context, zookeeper, zookeeper_path);

  if (parallel_mode == UpdateParallelMode::AUTO) return getLockForAutoMode(commands, context, zookeeper, zookeeper_path);
