...
void PatchJoinCache::init(const RangesInPatchParts &ranges_in_patches) {
  ...
  for (const auto &[patch_name, ranges] : all_ranges) {
    ...
    for (size_t i = 0; i < current_buckets; ++i) entries.push_back(std::make_shared<PatchJoinCache::Entry>());
...
PatchJoinCache::PatchStatsEntryPtr PatchJoinCache::getStatsEntry(const DataPartPtr &patch_part, const MergeTreeReaderSettings &settings) {
  ...
  if (stats_entry->initialized) return stats_entry;
  ...
  for (const auto &[range, _] : it->second) all_patch_ranges.push_back(range);
...
PatchJoinCache::PatchStatsEntryPtr PatchJoinCache::getOrCreatePatchStats(const String &patch_name) {
  ...
  if (!entry) entry = std::make_shared<PatchStatsEntry>();
  return entry;
}
...
std::pair<PatchJoinCache::Entries, std::vector<MarkRanges>> PatchJoinCache::getEntriesAndRanges(const String &patch_name,
...
std::vector<std::shared_future<void>> PatchJoinCache::Entry::addRangesAsync(const MarkRanges &ranges, Reader reader) {
  ...
  if (!ranges_to_read.empty()) {
    try {
      ...
      for (const auto &promise : promises) promise->set_value();
    } catch (...) {
      for (const auto &promise : promises) promise->set_exception(std::current_exception());
      throw;
    }
  }
...
void PatchJoinCache::Entry::addBlock(Block read_block) {
  ...
  for (const auto &block : blocks) assertCompatibleHeader(*block_with_data, *block, "patch join cache");
#endif
  ...
  for (size_t i = 0; i < num_read_rows; ++i) {
    ...
    if (inserted) {
      ...
      if (current_version > existing_version) it->second = std::make_pair(static_cast<UInt32>(new_block_idx), static_cast<UInt32>(i));
    }
  }
}
