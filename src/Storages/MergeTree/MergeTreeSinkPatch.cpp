...
MergeTreeSinkPatch::MergeTreeSinkPatch(StorageMergeTree& storage_, StorageMetadataPtr metadata_snapshot_,
                                       PlainLightweightUpdateHolder update_holder_, ContextPtr context_)
    : MergeTreeSink(storage_, std::move(metadata_snapshot_),
                    /*max_parts_per_block=*/0, std::move(context_)),
...
void MergeTreeSinkPatch::finishDelayedChunk() {
  ...
  for (auto& partition : delayed_chunk->partitions) {
    ...
    const auto deduplication_hashes =
        partition.deduplication_info->getDeduplicationHashes(part->info.getPartitionId(), storage.getDeduplicationLog() != nullptr);
