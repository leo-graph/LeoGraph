...
struct AsyncBlockIDsCache<TStorage>::Cache : public std::unordered_set<String> {
  ...
  explicit Cache(std::unordered_set<String>&& set_)
      : std::unordered_set<String>(std::move(set_)), cache_size_increment(CurrentMetrics::AsyncInsertCacheSize, size()) {}
};

template <typename TStorage>
void AsyncBlockIDsCache<TStorage>::update() try {
...
AsyncBlockIDsCache<TStorage>::AsyncBlockIDsCache(TStorage& storage_, const std::string& dir_name)
    : storage(storage_),
      update_wait(
          std::chrono::milliseconds((*storage.getSettings())[MergeTreeSetting::async_block_ids_cache_update_wait_ms].totalMilliseconds())),
...
void AsyncBlockIDsCache<TStorage>::start() {
  if ((*storage.getSettings())[MergeTreeSetting::use_async_block_ids_cache]) task->activateAndSchedule();
}
...
void AsyncBlockIDsCache<TStorage>::triggerCacheUpdate() {
  ...
  if (!task->schedule()) LOG_TRACE(log, "Task is already scheduled, will wait for update for {}ms", update_wait.count());
}
...
template <typename TStorage>
std::vector<DeduplicationHash> AsyncBlockIDsCache<TStorage>::detectConflicts(const std::vector<DeduplicationHash>& deduplication_hashes,
                                                                             UInt64& last_version) {
  if (!(*storage.getSettings())[MergeTreeSetting::use_async_block_ids_cache]) return {};
  ...
  CachePtr cur_cache;
  {
    ...
    if (version == last_version) LOG_INFO(log, "Read cache with a old version {}", last_version);

    cur_cache = cache_ptr;
    last_version = version;
  }

  if (cur_cache == nullptr) return {};
