...
StorageFromMergeTreeProjection::StorageFromMergeTreeProjection(StorageID storage_id_, StoragePtr parent_storage_,
                                                               StorageMetadataPtr parent_metadata_, ProjectionDescriptionRawPtr projection_)
...
void StorageFromMergeTreeProjection::read(QueryPlan &query_plan, const Names &column_names, const StorageSnapshotPtr &storage_snapshot,
  ...
  auto step = MergeTreeDataSelectExecutor(merge_tree, projection)
                  .readFromParts(std::make_shared<RangesInDataParts>(projection_parts), snapshot_data.mutations_snapshot->cloneEmpty(),
                                 column_names, storage_snapshot, query_info, context, max_block_size, num_streams);
...
StorageSnapshotPtr StorageFromMergeTreeProjection::getStorageSnapshot(const StorageMetadataPtr &metadata_snapshot,
