...
class ParallelReadingExtension {
 public:
  ParallelReadingExtension(MergeTreeAllRangesCallback all_callback_, MergeTreeReadTaskCallback callback_, size_t number_of_current_replica_,
  ...
  std::optional<ParallelReadResponse> sendReadRequest(CoordinationMode mode, size_t min_number_of_marks,
...
struct MergeTreeIndexBuildContext {
  ...
  MergeTreeIndexBuildContext(RangesByIndex read_ranges_, ProjectionRangesByIndex projection_read_ranges_,
                             MergeTreeIndexReadResultPoolPtr index_reader_pool_, PartRemainingMarks part_remaining_marks_);
...
class MergeTreeSelectProcessor : private boost::noncopyable {
 public:
  MergeTreeSelectProcessor(MergeTreeReadPoolPtr pool_, MergeTreeSelectAlgorithmPtr algorithm_, const FilterDAGInfoPtr& row_level_filter_,
                           const PrewhereInfoPtr& prewhere_info_, const IndexReadTasks& index_read_tasks_,
  ...
  static PrewhereExprInfo getPrewhereActions(const FilterDAGInfoPtr& row_level_filter, const PrewhereInfoPtr& prewhere_info,
                                             const IndexReadTasks& index_read_tasks, const ExpressionActionsSettings& actions_settings,
                                             bool enable_multiple_prewhere_read_steps, bool force_short_circuit_execution);
