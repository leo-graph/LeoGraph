...
class IStorageURLBase : public IStorage {
 public:
  ...
  static ColumnsDescription getTableStructureFromData(const String &format, const String &uri, CompressionMethod compression_method,
                                                      const HTTPHeaderEntries &headers,
                                                      const std::optional<FormatSettings> &format_settings, const ContextPtr &context);

  static std::pair<ColumnsDescription, String> getTableStructureAndFormatFromData(const String &uri, CompressionMethod compression_method,
                                                                                  const HTTPHeaderEntries &headers,
  ...
  static std::optional<time_t> tryGetLastModificationTime(const String &url, const HTTPHeaderEntries &headers,
                                                          const Poco::Net::HTTPBasicCredentials &credentials, const ContextPtr &context);

 protected:
  friend class ReadFromURL;

  IStorageURLBase(const String &uri_, const ContextPtr &context_, const StorageID &id_, const String &format_name_,
                  ...
                  const HTTPHeaderEntries &headers_ = {}, const String &method_ = "", ASTPtr partition_by = nullptr,
  ...
  virtual std::vector<std::pair<std::string, std::string>> getReadURIParams(const Names &column_names,
                                                                            ...
                                                                            const SelectQueryInfo &query_info, const ContextPtr &context,
 ...
 private:
  static std::pair<ColumnsDescription, String> getTableStructureAndFormatFromDataImpl(std::optional<String> format, const String &uri,
                                                                                      CompressionMethod compression_method,
                                                                                      const HTTPHeaderEntries &headers,
                                                                                      const std::optional<FormatSettings> &format_settings,
                                                                                      const ContextPtr &context);
...
class StorageURLSource : public ISource, WithContext {
 ...
 public:
  class DisclosedGlobIterator {
   public:
    DisclosedGlobIterator(const String &uri_, size_t max_addresses, const ActionsDAG::Node *predicate,
                          const NamesAndTypesList &virtual_columns, const NamesAndTypesList &hive_columns, const ContextPtr &context);
  ...
  StorageURLSource(const ReadFromFormatInfo &info, std::shared_ptr<IteratorWrapper> uri_iterator_, const std::string &http_method,
                   std::function<void(std::ostream &)> callback, const String &format, const std::optional<FormatSettings> &format_settings,
                   String name_, const ContextPtr &context, UInt64 max_block_size, const ConnectionTimeouts &timeouts,
                   CompressionMethod compression_method, FormatParserSharedResourcesPtr parser_shared_resources_,
                   FormatFilterInfoPtr format_filter_info_, const HTTPHeaderEntries &headers_ = {}, const URIParams &params = {},
                   bool glob_url = false, bool need_only_count_ = false);
  ...
  static std::pair<Poco::URI, std::unique_ptr<ReadWriteBufferFromHTTP>> getFirstAvailableURIAndReadBuffer(
      std::vector<String>::const_iterator &option, const std::vector<String>::const_iterator &end, ContextPtr context,
...
class StorageURLSink : public SinkToStorage {
 public:
  StorageURLSink(String uri_, String format_, const std::optional<FormatSettings> &format_settings, const Block &sample_block,
                 const ContextPtr &context, const ConnectionTimeouts &timeouts, const CompressionMethod &compression_method,
                 HTTPHeaderEntries headers = {}, String method = Poco::Net::HTTPRequest::HTTP_POST);
...
class StorageURL : public IStorageURLBase {
 public:
  StorageURL(const String &uri_, const StorageID &table_id_, const String &format_name_,
             const std::optional<FormatSettings> &format_settings_, const ColumnsDescription &columns_,
             const ConstraintsDescription &constraints_, const String &comment, const ContextPtr &context_,
             const String &compression_method_, const HTTPHeaderEntries &headers_ = {}, const String &method_ = "",
             ASTPtr partition_by_ = nullptr, bool distributed_processing_ = false);
...
class StorageURLWithFailover final : public StorageURL {
 public:
  StorageURLWithFailover(const std::vector<String> &uri_options_, const StorageID &table_id_, const String &format_name_,
                         const std::optional<FormatSettings> &format_settings_, const ColumnsDescription &columns_,
                         const ConstraintsDescription &constraints_, const ContextPtr &context_, const String &compression_method_);
