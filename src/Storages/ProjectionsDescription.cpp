...
bool ProjectionDescription::isPrimaryKeyColumnPossiblyWrappedInFunctions(const ASTPtr &node) const {
  ...
  for (const auto &key_name : metadata->getPrimaryKeyColumns())
    if (column_name == key_name) return true;

  if (const auto *func = node->as<ASTFunction>())
    if (func->arguments->children.size() == 1) return isPrimaryKeyColumnPossiblyWrappedInFunctions(func->arguments->children.front());
...
ProjectionDescription ProjectionDescription::clone() const {
  ...
  if (definition_ast) other.definition_ast = definition_ast->clone();
  if (query_ast) other.query_ast = query_ast->clone();
...
ProjectionsDescription ProjectionsDescription::clone() const {
  ...
  for (const auto &projection : projections) other.add(projection.clone());
...
class StorageProjectionSource final : public IStorage {
 public:
  explicit StorageProjectionSource(ColumnsDescription columns_description) : IStorage({"_", "_"}) {
  ...
  Pipe read(const Names &column_names, const StorageSnapshotPtr &storage_snapshot, SelectQueryInfo &, ContextPtr /*context*/,
            QueryProcessingStage::Enum /*processing_stage*/, size_t /*max_block_size*/, size_t /*num_streams*/) override {
...
class ProjectionDataSource : public ISource {
 public:
  explicit ProjectionDataSource(SharedHeader block)
      : ISource(std::make_shared<const Block>(block->cloneEmpty())), chunk(block->getColumns(), block->rows()) {}
...
class ProjectionDataSink : public ISink {
 public:
  ProjectionDataSink(SharedHeader header, size_t max_rows_allowed_) : ISink(std::move(header)), max_rows_allowed(max_rows_allowed_) {}
...
ProjectionDescription ProjectionDescription::getProjectionFromAST(const ASTPtr &definition_ast, const ColumnsDescription &columns,
  ...
  if (projection_definition->name.empty()) throw Exception(ErrorCodes::INCORRECT_QUERY, "Projection must have name in definition.");
  ...
  if (projection_definition->index) {
    ...
    if (projection_definition->with_settings) result.loadSettings(projection_definition->with_settings->changes);
    return result;
  }
...
void ProjectionDescription::fillProjectionDescriptionByQuery(ProjectionDescription &result, const ASTProjectionSelectQuery &query,
                                                             const ColumnsDescription &columns, ContextPtr query_context) {
  ...
  InterpreterSelectQuery select(
      result.query_ast, mut_context, storage, {},
  ...
  if (is_aggregate) {
    ...
    if (const auto &group_expression_list = query_select.groupBy()) {
      ...
      if (group_expression_list->children.size() == 1) {
        ...
        for (auto &child : function_node->arguments->children) child = make_intrusive<ASTIdentifier>(child->getColumnName());
    ...
    for (const auto &key : select.getQueryAnalyzer()->aggregationKeys()) result.sample_block_for_keys.insert({nullptr, key.type, key.name});
  } else {
...
ProjectionDescription ProjectionDescription::getMinMaxCountProjection(const ColumnsDescription &columns, ASTPtr partition_columns,
                                                                      const Names &minmax_columns, const KeyDescription &primary_key,
                                                                      ContextPtr query_context) {
  ProjectionDescription result;
  ...
  if (partition_columns && !partition_columns->children.empty()) {
    ...
    for (const auto &partition_column : partition_columns->children) KeyDescription::moduloToModuloLegacyRecursive(partition_column);
  ...
  for (size_t i = 0; i < result.sample_block.columns(); ++i) {
    if (typeid_cast<const ColumnConst *>(result.sample_block.getByPosition(i).column.get())) constant_positions.insert(i);
  }
  ...
  if (!primary_key_asts.empty() && result.sample_block.columns() == 2 * (minmax_columns.size() + 1) /* minmax columns */ + 1 /* count() */
...
Block ProjectionDescription::calculate(const Block &block, UInt64 starting_offset, ContextPtr context,
                                       const IColumnPermutation *perm_ptr) const {
  if (index) return index->calculate(*this, block, starting_offset, context, perm_ptr);
  return calculateByQuery(block, starting_offset, context, perm_ptr);
}

Block ProjectionDescription::calculateByQuery(const Block &block, UInt64 starting_offset, ContextPtr context,
                                              const IColumnPermutation *perm_ptr) const {
  ...
  if (block.rows() == 0) return sample_block.cloneEmpty();
  ...
  Block source_block;
  for (const auto &column : required_columns) source_block.insert(block.getByName(column));
  ...
  if (block.has(RowExistsColumn::name)) {
    ...
    if (!select_row_exists) throw Exception(ErrorCodes::LOGICAL_ERROR, "Cannot get ASTSelectQuery when adding _row_exists = 1. It's a bug");
  ...
  if (with_parent_part_offset) {
    ...
    if (perm_ptr) {
      ...
      for (size_t i = 0; i < block.rows(); ++i) offset[(*perm_ptr)[i]] = i;
    } else {
      /// Rebuilding path
  ...
  auto builder =
      InterpreterSelectQuery(query_ast_copy ? query_ast_copy : query_ast, mut_context,
                             Pipe(std::make_shared<ProjectionDataSource>(std::make_shared<const Block>(std::move(source_block)))),
                             SelectQueryOptions{type == ProjectionDescription::Type::Normal ? QueryProcessingStage::FetchColumns
...
String ProjectionsDescription::toString() const {
  ...
  for (const auto &projection : projections) list.children.push_back(projection.definition_ast);
...
bool ProjectionsDescription::has(const String &projection_name) const { return map.contains(projection_name); }
...
const ProjectionDescription &ProjectionsDescription::get(const String &projection_name) const {
  ...
  if (it == map.end()) {
    throw Exception(ErrorCodes::NO_SUCH_PROJECTION_IN_TABLE, "There is no projection {} in table{}", projection_name,
...
void ProjectionsDescription::add(ProjectionDescription &&projection, const String &after_projection, bool first, bool if_not_exists) {
  if (has(projection.name)) {
    ...
    throw Exception(ErrorCodes::ILLEGAL_PROJECTION, "Cannot add projection {}: projection with this name already exists", projection.name);
  }
  ...
  if (first)
    ...
    auto it = std::find_if(projections.cbegin(), projections.cend(),
                           [&after_projection](const auto &projection_) { return projection_.name == after_projection; });
    if (it != projections.cend()) ++it;
    insert_it = it;
  }
...
std::vector<String> ProjectionsDescription::getAllRegisteredNames() const {
  ...
  for (const auto &pair : map) names.push_back(pair.first);
  return names;
}

ExpressionActionsPtr ProjectionsDescription::getSingleExpressionForProjections(const ColumnsDescription &columns,
                                                                               ContextPtr query_context) const {
  ASTPtr combined_expr_list = make_intrusive<ASTExpressionList>();
  for (const auto &projection : projections)
    for (const auto &projection_expr : projection.query_ast->children) combined_expr_list->children.push_back(projection_expr->clone());
