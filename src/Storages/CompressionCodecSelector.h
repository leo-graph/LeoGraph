 ...
 private:
  struct Element {
    ...
    Element(const Poco::Util::AbstractConfiguration& config, const std::string& config_prefix) {
      ...
      if (config.has(config_prefix + ".level")) level = config.getInt64(config_prefix + ".level");
    }
 ...
 public:
  ...
  CompressionCodecPtr choose(size_t part_size, double part_size_ratio) const {
    ...
    for (const auto& element : elements)
      if (element.check(part_size, part_size_ratio)) res = factory.get(element.family_name, element.level);
