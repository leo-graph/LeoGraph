...
AlterCommand::RemoveProperty removePropertyFromString(const String &property) {
  if (property.empty()) return AlterCommand::RemoveProperty::NO_PROPERTY;
  if (property == "DEFAULT") return AlterCommand::RemoveProperty::DEFAULT;
  if (property == "MATERIALIZED") return AlterCommand::RemoveProperty::MATERIALIZED;
  if (property == "ALIAS") return AlterCommand::RemoveProperty::ALIAS;
  if (property == "COMMENT") return AlterCommand::RemoveProperty::COMMENT;
  if (property == "CODEC") return AlterCommand::RemoveProperty::CODEC;
  if (property == "TTL") return AlterCommand::RemoveProperty::TTL;
  if (property == "SETTINGS") return AlterCommand::RemoveProperty::SETTINGS;
...
std::optional<AlterCommand> AlterCommand::parse(const ASTAlterCommand *command_ast) {
  ...
  if (command_ast->type == ASTAlterCommand::ADD_COLUMN) {
    ...
    if (command_ast->column) command.after_column = getIdentifierName(command_ast->column);

    if (ast_col_decl.getTTL()) command.ttl = ast_col_decl.getTTL();
  ...
  if (command_ast->type == ASTAlterCommand::DROP_COLUMN) {
    ...
    if (command_ast->clear_column) command.clear = true;

    if (command_ast->partition) command.partition = command_ast->partition->clone();
    return command;
  }
  if (command_ast->type == ASTAlterCommand::MODIFY_COLUMN) {
    ...
    if (ast_col_decl.getCodec()) command.codec = ast_col_decl.getCodec();

    if (ast_col_decl.getSettings()) command.settings_changes = ast_col_decl.getSettings()->as<ASTSetQuery &>().changes;
  ...
  if (command_ast->type == ASTAlterCommand::ADD_INDEX) {
    ...
    if (command_ast->index) command.after_index_name = command_ast->index->as<ASTIdentifier &>().name();
  ...
  if (command_ast->type == ASTAlterCommand::ADD_PROJECTION) {
    ...
    if (command_ast->projection) command.after_projection_name = command_ast->projection->as<ASTIdentifier &>().name();
  ...
  if (command_ast->type == ASTAlterCommand::DROP_INDEX) {
    ...
    command.if_exists = command_ast->if_exists;
    if (command_ast->clear_index) command.clear = true;
  ...
  if (command_ast->type == ASTAlterCommand::DROP_PROJECTION) {
    ...
    command.if_exists = command_ast->if_exists;
    if (command_ast->clear_projection) command.clear = true;
  ...
  if (command_ast->type == ASTAlterCommand::RESET_SETTING) {
    ...
    for (const ASTPtr &identifier_ast : command_ast->settings_resets->children) {
      ...
      if (!insertion.second) throw Exception(ErrorCodes::BAD_ARGUMENTS, "Duplicate setting name {}", backQuote(identifier.name()));
    }
    return command;
  }
...
void AlterCommand::apply(StorageInMemoryMetadata &metadata, ContextPtr context) const {
  ...
  auto should_skip_column_operation = [&]() -> bool { return if_exists && !metadata.columns.has(column_name); };
  ...
  if (type == ADD_COLUMN) {
    ...
    if (codec) column.codec = CompressionCodecFactory::instance().validateCodecAndGetPreprocessedAST(codec, data_type, false, true);
    ...
    if (context->getSettingsRef()[Setting::flatten_nested]) {
      ...
      if (!after_column.empty() || first) {
        for (const auto &col : transformed_columns | std::views::reverse) add_column(col.name);
      } else {
        for (const auto &col : transformed_columns) add_column(col.name);
      }
    } else {
    ...
    if (!clear && !partition) {
      if (should_skip_column_operation()) return;
      metadata.columns.remove(column_name);
    }
  } else if (type == MODIFY_COLUMN) {
    if (should_skip_column_operation()) return;
    metadata.columns.modify(column_name, after_column, first, [&](ColumnDescription &column) {
      if (to_remove == RemoveProperty::DEFAULT || to_remove == RemoveProperty::MATERIALIZED || to_remove == RemoveProperty::ALIAS) {
        ...
        if (codec)
          column.codec = CompressionCodecFactory::instance().validateCodecAndGetPreprocessedAST(codec, data_type ? data_type : column.type,
                                                                                                false, true);
        ...
        if (data_type) {
          ...
          if (!column.statistics.empty()) column.statistics.data_type = data_type;
        ...
        if (!settings_changes.empty()) {
          ...
          if (append_column_setting)
            for (const auto &change : settings_changes) column.settings.setSetting(change.name, change.value);
          else
            column.settings = settings_changes;
        }

        if (!settings_resets.empty()) {
          for (const auto &setting : settings_resets) column.settings.removeSetting(setting);
        }
    ...
    if (std::any_of(metadata.secondary_indices.cbegin(), metadata.secondary_indices.cend(),
                    [this](const auto &index) { return index.name == index_name; })) {
    ...
    if (first) insert_it = metadata.secondary_indices.begin();

    if (!after_index_name.empty()) {
      insert_it = std::find_if(metadata.secondary_indices.begin(), metadata.secondary_indices.end(),
                               [this](const auto &index) { return index.name == after_index_name; });
      ...
      if (insert_it == metadata.secondary_indices.end()) {
        ...
        throw Exception(ErrorCodes::BAD_ARGUMENTS, "Wrong index name. Cannot find index {} to insert after{}", backQuote(after_index_name),
                        hints_string);
      }
    ...
    metadata.secondary_indices.emplace(insert_it,
                                       IndexDescription::getIndexFromAST(index_decl, metadata.columns, /* is_implicitly_created */ false,
                                                                         metadata.escape_index_filenames, context));
  } else if (type == DROP_INDEX) {
    if (!partition && !clear) {
      auto erase_it = std::find_if(metadata.secondary_indices.begin(), metadata.secondary_indices.end(),
                                   [this](const auto &index) { return index.name == index_name; });
      ...
      if (erase_it == metadata.secondary_indices.end()) {
        ...
        auto hints_string = !hints.empty() ? ", may be you meant: " + toString(hints) : "";
        throw Exception(ErrorCodes::BAD_ARGUMENTS, "Wrong index name. Cannot find index {} to drop{}", backQuote(index_name), hints_string);
      }
    ...
    for (const auto &statistics_column_name : statistics_columns) {
      if (!metadata.columns.has(statistics_column_name)) {
        throw Exception(ErrorCodes::ILLEGAL_STATISTICS, "Cannot add statistics for column {}: this column is not found",
                        statistics_column_name);
      }
    }
    ...
    for (const auto &statistics_column_name : statistics_columns) {
      if (!metadata.columns.has(statistics_column_name) || metadata.columns.get(statistics_column_name).statistics.empty()) {
        if (if_exists) return;
        throw Exception(ErrorCodes::ILLEGAL_STATISTICS, "Wrong statistics name. Cannot find statistics {} to drop",
                        backQuote(statistics_column_name));
      }

      if (!clear && !partition)
        metadata.columns.modify(statistics_column_name, [&](ColumnDescription &column) { column.statistics.clear(); });
    }
  } else if (type == MODIFY_STATISTICS) {
    for (const auto &statistics_column_name : statistics_columns) {
      if (!metadata.columns.has(statistics_column_name)) {
        throw Exception(ErrorCodes::ILLEGAL_STATISTICS, "Cannot modify statistics for column {}: this column is not found",
                        statistics_column_name);
      }
    }
    ...
    auto stats_vec = ColumnStatisticsDescription::fromAST(statistics_decl, metadata.columns);
    for (const auto &[stats_column_name, stats] : stats_vec) {
      metadata.columns.modify(stats_column_name, [&](ColumnDescription &column) { column.statistics.assign(stats); });
    }
  } else if (type == ADD_CONSTRAINT) {
    auto constraints = metadata.constraints.getConstraints();
    if (std::any_of(constraints.cbegin(), constraints.cend(), [this](const ASTPtr &constraint_ast) {
          return constraint_ast->as<ASTConstraintDeclaration &>().name == constraint_name;
        })) {
      if (if_not_exists) return;
      throw Exception(ErrorCodes::ILLEGAL_COLUMN, "Cannot add constraint {}: constraint with this name already exists", constraint_name);
    }
    ...
    auto constraints = metadata.constraints.getConstraints();
    auto erase_it = std::find_if(constraints.begin(), constraints.end(), [this](const ASTPtr &constraint_ast) {
      return constraint_ast->as<ASTConstraintDeclaration &>().name == constraint_name;
    });

    if (erase_it == constraints.end()) {
      if (if_exists) return;
      throw Exception(ErrorCodes::BAD_ARGUMENTS, "Wrong constraint name. Cannot find constraint `{}` to drop", constraint_name);
    }
    ...
    if (!partition && !clear) metadata.projections.remove(projection_name, if_exists);
    ...
    if (context->getClientInfo().is_shared_catalog_internal && !SharedDatabaseCatalog::isInitialQuery(context)) return;
#endif
    ...
    if (context->getSettingsRef()[Setting::allow_experimental_analyzer]) {
      ...
      as_select_sample = InterpreterSelectWithUnionQuery::getSampleBlock(select->clone(), context, false /* is_subquery */,
    ...
    for (const auto &column : metadata.columns) {
      metadata.columns.modify(column.name, [&](ColumnDescription &column_to_modify) {
        if (column_to_modify.default_desc.expression) rename_visitor.visit(column_to_modify.default_desc.expression);
        if (column_to_modify.ttl) rename_visitor.visit(column_to_modify.ttl);
      });
    }
    if (metadata.table_ttl.definition_ast) rename_visitor.visit(metadata.table_ttl.definition_ast);

    auto constraints_data = metadata.constraints.getConstraints();
    for (auto &constraint : constraints_data) rename_visitor.visit(constraint);
    ...
    if (metadata.isPrimaryKeyDefined()) rename_visitor.visit(metadata.primary_key.definition_ast);

    if (metadata.isSamplingKeyDefined()) rename_visitor.visit(metadata.sampling_key.definition_ast);

    if (metadata.isPartitionKeyDefined()) rename_visitor.visit(metadata.partition_key.definition_ast);
...
bool isJSONTypeHintOnlyChange(const IDataType *from_type, const IDataType *to_type) {
  ...
  if (!from_json || !to_json) return false;

  if (from_json->getSchemaFormat() != DataTypeObject::SchemaFormat::JSON ||
      to_json->getSchemaFormat() != DataTypeObject::SchemaFormat::JSON)
    return false;

  if (from_json->getMaxDynamicPaths() != to_json->getMaxDynamicPaths()) return false;

  if (from_json->getMaxDynamicTypes() != to_json->getMaxDynamicTypes()) return false;

  if (from_json->getPathsToSkip() != to_json->getPathsToSkip()) return false;

  if (from_json->getPathRegexpsToSkip() != to_json->getPathRegexpsToSkip()) return false;
...
bool isMetadataOnlyConversion(const IDataType *from, const IDataType *to, const ContextPtr &context) {
  auto is_compatible_enum_types_conversion = [](const IDataType *from_type, const IDataType *to_type) {
    if (const auto *from_enum8 = typeid_cast<const DataTypeEnum8 *>(from_type)) {
      if (const auto *to_enum8 = typeid_cast<const DataTypeEnum8 *>(to_type)) return to_enum8->contains(*from_enum8);
    }

    if (const auto *from_enum16 = typeid_cast<const DataTypeEnum16 *>(from_type)) {
      if (const auto *to_enum16 = typeid_cast<const DataTypeEnum16 *>(to_type)) return to_enum16->contains(*from_enum16);
    }
  ...
  static const std::unordered_multimap<std::type_index, const std::type_info &> allowed_conversions = {
      {typeid(DataTypeEnum8), typeid(DataTypeInt8)},      {typeid(DataTypeEnum16), typeid(DataTypeInt16)},
      {typeid(DataTypeDateTime), typeid(DataTypeUInt32)}, {typeid(DataTypeUInt32), typeid(DataTypeDateTime)},
      {typeid(DataTypeDate), typeid(DataTypeUInt16)},     {typeid(DataTypeUInt16), typeid(DataTypeDate)},
  };
  ...
  while (true) {
    ...
    if (is_compatible_enum_types_conversion(from, to)) return true;
    ...
    if (context && context->getSettingsRef()[Setting::allow_experimental_json_lazy_type_hints]) {
      if (isJSONTypeHintOnlyChange(from, to)) return true;
    }

    /// Types changed, but representation on disk didn't
    auto it_range = allowed_conversions.equal_range(typeid(*from));
    for (auto it = it_range.first; it != it_range.second; ++it) {
      if (it->second == typeid(*to)) return true;
    }
...
bool AlterCommand::isSettingsAlter() const { return type == MODIFY_SETTING || type == RESET_SETTING; }
...
bool AlterCommand::isRequireMutationStage(const StorageInMemoryMetadata &metadata, const ContextPtr &context) const {
  ...
  if (isRemovingProperty() || type == REMOVE_TTL || type == REMOVE_SAMPLE_BY) return false;

  if (type == DROP_INDEX || type == DROP_PROJECTION || type == RENAME_COLUMN || type == DROP_STATISTICS) return true;

  /// Drop alias is metadata alter, in other case mutation is required.
  if (type == DROP_COLUMN) return metadata.columns.hasColumnOrNested(GetColumnsOptions::AllPhysical, column_name);

  if (type != MODIFY_COLUMN || data_type == nullptr) return false;

  for (const auto &column : metadata.columns.getAllPhysical()) {
    if (column.name == column_name && !isMetadataOnlyConversion(column.type.get(), data_type.get(), context)) return true;
  }
  return false;
}
...
bool AlterCommand::isTTLAlter(const StorageInMemoryMetadata &metadata) const {
  if (type == MODIFY_TTL) {
    if (!metadata.table_ttl.definition_ast) return true;
  ...
  if (!ttl || type != MODIFY_COLUMN) return false;
...
bool AlterCommand::isRemovingProperty() const { return to_remove != RemoveProperty::NO_PROPERTY; }

bool AlterCommand::isDropOrRename() const {
  return type == Type::DROP_COLUMN || type == Type::DROP_INDEX || type == Type::DROP_STATISTICS || type == Type::DROP_CONSTRAINT ||
         type == Type::DROP_PROJECTION || type == Type::RENAME_COLUMN;
}
...
std::optional<MutationCommand> AlterCommand::tryConvertToMutationCommand(StorageInMemoryMetadata &metadata, ContextPtr context) const {
  if (!isRequireMutationStage(metadata, context)) return {};
  ...
  if (type == MODIFY_COLUMN) {
    ...
    if (partition) result.partition = partition;
    result.predicate = nullptr;
  } else if (type == DROP_INDEX) {
...
bool AlterCommands::hasTextIndex(const StorageInMemoryMetadata &metadata) {
  for (const auto &index : metadata.secondary_indices) {
    if (index.type == TEXT_INDEX_NAME) return true;
  }
  return false;
}
...
bool AlterCommands::hasVectorSimilarityIndex(const StorageInMemoryMetadata &metadata) {
  for (const auto &index : metadata.secondary_indices) {
    if (index.type == "vector_similarity") return true;
  }
  return false;
}

void AlterCommands::apply(StorageInMemoryMetadata &metadata, ContextPtr context) const {
  if (!prepared) throw DB::Exception(ErrorCodes::LOGICAL_ERROR, "Alter commands is not prepared. Cannot apply. It's a bug");

  auto metadata_copy = metadata;

  for (const AlterCommand &command : *this)
    if (!command.ignore) command.apply(metadata_copy, context);
  ...
  for (auto &index : metadata_copy.secondary_indices) {
    try {
      index = IndexDescription::getIndexFromAST(index.definition_ast, metadata_copy.columns, index.isImplicitlyCreated(),
                                                index.escape_filenames, context);
...
void AlterCommands::prepare(const StorageInMemoryMetadata &metadata) {
  ...
  for (size_t i = 0; i < size(); ++i) {
    ...
    if (command.type == AlterCommand::MODIFY_COLUMN) {
      if (!has_column && command.if_exists) command.ignore = true;
      ...
      if (has_column && command.if_not_exists) command.ignore = true;
    } else if (command.type == AlterCommand::DROP_COLUMN || command.type == AlterCommand::COMMENT_COLUMN ||
               command.type == AlterCommand::RENAME_COLUMN) {
      if (!has_column && command.if_exists) command.ignore = true;
    } else if (command.type == AlterCommand::MODIFY_ORDER_BY) {
      if (ast_to_str(command.order_by) == ast_to_str(metadata.sorting_key.definition_ast)) command.ignore = true;
    }
  }
...
void AlterCommands::validate(const StoragePtr &table, ContextPtr context) const {
    ...
    if (command.type == AlterCommand::ADD_COLUMN) {
      if (all_columns.has(column_name) || all_columns.hasNested(column_name)) {
        if (!command.if_not_exists)
          throw Exception(ErrorCodes::DUPLICATE_COLUMN, "Cannot add column {}: column with this name already exists",
                          backQuote(column_name));
        continue;
      }

      if (!command.data_type)
        throw Exception(ErrorCodes::BAD_ARGUMENTS, "Data type have to be specified for column {} to add", backQuote(column_name));
      ...
      if (virtuals->tryGet(column_name, VirtualsKind::Persistent))
        throw Exception(ErrorCodes::ILLEGAL_COLUMN, "Cannot add column {}: this column name is reserved for persistent virtual column",
                        backQuote(column_name));

      if (command.codec) {
        const auto &settings = context->getSettingsRef();
        CompressionCodecFactory::instance().validateCodecAndGetPreprocessedAST(
            command.codec, command.data_type, !settings[Setting::allow_suspicious_codecs], settings[Setting::allow_experimental_codecs]);
      }
      ...
      if (!all_columns.has(column_name)) {
        if (!command.if_exists) {
          throw Exception(ErrorCodes::NOT_FOUND_COLUMN_IN_BLOCK, "Wrong column. Cannot find column {} to modify{}", backQuote(column_name),
                          all_columns.getHintsMessage(column_name));
        }
        continue;
      }
      ...
      if (renamed_columns.contains(column_name))
        throw Exception(ErrorCodes::NOT_IMPLEMENTED,
                        "Cannot rename and modify the same column {} "
                        "in a single ALTER query",
                        backQuote(column_name));

      if (command.codec) {
        if (all_columns.hasAlias(column_name)) throw Exception(ErrorCodes::BAD_ARGUMENTS, "Cannot specify codec for column type ALIAS");
        CompressionCodecFactory::instance().validateCodecAndGetPreprocessedAST(
            command.codec, command.data_type, !context->getSettingsRef()[Setting::allow_suspicious_codecs],
      ...
      /// Looks like there is something around default expression for this column (method `getDefault` is not implemented for the data type
      /// Object). But after ALTER TABLE MODIFY COLUMN we need to fill existing rows with something (exactly the default value) or calculate
      /// the common type for it. So we don't allow to do it for now.
      if (command.data_type) {
        validateDataType(command.data_type, DataTypeValidationSettings(context->getSettingsRef()));
        checkAllTypesAreAllowedInTable(NamesAndTypesList{{command.column_name, command.data_type}});
      ...
      if (command.isRemovingProperty()) {
        if (!column_default && command.to_remove == AlterCommand::RemoveProperty::DEFAULT)
          throw Exception(ErrorCodes::BAD_ARGUMENTS, "Column {} doesn't have DEFAULT, cannot remove it", backQuote(column_name));

        if (!column_default && command.to_remove == AlterCommand::RemoveProperty::ALIAS)
          throw Exception(ErrorCodes::BAD_ARGUMENTS, "Column {} doesn't have ALIAS, cannot remove it", backQuote(column_name));

        if (!column_default && command.to_remove == AlterCommand::RemoveProperty::MATERIALIZED)
          throw Exception(ErrorCodes::BAD_ARGUMENTS, "Column {} doesn't have MATERIALIZED, cannot remove it", backQuote(column_name));
        ...
        if (command.to_remove == AlterCommand::RemoveProperty::TTL && column_from_table.ttl == nullptr)
          throw Exception(ErrorCodes::BAD_ARGUMENTS, "Column {} doesn't have TTL, cannot remove it", backQuote(column_name));
        if (command.to_remove == AlterCommand::RemoveProperty::CODEC && column_from_table.codec == nullptr)
          throw Exception(ErrorCodes::BAD_ARGUMENTS, "Column {} doesn't have CODEC, cannot remove it", backQuote(column_name));
        if (command.to_remove == AlterCommand::RemoveProperty::COMMENT && column_from_table.comment.empty())
          throw Exception(ErrorCodes::BAD_ARGUMENTS, "Column {} doesn't have COMMENT, cannot remove it", backQuote(column_name));
      }
      ...
      if (all_columns.has(command.column_name) || all_columns.hasNested(command.column_name)) {
        ...
        auto message = PreformattedMessage::create("Wrong column name. Cannot find column {} to drop", backQuote(command.column_name));
      ...
      if (!all_columns.has(command.column_name)) {
        if (!command.if_exists) {
          auto message = PreformattedMessage::create("Wrong column name. Cannot find column {} to comment", backQuote(command.column_name));
          all_columns.appendHintsMessage(message.text, command.column_name);
          throw Exception(std::move(message), ErrorCodes::NOT_FOUND_COLUMN_IN_BLOCK);
        }
      }
      ...
      if (!all_columns.has(command.column_name)) {
        if (!command.if_exists) {
          auto message = PreformattedMessage::create("Wrong column name. Cannot find column {} to rename", backQuote(command.column_name));
          all_columns.appendHintsMessage(message.text, command.column_name);
          throw Exception(std::move(message), ErrorCodes::NOT_FOUND_COLUMN_IN_BLOCK);
        }
        continue;
      ...
      if (all_columns.has(command.rename_to))
        throw Exception(ErrorCodes::DUPLICATE_COLUMN, "Cannot rename to {}: column with this name already exists",
                        backQuote(command.rename_to));

      if (virtuals->tryGet(command.rename_to, VirtualsKind::Persistent))
        throw Exception(ErrorCodes::ILLEGAL_COLUMN, "Cannot rename to {}: this column name is reserved for persistent virtual column",
                        backQuote(command.rename_to));
    ...
    if (command.type == AlterCommand::MODIFY_COLUMN || command.type == AlterCommand::ADD_COLUMN) {
      if (command.default_expression) {
        ...
        default_expr_list->children.emplace_back(
            setAlias(addTypeConversionToAST(make_intrusive<ASTIdentifier>(tmp_column_name), data_type_ptr->getName()), final_column_name));
        ...
        if (!column_in_table.default_desc.expression) continue;
  ...
  if (!is_parameterized_view && all_columns.empty()) throw Exception(ErrorCodes::BAD_ARGUMENTS, "Cannot DROP or CLEAR all columns");
...
MutationCommands AlterCommands::getMutationCommands(StorageInMemoryMetadata metadata, bool materialize_ttl, ContextPtr context,
                                                    bool with_alters) const {
