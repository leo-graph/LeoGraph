...
UInt64 getMaximumFileNumber(const std::string &dir_path) {
  ...
  for (auto it = begin; it != end; ++it) {
    ...
    if (!std::filesystem::is_regular_file(*it) || !endsWith(file_path.filename().string(), ".bin")) continue;

    UInt64 num = 0;
    try {
      num = parse<UInt64>(file_path.filename().stem().string());
    } catch (Exception &e) {
      e.addMessage("Unexpected file name " + file_path.filename().string() + " found at " + file_path.parent_path().string() +
...
void checkShardingKeyExistsAndIsNumeric(const ASTPtr &sharding_key_ast, ContextPtr context, const NamesAndTypesList &columns) {
  if (!sharding_key_ast) return;

  auto sharding_expr = buildShardingKeyExpression(sharding_key_ast, context, columns, true);
  const Block &block = sharding_expr->getSampleBlock();

  if (block.columns() != 1) throw Exception(ErrorCodes::INCORRECT_NUMBER_OF_COLUMNS, "Sharding expression must return exactly one column");
...
bool isExpressionActionsDeterministic(const ExpressionActionsPtr &actions) {
  for (const auto &action : actions->getActions()) {
    if (action.node->type != ActionsDAG::ActionType::FUNCTION) continue;
    if (!action.node->function_base->isDeterministic()) return false;
  }
  return true;
}
...
class ReplacingConstantExpressionsMatcher {
 public:
  ...
  static void visit(ASTPtr &node, Block &block_with_constants) {
    if (!node->as<ASTFunction>()) return;

    std::string name = node->getColumnName();
    if (block_with_constants.has(name)) {
      const auto &result = block_with_constants.getByName(name);
      if (!isColumnConst(*result.column)) return;
...
void replaceConstantExpressions(ASTPtr &node, ContextPtr context, const NamesAndTypesList &columns, ConstStoragePtr storage,
...
VirtualColumnsDescription StorageDistributed::createVirtuals() {
  ...
  desc.addEphemeral("_database", std::make_shared<DataTypeLowCardinality>(std::make_shared<DataTypeString>()),
                    "The name of database which the row comes from");
  desc.addEphemeral("_table", std::make_shared<DataTypeLowCardinality>(std::make_shared<DataTypeString>()),
                    "The name of table which the row comes from");

  return desc;
}

StorageDistributed::StorageDistributed(const StorageID &id_, const ColumnsDescription &columns_, const ConstraintsDescription &constraints_,
                                       const String &comment, const String &remote_database_, const String &remote_table_,
                                       const String &cluster_name_, ContextPtr context_, const ASTPtr &sharding_key_,
                                       const String &storage_policy_name_, const String &relative_data_path_,
                                       const DistributedSettings &distributed_settings_, LoadingStrictnessLevel mode,
                                       ClusterPtr owned_cluster_, ASTPtr remote_table_function_ptr_, bool is_remote_function_)
    : IStorage(id_),
  ...
  if (!relative_data_path.empty()) {
    ...
    if (storage_policy->getVolumes().size() > 1)
      LOG_WARNING(log,
                  "Storage policy for Distributed table has multiple volumes. "
                  "Only {} volume will be used to store data. Other will be ignored.",
                  data_volume->getName());
  }
...
QueryProcessingStage::Enum StorageDistributed::getQueryProcessingStage(ContextPtr local_context, QueryProcessingStage::Enum to_stage,
                                                                       const StorageSnapshotPtr &storage_snapshot,
  ...
  if (settings[Setting::distributed_group_by_no_merge]) {
    if (settings[Setting::distributed_group_by_no_merge] == DISTRIBUTED_GROUP_BY_NO_MERGE_AFTER_AGGREGATION) {
      if (settings[Setting::distributed_push_down_limit]) return QueryProcessingStage::WithMergeableStateAfterAggregationAndLimit;
  ...
  if (to_stage == QueryProcessingStage::WithMergeableState) return QueryProcessingStage::WithMergeableState;
  ...
  if (optimized_stage) {
    if (*optimized_stage == QueryProcessingStage::Complete) return std::min(to_stage, *optimized_stage);
...
/// 2. Sharding key expression is a deterministic function of col1, ..., coln and expression key is injective functions of these col1, ...,
/// coln.
/// 3. If the expression contains non-injective function, return false.
bool StorageDistributed::isShardingKeySuitsQueryTreeNodeExpression(const QueryTreeNodePtr &expr, const SelectQueryInfo &query_info) const {
  ...
  auto [expression_dag, correlated_subtrees] =
      buildActionsDAGFromExpressionNode(expr, empty_input_columns, query_info.planner_context, empty_correlated_columns_set,
                                        false /* use_column_identifier_as_action_node_name */);

  correlated_subtrees.assertEmpty("in sharding key expression");

  if (expression_dag.hasArrayJoin() || expression_dag.hasStatefulFunctions() || expression_dag.hasNonDeterministic()) return false;
  ...
  for (const auto &col : sharding_key_dag.getRequiredColumnsNames()) {
    if (std::ranges::find(expr_key_required_columns, col) == expr_key_required_columns.end()) return false;
  }
...
std::optional<QueryProcessingStage::Enum> StorageDistributed::getOptimizedQueryProcessingStageAnalyzer(const SelectQueryInfo &query_info,
                                                                                                       const Settings &settings) const {
  bool optimize_sharding_key_aggregation =
      settings[Setting::optimize_skip_unused_shards] && settings[Setting::optimize_distributed_group_by_sharding_key] && has_sharding_key &&
  ...
  if (settings[Setting::distributed_push_down_limit]) default_stage = QueryProcessingStage::WithMergeableStateAfterAggregationAndLimit;
  ...
  if (query_node.isGroupByWithTotals() || query_node.isGroupByWithRollup() || query_node.isGroupByWithCube()) return {};

  // Window functions are not supported.
  if (hasWindowFunctionNodes(query_info.query_tree)) return {};
  // TODO: extremes support can be implemented
  if (settings[Setting::extremes]) return {};
  ...
  if (query_info.has_aggregates || query_node.hasGroupBy()) {
    if (!optimize_sharding_key_aggregation || !query_node.hasGroupBy() || query_node.isGroupByWithGroupingSets() ||
        !isShardingKeySuitsQueryTreeNodeExpression(query_node.getGroupByNode(), query_info))
      return {};
  }
  ...
  // ORDER BY
  if (query_node.hasOrderBy()) return default_stage;

  // LIMIT
  // OFFSET
  if (query_node.hasLimit() || query_node.hasOffset()) return default_stage;

  // Only simple SELECT FROM GROUP BY sharding_key can use Complete state.
  return QueryProcessingStage::Complete;
}

std::optional<QueryProcessingStage::Enum> StorageDistributed::getOptimizedQueryProcessingStage(const SelectQueryInfo &query_info,
                                                                                               const Settings &settings) const {
  bool optimize_sharding_key_aggregation =
      settings[Setting::optimize_skip_unused_shards] && settings[Setting::optimize_distributed_group_by_sharding_key] && has_sharding_key &&
      (settings[Setting::allow_nondeterministic_optimize_skip_unused_shards] || sharding_key_is_deterministic);
  ...
  auto expr_contains_sharding_key = [&](const auto &exprs) -> bool {
    ...
    for (const auto &column : sharding_key_expr->getRequiredColumns()) {
      if (!expr_columns.contains(column)) return false;
    }
  ...
  // - TODO: WITH TOTALS can be implemented
  // - TODO: WITH ROLLUP can be implemented (I guess)
  if (select.group_by_with_totals || select.group_by_with_rollup || select.group_by_with_cube) return {};
  // Window functions are not supported.
  if (query_info.has_window) return {};
  // TODO: extremes support can be implemented
  if (settings[Setting::extremes]) return {};

  // DISTINCT
  if (select.distinct) {
    if (!optimize_sharding_key_aggregation || !expr_contains_sharding_key(select.select()->children)) return {};
  }
  ...
  if (query_info.syntax_analyzer_result) has_aggregates = !query_info.syntax_analyzer_result->aggregates.empty();

  if (has_aggregates || group_by) {
    if (!optimize_sharding_key_aggregation || !group_by || !expr_contains_sharding_key(group_by->children)) return {};
  }

  // LIMIT BY
  if (const ASTPtr limit_by = select.limitBy()) {
    if (!optimize_sharding_key_aggregation || !expr_contains_sharding_key(limit_by->children)) return {};
  }

  // ORDER BY
  if (const ASTPtr order_by = select.orderBy()) return default_stage;

  // LIMIT
  // OFFSET
  if (select.limitLength() || select.limitOffset()) return default_stage;
...
class ReplaseAliasColumnsVisitor : public InDepthQueryTreeVisitor<ReplaseAliasColumnsVisitor> {
  static QueryTreeNodePtr getColumnNodeAliasExpression(const QueryTreeNodePtr &node) {
    ...
    if (!column_node || !column_node->hasExpression()) return nullptr;

    const auto &column_source = column_node->getColumnSourceOrNull();
    if (!column_source || column_source->getNodeType() == QueryTreeNodeType::JOIN ||
        column_source->getNodeType() == QueryTreeNodeType::CROSS_JOIN || column_source->getNodeType() == QueryTreeNodeType::ARRAY_JOIN)
      return nullptr;
 ...
 public:
  void visitImpl(QueryTreeNodePtr &node) {
    if (auto column_expression = getColumnNodeAliasExpression(node)) node = column_expression;
  }
};
  ...
  void enterImpl(QueryTreeNodePtr &node) {
    if (auto *function_node = node->as<FunctionNode>(); function_node && isNameOfLocalInFunction(function_node->getFunctionName())) {
      ...
      auto result_function =
          std::make_shared<FunctionNode>(getGlobalInFunctionNameForLocalInFunctionName(function_node->getFunctionName()));
...
bool rewriteJoinToGlobalJoinIfNeeded(QueryTreeNodePtr join_tree) {
  ...
  if (QueryNode *query = table_expression->as<QueryNode>())
    ...
    if (!typeid_cast<const StorageDistributed *>(table_node_typed->getStorage().get())) rewrite = true;
  } else if (const TableFunctionNode *table_function_node_typed = table_expression->as<TableFunctionNode>()) {
    if (!typeid_cast<const StorageDistributed *>(table_function_node_typed->getStorage().get())) rewrite = true;
  }

  if (rewrite) join->setLocality(JoinLocality::Global);
...
QueryTreeNodePtr buildQueryTreeDistributed(SelectQueryInfo &query_info, const StorageSnapshotPtr &distributed_storage_snapshot,
                                           const StorageID &remote_storage_id, const ASTPtr &remote_table_function) {
  ...
  if (remote_table_function) {
    ...
    if (table_expression_modifiers) table_function_node->setTableExpressionModifiers(*table_expression_modifiers);
    ...
    if (table_expression_modifiers) table_node->setTableExpressionModifiers(*table_expression_modifiers);
...
void StorageDistributed::read(QueryPlan &query_plan, const Names &, const StorageSnapshotPtr &storage_snapshot, SelectQueryInfo &query_info,
                              ContextPtr local_context, QueryProcessingStage::Enum processed_stage, const size_t /*max_block_size*/,
  ...
  if (settings[Setting::allow_experimental_analyzer]) {
    ...
    auto query_tree_distributed = buildQueryTreeDistributed(
        modified_query_info, query_info.initial_storage_snapshot ? query_info.initial_storage_snapshot : storage_snapshot,
        remote_storage_id, remote_table_function_ptr);
    Block block = *InterpreterSelectQueryAnalyzer::getSampleBlock(query_tree_distributed, local_context,
                                                                  SelectQueryOptions(processed_stage).analyze());
     ...
     */
    for (auto &column : block) column.column = column.column->convertToFullColumnIfConst();
    ...
    if (modified_query_info.getCluster()->getShardsInfo().empty()) return;
  } else {
    header =
        InterpreterSelectQuery(modified_query_info.query, local_context, SelectQueryOptions(processed_stage).analyze()).getSampleBlock();

    modified_query_info.query = ClusterProxy::rewriteSelectQuery(local_context, modified_query_info.query, remote_database, remote_table,
  ...
  ClusterProxy::SelectStreamFactory select_stream_factory = ClusterProxy::SelectStreamFactory(header, storage_snapshot, processed_stage);

  auto shard_filter_generator = ClusterProxy::getShardFilterGeneratorForCustomKey(*modified_query_info.getCluster(), local_context,
                                                                                  getInMemoryMetadataPtr()->columns);

  ClusterProxy::executeQuery(query_plan, header, processed_stage, remote_storage, remote_table_function_ptr, select_stream_factory, log,
                             local_context, modified_query_info, sharding_key_expr, sharding_key_column_name, *distributed_settings,
                             shard_filter_generator, is_remote_function);

  /// This is a bug, it is possible only when there is no shards to query, and this is handled earlier.
  if (!query_plan.isInitialized()) throw Exception(ErrorCodes::LOGICAL_ERROR, "Pipeline is not initialized");
}
...
SinkToStoragePtr StorageDistributed::write(const ASTPtr &, const StorageMetadataPtr &metadata_snapshot, ContextPtr local_context,
  ...
  if (!settings[Setting::insert_shard_id] && !settings[Setting::insert_distributed_one_random_shard] && !has_sharding_key &&
      shard_num >= 2) {
  ...
  bool insert_sync =
      settings[Setting::distributed_foreground_insert] || settings[Setting::insert_shard_id] || owned_cluster || relative_data_path.empty();
...
std::optional<QueryPipeline> StorageDistributed::distributedWriteBetweenDistributedTables(const StorageDistributed &src_distributed,
  ...
  if (src_addresses != dst_addresses) {
    ...
    if (local_context->getClientInfo().distributed_depth == 0) {
      LOG_WARNING(log,
                  ...
                  src_distributed.getClusterName(), src_addresses.size(), getClusterName(), dst_addresses.size());
    }
    return {};
  }
  ...
  for (size_t shard_index : collections::range(0, shards_info.size())) {
    ...
    if (shard_info.isLocal() && settings[Setting::prefer_localhost_replica]) {
      ...
      if (connections.empty() || connections.front().isNull()) {
        if (settings[Setting::skip_unavailable_shards]) continue;
        throw Exception(ErrorCodes::LOGICAL_ERROR, "Expected exactly one connection for shard {}", shard_info.shard_num);
      }
      ...
      QueryPipeline remote_pipeline(std::make_shared<RemoteSource>(remote_query_executor, false, settings[Setting::async_socket_for_remote],
  ...
  if (available_shards == 0) throw Exception(ErrorCodes::ALL_CONNECTION_TRIES_FAILED, "No available shards to write to");
  ...
  while (!nodes.empty()) {
    ...
    if (auto *with_filter = dynamic_cast<SourceStepWithFilter *>(node->step.get())) {
      if (source) {
        ...
        throw Exception(ErrorCodes::LOGICAL_ERROR, "Found multiple source steps for query\n{}\nPlan\n{}", ast->formatForErrorMessage(),
                        buf.str());
      }
...
std::optional<QueryPipeline> StorageDistributed::distributedWriteFromClusterStorage(const IStorageCluster &src_storage_cluster,
                                                                                    const ASTInsertQuery &query,
                                                                                    ContextPtr local_context) const {
  const auto &settings = local_context->getSettingsRef();
  ...
  if (filter) predicate = filter->getOutputs().at(0);
  ...
  auto extension = src_storage_cluster.getTaskIteratorExtension(predicate, filter.get(), local_context, cluster,
                                                                src_storage_cluster.getInMemoryMetadataPtr());
  ...
  for (const auto &replicas : cluster->getShardsInfo()) {
    ...
    auto try_results =
        replicas.pool->getMany(timeouts, current_settings, PoolMode::GET_MANY, /*async_callback*/ {}, /*skip_unavailable_endpoints*/ true);
    ...
    for (const auto &try_result : try_results) {
      ...
      auto remote_query_executor = std::make_shared<RemoteQueryExecutor>(
          std::vector<IConnectionPool::Entry>{try_result}, new_query_str, std::make_shared<const Block>(Block{}), query_context,
          /*throttler=*/nullptr, Scalars{}, Tables{}, QueryProcessingStage::Complete, nullptr,
          RemoteQueryExecutor::Extension{.task_iterator = extension.task_iterator, .replica_info = std::move(replica_info)});

      Pipe pipe{std::make_shared<RemoteSource>(remote_query_executor, false, settings[Setting::async_socket_for_remote],
                                               settings[Setting::async_query_sending_for_remote])};
    ...
    if (command.type == AlterCommand::DROP_COLUMN && !command.clear) {
      ...
      if (!deps_mv.empty()) {
        throw Exception(ErrorCodes::ALTER_OF_COLUMN_IS_FORBIDDEN,
                        "Trying to ALTER DROP column {} which is referenced by materialized view {}", backQuoteIfNeed(command.column_name),
                        toString(deps_mv));
      }
    }
  }
...
void StorageDistributed::initializeFromDisk() {
  ...
  ThreadPool pool(CurrentMetrics::StorageDistributedThreads, CurrentMetrics::StorageDistributedThreadsActive,
                  CurrentMetrics::StorageDistributedThreadsScheduled, disks.size());
  ThreadPoolCallbackRunnerLocal<void> runner(pool, ThreadName::DISTRIBUTED_INIT);

  for (const DiskPtr &disk : disks) {
    runner.enqueueAndKeepTrack([this, disk_to_init = disk] { initializeDirectoryQueuesForDisk(disk_to_init); });
  }
  ...
  for (size_t i = 0; i < paths.size(); ++i) {
    ...
    runner.enqueueAndKeepTrack([&paths, &last_increment, i] { last_increment[i] = getMaximumFileNumber(paths[i]); });
  }
  runner.waitForAllToFinishAndRethrowFirstError();

  for (const auto inc : last_increment) {
    if (inc > file_names_increment.value) file_names_increment.value.store(inc);
  }
...
void StorageDistributed::drop() {
  ...
  if (relative_data_path.empty()) return;
...
Strings StorageDistributed::getDataPaths() const {
  ...
  if (relative_data_path.empty()) return paths;

  for (const DiskPtr &disk : data_volume->getDisks()) paths.push_back(disk->getPath() + relative_data_path);
...
StoragePolicyPtr StorageDistributed::getStoragePolicy() const { return storage_policy; }
...
void StorageDistributed::initializeDirectoryQueuesForDisk(const DiskPtr &disk) {
  ...
  for (auto it = begin; it != end; ++it) {
    ...
    if (std::filesystem::is_directory(dir_path)) {
      ...
      if (std::filesystem::is_directory(tmp_path) && std::filesystem::is_empty(tmp_path)) std::filesystem::remove(tmp_path);

      const auto &broken_path = dir_path / "broken";
      if (std::filesystem::is_directory(broken_path) && std::filesystem::is_empty(broken_path)) std::filesystem::remove(broken_path);
...
std::shared_ptr<DistributedAsyncInsertDirectoryQueue> StorageDistributed::getDirectoryQueue(const DiskPtr &disk, const std::string &name) {
  ...
  if (!node_data.directory_queue ||
    ...
    node_data.directory_queue =
        std::make_shared<DistributedAsyncInsertDirectoryQueue>(*this, disk, relative_data_path + name, node_data.connection_pool,
                                                               async_insert_blocker, getContext()->getDistributedSchedulePool());
  }
...
std::vector<DistributedAsyncInsertDirectoryQueue::Status> StorageDistributed::getDirectoryQueueStatuses() const {
  ...
  for (const auto &node : cluster_nodes_data) statuses.push_back(node.second.directory_queue->getStatus());
  return statuses;
}
  ...
  for (auto it = boost::make_split_iterator(name, boost::first_finder(",")); it != decltype(it){}; ++it) {
    ...
    if (address.shard_index) {
      ...
      if (dirname.ends_with("_all_replicas")) {
        for (const auto &replica_address : replicas_addresses) addresses.push_back(replica_address);
        continue;
      }

      if (address.replica_index == 0 || address.replica_index > replicas) {
        LOG_ERROR(log,
                  "Invalid replica_index={} for directory '{}' (cluster has {} replicas for shard {}). "
...
std::optional<UInt64> StorageDistributed::totalBytes(ContextPtr) const {
  ...
  for (const auto &status : getDirectoryQueueStatuses()) total_bytes += status.bytes_count;
  return total_bytes;
}

size_t StorageDistributed::getShardCount() const { return getCluster()->getShardCount(); }

ClusterPtr StorageDistributed::getCluster() const { return owned_cluster ? owned_cluster : getContext()->getCluster(cluster_name); }
...
ClusterPtr StorageDistributed::getOptimizedCluster(ContextPtr local_context, const StorageSnapshotPtr &storage_snapshot,
  ...
  if (force == FORCE_OPTIMIZE_SKIP_UNUSED_SHARDS_ALWAYS ||
    ...
    if (!has_sharding_key) throw Exception(ErrorCodes::UNABLE_TO_SKIP_UNUSED_SHARDS, "No sharding key");
    if (!sharding_key_is_usable) throw Exception(ErrorCodes::UNABLE_TO_SKIP_UNUSED_SHARDS, "Sharding key is not deterministic");
...
ClusterPtr StorageDistributed::skipUnusedShardsWithAnalyzer(ClusterPtr cluster, const SelectQueryInfo &query_info,
                                                            [[maybe_unused]] const StorageSnapshotPtr &storage_snapshot,
                                                            ContextPtr local_context) const {
  if (!query_info.filter_actions_dag) return nullptr;
  ...
  if (!expr_node)
    throw Exception(ErrorCodes::LOGICAL_ERROR, "Cannot find sharding key column {} in expression {}", sharding_key_column_name,
                    sharding_key_dag.dumpDAG());
...
ClusterPtr StorageDistributed::skipUnusedShards(ClusterPtr cluster, const SelectQueryInfo &query_info,
                                                ...
                                                const StorageSnapshotPtr &storage_snapshot, ContextPtr local_context) const {
  if (local_context->getSettingsRef()[Setting::allow_experimental_analyzer])
    return skipUnusedShardsWithAnalyzer(cluster, query_info, storage_snapshot, local_context);

  const auto &select = query_info.query->as<ASTSelectQuery &>();
  if (!select.prewhere() && !select.where()) return nullptr;

  /// FIXME: support analyzer
  if (!syntax_analyzer_result) return nullptr;
  ...
  /// But only the conditions for the left table should be analyzed for shard skipping.
  {
    ...
    if (!select_without_join.prewhere() && !select_without_join.where()) return nullptr;
  ...
  replaceConstantExpressions(condition_ast, local_context, storage_snapshot->metadata->getColumns().getAll(), shared_from_this(),
                             storage_snapshot);
...
ActionLock StorageDistributed::getActionLock(StorageActionBlockType type) {
  if (type == ActionLocks::DistributedSend) return async_insert_blocker.cancel();
  return {};
}
...
void StorageDistributed::flushClusterNodesAllDataImpl(ContextPtr local_context, const SettingsChanges &settings_changes, bool flush) {
...

  {
    ...
    for (auto &node : cluster_nodes_data) directory_queues.push_back(node.second.directory_queue);
  }
  ...
  if (flush) {
    ...
    Stopwatch watch;
    ThreadPool pool(CurrentMetrics::StorageDistributedThreads, CurrentMetrics::StorageDistributedThreadsActive,
                    CurrentMetrics::StorageDistributedThreadsScheduled, directory_queues.size());
    ...
    for (const auto &node : directory_queues) {
      ...
      runner.enqueueAndKeepTrack([node_to_flush = node, &settings_changes] { node_to_flush->flushAllData(settings_changes); });
    }
    ...
    for (auto &node : directory_queues) node->shutdownWithoutFlush();
  }
}
...
void StorageDistributed::rename(const String &new_path_to_table_data, const StorageID &new_table_id) {
  ...
  if (!relative_data_path.empty()) renameOnDisk(new_path_to_table_data);
  renameInMemory(new_table_id);
}

size_t StorageDistributed::getRandomShardIndex(const Cluster::ShardsInfo &shards) {
  UInt32 total_weight = 0;
  for (const auto &shard : shards) total_weight += shard.weight;
  ...
  for (auto i = 0ul, s = shards.size(); i < s; ++i) {
    if (shards[i].weight > res) return i;
    res -= shards[i].weight;
  }
...
void StorageDistributed::renameOnDisk(const String &new_path_to_table_data) {
  for (const DiskPtr &disk : data_volume->getDisks()) {
    ...
    std::lock_guard lock(cluster_nodes_mutex);
    for (auto &node : cluster_nodes_data) node.second.directory_queue->updatePath(new_path_to_table_data);
  }
...
void StorageDistributed::delayInsertOrThrowIfNeeded() const {
  ...
  if ((*distributed_settings)[DistributedSetting::bytes_to_throw_insert] &&
      total_bytes > (*distributed_settings)[DistributedSetting::bytes_to_throw_insert]) {
  ...
  if ((*distributed_settings)[DistributedSetting::bytes_to_delay_insert] &&
      total_bytes > (*distributed_settings)[DistributedSetting::bytes_to_delay_insert]) {
    /// Step is 5% of the delay and minimal one second.
    /// NOTE: max_delay_to_insert is in seconds, and step is in ms.
    const size_t step_ms = static_cast<size_t>(
        std::min<double>(1., static_cast<double>((*distributed_settings)[DistributedSetting::max_delay_to_insert]) * 1'000 * 0.05));
    UInt64 delayed_ms = 0;

    do {
      delayed_ms += step_ms;
      std::this_thread::sleep_for(std::chrono::milliseconds(step_ms));
    } while (*totalBytes(getContext()) > (*distributed_settings)[DistributedSetting::bytes_to_delay_insert] &&
             delayed_ms < (*distributed_settings)[DistributedSetting::max_delay_to_insert] * 1000);
    ...
    LOG_INFO(log, "Too many bytes pending for async INSERT: was {}, now {}, INSERT was delayed to {} ms",
             formatReadableSizeWithBinarySuffix(total_bytes), formatReadableSizeWithBinarySuffix(new_total_bytes), delayed_ms);
  ...
  factory.registerStorage(
      ...
      [](const StorageFactory::Arguments &args) {
        ...
        if (distributed_settings[DistributedSetting::max_delay_to_insert] < 1)
          throw Exception(ErrorCodes::ARGUMENT_OUT_OF_BOUND, "max_delay_to_insert cannot be less then 1");

        if (distributed_settings[DistributedSetting::bytes_to_throw_insert] &&
            distributed_settings[DistributedSetting::bytes_to_delay_insert] &&
            distributed_settings[DistributedSetting::bytes_to_throw_insert] <=
                distributed_settings[DistributedSetting::bytes_to_delay_insert]) {
        ...
        if (!distributed_settings[DistributedSetting::background_insert_batch].changed)
          distributed_settings[DistributedSetting::background_insert_batch] =
              context->getSettingsRef()[Setting::distributed_background_insert_batch];
        ...
        if (!distributed_settings[DistributedSetting::background_insert_sleep_time_ms].changed)
          distributed_settings[DistributedSetting::background_insert_sleep_time_ms] =
              context->getSettingsRef()[Setting::distributed_background_insert_sleep_time_ms];
        ...
        return std::make_shared<StorageDistributed>(args.table_id, args.columns, args.constraints, args.comment, remote_database,
                                                    remote_table, cluster_name, context, sharding_key_ast, storage_policy,
                                                    args.relative_data_path, distributed_settings, args.mode);
      },
      {
...
bool StorageDistributed::initializeDiskOnConfigChange(const std::set<String> &new_added_disks) {
  if (!storage_policy || !data_volume) return true;
