...
int main(int argc, char** argv) try {
  ...
  for (const String& entry : zookeeper->getChildren(fs::path(zookeeper_path) / "temp")) {
    if (startsWith(entry, "abandonable_lock-")) lock_holder_paths.insert(fs::path(zookeeper_path) / "temp" / entry);
  }
  std::cerr << "Stage 1 (get lock holders): " << lock_holder_paths.size() << " lock holders, elapsed: " << stage.elapsedSeconds() << "s."
            << std::endl;
  stage.restart();

  if (!lock_holder_paths.empty()) {
    Strings partitions = zookeeper->getChildren(fs::path(zookeeper_path) / "block_numbers");
    std::cerr << "Stage 2 (get partitions): " << partitions.size() << " partitions, elapsed: " << stage.elapsedSeconds() << "s."
              << std::endl;
    stage.restart();
    ...
    for (size_t i = 0; i < partitions.size(); ++i) {
      ...
      for (const String& entry : partition_block_numbers) {
        ...
        block_infos.push_back(BlockInfo{partitions[i], block_number, zk_path, zookeeper->asyncTryGet(zk_path)});
      }
    }
    std::cerr << "Stage 3 (get block numbers): " << block_infos.size() << " block numbers, elapsed: " << stage.elapsedSeconds() << "s."
              << std::endl;
    stage.restart();
    ...
    std::cerr << "Stage 4 (get block number contents): " << total_count << " current_inserts, elapsed: " << stage.elapsedSeconds() << "s."
              << std::endl;
    stage.restart();
  }
  ...
  for (const auto& kv : current_inserts) {
    std::cout << kv.first << ": ";
    for (Int64 num : kv.second) std::cout << num << ", ";
    std::cout << std::endl;
  }

  return 0;
} catch (const Exception& e) {
  std::cerr << e.what() << ", " << e.displayText() << ": " << std::endl << e.getStackTraceString() << std::endl;
  throw;
