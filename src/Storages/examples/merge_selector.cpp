...
int main(int, char **) {
  ...
  while (!in.eof()) {
    ...
    auto part_info = MergeTreePartInfo::fromPartName(fmt::format("all_{}_{}_0", parts.size(), parts.size()),
  ...
  while (parts.size() > 1) {
    ...
    for (const auto &part : parts) {
      ...
      if (part.name == selected_parts.back().name) {
        ...
        auto part_info = MergeTreePartInfo::fromPartName(fmt::format("all_{}_{}_{}", min_block, max_block, max_level + 1),
                                                         MERGE_TREE_DATA_MIN_FORMAT_VERSION_WITH_CUSTOM_PARTITIONING);
  ...
  for (const auto &part : parts) std::cout << part.name << ", size: " << part.size << "\n";
