...
class MergeTreeAnalyzeIndexSource : public ISource, WithContext {
 public:
  MergeTreeAnalyzeIndexSource(SharedHeader header_, std::vector<UInt8> columns_mask_, const StoragePtr &storage_,
                              const SelectQueryInfo &query_info_, size_t num_streams_, MergeTreeData::DataPartsVector data_parts_,
                              MergeTreeSettingsPtr table_settings_, const ASTPtr &predicate_,
                              const OptionalVectorSearchParameters &vector_search_parameters_, ContextPtr context_)
      : ISource(header_),
 ...
 protected:
  Chunk generate() override {
    if (std::exchange(analyzed, true)) return {};

    if (data_parts.empty()) return {};
    ...
    for (const auto &ranges_in_part : ranges) {
      size_t src_index = 0;
      size_t res_index = 0;
      if (columns_mask[src_index++]) res_columns[res_index++]->insert(ranges_in_part.data_part->name);

      /// ranges
      if (columns_mask[src_index++]) {
        Array field;
        for (const auto &range : ranges_in_part.ranges) field.push_back(Tuple{range.begin, range.end});
    ...
    for (const auto &part : data_parts) {
      if (processed_parts.contains(part->name)) continue;

      size_t src_index = 0;
      size_t res_index = 0;
      if (columns_mask[src_index++]) res_columns[res_index++]->insert(part->name);
  ...
  RangesInDataParts getIndexAnalysis() {
    ...
    ReadFromMergeTree::buildIndexes(indexes, filter_dag ? &filter_dag.value() : nullptr, *merge_tree_data, parts_ranges,
                                    vector_search_parameters,
                                    /*top_k_filter_info=*/std::nullopt, context, query_info, metadata_snapshot);
...
class ReadFromMergeTreeAnalyzeIndexes : public SourceStepWithFilter {
 public:
  ...
  ReadFromMergeTreeAnalyzeIndexes(const Names &column_names_, const SelectQueryInfo &query_info_, size_t num_streams_,
                                  const StorageSnapshotPtr &storage_snapshot_, const ContextPtr &context_, SharedHeader sample_block,
                                  std::vector<UInt8> columns_mask_, std::shared_ptr<StorageMergeTreeAnalyzeIndexes> storage_)
...
void ReadFromMergeTreeAnalyzeIndexes::initializePipeline(QueryPipelineBuilder &pipeline, const BuildQueryPipelineSettings &) {
  LOG_DEBUG(log, "Analyzing index from {} parts of table {}", storage->data_parts.size(),
            storage->source_table->getStorageID().getNameForLogs());

  pipeline.init(Pipe(std::make_shared<MergeTreeAnalyzeIndexSource>(getOutputHeader(), columns_mask, storage->source_table, getQueryInfo(),
                                                                   num_streams, storage->data_parts, storage->table_settings,
                                                                   storage->predicate, storage->vector_search_parameters, context)));
}

///
/// StorageMergeTreeAnalyzeIndex
///
StorageMergeTreeAnalyzeIndexes::StorageMergeTreeAnalyzeIndexes(const StorageID &table_id_, const StoragePtr &source_table_,
                                                               const ColumnsDescription &columns, std::vector<String> parts_,
                                                               const ASTPtr &predicate_,
                                                               const OptionalVectorSearchParameters &vector_search_parameters_)
    : IStorage(table_id_), source_table(source_table_), predicate(predicate_), vector_search_parameters(vector_search_parameters_) {
  const auto *merge_tree_data = dynamic_cast<const MergeTreeData *>(source_table.get());
  if (!merge_tree_data)
    throw Exception(ErrorCodes::BAD_ARGUMENTS, "Storage MergeTreeAnalyzeIndexes expected MergeTree table, got: {}",
...
void StorageMergeTreeAnalyzeIndexes::read(QueryPlan &query_plan, const Names &column_names, const StorageSnapshotPtr &storage_snapshot,
                                          SelectQueryInfo &query_info, ContextPtr context, QueryProcessingStage::Enum,
  ...
  auto reading =
      std::make_unique<ReadFromMergeTreeAnalyzeIndexes>(column_names, query_info, num_streams, storage_snapshot, std::move(context),
                                                        std::make_shared<Block>(header), std::move(columns_mask), std::move(this_ptr));
