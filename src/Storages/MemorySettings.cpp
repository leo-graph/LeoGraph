...
MemorySettings::MemorySettings(MemorySettings&& settings) noexcept
    : impl(std::make_unique<MemorySettingsImpl>(std::move(*settings.impl))) {}
...
ASTPtr MemorySettings::getSettingsChangesQuery() {
  ...
  for (const auto& change : impl->changes()) settings_ast->changes.push_back(change);
...
void MemorySettings::sanityCheck() const {
  if (impl->min_bytes_to_keep > impl->max_bytes_to_keep)
    throw Exception(
        ...
        impl->min_bytes_to_keep.value, impl->max_bytes_to_keep.value);
  ...
  if (impl->min_rows_to_keep > impl->max_rows_to_keep)
    throw Exception(
        ...
        impl->min_rows_to_keep.value, impl->max_rows_to_keep.value);
}

void MemorySettings::applyChanges(const DB::SettingsChanges& changes) { impl->applyChanges(changes); }

bool MemorySettings::hasBuiltin(std::string_view name) { return MemorySettingsImpl::hasBuiltin(name); }
}  // namespace DB
