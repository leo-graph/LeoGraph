...
void buildSetsForDagImpl(const ActionsDAG &dag, const ContextPtr &context, bool ordered) {
  for (const auto &node : dag.getNodes()) {
    if (node.type == ActionsDAG::ActionType::COLUMN) {
      ...
      if (!column_set) column_set = checkAndGetColumn<const ColumnSet>(node.column.get());
...
void buildSetsForDAG(const ActionsDAG &dag, const ContextPtr &context) { buildSetsForDagImpl(dag, context, /* ordered = */ false); }

void buildOrderedSetsForDAG(const ActionsDAG &dag, const ContextPtr &context) { buildSetsForDagImpl(dag, context, /* ordered = */ true); }
...
NameSet getVirtualNamesForFileLikeStorage() { return getCommonVirtualsForFileLikeStorage().getNameSet(); }
...
std::string_view findHivePartitioningInPath(const String &path) {
  ...
  if (key_values.empty()) return std::string_view();
  ...
  if (start < path.data() || start > path.data() + path.size() || end < path.data() || end > path.data() + path.size() || end < start) {
...
VirtualColumnsDescription getVirtualsForFileLikeStorage(ColumnsDescription &storage_columns, ContextPtr context,
  ...
  auto add_virtual = [&](const NameAndTypePair &pair) {
    ...
    if (storage_columns.has(name)) return;

    const auto &type = pair.getTypeInStorage();
    desc.addEphemeral(name, type, "");
  };

  for (const auto &item : getCommonVirtualsForFileLikeStorage()) add_virtual(item);

  if (!path.empty()) {
    if (!partition_strategy.has_value()) throw Exception(ErrorCodes::LOGICAL_ERROR, "Expected partition strategy to be specified");

    /// If partition_stategy == none, we add hive columns, if present, to virtual columns.
    if (context->getSettingsRef()[Setting::use_hive_partitioning] && partition_strategy == PartitionStrategyFactory::StrategyType::NONE) {
      auto hive_columns = HivePartitioningUtils::extractHivePartitionColumnsFromPath(storage_columns, path, format_settings, context);
      for (const auto &column : hive_columns) add_virtual(column);
    }
  }
...
static void addPathAndFileToVirtualColumns(Block &block, const String &path, size_t idx, const FormatSettings &format_settings,
                                           bool parse_hive_columns) {
  if (block.has("_path")) block.getByName("_path").column->assumeMutableRef().insert(path);
...
std::optional<ActionsDAG> createPathAndFileFilterDAG(const ActionsDAG::Node *predicate, const NamesAndTypesList &virtual_columns,
                                                     const ContextPtr &context, const NamesAndTypesList &hive_columns) {
  if (!predicate || virtual_columns.empty()) return {};
...
ColumnPtr getFilterByPathAndFileIndexes(const std::vector<String> &paths, const ExpressionActionsPtr &actions,
                                        const NamesAndTypesList &virtual_columns, const NamesAndTypesList &hive_columns,
  ...
  for (size_t i = 0; i != paths.size(); ++i) {
    addPathAndFileToVirtualColumns(
        block, paths[i],
        /* idx */ i, getFormatSettings(context),
...
void addRequestedFileLikeStorageVirtualsToChunk(Chunk &chunk, const NamesAndTypesList &requested_virtual_columns,
  ...
  for (const auto &virtual_column : requested_virtual_columns) {
    if (virtual_column.name == "_path") {
      ...
      if (virtual_values.filename) {
        chunk.addColumn(
            virtual_column.type->createColumnConst(chunk.getNumRows(), (*virtual_values.filename))->convertToFullColumnIfConst());
      } else {
      ...
      if (virtual_values.last_modified)
        chunk.addColumn(virtual_column.type->createColumnConst(chunk.getNumRows(), virtual_values.last_modified->epochTime())
      ...
      if (virtual_values.tags) {
        Map map_value;
        for (const auto &[key, value] : *virtual_values.tags) map_value.push_back(Field(Tuple{Field(key), Field(value)}));
      ...
      if (virtual_values.data_lake_snapshot_version)
        chunk.addColumn(virtual_column.type->createColumnConst(chunk.getNumRows(), *virtual_values.data_lake_snapshot_version)
                            ->convertToFullColumnIfConst());
      else
        chunk.addColumn(virtual_column.type->createColumnConstWithDefaultValue(chunk.getNumRows())->convertToFullColumnIfConst());
      ...
      if (chunk_info) {
        ...
        for (size_t i = 0; i < num_indices; ++i)
          if (!applied_filter.has_value() || applied_filter.value()[i]) column->insertValue(i + row_num_offset);
      ...
      chunk.addColumn(
          virtual_column.type->createColumnConst(chunk.getNumRows(), convertFieldToType(Field(it->second), *virtual_column.type))
              ->convertToFullColumnIfConst());
    }
  }
}
...
static bool canEvaluateSubtree(const ActionsDAG::Node *node, const Block *allowed_inputs) {
  ...
  while (!nodes.empty()) {
    ...
    if (cur->type == ActionsDAG::ActionType::ARRAY_JOIN) return false;

    if (cur->type == ActionsDAG::ActionType::INPUT && allowed_inputs && !allowed_inputs->has(cur->result_name)) return false;

    for (const auto *child : cur->children) nodes.push(child);
  }
...
bool isDeterministic(const ActionsDAG::Node *node) {
  for (const auto *child : node->children) {
    if (!isDeterministic(child)) return false;
  }

  if (node->type == ActionsDAG::ActionType::COLUMN) return node->isDeterministic();

  if (node->type != ActionsDAG::ActionType::FUNCTION) return true;

  if (!node->function_base->isDeterministic()) return false;
...
bool isDeterministicInScopeOfQuery(const ActionsDAG::Node *node) {
  for (const auto *child : node->children) {
    if (!isDeterministicInScopeOfQuery(child)) return false;
  }

  if (node->type != ActionsDAG::ActionType::FUNCTION) return true;

  if (!node->function_base->isDeterministicInScopeOfQuery()) return false;

  return true;
}

static const ActionsDAG::Node *splitFilterNodeForAllowedInputs(const ActionsDAG::Node *node, const Block *allowed_inputs,
                                                               ActionsDAG::Nodes &additional_nodes, const ContextPtr &context,
  ...
  if (node->type == ActionsDAG::ActionType::FUNCTION) {
    if (node->function_base->getName() == "and") {
      ...
      if (node_copy.children.empty()) return nullptr;
      ...
      if (const auto *adaptor = typeid_cast<const FunctionToFunctionBaseAdaptor *>(node->function_base.get())) {
        if (const auto *index_hint = typeid_cast<const FunctionIndexHint *>(adaptor->getFunction().get())) {
          ...
          if (!atoms.empty()) {
            ...
            if (!res->result_type->equals(*node->result_type)) res = &index_hint_dag.addCast(*res, node->result_type, {}, context);
...
void filterBlockWithPredicate(const ActionsDAG::Node *predicate, Block &block, ContextPtr context,
                              bool allow_filtering_with_partial_predicate) {
  ...
  if (dag) filterBlockWithExpression(buildFilterExpression(std::move(*dag), context), block);
}
...
std::optional<Strings> extractPathValuesFromFilter(const ActionsDAG *filter_dag, ContextPtr context, size_t limit) {
  ...
  if (filter_dag->getOutputs().size() != 1) return {};
  ...
  for (const auto &block : variants.value()) {
    ...
    if (block.size() != 1 || !block.at(0).column) return {};

    // Check for unexpected column data type
    if (!recursiveRemoveLowCardinality(block.at(0).type)->equals(DataTypeString())) return {};

    const auto &column = block.at(0).column;
    for (size_t i = 0; i < column->size(); ++i) result.push_back((*column)[i].safeGet<String>());
  }

  return result;
}

DataPartsVector filterDataPartsWithExpression(const DataPartsVector &data_parts,
                                              const std::shared_ptr<ExpressionActions> &virtual_columns_filter) {
  if (!virtual_columns_filter) return data_parts;

  auto all_part_names = ColumnString::create();
  for (const auto &part : data_parts) all_part_names->insert(part->name);
  ...
  for (size_t i = 0; i < part_names_str.size(); ++i) part_names_set.insert(part_names_str.getDataAt(i));

  DataPartsVector filtered_parts;
  for (const auto &part : data_parts)
    if (part_names_set.has(part->name)) filtered_parts.push_back(part);
