...
static std::pair<Block, Block> getHeaders(const StorageSnapshotPtr& storage_snapshot, const Names& column_names) {
  ...
  for (const auto& column_name : column_names) {
    if (non_virtual_header.has(column_name) || virtual_header.has(column_name)) continue;
...
static Block getSampleBlock(const Block& non_virtual_header, const Block& virtual_header) {
  ...
  for (const auto& column : virtual_header) header.insert(column);

  return header;
}

RabbitMQSource::RabbitMQSource(StorageRabbitMQ& storage_, const StorageSnapshotPtr& storage_snapshot_, ContextPtr context_,
                               const Names& columns, size_t max_block_size_, UInt64 max_execution_time_,
                               StreamingHandleErrorMode handle_error_mode_, bool nack_broken_messages_, bool ack_in_suffix_, LoggerPtr log_)
    : RabbitMQSource(storage_, storage_snapshot_, getHeaders(storage_snapshot_, columns), context_, columns, max_block_size_,
                     max_execution_time_, handle_error_mode_, nack_broken_messages_, ack_in_suffix_, log_) {}

RabbitMQSource::RabbitMQSource(StorageRabbitMQ& storage_, const StorageSnapshotPtr& storage_snapshot_, std::pair<Block, Block> headers,
                               ContextPtr context_, const Names& columns, size_t max_block_size_, UInt64 max_execution_time_,
                               StreamingHandleErrorMode handle_error_mode_, bool nack_broken_messages_, bool ack_in_suffix_, LoggerPtr log_)
...
Chunk RabbitMQSource::generateImpl() {
  ...
  if (is_finished || !consumer || consumer->isConsumerStopped()) {
    LOG_TRACE(log, "RabbitMQSource is stopped (is_finished: {}, consumer_stopped: {})", is_finished,
  ...
  auto input_format =
      FormatFactory::instance().getInput(storage.getFormatName(), empty_buf, non_virtual_header, context, max_block_size, std::nullopt,
  ...
  while (true) {
    ...
    if (new_rows || is_dead_letter) {
      ...
      LOG_TEST(log,
               ...
               new_rows, message.delivery_tag, commit_info.delivery_tag, message.redelivered, commit_info.failed_delivery_tags.size(),
      ...
      if (is_dead_letter) {
        ...
        if (!dead_letter_queue)
          ...
          dead_letter_queue->add(
              DeadLetterQueueElement{.table_engine = DeadLetterQueueElement::StreamType::RabbitMQ,
                                     ...
                                     .details = DeadLetterQueueElement::RabbitMQDetails{.exchange_name = exchange_name,
    ...
    if (max_execution_time_ms) {
      ...
      if (!is_time_limit_exceeded) remaining_execution_time = max_execution_time_ms - elapsed_time_ms;
    }
  ...
  LOG_TEST(log, "Flushing {} rows (max block size: {}, time: {} / {} ms)", total_rows, max_block_size,
           total_stopwatch.elapsedMilliseconds(), max_execution_time_ms);

  if (total_rows == 0) return {};

  auto result_columns = executor.getResultColumns();
  for (auto& column : virtual_columns) result_columns.push_back(std::move(column));

  return Chunk(std::move(result_columns), total_rows);
}

bool RabbitMQSource::sendAck() { return consumer && consumer->ackMessages(commit_info); }

bool RabbitMQSource::sendNack() { return consumer && consumer->nackMessages(commit_info); }
