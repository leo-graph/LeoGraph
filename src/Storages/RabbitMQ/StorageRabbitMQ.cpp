...
StorageRabbitMQ::StorageRabbitMQ(const StorageID &table_id_, ContextPtr context_, const ColumnsDescription &columns_, const String &comment,
                                 std::unique_ptr<RabbitMQSettings> rabbitmq_settings_, LoadingStrictnessLevel mode)
    : IStorage(table_id_),
      ...
      queue_settings_list(
          parseSettings(getContext()->getMacros()->expand((*rabbitmq_settings)[RabbitMQSetting::rabbitmq_queue_settings_list]))),
  ...
  reject_unhandled_messages =
      (*rabbitmq_settings)[RabbitMQSetting::reject_unhandled_messages] ||
      queue_settings_list.end() != std::find_if(queue_settings_list.begin(), queue_settings_list.end(),
  ...
  if ((*rabbitmq_settings)[RabbitMQSetting::rabbitmq_host_port].changed) {
    ...
    if (username.empty() || password.empty())
      throw Exception(ErrorCodes::BAD_ARGUMENTS, "No username or password. They can be specified either in config or in storage settings");
    ...
    if (parsed_address.first.empty())
      throw Exception(ErrorCodes::BAD_ARGUMENTS, "Host or port is incorrect (host: {}, port: {})", parsed_address.first,
                      parsed_address.second);
  ...
  if (configuration.secure) SSL_library_init();

  if (!columns_.getMaterialized().empty() || !columns_.getAliases().empty() || !columns_.getDefaults().empty() ||
      !columns_.getEphemeral().empty()) {
    context_->addOrUpdateWarningMessage(Context::WarningType::RABBITMQ_UNSUPPORTED_COLUMNS,
                                        PreformattedMessage::create("RabbitMQ table engine doesn't support ALIAS, DEFAULT or MATERIALIZED "
                                                                    "columns. They will be ignored and filled with default values"));
  }
  ...
  try {
    ...
    if (mode <= LoadingStrictnessLevel::CREATE) throw;
  }
  ...
  streaming_task =
      getContext()->getMessageBrokerSchedulePool().createTask(getStorageID(), "RabbitMQStreamingTask", [this] { streamingToViewsFunc(); });
  streaming_task->deactivate();

  init_task =
      getContext()->getMessageBrokerSchedulePool().createTask(getStorageID(), "RabbitMQConnectionTask", [this] { connectionFunc(); });
...
Names StorageRabbitMQ::parseSettings(String settings_list) {
  Names result;
  if (settings_list.empty()) return result;
  boost::split(result, settings_list, [](char c) { return c == ','; });
  for (String &key : result) boost::trim(key);
...
AMQP::ExchangeType StorageRabbitMQ::defineExchangeType(String exchange_type_) {
  ...
  if (exchange_type_ != ExchangeType::DEFAULT) {
    if (exchange_type_ == ExchangeType::FANOUT)
      type = AMQP::ExchangeType::fanout;
    else if (exchange_type_ == ExchangeType::DIRECT)
      type = AMQP::ExchangeType::direct;
    else if (exchange_type_ == ExchangeType::TOPIC)
      type = AMQP::ExchangeType::topic;
    else if (exchange_type_ == ExchangeType::HASH)
      type = AMQP::ExchangeType::consistent_hash;
    else if (exchange_type_ == ExchangeType::HEADERS)
      type = AMQP::ExchangeType::headers;
    else
      throw Exception(ErrorCodes::BAD_ARGUMENTS, "Invalid exchange type");
  } else {
    type = AMQP::ExchangeType::fanout;
  }
...
String StorageRabbitMQ::getTableBasedName(String name, const StorageID &table_id) {
  if (name.empty()) return fmt::format("{}_{}", table_id.database_name, table_id.table_name);
...
ContextMutablePtr StorageRabbitMQ::addSettings(ContextPtr local_context) const {
  ...
  if ((*rabbitmq_settings)[RabbitMQSetting::rabbitmq_handle_error_mode] == StreamingHandleErrorMode::DEFAULT)
    modified_context->setSetting("input_format_allow_errors_num",
                                 (*rabbitmq_settings)[RabbitMQSetting::rabbitmq_skip_broken_messages].value);
  else
  ...
  if (!schema_name.empty()) modified_context->setSetting("format_schema", schema_name);
...
void StorageRabbitMQ::loopingFunc() { connection->getHandler().startLoop(); }

void StorageRabbitMQ::stopLoop() { connection->getHandler().updateLoopState(Loop::STOP); }
...
void StorageRabbitMQ::incrementReader() { ++readers_count; }

void StorageRabbitMQ::decrementReader() { --readers_count; }
...
void StorageRabbitMQ::initRabbitMQ() {
  if (shutdown_called || initialized) return;
  ...
  if (use_user_setup) {
    ...
    for (const auto i : collections::range(0, num_queues)) bindQueue(i + 1, *rabbit_channel);

    if (queues.size() != num_queues) {
      throw Exception(ErrorCodes::LOGICAL_ERROR, "Expected all queues to be initialized (but having {}/{})", queues.size(), num_queues);
    }
...
void StorageRabbitMQ::bindExchange(AMQP::TcpChannel &rabbit_channel) {
  ...
  std::string error;
  int error_code;
  rabbit_channel.declareExchange(exchange_name, exchange_type, AMQP::durable).onError([&](const char *message) {
  ...
  rabbit_channel.declareExchange(bridge_exchange, AMQP::fanout, AMQP::durable | AMQP::autodelete).onError([&](const char *message) {
    connection->getHandler().stopLoop();
    /// This error is not supposed to happen as this exchange name is always unique to type and its settings.
    if (error.empty()) {
      error = fmt::format("Unable to declare bridge exchange ({}). Reason: {}", bridge_exchange, std::string(message));
      error_code = ErrorCodes::CANNOT_DECLARE_RABBITMQ_EXCHANGE;
    }
  });
  ...
  if (hash_exchange) {
    ...
    rabbit_channel.declareExchange(sharding_exchange, AMQP::consistent_hash, AMQP::durable | AMQP::autodelete, binding_arguments)
        .onError([&](const char *message) {
          ...
          /// to be the same as some other exchange (which purpose is not for sharding). So probably actual error reason: queue_base
          /// parameter is bad.
          if (error.empty()) {
            error = fmt::format("Unable to declare sharding exchange ({}). Reason: {}", sharding_exchange, std::string(message));
            error_code = ErrorCodes::CANNOT_DECLARE_RABBITMQ_EXCHANGE;
          }
        });

    rabbit_channel.bindExchange(bridge_exchange, sharding_exchange, routing_keys[0]).onError([&](const char *message) {
      connection->getHandler().stopLoop();
      if (error.empty()) {
        error = fmt::format("Unable to bind bridge exchange ({}) to sharding exchange ({}). Reason: {}", bridge_exchange, sharding_exchange,
                            std::string(message));
        error_code = ErrorCodes::CANNOT_DECLARE_RABBITMQ_EXCHANGE;
      }
    });
  ...
  if (exchange_type == AMQP::ExchangeType::headers) {
    ...
    rabbit_channel.bindExchange(exchange_name, bridge_exchange, routing_keys[0], bind_headers)
        ...
        .onError([&](const char *message) {
          connection->getHandler().stopLoop();
          error = fmt::format("Unable to bind exchange {} to bridge exchange ({}). Reason: {}", exchange_name, bridge_exchange,
                              std::string(message));
    ...
    for (const auto &routing_key : routing_keys) {
      rabbit_channel.bindExchange(exchange_name, bridge_exchange, routing_key)
          .onSuccess([&]() {
            ++bound_keys;
            if (bound_keys == routing_keys.size()) connection->getHandler().stopLoop();
          })
          .onError([&](const char *message) {
            connection->getHandler().stopLoop();
            if (error.empty()) {
  ...
  if (!error.empty()) throw Exception(error_code, "{}", error);
}
...
void StorageRabbitMQ::bindQueue(size_t queue_id, AMQP::TcpChannel &rabbit_channel) {
  ...
  auto success_callback = [&](const std::string &queue_name, int msgcount, int /* consumercount */) {
    ...
    if (msgcount) LOG_INFO(log, "Queue {} is non-empty. Non-consumed messaged will also be delivered", queue_name);
    ...
    rabbit_channel.bindQueue(consumer_exchange, queue_name, std::to_string(queue_id))
        ...
        .onError([&](const char *message) {
          connection->getHandler().stopLoop();
          error = fmt::format("Failed to create queue binding for exchange {}. Reason: {}", exchange_name, std::string(message));
        });
  };
  ...
  std::unordered_set<String> integer_settings = {"x-max-length", "x-max-length-bytes", "x-message-ttl",
                                                 "x-expires",    "x-priority",         "x-max-priority"};
  ...
  if (!queue_settings_list.empty()) {
    for (const auto &setting : queue_settings_list) {
      ...
      if (setting_values.size() != 2) throw Exception(ErrorCodes::BAD_ARGUMENTS, "Invalid settings string: {}", setting);
  ...
  connection->getHandler().startBlockingLoop();
  if (!error.empty()) throw Exception(ErrorCodes::CANNOT_CREATE_RABBITMQ_QUEUE_BINDING, "{}", error);
}
...
void StorageRabbitMQ::unbindExchange() {
  ...
  if (!exchange_removed.exchange(true)) {
    try {
      ...
      connection->getHandler().startBlockingLoop();
      rabbit_channel->close();
      if (!error.empty()) throw Exception(ErrorCodes::CANNOT_REMOVE_RABBITMQ_EXCHANGE, "{}", error);
    } catch (...) {
...
void StorageRabbitMQ::read(QueryPlan &query_plan, const Names &column_names, const StorageSnapshotPtr &storage_snapshot,
  ...
  if (!initialized) throw Exception(ErrorCodes::CANNOT_CONNECT_RABBITMQ, "RabbitMQ setup not finished. Connection might be lost");
  ...
  if (!local_context->getSettingsRef()[Setting::stream_like_engine_allow_direct_select])
    throw Exception(ErrorCodes::QUERY_NOT_ALLOWED,
                    "Direct select is not allowed. To enable use setting `stream_like_engine_allow_direct_select`. Be aware that usually "
                    "the read data is removed from the queue.");

  if (mv_attached) throw Exception(ErrorCodes::QUERY_NOT_ALLOWED, "Cannot read from StorageRabbitMQ with attached materialized views");
  ...
  if (!connection->isConnected()) {
    if (connection->getHandler().loopRunning()) deactivateTask(looping_task, false, true);
    if (!connection->reconnect())
  ...
  uint64_t max_execution_time_ms =
      (*rabbitmq_settings)[RabbitMQSetting::rabbitmq_flush_interval_ms].changed
  ...
  for (size_t i = 0; i < num_created_consumers; ++i) {
    auto rabbit_source = std::make_shared<RabbitMQSource>(
        *this, storage_snapshot, modified_context, column_names, /* max_block_size */ 1, max_execution_time_ms,
        (*rabbitmq_settings)[RabbitMQSetting::rabbitmq_handle_error_mode], reject_unhandled_messages,
        /* ack_in_suffix */ (*rabbitmq_settings)[RabbitMQSetting::rabbitmq_commit_on_select], log);

    auto converting_dag =
        ActionsDAG::makeConvertingActions(rabbit_source->getPort().getHeader().getColumnsWithTypeAndName(),
                                          sample_block.getColumnsWithTypeAndName(), ActionsDAG::MatchColumnsMode::Name, local_context);
  ...
  if (!connection->getHandler().loopRunning() && connection->isConnected()) startLoop();
...
SinkToStoragePtr StorageRabbitMQ::write(const ASTPtr &, const StorageMetadataPtr &metadata_snapshot, ContextPtr local_context,
                                        bool /*async_insert*/) {
  auto producer = std::make_unique<RabbitMQProducer>(configuration, routing_keys, exchange_name, exchange_type, producer_id.fetch_add(1),
                                                     persistent, shutdown_called, log);
  ...
  return std::make_shared<MessageQueueSink>(std::make_shared<const Block>(metadata_snapshot->getSampleBlockNonMaterialized()),
                                            getFormatName(), max_rows, std::move(producer), getName(), local_context);
}
...
void StorageRabbitMQ::shutdown(bool) {
  ...
  for (auto &consumer : consumers_ref) consumer.lock()->stop();
  ...
  try {
    for (auto &consumer : consumers_ref) consumer.lock()->closeConnections();

    if (drop_table) cleanupRabbitMQ();
    ...
    for (size_t i = 0; i < num_created_consumers; ++i) popConsumer();
  } catch (...) {
    tryLogCurrentException(log);
  }
...
void StorageRabbitMQ::cleanupRabbitMQ() const {
  if (use_user_setup) return;

  connection->heartbeat();
  if (!connection->isConnected()) {
    String queue_names;
    for (const auto &queue : queues) {
      if (!queue_names.empty()) queue_names += ", ";
      queue_names += queue;
    }
    LOG_WARNING(log,
...
RabbitMQConsumerPtr StorageRabbitMQ::popConsumer() { return popConsumer(std::chrono::milliseconds::zero()); }
...
RabbitMQConsumerPtr StorageRabbitMQ::popConsumer(std::chrono::milliseconds timeout) {
  ...
  if (timeout == std::chrono::milliseconds::zero())
    ...
    if (!semaphore.tryWait(timeout.count())) return nullptr;
  }
...
RabbitMQConsumerPtr StorageRabbitMQ::createConsumer() {
  return std::make_shared<RabbitMQConsumer>(connection->getHandler(), queues, ++consumer_id, unique_strbase, log, queue_size);
}
...
bool StorageRabbitMQ::hasDependencies(const StorageID &table_id) {
  ...
  if (view_ids.empty()) return false;
  ...
  for (const auto &view_id : view_ids) {
    ...
    if (materialized_view && !materialized_view->tryGetTargetTable()) return false;
  }
...
void StorageRabbitMQ::streamingToViewsFunc() {
  ...
  try {
    ...
    if (connection->getHandler().loopRunning()) stopLoopIfNoReaders();
  } catch (...) {
    tryLogCurrentException(__PRETTY_FUNCTION__);
  }
...
void StorageRabbitMQ::streamToViewsImpl() {
  ...
  if (num_views && rabbit_connected) {
    ...
    while (!shutdown_called && num_created_consumers > 0) {
      if (!hasDependencies(table_id)) break;
...
bool StorageRabbitMQ::tryStreamToViews() {
  ...
  for (size_t i = 0; i < num_created_consumers; ++i) {
    auto source = std::make_shared<RabbitMQSource>(*this, storage_snapshot, new_context, Names{}, block_size, max_execution_time_ms,
                                                   (*rabbitmq_settings)[RabbitMQSetting::rabbitmq_handle_error_mode],
  ...
  if (!sources.empty()) {
    ...
    for (const auto &column : header) column_list->children.emplace_back(make_intrusive<ASTIdentifier>(column.name));
    insert->columns = std::move(column_list);
  }

  // Only insert into dependent views and expect that input blocks contain virtual columns
  InterpreterInsertQuery interpreter(insert, new_context,
  ...
  if (!connection->getHandler().loopRunning()) startLoop();
  ...
  if (!connection->isConnected()) {
    ...
    if (connection->reconnect()) {
      ...
      for (auto &source : sources) source->updateChannel();
    } else {
    ...
    for (auto &source : sources) {
      if (!source->hasPendingMessages()) ++queue_empty;
...
void registerStorageRabbitMQ(StorageFactory &factory) {
  auto creator_fn = [](const StorageFactory::Arguments &args) {
    ...
    if (args.storage_def->settings) rabbitmq_settings->loadFromQuery(*args.storage_def);
    ...
    return std::make_shared<StorageRabbitMQ>(args.table_id, args.getContext(), args.columns, args.comment, std::move(rabbitmq_settings),
                                             args.mode);
  };

  factory.registerStorage("RabbitMQ", creator_fn,
