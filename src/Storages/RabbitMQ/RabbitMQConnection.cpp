...
RabbitMQConnection::RabbitMQConnection(const RabbitMQConfiguration& configuration_, LoggerPtr log_)
    : configuration(configuration_), log(log_), event_handler(loop.getLoop(), log) {}

String RabbitMQConnection::connectionInfoForLog() const { return configuration.host + ':' + toString(configuration.port); }
...
bool RabbitMQConnection::reconnect() {
  std::lock_guard lock(mutex);
  if (isConnectedImpl()) return true;

  disconnectImpl();

  /// This will force immediate closure if not yet closed
  if (!connection->closed()) connection->close(true);
...
bool RabbitMQConnection::isConnectedImpl() const { return event_handler.connectionRunning() && connection->usable(); }
...
void RabbitMQConnection::connectImpl() {
  ...
  while (true) {
    ...
    if (connection->ready() || cnt_retries++ == RETRIES_MAX) break;
...
void RabbitMQConnection::disconnectImpl(bool immediately) {
  ...
  while (!connection->closed() && cnt_retries++ != RETRIES_MAX) event_handler.iterateLoop();
}
