...
#define RABBITMQ_RELATED_SETTINGS(DECLARE, ALIAS)                                                                                          \
  ...
  DECLARE(UInt64, rabbitmq_empty_queue_backoff_start_ms, 10, "A minimum backoff point to reschedule read if the RabbitMQ queue is empty",  \
          0)                                                                                                                               \
  DECLARE(UInt64, rabbitmq_empty_queue_backoff_end_ms, 10000, "A maximum backoff point to reschedule read if the RabbitMQ queue is empty", \
          0)                                                                                                                               \
  ...
  DECLARE(Bool, rabbitmq_queue_consume, false,                                                                                             \
          "Use user-defined queues and do not make any RabbitMQ setup: declaring exchanges, queues, bindings", 0)                          \
  DECLARE(String, rabbitmq_username, "", "RabbitMQ username", 0)                                                                           \
  DECLARE(String, rabbitmq_password, "", "RabbitMQ password", 0)                                                                           \
  DECLARE(Bool, reject_unhandled_messages, false,                                                                                          \
          "Allow messages to be rejected in case they cannot be processed. This also automatically implies if there is a "                 \
          "x-deadletter-exchange queue setting added",                                                                                     \
          0)                                                                                                                               \
  ...
  DECLARE(StreamingHandleErrorMode, rabbitmq_handle_error_mode, StreamingHandleErrorMode::DEFAULT,                                         \
          "How to handle errors for RabbitMQ engine. Possible values: default (throw an exception after rabbitmq_skip_broken_messages "    \
          "broken messages), stream (save broken messages and errors in virtual columns _raw_message, _error), dead_letter_queue (error "  \
          "related data will be saved in system.dead_letter_queue).",                                                                      \
          0)

#define OBSOLETE_RABBITMQ_SETTINGS(M, ALIAS) MAKE_OBSOLETE(M, Char, rabbitmq_row_delimiter, '\0')
...
SettingsChanges RabbitMQSettings::getFormatSettings() const {
  ...
  for (const auto& setting : *impl) {
    ...
    if (!setting_name.starts_with("rabbitmq_")) values.emplace_back(setting_name, setting.getValue());
  }

  return values;
}

bool RabbitMQSettings::hasBuiltin(std::string_view name) { return RabbitMQSettingsImpl::hasBuiltin(name); }
}  // namespace DB
