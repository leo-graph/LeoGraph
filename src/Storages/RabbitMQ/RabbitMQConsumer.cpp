...
RabbitMQConsumer::RabbitMQConsumer(RabbitMQHandler& event_handler_, std::vector<String>& queues_, size_t channel_id_base_,
                                   const String& channel_base_, LoggerPtr log_, uint32_t queue_size_)
...
void RabbitMQConsumer::closeConnections() {
  if (consumer_channel) consumer_channel->close();
}
  ...
  for (const auto& queue_name : queues) {
    consumer_channel->consume(queue_name)
        .onSuccess([&](const std::string& /* consumer_tag */) {
          LOG_TRACE(log, "Consumer on channel {} ({}/{}) is subscribed to queue {}", channel_id, subscriptions_num, queues.size(),
                    queue_name);
        })
        .onReceived([&](const AMQP::Message& message, uint64_t delivery_tag, bool redelivered) {
          if (message.bodySize()) {
            ...
            MessageData result{.message = message_received,
            ...
            if (!received.push(std::move(result))) throw Exception(ErrorCodes::LOGICAL_ERROR, "Could not push to received queue");
...
bool RabbitMQConsumer::ackMessages(const CommitInfo& commit_info) {
  ...
  for (const auto& delivery_tag : commit_info.failed_delivery_tags) {
    if (consumer_channel->reject(delivery_tag))
      LOG_TRACE(log, "Consumer rejected message with deliveryTag {} on channel {}", delivery_tag, channel_id);
    else
      LOG_WARNING(log, "Failed to reject message with deliveryTag {} on channel {}", delivery_tag, channel_id);
  }

  /// Duplicate ack?
  if (commit_info.delivery_tag > last_commited_delivery_tag && consumer_channel->ack(commit_info.delivery_tag, AMQP::multiple)) {
    last_commited_delivery_tag = commit_info.delivery_tag;

    LOG_TRACE(log, "Consumer committed messages with deliveryTags up to {} on channel {}", last_commited_delivery_tag, channel_id);

    return true;
  }

  if (commit_info.delivery_tag) {
    LOG_ERROR(log, "Did not commit messages for {}:{}, (current commit point {}:{})", commit_info.channel_id, commit_info.delivery_tag,
...
bool RabbitMQConsumer::nackMessages(const CommitInfo& commit_info) {
  ...
  if (!commit_info.delivery_tag || commit_info.delivery_tag <= last_commited_delivery_tag) {
    LOG_TEST(log, "Delivery tag is {}, last committed delivery tag: {}, Will not nack messages", commit_info.delivery_tag,
             last_commited_delivery_tag);
    return false;
  }

  if (consumer_channel->reject(commit_info.delivery_tag, AMQP::multiple)) {
    LOG_TRACE(log, "Consumer rejected messages with deliveryTags from {} to {} on channel {}", last_commited_delivery_tag,
              commit_info.delivery_tag, channel_id);

    return true;
  }

  LOG_ERROR(log, "Failed to reject messages for {}:{}, (current commit point {}:{})", commit_info.channel_id, commit_info.delivery_tag,
            channel_id, last_commited_delivery_tag);
...
void RabbitMQConsumer::updateChannel(RabbitMQConnection& connection) {
  ...
  consumer_channel->onError([&](const char* message) {
    LOG_ERROR(log, "Channel {} received an error: {} (usable: {}, connected: {})", channel_id, message, consumer_channel->usable(),
              consumer_channel->connected());
...
ReadBufferPtr RabbitMQConsumer::consume() {
  if (stopped || !received.tryPop(current)) return nullptr;
