...
RabbitMQHandler::RabbitMQHandler(uv_loop_t* loop_, LoggerPtr log_)
    : AMQP::LibUvHandler(loop_), loop(loop_), log(log_), connection_running(false), loop_running(false), loop_state(Loop::STOP) {}
...
void RabbitMQHandler::startLoop() {
  ...
  while (loop_state.load() == Loop::RUN) uv_run(loop, UV_RUN_NOWAIT);
...
int RabbitMQHandler::iterateLoop() {
  ...
  if (lock.try_lock()) return uv_run(loop, UV_RUN_NOWAIT);
