...
RabbitMQProducer::RabbitMQProducer(const RabbitMQConfiguration &configuration_, const Names &routing_keys_, const String &exchange_name_,
                                   AMQP::ExchangeType exchange_type_, size_t channel_id_base_, bool persistent_,
                                   std::atomic<bool> &shutdown_called_, LoggerPtr log_)
...
void RabbitMQProducer::stopProducingTask() { payloads.finish(); }

void RabbitMQProducer::finishImpl() { connection.disconnect(); }
...
void RabbitMQProducer::produce(const String &message, size_t, const Columns &, size_t) {
  ...
  if (!payloads.push(std::move(payload))) throw Exception(ErrorCodes::LOGICAL_ERROR, "Could not push to payloads queue");
}
...
void RabbitMQProducer::setupChannel() {
  ...
  producer_channel->onError([&](const char *message) {
    ...
    for (const auto &record : delivery_record)
      if (!returned.push(record.second)) throw Exception(ErrorCodes::LOGICAL_ERROR, "Could not push to returned queue");

    LOG_DEBUG(log, "Producer on channel {} hasn't confirmed {} messages, {} waiting to be published", channel_id, delivery_record.size(),
              payloads.size());
  ...
  producer_channel->onReady([&]() {
    ...
    producer_channel->confirmSelect()
        .onAck([&](uint64_t acked_delivery_tag, bool multiple) { removeRecord(acked_delivery_tag, multiple, false); })
        .onNack(
            [&](uint64_t nacked_delivery_tag, bool multiple, bool /* requeue */) { removeRecord(nacked_delivery_tag, multiple, true); });
    producer_ready = true;
  });
}
...
void RabbitMQProducer::removeRecord(UInt64 received_delivery_tag, bool multiple, bool republish) {
  ...
  if (multiple) {
    ...
    if (republish)
      if (!returned.push(record_iter->second)) throw Exception(ErrorCodes::LOGICAL_ERROR, "Could not push to returned queue");
...
void RabbitMQProducer::startProducingTaskLoop() {
  ...
  while ((!payloads.isFinishedAndEmpty() || !returned.empty() || !delivery_record.empty()) && !shutdown_called.load()) {
    ...
    if (!producer_channel->usable()) {
      if (connection.reconnect()) setupChannel();
    }
  }
...
int RabbitMQProducer::iterateEventLoop() { return connection.getHandler().iterateLoop(); }
