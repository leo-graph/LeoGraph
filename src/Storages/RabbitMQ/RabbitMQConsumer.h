...
class RabbitMQConsumer {
 public:
  RabbitMQConsumer(RabbitMQHandler& event_handler_, std::vector<String>& queues_, size_t channel_id_base_, const String& channel_base_,
                   LoggerPtr log_, uint32_t queue_size_);
  ...
  void waitForMessages(std::optional<uint64_t> timeout_ms = std::nullopt) {
    ...
    if (!timeout_ms) timeout_ms = SANITY_TIMEOUT;
