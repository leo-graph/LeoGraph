  ...
  if (storage_def.settings) {
    ...
    if (disk_change != changes.end() && storage_policy_change != changes.end())
      throw Exception(ErrorCodes::INVALID_SETTING_VALUE,
                      "Could not specify `disk` and `storage_policy` at the same time for storage Log Family");

    if (disk_change != changes.end()) return disk_change->value.safeGet<String>();
...
bool StorageLogSettings::hasBuiltin(std::string_view name) { return name == "disk" || name == "storage_policy"; }
}  // namespace DB
