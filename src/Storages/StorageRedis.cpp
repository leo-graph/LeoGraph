...
class RedisDataSource : public ISource {
 public:
  RedisDataSource(StorageRedis &storage_, SharedHeader header, FieldVectorPtr keys_, FieldVector::const_iterator begin_,
                  FieldVector::const_iterator end_, const size_t max_block_size_)
      : ISource(header),
        storage(storage_),
  ...
  Chunk generateFullScan() {
    ...
    if (scan_keys.isNull() || scan_keys.size() == 0) return generateFullScan();
    ...
    for (size_t i = 0; i < scan_keys.size() && !values.get<RedisBulkString>(i).isNull(); i++) {
      fillColumns(scan_keys.get<RedisBulkString>(i).value(), values.get<RedisBulkString>(i).value(), primary_key_pos, sample_block,
                  columns);
    }
...
RedisSink::RedisSink(StorageRedis &storage_, const StorageMetadataPtr &metadata_snapshot_)
  ...
  for (const auto &column : getHeader()) {
    if (column.name == storage.getPrimaryKey()[0]) break;
    ++primary_key_pos;
  }
}
...
StorageRedis::StorageRedis(const StorageID &table_id_, const RedisConfiguration &configuration_, ContextPtr context_,
                           const StorageInMemoryMetadata &storage_metadata, const String &primary_key_)
    : IStorage(table_id_),
...
class ReadFromRedis : public SourceStepWithFilter {
 public:
  ...
  ReadFromRedis(const Names &column_names_, const SelectQueryInfo &query_info_, const StorageSnapshotPtr &storage_snapshot_,
                const ContextPtr &context_, SharedHeader sample_block, StorageRedis &storage_, size_t max_block_size_, size_t num_streams_)
...
void StorageRedis::read(QueryPlan &query_plan, const Names &column_names, const StorageSnapshotPtr &storage_snapshot,
  ...
  auto reading = std::make_unique<ReadFromRedis>(column_names, query_info, storage_snapshot, context_, std::move(sample_block), *this,
...
void ReadFromRedis::initializePipeline(QueryPipelineBuilder &pipeline, const BuildQueryPipelineSettings &) {
  ...
  if (all_scan) {
    ...
    for (size_t thread_idx = 0; thread_idx < num_threads; ++thread_idx) {
      ...
      auto source =
          std::make_shared<RedisDataSource>(storage, sample_block, keys, keys->begin() + begin, keys->begin() + end, max_block_size);
      source->setStorageLimits(query_info.storage_limits);
...
RedisConfiguration getRedisConfiguration(ASTs &engine_args, ContextPtr context, const StorageID &table_id) {
  ...
  if (engine_args.empty()) throw Exception(ErrorCodes::BAD_ARGUMENTS, "Bad arguments count when creating Redis table engine");
...
StoragePtr createStorageRedis(const StorageFactory::Arguments &args) {
  ...
  if (!args.storage_def->primary_key) throw Exception(ErrorCodes::BAD_ARGUMENTS, "StorageRedis must require one column in primary key");
...
Chunk StorageRedis::getBySerializedKeys(const std::vector<std::string> &keys, PaddedPODArray<UInt8> *null_map) const {
  ...
  for (const auto &key : keys) redis_keys.add(key);
...
Chunk StorageRedis::getBySerializedKeys(const RedisArray &keys, PaddedPODArray<UInt8> *null_map) const {
  ...
  if (values.isNull() || values.size() == 0) return Chunk(std::move(columns), 0);
  ...
  for (size_t i = 0; i < values.size(); ++i) {
    if (!values.get<RedisBulkString>(i).isNull()) {
      fillColumns(keys.get<RedisBulkString>(i).value(), values.get<RedisBulkString>(i).value(), primary_key_pos, sample_block, columns);
    } else  /// key not found
    {
      if (null_map) {
        (*null_map)[i] = 0;
...
RedisArray StorageRedis::multiGet(const RedisArray &keys) const {
  if (keys.isNull() || keys.size() == 0) return {};

  auto connection = getRedisConnection(pool, configuration);

  RedisCommand cmd_mget("MGET");
  for (size_t i = 0; i < keys.size(); ++i) cmd_mget.add(keys.get<RedisBulkString>(i));
...
void StorageRedis::multiSet(const RedisArray &data) const {
  ...
  for (size_t i = 0; i < data.size(); ++i) cmd_mget.add(data.get<RedisBulkString>(i));

  auto ret = connection->client->execute<RedisSimpleString>(cmd_mget);
  if (ret != "OK")
    throw Exception(ErrorCodes::INTERNAL_REDIS_ERROR, "Fail to write to redis table {}, for {}", getStorageID().getFullNameNotQuoted(),
                    ret);
}
...
RedisInteger StorageRedis::multiDelete(const RedisArray &keys) const {
  ...
  for (size_t i = 0; i < keys.size(); ++i) cmd.add(keys.get<RedisBulkString>(i));

  auto ret = connection->client->execute<RedisInteger>(cmd);
  if (ret != static_cast<RedisInteger>(keys.size()))
    LOG_DEBUG(log, "Try to delete {} rows but actually deleted {} rows from redis table {}.", keys.size(), ret,
              getStorageID().getFullNameNotQuoted());

  return ret;
}

Chunk StorageRedis::getByKeys(const ColumnsWithTypeAndName &keys, const Names &, PaddedPODArray<UInt8> &null_map,
                              IColumn::Offsets & /* out_offsets */) const {
  if (keys.size() != 1) throw Exception(ErrorCodes::LOGICAL_ERROR, "StorageRedis supports only one key, got: {}", keys.size());
...
Block StorageRedis::getSampleBlock(const Names &) const { return getInMemoryMetadataPtr()->getSampleBlock(); }
...
void StorageRedis::truncate(const ASTPtr &query, const StorageMetadataPtr &, ContextPtr, TableExclusiveLockHolder &) {
  ...
  if (!truncate_query->sync) cmd_flush_db.add("ASYNC");

  auto ret = connection->client->execute<RedisSimpleString>(cmd_flush_db);

  if (ret != "OK")
    throw Exception(ErrorCodes::INTERNAL_REDIS_ERROR, "Fail to truncate redis table {}, for {}", getStorageID().getFullNameNotQuoted(),
                    ret);
}
...
void StorageRedis::checkMutationIsPossible(const MutationCommands &commands, const Settings & /* settings */) const {
  ...
  if (commands.size() > 1) throw Exception(ErrorCodes::BAD_ARGUMENTS, "Mutations cannot be combined for StorageRedis");
