...
bool columnIsPhysical(ColumnDefaultKind kind) { return kind == ColumnDefaultKind::Default || kind == ColumnDefaultKind::Materialized; }

bool columnDefaultKindHasSameType(ColumnDefaultKind lhs, ColumnDefaultKind rhs) {
  if (lhs == rhs) return true;

  if (columnIsPhysical(lhs) == columnIsPhysical(rhs)) return true;
...
void rewriteEntityInAst(ASTPtr ast, const String &column_name, const Field &value) {
  ...
  if (!select.with()) select.setExpression(ASTSelectQuery::Expression::WITH, make_intrusive<ASTExpressionList>());
...
StorageMerge::DatabaseNameOrRegexp::DatabaseNameOrRegexp(const String &source_database_name_or_regexp_, bool database_is_regexp_,
...
StorageMerge::StorageMerge(const StorageID &table_id_, const ColumnsDescription &columns_, const String &comment,
                           const String &source_database_name_or_regexp_, bool database_is_regexp_,
                           const DBToTableSetMap &source_databases_and_tables_, ContextPtr context_)
    : IStorage(table_id_),
      WithContext(context_->getGlobalContext()),
      database_name_or_regexp(source_database_name_or_regexp_, database_is_regexp_, source_database_name_or_regexp_, {},
                              source_databases_and_tables_) {
  StorageInMemoryMetadata storage_metadata;
  storage_metadata.setColumns(columns_.empty() ? getColumnsDescriptionFromSourceTables(context_) : columns_);
...
StorageMerge::StorageMerge(const StorageID &table_id_, const ColumnsDescription &columns_, const String &comment,
                           const String &source_database_name_or_regexp_, bool database_is_regexp_, const String &source_table_regexp_,
                           ContextPtr context_)
    : IStorage(table_id_),
      WithContext(context_->getGlobalContext()),
      database_name_or_regexp(source_database_name_or_regexp_, database_is_regexp_, source_database_name_or_regexp_, source_table_regexp_,
                              {}) {
  StorageInMemoryMetadata storage_metadata;
  storage_metadata.setColumns(columns_.empty() ? getColumnsDescriptionFromSourceTables(context_) : columns_);
  storage_metadata.setComment(comment);
...
ColumnsDescription StorageMerge::getColumnsDescriptionFromSourceTables(const ContextPtr &query_context,
                                                                       const String &source_database_name_or_regexp,
                                                                       bool database_is_regexp, const String &source_table_regexp,
                                                                       size_t max_tables_to_look) {
  DatabaseNameOrRegexp database_name_or_regexp(source_database_name_or_regexp, database_is_regexp, source_database_name_or_regexp,
                                               source_table_regexp, {});
...
ColumnsDescription StorageMerge::getColumnsDescriptionFromSourceTables(const ContextPtr &query_context) const {
  ...
  if (res.empty())
    throw Exception{DB::ErrorCodes::CANNOT_EXTRACT_TABLE_STRUCTURE,
                    "There are no tables satisfied provided regexp, you must specify table structure manually"};
  return res;
}

ColumnsDescription StorageMerge::getColumnsDescriptionFromSourceTablesImpl(const ContextPtr &query_context,
                                                                           const DatabaseNameOrRegexp &database_name_or_regexp,
                                                                           size_t max_tables_to_look, const IStorage *ignore_self) {
  ...
  traverseTablesUntilImpl(query_context, ignore_self, database_name_or_regexp, [&table_num, &access, &res, max_tables_to_look](auto &&t) {
    ...
    if (!access->isGranted(AccessType::SHOW_TABLES, storage_id.database_name, storage_id.table_name)) return false;
    ...
    for (const ColumnDescription &column : structure) {
      if (!res.has(column.name)) {
        ...
        res.modify(column.name, [&column](ColumnDescription &what) {
          ...
          if (what.default_desc != column.default_desc) what.default_desc = {};
        });
      }
...
template <typename F>
StoragePtr StorageMerge::traverseTablesUntilImpl(const ContextPtr &query_context, const IStorage *ignore_self,
                                                 const DatabaseNameOrRegexp &database_name_or_regexp, F &&predicate) {
  ...
  for (auto &iterator : database_table_iterators) {
    while (iterator->isValid()) {
      ...
      if (table.get() != ignore_self && predicate(table)) return table;
...
std::optional<NameSet> StorageMerge::supportedPrewhereColumns() const {
  ...
  for (const auto &name_type : columns.getAll()) {
    ...
    column_info.emplace(name_type.name, std::make_pair(name_type.type.get(), column_default.kind));
  ...
  forEachTable([&](const StoragePtr &table) {
    ...
    if (!table_metadata_ptr) supports_prewhere = false;
    if (!supports_prewhere) return;
    ...
    for (const auto &column : table_columns.getAll()) {
      ...
      if ((root_type && !root_type->equals(*column.type)) || !columnDefaultKindHasSameType(src_default_kind, column_default.kind)) {
...
QueryProcessingStage::Enum StorageMerge::getQueryProcessingStage(ContextPtr local_context, QueryProcessingStage::Enum to_stage,
                                                                 const StorageSnapshotPtr &, SelectQueryInfo &query_info) const {
    ...
    while (iterator->isValid()) {
      ...
      if (table && table.get() != this) {
        ...
        stage_in_source_tables =
            std::max(stage_in_source_tables,
...
StorageSnapshotPtr StorageMerge::getStorageSnapshot(const StorageMetadataPtr &metadata_snapshot, ContextPtr query_context) const {
  ...
  if (auto first_table = traverseTablesUntil([access](auto &&table) {
    ...
    for (const auto &column : *table_virtuals) {
      if (virtuals.has(column.name)) continue;
...
void StorageMerge::read(QueryPlan &query_plan, const Names &column_names, const StorageSnapshotPtr &storage_snapshot,
                        SelectQueryInfo &query_info, ContextPtr local_context, QueryProcessingStage::Enum processed_stage,
                        const size_t max_block_size, size_t num_streams) {
  ...
  if (local_context->getSettingsRef()[Setting::allow_experimental_analyzer] && processed_stage != QueryProcessingStage::FetchColumns) {
  ...
  auto step = std::make_unique<ReadFromMerge>(column_names, query_info, storage_snapshot, local_context, common_header, max_block_size,
                                              num_streams, shared_from_this(), processed_stage);

  query_plan.addStep(std::move(step));
}

ReadFromMerge::ReadFromMerge(const Names &column_names_, const SelectQueryInfo &query_info_, const StorageSnapshotPtr &storage_snapshot_,
                             const ContextPtr &context_, SharedHeader common_header_, size_t max_block_size, size_t num_streams,
                             StoragePtr storage, QueryProcessingStage::Enum processed_stage)
...
void ReadFromMerge::addFilter(FilterDAGInfo filter) {
  ...
  if (child_plans) {
    ...
    for (auto &child : *child_plans) {
      if (!child.plan.isInitialized()) continue;

      auto filter_step =
          std::make_unique<FilterStep>(child.plan.getCurrentHeader(), filter.actions.clone(), filter.column_name, filter.do_remove_column);
...
void ReadFromMerge::initializePipeline(QueryPipelineBuilder &pipeline, const BuildQueryPipelineSettings &) {
  ...
  const bool should_not_narrow =
      query_info.input_order_info || (context->getSettingsRef()[Setting::distributed_aggregation_memory_efficient] &&
  ...
  if (!should_not_narrow) {
    ...
    Float64 num_streams_multiplier =
        std::min(static_cast<Float64>(tables_count),
                 static_cast<Float64>(
...
std::vector<ReadFromMerge::ChildPlan> ReadFromMerge::createChildrenPlans(SelectQueryInfo &query_info_) const {
  if (selected_tables.empty()) return {};
  ...
  for (const auto &table : selected_tables) {
    ...
    try {
      ...
      if (query_info.table_expression_modifiers) sampling_requested = query_info.table_expression_modifiers->hasSampleSizeRatio();
      ...
      if (use_analyzer && has_table_virtual_column &&
          (common_processed_stage != QueryProcessingStage::FetchColumns || std::dynamic_pointer_cast<StorageMerge>(storage) ||
      ...
      auto row_policy_filter_ptr = modified_context->getRowPolicyFilter(database_name, table_name, RowPolicyFilterType::SELECT_FILTER);
      ...
      auto modified_query_info = getModifiedQueryInfo(modified_context, table, nested_storage_snapshot, real_column_names,
                                                      column_names_as_aliases, is_smallest_column_requested, aliases);

      if (!context->getSettingsRef()[Setting::allow_experimental_analyzer]) {
        auto storage_columns = storage_metadata_snapshot->getColumns();
        auto syntax_result =
            TreeRewriter(context).analyzeSelect(modified_query_info.query, TreeRewriterResult({}, storage, nested_storage_snapshot));
        ...
        if (with_aliases) {
          ...
          for (const auto &column : real_column_names) {
            ...
            if (is_alias) {
              ...
              replaceAliasColumnsInQuery(column_expr, storage_metadata_snapshot->getColumns(), syntax_result->array_join_result_to_source,
                                         context);
          ...
          syntax_result = TreeRewriter(context).analyze(required_columns_expr_list, storage_columns.getAllPhysical(), storage,
          ...
          if (column_names_as_aliases.empty()) {
            column_names_as_aliases.push_back(
                ExpressionActions::getSmallestColumn(storage_metadata_snapshot->getColumns().getAllPhysical()).name);
      ...
      std::erase_if(column_names_to_read,
                    [existing_columns = nested_storage_snapshot->getAllColumnsDescription()](const auto &column_name) {
                      return !existing_columns.has(column_name) && !existing_columns.hasSubcolumn(GetColumnsOptions::All, column_name);
                    });

      auto child =
          createPlanForTable(nested_storage_snapshot, modified_query_info, common_processed_stage, required_max_block_size, table,
                             column_names_to_read, is_smallest_column_requested, row_policy_data_opt, modified_context, current_streams);
      ...
      if (child.plan.isInitialized()) {
        ...
        convertAndFilterSourceStream(*common_header, modified_query_info, nested_storage_snapshot, aliases, row_policy_data_opt, context,
                                     child, is_smallest_column_requested);

        for (const auto &filter_info : pushed_down_filters) {
          auto filter_step = std::make_unique<FilterStep>(child.plan.getCurrentHeader(), filter_info.actions.clone(),
  ...
  void visitImpl(QueryTreeNodePtr &node) {
    if (auto *column = node->as<ColumnNode>()) {
      if (column->hasExpression()) {
        ...
        if (column_source && column_source->getNodeType() == QueryTreeNodeType::LAMBDA) return;
...
QueryTreeNodePtr replaceTableExpressionAndRemoveJoin(QueryTreeNodePtr query, QueryTreeNodePtr original_table_expression,
                                                     QueryTreeNodePtr replacement_table_expression, const ContextPtr &context,
  ...
  if (join_tree_type == QueryTreeNodeType::TABLE || join_tree_type == QueryTreeNodeType::TABLE_FUNCTION) return modified_query;
  ...
  if (query_node->hasLimitByLimit()) query_node->getLimitByLimit() = {};
  if (query_node->hasLimitByOffset()) query_node->getLimitByOffset() = {};
  ...
  for (auto const &column_name : required_column_names) {
    ...
    if (!resolved_column) throw Exception(ErrorCodes::LOGICAL_ERROR, "Required column '{}' is not resolved", column_name);
...
SelectQueryInfo ReadFromMerge::getModifiedQueryInfo(const ContextMutablePtr &modified_context,
                                                    ...
                                                    const StorageSnapshotPtr &storage_snapshot_, Names required_column_names,
                                                    Names &column_names_as_aliases, bool &is_smallest_column_requested,
  ...
  if (modified_query_info.table_expression) {
    ...
    modified_query_info.query_tree =
        replaceTableExpressionAndRemoveJoin(modified_query_info.query_tree, modified_query_info.table_expression,
                                            replacement_table_expression, modified_context, required_column_names);
    ...
    auto get_column_options = GetColumnsOptions(GetColumnsOptions::All).withSubcolumns(storage_snapshot_->storage.supportsSubcolumns());
    ...
    if (with_aliases) {
      ...
      for (const auto &column : required_column_names) {
        ...
        PlannerActionsVisitor actions_visitor(modified_query_info.planner_context, empty_correlated_columns_set,
      ...
      if (column_names_as_aliases.empty()) {
        column_names_as_aliases.push_back(
            ExpressionActions::getSmallestColumn(storage_snapshot_->metadata->getColumns().getAllPhysical()).name);
        is_smallest_column_requested = true;
      }
    }

    if (!column_name_to_node.empty()) {
      replaceColumns(modified_query_info.query_tree, replacement_table_expression, column_name_to_node);
    }
...
bool recursivelyApplyToReadingSteps(QueryPlan::Node *node, const std::function<bool(ReadFromMergeTree &)> &func) {
  bool ok = true;
  for (auto *child : node->children) ok &= recursivelyApplyToReadingSteps(child, func);
  ...
  if (auto *read_from_merge_tree = typeid_cast<ReadFromMergeTree *>(node->step.get())) ok &= func(*read_from_merge_tree);

  return ok;
}

void ReadFromMerge::addVirtualColumns(ChildPlan &child, SelectQueryInfo &modified_query_info, QueryProcessingStage::Enum processed_stage,
  ...
  if (context->getSettingsRef()[Setting::allow_experimental_analyzer]) {
    ...
    String database_column =
        table_alias.empty() || processed_stage == QueryProcessingStage::FetchColumns ? "_database" : table_alias + "._database";
    String table_column = table_alias.empty() || processed_stage == QueryProcessingStage::FetchColumns ? "_table" : table_alias + "._table";

    if (has_database_virtual_column && common_header->has(database_column) && child.stage == QueryProcessingStage::FetchColumns &&
        !plan_header->has(database_column)) {
    ...
    if (has_table_virtual_column && common_header->has(table_column) && processed_stage == QueryProcessingStage::FetchColumns &&
        !plan_header->has(table_column)) {
      ColumnWithTypeAndName column;
...
QueryPipelineBuilderPtr ReadFromMerge::buildPipeline(ChildPlan &child, QueryProcessingStage::Enum processed_stage) const {
  if (!child.plan.isInitialized()) return nullptr;
  ...
  if (!builder->initialized()) return builder;
...
ReadFromMerge::ChildPlan ReadFromMerge::createPlanForTable(const StorageSnapshotPtr &storage_snapshot_,
                                                           SelectQueryInfo &modified_query_info, QueryProcessingStage::Enum processed_stage,
                                                           UInt64 max_block_size, const StorageWithLockAndName &storage_with_lock,
                                                           const Names &real_column_names_read_from_the_source_table,
                                                           bool &is_smallest_column_requested, const RowPolicyDataOpt &row_policy_data_opt,
                                                           ContextMutablePtr modified_context, size_t streams_num) const {
  const auto &[database_name, storage, _, table_name] = storage_with_lock;

  auto &modified_select = modified_query_info.query->as<ASTSelectQuery &>();

  if (!InterpreterSelectQuery::isQueryWithFinal(modified_query_info) &&
      storage->needRewriteQueryWithFinal(real_column_names_read_from_the_source_table)) {
  ...
  auto storage_stage = storage->getQueryProcessingStage(modified_context, processed_stage, storage_snapshot_, modified_query_info);
  ...
  if (processed_stage <= storage_stage && !must_return_interpreter_select_query_plan) {
    ...
    storage->read(plan, real_column_names, storage_snapshot_, modified_query_info, modified_context, processed_stage, max_block_size,
                  UInt32(streams_num));
    ...
    if (use_analyzer) {
      ...
      InterpreterSelectQueryAnalyzer interpreter(ast, modified_context, SelectQueryOptions(processed_stage));
...
ReadFromMerge::RowPolicyData::RowPolicyData(RowPolicyFilterPtr row_policy_filter_ptr, std::shared_ptr<DB::IStorage> storage,
  ...
  filter_actions =
      std::make_shared<ExpressionActions>(actions_dag.clone(), ExpressionActionsSettings(local_context, CompileExpressions::yes));
  ...
  if (!deleted.empty() || added.size() != 1) {
    throw Exception(ErrorCodes::LOGICAL_ERROR, "Cannot determine row level filter; {} columns deleted, {} columns added", deleted.size(),
                    added.size());
  }
...
void ReadFromMerge::RowPolicyData::addFilterTransform(QueryPlan &plan) const {
  auto filter_step =
      std::make_unique<FilterStep>(plan.getCurrentHeader(), actions_dag.clone(), filter_column_name, true /* remove filter column */);
  plan.addStep(std::move(filter_step));
}

StorageMerge::StorageListWithLocks ReadFromMerge::getSelectedTables(ContextPtr query_context, bool filter_by_database_virtual_column,
  ...
  if (filter_actions_dag && (filter_by_database_virtual_column || filter_by_table_virtual_column)) {
    ...
    Block sample_block = {ColumnWithTypeAndName(lc_string_type, "_database"), ColumnWithTypeAndName(lc_string_type, "_table")};
    // Extract predicate part, that could be evaluated only with _database and _table columns
    auto table_filter_dag =
        VirtualColumnUtils::splitFilterDagForAllowedInputs(filter_actions_dag->getOutputs().at(0), &sample_block, query_context);
    if (table_filter_dag) {
      ...
      table_filter = [filter = std::move(filter_expression), column_name = std::move(filter_column_name), lc_string_type](
                         const auto &database_name, const auto &table_name) {
        ...
        Block block{ColumnWithTypeAndName(std::move(database_column), lc_string_type, "_database"),
...
DatabaseTablesIteratorPtr StorageMerge::DatabaseNameOrRegexp::getDatabaseIterator(const String &database_name,
...
StorageMerge::DatabaseTablesIterators StorageMerge::DatabaseNameOrRegexp::getDatabaseIterators(ContextPtr local_context) const {
  try {
    ...
    e.addMessage(
        "while getting table iterator of Merge table. Maybe caused by two Merge tables that will endlessly try to read each other's data");
    throw;
  }
  ...
  if (!database_is_regexp) {
    ...
    for (const auto &db : databases) {
      if (source_database_regexp->match(db.first)) database_table_iterators.emplace_back(getDatabaseIterator(db.first, local_context));
    }
  }
...
void StorageMerge::alter(const AlterCommands &params, ContextPtr local_context, AlterLockHolder &) {
  ...
  DatabaseCatalog::instance()
      ...
      ->alterTable(local_context, table_id, storage_metadata, /*validate_new_create_query=*/true);
  setInMemoryMetadata(storage_metadata);
  setVirtuals(createVirtuals());
}

void ReadFromMerge::convertAndFilterSourceStream(const Block &header, SelectQueryInfo &modified_query_info,
                                                 const StorageSnapshotPtr &snapshot, const Aliases &aliases,
                                                 const RowPolicyDataOpt &row_policy_data_opt, ContextPtr local_context, ChildPlan &child,
  ...
  if (local_context->getSettingsRef()[Setting::allow_experimental_analyzer]) {
    for (const auto &alias : aliases) {
      ...
      if (nodes.size() != 1) throw Exception(ErrorCodes::LOGICAL_ERROR, "Expected to have 1 output but got {}", nodes.size());
  ...
  if (row_policy_data_opt) row_policy_data_opt->addFilterTransform(child.plan);
  ...
  auto convert_actions_dag =
      ActionsDAG::makeConvertingActions(current_step_columns, converted_columns, ActionsDAG::MatchColumnsMode::Position, local_context);
  ...
  {
    auto adding_missing_defaults_dag = addMissingDefaults(*child.plan.getCurrentHeader(), header.getNamesAndTypesList(),
                                                          snapshot->getAllColumnsDescription(), local_context, false);

    auto adding_missing_defaults_step =
        std::make_unique<ExpressionStep>(child.plan.getCurrentHeader(), std::move(adding_missing_defaults_dag));
...
bool ReadFromMerge::requestReadingInOrder(InputOrderInfoPtr order_info_) {
  ...
  if (order_info_->direction != 1 && InterpreterSelectQuery::isQueryWithFinal(query_info)) return false;

  auto request_read_in_order = [order_info_](ReadFromMergeTree &read_from_merge_tree) {
    return read_from_merge_tree.requestReadingInOrder(order_info_->used_prefix_of_sorting_key_size, order_info_->direction,
                                                      order_info_->limit);
  };

  bool ok = true;
  for (const auto &child_plan : *child_plans)
    if (child_plan.plan.isInitialized()) ok &= recursivelyApplyToReadingSteps(child_plan.plan.getRootNode(), request_read_in_order);
...
QueryPlanRawPtrs ReadFromMerge::getChildPlans() {
  ...
  for (auto &child_plan : *child_plans)
    if (child_plan.plan.isInitialized()) plans.push_back(&child_plan.plan);
...
IStorage::ColumnSizeByName StorageMerge::getColumnSizes() const {
  ...
  forEachTable([&](const auto &table) {
    for (const auto &[name, size] : table->getColumnSizes()) column_sizes[name].add(size);
  });
...
std::optional<IStorage::ColumnSizeByName> StorageMerge::tryGetColumnSizes() const {
  try {
    ...
    if (e.code() == ErrorCodes::UNKNOWN_DATABASE) return std::nullopt;
    throw;
  }
}
...
std::tuple<bool /* is_regexp */, ASTPtr> StorageMerge::evaluateDatabaseName(const ASTPtr &node, ContextPtr context_) {
  if (const auto *func = node->as<ASTFunction>(); func && func->name == "REGEXP") {
    if (func->arguments->children.size() != 1) throw Exception(ErrorCodes::BAD_ARGUMENTS, "REGEXP in Merge ENGINE takes only one argument");
...
void registerStorageMerge(StorageFactory &factory) {
  factory.registerStorage("Merge",
                          [](const StorageFactory::Arguments &args) {
                            ...
                            if (!is_regexp) engine_args[0] = database_ast;
                            ...
                            return std::make_shared<StorageMerge>(args.table_id, args.columns, args.comment, source_database_name_or_regexp,
                                                                  is_regexp, table_name_regexp, args.getLocalContext());
                          },
