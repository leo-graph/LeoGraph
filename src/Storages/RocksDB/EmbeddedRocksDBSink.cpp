...
void EmbeddedRocksDBSink::consume(Chunk& chunk) {
  ...
  for (size_t i = 0; i < rows; ++i) {
    ...
    for (const auto idx : primary_key_pos) serializations[idx]->serializeBinary(*columns[idx], i, wb_key, {});

    for (const auto idx : value_column_pos) serializations[idx]->serializeBinary(*columns[idx], i, wb_value, {});
