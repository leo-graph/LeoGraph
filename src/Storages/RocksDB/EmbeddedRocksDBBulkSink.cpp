...
static rocksdb::Status buildSSTFile(const String& path, const ColumnString& keys, const ColumnString& values,
                                    const std::optional<IColumn::Permutation>& perm_ = {}) {
  ...
  for (size_t idx = 0; idx < rows;) {
    ...
    while (next_idx < rows && keys.compareAt(perm[idx], perm[next_idx], keys, 1) == 0) ++next_idx;
...
EmbeddedRocksDBBulkSink::EmbeddedRocksDBBulkSink(ContextPtr context_, StorageEmbeddedRocksDB& storage_,
  ...
  min_block_size_rows = std::max(storage.getSettings()[RocksDBSetting::bulk_insert_block_size],
...
EmbeddedRocksDBBulkSink::~EmbeddedRocksDBBulkSink() {
  try {
    if (fs::exists(insert_directory_queue)) (void)fs::remove_all(insert_directory_queue);
  } catch (...) {
...
std::pair<ColumnString::Ptr, ColumnString::Ptr> EmbeddedRocksDBBulkSink::serializeChunks(std::vector<Chunk>&& input_chunks) const {
...

  {
    ...
    [[maybe_unused]] auto get_rocksdb_ts = [this](String& ts_string) {
      ...
      if (!st.ok()) throw Exception(ErrorCodes::ROCKSDB_ERROR, "RocksDB error: {}", st.ToString());
    ...
    for (auto&& chunk : input_chunks) {
      ...
      if constexpr (with_timestamp) get_rocksdb_ts(ts_string);
...
void EmbeddedRocksDBBulkSink::consume(Chunk& chunk_) {
  ...
  if (chunks_to_write.empty()) return;
  ...
  LOG_DEBUG(getLogger("EmbeddedRocksDBBulkSink"), "Writing {} rows from {} chunks to SST file {}", serialized_key_column->size(),
            num_chunks, sst_file_path);
  ...
  if (fs::exists(sst_file_path)) (void)fs::remove(sst_file_path);
}
...
String EmbeddedRocksDBBulkSink::getTemporarySSTFilePath() { return fs::path(insert_directory_queue) / (toString(file_counter++) + ".sst"); }

bool EmbeddedRocksDBBulkSink::isEnoughSize(const std::vector<Chunk>& input_chunks) const {
  size_t total_rows = 0;
  for (const auto& chunk : input_chunks) total_rows += chunk.getNumRows();
  return total_rows >= min_block_size_rows;
}

bool EmbeddedRocksDBBulkSink::isEnoughSize(const Chunk& chunk) const { return chunk.getNumRows() >= min_block_size_rows; }
