...
class EmbeddedRocksDBSource : public ISource {
 public:
  EmbeddedRocksDBSource(const StorageEmbeddedRocksDB &storage_, SharedHeader header, FieldVectorPtr keys_,
                        FieldVector::const_iterator begin_, FieldVector::const_iterator end_, const size_t max_block_size_)
      : ISource(header), storage(storage_), keys(keys_), begin(begin_), end(end_), it(begin), max_block_size(max_block_size_) {}

  EmbeddedRocksDBSource(const StorageEmbeddedRocksDB &storage_, SharedHeader header, std::unique_ptr<rocksdb::Iterator> iterator_,
                        const size_t max_block_size_)
      : ISource(header), storage(storage_), iterator(std::move(iterator_)), max_block_size(max_block_size_) {}

  String getName() const override { return storage.getName(); }

  Chunk generate() override {
    if (keys) return generateWithKeys();
  ...
  Chunk generateFullScan() {
    ...
    if (!iterator->status().ok()) {
      throw Exception(ErrorCodes::ROCKSDB_ERROR, "Engine {} got error while seeking key value data: {}", getName(),
...
StorageEmbeddedRocksDB::StorageEmbeddedRocksDB(const StorageID &table_id_, const String &relative_data_path_,
                                               const StorageInMemoryMetadata &metadata_, LoadingStrictnessLevel mode, ContextPtr context_,
                                               std::unique_ptr<RocksDBSettings> settings_, Names primary_keys_, Int32 ttl_,
                                               String rocksdb_dir_, bool read_only_)
    : IStorage(table_id_),
  ...
  if (rocksdb_dir.empty()) {
    ...
    if (fs::path(rocksdb_dir).is_relative()) rocksdb_dir = user_files_path / rocksdb_dir;
  ...
  for (size_t i = 0; i < is_pk.size(); ++i) {
    if (!is_pk[i]) value_column_pos.push_back(i);
  }
...
void StorageEmbeddedRocksDB::checkMutationIsPossible(const MutationCommands &commands, const Settings & /* settings */) const {
  ...
  if (commands.size() > 1) throw Exception(ErrorCodes::BAD_ARGUMENTS, "Mutations cannot be combined for EmbeddedRocksDB");
  ...
  if (commands.front().type == MutationCommand::Type::DELETE) {
    ...
    while (executor.pull(block)) {
      ...
      for (size_t i = 0; i < size; ++i) {
        ...
        if (!status.ok()) throw Exception(ErrorCodes::ROCKSDB_ERROR, "RocksDB write error: {}", status.ToString());
      }
...
bool StorageEmbeddedRocksDB::optimize(const ASTPtr & /*query*/, const StorageMetadataPtr & /*metadata_snapshot*/, const ASTPtr &partition,
  ...
  if (final) throw Exception(ErrorCodes::NOT_IMPLEMENTED, "FINAL cannot be specified when optimizing table of type EmbeddedRocksDB");

  if (deduplicate)
    throw Exception(ErrorCodes::NOT_IMPLEMENTED, "DEDUPLICATE cannot be specified when optimizing table of type EmbeddedRocksDB");

  if (cleanup) throw Exception(ErrorCodes::NOT_IMPLEMENTED, "CLEANUP cannot be specified when optimizing table of type EmbeddedRocksDB");

  std::shared_lock lock(rocksdb_ptr_mx);
  rocksdb::CompactRangeOptions compact_options;
  auto status = rocksdb_ptr->CompactRange(compact_options, nullptr, nullptr);
  if (!status.ok()) throw Exception(ErrorCodes::ROCKSDB_ERROR, "Compaction failed: {}", status.ToString());
  return true;
}
...
class StorageEmbeddedRocksDBLogger : public rocksdb::Logger {
 public:
  explicit StorageEmbeddedRocksDBLogger(const rocksdb::InfoLogLevel log_level, LoggerRawPtr log_) : rocksdb::Logger(log_level), log(log_) {}
  ...
  void Logv(const rocksdb::InfoLogLevel log_level, const char *format, va_list ap) override __attribute__((format(printf, 3, 0))) {
    ...
    /// HdfsLogger)
    {
      ...
      if (vsnprintf(buffer.get(), buffer_size, format, backup_ap) >= buffer_size) buffer[buffer_size - 1] = 0;
      va_end(backup_ap);
...
void StorageEmbeddedRocksDB::initDB() {
  ...
  if (config.has("rocksdb.options")) {
    ...
    if (!status.ok()) {
      throw Exception(ErrorCodes::ROCKSDB_ERROR, "Fail to merge rocksdb options from 'rocksdb.options' at: {}: {}", rocksdb_dir,
                      status.ToString());
    }
  }
  ...
  if (config.has("rocksdb.tables")) {
    ...
    for (const auto &key : keys) {
      ...
      if (config.getString(key_prefix + ".name") != table_name) continue;
      ...
      if (config.has(config_key)) {
        ...
        if (!status.ok()) {
          throw Exception(ErrorCodes::ROCKSDB_ERROR, "Fail to merge rocksdb options from '{}' at: {}: {}", config_key, rocksdb_dir,
                          status.ToString());
        }
      }
  ...
  if (ttl > 0) {
    ...
    if (!status.ok()) throw Exception(ErrorCodes::ROCKSDB_ERROR, "Failed to open rocksdb path at: {}: {}", rocksdb_dir, status.ToString());
...
class ReadFromEmbeddedRocksDB : public SourceStepWithFilter {
 public:
  ...
  ReadFromEmbeddedRocksDB(const Names &column_names_, const SelectQueryInfo &query_info_, const StorageSnapshotPtr &storage_snapshot_,
                          const ContextPtr &context_, SharedHeader sample_block, const StorageEmbeddedRocksDB &storage_,
...
void StorageEmbeddedRocksDB::read(QueryPlan &query_plan, const Names &column_names, const StorageSnapshotPtr &storage_snapshot,
  ...
  auto reading =
      std::make_unique<ReadFromEmbeddedRocksDB>(column_names, query_info, storage_snapshot, context_,
                                                std::make_shared<const Block>(std::move(sample_block)), *this, max_block_size, num_streams);
...
void ReadFromEmbeddedRocksDB::initializePipeline(QueryPipelineBuilder &pipeline, const BuildQueryPipelineSettings &) {
  ...
  for (size_t thread_idx = 0; thread_idx < num_threads; ++thread_idx) {
    ...
    auto source =
        std::make_shared<EmbeddedRocksDBSource>(storage, sample_block, keys, keys->begin() + begin, keys->begin() + end, max_block_size);
    source->setStorageLimits(query_info.storage_limits);
...
SinkToStoragePtr StorageEmbeddedRocksDB::write(const ASTPtr & /*query*/, const StorageMetadataPtr &metadata_snapshot,
                                               ContextPtr query_context, bool /*async_insert*/) {
...
static StoragePtr create(const StorageFactory::Arguments &args) {
  ...
  if (!engine_args.empty()) ttl = static_cast<Int32>(checkAndGetLiteralArgument<UInt64>(engine_args[0], "ttl"));
  if (engine_args.size() > 1) rocksdb_dir = checkAndGetLiteralArgument<String>(engine_args[1], "rocksdb_dir");
  if (engine_args.size() > 2) read_only = checkAndGetLiteralArgument<bool>(engine_args[2], "read_only");
  ...
  return std::make_shared<StorageEmbeddedRocksDB>(args.table_id, args.relative_data_path, metadata, args.mode, args.getContext(),
                                                  std::move(settings), primary_key_names, ttl, std::move(rocksdb_dir), read_only);
}

std::shared_ptr<rocksdb::Statistics> StorageEmbeddedRocksDB::getRocksDBStatistics() const {
  std::shared_lock lock(rocksdb_ptr_mx);
  if (!rocksdb_ptr) return nullptr;
  return rocksdb_ptr->GetOptions().statistics;
}

std::vector<rocksdb::Status> StorageEmbeddedRocksDB::multiGet(const std::vector<rocksdb::Slice> &slices_keys,
                                                              std::vector<String> &values) const {
  std::shared_lock lock(rocksdb_ptr_mx);
  if (!rocksdb_ptr) return {};
  return rocksdb_ptr->MultiGet(rocksdb::ReadOptions(), slices_keys, &values);
}

Chunk StorageEmbeddedRocksDB::getByKeys(const ColumnsWithTypeAndName &keys, const Names &, PaddedPODArray<UInt8> &null_map,
  ...
  for (size_t i = 0; i < keys.size(); ++i) {
    ...
    if (!key_type->equals(*primary_key_type))
      throw DB::Exception(ErrorCodes::LOGICAL_ERROR, "Primary key type mismatch, expected {}, got {}.", primary_key_types[i]->getName(),
                          keys[i].type->getName());
  }
...
Block StorageEmbeddedRocksDB::getSampleBlock(const Names &) const { return getInMemoryMetadataPtr()->getSampleBlock(); }
...
Chunk StorageEmbeddedRocksDB::getBySerializedKeys(const std::vector<std::string> &keys, PaddedPODArray<UInt8> *in_out_null_map) const {
  ...
  for (const auto &key : keys) slices_keys.emplace_back(key);
...
std::optional<UInt64> StorageEmbeddedRocksDB::totalRows(ContextPtr query_context) const {
  if (!query_context->getSettingsRef()[Setting::optimize_trivial_approximate_count_query]) return {};
  std::shared_lock lock(rocksdb_ptr_mx);
  if (!rocksdb_ptr) return {};
  UInt64 estimated_rows;
  if (!rocksdb_ptr->GetIntProperty("rocksdb.estimate-num-keys", &estimated_rows)) return {};
...
std::optional<UInt64> StorageEmbeddedRocksDB::totalBytes(ContextPtr) const {
  ...
  if (!rocksdb_ptr->GetAggregatedIntProperty("rocksdb.estimate-live-data-size", &estimated_bytes)) return {};
