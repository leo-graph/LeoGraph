...
void StorageSystemRocksDB::fillData(MutableColumns& res_columns, ContextPtr context, const ActionsDAG::Node* predicate,
  ...
  for (size_t i = 0, tables_size = col_database_to_filter->size(); i < tables_size; ++i) {
    ...
    if (!statistics) throw Exception(ErrorCodes::LOGICAL_ERROR, "RocksDB statistics are not available");

    for (auto [tick, name] : rocksdb::TickersNameMap) {
      UInt64 value = statistics->getTickerCount(tick);
      if (!value && !show_zeros) continue;

      /// trim "rocksdb."
      if (startsWith(name, "rocksdb.")) name = name.substr(strlen("rocksdb."));
