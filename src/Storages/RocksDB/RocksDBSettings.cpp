...
#define LIST_OF_ROCKSDB_SETTINGS(DECLARE, ALIAS)                                                                                        \
  DECLARE(Bool, optimize_for_bulk_insert, true,                                                                                         \
          "Table is optimized for bulk insertions (insert pipeline will create SST files and import to rocksdb database instead of "    \
          "writing to memtables)",                                                                                                      \
          0)                                                                                                                            \
  DECLARE(UInt64, bulk_insert_block_size, DEFAULT_INSERT_BLOCK_SIZE,                                                                    \
          "Size of block for bulk insert, if it's smaller than query setting min_insert_block_size_rows then it will be overridden by " \
...
void RocksDBSettings::applyChanges(const SettingsChanges& changes) { impl->applyChanges(changes); }
...
bool RocksDBSettings::hasBuiltin(std::string_view name) { return RocksDBSettingsImpl::hasBuiltin(name); }
}  // namespace DB
