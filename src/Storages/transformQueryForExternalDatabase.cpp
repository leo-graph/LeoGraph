...
class DropAliasesMatcher {
 public:
  ...
  static void visit(ASTPtr &node, Data) {
    if (!node->tryGetAlias().empty()) node->setAlias({});
  }
};
...
bool isCompatible(ASTPtr &node) {
  if (auto *function = node->as<ASTFunction>()) {
    ...
    if (!function->arguments) throw Exception(ErrorCodes::LOGICAL_ERROR, "function->arguments is not set");

    String name = function->name;

    if (!(name == "and" || name == "or" || name == "not" || name == "equals" || name == "notEquals" || name == "less" ||
          name == "greater" || name == "lessOrEquals" || name == "greaterOrEquals" || name == "like" || name == "notLike" || name == "in" ||
          name == "notIn" || name == "isNull" || name == "isNotNull" || name == "tuple"))
      return false;
    ...
    for (auto &expr : function->arguments->children)
      if (!isCompatible(expr)) return false;
      ...
      if (lhs_is_tuple) {
        ...
        if (const auto *rhs_literal = rhs->as<ASTLiteral>()) {
          if (rhs_literal->value.getType() == Field::Types::Tuple) {
            ...
            bool is_single_row = !tup.empty() && tup[0].getType() != Field::Types::Tuple;
            if (is_single_row) rhs = makeASTFunction("", rhs);
          }
        }
      }
    }
...
void removeUnknownChildren(ASTs &children, const NameSet &known_names) {
  ...
  for (auto &child : children) {
    ...
    if (leave_child) new_children.push_back(child);
  }
...
bool removeUnknownSubexpressions(ASTPtr &node, const NameSet &known_names) {
  if (const auto *ident = node->as<ASTIdentifier>()) return known_names.contains(ident->name());

  if (node->as<ASTLiteral>() != nullptr) return true;
...
bool removeUnknownSubexpressionsFromWhere(ASTPtr &node, const NamesAndTypesList &available_columns) {
  ...
  NameSet known_names;
  for (const auto &col : available_columns) known_names.insert(col.name);
...
String transformQueryForExternalDatabaseImpl(ASTPtr clone_query, Names used_columns, const NamesAndTypesList &available_columns,
                                             IdentifierQuotingStyle identifier_quoting_style, LiteralEscapingStyle literal_escaping_style,
                                             const String &database, const String &table, ContextPtr context, std::optional<size_t> limit) {
  ...
  for (const auto &name : used_columns) select_expr_list->children.push_back(make_intrusive<ASTIdentifier>(name));
  ...
  if (limit) select->setExpression(ASTSelectQuery::Expression::LIMIT_LENGTH, make_intrusive<ASTLiteral>(*limit));
...
String transformQueryForExternalDatabase(const SelectQueryInfo &query_info, const Names &column_names,
                                         const NamesAndTypesList &available_columns, IdentifierQuotingStyle identifier_quoting_style,
                                         LiteralEscapingStyle literal_escaping_style, const String &database, const String &table,
                                         ContextPtr context, std::optional<size_t> limit) {
  if (!query_info.syntax_analyzer_result) {
    if (!query_info.query_tree) throw Exception(ErrorCodes::UNSUPPORTED_METHOD, "Query is not analyzed: no query tree");
    if (!query_info.planner_context) throw Exception(ErrorCodes::UNSUPPORTED_METHOD, "Query is not analyzed: no planner context");
    if (!query_info.table_expression) throw Exception(ErrorCodes::UNSUPPORTED_METHOD, "Query is not analyzed: no table expression");
    ...
    return transformQueryForExternalDatabaseImpl(clone_query, column_names, available_columns, identifier_quoting_style,
                                                 literal_escaping_style, database, table, context, limit);
  }

  auto clone_query = query_info.query->clone();
  return transformQueryForExternalDatabaseImpl(clone_query, query_info.syntax_analyzer_result->requiredSourceColumns(), available_columns,
                                               identifier_quoting_style, literal_escaping_style, database, table, context, limit);
}
