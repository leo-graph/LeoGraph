...
StatisticsType stringToStatisticsType(String type) {
  ...
  if (type == "tdigest") return StatisticsType::TDigest;
  if (type == "uniq") return StatisticsType::Uniq;
  if (type == "countmin") return StatisticsType::CountMinSketch;
  if (type == "minmax") return StatisticsType::MinMax;

  throw Exception(ErrorCodes::INCORRECT_QUERY,
                  "Unknown statistics type: {}. Supported statistics types are 'countmin', 'minmax', 'tdigest' and 'uniq'.", type);
}
...
String SingleStatisticsDescription::getTypeName() const { return statisticsTypeToString(type); }
...
bool ColumnStatisticsDescription::operator==(const ColumnStatisticsDescription &other) const {
  if (!data_type) return !other.data_type;

  if (!other.data_type) return false;

  return types_to_desc == other.types_to_desc && data_type->equals(*other.data_type);
}

bool ColumnStatisticsDescription::empty() const { return types_to_desc.empty(); }
...
void ColumnStatisticsDescription::merge(const ColumnStatisticsDescription &other, const String &merging_column_name,
                                        DataTypePtr merging_column_type, bool if_not_exists) {
  ...
  for (const auto &[stats_type, stats_desc] : other.types_to_desc) {
    ...
    if (!types_to_desc.contains(stats_type)) types_to_desc.emplace(stats_type, stats_desc);
  }
}
...
void ColumnStatisticsDescription::clear() { types_to_desc.clear(); }

std::vector<std::pair<String, ColumnStatisticsDescription>> ColumnStatisticsDescription::fromAST(const ASTPtr &definition_ast,
                                                                                                 const ColumnsDescription &columns) {
  const auto *stat_definition_ast = definition_ast->as<ASTStatisticsDeclaration>();
  if (!stat_definition_ast) throw Exception(ErrorCodes::LOGICAL_ERROR, "Cannot cast AST to ASTSingleStatisticsDeclaration");
  ...
  if (result.empty()) throw Exception(ErrorCodes::INCORRECT_QUERY, "Empty statistics column list is not allowed.");

  return result;
}

ColumnStatisticsDescription ColumnStatisticsDescription::fromStatisticsDescriptionAST(const ASTPtr &statistics_desc,
                                                                                      const String &column_name, DataTypePtr data_type) {
...
ASTPtr ColumnStatisticsDescription::getAST() const {
  ...
  for (const auto &[type, desc] : types_to_desc) {
    if (desc.ast == nullptr) throw Exception(ErrorCodes::LOGICAL_ERROR, "Unknown ast");

    if (!desc.is_implicit) function_node->arguments->children.push_back(desc.ast);
  }
