...
std::unique_lock<std::mutex> StorageBuffer::Buffer::lockForReading() const { return lockImpl(/* read= */ true); }
std::unique_lock<std::mutex> StorageBuffer::Buffer::lockForWriting() const { return lockImpl(/* read= */ false); }
...
StoragePtr StorageBuffer::getDestinationTable() const {
  ...
  if (destination.get() == this) throw Exception(ErrorCodes::INFINITE_LOOP, "Destination table is myself. Will lead to infinite loop.");

  return destination;
}

std::string StorageBuffer::Thresholds::toString() const { return fmt::format("time={}, rows={}, bytes={}", time, rows, bytes); }

StorageBuffer::StorageBuffer(const StorageID &table_id_, const ColumnsDescription &columns_, const ConstraintsDescription &constraints_,
                             const String &comment, ContextPtr context_, size_t num_shards_, const Thresholds &min_thresholds_,
                             const Thresholds &max_thresholds_, const Thresholds &flush_thresholds_, const StorageID &destination_id_,
  ...
  if (num_shards > 1) {
    flush_pool = std::make_unique<ThreadPool>(CurrentMetrics::StorageBufferFlushThreads, CurrentMetrics::StorageBufferFlushThreadsActive,
                                              CurrentMetrics::StorageBufferFlushThreadsScheduled, num_shards, 0, num_shards);
  }
  flush_handle = bg_pool.createTask(getStorageID(), log->name() + "/Bg", [this] { backgroundFlush(); });

  LOG_TRACE(log, "Buffer(flush: ({}), min: ({}), max: ({}))", flush_thresholds.toString(), min_thresholds.toString(),
            max_thresholds.toString());
}
...
class BufferSource : public ISource {
 ...
 protected:
  Chunk generate() override {
    ...
    if (has_been_read) return res;
    has_been_read = true;

    std::unique_lock lock(buffer.lockForReading());

    if (!buffer.data.rows() || buffer.metadata_version != metadata_version) return res;

    Columns columns;
    columns.reserve(column_names_and_types.size());

    for (const auto &elem : column_names_and_types) columns.emplace_back(getColumnFromBlock(buffer.data, elem));
...
QueryProcessingStage::Enum StorageBuffer::getQueryProcessingStage(ContextPtr local_context, QueryProcessingStage::Enum to_stage,
  ...
  if (auto destination = getDestinationTable()) {
    ...
    return destination->getQueryProcessingStage(local_context, to_stage,
                                                destination->getStorageSnapshot(destination_metadata, local_context), query_info);
  }
...
void StorageBuffer::read(QueryPlan &query_plan, const Names &column_names, const StorageSnapshotPtr &storage_snapshot,
      ...
      if (table_node.getStorageID().getFullNameNotQuoted() == current_storage_id.getFullNameNotQuoted()) {
        ...
        if (count_of_current_storage > 1)
          throw Exception(ErrorCodes::NOT_IMPLEMENTED,
                          "StorageBuffer over Distributed does not support using the same table more than once in the query");
      }
    }
  }
  ...
  if (auto destination = getDestinationTable()) {
    ...
    const bool dst_has_same_structure = std::all_of(
        column_names.begin(), column_names.end(), [metadata_snapshot, destination_metadata_snapshot](const String &column_name) {
          ...
          return dest_columm &&
                 dest_columm->type->equals(*our_columns.getColumnOrSubcolumn(GetColumnsOptions::AllPhysical, column_name).type);
        });
    ...
    if (dst_has_same_structure) {
      ...
      destination->read(query_plan, column_names, destination_snapshot, query_info, local_context, processed_stage, max_block_size,
                        num_streams);
    } else {
      ...
      for (const String &column_name : column_names) {
        if (!dest_columns.hasPhysical(column_name)) {
          LOG_WARNING(log, "Destination table {} doesn't have column {}. The default values are used.", destination_id.getNameForLogs(),
        ...
        if (!dst_col.type->equals(*col.type)) {
          LOG_WARNING(log, "Destination table {} has different type of column {} ({} != {}). Data from destination table are converted.",
                      destination_id.getNameForLogs(), backQuoteIfNeed(column_name), dst_col.type->getName(), col.type->getName());
          header_after_adding_defaults.getByName(column_name) = ColumnWithTypeAndName(dst_col.type, column_name);
        }
      }

      if (columns_intersection.empty()) {
        LOG_WARNING(log, "Destination table {} has no common columns with block in buffer. Block of data is skipped.",
        ...
        if (src_table_query_info.prewhere_info || src_table_query_info.row_level_filter) {
          converting_dag =
              ActionsDAG::makeConvertingActions(header_after_adding_defaults.getColumnsWithTypeAndName(),
                                                header.getColumnsWithTypeAndName(), ActionsDAG::MatchColumnsMode::Name, local_context);
        }
        ...
        if (src_table_query_info.row_level_filter) {
          ...
          row_level_filter->actions = ActionsDAG::merge(converting_dag.clone(), src_table_query_info.row_level_filter->actions.clone());
        ...
        if (src_table_query_info.prewhere_info) {
          src_table_query_info.prewhere_info = std::make_shared<PrewhereInfo>(src_table_query_info.prewhere_info->clone());
          {
            src_table_query_info.prewhere_info->prewhere_actions =
                ActionsDAG::merge(converting_dag.clone(), std::move(src_table_query_info.prewhere_info->prewhere_actions));
        ...
        destination->read(query_plan, columns_intersection, destination_snapshot, src_table_query_info, local_context, processed_stage,
        ...
        if (query_plan.isInitialized() && processed_stage <= QueryProcessingStage::FetchColumns) {
           ...
           */
          auto actions = addMissingDefaults(*query_plan.getCurrentHeader(), header_after_adding_defaults.getNamesAndTypesList(),
                                            metadata_snapshot->getColumns(), local_context);

          auto adding_missed = std::make_unique<ExpressionStep>(query_plan.getCurrentHeader(), std::move(actions));
  ...
  Pipe pipe_from_buffers;
  {
    ...
    for (auto &buf : buffers) pipes_from_buffers.emplace_back(std::make_shared<BufferSource>(column_names, buf, storage_snapshot));
  ...
  if (pipe_from_buffers.empty()) return;
  ...
  if (processed_stage > QueryProcessingStage::FetchColumns) {
    if (enable_analyzer) {
      auto storage = std::make_shared<StorageValues>(getStorageID(), storage_snapshot->getAllColumnsDescription(),
                                                     std::move(pipe_from_buffers), *getVirtualsPtr());

      auto interpreter = InterpreterSelectQueryAnalyzer(query_info.query, local_context, SelectQueryOptions(processed_stage), storage);
      interpreter.addStorageLimits(*query_info.storage_limits);
      buffers_plan = std::move(interpreter).extractQueryPlan();
    } else {
      auto interpreter =
          InterpreterSelectQuery(query_info.query, local_context, std::move(pipe_from_buffers), SelectQueryOptions(processed_stage));
      interpreter.addStorageLimits(*query_info.storage_limits);
    ...
    if (query_info.row_level_filter) {
      ...
      pipe_from_buffers.addSimpleTransform([&](const SharedHeader &header) {
        return std::make_shared<FilterTransform>(header, actions, query_info.row_level_filter->column_name,
    ...
    if (query_info.prewhere_info) {
      ...
      pipe_from_buffers.addSimpleTransform([&](const SharedHeader &header) {
        return std::make_shared<FilterTransform>(header, actions, query_info.prewhere_info->prewhere_column_name,
                                                 query_info.prewhere_info->remove_prewhere_column);
      });
    }

    for (const auto &processor : pipe_from_buffers.getProcessors()) processor->setStorageLimits(query_info.storage_limits);
  ...
  if (!blocksHaveEqualStructure(*query_plan.getCurrentHeader(), *result_header)) {
    auto convert_actions_dag =
        ActionsDAG::makeConvertingActions(query_plan.getCurrentHeader()->getColumnsWithTypeAndName(),
                                          result_header->getColumnsWithTypeAndName(), ActionsDAG::MatchColumnsMode::Name, local_context);
...
static void appendBlock(LoggerPtr log, const Block &from, Block &to) {
  ...
  if (to.empty()) to = from.cloneEmpty();
  ...
  try {
    ...
    try {
      for (size_t column_no = 0; column_no < mutated_columns; ++column_no) {
        ...
        if (!col_to) throw Exception(ErrorCodes::LOGICAL_ERROR, "No column to rollback");
...
class BufferSink : public SinkToStorage, WithContext {
 public:
  explicit BufferSink(StorageBuffer &storage_, const StorageMetadataPtr &metadata_snapshot_, const ContextPtr &context_)
 ...
 private:
  ...
  void insertIntoBuffer(const Block &block, StorageBuffer::Buffer &buffer, int32_t metadata_version) {
    ...
    if (!buffer.first_write_time) buffer.first_write_time = current_time;
...
SinkToStoragePtr StorageBuffer::write(const ASTPtr & /*query*/, const StorageMetadataPtr &metadata_snapshot, ContextPtr local_context,
                                      bool /*async_insert*/) {
...
void StorageBuffer::flushAndPrepareForShutdown() {
  if (!flush_handle) return;

  flush_handle->deactivate();

  try {
    optimize(nullptr /*query*/, getInMemoryMetadataPtr(), {} /*partition*/, false /*final*/, false /*deduplicate*/, {}, false /*cleanup*/,
             getContext());
  } catch (...) {
 ...
 */
bool StorageBuffer::optimize(const ASTPtr & /*query*/, const StorageMetadataPtr & /*metadata_snapshot*/, const ASTPtr &partition,
                             bool final, bool deduplicate, const Names & /* deduplicate_by_columns */, bool cleanup,
                             ContextPtr /*context*/) {
  if (partition) throw Exception(ErrorCodes::NOT_IMPLEMENTED, "Partition cannot be specified when optimizing table of type Buffer");

  if (final) throw Exception(ErrorCodes::NOT_IMPLEMENTED, "FINAL cannot be specified when optimizing table of type Buffer");

  if (deduplicate) throw Exception(ErrorCodes::NOT_IMPLEMENTED, "DEDUPLICATE cannot be specified when optimizing table of type Buffer");

  if (cleanup) throw Exception(ErrorCodes::NOT_IMPLEMENTED, "CLEANUP cannot be specified when optimizing table of type Buffer");
...
bool StorageBuffer::supportsPrewhere() const {
  if (auto destination = getDestinationTable()) return destination->supportsPrewhere();
  return false;
}

bool StorageBuffer::checkThresholds(const Buffer &buffer, bool direct, time_t current_time, size_t additional_rows,
                                    size_t additional_bytes) const {
  time_t time_passed = 0;
  if (buffer.first_write_time) time_passed = current_time - buffer.first_write_time;
...
void StorageBuffer::flushAllBuffers(bool check_thresholds) {
  ...
  if (flush_pool) runner.emplace(*flush_pool, ThreadName::BACKGROUND_BUFFER_FLUSH_SCHEDULE_POOL);
  for (auto &buf : buffers) {
    if (runner) {
      /// Passing buf as a reference is fine since it's a reference to this, which outlives the runner
      runner->enqueueAndKeepTrack([this, &buf, check_thresholds]() { flushBuffer(buf, check_thresholds, false); });
    } else {
      flushBuffer(buf, check_thresholds, false);
    }
  }
  if (runner) runner->waitForAllToFinishAndRethrowFirstError();
}
...
bool StorageBuffer::flushBuffer(Buffer &buffer, bool check_thresholds, bool locked) {
  ...
  if (!locked) lock.emplace(buffer.lockForReading());
  ...
  if (check_thresholds) {
    if (!checkThresholdsImpl(/* direct= */ false, rows, bytes, time_passed)) return false;
  }
  ...
  if (!destination_id) {
    ...
    LOG_DEBUG(log, "Flushing buffer with {} rows (discarded), {} bytes, age {} seconds {}.", rows, bytes, time_passed,
              (check_thresholds ? "(bg)" : "(direct)"));
    return true;
  }
  ...
  LOG_DEBUG(log, "Flushing buffer with {} rows, {} bytes, age {} seconds, took {} ms {}.", rows, bytes, time_passed, milliseconds,
            (check_thresholds ? "(bg)" : "(direct)"));
  return true;
}

void StorageBuffer::writeBlockToDestination(const Block &block, StoragePtr table) {
  if (!destination_id || block.empty()) return;
  ...
  Block structure_of_destination_table =
      allow_materialized ? destination_metadata_snapshot->getSampleBlock() : destination_metadata_snapshot->getSampleBlockNonMaterialized();
  Block block_to_write;
  for (size_t i : collections::range(0, structure_of_destination_table.columns())) {
    ...
    if (block.has(dst_col.name)) {
      ...
      if (!column.type->equals(*dst_col.type)) {
        LOG_WARNING(log, "Destination table {} have different type of column {} ({} != {}). Block of data is converted.",
                    destination_id.getNameForLogs(), backQuoteIfNeed(column.name), dst_col.type->getName(), column.type->getName());
  ...
  for (const auto &column : block_to_write) list_of_columns->children.push_back(make_intrusive<ASTIdentifier>(column.name));
...
void StorageBuffer::reschedule(size_t min_delay) {
  ...
  LOG_TRACE(log, "Reschedule in {} sec (processed buffers: {}, rows in processed buffers: {}, time passed: {})", reschedule_sec,
            processed_buffers, rows, time_passed);
}
...
std::optional<UInt64> StorageBuffer::totalRows(ContextPtr query_context) const {
  ...
  if (auto destination = getDestinationTable()) underlying_rows = destination->totalRows(query_context);

  return total_writes.rows + underlying_rows.value_or(0);
}

std::optional<UInt64> StorageBuffer::totalBytes(ContextPtr) const { return total_writes.bytes; }
...
void StorageBuffer::alter(const AlterCommands &params, ContextPtr local_context, AlterLockHolder &) {
  ...
  optimize({} /*query*/, metadata_snapshot, {} /*partition_id*/, false /*final*/, false /*deduplicate*/, {}, false /*cleanup*/,
           local_context);
...
UInt64 checkUnderflowAndGetUInt64(const ASTPtr &arg, const String &arg_name) {
  ...
  if (value.getType() != Field::Types::UInt64) {
    ...
    if (val < 0) {
      throw Exception(ErrorCodes::BAD_ARGUMENTS, "Argument '{}' must be non-negative value, get {}", arg_name, val);
    }
  }
...
void registerStorageBuffer(StorageFactory &factory) {
  ...
  factory.registerStorage(
      ...
      [](const StorageFactory::Arguments &args) {
        ...
        for (size_t i = 0; i < engine_args.size(); ++i) {
          if (!typeid_cast<ASTLiteral *>(engine_args[i].get())) {
            throw Exception(ErrorCodes::BAD_ARGUMENTS,
                            ...
                            " instead",
                            i, engine_args[i]->formatForErrorMessage());
          }
        }
        ...
        if (num_buckets == 0) {
          throw Exception(ErrorCodes::BAD_ARGUMENTS, "Argument 'num_buckets' must be a positive integer, got '{}'", num_buckets);
        }
        ...
        if (engine_args.size() > i) flush.time = checkUnderflowAndGetUInt64(engine_args[i++], "flush_time");
        if (engine_args.size() > i) flush.rows = checkUnderflowAndGetUInt64(engine_args[i++], "flush_rows");
        if (engine_args.size() > i) flush.bytes = checkUnderflowAndGetUInt64(engine_args[i++], "flush_bytes");
        ...
        return std::make_shared<StorageBuffer>(
            args.table_id, args.columns, args.constraints, args.comment, args.getContext(), num_buckets, min, max, flush, destination_id,
