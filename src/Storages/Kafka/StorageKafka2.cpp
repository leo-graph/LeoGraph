...
StorageKafka2::StorageKafka2(const StorageID &table_id_, ContextPtr context_, const ColumnsDescription &columns_, const String &comment,
                             std::unique_ptr<KafkaSettings> kafka_settings_, const String &collection_name_)
    : IStorage(table_id_),
      ...
      client_id((*kafka_settings)[KafkaSetting::kafka_client_id].value.empty()
  ...
  if (!first_replica) createReplica();

  activating_task =
      getContext()->getSchedulePool().createTask(getStorageID(), log->name() + " (activating task)", [this]() { activateAndReschedule(); });
...
bool StorageKafka2::activate() {
  ...
  auto activate_in_keeper = [this]() {
    try {
      ...
      replica_is_active_node = nullptr;
      if (e.code() != ErrorCodes::REPLICA_IS_ALREADY_ACTIVE) throw;
...
void StorageKafka2::assertActive() const {
  ...
  if (!is_active) throw Exception(ErrorCodes::LOGICAL_ERROR, "Table is not active (replica path: {})", replica_path);
}

Pipe StorageKafka2::read(const Names & /*column_names */, const StorageSnapshotPtr & /* storage_snapshot */,
                         SelectQueryInfo & /* query_info */, ContextPtr /* local_context */,
                         QueryProcessingStage::Enum /* processed_stage */, size_t /* max_block_size */, size_t /* num_streams */) {
  throw Exception(ErrorCodes::NOT_IMPLEMENTED, "Direct read from the new Kafka storage is not implemented");
}

StreamingHandleErrorMode StorageKafka2::getHandleKafkaErrorMode() const { return (*kafka_settings)[KafkaSetting::kafka_handle_error_mode]; }

SinkToStoragePtr StorageKafka2::write(const ASTPtr &, const StorageMetadataPtr &metadata_snapshot, ContextPtr local_context,
                                      bool /*async_insert*/) {
  ...
  if (topics.size() > 1) throw Exception(ErrorCodes::NOT_IMPLEMENTED, "Can't write to Kafka table with multiple topics!");
  ...
  auto producer = std::make_unique<KafkaProducer>(std::make_shared<cppkafka::Producer>(conf), topics[0],
                                                  std::chrono::milliseconds(poll_timeout), shutdown_called, header);
  ...
  return std::make_shared<MessageQueueSink>(std::make_shared<const Block>(std::move(header)), getFormatName(), max_rows,
                                            std::move(producer), getName(), modified_context);
}
...
void StorageKafka2::drop() { dropReplica(); }
...
cppkafka::Configuration StorageKafka2::getConsumerConfiguration(size_t consumer_number, IKafkaExceptionInfoSinkPtr exception_sink) {
  KafkaConfigLoader::ConsumerConfigParams params{{getContext()->getConfigRef(), collection_name, topics, log},
                                                 brokers,
                                                 group,
                                                 num_consumers > 1,
                                                 consumer_number,
  ...
  // It is disabled, because in case of no materialized views are attached, it can cause live memory leak. To enable it, a similar cleanup
  // mechanism must be introduced as for StorageKafka.
...
size_t StorageKafka2::getSchemaRegistrySkipBytes() const { return (*kafka_settings)[KafkaSetting::kafka_schema_registry_skip_bytes].value; }
...
bool StorageKafka2::removeTableNodesFromZooKeeper(zkutil::ZooKeeperPtr keeper_to_use, const zkutil::EphemeralNodeHolder::Ptr &drop_lock) {
  ...
  for (const auto &child : children)
    if (child != "dropped") keeper_to_use->tryRemoveRecursive(fs_keeper_path / child);
...
void StorageKafka2::dropReplica() {
  ...
  Strings replicas;
  if (Coordination::Error::ZOK != my_keeper->tryGetChildren(keeper_path + "/replicas", replicas) || !replicas.empty()) return;
...
std::optional<StorageKafka2::BlocksAndGuard> StorageKafka2::pollConsumer(KeeperHandlingConsumer &consumer, const Stopwatch &watch,
  ...
  auto input_format =
      FormatFactory::instance().getInput(getFormatName(), empty_buf, non_virtual_header, modified_context, getMaxBlockSize(), std::nullopt,
  ...
  auto on_error = [&](const MutableColumns &result_columns, const ColumnCheckpoints &checkpoints, Exception &e) {
    ...
    switch (getHandleKafkaErrorMode()) {
      ...
      case StreamingHandleErrorMode::DEFAULT: {
        e.addMessage("while parsing Kafka message (topic: {}, partition: {}, offset: {})'", current_msg_info->currentTopic(),
                     current_msg_info->currentPartition(), current_msg_info->currentOffset());
        throw std::move(e);
      }
    }
  };
  ...
  const auto check_time_limit = [&max_execution_time, &watch]() {
    if (max_execution_time != 0) {
      ...
      if (elapsed_ns > static_cast<UInt64>(max_execution_time.totalMicroseconds()) * 1000) return false;
    }

    return true;
  };

  KeeperHandlingConsumer::MessageSinkFunction msg_sink = [&](ReadBufferPtr buf, const KeeperHandlingConsumer::MessageInfo &msg_info,
                                                             bool has_more_polled_messages, bool stalled) mutable {
    size_t new_rows = 0;
    ...
    if (new_rows || is_dead_letter) {
      ...
      if (is_dead_letter) {
        ...
        if (!dead_letter_queue)
          ...
          dead_letter_queue->add(
              DeadLetterQueueElement{.table_engine = DeadLetterQueueElement::StreamType::Kafka,
                                     ...
                                     .details = DeadLetterQueueElement::KafkaDetails{.topic_name = msg_info.currentTopic(),
      ...
      LOG_DEBUG(log, "Parsing of message (topic: {}, partition: {}, offset: {}) return no rows.", msg_info.currentTopic(),
                msg_info.currentPartition(), msg_info.currentOffset());
    }

    if (!has_more_polled_messages &&
        (total_rows >= getMaxBlockSize() || !check_time_limit() || failed_poll_attempts >= MAX_FAILED_POLL_ATTEMPTS)) {
      LOG_TRACE(log, "Stopped collecting message for current batch. There are {} failed polled attempts, {} total rows",
                failed_poll_attempts, total_rows);
      return true;
    }
    return false;
  };
  ...
  if (!maybe_guard.has_value() || total_rows == 0) return {};
  ...
  for (const auto &column : virtual_block.getColumnsWithTypeAndName()) result_block.insert(column);
    ...
    if (num_views) {
      ...
      while (!task->stream_cancelled && num_created_consumers > 0) {
        ...
        if (maybe_stall_reason = streamToViews(idx); maybe_stall_reason.has_value()) {
          LOG_TRACE(log, "Stream stalled. Rescheduling in {} ms",
        ...
        if (duration.count() > KAFKA_MAX_THREAD_WORK_DURATION_MS) {
          LOG_TRACE(log, "Thread work duration limit exceeded. Rescheduling in {} ms",
                    (*kafka_settings)[KafkaSetting::kafka_consumer_reschedule_ms].totalMilliseconds());
          break;
        }
      }
...
std::optional<StorageKafka2::StallKind> StorageKafka2::streamToViews(size_t idx) {
  ...
  try {
    if (consumer->needsNewKeeper()) consumer->setKeeper(getZooKeeperAndAssertActive());

    if (const auto cannot_poll_reason = consumer->prepareToPoll(); cannot_poll_reason.has_value()) return getStallKind(*cannot_poll_reason);
...
StorageKafka2::KeeperHandlingConsumerPtr StorageKafka2::acquireConsumer(size_t idx) {
  ...
  const auto created_consumer = consumer->startUsing(
      [&](IKafkaExceptionInfoSinkPtr exception_sink) { return getConsumerConfiguration(idx, std::move(exception_sink)); });

  if (created_consumer) LOG_TRACE(log, "Created #{} consumer", idx);
...
std::optional<size_t> StorageKafka2::streamFromConsumer(KeeperHandlingConsumer &consumer, const Stopwatch &watch) {
  ...
  InterpreterInsertQuery interpreter(insert, modified_context,
  ...
  auto converting_dag = ActionsDAG::makeConvertingActions(blocks.front().cloneEmpty().getColumnsWithTypeAndName(),
                                                          block_io.pipeline.getHeader().getColumnsWithTypeAndName(),
                                                          ActionsDAG::MatchColumnsMode::Name, modified_context);

  auto converting_actions = std::make_shared<ExpressionActions>(std::move(converting_dag));

  for (auto &block : blocks) converting_actions->execute(block);
