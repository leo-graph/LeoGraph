...
class ReadFromStorageKafka final : public ReadFromStreamLikeEngine {
 public:
  ReadFromStorageKafka(const Names &column_names_, StoragePtr storage_, const StorageSnapshotPtr &storage_snapshot_,
 ...
 private:
  Pipe makePipe() final {
    ...
    if (kafka_storage.shutdown_called) throw Exception(ErrorCodes::ABORTED, "Table is detached");
    ...
    for (size_t i = 0; i < kafka_storage.num_consumers; ++i) {
      ...
      pipes.emplace_back(std::make_shared<KafkaSource>(kafka_storage, storage_snapshot, modified_context, column_names, kafka_storage.log,
                                                       1, (*kafka_storage.kafka_settings)[KafkaSetting::kafka_commit_on_select]));
    }
...
StorageKafka::StorageKafka(const StorageID &table_id_, ContextPtr context_, const ColumnsDescription &columns_, const String &comment,
  ...
  for (size_t i = 0; i < num_consumers; ++i) consumers[i] = createKafkaConsumer(i);
...
StorageKafka::~StorageKafka() {
  if (!shutdown_called) shutdown(false);
}

void StorageKafka::read(QueryPlan &query_plan, const Names &column_names, const StorageSnapshotPtr &storage_snapshot,
  ...
  query_plan.addStep(
      std::make_unique<ReadFromStorageKafka>(column_names, shared_from_this(), storage_snapshot, query_info, std::move(query_context)));
}
...
void StorageKafka::shutdown(bool) {
  ...
  for (auto &task : tasks) task->stream_cancelled = true;
...
void StorageKafka::cleanConsumers() {
  /// We need to clear the cppkafka::Consumer separately from KafkaConsumer, since cppkafka::Consumer holds a weak_ptr to the KafkaConsumer
  /// (for logging callback) So if we will remove cppkafka::Consumer from KafkaConsumer destructor, then due to librdkafka will call the
  /// logging again from destructor, it will lead to a deadlock
...
KafkaConsumerPtr StorageKafka::popConsumer(std::chrono::milliseconds timeout) {
  ...
  if (shutdown_called) throw Exception(ErrorCodes::ABORTED, "Table is detached");
  ...
  for (size_t i = 0; i < consumers.size(); ++i) {
    ...
    if (consumer_ptr->isInUse()) continue;
  ...
  /// 3. There is no free consumer and num_consumers already created, waiting @timeout.
    ...
    cv.wait_for(lock, timeout, [&]() {
      ...
      auto it = std::find_if(consumers.begin(), consumers.end(), [](const auto &ptr) { return !ptr->isInUse() && ptr->hasConsumer(); });
      if (it != consumers.end()) {
        ret_consumer_ptr = *it;
        return true;
      }
...
KafkaConsumerPtr StorageKafka::createKafkaConsumer(size_t consumer_number) {
  ...
  KafkaConsumerPtr kafka_consumer_ptr = std::make_shared<KafkaConsumer>(
      log, getPollMaxBatchSize(), getPollTimeoutMillisecond(), intermediate_commit, stream_cancelled, topics, getSchemaRegistrySkipBytes());
  ...
  while (!cleanup_cv.wait_for(lock, timeout, [this]() { return shutdown_called == true; })) {
    ...
    UInt64 now_usec = timeInMicroseconds(std::chrono::system_clock::now());
    {
      for (size_t i = 0; i < consumers.size(); ++i) {
        ...
        if (!consumer_ptr->hasConsumer()) continue;
        if (consumer_ptr->isInUse()) continue;
    ...
    if (!consumers_to_close.empty()) {
      ...
      consumers_to_close.clear();
      LOG_TRACE(log, "{} consumers had been closed (due to {} usec timeout). Took {} ms.", closed, ttl_usec, watch.elapsedMilliseconds());
...
size_t StorageKafka::getSchemaRegistrySkipBytes() const { return (*kafka_settings)[KafkaSetting::kafka_schema_registry_skip_bytes].value; }
    ...
    if (num_views) {
      ...
      while (!task->stream_cancelled) {
        ...
        if (some_stream_is_stalled) {
          LOG_TRACE(log, "Stream(s) stalled. Rescheduling in {} ms.",
...
bool StorageKafka::streamToViews() {
  ...
  InterpreterInsertQuery interpreter(insert, kafka_context,
  ...
  for (size_t i = 0; i < stream_count; ++i) {
    auto source = std::make_shared<KafkaSource>(*this, storage_snapshot, kafka_context, block_io.pipeline.getHeader().getNames(), log,
                                                block_size, false);
  ...
  LOG_DEBUG(log, "Pushing {} rows to {} took {} ms.", formatReadableQuantity(rows.load()), table_id.getNameForLogs(), milliseconds);
