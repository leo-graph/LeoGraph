...
KafkaSource::KafkaSource(StorageKafka &storage_, const StorageSnapshotPtr &storage_snapshot_, const ContextPtr &context_,
                         const Names &columns, LoggerPtr log_, size_t max_block_size_, bool commit_in_suffix_)
...
Chunk KafkaSource::generateImpl() {
  ...
  auto on_error = [&](const MutableColumns &result_columns, const ColumnCheckpoints &checkpoints, Exception &e) {
    ...
    switch (handle_error_mode) {
      ...
      case StreamingHandleErrorMode::DEFAULT: {
        e.addMessage("while parsing Kafka message (topic: {}, partition: {}, offset: {})'", consumer->currentTopic(),
                     consumer->currentPartition(), consumer->currentOffset());
  ...
  while (true) {
    ...
    if (new_rows || is_dead_letter) {
      ...
      if (consumer->isStalled()) throw Exception(ErrorCodes::LOGICAL_ERROR, "Polled messages became unusable");
        ...
        if (!dead_letter_queue)
          ...
          dead_letter_queue->add(
              DeadLetterQueueElement{.table_engine = DeadLetterQueueElement::StreamType::Kafka,
                                     ...
                                     .details = DeadLetterQueueElement::KafkaDetails{.topic_name = consumer->currentTopic(),
      ...
      consumer->storeLastReadMessageOffset();
      LOG_DEBUG(log, "Parsing of message (topic: {}, partition: {}, offset: {}) return no rows.", consumer->currentTopic(),
                consumer->currentPartition(), consumer->currentOffset());
    }
  ...
  auto converting_dag =
      ActionsDAG::makeConvertingActions(result_block.cloneEmpty().getColumnsWithTypeAndName(),
                                        getPort().getHeader().getColumnsWithTypeAndName(), ActionsDAG::MatchColumnsMode::Name, context);
...
Chunk KafkaSource::generate() {
  ...
  if (!chunk && commit_in_suffix) commit();
