...
std::optional<int64_t> getNumber(zkutil::ZooKeeper& keeper, const fs::path& path) {
  std::string result;
  if (!keeper.tryGet(path, result)) return std::nullopt;

  return DB::parse<int64_t>(result);
}
}  // namespace

KeeperHandlingConsumer::MessageInfo::MessageInfo(const KafkaConsumer2& kafka_consumer_) : kafka_consumer(kafka_consumer_) {}

String KeeperHandlingConsumer::MessageInfo::currentTopic() const { return kafka_consumer.currentTopic(); }

int32_t KeeperHandlingConsumer::MessageInfo::currentPartition() const { return kafka_consumer.currentPartition(); }

int64_t KeeperHandlingConsumer::MessageInfo::currentOffset() const { return kafka_consumer.currentOffset(); }

String KeeperHandlingConsumer::MessageInfo::currentKey() const { return kafka_consumer.currentKey(); }

String KeeperHandlingConsumer::MessageInfo::currentPayload() const { return kafka_consumer.currentPayload(); }
...
KeeperHandlingConsumer::OffsetGuard::OffsetGuard(KeeperHandlingConsumer& consumer_, const int64_t new_offset_)
    : consumer(&consumer_), new_offset(new_offset_) {}

KeeperHandlingConsumer::OffsetGuard::OffsetGuard(OffsetGuard&& other) noexcept
    : needs_rollback(other.needs_rollback), consumer(other.consumer), new_offset(other.new_offset) {
  if (&other != this) {
...
KeeperHandlingConsumer::OffsetGuard::~OffsetGuard() {
  if (consumer && needs_rollback) consumer->rollbackToCommittedOffsets();
}
...
KeeperHandlingConsumer::KeeperHandlingConsumer(const KafkaConsumer2Ptr& kafka_consumer_, const std::shared_ptr<zkutil::ZooKeeper>& keeper_,
                                               const std::filesystem::path& keeper_path_, const String& replica_name_, size_t idx_,
                                               const LoggerPtr& log_)
    : keeper_path(keeper_path_), replica_name(replica_name_), idx(idx_), kafka_consumer(kafka_consumer_), keeper(keeper_), log(log_) {
  if (!keeper) throw Exception(ErrorCodes::LOGICAL_ERROR, "KeeperHandlingConsumer requires a valid ZooKeeper instance");
}

bool KeeperHandlingConsumer::needsNewKeeper() const { return keeper->expired(); }
...
std::optional<KeeperHandlingConsumer::CannotPollReason> KeeperHandlingConsumer::prepareToPoll() {
  ...
  if (!assigned_topic_partitions.empty() && poll_count <= LOCKS_REFRESH_POLLS) return std::nullopt;

  assigned_topic_partitions.clear();
  // Clear temporary locks to give a chance to lock them as permanent locks and to make it possible to gather available topic partitions
  // only once.
  ...
  {
    std::lock_guard lock(topic_partition_locks_mutex);
    // These operations are expected to be fast, because they only modify in-memory data and read/write to Keeper. For huge number of topic
    // partitions
...
KeeperHandlingConsumer::LockedTopicPartitionInfo& KeeperHandlingConsumer::getTopicPartitionLockLocked(const TopicPartition& topic_partition)
    TSA_REQUIRES(topic_partition_locks_mutex) {
  auto locks_it = permanent_locks.find(topic_partition);

  if (locks_it != permanent_locks.end()) return locks_it->second;

  locks_it = tmp_locks.find(topic_partition);

  if (locks_it != tmp_locks.end()) return locks_it->second;

  throw Exception(ErrorCodes::LOGICAL_ERROR, "Cannot find locks for topic partition {}:{}", topic_partition.topic,
...
KeeperHandlingConsumer::getAvailableTopicPartitions(const TopicPartitionOffsets& all_topic_partitions) {
  ...
  for (const auto& partition : all_topic_partitions) {
    if (!already_locked_partitions.contains(partition)) available_topic_partitions.push_back(partition);
  }
...
std::optional<KeeperHandlingConsumer::LockedTopicPartitionInfo> KeeperHandlingConsumer::createLocksInfoIfFree(
    const TopicPartition& partition_to_lock) {
  ...
  try {
    ...
    LockedTopicPartitionInfo lock_info{EphemeralNodeHolder::create(lock_file_path, *keeper, replica_name),
                                       getNumber(*keeper, topic_partition_path / commit_file_name),
                                       getNumber(*keeper, topic_partition_path / intent_file_name)};

    LOG_TRACE(log, "Locked topic partition: {}:{} at offset (present: {}) {} with intent size (present: {}) {}", partition_to_lock.topic,
              partition_to_lock.partition_id, lock_info.committed_offset.has_value(), lock_info.committed_offset.value_or(0),
              lock_info.intent_size.has_value(), lock_info.intent_size.value_or(0));

    return lock_info;
  } catch (const Coordination::Exception& e) {
    if (e.code == Coordination::Error::ZNODEEXISTS) {
      LOG_TRACE(log, "Skipping lock for topic partition {}:{} because it already exists", partition_to_lock.topic,
...
void KeeperHandlingConsumer::lockTemporaryLocksLocked(const TopicPartitions& available_topic_partitions,
                                                      const bool has_replica_without_locks) TSA_REQUIRES(topic_partition_locks_mutex) {
  ...
  for (const auto& tp : available_topic_partitions) {
    if (tmp_locks.size() >= tmp_locks_quota) break;
    auto maybe_lock = createLocksInfoIfFree(tp);
    if (!maybe_lock.has_value()) continue;
...
void KeeperHandlingConsumer::updatePermanentLocksLocked(const TopicPartitions& available_topic_partitions,
                                                        const size_t topic_partitions_count, const size_t active_replica_count)
    TSA_REQUIRES(topic_partition_locks_mutex) {
  ...
  if (can_lock_partitions < permanent_locks.size()) {
    ...
    for (size_t i = 0; i < need_to_unlock && permanent_locks_it != permanent_locks.end(); ++i) {
      LOG_TEST(log, "Releasing topic partition lock for [{}:{}] at offset", permanent_locks_it->first.topic,
...
void KeeperHandlingConsumer::saveIntentSize(const KafkaConsumer2::TopicPartition& topic_partition, const std::optional<int64_t>& offset,
  ...
  LOG_TEST(log, "Saving intent of {} for topic-partition [{}:{}] at offset {}", intent, topic_partition.topic, topic_partition.partition_id,
...
void KeeperHandlingConsumer::saveCommittedOffset(const int64_t new_offset) {
  if (topic_partition_index_to_consume_from >= assigned_topic_partitions.size())
    throw Exception(ErrorCodes::LOGICAL_ERROR,
                    ...
                    topic_partition_index_to_consume_from, assigned_topic_partitions.size());
  ...
  LOG_TEST(log, "Saved offset {} for topic-partition [{}:{}]", new_offset, topic_partition.topic, topic_partition.partition_id);
...
bool KeeperHandlingConsumer::startUsing(const ConfigCreator& config_creator) {
  ...
  if (create_consumer) kafka_consumer->createConsumer(config_creator(kafka_consumer));
...
std::optional<KeeperHandlingConsumer::OffsetGuard> KeeperHandlingConsumer::poll(MessageSinkFunction& message_sink) {
  ...
  LOG_TRACE(log, "Will fetch {}:{} (topic_partition_index_to_consume_from is {})", topic_partition.topic, topic_partition.partition_id,
  ...
  while (true) {
    ...
    if (message_sink(buf, message_info, kafka_consumer->hasMorePolledMessages(), kafka_consumer->isStalled())) {
      if (consumed_messages == 0) return std::nullopt;
...
StorageKafkaUtils::ConsumerStatistics KeeperHandlingConsumer::getStat() const {
  ...
  using CommittedOffsetAndIntentSizes = std::unordered_map<KafkaConsumer2::TopicPartition, CommittedOffsetAndIntentSize,
                                                           KafkaConsumer2::TopicPartitionHash, KafkaConsumer2::TopicPartitionEquality>;
  ...
  KafkaConsumer2::Stat consumer_stat;
  {
    ...
    for (const auto& [topic_partition, info] : permanent_locks)
      committed_offsets_and_intent_sizes[topic_partition] = {info.committed_offset.value_or(KafkaConsumer2::INVALID_OFFSET),
                                                             info.intent_size};
  ...
  for (const auto& [topic_partition, committed_offset_and_intent_size] : committed_offsets_and_intent_sizes) {
    ...
    if (auto it = consumer_stat.current_offsets.find(topic_partition); it != consumer_stat.current_offsets.end()) offset = it->second;
...
IKafkaExceptionInfoSinkPtr KeeperHandlingConsumer::getExceptionInfoSink() const { return kafka_consumer; }
...
void KeeperHandlingConsumer::appendToAssignedTopicPartitions(const TopicPartitionLocks& locks) {
  for (const auto& [topic_partition, info] : locks) assigned_topic_partitions.push_back(topic_partition);
}
}  // namespace DB
