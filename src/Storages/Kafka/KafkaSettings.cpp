...
#define KAFKA_RELATED_SETTINGS(DECLARE, ALIAS)                                                                                             \
  ...
  DECLARE(String, kafka_group_name, "", "Client group id string. All Kafka consumers sharing the same group.id belong to the same group.", \
          0)                                                                                                                               \
  ...
  DECLARE(Bool, kafka_commit_every_batch, false,                                                                                           \
          "Commit every consumed and handled batch instead of a single commit after writing a whole block", 0)                             \
  ...
  DECLARE(StreamingHandleErrorMode, kafka_handle_error_mode, StreamingHandleErrorMode::DEFAULT,                                            \
          "How to handle errors for Kafka engine. Possible values: default (throw an exception after kafka_skip_broken_messages broken "   \
          "messages), stream (save broken messages and errors in virtual columns _raw_message, _error), dead_letter_queue (error related " \
          "data will be saved in system.dead_letter).",                                                                                    \
          0)                                                                                                                               \
  ...
  DECLARE(String, kafka_sasl_mechanism, "",                                                                                                \
          "SASL mechanism to use for authentication. Supported: GSSAPI, PLAIN, SCRAM-SHA-256, SCRAM-SHA-512, OAUTHBEARER.", 0)             \
  ...
  DECLARE(                                                                                                                                 \
      String, kafka_compression_codec, "",                                                                                                 \
      "Compression codec used for producing messages. Supported: empty string, none, gzip, snappy, lz4, zstd. In case of empty string "    \
      "the compression codec is not set by the table, thus values from the config files or default value from `librdkafka` will be used.", \
      0)                                                                                                                                   \
  DECLARE(Int64, kafka_compression_level, -1,                                                                                              \
          "Compression level parameter for algorithm selected by kafka_compression_codec. Higher values will result in better "            \
          "compression at the cost of more CPU usage. Usable range is algorithm-dependent: [0-9] for gzip; [0-12] for lz4; only 0 for "    \
          "snappy; [0-12] for zstd; -1 = codec-dependent default compression level.",                                                      \
          0)                                                                                                                               \
  DECLARE(UInt64, kafka_schema_registry_skip_bytes, 0,                                                                                     \
          "Number of bytes to skip from the beginning of each Kafka message (e.g., 5 for Confluent Schema Registry, 19 for AWS Glue "      \
          "Schema Registry envelope header). Maximum: 255 bytes.",                                                                         \
          0)

#define OBSOLETE_KAFKA_SETTINGS(M, ALIAS) MAKE_OBSOLETE(M, Char, kafka_row_delimiter, '\0')
...
void KafkaSettings::loadFromNamedCollection(const MutableNamedCollectionPtr& named_collection) {
  for (const auto& setting : impl->all()) {
    ...
    if (named_collection->has(setting_name)) impl->set(setting_name, named_collection->get<String>(setting_name));
  }
}
...
void KafkaSettings::sanityCheck(ContextPtr global_context) const {
  ...
  if (impl->kafka_consumers_pool_ttl_ms < kafka_consumer_reschedule_ms)
    throw Exception(ErrorCodes::BAD_ARGUMENTS,
                    ...
                    impl->kafka_consumers_pool_ttl_ms.value, kafka_consumer_reschedule_ms);
  ...
  if (impl->kafka_consumers_pool_ttl_ms > KAFKA_CONSUMERS_POOL_TTL_MS_MAX)
    throw Exception(
        ...
        impl->kafka_consumers_pool_ttl_ms.value, KAFKA_CONSUMERS_POOL_TTL_MS_MAX);

  if (impl->kafka_handle_error_mode == StreamingHandleErrorMode::DEAD_LETTER_QUEUE && !global_context->getDeadLetterQueue())
    throw Exception(ErrorCodes::BAD_ARGUMENTS,
                    "The table system.dead_letter_queue is not configured on the server. You cannot create a table with this "
...
SettingsChanges KafkaSettings::getFormatSettings() const {
  ...
  for (const auto& setting : *impl) {
    ...
    if (!setting_name.starts_with("kafka_")) values.emplace_back(setting_name, setting.getValue());
  }

  return values;
}

bool KafkaSettings::hasBuiltin(std::string_view name) { return KafkaSettingsImpl::hasBuiltin(name); }
}  // namespace DB
