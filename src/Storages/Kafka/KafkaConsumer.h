...
class KafkaConsumer : public IKafkaExceptionInfoSink {
 public:
  ...
  KafkaConsumer(LoggerPtr log_, size_t max_batch_size, size_t poll_timeout_, bool intermediate_commit_, const std::atomic<bool>& stopped_,
                const Names& _topics, size_t skip_bytes_ = 0);
  ...
  bool hasMorePolledMessages() const { return (stalled_status == NOT_STALLED) && (current != messages.end()); }

  bool polledDataUnusable() const { return (stalled_status != NOT_STALLED) && (stalled_status != NO_MESSAGES_RETURNED); }
 ...
 private:
  ...
  enum StalledStatus { NOT_STALLED, NO_MESSAGES_RETURNED, REBALANCE_HAPPENED, CONSUMER_STOPPED, NO_ASSIGNMENT, ERRORS_RETURNED };
