...
KafkaConsumer::KafkaConsumer(LoggerPtr log_, size_t max_batch_size, size_t poll_timeout_, bool intermediate_commit_,
                             const std::atomic<bool> &stopped_, const Names &_topics, size_t skip_bytes_)
    : log(log_),
...
void KafkaConsumer::createConsumer(cppkafka::Configuration consumer_config) {
  ...
  if (consumer_config.get("statistics.interval.ms") != "0") {
    consumer_config.set_stats_callback([this](cppkafka::KafkaHandleBase &, const std::string &stat_json) { setRDKafkaStat(stat_json); });
  }
...
ConsumerPtr &&KafkaConsumer::moveConsumer() {
  ...
  StorageKafkaUtils::consumerGracefulStop(*consumer, DRAIN_TIMEOUT_MS, log, [this](const cppkafka::Error &err) {
    IKafkaExceptionInfoSink::setExceptionInfo(err, /* with_stacktrace = */ true);
  });
  ...
  auto print_offsets = [this](const char *prefix, const cppkafka::TopicPartitionList &offsets) {
    for (const auto &topic_part : offsets) {
      auto print_special_offset = [&topic_part] {
        switch (topic_part.get_offset()) {
          ...
          case cppkafka::TopicPartition::OFFSET_STORED:
            return "STORED";
          case cppkafka::TopicPartition::OFFSET_INVALID:
            return "INVALID";
          default:
            return "";
        }
      ...
      if (topic_part.get_offset() < 0) {
        LOG_TRACE(log, "{} {} (topic: {}, partition: {})", prefix, print_special_offset(), topic_part.get_topic(),
                  topic_part.get_partition());
      } else {
        LOG_TRACE(log, "{} {} (topic: {}, partition: {})", prefix, topic_part.get_offset(), topic_part.get_topic(),
                  topic_part.get_partition());
      }
    }
  };
...
void KafkaConsumer::subscribe() {
  ...
  if (stalled_status != CONSUMER_STOPPED) stalled_status = NO_MESSAGES_RETURNED;
  ...
  if (!subscription.empty()) {
    ...
    if (current_subscription_valid) return;
  }
  ...
  while (true) {
    ...
    try {
      ...
      if (max_retries > 0 && e.get_error() == RD_KAFKA_RESP_ERR__TIMED_OUT) continue;
...
void KafkaConsumer::cleanAssignment() {
  if (assignment.has_value()) {
    ...
    if (!assignment->empty()) CurrentMetrics::sub(CurrentMetrics::KafkaConsumersWithAssignment, 1);
    assignment.reset();
  }
}
...
void KafkaConsumer::doPoll() {
  ...
  while (true) {
    ...
    auto num_errors = StorageKafkaUtils::eraseMessageErrors(new_messages, log, [this](const cppkafka::Error &err) {
      IKafkaExceptionInfoSink::setExceptionInfo(err, /* with_stacktrace = */ true);
    });
...
ReadBufferPtr KafkaConsumer::consume() {
  ...
  if (polledDataUnusable()) return nullptr;

  if (hasMorePolledMessages()) return getNextMessage();

  if (intermediate_commit) commit();
...
ReadBufferPtr KafkaConsumer::getNextMessage() {
  if (current == messages.end()) return nullptr;
