...
class KeeperHandlingConsumer {
 public:
  ...
  using MessageSinkFunction = std::function<bool(ReadBufferPtr, const MessageInfo &, bool /*has_more_polled_messages*/, bool /*stalled*/)>;

  KeeperHandlingConsumer(const KafkaConsumer2Ptr &kafka_consumer_, const std::shared_ptr<zkutil::ZooKeeper> &keeper_,
                         const std::filesystem::path &keeper_path_, const String &replica_name_, size_t idx_, const LoggerPtr &log_);
 ...
 private:
  ...
  using TopicPartitionLocks = std::unordered_map<TopicPartition, LockedTopicPartitionInfo, KafkaConsumer2::TopicPartitionHash,
                                                 KafkaConsumer2::TopicPartitionEquality>;

  using TopicPartitionSet = std::unordered_set<TopicPartition, KafkaConsumer2::TopicPartitionHash, KafkaConsumer2::TopicPartitionEquality>;
  ...
  void updatePermanentLocksLocked(const TopicPartitions &available_topic_partitions, size_t topic_partitions_count,
                                  size_t active_replica_count) TSA_REQUIRES(topic_partition_locks_mutex);
  void lockTemporaryLocksLocked(const TopicPartitions &available_topic_partitions, bool has_replica_without_locks)
