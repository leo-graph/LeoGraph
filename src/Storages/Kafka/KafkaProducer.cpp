...
KafkaProducer::KafkaProducer(ProducerPtr producer_, const std::string &topic_, std::chrono::milliseconds poll_timeout,
                             std::atomic<bool> &shutdown_called_, const Block &header)
  ...
  if (header.has("_key")) {
    ...
    if (isString(column_info.type)) key_column_index = column_index;
  ...
  if (header.has("_timestamp")) {
    ...
    const auto &column_info = header.getByPosition(column_index);
    if (isDateTime(column_info.type)) timestamp_column_index = column_index;
  }
}
...
void KafkaProducer::finish() {
  ...
  while (true) {
    try {
      ...
      if (e.get_error() == RD_KAFKA_RESP_ERR__TIMED_OUT) continue;
...
void KafkaProducer::cancel() noexcept { /* no op */ }
