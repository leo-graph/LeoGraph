...
struct KafkaInterceptors {
  ...
  static rd_kafka_resp_err_t rdKafkaOnConfDup(rd_kafka_conf_t *new_conf, const rd_kafka_conf_t * /*old_conf*/, size_t /*filter_cnt*/,
                                              const char ** /*filter*/, void *ctx);
};

template <typename TStorageKafka>
rd_kafka_resp_err_t KafkaInterceptors<TStorageKafka>::rdKafkaOnThreadStart(rd_kafka_t *, rd_kafka_thread_type_t thread_type, const char *,
                                                                           void *ctx) {
...
rd_kafka_resp_err_t KafkaInterceptors<TStorageKafka>::rdKafkaOnThreadExit(rd_kafka_t *, rd_kafka_thread_type_t, const char *, void *ctx) {
  ...
  std::lock_guard lock(self->thread_statuses_mutex);
  const auto it = std::find_if(self->thread_statuses.begin(), self->thread_statuses.end(),
                               [](const auto &thread_status_ptr) { return thread_status_ptr.get() == current_thread; });
  if (it == self->thread_statuses.end()) throw Exception(ErrorCodes::LOGICAL_ERROR, "No thread status for this librdkafka thread.");
...
template <typename TStorageKafka>
rd_kafka_resp_err_t KafkaInterceptors<TStorageKafka>::rdKafkaOnNew(rd_kafka_t *rk, const rd_kafka_conf_t *, void *ctx, char * /*errstr*/,
                                                                   size_t /*errstr_size*/) {
  TStorageKafka *self = reinterpret_cast<TStorageKafka *>(ctx);
  ...
  if (status != RD_KAFKA_RESP_ERR_NO_ERROR) LOG_ERROR(self->log, "Cannot set on thread exit interceptor due to {} error", status);

  return status;
}

template <typename TStorageKafka>
rd_kafka_resp_err_t KafkaInterceptors<TStorageKafka>::rdKafkaOnConfDup(rd_kafka_conf_t *new_conf, const rd_kafka_conf_t * /*old_conf*/,
                                                                       size_t /*filter_cnt*/, const char ** /*filter*/, void *ctx) {
  TStorageKafka *self = reinterpret_cast<TStorageKafka *>(ctx);
  rd_kafka_resp_err_t status;
  ...
  if (status != RD_KAFKA_RESP_ERR_NO_ERROR) LOG_ERROR(self->log, "Cannot set on conf new interceptor due to {} error", status);
...
void loadConfigProperty(cppkafka::Configuration &kafka_config, const Poco::Util::AbstractConfiguration &config, const String &config_prefix,
                        const String &tag) {
...
void loadLegacyTopicConfig(cppkafka::Configuration &kafka_config, const Poco::Util::AbstractConfiguration &config,
                           const String &collection_name, const String &config_prefix) {
...
void loadTopicConfig(cppkafka::Configuration &kafka_config, const Poco::Util::AbstractConfiguration &config, const String &collection_name,
                     const String &config_prefix, const String &topic) {
  if (!collection_name.empty()) {
    ...
    for (const auto &key : collection->getKeys(1, config_prefix)) {
      ...
      if (!key.starts_with(topic_prefix)) continue;
    ...
    for (const auto &tag : tags) {
      if (tag == KafkaConfigLoader::CONFIG_NAME_TAG) continue;  // ignore <name>, it is used to match topic configurations
      loadConfigProperty(kafka_config, config, config_prefix, tag);
    }
  }
}
...
void loadFromConfig(cppkafka::Configuration &kafka_config, const KafkaConfigLoader::LoadConfigParams &params, const String &config_prefix) {
...
void loadLegacyConfigSyntax(cppkafka::Configuration &kafka_config, const Poco::Util::AbstractConfiguration &config,
                            const String &collection_name, const Names &topics) {
...
template <typename TKafkaStorage>
using SpecificConfigUpdaterFunc = void (*)(cppkafka::Configuration &kafka_config, const KafkaConfigLoader::LoadConfigParams &params);

template <typename TKafkaStorage>
void updateConfigurationFromConfig(SpecificConfigUpdaterFunc<TKafkaStorage> specific_config_updater, cppkafka::Configuration &kafka_config,
                                   TKafkaStorage &storage, const KafkaConfigLoader::LoadConfigParams &params,
  ...
  kafka_config.set_log_callback([log = params.log, sink = exception_info_sink_ptr](
    ...
    LOG_IMPL(log, client_logs_level, poco_level, "[client.id:{}] [rdk:{}] {}", kafka_object_config.get(client_id_key), facility, message);
  ...
  // XXX:  rdkafka uses pthread_set_name_np(), but glibc-compatibility overrides it to noop.
  {
    ...
    status = rd_kafka_conf_interceptor_add_on_new(kafka_config.get_handle(), "init", KafkaInterceptors<TKafkaStorage>::rdKafkaOnNew, self);
    if (status != RD_KAFKA_RESP_ERR_NO_ERROR) LOG_ERROR(params.log, "Cannot set new interceptor due to {} error", status);

    // cppkafka always copy the configuration
    status = rd_kafka_conf_interceptor_add_on_conf_dup(kafka_config.get_handle(), "init",
                                                       KafkaInterceptors<TKafkaStorage>::rdKafkaOnConfDup, self);
    if (status != RD_KAFKA_RESP_ERR_NO_ERROR) LOG_ERROR(params.log, "Cannot set dup conf interceptor due to {} error", status);
  }
}
...
cppkafka::Configuration KafkaConfigLoader::getConsumerConfiguration(TKafkaStorage &storage, const ConsumerConfigParams &params,
                                                                    IKafkaExceptionInfoSinkPtr exception_info_sink_ptr) {
  ...
  conf.set("queued.min.messages", std::min(std::max(params.max_block_size, default_queued_min_messages), max_allowed_queued_min_messages));
  ...
  for (auto &property : conf.get_all()) {
    if (property.first.find("password") != std::string::npos) continue;
...
template cppkafka::Configuration KafkaConfigLoader::getConsumerConfiguration<StorageKafka>(
    StorageKafka &storage, const ConsumerConfigParams &params, IKafkaExceptionInfoSinkPtr exception_info_sink_ptr);
template cppkafka::Configuration KafkaConfigLoader::getConsumerConfiguration<StorageKafka2>(
    StorageKafka2 &storage, const ConsumerConfigParams &params, IKafkaExceptionInfoSinkPtr exception_info_sink_ptr);
...
cppkafka::Configuration KafkaConfigLoader::getProducerConfiguration(TKafkaStorage &storage, const ProducerConfigParams &params) {
  ...
  for (auto &property : conf.get_all()) LOG_TRACE(params.log, "Producer set property {}:{}", property.first, property.second);
...
template cppkafka::Configuration KafkaConfigLoader::getProducerConfiguration<StorageKafka>(StorageKafka &storage,
                                                                                           const ProducerConfigParams &params);
template cppkafka::Configuration KafkaConfigLoader::getProducerConfiguration<StorageKafka2>(StorageKafka2 &storage,
                                                                                            const ProducerConfigParams &params);
