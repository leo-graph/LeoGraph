...
KafkaConsumer2::KafkaConsumer2(LoggerPtr log_, size_t max_batch_size, size_t poll_timeout_, const std::atomic<bool> &stopped_,
                               const Names &topics_, size_t skip_bytes_)
...
void KafkaConsumer2::createConsumer(cppkafka::Configuration consumer_config) {
  if (consumer) throw Exception(ErrorCodes::LOGICAL_ERROR, "KafkaConsumer2::createConsumer called when consumer already exists");

  if (consumer_config.get("statistics.interval.ms") != "0") {
    consumer_config.set_stats_callback([weak_this = weak_from_this()](cppkafka::KafkaHandleBase &, const std::string &stat_json) {
...
CppKafkaConsumerPtr &&KafkaConsumer2::moveConsumer() {
  ...
  StorageKafkaUtils::consumerStopWithoutRebalance(*consumer, DRAIN_TIMEOUT_MS, log, [this](const cppkafka::Error &err) {
...
bool KafkaConsumer2::polledDataUnusable(const TopicPartition &topic_partition) const {
  const auto different_topic_partition = current == messages.end() ? false
                                                                   : (current->get_topic() != topic_partition.topic ||
                                                                      current->get_partition() != topic_partition.partition_id);
...
void KafkaConsumer2::updateOffsets(TopicPartitionOffsets &&topic_partition_offsets) {
  ...
  std::transform(std::move_iterator(topic_partition_offsets.begin()), std::move_iterator(topic_partition_offsets.end()),
...
void KafkaConsumer2::initializeQueues(const cppkafka::TopicPartitionList &topic_partitions) {
  ...
  for (const auto &topic_partition : topic_partitions)
    ...
    queues.emplace(TopicPartition{topic_partition.get_topic(), topic_partition.get_partition()},
...
ReadBufferPtr KafkaConsumer2::consume(const TopicPartition &topic_partition, const std::optional<uint64_t> &message_count) {
  ...
  if (polledDataUnusable(topic_partition)) return nullptr;

  if (hasMorePolledMessages()) {
    if (auto next_message = getNextMessage(); next_message) return next_message;
  }
  ...
  while (true) {
    ...
    current = messages.begin();
    LOG_TRACE(log, "Polled batch of {} messages. Offsets position: {}", messages.size(),
...
void KafkaConsumer2::commit(const TopicPartitionOffset &topic_partition_offset) {
  ...
  LOG_TEST(log, "Trying to commit offset {} to Kafka for topic-partition [{}:{}]", topic_partition_offset.offset,
           topic_partition_offset.topic, topic_partition_offset.partition_id);
  ...
  for (auto try_count = 0; try_count < max_retries && !committed; ++try_count) {
    try {
      ...
      committed = true;
      LOG_INFO(log, "Committed offset {} to Kafka for topic-partition [{}:{}]", topic_partition_offset.offset, topic_partition_offset.topic,
               topic_partition_offset.partition_id);
  ...
  if (!committed) {
    // The failure is not the biggest issue, it only counts when a table is dropped and recreated, otherwise the offsets are taken from
    // keeper.
...
KafkaConsumer2::TopicPartitionOffsets KafkaConsumer2::getAllTopicPartitionOffsets() const {
  ...
  try {
    ...
    for (const auto &topic_metadata : metadata.get_topics()) {
      if (!topics_set.contains(topic_metadata.get_name())) continue;
...
ReadBufferPtr KafkaConsumer2::getNextMessage() {
  while (current != messages.end()) {
    ...
    if (data && size >= skip_bytes) return std::make_shared<ReadBufferFromMemory>(data + skip_bytes, size - skip_bytes);
  }
...
void KafkaConsumer2::filterMessageErrors() {
  ...
  StorageKafkaUtils::eraseMessageErrors(
      messages, log, [this](const cppkafka::Error &err) { IKafkaExceptionInfoSink::setExceptionInfo(err, /* with_stacktrace = */ true); });
  current = messages.begin();
}
