...
#define CHECK_KAFKA_STORAGE_ARGUMENT(ARG_NUM, PAR_NAME, EVAL, TYPE)                                                                     \
  ...
  if (args_count >= (ARG_NUM)) {                                                                                                        \
    ...
    if constexpr ((EVAL) == 2) {                                                                                                        \
      engine_args[(ARG_NUM) - 1] = evaluateConstantExpressionOrIdentifierAsLiteral(engine_args[(ARG_NUM) - 1], args.getLocalContext()); \
    }                                                                                                                                   \
...
#undef CHECK_KAFKA_STORAGE_ARGUMENT
    ...
    if ((*kafka_settings)[KafkaSetting::kafka_poll_max_batch_size].changed &&
        (*kafka_settings)[KafkaSetting::kafka_poll_max_batch_size].value < 1) {
    ...
    if ((*kafka_settings)[KafkaSetting::kafka_schema_registry_skip_bytes].value > MAX_SKIP_BYTES) {
      throw Exception(ErrorCodes::BAD_ARGUMENTS, "kafka_schema_registry_skip_bytes value {} must be between 0 and {}",
    ...
    for (const auto &column : args.columns) {
      if (column.default_desc.kind == ColumnDefaultKind::Alias) supported_columns.emplace_back(column.name, column.type);
    ...
    const auto has_keeper_path =
        (*kafka_settings)[KafkaSetting::kafka_keeper_path].changed && !(*kafka_settings)[KafkaSetting::kafka_keeper_path].value.empty();
    const auto has_replica_name =
        (*kafka_settings)[KafkaSetting::kafka_replica_name].changed && !(*kafka_settings)[KafkaSetting::kafka_replica_name].value.empty();

    if (!has_keeper_path && !has_replica_name)
      return std::make_shared<StorageKafka>(args.table_id, args.getContext(), args.columns, args.comment, std::move(kafka_settings),
                                            collection_name);
    ...
    const auto is_replicated_database =
        args.getLocalContext()->getClientInfo().query_kind == ClientInfo::QueryKind::SECONDARY_QUERY &&
    ...
    if (args.mode < LoadingStrictnessLevel::ATTACH) {
      ...
      (*kafka_settings)[KafkaSetting::kafka_keeper_path].value =
          context->getMacros()->expand((*kafka_settings)[KafkaSetting::kafka_keeper_path].value, info);

      info.level = 0;
      (*kafka_settings)[KafkaSetting::kafka_replica_name].value =
          context->getMacros()->expand((*kafka_settings)[KafkaSetting::kafka_replica_name].value, info);
    }
    ...
    // Expand other macros (such as {replica}). We do not expand them on previous step to make possible copying metadata files between
    // replicas. Disable expanding {shard} macro, because it can lead to incorrect behavior and it doesn't make sense to shard Kafka tables.
    Macros::MacroExpansionInfo info;
    info.table_id = args.table_id;
    ...
    if (!allow_uuid_macro) info.table_id.uuid = UUIDHelpers::Nil;
    (*kafka_settings)[KafkaSetting::kafka_keeper_path].value =
        context->getMacros()->expand((*kafka_settings)[KafkaSetting::kafka_keeper_path].value, info);
    ...
    return std::make_shared<StorageKafka2>(args.table_id, args.getContext(), args.columns, args.comment, std::move(kafka_settings),
                                           collection_name);
  };

  factory.registerStorage("Kafka", creator_fn,
...
void stopConsumerImpl(cppkafka::Consumer &consumer, RevocationCb revocation_cb, AssignmentCb assignment_cb,
                      const std::chrono::milliseconds drain_timeout, const LoggerPtr &log, StorageKafkaUtils::ErrorHandler error_handler) {
...
Names parseTopics(String topic_list) {
  ...
  for (String &topic : result) boost::trim(topic);
  return result;
}
...
void consumerGracefulStop(cppkafka::Consumer &consumer, const std::chrono::milliseconds drain_timeout, const LoggerPtr &log,
...
void consumerStopWithoutRebalance(cppkafka::Consumer &consumer, const std::chrono::milliseconds drain_timeout, const LoggerPtr &log,
                                  ErrorHandler error_handler) {
  stopConsumerImpl(
      consumer,
      /*revocation*/
...
void drainConsumer(cppkafka::Consumer &consumer, const std::chrono::milliseconds drain_timeout, const LoggerPtr &log,
                   ErrorHandler error_handler) {
...
size_t eraseMessageErrors(Messages &messages, const LoggerPtr &log, ErrorHandler error_handler) {
  size_t skipped = std::erase_if(messages, [&](auto &message) {
  ...
  if (skipped) LOG_ERROR(log, "There were {} messages with an error", skipped);
...
SettingsChanges createSettingsAdjustments(KafkaSettings &kafka_settings, const String &schema_name) {
  ...
  if (!schema_name.empty()) result.emplace_back("format_schema", schema_name);
