...
StoragePostgreSQL::StoragePostgreSQL(const StorageID &table_id_, postgres::PoolWithFailoverPtr pool_, const String &remote_table_name_,
                                     const ColumnsDescription &columns_, const ConstraintsDescription &constraints_, const String &comment,
                                     ContextPtr context_, const String &remote_table_schema_, const String &on_conflict_)
    : IStorage(table_id_),
...
ColumnsDescription StoragePostgreSQL::getTableStructureFromData(const postgres::PoolWithFailoverPtr &pool_, const String &table,
                                                                const String &schema, const ContextPtr &context_) {
  const bool use_nulls = context_->getSettingsRef()[Setting::external_table_functions_use_nulls];
  auto connection_holder = pool_->get();
  auto columns_info = fetchPostgreSQLTableStructure(connection_holder->get(), table, schema, use_nulls).physical_columns;

  if (!columns_info) throw Exception(ErrorCodes::BAD_ARGUMENTS, "Table structure not returned");
...
class ReadFromPostgreSQL : public SourceStepWithFilter {
 public:
  ReadFromPostgreSQL(const Names &column_names_, const SelectQueryInfo &query_info_, const StorageSnapshotPtr &storage_snapshot_,
                     const ContextPtr &context_, SharedHeader sample_block, size_t max_block_size_, String remote_table_schema_,
                     String remote_table_name_, postgres::PoolWithFailoverPtr pool_)
  ...
  QueryPlanStepPtr clone() const override {
    return std::make_unique<ReadFromPostgreSQL>(requiredSourceColumns(), query_info, storage_snapshot, context, getOutputHeader(),
                                                max_block_size, remote_table_schema, remote_table_name, pool);
  }

  void initializePipeline(QueryPipelineBuilder &pipeline, const BuildQueryPipelineSettings &) override {
    std::optional<size_t> transform_query_limit;
    if (limit && !filter_actions_dag) transform_query_limit = limit;
    ...
    String query = transformQueryForExternalDatabase(
        ...
        LiteralEscapingStyle::PostgreSQL, remote_table_schema, remote_table_name, context, transform_query_limit);
...
void StoragePostgreSQL::read(QueryPlan &query_plan, const Names &column_names, const StorageSnapshotPtr &storage_snapshot,
  ...
  for (const String &column_name : column_names) {
    ...
    if (which.isEnum()) column_data.type = std::make_shared<DataTypeString>();
    sample_block.insert({column_data.type, column_data.name});
  }

  auto reading = std::make_unique<ReadFromPostgreSQL>(column_names, query_info, storage_snapshot, local_context,
                                                      std::make_shared<const Block>(sample_block), max_block_size, remote_table_schema,
                                                      remote_table_name, pool);
...
class PostgreSQLSink : public SinkToStorage {
 ...
 public:
  explicit PostgreSQLSink(const StorageMetadataPtr &metadata_snapshot_, postgres::ConnectionHolderPtr connection_holder_,
                          const String &remote_table_name_, const String &remote_table_schema_, const String &on_conflict_)
  ...
  void consume(Chunk &chunk) override {
    ...
    if (!inserter) {
      if (on_conflict.empty()) {
        ...
        inserter = std::make_unique<PreparedInsert>(connection_holder->get(), remote_table_name, remote_table_schema,
                                                    block.getColumnsWithTypeAndName(), on_conflict);
      }
    }
  ...
  static void parseArray(const Field &array_field, const DataTypePtr &data_type, WriteBuffer &ostr) {
    ...
    for (auto iter = array.begin(); iter != array.end(); ++iter) {
      if (iter != array.begin()) writeText(", ", ostr);
  ...
  static MutableColumnPtr createNested(DataTypePtr nested) {
    ...
    if (which.isString() || which.isFixedString())
      nested_column = ColumnString::create();
    else if (which.isInt8() || which.isInt16())
      nested_column = ColumnInt16::create();
    else if (which.isUInt8() || which.isUInt16())
      nested_column = ColumnUInt16::create();
    else if (which.isInt32())
      nested_column = ColumnInt32::create();
    else if (which.isUInt32())
      nested_column = ColumnUInt32::create();
    else if (which.isInt64())
      nested_column = ColumnInt64::create();
    else if (which.isUInt64())
      nested_column = ColumnUInt64::create();
    else if (which.isFloat32())
      nested_column = ColumnFloat32::create();
    else if (which.isFloat64())
      nested_column = ColumnFloat64::create();
      ...
      nested_column = ColumnUUID::create();
 ...
 private:
  struct Inserter {
    ...
    explicit Inserter(pqxx::connection &connection_) : connection(connection_), tx(connection) {}
  ...
  struct StreamTo : Inserter {
    ...
    void insert(const Row &row) override { stream.write_values(row); }
  };

  struct PreparedInsert : Inserter {
    PreparedInsert(pqxx::connection &connection_, const String &table, const String &schema, const ColumnsWithTypeAndName &columns,
                   const String &on_conflict_)
        : Inserter(connection_), statement_name("insert_" + getHexUIntLowercase(thread_local_rng())) {
    ...
    ~PreparedInsert() override {
      try {
        if (prepared) connection.unprepare(statement_name);
      } catch (...) {
...
SinkToStoragePtr StoragePostgreSQL::write(const ASTPtr & /*query*/, const StorageMetadataPtr &metadata_snapshot, ContextPtr /* context */,
                                          bool /*async_insert*/) {
  return std::make_shared<PostgreSQLSink>(metadata_snapshot, pool->get(), remote_table_name, remote_table_schema, on_conflict);
}

StoragePostgreSQL::Configuration StoragePostgreSQL::processNamedCollectionResult(const NamedCollection &named_collection,
                                                                                 ContextPtr context_, bool require_table) {
  StoragePostgreSQL::Configuration configuration;
  ValidateKeysMultiset<ExternalDatabaseEqualKeysSet> required_arguments = {"user", "username", "password", "database", "db"};
  if (require_table) required_arguments.insert("table");
  ...
  if (require_table) configuration.table = named_collection.get<String>("table");
...
StoragePostgreSQL::Configuration StoragePostgreSQL::getConfiguration(ASTs engine_args, ContextPtr context, const StorageID *table_id) {
  ...
  if (auto named_collection = tryGetNamedCollectionWithOverrides(engine_args, context, true, nullptr, table_id)) {
    ...
    if (engine_args.size() >= 6) configuration.schema = checkAndGetLiteralArgument<String>(engine_args[5], "schema");
    if (engine_args.size() >= 7) configuration.on_conflict = checkAndGetLiteralArgument<String>(engine_args[6], "on_conflict");
  }
  ...
  factory.registerStorage(
      ...
      [](const StorageFactory::Arguments &args) {
        ...
        auto pool = std::make_shared<postgres::PoolWithFailover>(
            configuration, settings[Setting::postgresql_connection_pool_size], settings[Setting::postgresql_connection_pool_wait_timeout],
            settings[Setting::postgresql_connection_pool_retries], settings[Setting::postgresql_connection_pool_auto_close_connection],
            settings[Setting::postgresql_connection_attempt_timeout]);

        return std::make_shared<StoragePostgreSQL>(args.table_id, std::move(pool), configuration.table, args.columns, args.constraints,
                                                   args.comment, args.getContext(), configuration.schema, configuration.on_conflict);
      },
      {
