...
class StorageMaterializedView final : public IStorage, WithMutableContext {
 public:
  StorageMaterializedView(const StorageID &table_id_, ContextPtr local_context, const ASTCreateQuery &query,
  ...
  std::optional<String> getCoordinationPath() const {
    if (!refresher.ptr) return std::nullopt;
 ...
 private:
  ...
  std::tuple<boost::intrusive_ptr<ASTInsertQuery>, CurrentThread::QueryScope> prepareRefresh(
