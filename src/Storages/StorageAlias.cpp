...
StorageAlias::StorageAlias(const StorageID &table_id_, ContextPtr context_, const String &target_database_, const String &target_table_)
    : IStorage(table_id_), WithContext(context_->getGlobalContext()), target_database(target_database_), target_table(target_table_) {
  StorageID target_id(target_database, target_table);
  if (table_id_ == target_id) throw Exception(ErrorCodes::BAD_ARGUMENTS, "Alias table cannot refer to itself");
...
class AliasSink : public SinkToStorage, WithContext {
 public:
  AliasSink(StorageAlias &storage_, ContextPtr context_, const StorageMetadataPtr &metadata_snapshot_)
  ...
  void consume(Chunk &chunk) override {
    if (chunk.getNumRows() == 0) return;

    auto block = getHeader().cloneWithColumns(chunk.getColumns());

    Block non_materialized_block;
    for (const auto &col : non_materialized_header) non_materialized_block.insert(block.getByName(col.name));
...
void StorageAlias::read(QueryPlan &query_plan, const Names &column_names, const StorageSnapshotPtr & /*storage_snapshot*/,
  ...
  auto lock =
      target_storage->lockForShare(local_context->getCurrentQueryId(), local_context->getSettingsRef()[Setting::lock_acquire_timeout]);
  ...
  target_storage->read(query_plan, column_names, target_snapshot, query_info, local_context, processed_stage, max_block_size, num_streams);

  query_plan.addStorageHolder(target_storage);
  query_plan.addTableLock(std::move(lock));
}

SinkToStoragePtr StorageAlias::write(const ASTPtr & /*query*/, const StorageMetadataPtr & /*metadata_snapshot*/, ContextPtr local_context,
                                     bool /*async_insert*/) {
...
void StorageAlias::alter(const AlterCommands &params, ContextPtr local_context, AlterLockHolder &table_lock_holder) {
...
void StorageAlias::truncate(const ASTPtr &query, const StorageMetadataPtr & /*metadata_snapshot*/, ContextPtr local_context,
...
bool StorageAlias::optimize(const ASTPtr &query, const StorageMetadataPtr & /*metadata_snapshot*/, const ASTPtr &partition, bool final,
  ...
  auto target_metadata = target_storage->getInMemoryMetadataPtr();
  return target_storage->optimize(query, target_metadata, partition, final, deduplicate, deduplicate_by_columns, cleanup, local_context);
}

Pipe StorageAlias::alterPartition(const StorageMetadataPtr &, const PartitionCommands &commands, ContextPtr local_context) {
  auto target_storage = getTargetTable(TargetAccess{local_context, AccessType::ALTER});
  auto target_metadata = target_storage->getInMemoryMetadataPtr();
  return target_storage->alterPartition(target_metadata, commands, local_context);
}

void StorageAlias::checkAlterPartitionIsPossible(const PartitionCommands &commands, const StorageMetadataPtr & /*metadata_snapshot*/,
...
CancellationCode StorageAlias::killMutation(const String &mutation_id) { return getTargetTable()->killMutation(mutation_id); }
...
void StorageAlias::setMutationCSN(const String &mutation_id, UInt64 csn) { getTargetTable()->setMutationCSN(mutation_id, csn); }

CancellationCode StorageAlias::killPartMoveToShard(const UUID &task_uuid) { return getTargetTable()->killPartMoveToShard(task_uuid); }
...
StorageSnapshotPtr StorageAlias::getStorageSnapshotWithoutData(const StorageMetadataPtr &metadata_snapshot,
...
QueryProcessingStage::Enum StorageAlias::getQueryProcessingStage(ContextPtr local_context, QueryProcessingStage::Enum to_stage,
      ...
      [](const StorageFactory::Arguments &args) {
        ...
        if (args.mode == LoadingStrictnessLevel::CREATE && !local_context->getSettingsRef()[Setting::allow_experimental_alias_table_engine])
          throw Exception(ErrorCodes::SUPPORT_IS_DISABLED,
                          "Experimental Alias table engine is not enabled (turn on setting 'allow_experimental_alias_table_engine')");
        ...
        if (!args.columns.empty() && args.mode == LoadingStrictnessLevel::CREATE) {
          throw Exception(ErrorCodes::BAD_ARGUMENTS, "Storage Alias does not support explicit column definitions");
        }

        return std::make_shared<StorageAlias>(args.table_id, local_context, target_database, target_table);
      },
