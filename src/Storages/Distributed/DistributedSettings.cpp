...
#define LIST_OF_DISTRIBUTED_SETTINGS(DECLARE, DECLARE_WITH_ALIAS)                                                                          \
  DECLARE(Bool, fsync_after_insert, false,                                                                                                 \
          "Do fsync for every inserted. Will decreases performance of inserts (only for background INSERT, i.e. "                          \
          "distributed_foreground_insert=false)",                                                                                          \
          0)                                                                                                                               \
  DECLARE(Bool, fsync_directories, false,                                                                                                  \
          "Do fsync for temporary directory (that is used for background INSERT only) after all part operations (writes, renames, etc.).", \
          0)                                                                                                                               \
  /** This is the distributed version of the skip_unavailable_shards setting available in src/Core/Settings.cpp */                         \
  DECLARE(Bool, skip_unavailable_shards, false,                                                                                            \
          "If true, ClickHouse silently skips unavailable shards. Shard is marked as unavailable when: 1) The shard cannot be reached "    \
          "due to a connection failure. 2) Shard is unresolvable through DNS. 3) Table does not exist on the shard.",                      \
          0)                                                                                                                               \
  /** Inserts settings. */                                                                                                                 \
  DECLARE(UInt64, bytes_to_throw_insert, 0,                                                                                                \
          "If more than this number of compressed bytes will be pending for background INSERT, an exception will be thrown. 0 - do not "   \
          "throw.",                                                                                                                        \
          0)                                                                                                                               \
  DECLARE(                                                                                                                                 \
      UInt64, bytes_to_delay_insert, 0,                                                                                                    \
      ...
      0)                                                                                                                                   \
  DECLARE(UInt64, max_delay_to_insert, 60,                                                                                                 \
          "Max delay of inserting data into Distributed table in seconds, if there are a lot of pending bytes for background send.", 0)    \
  ...
  DECLARE_WITH_ALIAS(UInt64, background_insert_split_batch_on_failure, 0,                                                                  \
                     "Default - distributed_background_insert_split_batch_on_failure", 0, monitor_split_batch_on_failure)                  \
  DECLARE_WITH_ALIAS(Milliseconds, background_insert_sleep_time_ms, 0, "Default - distributed_background_insert_sleep_time_ms", 0,         \
                     monitor_sleep_time_ms)                                                                                                \
  DECLARE_WITH_ALIAS(Milliseconds, background_insert_max_sleep_time_ms, 0, "Default - distributed_background_insert_max_sleep_time_ms", 0, \
...
#define INITIALIZE_SETTING_EXTERN(TYPE, NAME, DEFAULT, DESCRIPTION, FLAGS, ...) \
  DistributedSettings##TYPE NAME = &DistributedSettingsImpl ::NAME;
...
void DistributedSettings::loadFromConfig(const String& config_elem, const Poco::Util::AbstractConfiguration& config) {
  ...
  try {
    ...
    if (e.code() == ErrorCodes::UNKNOWN_SETTING) e.addMessage("in Distributed config");
    throw;
  }
}
...
bool DistributedSettings::hasBuiltin(std::string_view name) { return DistributedSettingsImpl::hasBuiltin(name); }
}  // namespace DB
