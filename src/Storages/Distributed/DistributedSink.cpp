...
static Block adoptBlock(const Block& header, const Block& block, LoggerPtr log) {
  if (blocksHaveEqualStructure(header, block)) return block;

  LOG_WARNING(log, "Structure does not match (remote: {}, local: {}), implicit conversion will be done.", header.dumpStructure(),
              block.dumpStructure());

  auto converting_dag = ActionsDAG::makeConvertingActions(block.cloneEmpty().getColumnsWithTypeAndName(),
                                                          header.getColumnsWithTypeAndName(), ActionsDAG::MatchColumnsMode::Name, nullptr);
...
static void writeBlockConvert(PushingPipelineExecutor& executor, const Block& block, size_t repeats, LoggerPtr log) {
  ...
  for (size_t i = 0; i < repeats; ++i) executor.push(adopted_block);
}
...
static ASTPtr createInsertToRemoteTableQuery(const std::string& database, const std::string& table, const Names& column_names) {
  ...
  for (const auto& column_name : column_names) columns->children.push_back(make_intrusive<ASTIdentifier>(column_name));
  return query;
}

DistributedSink::DistributedSink(ContextPtr context_, StorageDistributed& storage_, const StorageMetadataPtr& metadata_snapshot_,
                                 const ClusterPtr& cluster_, bool insert_sync_, UInt64 insert_timeout_, const Names& columns_to_send_)
...
Block DistributedSink::removeSuperfluousColumns(Block block) const {
  for (size_t i = block.columns(); i;) {
    --i;
    if (!columns_to_send.contains(block.getByPosition(i).name)) block.erase(i);
  }
  return block;
}
  ...
  for (auto& shard_jobs : per_shard_jobs)
    for (JobReplica& job : shard_jobs.replicas_jobs) {
      ...
      if (job.blocks_started > 0) {
        ...
        if (job.blocks_started > 1) buffer << ", the slowest block " << job.max_elapsed_time_for_block_ms << " ms";
        buffer << ")";
      }
...
DistributedSink::JobReplica::JobReplica(size_t shard_index_, size_t replica_index_, bool is_local_job_, const Block& sample_block)
    : shard_index(shard_index_),
      ...
      is_local_job(is_local_job_),
      current_shard_block(sample_block.cloneEmpty()) {}
...
void DistributedSink::initWritingJobs(const Block& first_block, size_t start, size_t end) {
  ...
  for (size_t shard_index : collections::range(start, end)) {
    ...
    if (!shard_info.hasInternalReplication() || !shard_info.isLocal() || !settings[Setting::prefer_localhost_replica]) {
      ...
      for (size_t replica_index : collections::range(0, replicas.size())) {
        if (!replicas[replica_index].is_local || !settings[Setting::prefer_localhost_replica]) {
          ...
          if (shard_info.hasInternalReplication()) break;
        }
      }
    }
    ...
    if (num_shards > 1) shard_jobs.shard_current_block_permutation.reserve(first_block.rows());
  }
}
...
void DistributedSink::waitForJobs() {
  ...
  if (random_shard_insert) {
    ...
    if (num_finished_jobs < jobs_count) LOG_WARNING(log, "Expected {} writing jobs, but finished only {}", jobs_count, num_finished_jobs);
  }
}

ThreadPool::Job DistributedSink::runWritingJob(JobReplica& job, const Block& current_block, size_t num_shards) {
  ...
  return [this, thread_group, &job, &current_block, num_shards]() {
    ...
    if (isCancelled()) throw Exception(ErrorCodes::ABORTED, "Writing job was cancelled");
    ...
    if (!job.is_local_job || !settings[Setting::prefer_localhost_replica]) {
      if (!job.executor) {
        ...
        if (shard_info.hasInternalReplication()) {
          ...
          auto results =
              shard_info.pool->getManyCheckedForInsert(timeouts, settings, PoolMode::GET_ONE, storage.remote_storage.getQualifiedName());
        ...
        if (throttler) job.connection_entry->setThrottler(throttler);

        job.pipeline =
            QueryPipeline(std::make_shared<RemoteSink>(*job.connection_entry, timeouts, query_string, settings, context->getClientInfo()));
...
void DistributedSink::writeSync(const Block& block) {
  ...
  if (!pool) {
    ...
    pool.emplace(CurrentMetrics::DistributedInsertThreads, CurrentMetrics::DistributedInsertThreadsActive,
                 CurrentMetrics::DistributedInsertThreadsScheduled, max_threads, max_threads, jobs_count);
  ...
  if (num_shards > 1) {
    ...
    for (size_t shard_index : collections::range(start, end)) per_shard_jobs[shard_index].shard_current_block_permutation.resize(0);

    for (size_t i = 0; i < block.rows(); ++i) per_shard_jobs[current_selector[i]].shard_current_block_permutation.push_back(i);
  }
...
void DistributedSink::onFinish() {
  auto log_performance = [this]() {
    ...
    LOG_DEBUG(log, "It took {} sec. to insert {} blocks, {} rows per second. {}", elapsed, inserted_blocks,
              static_cast<double>(inserted_rows) / elapsed, getCurrentStateDescription());
  };
...
Blocks DistributedSink::splitBlock(const Block& block) {
  ...
  for (size_t shard_idx = 0; shard_idx < num_shards; ++shard_idx) split_blocks[shard_idx] = block.cloneEmpty();
...
void DistributedSink::writeSplitAsync(const Block& block) {
  ...
  for (size_t shard_idx = 0; shard_idx < num_shards; ++shard_idx)
    if (split_blocks[shard_idx].rows()) writeAsyncImpl(split_blocks[shard_idx], shard_idx);
...
void DistributedSink::writeAsyncImpl(const Block& block, size_t shard_id) {
  ...
  if (shard_info.hasInternalReplication()) {
    if (shard_info.isLocal() && settings[Setting::prefer_localhost_replica])
      ...
      const auto& path = shard_info.insertPathForInternalReplication(settings[Setting::prefer_localhost_replica],
                                                                     settings[Setting::use_compact_format_in_distributed_parts_names]);
      if (path.empty()) throw Exception(ErrorCodes::LOGICAL_ERROR, "Directory name for async inserts is empty");
    ...
    if (!dir_names.empty()) writeToShard(shard_info, block_to_send, dir_names);
  }
}
...
void DistributedSink::writeToShard(const Cluster::ShardInfo& shard_info, const Block& block, const std::vector<std::string>& dir_names) {
  ...
  if (compression_method == "ZSTD") compression_level = settings[Setting::network_zstd_compression_level];

  CompressionCodecFactory::instance().validateCodec(compression_method, compression_level, !settings[Setting::allow_suspicious_codecs],
