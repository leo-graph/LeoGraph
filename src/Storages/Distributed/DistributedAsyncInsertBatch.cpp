...
bool isSplittableErrorCode(int code, bool remote) {
  return code == ErrorCodes::MEMORY_LIMIT_EXCEEDED
         ...
         || code == ErrorCodes::ARGUMENT_OUT_OF_BOUND || code == ErrorCodes::TOO_MANY_PARTS || code == ErrorCodes::TOO_MANY_BYTES ||
         code == ErrorCodes::TOO_MANY_ROWS_OR_BYTES || code == ErrorCodes::TOO_MANY_PARTITIONS ||
         code == ErrorCodes::DISTRIBUTED_TOO_MANY_PENDING_BYTES || code == ErrorCodes::DISTRIBUTED_BROKEN_BATCH_INFO ||
...
void DistributedAsyncInsertBatch::send(const SettingsChanges& settings_changes, bool update_current_batch) {
  ...
  try {
    ...
    if (isDistributedSendBroken(e.code(), e.isRemoteException())) {
      ...
      batch_broken = true;
      if (!e.isRemoteException() && e.code() == ErrorCodes::DISTRIBUTED_BROKEN_BATCH_FILES) batch_marked_as_broken = true;
    } else {
      e.addMessage(
          fmt::format("While sending a batch of {} files, files: {}", files.size(), fmt::join(files | std::ranges::views::take(8), "\n")));
      throw;
    }
  }
  ...
  if (!batch_broken) {
    ...
    for (const auto& file : files) parent.markAsSend(file);
  } else if (!batch_marked_as_broken) {
    LOG_ERROR(parent.log, "Marking a batch of {} files as broken, files: {}", files.size(),
              fmt::join(files | std::ranges::views::take(8), "\n"));

    for (const auto& file : files) parent.markAsBroken(file);
  }
...
void DistributedAsyncInsertBatch::serialize() {
  ...
  auto dir_sync_guard = parent.getDirectorySyncGuard(parent.relative_path);
  if (std::filesystem::exists(tmp_file)) LOG_ERROR(parent.log, "Temporary file {} exists. Unclean shutdown?", backQuote(tmp_file));
...
void DistributedAsyncInsertBatch::sendBatch(const SettingsChanges& settings_changes) {
  ...
  try {
    for (const auto& file : files) {
      ...
      tracing_contexts.emplace_back(
          distributed_header.createTracingContextHolder(__PRETTY_FUNCTION__, parent.storage.getContext()->getOpenTelemetrySpanLog()));
      ...
      if (!remote) {
        ...
        auto results = parent.pool->getManyCheckedForInsert(timeouts, insert_settings, PoolMode::GET_ONE,
                                                            parent.storage.remote_storage.getQualifiedName());
        ...
        LOG_DEBUG(parent.log, "Sending a batch of {} files to {} ({} rows, {} bytes).", files.size(), connection->getDescription(),
                  formatReadableQuantity(total_rows), formatReadableSizeWithBinarySuffix(total_bytes));

        remote = std::make_unique<RemoteInserter>(*connection, timeouts, distributed_header.insert_query, insert_settings,
    ...
    if (remote) remote->onFinish();
  } catch (...) {
    try {
      for (auto& tracing_context : tracing_contexts) tracing_context->root_span.addAttribute(std::current_exception());
    } catch (...) {
...
void DistributedAsyncInsertBatch::sendSeparateFiles(const SettingsChanges& settings_changes) {
  ...
  if (broken_files) throw Exception(ErrorCodes::DISTRIBUTED_BROKEN_BATCH_FILES, "Failed to send {} files", broken_files);
}
