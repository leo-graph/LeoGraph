...
ConnectionPoolPtrs createPoolsForAddresses(const Cluster::Addresses& addresses, PoolFactory&& factory, LoggerPtr log) {
  ...
  for (const auto& address : addresses) make_connection(address);

  return pools;
}

uint64_t doubleToUInt64(double d) {
  if (d >= static_cast<double>(std::numeric_limits<uint64_t>::max())) return std::numeric_limits<uint64_t>::max();
  return static_cast<uint64_t>(d);
}

}  // namespace

DistributedAsyncInsertDirectoryQueue::DistributedAsyncInsertDirectoryQueue(StorageDistributed& storage_, const DiskPtr& disk_,
                                                                           const std::string& relative_path_,
                                                                           ConnectionPoolWithFailoverPtr pool_,
                                                                           ActionBlocker& monitor_blocker_, BackgroundSchedulePool& bg_pool)
    : storage(storage_),
      pool(std::move(pool_)),
      disk(disk_),
...
void DistributedAsyncInsertDirectoryQueue::flushAllData(const SettingsChanges& settings_changes) {
  if (pending_files.isFinished()) return;
...
void DistributedAsyncInsertDirectoryQueue::run() {
  ...
  if (!pending_files.isFinished() && do_sleep) task_handle->scheduleAfter(sleep_time.count());
}

ConnectionPoolWithFailoverPtr DistributedAsyncInsertDirectoryQueue::createPool(const Cluster::Addresses& addresses,
  ...
  const auto pool_factory = [&storage](const Cluster::Address& address) -> ConnectionPoolPtr {
    ...
    for (size_t shard_index = 0; shard_index < shards_info.size(); ++shard_index) {
      ...
      for (size_t replica_index = 0; replica_index < replicas_addresses.size(); ++replica_index) {
        ...
        if (address.user == replica_address.user && address.password == replica_address.password &&
            address.host_name == replica_address.host_name && address.port == replica_address.port &&
            address.default_database == replica_address.default_database && address.secure == replica_address.secure &&
    ...
    return std::make_shared<ConnectionPool>(1, /* max_connections */
                                            address.host_name, address.port, address.default_database, address.user, address.password,
                                            address.proto_send_chunked, address.proto_recv_chunked, address.quota_key, address.cluster,
                                            address.cluster_secret, storage.getName() + '_' + address.user, /* client */
                                            Protocol::Compression::Enable, address.secure, address.bind_host);
  };
  ...
  return std::make_shared<ConnectionPoolWithFailover>(std::move(pools), settings[Setting::load_balancing],
...
void DistributedAsyncInsertDirectoryQueue::processFiles(bool force, const SettingsChanges& settings_changes) try {
  if (should_batch_inserts)
    processFilesWithBatching(force, settings_changes);
  else {
    /// Process unprocessed file.
    if (!current_file.empty()) processFile(current_file, settings_changes);
...
void DistributedAsyncInsertDirectoryQueue::processFile(std::string& file_path, const SettingsChanges& settings_changes) {
  ...
  try {
    ...
    thread_trace_context =
        distributed_header.createTracingContextHolder(__PRETTY_FUNCTION__, storage.getContext()->getOpenTelemetrySpanLog());
    ...
    LOG_DEBUG(log, "Sending `{}` to {} ({} rows, {} bytes)", file_path, connection->getDescription(),
              formatReadableQuantity(distributed_header.rows), formatReadableSizeWithBinarySuffix(distributed_header.bytes));

    RemoteInserter remote{*connection, timeouts, distributed_header.insert_query, insert_settings, distributed_header.client_info};
    remote.initialize();
    ...
    if (thread_trace_context) thread_trace_context->root_span.addAttribute(std::current_exception());
...
struct DistributedAsyncInsertDirectoryQueue::BatchHeader {
  ...
  BatchHeader(Settings settings_, String query_, ClientInfo client_info_, Block header_)
      : settings(settings_), query(std::move(query_)), client_info(std::move(client_info_)), header(std::move(header_)) {}

  bool operator==(const BatchHeader& other) const {
    return std::tie(settings, query, client_info.query_kind) == std::tie(other.settings, other.query, other.client_info.query_kind) &&
...
void DistributedAsyncInsertDirectoryQueue::processFilesWithBatching(bool force, const SettingsChanges& settings_changes) {
  ...
  if (fs::exists(current_batch_file_path)) {
    ...
    if (batch.recoverBatch()) batch.send(settings_changes, /*update_current_batch=*/false);
  ...
  try {
    while ((force || !monitor_blocker.isCancelled()) && !pending_files.isFinished() && pending_files.tryPop(file_path)) {
      ...
      try {
        ...
        if (!distributed_header.block_header.empty()) header = distributed_header.block_header;
        ...
        if (!total_rows || header.empty()) {
          ...
          for (Block block = block_in.read(); !block.empty(); block = block_in.read()) {
            ...
            if (header.empty()) header = block.cloneEmpty();
          }
        }
      } catch (const Exception& e) {
        if (isDistributedSendBroken(e.code(), e.isRemoteException())) {
      ...
      BatchHeader batch_header(std::move(*distributed_header.insert_settings), std::move(distributed_header.insert_query),
                               std::move(distributed_header.client_info), std::move(header));
      ...
      if (batch.isEnoughSize()) batch.send(settings_changes, /*update_current_batch=*/true);
    }
    ...
    for (const auto& [_, batch] : header_to_batch) {
      for (const auto& file : batch.files) {
        if (!pending_files.pushFront(file)) LOG_DEBUG(log, "File {} had not been scheduled, since the table had been detached", file);
      }
    }
    /// Rethrow exception
    throw;
...

  {
    ...
    if (fs::exists(current_batch_file_path)) fs::remove(current_batch_file_path);
  }
}
...
SyncGuardPtr DistributedAsyncInsertDirectoryQueue::getDirectorySyncGuard(const std::string& dir_path) {
  if (dir_fsync) return disk->getDirectorySyncGuard(dir_path);
  return nullptr;
}
