...
DistributedAsyncInsertHeader::DistributedAsyncInsertHeader() : insert_settings(std::make_unique<Settings>()) {}
...
DistributedAsyncInsertHeader DistributedAsyncInsertHeader::read(ReadBufferFromFile& in, LoggerPtr log) {
  ...
  if (query_size == DBMS_DISTRIBUTED_SIGNATURE_HEADER) {
    ...
    CityHash_v1_0_2::uint128 calculated_checksum = CityHash_v1_0_2::CityHash128(header_data.data(), header_data.size());
    if (expected_checksum != calculated_checksum) {
      throw Exception(ErrorCodes::CHECKSUM_DOESNT_MATCH, "Checksum of extra info doesn't match: corrupted data. Reference: {}. Actual: {}.",
    ...
    if (header_buf.hasPendingData()) distributed_header.client_info.read(header_buf, distributed_header.revision);
...
OpenTelemetry::TracingContextHolderPtr DistributedAsyncInsertHeader::createTracingContextHolder(
    const char* function, std::shared_ptr<OpenTelemetrySpanLog> open_telemetry_span_log) const {
  OpenTelemetry::TracingContextHolderPtr trace_context =
      std::make_unique<OpenTelemetry::TracingContextHolder>(function, client_info.client_trace_context, std::move(open_telemetry_span_log));
