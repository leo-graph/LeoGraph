...
void StorageObjectStorageSettings::loadFromQuery(ASTSetQuery& settings_ast) { impl->applyChanges(settings_ast.changes); }

Field StorageObjectStorageSettings::get(const std::string& name) { return impl->get(name); }

bool StorageObjectStorageSettings::hasBuiltin(std::string_view name) { return StorageObjectStorageSettingsImpl::hasBuiltin(name); }
