...
class DataLakeConfiguration : public BaseStorageConfiguration, public std::enable_shared_from_this<StorageObjectStorageConfiguration> {
 public:
  ...
  void lazyInitializeIfNeeded(ObjectStoragePtr object_storage, ContextPtr local_context) override {
    if (current_metadata != nullptr) return;
    BaseStorageConfiguration::update(object_storage, local_context);
    assertLocalPathCorrect(object_storage, local_context);
    current_metadata = DataLakeMetadata::create(object_storage, weak_from_this(), local_context);
  }

  void create(ObjectStoragePtr object_storage, ContextPtr local_context, const std::optional<ColumnsDescription> &columns,
              ASTPtr partition_by, ASTPtr order_by, bool if_not_exists, std::shared_ptr<DataLake::ICatalog> catalog,
              const StorageID &table_id_) override {
    BaseStorageConfiguration::update(object_storage, local_context);

    assertLocalPathCorrect(object_storage, local_context);
    DataLakeMetadata::createInitial(object_storage, weak_from_this(), local_context, columns, partition_by, order_by, if_not_exists,
                                    catalog, table_id_);
  }
  ...
  void mutate(const MutationCommands &commands, ContextPtr context, const StorageID &storage_id, StorageMetadataPtr metadata_snapshot,
              std::shared_ptr<DataLake::ICatalog> catalog, const std::optional<FormatSettings> &format_settings) override {
    assertInitialized();
  ...
  ObjectStoragePtr createObjectStorage(
      ContextPtr context, bool is_readonly,
      StorageObjectStorageConfiguration::CredentialsConfigurationCallback refresh_credentials_callback) override {
    if (ready_object_storage) return ready_object_storage;
  ...
  std::optional<ColumnsDescription> tryGetTableStructureFromMetadata(ContextPtr local_context) const override {
    assertInitialized();
    if (auto schema = current_metadata->getTableSchema(local_context); !schema.empty()) return ColumnsDescription(std::move(schema));
    return std::nullopt;
  }
  ...
  std::unique_ptr<StorageInMemoryMetadata> buildStorageMetadataFromState(const DataLakeTableStateSnapshot &state,
                                                                         ContextPtr context) const override {
    assertInitialized();
    auto metadata = current_metadata->buildStorageMetadataFromState(state, context);
    if (metadata)
      LOG_TEST(log, "Built storage metadata from state with columns: {}", metadata->getColumns().toString(/* include_comments */ false));
    return metadata;
  }
  ...
  ObjectIterator iterate(const ActionsDAG *filter_dag, IDataLakeMetadata::FileProgressCallback callback, size_t list_batch_size,
                         StorageMetadataPtr storage_metadata, ContextPtr context) override {
    assertInitialized();
  ...
  DeltaLakePartitionColumns getDeltaLakePartitionColumns() const {
    ...
    if (delta_lake_metadata) return delta_lake_metadata->getPartitionColumns();
    return {};
  }
#endif
  ...
  void drop(ContextPtr local_context) override {
    if (current_metadata) current_metadata->drop(local_context);
  }

  SinkToStoragePtr write(SharedHeader sample_block, const StorageID &table_id, ObjectStoragePtr object_storage,
    ...
    return current_metadata->write(sample_block, table_id, object_storage, shared_from_this(),
                                   format_settings.has_value() ? *format_settings : FormatSettings{}, context, catalog);
  }
  ...
  std::shared_ptr<DataLake::ICatalog> getCatalog([[maybe_unused]] ContextPtr context, [[maybe_unused]] bool is_attach) const override {
    ...
    if ((*settings)[DataLakeStorageSetting::storage_catalog_type].value == DatabaseDataLakeCatalogType::GLUE) {
      auto catalog_parameters =
          DataLake::CatalogSettings{.storage_endpoint = (*settings)[DataLakeStorageSetting::object_storage_endpoint].value,
      ...
      return std::make_shared<DataLake::GlueCatalog>((*settings)[DataLakeStorageSetting::storage_catalog_url].value, context,
                                                     catalog_parameters,
    ...
    if ((*settings)[DataLakeStorageSetting::storage_catalog_type].value == DatabaseDataLakeCatalogType::ICEBERG_REST ||
      ...
      return std::make_shared<DataLake::RestCatalog>(
          (*settings)[DataLakeStorageSetting::storage_warehouse].value, (*settings)[DataLakeStorageSetting::storage_catalog_url].value,
          (*settings)[DataLakeStorageSetting::storage_catalog_credential].value,
          (*settings)[DataLakeStorageSetting::storage_auth_scope].value, (*settings)[DataLakeStorageSetting::storage_auth_header],
          (*settings)[DataLakeStorageSetting::storage_oauth_server_uri].value,
          (*settings)[DataLakeStorageSetting::storage_oauth_server_use_request_body].value, context);
    }
  ...
  bool optimize(const StorageMetadataPtr &metadata_snapshot, ContextPtr context,
  ...
  void addDeleteTransformers(ObjectInfoPtr object_info, QueryPipelineBuilder &builder, const std::optional<FormatSettings> &format_settings,
                             FormatParserSharedResourcesPtr parser_shared_resources, ContextPtr local_context) const override {
  ...
  void fromDisk(const String &disk_name, ASTs &args, ContextPtr context, bool with_structure) override {
    if (!Context::getGlobalContextInstance()->getAllowedDisksForTableEngines().contains(disk_name))
      throw Exception(
          ...
          "Disk {} is not allowed for usage in storage engines. The list of allowed disks is defined by `allowed_disks_for_table_engines",
          disk_name);
 ...
 private:
  ...
  void assertInitialized() const {
    if (!current_metadata) throw Exception(ErrorCodes::LOGICAL_ERROR, "Metadata is not initialized");
  }

  ReadFromFormatInfo prepareReadingFromFormat(ObjectStoragePtr object_storage, const Strings &requested_columns,
                                              const StorageSnapshotPtr &storage_snapshot, bool supports_subset_of_columns,
                                              bool supports_tuple_elements, ContextPtr local_context,
