...
void exportTable(std::shared_ptr<arrow::Table> table, ffi::FFI_ArrowArray &array, ffi::FFI_ArrowSchema &schema) {
  auto batch = table->CombineChunksToBatch();
  if (!batch.ok()) throw DB::Exception(DB::ErrorCodes::UNKNOWN_EXCEPTION, "Failed to create chunks batch: {}", batch.status().ToString());

  arrow::Status status =
      arrow::ExportRecordBatch(**batch, reinterpret_cast<ArrowArray *>(&array), reinterpret_cast<ArrowSchema *>(&schema));

  if (!status.ok()) {
    throw DB::Exception(DB::ErrorCodes::UNKNOWN_EXCEPTION, "Failed to export record batch: {}", status.ToString());
  }
}

std::shared_ptr<arrow::Table> getWriteMetadata(const std::vector<WriteTransaction::CommitFile> &files, LoggerPtr log) {
  DB::ColumnsWithTypeAndName names_and_types{
      {std::make_shared<DB::DataTypeString>(), "path"},
      {std::make_shared<DB::DataTypeMap>(std::make_shared<DB::DataTypeString>(), std::make_shared<DB::DataTypeString>()),
       "partitionValues"},
      {std::make_shared<DB::DataTypeInt64>(), "size"},
      {std::make_shared<DB::DataTypeInt64>(), "modificationTime"},
      {std::make_shared<DB::DataTypeTuple>(DB::DataTypes{std::make_shared<DB::DataTypeString>()}, DB::Names{"stats_json"}), "stats"}};

  DB::MutableColumns columns;
  columns.reserve(names_and_types.size());
  for (const auto &[_, type, name] : names_and_types) columns.push_back(type->createColumn());

  for (const auto &[path, size_bytes, size_rows, partition_values] : files) {
    if (path.empty()) throw DB::Exception(DB::ErrorCodes::LOGICAL_ERROR, "Commit file path cannot be empty");

    LOG_TEST(log, "Committing file: {}, bytes: {}, rows: {}, partition values: {}", path, size_bytes, size_rows, partition_values.size());
  ...
  auto arrow_column = std::make_unique<DB::CHColumnToArrowColumn>(
      DB::Block(names_and_types), "Arrow",
      DB::CHColumnToArrowColumn::Settings{/* output_string_as_string */ true, format_settings.arrow.output_fixed_string_as_fixed_byte_array,
...
WriteTransaction::WriteTransaction(DeltaLake::KernelHelperPtr kernel_helper_)
    : kernel_helper(kernel_helper_), log(getLogger("WriteTransaction")) {}

void WriteTransaction::assertTransactionCreated() const {
  if (!transaction.get()) throw DB::Exception(DB::ErrorCodes::LOGICAL_ERROR, "Transaction was not created");
}
...
void WriteTransaction::create() {
  ...
  auto *new_transaction = DeltaLake::KernelUtils::unwrapResult(
      ffi::transaction(DeltaLake::KernelUtils::toDeltaString(kernel_helper->getTableLocation()), engine.get()), "transaction");
  transaction = DeltaLake::KernelUtils::unwrapResult(
      ffi::with_engine_info(new_transaction, DeltaLake::KernelUtils::toDeltaString(engine_info), engine.get()), "with_engine_info");

  write_context = ffi::get_write_context(transaction.get());
  write_schema = DeltaLake::getWriteSchema(write_context.get());

  auto *write_path_raw = static_cast<std::string *>(ffi::get_write_path(write_context.get(), DeltaLake::KernelUtils::allocateString));
  if (!write_path_raw) throw DB::Exception(DB::ErrorCodes::LOGICAL_ERROR, "Failed to get write path");
  ...
  if (pos == std::string::npos) {
    throw DB::Exception(DB::ErrorCodes::NOT_IMPLEMENTED, "Unexpected path format: {}", write_path);
  }
  ...
  LOG_TEST(log, "Write path: {}, data prefix: {} schema: {}", write_path, path_prefix, write_schema.toString());
}
...
void WriteTransaction::validateSchema(const DB::Block &header) const {
  ...
  if (write_column_names != header_column_names) {
    throw DB::Exception(DB::ErrorCodes::LOGICAL_ERROR, "Header does not match write schema. Expected: {}, got: {}",
...
void WriteTransaction::commit(const std::vector<CommitFile> &files) {
  ...
  try {
    ...
    engine_data =
        DeltaLake::KernelUtils::unwrapResult(ffi::get_engine_data(array, &schema, &KernelUtils::allocateError), "get_engine_data");
  } catch (...) {
