...
DeltaLakeSink::DeltaLakeSink(DeltaLake::WriteTransactionPtr delta_transaction_, ObjectStoragePtr object_storage_, ContextPtr context_,
                             SharedHeader sample_block_, const std::optional<FormatSettings>& format_settings_, const String& format,
...
DeltaLakeSink::StorageSinkPtr DeltaLakeSink::createStorageSink() const {
  return std::make_unique<StorageObjectStorageSink>(DeltaLake::generateWritePath(delta_transaction->getDataPath(), write_format),
                                                    object_storage, format_settings, sample_block, getContext(), write_format,
...
void DeltaLakeSink::consume(Chunk& chunk) {
  ...
  if (data_files.empty() || data_files.back().written_bytes >= data_file_max_bytes ||
