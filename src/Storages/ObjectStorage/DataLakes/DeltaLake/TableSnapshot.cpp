...
Field parseFieldFromString(const String &value, DB::DataTypePtr data_type) {
  try {
    ...
    throw Exception(ErrorCodes::NOT_IMPLEMENTED, "Cannot parse {} for data type {}: {}", value, data_type->getName(),
...
class TableSnapshot::Iterator final : public DB::IObjectIterator {
 private:
  ...
  struct ScannedDataFile {
    ...
    ScannedDataFile(DB::ObjectInfoPtr object_, KernelDvInfo dv_info_, std::optional<KernelExpression> transform_)
        : object(std::move(object_)), dv_info_handle(std::move(dv_info_)), transform_handle(std::move(transform_)) {}
  };

 public:
  using UpdateStatsFunc = std::function<void(SnapshotStats &&)>;

  Iterator(std::shared_ptr<KernelSnapshotState> kernel_snapshot_state_, KernelHelperPtr helper_, const ReadSchema &read_schema_,
           const TableSchema &table_schema_, const DB::NameToNameMap &physical_names_map_, const DB::Names &partition_columns_,
           DB::ObjectStoragePtr object_storage_, const DB::ActionsDAG *filter_, DB::IDataLakeMetadata::FileProgressCallback callback_,
           size_t list_batch_size_, bool enable_expression_visitor_logging_, bool throw_on_engine_predicate_error_,
           bool enable_engine_predicate_, UpdateStatsFunc update_stats_func_, LoggerPtr log_)
    ...
    if (filter_) {
      ...
      pruner.emplace(*filter, table_schema_, partition_columns_, physical_names_map_, DB::Context::getGlobalContextInstance());
    ...
    if (!physical_names_map_.empty()) {
      for (auto &[name, value] : expression_schema) name = getPhysicalName(name, physical_names_map_);

      for (auto &name : partition_columns) name = getPhysicalName(name, physical_names_map_);
    }

    thread = ThreadFromGlobalPool([&, thread_group = DB::CurrentThread::getGroup()] {
  ...
  void initScanState() {
    if (filter.has_value() && enable_engine_predicate) {
      ...
      scan =
          KernelUtils::unwrapResult(ffi::scan(kernel_snapshot_state->snapshot.get(), kernel_snapshot_state->engine.get(), predicate.get(),
                                              /* schema */ nullptr),
                                    "scan");
    } else {
      scan = KernelUtils::unwrapResult(ffi::scan(kernel_snapshot_state->snapshot.get(), kernel_snapshot_state->engine.get(),
                                                 /* predicate */ nullptr,
                                                 /* schema */ nullptr),
                                       "scan");
    }
    ...
    scan_data_iterator =
        KernelUtils::unwrapResult(ffi::scan_metadata_iter_init(kernel_snapshot_state->engine.get(), scan.get()), "scan_metadata_iter_init");
  }
  ...
  void scanDataFunc() {
    try {
      ...
      while (!shutdown.load()) {
        bool have_scan_data_res =
            KernelUtils::unwrapResult(ffi::scan_metadata_next(scan_data_iterator.get(), this, visitData), "scan_metadata_next");

        if (have_scan_data_res) {
          std::unique_lock lock(next_mutex);
          LOG_TEST(log, "List batch size is {}/{}, shutdown: {}", data_files.size(),
                   list_batch_size ? DB::toString(list_batch_size) : "Unlimitted", shutdown.load());
          ...
          LOG_TRACE(log,
                    "All data files at version {} were listed "
                    "(scan exception: {}, total data files: {}, total rows: {}, total bytes: {})",
                    kernel_snapshot_state->snapshot_version, bool(scan_exception), total_data_files,
                    total_rows ? DB::toString(*total_rows) : "Unknown", total_bytes);

          if (update_stats_func && !scan_exception && (!filter.has_value() || !enable_engine_predicate)) {
            update_stats_func(SnapshotStats{.total_bytes = total_bytes,
  ...
  std::optional<UInt64> getSnapshotVersion() const override { return kernel_snapshot_state->snapshot_version; }
  ...
  DB::ObjectInfoPtr next(size_t) override {
    while (true) {
      std::optional<ScannedDataFile> scan_item;
      {
        ...
        if (engine_predicate_exception && throw_on_engine_predicate_error) std::rethrow_exception(engine_predicate_exception);

        if (scan_exception) std::rethrow_exception(scan_exception);
  ...
  void parseTransformHandle(ScannedDataFile &scan_item, DB::ObjectInfoPtr &object) {
    ...
    if (scan_item.transform_handle.has_value()) {
      if (!partition_columns.empty()) {
        metadata->schema_transform = visitScanCallbackExpression(scan_item.transform_handle->get(), read_schema, expression_schema,
                                                                 enable_expression_visitor_logging);

        LOG_TEST(log, "Parsed transform for file: {}, transform: {}", object->getPath(), metadata->schema_transform->dumpNames());
      }
    }
  }
  ...
  void parseDVHandle(ScannedDataFile &scan_item, DB::ObjectInfoPtr &object) {
    ...
    if (auto *dv_info_ptr = scan_item.dv_info_handle.get(); dv_info_ptr && ffi::dv_info_has_vector(dv_info_ptr)) {
      ...
      ffi::KernelRowIndexArray row_indexes = KernelUtils::unwrapResult(
          ffi::row_indexes_from_dv(dv_info_ptr, kernel_snapshot_state->engine.get(), KernelUtils::toDeltaString(getTableLocation())),
          "row_indexes_from_dv");

      SCOPE_EXIT({ ffi::free_row_indexes(row_indexes); });
  ...
  static bool scanCallback(ffi::NullableCvoid engine_context, struct ffi::KernelStringSlice path, int64_t size, int64_t /*mod_time*/,
                           const ffi::Stats *stats, ffi::SharedDvInfo *dv_info, ffi::OptionalValue<ffi::SharedExpression *> transform,
  ...
  static bool scanCallbackImpl(ffi::NullableCvoid engine_context, struct ffi::KernelStringSlice path, int64_t size, const ffi::Stats *stats,
                               ffi::SharedDvInfo *dv_info, ffi::OptionalValue<ffi::SharedExpression *> transform,
                               const struct ffi::CStringMap * /* deprecated */) {
    /// Wrap handles in RAII immediately to ensure cleanup on any exit path
    KernelDvInfo dv_info_handle(dv_info);
    std::optional<KernelExpression> transform_handle;
    if (transform.tag == ffi::OptionalValue<ffi::SharedExpression *>::Tag::Some) transform_handle.emplace(transform.some._0);

    auto *context = static_cast<TableSnapshot::Iterator *>(engine_context);
    if (context->shutdown) {
      LOG_TEST(context->log, "Callback: shutdown detected at first check");
    ...
    if (context->list_batch_size > 0) {
      ...
      if (context->data_files.size() >= context->list_batch_size && !context->shutdown.load()) {
        LOG_TEST(context->log, "Callback pausing: queue size {}/{}", context->data_files.size(), context->list_batch_size);

        context->schedule_next_batch_cv.wait(
            lock, [&]() { return (context->data_files.size() < context->list_batch_size) || context->shutdown.load(); });
      }

      if (context->shutdown.load()) {
        LOG_TEST(context->log, "Callback: shutdown detected after queue wait");
    ...
    LOG_TEST(context->log, "Scanned file: {}, size: {}, num records: {}", object->getPath(), size,
             stats ? DB::toString(stats->num_records) : "Unknown");
...
TableSnapshot::TableSnapshot(std::optional<size_t> version_, KernelHelperPtr helper_, DB::ObjectStoragePtr object_storage_, LoggerPtr log_)
    : helper(helper_), object_storage(object_storage_), log(log_), snapshot_version_to_read(version_) {
...
size_t TableSnapshot::getVersionUnlocked() const { return getKernelSnapshotState()->snapshot_version; }
...
TableSnapshot::SnapshotStats TableSnapshot::getSnapshotStats() const {
  if (!snapshot_stats.has_value()) {
    ...
    LOG_TEST(log, "Updated statistics for snapshot version {}", getVersionUnlocked());
  }
...
TableSnapshot::SnapshotStats TableSnapshot::getSnapshotStatsImpl() const {
  ...
  fallback_scan = KernelUtils::unwrapResult(ffi::scan(state->snapshot.get(), state->engine.get(),
                                                      /* predicate */ nullptr,
                                                      /* schema */ nullptr),
                                            "scan");

  KernelScanMetadataIterator fallback_scan_data_iterator;
  fallback_scan_data_iterator =
      KernelUtils::unwrapResult(ffi::scan_metadata_iter_init(state->engine.get(), fallback_scan.get()), "scan_metadata_iter_init");
  ...
  struct StatsVisitor {
    ...
    static bool visit(ffi::NullableCvoid engine_context, struct ffi::KernelStringSlice /* path */, int64_t size, int64_t /* mod_time */,
                      const ffi::Stats *stats, ffi::SharedDvInfo *dv_info, ffi::OptionalValue<ffi::SharedExpression *> transform,
                      const struct ffi::CStringMap * /* deprecated */) {
      /// Wrap handles in RAII immediately to ensure cleanup on any exit path
  ...
  while (true) {
    bool have_scan_data = KernelUtils::unwrapResult(
        ffi::scan_metadata_next(fallback_scan_data_iterator.get(), &visitor, StatsVisitor::visitData), "scan_metadata_next");

    if (!have_scan_data) break;
  }

  LOG_TEST(log, "Snapshot at version {} data files: {}, total rows: {}, total bytes: {}", state->snapshot_version, visitor.total_data_files,
           visitor.total_rows ? DB::toString(*visitor.total_rows) : "Unknown", visitor.total_bytes);
...
void TableSnapshot::initOrUpdateSnapshot() const {
  if (kernel_snapshot_state) return;
  ...
  LOG_TRACE(log, "Initialized snapshot. Snapshot version: {}", kernel_snapshot_state->snapshot_version);
}
...
TableSnapshot::KernelSnapshotState::KernelSnapshotState(const IKernelHelper &helper_, std::optional<size_t> snapshot_version_) {
  ...
  if (snapshot_version_.has_value()) {
    snapshot = KernelUtils::unwrapResult(
        ffi::snapshot_at_version(KernelUtils::toDeltaString(helper_.getTableLocation()), engine.get(), snapshot_version_.value()),
        "snapshot");
  } else {
    snapshot = KernelUtils::unwrapResult(ffi::snapshot(KernelUtils::toDeltaString(helper_.getTableLocation()), engine.get()), "snapshot");
  }
  snapshot_version = ffi::version(snapshot.get());
  scan = KernelUtils::unwrapResult(ffi::scan(snapshot.get(), engine.get(), /* predicate */ {}, /* engine_schema */ nullptr), "scan");
}

DB::ObjectIterator TableSnapshot::iterate(const DB::ActionsDAG *filter_dag, DB::IDataLakeMetadata::FileProgressCallback callback,
                                          size_t list_batch_size, DB::ContextPtr context) {
  const auto &settings = context->getSettingsRef();
  std::lock_guard lock(mutex);
  initOrUpdateSchemaIfChanged();
  auto state = getKernelSnapshotState();
  auto update_stats_func = [self = shared_from_this(), version = state->snapshot_version, this](SnapshotStats &&stats) {
    std::unique_lock lk(mutex, std::defer_lock);
    if (lk.try_lock()) {
      if (!snapshot_stats.has_value()) {
        snapshot_stats.emplace(std::move(stats));
        LOG_TEST(log, "Updated statistics from data files iterator for snapshot version {}", version);
      }
    }
  };
  return std::make_shared<TableSnapshot::Iterator>(
      state, helper, schema->read_schema, schema->table_schema, schema->physical_names_map, schema->partition_columns, object_storage,
      filter_dag, callback, list_batch_size, settings[DB::Setting::delta_lake_enable_expression_visitor_logging],
      settings[DB::Setting::delta_lake_throw_on_engine_predicate_error], settings[DB::Setting::delta_lake_enable_engine_predicate],
      std::move(update_stats_func), log);
}
...
void TableSnapshot::initOrUpdateSchemaIfChanged() const {
  if (!schema.has_value()) {
    ...
    if (table_schema.empty()) throw DB::Exception(DB::ErrorCodes::BAD_ARGUMENTS, "Table schema cannot be empty");
    ...
    LOG_TRACE(log,
              "Table logical schema: {}, read schema: {}, "
              "partition columns: {}, physical names map size: {}",
              fmt::join(table_schema.getNames(), ", "), fmt::join(read_schema.getNames(), ", "), fmt::join(partition_columns, ", "),
