...
std::unique_ptr<IPartitionStrategy> createPartitionStrategy(const Names& partition_columns, const Block& header, ContextPtr context) {
  ASTs partition_columns_asts;
  for (const auto& column : partition_columns) partition_columns_asts.push_back(make_intrusive<ASTIdentifier>(column));

  ASTPtr partition_by = makeASTFunction("tuple", partition_columns_asts);
  auto key_description = KeyDescription::getKeyFromAST(partition_by, ColumnsDescription(header.getNamesAndTypesList()), context);

  return std::make_unique<HiveStylePartitionStrategy>(key_description, header, context, "parquet",
                                                      /* partition_columns_in_data_file */ false);
}
}  // namespace

DeltaLakePartitionedSink::DeltaLakePartitionedSink(DeltaLake::WriteTransactionPtr delta_transaction_, const Names& partition_columns_,
...
void DeltaLakePartitionedSink::consume(Chunk& chunk) {
  ...
  for (size_t row = 0; row < chunk_rows; ++row) {
    ...
    if (inserted) it->value.first = copyStringInArena(partition_keys_arena, partition_key);
...
DeltaLakePartitionedSink::PartitionInfoPtr DeltaLakePartitionedSink::getPartitionDataForPartitionKey(std::string_view partition_key) {
...
DeltaLakePartitionedSink::StorageSinkPtr DeltaLakePartitionedSink::createSinkForPartition(std::string_view partition_key) {
  auto data_prefix = std::filesystem::path(delta_transaction->getDataPath()) / partition_key;
  return std::make_unique<StorageObjectStorageSink>(DeltaLake::generateWritePath(std::move(data_prefix), write_format), object_storage,
                                                    format_settings, std::make_shared<Block>(partition_strategy->getFormatHeader()),
                                                    getContext(), write_format, write_compression_method);
}

void DeltaLakePartitionedSink::onFinish() {
  if (isCancelled() || partitions_data.empty()) return;
  ...
  for (auto& [_, partition_info] : partitions_data) {
    ...
    for (const auto& [key, value] : keys_and_values) partition_values.emplace_back(DB::Tuple({key, value}));

    for (const auto& [sink, written_bytes, written_rows] : data_files) {
      sink->onFinish();
      files.emplace_back(sink->getPath().substr(data_prefix.size()),
                         /// We use file size from sink to count all file, not just actual data.
                         sink->getFileSize(), written_rows, partition_values);
    }
  }
