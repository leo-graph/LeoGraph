...
DB::ColumnsDescription getPartitionColumnsDescription(const DB::Names& partition_columns, const DB::NamesAndTypesList& table_schema) {
  DB::NamesAndTypesList names_and_types;
  for (const auto& column_name : partition_columns) {
    auto column = table_schema.tryGetByName(column_name);
    if (!column.has_value()) throw DB::Exception(DB::ErrorCodes::LOGICAL_ERROR, "Not found partition column in schema: {}", column_name);
...
PartitionPruner::PartitionPruner(const DB::ActionsDAG& filter_dag, const DB::NamesAndTypesList& table_schema_,
                                 const DB::Names& partition_columns_, const DB::NameToNameMap& physical_names_map_, DB::ContextPtr context)
  ...
  if (!partition_columns_.empty()) {
    ...
    partition_key = DB::KeyDescription::getKeyFromAST(partition_key_ast, partition_columns_description, context);
  ...
  if (!physical_names_map_.empty()) {
    for (auto& name : physical_partition_columns) name = getPhysicalName(name, physical_names_map_);
  }
}

bool PartitionPruner::canBePruned(const DB::ObjectInfo& object_info) const {
  if (!key_condition.has_value()) return false;

  if (!object_info.data_lake_metadata.has_value()) throw DB::Exception(DB::ErrorCodes::LOGICAL_ERROR, "Data lake metadata is not set");
  ...
  auto partition_values =
      DeltaLake::getConstValuesFromExpression(physical_partition_columns, *object_info.data_lake_metadata->schema_transform);

  if (partition_values.empty()) return false;
  ...
  bool can_be_true = key_condition->mayBeTrueInRange(partition_key_values_ref.size(), partition_key_values_ref.data(),
                                                     partition_key_values_ref.data(), partition_key.data_types);
