...
class ReadFromDeltaLakeTableChangesStep : public SourceStepWithFilter {
 public:
  ...
  ReadFromDeltaLakeTableChangesStep(const DeltaLake::TableChangesPtr &table_changes_, const Block &header_, const Names &columns_to_read_,
                                    const SelectQueryInfo &query_info_, const StorageSnapshotPtr &storage_snapshot_, size_t num_streams_,
                                    ContextPtr context_);
