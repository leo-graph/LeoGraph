...
class SchemaVisitorData {
 ...
 private:
  struct Field;
  DB::NamesAndTypesList getNamesAndTypesFromList(size_t list_idx, const std::string &parent_logical_path,
                                                 const std::string &parent_physical_path, DB::NameToNameMap &physical_names_map);
...
class SchemaVisitor {
 ...
 private:
  ...
  static uintptr_t makeFieldList(void *data, uintptr_t capacity_hint) {
    ...
    if (capacity_hint > 0) list->reserve(capacity_hint);
  ...
  static std::unique_ptr<std::string> extractPhysicalName(const ffi::CStringMap *metadata) {
    std::string *physical_name = static_cast<std::string *>(
        ffi::get_from_string_map(metadata, KernelUtils::toDeltaString("delta.columnMapping.physicalName"), KernelUtils::allocateString));
  ...
  static void simpleTypeVisitor(void *data, uintptr_t sibling_list_id, ffi::KernelStringSlice name, bool nullable,
    ...
    LOG_TEST(state->log, "List id: {}, column name: {} (physical name: {}), type: {}, nullable: {}", sibling_list_id, column_name,
             physical_name, type, nullable);
  ...
  static void decimalTypeVisitor(void *data, uintptr_t sibling_list_id, ffi::KernelStringSlice name, bool nullable,
                                 const ffi::CStringMap *metadata, uint8_t precision, uint8_t scale) {
  ...
  static void arrayTypeVisitor(void *data, uintptr_t sibling_list_id, ffi::KernelStringSlice name, bool nullable,
  ...
  static void tupleTypeVisitor(void *data, uintptr_t sibling_list_id, ffi::KernelStringSlice name, bool nullable,
                               const ffi::CStringMap *metadata, uintptr_t child_list_id) {
    listBasedTypeVisitor<DB::TypeIndex::Tuple>(data, sibling_list_id, name, nullable, metadata, child_list_id);
  }

  static void mapTypeVisitor(void *data, uintptr_t sibling_list_id, ffi::KernelStringSlice name, bool nullable,
                             const ffi::CStringMap *metadata, uintptr_t child_list_id) {
  ...
  static void listBasedTypeVisitor(void *data, uintptr_t sibling_list_id, ffi::KernelStringSlice name, bool nullable,
                                   const ffi::CStringMap *metadata, uintptr_t child_list_id) {
    SchemaVisitorData *state = static_cast<SchemaVisitorData *>(data);
    auto it = state->type_lists.find(sibling_list_id);
    if (it == state->type_lists.end()) {
...
DB::NamesAndTypesList SchemaVisitorData::getNamesAndTypesFromList(size_t list_idx, const std::string &parent_logical_path,
  ...
  for (const auto &field : *type_lists[list_idx]) {
    ...
    if (field.is_bool) {
      ...
      if (field.nullable) type = std::make_shared<DB::DataTypeNullable>(type);
      ...
      if (which.isTuple()) {
        ...
        throw DB::Exception(DB::ErrorCodes::NOT_IMPLEMENTED, "Column {} has unsupported complex data type: {}", field.name, field.type);
      }
    }
    chassert(type);
