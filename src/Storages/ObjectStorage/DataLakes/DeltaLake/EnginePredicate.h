...
class EnginePredicate : public ffi::EnginePredicate {
 ...
 public:
  explicit EnginePredicate(const DB::ActionsDAG& filter_, std::exception_ptr& exception_, DB::ContextPtr context_)
      : filter(filter_), exception(exception_), context(context_) {
    predicate = this;
  ...
  void setException(std::exception_ptr exception_) {
    ...
    if (!exception) exception = exception_;
  }
