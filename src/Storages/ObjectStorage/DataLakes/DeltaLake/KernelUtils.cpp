...
std::string KernelUtils::fromDeltaString(ffi::KernelStringSlice slice) { return std::string(slice.ptr, slice.len); }

void* KernelUtils::allocateString(ffi::KernelStringSlice slice) { return new std::string(slice.ptr, slice.len); }
...
uintptr_t ptrToInt(void* error) { return reinterpret_cast<uintptr_t>(error); }
...
void recordKernelErrorAllocation(uintptr_t error, std::string error_message) {
  ...
  if (!inserted) {
    throw DB::Exception(DB::ErrorCodes::LOGICAL_ERROR,
                        "Trying to record already allocated error. (Current Pointer: {}, Current Message: {}, Previous Message: {})", error,
...
ffi::EngineError* KernelUtils::allocateError(ffi::KernelError etype, ffi::KernelStringSlice message) {
  ...
  LOG_TRACE(getLogger("KernelUtils"), "Allocated KernelError (Pointer: {}, EType: {}, Message: {})", ptrToInt(error), etype, error_message);

  return error;
}

[[noreturn]] void KernelUtils::throwError(ffi::EngineError* error, const std::string& from) {
  if (error == nullptr) throw DB::Exception(DB::ErrorCodes::LOGICAL_ERROR, "Unpacking nullptr DeltaLake kernel error. (in: {})", from);

  uintptr_t error_ptr = ptrToInt(error);

  if (isKernelErrorAllocation(error_ptr)) {
    LOG_TRACE(getLogger("KernelUtils"), "Deallocating KernelError (Pointer: {}) (in {})", error_ptr, from);
    ...
    throw DB::Exception(DB::ErrorCodes::DELTA_KERNEL_ERROR, "Received DeltaLake kernel error {}: {} (in {})", etype, error_message, from);
  } else {
    ffi::KernelError etype = error->etype;
    delete error;

    LOG_ERROR(getLogger("KernelUtils"), "Received unknown error from DeltaLake kernel. (Pointer: {}, EType: {}) (in {})", error_ptr, etype,
              from);

    throw DB::Exception(DB::ErrorCodes::LOGICAL_ERROR, "Received unknown error from DeltaLake kernel. (Pointer: {}, EType: {}) (in {})",
...
std::optional<std::string> tryGetPhysicalName(const std::string& name, const DB::NameToNameMap& physical_names_map) {
  if (physical_names_map.empty()) return name;

  auto it = physical_names_map.find(name);
  if (it == physical_names_map.end()) return std::nullopt;
  return it->second;
}
...
std::string getPhysicalName(const std::string& name, const DB::NameToNameMap& physical_names_map) {
  ...
  if (!physical_name) {
    ...
    for (const auto& [key, _] : physical_names_map) keys.push_back(key);

    throw DB::Exception(DB::ErrorCodes::LOGICAL_ERROR, "Not found column {} in physical names map. There are only columns: {}", name,
                        fmt::join(keys, ", "));
  }
