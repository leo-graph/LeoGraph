...
TableChanges::TableChanges(const TableChangesVersionRange &version_range_, KernelHelperPtr helper_, const DB::Block &header_,
                           const std::optional<DB::FormatSettings> &format_settings_, const std::string &format_name_,
...
TableChangesVersionRange TableChanges::getVersionRange(int64_t start_version, int64_t end_version) {
  if (start_version < 0)
    throw DB::Exception(DB::ErrorCodes::BAD_ARGUMENTS, "Snapshot start version cannot be a negative value ({})", start_version);

  if (end_version != TableSnapshot::LATEST_SNAPSHOT_VERSION) {
    if (end_version < 0)
      throw DB::Exception(DB::ErrorCodes::BAD_ARGUMENTS, "Snapshot end version cannot be a negative value ({})", end_version);

    if (end_version < start_version)
      throw DB::Exception(DB::ErrorCodes::BAD_ARGUMENTS, "Snapshot end version cannot be less than snapshot start version ({} < {})",
                          end_version, start_version);
  }

  return end_version == TableSnapshot::LATEST_SNAPSHOT_VERSION ? DeltaLake::TableChangesVersionRange{start_version, std::nullopt}
...
DB::NamesAndTypesList TableChanges::getSchemaUnlocked() const {
  if (schema.has_value()) return schema.value();
...
TableChanges::KernelTableChanges &TableChanges::getTableChanges() const {
  if (table_changes.get()) return table_changes;

  auto *engine_builder = helper->createBuilder();
  engine = KernelUtils::unwrapResult(ffi::builder_build(engine_builder), "builder_build");
  table_changes =
      version_range.second.has_value()
          ? KernelUtils::unwrapResult(ffi::table_changes_between_versions(KernelUtils::toDeltaString(helper->getTableLocation()),
                                                                          engine.get(), version_range.first, version_range.second.value()),
                                      "table_changes_between_versions")
          : KernelUtils::unwrapResult(
                ffi::table_changes_from_version(KernelUtils::toDeltaString(helper->getTableLocation()), engine.get(), version_range.first),
...
TableChanges::KernelTableChangesScanIterator &TableChanges::getTableChangesScanIterator() const {
  if (table_changes_scan_iterator.get()) return table_changes_scan_iterator;
  ...
  if (filter.has_value()) {
    ...
    table_changes_scan = KernelUtils::unwrapResult(ffi::table_changes_scan(getTableChanges().release(), engine.get(), predicate.get()),
                                                   "table_changes_scan (with predicate)");
  } else {
    table_changes_scan = KernelUtils::unwrapResult(ffi::table_changes_scan(getTableChanges().release(), engine.get(),
                                                                           /* predicate */ nullptr),
                                                   "table_changes_scan");
  }

  if (engine_predicate_exception) std::rethrow_exception(engine_predicate_exception);

  table_changes_scan_iterator =
      KernelUtils::unwrapResult(ffi::table_changes_scan_execute(table_changes_scan.get(), engine.get()), "table_changes_scan_execute");
...
DB::Chunk TableChanges::next() {
  ...
  ffi::ArrowFFIData ffi_arrow_data =
      KernelUtils::unwrapResult(ffi::scan_table_changes_next(getTableChangesScanIterator().get()), "scan_table_changes_next");

  if (ffi_arrow_data.array.length == 0) return {};

  auto record_batch = arrow::ImportRecordBatch(reinterpret_cast<ArrowArray *>(&ffi_arrow_data.array),
                                               reinterpret_cast<ArrowSchema *>(&ffi_arrow_data.schema));

  if (!record_batch.ok()) {
    throw DB::Exception(DB::ErrorCodes::UNKNOWN_EXCEPTION, "Failed to create chunks batch: {}", record_batch.status().ToString());
  }

  auto table_result = arrow::Table::FromRecordBatches(std::vector<std::shared_ptr<arrow::RecordBatch>>{*record_batch});
  if (!table_result.ok())
    throw DB::Exception(DB::ErrorCodes::UNKNOWN_EXCEPTION, "Error while reading batch of Arrow data: {}", table_result.status().ToString());
  ...
  DB::ArrowColumnToCHColumn arrow_conv(header, format_name, format_settings,
                                       /* parquet_columns_to_clickhouse */ std::nullopt,
                                       /* clickhouse_columns_to_parquet */ std::nullopt, format_settings.parquet.allow_missing_columns,
                                       format_settings.null_as_default, format_settings.date_time_overflow_behavior,
                                       format_settings.parquet.allow_geoparquet_parser,
                                       format_settings.parquet.case_insensitive_column_matching,
                                       /* is_stream_ */ false, format_settings.parquet.enable_json_parsing);
