...
class DeltaLakeTableChangesSource : public ISource {
 public:
  DeltaLakeTableChangesSource(DeltaLake::TableChangesPtr table_changes_, std::shared_ptr<const Block> source_header_)
      : ISource(source_header_, false), table_changes(table_changes_) {}

  String getName() const override { return "DeltaLakeTableChangesSource"; }

  Chunk generate() override { return table_changes->next(); }
...
ReadFromDeltaLakeTableChangesStep::ReadFromDeltaLakeTableChangesStep(const DeltaLake::TableChangesPtr &table_changes_, const Block &header_,
                                                                     const Names &columns_to_read_, const SelectQueryInfo &query_info_,
                                                                     const StorageSnapshotPtr &storage_snapshot_, size_t num_streams_,
                                                                     ContextPtr context_)
    : SourceStepWithFilter(std::make_shared<const Block>(header_), columns_to_read_, query_info_, storage_snapshot_, context_),
      table_changes(table_changes_),
      header(header_),
      num_streams(num_streams_) {}

QueryPlanStepPtr ReadFromDeltaLakeTableChangesStep::clone() const { return std::make_unique<ReadFromDeltaLakeTableChangesStep>(*this); }
...
void ReadFromDeltaLakeTableChangesStep::initializePipeline(QueryPipelineBuilder &pipeline, const BuildQueryPipelineSettings &) {
  if (filter_actions_dag) table_changes->setFilter(filter_actions_dag.get());
  ...
  if (pipe.empty()) pipe = Pipe(std::make_shared<NullSource>(source_header));
