...
class ExpressionVisitorData {
 ...
 public:
  ...
  explicit ExpressionVisitorData(const DB::NamesAndTypesList &read_schema_, const DB::NamesAndTypesList &expression_schema_,
  ...
  std::shared_ptr<DB::ActionsDAG> getScanCallbackExpressionResult() {
    ...
    if (node_lists.size() != 1) {
      throw DB::Exception(DB::ErrorCodes::LOGICAL_ERROR, "Unexpected size of a result expression: {}", node_lists.size());
    }
    if (node_lists[0].size() != 1) {
      throw DB::Exception(DB::ErrorCodes::LOGICAL_ERROR, "Unexpected size of a result expression at root node: {}", node_lists[0].size());
    }

    const auto &nodes = node_lists[0][0]->children;
    if (nodes.size() != expression_schema.size()) {
      throw DB::Exception(DB::ErrorCodes::LOGICAL_ERROR, "Unexpected size of expression list: {} (expected: {})", nodes.size(),
                          expression_schema.size());
    }
    ...
    for (const auto &node : nodes) {
      if (enable_logging) LOG_TEST(log, "Node type: {}, result name: {}", node->type, node->result_name);
      ...
      if (node->type == DB::ActionsDAG::ActionType::COLUMN) {
        ...
        if (node->result_name != schema_it->name) {
          throw DB::Exception(DB::ErrorCodes::LOGICAL_ERROR, "Expected column at pos {} to have name {}, got {}",
      ...
      if (enable_logging)
        LOG_TEST(log, "Added output: {}, type: {}", column_with_type_and_name.name, column_with_type_and_name.type->getTypeId());
    ...
    DB::FunctionOverloadResolverPtr function =
        std::make_unique<DB::FunctionToOverloadResolverAdaptor>(std::make_shared<DB::FunctionTuple>());
  ...
  void setException(std::exception_ptr exception) {
    if (!visitor_exception) visitor_exception = exception;
  }
  ...
  size_t makeNewList(size_t capacity_hint) {
    ...
    chassert(inserted);
    if (capacity_hint > 0) it->second.reserve(capacity_hint);
    return id;
  }
  ...
  void addLiteral(size_t list_id, DB::Field value, DB::DataTypePtr type) {
    ...
    auto column = DB::ColumnWithTypeAndName(col, type,
    ...
    if (enable_logging) LOG_TEST(log, "Added list id {}", list_id);
  }
  ...
  void addIdentifier(size_t list_id, const std::string &name) {
    ...
    if (!name_and_type.has_value()) {
      throw DB::Exception(DB::ErrorCodes::LOGICAL_ERROR, "Cannot find column {} in schema ({})", name, expression_schema.toString());
    }

    auto column = DB::ColumnWithTypeAndName(name_and_type->type->createColumn(), name_and_type->type, name_and_type->name);
  ...
  void addFunction(size_t list_id, size_t child_list_id, DB::FunctionOverloadResolverPtr function) {
    ...
    if (it == node_lists.end()) {
      throw DB::Exception(DB::ErrorCodes::LOGICAL_ERROR, "Cannot find child list id {}", child_list_id);
    }
    ...
    if (enable_logging) LOG_TEST(log, "Removed list id {}", child_list_id);
  ...
  std::pair<ValueContainer, DB::DataTypes> extractLiteralList(size_t list_id) {
    ...
    if (it == node_lists.end()) {
      throw DB::Exception(DB::ErrorCodes::LOGICAL_ERROR, "Cannot find list id {}", list_id);
    }
    ...
    for (const auto &node : it->second) {
      if (node->type != DB::ActionsDAG::ActionType::COLUMN) {
        throw DB::Exception(DB::ErrorCodes::LOGICAL_ERROR, "Not a constant column: {} (list id: {})", magic_enum::enum_name(node->type),
                            list_id);
      }
    ...
    if (enable_logging) LOG_TEST(log, "Removed list id {}", list_id);
  ...
  void moveList(size_t to_list_id, size_t from_list_id, const std::string *after_field_name, bool prepend) {
    auto from_it = node_lists.find(from_list_id);
    if (from_it == node_lists.end()) {
      throw DB::Exception(DB::ErrorCodes::LOGICAL_ERROR, "Cannot find list id {}", from_list_id);
    }

    auto to_it = node_lists.find(to_list_id);
    if (to_it == node_lists.end()) {
      throw DB::Exception(DB::ErrorCodes::LOGICAL_ERROR, "Cannot find list id {}", to_list_id);
    }
    ...
    if (to_list.empty()) {
      for (const auto &elem : read_schema) addIdentifier(to_list_id, elem.name);
    ...
    if (after_field_name) {
      ...
      for (auto elem_it = to_list.begin(); elem_it != to_list.end(); ++elem_it) {
        ...
        if (node->type == DB::ActionsDAG::ActionType::INPUT && node->result_name == *after_field_name) {
          result_pos = pos;
          to_list.insert(std::next(elem_it), std::make_move_iterator(from_list.begin()), std::make_move_iterator(from_list.end()));
          break;
        }
      }
      if (!result_pos) {
        throw DB::Exception(DB::ErrorCodes::LOGICAL_ERROR, "Did not find {} in list by id {}", *after_field_name, to_list_id);
      }
    } else {
      /// FIXME: prepend is suboptimal,
      /// but DB::ActionsDAG::NodeRawConstPtrs is a vector
      /// and changing it to deque is not an option.
      to_list.insert(prepend ? to_list.begin() : to_list.end(), std::make_move_iterator(from_list.begin()),
    ...
    if (enable_logging) {
      LOG_TEST(log,
               "Appended to list id {} from list id {} "
               "(size: {}, appended after field: {})",
               to_list_id, from_list_id, to_it->second.size(), after_field_name ? *after_field_name : "None");
    }
  }
};
...
class ExpressionVisitor {
 public:
  static void visit(const ffi::Expression *expression, ExpressionVisitorData &data) {
    ...
    if (auto e = data.getException()) std::rethrow_exception(e);
  }
 ...
 private:
  ...
  static void throwNotImplemented(void *data, uintptr_t sibling_list_id, uintptr_t child_list_id) {
    UNUSED(sibling_list_id);
    UNUSED(child_list_id);

    ExpressionVisitorData *state = static_cast<ExpressionVisitorData *>(data);
    visitorImpl(*state, [&]() {
      throw DB::Exception(DB::ErrorCodes::NOT_IMPLEMENTED, "Method {} not implemented", magic_enum::enum_name(method));
    });
  }

  static void throwNotImplementedOpaqueExpression(void *data, uintptr_t sibling_list_id, ffi::SharedOpaqueExpressionOp *op,
    ...
    ExpressionVisitorData *state = static_cast<ExpressionVisitorData *>(data);
    visitorImpl(*state, [&]() { throw DB::Exception(DB::ErrorCodes::NOT_IMPLEMENTED, "Method OpaqueExpr not implemented"); });
  }

  static void throwNotImplementedOpaquePredicate(void *data, uintptr_t sibling_list_id, ffi::SharedOpaquePredicateOp *op,
                                                 uintptr_t child_list_id) {
    UNUSED(sibling_list_id);
    UNUSED(child_list_id);
    ...
    ExpressionVisitorData *state = static_cast<ExpressionVisitorData *>(data);
    visitorImpl(*state, [&]() { throw DB::Exception(DB::ErrorCodes::NOT_IMPLEMENTED, "Method OpaquePred not implemented"); });
  }

  static void throwNotImplementedUnknown(void *data, uintptr_t sibling_list_id, ffi::KernelStringSlice name) {
    UNUSED(data);
    UNUSED(sibling_list_id);

    ExpressionVisitorData *state = static_cast<ExpressionVisitorData *>(data);
    visitorImpl(*state, [&]() {
      throw DB::Exception(DB::ErrorCodes::NOT_IMPLEMENTED, "Method Unknown not implemented (name: {})", KernelUtils::fromDeltaString(name));
    });
  }

  template <typename Func>
  static void visitFunction(void *data, uintptr_t sibling_list_id, uintptr_t child_list_id) {
    ExpressionVisitorData *state = static_cast<ExpressionVisitorData *>(data);
    visitorImpl(*state, [&]() {
      if (state->enableLogging())
        LOG_TEST(state->logger(), "List id: {}, child list id: {}, type: Function {}", sibling_list_id, child_list_id, Func::name);
  ...
  static void visitColumnExpression(void *data, uintptr_t sibling_list_id, ffi::KernelStringSlice name) {
    ...
    visitorImpl(*state, [&]() {
      ...
      if (state->enableLogging()) LOG_TEST(state->logger(), "List id: {}, name: {}, type: Column", sibling_list_id, name_str);

      state->addIdentifier(sibling_list_id, name_str);
    });
  }

  static void visitStructExpression(void *data, uintptr_t sibling_list_id, uintptr_t child_list_id) {
    ExpressionVisitorData *state = static_cast<ExpressionVisitorData *>(data);
    visitorImpl(*state, [&]() {
      if (state->enableLogging())
        LOG_TEST(state->logger(), "List id: {}, child list id: {}, type: StructExpression", sibling_list_id, child_list_id);
  ...
  static void visitSimpleLiteral(void *data, uintptr_t sibling_list_id, T value) {
    ExpressionVisitorData *state = static_cast<ExpressionVisitorData *>(data);
    visitorImpl(*state, [&]() {
      if (state->enableLogging()) LOG_TEST(state->logger(), "List id: {}, type: {}", sibling_list_id, DataType::type_id);
  ...
  static void visitDecimalLiteral(void *data, uintptr_t sibling_list_id, int64_t value_ms, uint64_t value_ls, uint8_t precision,
                                  uint8_t scale) {
    ExpressionVisitorData *state = static_cast<ExpressionVisitorData *>(data);
    visitorImpl(*state, [&]() {
      ...
      if (state->enableLogging()) LOG_TEST(state->logger(), "List id: {}, type: Decimal", sibling_list_id);
    });
  }
  ...
  static void visitDateLiteral(void *data, uintptr_t sibling_list_id, int32_t value) {
    ExpressionVisitorData *state = static_cast<ExpressionVisitorData *>(data);
    visitorImpl(*state, [&]() {
      if (state->enableLogging()) LOG_TEST(state->logger(), "List id: {}, type: Date", sibling_list_id);
  ...
  static void visitTimestampLiteral(void *data, uintptr_t sibling_list_id, int64_t value) {
    ExpressionVisitorData *state = static_cast<ExpressionVisitorData *>(data);
    visitorImpl(*state, [&]() {
      if (state->enableLogging()) LOG_TEST(state->logger(), "List id: {}, type: Timestamp", sibling_list_id);
  ...
  static void visitTimestampNtzLiteral(void *data, uintptr_t sibling_list_id, int64_t value) {
    ExpressionVisitorData *state = static_cast<ExpressionVisitorData *>(data);
    visitorImpl(*state, [&]() {
      if (state->enableLogging()) LOG_TEST(state->logger(), "List id: {}, type: TimestampNtz", sibling_list_id);
  ...
  static void visitBinaryLiteral(void *data, uintptr_t sibling_list_id, const uint8_t *buffer, uintptr_t len) {
    ExpressionVisitorData *state = static_cast<ExpressionVisitorData *>(data);
    visitorImpl(*state, [&]() {
      if (state->enableLogging()) LOG_TEST(state->logger(), "List id: {}, type: Binary", sibling_list_id);
  ...
  static void visitNullLiteral(void *data, uintptr_t sibling_list_id) {
    ExpressionVisitorData *state = static_cast<ExpressionVisitorData *>(data);
    visitorImpl(*state, [&]() {
      if (state->enableLogging()) LOG_TEST(state->logger(), "List id: {}, type: Null", sibling_list_id);

      state->addLiteral(sibling_list_id, DB::Null(), std::make_shared<DB::DataTypeNullable>(std::make_shared<DB::DataTypeNothing>()));
    });
  }
  ...
  static void visitArrayLiteral(void *data, uintptr_t sibling_list_id, uintptr_t child_list_id) {
    ExpressionVisitorData *state = static_cast<ExpressionVisitorData *>(data);
    visitorImpl(*state, [&]() {
      if (state->enableLogging()) LOG_TEST(state->logger(), "List id: {}, child list id: {}, type: Array", sibling_list_id, child_list_id);

      auto [values, types] = state->extractLiteralList<DB::Array>(child_list_id);
      state->addLiteral(sibling_list_id, std::move(values), std::make_shared<DB::DataTypeArray>(types[0]));
    });
  }

  static void visitStructLiteral(void *data, uintptr_t sibling_list_id, uintptr_t child_field_list_id, uintptr_t child_value_list_id) {
    ExpressionVisitorData *state = static_cast<ExpressionVisitorData *>(data);
    visitorImpl(*state, [&]() {
      if (state->enableLogging())
        LOG_TEST(state->logger(), "List id: {}, child field list id: {}, child value list id: {}, type: Struct", sibling_list_id,
                 child_field_list_id, child_value_list_id);
  ...
  static void visitMapLiteral(void *data, uintptr_t sibling_list_id, uintptr_t key_list_id, uintptr_t value_list_id) {
    ExpressionVisitorData *state = static_cast<ExpressionVisitorData *>(data);
    visitorImpl(*state, [&]() {
      if (state->enableLogging())
        LOG_TEST(state->logger(), "List id: {}, key list id: {}, value list id: {}, type: Map", sibling_list_id, key_list_id,
                 value_list_id);
      ...
      if (keys.empty()) throw DB::Exception(DB::ErrorCodes::LOGICAL_ERROR, "Cannot have empty keys");

      if (keys.size() != values.size()) {
        throw DB::Exception(DB::ErrorCodes::LOGICAL_ERROR, "Keys size does not equal values size. Keys: {}, values: {}", keys.size(),
                            values.size());
      }
  ...
  static void visitTransformExpression(void *data, uintptr_t sibling_list_id, uintptr_t input_path_list_id,
    ...
    visitorImpl(*state, [&]() {
      if (state->enableLogging())
        LOG_TEST(state->logger(), "List id: {}, input path list id: {}, field transform list id: {}", sibling_list_id, input_path_list_id,
                 field_transform_list_id);

      if (input_path_list_id) {
        throw DB::Exception(DB::ErrorCodes::NOT_IMPLEMENTED, "Non zero input_path_list_id is not supported at the moment");
      }
  ...
  static void visitFieldTransform(void *data, uintptr_t sibling_list_id, const ffi::KernelStringSlice *field_name, uintptr_t expr_list_id,
                                  bool is_replace) {
    ExpressionVisitorData *state = static_cast<ExpressionVisitorData *>(data);
    visitorImpl(*state, [&]() {
      std::string field_name_str;
      if (field_name) field_name_str = KernelUtils::fromDeltaString(*field_name);

      if (state->enableLogging())
        LOG_TEST(state->logger(), "List id: {}, field name: {}, expr list id: {}, is replace: {}", sibling_list_id, field_name_str,
                 expr_list_id, is_replace);
      ...
      if (expr_list_id) {
        state->moveList(sibling_list_id, expr_list_id, field_name ? &field_name_str : nullptr,
                        /* prepend */ !field_name);
      } else {
        /// Will be implemented with CDF support.
...
std::vector<DB::Field> getConstValuesFromExpression(const DB::Names &columns, const DB::ActionsDAG &dag) {
  ...
  for (const auto &node : nodes) {
    if (node->type != DB::ActionsDAG::ActionType::COLUMN || !DB::isColumnConst(*node->column)) {
      throw DB::Exception(DB::ErrorCodes::LOGICAL_ERROR, "Not a constant column: {} (column type: {})", magic_enum::enum_name(node->type),
                          node->column->getDataType());
    }
