...
ChunkPartitioner::ChunkPartitioner(Poco::JSON::Array::Ptr partition_specification, Poco::JSON::Object::Ptr schema, ContextPtr context,
                                   SharedHeader sample_block_)
    : sample_block(sample_block_), max_partitions_count(context->getSettingsRef()[Setting::iceberg_insert_max_partitions]) {
  ...
  for (size_t i = 0; i != partition_specification->size(); ++i) {
    ...
    if (!transform_and_argument) throw Exception(ErrorCodes::BAD_ARGUMENTS, "Unknown transform {}", transform_name);
...
size_t ChunkPartitioner::PartitionKeyHasher::operator()(const PartitionKey& key) const {
  size_t result = 0;
  for (const auto& part_key : key) result ^= hasher(part_key.dump());
  return result;
}

std::vector<std::pair<ChunkPartitioner::PartitionKey, Chunk>> ChunkPartitioner::partitionChunk(const Chunk& chunk) {
  ...
  auto get_partition = [&](size_t row_num) { return transform_results[row_num]; };
  ...
  if (partitions_count > max_partitions_count)
    throw Exception(ErrorCodes::BAD_ARGUMENTS,
                    "Too many partitions per insert operation: {}. The maximum allowed number of partitions is {}. To change this, use the "
                    "`iceberg_insert_max_partitions` setting",
                    partitions_count, max_partitions_count);
  ...
  for (size_t col = 0; col < chunk.getNumColumns(); ++col) {
    if (partitions_count > 1) {
      ...
      for (size_t i = 0; i < partitions_count; ++i) result_columns[i].second[col] = std::move(scattered[i]);
    } else {
