...
struct ManifestFilePlan {
  explicit ManifestFilePlan(Poco::JSON::Array::Ptr schema_) : statistics(schema_) {}
  ...
  class ParititonEncoder {
   public:
    size_t encodePartition(const Row& row) {
      if (auto it = partition_value_to_index.find(row); it != partition_value_to_index.end()) return it->second;
   ...
   private:
    struct PartitionValueHasher {
      ...
      size_t operator()(const Row& row) const {
        size_t result = 0;
        for (const auto& value : row) result ^= hasher(value.dump());
        return result;
      }
    };
...
Plan getPlan(IcebergHistory snapshots_info, const DataLakeStorageSettings& data_lake_settings,
             const PersistentTableComponents& persistent_table_components, ObjectStoragePtr object_storage, const String& write_format,
             ContextPtr context, CompressionMethod compression_method) {
  LoggerPtr log = getLogger("IcebergCompaction::getPlan");

  Plan plan;
  plan.generator = FileNamesGenerator(persistent_table_components.table_path, persistent_table_components.table_path, false,
                                      compression_method, write_format);
  ...
  Poco::JSON::Object::Ptr initial_metadata_object =
      getMetadataJSONObject(metadata_file_path, object_storage, persistent_table_components.metadata_cache, context, log,
  ...
  for (const auto& snapshot : snapshots_info) {
    ...
    for (const auto& manifest_file : manifest_list) {
      ...
      auto files_handle = getManifestFileEntriesHandle(object_storage, persistent_table_components, context, log, manifest_file,
      ...
      for (const auto& data_file : files_handle.getFilesWithoutDeleted(FileContentType::DATA)) {
        ...
        if (plan.partitions.size() <= partition_index) plan.partitions.push_back({});
        ...
        if (!plan.path_to_data_file.contains(manifest_file.manifest_file_path)) {
          data_file_ptr = std::make_shared<DataFilePlan>(DataFilePlan{.data_object_info = data_object_info,
  ...
  for (const auto& delete_file : all_positional_delete_files) {
    ...
    if (partition_index >= plan.partitions.size()) continue;
...
static void writeDataFiles(Plan& initial_plan, SharedHeader sample_block, ObjectStoragePtr object_storage,
                           const std::optional<FormatSettings>& format_settings, ContextPtr context, const String& write_format,
  ...
  for (auto& [_, data_file] : initial_plan.path_to_data_file) {
    auto delete_file_transform = std::make_shared<IcebergBitmapPositionDeleteTransform>(
        sample_block, data_file->data_object_info, object_storage, format_settings,
        // todo make compaction using same FormatParserSharedResources
        std::make_shared<FormatParserSharedResources>(context->getSettingsRef(), 1), context);
    ...
    auto parser_shared_resources = std::make_shared<FormatParserSharedResources>(settings,
                                                                                 /*num_streams_=*/1);

    auto input_format = FormatFactory::instance().getInput(
        data_file->data_object_info->getFileFormat().value_or(write_format), *read_buffer, *sample_block, context, 8192, format_settings,
        parser_shared_resources, std::make_shared<FormatFilterInfo>(nullptr, context, nullptr, nullptr, nullptr), true /* is_remote_fs */,
        chooseCompressionMethod(data_file->data_object_info->getPath(), toContentEncodingName(write_compression_method)), false);

    auto write_buffer = object_storage->writeObject(StoredObject(data_file->patched_path.path_in_storage), WriteMode::Rewrite, std::nullopt,
                                                    DBMS_DEFAULT_BUFFER_SIZE, context->getWriteSettings());

    auto output_format = FormatFactory::instance().getOutputFormat(write_format, *write_buffer, *sample_block, context, format_settings);
...
void writeMetadataFiles(Plan& plan, ObjectStoragePtr object_storage, ContextPtr context, SharedHeader sample_block_, String write_format,
                        String table_path) {
  ...
  for (const auto& history_record : plan.history) {
    ...
    auto new_snapshot = metadata_generator.generateNextMetadata(plan.generator, generated_metadata_name.path_in_metadata,
                                                                history_record.parent_id, history_record.added_files, total_records_count,
                                                                history_record.added_files_size, history_record.num_partitions, 0, 0,
                                                                history_record.snapshot_id, history_record.made_current_at.value);
...

  {
    ...
    for (auto& [manifest_entry, data_filenames] : grouped_by_manifest_files_result) {
      ...
      auto buffer_manifest_entry =
          object_storage->writeObject(StoredObject(manifest_entry->patched_path.path_in_storage), WriteMode::Rewrite, std::nullopt,
                                      DBMS_DEFAULT_BUFFER_SIZE, context->getWriteSettings());
      ...
      generateManifestFile(metadata_object, partition_columns,
                           ...
                           std::vector(data_filenames.begin(), data_filenames.end()), manifest_entry->statistics, sample_block_, snapshot,
                           write_format, partititon_spec, partition_spec_id, *buffer_manifest_entry, Iceberg::FileContentType::DATA);
  ...
  for (size_t i = 0; i < plan.history.size(); ++i) {
    if (plan.history[i].added_files == 0) continue;
  ...
  for (size_t i = 0; i < plan.history.size(); ++i) {
    ...
    auto buffer_manifest_list = object_storage->writeObject(StoredObject(renamed_manifest_list), WriteMode::Rewrite, std::nullopt,
                                                            DBMS_DEFAULT_BUFFER_SIZE, context->getWriteSettings());
    generateManifestList(plan.generator, metadata_object, object_storage, context, renamed_manifest_entries, new_snapshots[i].snapshot,
                         total_manifest_file_sizes, *buffer_manifest_list, Iceberg::FileContentType::DATA, false);
...

  {
    ...
    auto buffer_metadata = object_storage->writeObject(StoredObject(generated_metadata_name.path_in_storage), WriteMode::Rewrite,
...
std::vector<String> getOldFiles(ObjectStoragePtr object_storage, const String& table_path) {
  ...
  for (auto&& data_file : data_files) metadata_files.push_back(data_file);
...
void compactIcebergTable(IcebergHistory snapshots_info, const PersistentTableComponents& persistent_table_components,
                         ObjectStoragePtr object_storage_, const DataLakeStorageSettings& data_lake_settings,
                         const std::optional<FormatSettings>& format_settings_, SharedHeader sample_block_, ContextPtr context_,
                         const String& write_format) {
  auto plan = getPlan(std::move(snapshots_info), data_lake_settings, persistent_table_components, object_storage_, write_format, context_,
                      persistent_table_components.metadata_compression_method);
  if (plan.need_optimize) {
    auto old_files = getOldFiles(object_storage_, persistent_table_components.table_path);
    writeDataFiles(plan, sample_block_, object_storage_, format_settings_, context_, write_format,
                   persistent_table_components.metadata_compression_method);
