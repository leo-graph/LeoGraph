...
// We work a lot with avro library. Clang analyzer is about GenericDatum structure. It thinks that value in generic datum can be
// uninitialized. No idea why
namespace {
...
std::vector<uint8_t> dumpFieldToBytes(const Field &field, DataTypePtr type) {
  switch (type->getTypeId()) {
    ...
    case TypeIndex::String: {
      ...
      for (auto elem : value) bytes.push_back(elem);
      return bytes;
    }
...
bool canWriteStatistics(const std::vector<std::pair<size_t, Field>> &statistics,
                        const std::unordered_map<size_t, size_t> &field_id_to_column_index, SharedHeader sample_block) {
  if (statistics.empty()) return false;

  for (const auto &[field_id, stat] : statistics) {
    auto type = sample_block->getDataTypes()[field_id_to_column_index.at(field_id)];
    if (!canDumpIcebergStats(stat, type)) return false;
  }
  return true;
}
...
void extendSchemaForPartitions(String &schema, const std::vector<String> &partition_columns,
...
void generateManifestFile(Poco::JSON::Object::Ptr metadata, const std::vector<String> &partition_columns,
                          const std::vector<Field> &partition_values, const std::vector<DataTypePtr> &partition_types,
                          const std::vector<String> &data_file_names, const std::optional<DataFileStatistics> &data_file_statistics,
                          SharedHeader sample_block, Poco::JSON::Object::Ptr new_snapshot, const String &format,
                          Poco::JSON::Object::Ptr partition_spec, Int64 partition_spec_id, WriteBuffer &buf,
  ...
  if (root_schema->type() != avro::AVRO_RECORD) throw Exception(ErrorCodes::LOGICAL_ERROR, "Iceberg manifest file schema must be record");
  ...
  for (const auto &data_file_name : data_file_names) {
    ...
    if (version > 1) data_file.field(Iceberg::f_content) = avro::GenericDatum(static_cast<Int32>(content_type));
    ...
    if (data_file_statistics) {
      auto set_fields = [&]<typename T, typename U>(const std::vector<std::pair<size_t, T>> &statistics, const std::string &field_name,
                                                    U &&dump_function) {
      ...
      for (size_t i = 0; i < field_ids.size(); ++i) field_id_to_column_index[field_ids[i]] = i;
    ...
    for (size_t i = 0; i < partition_columns.size(); ++i) {
      switch (partition_values[i].getType()) {
        ...
        default:
          throw Exception(ErrorCodes::BAD_ARGUMENTS, "Unsupported type to write into avro file {}", partition_values[i].getType());
      }
    }
...
void generateManifestList(const FileNamesGenerator &filename_generator, Poco::JSON::Object::Ptr metadata, ObjectStoragePtr object_storage,
                          ContextPtr context, const Strings &manifest_entry_names, Poco::JSON::Object::Ptr new_snapshot,
                          Int64 manifest_length, WriteBuffer &buf, Iceberg::FileContentType content_type, bool use_previous_snapshots) {
  Int32 version = metadata->getValue<Int32>(Iceberg::f_format_version);
  String schema_representation;
  if (version == 1)
  ...
  for (const auto &manifest_entry_name : manifest_entry_names) {
    ...
    if (version == 1) {
      ...
      entry.field(Iceberg::f_existing_files_count) = summary->getValue<Int32>(Iceberg::f_total_data_files);
    ...
    if (summary->has(Iceberg::f_added_records)) {
      set_versioned_field(summary->getValue<Int32>(Iceberg::f_added_records), Iceberg::f_added_rows_count);
    } else {
        ...
        while (reader.read(datum)) {
          if (version == 1) {
            ...
            if (old_entry.hasField(Iceberg::f_added_snapshot_id)) {
              ...
              if (old_added_snapshot_id_entry.isUnion()) {
                if (old_added_snapshot_id_entry.unionBranch() == 0)  /// it means add_snapshot_id is null
                {
                  ...
                  throw Exception(ErrorCodes::ICEBERG_SPECIFICATION_VIOLATION,
                                  ...
                                  relative_path_with_metadata.getPath(), Iceberg::f_added_snapshot_id);
                }
              }
              ...
              /// This only happens when we read data written by a old version of iceberg, which violent the spec of iceberg.
              throw Exception(ErrorCodes::ICEBERG_SPECIFICATION_VIOLATION,
                              "Manifest list {} has null value for field '{}', but it is required", relative_path_with_metadata.getPath(),
                              Iceberg::f_added_snapshot_id);
            auto add_field_to_datum = [&](const String &field) {
              if (old_entry.hasField(field)) new_entry.field(field) = old_entry.field(field);
            };
...
IcebergStorageSink::IcebergStorageSink(ObjectStoragePtr object_storage_, StorageObjectStorageConfigurationPtr configuration_,
                                       const std::optional<FormatSettings> &format_settings_, SharedHeader sample_block_,
                                       ContextPtr context_, std::shared_ptr<DataLake::ICatalog> catalog_,
                                       const Iceberg::PersistentTableComponents &persistent_table_components_, const StorageID &table_id_)
  ...
  auto [last_version, metadata_path, compression_method] = getLatestOrExplicitMetadataFileAndVersion(
      object_storage, persistent_table_components.table_path, data_lake_settings, persistent_table_components.metadata_cache, context_,
      log.get(), persistent_table_components.table_uuid);

  metadata = getMetadataJSONObject(metadata_path, object_storage, persistent_table_components.metadata_cache, context, log,
  ...
  if (config_path.empty() || config_path.back() != '/') config_path += "/";
  ...
  if (!context_->getSettingsRef()[Setting::write_full_path_in_iceberg_metadata]) {
    filename_generator = FileNamesGenerator(config_path, config_path, (catalog != nullptr && catalog->isTransactional()),
                                            metadata_compression_method, write_format);
  } else {
    auto bucket = metadata->getValue<String>(Iceberg::f_location);
    if (bucket.empty() || bucket.back() != '/') bucket += "/";
  ...
  for (size_t i = 0; i < partitions_specs->size(); ++i) {
    ...
    if (current_partition_spec->getValue<Int64>(Iceberg::f_spec_id) == partition_spec_id) {
      ...
      if (!sort_description.column_names.empty()) sortBlockByKeyDescription(extended_block_for_sorting, sort_description, context);

      if (current_partition_spec->getArray(Iceberg::f_fields)->size() > 0)
        partitioner = ChunkPartitioner(current_partition_spec->getArray(Iceberg::f_fields), current_schema, context_,
...
void IcebergStorageSink::consume(Chunk &chunk) {
  ...
  if (!sort_description.column_names.empty()) {
    ...
    for (size_t i = 0; i < block.columns(); ++i) column_name_to_column_index[block.getNames()[i]] = i;
  ...
  for (const auto &[partition_key, part_chunk] : partition_result) {
    if (!writer_per_partition_key.contains(partition_key)) {
      auto writer = MultipleFileWriter(context->getSettingsRef()[Setting::iceberg_insert_max_rows_in_data_file],
                                       context->getSettingsRef()[Setting::iceberg_insert_max_bytes_in_data_file],
                                       current_schema->getArray(Iceberg::f_fields), filename_generator, object_storage, context,
                                       format_settings, write_format, sample_block);
    ...
    if (!sort_description.column_names.empty() && part_chunk.hasRows() && last_fields_of_last_chunks.contains(partition_key)) {
      ...
      if (!last_fields.empty()) {
        ...
        if (should_create_new_file) writer_per_partition_key.at(partition_key).startNewFile();
      }
...
void IcebergStorageSink::onException(std::exception_ptr /* exception */) { cancelBuffers(); }
...
void IcebergStorageSink::finalizeBuffers() {
  ...
  if (writer_per_partition_key.empty()) return;

  size_t i = 0;
  while (i < MAX_TRANSACTION_RETRIES) {
    if (initializeMetadata()) break;
    ++i;
  }
}
...
bool IcebergStorageSink::initializeMetadata() {
  ...
  for (const auto &[_, writer] : writer_per_partition_key) total_data_files += writer.getDataFiles().size();
  auto [new_snapshot, manifest_list_name, storage_manifest_list_name] = MetadataGenerator(metadata).generateNextMetadata(
      filename_generator, metadata_name, parent_snapshot, total_data_files, total_rows, total_chunks_size, total_data_files,
      /* added_delete_files */ 0, /* num_deleted_rows */ 0);
  ...
  auto cleanup = [&](bool retry_because_of_metadata_conflict) {
    if (!retry_because_of_metadata_conflict) {
      for (const auto &[_, writer] : writer_per_partition_key) writer.clearAllDataFiles();
    }
    ...
    if (retry_because_of_metadata_conflict) {
      auto [last_version, metadata_path, compression_method] = getLatestOrExplicitMetadataFileAndVersion(
          ...
          getLogger("IcebergWrites").get(), persistent_table_components.table_uuid);
      ...
      metadata = getMetadataJSONObject(metadata_path, object_storage, persistent_table_components.metadata_cache, context,
                                       getLogger("IcebergWrites"), compression_method, persistent_table_components.table_uuid);
      partition_spec_id = metadata->getValue<Int64>(Iceberg::f_default_spec_id);
      auto partitions_specs = metadata->getArray(Iceberg::f_partition_specs);
  ...
  try {
    for (const auto &[partition_key, writer] : writer_per_partition_key) {
      ...
      try {
        generateManifestFile(metadata, partitioner ? partitioner->getColumns() : std::vector<String>{}, partition_key,
                             partitioner ? partitioner->getResultTypes() : std::vector<DataTypePtr>{}, writer.getDataFiles(),
                             writer.getResultStatistics(), sample_block, new_snapshot, write_format, partititon_spec, partition_spec_id,
                             *buffer_manifest_entry, Iceberg::FileContentType::DATA);
    ...
    }
    {
      ...
      try {
        generateManifestList(filename_generator, metadata, object_storage, context, manifest_entries, new_snapshot, manifest_lengths,
                             *buffer_manifest_list, Iceberg::FileContentType::DATA);
...

    {
      ...
      if (!writeMetadataFileAndVersionHint(storage_metadata_name, json_representation, hint.path_in_storage, storage_metadata_name,
                                           object_storage, context, metadata_compression_method,
