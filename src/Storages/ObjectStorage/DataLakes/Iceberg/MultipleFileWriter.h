...
class MultipleFileWriter {
 public:
  explicit MultipleFileWriter(UInt64 max_data_file_num_rows_, UInt64 max_data_file_num_bytes_, Poco::JSON::Array::Ptr schema,
  ...
  const std::vector<String>& getDataFiles() const { return data_file_names; }

  const DataFileStatistics& getResultStatistics() const { return stats; }
