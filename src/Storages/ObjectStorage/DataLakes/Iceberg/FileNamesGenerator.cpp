...
FileNamesGenerator::FileNamesGenerator(const String& table_dir_, const String& storage_dir_, bool use_uuid_in_metadata_,
                                       CompressionMethod compression_method_, const String& format_name_)
...
FileNamesGenerator::Result FileNamesGenerator::generateDataFileName() {
  ...
  return Result{.path_in_metadata = fmt::format("{}data-{}.{}", data_dir, uuid_str, format_name),
...
FileNamesGenerator::Result FileNamesGenerator::generatePositionDeleteFile() {
  ...
  return Result{.path_in_metadata = fmt::format("{}{}-deletes.{}", data_dir, uuid_str, format_name),
...
String FileNamesGenerator::convertMetadataPathToStoragePath(const String& metadata_path) const {
  if (!metadata_path.starts_with(table_dir))
    throw Exception(ErrorCodes::BAD_ARGUMENTS,
                    "Paths in Iceberg must use a consistent format — either /your/path or s3://your/path. Use the "
                    "write_full_path_in_iceberg_metadata setting to control this behavior {} {}",
