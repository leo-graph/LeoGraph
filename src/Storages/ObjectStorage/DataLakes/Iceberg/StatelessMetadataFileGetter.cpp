...
namespace Iceberg {
Iceberg::ManifestFileCacheableInfo getManifestFile(ObjectStoragePtr object_storage,
                                                   const PersistentTableComponents& persistent_table_components, ContextPtr local_context,
                                                   LoggerPtr log, const String& filename, size_t bytes_size) {
  ...
  auto create_fn = [&, use_iceberg_metadata_cache]() {
    ...
    if (use_iceberg_metadata_cache) read_settings.enable_filesystem_cache = false;

    auto buffer = createReadBuffer(manifest_object_info, object_storage, local_context, log, read_settings);
    auto manifest_file_deserializer =
        std::make_unique<Iceberg::AvroForIcebergDeserializer>(std::move(buffer), filename, getFormatSettings(local_context));
...
Iceberg::ManifestFileIterator::ManifestFileEntriesHandle getManifestFileEntriesHandle(
    ObjectStoragePtr object_storage, const PersistentTableComponents& persistent_table_components, ContextPtr local_context, LoggerPtr log,
    const ManifestFileCacheKey& cache_key, Int32 table_snapshot_schema_id) {
  auto cacheable_info = getManifestFile(object_storage, persistent_table_components, local_context, log, cache_key.manifest_file_path,
                                        static_cast<size_t>(cache_key.manifest_file_byte_size));

  auto iterator = Iceberg::ManifestFileIterator::create(cacheable_info.deserializer, cache_key.manifest_file_path,
                                                        persistent_table_components.format_version, persistent_table_components.table_path,
                                                        *persistent_table_components.schema_processor, cache_key.added_sequence_number,
                                                        cache_key.added_snapshot_id, persistent_table_components.table_location,
                                                        local_context, cache_key.manifest_file_path, nullptr, table_snapshot_schema_id);
...
ManifestFileCacheKeys getManifestList(ObjectStoragePtr object_storage, const PersistentTableComponents& persistent_table_components,
                                      ContextPtr local_context, const String& filename, LoggerPtr log) {
  ...
  auto create_fn = [&, use_iceberg_metadata_cache]() {
    ...
    insertRowToLogTable(local_context, manifest_list_deserializer.getMetadataContent(), DB::IcebergMetadataLogLevel::ManifestListMetadata,
                        persistent_table_components.table_path, filename, std::nullopt, std::nullopt);
    ...
    for (size_t i = 0; i < manifest_list_deserializer.rows(); ++i) {
      ...
      const auto manifest_file_name =
          getProperFilePathFromMetadataInfo(file_path, persistent_table_components.table_path, persistent_table_components.table_location);
      ...
      if (added_snapshot_id.isNull())
        throw Exception(ErrorCodes::ICEBERG_SPECIFICATION_VIOLATION,
                        "Manifest list entry at index {} has null value for field '{}', but it is required", i, f_added_snapshot_id);

      ManifestFileContentType content_type = ManifestFileContentType::DATA;
      Int64 manifest_length = manifest_list_deserializer.getValueFromRowByName(i, f_manifest_length, TypeIndex::Int64).safeGet<Int64>();
      if (manifest_length < 0) {
        throw Exception(ErrorCodes::ICEBERG_SPECIFICATION_VIOLATION,
                        "Manifest list entry at index {} has negative value for field '{}', but it is required", i, f_manifest_length);
      }
      ...
      manifest_file_cache_keys.emplace_back(manifest_file_name, manifest_length, added_sequence_number, added_snapshot_id.safeGet<Int64>(),
                                            content_type);

      insertRowToLogTable(local_context, manifest_list_deserializer.getContent(i), DB::IcebergMetadataLogLevel::ManifestListEntry,
                          persistent_table_components.table_path, filename, i, std::nullopt);
    }
