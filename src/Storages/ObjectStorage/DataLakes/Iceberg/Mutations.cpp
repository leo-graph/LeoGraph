...
using DataFileWriteResultByPartitionKey =
    std::unordered_map<ChunkPartitioner::PartitionKey, DeleteFileWriteResult, ChunkPartitioner::PartitionKeyHasher>;
using DataFileStatisticsByPartitionKey =
    std::unordered_map<ChunkPartitioner::PartitionKey, DataFileStatistics, ChunkPartitioner::PartitionKeyHasher>;
...
static Block getNonVirtualColumns(const Block& block) {
  ...
  for (const auto& column_desc : virtual_columns_desc) virtual_columns.insert(column_desc);
  ColumnsWithTypeAndName columns;
  for (size_t i = 0; i < block.getNames().size(); ++i) {
    if (virtual_columns.contains(block.getNames()[i])) continue;
    columns.push_back(ColumnWithTypeAndName(block.getColumns()[i], block.getDataTypes()[i], block.getNames()[i]));
  }
  return Block(columns);
}

static std::vector<std::pair<ChunkPartitioner::PartitionKey, Chunk>> getPartitionedChunks(
    const Chunk& chunk, std::optional<ChunkPartitioner>& chunk_partitioner) {
  if (chunk_partitioner.has_value()) return chunk_partitioner->partitionChunk(chunk);
...
static std::optional<WriteDataFilesResult> writeDataFiles(const MutationCommands& commands, ContextPtr context, StorageMetadataPtr metadata,
                                                          StorageID storage_id, ObjectStoragePtr object_storage, String write_format,
                                                          FileNamesGenerator& generator,
                                                          const std::optional<FormatSettings>& format_settings,
                                                          std::optional<ChunkPartitioner>& chunk_partitioner,
                                                          Poco::JSON::Object::Ptr data_schema, const String& blob_storage_namespace_name) {
  ...
  std::unordered_map<ChunkPartitioner::PartitionKey, std::unique_ptr<WriteBuffer>, ChunkPartitioner::PartitionKeyHasher>
      delete_data_write_buffers;
  ...
  if (commands[0].type == MutationCommand::UPDATE || commands[0].type == MutationCommand::DELETE) {
    ...
    while (executor.pull(block)) {
      ...
      for (const auto& [partition_key, partition_chunk] : partition_result) {
        ...
        if (!delete_data_writers.contains(partition_key)) {
          ...
          auto write_buffer = object_storage->writeObject(StoredObject(delete_file_info.path_in_storage), WriteMode::Rewrite, std::nullopt,
          ...
          field_ids[IcebergPositionDeleteTransform::data_file_path_column_name] =
          ...
          auto output_format = FormatFactory::instance().getOutputFormat(write_format, *write_buffer, delete_file_sample_block, context,
                                                                         format_settings, format_filter_info);
        ...
        for (size_t i = 0; i < col_data_filename.column->size(); ++i) {
          ...
          if (!path_without_namespace.starts_with('/')) path_without_namespace = "/" + path_without_namespace;
    ...
    if (!has_any_rows) return std::nullopt;
    ...
    while (executor.pull(block)) {
      ...
      for (const auto& [partition_key, partition_chunk] : partition_result) {
        ...
        if (it == update_data_writers.end()) {
          ...
          auto data_write_buffer = object_storage->writeObject(StoredObject(data_file_info.path_in_storage), WriteMode::Rewrite,
                                                               std::nullopt, DBMS_DEFAULT_BUFFER_SIZE, context->getWriteSettings());

          auto data_output_format =
              FormatFactory::instance().getOutputFormat(write_format, *data_write_buffer, data_block, context, format_settings, nullptr);
  ...
  if (commands[0].type == MutationCommand::DELETE)
    ...
    return WriteDataFilesResult{DataFileWriteResultWithStats{delete_data_result, delete_data_statistics},
                                DataFileWriteResultWithStats{update_data_result, update_data_statistics}};
  else
    return {};
}

static bool writeMetadataFiles(DataFileWriteResultWithStats& delete_filenames, ObjectStoragePtr object_storage, ContextPtr context,
                               FileNamesGenerator& filename_generator, const DataLakeStorageSettings& data_lake_settings,
                               String write_format, std::shared_ptr<DataLake::ICatalog> catalog, StorageID table_id,
                               Poco::JSON::Object::Ptr metadata, Poco::JSON::Object::Ptr partititon_spec, Int32 partition_spec_id,
                               std::optional<ChunkPartitioner>& chunk_partitioner, Iceberg::FileContentType content_type,
                               SharedHeader sample_block, CompressionMethod compression_method, bool write_metadata_json_file,
                               const String& blob_storage_type_name, const String& blob_storage_namespace_name) {
  auto [metadata_name, storage_metadata_name] = filename_generator.generateMetadataName();
  Int64 parent_snapshot = -1;
  if (metadata->has(Iceberg::f_current_snapshot_id)) parent_snapshot = metadata->getValue<Int64>(Iceberg::f_current_snapshot_id);
  ...
  if (content_type == Iceberg::FileContentType::POSITION_DELETE) {
    auto result_generation_metadata = MetadataGenerator(metadata).generateNextMetadata(filename_generator, metadata_name, parent_snapshot,
                                                                                       /* added_files */ 0,
                                                                                       /* added_records */ 0, total_bytes,
                                                                                       /* num_partitions */ total_files,
                                                                                       /* added_delete_files */ total_files, total_rows);
    ...
    auto result_generation_metadata = MetadataGenerator(metadata).generateNextMetadata(filename_generator, metadata_name, parent_snapshot,
                                                                                       ...
                                                                                       /* added_records */ total_rows, total_bytes,
                                                                                       /* num_partitions */ total_files,
  ...
  try {
    for (const auto& [partition_key, delete_filename] : delete_filenames.delete_file) {
      ...
      auto buffer_manifest_entry = object_storage->writeObject(StoredObject(storage_manifest_entry_name), WriteMode::Rewrite, std::nullopt,
                                                               DBMS_DEFAULT_BUFFER_SIZE, context->getWriteSettings());
      try {
        generateManifestFile(metadata, chunk_partitioner ? chunk_partitioner->getColumns() : std::vector<String>{}, partition_key,
                             chunk_partitioner ? chunk_partitioner->getResultTypes() : std::vector<DataTypePtr>{},
                             {delete_filename.path.path_in_metadata}, delete_filenames.delete_statistic.at(partition_key), sample_block,
                             new_snapshot, write_format, partititon_spec, partition_spec_id, *buffer_manifest_entry, content_type);
    ...
    {
      auto buffer_manifest_list = object_storage->writeObject(StoredObject(storage_manifest_list_name), WriteMode::Rewrite, std::nullopt,
                                                              DBMS_DEFAULT_BUFFER_SIZE, context->getWriteSettings());

      try {
        generateManifestList(filename_generator, metadata, object_storage, context, manifest_entries, new_snapshot, manifest_lengths,
                             *buffer_manifest_list, content_type);
    ...
    if (write_metadata_json_file) {
      ...
      if (!writeMetadataFileAndVersionHint(storage_metadata_name, json_representation, hint.path_in_storage, storage_metadata_name,
                                           object_storage, context, compression_method,
...
void mutate(const MutationCommands& commands, ContextPtr context, StorageMetadataPtr storage_metadata, StorageID storage_id,
            ObjectStoragePtr object_storage, const DataLakeStorageSettings& data_lake_settings,
            PersistentTableComponents& persistent_table_components, const String& write_format,
            const std::optional<FormatSettings>& format_settings, std::shared_ptr<DataLake::ICatalog> catalog,
            const String& blob_storage_type_name, const String& blob_storage_namespace_name) {
  auto common_path = persistent_table_components.table_path;
  if (!common_path.starts_with('/')) common_path = "/" + common_path;
  ...
  while (--max_retries > 0) {
    ...
    auto [last_version, metadata_path, compression_method] = getLatestOrExplicitMetadataFileAndVersion(
        object_storage, persistent_table_components.table_path, data_lake_settings, persistent_table_components.metadata_cache, context,
        log.get(), persistent_table_components.table_uuid);

    filename_generator.setVersion(last_version + 1);
    filename_generator.setCompressionMethod(compression_method);

    auto metadata = getMetadataJSONObject(metadata_path, object_storage, persistent_table_components.metadata_cache, context, log,
                                          compression_method, persistent_table_components.table_uuid);
    ...
    auto mutation_files = writeDataFiles(commands, context, storage_metadata, storage_id, object_storage, write_format, filename_generator,
                                         format_settings, chunk_partitioner, current_schema, blob_storage_namespace_name);

    if (mutation_files) {
      auto result_delete_files_metadata =
          writeMetadataFiles(mutation_files->delete_file, object_storage, context, filename_generator, data_lake_settings, write_format,
                             catalog, storage_id, metadata, partititon_spec, static_cast<Int32>(partition_spec_id), chunk_partitioner,
                             Iceberg::FileContentType::POSITION_DELETE, std::make_shared<const Block>(getPositionDeleteFileSampleBlock()),
                             compression_method, !mutation_files->data_file, blob_storage_type_name, blob_storage_namespace_name);
      if (!result_delete_files_metadata) continue;

      if (mutation_files->data_file) {
        auto result_data_files_metadata = writeMetadataFiles(
            *mutation_files->data_file, object_storage, context, filename_generator, data_lake_settings, write_format, catalog, storage_id,
            metadata, partititon_spec, static_cast<Int32>(partition_spec_id), chunk_partitioner, Iceberg::FileContentType::DATA,
            sample_block, compression_method, true, blob_storage_type_name, blob_storage_namespace_name);
  ...
  if (max_retries == 0) throw Exception(ErrorCodes::LIMIT_EXCEEDED, "Too many unsuccessed retries to create iceberg snapshot");
}

void alter(const AlterCommands& params, ContextPtr context, ObjectStoragePtr object_storage,
           const DataLakeStorageSettings& data_lake_settings, PersistentTableComponents& persistent_table_components,
           const String& write_format) {
  if (params.size() != 1) throw Exception(ErrorCodes::BAD_ARGUMENTS, "Params with size 1 is not supported");

  size_t i = 0;
  while (i++ < MAX_TRANSACTION_RETRIES) {
    FileNamesGenerator filename_generator(persistent_table_components.table_path, persistent_table_components.table_path, false,
                                          CompressionMethod::None, write_format);
    auto log = getLogger("IcebergMutations");
    auto [last_version, metadata_path, compression_method] = getLatestOrExplicitMetadataFileAndVersion(
        object_storage, persistent_table_components.table_path, data_lake_settings, persistent_table_components.metadata_cache, context,
        log.get(), persistent_table_components.table_uuid);
  ...
  if (i == MAX_TRANSACTION_RETRIES) throw Exception(ErrorCodes::LIMIT_EXCEEDED, "Too many unsuccessed retries to alter iceberg table");
}
...
static RetentionPolicy readRetentionPolicy(const Poco::JSON::Object::Ptr& metadata) {
  ...
  if (!metadata->has(Iceberg::f_properties)) return policy;
  ...
  if (props->has(Iceberg::f_max_ref_age_ms)) policy.max_ref_age_ms = std::stoll(props->getValue<String>(Iceberg::f_max_ref_age_ms));
  return policy;
}
 ...
 public:
  ...
  void walkBranchAncestors(Int64 now_ms, Int64 head_id, Int32 min_keep, Int64 max_age_ms, std::set<Int64>& retained) const {
    ...
    while (hasSnapshot(walk_id)) {
      ...
      if (!within_min_keep && !within_max_age) break;
...
static std::pair<std::set<Int64>, Strings> applyRetentionPolicy(const Poco::JSON::Object::Ptr& metadata, Int64 current_snapshot_id,
                                                                const SnapshotGraph& graph, const RetentionPolicy& policy, Int64 now_ms) {
  ...
  if (metadata->has(Iceberg::f_refs)) {
    ...
    for (const auto& ref_name : refs->getNames()) {
      ...
      Int64 ref_max_ref_age =
          ref_obj->has(Iceberg::f_ref_max_ref_age_ms) ? ref_obj->getValue<Int64>(Iceberg::f_ref_max_ref_age_ms) : policy.max_ref_age_ms;

      bool is_main = (ref_name == Iceberg::f_main);

      if (!is_main && !graph.hasSnapshot(ref_snap_id)) {
        LOG_WARNING(getLogger("IcebergExpireSnapshots"), "Removing invalid ref {}: snapshot {} does not exist", ref_name, ref_snap_id);
      ...
      if (ref_type == Iceberg::f_branch) {
        Int32 min_keep = ref_obj->has(Iceberg::f_ref_min_snapshots_to_keep) ? ref_obj->getValue<Int32>(Iceberg::f_ref_min_snapshots_to_keep)
                                                                            : policy.min_snapshots_to_keep;
        Int64 max_age = ref_obj->has(Iceberg::f_ref_max_snapshot_age_ms) ? ref_obj->getValue<Int64>(Iceberg::f_ref_max_snapshot_age_ms)
                                                                         : policy.max_snapshot_age_ms;
        graph.walkBranchAncestors(now_ms, ref_snap_id, min_keep, max_age, retained);
        if (is_main) main_branch_walked = true;
...
static void collectAllFilePaths(const Iceberg::ManifestFileIterator::ManifestFileEntriesHandle& entries_handle, std::set<String>& out) {
  for (const auto& entry : entries_handle.getFilesWithoutDeleted(FileContentType::DATA)) out.insert(entry->file_path);
  for (const auto& entry : entries_handle.getFilesWithoutDeleted(FileContentType::POSITION_DELETE)) out.insert(entry->file_path);
  for (const auto& entry : entries_handle.getFilesWithoutDeleted(FileContentType::EQUALITY_DELETE)) out.insert(entry->file_path);
}
...
/// DELETED entries.
static void collectRetainedFiles(const Poco::JSON::Array::Ptr& retained_snapshots, ObjectStoragePtr object_storage,
                                 PersistentTableComponents& persistent_table_components, ContextPtr context, LoggerPtr log,
                                 Int32 current_schema_id, std::set<String>& retained_manifest_paths,
                                 std::set<String>& retained_data_file_paths, std::set<String>& retained_manifest_list_paths) {
  for (UInt32 i = 0; i < retained_snapshots->size(); ++i) {
    auto snapshot = retained_snapshots->getObject(i);
    if (!snapshot->has(Iceberg::f_manifest_list)) continue;
    ...
    String storage_manifest_list_path = getProperFilePathFromMetadataInfo(manifest_list_path, persistent_table_components.table_path,
                                                                          persistent_table_components.table_location);

    auto manifest_keys = getManifestList(object_storage, persistent_table_components, context, storage_manifest_list_path, log);

    for (const auto& mf_key : manifest_keys) {
      retained_manifest_paths.insert(mf_key.manifest_file_path);
      auto entries_handle =
          getManifestFileEntriesHandle(object_storage, persistent_table_components, context, log, mf_key, current_schema_id);
...
static ExpiredFiles collectExpiredFiles(const std::vector<String>& expired_manifest_list_paths,
                                        const std::set<String>& retained_manifest_list_paths,
                                        const std::set<String>& retained_manifest_paths, const std::set<String>& retained_data_file_paths,
                                        ObjectStoragePtr object_storage, PersistentTableComponents& persistent_table_components,
                                        ContextPtr context, LoggerPtr log, Int32 current_schema_id) {
  ExpiredFiles result;
  for (const auto& ml_path : expired_manifest_list_paths) {
    if (retained_manifest_list_paths.contains(ml_path)) continue;

    String storage_ml_path =
        getProperFilePathFromMetadataInfo(ml_path, persistent_table_components.table_path, persistent_table_components.table_location);

    ManifestFileCacheKeys manifest_keys;
    try {
      manifest_keys = getManifestList(object_storage, persistent_table_components, context, storage_ml_path, log);
    } catch (...) {
    ...
    for (const auto& mf_key : manifest_keys) {
      if (retained_manifest_paths.contains(mf_key.manifest_file_path)) continue;
...
static void trimSnapshotLog(Poco::JSON::Object::Ptr metadata, const std::set<Int64>& expired_snapshot_ids) {
  if (!metadata->has(Iceberg::f_snapshot_log)) return;
  ...
  for (Int32 j = static_cast<Int32>(snapshot_log->size()) - 1; j >= 0; --j) {
    ...
    if (expired_snapshot_ids.contains(snap_id)) break;
    suffix_start = j;
  }
  Poco::JSON::Array::Ptr retained_log = new Poco::JSON::Array;
  for (UInt32 j = static_cast<UInt32>(suffix_start); j < snapshot_log->size(); ++j) retained_log->add(snapshot_log->getObject(j));
...
static SnapshotPartition partitionSnapshots(const Poco::JSON::Array::Ptr& snapshots, const std::set<Int64>& retention_retained_ids,
...
static void updateMetadataForExpiration(Poco::JSON::Object::Ptr metadata, const Strings& expired_ref_names,
                                        const Poco::JSON::Array::Ptr& retained_snapshots, const std::set<Int64>& expired_snapshot_ids) {
  for (const auto& ref_name : expired_ref_names) metadata->getObject(Iceberg::f_refs)->remove(ref_name);
...
static void deleteExpiredFiles(const Strings& files_to_delete, ObjectStoragePtr object_storage, LoggerPtr log) {
...
ExpireSnapshotsResult expireSnapshots(std::optional<Int64> expire_before_ms, ContextPtr context, ObjectStoragePtr object_storage,
                                      const DataLakeStorageSettings& data_lake_settings,
                                      PersistentTableComponents& persistent_table_components, const String& write_format,
                                      std::shared_ptr<DataLake::ICatalog> catalog, const String& blob_storage_type_name,
                                      const String& blob_storage_namespace_name, const String& table_name) {
  auto common_path = persistent_table_components.table_path;
  if (!common_path.starts_with('/')) common_path = "/" + common_path;
  ...
  while (--max_retries > 0) {
    ...
    collectRetainedFiles(partition.retained_snapshots, object_storage, persistent_table_components, context, log, current_schema_id,
                         retained_manifest_paths, retained_data_file_paths, retained_manifest_list_paths);
    auto expired_files =
        collectExpiredFiles(partition.expired_manifest_list_paths, retained_manifest_list_paths, retained_manifest_paths,
                            retained_data_file_paths, object_storage, persistent_table_components, context, log, current_schema_id);
    ...
    LOG_INFO(log, "Deleting {} expired files for {} expired snapshots", expired_files.all_paths.size(),
  ...
  if (max_retries == 0) throw Exception(ErrorCodes::LIMIT_EXCEEDED, "Too many unsuccessful retries to expire iceberg snapshots");
