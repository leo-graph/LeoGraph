...
class DeletingTransform : public IIcebergSchemaTransform {
 public:
  explicit DeletingTransform(const IcebergDeletingOperation& operation_, DataTypePtr old_type, const std::vector<size_t>& permutation)
    ...
    for (const auto& path : operation_.getPath()) {
      switch (path.parent_type) {
        case TransformType::STRUCT: {
          ...
          if (result_index == current_types.size()) throw Exception(ErrorCodes::BAD_ARGUMENTS, "Can not find path {} in data", path.path);
        ...
        case TransformType::MAP: {
          index_path.push_back(-1);
          old_type =
              std::static_pointer_cast<const DataTypeTuple>(
                  std::static_pointer_cast<const DataTypeArray>(std::static_pointer_cast<const DataTypeMap>(old_type)->getNestedType())
                      ->getNestedType())
                  ->getElement(1);
          break;
        }
    ...
    for (size_t i = 0; i < current_types.size(); ++i) {
      if (element_names[permutation[i]] == operation.field_name) index_to_delete = i;
    }
...
class AddingTransform : public IIcebergSchemaTransform {
 public:
  explicit AddingTransform(const IcebergAddingOperation& operation_, DataTypePtr type, DataTypePtr old_type)
      ...
      switch (path.parent_type) {
        ...
        case TransformType::MAP: {
          ...
          type = std::static_pointer_cast<const DataTypeTuple>(
                     std::static_pointer_cast<const DataTypeArray>(std::static_pointer_cast<const DataTypeMap>(type)->getNestedType())
                         ->getNestedType())
                     ->getElement(1);
          break;
        }
    ...
    for (size_t i = 0; i < current_types.size(); ++i) {
      if (element_names[i] == operation.field_name) index_to_insert = i;
    }
...
class ReorderingTransform : public IIcebergSchemaTransform {
 public:
  ...
  void transformChildNode(Tuple& initial_node) override {
    Tuple permuted_node;
    for (auto index : operation.permutation) permuted_node.push_back(initial_node[index]);
...
ColumnPtr ExecutableEvolutionFunction::executeImpl(const ColumnsWithTypeAndName& arguments, const DataTypePtr& /*result_type*/,
  ...
  for (size_t i = 0; i < input_rows_count; ++i) {
    ...
    if (column_field.tryGet(current_tuple_node)) {
      ...
      for (const auto& transform : transforms) transform->transform(tuple_node);
...
void ExecutableEvolutionFunction::fillMissingElementsInPermutation(std::vector<size_t>& permutation, size_t target_size) {
  ...
  for (auto elem : permutation) existing_elements.insert(elem);

  for (size_t i = 0; i < target_size; ++i) {
    if (!existing_elements.contains(i)) permutation.push_back(i);
  }
}
...
void ExecutableEvolutionFunction::pushNewNode(std::stack<ExecutableEvolutionFunction::TraverseItem>& walk_stack,
                                              Poco::JSON::Object::Ptr old_node, Poco::JSON::Object::Ptr new_node,
                                              std::vector<IcebergChangeSchemaOperation::Edge>* current_path) {
  if (!old_node->isObject("type")) return;
  ...
  if (old_node->getObject("type")->getValue<std::string>("type") == "struct") {
    ...
    if (current_path)
      current_path->push_back({old_node->getValue<std::string>("name"), new_node->getValue<std::string>("name"), TransformType::STRUCT});

    walk_stack.push(TraverseItem{old_subfields, subfields, current_path ? *current_path : std::vector<IcebergChangeSchemaOperation::Edge>{},
                                 TransformType::STRUCT});
    if (current_path) current_path->pop_back();
    ...
    if (current_path) current_path->push_back({"", "", TransformType::ARRAY});

    walk_stack.push({makeArrayFromObject(old_subfields), makeArrayFromObject(subfields),
                     current_path ? *current_path : std::vector<IcebergChangeSchemaOperation::Edge>{}, TransformType::ARRAY});
    if (current_path) current_path->pop_back();
    ...
    if (current_path) current_path->push_back({"", "", TransformType::MAP});

    walk_stack.push({makeArrayFromObject(old_subfields), makeArrayFromObject(subfields),
                     current_path ? *current_path : std::vector<IcebergChangeSchemaOperation::Edge>{}, TransformType::MAP});
    if (current_path) current_path->pop_back();
  }
}

std::shared_ptr<ReorderingTransform> makeReorderingTransform(const std::vector<IcebergChangeSchemaOperation::Edge>& current_path,
                                                             const std::vector<size_t>& initial_permutation, DataTypePtr old_type) {
...
std::shared_ptr<DeletingTransform> makeDeletingTransform(const std::vector<IcebergChangeSchemaOperation::Edge>& current_path,
                                                         const std::vector<size_t>& initial_permutation, DataTypePtr old_type,
...
std::shared_ptr<AddingTransform> makeAddingTransform(const std::vector<IcebergChangeSchemaOperation::Edge>& current_path, DataTypePtr type,
                                                     DataTypePtr old_type, Poco::JSON::Object::Ptr subfield) {
...
void ExecutableEvolutionFunction::lazyInitialize() const {
  ...
  while (!walk_stack.empty()) {
    ...
    if (transform_type != TransformType::STRUCT) {
      ...
      if (old_subfield->has("type") && old_subfield->getValue<std::string>("type") == "struct") {
        walk_stack.push({old_subfield->get("fields").extract<Poco::JSON::Array::Ptr>(),
                         subfield->get("fields").extract<Poco::JSON::Array::Ptr>(), current_path, TransformType::STRUCT});
      } else if (old_subfield->has("type") && old_subfield->getValue<std::string>("type") == "list" && old_subfield->isObject("element")) {
        walk_stack.push({makeArrayFromObject(old_subfield->getObject("element")), makeArrayFromObject(subfield->getObject("element")),
                         current_path, TransformType::ARRAY});
      } else if (old_subfield->has("type") && old_subfield->getValue<std::string>("type") == "map" && old_subfield->isObject("value")) {
        walk_stack.push({makeArrayFromObject(old_subfield->getObject("value")), makeArrayFromObject(subfield->getObject("value")),
                         current_path, TransformType::MAP});
      }
    ...
    std::sort(delete_transforms.begin(), delete_transforms.end(), [](const auto& lhs, const auto& rhs) { return lhs.first > rhs.first; });
    for (const auto& [_, transform] : delete_transforms) per_layer_transforms[current_path.size()].push_back(transform);
  ...
  for (auto it = per_layer_transforms.rbegin(); it != per_layer_transforms.rend(); ++it)
    for (const auto& transform : it->second) transforms.push_back(transform);
}
