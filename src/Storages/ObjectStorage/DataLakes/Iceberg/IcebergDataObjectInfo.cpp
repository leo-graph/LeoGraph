...
IcebergDataObjectInfo::IcebergDataObjectInfo(Iceberg::ProcessedManifestFileEntryPtr data_manifest_file_entry_,
                                             Int32 schema_id_relevant_to_iterator_)
    : ObjectInfo(RelativePathWithMetadata(data_manifest_file_entry_->file_path)),
      info{data_manifest_file_entry_->parsed_entry->file_path_key,
...
IcebergDataObjectInfo::IcebergDataObjectInfo(const RelativePathWithMetadata& path_) : ObjectInfo(path_) {}

std::shared_ptr<ISimpleTransform> IcebergDataObjectInfo::getPositionDeleteTransformer(
    ObjectStoragePtr object_storage, const SharedHeader& header, const std::optional<FormatSettings>& format_settings,
    FormatParserSharedResourcesPtr parser_shared_resources, ContextPtr context_) {
  ...
  if (!context_->getSettingsRef()[Setting::use_roaring_bitmap_iceberg_positional_deletes].value)
    return std::make_shared<IcebergStreamingPositionDeleteTransform>(header, self, object_storage, format_settings, parser_shared_resources,
                                                                     context_);
  else
    return std::make_shared<IcebergBitmapPositionDeleteTransform>(header, self, object_storage, format_settings, parser_shared_resources,
                                                                  context_);
}
...
void IcebergDataObjectInfo::addPositionDeleteObject(Iceberg::ProcessedManifestFileEntryPtr position_delete_object) {
  if (Poco::toUpper(info.file_format) != "PARQUET") {
    throw Exception(ErrorCodes::NOT_IMPLEMENTED,
                    "Position deletes are only supported for data files of Parquet format in Iceberg, but got {}", info.file_format);
  }
  info.position_deletes_objects.emplace_back(position_delete_object->file_path, position_delete_object->parsed_entry->file_format,
                                             std::nullopt);
}

void IcebergDataObjectInfo::addEqualityDeleteObject(const Iceberg::ProcessedManifestFileEntryPtr& equality_delete_object) {
  info.equality_deletes_objects.emplace_back(equality_delete_object->file_path, equality_delete_object->parsed_entry->file_format,
...
void IcebergObjectSerializableInfo::checkVersion(size_t protocol_version) const {
  if (protocol_version < DBMS_CLUSTER_PROCESSING_PROTOCOL_VERSION_WITH_ICEBERG_METADATA) {
    throw Exception(ErrorCodes::UNKNOWN_PROTOCOL,
                    "IcebergObjectSerializableInfo serialization is supported since protocol version {}, got: {}",
                    DBMS_CLUSTER_PROCESSING_PROTOCOL_VERSION_WITH_ICEBERG_METADATA, protocol_version);
  }
}
}  // namespace DB
