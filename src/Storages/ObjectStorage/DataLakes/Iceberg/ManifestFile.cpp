...
bool operator<(const PartitionSpecification& lhs, const PartitionSpecification& rhs) { return less(lhs, rhs); }

bool operator<(const DB::Row& lhs, const DB::Row& rhs) { return less(lhs, rhs); }
...
String dumpPartitionSpecification(const PartitionSpecification& partition_specification) {
  if (partition_specification.empty())
    ...
    for (size_t i = 0; i < partition_specification.size(); ++i) {
      ...
      answer += fmt::format("(source id: {}, transform name: {}, partition name: {})", entry.source_id, entry.transform_name,
                            entry.partition_name);
      if (i != partition_specification.size() - 1) answer += ", ";
    }
    answer += ']';
    return answer;
  }
...
String dumpPartitionKeyValue(const DB::Row& partition_key_value) {
  if (partition_key_value.empty())
    ...
    for (size_t i = 0; i < partition_key_value.size(); ++i) {
      ...
      answer += entry.dump();
      if (i != partition_key_value.size() - 1) answer += ", ";
    }
    answer += ']';
    return answer;
  }
...
String ProcessedManifestFileEntry::dumpDeletesMatchingInfo() const {
  return fmt::format("Partition specification: {}, partition key value: {}, added sequence number: {}",
                     dumpPartitionSpecification(*common_partition_specification), dumpPartitionKeyValue(parsed_entry->partition_key_value),
                     sequence_number);
}
}  // namespace DB::Iceberg
