...
void IcebergPositionDeleteTransform::initializeDeleteSources() {
  ...
  ASTPtr where_ast = makeASTFunction("equals", make_intrusive<ASTIdentifier>(IcebergPositionDeleteTransform::data_file_path_column_name),
  ...
  for (const auto& position_deletes_object : iceberg_object_info->info.position_deletes_objects) {
    ...
    std::shared_ptr<const ActionsDAG> actions_dag_ptr = [&actions]() {
      if (actions.has_value()) return std::make_shared<const ActionsDAG>(std::move(actions.value()));
      return std::shared_ptr<const ActionsDAG>();
    }();

    auto delete_format = FormatFactory::instance().getInput(
        format, *delete_read_buffers.back(), initial_header, context, context->getSettingsRef()[DB::Setting::max_block_size],
        format_settings, parser_shared_resources, std::make_shared<FormatFilterInfo>(actions_dag_ptr, context, nullptr, nullptr, nullptr),
        true /* is_remote_fs */, compression_method);
...
void IcebergBitmapPositionDeleteTransform::transform(Chunk& chunk) { DeletionVectorTransform::transform(chunk, bitmap); }
...
void IcebergStreamingPositionDeleteTransform::initialize() {
  for (size_t i = 0; i < delete_sources.size(); ++i) {
    ...
    delete_source_column_indices.push_back(PositionDeleteFileIndexes{.filename_index = filename_index, .position_index = position_index});
...
void IcebergStreamingPositionDeleteTransform::transform(Chunk& chunk) {
  ...
  if (!chunk_info) throw Exception(ErrorCodes::LOGICAL_ERROR, "ChunkInfoRowNumbers does not exist");
        ...
        if (it->first < row_idx) {
          ...
          if (iterator_at_latest_chunks[delete_source_index] + 1 >= latest_chunks[delete_source_index].getNumRows() &&
            ...
            size_t next_index_value_in_positional_delete_file =
                latest_chunks[delete_source_index].getColumns()[position_index]->get64(iterator_at_latest_chunks[delete_source_index]);
          ...
          if (chunk_info->applied_filter.has_value()) chunk_info->applied_filter.value()[i] = false;
  ...
  for (auto& column : columns) column = column->filter(filter, -1);

  if (!chunk_info->applied_filter.has_value()) chunk_info->applied_filter.emplace(std::move(filter));
