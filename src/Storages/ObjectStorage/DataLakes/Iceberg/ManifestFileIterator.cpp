...
std::optional<DB::Field> deserializeFieldFromBinaryRepr(std::string str, DB::DataTypePtr expected_type, bool lower_bound) {
  ...
  if (DB::WhichDataType(non_nullable_type).isDecimal()) {
    ...
    for (const auto byte : str) unscaled_value = (unscaled_value << 8) | static_cast<uint8_t>(byte);
...
const std::vector<ProcessedManifestFileEntryPtr> &ManifestFileIterator::ManifestFileEntriesHandle::getFilesWithoutDeleted(
...
bool ManifestFileIterator::ManifestFileEntriesHandle::areAllDataFilesSortedBySortOrderID(Int32 sort_order_id) const {
  for (const auto &file : *data_files) {
    ...
    if (!file->parsed_entry->sort_order_id.has_value() || (*file->parsed_entry->sort_order_id != sort_order_id)) return false;
  }
  /// Empty manifest (no data files) is considered sorted by definition
  return true;
}

std::optional<Int64> ManifestFileIterator::ManifestFileEntriesHandle::getRowsCountInAllFilesExcludingDeleted(
    FileContentType content) const {
  Int64 result = 0;
...
ManifestFileIterator::ManifestFileEntriesHandle ManifestFileIterator::getFilesWithoutDeletedHandle() const {
  if (!isInitialized()) throw Exception(ErrorCodes::LOGICAL_ERROR, "Cannot get files from manifest file before it is fully initialized");

  SharedLockGuard lock{files_mutex};
  return ManifestFileEntriesHandle{data_files_without_deleted, position_deletes_files_without_deleted,
...
std::shared_ptr<ManifestFileIterator> ManifestFileIterator::create(
    std::shared_ptr<AvroForIcebergDeserializer> manifest_file_deserializer_, const String &manifest_file_name_, Int32 format_version_,
    const String &common_path_, IcebergSchemaProcessor &schema_processor, Int64 inherited_sequence_number_, Int64 inherited_snapshot_id_,
    const String &table_location_, DB::ContextPtr context_, const String &path_to_manifest_file_,
    std::shared_ptr<const ActionsDAG> filter_dag_, Int32 table_snapshot_schema_id_) {
  insertRowToLogTable(context_, manifest_file_deserializer_->getMetadataContent(), DB::IcebergMetadataLogLevel::ManifestFileMetadata,
                      common_path_, path_to_manifest_file_, std::nullopt, std::nullopt);

  for (const auto &column_name : {f_status, f_data_file}) {
    if (!manifest_file_deserializer_->hasPath(column_name))
      throw Exception(DB::ErrorCodes::ICEBERG_SPECIFICATION_VIOLATION, "Required columns are not found in manifest file: {}", column_name);
  }

  if (format_version_ > 1 && !manifest_file_deserializer_->hasPath(f_sequence_number))
    throw Exception(ErrorCodes::ICEBERG_SPECIFICATION_VIOLATION, "Required columns are not found in manifest file: {}", f_sequence_number);
  ...
  if (!schema_json_string.has_value())
    throw Exception(ErrorCodes::BAD_ARGUMENTS, "Cannot read Iceberg table: manifest file '{}' doesn't have field '{}' in its metadata",
                    manifest_file_name_, f_schema);
  ...
  for (size_t i = 0; i != partition_specification->size(); ++i) {
    ...
    if (!manifest_file_column_characteristics.has_value()) continue;
    ...
    if (partition_ast == nullptr) continue;
  ...
  return std::shared_ptr<ManifestFileIterator>(
      new ManifestFileIterator(std::move(manifest_file_deserializer_), path_to_manifest_file_, manifest_file_name_, format_version_,
                               common_path_, table_location_, schema_processor, inherited_sequence_number_, inherited_snapshot_id_,
                               context_, manifest_schema_id, std::make_shared<const PartitionSpecification>(std::move(partition_spec_vec)),
                               std::move(partition_key_description), total_rows, std::move(filter_dag_), table_snapshot_schema_id_));
}

ManifestFileIterator::ManifestFileIterator(std::shared_ptr<AvroForIcebergDeserializer> manifest_file_deserializer_,
                                           const String &path_to_manifest_file_, const String &manifest_file_name_, Int32 format_version_,
                                           const String &common_path_, const String &table_location_,
                                           IcebergSchemaProcessor &schema_processor, Int64 inherited_sequence_number_,
                                           Int64 inherited_snapshot_id_, DB::ContextPtr context_, Int32 manifest_schema_id_,
                                           std::shared_ptr<const PartitionSpecification> common_partition_specification_,
                                           std::optional<DB::KeyDescription> partition_key_description_, size_t total_rows_,
                                           std::shared_ptr<const ActionsDAG> filter_dag_, Int32 table_snapshot_schema_id_)
...
ProcessedManifestFileEntryPtr ManifestFileIterator::processRow(size_t row_index) {
  ...
  if (!parsed_entry)
    throw Exception(ErrorCodes::LOGICAL_ERROR, "Got null pure manifest file entry for row {} in manifest file '{}'", row_index,
                    path_to_manifest_file);

  if (parsed_entry->status == ManifestEntryStatus::DELETED) {
    insertRowToLogTable(context, manifest_file_deserializer->getContent(row_index), DB::IcebergMetadataLogLevel::ManifestFileEntry,
                        common_path, path_to_manifest_file, row_index, std::nullopt);
    return nullptr;
  }
  ...
  if (parsed_entry->parsed_snapshot_id.has_value()) {
    ...
    throw Exception(ErrorCodes::ICEBERG_SPECIFICATION_VIOLATION,
                    ...
                    manifest_file_name, resolved_snapshot_id);
  } else {
  ...
  if (!schema_id_opt.has_value()) {
    ...
    /// That's actually an error because it can lead to incorrect query results, so we are creating an exception to put it to
    /// system.error_log.
    try {
      throw Exception(
  ...
  if (format_version > 1) {
    if (parsed_entry->parsed_sequence_number.has_value()) {
      ...
      if (!parsed_entry->parsed_sequence_number.has_value())
        throw Exception(DB::ErrorCodes::ICEBERG_SPECIFICATION_VIOLATION,
  ...
  auto entry = std::make_shared<ProcessedManifestFileEntry>(parsed_entry, common_partition_specification, file_path,
                                                            resolved_sequence_number, resolved_schema_id);
  ...
  if (filter_dag) {
    ...
    if (parsed_entry->content_type == FileContentType::DATA) {
      for (const auto &[column_id, bounds] : parsed_entry->value_bounds) {
        ...
        if (!field_characteristics) continue;
        ...
        if (!bounds.first.tryGet(left_str) || !bounds.second.tryGet(right_str)) continue;
  ...
  insertRowToLogTable(context, manifest_file_deserializer->getContent(row_index), DB::IcebergMetadataLogLevel::ManifestFileEntry,
                      common_path, path_to_manifest_file, row_index, pruning_status);
...
const ManifestFilesPruner *ManifestFileIterator::getOrCreatePruner(Int32 schema_id) {
  ...
  if (it != pruners_by_schema_id.end()) return it->second.get();

  auto pruner =
      std::make_unique<ManifestFilesPruner>(*schema_processor_ptr, table_snapshot_schema_id, schema_id, filter_dag.get(), *this, context);
  auto *raw_ptr = pruner.get();
  pruners_by_schema_id.emplace(schema_id, std::move(pruner));
  return raw_ptr;
}

bool ManifestFileIterator::isInitialized() const { return fully_initialized && active_fetchers == 0; }

ProcessedManifestFileEntryPtr ManifestFileIterator::next() {
  if (fully_initialized.load()) return nullptr;
...
bool ManifestFileIterator::hasPartitionKey() const { return partition_key_description.has_value(); }

const DB::KeyDescription &ManifestFileIterator::getPartitionKeyDescription() const {
  if (!hasPartitionKey()) throw Exception(ErrorCodes::LOGICAL_ERROR, "Table has no partition key, but it was requested");
