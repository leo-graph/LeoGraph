...
void DataFileStatistics::update(const Chunk& chunk) {
  ...
  for (size_t i = 0; i < num_columns; ++i) {
    ...
    for (size_t j = 0; j < chunk.getNumRows(); ++j) null_counts[i] += (chunk.getColumns()[i]->isNullAt(j));
...
Range DataFileStatistics::uniteRanges(const Range& left, const Range& right) {
  return Range(Range::less(left.left, right.left) ? left.left : right.left, true,
               Range::less(right.right, left.right) ? left.right : right.right, true);
}
