...
static bool isTemporaryMetadataFile(const String& file_name) {
  ...
  if (string_position == String::npos) return true;
...
static Iceberg::MetadataFileWithInfo getMetadataFileAndVersion(const std::string& path) {
  ...
  if (isTemporaryMetadataFile(file_name)) {
    throw Exception(
        ...
        "Temporary metadata file '{}' should not be used for reading. It is created during commit operation and should be ignored", path);
  }
  String version_str;
  /// v<V>.metadata.json
  if (file_name.starts_with('v')) version_str = String(file_name.begin() + 1, file_name.begin() + file_name.find_first_of('.'));
...
void writeMessageToFile(const String& data, const String& filename, ObjectStoragePtr object_storage, ContextPtr context,
                        const std::string& write_if_none_match, const std::string& write_if_match, CompressionMethod compression_method) {
  ...
  auto buffer_metadata =
      object_storage->writeObject(StoredObject(filename), WriteMode::Rewrite, std::nullopt, DBMS_DEFAULT_BUFFER_SIZE, write_settings);
  if (compression_method != CompressionMethod::None) {
    auto settings = context->getSettingsRef();
    auto compressed_buffer_metadata = wrapWriteBufferWithCompressionMethod(
        std::move(buffer_metadata), compression_method, static_cast<int>(settings[Setting::output_format_compression_level]));
...
bool writeMetadataFileAndVersionHint(const std::string& metadata_file_path, const std::string& metadata_file_content,
                                     const std::string& version_hint_path, std::string version_hint_content,
                                     DB::ObjectStoragePtr object_storage, DB::ContextPtr context, DB::CompressionMethod compression_method,
                                     bool try_write_version_hint) {
  try {
    if (object_storage->exists(StoredObject(metadata_file_path))) return false;

    Iceberg::writeMessageToFile(metadata_file_content, metadata_file_path, object_storage, context, /* write-if-none-match */ "*", "",
                                compression_method);
  } catch (...) {
  ...
  if (try_write_version_hint) {
    if (version_hint_content.starts_with('/')) version_hint_content = version_hint_content.substr(1);
    ...
    while (i < MAX_TRANSACTION_RETRIES) {
      ...
      if (old_version < new_version) {
        try {
          Iceberg::writeMessageToFile(version_hint_content, version_hint_path, object_storage, context, write_if_none_match,
                                      /* write-if-match */ etag);
          break;
        } catch (...) {
          tryLogCurrentException(__PRETTY_FUNCTION__);
...
std::optional<TransformAndArgument> parseTransformAndArgument(const String& transform_name_src) {
  ...
  if (transform_name == "year" || transform_name == "years") return TransformAndArgument{"toYearNumSinceEpoch", std::nullopt};

  if (transform_name == "month" || transform_name == "months") return TransformAndArgument{"toMonthNumSinceEpoch", std::nullopt};
  ...
  if (transform_name == "hour" || transform_name == "hours") return TransformAndArgument{"toRelativeHourNum", std::nullopt};

  if (transform_name == "identity") return TransformAndArgument{"identity", std::nullopt};

  if (transform_name == "void") return TransformAndArgument{"tuple", std::nullopt};

  if (transform_name.starts_with("truncate") || transform_name.starts_with("bucket")) {
    /// should look like transform[N] or bucket[N]

    if (transform_name.back() != ']') return std::nullopt;

    auto argument_start = transform_name.find('[');

    if (argument_start == std::string::npos) throw Exception(DB::ErrorCodes::BAD_ARGUMENTS, "Incorrect transform name {}", transform_name);
...
// This function is used to get the file path inside the directory which corresponds to iceberg table from the full blob path which is
// written in manifest and metadata files. For example, if the full blob path is s3://bucket/table_name/data/00000-1-1234567890.avro, the
// function will return table_name/data/00000-1-1234567890.avro Common path should end with "<table_name>" or "<table_name>/".
...
enum class MostRecentMetadataFileSelectionWay { BY_LAST_UPDATED_MS_FIELD, BY_METADATA_FILE_VERSION };
...
Poco::JSON::Object::Ptr getMetadataJSONObject(const String& metadata_file_path, ObjectStoragePtr object_storage,
                                              IcebergMetadataFilesCachePtr metadata_cache, const ContextPtr& local_context, LoggerPtr log,
                                              CompressionMethod compression_method, const std::optional<String>& table_uuid) {
  auto create_fn = [&]() {
    ObjectInfo object_info(metadata_file_path);

    auto read_settings = local_context->getReadSettings();
    /// Do not utilize filesystem cache if more precise cache enabled
    if (metadata_cache) read_settings.enable_filesystem_cache = false;
  ...
  if (metadata_cache && table_uuid.has_value())
    metadata_json_str =
        metadata_cache->getOrSetTableMetadata(IcebergMetadataFilesCache::getKey(*table_uuid, metadata_file_path), create_fn);
  else
...
Poco::JSON::Object::Ptr getPartitionField(ASTPtr partition_by_element, const std::unordered_map<String, Int32>& column_name_to_source_id,
  ...
  if (!partition_function) {
    ...
    if (it == column_name_to_source_id.end()) throw Exception(ErrorCodes::BAD_ARGUMENTS, "Unknown field to partition {}", field);
  ...
  for (const auto& child : partition_function->children) {
    ...
    if (!expression_list) throw Exception(ErrorCodes::BAD_ARGUMENTS, "Unsupported partitioning for Iceberg table.");
  ...
  if (!field) throw Exception(ErrorCodes::BAD_ARGUMENTS, "Functions with no arguments are not supported in Iceberg.");

  Poco::JSON::Object::Ptr result = new Poco::JSON::Object;
  result->set(Iceberg::f_name, field.value());

  if (!column_name_to_source_id.contains(*field)) throw Exception(ErrorCodes::BAD_ARGUMENTS, "Unknown field to partition {}", *field);
...
std::pair<Poco::JSON::Object::Ptr, Int32> getPartitionSpec(ASTPtr partition_by,
...
static String parseColumnArgument(const ASTPtr& arg_ast, const String& clickhouse_name, const String& error_suffix) {
  ...
  if (!identifier) throw Exception(ErrorCodes::BAD_ARGUMENTS, "Invalid iceberg sort order function {}: {}", clickhouse_name, error_suffix);
...
static std::pair<String, String> parseFunction(const ASTPtr& func_object) {
  const static std::unordered_map<String, String> clickhouse_name_to_iceberg = {
      {"identity", "identity"},          {"icebergBucket", "bucket"}, {"icebergTruncate", "truncate"}, {"toYearNumSinceEpoch", "year"},
      {"toMonthNumSinceEpoch", "month"}, {"toRelativeDayNum", "day"}, {"toRelativeHourNum", "hour"}};

  const auto* func = func_object ? func_object->as<ASTFunction>() : nullptr;
  if (!func) throw Exception(ErrorCodes::BAD_ARGUMENTS, "Invalid iceberg sort order expression, expected a function");
  ...
  if (args.size() == 1) {
    ...
    column_name =
        parseColumnArgument(args[1], clickhouse_name, "expected (integer_literal, column_identifier), but there is no column_identifier");
    ...
    if (literal->value.tryGet(u_param))
      ...
      throw Exception(ErrorCodes::BAD_ARGUMENTS,
                      "Invalid iceberg sort order function {}: expected a non-negative integer literal as first argument", clickhouse_name);
  } else {
    throw Exception(ErrorCodes::BAD_ARGUMENTS, "Invalid iceberg sort order function {}: expected 1 or 2 arguments, but got {}",
                    clickhouse_name, args.size());
  }

  String transform = it->second;
  if (arg.has_value()) transform += "[" + std::to_string(arg.value()) + "]";

  return {transform, column_name};
}

static ASTPtr unwrapOrderByElement(ASTPtr ast) {
  if (!ast) throw Exception(ErrorCodes::BAD_ARGUMENTS, "Invalid iceberg sort order expression");
...
static std::pair<String, String> parseTransformAndColumnElement(ASTPtr element) {
  ...
  if (const auto* identifier = element->as<ASTIdentifier>()) return {"identity", identifier->name()};

  if (const auto* func = element->as<ASTFunction>(); func && func->name != "tuple") return parseFunction(element);
...
static std::vector<std::pair<String, String>> parseTransformAndColumnPairs(ASTPtr object) {
  if (!object) throw Exception(ErrorCodes::LOGICAL_ERROR, "Invalid iceberg sort order expression");
  ...
  if (const auto* func = object->as<ASTFunction>(); func && func->name == "tuple") {
    const auto* args_list = func->arguments ? func->arguments->as<ASTExpressionList>() : nullptr;
    if (!args_list) throw Exception(ErrorCodes::BAD_ARGUMENTS, "Invalid iceberg sort order tuple expression");
...
std::pair<Poco::JSON::Object::Ptr, String> createEmptyMetadataFile(String path_location, const ColumnsDescription& columns,
                                                                   ASTPtr partition_by, ASTPtr order_by, ContextPtr context,
  ...
  if (format_version > 1) new_metadata_file_content->set(Iceberg::f_last_sequence_number, 0);
  ...
  if (order_by) {
    ...
    if (transform_and_column_pairs.size() != sort_columns.size())
      throw Exception(ErrorCodes::BAD_ARGUMENTS, "Invalid iceberg sort order: expected {} elements, but got {}", sort_columns.size(),
 ...
 */
static MetadataFileWithInfo getLatestMetadataFileAndVersion(const ObjectStoragePtr& object_storage, const String& table_path,
                                                            const DataLakeStorageSettings& data_lake_settings,
                                                            IcebergMetadataFilesCachePtr metadata_cache, const ContextPtr& local_context,
  ...
  for (const auto& path : metadata_files) {
    ...
    if (isTemporaryMetadataFile(filename)) continue;
    auto [version, metadata_file_path, compression_method] = getMetadataFileAndVersion(path);

    if (need_all_metadata_files_parsing) {
      auto metadata_file_object =
          getMetadataJSONObject(metadata_file_path, object_storage, metadata_cache, local_context, log, compression_method, table_uuid);
      if (table_uuid.has_value() && use_table_uuid_for_metadata_file_selection) {
        if (metadata_file_object->has(Iceberg::f_table_uuid)) {
          ...
          if (normalizeUuid(table_uuid.value()) == normalizeUuid(current_table_uuid)) {
            metadata_files_with_versions.emplace_back(version, metadata_file_object->getValue<UInt64>(Iceberg::f_last_updated_ms),
                                                      metadata_file_path);
          }
        } else {
          Int64 format_version = metadata_file_object->getValue<Int64>(Iceberg::f_format_version);
          throw Exception(format_version == 1 ? ErrorCodes::BAD_ARGUMENTS : ErrorCodes::ICEBERG_SPECIFICATION_VIOLATION,
                          "Table UUID is not specified in some metadata files for table by path {}", metadata_file_path);
        }
      } else {
        metadata_files_with_versions.emplace_back(version, metadata_file_object->getValue<UInt64>(Iceberg::f_last_updated_ms),
                                                  metadata_file_path);
  ...
  if (metadata_files_with_versions.empty()) {
    if (table_uuid.has_value() && use_table_uuid_for_metadata_file_selection) {
      throw Exception(ErrorCodes::FILE_DOESNT_EXIST, "The metadata file for Iceberg table with path {} and table UUID {} doesn't exist",
                      table_path, table_uuid.value());
    }
    throw Exception(ErrorCodes::FILE_DOESNT_EXIST, "The metadata file for Iceberg table with path {} doesn't exist", table_path);
  }
  ...
  const ShortMetadataFileInfo& latest_metadata_file_info = [&]() {
    if (selection_way == MostRecentMetadataFileSelectionWay::BY_LAST_UPDATED_MS_FIELD) {
      return *std::max_element(
          metadata_files_with_versions.begin(), metadata_files_with_versions.end(),
...
static String resolveContained(const std::filesystem::path& base, const std::filesystem::path& relative) {
  ...
  if (rel.empty() || rel.begin()->string() == "..") {
    throw Exception(ErrorCodes::PATH_ACCESS_DENIED, "Explicit metadata file path `{}` should be in the table path directory : `{}`",
                    relative.string(), base.string());
  }

  return combined.string();
}

MetadataFileWithInfo getLatestOrExplicitMetadataFileAndVersion(const ObjectStoragePtr& object_storage, const String& table_path,
                                                               const DataLakeStorageSettings& data_lake_settings,
                                                               IcebergMetadataFilesCachePtr metadata_cache, const ContextPtr& local_context,
                                                               Poco::Logger* log, const std::optional<String>& table_uuid) {
  if (data_lake_settings[DataLakeStorageSetting::iceberg_metadata_file_path].changed) {
    ...
    auto it = p.begin();
    if (it != p.end()) {
      if (*it == "." || *it == "..") throw Exception(ErrorCodes::BAD_ARGUMENTS, "Relative paths are not allowed");
    }
    ...
    if (table_uuid.has_value()) {
      if (normalizeUuid(explicit_table_uuid) != table_uuid.value()) {
        throw Exception(ErrorCodes::BAD_ARGUMENTS, "Explicit table UUID '{}' doesn't match the one from table properties '{}'",
                        normalizeUuid(explicit_table_uuid), table_uuid.value());
      }
    }
    LOG_TEST(log, "Explicit table UUID is specified {}, will read the latest metadata file for Iceberg table at path {}",
             explicit_table_uuid, table_path);
    return getLatestMetadataFileAndVersion(object_storage, table_path, data_lake_settings, metadata_cache, local_context,
                                           normalizeUuid(explicit_table_uuid), true);
    ...
    return getLatestMetadataFileAndVersion(object_storage, table_path, data_lake_settings, metadata_cache, local_context, table_uuid,
                                           false);
  }
}
...
std::pair<Poco::JSON::Object::Ptr, Int32> parseTableSchemaV2Method(const Poco::JSON::Object::Ptr& metadata_object) {
  ...
  if (schemas->size() == 0) throw Exception(ErrorCodes::BAD_ARGUMENTS, "Cannot parse Iceberg table schema: '{}' field is empty", f_schemas);
  ...
  if (!schema)
    throw Exception(ErrorCodes::BAD_ARGUMENTS, R"(There is no schema with "{}" that matches "{}" in metadata)", f_schema_id,
                    f_current_schema_id);
  if (schema->getValue<int>(f_schema_id) != current_schema_id)
    throw Exception(ErrorCodes::BAD_ARGUMENTS, R"(Field "{}" of the schema doesn't match "{}" in metadata)", f_schema_id,
                    f_current_schema_id);
...
KeyDescription getSortingKeyDescriptionFromMetadata(Poco::JSON::Object::Ptr metadata_object, const NamesAndTypesList& ch_schema,
  ...
  for (size_t i = 0; i < ch_schema.size(); ++i) column_description.add(ColumnDescription(ch_schema.getNames()[i], ch_schema.getTypes()[i]));

  String order_by_str;

  for (UInt32 i = 0; i < sort_orders->size(); ++i) {
    auto sort_order = sort_orders->getObject(i);
    if (sort_order->getValue<Int64>(f_order_id) != sort_order_id) continue;
  ...
  if (order_by_str.empty()) return KeyDescription{};
...
DataTypePtr getFunctionResultType(const String& iceberg_transform_name, DataTypePtr source_type) {
  if (iceberg_transform_name.starts_with("identity") || iceberg_transform_name.starts_with("truncate")) return source_type;
  if (iceberg_transform_name.starts_with("year")) return std::make_shared<DataTypeUInt16>();
  if (iceberg_transform_name.starts_with("month") || iceberg_transform_name.starts_with("day") ||
      iceberg_transform_name.starts_with("hour"))
...
void sortBlockByKeyDescription(Block& block, const KeyDescription& sort_description, ContextPtr context) {
  ...
  for (const auto& column_name : initial_column_names) reordered_columns.push_back(block.getByName(column_name));
  for (size_t i = 0; i < block.columns(); ++i)
    if (!initial_column_names_set.contains(block.getNames()[i])) reordered_columns.push_back(block.getColumnsWithTypeAndName()[i]);
