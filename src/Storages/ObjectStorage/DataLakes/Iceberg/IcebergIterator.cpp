...
namespace {
std::span<const ProcessedManifestFileEntryPtr> defineDeletesSpan(ProcessedManifestFileEntryPtr data_object_,
                                                                 const std::vector<ProcessedManifestFileEntryPtr>& deletes_objects,
                                                                 bool is_equality_delete, LoggerPtr logger) {
  ...
  auto beg_it = is_equality_delete ? std::upper_bound(deletes_objects.begin(), deletes_objects.end(), data_object_)
                                   : std::lower_bound(deletes_objects.begin(), deletes_objects.end(), data_object_);
  auto end_it = std::upper_bound(deletes_objects.begin(), deletes_objects.end(), data_object_,
  ...
  if (beg_it - deletes_objects.begin() > end_it - deletes_objects.begin()) {
    throw DB::Exception(DB::ErrorCodes::LOGICAL_ERROR,
                        ...
                        is_equality_delete ? "Equality" : "Position", beg_it - deletes_objects.begin(), end_it - deletes_objects.begin(),
  ...
  if (beg_it != end_it) {
    ...
    LOG_DEBUG(
        ...
        std::distance(beg_it, end_it), is_equality_delete ? "equality" : "position", data_object_->file_path,
        data_object_->dumpDeletesMatchingInfo(), (*beg_it)->dumpDeletesMatchingInfo(), (*previous_it)->dumpDeletesMatchingInfo());
  } else {
    LOG_DEBUG(logger, "No {} delete elements for data file {}, taken data file object info: {}",
              is_equality_delete ? "equality" : "position", data_object_->file_path, data_object_->dumpDeletesMatchingInfo());
  }
...
std::optional<ProcessedManifestFileEntryPtr> SingleThreadIcebergKeysIterator::next() {
  ...
  while (true) {
    ...
    while (manifest_file_index < data_snapshot->manifest_list_entries.size()) {
      ...
      if (mle.content_type != manifest_file_content_type) continue;

      auto manifest_file_cacheable_part = Iceberg::getManifestFile(object_storage, persistent_components, local_context, log,
                                                                   mle.manifest_file_path, mle.manifest_file_byte_size);

      current_manifest_file_iterator = Iceberg::ManifestFileIterator::create(
          manifest_file_cacheable_part.deserializer, mle.manifest_file_path, persistent_components.format_version,
          persistent_components.table_path, *persistent_components.schema_processor, mle.added_sequence_number, mle.added_snapshot_id,
          persistent_components.table_location, local_context, mle.manifest_file_path, filter_dag, table_snapshot->schema_id);
      break;
    }

    if (!current_manifest_file_iterator) return std::nullopt;
  }
}

SingleThreadIcebergKeysIterator::SingleThreadIcebergKeysIterator(ObjectStoragePtr object_storage_, ContextPtr local_context_,
    ...
    : object_storage(object_storage_),
      filter_dag([&]() -> std::shared_ptr<const ActionsDAG> {
        if (!filter_dag_) return nullptr;
        if (!local_context_) {
          throw DB::Exception(DB::ErrorCodes::LOGICAL_ERROR,
                              "Context is required with non-empty filter_dag to implement "
                              "partition pruning for Iceberg table");
        }
        if (!local_context_->getSettingsRef()[Setting::use_iceberg_partition_pruning].value) return nullptr;
...
IcebergIterator::IcebergIterator(ObjectStoragePtr object_storage_, ContextPtr local_context_, const ActionsDAG* filter_dag_,
                                 IDataLakeMetadata::FileProgressCallback callback_, Iceberg::TableStateSnapshotPtr table_snapshot_,
                                 Iceberg::IcebergDataSnapshotPtr data_snapshot_, PersistentTableComponents persistent_components_)
    : logger(getLogger("IcebergIterator")),
      ...
      persistent_components(persistent_components_),
      data_files_iterator(object_storage, local_context_, Iceberg::ManifestFileContentType::DATA, filter_dag.get(), table_snapshot_,
                          data_snapshot_, persistent_components_),
      deletes_iterator(object_storage, local_context_, Iceberg::ManifestFileContentType::DELETE, filter_dag.get(), table_snapshot_,
                       data_snapshot_, persistent_components_),
      blocking_queue(100),
  ...
  LOG_DEBUG(logger, "Taken {} position deletes file and {} equality deletes files in iceberg iterator", position_deletes_files.size(),
  ...
  producer_task.emplace([this, thread_group = CurrentThread::getGroup()]() {
...
ObjectInfoPtr IcebergIterator::next(size_t) {
  ...
  if (blocking_queue.pop(manifest_file_entry)) {
    IcebergDataObjectInfoPtr object_info = std::make_shared<IcebergDataObjectInfo>(manifest_file_entry, table_state_snapshot->schema_id);
    for (const auto& position_delete :
      ...
      if (!can_contain_data_file_deletes) {
        ...
        LOG_TEST(logger,
                 ...
                 position_delete->file_path, data_file_path, lower.value_or("[no lower bound]"), upper.value_or("[no upper bound]"));
      } else {
    ...
    if (!object_info->info.position_deletes_objects.empty()) {
      LOG_DEBUG(logger, "Finally got {} position delete elements for data file {}", object_info->info.position_deletes_objects.size(),
    ...
    if (!object_info->info.equality_deletes_objects.empty()) {
      LOG_DEBUG(logger, "Finally got {} equality delete elements for data file {}", object_info->info.equality_deletes_objects.size(),
                object_info->info.data_object_file_path_key);
    }
...
size_t IcebergIterator::estimatedKeysCount() { return std::numeric_limits<size_t>::max(); }
