...
Iceberg::TableStateSnapshotPtr extractIcebergSnapshotIdFromMetadataObject(StorageMetadataPtr storage_metadata) {
  if (!storage_metadata || !storage_metadata->datalake_table_state.has_value()) return nullptr;
...
Iceberg::PersistentTableComponents IcebergMetadata::initializePersistentTableComponents(StorageObjectStorageConfigurationPtr configuration,
                                                                                        IcebergMetadataFilesCachePtr cache_ptr,
                                                                                        ContextPtr context_) {
  const auto [metadata_version, metadata_file_path, compression_method] =
      getLatestOrExplicitMetadataFileAndVersion(object_storage, configuration->getPathForRead().path, configuration->getDataLakeSettings(),
                                                cache_ptr, context_, log.get(), std::nullopt);
  ...
  if (metadata_object->has(Iceberg::f_table_uuid)) {
    ...
    if (format_version >= 2) {
      throw Exception(ErrorCodes::ICEBERG_SPECIFICATION_VIOLATION, "Iceberg table metadata file '{}' doesn't contain required field '{}'",
                      metadata_file_path, Iceberg::f_table_uuid);
    }
  }
...
std::pair<IcebergDataSnapshotPtr, TableStateSnapshot> IcebergMetadata::getRelevantState(const ContextPtr &context) const {
  const auto [metadata_version, metadata_file_path, compression_method] =
      getLatestOrExplicitMetadataFileAndVersion(object_storage, persistent_components.table_path, data_lake_settings,
                                                persistent_components.metadata_cache, context, log.get(), persistent_components.table_uuid);
  return getState(context, metadata_file_path, metadata_version);
}

IcebergMetadata::IcebergMetadata(ObjectStoragePtr object_storage_, StorageObjectStorageConfigurationPtr configuration_,
                                 const ContextPtr &context_, IcebergMetadataFilesCachePtr cache_ptr)
...
Int32 IcebergMetadata::parseTableSchema(const Poco::JSON::Object::Ptr &metadata_object, IcebergSchemaProcessor &schema_processor,
  ...
  if (format_version == 2) {
    ...
    try {
      ...
      if (first_error.code() != ErrorCodes::BAD_ARGUMENTS) throw;
      try {
        auto [schema, current_schema_id] = parseTableSchemaV2Method(metadata_object);
        schema_processor.addIcebergTableSchema(schema);
        LOG_WARNING(metadata_logger,
        ...
        throw Exception(ErrorCodes::BAD_ARGUMENTS,
                        ...
                        first_error.displayText(), second_error.displayText());
      }
    }
  }
}

Poco::JSON::Object::Ptr traverseMetadataAndFindNecessarySnapshotObject(Poco::JSON::Object::Ptr metadata_object, Int64 snapshot_id,
...
IcebergDataSnapshotPtr IcebergMetadata::createIcebergDataSnapshotFromSnapshotJSON(Poco::JSON::Object::Ptr snapshot_object,
                                                                                  Int64 snapshot_id, ContextPtr local_context) const {
  if (!snapshot_object->has(f_manifest_list))
    throw Exception(ErrorCodes::ICEBERG_SPECIFICATION_VIOLATION,
                    "Snapshot object doesn't contain a manifest list path for snapshot with id `{}`", snapshot_id);
  ...
  if (snapshot_object->has(f_summary)) {
    ...
    if (summary_object->has(f_total_records)) total_rows = summary_object->getValue<Int64>(f_total_records);

    if (summary_object->has(f_total_files_size)) total_bytes = summary_object->getValue<Int64>(f_total_files_size);
  ...
  return std::make_shared<IcebergDataSnapshot>(
      getManifestList(object_storage, persistent_components, local_context,
                      getProperFilePathFromMetadataInfo(manifest_list_file_path, persistent_components.table_path,
                                                        persistent_components.table_location),
                      log),
      snapshot_id, schema_id, total_rows, total_bytes, total_position_deletes);
}

IcebergDataSnapshotPtr IcebergMetadata::getIcebergDataSnapshot(Poco::JSON::Object::Ptr metadata_object, Int64 snapshot_id,
  ...
  if (!object) throw Exception(ErrorCodes::ICEBERG_SPECIFICATION_VIOLATION, "No snapshot found for id `{}`", snapshot_id);

  return createIcebergDataSnapshotFromSnapshotJSON(object, snapshot_id, local_context);
}

bool IcebergMetadata::optimize(const StorageMetadataPtr &metadata_snapshot, ContextPtr context,
  ...
  if (context->getSettingsRef()[Setting::allow_experimental_iceberg_compaction]) {
    ...
    compactIcebergTable(snapshots_info, persistent_components, object_storage, data_lake_settings, format_settings, sample_block, context,
                        write_format);
    return true;
  } else {
    throw Exception(ErrorCodes::BAD_ARGUMENTS,
                    "Enable 'allow_experimental_iceberg_compaction' setting to call optimize for iceberg tables.");
  }
}

std::pair<IcebergDataSnapshotPtr, Int32> IcebergMetadata::getStateImpl(const ContextPtr &local_context,
                                                                       Poco::JSON::Object::Ptr metadata_object) const {
  ...
  if (timestamp_changed) {
    ...
    if (!current_snapshot_id)
      throw Exception(ErrorCodes::BAD_ARGUMENTS, "No snapshot found in snapshot log before requested timestamp for iceberg table {}",
                      persistent_components.table_path);
...
std::pair<IcebergDataSnapshotPtr, TableStateSnapshot> IcebergMetadata::getState(const ContextPtr &local_context,
                                                                                const String &metadata_path, Int32 metadata_version) const {
  IcebergDataSnapshotPtr data_snapshot;
  TableStateSnapshot table_state_snapshot;
  auto metadata_object = getMetadataJSONObject(metadata_path, object_storage, persistent_components.metadata_cache, local_context, log,
                                               persistent_components.metadata_compression_method, persistent_components.table_uuid);

  insertRowToLogTable(local_context, dumpMetadataObjectToString(metadata_object), DB::IcebergMetadataLogLevel::Metadata,
                      persistent_components.table_path, metadata_path, std::nullopt, std::nullopt);
...
std::shared_ptr<NamesAndTypesList> IcebergMetadata::getInitialSchemaByPath(ContextPtr, ObjectInfoPtr object_info) const {
  ...
  if (!iceberg_object_info) return nullptr;
...
void IcebergMetadata::mutate(const MutationCommands &commands, StorageObjectStorageConfigurationPtr configuration, ContextPtr context,
                             const StorageID &storage_id, StorageMetadataPtr metadata_snapshot, std::shared_ptr<DataLake::ICatalog> catalog,
                             const std::optional<FormatSettings> &format_settings) {
  ...
  DB::Iceberg::mutate(commands, context, metadata_snapshot, storage_id, object_storage, data_lake_settings, persistent_components,
                      write_format, format_settings, catalog, configuration->getTypeName(), configuration->getNamespace());
}
...
Pipe IcebergMetadata::executeCommand(const String &command_name, const ASTPtr &args, ObjectStoragePtr object_storage_,
                                     StorageObjectStorageConfigurationPtr configuration_, std::shared_ptr<DataLake::ICatalog> catalog_,
                                     ContextPtr context, const StorageID &storage_id) {
  if (!context->getSettingsRef()[Setting::allow_insert_into_iceberg].value) {
    throw Exception(ErrorCodes::SUPPORT_IS_DISABLED,
  ...
  if (command_name == "expire_snapshots") {
    if (args && args->children.size() > 1)
      throw Exception(ErrorCodes::BAD_ARGUMENTS, "expire_snapshots expects zero or one argument (timestamp), got {}",
                      args->children.size());
    ...
    auto result =
        Iceberg::expireSnapshots(expire_before_ms, context, object_storage_, data_lake_settings, persistent_components, write_format,
                                 catalog_, configuration_->getTypeName(), configuration_->getNamespace(), storage_id.getTableName());
...
void IcebergMetadata::createInitial(const ObjectStoragePtr &object_storage, const StorageObjectStorageConfigurationWeakPtr &configuration,
                                    const ContextPtr &local_context, const std::optional<ColumnsDescription> &columns, ASTPtr partition_by,
                                    ASTPtr order_by, bool if_not_exists, std::shared_ptr<DataLake::ICatalog> catalog,
  ...
  if (!metadata_files.empty()) {
    if (if_not_exists)
      ...
      throw Exception(ErrorCodes::TABLE_ALREADY_EXISTS, "Iceberg table with path {} already exists",
  ...
  auto [metadata_content_object, metadata_content] =
      createEmptyMetadataFile(location_path, *columns, partition_by, order_by, local_context,
                              configuration_ptr->getDataLakeSettings()[DataLakeStorageSetting::iceberg_format_version]);
  ...
  if (!compression_suffix.empty()) compression_suffix = "." + compression_suffix;
  ...
  try {
    ...
    if (if_not_exists && e.code() == ErrorCodes::S3_ERROR && e.message().find("PreconditionFailed") != String::npos) return;
    throw;
  }
...
Iceberg::IcebergDataSnapshotPtr IcebergMetadata::getRelevantDataSnapshotFromTableStateSnapshot(
  ...
  IcebergDataSnapshotPtr data_snapshot;
  auto metadata_object =
      getMetadataJSONObject(table_state_snapshot.metadata_file_path, object_storage, persistent_components.metadata_cache, local_context,
                            log, persistent_components.metadata_compression_method, persistent_components.table_uuid);
  if (!table_state_snapshot.snapshot_id.has_value()) return nullptr;
...
DataLakeMetadataPtr IcebergMetadata::create(const ObjectStoragePtr &object_storage,
...
IcebergMetadata::IcebergHistory IcebergMetadata::getHistory(ContextPtr local_context) const {
  const auto [metadata_version, metadata_file_path, compression_method] = getLatestOrExplicitMetadataFileAndVersion(
      object_storage, persistent_components.table_path, data_lake_settings, persistent_components.metadata_cache, local_context, log.get(),
      persistent_components.table_uuid);

  auto metadata_object = getMetadataJSONObject(metadata_file_path, object_storage, persistent_components.metadata_cache, local_context, log,
                                               compression_method, persistent_components.table_uuid);
  chassert(persistent_components.format_version == metadata_object->getValue<int>(f_format_version));
  ...
  for (size_t i = 0; i < snapshots->size(); ++i) {
    ...
    if (summary->has(f_added_data_files)) history_record.added_files = summary->getValue<Int32>(f_added_data_files);
    if (summary->has(f_added_records)) history_record.added_records = summary->getValue<Int32>(f_added_records);
...
bool IcebergMetadata::isDataSortedBySortingKey(StorageMetadataPtr storage_metadata_snapshot, ContextPtr context) const {
  if (!storage_metadata_snapshot->hasSortingKey()) return false;

  auto sorting_key = storage_metadata_snapshot->getSortingKey();
  if (!sorting_key.sort_order_id.has_value()) return false;
  ...
  if (table_state_snapshot == nullptr) {
    throw Exception(ErrorCodes::LOGICAL_ERROR, "Can't extract iceberg table state from storage snapshot for table location {}",
                    persistent_components.table_location);
  }

  /// Empty table is sorted table
  auto data_snapshot = getRelevantDataSnapshotFromTableStateSnapshot(*table_state_snapshot, context);
  if (!data_snapshot) return true;

  for (const auto &manifest_list_entry : data_snapshot->manifest_list_entries) {
    auto files_handle = getManifestFileEntriesHandle(object_storage, persistent_components, context, log, manifest_list_entry,
                                                     table_state_snapshot->schema_id);

    if (!files_handle.areAllDataFilesSortedBySortOrderID(sorting_key.sort_order_id.value())) return false;
  }
  return true;
}
...
std::optional<size_t> IcebergMetadata::totalRows(ContextPtr local_context) const {
  ...
  for (const auto &manifest_list_entry : actual_data_snapshot->manifest_list_entries) {
    auto manifest_file_ptr = getManifestFileEntriesHandle(object_storage, persistent_components, local_context, log, manifest_list_entry,
    ...
    if (!data_count.has_value() || !position_deletes_count.has_value()) return {};
...
std::optional<size_t> IcebergMetadata::totalBytes(ContextPtr local_context) const {
  ...
  if (!actual_data_snapshot) return 0;

  /// All these "hints" with total rows or bytes are optional both in
  /// metadata files and in manifest files, so we try all of them one by one
  if (actual_data_snapshot->total_bytes.has_value()) return actual_data_snapshot->total_bytes;
...
ObjectIterator IcebergMetadata::iterate(const ActionsDAG *filter_dag, FileProgressCallback callback, size_t /* list_batch_size */,
                                        StorageMetadataPtr storage_metadata, ContextPtr local_context) const {
  ...
  return std::make_shared<IcebergIterator>(object_storage, local_context, filter_dag, callback, iceberg_table_state,
...
std::unique_ptr<StorageInMemoryMetadata> IcebergMetadata::buildStorageMetadataFromState(const DataLakeTableStateSnapshot &state,
                                                                                        ContextPtr local_context) const {
  ...
  result->setColumns(ColumnsDescription{*persistent_components.schema_processor->getClickhouseTableSchemaById(iceberg_state.schema_id)});
  result->setDataLakeTableState(state);
  result->sorting_key = getSortingKey(local_context, iceberg_state);
  return result;
}

bool IcebergMetadata::shouldReloadSchemaForConsistency(ContextPtr) const { return true; }
...
void IcebergMetadata::addDeleteTransformers(ObjectInfoPtr object_info, QueryPipelineBuilder &builder,
                                            const std::optional<FormatSettings> &format_settings,
                                            FormatParserSharedResourcesPtr parser_shared_resources, ContextPtr local_context) const {
  auto iceberg_object_info = std::dynamic_pointer_cast<IcebergDataObjectInfo>(object_info);
  if (!iceberg_object_info) return;

  if (!iceberg_object_info->info.position_deletes_objects.empty()) {
    builder.addSimpleTransform([&](const SharedHeader &header) {
      return iceberg_object_info->getPositionDeleteTransformer(object_storage, header, format_settings, parser_shared_resources,
                                                               local_context);
    });
  }
  ...
  for (const EqualityDeleteObject &delete_file : delete_files) {
    auto simple_transform_adder = [&](const SharedHeader &header) {
      ...
      auto delete_format = FormatFactory::instance().getInput(delete_file.file_format, *data_read_buffer, delete_file_header, local_context,
                                                              local_context->getSettingsRef()[DB::Setting::max_block_size], format_settings,
                                                              parser_shared_resources, nullptr, true, compression_method);
      /// only get the delete columns and construct a set by 'block_for_set'
      while (true) {
        Chunk delete_chunk = delete_format->read();
        if (!delete_chunk) break;
      ...
      FutureSetPtr future_set = std::make_shared<FutureSetFromTuple>(CityHash_v1_0_2::uint128(), nullptr,
                                                                     block_for_set.getColumnsWithTypeAndName(), true, size_limits_for_set);
      ...
      for (const auto &output : dag.getOutputs()) outputs.emplace(output->result_name, output);
      ...
      for (Int32 col_id : equality_ids) {
        ...
        if (it == outputs.end()) throw Exception(ErrorCodes::LOGICAL_ERROR, "Cannot find column {} in dag outputs", name_and_type.name);
...
SinkToStoragePtr IcebergMetadata::write(SharedHeader sample_block, const StorageID &table_id, ObjectStoragePtr /*object_storage*/,
                                        StorageObjectStorageConfigurationPtr configuration,
                                        const std::optional<FormatSettings> &format_settings, ContextPtr context,
                                        std::shared_ptr<DataLake::ICatalog> catalog) {
  if (context->getSettingsRef()[Setting::allow_insert_into_iceberg]) {
    return std::make_shared<IcebergStorageSink>(object_storage, configuration, format_settings, sample_block, context, catalog,
                                                persistent_components, table_id);
  } else {
    throw Exception(ErrorCodes::SUPPORT_IS_DISABLED,
...
void IcebergMetadata::drop(ContextPtr context) {
  if (context->getSettingsRef()[Setting::iceberg_delete_data_on_drop].value) {
    ...
    for (const auto &file : files) object_storage->removeObjectIfExists(StoredObject(file));
  }
}
...
ColumnMapperPtr IcebergMetadata::getColumnMapperForObject(ObjectInfoPtr object_info) const {
  ...
  if (Poco::toLower(*object_info->getFileFormat()) != "parquet") return nullptr;
...
// (TODO): usage of this function is wrong and should be removed completely (it was done as a temporary workaround for a bug which occurred
// during supporting concurrent SELECTs).
ColumnMapperPtr IcebergMetadata::getColumnMapperForCurrentSchema(StorageMetadataPtr storage_metadata_snapshot, ContextPtr context) const {
  if (Poco::toLower(write_format) != "parquet") return nullptr;
  auto iceberg_table_state = extractIcebergSnapshotIdFromMetadataObject(storage_metadata_snapshot);
  // This is a temporary cludge for cluster functions because now the state on replicas is not synchronized with the primary state on the
  // coordinator
...
KeyDescription IcebergMetadata::getSortingKey(ContextPtr local_context, TableStateSnapshot actual_table_state_snapshot) const {
  auto metadata_object =
      getMetadataJSONObject(actual_table_state_snapshot.metadata_file_path, object_storage, persistent_components.metadata_cache,
                            local_context, log, persistent_components.metadata_compression_method, persistent_components.table_uuid);

  auto [schema, current_schema_id] = parseTableSchemaV2Method(metadata_object);
  auto result = getSortingKeyDescriptionFromMetadata(
      metadata_object, *persistent_components.schema_processor->getClickhouseTableSchemaById(current_schema_id), local_context);
