...
void traverseComplexType(Poco::JSON::Object::Ptr type, std::unordered_map<String, Int64> &result, const String &current_path) {
  ...
  if (type_str == "map") {
    ...
    if (type->isObject(Iceberg::f_key)) traverseComplexType(type->getObject(Iceberg::f_key), result, key_name);
    result[key_name] = key_id;

    if (type->isObject(Iceberg::f_value)) traverseComplexType(type->getObject(Iceberg::f_value), result, value_name);
    result[value_name] = value_id;
    return;
  }
  if (type_str == "list") {
    auto element_id = type->getValue<Int64>(Iceberg::f_element_id);
    if (type->isObject(Iceberg::f_element)) traverseComplexType(type->getObject(Iceberg::f_element), result, current_path);
  ...
  if (type_str == "struct") {
    ...
    for (UInt32 i = 0; i < fields->size(); ++i) {
      ...
      if (field->isObject(Iceberg::f_type)) traverseComplexType(field->getObject(Iceberg::f_type), result, child_path);
...
bool operator==(const Poco::JSON::Array &first, const Poco::JSON::Array &second) { return equals(first, second); }

bool schemasAreIdentical(const Poco::JSON::Object &first, const Poco::JSON::Object &second) {
  if (!first.isArray(f_fields) || !second.isArray(f_fields)) return false;
...
std::optional<NameAndTypePair> IcebergSchemaProcessor::tryGetFieldCharacteristics(Int32 schema_version, Int32 source_id) const {
  ...
  auto it = clickhouse_types_by_source_ids.find({schema_version, source_id});
  if (it == clickhouse_types_by_source_ids.end()) return {};
  return it->second;
}
...
std::optional<Int32> IcebergSchemaProcessor::tryGetColumnIDByName(Int32 schema_id, const std::string &name) const {
  ...
  if (it == clickhouse_ids_by_source_names.end()) return {};
  return it->second;
}
...
NamesAndTypesList IcebergSchemaProcessor::tryGetFieldsCharacteristics(Int32 schema_id, const std::vector<Int32> &source_ids) const {
  ...
  for (const auto &source_id : source_ids) {
    ...
    if (it != clickhouse_types_by_source_ids.end()) fields.push_back(it->second);
  }
  return fields;
}

DataTypePtr IcebergSchemaProcessor::getSimpleType(const String &type_name) {
  if (type_name == f_boolean) return DataTypeFactory::instance().get("Bool");
  if (type_name == f_int) return std::make_shared<DataTypeInt32>();
  if (type_name == f_long || type_name == f_bigint) return std::make_shared<DataTypeInt64>();
  if (type_name == f_float) return std::make_shared<DataTypeFloat32>();
  if (type_name == f_double) return std::make_shared<DataTypeFloat64>();
  if (type_name == f_date) return std::make_shared<DataTypeDate32>();
  if (type_name == f_time) return std::make_shared<DataTypeInt64>();
  if (type_name == f_timestamp) return std::make_shared<DataTypeDateTime64>(6);
  if (type_name == f_timestamptz) return std::make_shared<DataTypeDateTime64>(6, "UTC");
  if (type_name == f_string || type_name == f_binary) return std::make_shared<DataTypeString>();
  if (type_name == f_uuid) return std::make_shared<DataTypeUUID>();
...
DataTypePtr IcebergSchemaProcessor::getComplexTypeFromObject(const Poco::JSON::Object::Ptr &type, String &current_full_name,
  ...
  if (type_name == f_struct) {
    ...
    for (size_t i = 0; i != fields->size(); ++i) {
      ...
      if (is_subfield_of_root) {
        /// NOTE: getComplexTypeFromObject() with is_subfield_of_root==true called only from addIcebergTableSchema(), which already holds
        /// the exclusive lock So it is OK to use TSA_SUPPRESS_WARNING_FOR_READ/TSA_SUPPRESS_WARNING_FOR_WRITE
...
DataTypePtr IcebergSchemaProcessor::getFieldType(const Poco::JSON::Object::Ptr &field, const String &type_key, bool required,
                                                 String &current_full_name, bool is_subfield_of_root) {
  if (field->isObject(type_key)) return getComplexTypeFromObject(field->getObject(type_key), current_full_name, is_subfield_of_root);
...
std::shared_ptr<ActionsDAG> IcebergSchemaProcessor::getSchemaTransformationDag(const Poco::JSON::Object::Ptr &old_schema,
                                                                               const Poco::JSON::Object::Ptr &new_schema, Int32 old_id,
                                                                               Int32 new_id) {
  ...
  for (size_t i = 0; i != new_schema_fields->size(); ++i) {
    ...
    if (old_node_it != old_schema_entries.end()) {
      ...
      if (field->isObject(f_type) && (field->getObject(f_type)->getValue<std::string>(f_type) == "struct" ||
        ...
        if (old_json->isObject(f_type) && !field->isObject(f_type)) {
          throw Exception(ErrorCodes::LOGICAL_ERROR,
                          "Can't cast primitive type to the complex type, field id is {}, old schema id is {}, new schema id is {}", id,
                          old_id, new_id);
        }
...
std::shared_ptr<const ActionsDAG> IcebergSchemaProcessor::getSchemaTransformationDagByIds(Int32 old_id, Int32 new_id) {
  if (old_id == new_id) return nullptr;

  std::lock_guard lock(mutex);
  auto required_transform_dag_it = transform_dags_by_ids.find({old_id, new_id});
  if (required_transform_dag_it != transform_dags_by_ids.end()) return required_transform_dag_it->second;
...
Poco::JSON::Object::Ptr IcebergSchemaProcessor::getIcebergTableSchemaById(Int32 id) const {
  ...
  if (it == iceberg_table_schemas_by_ids.end()) throw Exception(ErrorCodes::BAD_ARGUMENTS, "Schema with id {} is unknown", id);
  return it->second;
}
...
void IcebergSchemaProcessor::registerSnapshotWithSchemaId(Int64 snapshot_id, Int32 schema_id) {
  ...
  if (it != schema_id_by_snapshot.end()) {
    ...
    if (old_id != schema_id) {
      throw Exception(ErrorCodes::LOGICAL_ERROR,
                      "Snapshot with id {} already registered with schema id {}, trying to register with new schema id {}", snapshot_id,
                      old_id, schema_id);
    }
  } else {
...
std::optional<Int32> IcebergSchemaProcessor::tryGetSchemaIdForSnapshot(Int64 snapshot_id) const {
  SharedLockGuard lock(mutex);
  auto it = schema_id_by_snapshot.find(snapshot_id);
  if (it != schema_id_by_snapshot.end()) return it->second;
  return std::nullopt;
}
...
std::shared_ptr<NamesAndTypesList> IcebergSchemaProcessor::getClickhouseTableSchemaById(Int32 id) {
  ...
  if (it == clickhouse_table_schemas_by_ids.end()) throw Exception(ErrorCodes::BAD_ARGUMENTS, "Schema with id {} is unknown", id);
  return it->second;
}
...
std::unordered_map<String, Int64> IcebergSchemaProcessor::traverseSchema(Poco::JSON::Array::Ptr schema) {
  ...
  for (UInt32 i = 0; i < schema->size(); ++i) {
    ...
    if (current_object->isObject(Iceberg::f_type)) traverseComplexType(current_object->getObject(Iceberg::f_type), result, cur_name);
