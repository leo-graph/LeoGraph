...
DB::ASTPtr getASTFromTransform(const String& transform_name_src, const String& column_name) {
  ...
  if (transform_name == "identity") return make_intrusive<ASTIdentifier>(column_name);

  if (transform_name == "void") return makeASTOperator("tuple");

  if (transform_and_argument->argument.has_value()) {
    return makeASTFunction(transform_and_argument->transform_name, make_intrusive<ASTLiteral>(*transform_and_argument->argument),
...
std::unique_ptr<DB::ActionsDAG> ManifestFilesPruner::transformFilterDagForManifest(const DB::ActionsDAG* source_dag,
                                                                                   std::vector<Int32>& used_columns_in_filter) const {
  ...
  for (const auto& input : inputs) {
    if (input->type == ActionsDAG::ActionType::INPUT) {
      ...
      if (input_id) used_columns_in_filter.push_back(*input_id);
    }
  }
  ...
  for (const auto column_id : used_columns_in_filter) {
    ...
    if (!column_from_manifest.has_value()) continue;
...
ManifestFilesPruner::ManifestFilesPruner(const IcebergSchemaProcessor& schema_processor_, Int32 current_schema_id_,
                                         Int32 initial_schema_id_, const DB::ActionsDAG* filter_dag,
                                         const ManifestFileIterator& manifest_file, DB::ContextPtr context)
    : schema_processor(schema_processor_), current_schema_id(current_schema_id_), initial_schema_id(initial_schema_id_) {
  ...
  for (Int32 used_column_id : used_columns_in_filter) {
    ...
    if (!name_and_type.has_value()) continue;
...
PruningReturnStatus ManifestFilesPruner::canBePruned(const ProcessedManifestFileEntryPtr& entry,
                                                     const std::unordered_map<Int32, DB::Range>& entry_hyperrectangles) const {
  if (partition_key_condition.has_value()) {
    ...
    bool can_be_true = partition_key_condition->mayBeTrueInRange(partition_value.size(), index_value.data(), index_value.data(),
  ...
  for (const auto& [column_id, key_condition] : min_max_key_conditions) {
    ...
    if (rect_it == entry_hyperrectangles.end()) continue;

    auto info_it = entry->parsed_entry->columns_infos.find(column_id);
    bool has_no_nulls =
        info_it != entry->parsed_entry->columns_infos.end() && info_it->second.nulls_count.has_value() && *info_it->second.nulls_count == 0;
