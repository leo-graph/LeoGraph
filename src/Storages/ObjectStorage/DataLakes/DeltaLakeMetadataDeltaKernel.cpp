...
void tracingCallback(struct ffi::Event event) {
  ...
  if (event.message.len > 200) return;

  const auto message =
      fmt::format("Message: {}, code line: {}:{}, target: {}", std::string_view(event.message.ptr, event.message.len),
                  DeltaLake::KernelUtils::fromDeltaString(event.file), event.line, DeltaLake::KernelUtils::fromDeltaString(event.target));
...
std::optional<size_t> extractDeltaLakeSnapshotVersionFromMetadata(StorageMetadataPtr storage_metadata) {
  if (!storage_metadata || !storage_metadata->datalake_table_state.has_value()) return std::nullopt;

  if (!std::holds_alternative<DeltaLake::TableStateSnapshot>(storage_metadata->datalake_table_state.value())) return std::nullopt;
...
DeltaLakeMetadataDeltaKernel::DeltaLakeMetadataDeltaKernel(ObjectStoragePtr object_storage_,
    ...
    : log(getLogger("DeltaLakeMetadata")),
      ...
      ,
      snapshots(CurrentMetrics::end(), CurrentMetrics::DeltaLakeSnapshotCacheSizeElements,
                /* max_size_in_bytes */ 10,
                /* max_count */ 10) {}

static std::optional<DeltaLakeMetadataDeltaKernel::SnapshotVersion> getSnapshotVersion(const Settings &settings) {
  const auto &value = settings[Setting::delta_lake_snapshot_version].value;
  if (value >= 0) return static_cast<UInt64>(value);

  if (value == DeltaLake::TableSnapshot::LATEST_SNAPSHOT_VERSION) return std::nullopt;
...
DeltaLake::TableSnapshotPtr DeltaLakeMetadataDeltaKernel::getTableSnapshot(std::optional<SnapshotVersion> version) const {
  ...
  std::optional<SnapshotVersion> result_snapshot_version = version.has_value() ? version : latest_snapshot_version;

  auto snapshot_creator = [&]() {
    /// Constructor itself is lightweight.
    return std::make_shared<DeltaLake::TableSnapshot>(result_snapshot_version, kernel_helper, object_storage, log);
  };

  DeltaLake::TableSnapshotPtr snapshot;
  bool created = true;
  if (result_snapshot_version.has_value()) {
    std::tie(snapshot, created) = snapshots.getOrSet(result_snapshot_version.value(), std::move(snapshot_creator));
  } else {
  ...
  LOG_TEST(log, "Using snapshot version: {}, latest loaded snapshot version: {}, reused cached snapshot: {}",
           result_snapshot_version.has_value() ? result_snapshot_version.value() : snapshot->getVersion(), latestSnapshotVersionToStr(),
           !created);
...
std::optional<size_t> DeltaLakeMetadataDeltaKernel::totalRows(ContextPtr context) const {
  ...
  try {
    ...
    DB::tryLogCurrentException(log, "Failed to get total rows for Delta Lake table at location " + kernel_helper->getTableLocation());
    return std::nullopt;
  }
}
...
std::optional<size_t> DeltaLakeMetadataDeltaKernel::totalBytes(ContextPtr context) const {
  ...
  try {
    ...
    DB::tryLogCurrentException(log, "Failed to get total bytes for Delta Lake table at location " + kernel_helper->getTableLocation());
    return std::nullopt;
  }
}
...
void DeltaLakeMetadataDeltaKernel::update(const ContextPtr &context) {
  ...
  if (!snapshot_version.has_value()) {
    ...
    LOG_TEST(log, "Updating latest snapshot version from {} to {}", latestSnapshotVersionToStr(), version);

    latest_snapshot_version = version;
  }
}

DeltaLake::TableChangesPtr DeltaLakeMetadataDeltaKernel::getTableChanges(const DeltaLake::TableChangesVersionRange &version_range,
                                                                         const Block &header,
...
ObjectIterator DeltaLakeMetadataDeltaKernel::iterate(const ActionsDAG *filter_dag, FileProgressCallback callback, size_t list_batch_size,
                                                     StorageMetadataPtr storage_metadata_snapshot, ContextPtr context) const {
...
NamesAndTypesList DeltaLakeMetadataDeltaKernel::getTableSchema(ContextPtr local_context) const {
  ...
  if (auto start_version = settings[Setting::delta_lake_snapshot_start_version].value;
      start_version != DeltaLake::TableSnapshot::LATEST_SNAPSHOT_VERSION) {
    auto version_range = DeltaLake::TableChanges::getVersionRange(start_version, settings[Setting::delta_lake_snapshot_end_version].value);
    ...
    return getTableChanges(version_range,
                           /* source_header */ {},
                           /* format_settings */ {}, local_context)
        ->getSchema();
  }
...
static std::pair<Names, NamesAndTypesList> splitVirtualColumns(const Names &columns, VirtualsDescriptionPtr virtual_columns_description) {
...
static DataTypePtr replaceTypeNamesToPhysicalRecursively(const DataTypePtr &type, const std::string &parent_logical_name,
                                                         const std::string &parent_physical_name, const NameToNameMap &physical_names_map) {
  const auto *tuple_type = typeid_cast<const DataTypeTuple *>(type.get());
  if (!tuple_type || !tuple_type->hasExplicitNames()) return type;
  ...
  for (size_t i = 0; i < element_names.size(); ++i) {
    ...
    if (physical_name) {
      ...
      full_child_physical_path = parent_physical_name.empty() ? element_name : parent_physical_name + "." + element_name;
...
static std::pair<NameAndTypePair, bool> getPhysicalNameAndType(const NameAndTypePair &column, const NamesAndTypesList &read_schema,
                                                               const NameToNameMap &physical_names_map, LoggerPtr log) {
  auto physical_name_in_storage = DeltaLake::getPhysicalName(column.getNameInStorage(), physical_names_map);
  auto physical_type_in_storage = replaceTypeNamesToPhysicalRecursively(column.getTypeInStorage(), column.getNameInStorage(),
                                                                        physical_name_in_storage, physical_names_map);
  ...
  LOG_TEST(log,
           "Name: {}, name in storage: {}, subcolumn name: {}, "
           "physical name in storage: {}, physical type in storage: {}, read schema: {}",
           column.name, column.getNameInStorage(), column.getSubcolumnName(), physical_name_in_storage, physical_type_in_storage,
           read_schema_column.has_value() ? read_schema_column->dump() : "None");
  ...
  if (column.isSubcolumn()) {
    result_column = NameAndTypePair(physical_name_in_storage, column.getSubcolumnName(), physical_type_in_storage, column.type);
  } else {
...
ReadFromFormatInfo DeltaLakeMetadataDeltaKernel::prepareReadingFromFormat(const Strings &requested_columns,
                                                                          const StorageSnapshotPtr &storage_snapshot,
                                                                          const ContextPtr &context, bool supports_subset_of_columns,
  ...
  if (auto version_from_metadata = extractDeltaLakeSnapshotVersionFromMetadata(storage_snapshot->metadata)) {
    ...
    LOG_TEST(log, "Using snapshot version {} from settings (no version in metadata)",
  ...
  std::tie(columns_to_read, info.requested_virtual_columns) = splitVirtualColumns(requested_columns, storage_snapshot->virtual_columns);
  ...
  for (auto &name_and_type : info.requested_columns) {
    auto [result_name_and_type, _] = getPhysicalNameAndType(name_and_type, readable_columns_with_subcolumns, physical_names_map, log);
  ...
  if (supports_subset_of_columns) {
    ...
    if (columns_to_read.empty()) {
      ...
      for (const auto &name_and_type : table_columns)
        if (!partition_columns.contains(name_and_type.name)) non_partition_columns.push_back(name_and_type);
  ...
  for (auto &name_and_type : columns_to_read_desc) {
    auto [result_name_and_type, readable] =
        getPhysicalNameAndType(name_and_type, readable_columns_with_subcolumns, physical_names_map, log);

    if (readable) info.format_header.insert(ColumnWithTypeAndName{result_name_and_type.type, result_name_and_type.name});
  }

  for (const auto &name_and_type : info.columns_description)
    info.columns_description.rename(name_and_type.name, DeltaLake::getPhysicalName(name_and_type.name, physical_names_map));
...
SinkToStoragePtr DeltaLakeMetadataDeltaKernel::write(SharedHeader sample_block, const StorageID & /* table_id */,
                                                     ObjectStoragePtr object_storage_, StorageObjectStorageConfigurationPtr configuration,
  ...
  if (!context->getSettingsRef()[Setting::allow_experimental_delta_lake_writes]) {
    throw Exception(ErrorCodes::SUPPORT_IS_DISABLED, "To enable delta lake writes, use allow_experimental_delta_lake_writes = 1");
  }
  ...
  if (partition_columns.empty()) {
    return std::make_shared<DeltaLakeSink>(delta_transaction, object_storage_, context, sample_block, format_settings,
                                           configuration->format, configuration->compression_method);
  }

  return std::make_shared<DeltaLakePartitionedSink>(delta_transaction, partition_columns, object_storage_, context, sample_block,
                                                    format_settings, configuration->format, configuration->compression_method);
}

void DeltaLakeMetadataDeltaKernel::logMetadataFiles(ContextPtr context) const {
  if (!context->getSettingsRef()[Setting::delta_lake_log_metadata].value) return;
