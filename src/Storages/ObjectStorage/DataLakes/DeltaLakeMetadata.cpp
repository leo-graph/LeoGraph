...
struct DeltaLakeMetadataImpl {
  ...
  DeltaLakeMetadataImpl(ObjectStoragePtr object_storage_, StorageObjectStorageConfigurationWeakPtr configuration_, ContextPtr context_)
      : object_storage(object_storage_), table_path(configuration_.lock()->getPathForRead().path), context(context_) {}
  ...
  DeltaLakeMetadata processMetadataFiles() const {
    ...
    if (checkpoint_version) {
      ...
      while (true) {
        ...
        if (!object_storage->exists(StoredObject(file_path))) break;

        processMetadataFile(file_path, current_schema, current_partition_columns, result_files);
      }

      LOG_TRACE(log, "Processed metadata files from checkpoint {} to {}", checkpoint_version, current_version - 1);
    } else {
      const auto keys = listFiles(*object_storage, table_path, deltalake_metadata_directory, metadata_file_suffix);
      for (const String &key : keys) processMetadataFile(key, current_schema, current_partition_columns, result_files);
    }
  ...
  void processMetadataFile(const String &metadata_file_path, NamesAndTypesList &file_schema,
                           DeltaLakePartitionColumns &file_partition_columns, std::set<String> &result) const {
    ...
    while (!buf->eof()) {
      ...
      if (!object) throw Exception(ErrorCodes::LOGICAL_ERROR, "Failed to parse metadata file");
...
#  endif
      ...
      if (object->has("metaData")) {
        ...
        if (!metadata_object) throw Exception(ErrorCodes::LOGICAL_ERROR, "Failed to extract `metaData` field");
        ...
        if (!fields_object) throw Exception(ErrorCodes::LOGICAL_ERROR, "Failed to extract `fields` field");
      ...
      if (object->has("add")) {
        ...
        if (!add_object) throw Exception(ErrorCodes::LOGICAL_ERROR, "Failed to extract `add` field");
        ...
        if (it == file_partition_columns.end()) {
          if (add_object->has("partitionValues")) {
            ...
            if (!partition_values) throw Exception(ErrorCodes::LOGICAL_ERROR, "Failed to extract `partitionValues` field");
            ...
            if (partition_values->size()) {
              ...
              for (const auto &partition_name : partition_values->getNames()) {
                ...
                if (!name_and_type) {
                  throw Exception(ErrorCodes::LOGICAL_ERROR, "No such column in schema: {} (schema: {})", partition_name,
                                  file_schema.toNamesAndTypesDescription());
                }

                LOG_TEST(log, "Partition {} value is {} (data type: {}, file: {})", partition_name, value, name_and_type->type->getName(),
                         filename);
        ...
        if (!remove_object) throw Exception(ErrorCodes::LOGICAL_ERROR, "Failed to extract `remove` field");
  ...
  NamesAndTypesList parseMetadata(const Poco::JSON::Object::Ptr &metadata_json) const {
    ...
    for (size_t i = 0; i < fields->size(); ++i) {
      ...
      LOG_TEST(log, "Found column: {}, type: {}, nullable: {}, physical name: {}", column_name, type, is_nullable, physical_name);
  ...
  size_t readLastCheckpointIfExists() const {
    ...
    if (!object_storage->exists(StoredObject(last_checkpoint_file))) return 0;
 ...
 *  txn:      (NULL,NULL,NULL)
 *  add:
 * ('part-00000-8887e898-91dd-4951-a367-48f7eb7bd5fd-c000.parquet',{},1063,1680614318485,false,{},'{"numRecords":1,"minValues":{"col-360dade5-6d0e-4831-8467-a25d64695975":2,"col-e27b0253-569a-4fe1-8f02-f3342c54d08b":"3"},"maxValues":{"col-360dade5-6d0e-4831-8467-a25d64695975":2,"col-e27b0253-569a-4fe1-8f02-f3342c54d08b":"3"},"nullCount":{"col-360dade5-6d0e-4831-8467-a25d64695975":0,"col-e27b0253-569a-4fe1-8f02-f3342c54d08b":0}}')
 *  remove:   (NULL,NULL,NULL,NULL,{},NULL,{})
 *  metaData: (NULL,NULL,NULL,(NULL,{}),NULL,[],{},NULL)
 *  protocol: (NULL,NULL)
 *
...
#  define THROW_ARROW_NOT_OK(status)                                                                                                    \
    do {                                                                                                                                \
      if (const ::arrow::Status &_s = (status); !_s.ok()) throw Exception(ErrorCodes::BAD_ARGUMENTS, "Arrow error: {}", _s.ToString()); \
    } while (false)

  size_t getCheckpointIfExists(std::set<String> &result, NamesAndTypesList &file_schema,
    ...
    auto columns = format_settings.parquet.use_native_reader_v3 ? NativeParquetSchemaReader(*buf, format_settings).readSchema()
    ...
    Block header;
    for (const auto &column : columns) header.insert({column.type->createColumn(), column.type, column.name});

    std::atomic<int> is_stopped{0};

    auto open_file_res = parquet::arrow::OpenFile(asArrowFile(*buf, format_settings, is_stopped, "Parquet", PARQUET_MAGIC_BYTES),
                                                  arrow::default_memory_pool());
    THROW_ARROW_NOT_OK(open_file_res.status());
    auto reader = *std::move(open_file_res);

    ArrowColumnToCHColumn column_reader(
        header, "Parquet", format_settings, std::nullopt, std::nullopt, format_settings.parquet.allow_missing_columns,
        /* null_as_default */ true, format_settings.date_time_overflow_behavior, format_settings.parquet.allow_geoparquet_parser,
    ...
    Chunk chunk = column_reader.arrowTableToCHChunk(table, reader->parquet_reader()->metadata()->num_rows(),
                                                    reader->parquet_reader()->metadata()->key_value_metadata());
    ...
    for (size_t i = 0; i < path_column.size(); ++i) {
      ...
      if (!inserted) throw Exception(ErrorCodes::INCORRECT_DATA, "File already exists {}", path);
    }
...
DeltaLakeMetadata::DeltaLakeMetadata(ObjectStoragePtr object_storage_, StorageObjectStorageConfigurationWeakPtr configuration_,
                                     ContextPtr context_) {
  ...
  LOG_TRACE(impl.log, "Found {} data files, {} partition files, schema: {}", data_files.size(), partition_columns.size(),
            schema.toString());
}
...
DataLakeMetadataPtr DeltaLakeMetadata::create(ObjectStoragePtr object_storage, StorageObjectStorageConfigurationWeakPtr configuration,
...
DataTypePtr DeltaLakeMetadata::getFieldType(const Poco::JSON::Object::Ptr &field, const String &type_key, bool is_nullable) {
  if (field->isObject(type_key)) return getComplexTypeFromObject(field->getObject(type_key));
...
DataTypePtr DeltaLakeMetadata::getSimpleTypeByName(const String &type_name) {
  ...
  if (type_name == "string" || type_name == "binary") return std::make_shared<DataTypeString>();
  if (type_name == "long") return std::make_shared<DataTypeInt64>();
  if (type_name == "integer") return std::make_shared<DataTypeInt32>();
  if (type_name == "short") return std::make_shared<DataTypeInt16>();
  if (type_name == "byte") return std::make_shared<DataTypeInt8>();
  if (type_name == "float") return std::make_shared<DataTypeFloat32>();
  if (type_name == "double") return std::make_shared<DataTypeFloat64>();
  if (type_name == "boolean") return DataTypeFactory::instance().get("Bool");
  if (type_name == "date") return std::make_shared<DataTypeDate32>();
  if (type_name == "timestamp" || type_name == "timestamp_ntz") return std::make_shared<DataTypeDateTime64>(6);
...
Field DeltaLakeMetadata::getFieldValue(const String &value, DataTypePtr data_type) {
  ...
  if (which.isStringOrFixedString()) return value;
  if (isBool(check_type)) return parse<bool>(value);
  if (which.isInt8()) return parse<Int8>(value);
  if (which.isUInt8()) return parse<UInt8>(value);
  if (which.isInt16()) return parse<Int16>(value);
  if (which.isUInt16()) return parse<UInt16>(value);
  if (which.isInt32()) return parse<Int32>(value);
  if (which.isUInt32()) return parse<UInt32>(value);
  if (which.isInt64()) return parse<Int64>(value);
  if (which.isUInt64()) return parse<UInt64>(value);
  if (which.isFloat32()) return parse<Float32>(value);
  if (which.isFloat64()) return parse<Float64>(value);
  if (which.isDate()) return UInt16{LocalDate{std::string(value)}.getDayNum()};
  if (which.isDate32()) return Int32{LocalDate{std::string(value)}.getExtenedDayNum()};
...
ObjectIterator DeltaLakeMetadata::iterate(const ActionsDAG *filter_dag, FileProgressCallback callback, size_t /* list_batch_size */,
