...
DeletionVectorTransform::DeletionVectorTransform(const DB::SharedHeader& header_, ExcludedRowsPtr excluded_rows_)
    : ISimpleTransform(header_, header_, /* skip_empty_chunks */ false), excluded_rows(excluded_rows_) {}

void DeletionVectorTransform::transform(DB::Chunk& chunk) { transform(chunk, *excluded_rows); }
...
void DeletionVectorTransform::transform(DB::Chunk& chunk, const ExcludedRows& excluded_rows) {
  ...
  if (!chunk_info) throw DB::Exception(DB::ErrorCodes::LOGICAL_ERROR, "ChunkInfoRowNumbers does not exist");
  ...
  for (size_t i = 0; i < num_indices; i++) {
    if (!applied_filter.has_value() || applied_filter.value()[i]) {
      if (excluded_rows.rb_contains(chunk_info->row_num_offset + i)) {
        ...
        if (applied_filter.has_value()) applied_filter.value()[i] = false;
      }
      idx_in_chunk += 1;
    }
  }
  ...
  if (num_rows_after == num_rows_before) return;
  ...
  if (!applied_filter.has_value()) applied_filter.emplace(std::move(filter));
