...
Strings HudiMetadata::getDataFilesImpl() const {
  ...
  for (const auto &key : keys) {
    ...
    if (file_parts.size() != 3) throw Exception(ErrorCodes::LOGICAL_ERROR, "Unexpected format for file: {}", key);
  ...
  for (auto &[partition, partition_data] : files) {
    ...
    for (auto &[file_id, file_data] : partition_data) result.push_back(std::move(file_data.key));
  }
  return result;
}
...
Strings HudiMetadata::getDataFiles(const ActionsDAG *) const {
  if (data_files.empty()) data_files = getDataFilesImpl();
  return data_files;
}

ObjectIterator HudiMetadata::iterate(const ActionsDAG *filter_dag, FileProgressCallback callback, size_t /* list_batch_size */,
