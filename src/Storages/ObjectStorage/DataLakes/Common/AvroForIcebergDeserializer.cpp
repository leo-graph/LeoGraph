...
AvroForIcebergDeserializer::AvroForIcebergDeserializer(std::unique_ptr<ReadBufferFromFileBase> buffer_,
                                                       const std::string& manifest_file_path_,
                                                       const DB::FormatSettings& format_settings) try
    : buffer(std::move(buffer_)), manifest_file_path(manifest_file_path_) {
...
size_t AvroForIcebergDeserializer::rows() const { return parsed_column->size(); }

bool AvroForIcebergDeserializer::hasPath(const std::string& path) const { return extractSubcolumnWithType(path).has_value(); }
...
Int64 AvroForIcebergDeserializer::getFormatVersionFromManifestFileMetadata() const {
  ...
  if (!format_version_value.has_value()) return 1;
  try {
    return std::stoi(format_version_value.value());
  } catch (const std::exception& e) {
    throw Exception(ErrorCodes::ICEBERG_SPECIFICATION_VIOLATION,
                    "Cannot read iceberg table format version from Iceberg avro manifest file '{}': {}", manifest_file_path, e.what());
  }
}
...
ParsedManifestFileEntryPtr AvroForIcebergDeserializer::createParsedManifestFileEntry(size_t row_index) const {
  ...
  for (const auto& value : tuple) partition_key_value.emplace_back(value);
  ...
  switch (content_type) {
    case FileContentType::DATA: {
      return std::make_shared<const ParsedManifestFileEntry>(FileContentType::DATA, file_path_key, row_index, status, sequence_number,
                                                             snapshot_id, partition_key_value, columns_infos, value_for_bounds, file_format,
                                                             /*lower_reference_data_file_path_ = */ std::nullopt,
                                                             /*upper_reference_data_file_path_ = */ std::nullopt,
                                                             /*equality_ids*/ std::nullopt, sort_order_id);
    }
    case FileContentType::POSITION_DELETE: {
      /// reference_file_path can be absent in schema for some reason, though it is present in specification:
      ...
      if (!bounds_set_by_referenced_data_file) {
        if (auto it = value_for_bounds.find(IcebergPositionDeleteTransform::data_file_path_column_field_id); it != value_for_bounds.end()) {
          auto& [lower, upper] = it->second;
          if (!lower.isNull()) lower_reference_data_file_path = lower.safeGet<String>();
          if (!upper.isNull()) upper_reference_data_file_path = upper.safeGet<String>();
        }
      }
      return std::make_shared<const ParsedManifestFileEntry>(
          FileContentType::POSITION_DELETE, file_path_key, row_index, status, sequence_number, snapshot_id, partition_key_value,
          columns_infos, value_for_bounds, file_format, lower_reference_data_file_path, upper_reference_data_file_path,
    ...
    case FileContentType::EQUALITY_DELETE: {
      ...
      if (hasPath(c_data_file_equality_ids)) {
        ...
        for (const Field& id : equality_ids_field.safeGet<Array>()) equality_ids.push_back(static_cast<Int32>(id.safeGet<Int32>()));
      } else
        throw Exception(DB::ErrorCodes::ICEBERG_SPECIFICATION_VIOLATION, "Couldn't find field {} in equality delete file entry",
                        c_data_file_equality_ids);
      return std::make_shared<const ParsedManifestFileEntry>(FileContentType::EQUALITY_DELETE, file_path_key, row_index, status,
                                                             sequence_number, snapshot_id, partition_key_value, columns_infos,
                                                             value_for_bounds, file_format,
                                                             /*lower_reference_data_file_path_ = */ std::nullopt,
                                                             /*upper_reference_data_file_path_ = */ std::nullopt, equality_ids,
                                                             /*sort_order_id = */ std::nullopt);
    }
  }
}
...
ParsedManifestFileEntryPtr AvroForIcebergDeserializer::getParsedManifestFileEntry(size_t row_index) const {
  ...
  if (!parsed_manifest_file_entries[row_index]) {
    ...
    if (!parsed_manifest_file_entries[row_index]) parsed_manifest_file_entries[row_index] = std::move(entry);
...
std::optional<std::pair<ColumnPtr, DataTypePtr>>& AvroForIcebergDeserializer::extractSubcolumnWithType(const std::string& path) const {
  // First, try to read from cache with shared lock (for concurrent reads)
  {
    ...
    if (it != cache_extracted_subcolumns_with_types.end()) return it->second;
  }
...
Field AvroForIcebergDeserializer::getValueFromRowByName(size_t row_num, const std::string& path,
                                                        std::optional<TypeIndex> expected_type) const {
  auto& subcolumn_with_type = extractSubcolumnWithType(path);
  if (!subcolumn_with_type.has_value())
    throw Exception(ErrorCodes::ICEBERG_SPECIFICATION_VIOLATION, "Cannot find column {} in manifest file {}", path, manifest_file_path);
  ...
  if (expected_type && WhichDataType(current_data_type).idx != *expected_type)
    throw Exception(ErrorCodes::ICEBERG_SPECIFICATION_VIOLATION, "Got wrong data type for key {} in manifest file {}, expected {}, got {}",
...
String AvroForIcebergDeserializer::getContent(size_t row_number) const {
  // First check with shared lock if cache is already initialized
  {
    ...
    if (cache_parsed_columns) return formatRowAsJSON(cache_parsed_columns.value(), row_number);
  }
