...
void DataLakeStorageSettings::loadFromQuery(ASTSetQuery& settings_ast) { impl->applyChanges(settings_ast.changes); }

Field DataLakeStorageSettings::get(const std::string& name) { return impl->get(name); }

bool DataLakeStorageSettings::hasBuiltin(std::string_view name) { return DataLakeStorageSettingsImpl::hasBuiltin(name); }

void DataLakeStorageSettings::loadFromSettingsChanges(const SettingsChanges& changes) {
  for (const auto& [name, value] : changes) {
    if (impl->has(name)) impl->set(name, value);
  }
}

void DataLakeStorageSettings::serialize(WriteBuffer& out) const { impl->writeChangedBinary(out); }
