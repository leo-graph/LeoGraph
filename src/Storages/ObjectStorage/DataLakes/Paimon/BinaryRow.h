    ...
    if (precision <= 18) {
      ...
      auto add_leading_zero = [](const String& data, size_t target_size) {
        if (data.size() == target_size)
          ...
          throw Exception(ErrorCodes::BAD_ARGUMENTS, "data size larger than target_size, data size: {}, target_size: {}.", data.size(),
                          target_size);
        String result(target_size, 0);
        size_t start_pos = target_size - data.size();
        for (size_t i = 0; i < data.size(); ++i) result[start_pos + i] = data[i];
        return result;
      };
      ...
      if (precision <= 38) {
        if (bytes_string.length() > 16)
          throw Exception(ErrorCodes::BAD_ARGUMENTS, "Get unexpected decimal bytes length: {}, expected <= 16", bytes_string.length());
        ...
        if (bytes_string.length() > 32)
          throw Exception(ErrorCodes::BAD_ARGUMENTS, "Get unexpected decimal bytes length: {}, expected <= 32", bytes_string.length());
