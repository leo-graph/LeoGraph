...
DataLakeMetadataPtr PaimonMetadata::create(const ObjectStoragePtr &object_storage,
                                           const StorageObjectStorageConfigurationWeakPtr &configuration, const ContextPtr &local_context) {
  auto configuration_ptr = configuration.lock();
  LOG_TEST(&Poco::Logger::get("PaimonMetadata"), "path: {} raw path: {}", configuration_ptr->getPathForRead().path,
...
void PaimonMetadata::updateState() {
  ...
  auto get_or_default = [](const std::string &key, const std::string &default_value,
                           std::unordered_map<String, String> &options) -> std::string {
  ...
  for (const auto &manifest_meta : base_manifest_list) {
    base_manifest.emplace_back(
        table_client_ptr->getDataManifest(manifest_meta.file_name, *table_schema,
  ...
  for (const auto &manifest_meta : delta_manifest_list) {
    delta_manifest.emplace_back(
        table_client_ptr->getDataManifest(manifest_meta.file_name, *table_schema,
                                          get_or_default(PAIMON_DEFAULT_PARTITION_NAME, PARTITION_DEFAULT_VALUE, table_schema->options)));
  }
}

PaimonMetadata::PaimonMetadata(ObjectStoragePtr object_storage_, StorageObjectStorageConfigurationPtr configuration_,
                               const DB::ContextPtr &context_, const Poco::JSON::Object::Ptr &schema_json_object_,
...
NamesAndTypesList PaimonMetadata::getTableSchema(ContextPtr /*local_context*/) const {
  ...
  if (!table_schema.has_value()) return names_types_list;
...
void PaimonMetadata::update(const ContextPtr &) { updateState(); }

ObjectIterator PaimonMetadata::iterate(const ActionsDAG *filter_dag_, FileProgressCallback callback_, size_t /* list_batch_size */,
                                       StorageMetadataPtr /*storage_metadata*/, ContextPtr context_) const {
  SharedLockGuard shared_lock(mutex);
  Strings data_files;
