...
struct PaimonSnapshot {
  ...
  bool operator==(const PaimonSnapshot& other) const { return version == other.version && id == other.id && schema_id == other.schema_id; }
...
struct PaimonManifestFileMeta {
  ...
  PaimonManifestFileMeta(const Iceberg::AvroForIcebergDeserializer& avro_deserializer, const String& root_path, const size_t row_num)
    ...
    file_name =
        avro_deserializer.getValueFromRowByName(row_num, concatPath({root_path, COLUMN_PAIMON_MANIFEST_LIST_FILE_NAME}), TypeIndex::String)
            .safeGet<std::string>();
    file_size =
        avro_deserializer.getValueFromRowByName(row_num, concatPath({root_path, COLUMN_PAIMON_MANIFEST_LIST_FILE_SIZE}), TypeIndex::Int64)
            .safeGet<Int64>();
    num_added_files =
        avro_deserializer
...
struct PaimonManifestEntry {
  ...
  struct DataFileMeta {
   private:
    template <typename T>
    void getNullableValueFromRowByName(std::optional<T>& res, const Iceberg::AvroForIcebergDeserializer& avro_deserializer,
                                       const size_t row_num, const String& path) {
      auto field = avro_deserializer.getValueFromRowByName(row_num, path);
      if (!field.isNull()) res = field.safeGet<T>();
    }
   ...
   public:
    ...
    DataFileMeta(const Iceberg::AvroForIcebergDeserializer& avro_deserializer, const String& root_path, const size_t row_num,
                 const String& partition_, Int32 bucket_, const PaimonTableSchema& table_schema, const String& partition_default_name)
      ...
      file_size =
          avro_deserializer.getValueFromRowByName(row_num, concatPath({root_path, COLUMN_PAIMON_MANIFEST_FILE_FILE_SIZE}), TypeIndex::Int64)
              .safeGet<Int64>();
      row_count =
          avro_deserializer.getValueFromRowByName(row_num, concatPath({root_path, COLUMN_PAIMON_MANIFEST_FILE_ROW_COUNT}), TypeIndex::Int64)
              .safeGet<Int64>();
      min_key =
          avro_deserializer.getValueFromRowByName(row_num, concatPath({root_path, COLUMN_PAIMON_MANIFEST_FILE_MIN_KEY}), TypeIndex::String)
              .safeGet<String>();
      max_key =
          avro_deserializer.getValueFromRowByName(row_num, concatPath({root_path, COLUMN_PAIMON_MANIFEST_FILE_MAX_KEY}), TypeIndex::String)
              .safeGet<String>();
      min_sequence_number =
          avro_deserializer
              .getValueFromRowByName(row_num, concatPath({root_path, COLUMN_PAIMON_MANIFEST_FILE_MIN_SEQUENCE_NUMBER}), TypeIndex::Int64)
              .safeGet<Int64>();
      max_sequence_number =
          avro_deserializer
              .getValueFromRowByName(row_num, concatPath({root_path, COLUMN_PAIMON_MANIFEST_FILE_MAX_SEQUENCE_NUMBER}), TypeIndex::Int64)
              .safeGet<Int64>();
      schema_id =
          avro_deserializer.getValueFromRowByName(row_num, concatPath({root_path, COLUMN_PAIMON_MANIFEST_FILE_SCHEMA_ID}), TypeIndex::Int64)
              .safeGet<Int64>();
      level = static_cast<Int32>(
          avro_deserializer.getValueFromRowByName(row_num, concatPath({root_path, COLUMN_PAIMON_MANIFEST_FILE_LEVEL}), TypeIndex::Int32)
              .safeGet<Int32>());
      extra_files =
          avro_deserializer.getValueFromRowByName(row_num, concatPath({root_path, COLUMN_PAIMON_MANIFEST_EXTRA_FILES}), TypeIndex::Array)
              .safeGet<Array>();

      getNullableValueFromRowByName(creation_time, avro_deserializer, row_num,
                                    concatPath({root_path, COLUMN_PAIMON_MANIFEST_CREATION_TIME}));
      getNullableValueFromRowByName(delete_row_count, avro_deserializer, row_num,
                                    concatPath({root_path, COLUMN_PAIMON_MANIFEST_DELETE_ROW_COUNT}));
      getNullableValueFromRowByName(embedded_file_index, avro_deserializer, row_num,
                                    concatPath({root_path, COLUMN_PAIMON_MANIFEST_EMBEDDED_FILE_INDEX}));
      {
        std::optional<Int8> file_source_value;
        getNullableValueFromRowByName(file_source_value, avro_deserializer, row_num,
                                      concatPath({root_path, COLUMN_PAIMON_MANIFEST_FILE_SOURCE}));
        if (file_source_value.has_value()) file_source = toFileSource(file_source_value.value());
      }
      getNullableValueFromRowByName(value_stats_cols, avro_deserializer, row_num,
                                    concatPath({root_path, COLUMN_PAIMON_MANIFEST_VALUE_STATS_COLS}));
    }
  };
  Kind kind;
  String partition;
  ...
  PaimonManifestEntry(const Iceberg::AvroForIcebergDeserializer& avro_deserializer, const String& root_path, const size_t row_num,
                      const PaimonTableSchema& table_schema_, const String& partition_default_name_)
      : kind(toKind(static_cast<int8_t>(
        ...
        total_buckets(static_cast<Int32>(
            avro_deserializer
                ...
                .safeGet<Int32>())),
        file(avro_deserializer, concatPath({root_path, COLUMN_PAIMON_MANIFEST_FILE}), row_num, partition, bucket, table_schema_,
