...
DB::ColumnsDescription getPartitionColumnsDescription(const DB::PaimonTableSchema& table_schema) {
...
PartitionPruner::PartitionPruner(const PaimonTableSchema& table_schema_, const DB::ActionsDAG& filter_dag_, DB::ContextPtr context_)
  ...
  if (!table_schema.partition_keys.empty()) {
    ...
    partition_key = DB::KeyDescription::getKeyFromAST(partition_key_ast, partition_columns_description, context_);

    DB::ActionsDAGWithInversionPushDown inverted_dag(filter_dag_.getOutputs().front(), context_);
    key_condition.emplace(inverted_dag, context_, partition_key.column_names, partition_key.expression, true /* single_point */);
  }
}
...
bool PartitionPruner::canBePruned(const DB::PaimonManifestEntry& manifest_entry) {
  ...
  for (auto& value : partition_key_values) {
    if (value.isNull()) value = POSITIVE_INFINITY;
  }
  if (partition_key_values.empty()) return false;
  std::vector<DB::FieldRef> partition_key_values_ref(partition_key_values.begin(), partition_key_values.end());
  return !key_condition->mayBeTrueInRange(partition_key_values_ref.size(), partition_key_values_ref.data(), partition_key_values_ref.data(),
