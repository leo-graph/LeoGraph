...
PaimonTableClient::PaimonTableClient(ObjectStoragePtr object_storage_, const String& table_location_, const DB::ContextPtr& context_)
    : WithContext(context_), object_storage(object_storage_), table_location(table_location_), log(getLogger("PaimonTableClient")) {}

std::pair<Int32, String> PaimonTableClient::getLastestTableSchemaInfo() {
  /// list all schema files
  const auto schema_files =
      listFiles(*object_storage, table_location, PAIMON_SCHEMA_DIR, [](const RelativePathWithMetadata& path_with_metadata) {
...
std::optional<std::pair<Int64, String>> PaimonTableClient::getLastestTableSnapshotInfo() {
  ...
  RelativePathWithMetadata relative_path_with_metadata(std::filesystem::path(table_location) / PAIMON_SNAPSHOT_DIR /
                                                       PAIMON_SNAPSHOT_LATEST_HINT);
  if (object_storage->exists(StoredObject(relative_path_with_metadata.relative_path))) {
    ...
    {
      auto [_, ec] = std::from_chars(hint_version_string.data(), hint_version_string.data() + hint_version_string.size(), snapshot_version);
      if (ec != std::errc()) {
  ...
  if (!latest_snapshot_path.empty()) {
    ...
    StoredObject store_object(std::filesystem::path(table_location) / (PAIMON_SNAPSHOT_DIR) /
  ...
  // find real latest snapshot
  auto snapshot_files =
      listFiles(*object_storage, table_location, PAIMON_SNAPSHOT_DIR, [](const RelativePathWithMetadata& path_with_metadata) {
        String relative_path = path_with_metadata.relative_path;
        String file_name(relative_path.begin() + relative_path.find_last_of('/') + 1, relative_path.end());
        return file_name.starts_with(PAIMON_SNAPSHOT_PRIFIX);
      });
  if (snapshot_files.empty()) return std::nullopt;
...
std::vector<PaimonManifestFileMeta> PaimonTableClient::getManifestMeta(String manifest_list_path) {
  ...
  Iceberg::AvroForIcebergDeserializer manifest_list_deserializer(std::move(manifest_list_buf), manifest_list_path,
...
PaimonManifest PaimonTableClient::getDataManifest(String manifest_path, const PaimonTableSchema& table_schema,
  ...
  if (manifest_file_name.starts_with("index-manifest-")) return {};
