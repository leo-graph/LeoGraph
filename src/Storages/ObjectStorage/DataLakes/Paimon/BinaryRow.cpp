...
BinaryRow::BinaryRow(const String &bytes_) : reader(bytes_), need_flip(!isLittleEndian()), log(getLogger("BinaryRow")) {
  ...
  if (!need_flip) arity = Poco::ByteOrder::flipBytes(arity);
  LOG_TEST(log, "arity: {} need_flip: {} bytes_size: {} bytes: {}", arity, need_flip, reader.length(), to_hex_string(reader));
}

bool BinaryRow::isLittleEndian() { return std::endian::native == std::endian::little; }
...
Int8 BinaryRow::getByte(Int32 pos) { return getFixedSizeData<char>(pos); }

Int16 BinaryRow::getShort(Int32 pos) { return getFixedSizeData<int16_t>(pos); }
Int32 BinaryRow::getInt(Int32 pos) { return getFixedSizeData<Int32>(pos); }

Int64 BinaryRow::getLong(Int32 pos) { return getFixedSizeData<Int32>(pos); }
...
String BinaryRow::getBinary(Int32 pos) { return getString(pos); }
