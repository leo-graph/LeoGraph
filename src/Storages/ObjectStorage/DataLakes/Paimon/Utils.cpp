...
static String formatFloat(const Float64 x) {
  ...
  if (!result) throw Exception(ErrorCodes::CANNOT_PRINT_FLOAT_OR_DOUBLE_NUMBER, "Cannot print float or double number");
...
String formatDateTime(const DateTime64 &x, UInt32 scale, const DateLUTImpl &time_zone) {
  ...
  auto res = wb.str();
  if (res.ends_with(":00")) res = res.substr(0, res.length() - 3);
  return res;
}
...
DB::Row getPartitionFields(const String &partition_string, const PaimonTableSchema &table_schema) {
    ...
    switch (data_type.root_type) {
      ...
      case RootDataType::DECIMAL: {
        if (const auto *decimal_type = typeid_cast<const DataTypeDecimal32 *>(removeNullable(data_type.clickhouse_data_type).get()))
          return DecimalField<Decimal32>(partition.getDecimal<Int32>(i, decimal_type->getPrecision(), decimal_type->getScale()),
        ...
        if (const auto *decimal_type = typeid_cast<const DataTypeDecimal64 *>(removeNullable(data_type.clickhouse_data_type).get()))
          return DecimalField<Decimal64>(partition.getDecimal<Int64>(i, decimal_type->getPrecision(), decimal_type->getScale()),
                                         decimal_type->getScale());
        if (const auto *decimal_type = typeid_cast<const DataTypeDecimal128 *>(removeNullable(data_type.clickhouse_data_type).get()))
          return DecimalField<Decimal128>(partition.getDecimal<Int128>(i, decimal_type->getPrecision(), decimal_type->getScale()),
                                          decimal_type->getScale());
        if (const auto *decimal_type = typeid_cast<const DataTypeDecimal256 *>(removeNullable(data_type.clickhouse_data_type).get()))
          return DecimalField<Decimal256>(partition.getDecimal<Int256>(i, decimal_type->getPrecision(), decimal_type->getScale()),
                                          decimal_type->getScale());
        else
      ...
      case RootDataType::TIMESTAMP_WITH_LOCAL_TIME_ZONE: {
        ...
        LOG_TEST(&Poco::Logger::get("getPartitionString"), "getPrecision: {}, getScale: {}", type->getPrecision(), type->getScale());
  ...
  for (size_t i = 0; i < table_schema.partition_keys.size(); ++i) {
    ...
    if (it == table_schema.fields_by_name_indexes.end() || it->second >= table_schema.fields.size()) {
      throw DB::Exception(ErrorCodes::BAD_ARGUMENTS, "{} is not found in table schema fields: [{}]", table_schema.partition_keys[i],
...
String getPartitionString(Paimon::BinaryRow &partition, const PaimonTableSchema &table_schema, const String &partition_default_name) {
  auto get_partition_value = [&partition, &partition_default_name](Int32 i, Paimon::DataType &data_type) -> String {
    ...
    switch (data_type.root_type) {
      ...
      case RootDataType::DECIMAL: {
        if (const auto *decimal_type = typeid_cast<const DataTypeDecimal32 *>(removeNullable(data_type.clickhouse_data_type).get()))
          return formatDecimal(partition.getDecimal<Int32>(i, decimal_type->getPrecision(), decimal_type->getScale()),
                               decimal_type->getScale());
        if (const auto *decimal_type = typeid_cast<const DataTypeDecimal64 *>(removeNullable(data_type.clickhouse_data_type).get()))
          return formatDecimal(partition.getDecimal<Int64>(i, decimal_type->getPrecision(), decimal_type->getScale()),
                               decimal_type->getScale());
        if (const auto *decimal_type = typeid_cast<const DataTypeDecimal128 *>(removeNullable(data_type.clickhouse_data_type).get()))
          return formatDecimal(partition.getDecimal<Int128>(i, decimal_type->getPrecision(), decimal_type->getScale()),
                               decimal_type->getScale());
        if (const auto *decimal_type = typeid_cast<const DataTypeDecimal256 *>(removeNullable(data_type.clickhouse_data_type).get()))
          return formatDecimal(partition.getDecimal<Int256>(i, decimal_type->getPrecision(), decimal_type->getScale()),
                               decimal_type->getScale());
        else
          throw Exception(ErrorCodes::BAD_ARGUMENTS, "Unknown type {}", data_type.clickhouse_data_type->getName());
      }
  ...
  if (!partition_entries.empty()) writeString("/", path_writer);
...
String concatPath(std::initializer_list<String> paths) {
  if (paths.size() == 1) return *paths.begin();
