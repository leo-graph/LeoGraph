...
class TestIterator : public IObjectIterator {
 public:
  explicit TestIterator(const std::vector<std::string>& paths) {
    for (const auto& path : paths) objects.push_back(std::make_shared<ObjectInfo>(path));
  }
  ...
  size_t estimatedKeysCount() override { return objects.size(); }
...
std::shared_ptr<IObjectIterator> makeIterator() {
  ...
  for (size_t i = 0; i < 10; ++i) paths.emplace_back(makePath(i));

  return std::make_shared<TestIterator>(paths);
}

// Get next N objects for replica replica_id
bool extractNForReplica(StorageObjectStorageStableTaskDistributor& distributor, std::vector<std::string>& paths, size_t replica_id,
                        size_t n) {
...
bool checkHead(const std::vector<std::string>& paths, std::vector<size_t> file_nums) {
  if (paths.size() < file_nums.size()) return false;
