...
AsynchronousReadBufferFromHDFS::AsynchronousReadBufferFromHDFS(IAsynchronousReader& reader_, const ReadSettings& settings_,
...
bool AsynchronousReadBufferFromHDFS::hasPendingDataToRead() {
  if (read_until_position) {
    ...
    if (file_offset_of_buffer_end > *read_until_position)
      throw Exception(ErrorCodes::LOGICAL_ERROR, "Read beyond last offset ({} > {}, info: {})", file_offset_of_buffer_end,
                      *read_until_position, impl->getInfoForLog());
  }
  return true;
}
...
void AsynchronousReadBufferFromHDFS::prefetch(Priority priority) {
  ...
  if (prefetch_future.valid()) return;

  if (!hasPendingDataToRead()) return;

  prefetch_future = asyncReadInto(prefetch_buffer.data(), prefetch_buffer.size(), priority);
  ProfileEvents::increment(ProfileEvents::RemoteFSPrefetches);
}

std::optional<size_t> AsynchronousReadBufferFromHDFS::tryGetFileSize() { return impl->tryGetFileSize(); }

String AsynchronousReadBufferFromHDFS::getFileName() const { return impl->getFileName(); }

bool AsynchronousReadBufferFromHDFS::nextImpl() {
  if (!hasPendingDataToRead()) return false;
  ...
  if (use_prefetch && bytes_read) prefetch(DEFAULT_PREFETCH_PRIORITY);
...
off_t AsynchronousReadBufferFromHDFS::seek(off_t offset, int whence) {
  ...
  while (true) {
    ...
    if (read_from_prefetch) ProfileEvents::increment(ProfileEvents::RemoteFSCancelledPrefetches);
    break;
  }
...
AsynchronousReadBufferFromHDFS::~AsynchronousReadBufferFromHDFS() {
  LOG_TEST(log, "object:{} next_times:{} seek_times:{} interval:{}|{} duration:{}|{} wait:{}|{}", reinterpret_cast<std::uintptr_t>(this),
           next_times, seek_times, sum_interval, static_cast<double>(sum_interval) / static_cast<double>(next_times), sum_duration,
           static_cast<double>(sum_duration) / static_cast<double>(next_times), sum_wait,
           static_cast<double>(sum_wait) / static_cast<double>(next_times));

  finalize();
}

off_t AsynchronousReadBufferFromHDFS::getPosition() { return file_offset_of_buffer_end - available(); }

size_t AsynchronousReadBufferFromHDFS::getFileOffsetOfBufferEnd() const { return file_offset_of_buffer_end; }
