...
HDFSFileInfo::~HDFSFileInfo() { hdfsFreeFileInfo(file_info, length); }

void HDFSBuilderWrapper::loadFromConfig(const Poco::Util::AbstractConfiguration &config, const String &prefix,
                                        [[maybe_unused]] bool isUser) {
...
#  else   // USE_KRB5
      LOG_WARNING(getLogger("HDFSClient"),
                  "hadoop_kerberos_keytab parameter is ignored because ClickHouse was built without support of krb5 library.");
#  endif  // USE_KRB5
      continue;
    }
...
HDFSBuilderWrapper createHDFSBuilder(const String &uri_str, const Poco::Util::AbstractConfiguration &config) {
  ...
  if (host.empty()) throw Exception(ErrorCodes::BAD_ARGUMENTS, "Illegal HDFS URI: {}", uri.toString());

  HDFSBuilderWrapper builder;
  if (builder.get() == nullptr)
    throw Exception(ErrorCodes::HDFS_ERROR, "Unable to create builder to connect to HDFS: {} {}", uri.toString(),
                    String(hdfsGetLastError()));
...
HDFSFSPtr createHDFSFS(hdfsBuilder *builder) {
  ...
  if (fs == nullptr) throw Exception(ErrorCodes::HDFS_ERROR, "Unable to connect to HDFS: {}", String(hdfsGetLastError()));
