...
struct WriteBufferFromHDFS::WriteBufferFromHDFSImpl : public HDFSErrorWrapper {
  ...
  WriteBufferFromHDFSImpl(const std::string& hdfs_uri_, const std::string& hdfs_file_path_,
                          const Poco::Util::AbstractConfiguration& config_, int replication_, const WriteSettings& write_settings_,
                          int flags)
      : HDFSErrorWrapper(hdfs_uri_, config_), hdfs_uri(hdfs_uri_), hdfs_file_path(hdfs_file_path_), write_settings(write_settings_) {
    ...
    if (fout == nullptr) {
      throw Exception(ErrorCodes::CANNOT_OPEN_FILE, "Unable to open HDFS file: {} ({}) error: {}", hdfs_file_path, hdfs_uri,
  ...
  int write(const char* start, size_t size) {
    ...
    if (bytes_written < 0)
      throw Exception(ErrorCodes::NETWORK_ERROR, "Fail to write HDFS file: {}, hdfs_uri: {}, {}", hdfs_file_path, hdfs_uri,
                      std::string(hdfsGetLastError()));

    if (write_settings.remote_throttler) write_settings.remote_throttler->throttle(bytes_written);
  ...
  void sync() const {
    ...
    if (result < 0)
      throw ErrnoException(ErrorCodes::CANNOT_FSYNC, "Cannot HDFS sync {}, hdfs_url: {}, {}", hdfs_file_path, hdfs_uri,
                           std::string(hdfsGetLastError()));
  }
};

WriteBufferFromHDFS::WriteBufferFromHDFS(const std::string& hdfs_uri_, const std::string& hdfs_file_path_,
                                         const Poco::Util::AbstractConfiguration& config_, int replication_,
                                         const WriteSettings& write_settings_, size_t buf_size_, int flags_,
...
void WriteBufferFromHDFS::nextImpl() {
  ...
  while (bytes_written != offset()) bytes_written += impl->write(working_buffer.begin() + bytes_written, offset() - bytes_written);
