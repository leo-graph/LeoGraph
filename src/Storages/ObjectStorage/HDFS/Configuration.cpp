...
ObjectStoragePtr StorageHDFSConfiguration::createObjectStorage(  /// NOLINT
  ...
  auto hdfs_settings =
      std::make_unique<HDFSObjectStorageSettings>(settings[Setting::remote_read_min_bytes_for_seek], settings[Setting::hdfs_replication]);
  return std::make_shared<HDFSObjectStorage>(url, std::move(hdfs_settings), context->getConfigRef(), /* lazy_initialize */ true);
}
...
void HDFSStorageParsedArguments::fromAST(ASTs& args, ContextPtr context, bool with_structure) {
  if (args.empty() || args.size() > getMaxNumberOfArguments(with_structure))
    throw Exception(ErrorCodes::NUMBER_OF_ARGUMENTS_DOESNT_MATCH, "Storage HDFS requires 1 to {} arguments. All supported signatures:\n{}",
                    getMaxNumberOfArguments(with_structure), getSignatures(with_structure));
...
void StorageHDFSConfiguration::setURL(const std::string& url_) {
  ...
  if (pos == std::string::npos)
    throw Exception(ErrorCodes::BAD_ARGUMENTS, "Bad HDFS URL: {}. It should have the following structure 'hdfs://<host_name>:<port>/path'",
                    url_);
  ...
  if (!path.path.starts_with('/')) path = '/' + path.path;
...
static void addStructureAndFormatToArgsIfNeededHDFS(ASTs& args, const String& structure_, const String& format_, ContextPtr context,
                                                    bool with_structure) {
  if (auto collection = tryGetNamedCollectionWithOverrides(args, context)) {
    ...
    if (count == 0 || count > HDFSStorageParsedArguments::getMaxNumberOfArguments())
      throw Exception(ErrorCodes::LOGICAL_ERROR, "Expected 1 to {} arguments in table function hdfs, got {}",
                      HDFSStorageParsedArguments::getMaxNumberOfArguments(), count);
    ...
    /// hdfs(url, format)
      ...
      if (checkAndGetLiteralArgument<String>(args[1], "format") == "auto") args.back() = format_literal;
      if (with_structure) args.push_back(structure_literal);
    }
    ...
    /// hdfs(url, format, compression_method)
      ...
      if (checkAndGetLiteralArgument<String>(args[1], "format") == "auto") args[1] = format_literal;
...
void StorageHDFSConfiguration::addStructureAndFormatToArgsIfNeeded(ASTs& args, const String& structure_, const String& format_,
