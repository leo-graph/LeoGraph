...
String StorageObjectStorage::getPathSample(ContextPtr context) {
  ...
  if (context->getSettingsRef()[Setting::use_hive_partitioning]) local_distributed_processing = false;

  auto file_iterator = StorageObjectStorageSource::createFileIterator(configuration, query_settings, object_storage,
                                                                      nullptr,                                    // storage_metadata
                                                                      local_distributed_processing, context, {},  // predicate
                                                                      {}, {},                                     // virtual_columns
                                                                      {},                                         // hive_columns
                                                                      nullptr,                                    // read_keys
  ...
  if (!configuration->isArchive() && !path.hasGlobs() && !local_distributed_processing) return path.path;

  if (auto file = file_iterator->next(0)) return file->getPath();
  return "";
}

StorageObjectStorage::StorageObjectStorage(StorageObjectStorageConfigurationPtr configuration_, ObjectStoragePtr object_storage_,
                                           ContextPtr context, const StorageID &table_id_,
                                           const ColumnsDescription &columns_in_table_or_function_definition,
                                           const ConstraintsDescription &constraints_, const String &comment,
                                           std::optional<FormatSettings> format_settings_, LoadingStrictnessLevel mode,
                                           std::shared_ptr<DataLake::ICatalog> catalog_, bool if_not_exists_, bool is_datalake_query,
                                           bool distributed_processing_, ASTPtr partition_by_, ASTPtr order_by_, bool is_table_function_,
                                           bool lazy_init)
    : IStorage(table_id_),
  ...
  const bool need_resolve_sample_path = context->getSettingsRef()[Setting::use_hive_partitioning] && !configuration->partition_strategy &&
                                        !configuration->isDataLakeConfiguration();
  const bool do_lazy_init = lazy_init && !need_resolve_columns_or_format && !need_resolve_sample_path;

  LOG_DEBUG(log,
            "StorageObjectStorage: lazy_init={}, need_resolve_columns_or_format={}, "
            "need_resolve_sample_path={}, is_table_function={}, is_datalake_query={}, columns_in_table_or_function_definition={}",
            lazy_init, need_resolve_columns_or_format, need_resolve_sample_path, is_table_function, is_datalake_query,
  ...
  if (!is_table_function && !columns_in_table_or_function_definition.empty() && !is_datalake_query &&
      mode == LoadingStrictnessLevel::CREATE) {
    LOG_DEBUG(log, "Creating new storage with specified columns");
    configuration->create(object_storage, context, columns_in_table_or_function_definition, partition_by_, order_by_, if_not_exists_,
                          catalog, storage_id);
  }
  ...
  std::tie(hive_partition_columns_to_read_from_file_path, file_columns) = HivePartitioningUtils::setupHivePartitioningForObjectStorage(
      columns, configuration, sample_path, columns_in_table_or_function_definition.empty(), format_settings, context);

  bool validate_schema_with_remote = !need_resolve_columns_or_format && !configuration->isDataLakeConfiguration() &&
                                     !columns_in_table_or_function_definition.empty() && !is_table_function &&
                                     mode == LoadingStrictnessLevel::CREATE && !do_lazy_init;

  validateColumns(columns, configuration_, validate_schema_with_remote, object_storage_, &format_settings, &sample_path, context,
                  &hive_partition_columns_to_read_from_file_path, &columns_in_table_or_function_definition, log);

  // Assert file contains at least one column. The assertion only takes place if we were able to deduce the schema. The storage might be
  // empty.
  if (!columns.empty() && file_columns.empty()) {
    throw Exception(ErrorCodes::INCORRECT_DATA,
                    "File without physical columns is not supported. Please try it with `use_hive_partitioning=0` and or "
                    "`partition_strategy=wildcard`. File {}",
                    sample_path);
  }
  ...
  if (!do_lazy_init && is_table_function && configuration->isDataLakeConfiguration()) {
    ...
    if (auto state = configuration->getTableStateSnapshot(context)) {
      ...
      if (configuration->shouldReloadSchemaForConsistency(context)) {
        if (auto metadata_snapshot = configuration->buildStorageMetadataFromState(*state, context)) metadata = *metadata_snapshot;
      }
    }
  }
  ...
  if (configuration->partition_strategy) metadata.partition_key = configuration->partition_strategy->getPartitionKeyDescription();

  setVirtuals(VirtualColumnUtils::getVirtualsForFileLikeStorage(metadata.columns, context, format_settings,
                                                                configuration->partition_strategy_type, sample_path));

  setInMemoryMetadata(metadata);
}

String StorageObjectStorage::getName() const { return configuration->getEngineName(); }
...
bool StorageObjectStorage::supportsPrewhere() const { return supports_prewhere; }

bool StorageObjectStorage::canMoveConditionsToPrewhere() const { return supports_prewhere; }
...
IStorage::ColumnSizeByName StorageObjectStorage::getColumnSizes() const { return getInMemoryMetadataPtr()->getFakeColumnSizes(); }

IDataLakeMetadata *StorageObjectStorage::getExternalMetadata(ContextPtr query_context) {
  if (!configuration->isDataLakeConfiguration()) return nullptr;
...
void StorageObjectStorage::updateExternalDynamicMetadataIfExists(ContextPtr query_context) {
  if (!configuration->isDataLakeConfiguration()) return;
  ...
  if (configuration->shouldReloadSchemaForConsistency(query_context)) {
    if (auto metadata_snapshot = configuration->buildStorageMetadataFromState(*state, query_context)) new_metadata = *metadata_snapshot;
  }
...
std::optional<UInt64> StorageObjectStorage::totalRows(ContextPtr query_context) const {
  if (!configuration->supportsTotalRows(query_context, object_storage->getType())) return std::nullopt;
  ...
  if (distributed_processing) return std::nullopt;
...
std::optional<UInt64> StorageObjectStorage::totalBytes(ContextPtr query_context) const {
  if (!configuration->supportsTotalBytes(query_context, object_storage->getType())) return std::nullopt;
...
void StorageObjectStorage::read(QueryPlan &query_plan, const Names &column_names, const StorageSnapshotPtr &storage_snapshot,
  ...
  if (configuration->isDataLakeConfiguration()) {
    if (auto start_version = settings[Setting::delta_lake_snapshot_start_version].value;
      ...
      if (const auto *delta_kernel_metadata = dynamic_cast<const DeltaLakeMetadataDeltaKernel *>(configuration->getExternalMetadata());
        ...
        auto read_step = std::make_unique<ReadFromDeltaLakeTableChangesStep>(std::move(table_changes), source_header, column_names,
                                                                             query_info, storage_snapshot, num_streams, local_context);
  ...
  auto read_from_format_info = configuration->prepareReadingFromFormat(
      object_storage, column_names, storage_snapshot, supportsSubsetOfColumns(local_context), supports_tuple_elements, local_context,
  ...
  if (!modified_format_settings.has_value()) modified_format_settings.emplace(getFormatSettings(local_context));

  configuration->modifyFormatSettings(modified_format_settings.value(), *local_context);

  auto read_step = std::make_unique<ReadFromObjectStorageStep>(
      object_storage, configuration, column_names, getVirtualsList(), query_info, storage_snapshot, modified_format_settings,
      distributed_processing, read_from_format_info, need_only_count, local_context, max_block_size, num_streams);

  query_plan.addStep(std::move(read_step));
}

SinkToStoragePtr StorageObjectStorage::write(const ASTPtr &, const StorageMetadataPtr &metadata_snapshot, ContextPtr local_context,
                                             bool /* async_insert */) {
  /// For data lake we did update in getExternalDynamicMetadata.
  if (!is_table_function && !configuration->isDataLakeConfiguration()) {
    configuration->update(object_storage, local_context);
  ...
  if (configuration->isArchive()) {
    throw Exception(ErrorCodes::NOT_IMPLEMENTED, "Path '{}' contains archive. Write into archive is not supported", raw_path.path);
  }
  ...
  if (!configuration->supportsWrites()) throw Exception(ErrorCodes::NOT_IMPLEMENTED, "Writes are not supported for engine");
  ...
  if (configuration->partition_strategy) {
    return std::make_shared<PartitionedStorageObjectStorageSink>(object_storage, configuration, format_settings, sample_block,
                                                                 local_context);
  }
  ...
  return std::make_shared<StorageObjectStorageSink>(paths.back().path, object_storage, format_settings, sample_block, local_context,
                                                    configuration->format, configuration->compression_method);
}

bool StorageObjectStorage::optimize(const ASTPtr & /*query*/, [[maybe_unused]] const StorageMetadataPtr &metadata_snapshot,
                                    const ASTPtr & /*partition*/, bool /*final*/, bool /*deduplicate*/,
                                    const Names & /* deduplicate_by_columns */, bool /*cleanup*/, [[maybe_unused]] ContextPtr context) {
  return configuration->optimize(metadata_snapshot, context, format_settings);
}

void StorageObjectStorage::truncate(const ASTPtr & /* query */, const StorageMetadataPtr & /* metadata_snapshot */,
                                    ContextPtr /* context */, TableExclusiveLockHolder & /* table_holder */) {
  const auto path = configuration->getRawPath();

  if (configuration->isArchive()) {
    throw Exception(ErrorCodes::NOT_IMPLEMENTED, "Path '{}' contains archive. Table cannot be truncated", path.path);
  }
  ...
  if (path.hasGlobs()) {
    throw Exception(ErrorCodes::DATABASE_ACCESS_DENIED,
                    "{} key '{}' contains globs, so the table is in readonly mode and cannot be truncated", getName(), path.path);
  }
...
std::unique_ptr<ReadBufferIterator> StorageObjectStorage::createReadBufferIterator(
    const ObjectStoragePtr &object_storage, const StorageObjectStorageConfigurationPtr &configuration,
    const std::optional<FormatSettings> &format_settings, ObjectInfos &read_keys, const ContextPtr &context) {
  auto file_iterator = StorageObjectStorageSource::createFileIterator(configuration, configuration->getQuerySettings(context),
                                                                      object_storage, nullptr, /* storage_metadata */
  ...
  return std::make_unique<ReadBufferIterator>(object_storage, configuration, file_iterator, format_settings,
                                              getSchemaCache(context, configuration->getTypeName()), read_keys, context);
}

ColumnsDescription StorageObjectStorage::resolveSchemaFromData(const ObjectStoragePtr &object_storage,
                                                               const StorageObjectStorageConfigurationPtr &configuration,
                                                               const std::optional<FormatSettings> &format_settings,
                                                               std::string &sample_path, const ContextPtr &context) {
...
std::string StorageObjectStorage::resolveFormatFromData(const ObjectStoragePtr &object_storage,
                                                        const StorageObjectStorageConfigurationPtr &configuration,
                                                        const std::optional<FormatSettings> &format_settings, std::string &sample_path,
...
std::pair<ColumnsDescription, std::string> StorageObjectStorage::resolveSchemaAndFormatFromData(
    const ObjectStoragePtr &object_storage, const StorageObjectStorageConfigurationPtr &configuration,
    const std::optional<FormatSettings> &format_settings, std::string &sample_path, const ContextPtr &context) {
  ObjectInfos read_keys;
  auto iterator = createReadBufferIterator(object_storage, configuration, format_settings, read_keys, context);
...
Pipe StorageObjectStorage::executeCommand(const String &command_name, const ASTPtr &args, ContextPtr context) {
  ...
  if (!metadata) throw Exception(ErrorCodes::NOT_IMPLEMENTED, "EXECUTE command '{}' is not supported by this storage", command_name);
