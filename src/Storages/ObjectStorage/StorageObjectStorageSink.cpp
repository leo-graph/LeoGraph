...
StorageObjectStorageSink::StorageObjectStorageSink(const std::string &path_, ObjectStoragePtr object_storage,
                                                   const std::optional<FormatSettings> &format_settings_, SharedHeader sample_block_,
                                                   ContextPtr context, const String &format, const String &compression_method)
    : SinkToStorage(sample_block_), path(path_), sample_block(sample_block_) {
  const auto &settings = context->getSettingsRef();
  const auto chosen_compression_method = chooseCompressionMethod(path, compression_method);

  auto buffer = object_storage->writeObject(StoredObject(path), WriteMode::Rewrite, std::nullopt, DBMS_DEFAULT_BUFFER_SIZE,
                                            context->getWriteSettings());

  write_buf = wrapWriteBufferWithCompressionMethod(std::move(buffer), chosen_compression_method,
...
size_t StorageObjectStorageSink::getFileSize() const {
  if (!result_file_size) throw Exception(ErrorCodes::LOGICAL_ERROR, "Sink must be finalized before requesting result file size");
  return *result_file_size;
}

PartitionedStorageObjectStorageSink::PartitionedStorageObjectStorageSink(ObjectStoragePtr object_storage_,
                                                                         StorageObjectStorageConfigurationPtr configuration_,
                                                                         std::optional<FormatSettings> format_settings_,
                                                                         SharedHeader sample_block_, ContextPtr context_)
...
SinkPtr PartitionedStorageObjectStorageSink::createSinkForPartition(const String &partition_id) {
  ...
  if (auto new_key =
          checkAndGetNewFileOnInsertIfNeeded(*object_storage, *configuration, query_settings, file_path, /* sequence_number */ 1)) {
    file_path = *new_key;
  }

  return std::make_shared<StorageObjectStorageSink>(file_path, object_storage, format_settings,
                                                    std::make_shared<Block>(partition_strategy->getFormatHeader()), context,
