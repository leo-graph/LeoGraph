...
struct AzureStorageParsedArguments : private StorageParsedArguments {
  ...
  static bool collectCredentials(ASTPtr maybe_credentials, std::optional<String> &client_id, std::optional<String> &tenant_id,
  ...
  void initializeForOneLake(ASTs &args, ContextPtr context);
...
class StorageAzureConfiguration : public StorageObjectStorageConfiguration {
 ...
 public:
  ...
  String getDataSourceDescription() const override {
    return std::filesystem::path(connection_params.getConnectionURL()) / connection_params.getContainer();
  }
  ...
  ObjectStoragePtr createObjectStorage(ContextPtr context, bool is_readonly,
                                       CredentialsConfigurationCallback refresh_credentials_callback) override;

  void addStructureAndFormatToArgsIfNeeded(ASTs &args, const String &structure_, const String &format_, ContextPtr context,
