...
const std::unordered_set<std::string_view> optional_configuration_keys = {
    "format",      "compression",       "structure",           "compression_method", "account_name",
    "account_key", "connection_string", "storage_account_url", "partition_strategy", "partition_columns_in_data_file",
    "client_id",   "tenant_id",
};
...
ObjectStoragePtr StorageAzureConfiguration::createObjectStorage(
    ContextPtr context, bool is_readonly, CredentialsConfigurationCallback /*refresh_credentials_callback*/)  /// NOLINT
{
  assertInitialized();
  ...
  return std::make_unique<AzureObjectStorage>("AzureBlobStorage", connection_params.auth_method, std::move(client), std::move(settings),
                                              connection_params, connection_params.getContainer(), connection_params.getConnectionURL(),
                                              /*common_key_prefix*/ "");
}

AzureBlobStorage::ConnectionParams getAzureConnectionParams(const String &connection_url, const String &container_name,
                                                            const std::optional<String> &account_name,
                                                            const std::optional<String> &account_key,
                                                            const std::optional<String> &client_id, const std::optional<String> &tenant_id,
  ...
  if (client_id || tenant_id) {
    if (!client_id || !tenant_id)
      throw Exception(ErrorCodes::BAD_ARGUMENTS, "Both 'client_id' and 'tenant_id' need to be provided, but '{}' is missing",
                      client_id ? "tenant_id" : "client_id");
  ...
  if (account_name || account_key) {
    ...
    if (!account_name || !account_key)
      throw Exception(ErrorCodes::BAD_ARGUMENTS, "Both 'account_name' and 'account_key' need to be provided, but '{}' is missing",
                      account_name ? "account_key" : "account_name");
  ...
  connection_params.client_options =
      AzureBlobStorage::getClientOptions(local_context, local_context->getSettingsRef(), *request_settings, /*for_disk=*/false);
...
void AzureStorageParsedArguments::fromNamedCollection(const NamedCollection &collection, ContextPtr context) {
  ...
  if (collection.has("account_name")) account_name = collection.get<String>("account_name");

  if (collection.has("account_key")) account_key = collection.get<String>("account_key");

  if (collection.has("client_id")) client_id = collection.get<String>("client_id");

  if (collection.has("tenant_id")) tenant_id = collection.get<String>("tenant_id");
...
bool AzureStorageParsedArguments::collectCredentials(ASTPtr maybe_credentials, std::optional<String> &client_id,
                                                     std::optional<String> &tenant_id, ContextPtr local_context) {
...
void AzureStorageParsedArguments::initializeForOneLake(ASTs &args, ContextPtr context) {
  if (args.size() != 1) throw Exception(ErrorCodes::BAD_ARGUMENTS, "Only one argument should be provided in OneLake catalog");
  ...
  connection_params.client_options =
      AzureBlobStorage::getClientOptions(context, context->getSettingsRef(), *request_settings, /*for_disk=*/false);
}
...
void AzureStorageParsedArguments::fromAST(ASTs &engine_args, ContextPtr context, bool with_structure) {
  ...
  if (engine_args.empty() || engine_args.size() > AzureStorageParsedArguments::getMaxNumberOfArguments(with_structure)) {
    throw Exception(
        ErrorCodes::NUMBER_OF_ARGUMENTS_DOESNT_MATCH, "Storage AzureBlobStorage requires 1 to {} arguments. All supported signatures:\n{}",
        AzureStorageParsedArguments::getMaxNumberOfArguments(with_structure), AzureStorageParsedArguments::getSignatures(with_structure));
  }
  ...
  if (engine_args.size() == 4) {
    ...
    if (is_format_arg(fourth_arg)) {
      ...
      if (with_structure)
        ...
        throw Exception(ErrorCodes::BAD_ARGUMENTS, "Unknown format or account name specified without account key: {}", fourth_arg);
    }
    ...
    if (is_format_arg(fourth_arg)) {
      ...
      if (const auto seventh_arg = tryGetLiteralArgument<String>(engine_args[6], "structure/partition_columns_in_data_file")) {
        if (with_structure) {
          ...
          throw Exception(ErrorCodes::BAD_ARGUMENTS, "Expected `partition_columns_in_data_file` of type boolean, but found: {}",
                          seventh_arg.value());
        }
      } else {
      ...
      if (!is_format_arg(sixth_arg)) throw Exception(ErrorCodes::BAD_ARGUMENTS, "Unknown format {}", sixth_arg);
      format = sixth_arg;
...
void addStructureAndFormatToArgsIfNeededAzure(ASTs &args, const String &structure_, const String &format_, ContextPtr context,
                                              bool with_structure) {
  if (auto collection = tryGetNamedCollectionWithOverrides(args, context)) {
    ...
    if (args.size() < 3 || args.size() > AzureStorageParsedArguments::getMaxNumberOfArguments())
      throw Exception(ErrorCodes::LOGICAL_ERROR, "Expected 3 to {} arguments in table function azureBlobStorage, got {}",
                      AzureStorageParsedArguments::getMaxNumberOfArguments(), args.size());

    for (auto &arg : args) arg = evaluateConstantExpressionOrIdentifierAsLiteral(arg, context);
    ...
    /// We can distinguish them by looking at the 4-th argument: check if it's format name or not.
      ...
      if (is_format_arg(fourth_arg)) {
        if (fourth_arg == "auto") args[3] = format_literal;
        if (with_structure) {
          /// Add compression=auto before structure argument.
          args.push_back(make_intrusive<ASTLiteral>("auto"));
          args.push_back(structure_literal);
    ...
    /// (storage_account_url, container_name, blobpath, account_name, account_key, format)
      ...
      if (is_format_arg(fourth_arg)) {
        if (fourth_arg == "auto") args[3] = format_literal;
        if (with_structure && checkAndGetLiteralArgument<String>(args[5], "structure") == "auto") args[5] = structure_literal;
      }
      /// (..., account_name, account_key, format) -> (..., account_name, account_key, format, compression, structure)
      else if (is_format_arg(sixth_arg)) {
        if (sixth_arg == "auto") args[5] = format_literal;
        if (with_structure) {
          /// Add compression=auto before structure argument.
          args.push_back(make_intrusive<ASTLiteral>("auto"));
          args.push_back(structure_literal);
    ...
    /// (storage_account_url, container_name, blobpath, account_name, account_key, format, compression)
      ...
      /// (..., format, compression) -> (..., format, compression, structure)
      if (checkAndGetLiteralArgument<String>(args[5], "format") == "auto") args[5] = format_literal;
      if (with_structure) args.push_back(structure_literal);
    }
    /// (storage_account_url, container_name, blobpath, account_name, account_key, format, compression, structure)
    else if (args.size() == 8) {
      if (checkAndGetLiteralArgument<String>(args[5], "format") == "auto") args[5] = format_literal;
      if (with_structure && checkAndGetLiteralArgument<String>(args[7], "structure") == "auto") args[7] = structure_literal;
    }
  }
}
...
void StorageAzureConfiguration::addStructureAndFormatToArgsIfNeeded(ASTs &args, const String &structure_, const String &format_,
...
void StorageAzureConfiguration::fromAST(ASTs &engine_args, ContextPtr context, bool with_structure) {
  ...
  if (!onelake_client_id.empty()) {
    ...
    parsed_arguments.connection_params.auth_method =
        std::make_shared<Azure::Identity::ClientSecretCredential>(onelake_tenant_id, onelake_client_id, onelake_client_secret);
  } else {
