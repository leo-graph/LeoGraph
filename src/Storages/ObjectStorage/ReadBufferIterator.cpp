...
ReadBufferIterator::ReadBufferIterator(ObjectStoragePtr object_storage_, StorageObjectStorageConfigurationPtr configuration_,
                                       const ObjectIterator &file_iterator_, const std::optional<FormatSettings> &format_settings_,
                                       SchemaCache &schema_cache_, ObjectInfos &read_keys_, const ContextPtr &context_)
  ...
  if (configuration->format != "auto") format = configuration->format;
}
...
SchemaCache::Keys ReadBufferIterator::getKeysForSchemaCache() const {
  ...
  std::transform(read_keys.begin(), read_keys.end(), std::back_inserter(sources),
...
std::optional<ColumnsDescription> ReadBufferIterator::tryGetColumnsFromCache(const ObjectInfos::iterator &begin,
                                                                             const ObjectInfos::iterator &end) {
  if (!query_settings.schema_inference_use_cache) return std::nullopt;
  ...
  for (auto it = begin; it < end; ++it) {
    ...
    auto get_last_mod_time = [&] -> std::optional<time_t> {
      ...
      if (!object_info->getObjectMetadata()) {
        ...
        if (meta) object_info->setObjectMetadata(*meta);
      }
...
void ReadBufferIterator::setNumRowsToLastFile(size_t num_rows) {
  if (query_settings.schema_inference_use_cache) schema_cache.addNumRows(getKeyForSchemaCache(*current_object_info, *format), num_rows);
}

void ReadBufferIterator::setSchemaToLastFile(const ColumnsDescription &columns) {
  if (query_settings.schema_inference_use_cache) schema_cache.addColumns(getKeyForSchemaCache(*current_object_info, *format), columns);
}

void ReadBufferIterator::setFormatName(const String &format_name) { format = format_name; }

String ReadBufferIterator::getLastFilePath() const {
  if (current_object_info) return current_object_info->getPath();
  return "";
}
...
std::unique_ptr<ReadBuffer> ReadBufferIterator::recreateLastReadBuffer() {
  ...
  auto impl = createReadBuffer(current_object_info->relative_path_with_metadata, object_storage, context, getLogger("ReadBufferIterator"));
  ...
  while (true) {
    ...
    if (read_keys.size() > prev_read_keys_size) {
      ...
      if (getContext()->getSettingsRef()[Setting::schema_inference_mode] == SchemaInferenceMode::DEFAULT) {
        ...
        if (columns_from_cache) return {nullptr, columns_from_cache, format};
      }
    ...
    if (const auto *object_info_in_archive = dynamic_cast<const ObjectInfoInArchive *>(current_object_info.get())) {
      ...
      compression_method = chooseCompressionMethod(filename, configuration->compression_method);
      read_buf =
          createReadBuffer(current_object_info->relative_path_with_metadata, object_storage, getContext(), getLogger("ReadBufferIterator"));
    }

    if (!query_settings.skip_empty_files || !read_buf->eof()) {
      first = false;

      read_buf = wrapReadBufferWithCompressionMethod(std::move(read_buf), compression_method,
