...
std::optional<String> checkAndGetNewFileOnInsertIfNeeded(const IObjectStorage &object_storage,
                                                         const StorageObjectStorageConfiguration &configuration,
                                                         const StorageObjectStorageQuerySettings &settings, const String &key,
                                                         size_t sequence_number) {
  if (settings.truncate_on_insert || !object_storage.exists(StoredObject(key))) return std::nullopt;
...
void resolveSchemaAndFormat(ColumnsDescription &columns, std::string &format, ObjectStoragePtr object_storage,
                            const StorageObjectStorageConfigurationPtr &configuration, std::optional<FormatSettings> format_settings,
  ...
  if (columns.empty()) {
    if (configuration->isDataLakeConfiguration()) {
      ...
      if (table_structure) columns = table_structure.value();
    }
...
void validateColumns(const ColumnsDescription &columns, StorageObjectStorageConfigurationPtr configuration,
                     bool validate_schema_with_remote, ObjectStoragePtr object_storage,
                     const std::optional<FormatSettings> *format_settings, const std::string *sample_path, ContextPtr context,
                     const NamesAndTypesList *hive_partition_columns_to_read_from_file_path,
                     const ColumnsDescription *columns_in_table_or_function_definition, LoggerPtr log) {
  if (!columns.hasOnlyOrdinary()) {
    /// We don't allow special columns.
    throw Exception(ErrorCodes::BAD_ARGUMENTS, "Special columns like MATERIALIZED, ALIAS or EPHEMERAL are not supported for {} storage.",
  ...
  if (!object_storage || !configuration || !format_settings || !sample_path || !context || !hive_partition_columns_to_read_from_file_path ||
      !columns_in_table_or_function_definition || !log)
    return;

  /// We don't check csv and tsv formats because they change column names.
  if (!validate_schema_with_remote || configuration->format == "CSV" || configuration->format == "TSV") return;
  ...
  try {
    schema_file = StorageObjectStorage::resolveSchemaFromData(object_storage, configuration, *format_settings, sample_path_schema, context);
  } catch (...) {
  ...
  if (schema_file) {
    ...
    for (const auto &column : *columns_in_table_or_function_definition)
      if (!schema_file->tryGet(column.name) && !hive_partitioning_columns.contains(column.name)) {
        String hive_columns;
        for (const auto &hive_column : hive_partitioning_columns) hive_columns += hive_column + ";";
        throw Exception(ErrorCodes::BAD_ARGUMENTS, "Cannot find column {} in schema {}, hive columns {}", column.name,
                        schema_file->toString(false), hive_columns);
      }
  }
}
...
ASTs::iterator getFirstKeyValueArgument(ASTs &args) {
  ...
  for (auto it = args.begin(); it != args.end(); ++it) {
    ...
    if (function_ast && function_ast->name == "equals") {
      if (first_key_value_arg_it == args.end()) first_key_value_arg_it = it;
...
std::unordered_map<std::string, Field> parseKeyValueArguments(const ASTs &function_args, ContextPtr context) {
  ...
  for (const auto &arg : function_args) {
    ...
    if (!inserted) throw Exception(ErrorCodes::LOGICAL_ERROR, "Duplicate key value argument: {}", arg_name);
  }
...
ParseFromDiskResult parseFromDisk(ASTs args, bool with_structure, ContextPtr context, const fs::path &prefix) {
  if (args.size() > 3 + with_structure)
    throw Exception(ErrorCodes::NUMBER_OF_ARGUMENTS_DOESNT_MATCH, "Storage requires {} arguments maximum.", 2 + with_structure);
  ...
  if (auto path_value = getFromPositionOrKeyValue<String>("path", args, engine_args_to_idx, key_value_args); path_value.has_value()) {
    result.path_suffix = path_value.value();
    if (result.path_suffix.empty()) throw Exception(ErrorCodes::BAD_ARGUMENTS, "Empty path is not allowed");

    if (result.path_suffix.starts_with('/') || !pathStartsWith(prefix / fs::path(result.path_suffix), prefix))
      throw Exception(ErrorCodes::BAD_ARGUMENTS,
                      "Path suffixes starting with '.' or '..' and absolute paths are not allowed. Please specify relative path");
  } else
    throw Exception(ErrorCodes::BAD_ARGUMENTS,
                    "Path should be specified as first argument or via `path = <path/to/data>` key value argument");
