...
StorageObjectStorageSource::StorageObjectStorageSource(String name_, ObjectStoragePtr object_storage_,
                                                       StorageObjectStorageConfigurationPtr configuration_,
                                                       StorageSnapshotPtr storage_snapshot_, const ReadFromFormatInfo &info,
                                                       const std::optional<FormatSettings> &format_settings_, ContextPtr context_,
                                                       UInt64 max_block_size_, std::shared_ptr<IObjectIterator> file_iterator_,
                                                       FormatParserSharedResourcesPtr parser_shared_resources_,
                                                       FormatFilterInfoPtr format_filter_info_, bool need_only_count_)
    : ISource(std::make_shared<const Block>(info.source_header), false),
      ...
      create_reader_pool(std::make_shared<ThreadPool>(CurrentMetrics::StorageObjectStorageThreads,
                                                      CurrentMetrics::StorageObjectStorageThreadsActive,
                                                      CurrentMetrics::StorageObjectStorageThreadsScheduled, 1 /* max_threads */)),
...
StorageObjectStorageSource::~StorageObjectStorageSource() { create_reader_pool->wait(); }

std::string StorageObjectStorageSource::getUniqueStoragePathIdentifier(const StorageObjectStorageConfiguration &configuration,
                                                                       const ObjectInfo &object_info, bool include_connection_info) {
  auto path = object_info.getPath();
  if (path.starts_with("/")) path = path.substr(1);

  if (include_connection_info) return fs::path(configuration.getDataSourceDescription()) / path;
...
std::shared_ptr<IObjectIterator> StorageObjectStorageSource::createFileIterator(
    StorageObjectStorageConfigurationPtr configuration, const StorageObjectStorageQuerySettings &query_settings,
    ObjectStoragePtr object_storage, StorageMetadataPtr storage_metadata, bool distributed_processing, const ContextPtr &local_context,
    const ActionsDAG::Node *predicate, const ActionsDAG *filter_actions_dag, const NamesAndTypesList &virtual_columns,
    const NamesAndTypesList &hive_columns, ObjectInfos *read_keys, std::function<void(FileProgress)> file_progress_callback,
    bool ignore_archive_globs, bool skip_object_metadata, bool with_tags) {
  ...
  if (distributed_processing) {
    ...
    auto distributed_iterator = std::make_unique<ReadTaskIterator>(
        ...
        /*is_archive_=*/is_archive && !expect_whole_archive, object_storage, local_context);

    if (is_archive && expect_whole_archive) {
      return std::make_shared<ArchiveIterator>(object_storage, configuration, std::move(distributed_iterator), local_context,
                                               /* read_keys */ nullptr);
    }
  ...
  if (reading_path.hasGlobs() && hasExactlyOneBracketsExpansion(reading_path.path)) {
    ...
    iterator =
        std::make_unique<KeysIterator>(paths, object_storage, virtual_columns, is_archive ? nullptr : read_keys,
                                       query_settings.ignore_non_existent_file, skip_object_metadata, with_tags, file_progress_callback);
    ...
    if (filter_actions_dag && local_context->getSettingsRef()[Setting::s3_path_filter_limit])
      paths = VirtualColumnUtils::extractPathValuesFromFilter(filter_actions_dag, local_context,
                                                              local_context->getSettingsRef()[Setting::s3_path_filter_limit]);
    ...
    if (!paths)
      iterator = std::make_unique<GlobIterator>(object_storage, configuration, predicate, virtual_columns, hive_columns, local_context,
                                                is_archive ? nullptr : read_keys, query_settings.list_object_keys_size,
                                                query_settings.throw_on_zero_files_match, with_tags, file_progress_callback);
    else {
      ...
      if (matcher.ok()) {
        for (const auto &path : paths.value()) {
          ...
          if (RE2::FullMatch(relative_path, matcher)) validated_paths.push_back(relative_path);
        }
      } else
        throw Exception(ErrorCodes::CANNOT_COMPILE_REGEXP, "Cannot compile regex from glob ({}): {}", reading_path.path, matcher.error());

      iterator =
          std::make_unique<KeysIterator>(validated_paths, object_storage, virtual_columns, is_archive ? nullptr : read_keys,
                                         query_settings.ignore_non_existent_file, skip_object_metadata, with_tags, file_progress_callback);
    }
  } else if (configuration->supportsFileIterator()) {
    auto iter = configuration->iterate(filter_actions_dag, file_progress_callback, query_settings.list_object_keys_size, storage_metadata,
                                       local_context);

    if (filter_actions_dag) {
      iter = std::make_shared<ObjectIteratorWithPathAndFileFilter>(std::move(iter), *filter_actions_dag, virtual_columns, hive_columns,
                                                                   configuration->getNamespace(), local_context);
    }
    return iter;
  } else {
    Strings paths;
    ...
    if (filter_dag) {
      ...
      for (const auto &key : keys) paths.push_back(fs::path(configuration->getNamespace()) / key);
    ...
    iterator = std::make_unique<KeysIterator>(paths, object_storage, virtual_columns, is_archive ? nullptr : read_keys,
...
void StorageObjectStorageSource::lazyInitialize() {
  ...
  reader = createReader();
  if (reader) reader_future = createReaderAsync();
  initialized = true;
}
...
Chunk StorageObjectStorageSource::generate() {
  ...
  while (true) {
    ...
    if (reader->pull(chunk)) {
      ...
      size_t chunk_size = 0;
      if (const auto *input_format = reader.getInputFormat()) chunk_size = input_format->getApproxBytesReadForChunk();
      ...
      if (!full_path.starts_with(reading_path)) full_path = fs::path(reading_path) / object_info->getPath();
      ...
      if (!read_from_format_info.hive_partition_columns_to_read_from_file_path.empty()) {
        HivePartitioningUtils::addPartitionColumnsToChunk(chunk, read_from_format_info.hive_partition_columns_to_read_from_file_path, path);
      }
...
#  endif
            ...
            if (!partition_columns.empty()) {
              ...
              if (partition_values != partition_columns.end()) {
                for (const auto &[name_and_type, value] : partition_values->second) {
                  if (!read_from_format_info.source_header.has(name_and_type.name)) continue;
      ...
      if (chunk.hasColumns()) {
        ...
        for (auto &column : columns) {
          if (column->isConst()) column = column->cloneResized(chunk_num_rows)->convertToFullColumnIfConst();
        }
...
void StorageObjectStorageSource::addNumRowsToCache(const ObjectInfo &object_info, size_t num_rows) {
  const auto cache_key = getKeyForSchemaCache(getUniqueStoragePathIdentifier(*configuration, object_info),
                                              object_info.getFileFormat().value_or(configuration->format), format_settings, read_context);
...
StorageObjectStorageSource::ReaderHolder StorageObjectStorageSource::createReader() {
  return createReader(0, file_iterator, configuration, object_storage, read_from_format_info, format_settings, read_context, &schema_cache,
                      log, max_block_size, parser_shared_resources, format_filter_info, need_only_count);
}

StorageObjectStorageSource::ReaderHolder StorageObjectStorageSource::createReader(
    size_t processor, const std::shared_ptr<IObjectIterator> &file_iterator, const StorageObjectStorageConfigurationPtr &configuration,
    const ObjectStoragePtr &object_storage, ReadFromFormatInfo &read_from_format_info, const std::optional<FormatSettings> &format_settings,
    const ContextPtr &context_, SchemaCache *schema_cache, const LoggerPtr &log, size_t max_block_size,
    FormatParserSharedResourcesPtr parser_shared_resources, FormatFilterInfoPtr format_filter_info, bool need_only_count) {
  ...
  do {
    ...
    if (!object_info || object_info->getPath().empty()) return {};
  ...
  auto try_get_num_rows_from_cache = [&]() -> std::optional<size_t> {
    if (!schema_cache) return std::nullopt;

    const auto cache_key = getKeyForSchemaCache(getUniqueStoragePathIdentifier(*configuration, *object_info),
                                                object_info->getFileFormat().value_or(configuration->format), format_settings, context_);
  ...
  if (num_rows_from_cache) {
    ...
    for (const auto &[name, type] : names_and_types) columns.emplace_back(type->createColumn(), type, name);
    builder.init(Pipe(std::make_shared<ConstChunkGenerator>(std::make_shared<const Block>(columns), *num_rows_from_cache, max_block_size)));
  } else {
    ...
    auto filter_info = [&]() {
      if (!schema_changed) return format_filter_info;
      auto mapper = configuration->getColumnMapperForObject(object_info);
      if (!mapper) return format_filter_info;
    ...
    LOG_DEBUG(log, "Reading object '{}', size: {} bytes, with format: {}", object_info->getPath(),
              object_info->getObjectMetadata()->size_bytes, object_info->getFileFormat().value_or(configuration->format));
    ...
    if (context_->getSettingsRef()[Setting::use_parquet_metadata_cache] && use_native_reader_v3 &&
        (object_info->getFileFormat().value_or(configuration->format) == "Parquet") && !object_info->getObjectMetadata()->etag.empty()) {
      ...
      input_format = FormatFactory::instance().getInputWithMetadata(
          object_info->getFileFormat().value_or(configuration->format), *read_buf, initial_header, context_, max_block_size,
          object_with_metadata, format_settings, parser_shared_resources, filter_info, true /* is_remote_fs */, compression_method,
          need_only_count, std::nullopt /*min_block_size_bytes*/, std::nullopt /*min_block_size_rows*/,
          std::nullopt /*max_block_size_bytes*/);
    } else {
      input_format = FormatFactory::instance().getInput(object_info->getFileFormat().value_or(configuration->format), *read_buf,
                                                        initial_header, context_, max_block_size, format_settings, parser_shared_resources,
                                                        filter_info, true /* is_remote_fs */, compression_method, need_only_count);
    }
    ...
    if (!schema_transform) {
      ...
      if (transform) schema_transform = transform->clone();
    }
    ...
    if (schema_transform.has_value()) {
      ...
      builder.addSimpleTransform(
          [&](const SharedHeader &header) { return std::make_shared<ExpressionTransform>(header, schema_modifying_actions); });
    }
  ...
  return ReaderHolder(object_info, std::move(read_buf), std::move(source), std::move(pipeline), std::move(current_reader));
}
...
std::unique_ptr<ReadBufferFromFileBase> createReadBuffer(RelativePathWithMetadata &object_info, const ObjectStoragePtr &object_storage,
                                                         const ContextPtr &context_, const LoggerPtr &log,
  ...
  if (settings[Setting::table_engine_read_through_distributed_cache] &&
    ...
    if (connection_info) use_distributed_cache = true;
  }
#endif
  ...
  bool use_page_cache = !use_distributed_cache && !use_filesystem_cache && effective_read_settings.page_cache &&
                        effective_read_settings.use_page_cache_for_object_storage;
  ...
  if (!object_info.metadata) object_info.metadata = object_storage->getObjectMetadata(object_info.getPath(), /*with_tags=*/false);
  ...
  if (use_distributed_cache) {
    ...
    impl = std::make_unique<ReadBufferFromDistributedCache>(path, StoredObjects({object}), effective_read_settings, connection_info,
                                                            ConnectionTimeouts::getTCPTimeoutsWithoutFailover(context_->getSettingsRef()),
                                                            read_buffer_creator,
                                                            /*use_external_buffer*/ use_async_buffer, context_->getDistributedCacheLog(),
...
#endif
  {
    ...
    if (object_info.metadata->etag.empty()) {
      ...
      auto read_buffer_creator = [path = object_info.getPath(),
                                  nested_buffer_read_settings = modified_read_settings.withNestedBuffer(/* seekable */ false), object_size,
                                  object_storage]() {
      ...
      impl = std::make_unique<CachedOnDiskReadBufferFromFile>(
          object_info.getPath(), cache_key, cache, FileCache::getCommonOrigin(), read_buffer_creator,
          use_async_buffer ? modified_read_settings.withNestedBuffer(/* seekable */ true) : modified_read_settings,
          std::string(CurrentThread::getQueryId()), object_size,
          /* allow_seeks */ true,
          /* use_external_buffer */ use_async_buffer,
          /* read_until_position */ std::nullopt, context_->getFilesystemCacheLog());

      LOG_TRACE(log, "Using filesystem cache `{}` (path: {}, etag: {}, hash: {})", filesystem_cache_name, object_info.getPath(),
  ...
  if (use_page_cache) {
    ...
    impl = std::make_unique<CachedInMemoryReadBufferFromFile>(key, effective_read_settings.page_cache, std::move(impl),
  ...
  if (object_size) buffer_size = std::min<size_t>(object_size, buffer_size);

  LOG_TRACE(log, "Downloading object {} of size {} {} initial prefetch (buffer size: {})", object_info.getPath(), object_size,
            use_prefetch ? "with" : "without", buffer_size);

  auto &reader = context_->getThreadPoolReader(FilesystemReaderType::ASYNCHRONOUS_REMOTE_FS_READER);
  impl = std::make_unique<AsynchronousBoundedReadBuffer>(std::move(impl), reader, modified_read_settings, buffer_size,
                                                         modified_read_settings.remote_read_min_bytes_for_seek,
                                                         context_->getAsyncReadCounters(), context_->getFilesystemReadPrefetchesLog());
...
StorageObjectStorageSource::GlobIterator::GlobIterator(ObjectStoragePtr object_storage_,
                                                       StorageObjectStorageConfigurationPtr configuration_,
                                                       const ActionsDAG::Node *predicate, const NamesAndTypesList &virtual_columns_,
                                                       const NamesAndTypesList &hive_columns_, ContextPtr context_, ObjectInfos *read_keys_,
                                                       size_t list_object_keys_size, bool throw_on_zero_files_match_, bool with_tags,
  ...
  if (reading_path.hasGlobs()) {
    ...
    throw Exception(ErrorCodes::BAD_ARGUMENTS, "Using glob iterator with path without globs is not allowed (used path: {})",
                    reading_path.path);
  }
}
...
ObjectInfoPtr StorageObjectStorageSource::GlobIterator::next(size_t processor) {
  ...
  if (first_iteration && !object_info && throw_on_zero_files_match) {
    throw Exception(ErrorCodes::FILE_DOESNT_EXIST, "Can not match any files with path {}", configuration->getPathForRead().path);
  }
...
ObjectInfoPtr StorageObjectStorageSource::GlobIterator::nextUnlocked(size_t /* processor */) {
  ...
  if (is_finished && current_batch_processed) return {};
  ...
  if (current_batch_processed) {
    ...
    while (new_batch.empty()) {
      ...
      std::transform(result->begin(), result->end(), std::back_inserter(new_batch),
      ...
      if (filter_expr) {
        ...
        for (const auto &object_info : new_batch) paths.push_back(getUniqueStoragePathIdentifier(*configuration, *object_info, false));
    ...
    if (read_keys) read_keys->insert(read_keys->end(), new_batch.begin(), new_batch.end());
  ...
  if (index >= object_infos.size()) {
    throw Exception(ErrorCodes::LOGICAL_ERROR, "Index out of bound for blob metadata. Index: {}, size: {}", index, object_infos.size());
  }

  return object_infos[index++];
}

StorageObjectStorageSource::KeysIterator::KeysIterator(const Strings &keys_, ObjectStoragePtr object_storage_,
                                                       const NamesAndTypesList &virtual_columns_, ObjectInfos *read_keys_,
                                                       bool ignore_non_existent_files_, bool skip_object_metadata_, bool with_tags_,
                                                       std::function<void(FileProgress)> file_progress_callback_)
    : object_storage(object_storage_),
      virtual_columns(virtual_columns_),
      file_progress_callback(file_progress_callback_),
...
ObjectInfoPtr StorageObjectStorageSource::KeysIterator::next(size_t /* processor */) {
  while (true) {
    ...
    if (current_index >= keys.size()) return nullptr;
    ...
    if (file_progress_callback) file_progress_callback(FileProgress(0, object_metadata.size_bytes));

    return std::make_shared<ObjectInfo>(RelativePathWithMetadata(key, object_metadata));
  }
}

StorageObjectStorageSource::ReaderHolder::ReaderHolder(ObjectInfoPtr object_info_, std::unique_ptr<ReadBuffer> read_buf_,
                                                       std::shared_ptr<ISource> source_, std::unique_ptr<QueryPipeline> pipeline_,
...
StorageObjectStorageSource::ReadTaskIterator::ReadTaskIterator(const ClusterFunctionReadTaskCallback &callback_, size_t max_threads_count,
                                                               bool is_archive_, ObjectStoragePtr object_storage_, ContextPtr context_)
    : WithContext(context_), callback(callback_), is_archive(is_archive_), object_storage(object_storage_) {
  ThreadPool pool(CurrentMetrics::StorageObjectStorageThreads, CurrentMetrics::StorageObjectStorageThreadsActive,
  ...
  for (size_t i = 0; i < max_threads_count; ++i)
    objects.push_back(pool_scheduler(
        [this]() -> ObjectInfoPtr {
          auto task = callback();
          if (!task || task->isEmpty()) return nullptr;
  ...
  for (auto &object_future : objects) {
    ...
    if (object) buffer.push_back(object);
  }
}
...
ObjectInfoPtr StorageObjectStorageSource::ReadTaskIterator::next(size_t) {
  ...
  if (!is_archive) return object_info;

  auto [path_to_archive, path_in_archive] = getURIAndArchivePattern(object_info->getPath());
  if (!path_in_archive.has_value()) return object_info;

  return createObjectInfoInArchive(path_to_archive, path_in_archive.value());
}

ObjectInfoPtr StorageObjectStorageSource::ReadTaskIterator::createObjectInfoInArchive(const std::string &path_to_archive,
  ...
  return std::make_shared<ArchiveIterator::ObjectInfoInArchive>(archive_object, path_in_archive, archive_reader,
                                                                archive_reader->getFileInfo(path_in_archive));
}
...
static IArchiveReader::NameFilter createArchivePathFilter(const std::string &archive_pattern) {
  ...
  if (!matcher->ok()) {
    throw Exception(ErrorCodes::CANNOT_COMPILE_REGEXP, "Cannot compile regex from glob ({}): {}", archive_pattern, matcher->error());
  }
  return [matcher](const std::string &p) mutable { return re2::RE2::FullMatch(p, *matcher); };
}

StorageObjectStorageSource::ArchiveIterator::ObjectInfoInArchive::ObjectInfoInArchive(ObjectInfoPtr archive_object_,
...
StorageObjectStorageSource::ArchiveIterator::ArchiveIterator(ObjectStoragePtr object_storage_,
                                                             StorageObjectStorageConfigurationPtr configuration_,
                                                             std::unique_ptr<IObjectIterator> archives_iterator_, ContextPtr context_,
...
std::shared_ptr<IArchiveReader> StorageObjectStorageSource::ArchiveIterator::createArchiveReader(ObjectInfoPtr object_info) const {
  const auto size = object_info->getObjectMetadata()->size_bytes;
  return DB::createArchiveReader(
      /* path_to_archive */
      object_info->getPath(),
      /* archive_read_function */
      [=, this]() { return createReadBuffer(object_info->relative_path_with_metadata, object_storage, getContext(), log); },
      /* archive_size */ size);
}
...
ObjectInfoPtr StorageObjectStorageSource::ArchiveIterator::next(size_t processor) {
  ...
  while (true) {
    if (filter) {
      ...
      if (!filter(path_in_archive)) continue;
      current_file_info = file_enumerator->getFileInfo();
    } else {
      archive_object = archives_iterator->next(processor);
      if (!archive_object) return {};

      if (!archive_object->getObjectMetadata())
        archive_object->setObjectMetadata(object_storage->getObjectMetadata(archive_object->getPath(), /*with_tags=*/false));

      archive_reader = createArchiveReader(archive_object);
      if (!archive_reader->fileExists(path_in_archive)) continue;
  ...
  if (read_keys != nullptr) read_keys->push_back(object_in_archive);

  return object_in_archive;
}

size_t StorageObjectStorageSource::ArchiveIterator::estimatedKeysCount() { return archives_iterator->estimatedKeysCount(); }
