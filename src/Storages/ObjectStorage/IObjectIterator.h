...
struct ObjectInfo {
  ...
  explicit ObjectInfo(const String& relative_path_) : relative_path_with_metadata(RelativePathWithMetadata(relative_path_)) {}
  explicit ObjectInfo(RelativePathWithMetadata relative_path_with_metadata_) : relative_path_with_metadata(relative_path_with_metadata_) {}
...
class ObjectIteratorWithPathAndFileFilter : public IObjectIterator, private WithContext {
 public:
  ObjectIteratorWithPathAndFileFilter(ObjectIterator iterator_, const DB::ActionsDAG& filter_, const NamesAndTypesList& virtual_columns_,
                                      const NamesAndTypesList& hive_partition_columns_, const std::string& object_namespace_,
...
class ObjectIteratorSplitByBuckets : public IObjectIterator, private WithContext {
 public:
  ObjectIteratorSplitByBuckets(ObjectIterator iterator_, const String& format_, ObjectStoragePtr object_storage_,
                               const ContextPtr& context_);
