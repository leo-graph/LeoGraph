...
static ExpressionActionsPtr getExpressionActions(const DB::ActionsDAG& filter_, const NamesAndTypesList& virtual_columns_,
...
ObjectIteratorWithPathAndFileFilter::ObjectIteratorWithPathAndFileFilter(ObjectIterator iterator_, const DB::ActionsDAG& filter_,
                                                                         const NamesAndTypesList& virtual_columns_,
                                                                         const NamesAndTypesList& hive_partition_columns_,
                                                                         const std::string& object_namespace_, const ContextPtr& context_)
...
ObjectInfoPtr ObjectIteratorWithPathAndFileFilter::next(size_t id) {
  while (true) {
    ...
    if (filter_actions) {
      ...
      VirtualColumnUtils::filterByPathOrFile(keys, std::vector<std::string>{path}, filter_actions, virtual_columns, hive_partition_columns,
                                             getContext());
...
std::string ObjectInfo::getPathOrPathToArchiveIfArchive() const {
  if (isArchive()) return getPathToArchive();
  return getPath();
}

ObjectIteratorSplitByBuckets::ObjectIteratorSplitByBuckets(ObjectIterator iterator_, const String& format_,
                                                           ObjectStoragePtr object_storage_, const ContextPtr& context_)
    : WithContext(context_),
      iterator(iterator_),
      format(format_),
...
String ObjectInfo::getIdentifier() const {
  ...
  if (file_bucket_info) result += file_bucket_info->getIdentifier();
  return result;
}
