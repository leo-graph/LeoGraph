...
std::string StorageS3Configuration::getPathInArchive() const {
  if (url.archive_pattern.has_value()) return url.archive_pattern.value();
...
void StorageS3Configuration::validateNamespace(const String &name) const { S3::URI::validateBucket(name, {}); }
...
ObjectStoragePtr StorageS3Configuration::createObjectStorage(ContextPtr context, bool /* is_readonly */,
                                                             CredentialsConfigurationCallback refresh_credentials_callback)  /// NOLINT
{
  assertInitialized();

  if (!headers_from_ast.empty()) {
    s3_settings->auth_settings.headers.insert(s3_settings->auth_settings.headers.end(), headers_from_ast.begin(), headers_from_ast.end());
  }
  ...
  auto client_refresher = [refresh_credentials_callback, this, context_ = Context::createCopy(context)]() -> std::unique_ptr<S3::Client> {
    if (!refresh_credentials_callback) return nullptr;
    auto new_client = getClient(url, *s3_settings, context_, /* for_disk_s3 */ false, /*opt_disk_name*/ {}, refresh_credentials_callback);
    return new_client;
  };
  return std::make_shared<S3ObjectStorage>(std::move(client), std::make_unique<S3Settings>(*s3_settings), url, *s3_capabilities,
                                           /*key_generator=*/nullptr, "StorageS3", false, client_refresher);
}
...
void S3StorageParsedArguments::fromNamedCollection(const NamedCollection &collection, ContextPtr context) {
  ...
  s3_settings->loadFromConfigForObjectStorage(config, "s3", context->getSettingsRef(), url.uri.getScheme(),
  ...
  if (collection.has("partition_strategy")) {
    ...
    const auto partition_strategy_type_opt =
        magic_enum::enum_cast<PartitionStrategyFactory::StrategyType>(partition_strategy_name, magic_enum::case_insensitive);
  ...
  partition_columns_in_data_file = collection.getOrDefault<bool>("partition_columns_in_data_file",
...
bool S3StorageParsedArguments::collectCredentials(ASTPtr maybe_credentials, S3::S3AuthSettings &auth_settings_, ContextPtr local_context) {
  if (!maybe_credentials) return false;

  const auto *credentials_ast_function = maybe_credentials->as<ASTFunction>();
  if (!credentials_ast_function || credentials_ast_function->name != "extra_credentials") return false;
  ...
  for (auto &credential_arg : credentials_function_args) {
    ...
    if (credential_args.size() != 2)
      throw Exception(ErrorCodes::BAD_ARGUMENTS, "Credentials argument is incorrect: expected 2 arguments, got {}", credential_args.size());
...
void S3StorageParsedArguments::fromDisk(const DiskPtr &disk, ASTs &args, ContextPtr context, bool with_structure) {
  ...
  *s3_settings = s3_object_storage.getS3Settings();
  ...
  if (parsing_result.format.has_value()) format = *parsing_result.format;
  if (parsing_result.compression_method.has_value()) compression_method = *parsing_result.compression_method;
  if (parsing_result.structure.has_value()) structure = *parsing_result.structure;
...
void S3StorageParsedArguments::fromAST(ASTs &args, ContextPtr context, bool with_structure) {
  ...
  ASTs key_value_asts;
  if (auto first_key_value_arg_it = getFirstKeyValueArgument(args); first_key_value_arg_it != args.end()) {
  ...
  if (count == 0 || count > S3StorageParsedArguments::getMaxNumberOfArguments(with_structure))
    throw Exception(ErrorCodes::NUMBER_OF_ARGUMENTS_DOESNT_MATCH, "Storage S3 requires 1 to {} arguments. All supported signatures:\n{}",
  ...
  if (key_value_args.contains("structure")) with_structure = false;
  ...
  /// - s3(source, access_key_id, secret_access_key, session_token, format, structure, compression_method)
    ...
    if (with_structure) {
      ...
      if (magic_enum::enum_contains<PartitionStrategyFactory::StrategyType>(sixth_arg)) {
        engine_args_to_idx = {{"access_key_id", 1}, {"secret_access_key", 2}, {"session_token", 3},
                              {"format", 4},        {"structure", 5},         {"partition_strategy", 6}};
      } else {
        engine_args_to_idx = {{"access_key_id", 1}, {"secret_access_key", 2}, {"session_token", 3},
      ...
      engine_args_to_idx = {{"access_key_id", 1}, {"secret_access_key", 2},  {"session_token", 3},
                            {"format", 4},        {"compression_method", 5}, {"partition_strategy", 6}};
    }
  }
  /// For 8 arguments we support:
  /// if with_structure == 0:
  /// - s3(source, access_key_id, secret_access_key, session_token, format, compression_method, partition_strategy,
  /// partition_columns_in_data_file) if with_structure == 1:
  ...
  /// - s3(source, access_key_id, secret_access_key, session_token, format, structure, compression_method, partition_strategy)
    ...
    if (with_structure) {
      auto sixth_arg = checkAndGetLiteralArgument<String>(args[6], "compression_method/partition_strategy");
      if (magic_enum::enum_contains<PartitionStrategyFactory::StrategyType>(sixth_arg)) {
        engine_args_to_idx = {{"access_key_id", 1},
                              {"secret_access_key", 2},
                              {"session_token", 3},
                              {"format", 4},
                              {"structure", 5},
                              {"partition_strategy", 6},
                              {"partition_columns_in_data_file", 7}};
      } else {
        engine_args_to_idx = {{"access_key_id", 1}, {"secret_access_key", 2},  {"session_token", 3},     {"format", 4},
                              {"structure", 5},     {"compression_method", 6}, {"partition_strategy", 7}};
      }
    } else {
      engine_args_to_idx = {{"access_key_id", 1},
                            {"secret_access_key", 2},
  ...
  /// with_structure == 0:
  ///     s3(source, access_key_id, secret_access_key, session_token, format, compression_method, partition_strategy,
  ///     partition_columns_in_data_file, storage_class_name)
  else if (count == 9) {
    if (with_structure)
      engine_args_to_idx = {
          {"access_key_id", 1}, {"secret_access_key", 2},  {"session_token", 3},      {"format", 4},
          {"structure", 5},     {"compression_method", 6}, {"partition_strategy", 7}, {"partition_columns_in_data_file", 8}};
    else
      engine_args_to_idx = {{"access_key_id", 1},
                            {"secret_access_key", 2},
                            {"session_token", 3},
  ...
  if (auto format_value = getFromPositionOrKeyValue<String>("format", args, engine_args_to_idx, key_value_args); format_value.has_value()) {
  ...
  if (auto partition_columns_in_data_file_value =
          getFromPositionOrKeyValue<bool>("partition_columns_in_data_file", args, engine_args_to_idx, key_value_args);
  ...
  } else if (auto no_sign_value = getFromPositionOrKeyValue<bool>("no_sign", args, {}, key_value_args); no_sign_value.has_value()) {
  ...
  if (extra_credentials) args.push_back(extra_credentials);

  if (context->getSettingsRef()[Setting::s3_validate_request_settings]) s3_settings->request_settings.validateUploadSettings();
}

static void addStructureAndFormatToArgsIfNeededS3(ASTs &args, const String &structure_, const String &format_, ContextPtr context,
                                                  bool with_structure, size_t max_number_of_arguments) {
  if (auto collection = tryGetNamedCollectionWithOverrides(args, context)) {
    ...
    if (count > max_number_of_arguments) {
      throw Exception(ErrorCodes::LOGICAL_ERROR, "Expected 1 to {} arguments in table function s3, got {}", max_number_of_arguments, count);
    }
    ...
    for (auto it = first_key_value_arg_it; it != args.end(); ++it) {
      ...
      if (!function_ast || function_ast->name != "equals") continue;

      auto *args_expr = assert_cast<ASTExpressionList *>(function_ast->arguments.get());
      auto &children = args_expr->children;
      if (children.size() != 2) {
        throw Exception(ErrorCodes::BAD_ARGUMENTS, "Key value argument is incorrect: expected 2 arguments, got {}", children.size());
      }
    ...
    if (count == 1) {
      ...
      if (with_structure) args.push_back(structure_literal);
    }
    /// s3(s3_url, format) or
    /// s3(s3_url, NOSIGN)
    /// We can distinguish them by looking at the 2-nd argument: check if it's NOSIGN or not.
    ...
    /// We can distinguish them by looking at the 2-nd argument: check if it's NOSIGN, format name or neither.
      ...
      auto second_arg = checkAndGetLiteralArgument<String>(args[1], "format/NOSIGN");
      if (boost::iequals(second_arg, "NOSIGN")) {
        if (checkAndGetLiteralArgument<String>(args[2], "format") == "auto") args[2] = format_literal;
        if (with_structure) args.push_back(structure_literal);
      } else if (second_arg == "auto" || FormatFactory::instance().exists(second_arg)) {
        if (second_arg == "auto") args[1] = format_literal;
        if (with_structure && checkAndGetLiteralArgument<String>(args[2], "structure") == "auto") args[2] = structure_literal;
      } else {
    ...
    /// We can distinguish them by looking at the 2-nd argument: check if it's NOSIGN, format name or neither.
      ...
      if (boost::iequals(second_arg, "NOSIGN")) {
        if (checkAndGetLiteralArgument<String>(args[2], "format") == "auto") args[2] = format_literal;
        if (with_structure && checkAndGetLiteralArgument<String>(args[3], "structure") == "auto") args[3] = structure_literal;
      } else if (second_arg == "auto" || FormatFactory::instance().exists(second_arg)) {
        if (second_arg == "auto") args[1] = format_literal;
        if (with_structure && checkAndGetLiteralArgument<String>(args[2], "structure") == "auto") args[2] = structure_literal;
      } else {
        auto fourth_arg = checkAndGetLiteralArgument<String>(args[3], "format/session_token");
        if (fourth_arg == "auto" || FormatFactory::instance().exists(fourth_arg)) {
          if (checkAndGetLiteralArgument<String>(args[3], "format") == "auto") args[3] = format_literal;
          if (with_structure) args.push_back(structure_literal);
        } else {
          args.push_back(format_literal);
          if (with_structure) args.push_back(structure_literal);
    ...
    /// We can distinguish them by looking at the 2-nd argument: check if it's a NOSIGN keyword name or not.
      ...
      if (boost::iequals(second_arg, "NOSIGN")) {
        if (checkAndGetLiteralArgument<String>(args[2], "format") == "auto") args[2] = format_literal;
        if (with_structure && checkAndGetLiteralArgument<String>(args[2], "structure") == "auto") args[3] = structure_literal;
      } else {
        auto fourth_arg = checkAndGetLiteralArgument<String>(args[3], "format/session_token");
        if (fourth_arg == "auto" || FormatFactory::instance().exists(fourth_arg)) {
          if (checkAndGetLiteralArgument<String>(args[3], "format") == "auto") args[3] = format_literal;
          if (with_structure && checkAndGetLiteralArgument<String>(args[4], "structure") == "auto") args[4] = structure_literal;
        } else {
          if (checkAndGetLiteralArgument<String>(args[4], "format") == "auto") args[4] = format_literal;
          if (with_structure) args.push_back(structure_literal);
        }
      }
    }
    ...
    /// s3(source, access_key_id, secret_access_key, session_token, format, compression_method)
      ...
      if (fourth_arg == "auto" || FormatFactory::instance().exists(fourth_arg)) {
        ...
        if (checkAndGetLiteralArgument<String>(args[4], "format") == "auto") args[4] = format_literal;
        if (with_structure && checkAndGetLiteralArgument<String>(args[5], "format") == "auto") args[5] = structure_literal;
      }
    }
    /// s3(source, access_key_id, secret_access_key, session_token, format, structure, compression_method)
    else {
...
void StorageS3Configuration::addStructureAndFormatToArgsIfNeeded(ASTs &args, const String &structure_, const String &format_,
                                                                 ContextPtr context, bool with_structure) {
  addStructureAndFormatToArgsIfNeededS3(args, structure_, format_, context, with_structure,
                                        S3StorageParsedArguments::getMaxNumberOfArguments(with_structure));
}
}  // namespace DB
