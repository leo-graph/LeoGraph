...
namespace {
void checkNamesAndTypesCompatibleWithDictionary(const String& dictionary_name, const ColumnsDescription& columns,
  ...
  for (const auto& column : columns.getOrdinary()) {
    if (!names_and_types_set.contains(column)) {
      throw Exception(ErrorCodes::THERE_IS_NO_COLUMN, "Not found column {} {} in dictionary {}. There are only columns {}", column.name,
                      column.type->getName(), backQuote(dictionary_name), dictionary_names_and_types.toNamesAndTypesDescription());
    }
  }
}
}  // namespace
...
NamesAndTypesList StorageDictionary::getNamesAndTypes(const DictionaryStructure& dictionary_structure, bool validate_id_type) {
  ...
  if (dictionary_structure.id) {
    if (validate_id_type && dictionary_structure.id->type->getTypeId() != TypeIndex::UInt64)
      throw Exception(ErrorCodes::BAD_ARGUMENTS, "Incorrect type of ID column: must be UInt64, but it is {}",
                      dictionary_structure.id->type->getFamilyName());
  ...
  if (dictionary_structure.key) {
    for (const auto& attribute : *dictionary_structure.key) dictionary_names_and_types.emplace_back(attribute.name, attribute.type);
  }

  for (const auto& attribute : dictionary_structure.attributes) {
    /// Some attributes can be already added (range_min and range_max)
    if (!dictionary_names_and_types.contains(attribute.name)) dictionary_names_and_types.emplace_back(attribute.name, attribute.type);
  }

  return dictionary_names_and_types;
}

StorageDictionary::StorageDictionary(const StorageID& table_id_, const String& dictionary_name_, const ColumnsDescription& columns_,
                                     const String& comment, Location location_, ContextPtr context_)
...
StorageDictionary::StorageDictionary(const StorageID& table_id_, const String& dictionary_name_,
                                     const DictionaryStructure& dictionary_structure_, const String& comment, Location location_,
                                     ContextPtr context_)
    : StorageDictionary(table_id_, dictionary_name_,
                        ColumnsDescription{getNamesAndTypes(dictionary_structure_,
                                                            context_->getSettingsRef()[Setting::dictionary_validate_primary_key_type])},
                        comment, location_, context_) {}

StorageDictionary::StorageDictionary(const StorageID& table_id, LoadablesConfigurationPtr dictionary_configuration, ContextPtr context_)
    : StorageDictionary(table_id, table_id.getFullNameNotQuoted(),
                        context_->getExternalDictionariesLoader().getDictionaryStructure(
                            *dictionary_configuration),  /// NOLINT(readability-static-accessed-through-instance)
                        dictionary_configuration->getString("dictionary.comment", ""), Location::SameDatabaseAndNameAsDictionary,
                        context_) {
...
StorageDictionary::~StorageDictionary() { removeDictionaryConfigurationFromRepository(); }
...
void StorageDictionary::checkTableCanBeDropped([[maybe_unused]] ContextPtr query_context) const {
  if (location == Location::SameDatabaseAndNameAsDictionary)
    throw Exception(ErrorCodes::CANNOT_DETACH_DICTIONARY_AS_TABLE,
                    "Cannot drop/detach dictionary {} as table, use DROP DICTIONARY or DETACH DICTIONARY query instead", dictionary_name);
  if (location == Location::DictionaryDatabase)
    throw Exception(
        ErrorCodes::CANNOT_DETACH_DICTIONARY_AS_TABLE,
...
Pipe StorageDictionary::read(const Names& column_names, const StorageSnapshotPtr& /*storage_snapshot*/, SelectQueryInfo& /*query_info*/,
   ...
   */
  bool has_dict_get = local_context->getAccess()->isGranted(AccessType::dictGet, dictionary->getDatabaseOrNoDatabaseTag(),
                                                            dictionary->getDictionaryID().getTableName());
  bool has_select = local_context->getAccess()->isGranted(AccessType::SELECT, dictionary->getDatabaseOrNoDatabaseTag(),
                                                          dictionary->getDictionaryID().getTableName());
...
void StorageDictionary::shutdown(bool) { removeDictionaryConfigurationFromRepository(); }
...
void StorageDictionary::removeDictionaryConfigurationFromRepository() { remove_repository_callback.reset(); }
...
void StorageDictionary::renameInMemory(const StorageID& new_table_id) {
  ...
  if (location != Location::SameDatabaseAndNameAsDictionary) return;
  ...
  if (recreate_dictionary) {
    ...
    /// so we cannot change the dictionary name or the database name on the fly (without extra reloading) and have to recreate the config
    /// repository.
...
void StorageDictionary::alter(const AlterCommands& params, ContextPtr alter_context, AlterLockHolder& lock_holder) {
  ...
  if (location == Location::Custom) return;
  ...
  factory.registerStorage("Dictionary", [](const StorageFactory::Arguments& args) {
    ...
    if (query.is_dictionary) {
      ...
      if (external_dictionaries_loader.getCurrentStatus(dictionary_id.getFullNameNotQuoted()) != ExternalLoader::Status::NOT_EXIST)
        throw Exception(ErrorCodes::DICTIONARY_ALREADY_EXISTS, "Dictionary {} already exists.", dictionary_id.getFullNameNotQuoted());
    ...
    return std::make_shared<StorageDictionary>(args.table_id, dictionary_name, args.columns, args.comment,
                                               StorageDictionary::Location::Custom, local_context);
  });
}
