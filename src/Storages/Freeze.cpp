...
bool FreezeMetaData::load(DiskPtr data_disk, const String &path) {
  ...
  if (!metadata_storage->existsFile(file_path)) return false;
...
String FreezeMetaData::getFileName(const String &path) { return fs::path(path) / "frozen_metadata.txt"; }

Unfreezer::Unfreezer(ContextPtr context) : local_context(context) {
  if (local_context->hasZooKeeper()) zookeeper = local_context->getZooKeeper();
}
...
BlockIO Unfreezer::systemUnfreeze(const String &backup_name) {
  ...
  if (!config.getBool(config_key, false)) {
    throw Exception(ErrorCodes::SUPPORT_IS_DISABLED,
                    "Support for SYSTEM UNFREEZE query is disabled. You can enable it via '{}' server setting", config_key);
  }
  ...
  for (auto &[_, disk] : disks_map) {
    ...
    for (const auto &store_path : store_paths) {
      if (!disk->existsDirectory(store_path)) continue;
      ...
      for (auto prefix_it = disk->iterateDirectory(store_path); prefix_it->isValid(); prefix_it->next()) {
        ...
        for (auto table_it = disk->iterateDirectory(prefix_directory); table_it->isValid(); table_it->next()) {
          ...
          auto current_result_info =
              unfreezePartitionsFromTableDirectory([](const String &) { return true; }, backup_name, {disk}, table_directory);
          for (auto &command_result : current_result_info) command_result.command_type = "SYSTEM UNFREEZE";
          result_info.insert(result_info.end(), std::make_move_iterator(current_result_info.begin()),
...
bool Unfreezer::removeFrozenPart(DiskPtr disk, const String &path, const String &part_name, ContextPtr local_context,
                                 zkutil::ZooKeeperPtr zookeeper) {
  if (disk->supportZeroCopyReplication()) {
    FreezeMetaData meta;
    if (meta.load(disk, path)) {
      FreezeMetaData::clean(disk, path);
      return StorageReplicatedMergeTree::removeSharedDetachedPart(disk, path, part_name, meta.table_shared_id, meta.replica_name, "",
                                                                  local_context, zookeeper);
    }
  }
...
PartitionCommandsResultInfo Unfreezer::unfreezePartitionsFromTableDirectory(MergeTreeData::MatcherFn matcher, const String &backup_name,
                                                                            const Disks &disks, const fs::path &table_directory) {
  PartitionCommandsResultInfo result;

  for (const auto &disk : disks) {
    if (!disk->existsDirectory(table_directory)) continue;
    ...
    for (auto it = disk->iterateDirectory(table_directory); it->isValid(); it->next()) {
      ...
      if (!matcher(partition_id)) continue;
