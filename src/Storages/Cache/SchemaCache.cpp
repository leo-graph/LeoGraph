...
void SchemaCache::addManyColumns(const Keys& keys, const ColumnsDescription& columns) {
  std::lock_guard lock(mutex);
  for (const auto& key : keys) addUnlocked(key, columns, std::nullopt);
}
...
void SchemaCache::addUnlocked(const Key& key, const std::optional<ColumnsDescription>& columns, std::optional<size_t> num_rows) {
  ...
  if (auto it = data.find(key); it != data.end()) {
    if (columns) it->second.schema_info.columns = columns;
    if (num_rows) it->second.schema_info.num_rows = num_rows;
    return;
  }
...
void SchemaCache::checkOverflow() {
  if (queue.size() <= max_elements) return;
...
std::optional<ColumnsDescription> SchemaCache::tryGetColumns(const DB::SchemaCache::Key& key,
                                                             DB::SchemaCache::LastModificationTimeGetter get_last_mod_time) {
  auto schema_info = tryGetImpl(key, get_last_mod_time);
  if (!schema_info) return std::nullopt;
...
std::optional<size_t> SchemaCache::tryGetNumRows(const DB::SchemaCache::Key& key,
                                                 DB::SchemaCache::LastModificationTimeGetter get_last_mod_time) {
  auto schema_info = tryGetImpl(key, get_last_mod_time);
  if (!schema_info) return std::nullopt;
...
std::optional<SchemaCache::SchemaInfo> SchemaCache::tryGetImpl(const Key& key, LastModificationTimeGetter get_last_mod_time) {
  ...
  if (get_last_mod_time) {
    ...
    if (!last_mod_time) return std::nullopt;
...
std::unordered_map<SchemaCache::Key, SchemaCache::SchemaInfo, SchemaCache::KeyHash> SchemaCache::getAll() {
  ...
  for (const auto& [key, value] : data) result[key] = value.schema_info;
