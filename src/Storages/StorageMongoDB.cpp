...
void MongoDBConfiguration::checkHosts(const ContextPtr &context) const {
  ...
  for (const auto &host : uri->hosts()) context->getRemoteHostFilter().checkHostAndPort(host.name, toString(host.port));
}

StorageMongoDB::StorageMongoDB(const StorageID &table_id_, MongoDBConfiguration configuration_, const ColumnsDescription &columns_,
...
Pipe StorageMongoDB::read(const Names &column_names, const StorageSnapshotPtr &storage_snapshot, SelectQueryInfo &query_info,
                          ContextPtr context, QueryProcessingStage::Enum /*processed_stage*/, size_t max_block_size,
                          size_t /*num_streams*/) {
  ...
  return Pipe(std::make_shared<MongoDBSource>(*configuration.uri, configuration.collection, mongo_query, std::move(options),
                                              std::make_shared<const Block>(std::move(sample_block)), max_block_size));
}
...
MongoDBConfiguration StorageMongoDB::getConfigurationFromCollection(MutableNamedCollectionPtr named_collection, ContextPtr context) {
  ...
  if (named_collection->has("uri")) {
    ...
    validateNamedCollection(*named_collection, {"host", "port", "user", "password", "database", "collection"}, {"options", "oid_columns"});
    ...
    configuration.uri = std::make_unique<mongocxx::uri>(
        fmt::format("mongodb://{}{}:{}/{}?{}", auth_string, named_collection->get<String>("host"), named_collection->get<String>("port"),
                    named_collection->get<String>("database"), named_collection->getOrDefault<String>("options", "")));
  }
...
static MongoDBConfiguration getConfigurationImpl(const StorageID *table_id, ASTs engine_args, ContextPtr context,
                                                 bool allow_excessive_path_in_host) {
  if (auto named_collection = tryGetNamedCollectionWithOverrides(engine_args, context))
    return StorageMongoDB::getConfigurationFromCollection(named_collection, context);

  MongoDBConfiguration configuration;

  for (auto &engine_arg : engine_args) engine_arg = evaluateConstantExpressionOrIdentifierAsLiteral(engine_arg, context);

  if (engine_args.size() >= 5 && engine_args.size() <= 7) {
    configuration.collection = checkAndGetLiteralArgument<String>(engine_args[2], "collection");

    String options;
    if (engine_args.size() >= 6) options = checkAndGetLiteralArgument<String>(engine_args[5], "options");
    ...
    try {
      ...
      if (!allow_excessive_path_in_host || pos == String::npos) throw;

      LOG_WARNING(getLogger("StorageMongoDB"),
                  "Failed to parse MongoDB connection string: '{}', trying to remove everything after slash from the hostname", e.what());
    ...
    if (engine_args.size() == 7)
      boost::split(configuration.oid_fields, checkAndGetLiteralArgument<String>(engine_args[6], "oid_columns"), boost::is_any_of(","));
    ...
    if (engine_args.size() == 3)
      boost::split(configuration.oid_fields, checkAndGetLiteralArgument<String>(engine_args[2], "oid_columns"), boost::is_any_of(","));
  } else
...
static std::string mongoFuncName(const std::string &func) {
  if (func == "equals") return "$eq";
  if (func == "notEquals") return "$ne";
  if (func == "greaterThan" || func == "greater") return "$gt";
  if (func == "lessThan" || func == "less") return "$lt";
  if (func == "greaterOrEquals") return "$gte";
  if (func == "lessOrEquals") return "$lte";
  if (func == "in") return "$in";
  if (func == "notIn") return "$nin";
  if (func == "lessThan") return "$lt";
  if (func == "and") return "$and";
  if (func == "or") return "$or";

  return "";
}

static std::string invertMongoComparison(const std::string &func) {
  if (func == "$gt") return "$lt";
  if (func == "$gte") return "$lte";
  if (func == "$lt") return "$gt";
  if (func == "$lte") return "$gte";
...
static const ColumnNode *getColumnNode(const QueryTreeNodePtr &node, const JoinNode *join_node, const StorageID &storage_id) {
  ...
  if (!table && !table_function) return {};

  // Skip columns from other tables in JOIN queries.
  if (table && table->getStorage()->getStorageID() != storage_id) return {};
  if (table_function && table_function->getStorage()->getStorageID() != storage_id) return {};
  if (join_node && column->getColumnSource() != join_node->getLeftTableExpression()) return {};

  return column;
}

std::optional<bsoncxx::document::value> StorageMongoDB::visitWhereConstant(const ContextPtr &context, const ConstantNode *const_node,
...
std::optional<bsoncxx::document::value> StorageMongoDB::visitWhereFunctionArguments(const ColumnNode *column_node,
                                                                                    const ConstantNode *const_node,
                                                                                    const FunctionNode *func, bool invert_comparison) {
  ...
  auto func_value = BSONCXXHelper::fieldAsBSONValue(const_value, const_type, configuration.isOidColumn(column_node->getColumnName()));

  if (func_name == "$in" && func_value.view().type() != bsoncxx::v_noabi::type::k_array) func_name = "$eq";
  if (func_name == "$nin" && func_value.view().type() != bsoncxx::v_noabi::type::k_array) func_name = "$ne";
...
std::optional<bsoncxx::document::value> StorageMongoDB::visitWhereFunction(const ContextPtr &context, const FunctionNode *func,
  ...
  if (func->getArguments().getNodes().empty()) return {};
  ...
  if (argumnet_nodes.size() == 2) {
    ...
    if (left_column && right_const) return visitWhereFunctionArguments(left_column, right_const, func, false);

    /// We cannot invert "in" and "notIn" functions.
    if (func->getFunctionName() == "in" || func->getFunctionName() == "notIn") return {};
    ...
    if (left_const && right_column) return visitWhereFunctionArguments(right_column, left_const, func, true);
  }
...
std::optional<bsoncxx::document::value> StorageMongoDB::visitWhereNode(const ContextPtr &context, const QueryTreeNodePtr &where_node,
                                                                       const JoinNode *join_node) {
  if (const auto *func = where_node->as<FunctionNode>()) return visitWhereFunction(context, func, join_node);

  if (const auto *const_expr = where_node->as<ConstantNode>()) return visitWhereConstant(context, const_expr, join_node);

  return {};
}

bsoncxx::document::value StorageMongoDB::buildMongoDBQuery(const ContextPtr &context, mongocxx::options::find &options,
                                                           const SelectQueryInfo &query, const Block &sample_block) {
  document projection{};
  for (const auto &column : sample_block) projection.append(kvp(column.name, 1));
  ...
  if (!context->getSettingsRef()[Setting::allow_experimental_analyzer]) {
    if (throw_on_error) throw Exception(ErrorCodes::NOT_IMPLEMENTED, "MongoDB storage does not support 'enable_analyzer = 0' setting");
  ...
  if (throw_on_error) {
    if (query_tree.hasHaving())
      throw Exception(ErrorCodes::NOT_IMPLEMENTED,
                      "HAVING section is not supported. You can disable this error with 'SET mongodb_throw_on_unsupported_query=0', but "
                      "this may cause poor performance, and is highly not recommended");
    if (query_tree.hasGroupBy())
      throw Exception(ErrorCodes::NOT_IMPLEMENTED,
                      "GROUP BY section is not supported. You can disable this error with 'SET mongodb_throw_on_unsupported_query=0', but "
                      "this may cause poor performance, and is highly not recommended");
    if (query_tree.hasWindow())
      throw Exception(ErrorCodes::NOT_IMPLEMENTED,
                      "WINDOW section is not supported. You can disable this error with 'SET mongodb_throw_on_unsupported_query=0', but "
                      "this may cause poor performance, and is highly not recommended");
    if (query_tree.hasPrewhere())
      throw Exception(ErrorCodes::NOT_IMPLEMENTED,
                      "PREWHERE section is not supported. You can disable this error with 'SET mongodb_throw_on_unsupported_query=0', but "
                      "this may cause poor performance, and is highly not recommended");
    if (query_tree.hasLimitBy())
      throw Exception(ErrorCodes::NOT_IMPLEMENTED,
                      "LIMIT BY section is not supported. You can disable this error with 'SET mongodb_throw_on_unsupported_query=0', but "
                      "this may cause poor performance, and is highly not recommended");
  }
  ...
  auto on_error = [&](const auto *node) {
    ...
    if (throw_on_error)
      throw Exception(ErrorCodes::NOT_IMPLEMENTED,
                      ...
                      "You can disable this restriction with 'SET mongodb_throw_on_unsupported_query=0', to read the full table and "
                      "process on ClickHouse side (this may cause poor performance)",
  ...
  if (query_tree.hasOrderBy()) {
    ...
    for (const auto &child : query_tree.getOrderByNode()->getChildren()) {
      if (const auto *sort_node = child->as<SortNode>()) {
        if (sort_node->withFill() || sort_node->hasFillTo() || sort_node->hasFillFrom() || sort_node->hasFillStep()) on_error(sort_node);
  ...
  factory.registerStorage(
      ...
      [](const StorageFactory::Arguments &args) {
        ...
        return std::make_shared<StorageMongoDB>(
            args.table_id, getConfigurationImpl(&args.table_id, args.engine_args, args.getLocalContext(), allow_excessive_path_in_host),
