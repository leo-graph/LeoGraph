...
class LogSource final : public ISource {
 public:
  static Block getHeader(const NamesAndTypesList &columns) {
    ...
    for (const auto &name_type : columns) res.insert({name_type.type->createColumn(), name_type.type, name_type.name});

    return res;
  }

  LogSource(size_t block_size_, const NamesAndTypesList &columns_, std::shared_ptr<const StorageLog> storage_, size_t rows_limit_,
            const std::vector<size_t> &offsets_, const std::vector<size_t> &file_sizes_, bool limited_by_file_sizes_,
 ...
 private:
  ...
  struct Stream {
    Stream(const DiskPtr &disk, const String &data_path, size_t offset, size_t file_size, bool limited_by_file_size,
           ReadSettings read_settings_) {
      plain = disk->readFile(data_path, read_settings_.adjustBufferSize(file_size));

      if (offset) plain->seek(offset, SEEK_SET);
  ...
  void readPrefix(const NameAndTypePair &name_and_type, ISerialization::SubstreamsCache &cache,
                  ISerialization::SubstreamsDeserializeStatesCache &deserialize_state_cache);
...
String LogSource::getCacheKey(const NameAndTypePair &name_and_type_on_disk) const {
  ...
  if (Nested::isSubcolumnOfNested(name_in_storage, storage->columns_with_collected_nested)) return Nested::splitName(name_in_storage).first;
...
Chunk LogSource::generate() {
  ...
  for (const auto &name_type : columns) {
    ...
    if (!column->empty()) res.insert(ColumnWithTypeAndName(column, name_type_on_disk.type, name_type_on_disk.name));
  }

  if (!res.empty()) rows_read += res.rows();

  if (res.empty()) is_finished = true;
...
void LogSource::readPrefix(const NameAndTypePair &name_and_type, ISerialization::SubstreamsCache &cache,
                           ISerialization::SubstreamsDeserializeStatesCache &deserialize_state_cache) {
  if (deserialize_states.contains(name_and_type.name)) return;
  ...
  settings.getter = [&](const ISerialization::SubstreamPath &path) -> ReadBuffer * {
    if (cache.contains(ISerialization::getSubcolumnNameForStream(path))) return nullptr;
    ...
    auto it =
        streams.try_emplace(data_file_name, storage->disk, data_file.path, offset, file_size, limited_by_file_sizes, read_settings).first;
...
void LogSource::readData(const NameAndTypePair &name_and_type, ColumnPtr &column, size_t max_rows_to_read,
  ...
  if (column->getDataType() != name_and_type.type->getColumnType())
    throw Exception(ErrorCodes::LOGICAL_ERROR, "Unexpected return type when reading column '{}' from {}. Expected {}. Got {}",
                    name_and_type.name, storage->getStorageID().getFullTableName(), name_and_type.type->getColumnType(),
                    column->getDataType());
}
...
class LogSink final : public SinkToStorage {
 ...
 private:
  ...
  struct Stream {
    Stream(const DiskPtr &disk, const String &data_path, size_t initial_data_size, CompressionCodecPtr codec,
...
void LogSink::consume(Chunk &chunk) {
  ...
  for (auto &stream : streams | boost::adaptors::map_values) stream.written = false;
...
void LogSink::onFinish() {
  ...
  /// Finish write.
  for (auto &stream : streams | boost::adaptors::map_values) stream.finalize();
  streams.clear();
...
ISerialization::OutputStreamGetter LogSink::createStreamGetter(const NameAndTypePair &name_and_type) {
  return [&](const ISerialization::SubstreamPath &path) -> WriteBuffer * {
    ...
    if (it == streams.end()) throw Exception(ErrorCodes::LOGICAL_ERROR, "Stream was not created when writing data in LogSink");

    Stream &stream = it->second;
    if (stream.written) return nullptr;
...
CompressionCodecPtr LogSink::getCodecOrDefault(const String &column_name, CompressionCodecPtr default_codec) const {
  auto get_codec_or_default = [&default_codec](const auto &column_desc) {
    return column_desc.codec ? CompressionCodecFactory::instance().get(column_desc.codec, column_desc.type, default_codec) : default_codec;
  };

  const auto &columns = metadata_snapshot->getColumns();
  if (const auto *column_desc = columns.tryGet(column_name)) return get_codec_or_default(*column_desc);
...
void LogSink::writeData(const NameAndTypePair &name_and_type, const IColumn &column) {
  ...
  serialization->enumerateStreams([&](const ISerialization::SubstreamPath &path) {
    ...
    if (it == streams.end()) {
      ...
      it = streams
               .try_emplace(data_file.name, storage.disk, data_file.path, storage.file_checker.getFileSize(data_file.path), compression,
  ...
  if (!serialize_states.contains(name)) serialization->serializeBinaryBulkStatePrefix(column, settings, serialize_states[name]);
...
void checkSupportedDataTypes(const ColumnsDescription &columns, const String &storage_name) {
  for (const auto &column : columns.getAll()) {
    auto callback = [&](const IDataType &type) {
      ...
      if (isVariant(type)) throw Exception(ErrorCodes::ILLEGAL_COLUMN, "Engine {} doesn't support Variant data type", storage_name);
    };
...
StorageLog::StorageLog(const String &engine_name_, DiskPtr disk_, const String &relative_path_, const StorageID &table_id_,
                       const ColumnsDescription &columns_, const ConstraintsDescription &constraints_, const String &comment,
                       LoadingStrictnessLevel mode, ContextMutablePtr context_)
    : IStorage(table_id_),
  ...
  if (relative_path_.empty()) throw Exception(ErrorCodes::INCORRECT_FILE_NAME, "Storage {} requires data path", getName());

  /// Enumerate data files.
  for (const auto &column : storage_metadata.getColumns().getAllPhysical()) addDataFiles(column);

  /// Ensure the file checker is initialized.
  if (file_checker.empty()) {
    for (const auto &data_file : data_files) file_checker.setEmpty(data_file.path);
    if (use_marks_file) file_checker.setEmpty(marks_file_path);
  }
...
void StorageLog::addDataFiles(const NameAndTypePair &column) {
  if (data_files_by_names.contains(column.name))
    throw Exception(ErrorCodes::DUPLICATE_COLUMN, "Duplicate column with name {} in constructor of StorageLog.", column.name);
  ...
  for (auto &data_file : data_files) data_files_by_names[data_file.name] = &data_file;
}

void StorageLog::loadMarks(std::chrono::seconds lock_timeout) {
  if (!use_marks_file || marks_loaded) return;
...
void StorageLog::loadMarks(const WriteLock &lock /* already locked exclusively */) {
  ...
  if (disk->existsFile(marks_file_path)) {
    ...
    for (auto &data_file : data_files) data_file.marks.resize(num_marks);
...
void StorageLog::saveMarks(const WriteLock & /* already locked for writing */) {
  if (!use_marks_file) return;

  size_t num_marks = num_data_files ? data_files[0].marks.size() : 0;
  if (num_marks_saved == num_marks) return;
...
void StorageLog::removeUnsavedMarks(const WriteLock & /* already locked for writing */) {
  ...
  for (auto &data_file : data_files) {
    if (data_file.marks.size() > num_marks_saved) data_file.marks.resize(num_marks_saved);
  }
}

void StorageLog::saveFileSizes(const WriteLock & /* already locked for writing */) {
  for (const auto &data_file : data_files) file_checker.update(data_file.path);

  if (use_marks_file) file_checker.update(marks_file_path);
...
void StorageLog::rename(const String &new_path_to_table_data, const StorageID &new_table_id) {
  assert(table_path != new_path_to_table_data);
  {
    ...
    for (auto &data_file : data_files) data_file.path = table_path + fileName(data_file.path);
...
void StorageLog::drop() { disk->removeRecursive(table_path); }
...
void StorageLog::truncate(const ASTPtr &, const StorageMetadataPtr &, ContextPtr local_context, TableExclusiveLockHolder &) {
  ...
  for (auto &data_file : data_files) clear_tx->removeFileIfExists(data_file.path);

  if (use_marks_file) clear_tx->removeFileIfExists(marks_file_path);
...
Pipe StorageLog::createReadingPipe(const Names &column_names, ContextPtr local_context, const StorageSnapshotPtr &storage_snapshot,
  ...
  for (const auto &data_file : data_files) file_sizes[data_file.index] = file_checker.getFileSize(data_file.path);
  ...
  for (size_t stream = 0; stream < num_streams; ++stream) {
    if (use_marks_file) {
      ...
      for (const auto &data_file : data_files) offsets[data_file.index] = data_file.marks[mark_begin].offset;
    }

    pipes.emplace_back(std::make_shared<LogSource>(max_block_size, all_columns,
                                                   std::static_pointer_cast<const StorageLog>(shared_from_this()), row_limit, offsets,
                                                   file_sizes, limited_by_file_sizes, read_settings));
  }
...
void StorageLog::read(QueryPlan &plan, const Names &column_names, const StorageSnapshotPtr &storage_snapshot,
                      SelectQueryInfo & /*query_info*/, ContextPtr local_context, QueryProcessingStage::Enum /*processed_stage*/,
                      size_t max_block_size, size_t num_streams) {
  storage_snapshot->check(column_names);

  plan.addStep(std::make_unique<ReadFromStorageLogStep>(column_names, local_context,
                                                        std::static_pointer_cast<StorageLog>(shared_from_this()), storage_snapshot,
...
IStorage::DataValidationTasksPtr StorageLog::getCheckTaskList(const std::variant<std::monostate, ASTPtr, String> &check_task_filter,
...
std::optional<UInt64> StorageLog::totalRows(ContextPtr) const {
  if (use_marks_file && marks_loaded) return total_rows;
...
std::optional<UInt64> StorageLog::totalBytes(ContextPtr) const { return total_bytes; }

void StorageLog::backupData(BackupEntriesCollector &backup_entries_collector, const String &data_path_in_backup,
  ...
  if (!num_data_files || !file_checker.getFileSize(data_files[INDEX_WITH_REAL_ROW_COUNT].path)) return;
  ...
  backup_entries_collector.addBackupEntry(data_path_in_backup_fs / fileName(files_info_path),
                                          std::make_unique<BackupEntryFromSmallFile>(disk, files_info_path, read_settings, copy_encrypted));
  ...
  if (use_marks_file) {
    ...
    backup_entries_collector.addBackupEntry(data_path_in_backup_fs / "count.txt",
                                            std::make_unique<BackupEntryFromMemory>(toString(num_rows)));
  }
}

void StorageLog::restoreDataFromBackup(RestorerFromBackup &restorer, const String &data_path_in_backup,
                                       const std::optional<ASTs> & /* partitions */) {
  auto backup = restorer.getBackup();
  if (!backup->hasFiles(data_path_in_backup)) return;

  if (!num_data_files) return;

  if (!restorer.isNonEmptyTableAllowed() && total_bytes) RestorerFromBackup::throwTableIsNotEmpty(getStorageID());

  auto lock_timeout = getLockTimeout(restorer.getContext());
  restorer.addDataRestoreTask([storage = std::static_pointer_cast<StorageLog>(shared_from_this()), backup, data_path_in_backup,
                               lock_timeout] { storage->restoreDataImpl(backup, data_path_in_backup, lock_timeout); });
}
...
void StorageLog::restoreDataImpl(const BackupPtr &backup, const String &data_path_in_backup, std::chrono::seconds lock_timeout) {
  ...
  try {
    ...
    if (use_marks_file) {
      ...
      for (auto &data_file : data_files) data_file.marks.reserve(num_marks + num_extra_marks);
...
SharedHeader getHeader(const Names &column_names, const StorageSnapshotPtr &storage_snapshot) {
  auto options = GetColumnsOptions(GetColumnsOptions::AllPhysical).withSubcolumns();
  auto all_columns = storage_snapshot->getColumnsByNames(options, column_names);
  all_columns = Nested::convertToSubcolumns(all_columns);
...
ReadFromStorageLogStep::ReadFromStorageLogStep(const Names &column_names_, ContextPtr local_context_, std::shared_ptr<StorageLog> storage_,
                                               const StorageSnapshotPtr &storage_snapshot_, size_t max_block_size_, size_t num_streams_)
...
void registerStorageLog(StorageFactory &factory) {
  ...
  auto create_fn = [](const StorageFactory::Arguments &args) {
    if (!args.engine_args.empty())
      throw Exception(ErrorCodes::NUMBER_OF_ARGUMENTS_DOESNT_MATCH, "Engine {} doesn't support any arguments ({} given)", args.engine_name,
                      args.engine_args.size());

    String disk_name = getDiskName(*args.storage_def, args.getContext());
    DiskPtr disk = args.getContext()->getDisk(disk_name);

    return std::make_shared<StorageLog>(args.engine_name, disk, args.relative_data_path, args.table_id, args.columns, args.constraints,
                                        args.comment, args.mode, args.getContext());
  };
