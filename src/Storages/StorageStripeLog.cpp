...
class StripeLogSource final : public ISource {
 public:
  static Block getHeader(const StorageSnapshotPtr &storage_snapshot, const Names &column_names,
                         IndexForNativeFormat::Blocks::const_iterator index_begin, IndexForNativeFormat::Blocks::const_iterator index_end) {
    if (index_begin == index_end) return storage_snapshot->getSampleBlockForColumns(column_names);
  ...
  StripeLogSource(std::shared_ptr<const StorageStripeLog> storage_, const StorageSnapshotPtr &storage_snapshot_, const Names &column_names,
                  ReadSettings read_settings_, std::shared_ptr<const IndexForNativeFormat> indices_,
                  IndexForNativeFormat::Blocks::const_iterator index_begin_, IndexForNativeFormat::Blocks::const_iterator index_end_,
                  size_t file_size_)
 ...
 public:
  ...
  explicit StripeLogSink(StorageStripeLog &storage_, const StorageMetadataPtr &metadata_snapshot_, WriteLock &&lock_)
      : SinkToStorage(std::make_shared<const Block>(metadata_snapshot_->getSampleBlock())),
        ...
        data_out(std::make_unique<CompressedWriteBuffer>(*data_out_compressed, CompressionCodecFactory::instance().getDefaultCodec(),
    ...
    block_out = std::make_unique<NativeWriter>(*data_out, 0, std::make_shared<const Block>(metadata_snapshot->getSampleBlock()),
                                               std::nullopt, false, &storage.indices, initial_data_size);
  }
  ...
  void consume(Chunk &chunk) override { block_out->write(getHeader().cloneWithColumns(chunk.getColumns())); }
...
StorageStripeLog::StorageStripeLog(DiskPtr disk_, const String &relative_path_, const StorageID &table_id_,
...
Pipe StorageStripeLog::read(const Names &column_names, const StorageSnapshotPtr &storage_snapshot, SelectQueryInfo & /*query_info*/,
  ...
  for (size_t stream = 0; stream < num_streams; ++stream) {
    ...
    pipes.emplace_back(std::make_shared<StripeLogSource>(std::static_pointer_cast<const StorageStripeLog>(shared_from_this()),
                                                         storage_snapshot, column_names, read_settings, indices_for_selected_columns, begin,
                                                         end, data_file_size));
  }
...
IStorage::DataValidationTasksPtr StorageStripeLog::getCheckTaskList(const std::variant<std::monostate, ASTPtr, String> &check_task_filter,
...
void StorageStripeLog::saveIndices(const WriteLock & /* already locked for writing */) {
  ...
  if (num_indices_saved == num_indices) return;
  ...
  for (size_t i = start; i != num_indices; ++i) indices.blocks[i].write(*index_out);
...
void StorageStripeLog::removeUnsavedIndices(const WriteLock & /* already locked for writing */) {
  if (indices.blocks.size() > num_indices_saved) indices.blocks.resize(num_indices_saved);
}
...
void StorageStripeLog::updateTotalRows(const WriteLock &) {
  ...
  for (const auto &block : indices.blocks) new_total_rows += block.num_rows;
  total_rows = new_total_rows;
}

std::optional<UInt64> StorageStripeLog::totalRows(ContextPtr) const {
  if (indices_loaded) return total_rows;
...
std::optional<UInt64> StorageStripeLog::totalBytes(ContextPtr) const { return total_bytes; }

void StorageStripeLog::backupData(BackupEntriesCollector &backup_entries_collector, const String &data_path_in_backup,
  ...
  if (!file_checker.getFileSize(data_file_path)) return;
  ...
  /// count.txt
  size_t num_rows = 0;
  for (const auto &block : indices.blocks) num_rows += block.num_rows;
...
void StorageStripeLog::restoreDataFromBackup(RestorerFromBackup &restorer, const String &data_path_in_backup,
                                             const std::optional<ASTs> & /* partitions */) {
  ...
  restorer.addDataRestoreTask([storage = std::static_pointer_cast<StorageStripeLog>(shared_from_this()), backup, data_path_in_backup,
  ...
  try {
    ...
    /// Append the index.
    {
      ...
      for (auto &block : extra_indices.blocks) {
        for (auto &column : block.columns) column.location.offset_in_compressed_file += old_data_size;
      }
...
void registerStorageStripeLog(StorageFactory &factory) {
  ...
  factory.registerStorage(
      ...
      [](const StorageFactory::Arguments &args) {
        ...
        return std::make_shared<StorageStripeLog>(disk, args.relative_data_path, args.table_id, args.columns, args.constraints,
