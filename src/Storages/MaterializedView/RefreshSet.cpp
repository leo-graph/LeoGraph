...
RefreshSet::Handle::Handle(Handle&& other) noexcept { *this = std::move(other); }
...
RefreshSet::Handle::~Handle() { reset(); }
...
void RefreshSet::Handle::rename(StorageID new_id, std::optional<StorageID> new_inner_table_id) {
  ...
  if (inner_table_id) parent_set->removeInnerTableLocked(*inner_table_id, inner_table_iter);
  id = new_id;
  inner_table_id = new_inner_table_id;
  iter = parent_set->addTaskLocked(id, task);
  if (inner_table_id) inner_table_iter = parent_set->addInnerTableLocked(*inner_table_id, task);
...
void RefreshSet::emplace(StorageID id, std::optional<StorageID> inner_table_id, const std::vector<StorageID>& dependencies,
                         RefreshTaskPtr task) {
  std::lock_guard guard(mutex);
  const auto iter = addTaskLocked(id, task);
  RefreshTaskList::iterator inner_table_iter;
  if (inner_table_id) inner_table_iter = addInnerTableLocked(*inner_table_id, task);
...
void RefreshSet::removeTaskLocked(StorageID id, RefreshTaskList::iterator iter) {
  ...
  if (it->second.empty()) tasks.erase(it);
}
...
void RefreshSet::removeInnerTableLocked(StorageID inner_table_id, RefreshTaskList::iterator inner_table_iter) {
  ...
  if (it->second.empty()) inner_tables.erase(it);
}

void RefreshSet::addDependenciesLocked(RefreshTaskPtr task, const std::vector<StorageID>& dependencies) {
  for (const StorageID& dep : dependencies) dependents[dep].insert(task);
}
...
void RefreshSet::removeDependenciesLocked(RefreshTaskPtr task, const std::vector<StorageID>& dependencies) {
  for (const StorageID& dep : dependencies) {
    ...
    set.erase(task);
    if (set.empty()) dependents.erase(dep);
  }
}

RefreshTaskList RefreshSet::findTasks(const StorageID& id) const {
  std::lock_guard lock(mutex);
  if (auto it = tasks.find(id); it != tasks.end()) return it->second;
  return {};
}
...
std::vector<RefreshTaskPtr> RefreshSet::getTasks() const {
  ...
  for (const auto& [_, list] : tasks)
    for (const auto& task : list) res.push_back(task);
  return res;
}
...
RefreshTaskPtr RefreshSet::tryGetTaskForInnerTable(const StorageID& inner_table_id) const {
  ...
  if (it == inner_tables.end()) return nullptr;
...
void RefreshSet::notifyDependents(const StorageID& id) const {
  std::vector<RefreshTaskPtr> res;
  {
    ...
    for (const auto& task : it->second) res.push_back(task);
  }
  for (const RefreshTaskPtr& t : res) t->notify();
}
...
void RefreshSet::setRefreshesStopped(bool stopped) {
  ...
  {
    std::lock_guard lock(mutex);
    if (refreshes_stopped.exchange(stopped) == stopped) return;
    if (stopped) refreshes_stopped_at = std::chrono::steady_clock::now();
    tasks_copy = tasks;
  }
  for (const auto& kv : tasks_copy)
    for (const RefreshTaskPtr& t : kv.second) t->notify();
}

bool RefreshSet::refreshesStopped() const { return refreshes_stopped.load(); }
...
bool RefreshSet::joinBackgroundTasks(std::chrono::steady_clock::time_point deadline) {
  ...
  std::chrono::steady_clock::time_point stopped_at;
  {
    ...
    for (const auto& [_, list] : tasks) remaining_tasks.insert(remaining_tasks.end(), list.begin(), list.end());
  }
  std::erase_if(remaining_tasks, [&](const auto& t) { return t->tryJoinBackgroundTask(deadline); });
...
RefreshSet::Handle::Handle(RefreshSet* parent_set_, StorageID id_, std::optional<StorageID> inner_table_id_,
                           RefreshTaskList::iterator iter_, RefreshTaskList::iterator inner_table_iter_,
                           std::vector<StorageID> dependencies_)
    : parent_set(parent_set_),
      id(std::move(id_)),
      inner_table_id(std::move(inner_table_id_)),
      dependencies(std::move(dependencies_)),
      iter(iter_),
      inner_table_iter(inner_table_iter_),
