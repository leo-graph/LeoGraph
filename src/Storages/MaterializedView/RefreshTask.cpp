...
RefreshTask::RefreshTask(StorageMaterializedView *view_, ContextPtr context, const DB::ASTRefreshStrategy &strategy, bool attach,
                         bool coordinated, bool empty, bool is_restore_from_backup)
    : log(getLogger("RefreshTask")), view(view_), refresh_schedule(strategy), refresh_append(strategy.append) {
  auto component_guard = Coordination::setCurrentComponent("RefreshTask::RefreshTask");
  if (strategy.settings != nullptr) refresh_settings.applyChanges(strategy.settings->changes);

  coordination.root_znode.randomize();
  if (empty) coordination.root_znode.last_completed_timeslot = std::chrono::floor<std::chrono::seconds>(currentTime());
  if (coordinated) {
    ...
    if (!replica_path_existed) {
      if (!attach && !is_restore_from_backup && !zookeeper->isFeatureEnabled(KeeperFeatureFlag::MULTI_READ))
      ...
      ops.emplace_back(zkutil::makeCreateRequest(coordination.path, coordination.root_znode.toString(), zkutil::CreateMode::Persistent,
      ...
      if (is_restore_from_backup)
        ops.emplace_back(zkutil::makeCreateRequest(coordination.path + "/paused", "restored from backup", zkutil::CreateMode::Persistent,
                                                   /*ignore_if_exists*/ true));

      zookeeper->multi(ops);
    }

    if (server_settings[ServerSetting::disable_insertion_and_mutation]) coordination.read_only = true;
  } else {
    if (is_restore_from_backup) scheduling.stop_requested = true;
  }
}

OwnedRefreshTask RefreshTask::create(StorageMaterializedView *view, ContextMutablePtr context, const DB::ASTRefreshStrategy &strategy,
                                     bool attach, bool coordinated, bool empty, bool is_restore_from_backup) {
  auto task = std::make_shared<RefreshTask>(view, context, strategy, attach, coordinated, empty, is_restore_from_backup);

  task->refresh_task =
      context->getSchedulePool().createTask(view->getStorageID(), "RefreshTask", [self = task.get()] { self->refreshTask(); });

  task->refresh_task_watch_callback = std::make_shared<Coordination::WatchCallback>(
      [w = task->coordination.watches, task_waker = task->refresh_task->getWatchCallback()](const Coordination::WatchResponse &response) {
...
bool RefreshTask::canCreateOrDropOtherTables() const { return !refresh_append; }

void RefreshTask::startup() {
  if (view->getContext()->getSettingsRef()[Setting::stop_refreshable_materialized_views_on_startup]) scheduling.stop_requested = true;
...
void RefreshTask::shutdown() {
  {
    ...
    if (view == nullptr) return;  // already shut down
  ...
  /// Without this we can deadlock waiting for refresh_task because this shutdown happens from the same DDL thread for which
  /// CREATE/EXCHANGE/DROP wait.
...
void RefreshTask::drop(ContextPtr context) {
  if (coordination.coordinated) {
    ...
    if (zookeeper->exists(paused_path)) ops.emplace_back(zkutil::makeRemoveRequest(paused_path, -1));
...
void RefreshTask::rename(StorageID new_id, StorageID new_inner_table_id) {
  std::lock_guard guard(mutex);
  if (set_handle) set_handle.rename(new_id, refresh_append ? std::nullopt : std::make_optional(new_inner_table_id));
}

void RefreshTask::checkAlterIsPossible(const DB::ASTRefreshStrategy &new_strategy) {
  RefreshSettings s;
  if (new_strategy.settings) s.applyChanges(new_strategy.settings->changes);
  ...
  if (new_strategy.append != refresh_append) throw Exception(ErrorCodes::NOT_IMPLEMENTED, "Adding or removing APPEND is not supported.");
}
...
void RefreshTask::alterRefreshParams(const DB::ASTRefreshStrategy &new_strategy) {
...

  {
    ...
    if (new_strategy.dependencies)
      for (auto &&dependency : new_strategy.dependencies->children) deps.emplace_back(dependency->as<const ASTTableIdentifier &>());

    /// Update dependency graph.
    if (set_handle) set_handle.changeDependencies(deps);

    scheduleRefresh(guard);
    scheduling.dependencies_satisfied_until = std::chrono::sys_seconds(std::chrono::seconds(-1));

    refresh_settings = {};
    if (new_strategy.settings != nullptr) refresh_settings.applyChanges(new_strategy.settings->changes);

    if (view) view_storage_id = view->getStorageID();
  }
...
RefreshTask::Info RefreshTask::getInfo() const {
  std::lock_guard guard(mutex);
  return Info{.view_id = set_handle.getID(),
              .state = state,
              .next_refresh_time = next_refresh_time,
              .znode = coordination.root_znode,
              .replica_name = coordination.replica_name,
              .refresh_running = coordination.running_znode_exists,
              .progress = execution.progress.getValues(),
              .unexpected_error = scheduling.unexpected_error};
}

void RefreshTask::start() {
  std::lock_guard guard(mutex);
  if (!std::exchange(scheduling.stop_requested, false)) return;
...
void RefreshTask::stop() {
  std::lock_guard guard(mutex);
  if (std::exchange(scheduling.stop_requested, true)) return;
  interruptExecution();
  scheduleRefresh(guard);
}
...
void RefreshTask::startReplicated() {
  ...
  if (!coordination.coordinated) throw Exception(ErrorCodes::INCORRECT_QUERY, "Refreshable materialized view is not coordinated.");

  const auto zookeeper = [this]() {
    std::lock_guard guard(mutex);
    if (!view) throw Exception(ErrorCodes::TABLE_IS_DROPPED, "The table was dropped or detached");
...
void RefreshTask::stopReplicated(const String &reason) {
  ...
  if (code != Coordination::Error::ZOK && code != Coordination::Error::ZNODEEXISTS) throw Coordination::Exception::fromPath(code, path);
}

void RefreshTask::run() {
  std::lock_guard guard(mutex);
  if (std::exchange(scheduling.out_of_schedule_refresh_requested, true)) return;
  scheduleRefresh(guard);
}
...
void RefreshTask::wait() {
  auto throw_if_error = [&] {
    ...
    if (!coordination.running_znode_exists && !coordination.root_znode.last_attempt_succeeded &&
        coordination.root_znode.last_attempt_time.time_since_epoch().count() != 0)
      throw Exception(
          ErrorCodes::REFRESH_FAILED, "Refresh failed{}: {}",
          coordination.coordinated ? " (on replica " + coordination.root_znode.last_attempt_replica + ")" : "",
  ...
  refresh_cv.wait(lock, [&] {
    return state != RefreshState::Running && state != RefreshState::Scheduling && state != RefreshState::RunningOnAnotherReplica &&
  ...
  if (coordination.coordinated && !refresh_append) {
    ...
    while (true) {
      ...
      if (storage && storage->getStorageID().uuid == expected_table_uuid) return;
...
bool RefreshTask::tryJoinBackgroundTask(std::chrono::steady_clock::time_point deadline) {
  ...
  return refresh_cv.wait_for(lock, duration, [&] { return state != RefreshState::Running && state != RefreshState::Scheduling; });
}
...
void RefreshTask::notify() {
  std::lock_guard guard(mutex);
  if (view && view->getContext()->getRefreshSet().refreshesStopped()) interruptExecution();
  scheduling.dependencies_satisfied_until = std::chrono::sys_seconds(std::chrono::seconds(-1));
  scheduleRefresh(guard);
}
...
void RefreshTask::refreshTask() {
  ...
  try {
    ...
    while (true) {
      ...
      if (coordination.coordinated) zookeeper = view->getContext()->getZooKeeper();
      ...
      if (coordination.running_znode_exists) {
        if (coordination.root_znode.last_attempt_replica == coordination.replica_name) {
          LOG_ERROR(log, "Znode {} indicates that this replica is running a refresh, but it isn't. Likely a bug.",
          ...
          if (coordination.coordinated) removeRunningZnodeIfMine(zookeeper);
      ...
      if (scheduling.stop_requested || coordination.paused_znode_exists || view->getContext()->getRefreshSet().refreshesStopped() ||
      ...
      if (out_of_schedule) {
        ...
        if (scheduling.fake_clock.load(std::memory_order_relaxed) != INT64_MIN) delay_ms = 100;
      ...
      if (refreshed) {
        ...
        znode.last_completed_timeslot = refresh_schedule.timeslotForCompletedRefresh(znode.last_completed_timeslot, start_time_seconds,
                                                                                     end_time_seconds, out_of_schedule);
      ...
      chassert(lock.owns_lock());
      if (!ok) throw Exception(ErrorCodes::LOGICAL_ERROR, "Refresh coordination znode was changed while refresh was in progress.");
    ...
    tryLogCurrentException(log, "Exception in refresh scheduling. The view will be stopped.");
#ifdef DEBUG_OR_SANITIZER_BUILD
...
#else
    if (coordination.coordinated) removeRunningZnodeIfMine(view->getContext()->getZooKeeper());
#endif
  }
}

std::optional<UUID> RefreshTask::executeRefreshUnlocked(bool append, int32_t root_znode_version,
                                                        std::chrono::system_clock::time_point start_time, const Stopwatch &stopwatch,
                                                        const String &log_comment, String &out_error_message) {
  ...
  try {
...

    {
      ...
      query_for_logging = refresh_query->formatForLogging(refresh_context->getSettingsRef()[Setting::log_queries_cut_to_length]);
      normalized_query_hash = normalizedQueryHash(query_for_logging, false);

      process_list_entry = refresh_context->getProcessList().insert(query_for_logging, normalized_query_hash, refresh_query.get(),
                                                                    refresh_context, Stopwatch{CLOCK_MONOTONIC}.getStart(), internal);

      refresh_context->setProcessListElement(process_list_entry->getQueryStatus());
      refresh_context->setProgressCallback([this](const Progress &prog) { execution.progress.incrementPiecewiseAtomically(prog); });

      /// Run the query.

      InterpreterInsertQuery interpreter(refresh_query, refresh_context,
      ...
      query_log_elem =
          logQueryStart(start_time, refresh_context, query_for_logging, normalized_query_hash, refresh_query, pipeline, &interpreter,
                        /*internal*/ internal, view_storage_id.database_name, view_storage_id.table_name, /*async_insert*/ false);
      ...
      logQueryFinish(*query_log_elem, refresh_context, refresh_query, std::move(pipeline), /*pulling_pipeline=*/false, query_span,
                     QueryResultCacheUsage::None, /*internal=*/internal);
    ...
    if (query_log_elem.has_value()) {
      logQueryException(*query_log_elem, refresh_context, stopwatch, refresh_query, query_span, /*internal*/ internal,
                        /*log_error*/ !cancelled);
    } else {
  ...
  if (table_to_drop.has_value()) view->dropTempTable(table_to_drop.value(), refresh_context, out_error_message);
...
void RefreshTask::updateDependenciesIfNeeded(std::unique_lock<std::mutex> &lock) {
  while (true) {
    chassert(lock.owns_lock());
    if (scheduling.dependencies_satisfied_until.time_since_epoch().count() >= 0) return;
...
static std::chrono::milliseconds backoff(Int64 retry_idx, const RefreshSettings &refresh_settings) {
  ...
  if (refresh_settings[RefreshSetting::refresh_retry_initial_backoff_ms] <=
      refresh_settings[RefreshSetting::refresh_retry_max_backoff_ms] / multiplier)
...
RefreshTask::determineNextRefreshTime(std::chrono::sys_seconds now) {
  ...
  if (refresh_settings[RefreshSetting::refresh_retries] >= 0 && znode.attempt_number > refresh_settings[RefreshSetting::refresh_retries]) {
    ...
    znode.last_completed_timeslot = refresh_schedule.timeslotForCompletedRefresh(znode.last_completed_timeslot, znode.last_attempt_time,
                                                                                 znode.last_attempt_time, false);
    znode.attempt_number = 0;
  }
...
void RefreshTask::scheduleRefresh(std::lock_guard<std::mutex> &) {
  if (state != RefreshState::Running) state = RefreshState::Scheduling;
  refresh_task->schedule();
}
...
void RefreshTask::setState(RefreshState s, std::unique_lock<std::mutex> &lock) {
  chassert(lock.owns_lock());
  state = s;
  if (s != RefreshState::Running && s != RefreshState::Scheduling) refresh_cv.notify_all();
}

void RefreshTask::readZnodesIfNeeded(std::shared_ptr<zkutil::ZooKeeper> zookeeper, std::unique_lock<std::mutex> &lock) {
  chassert(lock.owns_lock());
  if (!coordination.coordinated || !coordination.watches->should_reread_znodes.load()) return;
  ...
  if (responses[0].error != Coordination::Error::ZOK) throw Coordination::Exception::fromPath(responses[0].error, paths[0]);
...
bool RefreshTask::updateCoordinationState(CoordinationZnode root, bool running, std::shared_ptr<zkutil::ZooKeeper> zookeeper,
...
void RefreshTask::interruptExecution() {
  ...
  if (execution.interrupt_execution.exchange(true)) return;
...
std::tuple<StoragePtr, TableLockHolder> RefreshTask::getAndLockTargetTable(const StorageID &storage_id, const ContextPtr &context) {
  ...
  for (int attempt = 0; attempt < 10; ++attempt) {
    ...
    TableLockHolder storage_lock =
        storage->tryLockForShare(context->getCurrentQueryId(), context->getSettingsRef()[Setting::lock_acquire_timeout]);
    if (!storage_lock) {
    ...
    if (coordination.coordinated) {
      ...
      if (uuid != last_synced_inner_uuid) {
        try {
          ...
          if (e.code() != ErrorCodes::ABORTED) throw;
...
std::chrono::system_clock::time_point RefreshTask::currentTime() const {
  ...
  if (fake == INT64_MIN) return std::chrono::system_clock::now();
  return std::chrono::system_clock::time_point(std::chrono::seconds(fake));
}

void RefreshTask::setRefreshSetHandleUnlock(RefreshSet::Handle &&set_handle_) { set_handle = std::move(set_handle_); }
