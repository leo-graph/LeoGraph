...
RefreshSchedule::RefreshSchedule(const ASTRefreshStrategy& strategy) {
  ...
  if (strategy.offset) offset = strategy.offset->interval;
  if (strategy.spread) spread = strategy.spread->interval;
}
...
static std::chrono::sys_seconds advanceEvery(std::chrono::system_clock::time_point prev, CalendarTimeInterval period,
...
std::chrono::sys_seconds RefreshSchedule::timeslotForCompletedRefresh(std::chrono::sys_seconds last_completed_timeslot,
                                                                      std::chrono::sys_seconds start_time,
                                                                      std::chrono::sys_seconds end_time, bool out_of_schedule) const {
  if (kind == RefreshScheduleKind::AFTER) return end_time;
...
std::chrono::sys_seconds RefreshSchedule::advance(std::chrono::sys_seconds last_completed_timeslot) const {
  if (kind == RefreshScheduleKind::AFTER) return period.advance(last_completed_timeslot);
...
std::chrono::system_clock::time_point RefreshSchedule::addRandomSpread(std::chrono::sys_seconds timeslot, Int64 randomness) const {
  return timeslot +
         std::chrono::milliseconds(Int64(static_cast<double>(spread.minSeconds()) * 1e3 / 2 * static_cast<double>(randomness) / 1e9));
}
