 ...
 public:
  ...
  static Block getHeader(Block header, const SourcesInfoPtr &source_info) {
    if (source_info->need_path_column)
      header.insert({DataTypeLowCardinality{std::make_shared<DataTypeString>()}.createColumn(),
                     std::make_shared<DataTypeLowCardinality>(std::make_shared<DataTypeString>()), "_path"});
    if (source_info->need_file_column)
      header.insert({DataTypeLowCardinality{std::make_shared<DataTypeString>()}.createColumn(),
                     std::make_shared<DataTypeLowCardinality>(std::make_shared<DataTypeString>()), "_file"});
  ...
  StorageHiveSource(SourcesInfoPtr source_info_, String hdfs_namenode_url_, String format_, String compression_method_, Block sample_block_,
                    ContextPtr context_, UInt64 max_block_size_, const StorageHive &storage_, const Names &text_input_field_names_ = {})
    ...
    for (const auto &name_type : source_info->partition_name_types) {
      if (to_read_block.has(name_type.name)) to_read_block.erase(name_type.name);
    }

    /// Apply read buffer prefetch for HiveText format, because it is read sequentially
    if (read_settings.remote_fs_prefetch) read_settings.remote_fs_prefetch = format == "HiveText";
    ...
    if (!generate_chunk_from_metadata && !to_read_block.columns()) {
      ...
      for (const auto &column : metadata->getColumns().getAllPhysical()) {
        ...
        if (!is_partition_column) to_read_block.insert(ColumnWithTypeAndName(column.type, column.name));
      }
    }
  }
  ...
  Chunk generate() override {
    while (true) {
      bool need_next_file = (!generate_chunk_from_metadata && !reader) || (generate_chunk_from_metadata && !current_file_remained_rows);
      if (need_next_file) {
        current_idx = source_info->next_uri_to_read.fetch_add(1);
        if (current_idx >= source_info->hive_files.size()) return {};
        ...
        try {
          auto get_raw_read_buf = [&]() -> std::unique_ptr<ReadBuffer> {
            ...
            return std::make_unique<ReadBufferFromHDFS>(hdfs_namenode_url, current_path, getContext()->getGlobalContext()->getConfigRef(),
                                                        getContext()->getReadSettings());
          };

          raw_read_buf = get_raw_read_buf();
          if (read_settings.remote_fs_prefetch) raw_read_buf->prefetch(DEFAULT_PREFETCH_PRIORITY);
        ...
        auto input_format = FormatFactory::instance().getInput(format, *read_buf, to_read_block, context, max_block_size,
      ...
      if (generate_chunk_from_metadata) return generateChunkFromMetadata();
  ...
  Chunk getResultChunk(const Block &source_block, UInt64 num_rows) const {
    ...
    for (const auto &column : result_header) {
      ...
      if (column.name == "_file") {
        ...
        auto file_column = DataTypeLowCardinality{std::make_shared<DataTypeString>()}.createColumnConst(num_rows, std::move(file_name));
...
StorageHive::StorageHive(const String &hive_metastore_url_, const String &hive_database_, const String &hive_table_,
                         const StorageID &table_id_, const ColumnsDescription &columns_, const ConstraintsDescription &constraints_,
                         const String &comment_, const ASTPtr &partition_by_ast_, std::unique_ptr<HiveSettings> storage_settings_,
                         ContextPtr context_)
    : IStorage(table_id_),
      WithContext(context_),
...
void StorageHive::initMinMaxIndexExpression() {
  ...
  if (!partition_key_expr_list->children.empty()) {
    ...
    partition_minmax_idx_expr =
        std::make_shared<ExpressionActions>(ActionsDAG(partition_name_types), ExpressionActionsSettings(getContext()));
  }

  NamesAndTypesList all_name_types = metadata_snapshot->getColumns().getAllPhysical();
  for (const auto &column : all_name_types) {
    if (!partition_name_types.contains(column.name)) hivefile_name_types.push_back(column);
  }
  hivefile_minmax_idx_expr = std::make_shared<ExpressionActions>(ActionsDAG(hivefile_name_types), ExpressionActionsSettings(getContext()));
}

ASTPtr StorageHive::extractKeyExpressionList(const ASTPtr &node) {
  if (!node) return make_intrusive<ASTExpressionList>();
...
static HiveFilePtr createHiveFile(const String &format_name, const FieldVector &fields, const String &namenode_url, const String &path,
                                  UInt64 ts, size_t size, const NamesAndTypesList &index_names_and_types,
                                  const std::shared_ptr<HiveSettings> &hive_settings, const ContextPtr &context) {
...
HiveFiles StorageHive::collectHiveFilesFromPartition(const Apache::Hadoop::Hive::Partition &partition, const ActionsDAG *filter_actions_dag,
                                                     const HiveTableMetadataPtr &hive_table_metadata, const HDFSFSPtr &fs,
                                                     const ContextPtr &context_, PruneLevel prune_level) const {
  LOG_DEBUG(log, "Collect hive files from partition {}, prune_level:{}", boost::join(partition.values, ","),
  ...
  if (has_default_partition) return {};

  /// Check partition values
  if (partition.values.size() != partition_names.size())
    throw Exception(ErrorCodes::INVALID_PARTITION_VALUE, "Partition value size not match, expect {}, but got {}", partition_names.size(),
                    partition.values.size());

  /// Join partition values in CSV format
  WriteBufferFromOwnString wb;
  for (size_t i = 0; i < partition.values.size(); ++i) {
    if (i != 0) writeString(",", wb);
  ...
  ContextPtr context = getContext();
  auto format = FormatFactory::instance().getInput("CSV", buffer, partition_key_expr->getSampleBlock(), context,
                                                   context->getSettingsRef()[Setting::max_block_size], std::nullopt,
                                                   FormatParserSharedResources::singleThreaded(context->getSettingsRef()));
  ...
  for (size_t i = 0; i < partition_names.size(); ++i) block.getByPosition(i).column->get(0, fields[i]);

  if (prune_level >= PruneLevel::Partition) {
    std::vector<Range> ranges;
    ranges.reserve(partition_names.size());
    for (size_t i = 0; i < partition_names.size(); ++i) ranges.emplace_back(fields[i]);
    ...
    if (!partition_key_condition.checkInHyperrectangle(ranges, partition_types).can_be_true) return {};
  }
  ...
  for (const auto &file_info : file_infos) {
    ...
    if (hive_file) {
      LOG_TRACE(log, "Append hive file {} from partition {}, prune_level:{}", hive_file->getPath(), boost::join(partition.values, ","),
                pruneLevelToString(prune_level));
...
std::vector<StorageHive::FileInfo> StorageHive::listDirectory(const String &path, const HiveTableMetadataPtr &hive_table_metadata,
                                                              const HDFSFSPtr &fs) {
  return hive_table_metadata->getFilesByLocation(fs, path);
}

HiveFilePtr StorageHive::getHiveFileIfNeeded(const FileInfo &file_info, const FieldVector &fields, const ActionsDAG *filter_actions_dag,
                                             const HiveTableMetadataPtr &hive_table_metadata, const ContextPtr &context_,
                                             PruneLevel prune_level) const {
  String filename = getBaseName(file_info.path);
  /// Skip temporary files starts with '.'
  if (startsWith(filename, ".")) return {};
  ...
  if (!hive_file || hive_file->getLastModTs() < file_info.last_modify_time) {
    ...
    hive_file = createHiveFile(format_name, fields, hdfs_namenode_url, file_info.path, file_info.last_modify_time, file_info.size,
                               hivefile_name_types, storage_settings, context_->getGlobalContext());
      ...
      if (hive_file->useSplitMinMaxIndex()) {
        ...
        for (size_t i = 0; i < sub_minmax_idxes.size(); ++i) {
          if (!hivefile_key_condition.checkInHyperrectangle(sub_minmax_idxes[i]->hyperrectangle, hivefile_name_types.getTypes())
                   .can_be_true) {
            LOG_TRACE(log, "Skip split {} of hive file {} by index {}", i, hive_file->getPath(),
...
bool StorageHive::supportsSubsetOfColumns() const { return format_name == "Parquet" || format_name == "ORC"; }
...
class ReadFromHive : public SourceStepWithFilter {
 public:
  ...
  ReadFromHive(const Names &column_names_, const SelectQueryInfo &query_info_, const StorageSnapshotPtr &storage_snapshot_,
               const ContextPtr &context_, Block header, std::shared_ptr<StorageHive> storage_,
               std::shared_ptr<StorageHiveSource::SourcesInfo> sources_info_, HDFSBuilderWrapper builder_, HDFSFSPtr fs_,
               HiveMetastoreClient::HiveTableMetadataPtr hive_table_metadata_, Block sample_block_, LoggerPtr log_, size_t max_block_size_,
               size_t num_streams_)
...
void ReadFromHive::createFiles() {
  if (hive_files) return;

  hive_files =
      storage->collectHiveFiles(num_streams, filter_actions_dag ? &*filter_actions_dag : nullptr, hive_table_metadata, fs, context);
  LOG_INFO(log, "Collect {} hive files to read", hive_files->size());
}

void StorageHive::read(QueryPlan &query_plan, const Names &column_names, const StorageSnapshotPtr &storage_snapshot,
  ...
  auto case_insensitive_matching = [&]() -> bool {
    if (format_name == "Parquet") return settings[Setting::input_format_parquet_case_insensitive_column_matching];
    if (format_name == "ORC") return settings[Setting::input_format_orc_case_insensitive_column_matching];
    return false;
  };
  Block sample_block;
  ...
  for (const auto &column : column_names) {
    ...
    if (column == "_path") sources_info->need_path_column = true;
    if (column == "_file") sources_info->need_file_column = true;
  }

  auto this_ptr = std::static_pointer_cast<StorageHive>(shared_from_this());

  auto reading = std::make_unique<ReadFromHive>(column_names, query_info, storage_snapshot, context_,
                                                StorageHiveSource::getHeader(sample_block, sources_info), std::move(this_ptr),
                                                std::move(sources_info), std::move(builder), std::move(fs), std::move(hive_table_metadata),
                                                std::move(sample_block), log, max_block_size, num_streams);
...
void ReadFromHive::initializePipeline(QueryPipelineBuilder &pipeline, const BuildQueryPipelineSettings &) {
  ...
  for (size_t i = 0; i < num_streams; ++i) {
    pipes.emplace_back(std::make_shared<StorageHiveSource>(sources_info, storage->hdfs_namenode_url, storage->format_name,
                                                           storage->compression_method, sample_block, context, max_block_size, *storage,
                                                           storage->text_input_field_names));
  }

  auto pipe = Pipe::unitePipes(std::move(pipes));
  if (pipe.empty()) pipe = Pipe(std::make_shared<NullSource>(getOutputHeader()));
...
HiveFiles StorageHive::collectHiveFiles(size_t max_threads, const ActionsDAG *filter_actions_dag,
                                        const HiveTableMetadataPtr &hive_table_metadata, const HDFSFSPtr &fs, const ContextPtr &context_,
                                        PruneLevel prune_level) const {
  std::vector<Apache::Hadoop::Hive::Partition> partitions = hive_table_metadata->getPartitions();
  /// Hive table have no partition
  if (!partition_name_types.empty() && partitions.empty()) return {};
  ...
  ThreadPool pool{CurrentMetrics::StorageHiveThreads, CurrentMetrics::StorageHiveThreadsActive, CurrentMetrics::StorageHiveThreadsScheduled,
                  max_threads};
  if (!partitions.empty()) {
...
std::optional<UInt64> StorageHive::totalRowsImpl(const Settings &settings, const ActionsDAG *filter_actions_dag, ContextPtr context_,
                                                 PruneLevel prune_level) const {
  /// Row-based format like Text doesn't support totalRowsByPartitionPredicate
  if (!supportsSubsetOfColumns()) return {};
  ...
  HDFSFSPtr fs = createHDFSFS(builder.get());
  HiveFiles hive_files =
      collectHiveFiles(settings[Setting::max_threads], filter_actions_dag, hive_table_metadata, fs, context_, prune_level);

  UInt64 total_rows = 0;
  for (const auto &hive_file : hive_files) {
    auto file_rows = hive_file->getRows();
    if (!file_rows)
      throw Exception(ErrorCodes::LOGICAL_ERROR, "Rows of hive file:{} with format:{} not initialized", hive_file->getPath(), format_name);
...
void registerStorageHive(StorageFactory &factory) {
  factory.registerStorage(
      ...
      [](const StorageFactory::Arguments &args) {
        ...
        if (have_settings) hive_settings->loadFromQuery(*args.storage_def);
        ...
        if (!partition_by) throw Exception(ErrorCodes::BAD_ARGUMENTS, "Storage Hive requires partition by clause");
        ...
        return std::make_shared<StorageHive>(hive_metastore_url, hive_database, hive_table, args.table_id, args.columns, args.constraints,
                                             args.comment, partition_by->ptr(), std::move(hive_settings), args.getContext());
      },
