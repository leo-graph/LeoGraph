...
class HiveMetastoreClient {
 public:
  struct FileInfo {
    ...
    FileInfo(const String &path_, UInt64 last_modify_time_, size_t size_) : path(path_), last_modify_time(last_modify_time_), size(size_) {}
  };
  ...
  class HiveTableMetadata : boost::noncopyable {
   public:
    HiveTableMetadata(const String &db_name_, const String &table_name_, std::shared_ptr<Apache::Hadoop::Hive::Table> table_,
        ...
        : db_name(db_name_),
          ...
          hive_files_cache(
              std::make_shared<HiveFilesCache>(CurrentMetrics::HiveFilesCacheBytes, CurrentMetrics::HiveFilesCacheFiles, 10000)) {
      std::lock_guard lock(mutex);
      for (const auto &partition : partitions_) partition_infos.emplace(partition.sd.location, PartitionInfo(partition));
    }
