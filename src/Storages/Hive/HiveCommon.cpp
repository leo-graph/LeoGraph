...
void HiveMetastoreClient::tryCallHiveClient(std::function<void(ThriftHiveMetastoreClientPool::Entry &)> func) {
  ...
  if (i >= max_hive_metastore_client_retry) throw Exception(ErrorCodes::NO_HIVEMETASTORE, "Hive Metastore expired because {}", err_msg);
}
...
void HiveMetastoreClient::clearTableMetadata(const String &db_name, const String &table_name) {
  ...
  HiveTableMetadataPtr metadata = table_metadata_cache.get(cache_key);
  if (metadata) table_metadata_cache.remove(cache_key);
}
...
bool HiveMetastoreClient::PartitionInfo::haveSameParameters(const Apache::Hadoop::Hive::Partition &other) const {
  ...
  for (; it != partition.parameters.cend() && oit != other.parameters.cend(); ++it, ++oit) {
    if (it->first != oit->first || it->second != oit->second) return false;
  }
...
std::vector<Apache::Hadoop::Hive::Partition> HiveMetastoreClient::HiveTableMetadata::getPartitions() const {
  ...
  for (const auto &partition_info : partition_infos) result.emplace_back(partition_info.second.partition);
  return result;
}

std::vector<HiveMetastoreClient::FileInfo> HiveMetastoreClient::HiveTableMetadata::getFilesByLocation(const HDFSFSPtr &fs,
  ...
  if (!empty_partition_keys) {
    ...
    if (it == partition_infos.end()) throw Exception(ErrorCodes::BAD_ARGUMENTS, "Invalid location {}", location);
  ...
  for (int i = 0; i < ls.length; ++i) {
    ...
    if (file_info.mKind != 'D' && file_info.mSize > 0) result.emplace_back(String(file_info.mName), file_info.mLastMod, file_info.mSize);
  }
...
HiveFilesCachePtr HiveMetastoreClient::HiveTableMetadata::getHiveFilesCache() const { return hive_files_cache; }

void HiveMetastoreClient::HiveTableMetadata::updateIfNeeded(const std::vector<Apache::Hadoop::Hive::Partition> &partitions) {
  std::lock_guard lock{mutex};

  if (!shouldUpdate(partitions)) return;
...
bool HiveMetastoreClient::HiveTableMetadata::shouldUpdate(const std::vector<Apache::Hadoop::Hive::Partition> &partitions) {
  ...
  if (old_partiton_infos.size() != partitions.size()) return true;

  for (const auto &partition : partitions) {
    auto it = old_partiton_infos.find(partition.sd.location);
    if (it == old_partiton_infos.end()) return true;

    const auto &old_partition_info = it->second;
    if (!old_partition_info.haveSameParameters(partition)) return true;
  }
  return false;
}
