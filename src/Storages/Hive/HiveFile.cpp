...
#  define THROW_ARROW_NOT_OK(status)                                                                                                   \
    do {                                                                                                                               \
      if (const ::arrow::Status &_s = (status); !_s.ok()) throw Exception::createDeprecated(_s.ToString(), ErrorCodes::BAD_ARGUMENTS); \
    } while (false)
...
Range createRangeFromParquetStatistics(std::shared_ptr<StatisticsType> stats) {
  ...
  /// null values or NaN/Inf values of double type.
  if (!stats->HasMinMax()) return Range::createWholeUniverseWithoutNull();
...
void IHiveFile::loadFileMinMaxIndex() {
  if (file_minmax_idx_loaded) return;
  std::lock_guard lock(mutex);
  if (file_minmax_idx_loaded) return;
...
void IHiveFile::loadSplitMinMaxIndexes() {
  if (split_minmax_idxes_loaded) return;
  std::lock_guard lock(mutex);
  if (split_minmax_idxes_loaded) return;
...
Range HiveORCFile::buildRange(const orc::ColumnStatistics *col_stats) {
  if (!col_stats || col_stats->hasNull()) return Range::createWholeUniverseWithoutNull();
...
void HiveORCFile::prepareReader() {
  ...
  auto result = arrow::adapters::orc::ORCFileReader::Open(asArrowFile(*in, format_settings, is_stopped, "ORC", ORC_MAGIC_BYTES),
...
bool HiveORCFile::useFileMinMaxIndex() const { return (*storage_settings)[HiveSetting::enable_orc_file_minmax_index]; }
...
bool HiveORCFile::useSplitMinMaxIndex() const { return (*storage_settings)[HiveSetting::enable_orc_stripe_minmax_index]; }
...
void HiveORCFile::loadSplitMinMaxIndexesImpl() {
  ...
  if (stripe_num != stripe_stats_num)
    throw Exception(ErrorCodes::BAD_ARGUMENTS, "orc file:{} has different strip num {} and strip statistics num {}", path, stripe_num,
                    stripe_stats_num);
...
bool HiveParquetFile::useSplitMinMaxIndex() const { return (*storage_settings)[HiveSetting::enable_parquet_rowgroup_minmax_index]; }
...
void HiveParquetFile::prepareReader() {
  ...
  std::atomic<int> is_stopped{0};
  auto open_file_res =
      parquet::arrow::OpenFile(asArrowFile(*in, format_settings, is_stopped, "Parquet", PARQUET_MAGIC_BYTES), arrow::default_memory_pool());
  THROW_ARROW_NOT_OK(open_file_res.status());
  reader = *std::move(open_file_res);
}

void HiveParquetFile::loadSplitMinMaxIndexesImpl() {
  if (!reader) prepareReader();
  ...
  for (size_t i = 0; i < num_row_groups; ++i) {
    ...
    for (; it != index_names_and_types.end(); ++j, ++it) {
      ...
      if (mit == parquet_column_positions.end()) continue;
      ...
      if (stats->HasNullCount() && stats->null_count() > 0) continue;
