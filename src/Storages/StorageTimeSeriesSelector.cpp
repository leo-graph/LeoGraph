...
StorageTimeSeriesSelector::Configuration StorageTimeSeriesSelector::getConfiguration(ASTs &args, const ContextPtr &context) {
  ...
  if ((args.size() < min_num_args) || (args.size() > max_num_args)) {
    ...
    throw Exception(ErrorCodes::NUMBER_OF_ARGUMENTS_DOESNT_MATCH, "Table function '{}' requires {}..{} arguments: {}({})", function_name,
                    min_num_args, max_num_args, function_name, expected_args);
  }
  ...
  if (time_series_storage_id.empty()) {
    if (args.size() == min_num_args) {
      ...
      if (table_name_field.getType() != Field::Types::String)
        throw Exception(ErrorCodes::BAD_ARGUMENTS, "Argument 'table_name' must be a literal with type String, got {}",
                        table_name_field.getType());
      ...
      if (database_name_field.getType() != Field::Types::String)
        throw Exception(ErrorCodes::BAD_ARGUMENTS, "Argument 'database_name' must be a literal with type String, got {}",
                        database_name_field.getType());
...
StorageTimeSeriesSelector::StorageTimeSeriesSelector(const StorageID &table_id_, const ColumnsDescription &columns_,
...
ASTPtr tagNameToAST(const String &tag_name, const std::unordered_map<String, String> &column_name_by_tag_name) {
  if (tag_name == TimeSeriesTagNames::MetricName) return make_intrusive<ASTIdentifier>(TimeSeriesColumnNames::MetricName);

  auto it = column_name_by_tag_name.find(tag_name);
  if (it != column_name_by_tag_name.end()) return make_intrusive<ASTIdentifier>(it->second);
...
ASTPtr matcherToAST(const PrometheusQueryTree::Matcher &matcher, const std::unordered_map<String, String> &column_name_by_tag_name) {
  ...
  if (add_not) res = makeASTFunction("not", res);
  return res;
}

ASTPtr makeWhereFilterForTagsTable(const PrometheusQueryTree::MatcherList &matchers,
                                   const std::unordered_map<String, String> &column_name_by_tag_name,
                                   const std::optional<DateTime64> &min_time, const std::optional<DateTime64> &max_time,
                                   const DataTypePtr &timestamp_data_type) {
  ASTs asts;
  for (const auto &matcher : matchers) asts.push_back(matcherToAST(matcher, column_name_by_tag_name));

  if (asts.empty()) throw Exception(ErrorCodes::LOGICAL_ERROR, "Instant selector without matchers is not allowed");

  if (min_time) {
    /// tags_table.max_time >= min_time
    asts.push_back(makeASTFunction("greaterOrEquals", make_intrusive<ASTIdentifier>(TimeSeriesColumnNames::MaxTime),
                                   timeSeriesTimestampToAST(*min_time, timestamp_data_type)));
  }

  if (max_time) {
    /// tags_table.min_time <= max_time
    asts.push_back(makeASTFunction("lessOrEquals", make_intrusive<ASTIdentifier>(TimeSeriesColumnNames::MinTime),
...
ASTPtr makeSelectQueryFromTagsTable(const StorageID &tags_table_id, const PrometheusQueryTree::MatcherList &matchers,
                                    const std::unordered_map<String, String> &column_name_by_tag_name,
                                    const std::optional<DateTime64> &min_time, const std::optional<DateTime64> &max_time,
                                    const DataTypePtr &timestamp_data_type) {
...
ASTPtr makeWhereFilterForDataTable(ASTPtr select_query_from_tags_table, DateTime64 min_time, DateTime64 max_time,
                                   const DataTypePtr &timestamp_data_type) {
  ASTs conditions;
  ...
  /// timestamp >= min_time
  conditions.push_back(makeASTFunction("greaterOrEquals", make_intrusive<ASTIdentifier>(TimeSeriesColumnNames::Timestamp),
                                       timeSeriesTimestampToAST(min_time, timestamp_data_type)));

  /// timestamp <= max_time
  conditions.push_back(makeASTFunction("lessOrEquals", make_intrusive<ASTIdentifier>(TimeSeriesColumnNames::Timestamp),
                                       timeSeriesTimestampToAST(max_time, timestamp_data_type)));

  return makeASTForLogicalAnd(std::move(conditions));
}

ASTPtr makeSelectQueryFromDataTable(const StorageID &data_table_id, ASTPtr select_query_from_tags_table, DateTime64 min_time,
                                    DateTime64 max_time, const DataTypePtr &id_data_type, const DataTypePtr &timestamp_data_type,
  ...
  /// SELECT id, timestamp, value
  {
    ...
    select_list.push_back(makeASTFunction("CAST", make_intrusive<ASTIdentifier>(TimeSeriesColumnNames::ID),
                                          make_intrusive<ASTLiteral>(id_data_type->getName())));
    select_list.back()->setAlias(TimeSeriesColumnNames::ID);

    select_list.push_back(timeSeriesTimestampASTCast(make_intrusive<ASTIdentifier>(TimeSeriesColumnNames::Timestamp), timestamp_data_type));
...
void StorageTimeSeriesSelector::read(QueryPlan &query_plan, const Names &column_names, const StorageSnapshotPtr & /* storage_snapshot */,
  ...
  ASTPtr select_query_from_data_table =
      makeSelectQueryFromDataTable(data_table_id, select_query_from_tags_table, config.min_time, config.max_time, config.id_data_type,
                                   config.timestamp_data_type, config.scalar_data_type);
