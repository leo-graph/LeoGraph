...
class StorageLog final : public IStorage, public WithMutableContext {
 ...
 public:
   ...
   */
  StorageLog(const String &engine_name_, DiskPtr disk_, const std::string &relative_path_, const StorageID &table_id_,
  ...
  Pipe createReadingPipe(const Names &column_names, ContextPtr local_context, const StorageSnapshotPtr &storage_snapshot,
...
class ReadFromStorageLogStep : public ISourceStep {
 public:
  ReadFromStorageLogStep(const Names &column_names_, ContextPtr local_context_, std::shared_ptr<StorageLog> storage_,
  ...
  QueryPlanStepPtr clone() const override { return std::make_unique<ReadFromStorageLogStep>(*this); }
