...
class StorageFuzzQuery final : public IStorage {
 public:
  ...
  StorageFuzzQuery(const StorageID& table_id_, const ColumnsDescription& columns_, const String& comment_, const Configuration& config_);
...
class FuzzQuerySource : public ISource {
 public:
  FuzzQuerySource(UInt64 block_size_, SharedHeader block_header_, const StorageFuzzQuery::Configuration& config_, ASTPtr query_)
