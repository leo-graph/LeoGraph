...
void fillBufferWithRandomData(char *__restrict data, size_t limit, size_t size_of_type, pcg64 &rng,
                              [[maybe_unused]] bool flip_bytes = false) {
...
size_t estimateValueSize(const DataTypePtr type, UInt64 max_array_length, UInt64 max_string_length) {
  if (type->haveMaximumSizeOfValue()) return type->getMaximumSizeOfValueInMemory();
  ...
  switch (idx) {
    ...
    case TypeIndex::Tuple: {
      ...
      for (size_t i = 0; i < tuple_size; ++i) res += estimateValueSize(elements[i], max_array_length, max_string_length);
...
ColumnPtr fillColumnWithRandomData(DataTypePtr type, UInt64 limit, UInt64 max_array_length, UInt64 max_string_length, pcg64 &rng,
                                   ContextPtr context) {
  TypeIndex idx = type->getTypeId();
  ...
  switch (idx) {
    ...
    case TypeIndex::Tuple: {
      auto elements = typeid_cast<const DataTypeTuple *>(type.get())->getElements();
      if (elements.empty()) return ColumnTuple::create(limit);
    ...
    case TypeIndex::Nullable: {
      ...
      for (UInt64 i = 0; i < limit; ++i) null_map[i] = rng() % 16 == 0;  /// No real motivation for this.
    ...
    case TypeIndex::UInt8: {
      ...
      data.resize(limit);
      if (isBool(type)) {
        for (size_t i = 0; i < limit; ++i) data[i] = rng() % 2;
      } else {
    ...
    case TypeIndex::Date32: {
      ...
      for (size_t i = 0; i < limit; ++i) column->getData()[i] = (rng() % static_cast<UInt64>(DATE_LUT_SIZE)) - DAYNUM_OFFSET_EPOCH;
    ...
    case TypeIndex::Decimal32: {
      ...
      for (size_t i = 0; i != limit; ++i) data[i] = static_cast<Int32>(rng()) % range;
      return column;
    }
    case TypeIndex::Decimal64: {
      ...
      for (size_t i = 0; i != limit; ++i) data[i] = static_cast<Int64>(rng()) % range;
    ...
    case TypeIndex::Decimal128: {
      ...
      for (size_t i = 0; i != limit; ++i) data[i] = Int128({rng(), rng()}) % range;
      return column;
    }
    case TypeIndex::Decimal256: {
      ...
      for (size_t i = 0; i != limit; ++i) data[i] = Int256({rng(), rng(), rng(), rng()}) % range;
      return column;
    }
    ...
    case TypeIndex::DateTime64: {
      ...
      for (size_t i = 0; i < limit; ++i) column_concrete.getData()[i] = rng() % range;  /// Slow
...
class GenerateSource : public ISource {
 public:
  GenerateSource(UInt64 block_size_, UInt64 max_array_length_, UInt64 max_string_length_, UInt64 random_seed_, Block block_header_,
                 ContextPtr context_, GenerateRandomStatePtr state_)
 ...
 protected:
  Chunk generate() override {
    ...
    if (total_rows) addTotalRowsApprox(total_rows);
 ...
 private:
  ...
  static Block &prepareBlockToFill(Block &block) {
    ...
    for (auto &column : names_and_types) block.insert(ColumnWithTypeAndName(column.type, column.name));
...
StorageGenerateRandom::StorageGenerateRandom(const StorageID &table_id_, const ColumnsDescription &columns_, const String &comment,
                                             UInt64 max_array_length_, UInt64 max_string_length_, const std::optional<UInt64> &random_seed_)
  ...
  if (max_array_length > MAX_ARRAY_SIZE)
    throw Exception(ErrorCodes::TOO_LARGE_ARRAY_SIZE, "Too large array size in GenerateRandom: {}, maximum: {}", max_array_length,
                    MAX_ARRAY_SIZE);
  if (max_string_length > MAX_STRING_SIZE)
    throw Exception(ErrorCodes::TOO_LARGE_STRING_SIZE, "Too large string size in GenerateRandom: {}, maximum: {}", max_string_length,
                    MAX_STRING_SIZE);
...
void registerStorageGenerateRandom(StorageFactory &factory) {
  factory.registerStorage("GenerateRandom", [](const StorageFactory::Arguments &args) {
    ...
    return std::make_shared<StorageGenerateRandom>(args.table_id, args.columns, args.comment, max_array_length, max_string_length,
                                                   random_seed);
  });
}

Pipe StorageGenerateRandom::read(const Names &column_names, const StorageSnapshotPtr &storage_snapshot, SelectQueryInfo &query_info,
  ...
  if (preferred_block_size_bytes) {
    size_t estimated_row_size_bytes =
        estimateValueSize(std::make_shared<DataTypeTuple>(block_header.getDataTypes()), max_array_length, max_string_length);
    ...
    if (common::mulOverflow(max_block_size, estimated_row_size_bytes, estimated_block_size_bytes))
      throw Exception(ErrorCodes::TOO_LARGE_ARRAY_SIZE,
                      "Too large estimated block size in GenerateRandom table: its estimation leads to 64bit overflow");

    if (estimated_block_size_bytes > preferred_block_size_bytes) {
      max_block_size = static_cast<size_t>(static_cast<double>(max_block_size) * (static_cast<double>(preferred_block_size_bytes) /
                                                                                  static_cast<double>(estimated_block_size_bytes)));
      if (max_block_size == 0) max_block_size = 1;
    }
  }
