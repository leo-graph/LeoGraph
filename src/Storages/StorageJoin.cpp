...
StorageJoin::StorageJoin(DiskPtr disk_, const String &relative_path_, const StorageID &table_id_, const Names &key_names_, bool use_nulls_,
                         SizeLimits limits_, JoinKind kind_, JoinStrictness strictness_, const ColumnsDescription &columns_,
                         const ConstraintsDescription &constraints_, const String &comment, bool overwrite_, bool persistent_)
...
RWLockImpl::LockHolder StorageJoin::tryLockTimedWithContext(const RWLock &lock, RWLockImpl::Type type, ContextPtr context) const {
  ...
  const std::chrono::milliseconds acquire_timeout =
      context ? std::chrono::milliseconds(context->getSettingsRef()[Setting::lock_acquire_timeout].totalMilliseconds())
              : std::chrono::seconds(DBMS_DEFAULT_LOCK_ACQUIRE_TIMEOUT_SEC);
...
bool StorageJoin::optimize(const ASTPtr & /*query*/, const StorageMetadataPtr & /*metadata_snapshot*/, const ASTPtr &partition, bool final,
                           bool deduplicate, const Names & /* deduplicate_by_columns */, bool cleanup, ContextPtr context) {
  if (partition) throw Exception(ErrorCodes::NOT_IMPLEMENTED, "Partition cannot be specified when optimizing table of type Join");

  if (final) throw Exception(ErrorCodes::NOT_IMPLEMENTED, "FINAL cannot be specified when optimizing table of type Join");

  if (deduplicate) throw Exception(ErrorCodes::NOT_IMPLEMENTED, "DEDUPLICATE cannot be specified when optimizing table of type Join");

  if (cleanup) throw Exception(ErrorCodes::NOT_IMPLEMENTED, "CLEANUP cannot be specified when optimizing table of type Join");
...
void StorageJoin::mutate(const MutationCommands &commands, ContextPtr context) {
  ...
  // New scope controls lifetime of pipeline.
  {
    ...
    while (executor.pull(block)) {
      ...
      if (persistent) backup_stream.write(block);
    }
  }
  ...
  if (persistent) {
    ...
    for (const auto &file_name : files) {
      if (file_name.ends_with(".bin")) disk->removeFileIfExists(path + file_name);
    }
...
HashJoinPtr StorageJoin::getJoinLocked(std::shared_ptr<TableJoin> analyzed_join, String query_id, std::chrono::milliseconds acquire_timeout,
                                       const Names &required_columns_names) const {
  auto metadata_snapshot = getInMemoryMetadataPtr();
  ...
  if ((analyzed_join->forceNullableRight() && !use_nulls) ||
    ...
    throw Exception(ErrorCodes::INCOMPATIBLE_TYPE_OF_JOIN, "Table {} needs the same join_use_nulls setting as present in LEFT or FULL JOIN",
  ...
  if (key_names.size() != key_names_right.size() || key_names.size() != key_names_left.size())
    throw Exception(ErrorCodes::INCOMPATIBLE_TYPE_OF_JOIN,
                    "Number of keys in JOIN ON section ({}) doesn't match number of keys in Join engine ({})", key_names_right.size(),
                    key_names.size());
  ...
  for (const auto &key_name : key_names) {
    ...
    if (it == key_names_right.end())
      throw Exception(ErrorCodes::INCOMPATIBLE_TYPE_OF_JOIN,
                      "Key '{}' not found in JOIN ON section. Join engine key{} '{}' have to be used", key_name,
                      key_names.size() > 1 ? "s" : "", fmt::join(key_names, ", "));
  ...
  for (const auto &name : required_columns_names) right_sample_block.insert(getRightSampleBlock().getByName(name));
...
HashJoinPtr StorageJoin::getJoinLocked(std::shared_ptr<TableJoin> analyzed_join, ContextPtr context,
                                       const Names &required_columns_names) const {
  const String query_id = context ? context->getInitialQueryId() : RWLockImpl::NO_QUERY;
  const std::chrono::milliseconds acquire_timeout =
      context ? std::chrono::milliseconds(context->getSettingsRef()[Setting::lock_acquire_timeout].totalMilliseconds())
...
void StorageJoin::convertRightBlock(Block &block) const {
  ...
  if (!need_covert) return;

  for (auto &col : block) JoinCommon::convertColumnToNullable(col);
}

void registerStorageJoin(StorageFactory &factory) {
  auto has_builtin_fn = [](std::string_view name) {
    static const std::unordered_set<std::string_view> valid_settings = {"join_use_nulls",
                                                                        "max_rows_in_join",
                                                                        "max_bytes_in_join",
                                                                        "join_overflow_mode",
  ...
  auto creator_fn = [](const StorageFactory::Arguments &args) {
    ...
    if (auto opt_kind_id = tryGetIdentifierName(engine_args[1])) {
      ...
      if (kind_str == "left")
        ...
        if (strictness == JoinStrictness::Any) throw Exception(ErrorCodes::NOT_IMPLEMENTED, "ANY FULL JOINs are not implemented");
        kind = JoinKind::Full;
      }
    }
    ...
    for (size_t i = 2, size = engine_args.size(); i < size; ++i) {
      ...
      if (!opt_key) throw Exception(ErrorCodes::BAD_ARGUMENTS, "Parameter №{} of storage Join don't look like column name.", i + 1);

      key_names.push_back(*opt_key);
    }

    return std::make_shared<StorageJoin>(disk, args.relative_data_path, args.table_id, key_names, join_use_nulls,
                                         SizeLimits{max_rows_in_join, max_bytes_in_join, join_overflow_mode}, kind, strictness,
                                         args.columns, args.constraints, args.comment, join_any_take_last_row, persistent);
  };

  factory.registerStorage("Join", creator_fn,
...
class JoinSource : public ISource {
 ...
 protected:
  Chunk generate() override {
    if (join->data->columns.empty()) return {};

    Chunk chunk;
    if (!joinDispatch(join->kind, join->strictness, join->data->maps.front(), join->preferUseMapsAll(),
...
#undef M
    ...
    if (!rows_added) return {};

    Columns columns;
    columns.reserve(mut_columns.size());
    for (auto &col : mut_columns) columns.emplace_back(std::move(col));
  ...
  size_t fillColumns(const Map &map, MutableColumns &columns) {
    ...
    if (!position)
      position = decltype(position)(static_cast<void *>(new typename Map::const_iterator(map.begin())),
...
Pipe StorageJoin::read(const Names &column_names, const StorageSnapshotPtr &storage_snapshot, SelectQueryInfo & /*query_info*/,
