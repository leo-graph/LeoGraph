...
class StorageMySQL final : public IStorage, WithContext {
 public:
  StorageMySQL(const StorageID &table_id_, mysqlxx::PoolWithFailover &&pool_, const std::string &remote_database_name_,
               const std::string &remote_table_name_, bool replace_query_, const std::string &on_duplicate_clause_,
               const ColumnsDescription &columns_, const ConstraintsDescription &constraints_, const String &comment, ContextPtr context_,
  ...
  static Configuration processNamedCollectionResult(const NamedCollection &named_collection, MySQLSettings &storage_settings,
                                                    ContextPtr context_, bool require_table = true);

  static ColumnsDescription getTableStructureFromData(mysqlxx::PoolWithFailover &pool_, const String &database, const String &table,
...
class ReadFromMySQLStep final : public ISourceStep {
 public:
  ReadFromMySQLStep(const Block &sample_block_, mysqlxx::PoolWithFailoverPtr pool_, const std::string &query_str_,
  ...
  QueryPlanStepPtr clone() const override { return std::make_unique<ReadFromMySQLStep>(*this); }
