...
StorageExecutable::StorageExecutable(const StorageID& table_id_, const String& format, const ExecutableSettings& settings_,
                                     const std::vector<ASTPtr>& input_queries_, const ColumnsDescription& columns,
                                     const ConstraintsDescription& constraints, const String& comment)
    : IStorage(table_id_),
...
String StorageExecutable::getName() const {
  if (settings->is_executable_pool) return "ExecutablePool";
  return "Executable";
}

void StorageExecutable::read(QueryPlan& query_plan, const Names& column_names, const StorageSnapshotPtr& storage_snapshot,
                             SelectQueryInfo& query_info, ContextPtr context, QueryProcessingStage::Enum /*processed_stage*/,
                             size_t max_block_size, size_t /*threads*/) {
  ...
  if (!fileOrSymlinkPathStartsWith(script_path, user_scripts_path))
    throw Exception(ErrorCodes::UNSUPPORTED_METHOD, "Executable file {} must be inside user scripts folder {}", script_name,
                    user_scripts_path);

  if (!FS::exists(script_path))
    throw Exception(ErrorCodes::UNSUPPORTED_METHOD, "Executable file {} does not exist inside user scripts folder {}", script_name,
                    user_scripts_path);

  if (!FS::canExecute(script_path))
    throw Exception(ErrorCodes::UNSUPPORTED_METHOD, "Executable file {} is not executable inside user scripts folder {}", script_name,
                    user_scripts_path);
  ...
  if (settings->is_executable_pool) transformToSingleBlockSources(inputs);
  ...
  auto pipe =
      coordinator->createPipe(script_path, settings->script_arguments, std::move(inputs), std::move(sample_block), context, configuration);
...
void registerStorageExecutable(StorageFactory& factory) {
  auto register_storage = [](const StorageFactory::Arguments& args, bool is_executable_pool) -> StoragePtr {
    ...
    for (size_t i = 2; i < args.engine_args.size(); ++i) {
      if (args.engine_args[i]->children.empty())
        throw Exception(ErrorCodes::BAD_ARGUMENTS, "StorageExecutable argument \"{}\" is invalid query",
    ...
    if (is_executable_pool) {
      ...
      size_t max_execution_time_seconds =
          static_cast<size_t>(args.getContext()->getSettingsRef()[Setting::max_execution_time].totalSeconds());
    ...
    if (args.storage_def->settings) settings.loadFromQuery(*args.storage_def);
  ...
  factory.registerStorage(
      "Executable", [&](const StorageFactory::Arguments& args) { return register_storage(args, false /*is_executable_pool*/); },
      storage_features);

  factory.registerStorage(
      "ExecutablePool", [&](const StorageFactory::Arguments& args) { return register_storage(args, true /*is_executable_pool*/); },
      storage_features);
}
