...
RelationProfile ConditionSelectivityEstimator::estimateRelationProfile(const StorageMetadataPtr& metadata, const ActionsDAG::Node* filter,
                                                                       const ActionsDAG::Node* prewhere) const {
  ...
  std::vector<RPNElement> prewhere_rpn =
      RPNBuilder<RPNElement>(prewhere, getContext(), [&](const RPNBuilderTreeNode& node_, RPNElement& out) {
        return extractAtomFromTree(metadata, node_, out);
      }).extractRPN();
...
RelationProfile ConditionSelectivityEstimator::estimateRelationProfile(const StorageMetadataPtr& metadata,
                                                                       const RPNBuilderTreeNode& node) const {
...
RelationProfile ConditionSelectivityEstimator::estimateRelationProfileImpl(std::vector<RPNElement>& rpn,
    ...
    switch (element.function) {
      ...
      case RPNElement::FUNCTION_OR: {
        ...
        if (right_element->function == RPNElement::ALWAYS_TRUE || left_element->function == RPNElement::ALWAYS_FALSE)
          ...
          if (element.function == RPNElement::FUNCTION_AND) element.selectivity = left_element->selectivity * right_element->selectivity;
  ...
  for (const auto& [column_name, estimator] : column_estimators) {
    if (!isCompatibleStatistics(metadata, estimator.stats, column_name)) continue;
...
RelationProfile ConditionSelectivityEstimator::estimateRelationProfile(const StorageMetadataPtr& metadata,
                                                                       const ActionsDAG::Node* node) const {
...
bool ConditionSelectivityEstimator::isStale(const std::vector<DataPartPtr>& data_parts) const {
  if (data_parts.size() != parts_names.size()) return true;
  size_t idx = 0;
  for (const auto& data_part : data_parts) {
    if (parts_names[idx++] != data_part->name) return true;
  }
  return false;
}

bool ConditionSelectivityEstimator::extractAtomFromTree(const StorageMetadataPtr& metadata, const RPNBuilderTreeNode& node,
  ...
  if (node.isFunction()) {
    ...
    if (num_args == 2) {
      ...
      if (is_in_operator && !func.getArgumentAt(1).getASTNode()) {
        ...
        for (size_t i = 0; i < columns[0]->size(); ++i) tuple[i] = (*columns[0])[i];
      ...
      if (metadata) {
        ...
        if (column_desc) column_type = removeLowCardinalityAndNullable(column_desc->type);
      }
      /// In some cases we need to cast the type of const
      bool cast_not_needed =
          !column_type || !const_type ||
...
void ConditionSelectivityEstimatorBuilder::incrementRowCount(UInt64 rows) { estimator->total_rows += rows; }
...
ConditionSelectivityEstimatorPtr ConditionSelectivityEstimatorBuilder::getEstimator() const { return has_data ? estimator : nullptr; }
...
Float64 ConditionSelectivityEstimator::ColumnEstimator::estimateRanges(const PlainRanges& ranges) const {
  ...
  if (stats->getNumRows() == 0) return 0;
  Float64 selectivity = result / static_cast<Float64>(stats->getNumRows());
  selectivity = std::max<Float64>(selectivity, 0);
  return selectivity;
}

UInt64 ConditionSelectivityEstimator::ColumnEstimator::estimateCardinality() const { return stats->estimateCardinality(); }
...
const ConditionSelectivityEstimator::AtomMap ConditionSelectivityEstimator::atom_map{
    ...
    {"greaterOrEquals", [](RPNElement& out, const String& column, const Field& value) {
       out.function = RPNElement::FUNCTION_IN_RANGE;
...
bool ConditionSelectivityEstimator::RPNElement::tryToMergeClauses(RPNElement& lhs, RPNElement& rhs) {
  auto can_merge_with = [](const RPNElement& e, Function function_to_merge) {
    return (e.function == FUNCTION_IN_RANGE
            ...
            || (e.column_ranges.size() + e.column_not_ranges.size()) == 1 || e.function == FUNCTION_UNKNOWN) &&
           !e.finalized;
  };
  ...
  auto merge_column_ranges = [this](ColumnRanges& result_ranges, ColumnRanges& l_ranges, ColumnRanges& r_ranges, bool is_not) {
    ...
    for (auto& [column_name, ranges] : r_ranges) {
      if (!l_ranges.contains(column_name)) result_ranges.emplace(column_name, ranges);
    }
  };
...
void ConditionSelectivityEstimator::RPNElement::finalize(const ColumnEstimators& column_estimators_, const StorageMetadataPtr& metadata) {
  ...
  auto estimate_unknown_ranges = [&](const PlainRanges& ranges) {
    ...
    for (const Range& range : ranges.ranges) {
      if (range.isInfinite()) return 1.0;
  ...
  if (function == FUNCTION_OR) selectivity = 1 - selectivity;
}
}  // namespace DB
