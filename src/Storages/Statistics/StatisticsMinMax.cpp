...
StatisticsMinMax::StatisticsMinMax(const SingleStatisticsDescription &description, const DataTypePtr &data_type_)
    : IStatistics(description), data_type(removeNullable(data_type_)) {}
...
Float64 StatisticsMinMax::estimateLess(const Field &val) const {
  ...
  if (val_as_float > max) return static_cast<Float64>(row_count);

  if (min == max) return (val_as_float != max) ? 0 : static_cast<Float64>(row_count);

  return ((*val_as_float - min) / (max - min)) * static_cast<Float64>(row_count);
}

String StatisticsMinMax::getNameForLogs() const { return fmt::format("MinMax: ({}, {})", min, max); }
