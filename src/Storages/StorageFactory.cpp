...
void checkAllTypesAreAllowedInTable(const NamesAndTypesList &names_and_types) {
  for (const auto &elem : names_and_types)
    if (elem.type->cannotBeStoredInTables())
      throw Exception(ErrorCodes::DATA_TYPE_CANNOT_BE_USED_IN_TABLES, "Data type {} of column '{}' cannot be used in tables",
                      elem.type->getName(), elem.name);
}
...
void StorageFactory::registerStorage(const std::string &name, CreatorFn creator_fn, StorageFeatures features) {
  if (features.supports_settings && !features.has_builtin_setting_fn)
    throw Exception(ErrorCodes::LOGICAL_ERROR, "StorageFactory: Storage '{}' supports settings but has_builtin_setting_fn is not provided",
                    name);
...
StoragePtr StorageFactory::get(const ASTCreateQuery &query, const String &relative_data_path, ContextMutablePtr local_context,
                               ContextMutablePtr context, const ColumnsDescription &columns, const ConstraintsDescription &constraints,
                               LoadingStrictnessLevel mode, bool is_restore_from_backup) const {
  String name;
  String comment;
  ...
  if (query.is_ordinary_view) {
    if (query.storage) throw Exception(ErrorCodes::INCORRECT_QUERY, "Specifying ENGINE is not allowed for a View");

    name = "View";
  } else if (query.is_dictionary) {
    if (query.storage) throw Exception(ErrorCodes::INCORRECT_QUERY, "Specifying ENGINE is not allowed for a Dictionary");
    ...
    if (query.is_materialized_view) {
      ...
      if (!query.storage) throw Exception(ErrorCodes::INCORRECT_QUERY, "Incorrect CREATE query: storage required");

      if (!storage_def->engine) throw Exception(ErrorCodes::ENGINE_REQUIRED, "Incorrect CREATE query: ENGINE required");
      ...
      if (engine_def.arguments) has_engine_args = true;
      ...
      if (name == "Loop") {
        throw Exception(ErrorCodes::INCORRECT_QUERY,
                        "Direct creation of tables with ENGINE Loop is not supported, use Loop as a table function only");
      }
      ...
      if (storage_def->settings) check_feature("SETTINGS clause", [](StorageFeatures features) { return features.supports_settings; });

      if (storage_def->partition_by || storage_def->primary_key || storage_def->order_by || storage_def->sample_by)
        check_feature("PARTITION_BY, PRIMARY_KEY, ORDER_BY or SAMPLE_BY clauses",
                      [](StorageFeatures features) { return features.supports_sort_order; });

      if (storage_def->ttl_table || !columns.getColumnTTLs().empty())
        check_feature("TTL clause", [](StorageFeatures features) { return features.supports_ttl; });

      if (query.columns_list && query.columns_list->indices && !query.columns_list->indices->children.empty())
        check_feature("skipping indices", [](StorageFeatures features) { return features.supports_skipping_indices; });

      if (query.columns_list && query.columns_list->projections && !query.columns_list->projections->children.empty())
        check_feature("projections", [](StorageFeatures features) { return features.supports_projections; });
    }
  }

  if (query.comment) comment = query.comment->as<ASTLiteral &>().value.safeGet<String>();

  ASTs empty_engine_args;
  Arguments arguments{.engine_name = name,
...
std::optional<AccessTypeObjects::Source> StorageFactory::getSourceAccessObject(const String &table_engine) const {
  if (table_engine.empty()) return {};
  const auto it = storages.find(table_engine);
  if (it == storages.end()) throw Exception(ErrorCodes::UNKNOWN_STORAGE, "Unknown table engine {}", table_engine);
...
const StorageFactory::StorageFeatures &StorageFactory::getStorageFeatures(const String &storage_name) const {
  ...
  if (it == storages.end()) throw Exception(ErrorCodes::UNKNOWN_STORAGE, "Unknown table engine {}", storage_name);
