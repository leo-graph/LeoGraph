...
ColumnPtr FuzzQuerySource::createColumn() {
  ...
  while (row_num < block_size) {
    ...
    if (base_before_fuzz == fuzzed_text) continue;
...
StorageFuzzQuery::StorageFuzzQuery(const StorageID &table_id_, const ColumnsDescription &columns_, const String &comment_,
...
Pipe StorageFuzzQuery::read(const Names &column_names, const StorageSnapshotPtr &storage_snapshot, SelectQueryInfo & /*query_info*/,
...
StorageFuzzQuery::Configuration StorageFuzzQuery::getConfiguration(ASTs &engine_args, ContextPtr local_context) {
  ...
  if (engine_args.empty() || engine_args.size() > 3)
    throw Exception(ErrorCodes::NUMBER_OF_ARGUMENTS_DOESNT_MATCH,
                    "FuzzQuery requires 1 to 3 arguments: query, max_query_length, random_seed");
  ...
  if (engine_args.size() >= 2) {
    ...
    if (!literal.value.isNull()) configuration.max_query_length = checkAndGetLiteralArgument<UInt64>(literal, "max_query_length");
  }
...
void registerStorageFuzzQuery(StorageFactory &factory) {
  factory.registerStorage("FuzzQuery", [](const StorageFactory::Arguments &args) -> std::shared_ptr<StorageFuzzQuery> {
    ASTs &engine_args = args.engine_args;

    if (engine_args.empty()) throw Exception(ErrorCodes::NUMBER_OF_ARGUMENTS_DOESNT_MATCH, "Storage FuzzQuery must have arguments.");
