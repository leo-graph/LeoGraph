...
StorageXDBC::StorageXDBC(const StorageID &table_id_, const std::string &remote_database_name_, const std::string &remote_table_name_,
                         ColumnsDescription columns_, ConstraintsDescription constraints_, const String &comment, ContextPtr context_,
                         const BridgeHelperPtr bridge_helper_)
    : IStorageURLBase("", context_, table_id_, IXDBCBridgeHelper::DEFAULT_FORMAT, getFormatSettings(context_), columns_, constraints_,
...
std::string StorageXDBC::getReadMethod() const { return Poco::Net::HTTPRequest::HTTP_POST; }
...
std::function<void(std::ostream &)> StorageXDBC::getReadPOSTDataCallback(const Names &column_names,
                                                                         ...
                                                                         const SelectQueryInfo &query_info, const ContextPtr &local_context,
                                                                         QueryProcessingStage::Enum & /*processed_stage*/,
                                                                         size_t /*max_block_size*/) const {
  String query = transformQueryForExternalDatabase(query_info, column_names, columns_description.getOrdinary(),
                                                   bridge_helper->getIdentifierQuotingStyle(), LiteralEscapingStyle::Regular,
                                                   remote_database_name, remote_table_name, local_context);
...
void StorageXDBC::read(QueryPlan &query_plan, const Names &column_names, const StorageSnapshotPtr &storage_snapshot,
...
SinkToStoragePtr StorageXDBC::write(const ASTPtr & /* query */, const StorageMetadataPtr &metadata_snapshot, ContextPtr local_context,
  ...
  for (const auto &[param, value] : url_params) request_uri.addQueryParameter(param, value);
  ...
  return std::make_shared<StorageURLSink>(
      request_uri.toString(), format_name, getFormatSettings(local_context), metadata_snapshot->getSampleBlock(), local_context,
      ConnectionTimeouts::getHTTPTimeouts(local_context->getSettingsRef(), local_context->getServerSettings()), compression_method);
}

bool StorageXDBC::supportsSubsetOfColumns(const ContextPtr &) const { return true; }
...
std::string StorageXDBC::getName() const { return bridge_helper->getName(); }
...
void registerXDBCStorage(StorageFactory &factory, const std::string &name) {
  factory.registerStorage(
      name,
      [name](const StorageFactory::Arguments &args) {
        ...
        if (auto named_collection =
                tryGetNamedCollectionWithOverrides(engine_args, args.getLocalContext(), true, nullptr, &args.table_id)) {
          if (Poco::toLower(name) == "jdbc") {
            validateNamedCollection<>(*named_collection, {"datasource"}, {"schema", "external_database", "external_table", "table"});
            ...
            if (named_collection->has("external_table") == named_collection->has("table"))
              throw Exception(ErrorCodes::BAD_ARGUMENTS, "Table function '{0}' must have exactly one `external_table` / `table` argument",
                              name);
        ...
        BridgeHelperPtr bridge_helper = std::make_shared<XDBCBridgeHelper<BridgeHelperMixin>>(
            args.getContext(), args.getContext()->getSettingsRef()[Setting::http_receive_timeout].value, connection_string,
            args.getContext()->getSettingsRef()[Setting::odbc_bridge_use_connection_pooling].value);
        return std::make_shared<StorageXDBC>(args.table_id, database_or_schema, table, args.columns, args.constraints, args.comment,
                                             args.getContext(), bridge_helper);
      },
      {
...
void registerStorageJDBC(StorageFactory &factory) { registerXDBCStorage<JDBCBridgeMixin>(factory, "JDBC"); }

void registerStorageODBC(StorageFactory &factory) { registerXDBCStorage<ODBCBridgeMixin>(factory, "ODBC"); }
}  // namespace DB
