...
struct MarksWeightFunction {
  ...
  size_t operator()(const MarksInCompressedFile &marks) const { return marks.approximateMemoryUsage() + MARK_CACHE_OVERHEAD; }
};
...
class MarkCache : public CacheBase<UInt128, MarksInCompressedFile, UInt128TrivialHash, MarksWeightFunction> {
 ...
 public:
  MarkCache(const String &cache_policy, CurrentMetrics::Metric size_in_bytes_metric, CurrentMetrics::Metric count_metric,
            size_t max_size_in_bytes, double size_ratio);
  ...
  MappedPtr getForAsyncLoading(const Key &key) {
    ...
    if (result) ProfileEvents::increment(ProfileEvents::MarkCacheHits);
