...
class GeneralizeGroupingFunctionForDistributedVisitor : public InDepthQueryTreeVisitor<GeneralizeGroupingFunctionForDistributedVisitor> {
 public:
  static void visitImpl(QueryTreeNodePtr& node) {
    ...
    if (!ordinary_grouping && function_name != "groupingForRollup" && function_name != "groupingForCube" &&
    ...
    if (!ordinary_grouping) {
      ...
      if (arguments.empty()) throw Exception(ErrorCodes::LOGICAL_ERROR, "Grouping function specialization must have arguments");
    ...
    auto function_adaptor = std::make_shared<FunctionToOverloadResolverAdaptor>(std::make_shared<FunctionGrouping>(false));
