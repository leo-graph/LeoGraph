...
class StorageDummy final : public IStorage {
 public:
  StorageDummy(const StorageID& table_id_, const ColumnsDescription& columns_,
               const StorageSnapshotPtr& original_storage_snapshot_ = nullptr, bool supports_replication_ = false);
...
class ReadFromDummy final : public SourceStepWithFilter {
 public:
  explicit ReadFromDummy(const Names& column_names_, const SelectQueryInfo& query_info_, const StorageSnapshotPtr& storage_snapshot_,
                         const ContextPtr& context_, const StorageDummy& storage_);

  const StorageDummy& getStorage() const { return storage; }
