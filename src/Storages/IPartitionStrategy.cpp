...
ASTPtr buildHivePartitionAST(ASTPtr partition_by, const NamesAndTypesList& partition_columns) {
  ASTs concat_args;

  if (const auto* tuple_function = partition_by->as<ASTFunction>(); tuple_function && tuple_function->name == "tuple") {
    ...
    if (partition_columns.size() != 1) {
      throw Exception(ErrorCodes::LOGICAL_ERROR, "Expected partition expression to contain a single argument, got {} instead",
...
Block buildBlockWithoutPartitionColumns(const Block& sample_block,
...
std::shared_ptr<IPartitionStrategy> createHivePartitionStrategy(ASTPtr partition_by, const Block& sample_block, ContextPtr context,
                                                                const std::string& file_format, bool globbed_path,
  ...
  const auto partition_key_description =
  ...
  for (const auto& partition_expression_column : partition_key_description.sample_block) {
    if (!sample_block.has(partition_expression_column.name)) {
      throw Exception(ErrorCodes::BAD_ARGUMENTS,
                      "Hive partitioning expects that the partition by expression columns are a part of the storage columns, could not "
                      "find '{}' in storage",
                      partition_expression_column.name);
    }

    const auto& type = partition_expression_column.type;
    const bool is_type_supported =
        isInteger(type) || isDateOrDate32OrTimeOrTime64OrDateTimeOrDateTime64(type) || isStringOrFixedString(type);
  ...
  return std::make_shared<HiveStylePartitionStrategy>(partition_key_description, sample_block, context, file_format,
                                                      partition_columns_in_data_file);
}

std::shared_ptr<IPartitionStrategy> createWildcardPartitionStrategy(ASTPtr partition_by, const Block& sample_block, ContextPtr context,
                                                                    bool contains_partition_wildcard, bool partition_columns_in_data_file) {
  if (!partition_by) {
    return nullptr;
  }
...
NamesAndTypesList IPartitionStrategy::getPartitionColumns() const { return partition_key_description.sample_block.getNamesAndTypesList(); }

const KeyDescription& IPartitionStrategy::getPartitionKeyDescription() const { return partition_key_description; }

IPartitionStrategy::PartitionExpressionActionsAndColumnName IPartitionStrategy::getPartitionExpressionActions(ASTPtr& expression_ast) {
  ...
  result.actions = std::make_shared<ExpressionActions>(std::move(actions_dag), ExpressionActionsSettings(context), false);
  result.column_name = expression_ast->getColumnName();

  if (!result.actions->getActionsDAG().hasNonDeterministic()) cached_result = result;

  return result;
}

std::shared_ptr<IPartitionStrategy> PartitionStrategyFactory::get(StrategyType strategy, ASTPtr partition_by,
                                                                  const NamesAndTypesList& partition_columns, ContextPtr context,
                                                                  const std::string& file_format, bool globbed_path,
                                                                  bool contains_partition_wildcard, bool partition_columns_in_data_file) {
  Block block;
  for (const auto& partition_column : partition_columns) {
  ...
  switch (strategy) {
    case StrategyType::WILDCARD:
      return createWildcardPartitionStrategy(partition_by, block, context, contains_partition_wildcard, partition_columns_in_data_file);
    case StrategyType::HIVE:
      return createHivePartitionStrategy(partition_by, block, context, file_format, globbed_path, partition_columns_in_data_file);
    case StrategyType::NONE: {
      if (!partition_columns_in_data_file && strategy == PartitionStrategyFactory::StrategyType::NONE) {
        throw Exception(ErrorCodes::BAD_ARGUMENTS, "Partition strategy `none` cannot be used with partition_columns_in_data_file=0");
      }
...
WildcardPartitionStrategy::WildcardPartitionStrategy(KeyDescription partition_key_description_, const Block& sample_block_,
                                                     ContextPtr context_)
...
std::string WildcardPartitionStrategy::getPathForRead(const std::string& prefix) { return prefix; }

std::string WildcardPartitionStrategy::getPathForWrite(const std::string& prefix, const std::string& partition_key) {
  return PartitionedSink::replaceWildcards(prefix, partition_key);
}

HiveStylePartitionStrategy::HiveStylePartitionStrategy(KeyDescription partition_key_description_, const Block& sample_block_,
                                                       ContextPtr context_, const std::string& file_format_,
...
std::string HiveStylePartitionStrategy::getPathForRead(const std::string& prefix) { return prefix + "**." + Poco::toLower(file_format); }

std::string HiveStylePartitionStrategy::getPathForWrite(const std::string& prefix, const std::string& partition_key) {
  std::string path;
...
ColumnRawPtrs HiveStylePartitionStrategy::getFormatChunkColumns(const Chunk& chunk) {
  ...
  if (chunk.getNumColumns() != sample_block.columns()) {
    throw Exception(ErrorCodes::LOGICAL_ERROR, "Incorrect number of columns in chunk. Expected {}, found {}", sample_block.columns(),
                    chunk.getNumColumns());
  }
