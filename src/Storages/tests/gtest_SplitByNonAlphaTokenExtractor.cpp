...
std::ostream& operator<<(std::ostream& ostr, const SplitByNonAlphaTokenizerTestCase& test_case) { return ostr << test_case.description; }
...
INSTANTIATE_TEST_SUITE_P(NoTokens, SplitByNonAlphaTokenizerTest,
...
INSTANTIATE_TEST_SUITE_P(ShortSingleToken, SplitByNonAlphaTokenizerTest,
                         ::testing::ValuesIn(std::initializer_list<SplitByNonAlphaTokenizerTestCase>{
                             {"Short single token", "foo", {"foo"}},
                             {"Short single token surruonded by whitespace", "\t\vfoo\n\r", {"foo"}}}));

INSTANTIATE_TEST_SUITE_P(UTF8, SplitByNonAlphaTokenizerTest,
                         ::testing::ValuesIn(std::initializer_list<SplitByNonAlphaTokenizerTestCase>{
...
INSTANTIATE_TEST_SUITE_P(MultipleTokens, SplitByNonAlphaTokenizerTest,
                         ::testing::ValuesIn(std::initializer_list<SplitByNonAlphaTokenizerTestCase>{
                             {"Multiple tokens separated by whitespace", "\nabc 123\tXYZ\r", {"abc", "123", "XYZ"}},
...
INSTANTIATE_TEST_SUITE_P(
    SIMD_Cases, SplitByNonAlphaTokenizerTest,
    ::testing::ValuesIn(std::initializer_list<SplitByNonAlphaTokenizerTestCase>{
        {"First 16 bytes are empty, then a shor token", "                abcdef", {"abcdef"}},
        {"Token crosses boundary of 16-byte chunk", "            abcdef", {"abcdef"}},
        {"Token ends at the end of 16-byte chunk", "          abcdef", {"abcdef"}},
        {"Token crosses bondaries of multiple 16-byte chunks", "abcdefghijklmnopqrstuvwxyz", {"abcdefghijklmnopqrstuvwxyz"}},
    }));
