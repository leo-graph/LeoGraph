...
TEST(AdaptiveIndexGranularity, FillGranularityToyTests) {
  ...
  {  /// Granule size is less than block size. Block contains multiple granules.
    ...
    for (size_t i = 0; i < index_granularity.getMarksCount(); ++i) EXPECT_EQ(index_granularity.getMarkRows(i), 2);
  }
  ...
  {  /// Granule size is more than block size. Whole block (and maybe more) can be placed in single granule.
    ...
    fillIndexGranularityImpl(index_granularity, 0, granularity, block1.rows());
    EXPECT_EQ(index_granularity.getMarksCount(), 1);
    for (size_t i = 0; i < index_granularity.getMarksCount(); ++i) EXPECT_EQ(index_granularity.getMarkRows(i), 64);
  }
  ...
  {  /// Blocks with granule size
    ...
    fillIndexGranularityImpl(index_granularity, 0, granularity, block1.rows());
    EXPECT_EQ(index_granularity.getMarksCount(), 1);
    for (size_t i = 0; i < index_granularity.getMarksCount(); ++i) EXPECT_EQ(index_granularity.getMarkRows(i), block1.rows());
  }
...
TEST(AdaptiveIndexGranularity, FillGranularitySequenceOfBlocks) {
  {  /// Three equal blocks
    ...
    for (size_t i = 0; i < index_granularity.getMarksCount(); ++i) EXPECT_EQ(index_granularity.getMarkRows(i), 128);
  }
  {  /// Three blocks of different size
    ...
    for (size_t i = 0; i < index_granularity.getMarksCount(); ++i) EXPECT_EQ(index_granularity.getMarkRows(i), 32);
  }
  {  /// Three small blocks
    ...
    for (size_t i = 0; i < index_granularity.getMarksCount(); ++i) EXPECT_EQ(index_granularity.getMarkRows(i), 512);
  }
}
...
TEST(AdaptiveIndexGranularity, TestIndexGranularityConstant) {
  auto test = [](MergeTreeIndexGranularity& index_granularity, size_t granularity_rows) {
    ...
    for (size_t i = 0; i < 10; ++i) index_granularity.appendMark(granularity_rows);
