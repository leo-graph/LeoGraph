...
DB::StoragePtr createStorage(DB::DiskPtr &disk) {
  ...
  StoragePtr table = std::make_shared<StorageLog>("Log", disk, "table/", StorageID("test", "test"), ColumnsDescription{names_and_types},
...
std::string readData(DB::StoragePtr &table, const DB::ContextPtr context) {
  ...
  QueryProcessingStage::Enum stage = table->getQueryProcessingStage(context, QueryProcessingStage::Complete, storage_snapshot, query_info);

  QueryPlan plan;
  table->read(plan, column_names, storage_snapshot, query_info, context, stage, 8192, 1);

  auto pipeline = QueryPipelineBuilder::getPipeline(
      std::move(*plan.buildQueryPipeline(QueryPlanOptimizationSettings(context), BuildQueryPipelineSettings(context))));
