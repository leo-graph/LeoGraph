...
struct State {
  ...
  const NamesAndTypesList& getColumns(size_t idx = 0) const { return tables[idx].columns; }

  std::vector<TableWithColumnNamesAndTypes> getTables(size_t num = 0) const {
    std::vector<TableWithColumnNamesAndTypes> res;
    for (size_t i = 0; i < std::min(num, tables.size()); ++i) res.push_back(tables[i]);
    return res;
  }
 ...
 private:
  ...
  const std::vector<TableWithColumnNamesAndTypes> tables{
      TableWithColumnNamesAndTypes(createDBAndTable("table"),
                                   {
      ...
      TableWithColumnNamesAndTypes(createDBAndTable("table2"),
                                   {
                                       {"num", std::make_shared<DataTypeUInt8>()},
                                       {"attr", std::make_shared<DataTypeString>()},
                                   }),
      TableWithColumnNamesAndTypes(createDBAndTable("external_table"),
                                   {
  ...
  explicit State() : context(Context::createCopy(getContext().context)) {
    ...
    for (const auto& tab : tables) {
      ...
      database->attachTable(context, table_name,
                            std::make_shared<StorageMemory>(StorageID(db_name, table_name), ColumnsDescription{tab.columns},
                                                            ConstraintsDescription{}, String{}, MemorySettings{}),
                            {});
    }
...
static void checkOld(const State& state, size_t table_num, const std::string& query, const std::string& expected) {
  ...
  query_info.syntax_analyzer_result =
      TreeRewriter(state.context)
          .analyzeSelect(ast, DB::TreeRewriterResult(state.getColumns(0)), select_options, state.getTables(table_num));
  query_info.query = ast;
  std::string transformed_query = transformQueryForExternalDatabase(query_info, query_info.syntax_analyzer_result->requiredSourceColumns(),
...
static QueryTreeNodePtr findTableExpression(const QueryTreeNodePtr& node, const String& table_name) {
  if (node->getNodeType() == QueryTreeNodeType::TABLE) {
    if (node->as<TableNode>()->getStorageID().table_name == table_name) return node;
  }

  if (node->getNodeType() == QueryTreeNodeType::JOIN) {
    if (auto res = findTableExpression(node->as<JoinNode>()->getLeftTableExpression(), table_name)) return res;
    if (auto res = findTableExpression(node->as<JoinNode>()->getRightTableExpression(), table_name)) return res;
  }
  return nullptr;
}

/// `column_names` - Normally it's passed to query plan step. But in test we do it manually.
static void checkNewAnalyzer(const State& state, const Names& column_names, const std::string& query, const std::string& expected) {
  ParserSelectQuery parser;
  ASTPtr ast = parseQuery(parser, query, 1000, 1000, 1000000);
  ...
  if (!query_node) throw Exception(ErrorCodes::LOGICAL_ERROR, "QueryNode expected");

  query_info.table_expression = findTableExpression(query_node->getJoinTree(), "table");

  std::string transformed_query =
      transformQueryForExternalDatabase(query_info, column_names, state.getColumns(0), IdentifierQuotingStyle::DoubleQuotes,
                                        LiteralEscapingStyle::Regular, "test", "table", state.context);

  EXPECT_EQ(transformed_query, expected) << query;
}

static void check(const State& state, size_t table_num, const Names& column_names, const std::string& query, const std::string& expected,
...
TEST(TransformQueryForExternalDatabase, InWithSingleElement) {
  ...
  check(state, 1, {"column"}, "SELECT column FROM test.table WHERE 1 IN (1)", R"(SELECT "column" FROM "test"."table" WHERE 1 = 1)",
        R"(SELECT "column" FROM "test"."table")");

  check(state, 1, {"column"}, "SELECT column FROM test.table WHERE column IN (1, 2)",
        R"(SELECT "column" FROM "test"."table" WHERE "column" IN (1, 2))");

  check(state, 1, {"field"}, "SELECT field FROM test.table WHERE field NOT IN ('hello', 'world')",
...
TEST(TransformQueryForExternalDatabase, InWithMultipleColumns) {
  ...
  check(state, 1, {"column"}, "SELECT column FROM test.table WHERE (1,1) IN ((1,1))", R"(SELECT "column" FROM "test"."table" WHERE 1 = 1)",
        R"(SELECT "column" FROM "test"."table")");
  check(state, 1, {"field", "value"}, "SELECT field, value FROM test.table WHERE (field, value) IN (('foo', 'bar'))",
        R"(SELECT "field", "value" FROM "test"."table" WHERE ("field", "value") IN (('foo', 'bar')))");
  check(state, 1, {"field", "value"}, "SELECT field, value FROM test.table WHERE (field, value) IN (('foo', 'bar'), ('qux', 'baz'))",
...
TEST(TransformQueryForExternalDatabase, InWithTable) {
  ...
  check(state, 1, {"column"}, "SELECT column FROM test.table WHERE 1 IN external_table", R"(SELECT "column" FROM "test"."table")");
  check(state, 1, {"column"}, "WITH x as (SELECT * FROM external_table) SELECT column FROM test.table WHERE 1 IN (x)",
        R"(SELECT "column" FROM "test"."table")");
  check(state, 1, {"column", "field", "value"}, "SELECT column, field, value FROM test.table WHERE column IN (1, 2)",
        R"(SELECT "column", "field", "value" FROM "test"."table" WHERE "column" IN (1, 2))");
  check(state, 1, {"column"}, "SELECT column FROM test.table WHERE column NOT IN external_table AND column = 123",
...
TEST(TransformQueryForExternalDatabase, Like) {
  ...
  check(state, 1, {"field"}, "SELECT field FROM test.table WHERE field LIKE '%hello%'",
        R"(SELECT "field" FROM "test"."table" WHERE "field" LIKE '%hello%')");
  check(state, 1, {"field"}, "SELECT field FROM test.table WHERE field NOT LIKE 'w%rld'",
...
TEST(TransformQueryForExternalDatabase, MultipleAndSubqueries) {
  ...
  check(state, 1, {"column"},
        "SELECT column FROM test.table WHERE toString(column) = '42' AND left(toString(column), 10) = RIGHT(toString(column), 10) AND "
        "column = 42",
...
TEST(TransformQueryForExternalDatabase, Issue7245) {
  ...
  check(
      state, 1, {"apply_id", "apply_type", "apply_status", "create_time"},
      "SELECT apply_id FROM test.table WHERE apply_type = 2 AND create_time > addDays(toDateTime('2019-01-01 01:02:03', 'UTC'),-7) AND "
      "apply_status IN (3,4)",
...
TEST(TransformQueryForExternalDatabase, Aliases) {
  ...
  check(state, 1, {"field"}, "SELECT field AS value, field AS display FROM table WHERE field NOT IN ('') AND display LIKE '%test%'",
...
TEST(TransformQueryForExternalDatabase, TupleSurroundPredicates) {
  ...
  check(state, 1, {"column", "field", "a"}, "SELECT column, field, a FROM table WHERE ((column > 10) AND (length(field) > 0)) AND a > 0",
...
TEST(TransformQueryForExternalDatabase, NoStrict) {
  ...
  check(state, 1, {"field"}, "SELECT field FROM table WHERE field IN (SELECT attr FROM table2)", R"(SELECT "field" FROM "test"."table")");
}
...
TEST(TransformQueryForExternalDatabase, Strict) {
  ...
  check(state, 1, {"field"}, "SELECT field FROM table WHERE field = '1'", R"(SELECT "field" FROM "test"."table" WHERE "field" = '1')");
  check(state, 1, {"field"}, "SELECT field FROM table WHERE field IN ('1', '2')",
        R"(SELECT "field" FROM "test"."table" WHERE "field" IN ('1', '2'))");
  check(state, 1, {"field"}, "SELECT field FROM table WHERE field LIKE '%test%'",
...
TEST(TransformQueryForExternalDatabase, Null) {
  ...
  check(state, 1, {"field"}, "SELECT field FROM table WHERE field IS NULL", R"(SELECT "field" FROM "test"."table" WHERE "field" IS NULL)",
        R"(SELECT "field" FROM "test"."table" WHERE 1 = 0)");
  check(state, 1, {"field"}, "SELECT field FROM table WHERE field IS NOT NULL",
        R"(SELECT "field" FROM "test"."table" WHERE "field" IS NOT NULL)", R"(SELECT "field" FROM "test"."table")");

  check(state, 1, {"field"}, "SELECT field FROM table WHERE isNull(field)", R"(SELECT "field" FROM "test"."table" WHERE "field" IS NULL)",
        R"(SELECT "field" FROM "test"."table" WHERE 1 = 0)");
  check(state, 1, {"field"}, "SELECT field FROM table WHERE isNotNull(field)",
        R"(SELECT "field" FROM "test"."table" WHERE "field" IS NOT NULL)", R"(SELECT "field" FROM "test"."table")");
}
...
TEST(TransformQueryForExternalDatabase, ToDate) {
  ...
  check(state, 1, {"a", "b", "foo"}, "SELECT foo FROM table WHERE a=10 AND b=toDate('2019-10-05', 'UTC')",
...
TEST(TransformQueryForExternalDatabase, Analyzer) {
  ...
  check(state, 1, {"field"}, "SELECT count() FROM table WHERE field LIKE '%name_%'",
        R"(SELECT "field" FROM "test"."table" WHERE "field" LIKE '%name_%')");

  check(state, 1, {"column"}, "SELECT 1 FROM table", R"(SELECT "column" FROM "test"."table")");

  check(state, 1, {"column"}, "SELECT sleepEachRow(1) FROM table", R"(SELECT "column" FROM "test"."table")");

  check(
      state, 1, {"column", "apply_id", "apply_type", "apply_status", "create_time", "field", "value", "a", "b", "foo"},
  ...
  check(state, 1, {"is_value"}, "SELECT is_value FROM table WHERE is_value = true",
        R"(SELECT "is_value" FROM "test"."table" WHERE "is_value" = true)");

  check(state, 1, {"is_value"}, "SELECT is_value FROM table WHERE is_value = 1",
