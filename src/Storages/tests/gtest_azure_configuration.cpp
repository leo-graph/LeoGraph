...
#  define ASSERT_THROW_ERROR_CODE(statement, expected_exception, expected_code, expected_msg) \
    ASSERT_THROW(                                                                             \
        try { (void)(statement); } catch (const expected_exception& e) {                      \
...
class StorageAzureConfigurationFriend : private StorageAzureConfiguration {
 public:
  ...
  const AzureBlobStorage::ConnectionParams& getConnectionParams() { return connection_params; }
};
...
TEST(StorageAzureConfiguration, FromNamedCollectionWithExtraCredentialsAndAccount) {
  ...
  ASSERT_THROW_ERROR_CODE(conf.fromNamedCollection(*collection, Context::getGlobalContextInstance()), Exception, ErrorCodes::BAD_ARGUMENTS,
                          "Choose only one");
}
...
TEST(StorageAzureConfiguration, FromASTWithExtraCredentials) {
  std::string query =
      "DESCRIBE TABLE azureBlobStorage('https://azurite1:10000/devstoreaccount1', 'test_container', 'test_blob.csv', "
      "extra_credentials(client_id='test_client_id', tenant_id='test_tenant_id'))";
...
TEST(StorageAzureConfiguration, FromASTWithAccount) {
  std::string query =
      "DESCRIBE TABLE azureBlobStorage('https://azurite1:10000/devstoreaccount1', 'test_container', 'test_blob.csv', 'account_name', "
      "'account_key')";
...
TEST(StorageAzureConfiguration, FromASTWithURL) {
  std::string query =
      "DESCRIBE TABLE azureBlobStorage('https://azurite1:10000/devstoreaccount1?foo=bar', 'test_container', 'test_blob.csv')";
...
TEST(StorageAzureConfiguration, FromASTWithExtraCredentialsAndAccount) {
  std::string query =
      "DESCRIBE TABLE azureBlobStorage('https://azurite1:10000/devstoreaccount1', 'test_container', 'test_blob.csv', 'account_name', "
      "'account_key', extra_credentials(client_id='test_client_id', tenant_id='test_tenant_id'))";

  ASTs engine_args = getEngineArgs(query);
  StorageAzureConfigurationFriend conf;

  ASSERT_THROW_ERROR_CODE(conf.fromAST(engine_args, Context::getGlobalContextInstance(), false), Exception, ErrorCodes::BAD_ARGUMENTS,
                          "Choose only one");
}
...
TEST(StorageAzureConfiguration, FromASTWithPartialExtraCredentials) {
  std::string query =
      "DESCRIBE TABLE azureBlobStorage('https://azurite1:10000/devstoreaccount1', 'test_container', 'test_blob.csv', 'account_name', "
      "'account_key', extra_credentials(tenant_id='test_tenant_id'))";
