...
TEST(IndexGranularityCompactParts, FillGranularitySequenceOfBlocks) {
  ...
  {  /// Three large blocks
    ...
    for (size_t i = 0; i < 3; ++i) fillIndexGranularityImpl(index_granularity, index_offset, granularity, rows);

    EXPECT_EQ(index_granularity.getMarksCount(), 3);
    for (size_t i = 0; i < 3; ++i) EXPECT_EQ(index_granularity.getMarkRows(i), rows);
  }
}
