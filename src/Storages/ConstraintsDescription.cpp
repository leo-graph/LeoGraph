...
String ConstraintsDescription::toString() const {
  ...
  for (const auto &constraint : constraints) list.children.push_back(constraint);
...
ConstraintsDescription ConstraintsDescription::parse(const String &str) {
  ...
  for (const auto &constraint : list->children) res.constraints.push_back(constraint);
...
std::vector<std::vector<CNFQueryAtomicFormula>> ConstraintsDescription::buildConstraintData() const {
  ...
  for (const auto &constraint : filterConstraints(ConstraintsDescription::ConstraintType::ALWAYS_TRUE)) {
    ...
    for (const auto &group : cnf.getStatements()) constraint_data.emplace_back(std::begin(group), std::end(group));
  }
...
std::vector<CNFQueryAtomicFormula> ConstraintsDescription::getAtomicConstraintData() const {
  ...
  for (const auto &constraint : filterConstraints(ConstraintsDescription::ConstraintType::ALWAYS_TRUE)) {
    const auto cnf = TreeCNFConverter::toCNF(constraint->as<ASTConstraintDeclaration>()->expr).pullNotOutFunctions();
    for (const auto &group : cnf.getStatements()) {
      if (group.size() == 1) constraint_data.push_back(*group.begin());
    }
  }
...
ConstraintsExpressions ConstraintsDescription::getExpressions(const DB::ContextPtr context,
  ...
  for (const auto &constraint : constraints) {
    ...
    if (constraint_ptr->type == ASTConstraintDeclaration::Type::CHECK) {
      ...
      res.push_back(
          ExpressionAnalyzer(constraint_ptr->expr->clone(), syntax_result, context).getActions(false, true, CompileExpressions::yes));
    }
  }
  return res;
}

const ComparisonGraph<ASTPtr> &ConstraintsDescription::getGraph() const { return *graph; }

const std::vector<std::vector<CNFQueryAtomicFormula>> &ConstraintsDescription::getConstraintData() const { return cnf_constraints; }

const ASTs &ConstraintsDescription::getConstraints() const { return constraints; }
...
std::optional<ConstraintsDescription::AtomIds> ConstraintsDescription::getAtomIds(const ASTPtr &ast) const {
  ...
  if (it != ast_to_atom_ids.end()) return it->second;
  return std::nullopt;
}
...
std::vector<CNFQueryAtomicFormula> ConstraintsDescription::getAtomsById(const ConstraintsDescription::AtomIds &ids) const {
  ...
  for (const auto &id : ids) result.push_back(cnf_constraints[id.group_id][id.atom_id]);
  return result;
}

ConstraintsDescription::QueryTreeData ConstraintsDescription::getQueryTreeData(const ContextPtr &context,
                                                                               const QueryTreeNodePtr &table_node) const {
  QueryTreeData data;
  ...
  for (const auto &constraint : filterConstraints(ConstraintsDescription::ConstraintType::ALWAYS_TRUE)) {
    ...
    const auto cnf = Analyzer::CNF::toCNF(query_tree, context).pullNotOutFunctions(context);
    for (const auto &group : cnf.getStatements()) {
      data.cnf_constraints.emplace_back(group.begin(), group.end());

      if (group.size() == 1) atomic_constraints_data.emplace_back(*group.begin());
    }
...
const QueryTreeNodes &ConstraintsDescription::QueryTreeData::getConstraints() const { return constraints; }
...
const ComparisonGraph<QueryTreeNodePtr> &ConstraintsDescription::QueryTreeData::getGraph() const { return *graph; }

std::optional<ConstraintsDescription::AtomIds> ConstraintsDescription::QueryTreeData::getAtomIds(
    const QueryTreeNodePtrWithHash &node_with_hash) const {
  auto it = query_node_to_atom_ids.find(node_with_hash);
  if (it != query_node_to_atom_ids.end()) return it->second;
  return std::nullopt;
}
...
ConstraintsDescription::ConstraintsDescription(const ASTs &constraints_) : constraints(constraints_) { update(); }
...
ConstraintsDescription::ConstraintsDescription(const ConstraintsDescription &other) {
  ...
  for (const auto &constraint : other.constraints) constraints.emplace_back(constraint->clone());
  update();
}
...
ConstraintsDescription &ConstraintsDescription::operator=(const ConstraintsDescription &other) {
  ...
  for (size_t i = 0; i < constraints.size(); ++i) constraints[i] = other.constraints[i]->clone();
  update();
  return *this;
}

ConstraintsDescription::ConstraintsDescription(ConstraintsDescription &&other) noexcept : constraints(std::move(other.constraints)) {
  update();
}
