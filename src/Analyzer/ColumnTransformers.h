...
enum class ColumnTransfomerType : uint8_t { APPLY, EXCEPT, REPLACE };
...
class IColumnTransformerNode : public IQueryTreeNode {
 public:
  ...
  const char *getTransformerTypeName() const { return toString(getTransformerType()); }

  QueryTreeNodeType getNodeType() const final { return QueryTreeNodeType::TRANSFORMER; }
...
enum class ApplyColumnTransformerType : uint8_t { LAMBDA, FUNCTION };
...
class ApplyColumnTransformerNode final : public IColumnTransformerNode {
 public:
  ...
  ApplyColumnTransformerType getApplyTransformerType() const { return apply_transformer_type; }

  /// Get apply transformer expression node
  const QueryTreeNodePtr &getExpressionNode() const { return children[expression_child_index]; }

  ColumnTransfomerType getTransformerType() const override { return ColumnTransfomerType::APPLY; }
...
class ExceptColumnTransformerNode final : public IColumnTransformerNode {
 public:
  ...
  ExceptColumnTransformerType getExceptTransformerType() const { return except_transformer_type; }
   ...
   */
  const Names &getExceptColumnNames() const { return except_column_names; }

  ColumnTransfomerType getTransformerType() const override { return ColumnTransfomerType::EXCEPT; }
...
class ReplaceColumnTransformerNode final : public IColumnTransformerNode {
 public:
  ...
  ColumnTransfomerType getTransformerType() const override { return ColumnTransfomerType::REPLACE; }

  /// Get replacements
  const ListNode &getReplacements() const { return children[replacements_child_index]->as<ListNode &>(); }

  /// Get replacements node
  const QueryTreeNodePtr &getReplacementsNode() const { return children[replacements_child_index]; }
 ...
 private:
  ListNode &getReplacements() { return children[replacements_child_index]->as<ListNode &>(); }
