...
struct QueryTreeNodeWithHash {
  QueryTreeNodeWithHash(QueryTreeNodePtrType node_)  /// NOLINT
      : node(std::move(node_)), hash(node->getTreeHash({.compare_aliases = compare_aliases, .compare_types = compare_types})) {}
...
inline bool operator==(const QueryTreeNodeWithHash<T, compare_aliases, compare_types> &lhs,
                       const QueryTreeNodeWithHash<T, compare_aliases, compare_types> &rhs) {
