...
void TableExpressionModifiers::dump(WriteBuffer& buffer) const {
  ...
  if (sample_size_ratio) buffer << ", sample_size: " << ASTSampleRatio::toString(*sample_size_ratio);

  if (sample_offset_ratio) buffer << ", sample_offset: " << ASTSampleRatio::toString(*sample_offset_ratio);
}
...
String TableExpressionModifiers::formatForErrorMessage() const {
  ...
  if (sample_offset_ratio) {
    if (has_final || sample_size_ratio) buffer << ' ';
