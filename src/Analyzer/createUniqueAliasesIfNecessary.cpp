...
class CreateUniqueTableAliasesVisitor : public InDepthQueryTreeVisitorWithContext<CreateUniqueTableAliasesVisitor> {
 public:
  ...
  explicit CreateUniqueTableAliasesVisitor(const ContextPtr &context) : Base(context) {
  ...
  void leaveImpl(QueryTreeNodePtr &node) {
    if (scope_nodes_stack.back() == node) {
      if (auto it = scope_to_nodes_with_aliases.find(scope_nodes_stack.back()); it != scope_to_nodes_with_aliases.end()) {
...
class CreateUniqueArrayJoinAliasesVisitor : public InDepthQueryTreeVisitorWithContext<CreateUniqueArrayJoinAliasesVisitor> {
 public:
  ...
  void enterImpl(QueryTreeNodePtr &node) {
    ...
    if (!column_node || replaced_nodes_set.contains(node)) return;
  ...
  void populateRenamingMap(ArrayJoinNode *array_join, RenamingMap &result) {
    if (result.empty()) {
      for (auto &array_join_expression : array_join->getJoinExpressions()) {
        ...
        auto replacement_column_node =
            std::make_shared<ColumnNode>(replacement_column, array_join_column->getExpression(), array_join_column->getColumnSource());
