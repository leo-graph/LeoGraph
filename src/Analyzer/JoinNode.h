...
class JoinNode final : public IQueryTreeNode {
 public:
   ...
   */
  JoinNode(QueryTreeNodePtr left_table_expression_, QueryTreeNodePtr right_table_expression_, QueryTreeNodePtr join_expression_,
           JoinLocality locality_, JoinStrictness strictness_, JoinKind kind_, bool is_using_join_expression_);

  /// Get left table expression
  const QueryTreeNodePtr& getLeftTableExpression() const { return children[left_table_expression_child_index]; }

  /// Get left table expression
  QueryTreeNodePtr& getLeftTableExpression() { return children[left_table_expression_child_index]; }

  /// Get right table expression
  const QueryTreeNodePtr& getRightTableExpression() const { return children[right_table_expression_child_index]; }

  /// Get right table expression
  QueryTreeNodePtr& getRightTableExpression() { return children[right_table_expression_child_index]; }

  /// Returns true if join has join expression, false otherwise
  bool hasJoinExpression() const { return children[join_expression_child_index] != nullptr; }

  /// Get join expression
  const QueryTreeNodePtr& getJoinExpression() const { return children[join_expression_child_index]; }

  /// Get join expression
  QueryTreeNodePtr& getJoinExpression() { return children[join_expression_child_index]; }

  /// Returns true if join has USING join expression, false otherwise
  bool isUsingJoinExpression() const { return hasJoinExpression() && is_using_join_expression; }

  /// Returns true if join has ON join expression, false otherwise
  bool isOnJoinExpression() const { return hasJoinExpression() && !is_using_join_expression; }

  /// Get join locality
  JoinLocality getLocality() const { return locality; }

  /// Set join locality
  void setLocality(JoinLocality locality_value) { locality = locality_value; }
  ...
  QueryTreeNodeType getNodeType() const override { return QueryTreeNodeType::JOIN; }
...
class CrossJoinNode final : public IQueryTreeNode {
 public:
  ...
  const QueryTreeNodes& getTableExpressions() const { return children; }

  QueryTreeNodes& getTableExpressions() { return children; }

  /// The size is getTableExpressions.size() - 1
  const JoinTypes& getJoinTypes() { return join_types; }

  QueryTreeNodeType getNodeType() const override { return QueryTreeNodeType::CROSS_JOIN; }
