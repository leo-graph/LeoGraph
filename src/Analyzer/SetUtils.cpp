...
const DataTypeTuple *getTupleType(const DataTypePtr &type) {
  ...
  if (const auto *lc_type = typeid_cast<const DataTypeLowCardinality *>(current)) current = lc_type->getDictionaryType().get();

  if (const auto *nullable_type = typeid_cast<const DataTypeNullable *>(current)) current = nullable_type->getNestedType().get();
...
std::optional<Field> convertFieldToTypeCheckEnum(const Field &from_value, const IDataType &from_type, const IDataType &to_type,
                                                 bool forbid_unknown_enum_values) {
  try {
    return convertFieldToTypeStrict(from_value, from_type, to_type);
  } catch (const Exception &e) {
    if (!forbid_unknown_enum_values && isEnum(to_type) && e.code() == ErrorCodes::UNKNOWN_ELEMENT_OF_ENUM) return {};
    throw;
  }
}
...
ColumnsWithTypeAndName createBlockFromCollection(const Collection &rhs_collection, const DataTypes &rhs_types,
                                                 const DataTypes &lhs_unpacked_types, GetSetElementParams params) {
  ...
  if (num_elements == 1) {
    ...
    if (lhs_tuple) {
      ...
      for (size_t collection_index = 0; collection_index < rhs_collection.size(); ++collection_index) {
        ...
        if (rhs_element.isNull()) {
          if (params.transform_null_in) column->insert(Null{});
          continue;
        }
        ...
        if (tuple_size != lhs_tuple_element_types.size())
          throw Exception(ErrorCodes::INCORRECT_ELEMENT_OF_SET, "Incorrect size of tuple in set: {} instead of {}", tuple_size,
        ...
        if (!rhs_tuple_type)
          throw Exception(ErrorCodes::INCORRECT_ELEMENT_OF_SET, "Invalid element type in set. Expected Tuple, got {}",
                          rhs_element_type->getName());
        ...
        for (size_t i = 0; i < tuple_size; ++i) {
          auto converted_field = convertFieldToTypeCheckEnum(rhs_element_as_tuple[i], *rhs_tuple_element_types[i],
                                                             *lhs_tuple_element_types[i], params.forbid_unknown_enum_values);
        ...
        if (!skip_tuple_value) column->insert(converted_tuple);
      }
    ...
    for (size_t collection_index = 0; collection_index < rhs_collection.size(); ++collection_index) {
      ...
      auto field = convertFieldToTypeCheckEnum(rhs_element, *rhs_types[collection_index], *lhs_type, params.forbid_unknown_enum_values);

      bool need_insert_null = params.transform_null_in && column->isNullable();
      if (field && (!field->isNull() || need_insert_null)) column->insert(*field);
    }
  ...
  for (size_t collection_index = 0; collection_index < rhs_collection.size(); ++collection_index) {
    ...
    if (tuple_size != num_elements)
      throw Exception(ErrorCodes::INCORRECT_ELEMENT_OF_SET, "Incorrect size of tuple in set: {} instead of {}", tuple_size, num_elements);

    if (converted_rhs_element_unpacked.empty()) converted_rhs_element_unpacked.resize(tuple_size);

    size_t i = 0;
    for (; i < tuple_size; ++i) {
      auto converted_field = convertFieldToTypeCheckEnum(rhs_element_as_tuple[i], *rhs_element_unpacked_types[i], *lhs_unpacked_types[i],
                                                         params.forbid_unknown_enum_values);
      if (!converted_field) break;
      ...
      if (converted_rhs_element_unpacked[i].isNull() && !need_insert_null) break;
    }

    if (i == tuple_size) {
      for (i = 0; i < tuple_size; ++i) columns[i]->insert(converted_rhs_element_unpacked[i]);
    }
  }
...
bool hasTupleInCollection(const Field &rhs, const WhichDataType &rhs_which_type) {
  if (rhs_which_type.isArray()) {
    const auto &rhs_array = rhs.safeGet<Array>();
    for (const auto &elem : rhs_array)
      if (elem.getType() == Field::Types::Tuple) return true;
  } else if (rhs_which_type.isTuple()) {
    const auto &rhs_tuple = rhs.safeGet<Tuple>();
    for (const auto &elem : rhs_tuple)
      if (elem.getType() == Field::Types::Tuple) return true;
...
ColumnsWithTypeAndName getSetElementsForConstantValue(const DataTypePtr &lhs_expression_type, const Field &rhs, const DataTypePtr &rhs_type,
  ...
  if (lhs_tuple_type) {
    ...
    if (!lhs_nullable_type && lhs_tuple_type->getElements().size() > 1) lhs_unpacked_types = lhs_tuple_type->getElements();
  }
  ...
  auto append_set_elements = [](ColumnsWithTypeAndName &destination, const ColumnsWithTypeAndName &source) {
    ...
    for (size_t i = 0; i < destination.size(); ++i) {
      ...
      if (source[i].column->empty()) continue;
  ...
  if (lhs_type_depth == rhs_type_depth + 1) {
    if (rhs.isNull()) return build_from_single_value(rhs, rhs_type);
    ...
    if (lhs_is_tuple && rhs_which_type.isTuple() && is_null_in_rhs && (lhs_is_nullable || has_tuple_in_rhs)) {
      ...
      if (!rhs_tuple_has_non_null_non_tuple) {
        ...
        if (lhs_is_nullable && rhs_tuple_all_null && rhs_tuple.size() == lhs_tuple_type->getElements().size()) {
          ...
          /// {NULL, NULL, (NULL, NULL)}
          if (all_tuple_elements_nullable) append_set_elements(res, build_from_single_value(rhs, rhs_type));
        }
    ...
    if (rhs_which_type.isArray()) return build_from_array(rhs, rhs_type);

    if (rhs_which_type.isTuple()) return build_from_tuple(rhs, rhs_type);

    throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT,
                    "Unsupported type at the right-side of IN. Expected Array or Tuple or Nullable(Tuple). Actual {}", rhs_type->getName());
  }

  throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT, "Unsupported types for IN. First argument type {}. Second argument type {}",
                  lhs_expression_type->getName(), rhs_type->getName());
}
