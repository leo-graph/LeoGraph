...
class IQueryTreeNode : public TypePromotion<IQueryTreeNode> {
 public:
  ...
  const char *getNodeTypeName() const { return toString(getNodeType()); }
   ...
   */
  bool isEqual(const IQueryTreeNode &rhs,
               CompareOptions compare_options = {.compare_aliases = true, .compare_types = true, .ignore_cte = false}) const;
  ...
  const String &getOriginalAlias() const { return original_alias.empty() ? alias : original_alias; }

  /// Set node alias
  void setAlias(String alias_value) {
    if (original_alias.empty()) original_alias = std::move(alias);
  ...
  bool hasOriginalAST() const { return original_ast != nullptr; }

  /// Get query tree node original AST
  const ASTPtr &getOriginalAST() const { return original_ast; }

  /** Set query tree node original AST.
   * This AST will not be modified later.
   */
  void setOriginalAST(ASTPtr original_ast_value) { original_ast = std::move(original_ast_value); }
  ...
  String formatASTForErrorMessage() const {
    if (original_ast) return formatOriginalASTForErrorMessage();
  ...
  QueryTreeNodes &getChildren() { return children; }

  /// Get query tree node children
  const QueryTreeNodes &getChildren() const { return children; }
