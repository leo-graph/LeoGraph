...
UnionNode::UnionNode(ContextMutablePtr context_, SelectUnionMode union_mode_)
    : IQueryTreeNode(children_size), context(std::move(context_)), union_mode(union_mode_) {
...
NamesAndTypes UnionNode::computeProjectionColumns() const {
  if (recursive_cte_table) return recursive_cte_table->columns;
...
void UnionNode::removeUnusedProjectionColumns(const std::unordered_set<size_t> &used_projection_columns_indexes) {
  ...
  ///         1
  /// );
  if (union_mode > SelectUnionMode::UNION_DISTINCT) return;
...
void UnionNode::dumpTreeImpl(WriteBuffer &buffer, FormatState &format_state, size_t indent) const {
  ...
  if (is_recursive_cte) buffer << ", is_recursive_cte: " << is_recursive_cte;

  if (recursive_cte_table) buffer << ", recursive_cte_table: " << recursive_cte_table->storage->getStorageID().getNameForLogs();
...
bool UnionNode::isEqualImpl(const IQueryTreeNode &rhs, CompareOptions) const {
  ...
  if (recursive_cte_table && rhs_typed.recursive_cte_table &&
    ...
    return false;
  if ((recursive_cte_table && !rhs_typed.recursive_cte_table) || (!recursive_cte_table && rhs_typed.recursive_cte_table)) return false;
...
ASTPtr UnionNode::toASTImpl(const ConvertToASTOptions &options) const {
  ...
  if (is_subquery) {
    ...
    if (set_subquery_cte_name) subquery->cte_name = cte_name;
