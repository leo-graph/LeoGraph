...
TableNode::TableNode(StoragePtr storage_, const ContextPtr &context)
    : TableNode(storage_, storage_->lockForShare(context->getInitialQueryId(), context->getSettingsRef()[Setting::lock_acquire_timeout]),
...
void TableNode::dumpTreeImpl(WriteBuffer &buffer, FormatState &format_state, size_t indent) const {
  ...
  if (!temporary_table_name.empty()) buffer << ", temporary_table_name: " << temporary_table_name;
...
ASTPtr TableNode::toASTImpl(const ConvertToASTOptions & /* options */) const { return toASTIdentifier(); }

boost::intrusive_ptr<ASTTableIdentifier> TableNode::toASTIdentifier() const {
  if (!temporary_table_name.empty()) return make_intrusive<ASTTableIdentifier>(temporary_table_name);

  // In case of cross-replication we don't know what database is used for the table.
  // `storage_id.hasDatabase()` can return false only on the initiator node.
  // Each shard will use the default database (in the case of cross-replication shards may have different defaults).
  if (!storage_id.hasDatabase()) return make_intrusive<ASTTableIdentifier>(storage_id.getTableName());
