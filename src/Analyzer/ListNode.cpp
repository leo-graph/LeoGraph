...
ListNode::ListNode() : IQueryTreeNode(0 /*children_size*/) {}

ListNode::ListNode(QueryTreeNodes nodes) : IQueryTreeNode(0 /*children_size*/) { children = std::move(nodes); }
...
QueryTreeNodePtr ListNode::cloneImpl() const { return std::make_shared<ListNode>(); }
...
ASTPtr ListNode::toASTImpl(const ConvertToASTOptions &options) const {
  ...
  for (size_t i = 0; i < children_size; ++i) expression_list_ast->children[i] = children[i]->toAST(options);
