...
MatcherNode::MatcherNode(ColumnTransformersNodes column_transformers_)
    : MatcherNode(MatcherNodeType::ASTERISK, {} /*qualified_identifier*/, {} /*columns_identifiers*/, {} /*columns_matcher*/,
...
MatcherNode::MatcherNode(Identifier qualified_identifier_, ColumnTransformersNodes column_transformers_)
    : MatcherNode(MatcherNodeType::ASTERISK, std::move(qualified_identifier_), {} /*columns_identifiers*/, {} /*columns_matcher*/,
                  std::move(column_transformers_)) {}

MatcherNode::MatcherNode(String pattern_, ColumnTransformersNodes column_transformers_)
    : MatcherNode(MatcherNodeType::COLUMNS_REGEXP, {} /*qualified_identifier*/, {} /*columns_identifiers*/, std::move(pattern_),
                  std::move(column_transformers_)) {}

MatcherNode::MatcherNode(Identifier qualified_identifier_, String pattern_, ColumnTransformersNodes column_transformers_)
    : MatcherNode(MatcherNodeType::COLUMNS_REGEXP, std::move(qualified_identifier_), {} /*columns_identifiers*/, std::move(pattern_),
                  std::move(column_transformers_)) {}

MatcherNode::MatcherNode(Identifiers columns_identifiers_, ColumnTransformersNodes column_transformers_)
    : MatcherNode(MatcherNodeType::COLUMNS_LIST, {} /*qualified_identifier*/, std::move(columns_identifiers_), {} /*columns_matcher*/,
                  std::move(column_transformers_)) {}
...
MatcherNode::MatcherNode(Identifier qualified_identifier_, Identifiers columns_identifiers_, ColumnTransformersNodes column_transformers_)
    : MatcherNode(MatcherNodeType::COLUMNS_LIST, std::move(qualified_identifier_), std::move(columns_identifiers_), {} /*columns_matcher*/,
                  std::move(column_transformers_)) {}

MatcherNode::MatcherNode(MatcherNodeType matcher_type_, Identifier qualified_identifier_, Identifiers columns_identifiers_,
                         std::optional<String> pattern_, ColumnTransformersNodes column_transformers_)
  ...
  if (pattern_) {
    ...
    if (!columns_matcher->ok())
      throw DB::Exception(ErrorCodes::CANNOT_COMPILE_REGEXP, "COLUMNS pattern {} cannot be compiled: {}", *pattern_,
                          columns_matcher->error());
  }
  ...
  for (auto &&column_transformer : column_transformers_) column_transformers_nodes.emplace_back(std::move(column_transformer));
  ...
  for (auto &column_identifier : columns_identifiers) columns_identifiers_set.insert(column_identifier.getFullName());
}

bool MatcherNode::isMatchingColumn(const std::string &column_name) {
  if (matcher_type == MatcherNodeType::ASTERISK) return true;

  if (columns_matcher) return RE2::PartialMatch(column_name, *columns_matcher);
...
void MatcherNode::dumpTreeImpl(WriteBuffer &buffer, FormatState &format_state, size_t indent) const {
  ...
  if (!qualified_identifier.empty()) buffer << ", qualified_identifier: " << qualified_identifier.getFullName();
...
bool MatcherNode::isEqualImpl(const IQueryTreeNode &rhs, CompareOptions) const {
  ...
  if (matcher_type != rhs_typed.matcher_type || qualified_identifier != rhs_typed.qualified_identifier ||
      columns_identifiers != rhs_typed.columns_identifiers || columns_identifiers_set != rhs_typed.columns_identifiers_set)
    return false;

  const auto &rhs_columns_matcher = rhs_typed.columns_matcher;

  if (!columns_matcher && !rhs_columns_matcher) return true;
  if (columns_matcher && !rhs_columns_matcher) return false;
  if (!columns_matcher && rhs_columns_matcher) return false;
...
ASTPtr MatcherNode::toASTImpl(const ConvertToASTOptions &options) const {
  ...
  if (!column_transformers.empty()) {
    ...
    for (const auto &column_transformer : column_transformers) transformers->children.push_back(column_transformer->toAST(options));
  }
