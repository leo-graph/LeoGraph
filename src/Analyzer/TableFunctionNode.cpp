...
TableFunctionNode::TableFunctionNode(String table_function_name_)
    : IQueryTreeNode(children_size), table_function_name(table_function_name_), storage_id("system", "one") {
...
const StorageID &TableFunctionNode::getStorageID() const {
  if (!storage) throw Exception(ErrorCodes::LOGICAL_ERROR, "Table function node {} is not resolved", table_function_name);
...
bool TableFunctionNode::isEqualImpl(const IQueryTreeNode &rhs, CompareOptions) const {
  ...
  if (table_function_name != rhs_typed.table_function_name) return false;

  if (storage && rhs_typed.storage) return storage_id == rhs_typed.storage_id;

  if (settings_changes != rhs_typed.settings_changes) return false;
...
void TableFunctionNode::updateTreeHashImpl(HashState &state, CompareOptions) const {
  ...
  if (table_expression_modifiers) table_expression_modifiers->updateTreeHash(state);
