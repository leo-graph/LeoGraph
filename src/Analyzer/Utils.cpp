...
bool isNodePartOfTree(const IQueryTreeNode *node, const IQueryTreeNode *root) {
  ...
  while (!nodes_to_process.empty()) {
    ...
    if (subtree_node == node) return true;

    for (const auto &child : subtree_node->getChildren()) {
      if (child) nodes_to_process.push_back(child.get());
    }
  }
...
bool isStorageUsedInTree(const StoragePtr &storage, const IQueryTreeNode *root) {
  ...
  while (!nodes_to_process.empty()) {
    ...
    if (table_node || table_function_node) {
      ...
      if (table_storage->getStorageID() == storage->getStorageID()) return true;
    }
...
bool isNameOfInFunction(const std::string &function_name) {
  bool is_special_function_in =
      function_name == "in" || function_name == "globalIn" || function_name == "notIn" || function_name == "globalNotIn" ||
      function_name == "nullIn" || function_name == "globalNullIn" || function_name == "notNullIn" || function_name == "globalNotNullIn" ||
      function_name == "inIgnoreSet" || function_name == "globalInIgnoreSet" || function_name == "notInIgnoreSet" ||
      function_name == "globalNotInIgnoreSet" || function_name == "nullInIgnoreSet" || function_name == "globalNullInIgnoreSet" ||
      function_name == "notNullInIgnoreSet" || function_name == "globalNotNullInIgnoreSet";

  return is_special_function_in;
}

bool isNameOfLocalInFunction(const std::string &function_name) {
  bool is_special_function_in = function_name == "in" || function_name == "notIn" || function_name == "nullIn" ||
                                function_name == "notNullIn" || function_name == "inIgnoreSet" || function_name == "notInIgnoreSet" ||
                                function_name == "nullInIgnoreSet" || function_name == "notNullInIgnoreSet";

  return is_special_function_in;
}

bool isNameOfGlobalInFunction(const std::string &function_name) {
  bool is_special_function_in = function_name == "globalIn" || function_name == "globalNotIn" || function_name == "globalNullIn" ||
                                function_name == "globalNotNullIn" || function_name == "globalInIgnoreSet" ||
                                function_name == "globalNotInIgnoreSet" || function_name == "globalNullInIgnoreSet" ||
...
std::string getGlobalInFunctionNameForLocalInFunctionName(const std::string &function_name) {
  if (function_name == "in") return "globalIn";
  if (function_name == "notIn") return "globalNotIn";
  if (function_name == "nullIn") return "globalNullIn";
  if (function_name == "notNullIn") return "globalNotNullIn";
  if (function_name == "inIgnoreSet") return "globalInIgnoreSet";
  if (function_name == "notInIgnoreSet") return "globalNotInIgnoreSet";
  if (function_name == "nullInIgnoreSet") return "globalNullInIgnoreSet";
  if (function_name == "notNullInIgnoreSet") return "globalNotNullInIgnoreSet";
...
bool isFunctionExpressionNodeType(QueryTreeNodeType node_type) { return node_type == QueryTreeNodeType::LAMBDA; }
...
bool isTableExpressionNodeType(QueryTreeNodeType node_type) {
  return node_type == QueryTreeNodeType::TABLE || node_type == QueryTreeNodeType::TABLE_FUNCTION || isSubqueryNodeType(node_type);
}
...
bool isQueryOrUnionNode(const QueryTreeNodePtr &node) { return isQueryOrUnionNode(node.get()); }
...
bool checkCorrelatedColumn(const IdentifierResolveScope *scope_to_check, const QueryTreeNodePtr &column) {
  ...
  if (source_type == QueryTreeNodeType::LAMBDA || source_type == QueryTreeNodeType::INTERPOLATE) return false;

  bool is_correlated = false;

  while (scope_to_check != nullptr) {
    /// Check if column source is in the FROM section of the current scope (query).
    if (scope_to_check->registered_table_expression_nodes.contains(column_source)) break;
    ...
    if (scope_to_check->table_expressions_in_resolve_process.contains(column_source.get())) break;
  ...
  if (!scope_to_check)
    throw Exception(ErrorCodes::LOGICAL_ERROR, "Cannot find the original scope of the column '{}'. Current scope: {}",
                    column_node->getColumnName(), current_scope->scope_node->formatASTForErrorMessage());
...
DataTypePtr getExpressionNodeResultTypeOrNull(const QueryTreeNodePtr &query_tree_node) {
  ...
  switch (node_type) {
    ...
    case QueryTreeNodeType::FUNCTION: {
      ...
      if (function_node.isResolved()) return function_node.getResultType();
      break;
    }
    default: {
      break;
...
QueryTreeNodePtr buildCastFunction(const QueryTreeNodePtr &expression, const DataTypePtr &type, const ContextPtr &context, bool resolve) {
...
std::optional<bool> tryExtractConstantFromConditionNode(const QueryTreeNodePtr &condition_node) {
  ...
  if (!which_constant_type.isUInt8() && !which_constant_type.isNothing()) return {};
...
static ASTPtr convertIntoTableExpressionAST(const QueryTreeNodePtr &table_expression_node,
  ...
  if (node_type == QueryTreeNodeType::IDENTIFIER) {
    ...
    if (identifier.getPartsSize() == 1)
      ...
      throw Exception(ErrorCodes::LOGICAL_ERROR, "Identifier for table expression must contain 1 or 2 parts. Actual '{}'",
  ...
  if (node_type == QueryTreeNodeType::QUERY || node_type == QueryTreeNodeType::UNION) {
    ...
    throw Exception(ErrorCodes::LOGICAL_ERROR, "Expected identifier, table, query, union or table function. Actual {}",
  ...
  if (table_expression_modifiers) {
    ...
    if (sample_size_ratio.has_value()) result_table_expression->sample_size = make_intrusive<ASTSampleRatio>(*sample_size_ratio);

    const auto &sample_offset_ratio = table_expression_modifiers->getSampleOffsetRatio();
    if (sample_offset_ratio.has_value()) result_table_expression->sample_offset = make_intrusive<ASTSampleRatio>(*sample_offset_ratio);
  }
...
QueryTreeNodes extractAllTableReferences(const QueryTreeNodePtr &tree) {
  ...
  while (!nodes_to_process.empty()) {
    ...
    switch (node_type) {
      ...
      case QueryTreeNodeType::UNION: {
        for (const auto &union_node : node_to_process->as<UnionNode>()->getQueries().getNodes()) nodes_to_process.push_back(union_node);
        break;
      }
      case QueryTreeNodeType::TABLE_FUNCTION: {
      ...
      case QueryTreeNodeType::CROSS_JOIN: {
        ...
        for (const auto &expr : cross_join_node.getTableExpressions()) nodes_to_process.push_back(expr);
        break;
      }
      case QueryTreeNodeType::JOIN: {
...
QueryTreeNodes extractTableExpressions(const QueryTreeNodePtr &join_tree_node, bool add_array_join, bool recursive) {
  ...
  while (!nodes_to_process.empty()) {
    ...
    switch (node_type) {
      ...
      case QueryTreeNodeType::QUERY: {
        if (recursive) nodes_to_process.push_back(node_to_process->as<QueryNode>()->getJoinTree());
        result.push_back(std::move(node_to_process));
        break;
      }
      case QueryTreeNodeType::UNION: {
      ...
      case QueryTreeNodeType::ARRAY_JOIN: {
        ...
        if (add_array_join) result.push_back(std::move(node_to_process));
        break;
      }
      case QueryTreeNodeType::CROSS_JOIN: {
        auto &join_node = node_to_process->as<CrossJoinNode &>();
        for (const auto &expr : std::ranges::reverse_view(join_node.getTableExpressions())) nodes_to_process.push_front(expr);
        break;
      }
      case QueryTreeNodeType::JOIN: {
        auto &join_node = node_to_process->as<JoinNode &>();
...
void buildTableExpressionsStackImpl(const QueryTreeNodePtr &join_tree_node, QueryTreeNodes &result) {
  ...
  switch (node_type) {
    ...
    case QueryTreeNodeType::CROSS_JOIN: {
      ...
      for (const auto &expr : cross_join_node.getTableExpressions()) buildTableExpressionsStackImpl(expr, result);
...
class CheckFunctionExistsVisitor : public ConstInDepthQueryTreeVisitor<CheckFunctionExistsVisitor> {
 public:
  explicit CheckFunctionExistsVisitor(std::string_view function_name_) : function_name(function_name_) {}

  void visitImpl(const QueryTreeNodePtr &node) {
    if (has_function) return;
  ...
  bool hasFunction() const { return has_function; }
...
class ReplaceColumnsVisitor : public InDepthQueryTreeVisitor<ReplaceColumnsVisitor> {
 public:
  explicit ReplaceColumnsVisitor(const QueryTreeNodePtr &table_expression_node_,
      ...
      : table_expression_node(table_expression_node_), column_name_to_node(column_name_to_node_) {}
  ...
  void visitImpl(QueryTreeNodePtr &node) {
    ...
    if (column_source != table_expression_node) return;

    auto node_it = column_name_to_node.find(column_node->getColumnName());
    if (node_it == column_name_to_node.end()) return;
...
void replaceColumns(QueryTreeNodePtr &node, const QueryTreeNodePtr &table_expression_node,
                    const std::unordered_map<std::string, QueryTreeNodePtr> &column_name_to_node) {
...
class CollectIdentifiersFullNamesVisitor : public ConstInDepthQueryTreeVisitor<CollectIdentifiersFullNamesVisitor> {
 public:
  explicit CollectIdentifiersFullNamesVisitor(NameSet &used_identifiers_) : used_identifiers(used_identifiers_) {}
...
std::pair<QueryTreeNodePtr, bool> getExpressionSource(const QueryTreeNodePtr &node) {
  if (const auto *column = node->as<ColumnNode>()) {
    ...
    if (!source) return {nullptr, false};
    return {source, true};
  }
  ...
  if (node->as<ConstantNode>()) return {nullptr, true};
...
QueryTreeNodePtr buildQueryToReadColumnsFromTableExpression(const NamesAndTypes &columns, const QueryTreeNodePtr &table_expression,
  ...
  for (const auto &column : projection_columns) subquery_projection_nodes.push_back(std::make_shared<ColumnNode>(column, table_expression));
...
QueryTreeNodePtr buildSubqueryToReadColumnsFromTableExpression(const NamesAndTypes &columns, const QueryTreeNodePtr &table_expression,
                                                               ContextMutablePtr &context) {
  ...
  while (!stack.empty()) {
    ...
    switch (current->getNodeType()) {
      ...
      case QueryTreeNodeType::COLUMN: {
        ...
        if (source != table_expression) return true;
        break;
      }
      default: {
...
void removeExpressionsThatDoNotDependOnTableIdentifiers(QueryTreeNodePtr &expression, const QueryTreeNodePtr &table_expression,
                                                        const ContextPtr &context) {
  auto *function = expression->as<FunctionNode>();
  if (!function) return;

  if (function->getFunctionName() != "and") {
    if (hasUnknownColumn(expression, table_expression)) expression = nullptr;
    return;
  }
  ...
  for (const auto &node : processing) {
    if (!hasUnknownColumn(node, table_expression)) conjunctions.push_back(node);
  }
...
Field getFieldFromColumnForASTLiteralImpl(const ColumnPtr &column, size_t row, const DataTypePtr &data_type, bool is_inside_object) {
  if (isColumnConst(*column))
    return getFieldFromColumnForASTLiteralImpl(assert_cast<const ColumnConst &>(*column).getDataColumnPtr(), 0, data_type,
                                               is_inside_object);
  ...
  switch (data_type->getTypeId()) {
    case TypeIndex::Nullable: {
      ...
      if (nullable_column.isNullAt(row)) return Null();
      return getFieldFromColumnForASTLiteralImpl(nullable_column.getNestedColumnPtr(), row, nullable_data_type.getNestedType(),
                                                 is_inside_object);
    }
    case TypeIndex::Date:
      [[fallthrough]];
    ...
    case TypeIndex::UInt8: {
      ...
      if (isBool(data_type)) return bool(field.safeGet<bool>());
      return field;
    }
    ...
    case TypeIndex::Map: {
      ...
      if (is_inside_object) {
        ...
        for (size_t i = start; i != end; ++i) {
          auto key_field =
              convertFieldToString(getFieldFromColumnForASTLiteralImpl(key_column, i, map_type.getKeyType(), is_inside_object));
    ...
    case TypeIndex::Variant: {
      ...
      if (global_discr == ColumnVariant::NULL_DISCRIMINATOR) return Null();
    ...
    case TypeIndex::Dynamic: {
      ...
      if (global_discr != dynamic_column.getSharedVariantDiscriminator())
        return getFieldFromColumnForASTLiteralImpl(dynamic_column.getVariantColumnPtr(), row, dynamic_column.getVariantInfo().variant_type,
                                                   is_inside_object);
