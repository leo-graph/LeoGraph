...
class LambdaNode final : public IQueryTreeNode {
 public:
  ...
  /// Get argument names
  const Names &getArgumentNames() const { return argument_names; }
  ...
  QueryTreeNodeType getNodeType() const override { return QueryTreeNodeType::LAMBDA; }

  DataTypePtr getResultType() const override { return result_type; }

  void resolve(DataTypePtr lambda_type) { result_type = std::move(lambda_type); }
