 ...
 *
 * During query analysis pass function must be resolved using `resolveAsFunction`, `resolveAsAggregateFunction`, `resolveAsWindowFunction`
 * methods. Resolved function is function that has result type and is initialized with concrete aggregate or non aggregate function.
 */
class FunctionNode;
...
class FunctionNode final : public IQueryTreeNode {
 public:
  ...
  FunctionBasePtr getFunction() const {
    if (kind != FunctionKind::ORDINARY) return {};
  ...
  FunctionBasePtr getFunctionOrThrow() const {
    if (kind != FunctionKind::ORDINARY)
      throw Exception(ErrorCodes::LOGICAL_ERROR, "Function node with name '{}' is not resolved as ordinary function", function_name);
  ...
  void resolveAsFunction(const FunctionOverloadResolverPtr &resolver) { resolveAsFunction(resolver->build(getArgumentColumns())); }
  ...
  DataTypePtr getResultType() const override {
    if (!function) throw Exception(ErrorCodes::UNSUPPORTED_METHOD, "Function node with name '{}' is not resolved", function_name);
    auto type = function->getResultType();
    if (wrap_with_nullable) return makeNullableSafe(type);
    return type;
  }
  ...
  void convertToNullable() override {
    ...
    if (kind == FunctionKind::ORDINARY) wrap_with_nullable = true;
  }
