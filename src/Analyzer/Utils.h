 ...
 */
QueryTreeNodePtr buildCastFunction(const QueryTreeNodePtr& expression, const DataTypePtr& type, const ContextPtr& context,
                                   bool resolve = true);
 ...
 * If table expression node is not of identifier node, table node, query node, table function node, join node or array join node type throws
 ...
 */
void removeExpressionsThatDoNotDependOnTableIdentifiers(QueryTreeNodePtr& expression, const QueryTreeNodePtr& replacement_table_expression,
                                                        const ContextPtr& context);
