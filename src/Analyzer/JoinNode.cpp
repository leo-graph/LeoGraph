...
JoinNode::JoinNode(QueryTreeNodePtr left_table_expression_, QueryTreeNodePtr right_table_expression_, QueryTreeNodePtr join_expression_,
...
static ASTPtr tryMakeUsingColumnASTWithAlias(const QueryTreeNodePtr &node) {
  ...
  if (!column_node) return nullptr;

  const auto &expr = column_node->getExpression();
  if (!expr) return nullptr;

  const auto *expr_list_node = expr->as<ListNode>();
  if (!expr_list_node) return nullptr;

  if (expr_list_node->getNodes().size() != 2) return nullptr;
  ...
  if (!lhs_column_node || !rhs_column_node) return nullptr;

  /// If USING column resolved from projection, keep its name
  if (lhs_column_node->hasExpression()) return nullptr;

  if (lhs_column_node->getColumnName() == rhs_column_node->getColumnName()) return nullptr;
...
static ASTPtr makeUsingAST(const QueryTreeNodePtr &node) {
  ...
  for (const auto &child : list_node.getNodes()) {
    ...
    if (!node_ast) node_ast = child->toAST();
...
void JoinNode::dumpTreeImpl(WriteBuffer &buffer, FormatState &format_state, size_t indent) const {
  ...
  if (locality != JoinLocality::Unspecified) buffer << ", locality: " << toString(locality);

  if (strictness != JoinStrictness::Unspecified) buffer << ", strictness: " << toString(strictness);
...
CrossJoinNode::CrossJoinNode(QueryTreeNodePtr table_expression) : IQueryTreeNode(1) { children = {std::move(table_expression)}; }

CrossJoinNode::CrossJoinNode(QueryTreeNodes table_expressions, JoinTypes join_types_)
    : IQueryTreeNode(table_expressions.size()), join_types(std::move(join_types_)) {
  children = std::move(table_expressions);
  if (children.size() != join_types.size() + 1)
    throw Exception(ErrorCodes::LOGICAL_ERROR, "Invalid number of join tables for CrossJoinNode. Expected {} got {}", join_types.size() + 1,
                    children.size());
}
...
bool CrossJoinNode::isEqualImpl(const IQueryTreeNode &rhs, CompareOptions) const {
  ...
  if (rhs_typed.join_types.size() != join_types.size()) return false;

  for (size_t i = 0; i < join_types.size(); ++i)
    if (!(join_types[i].is_comma == rhs_typed.join_types[i].is_comma && join_types[i].locality == rhs_typed.join_types[i].locality))
      return false;
...
QueryTreeNodePtr CrossJoinNode::cloneImpl() const { return std::make_shared<CrossJoinNode>(children, join_types); }
