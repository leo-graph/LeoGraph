...
class QueryTreeBuilder {
 ...
 private:
  QueryTreeNodePtr buildSelectOrUnionExpression(const ASTPtr &select_or_union_query, bool is_subquery, const std::string &cte_name,
                                                const ASTPtr &aliases, const ContextPtr &context) const;

  QueryTreeNodePtr buildSelectWithUnionExpression(const ASTPtr &select_with_union_query, bool is_subquery, const std::string &cte_name,
                                                  const ASTPtr &aliases, const ContextPtr &context) const;

  QueryTreeNodePtr buildSelectIntersectExceptQuery(const ASTPtr &select_intersect_except_query, bool is_subquery,
                                                   const std::string &cte_name, const ContextPtr &context) const;

  QueryTreeNodePtr buildSelectExpression(const ASTPtr &select_query, bool is_subquery, const std::string &cte_name, const ASTPtr &aliases,
...
QueryTreeNodePtr QueryTreeBuilder::buildQueryTreeNode(ASTPtr query_, ContextPtr context_) {
  ...
  if (query->as<ASTSelectWithUnionQuery>() || query->as<ASTSelectIntersectExceptQuery>() || query->as<ASTSelectQuery>())
...
QueryTreeNodePtr QueryTreeBuilder::buildSelectOrUnionExpression(const ASTPtr &select_or_union_query, bool is_subquery,
                                                                const std::string &cte_name, const ASTPtr &aliases,
                                                                const ContextPtr &context) const {
...
QueryTreeNodePtr QueryTreeBuilder::buildSelectWithUnionExpression(const ASTPtr &select_with_union_query, bool is_subquery,
                                                                  const std::string &cte_name, const ASTPtr &aliases,
                                                                  const ContextPtr &context) const {
...
QueryTreeNodePtr QueryTreeBuilder::buildSelectIntersectExceptQuery(const ASTPtr &select_intersect_except_query, bool is_subquery,
                                                                   const std::string &cte_name, const ContextPtr &context) const {
  ...
  if (select_lists.size() == 1) return buildSelectExpression(select_lists[0], is_subquery, cte_name, nullptr /*aliases*/, context);
  ...
  for (size_t i = 0; i < select_lists_size; ++i) {
    ...
    QueryTreeNodePtr query_node =
        buildSelectOrUnionExpression(select_list_node, false /*is_subquery*/, {} /*cte_name*/, nullptr /*aliases*/, context);
    union_node->getQueries().getNodes().push_back(std::move(query_node));
  }
...
QueryTreeNodePtr QueryTreeBuilder::buildSelectExpression(const ASTPtr &select_query, bool is_subquery, const std::string &cte_name,
                                                         const ASTPtr &aliases, const ContextPtr &context) const {
  ...
  if (order_by_all_enabled) current_query_tree->setIsOrderByAll(true);
  ...
  if (select_expression_list) current_query_tree->getProjectionNode() = buildExpressionList(select_expression_list, current_context);
  ...
  if (prewhere_expression) current_query_tree->getPrewhere() = buildExpression(prewhere_expression, current_context);

  auto where_expression = select_query_typed.where();
  if (where_expression) current_query_tree->getWhere() = buildExpression(where_expression, current_context);
  ...
  if (having_expression) current_query_tree->getHaving() = buildExpression(having_expression, current_context);

  auto window_list = select_query_typed.window();
  if (window_list) current_query_tree->getWindowNode() = buildWindowList(window_list, current_context);

  auto qualify_expression = select_query_typed.qualify();
  if (qualify_expression) current_query_tree->getQualify() = buildExpression(qualify_expression, current_context);
  ...
  if (interpolate_list) current_query_tree->getInterpolate() = buildInterpolateList(interpolate_list, current_context);

  auto select_limit_by_limit = select_query_typed.limitByLength();
  if (select_limit_by_limit) current_query_tree->getLimitByLimit() = buildExpression(select_limit_by_limit, current_context);

  auto select_limit_by_offset = select_query_typed.limitByOffset();
  if (select_limit_by_offset) current_query_tree->getLimitByOffset() = buildExpression(select_limit_by_offset, current_context);

  auto select_limit_by = select_query_typed.limitBy();
  if (select_limit_by) current_query_tree->getLimitByNode() = buildExpressionList(select_limit_by, current_context);
...
QueryTreeNodePtr QueryTreeBuilder::buildSortList(const ASTPtr &order_by_expression_list, const ContextPtr &context) const {
  ...
  for (auto &expression : expression_list_typed.children) {
    ...
    auto sort_node = std::make_shared<SortNode>(std::move(sort_expression), sort_direction, nulls_sort_direction, std::move(collator),
                                                order_by_element.with_fill);

    if (order_by_element.getFillFrom()) sort_node->getFillFrom() = buildExpression(order_by_element.getFillFrom(), context);
    if (order_by_element.getFillTo()) sort_node->getFillTo() = buildExpression(order_by_element.getFillTo(), context);
    if (order_by_element.getFillStep()) sort_node->getFillStep() = buildExpression(order_by_element.getFillStep(), context);
    if (order_by_element.getFillStaleness()) sort_node->getFillStaleness() = buildExpression(order_by_element.getFillStaleness(), context);
  ...
  if (const auto *ast_identifier = expression->as<ASTIdentifier>()) {
    ...
    if (context->getSettingsRef()[Setting::use_variant_as_common_type])
      result = std::make_shared<ConstantNode>(ast_literal->value,
                                              applyVisitor(FieldToDataType<LeastSupertypeOnError::Variant>(), ast_literal->value));
    else
      result = std::make_shared<ConstantNode>(ast_literal->value);
    ...
    if (function->isLambdaFunction() || isASTLambdaFunction(*function)) {
      ...
      if (lambda_arguments_tuple.arguments) {
        ...
        for (const auto &lambda_argument : lambda_arguments_list) {
          ...
          if (!lambda_argument_identifier)
            throw Exception(ErrorCodes::BAD_ARGUMENTS, "Lambda {} argument is not identifier", function->formatForErrorMessage());

          if (lambda_argument_identifier->name_parts.size() > 1)
            throw Exception(ErrorCodes::BAD_ARGUMENTS, "Lambda {} argument identifier must contain single part. Actual {}",
                            function->formatForErrorMessage(), lambda_argument_identifier->full_name);
          ...
          if (!inserted)
            throw Exception(ErrorCodes::BAD_ARGUMENTS, "Lambda {} multiple arguments with same name {}", function->formatForErrorMessage(),
                            argument_name);
      ...
      if (!function->name.empty() &&
          std::any_of(group_concat_aliases.begin(), group_concat_aliases.end(),
    ...
    auto query_node = buildSelectWithUnionExpression(with_element_subquery, true /*is_subquery*/, with_element->name /*cte_name*/,
                                                     with_element->aliases /*aliases*/, context);
    ...
    result = std::make_shared<MatcherNode>(Identifier(qualified_identifier.name_parts), qualified_columns_regexp_matcher->getPattern(),
                                           std::move(column_transformers));
    ...
    result = std::make_shared<MatcherNode>(Identifier(qualified_identifier.name_parts), std::move(column_list_identifiers),
                                           std::move(column_transformers));
  } else if (const auto *query_parameter = expression->as<ASTQueryParameter>()) {
    throw Exception(ErrorCodes::UNKNOWN_QUERY_PARAMETER, "Query parameter {} was not set", backQuote(query_parameter->name));
  } else {
...
QueryTreeNodePtr QueryTreeBuilder::buildWindow(const ASTPtr &window_definition, const ContextPtr &context) const {
  ...
  if (window_definition_typed.order_by) window_node->getOrderByNode() = buildSortList(window_definition_typed.order_by, context);
  ...
  if (table_function_expression.arguments) {
    ...
    for (const auto &argument : function_arguments_list) {
      ...
      if (argument->as<ASTSelectQuery>() || argument->as<ASTSelectWithUnionQuery>() || argument->as<ASTSelectIntersectExceptQuery>())
        node->getArguments().getNodes().push_back(
            buildSelectOrUnionExpression(argument, false /*is_subquery*/, {} /*cte_name*/, nullptr /*aliases*/, context));
...
QueryTreeNodePtr QueryTreeBuilder::buildJoinTree(bool is_subquery, const ASTSelectQuery &select_query, const ContextPtr &context) const {
  ...
  if (!tables_in_select_query) {
    ...
    if (!is_subquery) {
      ...
      if (!implicit_table.empty()) return std::make_shared<IdentifierNode>(Identifier(implicit_table));
    }
  ...
  for (const auto &table_element_untyped : tables.children) {
    ...
    if (table_element.table_expression) {
      ...
      if (table_expression.database_and_table_name) {
        ...
        auto node =
            buildSelectWithUnionExpression(select_with_union_query, true /*is_subquery*/, {} /*cte_name*/, select_query.aliases(), context);
        ...
        if (table_expression.column_aliases) {
          ...
          for (const auto &column_alias : column_aliases_list.children) {
            ...
            if (!seen_aliases.insert(alias_name).second)
              throw Exception(ErrorCodes::BAD_ARGUMENTS, "Duplicate column alias '{}' in table expression column list", alias_name);
          ...
          if (auto *query_node = node->as<QueryNode>()) {
            ...
            /// for UNIONs, apply aliases to the first query in the union, projection column names come from the first query (see
            /// UnionNode::computeProjectionColumns) we find the first QueryNode in case of nested UNIONs
        ...
        if (table_expression_modifiers) {
          throw Exception(ErrorCodes::UNSUPPORTED_METHOD, "Table expression modifiers {} are not supported for subquery {}",
                          table_expression_modifiers->formatForErrorMessage(), node->formatASTForErrorMessage());
        }
        ...
        if (table_expression_modifiers) node->setTableExpressionModifiers(*table_expression_modifiers);
    ...
    if (table_element.table_join) {
      ...
      if (result_join_strictness == JoinStrictness::Unspecified &&
      ...
      if (any_join_distinct_right_table_keys) {
        ...
        if (result_join_strictness == JoinStrictness::Any) result_join_strictness = JoinStrictness::RightAny;
      ...
      if (result_join_kind == JoinKind::Cross || result_join_kind == JoinKind::Comma) {
        ...
        if (auto *right_cross_join = right_table_expression->as<CrossJoinNode>()) {
          ...
          for (size_t i = 0; i < join_types.size(); ++i) cross_join->appendTable(expr[i + 1], join_types[i]);
        } else
          cross_join->appendTable(right_table_expression, join_type);

        join_node = std::move(left_table_expression);
      } else {
        join_node = std::make_shared<JoinNode>(std::move(left_table_expression), std::move(right_table_expression),
                                               std::move(join_expression), table_join.locality, result_join_strictness, result_join_kind,
    ...
    if (table_element.array_join) {
      ...
      auto array_join_node =
          std::make_shared<ArrayJoinNode>(std::move(last_table_expression), std::move(array_join_expressions_list), is_left_array_join);
  ...
  if (table_expressions.empty()) throw Exception(ErrorCodes::LOGICAL_ERROR, "Query FROM section cannot be empty");
...
ColumnTransformersNodes QueryTreeBuilder::buildColumnTransformers(const ASTPtr &matcher_expression, const ContextPtr &context) const {
  ...
  if (!matcher_expression) return column_transformers;
  ...
  for (const auto &child : matcher_expression->children) {
    if (auto *apply_transformer = child->as<ASTColumnsApplyTransformer>()) {
      if (apply_transformer->lambda) {
        ...
        if (apply_transformer->parameters) function_node->getParametersNode() = buildExpressionList(apply_transformer->parameters, context);
      ...
      if (matcher) {
        ...
        column_transformers.emplace_back(
            std::make_shared<ExceptColumnTransformerNode>(std::move(except_column_names), except_transformer->is_strict));
      }
      ...
      for (const auto &replace_transformer_child : replace_transformer->children) {
        ...
        replacements.emplace_back(
            ReplaceColumnTransformerNode::Replacement{replacement.name, buildExpression(replacement.children[0], context)});
      }
