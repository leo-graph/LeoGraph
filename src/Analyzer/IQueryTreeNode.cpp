...
IQueryTreeNode::IQueryTreeNode(size_t children_size) { children.resize(children_size); }
...
bool IQueryTreeNode::isEqual(const IQueryTreeNode &rhs, CompareOptions compare_options) const {
  ...
  while (!nodes_to_process.empty()) {
    ...
    if (equals_pairs.contains(std::make_pair(lhs_node_to_compare, rhs_node_to_compare))) continue;
    ...
    if (compare_options.compare_aliases && lhs_node_to_compare->alias != rhs_node_to_compare->alias) return false;
    ...
    if (lhs_children_size != rhs_children.size()) return false;
    ...
    if (lhs_weak_pointers_size != rhs_weak_pointers.size()) return false;
    ...
    for (size_t i = 0; i < lhs_weak_pointers_size; ++i) {
      ...
      if (!lhs_strong_pointer && !rhs_strong_pointer) continue;
      if (lhs_strong_pointer && !rhs_strong_pointer) return false;
      if (!lhs_strong_pointer && rhs_strong_pointer) return false;
...
QueryTreeNodePtr IQueryTreeNode::clone() const { return cloneAndReplace({}); }
...
QueryTreeNodePtr IQueryTreeNode::cloneAndReplace(const ReplacementMap &replacement_map) const {
  ...
  while (!nodes_to_clone.empty()) {
    ...
    *place_for_cloned_node = node_clone;
    ...
    if (it != replacement_map.end()) continue;
   ...
   */
  for (const auto &[old_ptr, new_ptr] : replacement_map) old_pointer_to_new_pointer.emplace(old_ptr, new_ptr);
  ...
  for (auto &weak_pointer_ptr : weak_pointers_to_update_after_clone) {
     ...
     */
    if (it == old_pointer_to_new_pointer.end()) continue;
...
ASTPtr IQueryTreeNode::toAST(const ConvertToASTOptions &options) const {
  ...
  if (auto * /*ast_with_alias*/ _ = dynamic_cast<ASTWithAlias *>(converted_node.get())) converted_node->setAlias(alias);

  return converted_node;
}

String IQueryTreeNode::formatOriginalASTForErrorMessage() const {
  if (!original_ast) throw Exception(ErrorCodes::UNSUPPORTED_METHOD, "Original AST was not set");

  return original_ast->formatForErrorMessage();
}

String IQueryTreeNode::formatConvertedASTForErrorMessage() const { return toAST()->formatForErrorMessage(); }
