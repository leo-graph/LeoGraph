...
FunctionNode::FunctionNode(String function_name_) : IQueryTreeNode(children_size), function_name(function_name_) {
  children[parameters_child_index] = std::make_shared<ListNode>();
  children[arguments_child_index] = std::make_shared<ListNode>();
}

const DataTypes &FunctionNode::getArgumentTypes() const {
  if (!function) throw Exception(ErrorCodes::LOGICAL_ERROR, "Function {} is not resolved", function_name);
...
ColumnsWithTypeAndName FunctionNode::getArgumentColumns() const {
  ...
  for (size_t i = 0; i < arguments_size; ++i) {
    ...
    if (constant && !isNotCreatable(argument_column.type)) argument_column.column = constant->getColumn();
...
AggregateFunctionPtr FunctionNode::getAggregateFunction() const {
  if (kind == FunctionKind::UNKNOWN || kind == FunctionKind::ORDINARY) return {};
...
void FunctionNode::resolveAsWindowFunction(AggregateFunctionPtr window_function_value) {
  if (!hasWindow())
    throw Exception(ErrorCodes::LOGICAL_ERROR, "Trying to resolve FunctionNode without window definition as a window function {}",
...
void FunctionNode::dumpTreeImpl(WriteBuffer &buffer, FormatState &format_state, size_t indent) const {
  ...
  if (function) buffer << ", result_type: " + getResultType()->getName();
...
bool FunctionNode::isEqualImpl(const IQueryTreeNode &rhs, CompareOptions compare_options) const {
  ...
  if (!compare_options.compare_types) return true;

  if (isResolved() != rhs_typed.isResolved()) return false;
  if (!isResolved()) return true;

  auto lhs_result_type = getResultType();
  auto rhs_result_type = rhs.getResultType();

  if (lhs_result_type && rhs_result_type && !lhs_result_type->equals(*rhs_result_type)) return false;
  if (lhs_result_type && !rhs_result_type) return false;
  if (!lhs_result_type && rhs_result_type) return false;
...
void FunctionNode::updateTreeHashImpl(HashState &hash_state, CompareOptions compare_options) const {
  ...
  if (!compare_options.compare_types) return;

  if (!isResolved()) return;

  if (auto result_type = getResultType()) result_type->updateHash(hash_state);
}
