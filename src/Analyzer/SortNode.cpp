...
SortNode::SortNode(QueryTreeNodePtr expression_, SortDirection sort_direction_, std::optional<SortDirection> nulls_sort_direction_,
                   std::shared_ptr<Collator> collator_, bool with_fill_)
  ...
  if (expression_)
    if (auto *identifier = expression_->as<IdentifierNode>()) column_name = identifier->getIdentifier().getFullName();
...
void SortNode::dumpTreeImpl(WriteBuffer &buffer, FormatState &format_state, size_t indent) const {
  ...
  if (nulls_sort_direction) buffer << ", nulls_sort_direction: " << toString(*nulls_sort_direction);

  if (collator) buffer << ", collator: " << collator->getLocale();

  buffer << ", with_fill: " << with_fill;

  buffer << '\n' << std::string(indent + 2, ' ') << "EXPRESSION";
  if (!column_name.empty()) buffer << " " << column_name;
  buffer << "\n";
...
bool SortNode::isEqualImpl(const IQueryTreeNode &rhs, CompareOptions) const {
  ...
  if (sort_direction != rhs_typed.sort_direction || nulls_sort_direction != rhs_typed.nulls_sort_direction ||
      with_fill != rhs_typed.with_fill)
    return false;

  if (!collator && !rhs_typed.collator) return true;
  if (collator && !rhs_typed.collator) return false;
  if (!collator && rhs_typed.collator) return false;
...
ASTPtr SortNode::toASTImpl(const ConvertToASTOptions &options) const {
  ...
  if (nulls_sort_direction) result->nulls_direction = *nulls_sort_direction == SortDirection::ASCENDING ? 1 : -1;
  ...
  if (collator) result->setCollation(make_intrusive<ASTLiteral>(Field(collator->getLocale())));

  result->with_fill = with_fill;
  if (hasFillFrom()) result->setFillFrom(getFillFrom()->toAST(options));
  if (hasFillTo()) result->setFillTo(getFillTo()->toAST(options));
  if (hasFillStep()) result->setFillStep(getFillStep()->toAST(options));
  if (hasFillStaleness()) result->setFillStaleness(getFillStaleness()->toAST(options));
