...
class OptimizeGroupByInjectiveFunctionsVisitor : public InDepthQueryTreeVisitorWithContext<OptimizeGroupByInjectiveFunctionsVisitor> {
 ...
 public:
  explicit OptimizeGroupByInjectiveFunctionsVisitor(ContextPtr context) : Base(std::move(context)) {}

  void enterImpl(QueryTreeNodePtr& node) {
    if (!getSettings()[Setting::optimize_injective_functions_in_group_by]) return;
 ...
 private:
      ...
      while (!nodes_to_process.empty()) {
        ...
        if (can_be_eliminated) {
          for (auto const& argument : function_node->getArguments()) {
            ...
            if (argument->getNodeType() != QueryTreeNodeType::CONSTANT) nodes_to_process.push(argument);
          }
        } else
          new_group_by_keys.push_back(node_to_process);
      }
  ...
  bool isValidGroupByKeyTypes(const ColumnsWithTypeAndName& columns) const {
    if (getContext()->getSettingsRef()[Setting::allow_suspicious_types_in_group_by]) return true;
