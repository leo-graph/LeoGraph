...
class SumIfToCountIfVisitor : public InDepthQueryTreeVisitorWithContext<SumIfToCountIfVisitor> {
 public:
  ...
  void enterImpl(QueryTreeNodePtr& node) {
    if (!getSettings()[Setting::optimize_rewrite_sum_if_to_count_if]) return;

    auto* function_node = node->as<FunctionNode>();
    if (!function_node || !function_node->isAggregateFunction() || !function_node->getResultType()->equals(DataTypeUInt64())) return;
    ...
    if (lower_function_name != "sum" && (lower_function_name != "sumif" || !function_name.ends_with("If"))) return;
    ...
    if (lower_function_name == "sumif") {
      if (function_node_arguments_nodes.size() != 2) return;

      const auto* constant_node = function_node_arguments_nodes[0]->as<ConstantNode>();
      if (!constant_node) return;

      if (auto constant_type = constant_node->getResultType(); !isNativeInteger(constant_type)) return;

      const auto& constant_value_literal = constant_node->getValue();
      if (getSettings()[Setting::aggregate_functions_null_for_empty]) return;
    ...
    if (!if_true_condition_constant_node || !if_false_condition_constant_node) return;

    if (auto constant_type = if_true_condition_constant_node->getResultType(); !isNativeInteger(constant_type)) return;

    if (auto constant_type = if_false_condition_constant_node->getResultType(); !isNativeInteger(constant_type)) return;
    ...
    if (if_true_condition_value == 0 && !cond_argument->getResultType()->isNullable()) {
      ...
      if (condition_result_type->isNullable()) not_function_result_type = makeNullable(not_function_result_type);
      ...
      if (if_false_condition_value != 1) {
        /// Rewrite `sum(if(cond, 0, 123))` into `123 * countIf(not(cond))` if condition is not Nullable (otherwise the result can be
        /// different).
 ...
 private:
  QueryTreeNodePtr getMultiplyFunction(QueryTreeNodePtr left, QueryTreeNodePtr right) {
    ...
    auto multiply_function_base =
        FunctionFactory::instance().get("multiply", getContext())->build(multiply_function_node->getArgumentColumns());
