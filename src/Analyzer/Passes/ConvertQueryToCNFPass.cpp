...
std::optional<Analyzer::CNF> tryConvertQueryToCNF(const QueryTreeNodePtr &node, const ContextPtr &context) {
  ...
  if (!cnf_form) return std::nullopt;
...
MatchState match(const Analyzer::CNFAtomicFormula &a, const Analyzer::CNFAtomicFormula &b) {
  ...
  if (a.node_with_hash != b.node_with_hash) return NONE;

  return a.negative == b.negative ? FULL_MATCH : PARTIAL_MATCH;
}

bool checkIfGroupAlwaysTrueFullMatch(const Analyzer::CNF::OrGroup &group,
                                     const ConstraintsDescription::QueryTreeData &query_tree_constraints) {
  ...
  for (size_t i = 0; i < constraints_data.size(); ++i) found[i] = constraints_data[i].size();
    ...
    if (constraint_atom_ids) {
      ...
      for (size_t i = 0; i < constraint_atoms.size(); ++i) {
        if (match(constraint_atoms[i], atom) == MatchState::FULL_MATCH) {
          if ((--found[(*constraint_atom_ids)[i].group_id]) == 0) return true;
        }
      }
    }
  }
  ...
  for (const auto &atom : group) {
    ...
    if (function_node) {
      ...
      if (arguments.size() == 2) {
        ...
        if (graph.isAlwaysCompare(expected, arguments[0], arguments[1])) return true;
      }
    }
  }
...
bool checkIfAtomAlwaysFalseFullMatch(const Analyzer::CNFAtomicFormula &atom,
                                     const ConstraintsDescription::QueryTreeData &query_tree_constraints) {
  const auto constraint_atom_ids = query_tree_constraints.getAtomIds(atom.node_with_hash);
  if (constraint_atom_ids) {
    for (const auto &constraint_atom : query_tree_constraints.getAtomsById(*constraint_atom_ids)) {
      ...
      if (match_result == MatchState::PARTIAL_MATCH) return true;
    }
  }
...
void replaceToConstants(QueryTreeNodePtr &term, const ComparisonGraph<QueryTreeNodePtr> &graph) {
  ...
  for (auto &child : term->getChildren()) {
    if (child) replaceToConstants(child, graph);
  }
}
...
StorageSnapshotPtr getStorageSnapshot(const QueryTreeNodePtr &node) {
  ...
  if (auto *table_node = node->as<TableNode>()) return table_node->getStorageSnapshot();
  if (auto *table_function_node = node->as<TableFunctionNode>()) return table_function_node->getStorageSnapshot();
...
bool onlyIndexColumns(const QueryTreeNodePtr &node, const std::unordered_set<std::string_view> &primary_key_set) {
  ...
  if (column_node && !primary_key_set.contains(column_node->getColumnName())) return false;

  for (const auto &child : node->getChildren()) {
    if (child && !onlyIndexColumns(child, primary_key_set)) return false;
  }
...
const std::unordered_map<std::string_view, ComparisonGraphCompareResult> &getRelationMap() {
  ...
  static const std::unordered_map<std::string_view, ComparisonGraphCompareResult> relations = {
      {"equals", EQUAL}, {"less", LESS}, {"lessOrEquals", LESS_OR_EQUAL}, {"greaterOrEquals", GREATER_OR_EQUAL}, {"greater", GREATER},
  };
  return relations;
}
...
const std::unordered_map<ComparisonGraphCompareResult, std::string> &getReverseRelationMap() {
  ...
  static const std::unordered_map<ComparisonGraphCompareResult, std::string> relations = {
      {EQUAL, "equals"}, {LESS, "less"}, {LESS_OR_EQUAL, "lessOrEquals"}, {GREATER_OR_EQUAL, "greaterOrEquals"}, {GREATER, "greater"},
  };
  return relations;
}
...
bool canBeSequence(const ComparisonGraphCompareResult left, const ComparisonGraphCompareResult right) {
  using enum ComparisonGraphCompareResult;
  if (left == UNKNOWN || right == UNKNOWN || left == NOT_EQUAL || right == NOT_EQUAL) return false;
  if ((left == GREATER || left == GREATER_OR_EQUAL) && (right == LESS || right == LESS_OR_EQUAL)) return false;
  if ((right == GREATER || right == GREATER_OR_EQUAL) && (left == LESS || left == LESS_OR_EQUAL)) return false;
  return true;
}

ComparisonGraphCompareResult mostStrict(const ComparisonGraphCompareResult left, const ComparisonGraphCompareResult right) {
  using enum ComparisonGraphCompareResult;
  if (left == LESS || left == GREATER) return left;
  if (right == LESS || right == GREATER) return right;
  if (left == LESS_OR_EQUAL || left == GREATER_OR_EQUAL) return left;
  if (right == LESS_OR_EQUAL || right == GREATER_OR_EQUAL) return right;
  if (left == EQUAL) return left;
  if (right == EQUAL) return right;
  return UNKNOWN;
}
...
Analyzer::CNF::OrGroup createIndexHintGroup(const Analyzer::CNF::OrGroup &group, const ComparisonGraph<QueryTreeNodePtr> &graph,
                                            const QueryTreeNodes &primary_key_only_nodes, const ContextPtr &context) {
  Analyzer::CNF::OrGroup result;
  for (const auto &atom : group) {
    const auto *function_node = atom.node_with_hash.node->as<FunctionNode>();
    if (!function_node || !getRelationMap().contains(function_node->getFunctionName())) continue;

    const auto &arguments = function_node->getArguments().getNodes();
    if (arguments.size() != 2) continue;

    auto check_and_insert = [&](const size_t index, const ComparisonGraphCompareResult expected_result) {
      if (!onlyConstants(arguments[1 - index])) return false;
    ...
    if (!check_and_insert(0, expected) && !check_and_insert(1, expected)) return {};
  }
...
void addIndexConstraint(Analyzer::CNF &cnf, const QueryTreeNodes &table_expressions, const ContextPtr &context) {
  for (const auto &table_expression : table_expressions) {
    ...
    if (!snapshot || !snapshot->metadata) continue;
    ...
    for (const auto &vertex : graph.getVertices()) {
      for (const auto &node : vertex) {
        if (onlyIndexColumns(node, primary_key_set)) primary_key_only_nodes.push_back(node);
      }
    }
    ...
    for (const auto &group : statements) {
      ...
      if (!new_group.empty()) and_group.emplace(std::move(new_group));
    }
...
struct ColumnPrice {
  ...
  ColumnPrice(const Int64 compressed_size_, const Int64 uncompressed_size_)
      : compressed_size(compressed_size_), uncompressed_size(uncompressed_size_) {}
...
class ComponentCollectorVisitor : public ConstInDepthQueryTreeVisitor<ComponentCollectorVisitor> {
 public:
  ComponentCollectorVisitor(std::set<UInt64> &components_, QueryTreeNodePtrWithHashMap<UInt64> &query_node_to_component_,
...
class ColumnNameCollectorVisitor : public ConstInDepthQueryTreeVisitor<ColumnNameCollectorVisitor> {
 public:
  ColumnNameCollectorVisitor(std::unordered_set<std::string> &column_names_,
  ...
  void visitImpl(const QueryTreeNodePtr &node) {
    if (query_node_to_component && query_node_to_component->contains(node)) return;

    if (const auto *column_node = node->as<ColumnNode>()) column_names.insert(column_node->getColumnName());
  }
...
class SubstituteColumnVisitor : public InDepthQueryTreeVisitor<SubstituteColumnVisitor> {
 public:
  SubstituteColumnVisitor(const QueryTreeNodePtrWithHashMap<UInt64> &query_node_to_component_,
                          const std::unordered_map<UInt64, QueryTreeNodePtr> &id_to_query_node_map_, ContextPtr context_)
  ...
  void visitImpl(QueryTreeNodePtr &node) {
    ...
    if (component_id_it == query_node_to_component.end()) return;
...
ColumnPrice calculatePrice(const ColumnPriceByName &column_prices, const std::unordered_set<std::string> &column_names) {
  ColumnPrice result(0, 0);

  for (const auto &column : column_names) {
    if (auto it = column_prices.find(column); it != column_prices.end()) result += it->second;
  }

  return result;
}

void bruteForce(const ComparisonGraph<QueryTreeNodePtr> &graph, const std::vector<UInt64> &components, size_t current_component,
                const ColumnPriceByName &column_prices, ColumnPrice current_price, std::vector<QueryTreeNodePtr> &expressions_stack,
                ColumnPrice &min_price, std::vector<QueryTreeNodePtr> &min_expressions) {
  ...
  for (const auto &node : graph.getComponent(components[current_component])) {
    ...
    for (const auto &column : column_names) new_prices.insert_or_assign(column, ColumnPrice(0, 0));

    bruteForce(graph, components, current_component + 1, new_prices, current_price, expressions_stack, min_price, min_expressions);
...
void substituteColumns(QueryNode &query_node, const QueryTreeNodes &table_expressions, const ContextPtr &context) {
  ...
  for (const auto &table_expression : table_expressions) {
    ...
    auto run_for_all = [&](const auto function) {
      ...
      if (query_node.hasWhere()) function(query_node.getWhere());

      if (query_node.hasPrewhere()) function(query_node.getPrewhere());

      if (query_node.hasHaving()) function(query_node.getHaving());
    };
    ...
    for (const auto &column_name : primary_key) column_prices.insert_or_assign(column_name, ColumnPrice(INDEX_PRICE, INDEX_PRICE));

    for (const auto &column_name : column_names) column_prices.insert_or_assign(column_name, ColumnPrice(0, 0));
    ...
    bruteForce(graph, components_list, 0, column_prices, ColumnPrice(0, 0), expressions_stack, min_price, min_expressions);

    for (size_t i = 0; i < components_list.size(); ++i) id_to_query_node_map[components_list[i]] = min_expressions[i];

    SubstituteColumnVisitor substitute_column{query_node_to_component, id_to_query_node_map, context};

    run_for_all([&](QueryTreeNodePtr &node) { substitute_column.visit(node); });
  }
}
...
void optimizeWithConstraints(Analyzer::CNF &cnf, const QueryTreeNodes &table_expressions, const ContextPtr &context) {
  ...
  for (const auto &table_expression : table_expressions) {
    ...
    cnf.filterAlwaysTrueGroups([&](const auto &group) {
         ...
         return !checkIfGroupAlwaysTrueFullMatch(group, query_tree_constraints) && !checkIfGroupAlwaysTrueGraph(group, compare_graph) &&
                !checkIfGroupAlwaysTrueAtoms(group);
       })
        ...
        .transformAtoms([&](const auto &atom) { return replaceTermsToConstants(atom, compare_graph); })
        .reduce();
  }
  ...
  if (settings[Setting::optimize_append_index]) addIndexConstraint(cnf, table_expressions, context);
}
...
void optimizeNode(QueryTreeNodePtr &node, const QueryTreeNodes &table_expressions, const ContextPtr &context) {
  ...
  if (settings[Setting::optimize_using_constraints]) optimizeWithConstraints(*cnf, table_expressions, context);
...
class ConvertQueryToCNFVisitor : public InDepthQueryTreeVisitorWithContext<ConvertQueryToCNFVisitor> {
 public:
  ...
  void enterImpl(QueryTreeNodePtr &node) {
    ...
    if (has_filter && settings[Setting::optimize_substitute_columns]) substituteColumns(*query_node, table_expressions, context);
  }
};
...
void ConvertLogicalExpressionToCNFPass::run(QueryTreeNodePtr &query_tree_node, ContextPtr context) {
  const auto &settings = context->getSettingsRef();
  if (!settings[Setting::convert_query_to_cnf]) return;
