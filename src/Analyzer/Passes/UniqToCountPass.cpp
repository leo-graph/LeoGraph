...
NamesAndTypes extractProjectionColumnsForGroupBy(const QueryNode* query_node) {
  if (!query_node->hasGroupBy()) return {};
  ...
  if (result.size() != group_by_elements.size()) return {};
...
bool nodeListEquals(const QueryTreeNodes& query_columns, const NamesAndTypes& subquery_columns) {
  if (query_columns.size() != subquery_columns.size()) return false;

  for (const auto& query_column : query_columns) {
    auto find = std::find_if(subquery_columns.begin(), subquery_columns.end(), [&](const auto& subquery_column) -> bool {
    ...
    if (find == subquery_columns.end()) return false;
  }
  return true;
}
...
class UniqToCountVisitor : public InDepthQueryTreeVisitorWithContext<UniqToCountVisitor> {
 public:
  ...
  void enterImpl(QueryTreeNodePtr& node) {
    if (!getSettings()[Setting::optimize_uniq_to_count]) return;
    ...
    if (!matchFnUniq(function_node->getFunctionName())) return;
    ...
    auto match_subquery_with_distinct = [&]() -> bool {
      if (!subquery_node->isDistinct()) return false;

      /// uniq expression list == subquery projection columns
      if (!nodeListEquals(uniq_arguments_nodes, subquery_node->getProjectionColumns())) return false;
    ...
    auto match_subquery_with_group_by = [&]() -> bool {
      if (!subquery_node->hasGroupBy()) return false;
      ...
      if (!nodeListEquals(uniq_arguments_nodes, group_by_columns)) return false;

      /// subquery projection columns must contain all columns in uniq argument node list
      if (!nodeListContainsAll(uniq_arguments_nodes, subquery_node->getProjectionColumns())) return false;
