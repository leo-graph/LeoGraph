 ...
 * Out of scope:
 *     - `ILIKE` is not rewritten until starts/endsWithCaseInsensitive is implemented, because lower() or left() based rewrites are
 * suboptimal Type requirement on the left operand, `col`:
 ...
 *     - `endsWith`: same as `startsWith` except `FixedString` (nested or not), because `c LIKE '%suffix\0'` is not equal to `endsWith(c,
 * 'suffix\0')`.
 */
class LikePerfectAffixRewriteVisitor : public InDepthQueryTreeVisitorWithContext<LikePerfectAffixRewriteVisitor> {
 public:
  ...
  void enterImpl(QueryTreeNodePtr &node) {
    if (!getSettings()[Setting::optimize_rewrite_like_perfect_affix]) return;
    ...
    if (!isResultTypeSupported(args[0]->getResultType(), is_suffix)) return;

    /// Only rewrite for perfect prefix or suffix
    /// Suffix is prefix in reverse
    if (is_suffix) std::reverse(pattern.begin(), pattern.end());

    auto [affix, is_perfect] = extractFixedPrefixFromLikePattern(pattern, true);
    if (!is_perfect || affix.empty()) return;

    if (is_suffix) std::reverse(affix.begin(), affix.end());
    ...
    if (is_not_like) new_node = operation("not", new_node);
