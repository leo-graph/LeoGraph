...
class OptimizeGroupByFunctionKeysVisitor : public InDepthQueryTreeVisitorWithContext<OptimizeGroupByFunctionKeysVisitor> {
 public:
  ...
  void enterImpl(QueryTreeNodePtr& node) {
    if (!getSettings()[Setting::optimize_group_by_function_keys]) return;
    ...
    if (getSettings()[Setting::group_by_use_nulls]) return;
    ...
    if (query->isGroupByWithCube() || query->isGroupByWithRollup()) return;
 ...
 private:
  ...
  static bool canBeEliminated(QueryTreeNodePtr& node, const QueryTreeNodePtrWithHashSet& group_by_keys) {
    ...
    if (!function || function->getArguments().getNodes().empty()) return false;
    ...
    for (auto it = function_arguments.rbegin(); it != function_arguments.rend(); ++it) candidates.push_back({*it, is_deterministic});
  ...
  static void optimizeGroupingSet(QueryTreeNodes& grouping_set) {
    ...
    for (auto& group_by_elem : grouping_set) {
      if (!canBeEliminated(group_by_elem, group_by_keys)) new_group_by_keys.push_back(group_by_elem);
    }
