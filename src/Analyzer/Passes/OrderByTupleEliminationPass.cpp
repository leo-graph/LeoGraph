...
class OrderByTupleEliminationVisitor : public InDepthQueryTreeVisitor<OrderByTupleEliminationVisitor> {
 public:
  static void visitImpl(QueryTreeNodePtr &node) {
    ...
    if (!query_node || !query_node->hasOrderBy()) return;
    ...
    for (auto &sort_node : query_node->getOrderBy().getNodes()) {
      ...
      for (auto &argument_node : tuple_arguments_nodes) {
        auto result_sort_node = std::make_shared<SortNode>(argument_node, sort_node_typed.getSortDirection(),
                                                           sort_node_typed.getNullsSortDirection(), sort_node_typed.getCollator());
