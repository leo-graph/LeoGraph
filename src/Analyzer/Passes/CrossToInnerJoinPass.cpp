...
void extractJoinConditions(const QueryTreeNodePtr &node, QueryTreeNodes &equi_conditions, QueryTreeNodes &other) {
  ...
  if (args.size() == 2 && func->getFunctionName() == "equals") {
    ...
    for (const auto &arg : args) extractJoinConditions(arg, equi_conditions, other);
  } else {
    other.push_back(node);
  }
}
...
bool findInTableExpression(const QueryTreeNodePtr &source, const QueryTreeNodePtr &table_expression) {
  ...
  if (source->isEqual(*table_expression)) return true;
...
void getJoinNodes(QueryTreeNodePtr &join_tree_node, std::vector<JoinNode *> &join_nodes) {
  ...
  if (!isCrossOrComma(join_node->getKind())) return;
...
class CrossToInnerJoinVisitor : public InDepthQueryTreeVisitorWithContext<CrossToInnerJoinVisitor> {
 public:
  ...
  JoinGraph buildJoinGraph(const QueryTreeNodes &table_nodes, const CrossJoinNode::JoinTypes &join_types, QueryTreeNodes &equi_conditions,
                           QueryTreeNodes &other_conditions) {
    JoinGraph graph;

    graph.nodes.reserve(table_nodes.size());
    for (const auto &table_node : table_nodes) graph.nodes.emplace_back(JoinGraph::Node{.table_node = table_node});

    for (size_t i = 0; i < join_types.size(); ++i) graph.nodes[i + 1].join_type = join_types[i];
    ...
    for (auto &condition : equi_conditions) {
      ...
      if (common_key_type) {
        ...
        if (left_src && right_src) {
          ...
          for (size_t i = 0; i < table_nodes.size(); ++i) {
            ...
            if (findInTableExpression(left_src, table_node)) lhs_index = i;
            if (findInTableExpression(right_src, table_node)) rhs_index = i;
          }
      ...
      if (!is_useful) other_conditions.push_back(std::move(condition));
    }
  ...
  QueryTreeNodePtr rebuildJoins(std::vector<TableWithConditions> tables) {
    ...
    for (auto &table_with_condition : tables) {
      ...
      auto join_node = std::make_shared<JoinNode>(std::move(lhs), std::move(table_with_condition.table_node), nullptr,
                                                  table_with_condition.join_type.locality, JoinStrictness::Unspecified,
                                                  table_with_condition.join_type.is_comma ? JoinKind::Comma : JoinKind::Cross, false);
  ...
  void enterImpl(QueryTreeNodePtr &node) {
    ...
    if (!join_tree_node || join_tree_node->getNodeType() != QueryTreeNodeType::CROSS_JOIN) return;
    ...
    auto join_graph =
        buildJoinGraph(cross_join_node.getTableExpressions(), cross_join_node.getJoinTypes(), equi_conditions, other_conditions);
 ...
 private:
  ...
  void checkNotRewritten(const CrossJoinNode::JoinTypes &join_types, const QueryTreeNodePtr &join_node) {
    for (const auto &item : join_types) checkNotRewritten(item.is_comma, join_node);
  }
  ...
  QueryTreeNodePtr makeConjunction(const QueryTreeNodes &nodes) {
    ...
    for (const auto &node : nodes) function_node->getArguments().getNodes().push_back(node);
