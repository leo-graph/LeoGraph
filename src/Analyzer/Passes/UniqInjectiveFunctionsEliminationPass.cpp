...
bool isUniqFunction(const String& function_name) {
  return function_name == "uniq" || function_name == "uniqExact" || function_name == "uniqHLL12" || function_name == "uniqCombined" ||
         function_name == "uniqCombined64" || function_name == "uniqTheta";
}
...
class UniqInjectiveFunctionsEliminationVisitor : public InDepthQueryTreeVisitorWithContext<UniqInjectiveFunctionsEliminationVisitor> {
 public:
  ...
  void enterImpl(QueryTreeNodePtr& node) {
    if (!getSettings()[Setting::optimize_injective_functions_inside_uniq]) return;

    auto* function_node = node->as<FunctionNode>();
    if (!function_node || !function_node->isAggregateFunction() || !isUniqFunction(function_node->getFunctionName())) return;
    ...
    auto remove_injective_function = [&replaced_argument](QueryTreeNodePtr& arg) -> bool {
      ...
      if (!arg_typed || !arg_typed->isOrdinaryFunction()) return false;
      ...
      if (arg_arguments_nodes.size() != 1) return false;

      const auto& arg_function = arg_typed->getFunction();
      if (!arg_function->isInjective({})) return false;
    ...
    auto replaced_aggregate_function =
        AggregateFunctionFactory::instance().get(function_node->getFunctionName(), NullsAction::EMPTY, replaced_argument_types,
                                                 current_aggregate_function->getParameters(), properties);

    /// uniqCombined returns nullable with nullable arguments so the result type might change which breaks the pass
    if (!replaced_aggregate_function->getResultType()->equals(*current_aggregate_function->getResultType())) return;
