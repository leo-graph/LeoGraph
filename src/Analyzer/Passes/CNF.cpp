...
size_t countAtoms(const QueryTreeNodePtr &node) {
  ...
  if (!function_node || !isLogicalFunction(*function_node)) return 1;

  size_t atom_count = 0;
  const auto &arguments = function_node->getArguments().getNodes();
  for (const auto &argument : arguments) atom_count += countAtoms(argument);
...
class SplitMultiLogicVisitor {
 public:
  explicit SplitMultiLogicVisitor(ContextPtr context) : current_context(std::move(context)) {}

  void visit(QueryTreeNodePtr &node) {
    checkStackSize();

    auto *function_node = node->as<FunctionNode>();
    if (!function_node || !isLogicalFunction(*function_node)) return;
    ...
    if (name == "and" || name == "or") {
      ...
      if (arguments.size() > 2) {
        ...
        for (size_t i = 1; i < arguments.size(); ++i) current = createFunctionNode(function_resolver, std::move(current), arguments[i]);
 ...
 public:
  ...
  void visit(QueryTreeNodePtr &node, bool add_negation) {
    ...
    if (!function_node || !isLogicalFunction(*function_node)) {
      if (add_negation) node = createFunctionNode(not_function_resolver, std::move(node));
      return;
    }
    ...
    if (function_name == "and" || function_name == "or") {
      ...
      auto &arguments = function_node->getArguments().getNodes();
      for (auto &argument : arguments) visit(argument, add_negation);
      return;
    }
...
class PushOrVisitor {
 public:
  ...
  bool visit(QueryTreeNodePtr &node, size_t num_atoms) {
    if (max_atoms && num_atoms > max_atoms) return false;

    checkStackSize();

    auto *function_node = node->as<FunctionNode>();

    if (!function_node) return true;
    ...
    if (name == "or" || name == "and") {
      ...
      for (auto &argument : arguments) {
        if (!visit(argument, num_atoms)) return false;
      }
    }
    ...
    if (name == "or") {
      ...
      if (and_node_id == arguments.size()) return true;
...
class CollectGroupsVisitor {
 public:
  void visit(QueryTreeNodePtr &node) {
    ...
    if (!or_group.empty()) and_group.insert(std::move(or_group));
  }
  ...
  void visitImpl(QueryTreeNodePtr &node, CNF::OrGroup &or_group) {
    ...
    if (name == "and") {
      ...
      for (auto &argument : arguments) {
        ...
        if (!argument_or_group.empty()) and_group.insert(std::move(argument_or_group));
      }
    } else if (name == "or") {
      auto &arguments = function_node->getArguments().getNodes();
      for (auto &argument : arguments) visitImpl(argument, or_group);
    } else {
      assert(name == "not");
      auto &arguments = function_node->getArguments().getNodes();
...
std::optional<CNFAtomicFormula> tryInvertFunction(const CNFAtomicFormula &atom, const ContextPtr &context,
  ...
  if (!function_node) return std::nullopt;
...
std::string CNF::dump() const {
  ...
  for (const auto &group : statements) {
    ...
    for (const auto &atom : group) {
      if (!first_in_group) res << " OR ";
      first_in_group = false;
      if (atom.negative) res << " NOT ";
...
CNF &CNF::transformGroups(std::function<OrGroup(const OrGroup &)> fn) {
  ...
  for (const auto &group : statements) {
    ...
    if (!new_group.empty()) result.insert(std::move(new_group));
  }
...
CNF &CNF::transformAtoms(std::function<CNFAtomicFormula(const CNFAtomicFormula &)> fn) {
  transformGroups([fn](const OrGroup &group) {
    ...
    for (const auto &atom : group) {
      ...
      if (new_atom.node_with_hash.node) result.insert(std::move(new_atom));
    }
...
CNF &CNF::pushNotIntoFunctions(const ContextPtr &context) {
  transformAtoms([&](const CNFAtomicFormula &atom) { return pushNotIntoFunction(atom, context); });

  return *this;
}

CNFAtomicFormula CNF::pushNotIntoFunction(const CNFAtomicFormula &atom, const ContextPtr &context) {
  if (!atom.negative) return atom;

  static const std::unordered_map<std::string, std::string> inverse_relations = {
      {"equals", "notEquals"}, {"less", "greaterOrEquals"}, {"lessOrEquals", "greater"},
      {"in", "notIn"},         {"like", "notLike"},         {"empty", "notEmpty"},
      {"notEquals", "equals"}, {"greaterOrEquals", "less"}, {"greater", "lessOrEquals"},
      {"notIn", "in"},         {"notLike", "like"},         {"notEmpty", "empty"},
  };

  if (auto inverted_atom = tryInvertFunction(atom, context, inverse_relations); inverted_atom.has_value()) return std::move(*inverted_atom);
...
CNF &CNF::pullNotOutFunctions(const ContextPtr &context) {
  transformAtoms([&](const CNFAtomicFormula &atom) {
    ...
    if (auto inverted_atom = tryInvertFunction(atom, context, inverse_relations); inverted_atom.has_value())
...
CNF &CNF::filterAlwaysTrueGroups(std::function<bool(const OrGroup &)> predicate) {
  ...
  for (const auto &or_group : statements) {
    if (predicate(or_group)) filtered.insert(or_group);
  }
...
CNF &CNF::filterAlwaysFalseAtoms(std::function<bool(const CNFAtomicFormula &)> predicate) {
  ...
  for (const auto &or_group : statements) {
    ...
    for (const auto &atom : or_group) {
      if (predicate(atom)) filtered_group.insert(atom);
    }
...
void CNF::appendGroup(const AndGroup &and_group) {
  for (const auto &or_group : and_group) statements.emplace(or_group);
}

CNF::CNF(AndGroup statements_) : statements(std::move(statements_)) {}
...
std::optional<CNF> CNF::tryBuildCNF(const QueryTreeNodePtr &node, ContextPtr context, size_t max_growth_multiplier) {
  ...
  if (PushOrVisitor visitor(max_atoms); !visitor.visit(node_cloned, atom_count)) return std::nullopt;

  CollectGroupsVisitor collect_visitor;
  collect_visitor.visit(node_cloned);

  if (collect_visitor.and_group.empty()) return std::nullopt;
...
QueryTreeNodePtr CNF::toQueryTree() const {
  ...
  const auto function_node_from_atom = [&](const auto &atom) -> QueryTreeNodePtr {
    ...
    if (atom.negative) return createFunctionNode(not_resolver, std::move(cloned_node));
  ...
  for (const auto &or_group : statements) {
    if (or_group.size() == 1) {
      ...
      for (const auto &atom : or_group) or_arguments.push_back(function_node_from_atom(atom));
  ...
  if (and_arguments.size() == 1) return std::move(and_arguments[0]);
