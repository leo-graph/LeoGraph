...
class MultiIfToIfVisitor : public InDepthQueryTreeVisitorWithContext<MultiIfToIfVisitor> {
 public:
  ...
  explicit MultiIfToIfVisitor(FunctionOverloadResolverPtr if_function_ptr_, ContextPtr context)
      : Base(std::move(context)), if_function_ptr(std::move(if_function_ptr_)) {}

  void enterImpl(QueryTreeNodePtr& node) {
    if (!getSettings()[Setting::optimize_multiif_to_if]) return;

    auto* function_node = node->as<FunctionNode>();
    if (!function_node || function_node->getFunctionName() != "multiIf") return;
...
void MultiIfToIfPass::run(QueryTreeNodePtr& query_tree_node, ContextPtr context) {
  ...
  auto if_function_ptr = createInternalFunctionIfOverloadResolver(settings[Setting::use_variant_as_common_type]);
