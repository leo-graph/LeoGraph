...
class DistanceTransposedPartialReadsPassVisitor : public InDepthQueryTreeVisitorWithContext<DistanceTransposedPartialReadsPassVisitor> {
 public:
  ...
  void enterImpl(QueryTreeNodePtr& node) {
    if (!getSettings()[Setting::optimize_qbit_distance_function_reads]) return;
    ...
    if (!is_distance_function) return;

    auto& function_arguments_nodes = function_node->getArguments().getNodes();
    if (function_arguments_nodes.size() != 3) return;
    ...
    if (!storage->supportsOptimizationToSubcolumns() || storage->isVirtualColumn(column_name_type.name, storage_snapshot->metadata)) return;

    auto column_in_table = storage_snapshot->tryGetColumn(GetColumnsOptions::All, column_name_type.name);
    if (!column_in_table || !column_in_table->type->equals(*column_name_type.type)) return;
    ...
    if (function_node->getResultType()->onlyNull()) return;
    ...
    if (precision == 0 || precision > data_width) return;
    ...
    if (!function_node->getResultType()->equals(*original_result_type))
      throw Exception(ErrorCodes::LOGICAL_ERROR,
                      ...
                      "after: {}",
                      node->getNodeTypeName(), original_result_type->getName(), function_node->getResultType()->getName());
  }
};
