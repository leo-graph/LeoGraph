...
class RewriteSumFunctionWithSumAndCountVisitor : public InDepthQueryTreeVisitorWithContext<RewriteSumFunctionWithSumAndCountVisitor> {
 public:
  ...
  void enterImpl(QueryTreeNodePtr& node) {
    ...
    static const std::unordered_set<String> func_supported = {"plus", "minus"};

    auto* function_node = node->as<FunctionNode>();
    if (!function_node || Poco::toLower(function_node->getFunctionName()) != "sum") return;
    ...
    if (!func_plus_minus_node || !func_supported.contains(Poco::toLower(func_plus_minus_node->getFunctionName()))) return;

    const auto& func_plus_minus_nodes = func_plus_minus_node->getArguments().getNodes();
    if (func_plus_minus_nodes.size() != 2) return;
    ...
    if (!literal_type || !WhichDataType(literal_type).isNumber()) return;
