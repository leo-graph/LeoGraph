 ...
 * 4. All lambda expressions that are function arguments are resolved. Next passes can expect that LambaNode expression is resolved, and
 * lambda has concrete arguments.
 * 5. All standalone lambda expressions are resolved. Next passes can expect that there will be no standalone LambaNode expressions in
 * query.
 ...
 * Functions `in`, `notIn`, `globalIn`, `globalNotIn` converted into `nullIn`, `notNullIn`, `globalNullIn`, `globalNotNullIn` if setting
 * transform_null_in is true.
 *
 ...
 * If right argument resolved as table, and table is not of type Set, it is replaced with query that read only ordinary columns from
 * underlying storage. Example: SELECT id FROM test_table WHERE id IN test_table_other; Result: SELECT id FROM test_table WHERE id IN
 * (SELECT test_table_column FROM test_table_other);
 */
class QueryAnalysisPass final : public IQueryTreePass {
 public:
  ...
  String getName() override { return "QueryAnalysis"; }

  String getDescription() override {
    return "Resolve type for each query expression. Replace identifiers, matchers with query expressions. Perform constant folding. "
