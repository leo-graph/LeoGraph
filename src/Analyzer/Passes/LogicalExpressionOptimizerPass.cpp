...
bool isBooleanFunction(const String &func_name) {
  return std::any_of(boolean_functions.begin(), boolean_functions.end(),
                     [&](const auto boolean_func) { return func_name == boolean_func; });
}

bool isNodeFunction(const QueryTreeNodePtr &node, const String &func_name) {
  if (const auto *function_node = node->as<FunctionNode>()) return function_node->getFunctionName() == func_name;
  return false;
}
...
QueryTreeNodePtr getFunctionArgument(const QueryTreeNodePtr &node, size_t idx) {
  ...
  throw Exception(ErrorCodes::LOGICAL_ERROR, "Expected '{}' to be a function with at least {} arguments", node->formatASTForErrorMessage(),
                  idx + 1);
}
...
bool matchIsNullOfTwoArgs(const QueryTreeNodes &nodes, QueryTreeNodePtr &lhs, QueryTreeNodePtr &rhs) {
  ...
  for (const auto &node : nodes) {
    ...
    if (func_node->getFunctionName() == "isNull" && arguments.size() == 1) {
      ...
      if (arguments[0]->isEqual(*arguments[1])) return false;
      all_arguments.insert(QueryTreeNodePtrWithHash(arguments[0]));
      all_arguments.insert(QueryTreeNodePtrWithHash(arguments[1]));
    } else
      return false;

    if (all_arguments.size() > 2) return false;
  }

  if (all_arguments.size() != 2 || is_null_arguments.size() != 2) return false;
...
bool isBooleanConstant(const QueryTreeNodePtr &node, bool expected_value) {
  ...
  if (!constant_node || !constant_node->getResultType()->equals(DataTypeUInt8())) return false;
...
bool isOnlyConjunctionOfFunctions(const QueryTreeNodePtr &node, const String &func_name,
                                  const QueryTreeNodePtrWithHashSet &allowed_arguments) {
  if (isBooleanConstant(node, true)) return true;

  const auto *node_function = node->as<FunctionNode>();
  if (!node_function) return false;

  if (node_function->getFunctionName() == func_name && allowed_arguments.contains(node_function->getArgumentsNode())) return true;

  if (node_function->getFunctionName() == "and") {
    for (const auto &and_argument : node_function->getArguments().getNodes()) {
      if (!isOnlyConjunctionOfFunctions(and_argument, func_name, allowed_arguments)) return false;
    }
    return true;
  }
  return false;
...
void insertIfNotPresentInSet(QueryTreeNodePtrWithHashSet &set, QueryTreeNodes &nodes, QueryTreeNodePtr node) {
  ...
  if (inserted) nodes.push_back(std::move(node));
}
...
std::shared_ptr<FunctionNode> getFlattenedLogicalExpression(const FunctionNode &node, const ContextPtr &context) {
  ...
  if (function_name != "or" && function_name != "and") return nullptr;
  ...
  if (!flattened_anything && new_arguments.size() == arguments.size()) return {};
...
// Result: A or B
std::optional<CommonExpressionExtractionResult> tryExtractCommonExpressionsInDisjunction(const QueryTreeNodes &disjuncts,
  ...
  if (removed == disjuncts.size() - 1) {
    for (size_t i = 0; i < disjuncts.size(); ++i) {
      if (should_keep[i]) return CommonExpressionExtractionResult{.new_node = nullptr, .common_expressions = {disjuncts[i]}};
    }
  }
  ...
  for (size_t i = 0; i < disjuncts.size(); ++i) {
    if (should_keep[i]) new_disjuncts.emplace_back(disjuncts[i]);
  }
...
std::optional<CommonExpressionExtractionResult> tryExtractCommonExpressions(const QueryTreeNodePtr &node, const ContextPtr &context) {
  ...
  if (!or_node || or_node->getFunctionName() != "or") return {};  // the optimization can only be done on or nodes

  auto flattened_or_node = getFlattenedLogicalExpression(*or_node, context);
  if (flattened_or_node) or_node = flattened_or_node.get();
  ...
  for (auto &maybe_and_node : or_argument_nodes) {
    ...
    if (first_argument) {
      ...
      for (auto &and_argument : current_arguments) insertIfNotPresentInSet(common_exprs_set, common_exprs, and_argument);
      ...
      for (auto &and_argument : and_node->getArguments()) {
        if (common_exprs_set.contains(and_argument)) insertIfNotPresentInSet(new_common_exprs_set, new_common_exprs, and_argument);
      }
  ...
  for (auto &or_argument : or_argument_nodes) {
    if (auto it = flattened_ands.find(or_argument); it != flattened_ands.end()) or_argument = it->second;
    ...
    std::copy_if(and_arguments.begin(), and_arguments.end(), std::back_inserter(filtered_and_arguments),
  ...
  // There are at least two arguments in the passed-in OR expression, thus we either completely eliminated at least one arguments, or there
  // should be at least 2 remaining arguments. The complete elimination is handled above, so at this point we can be sure there are at least
  // 2 arguments.
  ...
  if (auto optimized_disjunction = tryExtractCommonExpressionsInDisjunction(new_or_arguments, context)) {
    auto new_disjunction_node =
        optimized_disjunction->new_node ? optimized_disjunction->new_node : optimized_disjunction->common_expressions[0];
...
void tryOptimizeCommonExpressionsInOr(QueryTreeNodePtr &node, const ContextPtr &context) {
  ...
  if (auto maybe_result = tryExtractCommonExpressions(node, context); maybe_result.has_value()) {
    ...
    if (result.new_node != nullptr) new_root_arguments.push_back(std::move(result.new_node));
...
void tryOptimizeCommonExpressionsInAnd(QueryTreeNodePtr &node, const ContextPtr &context) {
  ...
  for (const auto &argument : root_node->getArguments()) {
    if (auto maybe_result = tryExtractCommonExpressions(argument, context)) {
      ...
      auto &result = *maybe_result;
      if (result.new_node != nullptr) insert_possible_new_top_level_arg(std::move(result.new_node));
      for (auto &common_expr : result.common_expressions) insert_possible_new_top_level_arg(std::move(common_expr));
    } else {
      insert_possible_new_top_level_arg(argument);
    }
  }

  if (!extracted_something) return;
...
class JoinOnLogicalExpressionOptimizerVisitor : public InDepthQueryTreeVisitorWithContext<JoinOnLogicalExpressionOptimizerVisitor> {
 public:
  ...
  explicit JoinOnLogicalExpressionOptimizerVisitor(const JoinNode &join_node_, ContextPtr context)
      : Base(std::move(context)), join_node(join_node_) {}
  ...
  void enterImpl(QueryTreeNodePtr &node) {
     ...
     * Values `NULL` and `false` are not equivalent in SELECT, so we cannot change type from Nullable(UInt8) to UInt8 there, while it's
     * valid for `JOIN ON`. Also, operator <=> can be used in JOIN ON, but not in SELECT, so we need to keep original expression `a = b OR
     * isNull(a) AND isNull(b) there.
     *
     * FIXME: May be removed after https://github.com/ClickHouse/ClickHouse/pull/66143
     */
    if (node.use_count() > 1) node = node->clone();
  ...
  void leaveImpl(QueryTreeNodePtr &node) {
    ...
    if (need_rerun_resolve) rerunFunctionResolve(function_node, getContext());

    // The optimization only makes sense on the top level
    if (node != join_node.getJoinExpression() || !getSettings()[Setting::optimize_extract_common_expressions]) return;
 ...
 private:
  ...
  QueryTreeNodePtr tryOptimizeJoinOnNulls(const QueryTreeNodes &nodes, const ContextPtr &context) {
    ...
    for (const auto &node : nodes) {
      ...
      if (!argument_function) continue;

      const auto &func_name = argument_function->getFunctionName();
      if (func_name == "equals" || func_name == "isNotDistinctFrom") {
        if (isTwoArgumentsFromDifferentSides(*argument_function, join_node)) equals_functions_indices.push_back(or_operands.size() - 1);
        ...
        if (const auto &equals_function = findEqualsFunction(and_arguments)) {
          ...
          for (const auto &and_argument : and_arguments) {
            if (and_argument.get() == equals_function.get()) continue;

            if (isOnlyConjunctionOfFunctions(and_argument, "isNotNull", allowed_arguments)) continue;
    ...
    for (size_t equals_function_idx : equals_functions_indices) {
      ...
      std::set_intersection(lhs_is_null_parents.begin(), lhs_is_null_parents.end(), rhs_is_null_parents.begin(), rhs_is_null_parents.end(),
    ...
    if (new_or_operands.empty()) return nullptr;

    if (new_or_operands.size() == 1) return new_or_operands[0];
...
class LogicalExpressionOptimizerVisitor : public InDepthQueryTreeVisitorWithContext<LogicalExpressionOptimizerVisitor> {
 public:
  ...
  explicit LogicalExpressionOptimizerVisitor(ContextPtr context) : Base(std::move(context)) {}
  ...
  void enterImpl(QueryTreeNodePtr &node) {
    ...
    if (function_node->getFunctionName() == "and") {
      ...
      if (getSettings()[Setting::optimize_and_compare_chain]) tryOptimizeAndCompareChain(node);
      return;
    }
  ...
  void leaveImpl(QueryTreeNodePtr &node) {
    if (!getSettings()[Setting::optimize_extract_common_expressions]) return;
 ...
 private:
  void tryOptimizeAndEqualsNotEqualsChain(QueryTreeNodePtr &node) {
    ...
    if (function_node.getResultType()->isNullable()) return;
    ...
    for (const auto &argument : function_node.getArguments()) {
      ...
      if (function_name == "equals") {
        /*
         * This function checks a pattern of `col = X and col = Y` such that if there are two different constant values are assigned for the
         * same expression. That pattern always yields to `FALSE`, except implicit conversion from a string to an integer / a boolean which
         * the pattern could be `col = X and col = 'X'`. In such cases, constant values are same.
         */
        const auto has_and_with_different_constant = [&](const QueryTreeNodePtr &expression, const ConstantNode *constant) {
          ...
          const auto convert_and_check_equals = [](const ConstantNode *string_value, const ConstantNode *expected) {
            ...
            if (!converted.isNull()) return accurateEquals(converted, expected->getValue());
            return false;
          };
        ...
        if (const auto *lhs_literal = lhs->as<ConstantNode>(); lhs_literal && !lhs_literal->getValue().isNull())
          add_not_equals_function_if_not_present(rhs, lhs_literal);
        else if (const auto *rhs_literal = rhs->as<ConstantNode>(); rhs_literal && !rhs_literal->getValue().isNull())
    ...
    if (and_operands.size() == function_node.getArguments().getNodes().size()) return;
  ...
  void tryOptimizeAndCompareChain(QueryTreeNodePtr &node) {
    if (node->getNodeType() != QueryTreeNodeType::FUNCTION) return;

    auto &function_node = node->as<FunctionNode &>();
    if (function_node.getFunctionName() != "and" || function_node.getResultType()->isNullable()) return;

    enum CompareType { less = 0, greater, lessOrEquals, greaterOrEquals, equals };
    ...
    const auto &arguments = flattened_and_node ? flattened_and_node->getArguments().getNodes() : function_node.getArguments().getNodes();

    for (const auto &argument : arguments) {
      auto *argument_function = argument->as<FunctionNode>();
      const auto valid_functions = std::unordered_set<std::string>{"less", "greater", "lessOrEquals", "greaterOrEquals", "equals"};
      if (!argument_function || !valid_functions.contains(argument_function->getFunctionName())) continue;
      ...
      if (function_name == "less") {
        if (rhs->as<ConstantNode>()) greater_constants.insert(rhs);
        greater_pairs[rhs].push_back({lhs, CompareType::less});
        if (lhs->as<ConstantNode>()) less_constants.insert(lhs);
        less_pairs[lhs].push_back({rhs, CompareType::greater});
      } else if (function_name == "greater") {
        if (lhs->as<ConstantNode>()) greater_constants.insert(lhs);
        greater_pairs[lhs].push_back({rhs, CompareType::less});
        if (rhs->as<ConstantNode>()) less_constants.insert(rhs);
        ...
        [&](const ComparePairs &pairs, QueryTreeNodePtr current, const ConstantNode *constant, CompareType type) {
          if (auto it = pairs.find(current); it != pairs.end()) {
            for (const auto &left : it->second) {
              if (visited.contains(left.first)) continue;
  ...
  void tryReplaceOrEqualsChainWithIn(QueryTreeNodePtr &node) {
    ...
    for (auto &[expression, equals_functions] : node_to_equals_functions) {
      ...
      if (type_low_cardinality) result_type = type_low_cardinality->getDictionaryType();
    ...
    if (or_operands.size() == function_node.getArguments().getNodes().size()) return;
  ...
  void tryOptimizeOutRedundantEquals(QueryTreeNodePtr &node) {
    ...
    if (function_arguments.size() != 2) return;
    ...
    if (!child_function || !isBooleanFunction(child_function->getFunctionName())) return;
    ...
    if (!function_node_type->equals(*node->getResultType())) {
      ...
      chassert(
          removeNullable(removeLowCardinality(function_node_type))->equals(*removeNullable(removeLowCardinality(node->getResultType()))));
