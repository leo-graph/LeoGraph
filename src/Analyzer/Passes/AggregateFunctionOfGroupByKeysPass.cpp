...
class EliminateFunctionVisitor : public InDepthQueryTreeVisitorWithContext<EliminateFunctionVisitor> {
 public:
  ...
  void enterImpl(QueryTreeNodePtr& node) {
    if (!getSettings()[Setting::optimize_aggregators_of_group_by_keys]) return;
      ...
      for (auto& group_key : query_node->getGroupBy().getNodes()) {
        ...
        if (auto* list = group_key->as<ListNode>()) {
          if (first_grouping_set) {
            for (auto& group_elem : list->getNodes()) group_by_keys.insert(group_elem);
            first_grouping_set = false;
          } else {
            QueryTreeNodePtrWithHashSet common_keys_set;
            for (auto& group_elem : list->getNodes()) {
              if (group_by_keys.contains(group_elem)) common_keys_set.insert(group_elem);
            }
  ...
  void leaveImpl(QueryTreeNodePtr& node) {
    ...
    if (node->getNodeType() == QueryTreeNodeType::FUNCTION) {
      if (aggregationCanBeEliminated(node, group_by_keys_stack.back())) node = node->as<FunctionNode>()->getArguments().getNodes()[0];
 ...
 private:
  ...
  bool aggregationCanBeEliminated(QueryTreeNodePtr& node, const QueryTreeNodePtrWithHashSet& group_by_keys) {
    if (group_by_keys.empty()) return false;

    auto* function = node->as<FunctionNode>();
    if (!function || !function->isAggregateFunction()) return false;

    if (!(function->getFunctionName() == "min" || function->getFunctionName() == "max" || function->getFunctionName() == "any" ||
    ...
    if (function_arguments.size() != 1)
      throw Exception(ErrorCodes::LOGICAL_ERROR, "Expected a single argument of function '{}' but received {}", function->getFunctionName(),
                      function_arguments.size());

    if (!function->getResultType()->equals(*function_arguments[0]->getResultType())) return false;
    ...
    while (!candidates.empty()) {
      ...
      switch (candidate->getNodeType()) {
        case QueryTreeNodeType::FUNCTION: {
          ...
          if (!found) {
            bool is_deterministic_function = parents_are_only_deterministic && func->getFunctionOrThrow()->isDeterministicInScopeOfQuery();
            for (auto it = arguments.rbegin(); it != arguments.rend(); ++it) candidates.push_back({*it, is_deterministic_function});
          }
          break;
        }
        ...
        case QueryTreeNodeType::CONSTANT:
          if (!parents_are_only_deterministic) return false;
          break;
        default:
          return false;
      }
