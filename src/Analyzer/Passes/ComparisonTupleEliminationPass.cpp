...
class ComparisonTupleEliminationPassVisitor : public InDepthQueryTreeVisitorWithContext<ComparisonTupleEliminationPassVisitor> {
 public:
  ...
  static bool needChildVisit(QueryTreeNodePtr &parent, QueryTreeNodePtr &child) {
    if (parent->getNodeType() == QueryTreeNodeType::JOIN) {
      ...
      if (parent->as<const JoinNode &>().getJoinExpression().get() == child.get()) return false;
    }
  ...
  void enterImpl(QueryTreeNodePtr &node) const {
    ...
    if (comparison_function_name != "equals" && comparison_function_name != "notEquals") return;
 ...
 private:
  QueryTreeNodePtr tryOptimizeComparisonTupleFunctions(const QueryTreeNodePtr &lhs_function_node, const QueryTreeNodePtr &rhs_function_node,
                                                       const std::string &comparison_function_name) const {
    const auto &lhs_function_node_typed = lhs_function_node->as<FunctionNode &>();
    if (lhs_function_node_typed.getFunctionName() != "tuple") return {};

    const auto &rhs_function_node_typed = rhs_function_node->as<FunctionNode &>();
    if (rhs_function_node_typed.getFunctionName() != "tuple") return {};
    ...
    if (lhs_tuple_function_arguments_nodes_size != rhs_tuple_function_arguments_nodes.size()) return {};
  ...
  QueryTreeNodePtr tryOptimizeComparisonTupleFunctionAndConstant(const QueryTreeNodePtr &function_node,
                                                                 const QueryTreeNodePtr &constant_node,
                                                                 const std::string &comparison_function_name) const {
    const auto &function_node_typed = function_node->as<FunctionNode &>();
    if (function_node_typed.getFunctionName() != "tuple") return {};

    auto &constant_node_typed = constant_node->as<ConstantNode &>();
    const auto &constant_node_value = constant_node_typed.getValue();
    if (constant_node_value.getType() != Field::Types::Which::Tuple) return {};
    ...
    if (function_arguments_nodes_size != constant_tuple.size()) return {};
    ...
    if (!tuple_data_type) return {};

    const auto &tuple_data_type_elements = tuple_data_type->getElements();
    if (tuple_data_type_elements.size() != function_arguments_nodes_size) return {};
  ...
  QueryTreeNodePtr makeComparisonFunction(QueryTreeNodePtr lhs_argument, QueryTreeNodePtr rhs_argument,
                                          const std::string &comparison_function_name) const {
