...
class ConvertOrLikeChainVisitor : public InDepthQueryTreeVisitorWithContext<ConvertOrLikeChainVisitor> {
 public:
  ...
  explicit ConvertOrLikeChainVisitor(FunctionOverloadResolverPtr or_function_resolver_,
                                     FunctionOverloadResolverPtr match_function_resolver_, ContextPtr context)
  ...
  bool needChildVisit(VisitQueryTreeNodeType &, VisitQueryTreeNodeType &) {
    ...
    return settings[Setting::optimize_or_like_chain] && settings[Setting::allow_hyperscan] &&
           settings[Setting::max_hyperscan_regexp_length] == 0 && settings[Setting::max_hyperscan_regexp_total_length] == 0;
  }

  void enterImpl(QueryTreeNodePtr &node) {
    auto *function_node = node->as<FunctionNode>();
    if (!function_node || function_node->getFunctionName() != "or") return;
    ...
    if (unique_elems.size() == 1) unique_elems.push_back(std::make_shared<ConstantNode>(static_cast<UInt8>(0)));
