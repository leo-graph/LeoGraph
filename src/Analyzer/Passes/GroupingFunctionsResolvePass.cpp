...
struct GroupByKeyComparator {
  GroupByKeyComparator(QueryTreeNodePtr node_)  /// NOLINT
      : node(std::move(node_)), hash(node->getTreeHash({.compare_aliases = false, .compare_types = true})) {}
  ...
  struct Hasher {
    size_t operator()(const GroupByKeyComparator &key) const { return key.hash.low64; }
  };
...
enum class GroupByKind : uint8_t { ORDINARY, ROLLUP, CUBE, GROUPING_SETS };

class GroupingFunctionResolveVisitor : public InDepthQueryTreeVisitorWithContext<GroupingFunctionResolveVisitor> {
 public:
  GroupingFunctionResolveVisitor(GroupByKind group_by_kind_, AggredationKeyNodeMap<size_t> aggregation_key_to_index_,
                                 ColumnNumbersList grouping_sets_keys_indices_, ContextPtr context_)
  ...
  void enterImpl(const QueryTreeNodePtr &node) {
    ...
    if (!function_node || function_node->getFunctionName() != "grouping") return;
    ...
    for (const auto &argument : function_arguments) {
      ...
      if (it == aggregation_key_to_index.end()) {
        throw Exception(ErrorCodes::BAD_ARGUMENTS, "Argument {} of GROUPING function is not a part of GROUP BY clause [{}]",
                        ...
                        fmt::join(aggregation_key_to_index |
                                      std::views::transform([](const auto &e) { return e.first.node->formatASTForErrorMessage(); }),
                                  ", "));
      }
    ...
    switch (group_by_kind) {
      ...
      case GroupByKind::ROLLUP: {
        auto grouping_rollup_function =
            std::make_shared<FunctionGroupingForRollup>(arguments_indexes, aggregation_keys_size, force_grouping_standard_compatibility);
      ...
      case GroupByKind::CUBE: {
        auto grouping_cube_function =
            std::make_shared<FunctionGroupingForCube>(arguments_indexes, aggregation_keys_size, force_grouping_standard_compatibility);
...
void resolveGroupingFunctions(QueryTreeNodePtr &query_node, ContextPtr context) {
    ...
    if (query_node_typed.isGroupByWithGroupingSets()) {
      for (const auto &grouping_set_keys_list_node : query_node_typed.getGroupBy().getNodes()) {
        ...
        for (auto &grouping_set_key_node : grouping_set_keys_list_node_typed.getNodes()) {
          if (used_keys_in_set.contains(grouping_set_key_node)) continue;
          used_keys_in_set.insert(grouping_set_key_node);
          grouping_sets_used_aggregation_keys.push_back(grouping_set_key_node);

          if (aggregation_key_to_index.contains(grouping_set_key_node)) continue;
      ...
      for (auto &group_by_key_node : query_node_typed.getGroupBy().getNodes()) {
        if (aggregation_key_to_index.contains(group_by_key_node)) continue;
  ...
  for (const auto &grouping_set_used_aggregation_keys : grouping_sets_used_aggregation_keys_list) {
    ...
    for (const auto &used_aggregation_key : grouping_set_used_aggregation_keys) {
      ...
      if (aggregation_node_index_it == aggregation_key_to_index.end())
        throw Exception(ErrorCodes::LOGICAL_ERROR, "Aggregation key {} in GROUPING SETS is not found in GROUP BY keys",
  ...
  GroupingFunctionResolveVisitor visitor(group_by_kind, std::move(aggregation_key_to_index), std::move(grouping_sets_keys_indexes),
                                         std::move(context));
...
class GroupingFunctionsResolveVisitor : public InDepthQueryTreeVisitorWithContext<GroupingFunctionsResolveVisitor> {
 ...
 public:
  explicit GroupingFunctionsResolveVisitor(ContextPtr context_) : Base(std::move(context_)) {}

  void enterImpl(QueryTreeNodePtr &node) {
    if (node->getNodeType() != QueryTreeNodeType::QUERY) return;
