...
class OptimizeDateOrDateTimeConverterWithPreimageVisitor
 ...
 public:
  ...
  static bool needChildVisit(QueryTreeNodePtr &node, QueryTreeNodePtr & /*child*/) {
    ...
    if (const auto *function = node->as<FunctionNode>()) return !relations.contains(function->getFunctionName());
  ...
  void enterImpl(QueryTreeNodePtr &node) const {
    ...
    if (!getSettings()[Setting::optimize_time_filter_with_preimage]) return;
    ...
    if (!literal || !literal->getResultType()->isValueRepresentedByUnsignedInteger()) return;
    ...
    if (column_id->getColumnName() == "__grouping_set") return;
 ...
 private:
  QueryTreeNodePtr generateOptimizedDateFilter(const String &comparator, const QueryTreeNodePtr &column_node,
                                               const FieldInterval &range) const {
    ...
    if (comparator == "notEquals") {
      return createFunctionNode("or", createFunctionNode("less", column_node, std::make_shared<ConstantNode>(start_date_or_date_time)),
