...
class ShardNumColumnToFunctionVisitor : public InDepthQueryTreeVisitorWithContext<ShardNumColumnToFunctionVisitor> {
 public:
  ...
  void enterImpl(QueryTreeNodePtr &node) const {
    ...
    if (column.name != "_shard_num") return;
    ...
    if (!table_node && !table_function_node) return;

    const auto &storage = table_node ? table_node->getStorage() : table_function_node->getStorage();
    if (!storage->isRemote()) return;

    const auto &storage_snapshot = table_node ? table_node->getStorageSnapshot() : table_function_node->getStorageSnapshot();
    if (!storage->isVirtualColumn(column.name, storage_snapshot->metadata)) return;

    const auto column_type = column_node->getColumnType();
    const auto shard_num_column_numeric_type =
        column_type->isNullable() ? typeid_cast<const DataTypeNullable *>(column_type.get())->getNestedType() : column_type;
    ...
    if (!function_result_type->equals(*shard_num_column_numeric_type)) return;

    node = std::move(shard_num_function_node);
    if (column_type->isNullable()) node = createCastFunction(node, column_type, getContext());
  }
};
