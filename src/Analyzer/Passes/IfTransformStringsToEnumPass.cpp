...
auto getDataEnumType(const std::set<std::string>& string_values) {
  ...
  size_t number = 1;
  for (const auto& value : string_values) enum_values.emplace_back(value, number++);

  return std::make_shared<EnumType>(std::move(enum_values));
}

DataTypePtr getEnumType(const std::set<std::string>& string_values) {
  if (string_values.size() >= 255) return getDataEnumType<DataTypeEnum16>(string_values);
...
void changeIfArguments(FunctionNode& if_node, const std::set<std::string>& string_values, const ContextPtr& context) {
...
/// transform(value, array_from, array_to, default_value) will be transformed to transform(value, array_from, _CAST(array_to,
/// Array(Enum...)), _CAST(default_value, Enum...)) where Enum is generated based on the possible values stored in string_values
void changeTransformArguments(FunctionNode& transform_node, const std::set<std::string>& string_values, const ContextPtr& context) {
  auto result_type = getEnumType(string_values);
...
class ConvertStringsToEnumVisitor : public InDepthQueryTreeVisitorWithContext<ConvertStringsToEnumVisitor> {
 public:
  ...
  void enterImpl(QueryTreeNodePtr& node) {
    if (!getSettings()[Setting::optimize_if_transform_strings_to_enum]) return;
    ...
    if (function_name == "if") {
      if (function_node->getArguments().getNodes().size() != 3) return;
      ...
      if (!first_literal || !second_literal) return;

      if (!isString(first_literal->getResultType()) || !isString(second_literal->getResultType())) return;
    ...
    if (function_name == "transform") {
      if (function_node->getArguments().getNodes().size() != 4) return;
      ...
      if (!isString(removeNullable(function_node->getResultType()))) return;

      const auto* literal_to = argument_nodes[2]->as<ConstantNode>();
      const auto* literal_default = argument_nodes[3]->as<ConstantNode>();

      if (!literal_to || !literal_default) return;

      if (!isArray(literal_to->getResultType()) || !isString(literal_default->getResultType())) return;

      auto array_to = literal_to->getValue().safeGet<Array>();

      if (array_to.empty()) return;

      if (!std::all_of(array_to.begin(), array_to.end(), [](const auto& field) { return field.getType() == Field::Types::Which::String; }))
        return;

      /// collect possible string values
      std::set<std::string> string_values;

      for (const auto& value : array_to) string_values.insert(value.safeGet<std::string>());
