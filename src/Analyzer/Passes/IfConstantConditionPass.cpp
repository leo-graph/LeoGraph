...
class IfConstantConditionVisitor : public InDepthQueryTreeVisitorWithContext<IfConstantConditionVisitor> {
 public:
  ...
  void enterImpl(QueryTreeNodePtr& node) {
    ...
    if (!function_node || (function_node->getFunctionName() != "if" && function_node->getFunctionName() != "multiIf")) return;
    ...
    if (!first_argument_constant_node) return;
    ...
    if (node->getResultType()->equals(*argument_node->getResultType())) node = argument_node;
  }
};
