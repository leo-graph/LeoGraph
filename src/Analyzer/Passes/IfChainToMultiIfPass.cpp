...
class IfChainToMultiIfPassVisitor : public InDepthQueryTreeVisitorWithContext<IfChainToMultiIfPassVisitor> {
 public:
  ...
  explicit IfChainToMultiIfPassVisitor(FunctionOverloadResolverPtr multi_if_function_ptr_, ContextPtr context)
      : Base(std::move(context)), multi_if_function_ptr(std::move(multi_if_function_ptr_)) {}

  void enterImpl(QueryTreeNodePtr& node) {
    if (!getSettings()[Setting::optimize_if_chain_to_multiif]) return;

    auto* function_node = node->as<FunctionNode>();
    if (!function_node || function_node->getFunctionName() != "if" || function_node->getArguments().getNodes().size() != 3) return;
    ...
    while (true) {
      ...
      if (!if_chain_function_node || if_chain_function_node->getFunctionName() != "if" ||
          if_chain_function_node->getArguments().getNodes().size() != 3)
        break;
    ...
    if (multi_if_arguments.size() <= 3) return;
    ...
    if (!multi_if_function->getResultType()->equals(*function_node->getResultType())) return;
...
void IfChainToMultiIfPass::run(QueryTreeNodePtr& query_tree_node, ContextPtr context) {
  ...
  auto multi_if_function_ptr = createInternalMultiIfOverloadResolver(settings[Setting::allow_execute_multiif_columnar],
