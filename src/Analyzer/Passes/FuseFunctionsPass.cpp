...
bool sourceHasAggregateProjections(const QueryTreeNodePtr &source, const ContextPtr &context) {
  ...
  if (!context->getSettingsRef()[Setting::optimize_use_projections]) return false;

  auto metadata = table_node->getStorage()->getInMemoryMetadataPtr();
  for (const auto &projection : metadata->projections) {
    if (projection.type == ProjectionDescription::Type::Aggregate) return true;
  }
  return false;
}
...
class FuseFunctionsVisitor : public InDepthQueryTreeVisitorWithContext<FuseFunctionsVisitor> {
 public:
  ...
  explicit FuseFunctionsVisitor(const std::unordered_set<String> names_to_collect_, ContextPtr context)
      : Base(std::move(context)), names_to_collect(names_to_collect_) {}

  void enterImpl(QueryTreeNodePtr &node) {
    if (!getSettings()[Setting::optimize_syntax_fuse_functions]) return;

    auto *function_node = node->as<FunctionNode>();
    if (!function_node || !function_node->isAggregateFunction() || !names_to_collect.contains(function_node->getFunctionName())) return;
    ...
    if (!argument_with_source) return;
  ...
  std::optional<ArgumentWithSource> createArgumentWithSource(const QueryTreeNodePtr &argument) {
    ...
    if (visitor.column_sources.size() != 1) return std::nullopt;
    return std::make_pair(argument, *visitor.column_sources.begin());
  }

  struct ArgumentWithSourceHash {
    size_t operator()(const ArgumentWithSource &p) const noexcept { return p.first.hash.low64 ^ std::hash<const void *>{}(p.second.get()); }
  };
...
FunctionNodePtr createResolvedAggregateFunction(const String &name, const QueryTreeNodePtr &argument, const Array &parameters = {}) {
  ...
  if (!parameters.empty()) {
    ...
    for (const auto &param : parameters) parameter_nodes.emplace_back(std::make_shared<ConstantNode>(param));
  ...
  auto aggregate_function =
      AggregateFunctionFactory::instance().get(name, NullsAction::EMPTY, {argument->getResultType()}, parameters, properties);
...
void replaceWithSumCount(QueryTreeNodePtr &node, const FunctionNodePtr &sum_count_node, ContextPtr context) {
  ...
  if (!sum_count_result_type || sum_count_result_type->getElements().size() != 2) {
    throw Exception(ErrorCodes::LOGICAL_ERROR, "Unexpected return type '{}' of function '{}', should be tuple of two elements",
...
FunctionNodePtr createFusedQuantilesNode(std::vector<QueryTreeNodePtr *> &nodes, const QueryTreeNodePtr &argument) {
  ...
  for (const auto *node : nodes) {
    ...
    if (!constant_node) throw Exception(ErrorCodes::BAD_ARGUMENTS, "Function '{}' should have constant parameter", function_name);

    const auto &value = constant_node->getValue();
    if (value.getType() != Field::Types::Float64)
      throw Exception(ErrorCodes::BAD_ARGUMENTS, "Function '{}' should have parameter of type Float64, got '{}'", function_name,
                      value.getTypeName());
...

  {
    ...
    std::sort(permutation.begin(), permutation.end(),
              [&](size_t i, size_t j) { return parameters[i].safeGet<Float64>() < parameters[j].safeGet<Float64>(); });
...
void tryFuseSumCountAvg(QueryTreeNodePtr query_tree_node, ContextPtr context) {
  ...
  for (auto &[argument, nodes] : visitor.argument_to_functions_mapping) {
    ...
    /// even though there is only one unique function — fusing in that case is wrong.
    {
      ...
      for (auto *node : nodes) distinct_names.insert((*node)->as<const FunctionNode &>().getFunctionName());
      if (distinct_names.size() < 2) continue;
    }

    if (isNullableOrLowCardinalityNullable(argument.first.node->getResultType()))
      /// Do not apply to functions with Nullable/LowCardinality(Nullable) arguments, because `sumCount` handles it different from `sum` and
      /// `avg`.
      continue;
    ...
    if (sourceHasAggregateProjections(argument.second, context))
      /// Fusion breaks projection matching because the optimizer cannot match `sumCount` to projections defined with `sum`, `count`, or
      /// `avg`.
      continue;
...
void tryFuseQuantiles(QueryTreeNodePtr query_tree_node, ContextPtr context) {
  ...
  for (auto &[argument, nodes_set] : visitor_quantile.argument_to_functions_mapping) {
    ...
    if (!result_array_type)
      throw Exception(ErrorCodes::LOGICAL_ERROR, "Unexpected return type '{}' of function '{}', should be array",
