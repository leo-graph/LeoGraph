...
/// Before columns to substream optimization, we need to make sure, that column with such name as substream does not exists, otherwise the
/// optimize will use it instead of substream.
...
void optimizeFunctionStringLength(QueryTreeNodePtr &node, FunctionNode &, ColumnContext &ctx) {
  ...
  if (sourceHasColumn(ctx.column_source, column.name) || !canOptimizeToSubcolumn(ctx.column_source, column.name)) return;
...
void optimizeFunctionEmpty(QueryTreeNodePtr &, FunctionNode &function_node, ColumnContext &ctx) {
  ...
  if (auto *table_node = ctx.column_source->as<TableNode>()) {
    auto actual =
        table_node->getStorageSnapshot()->tryGetColumn(GetColumnsOptions(GetColumnsOptions::All).withRegularSubcolumns(), column.name);
    if (actual && actual->type->isNullable()) return;
  }
...
std::optional<NameAndTypePair> getSubcolumnForElement(const Field &value, const DataTypeTuple &data_type_tuple) {
  ...
  if (value.getType() == Field::Types::Int64) {
    ...
    if (index == 0 || std::abs(index) > size) return {};
...
std::optional<NameAndTypePair> getSubcolumnForElement(const Field &value, const DataTypeVariant &data_type_variant) {
  if (value.getType() != Field::Types::String) return {};
...
std::optional<NameAndTypePair> getSubcolumnForElement(const Field &value, const DataTypeQBit &data_type_qbit) {
  ...
  if (index == 0 || index > data_type_qbit.getElementSize()) return {};
...
void optimizeTupleOrVariantElement(QueryTreeNodePtr &node, FunctionNode &function_node, ColumnContext &ctx) {
  /// Replace `tupleElement(tuple_argument, string_literal)`, `tupleElement(tuple_argument, integer_literal)` with
  /// `tuple_argument.column_name`. Replace `variantElement(variant_argument, string_literal)` with `variant_argument.column_name`.

  auto &function_arguments_nodes = function_node.getArguments().getNodes();
  if (function_arguments_nodes.size() != 2) return;

  const auto *second_argument_constant_node = function_arguments_nodes[1]->as<ConstantNode>();
  if (!second_argument_constant_node) return;
...
void optimizeDistinctJSONPaths(QueryTreeNodePtr &node, FunctionNode &, ColumnContext &ctx) {
  /// Replace distinctJSONPaths(json) to
  /// arraySort(groupArrayDistinct(arrayJoin(json.__special_subcolumn_name_for_distinct_paths_calculation)))
  NameAndTypePair column{ctx.column.name + "." + DataTypeObject::SPECIAL_SUBCOLUMN_NAME_FOR_DISTINCT_PATHS_CALCULATION,
...
std::map<std::pair<TypeIndex, String>, NodeToSubcolumnTransformer> node_transformers = {
    ...
    },
    {
        ...
        optimizeFunctionStringEmpty<true>,
    },
    {
        {TypeIndex::String, "notEmpty"},
        optimizeFunctionStringEmpty<false>,
    },
    {
        {TypeIndex::Array, "length"},
        optimizeFunctionLength,
    },
    {
        {TypeIndex::Array, "empty"},
        optimizeFunctionEmpty<true>,
    },
    {
        {TypeIndex::Array, "notEmpty"},
        optimizeFunctionEmpty<false>,
    },
    {
        {TypeIndex::Map, "length"},
        optimizeFunctionLength,
    },
    {
        {TypeIndex::Map, "empty"},
    ...
    },
    {
        ...
        [](QueryTreeNodePtr &node, FunctionNode &, ColumnContext &ctx) {
          ...
          if (auto *table_node = ctx.column_source->as<TableNode>()) {
            auto actual = table_node->getStorageSnapshot()->tryGetColumn(GetColumnsOptions(GetColumnsOptions::All).withRegularSubcolumns(),
                                                                         column.name);
            if (actual && actual->type->isNullable()) return;
          }
    ...
    },
    {
        ...
        optimizeTupleOrVariantElement<DataTypeTuple>,
    },
    {
        {TypeIndex::Variant, "variantElement"},
        optimizeTupleOrVariantElement<DataTypeVariant>,
    },
    {
        {TypeIndex::QBit, "tupleElement"},
        optimizeTupleOrVariantElement<DataTypeQBit>,  /// QBit uses tupleElement for subcolumns
    },
    {
        {TypeIndex::Object, "distinctJSONPaths"},
        optimizeDistinctJSONPaths,
    },
};
...
std::tuple<FunctionNode *, ColumnNode *, TableNode *> getTypedNodesForOptimization(const QueryTreeNodePtr &node,
  ...
  auto &function_arguments_nodes = function_node->getArguments().getNodes();
  if (function_arguments_nodes.empty() || function_arguments_nodes.size() > 2) return {};
  ...
  if (view_source && view_source->getStorageID().getFullNameNotQuoted() == storage->getStorageID().getFullNameNotQuoted()) return {};

  if (!storage->supportsOptimizationToSubcolumns() || storage->isVirtualColumn(column.name, storage_snapshot->metadata)) return {};

  auto column_in_table = storage_snapshot->tryGetColumn(GetColumnsOptions(GetColumnsOptions::All).withSubcolumns(), column.name);
  if (!column_in_table || !column_in_table->type->equals(*column.type)) return {};
...
class FunctionToSubcolumnsVisitorFirstPass : public InDepthQueryTreeVisitorWithContext<FunctionToSubcolumnsVisitorFirstPass> {
 public:
  ...
  void enterImpl(const QueryTreeNodePtr &node) {
    if (!getSettings()[Setting::optimize_functions_to_subcolumns]) return;
  ...
  std::unordered_set<Identifier> getIdentifiersToOptimize() const {
    ...
    for (const auto &[identifier, count] : optimized_identifiers_count) {
      if (all_key_columns.contains(identifier)) continue;

      auto it = identifiers_count.find(identifier);
      if (it != identifiers_count.end() && it->second == count) identifiers_to_optimize.insert(identifier);
    }
 ...
 private:
  ...
  void enterImpl(const TableNode &table_node) {
    ...
    if (!processed_tables.emplace(table_name).second) return;
  ...
  void enterImpl(const ColumnNode &column_node) {
    if (column_node.getColumnName() == "__grouping_set") return;
  ...
  void enterImpl(const FunctionNode &function_node, const ColumnNode &first_argument_column_node, const TableNode &table_node) {
    ...
    if (table_node.hasTableExpressionModifiers() && table_node.getTableExpressionModifiers()->hasFinal()) return;

    const auto &column = first_argument_column_node.getColumn();
    auto table_name = table_node.getStorage()->getStorageID().getFullTableName();
    Identifier qualified_name({table_name, column.name});

    if (has_where_prewhere_or_group_by && !canOptimizeWithWherePrewhereOrGroupBy(function_node.getFunctionName())) return;
...
class FunctionToSubcolumnsVisitorSecondPass : public InDepthQueryTreeVisitorWithContext<FunctionToSubcolumnsVisitorSecondPass> {
 ...
 public:
  ...
  void enterImpl(QueryTreeNodePtr &node) const {
    ...
    if (!function_node || !first_argument_column_node || !table_node) return;

    auto column = first_argument_column_node->getColumn();
    auto table_name = table_node->getStorage()->getStorageID().getFullTableName();

    Identifier qualified_name({table_name, column.name});
    if (!identifiers_to_optimize.contains(qualified_name)) return;
    ...
    if (transformer_it != node_transformers.end() &&
      ...
      if (!result_type->equals(*node->getResultType())) node = buildCastFunction(node, result_type, getContext());
    }
  }
};
 ...
 public:
  ...
  void enterImpl(const QueryTreeNodePtr &node) {
    ...
    if (const auto *table_node = node->as<TableNode>()) {
      if (need_skip_subtree) outer_joined_tables.insert(table_node);
      return;
    }
  }

  void leaveImpl(const QueryTreeNodePtr &node) {
    if (join_nodes_stack.empty()) return;

    const auto *current_join_node = join_nodes_stack.top();

    /// Leaving the left (or right) subtree of a LEFT (or RIGHT) JOIN.
    if (node.get() == current_join_node->getRightTableExpression().get() || node.get() == current_join_node->getLeftTableExpression().get())
      need_skip_subtree = false;

    /// Leaving a JOIN node.
    if (node.get() == current_join_node) join_nodes_stack.pop();
  }
...
void FunctionToSubcolumnsPass::run(QueryTreeNodePtr &query_tree_node, ContextPtr context) {
  ...
  if (identifiers_to_optimize.empty()) return;
  ...
  FunctionToSubcolumnsVisitorSecondPass second_visitor(std::move(context), std::move(identifiers_to_optimize),
                                                       std::move(outer_join_visitor.outer_joined_tables));
