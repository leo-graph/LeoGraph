...
class NormalizeCountVariantsVisitor : public InDepthQueryTreeVisitorWithContext<NormalizeCountVariantsVisitor> {
 public:
  ...
  void enterImpl(QueryTreeNodePtr& node) {
    if (!getSettings()[Setting::optimize_normalize_count_variants]) return;
    ...
    if (function_node->getArguments().getNodes().size() != 1) return;
    ...
    if (!function_node->getResultType()->equals(DataTypeUInt64())) return;
