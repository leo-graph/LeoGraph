...
std::unordered_set<size_t> convertUsedColumnNamesToUsedProjectionIndexes(const QueryTreeNodePtr &query_or_union_node,
                                                                         const std::unordered_set<std::string> &used_column_names) {
  ...
  for (size_t i = 0; i < projection_columns_size; ++i) {
    ...
    if (used_column_names.contains(projection_column.name)) result.insert(i);
  }
...
void updateUsedProjectionIndexes(const QueryTreeNodePtr &query_or_union_node, std::unordered_set<size_t> &used_projection_columns_indexes) {
  if (auto *union_node = query_or_union_node->as<UnionNode>()) {
    ...
    bool is_distinct = union_node_mode == SelectUnionMode::UNION_DISTINCT || union_node_mode == SelectUnionMode::INTERSECT_DISTINCT ||
    ...
    if (is_distinct) {
      ...
      for (size_t i = 0; i < union_projection_columns_size; ++i) used_projection_columns_indexes.insert(i);

      return;
    }

    for (auto &query_node : union_node->getQueries().getNodes()) updateUsedProjectionIndexes(query_node, used_projection_columns_indexes);
    return;
  }
  ...
  while (!nodes_to_visit.empty()) {
    ...
    if (auto *query_node = node_to_visit->as<QueryNode>()) {
      for (const auto &table_expression : extractTableExpressions(query_node->getJoinTree()))
        if (isQueryOrUnionNode(table_expression)) node_to_used_columns.emplace(table_expression, std::unordered_set<std::string>());
    }

    /// Collect information about what columns are used in the query.
    traverseQueryTree(
        node_to_visit,
        [&subqueries_nodes_to_visit, &node_to_used_columns](const QueryTreeNodePtr & /*parent*/, const QueryTreeNodePtr &child) {
          if (isQueryOrUnionNode(child)) {
            ...
            const auto &correlated_columns =
                query_node != nullptr ? query_node->getCorrelatedColumns() : union_node->getCorrelatedColumns();
    ...
    for (auto &[query_or_union_node, used_columns] : node_to_used_columns) {
      ...
      if (auto *query_node = query_or_union_node->as<QueryNode>()) {
        if (query_node->isDistinct()) continue;
      }
      ...
      if (used_projection_indexes.empty()) used_projection_indexes.insert(0);
    ...
    for (const auto &subquery_node_to_visit : subqueries_nodes_to_visit) nodes_to_visit.push_back(subquery_node_to_visit);
  }
}
