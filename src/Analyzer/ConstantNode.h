...
class ConstantNode final : public IQueryTreeNode {
 public:
  ...
  /// Get constant value
  const ColumnPtr &getColumn() const { return constant_value.getColumn(); }
  ...
  bool hasSourceExpression() const { return source_expression != nullptr; }

  /// Get source expression
  const QueryTreeNodePtr &getSourceExpression() const { return source_expression; }

  /// Get source expression
  QueryTreeNodePtr &getSourceExpression() { return source_expression; }

  QueryTreeNodeType getNodeType() const override { return QueryTreeNodeType::CONSTANT; }

  DataTypePtr getResultType() const override { return constant_value.getType(); }
  ...
  void setMaskId(size_t id = std::numeric_limits<decltype(mask_id)>::max()) { mask_id = id; }

  void convertToNullable() override;

  void dumpTreeImpl(WriteBuffer &buffer, FormatState &format_state, size_t indent) const override;

  String getValueName(const IColumn::Options &options) const { return constant_value.getValueName(options); }
