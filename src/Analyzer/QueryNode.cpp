...
QueryNode::QueryNode(ContextMutablePtr context_, SettingsChanges settings_changes_)
    : IQueryTreeNode(children_size), context(std::move(context_)), settings_changes(std::move(settings_changes_)) {
...
QueryNode::QueryNode(ContextMutablePtr context_) : QueryNode(std::move(context_), {} /*settings_changes*/) {}
  ...
  if (!this->projection_aliases_to_override.empty()) {
    if (this->projection_aliases_to_override.size() != projection_columns_value.size())
      throw Exception(ErrorCodes::BAD_ARGUMENTS,
                      ...
                      "Expected {}, got {}",
                      projection_columns_value.size(), this->projection_aliases_to_override.size());

    for (size_t i = 0; i < projection_columns_value.size(); ++i) projection_columns_value[i].name = this->projection_aliases_to_override[i];
  }
...
void QueryNode::removeUnusedProjectionColumns(const std::unordered_set<size_t> &used_projection_columns_indexes) {
  ...
  for (size_t i = 0; i < projection_columns_size; ++i) {
    if (!used_projection_columns_indexes.contains(i)) continue;
  ...
  if (hasInterpolate()) {
    ...
    for (const auto &projection : projection_columns) used_projection_columns.insert(projection.name);

    auto &interpolate_node = getInterpolate();
    auto &interpolate_list_nodes = interpolate_node->as<ListNode &>().getNodes();
    std::erase_if(interpolate_list_nodes, [&used_projection_columns](const QueryTreeNodePtr &interpolate) {
      return !used_projection_columns.contains(interpolate->as<InterpolateNode &>().getExpressionName());
    });

    if (interpolate_list_nodes.empty()) interpolate_node = nullptr;
  }
}
...
void QueryNode::addCorrelatedColumn(const QueryTreeNodePtr &correlated_column) {
  ...
  for (const auto &column : correlated_columns) {
    if (column->isEqual(*correlated_column)) return;
  }
...
void QueryNode::dumpTreeImpl(WriteBuffer &buffer, FormatState &format_state, size_t indent) const {
  ...
  if (is_subquery) buffer << ", is_subquery: " << is_subquery;

  if (is_cte) buffer << ", is_cte: " << is_cte;

  if (is_recursive_with) buffer << ", is_recursive_with: " << is_recursive_with;

  if (is_distinct) buffer << ", is_distinct: " << is_distinct;

  if (is_limit_with_ties) buffer << ", is_limit_with_ties: " << is_limit_with_ties;

  if (is_group_by_with_totals) buffer << ", is_group_by_with_totals: " << is_group_by_with_totals;

  if (is_group_by_all) buffer << ", is_group_by_all: " << is_group_by_all;

  if (is_order_by_all) buffer << ", is_order_by_all: " << is_order_by_all;

  if (is_limit_by_all) buffer << ", is_limit_by_all: " << is_limit_by_all;
  ...
  if (!group_by_type.empty()) buffer << ", group_by_type: " << group_by_type;

  if (!cte_name.empty()) buffer << ", cte_name: " << cte_name;
  ...
  if (!projection_columns.empty()) {
    ...
    for (size_t i = 0; i < projection_columns_size; ++i) {
      ...
      if (i + 1 != projection_columns_size) buffer << '\n';
    }
  }
  ...
  if (hasSettingsChanges()) {
    ...
    for (const auto &change : settings_changes) buffer << fmt::format(" {}={}", change.name, fieldToString(change.value));
  }
}
...
bool QueryNode::isEqualImpl(const IQueryTreeNode &rhs, CompareOptions options) const {
  ...
  return is_subquery == rhs_typed.is_subquery && (options.ignore_cte || (is_cte == rhs_typed.is_cte && cte_name == rhs_typed.cte_name)) &&
         is_recursive_with == rhs_typed.is_recursive_with && is_distinct == rhs_typed.is_distinct &&
         is_limit_with_ties == rhs_typed.is_limit_with_ties && is_group_by_with_totals == rhs_typed.is_group_by_with_totals &&
         is_group_by_with_rollup == rhs_typed.is_group_by_with_rollup && is_group_by_with_cube == rhs_typed.is_group_by_with_cube &&
         is_group_by_with_grouping_sets == rhs_typed.is_group_by_with_grouping_sets && is_group_by_all == rhs_typed.is_group_by_all &&
         is_order_by_all == rhs_typed.is_order_by_all && is_limit_by_all == rhs_typed.is_limit_by_all &&
         projection_columns == rhs_typed.projection_columns && settings_changes == rhs_typed.settings_changes;
}
...
ASTPtr QueryNode::toASTImpl(const ConvertToASTOptions &options) const {
  ...
  if (hasWith()) {
    ...
    for (const auto &with_node : with) {
      ...
      if (with_node_ast_subquery) with_node_ast_subquery->cte_name = "";
  ...
  if (!projection_columns.empty()) {
    for (size_t i = 0; i < projection_expression_list_ast_children_size; ++i) {
      ...
      if (ast_with_alias) ast_with_alias->setAlias(projection_columns[i].name);
    }
  }
  ...
  if (getPrewhere()) select_query->setExpression(ASTSelectQuery::Expression::PREWHERE, getPrewhere()->toAST(options));

  if (getWhere()) select_query->setExpression(ASTSelectQuery::Expression::WHERE, getWhere()->toAST(options));

  if (!is_group_by_all && hasGroupBy()) select_query->setExpression(ASTSelectQuery::Expression::GROUP_BY, getGroupBy().toAST(options));

  if (hasHaving()) select_query->setExpression(ASTSelectQuery::Expression::HAVING, getHaving()->toAST(options));

  if (hasWindow()) select_query->setExpression(ASTSelectQuery::Expression::WINDOW, getWindow().toAST(options));

  if (hasQualify()) select_query->setExpression(ASTSelectQuery::Expression::QUALIFY, getQualify()->toAST(options));

  if (hasOrderBy()) select_query->setExpression(ASTSelectQuery::Expression::ORDER_BY, getOrderBy().toAST(options));

  if (hasInterpolate()) select_query->setExpression(ASTSelectQuery::Expression::INTERPOLATE, getInterpolate()->toAST(options));

  if (hasLimitByLimit()) select_query->setExpression(ASTSelectQuery::Expression::LIMIT_BY_LENGTH, getLimitByLimit()->toAST(options));

  if (hasLimitByOffset()) select_query->setExpression(ASTSelectQuery::Expression::LIMIT_BY_OFFSET, getLimitByOffset()->toAST(options));

  if (hasLimitBy()) select_query->setExpression(ASTSelectQuery::Expression::LIMIT_BY, getLimitBy().toAST(options));

  if (hasLimit()) select_query->setExpression(ASTSelectQuery::Expression::LIMIT_LENGTH, getLimit()->toAST(options));

  if (hasOffset()) select_query->setExpression(ASTSelectQuery::Expression::LIMIT_OFFSET, getOffset()->toAST(options));
  ...
  if (is_subquery) {
    ...
    if (options.set_subquery_cte_name) subquery->cte_name = cte_name;
    return subquery;
  }
