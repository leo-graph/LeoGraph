...
void validateFilter(const QueryTreeNodePtr &filter_node, std::string_view exception_place_message, const QueryTreeNodePtr &query_node) {
  ...
  try {
    ...
    if (e.code() != ErrorCodes::UNSUPPORTED_METHOD) e.rethrow();
  }

  if (!filter_node_result_type)
    throw Exception(ErrorCodes::UNEXPECTED_EXPRESSION, "Unexpected expression '{}' in filter in {}. In query {}",
                    filter_node->formatASTForErrorMessage(), exception_place_message, query_node->formatASTForErrorMessage());

  if (!filter_node_result_type->canBeUsedInBooleanContext())
    throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_COLUMN_FOR_FILTER, "Invalid type for filter in {}: {}. In query {}",
                    exception_place_message, filter_node_result_type->getName(), query_node->formatASTForErrorMessage());
}
...
void validateFilters(const QueryTreeNodePtr &query_node) {
  ...
  if (query_node_typed.hasPrewhere()) {
    ...
    assertNoFunctionNodes(query_node_typed.getPrewhere(), "arrayJoin", ErrorCodes::ILLEGAL_PREWHERE, "ARRAY JOIN", "in PREWHERE");
  }

  if (query_node_typed.hasWhere()) validateFilter(query_node_typed.getWhere(), "WHERE", query_node);

  if (query_node_typed.hasHaving()) validateFilter(query_node_typed.getHaving(), "HAVING", query_node);

  if (query_node_typed.hasQualify()) validateFilter(query_node_typed.getQualify(), "QUALIFY", query_node);
}
...
bool areColumnSourcesEqual(const QueryTreeNodePtr &lhs, const QueryTreeNodePtr &rhs) {
  ...
  while (!nodes_to_process.empty()) {
    ...
    if (lhs_node->getNodeType() != rhs_node->getNodeType()) return false;
    ...
    if (lhs_node->getNodeType() == QueryTreeNodeType::COLUMN) {
      ...
      if (!lhs_column_node->getColumnSource()->isEqual(*rhs_column_node->getColumnSource())) return false;
    }
    ...
    for (size_t i = 0; i < lhs_children.size(); ++i) {
      ...
      if (!lhs_child && !rhs_child) continue;
      if (lhs_child && !rhs_child) return false;
      if (!lhs_child && rhs_child) return false;
...
bool compareGroupByKeys(const QueryTreeNodePtr &node, const QueryTreeNodePtr &group_by_key_node) {
  if (node->isEqual(*group_by_key_node, {.compare_aliases = false})) {
     ...
     */
    if (areColumnSourcesEqual(node, group_by_key_node)) return true;
  }
  return false;
}
...
class ValidateGroupByColumnsVisitor : public ConstInDepthQueryTreeVisitor<ValidateGroupByColumnsVisitor> {
 public:
  explicit ValidateGroupByColumnsVisitor(const QueryTreeNodes &group_by_keys_nodes_, const QueryTreeNodePtr &query_node_)
      : group_by_keys_nodes(group_by_keys_nodes_), query_node(query_node_) {}

  void visitImpl(const QueryTreeNodePtr &node) {
    auto query_tree_node_type = node->getNodeType();
    if (query_tree_node_type == QueryTreeNodeType::CONSTANT || query_tree_node_type == QueryTreeNodeType::SORT ||
        query_tree_node_type == QueryTreeNodeType::INTERPOLATE)
      return;

    if (nodeIsAggregateFunctionOrInGroupByKeys(node)) return;
    ...
    if (function_node && function_node->getFunctionName() == "grouping") {
      ...
      for (auto &grouping_function_arguments_node : grouping_function_arguments_nodes) {
        ...
        if (!found_argument_in_group_by_keys)
          throw Exception(ErrorCodes::BAD_ARGUMENTS, "GROUPING function argument {} is not in GROUP BY keys. In query {}",
                          grouping_function_arguments_node->formatASTForErrorMessage(), query_node->formatASTForErrorMessage());
      }
    ...
    if (column_node_source->getNodeType() == QueryTreeNodeType::LAMBDA) return;
    if (column_node_source->getNodeType() == QueryTreeNodeType::INTERPOLATE) return;

    throw Exception(ErrorCodes::NOT_AN_AGGREGATE, "Column '{}' is not under aggregate function and not in GROUP BY keys. In query {}",
                    column_node->formatConvertedASTForErrorMessage(), query_node->formatASTForErrorMessage());
  }

  bool needChildVisit(const QueryTreeNodePtr &parent_node, const QueryTreeNodePtr &child_node) {
    if (nodeIsAggregateFunctionOrInGroupByKeys(parent_node)) return false;
 ...
 private:
  bool nodeIsAggregateFunctionOrInGroupByKeys(const QueryTreeNodePtr &node) const {
    if (auto *function_node = node->as<FunctionNode>())
      if (function_node->isAggregateFunction()) return true;

    for (const auto &group_by_key_node : group_by_keys_nodes) {
      if (compareGroupByKeys(node, group_by_key_node)) return true;
    }
...
void validateAggregates(const QueryTreeNodePtr &query_node, AggregatesValidationParams params) {
  ...
  if (query_node_typed.hasHaving()) assertNoWindowFunctionNodes(query_node_typed.getHaving(), "in HAVING");

  if (query_node_typed.hasWindow()) assertNoWindowFunctionNodes(query_node_typed.getWindowNode(), "in WINDOW");
  ...
  for (auto &window_function_node : window_function_nodes) {
    ...
    if (query_node_typed.hasWindow()) assertNoWindowFunctionNodes(window_function_node_typed.getWindowNode(), "inside window definition");
  }
  ...
  for (const auto &node : query_node_typed.getGroupBy().getNodes()) {
    if (query_node_typed.isGroupByWithGroupingSets()) {
      ...
      for (auto &grouping_set_key : grouping_set_keys.getNodes()) {
        if (grouping_set_key->as<ConstantNode>()) continue;

        group_by_keys_nodes.push_back(grouping_set_key->clone());
        if (params.group_by_use_nulls) group_by_keys_nodes.back()->convertToNullable();
      }
    } else {
      if (node->as<ConstantNode>()) continue;
  ...
  if (query_node_typed.getGroupBy().getNodes().empty()) {
    if (query_node_typed.hasHaving()) assertNoGroupingFunctionNodes(query_node_typed.getHaving(), "in HAVING without GROUP BY");

    if (query_node_typed.hasOrderBy()) assertNoGroupingFunctionNodes(query_node_typed.getOrderByNode(), "in ORDER BY without GROUP BY");
  ...
  if (has_aggregation) {
    ...
    if (query_node_typed.hasHaving()) validate_group_by_columns_visitor.visit(query_node_typed.getHaving());

    if (query_node_typed.hasQualify()) validate_group_by_columns_visitor.visit(query_node_typed.getQualify());

    if (query_node_typed.hasOrderBy()) validate_group_by_columns_visitor.visit(query_node_typed.getOrderByNode());

    if (query_node_typed.hasInterpolate()) validate_group_by_columns_visitor.visit(query_node_typed.getInterpolate());

    if (query_node_typed.hasLimitBy()) validate_group_by_columns_visitor.visit(query_node_typed.getLimitByNode());

    validate_group_by_columns_visitor.visit(query_node_typed.getProjectionNode());
  }

  bool aggregation_with_rollup_or_cube_or_grouping_sets =
      query_node_typed.isGroupByWithRollup() || query_node_typed.isGroupByWithCube() || query_node_typed.isGroupByWithGroupingSets();
...
class ValidateFunctionNodesVisitor : public ConstInDepthQueryTreeVisitor<ValidateFunctionNodesVisitor> {
 public:
  explicit ValidateFunctionNodesVisitor(std::string_view function_name_, int exception_code_, std::string_view exception_function_name_,
  ...
  void visitImpl(const QueryTreeNodePtr &node) {
    ...
    if (function_node && function_node->getFunctionName() == function_name)
      throw Exception(exception_code, "{} function {} is found {} in query", exception_function_name,
                      function_node->formatASTForErrorMessage(), exception_place_message);
  }
...
void assertNoFunctionNodes(const QueryTreeNodePtr &node, std::string_view function_name, int exception_code,
                           std::string_view exception_function_name, std::string_view exception_place_message) {
  ValidateFunctionNodesVisitor visitor(function_name, exception_code, exception_function_name, exception_place_message);
  visitor.visit(node);
}

void validateTreeSize(const QueryTreeNodePtr &node, size_t max_size, std::unordered_map<QueryTreeNodePtr, size_t> &node_to_tree_size) {
  size_t tree_size = 0;
  ...
  while (!nodes_to_process.empty()) {
    ...
    if (processed_children) {
      ...
      for (const auto &node_to_process_child : node_to_process->getChildren()) {
        if (!node_to_process_child) continue;

        subtree_size += node_to_tree_size[node_to_process_child];
      }

      auto *constant_node = node_to_process->as<ConstantNode>();
      if (constant_node && constant_node->hasSourceExpression()) subtree_size += node_to_tree_size[constant_node->getSourceExpression()];
    ...
    auto *constant_node = node_to_process->as<ConstantNode>();
    if (constant_node && constant_node->hasSourceExpression()) nodes_to_process.emplace_back(constant_node->getSourceExpression(), false);
  }

  if (tree_size > max_size) throw Exception(ErrorCodes::BAD_ARGUMENTS, "Query tree is too big. Maximum: {}", max_size);
}
  ...
  while (!nodes_to_process.empty()) {
    ...
    switch (current_node->getNodeType()) {
      case QueryTreeNodeType::QUERY: {
        ...
        if (query_node.isCorrelated()) has_correlated_subquery = true;
        break;
      }
      case QueryTreeNodeType::UNION: {
        auto &union_node = current_node->as<UnionNode &>();
        if (union_node.isCorrelated()) has_correlated_subquery = true;
        break;
      }
      case QueryTreeNodeType::TABLE: {
        auto &table_node = current_node->as<TableNode &>();
        const auto &storage = table_node.getStorage();
        if (storage && storage->isRemote()) has_remote = true;
        break;
      }
    ...
    if (has_remote && has_correlated_subquery)
      throw Exception(ErrorCodes::NOT_IMPLEMENTED, "Correlated subqueries are not supported with remote tables. In query {}",
                      node->formatASTForErrorMessage());

    for (const auto &child : current_node->getChildren()) {
      if (child) nodes_to_process.push_back(child);
    }
  }
}
  ...
  while (!nodes_to_process.empty()) {
    ...
    switch (node_type) {
        ...
        for (const auto &column : correlated_columns) {
          if (!correlated_columns_set.contains(std::static_pointer_cast<ColumnNode>(column)))
            throw Exception(ErrorCodes::NOT_IMPLEMENTED,
                            ...
                            column->formatASTForErrorMessage(), node->formatASTForErrorMessage());
        }
        break;
      }
      case QueryTreeNodeType::UNION: {
      ...
      case QueryTreeNodeType::CROSS_JOIN: {
        ...
        for (const auto &expr : std::ranges::reverse_view(join_node.getTableExpressions())) nodes_to_process.push_back(expr);
        break;
      }
