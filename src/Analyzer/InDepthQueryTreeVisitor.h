...
class InDepthQueryTreeVisitor {
 public:
  ...
  bool shouldTraverseTopToBottom() const { return true; }

  /// Return true if visitor should visit child, false otherwise
  bool needChildVisit(VisitQueryTreeNodeType &parent [[maybe_unused]], VisitQueryTreeNodeType &child [[maybe_unused]]) { return true; }

  void visit(VisitQueryTreeNodeType &query_tree_node) {
    bool traverse_top_to_bottom = getDerived().shouldTraverseTopToBottom();
    if (!traverse_top_to_bottom) visitChildren(query_tree_node);
 ...
 private:
  ...
  void visitChildren(VisitQueryTreeNodeType &expression) {
    for (auto &child : expression->getChildren()) {
      ...
      if (need_visit_child) visit(child);
    }
  }
};
...
class InDepthQueryTreeVisitorWithContext {
 public:
  ...
  explicit InDepthQueryTreeVisitorWithContext(ContextPtr context, size_t initial_subquery_depth = 0)
      : current_context(std::move(context)), subquery_depth(initial_subquery_depth) {}

  /// Return true if visitor should visit child, false otherwise
  bool needChildVisit(VisitQueryTreeNodeType &parent [[maybe_unused]], VisitQueryTreeNodeType &child [[maybe_unused]]) { return true; }

  const ContextPtr &getContext() const { return current_context; }

  const Settings &getSettings() const { return current_context->getSettingsRef(); }

  size_t getSubqueryDepth() const { return subquery_depth; }

  void visit(VisitQueryTreeNodeType &query_tree_node) {
    auto current_scope_context_ptr = current_context;
    SCOPE_EXIT(current_context = std::move(current_scope_context_ptr); --subquery_depth;);
 ...
 private:
  ...
  void visitChildIfNeeded(VisitQueryTreeNodeType &parent, VisitQueryTreeNodeType &child) {
    bool need_visit_child = getDerived().needChildVisit(parent, child);
    if (!need_visit_child) return;
    ...
    if (auto *table_function_node = parent->as<TableFunctionNode>()) {
      ...
      for (auto &argument : table_function_node->getArguments()) {
        if (std::find(unresolved_indexes.begin(), unresolved_indexes.end(), index) == unresolved_indexes.end()) visit(argument);
        ++index;
      }
      return;
    }
  ...
  void visitChildren(VisitQueryTreeNodeType &expression) {
    for (auto &child : expression->getChildren()) {
      if (child) visitChildIfNeeded(expression, child);
    }
  }
