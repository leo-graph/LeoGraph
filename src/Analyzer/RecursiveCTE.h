 ...
 *
 * During query planning if UNION node contains recursive CTE table, we add ReadFromRecursiveCTEStep to query plan. That step is responsible
 * for whole recursive CTE query execution.
 *
...
class RecursiveCTETable {
 public:
  RecursiveCTETable(TemporaryTableHolderPtr holder_, StoragePtr storage_, NamesAndTypes columns_);
