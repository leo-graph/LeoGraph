...
ColumnNode::ColumnNode(NameAndTypePair column_, QueryTreeNodePtr expression_node_, QueryTreeNodeWeakPtr column_source_)
    : IQueryTreeNode(children_size, weak_pointers_size), column(std::move(column_)) {
...
QueryTreeNodePtr ColumnNode::getColumnSource() const {
  ...
  if (!lock)
    throw Exception(ErrorCodes::LOGICAL_ERROR, "Column {} {} query tree node does not have valid source node", column.name,
                    column.type->getName());

  return lock;
}

QueryTreeNodePtr ColumnNode::getColumnSourceOrNull() const { return getSourceWeakPointer().lock(); }
...
void ColumnNode::dumpTreeImpl(WriteBuffer &buffer, FormatState &state, size_t indent) const {
  ...
  if (column_source_ptr) buffer << ", source_id: " << state.getNodeId(column_source_ptr.get());
...
bool ColumnNode::isEqualImpl(const IQueryTreeNode &rhs, CompareOptions compare_options) const {
  ...
  if (column.name != rhs_typed.column.name) return false;
...
void ColumnNode::updateTreeHashImpl(HashState &hash_state, CompareOptions compare_options) const {
  ...
  if (compare_options.compare_types) column.type->updateHash(hash_state);
}

QueryTreeNodePtr ColumnNode::cloneImpl() const { return std::make_shared<ColumnNode>(column, getSourceWeakPointer()); }
...
ASTPtr ColumnNode::toASTImpl(const ConvertToASTOptions &options) const {
  ...
  if (column_source && options.fully_qualified_identifiers) {
    ...
    if (node_type == QueryTreeNodeType::TABLE || node_type == QueryTreeNodeType::TABLE_FUNCTION || node_type == QueryTreeNodeType::QUERY ||
