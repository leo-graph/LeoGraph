...
QueryTreeNodePtr IdentifierResolver::convertJoinedColumnTypeToNullIfNeeded(const QueryTreeNodePtr &resolved_identifier,
                                                                           DataTypePtr result_type, const JoinKind &join_kind,
                                                                           std::optional<JoinTableSide> resolved_side,
                                                                           IdentifierResolveScope &scope) {
  bool need_nullable = scope.join_use_nulls && JoinCommon::canBecomeNullable(resolved_identifier->getResultType()) &&
                       (isFull(join_kind) || (isLeft(join_kind) && resolved_side == JoinTableSide::Right) ||
                        (isRight(join_kind) && resolved_side == JoinTableSide::Left));

  if (resolved_identifier->getNodeType() == QueryTreeNodeType::FUNCTION) {
    if (!need_nullable) return resolved_identifier;
    ...
    for (auto &arg : arguments) {
      if (arg->getNodeType() == QueryTreeNodeType::COLUMN) {
        auto nullable_arg = convertJoinedColumnTypeToNullIfNeeded(arg, arg->getResultType(), join_kind, resolved_side, scope);
    ...
    if (any_changed) {
      ...
      if (function_resolver) function_node.resolveAsFunction(function_resolver->build(function_node.getArgumentColumns()));
    }

    return function_clone;
  }

  if (resolved_identifier->getNodeType() != QueryTreeNodeType::COLUMN) return resolved_identifier;

  if (need_nullable) result_type = makeNullableOrLowCardinalityNullable(result_type);
...
static FunctionNodePtr wrapExpressionNodeInFunctionWithSecondConstantStringArgument(QueryTreeNodePtr expression, std::string function_name,
...
static FunctionNodePtr wrapExpressionNodeInTupleElement(QueryTreeNodePtr expression, std::string subcolumn_name,
                                                        const ContextPtr &context) {
 ...
 */
QueryTreeNodePtr IdentifierResolver::wrapExpressionNodeInTupleElement(QueryTreeNodePtr expression_node, IdentifierView nested_path,
                                                                      const ContextPtr &context) {
...
std::shared_ptr<TableNode> IdentifierResolver::tryResolveTableIdentifier(const Identifier &table_identifier, const ContextPtr &context) {
  ...
  if (parts_size < 1 || parts_size > 2)
    throw Exception(ErrorCodes::INVALID_IDENTIFIER, "Expected table identifier to contain 1 or 2 parts. Actual '{}'",
  ...
  if (!storage && storage_id.hasUUID()) {
    // If `storage_id` has UUID, it is possible that the UUID is removed from `DatabaseCatalog` after
    // `context->resolveStorageID(storage_id)` We try to get the table with the database name and the table name.
    auto database = DatabaseCatalog::instance().tryGetDatabase(storage_id.getDatabaseName());
    if (database) storage = database->tryGetTable(table_name, context);
  }
  if (!storage) return {};
  ...
  if (is_temporary_table) result->setTemporaryTableName(table_name);

  return result;
}

IdentifierResolveResult IdentifierResolver::tryResolveTableIdentifierFromDatabaseCatalog(const Identifier &table_identifier,
                                                                                         const ContextPtr &context) {
...
QueryTreeNodePtr IdentifierResolver::tryResolveIdentifierFromCompoundExpression(const Identifier &expression_identifier,
                                                                                ...
                                                                                IdentifierResolveScope &scope, bool can_be_not_found) {
  Identifier compound_expression_identifier;
  for (size_t i = 0; i < identifier_bind_size; ++i) compound_expression_identifier.push_back(expression_identifier[i]);
  ...
  if (!expression_type->hasSubcolumn(nested_path.getFullName())) {
    if (can_be_not_found) return {};

    std::unordered_set<Identifier> valid_identifiers;
    TypoCorrection::collectCompoundExpressionValidIdentifiers(expression_identifier, expression_type, compound_expression_identifier,
                                                              valid_identifiers);
    ...
    throw Exception(ErrorCodes::UNKNOWN_IDENTIFIER, "Identifier {} nested path {} cannot be resolved from type {}{}. In scope {}{}",
                    expression_identifier, nested_path, expression_type->getName(), compound_expression_from_error_message,
                    scope.scope_node->formatASTForErrorMessage(), getHintsErrorMessageSuffix(hints));
  }
 ...
 */
IdentifierResolveResult IdentifierResolver::tryResolveIdentifierFromExpressionArguments(const IdentifierLookup &identifier_lookup,
                                                                                        IdentifierResolveScope &scope) {
  ...
  if (!resolve_full_identifier) {
    ...
    if (it == scope.expression_argument_name_to_node.end()) return {};
  }

  auto node_type = it->second->getNodeType();
  if (identifier_lookup.isExpressionLookup() && !isExpressionNodeType(node_type)) return {};
  if (identifier_lookup.isTableExpressionLookup() && !isTableExpressionNodeType(node_type)) return {};
  if (identifier_lookup.isFunctionLookup() && !isFunctionExpressionNodeType(node_type)) return {};

  if (!resolve_full_identifier && identifier_lookup.identifier.isCompound() && identifier_lookup.isExpressionLookup()) {
    if (auto resolved_identifier =
            tryResolveIdentifierFromCompoundExpression(identifier_lookup.identifier, 1 /*identifier_bind_size*/, it->second, {}, scope))
...
bool IdentifierResolver::tryBindIdentifierToJoinUsingColumn(const IdentifierLookup &identifier_lookup,
  ...
  for (const auto *join_using : scope.join_using_columns) {
    for (const auto &[using_column_name, _] : *join_using) {
      ...
      if (identifier_lookup.identifier.getFullName() == using_column_name) return true;
    }
  }
 ...
 */
QueryTreeNodePtr IdentifierResolver::tryResolveIdentifierFromTableColumns(const IdentifierLookup &identifier_lookup,
                                                                          IdentifierResolveScope &scope) {
  if (!scope.table_expression_data_for_alias_resolution || !identifier_lookup.isExpressionLookup()) return {};
  ...
  if (it != scope.table_expression_data_for_alias_resolution->column_name_to_column_node.end()) return it->second;
  ...
  if (auto subcolumn_info = scope.table_expression_data_for_alias_resolution->tryGetSubcolumnInfo(identifier_full_name)) {
    ...
    if (scope.table_expression_data_for_alias_resolution->supports_subcolumns && !subcolumn_info->column_node->hasExpression())
      return std::make_shared<ColumnNode>(NameAndTypePair{identifier_full_name, subcolumn_info->subcolumn_type},
                                          subcolumn_info->column_node->getColumnSource());
...
bool IdentifierResolver::tryBindIdentifierToTableExpression(const IdentifierLookup &identifier_lookup,
  ...
  if (table_expression_node_type != QueryTreeNodeType::TABLE && table_expression_node_type != QueryTreeNodeType::TABLE_FUNCTION &&
      table_expression_node_type != QueryTreeNodeType::QUERY && table_expression_node_type != QueryTreeNodeType::UNION)
  ...
  if (identifier_lookup.isTableExpressionLookup()) {
    ...
    if (parts_size != 1 && parts_size != 2)
      throw Exception(ErrorCodes::INVALID_IDENTIFIER,
                      ...
                      identifier_lookup.identifier.getFullName(), table_expression_node->formatASTForErrorMessage());

    if (parts_size == 1 && path_start == table_name) return true;
    if (parts_size == 2 && path_start == database_name && identifier[1] == table_name) return true;
    return false;
  }

  if (table_expression_data.hasFullIdentifierName(IdentifierView(identifier)) ||
      table_expression_data.canBindIdentifier(IdentifierView(identifier)))
    return true;

  if (identifier.getPartsSize() == 1) return false;

  if ((!table_name.empty() && path_start == table_name) ||
      (table_expression_node->hasAlias() && path_start == table_expression_node->getAlias()))
    return true;

  if (identifier.getPartsSize() == 2) return false;

  if (!database_name.empty() && path_start == database_name && identifier[1] == table_name) return true;
...
bool IdentifierResolver::tryBindIdentifierToTableExpressions(const IdentifierLookup &identifier_lookup,
  ...
  for (const auto &[table_expression_node, _] : scope.table_expression_node_to_data) {
    if (table_expression_node.get() == table_expression_node_to_ignore.get()) continue;

    can_bind_identifier_to_table_expression = tryBindIdentifierToTableExpression(identifier_lookup, table_expression_node, scope);
    if (can_bind_identifier_to_table_expression) break;
  }

  return can_bind_identifier_to_table_expression;
}

bool IdentifierResolver::tryBindIdentifierToArrayJoinExpressions(const IdentifierLookup &identifier_lookup,
                                                                 const IdentifierResolveScope &scope) {
  bool result = false;
  ...
  for (const auto &table_expression : scope.registered_table_expression_nodes) {
    ...
    if (!array_join_node) continue;

    for (const auto &array_join_expression : array_join_node->getJoinExpressions()) {
      auto array_join_expression_alias = array_join_expression->getAlias();
      if (identifier_lookup.identifier.front() == array_join_expression_alias) return true;
    }
  }
...
IdentifierResolveResult IdentifierResolver::tryResolveIdentifierFromStorage(const IdentifierLookup &identifier_lookup,
                                                                            const QueryTreeNodePtr &table_expression_node,
  ...
  if (auto it = table_expression_data.column_name_to_column_node.find(identifier_full_name);
      it != table_expression_data.column_name_to_column_node.end()) {
  ...
  /// Check if it's a subcolumn
    ...
    if (auto subcolumn_info = table_expression_data.tryGetSubcolumnInfo(identifier_full_name)) {
      ...
      if (table_expression_data.supports_subcolumns && !subcolumn_info->column_node->hasExpression())
        result_expression = std::make_shared<ColumnNode>(NameAndTypePair{identifier_full_name, subcolumn_info->subcolumn_type},
                                                         subcolumn_info->column_node->getColumnSource());
      else
        result_expression =
            wrapExpressionNodeInSubcolumn(subcolumn_info->column_node, String(subcolumn_info->subcolumn_name), scope.context);
    }
  }
  ...
  if (!result_expression) {
    ...
    for (const auto &[column_name, _] : table_expression_data.column_names_and_types) {
      if (table_expression_data.subcolumn_names.contains(column_name)) continue;
      ...
      for (const auto &part : identifier_without_column_qualifier.getParts()) {
        if (suffix.empty() || part != suffix.front()) break;

        suffix.popFirst();
      }

      if (suffix.empty() || prefix_size + suffix.getPartsSize() != column_identifier.getPartsSize()) continue;

      /// Ignore compound identifiers because of the compatibility setting.
      if (!allow_compound && suffix.getPartsSize() > 1) continue;
      ...
      if (prefix_size != 1) continue;

      auto column_node_it = table_expression_data.column_name_to_column_node.find(column_name);
      if (column_node_it == table_expression_data.column_name_to_column_node.end()) continue;
    ...
    if (!nested_types.empty()) {
      ...
      auto nested_function_names_constant_node =
          std::make_shared<ConstantNode>(std::move(nested_names_array), std::move(nested_function_names_array_type));
      nested_function_node_arguments.push_back(std::move(nested_function_names_constant_node));
      nested_function_node_arguments.insert(nested_function_node_arguments.end(), nested_column_nodes.begin(), nested_column_nodes.end());
  ...
  if (!result_expression) {
    if (can_be_not_found) return {};
    std::unordered_set<Identifier> valid_identifiers;
    TypoCorrection::collectTableExpressionValidIdentifiers(identifier, table_expression_node, table_expression_data, valid_identifiers);

    auto hints = TypoCorrection::collectIdentifierTypoHints(identifier, valid_identifiers);

    throw Exception(ErrorCodes::UNKNOWN_IDENTIFIER, "Identifier '{}' cannot be resolved from {}. In scope {}{}", identifier.getFullName(),
                    table_expression_source, scope.scope_node->formatASTForErrorMessage(), getHintsErrorMessageSuffix(hints));
  }
  ...
  for (size_t i = 0; i < identifier_column_qualifier_parts; ++i) {
    ...
    if (qualified_identifier_with_removed_part.empty()) break;

    IdentifierLookup column_identifier_lookup = {qualified_identifier_with_removed_part, IdentifierLookupContext::EXPRESSION};
    if (tryBindIdentifierToAliases(column_identifier_lookup, scope)) break;
    ...
    if (tryBindIdentifierToJoinUsingColumn(column_identifier_lookup, scope)) break;
...
IdentifierResolveResult IdentifierResolver::tryResolveIdentifierFromTableExpression(const IdentifierLookup &identifier_lookup,
  ...
  /** If identifier first part binds to some column start or table has full identifier name. Then we can try to find whole identifier in
   * table.
   * 1. Try to bind identifier first part to column in table, if true get full identifier from table or throw exception.
   * 2. Try to bind identifier first part to table name or storage alias, if true remove first part and try to get full identifier from
   * table or throw exception. Storage alias works for subquery, table function as well.
   * 3. Try to bind identifier first parts to database name and table name, if true remove first two parts and try to get full identifier
   * from table or throw exception.
   */
  if (table_expression_data.hasFullIdentifierName(IdentifierView(identifier)))
    return tryResolveIdentifierFromStorage(identifier_lookup, table_expression_node, table_expression_data, scope,
                                           0 /*identifier_column_qualifier_parts*/);
  ...
  if (table_expression_data.canBindIdentifier(IdentifierView(identifier))) {
     ...
     * Initially, we will try to resolve t.t from `a` because `t.` is bound to `1 as t`. However, as it is not a nested column, we will need
     * to resolve it from the second table expression.
     */
    auto lookup_result = tryResolveIdentifierFromStorage(identifier_lookup, table_expression_node, table_expression_data, scope,
                                                         0 /*identifier_column_qualifier_parts*/, true /*can_be_not_found*/);
    if (lookup_result.resolved_identifier) return lookup_result;
  }
...
QueryTreeNodePtr checkIsMissedObjectJSONSubcolumn(const QueryTreeNodePtr &left_resolved_identifier,
  ...
  if (left_resolved_identifier && right_resolved_identifier && left_resolved_identifier->getNodeType() == QueryTreeNodeType::CONSTANT &&
    ...
    auto &left_resolved_column = left_resolved_identifier->as<ConstantNode &>();
    if (left_resolved_column.getValueStringRepresentation() == "NULL") return left_resolved_identifier;
  } else if (right_resolved_identifier && right_resolved_identifier->getNodeType() == QueryTreeNodeType::CONSTANT) {
    auto &right_resolved_column = right_resolved_identifier->as<ConstantNode &>();
    if (right_resolved_column.getValueStringRepresentation() == "NULL") return right_resolved_identifier;
  }
  return {};
}

static JoinTableSide choseSideForEqualIdenfifiersFromJoin(const ColumnNode &left_resolved_identifier_column,
  ...
  bool column_resolved_without_using_left_alias =
      !left_column_source_alias.empty() && right_column_source_alias.empty() && identifier_path_part != left_column_source_alias;

  if (column_resolved_using_right_alias || column_resolved_without_using_left_alias) return JoinTableSide::Right;
...
IdentifierResolveResult IdentifierResolver::tryResolveIdentifierFromCrossJoin(const IdentifierLookup &identifier_lookup,
  ...
  for (const auto &expr : from_cross_join_node.getTableExpressions()) {
    ...
    if (!identifier_lookup.isExpressionLookup())
      throw Exception(ErrorCodes::AMBIGUOUS_IDENTIFIER, "JOIN {} ambiguous identifier {}. In scope {}",
                      table_expression_node->formatASTForErrorMessage(), identifier_lookup.dump(),
    ...
    if (resolve_result.resolved_identifier->isEqual(*identifier.resolved_identifier,
                                                    IQueryTreeNode::CompareOptions{.compare_aliases = false})) {
      ...
      if (left_resolved_identifier_column && right_resolved_identifier_column) {
        auto resolved_side =
            choseSideForEqualIdenfifiersFromJoin(*left_resolved_identifier_column, *right_resolved_identifier_column, identifier_path_part);
        if (resolved_side == JoinTableSide::Right) resolve_result = identifier;
      }
    } else if (!prefer_left_table) {
      throw Exception(ErrorCodes::AMBIGUOUS_IDENTIFIER, "JOIN {} ambiguous identifier '{}'. In scope {}",
                      table_expression_node->formatASTForErrorMessage(), identifier_lookup.identifier.getFullName(),
                      scope.scope_node->formatASTForErrorMessage());
    }
  }
...
bool resolvedIdenfiersFromJoinAreEquals(const QueryTreeNodePtr &left_resolved_identifier, const QueryTreeNodePtr &right_resolved_identifier,
                                        const IdentifierResolveScope &scope) {
  auto left_original_node =
      ReplaceColumnsVisitor::findTransitiveReplacement(left_resolved_identifier, scope.join_columns_with_changed_types);
  ...
  auto right_original_node =
      ReplaceColumnsVisitor::findTransitiveReplacement(right_resolved_identifier, scope.join_columns_with_changed_types);
...
QueryTreeNodePtr createProjectionForUsing(const ColumnNode &using_column_node, JoinKind join_kind, IdentifierResolveScope &scope) {
  ...
  for (size_t i = 0; i < arguments.size(); ++i) {
    ...
    auto converted_argument = IdentifierResolver::convertJoinedColumnTypeToNullIfNeeded(arguments[i], using_column_node.getResultType(),
                                                                                        join_kind, resolved_side, scope);
    if (converted_argument) arguments[i] = converted_argument;
  }

  if (join_kind == JoinKind::Right) return arguments.back();

  if (join_kind != JoinKind::Full) arguments.pop_back();

  if (arguments.size() == 1) return arguments.front();
...
IdentifierResolveResult IdentifierResolver::tryResolveIdentifierFromJoin(const IdentifierLookup &identifier_lookup,
  ...
  auto try_resolve_identifier_from_join_tree_node = [&](const QueryTreeNodePtr &join_tree_node, bool may_be_override_by_using_column) {
    ...
    if (may_be_override_by_using_column && !join_using_column_name_to_column_node.empty()) scope.join_using_columns.pop_back();

    return std::move(res.resolved_identifier);
  };

  auto left_resolved_identifier =
      try_resolve_identifier_from_join_tree_node(from_join_node.getLeftTableExpression(), join_kind == JoinKind::Right);
  auto right_resolved_identifier =
      try_resolve_identifier_from_join_tree_node(from_join_node.getRightTableExpression(), join_kind != JoinKind::Right);
  ...
  if (!identifier_lookup.isExpressionLookup()) {
    ...
    if (!left_resolved_identifier && !right_resolved_identifier) return {};

    return {.resolved_identifier = left_resolved_identifier ? left_resolved_identifier : right_resolved_identifier,
    ...
    for (const auto &argument_node : function_argument_nodes) {
      if (argument_node->getNodeType() == QueryTreeNodeType::COLUMN) {
        ...
        if (join_using_column_name_to_column_node.contains(column_name))
          throw Exception(ErrorCodes::AMBIGUOUS_IDENTIFIER, "Cannot select subcolumn for identifier '{}' while joining using column '{}'",
                          identifier_lookup.identifier, column_name);
      } else if (argument_node->getNodeType() == QueryTreeNodeType::CONSTANT) {
        continue;
      } else {
        throw Exception(ErrorCodes::LOGICAL_ERROR, "Unexpected node type {} for argument node in {}", argument_node->getNodeType(),
  ...
  auto convert_resolved_result_type_if_needed = [](const QueryTreeNodePtr &resolved_identifier_candidate,
                                                   const std::unordered_map<std::string, ColumnNodePtr> &using_column_name_to_column_node,
                                                   QueryTreeNodePtr &resolve_result, IdentifierResolveScope &current_scope,
    ...
    if (using_column_node_it == using_column_name_to_column_node.end()) return;
    ...
    if (std::ranges::none_of(using_column_list.getNodes(), matches_using_column)) return;
  ...
  if (left_resolved_identifier && right_resolved_identifier) {
    ...
    if (left_resolved_identifier->getNodeType() == QueryTreeNodeType::COLUMN &&
      ...
      if (left_resolved_identifier->getNodeType() != QueryTreeNodeType::COLUMN) check_nested_column_not_in_using(left_resolved_identifier);
    ...
    if (left_resolved_identifier->getNodeType() != QueryTreeNodeType::COLUMN) {
      ...
      convert_resolved_result_type_if_needed(left_resolved_identifier, join_using_column_name_to_column_node, resolved_identifier, scope,
    ...
    if (right_resolved_identifier->getNodeType() != QueryTreeNodeType::COLUMN) {
      ...
      convert_resolved_result_type_if_needed(right_resolved_identifier, join_using_column_name_to_column_node, resolved_identifier, scope,
                                             node_to_projection_name);
    }
  }
  ...
  if (join_node_in_resolve_process) return {.resolved_identifier = resolved_identifier, .resolve_place = IdentifierResolvePlace::JOIN_TREE};

  if (scope.join_use_nulls) {
    auto projection_name_it = node_to_projection_name.find(resolved_identifier);
    auto nullable_resolved_identifier =
        convertJoinedColumnTypeToNullIfNeeded(resolved_identifier, resolved_identifier->getResultType(), join_kind, resolved_side, scope);
...
QueryTreeNodePtr IdentifierResolver::matchArrayJoinSubcolumns(const QueryTreeNodePtr &array_join_column_inner_expression,
                                                              const ColumnNode &array_join_column_expression_typed,
                                                              const QueryTreeNodePtr &resolved_expression, IdentifierResolveScope &scope) {
  const auto *resolved_function = resolved_expression->as<FunctionNode>();
  if (!resolved_function || resolved_function->getFunctionName() != "getSubcolumn") return {};
   ...
   * Since we need to perform array join on `getSubcolumn(t, 'map')`, `t.map.values` should become `getSubcolumn(getSubcolumn(t, 'map'),
   * 'values')`
   *
  ...
  if (array_join_column_inner_expression_function && array_join_column_inner_expression_function->getFunctionName() == "getSubcolumn") {
  ...
  if (argument_nodes.size() != 2 && !array_join_parent_column->isEqual(*argument_nodes.at(0))) return {};
  ...
  if (!second_argument || second_argument->getValue().getType() != Field::Types::String)
    throw Exception(ErrorCodes::LOGICAL_ERROR, "Expected constant string as second argument of getSubcolumn function {}",
                    resolved_function->dumpTree());

  auto resolved_subcolumn_path = second_argument->getValue().safeGet<String>();
  if (!startsWith(resolved_subcolumn_path, array_join_subcolumn_prefix)) return {};
  ...
  return wrapExpressionNodeInSubcolumn(std::move(column_node), resolved_subcolumn_path.substr(array_join_subcolumn_prefix.size()),
                                       scope.context);
}

QueryTreeNodePtr IdentifierResolver::tryResolveExpressionFromArrayJoinNestedExpression(
    const QueryTreeNodePtr &resolved_expression, IdentifierResolveScope &scope, ColumnNode &array_join_column_expression_typed,
    QueryTreeNodePtr &array_join_column_inner_expression) {
  auto *array_join_column_inner_expression_function = array_join_column_inner_expression->as<FunctionNode>();

  if (array_join_column_inner_expression_function && array_join_column_inner_expression_function->getFunctionName() == "nested" &&
    ...
    if (nested_keys_names_size == nested_function_arguments_size - 1) {
      for (size_t i = 1; i < nested_function_arguments_size; ++i) {
        if (!nested_function_arguments[i]->isEqual(*resolved_expression)) continue;
...
QueryTreeNodePtr IdentifierResolver::tryResolveExpressionFromArrayJoinExpressions(const QueryTreeNodePtr &resolved_expression,
  ...
  for (const auto &array_join_column_expression : array_join_column_expressions_nodes) {
    ...
    if (array_join_column_expression_typed.hasAlias()) continue;

    auto &array_join_column_inner_expression = array_join_column_expression_typed.getExpressionOrThrow();
    array_join_resolved_expression = tryResolveExpressionFromArrayJoinNestedExpression(
        resolved_expression, scope, array_join_column_expression_typed, array_join_column_inner_expression);
    if (array_join_resolved_expression) break;
    ...
    array_join_resolved_expression =
        matchArrayJoinSubcolumns(array_join_column_inner_expression, array_join_column_expression_typed, resolved_expression, scope);
    if (array_join_resolved_expression) break;
  }
...
std::optional<size_t> getCompoundIdentifierPrefixSize(const Identifier &identifier, const String &name) {
  ...
  if (identifier_view.getLength() != name.length() || identifier_view.getFullName() != name) return std::nullopt;
...
IdentifierResolveResult IdentifierResolver::tryResolveIdentifierFromArrayJoin(const IdentifierLookup &identifier_lookup,
  ...
  for (const auto &array_join_column_expression : array_join_column_expressions_nodes) {
    ...
    const auto &alias_or_name = array_join_column_expression_typed.hasAlias() ? array_join_column_expression_typed.getAlias()
    ...
    auto array_join_column =
        std::make_shared<ColumnNode>(array_join_column_expression_typed.getColumn(), array_join_column_expression_typed.getColumnSource());
    if (identifier_view.empty()) return {.resolved_identifier = array_join_column, .resolve_place = IdentifierResolvePlace::JOIN_TREE};

    if (resolve_result.resolved_identifier) {
      auto resolved_nested_subcolumn =
          tryResolveExpressionFromArrayJoinNestedExpression(resolve_result.resolved_identifier, scope, array_join_column_expression_typed,
                                                            array_join_column_expression_typed.getExpressionOrThrow());
    ...
    auto compound_expr = tryResolveIdentifierFromCompoundExpression(
        identifier_lookup.identifier, identifier_lookup.identifier.getPartsSize() - identifier_view.getPartsSize() /*identifier_bind_size*/,
        array_join_column, {} /* compound_expression_source */, scope, true /* can_be_not_found */);

    if (compound_expr) return {.resolved_identifier = compound_expr, .resolve_place = IdentifierResolvePlace::JOIN_TREE};
  }

  if (!resolve_result.resolved_identifier) return {};

  auto array_join_resolved_expression =
      tryResolveExpressionFromArrayJoinExpressions(resolve_result.resolved_identifier, table_expression_node, scope);
...
IdentifierResolveResult IdentifierResolver::tryResolveIdentifierFromJoinTreeNode(const IdentifierLookup &identifier_lookup,
  ...
  switch (join_tree_node_type) {
    ...
    case QueryTreeNodeType::TABLE_FUNCTION: {
       ...
       */
      if (scope.table_expressions_in_resolve_process.contains(join_tree_node.get())) return {};
...
IdentifierResolveResult IdentifierResolver::tryResolveIdentifierFromJoinTree(const IdentifierLookup &identifier_lookup,
                                                                             IdentifierResolveScope &scope) {
  if (identifier_lookup.isFunctionLookup()) return {};
  ...
  if (!query_scope_node || !query_scope_node->getJoinTree()) return {};
