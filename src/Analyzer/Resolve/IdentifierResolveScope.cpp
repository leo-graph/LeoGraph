...
IdentifierResolveScope::IdentifierResolveScope(QueryTreeNodePtr scope_node_, IdentifierResolveScope* parent_scope_)
    : scope_node(std::move(scope_node_)), parent_scope(parent_scope_) {
  if (parent_scope) {
  ...
  if (auto* union_node = scope_node->as<UnionNode>()) {
    if (parent_scope && parent_scope->context) union_node->getMutableContext()->setDistributed(parent_scope->context->isDistributed());

    context = union_node->getContext();
  } else if (auto* query_node = scope_node->as<QueryNode>()) {
    if (parent_scope && parent_scope->context) query_node->getMutableContext()->setDistributed(parent_scope->context->isDistributed());
...
[[maybe_unused]] const IdentifierResolveScope* IdentifierResolveScope::getNearestQueryScope() const {
  ...
  while (scope_to_check != nullptr) {
    if (scope_to_check->scope_node->getNodeType() == QueryTreeNodeType::QUERY) break;
...
AnalysisTableExpressionData& IdentifierResolveScope::getTableExpressionDataOrThrow(const QueryTreeNodePtr& table_expression_node) {
  ...
  if (it == table_expression_node_to_data.end()) {
    throw Exception(ErrorCodes::LOGICAL_ERROR, "Table expression {} data must be initialized. In scope {}",
                    table_expression_node->formatASTForErrorMessage(), scope_node->formatASTForErrorMessage());
  }

  return it->second;
}

const AnalysisTableExpressionData& IdentifierResolveScope::getTableExpressionDataOrThrow(
    const QueryTreeNodePtr& table_expression_node) const {
  auto it = table_expression_node_to_data.find(table_expression_node);
  if (it == table_expression_node_to_data.end()) {
    throw Exception(ErrorCodes::LOGICAL_ERROR, "Table expression {} data must be initialized. In scope {}",
                    table_expression_node->formatASTForErrorMessage(), scope_node->formatASTForErrorMessage());
...
void IdentifierResolveScope::pushExpressionNode(const QueryTreeNodePtr& node) { expressions_in_resolve_process_stack.push(node); }

void IdentifierResolveScope::popExpressionNode() { expressions_in_resolve_process_stack.pop(); }
...
void dump_list(WriteBuffer& buffer, const String& list_name, const std::ranges::viewable_range auto& list) {
  ...
  for (const auto& node : list) buffer << " { '" << node->getAlias() << "' : " << node->formatASTForErrorMessage() << " }\n";
}
