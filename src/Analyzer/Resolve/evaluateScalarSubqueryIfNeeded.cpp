...
bool subtreeHasViewSource(const IQueryTreeNode *node, const Context &context) {
  ...
  if (const auto *table_node = node->as<TableNode>()) {
    if (table_node->getStorageID().getFullNameNotQuoted() == context.getViewSource()->getStorageID().getFullNameNotQuoted()) return true;
  }

  for (const auto &child : node->getChildren())
    if (subtreeHasViewSource(child.get(), context)) return true;
...
void QueryAnalyzer::evaluateScalarSubqueryIfNeeded(QueryTreeNodePtr &node, IdentifierResolveScope &scope, bool execute_for_exists) {
  ...
  if (!query_node && !union_node)
    throw Exception(ErrorCodes::LOGICAL_ERROR, "Node must have query or union type. Actual: {} {}", node->getNodeTypeName(),
                    node->formatASTForErrorMessage());

  bool is_correlated_subquery =
      (query_node != nullptr && query_node->isCorrelated()) || (union_node != nullptr && union_node->isCorrelated());
  if (is_correlated_subquery) throw Exception(ErrorCodes::BAD_ARGUMENTS, "Cannot evaluate correlated scalar subquery");
  ...
  if (scalars_cache.contains(node_with_hash)) {
    ...
    if (auto *new_query_node = query_tree->as<QueryNode>()) new_query_node->getMutableContext() = subquery_context;
    if (auto *new_union_node = query_tree->as<UnionNode>()) new_union_node->getMutableContext() = subquery_context;
    ...
    if (auto storage = subquery_context->getViewSource()) replaceStorageInQueryTree(query_tree, subquery_context, storage);
    ...
    auto wrap_with_nullable_or_tuple = [](Block &block) {
      ...
      if (block.columns() == 1) {
        ...
        block = Block(
            {{ColumnTuple::create(block.getColumns()), std::make_shared<DataTypeTuple>(block.getDataTypes(), block.getNames()), "tuple"}});
      }
    };
    ...
    if (can_use_global_scalars && context->hasQueryContext()) context->getQueryContext()->addScalar(str_hash, scalar_block);
  }
  ...
  static const std::set<std::string_view> useless_literal_types = {"Array",    "Tuple", "AggregateFunction",
  ...
  if (mutable_context->hasQueryContext()) mutable_context->getQueryContext()->addScalar(scalar_query_hash_string, scalar_block);
