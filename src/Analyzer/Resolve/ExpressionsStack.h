 ...
 public:
  void push(const QueryTreeNodePtr& node) {
    ...
    if (const auto* function = node->as<FunctionNode>()) {
      if (AggregateFunctionFactory::instance().isAggregateFunctionName(function->getFunctionName())) ++aggregate_functions_counter;
    }
  ...
  void pop() {
    ...
    if (!top_expression_alias.empty()) {
      ...
      if (alias_expressions.empty()) alias_name_to_expressions.erase(it);
    }
  ...
  [[maybe_unused]] const QueryTreeNodePtr& getRoot() const { return expressions.front(); }

  const QueryTreeNodePtr& getTop() const { return expressions.back(); }

  [[maybe_unused]] const QueryTreeNodePtr& operator[](int32_t n) const {
    if (n < 0) n = static_cast<int32_t>(expressions.size()) + n - 1;
    return expressions[n];
  }

  [[maybe_unused]] bool hasExpressionWithAlias(const std::string& alias) const { return alias_name_to_expressions.contains(alias); }

  bool hasAggregateFunction() const { return aggregate_functions_counter > 0; }
  ...
  QueryTreeNodePtr getExpressionWithAlias(const std::string& alias) const {
    ...
    if (expression_it == alias_name_to_expressions.end()) return {};
  ...
  [[maybe_unused]] size_t size() const { return expressions.size(); }
  ...
  void dump(WriteBuffer& buffer) const {
    ...
    for (const auto& expression : expressions) {
      ...
      if (!alias.empty()) buffer << " alias " << alias;
