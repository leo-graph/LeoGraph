...
class QueryExpressionsAliasVisitor : public InDepthQueryTreeVisitor<QueryExpressionsAliasVisitor> {
 public:
  explicit QueryExpressionsAliasVisitor(ScopeAliases &aliases_) : aliases(aliases_) {}

  void visitImpl(QueryTreeNodePtr &node) { updateAliasesIfNeeded(node); }
  ...
  bool needChildVisit(const QueryTreeNodePtr &, const QueryTreeNodePtr &child) {
    ...
    if (auto *query_tree_node = child->as<QueryNode>()) {
      if (query_tree_node->isCTE()) return false;

      updateAliasesIfNeeded(child);
      return false;
    }
    if (auto *union_node = child->as<UnionNode>()) {
      if (union_node->isCTE()) return false;
 ...
 private:
  void addDuplicatingAlias(const QueryTreeNodePtr &node) { aliases.nodes_with_duplicated_aliases.emplace_back(node); }

  void updateAliasesIfNeeded(const QueryTreeNodePtr &node) {
    if (!node->hasAlias()) return;

    auto node_type = node->getNodeType();

    // We should not resolve expressions to WindowNode
    if (node_type == QueryTreeNodeType::WINDOW) return;
    ...
    if (node_type == QueryTreeNodeType::TABLE || node_type == QueryTreeNodeType::TABLE_FUNCTION) return;
    ...
    switch (node_type) {
      case QueryTreeNodeType::LAMBDA: {
        ...
        if (!inserted || aliases.alias_name_to_expression_node.contains(alias)) addDuplicatingAlias(cloned_alias_node);
        break;
      }
      case QueryTreeNodeType::IDENTIFIER: {
        ...
        if (!inserted_expression || !inserted_lambda || !inserted_table_expression) addDuplicatingAlias(cloned_alias_node);
        break;
      }
      default: {
        auto [_, inserted] = aliases.alias_name_to_expression_node.emplace(alias, cloned_alias_node);
        if (!inserted || aliases.alias_name_to_lambda_node.contains(alias)) addDuplicatingAlias(cloned_alias_node);
        break;
      }
    }
  }
