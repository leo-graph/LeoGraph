...
void TypoCorrection::collectCompoundExpressionValidIdentifiers(const Identifier &unresolved_identifier,
  ...
  IDataType::forEachSubcolumn(
      [&](const auto &, const auto &name, const auto &) {
        ...
        if (new_identifier_size == unresolved_identifier.getPartsSize()) {
          ...
          for (const auto &part : subcolumn_indentifier) new_identifier.push_back(part);
...
void TypoCorrection::collectTableExpressionValidIdentifiers(const Identifier &unresolved_identifier,
  ...
  for (const auto &[column_name, column_node] : table_expression_data.column_name_to_column_node) {
    ...
    if (unresolved_identifier.getPartsSize() == column_identifier.getPartsSize()) valid_identifiers_result.insert(column_identifier);

    collectCompoundExpressionValidIdentifiers(unresolved_identifier, column_node->getColumnType(), column_identifier,
                                              valid_identifiers_result);

    if (table_expression->hasAlias()) {
      Identifier column_identifier_with_alias({table_expression->getAlias()});
      for (const auto &column_identifier_part : column_identifier) column_identifier_with_alias.push_back(column_identifier_part);
      ...
      collectCompoundExpressionValidIdentifiers(unresolved_identifier, column_node->getColumnType(), column_identifier_with_alias,
                                                valid_identifiers_result);
    }

    if (!table_expression_data.table_name.empty()) {
      Identifier column_identifier_with_table_name({table_expression_data.table_name});
      for (const auto &column_identifier_part : column_identifier) column_identifier_with_table_name.push_back(column_identifier_part);
      ...
      collectCompoundExpressionValidIdentifiers(unresolved_identifier, column_node->getColumnType(), column_identifier_with_table_name,
                                                valid_identifiers_result);
    }

    if (!table_expression_data.database_name.empty() && !table_expression_data.table_name.empty()) {
      Identifier column_identifier_with_table_name_and_database_name(
      ...
      collectCompoundExpressionValidIdentifiers(unresolved_identifier, column_node->getColumnType(),
                                                column_identifier_with_table_name_and_database_name, valid_identifiers_result);
    }
  }
}

/// Get valid identifiers for typo correction from scope without looking at parent scopes
void TypoCorrection::collectScopeValidIdentifiers(const Identifier &unresolved_identifier, const IdentifierResolveScope &scope,
                                                  bool allow_expression_identifiers, bool allow_function_identifiers,
  ...
  if (allow_expression_identifiers) {
    ...
    if (identifier_is_compound) {
      for (const auto &[name, expression_type] : scope.aliases.alias_name_to_expression_type) {
        ...
        collectCompoundExpressionValidIdentifiers(unresolved_identifier, expression_type, expression_identifier, valid_identifiers_result);
      }
    }

    for (const auto &[table_expression, table_expression_data] : scope.table_expression_node_to_data) {
      collectTableExpressionValidIdentifiers(unresolved_identifier, table_expression, table_expression_data, valid_identifiers_result);
    }
  }

  if (identifier_is_short) {
    if (allow_function_identifiers) {
      for (const auto &[name, _] : scope.aliases.alias_name_to_expression_node) valid_identifiers_result.insert(Identifier(name));
    }

    if (allow_table_expression_identifiers) {
      for (const auto &[name, _] : scope.aliases.alias_name_to_table_expression_node) valid_identifiers_result.insert(Identifier(name));
    }
  }
  ...
  for (const auto &[argument_name, expression] : scope.expression_argument_name_to_node) {
    ...
    if (allow_expression_identifiers && isExpressionNodeType(expression_node_type)) {
      ...
      if (identifier_is_compound && result_type) {
        collectCompoundExpressionValidIdentifiers(unresolved_identifier, result_type, expression_identifier, valid_identifiers_result);
      }
...
void TypoCorrection::collectScopeWithParentScopesValidIdentifiers(const Identifier &unresolved_identifier,
                                                                  const IdentifierResolveScope &scope, bool allow_expression_identifiers,
                                                                  bool allow_function_identifiers, bool allow_table_expression_identifiers,
                                                                  std::unordered_set<Identifier> &valid_identifiers_result) {
  const IdentifierResolveScope *current_scope = &scope;

  while (current_scope) {
    collectScopeValidIdentifiers(unresolved_identifier, *current_scope, allow_expression_identifiers, allow_function_identifiers,
                                 allow_table_expression_identifiers, valid_identifiers_result);

    current_scope = current_scope->parent_scope;
  }
}

std::vector<String> TypoCorrection::collectIdentifierTypoHints(const Identifier &unresolved_identifier,
  ...
  for (const auto &valid_identifier : valid_identifiers) prompting_strings.push_back(valid_identifier.getFullName());
