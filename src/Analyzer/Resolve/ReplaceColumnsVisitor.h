...
class ReplaceColumnsVisitor : public InDepthQueryTreeVisitor<ReplaceColumnsVisitor> {
 public:
  explicit ReplaceColumnsVisitor(const QueryTreeNodePtrWithHashMap<QueryTreeNodePtr>& replacement_map_, const ContextPtr& context_)
      : replacement_map(replacement_map_), context(context_) {}

  /// Apply replacement transitively, because column may change it's type twice, one to have a supertype and then because of
  /// `joun_use_nulls`
  static QueryTreeNodePtr findTransitiveReplacement(QueryTreeNodePtr node,
    ...
    for (; it != replacement_map_.end(); it = replacement_map_.find(result_node)) {
      if (result_node && result_node->isEqual(*it->second)) {
        ...
        for (const auto& [k, v] : replacement_map_)
          map_dump.push_back(
              fmt::format("{} -> {} (is_equals: {}, is_same: {})", k.node->dumpTree(), v->dumpTree(), k.node->isEqual(*v), k.node == v));
  ...
  void visitImpl(QueryTreeNodePtr& node) {
    if (auto replacement_node = findTransitiveReplacement(node, replacement_map)) node = replacement_node;
  ...
  bool needChildVisit(QueryTreeNodePtr& /* parent */, QueryTreeNodePtr& child) {
    ...
    return child->getNodeType() == QueryTreeNodeType::IDENTIFIER || child->getNodeType() == QueryTreeNodeType::LIST ||
           child->getNodeType() == QueryTreeNodeType::FUNCTION || child->getNodeType() == QueryTreeNodeType::COLUMN;
  }
