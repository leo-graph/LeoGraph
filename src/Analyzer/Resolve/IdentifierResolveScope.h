 ...
 * If node is resolved as list node this is case for `untuple` function or `matcher` result projection names array must contain projection
 * names for result nodes. If node is not resolved as list node, projection names array contain single projection name for node.
 *
 ...
 * In such case projection name is `t1.id` because if additional qualification is removed then column projection name `id` will be
 * ambiguous.
 *
 * Example: SELECT default.test_table_1.id FROM test_table_1 AS t1, test_table_2 AS t2
 * In such case projection name is `test_table_1.id` because we remove unnecessary database qualification, but table name qualification
 * cannot be removed because otherwise column projection name `id` will be ambiguous.
 *
 ...
 *
 * Special case is `untuple` function. If `untuple` function specified with alias, then result nodes will have alias.tuple_column_name
 * projection names. Example: SELECT cast(tuple(1), 'Tuple(id UInt64)') AS value, untuple(value) AS a; Result projection names are `value`,
 * `a.id`.
 *
 * If `untuple` function does not have alias then result nodes will have `tupleElement(untuple_expression_projection_name,
 * 'tuple_column_name') projection names.
 *
