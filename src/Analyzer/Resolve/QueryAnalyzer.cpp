...
QueryAnalyzer::QueryAnalyzer(bool only_analyze_) : identifier_resolver(node_to_projection_name), only_analyze(only_analyze_) {}
...
void QueryAnalyzer::resolve(QueryTreeNodePtr &node, const QueryTreeNodePtr &table_expression, ContextPtr context) {
  ...
  if (!scope.context) scope.context = context;

  auto node_type = node->getNodeType();

  switch (node_type) {
    case QueryTreeNodeType::QUERY: {
      if (table_expression) throw Exception(ErrorCodes::LOGICAL_ERROR, "For query analysis table expression must be empty");

      resolveQuery(node, scope);
      break;
    }
    case QueryTreeNodeType::UNION: {
      if (table_expression) throw Exception(ErrorCodes::LOGICAL_ERROR, "For union analysis table expression must be empty");
...
bool isFromJoinTree(const IQueryTreeNode *node_source, const IQueryTreeNode *tree_node) {
  if (node_source == tree_node) return true;
  ...
  while (!stack.empty()) {
    ...
    if (node_source == current) return true;
...
std::optional<JoinTableSide> QueryAnalyzer::getColumnSideFromJoinTree(const QueryTreeNodePtr &resolved_identifier,
                                                                      const JoinNode &join_node) {
  if (resolved_identifier->getNodeType() == QueryTreeNodeType::CONSTANT) return {};
  ...
  if (resolved_identifier->getNodeType() == QueryTreeNodeType::FUNCTION) {
    ...
    for (const auto &argument_node : argument_nodes) {
      ...
      if (table_side && result && *table_side != *result) return {};
      result = table_side;
    }
    return result;
  }
  ...
  if (isFromJoinTree(column_src, join_node.getLeftTableExpression().get())) return JoinTableSide::Left;
  if (isFromJoinTree(column_src, join_node.getRightTableExpression().get())) return JoinTableSide::Right;
  return {};
}

IdentifierResolveScope &QueryAnalyzer::createIdentifierResolveScope(const QueryTreeNodePtr &scope_node,
                                                                    IdentifierResolveScope *parent_scope) {
  auto [it, _] = node_to_scope_map.emplace(scope_node, IdentifierResolveScope{scope_node, parent_scope});
  return it->second;
}

ProjectionName QueryAnalyzer::calculateFunctionProjectionName(const QueryTreeNodePtr &function_node,
  ...
  if (!is_array_function && !is_tuple_function) buffer << function_node_name;
  ...
  if (!parameters_projection_names.empty()) {
    ...
    for (size_t i = 0; i < function_parameters_projection_names_size; ++i) {
      ...
      if (i + 1 != function_parameters_projection_names_size) buffer << ", ";
    }
  ...
  for (size_t i = 0; i < function_arguments_projection_names_size; ++i) {
    ...
    if (i + 1 != function_arguments_projection_names_size) buffer << ", ";
  }
...
ProjectionName QueryAnalyzer::calculateWindowProjectionName(const QueryTreeNodePtr &window_node, const QueryTreeNodePtr &parent_window_node,
  ...
  if (!parent_window_name.empty()) buffer << parent_window_name;

  if (!partition_by_projection_names.empty() && !parent_window_node_has_partition_by) {
    if (!parent_window_name.empty()) buffer << ' ';
    ...
    for (size_t i = 0; i < partition_by_projection_names_size; ++i) {
      ...
      if (i + 1 != partition_by_projection_names_size) buffer << ", ";
    }
  }

  if (!order_by_projection_names.empty() && !parent_window_node_has_order_by) {
    if (!partition_by_projection_names.empty() || !parent_window_name.empty()) buffer << ' ';
    ...
    for (size_t i = 0; i < order_by_projection_names_size; ++i) {
      ...
      if (i + 1 != order_by_projection_names_size) buffer << ", ";
    }
  }

  if (!window_frame.is_default) {
    if (!partition_by_projection_names.empty() || !order_by_projection_names.empty() || !parent_window_name.empty()) buffer << ' ';
...
ProjectionName QueryAnalyzer::calculateSortColumnProjectionName(const QueryTreeNodePtr &sort_column_node,
  ...
  if (nulls_sort_direction) sort_column_projection_name_buffer << " NULLS " << (nulls_sort_direction == sort_direction ? "LAST" : "FIRST");

  if (auto collator = sort_node_typed.getCollator()) sort_column_projection_name_buffer << " COLLATE " << collator->getLocale();

  if (sort_node_typed.withFill()) {
    sort_column_projection_name_buffer << " WITH FILL";

    if (sort_node_typed.hasFillFrom()) sort_column_projection_name_buffer << " FROM " << fill_from_expression_projection_name;

    if (sort_node_typed.hasFillTo()) sort_column_projection_name_buffer << " TO " << fill_to_expression_projection_name;

    if (sort_node_typed.hasFillStep()) sort_column_projection_name_buffer << " STEP " << fill_step_expression_projection_name;
...
QueryTreeNodePtr QueryAnalyzer::tryGetLambdaFromUserDefinedSQLFunctions(const ASTPtr &create_function_ast, ContextPtr context) {
  ...
  if (!create_function_query) return nullptr;
  ...
  if (it != function_name_to_user_defined_lambda.end()) return it->second;
  ...
  if (result_node->getNodeType() != QueryTreeNodeType::LAMBDA)
    throw Exception(ErrorCodes::LOGICAL_ERROR, "SQL user defined function {} must represent lambda expression. Actual: {}", function_name,
...
void QueryAnalyzer::mergeWindowWithParentWindow(const QueryTreeNodePtr &window_node, const QueryTreeNodePtr &parent_window_node,
  ...
  if (window_node_typed.hasPartitionBy()) {
    throw Exception(ErrorCodes::BAD_ARGUMENTS, "Derived window definition '{}' is not allowed to override PARTITION BY. In scope {}",
                    window_node_typed.formatASTForErrorMessage(), scope.scope_node->formatASTForErrorMessage());
  }
  ...
  if (!parent_window_node_typed.getWindowFrame().is_default) {
    throw Exception(ErrorCodes::BAD_ARGUMENTS,
                    ...
                    parent_window_name, window_node_typed.formatASTForErrorMessage(), scope.scope_node->formatASTForErrorMessage());
  }

  window_node_typed.getPartitionByNode() = parent_window_node_typed.getPartitionBy().clone();

  if (parent_window_node_typed.hasOrderBy()) window_node_typed.getOrderByNode() = parent_window_node_typed.getOrderBy().clone();
}
 ...
 */
void QueryAnalyzer::replaceNodesWithPositionalArguments(QueryTreeNodePtr &node_list, const QueryTreeNodes &projection_nodes,
                                                        IdentifierResolveScope &scope) {
  ...
  for (auto &node : node_list_typed.getNodes()) {
    ...
    if (auto *sort_node = node->as<SortNode>()) node_to_replace = &sort_node->getExpression();
    ...
    } else  // Int64
    {
      ...
      if (value > 0)
        ...
        if (value < -static_cast<Int64>(projection_nodes.size()))
          throw Exception(ErrorCodes::BAD_ARGUMENTS,
                          "Negative positional argument number {} is out of bounds. Expected in range [-{}, -1]. In scope {}", value,
                          projection_nodes.size(), scope.scope_node->formatASTForErrorMessage());
        pos = projection_nodes.size() + value + 1;
      }
    }

    if (!pos || pos > projection_nodes.size())
      throw Exception(ErrorCodes::BAD_ARGUMENTS, "Positional argument number {} is out of bounds. Expected in range [1, {}]. In scope {}",
                      pos, projection_nodes.size(), scope.scope_node->formatASTForErrorMessage());
...
void QueryAnalyzer::convertLimitOffsetExpression(QueryTreeNodePtr &expression_node, const String &expression_description,
                                                 IdentifierResolveScope &scope) {
  ...
  if (!limit_offset_constant_node || !isNativeNumber(removeNullable(limit_offset_constant_node->getResultType())))
    throw Exception(ErrorCodes::INVALID_LIMIT_EXPRESSION, "{} expression must be constant with numeric type. Actual: {}. In scope {}",
                    expression_description, expression_node->formatASTForErrorMessage(), scope.scope_node->formatASTForErrorMessage());
...
void QueryAnalyzer::validateTableExpressionModifiers(const QueryTreeNodePtr &table_expression_node, IdentifierResolveScope &scope) {
  ...
  if (!table_node && !table_function_node && !query_node && !union_node)
    throw Exception(ErrorCodes::LOGICAL_ERROR,
                    ...
                    table_expression_node->formatASTForErrorMessage(), scope.scope_node->formatASTForErrorMessage());

  if (table_node || table_function_node) {
    auto table_expression_modifiers =
    ...
    if (table_expression_modifiers.has_value()) {
      ...
      if (table_expression_modifiers->hasSampleSizeRatio() && !storage->supportsSampling())
        throw Exception(ErrorCodes::SAMPLING_NOT_SUPPORTED, "Storage {} doesn't support sampling",
                        storage->getStorageID().getFullNameNotQuoted());
    }
  }
}

void QueryAnalyzer::validateJoinTableExpressionWithoutAlias(const QueryTreeNodePtr &join_node,
                                                            const QueryTreeNodePtr &table_expression_node, IdentifierResolveScope &scope) {
  if (!scope.context->getSettingsRef()[Setting::joined_subquery_requires_alias]) return;

  bool table_expression_has_alias = table_expression_node->hasAlias();
  if (table_expression_has_alias) return;

  if (const auto *join = join_node->as<const JoinNode>(); join && join->getKind() == JoinKind::Paste) return;

  auto *query_node = table_expression_node->as<QueryNode>();
  auto *union_node = table_expression_node->as<UnionNode>();
  if ((query_node && !query_node->getCTEName().empty()) || (union_node && !union_node->getCTEName().empty())) return;

  auto table_expression_node_type = table_expression_node->getNodeType();

  if (table_expression_node_type == QueryTreeNodeType::TABLE_FUNCTION || table_expression_node_type == QueryTreeNodeType::QUERY ||
    ...
    throw Exception(ErrorCodes::ALIAS_REQUIRED,
                    ...
                    join_node->formatASTForErrorMessage(), table_expression_node->formatASTForErrorMessage(),
...
std::pair<bool, UInt64> QueryAnalyzer::recursivelyCollectMaxOrdinaryExpressions(QueryTreeNodePtr &node, QueryTreeNodes &into) {
  ...
  if (!has_aggregate) {
    for (UInt64 i = 0; i < pushed_children; i++) into.pop_back();
...
void QueryAnalyzer::expandGroupByAll(QueryNode &query_tree_node_typed) {
  if (!query_tree_node_typed.isGroupByAll()) return;

  auto &group_by_nodes = query_tree_node_typed.getGroupBy().getNodes();
  auto &projection_list = query_tree_node_typed.getProjection();

  for (auto &node : projection_list.getNodes()) recursivelyCollectMaxOrdinaryExpressions(node, group_by_nodes);
...
void QueryAnalyzer::expandOrderByAll(QueryNode &query_tree_node_typed, const Settings &settings) {
  if (!settings[Setting::enable_order_by_all] || !query_tree_node_typed.isOrderByAll()) return;

  auto *all_node = query_tree_node_typed.getOrderBy().getNodes()[0]->as<SortNode>();
  if (!all_node) throw Exception(ErrorCodes::LOGICAL_ERROR, "Select analyze for not sort node.");
  ...
  for (auto &node : projection_nodes) {
    ...
    /// - SELECT a, b ORDER BY all;             -- tricky in other way: does the user want to sort by columns in SELECT clause or by not
    /// SELECTed column "all"?
    ...
    if (resolved_expression_it != resolved_expressions.end()) {
      ...
      if (projection_names.size() != 1)
        throw Exception(ErrorCodes::LOGICAL_ERROR, "Expression nodes list expected 1 projection names. Actual: {}",
...
void QueryAnalyzer::expandLimitByAll(QueryNode &query_tree_node_typed) {
  if (!query_tree_node_typed.isLimitByAll()) return;

  if (!query_tree_node_typed.hasLimitByLimit()) {
    throw Exception(ErrorCodes::SYNTAX_ERROR, "LIMIT BY ALL requires a limit expression. Use LIMIT n BY ALL");
  }
  ...
  for (auto &projection_node : projection_nodes) {
    if (hasAggregateFunctionNodes(projection_node)) continue;

    limit_by_nodes.push_back(projection_node->clone());
  }

  if (limit_by_nodes.empty()) {
    throw Exception(ErrorCodes::SYNTAX_ERROR, "LIMIT BY ALL requires at least one non-aggregate expression in SELECT");
  }

  query_tree_node_typed.setIsLimitByAll(false);
}

std::string QueryAnalyzer::rewriteAggregateFunctionNameIfNeeded(const std::string &aggregate_function_name, NullsAction action,
  ...
  if (need_add_or_null) {
    ...
    if (!properties->returns_default_when_only_null) result_aggregate_function_name += "OrNull";
  }
  ...
  if (result_aggregate_function_name.ends_with("OrNull")) {
    ...
    if (function_properies && !function_properies->returns_default_when_only_null) {
      ...
      for (const auto &suffix : suffixes_to_replace) {
        ...
        if (!result_aggregate_function_name.ends_with(suffix_to_check)) continue;
 ...
 * 2. If alias is registered in current expressions that are in resolve process and if top expression is not part of bottom expression with
 * the same alias subtree it may be cyclic aliases. In that case return nullptr and allow to continue identifier resolution in other places.
 ...
 * If identifier is resolved, depending on lookup context, erase entry from expression or lambda map. Check QueryExpressionsAliasVisitor
 * documentation. Pop node from current scope expressions in resolve process stack.
 *
...
IdentifierResolveResult QueryAnalyzer::tryResolveIdentifierFromAliases(const IdentifierLookup &identifier_lookup,
  ...
  if (!alias_node)
    throw Exception(ErrorCodes::LOGICAL_ERROR, "Node with alias {} is not valid. In scope {}", identifier_bind_part,
                    scope.scope_node->formatASTForErrorMessage());
  ...
  if (node_type == QueryTreeNodeType::IDENTIFIER) {
    ...
    if (alias_node->hasOriginalAST()) alias_identifier_lookup.original_ast_node = alias_node->getOriginalAST();
    ...
    resolveExpressionNode(alias_node, *scope_to_resolve_alias_expression, false /*allow_lambda_expression*/,
                          false /*allow_table_expression*/, false /*ignore_alias*/,
    ...
    if (identifier_resolve_context.allow_to_resolve_subquery_during_identifier_resolution)
      resolveExpressionNode(alias_node, *scope_to_resolve_alias_expression, false /*allow_lambda_expression*/,
                            identifier_lookup.isTableExpressionLookup() /*allow_table_expression*/);
  }
  ...
  if (identifier_lookup.identifier.isCompound() && alias_node) {
    if (identifier_lookup.isExpressionLookup()) {
      if (auto resolved_identifier = identifier_resolver.tryResolveIdentifierFromCompoundExpression(
              identifier_lookup.identifier, 1 /*identifier_bind_size*/, alias_node, {} /* compound_expression_source */, scope,
    ...
    if (identifier_lookup.isFunctionLookup() || identifier_lookup.isTableExpressionLookup()) {
      throw Exception(ErrorCodes::UNKNOWN_IDENTIFIER, "Compound identifier '{}' cannot be resolved as {}. In scope {}",
                      identifier_lookup.identifier.getFullName(), identifier_lookup.isFunctionLookup() ? "function" : "table expression",
                      scope.scope_node->formatASTForErrorMessage());
    }
  }
 ...
 */
IdentifierResolveResult QueryAnalyzer::tryResolveIdentifierInParentScopes(const IdentifierLookup &identifier_lookup,
                                                                          IdentifierResolveScope &scope,
                                                                          IdentifierResolveContext identifier_resolve_context) {
  if (!scope.parent_scope) return {};
  ...
  if (identifier_lookup.isTableExpressionLookup()) {
    ...
    bool is_table_from_expression_arguments =
        resolve_result.isResolvedFromExpressionArguments() && resolved_identifier->getNodeType() == QueryTreeNodeType::TABLE;
    /// Resolved from alias or join tree in the parent scope when initial scope is not query.
    bool is_from_join_tree_or_aliases =
        !initial_scope_is_query && (resolve_result.isResolvedFromJoinTree() || resolve_result.isResolvedFromAliases());
    bool is_valid_table_expression = is_cte || is_table_from_expression_arguments || is_from_join_tree_or_aliases;

    if (!is_valid_table_expression) return {};
    return resolve_result;
  }
  if (identifier_lookup.isFunctionLookup()) return resolve_result;

  CorrelatedColumnsCollector correlated_columns_collector{resolved_identifier, identifier_resolve_context.scope_to_resolve_alias_expression,
                                                          node_to_scope_map};
  if (correlated_columns_collector.has() && !scope.context->getSettingsRef()[Setting::allow_experimental_correlated_subqueries]) {
    throw Exception(ErrorCodes::UNSUPPORTED_METHOD,
                    ...
                    identifier_lookup.identifier.getFullName(), resolved_identifier->formatASTForErrorMessage(),
                    fmt::join(correlated_columns_collector.get() |
                                  std::views::transform([](const auto &e) { return e->template as<ColumnNode>()->getColumnName(); }),
                              "', '"),
                    scope.scope_node->formatASTForErrorMessage());
  }
...
static void correctColumnExpressionType(ColumnNode &column_node, const ContextPtr &context) {
  if (!column_node.hasExpression()) return;
  auto &column_expression = column_node.getExpression();
  if (column_node.getColumnType()->equals(*column_expression->getResultType())) return;
 ...
 */
IdentifierResolveResult QueryAnalyzer::tryResolveIdentifier(const IdentifierLookup &identifier_lookup, IdentifierResolveScope &scope,
  ...
  if (!resolve_result.resolved_identifier) {
    ...
    if (identifier_lookup.isExpressionLookup()) {
      ...
      if (alias_it && (*alias_it)->getNodeType() == QueryTreeNodeType::COLUMN) {
        ...
        if (column_node.getColumnSource()->getNodeType() == QueryTreeNodeType::ARRAY_JOIN) prefer_column_name_to_alias = true;
      }
    }
  ...
  if (!resolve_result.resolved_identifier && identifier_resolve_settings.allow_to_check_cte &&
      identifier_lookup.isTableExpressionLookup()) {
 ...
 */
void QueryAnalyzer::qualifyColumnNodesWithProjectionNames(const QueryTreeNodes &column_nodes, const QueryTreeNodePtr &table_expression_node,
  ...
  if (table_expression_node->hasAlias())
    ...
    if (!table_node->getTemporaryTableName().empty()) additional_column_qualification_parts = {table_node->getTemporaryTableName()};
  ...
  for (const auto &column_node : column_nodes) {
    ...
    for (size_t i = 0; i < additional_column_qualification_parts_size; ++i) {
      ...
      if (IdentifierResolver::tryBindIdentifierToAliases(identifier_lookup, scope) ||
          IdentifierResolver::tryBindIdentifierToArrayJoinExpressions(identifier_lookup, scope))
...
GetColumnsOptions QueryAnalyzer::buildGetColumnsOptions(QueryTreeNodePtr &matcher_node, const ContextPtr &context) {
  ...
  if (matcher_node_typed.isAsteriskMatcher()) {
    ...
    if (settings[Setting::asterisk_include_alias_columns]) get_columns_options_kind |= GetColumnsOptions::Kind::Aliases;

    if (settings[Setting::asterisk_include_materialized_columns]) get_columns_options_kind |= GetColumnsOptions::Kind::Materialized;
  }
...
QueryAnalyzer::QueryTreeNodesWithNames QueryAnalyzer::getMatchedColumnNodesWithNames(const QueryTreeNodePtr &matcher_node,
  ...
  if (nearest_query_scope) table_expression_data = &nearest_query_scope->getTableExpressionDataOrThrow(table_expression_node);

  QueryTreeNodes matched_column_nodes;

  for (const auto &column : matched_columns) {
    const auto &column_name = column.name;
    if (!matcher_node_typed.isMatchingColumn(column_name)) continue;
...
bool hasTableExpressionInJoinTree(const QueryTreeNodePtr &join_tree_node, const QueryTreeNodePtr &table_expression) {
  ...
  while (!nodes_to_process.empty()) {
    ...
    if (node_to_process == table_expression) return true;
    ...
    if (node_type == QueryTreeNodeType::JOIN) {
      ...
      for (const auto &expr : join_node.getTableExpressions()) nodes_to_process.push_back(expr);
...
void QueryAnalyzer::updateMatchedColumnsFromJoinUsing(QueryTreeNodesWithNames &result_matched_column_nodes_with_names,
                                                      IdentifierResolveScope &scope) {
  ...
  if (!nearest_query_scope_query_node || !nearest_query_scope_query_node->getJoinTree()) {
    throw Exception(ErrorCodes::UNSUPPORTED_METHOD, "There are no table sources. In scope {}",
                    scope.scope_node->formatASTForErrorMessage());
  }
  ...
  if (!join_node_in_resolve_process && join_node && join_node->isUsingJoinExpression()) {
    ...
    for (auto &[matched_column_node, _] : result_matched_column_nodes_with_names) {
      ...
      for (const auto &join_using_node : join_using_nodes) {
        ...
        if (matched_column_name != join_using_column_name) continue;
        ...
        for (const auto &join_using_column_inner_node : join_using_column_nodes) {
          ...
          if (!join_using_column_inner_column_node || !join_using_column_inner_column_node->hasExpression()) continue;
        ...
        if (it != node_to_projection_name.end()) node_to_projection_name.emplace(matched_column_node, it->second);
 ...
 */
QueryAnalyzer::QueryTreeNodesWithNames QueryAnalyzer::resolveQualifiedMatcher(QueryTreeNodePtr &matcher_node,
                                                                              IdentifierResolveScope &scope) {
  auto &matcher_node_typed = matcher_node->as<MatcherNode &>();
  ...
  if (expression_query_tree_node) {
    ...
    if (!tuple_data_type)
      throw Exception(
          ...
          matcher_node->formatASTForErrorMessage(), expression_query_tree_node->formatASTForErrorMessage(),
          expression_query_tree_node->getResultType()->getName(), scope.scope_node->formatASTForErrorMessage());
    ...
    for (const auto &element_name : element_names) {
      if (!matcher_node_typed.isMatchingColumn(element_name)) continue;
  ...
  if (!table_expression_node) {
    throw Exception(ErrorCodes::UNKNOWN_IDENTIFIER, "Qualified matcher {} does not find table. In scope {}",
                    matcher_node->formatASTForErrorMessage(), scope.scope_node->formatASTForErrorMessage());
  }
  ...
  if (table_expression_query_node || table_expression_union_node) {
    ...
    throw Exception(ErrorCodes::LOGICAL_ERROR, "Invalid table expression node {}. In scope {}",
                    table_expression_node->formatASTForErrorMessage(), scope.scope_node->formatASTForErrorMessage());
  }

  auto result_matched_column_nodes_with_names = getMatchedColumnNodesWithNames(matcher_node, table_expression_node, matched_columns, scope);
...
QueryAnalyzer::QueryTreeNodesWithNames QueryAnalyzer::resolveUnqualifiedMatcher(QueryTreeNodePtr &matcher_node,
                                                                                IdentifierResolveScope &scope) {
  auto &matcher_node_typed = matcher_node->as<MatcherNode &>();
  ...
  /// If there are no parent query scope or query scope does not have join tree
  if (!nearest_query_scope_query_node || !nearest_query_scope_query_node->getJoinTree()) {
    throw Exception(ErrorCodes::UNSUPPORTED_METHOD, "Unqualified matcher {} cannot be resolved. There are no table sources. In scope {}",
                    matcher_node->formatASTForErrorMessage(), scope.scope_node->formatASTForErrorMessage());
  }
  ...
  if (matcher_node_typed.getMatcherType() == MatcherNodeType::COLUMNS_LIST) {
    ...
    for (const auto &identifier : identifiers) {
      ...
      if (!resolve_result.isResolved())
        throw Exception(ErrorCodes::UNKNOWN_IDENTIFIER, "Unknown identifier '{}' inside COLUMNS matcher. In scope {}",
      ...
      if (!resolved_column)
        throw Exception(
            ErrorCodes::UNKNOWN_IDENTIFIER, "Identifier '{}' inside COLUMNS matcher must resolve into a column, but got {}. In scope {}",
            identifier.getFullName(), resolve_result.resolved_identifier->getNodeTypeName(), scope.scope_node->formatASTForErrorMessage());
  ...
  for (auto &table_expression : table_expressions_stack) {
    ...
    if (table_expression->as<ArrayJoinNode>()) {
      if (table_expressions_column_nodes_with_names_stack.empty())
        throw Exception(ErrorCodes::LOGICAL_ERROR, "Expected at least 1 table expressions on stack before ARRAY JOIN processing");

      if (table_expression_in_resolve_process) continue;
      ...
      for (auto &[table_expression_column_node, _] : table_expression_column_nodes_with_names) {
        auto array_join_resolved_expression =
            identifier_resolver.tryResolveExpressionFromArrayJoinExpressions(table_expression_column_node, table_expression, scope);
        if (array_join_resolved_expression) table_expression_column_node = std::move(array_join_resolved_expression);
      }
    ...
    if (cross_join_node) {
      ...
      if (stack_size < cross_join_node->getTableExpressions().size())
        throw Exception(ErrorCodes::LOGICAL_ERROR,
                        "Expected at least {} table expressions on stack before CROSS_JOIN processing. Actual: {}", num_tables, stack_size);
    ...
    if (join_node) {
      ...
      if (table_expressions_column_nodes_with_names_stack_size < 2)
        throw Exception(ErrorCodes::LOGICAL_ERROR, "Expected at least 2 table expressions on stack before JOIN processing. Actual: {}",
      ...
      if (scope.allow_resolve_from_using && !table_expression_in_resolve_process && join_node->isUsingJoinExpression()) {
        ...
        for (auto &join_using_node : join_using_list.getNodes()) {
          ...
          if (!matcher_node_typed.isMatchingColumn(join_using_column_name)) continue;
          ...
          auto is_column_from_parent_scope = [&scope](const QueryTreeNodePtr &using_node_from_table) {
            if (using_node_from_table->getNodeType() != QueryTreeNodeType::COLUMN) return false;
          ...
          if (is_column_from_parent_scope(join_using_column_nodes.at(0)) || is_column_from_parent_scope(join_using_column_nodes.at(1)))
            continue;
      ...
      for (auto &&left_table_column_with_name : left_table_expression_columns) {
        if (table_expression_column_names_to_skip.contains(left_table_column_with_name.second)) continue;
      ...
      for (auto &&right_table_column_with_name : right_table_expression_columns) {
        if (table_expression_column_names_to_skip.contains(right_table_column_with_name.second)) continue;
    ...
    if (query_node || union_node) {
      ...
      throw Exception(ErrorCodes::LOGICAL_ERROR, "Unqualified matcher {} resolve unexpected table expression. In scope {}",
                      matcher_node_typed.formatASTForErrorMessage(), scope.scope_node->formatASTForErrorMessage());
    }

    auto matched_column_nodes_with_names = getMatchedColumnNodesWithNames(matcher_node, table_expression, table_expression_columns, scope);
...
/** Resolve query tree matcher. Check MatcherNode.h for detailed matcher description. Check ColumnTransformers.h for detailed transformers
 * description.
 *
...
ProjectionNames QueryAnalyzer::resolveMatcher(QueryTreeNodePtr &matcher_node, IdentifierResolveScope &scope) {
  ...
  if (scope.join_use_nulls) {
    ...
    for (const auto &table_expression : scope.registered_table_expression_nodes) {
      ...
      if (!nearest_scope_join_node) continue;
      ...
      for (auto &[node, node_name] : matched_expression_nodes_with_names) {
        ...
        if (!join_identifier_side) continue;
        auto projection_name_it = node_to_projection_name.find(node);
        auto nullable_node = IdentifierResolver::convertJoinedColumnTypeToNullIfNeeded(
            node, node->getResultType(), nearest_scope_join_node->getKind(), join_identifier_side, scope);
        if (nullable_node) {
          node = nullable_node;
      ...
      if (auto *apply_transformer = transformer->as<ApplyColumnTransformerNode>()) {
        ...
        if (apply_transformer->getApplyTransformerType() == ApplyColumnTransformerType::LAMBDA) {
          ...
          throw Exception(
              ...
              transformer->formatASTForErrorMessage(), scope.scope_node->formatASTForErrorMessage());
        }

        execute_apply_transformer = true;
      } else if (auto *except_transformer = transformer->as<ExceptColumnTransformerNode>()) {
        if (apply_transformer_was_used || replace_transformer_was_used) continue;

        if (except_transformer->isColumnMatching(column_name)) {
          if (except_transformer->isStrict()) strict_transformer_to_used_column_names[except_transformer].insert(column_name);
        ...
        if (!replace_expression) continue;

        replace_transformer_was_used = true;

        if (replace_transformer->isStrict()) strict_transformer_to_used_column_names[replace_transformer].insert(column_name);
      ...
      if (execute_apply_transformer || execute_replace_transformer) {
        if (auto *node_list = node->as<ListNode>()) {
          ...
          if (node_list_nodes_size != 1)
            throw Exception(ErrorCodes::UNSUPPORTED_METHOD, "{} transformer {} resolved as list node with size {}. Expected 1. In scope {}",
                            execute_apply_transformer ? "APPLY" : "REPLACE", transformer->formatASTForErrorMessage(), node_list_nodes_size,
                            scope.scope_node->formatASTForErrorMessage());
  ...
  for (auto &[strict_transformer, used_column_names] : strict_transformer_to_used_column_names) {
    ...
    switch (strict_transformer_type) {
      ...
      default: {
        throw Exception(ErrorCodes::LOGICAL_ERROR, "Expected strict EXCEPT or REPLACE column transformer. Actual: type {}. In scope {}",
                        toString(strict_transformer_type), scope.scope_node->formatASTForErrorMessage());
      }
    }

    if (!strict_transformer_column_names) continue;
    ...
    for (size_t i = 0; i < strict_transformer_column_names_size; ++i) {
      ...
      if (!used_column_names.contains(column_name)) non_matched_column_names.push_back(column_name);
    }

    throw Exception(ErrorCodes::BAD_ARGUMENTS, "Strict {} column transformer {} expects following column(s) : {}. In scope {}",
                    toString(strict_transformer_type), strict_transformer->formatASTForErrorMessage(),
                    fmt::join(non_matched_column_names, ", "), scope.scope_node->formatASTForErrorMessage());
  }

  auto original_ast = matcher_node->getOriginalAST();
  matcher_node = std::move(list);
  if (original_ast) matcher_node->setOriginalAST(original_ast);
        ...
        auto replace_identifiers_in_node = [&](QueryTreeNodePtr &node) -> void {
          ...
          std::function<void(QueryTreeNodePtr &)> replace_recursive = [&](QueryTreeNodePtr &current) -> void {
            ...
            if (auto *function_node = current->as<FunctionNode>()) {
              ...
              for (auto &arg : arguments) replace_recursive(arg);
              ...
              for (auto &child_node : nodes) replace_recursive(child_node);
            } else if (auto *sort_node = current->as<SortNode>()) {
              if (auto &expression = sort_node->getExpression()) replace_recursive(expression);
...
ProjectionName QueryAnalyzer::resolveWindow(QueryTreeNodePtr &node, IdentifierResolveScope &scope) {
  ...
  if (!parent_window_name.empty()) {
    ...
    if (!nearest_query_scope) throw Exception(ErrorCodes::BAD_ARGUMENTS, "Window '{}' does not exist.", parent_window_name);
    ...
    if (window_node_it == scope_window_name_to_window_node.end())
      throw Exception(ErrorCodes::BAD_ARGUMENTS, "Window '{}' does not exist. In scope {}", parent_window_name,
    ...
    if (!inserted)
      throw Exception(ErrorCodes::UNSUPPORTED_METHOD, "Recursive window {}. In scope {}", node->formatASTForErrorMessage(),
                      scope.scope_node->formatASTForErrorMessage());
  ...
  ProjectionNames partition_by_projection_names = resolveExpressionNodeList(
      window_node.getPartitionByNode(), scope, false /*allow_lambda_expression*/, false /*allow_table_expression*/);
  ...
  if (window_node.hasFrameBeginOffset()) {
    frame_begin_offset_projection_names = resolveExpressionNode(window_node.getFrameBeginOffsetNode(), scope,
    ...
    if (!window_frame_begin_constant_node || !isNativeNumber(removeNullable(window_frame_begin_constant_node->getResultType())))
      throw Exception(ErrorCodes::BAD_ARGUMENTS,
                      ...
                      window_node.getFrameBeginOffsetNode()->formatASTForErrorMessage(), scope.scope_node->formatASTForErrorMessage());
    ...
    if (frame_begin_offset_projection_names.size() != 1)
      throw Exception(ErrorCodes::LOGICAL_ERROR, "Window FRAME begin offset expected 1 projection name. Actual: {}",
                      frame_begin_offset_projection_names.size());
  }

  if (window_node.hasFrameEndOffset()) {
    frame_end_offset_projection_names = resolveExpressionNode(window_node.getFrameEndOffsetNode(), scope, false /*allow_lambda_expression*/,
    ...
    if (!window_frame_end_constant_node || !isNativeNumber(removeNullable(window_frame_end_constant_node->getResultType())))
      throw Exception(ErrorCodes::BAD_ARGUMENTS,
                      "Window frame begin OFFSET expression must be constant with numeric type. Actual: {}. In scope {}",
                      window_node.getFrameEndOffsetNode()->formatASTForErrorMessage(), scope.scope_node->formatASTForErrorMessage());
  ...
  if (result_projection_name.empty()) {
    result_projection_name = calculateWindowProjectionName(
        node, parent_window_node, parent_window_name, partition_by_projection_names, order_by_projection_names,
 ...
 */
ProjectionNames QueryAnalyzer::resolveLambda(const QueryTreeNodePtr &lambda_node, const QueryTreeNodePtr &lambda_node_to_resolve,
                                             const QueryTreeNodes &lambda_arguments, IdentifierResolveScope &scope) {
  ...
  if (it != lambdas_in_resolve_process.end())
    throw Exception(ErrorCodes::UNSUPPORTED_METHOD, "Recursive lambda {}. In scope {}", lambda_node->formatASTForErrorMessage(),
                    scope.scope_node->formatASTForErrorMessage());
  ...
  if (lambda_arguments_nodes_size != arguments_size)
    throw Exception(ErrorCodes::BAD_ARGUMENTS, "Lambda {} expect {} arguments. Actual: {}. In scope {}",
                    lambda_to_resolve.formatASTForErrorMessage(), lambda_arguments_nodes_size, arguments_size,
                    scope.scope_node->formatASTForErrorMessage());
  ...
  for (size_t i = 0; i < lambda_arguments_nodes_size; ++i) {
    ...
    const auto &lambda_argument_name =
        lambda_argument_identifier ? lambda_argument_identifier->getIdentifier().getFullName() : lambda_argument_column->getColumnName();
    ...
    if (has_expression_node || has_alias_node) {
      throw Exception(ErrorCodes::BAD_ARGUMENTS, "Alias name '{}' inside lambda {} cannot have same name as lambda argument. In scope {}",
                      lambda_argument_name, lambda_argument_node->formatASTForErrorMessage(), scope.scope_node->formatASTForErrorMessage());
    }
  ...
  auto result_projection_names =
      resolveExpressionNode(lambda_to_resolve.getExpression(), scope, false /*allow_lambda_expression*/, false /*allow_table_expression*/);
 ...
 *
 * Steps:
 * 1. If node has alias, replace node with its value in scope alias map. Register alias in expression_aliases_in_resolve_process, to prevent
 * resolving identifier which can bind to expression alias name. Check tryResolveIdentifierFromAliases documentation for additional
 * explanation. Example: SELECT id AS id FROM test_table; SELECT value.value1 AS value FROM test_table;
 *
 ...
 */
ProjectionNames QueryAnalyzer::resolveExpressionNode(QueryTreeNodePtr &node, IdentifierResolveScope &scope, bool allow_lambda_expression,
                                                     bool allow_table_expression, bool ignore_alias, bool allow_niladic_functions) {
  checkStackSize();
  ...
  if (node_alias.empty()) {
    auto projection_name_it = node_to_projection_name.find(node);
    if (projection_name_it != node_to_projection_name.end()) result_projection_names.push_back(projection_name_it->second);
  } else {
    ...
    if (!ignore_alias) scope.aliases.node_to_remove_aliases.push_back(node);
  }
  ...
  switch (node_type) {
    case QueryTreeNodeType::IDENTIFIER: {
      ...
      if (node->hasOriginalAST()) identifier_lookup.original_ast_node = node->getOriginalAST();
      auto resolve_identifier_expression_result =
          tryResolveIdentifier(identifier_lookup, scope, {.allow_to_resolve_niladic_functions = allow_niladic_functions});
      ...
      if (resolved_identifier_node && result_projection_names.empty() &&
          (resolve_identifier_expression_result.isResolvedFromJoinTree() ||
           resolve_identifier_expression_result.isResolvedFromExpressionArguments())) {
      ...
      if (!resolved_identifier_node && allow_lambda_expression)
        resolved_identifier_node = tryResolveIdentifier({unresolved_identifier, IdentifierLookupContext::FUNCTION}, scope,
                                                        {.allow_to_resolve_niladic_functions = allow_niladic_functions})
                                       .resolved_identifier;

      if (!resolved_identifier_node && allow_table_expression) {
        resolved_identifier_node = tryResolveIdentifier({unresolved_identifier, IdentifierLookupContext::TABLE_EXPRESSION}, scope,
                                                        {.allow_to_resolve_niladic_functions = allow_niladic_functions})
                                       .resolved_identifier;
      ...
      if (!resolved_identifier_node) {
        ...
        if (allow_lambda_expression) message_clarification = std::string(" or ") + toStringLowercase(IdentifierLookupContext::FUNCTION);
        ...
        TypoCorrection::collectScopeWithParentScopesValidIdentifiers(unresolved_identifier, scope, true, allow_lambda_expression,
                                                                     allow_table_expression, valid_identifiers);

        auto hints = TypoCorrection::collectIdentifierTypoHints(unresolved_identifier, valid_identifiers);

        throw Exception(ErrorCodes::UNKNOWN_IDENTIFIER, "Unknown {}{} identifier {} in scope {}{}",
                        toStringLowercase(IdentifierLookupContext::EXPRESSION), message_clarification,
                        backQuote(unresolved_identifier.getFullName()), scope.scope_node->formatASTForErrorMessage(),
      ...
      if (node->getNodeType() == QueryTreeNodeType::LIST) {
        ...
        if (resolved_expression_it != resolved_expressions.end()) return resolved_expression_it->second;
        throw Exception(ErrorCodes::LOGICAL_ERROR,
                        "Identifier '{}' resolve into list node and list node projection names are not initialized. In scope {}",
                        unresolved_identifier.getFullName(), scope.scope_node->formatASTForErrorMessage());
      }

      if (result_projection_names.empty()) result_projection_names.push_back(unresolved_identifier.getFullName());
    ...
    case QueryTreeNodeType::COLUMN: {
      ...
      if (result_projection_names.empty()) result_projection_names.push_back(column_node.getColumnName());
    ...
    case QueryTreeNodeType::LAMBDA: {
      if (!allow_lambda_expression)
        throw Exception(ErrorCodes::BAD_ARGUMENTS, "Lambda {} is not allowed in expression context. In scope {}",
                        node->formatASTForErrorMessage(), scope.scope_node->formatASTForErrorMessage());

      if (result_projection_names.empty()) result_projection_names.push_back(PROJECTION_NAME_PLACEHOLDER);
    ...
    case QueryTreeNodeType::UNION: {
      ...
      bool is_correlated_subquery =
          node_type == QueryTreeNodeType::QUERY ? node->as<QueryNode>()->isCorrelated() : node->as<UnionNode>()->isCorrelated();

      if (!allow_table_expression && !is_correlated_subquery) evaluateScalarSubqueryIfNeeded(node, subquery_scope);

      if (result_projection_names.empty()) result_projection_names.push_back(std::move(projection_name));

      break;
    }
    case QueryTreeNodeType::TABLE: {
      if (!allow_table_expression)
        throw Exception(ErrorCodes::BAD_ARGUMENTS, "Table {} is not allowed in expression context. In scope {}",
                        node->formatASTForErrorMessage(), scope.scope_node->formatASTForErrorMessage());

      auto &table_node = node->as<TableNode &>();
      if (result_projection_names.empty()) result_projection_names.push_back(table_node.getStorageID().getFullNameNotQuoted());
  ...
  for (const auto *scope_ptr = &scope; scope_ptr; scope_ptr = scope_ptr->parent_scope) {
    ...
    if (scope_ptr->scope_node->getNodeType() == QueryTreeNodeType::QUERY) break;
  }
 ...
 */
ProjectionNames QueryAnalyzer::resolveExpressionNodeList(QueryTreeNodePtr &node_list, IdentifierResolveScope &scope,
                                                         bool allow_lambda_expression, bool allow_table_expression,
  ...
  for (auto &node : node_list_typed.getNodes()) {
    ...
    auto expression_node_projection_names = resolveExpressionNode(node_to_resolve, scope, allow_lambda_expression, allow_table_expression,
                                                                  false /*ignore_alias*/, allow_niladic_functions);
    ...
    if (auto *expression_list = node_to_resolve->as<ListNode>()) {
      ...
      for (auto &expression_list_node : expression_list->getNodes()) result_nodes.push_back(expression_list_node);
    } else {
      result_nodes.push_back(std::move(node_to_resolve));
    }

    if (expression_node_projection_names.size() != expected_projection_names_size)
      throw Exception(ErrorCodes::LOGICAL_ERROR, "Expression nodes list expected {} projection names. Actual: {}",
                      expected_projection_names_size, expression_node_projection_names.size());

    result_projection_names.insert(result_projection_names.end(), expression_node_projection_names.begin(),
                                   expression_node_projection_names.end());
...
ProjectionNames QueryAnalyzer::resolveSortNodeList(QueryTreeNodePtr &sort_node_list, IdentifierResolveScope &scope) {
  ...
  for (auto &node : sort_node_list_typed.getNodes()) {
    ...
    sort_expression_projection_names =
    ...
    if (auto *sort_column_list_node = sort_expression->as<ListNode>()) {
      ...
      if (sort_column_list_node_size != 1) {
        throw Exception(ErrorCodes::UNSUPPORTED_METHOD,
                        "Sort column node expression resolved into list with size {}. Expected 1. In scope {}", sort_column_list_node_size,
                        scope.scope_node->formatASTForErrorMessage());
      }
    ...
    if (sort_expression_projection_names_size != 1)
      throw Exception(ErrorCodes::LOGICAL_ERROR, "Sort expression expected 1 projection name. Actual: {}",
    ...
    if (sort_node.hasFillFrom()) {
      ...
      if (!constant_node || !isColumnedAsNumber(constant_node->getResultType()))
        throw Exception(ErrorCodes::INVALID_WITH_FILL_EXPRESSION,
                        ...
                        sort_node.getFillFrom()->formatASTForErrorMessage(), scope.scope_node->formatASTForErrorMessage());
      ...
      if (fill_from_expression_projection_names_size != 1)
        throw Exception(ErrorCodes::LOGICAL_ERROR, "Sort node FILL FROM expression expected 1 projection name. Actual: {}",
                        fill_from_expression_projection_names_size);
    }

    if (sort_node.hasFillTo()) {
      fill_to_expression_projection_names =
      ...
      if (!constant_node || !isColumnedAsNumber(constant_node->getResultType()))
        throw Exception(ErrorCodes::INVALID_WITH_FILL_EXPRESSION,
                        ...
                        sort_node.getFillTo()->formatASTForErrorMessage(), scope.scope_node->formatASTForErrorMessage());
      ...
      if (fill_to_expression_projection_names_size != 1)
        throw Exception(ErrorCodes::LOGICAL_ERROR, "Sort node FILL TO expression expected 1 projection name. Actual: {}",
    ...
    if (sort_node.hasFillStep()) {
      ...
      if (!constant_node)
        throw Exception(ErrorCodes::INVALID_WITH_FILL_EXPRESSION,
                        ...
                        sort_node.getFillStep()->formatASTForErrorMessage(), scope.scope_node->formatASTForErrorMessage());
      ...
      if (fill_step_expression_projection_names_size != 1)
        throw Exception(ErrorCodes::LOGICAL_ERROR, "Sort FILL STEP expression expected 1 projection name. Actual: {}",
                        fill_step_expression_projection_names_size);
    }

    if (sort_node.hasFillStaleness()) {
      fill_staleness_expression_projection_names =
      ...
      if (!constant_node)
        throw Exception(ErrorCodes::INVALID_WITH_FILL_EXPRESSION,
                        ...
                        sort_node.getFillStaleness()->formatASTForErrorMessage(), scope.scope_node->formatASTForErrorMessage());
      ...
      if (fill_staleness_expression_projection_names_size != 1)
        throw Exception(ErrorCodes::LOGICAL_ERROR, "Sort FILL STALENESS expression expected 1 projection name. Actual: {}",
                        fill_staleness_expression_projection_names_size);
    }

    auto sort_column_projection_name = calculateSortColumnProjectionName(
        node, sort_expression_projection_names[0],
...
void QueryAnalyzer::validateSortingKeyType(const DataTypePtr &sorting_key_type, const IdentifierResolveScope &scope) const {
  auto check = [&](const IDataType &type) {
    ...
    if (!type.isComparable())
      throw Exception(ErrorCodes::ILLEGAL_COLUMN, "Data type {} is not allowed in ORDER BY keys, because its values are not comparable",
                      type.getName());
  };
  ...
  if (query_node_typed.isGroupByWithGroupingSets()) {
    ...
    for (const auto &grouping_set : query_node_typed.getGroupBy().getNodes()) {
      for (const auto &group_by_elem : grouping_set->as<ListNode>()->getNodes()) {
        ...
        if (scope.group_by_use_nulls) scope.nullable_group_by_keys.insert(group_by_elem);
      }
    }
  } else {
    replaceNodesWithPositionalArguments(query_node_typed.getGroupByNode(), query_node_typed.getProjection().getNodes(), scope);

    resolveExpressionNodeList(query_node_typed.getGroupByNode(), scope, false /*allow_lambda_expression*/,
                              false /*allow_table_expression*/);
...
void QueryAnalyzer::validateGroupByKeyType(const DataTypePtr &group_by_key_type, const IdentifierResolveScope &scope) const {
  if (scope.context->getSettingsRef()[Setting::allow_suspicious_types_in_group_by]) return;
...
void QueryAnalyzer::resolveInterpolateColumnsNodeList(QueryTreeNodePtr &interpolate_node_list, IdentifierResolveScope &scope) {
  ...
  for (auto &interpolate_node : interpolate_node_list_typed.getNodes()) {
    ...
    resolveExpressionNode(interpolate_node_typed.getExpression(), scope, false /*allow_lambda_expression*/,
                          false /*allow_table_expression*/);
    ...
    auto fake_column_node = std::make_shared<ColumnNode>(
        NameAndTypePair(column_to_interpolate_name, interpolate_node_typed.getExpression()->getResultType()), interpolate_node);
...
void QueryAnalyzer::resolveWindowNodeList(QueryTreeNodePtr &window_node_list, IdentifierResolveScope &scope) {
  ...
  for (auto &node : window_node_list_typed.getNodes()) resolveWindow(node, scope);
}
...
NamesAndTypes QueryAnalyzer::resolveProjectionExpressionNodeList(QueryTreeNodePtr &projection_node_list, IdentifierResolveScope &scope) {
  ProjectionNames projection_names =
      resolveExpressionNodeList(projection_node_list, scope, false /*allow_lambda_expression*/, false /*allow_table_expression*/);
  ...
  for (size_t i = 0; i < projection_nodes_size; ++i) {
    ...
    if (!isExpressionNodeType(projection_node->getNodeType()))
      throw Exception(ErrorCodes::UNSUPPORTED_METHOD, "Projection node must be constant, function, column, query or union");
  ...
  while (!join_tree_node_ptrs_to_process_queue.empty()) {
    ...
    switch (current_join_tree_node_type) {
      case QueryTreeNodeType::IDENTIFIER: {
        ...
        if (current_join_tree_node->hasOriginalAST()) table_identifier_lookup.original_ast_node = current_join_tree_node->getOriginalAST();
        ...
        if (!resolved_identifier)
          throw Exception(ErrorCodes::UNKNOWN_TABLE, "Unknown table expression identifier '{}' in scope {}",
                          from_table_identifier.getIdentifier().getFullName(), scope.scope_node->formatASTForErrorMessage());

        resolved_identifier = resolved_identifier->clone();
        if (table_identifier_lookup.original_ast_node) resolved_identifier->setOriginalAST(table_identifier_lookup.original_ast_node);
        ...
        if (resolved_identifier_query_node || resolved_identifier_union_node) {
          ...
          if (is_cte) cte_copy_to_original_map.emplace(resolved_identifier.get(), table_identifier_resolve_result.resolved_identifier);

          if (table_expression_modifiers.has_value()) {
            throw Exception(ErrorCodes::UNSUPPORTED_METHOD, "Table expression modifiers {} are not supported for subquery {}",
                            table_expression_modifiers->formatForErrorMessage(), resolved_identifier->formatASTForErrorMessage());
          }
          ...
          throw Exception(ErrorCodes::LOGICAL_ERROR, "Identifier in JOIN TREE '{}' resolved into unexpected table expression. In scope {}",
                          from_table_identifier.getIdentifier().getFullName(), scope.scope_node->formatASTForErrorMessage());
        }
      ...
      case QueryTreeNodeType::CROSS_JOIN: {
        ...
        for (auto &expr : join.getTableExpressions()) join_tree_node_ptrs_to_process_queue.push_back(&expr);
      ...
      default: {
        throw Exception(ErrorCodes::BAD_ARGUMENTS,
                        ...
                        current_join_tree_node->getNodeTypeName(), current_join_tree_node->formatASTForErrorMessage(),
                        scope.scope_node->formatASTForErrorMessage());
      }
    }
  }
...
void QueryAnalyzer::initializeTableExpressionData(const QueryTreeNodePtr &table_expression_node, IdentifierResolveScope &scope) {
  ...
  if (table_expression_data_it != scope.table_expression_node_to_data.end()) return;
  ...
  if (table_expression_node->hasAlias()) table_expression_data.table_expression_name = table_expression_node->getAlias();
...
bool findIdentifier(const FunctionNode &function) {
  for (const auto &argument : function.getArguments()) {
    if (argument->as<IdentifierNode>()) return true;
    if (const auto *f = argument->as<FunctionNode>(); f && findIdentifier(*f)) return true;
  }
  return false;
}

/// Resolve table function node in scope
void QueryAnalyzer::resolveTableFunction(QueryTreeNodePtr &table_function_node, IdentifierResolveScope &scope,
                                         QueryExpressionsAliasVisitor &expressions_visitor, bool nested_table_function) {
  auto &table_function_node_typed = table_function_node->as<TableFunctionNode &>();

  if (!nested_table_function) expressions_visitor.visit(table_function_node_typed.getArgumentsNode());
  ...
  if (!table_function_ptr) {
    ...
    for (const auto &argument : table_function_node_typed.getArguments()) {
      if (auto *arg_func = argument->as<FunctionNode>()) {
        if (arg_func->getFunctionName() != "equals") continue;
    ...
    auto parameterized_view_storage = context->buildParameterizedViewStorage(database_name, table_name, view_params);
    ...
    if (!hints.empty())
      throw Exception(ErrorCodes::UNKNOWN_FUNCTION, "Unknown table function {}. Maybe you meant: {}", table_function_name,
                      DB::toString(hints));
  ...
  for (size_t table_function_argument_index = 0; table_function_argument_index < table_function_arguments_size;
       ++table_function_argument_index) {
    auto &table_function_argument = table_function_arguments[table_function_argument_index];

    auto skip_argument_index_it =
        std::find(skip_analysis_arguments_indexes.begin(), skip_analysis_arguments_indexes.end(), table_function_argument_index);
    ...
    if (auto *identifier_node = table_function_argument->as<IdentifierNode>()) {
      ...
      auto identifier_resolve_result = tryResolveIdentifier({unresolved_identifier, IdentifierLookupContext::EXPRESSION}, scope,
                                                            {.allow_to_resolve_niladic_functions = false});
    ...
    try {
      resolveExpressionNode(table_function_argument, scope, false /*allow_lambda_expression*/, false /*allow_table_expression*/,
                            false /*ignore_alias*/, false /*allow_niladic_functions*/);
    ...
    if (auto *expression_list = table_function_argument->as<ListNode>()) {
      if (expression_list->getNodes().empty())
        throw Exception(ErrorCodes::BAD_ARGUMENTS,
                        ...
                        table_function_name, table_function_argument_index + 1, table_function_argument->formatASTForErrorMessage());

      for (auto &expression_list_node : expression_list->getNodes()) result_table_function_arguments.push_back(expression_list_node);
    } else {
      result_table_function_arguments.push_back(table_function_argument);
    }
  }
  ...
  if (!nested_table_function && !scope.subquery_depth && use_structure_from_insertion_table_in_table_functions &&
      scope_context->hasInsertionTableColumnsDescription() && table_function_ptr->needStructureHint() &&
    ...
    const auto &insert_column_names = scope_context->hasInsertionTableColumnNames() ? *scope_context->getInsertionTableColumnNames()
                                                                                    : insert_columns.getOrdinary().getNames();
    ...
    for (; expression != expression_list.end() && insert_column_name_it != insert_column_names_end; ++expression) {
      ...
      } else if (auto *matcher_node = (*expression)->as<MatcherNode>();
                 matcher_node && matcher_node->getMatcherType() == MatcherNodeType::ASTERISK) {
        if (asterisk) {
          if (use_structure_from_insertion_table_in_table_functions == 1)
    ...
    if (use_structure_from_insertion_table_in_table_functions == 2 && !asterisk) {
      ...
      if (table_function_ptr->getName() == "input")
        use_columns_from_insert_query =
            FormatFactory::instance().checkIfFormatSupportsSubsetOfColumns(scope.context->getInsertFormat(), scope.context);
      else
  ...
  ContextPtr execution_context = scope_context->getQueryContext();
  {
    ...
    while (scope_to_check != nullptr) {
      if (auto *query_node = scope_to_check->scope_node->as<QueryNode>();
        ...
        auto *nearest_query_scope = scope.getNearestQueryScope();
        if (nearest_query_scope) execution_context = nearest_query_scope->scope_node->as<QueryNode &>().getMutableContext();
        break;
      }
      scope_to_check = scope_to_check->parent_scope;
    }
  }
  auto table_function_storage =
      scope_context->getQueryContext()->executeTableFunction(table_function_ast, table_function_ptr, execution_context);
  table_function_node_typed.resolve(std::move(table_function_ptr), std::move(table_function_storage), scope_context,
                                    std::move(skip_analysis_arguments_indexes));
}

/// Resolve array join node in scope
void QueryAnalyzer::resolveArrayJoin(QueryTreeNodePtr &array_join_node, IdentifierResolveScope &scope,
  ...
  if (array_join_nodes_size == 0)
    throw Exception(ErrorCodes::NUMBER_OF_ARGUMENTS_DOESNT_MATCH, "ARRAY JOIN requires at least single expression");

  /// Register expression aliases in the scope
  for (const auto &elem : array_join_nodes) {
    for (auto &child : elem->getChildren()) {
      if (child) expressions_visitor.visit(child);
    }
  }
  ...
  for (auto &array_join_expression : array_join_nodes) {
    ...
    auto process_array_join_expression = [&](const QueryTreeNodePtr &expression) {
      ...
      if (!is_array_type && !is_map_type)
        throw Exception(ErrorCodes::TYPE_MISMATCH, "ARRAY JOIN {} requires expression {} with Array or Map type. Actual: {}. In scope {}",
                        array_join_node_typed.formatASTForErrorMessage(), expression->formatASTForErrorMessage(), result_type->getName(),
                        scope.scope_node->formatASTForErrorMessage());

      if (is_map_type) result_type = assert_cast<const DataTypeMap &>(*result_type).getNestedType();
      ...
      if (array_join_column_names.contains(array_join_column_name))
        throw Exception(ErrorCodes::BAD_ARGUMENTS, "ARRAY JOIN {} multiple columns with name {}. In scope {}",
                        array_join_node_typed.formatASTForErrorMessage(), array_join_column_name,
                        scope.scope_node->formatASTForErrorMessage());
    ...
    if (auto *columns_list = array_join_expression->as<ListNode>()) {
      for (auto &array_join_subexpression : columns_list->getNodes()) process_array_join_expression(array_join_subexpression);
    } else {
  ...
  if (array_join_column_expressions.empty())
    throw Exception(ErrorCodes::NUMBER_OF_ARGUMENTS_DOESNT_MATCH, "ARRAY JOIN requires at least one expression after resolving COLUMNS");
...
void QueryAnalyzer::checkDuplicateTableNamesOrAliasForPasteJoin(const JoinNode &join_node, IdentifierResolveScope &scope) {
  ...
  if (join_node.getKind() != JoinKind::Paste) return;
  ...
  if (left_node)
    for (const auto &name_and_type : left_node->getProjectionColumns()) column_names.push_back(name_and_type.name);
  if (right_node)
    for (const auto &name_and_type : right_node->getProjectionColumns()) column_names.push_back(name_and_type.name);

  if (column_names.empty()) throw Exception(ErrorCodes::LOGICAL_ERROR, "Names of projection columns cannot be empty");
...
void QueryAnalyzer::resolveCrossJoin(QueryTreeNodePtr &cross_join_node, IdentifierResolveScope &scope,
                                     QueryExpressionsAliasVisitor &expressions_visitor) {
  ...
  while (!nodes_to_process.empty()) {
    ...
    switch (table_expression->getNodeType()) {
      case QueryTreeNodeType::TABLE: {
        ...
        for (const auto &column : table_node->getStorageSnapshot()->getColumns(get_column_options)) existing_columns.insert(column.name);
      ...
      case QueryTreeNodeType::QUERY: {
        ...
        for (const auto &column : query_node->getProjectionColumns()) existing_columns.insert(column.name);
      ...
      case QueryTreeNodeType::UNION: {
        ...
        for (const auto &column : union_node->computeProjectionColumns()) existing_columns.insert(column.name);
        break;
      }
      case QueryTreeNodeType::JOIN: {
      ...
      case QueryTreeNodeType::CROSS_JOIN: {
        ...
        for (const auto &table_expr : cross_join_node->getTableExpressions()) nodes_to_process.push(table_expr.get());
        break;
      }
      default: {
        return false;
...
/// Resolve join node in scope
void QueryAnalyzer::resolveJoin(QueryTreeNodePtr &join_node, IdentifierResolveScope &scope,
                                QueryExpressionsAliasVisitor &expressions_visitor) {
  ...
  if (isCorrelatedQueryOrUnionNode(join_node_typed.getLeftTableExpression()))
    throw Exception(ErrorCodes::NOT_IMPLEMENTED, "Correlated subqueries are not supported in JOINs yet, but found in expression: {}",
  ...
  if (join_node_typed.isOnJoinExpression()) {
    ...
    for (auto &join_using_node : join_using_list.getNodes()) {
      ...
      if (!identifier_node)
        throw Exception(ErrorCodes::BAD_ARGUMENTS, "JOIN {} USING clause expected identifier. Actual: {}",
                        join_node_typed.formatASTForErrorMessage(), join_using_node->formatASTForErrorMessage());
      ...
      if (join_using_identifiers.contains(identifier_full_name))
        throw Exception(ErrorCodes::BAD_ARGUMENTS, "JOIN {} identifier '{}' appears more than once in USING clause",
                        join_node_typed.formatASTForErrorMessage(), identifier_full_name);
        ...
        for (const auto &projection_node : projection_list.getNodes()) {
          if (projection_node->hasAlias() && identifier_full_name_ == projection_node->getAlias()) {
            ...
            if (resolved_nodes.size() == 1) {
              ...
              if (!getColumnsFromTableExpression(left_table_expression, existing_columns)) return nullptr;

              NameAndTypePair column_name_type(identifier_full_name_, resolved_nodes.front()->getResultType());
              while (existing_columns.contains(column_name_type.name)) column_name_type.name = "_" + column_name_type.name;
              ...
              if (!is_single_source) return nullptr;
      ...
      if (settings[Setting::analyzer_compatibility_join_using_top_level_identifier])
        result_left_table_expression =
            try_resolve_identifier_from_query_projection(identifier_full_name, join_node_typed.getLeftTableExpression(), scope);
      ...
      if (!result_left_table_expression) {
        ...
        result_left_table_expression =
            identifier_resolver.tryResolveIdentifierFromJoinTreeNode(identifier_lookup, join_node_typed.getLeftTableExpression(), scope)
                .resolved_identifier;
      }
      ...
      if (!result_left_table_expression) {
        ...
        throw Exception(
            ErrorCodes::UNKNOWN_IDENTIFIER, "JOIN {} using identifier '{}' cannot be resolved from left table expression{}. In scope {}",
            join_node_typed.formatASTForErrorMessage(), identifier_full_name, extra_message, scope.scope_node->formatASTForErrorMessage());
      }
      ...
      if (result_left_table_expression->getNodeType() != QueryTreeNodeType::COLUMN) {
        if (result_left_table_expression->getNodeType() == QueryTreeNodeType::FUNCTION) {
          ...
          if (function_node.getFunctionName() == "firstNonDefault" &&
              std::ranges::all_of(function_node.getArguments().getNodes(), is_column_node)) {
      ...
      if (result_table_expressions.empty())
        throw Exception(ErrorCodes::UNSUPPORTED_METHOD,
                        ...
                        join_node_typed.formatASTForErrorMessage(), identifier_full_name, scope.scope_node->formatASTForErrorMessage());
      ...
      auto result_right_table_expression =
          identifier_resolver.tryResolveIdentifierFromJoinTreeNode(identifier_lookup, join_node_typed.getRightTableExpression(), scope)
              .resolved_identifier;
      if (!result_right_table_expression)
        throw Exception(ErrorCodes::UNKNOWN_IDENTIFIER,
                        "JOIN {} using identifier '{}' cannot be resolved from right table expression. In scope {}",
                        join_node_typed.formatASTForErrorMessage(), right_name, scope.scope_node->formatASTForErrorMessage());
      ...
      if (!common_type)
        throw Exception(
            ErrorCodes::NO_COMMON_TYPE, "JOIN {} cannot infer common type for {} and {} in USING for identifier '{}'. In scope {}",
            join_node_typed.formatASTForErrorMessage(), result_left_table_expression->getResultType()->getName(),
            result_right_table_expression->getResultType()->getName(), right_name, scope.scope_node->formatASTForErrorMessage());
 ...
 */
void QueryAnalyzer::resolveQueryJoinTreeNode(QueryTreeNodePtr &join_tree_node, IdentifierResolveScope &scope,
                                             QueryExpressionsAliasVisitor &expressions_visitor) {
  ...
  switch (from_node_type) {
    ...
    case QueryTreeNodeType::UNION: {
      ...
      if (original_cte_node) ctes_in_resolve_process.insert(original_cte_node);

      resolveExpressionNode(join_tree_node, scope, false /*allow_lambda_expression*/, true /*allow_table_expression*/,
                            true /*ignore_alias=*/);

      if (original_cte_node) ctes_in_resolve_process.erase(original_cte_node);
      break;
    }
    case QueryTreeNodeType::TABLE_FUNCTION: {
    ...
    case QueryTreeNodeType::IDENTIFIER: {
      throw Exception(ErrorCodes::LOGICAL_ERROR, "Identifiers in FROM section must be already resolved. Node {}, scope {}",
                      join_tree_node->formatASTForErrorMessage(), scope.scope_node->formatASTForErrorMessage());
    }
    default: {
      throw Exception(ErrorCodes::BAD_ARGUMENTS,
  ...
  auto add_table_expression_alias_into_scope = [&](const QueryTreeNodePtr &table_expression_node) {
    ...
    if (!inserted)
      throw Exception(
          ...
          alias_name, table_expression_node->formatASTForErrorMessage(), it->second->formatASTForErrorMessage());
  };
...
void QueryAnalyzer::resolveQuery(const QueryTreeNodePtr &query_node, IdentifierResolveScope &scope) {
   ...
   */
  if (query_node_typed.isResolved()) return;
  ...
  if (!query_node_typed.getProjection().getNodes().empty()) visitor.visit(query_node_typed.getProjectionNode());

  if (query_node_typed.getPrewhere()) visitor.visit(query_node_typed.getPrewhere());

  if (query_node_typed.getWhere()) visitor.visit(query_node_typed.getWhere());

  if (query_node_typed.hasGroupBy()) visitor.visit(query_node_typed.getGroupByNode());

  if (query_node_typed.hasHaving()) visitor.visit(query_node_typed.getHaving());

  if (query_node_typed.hasWindow()) visitor.visit(query_node_typed.getWindowNode());

  if (query_node_typed.hasQualify()) visitor.visit(query_node_typed.getQualify());

  if (query_node_typed.hasOrderBy()) visitor.visit(query_node_typed.getOrderByNode());

  if (query_node_typed.hasInterpolate()) visitor.visit(query_node_typed.getInterpolate());

  if (query_node_typed.hasLimitByLimit()) visitor.visit(query_node_typed.getLimitByLimit());

  if (query_node_typed.hasLimitByOffset()) visitor.visit(query_node_typed.getLimitByOffset());

  if (query_node_typed.hasLimitBy()) visitor.visit(query_node_typed.getLimitByNode());

  if (query_node_typed.hasLimit()) visitor.visit(query_node_typed.getLimit());

  if (query_node_typed.hasOffset()) visitor.visit(query_node_typed.getOffset());
  ...
  for (auto &node : with_nodes) {
    ...
    if (!inserted)
      throw Exception(ErrorCodes::MULTIPLE_EXPRESSIONS_FOR_ALIAS, "CTE with name {} already exists. In scope {}", cte_name,
                      scope.scope_node->formatASTForErrorMessage());
  }
  ...
  for (auto &window_node : query_node_typed.getWindow().getNodes()) {
    ...
    if (!inserted)
      throw Exception(ErrorCodes::BAD_ARGUMENTS, "Window '{}' is already defined. In scope {}", window_node_typed.getAlias(),
                      scope.scope_node->formatASTForErrorMessage());
  }
  ...
  if (!scope.group_by_use_nulls) {
    ...
    if (query_node_typed.getProjection().getNodes().empty())
      throw Exception(ErrorCodes::EMPTY_LIST_OF_COLUMNS_QUERIED, "Empty list of columns in projection. In scope {}",
                      scope.scope_node->formatASTForErrorMessage());
  }
  ...
  if (query_node_typed.hasGroupBy()) resolveGroupByNode(query_node_typed, scope);
  ...
  if (query_node_typed.hasWindow()) resolveWindowNodeList(query_node_typed.getWindowNode(), scope);
  ...
  if (query_node_typed.hasInterpolate()) resolveInterpolateColumnsNodeList(query_node_typed.getInterpolate(), scope);
  ...
  if (query_node_typed.hasLimitBy()) {
    ...
    resolveExpressionNodeList(query_node_typed.getLimitByNode(), scope, false /*allow_lambda_expression*/,
                              false /*allow_table_expression*/);
  }
  ...
  for (const auto &node_with_duplicated_alias : scope.aliases.nodes_with_duplicated_aliases) {
    ...
    if (it != scope.aliases.alias_name_to_expression_node.end()) {
      ...
      if (!matched)
        ...
        if (const auto *constant_node = original_node->as<ConstantNode>())
          if (const auto &source_expression = constant_node->getSourceExpression()) matched = source_expression->isEqual(*node);

      if (!matched)
        throw Exception(ErrorCodes::MULTIPLE_EXPRESSIONS_FOR_ALIAS, "Multiple expressions {} and {} for alias {}. In scope {}",
                        node->formatASTForErrorMessage(), original_node->formatASTForErrorMessage(), node_alias,
                        scope.scope_node->formatASTForErrorMessage());
    }
    ...
    if (!has_node_in_alias_table)
      throw Exception(ErrorCodes::LOGICAL_ERROR, "Node {} with duplicate alias {} does not exist in alias table. In scope {}",
                      node->formatASTForErrorMessage(), node_alias, scope.scope_node->formatASTForErrorMessage());
  ...
  for (const auto &column : projection_columns) {
    if (isNotCreatable(column.type))
      throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT, "Invalid projection column with type {}. In scope {}", column.type->getName(),
                      scope.scope_node->formatASTForErrorMessage());
  }
...
void QueryAnalyzer::resolveUnion(const QueryTreeNodePtr &union_node, IdentifierResolveScope &scope) {
  ...
  if (union_node_typed.isResolved()) return;
  ...
  if (union_node_typed.isCTE() && union_node_typed.isRecursiveCTE()) {
    ...
    auto &non_recursive_query_mutable_context = non_recursive_query_is_query_node
                                                    ? non_recursive_query->as<QueryNode &>().getMutableContext()
    ...
    auto temporary_table_columns = non_recursive_query_is_query_node ? non_recursive_query->as<QueryNode &>().getProjectionColumns()
    ...
    auto temporary_table_holder = std::make_shared<TemporaryTableHolder>(
        ...
        ColumnsDescription{NamesAndTypesList{temporary_table_columns.begin(), temporary_table_columns.end()}}, ConstraintsDescription{},
  ...
  for (size_t i = recursive_cte_table.has_value(); i < queries_nodes_size; ++i) {
    ...
    if (recursive_cte_table_node) subquery_scope.expression_argument_name_to_node[union_node_typed.getCTEName()] = recursive_cte_table_node;
    ...
    if (query_node_type == QueryTreeNodeType::QUERY) {
      ...
      throw Exception(ErrorCodes::UNSUPPORTED_METHOD, "UNION unsupported node {}. In scope {}", query_node->formatASTForErrorMessage(),
                      scope.scope_node->formatASTForErrorMessage());
    }
  }
  ...
  if (recursive_cte_table && isStorageUsedInTree(recursive_cte_table->storage, union_node.get())) {
    if (union_node_typed.getUnionMode() != SelectUnionMode::UNION_ALL)
      throw Exception(ErrorCodes::UNSUPPORTED_METHOD,
                      ...
                      union_node_typed.formatASTForErrorMessage(), toString(union_node_typed.getUnionMode()));
