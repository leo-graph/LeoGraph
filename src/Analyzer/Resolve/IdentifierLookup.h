...
inline const char* toString(IdentifierLookupContext identifier_lookup_context) {
  switch (identifier_lookup_context) {
    case IdentifierLookupContext::EXPRESSION:
      return "EXPRESSION";
    case IdentifierLookupContext::FUNCTION:
      return "FUNCTION";
...
struct IdentifierLookup {
  ...
  bool isExpressionLookup() const { return lookup_context == IdentifierLookupContext::EXPRESSION; }

  bool isFunctionLookup() const { return lookup_context == IdentifierLookupContext::FUNCTION; }

  bool isTableExpressionLookup() const { return lookup_context == IdentifierLookupContext::TABLE_EXPRESSION; }

  String dump() const { return identifier.getFullName() + ' ' + toString(lookup_context); }
};
...
[[maybe_unused]] inline bool operator!=(const IdentifierLookup& lhs, const IdentifierLookup& rhs) { return !(lhs == rhs); }
...
inline const char* toString(IdentifierResolvePlace resolved_identifier_place) {
  switch (resolved_identifier_place) {
    case IdentifierResolvePlace::NONE:
      return "NONE";
    case IdentifierResolvePlace::EXPRESSION_ARGUMENTS:
      return "EXPRESSION_ARGUMENTS";
    case IdentifierResolvePlace::ALIASES:
      return "ALIASES";
    case IdentifierResolvePlace::JOIN_TREE:
      return "JOIN_TREE";
    case IdentifierResolvePlace::CTE:
      return "CTE";
    case IdentifierResolvePlace::DATABASE_CATALOG:
      return "DATABASE_CATALOG";
    case IdentifierResolvePlace::NILADIC_FUNCTION:
      return "NILADIC_FUNCTION";
  }
}
...
struct IdentifierResolveResult {
 ...
 private:
  bool check_invariant() const noexcept { return (resolved_identifier == nullptr) == (resolve_place == IdentifierResolvePlace::NONE); }
};
...
struct IdentifierResolveContext {
  ...
  IdentifierResolveContext& resolveAliasesAt(IdentifierResolveScope* scope_to_resolve_alias_expression_) {
    if (!scope_to_resolve_alias_expression) scope_to_resolve_alias_expression = scope_to_resolve_alias_expression_;
    return *this;
  }
};
