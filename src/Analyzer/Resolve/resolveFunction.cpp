...
void checkFunctionNodeHasEmptyNullsAction(FunctionNode const &node) {
  if (node.getNullsAction() != NullsAction::EMPTY)
    throw Exception(ErrorCodes::SYNTAX_ERROR, "Function with name {} cannot use {} NULLS", backQuote(node.getFunctionName()),
...
bool isNullConstant(const QueryTreeNodePtr &node) {
  if (const auto *const_node = node->as<ConstantNode>()) return const_node->getValue().isNull();
  return false;
}
...
std::pair<QueryTreeNodePtr, ProjectionNames> QueryAnalyzer::makeNullSafeHas(QueryTreeNodePtr array_arg,    // [1,2,number]
  ...
  auto null_const = std::make_shared<ConstantNode>(Field{}, std::make_shared<DataTypeNullable>(std::make_shared<DataTypeUInt8>()));
...
ProjectionNames QueryAnalyzer::buildHasExpression(QueryTreeNodePtr &node, QueryTreeNodePtr array_arg, QueryTreeNodePtr element_arg,
                                                  bool is_not_in, bool transform_null_in, const ProjectionNames &arguments_projection_names,
                                                  const ProjectionNames &parameters_projection_names, IdentifierResolveScope &scope) {
...
ProjectionNames QueryAnalyzer::handleNullInTuple(const QueryTreeNodes &tuple_args, const std::string &function_name,
                                                 const ProjectionNames &parameters_projection_names,
                                                 const ProjectionNames &arguments_projection_names, IdentifierResolveScope &scope,
...
QueryTreeNodePtr QueryAnalyzer::convertTupleToArray(const QueryTreeNodes &tuple_args, const QueryTreeNodePtr &in_first_argument,
                                                    IdentifierResolveScope &scope) {
  ...
  bool has_null = std::any_of(tuple_args.begin(), tuple_args.end(), [](const auto &arg) { return isNullConstant(arg); });
  ...
  for (const auto &arg : tuple_args) array_arguments_list->getNodes().push_back(castNodeToType(arg, common_type, scope));
...
QueryTreeNodePtr QueryAnalyzer::castNodeToType(const QueryTreeNodePtr &node, const DataTypePtr &target_type,
                                               IdentifierResolveScope &scope) {
  if (node->getResultType()->equals(*target_type)) return node;

  auto cast_node = std::make_shared<FunctionNode>("CAST");
  auto cast_args = std::make_shared<ListNode>();
  cast_args->getNodes().push_back(node);
  cast_args->getNodes().push_back(std::make_shared<ConstantNode>(target_type->getName(), std::make_shared<DataTypeString>()));
 ...
 * 3. If function is count function, that take unqualified ASTERISK matcher, remove it from its arguments. Example: SELECT count(*) FROM
 * test_table;
 ...
 * 7. If function name identifier was not resolved as function in current scope, try to lookup lambda from sql user defined functions
 * factory.
 * 8. If function was resolve as lambda from step 2 or 7, then resolve lambda using function arguments and replace function node with lambda
 * result. After than function node is resolved.
 * 9. If function was not resolved during step 6 as lambda, then try to resolve function as window function or executable user defined
 * function or ordinary function or aggregate function.
 *
 ...
 *
 * 8. If function was resolved as non aggregate function. Then if some of function arguments are lambda expressions, their result types need
 * to be initialized and they must be resolved.
...
ProjectionNames QueryAnalyzer::resolveFunction(QueryTreeNodePtr &node, IdentifierResolveScope &scope, bool allow_niladic_functions) {
  ...
  auto parameters_projection_names =
      resolveExpressionNodeList(function_node_ptr->getParametersNode(), scope, false /*allow_lambda_expression*/,
                                false /*allow_table_expression*/, allow_niladic_functions);
  ...
  for (auto &parameter_node : parameters_nodes) {
    ...
    if (!constant_node)
      throw Exception(ErrorCodes::BAD_ARGUMENTS, "Parameter for function '{}' expected to have constant value. Actual: {}. In scope {}",
                      function_name, parameter_node->formatASTForErrorMessage(), scope.scope_node->formatASTForErrorMessage());
   ...
   */
  if ((is_special_function_dict_get || is_special_function_join_get) && !function_node_ptr->getArguments().getNodes().empty() &&
    ...
    if (resolve_result.isResolved()) {
      ...
      if (parts_size < 1 || parts_size > 2)
        throw Exception(ErrorCodes::INVALID_IDENTIFIER,
                        "Expected {} function first argument identifier to contain 1 or 2 parts. Actual '{}'. In scope {}", function_name,
      ...
      if (is_special_function_dict_get) {
        ...
        if (!table_node)
          throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT, "Function {} first argument expected table identifier '{}'. In scope {}",
                          function_name, identifier.getFullName(), scope.scope_node->formatASTForErrorMessage());

        auto &table_node_typed = table_node->as<TableNode &>();
        if (!std::dynamic_pointer_cast<StorageJoin>(table_node_typed.getStorage()))
          throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT, "Function {} table '{}' should have engine StorageJoin. In scope {}",
                          function_name, identifier.getFullName(), scope.scope_node->formatASTForErrorMessage());
      }
  ...
  if (is_special_function_if && !function_node_ptr->getArguments().getNodes().empty()) {
    ...
    if (constant_condition.has_value() && if_function_arguments.size() == 3) {
      ...
      try {
        resolveExpressionNode(possibly_invalid_argument_node, scope, false /*allow_lambda_expression*/, false /*allow_table_expression*/,
                              allow_niladic_functions);
      ...
      if (apply_constant_if_optimization) {
        auto result_projection_names = resolveExpressionNode(constant_if_result_node, scope, false /*allow_lambda_expression*/,
                                                             false /*allow_table_expression*/, allow_niladic_functions);
  ...
  /// Replace IN (subquery)
  /// NOTE: the resulting subquery in the argument of EXISTS will have correlated column x, that's why this rewriting has to be before
  /// handling EXISTS which is done below in 'if (is_special_function_exists)' case.
  if (is_special_function_in && (function_name == "in" || function_name == "notIn") &&
    ...
    resolveExpressionNode(in_first_argument, scope, true /*allow_lambda_expression*/, true /*allow_table_expression*/,
                          allow_niladic_functions);
    ...
    if (!in_first_argument->as<ConstantNode>()) {
      ...
      resolveExpressionNode(in_second_argument, scope, true /*allow_lambda_expression*/, true /*allow_table_expression*/,
                            allow_niladic_functions);
      ...
      if (in_second_argument->as<QueryNode>()) {
        ...
        if (subquery_projection_columns.size() == 1) {
          ...
          for (const auto &c : subquery_projection_columns) wrapper_tuple_element_types.push_back(c.type);
        ...
        exists_function_node_ptr->getArguments().getNodes() = {std::move(new_exists_subquery)};

        if (is_function_not_in) {
          /// NOT IN is rewritten to NOT EXISTS
          function_node_ptr = std::make_shared<FunctionNode>("not");
          function_node_ptr->getArguments().getNodes() = {std::move(exists_function_node_ptr)};
  ...
  if (is_special_function_exists) {
    ...
    if (exists_subquery_argument_node_type != QueryTreeNodeType::QUERY && exists_subquery_argument_node_type != QueryTreeNodeType::UNION) {
      throw Exception(ErrorCodes::BAD_ARGUMENTS, "Function 'exists' expects a subquery argument. Actual: {}. In scope {}",
                      exists_subquery_argument->formatASTForErrorMessage(), scope.scope_node->formatASTForErrorMessage());
    }
    ...
    auto exists_arguments_projection_names = resolveExpressionNode(new_exists_argument, scope, true /*allow_lambda_expression*/,
                                                                   true /*allow_table_expression*/, allow_niladic_functions);

    if (new_exists_subquery->isCorrelated()) {
      function_node_ptr->getArguments().getNodes() = {std::move(new_exists_argument)};
      ...
      function_node_ptr->resolveAsFunction(
          std::make_shared<FunctionToFunctionBaseAdaptor>(function_exists, DataTypes{}, function_exists->getReturnTypeImpl({})));
      ...
      if (only_analyze || !scope.context->getSettingsRef()[Setting::execute_exists_as_scalar_subquery]) {
        ...
        function_node_ptr->getArguments().getNodes() = {constant, std::move(new_exists_argument)};
  ...
  auto arguments_projection_names =
      resolveExpressionNodeList(function_node_ptr->getArgumentsNode(), scope, true /*allow_lambda_expression*/,
                                allow_table_expressions /*allow_table_expression*/, allow_niladic_functions);

  /// Mask arguments if needed
  if (!scope.context->getSettingsRef()[Setting::format_display_secrets_in_show_and_select]) {
    if (FunctionSecretArgumentsFinder::Result secret_arguments = FunctionSecretArgumentsFinderTreeNode(*function_node_ptr).getResult();
        secret_arguments.count) {
  ...
  if (is_special_function_in) {
    ...
    if (table_node) {
      ...
      if (auto *non_const_set_candidate = in_second_argument->as<FunctionNode>()) {
        ...
        const bool is_not_in = (function_name == "notIn" || function_name == "globalNotIn" || function_name == "notNullIn" ||
        ...
        bool is_array_type =
            (candidate_name == "array") || (non_const_set_candidate->isResolved() && isArray(non_const_set_candidate->getResultType()));
        bool is_tuple_type = (candidate_name == "tuple");
        bool is_not_array_or_tuple_type = non_const_set_candidate->isResolved() && !isArray(non_const_set_candidate->getResultType()) &&
                                          !isTuple(non_const_set_candidate->getResultType());

        /// Case 1: array(..) or any function returning Array type -> rewrite to has()
        if (is_array_type)
          return buildHasExpression(node, fn_args[1], fn_args[0], is_not_in, transform_null_in, arguments_projection_names,
                                    parameters_projection_names, scope);
        ...
        if (is_tuple_type && in_first_argument->getNodeType() != QueryTreeNodeType::LAMBDA) {
          ...
          if (left_is_null) {
            if (transform_null_in)
              return handleNullInTuple(tuple_args, function_name, parameters_projection_names, arguments_projection_names, scope, node);

            auto proj = calculateFunctionProjectionName(node, parameters_projection_names, arguments_projection_names);
            node = std::make_shared<ConstantNode>(Field{}, std::make_shared<DataTypeNullable>(std::make_shared<DataTypeUInt8>()));
            return ProjectionNames{proj};
          }
          ...
          return buildHasExpression(node, array_arg, in_first_argument, is_not_in, transform_null_in, arguments_projection_names,
                                    parameters_projection_names, scope);
        }
  ...
  for (size_t function_argument_index = 0; function_argument_index < function_arguments_size; ++function_argument_index) {
    ...
    if (!argument_column.type)
      throw Exception(ErrorCodes::LOGICAL_ERROR, "Function '{}' argument is not resolved. In scope {}", function_name,
  ...
  ProjectionNames result_projection_names = {
      calculateFunctionProjectionName(node, parameters_projection_names, arguments_projection_names)};
  ...
  if (!function_node.isWindowFunction()) {
    ...
    if (lambda_expression_untyped) {
      ...
      if (!lambda_expression)
        throw Exception(ErrorCodes::LOGICAL_ERROR, "Function identifier '{}' must be resolved as lambda. Actual: {}. In scope {}",
                        function_node.getFunctionName(), lambda_expression_untyped->formatASTForErrorMessage(),
                        scope.scope_node->formatASTForErrorMessage());

      checkFunctionNodeHasEmptyNullsAction(function_node);

      if (!parameters.empty()) {
        throw Exception(ErrorCodes::FUNCTION_CANNOT_HAVE_PARAMETERS, "Function {} is not parametric",
      ...
      ProjectionNames lambda_projection_names =
          resolveLambda(lambda_expression_untyped, lambda_expression_clone, function_arguments, lambda_scope);

      auto &resolved_lambda = lambda_expression_clone->as<LambdaNode &>();
      node = resolved_lambda.getExpression();

      if (node->getNodeType() == QueryTreeNodeType::LIST) result_projection_names = std::move(lambda_projection_names);
    ...
    if (function_name == "untuple") {
      ...
      if (function_arguments.size() != 1)
        throw Exception(ErrorCodes::UNSUPPORTED_METHOD, "Function 'untuple' must have 1 argument. In scope {}",
                        scope.scope_node->formatASTForErrorMessage());
      ...
      if (!tuple_data_type)
        throw Exception(ErrorCodes::UNSUPPORTED_METHOD, "Function 'untuple' argument must have compound type. Actual type {}. In scope {}",
                        result_type->getName(), scope.scope_node->formatASTForErrorMessage());
    ...
    if (function_name == "grouping") {
      ...
      if (function_arguments_size > 64)
        throw Exception(ErrorCodes::TOO_MANY_ARGUMENTS_FOR_FUNCTION, "Function GROUPING can have up to 64 arguments, but {} provided",
  ...
  if (function_node.isWindowFunction()) {
    ...
    if (!function_lambda_arguments_indexes.empty())
      throw Exception(ErrorCodes::UNSUPPORTED_METHOD, "Window function '{}' does not support lambda arguments", function_name);
    ...
    if (function_name == "lag" || function_name == "lead") {
      ...
      if (!frame.is_default) {
        throw Exception(ErrorCodes::BAD_ARGUMENTS,
                        "Window function '{}' does not expect window frame to be explicitly specified. In expression {}", function_name,
                        function_node.formatASTForErrorMessage());
      }
    }
  ...
  FunctionOverloadResolverPtr function = UserDefinedExecutableFunctionFactory::instance().tryGet(
      function_name, scope.context, parameters);  /// NOLINT(readability-static-accessed-through-instance)
  ...
  if (!function) {
    ...
    if (function_name != "getSetting" && function_name != "rowNumberInAllBlocks") {
      ...
      if (!function_cache->resolver) function_cache->resolver = FunctionFactory::instance().tryGet(function_name, scope.context);
  ...
  if (function) {
    ...
    if (!AggregateFunctionFactory::instance().isAggregateFunctionName(function_name)) {
      ...
      auto function_names = UserDefinedExecutableFunctionFactory::instance().getRegisteredNames(
          scope.context);  /// NOLINT(readability-static-accessed-through-instance)
      ...
      for (auto &[name, lambda_node] : scope.aliases.alias_name_to_lambda_node) {
        if (lambda_node->getNodeType() == QueryTreeNodeType::LAMBDA) possible_function_names.push_back(name);
      }

      auto hints = NamePrompter<2>::getHints(function_name, possible_function_names);

      throw Exception(ErrorCodes::UNKNOWN_FUNCTION, "Function with name {} does not exist. In scope {}{}", backQuote(function_name),
                      scope.scope_node->formatASTForErrorMessage(), getHintsErrorMessageSuffix(hints));
    }

    if (!function_lambda_arguments_indexes.empty())
      throw Exception(ErrorCodes::UNSUPPORTED_METHOD, "Aggregate function {} does not support lambda arguments", backQuote(function_name));
    ...
    for (auto &function_lambda_argument_index : function_lambda_arguments_indexes) {
      ...
      if (!function_data_type)
        throw Exception(ErrorCodes::LOGICAL_ERROR,
                        ...
                        function_name, function_lambda_argument_index, argument_types[function_lambda_argument_index]->getName(),
                        scope.scope_node->formatASTForErrorMessage());
      ...
      if (function_data_type_arguments_size != lambda_arguments_size)
        throw Exception(ErrorCodes::LOGICAL_ERROR,
                        ...
                        function_name, function_data_type_arguments_size, lambda_arguments_size,
                        argument_types[function_lambda_argument_index]->getName(), scope.scope_node->formatASTForErrorMessage());
      ...
      for (size_t i = 0; i < function_data_type_arguments_size; ++i) {
        if (!function_data_type_argument_types[i])
          throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT,
                          "Function '{}' does not expect a lambda expression as argument {}. In scope {}", function_name,
                          function_lambda_argument_index + 1, scope.scope_node->formatASTForErrorMessage());
      }
      ...
      if (auto *lambda_list_node_result = lambda_to_resolve_typed.getExpression()->as<ListNode>()) {
        ...
        if (lambda_list_node_result_nodes_size != 1)
          throw Exception(ErrorCodes::UNSUPPORTED_METHOD,
                          ...
                          lambda_list_node_result_nodes_size, lambda_to_resolve->formatASTForErrorMessage());
      ...
      if (arguments_projection_names.at(function_lambda_argument_index) == PROJECTION_NAME_PLACEHOLDER) {
        ...
        if (lambda_projection_names_size != 1)
          throw Exception(ErrorCodes::LOGICAL_ERROR, "Lambda argument inside function expected to have 1 projection name. Actual: {}",
        ...
        for (size_t i = 0; i < lambda_argument_names_size; ++i) {
          ...
          if (i + 1 != lambda_argument_names_size) lambda_argument_projection_name_buffer << ", ";
        }
      ...
      auto lambda_resolved_type =
          std::make_shared<DataTypeFunction>(function_data_type_argument_types, lambda_to_resolve_typed.getExpression()->getResultType());
  ...
  if (is_special_function_in) {
    ...
    if (first_argument_constant_node && second_argument_constant_node) {
      ...
      auto result_block =
          getSetElementsForConstantValue(first_argument_constant_type, second_argument_constant_literal, second_argument_constant_type,
                                         GetSetElementParams{
      ...
      SizeLimits size_limits_for_set = {settings[Setting::max_rows_in_set], settings[Setting::max_bytes_in_set],
                                        settings[Setting::set_overflow_mode]};

      auto hash = function_arguments[1]->getTreeHash({.ignore_cte = true});
      auto ast = function_arguments[1]->toAST();
      auto future_set = std::make_shared<FutureSetFromTuple>(hash, std::move(ast), std::move(result_block),
                                                             settings[Setting::transform_null_in], size_limits_for_set);
  ...
  try {
    ...
    if (function_cache && !has_lambda_arguments) {
      ...
      if (!cached_function) cached_function = function->build(argument_columns);
    ...
    const auto *join_node =
        nearest_join_query_scope_query_node ? nearest_join_query_scope_query_node->getJoinTree()->as<JoinNode>() : nullptr;
    ...
    if (allow_constant_folding && function_base->isSuitableForConstantFolding()) {
      ...
      if (all_arguments_constants) {
        size_t num_rows = 1;
        if (!argument_columns.empty()) num_rows = argument_columns.front().column->size();
        column = executable_function->execute(argument_columns, result_type, num_rows, true);

        /// All constant (literal) columns in block are added with size 1.
        /// But if there was no columns in block before executing a function, the result has size 0.
        /// Change the size to 1.
        if (column && column->empty() && isColumnConst(*column)) column = column->cloneResized(1);
      } else {
      ...
      if (column && column->getDataType() != result_type->getColumnType())
        throw Exception(ErrorCodes::LOGICAL_ERROR, "Unexpected return type from {}. Expected {}. Got {}", function->getName(),
                        result_type->getColumnType(), column->getDataType());
  ...
  if (constant_node) node = std::move(constant_node);
