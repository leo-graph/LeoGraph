...
struct CorrelatedColumnsCollector {
  explicit CorrelatedColumnsCollector(const QueryTreeNodePtr& expression, const IdentifierResolveScope* current_scope,
                                      const std::unordered_map<QueryTreeNodePtr, IdentifierResolveScope>& map)
      : node_to_scope_map(map), correlated_columns() {
    ...
    while (!nodes_to_process.empty()) {
      ...
      if (current_node_type == QueryTreeNodeType::QUERY || current_node_type == QueryTreeNodeType::UNION) {
        if (current != current_scope->scope_node) {
          ...
          if (it != node_to_scope_map.end()) visitExpression(current, &it->second);
      ...
      if (current_node_type == QueryTreeNodeType::COLUMN) {
        if (checkCorrelatedColumn(current_scope, current)) correlated_columns.push_back(current);
      }
