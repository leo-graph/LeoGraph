...
const char *toString(ColumnTransfomerType type) {
  switch (type) {
    case ColumnTransfomerType::APPLY:
      return "APPLY";
...
IColumnTransformerNode::IColumnTransformerNode(size_t children_size) : IQueryTreeNode(children_size) {}

/// ApplyColumnTransformerNode implementation

const char *toString(ApplyColumnTransformerType type) {
  switch (type) {
    case ApplyColumnTransformerType::LAMBDA:
      return "LAMBDA";
    case ApplyColumnTransformerType::FUNCTION:
      return "FUNCTION";
  }
}

ApplyColumnTransformerNode::ApplyColumnTransformerNode(QueryTreeNodePtr expression_node_) : IColumnTransformerNode(children_size) {
  if (expression_node_->getNodeType() == QueryTreeNodeType::LAMBDA)
    ...
    throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT, "Apply column transformer expression must be lambda or function. Actual {}",
...
QueryTreeNodePtr ApplyColumnTransformerNode::cloneImpl() const { return std::make_shared<ApplyColumnTransformerNode>(getExpressionNode()); }
...
ASTPtr ApplyColumnTransformerNode::toASTImpl(const ConvertToASTOptions &options) const {
  ...
  if (apply_transformer_type == ApplyColumnTransformerType::FUNCTION) {
    ...
    if (!lambda_expression.getArgumentNames().empty()) ast_apply_transformer->lambda_arg = lambda_expression.getArgumentNames()[0];
...
bool ExceptColumnTransformerNode::isColumnMatching(const std::string &column_name) const {
  if (column_matcher) return re2::RE2::PartialMatch(column_name, *column_matcher);
...
void ExceptColumnTransformerNode::dumpTreeImpl(WriteBuffer &buffer, FormatState &format_state, size_t indent) const {
  ...
  for (size_t i = 0; i < except_column_names_size; ++i) {
    ...
    if (i + 1 != except_column_names_size) buffer << ", ";
  }
}
...
bool ExceptColumnTransformerNode::isEqualImpl(const IQueryTreeNode &rhs, CompareOptions) const {
  ...
  if (except_transformer_type != rhs_typed.except_transformer_type || is_strict != rhs_typed.is_strict ||
      except_column_names != rhs_typed.except_column_names)
    return false;

  const auto &rhs_column_matcher = rhs_typed.column_matcher;

  if (!column_matcher && !rhs_column_matcher) return true;
  if (column_matcher && !rhs_column_matcher) return false;
  if (!column_matcher && rhs_column_matcher) return false;
...
QueryTreeNodePtr ExceptColumnTransformerNode::cloneImpl() const {
  if (except_transformer_type == ExceptColumnTransformerType::REGEXP) return std::make_shared<ExceptColumnTransformerNode>(column_matcher);
...
ASTPtr ExceptColumnTransformerNode::toASTImpl(const ConvertToASTOptions & /* options */) const {
  ...
  for (const auto &name : except_column_names) ast_except_transformer->children.push_back(make_intrusive<ASTIdentifier>(name));
...
ReplaceColumnTransformerNode::ReplaceColumnTransformerNode(const std::vector<Replacement> &replacements_, bool is_strict_)
    : IColumnTransformerNode(children_size), is_strict(is_strict_) {
...
QueryTreeNodePtr ReplaceColumnTransformerNode::findReplacementExpression(const std::string &expression_name) {
  ...
  if (it == replacements_names.end()) return {};
...
void ReplaceColumnTransformerNode::dumpTreeImpl(WriteBuffer &buffer, FormatState &format_state, size_t indent) const {
  ...
  for (size_t i = 0; i < replacements_size; ++i) {
    ...
    if (i + 1 != replacements_size) buffer << '\n';
  }
}
