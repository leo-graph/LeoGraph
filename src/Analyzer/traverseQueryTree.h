...
/// @param should_visit_predicate A callable that takes (parent_node, child_node) and returns a bool indicating whether to visit the child
/// node.
    ...
    if (auto* table_function_node = current_node->as<TableFunctionNode>()) {
      for (const auto& child : current_node->getChildren()) {
        ...
        if (child == table_function_node->getArgumentsNode()) {
          ...
          if (should_visit_predicate(current_node, child)) nodes_to_process.push_back(child);
        }
      }
    } else {
      for (const auto& child : current_node->getChildren()) {
