...
static String getLoadSuggestionQueryUsingSystemTables(Int32 suggestion_limit, bool basic_suggestion, UInt64 server_revision) {
  ...
  auto add_column = [&](std::string_view column_name, std::string_view table_name, bool distinct, std::optional<Int64> limit) {
    add_subquery(fmt::format("SELECT {}{} FROM system.{}{}", (distinct ? "DISTINCT " : ""), column_name, table_name,
  ...
  if (server_revision >= DBMS_MIN_REVISION_WITH_SYSTEM_KEYWORDS_TABLE) add_column("keyword", "keywords", false, {});
  ...
  add_subquery(
      "SELECT concat(func.name, comb.name) AS x FROM system.functions AS func CROSS JOIN system.aggregate_function_combinators AS comb "
      "WHERE is_aggregate",
      "x");
...
static String getLoadSuggestionQueryUsingSystemCompletionsTable(Int32 suggestion_limit, bool basic_suggestion, UInt64 server_revision) {
  ...
  String unlimited_contexts = fmt::format(
      "('function', 'table engine', 'format', 'table function', 'data type', 'merge tree setting', 'setting', 'aggregate function "
      "combinator pair'{}{})",
      (server_revision >= DBMS_MIN_REVISION_WITH_SYSTEM_KEYWORDS_TABLE ? ", 'keyword'" : ""),
      (basic_suggestion ? "" : ", 'cluster', 'macro', 'policy'"));
  String query = fmt::format("SELECT word FROM system.completions WHERE context IN {}", unlimited_contexts);

  /// The user may disable loading of databases, tables, columns by setting suggestion_limit to zero.
  if (suggestion_limit > 0) {
    String limited_contexts = fmt::format("('database', 'table', 'column'{})", (basic_suggestion ? "" : ", 'dictionary'"));
    query += fmt::format(
        ...
        ") WHERE rn <= {}",
        limited_contexts, suggestion_limit);
  }
...
static String getLoadSuggestionQuery(IServerConnection &connection, Int32 suggestion_limit, bool basic_suggestion,
  ...
  if (server_major_version > CLICKHOUSE_SERVER_MIN_MAJOR_VERSION_WITH_SYSTEM_COMPLETIONS ||
      (server_major_version == CLICKHOUSE_SERVER_MIN_MAJOR_VERSION_WITH_SYSTEM_COMPLETIONS &&
       server_minor_version >= CLICKHOUSE_SERVER_MIN_MINOR_VERSION_WITH_SYSTEM_COMPLETIONS))
...
void Suggest::load(ContextPtr context, const ConnectionParameters &connection_parameters, Int32 suggestion_limit, bool wait_for_load) {
  loading_thread = std::thread([my_context = Context::createCopy(context), connection_parameters, suggestion_limit, this] {
    ...
    if constexpr (!std::is_same_v<ConnectionType, LocalConnection>) query_scope = CurrentThread::QueryScope::create(my_context);
    ...
    for (size_t retry = 0; retry < 10; ++retry) {
      try {
        ...
        fetch(*connection, connection_parameters.timeouts, suggestion_query, my_context->getClientInfo());
      } catch (const Exception &e) {
        last_error = e.code();
        if (e.code() == ErrorCodes::DEADLOCK_AVOIDED) continue;
  ...
  if (wait_for_load) loading_thread.join();
}

void Suggest::load(IServerConnection &connection, const ConnectionTimeouts &timeouts, Int32 suggestion_limit,
...
void Suggest::fetch(IServerConnection &connection, const ConnectionTimeouts &timeouts, const std::string &query,
                    const ClientInfo &client_info) {
  connection.sendQuery(timeouts, query, {} /* query_parameters */, "" /* query_id */, QueryProcessingStage::Complete, nullptr, &client_info,
                       false, {} /* external_roles*/, {});
...
void Suggest::fillWordsFromBlock(const Block &block) {
  ...
  for (size_t i = 0; i < rows; ++i) new_words.emplace_back(column[i].safeGet<String>());

  addWords(std::move(new_words));
}

template void Suggest::load<Connection>(ContextPtr context, const ConnectionParameters &connection_parameters, Int32 suggestion_limit,
                                        bool wait_for_load);

template void Suggest::load<LocalConnection>(ContextPtr context, const ConnectionParameters &connection_parameters, Int32 suggestion_limit,
                                             bool wait_for_load);
}  // namespace DB
