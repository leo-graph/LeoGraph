...
Connection::Connection(const String &host_, UInt16 port_, const String &default_database_, const String &user_, const String &password_,
                       const String &proto_send_chunked_, const String &proto_recv_chunked_,
                       [[maybe_unused]] const SSHKey &ssh_private_key_, [[maybe_unused]] const String &jwt_, const String &quota_key_,
                       const String &cluster_, const String &cluster_secret_, const String &client_name_,
                       Protocol::Compression compression_, Protocol::Secure secure_, const String &tls_sni_override_,
...
void Connection::connect(const ConnectionTimeouts &timeouts) {
  ...
  try {
    LOG_TRACE(log_wrapper.get(), "Connecting. Database: {}. User: {}{}{}. Bind_Host: {}",
              default_database.empty() ? "(not specified)" : default_database, user, static_cast<bool>(secure) ? ". Secure" : "",
              static_cast<bool>(compression) ? "" : ". Uncompressed", bind_host.empty() ? "(not specified)" : bind_host);
    ...
    for (auto it = addresses.begin(); it != addresses.end();) {
      ...
      LOG_TRACE(log_wrapper.get(), "Connecting to {}:{} (using address {}, {}/{})", host, port, it->toString(),
                std::distance(addresses.begin(), it) + 1, addresses.size());
      ...
      if (static_cast<bool>(secure)) {
        ...
        static_cast<Poco::Net::SecureStreamSocket *>(socket.get())->setPeerHostName(tls_sni_override.empty() ? host : tls_sni_override);
      ...
      try {
        if (async_callback) {
          ...
          while (!socket->poll(0, Poco::Net::Socket::SELECT_READ | Poco::Net::Socket::SELECT_WRITE | Poco::Net::Socket::SELECT_ERROR)) {
            async_callback(socket->impl()->sockfd(), connection_timeout, AsyncEventTimeoutType::CONNECT, description,
                           AsyncTaskExecutor::READ | AsyncTaskExecutor::WRITE | AsyncTaskExecutor::ERROR);
            if (address_connect_timeout_expired) throw Poco::TimeoutException("Connection timeout expired for address: " + it->toString());
          }
    ...
    if (server_revision >= DBMS_MIN_PROTOCOL_VERSION_WITH_CHUNKED_PACKETS) {
      ...
      auto is_chunked = [](const String &chunked_srv_str, const String &chunked_cl_str, const String &direction) {
        ...
        if (optional_cl) return chunked_srv;
        if (chunked_cl != chunked_srv)
          throw NetException(ErrorCodes::NETWORK_ERROR, "Incompatible protocol: {} set to {}, server requires {}", direction,
                             chunked_cl ? "chunked" : "notchunked", chunked_srv ? "chunked" : "notchunked");
    ...
    if (server_revision >= DBMS_MIN_PROTOCOL_VERSION_WITH_ADDENDUM) sendAddendum();

    if (proto_send_chunked == "chunked") out->enableChunked();
    if (proto_recv_chunked == "chunked") in->enableChunked();

    LOG_TRACE(log_wrapper.get(), "Connected to {} server version {}.{}.{}.", server_name, server_version_major, server_version_minor,
    ...
    const auto &connection_timeout = static_cast<bool>(secure) ? timeouts.secure_connection_timeout : timeouts.connection_timeout;
    throw NetException(ErrorCodes::SOCKET_TIMEOUT, "{} ({}, connection timeout {} ms)", e.displayText(),
                       getDescription(/*with_extra*/ true), connection_timeout.totalMilliseconds());
  } catch (...) {
    disconnect();
    throw;
  }
...
void Connection::cancel() noexcept {
  if (maybe_compressed_out) maybe_compressed_out->cancel();
...
void Connection::sendHello() {
  ...
  auto has_control_character = [](const std::string &s) {
    for (auto c : s)
      if (isControlASCII(c)) return true;
    return false;
  };
...
void Connection::sendAddendum() {
  if (server_revision >= DBMS_MIN_PROTOCOL_VERSION_WITH_QUOTA_KEY) writeStringBinary(quota_key, *out);
...
void Connection::performHandshakeForSSHAuth() {
  String challenge;
  {
    ...
    if (in->eof()) throw Poco::Net::NetException("Connection reset by peer");
...
void Connection::receiveHello() {
  ...
  if (packet_type == Protocol::Server::Hello) {
    ...
    if (server_revision >= DBMS_MIN_REVISION_WITH_SERVER_TIMEZONE) readStringBinary(server_timezone, *in);
    if (server_revision >= DBMS_MIN_REVISION_WITH_SERVER_DISPLAY_NAME) readStringBinary(server_display_name, *in);
...
void Connection::setDefaultDatabase(const String &database) { default_database = database; }

const String &Connection::getDefaultDatabase() const { return default_database; }
...
{
  if (with_extra) return full_description;
  return description;
}

const String &Connection::getHost() const { return host; }

UInt16 Connection::getPort() const { return port; }

void Connection::getServerVersion(const ConnectionTimeouts &timeouts, String &name, UInt64 &version_major, UInt64 &version_minor,
                                  UInt64 &version_patch, UInt64 &revision) {
  if (!isConnected()) connect(timeouts);
...
bool Connection::ping(const ConnectionTimeouts &timeouts) {
  try {
    ...
    if (pong != Protocol::Server::Pong) throwUnexpectedPacket(pong, "Pong", &timeout_setter);
...
void Connection::sendQuery(const ConnectionTimeouts &timeouts, const String &query, const NameToNameMap &query_parameters,
                           const String &query_id_, UInt64 stage, const Settings *settings, const ClientInfo *client_info,
                           bool with_pending_data, const std::vector<String> &external_roles, std::function<void(const Progress &)>) {
  ...
  if (settings) {
    ...
    if (method == "ZSTD") level = (*settings)[Setting::network_zstd_compression_level];

    CompressionCodecFactory::instance().validateCodec(method, level, !(*settings)[Setting::allow_suspicious_codecs],
  ...
  if (server_revision >= DBMS_MIN_REVISION_WITH_INTERSERVER_SECRET) {
    ...
    if (client_info && !cluster_secret.empty() && client_info->query_kind != ClientInfo::QueryKind::INITIAL_QUERY) {
      ...
      if (server_revision >= DBMS_MIN_PROTOCOL_VERSION_WITH_INTERSERVER_EXTERNALLY_GRANTED_ROLES) data += external_roles_str;
  ...
  if (server_revision >= DBMS_MIN_PROTOCOL_VERSION_WITH_PARAMETERS) {
    Settings params;
    for (const auto &[name, value] : query_parameters) params.set(name, value);
    params.write(*out, SettingsWriteFormat::STRINGS_WITH_FLAGS);
  }

  maybe_compressed_in.reset();
  if (maybe_compressed_out && maybe_compressed_out != out) maybe_compressed_out->finalize();
  maybe_compressed_out.reset();
  block_in.reset();
  block_logs_in.reset();
...
void Connection::sendCancel() {
  ...
  if (!out) throw Exception(ErrorCodes::NETWORK_ERROR, "Connection to {} terminated", getDescription());
...
void Connection::sendData(const Block &block, const String &name, bool scalar) {
  if (!block_out) {
    ...
    block_out = std::make_unique<NativeWriter>(*maybe_compressed_out, server_revision, std::make_shared<const Block>(block.cloneEmpty()),
                                               format_settings);
  }
  ...
  if (maybe_compressed_out != out) maybe_compressed_out->next();
  if (block.empty()) out->finishChunk();
  out->next();

  if (throttler) throttler->throttle(out->count() - prev_bytes);
}
...
void Connection::sendPreparedData(ReadBuffer &input, size_t size, const String &name) {
  ...
  if (input.eof()) throw Exception(ErrorCodes::EMPTY_DATA_PASSED, "Buffer is empty (some kind of corruption)");
...
void Connection::sendScalarsData(Scalars &data) {
  ...
  /// revision 54428.
  if (server_revision < DBMS_MIN_REVISION_WITH_SCALARS) return;
  ...
  if (compression == Protocol::Compression::Enable)
    LOG_DEBUG(log_wrapper.get(),
              ...
              data.size(), rows, elapsed, static_cast<size_t>(static_cast<double>(rows) / watch.elapsedSeconds()),
              ReadableSize(maybe_compressed_out_bytes),
              ReadableSize(static_cast<double>(maybe_compressed_out_bytes) / watch.elapsedSeconds()),
              static_cast<double>(maybe_compressed_out_bytes) / static_cast<double>(out_bytes), ReadableSize(out_bytes),
              ReadableSize(static_cast<double>(out_bytes) / watch.elapsedSeconds()));
  else
    LOG_DEBUG(log_wrapper.get(), "Sent data for {} scalars, total {} rows in {} sec., {} rows/sec., {} ({}/sec.), no compression.",
              data.size(), rows, elapsed, static_cast<size_t>(static_cast<double>(rows) / watch.elapsedSeconds()),
              ReadableSize(maybe_compressed_out_bytes),
              ReadableSize(static_cast<double>(maybe_compressed_out_bytes) / watch.elapsedSeconds()));
}
...
class ExternalTableDataSink : public ISink {
 public:
  ...
  ExternalTableDataSink(SharedHeader header, Connection &connection_, ExternalTableData &table_data_, OnCancell callback)
      : ISink(std::move(header)), connection(connection_), table_data(table_data_), on_cancell(std::move(callback)) {}
...
void Connection::sendExternalTablesData(ExternalTablesData &data) {
  ...
  for (auto &elem : data) {
    ...
    if (!elem->pipe) elem->pipe = elem->creating_pipe_callback();
    ...
    pipeline.setSinks([&](const SharedHeader &, QueryPipelineBuilder::StreamType type) -> ProcessorPtr {
      if (type != QueryPipelineBuilder::StreamType::Main) return nullptr;
      return sink;
    });
    ...
    if (read_rows == 0) sendData(sink->getPort().getHeader(), elem->table_name, false);
  }
  ...
  if (compression == Protocol::Compression::Enable)
    ...
    LOG_DEBUG(log_wrapper.get(), "Sent data for {} external tables, total {} rows in {} sec., {} rows/sec., {} ({}/sec.), no compression.",
              data.size(), rows, elapsed, static_cast<size_t>(static_cast<double>(rows) / watch.elapsedSeconds()),
              ReadableSize(maybe_compressed_out_bytes),
              ReadableSize(static_cast<double>(maybe_compressed_out_bytes) / watch.elapsedSeconds()));
}

std::optional<Poco::Net::SocketAddress> Connection::getResolvedAddress() const { return current_resolved_address; }

bool Connection::poll(size_t timeout_microseconds) { return in->poll(timeout_microseconds); }

bool Connection::hasReadPendingData() const { return last_input_packet_type.has_value() || in->hasBufferedData(); }

std::optional<UInt64> Connection::checkPacket(size_t timeout_microseconds) {
  if (last_input_packet_type.has_value()) return last_input_packet_type;
...
UInt64 Connection::receivePacketType() {
  ...
  if (last_input_packet_type) return *last_input_packet_type;
...
Packet Connection::receivePacket() {
  try {
    ...
    if (!in) throw Exception(ErrorCodes::LOGICAL_ERROR, "Connection to {} is terminated", getDescription());
    ...
    switch (res.type) {
      ...
      default:
        ...
        throw Exception(ErrorCodes::UNKNOWN_PACKET_FROM_SERVER, "Unknown packet {} from server {}", toString(res.type), getDescription());
    }
  } catch (Exception &e) {
    disconnect();
    ...
    if (e.code() != ErrorCodes::UNKNOWN_PACKET_FROM_SERVER) e.addMessage("while receiving packet from " + getDescription(true));
...
Block Connection::receiveDataImpl(NativeReader &reader) {
  ...
  if (throttler) throttler->throttle(in->count() - prev_bytes);
...
void Connection::setDescription() {
  ...
  if (resolved_address) {
    ...
    if (host != ip_address) full_description += ", " + ip_address;
  }
...
void Connection::throwUnexpectedPacket(UInt64 packet_type, const char *expected, TimeoutSetter *timeout_setter) {
  ...
  if (timeout_setter) timeout_setter->reset();

  /// Close connection, to avoid leaving it in an unsynchronised state.
  disconnect();

  throw NetException(ErrorCodes::UNEXPECTED_PACKET_FROM_SERVER, "Unexpected packet from server {} (expected {}, got {})", getDescription(),
                     expected, String(Protocol::Server::toString(packet_type)));
}

ServerConnectionPtr Connection::createConnection(const ConnectionParameters &parameters, ContextPtr) {
  return std::make_unique<Connection>(
      parameters.host, parameters.port, parameters.default_database, parameters.user, parameters.password, parameters.proto_send_chunked,
      parameters.proto_recv_chunked, parameters.ssh_private_key, parameters.jwt, parameters.quota_key, "", /* cluster */
      "",                                                                                                  /* cluster_secret */
      std::string(DEFAULT_CLIENT_NAME), parameters.compression, parameters.security, parameters.tls_sni_override, parameters.bind_host
#if USE_JWT_CPP && USE_SSL
      ,
