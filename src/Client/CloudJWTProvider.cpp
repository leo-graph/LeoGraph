...
const std::map<std::string, CloudJWTProvider::AuthEndpoints> CloudJWTProvider::managed_service_endpoints = {
    {".clickhouse-dev.com",
     {"https://auth.control-plane.clickhouse-dev.com", "dKv0XkTAw7rghGiAa5sjPFYGQUVtjzuz",
      "https://console-api-internal.clickhouse-dev.com"}},
    {".clickhouse-staging.com",
     {"https://auth.control-plane.clickhouse-staging.com", "rpEkizLMmAU95MP4JL8ERefbVXtUQSFs",
      "https://console-api-internal.clickhouse-staging.com"}},
    {".clickhouse.cloud",
     {"https://auth.clickhouse.cloud", "9Wf1YpSocOg5sp7GOcCjtrt6DWRAJ19S", "https://console-api-internal.clickhouse.cloud"}}};
...
const CloudJWTProvider::AuthEndpoints* CloudJWTProvider::getAuthEndpoints(const std::string& host) {
  for (const auto& [suffix, endpoints] : managed_service_endpoints) {
    if (endsWith(host, suffix)) return &endpoints;
  }
  return nullptr;
}

CloudJWTProvider::CloudJWTProvider(std::string auth_url, std::string client_id, std::string audience, std::string host, std::ostream& out,
                                   std::ostream& err)
    : JWTProvider(std::move(auth_url), std::move(client_id), std::move(audience), out, err), host_str(std::move(host)) {
  if (oauth_url.empty() || oauth_client_id.empty()) {
    if (const auto* endpoints = getAuthEndpoints(host_str)) {
      if (oauth_url.empty()) oauth_url = endpoints->auth_url;
      if (oauth_client_id.empty()) oauth_client_id = endpoints->client_id;
    }
  }
}
...
std::string CloudJWTProvider::getJWT() {
  ...
  if (!clickhouse_jwt.empty() && now < clickhouse_jwt_expires_at - expiration_buffer) return clickhouse_jwt;
...
std::string CloudJWTProvider::getAudience() const { return "token-exchange"; }
...
void CloudJWTProvider::exchangeIdPTokenForClickHouseJWT(bool show_messages) {
  ...
  if (!endpoints)
    throw Exception(ErrorCodes::BAD_ARGUMENTS,
                    "Cannot determine token exchange endpoint from hostname {}. Please provide a ClickHouse Cloud hostname.", host_str);

  Poco::URI exchange_url = Poco::URI(endpoints->api_host + "/.api/auth/tokenExchange");

  if (show_messages) output_stream << "Authenticating access to " << host_str << "." << std::endl;
  ...
  if (show_messages) output_stream << "Authenticated with ClickHouse Cloud.\n" << std::endl;
}
