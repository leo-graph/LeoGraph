...
LocalConnection::LocalConnection(std::unique_ptr<Session> &&session_, ReadBuffer *in_, bool send_progress_, bool send_profile_events_,
                                 const String &server_display_name_)
...
bool LocalConnection::hasReadPendingData() const { return !state->is_finished; }

std::optional<UInt64> LocalConnection::checkPacket(size_t) { return next_packet_type; }

void LocalConnection::updateProgress(const Progress &value) { state->progress.incrementPiecewiseAtomically(value); }
...
void LocalConnection::sendQuery(const ConnectionTimeouts &, const String &query, const NameToNameMap &query_parameters,
                                const String &query_id, UInt64 stage, const Settings *, const ClientInfo *client_info, bool,
                                const std::vector<String> & /*external_roles*/,
                                std::function<void(const Progress &)> process_progress_callback) {
  /// Last query may not have been finished or cancelled due to exception on client side.
  if (state && !state->is_finished && !state->is_cancelled) sendCancel();
  ...
  if (send_progress) state->after_send_progress.restart();

  if (send_profile_events) state->after_send_profile_events.restart();
  ...
  query_context->setInputInitializer([this](ContextPtr context, const StoragePtr &input_storage) {
    ...
    if (dialect == Dialect::kusto)
      ...
      parser = std::make_unique<ParserPRQLQuery>(settings[Setting::max_query_size], settings[Setting::max_parser_depth],
    ...
    auto source = context->getInputFormat(current_format, *in, sample, settings[Setting::max_insert_block_size], std::nullopt,
    ...
    if (state->io.pipeline.pushing()) {
      ...
      if (process_progress_callback) {
        auto callback = [this, &process_progress_callback]() {
          if (state->is_cancelled) return true;
...
void LocalConnection::sendQueryPlan(const QueryPlan &) { throw Exception(ErrorCodes::NOT_IMPLEMENTED, "Not implemented"); }
...
void LocalConnection::sendData(const Block &block, const String &, bool) {
  ...
  if (send_profile_events) sendProfileEvents();
}

bool LocalConnection::isSendDataNeeded() const { return !state || state->input_pipeline == nullptr; }

void LocalConnection::sendCancel() {
  state->is_cancelled = true;
  if (state->executor) state->executor->cancel();
  if (state->pushing_executor) state->pushing_executor->cancel();
  if (state->pushing_async_executor) state->pushing_async_executor->cancel();
}

bool LocalConnection::pullBlock(Block &block) {
  if (state->executor) return state->executor->pull(block, query_context->getSettingsRef()[Setting::interactive_delay] / 1000);
...
bool LocalConnection::poll(size_t) {
  ...
  if (next_packet_type) return true;
  ...
  if (!state->is_finished) {
    if (needSendProgressOrMetrics()) return true;

    if (needSendLogs()) return true;
  ...
  if (state->is_finished && !state->sent_totals) {
    ...
    if (state->executor) totals = state->executor->getTotalsBlock();
  ...
  if (state->is_finished && !state->sent_extremes) {
    ...
    if (state->executor) extremes = state->executor->getExtremesBlock();
...
bool LocalConnection::needSendLogs() {
  if (!state->logs_queue) return false;
...
Packet LocalConnection::receivePacket() {
  ...
  switch (next_packet_type.value()) {
    ...
    default:
      throw Exception(ErrorCodes::UNKNOWN_PACKET_FROM_SERVER, "Unknown packet {} for {}", toString(packet.type), getDescription());
  }

  return packet;
}

void LocalConnection::getServerVersion(const ConnectionTimeouts & /* timeouts */, String &name, UInt64 &version_major,
                                       UInt64 &version_minor, UInt64 &version_patch, UInt64 &revision) {
...
void LocalConnection::setDefaultDatabase(const String &database) { current_database = database; }

UInt64 LocalConnection::getServerRevision(const ConnectionTimeouts &) { return DBMS_TCP_PROTOCOL_VERSION; }
...
void LocalConnection::sendExternalTablesData(ExternalTablesData &) { throw Exception(ErrorCodes::NOT_IMPLEMENTED, "Not implemented"); }

void LocalConnection::sendMergeTreeReadTaskResponse(const ParallelReadResponse &) {
  throw Exception(ErrorCodes::NOT_IMPLEMENTED, "Not implemented");
}

ServerConnectionPtr LocalConnection::createConnection(const ConnectionParameters &, ContextPtr current_context, ReadBuffer *in,
                                                      bool send_progress, bool send_profile_events, const String &server_display_name) {
  return std::make_unique<LocalConnection>(current_context, in, send_progress, send_profile_events, server_display_name);
}

ServerConnectionPtr LocalConnection::createConnection(const ConnectionParameters &, std::unique_ptr<Session> &&session, ReadBuffer *in,
                                                      bool send_progress, bool send_profile_events, const String &server_display_name) {
