...
void PacketReceiver::processAsyncEvent(int fd [[maybe_unused]], Poco::Timespan socket_timeout, AsyncEventTimeoutType, const std::string &,
                                       uint32_t) {
  assert(fd == socket_fd);
...
bool PacketReceiver::checkTimeout() {
  ...
  for (size_t i = 0; i != ready_count; ++i) {
    ...
    if (events[i].data.fd == timeout_descriptor.getDescriptor()) is_timeout_expired = true;
  }
