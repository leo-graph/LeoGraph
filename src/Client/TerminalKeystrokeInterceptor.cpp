...
void TerminalKeystrokeInterceptor::registerCallback(char key, TerminalKeystrokeInterceptor::Callback cb) { callbacks.emplace(key, cb); }

void TerminalKeystrokeInterceptor::startIntercept() {
  std::unique_lock<std::mutex> lock(mutex);

  if (intercept_thread && intercept_thread->joinable()) return;
  ...
  if (tcgetattr(fd, orig_termios.get()))
    throw DB::ErrnoException(DB::ErrorCodes::SYSTEM_ERROR, "Cannot get the state of the terminal referred to by file descriptor '{}'", fd);
  ...
  if (tcsetattr(fd, TCSAFLUSH, &raw))
    throw DB::ErrnoException(DB::ErrorCodes::SYSTEM_ERROR,
                             "Cannot set terminal to the raw mode for the terminal referred to by file descriptor '{}'", fd);
...
void TerminalKeystrokeInterceptor::stopIntercept() {
  ...
  if (intercept_thread && intercept_thread->joinable()) intercept_thread->join();
...
void TerminalKeystrokeInterceptor::runImpl(const DB::TerminalKeystrokeInterceptor::CallbackMap& map) const {
  ...
  int available = 0;
  if (ioctl(fd, FIONREAD, &available) < 0) throw DB::ErrnoException(DB::ErrorCodes::SYSTEM_ERROR, "ioctl({}, FIONREAD)", fd);
