...
HedgedConnections::HedgedConnections(const ConnectionPoolWithFailoverPtr &pool_, ContextPtr context_, const ConnectionTimeouts &timeouts_,
                                     const ThrottlerPtr &throttler_, PoolMode pool_mode,
                                     std::shared_ptr<QualifiedTableName> table_to_check_, AsyncCallback async_callback,
                                     GetPriorityForLoadBalancing::Func priority_func)
    : hedged_connections_factory(pool_, context_->getSettingsRef(), timeouts_,
                                 ...
                                 context_->getSettingsRef()[Setting::skip_unavailable_shards].value, table_to_check_, priority_func),
...
void HedgedConnections::Pipeline::add(std::function<void(ReplicaState &replica)> send_function) { pipeline.push_back(send_function); }

void HedgedConnections::Pipeline::run(ReplicaState &replica) {
  for (auto &send_func : pipeline) send_func(replica);
}

void HedgedConnections::sendScalarsData(Scalars &data) {
  std::lock_guard lock(cancel_mutex);

  if (!sent_query) throw Exception(ErrorCodes::LOGICAL_ERROR, "Cannot send scalars data: query not yet sent.");
  ...
  for (auto &offset_state : offset_states)
    for (auto &replica : offset_state.replicas)
      if (replica.connection) send_scalars_data(replica);
...
void HedgedConnections::sendQueryPlan(const QueryPlan &query_plan) {
  ...
  if (!sent_query) throw Exception(ErrorCodes::LOGICAL_ERROR, "Cannot send query plan: query not yet sent.");

  auto send_query_plan = [&query_plan](ReplicaState &replica) { replica.connection->sendQueryPlan(query_plan); };

  for (auto &offset_state : offset_states)
    for (auto &replica : offset_state.replicas)
      if (replica.connection) send_query_plan(replica);
...
void HedgedConnections::sendExternalTablesData(std::vector<ExternalTablesData> &data) {
  ...
  if (!sent_query) throw Exception(ErrorCodes::LOGICAL_ERROR, "Cannot send external tables data: query not yet sent.");

  if (data.size() != size()) throw Exception(ErrorCodes::MISMATCH_REPLICAS_DATA_SOURCES, "Mismatch between replicas and data sources");
  ...
  for (auto &offset_state : offset_states)
    for (auto &replica : offset_state.replicas)
      if (replica.connection) send_external_tables_data(replica);
...
void HedgedConnections::sendIgnoredPartUUIDs(const std::vector<UUID> &uuids) {
  ...
  if (sent_query) throw Exception(ErrorCodes::LOGICAL_ERROR, "Cannot send uuids after query is sent.");

  auto send_ignored_part_uuids = [&uuids](ReplicaState &replica) { replica.connection->sendIgnoredPartUUIDs(uuids); };

  for (auto &offset_state : offset_states)
    for (auto &replica : offset_state.replicas)
      if (replica.connection) send_ignored_part_uuids(replica);

  pipeline_for_new_replicas.add(send_ignored_part_uuids);
}

void HedgedConnections::sendQuery(const ConnectionTimeouts &timeouts, const String &query, const String &query_id, UInt64 stage,
                                  ClientInfo &client_info, bool with_pending_data, const std::vector<String> &external_roles) {
  std::lock_guard lock(cancel_mutex);

  if (sent_query) throw Exception(ErrorCodes::LOGICAL_ERROR, "Query already sent.");
  ...
  for (auto &offset_state : offset_states) {
    ...
    if (disable_two_level_aggregation) break;
  }
  ...
  auto send_query = [this, timeouts, query, query_id, stage, client_info, with_pending_data, external_roles](ReplicaState &replica) {
    ...
    replica.connection->sendQuery(timeouts, query, /* query_parameters */ {}, query_id, stage, &modified_settings, &client_info,
                                  with_pending_data, external_roles, {});
  ...
  for (auto &offset_status : offset_states)
    for (auto &replica : offset_status.replicas) send_query(replica);
...
void HedgedConnections::disconnect() {
  ...
  for (auto &offset_status : offset_states)
    for (auto &replica : offset_status.replicas)
      if (replica.connection) finishProcessReplica(replica, true);

  if (hedged_connections_factory.hasEventsInProcess()) {
    if (hedged_connections_factory.numberOfProcessingReplicas() > 0) epoll.remove(hedged_connections_factory.getFileDescriptor());
...
void HedgedConnections::sendCancel() {
  ...
  if (!sent_query || cancelled) throw Exception(ErrorCodes::LOGICAL_ERROR, "Cannot cancel. Either no query sent or already cancelled.");
  ...
  if (hedged_connections_factory.hasEventsInProcess()) hedged_connections_factory.stopChoosingReplicas();

  cancelled = true;

  for (auto &offset_status : offset_states)
    for (auto &replica : offset_status.replicas)
      if (replica.connection) replica.connection->sendCancel();
}

Packet HedgedConnections::drain() {
  std::lock_guard lock(cancel_mutex);

  if (!cancelled) throw Exception(ErrorCodes::LOGICAL_ERROR, "Cannot drain connections: cancel first.");
...
Packet HedgedConnections::receivePacketUnlocked(AsyncCallback async_callback) {
  if (!sent_query) throw Exception(ErrorCodes::LOGICAL_ERROR, "Cannot receive packets: no query sent.");
  if (!hasActiveConnections()) throw Exception(ErrorCodes::LOGICAL_ERROR, "No more packets are available.");

  if (epoll.empty()) throw Exception(ErrorCodes::LOGICAL_ERROR, "No pending events in epoll.");
...
HedgedConnections::ReplicaLocation HedgedConnections::getReadyReplicaLocation(AsyncCallback async_callback) {
  ...
  while (true) {
    ...
    if (event_fd == hedged_connections_factory.getFileDescriptor())
      ...
      if (resumePacketReceiver(location)) return location;
...
bool HedgedConnections::resumePacketReceiver(const HedgedConnections::ReplicaLocation &location) {
  ...
  if (replica_state.packet_receiver->isTimeoutExpired()) {
    ...
    if (offset_states[location.offset].active_connection_count == 0 && !offset_states[location.offset].next_replica_in_process)
      throw NetException(ErrorCodes::SOCKET_TIMEOUT, "Timeout exceeded while reading from socket ({}, receive timeout {} ms)", description,
...
int HedgedConnections::getReadyFileDescriptor(AsyncCallback async_callback) {
  ...
  while (events_count == 0) {
    ...
    if (!events_count && async_callback)
      async_callback(epoll.getFileDescriptor(), 0, AsyncEventTimeoutType::NONE, epoll.getDescription(),
...
Packet HedgedConnections::receivePacketFromReplica(const ReplicaLocation &replica_location) {
  ...
  switch (packet.type) {
    case Protocol::Server::Data:
      ...
      if (offset_states[replica_location.offset].can_change_replica && packet.block.rows() > 0) disableChangingReplica(replica_location);
    ...
    case Protocol::Server::EndOfStream:
      ...
      if (offset_states[replica_location.offset].can_change_replica) disableChangingReplica(replica_location);
  ...
  switch (state) {
    case HedgedConnectionsFactory::State::READY: {
      ...
      fd_to_replica_location[replica.packet_receiver->getFileDescriptor()] =
          ReplicaLocation{offset, offset_states[offset].replicas.size() - 1};
      epoll.add(replica.change_replica_timeout.getDescriptor());
      timeout_fd_to_replica_location[replica.change_replica_timeout.getDescriptor()] =
          ReplicaLocation{offset, offset_states[offset].replicas.size() - 1};
...
void HedgedConnections::finishProcessReplica(ReplicaState &replica, bool disconnect) {
  ...
  if (disconnect) replica.connection->disconnect();
...
void HedgedConnections::setAsyncCallback(AsyncCallback async_callback) {
  for (auto &offset_status : offset_states) {
    for (auto &replica : offset_status.replicas) {
      if (replica.connection) replica.connection->setAsyncCallback(async_callback);
    }
  }
}
