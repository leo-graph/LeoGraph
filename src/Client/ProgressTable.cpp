...
std::string moveUpNLines(size_t N) { return fmt::format("\033[{}A", N); }
...
std::string_view setColorForProgress(double progress, double max_progress) {
  ...
  if (max_progress == 0) return colors.front();
...
std::string_view setColorForBytesBasedMetricsProgress(double progress) {
  ...
  constexpr std::array<uint64_t, 6> thresholds = {
      1LL << 20, 100LL << 20, 1'000LL << 20, 10'000LL << 20, 100'000LL << 20, 1'000'000LL << 20,
  };
...
void ProgressTable::writeTable(WriteBufferFromFileDescriptor &message, std::unique_lock<std::mutex> &, bool show_table, bool toggle_enabled,
                               bool is_final) {
  ...
  if (terminal_width < column_event_name_width + COLUMN_VALUE_WIDTH + COLUMN_PROGRESS_WIDTH) return;
  ...
  if (col_doc_width) writeWithWidth(message, COLUMN_DOCUMENTATION_NAME, col_doc_width);
  ...
  for (auto &elem : metrics) {
    if (elapsed_sec - elem.second.updateTime() < 1) elem.second.increaseDisplayPriority();
  ...
  for (auto *ptr : sorted_metrics) {
    if (!is_final && row_num + 3 > terminal_height) break;
    ...
    auto progress = is_final ? per_host_info.getSummaryAverageProgress(elapsed_sec) : per_host_info.getSummaryRecentProgress(elapsed_sec);
    switch (value_type) {
      case ProfileEvents::ValueType::Number:
...
void ProgressTable::updateTable(const Block &block) {
  ...
  for (size_t row_num = 0, rows = block.rows(); row_num < rows; ++row_num) {
    ...
    if (!event_name_to_event.contains(name)) continue;
...
size_t ProgressTable::getColumnDocumentationWidth(size_t terminal_width) const {
  ...
  if (terminal_width < fixed_columns_width + COLUMN_DOCUMENTATION_MIN_WIDTH) return 0;
...
void ProgressTable::MetricInfo::updateValue(Int64 new_value, double new_time) {
  ...
  if (new_snapshot.time - cur_shapshot.time >= 0.5) prev_shapshot = std::exchange(cur_shapshot, new_snapshot);
...
double ProgressTable::MetricInfo::calculateRecentProgress(double time_now) const {
  ...
  if (time_now - new_snapshot.time >= 0.5) return 0;
...
double ProgressTable::MetricInfo::getValue() const { return static_cast<double>(new_snapshot.value); }
...
void ProgressTable::MetricInfoPerHost::updateHostValue(const HostName &host, ProfileEvents::Type type, Int64 new_value, double new_time) {
  ...
  if (it == host_to_metric.end()) it = host_to_metric.emplace(host, type).first;
  it->second.updateValue(new_value, new_time);
}

double ProgressTable::MetricInfoPerHost::getSummaryValue() const {
  double res = 0.0;
  for (const auto &elem : host_to_metric) res += elem.second.getValue();
  return res;
}

double ProgressTable::MetricInfoPerHost::getSummaryRecentProgress(double time_now) {
  double res = 0.0;
  for (const auto &elem : host_to_metric) res += elem.second.calculateRecentProgress(time_now);
...
double ProgressTable::MetricInfoPerHost::getSummaryAverageProgress(double time_now) const {
  double res = 0.0;
  for (const auto &elem : host_to_metric) res += elem.second.calculateAverageProgress(time_now);
  return res;
}

double ProgressTable::MetricInfoPerHost::getMaxProgress() const { return max_progress; }

double ProgressTable::MetricInfoPerHost::updateTime() const {
  double res = 0.0;
  for (const auto &elem : host_to_metric) res = std::max(res, elem.second.updateTime());
  return res;
}
