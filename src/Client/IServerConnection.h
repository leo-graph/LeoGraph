...
class IServerConnection : boost::noncopyable {
 public:
  ...
  enum class Type : uint8_t { SERVER, LOCAL };

  virtual Type getConnectionType() const = 0;

  virtual void setDefaultDatabase(const String &database) = 0;

  virtual void getServerVersion(const ConnectionTimeouts &timeouts, String &name, UInt64 &version_major, UInt64 &version_minor,
  ...
  virtual void sendQuery(const ConnectionTimeouts &timeouts, const String &query, const NameToNameMap &query_parameters,
                         const String &query_id_, UInt64 stage, const Settings *settings, const ClientInfo *client_info,
                         bool with_pending_data, const std::vector<String> &external_roles,
  ...
  /// False if the server will read the data through other means (in particular if clickhouse-local added input reading step directly into
  /// the query pipeline).
