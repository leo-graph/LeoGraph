...
std::string AISQLGenerator::generateSQL(const std::string& prompt) {
  try {
    ...
    if (schema_tools) options.tools = schema_tools->getToolSet();
...
bool AISQLGenerator::isAvailable() const { return client.is_valid(); }

std::string AISQLGenerator::getProviderName() const { return client.provider_name(); }

std::string AISQLGenerator::buildSystemPrompt() const {
  if (!config.system_prompt.empty()) return config.system_prompt;
