...
TEST(AIClientFactory, InvalidProvider) {
  ...
  EXPECT_THROW({ AIClientFactory::createClient(config); }, Exception);
...
TEST(AIClientFactory, LoadConfigurationInvalidProvider) {
  ...
  EXPECT_THROW({ AIClientFactory::loadConfiguration(*config); }, Exception);
...
TEST(AIClientFactory, MalformedConfigurationValues) {
  ...
  EXPECT_THROW({ ai_config = AIClientFactory::loadConfiguration(*config); }, Poco::SyntaxException);

  // Test with valid values
  config->setInt("ai.max_tokens", 1000);
  config->setInt("ai.timeout_seconds", 0);  // Zero is accepted

  EXPECT_NO_THROW({ ai_config = AIClientFactory::loadConfiguration(*config); });
