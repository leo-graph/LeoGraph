...
bool containsSQLKeywords(const std::string &result) {
  return result.find("SELECT") != std::string::npos || result.find("SHOW") != std::string::npos ||
         result.find("COUNT") != std::string::npos || result.find("DESCRIBE") != std::string::npos ||
         result.find("CREATE") != std::string::npos || result.find("INSERT") != std::string::npos ||
         result.find("UPDATE") != std::string::npos || result.find("DELETE") != std::string::npos;
}
...
class AITestFixture : public ::testing::Test {
 protected:
  ...
  AIConfiguration createStandardConfig(bool use_mini_model = true) const {
    ...
    config.model = provider == "openai" ? (use_mini_model ? "gpt-4o-mini" : "gpt-4")
...
QueryExecutor createSQLInjectionTestExecutor() {
  return [](const std::string &query) -> std::string {
    ...
    if (query.find("'; DROP TABLE") != std::string::npos || query.find("--") != std::string::npos ||
        query.find("/*") != std::string::npos || query.find("UNION SELECT") != std::string::npos) {
...
TEST_F(AITestFixture, SQLInjectionProtection) {
  ...
  std::vector<std::string> injection_attempts = {"show all users'; DROP TABLE users; --", "select * from users where name = 'admin'--",
  ...
  for (const auto &attempt : injection_attempts) {
    ...
    EXPECT_EQ(result.find("DROP TABLE"), std::string::npos) << "Generated SQL should not contain DROP TABLE";
    EXPECT_EQ(result.find("UNION SELECT"), std::string::npos) << "Generated SQL should not contain UNION SELECT";
    ...
    if (!result.empty() && containsSQLKeywords(result)) {
      ...
      EXPECT_EQ(result.find("--"), std::string::npos) << "Generated SQL should not contain SQL comments";
      EXPECT_EQ(result.find("/*"), std::string::npos) << "Generated SQL should not contain block comments";
    }
...
TEST_F(AITestFixture, AmbiguousQueryHandling) {
  ...
  for (const auto &[query, expectation] : queries) {
    ...
    EXPECT_TRUE(containsSQLKeywords(result)) << "Should generate valid SQL for: " << query << " (got: " << result << ")";
  }
}
...
TEST_F(AITestFixture, RealAIGeneration) {
  ...
  if (containsSQLKeywords(result)) {
    ...
    EXPECT_NE(result_lower.find("customer"), std::string::npos) << "Generated SQL should reference customers table";
  }
...
TEST_F(AITestFixture, ComplexQueryGeneration) {
  ...
  std::string result = generator.generateSQL("show me the total revenue by customer name for all orders");
  ...
  if (result.find("SELECT") != std::string::npos) {
    ...
    bool has_aggregation = result_lower.find("sum") != std::string::npos || result_lower.find("group by") != std::string::npos;
    bool has_join = result_lower.find("join") != std::string::npos;

    EXPECT_TRUE(has_aggregation || has_join) << "Complex query should use aggregation or joins";

    // Should reference relevant tables (customers, orders, or products)
    bool references_tables = result_lower.find("customer") != std::string::npos || result_lower.find("order") != std::string::npos ||
                             result_lower.find("product") != std::string::npos;

    EXPECT_TRUE(references_tables) << "Query should reference relevant tables";
  }
}
...
TEST_F(AITestFixture, SchemaExploration) {
  ...
  EXPECT_TRUE(output_str.find("list_databases") != std::string::npos || output_str.find("list_tables_in_database") != std::string::npos)
...
TEST_F(AITestFixture, SQLCleaningWithRealAI) {
  ...
  std::string result_lower = toLower(result);
  bool has_sql_keywords = result_lower.find("select") != std::string::npos || result_lower.find("from") != std::string::npos ||
...
TEST(AISQLGenerator, NetworkTimeoutRecovery) {
  ...
  try {
    ...
    EXPECT_TRUE(e.code() == ErrorCodes::BAD_ARGUMENTS || e.code() == ErrorCodes::NETWORK_ERROR);
  }
}
...
TEST_F(AITestFixture, SpecialCharacterHandling) {
  ...
  if (result.find("user-accounts") != std::string::npos) {
    ...
    bool is_quoted = (result.find("`user-accounts`") != std::string::npos) || (result.find("\"user-accounts\"") != std::string::npos) ||
                     (result.find("'user-accounts'") != std::string::npos);
    EXPECT_TRUE(is_quoted) << "Table names with special characters should be properly quoted";
  }
