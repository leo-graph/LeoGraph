...
SchemaExplorationTools::SchemaExplorationTools(QueryExecutor executor) : query_executor(std::move(executor)) { initializeTools(); }

void SchemaExplorationTools::initializeTools() {
  // list_databases tool
  tools["list_databases"] = ai::create_simple_tool(
      "list_databases", "List all available databases in the ClickHouse instance", {},  // No parameters
  ...
  tools["list_tables_in_database"] = ai::create_simple_tool(
      "list_tables_in_database", "List all tables in a specific database", {{"database", "string"}},
  ...
  tools["get_schema_for_table"] = ai::create_simple_tool(
      "get_schema_for_table", "Get the CREATE TABLE statement (schema) for a specific table", {{"database", "string"}, {"table", "string"}},
      [this](const ai::JsonValue& args, const ai::ToolExecutionContext& context) { return getSchemaForTable(args, context); });
}

ai::JsonValue SchemaExplorationTools::listDatabases(const ai::JsonValue& args [[maybe_unused]],
                                                    const ai::ToolExecutionContext& context [[maybe_unused]]) {
  try {
...
ai::JsonValue SchemaExplorationTools::listTablesInDatabase(const ai::JsonValue& args,
                                                           const ai::ToolExecutionContext& context [[maybe_unused]]) {
  try {
...
ai::JsonValue SchemaExplorationTools::getSchemaForTable(const ai::JsonValue& args,
                                                        const ai::ToolExecutionContext& context [[maybe_unused]]) {
  try {
    std::string database = args["database"].get<std::string>();
    ...
    return ai::JsonValue{{"success", true},
