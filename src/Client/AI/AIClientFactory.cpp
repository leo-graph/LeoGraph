...
AIClientResult AIClientFactory::createClient(const AIConfiguration& config) {
  ...
  // Unknown provider
  throw Exception(ErrorCodes::BAD_ARGUMENTS, "Unknown AI provider: '{}'. Supported providers are: 'openai', 'anthropic'", config.provider);
}
...
AIConfiguration AIClientFactory::loadConfiguration(const Poco::Util::AbstractConfiguration& config) {
  ...
  // Load basic settings
  if (config.has("ai.api_key")) ai_config.api_key = config.getString("ai.api_key");

  if (config.has("ai.provider")) ai_config.provider = config.getString("ai.provider");

  if (config.has("ai.model")) ai_config.model = config.getString("ai.model");

  // Load optional settings with defaults
  if (config.has("ai.temperature")) ai_config.temperature = config.getDouble("ai.temperature");

  if (config.has("ai.max_tokens")) ai_config.max_tokens = config.getUInt64("ai.max_tokens");

  if (config.has("ai.timeout_seconds")) ai_config.timeout_seconds = config.getUInt64("ai.timeout_seconds");

  if (config.has("ai.max_steps")) ai_config.max_steps = config.getUInt64("ai.max_steps");

  // Load custom system prompt if provided
  if (config.has("ai.system_prompt")) ai_config.system_prompt = config.getString("ai.system_prompt");

  // Load custom base URL for OpenAI-compatible APIs
  if (config.has("ai.base_url")) ai_config.base_url = config.getString("ai.base_url");

  if (config.has("ai.enable_schema_access")) ai_config.enable_schema_access = config.getBool("ai.enable_schema_access");
