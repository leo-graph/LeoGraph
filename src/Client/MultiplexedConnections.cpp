...
MultiplexedConnections::MultiplexedConnections(std::shared_ptr<Connection> connection_ptr_, ContextPtr context_,
...
MultiplexedConnections::MultiplexedConnections(std::vector<IConnectionPool::Entry>&& connections, ContextPtr context_,
                                               const ThrottlerPtr& throttler)
    : context(std::move(context_)), settings(context->getSettingsRef()) {
...
void MultiplexedConnections::sendScalarsData(Scalars& data) {
  ...
  for (ReplicaState& state : replica_states) {
    ...
    if (connection != nullptr) connection->sendScalarsData(data);
  }
}
...
void MultiplexedConnections::sendQueryPlan(const QueryPlan& query_plan) {
  ...
  for (ReplicaState& state : replica_states) {
    Connection* connection = state.connection;
    if (connection != nullptr) connection->sendQueryPlan(query_plan);
  }
}
...
void MultiplexedConnections::sendQuery(const ConnectionTimeouts& timeouts, const String& query, const String& query_id, UInt64 stage,
  ...
  for (auto& replica : replica_states) {
    if (!replica.connection) throw Exception(ErrorCodes::LOGICAL_ERROR, "MultiplexedConnections: Internal error");
  ...
  if (num_replicas > 1) {
    ...
    for (size_t i = 0; i < num_replicas; ++i) {
      if (enable_offset_parallel_processing) modified_settings[Setting::parallel_replica_offset] = i;

      replica_states[i].connection->sendQuery(timeouts, query, /* query_parameters */ {}, query_id, stage, &modified_settings, &client_info,
                                              with_pending_data, external_roles, {});
    }
  } else {
    /// Use single replica.
    replica_states[0].connection->sendQuery(timeouts, query, /* query_parameters */ {}, query_id, stage, &modified_settings, &client_info,
                                            with_pending_data, external_roles, {});
  }
...
void MultiplexedConnections::sendIgnoredPartUUIDs(const std::vector<UUID>& uuids) {
  ...
  for (ReplicaState& state : replica_states) {
    Connection* connection = state.connection;
    if (connection != nullptr) connection->sendIgnoredPartUUIDs(uuids);
  }
}
...
void MultiplexedConnections::sendCancel() {
  ...
  for (ReplicaState& state : replica_states) {
    Connection* connection = state.connection;
    if (connection != nullptr) connection->sendCancel();
  }
...
UInt64 MultiplexedConnections::receivePacketTypeUnlocked(AsyncCallback async_callback) {
  ...
  if (current_connection == nullptr) throw Exception(ErrorCodes::NO_AVAILABLE_REPLICA, "No available replica");
...
MultiplexedConnections::ReplicaState& MultiplexedConnections::getReplicaForReading() {
  if (replica_states.size() == 1) return replica_states[0];
  ...
  for (const ReplicaState& state : replica_states) {
    Connection* connection = state.connection;
    if ((connection != nullptr) && connection->hasReadPendingData()) read_list.push_back(*connection->socket);
  }
  ...
  if (read_list.empty()) {
    ...
    while (true) {
      ...
      for (const ReplicaState& state : replica_states) {
        Connection* connection = state.connection;
        if (connection != nullptr) read_list.push_back(*connection->socket);
      }

      /// poco returns 0 on EINTR, let's reset errno to ensure that EINTR came from select().
      errno = 0;

      n = Poco::Net::Socket::select(read_list, write_list, except_list, timeout);
    ...
    if (n == 0) {
      ...
      throw Exception(ErrorCodes::TIMEOUT_EXCEEDED, "Timeout ({} ms) exceeded while reading from {}", timeout.totalMilliseconds(),
                      addresses);
    }
  }
...
void MultiplexedConnections::setAsyncCallback(AsyncCallback async_callback) {
  for (ReplicaState& state : replica_states) {
    if (state.connection) state.connection->setAsyncCallback(async_callback);
  }
}
