...
class IConnectionPool : private boost::noncopyable {
 public:
  ...
  virtual Entry get(const ConnectionTimeouts& timeouts,  /// NOLINT
                    const Settings& settings, bool force_connected = true) = 0;
...
class ConnectionPool : public IConnectionPool, private PoolBase<Connection> {
 public:
  ...
  ConnectionPool(unsigned max_connections_, const String& host_, UInt16 port_, const String& default_database_, const String& user_,
                 const String& password_, const String& proto_send_chunked_, const String& proto_recv_chunked_, const String& quota_key_,
                 const String& cluster_, const String& cluster_secret_, const String& client_name_, Protocol::Compression compression_,
                 Protocol::Secure secure_, const String& bind_host_, Priority config_priority_ = Priority{1});
  ...
  Entry get(const ConnectionTimeouts& timeouts,  /// NOLINT
            const Settings& settings, bool force_connected) override;
 ...
 protected:
  ...
  ConnectionPtr allocObject() override {
    return std::make_shared<Connection>(host, port, default_database, user, password, proto_send_chunked, proto_recv_chunked, SSHKey(),
                                        /*jwt*/ "", quota_key, cluster, cluster_secret, client_name, compression, secure, "", bind_host);
  }
...
class ConnectionPoolFactory final : private boost::noncopyable {
 public:
  ...
  ConnectionPoolPtr get(unsigned max_connections, String host, UInt16 port, String default_database, String user, String password,
                        String proto_send_chunked, String proto_recv_chunked, String quota_key, String cluster, String cluster_secret,
                        String client_name, Protocol::Compression compression, Protocol::Secure secure, String bind_host,
                        Priority priority);
...
inline bool operator==(const ConnectionPoolFactory::Key& lhs, const ConnectionPoolFactory::Key& rhs) {
  return lhs.max_connections == rhs.max_connections && lhs.host == rhs.host && lhs.port == rhs.port &&
         ...
         lhs.quota_key == rhs.quota_key && lhs.cluster == rhs.cluster && lhs.cluster_secret == rhs.cluster_secret &&
         lhs.client_name == rhs.client_name && lhs.compression == rhs.compression && lhs.secure == rhs.secure &&
         lhs.bind_host == rhs.bind_host && lhs.priority == rhs.priority;
}
