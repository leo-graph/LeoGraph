...
JWTProvider::JWTProvider(std::string auth_url, std::string client_id, std::string audience, std::ostream &out, std::ostream &err)
...
std::string JWTProvider::getJWT() {
  ...
  if (!idp_access_token.empty() && now < idp_access_token_expires_at - expiration_buffer) return idp_access_token;
...
void JWTProvider::deviceCodeLogin() {
  ...
  Poco::Net::HTTPRequest device_code_request(Poco::Net::HTTPRequest::HTTP_POST, device_code_url.getPathAndQuery(),
  ...
  if (device_code_response.getStatus() != Poco::Net::HTTPResponse::HTTP_OK) {
    ...
    throw Exception(ErrorCodes::NETWORK_ERROR, "Error requesting device code: {} {}\nResponse: {}", device_code_response.getStatus(),
                    device_code_response.getReason(), error_body);
  }
  ...
  while (Poco::Timestamp().epochTime() < expires_at_ts) {
    ...
    std::string token_request_body =
        "grant_type=urn:ietf:params:oauth:grant-type:device_code&device_code=" + device_code + "&client_id=" + oauth_client_id;
    ...
    if (token_response.getStatus() == Poco::Net::HTTPResponse::HTTP_OK) {
      ...
      if (token_object->has("refresh_token")) idp_refresh_token = token_object->getValue<std::string>("refresh_token");
      return;
    }

    std::string error = token_object->optValue<std::string>("error", "unknown_error");
    if (error != "authorization_pending" && error != "slow_down") {
      throw Exception(ErrorCodes::AUTHENTICATION_FAILED, "IdP login failed: {}",
...
void JWTProvider::refreshIdPAccessToken() {
  ...
  idp_access_token_expires_at = Poco::Timestamp::fromEpochTime(jwt::decode(idp_access_token).get_payload_claim("exp").as_integer());
  if (object->has("refresh_token")) idp_refresh_token = object->getValue<std::string>("refresh_token");
}
...
std::unique_ptr<JWTProvider> createJwtProvider(const std::string &auth_url, const std::string &client_id, const std::string &audience,
                                               const std::string &host, std::ostream &out, std::ostream &err) {
