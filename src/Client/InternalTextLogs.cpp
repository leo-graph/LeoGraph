...
void InternalTextLogs::writeLogs(const Block &block) {
  ...
  for (size_t row_num = 0; row_num < block.rows(); ++row_num) {
    ...
    if (!host_name.empty()) {
      writeCString("[", wb);
      if (color) writeString(setColor(StringViewHash()(host_name)), wb);
      writeString(host_name, wb);
      if (color) writeCString(resetColor(), wb);
      writeCString("] ", wb);
    }
    ...
    writeCString(" [ ", wb);
    if (color) writeString(setColor(intHash64(thread_id)), wb);
    ...
    if (!query_id.empty()) {
      writeCString(" {", wb);
      if (color) writeString(setColor(StringViewHash()(query_id)), wb);
    ...
    writeCString(" <", wb);
    if (color) writeCString(setColorForLogPriority(priority), wb);
    writeString(InternalTextLogsQueue::getPriorityName(priority), wb);
    if (color) writeCString(resetColor(), wb);
    writeCString("> ", wb);

    auto source = column_source.getDataAt(row_num);
    if (color) writeString(setColor(StringViewHash()(source)), wb);
...
void InternalTextLogs::writeProfileEvents(const Block &block) {
  ...
  for (size_t row_num = 0; row_num < block.rows(); ++row_num) {
    ...
    if (color) writeString(setColor(StringViewHash()(name)), wb);
    ...
    writeCString(" (", wb);
    if (color) writeString(setColor(intHash64(type)), wb);
...
void InternalTextLogs::flush() { wb.next(); }
}  // namespace DB
