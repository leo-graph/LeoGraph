...
class HedgedConnectionsFactory {
 public:
  ...
  struct ReplicaStatus {
    explicit ReplicaStatus(std::unique_ptr<ConnectionEstablisherAsync> connection_stablisher_)
        : connection_establisher(std::move(connection_stablisher_)) {}
  ...
  HedgedConnectionsFactory(const ConnectionPoolWithFailoverPtr &pool_, const Settings &settings_, const ConnectionTimeouts &timeouts_,
                           UInt64 max_tries_, bool fallback_to_stale_replicas_, UInt64 max_parallel_replicas_,
                           bool skip_unavailable_shards_, std::shared_ptr<QualifiedTableName> table_to_check_ = nullptr,
