...
ConnectionEstablisher::ConnectionEstablisher(ConnectionPoolPtr pool_, const ConnectionTimeouts* timeouts_, const Settings& settings_,
                                             LoggerPtr log_, const QualifiedTableName* table_to_check_)
...
void ConnectionEstablisher::run(ConnectionEstablisher::TryResult& result, std::string& fail_message, bool force_connected) {
  try {
    ...
    if (table_to_check) server_revision = result.entry->getServerRevision(*timeouts);

    if (!table_to_check || server_revision < DBMS_MIN_REVISION_WITH_TABLES_STATUS) {
      if (!force_connected) result.entry->forceConnected(*timeouts);
    ...
    if (table_status_it == status_response.table_states_by_id.end()) {
      LOG_WARNING(LogToStr(fail_message, log), "There is no table {}.{} on server: {}", backQuote(table_to_check->database),
                  backQuote(table_to_check->table), result.entry->getDescription());
    ...
    if (table_status_it->second.is_readonly) {
      ...
      LOG_TRACE(log, "Table {}.{} is readonly on server {}", table_to_check->database, table_to_check->table,
    ...
    if (delay < max_allowed_delay) {
      ...
      LOG_TRACE(log, "Server {} has unacceptable replica delay for table {}.{}: {}", result.entry->getDescription(),
                table_to_check->database, table_to_check->table, delay);
...
ConnectionEstablisherAsync::ConnectionEstablisherAsync(ConnectionPoolPtr pool_, const ConnectionTimeouts* timeouts_,
                                                       const Settings& settings_, LoggerPtr log_, const QualifiedTableName* table_to_check_)
...
void ConnectionEstablisherAsync::Task::run(AsyncCallback async_callback, SuspendCallback) {
  ...
  connection_establisher_async.connection_establisher.run(connection_establisher_async.result, connection_establisher_async.fail_message,
                                                          connection_establisher_async.force_connected);
  connection_establisher_async.is_finished = true;
}

void ConnectionEstablisherAsync::processAsyncEvent(int fd, Poco::Timespan socket_timeout, AsyncEventTimeoutType type,
                                                   const std::string& description, uint32_t events) {
  socket_fd = fd;
...
bool ConnectionEstablisherAsync::checkTimeout() {
  ...
  for (size_t i = 0; i != ready_count; ++i) {
    if (events[i].data.fd == socket_fd) is_socket_ready = true;
    if (events[i].data.fd == timeout_descriptor.getDescriptor()) is_timeout_alarmed = true;
  }
  ...
  if (is_timeout_alarmed && !is_socket_ready) {
    if (haveMoreAddressesToConnect()) {
      ...
      if (!result.entry.isNull()) result.entry->setAddressConnectTimeoutExpired();
