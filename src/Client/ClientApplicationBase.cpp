...
static ClientInfo::QueryKind parseQueryKind(const String& query_kind) {
  if (query_kind == "initial_query") return ClientInfo::QueryKind::INITIAL_QUERY;
  if (query_kind == "secondary_query") return ClientInfo::QueryKind::SECONDARY_QUERY;
  if (query_kind == "no_query") return ClientInfo::QueryKind::NO_QUERY;
...
void interruptSignalHandler(int signum) {
  ...
  if (auto* instance = ClientApplicationBase::instanceRawPtr(); instance)
    if (auto* base = dynamic_cast<ClientApplicationBase*>(instance); base)
      if (base->tryStopQuery()) safeExit(128 + signum);
}
...
bool ClientApplicationBase::isEmbeeddedClient() const { return false; }
...
#else
  if (sigemptyset(&new_act.sa_mask)) throw ErrnoException(ErrorCodes::CANNOT_SET_SIGNAL_HANDLER, "Cannot set signal handler");
#endif

  if (sigaction(SIGINT, &new_act, nullptr)) throw ErrnoException(ErrorCodes::CANNOT_SET_SIGNAL_HANDLER, "Cannot set signal handler");

  if (sigaction(SIGQUIT, &new_act, nullptr)) throw ErrnoException(ErrorCodes::CANNOT_SET_SIGNAL_HANDLER, "Cannot set signal handler");
}
...
Poco::Util::LayeredConfiguration& ClientApplicationBase::getClientConfiguration() { return config(); }
...
void ClientApplicationBase::init(int argc, char** argv) {
...

  {
    ...
    for (const auto& option : options_description.main_description->options()) alias_names.insert(option->long_name());
  ...
  if (options.contains("client_logs_file")) {
    if (isEmbeeddedClient()) throw Exception(ErrorCodes::SUPPORT_IS_DISABLED, "Writing logs to a file is disabled in an embedded mode.");
