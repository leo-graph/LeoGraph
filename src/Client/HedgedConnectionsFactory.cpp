...
HedgedConnectionsFactory::HedgedConnectionsFactory(const ConnectionPoolWithFailoverPtr &pool_, const Settings &settings_,
                                                   const ConnectionTimeouts &timeouts_, UInt64 max_tries_, bool fallback_to_stale_replicas_,
                                                   UInt64 max_parallel_replicas_, bool skip_unavailable_shards_,
...
std::vector<Connection *> HedgedConnectionsFactory::getManyConnections(PoolMode pool_mode, AsyncCallback async_callback) {
  ...
  for (size_t i = 0; i != max_entries; ++i) {
    ...
    if (state == State::READY) connections.push_back(connection);
    if (state == State::CANNOT_CHOOSE) break;
  }
  ...
  while (connections.size() < max_entries) {
    ...
    if (state == State::READY)
      ...
      if (connections.size() >= min_entries) break;
      ...
      if (!fallback_to_stale_replicas && up_to_date_count < min_entries)
        throw Exception(DB::ErrorCodes::ALL_REPLICAS_ARE_STALE,
                        "Could not find enough connections to up-to-date replicas. Got: {}, needed: {}", connections.size(), min_entries);
      if (usable_count < min_entries)
        throw NetException(DB::ErrorCodes::ALL_CONNECTION_TRIES_FAILED, "All connection tries failed. Log: \n\n{}\n", fail_messages);
...
HedgedConnectionsFactory::State HedgedConnectionsFactory::startNewConnection(Connection *&connection_out) {
  ...
  if (state == State::CANNOT_CHOOSE && !epoll.empty()) state = State::NOT_READY;
...
HedgedConnectionsFactory::State HedgedConnectionsFactory::waitForReadyConnectionsImpl(bool blocking, Connection *&connection_out,
  ...
  if (!fallback_to_stale_replicas) return State::CANNOT_CHOOSE;
...
int HedgedConnectionsFactory::getNextIndex() {
  ...
  if (entries_count + replicas_in_process_count + failed_pools_count >= shuffled_pools.size()) return -1;
...
HedgedConnectionsFactory::State HedgedConnectionsFactory::startNewConnectionImpl(Connection *&connection_out) {
  ...
  do {
    index = getNextIndex();
    if (index == -1) return State::CANNOT_CHOOSE;
...
HedgedConnectionsFactory::State HedgedConnectionsFactory::processEpollEvents(bool blocking, Connection *&connection_out,
  ...
  while (!epoll.empty()) {
    ...
    if (event_fd == -1) return State::NOT_READY;
    ...
    if (fd_to_replica_index.contains(event_fd)) {
      ...
      if (state == State::NOT_READY) continue;
    ...
    if (startNewConnectionImpl(connection_out) == State::READY) return State::READY;
  }
...
HedgedConnectionsFactory::State HedgedConnectionsFactory::resumeConnectionEstablisher(int index, Connection *&connection_out) {
  ...
  if (replicas[index].connection_establisher->isCancelled()) return State::CANNOT_CHOOSE;
  ...
  if (!fd_to_replica_index.contains(fd)) addNewReplicaToEpoll(index, fd);

  return State::NOT_READY;
}

HedgedConnectionsFactory::State HedgedConnectionsFactory::processFinishedConnection(int index, TryResult result,
...
HedgedConnectionsFactory::State HedgedConnectionsFactory::setBestUsableReplica(Connection *&connection_out) {
  ...
  for (size_t i = 0; i != replicas.size(); ++i) {
    ...
    if (!result.entry.isNull() && result.is_usable && !replicas[i].is_ready &&
  ...
  if (indexes.empty()) return State::CANNOT_CHOOSE;
