...
CONV_FN(Index, idx) { ret += idx.index(); }

CONV_FN(Projection, proj) { ret += proj.projection(); }

CONV_FN(Constraint, constr) { ret += constr.constraint(); }

CONV_FN(Database, db) { ret += db.database(); }
...
CONV_FN(Function, func) { ret += func.function(); }
...
CONV_FN(Window, win) { ret += win.window(); }
...
CONV_FN_QUOTE(TypeName, tp) { TopTypeNameToString(ret, quote, tp.type()); }
...
CONV_FN(ExprLike, elike) {
  ...
  ret += " ";
  if (elike.not_() && elike.keyword() != ExprLike_PossibleKeywords::ExprLike_PossibleKeywords_REGEXP) ret += "NOT ";
...
CONV_FN(WhereStatement, ws) { ExprComparisonHighProbabilityToString(ret, ws.expr()); }
...
CONV_FN(TableEngine, te) {
  if (te.has_engine()) {
    ...
    ret += " ENGINE = ";
    if (te.has_toption() && ((teng >= TableEngineValues::MergeTree && teng <= TableEngineValues::GraphiteMergeTree) ||
