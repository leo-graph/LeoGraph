...
uint64_t RandomGenerator::getSeed() const { return seed; }

uint32_t RandomGenerator::nextSmallNumber() { return dist1(generator); }

uint32_t RandomGenerator::nextMediumNumber() { return dist2(generator); }

uint32_t RandomGenerator::nextLargeNumber() { return dist3(generator); }

uint8_t RandomGenerator::nextRandomUInt8() { return uints8(generator); }

int8_t RandomGenerator::nextRandomInt8() { return ints8(generator); }

uint16_t RandomGenerator::nextRandomUInt16() { return uints16(generator); }

int16_t RandomGenerator::nextRandomInt16() { return ints16(generator); }

uint32_t RandomGenerator::nextRandomUInt32() { return uints32(generator); }

int32_t RandomGenerator::nextRandomInt32() { return ints32(generator); }

uint64_t RandomGenerator::nextRandomUInt64() { return uints64(generator); }

int64_t RandomGenerator::nextRandomInt64() { return ints64(generator); }

uint64_t RandomGenerator::nextInFullRange() { return full_range(generator); }

uint32_t RandomGenerator::nextStrlen() { return strlens(generator); }

char RandomGenerator::nextDigit() { return static_cast<char>(digits(generator)); }

bool RandomGenerator::nextBool() { return dist4(generator) == 2; }
...
String RandomGenerator::nextDate(const String& separator, const bool allow_func) {
  ...
  return fmt::format("{}{}-{}{}-{}{}{}", separator, 1970 + date_years(generator), month < 10 ? "0" : "", month, day < 10 ? "0" : "", day,
                     separator);
}
...
String RandomGenerator::nextDate32(const String& separator, const bool allow_func) {
  ...
  const uint32_t month = months(generator);
  const uint32_t day = days[month - 1](generator);
  return fmt::format("{}{}-{}{}-{}{}{}", separator, 1900 + datetime64_years(generator), month < 10 ? "0" : "", month, day < 10 ? "0" : "",
                     day, separator);
}
...
String RandomGenerator::nextTime64(const String& separator, const bool allow_func, const bool has_subseconds) {
  ...
  const uint32_t minute = minutes(generator);
  const uint32_t second = minutes(generator);
  return fmt::format("{}{}:{}{}:{}{}{}{}{}", separator, hour, minute < 10 ? "0" : "", minute, second < 10 ? "0" : "", second,
                     has_subseconds ? "." : "", has_subseconds ? std::to_string(subseconds(generator)) : "", separator);
}
...
String RandomGenerator::nextDateTime(const String& separator, const bool allow_func, const bool has_subseconds) {
  ...
  const uint32_t minute = minutes(generator);
  const uint32_t second = minutes(generator);
  return fmt::format("{}{}-{}{}-{}{} {}{}:{}{}:{}{}{}{}{}", separator, 1970 + datetime_years(generator), month < 10 ? "0" : "", month,
...
String RandomGenerator::nextDateTime64(const String& separator, const bool allow_func, const bool has_subseconds) {
  ...
  const uint32_t minute = minutes(generator);
  const uint32_t second = minutes(generator);
  return fmt::format("{}{}-{}{}-{}{} {}{}:{}{}:{}{}{}{}{}", separator, 1900 + datetime64_years(generator), month < 10 ? "0" : "", month,
                     day < 10 ? "0" : "", day, hour < 10 ? "0" : "", hour, minute < 10 ? "0" : "", minute, second < 10 ? "0" : "", second,
                     has_subseconds ? "." : "", has_subseconds ? std::to_string(subseconds(generator)) : "", separator);
}
...
double RandomGenerator::randomZeroOne() { return zero_one(generator); }

String RandomGenerator::nextJSONCol() { return pickRandomly(jcols); }
...
String RandomGenerator::nextString(const String& delimiter, const bool allow_nasty, const uint32_t limit) {
  ...
  if (this->nextMediumNumber() > 2) {
    ...
    if (!use_bad_utf8 && this->nextMediumNumber() < 4) {
      ...
      if (delimiter.size() == 1 && c == delimiter[0]) c = delimiter[0] == 'a' ? 'b' : 'a';
      ret += String(this->randomInt<uint32_t>(0, std::min(limit, UINT32_C(65536))), c);
    } else {
      const String& pick =
          pickRandomly(use_bad_utf8 ? bad_utf8
...
String RandomGenerator::nextUUID() {
  ...
  return fmt::format("{}{}{}{}{}{}{}{}-{}{}{}{}-{}{}{}{}-{}{}{}{}-{}{}{}{}{}{}{}{}{}{}{}{}", hexDigits[hex_digits_dist(generator)],
                     hexDigits[hex_digits_dist(generator)], hexDigits[hex_digits_dist(generator)], hexDigits[hex_digits_dist(generator)],
                     hexDigits[hex_digits_dist(generator)], hexDigits[hex_digits_dist(generator)], hexDigits[hex_digits_dist(generator)],
                     hexDigits[hex_digits_dist(generator)], hexDigits[hex_digits_dist(generator)], hexDigits[hex_digits_dist(generator)],
                     hexDigits[hex_digits_dist(generator)], hexDigits[hex_digits_dist(generator)], hexDigits[hex_digits_dist(generator)],
                     hexDigits[hex_digits_dist(generator)], hexDigits[hex_digits_dist(generator)], hexDigits[hex_digits_dist(generator)],
