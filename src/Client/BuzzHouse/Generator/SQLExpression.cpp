...
void StatementGenerator::addColNestedAccess(RandomGenerator &rg, ExprColumn *expr, const uint32_t nested_prob) {
  ...
  if (!has_nested) {
    ...
    if (rg.nextMediumNumber() < nested_prob) {
      ...
      this->next_type_mask = fc.type_mask & ~(allow_nested);
      auto tp =
          std::unique_ptr<SQLType>(randomNextType(rg, this->next_type_mask, col_counter, expr->mutable_dynamic_subtype()->mutable_type()));
      this->next_type_mask = type_mask_backup;
    }
    if (nsuboption < 6) {
...
void StatementGenerator::generateLiteralValueInternal(RandomGenerator &rg, const bool complex, Expr *expr) {
  ...
  switch (static_cast<LitOp>(litGen.nextOp()))  /// drifts over time
  {
    ...
    case LitOp::LitInt: {
      ...
      if (complex && rg.nextSmallNumber() < 9) {
        std::uniform_int_distribution<uint32_t> integers_range(static_cast<uint32_t>(Integers::Int8), static_cast<uint32_t>(Integers::Int));
    ...
    case LitOp::LitSpecial: {
      ...
      val->set_val((sval != SpecialVal_SpecialValEnum_VAL_STAR || this->allow_not_deterministic ||
...
void StatementGenerator::generateLiteralValue(RandomGenerator &rg, const bool complex, Expr *expr) {
  ...
  if (nopt < 15 && ((this->next_type_mask & allow_array) != 0 || (complex && ((this->next_type_mask & allow_tuple) != 0)))) {
    ...
    ExprList *elist = ((this->next_type_mask & allow_tuple) == 0 || !complex || rg.nextBool()) ? expr->mutable_comp_expr()->mutable_array()
...
void StatementGenerator::generateSubquery(RandomGenerator &rg, ExplainQuery *eq) {
  ...
  if (rg.nextMediumNumber() < 6) {
    ...
    this->generateSelect(rg, true, false, 1, std::numeric_limits<uint32_t>::max(), std::nullopt,
...
Expr *StatementGenerator::generatePartialSearchExpr(RandomGenerator &rg, Expr *expr) const {
  ...
  SQLFuncCall *sfc = expr->mutable_comp_expr()->mutable_func_call();
  static const std::vector<SQLFunc> searchFuncs = {SQLFunc::FUNCendsWith,       SQLFunc::FUNChas,          SQLFunc::FUNChasToken,
                                                   SQLFunc::FUNChasTokenOrNull, SQLFunc::FUNCmapContains,  SQLFunc::FUNCmatch,
                                                   SQLFunc::FUNChasAllTokens,   SQLFunc::FUNChasAnyTokens, SQLFunc::FUNCstartsWith};
...
void StatementGenerator::generatePredicate(RandomGenerator &rg, Expr *expr) {
  ...
  switch (static_cast<PredOp>(predGen.nextOp()))  /// drifts over time
  {
    ...
    case PredOp::BinaryExpr: {
      ...
      std::uniform_int_distribution<uint32_t> op_range(limited ? static_cast<uint32_t>(BinaryOperator::BINOP_AND) : 1,
...
void StatementGenerator::generateFuncCall(RandomGenerator &rg, const bool allow_funcs, const bool allow_aggr, SQLFuncCall *func_call) {
  ...
  if (!nallow_funcs || nopt >= static_cast<uint32_t>(funcs_size)) {
    ...
    if (!this->functions.empty() && this->peer_query != PeerQuery::ClickHouseOnly &&
      ...
      const std::reference_wrapper<const SQLFunction> &func =
          this->allow_not_deterministic ? std::ref<const SQLFunction>(rg.pickValueRandomlyFromMap(this->functions))
...
void StatementGenerator::generateExpression(RandomGenerator &rg, Expr *expr) {
  ...
  expMask[static_cast<size_t>(ExpOp::WindowFuncExpr)] = this->fc.max_depth > this->depth &&
                                                        this->levels[this->current_level].allow_window_funcs &&
                                                        !this->levels[this->current_level].inside_aggregate;
  ...
  switch (static_cast<ExpOp>(expGen.nextOp()))  /// drifts over time
  {
    ...
    case ExpOp::JoinExpr: {
      ...
      const uint32_t nkeys = std::max<uint32_t>(1, std::min<uint32_t>(this->fc.max_width - this->width, rg.randomInt<uint32_t>(1, 3)));
    ...
    case ExpOp::LitAggrState: {
      ...
      static const std::vector<SQLFunc> simple_funcs = {
          SQLFunc::FUNCany, SQLFunc::FUNCanyLast, SQLFunc::FUNCgroupBitAnd, SQLFunc::FUNCgroupBitOr,      SQLFunc::FUNCgroupBitXor,
          SQLFunc::FUNCmax, SQLFunc::FUNCmin,     SQLFunc::FUNCsum,         SQLFunc::FUNCsumWithOverflow,
      };
      ...
      if (use_random_bytes) {
        ...
        for (uint32_t i = 0; i < nchunks; i++) hex_bytes += fmt::format("{:016x}", rg.nextInFullRange());
      }
      ...
      constexpr uint64_t scalar_mask = allow_unsigned_int | allow_int8 | allow_int16 | allow_int64 | allow_float32 | allow_float64 |
                                       allow_strings | allow_dates | allow_datetimes | allow_decimals | allow_nullable |
      ...
      if (!use_simple && !use_if && aggr == SQLFunc::FUNCcount) {
        ...
        expr->mutable_lit_val()->set_no_quote_str(
            use_random_bytes ? fmt::format("finalizeAggregation(CAST(unhex('{}'), 'AggregateFunction(count)'))", hex_bytes)
        ...
        if (use_simple) {
          ...
          init_args = fmt::format("'{}', CAST({}, '{}'), CAST(1, 'UInt8')", func_name, subtype->appendRandomRawValue(rg, *this), type_name);
        ...
        expr->mutable_lit_val()->set_no_quote_str(
            use_random_bytes ? fmt::format("finalizeAggregation(CAST(unhex('{}'), '{}'))", hex_bytes, aggr_type)
    ...
    case ExpOp::LitReinterpret: {
      ...
      if (rg.nextSmallNumber() < 4) {
        ...
        static const std::vector<std::pair<String, uint32_t>> reinterpret_targets = {
            ...
            {"Float32", 4}, {"Float64", 8}, {"Date", 2},   {"DateTime", 4}, {"UUID", 16},
        };
      ...
      if (this->allow_not_deterministic && rg.nextBool()) {
        ...
        String hex;
        for (uint32_t i = 0; i < byte_count; i++) hex += fmt::format("{:02x}", rg.randomInt<uint8_t>(0, 255));
