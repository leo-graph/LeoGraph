...
const DB::Strings compressionMethods = {"auto", "none", "gz",   "gzip", "deflate", "brotli", "br",
                                        "xz",   "zst",  "zstd", "lzma", "lz4",     "bz2",    "snappy"};

const DB::Strings codecs = {"LZ4", "LZ4HC", "ZSTD", "Delta",           "DoubleDelta",     "Gorilla", "T64",
                            "FPC", "GCD",   "ALP",  "AES_128_GCM_SIV", "AES_256_GCM_SIV", "NONE"};
...
static std::optional<ServerCredentials> loadServerCredentials(const JSONParserImpl::Element &jobj, const String &sname,
                                                              const uint32_t default_port, const uint32_t default_mysql_port = 0) {
  uint32_t port = default_port;
  ...
  for (const auto [key, value] : jobj.getObject()) {
    ...
    if (!configEntries.contains(nkey)) throw DB::Exception(DB::ErrorCodes::BUZZHOUSE, "Unknown server option: {}", nkey);

    configEntries.at(nkey)(value);
  }

  return std::optional<ServerCredentials>(ServerCredentials(client_hostname, server_hostname, container, port, mysql_port, unix_socket,
                                                            user, password, secret, database, named_collection, user_files_dir,
                                                            query_log_file, glue_catalog, hive_catalog, rest_catalog, unity_catalog));
}

static PerformanceMetric loadPerformanceMetric(const JSONParserImpl::Element &jobj, const uint32_t default_threshold,
                                               const uint32_t default_minimum) {
  bool enabled = false;
  ...
  for (const auto [key, value] : jobj.getObject()) {
    ...
    if (!metricEntries.contains(nkey)) throw DB::Exception(DB::ErrorCodes::BUZZHOUSE, "Unknown metric option: {}", nkey);
...
static std::function<void(const JSONObjectType &)> parseDisabledOptions(uint64_t &res, const String &text,
                                                                        const std::unordered_map<std::string_view, uint64_t> &entries) {
...
FuzzConfig::FuzzConfig(DB::ClientBase *c, const String &path) : cb(c), log(getLogger("BuzzHouse")) {
  ...
  static const std::unordered_map<std::string_view, uint64_t> type_entries = {{"bool", allow_bool},
  ...
  const SettingEntries configEntries = {
      ...
      {"max_reconnection_attempts",
       [&](const JSONObjectType &value) { max_reconnection_attempts = std::max(UINT32_C(1), static_cast<uint32_t>(value.getUInt64())); }},
  ...
  if (min_insert_rows > max_insert_rows) {
    throw DB::Exception(DB::ErrorCodes::BUZZHOUSE, "min_insert_rows value ({}) is higher than max_insert_rows value ({})", min_insert_rows,
                        max_insert_rows);
  }
  if (min_nested_rows > max_nested_rows) {
    throw DB::Exception(DB::ErrorCodes::BUZZHOUSE, "min_nested_rows value ({}) is higher than max_nested_rows value ({})", min_nested_rows,
                        max_nested_rows);
  }
  if (min_string_length > max_string_length) {
    throw DB::Exception(DB::ErrorCodes::BUZZHOUSE, "min_string_length value ({}) is higher than max_string_length value ({})",
...
bool FuzzConfig::processServerQuery(const bool outlog, const String &query) {
  ...
  if (!res) {
    if (!caught_exception) LOG_ERROR(log, "Unknown error on processing query '{}'\n", query);
...
void FuzzConfig::loadServerSettings(std::vector<T> &out, const String &desc, const String &query) {
  ...
  if (processServerQuery(false,
                         fmt::format(R"({} INTO OUTFILE '{}' TRUNCATE FORMAT TabSeparated;)", query, fuzzer_out_file.generic_string()))) {
...
void FuzzConfig::loadServerConfigurations() {
  ...
  loadServerSettings<String>(this->storage_policies, "storage policies",
                             R"(SELECT DISTINCT "policy_name" FROM "system"."storage_policies")");
  loadServerSettings<String>(this->disks, "disks", R"(SELECT DISTINCT "name" FROM "system"."disks")");
  loadServerSettings<String>(this->keeper_disks, "keeper disks",
                             R"(SELECT DISTINCT "name" FROM "system"."disks" WHERE metadata_type = 'Keeper')");
  ...
  loadServerSettings<String>(this->failpoints, "failpoints",
...
void FuzzConfig::loadSystemTables(std::vector<SystemTable> &tables) {
  ...
  if (processServerQuery(
          false,
          fmt::format("SELECT c.database, c.table, c.name from system.columns c WHERE c.database IN ('system', 'INFORMATION_SCHEMA', "
...
bool FuzzConfig::tableHasPartitions(const bool detached, const String &database, const String &table) {
  ...
  if (processServerQuery(
          ...
          fmt::format(
              ...
              detached_tbl, db_clause, table, fuzzer_out_file.generic_string()))) {
    std::ifstream infile(fuzzer_out_file);
...
bool FuzzConfig::hasMutations() {
  ...
  if (processServerQuery(false, fmt::format(R"(SELECT count() FROM "system"."mutations" INTO OUTFILE '{}' TRUNCATE FORMAT TabSeparated;)",
                                            fuzzer_out_file.generic_string()))) {
    std::ifstream infile(fuzzer_out_file);
    if (std::getline(infile, buf)) {
      return !buf.empty() && buf[0] != '0';
...
String FuzzConfig::getRandomMutation(const uint64_t rand_val) {
  ...
  if (processServerQuery(
          false,
          fmt::format("SELECT z.y FROM (SELECT (row_number() OVER () - 1) AS x, \"mutation_id\" AS y FROM \"system\".\"mutations\") as z "
                      "WHERE z.x = (SELECT {} % max2(count(), 1) FROM \"system\".\"mutations\") INTO OUTFILE '{}' TRUNCATE "
                      "FORMAT TabSeparated;",
                      rand_val, fuzzer_out_file.generic_string()))) {
...
String FuzzConfig::getRandomIcebergHistoryValue(const String &property) {
  ...
  if (processServerQuery(
          false, fmt::format(
                     ...
                     property, fuzzer_out_file.generic_string()))) {
    std::ifstream infile(fuzzer_out_file, std::ios::in);
    std::getline(infile, res);
  }
...
String FuzzConfig::tableGetRandomPartitionOrPart(const uint64_t rand_val, const bool detached, const bool partition, const String &database,
                                                 const String &table) {
  String res;
  const String &detached_tbl = detached ? "detached_parts" : "parts";
  const String &db_clause = database.empty() ? "" : (R"("database" = ')" + database + "' AND ");
  ...
  if (processServerQuery(
          ...
          fmt::format(
              ...
              partition ? "partition_id" : "name", detached_tbl, db_clause, table, rand_val, detached_tbl, db_clause, table,
              fuzzer_out_file.generic_string()))) {
    std::ifstream infile(fuzzer_out_file, std::ios::in);
    std::getline(infile, res);
  }
...
void FuzzConfig::validateClickHouseHealth() {
  if (processServerQuery(
    ...
    size_t i = 0;
    std::ifstream infile(fuzzer_out_file, std::ios::in);
    static const DB::Strings health_errors = {"broken detached part(s)",
                                              "broken replica(s)",
                                              "broken data part(s)",
    ...
    while (std::getline(infile, buf) && !buf.empty() && i < health_errors.size()) {
      ...
      if (val != 0) {
        ...
        if (i < detail_queries.size() && !detail_queries[i].empty() &&
            processServerQuery(false, fmt::format("{} INTO OUTFILE '{}' TRUNCATE FORMAT TabSeparated;", detail_queries[i],
                                                  fuzzer_out_file.generic_string()))) {
          String dbuf;
          std::ifstream detail_file(fuzzer_out_file, std::ios::in);
          while (std::getline(detail_file, dbuf)) {
            if (!dbuf.empty()) details += "\n  " + dbuf;
          }
        }
        throw DB::Exception(DB::ErrorCodes::BUZZHOUSE, "ClickHouse health check on {}:{}: found {} {}{}", host, port, val, health_errors[i],
                            details.empty() ? "" : "\nDetails:" + details);
      }
      i++;
      buf.resize(0);
      ...
      if (val.enabled) {
        if (val.minimum < server.metrics.at(key) &&
            server.metrics.at(key) >
                static_cast<uint64_t>(static_cast<double>(peer.metrics.at(key)) * (1 + (static_cast<double>(val.threshold) / 100.0f)))) {
          throw DB::Exception(DB::ErrorCodes::BUZZHOUSE, "{}: ClickHouse peer server {}: {} was less than the target server: {}",
                              oracle_name, key, peer.result_strings.at(key), server.result_strings.at(key));
        }
      }
    }
  }
  for (const auto &[key, val] : metrics) {
    LOG_INFO(log, "{}: server {}: {} vs peer {}: {}", oracle_name, key, server.result_strings.at(key), key, peer.result_strings.at(key));
  }
}
