...
static const auto bucketsRangeSetting =
    CHSetting([](RandomGenerator &rg, FuzzConfig &) { return std::to_string(rg.thresholdGenerator<uint64_t>(0.2, 0.2, 0, 16)); },
              {"0", "1", "2", "4", "8", "16"}, false);
...
static std::unordered_map<String, CHSetting> mergeTreeTableSettings = {
    ...
    {"alter_column_secondary_index_mode", CHSetting(
                                              [](RandomGenerator &rg, FuzzConfig &) {
                                                static const DB::Strings choices = {"'throw'", "'drop'", "'rebuild'", "'compatibility'"};
                                                return rg.pickRandomly(choices);
                                              },
                                              {"'throw'", "'drop'", "'rebuild'", "'compatibility'"}, false)},
    ...
    {"auto_statistics_types", CHSetting(
                                  [](RandomGenerator &rg, FuzzConfig &) {
                                    String res;
                                  ...
                                  {"'tdigest'", "'countmin'", "'minmax'", "'uniq'"}, false)},
    ...
    {"concurrent_part_removal_threshold",
     CHSetting([](RandomGenerator &rg, FuzzConfig &) { return std::to_string(rg.thresholdGenerator<uint64_t>(0.2, 0.2, 0, 100)); }, {},
               false)},
    {"deduplicate_merge_projection_mode", CHSetting(
                                              [](RandomGenerator &rg, FuzzConfig &) {
                                                static const DB::Strings choices = {"'ignore'", "'throw'", "'drop'", "'rebuild'"};
                                                return rg.pickRandomly(choices);
                                              },
                                              {"'ignore'", "'throw'", "'drop'", "'rebuild'"}, false)},
    ...
    {"distributed_index_analysis_min_indexes_size_to_activate",
     CHSetting([](RandomGenerator &rg, FuzzConfig &) { return std::to_string(rg.thresholdGenerator<uint64_t>(0.2, 0.2, 0, 128)); },
               {"0", "1"}, false)},
    {"dynamic_serialization_version", CHSetting(
                                          [](RandomGenerator &rg, FuzzConfig &) {
    ...
    {"exclude_materialize_skip_indexes_on_merge", CHSetting(
                                                      [](RandomGenerator &rg, FuzzConfig &) {
                                                        String res;
                                                        std::vector<uint32_t> choices = {0, 1, 2, 3, 4};
                                                        const uint32_t nchoices =
                                                            rg.randomInt<uint32_t>(0, static_cast<uint32_t>(choices.size()));
    ...
    {"lightweight_mutation_projection_mode", CHSetting(
                                                 [](RandomGenerator &rg, FuzzConfig &) {
                                                   static const DB::Strings choices = {"'throw'", "'drop'", "'rebuild'"};
                                                   return rg.pickRandomly(choices);
                                                 },
                                                 {"'throw'", "'drop'", "'rebuild'"}, false)},
    ...
    {"max_files_to_modify_in_alter_columns",
     CHSetting([](RandomGenerator &rg, FuzzConfig &) { return std::to_string(rg.thresholdGenerator<uint64_t>(0.2, 0.2, 1, 100)); }, {},
               false)},
    ...
    {"max_projections",
     CHSetting([](RandomGenerator &rg, FuzzConfig &) { return std::to_string(rg.thresholdGenerator<uint64_t>(0.3, 0.2, 0, 25)); },
               {"0", "1", "5", "25"}, false)},
    {"max_parts_to_merge_at_once",
     CHSetting([](RandomGenerator &rg, FuzzConfig &) { return std::to_string(rg.thresholdGenerator<uint64_t>(0.2, 0.2, 0, 1000)); },
               {"0", "1", "2", "8", "10", "100"}, false)},
    ...
    {"merge_max_dynamic_subcolumns_in_wide_part",
     CHSetting([](RandomGenerator &rg, FuzzConfig &) { return std::to_string(rg.thresholdGenerator<uint64_t>(0.2, 0.2, 0, 100)); },
               {"0", "1", "2", "8", "10", "100"}, false)},
    {"merge_selector_algorithm", CHSetting(
                                     [](RandomGenerator &rg, FuzzConfig &) {
                                       static const DB::Strings choices = {"'Simple'", "'Trivial'", "'StochasticSimple'"};
                                       return rg.pickRandomly(choices);
                                     },
                                     {"'Simple'", "'Trivial'", "'StochasticSimple'"}, false)},
    {"merge_selector_base",
     CHSetting([](RandomGenerator &rg, FuzzConfig &) { return std::to_string(rg.thresholdGenerator<double>(0.2, 0.2, 0.0, 8.0)); }, {},
               false)},
    ...
    {"min_age_to_force_merge_seconds",
     CHSetting([](RandomGenerator &rg, FuzzConfig &) { return std::to_string(rg.thresholdGenerator<uint64_t>(0.3, 0.2, 0, 60)); },
               {"0", "1", "5", "10"}, false)},
    {"min_bytes_for_compact_part", bytesRangeSetting},
    {"min_bytes_for_full_part_storage", bytesRangeSetting},
    {"min_bytes_for_wide_part", CHSetting(
                                    [](RandomGenerator &rg, FuzzConfig &) {
                                      return std::to_string(
                                          rg.thresholdGenerator<uint64_t>(0.4, 0.2, 0, UINT32_C(10) * UINT32_C(1024) * UINT32_C(1024)));
                                    },
                                    {"0", "1"}, false)},
    ...
    {"min_level_for_full_part_storage",
     CHSetting([](RandomGenerator &rg, FuzzConfig &) { return std::to_string(rg.thresholdGenerator<uint64_t>(0.2, 0.2, 0, 10)); },
               {"0", "1"}, false)},
    ...
    {"number_of_free_entries_in_pool_to_execute_mutation",
     CHSetting([](RandomGenerator &rg, FuzzConfig &) { return std::to_string(rg.thresholdGenerator<uint64_t>(0.2, 0.2, 1, 30)); }, {},
               false)},
    {"number_of_free_entries_in_pool_to_execute_optimize_entire_partition",
     CHSetting([](RandomGenerator &rg, FuzzConfig &) { return std::to_string(rg.thresholdGenerator<uint64_t>(0.2, 0.2, 1, 30)); }, {},
               false)},
    {"number_of_free_entries_in_pool_to_lower_max_size_of_merge",
     CHSetting([](RandomGenerator &rg, FuzzConfig &) { return std::to_string(rg.thresholdGenerator<uint64_t>(0.2, 0.2, 1, 20)); }, {},
               false)},
    {"number_of_mutations_to_delay",
     CHSetting([](RandomGenerator &rg, FuzzConfig &) { return std::to_string(rg.thresholdGenerator<uint64_t>(0.2, 0.2, 10, 1000)); }, {},
               false)},
    {"number_of_mutations_to_throw",
     CHSetting([](RandomGenerator &rg, FuzzConfig &) { return std::to_string(rg.thresholdGenerator<uint64_t>(0.2, 0.2, 20, 2000)); }, {},
               false)},
    {"object_serialization_version", CHSetting(
                                         [](RandomGenerator &rg, FuzzConfig &) {
                                           static const DB::Strings choices = {"'v1'", "'v2'", "'v3'"};
                                           return rg.pickRandomly(choices);
                                         },
    ...
    {"object_shared_data_serialization_version", CHSetting(
                                                     [](RandomGenerator &rg, FuzzConfig &) {
                                                       static const DB::Strings choices = {"'map'", "'map_with_buckets'", "'advanced'"};
                                                       return rg.pickRandomly(choices);
                                                     },
                                                     {"'map'", "'map_with_buckets'", "'advanced'"}, false)},
    {"object_shared_data_serialization_version_for_zero_level_parts", CHSetting(
                                                                          [](RandomGenerator &rg, FuzzConfig &) {
                                                                          ...
                                                                          {"'map'", "'map_with_buckets'", "'advanced'"}, false)},
    {"old_parts_lifetime",
     CHSetting([](RandomGenerator &rg, FuzzConfig &) { return std::to_string(rg.thresholdGenerator<uint64_t>(0.2, 0.2, 10, 8 * 60)); }, {},
               false)},
    ...
    {"refresh_parts_interval",
     CHSetting([](RandomGenerator &rg, FuzzConfig &) { return std::to_string(rg.thresholdGenerator<uint64_t>(0.2, 0.2, 0, 5)); },
               {"0", "1", "2", "5"}, false)},
    ...
    {"replicated_deduplication_window_seconds",
     CHSetting([](RandomGenerator &rg, FuzzConfig &) { return std::to_string(rg.thresholdGenerator<uint64_t>(0.2, 0.2, 0, 3600)); }, {},
               false)},
    {"replicated_deduplication_window_seconds_for_async_inserts",
     CHSetting([](RandomGenerator &rg, FuzzConfig &) { return std::to_string(rg.thresholdGenerator<uint64_t>(0.2, 0.2, 0, 86400)); }, {},
               false)},
    {"replicated_max_mutations_in_one_entry",
     CHSetting([](RandomGenerator &rg, FuzzConfig &) { return std::to_string(rg.thresholdGenerator<uint64_t>(0.2, 0.2, 0, 10000)); }, {},
               false)},
    {"replicated_max_ratio_of_wrong_parts", probRangeSetting},
    {"search_orphaned_parts_disks", CHSetting(
                                        [](RandomGenerator &rg, FuzzConfig &) {
                                          static const DB::Strings choices = {"'none'", "'local'", "'any'"};
                                          return rg.pickRandomly(choices);
                                        },
                                        {"'none'", "'local'", "'any'"}, false)},
    {"serialization_info_version", CHSetting(
                                       [](RandomGenerator &rg, FuzzConfig &) {
                                         static const DB::Strings choices = {"'basic'", "'with_types'"};
                                         return rg.pickRandomly(choices);
                                       },
                                       {"'basic'", "'with_types'"}, false)},
    /// ClickHouse cloud setting
    ...
    {"simultaneous_parts_removal_limit",
     CHSetting([](RandomGenerator &rg, FuzzConfig &) { return std::to_string(rg.thresholdGenerator<uint64_t>(0.2, 0.2, 0, 128)); },
               {"0", "1", "2", "8", "10", "100"}, false)},
    {"string_serialization_version", CHSetting(
                                         [](RandomGenerator &rg, FuzzConfig &) {
                                           static const DB::Strings choices = {"'single_stream'", "'with_size_stream'"};
                                           return rg.pickRandomly(choices);
                                         },
                                         {"'single_stream'", "'with_size_stream'"}, false)},
    {"nullable_serialization_version", CHSetting(
                                           [](RandomGenerator &rg, FuzzConfig &) {
                                             static const DB::Strings choices = {"'basic'", "'allow_sparse'"};
                                             return rg.pickRandomly(choices);
                                           },
                                           {"'basic'", "'allow_sparse'"}, false)},
    ...
    {"vertical_merge_algorithm_min_columns_to_activate",
     CHSetting([](RandomGenerator &rg, FuzzConfig &) { return std::to_string(rg.thresholdGenerator<uint64_t>(0.2, 0.2, 1, 16)); },
               {"0", "1"}, false)},
...
std::unordered_map<String, CHSetting> restoreSettings = {{"allow_azure_native_copy", trueOrFalseSettingNoOracle},
...
static std::unordered_map<String, CHSetting> flatLayoutSettings = {{"INITIAL_ARRAY_SIZE", CHSetting(bytesRange, {}, false)},
                                                                   {"MAX_ARRAY_SIZE", CHSetting(bytesRange, {}, false)}};
...
static std::unordered_map<String, CHSetting> hashedLayoutSettings = {
    ...
    {"SHARD_LOAD_QUEUE_BACKLOG", CHSetting(
                                     [](RandomGenerator &rg, FuzzConfig &) {
                                       return std::to_string(rg.thresholdGenerator<uint64_t>(
                                           0.2, 0.2, 0, UINT32_C(10) * UINT32_C(1024) * UINT32_C(1024) * UINT32_C(1024)));
                                     },
                                     {}, false)},
...
static std::unordered_map<String, CHSetting> cachedLayoutSettings = {
    ...
    {"MAX_UPDATE_QUEUE_SIZE", CHSetting(
                                  [](RandomGenerator &rg, FuzzConfig &) {
                                    return std::to_string(rg.thresholdGenerator<uint64_t>(
                                        0.2, 0.2, 0, UINT32_C(10) * UINT32_C(1024) * UINT32_C(1024) * UINT32_C(1024)));
                                  },
...
static std::unordered_map<String, CHSetting> ssdCachedLayoutSettings = {{"BLOCK_SIZE", CHSetting(bytesRange, {}, false)},
...
static std::unordered_map<String, CHSetting> fileTableSettings = {
    ...
    {"storage_file_read_method", CHSetting(
                                     ...
                                     {"'read'", "'pread'", "'mmap'"}, false)}};

static std::unordered_map<String, CHSetting> distributedTableSettings = {
    {"background_insert_batch", trueOrFalseSetting}, {"background_insert_split_batch_on_failure", trueOrFalseSetting},
    {"flush_on_detach", trueOrFalseSetting},         {"fsync_after_insert", trueOrFalseSetting},
    {"fsync_directories", trueOrFalseSetting},       {"skip_unavailable_shards", trueOrFalseSetting}};

static std::unordered_map<String, CHSetting> memoryTableSettings = {{"min_bytes_to_keep", CHSetting(bytesRange, {}, false)},
...
void loadFuzzerTableSettings(const FuzzConfig &fc) {
  ...
  if (fc.enable_fault_injection_settings) {
    mergeTreeTableSettings.insert({{"fault_probability_after_part_commit", CHSetting(probRange, {}, false)},
  ...
  std::unordered_map<String, CHSetting> queueSettings = {
      {"after_processing", CHSetting(
                               [](RandomGenerator &rg, FuzzConfig &) {
                                 static const DB::Strings choices = {"'keep'", "'delete'", "'move'", "'tag'"};
                                 return rg.pickRandomly(choices);
                               },
                               {"'keep'", "'delete'", "'move'", "'tag'"}, false)},
      {"buckets",
       CHSetting([](RandomGenerator &rg, FuzzConfig &) { return std::to_string(rg.thresholdGenerator<uint64_t>(0.2, 0.2, 0, 3000)); },
                 {"0", "1", "2", "8", "10", "100", "1000"}, false)},
      {"bucketing_mode", CHSetting(
                             [](RandomGenerator &rg, FuzzConfig &) {
      ...
      {"partitioning_mode", CHSetting(
                                [](RandomGenerator &rg, FuzzConfig &) {
                                  static const DB::Strings choices = {"'none'", "'hive'", "'regex'"};
                                  return rg.pickRandomly(choices);
                                },
                                {"'none'", "'hive'", "'regex'"}, false)},
  ...
  s3QueueTableSettings.insert({{"s3queue_buckets", bucketsRangeSetting},
  ...
  allTableSettings.insert({{MergeTree, mergeTreeTableSettings},
  ...
  allColumnSettings.insert({{MergeTree, mergeTreeColumnSettings},
  ...
  allDictionaryLayoutSettings.insert({{CACHE, cachedLayoutSettings},
