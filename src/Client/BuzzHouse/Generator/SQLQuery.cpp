...
void StatementGenerator::generateDerivedTable(RandomGenerator &rg, SQLRelation &rel, const uint32_t allowed_clauses, const uint32_t ncols,
                                              const bool backup, std::optional<String> recursive, Select *sel) {
  Select *osel = sel->New();
  std::unordered_map<uint32_t, QueryLevel> levels_backup;
  std::unordered_map<uint32_t, std::unordered_map<String, SQLRelation>> ctes_backup;
...
void StatementGenerator::setTableFunction(RandomGenerator &rg, const TableFunctionUsage usage, const SQLTable &t, TableFunction *tfunc) {
  ...
  } else if ((usage == TableFunctionUsage::EngineReplace && t.isPostgreSQLEngine()) ||
  ...
  } else if ((usage == TableFunctionUsage::EngineReplace && t.isSQLiteEngine()) ||
             (usage == TableFunctionUsage::PeerTable && t.hasSQLitePeer())) {
      ...
      if (!this->allow_not_deterministic || rg.nextLargeNumber() < 971) {
        switch (t.teng) {
          ...
          case TableEngineValues::S3Queue:
            val = rg.nextBool() ? ObjectStoreFunc_FName::ObjectStoreFunc_FName_s3 : ObjectStoreFunc_FName::ObjectStoreFunc_FName_gcs;
            break;
...
bool StatementGenerator::joinedTableOrFunction(RandomGenerator &rg, const String &rel_name, const uint32_t allowed_clauses,
                                               const bool under_remote, TableOrFunction *tof) {
  ...
  const auto has_view_lambda = [&](const SQLView &vv) { return vv.isAttached() && (vv.is_deterministic || this->allow_not_deterministic); };
  const auto has_dictionary_lambda = [&](const SQLDictionary &d) {
    return d.isAttached() && (d.is_deterministic || this->allow_not_deterministic);
  };
  ...
  switch (nopt) {
    case QueryOp::DerivatedTable: {
      ...
      if (ncols == 1 && rg.nextMediumNumber() < 6) {
        ...
        generateDerivedTable(rg, rel, allowed_clauses, ncols, true, std::nullopt,
    ...
    case QueryOp::RandomDataUDF: {
      ...
      addRandomRelation(rg, rel_name, (rg.nextSmallNumber() < 8) ? rg.nextSmallNumber() : rg.nextMediumNumber(),
...
void StatementGenerator::addWhereSide(RandomGenerator &rg, const std::vector<GroupCol> &available_cols, SQLType *tp,
                                      const ColumnSpecial special, Expr *expr) {
  ...
  switch (static_cast<PredOp>(predGen.nextOp()))  /// drifts over time
  {
    ...
    case PredOp::UnaryExpr: {
      ...
      if (rg.nextSmallNumber() < 8) {
        ...
        /// Sometimes do the function call instead
        SQLFuncCall *sfc = expr->mutable_comp_expr()->mutable_func_call();
        static const auto nullFuncs = {SQLFunc::FUNCisNull, SQLFunc::FUNCisNullable, SQLFunc::FUNCisNotNull, SQLFunc::FUNCisZeroOrNull};
    ...
    case PredOp::InExpr: {
      ...
      if (rg.nextSmallNumber() < 8) {
        ...
        if (rg.nextBool()) {
          ...
          for (uint32_t i = 0; i < nclauses; i++) {
            addWhereSide(rg, available_cols, gcol.getType(), gcol.getSpecial(), i == 0 ? elist->mutable_expr() : elist->add_extra_exprs());
          }
        } else {
...
void StatementGenerator::generateGroupByExpr(RandomGenerator &rg, const bool enforce_having, const uint32_t offset, const uint32_t ncols,
...
bool StatementGenerator::generateGroupBy(RandomGenerator &rg, const uint32_t ncols, const bool enforce_having, const bool allow_settings,
  ...
  if (enforce_having || !allow_settings || rg.nextSmallNumber() < (available_cols.empty() ? 3 : 9)) {
    ...
    if (no_grouping_sets) {
      /// Group list
      ExprList *elist =
          (!allow_settings || next_opt < 51) ? gbl->mutable_exprs() : ((next_opt < 71) ? gbl->mutable_rollup() : gbl->mutable_cube());
      ...
      for (uint32_t i = 0; i < nclauses; i++) {
        const uint32_t nelems =
            std::min<uint32_t>(this->fc.max_width - this->width,
...
void StatementGenerator::generateOrderBy(RandomGenerator &rg, const uint32_t ncols, const bool allow_settings, const bool is_window,
                                         OrderByStatement *ob) {
  if (allow_settings && !is_window && rg.nextSmallNumber() < 3) {
    ...
    for (uint32_t i = 0; i < nclauses; i++) {
      ...
      if (allow_settings) {
        ...
        if (rg.nextSmallNumber() < 7) {
          eot->set_nulls_order(rg.nextBool() ? ExprOrderingTerm_NullsOrder::ExprOrderingTerm_NullsOrder_FIRST
  ...
  for (uint32_t i = 0; i < nclauses; i++) {
    ...
    if (rg.nextBool()) {
      ...
      const bool recursive =
          fc.allow_infinite_tables && this->depth<this->fc.max_depth &&this->fc.max_width> this->width + 1 && rg.nextSmallNumber() < 4;

      nqcte->set_recursive(recursive);
      generateDerivedTable(rg, rel, allowed_clauses, ncols, rg.nextSmallNumber() < 7,
                           recursive ? std::make_optional<String>(name) : std::nullopt, nqcte->mutable_query());
...
void StatementGenerator::generateSelect(RandomGenerator &rg, const bool top, bool force_global_agg, const uint32_t ncols,
                                        uint32_t allowed_clauses, std::optional<String> recursive, Select *sel) {
  CTEs *qctes = nullptr;
  ...
  if ((allowed_clauses & allow_set) && !force_global_agg && this->depth<this->fc.max_depth &&this->fc.max_width> this->width + 1 &&
    ...
    setq->set_set_op(recursive.has_value() ? SetQuery_SetOp::SetQuery_SetOp_UNION : static_cast<SetQuery_SetOp>(set_range(rg.generator)));
    ...
    if ((allowed_clauses & allow_qualify) && this->depth < this->fc.max_depth && this->width < this->fc.max_width &&
      ...
      if ((ssc->has_pre_where() || ssc->has_where() || (ssc->has_groupby() && ssc->groupby().has_having_expr())) &&
        ...
        /// Sometimes use the same predicate for having and where
        if (ssc->has_pre_where()) candidates.push_back([&]() -> const WhereStatement & { return ssc->pre_where(); });
        if (ssc->has_where()) candidates.push_back([&]() -> const WhereStatement & { return ssc->where(); });
...
void StatementGenerator::generateTopSelect(RandomGenerator &rg, const bool force_global_agg, const uint32_t allowed_clauses,
                                           TopSelect *ts) {
  const uint32_t ncols = std::max(std::min(this->fc.max_width - this->width, rg.randomInt<uint32_t>(1, 5)), UINT32_C(1));
  ...
  if (fc.truncate_output || rg.nextSmallNumber() < 3) {
    ...
    if (fc.truncate_output) {
      ...
      if (rg.nextSmallNumber() < 10) {
        std::uniform_int_distribution<uint32_t> step_range(1,
                                                           static_cast<uint32_t>(SelectIntoFile_SelectIntoFileStep_SelectIntoFileStep_MAX));
