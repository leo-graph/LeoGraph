...
void collectColumnPaths(const String cname, SQLType *tp, const uint32_t flags, ColumnPathChain &next, std::vector<ColumnPathChain> &paths) {
  bool is_lcard = false;
  ...
  if (tp && (flags & collect_generated) != 0 && (tp->getTypeClass() == SQLTypeClass::ARRAY || tp->getTypeClass() == SQLTypeClass::MAP)) {
    ...
    for (const auto &entry : ttp->subtypes) {
      collectColumnPaths(entry.cname.has_value() ? ("c" + std::to_string(entry.cname.value())) : std::to_string(i), entry.subtype, flags,
                         next, paths);
      i++;
    }
...
void StatementGenerator::flatTableColumnPath(const uint32_t flags, const std::unordered_map<uint32_t, SQLColumn> &cols,
                                             std::function<bool(const SQLColumn &c)> col_filter) {
  auto &res =
      ((flags & to_table_entries) != 0) ? this->table_entries : (((flags & to_remote_entries) != 0) ? this->remote_entries : this->entries);
...
SQLRelation StatementGenerator::createTableRelation(RandomGenerator &rg, const bool allow_internal_cols, const String &rel_name,
                                                    const SQLTable &t) {
  SQLRelation rel(rel_name);

  flatTableColumnPath(flat_tuple | flat_nested | flat_json | to_table_entries | collect_generated, t.cols,
  ...
  if (allow_internal_cols && (rel.cols.empty() || (rg.nextSmallNumber() < (this->inside_projection ? 6 : 3)))) {
    ...
    } else if (t.isAnyS3Engine() || t.isAnyAzureEngine() || t.isAnyDeltaLakeEngine() || t.isAnyIcebergEngine() || t.isFileEngine() ||
               t.isURLEngine()) {
...
void StatementGenerator::addDictionaryRelation(const String &rel_name, const SQLDictionary &d) {
  ...
  flatTableColumnPath(flat_tuple | flat_nested | flat_json | to_table_entries | collect_generated, d.cols,
                      [](const SQLColumn &) { return true; });
...
void StatementGenerator::generateTableExpression(RandomGenerator &rg, std::optional<SQLRelation> &rel, const bool use_global_agg,
                                                 const bool pred, Expr *expr) {
...
void StatementGenerator::generateNextTTL(RandomGenerator &rg, const std::optional<SQLTable> &t, const TableEngine *te, TTLExpr *ttl_expr) {
  ...
  for (uint32_t i = 0; i < nttls; i++) {
    ...
    } else if (nopt < 41 && te && !entries.empty() &&
...
static const std::vector<SQLFunc> multicolHash = {
    SQLFunc::FUNChalfMD5,        SQLFunc::FUNCsipHash64,         SQLFunc::FUNCsipHash128,      SQLFunc::FUNCsipHash128Reference,
    SQLFunc::FUNCcityHash64,     SQLFunc::FUNCfarmFingerprint64, SQLFunc::FUNCfarmHash64,      SQLFunc::FUNCmetroHash64,
    SQLFunc::FUNCmurmurHash2_32, SQLFunc::FUNCmurmurHash2_64,    SQLFunc::FUNCgccMurmurHash,   SQLFunc::FUNCkafkaMurmurHash,
    SQLFunc::FUNCmurmurHash3_32, SQLFunc::FUNCmurmurHash3_64,    SQLFunc::FUNCmurmurHash3_128, SQLFunc::FUNCxxh3,
    SQLFunc::FUNCxxh3_128};

static const std::vector<SQLFunc> datesHash = {SQLFunc::FUNCtoYear,
...
static const std::vector<SQLFunc> arithmeticFuncs = {
    SQLFunc::FUNCplus,         SQLFunc::FUNCminus,          SQLFunc::FUNCmultiply,      SQLFunc::FUNCdivide,
    SQLFunc::FUNCintDiv,       SQLFunc::FUNCintDivOrZero,   SQLFunc::FUNCifNotFinite,   SQLFunc::FUNCmodulo,
    SQLFunc::FUNCmoduloOrZero, SQLFunc::FUNCpositiveModulo, SQLFunc::FUNCgcd,           SQLFunc::FUNClcm,
    SQLFunc::FUNCmax2,         SQLFunc::FUNCmin2,           SQLFunc::FUNCicebergBucket, SQLFunc::FUNCicebergTruncate};
...
void StatementGenerator::colRefOrExpression(RandomGenerator &rg, const SQLRelation &rel, const SQLBase &b, const ColumnPathChain &entry,
                                            Expr *expr) {
  SQLType *tp = entry.getBottomType();
  const uint32_t datetime_func =
      15 * static_cast<uint32_t>(hasType<DateType>(false, true, false, tp) || hasType<TimeType>(false, true, false, tp) ||
  ...
  if (datetime_func && nopt < (datetime_func + 1)) {
    ...
    func_call->mutable_func()->set_catalog_func((b.isAnyIcebergEngine() && rg.nextBool())
                                                    ? SQLFunc::FUNCidentity
  ...
  } else if (arithmetic_func &&
             nopt < (datetime_func + modulo_func + one_arg_func + hash_func + rand_expr + rand_func + arithmetic_func + 1)) {
    ...
    entryOrConstant(rg, entry, func_call->add_args()->mutable_expr());
             ...
             nopt < (datetime_func + modulo_func + one_arg_func + hash_func + rand_expr + rand_func + arithmetic_func + col_ref + 1)) {
    /// Reference a column
    columnPathRef(entry, expr);
  } else {
    UNREACHABLE();
...
void StatementGenerator::generateTableKey(RandomGenerator &rg, const SQLRelation &rel, const SQLBase &b, const bool allow_asc_desc,
                                          TableKey *tkey) {
...
void StatementGenerator::generateMergeTreeEngineDetails(RandomGenerator &rg, const SQLRelation &rel, SQLBase &b, const bool add_pkey,
                                                        TableEngine *te) {
      ...
      if ((itp = dynamic_cast<IntType *>(tp)) && itp->is_unsigned) {
        ...
        for (int j = 0; j < npkey; j++) {
          if (tpk.exprs(j).expr().has_comp_expr() && tpk.exprs(j).expr().comp_expr().has_expr_stc() &&
              (tpk.exprs(j).expr().comp_expr().expr_stc().col().path().sub_cols_size() + 1) == static_cast<int>(entry.path.size())) {
            bool ok = true;
  ...
  if (te->has_engine() && b.random_engine) {
    ...
    if (te->has_engine() && (b.teng == SummingMergeTree || b.teng == CoalescingMergeTree) && rg.nextSmallNumber() < 4 && !entries.empty()) {
...
void StatementGenerator::setRandomShardKey(RandomGenerator &rg, const std::optional<SQLTable> &t, Expr *expr) {
  if (this->allow_not_deterministic && rg.nextMediumNumber() < 26) {
    ...
    flatTableColumnPath(to_remote_entries | flat_tuple | flat_nested | flat_json | collect_generated, tt.cols,
                        [](const SQLColumn &) { return true; });
...
String StatementGenerator::getTableStructure(RandomGenerator &rg, const SQLTable &t, const bool escape) {
  ...
  for (const auto &entry : this->remote_entries) {
    buf += fmt::format("{}{} {}{}", first ? "" : ", ", entry.getBottomName(), entry.getBottomType()->typeName(escape, false),
...
void StatementGenerator::generateEngineDetails(RandomGenerator &rg, const SQLRelation &rel, SQLBase &b, const bool add_pkey,
                                               TableEngine *te) {
  ...
  } else if (te->has_engine() && (b.isMySQLEngine() || b.isPostgreSQLEngine() || b.isMaterializedPostgreSQLEngine() || b.isSQLiteEngine() ||
                                  b.isMongoDBEngine() || b.isRedisEngine() || b.isKafkaEngine() || b.isURLEngine())) {
  ...
  if (te->has_engine()) {
    ...
    if ((b.isMergeTreeFamily() || b.isLogFamily() || smt_disk) && (smt_disk || rg.nextSmallNumber() < 3) &&
        ...
        (!svs || (svs->set_value().property() != "storage_policy" && svs->set_value().property() != "disk" &&
                  (!svs->other_values_size() || std::find_if(svs->other_values().begin(), svs->other_values().end(), [](const auto &val) {
                                                  return val.property() == "storage_policy" || val.property() == "disk";
...
void StatementGenerator::addTableColumnInternal(RandomGenerator &rg, SQLTable &t, const uint32_t cname, const bool modify, const bool is_pk,
                                                const ColumnSpecial special, SQLColumn &col, ColumnDef *cd) {
  SQLType *tp = nullptr;
  ...
  if (special == ColumnSpecial::SIGN || special == ColumnSpecial::IS_DELETED) {
    ...
    cd->mutable_type()->mutable_type()->mutable_non_nullable()->set_integers(special == ColumnSpecial::IS_DELETED ? Integers::UInt8
                                                                                                                  : Integers::Int8);
    ...
    if (((this->next_type_mask & (allow_dates | allow_datetimes)) == 0) || rg.nextBool()) {
      ...
      tp = randomDateTimeType(rg, this->next_type_mask, cd->mutable_type()->mutable_type()->mutable_non_nullable()->mutable_datetimes());
    }
    ...
    static const std::vector<SQLFunc> ttlfuncs = {SQLFunc::FUNCnow, SQLFunc::FUNCnowInBlock, SQLFunc::FUNCnow64, SQLFunc::FUNCnowInBlock64};
    const SQLFunc tfunc = rg.pickRandomly(ttlfuncs);

    tp = randomDateTimeType(rg, std::numeric_limits<uint64_t>::max(),
                            cd->mutable_type()->mutable_type()->mutable_non_nullable()->mutable_datetimes());
...
void StatementGenerator::addTableColumn(RandomGenerator &rg, SQLTable &t, const uint32_t cname, const bool staged, const bool modify,
                                        const bool is_pk, const ColumnSpecial special, ColumnDef *cd) {
  SQLColumn col;
  ...
  if ((t.isMySQLEngine() && (t.is_deterministic || rg.nextSmallNumber() < 8)) || t.hasMySQLPeer()) {
    this->next_type_mask &= ~(allow_int128 | allow_dynamic | allow_JSON | allow_array | allow_map | allow_tuple | allow_variant |
                              allow_nested | allow_geo | set_no_decimal_limit | allow_qbit | allow_aggregate | allow_simple_aggregate);
  }
  if ((t.isPostgreSQLEngine() && (t.is_deterministic || rg.nextSmallNumber() < 8)) || t.hasPostgreSQLPeer()) {
    this->next_type_mask &= ~(allow_int128 | allow_unsigned_int | allow_dynamic | allow_JSON | allow_map | allow_tuple | allow_variant |
  ...
  if ((t.isSQLiteEngine() && (t.is_deterministic || rg.nextSmallNumber() < 8)) || t.hasSQLitePeer()) {
    this->next_type_mask &= ~(allow_int128 | allow_unsigned_int | allow_dynamic | allow_JSON | allow_array | allow_map | allow_tuple |
                              allow_variant | allow_nested | allow_geo | allow_qbit | allow_aggregate | allow_simple_aggregate);
    if (t.hasSQLitePeer()) {
...
void StatementGenerator::addTableIndex(RandomGenerator &rg, SQLTable &t, const bool staged, const bool projection, IndexDef *idef) {
  ...
  auto generate_idx = [&]() -> IndexType {
    ...
    if (raw_idx == 3) raw_idx = static_cast<uint32_t>(IndexType::IDX_minmax);
  ...
  if (rg.nextBool()) {
    ...
    if ((need_string || need_array) && rg.nextBool()) {
      flatTableColumnPath(flat_tuple | flat_nested | flat_json | skip_nested_node, t.cols, [&](const SQLColumn &c) {
        SQLType *tp = c.tp;
...
void StatementGenerator::addTableProjection(RandomGenerator &rg, SQLTable &t, const bool staged, ProjectionDef *pdef) {
  ...
  if (rg.nextBool()) {
    ...
    generateSelect(rg, true, false, ncols, allow_groupby | allow_orderby | allow_global_aggregate, std::nullopt, psdef->mutable_select());
    this->allow_subqueries = prev_allow_subqueries;
    this->levels.clear();
...
void StatementGenerator::generateNextCreateTable(RandomGenerator &rg, const bool in_parallel, CreateTable *ct) {
  ...
  const auto replaceTableLambda = [&next](const SQLTable &t) {
    return t.isAttached() && !t.hasDatabasePeer() && (t.is_deterministic || !next.is_deterministic);
  };
  ...
  ct->set_create_opt(replace ? (rg.nextBool() ? CreateReplaceOption::CreateOrReplace : CreateReplaceOption::Replace)
                             : CreateReplaceOption::Create);
  ...
  if (!collectionHas<SQLTable>(tableLikeLambda) || rg.nextSmallNumber() < 9) {
    ...
    const uint32_t to_addidxs = rg.randomInt<uint32_t>(1, 3) * static_cast<uint32_t>(next.isMergeTreeFamily() && rg.nextSmallNumber() < 8);
    const uint32_t to_addprojs = rg.randomInt<uint32_t>(1, 2) * static_cast<uint32_t>(next.isMergeTreeFamily() && rg.nextSmallNumber() < 8);
    ...
    for (uint32_t i = 0; i < total_to_add; i++) {
      ...
      const uint32_t add_is_deleted = 2 * static_cast<uint32_t>(added_is_deleted < to_add_is_deleted && added_version == to_add_version);
      const uint32_t prob_space = add_idx + add_proj + add_const + add_col + add_ttl + add_id + add_sign + add_version + add_is_deleted;
      std::uniform_int_distribution<uint32_t> next_dist(1, prob_space);
      const uint32_t nopt = next_dist(rg.generator);
      ...
      if (add_idx && nopt < (add_idx + 1)) {
        ...
        const auto sp = add_ttl_col
                            ...
                            : (add_id_col ? ColumnSpecial::ID_COL
                                          : (add_sign_col ? ColumnSpecial::SIGN
                                                          : (add_version_col ? ColumnSpecial::VERSION : ColumnSpecial::IS_DELETED)));
    ...
    if (rg.nextSmallNumber() < 4) {
      ...
      this->levels[this->current_level] = QueryLevel(this->current_level);
      generateSelect(rg, true, false, static_cast<uint32_t>(std::max<size_t>(1, next.numberOfInsertableColumns(rg.nextSmallNumber() < 4))),
                     std::numeric_limits<uint32_t>::max(), std::nullopt, cts->mutable_select());
      this->levels.clear();
    }
  } else {
    /// Create table as
...
void StatementGenerator::generateNextCreateDictionary(RandomGenerator &rg, CreateDictionary *cd) {
  ...
  this->allow_not_deterministic = !next.is_deterministic;
  this->enforce_final = next.is_deterministic;
  const auto replaceDictionaryLambda = [&next](const SQLDictionary &d) {
    return d.isAttached() && (d.is_deterministic || !next.is_deterministic);
  };
  ...
  cd->set_create_opt(replace ? (rg.nextBool() ? CreateReplaceOption::CreateOrReplace : CreateReplaceOption::Replace)
                             : CreateReplaceOption::Create);
  next.setName(cd->mutable_est(), false);

  const auto &dictionary_table_lambda = [&next](const SQLTable &t) {
    return t.isAttached() && (t.is_deterministic || !next.is_deterministic);
  };
  const auto &dictionary_view_lambda = [&next](const SQLView &v) {
    return v.isAttached() && (v.is_deterministic || !next.is_deterministic);
  };
  const auto &dictionary_dictionary_lambda = [&next](const SQLDictionary &v) {
    return v.isAttached() && (v.is_deterministic || !next.is_deterministic);
  };
  ...
  for (uint32_t i = 0; i < dictionary_ncols; i++) {
    ...
    this->next_type_mask = fc.type_mask & ~(allow_JSON | allow_variant | allow_dynamic | allow_tuple | allow_low_cardinality | allow_map |
                                            allow_enum | allow_geo | allow_fixed_strings | allow_time);
    ...
    if (!has_hierarchical && rg.nextSmallNumber() < 8) {
      ...
      if (eff_tp && eff_tp->getTypeClass() == SQLTypeClass::NULLABLE) eff_tp = static_cast<const Nullable *>(eff_tp)->subtype;
...
void StatementGenerator::generateDatabaseEngineDetails(RandomGenerator &rg, SQLDatabase &d, DatabaseEngine *de) {
  if (d.isReplicatedDatabase()) {
    ...
    for (const auto &[k, b] : backups)
      if (!b.databases.empty()) candidates.emplace_back(std::cref(b));
