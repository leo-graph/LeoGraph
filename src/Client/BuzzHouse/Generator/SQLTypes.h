...
class SQLType {
 public:
  ...
  virtual String insertNumberEntry(RandomGenerator &rg, StatementGenerator &gen, uint32_t max_strlen, uint32_t max_nested_rows) const = 0;
...
class TimeType : public SQLType {
 public:
  ...
  TimeType(const bool ex, const std::optional<const uint32_t> p) : extended(ex), precision(p) {}
...
class DecimalType : public SQLType {
 public:
  ...
  DecimalType(const std::optional<DecimalN_DecimalPrecision> sn, const std::optional<const uint32_t> p,
              const std::optional<const uint32_t> s)
...
class TupleType : public SQLType {
 public:
  ...
  explicit TupleType(const bool n, const std::vector<SubType> s) : nullable(n), subtypes(s) {}
...
class QBitType : public SQLType {
 public:
  ...
  QBitType(SQLType *s, const uint32_t d) : subtype(s), dimension(d) {}
...
class AggregateFunctionType : public SQLType {
 public:
  ...
  AggregateFunctionType(const bool s, const SQLFunc aggr, const std::vector<SQLType *> subs) : simple(s), aggregate(aggr), subtypes(subs) {}
