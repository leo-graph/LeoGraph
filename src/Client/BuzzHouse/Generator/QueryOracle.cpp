...
static void finishSettings(SettingValues *svs) {
  ...
  static const std::unordered_map<String, String> toSet = {{"alter_sync", "2"},
...
void QueryOracle::generateCorrectnessTestFirstQuery(RandomGenerator &rg, StatementGenerator &gen, SQLQuery &sq1) {
  ...
  if (combination != 1) {
    ...
    bexpr->mutable_rhs()->mutable_lit_val()->mutable_special_val()->set_val(SpecialVal_SpecialValEnum::SpecialVal_SpecialValEnum_VAL_TRUE);
...
void QueryOracle::generateRoundtripOracleQueries(RandomGenerator &rg, StatementGenerator &gen, const SQLTable &t, SQLQuery &sq1,
                                                 SQLQuery &sq2) {
  ...
  switch (rg.randomInt<uint32_t>(0, 3)) {
    ...
    default: {
      ...
      static const std::vector<CipherSpec> ciphers = {
          {"aes-128-ecb", 16, 0},     {"aes-192-ecb", 24, 0},  {"aes-256-ecb", 32, 0},     {"aes-128-cbc", 16, 16},
          {"aes-192-cbc", 24, 16},    {"aes-256-cbc", 32, 16}, {"aes-128-cfb128", 16, 16}, {"aes-192-cfb128", 24, 16},
          {"aes-256-cfb128", 32, 16}, {"aes-128-ofb", 16, 16}, {"aes-192-ofb", 24, 16},    {"aes-256-ofb", 32, 16},
          {"aes-128-gcm", 16, 12},    {"aes-192-gcm", 24, 12}, {"aes-256-gcm", 32, 12},
      };
      const CipherSpec &spec = rg.pickRandomly(ciphers);

      auto gen_hex = [&](uint32_t bytes) -> String {
        String hex;
        for (uint32_t i = 0; i < bytes; i++) hex += fmt::format("{:02x}", rg.randomInt<uint8_t>(0, 255));
        return hex;
      };
      const String key_hex = gen_hex(spec.key_bytes);

      if (spec.iv_bytes == 0) {
        roundtrip_pred = fmt::format("decrypt('{2}', encrypt('{2}', {0}, unhex('{1}')), unhex('{1}')) = {0}", val, key_hex, spec.name);
      } else {
        const String iv_hex = gen_hex(spec.iv_bytes);
        roundtrip_pred = fmt::format("decrypt('{3}', encrypt('{3}', {0}, unhex('{1}'), unhex('{2}')), unhex('{1}'), unhex('{2}')) = {0}",
                                     val, key_hex, iv_hex, spec.name);
      }
      break;
    }
  }
...
void QueryOracle::insertOnTableOrCluster(RandomGenerator &rg, StatementGenerator &gen, const SQLTable &t, const bool peer,
                                         TableOrFunction *tof) const {
  ...
  if (peer || cluster.has_value() || replaceable || rg.nextMediumNumber() < 16) {
    const TableFunctionUsage usage =
        peer ? TableFunctionUsage::PeerTable
...
void QueryOracle::dumpTableContent(RandomGenerator &rg, StatementGenerator &gen, const DumpOracleStrategy strategy, const bool test_content,
                                   const SQLTable &t, SQLQuery &sq1, SQLQuery &sq2) {
  TopSelect *ts = sq1.mutable_single_query()->mutable_explain()->mutable_inner_query()->mutable_select();
  SelectIntoFile *sif = ts->mutable_intofile();
  Select *sel = ts->mutable_sel();
  SelectStatementCore *ssc = sel->mutable_select_core();
  ...
  switch (strategy) {
    ...
    case DumpOracleStrategy::INSERT_COUNT: {
      ...
      scc.add_result_columns()->mutable_eca()->mutable_expr()->mutable_comp_expr()->mutable_func_call()->mutable_func()->set_catalog_func(
          FUNCcount);
    } break;
  }
}

void QueryOracle::generateExportQuery(RandomGenerator &rg, StatementGenerator &gen, const bool test_content, const SQLTable &t,
                                      SQLQuery &sq2) {
  ...
  if (!can_test_oracle_result && rg.nextSmallNumber() < 3) {
    ...
    for (const auto &entry : gen.entries) {
      buf += fmt::format("{}{} {}{}{}{}", first ? "" : ", ", entry.columnPathRef(), entry.path.size() > 1 ? "Array(" : "",
...
void QueryOracle::dumpOracleIntermediateSteps(RandomGenerator &rg, StatementGenerator &gen, SQLTable &t, const DumpOracleStrategy strategy,
                                              const bool test_content, std::vector<SQLQuery> &intermediate_queries) {
  SQLQuery next;
  const std::optional<String> &cluster = t.getCluster();
...
void QueryOracle::generateImportQuery(RandomGenerator &rg, StatementGenerator &gen, const SQLTable &t, const SQLQuery &sq2,
                                      SQLQuery &sq4) const {
...
void QueryOracle::generateSecondSetting(RandomGenerator &rg, StatementGenerator &gen, const bool use_settings, const SQLQuery &sq1,
                                        SQLQuery &sq3) {
...
void QueryOracle::generateOracleSelectQuery(RandomGenerator &rg, const PeerQuery pq, StatementGenerator &gen, SQLQuery &sq2) {
  ...
  if (compare_explain) {
    ...
    fcall->add_args()->mutable_expr()->mutable_comp_expr()->mutable_expr_stc()->mutable_col()->mutable_path()->mutable_col()->set_column(
        "explain");
    ...
    fcall2->add_args()->mutable_expr()->mutable_comp_expr()->mutable_expr_stc()->mutable_col()->mutable_path()->mutable_col()->set_column(
        "explain");
...
void QueryOracle::maybeUpdateOracleSelectQuery(RandomGenerator &rg, StatementGenerator &gen, const SQLQuery &sq1, SQLQuery &sq2) {
  ...
  if (rg.nextBool()) {
    ...
    rules.push_back(MatchHandler{
        .predicate = [](const google::protobuf::Message &m) { return m.GetDescriptor()->full_name() == "BuzzHouse.TableOrFunction"; },
        .handler =
...
void QueryOracle::replaceQueryWithTablePeers(RandomGenerator &rg, const SQLQuery &sq1, StatementGenerator &gen,
                                             std::vector<SQLQuery> &peer_queries, SQLQuery &sq2) {
  std::vector<MatchHandler> rules;
  found_tables.clear();
  peer_queries.clear();
...
void QueryOracle::setIntermediateStepSuccess(const bool success) { other_steps_success &= success; }
...
void QueryOracle::processSecondOracleQueryResult(const int errcode, ExternalIntegrations &ei, const String &oracle_name) {
  if (other_steps_success && can_test_oracle_result) {
    if (can_test_success && ((first_errcode && !errcode) || (!first_errcode && errcode)) &&
      ...
      throw DB::Exception(DB::ErrorCodes::BUZZHOUSE, "{}: failed with different success results: {} vs {}", oracle_name,
                          DB::ErrorCodes::getName(first_errcode), DB::ErrorCodes::getName(errcode));
    }
