...
bool ClickHouseIntegratedDatabase::performTableIntegration(RandomGenerator &rg, SQLTable &t, const bool can_shuffle,
  ...
  if (!performQuery(fmt::format("DROP TABLE IF EXISTS {};", str_tname))) {
    ...
    for (const auto &entry : entries) {
      ...
      buf +=
          fmt::format("{}{} {} {}NULL", first ? "" : ", ", entry.getBottomName(), columnTypeAsString(rg, t.is_deterministic, tp),
...
void ClickHouseIntegratedDatabase::swapTableDefinitions(RandomGenerator &rg, CreateTable &newt) {
  ...
  if (teng >= TableEngineValues::MergeTree && teng <= TableEngineValues::GraphiteMergeTree) {
    ...
    static const std::vector<TableEngineValues> logEngines = {TableEngineValues::StripeLog, TableEngineValues::Log,
...
bool ClickHouseIntegratedDatabase::performCreatePeerTable(RandomGenerator &rg, const bool is_clickhouse_integration, SQLTable &t,
                                                          const CreateTable *ct, std::vector<ColumnPathChain> &entries) {
...
std::unique_ptr<MySQLIntegration> MySQLIntegration::testAndAddMySQLConnection(FuzzConfig &fcc, const ServerCredentials &scc,
                                                                              const bool read_log, const String &server) {
  MYSQL *mcon = nullptr;

  if (!(mcon = mysql_init(nullptr))) {
    LOG_ERROR(fcc.log, "Could not initialize MySQL handle");
  } else if (!mysql_real_connect(mcon, scc.client_hostname.empty() ? nullptr : scc.client_hostname.c_str(),
                                 scc.user.empty() ? nullptr : scc.user.c_str(), scc.password.empty() ? nullptr : scc.password.c_str(),
                                 nullptr, scc.mysql_port ? scc.mysql_port : scc.port,
                                 scc.unix_socket.empty() ? nullptr : scc.unix_socket.c_str(), 0)) {
    ...
    if (read_log || (!mysql->performQuery("DROP DATABASE IF EXISTS " + scc.database + ";") &&
...
String MySQLIntegration::truncateStatement() { return fmt::format("TRUNCATE{}", is_clickhouse ? " TABLE" : ""); }
...
String MySQLIntegration::columnTypeAsString(RandomGenerator &rg, const bool is_deterministic, SQLType *tp) const {
  if (!is_deterministic && rg.nextSmallNumber() < 4) {
    ...
    if (nopt < 76) {
      static const DB::Strings baseTypes = {"TINYINT",
                                            "SMALLINT",
                                            "MEDIUMINT",
                                            "INT",
                                            "INTEGER",
...
#else
std::unique_ptr<MySQLIntegration> MySQLIntegration::testAndAddMySQLConnection(FuzzConfig &fcc, const ServerCredentials &, const bool,
                                                                              const String &) {
...
std::unique_ptr<PostgreSQLIntegration> PostgreSQLIntegration::testAndAddPostgreSQLIntegration(FuzzConfig &fcc, const ServerCredentials &scc,
                                                                                              const bool read_log) {
...
String PostgreSQLIntegration::getTableName(std::shared_ptr<SQLDatabase>, const uint32_t tname) { return "test.t" + std::to_string(tname); }

String PostgreSQLIntegration::truncateStatement() { return "TRUNCATE"; }
...
String PostgreSQLIntegration::columnTypeAsString(RandomGenerator &rg, const bool is_deterministic, SQLType *tp) const {
  if (!is_deterministic && rg.nextSmallNumber() < 4) {
    ...
    if (nopt < 81) {
      static const DB::Strings baseTypes = {"SMALLINT",    "INTEGER",   "BIGINT",    "NUMERIC",  "DECIMAL", "REAL",      "DOUBLE PRECISION",
                                            "SMALLSERIAL", "SERIAL",    "BIGSERIAL", "MONEY",    "TEXT",    "BPCHAR",    "BYTEA",
                                            "TIME",        "TIMESTAMP", "DATE",      "BOOLEAN",  "POINT",   "LINE",      "LSEG",
                                            "BOX",         "PATH",      "POLYGON",   "CIRCLE",   "CIDR",    "INET",      "MACADDR",
                                            "MACADDR8",    "UUID",      "XML",       "JSON",     "JSONB",   "int4range", "int8range",
      ...
      baseType = fmt::format("TIME{}({}){}", rg.nextBool() ? "STAMP" : "", lengths(rg.generator), rg.nextBool() ? " WITH TIME ZONE" : "");
    }
...
#else
std::unique_ptr<PostgreSQLIntegration> PostgreSQLIntegration::testAndAddPostgreSQLIntegration(FuzzConfig &fcc, const ServerCredentials &,
                                                                                              const bool) {
...
String SQLiteIntegration::getTableName(std::shared_ptr<SQLDatabase>, const uint32_t tname) { return "t" + std::to_string(tname); }

String SQLiteIntegration::truncateStatement() { return "DELETE FROM"; }
...
bool RedisIntegration::performTableIntegration(RandomGenerator &, SQLTable &, const bool, std::vector<ColumnPathChain> &) { return true; }
...
void MongoDBIntegration::documentAppendBottomType(RandomGenerator &rg, const String &cname, T &output, SQLType *tp) {
  ...
  if ((itp = dynamic_cast<IntType *>(tp))) {
    ...
    if (rg.nextBool()) {
      ...
      bsoncxx::types::b_binary val{bsoncxx::binary_sub_type::k_binary, limit, reinterpret_cast<const std::uint8_t *>(binary_data.data())};
      if constexpr (is_document<T>) {
        output << cname << val;
      } else {
        output << val;
...
void MongoDBIntegration::documentAppendArray(RandomGenerator &rg, const String &cname, bsoncxx::builder::stream::document &document,
                                             ArrayType *at) {
  ...
  for (uint64_t i = 0; i < limit; i++) {
    ...
    if (nopt < 31) {
      ...
      array << bsoncxx::types::b_minkey{};
               ...
               dynamic_cast<DateTimeType *>(tp) || dynamic_cast<DecimalType *>(tp) || dynamic_cast<StringType *>(tp) ||
               dynamic_cast<const BoolType *>(tp) || dynamic_cast<EnumType *>(tp) || dynamic_cast<UUIDType *>(tp) ||
               dynamic_cast<IPv4Type *>(tp) || dynamic_cast<IPv6Type *>(tp) || dynamic_cast<JSONType *>(tp) ||
               dynamic_cast<GeoType *>(tp) || dynamic_cast<TimeType *>(tp)) {
...
void MongoDBIntegration::documentAppendAnyValue(RandomGenerator &rg, const String &cname, bsoncxx::builder::stream::document &document,
                                                SQLType *tp) {
  Nullable *nl;
  ArrayType *at;
  VariantType *vtp;
  ...
  if (nopt < 31) {
    ...
    document << cname << bsoncxx::types::b_minkey{};
             ...
             dynamic_cast<DateTimeType *>(tp) || dynamic_cast<DecimalType *>(tp) || dynamic_cast<StringType *>(tp) ||
             dynamic_cast<const BoolType *>(tp) || dynamic_cast<EnumType *>(tp) || dynamic_cast<UUIDType *>(tp) ||
             dynamic_cast<IPv4Type *>(tp) || dynamic_cast<IPv6Type *>(tp) || dynamic_cast<JSONType *>(tp) || dynamic_cast<GeoType *>(tp) ||
             dynamic_cast<TimeType *>(tp)) {
...
bool MongoDBIntegration::performTableIntegration(RandomGenerator &rg, SQLTable &t, const bool can_shuffle,
                                                 std::vector<ColumnPathChain> &entries) {
  try {
...
bool MinIOIntegration::performTableIntegration(RandomGenerator &, SQLTable &, const bool, std::vector<ColumnPathChain> &) { return true; }
...
bool AzuriteIntegration::performTableIntegration(RandomGenerator &, SQLTable &, const bool, std::vector<ColumnPathChain> &) { return true; }
...
bool HTTPIntegration::performTableIntegration(RandomGenerator &, SQLTable &, const bool, std::vector<ColumnPathChain> &) { return true; }
...
bool DolorIntegration::performDatabaseIntegration(RandomGenerator &rg, SQLDatabase &d) {
  ...
  buf += fmt::format(R"({{"seed":{},"database_name":"{}","storage":"{}","engine":"{}","catalog":"{}"}})", rg.nextInFullRange(),
                     d.getSparkCatalogName(), d.storage == LakeStorage::S3 ? "s3" : (d.storage == LakeStorage::Azure ? "azure" : "local"),
                     d.format == LakeFormat::DeltaLake ? "deltalake" : "iceberg", catalog);
  fc.outf << "--External database " << buf << std::endl;
  return httpPut("/sparkdatabase", buf);
}

bool DolorIntegration::reRunCreateDatabase(const String &body) { return httpPut("/sparkdatabase", body); }
  ...
  switch (d.catalog) {
    ...
    case LakeCatalog::Unity:
      ...
      de->add_params()->set_svalue(
          fmt::format("http://{}:{}{}{}", cat->server_hostname, cat->port, cat->path, d.format == LakeFormat::Iceberg ? "/iceberg" : ""));
...
bool DolorIntegration::performTableIntegration(RandomGenerator &rg, SQLTable &t, const bool, std::vector<ColumnPathChain> &) {
  ...
  /// Common information
  buf += fmt::format(R"({{"seed":{},"database_name":"{}","table_name":"{}","format":"{}","deterministic":{},"columns":[)",
                     rg.nextInFullRange(), t.getDatabaseName(), t.getTableName(false),
                     t.file_format.has_value() ? InOutFormat_Name(t.file_format.value()).substr(6) : "any", t.is_deterministic ? "1" : "0");
  for (const auto &entry : entries) {
    buf += fmt::format(R"({}{{"name":"{}","type":"{}"}})", first ? "" : ",", entry.getBottomName(),
                       entry.getBottomType()->typeName(false, true));
    first = false;
  }
  buf += "]";
  if (t.isAnyIcebergEngine() || t.isAnyDeltaLakeEngine()) {
    buf += fmt::format(R"(,"engine":"{}","catalog_name":"{}","storage":"{}")", t.isAnyDeltaLakeEngine() ? "deltalake" : "iceberg",
                       t.getSparkCatalogName(), t.isOnS3() ? "s3" : (t.isOnAzure() ? "azure" : "local"));
...
bool DolorIntegration::reRunCreateTable(const String &body) { return httpPut("/sparktable", body); }
...
void DolorIntegration::setTableEngineDetails(RandomGenerator &rg, const SQLTable &t, TableEngine *te) {
  if (t.isAnyIcebergEngine() || t.isAnyDeltaLakeEngine()) {
    ...
    if (catalog == LakeCatalog::None) {
      te->add_params()->set_rvalue(t.isOnS3() ? fc.minio_server.value().named_collection
...
bool DolorIntegration::performExternalCommand(const uint64_t seed, const bool async, const String &engine, const String &cname,
                                              const String &tname) {
  return httpPut("/sparkupdate", fmt::format(R"({{"seed":{},"async":{},"engine":"{}","catalog_name":"{}","table_name":"{}"}})", seed,
                                             async ? 1 : 0, engine, cname, tname));
}

ExternalIntegrations::ExternalIntegrations(FuzzConfig &fcc) : fc(fcc) {
...
void ExternalIntegrations::createExternalDatabaseTable(RandomGenerator &rg, SQLTable &t, std::vector<ColumnPathChain> &entries,
                                                       TableEngine *te) {
  ClickHouseIntegration *next = nullptr;
...
bool ExternalIntegrations::performExternalCommand(const uint64_t seed, const bool async, const IntegrationCall ic, const String &engine,
                                                  const String &cname, const String &tname) {
  ClickHouseIntegration *next = nullptr;
...
void ExternalIntegrations::createPeerTable(RandomGenerator &rg, const PeerTableDatabase pt, SQLTable &t, const CreateTable *ct,
                                           std::vector<ColumnPathChain> &entries) {
  requires_external_call_check++;
  next_calls_succeeded.emplace_back(getPeerPtr(pt)->performCreatePeerTable(rg, pt == PeerTableDatabase::ClickHouse, t, ct, entries));
}

bool ExternalIntegrations::truncatePeerTableOnRemote(const SQLTable &t) { return getPeerPtr(t.peer_table)->truncatePeerTableOnRemote(t); }
...
bool ExternalIntegrations::getPerformanceMetricsForLastQuery(const PeerTableDatabase pt, PerformanceResult &res) {
  ...
  if (clickhouse->performQueryOnServerOrRemote(
    ...
    if (std::getline(infile, buf) && buf.size() > 1) {
      ...
      res.metrics.insert({{"query_time", static_cast<uint64_t>(std::stoull(buf))},
...
void ExternalIntegrations::replicateSettings(const PeerTableDatabase pt) {
  ...
  if (fc.processServerQuery(
          false,
          fmt::format("SELECT `name`, `value` FROM system.settings WHERE changed = 1 INTO OUTFILE '{}' TRUNCATE FORMAT TabSeparated;",
