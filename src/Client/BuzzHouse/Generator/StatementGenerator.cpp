...
StatementGenerator::StatementGenerator(RandomGenerator &rg, FuzzConfig &fuzzc, ExternalIntegrations &conn,
                                       const bool supports_cloud_features_)
    : fc(fuzzc),
      ...
      SQLGen(ProbabilityGenerator(static_cast<ProbabilityStrategy>(rg.randomInt<uint32_t>(0, 2)), rg.nextInFullRange(),
                                  {{
                                  ...
                                  "SQL statements")),
      litGen(ProbabilityGenerator(static_cast<ProbabilityStrategy>(rg.randomInt<uint32_t>(0, 2)), rg.nextInFullRange(),
                                  {{
      ...
      expGen(ProbabilityGenerator(static_cast<ProbabilityStrategy>(rg.randomInt<uint32_t>(0, 2)), rg.nextInFullRange(),
                                  {{
                                      {0.01, 0.40},  /// Literal
                                      {0.01, 0.60},  /// ColumnRef
                                      {0.01, 0.25},  /// Predicate
                                  ...
                                  "SQL expressions")),
      predGen(ProbabilityGenerator(static_cast<ProbabilityStrategy>(rg.randomInt<uint32_t>(0, 2)), rg.nextInFullRange(),
                                   {{
                                       {0.03, 0.30},  /// UnaryExpr
                                   ...
                                   "SQL predicates")),
      queryGen(ProbabilityGenerator(static_cast<ProbabilityStrategy>(rg.randomInt<uint32_t>(0, 2)), rg.nextInFullRange(),
                                    {{
...
static void pickWeightedAction(RandomGenerator &rg, const std::vector<std::pair<uint32_t, std::function<void()>>> &options) {
  ...
  for (const auto &[weight, _] : options) total_weight += weight;
...
void StatementGenerator::setClusterClause(RandomGenerator &rg, const std::optional<String> &cluster, Cluster *clu, const bool force) const {
  if ((cluster.has_value() && (!this->allow_not_deterministic || rg.nextSmallNumber() < 9)) ||
      (!fc.clusters.empty() && (force || rg.nextMediumNumber() < 19))) {
    clu->set_cluster((cluster.has_value() && (!this->allow_not_deterministic || fc.clusters.empty() || rg.nextSmallNumber() < 7))
                         ? cluster.value()
...
void StatementGenerator::generateHotTableSettingsValues(RandomGenerator &rg, const bool create, SettingValues *vals) {
  ...
  for (size_t i = 0; i < nsets; i++) {
    ...
    sv->set_value((create && ov.size() == 2 && ov.contains("0") && ov.contains("1"))
                      ? "1"
...
void StatementGenerator::generateSettingValues(RandomGenerator &rg, const std::unordered_map<String, CHSetting> &settings,
                                               const size_t nvalues, SettingValues *vals) {
...
void StatementGenerator::generateNextCreateView(RandomGenerator &rg, CreateView *cv) {
  ...
  cv->set_create_opt(replace ? (rg.nextBool() ? CreateReplaceOption::CreateOrReplace : CreateReplaceOption::Replace)
  ...
  generateSelect(rg, false, false, view_ncols,
                 next.is_materialized && rg.nextBool() ? (~allow_prewhere) : std::numeric_limits<uint32_t>::max(), std::nullopt,
...
void StatementGenerator::generateNextTablePartition(RandomGenerator &rg, const uint32_t allow_parts, const bool detached,
                                                    const bool supports_all, const SQLTable &t, PartitionExpr *pexpr) {
...
void StatementGenerator::generateNextOptimizeTableInternal(RandomGenerator &rg, const SQLTable &t, bool strict, OptimizeTable *ot) {
  ...
  if (fc.truncate_output || rg.nextSmallNumber() < 3) {
    ot->set_format(fc.truncate_output ? OutFormat::OUT_Null
...
void StatementGenerator::generateNextCheckTable(RandomGenerator &rg, CheckTable *ct) {
  ...
  if (fc.truncate_output || rg.nextSmallNumber() < 3) {
    ct->set_format(fc.truncate_output ? OutFormat::OUT_Null
                                      : (static_cast<OutFormat>((rg.nextLargeNumber() % static_cast<uint32_t>(OutFormat_MAX)) + 1)));
  }
}
...
bool StatementGenerator::tableOrFunctionRef(RandomGenerator &rg, const SQLTable &t, const bool allow_remote_cluster, TableOrFunction *tof) {
  bool is_url = false;
  ...
  if (engine_func && (nopt2 < engine_func + 1)) {
    ...
    if (!this->entries.empty()) {
      ...
      for (const auto &entry : this->entries) {
        ...
        buf += fmt::format("{}{} {}{}{}", first ? "" : ", ", entry.getBottomName(), entry.path.size() > 1 ? "Array(" : "",
                           entry.getBottomType()->typeName(false, false), entry.path.size() > 1 ? ")" : "");
        first = false;
      }
      url += ")";
    }
...
void StatementGenerator::generateNextDescTable(RandomGenerator &rg, DescribeStatement *dt) {
  ...
  if (desc_table && nopt < (desc_table + 1)) {
    ...
    if (rg.nextMediumNumber() < 6) {
      ...
      this->levels[this->current_level] = QueryLevel(this->current_level);
      generateSelect(rg, false, false, rg.randomInt<uint32_t>(1, 5), std::numeric_limits<uint32_t>::max(), std::nullopt,
  ...
  if (fc.truncate_output || rg.nextSmallNumber() < 3) {
    dt->set_format(fc.truncate_output ? OutFormat::OUT_Null
                                      : (static_cast<OutFormat>((rg.nextLargeNumber() % static_cast<uint32_t>(OutFormat_MAX)) + 1)));
  }
}

void StatementGenerator::generateInsertToTable(RandomGenerator &rg, const SQLTable &t, const bool in_parallel, std::optional<uint64_t> rows,
                                               Insert *ins) {
  String buf;
  ...
  if (hardcoded_insert && (nopt < hardcoded_insert + 1)) {
    ...
    if (generate_random && nopt < (hardcoded_insert + random_values + generate_random + 1)) {
      ...
      for (const auto &entry : this->entries) {
        ...
        buf += fmt::format("{}{} {}{}{}", first ? "" : ", ", bottomName, entry.path.size() > 1 ? "Array(" : "",
                           entry.getBottomType()->typeName(false, false), entry.path.size() > 1 ? ")" : "");
      ...
      for (const auto &entry : this->entries) {
        ...
        if (entry.special == ColumnSpecial::SIGN || entry.special == ColumnSpecial::IS_DELETED) {
          ...
          nval = entry.getBottomType()->insertNumberEntry(rg, *this, static_cast<uint32_t>(string_length_dist(rg.generator)),
                                                          static_cast<uint32_t>(nested_nrows));
        }
        buf += fmt::format("{}{}{}{}", first ? "" : ", ", entry.path.size() > 1 ? "arrayResize([" : "", nval,
      ...
      generateSelect(rg, true, false, std::max<uint32_t>(1, static_cast<uint32_t>(this->entries.size())),
                     std::numeric_limits<uint32_t>::max(), std::nullopt, sel);
      this->levels.clear();
    } else {
      UNREACHABLE();
    }
...
std::optional<String> StatementGenerator::alterSingleTable(RandomGenerator &rg, SQLTable &t, const uint32_t nalters, const bool no_oracle,
                                                           const bool can_update, const bool in_parallel, Alter *at) {
  const bool prev_enforce_final = this->enforce_final;
  const bool prev_allow_not_deterministic = this->allow_not_deterministic;
  ...
  if (alter_view && nopt2 < (alter_view + 1)) {
    ...
    for (uint32_t i = 0; i < nalters; i++) {
      ...
      if (alter_refresh && nopt < (alter_refresh + 1)) {
        ...
        this->levels[this->current_level] = QueryLevel(this->current_level);
        this->allow_in_expression_alias = rg.nextSmallNumber() < 3;
        generateSelect(rg, false, false, v.staged_ncols,
                       v.is_materialized && rg.nextBool() ? (~allow_prewhere) : std::numeric_limits<uint32_t>::max(), std::nullopt,
                       sparen->mutable_select());
        this->levels.clear();
        this->allow_in_expression_alias = true;
...
static const auto has_mergeable_mt_func = [](const SQLTable &t) {
  return t.isAttached() && t.isMergeTreeFamily() && t.can_run_merges && !t.hasDatabasePeer();
};

static const auto has_non_mergeable_mt_func = [](const SQLTable &t) {
  return t.isAttached() && t.isMergeTreeFamily() && !t.can_run_merges;
};

static const auto has_refreshable_view_func = [](const SQLView &v) { return v.isAttached() && v.is_refreshable; };

static const std::function<bool(const std::shared_ptr<SQLDatabase> &)> db_has_replicas = [](const std::shared_ptr<SQLDatabase> &db) {
  return db->isAttached() && db->replica_counter > 0;
};

static const std::function<bool(const SQLTable &)> table_has_replicas = [](const SQLTable &t) {
  return t.isAttached() && t.replica_counter > 0;
};
...
void StatementGenerator::generateNextSystemStatement(RandomGenerator &rg, const bool allow_table_statements, SystemCommand *sc) {
  ...
  const uint32_t has_non_mergeable_mt = static_cast<uint32_t>(allow_table_statements && collectionHas<SQLTable>(has_non_mergeable_mt_func));
  const uint32_t has_refreshable_view = static_cast<uint32_t>(allow_table_statements && collectionHas<SQLView>(has_refreshable_view_func));
  ...
  std::vector<std::pair<uint32_t, Action>> options = {
      ...
      {15 * has_merge_tree, [&] { cluster = setTableSystemStatement<SQLTable>(rg, has_merge_tree_func, sc->mutable_start_ttl_merges()); }},
      ...
      {8 * has_merge_tree, [&] { cluster = setTableSystemStatement<SQLTable>(rg, has_merge_tree_func, sc->mutable_wait_loading_parts()); }},
      ...
      {8 * has_merge_tree,
       [&] {
         ...
         if (nopt2 < 4)
           ...
           if (t.replica_counter > 0) lsrep->add_replicas("r" + std::to_string(rg.randomInt<uint32_t>(0, t.replica_counter - 1)));
         } else
           srep->set_pull(true);
       }},
      {8 * has_database,
      ...
      {8 * has_table, [&] { cluster = setTableSystemStatement<SQLTable>(rg, attached_tables, sc->mutable_prewarm_primary_index_cache()); }},
      /// Drop caches
      ...
      {3 * has_replicated_database,
       [&] {
         ...
         if (d->shard_counter > 0 && rg.nextBool()) dr->set_shard("s" + std::to_string(rg.randomInt<uint32_t>(0, d->shard_counter - 1)));
         if (rg.nextSmallNumber() < 4) d->setName(dr->mutable_database());
       }},
  };
...
void StatementGenerator::generateNextBackup(RandomGenerator &rg, BackupRestore *br) {
  ...
  if (backup_table && nopt < (backup_table + 1)) {
    ...
    cluster = backupOrRestoreDatabase(bre->mutable_bobject(),
  ...
  if (rg.nextBool()) {
    ...
    br->set_outformat(rg.nextBool() ? rg.pickRandomly(rg.pickRandomly(outFormats))
                                    : static_cast<OutFormat>((rg.nextLargeNumber() % static_cast<uint32_t>(OutFormat_MAX)) + 1));
  }
}
...
void StatementGenerator::generateNextRestore(RandomGenerator &rg, BackupRestore *br) {
  ...
  if (backup.everything) {
    ...
    if (restore_table && (nopt < restore_table + 1)) {
      ...
      d.setName(bro->mutable_object()->mutable_est(), false);
      cluster = backupOrRestoreObject(bro, SQLObject::DICTIONARY, d);
    } else if (restore_database &&
               nopt < (restore_table + restore_system_table + restore_view + restore_dictionary + restore_database + 1)) {
  ...
  if (backup.out_format.has_value()) {
    br->set_informat(outIn.contains(backup.out_format.value()) && rg.nextBool()
...
void StatementGenerator::generateNextQuery(RandomGenerator &rg, const bool in_parallel, SQLQueryInner *sq) {
  ...
  SQLMask[static_cast<size_t>(SQLOp::Drop)] =
      !in_parallel && (collectionCount<SQLTable>(attached_tables) > 3 || collectionCount<SQLView>(attached_views) > 3 ||
  ...
  SQLMask[static_cast<size_t>(SQLOp::Exchange)] =
      this->fc.enable_renames && !in_parallel &&
  ...
  SQLMask[static_cast<size_t>(SQLOp::Attach)] =
      !in_parallel &&
  ...
  SQLMask[static_cast<size_t>(SQLOp::Detach)] =
      !in_parallel &&
      (collectionCount<SQLTable>(attached_tables) > 3 || collectionCount<SQLView>(attached_views) > 3 ||
       collectionCount<SQLDictionary>(attached_dictionaries) > 3 || collectionCount<std::shared_ptr<SQLDatabase>>(attached_databases) > 3);
...
struct ExplainOptValues {
  ...
  ExplainOptValues(const ExplainOption_ExplainOpt &e, const std::function<uint32_t(RandomGenerator &)> &rf) : opt(e), random_func(rf) {}
};
...
static const std::vector<ExplainOptValues> explainSettings{
    ...
    ExplainOptValues(ExplainOption_ExplainOpt::ExplainOption_ExplainOpt_passes,
    ...
    ExplainOptValues(ExplainOption_ExplainOpt::ExplainOption_ExplainOpt_query_tree_passes,
                     [](RandomGenerator &rg) { return rg.randomInt<uint32_t>(0, 32); }),
...
void StatementGenerator::renameObjects(const uint32_t old_tname, const uint32_t new_tname, const std::optional<uint32_t> &new_db) {
  auto &container = getNextCollection<T>();
  if (!container.contains(old_tname)) return;
  if constexpr (!std::is_same_v<T, std::shared_ptr<SQLDatabase>>) {
    if (new_db.has_value() && !this->databases.contains(new_db.value())) return;
  }
  ...
  if (container.contains(tname)) {
    ...
    if constexpr (std::is_same_v<T, std::shared_ptr<SQLDatabase>>) {
      ...
      for (auto &[_, view] : this->views) {
        if (view.db && view.db->dname == tname && (status == DetachStatus::PERM_DETACHED || view.attached != DetachStatus::PERM_DETACHED)) {
...
void StatementGenerator::updateGeneratorFromSingleQuery(const SingleSQLQuery &ssq, ExternalIntegrations &ei, bool success) {
  ...
  if (ssq.has_explain() && query.has_create_table()) {
    ...
    if (isview) {
      ...
      for (int i = 0; i < at.other_alters_size() + 1; i++) {
        ...
        if (ati.has_add_column()) {
          ...
          const Column &cstr = is_nested ? ati.add_column().new_col().col().sub_cols(ati.add_column().new_col().col().sub_cols_size() - 1)
          ...
          t.staged_cols.erase(cname);
        } else if (ati.has_column_remove_property() && success &&
    ...
    const DetachStatus status =
        query.has_attach() ? DetachStatus::ATTACHED : (query.detach().permanently() ? DetachStatus::PERM_DETACHED : DetachStatus::DETACHED);
    ...
    if (br.command() == BackupRestore_BackupCommand_BACKUP) {
      ...
      if (bre.has_all()) {
        ...
        if (!est.has_database() || (est.database().database() != "system" && est.database().database() != "INFORMATION_SCHEMA" &&
