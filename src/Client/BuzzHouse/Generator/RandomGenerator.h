...
class RandomGenerator {
 private:
  ...
  std::uniform_int_distribution<uint32_t> days[12] = {
      std::uniform_int_distribution<uint32_t>(1, 31), std::uniform_int_distribution<uint32_t>(1, 28),
      std::uniform_int_distribution<uint32_t>(1, 31), std::uniform_int_distribution<uint32_t>(1, 30),
      std::uniform_int_distribution<uint32_t>(1, 31), std::uniform_int_distribution<uint32_t>(1, 30),
      std::uniform_int_distribution<uint32_t>(1, 31), std::uniform_int_distribution<uint32_t>(1, 31),
      std::uniform_int_distribution<uint32_t>(1, 30), std::uniform_int_distribution<uint32_t>(1, 31),
      std::uniform_int_distribution<uint32_t>(1, 30), std::uniform_int_distribution<uint32_t>(1, 31)};
  ...
  const DB::Strings common_chinese{"认识你很高兴", "美国", "叫",       "名字", "你们", "日本",
                                   "哪国人",       "爸爸", "兄弟姐妹", "漂亮", "照片", "😉"};

  /// Use bad_utf8 on x' strings!
  const DB::Strings bad_utf8{"00",  /// Null byte
                             "FF",       "C328",     "A0A1",     "E228A1",           "E28228", "F0288CBC", "F09028BC",
                             "F0288C28", "C328A0A1", "E28228FF", "F0288CBCF0288CBC", "C328FF", "AAC328"};

  const DB::Strings jcols{"_", ".",     "1",       "叫",     "c0",   "c1",    "c0.c1",          "😆",           "😉😉", "123",
                          "0", "a.b.c", "a.b.c.d", "select", "from", "where", "key with space", "key-with-dash"};
 ...
 public:
  ...
  const DB::Strings nasty_strings{"a\"a", "b\\tb", "c\\nc", "e e", "", "😉", "\"", "'", "''", "\\t", "\\n", "--", "0", "1", "-1", "{", "}",
                                  ...
                                  "NaN", "Inf", "-Inf", "1e5", "1.0", "0.0",
                                  /// Date/time-looking strings (test type coercions)
                                  "2024-01-01", "00:00:00", "1970-01-01 00:00:00",
