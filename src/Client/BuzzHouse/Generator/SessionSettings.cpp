...
std::unordered_map<String, CHSetting> performanceSettings = {
    ...
    {"count_distinct_implementation", CHSetting(
                                          [](RandomGenerator &rg, FuzzConfig &) {
                                            static const DB::Strings choices = {"'uniq'", "'uniqCombined'", "'uniqCombined64'",
                                                                                "'uniqHLL12'", "'uniqExact'"};
                                            return rg.pickRandomly(choices);
                                          },
                                          {"'uniq'", "'uniqCombined'", "'uniqCombined64'", "'uniqHLL12'", "'uniqExact'"}, false)},
    ...
    {"join_algorithm", CHSetting(
                           [](RandomGenerator &rg, FuzzConfig &) {
                             return settingCombinations(rg, {"auto", "default", "direct", "full_sorting_merge", "grace_hash", "hash",
                                                             "parallel_hash", "partial_merge", "prefer_partial_merge"});
                           },
                           {"'default'", "'grace_hash'", "'direct, hash'", "'hash'", "'parallel_hash'", "'partial_merge'", "'direct'",
                            "'auto'", "'full_sorting_merge'", "'prefer_partial_merge'"},
                           false)},
    ...
    {"query_plan_optimize_join_order_limit",
     CHSetting([](RandomGenerator &rg, FuzzConfig &) { return std::to_string(rg.randomInt<uint32_t>(0, 64)); },
               {"0", "1", "2", "4", "16", "64"}, false)},
    {"query_plan_join_shard_by_pk_ranges", trueOrFalseSetting},
    {"query_plan_join_swap_table", CHSetting(
                                       [](RandomGenerator &rg, FuzzConfig &) {
    ...
    {"query_plan_optimize_join_order_algorithm",
     CHSetting([](RandomGenerator &rg, FuzzConfig &) { return settingCombinations(rg, {"greedy", "dpsize"}); }, {"'greedy'", "'dpsize'"},
               false)},
    ...
    {"read_in_order_two_level_merge_threshold",
     CHSetting([](RandomGenerator &rg, FuzzConfig &) { return std::to_string(rg.randomInt<uint32_t>(0, 100)); }, {"0", "1", "10", "100"},
               false)},
    {"read_in_order_use_buffering", trueOrFalseSetting},
    {"read_in_order_use_virtual_row", trueOrFalseSetting},
    {"remerge_sort_lowered_memory_bytes_ratio",
     CHSetting([](RandomGenerator &rg, FuzzConfig &) { return std::to_string(rg.thresholdGenerator<double>(0.2, 0.2, 0.0, 4.0)); },
               {"0", "0.001", "0.01", "0.1", "0.5", "0.9", "0.99", "0.999", "1", "1.5", "2", "2.5"}, false)},
    {"rewrite_count_distinct_if_with_count_distinct_implementation", trueOrFalseSetting},
    {"rewrite_in_to_join", trueOrFalseSetting},
    {"short_circuit_function_evaluation", CHSetting(
                                              [](RandomGenerator &rg, FuzzConfig &) {
                                                static const DB::Strings choices = {"'enable'", "'force_enable'", "'disable'"};
                                                return rg.pickRandomly(choices);
                                              },
                                              {"'enable'", "'force_enable'", "'disable'"}, false)},
    ...
    {"temporary_files_codec", CHSetting(
                                  [](RandomGenerator &rg, FuzzConfig &) {
...
std::unordered_map<String, CHSetting> serverSettings = {
    ...
    {"aggregate_function_input_format", CHSetting(
                                            [](RandomGenerator &rg, FuzzConfig &) {
    ...
    {"apply_patch_parts_join_cache_buckets",
     CHSetting([](RandomGenerator &rg, FuzzConfig &) { return std::to_string(rg.thresholdGenerator<uint64_t>(0.2, 0.2, 1, 64)); },
               {"0", "1", "2", "4", "16", "64"}, false)},
    ...
    {"cast_string_to_date_time_mode", CHSetting(
                                          [](RandomGenerator &rg, FuzzConfig &) {
                                            static const DB::Strings choices = {"'best_effort'", "'best_effort_us'", "'basic'"};
                                            return rg.pickRandomly(choices);
                                          },
                                          {"'best_effort'", "'best_effort_us'", "'basic'"}, false)},
    ...
    {"cluster_table_function_split_granularity", CHSetting(
                                                     [](RandomGenerator &rg, FuzzConfig &) {
    ...
    {"correlated_subqueries_default_join_kind", CHSetting(
                                                    [](RandomGenerator &rg, FuzzConfig &) {
    ...
    {"date_time_output_format", CHSetting(
                                    [](RandomGenerator &rg, FuzzConfig &) {
                                      static const DB::Strings choices = {"'simple', date_time_input_format = 'basic'",
                                                                          "'iso', date_time_input_format = 'best_effort'"};
                                      return rg.pickRandomly(choices);
                                    },
                                    {}, false)},
    {"date_time_overflow_behavior", CHSetting(
                                        [](RandomGenerator &rg, FuzzConfig &) {
    ...
    /// ClickHouse cloud setting
    {"distributed_cache_log_mode", CHSetting(
                                       [](RandomGenerator &rg, FuzzConfig &) {
    ...
    {"filesystem_cache_segments_batch_size", CHSetting(
                                                 [](RandomGenerator &rg, FuzzConfig &) {
    ...
    {"grace_hash_join_initial_buckets",
     CHSetting([](RandomGenerator &rg, FuzzConfig &) { return std::to_string(rg.thresholdGenerator<uint64_t>(0.2, 0.2, 1, 1024)); }, {},
               false)},
    ...
    {"iceberg_metadata_log_level", CHSetting(
                                       [](RandomGenerator &rg, FuzzConfig &) {
                                         static const DB::Strings choices = {"'none'",
                                                                             "'metadata'",
    ...
    {"iceberg_timestamp_ms", CHSetting(
    ...
    {"ignore_cold_parts_seconds",
     CHSetting([](RandomGenerator &rg, FuzzConfig &) { return std::to_string(rg.thresholdGenerator<uint64_t>(0.2, 0.2, 0, 60)); }, {},
               false)},
    ...
    {"insert_select_deduplicate", CHSetting(
                                      [](RandomGenerator &rg, FuzzConfig &) {
                                        static const DB::Strings choices = {"0", "1", "'auto'"};
                                        return rg.pickRandomly(choices);
                                      },
                                      {}, false)},
    {"deduplicate_insert", CHSetting(
                               [](RandomGenerator &rg, FuzzConfig &) {
                                 static const DB::Strings choices = {"'disable'", "'enable'", "'backward_compatible_choice'"};
                                 return rg.pickRandomly(choices);
                               },
                               {}, false)},
    {"deduplicate_insert_select", CHSetting(
                                      [](RandomGenerator &rg, FuzzConfig &) {
                                        static const DB::Strings choices = {"'enable_when_possible'", "'force_enable'", "'disable'",
    ...
    {"insert_shard_id",
     CHSetting([](RandomGenerator &rg, FuzzConfig &) { return std::to_string(rg.thresholdGenerator<uint64_t>(0.2, 0.2, 1, 2)); }, {},
               false)},
    ...
    {"join_default_strictness", CHSetting(
                                    [](RandomGenerator &rg, FuzzConfig &) {
    ...
    {"join_on_disk_max_files_to_merge",
     CHSetting([](RandomGenerator &rg, FuzzConfig &) { return std::to_string(rg.thresholdGenerator<uint64_t>(0.2, 0.2, 2, 32)); }, {},
               false)},
    {"join_runtime_bloom_filter_hash_functions",
     CHSetting([](RandomGenerator &rg, FuzzConfig &) { return std::to_string(rg.thresholdGenerator<uint32_t>(0.2, 0.2, 0, 10)); },
               {"0", "1", "2", "3", "4", "10"}, false)},
    ...
    {"lightweight_delete_mode", CHSetting(
                                    [](RandomGenerator &rg, FuzzConfig &) {
                                      static const DB::Strings choices = {"'alter_update'", "'lightweight_update'",
    ...
    {"load_balancing",
     CHSetting(
         [](RandomGenerator &rg, FuzzConfig &) {
           static const DB::Strings choices = {"'round_robin'",      "'in_order'",        "'hostname_levenshtein_distance'",
                                               "'nearest_hostname'", "'first_or_random'", "'random'"};
           return rg.pickRandomly(choices);
         },
         {"'round_robin'", "'in_order'", "'hostname_levenshtein_distance'", "'nearest_hostname'", "'first_or_random'", "'random'"}, false)},
    {"load_balancing_first_offset",
     CHSetting([](RandomGenerator &rg, FuzzConfig &) { return std::to_string(rg.thresholdGenerator<uint64_t>(0.2, 0.2, 0, 8)); }, {},
               false)},
    {"load_marks_asynchronously", trueOrFalseSettingNoOracle},
    {"local_filesystem_read_method", CHSetting(
                                         [](RandomGenerator &rg, FuzzConfig &) {
                                           static const DB::Strings choices = {"'read'", "'pread'", "'mmap'", "'pread_threadpool'",
                                                                               "'io_uring'"};
                                           return rg.pickRandomly(choices);
                                         },
                                         {"'read'", "'pread'", "'mmap'", "'pread_threadpool'", "'io_uring'"}, false)}};
...
static std::unordered_map<String, CHSetting> serverSettings2 = {
    ...
    {"max_parallel_replicas",
     CHSetting([](RandomGenerator &rg, FuzzConfig &) { return std::to_string(rg.thresholdGenerator<uint64_t>(0.3, 0.2, 0, 5)); }, {},
               false)},
    ...
    {"mysql_datatypes_support_level",
     CHSetting(
         [](RandomGenerator &rg, FuzzConfig &) { return settingCombinations(rg, {"decimal", "datetime64", "date2Date32", "date2String"}); },
         {}, false)},
    ...
    {"optimize_min_equality_disjunction_chain_length",
     CHSetting([](RandomGenerator &rg, FuzzConfig &) { return std::to_string(rg.randomInt<uint32_t>(0, 10)); }, {"0", "1", "5", "10"},
               false)},
    ...
    {"optimize_skip_unused_shards_limit",
     CHSetting([](RandomGenerator &rg, FuzzConfig &) { return std::to_string(rg.randomInt<uint32_t>(0, 30)); }, {"0", "1", "5", "10"},
               false)},
    ...
    {"output_format_arrow_compression_method", CHSetting(
                                                   [](RandomGenerator &rg, FuzzConfig &) {
    ...
    {"output_format_avro_codec", CHSetting(
                                     [](RandomGenerator &rg, FuzzConfig &) {
                                       static const DB::Strings choices = {"'null'", "'deflate'", "'snappy'", "'zstd'"};
                                       return rg.pickRandomly(choices);
                                     },
                                     {}, false)},
    {"output_format_avro_sync_interval", CHSetting(
                                             [](RandomGenerator &rg, FuzzConfig &) {
                                               return std::to_string(rg.thresholdGenerator<uint64_t>(
                                                   0.2, 0.2, 32, UINT32_C(10) * UINT32_C(1024) * UINT32_C(1024)));
                                             },
                                             {"32", "1024", "4096", "16384", "'10M'"}, false)},
    ...
    {"output_format_compression_level",
     CHSetting([](RandomGenerator &rg, FuzzConfig &) { return std::to_string(rg.thresholdGenerator<uint64_t>(0.2, 0.2, 1, 22)); }, {},
               false)},
    ...
    {"output_format_orc_compression_method", CHSetting(
                                                 [](RandomGenerator &rg, FuzzConfig &) {
    ...
    {"output_format_parquet_bloom_filter_bits_per_value",
     CHSetting([](RandomGenerator &rg, FuzzConfig &) { return std::to_string(rg.thresholdGenerator<double>(0.2, 0.2, 0.0, 100.0)); }, {},
               false)},
    {"output_format_parquet_bloom_filter_flush_threshold_bytes", CHSetting(
                                                                     [](RandomGenerator &rg, FuzzConfig &) {
                                                                       return std::to_string(rg.thresholdGenerator<uint64_t>(
                                                                           0.2, 0.2, 0, UINT32_C(1024) * UINT32_C(1024) * UINT32_C(1024)));
                                                                     },
                                                                     {}, false)},
    {"output_format_parquet_compliant_nested_types", trueOrFalseSettingNoOracle},
    {"output_format_parquet_compression_method", CHSetting(
                                                     [](RandomGenerator &rg, FuzzConfig &) {
                                                       static const DB::Strings choices = {"'snappy'", "'lz4'",  "'brotli'",
                                                                                           "'zstd'",   "'gzip'", "'none'"};
                                                       return rg.pickRandomly(choices);
                                                     },
                                                     {}, false)},
    ...
    {"output_format_parquet_max_dictionary_size", CHSetting(
                                                      [](RandomGenerator &rg, FuzzConfig &) {
                                                        return std::to_string(rg.thresholdGenerator<uint32_t>(
                                                            0.3, 0.3, 0, UINT32_C(1024) * UINT32_C(1024) * UINT32_C(1024)));
                                                      },
                                                      {}, false)},
    ...
    {"output_format_parquet_version", CHSetting(
                                          [](RandomGenerator &rg, FuzzConfig &) {
    ...
    {"output_format_pretty_color", CHSetting(
                                       [](RandomGenerator &rg, FuzzConfig &) {
                                         static const DB::Strings choices = {"0", "1", "'auto'"};
                                         return rg.pickRandomly(choices);
                                       },
    ...
    {"parallel_replicas_mode", CHSetting(
                                   [](RandomGenerator &rg, FuzzConfig &) {
                                     static const DB::Strings choices = {"'sampling_key'", "'read_tasks'", "'custom_key_range'",
                                                                         "'custom_key_sampling'", "'auto'"};
                                     return rg.pickRandomly(choices);
                                   },
                                   {"'sampling_key'", "'read_tasks'", "'custom_key_range'", "'custom_key_sampling'", "'auto'"}, false)},
    ...
    {"query_plan_max_limit_for_top_k_optimization",
     CHSetting([](RandomGenerator &rg, FuzzConfig &) { return std::to_string(rg.thresholdGenerator<uint64_t>(0.2, 0.2, 0, 1000)); },
               {"0", "1", "2", "4", "16", "64"}, false)},
    ...
    {"send_logs_level", CHSetting(
                            [](RandomGenerator &rg, FuzzConfig &) {
                              static const DB::Strings choices = {"'debug'", "'information'", "'trace'", "'error'",
                                                                  "'test'",  "'warning'",     "'fatal'", "'none'"};
                              return rg.pickRandomly(choices);
                            },
                            {}, false)},
    ...
    {"show_create_query_identifier_quoting_rule", CHSetting(
                                                      [](RandomGenerator &rg, FuzzConfig &) {
                                                        static const DB::Strings choices = {"'when_necessary'", "'always'",
                                                                                            "'user_display'"};
                                                        return rg.pickRandomly(choices);
                                                      },
                                                      {}, false)},
    {"show_create_query_identifier_quoting_style", CHSetting(
                                                       [](RandomGenerator &rg, FuzzConfig &) {
                                                         static const DB::Strings choices = {"'Backticks'", "'DoubleQuotes'",
                                                                                             "'BackticksMySQL'"};
                                                         return rg.pickRandomly(choices);
                                                       },
                                                       {}, false)},
    ...
    {"storage_file_read_method", CHSetting(
                                     [](RandomGenerator &rg, FuzzConfig &) {
    ...
    {"table_function_remote_max_addresses",
     CHSetting([](RandomGenerator &rg, FuzzConfig &) { return std::to_string(rg.thresholdGenerator<uint64_t>(0.3, 0.2, 1, 100)); }, {},
               false)},
    ...
    {"totals_mode", CHSetting(
                        [](RandomGenerator &rg, FuzzConfig &) {
                          static const DB::Strings choices = {"'before_having'", "'after_having_exclusive'", "'after_having_inclusive'",
                                                              "'after_having_auto'"};
                          return rg.pickRandomly(choices);
                        },
                        {}, false)},
    ...
    {"update_parallel_mode", CHSetting(
                                 [](RandomGenerator &rg, FuzzConfig &) {
    ...
    {"zstd_window_log_max",
     CHSetting([](RandomGenerator &rg, FuzzConfig &) { return std::to_string(rg.thresholdGenerator<uint64_t>(0.3, 0.2, 0, 31)); }, {},
               false)}};
...
void loadFuzzerServerSettings(const FuzzConfig &fc) {
  ...
  DB::Strings max_bytes_values = {
      "aggregation_in_order_max_block_bytes", "archive_adaptive_buffer_max_size_bytes", "async_insert_max_data_size",
      "automatic_parallel_replicas_min_bytes_per_replica", "azure_max_single_part_upload_size", "cross_join_min_bytes_to_compress",
      "default_max_bytes_in_join", "delta_lake_insert_max_bytes_in_data_file",
      /// ClickHouse cloud setting
      "distributed_cache_alignment",
      /// ClickHouse cloud setting
      "distributed_cache_min_bytes_for_seek",
      /// ClickHouse cloud setting
      "distributed_cache_read_alignment", "external_storage_max_read_bytes", "filesystem_cache_boundary_alignment",
      "filesystem_cache_max_download_size", "filesystem_prefetch_min_bytes_for_single_read_task", "filesystem_prefetch_step_bytes",
      "group_by_two_level_threshold_bytes", "iceberg_insert_max_bytes_in_data_file", "input_format_max_block_size_bytes",
      "input_format_parquet_local_file_min_bytes_for_seek", "input_format_parquet_prefer_block_bytes",
      "input_format_parquet_memory_low_watermark", "input_format_parquet_memory_high_watermark", "join_runtime_bloom_filter_bytes",
      "max_bytes_before_external_group_by", "max_bytes_before_external_sort", "max_bytes_before_remerge_sort", "max_download_buffer_size",
      "max_insert_block_size_bytes", "max_joined_block_size_bytes", "max_partition_size_to_drop", "max_partitions_per_insert_block",
      "max_read_buffer_size", "max_read_buffer_size_local_fs", "max_read_buffer_size_remote_fs", "max_table_size_to_drop",
      "max_temporary_data_on_disk_size_for_query", "max_temporary_data_on_disk_size_for_user", "max_untracked_memory",
      "max_reverse_dictionary_lookup_cache_size_bytes", "merge_tree_max_bytes_to_use_cache", "merge_tree_min_bytes_for_concurrent_read",
      "merge_tree_min_bytes_for_concurrent_read_for_remote_filesystem", "merge_tree_min_bytes_for_seek",
      "merge_tree_min_bytes_per_task_for_remote_reading", "min_bytes_to_use_direct_io", "min_bytes_to_use_mmap_io",
      "min_chunk_bytes_for_parallel_parsing", "min_external_table_block_size_bytes", "min_insert_block_size_bytes",
      "min_insert_block_size_bytes_for_materialized_views", "min_joined_block_size_bytes", "output_format_parquet_row_group_size_bytes",
      "page_cache_block_size", "partial_merge_join_left_table_buffer_bytes", "prefer_external_sort_block_bytes",
      "preferred_block_size_bytes", "preferred_max_column_in_block_size_bytes", "prefetch_buffer_size", "query_cache_max_size_in_bytes",
      "remote_read_min_bytes_for_seek", "temporary_files_buffer_size",
      /// ClickHouse cloud setting
      "write_through_distributed_cache_buffer_size"};
  DB::Strings max_rows_values = {"cluster_table_function_buckets_batch_size",
  ...
  for (const auto &entry : max_bytes_values) {
    ...
    serverSettings.insert({{entry, CHSetting(bytesRange, {"0", "1", "2", "4", "8", "16", "32", "1024", "2048", "4096", "16384"}, false)}});
  }
  for (const auto &entry : max_rows_values) {
    ...
    serverSettings.insert({{entry, CHSetting(rowsRange, {"0", "1", "2", "4", "8", "16", "32", "1024", "2048", "4096", "16384"}, false)}});
  }
  ...
  if (!fc.timezones.empty()) {
    ...
    formatSettings.insert({{"input_format_orc_reader_time_zone_name", CHSetting(timeZones, {}, false)},
    ...
    serverSettings.insert(
        {{"backup_restore_keeper_fault_injection_probability", CHSetting(probRange, {}, false)},
         ...
         {"unknown_packet_in_send_data",
          CHSetting([](RandomGenerator &rg, FuzzConfig &) { return std::to_string(rg.thresholdGenerator<uint64_t>(0.1, 0.1, 0, 16384)); },
                    {}, false)}});
  }
  ...
  if (fc.enable_overflow_settings) {
    ...
    serverSettings.insert({{"distinct_overflow_mode", overflowSetting},
  ...
  if (fc.enable_compatibility_settings) {
    serverSettings.insert({{"compatibility", CHSetting(
                                                 [&](RandomGenerator &rg, FuzzConfig &) {
                                                   ...
                                                   const uint32_t currentYear = std::max<uint32_t>(
                                                       minYear, static_cast<uint32_t>(static_cast<int>(ymd.year()) % 100));  /// YY
  ...
  if (fc.enable_memory_settings) {
    static const auto memorySetting = CHSetting(
        [](RandomGenerator &rg, FuzzConfig &) {
          return std::to_string(rg.thresholdGenerator<uint64_t>(0.2, 0.2, UINT32_C(10) * UINT32_C(1024) * UINT32_C(1024),
                                                                UINT32_C(100) * UINT32_C(1024) * UINT32_C(1024)));
        },
        {"'10M'", "'20M'", "'50M'", "'100M'"}, false);

    serverSettings.insert({{"filesystem_prefetch_max_memory_usage", memorySetting},
  ...
  formatSettings.insert(
      {{"bool_false_representation", CHSetting(nastyStrings, {}, false)},
       ...
       {"format_capn_proto_enum_comparising_mode", CHSetting(
                                                       [](RandomGenerator &rg, FuzzConfig &) {
                                                         static const DB::Strings choices = {"'by_names'", "'by_names_case_insensitive'",
                                                                                             "'by_values'"};
                                                         return rg.pickRandomly(choices);
                                                       },
                                                       {}, false)},
       ...
       {"format_custom_escaping_rule", CHSetting(
                                           [](RandomGenerator &rg, FuzzConfig &) {
                                             static const DB::Strings choices = {"'None'", "'Escaped'", "'Quoted'", "'CSV'",
                                                                                 "'JSON'", "'XML'",     "'Raw'"};
                                             return rg.pickRandomly(choices);
                                           },
                                           {}, false)},
       ...
       {"output_format_orc_dictionary_key_size_threshold",
        CHSetting([](RandomGenerator &rg, FuzzConfig &) { return std::to_string(rg.thresholdGenerator<double>(0.2, 0.2, 0.0, 1.0)); }, {},
                  false)},
       ...
       {"output_format_msgpack_uuid_representation", CHSetting(
                                                         [](RandomGenerator &rg, FuzzConfig &) {
