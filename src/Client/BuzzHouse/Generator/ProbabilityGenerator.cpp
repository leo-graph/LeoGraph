...
uint64_t ProbabilityGenerator::getSeed() const { return seed; }

ProbabilityStrategy ProbabilityGenerator::getStrategy() const { return strategy; }

const std::vector<double>& ProbabilityGenerator::getProbs() const { return probabilities; }

const std::vector<bool>& ProbabilityGenerator::getEnabledMask() const { return enabled_values; }
...
void ProbabilityGenerator::setEnabled(const size_t i, const bool on) {
  if (i >= nvalues) throw std::out_of_range("setEnabled index out of range");
...
void ProbabilityGenerator::ensureAtLeastOneEnabled(const std::vector<bool>& mask) {
  ...
  if (!any) throw DB::Exception(DB::ErrorCodes::BUZZHOUSE, "At least one option must be enabled");
}

size_t ProbabilityGenerator::lastEnabledEnum() const {
  for (size_t i = nvalues; i-- > 0;)
    if (enabled_values[i]) return i;
...
std::vector<double> ProbabilityGenerator::genBalanced() {
  ...
  for (int attempt = 0; attempt < std::max(1, balanced_resample_attempts); ++attempt) {
    ...
    for (size_t i = 0; i < nvalues; ++i) w[i] = enabled_values[i] ? (exp(generator) + 1e-12) : 0.0;
    ...
    if (ratio <= balanced_max_ratio) return w;
  }
...
std::vector<double> ProbabilityGenerator::genBounded() {
  ...
  for (size_t i = 0; i < nvalues; ++i) {
    ...
    if (b.min < 0.0 || b.max < 0.0 || b.min > b.max) throw DB::Exception(DB::ErrorCodes::BUZZHOUSE, "Invalid bounds");
...
void ProbabilityGenerator::normalizeEnabledInPlace(std::vector<double>& v, const std::vector<bool>& enabled) const {
  ...
  for (size_t i = 0; i < nvalues; ++i) v[i] = enabled[i] ? (v[i] / sum) : 0.0;
}
...
std::pair<double, double> ProbabilityGenerator::minmaxEnabled(const std::vector<double>& v, const std::vector<bool>& enabled) const {
  ...
  if (!std::isfinite(mn)) mn = 0.0;  /// should not happen
  return {mn, mx};
}
...
void ProbabilityGenerator::clampToBoundsAndRenormEnabled(std::vector<double>& p, const std::vector<bool>& enabled) {
  ...
  for (int iter = 0; iter < 10; ++iter) {
    ...
    double used = 0.0;
    for (size_t i = 0; i < nvalues; ++i) used += p[i];
    ...
    for (size_t i = 0; i < nvalues; ++i)
      if (enabled[i] && !fixed[i]) free_mass += p[i];
    ...
    for (size_t i = 0; i < nvalues; ++i)
      if (enabled[i] && !fixed[i]) p[i] += remaining * (p[i] / free_mass);
  }
