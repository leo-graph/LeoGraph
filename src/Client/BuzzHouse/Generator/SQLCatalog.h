...
enum class ColumnSpecial { NONE = 0, SIGN = 1, IS_DELETED = 2, VERSION = 3, TTL_COL = 4, ID_COL = 5 };
...
enum class PeerQuery { None = 0, ClickHouseOnly = 1, AllPeers = 2 };

enum class LakeFormat { All = 0, Iceberg = 1, DeltaLake = 2 };

enum class LakeStorage { All = 0, S3 = 1, Azure = 2, Local = 3 };

enum class LakeCatalog { None = 0, Glue = 1, Hive = 2, REST = 3, Unity = 4 };
...
struct SQLBase {
 public:
  ...
  SQLBase() = default;
  explicit SQLBase(const String &&p) : prefix(p) {}
...
struct ColumnPathChain {
 public:
  ...
  ColumnPathChain(const std::optional<bool> nu, const ColumnSpecial cs, const std::optional<DModifier> dm,
                  const std::vector<ColumnPathChainEntry> p)
