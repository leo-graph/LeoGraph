...
void ClientApplicationBase::parseAndCheckOptions(OptionsDescription& options_description, po::variables_map& options,
                                                 Arguments& arguments) {
  ...
  if (!unrecognized_options.empty()) {
    ...
    if (!hints.empty())
      throw Exception(ErrorCodes::UNRECOGNIZED_ARGUMENTS, "Unrecognized option '{}'. Maybe you meant {}", unrecognized_options[0],
                      toString(hints));
...
OptionsAliasParser::OptionsAliasParser(const boost::program_options::options_description& options) {
  ...
  for (const auto& option : options.options()) options_names.insert(option->long_name());
}

std::pair<std::string, std::string> OptionsAliasParser::operator()(const std::string& token) const {
  if (!token.starts_with("--")) return {};
  ...
  std::string value;
  if (pos_eq != std::string::npos && pos_eq < arg.size()) value = arg.substr(pos_eq + 1);
