...
class Suggest : public LineReader::Suggest, boost::noncopyable {
 public:
  ...
  ~Suggest() {
    if (loading_thread.joinable()) loading_thread.join();
  }
  ...
  void load(IServerConnection& connection, const ConnectionTimeouts& timeouts, Int32 suggestion_limit, const ClientInfo& client_info);
