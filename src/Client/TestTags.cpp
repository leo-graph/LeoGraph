...
size_t getTestTagsLength(const String& multiline_query) {
  ...
  while (true) {
    ...
    if (text.compare(pos, strlen(comment), comment) != 0) return line_start;
    pos += strlen(comment);
    ...
    if (first_line) {
      ...
      if (text.compare(pos, strlen(tags_prefix), tags_prefix) != 0) return 0;
    ...
    if (eol_pos == String::npos) return text.length();
