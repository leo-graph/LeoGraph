...
ConnectionPoolWithFailover::ConnectionPoolWithFailover(ConnectionPoolPtrs nested_pools_, LoadBalancing load_balancing,
                                                       time_t decrease_error_period_, size_t max_error_cap_)
  ...
  for (size_t i = 0; i < nested_pools.size(); ++i) {
    ...
    get_priority_load_balancing.hostname_levenshtein_distance[i] =
        getHostNameLevenshteinDistance(local_hostname, connection_pool.getHost());
  }
}
...
IConnectionPool::Entry ConnectionPoolWithFailover::get(const ConnectionTimeouts &timeouts, const Settings &settings,
  ...
  GetPriorityFunc get_priority =
      get_priority_load_balancing.getPriorityFunc(settings[Setting::load_balancing], offset, nested_pools.size());
...
ConnectionPoolWithFailover::Status ConnectionPoolWithFailover::getStatus() const {
  ...
  for (size_t i = 0; i < states.size(); ++i) {
    ...
    const auto seconds_to_zero_errors =
        std::max(static_cast<time_t>(0),
                 std::max(rounds_to_zero_errors, rounds_to_zero_slowdowns) * decrease_error_period - since_last_error_decrease);

    result.emplace_back(NestedPoolStatus{pools[i], updated_states[i].error_count, updated_states[i].slowdown_count,
...
std::vector<IConnectionPool::Entry> ConnectionPoolWithFailover::getMany(const ConnectionTimeouts &timeouts, const Settings &settings,
                                                                        PoolMode pool_mode, AsyncCallback async_callback,
  ...
  for (auto &result : results) entries.emplace_back(std::move(result.entry));
  return entries;
}

std::vector<ConnectionPoolWithFailover::TryResult> ConnectionPoolWithFailover::getManyForTableFunction(const ConnectionTimeouts &timeouts,
...
std::vector<ConnectionPoolWithFailover::TryResult> ConnectionPoolWithFailover::getManyChecked(
    const ConnectionTimeouts &timeouts, const Settings &settings, PoolMode pool_mode, const QualifiedTableName &table_to_check,
    AsyncCallback async_callback, std::optional<bool> skip_unavailable_endpoints, GetPriorityForLoadBalancing::Func priority_func) {
  TryGetEntryFunc try_get_entry = [&](const NestedPoolPtr &pool, std::string &fail_message) {
...
std::vector<ConnectionPoolWithFailover::TryResult> ConnectionPoolWithFailover::getManyCheckedForInsert(
  ...
  return getManyImpl(settings, pool_mode, try_get_entry,
                     /*skip_unavailable_endpoints=*/false,  /// skip_unavailable_endpoints is used to get the min number of entries, and we
                                                            /// need at least one
                     /*priority_func=*/{}, settings[Setting::distributed_insert_skip_read_only_replicas]);
}
...
std::vector<ConnectionPoolWithFailover::TryResult> ConnectionPoolWithFailover::getManyImpl(const Settings &settings, PoolMode pool_mode,
  ...
  if (!skip_unavailable_endpoints.has_value()) skip_unavailable_endpoints = settings[Setting::skip_unavailable_shards];
  ...
  if (!priority_func) priority_func = makeGetPriorityFunc(settings);
  ...
  return Base::getMany(min_entries, max_entries, max_tries, max_ignored_errors, fallback_to_stale_replicas, skip_read_only_replicas,
                       try_get_entry, priority_func);
}

ConnectionPoolWithFailover::TryResult ConnectionPoolWithFailover::tryGetEntry(
    const ConnectionPoolPtr &pool, const ConnectionTimeouts &timeouts, std::string &fail_message, const Settings &settings,
    const QualifiedTableName *table_to_check, [[maybe_unused]] AsyncCallback async_callback, bool force_connected) {
#if defined(OS_LINUX)
  if (async_callback) {
    ...
    while (true) {
      ...
      if (connection_establisher_async.isFinished()) break;

      async_callback(connection_establisher_async.getFileDescriptor(), 0, AsyncEventTimeoutType::NONE,
                     "Connection establisher file descriptor", AsyncTaskExecutor::Event::READ | AsyncTaskExecutor::Event::ERROR);
    }
...
std::vector<ConnectionPoolWithFailover::Base::ShuffledPool> ConnectionPoolWithFailover::getShuffledPools(
    const Settings &settings, GetPriorityForLoadBalancing::Func priority_func, bool use_slowdown_count) {
  if (!priority_func) priority_func = makeGetPriorityFunc(settings);
