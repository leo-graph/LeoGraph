...
class ProgressTable {
 public:
  explicit ProgressTable(int in_fd_ = STDIN_FILENO, int err_fd_ = STDERR_FILENO) : in_fd(in_fd_), err_fd(err_fd_) {}

  /// Write progress table with metrics.
  void writeTable(WriteBufferFromFileDescriptor& message, std::unique_lock<std::mutex>& message_lock, bool show_table, bool toggle_enabled,
                  bool is_final);
