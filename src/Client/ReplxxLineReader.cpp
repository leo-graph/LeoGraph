...
int executeCommand(char* const argv[]) {
...
#endif
  if (!real_vfork) throw std::runtime_error("Cannot find vfork symbol");

  pid_t pid = reinterpret_cast<pid_t (*)()>(real_vfork)();

  if (-1 == pid) throw std::runtime_error(fmt::format("Cannot vfork {}: {}", argv[0], errnoToString()));
  ...
  if (WIFSIGNALED(status)) throw std::runtime_error(fmt::format("Child process was terminated by signal {}", WTERMSIG(status)));
  if (WIFSTOPPED(status)) throw std::runtime_error(fmt::format("Child process was stopped by signal {}", WSTOPSIG(status)));
...
void writeRetry(int fd, const std::string& data) {
  ...
  while (bytes_written != offset) {
    ...
    if ((-1 == res || 0 == res) && errno != EINTR) throw std::runtime_error(fmt::format("Cannot write to {}: {}", fd, errnoToString()));
    bytes_written += res;
  }
}
...
class TemporaryFile {
 ...
 public:
  explicit TemporaryFile(const char* pattern) : path(pattern) {
    ...
    if (-1 == fd) throw std::runtime_error(fmt::format("Cannot create temporary file {}: {}", path, errnoToString()));
  }
  ~TemporaryFile() {
    try {
      close();
  ...
  void close() {
    ...
    if (0 != ::close(fd)) throw std::runtime_error(fmt::format("Cannot close temporary file {}: {}", path, errnoToString()));
    fd = -1;
  }

  void write(const std::string& data) {
    if (fd == -1) throw std::runtime_error(fmt::format("Cannot write to uninitialized file {}", path));

    writeRetry(fd, data);
  }

  void unlink() {
    if (0 != ::unlink(path.c_str())) throw std::runtime_error(fmt::format("Cannot remove temporary file {}: {}", path, errnoToString()));
  }
...
std::string replxx_now_ms_str() {
  ...
  tm broken;
  if (!localtime_r(&t, &broken)) return {};

  static int const BUFF_SIZE(32);
  char str[BUFF_SIZE];
  if (strftime(str, BUFF_SIZE, "%Y-%m-%d %H:%M:%S.", &broken) <= 0) return {};

  if (snprintf(str + sizeof("YYYY-mm-dd HH:MM:SS"), 5, "%03d", static_cast<int>(ms.count() % 1000)) <= 0) return {};
...
void ReplxxLineReader::setLastIsDelimiter(bool flag) { replxx_last_is_delimiter = flag; }
...
ReplxxLineReader::ReplxxLineReader(ReplxxLineReader::Options&& options)
  ...
  if (highlighter) rx.set_highlighter_callback(highlighter);
  ...
  if (options.ignore_shell_suspend) rx.bind_key_internal(replxx::Replxx::KEY::control('Z'), "insert_character");
  ...
  auto commit_action = [this](char32_t code) {
    ...
    if (highlighter && multiline && !replxx_last_is_delimiter) return rx.invoke(Replxx::ACTION::NEW_LINE, code);
  ...
  rx.bind_key(Replxx::KEY::meta(Replxx::KEY::BACKSPACE),
              [this](char32_t code) { return rx.invoke(Replxx::ACTION::KILL_TO_BEGINING_OF_WORD, code); });
  ...
  if (!options.embedded_mode) {
    rx.bind_key(Replxx::KEY::meta('E'), [this](char32_t) {
      openEditor(/*format_query=*/false);
      return Replxx::ACTION_RESULT::CONTINUE;
    });
    rx.bind_key(Replxx::KEY::meta('F'), [this](char32_t) {
      openEditor(/*format_query=*/true);
      return Replxx::ACTION_RESULT::CONTINUE;
    });
  }
  ...
  char key_fuzzy = 'R';
  char key_regular = 'T';
  if (options.interactive_history_legacy_keymap) std::swap(key_fuzzy, key_regular);
  ...
  if (!options.embedded_mode) {
    auto interactive_history_search = [this, key_regular](char32_t code) {
      std::vector<std::string> words;
      {
        ...
        while (hs.next()) words.push_back(hs.get().text());
      }
      ...
      if (!new_query.empty()) rx.set_state(replxx::Replxx::State(new_query.c_str(), static_cast<int>(new_query.size())));

      if (bracketed_paste_enabled) enableBracketedPaste();
...
ReplxxLineReader::~ReplxxLineReader() {
  if (history_file_fd >= 0 && close(history_file_fd)) rx.print("Close of history file failed: %s\n", errnoToString().c_str());

  /// Reset cursor blinking
  if (overwrite_mode) rx.print("%s", "\033[0 q");
}
...
LineReader::InputStatus ReplxxLineReader::readOneLine(const String& prompt) {
  ...
  if (cinput == nullptr) return (errno != EAGAIN) ? ABORT : RESET_LINE;
  input = cinput;
...
void ReplxxLineReader::addToHistory(const String& line) {
  ...
  if (history_file_fd >= 0 && !rx.history_save(history_file_path)) rx.print("Saving history failed: %s\n", errnoToString().c_str());
...
void ReplxxLineReader::openEditor(bool format_query) {
  ...
  try {
    ...
    if (format_query) query = formatQuery(std::move(query));
