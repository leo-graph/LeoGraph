...
bool isNewYearMode() {
  ...
  if (current_time % 3 != 0) return false;
...
bool isChineseNewYearMode(const String& local_tz) {
  ...
  static constexpr const char* chineseNewYearTimeZoneIndicators[] = {
      ...
      "Asia/Shanghai",  "Asia/Chongqing", "Asia/Harbin", "Asia/Urumqi", "Asia/Hong_Kong",
      "Asia/Chungking", "Asia/Macao",     "Asia/Macau",  "Asia/Taipei", "Asia/Singapore",
  ...
  if (chineseNewYearTimeZoneIndicators + M == std::find_if(chineseNewYearTimeZoneIndicators, chineseNewYearTimeZoneIndicators + M,
                                                           [&local_tz](const char* tz) { return tz == local_tz; }))
    return false;
  ...
  for (auto d : chineseNewYearIndicators) {
    ...
    if (d <= days && d + 25 >= days) return true;
...
std::string getChineseZodiac() {
  ...
  static constexpr const char* zodiacs[12] = {"鼠", "牛", "虎", "兔", "龙", "蛇", "马", "羊", "猴", "鸡", "狗", "猪"};
...
void highlight(const String& query, std::vector<replxx::Replxx::Color>& colors, const Context& context, int cursor_position) {
  ...
  IParser::Pos token_iterator(tokens, static_cast<uint32_t>(context.getSettingsRef()[Setting::max_parser_depth]),
  ...
  try {
    while (!token_iterator->isEnd()) {
      ...
      while (token_iterator->type == TokenType::Semicolon) ++token_iterator;
    }
  } catch (...) {
  ...
  const char* cursor_char_position =
      cursor_position >= 0 ? begin + UTF8::computeBytesBeforeCodePoint(reinterpret_cast<const UInt8*>(begin), query.size(), cursor_position)
                           : end;
  ...
  for (const auto& range : expected.highlights) {
    ...
    if (range.highlight == Highlight::string_like) metacharacters = "%_";
    if (range.highlight == Highlight::string_regexp) metacharacters = "|()^$.[]?*+{:-";
    ...
    if (it != type_to_color.end()) {
      ...
      if (!highlight_matching_identifiers && range.begin <= cursor_char_position && cursor_char_position < range.end &&
        ...
        if (is_regular_number) {
          if (*char_pos == '-') {
            ...
            if (!is_regular_number_finished) {
              if (!regular_number_before_decimal_code_point_first) regular_number_before_decimal_code_point_first = code_point_pos;
      ...
      /// Highlight digit groups inside numbers
      if (is_regular_number && regular_number_before_decimal_code_point_first && regular_number_before_decimal_code_point_last) {
  ...
  if (highlight_matching_identifiers) {
    ...
    for (const auto& range : expected.highlights) {
      if ((range.highlight == Highlight::identifier || range.highlight == Highlight::alias) &&
          std::string_view(range.begin, range.end) ==
              std::string_view(highlight_matching_identifiers->begin, highlight_matching_identifiers->end)) {
  ...
  IParser::Pos highlight_token_iterator(tokens, static_cast<uint32_t>(context.getSettingsRef()[Setting::max_parser_depth]),
                                        static_cast<uint32_t>(context.getSettingsRef()[Setting::max_parser_backtracks]));

  try {
    while (!highlight_token_iterator->isEnd()) {
      if (highlight_token_iterator->isError()) break;
      ...
      if (highlight_token_iterator->type == TokenType::ClosingRoundBracket) {
        ...
        if (highlight_pos < colors.size()) colors[highlight_pos] = color_stack.back();
        ...
        if (matching_brace_pos < colors.size()) colors[matching_brace_pos] = color_stack.back();
      ...
      while (highlight_token_iterator->type == TokenType::Semicolon) ++highlight_token_iterator;
    }
  } catch (...) {
    /// Skip highlighting in the case of exceptions during parsing.
    /// It is ok to ignore unknown exceptions here.
  ...
  if ((!parse_res || last_token.isError()) && !(insert_data && expected.max_parsed_pos >= insert_data) &&
    ...
    if (code_point_pos >= colors.size()) code_point_pos = colors.size() - 1;
  ...
  if (last_token.type == TokenType::Semicolon || last_token.type == TokenType::VerticalDelimiter || query.ends_with(';') ||
      query.ends_with("\\G"))  /// This is for raw data in INSERT queries, which is not necessarily tokenized.
  {
...
String highlighted(const String& query, const Context& context) {
  ...
  if (replxx::Replxx::Color::DEFAULT != prev_color) res += replxx::ansi_color(replxx::Replxx::Color::DEFAULT);
...
void skipSpacesAndComments(const char*& pos, const char* end, std::function<void(std::string_view)> comment_callback) {
  do {
    ...
    if (end - pos > 2 && *pos == '-' && *(pos + 1) == '-') {
      ...
      while (pos != end && *pos != '\n') ++pos;
      if (comment_callback) comment_callback(std::string_view(comment_begin, pos - comment_begin));
    }
...
String formatQuery(String query) {
  ...
  while (pos < end) {
    ...
    const ASTPtr ast = parseQueryAndMovePosition(parser, pos, end, "query in editor", /*allow_multi_statements=*/true, /*max_query_size=*/0,
                                                 max_parser_depth, max_parser_backtracks);
    ...
    if (auto* insert_ast = ast->as<ASTInsertQuery>(); insert_ast && insert_ast->data) {
      if (Poco::toLower(insert_ast->format) == "values") {
        ...
        if (*insert_ast->end == '\n') {
          while (insert_ast->end > insert_ast->data && std::isspace(*insert_ast->end)) --insert_ast->end;
        }
      } else
        pos = insert_ast->end;
