...
class ClientBase {
 public:
  ...
  explicit ClientBase(int in_fd_ = STDIN_FILENO, int out_fd_ = STDOUT_FILENO, int err_fd_ = STDERR_FILENO,
 ...
 protected:
  ...
  virtual bool buzzHouse() { throw Exception(ErrorCodes::NOT_IMPLEMENTED, "ClickHouse was compiled without BuzzHouse enabled"); }
  ...
  void processParsedSingleQuery(std::string_view query_, ASTPtr parsed_query, bool& is_async_insert_with_inlined_data,
  ...
  MultiQueryProcessingStage analyzeMultiQueryText(const char*& this_query_begin, const char*& this_query_end, const char* all_queries_end,
                                                  ASTPtr& parsed_query, std::unique_ptr<Exception>& current_exception);
  ...
  virtual void processOptions(const OptionsDescription& options_description, const CommandLineOptions& options,
 ...
 private:
  ...
  void sendDataFrom(ReadBuffer& buf, Block& sample, const ColumnsDescription& columns_description, ASTPtr parsed_query,
