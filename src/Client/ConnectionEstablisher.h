...
class ConnectionEstablisher {
 public:
  ...
  ConnectionEstablisher(ConnectionPoolPtr pool_, const ConnectionTimeouts* timeouts_, const Settings& settings_, LoggerPtr log,
                        const QualifiedTableName* table_to_check = nullptr);

  /// Establish connection and save it in result, write possible exception message in fail_message.
  /// The connection is returned from connection pool and it can be stale. Use force_connected flag to ensure that connection is working
  /// one. NOTE: force_connected is false by default due to the following consideration ...
...
class ConnectionEstablisherAsync : public AsyncTaskExecutor {
 public:
  ...
  ConnectionEstablisherAsync(ConnectionPoolPtr pool_, const ConnectionTimeouts* timeouts_, const Settings& settings_, LoggerPtr log_,
  ...
  void resumeConnectionWithForceOption(bool force_connected_) {
    force_connected = force_connected_;
    resume();
  }
 ...
 private:
  ...
  void processAsyncEvent(int fd, Poco::Timespan socket_timeout, AsyncEventTimeoutType type, const std::string& description,
