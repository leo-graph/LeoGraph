  ...
  if (const auto *query_with_output = dynamic_cast<const DB::ASTQueryWithOutput *>(parsed_query.get())) {
    if (query_with_output->isOutfileTruncate() && query_with_output->out_file) {
      ...
      try {
        ...
        throw DB::Exception(DB::ErrorCodes::CANNOT_WRITE_TO_FILE, "Cannot rename temporary file {} to {}: {}", tmp_file, out_file,
                            e.what());
      }
    }
  }
...
ProgressOption toProgressOption(std::string progress) {
  ...
  if (progress == "OFF" || progress == "FALSE" || progress == "0" || progress == "NO") return ProgressOption::OFF;
  if (progress == "TTY" || progress == "ON" || progress == "TRUE" || progress == "1" || progress == "YES") return ProgressOption::TTY;
  if (progress == "ERR") return ProgressOption::ERR;
  if (progress == "DEFAULT") return ProgressOption::DEFAULT;
...
static void incrementProfileEventsBlock(Block &dst, const Block &src) {
  ...
  for (size_t i = 0; i < dst.columns(); ++i) name_pos[dst.getByPosition(i).name] = i;
  ...
  for (size_t src_row = 0; src_row < src.rows(); ++src_row) {
    ...
    if (thread_id != THREAD_GROUP_ID) continue;
...
ClientBase::ClientBase(int in_fd_, int out_fd_, int err_fd_, std::istream &input_stream_, std::ostream &output_stream_,
...
ASTPtr ClientBase::parseQuery(const char *&pos, const char *end, const Settings &settings, bool allow_multi_statements) {
  ...
  if (!allow_multi_statements) max_length = settings[Setting::max_query_size];
...
void ClientBase::adjustQueryEnd(const char *&this_query_end, const char *all_queries_end, uint32_t max_parser_depth,
                                uint32_t max_parser_backtracks) {
...
void ClientBase::sendExternalTables(ASTPtr parsed_query) {
  ...
  for (auto &table : external_tables) data.emplace_back(table.getData(client_context));
...
void ClientBase::onData(Block &block, ASTPtr parsed_query) {
  ...
  if (block.rows() == 0 || (query_fuzzer_runs != 0 && processed_rows_from_blocks >= 100)) return;
  ...
  if (need_render_progress && tty_buf) {
    if (select_into_file && !select_into_file_and_stdout) error_stream << "\r";
    std::unique_lock lock(tty_mutex);
    progress_indication.writeProgress(*tty_buf, lock);
  }
  if (need_render_progress_table && tty_buf && !cancelled) {
    if (!need_render_progress && select_into_file && !select_into_file_and_stdout) error_stream << "\r";
    std::unique_lock lock(tty_mutex);
...
void ClientBase::initOutputFormat(const Block &block, ASTPtr parsed_query) try {
  if (!output_format) {
    ...
    if (const auto *query_with_output = dynamic_cast<const ASTQueryWithOutput *>(parsed_query.get())) {
      ...
      if (query_with_output->out_file) {
        ...
        out_file_buf =
            wrapWriteBufferWithCompressionMethod(std::make_unique<WriteBufferFromFile>(out_file, DBMS_DEFAULT_BUFFER_SIZE, flags),
                                                 compression_method, static_cast<int>(compression_level));
        ...
        if (is_interactive && is_default_format) current_format = "TabSeparated";
      }
      if (query_with_output->format_ast != nullptr) {
        if (has_vertical_output_suffix) throw Exception(ErrorCodes::CLIENT_OUTPUT_FORMAT_SPECIFIED, "Output format already specified");
        ...
        if (format_name) current_format = *format_name;
      }
    }

    if (has_vertical_output_suffix) current_format = "Vertical";
    ...
    if (!extras_into_stdout || select_only_into_file)
      output_format = client_context->getOutputFormatParallelIfPossible(current_format, out_file_buf ? *out_file_buf : *out_buf, block,
                                                                        format_settings);
    else
      output_format = client_context->getOutputFormat(current_format, out_file_buf ? *out_file_buf : *out_buf, block, format_settings);
    ...
    if ((!select_into_file || select_into_file_and_stdout) && stdout_is_a_tty && stdin_is_a_tty &&
      ...
      const auto question =
          fmt::format(R"(The requested output format `{}` is binary and could produce side-effects when output directly into the terminal.
...
Do you want to output it anyway? [y/N] )",
      ...
      if (!ask(question, *std_in, *std_out)) output_format = std::make_shared<NullOutputFormat>(std::make_shared<const Block>(block));

      *std_out << '\n';
    }
  }
} catch (...) {
  if (out_file_buf) out_file_buf->cancel();
  out_file_buf.reset();
  ...
  if (!logs_out_stream) {
    ...
    if (!out_logs_buf) {
      if (server_logs_file.empty()) {
        ...
        out_logs_buf = std::make_unique<AutoCanceledWriteBuffer<WriteBufferFromFile>>(server_logs_file, DBMS_DEFAULT_BUFFER_SIZE,
...
void ClientBase::setDefaultFormatsAndCompressionFromConfiguration() {
  if (getClientConfiguration().has("output-format")) {
    ...
    if (file_name) default_output_compression_method = chooseCompressionMethod(*file_name, "");
  ...
  if (getClientConfiguration().has("input-format")) {
    ...
    if (file_name) default_input_compression_method = chooseCompressionMethod(*file_name, "");
  }
...
void ClientBase::initTTYBuffer(ProgressOption progress_option, ProgressOption progress_table_option) {
  ...
  if (!need_render_progress && !need_render_progress_table) return;
  ...
  if (!ec && exists(tty) && is_character_file(tty) &&
    ...
    try {
      ...
      if (e.code() != ErrorCodes::CANNOT_OPEN_FILE) throw;
...
void ClientBase::initKeystrokeInterceptor() {
  if (is_interactive && need_render_progress_table && progress_table_toggle_enabled) {
    ...
    if (isEmbeeddedClient()) keystroke_interceptor->registerCallback(0x03, [this]() { tryStopQuery(); });
  }
}
...
String ClientBase::appendSmileyIfNeeded(const String &prompt_) {
  ...
  if (prompt_.empty()) return smiley;

  if (prompt_.ends_with(smiley)) return prompt_;
...
void ClientBase::updateSuggest(const ASTPtr &ast) {
  ...
  if (auto *create = ast->as<ASTCreateQuery>()) {
    if (create->database) new_words.push_back(create->getDatabase());
    new_words.push_back(create->getTable());

    if (create->columns_list && create->columns_list->columns) {
      for (const auto &elem : create->columns_list->columns->children) {
        if (const auto *column = elem->as<ASTColumnDeclaration>()) new_words.push_back(column->name);
      }
    }
  }
  ...
  if (!new_words.empty()) suggest->addWords(std::move(new_words));
}
...
bool ClientBase::processTextAsSingleQuery(const String &full_query) {
  ...
  auto parsed_query = parseQuery(begin, begin + full_query.size(), client_context->getSettingsRef(),
  ...
  if (suggest) updateSuggest(parsed_query);
  ...
  try {
    ...
    if (server_exception) server_exception->rethrow();
    if (!is_interactive) e.addMessage("(in query: {})", full_query);
    throw;
  }

  if (have_error) processError(full_query);
  return !have_error;
}
...
void ClientBase::processOrdinaryQuery(String query, ASTPtr parsed_query) {
  ...
  if (!query_parameters.empty() &&
    ...
    if (visitor.getNumberOfReplacedParameters()) query = parsed_query->formatWithSecretsOneLine();
  ...
  if (const auto *query_with_output = dynamic_cast<const ASTQueryWithOutput *>(parsed_query.get())) {
    if (query_with_output->out_file) {
      ...
      if (query_with_output->isOutfileTruncate()) {
        ...
        out_file =
            (out_file_path.parent_path() / fmt::format("tmp_{}.{}", UUIDHelpers::generateV4(), out_file_path.filename().string())).string();
      }
      ...
      if (client_context->getSettingsRef()[Setting::into_outfile_create_parent_directories]) {
        ...
        if (!parent_dir.empty() && !fs::exists(parent_dir)) {
          try {
            ...
            throw Exception(ErrorCodes::CANNOT_CREATE_DIRECTORY, "Cannot create parent directories for INTO OUTFILE '{}': {}",
      ...
      if (query_with_output->compression_level) {
        ...
        if (!res || compression_level < range.first || compression_level > range.second)
          throw Exception(ErrorCodes::BAD_ARGUMENTS, "Invalid compression level, must be positive integer in range {}-{}", range.first,
                          range.second);
      }
  ...
  while (retries_left) {
    try {
      ...
      try {
        connection->sendQuery(connection_parameters.timeouts, query, query_parameters, client_context->getCurrentQueryId(),
                              query_processing_stage, &client_context->getSettingsRef(), &client_context->getClientInfo(), true, {},
                              [&](const Progress &progress) { onProgress(progress); });

        if (send_external_tables) sendExternalTables(parsed_query);
      } catch (const NetException &) {
...
void ClientBase::receiveResult(ASTPtr parsed_query, Int32 signals_before_stop, bool partial_result_on_first_cancel) {
  ...
  while (true) {
    ...
    while (true) {
      ...
      if (connection->poll(poll_interval)) break;
    }

    try {
      if (!receiveAndProcessPacket(parsed_query, cancelled)) break;
    } catch (const LocalFormatError &) {
      /// Remember the first exception.
      if (!local_format_error) local_format_error = std::current_exception();
      sendCancel(std::current_exception());
    }
  }

  if (local_format_error) std::rethrow_exception(local_format_error);

  if (cancelled && is_interactive && !cancelled_printed.exchange(true)) output_stream << "Query was cancelled." << std::endl;
}
...
bool ClientBase::receiveAndProcessPacket(ASTPtr parsed_query, bool cancelled_) {
  ...
  switch (packet.type) {
    ...
    case Protocol::Server::Data:
      if (!cancelled_) onData(packet.block, parsed_query);
      return true;
    ...
    case Protocol::Server::Totals:
      if (!cancelled_) onTotals(packet.block, parsed_query);
      return true;

    case Protocol::Server::Extremes:
      if (!cancelled_) onExtremes(packet.block, parsed_query);
      return true;
    ...
    default:
      throw Exception(ErrorCodes::UNKNOWN_PACKET_FROM_SERVER, "Unknown packet {} from server {}", packet.type,
...
void ClientBase::onProgress(const Progress &value) {
  ...
  if (output_format) output_format->onProgress(value);
...
void ClientBase::onTimezoneUpdate(const String &tz) { client_context->setSetting("session_timezone", tz); }
...
void ClientBase::onEndOfStream() {
  ...
  if (output_format) {
    ...
    output_format->setStartTime(clock_gettime_ns(CLOCK_MONOTONIC) - static_cast<UInt64>(progress_indication.elapsedSeconds() * 1000000000),
                                is_running);
...
void ClientBase::resetOutput() {
  ...
  try {
    if (output_format) output_format->finalize();
  } catch (...) {
  ...
  if (out_file_buf) out_file_buf->finalize();
  out_file_buf.reset();
...
bool isStdinNotEmptyAndValid(ReadBuffer &std_in) {
  try {
    ...
    if (e.code() == ErrorCodes::CANNOT_READ_FROM_FILE_DESCRIPTOR) return false;
    throw;
  }
}
}  // namespace
...
void ClientBase::processInsertQuery(String query, ASTPtr parsed_query) {
  ...
  if ((!parsed_insert_query.data && !parsed_insert_query.infile) &&
    ...
    const auto &settings = client_context->getSettingsRef();
    if (settings[Setting::throw_if_no_data_to_insert]) throw Exception(ErrorCodes::NO_DATA_TO_INSERT, "No data to insert");
    return;
  }
  ...
  try {
    ...
    try {
      if (sendCancel(std::current_exception())) receiveEndOfQueryForInsert();
...
void ClientBase::sendData(Block &sample, const ColumnsDescription &columns_description, ASTPtr parsed_query) {
  ...
  if (!connection->isSendDataNeeded()) return;
  ...
  if (need_render_progress) {
    if (auto *std_in_desc = typeid_cast<ReadBufferFromFileDescriptor *>(std_in.get())) {
      ...
      if (tty_buf) progress_indication.setFileProgressCallback(client_context, *tty_buf, tty_mutex);
    }
  }
  ...
  if (parsed_insert_query->infile) {
    ...
    if (current_format.empty()) current_format = FormatFactory::instance().getFormatFromFileName(in_file);
    ...
    if (parsed_insert_query->columns) {
      auto columns = processColumnTransformers(client_context->getCurrentDatabase(), client_context->getInsertionTable(),
                                               columns_for_storage_file, parsed_insert_query->columns);
    ...
    try {
      ...
      QueryPlan plan;
      storage->read(plan, sample.getNames(), storage->getStorageSnapshot(metadata, client_context), query_info, client_context, {},
                    client_context->getSettingsRef()[Setting::max_block_size], getNumberOfCPUCoresToUse());
    ...
    if (have_data_in_stdin && !cancelled) sendDataFromStdin(sample, columns_description_for_query, parsed_query);
...
void ClientBase::sendDataFrom(ReadBuffer &buf, Block &sample, const ColumnsDescription &columns_description, ASTPtr parsed_query,
  ...
  if (const auto *insert = parsed_query->as<ASTInsertQuery>()) {
    if (!insert->format.empty()) current_format = insert->format;
  }

  const Settings &settings = client_context->getSettingsRef();

  /// Setting value from cmd arg overrides one from config.
  size_t insert_format_max_block_size_rows =
      settings[Setting::max_insert_block_size].changed
  ...
  size_t insert_format_max_block_size_bytes =
      settings[Setting::max_insert_block_size_bytes].changed
  ...
  size_t insert_format_min_block_size_rows =
      settings[Setting::min_insert_block_size_rows].changed
          ? settings[Setting::min_insert_block_size_rows]
          : insert_format_min_block_size_rows_from_config.value_or(settings[Setting::min_insert_block_size_rows]);

  size_t insert_format_min_block_size_bytes =
      settings[Setting::min_insert_block_size_bytes].changed
          ? settings[Setting::min_insert_block_size_bytes]
          : insert_format_min_block_size_bytes_from_config.value_or(settings[Setting::min_insert_block_size_bytes]);

  auto source = client_context->getInputFormat(current_format, buf, sample, insert_format_max_block_size_rows, std::nullopt,
                                               insert_format_max_block_size_bytes, insert_format_min_block_size_rows,
...
void ClientBase::sendDataFromPipe(Pipe &&pipe, ASTPtr parsed_query, bool have_more_data) {
  ...
  if (!have_more_data) connection->sendData({}, "", false);
}
...
void ClientBase::cancelQuery() {
  ...
  if (is_interactive) output_stream << "Cancelling query." << std::endl;

  cancelled = true;
}

void ClientBase::processParsedSingleQuery(std::string_view query_, ASTPtr parsed_query, bool &is_async_insert_with_inlined_data,
  ...
  if (const auto *create_user_query = parsed_query->as<ASTCreateUserQuery>()) {
    if (!create_user_query->attach && !create_user_query->authentication_methods.empty()) {
      for (const auto &authentication_method : create_user_query->authentication_methods) {
        ...
        if (password) client_context->getAccessControl().checkPasswordComplexityRules(*password);
      }
    }
  }
...

  {
    ...
    if (!connection->checkConnected(connection_parameters.timeouts)) connect();

    applySettingsFromServerIfNeeded();  // after connect() and applySettingsFromQuery()

    ASTPtr input_function;
    const auto *insert = parsed_query->as<ASTInsertQuery>();
    if (insert && insert->select) insert->tryFindInputFunction(input_function);
  ...
  if (!have_error) {
    if (const auto *set_query = parsed_query->as<ASTSetQuery>()) {
      ...
      for (const auto &change : set_query->changes) {
        if (change.name != "profile") client_context->applySettingChange(change);
      }
      ...
      for (const auto &[name, value] : set_query->query_parameters) query_parameters.insert_or_assign(name, value);
  ...
  if (is_interactive) {
    ...
    if (config.getBool("print-time-to-stderr", false)) error_stream << progress_indication.elapsedSeconds() << "\n";
...
MultiQueryProcessingStage ClientBase::analyzeMultiQueryText(const char *&this_query_begin, const char *&this_query_end,
                                                            const char *all_queries_end, ASTPtr &parsed_query,
                                                            std::unique_ptr<Exception> &current_exception) {
  if (!is_interactive && cancelled) return MultiQueryProcessingStage::QUERIES_END;

  if (this_query_begin >= all_queries_end) return MultiQueryProcessingStage::QUERIES_END;
  ...
  // the tests.
  while (this_query_begin < all_queries_end && isWhitespaceASCII(*this_query_begin)) ++this_query_begin;
  ...
  // and it makes more sense to treat them as such.
  {
    ...
    if (!token_iterator.isValid()) return MultiQueryProcessingStage::QUERIES_END;
  }

  this_query_end = this_query_begin;
  try {
    parsed_query = parseQuery(this_query_end, all_queries_end, client_context->getSettingsRef(),
  ...
  if (!parsed_query) {
    if (ignore_error) {
      Tokens tokens(this_query_begin, all_queries_end);
      IParser::Pos token_iterator(tokens, max_parser_depth, max_parser_backtracks);
      while (token_iterator->type != TokenType::Semicolon && token_iterator.isValid()) ++token_iterator;
  ...
  // - If the INSERT statement FORMAT is VALUES, we use the VALUES format parser to skip the inserted data until we reach the trailing
  // single semicolon.
  // - Other formats (e.g. FORMAT CSV) are arbitrarily more complex and tricky to parse. For example, we may be unable to distinguish if the
  // semicolon
  ...
  if (insert_ast && insert_ast->data) {
    if (insert_ast->format == "Values") {
      ...
      do {
        ...
        if (data_in.eof() || *data_in.position() == ';') break;
      ...
      if (pos_newline != this_query_end) {
        ...
        if (hint.hasClientErrors() || hint.hasServerErrors()) this_query_end = pos_newline;
      }
    } else {
    ...
    switch (stage) {
      ...
      case MultiQueryProcessingStage::PARSING_EXCEPTION: {
        ...
        if (!hint.hasExpectedClientError(current_exception->code()) || buzz_house) {
          if (hint.hasClientErrors()) current_exception->addMessage("\nExpected client error: {}.", hint.clientErrors());
      ...
      case MultiQueryProcessingStage::EXECUTE_QUERY: {
        ...
        auto query_to_execute =
            std::string_view(all_queries_text).substr(this_query_begin - all_queries_text.data(), this_query_end - this_query_begin);
        ...
        if (query_fuzzer_runs) {
          if (!processWithASTFuzzer(full_query)) return false;
        ...
        try {
          processParsedSingleQuery(query_to_execute, parsed_query, is_async_insert_with_inlined_data, insert_query_without_data_length);
        } catch (...) {
        ...
        if (need_retry) {
          ...
          if (test_hint.hasServerErrors() && test_hint.hasClientErrors()) {
            if (server_exception) {
              if (!test_hint.hasExpectedServerError(server_exception->code())) {
                ...
                error_stream << fmt::format("Expected one of the server error codes '{}' but got '{}' (query: {}).\n",
              ...
              if (!test_hint.hasExpectedClientError(client_exception->code())) {
                error_matches_hint = false;
                error_stream << fmt::format("Expected one of the client error codes '{}' but got '{}' (query: {}).\n",
                                            test_hint.clientErrors(), client_exception->code(), full_query);
              }
            } else {
              error_matches_hint = false;
              error_stream << fmt::format("Expected either a server or client error, but got none (query: {}).\n", full_query);
            }
            ...
            if (!server_exception) {
              ...
              error_stream << fmt::format("Expected server error code: {} but got: {} (query: {}).\n", test_hint.serverErrors(),
                                          server_exception->code(), full_query);
            }
            ...
            if (!client_exception) {
              ...
              error_stream << fmt::format("Expected client error code '{}' but got '{}' (query: {}).\n", test_hint.clientErrors(),
                                          client_exception->code(), full_query);
            }
          } else {
          ...
          if (test_hint.hasClientErrors() && test_hint.hasServerErrors()) {
            error_matches_hint = false;
            error_stream << fmt::format("The query succeeded but server or client errors '{}' were expected (query: {}).\n",
                                        test_hint.serverErrors(), full_query);
          } else if (test_hint.hasClientErrors()) {
            error_matches_hint = false;
            error_stream << fmt::format("The query succeeded but the client error '{}' was expected (query: {}).\n",
                                        test_hint.clientErrors(), full_query);
          } else if (test_hint.hasServerErrors()) {
            error_matches_hint = false;
            error_stream << fmt::format("The query succeeded but the server error '{}' was expected (query: {}).\n",
                                        test_hint.serverErrors(), full_query);
          }
        }
        ...
        if (have_error && (buzz_house || !ignore_error)) return buzz_house || is_interactive;

        if (!need_retry) this_query_begin = this_query_end;
        break;
      }
    }
  }
...
bool ClientBase::processQueryText(const String &text) {
  ...
  if (exit_strings.contains(trimmed_input)) return false;

  if (trimmed_input.starts_with("\\i")) {
    size_t skip_prefix_size = std::strlen("\\i");
    auto file_name =
        trim(trimmed_input.substr(skip_prefix_size, trimmed_input.size() - skip_prefix_size), [](char c) { return isWhitespaceASCII(c); });
...
String ClientBase::getPrompt() const { return prompt; }

void ClientBase::initQueryIdFormats() {
  if (!query_id_formats.empty()) return;

  /// Initialize query_id_formats if any
  if (getClientConfiguration().has("query_id_formats")) {
    Poco::Util::AbstractConfiguration::Keys keys;
    getClientConfiguration().keys("query_id_formats", keys);
    for (const auto &name : keys) query_id_formats.emplace_back(name + ":", getClientConfiguration().getString("query_id_formats." + name));
  }

  if (query_id_formats.empty()) query_id_formats.emplace_back("Query id:", " {query_id}\n");
}
...
bool ClientBase::addMergeTreeSettings(ASTCreateQuery &ast_create) {
  ...
  if (all_changed.begin() == all_changed.end()) return false;
...
void ClientBase::applySettingsFromServerIfNeeded() {
  const Settings &settings = client_context->getSettingsRef();
  if (!settings[Setting::apply_settings_from_server]) return;
  ...
  for (const SettingChange &change : settings_from_server) {
    ...
    if (!settings.isChanged(change.name)) changes_to_apply.push_back(change);
  }
...
std::string ClientBase::executeQueryForSingleString(const std::string &query) {
  ...
  try {
    ...
    /// Send the query
    connection->sendQuery(connection_parameters.timeouts, query, {},  /// query_parameters
                          "",                                         /// query_id
...
void ClientBase::addCommonOptions(OptionsDescription &options_description) {
  chassert(!options_description.main_description.has_value(),
  ...
  options_description.main_description->add_options()(
      ...
      ("config-file,C", po::value<std::string>(), "Path to config file")
          ...
          ("proto_caps", po::value<std::string>(),
           ...
           "send_notchunked, send_notchunked_optional, recv_chunked, recv_chunked_optional, recv_notchunked, recv_notchunked_optional")
              ...
              ("query,q", po::value<std::vector<std::string>>()->multitoken(),
               R"(Query. Can be specified multiple times (--query "SELECT 1" --query "SELECT 2") or once with multiple semicolon-separated queries (--query "SELECT 1; SELECT 2;"). In the latter case, INSERT queries with non-VALUE format must be separated by empty lines.)")(
                  ...
                  "multiquery,n", "Obsolete, does nothing")("multiline,m",
                                                            "If specified, allow multi-line queries (Enter does not send the query)")(
                  ...
                  ("history_file", po::value<std::string>(), "Path to a file containing command history")(
                      ...
                      ("stage", po::value<std::string>()->default_value("complete"),
                          ...
                          ("progress",
                           ...
                           "off|0|false|no; DEFAULT - interactive to TTY, non-interactive is off")(
                              ...
                              po::value<ProgressOption>()
                                  ->implicit_value(ProgressOption::TTY, "tty")
                                  ->default_value(ProgressOption::DEFAULT, "default"),
                              "Print a progress table with changing metrics during query execution - to TTY: tty|on|1|true|yes; to STDERR "
                              "non-interactive mode: err; OFF: off|0|false|no; DEFAULT - interactive to TTY, non-interactive is off")(
                              "enable-progress-table-toggle", po::value<bool>()->default_value(true),
                              ...
                              ("disable_suggestion,A",
                               ...
                               "to ClickHouse server. Recommended when pasting queries with TAB characters.")  /// Shorthand -A like in
                                                                                                               /// MySQL client
      ("wait_for_suggestions_to_load", "Load suggestion data synchonously")(
          ...
          ("time,t", "Print query execution time to stderr in non-interactive mode (for benchmarks)")(
              "memory-usage", po::value<std::string>()->implicit_value("default")->default_value("none"),
              "Print memory usage to stderr in non-interactive mode (for benchmarks). Values: 'none', 'default', 'readable'")

              ("echo", "In batch mode, print query before execution")

                  ("log-level", po::value<std::string>(), "Log level")("server_logs_file", po::value<std::string>(),
                                                                       "Write server logs to specified file")
                      ...
                      ("format,f", po::value<std::string>(),
                       "Default input and output format. In clickhouse-client only the default output format.")(
                          ...
                          ("highlight,hilite", po::value<bool>()->default_value(true),
                           "Toggle syntax highlighting in interactive mode (can also use --hilite)")

                              ("ignore-error", "Do not stop processing after an error occurred")("stacktrace",
                                                                                                 "Print stack traces of exceptions")(
                                  "hardware-utilization", "Print hardware utilization information in progress bar")(
                                  "print-profile-events", po::value(&profile_events.print)->zero_tokens(),
                                  "Printing ProfileEvents packets")(
                                  "profile-events-delay-ms", po::value<UInt64>()->default_value(profile_events.delay_ms),
                                  "Delay between printing `ProfileEvents` packets (-1 - print only totals, 0 - print every single packet)")(
                                  ...
                                  ("interactive", "Process queries-file or --query query and start interactive mode")(
                                      ...
                                      ("max_memory_usage_in_client", po::value<std::string>(), "Set memory limit in client/local server")
                                          ...
                                          ("client_logs_file", po::value<std::string>(),
...
void ClientBase::addOptionsToTheClientConfiguration(const CommandLineOptions &options) {
  if (options.contains("verbose")) getClientConfiguration().setBool("verbose", true);

  /// Output execution time to stderr in batch mode.
  if (options.contains("time")) getClientConfiguration().setBool("print-time-to-stderr", true);
  ...
  if (options.contains("query")) queries = options["query"].as<std::vector<std::string>>();
  if (options.contains("query_id")) getClientConfiguration().setString("query_id", options["query_id"].as<std::string>());
  if (options.contains("database")) getClientConfiguration().setString("database", options["database"].as<std::string>());
  if (options.contains("config-file")) getClientConfiguration().setString("config-file", options["config-file"].as<std::string>());
  if (options.contains("queries-file")) {
    if (isEmbeeddedClient())
      throw Exception(ErrorCodes::SUPPORT_IS_DISABLED,
                      "Reading queries from file is not allowed, because the client runs in an embedded mode");
    queries_files = options["queries-file"].as<std::vector<std::string>>();
  }
  if (options.contains("multiline")) getClientConfiguration().setBool("multiline", true);
  if (options.contains("ignore-error")) getClientConfiguration().setBool("ignore-error", true);
  if (options.contains("format")) getClientConfiguration().setString("format", options["format"].as<std::string>());
  if (options.contains("output-format")) getClientConfiguration().setString("output-format", options["output-format"].as<std::string>());
  if (options.contains("vertical")) getClientConfiguration().setBool("vertical", true);
  if (options.contains("stacktrace")) getClientConfiguration().setBool("stacktrace", true);
  if (options.contains("print-profile-events")) getClientConfiguration().setBool("print-profile-events", true);
  ...
  if (options.contains("processed-rows")) getClientConfiguration().setBool("print-num-processed-rows", true);
  ...
  if (options.contains("echo")) getClientConfiguration().setBool("echo", true);
  if (options.contains("disable_suggestion")) getClientConfiguration().setBool("disable_suggestion", true);
  if (options.contains("wait_for_suggestions_to_load")) getClientConfiguration().setBool("wait_for_suggestions_to_load", true);
  if (options.contains("suggestion_limit")) getClientConfiguration().setInt("suggestion_limit", options["suggestion_limit"].as<int>());
  if (options.contains("highlight")) getClientConfiguration().setBool("highlight", options["highlight"].as<bool>());
  ...
  if (options.contains("interactive")) getClientConfiguration().setBool("interactive", true);
  if (options.contains("pager")) {
    if (isEmbeeddedClient())
      throw Exception(ErrorCodes::SUPPORT_IS_DISABLED,
                      "Specifying custom pager is not allowed, because the client runs in an embedded mode");
    getClientConfiguration().setString("pager", options["pager"].as<std::string>());
  }

  if (options.contains("prompt")) getClientConfiguration().setString("prompt", options["prompt"].as<std::string>());

  if (options.contains("oauth-url")) getClientConfiguration().setString("oauth-url", options["oauth-url"].as<std::string>());
  ...
  if (options.contains("log-level")) Poco::Logger::root().setLevel(options["log-level"].as<std::string>());
  if (options.contains("server_logs_file")) {
    if (isEmbeeddedClient())
      throw Exception(ErrorCodes::SUPPORT_IS_DISABLED,
                      "Writing server logs to a file is disabled, because the client runs in an embedded mode");
...
void ClientBase::runInteractive() {
  ...
  if (load_suggestions) {
    ...
    if (client_context->getApplicationType() == Context::ApplicationType::CLIENT)
      suggest->load<Connection>(client_context, connection_parameters, getClientConfiguration().getInt("suggestion_limit"),
      ...
      suggest->load<LocalConnection>(client_context, connection_parameters, getClientConfiguration().getInt("suggestion_limit"),
                                     wait_for_suggestions_to_load);
  }

  if (home_path.empty()) {
    const char *home_path_cstr = getenv("HOME");  // NOLINT(concurrency-mt-unsafe)
    if (home_path_cstr) home_path = home_path_cstr;
  }
  ...
  if (!isEmbeeddedClient()) {
    ...
    if (!history_file.empty() && !fs::exists(history_file)) {
      /// Avoid TOCTOU issue.
      try {
        if (should_create_parent_directories && history_file.has_parent_path()) fs::create_directories(history_file.parent_path());

        FS::createFile(history_file);
      } catch (const ErrnoException &e) {
        if (e.getErrno() != EEXIST) error_stream << getCurrentExceptionMessage(false) << '\n';
      }
    }
...
#else
  lr = LineReader(history_file, getClientConfiguration().has("multiline"), query_extenders, query_delimiters, word_break_characters,
                  input_stream, output_stream, stdin_fd);
#endif
  ...
  do {
    ...
    if (suggest && suggest->getLastError() == ErrorCodes::USER_SESSION_LIMIT_EXCEEDED) {
      ...
      suggest->load(*connection, connection_parameters.timeouts, getClientConfiguration().getInt("suggestion_limit"),
                    client_context->getClientInfo());
    }

    try {
      if (!processQueryText(input)) break;
      last_input = input;
    } catch (const Exception &e) {
      if (e.code() == ErrorCodes::USER_EXPIRED) break;
...
void ClientBase::runNonInteractive() {
  if (delayed_interactive) initQueryIdFormats();
  ...
  if (!buzz_house && !queries_files.empty()) {
    for (const auto &queries_file : queries_files) {
      for (const auto &interleave_file : interleave_queries_files)
        if (!processMultiQueryFromFile(interleave_file)) return;

      if (!processMultiQueryFromFile(queries_file)) return;
    }
  ...
  if (buzz_house) {
    ...
    for (const auto &query : queries) {
      if (query_fuzzer_runs) {
        if (!processWithASTFuzzer(query)) return;
      } else {
        if (!processQueryText(query)) return;
      }
    }
  } else {
...
fs::path ClientBase::getHistoryFilePath() {
  ...
  if (history_file_from_env) return history_file_from_env;
  ...
  if (home_path) {
    ...
    if (fs::exists(path_in_home_dir)) return path_in_home_dir;
  }

  auto xdg_state_home = XDGBaseDirectories::getStateHome();
  if (!xdg_state_home.empty()) return xdg_state_home / "client-query-history";

  throw Exception(ErrorCodes::CANNOT_OPEN_FILE,
                  "Neither $CLICKHOUSE_HISTORY_FILE, $HOME nor $XDG_STATE_HOME is set; cannot place history file.");
}
