...
bool enableSecureConnection(const Poco::Util::AbstractConfiguration& config, const std::string& connection_host,
  ...
  if (config.getBool("secure", false)) return true;

  if (config.getBool("no-secure", false)) return false;

  if (isCloudEndpoint(connection_host)) return true;

  if (connection_port && connection_port.value() == DBMS_DEFAULT_SECURE_PORT) return true;
...
ConnectionParameters::ConnectionParameters(const Poco::Util::AbstractConfiguration& config, const Host& host_, const Database& database,
                                           std::optional<UInt16> port_)
    : host(host_), port(port_.value_or(getPortFromConfig(config, host_))), default_database(database) {
  ...
  if (config.has("jwt")) {
    ...
    if (config.has("ssh-key-passphrase")) {
      ...
      char buf[1000] = {};
      if (auto* result = readpassphrase(prompt.c_str(), buf, sizeof(buf), 0)) passphrase = result;
    }
    ...
    if (config.getBool("ask-password", false)) {
      ...
      if (password == ASK_PASSWORD) password_prompt = true;
    }
    if (password_prompt) {
      std::string prompt{"Password for user (" + user + "): "};
      char buf[1000] = {};
      if (auto* result = readpassphrase(prompt.c_str(), buf, sizeof(buf), 0)) password = result;
    }
    ...
    if (config.has("one-time-password")) {
      ...
      char buf[1000] = {};
      if (auto* result = readpassphrase(prompt.c_str(), buf, sizeof(buf), RPP_ECHO_ON)) password += "+" + std::string(result);
    }
  }
  ...
  compression = config.getBool("compression",
                               host != "localhost" && !isLocalAddress(DNSResolver::instance().resolveHostAllInOriginOrder(host).front()))
                    ? Protocol::Compression::Enable
                    : Protocol::Compression::Disable;

  timeouts =
      ConnectionTimeouts()
          .withConnectionTimeout(Poco::Timespan(config.getInt("connect_timeout", DBMS_DEFAULT_CONNECT_TIMEOUT_SEC), 0))
          .withSendTimeout(Poco::Timespan(config.getInt("send_timeout", DBMS_DEFAULT_SEND_TIMEOUT_SEC), 0))
          .withReceiveTimeout(Poco::Timespan(config.getInt("receive_timeout", DBMS_DEFAULT_RECEIVE_TIMEOUT_SEC), 0))
          .withTCPKeepAliveTimeout(Poco::Timespan(config.getInt("tcp_keep_alive_timeout", DEFAULT_TCP_KEEP_ALIVE_TIMEOUT), 0))
          .withHandshakeTimeout(Poco::Timespan(config.getInt("handshake_timeout_ms", DBMS_DEFAULT_RECEIVE_TIMEOUT_SEC * 1000) * 1000))
          .withSyncRequestTimeout(Poco::Timespan(config.getInt("sync_request_timeout", DBMS_DEFAULT_SYNC_REQUEST_TIMEOUT_SEC), 0));
}
...
UInt16 ConnectionParameters::getPortFromConfig(const Poco::Util::AbstractConfiguration& config, const std::string& connection_host) {
  bool is_secure = enableSecureConnection(config, connection_host);
  return static_cast<UInt16>(
      config.getInt("port", static_cast<UInt16>(config.getInt(is_secure ? "tcp_port_secure" : "tcp_port",
