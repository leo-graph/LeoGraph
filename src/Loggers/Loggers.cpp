...
Poco::AutoPtr<OwnPatternFormatter> getFormatForChannel(Poco::Util::AbstractConfiguration &config, const std::string &channel, bool color) {
  ...
  for (const auto &key : keys) {
    if (key != "formatting" && !key.starts_with("formatting[")) continue;
...
void Loggers::buildLoggers(Poco::Util::AbstractConfiguration &config, Poco::Logger &logger /*_root*/, const std::string &cmd_name) {
  ...
  if (config_logger.has_value() && *config_logger == current_logger) return;
  ...
  if (!log_path_prop.empty()) {
    ...
    std::string ext;
    if (config.getRawString("logger.stream_compress", "false") == "true") ext = ".lz4";
    ...
    log_file->setProperty(Poco::FileChannel::PROP_ROTATION,
                          config.getRawString("logger.rotation", config.getRawString("logger.size", "100M")));
    ...
    split->addChannel(log, "FileLog", log_level, ProfileEvents::AsyncLoggingFileLogTotalMessages,
                      ProfileEvents::AsyncLoggingFileLogDroppedMessages);
  }
  ...
  if (!errorlog_path_prop.empty()) {
    ...
    errorlog->open();
    split->addChannel(errorlog, "ErrorFileLog", errorlog_level, ProfileEvents::AsyncLoggingErrorFileLogTotalMessages,
  ...
  if (config.getBool("logger.use_syslog", false)) {
    ...
    split->addChannel(log, "Syslog", syslog_level, ProfileEvents::AsyncLoggingSyslogTotalMessages,
                      ProfileEvents::AsyncLoggingSyslogDroppedMessages);
  }
  ...
  if (config.getBool("logger.console", false) || (!config.hasProperty("logger.console") && !is_daemon && should_log_to_console)) {
    ...
    split->addChannel(log, "Console", console_log_level, ProfileEvents::AsyncLoggingConsoleTotalMessages,
  ...
  if (allowTextLog() && config.has("text_log")) {
    ...
    if (log_settings.max_size_rows < 1)
      throw DB::Exception(DB::ErrorCodes::BAD_ARGUMENTS, "text_log.max_size_rows {} should be 1 at least", log_settings.max_size_rows);
    ...
    if (log_settings.max_size_rows < log_settings.reserved_size_rows) {
      throw DB::Exception(DB::ErrorCodes::BAD_ARGUMENTS,
                          "text_log.max_size {0} should be greater or equal to text_log.reserved_size_rows {1}", log_settings.max_size_rows,
...
void Loggers::updateLevels(Poco::Util::AbstractConfiguration &config, Poco::Logger &logger) {
  ...
  if (log_file) split->setLevel("FileLog", log_level);
  ...
  for (const auto &name : names) logger.root().get(name).setLevel(max_log_level);
...
void Loggers::closeLogs(Poco::Logger &logger) {
  ...
  if (error_log_file) error_log_file->close();
  // Shouldn't syslog_channel be closed here too?

  if (!log_file) logger.warning("Logging to console but received signal to close log file (ignoring).");
}

void Loggers::flushTextLogs() {
  if (auto *async = dynamic_cast<DB::OwnAsyncSplitChannel *>(split.get())) async->flushTextLogs();
}

DB::AsyncLogQueueSizes Loggers::getAsynchronousMetricsFromAsyncLogs() {
  if (auto *async = dynamic_cast<DB::OwnAsyncSplitChannel *>(split.get())) return async->getAsynchronousMetrics();
  return {};
}
