...
OwnFormattingChannel::OwnFormattingChannel(Poco::AutoPtr<OwnPatternFormatter> pFormatter_, Poco::AutoPtr<Poco::Channel> pChannel_)
    : pFormatter(pFormatter_), pChannel(pChannel_) {
  if (!pFormatter_) throw DB::Exception(ErrorCodes::LOGICAL_ERROR, "Formatter cannot be null");
  if (!pChannel_) throw DB::Exception(ErrorCodes::LOGICAL_ERROR, "Channel cannot be null");
}
