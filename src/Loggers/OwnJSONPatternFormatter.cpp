...
OwnJSONPatternFormatter::OwnJSONPatternFormatter(Poco::Util::AbstractConfiguration& config, const std::string& config_prefix) {
  if (config.has(config_prefix + ".names.date_time")) date_time = config.getString(config_prefix + ".names.date_time", "");

  if (config.has(config_prefix + ".names.date_time_utc")) date_time_utc = config.getString(config_prefix + ".names.date_time_utc", "");

  if (config.has(config_prefix + ".names.thread_name")) thread_name = config.getString(config_prefix + ".names.thread_name", "");

  if (config.has(config_prefix + ".names.thread_id")) thread_id = config.getString(config_prefix + ".names.thread_id", "");

  if (config.has(config_prefix + ".names.level")) level = config.getString(config_prefix + ".names.level", "");

  if (config.has(config_prefix + ".names.query_id")) query_id = config.getString(config_prefix + ".names.query_id", "");

  if (config.has(config_prefix + ".names.logger_name")) logger_name = config.getString(config_prefix + ".names.logger_name", "");

  if (config.has(config_prefix + ".names.message")) message = config.getString(config_prefix + ".names.message", "");

  if (config.has(config_prefix + ".names.source_file")) source_file = config.getString(config_prefix + ".names.source_file", "");

  if (config.has(config_prefix + ".names.source_line")) source_line = config.getString(config_prefix + ".names.source_line", "");

  if (date_time.empty() && thread_name.empty() && thread_id.empty() && level.empty() && query_id.empty() && logger_name.empty() &&
      message.empty() && source_file.empty() && source_line.empty()) {
    date_time = "date_time";
