...
ExtendedLogMessage ExtendedLogMessage::getFrom(const Poco::Message& base) {
  ...
  ::timeval tv;
  if (0 != gettimeofday(&tv, nullptr)) throw ErrnoException(ErrorCodes::CANNOT_GETTIMEOFDAY, "Cannot gettimeofday");
  ...
  if (current_thread) {
    ...
    if (!query_id_ref.empty()) msg_ext.query_id.assign(query_id_ref.data(), query_id_ref.size());
  }
