...
void OwnSplitChannel::open() { stop_logging = false; }

void OwnSplitChannel::close() { stop_logging = true; }

void OwnSplitChannel::log(const Poco::Message &msg) { log(Poco::Message(msg)); }
...
void OwnSplitChannel::log(Poco::Message &&msg) {
  ...
  if (channels.empty() && (logs_queue == nullptr && !logs_queue->isNeeded(msg.getPriority(), msg.getSource()))) return;
...
void pushExtendedMessageToInternalTCPTextLogQueue(const ExtendedLogMessage &msg_ext,
...
void logToSystemTextLogQueue(const std::shared_ptr<SystemLogQueue<TextLogElement>> &text_log_locked, const ExtendedLogMessage &msg_ext,
...
#define SET_VALUE_IF_EXISTS(INDEX) \
  if ((INDEX) <= msg.getFormatStringArgs().size()) (elem.value##INDEX) = msg.getFormatStringArgs()[(INDEX) - 1]
...
void OwnSplitChannel::logSplit(const ExtendedLogMessage &msg_ext, const std::shared_ptr<InternalTextLogsQueue> &logs_queue,
                               ThreadName msg_thread_name) {
  const Poco::Message &msg = *msg_ext.base;
  ...
  try {
    ...
    for (auto &channel : channels | std::views::values) {
      ...
      if (priority >= msg.getPriority()) channel->logExtended(msg_ext);
    }
...
void OwnSplitChannel::addChannel(ChannelPtr channel, const std::string &name, int level, const ProfileEvents::Event &,
...
void OwnSplitChannel::setLevel(const std::string &name, int level) {
  ...
  if (it != channels.end()) it->second->setLevel(level);
}

void OwnSplitChannel::setChannelProperty(const std::string &channel_name, const std::string &name, const std::string &value) {
  auto it = channels.find(channel_name);
  if (it != channels.end()) it->second->setProperty(name, value);
}

OwnAsyncSplitChannel::OwnAsyncSplitChannel(size_t async_queue_size_)
    : async_queue_size(async_queue_size_),
      text_log_queue(async_queue_size_, ProfileEvents::AsyncLoggingTextLogTotalMessages,
                     ProfileEvents::AsyncLoggingTextLogDroppedMessages) {
  if (async_queue_size_ == 0) throw Exception(ErrorCodes::BAD_ARGUMENTS, "Asynchronous log message queue cannot have zero size");
}

OwnAsyncSplitChannel::~OwnAsyncSplitChannel() { OwnAsyncSplitChannel::close(); }
...
class AsyncLogMessage {
 public:
  ALWAYS_INLINE explicit AsyncLogMessage(Message &&msg_)
      : msg(std::move(msg_)), msg_ext(ExtendedLogMessage::getFrom(msg)), msg_thread_name(getThreadName()) {
    if (const auto &masker = SensitiveDataMasker::getInstance()) {
      auto message_text = msg.getText();
      auto matches = masker->wipeSensitiveDataThrow(message_text);
      if (matches > 0) msg.setText(message_text);
    }
  }
...
AsyncLogMessageQueue::AsyncLogMessageQueue(size_t max_size_, const ProfileEvents::Event &event_on_passed_message_,
                                           const ProfileEvents::Event &event_on_drop_message_)
    : event_on_passed_message(event_on_passed_message_), event_on_drop_message(event_on_drop_message_), max_size(max_size_) {
  if (max_size == 0) throw Exception(ErrorCodes::BAD_ARGUMENTS, "Asynchronous log message queue cannot have zero size");
}
...
void AsyncLogMessageQueue::enqueueMessage(AsyncLogMessagePtr message) {
  ...
  if (current_size > max_size / 2) request_flush = true;
...
void OwnAsyncSplitChannel::log(const Poco::Message &msg) { log(Poco::Message(msg)); }
...
void OwnAsyncSplitChannel::log(Poco::Message &&msg) {
  try {
    ...
    auto text_log_max_priority_loaded = text_log_max_priority.load(std::memory_order_relaxed);
    if (channels.empty() && !text_log_max_priority_loaded) return;

    for (size_t i = 0; i < queues.size(); i++) {
      if (channels[i]->getPriority() >= msg_priority) queues[i]->enqueueMessage(notification);
    }

    if (text_log_max_priority_loaded >= msg_priority) text_log_queue.enqueueMessage(std::move(notification));
  } catch (...) {
    const std::string &exception_message = getCurrentExceptionMessage(true);
...
AsyncLogQueueSizes OwnAsyncSplitChannel::getAsynchronousMetrics() {
  ...
  if (text_log.lock()) metrics.push_back({"TextLog", text_log_queue.getCurrentMessageSize()});
...
void OwnAsyncSplitChannel::runTextLog() {
  ...
  auto flush_queue = [&](const std::shared_ptr<SystemLogQueue<TextLogElement>> &text_log_locked) {
    ...
    while (!queue.empty()) {
      auto notif = std::move(queue.front());
      queue.pop_front();
      if (notif) log_notification(notif, text_log_locked);
    }
  };
  ...
  while (is_open) {
    try {
      if (text_log_queue.request_flush) {
        ...
        if (notification) log_notification(notification, text_log_locked);
...
void OwnAsyncSplitChannel::setChannelProperty(const std::string &channel_name, const std::string &name, const std::string &value) {
  if (auto it = name_to_channels.find(channel_name); it != name_to_channels.end()) it->second->setProperty(name, value);
}

void OwnAsyncSplitChannel::addChannel(ChannelPtr channel, const std::string &name, int level,
                                      const ProfileEvents::Event &event_on_passed_message_,
                                      const ProfileEvents::Event &event_on_dropped_message_) {
  if (is_open) throw Exception(ErrorCodes::LOGICAL_ERROR, "Attempted to register channel '{}' while the split channel is open", name);

  auto element = name_to_channels.try_emplace(name, channel);
  if (!element.second) throw Exception(ErrorCodes::BAD_ARGUMENTS, "Channel {} is already registered", name);
  channel->setLevel(level);
...
void OwnAsyncSplitChannel::addTextLog(std::shared_ptr<DB::TextLogQueue> log_queue, int max_priority) {
  if (is_open) throw Exception(ErrorCodes::LOGICAL_ERROR, "Attempted to register channel for text_log while the split channel is open");
  text_log = log_queue;
  text_log_max_priority.store(max_priority, std::memory_order_relaxed);
...
void OwnAsyncSplitChannel::setLevel(const std::string &name, int level) {
  if (auto it = name_to_channels.find(name); it != name_to_channels.end()) it->second->setLevel(level);
}
}  // namespace DB
