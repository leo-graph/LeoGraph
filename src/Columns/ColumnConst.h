...
class ColumnConst final : public COWHelper<IColumnHelper<ColumnConst>, ColumnConst> {
 ...
 public:
  ...
  TypeIndex getDataType() const override { return data->getDataType(); }
  ...
  std::string_view getDataAt(size_t) const override { return data->getDataAt(0); }
  ...
  Float32 getFloat32(size_t) const override { return data->getFloat32(0); }

  bool isDefaultAt(size_t) const override { return data->isDefaultAt(0); }
  ...
  bool tryInsert(const Field &field) override {
    ...
    if (!tmp->tryInsert(field)) return false;
    ++s;
    return true;
  }
  ...
  std::string_view serializeValueIntoArena(size_t, Arena &arena, char const *&begin,
  ...
  void skipSerializedInArena(ReadBuffer &in) const override { data->skipSerializedInArena(in); }

  void updateHashWithValue(size_t, SipHash &hash) const override { data->updateHashWithValue(0, hash); }

  WeakHash32 getWeakHash32() const override;

  void updateHashFast(SipHash &hash) const override { data->updateHashFast(hash); }
  ...
  size_t byteSizeAt(size_t) const override { return data->byteSizeAt(0); }
  ...
  void compareColumn(const IColumn &rhs, size_t rhs_row_num, PaddedPODArray<UInt64> *row_indexes, PaddedPODArray<Int8> &compare_results,
  ...
  void forEachSubcolumn(ColumnCallback callback) const override { callback(data); }
  ...
  void forEachMutableSubcolumn(MutableColumnCallback callback) override { callback(data); }
  ...
  bool structureEquals(const IColumn &rhs) const override {
    if (const auto *rhs_concrete = typeid_cast<const ColumnConst *>(&rhs)) return data->structureEquals(*rhs_concrete->data);
    return false;
  }

  double getRatioOfDefaultRows(double) const override { return data->isDefaultAt(0) ? 1.0 : 0.0; }

  UInt64 getNumberOfDefaultRows() const override { return data->isDefaultAt(0) ? s : 0; }
  ...
  template <typename T>
  T getValue() const {
    return static_cast<T>(getField().safeGet<T>());
  }
