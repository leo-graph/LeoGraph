...
String IColumn::getValueName(size_t n, const Options &options) const {
  ...
  if (options.notFull(name_buf)) return name_buf.str();
...
ColumnPtr IColumn::createWithOffsets(const Offsets &offsets, const ColumnConst &column_with_default_value, size_t total_rows,
                                     size_t shift) const {
  if (offsets.size() + shift != size())
    throw Exception(ErrorCodes::LOGICAL_ERROR, "Incompatible sizes of offsets ({}), shift ({}) and size of column {}", offsets.size(),
                    shift, size());
  ...
  for (size_t i = 0; i < offsets.size(); ++i) {
    ...
    if (offsets_diff > 1) res->insertManyFrom(column_with_default_value.getDataColumn(), 0, offsets_diff - 1);
...
std::string_view IColumn::serializeValueIntoArena(size_t /* n */, Arena & /* arena */, char const *& /* begin */,
...
void IColumn::batchSerializeValueIntoMemory(std::vector<char *> & /* memories */,
                                            const IColumn::SerializationSettings * /* settings */) const {
  throw Exception(ErrorCodes::NOT_IMPLEMENTED, "Method batchSerializeValueIntoMemory is not supported for {}", getName());
}

std::string_view IColumn::serializeValueIntoArenaWithNull(size_t /* n */, Arena & /* arena */, char const *& /* begin */,
                                                          const UInt8 * /* is_null */,
                                                          const IColumn::SerializationSettings * /* settings */) const {
  throw Exception(ErrorCodes::NOT_IMPLEMENTED, "Method serializeValueIntoArenaWithNull is not supported for {}", getName());
}

char *IColumn::serializeValueIntoMemoryWithNull(size_t /* n */, char * /* memory */, const UInt8 * /* is_null */,
                                                const IColumn::SerializationSettings * /* settings */) const {
  throw Exception(ErrorCodes::NOT_IMPLEMENTED, "Method serializeValueIntoMemoryWithNull is not supported for {}", getName());
}

void IColumn::batchSerializeValueIntoMemoryWithNull(std::vector<char *> & /* memories */, const UInt8 * /* is_null */,
                                                    const IColumn::SerializationSettings * /* settings */) const {
...
void IColumn::collectSerializedValueSizes(PaddedPODArray<UInt64> &sizes, const UInt8 *is_null, const SerializationSettings *) const {
  ...
  if (is_null) {
    for (size_t i = 0; i < rows; ++i) sizes[i] += 1 + !is_null[i] * byteSizeAt(i);
  } else {
    for (size_t i = 0; i < rows; ++i) sizes[i] += byteSizeAt(i);
  }
}
...
llvm::Value *IColumn::compileComparator(llvm::IRBuilderBase & /*builder*/, llvm::Value * /*lhs*/, llvm::Value * /*rhs*/,
...
void IColumn::getPermutationWithCollation(const Collator & /*collator*/, PermutationSortDirection /*direction*/,
...
void IColumn::updatePermutationWithCollation(const Collator & /*collator*/, PermutationSortDirection /*direction*/,
                                             PermutationSortStability /*stability*/, size_t /*limit*/, int /*nan_direction_hint*/,
...
bool isColumnNullable(const IColumn &column) { return checkColumn<ColumnNullable>(column); }
...
bool isColumnConst(const IColumn &column) { return checkColumn<ColumnConst>(column); }
...
MutableColumns IColumnHelper<Derived, Parent>::scatter(size_t num_columns, const IColumn::Selector &selector) const {
  ...
  for (auto &column : columns) column = self.cloneEmpty();

  {
    size_t reserve_size = static_cast<size_t>(static_cast<double>(num_rows) * 1.1 /
                                              static_cast<double>(num_columns));  /// 1.1 is just a guess. Better to use n-sigma rule.

    if (reserve_size > 1)
      for (auto &column : columns) column->reserve(reserve_size);
  }

  for (size_t i = 0; i < num_rows; ++i) static_cast<Derived &>(*columns[selector[i]]).insertFrom(*this, i);
...
void compareColumnImpl(const Derived &lhs, const Derived &rhs, size_t rhs_row_num, PaddedPODArray<UInt64> *row_indexes [[maybe_unused]],
                       PaddedPODArray<Int8> &compare_results, int nan_direction_hint) {
  size_t num_rows = lhs.size();
  if (compare_results.empty())
    compare_results.resize(num_rows);
  else if (compare_results.size() != num_rows)
    throw Exception(ErrorCodes::LOGICAL_ERROR, "Size of compare_results: {} doesn't match rows_num: {}", compare_results.size(), num_rows);
...
template <typename Derived, bool reversed>
void compareWithIndexImpl(const Derived &lhs, const Derived &rhs, size_t rhs_row_num, PaddedPODArray<UInt64> *row_indexes [[maybe_unused]],
                          PaddedPODArray<Int8> &compare_results, int nan_direction_hint) {
  size_t num_rows = lhs.size();
  if (compare_results.empty())
    compare_results.resize(num_rows);
...
template <typename Derived, typename Parent>
void IColumnHelper<Derived, Parent>::compareColumn(const IColumn &rhs_base, size_t rhs_row_num, PaddedPODArray<UInt64> *row_indexes,
                                                   PaddedPODArray<Int8> &compare_results, int direction, int nan_direction_hint) const {
...
bool IColumnHelper<Derived, Parent>::hasEqualValues() const {
  ...
  for (size_t i = 1; i < num_rows; ++i) {
    if (self.compareAt(i, 0, self, 1) != 0) return false;
  }
  return true;
}
...
double IColumnHelper<Derived, Parent>::getRatioOfDefaultRows(double sample_ratio) const {
  ...
  if (num_sampled_rows == num_rows || num_rows <= max_number_of_rows_for_full_search) {
    for (size_t i = 0; i < num_rows; ++i) res += self.isDefaultAt(i);
  ...
  if (num_checked_rows == 0) return 0.0;
...
void IColumnHelper<Derived, Parent>::getIndicesOfNonDefaultRows(IColumn::Offsets &indices, size_t from, size_t limit) const {
  ...
  for (size_t i = from; i < to; ++i) {
    if (!self.isDefaultAt(i)) indices.push_back(i);
  }
}
...
static void fillColumnFromRowRefs(ColumnType *col, const DataTypePtr &type, const size_t source_column_index_in_block,
                                  const UInt64 *row_refs_begin, const UInt64 *row_refs_end) {
    ...
    if (*row_ref) {
      ...
      if constexpr (row_refs_are_ranges) {
        ...
        if (const auto *source_replicated = row_ref_list->columns_info->replicated_columns[source_column_index_in_block]) {
          ...
          col->insertRangeFrom(*row_ref_list->columns_info->columns[source_column_index_in_block], row_ref_list->row_num,
                               row_ref_list->rows);
        }
      } else {
...
void IColumn::fillFromRowRefs(const DataTypePtr &type, size_t source_column_index_in_block, const UInt64 *row_refs_begin,
                              const UInt64 *row_refs_end, bool row_refs_are_ranges) {
...
/// Fills column values from RowRefsList
template <typename Derived, typename Parent>
void IColumnHelper<Derived, Parent>::fillFromRowRefs(const DataTypePtr &type, size_t source_column_index_in_block,
                                                     const UInt64 *row_refs_begin, const UInt64 *row_refs_end, bool row_refs_are_ranges) {
...
static void fillColumnFromBlocksAndRowNumbers(ColumnType *col, const DataTypePtr &type, size_t source_column_index_in_block,
...
/// Fills column values from list of blocks and row numbers
void IColumn::fillFromBlocksAndRowNumbers(const DataTypePtr &type, size_t source_column_index_in_block,
                                          ColumnsWithRowNumbers columns_with_row_numbers) {
...
/// Fills column values from list of blocks and row numbers
template <typename Derived, typename Parent>
void IColumnHelper<Derived, Parent>::fillFromBlocksAndRowNumbers(const DataTypePtr &type, size_t source_column_index_in_block,
                                                                 ColumnsWithRowNumbers columns_with_row_numbers) {
  auto &self = static_cast<Derived &>(*this);
...
template <typename Derived, typename Parent>
std::string_view IColumnHelper<Derived, Parent>::serializeValueIntoArenaWithNull(size_t n, Arena &arena, char const *&begin,
                                                                                 const UInt8 *is_null,
                                                                                 const IColumn::SerializationSettings *settings) const {
  const auto &self = static_cast<const Derived &>(*this);
  if (is_null) {
...
template <typename Derived, typename Parent>
std::string_view IColumnHelper<Derived, Parent>::serializeValueIntoArena(size_t n, Arena &arena, char const *&begin,
                                                                         const IColumn::SerializationSettings *settings) const {
  if constexpr (!std::is_base_of_v<ColumnFixedSizeHelper, Derived>) return IColumn::serializeValueIntoArena(n, arena, begin, settings);
...
template <typename Derived, typename Parent>
ALWAYS_INLINE char *IColumnHelper<Derived, Parent>::serializeValueIntoMemoryWithNull(size_t n, char *memory, const UInt8 *is_null,
                                                                                     const IColumn::SerializationSettings *settings) const {
  const auto &self = static_cast<const Derived &>(*this);
  if (is_null) {
    *memory = is_null[n];
    ++memory;
    if (is_null[n]) return memory;
  }
...
template <typename Derived, typename Parent>
void IColumnHelper<Derived, Parent>::batchSerializeValueIntoMemoryWithNull(std::vector<char *> &memories, const UInt8 *is_null,
                                                                           const IColumn::SerializationSettings *settings) const {
  const auto &self = static_cast<const Derived &>(*this);
  ...
  for (size_t i = 0; i < rows; ++i) {
    *memories[i] = is_null[i];
    ++memories[i];
    if (!is_null[i]) memories[i] = self.serializeValueIntoMemory(i, memories[i], settings);
  }
}

template <typename Derived, typename Parent>
ALWAYS_INLINE char *IColumnHelper<Derived, Parent>::serializeValueIntoMemory(size_t n, char *memory,
                                                                             const IColumn::SerializationSettings *settings) const {
  if constexpr (!std::is_base_of_v<ColumnFixedSizeHelper, Derived>) return IColumn::serializeValueIntoMemory(n, memory, settings);
...
template <typename Derived, typename Parent>
void IColumnHelper<Derived, Parent>::batchSerializeValueIntoMemory(std::vector<char *> &memories,
                                                                   const IColumn::SerializationSettings *settings) const {
  const auto &self = static_cast<const Derived &>(*this);
  chassert(memories.size() == self.size());
  for (size_t i = 0; i < self.size(); ++i) memories[i] = self.serializeValueIntoMemory(i, memories[i], settings);
}

template <typename Derived, typename Parent>
void IColumnHelper<Derived, Parent>::collectSerializedValueSizes(PaddedPODArray<UInt64> &sizes, const UInt8 *is_null,
                                                                 const IColumn::SerializationSettings *settings) const {
  if constexpr (!std::is_base_of_v<ColumnFixedSizeHelper, Derived>) return IColumn::collectSerializedValueSizes(sizes, is_null, settings);
  ...
  if (is_null) {
    for (size_t i = 0; i < rows; ++i) sizes[i] += 1 + !is_null[i] * element_size;
  } else {
    for (auto &sz : sizes) sz += element_size;
  }
}
...
static ColumnPtr updateFrom(const Derived &dst, const IColumn::Patch &patch) {
  ...
  if (num_patched_rows == 0) return dst.getPtr();
...
static void assertPatch(const Derived &dst, const IColumn::Patch &patch) {
  if (patch.sources.empty()) throw Exception(ErrorCodes::LOGICAL_ERROR, "Patch has no sources");

  if (patch.dst_row_indices.size() != patch.src_row_indices.size())
    throw Exception(ErrorCodes::LOGICAL_ERROR, "Size of destination indices ({}) doesn't match the size of source indices ({})",
                    patch.dst_row_indices.size(), patch.src_row_indices.size());

  if (patch.dst_versions.size() != dst.size())
    throw Exception(ErrorCodes::LOGICAL_ERROR, "Size of destination versions ({}) doesn't match the size of destination column ({})",
...
ColumnPtr IColumnHelper<Derived, Parent>::updateFrom(const IColumn::Patch &patch) const {
  ...
  if (patch.sources.size() == 1) return updateFrom<true>(dst, patch);
