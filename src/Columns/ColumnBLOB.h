...
/// Used to offload the (de)serialization and (de)compression of columns to the pipeline threads instead of TCPHandler and remote connection
/// threads. Methods `toBLOB` and `fromBLOB` are used to convert between the original column and the BLOB representation. See
/// `MarshallBlocksTransform`, `UnmarshallBlocksTransform`, and `SerializationDetached`.
class ColumnBLOB : public COWHelper<IColumnHelper<ColumnBLOB>, ColumnBLOB> {
 ...
 private:
  ...
  ColumnBLOB(ColumnWithTypeAndName wrapped_column_, CompressionCodecPtr codec, UInt64 client_revision,
             const FormatSettings &format_settings)
      : rows(wrapped_column_.column->size()), wrapped_column(wrapped_column_.column) {
  ...
  ColumnBLOB(FromBLOB task, ColumnPtr wrapped_column_, size_t rows_)
      : rows(rows_), wrapped_column(std::move(wrapped_column_)), from_blob_task(std::move(task)) {
    chassert(wrapped_column);
  }
 ...
 public:
  ...
  static void toBLOB(BLOB &blob, ColumnWithTypeAndName wrapped_column, CompressionCodecPtr codec, UInt64 client_revision,
    ...
    NativeWriter::writeData(*serialization, column_to_write, compressed_buffer, format_settings, 0, column_to_write->size(),
                            client_revision);
    compressed_buffer.finalize();
  }

  /// Decompresses and deserializes the blob into the source column.
  static ColumnPtr fromBLOB(const BLOB &blob, ColumnPtr nested, SerializationPtr nested_serialization, size_t rows,
                            const FormatSettings *format_settings) {
  ...
  void updatePermutation(IColumn::PermutationSortDirection, IColumn::PermutationSortStability, size_t, int, Permutation &,
                         EqualRanges &) const override {
    throwInapplicable();
  }
...
[[nodiscard]] inline Block convertBLOBColumns(const Block &block) {
  ...
  for (const auto &elem : block) {
    ...
    if (const auto *col = typeid_cast<const ColumnBLOB *>(column.column.get())) column.column = col->convertFrom();
