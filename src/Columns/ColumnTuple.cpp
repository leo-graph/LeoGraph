...
ColumnTuple::ColumnTuple(MutableColumns &&mutable_columns) {
  ...
  for (auto &column : mutable_columns) {
    if (isColumnConst(*column)) throw Exception(ErrorCodes::ILLEGAL_COLUMN, "ColumnTuple cannot have ColumnConst as its element");
...
ColumnTuple::Ptr ColumnTuple::create(const Columns &columns) {
  if (columns.empty()) throw Exception(ErrorCodes::LOGICAL_ERROR, "This function cannot be used to construct empty tuple. It is a bug");
...
MutableColumnPtr ColumnTuple::cloneEmpty() const {
  if (columns.empty()) return ColumnTuple::create(0);
...
MutableColumnPtr ColumnTuple::cloneResized(size_t new_size) const {
  if (columns.empty()) return ColumnTuple::create(new_size);

  const size_t tuple_size = columns.size();
  MutableColumns new_columns(tuple_size);
  for (size_t i = 0; i < tuple_size; ++i) new_columns[i] = columns[i]->cloneResized(new_size);
...
void ColumnTuple::get(size_t n, Field &res) const {
  ...
  for (size_t i = 0; i < tuple_size; ++i) res_tuple.push_back((*columns[i])[n]);
}
...
bool ColumnTuple::isDefaultAt(size_t n) const {
  const size_t tuple_size = columns.size();
  for (size_t i = 0; i < tuple_size; ++i)
    if (!columns[i]->isDefaultAt(n)) return false;
  return true;
}
...
void ColumnTuple::insert(const Field &x) {
  ...
  if (tuple.size() != tuple_size)
    throw Exception(ErrorCodes::CANNOT_INSERT_VALUE_OF_DIFFERENT_SIZE_INTO_TUPLE,
                    "Cannot insert value of different size into tuple. Got: {}, expected: {}", tuple.size(), tuple_size);

  ++column_length;

  for (size_t i = 0; i < tuple_size; ++i) columns[i]->insert(tuple[i]);
}

bool ColumnTuple::tryInsert(const Field &x) {
  if (x.getType() != Field::Types::Which::Tuple) return false;

  const auto &tuple = x.safeGet<Tuple>();

  const size_t tuple_size = columns.size();
  if (tuple.size() != tuple_size) return false;

  for (size_t i = 0; i < tuple_size; ++i) {
    if (!columns[i]->tryInsert(tuple[i])) {
      for (size_t j = 0; j != i; ++j) columns[j]->popBack(1);
...
#endif
{
  ...
  if (src.columns.size() != tuple_size)
    throw Exception(ErrorCodes::CANNOT_INSERT_VALUE_OF_DIFFERENT_SIZE_INTO_TUPLE,
                    "Cannot insert value of different size into tuple. Got: {}, expected: {}", src.columns.size(), tuple_size);

  ++column_length;

  for (size_t i = 0; i < tuple_size; ++i) columns[i]->insertFrom(*src.columns[i], n);
}
...
#endif
{
  ...
  if (src_tuple.columns.size() != tuple_size)
    throw Exception(ErrorCodes::CANNOT_INSERT_VALUE_OF_DIFFERENT_SIZE_INTO_TUPLE,
                    "Cannot insert value of different size into tuple. Got: {}, expected: {}", src_tuple.columns.size(), tuple_size);

  for (size_t i = 0; i < tuple_size; ++i) columns[i]->insertManyFrom(*src_tuple.columns[i], position, length);
...
void ColumnTuple::insertDefault() {
  ++column_length;
  for (auto &column : columns) column->insertDefault();
}
...
void ColumnTuple::popBack(size_t n) {
  ...
  column_length -= n;
  for (auto &column : columns) column->popBack(n);
}
...
ColumnCheckpointPtr ColumnTuple::getCheckpoint() const {
  ...
  for (const auto &column : columns) checkpoints.push_back(column->getCheckpoint());
...
void ColumnTuple::updateCheckpoint(ColumnCheckpoint &checkpoint) const {
  ...
  for (size_t i = 0; i < columns.size(); ++i) columns[i]->updateCheckpoint(*checkpoints[i]);
}
...
void ColumnTuple::rollback(const ColumnCheckpoint &checkpoint) {
  ...
  for (size_t i = 0; i < columns.size(); ++i) columns[i]->rollback(*checkpoints[i]);
}

std::string_view ColumnTuple::serializeValueIntoArena(size_t n, Arena &arena, char const *&begin,
...
char *ColumnTuple::serializeValueIntoMemory(size_t n, char *memory, const IColumn::SerializationSettings *settings) const {
  ...
  for (const auto &column : columns) memory = column->serializeValueIntoMemory(n, memory, settings);
...
void ColumnTuple::deserializeAndInsertFromArena(ReadBuffer &in, const IColumn::SerializationSettings *settings) {
  ...
  for (auto &column : columns) column->deserializeAndInsertFromArena(in, settings);
}
...
void ColumnTuple::skipSerializedInArena(ReadBuffer &in) const {
  ...
  for (const auto &column : columns) column->skipSerializedInArena(in);
}

void ColumnTuple::updateHashWithValue(size_t n, SipHash &hash) const {
  for (const auto &column : columns) column->updateHashWithValue(n, hash);
}
...
WeakHash32 ColumnTuple::getWeakHash32() const {
  ...
  for (const auto &column : columns) hash.update(column->getWeakHash32());

  return hash;
}

void ColumnTuple::updateHashFast(SipHash &hash) const {
  for (const auto &column : columns) column->updateHashFast(hash);
}
...
#endif
{
  column_length += length;
  const size_t tuple_size = columns.size();
  for (size_t i = 0; i < tuple_size; ++i) columns[i]->insertRangeFrom(*assert_cast<const ColumnTuple &>(src).columns[i], start, length);
}
...
ColumnPtr ColumnTuple::filter(const Filter &filt, ssize_t result_size_hint) const {
  ...
  for (size_t i = 0; i < tuple_size; ++i) new_columns[i] = columns[i]->filter(filt, result_size_hint);
...
void ColumnTuple::filter(const Filter &filt) {
  ...
  for (size_t i = 0; i < tuple_size; ++i) columns[i]->filter(filt);
...
void ColumnTuple::expand(const Filter &mask, bool inverted) {
  ...
  for (auto &column : columns) column->expand(mask, inverted);
}
...
ColumnPtr ColumnTuple::permute(const Permutation &perm, size_t limit) const {
  if (columns.empty()) {
    ...
    if (perm.size() < result_size)
      throw Exception(ErrorCodes::SIZES_OF_COLUMNS_DOESNT_MATCH, "Size of permutation ({}) is less than required ({})", perm.size(),
                      result_size);
  ...
  for (size_t i = 0; i < tuple_size; ++i) new_columns[i] = columns[i]->permute(perm, limit);
...
ColumnPtr ColumnTuple::index(const IColumn &indexes, size_t limit) const {
  if (columns.empty()) {
    ...
    if (indexes.size() < result_size)
      throw Exception(ErrorCodes::SIZES_OF_COLUMNS_DOESNT_MATCH, "Size of indexes ({}) is less than required ({})", indexes.size(),
                      result_size);
  ...
  for (size_t i = 0; i < tuple_size; ++i) new_columns[i] = columns[i]->index(indexes, limit);
...
ColumnPtr ColumnTuple::replicate(const Offsets &offsets) const {
  ...
  for (size_t i = 0; i < tuple_size; ++i) new_columns[i] = columns[i]->replicate(offsets);
...
void ColumnTuple::getPermutationImpl(IColumn::PermutationSortDirection direction, IColumn::PermutationSortStability stability, size_t limit,
                                     int nan_direction_hint, Permutation &res, const Collator *collator) const {
  size_t rows = size();
  res.resize(rows);
...
void ColumnTuple::updatePermutationImpl(IColumn::PermutationSortDirection direction, IColumn::PermutationSortStability stability,
  ...
  if (equal_ranges.empty()) return;

  for (const auto &column : columns) {
    while (!equal_ranges.empty() && limit && limit <= equal_ranges.back().from) equal_ranges.pop_back();
    ...
    if (equal_ranges.empty()) break;
  }
}

void ColumnTuple::getPermutation(IColumn::PermutationSortDirection direction, IColumn::PermutationSortStability stability, size_t limit,
                                 int nan_direction_hint, Permutation &res) const {
  getPermutationImpl(direction, stability, limit, nan_direction_hint, res, nullptr);
}

void ColumnTuple::updatePermutation(IColumn::PermutationSortDirection direction, IColumn::PermutationSortStability stability, size_t limit,
                                    int nan_direction_hint, IColumn::Permutation &res, EqualRanges &equal_ranges) const {
  updatePermutationImpl(direction, stability, limit, nan_direction_hint, res, equal_ranges);
}

void ColumnTuple::getPermutationWithCollation(const Collator &collator, IColumn::PermutationSortDirection direction,
                                              IColumn::PermutationSortStability stability, size_t limit, int nan_direction_hint,
                                              Permutation &res) const {
  getPermutationImpl(direction, stability, limit, nan_direction_hint, res, &collator);
}

void ColumnTuple::updatePermutationWithCollation(const Collator &collator, IColumn::PermutationSortDirection direction,
                                                 IColumn::PermutationSortStability stability, size_t limit, int nan_direction_hint,
...
void ColumnTuple::reserve(size_t n) {
  const size_t tuple_size = columns.size();
  for (size_t i = 0; i < tuple_size; ++i) getColumn(i).reserve(n);
}

size_t ColumnTuple::capacity() const {
  if (columns.empty()) return size();
...
void ColumnTuple::shrinkToFit() {
  const size_t tuple_size = columns.size();
  for (size_t i = 0; i < tuple_size; ++i) getColumn(i).shrinkToFit();
}

void ColumnTuple::ensureOwnership() {
  const size_t tuple_size = columns.size();
  for (size_t i = 0; i < tuple_size; ++i) getColumn(i).ensureOwnership();
}

size_t ColumnTuple::byteSize() const {
  size_t res = 0;
  for (const auto &column : columns) res += column->byteSize();
  return res;
}

size_t ColumnTuple::byteSizeAt(size_t n) const {
  size_t res = 0;
  for (const auto &column : columns) res += column->byteSizeAt(n);
  return res;
}

size_t ColumnTuple::allocatedBytes() const {
  size_t res = 0;
  for (const auto &column : columns) res += column->allocatedBytes();
  return res;
}

void ColumnTuple::protect() {
  for (auto &column : columns) column->protect();
}
...
void ColumnTuple::getExtremes(Field &min, Field &max, size_t start, size_t end) const {
  ...
  for (size_t i = 0; i < tuple_size; ++i) columns[i]->getExtremes(min_tuple[i], max_tuple[i], start, end);
...
void ColumnTuple::forEachMutableSubcolumn(MutableColumnCallback callback) {
  for (auto &column : columns) callback(column);
}
...
void ColumnTuple::forEachSubcolumn(ColumnCallback callback) const {
  for (const auto &column : columns) callback(column);
}
...
bool ColumnTuple::structureEquals(const IColumn &rhs) const {
  if (const auto *rhs_tuple = typeid_cast<const ColumnTuple *>(&rhs)) {
    const size_t tuple_size = columns.size();
    if (tuple_size != rhs_tuple->columns.size()) return false;

    for (size_t i = 0; i < tuple_size; ++i)
      if (!columns[i]->structureEquals(*rhs_tuple->columns[i])) return false;
...
bool ColumnTuple::isCollationSupported() const {
  for (const auto &column : columns) {
    if (column->isCollationSupported()) return true;
  }
  return false;
}
...
bool ColumnTuple::hasDynamicStructure() const {
  for (const auto &column : columns) {
    if (column->hasDynamicStructure()) return true;
  }
  return false;
}
...
bool ColumnTuple::dynamicStructureEquals(const IColumn &rhs) const {
  if (const auto *rhs_tuple = typeid_cast<const ColumnTuple *>(&rhs)) {
    ...
    for (size_t i = 0; i < tuple_size; ++i)
      if (!columns[i]->dynamicStructureEquals(*rhs_tuple->columns[i])) return false;
...
void ColumnTuple::takeDynamicStructureFromSourceColumns(const Columns &source_columns, std::optional<size_t> max_dynamic_subcolumns) {
  ...
  for (size_t i = 0; i != columns.size(); ++i) nested_source_columns[i].reserve(source_columns.size());

  for (const auto &source_column : source_columns) {
    const auto &nsource_columns = assert_cast<const ColumnTuple &>(*source_column).getColumns();
    for (size_t i = 0; i != nsource_columns.size(); ++i) nested_source_columns[i].push_back(nsource_columns[i]);
  }
...
void ColumnTuple::takeDynamicStructureFromColumn(const ColumnPtr &source_column) {
  ...
  for (size_t i = 0; i != columns.size(); ++i) columns[i]->takeDynamicStructureFromColumn(source_elements[i]);
}

void ColumnTuple::fixDynamicStructure() {
  for (auto &column : columns) column->fixDynamicStructure();
}

ColumnPtr ColumnTuple::compress(bool force_compression) const {
  if (columns.empty()) {
    return ColumnCompressed::create(size(), 0, [n = column_length] { return ColumnTuple::create(n); });
  }
  ...
  return ColumnCompressed::create(size(), byte_size, [my_compressed = std::move(compressed)]() mutable {
    Columns decompressed;
    decompressed.reserve(my_compressed.size());
    for (const auto &column : my_compressed) decompressed.push_back(column->decompress());
