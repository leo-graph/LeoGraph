...
class ColumnString final : public COWHelper<IColumnHelper<ColumnString>, ColumnString> {
 ...
 public:
  ...
  void getValueNameImpl(WriteBufferFromOwnString &name_buf, size_t n, const Options &options) const override {
    if (options.notFull(name_buf)) writeQuoted(std::string_view{reinterpret_cast<const char *>(&chars[offsetAt(n)]), sizeAt(n)}, name_buf);
  }
  ...
  void popBack(size_t n) override {
    if (n > size()) throwCannotPopBack(n, getName(), size());
  ...
  void insertManyDefaults(size_t length) override {
    auto last = offsets.back();
    for (size_t i = 0; i < length; ++i) offsets.push_back(last);
  }
  ...
  void getPermutationWithCollation(const Collator &collator, IColumn::PermutationSortDirection direction,
                                   IColumn::PermutationSortStability stability, size_t limit, int, Permutation &res) const override;

  void updatePermutationWithCollation(const Collator &collator, IColumn::PermutationSortDirection direction,
                                      IColumn::PermutationSortStability stability, size_t limit, int, Permutation &res,
  ...
  bool structureEquals(const IColumn &rhs) const override { return typeid(rhs) == typeid(ColumnString); }
