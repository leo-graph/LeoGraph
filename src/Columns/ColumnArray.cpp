...
ColumnArray::ColumnArray(MutableColumnPtr &&nested_column, MutableColumnPtr &&offsets_column)
  ...
  if (!offsets_concrete) throw Exception(ErrorCodes::LOGICAL_ERROR, "offsets_column must be a ColumnUInt64");
  ...
  if (!offsets_concrete->empty() && data && !data->empty()) {
    ...
    if (data->size() != last_offset)
      throw Exception(ErrorCodes::LOGICAL_ERROR, "offsets_column has data inconsistent with nested_column. Data size: {}, last offset: {}",
...
ColumnArray::ColumnArray(MutableColumnPtr &&nested_column) : data(std::move(nested_column)) {
  if (!data->empty()) throw Exception(ErrorCodes::LOGICAL_ERROR, "Not empty data passed to ColumnArray, but no offsets passed");
...
MutableColumnPtr ColumnArray::cloneResized(size_t to_size) const {
  ...
  if (to_size <= from_size) {
    ...
    for (size_t i = from_size; i < to_size; ++i) res->getOffsets()[i] = offset;
  }

  return res;
}

size_t ColumnArray::size() const { return getOffsets().size(); }
...
void ColumnArray::get(size_t n, Field &res) const {
  ...
  for (size_t i = 0; i < size; ++i) res_arr.push_back(getData()[offset + i]);
}
...
void ColumnArray::getValueNameImpl(WriteBufferFromOwnString &name_buf, size_t n, const Options &options) const {
  ...
  for (size_t i = 0; i < size; ++i) {
    if (options.notFull(name_buf) && i > 0) name_buf << ", ";
...
std::string_view ColumnArray::getDataAt(size_t n) const {
  ...
  if (!data->isFixedAndContiguous()) throw Exception(ErrorCodes::NOT_IMPLEMENTED, "Method getDataAt is not supported for {}", getName());

  size_t array_size = sizeAt(n);
  if (array_size == 0) return {nullptr, 0};
...
void ColumnArray::insertData(const char *pos, size_t length) {
  ...
  if (!data->isFixedAndContiguous()) throw Exception(ErrorCodes::NOT_IMPLEMENTED, "Method insertData is not supported for {}", getName());
  ...
  if (length) {
    ...
    for (; pos + field_size <= end; pos += field_size, ++elems) data->insertData(pos, field_size);

    if (pos != end) throw Exception(ErrorCodes::BAD_ARGUMENTS, "Incorrect length argument for method ColumnArray::insertData");
  }

  getOffsets().push_back(getOffsets().back() + elems);
}

std::string_view ColumnArray::serializeValueIntoArena(size_t n, Arena &arena, char const *&begin,
...
char *ColumnArray::serializeValueIntoMemory(size_t n, char *memory, const IColumn::SerializationSettings *settings) const {
  ...
  for (size_t i = 0; i < array_size; ++i) memory = getData().serializeValueIntoMemory(offset + i, memory, settings);
  return memory;
}
...
void ColumnArray::deserializeAndInsertFromArena(ReadBuffer &in, const IColumn::SerializationSettings *settings) {
  ...
  for (size_t i = 0; i < array_size; ++i) getData().deserializeAndInsertFromArena(in, settings);
...
void ColumnArray::skipSerializedInArena(ReadBuffer &in) const {
  ...
  for (size_t i = 0; i < array_size; ++i) getData().skipSerializedInArena(in);
}
...
void ColumnArray::updateHashWithValue(size_t n, SipHash &hash) const {
  ...
  for (size_t i = 0; i < array_size; ++i) getData().updateHashWithValue(offset + i, hash);
}
...
void ColumnArray::insert(const Field &x) {
  ...
  for (size_t i = 0; i < size; ++i) getData().insert(array[i]);
  getOffsets().push_back(getOffsets().back() + size);
}

bool ColumnArray::tryInsert(const Field &x) {
  if (x.getType() != Field::Types::Which::Array) return false;
...
void ColumnArray::popBack(size_t n) {
  ...
  if (nested_n) getData().popBack(nested_n);
...
struct ColumnArray::ComparatorBase {
  ...
  ComparatorBase(const ColumnArray &parent_, int nan_direction_hint_) : parent(parent_), nan_direction_hint(nan_direction_hint_) {}
...
size_t ColumnArray::capacity() const { return getOffsets().capacity(); }
...
void ColumnArray::ensureOwnership() { getData().ensureOwnership(); }

size_t ColumnArray::byteSize() const { return getData().byteSize() + getOffsets().size() * sizeof(getOffsets()[0]); }
...
size_t ColumnArray::byteSizeAt(size_t n) const {
  ...
  size_t res = sizeof(offsets_data[0]);
  for (; pos < end; ++pos) res += getData().byteSizeAt(pos);

  return res;
}

size_t ColumnArray::allocatedBytes() const { return getData().allocatedBytes() + getOffsets().allocated_bytes(); }
...
#endif
{
  ...
  if (start + length > src_concrete.getOffsets().size())
    throw Exception(ErrorCodes::PARAMETER_OUT_OF_BOUND,
                    "Parameter out of bound in ColumnArray::insertRangeFrom method. "
                    "[start({}) + length({}) > offsets.size({})]",
                    start, length, src_concrete.getOffsets().size());
...
MutableColumnPtr ColumnArray::getDataInRange(size_t start, size_t length) const {
  if (start + length > getOffsets().size())
    throw Exception(ErrorCodes::PARAMETER_OUT_OF_BOUND,
                    "Parameter out of bound in ColumnArray::getDataPtrForRange method. "
                    "[start({}) + length({}) > offsets.size({})]",
                    start, length, getOffsets().size());
...
ColumnPtr ColumnArray::filter(const Filter &filt, ssize_t result_size_hint) const {
  ...
  if (typeid_cast<const ColumnUInt128 *>(data.get())) return filterNumber<UInt128>(filt, result_size_hint);
  if (typeid_cast<const ColumnUInt256 *>(data.get())) return filterNumber<UInt256>(filt, result_size_hint);
  ...
  if (typeid_cast<const ColumnInt128 *>(data.get())) return filterNumber<Int128>(filt, result_size_hint);
  if (typeid_cast<const ColumnInt256 *>(data.get())) return filterNumber<Int256>(filt, result_size_hint);
  if (typeid_cast<const ColumnBFloat16 *>(data.get())) return filterNumber<BFloat16>(filt, result_size_hint);
  ...
  if (typeid_cast<const ColumnDecimal<Decimal32> *>(data.get())) return filterNumber<Decimal32>(filt, result_size_hint);
  if (typeid_cast<const ColumnDecimal<Decimal64> *>(data.get())) return filterNumber<Decimal64>(filt, result_size_hint);
  if (typeid_cast<const ColumnDecimal<Decimal128> *>(data.get())) return filterNumber<Decimal128>(filt, result_size_hint);
  if (typeid_cast<const ColumnDecimal<Decimal256> *>(data.get())) return filterNumber<Decimal256>(filt, result_size_hint);
...
void ColumnArray::expand(const IColumn::Filter &mask, bool inverted) {
  auto &offsets_data = getOffsets();
  if (mask.size() < offsets_data.size()) throw Exception(ErrorCodes::LOGICAL_ERROR, "Mask size should be no less than data size.");
  ...
  while (index >= 0) {
    ...
    if (!!mask[index] ^ inverted) {
      if (from < 0) throw Exception(ErrorCodes::LOGICAL_ERROR, "Too many bytes in mask");
  ...
  if (from != -1) throw Exception(ErrorCodes::LOGICAL_ERROR, "Not enough bytes in mask");
}
...
ColumnPtr ColumnArray::filterNumber(const Filter &filt, ssize_t result_size_hint) const {
  ...
  if (getOffsets().empty()) return ColumnArray::create(data);
...
ColumnPtr ColumnArray::filterGeneric(const Filter &filt, ssize_t result_size_hint) const {
  ...
  Offsets &res_offsets = res->getOffsets();
  if (result_size_hint) res_offsets.reserve_exact(result_size_hint > 0 ? result_size_hint : size);
...
ColumnPtr ColumnArray::filterNullable(const Filter &filt, ssize_t result_size_hint) const {
  ...
  return ColumnArray::create(ColumnNullable::create(filtered_array_of_nested.getDataPtr(), std::move(res_null_map)), filtered_offsets);
}
...
ColumnPtr ColumnArray::filterTuple(const Filter &filt, ssize_t result_size_hint) const {
  ...
  if (tuple_size == 0) return filterGeneric(filt, result_size_hint);

  Columns temporary_arrays(tuple_size);
  for (size_t i = 0; i < tuple_size; ++i)
    temporary_arrays[i] =
        ColumnArray(tuple.getColumns()[i]->assumeMutable(), getOffsetsPtr()->assumeMutable()).filter(filt, result_size_hint);

  Columns tuple_columns(tuple_size);
  for (size_t i = 0; i < tuple_size; ++i) tuple_columns[i] = assert_cast<const ColumnArray &>(*temporary_arrays[i]).getDataPtr();

  return ColumnArray::create(ColumnTuple::create(tuple_columns),
  ...
  for (size_t i = 0; i < col_size; ++i) {
    ...
    if (filt[i]) {
      if (array_size) {
        ...
        for (size_t j = 0; j < array_size; ++j)
          src_string_offsets[j + prev_res_offset] =
...
ColumnPtr ColumnArray::permute(const Permutation &perm, size_t limit) const { return permuteImpl(*this, perm, limit); }

ColumnPtr ColumnArray::index(const IColumn &indexes, size_t limit) const { return selectIndexImpl(*this, indexes, limit); }

template <typename T>
ColumnPtr ColumnArray::indexImpl(const PaddedPODArray<T> &indexes, size_t limit) const {
  assert(limit <= indexes.size());
  if (limit == 0) return ColumnArray::create(data->cloneEmpty());
  ...
  for (size_t i = 0; i < limit; ++i) {
    for (size_t j = 0; j < sizeAt(indexes[i]); ++j) nested_indexes.push_back(offsetAt(indexes[i]) + j);
...
void ColumnArray::getPermutation(PermutationSortDirection direction, PermutationSortStability stability, size_t limit,
...
void ColumnArray::updatePermutation(PermutationSortDirection direction, PermutationSortStability stability, size_t limit,
...
void ColumnArray::getPermutationWithCollation(const Collator &collator, PermutationSortDirection direction,
                                              PermutationSortStability stability, size_t limit, int nan_direction_hint,
                                              Permutation &res) const {
  if (direction == IColumn::PermutationSortDirection::Ascending && stability == IColumn::PermutationSortStability::Unstable)
    getPermutationImpl(limit, res, ComparatorCollationAscendingUnstable(*this, nan_direction_hint, &collator), DefaultSort(),
                       DefaultPartialSort());
  else if (direction == IColumn::PermutationSortDirection::Ascending && stability == IColumn::PermutationSortStability::Stable)
    getPermutationImpl(limit, res, ComparatorCollationAscendingStable(*this, nan_direction_hint, &collator), DefaultSort(),
                       DefaultPartialSort());
  else if (direction == IColumn::PermutationSortDirection::Descending && stability == IColumn::PermutationSortStability::Unstable)
    getPermutationImpl(limit, res, ComparatorCollationDescendingUnstable(*this, nan_direction_hint, &collator), DefaultSort(),
                       DefaultPartialSort());
  else if (direction == IColumn::PermutationSortDirection::Descending && stability == IColumn::PermutationSortStability::Stable)
    getPermutationImpl(limit, res, ComparatorCollationDescendingStable(*this, nan_direction_hint, &collator), DefaultSort(),
                       DefaultPartialSort());
}

void ColumnArray::updatePermutationWithCollation(const Collator &collator, PermutationSortDirection direction,
                                                 PermutationSortStability stability, size_t limit, int nan_direction_hint, Permutation &res,
                                                 EqualRanges &equal_ranges) const {
  auto comparator_equal = ComparatorCollationEqual(*this, nan_direction_hint, &collator);

  if (direction == IColumn::PermutationSortDirection::Ascending && stability == IColumn::PermutationSortStability::Unstable)
    updatePermutationImpl(limit, res, equal_ranges, ComparatorCollationAscendingUnstable(*this, nan_direction_hint, &collator),
                          comparator_equal, DefaultSort(), DefaultPartialSort());
  else if (direction == IColumn::PermutationSortDirection::Ascending && stability == IColumn::PermutationSortStability::Stable)
    updatePermutationImpl(limit, res, equal_ranges, ComparatorCollationAscendingStable(*this, nan_direction_hint, &collator),
                          comparator_equal, DefaultSort(), DefaultPartialSort());
  else if (direction == IColumn::PermutationSortDirection::Descending && stability == IColumn::PermutationSortStability::Unstable)
    updatePermutationImpl(limit, res, equal_ranges, ComparatorCollationDescendingUnstable(*this, nan_direction_hint, &collator),
                          comparator_equal, DefaultSort(), DefaultPartialSort());
  else if (direction == IColumn::PermutationSortDirection::Descending && stability == IColumn::PermutationSortStability::Stable)
    updatePermutationImpl(limit, res, equal_ranges, ComparatorCollationDescendingStable(*this, nan_direction_hint, &collator),
                          comparator_equal, DefaultSort(), DefaultPartialSort());
}
...
ColumnPtr ColumnArray::replicate(const Offsets &replicate_offsets) const {
  if (replicate_offsets.empty() || replicate_offsets.back() == 0) return cloneEmpty();
  ...
  if (typeid_cast<const ColumnUInt128 *>(data.get())) return replicateNumber<UInt128>(replicate_offsets);
  if (typeid_cast<const ColumnUInt256 *>(data.get())) return replicateNumber<UInt256>(replicate_offsets);
  ...
  if (typeid_cast<const ColumnInt128 *>(data.get())) return replicateNumber<Int128>(replicate_offsets);
  if (typeid_cast<const ColumnInt256 *>(data.get())) return replicateNumber<Int256>(replicate_offsets);
  if (typeid_cast<const ColumnBFloat16 *>(data.get())) return replicateNumber<BFloat16>(replicate_offsets);
  ...
  if (typeid_cast<const ColumnDecimal<Decimal32> *>(data.get())) return replicateNumber<Decimal32>(replicate_offsets);
  if (typeid_cast<const ColumnDecimal<Decimal64> *>(data.get())) return replicateNumber<Decimal64>(replicate_offsets);
  if (typeid_cast<const ColumnDecimal<Decimal128> *>(data.get())) return replicateNumber<Decimal128>(replicate_offsets);
  if (typeid_cast<const ColumnDecimal<Decimal256> *>(data.get())) return replicateNumber<Decimal256>(replicate_offsets);
...
ColumnPtr ColumnArray::replicateString(const Offsets &replicate_offsets) const {
  ...
  for (size_t i = 0; i < col_size; ++i) {
    ...
    size_t sum_chars_size =
        src_string_offsets[prev_src_offset + value_size - 1] - prev_src_string_offset;  /// -1th index is Ok, see PaddedPODArray.
    ...
    for (size_t j = 0; j < size_to_replicate; ++j) {
      ...
      if (sum_chars_size) {
        ...
        memcpySmallAllowReadWriteOverflow15(&res_chars[res_chars.size() - sum_chars_size], &src_chars[prev_src_string_offset],
                                            sum_chars_size);
      }
    }
...
ColumnPtr ColumnArray::replicateConst(const Offsets &replicate_offsets) const {
  ...
  if (0 == col_size) return cloneEmpty();
...
ColumnPtr ColumnArray::replicateGeneric(const Offsets &replicate_offsets) const {
  ...
  for (size_t i = 0; i < col_size; ++i) {
    ...
    for (size_t j = 0; j < size_to_replicate; ++j) res_concrete.insertFrom(*this, i);
  }
...
ColumnPtr ColumnArray::replicateNullable(const Offsets &replicate_offsets) const {
  ...
  auto array_of_nested =
      ColumnArray(nullable.getNestedColumnPtr()->assumeMutable(), getOffsetsPtr()->assumeMutable()).replicate(replicate_offsets);
  auto array_of_null_map =
      ColumnArray(nullable.getNullMapColumnPtr()->assumeMutable(), getOffsetsPtr()->assumeMutable()).replicate(replicate_offsets);

  return ColumnArray::create(ColumnNullable::create(assert_cast<const ColumnArray &>(*array_of_nested).getDataPtr(),
...
ColumnPtr ColumnArray::replicateTuple(const Offsets &replicate_offsets) const {
  ...
  if (tuple_size == 0) return replicateGeneric(replicate_offsets);

  Columns temporary_arrays(tuple_size);
  for (size_t i = 0; i < tuple_size; ++i)
    temporary_arrays[i] =
        ColumnArray(tuple.getColumns()[i]->assumeMutable(), getOffsetsPtr()->assumeMutable()).replicate(replicate_offsets);
