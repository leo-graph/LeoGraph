...
ColumnNullable::ColumnNullable(MutableColumnPtr &&nested_column_, MutableColumnPtr &&null_map_)
  ...
  if (isColumnConst(*null_map)) throw Exception(ErrorCodes::ILLEGAL_COLUMN, "ColumnNullable cannot have constant null map");
}

std::string_view ColumnNullable::getDataAt(size_t n) const {
  if (!isNullAt(n)) return getNestedColumn().getDataAt(n);
...
WeakHash32 ColumnNullable::getWeakHash32() const {
  ...
  for (size_t row = 0; row < s; ++row)
    if (null_map_data[row]) hash_data[row] = WeakHash32::kDefaultInitialValue;
...
UInt64 ColumnNullable::getUInt(size_t n) const {
  if (isNullAt(n)) throw Exception(ErrorCodes::BAD_ARGUMENTS, "The value of {} at {} is NULL while calling method getUInt", getName(), n);
  return getNestedColumn().getUInt(n);
}

Int64 ColumnNullable::getInt(size_t n) const {
  if (isNullAt(n)) throw Exception(ErrorCodes::BAD_ARGUMENTS, "The value of {} at {} is NULL while calling method getInt", getName(), n);
...
std::string_view ColumnNullable::serializeValueIntoArena(size_t n, Arena &arena, char const *&begin,
  ...
  *pos = arr[n];
  ...
  if (arr[n]) return std::string_view(pos, 1);
...
std::optional<size_t> ColumnNullable::getSerializedValueSize(size_t n, const IColumn::SerializationSettings *settings) const {
  if (isNullAt(n)) return 1;  /// Just the null mask byte for NULL values.

  auto nested_size = getNestedColumn().getSerializedValueSize(n, settings);
  if (!nested_size) return std::nullopt;
...
void ColumnNullable::skipSerializedInArena(ReadBuffer &in) const {
  ...
  if (val == 0) getNestedColumn().skipSerializedInArena(in);
}
...
bool ColumnNullable::tryInsert(const Field &x) {
  ...
  if (!getNestedColumn().tryInsert(x)) return false;
...
bool ColumnNullable::isComparatorCompilable() const { return nested_column->isComparatorCompilable(); }
...
int ColumnNullable::compareAtImpl(size_t n, size_t m, const IColumn &rhs_, int null_direction_hint, const Collator *collator) const {
  ...
  if (unlikely(lval_is_null || rval_is_null)) {
    if (lval_is_null && rval_is_null) return 0;
    return lval_is_null ? null_direction_hint : -null_direction_hint;
  }

  const IColumn &nested_rhs = nullable_rhs.getNestedColumn();
  if (collator) return getNestedColumn().compareAtWithCollation(n, m, nested_rhs, null_direction_hint, *collator);
...
int ColumnNullable::compareAtWithCollation(size_t n, size_t m, const IColumn &rhs_, int null_direction_hint,
...
void ColumnNullable::getPermutationImpl(IColumn::PermutationSortDirection direction, IColumn::PermutationSortStability stability,
  ...
  if (unlikely(stability == IColumn::PermutationSortStability::Stable)) limit = res_size;
...
void ColumnNullable::updatePermutationImpl(IColumn::PermutationSortDirection direction, IColumn::PermutationSortStability stability,
                                           size_t limit, int null_direction_hint, Permutation &res, EqualRanges &equal_ranges,
                                           const Collator *collator) const {
  ...
  if (is_nulls_last) {
    ...
    for (const auto &[first, last] : equal_ranges) {
      ...
      if (first != write_idx) new_ranges.emplace_back(first, write_idx);

      /// We have a range [write_idx, last) of NULL values
      if (write_idx != last) null_ranges.emplace_back(write_idx, last);
    }
  } else {
    /// Shift all NULL values to the beginning.
    for (const auto &[first, last] : equal_ranges) {
      ...
      if (write_idx != static_cast<ssize_t>(last)) new_ranges.emplace_back(write_idx + 1, last);

      /// We have a range [first, write_idx+1) of NULL values
      if (static_cast<ssize_t>(first) != write_idx) null_ranges.emplace_back(first, write_idx + 1);
    }
  }
...
void ColumnNullable::getPermutation(IColumn::PermutationSortDirection direction, IColumn::PermutationSortStability stability, size_t limit,
...
void ColumnNullable::getPermutationWithCollation(const Collator &collator, IColumn::PermutationSortDirection direction,
...
void ColumnNullable::updatePermutationWithCollation(const Collator &collator, IColumn::PermutationSortDirection direction,
                                                    IColumn::PermutationSortStability stability, size_t limit, int null_direction_hint,
...
size_t ColumnNullable::capacity() const { return getNullMapData().capacity(); }
...
void ColumnNullable::ensureOwnership() { getNestedColumn().ensureOwnership(); }

size_t ColumnNullable::byteSize() const { return getNestedColumn().byteSize() + getNullMapColumn().byteSize(); }

size_t ColumnNullable::byteSizeAt(size_t n) const { return sizeof(getNullMapData()[0]) + getNestedColumn().byteSizeAt(n); }
...
void getExtremesWithNulls(const IColumn &nested_column, const NullMap &null_array, Field &min, Field &max, size_t start, size_t end,
  ...
  if (number_of_nulls == 0) {
    ...
    if (null_last) max = POSITIVE_INFINITY;
  }
}
}  // namespace
...
void ColumnNullable::applyNullMapImpl(const NullMap &map) {
  ...
  if (arr.size() != map.size()) throw Exception(ErrorCodes::LOGICAL_ERROR, "Inconsistent sizes of ColumnNullable objects");

  for (size_t i = 0, size = arr.size(); i < size; ++i) arr[i] |= negative ^ map[i];
}

void ColumnNullable::applyNullMap(const NullMap &map) { applyNullMapImpl<false>(map); }

void ColumnNullable::applyNullMap(const ColumnUInt8 &map) { applyNullMapImpl<false>(map.getData()); }

void ColumnNullable::applyNegatedNullMap(const NullMap &map) { applyNullMapImpl<true>(map); }

void ColumnNullable::applyNegatedNullMap(const ColumnUInt8 &map) { applyNullMapImpl<true>(map.getData()); }
...
void ColumnNullable::checkConsistency() const {
  if (null_map->size() != getNestedColumn().size())
    throw Exception(ErrorCodes::LOGICAL_ERROR,
                    ...
                    null_map->size(), getNestedColumn().size());
}

ColumnPtr ColumnNullable::createWithOffsets(const IColumn::Offsets &offsets, const ColumnConst &column_with_default_value,
                                            size_t total_rows, size_t shift) const {
  if (empty()) return cloneEmpty();
  ...
  if (nullable_column_with_default_value.isNullAt(0)) {
    ...
    new_values = nested_column->createWithOffsets(offsets, *ColumnConst::create(nullable_column_with_default_value.getNestedColumnPtr(), 1),
                                                  total_rows, shift);
...
ColumnPtr ColumnNullable::getNestedColumnWithDefaultOnNull() const {
  ...
  while (start < nested_column->size()) {
    ...
    while (next_null_index < end && !null_map_data[next_null_index]) ++next_null_index;

    if (next_null_index != start) res->insertRangeFrom(*nested_column, start, next_null_index - start);

    size_t next_none_null_index = next_null_index;
    while (next_none_null_index < end && null_map_data[next_none_null_index]) ++next_none_null_index;

    if (next_null_index != next_none_null_index) res->insertManyDefaults(next_none_null_index - next_null_index);
...
ColumnPtr makeNullableOrLowCardinalityNullable(const ColumnPtr &column) {
  if (isColumnNullableOrLowCardinalityNullable(*column)) return column;

  if (isColumnConst(*column))
    return ColumnConst::create(makeNullable(assert_cast<const ColumnConst &>(*column).getDataColumnPtr()), column->size());

  if (column->lowCardinality()) return assert_cast<const ColumnLowCardinality &>(*column).cloneNullable();
...
ColumnPtr makeNullableSafe(const ColumnPtr &column) {
  ...
  if (column->canBeInsideNullable()) return makeNullable(column);
...
ColumnPtr makeNullableOrLowCardinalityNullableSafe(const ColumnPtr &column) {
  ...
  if (isColumnConst(*column))
    return ColumnConst::create(makeNullableOrLowCardinalityNullableSafe(assert_cast<const ColumnConst &>(*column).getDataColumnPtr()),
                               column->size());
...
ColumnPtr removeNullable(const ColumnPtr &column) {
  if (const auto *column_nullable = typeid_cast<const ColumnNullable *>(column.get())) return column_nullable->getNestedColumnPtr();
  return column;
}

ColumnPtr removeNullableOrLowCardinalityNullable(const ColumnPtr &column) {
  if (const auto *column_low_cardinality = typeid_cast<const ColumnLowCardinality *>(column.get())) {
    if (!column_low_cardinality->nestedIsNullable()) return column;
