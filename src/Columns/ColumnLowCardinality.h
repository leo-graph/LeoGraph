...
class ColumnLowCardinality final : public COWHelper<IColumnHelper<ColumnLowCardinality>, ColumnLowCardinality> {
 ...
 public:
  ...
  void updateHashWithValue(size_t n, SipHash &hash) const override { getDictionary().updateHashWithValue(getIndexes().getUInt(n), hash); }
  ...
  ColumnPtr filter(const Filter &filt, ssize_t result_size_hint) const override {
    return ColumnLowCardinality::create(dictionary.getColumnUniquePtr(), getIndexes().filter(filt, result_size_hint), isSharedDictionary());
  }

  void filter(const Filter &filt) override { idx.getIndexesPtr()->filter(filt); }

  void expand(const Filter &mask, bool inverted) override { idx.expand(mask, inverted); }
  ...
  void getPermutation(IColumn::PermutationSortDirection direction, IColumn::PermutationSortStability stability, size_t limit,
                      int nan_direction_hint, Permutation &res) const override;

  void updatePermutation(IColumn::PermutationSortDirection direction, IColumn::PermutationSortStability stability, size_t limit, int,
                         IColumn::Permutation &res, EqualRanges &equal_ranges) const override;

  void getPermutationWithCollation(const Collator &collator, IColumn::PermutationSortDirection direction,
                                   IColumn::PermutationSortStability stability, size_t limit, int nan_direction_hint,
                                   Permutation &res) const override;

  void updatePermutationWithCollation(const Collator &collator, IColumn::PermutationSortDirection direction,
                                      IColumn::PermutationSortStability stability, size_t limit, int nan_direction_hint, Permutation &res,
  ...
  void forEachSubcolumn(ColumnCallback callback) const override {
    ...
    if (!dictionary.isShared()) callback(dictionary.getColumnUniquePtr());
  }
  ...
  double getRatioOfDefaultRows(double sample_ratio) const override { return getIndexes().getRatioOfDefaultRows(sample_ratio); }

  UInt64 getNumberOfDefaultRows() const override { return getIndexes().getNumberOfDefaultRows(); }
  ...
  ALWAYS_INLINE size_t getIndexAt(size_t row) const {
    ...
    switch (idx.getSizeOfIndexType()) {
      case sizeof(UInt8):
        return assert_cast<const ColumnUInt8 *>(indexes)->getElement(row);
      case sizeof(UInt16):
        return assert_cast<const ColumnUInt16 *>(indexes)->getElement(row);
      case sizeof(UInt32):
        return assert_cast<const ColumnUInt32 *>(indexes)->getElement(row);
      case sizeof(UInt64):
        return assert_cast<const ColumnUInt64 *>(indexes)->getElement(row);
      default:
        throwUnexpectedLowCardinalityIndexType(idx.getSizeOfIndexType());
    }
  }
 ...
 private:
  ...
  void getPermutationImpl(IColumn::PermutationSortDirection direction, IColumn::PermutationSortStability stability, size_t limit,
                          int nan_direction_hint, Permutation &res, const Collator *collator = nullptr) const;

  template <typename IndexColumn>
  void updatePermutationWithIndexType(IColumn::PermutationSortStability stability, size_t limit,
