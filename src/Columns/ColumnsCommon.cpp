...
static UInt64 toBits64(const Int8 *bytes64) {
  ...
  UInt64 res =
      ...
      (static_cast<UInt64>(_mm_movemask_epi8(_mm_cmpeq_epi8(_mm_loadu_si128(reinterpret_cast<const __m128i *>(bytes64 + 16)), zero16)))
       << 16) |
      (static_cast<UInt64>(_mm_movemask_epi8(_mm_cmpeq_epi8(_mm_loadu_si128(reinterpret_cast<const __m128i *>(bytes64 + 32)), zero16)))
       << 32) |
      (static_cast<UInt64>(_mm_movemask_epi8(_mm_cmpeq_epi8(_mm_loadu_si128(reinterpret_cast<const __m128i *>(bytes64 + 48)), zero16)))
       << 48);
...
size_t countBytesInFilter(const UInt8 *filt, size_t start, size_t end) {
  ...
  for (; pos < end_pos64; pos += 64) count += std::popcount(toBits64(pos));

  /// TODO Add duff device for tail?
#endif

  for (; pos < end_pos; ++pos) count += *pos != 0;

  return count;
}

size_t countBytesInFilter(const IColumn::Filter &filt) { return countBytesInFilter(filt.data(), 0, filt.size()); }
...
size_t countBytesInFilterWithNull(const IColumn::Filter &filt, const UInt8 *null_map, size_t start, size_t end) {
  ...
  for (; pos < end_pos64; pos += 64, pos2 += 64) count += std::popcount(toBits64(pos) & ~toBits64(pos2));

  /// TODO Add duff device for tail?
#endif

  for (; pos < end_pos; ++pos, ++pos2) count += (*pos & ~*pos2) != 0;
...
bool memoryIsZero(const void *data, size_t start, size_t end) { return memoryIsByte(data, start, end, 0x0); }
...
void throwIndexesSizeTooSmall(size_t indexes_size, size_t limit) {
  throw Exception(ErrorCodes::SIZES_OF_COLUMNS_DOESNT_MATCH, "Size of indexes ({}) is less than required ({})", indexes_size, limit);
}
...
struct ResultOffsetsBuilder {
  ...
  void reserve(ssize_t result_size_hint, size_t src_size) { res_offsets.reserve_exact(result_size_hint > 0 ? result_size_hint : src_size); }
  ...
  void insertChunk(const IColumn::Offset *src_offsets_pos, bool first, IColumn::Offset chunk_offset, size_t chunk_size) {
...
struct InPlaceResultOffsetsBuilder {
  ...
  template <size_t SIMD_BYTES>
  void insertChunk(const IColumn::Offset *src_offsets_pos, IColumn::Offset chunk_offset, size_t chunk_size) {
    /// difference between current and actual offset
    const auto diff_offset = chunk_offset - current_src_offset;
    ...
    if (diff_offset) {
      /// adjust offsets
      for (size_t i = 0; i < SIMD_BYTES; ++i) res_offsets[size + i] -= diff_offset;
    }
...
void filterArraysImplGeneric(const PaddedPODArray<T> &src_elems, const IColumn::Offsets &src_offsets, PaddedPODArray<T> &res_elems,
...
template <typename T>
void filterArraysImpl(const PaddedPODArray<T> &src_elems, const IColumn::Offsets &src_offsets, PaddedPODArray<T> &res_elems,
                      IColumn::Offsets &res_offsets, const IColumn::Filter &filt, ssize_t result_size_hint) {
  return filterArraysImplGeneric<T, ResultOffsetsBuilder>(src_elems, src_offsets, res_elems, &res_offsets, filt, result_size_hint);
}

template <typename T>
void filterArraysImplInPlace(PaddedPODArray<T> &elems, IColumn::Offsets &offsets, const IColumn::Filter &filt) {
  ...
  while (filt_pos < filt_end) {
    if (*filt_pos) copy_array_inplace(offsets_pos);
...
template <typename T>
void filterArraysImplOnlyData(const PaddedPODArray<T> &src_elems, const IColumn::Offsets &src_offsets, PaddedPODArray<T> &res_elems,
...
#define INSTANTIATE(TYPE)                                                                                                                  \
  template void filterArraysImpl<TYPE>(const PaddedPODArray<TYPE> &, const IColumn::Offsets &, PaddedPODArray<TYPE> &, IColumn::Offsets &, \
                                       const IColumn::Filter &, ssize_t);                                                                  \
  template void filterArraysImplOnlyData<TYPE>(const PaddedPODArray<TYPE> &, const IColumn::Offsets &, PaddedPODArray<TYPE> &,             \
                                               const IColumn::Filter &, ssize_t);                                                          \
  template void filterArraysImplInPlace<TYPE>(PaddedPODArray<TYPE> &, IColumn::Offsets &, const IColumn::Filter &);
...
size_t getLimitForPermutation(size_t column_size, size_t perm_size, size_t limit) {
  ...
  if (perm_size < limit)
    throw Exception(ErrorCodes::SIZES_OF_COLUMNS_DOESNT_MATCH, "Size of permutation ({}) is less than required ({})", perm_size, limit);
