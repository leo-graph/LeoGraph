...
ColumnSparse::ColumnSparse(MutableColumnPtr &&values_) : values(std::move(values_)), _size(0) {
  if (!values->empty()) throw Exception(ErrorCodes::LOGICAL_ERROR, "Not empty values passed to ColumnSparse, but no offsets passed");
...
ColumnSparse::ColumnSparse(MutableColumnPtr &&values_, MutableColumnPtr &&offsets_, size_t size_)
  ...
  if (!offsets_concrete) throw Exception(ErrorCodes::LOGICAL_ERROR, "'offsets' column must be a ColumnUInt64, got: {}", offsets->getName());

  /// 'values' should contain one extra element: default value at 0 position.
  if (offsets->size() + 1 != values->size())
    throw Exception(ErrorCodes::LOGICAL_ERROR, "Values size ({}) is inconsistent with offsets size ({})", values->size(), offsets->size());

  if (_size < offsets->size())
    throw Exception(ErrorCodes::LOGICAL_ERROR, "Size of sparse column ({}) cannot be lower than number of non-default values ({})", _size,
                    offsets->size());

  if (!offsets_concrete->empty() && _size <= offsets_concrete->getData().back())
    throw Exception(ErrorCodes::LOGICAL_ERROR, "Size of sparse column ({}) should be greater than last position of non-default value ({})",
  ...
  if (it != offsets_data.end()) throw Exception(ErrorCodes::LOGICAL_ERROR, "Offsets of ColumnSparse must be strictly sorted");
#endif
}

MutableColumnPtr ColumnSparse::cloneResized(size_t new_size) const {
  if (new_size == 0) return ColumnSparse::create(values->cloneEmpty());

  if (new_size >= _size) return ColumnSparse::create(IColumn::mutate(values), IColumn::mutate(offsets), new_size);
...
bool ColumnSparse::isDefaultAt(size_t n) const { return getValueIndex(n) == 0; }

bool ColumnSparse::isNullAt(size_t n) const { return values->isNullAt(getValueIndex(n)); }

Field ColumnSparse::operator[](size_t n) const { return (*values)[getValueIndex(n)]; }

void ColumnSparse::get(size_t n, Field &res) const { values->get(getValueIndex(n), res); }
...
bool ColumnSparse::getBool(size_t n) const { return values->getBool(getValueIndex(n)); }

Float64 ColumnSparse::getFloat64(size_t n) const { return values->getFloat64(getValueIndex(n)); }

Float32 ColumnSparse::getFloat32(size_t n) const { return values->getFloat32(getValueIndex(n)); }

UInt64 ColumnSparse::getUInt(size_t n) const { return values->getUInt(getValueIndex(n)); }

Int64 ColumnSparse::getInt(size_t n) const { return values->getInt(getValueIndex(n)); }

UInt64 ColumnSparse::get64(size_t n) const { return values->get64(getValueIndex(n)); }

std::string_view ColumnSparse::getDataAt(size_t n) const { return values->getDataAt(getValueIndex(n)); }
...
std::string_view ColumnSparse::serializeValueIntoArena(size_t n, Arena &arena, char const *&begin,
...
void ColumnSparse::skipSerializedInArena(ReadBuffer &in) const { values->skipSerializedInArena(in); }
...
bool ColumnSparse::tryInsert(const Field &x) {
  if (!values->tryInsert(x)) return false;
...
void ColumnSparse::insertDefault() { ++_size; }

void ColumnSparse::insertManyDefaults(size_t length) { _size += length; }
...
void ColumnSparse::popBack(size_t n) {
  ...
  if (removed_values) values->popBack(removed_values);
...
void ColumnSparse::expand(const Filter &mask, bool inverted) {
  if (mask.size() < _size) throw Exception(ErrorCodes::LOGICAL_ERROR, "Mask size should be no less than data size.");
  ...
  for (size_t i = 0; i < mask.size(); ++i) {
    if (!!mask[i] ^ inverted) {
      if (it.getCurrentRow() == _size) throw Exception(ErrorCodes::LOGICAL_ERROR, "Too many bytes in mask");

      if (!it.isDefault()) res_offsets_data[it.getCurrentOffset()] = i;
...
ColumnPtr ColumnSparse::permute(const Permutation &perm, size_t limit) const { return permuteImpl(*this, perm, limit); }

ColumnPtr ColumnSparse::index(const IColumn &indexes, size_t limit) const { return selectIndexImpl(*this, indexes, limit); }

template <typename Type>
ColumnPtr ColumnSparse::indexImpl(const PaddedPODArray<Type> &indexes, size_t limit) const {
  assert(limit <= indexes.size());
  if (limit == 0) return ColumnSparse::create(values->cloneEmpty());
  ...
  if (execute_linear) {
    ...
    auto offset_it = begin();
    for (size_t i = 0; i < _size; ++i, ++offset_it) values_index[i] = offset_it.getValueIndex();
...
void ColumnSparse::compareColumn(const IColumn &rhs, size_t rhs_row_num, PaddedPODArray<UInt64> *row_indexes,
  ...
  if (row_indexes || !typeid_cast<const ColumnSparse *>(&rhs)) {
    ...
    values->compareColumn(rhs_sparse.getValuesColumn(), rhs_sparse.getValueIndex(rhs_row_num), nullptr, nested_result, direction,
                          nan_direction_hint);

    const auto &offsets_data = getOffsetsData();
    compare_results.resize(size());
    std::fill(compare_results.begin(), compare_results.end(), nested_result[0]);
    for (size_t i = 0; i < offsets_data.size(); ++i) compare_results[offsets_data[i]] = nested_result[i + 1];
  }
}
...
int ColumnSparse::compareAtWithCollation(size_t n, size_t m, const IColumn &rhs, int null_direction_hint, const Collator &collator) const {
  if (const auto *rhs_sparse = typeid_cast<const ColumnSparse *>(&rhs))
    return values->compareAtWithCollation(getValueIndex(n), rhs_sparse->getValueIndex(m), rhs_sparse->getValuesColumn(),
                                          null_direction_hint, collator);
...
void ColumnSparse::getPermutationImpl(IColumn::PermutationSortDirection direction, IColumn::PermutationSortStability stability,
  ...
  if (limit == 0 || limit > _size) limit = _size;
  ...
  for (size_t i = 0; i < perm.size() && row < limit; ++i) {
    if (perm[i] == 0) {
      if (!num_of_defaults) continue;

      /// Fill the positions of default values in the required quantity.
      auto offset_it = begin();
      while (row < limit) {
        while (offset_it.getCurrentRow() < _size && !offset_it.isDefault()) ++offset_it;

        if (offset_it.getCurrentRow() == _size) break;
...
void ColumnSparse::getPermutation(IColumn::PermutationSortDirection direction, IColumn::PermutationSortStability stability, size_t limit,
                                  int null_direction_hint, Permutation &res) const {
...
void ColumnSparse::updatePermutation(IColumn::PermutationSortDirection direction, IColumn::PermutationSortStability stability, size_t limit,
                                     int null_direction_hint, Permutation &res, EqualRanges &equal_ranges) const {
  auto this_full = convertToFullColumnIfSparse();
  this_full->updatePermutation(direction, stability, limit, null_direction_hint, res, equal_ranges);
}

void ColumnSparse::getPermutationWithCollation(const Collator &collator, IColumn::PermutationSortDirection direction,
                                               IColumn::PermutationSortStability stability, size_t limit, int null_direction_hint,
                                               Permutation &res) const {
  getPermutationImpl(direction, stability, limit, null_direction_hint, res, &collator);
}

void ColumnSparse::updatePermutationWithCollation(const Collator &collator, IColumn::PermutationSortDirection direction,
                                                  IColumn::PermutationSortStability stability, size_t limit, int null_direction_hint,
...
size_t ColumnSparse::byteSize() const { return values->byteSize() + offsets->byteSize() + sizeof(_size); }

size_t ColumnSparse::byteSizeAt(size_t n) const {
  size_t index = getValueIndex(n);
  size_t res = values->byteSizeAt(index);
  if (index) res += sizeof(UInt64);

  return res;
}

size_t ColumnSparse::allocatedBytes() const { return values->allocatedBytes() + offsets->allocatedBytes() + sizeof(_size); }
...
ColumnPtr ColumnSparse::replicate(const Offsets &replicate_offsets) const {
  ...
  if (_size == 0 || replicate_offsets.back() == 0) return ColumnSparse::create(values->cloneEmpty());
...
void ColumnSparse::updateHashWithValue(size_t n, SipHash &hash) const { values->updateHashWithValue(getValueIndex(n), hash); }
...
UInt64 ColumnSparse::getNumberOfDefaultRows() const { return _size - offsets->size(); }
...
bool ColumnSparse::structureEquals(const IColumn &rhs) const {
  if (const auto *rhs_sparse = typeid_cast<const ColumnSparse *>(&rhs)) return values->structureEquals(*rhs_sparse->values);
  return false;
}
...
const IColumn::Offsets &ColumnSparse::getOffsetsData() const { return assert_cast<const ColumnUInt64 &>(*offsets).getData(); }

IColumn::Offsets &ColumnSparse::getOffsetsData() { return assert_cast<ColumnUInt64 &>(*offsets).getData(); }
...
ColumnPtr recursiveRemoveSparse(const ColumnPtr &column) {
  ...
  if (const auto *column_tuple = typeid_cast<const ColumnTuple *>(column.get())) {
    ...
    if (columns.empty()) return column;

    for (auto &element : columns) element = recursiveRemoveSparse(element);
