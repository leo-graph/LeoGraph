...
struct ColumnVector<T>::less_stable {
  ...
  bool operator()(size_t lhs, size_t rhs) const {
    if (unlikely(parent.data[lhs] == parent.data[rhs])) return lhs < rhs;
...
template <typename T>
void moveNanToRequestedSide(typename ColumnVector<T>::Permutation::iterator begin, typename ColumnVector<T>::Permutation::iterator end,
                            const typename ColumnVector<T>::Container &data, size_t data_size, bool reverse, int nan_direction_hint) {
  ...
  if (nans_to_move) std::rotate(begin, begin + (is_nulls_last ? nans_to_move : data_size - nans_to_move), end);
}
...
MULTITARGET_FUNCTION_X86_V4_V3(MULTITARGET_FUNCTION_HEADER(template <typename T> void), compareColumnImpl,
                               MULTITARGET_FUNCTION_BODY((const typename ColumnVector<T>::Container &data, T value,
                                                          PaddedPODArray<Int8> &compare_results, int direction, int nan_direction_hint) {
...
template <typename T>
void ColumnVector<T>::compareColumn(const IColumn &rhs, size_t rhs_row_num, PaddedPODArray<UInt64> *row_indexes,
...
template <typename T>
void ColumnVector<T>::getPermutation(IColumn::PermutationSortDirection direction, IColumn::PermutationSortStability stability, size_t limit,
  ...
  if constexpr (is_arithmetic_v<T> && !is_big_int_v<T>) {
    if (!limit) {
      ...
      if (data_size >= 256 && data_size <= std::numeric_limits<UInt32>::max() && use_radix_sort) {
        ...
        for (UInt32 i = 0; i < static_cast<UInt32>(data_size); ++i) pairs[i] = {data[i], i};

        RadixSort<RadixSortTraits<T>>::executeLSD(pairs.data(), data_size, reverse, res.data());
        if constexpr (is_floating_point<T>) moveNanToRequestedSide<T>(res.begin(), res.end(), data, data_size, reverse, nan_direction_hint);
...
void ColumnVector<T>::updatePermutation(IColumn::PermutationSortDirection direction, IColumn::PermutationSortStability stability,
  ...
  auto sort = [&](auto begin, auto end, auto pred) {
    ...
    if constexpr (is_arithmetic_v<T> && !is_big_int_v<T>) {
      ...
      if (range_size >= 256 && range_size <= std::numeric_limits<UInt32>::max() && use_radix_sort) {
        ...
        if constexpr (is_floating_point<T>) moveNanToRequestedSide<T>(begin, end, data, range_size, reverse, nan_direction_hint);
  ...
  if (direction == IColumn::PermutationSortDirection::Ascending && stability == IColumn::PermutationSortStability::Unstable) {
    this->updatePermutationImpl(limit, res, equal_ranges, less(*this, nan_direction_hint), equals(*this, nan_direction_hint), sort,
                                partial_sort);
  } else if (direction == IColumn::PermutationSortDirection::Ascending && stability == IColumn::PermutationSortStability::Stable) {
    this->updatePermutationImpl(limit, res, equal_ranges, less_stable(*this, nan_direction_hint), equals(*this, nan_direction_hint), sort,
                                partial_sort);
  } else if (direction == IColumn::PermutationSortDirection::Descending && stability == IColumn::PermutationSortStability::Unstable) {
    this->updatePermutationImpl(limit, res, equal_ranges, greater(*this, nan_direction_hint), equals(*this, nan_direction_hint), sort,
                                partial_sort);
  } else if (direction == IColumn::PermutationSortDirection::Descending && stability == IColumn::PermutationSortStability::Stable) {
    this->updatePermutationImpl(limit, res, equal_ranges, greater_stable(*this, nan_direction_hint), equals(*this, nan_direction_hint),
                                sort, partial_sort);
  }
}
...
size_t ColumnVector<T>::estimateCardinalityInPermutedRange(const IColumn::Permutation &permutation, const EqualRange &equal_range) const {
  ...
  if (range_size <= 1) return range_size;
...
MutableColumnPtr ColumnVector<T>::cloneResized(size_t size) const {
  ...
  if (size > 0) {
    ...
    if (size > count) memset(static_cast<void *>(&new_col.data[count]), 0, (size - count) * sizeof(ValueType));
  }
...
void ColumnVector<T>::getValueNameImpl(WriteBufferFromOwnString &name_buf, size_t n, const IColumn::Options &options) const {
  ...
  if (options.notFull(name_buf)) name_buf << FieldVisitorToString()(castToNearestFieldType(data[n]));
}
...
uint8_t prefixToCopy(UInt64 mask) {
  if (mask == 0) return 0;
  if (mask == static_cast<UInt64>(-1)) return 64;
  ...
  if (mask == ((static_cast<UInt64>(-1) << leading_zeroes) >> leading_zeroes)) return static_cast<uint8_t>(64 - leading_zeroes);
  return 0xFF;
}
...
DECLARE_DEFAULT_CODE(template <typename T, typename Inserter, size_t SIMD_ELEMENTS>
                     inline void doFilterAligned(const UInt8 *&filt_pos, const UInt8 *&filt_end_aligned, const T *&data_pos,
                                                 Inserter &inserter) {
...
void resize(Container &res_data, size_t reserve_size) {
  ...
  res_data.resize_fill(reserve_size,
                       static_cast<T>(0));  // MSan doesn't recognize that all allocated memory is written by AVX-512 intrinsics.
#else
...
ColumnPtr ColumnVector<T>::filter(const IColumn::Filter &filt, ssize_t result_size_hint) const {
  ...
  if (result_size_hint) res_data.reserve_exact(result_size_hint > 0 ? result_size_hint : size);
...
void ColumnVector<T>::filter(const IColumn::Filter &filt) {
  ...
  while (filt_pos < filt_end) {
    if (*filt_pos) result_data[result_size++] = *data_pos;
...
MULTITARGET_FUNCTION_X86_V4_V3(
    MULTITARGET_FUNCTION_HEADER(
        template <typename ValueType, bool use_window,
                  int padding_elements = std::min(size_t(4), ColumnVector<ValueType>::Container::pad_right / sizeof(ValueType))>
        void),
    replicateImpl,
    MULTITARGET_FUNCTION_BODY((const ValueType *__restrict data, size_t size, [[maybe_unused]] size_t window_size,
                               const IColumn::Offsets &offsets, ValueType *__restrict result)  /// NOLINT
                              {
                                auto *it = result;
                                ...
                                if constexpr (use_window && padding_elements >= 2) {
                                  for (size_t i = 0; i < size; ++i) {
                                    ...
                                    if (!span_size) continue;
                                    /// We will do block writes of "padding_elements" size from left to write, so writing more bytes than
                                    /// necessary is ok as the data will be overwritten by the next offset (or it's part of the padding)
                                    ...
                                    for (size_t copy_iteration = 0; copy_iteration < iterations; copy_iteration++) {
                                      std::fill(it + copy_iteration * padding_elements, it + (copy_iteration + 1) * padding_elements,
                                                data[i]);
                                    }
                                    it = result + offsets[i];

                                    if constexpr (use_window)
                                      if (i + window_size - 1 < size && offsets[i] == offsets[i + window_size - 1]) i += window_size - 1;
                                  }
                                } else {
                                  for (size_t i = 0; i < size; ++i) {
...
ColumnPtr ColumnVector<T>::replicate(const IColumn::Offsets &offsets) const {
  ...
  if (size == 0 || offsets.back() == 0) return this->create();

  auto res = this->create(offsets.back());

  /// This formula provides the optimum for a very simplified and probably wrong model for the number of additional checks (offsets[i] ==
  /// offsets[i + window - 1]) The threshold of 16 is chosen experimentally, based on the case when all offsets are 0 and we spend no time
  /// doing actual copying (i.e. the overhead from these additional checks is pronounced the most).
...
ColumnPtr ColumnVector<T>::compress(bool force_compression) const {
  ...
  if (!compressed) return ColumnCompressed::wrap(this->getPtr());

  const size_t compressed_size = compressed->size();
  return ColumnCompressed::create(data_size, compressed_size, [my_compressed = std::move(compressed), column_size = data_size] {
    auto res = ColumnVector<T>::create(column_size);
    ColumnCompressed::decompressBuffer(my_compressed->data(), res->getData().data(), my_compressed->size(), column_size * sizeof(T));
    return res;
  });
}

template <typename T>
ColumnPtr ColumnVector<T>::createWithOffsets(const IColumn::Offsets &offsets, const ColumnConst &column_with_default_value,
...
DECLARE_DEFAULT_CODE(template <typename Container, typename Type>
...
DECLARE_X86_ICELAKE_SPECIFIC_CODE(template <typename Container, typename Type>
                                  __attribute__((no_sanitize("memory")))  /// False positive on _mm512_permutex2var_epi8
                                  void vectorIndexImpl(const Container &data, const PaddedPODArray<Type> &indexes, size_t limit,
                                                       Container &res_data) {
                                    ...
                                    if (limit == 0) return;  /// nothing to do, just return
