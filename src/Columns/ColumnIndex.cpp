...
ColumnIndex::ColumnIndex(MutableColumnPtr &&indexes_) : indexes(std::move(indexes_)) { updateSizeOfType(); }

ColumnIndex::ColumnIndex(ColumnPtr indexes_) : indexes(std::move(indexes_)) { updateSizeOfType(); }
...
size_t ColumnIndex::getSizeOfIndexType(const IColumn &column, size_t hint) {
  ...
  if (auto size = try_get_size_for(UInt8())) return size;
  if (auto size = try_get_size_for(UInt16())) return size;
  if (auto size = try_get_size_for(UInt32())) return size;
  if (auto size = try_get_size_for(UInt64())) return size;

  throw Exception(ErrorCodes::ILLEGAL_COLUMN, "Unexpected indexes type for ColumnLowCardinality. Expected UInt, got {}", column.getName());
}
...
void ColumnIndex::convertIndexes() {
  auto convert = [&]<typename CurIndexType>(CurIndexType /*type_value*/) {
    ...
    if (sizeof(CurIndexType) > sizeof(IndexType))
      throw Exception(ErrorCodes::LOGICAL_ERROR, "Converting indexes to smaller type: from {} to {}", sizeof(CurIndexType),
                      sizeof(IndexType));
    ...
    if (sizeof(CurIndexType) != sizeof(IndexType)) {
      ...
      for (size_t i = 0; i < size; ++i) new_data[i] = static_cast<IndexType>(data[i]);
...
void ColumnIndex::insertIndex(size_t index) {
  while (index > getMaxIndexForCurrentType()) expandType();
...
void ColumnIndex::insertIndexesRange(const IColumn &column, size_t offset, size_t limit) {
  auto insert_for_type = [&](auto type) {
    ...
    if (size_of_type < sizeof(ColumnType)) convertIndexes<ColumnType>();
    ...
    if (size_of_type == sizeof(ColumnType))
      ...
      auto copy = [&](auto cur_type) {
        ...
        for (size_t i = 0; i < limit; ++i) indexes_data[size + i] = static_cast<CurIndexType>(column_data[offset + i]);
      };
  ...
  if (!insert_for_type(UInt8()) && !insert_for_type(UInt16()) && !insert_for_type(UInt32()) && !insert_for_type(UInt64()))
    throw Exception(ErrorCodes::ILLEGAL_COLUMN, "Invalid column for ColumnLowCardinality index. Expected UInt, got {}", column.getName());
...
void ColumnIndex::insertIndexesRangeWithShift(const IColumn &column, size_t offset, size_t limit, size_t shift, size_t max_result_index) {
  while (max_result_index > getMaxIndexForCurrentType()) expandType();
...
void ColumnIndex::callForIndexes(std::function<void(size_t, size_t)> &&callback, size_t start, size_t end) const {
  auto callback_for_type = [&](auto cur_type) {
    ...
    for (size_t i = start; i != end; ++i) callback(i, data[i]);
  };
...
ColumnPtr ColumnIndex::removeUnusedRowsInIndexedData(const ColumnPtr &indexed_data) {
  ...
  auto create_filter = [&]<typename CurIndexType>(CurIndexType /*type_value*/) {
    const auto &data = getIndexesData<CurIndexType>();
    for (size_t i = 0; i != data.size(); ++i) filter[data[i]] = 1;
  };
  ...
  auto adjust_indexes = [&]<typename CurIndexType>(CurIndexType /*type_value*/) {
    ...
    for (size_t i = 0; i != filter.size(); ++i) {
      if (filter[i]) indexes_remapping[i] = static_cast<CurIndexType>(new_index++);
    }
    auto &data = getIndexesData<CurIndexType>();
    for (size_t i = 0; i != data.size(); ++i) data[i] = indexes_remapping[data[i]];
...
void ColumnIndex::getIndexesByMask(IColumn::Offsets &result_indexes, const PaddedPODArray<UInt8> &mask, size_t start, size_t end) const {
  auto create_filter = [&](auto cur_type) {
    ...
    for (size_t i = start; i != end; ++i) {
      if (mask[data[i]]) result_indexes.push_back(i);
    }
  };
...
void ColumnIndex::insertIndexesRange(size_t start, size_t length) {
  ...
  while (max_index > getMaxIndexForCurrentType()) expandType();

  auto insert = [&]<typename CurIndexType>(CurIndexType /*type_value*/) {
    auto &indexes_data = getIndexesData<CurIndexType>();
    indexes_data.reserve(indexes_data.size() + length);
    size_t end = start + length;
    for (size_t index = start; index != end; ++index) indexes_data.push_back(static_cast<CurIndexType>(index));
  };
...
void ColumnIndex::checkSizeOfType() {
  if (size_of_type != getSizeOfIndexType(*indexes, size_of_type))
    throw Exception(ErrorCodes::LOGICAL_ERROR, "Invalid size of type. Expected {}, but indexes are {}", 8 * size_of_type,
                    indexes->getName());
}
...
WeakHash32 ColumnIndex::getWeakHash(const WeakHash32 &dict_hash) const {
  ...
  auto update_weak_hash = [&](auto x) {
    ...
    for (size_t i = 0; i < size; ++i) hash_data[i] = dict_hash_data[data[i]];
  };
...
void ColumnIndex::collectSerializedValueSizes(PaddedPODArray<UInt64> &sizes, const PaddedPODArray<UInt64> &dict_sizes) const {
  auto func = [&](auto x) {
    using CurIndexType = decltype(x);
    auto &data = getIndexesData<CurIndexType>();

    size_t rows = sizes.size();
    for (size_t i = 0; i < rows; ++i) sizes[i] += dict_sizes[data[i]];
  };
  callForType(std::move(func), size_of_type);
}

void ColumnIndex::expand(const IColumn::Filter &mask, bool inverted) { indexes->expand(mask, inverted); }
