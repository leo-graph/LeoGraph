...
class ColumnSparse final : public COWHelper<IColumnHelper<ColumnSparse>, ColumnSparse> {
 ...
 public:
  ...
  void getPermutationImpl(IColumn::PermutationSortDirection direction, IColumn::PermutationSortStability stability, size_t limit,
                          int null_direction_hint, Permutation &res, const Collator *collator) const;
  ...
  size_t getNumberOfTrailingDefaults() const { return offsets->empty() ? _size : _size - getOffsetsData().back() - 1; }
  ...
  class Iterator {
   public:
    ...
    bool operator==(const Iterator &other) const {
      return size == other.size && current_offset == other.current_offset && current_row == other.current_row;
    }

    bool operator!=(const Iterator &other) const { return !(*this == other); }

    Iterator operator++() {
      if (!isDefault()) ++current_offset;
      ++current_row;
      return *this;
    }
