...
ColumnConst::ColumnConst(const ColumnPtr &data_, size_t s_) : data(data_), s(s_) {
  ...
  while (const ColumnConst *const_data = typeid_cast<const ColumnConst *>(data.get())) data = const_data->getDataColumnPtr();
...
void ColumnConst::expand(const Filter &mask, bool inverted) {
  if (mask.size() < s) throw Exception(ErrorCodes::LOGICAL_ERROR, "Mask size should be no less than data size.");

  size_t bytes_count = countBytesInFilter(mask);
  if (inverted) bytes_count = mask.size() - bytes_count;

  if (bytes_count < s) throw Exception(ErrorCodes::LOGICAL_ERROR, "Not enough bytes in mask");
  if (bytes_count > s) throw Exception(ErrorCodes::LOGICAL_ERROR, "Too many bytes in mask");
...
void ColumnConst::popBack(size_t n) {
  if (n > s) throw Exception(ErrorCodes::LOGICAL_ERROR, "Cannot pop {} rows from {}: there are only {} rows", n, getName(), size());
...
void ColumnConst::getPermutation(PermutationSortDirection /*direction*/, PermutationSortStability /*stability*/, size_t /*limit*/,
                                 int /*nan_direction_hint*/, Permutation &res) const {
  res.resize_exact(s);
  iota(res.data(), s, IColumn::Permutation::value_type(0));
}

void ColumnConst::updatePermutation(PermutationSortDirection /*direction*/, PermutationSortStability /*stability*/, size_t, int,
                                    Permutation &, EqualRanges &) const {}
...
void ColumnConst::compareColumn(const IColumn &rhs, size_t, PaddedPODArray<UInt64> *, PaddedPODArray<Int8> &compare_results, int,
