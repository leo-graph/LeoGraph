...
Field IColumnDummy::operator[](size_t) const { throw Exception(ErrorCodes::NOT_IMPLEMENTED, "Cannot get value from {}", getName()); }

void IColumnDummy::get(size_t, Field &) const { throw Exception(ErrorCodes::NOT_IMPLEMENTED, "Cannot get value from {}", getName()); }
...
void IColumnDummy::insert(const Field &) { throw Exception(ErrorCodes::NOT_IMPLEMENTED, "Cannot insert element into {}", getName()); }
...
std::string_view IColumnDummy::serializeValueIntoArena(size_t /*n*/, Arena &arena, char const *&begin,
...
void IColumnDummy::skipSerializedInArena(ReadBuffer &in) const { in.ignore(1); }
...
void IColumnDummy::filter(const Filter &filt) { s = countBytesInFilter(filt); }

void IColumnDummy::expand(const IColumn::Filter &mask, bool) { s = mask.size(); }

ColumnPtr IColumnDummy::permute(const Permutation &perm, size_t limit) const {
  if (s != perm.size()) throw Exception(ErrorCodes::SIZES_OF_COLUMNS_DOESNT_MATCH, "Size of permutation doesn't match size of column.");

  return cloneDummy(limit ? std::min(s, limit) : s);
}

ColumnPtr IColumnDummy::index(const IColumn &indexes, size_t limit) const {
  if (indexes.size() < limit) throw Exception(ErrorCodes::SIZES_OF_COLUMNS_DOESNT_MATCH, "Size of indexes is less than required.");
...
ColumnPtr IColumnDummy::replicate(const Offsets &offsets) const {
  if (s != offsets.size()) throw Exception(ErrorCodes::SIZES_OF_COLUMNS_DOESNT_MATCH, "Size of offsets doesn't match size of column.");

  return cloneDummy(offsets.back());
}

MutableColumns IColumnDummy::scatter(size_t num_columns, const Selector &selector) const {
  if (s != selector.size()) throw Exception(ErrorCodes::SIZES_OF_COLUMNS_DOESNT_MATCH, "Size of selector doesn't match size of column.");
