...
ColumnObject::ColumnObject(std::unordered_map<String, MutableColumnPtr> typed_paths_,
                           std::unordered_map<String, MutableColumnPtr> dynamic_paths_, MutableColumnPtr shared_data_,
                           size_t max_dynamic_paths_, size_t global_max_dynamic_paths_, size_t max_dynamic_types_,
...
ColumnObject::ColumnObject(std::unordered_map<String, MutableColumnPtr> typed_paths_, size_t max_dynamic_paths_, size_t max_dynamic_types_)
    : max_dynamic_paths(max_dynamic_paths_), global_max_dynamic_paths(max_dynamic_paths_), max_dynamic_types(max_dynamic_types_) {
  typed_paths.reserve(typed_paths_.size());
  sorted_typed_paths.reserve(typed_paths_.size());
  for (auto &[path, column] : typed_paths_) {
    if (!column->empty()) throw Exception(ErrorCodes::LOGICAL_ERROR, "Unexpected non-empty typed path column in ColumnObject constructor");
    auto it = typed_paths.emplace(path, std::move(column)).first;
    sorted_typed_paths.push_back(it->first);
  }
...
ColumnObject::ColumnObject(const ColumnObject &other)
  ...
  for (const auto &[path, _] : typed_paths) sorted_typed_paths.emplace_back(path);
  std::sort(sorted_typed_paths.begin(), sorted_typed_paths.end());

  sorted_dynamic_paths.clear();
  for (const auto &[path, _] : dynamic_paths) sorted_dynamic_paths.emplace(path);
}

ColumnObject::Ptr ColumnObject::create(const std::unordered_map<String, ColumnPtr> &typed_paths_,
                                       const std::unordered_map<String, ColumnPtr> &dynamic_paths_, const ColumnPtr &shared_data_,
                                       size_t max_dynamic_paths_, size_t global_max_dynamic_paths_, size_t max_dynamic_types_,
  ...
  for (const auto &[path, column] : typed_paths_) mutable_typed_paths[path] = typed_paths_.at(path)->assumeMutable();

  std::unordered_map<String, MutableColumnPtr> mutable_dynamic_paths;
  mutable_dynamic_paths.reserve(dynamic_paths_.size());
  for (const auto &[path, column] : dynamic_paths_) mutable_dynamic_paths[path] = dynamic_paths_.at(path)->assumeMutable();

  return ColumnObject::create(std::move(mutable_typed_paths), std::move(mutable_dynamic_paths), shared_data_->assumeMutable(),
                              max_dynamic_paths_, global_max_dynamic_paths_, max_dynamic_types_, statistics_);
}

ColumnObject::MutablePtr ColumnObject::create(std::unordered_map<String, MutableColumnPtr> typed_paths_,
                                              std::unordered_map<String, MutableColumnPtr> dynamic_paths_, MutableColumnPtr shared_data_,
                                              size_t max_dynamic_paths_, size_t global_max_dynamic_paths_, size_t max_dynamic_types_,
                                              const ColumnObject::StatisticsPtr &statistics_) {
  return Base::create(std::move(typed_paths_), std::move(dynamic_paths_), std::move(shared_data_), max_dynamic_paths_,
                      global_max_dynamic_paths_, max_dynamic_types_, statistics_);
}

ColumnObject::MutablePtr ColumnObject::create(std::unordered_map<String, MutableColumnPtr> typed_paths_, size_t max_dynamic_paths_,
...
std::string ColumnObject::getName() const {
  ...
  for (const auto &path : sorted_typed_paths) ss << ", " << path << " " << typed_paths.find(path)->second->getName();
  ss << ")";
  return ss.str();
}
...
MutableColumnPtr ColumnObject::cloneEmpty() const {
  ...
  for (const auto &[path, column] : typed_paths) empty_typed_paths[path] = column->cloneEmpty();

  std::unordered_map<String, MutableColumnPtr> empty_dynamic_paths;
  empty_dynamic_paths.reserve(dynamic_paths.size());
  for (const auto &[path, column] : dynamic_paths) empty_dynamic_paths[path] = column->cloneEmpty();

  return ColumnObject::create(std::move(empty_typed_paths), std::move(empty_dynamic_paths), shared_data->cloneEmpty(), max_dynamic_paths,
                              global_max_dynamic_paths, max_dynamic_types, statistics);
}
...
MutableColumnPtr ColumnObject::cloneResized(size_t size) const {
  ...
  for (const auto &[path, column] : typed_paths) resized_typed_paths[path] = column->cloneResized(size);

  std::unordered_map<String, MutableColumnPtr> resized_dynamic_paths;
  resized_dynamic_paths.reserve(dynamic_paths.size());
  for (const auto &[path, column] : dynamic_paths) resized_dynamic_paths[path] = column->cloneResized(size);

  return ColumnObject::create(std::move(resized_typed_paths), std::move(resized_dynamic_paths), shared_data->cloneResized(size),
                              max_dynamic_paths, global_max_dynamic_paths, max_dynamic_types, statistics);
}

Field ColumnObject::operator[](size_t n) const {
  Object object;

  for (const auto &[path, column] : typed_paths) object[path] = (*column)[n];
  for (const auto &[path, column] : dynamic_paths_ptrs) {
    ...
    if (!column->isNullAt(n)) object[path] = (*column)[n];
  }
...
void ColumnObject::get(size_t n, Field &res) const { res = (*this)[n]; }

void ColumnObject::getValueNameImpl(WriteBufferFromOwnString &name_buf, size_t n, const Options &options) const {
  if (options.notFull(name_buf)) name_buf << '{';

  bool first = true;

  if (options.notFull(name_buf)) {
    for (const auto &[path, column] : typed_paths) {
      if (!options.notFull(name_buf)) break;
...
bool ColumnObject::isDefaultAt(size_t n) const {
  for (const auto &[path, column] : typed_paths) {
    if (!column->isDefaultAt(n)) return false;
  }
  ...
  if (!shared_data->isDefaultAt(n)) return false;
...
ColumnDynamic *ColumnObject::tryToAddNewDynamicPath(std::string_view path) {
  if (dynamic_paths.size() == max_dynamic_paths) return nullptr;
...
void ColumnObject::addNewDynamicPath(std::string_view path, MutableColumnPtr column) {
  if (dynamic_paths.size() == max_dynamic_paths)
    throw Exception(ErrorCodes::LOGICAL_ERROR, "Cannot add new dynamic path as the limit ({}) on dynamic paths is reached",
                    max_dynamic_paths);
...
void ColumnObject::setDynamicPaths(const std::vector<String> &paths) {
  ...
  if (paths.size() > max_dynamic_paths)
    throw Exception(ErrorCodes::LOGICAL_ERROR, "Cannot set dynamic paths to Object column, the number of paths ({}) exceeds the limit ({})",
                    paths.size(), max_dynamic_paths);
  ...
  for (const auto &path : paths) {
    auto new_dynamic_column = ColumnDynamic::create(max_dynamic_types);
    if (size) new_dynamic_column->insertManyDefaults(size);
    auto it = dynamic_paths.emplace(path, std::move(new_dynamic_column)).first;
    dynamic_paths_ptrs[path] = assert_cast<ColumnDynamic *>(it->second.get());
    sorted_dynamic_paths.insert(it->first);
  }
...
void ColumnObject::insert(const Field &x) {
  ...
  for (auto &[_, column] : typed_paths) {
    if (column->size() == current_size) column->insertDefault();
  }
...
bool ColumnObject::tryInsert(const Field &x) {
  if (x.getType() != Field::Types::Which::Object) return false;
  ...
  auto restore_sizes = [&]() {
    for (auto &[_, column] : typed_paths) {
      if (column->size() != prev_size) column->popBack(column->size() - prev_size);
    }
    ...
    if (shared_data_values->size() != prev_values_size) shared_data_values->popBack(shared_data_values->size() - prev_values_size);
  };
  ...
  /// Fill all remaining typed and dynamic paths with default values.
  for (auto &[_, column] : typed_paths) {
    if (column->size() == prev_size) column->insertDefault();
  }
...
#endif
{
  ...
  for (const auto &[path, column] : src_object_column.typed_paths) typed_paths[path]->insertFrom(*column, n);
  ...
  for (const auto &[path, column] : src_object_column.dynamic_paths) {
    ...
    if (auto it = dynamic_paths_ptrs.find(path); it != dynamic_paths_ptrs.end()) it->second->insertFrom(*column, n);
...
#endif
{
  ...
  /// First, insert typed paths, they must be the same for both columns.
  for (const auto &[path, column] : src_object_column.typed_paths) typed_paths[path]->insertRangeFrom(*column, start, length);
  ...
  for (const auto &[path, column] : src_object_column.dynamic_paths) {
    /// Check if we already have such dynamic path.
    if (auto it = dynamic_paths_ptrs.find(path); it != dynamic_paths_ptrs.end()) it->second->insertRangeFrom(*column, start, length);
    /// Try to add a new dynamic path.
    else if (auto *dynamic_path_column = tryToAddNewDynamicPath(path))
...
void ColumnObject::insertFromSharedDataAndFillRemainingDynamicPaths(const DB::ColumnObject &src_object_column,
                                                                    std::vector<std::string_view> &&src_dynamic_paths_for_shared_data,
  ...
  if (src_shared_data_offsets[static_cast<ssize_t>(start) - 1] == src_shared_data_offsets[static_cast<ssize_t>(start) + length - 1]) {
    ...
    /// Otherwise insert required src dynamic columns into shared data.
      ...
      for (size_t i = start; i != start + length; ++i) {
        ...
        for (const auto path : src_dynamic_paths_for_shared_data)
          serializePathAndValueIntoSharedData(shared_data_paths, shared_data_values, path,
                                              *src_object_column.dynamic_paths_ptrs.find(path)->second, i);
        shared_data_offsets.push_back(shared_data_paths->size());
      }
    }
    ...
    for (auto &[_, column] : dynamic_paths_ptrs) {
      if (column->size() == current_size) column->insertManyDefaults(length);
    }
    return;
  }
    ...
    for (size_t i = offset; i != end; ++i) {
      ...
      if (auto it = dynamic_paths_ptrs.find(path); it != dynamic_paths_ptrs.end()) {
        ...
        if (it->second->size() != current_size)
          throw Exception(ErrorCodes::LOGICAL_ERROR, "Unexpected size of dynamic path {}: {} != {}", path, it->second->size(),
                          current_size);
        ...
        while (src_dynamic_paths_for_shared_data_index < src_dynamic_paths_for_shared_data.size() &&
          ...
          serializePathAndValueIntoSharedData(shared_data_paths, shared_data_values, dynamic_path,
                                              *src_object_column.dynamic_paths_ptrs.find(dynamic_path)->second, row);
...
void ColumnObject::serializePathAndValueIntoSharedData(ColumnString *shared_data_paths, ColumnString *shared_data_values,
                                                       std::string_view path, const ColumnDynamic &column, size_t n) {
...
void ColumnObject::insertDefault() {
  for (auto &[_, column] : typed_paths) column->insertDefault();
  for (auto &[_, column] : dynamic_paths_ptrs) column->insertDefault();
  shared_data->insertDefault();
}

void ColumnObject::insertManyDefaults(size_t length) {
  for (auto &[_, column] : typed_paths) column->insertManyDefaults(length);
  for (auto &[_, column] : dynamic_paths_ptrs) column->insertManyDefaults(length);
  shared_data->insertManyDefaults(length);
}

void ColumnObject::popBack(size_t n) {
  for (auto &[_, column] : typed_paths) column->popBack(n);
  for (auto &[_, column] : dynamic_paths_ptrs) column->popBack(n);
...
ColumnCheckpointPtr ColumnObject::getCheckpoint() const {
  auto get_checkpoints = [](const auto &columns) {
    ...
    for (const auto &[name, column] : columns) checkpoints[name] = column->getCheckpoint();

    return checkpoints;
  };

  return std::make_shared<ColumnObjectCheckpoint>(size(), get_checkpoints(typed_paths), get_checkpoints(dynamic_paths_ptrs),
...
std::string_view ColumnObject::serializeValueIntoArena(size_t n, Arena &arena, const char *&begin,
...
void ColumnObject::serializeDynamicPathsAndSharedDataIntoArena(size_t n, Arena &arena, const char *&begin, std::string_view &res) const {
  ...
  for (const auto &[_, column] : dynamic_paths) num_paths += !column->isNullAt(n);
...
void ColumnObject::serializePathAndValueIntoArena(DB::Arena &arena, const char *&begin, std::string_view path, std::string_view value,
...
void ColumnObject::deserializeAndInsertFromArena(ReadBuffer &in, const IColumn::SerializationSettings *settings) {
  ...
  for (auto path : sorted_typed_paths) typed_paths.find(path)->second->deserializeAndInsertFromArena(in, settings);
...
void ColumnObject::deserializeDynamicPathsAndSharedDataFromArena(ReadBuffer &in) {
  ...
  for (size_t i = 0; i != num_paths; ++i) {
    ...
    if (in.available() < path_size)
      throw Exception(ErrorCodes::ATTEMPT_TO_READ_AFTER_EOF, "Attempt to read {} bytes for the path size, but only {} bytes are available",
                      path_size, in.available());
    ...
    /// Serialized paths are sorted, so we can insert right away.
      ...
      if (in.available() < value_size)
        throw Exception(ErrorCodes::ATTEMPT_TO_READ_AFTER_EOF,
                        "Attempt to read {} bytes for the value size, but only {} bytes are available", value_size, in.available());
...
void ColumnObject::skipSerializedInArena(ReadBuffer &in) const {
  ...
  for (auto path : sorted_typed_paths) typed_paths.find(path)->second->skipSerializedInArena(in);
...
void ColumnObject::updateHashWithValue(size_t n, SipHash &hash) const {
  for (auto path : sorted_typed_paths) typed_paths.find(path)->second->updateHashWithValue(n, hash);
...
WeakHash32 ColumnObject::getWeakHash32() const {
  ...
  for (const auto &[_, column] : typed_paths) hash.update(column->getWeakHash32());
  for (const auto &[_, column] : dynamic_paths_ptrs) hash.update(column->getWeakHash32());
  hash.update(shared_data->getWeakHash32());
  return hash;
}

void ColumnObject::updateHashFast(SipHash &hash) const {
  for (const auto &[_, column] : typed_paths) column->updateHashFast(hash);
  for (const auto &[_, column] : dynamic_paths_ptrs) column->updateHashFast(hash);
...
ColumnPtr ColumnObject::filter(const Filter &filt, ssize_t result_size_hint) const {
  ...
  for (const auto &[path, column] : typed_paths) filtered_typed_paths[path] = column->filter(filt, result_size_hint);

  std::unordered_map<String, ColumnPtr> filtered_dynamic_paths;
  filtered_dynamic_paths.reserve(dynamic_paths_ptrs.size());
  for (const auto &[path, column] : dynamic_paths_ptrs) filtered_dynamic_paths[path] = column->filter(filt, result_size_hint);

  auto filtered_shared_data = shared_data->filter(filt, result_size_hint);
  return ColumnObject::create(filtered_typed_paths, filtered_dynamic_paths, filtered_shared_data, max_dynamic_paths,
                              global_max_dynamic_paths, max_dynamic_types);
}

void ColumnObject::filter(const Filter &filt) {
  for (const auto &[path, column] : typed_paths) column->assumeMutable()->filter(filt);

  for (const auto &[path, column] : dynamic_paths_ptrs) column->filter(filt);
...
void ColumnObject::expand(const Filter &mask, bool inverted) {
  for (auto &[_, column] : typed_paths) column->expand(mask, inverted);
  for (auto &[_, column] : dynamic_paths_ptrs) column->expand(mask, inverted);
...
ColumnPtr ColumnObject::permute(const Permutation &perm, size_t limit) const {
  ...
  for (const auto &[path, column] : typed_paths) permuted_typed_paths[path] = column->permute(perm, limit);

  std::unordered_map<String, ColumnPtr> permuted_dynamic_paths;
  permuted_dynamic_paths.reserve(dynamic_paths_ptrs.size());
  for (const auto &[path, column] : dynamic_paths_ptrs) permuted_dynamic_paths[path] = column->permute(perm, limit);

  auto permuted_shared_data = shared_data->permute(perm, limit);
  return ColumnObject::create(permuted_typed_paths, permuted_dynamic_paths, permuted_shared_data, max_dynamic_paths,
                              global_max_dynamic_paths, max_dynamic_types, statistics);
}
...
ColumnPtr ColumnObject::index(const IColumn &indexes, size_t limit) const {
  ...
  for (const auto &[path, column] : typed_paths) indexed_typed_paths[path] = column->index(indexes, limit);

  std::unordered_map<String, ColumnPtr> indexed_dynamic_paths;
  indexed_dynamic_paths.reserve(dynamic_paths_ptrs.size());
  for (const auto &[path, column] : dynamic_paths_ptrs) indexed_dynamic_paths[path] = column->index(indexes, limit);

  auto indexed_shared_data = shared_data->index(indexes, limit);
  return ColumnObject::create(indexed_typed_paths, indexed_dynamic_paths, indexed_shared_data, max_dynamic_paths, global_max_dynamic_paths,
                              max_dynamic_types);
}
...
ColumnPtr ColumnObject::replicate(const Offsets &replicate_offsets) const {
  ...
  for (const auto &[path, column] : typed_paths) replicated_typed_paths[path] = column->replicate(replicate_offsets);

  std::unordered_map<String, ColumnPtr> replicated_dynamic_paths;
  replicated_dynamic_paths.reserve(dynamic_paths_ptrs.size());
  for (const auto &[path, column] : dynamic_paths_ptrs) replicated_dynamic_paths[path] = column->replicate(replicate_offsets);

  auto replicated_shared_data = shared_data->replicate(replicate_offsets);
  return ColumnObject::create(replicated_typed_paths, replicated_dynamic_paths, replicated_shared_data, max_dynamic_paths,
                              global_max_dynamic_paths, max_dynamic_types);
}
...
MutableColumns ColumnObject::scatter(size_t num_columns, const Selector &selector) const {
  ...
  for (auto &typed_paths_ : scattered_typed_paths) typed_paths_.reserve(typed_paths.size());

  for (const auto &[path, column] : typed_paths) {
    auto scattered_columns = column->scatter(num_columns, selector);
    for (size_t i = 0; i != num_columns; ++i) scattered_typed_paths[i][path] = std::move(scattered_columns[i]);
  }

  std::vector<std::unordered_map<String, MutableColumnPtr>> scattered_dynamic_paths(num_columns);
  for (auto &dynamic_paths_ : scattered_dynamic_paths) dynamic_paths_.reserve(dynamic_paths_ptrs.size());

  for (const auto &[path, column] : dynamic_paths_ptrs) {
    auto scattered_columns = column->scatter(num_columns, selector);
    for (size_t i = 0; i != num_columns; ++i) scattered_dynamic_paths[i][path] = std::move(scattered_columns[i]);
  }
  ...
  for (size_t i = 0; i != num_columns; ++i)
    result_columns.emplace_back(ColumnObject::create(std::move(scattered_typed_paths[i]), std::move(scattered_dynamic_paths[i]),
                                                     std::move(scattered_shared_data_columns[i]), max_dynamic_paths,
                                                     global_max_dynamic_paths, max_dynamic_types));
...
struct ColumnObject::ComparatorBase {
  ...
  ComparatorBase(const ColumnObject &parent_, int nan_direction_hint_) : parent(parent_), nan_direction_hint(nan_direction_hint_) {}
...
void ColumnObject::getPermutation(PermutationSortDirection direction, PermutationSortStability stability, size_t limit,
...
void ColumnObject::updatePermutation(PermutationSortDirection direction, PermutationSortStability stability, size_t limit,
                                     int nan_direction_hint, Permutation &res, EqualRanges &equal_ranges) const {
  auto comparator_equal = ComparatorEqual(*this, nan_direction_hint);

  if (direction == IColumn::PermutationSortDirection::Ascending && stability == IColumn::PermutationSortStability::Unstable)
    updatePermutationImpl(limit, res, equal_ranges, ComparatorAscendingUnstable(*this, nan_direction_hint), comparator_equal, DefaultSort(),
                          DefaultPartialSort());
  else if (direction == IColumn::PermutationSortDirection::Ascending && stability == IColumn::PermutationSortStability::Stable)
    updatePermutationImpl(limit, res, equal_ranges, ComparatorAscendingStable(*this, nan_direction_hint), comparator_equal, DefaultSort(),
                          DefaultPartialSort());
  else if (direction == IColumn::PermutationSortDirection::Descending && stability == IColumn::PermutationSortStability::Unstable)
    updatePermutationImpl(limit, res, equal_ranges, ComparatorDescendingUnstable(*this, nan_direction_hint), comparator_equal,
                          DefaultSort(), DefaultPartialSort());
  else if (direction == IColumn::PermutationSortDirection::Descending && stability == IColumn::PermutationSortStability::Stable)
    updatePermutationImpl(limit, res, equal_ranges, ComparatorDescendingStable(*this, nan_direction_hint), comparator_equal, DefaultSort(),
                          DefaultPartialSort());
}

void ColumnObject::reserve(size_t n) {
  for (auto &[_, column] : typed_paths) column->reserve(n);
  for (auto &[_, column] : dynamic_paths_ptrs) column->reserve(n);
  shared_data->reserve(n);
}

size_t ColumnObject::capacity() const { return shared_data->capacity(); }

void ColumnObject::shrinkToFit() {
  for (auto &[_, column] : typed_paths) column->shrinkToFit();
  for (auto &[_, column] : dynamic_paths_ptrs) column->shrinkToFit();
  shared_data->shrinkToFit();
}

void ColumnObject::ensureOwnership() {
  for (auto &[_, column] : typed_paths) column->ensureOwnership();
  for (auto &[_, column] : dynamic_paths_ptrs) column->ensureOwnership();
  shared_data->ensureOwnership();
}

size_t ColumnObject::byteSize() const {
  size_t size = 0;
  for (const auto &[_, column] : typed_paths) size += column->byteSize();
...
size_t ColumnObject::byteSizeAt(size_t n) const {
  size_t size = 0;
  for (const auto &[_, column] : typed_paths) size += column->byteSizeAt(n);
  for (const auto &[_, column] : dynamic_paths_ptrs) size += column->byteSizeAt(n);
...
size_t ColumnObject::allocatedBytes() const {
  size_t size = 0;
  for (const auto &[_, column] : typed_paths) size += column->allocatedBytes();
...
void ColumnObject::protect() {
  for (auto &[_, column] : typed_paths) column->protect();
  for (auto &[_, column] : dynamic_paths_ptrs) column->protect();
  shared_data->protect();
}

void ColumnObject::forEachMutableSubcolumn(DB::IColumn::MutableColumnCallback callback) {
  for (auto &[_, column] : typed_paths) callback(column);
...
void ColumnObject::forEachSubcolumn(DB::IColumn::ColumnCallback callback) const {
  for (const auto &[_, column] : typed_paths) callback(column);
  for (const auto &[path, column] : dynamic_paths) callback(column);
...
bool ColumnObject::structureEquals(const IColumn &rhs) const {
  ...
  for (const auto &[path, column] : typed_paths) {
    ...
    if (it == rhs_object->typed_paths.end() || !it->second->structureEquals(*column)) return false;
  }
...
ColumnPtr ColumnObject::compress(bool force_compression) const {
  ...
  auto decompress = [my_compressed_typed_paths = std::move(compressed_typed_paths),
                     my_compressed_dynamic_paths = std::move(compressed_dynamic_paths),
                     my_compressed_shared_data = std::move(compressed_shared_data), my_max_dynamic_paths = max_dynamic_paths,
                     my_global_max_dynamic_paths = global_max_dynamic_paths, my_max_dynamic_types = max_dynamic_types,
    ...
    for (const auto &[path, column] : my_compressed_typed_paths) decompressed_typed_paths[path] = column->decompress();
    ...
    for (const auto &[path, column] : my_compressed_dynamic_paths) decompressed_dynamic_paths[path] = column->decompress();

    auto decompressed_shared_data = my_compressed_shared_data->decompress();
    return ColumnObject::create(decompressed_typed_paths, decompressed_dynamic_paths, decompressed_shared_data, my_max_dynamic_paths,
                                my_global_max_dynamic_paths, my_max_dynamic_types, my_statistics);
  };
...
void ColumnObject::finalize() {
  for (auto &[_, column] : typed_paths) column->finalize();
  for (auto &[_, column] : dynamic_paths_ptrs) column->finalize();
  shared_data->finalize();
}

bool ColumnObject::isFinalized() const {
  bool finalized = true;
  for (const auto &[_, column] : typed_paths) finalized &= column->isFinalized();
  for (const auto &[_, column] : dynamic_paths_ptrs) finalized &= column->isFinalized();
...
void ColumnObject::prepareForSquashing(const std::vector<ColumnPtr> &source_columns, size_t factor) {
  if (source_columns.empty()) return;
  ...
  auto add_dynamic_paths = [&](const ColumnObject &source_object) {
    for (const auto &[path, dynamic_column_ptr] : source_object.dynamic_paths_ptrs) {
      ...
      if (it == path_to_total_number_of_non_null_values.end()) it = path_to_total_number_of_non_null_values.emplace(path, 0).first;
      it->second += (dynamic_column_ptr->size() - dynamic_column_ptr->getNumberOfDefaultRows());
    }
  };

  for (const auto &source_column : source_columns) add_dynamic_paths(assert_cast<const ColumnObject &>(*source_column));
  ...
  if (getSharedDataPathsAndValues().first->empty()) max_dynamic_paths = global_max_dynamic_paths;
  ...
  if (path_to_total_number_of_non_null_values.size() > max_dynamic_paths) {
    ...
    for (const auto &[path, size] : path_to_total_number_of_non_null_values) {
      if (!dynamic_paths.contains(path)) paths_with_sizes.emplace_back(size, path);
    }
    ...
    std::sort(paths_with_sizes.begin(), paths_with_sizes.end(),
              [](const auto &left, const auto &right) { return std::tie(left.first, right.second) < std::tie(right.first, left.second); });
    ...
    for (size_t i = 0; i != paths_to_add; ++i) addNewDynamicPath(paths_with_sizes[i].second);
  }
  /// Otherwise keep all paths.
    ...
    for (const auto &[path, _] : path_to_total_number_of_non_null_values) {
      if (!dynamic_paths.contains(path)) addNewDynamicPath(path);
    }
  }
  ...
  for (const auto &[path, column] : typed_paths) typed_paths_source_columns[path].reserve(source_columns.size());

  for (const auto &[path, column] : dynamic_paths) dynamic_paths_source_columns[path].reserve(source_columns.size());
  ...
  for (const auto &source_column : source_columns) {
    ...
    for (const auto &[path, column] : source_object_column.typed_paths) typed_paths_source_columns.at(path).push_back(column);

    for (const auto &[path, column] : source_object_column.dynamic_paths) {
      if (dynamic_paths.contains(path)) dynamic_paths_source_columns.at(path).push_back(column);
    }
  }
...
bool ColumnObject::dynamicStructureEquals(const IColumn &rhs) const {
  ...
  for (const auto &[path, column] : typed_paths) {
    auto it = rhs_object->typed_paths.find(path);
    if (it == rhs_object->typed_paths.end() || !it->second->dynamicStructureEquals(*column)) return false;
  }

  for (const auto &[path, column] : dynamic_paths) {
    auto it = rhs_object->dynamic_paths.find(path);
    if (it == rhs_object->dynamic_paths.end() || !it->second->dynamicStructureEquals(*column)) return false;
  }
...
void ColumnObject::takeDynamicStructureFromSourceColumns(const Columns &source_columns, std::optional<size_t> max_dynamic_subcolumns) {
  ...
  for (const auto &source_column : source_columns) {
    ...
    for (const auto &[path, column_ptr] : source_object.dynamic_paths_ptrs) {
      ...
      if (source_statistics) {
        ...
        if (statistics_it != source_statistics->dynamic_paths_statistics.end()) size = statistics_it->second;
      }
      it->second += size;
    }
  ...
  if (path_to_total_number_of_non_null_values.size() > max_dynamic_paths) {
    ...
    for (const auto &[path, size] : path_to_total_number_of_non_null_values) paths_with_sizes.emplace_back(size, path);

    /// If sizes are equal, sort by path names in ascending order (for easier testing purposes).
    std::sort(paths_with_sizes.begin(), paths_with_sizes.end(), [](const auto &left, const auto &right) {
  ...
  for (const auto &[path, _] : dynamic_paths) new_statistics.dynamic_paths_statistics[path] = path_to_total_number_of_non_null_values[path];
  ...
  for (auto &[path, column] : dynamic_paths) {
    ...
    for (const auto &source_column : source_columns) {
      ...
      if (it != source_object.dynamic_paths.end()) dynamic_path_source_columns.push_back(it->second);
    }
...
void ColumnObject::takeDynamicStructureFromColumn(const ColumnPtr &source_column) {
  if (!empty()) throw Exception(ErrorCodes::LOGICAL_ERROR, "takeDynamicStructureFromColumn should be called only on empty Object column");

  const auto &source_object = assert_cast<const ColumnObject &>(*source_column);

  for (auto &[path, column] : typed_paths) column->takeDynamicStructureFromColumn(source_object.typed_paths.at(path));
...
size_t ColumnObject::findPathLowerBoundInSharedData(std::string_view path, const ColumnString &shared_data_paths, size_t start,
                                                    size_t end) {
...
void ColumnObject::fillPathColumnFromSharedData(IColumn &path_column, std::string_view path, const ColumnPtr &shared_data_column,
...
int ColumnObject::SortedPathsIterator::compare(const SortedPathsIterator &rhs, int nan_direction_hint) const {
  ...
  if (path != rhs_path) return path < rhs_path ? -1 : 1;
...
void ColumnObject::SortedPathsIterator::setCurrentPath() {
  ...
  std::array<std::pair<PathType, std::optional<std::string_view>>, 3> paths{
      ...
      std::pair{PathType::DYNAMIC,
                dynamic_paths_it == dynamic_paths_end ? std::nullopt : std::optional<std::string_view>(*dynamic_paths_it)},
      std::pair{PathType::SHARED_DATA, shared_data_it == shared_data_end
                                           ? std::nullopt
                                           : std::optional<std::string_view>(shared_data_paths->getDataAt(shared_data_it))},
  };

  auto min_path = std::ranges::min(paths, [](auto first_path, auto second_path) {
    if (!second_path.second) return true;

    if (!first_path.second) return false;

    return first_path.second < second_path.second;
  });

  if (min_path.second) current_path_type = min_path.first;
}
...
void ColumnObject::repairDuplicatesInDynamicPathsAndSharedData(size_t offset) {
  ...
  for (size_t i = offset; i < size; ++i) {
    ...
    if (first_row_with_duplicates) break;
  }

  if (!first_row_with_duplicates) return;
  ...
  const auto [new_shared_data_paths, new_shared_data_values, new_shared_data_offsets] =
      getSharedDataPathsValuesAndOffsets(*new_shared_data);
  ...
  for (size_t i = *first_row_with_duplicates; i < size; ++i) {
      ...
      /// and we cannot repair it anyhow. Throw logical error exception in this case.
        ...
        if (!isNothing(type_from_shared_data)) {
          ...
          throw Exception(ErrorCodes::LOGICAL_ERROR,
                          ...
                          path, i, type_from_dynamic_path->getName(), type_from_shared_data->getName());
        }
      }
    }
    ...
    for (auto &[path, column] : new_dynamic_paths) {
      if (column->size() == i) column->insertFrom(*dynamic_paths.at(path), i);
    }
  }
