...
MULTITARGET_FUNCTION_X86_V4_V3(MULTITARGET_FUNCTION_HEADER(template <typename T> void), convertColumnToBoolImpl,
                               MULTITARGET_FUNCTION_BODY((const typename ColumnVector<T>::Container &data, IColumnFilter &res) {
                                 for (size_t i = 0; i < res.size(); ++i) res[i] = static_cast<bool>(data[i]);
                               }));
...
bool tryConvertAnyColumnToBool(const IColumn &column, IColumnFilter &res) {
  return tryConvertColumnToBool<Int8>(column, res) || tryConvertColumnToBool<Int16>(column, res) ||
         tryConvertColumnToBool<Int32>(column, res) || tryConvertColumnToBool<Int64>(column, res) ||
         tryConvertColumnToBool<UInt16>(column, res) || tryConvertColumnToBool<UInt32>(column, res) ||
         tryConvertColumnToBool<UInt64>(column, res) || tryConvertColumnToBool<Float32>(column, res) ||
...
size_t FilterDescription::countBytesInFilter() const { return DB::countBytesInFilter(*data); }

ColumnPtr SparseFilterDescription::filter(const IColumn &column, ssize_t) const { return column.index(*filter_indices, 0); }

size_t SparseFilterDescription::countBytesInFilter() const { return filter_indices->size(); }

SparseFilterDescription::SparseFilterDescription(const IColumn &column) {
  auto throw_invalid_type = [&column]() {
    throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_COLUMN_FOR_FILTER,
                    "Illegal type {} of column for sparse filter. Must be Sparse(UInt8) or Sparse(Nullable(UInt8))", column.getName());
  };

  const auto *column_sparse = typeid_cast<const ColumnSparse *>(&column);
  if (!column_sparse) throw_invalid_type();
  ...
  if (const auto *nullable = typeid_cast<const ColumnNullable *>(&column_sparse->getValuesColumn())) {
    ...
    for (size_t i = 0; i < offsets.size(); ++i) {
      if (values->getBool(i + 1)) mutable_valid_offsets->insertValue(offsets[i]);
    }

    valid_offsets = std::move(mutable_valid_offsets);
    filter_indices = &assert_cast<const ColumnUInt64 &>(*valid_offsets);
  } else {
    if (!typeid_cast<const ColumnUInt8 *>(&column_sparse->getValuesColumn())) throw_invalid_type();
