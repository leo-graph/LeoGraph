...
class ColumnCompressed : public COWHelper<IColumnHelper<ColumnCompressed>, ColumnCompressed> {
 public:
  ...
  ColumnCompressed(size_t rows_, size_t bytes_, Lazy lazy_) : rows(rows_), bytes(bytes_), lazy(lazy_) {}
  ...
  ColumnPtr decompress() const override { return lazy(); }
  ...
  static ColumnPtr wrap(ColumnPtr column) {
    ...
    return ColumnCompressed::create(size, bytes, [my_column = std::move(column)] { return my_column; });
  }
  ...
  bool hasEqualValues() const override { throwMustBeDecompressed(); }
