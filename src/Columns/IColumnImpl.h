...
struct ComparatorHelperImpl : public ComparatorBase {
  ...
  bool operator()(size_t lhs, size_t rhs) const {
    ...
    if constexpr (stability == IColumn::PermutationSortStability::Stable) {
      if (unlikely(res == 0)) return lhs < rhs;
    }
...
template <typename ComparatorBase>
using ComparatorAscendingUnstableImpl =
    ComparatorHelperImpl<ComparatorBase, IColumn::PermutationSortDirection::Ascending, IColumn::PermutationSortStability::Unstable>;

template <typename ComparatorBase>
using ComparatorAscendingStableImpl =
    ComparatorHelperImpl<ComparatorBase, IColumn::PermutationSortDirection::Ascending, IColumn::PermutationSortStability::Stable>;

template <typename ComparatorBase>
using ComparatorDescendingUnstableImpl =
    ComparatorHelperImpl<ComparatorBase, IColumn::PermutationSortDirection::Descending, IColumn::PermutationSortStability::Unstable>;

template <typename ComparatorBase>
using ComparatorDescendingStableImpl =
    ComparatorHelperImpl<ComparatorBase, IColumn::PermutationSortDirection::Descending, IColumn::PermutationSortStability::Stable>;
...
void IColumn::getPermutationImpl(size_t limit, Permutation& res, Compare compare, Sort full_sort, PartialSort partial_sort) const {
...
void IColumn::updatePermutationImpl(size_t limit, Permutation& res, EqualRanges& equal_ranges, Compare compare, Equals equals,
                                    Sort full_sort, PartialSort partial_sort) const {
  if (equal_ranges.empty()) return;

  if (limit >= size() || limit > equal_ranges.back().to) limit = 0;

  EqualRanges new_ranges;

  size_t number_of_ranges = equal_ranges.size();
  if (limit) --number_of_ranges;
  ...
  for (size_t i = 0; i < number_of_ranges; ++i) {
    ...
    for (size_t j = first + 1; j < last; ++j) {
      if (!equals(res[j], res[new_first])) {
        if (j - new_first > 1) new_ranges.emplace_back(new_first, j);

        new_first = j;
      }
    }

    if (last - new_first > 1) new_ranges.emplace_back(new_first, last);
  }
  ...
  if (limit) {
    ...
    if (new_last - new_first > 1) new_ranges.emplace_back(new_first, new_last);
  }
