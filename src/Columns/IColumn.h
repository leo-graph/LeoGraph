...
struct ColumnCheckpointWithNested : public ColumnCheckpoint {
  ColumnCheckpointWithNested(size_t size_, ColumnCheckpointPtr nested_) : ColumnCheckpoint(size_), nested(std::move(nested_)) {}

  ColumnCheckpointPtr nested;
};

struct ColumnCheckpointWithMultipleNested : public ColumnCheckpoint {
  ColumnCheckpointWithMultipleNested(size_t size_, ColumnCheckpoints nested_) : ColumnCheckpoint(size_), nested(std::move(nested_)) {}
...
class IColumn : public COW<IColumn> {
 ...
 public:
  ...
  [[nodiscard]] virtual Ptr convertToFullIfNeeded() const {
    ...
    if (!any_changed) return converted;
   ...
   */
  virtual std::string_view serializeValueIntoArena(size_t /* n */, Arena & /* arena */, char const *& /* begin */,
  ...
  virtual std::string_view serializeValueIntoArenaWithNull(size_t /* n */, Arena & /* arena */, char const *& /* begin */,
                                                           const UInt8 * /* is_null */, const SerializationSettings *settings) const;

  virtual char *serializeValueIntoMemoryWithNull(size_t /* n */, char * /* memory */, const UInt8 * /* is_null */,
                                                 const SerializationSettings *settings) const;

  virtual void batchSerializeValueIntoMemoryWithNull(std::vector<char *> & /* memories */, const UInt8 * /* is_null */,
                                                     const SerializationSettings *settings) const;
  ...
  virtual void collectSerializedValueSizes(PaddedPODArray<UInt64> & /* sizes */, const UInt8 * /* is_null */,
                                           const SerializationSettings *settings) const;
  ...
  [[nodiscard]] virtual llvm::Value *compileComparator(llvm::IRBuilderBase & /*builder*/, llvm::Value * /*lhs*/, llvm::Value * /*rhs*/,
  ...
  virtual void compareColumn(const IColumn &rhs, size_t rhs_row_num, PaddedPODArray<UInt64> *row_indexes,
                             PaddedPODArray<Int8> &compare_results, int direction, int nan_direction_hint) const = 0;
  ...
  enum class PermutationSortDirection : uint8_t { Ascending = 0, Descending };

  enum class PermutationSortStability : uint8_t { Unstable = 0, Stable };
   ...
   */
  virtual void getPermutation(PermutationSortDirection direction, PermutationSortStability stability, size_t limit, int nan_direction_hint,
   ...
   */
  virtual void updatePermutation(PermutationSortDirection direction, PermutationSortStability stability, size_t limit,
                                 int nan_direction_hint, Permutation &res, EqualRanges &equal_ranges) const = 0;
   ...
   */
  virtual void getPermutationWithCollation(const Collator & /*collator*/, PermutationSortDirection /*direction*/,
                                           PermutationSortStability /*stability*/, size_t /*limit*/, int /*nan_direction_hint*/,
                                           Permutation & /*res*/) const;

  virtual void updatePermutationWithCollation(const Collator & /*collator*/, PermutationSortDirection /*direction*/,
                                              PermutationSortStability /*stability*/, size_t /*limit*/, int /*nan_direction_hint*/,
                                              Permutation & /*res*/, EqualRanges & /*equal_ranges*/) const;

  /// Estimate the cardinality (number of unique values) of the values in 'equal_range' after permutation, formally: |{
  /// column[permutation[r]] : r in equal_range }|.
  ...
  [[nodiscard]] virtual Ptr createWithOffsets(const Offsets &offsets, const ColumnConst &column_with_default_value, size_t total_rows,
                                              size_t shift) const;
  ...
  /// The flag `force_compression` indicates that compression should be performed even if it's not efficient (if only compression factor <
  /// 1).
  ...
  virtual void fillFromRowRefs(const DataTypePtr &type, size_t source_column_index_in_block, const UInt64 *row_refs_begin,
                               const UInt64 *row_refs_end, bool row_refs_are_ranges);

  /// Fills column values from list of blocks and row numbers
  virtual void fillFromBlocksAndRowNumbers(const DataTypePtr &type, size_t source_column_index_in_block,
   ...
   *
   * Example: Tuple store values of each component in separate subcolumn, so the values of Tuples with at least two components are not
   * contiguous. Another example is Nullable. Each value have null flag, that is stored separately, so the value is not contiguous in
   * memory.
   *
 ...
 protected:
  ...
  void updatePermutationImpl(size_t limit, Permutation &res, EqualRanges &equal_ranges, Compare compare, Equals equals, Sort full_sort,
 ...
 private:
  void assertTypeEquality(const IColumn &rhs) const {
    /// For Sparse and Const columns, we can compare only internal types. It is considered normal to e.g. insert from normal vector column
    /// to a sparse vector column. This case is specifically handled in ColumnSparse implementation. Similar situation with Const column.
...
class IColumnHelper : public Parent {
 private:
  ...
  void compareColumn(const IColumn &rhs_base, size_t rhs_row_num, PaddedPODArray<UInt64> *row_indexes,
                     PaddedPODArray<Int8> &compare_results, int direction, int nan_direction_hint) const override;
  ...
  void collectSerializedValueSizes(PaddedPODArray<UInt64> &sizes, const UInt8 *is_null,
  ...
  /// Fills column values from RowRefList
  /// If row_refs_are_ranges is true, then each RowRefList has one element with >=1 consecutive rows
  void fillFromRowRefs(const DataTypePtr &type, size_t source_column_index_in_block, const UInt64 *row_refs_begin,
                       const UInt64 *row_refs_end, bool row_refs_are_ranges) override;

  /// Fills column values from list of columns and row numbers
  void fillFromBlocksAndRowNumbers(const DataTypePtr &type, size_t source_column_index_in_block,
                                   ColumnsWithRowNumbers columns_with_row_numbers) override;

  /// Move common implementations into the same translation unit to ensure they are properly inlined.
  char *serializeValueIntoMemoryWithNull(size_t n, char *memory, const UInt8 *is_null,
                                         const IColumn::SerializationSettings *settings) const override;
  void batchSerializeValueIntoMemoryWithNull(std::vector<char *> &memories, const UInt8 *is_null,
                                             const IColumn::SerializationSettings *settings) const override;
  ...
  std::string_view serializeValueIntoArenaWithNull(size_t n, Arena &arena, char const *&begin, const UInt8 *is_null,
                                                   const IColumn::SerializationSettings *settings) const override;
