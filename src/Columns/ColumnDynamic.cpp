...
const DataTypePtr &ColumnDynamic::getSharedVariantDataType() {
  static thread_local DataTypePtr type = DataTypeFactory::instance().getCustom(
      "String", std::make_unique<DataTypeCustomDesc>(std::make_unique<DataTypeCustomFixedName>(getSharedVariantTypeName())));
  return type;
}

ColumnDynamic::ColumnDynamic(size_t max_dynamic_types_)
    : max_dynamic_types(max_dynamic_types_), global_max_dynamic_types(max_dynamic_types) {
  /// Create Variant with shared variant.
  setVariantType(std::make_shared<DataTypeVariant>(DataTypes{getSharedVariantDataType()}));
}

ColumnDynamic::ColumnDynamic(MutableColumnPtr variant_column_, const DataTypePtr &variant_type_, size_t max_dynamic_types_,
                             size_t global_max_dynamic_types_, const StatisticsPtr &statistics_)
...
ColumnDynamic::ColumnDynamic(MutableColumnPtr variant_column_, const VariantInfo &variant_info_, size_t max_dynamic_types_,
                             size_t global_max_dynamic_types_, const StatisticsPtr &statistics_)
    : variant_column(std::move(variant_column_)),
      variant_column_ptr(assert_cast<ColumnVariant *>(variant_column.get())),
...
bool ColumnDynamic::addNewVariant(const DataTypePtr &new_variant, const String &new_variant_name) {
  ...
  if (variant_info.variant_name_to_discriminator.contains(new_variant_name)) return true;
...
void extendVariantColumn(IColumn &variant_column, const DataTypePtr &old_variant_type, const DataTypePtr &new_variant_type,
  ...
  assert_cast<ColumnVariant &>(variant_column)
      .extend(current_to_new_discriminators, std::move(new_variant_columns_and_discriminators_to_add));
}
...
std::vector<ColumnVariant::Discriminator> *ColumnDynamic::combineVariants(const ColumnDynamic::VariantInfo &other_variant_info) {
  ...
  if (cache_it != variant_mappings_cache.end()) return &cache_it->second;

  /// Check if we already tried to combine these variants but failed due to max_dynamic_types limit.
  if (variants_with_failed_combination.contains(other_variant_info.variant_name)) return nullptr;
  ...
  for (size_t i = 0; i != other_variants.size(); ++i) {
    if (!variant_info.variant_name_to_discriminator.contains(other_variant_info.variant_names[i])) ++num_new_variants;
  }
...
void ColumnDynamic::get(size_t n, Field &res) const {
  ...
  if (variant_col.globalDiscriminatorAt(n) != getSharedVariantDiscriminator()) {
    ...
    if (!res.isNull()) res = convertFieldToType(res, *getTypeAt(n));
    return;
  }
...
#endif
{
  ...
  if (addNewVariant(variant_type)) {
    ...
    variant_col.insertIntoVariantFrom(discr, src_variant_col.getVariantByGlobalDiscriminator(src_global_discr),
                                      src_variant_col.offsetAt(n));
    return;
  }

  /// We reached maximum number of variants and couldn't add new variant.
  /// Insert this value into shared variant.
  insertValueIntoSharedVariant(src_variant_col.getVariantByGlobalDiscriminator(src_global_discr), variant_type,
...
#endif
{
  if (start + length > src_.size())
    throw Exception(ErrorCodes::PARAMETER_OUT_OF_BOUND,
                    "Parameter out of bound in ColumnDynamic::insertRangeFrom method. "
                    "[start({}) + length({}) > src.size()({})]",
                    start, length, src_.size());
  ...
  if (auto *global_discriminators_mapping = combineVariants(dynamic_src.variant_info)) {
    ...
    if (dynamic_src.getSharedVariant().empty()) return;
    ...
    const auto &src_shared_variant =
        assert_cast<const ColumnString &>(src_variant_column.getVariantByLocalDiscriminator(src_shared_variant_local_discr));
  ...
  /// We still can add some new variants, but not all of them. Let's choose the most frequent variants.
    ...
    for (const auto &[_, discr] : new_variants_with_sizes) {
      if (!canAddNewVariant(new_variants.size())) break;
      ...
      /// Insertion from usual variant.
        ...
        if (global_discr == shared_variant_discr) {
          serializeValueIntoSharedVariant(
              shared_variant, *src_variant_columns[src_local_discr], src_variants[src_global_discr],
...
#endif
{
  ...
  if (addNewVariant(variant_type)) {
    auto discr = variant_info.variant_name_to_discriminator[dynamic_src.variant_info.variant_names[src_global_discr]];
    variant_col.insertManyIntoVariantFrom(discr, src_variant_col.getVariantByGlobalDiscriminator(src_global_discr),
                                          src_variant_col.offsetAt(position), length);
    return;
  }
  ...
  serializeValueIntoSharedVariant(*tmp_shared_variant, src_variant_col.getVariantByGlobalDiscriminator(src_global_discr), variant_type,
...
void ColumnDynamic::serializeValueIntoSharedVariant(ColumnString &shared_variant, const IColumn &src, const DataTypePtr &type,
                                                    const SerializationPtr &serialization, size_t n) {
  auto &chars = shared_variant.getChars();
  {
    WriteBufferFromVector<ColumnString::Chars> value_buf(chars, AppendModeTag());
...
std::string_view ColumnDynamic::serializeValueIntoArena(size_t n, Arena &arena, const char *&begin,
  ...
  /// For regular variants serialize its type and value in binary format.
    ...
    variant_type->getDefaultSerialization()->serializeBinary(variant_col.getVariantByGlobalDiscriminator(discr), variant_col.offsetAt(n),
                                                             buf, getFormatSettings());
...
#endif
{
  ...
  if (left_discr == ColumnVariant::NULL_DISCRIMINATOR && right_discr == ColumnVariant::NULL_DISCRIMINATOR) return 0;
  if (left_discr == ColumnVariant::NULL_DISCRIMINATOR) return nan_direction_hint;
  if (right_discr == ColumnVariant::NULL_DISCRIMINATOR) return -nan_direction_hint;
  ...
  if (left_discr == left_shared_variant_discr && right_discr == right_shared_variant_discr) {
    ...
    if (left_data_type_name != right_data_type_name) return left_data_type_name < right_data_type_name ? -1 : 1;
  ...
  if (left_discr == left_shared_variant_discr) {
    ...
    return tmp_column->compareAt(0, right_variant.offsetAt(m), right_variant.getVariantByGlobalDiscriminator(right_discr),
                                 nan_direction_hint);
  }
  ...
  return left_variant.getVariantByGlobalDiscriminator(left_discr)
      .compareAt(left_variant.offsetAt(n), right_variant.offsetAt(m), right_variant.getVariantByGlobalDiscriminator(right_discr),
                 nan_direction_hint);
}
...
struct ColumnDynamic::ComparatorBase {
  ...
  ComparatorBase(const ColumnDynamic &parent_, int nan_direction_hint_) : parent(parent_), nan_direction_hint(nan_direction_hint_) {}

  ALWAYS_INLINE int compare(size_t lhs, size_t rhs) const { return parent.compareAt(lhs, rhs, parent, nan_direction_hint); }
};

void ColumnDynamic::getPermutation(IColumn::PermutationSortDirection direction, IColumn::PermutationSortStability stability, size_t limit,
                                   int nan_direction_hint, IColumn::Permutation &res) const {
...
void ColumnDynamic::updatePermutation(IColumn::PermutationSortDirection direction, IColumn::PermutationSortStability stability,
...
ColumnPtr ColumnDynamic::compress(bool force_compression) const {
  ...
  return ColumnCompressed::create(
      size(), byte_size,
      [my_variant_compressed = std::move(variant_compressed), my_variant_info = variant_info, my_max_dynamic_types = max_dynamic_types,
       my_global_max_dynamic_types = global_max_dynamic_types, my_statistics = statistics]() mutable {
        return ColumnDynamic::create(my_variant_compressed->decompress(), my_variant_info, my_max_dynamic_types,
...
void ColumnDynamic::rollback(const ColumnCheckpoint &checkpoint) {
  ...
  for (const auto &[name, discr] : variant_info.variant_name_to_discriminator) {
    ...
    if (it == variants_checkpoints.end())
      variant_column_ptr->getVariantPtrByGlobalDiscriminator(discr) =
          variant_column_ptr->getVariantPtrByGlobalDiscriminator(discr)->cloneEmpty();
    else
...
String ColumnDynamic::getTypeNameAt(size_t row_num) const {
  ...
  if (discr == ColumnVariant::NULL_DISCRIMINATOR) return "";
...
DataTypePtr ColumnDynamic::getTypeAt(size_t row_num) const {
  const auto &variant_col = getVariantColumn();
  const size_t discr = variant_col.globalDiscriminatorAt(row_num);
  if (discr == ColumnVariant::NULL_DISCRIMINATOR) return nullptr;
...
void ColumnDynamic::prepareVariantsForSquashing(const Columns &source_columns, size_t factor) {
  ...
  for (const auto &source_column : source_columns) add_variants(assert_cast<const ColumnDynamic &>(*source_column));
  ...
  if (getSharedVariant().empty()) max_dynamic_types = global_max_dynamic_types;
  ...
  if (!canAddNewVariants(0, all_variants.size())) {
    ...
    for (const auto &[_, new_variant] : variants_with_sizes) {
      if (!canAddNewVariant(result_variants.size())) break;
  ...
  if (!result_variant_type->equals(*variant_info.variant_type)) updateVariantInfoAndExpandVariantColumn(result_variant_type);
...
void ColumnDynamic::takeDynamicStructureFromSourceColumns(const Columns &source_columns, std::optional<size_t> max_dynamic_subcolumns) {
  ...
  for (const auto &source_column : source_columns) {
    ...
    for (size_t i = 0; i != source_variants.size(); ++i) {
      ...
      if (source_statistics) {
        ...
        if (statistics_it != source_statistics->variants_statistics.end()) size = statistics_it->second;
      }
  ...
  if (!canAddNewVariants(0, all_variants.size())) {
    ...
    for (const auto &variant : all_variants) {
      auto variant_name = variant->getName();
      if (variant_name != getSharedVariantTypeName()) variants_with_sizes.emplace_back(total_sizes[variant_name], variant_name, variant);
    }
    std::sort(variants_with_sizes.begin(), variants_with_sizes.end(), std::greater());
    ...
    for (const auto &[size, variant_name, variant_type] : variants_with_sizes) {
      ...
      if (canAddNewVariant(result_variants.size())) result_variants.push_back(variant_type);
  ...
  for (const auto &variant_name : variant_info.variant_names) new_statistics.variants_statistics[variant_name] = total_sizes[variant_name];
...
void ColumnDynamic::takeDynamicStructureFromColumn(const ColumnPtr &source_column) {
  if (!empty()) throw Exception(ErrorCodes::LOGICAL_ERROR, "takeDynamicStructureFromColumn should be called only on empty Dynamic column");
  ...
  auto &variant_col = getVariantColumn();
  for (size_t i = 0; i != variant_info.variant_names.size(); ++i)
    variant_col.getVariantByGlobalDiscriminator(i).takeDynamicStructureFromColumn(
        source_variant_column.getVariantPtrByGlobalDiscriminator(i));
}
...
void ColumnDynamic::applyNullMap(const ColumnVector<UInt8>::Container &null_map) { variant_column_ptr->applyNullMap(null_map); }
