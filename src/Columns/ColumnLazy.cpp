...
ColumnLazy::ColumnLazy(MutableColumns &&mutable_columns) {
  ...
  for (auto &column : mutable_columns) captured_columns.push_back(std::move(column));
}
...
MutableColumnPtr ColumnLazy::cloneResized(size_t new_size) const {
  if (captured_columns.empty()) return ColumnLazy::create(new_size)->assumeMutable();

  const size_t column_size = captured_columns.size();
  MutableColumns new_columns(column_size);
  for (size_t i = 0; i < column_size; ++i) new_columns[i] = captured_columns[i]->cloneResized(new_size);
...
void ColumnLazy::insert(const Field &) { throw Exception(ErrorCodes::NOT_IMPLEMENTED, "Method insert is not supported for {}", getName()); }
...
#endif
{
  ...
  for (size_t i = 0; i < column_size; ++i) captured_columns[i]->insertFrom(*src.captured_columns[i], n);
}
...
#endif
{
  ...
  for (size_t i = 0; i < column_size; ++i)
    captured_columns[i]->insertManyFrom(*assert_cast<const ColumnLazy &>(src).captured_columns[i], position, length);
}
...
void ColumnLazy::popBack(size_t) { throw Exception(ErrorCodes::NOT_IMPLEMENTED, "Method popBack is not supported for {}", getName()); }
...
#endif
{
  ...
  const size_t column_size = captured_columns.size();
  for (size_t i = 0; i < column_size; ++i)
    captured_columns[i]->insertRangeFrom(*assert_cast<const ColumnLazy &>(src).captured_columns[i], start, length);
}
...
ColumnPtr ColumnLazy::filter(const Filter &filt, ssize_t result_size_hint) const {
  ...
  for (size_t i = 0; i < column_size; ++i) new_columns[i] = captured_columns[i]->filter(filt, result_size_hint);
...
void ColumnLazy::filter(const Filter &filt) {
  ...
  const size_t column_size = captured_columns.size();
  for (size_t i = 0; i < column_size; ++i) captured_columns[i]->filter(filt);
}
...
ColumnPtr ColumnLazy::permute(const Permutation &perm, size_t limit) const {
  ...
  for (size_t i = 0; i < column_size; ++i) new_columns[i] = captured_columns[i]->permute(perm, limit);
...
ColumnPtr ColumnLazy::index(const IColumn &indexes, size_t limit) const {
  ...
  for (size_t i = 0; i < column_size; ++i) new_columns[i] = captured_columns[i]->index(indexes, limit);
...
void ColumnLazy::compareColumn(const IColumn &, size_t, PaddedPODArray<UInt64> *, PaddedPODArray<Int8> &, int, int) const {
...
void ColumnLazy::getPermutation(IColumn::PermutationSortDirection, IColumn::PermutationSortStability, size_t, int,
                                IColumn::Permutation &) const {
  throw Exception(ErrorCodes::NOT_IMPLEMENTED, "Method getPermutation is not supported for {}", getName());
}

void ColumnLazy::updatePermutation(IColumn::PermutationSortDirection, IColumn::PermutationSortStability, size_t, int,
                                   IColumn::Permutation &, EqualRanges &) const {
  throw Exception(ErrorCodes::NOT_IMPLEMENTED, "Method updatePermutation is not supported for {}", getName());
}

void ColumnLazy::getPermutationWithCollation(const Collator &, IColumn::PermutationSortDirection, IColumn::PermutationSortStability, size_t,
...
void ColumnLazy::updatePermutationWithCollation(const Collator &, IColumn::PermutationSortDirection, IColumn::PermutationSortStability,
...
void ColumnLazy::reserve(size_t n) {
  if (captured_columns.empty()) return;

  const size_t column_size = captured_columns.size();
  for (size_t i = 0; i < column_size; ++i) captured_columns[i]->reserve(n);
}
...
size_t ColumnLazy::byteSize() const {
  size_t res = 0;
  for (const auto &column : captured_columns) res += column->byteSize();
  return res;
}
...
size_t ColumnLazy::allocatedBytes() const {
  size_t res = 0;
  for (const auto &column : captured_columns) res += column->allocatedBytes();
  return res;
}

void ColumnLazy::protect() { throw Exception(ErrorCodes::NOT_IMPLEMENTED, "Method protect is not supported for {}", getName()); }
...
void ColumnLazy::finalize() { throw Exception(ErrorCodes::NOT_IMPLEMENTED, "Method finalize is not supported for {}", getName()); }
