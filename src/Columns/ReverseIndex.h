...
struct ReverseIndexHashTableCell
  ...
  bool keyEquals(std::string_view object, size_t hash_ [[maybe_unused]], const State &state) const {
    auto index = key;
    if constexpr (has_base_index) index -= state.base_index;
...
class ReverseIndexHashTableBase : public HashTable<Key, Cell, Hash, HashTableGrowerWithPrecalculation<>, HashTableAllocator> {
 ...
 public:
  ...
  template <typename ObjectToCompareWith>
  void ALWAYS_INLINE reverseIndexEmplaceNonZero(const Key &key, LookupResult &it, bool &inserted, size_t hash_value,
                                                const ObjectToCompareWith &object) {
  ...
  iterator ALWAYS_INLINE reverseIndexFind(ObjectToCompareWith x, size_t hash_value) {
    if (Cell::isZero(x, *this)) return this->hasZero() ? this->iteratorToZero() : this->end();
...
class ReverseIndexStringHashTable
    : public ReverseIndexHashTableBase<
          IndexType,
          ReverseIndexHashTableCell<IndexType, ReverseIndexHash, ReverseIndexStringHashTable<IndexType, ColumnType, has_base_index>,
                                    ColumnType, true, has_base_index>,
          ReverseIndexHash> {
  using Base = ReverseIndexHashTableBase<
      IndexType,
      ReverseIndexHashTableCell<IndexType, ReverseIndexHash, ReverseIndexStringHashTable<IndexType, ColumnType, has_base_index>, ColumnType,
                                true, has_base_index>,
      ReverseIndexHash>;

 public:
  using Base::Base;
  friend struct ReverseIndexHashTableCell<IndexType, ReverseIndexHash, ReverseIndexStringHashTable<IndexType, ColumnType, has_base_index>,
                                          ColumnType, true, has_base_index>;
};

template <typename IndexType, typename ColumnType, bool has_base_index>
class ReverseIndexNumberHashTable
    : public ReverseIndexHashTableBase<
          IndexType,
          ReverseIndexHashTableCell<IndexType, ReverseIndexHash, ReverseIndexNumberHashTable<IndexType, ColumnType, has_base_index>,
                                    ColumnType, false, has_base_index>,
          ReverseIndexHash> {
  using Base = ReverseIndexHashTableBase<
      IndexType,
      ReverseIndexHashTableCell<IndexType, ReverseIndexHash, ReverseIndexNumberHashTable<IndexType, ColumnType, has_base_index>, ColumnType,
                                false, has_base_index>,
      ReverseIndexHash>;

 public:
  using Base::Base;
  friend struct ReverseIndexHashTableCell<IndexType, ReverseIndexHash, ReverseIndexNumberHashTable<IndexType, ColumnType, has_base_index>,
                                          ColumnType, false, has_base_index>;
};
...
class ReverseIndex {
 public:
  ...
  const UInt64 *tryGetSavedHash() const {
    if (!use_saved_hash) return nullptr;
...
size_t ReverseIndex<IndexType, ColumnType>::size() const {
  if (!column) throw Exception(ErrorCodes::LOGICAL_ERROR, "ReverseIndex has not size because index column wasn't set.");
...
void ReverseIndex<IndexType, ColumnType>::buildIndex() {
  ...
  if (!column) throw Exception(ErrorCodes::LOGICAL_ERROR, "ReverseIndex can't build index because index column wasn't set.");

  auto size = column->size();
  index = std::make_unique<IndexMapType>(size);

  if constexpr (use_saved_hash) saved_hash = calcHashes();

  auto &state = index->getState();
  state.index_column = column;
  state.base_index = base_index;
  if constexpr (use_saved_hash) state.saved_hash_column = &saved_hash->getData();
  ...
  for (auto row : collections::range(num_prefix_rows_to_skip, size)) {
    ...
    if (!inserted) throw Exception(ErrorCodes::LOGICAL_ERROR, "Duplicating keys found in ReverseIndex.");
  }
}
...
ColumnUInt64::MutablePtr ReverseIndex<IndexType, ColumnType>::calcHashes() const {
  ...
  for (auto row : collections::range(0, size)) hash->getElement(row) = getHash(column->getDataAt(row));
...
UInt64 ReverseIndex<IndexType, ColumnType>::insert(std::string_view data) {
  ...
  if constexpr (use_saved_hash) {
    ...
    if (column_data.size() <= num_rows) column_data.resize(num_rows + 1);
  ...
  if constexpr (use_saved_hash) {
    if (inserted) column->insertData(data.data(), data.size());
  } else {
    if (!inserted) column->popBack(1);
  }
