...
class ColumnFixedString final : public COWHelper<IColumnHelper<ColumnFixedString, ColumnFixedSizeHelper>, ColumnFixedString> {
 ...
 public:
  ...
  size_t byteSizeAt(size_t) const override { return n; }

  size_t allocatedBytes() const override { return chars.allocated_bytes() + sizeof(n); }

  void protect() override { chars.protect(); }

  Field operator[](size_t index) const override { return Field{&chars[n * index], n}; }

  void get(size_t index, Field &res) const override { res = std::string_view{reinterpret_cast<const char *>(&chars[n * index]), n}; }

  void getValueNameImpl(WriteBufferFromOwnString &name_buf, size_t index, const Options &options) const override;

  std::string_view getDataAt(size_t index) const override { return {reinterpret_cast<const char *>(&chars[n * index]), n}; }
  ...
  void popBack(size_t elems) override {
    if (elems > size()) throwCannotPopBack(n, getName(), size());
  ...
  void updatePermutation(IColumn::PermutationSortDirection direction, IColumn::PermutationSortStability stability, size_t limit,
                         int nan_direction_hint, Permutation &res, EqualRanges &equal_ranges) const override;
  ...
  void reserve(size_t size) override { chars.reserve_exact(n * size); }

  size_t capacity() const override { return chars.capacity() / n; }

  void shrinkToFit() override { chars.shrink_to_fit(); }

  void resize(size_t size) { chars.resize(n * size); }

  void getExtremes(Field &min, Field &max, size_t start, size_t end) const override;

  bool structureEquals(const IColumn &rhs) const override {
    if (const auto *rhs_concrete = typeid_cast<const ColumnFixedString *>(&rhs)) return n == rhs_concrete->n;
    return false;
  }
