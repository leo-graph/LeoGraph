...
static String getTypeString(const AggregateFunctionPtr &func, std::optional<size_t> version = std::nullopt) {
  ...
  if (version && *version) stream << *version << ", ";
  ...
  for (const auto &argument_type : argument_types) stream << ", " << argument_type->getName();
...
ColumnAggregateFunction::~ColumnAggregateFunction() {
  if (!func->hasTrivialDestructor() && !src)
    for (auto *val : data) func->destroy(val);
}

void ColumnAggregateFunction::addArena(ConstArenaPtr arena_) { foreign_arenas.push_back(arena_); }

namespace {

ConstArenas concatArenas(const ConstArenas &array, ConstArenaPtr arena) {
  ConstArenas result = array;
  if (arena) result.push_back(std::move(arena));
...
std::string ColumnAggregateFunction::getName() const { return "AggregateFunction(" + func->getName() + ")"; }
...
MutableColumnPtr ColumnAggregateFunction::convertToValues(MutableColumnPtr column) {
  ...
  for (auto *val : data) func->insertResultInto(val, *res, &column_aggregate_func.createOrGetArena());
...
void ColumnAggregateFunction::ensureOwnership() {
  ...
  if (src) {
    ...
    fiu_do_on(FailPoints::column_aggregate_function_ensureOwnership_exception, { inject_memory_limit_exceeded = true; });
    ...
    try {
      for (size_t i = 0; i < size; ++i) {
        ...
        if (unlikely(inject_memory_limit_exceeded)) throw Exception(ErrorCodes::MEMORY_LIMIT_EXCEEDED, "Failpoint triggered");
      ...
      if (!func->hasTrivialDestructor()) {
        for (size_t i = 0; i < rollback_pos; ++i) func->destroy(new_data[i]);
      }
      throw;
    }
...
#endif
{
  ...
  if (force_data_ownership || (!empty() && src.get() != &from_concrete)) {
    ...
    for (size_t i = start; i < end; ++i) insertFromWithOwnership(from, i);
  } else {
...
ColumnPtr ColumnAggregateFunction::filter(const Filter &filter, ssize_t result_size_hint) const {
  size_t size = data.size();
  if (size != filter.size())
    throw Exception(ErrorCodes::SIZES_OF_COLUMNS_DOESNT_MATCH, "Size of filter ({}) doesn't match size of column ({})", filter.size(),
                    size);

  if (size == 0) return cloneEmpty();
  ...
  for (size_t i = 0; i < size; ++i)
    if (filter[i]) res_data.push_back(data[i]);

  /// To save RAM in case of too strong filtering.
  if (res_data.size() * 2 < res_data.capacity()) res_data = Container(res_data.cbegin(), res_data.cend());
...
void ColumnAggregateFunction::expand(const Filter &mask, bool inverted) {
  ...
  if (mask.size() < data.size()) throw Exception(ErrorCodes::LOGICAL_ERROR, "Mask size should be no less than data size.");
  ...
  while (index >= 0) {
    if (!!mask[index] ^ inverted) {
      ...
      if (index != from) data[index] = data[from];
      --from;
    } else {
...
ColumnPtr ColumnAggregateFunction::permute(const Permutation &perm, size_t limit) const { return permuteImpl(*this, perm, limit); }

ColumnPtr ColumnAggregateFunction::index(const IColumn &indexes, size_t limit) const { return selectIndexImpl(*this, indexes, limit); }
...
ColumnPtr ColumnAggregateFunction::indexImpl(const PaddedPODArray<Type> &indexes, size_t limit) const {
  ...
  for (size_t i = 0; i < limit; ++i) res->data[i] = data[indexes[i]];
...
size_t ColumnAggregateFunction::byteSize() const { return data.size() * sizeof(data[0]) + (my_arena ? my_arena->usedBytes() : 0); }
...
size_t ColumnAggregateFunction::allocatedBytes() const { return data.allocated_bytes() + (my_arena ? my_arena->allocatedBytes() : 0); }

void ColumnAggregateFunction::protect() { data.protect(); }

MutableColumnPtr ColumnAggregateFunction::cloneEmpty() const { return create(func, version); }
...
void ColumnAggregateFunction::get(size_t n, Field &res) const { res = operator[](n); }
...
std::string_view ColumnAggregateFunction::getDataAt(size_t n) const { return {reinterpret_cast<const char *>(&data[n]), sizeof(data[n])}; }
...
void ColumnAggregateFunction::insertMergeFrom(ConstAggregateDataPtr place) { func->merge(data.back(), place, &createOrGetArena()); }
...
Arena &ColumnAggregateFunction::createOrGetArena() {
  if (unlikely(!my_arena)) my_arena = std::make_shared<Arena>();
...
void ColumnAggregateFunction::insert(const Field &x) {
  if (x.getType() != Field::Types::AggregateFunctionState)
    throw Exception(ErrorCodes::LOGICAL_ERROR, "Inserting field of type {} into ColumnAggregateFunction. Expected {}", x.getTypeName(),
                    Field::Types::AggregateFunctionState);

  const auto &field_name = x.safeGet<AggregateFunctionStateData>().name;
  if (type_string != field_name)
    throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT, "Cannot insert filed with type {} into column with type {}", field_name,
                    type_string);
...
bool ColumnAggregateFunction::tryInsert(const DB::Field &x) {
  if (x.getType() != Field::Types::AggregateFunctionState) return false;

  const auto &field_name = x.safeGet<AggregateFunctionStateData>().name;
  if (type_string != field_name) return false;
...
std::string_view ColumnAggregateFunction::serializeValueIntoArena(size_t n, Arena &arena, const char *&begin,
...
void ColumnAggregateFunction::popBack(size_t n) {
  ...
  if (!src)
    for (size_t i = new_size; i < size; ++i) func->destroy(data[i]);
...
ColumnPtr ColumnAggregateFunction::replicate(const IColumn::Offsets &offsets) const {
  size_t size = data.size();
  if (size != offsets.size()) throw Exception(ErrorCodes::SIZES_OF_COLUMNS_DOESNT_MATCH, "Size of offsets doesn't match size of column.");

  if (size == 0 || offsets.back() == 0) return cloneEmpty();
...
MutableColumns ColumnAggregateFunction::scatter(size_t num_columns, const IColumn::Selector &selector) const {
  ...
  for (auto &column : columns) column = createView();

  size_t num_rows = size();

  {
    size_t reserve_size = static_cast<size_t>(static_cast<double>(num_rows) / static_cast<double>(num_columns) *
                                              1.1);  /// 1.1 is just a guess. Better to use n-sigma rule.

    if (reserve_size > 1)
      for (auto &column : columns) column->reserve(reserve_size);
  }

  for (size_t i = 0; i < num_rows; ++i) assert_cast<ColumnAggregateFunction &>(*columns[selector[i]]).data.push_back(data[i]);
...
void ColumnAggregateFunction::updatePermutation(PermutationSortDirection, PermutationSortStability, size_t, int, Permutation &,
                                                EqualRanges &) const {}
...
MutableColumnPtr ColumnAggregateFunction::cloneResized(size_t size) const {
  ...
  for (size_t i = from_size; i < size; ++i) res->insertDefault();
