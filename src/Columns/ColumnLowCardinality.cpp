...
MutableColumnPtr mapUniqueIndexImplRef(PaddedPODArray<T> &index) {
  ...
  for (auto val : index) hash_map.insert({val, static_cast<T>(hash_map.size())});

  auto res_col = ColumnVector<T>::create();
  auto &data = res_col->getData();

  data.resize(hash_map.size());
  for (const auto &val : hash_map) data[val.getMapped()] = val.getKey();

  for (auto &ind : index) ind = hash_map[ind];
...
MutableColumnPtr mapUniqueIndexImpl(PaddedPODArray<T> &index) {
  if (index.empty()) return ColumnVector<T>::create();

  auto size = index.size();

  T max_val = index[0];
  for (size_t i = 1; i < size; ++i) max_val = std::max(max_val, index[i]);

  /// May happen when dictionary is shared.
  if (max_val > size) return mapUniqueIndexImplRef(index);
  ...
  for (size_t i = 0; i < map_size; ++i) {
    auto val = map[i];
    if (val) data[val] = static_cast<T>(i);
  }
...
MutableColumnPtr mapUniqueIndex(IColumn &column) {
  if (auto *data_uint8 = getIndexesData<UInt8>(column)) return mapUniqueIndexImpl(*data_uint8);
  if (auto *data_uint16 = getIndexesData<UInt16>(column)) return mapUniqueIndexImpl(*data_uint16);
  if (auto *data_uint32 = getIndexesData<UInt32>(column)) return mapUniqueIndexImpl(*data_uint32);
  if (auto *data_uint64 = getIndexesData<UInt64>(column)) return mapUniqueIndexImpl(*data_uint64);
...
bool ColumnLowCardinality::tryInsert(const Field &x) {
  ...
  size_t index;
  if (!dictionary.getColumnUnique().tryUniqueInsert(x, index)) return false;

  idx.insertIndex(index);
  return true;
}

void ColumnLowCardinality::insertDefault() { idx.insertIndex(getDictionary().getDefaultValueIndex()); }
...
#endif
{
  ...
  if (!low_cardinality_src) throw Exception(ErrorCodes::ILLEGAL_COLUMN, "Expected ColumnLowCardinality, got {}", src.getName());
...
static void checkIndexesAreLimited(const IColumn &indexes, UInt64 limit) {
  auto check_for_type = [&](auto type) {
    ...
    if (!column_ptr) return false;

    const auto &data = column_ptr->getData();
    size_t num_rows = data.size();
    if (num_rows == 0) return true;
    UInt64 max_position = 0;
    for (size_t i = 0; i < num_rows; ++i) max_position = std::max<UInt64>(max_position, data[i]);

    if (max_position >= limit)
      throw Exception(ErrorCodes::INCORRECT_DATA,
                      "Index for LowCardinality is out of range. Dictionary size is {}, "
                      "but found index with value {}",
                      limit, max_position);

    return true;
  };

  if (!check_for_type(UInt8()) && !check_for_type(UInt16()) && !check_for_type(UInt32()) && !check_for_type(UInt64()))
    throw Exception(ErrorCodes::ILLEGAL_COLUMN, "Invalid column for ColumnLowCardinality index. Expected UInt, got {}", indexes.getName());
}
...
std::string_view ColumnLowCardinality::serializeValueIntoArena(size_t n, Arena &arena, char const *&begin,
...
void ColumnLowCardinality::collectSerializedValueSizes(PaddedPODArray<UInt64> &sizes, const UInt8 *is_null,
                                                       const IColumn::SerializationSettings *settings) const {
...
void ColumnLowCardinality::skipSerializedInArena(ReadBuffer &in) const { getDictionary().skipSerializedInArena(in); }
...
MutableColumnPtr ColumnLowCardinality::cloneResized(size_t size) const {
  ...
  if (size == 0) unique_ptr = unique_ptr->cloneEmpty();
...
MutableColumnPtr ColumnLowCardinality::cloneNullable() const {
  if (nestedIsNullable()) return cloneFinalized();
...
int ColumnLowCardinality::compareAtImpl(size_t n, size_t m, const IColumn &rhs, int nan_direction_hint, const Collator *collator) const {
  ...
  if (collator)
    return getDictionary().getNestedColumn()->compareAtWithCollation(
        n_index, m_index, *low_cardinality_column.getDictionary().getNestedColumn(), nan_direction_hint, *collator);
...
int ColumnLowCardinality::compareAtWithCollation(size_t n, size_t m, const IColumn &rhs, int nan_direction_hint,
...
bool ColumnLowCardinality::hasEqualValues() const {
  if (getDictionary().size() <= 1) return true;
...
void ColumnLowCardinality::getPermutationImpl(IColumn::PermutationSortDirection direction, IColumn::PermutationSortStability stability,
  ...
  if (collator)
    getDictionary().getNestedColumn()->getPermutationWithCollation(*collator, direction, stability, unique_limit, nan_direction_hint,
                                                                   unique_perm);
  else
  ...
  for (size_t row = 0; row < indexes_size; ++row) indexes_per_row[getIndexes().getUInt(row)].push_back(row);
...
struct LowCardinalityComparator {
  ...
  inline bool operator()(size_t lhs, size_t rhs) const {
    ...
    if (stable && ret == 0) return lhs < rhs;
...
void ColumnLowCardinality::updatePermutationWithIndexType(IColumn::PermutationSortStability stability, size_t limit,
                                                          const PaddedPODArray<UInt64> &position_by_index, IColumn::Permutation &res,
  ...
  auto equal_comparator = [real_indexes](size_t lhs, size_t rhs) { return real_indexes->getUInt(lhs) == real_indexes->getUInt(rhs); };

  const bool stable = (stability == IColumn::PermutationSortStability::Stable);
  if (stable)
    updatePermutationImpl(limit, res, equal_ranges, LowCardinalityComparator<IndexColumn, true>{*real_indexes, position_by_index},
                          equal_comparator, DefaultSort(), DefaultPartialSort());
  else
    updatePermutationImpl(limit, res, equal_ranges, LowCardinalityComparator<IndexColumn, false>{*real_indexes, position_by_index},
                          equal_comparator, DefaultSort(), DefaultPartialSort());
}
...
void ColumnLowCardinality::updatePermutation(IColumn::PermutationSortDirection direction, IColumn::PermutationSortStability stability,
  ...
  if (dict_perm.size() != getDictionary().size())
    throw Exception(ErrorCodes::LOGICAL_ERROR, "Dictionary permutation size {} is equal to dictionary size {}. It is a bug.",
                    dict_perm.size(), getDictionary().size());
  PaddedPODArray<UInt64> position_by_index(dict_perm.size());
  for (size_t i = 0; i < dict_perm.size(); ++i) position_by_index[dict_perm[i]] = i;
...
void ColumnLowCardinality::getPermutationWithCollation(const Collator &collator, IColumn::PermutationSortDirection direction,
...
void ColumnLowCardinality::updatePermutationWithCollation(const Collator &collator, IColumn::PermutationSortDirection direction,
                                                          IColumn::PermutationSortStability stability, size_t limit, int nan_direction_hint,
  ...
  auto comparator = [this, &collator, ascending, stability, nan_direction_hint](size_t lhs, size_t rhs) {
    ...
    if (unlikely(stability == IColumn::PermutationSortStability::Stable && ret == 0)) return lhs < rhs;

    if (ascending) return ret < 0;
    return ret > 0;
  };

  auto equal_comparator = [this, &collator, nan_direction_hint](size_t lhs, size_t rhs) {
    int ret = getDictionary().getNestedColumn()->compareAtWithCollation(getIndexes().getUInt(lhs), getIndexes().getUInt(rhs),
                                                                        *getDictionary().getNestedColumn(), nan_direction_hint, collator);
    return ret == 0;
  };
...
void ColumnLowCardinality::compactIfSharedDictionary() {
  if (dictionary.isShared()) compactInplace();
}

ColumnLowCardinality::DictionaryEncodedColumn ColumnLowCardinality::getMinimalDictionaryEncodedColumn(UInt64 offset, UInt64 limit) const {
...
bool ColumnLowCardinality::containsNull() const { return getDictionary().nestedColumnIsNullable() && idx.containsDefault(); }
...
ColumnPtr ColumnLowCardinality::cloneWithDefaultOnNull() const {
  if (!nestedIsNullable()) return getPtr();
  ...
  while (start < end) {
    ...
    while (next_null_index < end && !isNullAt(next_null_index)) ++next_null_index;

    if (next_null_index != start) lc_res.insertRangeFrom(*this, start, next_null_index - start);

    if (next_null_index < end) lc_res.insertDefault();
...
bool isColumnLowCardinalityNullable(const IColumn &column) {
  if (const auto *lc_column = checkAndGetColumn<ColumnLowCardinality>(&column)) return lc_column->nestedIsNullable();
  return false;
}
