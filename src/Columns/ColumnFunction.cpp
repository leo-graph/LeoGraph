...
ColumnFunction::ColumnFunction(size_t size, FunctionBasePtr function_, const ColumnsWithTypeAndName &columns_to_capture,
                               bool is_short_circuit_argument_, bool is_function_compiled_,
...
MutableColumnPtr ColumnFunction::cloneResized(size_t size) const {
  ...
  for (auto &column : capture) column.column = column.column->cloneResized(size);
...
ColumnPtr ColumnFunction::replicate(const Offsets &offsets) const {
  if (elements_size != offsets.size())
    throw Exception(ErrorCodes::SIZES_OF_COLUMNS_DOESNT_MATCH, "Size of offsets ({}) doesn't match size of column ({})", offsets.size(),
                    elements_size);

  ColumnsWithTypeAndName capture = captured_columns;
  for (auto &column : capture) column.column = column.column->replicate(offsets);
...
ColumnPtr ColumnFunction::cut(size_t start, size_t length) const {
  ColumnsWithTypeAndName capture = captured_columns;
  for (auto &column : capture) column.column = column.column->cut(start, length);
...
void ColumnFunction::get(size_t n, Field &res) const {
  ...
  for (size_t i = 0; i < tuple_size; ++i) res_tuple.push_back((*captured_columns[i].column)[n]);
}
...
ColumnPtr ColumnFunction::filter(const Filter &filt, ssize_t result_size_hint) const {
  ...
  ColumnsWithTypeAndName capture = captured_columns;
  for (auto &column : capture) column.column = column.column->filter(filt, result_size_hint);
...
void ColumnFunction::filter(const Filter &filt) {
  ...
  for (auto &column : captured_columns) column.column->assumeMutable()->filter(filt);
...
ColumnPtr ColumnFunction::permute(const Permutation &perm, size_t limit) const {
  ...
  ColumnsWithTypeAndName capture = captured_columns;
  for (auto &column : capture) column.column = column.column->permute(perm, limit);
...
ColumnPtr ColumnFunction::index(const IColumn &indexes, size_t limit) const {
  ColumnsWithTypeAndName capture = captured_columns;
  for (auto &column : capture) column.column = column.column->index(indexes, limit);

  return ColumnFunction::create(limit == 0 ? indexes.size() : limit, function, capture, is_short_circuit_argument, is_function_compiled,
                                recursively_convert_result_to_full_column_if_low_cardinality);
}

std::vector<MutableColumnPtr> ColumnFunction::scatter(size_t num_columns, const IColumn::Selector &selector) const {
  if (elements_size != selector.size())
    throw Exception(ErrorCodes::SIZES_OF_COLUMNS_DOESNT_MATCH, "Size of selector ({}) doesn't match size of column ({})", selector.size(),
                    elements_size);

  std::vector<size_t> counts;
  if (captured_columns.empty()) counts = countColumnsSizeInSelector(num_columns, selector);
  ...
  for (size_t capture = 0; capture < captured_columns.size(); ++capture) {
    ...
    for (size_t part = 0; part < num_columns; ++part) captures[part][capture].column = std::move(parts[part]);
  }
  ...
  for (size_t part = 0; part < num_columns; ++part) {
    ...
    columns.emplace_back(ColumnFunction::create(capture_size, function, std::move(capture), is_short_circuit_argument, is_function_compiled,
                                                recursively_convert_result_to_full_column_if_low_cardinality));
  }
...
size_t ColumnFunction::byteSize() const {
  size_t total_size = 0;
  for (const auto &column : captured_columns) total_size += column.column->byteSize();
...
size_t ColumnFunction::byteSizeAt(size_t n) const {
  size_t total_size = 0;
  for (const auto &column : captured_columns) total_size += column.column->byteSizeAt(n);
...
size_t ColumnFunction::allocatedBytes() const {
  size_t total_size = 0;
  for (const auto &column : captured_columns) total_size += column.column->allocatedBytes();
...
void ColumnFunction::updateHashWithValue(size_t n, SipHash &hash) const {
  ...
  for (const auto &column : captured_columns) column.column->updateHashWithValue(n, hash);
}

WeakHash32 ColumnFunction::getWeakHash32() const {
  WeakHash32 hash(elements_size);
  for (const auto &column : captured_columns) hash.update(column.column->getWeakHash32());
  return hash;
}

void ColumnFunction::updateHashFast(SipHash &hash) const {
  hash.update(function->getName());
  for (const auto &column : captured_columns) column.column->updateHashFast(hash);
}
...
void ColumnFunction::appendArguments(const ColumnsWithTypeAndName &columns) {
  ...
  if (were_captured + wanna_capture > args)
    throw Exception(ErrorCodes::LOGICAL_ERROR, "Cannot capture {} column(s) because function {} has {} arguments{}.", wanna_capture,
                    function->getName(), args,
                    (were_captured ? " and " + toString(were_captured) + " columns have already been captured" : ""));

  for (const auto &column : columns) appendArgument(column);
}
...
void ColumnFunction::appendArgument(const ColumnWithTypeAndName &column) {
  ...
  if (!is_short_circuit_argument && !column.type->equals(*argument_types[index]))
    throw Exception(ErrorCodes::LOGICAL_ERROR,
                    ...
                    "got {}, but {} is expected.",
                    argument_types.size(), column.type->getName(), argument_types[index]->getName());
...
DataTypePtr ColumnFunction::getResultType() const {
  if (recursively_convert_result_to_full_column_if_low_cardinality) return recursiveRemoveLowCardinality(function->getResultType());
...
ColumnWithTypeAndName ColumnFunction::reduce() const {
  ...
  if (args != captured)
    throw Exception(ErrorCodes::LOGICAL_ERROR,
                    "Cannot call function {} because is has {} "
  ...
  if (is_short_circuit_argument) {
    ...
    if (function->isShortCircuit(settings, args)) {
      for (size_t i : settings.arguments_with_disabled_lazy_execution) {
        if (const ColumnFunction *arg = checkAndGetShortCircuitArgument(columns[i].column)) columns[i] = arg->reduce();
      }
    } else {
      for (auto &col : columns) {
        if (const ColumnFunction *arg = checkAndGetShortCircuitArgument(col.column)) col = arg->reduce();
      }
    }
  }
  ...
  if (is_function_compiled) ProfileEvents::increment(ProfileEvents::CompiledFunctionExecute);
  ...
  if (res.column->getDataType() != res.type->getColumnType())
    throw Exception(ErrorCodes::LOGICAL_ERROR, "Unexpected return type from {}. Expected {}. Got {}", function->getName(),
                    res.type->getColumnType(), res.column->getDataType());
...
void ColumnFunction::forEachSubcolumn(ColumnCallback callback) const {
  for (const auto &column : captured_columns) callback(column.column);
}
