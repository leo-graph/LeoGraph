...
ColumnReplicated::ColumnReplicated(MutableColumnPtr &&nested_column_, MutableColumnPtr &&indexes_)
    : nested_column(std::move(nested_column_)), indexes(std::move(indexes_)), id(global_id_counter.fetch_add(1)) {}
...
MutableColumnPtr ColumnReplicated::cloneResized(size_t new_size) const {
  ...
  if (new_size == size()) return create(mutate(nested_column), mutate(indexes.getIndexes()));
...
MutableColumnPtr ColumnReplicated::cloneEmpty() const { return create(nested_column->cloneEmpty(), indexes.getIndexes()->cloneEmpty()); }

bool ColumnReplicated::isDefaultAt(size_t n) const { return nested_column->isDefaultAt(indexes.getIndexAt(n)); }

bool ColumnReplicated::isNullAt(size_t n) const { return nested_column->isNullAt(indexes.getIndexAt(n)); }

Field ColumnReplicated::operator[](size_t n) const { return (*nested_column)[indexes.getIndexAt(n)]; }

void ColumnReplicated::get(size_t n, Field &res) const { nested_column->get(indexes.getIndexAt(n), res); }
...
bool ColumnReplicated::getBool(size_t n) const { return nested_column->getBool(indexes.getIndexAt(n)); }

Float64 ColumnReplicated::getFloat64(size_t n) const { return nested_column->getFloat64(indexes.getIndexAt(n)); }

Float32 ColumnReplicated::getFloat32(size_t n) const { return nested_column->getFloat32(indexes.getIndexAt(n)); }

UInt64 ColumnReplicated::getUInt(size_t n) const { return nested_column->getUInt(indexes.getIndexAt(n)); }

Int64 ColumnReplicated::getInt(size_t n) const { return nested_column->getInt(indexes.getIndexAt(n)); }

UInt64 ColumnReplicated::get64(size_t n) const { return nested_column->get64(indexes.getIndexAt(n)); }

std::string_view ColumnReplicated::getDataAt(size_t n) const { return nested_column->getDataAt(indexes.getIndexAt(n)); }

ColumnPtr ColumnReplicated::convertToFullColumnIfReplicated() const { return nested_column->index(*indexes.getIndexes(), 0); }
...
std::string_view ColumnReplicated::serializeValueIntoArena(size_t n, Arena &arena, char const *&begin,
...
void ColumnReplicated::skipSerializedInArena(ReadBuffer &in) const { nested_column->skipSerializedInArena(in); }
...
#endif
{
  ...
  if (const auto *src_replicated = typeid_cast<const ColumnReplicated *>(&src)) {
    ...
    if (start == 0 && length == src_replicated->size()) {
      indexes.insertIndexesRangeWithShift(*src_replicated->getIndexesColumn(), start, length, nested_column->size(),
                                          nested_column->size() + src_replicated->getNestedColumn()->size());
...
void ColumnReplicated::expand(const Filter &mask, bool inverted) { indexes.expand(mask, inverted); }

ColumnPtr ColumnReplicated::permute(const Permutation &perm, size_t limit) const {
  if (size() != perm.size())
    throw Exception(ErrorCodes::SIZES_OF_COLUMNS_DOESNT_MATCH, "Size of permutation ({}) doesn't match size of column ({})", perm.size(),
                    size());
...
#endif
{
  if (const auto *rhs_replicated = typeid_cast<const ColumnReplicated *>(&rhs))
    return nested_column->compareAt(indexes.getIndexAt(n), rhs_replicated->indexes.getIndexAt(m), *rhs_replicated->nested_column,
                                    nan_direction_hint);

  return nested_column->compareAt(indexes.getIndexAt(n), m, rhs, nan_direction_hint);
}

int ColumnReplicated::compareAtWithCollation(size_t n, size_t m, const IColumn &rhs, int nan_direction_hint,
                                             const Collator &collator) const {
  if (const auto *rhs_replicated = typeid_cast<const ColumnReplicated *>(&rhs))
    return nested_column->compareAtWithCollation(indexes.getIndexAt(n), rhs_replicated->indexes.getIndexAt(m),
                                                 *rhs_replicated->nested_column, nan_direction_hint, collator);

  return nested_column->compareAtWithCollation(indexes.getIndexAt(n), m, rhs, nan_direction_hint, collator);
}

bool ColumnReplicated::hasEqualValues() const { return indexes.getIndexes()->hasEqualValues(); }

struct ColumnReplicated::ComparatorBase {
  const ColumnReplicated &parent;
  int nan_direction_hint;

  ComparatorBase(const ColumnReplicated &parent_, int nan_direction_hint_) : parent(parent_), nan_direction_hint(nan_direction_hint_) {}
...
void ColumnReplicated::getPermutation(PermutationSortDirection direction, PermutationSortStability stability, size_t limit,
...
void ColumnReplicated::updatePermutation(PermutationSortDirection direction, PermutationSortStability stability, size_t limit,
...
void ColumnReplicated::getPermutationWithCollation(const Collator &collator, PermutationSortDirection direction,
...
void ColumnReplicated::updatePermutationWithCollation(const Collator &collator, PermutationSortDirection direction,
                                                      PermutationSortStability stability, size_t limit, int nan_direction_hint,
                                                      Permutation &res, EqualRanges &equal_ranges) const {
...
size_t ColumnReplicated::byteSize() const { return indexes.getIndexes()->byteSize() + nested_column->byteSize(); }

size_t ColumnReplicated::byteSizeAt(size_t n) const { return nested_column->byteSizeAt(indexes.getIndexAt(n)); }

size_t ColumnReplicated::allocatedBytes() const { return indexes.getIndexes()->allocatedBytes() + nested_column->allocatedBytes(); }
...
void ColumnReplicated::getIndicesOfNonDefaultRows(Offsets &result_indexes, size_t from, size_t limit) const {
  ...
  for (size_t i = 0; i != nested_column->size(); ++i) default_values_mask[i] = !nested_column->isDefaultAt(i);
...
UInt64 ColumnReplicated::getNumberOfDefaultRows() const {
  ...
  for (size_t i = 0; i != nested_column->size(); ++i) {
    if (nested_column->isDefaultAt(i)) indexes_of_default_values.insert(i);
  }

  size_t result = 0;
  auto add = [&](size_t, size_t index) { result += indexes_of_default_values.contains(index); };
...
ColumnPtr convertOffsetsToIndexesImpl(const IColumn::Offsets &offsets) {
  ...
  for (size_t i = 0; i != offsets.size(); ++i) data.resize_fill(data.size() + offsets[i] - offsets[i - 1], static_cast<T>(i));
  return result;
}
...
ColumnPtr convertOffsetsToIndexes(const IColumn::Offsets &offsets) {
  ...
  if (max_index <= std::numeric_limits<UInt8>::max()) return convertOffsetsToIndexesImpl<UInt8>(offsets);
  if (max_index <= std::numeric_limits<UInt16>::max()) return convertOffsetsToIndexesImpl<UInt16>(offsets);
  if (max_index <= std::numeric_limits<UInt32>::max()) return convertOffsetsToIndexesImpl<UInt32>(offsets);
