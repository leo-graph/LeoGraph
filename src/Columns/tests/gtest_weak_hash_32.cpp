...
template <typename T>
void checkColumnImpl(const WeakHash32::Container& hash, const PaddedPODArray<T>& eq_class, UInt32 scale = 0) {
...
template <typename T>
void checkColumn(const WeakHash32::Container& hash, const PaddedPODArray<T>& eq_class) {
  checkColumnImpl(hash, eq_class);
}

template <is_decimal T>
void checkColumn(const WeakHash32::Container& hash, const PaddedPODArray<T>& eq_class, UInt32 scale) {
...
TEST(WeakHash32, ColumnVectorU8) {
  ...
  for (int idx [[maybe_unused]] : {1, 2}) {
    for (size_t i = 0; i < 265; ++i) data.push_back(static_cast<UInt8>(i));
  }
...
TEST(WeakHash32, ColumnVectorI8) {
  ...
  for (int idx [[maybe_unused]] : {1, 2}) {
    for (int i = -128; i < 128; ++i) data.push_back(static_cast<Int8>(i));
  }
...
TEST(WeakHash32, ColumnVectorU16) {
  ...
  for (int idx [[maybe_unused]] : {1, 2}) {
    for (size_t i = 0; i < 65536; ++i) data.push_back(static_cast<UInt16>(i));
  }
...
TEST(WeakHash32, ColumnVectorI16) {
  ...
  for (int idx [[maybe_unused]] : {1, 2}) {
    for (int i = -32768; i < 32768; ++i) data.push_back(static_cast<Int16>(i));
  }
...
TEST(WeakHash32, ColumnVectorU32) {
  ...
  for (int idx [[maybe_unused]] : {1, 2}) {
    for (uint32_t i = 0; i < 65536; ++i) data.push_back(i << 16u);
  }
...
TEST(WeakHash32, ColumnVectorI32) {
  ...
  for (int idx [[maybe_unused]] : {1, 2}) {
    for (int32_t i = -32768; i < 32768; ++i) data.push_back(i << 16);
  }
...
TEST(WeakHash32, ColumnVectorU64) {
  ...
  for (int idx [[maybe_unused]] : {1, 2}) {
    for (uint64_t i = 0; i < 65536; ++i) data.push_back(i << 32u);
  }
...
TEST(WeakHash32, ColumnVectorI64) {
  ...
  for (int idx [[maybe_unused]] : {1, 2}) {
    for (int64_t i = -32768; i < 32768; ++i) data.push_back(i << 32);
  }
...
TEST(WeakHash32, ColumnArray) {
  ...
  for (int idx [[maybe_unused]] : {1, 2}) {
    ...
    for (int64_t i = 0; i < 65536; ++i) {
      ...
      for (size_t j = 0; j < s; ++j) val_data.push_back(cur);
  ...
  for (int idx [[maybe_unused]] : {1, 2}) {
    ...
    for (int64_t i = 0; i < 3000; ++i) {
      ...
      for (size_t j = 0; j < s; ++j) {
        for (size_t k = 0; k < cur; ++k) val_data.push_back(0);
...
TEST(WeakHash32, ColumnConst) {
  ...
  for (size_t i = 0; i < 256; ++i) data.push_back(static_cast<UInt8>(0));
