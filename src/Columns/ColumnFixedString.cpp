...
MutableColumnPtr ColumnFixedString::cloneResized(size_t size) const {
  ...
  if (size > 0) {
    ...
    if (size > count) memset(&(new_col.chars[count * n]), '\0', (size - count) * n);
  }
...
void ColumnFixedString::getValueNameImpl(WriteBufferFromOwnString &name_buf, size_t index, const Options &options) const {
  if (options.notFull(name_buf)) writeQuoted(std::string_view{reinterpret_cast<const char *>(&chars[n * index]), n}, name_buf);
}
...
bool ColumnFixedString::tryInsert(const Field &x) {
  if (x.getType() != Field::Types::Which::String) return false;
  const String &s = x.safeGet<String>();
...
#endif
{
  ...
  if (n != src.getN()) throw Exception(ErrorCodes::SIZE_OF_FIXED_STRING_DOESNT_MATCH, "Size of FixedString doesn't match");
...
#endif
{
  ...
  if (n != src_concrete.getN()) throw Exception(ErrorCodes::SIZE_OF_FIXED_STRING_DOESNT_MATCH, "Size of FixedString doesn't match");
...
void ColumnFixedString::insertData(const char *pos, size_t length) {
  if (length > n) throw Exception(ErrorCodes::TOO_LARGE_STRING_SIZE, "Too large string for FixedString column");
...
void ColumnFixedString::skipSerializedInArena(ReadBuffer &in) const { in.ignore(n); }
...
struct ColumnFixedString::ComparatorBase {
  ...
  explicit ComparatorBase(const ColumnFixedString &parent_) : parent(parent_) {}
...
ColumnPtr ColumnFixedString::filter(const IColumn::Filter &filt, ssize_t result_size_hint) const {
  ...
  if (result_size_hint) res->chars.reserve_exact(result_size_hint > 0 ? result_size_hint * n : chars.size());
...
void ColumnFixedString::expand(const IColumn::Filter &mask, bool inverted) {
  if (mask.size() < size()) throw Exception(ErrorCodes::LOGICAL_ERROR, "Mask size should be no less than data size.");
...
ColumnPtr ColumnFixedString::permute(const Permutation &perm, size_t limit) const { return permuteImpl(*this, perm, limit); }

ColumnPtr ColumnFixedString::index(const IColumn &indexes, size_t limit) const { return selectIndexImpl(*this, indexes, limit); }

template <typename Type>
ColumnPtr ColumnFixedString::indexImpl(const PaddedPODArray<Type> &indexes, size_t limit) const {
  assert(limit <= indexes.size());
  if (limit == 0) return ColumnFixedString::create(n);
  ...
  size_t offset = 0;
  for (size_t i = 0; i < limit; ++i, offset += n) memcpySmallAllowReadWriteOverflow15(&res_chars[offset], &chars[indexes[i] * n], n);
...
ColumnPtr ColumnFixedString::replicate(const Offsets &offsets) const {
  ...
  if (col_size == 0 || offsets.back() == 0) return res;
...
ColumnPtr ColumnFixedString::compress(bool force_compression) const {
  ...
  return ColumnCompressed::create(column_size, compressed_size, [my_compressed = std::move(compressed), column_size, my_n = n] {
...
void ColumnFixedString::updateAt(const IColumn &src, size_t dst_pos, size_t src_pos) {
  ...
  if (n != src_fixed.getN()) throw Exception(ErrorCodes::LOGICAL_ERROR, "Size of FixedString doesn't match");
