...
std::string ColumnMap::getName() const {
  ...
  res << "Map(" << nested_tuple.getColumn(0).getName() << ", " << nested_tuple.getColumn(1).getName() << ")";

  return res.str();
}

ColumnMap::ColumnMap(MutableColumnPtr &&nested_) : nested(std::move(nested_)) {
  const auto *column_array = typeid_cast<const ColumnArray *>(nested.get());
  if (!column_array) throw Exception(ErrorCodes::LOGICAL_ERROR, "ColumnMap can be created only from array of tuples");

  const auto *column_tuple = typeid_cast<const ColumnTuple *>(column_array->getDataPtr().get());
  if (!column_tuple) throw Exception(ErrorCodes::LOGICAL_ERROR, "ColumnMap can be created only from array of tuples");
  ...
  for (const auto &column : column_tuple->getColumns())
    if (isColumnConst(*column)) throw Exception(ErrorCodes::ILLEGAL_COLUMN, "ColumnMap cannot have ColumnConst as its element");
}

MutableColumnPtr ColumnMap::cloneEmpty() const { return ColumnMap::create(nested->cloneEmpty()); }

MutableColumnPtr ColumnMap::cloneResized(size_t new_size) const { return ColumnMap::create(nested->cloneResized(new_size)); }
...
void ColumnMap::get(size_t n, Field &res) const {
  ...
  for (size_t i = 0; i < size; ++i) map.push_back(getNestedData()[offset + i]);
}
...
bool ColumnMap::isDefaultAt(size_t n) const { return nested->isDefaultAt(n); }
...
bool ColumnMap::tryInsert(const Field &x) {
  if (x.getType() != Field::Types::Which::Map) return false;

  const auto &map = x.safeGet<Map>();
  return nested->tryInsert(Array(map.begin(), map.end()));
}

void ColumnMap::insertDefault() { nested->insertDefault(); }
void ColumnMap::popBack(size_t n) { nested->popBack(n); }

std::string_view ColumnMap::serializeValueIntoArena(size_t n, Arena &arena, char const *&begin,
...
void ColumnMap::skipSerializedInArena(ReadBuffer &in) const { nested->skipSerializedInArena(in); }

void ColumnMap::updateHashWithValue(size_t n, SipHash &hash) const { nested->updateHashWithValue(n, hash); }

WeakHash32 ColumnMap::getWeakHash32() const { return nested->getWeakHash32(); }

void ColumnMap::updateHashFast(SipHash &hash) const { nested->updateHashFast(hash); }
...
#endif
{
  nested->insertRangeFrom(assert_cast<const ColumnMap &>(src).getNestedColumn(), start, length);
}
...
void ColumnMap::filter(const Filter &filt) { nested->filter(filt); }

void ColumnMap::expand(const IColumn::Filter &mask, bool inverted) { nested->expand(mask, inverted); }
...
MutableColumns ColumnMap::scatter(size_t num_columns, const Selector &selector) const {
  ...
  for (auto &&scattered : scattered_columns) res.push_back(ColumnMap::create(std::move(scattered)));
...
void ColumnMap::getPermutation(IColumn::PermutationSortDirection direction, IColumn::PermutationSortStability stability, size_t limit,
...
void ColumnMap::updatePermutation(IColumn::PermutationSortDirection direction, IColumn::PermutationSortStability stability, size_t limit,
...
void ColumnMap::reserve(size_t n) { nested->reserve(n); }

size_t ColumnMap::capacity() const { return nested->capacity(); }
...
void ColumnMap::shrinkToFit() { nested->shrinkToFit(); }

void ColumnMap::ensureOwnership() { nested->ensureOwnership(); }

size_t ColumnMap::byteSize() const { return nested->byteSize(); }

size_t ColumnMap::byteSizeAt(size_t n) const { return nested->byteSizeAt(n); }

size_t ColumnMap::allocatedBytes() const { return nested->allocatedBytes(); }

void ColumnMap::protect() { nested->protect(); }
...
ColumnCheckpointPtr ColumnMap::getCheckpoint() const { return nested->getCheckpoint(); }

void ColumnMap::updateCheckpoint(ColumnCheckpoint &checkpoint) const { nested->updateCheckpoint(checkpoint); }

void ColumnMap::rollback(const ColumnCheckpoint &checkpoint) { nested->rollback(checkpoint); }

void ColumnMap::forEachMutableSubcolumn(MutableColumnCallback callback) { callback(nested); }
...
void ColumnMap::forEachSubcolumn(ColumnCallback callback) const { callback(nested); }
...
bool ColumnMap::structureEquals(const IColumn &rhs) const {
  if (const auto *rhs_map = typeid_cast<const ColumnMap *>(&rhs)) return nested->structureEquals(*rhs_map->nested);
  return false;
}

bool ColumnMap::dynamicStructureEquals(const IColumn &rhs) const {
  if (const auto *rhs_map = typeid_cast<const ColumnMap *>(&rhs)) return nested->dynamicStructureEquals(*rhs_map->nested);
  return false;
}

const ColumnArray &ColumnMap::getNestedColumn() const { return assert_cast<const ColumnArray &>(*nested); }

ColumnArray &ColumnMap::getNestedColumn() { return assert_cast<ColumnArray &>(*nested); }

const ColumnTuple &ColumnMap::getNestedData() const { return assert_cast<const ColumnTuple &>(getNestedColumn().getData()); }

ColumnTuple &ColumnMap::getNestedData() { return assert_cast<ColumnTuple &>(getNestedColumn().getData()); }
...
ColumnPtr ColumnMap::compress(bool force_compression) const {
  ...
  return ColumnCompressed::create(size(), byte_size,
                                  [my_compressed = std::move(compressed)] { return ColumnMap::create(my_compressed->decompress()); });
}
