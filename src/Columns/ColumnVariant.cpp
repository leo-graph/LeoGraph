...
void ColumnVariant::constructOffsetsFromDiscriminators() {
  ...
  /// offsets column will contain just sequential offsets 0, 1, 2, ...
    ...
    for (size_t i = 0; i != discriminators_concrete->size(); ++i) offsets_data.push_back(i);
  }
...
ColumnVariant::ColumnVariant(MutableColumns &&variants_, const std::vector<Discriminator> &local_to_global_discriminators_) {
  ...
  if (!local_to_global_discriminators_.empty() && local_to_global_discriminators_.size() != variants_.size())
    throw Exception(ErrorCodes::LOGICAL_ERROR,
                    ...
                    local_to_global_discriminators_.size(), variants_.size());

  /// As variants are empty, column with local discriminators will be also empty and we can reorder variants according to global
  /// discriminators.
  variants.resize(variants_.size());
  for (size_t i = 0; i != variants_.size(); ++i) {
    if (isColumnConst(*variants_[i])) throw Exception(ErrorCodes::ILLEGAL_COLUMN, "ColumnVariant cannot have ColumnConst as its element");
    ...
    if (!local_to_global_discriminators_.empty() && local_to_global_discriminators_[i] > variants_.size())
      throw Exception(ErrorCodes::LOGICAL_ERROR, "Invalid global discriminator {}. The number of variants: {}",
                      UInt64(local_to_global_discriminators_[i]), variants_.size());
...
ColumnVariant::ColumnVariant(MutableColumnPtr local_discriminators_, MutableColumns &&variants_)
    : ColumnVariant(std::move(local_discriminators_), nullptr, std::move(variants_), {}) {}

ColumnVariant::ColumnVariant(MutableColumnPtr local_discriminators_, MutableColumns &&variants_,
                             const std::vector<Discriminator> &local_to_global_discriminators_)
    : ColumnVariant(std::move(local_discriminators_), nullptr, std::move(variants_), local_to_global_discriminators_) {}

ColumnVariant::ColumnVariant(DB::MutableColumnPtr local_discriminators_, DB::MutableColumnPtr offsets_, DB::MutableColumns &&variants_)
    : ColumnVariant(std::move(local_discriminators_), std::move(offsets_), std::move(variants_), {}) {}

ColumnVariant::ColumnVariant(DB::MutableColumnPtr local_discriminators_, DB::MutableColumnPtr offsets_, DB::MutableColumns &&variants_,
                             const std::vector<Discriminator> &local_to_global_discriminators_) {
  if (variants_.size() > MAX_NESTED_COLUMNS)
    throw Exception(ErrorCodes::BAD_ARGUMENTS, "Variant type with more than {} nested types is not allowed",
                    ColumnVariant::MAX_NESTED_COLUMNS);

  local_discriminators = std::move(local_discriminators_);
  const ColumnDiscriminators *discriminators_concrete = typeid_cast<const ColumnDiscriminators *>(local_discriminators.get());
  if (!discriminators_concrete) throw Exception(ErrorCodes::LOGICAL_ERROR, "discriminator column must be a ColumnUInt8");

  variants.reserve(variants_.size());
  size_t total_size = 0;
  for (auto &variant : variants_) {
    if (isColumnConst(*variant)) throw Exception(ErrorCodes::ILLEGAL_COLUMN, "ColumnVariant cannot have ColumnConst as its element");
  ...
  if (total_size > local_discriminators->size())
    throw Exception(ErrorCodes::LOGICAL_ERROR,
                    "Nested columns sizes are inconsistent with local_discriminators column size. Total column sizes: {}, "
                    "local_discriminators size: {}",
                    total_size, local_discriminators->size());
  ...
  if (local_to_global_discriminators_.empty()) {
    ...
    if (local_to_global_discriminators_.size() != variants.size())
      throw Exception(ErrorCodes::LOGICAL_ERROR,
                      "The number of values in local_to_global_discriminators mapping ({}) doesn't match the number of variants ({})",
                      local_to_global_discriminators_.size(), variants.size());
    ...
    for (Discriminator i = 0; i != local_to_global_discriminators.size(); ++i) {
      if (local_to_global_discriminators[i] > variants.size())
        throw Exception(ErrorCodes::LOGICAL_ERROR, "Invalid global discriminator {}. The number of variants: {}",
                        UInt64(local_to_global_discriminators[i]), variants_.size());
...
ColumnVariant::Ptr ColumnVariant::create(const DB::ColumnPtr &local_discriminators, const DB::Columns &variants,
                                         const std::vector<Discriminator> &local_to_global_discriminators) {
...
ColumnVariant::Ptr ColumnVariant::create(const DB::ColumnPtr &local_discriminators, const DB::ColumnPtr &offsets,
                                         const DB::Columns &variants, const std::vector<Discriminator> &local_to_global_discriminators) {
  return ColumnVariant::create(local_discriminators->assumeMutable(), offsets->assumeMutable(), getVariantsAssumeMutable(variants),
...
MutableColumnPtr ColumnVariant::cloneEmpty() const {
  ...
  for (const auto &variant : variants) new_variants.emplace_back(variant->cloneEmpty());
...
MutableColumnPtr ColumnVariant::cloneResized(size_t new_size) const {
  if (new_size == 0) return cloneEmpty();
  ...
  if (new_size >= size) {
    ...
    for (const auto &variant : variants) new_variants.emplace_back(IColumn::mutate(variant));

    auto res = ColumnVariant::create(IColumn::mutate(local_discriminators), IColumn::mutate(offsets), std::move(new_variants),
                                     local_to_global_discriminators);
  ...
  if (hasOnlyNulls()) {
    ...
    return ColumnVariant::create(local_discriminators->cloneResized(new_size), offsets->cloneResized(new_size), std::move(new_variants),
                                 local_to_global_discriminators);
  }
  ...
  if (2 * new_size <= size) {
    for (ssize_t i = new_size - 1; i > -1; --i) {
      ...
      if (discr != NULL_DISCRIMINATOR) {
        ...
        if (!seen_variants[discr]) {
          ...
          if (number_of_seen_variants == num_variants) break;
        }
      }
    }
    ...
    if (number_of_seen_variants != num_variants) {
      for (size_t discr = 0; discr != num_variants; ++discr)
        if (!seen_variants[discr]) new_variants[discr] = variants[discr]->cloneEmpty();
    }
  }
  /// [new_size, size)
  else {
    ...
    if (number_of_seen_variants != num_variants) {
      ...
      for (size_t discr = 0; discr != num_variants; ++discr)
        if (!seen_variants[discr]) new_variants[discr] = IColumn::mutate(variants[discr]);
    }
  }
...
Field ColumnVariant::operator[](size_t n) const {
  ...
  if (discr == NULL_DISCRIMINATOR) return Null();
...
void ColumnVariant::getValueNameImpl(WriteBufferFromOwnString &name_buf, size_t n, const Options &options) const {
  Discriminator discr = localDiscriminatorAt(n);
  if (discr == NULL_DISCRIMINATOR) {
    if (options.notFull(name_buf)) name_buf << "NULL";
    return;
  }
...
bool ColumnVariant::isDefaultAt(size_t n) const { return localDiscriminatorAt(n) == NULL_DISCRIMINATOR; }

bool ColumnVariant::isNullAt(size_t n) const { return localDiscriminatorAt(n) == NULL_DISCRIMINATOR; }
...
void ColumnVariant::insert(const Field &x) {
  if (!tryInsert(x)) throw Exception(ErrorCodes::NOT_IMPLEMENTED, "Cannot insert field {} into column {}", fieldToString(x), getName());
}
...
void ColumnVariant::insertFromImpl(const DB::IColumn &src_, size_t n,
                                   const std::vector<ColumnVariant::Discriminator> *global_discriminators_mapping) {
  const size_t num_variants = variants.size();
...
void ColumnVariant::insertRangeFromImpl(const DB::IColumn &src_, size_t start, size_t length,
                                        const std::vector<ColumnVariant::Discriminator> *global_discriminators_mapping,
                                        const Discriminator *skip_discriminator) {
  const size_t num_variants = variants.size();
  ...
  if (start + length > src.getLocalDiscriminators().size())
    throw Exception(ErrorCodes::PARAMETER_OUT_OF_BOUND,
                    "Parameter out of bound in ColumnVariant::insertRangeFrom method. "
                    "[start({}) + length({}) > local_discriminators.size({})]",
                    start, length, src.getLocalDiscriminators().size());
  ...
  if (auto non_empty_src_local_discr = src.getLocalDiscriminatorOfOneNoneEmptyVariantNoNulls()) {
    ...
    for (size_t i = 0; i != length; ++i) offsets_data.push_back(offset++);
    return;
  }
  ...
  for (size_t i = start; i != start + length; ++i) {
    ...
    if (local_discr == NULL_DISCRIMINATOR) {
      ...
      if (!nested_ranges[src_local_discr].second) nested_ranges[src_local_discr].first = src_offsets_data[i];
  ...
  for (Discriminator src_local_discr = 0; src_local_discr != nested_ranges.size(); ++src_local_discr) {
    ...
    if (!skip_discriminator || global_discr != *skip_discriminator) {
      Discriminator local_discr = localDiscriminatorByGlobal(global_discr);
      if (nested_length) variants[local_discr]->insertRangeFrom(*src.variants[src_local_discr], nested_start, nested_length);
    }
  }
}

void ColumnVariant::insertManyFromImpl(const DB::IColumn &src_, size_t position, size_t length,
                                       const std::vector<ColumnVariant::Discriminator> *global_discriminators_mapping) {
  const size_t num_variants = variants.size();
  const auto &src = assert_cast<const ColumnVariant &>(src_);
  if (!global_discriminators_mapping && src.variants.size() != num_variants)
  ...
  if (local_discr == NULL_DISCRIMINATOR) {
    ...
    offsets_data.reserve(offsets_data.size() + length);
    for (size_t i = 0; i != length; ++i) offsets_data.push_back(prev_offset + i);
...
void ColumnVariant::insertFrom(const DB::IColumn &src_, size_t n,
                               const std::vector<ColumnVariant::Discriminator> &global_discriminators_mapping) {
...
void ColumnVariant::insertRangeFrom(const IColumn &src_, size_t start, size_t length,
                                    const std::vector<ColumnVariant::Discriminator> &global_discriminators_mapping,
                                    Discriminator skip_discriminator) {
...
void ColumnVariant::insertRangeIntoVariantFrom(DB::ColumnVariant::Discriminator global_discr, const DB::IColumn &src_, size_t start,
                                               size_t length) {
  Discriminator local_discr = localDiscriminatorByGlobal(global_discr);
  auto &local_discriminators_data = getLocalDiscriminators();
  local_discriminators_data.resize_fill(local_discriminators_data.size() + length, local_discr);
  ...
  size_t offset = variants[local_discr]->size();
  offsets_data.reserve(offsets_data.size() + length);
  for (size_t i = 0; i != length; ++i) offsets_data.push_back(offset + i);

  variants[local_discr]->insertRangeFrom(src_, start, length);
}

void ColumnVariant::insertManyIntoVariantFrom(DB::ColumnVariant::Discriminator global_discr, const DB::IColumn &src_, size_t position,
                                              size_t length) {
  Discriminator local_discr = localDiscriminatorByGlobal(global_discr);
  auto &local_discriminators_data = getLocalDiscriminators();
  local_discriminators_data.resize_fill(local_discriminators_data.size() + length, local_discr);
...
void ColumnVariant::deserializeBinaryIntoVariant(ColumnVariant::Discriminator global_discr, const SerializationPtr &serialization,
...
void ColumnVariant::popBack(size_t n) {
  if (n > size()) throw Exception(ErrorCodes::LOGICAL_ERROR, "Cannot pop {} rows from {}: there are only {} rows", n, getName(), size());
  ...
  for (size_t i = size - n; i < size; ++i) {
    Discriminator discr = local_discriminators_data[i];
    if (discr != NULL_DISCRIMINATOR) ++nested_n[discr];
  }

  for (size_t i = 0; i != num_variants; ++i) {
    if (nested_n[i]) variants[i]->popBack(nested_n[i]);
  }
...
ColumnCheckpointPtr ColumnVariant::getCheckpoint() const {
  ...
  for (const auto &column : variants) checkpoints.push_back(column->getCheckpoint());
...
void ColumnVariant::updateCheckpoint(ColumnCheckpoint &checkpoint) const {
  ...
  for (size_t i = 0; i < variants.size(); ++i) variants[i]->updateCheckpoint(*checkpoints[i]);
}
...
void ColumnVariant::rollback(const ColumnCheckpoint &checkpoint) {
  ...
  for (size_t i = 0; i < variants.size(); ++i) variants[i]->rollback(*checkpoints[i]);
}

std::string_view ColumnVariant::serializeValueIntoArena(size_t n, Arena &arena, char const *&begin,
  ...
  if (global_discr == NULL_DISCRIMINATOR) return res;
...
void ColumnVariant::skipSerializedInArena(ReadBuffer &in) const {
  ...
  if (global_discr == NULL_DISCRIMINATOR) return;
...
char *ColumnVariant::serializeValueIntoMemory(size_t n, char *memory, const IColumn::SerializationSettings *settings) const {
  ...
  if (global_discr == NULL_DISCRIMINATOR) return memory;
...
std::optional<size_t> ColumnVariant::getSerializedValueSize(size_t n, const IColumn::SerializationSettings *settings) const {
  ...
  if (!variant_size) return std::nullopt;
...
void ColumnVariant::updateHashWithValue(size_t n, SipHash &hash) const {
  ...
  if (global_discr != NULL_DISCRIMINATOR) variants[localDiscriminatorByGlobal(global_discr)]->updateHashWithValue(offsetAt(n), hash);
}
...
WeakHash32 ColumnVariant::getWeakHash32() const {
  ...
  if (hasOnlyNulls()) return WeakHash32(s);
  ...
  for (const auto &variant : variants) nested_hashes.emplace_back(variant->getWeakHash32());
  ...
  for (size_t i = 0; i != local_discriminators_data.size(); ++i) {
    ...
    if (discr != NULL_DISCRIMINATOR) hash_data[i] = nested_hashes[discr].getData()[offsets_data[i]];
  }
...
void ColumnVariant::updateHashFast(SipHash &hash) const {
  ...
  for (const auto &variant : variants) variant->updateHashFast(hash);
}

ColumnPtr ColumnVariant::filter(const Filter &filt, ssize_t result_size_hint) const {
  if (size() != filt.size())
    throw Exception(ErrorCodes::SIZES_OF_COLUMNS_DOESNT_MATCH, "Size of filter ({}) doesn't match size of column ({})", filt.size(),
                    size());
  ...
  for (size_t i = 0; i != num_variants; ++i) nested_filters[i].reserve_exact(variants[i]->size());
...
void ColumnVariant::filter(const Filter &filt) {
  ...
  std::vector<bool> variant_result_is_empty(num_variants, true);
  ...
  for (size_t i = 0; i != num_variants; ++i) {
    /// It make sense to call filter() on variant only if the result size is not 0.
    if (!variant_result_is_empty[i]) variants[i]->filter(nested_filters[i]);
...
ColumnPtr ColumnVariant::permute(const Permutation &perm, size_t limit) const {
  ...
  if (hasOnlyNulls()) {
    if (limit) return cloneResized(limit ? std::min(size(), limit) : size());
...
ColumnPtr ColumnVariant::index(const IColumn &indexes, size_t limit) const {
  ...
  if (hasOnlyNulls()) return cloneResized(limit == 0 ? indexes.size() : limit);
  ...
  if (auto non_empty_local_discr = getLocalDiscriminatorOfOneNoneEmptyVariantNoNulls()) {
    ...
    for (size_t i = size(); i != new_size; ++i) new_offsets_data[i] = new_offsets_data[ssize_t(i) - 1] + 1;
    return ColumnVariant::create(std::move(new_discriminators), std::move(new_offsets), std::move(new_variants),
                                 local_to_global_discriminators);
  }
...
ColumnPtr ColumnVariant::indexImpl(const PaddedPODArray<Type> &indexes, size_t limit) const {
  ...
  for (size_t i = 0; i != limit; ++i) {
    ...
    if (discr != NULL_DISCRIMINATOR) variant_indexes_data[discr]->push_back(offsets_data[indexes[i]]);
  }
...
ColumnPtr ColumnVariant::replicate(const Offsets &replicate_offsets) const {
  if (size() != replicate_offsets.size())
    throw Exception(ErrorCodes::SIZES_OF_COLUMNS_DOESNT_MATCH, "Size of offsets {} doesn't match size of column {}",
                    replicate_offsets.size(), size());

  if (empty() || replicate_offsets.back() == 0) return cloneEmpty();

  /// If we have only NULLs, just resize column to the new size.
  if (hasOnlyNulls()) return cloneResized(replicate_offsets.back());
  ...
  if (auto non_empty_local_discr = getLocalDiscriminatorOfOneNoneEmptyVariantNoNulls()) {
    ...
    if (new_size > old_size) {
      ...
      for (size_t i = old_size; i < new_size; ++i) new_offsets_data.push_back(i);
    } else {
      new_offsets_data.resize(new_size);
    }

    return ColumnVariant::create(std::move(new_local_discriminators), std::move(new_offsets), std::move(new_variants),
                                 local_to_global_discriminators);
  }
  ...
  for (size_t i = 0; i != num_variants; ++i) nested_replicated_offsets[i].reserve_exact(variants[i]->size());
  ...
  Columns new_variants;
  new_variants.reserve(num_variants);
  for (size_t i = 0; i != num_variants; ++i) new_variants.emplace_back(variants[i]->replicate(nested_replicated_offsets[i]));
...
MutableColumns ColumnVariant::scatter(size_t num_columns, const Selector &selector) const {
  ...
  if (hasOnlyNulls()) {
    ...
    for (size_t i = 0; i != num_columns; ++i) {
      ...
      result.emplace_back(
          ColumnVariant::create(std::move(scattered_local_discriminators[i]), std::move(new_variants), local_to_global_discriminators));
    }
  ...
  if (auto non_empty_local_discr = getLocalDiscriminatorOfOneNoneEmptyVariantNoNulls()) {
    ...
    for (size_t i = 0; i != num_columns; ++i) {
      ...
      result.emplace_back(ColumnVariant::create(std::move(scattered_local_discriminators[i]), std::move(scattered_nested_variants),
                                                local_to_global_discriminators));
    }
  ...
  for (size_t i = 0; i != num_variants; ++i) nested_selectors[i].reserve_exact(variants[i]->size());

  const auto &local_discriminators_data = getLocalDiscriminators();
  for (size_t i = 0; i != local_discriminators_data.size(); ++i) {
    Discriminator discr = local_discriminators_data[i];
    if (discr != NULL_DISCRIMINATOR) nested_selectors[discr].push_back(selector[i]);
  }

  auto scattered_local_discriminators = local_discriminators->scatter(num_columns, selector);
  std::vector<MutableColumns> nested_scattered_variants;
  nested_scattered_variants.reserve(num_variants);
  for (size_t i = 0; i != num_variants; ++i) nested_scattered_variants.emplace_back(variants[i]->scatter(num_columns, nested_selectors[i]));
  ...
  for (size_t i = 0; i != num_columns; ++i) {
    MutableColumns new_variants;
    new_variants.reserve(num_variants);
    for (size_t j = 0; j != num_variants; ++j) new_variants.emplace_back(std::move(nested_scattered_variants[j][i]));
    result.emplace_back(
        ColumnVariant::create(std::move(scattered_local_discriminators[i]), std::move(new_variants), local_to_global_discriminators));
  }
...
bool ColumnVariant::hasEqualValues() const {
  if (local_discriminators->empty() || hasOnlyNulls()) return true;
...
#endif
{
  ...
  if (left_discr == NULL_DISCRIMINATOR && right_discr == NULL_DISCRIMINATOR) return 0;
  if (left_discr == NULL_DISCRIMINATOR) return nan_direction_hint;
  if (right_discr == NULL_DISCRIMINATOR) return -nan_direction_hint;

  /// If rows have different discriminators, row with least discriminator is considered the least.
  if (left_discr != right_discr) return left_discr < right_discr ? -1 : 1;

  /// If rows have the same discriminators, compare actual values from corresponding variants.
  return getVariantByGlobalDiscriminator(left_discr)
      .compareAt(offsetAt(n), rhs_variant.offsetAt(m), rhs_variant.getVariantByGlobalDiscriminator(right_discr), nan_direction_hint);
}
...
struct ColumnVariant::ComparatorBase {
  ...
  ComparatorBase(const ColumnVariant &parent_, int nan_direction_hint_) : parent(parent_), nan_direction_hint(nan_direction_hint_) {}

  ALWAYS_INLINE int compare(size_t lhs, size_t rhs) const { return parent.compareAt(lhs, rhs, parent, nan_direction_hint); }
};

void ColumnVariant::getPermutation(PermutationSortDirection direction, PermutationSortStability stability, size_t limit,
...
void ColumnVariant::updatePermutation(IColumn::PermutationSortDirection direction, IColumn::PermutationSortStability stability,
                                      size_t limit, int nan_direction_hint, IColumn::Permutation &res,
                                      DB::EqualRanges &equal_ranges) const {
...
void ColumnVariant::prepareForSquashing(const Columns &source_columns, size_t factor) {
  ...
  for (const auto &source_column : source_columns) new_size += source_column->size();
...
size_t ColumnVariant::capacity() const { return local_discriminators->capacity(); }

void ColumnVariant::shrinkToFit() {
  offsets->shrinkToFit();
  local_discriminators->shrinkToFit();
  const size_t num_variants = variants.size();
  for (size_t i = 0; i < num_variants; ++i) getVariantByLocalDiscriminator(i).shrinkToFit();
}
...
void ColumnVariant::ensureOwnership() {
  ...
  const size_t num_variants = variants.size();
  for (size_t i = 0; i < num_variants; ++i) getVariantByLocalDiscriminator(i).ensureOwnership();
}

size_t ColumnVariant::byteSize() const {
  size_t res = local_discriminators->byteSize() + offsets->byteSize();
  for (const auto &variant : variants) res += variant->byteSize();
  return res;
}
...
size_t ColumnVariant::byteSizeAt(size_t n) const {
  ...
  Discriminator discr = localDiscriminatorAt(n);
  if (discr == NULL_DISCRIMINATOR) return res;
...
size_t ColumnVariant::allocatedBytes() const {
  ...
  for (const auto &variant : variants) res += variant->allocatedBytes();
  return res;
}
...
void ColumnVariant::forEachMutableSubcolumn(MutableColumnCallback callback) {
  ...
  callback(offsets);
  for (auto &variant : variants) callback(variant);
}
...
void ColumnVariant::forEachSubcolumn(ColumnCallback callback) const {
  callback(local_discriminators);
  callback(offsets);
  for (const auto &variant : variants) callback(variant);
}
...
bool ColumnVariant::structureEquals(const IColumn &rhs) const {
  ...
  if (!rhs_variant) return false;

  const size_t num_variants = variants.size();
  if (num_variants != rhs_variant->variants.size()) return false;

  for (Discriminator i = 0; i < static_cast<Discriminator>(num_variants); ++i)
    if (!variants[i]->structureEquals(rhs_variant->getVariantByGlobalDiscriminator(globalDiscriminatorByLocal(i)))) return false;
...
bool ColumnVariant::dynamicStructureEquals(const IColumn &rhs) const {
  ...
  for (Discriminator i = 0; i < static_cast<Discriminator>(num_variants); ++i)
    if (!variants[i]->dynamicStructureEquals(rhs_variant->getVariantByGlobalDiscriminator(globalDiscriminatorByLocal(i)))) return false;
...
ColumnPtr ColumnVariant::compress(bool force_compression) const {
  ...
  return ColumnCompressed::create(size(), byte_size,
                                  [my_local_discriminators_compressed = std::move(local_discriminators_compressed),
                                   my_offsets_compressed = std::move(offsets_compressed), my_compressed = std::move(compressed),
                                   my_local_to_global_discriminators = this->local_to_global_discriminators]() mutable {
                                    Columns decompressed;
                                    decompressed.reserve(my_compressed.size());
                                    for (const auto &variant : my_compressed) decompressed.push_back(variant->decompress());
                                    return ColumnVariant::create(my_local_discriminators_compressed->decompress(),
                                                                 my_offsets_compressed->decompress(), decompressed,
                                                                 my_local_to_global_discriminators);
                                  });
}
...
UInt64 ColumnVariant::getNumberOfDefaultRows() const {
  ...
  for (const auto &variant : variants) total_variant_sizes += variant->size();
...
void ColumnVariant::getIndicesOfNonDefaultRows(Offsets &indices, size_t from, size_t limit) const {
  ...
  for (size_t i = from; i < to; ++i) {
    if (!isDefaultAt(i)) indices.push_back(i);
  }
}

void ColumnVariant::finalize() {
  for (auto &variant : variants) variant->finalize();
}
...
std::optional<ColumnVariant::Discriminator> ColumnVariant::getLocalDiscriminatorOfOneNoneEmptyVariantNoNulls() const {
  for (size_t i = 0; i != variants.size(); ++i) {
    if (variants[i]->size() == local_discriminators->size()) return i;
    if (!variants[i]->empty()) return std::nullopt;
  }
...
std::optional<ColumnVariant::Discriminator> ColumnVariant::getGlobalDiscriminatorOfOneNoneEmptyVariantNoNulls() const {
  if (auto local_discr = getLocalDiscriminatorOfOneNoneEmptyVariantNoNulls()) return globalDiscriminatorByLocal(*local_discr);
...
void ColumnVariant::applyNullMap(const ColumnVector<UInt8>::Container &null_map) { applyNullMapImpl<false>(null_map); }

void ColumnVariant::applyNegatedNullMap(const ColumnVector<UInt8>::Container &null_map) { applyNullMapImpl<true>(null_map); }
...
void ColumnVariant::applyNullMapImpl(const ColumnVector<UInt8>::Container &null_map) {
  ...
  for (size_t i = 0; i != local_discriminators_data.size(); ++i) {
    ...
    if (discr != NULL_DISCRIMINATOR) {
      if (null_map[i] ^ inverted) {
        ...
        if (variant_filter.empty()) variant_filter.resize_fill(variants[discr]->size(), 1);
  ...
  for (size_t i = 0; i != variants.size(); ++i) {
    if (!variant_filters[i].empty()) variants[i] = variants[i]->filter(variant_filters[i], variant_new_sizes[i]);
  }
}

void ColumnVariant::extend(const std::vector<Discriminator> &old_to_new_global_discriminators,
                           std::vector<std::pair<MutableColumnPtr, Discriminator>> &&new_variants_and_discriminators) {
  /// Update global discriminators for current variants.
  for (Discriminator &global_discr : local_to_global_discriminators) global_discr = old_to_new_global_discriminators[global_discr];
...
bool ColumnVariant::hasDynamicStructure() const {
  for (const auto &variant : variants) {
    if (variant->hasDynamicStructure()) return true;
  }
...
void ColumnVariant::takeDynamicStructureFromSourceColumns(const Columns &source_columns, std::optional<size_t> max_dynamic_subcolumns) {
  ...
  for (size_t i = 0; i != num_variants; ++i) variants_source_columns[i].reserve(source_columns.size());

  for (const auto &source_column : source_columns) {
    const auto &source_variant = assert_cast<const ColumnVariant &>(*source_column);
    for (size_t i = 0; i != num_variants; ++i) variants_source_columns[i].push_back(source_variant.getVariantPtrByGlobalDiscriminator(i));
  }
...
void ColumnVariant::fixDynamicStructure() {
  for (auto &variant : variants) variant->fixDynamicStructure();
}
...
void ColumnVariant::validateState() const {
  ...
  if (local_discriminators_data.size() != offsets_data.size())
    throw Exception(ErrorCodes::LOGICAL_ERROR, "Size of discriminators and offsets should be equal, but {} and {} were given",
                    local_discriminators_data.size(), offsets_data.size());

  std::vector<size_t> actual_variant_sizes(variants.size());
  for (size_t i = 0; i != variants.size(); ++i) actual_variant_sizes[i] = variants[i]->size();
  ...
  for (size_t i = 0; i != local_discriminators_data.size(); ++i) {
    ...
    if (local_discr != NULL_DISCRIMINATOR) {
      ...
      if (offsets_data[i] >= actual_variant_sizes[local_discr])
        throw Exception(ErrorCodes::LOGICAL_ERROR, "Offset at position {} is {}, but variant {} ({}) has size {}", i, offsets_data[i],
                        static_cast<UInt32>(local_discr), variants[local_discr]->getName(), variants[local_discr]->size());
    }
  }

  for (size_t i = 0; i != variants.size(); ++i) {
    if (variants[i]->size() != expected_variant_sizes[i])
      throw Exception(ErrorCodes::LOGICAL_ERROR, "Variant {} ({}) has size {}, but expected {}", i, variants[i]->getName(),
                      variants[i]->size(), expected_variant_sizes[i]);
  }
}
