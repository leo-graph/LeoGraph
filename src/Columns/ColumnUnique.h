...
class ColumnUnique final : public COWHelper<IColumnUnique, ColumnUnique<ColumnType>> {
 ...
 public:
  ...
  size_t byteSizeAt(size_t n) const override { return getNestedColumn()->byteSizeAt(n); }
  void protect() override { column_holder->protect(); }
  size_t allocatedBytes() const override {
    return column_holder->allocatedBytes() + reverse_index.allocatedBytes() + (nested_null_mask ? nested_null_mask->allocatedBytes() : 0);
  }
  ...
  void forEachMutableSubcolumn(IColumn::MutableColumnCallback callback) override {
    ...
    if (is_nullable) nested_column_nullable = ColumnNullable::create(column_holder, nested_null_mask);
  }
  ...
  bool structureEquals(const IColumn &rhs) const override {
    if (auto rhs_concrete = typeid_cast<const ColumnUnique *>(&rhs)) return column_holder->structureEquals(*rhs_concrete->column_holder);
    return false;
  }
  ...
  std::optional<UInt64> getOrFindValueIndex(std::string_view value) const override {
    if (std::optional<UInt64> res = reverse_index.getIndex(value); res) return res;

    const IColumn &nested = *getNestedColumn();

    for (size_t i = 0; i < nested.size(); ++i)
      if (!nested.isNullAt(i) && nested.getDataAt(i) == value) return i;
 ...
 private:
  ...
  MutableColumnPtr uniqueInsertRangeImpl(const IColumn &src, size_t start, size_t length, size_t num_added_rows,
                                         typename ColumnVector<IndexType>::MutablePtr &&positions_column,
                                         ReverseIndex<UInt64, ColumnType> *secondary_index, size_t max_dictionary_size);
};
...
ColumnUnique<ColumnType>::ColumnUnique(const IDataType &type)
  ...
  if (column_holder->valuesHaveFixedSize()) size_of_value_if_fixed = column_holder->sizeOfValueIfFixed();
}
...
ColumnUnique<ColumnType>::ColumnUnique(MutableColumnPtr &&holder, bool is_nullable_)
  ...
  if (column_holder->size() < numSpecialValues()) throw Exception(ErrorCodes::ILLEGAL_COLUMN, "Too small holder column for ColumnUnique.");
  if (isColumnNullable(*column_holder)) throw Exception(ErrorCodes::ILLEGAL_COLUMN, "Holder column for ColumnUnique can't be nullable.");
...
void ColumnUnique<ColumnType>::updateNullMask() {
  if (is_nullable) {
    if (!nested_null_mask) throw Exception(ErrorCodes::LOGICAL_ERROR, "Null mask for ColumnUnique is was not created.");

    size_t size = getRawColumnPtr()->size();

    if (nested_null_mask->size() != size) assert_cast<ColumnUInt8 &>(*nested_null_mask).getData().resize_fill(size);
  }
}
...
const ColumnPtr &ColumnUnique<ColumnType>::getNestedColumn() const {
  if (is_nullable) return nested_column_nullable;
...
size_t ColumnUnique<ColumnType>::getNullValueIndex() const {
  if (!is_nullable) throw Exception(ErrorCodes::LOGICAL_ERROR, "ColumnUnique can't contain null values.");
...
size_t ColumnUnique<ColumnType>::uniqueInsert(const Field &x) {
  if (x.isNull()) return getNullValueIndex();
...
bool ColumnUnique<ColumnType>::tryUniqueInsert(const Field &x, size_t &index) {
  ...
  auto single_value_column = column_holder->cloneEmpty();
  if (!single_value_column->tryInsert(x)) return false;
...
size_t ColumnUnique<ColumnType>::uniqueInsertFrom(const IColumn &src, size_t n) {
  if (is_nullable && src.isNullAt(n)) return getNullValueIndex();

  if (const auto *nullable = checkAndGetColumn<ColumnNullable>(&src)) return uniqueInsertFrom(nullable->getNestedColumn(), n);
...
size_t ColumnUnique<ColumnType>::uniqueInsertData(const char *pos, size_t length) {
  if (auto index = getNestedTypeDefaultValueIndex(); getRawColumnPtr()->getDataAt(index) == std::string_view(pos, length)) return index;
...
template <typename ColumnType>
void ColumnUnique<ColumnType>::collectSerializedValueSizes(PaddedPODArray<UInt64> &sizes, const UInt8 *is_null,
                                                           const IColumn::SerializationSettings *settings) const {
  /// nullable is handled internally.
  chassert(is_null == nullptr);
  if (IColumn::empty()) return;
...
template <typename ColumnType>
std::optional<size_t> ColumnUnique<ColumnType>::getSerializedValueSize(size_t n, const IColumn::SerializationSettings *settings) const {
  if (is_nullable) {
    if (n == getNullValueIndex()) return 1;
...
template <typename ColumnType>
std::string_view ColumnUnique<ColumnType>::serializeValueIntoArena(size_t n, Arena &arena, char const *&begin,
                                                                   const IColumn::SerializationSettings *settings) const {
  if (is_nullable) {
    ...
    if (n == getNullValueIndex()) return std::string_view(pos, s);
...
char *ColumnUnique<ColumnType>::serializeValueIntoMemory(size_t n, char *memory, const IColumn::SerializationSettings *settings) const {
  if (is_nullable) {
    ...
    if (n == getNullValueIndex()) return memory;
  }
...
size_t ColumnUnique<ColumnType>::uniqueDeserializeAndInsertFromArena(ReadBuffer &in, const IColumn::SerializationSettings *settings) {
  if (is_nullable) {
    ...
    if (val) return getNullValueIndex();
  }
  ...
  for (size_t i = 0; i < data.size(); ++i) {
    if (data[i] >= max_dictionary_size) {
      throw Exception(ErrorCodes::LOGICAL_ERROR,
                      "Found index {} at position {} which is grated or equal "
                      "than dictionary size {}",
                      toString(data[i]), toString(i), toString(max_dictionary_size));
    }
  }
}
...
template <typename ColumnType>
template <typename IndexType>
MutableColumnPtr ColumnUnique<ColumnType>::uniqueInsertRangeImpl(const IColumn &src, size_t start, size_t length, size_t num_added_rows,
                                                                 typename ColumnVector<IndexType>::MutablePtr &&positions_column,
  ...
  auto update_position = [&](UInt64 &next_position) -> MutableColumnPtr {
    ...
    if (next_position > std::numeric_limits<IndexType>::max()) {
      if (sizeof(SuperiorIndexType) == sizeof(IndexType))
        throw Exception(ErrorCodes::LOGICAL_ERROR, "Can't find superior index type for type {}", demangle(typeid(IndexType).name()));

      auto expanded_column = ColumnVector<SuperiorIndexType>::create(length);
      auto &expanded_data = expanded_column->getData();
      for (size_t i = 0; i < num_added_rows; ++i) expanded_data[i] = positions[i];

      return uniqueInsertRangeImpl<SuperiorIndexType>(src, start, length, num_added_rows, std::move(expanded_column), secondary_index,
                                                      max_dictionary_size);
    }
  ...
  if (src_column == nullptr)
    throw Exception(ErrorCodes::ILLEGAL_COLUMN,
                    ...
                    "Expected {}, got {}",
                    column_holder->getName(), src.getName());

  auto column = getRawColumnPtr();

  UInt64 next_position = column->size();
  if (secondary_index) next_position += secondary_index->size();
  ...
  auto insert_key = [&](std::string_view ref, ReverseIndex<UInt64, ColumnType> &cur_index) -> MutableColumnPtr {
    ...
    if (inserted_pos == next_position) return update_position(next_position);
...
MutableColumnPtr ColumnUnique<ColumnType>::uniqueInsertRangeFrom(const IColumn &src, size_t start, size_t length) {
  ...
  if (!positions_column) positions_column = call_for_type(UInt8());
  if (!positions_column) positions_column = call_for_type(UInt16());
  if (!positions_column) positions_column = call_for_type(UInt32());
  if (!positions_column) positions_column = call_for_type(UInt64());
  if (!positions_column) throw Exception(ErrorCodes::LOGICAL_ERROR, "Can't find index type for ColumnUnique");
...
template <typename ColumnType>
IColumnUnique::IndexesWithOverflow ColumnUnique<ColumnType>::uniqueInsertRangeWithOverflow(const IColumn &src, size_t start, size_t length,
                                                                                           size_t max_dictionary_size) {
  auto overflowed_keys = column_holder->cloneEmpty();
  auto *overflowed_keys_ptr = typeid_cast<ColumnType *>(overflowed_keys.get());
  if (!overflowed_keys_ptr) throw Exception(ErrorCodes::LOGICAL_ERROR, "Invalid keys type for ColumnUnique.");
  ...
  auto call_for_type = [this, &src, start, length, overflowed_keys_ptr, max_dictionary_size](auto x) -> MutableColumnPtr {
    ...
    if (size <= std::numeric_limits<IndexType>::max()) {
      ...
      return this->uniqueInsertRangeImpl<IndexType>(src, start, length, 0, std::move(positions), &secondary_index, max_dictionary_size);
    }
