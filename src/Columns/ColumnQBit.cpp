...
ColumnQBit::ColumnQBit(MutableColumnPtr &&tuple_, size_t dimension_) : tuple(std::move(tuple_)), dimension(dimension_) {}
...
MutableColumnPtr ColumnQBit::cloneResized(size_t new_size) const { return ColumnQBit::create(tuple->cloneResized(new_size), dimension); }

size_t ColumnQBit::getBitsCount() const { return assert_cast<const ColumnTuple *>(tuple.get())->getColumns().size(); }
...
void ColumnQBit::insertFrom(const IColumn &src_, size_t n) { tuple->insertFrom(assert_cast<const ColumnQBit &>(src_).getTupleColumn(), n); }
...
void ColumnQBit::get(size_t n, Field &res) const { tuple->get(n, res); }
...
void ColumnQBit::filter(const Filter &filt) { tuple->filter(filt); }

void ColumnQBit::expand(const Filter &mask, bool inverted) { tuple->expand(mask, inverted); }
...
ColumnPtr ColumnQBit::replicate(const Offsets &offsets) const { return ColumnQBit::create(tuple->replicate(offsets), dimension); }
...
ColumnPtr ColumnQBit::compress(bool force_compression) const {
  ...
  return ColumnCompressed::create(size(), byte_size, [my_compressed = std::move(compressed), my_dimension = dimension] {
    return ColumnQBit::create(my_compressed->decompress(), my_dimension);
  });
}

void ColumnQBit::forEachMutableSubcolumn(MutableColumnCallback callback) { callback(tuple); }
...
void ColumnQBit::forEachSubcolumn(ColumnCallback callback) const { callback(tuple); }
