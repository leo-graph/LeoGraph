...
AvailableCollationLocales::AvailableCollationLocales() {
  ...
  for (int32_t i = 0; i < available_locales_count; ++i) {
    ...
    int32_t lang_length = uloc_getDisplayLanguage(locale_name.c_str(), "en", lang_buffer, MAX_LANG_LENGTH, &status);
...
AvailableCollationLocales::LocalesVector AvailableCollationLocales::getAvailableCollations() const {
  ...
  for (const auto& name_and_locale : locales_map) result.push_back(name_and_locale.second);
...
Collator::Collator(const std::string& locale_) : locale(Poco::toLower(locale_)) {
  ...
  if (U_FAILURE(status))
    throw DB::Exception(DB::ErrorCodes::UNSUPPORTED_COLLATION_LOCALE, "Failed to get base name for locale: {}. Error: {}", locale,
                        u_errorName(status));
  ...
  if (U_FAILURE(status)) {
    ucol_close(collator);
    throw DB::Exception(DB::ErrorCodes::UNSUPPORTED_COLLATION_LOCALE, "Failed to open locale: {} with error: {}", locale,
                        u_errorName(status));
  }
#else
...
const std::string& Collator::getLocale() const { return locale; }
