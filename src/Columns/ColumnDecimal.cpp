...
void ColumnDecimal<T>::getPermutation(IColumn::PermutationSortDirection direction, IColumn::PermutationSortStability stability,
  ...
  auto comparator_ascending_stable = [this](size_t lhs, size_t rhs) {
    if (unlikely(data[lhs] == data[rhs])) return lhs < rhs;
  ...
  if constexpr (is_arithmetic_v<NativeT> && !is_big_int_v<NativeT>) {
    if (!limit) {
      ...
      if (data_size >= 256 && data_size <= std::numeric_limits<UInt32>::max() && use_radix_sort) {
        ...
        for (UInt32 i = 0; i < static_cast<UInt32>(data_size); ++i) pairs[i] = {data[i].value, i};
...
void ColumnDecimal<T>::getValueNameImpl(WriteBufferFromOwnString &name_buf, size_t n, const IColumn::Options &options) const {
  if (options.notFull(name_buf)) name_buf << FieldVisitorToString()(data[n], scale);
}
...
bool ColumnDecimal<T>::tryInsert(const Field &x) {
  ...
  if (!x.tryGet<DecimalField<T>>(value)) return false;
...
void ColumnDecimal<T>::filter(const IColumn::Filter &filt) {
  ...
  while (filt_pos < filt_end) {
    if (*filt_pos) res_data[res_size++] = *data_pos;
...
ColumnPtr ColumnDecimal<T>::replicate(const IColumn::Offsets &offsets) const {
  ...
  auto res = this->create(0, scale);
  if (size == 0 || offsets.back() == 0) return res;
...
ColumnPtr ColumnDecimal<T>::compress(bool force_compression) const {
  ...
  return ColumnCompressed::create(
      data_size, compressed_size, [my_compressed = std::move(compressed), column_size = data_size, my_scale = this->scale] {
