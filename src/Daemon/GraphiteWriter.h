 ...
 private:
  ...
  void writeImpl(const T& data, time_t timestamp, const std::string& custom_root_path) {
    ...
    try {
      ...
      LOG_WARNING(&Poco::Util::Application::instance().logger(), "Fail to write to Graphite {}:{}. e.what() = {}, e.message() = {}", host,
                  port, e.what(), e.message());
    }
  }
  ...
  void out(std::ostream& os, const KeyValueVector<T>& key_val_vec, time_t timestamp, const std::string& custom_root_path) {
    for (const auto& key_val : key_val_vec) out(os, key_val, timestamp, custom_root_path);
  }
