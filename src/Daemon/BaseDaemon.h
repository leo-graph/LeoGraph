...
class BaseDaemon : public Poco::Util::ServerApplication, public Loggers {
 ...
 public:
  ...
  template <class T>
  void writeToGraphite(const std::string &key, const T &value, const std::string &config_name = DEFAULT_GRAPHITE_CONFIG_NAME,
                       time_t timestamp = 0, const std::string &custom_root_path = "") {
    auto *writer = getGraphiteWriter(config_name);
    if (writer) writer->write(key, value, timestamp, custom_root_path);
  }

  template <class T>
  void writeToGraphite(const GraphiteWriter::KeyValueVector<T> &key_vals, const std::string &config_name = DEFAULT_GRAPHITE_CONFIG_NAME,
                       time_t timestamp = 0, const std::string &custom_root_path = "") {
    auto *writer = getGraphiteWriter(config_name);
    if (writer) writer->write(key_vals, timestamp, custom_root_path);
  }

  template <class T>
  void writeToGraphite(const GraphiteWriter::KeyValueVector<T> &key_vals, const std::chrono::system_clock::time_point &current_time,
  ...
  GraphiteWriter *getGraphiteWriter(const std::string &config_name = DEFAULT_GRAPHITE_CONFIG_NAME) {
    if (graphite_writers.contains(config_name)) return graphite_writers[config_name].get();
    return nullptr;
  }
...
std::optional<std::reference_wrapper<Daemon>> BaseDaemon::tryGetInstance() {
  ...
  if (ptr) return std::optional<std::reference_wrapper<Daemon>>(*ptr);
  return {};
}
