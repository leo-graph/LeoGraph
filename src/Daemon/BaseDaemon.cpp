...
static bool getenvBool(const char *name) {
  ...
  if (env_var && 0 == strcmp(env_var, "1")) res = true;
  return res;
}
...
void BaseDaemon::terminate() {
  if (::raise(SIGTERM) != 0) throw Poco::SystemException("cannot terminate process");
}
...
std::string BaseDaemon::getDefaultCorePath() const { return "/opt/cores/"; }

std::string BaseDaemon::getDefaultConfigFileName() const { return "config.xml"; }
...
#  endif
  ...
  if (fs::is_directory(proc_path))  /// Hooray, proc exists
  {
    ...
    std::vector<int> fds;
    for (const auto &path : fs::directory_iterator(proc_path)) fds.push_back(parse<int>(path.path().filename()));
...
void BaseDaemon::initialize(Application &self) {
  ...
  if (is_daemon) {
    ...
    if (0 != chdir(path.c_str())) throw Poco::Exception("Cannot change directory to " + path);
  }

  loadConfiguration();

#if USE_JEMALLOC
  Jemalloc::setup(config().getBool(Jemalloc::config_enable_global_profiler, Jemalloc::default_enable_global_profiler),
                  ...
                  config().getBool(Jemalloc::config_collect_global_profile_samples_in_trace_log,
                                   Jemalloc::default_collect_global_profile_samples_in_trace_log),
  ...
  ConfigProcessor(config_path)
      .savePreprocessedConfig(loaded_config, ""
#if USE_SSL
                              ,
  ...
  if (auto pending_signals = config().getUInt64("pending_signals", 0); pending_signals > 0) {
    struct rlimit rlim;
    if (getrlimit(RLIMIT_SIGPENDING, &rlim)) throw Poco::Exception("Cannot getrlimit");
  ...
  if (config().has("timezone")) {
    ...
    if (0 !=
        setenv("TZ", config_timezone.data(), 1))  // NOLINT(concurrency-mt-unsafe) // ok if not called concurrently with other setenv/getenv
  ...
  if (!log_path.empty()) log_path = fs::path(log_path).replace_filename("");
  ...
  if ((!log_path.empty() && is_daemon) || config().has("logger.stderr")) {
    ...
    if (!freopen(stderr_path.c_str(), "a+", stderr)) throw Poco::OpenFileException("Cannot attach stderr to " + stderr_path);
  ...
  if ((!log_path.empty() && is_daemon) || config().has("logger.stdout")) {
    ...
    if (!freopen(stdout_path.c_str(), "a+", stdout)) throw Poco::OpenFileException("Cannot attach stdout to " + stdout_path);
  }

  /// Change path for logging.
  if (!log_path.empty()) {
    std::string path = createDirectory(log_path);
    if (is_daemon && chdir(path.c_str()) != 0) throw Poco::Exception("Cannot change directory to " + path);
  } else {
    if (is_daemon && chdir("/tmp") != 0) throw Poco::Exception("Cannot change directory to /tmp");
  }
  ...
  if (should_setup_watchdog) setupWatchdog();

  /// Create pid file.
  if (config().has("pid")) pid_file.emplace(config().getString("pid"), StatusFile::write_pid);
  ...
  if (is_daemon) {
    ...
    if (core_path.empty()) core_path = getDefaultCorePath();
    ...
    if (0 != chdir(core_path.c_str())) throw Poco::Exception("Cannot change directory to " + core_path);
  }
...
void BaseDaemon::initializeTerminationAndSignalProcessing() {
  ...
  if (config().getBool("send_crash_reports.enabled", false) && config().getBool("send_crash_reports.send_logical_errors", false) &&
    ...
    Exception::callback = [](std::string_view format_string, int code, bool remote, const Exception::Trace &trace) {
      if (!remote && code == ErrorCodes::LOGICAL_ERROR) {
        ...
        for (size_t i = 0; i < trace.size(); ++i) frame_pointers[i] = trace[i];
  ...
  if (!executable_path.empty()) stored_binary_hash = Elf(executable_path).getStoredBinaryHash();
#endif
}

void BaseDaemon::logRevision() const {
  logger().information("Starting " + std::string{VERSION_FULL} + " (revision: " + std::to_string(ClickHouseRevision::getVersionRevision()) +
                       ", git hash: " + std::string(GIT_HASH) + ", build id: " + (build_id.empty() ? "<unknown>" : build_id) + ")" +
...
void BaseDaemon::defineOptions(Poco::Util::OptionSet &new_options) {
  ...
  new_options.addOption(
      Poco::Util::Option("log-file", "L", "use given log file").required(false).repeatable(false).argument("<file>").binding("logger.log"));
...
void BaseDaemon::handleSignal(int signal_id) {
  if (!(signal_id == SIGINT || signal_id == SIGQUIT || signal_id == SIGTERM))
    throw Exception::createDeprecated(std::string("Unsupported signal: ") + strsignal(signal_id),
                                      0);  // NOLINT(concurrency-mt-unsafe) // it is not thread-safe but ok in this context
  ...
  is_cancelled = true;
  LOG_INFO(&logger(), "Received termination signal ({})",
           strsignal(signal_id));  // NOLINT(concurrency-mt-unsafe) // it is not thread-safe but ok in this context
...
void BaseDaemon::setupWatchdog() {
  ...
  if (argv0) original_process_name = argv0;
  ...
  while (true) {
    ...
    if (async_channel) async_channel->close();
    pid = fork();
    ...
    if (async_channel) async_channel->open();

    if (-1 == pid) throw ErrnoException(ErrorCodes::SYSTEM_ERROR, "Cannot fork");

    if (0 == pid) {
      updateCurrentThreadIdAfterFork();
      logger().information("Forked a child process to watch");
#if defined(OS_LINUX)
      if (0 != prctl(PR_SET_PDEATHSIG, SIGKILL)) logger().warning("Cannot do prctl to ask termination with parent.");
#endif
    ...
    if (WIFSIGNALED(status)) {
      ...
      if (sig == SIGKILL) {
        logger().fatal(
            fmt::format("Child process was terminated by signal {} (KILL)."
        ...
        if (sig == SIGINT || sig == SIGTERM || sig == SIGQUIT) _exit(exit_code);
      }
    } else {
    ...
    if (restart) {
      ...
      if (argv0) memcpy(argv0, original_process_name.c_str(), original_process_name.size());
    } else
      _exit(exit_code);
  }
}

String BaseDaemon::getStoredBinaryHash() const { return stored_binary_hash; }

#if defined(OS_LINUX)
void systemdNotify(const std::string_view &command) {
  const char *path = getenv("NOTIFY_SOCKET");  // NOLINT(concurrency-mt-unsafe)

  if (path == nullptr) return;  /// not using systemd

  int s = socket(AF_UNIX, SOCK_DGRAM | SOCK_CLOEXEC, 0);

  if (s == -1) throw ErrnoException(ErrorCodes::SYSTEM_ERROR, "Can't create UNIX socket for systemd notify");
  ...
  while (sent_bytes_total < command.size()) {
    auto sent_bytes =
        sendto(s, command.data() + sent_bytes_total, command.size() - sent_bytes_total, 0, sock_addr, static_cast<socklen_t>(addrlen));
    if (sent_bytes == -1) {
