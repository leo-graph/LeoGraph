...
ASTIdentifier::ASTIdentifier(std::vector<String> &&name_parts_, bool special, ASTs &&name_params)
  ...
  if (!name_params.empty()) {
    ...
    for (const auto &part [[maybe_unused]] : name_parts) {
      if (part.empty()) ++params;
    }
    assert(params == name_params.size());
    children = std::move(name_params);
  } else {
    for (const auto &part [[maybe_unused]] : name_parts) assert(!part.empty());

    if (!special && name_parts.size() >= 2) semantic->table = name_parts.end()[-2];

    resetFullName();
  }
}

bool ASTIdentifier::isParam() const { return !children.empty(); }
...
bool ASTIdentifier::supposedToBeCompound() const { return semantic->legacy_compound; }
...
void ASTIdentifier::formatImplWithoutAlias(WriteBuffer &ostr, const FormatSettings &settings, FormatState &state,
...
void ASTIdentifier::appendColumnNameImpl(WriteBuffer &ostr) const { writeString(name(), ostr); }
...
void ASTIdentifier::resetFullName() {
  ...
  for (size_t i = 1; i < name_parts.size(); ++i) full_name += '.' + name_parts[i];
}
...
ASTTableIdentifier::ASTTableIdentifier(const StorageID &table_id, ASTs &&name_params)
    : ASTIdentifier(table_id.database_name.empty() ? std::vector<String>{table_id.table_name}
...
ASTPtr ASTTableIdentifier::getTable() const {
  if (name_parts.size() == 2) {
    if (!name_parts[1].empty()) return make_intrusive<ASTIdentifier>(name_parts[1]);

    if (name_parts[0].empty()) return make_intrusive<ASTIdentifier>("", children[1]->clone());
    return make_intrusive<ASTIdentifier>("", children[0]->clone());
  }
  if (name_parts.size() == 1) {
    if (name_parts[0].empty()) return make_intrusive<ASTIdentifier>("", children[0]->clone());
...
String getIdentifierName(const IAST *ast) {
  String res;
  if (tryGetIdentifierNameInto(ast, res)) return res;
  if (ast) throw Exception(ErrorCodes::UNEXPECTED_AST_STRUCTURE, "{} is not an identifier", ast->formatForErrorMessage());
...
void setIdentifierSpecial(ASTPtr &ast) {
  if (ast)
    if (auto *id = ast->as<ASTIdentifier>()) id->semantic->special = true;
}
