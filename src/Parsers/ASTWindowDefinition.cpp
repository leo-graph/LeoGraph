...
String ASTWindowDefinition::getID(char) const { return "WindowDefinition"; }

void ASTWindowDefinition::formatImpl(WriteBuffer& ostr, const FormatSettings& settings, FormatState& state,
                                     FormatStateStacked format_frame) const {
  ...
  if (!frame_is_default) {
    ...
    if (frame_begin_type == WindowFrame::BoundaryType::Current) {
      ...
      ostr << " " << (!frame_begin_preceding ? "FOLLOWING" : "PRECEDING");
    }
    ostr << " AND ";
    if (frame_end_type == WindowFrame::BoundaryType::Current) {
      ...
      ostr << " " << (!frame_end_preceding ? "FOLLOWING" : "PRECEDING");
    }
  }
}
...
String ASTWindowListElement::getID(char) const { return "WindowListElement"; }

void ASTWindowListElement::formatImpl(WriteBuffer& ostr, const FormatSettings& settings, FormatState& state,
