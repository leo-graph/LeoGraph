...
struct HighlightedRange {
  ...
  auto operator<=>(const HighlightedRange &other) const { return begin <=> other.begin; }
};
...
class IParser {
 public:
...
#endif
      if (depth % check_frequency == 0) checkStackSize();
    }

    ALWAYS_INLINE void decreaseDepth() {
      if (unlikely(depth == 0)) throw Exception(ErrorCodes::LOGICAL_ERROR, "Logical error in parser: incorrect calculation of parse depth");
      --depth;
    }
   ...
   */
  virtual Highlight highlight() const { return Highlight::none; }
