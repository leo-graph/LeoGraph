...
static inline bool parsePartitionExpression(IParser::Pos& pos, std::string& type, ASTPtr& node, Expected& expected,
                                            bool subpartition = false) {
  ASTPtr expression;
  ...
  if (!subpartition && ParserKeyword(Keyword::LIST).ignore(pos, expected)) {
    ...
    if (ParserKeyword(Keyword::LINEAR).ignore(pos, expected)) type = "linear_";
    ...
    if (ParserKeyword(Keyword::KEY).ignore(pos, expected)) {
      ...
      if (ParserKeyword(Keyword::ALGORITHM).ignore(pos, expected)) {
        ...
        if (!p_literal.parse(pos, algorithm, expected) || !algorithm->as<ASTLiteral>()) return false;

        UInt64 algorithm_type = algorithm->as<ASTLiteral>()->value.safeGet<UInt64>();

        if (algorithm_type != 1 && algorithm_type != 2) return false;
...
bool ParserDeclarePartitionOptions::parseImpl(Pos& pos, ASTPtr& node, Expected& expected) {
  ...
  if (!ParserKeyword(Keyword::PARTITION_BY).ignore(pos, expected)) return false;

  if (!parsePartitionExpression(pos, partition_type, partition_expression, expected)) return false;

  if (ParserKeyword(Keyword::PARTITIONS).ignore(pos, expected)) {
    ParserLiteral p_literal;
    if (!p_literal.parse(pos, partition_numbers, expected)) return false;
  }

  if (ParserKeyword(Keyword::SUBPARTITION_BY).ignore(pos, expected)) {
    if (!parsePartitionExpression(pos, subpartition_type, subpartition_expression, expected, true)) return false;

    if (ParserKeyword(Keyword::SUBPARTITIONS).ignore(pos, expected)) {
      ParserLiteral p_literal;
      if (!p_literal.parse(pos, subpartition_numbers, expected)) return false;
    }
  }
