...
bool ParserDeclareReference::parseImpl(IParser::Pos& pos, ASTPtr& node, Expected& expected) {
  ...
  if (!ParserKeyword(Keyword::REFERENCES).ignore(pos, expected)) return false;

  if (!p_identifier.parse(pos, table_name, expected)) return false;
  ...
  while (true) {
    if (ParserKeyword(Keyword::ON_DELETE).ignore(pos, expected)) {
      if (!parseReferenceOption(pos, delete_option, expected)) return false;
    } else if (ParserKeyword(Keyword::ON_UPDATE).ignore(pos, expected)) {
      if (!parseReferenceOption(pos, update_option, expected)) return false;
    } else
      break;
  }
