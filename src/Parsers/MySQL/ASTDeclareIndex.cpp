...
struct ParserIndexColumn : public IParserBase {
 protected:
  ...
  bool parseImpl(Pos& pos, ASTPtr& node, Expected& expected) override {
    ...
    if (!p_expression.parse(pos, node, expected)) return false;
...
static inline bool parseDeclareOrdinaryIndex(IParser::Pos& pos, String& index_name, String& index_type, Expected& expected) {
  ...
  if (ParserKeyword(Keyword::SPATIAL).ignore(pos, expected)) {
    if (!k_key.ignore(pos, expected)) k_index.ignore(pos, expected);

    index_type = "SPATIAL";
    if (p_identifier.parse(pos, temp_node, expected)) index_name = temp_node->as<ASTIdentifier>()->name();
    ...
    if (!k_key.ignore(pos, expected)) {
      if (!k_index.ignore(pos, expected)) return false;
    }
    ...
    if (ParserKeyword(Keyword::USING).ignore(pos, expected)) {
      if (!p_identifier.parse(pos, temp_node, expected)) return false;
...
static inline bool parseDeclareConstraintIndex(IParser::Pos& pos, String& index_name, String& index_type, Expected& expected) {
  ...
  if (ParserKeyword(Keyword::CONSTRAINT).ignore(pos, expected)) {
    if (!ParserKeyword(Keyword::PRIMARY).checkWithoutMoving(pos, expected) &&
        !ParserKeyword(Keyword::UNIQUE).checkWithoutMoving(pos, expected) &&
  ...
  if (ParserKeyword(Keyword::UNIQUE).ignore(pos, expected)) {
    if (!ParserKeyword(Keyword::KEY).ignore(pos, expected)) ParserKeyword(Keyword::INDEX).ignore(pos, expected);

    if (p_identifier.parse(pos, temp_node, expected)) index_name = temp_node->as<ASTIdentifier>()->name();  /// reset index_name
...
bool ParserDeclareIndex::parseImpl(IParser::Pos& pos, ASTPtr& node, Expected& expected) {
  ...
  if (!parseDeclareOrdinaryIndex(pos, index_name, index_type, expected)) {
    if (!parseDeclareConstraintIndex(pos, index_name, index_type, expected)) return false;
  }

  ParserToken s_opening_round(TokenType::OpeningRoundBracket);
  ParserToken s_closing_round(TokenType::ClosingRoundBracket);

  if (!s_opening_round.ignore(pos, expected)) return false;

  ParserList p_index_columns(std::make_unique<ParserIndexColumn>(), std::make_unique<ParserToken>(TokenType::Comma));

  if (!p_index_columns.parse(pos, index_columns, expected)) return false;

  if (!s_closing_round.ignore(pos, expected)) return false;

  if (index_type != "FOREIGN")
    p_index_options.parse(pos, index_options, expected);
  else {
    if (!ParserDeclareReference().parse(pos, declare_reference, expected)) return false;
  }
  ...
  if (declare_index->index_columns) declare_index->children.emplace_back(declare_index->index_columns);

  if (declare_index->index_options) declare_index->children.emplace_back(declare_index->index_options);

  if (declare_index->reference_definition) declare_index->children.emplace_back(declare_index->reference_definition);
