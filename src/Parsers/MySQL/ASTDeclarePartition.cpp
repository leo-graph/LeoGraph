...
bool ParserDeclarePartition::parseImpl(IParser::Pos& pos, ASTPtr& node, Expected& expected) {
  if (!ParserKeyword{Keyword::PARTITION}.ignore(pos, expected)) return false;
  ...
  if (!p_identifier.parse(pos, partition_name, expected)) return false;

  ParserKeyword p_values(Keyword::VALUES);
  if (p_values.ignore(pos, expected)) {
    if (ParserKeyword{Keyword::IN}.ignore(pos, expected)) {
      if (!p_expression.parse(pos, in_expression, expected)) return false;
    } else if (ParserKeyword{Keyword::LESS_THAN}.ignore(pos, expected)) {
      if (!p_expression.parse(pos, less_than, expected)) return false;
    }
  }
  ...
  if (partition_declare->less_than) partition_declare->children.emplace_back(partition_declare->less_than);

  if (partition_declare->in_expression) partition_declare->children.emplace_back(partition_declare->in_expression);

  if (partition_declare->subpartitions) partition_declare->children.emplace_back(partition_declare->subpartitions);
