...
ASTPtr ASTAlterCommand::clone() const {
  ...
  if (index_decl) res->set(res->index_decl, index_decl->clone());

  if (default_expression) res->set(res->default_expression, default_expression->clone());

  if (additional_columns) res->set(res->additional_columns, additional_columns->clone());

  if (order_by_columns) res->set(res->order_by_columns, order_by_columns->clone());

  if (properties) res->set(res->properties, properties->clone());
...
static inline bool parseAddCommand(IParser::Pos& pos, ASTPtr& node, Expected& expected) {
  ...
  if (index_p.parse(pos, declare_index, expected)) {
    ...
    if (ParserToken(TokenType::OpeningRoundBracket).ignore(pos, expected)) {
      ...
      if (!columns_p.parse(pos, additional_columns, expected)) return false;

      if (!ParserToken(TokenType::ClosingRoundBracket).ignore(pos, expected)) return false;
    } else {
      ASTPtr declare_column;
      if (!column_p.parse(pos, declare_column, expected)) return false;
      ...
      if (ParserKeyword(Keyword::FIRST).ignore(pos, expected))
        ...
        if (!identifier_p.parse(pos, after_column, expected)) return false;
...
static inline bool parseDropCommand(IParser::Pos& pos, ASTPtr& node, Expected& expected) {
  ...
  if (ParserKeyword(Keyword::PRIMARY_KEY).ignore(pos, expected)) {
    ...
    if (!identifier_p.parse(pos, name, expected)) return false;
...
static inline bool parseAlterCommand(IParser::Pos& pos, ASTPtr& node, Expected& expected) {
  ...
  if (ParserKeyword(Keyword::INDEX).ignore(pos, expected)) {
    ...
    if (!alter_command->index_visible && !ParserKeyword(Keyword::INVISIBLE).ignore(pos, expected)) return false;
    ...
    if (!ParserKeyword(Keyword::ENFORCED).ignore(pos, expected)) return false;
    ...
    if (ParserKeyword(Keyword::DROP_DEFAULT).ignore(pos, expected))
      ...
      if (!expression_p.parse(pos, default_expression, expected)) return false;
...
static inline bool parseRenameCommand(IParser::Pos& pos, ASTPtr& node, Expected& expected) {
  ...
  if (ParserKeyword(Keyword::COLUMN).ignore(pos, expected)) {
    if (!identifier_p.parse(pos, old_name, expected)) return false;

    if (!ParserKeyword(Keyword::TO).ignore(pos, expected)) return false;

    if (!identifier_p.parse(pos, new_name, expected)) return false;
    ...
    if (!ParserCompoundIdentifier(true).parse(pos, new_name, expected)) return false;
...
static inline bool parseOtherCommand(IParser::Pos& pos, ASTPtr& node, Expected& expected) {
  ...
  if (ParserKeyword(Keyword::ORDER_BY).ignore(pos, expected)) {
    ...
    ParserDeclareOption options_p{{OptionDescribe("FORCE", "force", std::make_shared<ParserAlwaysTrue>()),
    ...
    if (!options_p.parse(pos, properties_options, expected) && !table_options_p.parse(pos, properties_options, expected)) return false;
...
static inline bool parseModifyCommand(IParser::Pos& pos, ASTPtr& node, Expected& expected, bool exists_old_column_name = false) {
  ...
  ParserKeyword(Keyword::COLUMN).ignore(pos, expected);
  if (exists_old_column_name && !ParserIdentifier().parse(pos, old_column_name, expected)) return false;

  ASTPtr additional_column;
  if (!ParserDeclareColumn().parse(pos, additional_column, expected)) return false;
  ...
  if (exists_old_column_name) alter_command->old_name = getIdentifierName(old_column_name);
...
bool ParserAlterCommand::parseImpl(IParser::Pos& pos, ASTPtr& node, Expected& expected) {
  ...
  if (k_add.ignore(pos, expected)) return parseAddCommand(pos, node, expected);
  if (k_drop.ignore(pos, expected)) return parseDropCommand(pos, node, expected);
  if (k_alter.ignore(pos, expected)) return parseAlterCommand(pos, node, expected);
  if (k_rename.ignore(pos, expected)) return parseRenameCommand(pos, node, expected);
  if (k_modify.ignore(pos, expected)) return parseModifyCommand(pos, node, expected);
  if (k_change.ignore(pos, expected)) return parseModifyCommand(pos, node, expected, true);
