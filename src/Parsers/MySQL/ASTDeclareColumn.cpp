...
bool ParserDeclareColumn::parseImpl(Pos& pos, ASTPtr& node, Expected& expected) {
  ...
  if (!p_identifier.parse(pos, column_name, expected)) return false;

  if (!ParserDataType().parse(pos, column_data_type, expected)) return false;
  ...
  if (declare_column->data_type) declare_column->children.emplace_back(declare_column->data_type);

  if (declare_column->column_options) declare_column->children.emplace_back(declare_column->column_options);
