...
TEST(ParserPartition, AllPatitionOptions) {
  String input =
      "PARTITION partition_name ENGINE = engine_name COMMENT 'partition comment'"
...
TEST(ParserPartition, PatitionOptionsWithSubpartitions) {
  ...
  String partition_02 =
      "PARTITION partition_02 VALUES IN (NULL, 1991, MAXVALUE) STORAGE engine = engine_name (SUBPARTITION s_p01, SUBPARTITION s_p02)";
  ASTPtr ast_partition_02 = parseQuery(p_partition, partition_02.data(), partition_02.data() + partition_02.size(), "", 0, 0, 0);
