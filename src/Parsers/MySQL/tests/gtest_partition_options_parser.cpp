...
TEST(ParserPartitionOptions, HashPatitionOptions) {
  ...
  ASTPtr ast_02 = parseQuery(p_partition_options, linear_hash_partition.data(), linear_hash_partition.data() + linear_hash_partition.size(),
                             "", 0, 0, 0);
...
TEST(ParserPartitionOptions, KeyPatitionOptions) {
  ...
  ASTPtr ast_02 =
      parseQuery(p_partition_options, linear_key_partition.data(), linear_key_partition.data() + linear_key_partition.size(), "", 0, 0, 0);
  ...
  ASTPtr ast_03 = parseQuery(p_partition_options, key_partition_with_algorithm.data(),
                             key_partition_with_algorithm.data() + key_partition_with_algorithm.size(), "", 0, 0, 0);
...
TEST(ParserPartitionOptions, RangePatitionOptions) {
  ...
  ASTPtr ast_02 = parseQuery(p_partition_options, range_columns_partition.data(),
                             range_columns_partition.data() + range_columns_partition.size(), "", 0, 0, 0);
...
TEST(ParserPartitionOptions, PatitionWithSubpartitionOptions) {
  ...
  ParserDeclarePartitionOptions p_partition_options;
  ASTPtr ast = parseQuery(p_partition_options, partition_with_subpartition.data(),
                          partition_with_subpartition.data() + partition_with_subpartition.size(), "", 0, 0, 0);
...
TEST(ParserPartitionOptions, PatitionOptionsWithDeclarePartition) {
  String partition_options_with_declare =
      "PARTITION BY KEY(col_01) PARTITIONS 3 SUBPARTITION BY HASH(col_02) SUBPARTITIONS 4 (PARTITION partition_name)";

  ParserDeclarePartitionOptions p_partition_options;
  ASTPtr ast = parseQuery(p_partition_options, partition_options_with_declare.data(),
...
TEST(ParserPartitionOptions, PatitionOptionsWithDeclarePartitions) {
  String partition_options_with_declare =
      "PARTITION BY KEY(col_01) PARTITIONS 3 SUBPARTITION BY HASH(col_02) SUBPARTITIONS 4 (PARTITION partition_01, PARTITION partition_02)";
