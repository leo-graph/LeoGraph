...
bool ParserCreateQuery::parseImpl(Pos& pos, ASTPtr& node, Expected& expected) {
  ...
  if (!ParserKeyword(Keyword::CREATE).ignore(pos, expected)) return false;

  if (ParserKeyword(Keyword::TEMPORARY).ignore(pos, expected)) is_temporary = true;

  if (!ParserKeyword(Keyword::TABLE).ignore(pos, expected)) return false;

  if (ParserKeyword(Keyword::IF_NOT_EXISTS).ignore(pos, expected)) if_not_exists = true;

  if (!ParserCompoundIdentifier(true).parse(pos, table, expected)) return false;

  if (ParserKeyword(Keyword::LIKE).ignore(pos, expected)) {
    if (!ParserCompoundIdentifier(true).parse(pos, like_table, expected)) return false;
    ...
    if (ParserKeyword(Keyword::LIKE).ignore(pos, expected)) {
      ...
      if (!ParserCreateDefines().parse(pos, columns_list, expected)) return false;
  ...
  if (create_query->like_table) create_query->children.emplace_back(create_query->like_table);

  if (create_query->columns_list) create_query->children.emplace_back(create_query->columns_list);

  if (create_query->table_options) create_query->children.emplace_back(create_query->table_options);

  if (create_query->partition_options) create_query->children.emplace_back(create_query->partition_options);
