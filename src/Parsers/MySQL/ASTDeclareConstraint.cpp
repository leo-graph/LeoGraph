...
bool ParserDeclareConstraint::parseImpl(IParser::Pos& pos, ASTPtr& node, Expected& expected) {
  ...
  if (ParserKeyword(Keyword::CONSTRAINT).ignore(pos, expected)) {
    if (!ParserKeyword(Keyword::CHECK).checkWithoutMoving(pos, expected)) ParserIdentifier().parse(pos, constraint_symbol, expected);
  }

  if (!ParserKeyword(Keyword::CHECK).ignore(pos, expected)) return false;

  if (!p_expression.parse(pos, index_check_expression, expected)) return false;
  ...
  if (constraint_symbol) declare_constraint->constraint_name = constraint_symbol->as<ASTIdentifier>()->name();
