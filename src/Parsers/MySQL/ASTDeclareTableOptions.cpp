...
struct ParserBoolOption : public IParserBase {
 protected:
  ...
  bool parseImpl(Pos& pos, ASTPtr& node, Expected& expected) override {
    ...
    if (!p_literal.parse(pos, node, expected) || !node->as<ASTLiteral>()) return false;
...
struct ParserTablespaceName : public IParserBase {
 protected:
  ...
  bool parseImpl(Pos& pos, ASTPtr& node, Expected& expected) override {
    ...
    if (!p_identifier.parse(pos, node, expected)) return false;

    if (ParserKeyword(Keyword::STORAGE).ignore(pos, expected)) {
      if (!ParserKeyword(Keyword::DISK).ignore(pos, expected)) {
        if (!ParserKeyword(Keyword::MEMORY).ignore(pos, expected)) return false;
      }
    }
