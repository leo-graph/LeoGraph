 ...
 *  the next identifier part after it will include special delimiter and be back quoted always: json.a.b.:UInt32 -> ['json', 'a', 'b',
 * ':`UInt32`']. It's needed to distinguish identifiers json.a.b.:UInt32 and json.a.b.`:UInt32`. There is also a special syntax sugar for
 * reading JSON subcolumns of type Array(JSON): json.a.b[][].c -> json.a.b.:Array(Array(JSON)).c
 */
class ParserCompoundIdentifier : public IParserBase {
 public:
  explicit ParserCompoundIdentifier(bool table_name_with_optional_uuid_ = false, bool allow_query_parameter_ = false,
                                    Highlight highlight_type_ = Highlight::identifier)
...
class ParserColumnsTransformers : public IParserBase {
 public:
  ...
  explicit ParserColumnsTransformers(ColumnTransformers allowed_transformers_ = AllTransformers, bool is_strict_ = false)
      : allowed_transformers(allowed_transformers_), is_strict(is_strict_) {}
 ...
 * But parsing the whole array/tuple as a whole constant seriously speeds up the analysis of expressions in the case of very large
 * collection. We try to parse the array or tuple as a collection of literals first (fast path), and if it did not work out (when the
 * collection consists of complex expressions) - parse as an application of [] operator or 'tuple' function (slow path).
 */
