...
bool ParserUnionQueryElement::parseImpl(Pos& pos, ASTPtr& node, Expected& expected) {
  if (ParserSubquery().parse(pos, node, expected)) {
    if (const auto* ast_subquery = node->as<ASTSubquery>()) node = ast_subquery->children.at(0);
    ...
    if (const auto* union_query = node->as<ASTSelectWithUnionQuery>())
      if (union_query->list_of_selects->children.size() == 1) node = union_query->list_of_selects->children[0];

    return true;
  }

  if (ParserSelectQuery().parse(pos, node, expected)) return true;
