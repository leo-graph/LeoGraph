...
bool ParserShowTablesQuery::parseImpl(Pos &pos, ASTPtr &node, Expected &expected) {
  ...
  if (!s_show.ignore(pos, expected)) return false;
  ...
  if (s_databases.ignore(pos, expected)) {
    ...
    if (s_not.ignore(pos, expected)) query->not_like = true;
    ...
    if (s_limit.ignore(pos, expected)) {
      if (!exp_elem.parse(pos, query->limit_length, expected)) return false;
    }
    ...
    String cluster_str;
    if (!parseIdentifierOrStringLiteral(pos, expected, cluster_str)) return false;
    ...
    if (changed) {
      query->changed = true;
      if (!s_settings.ignore(pos, expected)) return false;
    }
    ...
    if (s_temporary.ignore(pos, expected)) query->temporary = true;
    ...
    if (bool insensitive = s_ilike.ignore(pos, expected); insensitive || s_like.ignore(pos, expected)) {
      ...
      if (!exp_elem.parse(pos, query->where_expression, expected)) return false;
