...
std::vector<ViewTarget::Kind> ASTViewTargets::getKinds() const {
  ...
  for (const auto &target : targets) kinds.push_back(target.kind);
  return kinds;
}
...
void ASTViewTargets::setTableID(ViewTarget::Kind kind, const StorageID &table_id_) {
  ...
  if (table_id_) targets.emplace_back(kind).table_id = table_id_;
}

StorageID ASTViewTargets::getTableID(ViewTarget::Kind kind) const {
  if (const auto *target = tryGetTarget(kind)) return target->table_id;
  return StorageID::createEmpty();
}

bool ASTViewTargets::hasTableID(ViewTarget::Kind kind) const {
  if (const auto *target = tryGetTarget(kind)) return !target->table_id.empty();
  return false;
}
...
void ASTViewTargets::setCurrentDatabase(const String &current_database) {
  for (auto &target : targets) {
    ...
    if (!table_id.table_name.empty() && table_id.database_name.empty()) table_id.database_name = current_database;
  }
}
...
void ASTViewTargets::setInnerUUID(ViewTarget::Kind kind, const UUID &inner_uuid_) {
  ...
  if (inner_uuid_ != UUIDHelpers::Nil) targets.emplace_back(kind).inner_uuid = inner_uuid_;
}

UUID ASTViewTargets::getInnerUUID(ViewTarget::Kind kind) const {
  if (const auto *target = tryGetTarget(kind)) return target->inner_uuid;
  return UUIDHelpers::Nil;
}

bool ASTViewTargets::hasInnerUUID(ViewTarget::Kind kind) const { return getInnerUUID(kind) != UUIDHelpers::Nil; }

void ASTViewTargets::resetInnerUUIDs() {
  for (auto &target : targets) target.inner_uuid = UUIDHelpers::Nil;
}

bool ASTViewTargets::hasInnerUUIDs() const {
  for (const auto &target : targets) {
    if (target.inner_uuid != UUIDHelpers::Nil) return true;
  }
  return false;
}
...
void ASTViewTargets::setInnerEngine(ViewTarget::Kind kind, ASTPtr storage_def) {
  ...
  for (auto &target : targets) {
    if (target.kind == kind) {
      if (target.inner_engine == new_inner_engine) return;
      if (new_inner_engine) children.push_back(new_inner_engine);
      if (target.inner_engine) std::erase(children, target.inner_engine);
...
ASTStorage *ASTViewTargets::getInnerEngine(ViewTarget::Kind kind) const {
  if (const auto *target = tryGetTarget(kind); target && target->inner_engine) return target->inner_engine->as<ASTStorage>();
  return nullptr;
}
...
std::vector<ASTStorage *> ASTViewTargets::getInnerEngines() const {
  ...
  for (const auto &target : targets) {
    if (target.inner_engine) res.push_back(target.inner_engine->as<ASTStorage>());
  }
  return res;
}
...
void ASTViewTargets::formatImpl(WriteBuffer &ostr, const FormatSettings &s, FormatState &state, FormatStateStacked frame) const {
  for (const auto &target : targets) formatTarget(target, ostr, s, state, frame);
}

void ASTViewTargets::formatTarget(ViewTarget::Kind kind, WriteBuffer &ostr, const FormatSettings &s, FormatState &state,
                                  FormatStateStacked frame) const {
  for (const auto &target : targets) {
    if (target.kind == kind) formatTarget(target, ostr, s, state, frame);
  }
}

void ASTViewTargets::formatTarget(const ViewTarget &target, WriteBuffer &ostr, const FormatSettings &s, FormatState &state,
                                  FormatStateStacked frame) {
  if (target.table_id) {
    auto keyword = getKeywordForTableID(target.kind);
    if (!keyword) throw Exception(ErrorCodes::LOGICAL_ERROR, "No keyword for table name of kind {}", toString(target.kind));
  ...
  if (target.inner_uuid != UUIDHelpers::Nil) {
    ...
    if (!keyword) throw Exception(ErrorCodes::LOGICAL_ERROR, "No prefix keyword for inner UUID of kind {}", toString(target.kind));
    ostr << " " << toStringView(*keyword) << " " << quoteString(toString(target.inner_uuid));
  }

  if (target.inner_engine) {
    auto keyword = getKeywordForInnerStorage(target.kind);
    if (!keyword) throw Exception(ErrorCodes::LOGICAL_ERROR, "No prefix keyword for table engine of kind {}", toString(target.kind));
    ostr << " " << toStringView(*keyword);
...
std::optional<Keyword> ASTViewTargets::getKeywordForTableID(ViewTarget::Kind kind) {
  switch (kind) {
    case ViewTarget::To:
      return Keyword::TO;  /// TO mydb.mydata
    case ViewTarget::Inner:
      return std::nullopt;
    case ViewTarget::Data:
...
std::optional<Keyword> ASTViewTargets::getKeywordForInnerStorage(ViewTarget::Kind kind) {
  switch (kind) {
    case ViewTarget::To:
      return std::nullopt;  /// ENGINE = MergeTree()
    case ViewTarget::Inner:
      return Keyword::INNER;  /// INNER ENGINE = MergeTree()
    case ViewTarget::Data:
...
std::optional<Keyword> ASTViewTargets::getKeywordForInnerUUID(ViewTarget::Kind kind) {
  switch (kind) {
    case ViewTarget::To:
      return Keyword::TO_INNER_UUID;  /// TO INNER UUID 'XXX'
    case ViewTarget::Inner:
      return std::nullopt;
    case ViewTarget::Data:
