...
void ASTFunction::appendColumnNameImpl(WriteBuffer &ostr) const {
  ...
  if (parameters) {
    ...
    for (auto it = parameters->children.begin(); it != parameters->children.end(); ++it) {
      if (it != parameters->children.begin()) writeCString(", ", ostr);
  ...
  if (arguments) {
    for (auto it = arguments->children.begin(); it != arguments->children.end(); ++it) {
      if (it != arguments->children.begin()) writeCString(", ", ostr);
...
void ASTFunction::finishFormatWithWindow(WriteBuffer &ostr, const FormatSettings &settings, FormatState &state,
...
String ASTFunction::getID(char delim) const { return "Function" + (delim + name); }
...
void ASTFunction::updateTreeHashImpl(SipHash &hash_state, bool ignore_aliases) const {
  ...
  if (isWindowFunction()) {
    ...
    if (window_definition) window_definition->updateTreeHashImpl(hash_state, ignore_aliases);
  }
}
...
ASTPtr ASTFunction::toLiteral() const {
  ...
  if (name == "array") return createLiteral<Array>(arguments->children);

  if (name == "tuple") return createLiteral<Tuple>(arguments->children);
...
static bool formatNamedArgWithHiddenValue(IAST *arg, WriteBuffer &ostr, const IAST::FormatSettings &settings, IAST::FormatState &state,
                                          IAST::FormatStateStacked frame) {
  const auto *equals_func = arg->as<ASTFunction>();
  if (!equals_func || (equals_func->name != "equals")) return false;
  ...
  if (equal_args.size() != 2) return false;
...
static bool isAcceptableArgumentsForLambdaExpression(const ASTs &arguments) {
  if (arguments.size() == 2) {
    ...
    if (first_argument->as<ASTIdentifier>()) return true;
    ...
    if (first_argument_function && (first_argument_function->name == "tuple") && first_argument_function->arguments) {
      ...
      if (all_tuple_arguments_are_identifiers) return true;
    }
  }
  return false;
}
...
void ASTFunction::formatImplWithoutAlias(WriteBuffer &ostr, const FormatSettings &settings, FormatState &state,
                                         FormatStateStacked frame) const {
  frame.expression_list_prepend_whitespace = false;
  auto kind = getKind();
  if (kind == Kind::CODEC || kind == Kind::STATISTICS || kind == Kind::BACKUP_NAME) frame.allow_operators = false;
  ...
  if (auto *query = tryGetQueryArgument()) {
    ...
    if (!name.empty()) ostr << backQuoteIfNeed(name);
    ostr << "(";
    ostr << nl_or_nothing;
  ...
  if (arguments && !parameters && arguments->children.size() == 2 && name == "viewIfPermitted"sv) {
    ...
    ostr << nl_or_nothing << indent1 << (settings.one_line ? " " : "") << "ELSE " << nl_or_nothing << indent2;
  ...
  if (isOperator() && arguments && !parameters && frame.allow_operators && getNullsAction() == NullsAction::EMPTY) {
    ...
    if (arguments->children.size() == 1) {
      ...
      if (auto it = std::ranges::find_if(operators, [&](const auto &op) { return boost::iequals(name, op.function_name); });
        ...
        bool is_array = (literal && literal->value.getType() == Field::Types::Array) || (function && function->name == "array");
        bool has_alias = !arguments->children[0]->tryGetAlias().empty();

        /// Do not add parentheses for tuple and array literal, otherwise extra parens will be added `-((3, 7, 3), 1)` -> `-(((3, 7, 3),
        /// 1))`, `-[1]` -> `-([1])`
        ...
        bool inside_parens = !has_alias && ((name == "negate" && (literal_need_parens || (function && function->name == "negate"))) ||
                                            (subquery && name == "not"));
        ...
        if (inside_parens) nested_need_parens.need_parens = false;

        if (outside_parens) ostr << '(';

        ostr << func_symbol;

        if (inside_parens) ostr << '(';
    ...
    if (!written && arguments->children.size() == 2) {
      static constexpr std::array<FunctionOperatorMapping, 21> operators = {{{"multiply", " * "},
                                                                             {"divide", " / "},
                                                                             {"modulo", " % "},
                                                                             {"plus", " + "},
                                                                             {"minus", " - "},
      ...
      if (auto it = std::ranges::find(operators, name, &FunctionOperatorMapping::function_name); it != operators.end()) {
        ...
        bool is_literal_tuple_or_array = second_arg_literal && (second_arg_literal->value.getType() == Field::Types::Tuple ||
        ...
        bool extra_parents_around_in_rhs =
            is_in_operator && ((!arguments->children[1]->as<ASTSubquery>() && !second_arg_func && !is_literal_tuple_or_array) ||
        ...
        if (!extra_parents_around_in_rhs) arguments->children[1]->format(ostr, settings, state, nested_need_parens);
      ...
      if (!written && name == "tupleElement"sv && arguments->children.size() == 2) {
        ...
        if (arguments->children[0]->as<ASTAsterisk>()) tuple_arguments_valid = false;
        ...
        if (tuple_arguments_valid && lit_right) {
          if (isInt64OrUInt64FieldType(lit_right->value.getType()) && lit_right->value.safeGet<Int64>() >= 0) {
            if (frame.need_parens) ostr << '(';
            ...
            bool left_needs_parens =
                arguments->children[0]->size() > 1 && !(left_func && (left_func->name == "array" || left_func->name == "tuple"));
      ...
      if (!written && name == "lambda"sv && isAcceptableArgumentsForLambdaExpression(arguments->children)) {
        ...
        if (frame.need_parens || frame.list_element_index > 0) ostr << '(';

        if (first_argument_is_tuple && first_argument_function->arguments &&
    ...
    if (!written && arguments->children.size() >= 2) {
      constexpr std::array<FunctionOperatorMapping, 2> operators{{{"and", " AND "}, {"or", " OR "}}};

      if (auto it = std::ranges::find(operators, name, &FunctionOperatorMapping::function_name); it != operators.end()) {
        if (frame.need_parens) ostr << '(';
        for (size_t i = 0; i < arguments->children.size(); ++i) {
          if (i != 0) ostr << it->operator_name;
          if (arguments->children[i]->as<ASTSetQuery>()) ostr << "SETTINGS ";
  ...
  if (arguments) {
    ...
    if (!settings.show_secrets) secret_arguments = FunctionSecretArgumentsFinderAST(*this).getResult();

    for (size_t i = 0, size = arguments->children.size(); i < size; ++i) {
      if (i != 0) ostr << ", ";

      const auto &argument = arguments->children[i];
      if (argument->as<ASTSetQuery>()) ostr << "SETTINGS ";
      ...
      if (!settings.show_secrets) {
        ...
        if (function && function->arguments &&
            std::count(secret_arguments.nested_maps.begin(), secret_arguments.nested_maps.end(), function->name) != 0) {
      ...
      if (arguments->children.size() > 1) nested_dont_need_parens.current_function = this;
      argument->format(ostr, settings, state, nested_dont_need_parens);
    }
  }
...
String getFunctionName(const IAST *ast) {
  String res;
  if (tryGetFunctionNameInto(ast, res)) return res;
  if (ast) throw Exception(ErrorCodes::UNEXPECTED_AST_STRUCTURE, "{} is not an function", ast->formatForErrorMessage());
