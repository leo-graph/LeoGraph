...
bool parseEnumValues(IParser::Pos &pos, std::vector<std::pair<String, Int64>> &values, Expected &expected) {
  ...
  while (true) {
    ...
    if (first_char == 'b' || first_char == 'B' || first_char == 'x' || first_char == 'X') {
      ...
      if (!string_literal_parser.parse(pos, literal_ast, expected)) return false;
      elem_name = literal_ast->as<ASTLiteral &>().value.safeGet<String>();
    } else {
      ReadBufferFromMemory in(pos->begin, pos->size());
      if (!tryReadQuotedStringWithSQLStyle(elem_name, in) || in.count() != pos->size()) return false;
      ++pos;
    }

    /// Must have explicit value - if not, fall back to generic parser for auto-assignment
    if (pos->type != TokenType::Equals) return false;
    ++pos;
    ...
    if (!tryReadIntText(abs_value, num_in) || num_in.count() != pos->size()) return false;
    ++pos;
...
class DynamicArgumentParser : public IParserBase {
 private:
  ...
  bool parseImpl(Pos &pos, ASTPtr &node, Expected &expected) override {
    ...
    if (!number_parser.parse(pos, number, expected)) return false;
  ...
  bool parseImpl(Pos &pos, ASTPtr &node, Expected &expected) override {
    ...
    if (ParserKeyword(Keyword::SKIP).ignore(pos)) {
      ...
      if (ParserKeyword(Keyword::REGEXP).ignore(pos)) {
        ...
        ASTPtr literal;
        if (!literal_parser.parse(pos, literal, expected)) return false;
      ...
      /// SKIP some.path
        ...
        if (!compound_identifier_parser.parse(pos, compound_identifier, expected)) return false;
    ...
    ParserCompoundIdentifier compound_identifier_parser;
    ASTPtr identifier;
    if (!compound_identifier_parser.parse(pos, identifier, expected)) return false;
...
bool ParserDataType::parseImpl(Pos &pos, ASTPtr &node, Expected &expected) {
  ...
  ASTPtr identifier;
  if (!name_parser.parse(pos, identifier, expected)) return false;
  ...
  /// column attribute rather than type name.
  {
    String n = type_name;
    boost::to_upper(n);
    if (n == "NOT" || n == "NULL" || n == "DEFAULT" || n == "MATERIALIZED" || n == "EPHEMERAL" || n == "ALIAS" || n == "AUTO" ||
        n == "PRIMARY" || n == "COMMENT" || n == "CODEC") {
      expected.add(pos, "type name");
      return false;
    }
  }
  ...
  if (type_name_upper == "NATIONAL") {
  ...
  } else if (type_name_upper == "BINARY" || type_name_upper == "CHARACTER" || type_name_upper == "CHAR" || type_name_upper == "NCHAR") {
    ...
    if (ParserKeyword(Keyword::PRECISION).ignore(pos)) type_name_suffix = toStringView(Keyword::PRECISION);
    ...
    if (ParserKeyword(Keyword::SIGNED).ignore(pos, expected))
      ...
      ++pos;
      if (pos->type == TokenType::Number) ++pos;
  ...
  if (!type_name_suffix.empty()) type_name = type_name_upper + " " + type_name_suffix;
  ...
  if (pos->type == TokenType::Comma) {
    ...
    auto test_pos = pos;
    ++test_pos;
    if (ParserToken(TokenType::ClosingRoundBracket)
            .ignore(test_pos, test_expected)) {  // the end of the type definition was reached and there was a trailing comma
      ++pos;
    }
  }
  ...
  while (true) {
    ...
    if (type_name == "Dynamic") {
      ...
      if (arg_num == (have_version_of_aggregate_function ? 1 : 0)) {
        ...
        ParserIdentifier identifier_parser;
        function_parser.parse(pos, arg, expected) || identifier_parser.parse(pos, arg, expected);
      } else {
      ...
      enum_parser.parse(pos, arg, expected) || literal_parser.parse(pos, arg, expected) || data_type_parser.parse(pos, arg, expected);
    }
  ...
  if (!expr_list_args->children.empty()) data_type_node->children.push_back(expr_list_args);
