...
ASTPtr ASTSelectQuery::clone() const {
  ...
  res->children.clear();
  for (const auto &child : children) res->children.push_back(child->clone());
...
void ASTSelectQuery::formatImpl(WriteBuffer &ostr, const FormatSettings &s, FormatState &state, FormatStateStacked frame) const {
  ...
  if (with()) {
    ...
    if (recursive_with) ostr << " RECURSIVE";
  ...
  if (!group_by_all && groupBy()) {
    ...
    if (!group_by_with_grouping_sets) {
      s.one_line ? groupBy()->format(ostr, s, state, frame)
                 : groupBy()->as<ASTExpressionList &>().formatImplMultiline(ostr, s, state, frame);
    }
  }

  if (group_by_all) ostr << s.nl_or_ws << indent_str << "GROUP BY ALL";
  ...
  if (group_by_with_grouping_sets && groupBy()) {
    ...
    ostr << " (";
    s.one_line ? groupBy()->format(ostr, s, state, nested_frame)
               : groupBy()->as<ASTExpressionList &>().formatImplMultiline(ostr, s, state, nested_frame);
    ostr << ")";
  }

  if (group_by_with_rollup) ostr << s.nl_or_ws << indent_str << (s.one_line ? "" : "    ") << "WITH ROLLUP";

  if (group_by_with_cube) ostr << s.nl_or_ws << indent_str << (s.one_line ? "" : "    ") << "WITH CUBE";

  if (group_by_with_totals) ostr << s.nl_or_ws << indent_str << (s.one_line ? "" : "    ") << "WITH TOTALS";
  ...
  if (window()) {
    ostr << s.nl_or_ws << indent_str << "WINDOW";
    s.one_line ? window()->format(ostr, s, state, frame) : window()->as<ASTExpressionList &>().formatImplMultiline(ostr, s, state, frame);
  }
  ...
  if (!order_by_all && orderBy()) {
    ...
    s.one_line ? orderBy()->format(ostr, s, state, frame) : orderBy()->as<ASTExpressionList &>().formatImplMultiline(ostr, s, state, frame);
  ...
  if (limitLength()) {
    ...
    if (limit_with_ties) ostr << s.nl_or_ws << indent_str << " WITH TIES";
  } else if (limitOffset()) {
...
static const ASTTablesInSelectQueryElement *getFirstTableJoin(const ASTSelectQuery &select) {
  ...
  const auto &tables_in_select_query = select.tables()->as<ASTTablesInSelectQuery &>();
  if (tables_in_select_query.children.empty()) return nullptr;
...
bool ASTSelectQuery::withFill() const {
  ...
  for (const auto &order_expression_element : order_by->children)
    if (order_expression_element->as<ASTOrderByElement &>().with_fill) return true;
...
std::pair<ASTPtr, bool> ASTSelectQuery::arrayJoinExpressionList() const {
  ...
  if (!array_join) return {};

  bool is_left = (array_join->kind == ASTArrayJoin::Kind::Left);
  return {array_join->expression_list, is_left};
}

const ASTTablesInSelectQueryElement *ASTSelectQuery::join() const { return getFirstTableJoin(*this); }

bool ASTSelectQuery::hasJoin() const {
  if (!tables()) return false;

  const auto &tables_in_select_query = tables()->as<ASTTablesInSelectQuery &>();
  if (tables_in_select_query.children.empty()) return false;

  for (const auto &child : tables_in_select_query.children) {
    const auto &tables_element = child->as<ASTTablesInSelectQueryElement &>();
    if (tables_element.table_join) return true;
  }
...
static String getTableExpressionAlias(const ASTTableExpression *table_expression) {
  if (table_expression->subquery) return table_expression->subquery->tryGetAlias();
  if (table_expression->table_function) return table_expression->table_function->tryGetAlias();
  if (table_expression->database_and_table_name) return table_expression->database_and_table_name->tryGetAlias();
...
void ASTSelectQuery::replaceDatabaseAndTable(const StorageID &table_id) {
  ...
  if (!table_alias.empty()) table_expression->database_and_table_name->setAlias(table_alias);
}
...
void ASTSelectQuery::addTableFunction(const ASTPtr &table_function_ptr) {
  ...
  if (table_alias.empty()) table_expression->table_function->setAlias(table_alias);
}
...
void ASTSelectQuery::setExpression(Expression expr, ASTPtr &&ast) {
  if (ast) {
    ...
    for (auto &pr : positions)
      if (pr.second > pos) --pr.second;
  }
}

ASTPtr &ASTSelectQuery::getExpression(Expression expr) {
  if (!positions.contains(expr)) throw Exception(ErrorCodes::LOGICAL_ERROR, "Get expression before set");
...
void ASTSelectQuery::normalizeChildrenOrder() {
  ...
  static constexpr Expression canonical_order[] = {
      Expression::WITH,         Expression::SELECT,       Expression::TABLES,          Expression::ALIASES,         Expression::CTE_ALIASES,
      Expression::PREWHERE,     Expression::WHERE,        Expression::GROUP_BY,        Expression::HAVING,          Expression::WINDOW,
      Expression::QUALIFY,      Expression::ORDER_BY,     Expression::LIMIT_BY_OFFSET, Expression::LIMIT_BY_LENGTH, Expression::LIMIT_BY,
      Expression::LIMIT_OFFSET, Expression::LIMIT_LENGTH, Expression::SETTINGS,        Expression::INTERPOLATE,
  };
...
void ASTSelectQuery::setFinal()  // NOLINT method can be made const
{
  ...
  if (tables_in_select_query.children.empty()) throw Exception(ErrorCodes::LOGICAL_ERROR, "Tables list is empty, it's a bug");

  auto &tables_element = tables_in_select_query.children[0]->as<ASTTablesInSelectQueryElement &>();

  if (!tables_element.table_expression) throw Exception(ErrorCodes::LOGICAL_ERROR, "There is no table expression, it's a bug");
