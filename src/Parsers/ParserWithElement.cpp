...
bool ParserWithElement::parseImpl(Pos& pos, ASTPtr& node, Expected& expected) {
  ...
  if (ASTPtr name_or_expr;
      ...
      s_as.ignore(pos, expected) && s_subquery.parse(pos, with_element->subquery, expected)) {
    if (name_or_expr) tryGetIdentifierNameInto(name_or_expr, with_element->name);
    ...
    if (!s_expr.parse(pos, node, expected)) return false;
  }
  return true;
}
