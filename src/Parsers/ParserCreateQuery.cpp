...
bool ParserSQLSecurity::parseImpl(Pos &pos, ASTPtr &node, Expected &expected) {
  ...
  result->type = type;
  if (definer) result->definer = boost::static_pointer_cast<ASTUserNameWithHost>(definer);
...
bool ParserColumnDeclarationList::parseImpl(Pos &pos, ASTPtr &node, Expected &expected) {
  return ParserList(std::make_unique<ParserColumnDeclaration>(require_type, allow_null_modifiers, check_keywords_after_name),
...
bool ParserIndexDeclaration::parseImpl(Pos &pos, ASTPtr &node, Expected &expected) {
  ...
  if (!name_p.parse(pos, name, expected)) return false;

  if (!expression_p.parse(pos, expr, expected)) return false;

  if (!s_type.ignore(pos, expected)) return false;
...
bool ParserStatisticsDeclaration::parseImpl(Pos &pos, ASTPtr &node, Expected &expected) {
  ...
  if (!columns_p.parse(pos, columns, expected)) return false;

  if (!s_type.ignore(pos, expected)) return false;

  if (!types_p.parse(pos, types, expected)) return false;
...
bool ParserProjectionDeclaration::parseImpl(Pos &pos, ASTPtr &node, Expected &expected) {
  ...
  if (s_lparen.ignore(pos, expected)) {
    if (!query_p.parse(pos, query, expected)) return false;

    if (!s_rparen.ignore(pos, expected)) return false;
  } else if (s_index.ignore(pos, expected)) {
    if (!expression_p.parse(pos, index, expected)) return false;
  ...
  if (s_with_settings.ignore(pos, expected)) {
    if (!s_lparen.ignore(pos, expected)) return false;

    if (!settings_p.parse(pos, with_settings, expected)) return false;
  ...
  if (query) projection->set(projection->query, query);
  if (index) projection->set(projection->index, index);
  if (type) projection->set(projection->type, type);
  if (with_settings) projection->set(projection->with_settings, with_settings);
  node = projection;
...
bool ParserForeignKeyDeclaration::parseImpl(Pos &pos, ASTPtr &node, Expected &expected) {
  ...
  if (!s_references.ignore(pos, expected)) return false;

  if (!table_name_p.parse(pos, name, expected)) return false;
  ...
  while (s_on.ignore(pos, expected)) {
    ...
    if (!s_delete.ignore(pos, expected) && !s_update.ignore(pos, expected)) return false;
    ...
    if (!s_restrict.ignore(pos, expected) && !s_cascade.ignore(pos, expected) && !s_set_null.ignore(pos, expected) &&
        !s_no_action.ignore(pos, expected) && !s_set_default.ignore(pos, expected)) {
      return false;
    }
  }
...
bool ParserTablePropertyDeclaration::parseImpl(Pos &pos, ASTPtr &node, Expected &expected) {
  ...
  if (s_index.ignore(pos, expected)) {
    if (!index_p.parse(pos, new_node, expected)) return false;
  } else if (s_constraint.ignore(pos, expected)) {
    if (!constraint_p.parse(pos, new_node, expected)) return false;
  } else if (s_projection.ignore(pos, expected)) {
    if (!projection_p.parse(pos, new_node, expected)) return false;
  } else if (s_primary_key.ignore(pos, expected)) {
    if (!primary_key_p.parse(pos, new_node, expected)) return false;
  } else if (s_foreign_key.ignore(pos, expected)) {
    if (!foreign_key_p.parse(pos, new_node, expected)) return false;
  } else {
    if (!column_p.parse(pos, new_node, expected)) return false;
  }
...
bool ParserTablePropertiesDeclarationList::parseImpl(Pos &pos, ASTPtr &node, Expected &expected) {
  ...
  for (const auto &elem : list->children) {
    if (auto *cd = elem->as<ASTColumnDeclaration>()) {
      if (cd->primary_key_specifier) {
        if (!primary_key_from_columns) primary_key_from_columns = makeASTOperator("tuple");
  ...
  if (!columns->children.empty()) res->set(res->columns, columns);
  if (!indices->children.empty()) res->set(res->indices, indices);
  if (!constraints->children.empty()) res->set(res->constraints, constraints);
  if (!projections->children.empty()) res->set(res->projections, projections);
  if (primary_key) res->set(res->primary_key, primary_key);
  if (primary_key_from_columns) res->set(res->primary_key_from_columns, primary_key_from_columns);
...
bool ParserStorageOrderByClause::parseImpl(Pos &pos, ASTPtr &node, Expected &expected) {
  ...
  if (order_elem_p.parse(pos, order_by, expected)) {
    ...
    if (const auto *elem = order_by->as<ASTStorageOrderByElement>(); elem && elem->direction > 0) order_by = elem->children.front();
  ...
  if (pos->type == TokenType::BareWord && std::string_view(pos->begin, pos->size()) == "tuple") ++pos;

  if (!s_lparen.ignore(pos, expected)) return false;

  if (!order_list_p.parse(pos, order_by, expected)) order_by = make_intrusive<ASTExpressionList>();
  ...
  if (all_default_direction) {
    for (auto &child : order_by->children) {
      if (const auto *elem = child->as<ASTStorageOrderByElement>()) child = elem->children.front();
    }
  }
...
bool ParserStorage::parseImpl(Pos &pos, ASTPtr &node, Expected &expected) {
  ...
  if (parsed_engine_keyword) {
    ...
    if (!ident_with_optional_params_p.parse(pos, engine, expected)) return false;
    storage_like = true;
  }
  ...
  while (true) {
    ...
    if ((engine_kind == TABLE_ENGINE || parsed_engine_keyword) && s_settings.ignore(pos, expected)) {
      if (!settings_p.parse(pos, settings, expected)) return false;
      storage_like = true;
    }
...
bool ParserCreateTableQuery::parseImpl(Pos &pos, ASTPtr &node, Expected &expected) {
  ...
  if (s_create.ignore(pos, expected)) {
    if (s_or_replace.ignore(pos, expected)) replace = or_replace = true;
  ...
  if (s_temporary.ignore(pos, expected)) is_temporary = true;
  if (!s_table.ignore(pos, expected)) return false;

  if (!replace && !or_replace && s_if_not_exists.ignore(pos, expected)) if_not_exists = true;

  if (!table_name_p.parse(pos, table, expected)) return false;

  if (ParserKeyword{Keyword::TO_INNER_UUID}.ignore(pos, expected)) {
    ParserStringLiteral literal_p;
    if (!literal_p.parse(pos, to_inner_uuid, expected)) return false;
  }
  ...
  if (attach) {
    if (s_from.ignore(pos, expected)) {
      ...
      if (!from_path_p.parse(pos, from_path, expected)) return false;
    } else if (s_as.ignore(pos, expected)) {
      if (s_not.ignore(pos, expected)) attach_as_replicated = false;
      if (!s_replicated.ignore(pos, expected)) return false;
      if (!attach_as_replicated.has_value()) attach_as_replicated = true;
    }
  }
  ...
  auto parse_storage = [&] {
    chassert(!storage);
    ASTPtr ast;
    if (!storage_p.parse(pos, ast, expected)) return false;
  ...
  auto try_parse_empty_or_clone = [&is_create_empty, &is_clone_as, &pos, &expected]() {
    if (is_create_empty || is_clone_as) return;
  ...
  if (s_lparen.ignore(pos, expected)) {
    ...
    if (!table_properties_p.parse(pos, columns_list, expected)) return false;
    ...
    if (is_create_empty || is_clone_as) {
      if (!ParserKeyword{Keyword::AS}.ignore(pos, expected)) return false;
      has_as = true;
    } else
      has_as = ParserKeyword{Keyword::AS}.ignore(pos, expected);

    if ((storage_parse_result || is_temporary) && has_as) {
      if (!select_p.parse(pos, select, expected)) return false;
    }

    if (!storage_parse_result && !is_temporary && has_as) {
      if (!table_function_p.parse(pos, as_table_function, expected)) return false;
    }
   ...
   */
    ...
    try_parse_empty_or_clone();
    if (!comment) comment = parseComment(pos, expected);
    try_parse_empty_or_clone();
    ...
    if (has_as) {
      if (!select_p.parse(pos, select, expected))  /// AS SELECT ...
      {
        ...
        if (storage || !table_function_p.parse(pos, as_table_function, expected)) {
          /// AS [db.]table
          if (!name_p.parse(pos, as_table, expected)) return false;
  ...
  if (comment) query->set(query->comment, comment);
  ...
  if (to_inner_uuid) {
    ...
    if (targets) throw Exception(ErrorCodes::BAD_ARGUMENTS, "targets are already defined {}", targets->formatForErrorMessage());
...
bool ParserCreateWindowViewQuery::parseImpl(Pos &pos, ASTPtr &node, Expected &expected) {
  ...
  if (!s_window.ignore(pos, expected)) return false;

  if (!s_view.ignore(pos, expected)) return false;
  ...
  if (s_to.ignore(pos, expected)) {
    if (!table_name_p.parse(pos, to_table, expected)) return false;
  }
  ...
  if (to_table || storage || inner_storage) {
    ...
    if (to_table) targets->setTableID(ViewTarget::To, to_table->as<ASTTableIdentifier>()->getTableId());
    if (storage) targets->setInnerEngine(ViewTarget::To, storage);
    if (inner_storage) targets->setInnerEngine(ViewTarget::Inner, inner_storage);
  }
  ...
  if (s_allowed_lateness.ignore(pos, expected)) {
    ...
    if (!lateness_p.parse(pos, lateness, expected)) return false;
  }
  ...
  auto try_parse_populate_or_empty = [&is_populate, &is_create_empty, &pos, &expected, &s_populate, &s_empty]() {
    if (is_populate || is_create_empty) return;
  ...
  /// AS SELECT ...
  if (!s_as.ignore(pos, expected)) return false;
...
bool ParserTableOverrideDeclaration::parseImpl(Pos &pos, ASTPtr &node, Expected &expected) {
  ...
  if (is_standalone) {
    if (!s_table_override.ignore(pos, expected)) return false;
    if (!table_name_p.parse(pos, table_name, expected)) return false;
    if (!lparen_p.ignore(pos, expected)) return false;
  }

  while (true) {
    if (!columns && s_columns.ignore(pos, expected)) {
      if (!lparen_p.ignore(pos, expected)) return false;
      if (!table_properties_p.parse(pos, columns, expected)) return false;
      if (!rparen_p.ignore(pos, expected)) return false;
    }

    if (!partition_by && s_partition_by.ignore(pos, expected)) {
      if (expression_p.parse(pos, partition_by, expected)) continue;
      return false;
    }

    if (!primary_key && s_primary_key.ignore(pos, expected)) {
      if (expression_p.parse(pos, primary_key, expected)) continue;
      return false;
    }

    if (!order_by && s_order_by.ignore(pos, expected)) {
      if (expression_p.parse(pos, order_by, expected)) continue;
      return false;
    }

    if (!sample_by && s_sample_by.ignore(pos, expected)) {
      if (expression_p.parse(pos, sample_by, expected)) continue;
      return false;
    }

    if (!ttl_table && s_ttl.ignore(pos, expected)) {
      if (parser_ttl_list.parse(pos, ttl_table, expected)) continue;
      return false;
    }
  ...
  if (is_standalone && !rparen_p.ignore(pos, expected)) return false;
  ...
  if (table_name) res->table_name = table_name->as<ASTIdentifier>()->name();
  res->is_standalone = is_standalone;
  res->set(res->storage, storage);
  if (columns) res->set(res->columns, columns);
...
bool ParserTableOverridesDeclarationList::parseImpl(Pos &pos, ASTPtr &node, Expected &expected) {
  ...
  auto parse_element = [&] {
    ASTPtr element;
    if (!table_override_p.parse(pos, element, expected)) return false;
    auto *table_override = element->as<ASTTableOverride>();
    if (!table_override) return false;
    res->setTableOverride(table_override->table_name, element);
    return true;
  };

  if (!ParserList::parseUtil(pos, expected, parse_element, s_comma, true)) return false;

  if (!res->children.empty()) node = res;
...
bool ParserCreateDatabaseQuery::parseImpl(Pos &pos, ASTPtr &node, Expected &expected) {
  ...
  if (!s_database.ignore(pos, expected)) return false;

  if (s_if_not_exists.ignore(pos, expected)) if_not_exists = true;

  if (!name_p.parse(pos, database, expected)) return false;

  if (s_uuid.ignore(pos, expected)) {
    ParserStringLiteral uuid_p;
    ASTPtr ast_uuid;
    if (!uuid_p.parse(pos, ast_uuid, expected)) return false;
  ...
  if (!table_overrides_p.parse(pos, table_overrides, expected)) return false;
  ...
  query->set(query->storage, storage);
  if (comment) query->set(query->comment, comment);
  if (table_overrides && !table_overrides->children.empty()) query->set(query->table_overrides, table_overrides);
...
bool ParserCreateViewQuery::parseImpl(Pos &pos, ASTPtr &node, Expected &expected) {
  ...
  if (!replace_view && s_if_not_exists.ignore(pos, expected)) if_not_exists = true;
  ...
  if (ParserKeyword{Keyword::REFRESH}.ignore(pos, expected)) {
    ...
    if (!is_materialized_view) return false;
    if (!ParserRefreshStrategy{}.parse(pos, refresh_strategy, expected)) return false;
  }
  ...
  if (s_lparen.ignore(pos, expected))  // Parsing cases like CREATE VIEW (a Int64, b Int64) (a, b) SELECT ...
  {
    ...
    if (!table_properties_p.parse(pos, columns_list, expected)) {
      ...
      if (s_lparen.ignore(pos, expected)) {
        has_aliases = true;
        if (!expr_list_aliases.parse(pos, aliases_list, expected)) return false;
      }
    }
    ...
    if (!to_table) {
      ...
      if (s_to.ignore(pos, expected))
        throw Exception(ErrorCodes::SYNTAX_ERROR,
                        "When creating a materialized view you can't declare both 'ENGINE' and 'TO [db].[table]'");
    } else {
      if (storage_p.ignore(pos, expected))
        throw Exception(ErrorCodes::SYNTAX_ERROR,
                        "When creating a materialized view you can't declare both 'TO [db].[table]' and 'ENGINE'");

      if (s_populate.ignore(pos, expected))
        throw Exception(ErrorCodes::SYNTAX_ERROR,
                        "When creating a materialized view you can't declare both 'TO [db].[table]' and 'POPULATE'");

      if (s_empty.ignore(pos, expected)) {
        if (!refresh_strategy)
          throw Exception(ErrorCodes::SYNTAX_ERROR,
                          "When creating a materialized view you can't declare both 'TO [db].[table]' and 'EMPTY'");
  ...
  auto try_parse_populate_or_empty = [&]() {
    if (!is_materialized_view || is_populate || is_create_empty) return;
    if (!to_table) {
      if (s_populate.ignore(pos, expected))
        is_populate = true;
      else if (s_empty.ignore(pos, expected))
  ...
  if (refresh_strategy) query->set(query->refresh_strategy, refresh_strategy);
  if (comment) query->set(query->comment, comment);
  if (sql_security) query->set(query->sql_security, sql_security);

  if (query->columns_list && query->columns_list->primary_key) {
    /// If engine is not set will use default one
    if (!storage) storage = make_intrusive<ASTStorage>();
    auto &storage_ref = typeid_cast<ASTStorage &>(*storage);
    if (storage_ref.primary_key) throw Exception(ErrorCodes::BAD_ARGUMENTS, "Multiple primary keys are not allowed.");
...
bool ParserCreateNamedCollectionQuery::parseImpl(Pos &pos, ASTPtr &node, Expected &expected) {
  ...
  if (!s_named_collection.ignore(pos, expected)) return false;

  if (s_if_not_exists.ignore(pos, expected)) if_not_exists = true;

  if (!name_p.parse(pos, collection_name, expected)) return false;
  ...
  while (true) {
    if (!changes.empty() && !s_comma.ignore(pos)) break;

    changes.push_back(SettingChange{});

    if (!ParserSetQuery::parseNameValuePair(changes.back(), pos, expected)) return false;
...
bool ParserCreateDictionaryQuery::parseImpl(IParser::Pos &pos, ASTPtr &node, Expected &expected) {
  ...
  if (!s_dictionary.ignore(pos, expected)) return false;

  if (s_if_not_exists.ignore(pos, expected)) if_not_exists = true;

  if (!dict_name_p.parse(pos, name, expected)) return false;
  ...
  if (!attach) {
    if (!s_left_paren.ignore(pos, expected)) return false;

    if (!attributes_p.parse(pos, attributes, expected)) return false;

    /// We allow a trailing comma in the columns list for user convenience.
    /// Although it diverges from the SQL standard slightly.
    s_comma.ignore(pos, expected);

    if (!s_right_paren.ignore(pos, expected)) return false;

    if (!dictionary_p.parse(pos, dictionary, expected)) return false;
  }
...
bool ParserCreateQuery::parseImpl(Pos &pos, ASTPtr &node, Expected &expected) {
  ...
  return table_p.parse(pos, node, expected) || database_p.parse(pos, node, expected) || view_p.parse(pos, node, expected) ||
         dictionary_p.parse(pos, node, expected) || window_view_p.parse(pos, node, expected);
}
