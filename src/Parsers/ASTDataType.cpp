...
String ASTDataType::getID(char delim) const { return "DataType" + (delim + name); }
...
ASTPtr ASTDataType::clone() const {
  ...
  if (arguments) res->children.push_back(arguments->clone());
...
void ASTDataType::resetArguments() { children.clear(); }
