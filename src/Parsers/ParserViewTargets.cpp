...
ParserViewTargets::ParserViewTargets() {
  for (auto kind : magic_enum::enum_values<ViewTarget::Kind>()) accept_kinds.push_back(kind);
}
...
bool ParserViewTargets::parseImpl(Pos& pos, ASTPtr& node, Expected& expected) {
  ...
  auto result = [&] -> ASTViewTargets& {
    if (!res) res = make_intrusive<ASTViewTargets>();
    return *res;
  };
  ...
  if (!res || res->targets.empty()) return false;
