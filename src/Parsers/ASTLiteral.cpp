...
void ASTLiteral::updateTreeHashImpl(SipHash &hash_state, bool ignore_aliases) const {
  ...
  if (!ignore_aliases) ASTWithAlias::updateTreeHashImpl(hash_state, ignore_aliases);
}

String ASTLiteral::getID(char delim) const { return "Literal" + (delim + applyVisitor(FieldVisitorDump(), value)); }
...
void ASTLiteral::formatImplWithoutAlias(WriteBuffer &ostr, const FormatSettings &settings, IAST::FormatState &,
