...
ASTPtr ASTTablesInSelectQuery::clone() const {
  ...
  for (const auto &child : children) res->children.emplace_back(child->clone());
...
void ASTTableExpression::formatImpl(WriteBuffer &ostr, const FormatSettings &settings, FormatState &state, FormatStateStacked frame) const {
  ...
  if (final) {
    ostr << settings.nl_or_ws << indent_str << "FINAL";
  }

  if (sample_size) {
    ostr << settings.nl_or_ws << indent_str << "SAMPLE ";
...
void ASTTableJoin::formatImplBeforeTable(WriteBuffer &ostr, const FormatSettings &settings, FormatState &, FormatStateStacked frame) const {
  ...
  if (kind != JoinKind::Comma) ostr << settings.nl_or_ws << indent_str;
...
void ASTTableJoin::formatImplAfterTable(WriteBuffer &ostr, const FormatSettings &settings, FormatState &state,
  ...
  if (using_expression_list) {
    ...
    if (!on_alias.empty()) {
      bool was_alias_defined_earlier =
...
void ASTArrayJoin::formatImpl(WriteBuffer &ostr, const FormatSettings &settings, FormatState &state, FormatStateStacked frame) const {
  ...
  ostr << settings.nl_or_ws << indent_str << (kind == Kind::Left ? "LEFT " : "") << "ARRAY JOIN";

  settings.one_line ? expression_list->format(ostr, settings, state, frame)
                    : expression_list->as<ASTExpressionList &>().formatImplMultiline(ostr, settings, state, frame);
}

void ASTTablesInSelectQueryElement::formatImpl(WriteBuffer &ostr, const FormatSettings &settings, FormatState &state,
                                               FormatStateStacked frame) const {
  if (table_expression) {
    if (table_join) table_join->as<ASTTableJoin &>().formatImplBeforeTable(ostr, settings, state, frame);

    table_expression->format(ostr, settings, state, frame);

    if (table_join) table_join->as<ASTTableJoin &>().formatImplAfterTable(ostr, settings, state, frame);
  } else if (array_join) {
    array_join->format(ostr, settings, state, frame);
  }
}

void ASTTablesInSelectQuery::formatImpl(WriteBuffer &ostr, const FormatSettings &settings, FormatState &state,
                                        FormatStateStacked frame) const {
  for (const auto &child : children) child->format(ostr, settings, state, frame);
}
