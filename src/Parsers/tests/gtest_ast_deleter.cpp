...
struct StackDecrementer {
  ...
  virtual ~StackDecrementer() { --depth; }
};
...
class ASTWithMembers : public ASTCounting {
 public:
  explicit ASTWithMembers(DB::ASTs members_, size_t& depth_) : ASTCounting(depth_), members(std::move(members_)) { children = members; }
...
class ASTWithDecrementer : public ASTWithMembers {
 public:
  ...
  ~ASTWithDecrementer() override {
    ++depth;
    if (depth == max_depth) std::cout << StackTrace().toString() << std::endl;
    if (depth > max_depth) EXPECT_LE(depth, max_depth);
  }
};
...
TEST(ASTDeleter, SimpleChain) {
...

  {
    ...
    for (size_t i = 0; i < chain_length; ++i) ast = make_intrusive<ASTWithDecrementer>(DB::ASTs{std::move(ast)}, depth, max_depth);
  }
}
...

  {
    ...
    for (size_t i = 0; i < chain_length; ++i) {
      ...
      if (i > member_depth) {
        ...
        for (size_t j = 0; j < member_depth; ++j) member = member->children.front();
