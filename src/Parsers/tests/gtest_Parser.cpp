...
std::ostream &operator<<(std::ostream &ostr, const std::shared_ptr<IParser> parser) { return ostr << "Parser: " << parser->getName(); }
...
TEST_P(ParserTest, parseQuery) {
  ...
  if (expected_ast) {
    if (std::string(expected_ast).starts_with("throws")) {
      EXPECT_THROW(parseQuery(*parser, input_text.data(), input_text.data() + input_text.size(), 0, 0, 0),
                   DB::Exception);  /// NOLINT(bugprone-suspicious-stringview-data-usage)
    } else {
      ASTPtr ast;
      ASSERT_NO_THROW(ast = parseQuery(*parser, input_text.data(), input_text.data() + input_text.size(), 0, 0,
      ...
      if (std::string("CREATE USER or ALTER USER query") != parser->getName() &&
        ...
        for (auto &child : ast_clone2->children) {
          if (auto *identifier = dynamic_cast<ASTIdentifier *>(child.get())) identifier->setShortName("new_name");
        }
    ...
    ASSERT_THROW(parseQuery(*parser, input_text.data(), input_text.data() + input_text.size(), 0, 0, 0),
                 DB::Exception);  /// NOLINT(bugprone-suspicious-stringview-data-usage)
  }
}

INSTANTIATE_TEST_SUITE_P(
    ParserOptimizeQuery, ParserTest,
    ::testing::Combine(
        ::testing::Values(std::make_shared<ParserOptimizeQuery>()),
        ::testing::ValuesIn(std::initializer_list<ParserTestCase>{
            {"OPTIMIZE TABLE table_name DEDUPLICATE BY COLUMNS('a, b')", "OPTIMIZE TABLE table_name DEDUPLICATE BY COLUMNS('a, b')"},
            {"OPTIMIZE TABLE table_name DEDUPLICATE BY COLUMNS('[a]')", "OPTIMIZE TABLE table_name DEDUPLICATE BY COLUMNS('[a]')"},
            {"OPTIMIZE TABLE table_name DEDUPLICATE BY COLUMNS('[a]') EXCEPT b",
             "OPTIMIZE TABLE table_name DEDUPLICATE BY COLUMNS('[a]') EXCEPT b"},
            {"OPTIMIZE TABLE table_name DEDUPLICATE BY COLUMNS('[a]') EXCEPT (a, b)",
             "OPTIMIZE TABLE table_name DEDUPLICATE BY COLUMNS('[a]') EXCEPT (a, b)"},
            {"OPTIMIZE TABLE table_name DEDUPLICATE BY a, b, c", "OPTIMIZE TABLE table_name DEDUPLICATE BY a, b, c"},
            {"OPTIMIZE TABLE table_name DEDUPLICATE BY *", "OPTIMIZE TABLE table_name DEDUPLICATE BY *"},
            {"OPTIMIZE TABLE table_name DEDUPLICATE BY * EXCEPT a", "OPTIMIZE TABLE table_name DEDUPLICATE BY * EXCEPT a"},
            {"OPTIMIZE TABLE table_name DEDUPLICATE BY * EXCEPT (a, b)", "OPTIMIZE TABLE table_name DEDUPLICATE BY * EXCEPT (a, b)"}})));

INSTANTIATE_TEST_SUITE_P(ParserOptimizeQuery_FAIL, ParserTest,
                         ::testing::Combine(::testing::Values(std::make_shared<ParserAlterCommand>(false)),
                                            ::testing::ValuesIn(std::initializer_list<ParserTestCase>{
                                                {
...
INSTANTIATE_TEST_SUITE_P(ParserAlterCommand_MODIFY_COMMENT_WITH_PARENS, ParserTest,
                         ::testing::Combine(::testing::Values(std::make_shared<ParserAlterCommand>(true)),
                                            ::testing::ValuesIn(std::initializer_list<ParserTestCase>{
                                                {
                                                    // Empty comment value
                                                    "(MODIFY COMMENT '')",
...
INSTANTIATE_TEST_SUITE_P(ParserCreateQuery_DICTIONARY_WITH_COMMENT, ParserTest,
                         ::testing::Combine(::testing::Values(std::make_shared<ParserCreateQuery>()),
                                            ::testing::ValuesIn(std::initializer_list<ParserTestCase>{{
...
INSTANTIATE_TEST_SUITE_P(
    ParserCreateDatabaseQuery, ParserTest,
    ::testing::Combine(
        ::testing::Values(std::make_shared<ParserCreateQuery>()),
        ::testing::ValuesIn(std::initializer_list<ParserTestCase>{
            {"CREATE DATABASE db ENGINE=Foo TABLE OVERRIDE `tbl` (), TABLE OVERRIDE a (COLUMNS (_created DateTime MATERIALIZED now())), "
             "TABLE OVERRIDE b (PARTITION BY rand())",
             "CREATE DATABASE db\nENGINE = Foo\nTABLE OVERRIDE tbl\n(\n\n),\nTABLE OVERRIDE a\n(\n    COLUMNS\n    (\n        `_created` "
             "DateTime MATERIALIZED now()\n    )\n),\nTABLE OVERRIDE b\n(\n    PARTITION BY rand()\n)"},
            {"CREATE DATABASE db TABLE OVERRIDE tbl (COLUMNS (INDEX foo foo TYPE minmax GRANULARITY 1) PARTITION BY if(_staged = 1, "
             "'staging', toYYYYMM(created)))",
             "CREATE DATABASE db\nTABLE OVERRIDE tbl\n(\n    COLUMNS\n    (\n        INDEX foo foo TYPE minmax GRANULARITY 1\n    )\n    "
             "PARTITION BY if(_staged = 1, 'staging', toYYYYMM(created))\n)"},
            {"CREATE DATABASE db TABLE OVERRIDE t1 (TTL inserted + INTERVAL 1 MONTH DELETE), TABLE OVERRIDE t2 (TTL `inserted` + INTERVAL "
             "2 MONTH DELETE)",
            ...
            {"CREATE DATABASE db ENGINE = Foo() SETTINGS a = 1, b = 2", "CREATE DATABASE db\nENGINE = Foo\nSETTINGS a = 1, b = 2"},
...
INSTANTIATE_TEST_SUITE_P(
    ParserCreateUserQuery, ParserTest,
    ::testing::Combine(
        ...
        ::testing::ValuesIn(std::initializer_list<ParserTestCase>{
            ...
            {"CREATE USER user1 IDENTIFIED WITH scram_sha256_password BY 'qwe123'",
             "CREATE USER user1 IDENTIFIED WITH scram_sha256_password BY 'qwe123'"},
            {"CREATE USER user1 IDENTIFIED WITH no_password", "CREATE USER user1 IDENTIFIED WITH no_password"},
            {"CREATE USER user1", "CREATE USER user1"},
            {"CREATE USER user1 IDENTIFIED WITH plaintext_password BY 'abc123', plaintext_password BY 'def123', sha256_password BY "
             "'ghi123'",
             "CREATE USER user1 IDENTIFIED WITH plaintext_password BY 'abc123', plaintext_password BY 'def123', sha256_password BY "
             "'ghi123'"},
            {"CREATE USER user1 IDENTIFIED WITH sha256_hash BY '7A37B85C8918EAC19A9089C0FA5A2AB4DCE3F90528DCDEEC108B23DDF3607B99' SALT "
             "'salt'",
             "CREATE USER user1 IDENTIFIED WITH sha256_hash BY '7A37B85C8918EAC19A9089C0FA5A2AB4DCE3F90528DCDEEC108B23DDF3607B99' SALT "
             "'salt'"},
            {"CREATE USER user1 IDENTIFIED WITH scram_sha256_hash BY '04e7a70338d7af7bb6142fe7e19fef46d9b605f3e78b932a60e8200ef9154976' "
             "SALT ''",
             "CREATE USER user1 IDENTIFIED WITH scram_sha256_hash BY '04e7a70338d7af7bb6142fe7e19fef46d9b605f3e78b932a60e8200ef9154976' "
             "SALT ''"},
            ...
            {"ALTER USER user1 IDENTIFIED WITH plaintext_password BY 'abc123', plaintext_password BY 'def123', sha256_password BY 'ghi123'",
             "ALTER USER user1 IDENTIFIED WITH plaintext_password BY 'abc123', plaintext_password BY 'def123', sha256_password BY "
             "'ghi123'"},
            {"ALTER USER user1 IDENTIFIED WITH sha256_hash BY '7A37B85C8918EAC19A9089C0FA5A2AB4DCE3F90528DCDEEC108B23DDF3607B99' SALT "
             "'salt'",
             "ALTER USER user1 IDENTIFIED WITH sha256_hash BY '7A37B85C8918EAC19A9089C0FA5A2AB4DCE3F90528DCDEEC108B23DDF3607B99' SALT "
             "'salt'"},
            {"CREATE USER user1 IDENTIFIED WITH sha256_password BY 'qwe123' SALT "
             "'EFFD7F6B03B3EA68B8F86C1E91614DD50E42EB31EF7160524916444D58B5E264'",
             "throws Syntax error"},
...
INSTANTIATE_TEST_SUITE_P(
    ParserAttachUserQuery, ParserTest,
    ::testing::Combine(::testing::Values(std::make_shared<ParserAttachAccessEntity>()),
                       ::testing::ValuesIn(std::initializer_list<ParserTestCase>{
                           {"ATTACH USER user1 IDENTIFIED WITH sha256_hash BY "
                            "'2CC4880302693485717D34E06046594CFDFE425E3F04AA5A094C4AABAB3CB0BF' SALT "
                            "'EFFD7F6B03B3EA68B8F86C1E91614DD50E42EB31EF7160524916444D58B5E264';",
                            "^[A-Za-z0-9]{64}$"},
                           {"ATTACH USER user1 IDENTIFIED WITH sha256_hash BY "
                            "'2CC4880302693485717D34E06046594CFDFE425E3F04AA5A094C4AABAB3CB0BF'",  // for users created in older releases
                                                                                                   // that sha256_password has no salt
                            "^$"}})));

INSTANTIATE_TEST_SUITE_P(ParserRenameQuery, ParserTest,
                         ::testing::Combine(::testing::Values(std::make_shared<ParserRenameQuery>()),
                                            ::testing::ValuesIn(std::initializer_list<ParserTestCase>{
