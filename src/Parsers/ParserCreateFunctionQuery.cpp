...
bool ParserCreateFunctionQuery::parseImpl(IParser::Pos& pos, ASTPtr& node, Expected& expected) {
  ...
  if (s_or_replace.ignore(pos, expected)) or_replace = true;

  if (!s_function.ignore(pos, expected)) return false;

  if (!or_replace && s_if_not_exists.ignore(pos, expected)) if_not_exists = true;

  if (!function_name_p.parse(pos, function_name, expected)) return false;
  ...
  if (s_as.ignore(pos, expected)) {
    ...
    if (!lambda_p.parse(pos, function_expression_body, expected)) return false;
    ...
    ASTPtr lang_literal;
    if (!ParserIdentifier{}.parse(pos, lang_literal, expected)) return false;

    if (lang_literal->as<ASTIdentifier>()->name() != "WASM") return false;
    ...
    while (true) {
      if (s_abi && s_abi->ignore(pos, expected)) {
        ...
        ASTPtr abi_literal;
        if (!ParserIdentifier{}.parse(pos, abi_literal, expected)) return false;
        ...
        } else if (ParserNameTypePairList{}.parse(pos, arguments_ast, expected) || ParserTypeList{}.parse(pos, arguments_ast, expected)) {
        ...
        ASTPtr return_type;
        if (!ParserDataType{}.parse(pos, return_type, expected)) return false;
        create_function_query->setReturnType(return_type);
      } else if (s_from && s_from->ignore(pos, expected)) {
        s_from.reset();

        ASTPtr module_name_ast;
        if (!ParserStringLiteral{}.parse(pos, module_name_ast, expected)) return false;
        ...
        if (ParserToken(TokenType::DoubleColon).ignore(pos, expected)) {
          ...
          if (!ParserStringLiteral{}.parse(pos, source_function_name_ast, expected)) return false;
        ...
        ASTPtr hash_ast;
        if (!ParserStringLiteral{}.parse(pos, hash_ast, expected)) return false;
    ...
    if (s_arguments) return s_arguments->ignore(pos, expected);
    if (s_returns) return s_returns->ignore(pos, expected);
    if (s_from) return s_from->ignore(pos, expected);
