...
bool parseWorkloadSetting(ASTCreateWorkloadQuery::SettingChange &change, IParser::Pos &pos, Expected &expected) {
  ...
  if (!name_p.parse(pos, name_node, expected)) return false;
  tryGetIdentifierNameInto(name_node, name);

  if (!s_eq.ignore(pos, expected)) return false;

  if (!value_p.parse(pos, value_node, expected)) return false;

  if (ParserKeyword(Keyword::FOR).ignore(pos, expected)) {
    if (!resource_name_p.parse(pos, resource_name_node, expected)) return false;
...
bool parseSettings(IParser::Pos &pos, Expected &expected, ASTCreateWorkloadQuery::SettingsChanges &changes) {
  return IParserBase::wrapParseImpl(pos, [&] {
    ...
    auto parse_setting = [&] {
      ...
      if (!parseWorkloadSetting(change, pos, expected)) return false;
...
bool ParserCreateWorkloadQuery::parseImpl(IParser::Pos &pos, ASTPtr &node, Expected &expected) {
  ...
  if (!s_workload.ignore(pos, expected)) return false;

  if (!or_replace && s_if_not_exists.ignore(pos, expected)) if_not_exists = true;

  if (!workload_name_p.parse(pos, workload_name, expected)) return false;
  ...
  if (s_in.ignore(pos, expected)) {
    if (!workload_name_p.parse(pos, workload_parent, expected)) return false;
  }
