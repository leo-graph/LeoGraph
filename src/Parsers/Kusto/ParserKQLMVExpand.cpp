...
std::unordered_map<String, String> ParserKQLMVExpand::type_cast = {
    {"bool", "Boolean"}, {"boolean", "Boolean"}, {"datetime", "DateTime"}, {"date", "DateTime"},  {"guid", "UUID"},
    {"int", "Int32"},    {"long", "Int64"},      {"real", "Float64"},      {"double", "Float64"}, {"string", "String"}};
...
bool ParserKQLMVExpand::parseColumnArrayExprs(ColumnArrayExprs& column_array_exprs, Pos& pos, Expected& expected) {
  ...
  while (isValidKQLPos(pos) && pos->type != TokenType::PipeMark && pos->type != TokenType::Semicolon) {
    ...
    if (s_to.ignore(pos, expected)) {
      ...
      if (!open_bracket.ignore(pos, expected)) return false;
    ...
    if ((pos->type == TokenType::Comma && bracket_count == 0) || String(pos->begin, pos->end) == "limit" ||
      ...
      if (pos->type == TokenType::Semicolon) break;
    }

    if (String(pos->begin, pos->end) == "limit") break;
    if (isValidKQLPos(pos)) ++pos;
...
bool ParserKQLMVExpand::parserMVExpand(KQLMVExpand& kql_mv_expand, Pos& pos, Expected& expected) {
  ...
  if (!parseColumnArrayExprs(column_array_exprs, pos, expected)) return false;

  if (s_limit.ignore(pos, expected)) limit = String(pos->begin, pos->end);
...
bool ParserKQLMVExpand::genQuery(KQLMVExpand& kql_mv_expand, ASTPtr& select_node, uint32_t max_depth, uint32_t max_backtracks) {
  ...
  for (auto column : kql_mv_expand.column_array_exprs) {
    ...
    if (!column.to_type.empty()) {
      ...
      if (type_cast[column.to_type] == "Boolean")
        rename_str =
            fmt::format("accurateCastOrNull(toInt64OrNull(toString({0})),'{1}') as {0}_ali", column.alias, type_cast[column.to_type]);
      else
      ...
      cast_type_column_restore = cast_type_column_restore.empty() ? fmt::format(" Except {}_ali ", column.alias)
  ...
  if (!kql_mv_expand.limit.empty()) expand_str += " LIMIT " + kql_mv_expand.limit;
  ...
  if (cast_type_column_remove.empty()) {
    ...
    if (!parseSQLQueryByString(std::make_unique<ParserSelectQuery>(), query, sub_query_node, max_depth, max_backtracks)) return false;
    if (!setSubQuerySource(sub_query_node, select_node, false, false)) return false;
    ...
    if (!parseSQLQueryByString(std::make_unique<ParserTablesInSelectQuery>(), query, sub_query_node, max_depth, max_backtracks))
      return false;
    if (!setSubQuerySource(sub_query_node, select_node, true, false)) return false;
    select_node = std::move(sub_query_node);
    ...
    if (!parseSQLQueryByString(std::make_unique<ParserTablesInSelectQuery>(), rename_query, sub_query_node, max_depth, max_backtracks))
      return false;
    if (!setSubQuerySource(sub_query_node, select_node, true, true)) return false;
...
bool ParserKQLMVExpand::parseImpl(Pos& pos, ASTPtr& node, Expected& expected) {
  ...
  if (!parserMVExpand(kql_mv_expand, pos, expected)) return false;
  if (!genQuery(kql_mv_expand, node, pos.max_depth, pos.max_backtracks)) return false;
  ...
  if (!ParserSetQuery(true).parse(pos_settings, setting, expected)) return false;
