...
std::string formatKQLTimespan(const Int64 ticks) {
  ...
  if (abs_ticks >= TICKS_PER_DAY) result.append(fmt::format("{}.", abs_ticks / TICKS_PER_DAY));

  result.append(fmt::format("{:02}:{:02}:{:02}", (abs_ticks / TICKS_PER_HOUR) % 24, (abs_ticks / TICKS_PER_MINUTE) % 60,
                            (abs_ticks / TICKS_PER_SECOND) % 60));

  if (const auto fractional_second = abs_ticks % TICKS_PER_SECOND) result.append(fmt::format(".{:07}", fractional_second));
