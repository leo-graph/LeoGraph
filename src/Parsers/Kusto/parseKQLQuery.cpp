...
UnmatchedParentheses checkKQLUnmatchedParentheses(TokenIterator begin) {
  std::unordered_set<String> valid_kql_negative_suffix({"between", "contains", "contains_cs", "endswith", "endswith_cs", "~", "=", "has",
                                                        "has_cs", "hasprefix", "hasprefix_cs", "hassuffix", "hassuffix_cs", "in",
  ...
  for (TokenIterator it = begin; !it->isEnd(); ++it) {
    if (!it.isValid())  // allow kql negative operators
    {
      if (it->type == TokenType::ErrorSingleExclamationMark) {
        ++it;
        if (!valid_kql_negative_suffix.contains(String(it.get().begin, it.get().end))) break;
        --it;
      } else if (it->type == TokenType::ErrorWrongNumber) {
        if (!ParserKQLDateTypeTimespan().parseConstKQLTimespan(String(it.get().begin, it.get().end))) break;
      } else {
        if (String(it.get().begin, it.get().end) == "~") {
          --it;
          if (const auto prev = String(it.get().begin, it.get().end); prev != "!" && prev != "=" && prev != "in") break;
          ++it;
        } else
          break;
      }
...
ASTPtr tryParseKQLQuery(IParser& parser, const char*& _out_query_end, /* also query begin as input parameter */
                        const char* all_queries_end, std::string& out_error_message, int* out_error_code, bool hilite,
                        const std::string& query_description, bool allow_multi_statements, size_t max_query_size, size_t max_parser_depth,
                        size_t max_parser_backtracks, bool skip_insignificant) {
  if (out_error_code) *out_error_code = ErrorCodes::SYNTAX_ERROR;
  ...
  try {
    ...
    if (out_error_code) *out_error_code = e.code();
    _out_query_end = token_iterator->begin;
    return nullptr;
  ...
  if (!parse_res) {
    /// Generic parse error.
    out_error_message = getSyntaxErrorMessage(query_begin, all_queries_end, last_token, expected, hilite, query_description);
    return nullptr;
  }
...
ASTPtr parseKQLQueryAndMovePosition(IParser& parser, const char*& pos, const char* end, const std::string& query_description,
                                    bool allow_multi_statements, size_t max_query_size, size_t max_parser_depth,
                                    size_t max_parser_backtracks) {
  std::string error_message;
  int error_code = ErrorCodes::SYNTAX_ERROR;
  ASTPtr res = tryParseKQLQuery(parser, pos, end, error_message, &error_code, false, query_description, allow_multi_statements,
...
ASTPtr parseKQLQuery(IParser& parser, const char* begin, const char* end, const std::string& query_description, size_t max_query_size,
...
ASTPtr parseKQLQuery(IParser& parser, const std::string& query, const std::string& query_description, size_t max_query_size,
                     size_t max_parser_depth, size_t max_parser_backtracks) {
  return parseKQLQuery(parser, query.data(), query.data() + query.size(), query_description, max_query_size, max_parser_depth,
                       max_parser_backtracks);
}

ASTPtr parseKQLQuery(IParser& parser, const std::string& query, size_t max_query_size, size_t max_parser_depth,
                     size_t max_parser_backtracks) {
