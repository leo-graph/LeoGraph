...
bool ParserKQLSummarize::parseImpl(Pos& pos, ASTPtr& node, Expected& expected) {
  ...
  std::unordered_set<String> aggregate_functions({"arg_max",
                                                  "arg_min",
                                                  "avg",
                                                  "avgif",
                                                  "binary_all_and",
      ...
      if (String(equal_pos->begin, equal_pos->end) != "=") {
        ...
        if (!aggregate_functions.contains(aggregate_fun)) {
          ...
          if (agg_colum_pos->type == TokenType::Comma || agg_colum_pos->type == TokenType::ClosingRoundBracket) {
            --agg_colum_pos;
            if (agg_colum_pos->type != TokenType::ClosingRoundBracket) alias = alias + String(agg_colum_pos->begin, agg_colum_pos->end);
          }
        }
      ...
      if (String(equal_pos->begin, equal_pos->end) != "=") {
        ...
        if (!equal_pos.isValid() || equal_pos->type == TokenType::Comma || equal_pos->type == TokenType::Semicolon ||
          ...
          if (groupby_fun == "bin" || groupby_fun == "bin_at") {
            ...
            ++bin_colum_pos;
            if (bin_colum_pos->type != TokenType::Comma) alias.clear();
          }
          if (alias.empty()) {
            alias = fmt::format("Columns{}", new_column_index);
            ++new_column_index;
  ...
  while (isValidKQLPos(pos) && pos->type != TokenType::PipeMark && pos->type != TokenType::Semicolon) {
    ...
    if ((bracket_count == 0 and pos->type == TokenType::Comma) || String(pos->begin, pos->end) == "by") {
      ...
      if (String(pos->begin, pos->end) == "by") groupby = true;
  ...
  for (auto const& expr : expr_aggregations) expr_aggregation = expr_aggregation.empty() ? expr : expr_aggregation + "," + expr;

  for (auto const& expr : expr_groupbys) expr_groupby = expr_groupby.empty() ? expr : expr_groupby + "," + expr;

  if (!expr_groupby.empty()) expr_columns = expr_groupby;
  ...
  if (!ParserNotEmptyExpressionList(true).parse(pos_converted_columns, select_expression_list, expected)) return false;
  ...
  if (groupby) {
    ...
    if (!ParserNotEmptyExpressionList(false).parse(postoken_converted_groupby, group_expression_list, expected)) return false;
