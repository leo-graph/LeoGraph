...
bool ArrayConcat::convertImpl(String& out, IParser::Pos& pos) { return directMapping(out, pos, "arrayConcat"); }
...
bool ArrayLength::convertImpl(String& out, IParser::Pos& pos) { return directMapping(out, pos, "length"); }
...
bool ArrayShiftLeft::convertImpl(String& out, IParser::Pos& pos) {
  ...
  out = fmt::format(
      ...
      array, count, fill ? *fill : "null", generateUniqueIdentifier());
...
bool ArrayShiftRight::convertImpl(String& out, IParser::Pos& pos) {
  ...
  out = kqlCallToExpression("array_shift_left",
...
bool ArraySlice::convertImpl(String& out, IParser::Pos& pos) {
  ...
  out = fmt::format(
      ...
      array, start, end, generateUniqueIdentifier());

  return true;
}

bool ArraySortAsc::convertImpl(String& out, IParser::Pos& pos) { return directMapping(out, pos, "kql_array_sort_asc"); }

bool ArraySortDesc::convertImpl(String& out, IParser::Pos& pos) { return directMapping(out, pos, "kql_array_sort_desc"); }
...
bool ArraySplit::convertImpl(String& out, IParser::Pos& pos) {
  ...
  out = fmt::format(
      ...
      array, indices, generateUniqueIdentifier());

  return true;
}

bool ArraySum::convertImpl(String& out, IParser::Pos& pos) { return directMapping(out, pos, "arraySum"); }
...
bool JaccardIndex::convertImpl(String& out, IParser::Pos& pos) {
  ...
  out = fmt::format("divide(length({0}), length({1}))", kqlCallToExpression("set_intersect", {lhs, rhs}, pos.max_depth, pos.max_backtracks),
...
bool PackArray::convertImpl(String& out, IParser::Pos& pos) { return directMapping(out, pos, "array", {1, Interval::max_bound}); }
...
bool Repeat::convertImpl(String& out, IParser::Pos& pos) {
  ...
  if (count.empty()) throw Exception(ErrorCodes::SYNTAX_ERROR, "number of arguments do not match in function: {}", function_name);
...
bool SetDifference::convertImpl(String& out, IParser::Pos& pos) {
  ...
  const auto rhs = std::invoke([&function_name, &pos] {
    std::vector<String> arrays{getArgument(function_name, pos, ArgumentState::Raw)};
    while (auto next_array = getOptionalArgument(function_name, pos, ArgumentState::Raw)) arrays.push_back(*next_array);
...
bool SetHasElement::convertImpl(String& out, IParser::Pos& pos) { return directMapping(out, pos, "has"); }

bool SetIntersect::convertImpl(String& out, IParser::Pos& pos) { return directMapping(out, pos, "arrayIntersect"); }

bool SetUnion::convertImpl(String& out, IParser::Pos& pos) {
  if (!directMapping(out, pos, "arrayConcat")) return false;
...
bool Zip::convertImpl(String& out, IParser::Pos& pos) {
  ...
  const auto arguments = std::invoke([&function_name, &pos] {
    std::vector<String> result;
    while (auto argument = getOptionalArgument(function_name, pos)) result.push_back(std::move(*argument));

    return result;
  });

  if (const auto size = arguments.size(); size < 2 || size > 16)
    throw Exception(ErrorCodes::NUMBER_OF_ARGUMENTS_DOESNT_MATCH, "Between 2 and 16 arguments are expected, but {} were provided", size);

  const auto unique_identifier = generateUniqueIdentifier();
  const auto resized_arguments = std::invoke([&arguments, &unique_identifier] {
    String lengths;
    for (int i = 0; i < std::ssize(arguments); ++i) {
      lengths.append(i > 0 ? ", " : "");
      lengths.append(
          fmt::format("length(if(match(toTypeName({0}), 'Array\\(Nullable\\(.*\\)\\)'), {0}, "
                      "cast({0}, concat('Array(Nullable(', extract(toTypeName({0}), 'Array\\((.*)\\)'), '))'))) as arg{1}_{2})",
                      arguments[i], i, unique_identifier));
    }
