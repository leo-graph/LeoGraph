...
class IParserKQLFunction {
 public:
  enum class ArgumentState : uint8_t { Parsed, Raw };
  ...
  static std::vector<std::string> getArguments(const String& function_name, DB::IParser::Pos& pos,
  ...
  static std::optional<String> getOptionalArgument(const String& function_name, DB::IParser::Pos& pos,
                                                   ArgumentState argument_state = ArgumentState::Parsed);
  static String kqlCallToExpression(std::string_view function_name, std::initializer_list<const std::string_view> params,
 ...
 protected:
  ...
  static bool directMapping(String& out, IParser::Pos& pos, std::string_view ch_fn,
                            const Interval& argument_count_interval = {0, Interval::max_bound});
