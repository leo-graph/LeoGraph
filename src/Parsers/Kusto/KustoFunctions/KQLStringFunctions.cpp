...
bool Base64EncodeToString::convertImpl(String& out, IParser::Pos& pos) { return directMapping(out, pos, "base64Encode"); }
...
bool Base64EncodeFromGuid::convertImpl(String& out, IParser::Pos& pos) {
  ...
  out = fmt::format(
      ...
      argument, generateUniqueIdentifier());
  return true;
}

bool Base64DecodeToString::convertImpl(String& out, IParser::Pos& pos) { return directMapping(out, pos, "base64Decode"); }
...
bool Extract::convertImpl(String& out, IParser::Pos& pos) {
  ...
  std::unordered_map<String, String> type_cast = {
      {"bool", "Boolean"}, {"boolean", "Boolean"}, {"datetime", "DateTime"}, {"date", "DateTime"}, {"guid", "UUID"},      {"int", "Int32"},
      {"long", "Int64"},   {"real", "Float64"},    {"double", "Float64"},    {"string", "String"}, {"decimal", "Decimal"}};
  ...
  if (capture_group_str.empty()) return false;
  ...
  if (pos->type == TokenType::Comma) {
    ...
    if (s_kql.ignore(pos, expected)) {
      if (!open_bracket.ignore(pos, expected)) throw Exception(ErrorCodes::SYNTAX_ERROR, "Syntax error near typeof");
      ...
      if (!close_bracket.ignore(pos, expected)) throw Exception(ErrorCodes::SYNTAX_ERROR, "Syntax error near typeof");
    }
  }
  ...
  if (type_literal == "Decimal") {
    ...
    if (type_literal == "Boolean") out = fmt::format("toInt64OrNull({})", out);

    if (!type_literal.empty()) out = "accurateCastOrNull(" + out + ", '" + type_literal + "')";
  }
  return true;
}
...
bool ExtractJSON::convertImpl(String& out, IParser::Pos& pos) {
  ...
  std::unordered_map<String, String> type_cast = {{"bool", "Boolean"},   {"boolean", "Boolean"}, {"datetime", "DateTime"},
                                                  {"date", "DateTime"},  {"dynamic", "Array"},   {"guid", "UUID"},
                                                  {"int", "Int32"},      {"long", "Int64"},      {"real", "Float64"},
                                                  {"double", "Float64"}, {"string", "String"},   {"decimal", "Decimal"}};
  ...
  if (datatype == "Decimal") {
    ...
    if (datatype == "Boolean") out = fmt::format("toInt64OrNull({})", json_val);

    if (!datatype.empty()) out = fmt::format("accurateCastOrNull({},'{}')", json_val, datatype);
  }
  return true;
}
...
bool HasAnyIndex::convertImpl(String& out, IParser::Pos& pos) {
  ...
  out = fmt::format("if(empty({1}), -1, indexOf(arrayMap(x->(x in {0}), if(empty({1}),[''], arrayMap(x->(toString(x)),{1}))),1) - 1)",
                    src_array, lookup);
  return true;
}
...
bool IsEmpty::convertImpl(String& out, IParser::Pos& pos) { return directMapping(out, pos, "empty"); }

bool IsNotEmpty::convertImpl(String& out, IParser::Pos& pos) { return directMapping(out, pos, "notEmpty"); }

bool IsNotNull::convertImpl(String& out, IParser::Pos& pos) { return directMapping(out, pos, "isNotNull"); }
...
bool ParseCommandLine::convertImpl(String& out, IParser::Pos& pos) {
  ...
  if (type != "'windows'") throw Exception(ErrorCodes::BAD_ARGUMENTS, "Supported type argument is windows for {}", fn_name);

  out =
      fmt::format("if(empty({0}) OR hasAll(splitByChar(' ', {0}) , ['']) , arrayMap(x->null, splitByChar(' ', '')), splitByChar(' ', {0}))",
                  json_string);
  return true;
}

bool IsNull::convertImpl(String& out, IParser::Pos& pos) { return directMapping(out, pos, "isNull"); }
...
bool ParseURL::convertImpl(String& out, IParser::Pos& pos) {
  ...
  const String username =
      fmt::format(R"(concat('"Username":"', arrayElement(splitByChar(':',arrayElement(splitByChar('@',{0}) ,1)),1),'"'))", username_pwd);
  const String password =
      fmt::format(R"(concat('"Password":"', arrayElement(splitByChar(':',arrayElement(splitByChar('@',{0}) ,1)),2),'"'))", username_pwd);
  const String query_parameters =
      fmt::format(R"(concat('"Query Parameters":', concat('{{"', replace(replace({}, '=', '":"'),'&','","') ,'"}}')))", query_string);

  out = fmt::format("concat('{{',{},',',{},',',{},',',{},',',{},',',{},',',{},',',{},'}}')", scheme, host, port, path, username, password,
...
bool ReplaceRegex::convertImpl(String& out, IParser::Pos& pos) { return directMapping(out, pos, "replaceRegexpAll"); }
...
bool Split::convertImpl(String& out, IParser::Pos& pos) {
  ...
  if (pos->type == TokenType::Comma) {
    ...
    out = fmt::format(
        ...
        ",arrayElement(NULL,1)))",
        split_res, requested_index);
  } else
    out = split_res;
  return true;
}

bool StrCat::convertImpl(String& out, IParser::Pos& pos) { return directMapping(out, pos, "concat"); }
...
bool StrCatDelim::convertImpl(String& out, IParser::Pos& pos) {
  ...
  if (arg_count < 2 || arg_count > 64) throw Exception(ErrorCodes::SYNTAX_ERROR, "argument count out of bound in function: {}", fn_name);
...
bool StrLen::convertImpl(String& out, IParser::Pos& pos) { return directMapping(out, pos, "lengthUTF8"); }
...
bool SubString::convertImpl(String& out, IParser::Pos& pos) {
  ...
  if (pos->type == TokenType::Comma) {
    ...
    if (starting_index.empty()) throw Exception(ErrorCodes::SYNTAX_ERROR, "number of arguments do not match in function: {}", fn_name);
...
bool ToLower::convertImpl(String& out, IParser::Pos& pos) { return directMapping(out, pos, "lower"); }

bool ToUpper::convertImpl(String& out, IParser::Pos& pos) { return directMapping(out, pos, "upper"); }
...
bool URLDecode::convertImpl(String& out, IParser::Pos& pos) { return directMapping(out, pos, "decodeURLComponent"); }

bool URLEncode::convertImpl(String& out, IParser::Pos& pos) { return directMapping(out, pos, "encodeURLComponent"); }
