...
bool DayOfMonth::convertImpl(String& out, IParser::Pos& pos) { return directMapping(out, pos, "toDayOfMonth"); }
...
bool DayOfYear::convertImpl(String& out, IParser::Pos& pos) { return directMapping(out, pos, "toDayOfYear"); }
...
bool EndOfMonth::convertImpl(String& out, IParser::Pos& pos) {
  ...
  if (pos->type == TokenType::Comma) {
    ...
    if (offset.empty()) throw Exception(ErrorCodes::SYNTAX_ERROR, "Number of arguments do not match in function: {}", fn_name);
  }
  out = fmt::format(
...
bool EndOfDay::convertImpl(String& out, IParser::Pos& pos) {
  ...
  out =
      fmt::format("toDateTime(toStartOfDay({}),9,'UTC') + (INTERVAL {} +1 DAY) - (INTERVAL 1 microsecond)", datetime_str, toString(offset));
...
bool EndOfWeek::convertImpl(String& out, IParser::Pos& pos) {
  ...
  out = fmt::format("toDateTime(toStartOfDay({}),9,'UTC') + (INTERVAL {} +1 WEEK) - (INTERVAL 1 microsecond)", datetime_str,
                    toString(offset));
...
bool EndOfYear::convertImpl(String& out, IParser::Pos& pos) {
  ...
  if (datetime_str.empty()) throw Exception(ErrorCodes::SYNTAX_ERROR, "Number of arguments do not match in function: {}", fn_name);
...
bool FormatDateTime::convertImpl(String& out, IParser::Pos& pos) {
  ...
  if (decimal > 0 && formatspecifier.contains('.')) {
    out = fmt::format(
        ...
        datetime, formatspecifier, decimal);
  } else
  ...
  while (i < format.size()) {
    ...
    if (!isalpha(c)) {
      ...
      if (arg == "s" || arg == "ss")
        ...
        if (is_hour_zero)  // To handle the CH limit for 12hr format(01-12). If not handled, 1.00:00:00 returned as 1.12:00:00(in 12 hr
                           // format)
          formatspecifier = formatspecifier + "%h";
        else
          formatspecifier = formatspecifier + "%H";
    ...
    if (decimal > 0) {
      if (format.substr(format.length() - decimal - 1, 1) == last_delim)
        out = fmt::format(
            ...
            datetime, formatspecifier, decimal, last_delim);
      else
        out = fmt::format(
            "concat(substring(toString(formatDateTime(toDateTime64({0},9,'UTC') ,'{1}')),1, length(toString(formatDateTime("
            "toDateTime64({0},9,'UTC'),'{1}'))) - position("
    ...
    if (decimal > 0) {
      if (format.substr(format.length() - decimal - 1, 1) == last_delim)
        out = fmt::format(
            ...
            "))",
            datetime, formatspecifier, decimal, pad, last_delim, day_val);
      else
        out = fmt::format(
            "concat(leftPad('{5}', {3}, '0'),substring(toString(formatDateTime(toDateTime64({0},9,'UTC'),'{1}')),1,"
            "length(toString(formatDateTime(toDateTime64({0},9,'UTC'),'{1}'))) - position("
      ...
      out = fmt::format("concat(leftPad('{3}',{2},'0'),toString(formatDateTime(toDateTime64({0},9,'UTC'),'{1}')))", datetime,
                        formatspecifier, pad, day_val);
  }
  return true;
}

bool GetMonth::convertImpl(String& out, IParser::Pos& pos) { return directMapping(out, pos, "toMonth"); }

bool GetYear::convertImpl(String& out, IParser::Pos& pos) { return directMapping(out, pos, "toYear"); }

bool HoursOfDay::convertImpl(String& out, IParser::Pos& pos) { return directMapping(out, pos, "toHour"); }
...
bool MakeTimeSpan::convertImpl(String& out, IParser::Pos& pos) {
  ...
  while (isValidKQLPos(pos) && pos->type != TokenType::ClosingRoundBracket) {
    ...
    if (pos->type == TokenType::Comma) ++pos;
    ++arg_count;
  }

  if (arg_count < 2 || arg_count > 4) throw Exception(ErrorCodes::SYNTAX_ERROR, "argument count out of bound in function: {}", fn_name);
  ...
  out = fmt::format("CONCAT('{}',toString(SUBSTRING(toString(toTimeWithFixedDate(parseDateTime64BestEffortOrNull('{}', 9,'UTC'))),12)))",
                    day, datetime_str);
  return true;
}
...
bool MakeDateTime::convertImpl(String& out, IParser::Pos& pos) {
  ...
  if (arg_count < 1 || arg_count > 7) throw Exception(ErrorCodes::SYNTAX_ERROR, "argument count out of bound in function: {}", fn_name);

  if (arg_count < 7) {
    for (int i = arg_count; i < 7; ++i) arguments = arguments + "0,";
  }
...
bool StartOfMonth::convertImpl(String& out, IParser::Pos& pos) {
  ...
  out = fmt::format("date_add(MONTH,{}, parseDateTime64BestEffortOrNull(toString((toStartOfMonth({}))), 9, 'UTC')) ", offset, datetime_str);
  return true;
}
...
bool StartOfWeek::convertImpl(String& out, IParser::Pos& pos) {
  ...
  out = fmt::format("date_add(Week,{}, parseDateTime64BestEffortOrNull(toString((toStartOfWeek({}))), 9, 'UTC')) ", offset, datetime_str);
  return true;
}
...
bool StartOfYear::convertImpl(String& out, IParser::Pos& pos) {
  ...
  out = fmt::format("date_add(YEAR,{}, parseDateTime64BestEffortOrNull(toString((toStartOfYear({}, 'UTC'))), 9, 'UTC'))", offset,
                    datetime_str);
  return true;
}
...
bool MonthOfYear::convertImpl(String& out, IParser::Pos& pos) { return directMapping(out, pos, "toMonth"); }
