...
bool Bin::convertImpl(String &out, IParser::Pos &pos) {
  ...
  if (round_to.empty()) return false;

  String value_no_spaces = value;
  value_no_spaces.erase(std::remove_if(value_no_spaces.begin(), value_no_spaces.end(), [](unsigned char c) { return std::isspace(c); }),
                        value_no_spaces.end());
  if (value_no_spaces.empty()) return false;
  try {
    size_t pos_end = 0;
    std::stod(value_no_spaces, &pos_end);
    if (pos_end != value_no_spaces.size()) throw std::invalid_argument("not a number");
  } catch (const std::exception &) {
    ParserKQLDateTypeTimespan value_tsp;
    if (value_tsp.parseConstKQLTimespan(value_no_spaces)) value = std::to_string(value_tsp.toSeconds());
  }
  ...
  try {
    ...
    if (!time_span_parser.parseConstKQLTimespan(round_to)) return false;
...
bool BinAt::convertImpl(String &out, IParser::Pos &pos) {
  ...
  expression_no_spaces.erase(
      std::remove_if(expression_no_spaces.begin(), expression_no_spaces.end(), [](unsigned char c) { return std::isspace(c); }),
      expression_no_spaces.end());
  if (expression_no_spaces.empty()) return false;

  ++pos;
  String bin_size_str = getConvertedArgument(fn_name, pos);
  bin_size_str.erase(std::remove_if(bin_size_str.begin(), bin_size_str.end(), [](unsigned char c) { return std::isspace(c); }),
                     bin_size_str.end());
  if (bin_size_str.empty()) return false;

  ++pos;
  String fixed_point_str = getConvertedArgument(fn_name, pos);
  String fixed_point_no_spaces = fixed_point_str;
  fixed_point_no_spaces.erase(
      std::remove_if(fixed_point_no_spaces.begin(), fixed_point_no_spaces.end(), [](unsigned char c) { return std::isspace(c); }),
      fixed_point_no_spaces.end());
  if (fixed_point_no_spaces.empty()) return false;
  ...
  try {
    ...
    ParserKQLDateTypeTimespan time_span_parser;
    if (!time_span_parser.parseConstKQLTimespan(bin_size_str)) return false;
    bin_size = time_span_parser.toSeconds();
  }
...
bool Iif::convertImpl(String &out, IParser::Pos &pos) {
  ...
  if (if_true.empty()) return false;

  ++pos;
  String if_false = getConvertedArgument(fn_name, pos);
  if (if_false.empty()) return false;
