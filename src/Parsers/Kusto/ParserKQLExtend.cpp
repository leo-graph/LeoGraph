...
bool ParserKQLExtend ::parseImpl(Pos& pos, ASTPtr& node, Expected& expected) {
  ...
  while (isValidKQLPos(npos)) {
    if (npos->type == TokenType::OpeningRoundBracket) ++round_bracket_count;
    if (npos->type == TokenType::OpeningSquareBracket) ++square_bracket_count;
    if (npos->type == TokenType::ClosingRoundBracket) --round_bracket_count;
    if (npos->type == TokenType::ClosingSquareBracket) --square_bracket_count;
  ...
  if (!ParserSelectQuery().parse(new_pos, select_query, expected)) return false;
  if (!setSubQuerySource(select_query, node, false, false)) return false;
