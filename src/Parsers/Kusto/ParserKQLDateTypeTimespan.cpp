...
bool ParserKQLDateTypeTimespan ::parseImpl(Pos& pos, [[maybe_unused]] ASTPtr& node, Expected& expected) {
  ...
  if (!parseConstKQLTimespan(token)) return false;
...
bool ParserKQLDateTypeTimespan ::parseConstKQLTimespan(const String& text) {
  std::unordered_map<String, KQLTimespanUint> timespan_suffixes = {{"d", KQLTimespanUint::day},
  ...
  if (*(ptr + number_len) != ':') {
    ...
    if (!timespan_suffixes.contains(timespan_suffix)) return false;
    ...
    if (sec_len > 0) {
      ...
      if (*(ptr + number_len) == '.') {
        ...
        if (sec_scale_len > 0) {
          ...
          if (nanoseconds > 1000000000) return false;
        }
      }
    }
  }
