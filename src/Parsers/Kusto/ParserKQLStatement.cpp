...
bool ParserKQLWithUnionQuery::parseImpl(Pos &pos, ASTPtr &node, Expected &expected) {
  ...
  if (!ParserKQLQuery().parse(pos, kql_query, expected)) return false;
...
bool ParserKQLTableFunction::parseImpl(Pos &pos, ASTPtr &node, Expected &expected) {
  ...
  if (!lparen.ignore(pos, expected)) return false;
  ...
  if (!ParserKQLWithUnionQuery().parse(pos_kql, node, kql_expected)) return false;
