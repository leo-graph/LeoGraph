...
bool ParserKQLBase::parseSQLQueryByString(ParserPtr&& parser, String& query, ASTPtr& select_node, uint32_t max_depth,
  ...
  if (!parser->parse(pos_subquery, select_node, expected)) return false;
  return true;
};
...
String ParserKQLBase::getExprFromPipe(Pos& pos) {
  ...
  while (isValidKQLPos(end) && end->type != TokenType::Semicolon) {
    ...
    if (end->type == TokenType::PipeMark && bracket_count.isZero()) break;
...
String ParserKQLBase::getExprFromToken(Pos& pos) {
  ...
  while (isValidKQLPos(pos) && pos->type != TokenType::Semicolon) {
    if (pos->type == TokenType::PipeMark && paren_count == 0) break;

    if (pos->type == TokenType::OpeningRoundBracket) ++paren_count;
    if (pos->type == TokenType::ClosingRoundBracket) --paren_count;
  ...
  auto set_columns = [&](Pos& start_pos, Pos& end_pos) {
    ...
    if (String(it_pos->begin, it_pos->end) == "=") throw Exception(ErrorCodes::SYNTAX_ERROR, "Invalid equal symbol (=)");
    ...
    while (it_pos < end_pos) {
      ...
      if (String(it_pos->begin, it_pos->end) == "=") {
        ...
        if (String(it_pos->begin, it_pos->end) != "~" && bracket_count.isZero()) {
          if (has_alias) throw Exception(ErrorCodes::SYNTAX_ERROR, "Invalid equal symbol (=)");
          has_alias = true;
        }

        --it_pos;
        if (equal_pos == start_pos) equal_pos = it_pos;
      }
      ++it_pos;
    }
    ...
    for (const auto& token : tokens) column_str = column_str.empty() ? token : column_str + " " + token;
      ...
      if (start_pos == equal_pos) {
        ...
        while (start_pos < equal_pos) {
          if (start_pos->type == TokenType::Comma) {
            alias_inside.clear();
            if (comma_meet) throw Exception(ErrorCodes::SYNTAX_ERROR, "{} has invalid alias for {}", whole_alias, function_name);
            comma_meet = true;
          } else {
  ...
  String res;
  for (const auto& token : columns) res = res.empty() ? token : res + "," + token;
  return res;
}

std::unique_ptr<IParserBase> ParserKQLQuery::getOperator(String& op_name) {
  if (op_name == "filter" || op_name == "where") return std::make_unique<ParserKQLFilter>();
  if (op_name == "limit" || op_name == "take") return std::make_unique<ParserKQLLimit>();
  if (op_name == "project") return std::make_unique<ParserKQLProject>();
  if (op_name == "distinct") return std::make_unique<ParserKQLDistinct>();
  if (op_name == "extend") return std::make_unique<ParserKQLExtend>();
  if (op_name == "sort by" || op_name == "order by") return std::make_unique<ParserKQLSort>();
  if (op_name == "summarize") return std::make_unique<ParserKQLSummarize>();
  if (op_name == "table") return std::make_unique<ParserKQLTable>();
  if (op_name == "make-series") return std::make_unique<ParserKQLMakeSeries>();
  if (op_name == "mv-expand") return std::make_unique<ParserKQLMVExpand>();
  if (op_name == "print") return std::make_unique<ParserKQLPrint>();
  return nullptr;
}
...
bool ParserKQLQuery::parseImpl(Pos& pos, ASTPtr& node, Expected& expected) {
  ...
  std::unordered_map<std::string, KQLOperatorDataFlowState> kql_parser = {{"filter", {"filter", false, false, 3}},
  ...
  while (isValidKQLPos(pos) && pos->type != TokenType::Semicolon) {
    if (pos->type == TokenType::OpeningRoundBracket) ++bracket_count;
    if (pos->type == TokenType::ClosingRoundBracket) --bracket_count;

    if (pos->type == TokenType::PipeMark && bracket_count == 0) {
      ++pos;
      if (!isValidKQLPos(pos)) return false;
      ...
      auto validate_kql_operator = [&] {
        ...
        if (!kql_parser.contains(kql_operator)) return false;
        return true;
      };

      if (!validate_kql_operator()) return false;
      ++pos;
      if (!isValidKQLPos(pos)) return false;
  ...
  if (!npos.isValid()) return false;

  auto kql_operator_p = getOperator(kql_operator_str);

  if (!kql_operator_p) return false;

  if (operation_pos.size() == 1) {
    if (kql_operator_str == "print") {
      ++npos;
      if (!ParserKQLPrint().parse(npos, node, expected)) return false;
    } else if (kql_operator_str == "table") {
      if (!kql_operator_p->parse(npos, node, expected)) return false;
    }
  } else if (operation_pos.size() == 2 && operation_pos.front().first == "table") {
    npos = operation_pos.front().second;
    if (!ParserKQLTable().parse(npos, node, expected)) return false;
    ...
    if (!kql_parser[last_op].need_input) {
      while (!operation_pos.empty()) {
        ...
        if (kql_parser[prev_op].gen_output) break;
        if (!project_clause.empty() && prev_op == "project") break;
    ...
    if (!operation_pos.empty()) {
      for (auto i = 0; i < kql_parser[last_op].backspace_steps; ++i) --last_pos;
      ...
      if (!ParserKQLSubquery().parse(pos_subquery, tables, expected)) return false;
      select_query->setExpression(ASTSelectQuery::Expression::TABLES, std::move(tables));
    } else {
      if (!ParserKQLTable().parse(last_pos, node, expected)) return false;
    }
    ...
    auto set_query_clasue = [&](String op_str, String op_calsue) {
      ...
      if (oprator) {
        ...
        if (!oprator->parse(pos_clause, node, expected)) return false;
      }
      return true;
    };

    if (!node->as<ASTSelectQuery>()->select()) {
      if (project_clause.empty()) project_clause = "*";
      if (!set_query_clasue("project", project_clause)) return false;
    }

    if (!order_clause.empty())
      if (!set_query_clasue("order by", order_clause)) return false;

    if (!where_clause.empty())
      if (!set_query_clasue("where", where_clause)) return false;

    if (!limit_clause.empty())
      if (!set_query_clasue("limit", limit_clause)) return false;
  ...
  if (!node->as<ASTSelectQuery>()->select()) {
    ...
    if (!std::make_unique<ParserKQLProject>()->parse(new_pos, node, expected)) return false;
  }
...
bool ParserKQLSubquery::parseImpl(Pos& pos, ASTPtr& node, Expected& expected) {
  ...
  if (!ParserKQLTableFunction().parse(pos, select_node, expected)) return false;
...
bool ParserSimpleCHSubquery::parseImpl(Pos& pos, ASTPtr& node, Expected& expected) {
  ...
  if (!select.parse(pos, sub_select_node, expected)) return false;
  ...
  if (parent_select_node && parent_select_node->as<ASTSelectQuery>()->tables()) {
    ...
    select_query->as<ASTSelectQuery>()->setExpression(ASTSelectQuery::Expression::TABLES,
                                                      parent_select_node->as<ASTSelectQuery>()->tables());
  }
