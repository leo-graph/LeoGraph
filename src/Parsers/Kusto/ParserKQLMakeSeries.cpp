...
bool ParserKQLMakeSeries ::parseAggregationColumns(AggregationColumns& aggregation_columns, Pos& pos) {
  std::unordered_set<String> allowed_aggregation({"avg", "avgif", "count", "countif", "dcount", "dcountif", "max", "maxif", "min", "minif",
                                                  "percentile", "take_any", "stdev", "sum", "sumif", "variance"});
  ...
  while (isValidKQLPos(pos) && pos->type != TokenType::PipeMark && pos->type != TokenType::Semicolon) {
    ...
    if (!allowed_aggregation.contains(aggregation_fun)) return false;
...
bool ParserKQLMakeSeries ::parseFromToStepClause(FromToStepClause& from_to_step, Pos& pos) {
  ...
  while (isValidKQLPos(pos) && pos->type != TokenType::PipeMark && pos->type != TokenType::Semicolon) {
    if (String(pos->begin, pos->end) == "from") from_pos = pos;
    if (String(pos->begin, pos->end) == "to") to_pos = pos;
    if (String(pos->begin, pos->end) == "step") step_pos = pos;
  ...
  if (end_pos == begin) end_pos = pos;

  if (String(step_pos->begin, step_pos->end) != "step") return false;
...
bool ParserKQLMakeSeries ::makeSeries(KQLMakeSeries& kql_make_series, ASTPtr& select_node, uint32_t max_depth, uint32_t max_backtracks) {
  ...
  if (!kql_make_series.from_to_step.to_str.empty()) end_str = getExprFromToken(from_to_step.to_str, max_depth, max_backtracks);
  ...
  auto date_type_cast = [&](String& src) {
    ...
    while (isValidKQLPos(pos)) {
      ...
      if (tmp == "parseDateTime64BestEffortOrNull") tmp = "toDateTime64";
  ...
  auto get_group_expression_alias = [&] {
    ...
    while (isValidKQLPos(pos)) {
      if (String(pos->begin, pos->end) == "AS") {
        if (!group_expression_tokens.empty()) group_expression_tokens.pop_back();
        ++pos;
    ...
    String res;
    for (auto const& token : group_expression_tokens) res = res + token + " ";
    return res;
  };
  ...
  {
    bin_str = fmt::format("toFloat64({0}) + (toInt64((({1} - toFloat64({0})) / {2})) * {2}) AS {3}_ali", start_str, axis_column_format,
                          step, axis_column);
    start = fmt::format("toUInt64({})", start_str);
  } else {
    if (from_to_step.is_timespan) diff = era_diff;
    bin_str = fmt::format(" toFloat64(toInt64(({0} + {1}) / {2}) * {2}) AS {3}_ali ", axis_column_format, diff, step, axis_column);
  }

  if (!end_str.empty()) end = fmt::format("toUInt64({})", end_str);
  ...
  if (group_expression.empty())
    sub_sub_query = fmt::format(" (Select {0}, {1} FROM {2} {4} GROUP BY  {3}_ali ORDER BY {3}_ali) ", subquery_columns, bin_str,
                                "table_name", axis_column, condition);
  else
    sub_sub_query = fmt::format(" (Select {0}, {1}, {2} FROM {3} {5} GROUP BY {0}, {4}_ali ORDER BY {4}_ali) ", group_expression,
                                subquery_columns, bin_str, "table_name", axis_column, condition);

  ASTPtr sub_query_node;

  if (!ParserSimpleCHSubquery(select_node).parseByString(sub_sub_query, sub_query_node, max_depth, max_backtracks)) return false;
  select_node->as<ASTSelectQuery>()->setExpression(ASTSelectQuery::Expression::TABLES, std::move(sub_query_node));

  if (!group_expression.empty()) main_query = fmt::format("{} ", group_expression_alias);
  ...
  if (from_to_step.is_timespan)
    ...
    axis_str = fmt::format("arrayDistinct(arrayConcat(groupArray({0}_ali), arrayMap(x->(toFloat64(x)), {1}))) as {0}", axis_column, range);
  ...
  if (!ParserSimpleCHSubquery(select_node).parseByString(sub_query, sub_query_node, max_depth, max_backtracks)) return false;
  select_node->as<ASTSelectQuery>()->setExpression(ASTSelectQuery::Expression::TABLES, std::move(sub_query_node));
...
bool ParserKQLMakeSeries ::parseImpl(Pos& pos, ASTPtr& node, Expected& expected) {
  ...
  if (!parseAggregationColumns(aggregation_columns, pos)) return false;
  ...
  if (!parseFromToStepClause(from_to_step, pos)) return false;

  if (s_by.ignore(pos, expected)) {
    group_expression = getExprFromToken(pos);
    if (group_expression.empty()) return false;
  }
  ...
  Tokens token_main_query(kql_make_series.main_query.data(), kql_make_series.main_query.data() + kql_make_series.main_query.size(), 0,
                          true);
  IParser::Pos pos_main_query(token_main_query, pos.max_depth, pos.max_backtracks);

  if (!ParserNotEmptyExpressionList(true).parse(pos_main_query, select_expression_list, expected)) return false;
