...
bool ParserKillQueryQuery::parseImpl(Pos& pos, ASTPtr& node, Expected& expected) {
  ...
  if (!p_kill.ignore(pos, expected)) return false;
  ...
  if (p_on.ignore(pos, expected) && !ASTQueryWithOnCluster::parse(pos, cluster_str, expected)) return false;

  if (!p_where.ignore(pos, expected) || !p_where_expression.parse(pos, query->where_expression, expected)) return false;
