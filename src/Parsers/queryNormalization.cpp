...
UInt64 normalizedQueryHash(const char* begin, const char* end, bool keep_names) {
  ...
  while (true) {
    ...
    if (token.type == TokenType::Number || token.type == TokenType::StringLiteral || token.type == TokenType::HereDoc) {
      if (0 == num_literals_in_sequence) hash.update("\x00", 1);
    ...
    if (token.type == TokenType::Comma) {
      ...
      if (num_literals_in_sequence > 1) hash.update("\x00", 1);

      if (prev_comma) hash.update(",", 1);
      ...
      if (keep_names) {
        ...
        if (token.size() < 36) {
          for (; pos != token.end; ++pos) {
            if (isWhitespaceASCII(*pos)) break;

            if (isNumericASCII(*pos)) {
              ++num_digits;
              if (num_digits > 2) break;
            }
          }
        }
    ...
    if (token.isEnd() || token.isError()) break;
...
void normalizeQueryToPODArray(const char* begin, const char* end, PaddedPODArray<UInt8>& res_data, bool keep_names) {
  ...
  while (true) {
    ...
    /// Literals.
    if (token.type == TokenType::Number || token.type == TokenType::StringLiteral || token.type == TokenType::HereDoc) {
      if (0 == num_literals_in_sequence) res_data.push_back('?');
      ++num_literals_in_sequence;
    ...
    if (token.type == TokenType::Comma) {
      ...
      if (prev_comma) res_data.push_back(',');

      if (prev_whitespace) res_data.push_back(' ');
