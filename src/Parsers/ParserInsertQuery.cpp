...
bool ParserInsertQuery::parseImpl(Pos& pos, ASTPtr& node, Expected& expected) {
  ...
  if (!s_insert_into.ignore(pos, expected)) return false;
  ...
  if (s_function.ignore(pos, expected)) {
    /// Read function name
    if (!table_function_p.parse(pos, table_function, expected)) return false;

    /// Support insertion values with partition by.
    if (s_partition_by.ignore(pos, expected)) {
      if (!exp_elem_p.parse(pos, partition_by_expr, expected)) return false;
    }
  } else {
  ...
  if (s_from_infile.ignore(pos, expected)) {
    ...
    if (!infile_name_p.parse(pos, infile, expected)) return false;
    ...
    if (s_compression.ignore(pos, expected)) {
      ...
      if (!compression_p.parse(pos, compression, expected)) return false;
    }
  }
  ...
  if (s_settings.ignore(pos, expected)) {
    ...
    if (!parser_settings.parse(pos, settings_ast, expected)) return false;
  }
  ...
  if (!infile && s_values.ignore(pos, expected)) {
    ...
    if (pos->type != TokenType::Semicolon) data = pos->begin;

    format_str = "Values";
  } else if (s_format.ignore(pos, expected)) {
    /// If FORMAT is defined, read format name
    if (!name_p.parse(pos, format, expected)) return false;
      ...
      for (const auto& child : children) {
        ...
        if (child_select) {
          if (child_select->getExpression(ASTSelectQuery::Expression::WITH, false))
            throw Exception(ErrorCodes::SYNTAX_ERROR, "Only one WITH should be presented, either before INSERT or SELECT.");
          child_select->setExpression(ASTSelectQuery::Expression::WITH, ASTPtr(with_expression_list));
    ...
    if (s_format.ignore(pos, expected) && !name_p.parse(pos, format, expected)) return false;
  ...
  if (allow_settings_after_format_in_insert && s_settings.ignore(pos, expected)) {
    ...
    if (data != nullptr) data = pos->begin;
  }
  ...
  if (infile) {
    ...
    if (compression) query->children.push_back(compression);
  }
  ...
  if (table_function) {
    ...
    if (partition_by_expr) query->children.push_back(partition_by_expr);
  } else {
  ...
  if (columns) query->children.push_back(columns);
  if (select) query->children.push_back(select);
  if (settings_ast) query->children.push_back(settings_ast);

  return true;
}

bool ParserInsertElement::parseImpl(Pos& pos, ASTPtr& node, Expected& expected) {
  return ParserColumnsMatcher().parse(pos, node, expected) || ParserQualifiedAsterisk().parse(pos, node, expected) ||
         ParserAsterisk().parse(pos, node, expected) || ParserCompoundIdentifier().parse(pos, node, expected);
}
