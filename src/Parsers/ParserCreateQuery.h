...
bool IParserColumnDeclaration<NameParser>::parseImpl(Pos &pos, ASTPtr &node, Expected &expected) {
  ...
  ASTPtr name;
  if (!name_parser.parse(pos, name, expected)) return false;
  ...
  if (!require_type && (s_remove.checkWithoutMoving(pos, expected) || s_modify_setting.checkWithoutMoving(pos, expected) ||
                        s_reset_setting.checkWithoutMoving(pos, expected))) {
    if (!check_keywords_after_name) return false;
  ...
  auto null_check_without_moving = [&]() -> bool {
    if (!allow_null_modifiers) return false;

    if (s_null.checkWithoutMoving(pos, expected)) return true;
  ...
  if (!null_check_without_moving() && !s_default.checkWithoutMoving(pos, expected) && !s_materialized.checkWithoutMoving(pos, expected) &&
      !s_ephemeral.checkWithoutMoving(pos, expected) && !s_alias.checkWithoutMoving(pos, expected) &&
      !s_auto_increment.checkWithoutMoving(pos, expected) && !s_primary_key.checkWithoutMoving(pos, expected) &&
      (require_type || (!s_comment.checkWithoutMoving(pos, expected) && !s_codec.checkWithoutMoving(pos, expected)))) {
    if (check_type_keyword && !s_type.ignore(pos, expected)) return false;
    if (!type_parser.parse(pos, type, expected)) return false;
    if (s_collate.ignore(pos, expected) && !collation_parser.parse(pos, collation_expression, expected)) return false;
  }

  if (allow_null_modifiers) {
    if (s_not.check(pos, expected)) {
      if (!s_null.check(pos, expected)) return false;
  ...
  if (!collation_expression && s_collate.ignore(pos, expected) && !collation_parser.parse(pos, collation_expression, expected))
    return false;
  ...
  if (s_default.ignore(pos, expected)) {
    ...
    if (!expr_parser.parse(pos, default_expression, expected)) return false;
    ...
    if (s_comment.ignore(pos, expected)) is_comment = true;
    ...
    if (!default_expression && !type) return false;
  ...
  if (default_expression && !dynamic_cast<const ASTWithAlias *>(default_expression.get())) return false;

  if (require_type && !type && !default_expression) return false;  /// reject column name without type

  if (!is_comment) {
    if ((type || default_expression) && allow_null_modifiers && !null_modifier.has_value()) {
      if (s_not.ignore(pos, expected)) {
        if (!s_null.ignore(pos, expected)) return false;
        null_modifier.emplace(false);
  ...
  if (is_comment || s_comment.ignore(pos, expected)) {
    ...
    if (!string_literal_parser.parse(pos, comment_expression, expected)) return false;
  }

  if (s_codec.ignore(pos, expected)) {
    if (!codec_parser.parse(pos, codec_expression, expected)) return false;
  }

  if (s_stat.ignore(pos, expected)) {
    if (!stat_type_parser.parse(pos, statistics_desc_expression, expected)) return false;
  }

  if (s_ttl.ignore(pos, expected)) {
    if (!expression_parser.parse(pos, ttl_expression, expected)) return false;
  }
  ...
  if (s_settings.ignore(pos, expected)) {
    ...
    if (parser_opening_bracket.ignore(pos, expected)) {
      if (!settings_parser.parse(pos, settings, expected)) return false;
      ParserToken parser_closing_bracket(TokenType::ClosingRoundBracket);
      if (!parser_closing_bracket.ignore(pos, expected)) return false;
    } else
      pos = old_pos;
  }
  ...
  if (type) column_declaration->setType(std::move(type));
  ...
  if (comment_expression) column_declaration->setComment(std::move(comment_expression));

  if (codec_expression) column_declaration->setCodec(std::move(codec_expression));

  if (settings) column_declaration->setSettings(std::move(settings));

  if (statistics_desc_expression) column_declaration->setStatisticsDesc(std::move(statistics_desc_expression));

  if (ttl_expression) column_declaration->setTTL(std::move(ttl_expression));

  if (collation_expression) column_declaration->setCollation(std::move(collation_expression));
...
class ParserColumnDeclarationList : public IParserBase {
 public:
  explicit ParserColumnDeclarationList(bool require_type_ = true, bool allow_null_modifiers_ = true,
...
/// CREATE|ATTACH WINDOW VIEW [IF NOT EXISTS] [db.]name [TO [db.]name] [INNER ENGINE engine] [ENGINE engine] [WATERMARK strategy]
/// [ALLOWED_LATENESS interval_function] [POPULATE] AS SELECT ...
...
/// CREATE[OR REPLACE]|ATTACH [[MATERIALIZED] VIEW] | [VIEW]] [IF NOT EXISTS] [db.]name [UUID 'uuid'] [TO [db.]name] [ENGINE = engine]
/// [POPULATE] AS SELECT ...
...
/// CREATE DICTIONARY [IF NOT EXISTS] [db.]name (attrs) PRIMARY KEY key SOURCE(s(params)) LAYOUT(l(params)) LIFETIME([min v1 max] v2)
/// [RANGE(min v1 max v2)]
