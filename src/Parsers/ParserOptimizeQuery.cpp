...
bool ParserOptimizeQueryColumnsSpecification::parseImpl(Pos& pos, ASTPtr& node, Expected& expected) {
  ...
  return ParserColumnsMatcher(allowed_transformers).parse(pos, node, expected) ||
         ParserAsterisk(allowed_transformers).parse(pos, node, expected) || ParserIdentifier(false).parse(pos, node, expected);
}
...
bool ParserOptimizeQuery::parseImpl(Pos& pos, ASTPtr& node, Expected& expected) {
  ...
  if (!s_optimize_table.ignore(pos, expected)) return false;
  ...
  if (ParserKeyword{Keyword::ON}.ignore(pos, expected) && !ASTQueryWithOnCluster::parse(pos, cluster_str, expected)) return false;

  if (s_partition.ignore(pos, expected)) {
    if (!partition_p.parse(pos, partition, expected)) return false;
  }

  if (s_dry_run.ignore(pos, expected)) {
    dry_run = true;
    if (!s_parts.ignore(pos, expected)) return false;

    ParserList parser_list(std::make_unique<ParserStringLiteral>(), std::make_unique<ParserToken>(TokenType::Comma), false);
    if (!parser_list.parse(pos, parts_list, expected)) return false;
  }

  if (s_final.ignore(pos, expected) || s_force.ignore(pos, expected)) final = true;

  if (s_deduplicate.ignore(pos, expected)) deduplicate = true;

  if (s_cleanup.ignore(pos, expected)) cleanup = true;
  ...
  if ((query->partition = partition)) query->children.push_back(partition);
  if ((query->parts_list = parts_list)) query->children.push_back(parts_list);
