...
class ASTSelectQuery : public IAST {
 public:
  ...
  ASTPtr getExpression(Expression expr, bool clone = false) const {
    ...
    if (it != positions.end()) return clone ? children[it->second]->clone() : children[it->second];
    return {};
  }
  ...
  bool isLimitByAll() const { return limit_by_all; }

  /// Set query node LIMIT BY ALL modifier value
  void setIsLimitByAll(bool is_limit_by_all_value) { limit_by_all = is_limit_by_all_value; }
