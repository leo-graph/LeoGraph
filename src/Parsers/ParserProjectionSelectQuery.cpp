...
bool ParserProjectionSelectQuery::parseImpl(Pos& pos, ASTPtr& node, Expected& expected) {
  ...
  /// WITH expr list
  {
    if (s_with.ignore(pos, expected)) {
      if (!exp_list_for_with_clause.parse(pos, with_expression_list, expected)) return false;
    }
  }

  /// SELECT [DISTINCT] [TOP N [WITH TIES]] expr list
  {
    if (!s_select.ignore(pos, expected)) return false;
  ...
  if (s_order_by.ignore(pos, expected)) {
    ASTPtr expr_list;
    if (!order_list_p.parse(pos, expr_list, expected)) return false;
