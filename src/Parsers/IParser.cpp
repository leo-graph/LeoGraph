...
IParser::Pos& IParser::Pos::operator=(const IParser::Pos& rhs) {
  ...
  if (rhs < *this) {
    ...
    if (max_backtracks && backtracks > max_backtracks)
      throw Exception(ErrorCodes::TOO_SLOW_PARSING,
                      "Maximum amount of backtracking ({}) exceeded in the parser. "
                      "Consider rising max_parser_backtracks parameter.",
                      max_backtracks);
  }
...
static bool intersects(T a_begin, T a_end, T b_begin, T b_end) {
  return (a_begin <= b_begin && b_begin < a_end) || (b_begin <= a_begin && a_begin < b_end);
}
...
void Expected::highlight(HighlightedRange range) {
  ...
  if (it != highlights.begin()) {
    ...
    if (range.begin < prev_it->end) it = prev_it;
  }
