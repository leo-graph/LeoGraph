...
class DumpASTNode {
 public:
  DumpASTNode(const IAST& ast_, WriteBuffer* ostr_, size_t& depth, const char* label_ = nullptr)
      : ast(ast_), ostr(ostr_), indent(depth), visit_depth(depth), label(label_) {
    if (!ostr) return;
    if (label && visit_depth == 0) (*ostr) << "-- " << label << '\n';
    ++visit_depth;
 ...
 private:
  ...
  void printNode() const {
    ...
    if (!alias.empty()) print("alias", alias, " ");

    if (!ast.children.empty()) print("children", ast.children.size(), " ");
  }
};
...
inline void dumpAST(const IAST& ast, WriteBuffer& ostr, DumpASTNode* parent = nullptr) {
  ...
  for (const auto& child : ast.children) dumpAST(*child, ostr, &dump);
}
...
class DumpASTNodeInDotFormat {
 public:
  DumpASTNodeInDotFormat(const IAST& ast_, WriteBuffer* ostr_, bool root_ = true, const char* label_ = nullptr)
    ...
    if (root) (*ostr) << "digraph " << (label ? String(label) : "") << "{\n    rankdir=\"UD\";\n";
  ...
  ~DumpASTNodeInDotFormat() {
    ...
    for (const auto& child : ast.children) printEdge(ast, *child);
...
inline void dumpASTInDotFormat(const IAST& ast, WriteBuffer& ostr, bool root = true) {
  ...
  for (const auto& child : ast.children) dumpASTInDotFormat(*child, ostr, false);
}
...
class DebugASTLog {
 public:
  DebugASTLog() : log(nullptr) {
    if constexpr (_enable) log = getLogger("AST");
  }

  ~DebugASTLog() {
    if constexpr (_enable) LOG_DEBUG(log, fmt::runtime(buf.str()));
  }
