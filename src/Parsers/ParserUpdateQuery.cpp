...
bool ParserUpdateQuery::parseImpl(Pos& pos, ASTPtr& node, Expected& expected) {
  ...
  if (!s_update.ignore(pos, expected)) return false;
  ...
  if (!parser_assignment_list.parse(pos, query->assignments, expected)) return false;
  ...
  auto add_to_children = [&](const auto& ast) {
    if (ast) query->children.push_back(ast);
  };
