...
String ASTMoveAccessEntityQuery::getID(char) const { return String("MOVE ") + toString(type) + " query"; }
...
void ASTMoveAccessEntityQuery::formatImpl(WriteBuffer &ostr, const FormatSettings &settings, FormatState &, FormatStateStacked) const {
  ostr << "MOVE " << AccessEntityTypeInfo::get(type).name;
  ...
  ostr << " TO " << backQuoteIfNeed(storage_name);
