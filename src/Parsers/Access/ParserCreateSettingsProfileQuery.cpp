...
bool ParserCreateSettingsProfileQuery::parseImpl(Pos& pos, ASTPtr& node, Expected& expected) {
  bool alter = false;
  if (attach_mode) {
    if (!ParserKeyword{Keyword::ATTACH_SETTINGS_PROFILE}.ignore(pos, expected) &&
        !ParserKeyword{Keyword::ATTACH_PROFILE}.ignore(pos, expected))
      return false;
  } else {
    if (ParserKeyword{Keyword::ALTER_SETTINGS_PROFILE}.ignore(pos, expected) || ParserKeyword{Keyword::ALTER_PROFILE}.ignore(pos, expected))
      alter = true;
    else if (!ParserKeyword{Keyword::CREATE_SETTINGS_PROFILE}.ignore(pos, expected) &&
             !ParserKeyword{Keyword::CREATE_PROFILE}.ignore(pos, expected))
      return false;
  }
