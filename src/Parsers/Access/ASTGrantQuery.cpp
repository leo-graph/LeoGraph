...
String ASTGrantQuery::getID(char) const { return "GrantQuery"; }
...
void ASTGrantQuery::formatImpl(WriteBuffer &ostr, const FormatSettings &settings, FormatState &, FormatStateStacked) const {
  ostr << (attach_mode ? "ATTACH " : "") << (is_revoke ? "REVOKE" : "GRANT");
  ...
  if (!is_revoke) {
    ...
    if (replace_access || replace_granted_roles) ostr << " WITH REPLACE OPTION";
  }
}

void ASTGrantQuery::replaceEmptyDatabase(const String &current_database) { access_rights_elements.replaceEmptyDatabase(current_database); }

void ASTGrantQuery::replaceCurrentUserTag(const String &current_user_name) const {
  if (grantees) grantees->replaceCurrentUserTag(current_user_name);
}
