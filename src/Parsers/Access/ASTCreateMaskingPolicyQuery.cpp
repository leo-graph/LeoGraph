...
void formatPriority(Int64 priority, WriteBuffer &ostr, const IAST::FormatSettings &) { ostr << " PRIORITY " << priority; }
}  // namespace

String ASTCreateMaskingPolicyQuery::getID(char) const { return "CREATE MASKING POLICY or ALTER MASKING POLICY query"; }
...
ASTPtr ASTCreateMaskingPolicyQuery::clone() const {
  ...
  if (update_assignments) res->update_assignments = update_assignments->clone();

  if (where_condition) res->where_condition = where_condition->clone();
...
void ASTCreateMaskingPolicyQuery::formatImpl(WriteBuffer &ostr, const FormatSettings &settings, FormatState &, FormatStateStacked) const {
  ...
  ostr << " ON ";
  if (!database.empty()) ostr << backQuoteIfNeed(database) << ".";
  ostr << backQuoteIfNeed(table_name);

  if (!storage_name.empty()) ostr << " IN " << backQuoteIfNeed(storage_name);

  if (!new_name.empty()) formatRenameTo(new_name, ostr, settings);

  if (update_assignments) formatUpdateExpression(update_assignments, ostr, settings);

  if (where_condition) formatWhereCondition(where_condition, ostr, settings);

  if (roles && (!roles->empty() || alter)) formatToRoles(*roles, ostr, settings);

  if (priority != 0) formatPriority(priority, ostr, settings);
}
...
void ASTCreateMaskingPolicyQuery::replaceEmptyDatabase(const String &current_database) const {
  if (database.empty()) const_cast<ASTCreateMaskingPolicyQuery *>(this)->database = current_database;
}
}  // namespace DB
