...
String ASTDropAccessEntityQuery::getID(char) const { return String("DROP ") + toString(type) + " query"; }

ASTPtr ASTDropAccessEntityQuery::clone() const {
  auto res = make_intrusive<ASTDropAccessEntityQuery>(*this);

  if (row_policy_names) res->row_policy_names = boost::static_pointer_cast<ASTRowPolicyNames>(row_policy_names->clone());

  if (masking_policy_name) res->masking_policy_name = std::make_shared<MaskingPolicyName>(*masking_policy_name);
...
void ASTDropAccessEntityQuery::formatImpl(WriteBuffer &ostr, const FormatSettings &settings, FormatState &, FormatStateStacked) const {
  ostr << "DROP " << AccessEntityTypeInfo::get(type).name << (if_exists ? " IF EXISTS" : "");
  ...
  if (!storage_name.empty()) ostr << " FROM " << backQuoteIfNeed(storage_name);

  formatOnCluster(ostr, settings);
}

void ASTDropAccessEntityQuery::replaceEmptyDatabase(const String &current_database) const {
  if (row_policy_names) row_policy_names->replaceEmptyDatabase(current_database);

  if (masking_policy_name && masking_policy_name->database.empty()) masking_policy_name->database = current_database;
}
}  // namespace DB
