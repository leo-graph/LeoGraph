...
std::optional<String> ASTAuthenticationData::getSalt() const {
  if (type && (*type == AuthenticationType::SHA256_PASSWORD || *type == AuthenticationType::SCRAM_SHA256_PASSWORD) &&
      children.size() == 2) {
...
void ASTAuthenticationData::formatImpl(WriteBuffer &ostr, const FormatSettings &settings, FormatState &, FormatStateStacked) const {
  ...
  if (type) {
    ...
    switch (*type) {
      ...
      case AuthenticationType::SHA256_PASSWORD: {
        if (contains_hash) auth_type_name = "sha256_hash";

        prefix = "BY";
        password = true;
        if (children.size() == 2) salt = true;
        break;
      }
      case AuthenticationType::SCRAM_SHA256_PASSWORD: {
        if (contains_hash) auth_type_name = "scram_sha256_hash";
      ...
      case AuthenticationType::DOUBLE_SHA1_PASSWORD: {
        if (contains_hash) auth_type_name = "double_sha1_hash";
      ...
      case AuthenticationType::BCRYPT_PASSWORD: {
        if (contains_hash) auth_type_name = "bcrypt_hash";
      ...
      case AuthenticationType::HTTP: {
        prefix = "SERVER";
        parameter = true;
        if (children.size() == 2) scheme = true;
        break;
      }
  ...
  if (password && !settings.show_secrets) {
    ...
    password = false;
    salt = false;
    if (type) auth_type_name = AuthenticationTypeInfo::get(*type).name;
  }
...
bool ASTAuthenticationData::hasSecretParts() const {
  ...
  auto auth_type = *type;
  if ((auth_type == AuthenticationType::PLAINTEXT_PASSWORD) || (auth_type == AuthenticationType::SHA256_PASSWORD) ||
      (auth_type == AuthenticationType::SCRAM_SHA256_PASSWORD) || (auth_type == AuthenticationType::DOUBLE_SHA1_PASSWORD) ||
