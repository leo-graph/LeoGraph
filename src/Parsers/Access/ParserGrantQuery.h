 ...
 * REVOKE access_type[(column_name [,...])] [,...] ON {db.table|db.*|*.*|table|*} FROM {user_name | CURRENT_USER} [,...] | ALL | ALL EXCEPT
 * {user_name | CURRENT_USER} [,...]
 */
class ParserGrantQuery : public IParserBase {
 public:
  ParserGrantQuery& useAttachMode(bool attach_mode_ = true) {
    attach_mode = attach_mode_;
    return *this;
  }

  ParserGrantQuery& setParseWithoutGrantees(bool allow_no_grantees_ = true) {
    allow_no_grantees = allow_no_grantees_;
    return *this;
  }
