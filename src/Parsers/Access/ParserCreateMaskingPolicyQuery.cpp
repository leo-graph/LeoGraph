...
bool parseUpdateAssignments(IParserBase::Pos& pos, Expected& expected, ASTPtr& assignments) {
  return IParserBase::wrapParseImpl(pos, [&] {
    if (!ParserKeyword{Keyword::UPDATE}.ignore(pos, expected)) return false;

    ParserList parser_assignment_list(std::make_unique<ParserAssignment>(), std::make_unique<ParserToken>(TokenType::Comma),
                                      /* allow_empty = */ false);

    if (!parser_assignment_list.parse(pos, assignments, expected)) return false;
...
bool parseWhereCondition(IParserBase::Pos& pos, Expected& expected, ASTPtr& condition) {
  return IParserBase::wrapParseImpl(pos, [&] {
    if (!ParserKeyword{Keyword::WHERE}.ignore(pos, expected)) return false;

    ParserExpression parser;
    if (!parser.parse(pos, condition, expected)) return false;
...
bool parsePriority(IParserBase::Pos& pos, Expected& expected, Int64& priority) {
  return IParserBase::wrapParseImpl(pos, [&] {
    if (!ParserKeyword{Keyword::PRIORITY}.ignore(pos, expected)) return false;

    ASTPtr ast;
    ParserNumber parser;
    if (!parser.parse(pos, ast, expected)) return false;
...
bool ParserCreateMaskingPolicy::parseImpl(Pos& pos, ASTPtr& node, Expected& expected) {
  ...
  String name;
  if (!parseIdentifierOrStringLiteral(pos, expected, name)) return false;

  if (!ParserKeyword{Keyword::ON}.ignore(pos, expected)) return false;

  String database;
  String table_name;
  if (!parseDatabaseAndTableName(pos, expected, database, table_name)) return false;
  ...
  while (true) {
    if (alter && new_name.empty() && parseRenameTo(pos, expected, new_name)) continue;

    if (!update_assignments && parseUpdateAssignments(pos, expected, update_assignments)) continue;

    if (!where_condition && parseWhereCondition(pos, expected, where_condition)) continue;

    if (!roles && parseToRoles(pos, expected, attach_mode, roles)) continue;
  ...
  if (!update_assignments && !alter) return false;
