...
String ASTShowCreateAccessEntityQuery::getID(char) const { return String("SHOW CREATE ") + getKeyword() + " query"; }
...
void ASTShowCreateAccessEntityQuery::formatQueryImpl(WriteBuffer &ostr, const FormatSettings &settings, FormatState &,
                                                     FormatStateStacked) const {
  ostr << "SHOW CREATE " << getKeyword();

  if (!names.empty()) formatNames(names, ostr);
  ...
  if (!short_name.empty()) ostr << " " << backQuoteIfNeed(short_name);
...
void ASTShowCreateAccessEntityQuery::replaceEmptyDatabase(const String &current_database) {
  ...
  if (database_and_table_name) {
    ...
    if (database.empty()) database = current_database;
  }
}
