...
bool parseParameterRegExp(IParser::Pos &pos, Expected &expected, String &parameter_regexp) {
  return IParserBase::wrapParseImpl(pos, [&] {
    if (!ParserToken{TokenType::OpeningRoundBracket}.ignore(pos, expected)) return true;

    if (!parseIdentifierOrStringLiteral(pos, expected, parameter_regexp)) return false;
...
bool parseColumnNames(IParser::Pos &pos, Expected &expected, Strings &columns) {
  return IParserBase::wrapParseImpl(pos, [&] {
    ...
    Strings res_columns;
    for (const auto &child : ast->children) res_columns.emplace_back(getIdentifierName(child));
...
bool parseAccessFlags(IParser::Pos &pos, Expected &expected, AccessFlags &access_flags) {
  static constexpr auto is_one_of_access_type_words = [](IParser::Pos &pos_) {
    if (pos_->type != TokenType::BareWord) return false;
    std::string_view word{pos_->begin, pos_->size()};
    return !(boost::iequals(word, toStringView(Keyword::ON)) || boost::iequals(word, toStringView(Keyword::TO)) ||
             boost::iequals(word, toStringView(Keyword::FROM)));
  };

  expected.add(pos, "access type");

  return IParserBase::wrapParseImpl(pos, [&] {
    if (!is_one_of_access_type_words(pos)) return false;
...
bool parseAccessFlagsWithColumns(IParser::Pos &pos, Expected &expected, std::vector<std::pair<AccessFlags, Strings>> &access_and_columns) {
  std::vector<std::pair<AccessFlags, Strings>> res;

  auto parse_access_and_columns = [&] {
    AccessFlags access_flags;
    if (!parseAccessFlags(pos, expected, access_flags)) return false;
  ...
  if (!ParserList::parseUtil(pos, expected, parse_access_and_columns, false)) return false;
...
bool parseAccessRightsElementsWithoutOptions(IParser::Pos &pos, Expected &expected, AccessRightsElements &elements) {
  return IParserBase::wrapParseImpl(pos, [&] {
    ...
    auto parse_around_on = [&] {
      ...
      if (!parseAccessFlagsWithColumns(pos, expected, access_and_columns)) return false;
      ...
      for (const auto &elem : access_and_columns) {
        if (elem.first.isGlobalWithParameter()) ++is_global_with_parameter;
      }

      if (!ParserKeyword{Keyword::ON}.ignore(pos, expected)) return false;
      ...
      if (is_global_with_parameter && is_global_with_parameter == access_and_columns.size()) {
        ...
        for (const auto &elem : access_and_columns) {
          if (!elem.first.validateParameter(parameter, add_to_expected)) return false;
        }

        if (ParserToken{TokenType::Asterisk}.ignore(pos, expected)) wildcard = true;

        /// GRANT READ ON S3('s3://foo/*')
        if (!parseParameterRegExp(pos, expected, filter)) return false;

        /// GRANT READ ON *(foo) is prohibited.
        if ((wildcard || parameter.empty()) && !filter.empty()) return false;
      ...
      for (auto &[access_flags, columns] : access_and_columns) {
        if ((wildcard || table_name.empty()) && !columns.empty()) return false;
    ...
    if (!ParserList::parseUtil(pos, expected, parse_around_on, false)) return false;
