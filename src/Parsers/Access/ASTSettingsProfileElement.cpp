...
void formatSettingsProfileElementsForAlter(std::string_view kind, const ASTSettingsProfileElements &elements, WriteBuffer &ostr,
...
void ASTSettingsProfileElement::formatImpl(WriteBuffer &ostr, const FormatSettings &settings, FormatState &, FormatStateStacked) const {
  ...
  if (min_value) {
    ostr << " MIN " << applyVisitor(FieldVisitorToString{}, *min_value);
  }

  if (max_value) {
    ostr << " MAX " << applyVisitor(FieldVisitorToString{}, *max_value);
  }
  ...
  if (writability) {
    switch (*writability) {
      ...
      case SettingConstraintWritability::CHANGEABLE_IN_READONLY:
        ...
        break;
      case SettingConstraintWritability::MAX:
        break;
    }
  }
}
...
ASTPtr ASTSettingsProfileElements::clone() const {
  ...
  for (auto &element : res->elements) element = boost::static_pointer_cast<ASTSettingsProfileElement>(element->clone());
...
void ASTSettingsProfileElements::setUseInheritKeyword(bool use_inherit_keyword_) {
  for (auto &element : elements) element->use_inherit_keyword = use_inherit_keyword_;
}

void ASTSettingsProfileElements::add(ASTSettingsProfileElements &&other) { insertAtEnd(elements, std::move(other.elements)); }

String ASTAlterSettingsProfileElements::getID(char) const { return "AlterSettingsProfileElements"; }

ASTPtr ASTAlterSettingsProfileElements::clone() const {
  auto res = make_intrusive<ASTAlterSettingsProfileElements>(*this);

  if (add_settings) res->add_settings = boost::static_pointer_cast<ASTSettingsProfileElements>(add_settings->clone());

  if (modify_settings) res->modify_settings = boost::static_pointer_cast<ASTSettingsProfileElements>(modify_settings->clone());

  if (drop_settings) res->drop_settings = boost::static_pointer_cast<ASTSettingsProfileElements>(drop_settings->clone());
...
void ASTAlterSettingsProfileElements::add(ASTAlterSettingsProfileElements &&other) {
  ...
  if (other.add_settings) {
    if (!add_settings) add_settings = make_intrusive<ASTSettingsProfileElements>();
  ...
  if (other.modify_settings) {
    if (!modify_settings) modify_settings = make_intrusive<ASTSettingsProfileElements>();
    modify_settings->add(std::move(*other.modify_settings));
  }

  if (other.drop_settings) {
    if (!drop_settings) drop_settings = make_intrusive<ASTSettingsProfileElements>();
