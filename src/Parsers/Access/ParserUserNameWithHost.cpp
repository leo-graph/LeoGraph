...
namespace {
bool parseUserNameWithHost(IParserBase::Pos &pos, Expected &expected, boost::intrusive_ptr<ASTUserNameWithHost> &ast,
  ...
  return IParserBase::wrapParseImpl(pos, [&] {
    ...
    if (ParserIdentifier(allow_query_parameter).parse(pos, name_ast, expected)) {
      if (ParserToken{TokenType::At}.ignore(pos, expected))
        if (!parseIdentifierOrStringLiteral(pos, expected, host_pattern) || host_pattern.empty()) return false;
...
bool ParserUserNameWithHost::parseImpl(Pos &pos, ASTPtr &node, Expected &expected) {
  ...
  if (!parseUserNameWithHost(pos, expected, res, allow_query_parameter)) return false;

  node = res;
  return true;
}

ParserUserNameWithHost::ParserUserNameWithHost(bool allow_query_parameter_) : allow_query_parameter(allow_query_parameter_) {}
...
bool ParserUserNamesWithHost::parseImpl(Pos &pos, ASTPtr &node, Expected &expected) {
  ...
  auto parse_single_name = [&] {
    ...
    if (!parseUserNameWithHost(pos, expected, ast, allow_query_parameter)) return false;

    names.emplace_back(std::move(ast));
    return true;
  };

  if (!ParserList::parseUtil(pos, expected, parse_single_name, false)) return false;
...
ParserUserNamesWithHost::ParserUserNamesWithHost(bool allow_query_parameter_) : allow_query_parameter(allow_query_parameter_) {}
