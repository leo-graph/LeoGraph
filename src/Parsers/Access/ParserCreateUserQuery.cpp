...
bool parseRenameTo(IParserBase::Pos &pos, Expected &expected, std::optional<String> &new_name) {
  return IParserBase::wrapParseImpl(pos, [&] {
    if (!ParserKeyword{Keyword::RENAME_TO}.ignore(pos, expected)) return false;

    String maybe_new_name;
    if (!parseUserName(pos, expected, maybe_new_name, /*allow_query_parameter=*/true)) return false;
...
bool parseValidUntil(IParserBase::Pos &pos, Expected &expected, ASTPtr &valid_until) {
  return IParserBase::wrapParseImpl(pos, [&] {
    if (!ParserKeyword{Keyword::VALID_UNTIL}.ignore(pos, expected)) return false;
...
bool parseAuthenticationData(IParserBase::Pos &pos, Expected &expected, boost::intrusive_ptr<ASTAuthenticationData> &auth_data,
                             bool is_type_specifier_mandatory, bool is_type_specifier_allowed, bool should_parse_no_password) {
  return IParserBase::wrapParseImpl(pos, [&] {
    ...
    auto parse_non_password_based_type = [&](auto check_type) {
      if (ParserKeyword{AuthenticationTypeInfo::get(check_type).keyword}.ignore(pos, expected)) {
        ...
        if (check_type == AuthenticationType::NO_AUTHENTICATION) return true;
...

    {
      ...
      for (auto check_type : collections::range(first_authentication_type_element_to_check, AuthenticationType::MAX)) {
        if (parse_non_password_based_type(check_type)) break;
      }
    }
    ...
    if (expect_password || expect_hash) {
      if (!ParserKeyword{Keyword::BY}.ignore(pos, expected) || !ParserStringAndSubstitution{}.parse(pos, value, expected)) return false;
      ...
      if (expect_hash && (type == AuthenticationType::SHA256_PASSWORD || type == AuthenticationType::SCRAM_SHA256_PASSWORD)) {
        if (ParserKeyword{Keyword::SALT}.ignore(pos, expected)) {
          if (!ParserStringAndSubstitution{}.parse(pos, parsed_salt, expected)) return false;
        }
      }
    } else if (expect_ldap_server_name) {
      if (!ParserKeyword{Keyword::SERVER}.ignore(pos, expected) || !ParserStringAndSubstitution{}.parse(pos, value, expected)) return false;
    } else if (expect_kerberos_realm) {
      if (ParserKeyword{Keyword::REALM}.ignore(pos, expected)) {
        if (!ParserStringAndSubstitution{}.parse(pos, value, expected)) return false;
      }
      ...
      if (!ssl_cert_subject_type) return false;

      if (!ParserList{std::make_unique<ParserStringAndSubstitution>(), std::make_unique<ParserToken>(TokenType::Comma), false}.parse(
              pos, ssl_cert_subjects, expected))
        return false;
    } else if (expect_public_ssh_key) {
      if (!ParserKeyword{Keyword::BY}.ignore(pos, expected)) return false;

      if (!ParserList{std::make_unique<ParserPublicSSHKey>(), std::make_unique<ParserToken>(TokenType::Comma), false}.parse(
              pos, public_ssh_keys, expected))
        return false;
    } else if (expect_http_auth_server) {
      if (!ParserKeyword{Keyword::SERVER}.ignore(pos, expected)) return false;
      if (!ParserStringAndSubstitution{}.parse(pos, value, expected)) return false;

      if (ParserKeyword{Keyword::SCHEME}.ignore(pos, expected)) {
        if (!ParserStringAndSubstitution{}.parse(pos, http_auth_scheme, expected)) return false;
      }
    }
    ...
    if (value) auth_data->children.push_back(std::move(value));

    if (parsed_salt) auth_data->children.push_back(std::move(parsed_salt));
    ...
    if (public_ssh_keys) auth_data->children = std::move(public_ssh_keys->children);
...
bool parseIdentifiedWith(IParserBase::Pos &pos, Expected &expected,
                         std::vector<boost::intrusive_ptr<ASTAuthenticationData>> &authentication_methods, bool should_parse_no_password) {
  return IParserBase::wrapParseImpl(pos, [&] {
    if (!ParserKeyword{Keyword::IDENTIFIED}.ignore(pos, expected)) return false;
...
bool parseIdentifiedOrNotIdentified(IParserBase::Pos &pos, Expected &expected,
...
bool parseHostsWithoutPrefix(IParserBase::Pos &pos, Expected &expected, AllowedClientHosts &hosts) {
  ...
  auto parse_host = [&] {
    if (ParserKeyword{Keyword::NONE}.ignore(pos, expected)) return true;
    ...
    if (ParserKeyword{Keyword::REGEXP}.ignore(pos, expected)) {
      ASTPtr ast;
      if (!ParserList{std::make_unique<ParserStringLiteral>(), std::make_unique<ParserToken>(TokenType::Comma), false}.parse(pos, ast,
                                                                                                                             expected))
        return false;
    ...
    if (ParserKeyword{Keyword::NAME}.ignore(pos, expected)) {
      ...
      for (const auto &name_ast : ast->children) res_hosts.addName(name_ast->as<const ASTLiteral &>().value.safeGet<String>());
    ...
    if (ParserKeyword{Keyword::IP}.ignore(pos, expected)) {
      ...
      for (const auto &subnet_ast : ast->children) res_hosts.addSubnet(subnet_ast->as<const ASTLiteral &>().value.safeGet<String>());
    ...
    if (ParserKeyword{Keyword::LIKE}.ignore(pos, expected)) {
      ...
      for (const auto &pattern_ast : ast->children) res_hosts.addLikePattern(pattern_ast->as<const ASTLiteral &>().value.safeGet<String>());
  ...
  if (!ParserList::parseUtil(pos, expected, parse_host, false)) return false;
...
bool parseHosts(IParserBase::Pos &pos, Expected &expected, std::string_view prefix, AllowedClientHosts &hosts) {
  return IParserBase::wrapParseImpl(pos, [&] {
    if (!prefix.empty() && !ParserKeyword::createDeprecated(prefix).ignore(pos, expected)) return false;

    if (!ParserKeyword{Keyword::HOST}.ignore(pos, expected)) return false;

    AllowedClientHosts res_hosts;
    if (!parseHostsWithoutPrefix(pos, expected, res_hosts)) return false;
...
bool parseRoles(IParserBase::Pos &pos, Expected &expected, bool default_roles, bool id_mode,
                boost::intrusive_ptr<ASTRolesOrUsersSet> &roles) {
  return IParserBase::wrapParseImpl(pos, [&] {
    if (!ParserKeyword{default_roles ? Keyword::DEFAULT_ROLE : Keyword::ROLE}.ignore(pos, expected)) return false;

    ParserRolesOrUsersSet roles_p;
    roles_p.allowRoles().useIDMode(id_mode);
    if (default_roles) roles_p.allowAll();

    ASTPtr ast;
    if (!roles_p.parse(pos, ast, expected)) return false;
...
bool parseSettings(IParserBase::Pos &pos, Expected &expected, bool id_mode, boost::intrusive_ptr<ASTSettingsProfileElements> &settings) {
  return IParserBase::wrapParseImpl(pos, [&] {
    ...
    if (!elements_p.parse(pos, ast, expected)) return false;
...
bool parseGrantees(IParserBase::Pos &pos, Expected &expected, bool id_mode, boost::intrusive_ptr<ASTRolesOrUsersSet> &grantees) {
  return IParserBase::wrapParseImpl(pos, [&] {
    if (!ParserKeyword{Keyword::GRANTEES}.ignore(pos, expected)) return false;

    ASTPtr ast;
    ParserRolesOrUsersSet grantees_p;
    grantees_p.allowAny().allowUsers().allowCurrentUser().allowRoles().useIDMode(id_mode);
    if (!grantees_p.parse(pos, ast, expected)) return false;
...
bool parseDefaultDatabase(IParserBase::Pos &pos, Expected &expected, boost::intrusive_ptr<ASTDatabaseOrNone> &default_database) {
  return IParserBase::wrapParseImpl(pos, [&] {
    if (!ParserKeyword{Keyword::DEFAULT_DATABASE}.ignore(pos, expected)) return false;

    ASTPtr ast;
    ParserDatabaseOrNone database_p;
    if (!database_p.parse(pos, ast, expected)) return false;
...
bool parseAddIdentifiedWith(IParserBase::Pos &pos, Expected &expected,
...
bool parseResetAuthenticationMethods(IParserBase::Pos &pos, Expected &expected) {
  return IParserBase::wrapParseImpl(pos, [&] { return ParserKeyword{Keyword::RESET_AUTHENTICATION_METHODS_TO_NEW}.ignore(pos, expected); });
}
}  // namespace
...
bool ParserCreateUserQuery::parseImpl(Pos &pos, ASTPtr &node, Expected &expected) {
  bool alter = false;
  if (attach_mode) {
    if (!ParserKeyword{Keyword::ATTACH_USER}.ignore(pos, expected)) return false;
  } else {
  ...
  if (alter) {
    if (ParserKeyword{Keyword::IF_EXISTS}.ignore(pos, expected)) if_exists = true;
  } else {
  ...
  ASTPtr names_ast;
  if (!ParserUserNamesWithHost(/*allow_query_parameter=*/true).parse(pos, names_ast, expected)) return false;
  ...
  while (true) {
    ...
    if (alter) {
      ...
      if (parseAlterSettings(pos, expected, new_alter_settings)) {
        if (!alter_settings) alter_settings = make_intrusive<ASTAlterSettingsProfileElements>();
      ...
      if (parseSettings(pos, expected, attach_mode, new_settings)) {
        if (!settings) settings = make_intrusive<ASTSettingsProfileElements>();
        settings->add(std::move(*new_settings));
        continue;
      }
    }

    if (!roles && !alter && !attach_mode && parseRoles(pos, expected, /* default_roles = */ false, attach_mode, roles)) continue;

    if (!default_roles && parseRoles(pos, expected, /* default_roles = */ true, attach_mode, default_roles)) continue;

    if (cluster.empty() && parseOnCluster(pos, expected, cluster)) continue;

    if (!grantees && parseGrantees(pos, expected, attach_mode, grantees)) continue;

    if (!default_database && parseDefaultDatabase(pos, expected, default_database)) continue;
  ...
  if (query->global_valid_until) query->children.push_back(query->global_valid_until);
