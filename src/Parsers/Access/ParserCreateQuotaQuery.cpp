...
bool parseKeyType(IParserBase::Pos &pos, Expected &expected, QuotaKeyType &key_type) {
  return IParserBase::wrapParseImpl(pos, [&] {
    ...
    if (!ParserKeyword{Keyword::KEY_BY}.ignore(pos, expected) && !ParserKeyword{Keyword::KEYED_BY}.ignore(pos, expected)) return false;

    Strings names;
    if (!parseIdentifiersOrStringLiterals(pos, expected, names)) return false;
    ...
    throw Exception(ErrorCodes::SYNTAX_ERROR, "Quota cannot be keyed by '{}'. Expected one of the following identifiers: {}", name,
                    all_types_str);
  });
}
...
bool parseMaxValue(IParserBase::Pos &pos, Expected &expected, QuotaType quota_type, QuotaValue &max_value) {
  ASTPtr ast;
  if (!ParserNumber{}.parse(pos, ast, expected) && !ParserStringLiteral{}.parse(pos, ast, expected)) return false;
...
bool parseLimits(IParserBase::Pos &pos, Expected &expected, std::vector<std::pair<QuotaType, QuotaValue>> &limits) {
  ...
  auto parse_limit = [&] {
    ...
    if (!parseQuotaType(pos, expected, quota_type)) return false;

    if (max_prefix_encountered) {
      ParserToken{TokenType::Equals}.ignore(pos, expected);
    } else {
      if (!ParserKeyword{Keyword::MAX}.ignore(pos, expected)) return false;
    }

    QuotaValue max_value;
    if (!parseMaxValue(pos, expected, quota_type, max_value)) return false;

    res_limits.emplace_back(quota_type, max_value);
    return true;
  };

  if (!ParserList::parseUtil(pos, expected, parse_limit, false)) return false;
...
bool parseIntervalsWithLimits(IParserBase::Pos &pos, Expected &expected, std::vector<ASTCreateQuotaQuery::Limits> &all_limits) {
  ...
  auto parse_interval_with_limits = [&] {
    if (!ParserKeyword{Keyword::FOR}.ignore(pos, expected)) return false;
    ...
    if (!ParserNumber{}.parse(pos, num_intervals_ast, expected)) return false;

    double num_intervals = applyVisitor(FieldVisitorConvertToNumber<double>(), num_intervals_ast->as<ASTLiteral &>().value);

    IntervalKind interval_kind;
    if (!parseIntervalKind(pos, expected, interval_kind)) return false;
    ...
    if (ParserKeyword{Keyword::NO_LIMITS}.ignore(pos, expected)) {
      ...
      for (const auto &[quota_type, max_value] : new_limits) limits.max[static_cast<size_t>(quota_type)] = max_value;
    } else
      return false;
  ...
  if (!ParserList::parseUtil(pos, expected, parse_interval_with_limits, false)) return false;
...
bool parseToRoles(IParserBase::Pos &pos, Expected &expected, bool id_mode, boost::intrusive_ptr<ASTRolesOrUsersSet> &roles) {
  return IParserBase::wrapParseImpl(pos, [&] {
    ...
    if (!ParserKeyword{Keyword::TO}.ignore(pos, expected) || !roles_p.parse(pos, node, expected)) return false;
...
bool ParserCreateQuotaQuery::parseImpl(Pos &pos, ASTPtr &node, Expected &expected) {
  bool alter = false;
  if (attach_mode) {
    if (!ParserKeyword{Keyword::ATTACH_QUOTA}.ignore(pos, expected)) return false;
  } else {
  ...
  while (true) {
    ...
    if (parseIntervalsWithLimits(pos, expected, all_limits)) continue;
