...
bool parseNameOrID(IParserBase::Pos &pos, Expected &expected, bool id_mode, String &res) {
  return IParserBase::wrapParseImpl(pos, [&] {
    if (!id_mode) return parseRoleName(pos, expected, res);
...
bool parseBeforeExcept(IParserBase::Pos &pos, Expected &expected, bool id_mode, bool allow_all, bool allow_any, bool allow_current_user,
                       bool &all, Strings &names, bool &current_user) {
  bool res_all = false;
  Strings res_names;
...
bool parseExceptAndAfterExcept(IParserBase::Pos &pos, Expected &expected, bool id_mode, bool allow_current_user, Strings &except_names,
                               bool &except_current_user) {
  return IParserBase::wrapParseImpl(pos, [&] {
    if (!ParserKeyword{Keyword::EXCEPT}.ignore(pos, expected)) return false;
...
bool ParserRolesOrUsersSet::parseImpl(Pos &pos, ASTPtr &node, Expected &expected) {
  ...
  if (!parseBeforeExcept(pos, expected, id_mode, allow_all, allow_any, allow_current_user, all, names, current_user)) return false;
