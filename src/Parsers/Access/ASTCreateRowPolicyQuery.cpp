...
void formatForClause(const boost::container::flat_set<std::string_view> &commands, const String &filter, const String &check, bool alter,
                     WriteBuffer &ostr, const IAST::FormatSettings &) {
  ostr << " FOR ";
  ...
  if (!filter.empty()) ostr << " USING" << filter;

  if (!check.empty() && (alter || (check != filter))) ostr << " WITH CHECK" << check;
}

void formatForClauses(const std::vector<std::pair<RowPolicyFilterType, ASTPtr>> &filters, bool alter, WriteBuffer &ostr,
  ...
  do {
    ...
    if (!filter.empty() || !check.empty()) formatForClause(commands, filter, check, alter, ostr, settings);
...
String ASTCreateRowPolicyQuery::getID(char) const { return "CREATE ROW POLICY or ALTER ROW POLICY query"; }

ASTPtr ASTCreateRowPolicyQuery::clone() const {
  auto res = make_intrusive<ASTCreateRowPolicyQuery>(*this);

  if (names) res->names = boost::static_pointer_cast<ASTRowPolicyNames>(names->clone());
  ...
  for (auto &[_, res_filter] : res->filters) {
    if (res_filter) res_filter = res_filter->clone();
  }
...
void ASTCreateRowPolicyQuery::formatImpl(WriteBuffer &ostr, const FormatSettings &settings, FormatState &, FormatStateStacked) const {
  ...
  if (!new_short_name.empty()) formatRenameTo(new_short_name, ostr, settings);

  if (is_restrictive) formatAsRestrictiveOrPermissive(*is_restrictive, ostr, settings);

  formatForClauses(filters, alter, ostr, settings);

  if (roles && (!roles->empty() || alter)) formatToRoles(*roles, ostr, settings);
}

void ASTCreateRowPolicyQuery::replaceCurrentUserTag(const String &current_user_name) const {
  if (roles) roles->replaceCurrentUserTag(current_user_name);
}

void ASTCreateRowPolicyQuery::replaceEmptyDatabase(const String &current_database) const {
  if (names) names->replaceEmptyDatabase(current_database);
}
}  // namespace DB
