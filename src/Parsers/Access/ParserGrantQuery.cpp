...
bool parseCurrentGrants(IParser::Pos& pos, Expected& expected, AccessRightsElements& elements) {
  if (ParserToken(TokenType::OpeningRoundBracket).ignore(pos, expected)) {
    ...
    if (!parseDatabaseAndTableNameOrAsterisks(pos, expected, database_name, table_name, wildcard, default_database)) return false;
...
bool parseToGrantees(IParser::Pos& pos, Expected& expected, bool is_revoke, boost::intrusive_ptr<ASTRolesOrUsersSet>& grantees) {
  return IParserBase::wrapParseImpl(pos, [&] {
    if (!ParserKeyword{is_revoke ? Keyword::FROM : Keyword::TO}.ignore(pos, expected)) return false;
...
bool ParserGrantQuery::parseImpl(Pos& pos, ASTPtr& node, Expected& expected) {
  if (attach_mode && !ParserKeyword{Keyword::ATTACH}.ignore(pos, expected)) return false;
  ...
  if (!is_revoke && ParserKeyword{Keyword::CURRENT_GRANTS}.ignore(pos, expected)) {
    current_grants = true;
    if (!parseCurrentGrants(pos, expected, elements)) return false;
  } else {
  ...
  if (!parseToGrantees(pos, expected, is_revoke, grantees) && !allow_no_grantees) return false;
  ...
  if (!is_revoke) {
    ...
    if (ParserKeyword{Keyword::WITH_REPLACE_OPTION}.ignore(pos, expected)) is_replace = true;
  }

  if (cluster.empty()) parseOnCluster(pos, expected, cluster);

  if (grant_option && roles) throw Exception(ErrorCodes::SYNTAX_ERROR, "GRANT OPTION should be specified for access types");
  if (admin_option && !elements.empty()) throw Exception(ErrorCodes::SYNTAX_ERROR, "ADMIN OPTION should be specified for roles");

  if (grant_option) {
    for (auto& element : elements) element.grant_option = true;
  }
  ...
  if (!is_revoke && !attach_mode) elements.throwIfNotGrantable();
