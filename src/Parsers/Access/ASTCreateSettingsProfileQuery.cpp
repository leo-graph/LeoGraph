...
String ASTCreateSettingsProfileQuery::getID(char) const { return "CreateSettingsProfileQuery"; }

ASTPtr ASTCreateSettingsProfileQuery::clone() const {
  auto res = make_intrusive<ASTCreateSettingsProfileQuery>(*this);

  if (to_roles) res->to_roles = boost::static_pointer_cast<ASTRolesOrUsersSet>(to_roles->clone());
...
void ASTCreateSettingsProfileQuery::formatImpl(WriteBuffer &ostr, const FormatSettings &format, FormatState &, FormatStateStacked) const {
  ...
  if (!new_name.empty()) formatRenameTo(new_name, ostr, format);
  ...
  if (to_roles && (!to_roles->empty() || alter)) formatToRoles(*to_roles, ostr, format);
}

void ASTCreateSettingsProfileQuery::replaceCurrentUserTag(const String &current_user_name) const {
  if (to_roles) to_roles->replaceCurrentUserTag(current_user_name);
}
}  // namespace DB
