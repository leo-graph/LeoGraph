...
bool parseProfileNameOrID(IParserBase::Pos &pos, Expected &expected, bool id_mode, String &res) {
  return IParserBase::wrapParseImpl(pos, [&] {
    ASTPtr ast;
    if (!id_mode) return parseIdentifierOrStringLiteral(pos, expected, res);

    if (!ParserKeyword{Keyword::ID}.ignore(pos, expected)) return false;
    if (!ParserToken(TokenType::OpeningRoundBracket).ignore(pos, expected)) return false;
...
bool parseValue(IParserBase::Pos &pos, Expected &expected, std::optional<Field> &res) {
  return IParserBase::wrapParseImpl(pos, [&] {
    if (!ParserToken{TokenType::Equals}.ignore(pos, expected)) return false;

    ASTPtr ast;
    if (!ParserLiteral{}.parse(pos, ast, expected)) return false;
...
bool parseMinMaxValue(IParserBase::Pos &pos, Expected &expected, std::optional<Field> &min_value, std::optional<Field> &max_value) {
  return IParserBase::wrapParseImpl(pos, [&] {
    ...
    if (!is_min_value && !is_max_value) return false;
...
bool parseConstraintWritabilityKeyword(IParserBase::Pos &pos, Expected &expected,
...
bool parseSettingName(IParserBase::Pos &pos, Expected &expected, String &res) {
  ASTPtr name_ast;
  if (!ParserCompoundIdentifier{}.parse(pos, name_ast, expected)) return false;

  res = getIdentifierName(name_ast);
  return true;
}

bool parseSettingNameWithValueOrConstraints(IParserBase::Pos &pos, Expected &expected, String &setting_name, std::optional<Field> &value,
                                            std::optional<Field> &min_value, std::optional<Field> &max_value,
                                            std::optional<SettingConstraintWritability> &writability) {
  return IParserBase::wrapParseImpl(pos, [&] {
    String res_setting_name;
    if (!parseSettingName(pos, expected, res_setting_name)) return false;
    ...
    if (!has_value_or_constraint) return false;
...
bool parseSettingsProfileElements(IParserBase::Pos &pos, Expected &expected, bool id_mode, bool use_inherit_keyword,
  ...
  auto parse_element = [&] {
    ...
    if (!expect_profiles && !expect_settings) return false;
  ...
  if (elements.empty() && !found_none) return false;
...
bool ParserSettingsProfileElement::parseImpl(Pos &pos, ASTPtr &node, Expected &expected) {
  std::vector<boost::intrusive_ptr<ASTSettingsProfileElement>> elements;
  if (!parseSettingsProfileElements(pos, expected, id_mode, use_inherit_keyword, elements)) return false;
...
bool ParserAlterSettingsProfileElements::parseImpl(Pos &pos, ASTPtr &node, Expected &expected) {
  ...
  if (parseSettingsProfileElements(pos, expected, /* id_mode= */ false, use_inherit_keyword, old_style_settings)) {
    ...
    auto parse_element = [&] {
      if (ParserKeyword{Keyword::ADD}.ignore(pos, expected)) {
        ...
        if (!add_settings.empty() || !modify_settings.empty()) return false;
        action = "DROP";
        target = "";
      ...
      if (target == "PROFILES") {
        auto element = make_intrusive<ASTSettingsProfileElement>();
        if (!parseProfileNameOrID(pos, expected, /* id_mode= */ false, element->parent_profile)) return false;
        if (action == "ADD") {
      ...
      if (target == "SETTINGS") {
        ...
        if (action == "ADD" || action == "MODIFY") {
          if (!parseSettingNameWithValueOrConstraints(pos, expected, element->setting_name, element->value, element->min_value,
                                                      element->max_value, element->writability))
            return false;
          if (action == "ADD")
            add_settings.push_back(element);
          else
  ...
  if (add_settings.empty() && modify_settings.empty() && drop_settings.empty() && !drop_all_settings && !drop_all_profiles) return false;
