...
bool parseAsRestrictiveOrPermissive(IParserBase::Pos& pos, Expected& expected, bool& is_restrictive) {
  return IParserBase::wrapParseImpl(pos, [&] {
    ...
    if (!ParserKeyword{Keyword::PERMISSIVE}.ignore(pos, expected)) return false;
...
bool parseFilterExpression(IParserBase::Pos& pos, Expected& expected, ASTPtr& expr) {
  return IParserBase::wrapParseImpl(pos, [&] {
    ...
    ASTPtr x;
    if (!parser.parse(pos, x, expected)) return false;
...
bool parseCommands(IParserBase::Pos& pos, Expected& expected, boost::container::flat_set<std::string_view>& commands) {
  ...
  if (!ParserList::parseUtil(pos, expected, parse_command, false)) return false;

  commands = std::move(res_commands);
  return true;
}

bool parseForClauses(IParserBase::Pos& pos, Expected& expected, bool alter, std::vector<std::pair<RowPolicyFilterType, ASTPtr>>& filters) {
  ...
  auto parse_for_clause = [&] {
    ...
    if (ParserKeyword{Keyword::FOR}.ignore(pos, expected)) {
      if (!parseCommands(pos, expected, commands)) return false;
    } else
    ...
    if (ParserKeyword{Keyword::USING}.ignore(pos, expected)) {
      if (!parseFilterExpression(pos, expected, filter.emplace())) return false;
    }
    if (ParserKeyword{Keyword::WITH_CHECK}.ignore(pos, expected)) {
      if (!parseFilterExpression(pos, expected, check.emplace())) return false;
    }

    if (!filter && !check) return false;
  ...
  if (!ParserList::parseUtil(pos, expected, parse_for_clause, false)) return false;
...
bool ParserCreateRowPolicyQuery::parseImpl(Pos& pos, ASTPtr& node, Expected& expected) {
  ...
  if (attach_mode) {
    ...
    if (ParserKeyword{Keyword::ALTER_POLICY}.ignore(pos, expected) || ParserKeyword{Keyword::ALTER_ROW_POLICY}.ignore(pos, expected))
      alter = true;
    else if (!ParserKeyword{Keyword::CREATE_POLICY}.ignore(pos, expected) &&
             !ParserKeyword{Keyword::CREATE_ROW_POLICY}.ignore(pos, expected))
      return false;
  }
  ...
  ASTPtr names_ast;
  if (!names_parser.parse(pos, names_ast, expected)) return false;
  ...
  while (true) {
    if (alter && (names->full_names.size() == 1) && new_short_name.empty() && parseRenameTo(pos, expected, new_short_name)) continue;
