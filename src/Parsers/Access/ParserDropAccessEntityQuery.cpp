...
bool parseMaskingPolicyName(IParserBase::Pos& pos, Expected& expected, MaskingPolicyName& name, String& cluster) {
  return IParserBase::wrapParseImpl(pos, [&] {
    String short_name;
    if (!parseIdentifierOrStringLiteral(pos, expected, short_name)) return false;
...
bool ParserDropAccessEntityQuery::parseImpl(Pos& pos, ASTPtr& node, Expected& expected) {
  if (!ParserKeyword{Keyword::DROP}.ignore(pos, expected)) return false;

  AccessEntityType type;
  if (!parseEntityType(pos, expected, type)) return false;
  ...
  if ((type == AccessEntityType::USER) || (type == AccessEntityType::ROLE)) {
    if (!parseUserNames(pos, expected, names, /*allow_query_parameter=*/false)) return false;
    ...
    if (!parseMaskingPolicyName(pos, expected, *masking_policy_name, cluster)) return false;
  } else {
    if (!parseIdentifiersOrStringLiterals(pos, expected, names)) return false;
  }

  if (ParserKeyword{Keyword::FROM}.ignore(pos, expected)) parseAccessStorageName(pos, expected, storage_name);
