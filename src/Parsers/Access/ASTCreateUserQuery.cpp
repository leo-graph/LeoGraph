...
void formatAuthenticationData(const std::vector<boost::intrusive_ptr<ASTAuthenticationData>> &authentication_methods, WriteBuffer &ostr,
...
void formatHosts(const char *prefix, const AllowedClientHosts &hosts, WriteBuffer &ostr, const IAST::FormatSettings &) {
  ...
  if (!addresses.empty() || !subnets.empty()) {
    ...
    for (const auto &address : addresses) {
      if (std::exchange(need_comma2, true)) ostr << ", ";
...
String ASTCreateUserQuery::getID(char) const { return "CreateUserQuery"; }
...
ASTPtr ASTCreateUserQuery::clone() const {
  ...
  if (names) res->names = boost::static_pointer_cast<ASTUserNamesWithHost>(names->clone());

  if (roles) res->roles = boost::static_pointer_cast<ASTRolesOrUsersSet>(roles->clone());

  if (default_roles) res->default_roles = boost::static_pointer_cast<ASTRolesOrUsersSet>(default_roles->clone());

  if (default_database) res->default_database = boost::static_pointer_cast<ASTDatabaseOrNone>(default_database->clone());

  if (grantees) res->grantees = boost::static_pointer_cast<ASTRolesOrUsersSet>(grantees->clone());

  if (settings) res->settings = boost::static_pointer_cast<ASTSettingsProfileElements>(settings->clone());

  if (alter_settings) res->alter_settings = boost::static_pointer_cast<ASTAlterSettingsProfileElements>(alter_settings->clone());
...
void ASTCreateUserQuery::formatImpl(WriteBuffer &ostr, const FormatSettings &format, FormatState &, FormatStateStacked) const {
  ...
  if (!storage_name.empty()) ostr << " IN " << backQuoteIfNeed(storage_name);

  formatOnCluster(ostr, format);

  if (new_name) formatRenameTo(*new_name, ostr, format);

  if (!authentication_methods.empty()) {
    if (add_identified_with) ostr << " ADD";

    ostr << " IDENTIFIED";
    formatAuthenticationData(authentication_methods, ostr, format);
  }

  if (global_valid_until) formatValidUntil(*global_valid_until, ostr, format);

  if (hosts) formatHosts(nullptr, *hosts, ostr, format);
  if (add_hosts) formatHosts("ADD", *add_hosts, ostr, format);
  if (remove_hosts) formatHosts("DROP", *remove_hosts, ostr, format);

  if (default_database) formatDefaultDatabase(*default_database, ostr, format);

  if (roles) formatRoles(*roles, ostr, format);

  if (default_roles) formatDefaultRoles(*default_roles, ostr, format);
  ...
  if (grantees) formatGrantees(*grantees, ostr, format);

  if (reset_authentication_methods_to_new) ostr << " RESET AUTHENTICATION METHODS TO NEW";
}
