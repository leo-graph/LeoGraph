...
void formatIntervalWithLimits(const ASTCreateQuotaQuery::Limits &limits, WriteBuffer &ostr, const IAST::FormatSettings &) {
  ...
  if (limits.drop) {
    ...
    for (auto quota_type : collections::range(QuotaType::MAX)) {
      ...
      if (limits.max[quota_type_i]) limit_found = true;
    }
    if (limit_found) {
      ostr << " MAX";
      bool need_comma = false;
...
void formatIntervalsWithLimits(const std::vector<ASTCreateQuotaQuery::Limits> &all_limits, WriteBuffer &ostr,
...
String ASTCreateQuotaQuery::getID(char) const { return "CreateQuotaQuery"; }
...
void ASTCreateQuotaQuery::formatImpl(WriteBuffer &ostr, const FormatSettings &settings, FormatState &, FormatStateStacked) const {
  ...
  if (key_type) formatKeyType(*key_type, ostr, settings);
