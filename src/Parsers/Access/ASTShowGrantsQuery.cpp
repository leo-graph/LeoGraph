...
namespace DB {
String ASTShowGrantsQuery::getID(char) const { return "ShowGrantsQuery"; }

ASTPtr ASTShowGrantsQuery::clone() const {
  auto res = make_intrusive<ASTShowGrantsQuery>(*this);

  if (for_roles) res->for_roles = boost::static_pointer_cast<ASTRolesOrUsersSet>(for_roles->clone());
