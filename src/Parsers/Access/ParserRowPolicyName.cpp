...
bool parseDBAndTableName(IParser::Pos& pos, Expected& expected, String& database, String& table_name) {
  return IParserBase::wrapParseImpl(pos, [&] {
    ...
    if (res_table_name.empty()) res_table_name = RowPolicyName::ANY_TABLE_MARK;
...
bool parseOnDBAndTableName(IParser::Pos& pos, Expected& expected, String& database, String& table_name) {
  return IParserBase::wrapParseImpl(
      pos, [&] { return ParserKeyword{Keyword::ON}.ignore(pos, expected) && parseDBAndTableName(pos, expected, database, table_name); });
}
...
bool parseOnDBAndTableNames(IParser::Pos& pos, Expected& expected, std::vector<std::pair<String, String>>& database_and_table_names) {
  return IParserBase::wrapParseImpl(pos, [&] {
    ...
    auto parse_db_and_table_name = [&] {
      String database;
      String table_name;
      if (!parseDBAndTableName(pos, expected, database, table_name)) return false;
      res.emplace_back(std::move(database), std::move(table_name));
      return true;
    };

    if (!ParserList::parseUtil(pos, expected, parse_db_and_table_name, false)) return false;
...
bool parseRowPolicyNamesAroundON(IParser::Pos& pos, Expected& expected, bool allow_multiple_short_names, bool allow_multiple_tables,
                                 bool allow_on_cluster, std::vector<RowPolicyName>& full_names, String& cluster) {
  return IParserBase::wrapParseImpl(pos, [&] {
    std::vector<String> short_names;
    if (allow_multiple_short_names) {
      if (!parseIdentifiersOrStringLiterals(pos, expected, short_names)) return false;
    } else {
      if (!parseIdentifierOrStringLiteral(pos, expected, short_names.emplace_back())) return false;
    }

    String res_cluster;
    if (allow_on_cluster) parseOnCluster(pos, expected, res_cluster);

    std::vector<std::pair<String, String>> database_and_table_names;
    if (allow_multiple_tables && (short_names.size() == 1)) {
      if (!parseOnDBAndTableNames(pos, expected, database_and_table_names)) return false;
    } else {
      String database;
      String table_name;
      if (!parseOnDBAndTableName(pos, expected, database, table_name)) return false;
      database_and_table_names.emplace_back(std::move(database), std::move(table_name));
    }

    if (allow_on_cluster && res_cluster.empty()) parseOnCluster(pos, expected, res_cluster);

    assert(!short_names.empty());
    assert(!database_and_table_names.empty());
    full_names.clear();
    for (const String& short_name : short_names)
      for (const auto& [database, table_name] : database_and_table_names) full_names.push_back({short_name, database, table_name});
...
bool ParserRowPolicyName::parseImpl(Pos& pos, ASTPtr& node, Expected& expected) {
  ...
  String cluster;
  if (!parseRowPolicyNamesAroundON(pos, expected, false, false, allow_on_cluster, full_names, cluster)) return false;
...
bool ParserRowPolicyNames::parseImpl(Pos& pos, ASTPtr& node, Expected& expected) {
  ...
  auto parse_around_on = [&] {
    if (!full_names.empty()) {
      if ((num_added_names_last_time != 1) || !cluster.empty()) return false;
    }
