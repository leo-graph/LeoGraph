...
class ASTPair : public IAST {
 public:
  ...
  explicit ASTPair(bool second_with_brackets_) : second_with_brackets(second_with_brackets_) {}
  ...
  void forEachPointerToChild(std::function<void(IAST **, boost::intrusive_ptr<IAST> *)> f) override { f(&second, nullptr); }
...
class ASTFunctionWithKeyValueArguments : public IAST {
 public:
  ...
  explicit ASTFunctionWithKeyValueArguments(bool has_brackets_ = true) : has_brackets(has_brackets_) {}
