...
bool parseDropQuery(IParser::Pos &pos, ASTPtr &node, Expected &expected, const ASTDropQuery::Kind kind) {
  ...
  if (s_all.checkWithoutMoving(pos, expected)) has_all = true;

  if (s_database.ignore(pos, expected)) {
    if (s_if_exists.ignore(pos, expected)) if_exists = true;

    if (s_if_empty.ignore(pos, expected)) if_empty = true;

    if (!name_p.parse(pos, database, expected)) return false;
  } else if ((s_tables.ignore(pos, expected) || (s_all.ignore(pos, expected) && s_tables.ignore(pos, expected))) &&
             kind == ASTDropQuery::Kind::Truncate) {
    ...
    bool not_like = false;
    if (s_not.ignore(pos, expected)) not_like = true;

    if (s_like.ignore(pos, expected)) {
      if (not_like) is_not_like = true;
      if (!like_p.parse(pos, like, expected)) return false;
      is_like = true;
    }
    ...
    if (s_temporary.ignore(pos, expected)) temporary = true;
    ...
    if (!tables_p.parse(pos, database_and_tables, expected)) return false;
  ...
  if (kind == ASTDropQuery::Kind::Detach && s_permanently.ignore(pos, expected)) permanently = true;

  /// actually for TRUNCATE NO DELAY / SYNC means nothing
  if (s_no_delay.ignore(pos, expected) || s_sync.ignore(pos, expected)) sync = true;
  ...
  if (database_and_tables) query->children.push_back(database_and_tables);

  if (is_like) query->like = like->as<ASTLiteral &>().value.safeGet<String>();
...
bool ParserDropQuery::parseImpl(Pos &pos, ASTPtr &node, Expected &expected) {
  ...
  if (s_drop.ignore(pos, expected)) return parseDropQuery(pos, node, expected, ASTDropQuery::Kind::Drop);
  if (s_detach.ignore(pos, expected)) return parseDropQuery(pos, node, expected, ASTDropQuery::Kind::Detach);
  if (s_truncate.ignore(pos, expected)) return parseDropQuery(pos, node, expected, ASTDropQuery::Kind::Truncate);
  return false;
}
