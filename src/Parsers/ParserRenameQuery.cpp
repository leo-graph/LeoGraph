...
bool ParserRenameQuery::parseImpl(Pos& pos, ASTPtr& node, Expected& expected) {
  ...
  if (s_rename_database.ignore(pos, expected)) {
    ...
    if (!db_name_p.parse(pos, from_db, expected)) return false;
    if (!s_to.ignore(pos, expected)) return false;
    if (!db_name_p.parse(pos, to_db, expected)) return false;
  ...
  while (true) {
    if (!elements.empty() && !s_comma.ignore(pos)) break;

    ASTRenameQuery::Element& ref = elements.emplace_back();

    if (!exchange) ref.if_exists = s_if_exists.ignore(pos, expected);

    if (!parseDatabaseAndTableAsAST(pos, expected, ref.from.database, ref.from.table) || !ignore_delim() ||
