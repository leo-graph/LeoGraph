...
ASTPtr ASTStorage::clone() const {
  ...
  if (engine) res->set(res->engine, engine->clone());
  if (partition_by) res->set(res->partition_by, partition_by->clone());
  if (primary_key) res->set(res->primary_key, primary_key->clone());
  if (order_by) res->set(res->order_by, order_by->clone());
  if (sample_by) res->set(res->sample_by, sample_by->clone());
  if (ttl_table) res->set(res->ttl_table, ttl_table->clone());

  if (settings) res->set(res->settings, settings->clone());
...
bool ASTStorage::isExtendedStorageDefinition() const { return partition_by || primary_key || order_by || sample_by || settings; }
...
class ASTColumnsElement : public IAST {
 public:
  ...
  void forEachPointerToChild(std::function<void(IAST **, boost::intrusive_ptr<IAST> *)> f) override { f(&elem, nullptr); }
...
ASTPtr ASTColumnsElement::clone() const {
  ...
  res->prefix = prefix;
  if (elem) res->set(res->elem, elem->clone());
  return res;
}
...
ASTPtr ASTColumns::clone() const {
  ...
  if (columns) res->set(res->columns, columns->clone());
  if (indices) res->set(res->indices, indices->clone());
  if (constraints) res->set(res->constraints, constraints->clone());
  if (projections) res->set(res->projections, projections->clone());
  if (primary_key) res->set(res->primary_key, primary_key->clone());
  if (primary_key_from_columns) res->set(res->primary_key_from_columns, primary_key_from_columns->clone());
...
ASTPtr ASTCreateQuery::clone() const {
  ...
  if (columns_list) res->set(res->columns_list, columns_list->clone());
  if (aliases_list) res->set(res->aliases_list, aliases_list->clone());
  if (storage) res->set(res->storage, storage->clone());
  if (select) res->set(res->select, select->clone());
  if (table_overrides) res->set(res->table_overrides, table_overrides->clone());
  if (targets) res->set(res->targets, targets->clone());
  if (sql_security) res->set(res->sql_security, sql_security->clone());
  if (watermark_function) res->set(res->watermark_function, watermark_function->clone());
  if (lateness_function) res->set(res->lateness_function, lateness_function->clone());
  ...
  if (refresh_strategy) res->set(res->refresh_strategy, refresh_strategy->clone());
  if (as_table_function) res->set(res->as_table_function, as_table_function->clone());
  if (comment) res->set(res->comment, comment->clone());
...
String ASTCreateQuery::getID(char delim) const {
  ...
  if (!database.empty()) res += (delim + getDatabase());
  res += (delim + getTable());
  return res;
}

void ASTCreateQuery::formatQueryImpl(WriteBuffer &ostr, const FormatSettings &settings, FormatState &state,
                                     FormatStateStacked frame) const {
  frame.need_parens = false;

  if (database && !table) {
    ostr << (attach ? "ATTACH DATABASE " : "CREATE DATABASE ") << (if_not_exists ? "IF NOT EXISTS " : "");

    database->format(ostr, settings, state, frame);

    if (uuid != UUIDHelpers::Nil) ostr << " UUID " << quoteString(toString(uuid));

    formatOnCluster(ostr, settings);

    if (storage) storage->format(ostr, settings, state, frame);
  ...
  if (!is_dictionary) {
    ...
    ostr << action;
    ostr << " ";
    ostr << (isTemporary() ? "TEMPORARY " : "") << what << " " << (if_not_exists ? "IF NOT EXISTS " : "");
    ...
    if (has_attach_from_path) ostr << " FROM " << quoteString(attach_from_path);
  ...
  if (auto to_inner_uuid = getTargetInnerUUID(ViewTarget::To); to_inner_uuid != UUIDHelpers::Nil) {
    ostr << " " << toStringView(Keyword::TO_INNER_UUID) << " " << quoteString(toString(to_inner_uuid));
  }

  bool should_add_empty = is_create_empty;
  auto add_empty_if_needed = [&] {
    if (!should_add_empty) return;
  ...
  if (!as_table.empty()) {
    add_empty_if_needed();
    add_clone_if_needed();
    ostr << " AS " << (!as_database.empty() ? backQuoteIfNeed(as_database) + "." : "") << backQuoteIfNeed(as_table);
  }
  ...
  if (auto *to_storage = getTargetInnerEngine(ViewTarget::To)) to_storage->format(ostr, settings, state, frame);
  ...
  if (dictionary) dictionary->format(ostr, settings, state, frame);
...
bool ASTCreateQuery::isParameterizedView() const {
  if (is_ordinary_view && select && select->hasQueryParameters()) return true;
  return false;
}

NameToNameMap ASTCreateQuery::getQueryParameters() const {
  if (!select || !isParameterizedView()) return {};

  return select->getQueryParameters();
}

void ASTCreateQuery::generateRandomUUIDs() { CreateQueryUUIDs{*this, /* generate_random= */ true}.copyToQuery(*this); }

void ASTCreateQuery::resetUUIDs() { CreateQueryUUIDs{}.copyToQuery(*this); }

StorageID ASTCreateQuery::getTargetTableID(ViewTarget::Kind target_kind) const {
  if (targets) return targets->getTableID(target_kind);
  return StorageID::createEmpty();
}

bool ASTCreateQuery::hasTargetTableID(ViewTarget::Kind target_kind) const {
  if (targets) return targets->hasTableID(target_kind);
  return false;
}

UUID ASTCreateQuery::getTargetInnerUUID(ViewTarget::Kind target_kind) const {
  if (targets) return targets->getInnerUUID(target_kind);
  return UUIDHelpers::Nil;
}

bool ASTCreateQuery::hasInnerUUIDs() const {
  if (targets) return targets->hasInnerUUIDs();
  return false;
}

ASTStorage *ASTCreateQuery::getTargetInnerEngine(ViewTarget::Kind target_kind) const {
  if (targets) return targets->getInnerEngine(target_kind);
  return nullptr;
}

void ASTCreateQuery::setTargetInnerEngine(ViewTarget::Kind target_kind, ASTPtr storage_def) {
  if (!targets) set(targets, make_intrusive<ASTViewTargets>());
