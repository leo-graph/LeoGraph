...
class ASTRenameQuery : public ASTQueryWithOutput, public ASTQueryWithOnCluster {
 public:
  ...
  struct Element {
    Table from;
    Table to;
    bool if_exists{false};  /// If this directive is used, one will not get an error if the table/database/dictionary to be
                            /// renamed/exchanged doesn't exist.
  };
  ...
  explicit ASTRenameQuery(Elements elements_ = {}) : elements(std::move(elements_)) {
    for (const auto &elem : elements) {
      if (elem.from.database) children.push_back(elem.from.database);
      if (elem.from.table) children.push_back(elem.from.table);
      if (elem.to.database) children.push_back(elem.to.database);
      if (elem.to.table) children.push_back(elem.to.table);
    }
  }
  ...
  void addElement(const String &from_db, const String &from_table, const String &to_db, const String &to_table) {
    ...
    elements.push_back(Element{.from = Table{.database = identifier(from_db), .table = identifier(from_table)},
                               .to = Table{.database = identifier(to_db), .table = identifier(to_table)}});
  }
 ...
 protected:
  void formatQueryImpl(WriteBuffer &ostr, const FormatSettings &settings, FormatState &state, FormatStateStacked frame) const override {
    if (database) {
      ...
      if (elements.at(0).if_exists) ostr << "IF EXISTS ";
    ...
    for (auto it = elements.cbegin(); it != elements.cend(); ++it) {
      if (it != elements.cbegin()) ostr << ", ";

      if (it->if_exists) ostr << "IF EXISTS ";
