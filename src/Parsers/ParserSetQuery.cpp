  ...
  bool parseImpl(Pos &pos, ASTPtr &node, Expected &expected) override {
    {
      ...
      if (literal.parse(pos, node, expected)) return true;
    }
    ...
    if (!l_br.ignore(pos, expected)) return false;

    Map map;

    while (!r_br.ignore(pos, expected)) {
      if (!map.empty() && !comma.ignore(pos, expected)) return false;

      ASTPtr key;
      ASTPtr val;

      if (!literal.parse(pos, key, expected)) return false;

      if (!colon.ignore(pos, expected)) return false;

      if (!literal.parse(pos, val, expected)) return false;
...
bool parseParameterValueIntoString(IParser::Pos &pos, String &value, Expected &expected) {
  ...
  if (literal_p.parse(pos, node, expected)) {
    ...
    /// writeQuoted is not always quoted in line with SQL standard
    /// https://github.com/ClickHouse/ClickHouse/blob/master/src/IO/WriteHelpers.h
...
bool ParserSetQuery::parseNameValuePair(SettingChange &change, IParser::Pos &pos, Expected &expected) {
  ...
  if (!s_eq.ignore(pos, expected)) return false;
  ...
  if (!literal_or_map_p.parse(pos, value, expected)) return false;
...
bool ParserSetQuery::parseNameValuePairWithParameterOrDefault(SettingChange &change, String &default_settings,
                                                              ParserSetQuery::Parameter &parameter, IParser::Pos &pos, Expected &expected,
  ...
  if (!name_p.parse(pos, node, expected)) return false;

  have_eq = s_eq.ignore(pos, expected);

  if (!enable_shorthand_syntax && !have_eq) return false;

  tryGetIdentifierNameInto(node, name);

  /// Parameter
  if (name.starts_with(QUERY_PARAMETER_NAME_PREFIX)) {
    if (!have_eq) return false;

    name = name.substr(strlen(QUERY_PARAMETER_NAME_PREFIX));

    if (name.empty()) throw Exception(ErrorCodes::BAD_ARGUMENTS, "Parameter name cannot be empty");

    String value;

    if (!parseParameterValueIntoString(pos, value, expected)) return false;
  ...
  if (have_eq) {
    ...
    if (!value_p.parse(pos, node, expected)) return false;
  } else {
    try {
...
bool ParserSetQuery::parseImpl(Pos &pos, ASTPtr &node, Expected &expected) {
  ...
  if (!parse_only_internals) {
    ...
    if (!s_set.ignore(pos, expected)) return false;

    /// Parse SET TRANSACTION ... queries using ParserTransactionControl
    if (ParserKeyword{Keyword::TRANSACTION}.check(pos, expected)) return false;
  }
  ...
  while (true) {
    if ((!changes.empty() || !query_parameters.empty() || !default_settings.empty()) && !s_comma.ignore(pos)) break;
