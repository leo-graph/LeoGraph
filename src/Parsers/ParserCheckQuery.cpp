...
bool ParserCheckQuery::parseCheckTable(Pos& pos, ASTPtr& node, Expected& expected) {
  ...
  if (!s_check_table.ignore(pos, expected)) return false;
  ...
  if (s_partition.ignore(pos, expected)) {
    if (!partition_parser.parse(pos, query->partition, expected)) return false;
  } else if (s_part.ignore(pos, expected)) {
    ASTPtr ast_part_name;
    if (!parser_string_literal.parse(pos, ast_part_name, expected)) return false;

    const auto* ast_literal = ast_part_name->as<ASTLiteral>();
    if (!ast_literal || ast_literal->value.getType() != Field::Types::String) return false;
