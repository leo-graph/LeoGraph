...
WriteBuffer &operator<<(WriteBuffer &out, const Expected &expected) {
  if (expected.variants.empty()) return out;

  if (expected.variants.size() == 1) return out << *expected.variants.begin();
...
void writeQueryWithHighlightedErrorPositions(WriteBuffer &out, const char *begin, const char *end,
...
void writeQueryAroundTheError(WriteBuffer &out, const char *begin, const char *end, bool hilite, const Token *positions_to_hilite,
                              size_t num_positions_to_hilite) {
  if (hilite) {
    out << ":\n\n";
    ...
    if (num_positions_to_hilite) {
      ...
      size_t show_bytes =
          UTF8::computeBytesBeforeWidth(reinterpret_cast<const UInt8 *>(example_begin), total_bytes, 0, SHOW_CHARS_ON_SYNTAX_ERROR);
      out << ": " << std::string(example_begin, show_bytes) << (show_bytes < total_bytes ? "... " : ". ");
    }
  }
}

void writeCommonErrorMessage(WriteBuffer &out, const char *begin, const char *end, Token last_token, const std::string &query_description) {
  out << "Syntax error";

  if (!query_description.empty()) out << " (" << query_description << ")";
  ...
  if (last_token.type == TokenType::EndOfStream || last_token.type == TokenType::Semicolon) {
    ...
    size_t token_preview_size_bytes =
        UTF8::computeBytesBeforeWidth(reinterpret_cast<const UInt8 *>(last_token.begin), token_size_bytes, 0, SHOW_CHARS_ON_SYNTAX_ERROR);

    out << " (" << std::string(last_token.begin, token_preview_size_bytes) << (token_preview_size_bytes < token_size_bytes ? "..." : "")
        << ")";
  }
...
std::string getSyntaxErrorMessage(const char *begin, const char *end, Token last_token, const Expected &expected, bool hilite,
  ...
  if (!expected.variants.empty()) out << "Expected " << expected;

  return out.str();
}

std::string getLexicalErrorMessage(const char *begin, const char *end, Token last_token, bool hilite,
                                   const std::string &query_description) {
  WriteBufferFromOwnString out;
...
std::string getUnmatchedParenthesesErrorMessage(const char *begin, const char *end, const UnmatchedParentheses &unmatched_parens,
                                                bool hilite, const std::string &query_description) {
  WriteBufferFromOwnString out;
  ...
  for (const Token &paren : unmatched_parens) out << *paren.begin;
...
const char *getInsertData(const ASTPtr &ast) {
  if (const ASTInsertQuery *insert = getInsertAST(ast)) return insert->data;
  return nullptr;
}

ASTPtr tryParseQuery(IParser &parser, const char *&_out_query_end, /* also query begin as input parameter */
                     const char *all_queries_end, std::string &out_error_message, bool hilite, const std::string &query_description,
                     bool allow_multi_statements, size_t max_query_size, size_t max_parser_depth, size_t max_parser_backtracks,
  ...
  if (res && max_parser_depth) res->checkDepth(max_parser_depth);
  ...
  /// most of the checks.
  if (res && getInsertData(res)) return res;

  // More granular checks for queries other than INSERT w/inline data.
  /// Lexical error
  if (last_token.isError()) {
    out_error_message = getLexicalErrorMessage(query_begin, all_queries_end, last_token, hilite, query_description);
    return nullptr;
  }
  ...
  if (!unmatched_parens.empty()) {
    out_error_message = getUnmatchedParenthesesErrorMessage(query_begin, all_queries_end, unmatched_parens, hilite, query_description);
    return nullptr;
  }

  IParser::Pos this_query_end_pos = token_iterator;
  while (!this_query_end_pos->isEnd() && !this_query_end_pos->isError() && this_query_end_pos->type != TokenType::Semicolon)
    ++this_query_end_pos;

  if (!parse_res) {
    /// Generic parse error.
    out_error_message = getSyntaxErrorMessage(query_begin, this_query_end_pos->end, last_token, expected, hilite, query_description);
    return nullptr;
  }
  ...
  if (!allow_multi_statements && !token_iterator->isEnd()) {
    out_error_message =
        getSyntaxErrorMessage(query_begin, all_queries_end, last_token, {}, hilite,
...
ASTPtr parseQueryAndMovePosition(IParser &parser, const char *&pos, const char *end, const std::string &query_description,
                                 bool allow_multi_statements, size_t max_query_size, size_t max_parser_depth,
  ...
  ASTPtr res = tryParseQuery(parser, pos, end, error_message, false, query_description, allow_multi_statements, max_query_size,
                             max_parser_depth, max_parser_backtracks, true);
...
ASTPtr parseQuery(IParser &parser, const char *begin, const char *end, const std::string &query_description, size_t max_query_size,
                  size_t max_parser_depth, size_t max_parser_backtracks) {
...
ASTPtr parseQuery(IParser &parser, const std::string &query, const std::string &query_description, size_t max_query_size,
                  size_t max_parser_depth, size_t max_parser_backtracks) {
  return parseQuery(parser, query.data(), query.data() + query.size(), query_description, max_query_size, max_parser_depth,
                    max_parser_backtracks);
}

ASTPtr parseQuery(IParser &parser, const std::string &query, size_t max_query_size, size_t max_parser_depth, size_t max_parser_backtracks) {
  return parseQuery(parser, query.data(), query.data() + query.size(), parser.getName(), max_query_size, max_parser_depth,
                    max_parser_backtracks);
}

std::pair<const char *, bool> splitMultipartQuery(const std::string &queries, std::vector<std::string> &queries_list, size_t max_query_size,
                                                  size_t max_parser_depth, size_t max_parser_backtracks,
                                                  bool allow_settings_after_format_in_insert, bool implicit_select) {
  ASTPtr ast;
