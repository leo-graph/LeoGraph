 ...
 public:
  class ArgumentAST : public Argument {
   public:
    ...
    std::unique_ptr<AbstractFunction> getFunction() const override {
      if (const auto* f = argument->as<ASTFunction>()) return std::make_unique<FunctionAST>(*f);
      return nullptr;
    }
    ...
    bool tryGetString(String* res, bool allow_identifier) const override {
      if (const auto* literal = argument->as<ASTLiteral>()) {
        if (literal->value.getType() != Field::Types::String) return false;
        if (res) *res = literal->value.safeGet<String>();
        return true;
      }
  ...
  class ArgumentsAST : public Arguments {
   public:
    ...
    std::unique_ptr<Argument> at(size_t n) const override { return std::make_unique<ArgumentAST>(arguments->at(n).get()); }
