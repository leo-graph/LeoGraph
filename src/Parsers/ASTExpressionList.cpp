...
void ASTExpressionList::formatImpl(WriteBuffer& ostr, const FormatSettings& settings, FormatState& state, FormatStateStacked frame) const {
  if (frame.expression_list_prepend_whitespace) ostr << ' ';

  for (size_t i = 0, size = children.size(); i < size; ++i) {
    if (i) {
      if (char sep = getSeparator()) ostr << sep;
      ostr << ' ';
    }
    ...
    if (frame.surround_each_list_element_with_parens) ostr << "(";
...
void ASTExpressionList::formatImplMultiline(WriteBuffer& ostr, const FormatSettings& settings, FormatState& state,
                                            FormatStateStacked frame) const {
  ++frame.indent;
  std::string indent_str = "\n" + std::string(4 * frame.indent, ' ');

  if (frame.expression_list_prepend_whitespace) {
    if (!(children.size() > 1 || frame.expression_list_always_start_on_new_line)) ostr << ' ';
  }

  for (size_t i = 0, size = children.size(); i < size; ++i) {
    if (char sep = getSeparator(); i && sep) ostr << sep;

    if (size > 1 || frame.expression_list_always_start_on_new_line) ostr << indent_str;
