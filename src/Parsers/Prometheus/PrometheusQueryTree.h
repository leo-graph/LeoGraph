 ...
 public:
  ...
  class InstantSelector : public Node {
   public:
    ...
    InstantSelector() {
      node_type = NodeType::InstantSelector;
      result_type = ResultType::INSTANT_VECTOR;
    }
    Node *clone(std::vector<std::unique_ptr<Node>> &node_list_) const override;
    String dumpNode(const PrometheusQueryTree &tree, size_t indent) const override;
    String toString(const PrometheusQueryTree &tree) const override;
  ...
  class RangeSelector : public Node {
   public:
    ...
    RangeSelector() {
      node_type = NodeType::RangeSelector;
  ...
  bool tryParse(std::string_view promql_query_, UInt32 timestamp_scale_ = 3, String *error_message_ = nullptr,
                size_t *error_pos_ = nullptr);
