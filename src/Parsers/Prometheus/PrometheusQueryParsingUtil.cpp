...
void setErrorMessage(String* error_message, fmt::format_string<Types...> format, Types&&... args) {
  if (error_message) *error_message = fmt::format(format, std::forward<Types>(args)...);
}

void setErrorPos(size_t* error_pos, size_t value) {
  if (error_pos) *error_pos = value;
}
...
bool tryUnescapeStringLiteral(std::string_view input, String& res_string, String* error_message, size_t* error_pos) {
  ...
  for (size_t pos = 0; pos < input.length();) {
    ...
    if (next_pos == String::npos) next_pos = input.length();
    ...
    if (pos + 2 > input.length()) {
      setErrorMessage(error_message, "Invalid escape sequence {}: Expected at least 2 characters", quoteString(input.substr(pos)));
    ...
    switch (c) {
      case 'a':
        res_string.push_back(0x07);
        pos += 2;
        break;  /// \a  U+0007 alert or bell
      case 'b':
        res_string.push_back(0x08);
        pos += 2;
        break;  /// \b  U+0008 backspace
      case 'f':
        res_string.push_back(0x0C);
        pos += 2;
        break;  /// \f  U+000C form feed
      case 'n':
        res_string.push_back(0x0A);
        pos += 2;
        break;  /// \n  U+000A line feed or newline
      case 'r':
        res_string.push_back(0x0D);
        pos += 2;
        break;  /// \r  U+000D carriage return
      case 't':
        res_string.push_back(0x09);
        pos += 2;
        break;  /// \t  U+0009 horizontal tab
      case 'v':
        res_string.push_back(0x0B);
        pos += 2;
        break;  /// \v  U+000B vertical tab
      case '\\':
        res_string.push_back('\\');
        pos += 2;
        break;  /// \\  U+005C backslash
      case '\'':
        res_string.push_back('\'');
        pos += 2;
        break;  /// \'  U+0027 single quote
      case '"':
        res_string.push_back('"');
        pos += 2;
        break;  /// \"  U+0022 double quote
      case 'x': {
        ...
        if (!tryParseIntInBase<16>(byte, input.substr(pos + 2, 2))) {
          setErrorMessage(error_message, "Invalid escape sequence {}: Cannot parse a hexadecimal number representing a single byte",
      ...
      case '7': {
        ...
        if (!tryParseIntInBase<8>(byte, input.substr(pos + 1, 3))) {
          setErrorMessage(error_message, "Invalid escape sequence {}: Cannot parse an octal number representing a single byte",
                          quoteString(input.substr(pos, 4)));
          setErrorPos(error_pos, pos);
          return false;
        }
        if (byte > 0xFF) {
          setErrorMessage(error_message, "Invalid escape sequence {}: An octal representation \nnn must represent a single byte",
                          quoteString(input.substr(pos, 4)));
          setErrorPos(error_pos, pos);
          return false;
        }
      ...
      case 'u': {
        ...
        if (!tryParseIntInBase<16>(code_point, input.substr(pos + 2, 4))) {
          setErrorMessage(error_message, "Invalid escape sequence {}: Cannot parse a hexadecimal number representing a Unicode code point",
      ...
      case 'U': {
        ...
        if (pos + 10 > input.length()) {
          setErrorMessage(error_message, "Invalid escape sequence {}: Expected 10 characters", quoteString(input.substr(pos)));
          setErrorPos(error_pos, pos);
          return false;
        }
        UInt32 code_point;
        ...
        {
          setErrorMessage(error_message, "Invalid escape sequence {}: A Unicode code point can't be greater than 0x10FFFF",
                          quoteString(input.substr(pos, 10)));
          setErrorPos(error_pos, pos);
          return false;
        }
...
bool PrometheusQueryParsingUtil::tryParseStringLiteral(std::string_view input, String& res_string, String* error_message,
                                                       size_t* error_pos) {
  res_string.clear();

  if (!input.starts_with('\'') && !input.starts_with('\"') && !input.starts_with('`')) {
    setErrorMessage(error_message,
                    "Cannot parse string literal {}: A string literal must open with a quote ', a double quote \" or a backtick `", input);
...
bool tryParseHexFormat(std::string_view input, UInt32 scale, T& result, String* error_message, size_t* error_pos) {
  ...
  if constexpr (is_decimal<T>) {
    if (common::mulOverflow(value, DecimalUtils::scaleMultiplier<T>(scale), result.value)) {
      setErrorMessage(error_message, "Cannot parse {} {} in hexadecimal format: Overflow, the number is too big", getTypeName<T>(),
                      quoteString(input));
      setErrorPos(error_pos, 0);
      return false;
    }
...
bool tryParseDurationFormat(std::string_view input, UInt32 scale, T& result, String* error_message, size_t* error_pos) {
  ...
  while (pos != input.length()) {
    ...
    if (pos == number_start_pos) {
      setErrorMessage(error_message, "Cannot parse {} {} in duration format: Expected a number combined with a time unit, got {}",
    ...
    if (!tryParse(number, number_as_str)) {
      setErrorMessage(error_message, "Cannot parse {} {} in duration format: Overflow, the number {} is too big", getTypeName<T>(),
                      quoteString(input), number_as_str);
    ...
    if (seconds_per_unit) {
      if (!DecimalUtils::tryMultiplyAdd(number, seconds_per_unit, seconds, seconds)) {
        setErrorMessage(error_message, "Cannot parse {} {} in duration format: Overflow, the duration is too big", getTypeName<T>(),
                        quoteString(input));
        setErrorPos(error_pos, 0);
        return false;
      }
  ...
  if (!has_time_units) {
    setErrorMessage(error_message, "Cannot parse {} {} in duration format: Expected numbers combined with time units", getTypeName<T>(),
                    quoteString(input));
    setErrorPos(error_pos, 0);
    return false;
  }
  ...
  if constexpr (is_decimal<T>) {
    ...
    if (scale > 3) {
      if (common::mulOverflow(milliseconds, DecimalUtils::scaleMultiplier<T>(scale - 3), scaled_milliseconds)) {
        setErrorMessage(error_message, "Cannot parse {} {}: Overflow, the duration is too big", getTypeName<T>(), quoteString(input));
        setErrorPos(error_pos, 0);
        return false;
      }
    } else if (scale < 3) {
...
bool tryParseNumber(std::string_view input, UInt32 scale, T& result, String* error_message, size_t* error_pos) {
  ...
  while (end_pos != pos && std::isspace(input[end_pos - 1])) --end_pos;
...
bool PrometheusQueryParsingUtil::tryParseTimestamp(std::string_view input, UInt32 timestamp_scale, TimestampType& res_timestamp,
                                                   String* error_message, size_t* error_pos) {
  return tryParseNumber(input, timestamp_scale, res_timestamp, error_message, error_pos);
}

bool PrometheusQueryParsingUtil::tryParseDuration(std::string_view input, UInt32 timestamp_scale, DurationType& res_duration,
                                                  String* error_message, size_t* error_pos) {
...
bool PrometheusQueryParsingUtil::tryParseSelectorRange(std::string_view input, UInt32 timestamp_scale, DurationType& res_range,
                                                       String* error_message, size_t* error_pos) {
  ...
  if (!tryParseDuration(input.substr(start_pos, end_pos - start_pos), timestamp_scale, res_range, error_message, error_pos)) {
    if (error_pos) *error_pos += start_pos;
    return false;
  }
...
bool PrometheusQueryParsingUtil::tryParseSubqueryRange(std::string_view input, UInt32 timestamp_scale, DurationType& res_range,
                                                       std::optional<DurationType>& res_step, String* error_message, size_t* error_pos) {
  /// Check opening and closing brackets.
  if (!input.starts_with('[')) {
  ...
  if (range_start_pos == range_end_pos) {
    setErrorMessage(error_message,
                    "Cannot parse time range {}: Expected a duration between opening bracket [ and colon :", quoteString(input));
    setErrorPos(error_pos, range_start_pos);
    return false;
  }

  if (!tryParseDuration(input.substr(range_start_pos, range_end_pos - range_start_pos), timestamp_scale, res_range, error_message,
                        error_pos)) {
    if (error_pos) *error_pos += range_start_pos;
    return false;
  }
  ...
  if (step_start_pos != step_end_pos) {
    if (!tryParseDuration(input.substr(step_start_pos, step_end_pos - step_start_pos), timestamp_scale, res_step.emplace(), error_message,
                          error_pos)) {
      if (error_pos) *error_pos += step_start_pos;
      return false;
    }
  }
