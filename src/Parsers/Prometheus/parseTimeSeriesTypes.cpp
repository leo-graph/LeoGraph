...
T getFromInt(Int64 int_value, UInt32 scale) {
  ...
  if (common::mulOverflow(int_value, DecimalUtils::scaleMultiplier<T>(scale), result.value)) {
    throw Exception(ErrorCodes::DECIMAL_OVERFLOW, "Cannot convert {} to {}: Overflow, the number is too big", int_value, getTypeName<T>());
  }
  return result;
}
...
T getFromFloat(Float64 float_value, UInt32 scale) {
  ...
  if ((scaled_value > static_cast<Float64>(std::numeric_limits<typename T::NativeType>::max())) ||
    ...
    throw Exception(ErrorCodes::DECIMAL_OVERFLOW, "Cannot convert {} to {}: Overflow, the number is too big", float_value,
                    getTypeName<T>());
  }
...
T getFromIntervalKind(Int64 intervals, IntervalKind interval_kind, UInt32 scale) {
  ...
  switch (interval_kind.kind) {
    ...
    default: {
      throw Exception(ErrorCodes::BAD_ARGUMENTS, "Cannot extract a {} from an interval of type {}", getTypeName<T>(),
                      interval_kind.toString());
    }
  }

  T result;
  if (common::mulOverflow(intervals, unit_multiplier, result.value) || common::mulOverflow(result.value, scale_multiplier, result.value)) {
    throw Exception(ErrorCodes::DECIMAL_OVERFLOW, "Cannot convert {} {}s to {}: Overflow, the number is too big", intervals,
                    interval_kind.toString(), getTypeName<T>());
  }
...
T parseFromString(std::string_view str, UInt32 scale) {
  ...
  if constexpr (std::is_same_v<T, DateTime64>) {
    if (PrometheusQueryParsingUtil::tryParseTimestamp(str, scale, result, &error_message, &error_pos)) return result;

    ReadBufferFromString buf{str};
    if (tryReadDateTime64Text(result, scale, buf)) return result;
  } else {
    if (PrometheusQueryParsingUtil::tryParseDuration(str, scale, result, &error_message, &error_pos)) return result;
  }

  throw Exception(ErrorCodes::BAD_ARGUMENTS, "Cannot parse a {} from string {}: {}", getTypeName<T>(), quoteString(str), error_message);
}
...
T getFromField(const Field &field, const DataTypePtr &field_data_type, UInt32 scale) {
  switch (field.getType()) {
    ...
    default: {
      throw Exception(ErrorCodes::BAD_ARGUMENTS, "Cannot extract a {} from a field of type {}", getTypeName<T>(), field.getType());
    }
  }
}
}  // namespace

DateTime64 parseTimeSeriesTimestamp(const String &str, UInt32 timestamp_scale) { return parseFromString<DateTime64>(str, timestamp_scale); }
...
Decimal64 parseTimeSeriesDuration(const String &str, UInt32 duration_scale) { return parseFromString<Decimal64>(str, duration_scale); }
