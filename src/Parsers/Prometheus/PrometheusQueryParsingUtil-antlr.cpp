...
class ErrorListener : public antlr4::BaseErrorListener {
 ...
 protected:
  void syntaxError(antlr4::Recognizer * /* recognizer */, antlr4::Token *offending_symbol, size_t line, size_t position_in_line,
                   const std::string &msg, std::exception_ptr /* exception */) override {
    chassert(!msg.empty());
...
class PrometheusQueryTreeBuilder : public antlr4_grammars::PromQLParserBaseVisitor {
 public:
  ...
  Node *makeNode(antlr4::ParserRuleContext *expression) {
    ...
    if (Node *node = anyToNodePtr(any)) return node;
 ...
 private:
  ...
  Node *addNode(std::unique_ptr<Node> new_node) { return nodes.emplace_back(std::move(new_node)).get(); }
  ...
  bool parseSubqueryRange(const antlr4::tree::TerminalNode *ctx, DurationType &res_range, std::optional<DurationType> &res_step) {
    String error_message;
    size_t error_pos;
    if (!PrometheusQueryParsingUtil::tryParseSubqueryRange(getText(ctx), timestamp_scale, res_range, res_step, &error_message,
                                                           &error_pos)) {
      error_listener.setError(error_message, error_pos + getStartPos(ctx));
      return false;
    }
  ...
  Strings getLabelNameList(antlr4_grammars::PromQLParser::LabelNameListContext *ctx) const {
    ...
    for (size_t i = 0; (label_name_ctx = ctx->labelName(i)) != nullptr; ++i) label_name_list.push_back(getLabelName(label_name_ctx));
  ...
  bool getMatcher(antlr4_grammars::PromQLParser::LabelMatcherContext *ctx, Matcher &res_matcher) {
    ...
    if (!label_name_ctx || !label_value_ctx || !op_ctx) throwInconsistentSchema("LabelMatcher", ctx->getText());
  ...
  Node *makeInstantSelector(antlr4_grammars::PromQLParser::InstantSelectorContext *ctx) {
    ...
    if (auto *metric_name_ctx = ctx->metricName()) matchers.push_back(getMatcherForMetricName(metric_name_ctx));
  ...
  Node *makeRangeSelector(antlr4_grammars::PromQLParser::RangeSelectorContext *ctx) {
    ...
    if (!instant_selector_ctx || !selector_range_ctx) throwInconsistentSchema("RangeSelector", ctx->getText());
  ...
  Node *makeSubquery(antlr4_grammars::PromQLParser::SubqueryOpContext *ctx, Node *expression) {
    ...
    if (!subquery_range_ctx) throwInconsistentSchema("SubqueryOp", ctx->getText());
    ...
    if (auto *offset_op_ctx = ctx->offsetOp()) res_node = makeOffset(offset_op_ctx, res_node);
  ...
  Node *makeOffset(antlr4_grammars::PromQLParser::OffsetOpContext *ctx, Node *expression) {
    ...
    if (auto *timestamp_ctx = ctx->timestamp()) {
      ...
      if (!number_ctx) throwInconsistentSchema("OffsetOp", ctx->getText());
    ...
    if (auto *offset_value_ctx = ctx->offsetValue()) {
      ...
      if (ok && offset_value_ctx->SUB()) offset_value = -offset_value;
    }
  ...
  Node *makeBinaryOperator(std::string_view operator_name, Node *left_argument, Node *right_argument,
    ...
    if (grouping) {
      if (auto *on_ctx = grouping->on_()) {
        ...
        if (!labels_ctx) throwInconsistentSchema("Grouping", grouping->getText());
        new_node->on = true;
      ...
      if (auto *group_left_ctx = grouping->groupLeft()) {
        ...
        if (auto *extra_labels_ctx = group_left_ctx->labelNameList()) new_node->extra_labels = getLabelNameList(extra_labels_ctx);
      } else if (auto *group_right_ctx = grouping->groupRight()) {
        new_node->group_right = true;
        if (auto *extra_labels_ctx = group_right_ctx->labelNameList()) new_node->extra_labels = getLabelNameList(extra_labels_ctx);
      }
    }
  ...
  Node *makeFunction(std::string_view function_name, const std::vector<Node *> &arguments) {
    ...
    for (auto *argument : arguments) addChild(new_node.get(), argument);
  ...
  Node *makeFunction(antlr4_grammars::PromQLParser::Function_Context *ctx, const std::vector<Node *> &arguments) {
    ...
    if (!function_name_ctx) throwInconsistentSchema("Function", ctx->getText());
  ...
  Node *makeAggregationOperator(std::string_view operator_name, const std::vector<Node *> &arguments,
                                antlr4_grammars::PromQLParser::ByContext *by, antlr4_grammars::PromQLParser::WithoutContext *without) {
    ...
    if (by) {
      ...
      if (!labels_ctx) throwInconsistentSchema("By", by->getText());
      new_node->labels = getLabelNameList(labels_ctx);
    } else if (without) {
      new_node->without = true;
      auto *labels_ctx = without->labelNameList();
      if (!labels_ctx) throwInconsistentSchema("Without", without->getText());
      new_node->labels = getLabelNameList(labels_ctx);
    }
  ...
  Node *makeAggregationOperator(antlr4_grammars::PromQLParser::AggregationContext *ctx, const std::vector<Node *> &arguments) {
    ...
    if (!operator_name_ctx) throwInconsistentSchema("Aggregation", ctx->getText());
  ...
  std::any visitInstantSelector(antlr4_grammars::PromQLParser::InstantSelectorContext *ctx) override { return makeInstantSelector(ctx); }

  std::any visitRangeSelector(antlr4_grammars::PromQLParser::RangeSelectorContext *ctx) override { return makeRangeSelector(ctx); }
  ...
  std::any visitSelectorWithOffset(antlr4_grammars::PromQLParser::SelectorWithOffsetContext *ctx) override {
    ...
    if (!offset_op_ctx) throwInconsistentSchema("Offset", ctx->getText());
  ...
  std::any visitAggregation(antlr4_grammars::PromQLParser::AggregationContext *ctx) override {
    ...
    if (!parameter_list_ctx) throwInconsistentSchema("Aggregation", ctx->getText());
  ...
  static Node *anyToNodePtr(std::any any) {
    ...
    if (!node_ptr) throw Exception(ErrorCodes::LOGICAL_ERROR, "PrometheusQueryTreeBuilder: got {} and not Node *", any.type().name());
...
bool PrometheusQueryParsingUtil::tryParseQuery([[maybe_unused]] std::string_view input, [[maybe_unused]] UInt32 timestamp_scale,
                                               [[maybe_unused]] PrometheusQueryTree &res_query, [[maybe_unused]] String *error_message,
  ...
  if (!error_listener.hasError()) expression = promql_parser.expression();

  if (!expression) error_listener.setError("Couldn't get an expression after parsing promql query", 0);
  ...
  if (error_listener.hasError()) {
    if (error_message) *error_message = error_listener.getErrorMessage();
    if (error_pos) *error_pos = error_listener.getErrorPos();
    return false;
  }

  if (!parsed_root) throw Exception(ErrorCodes::LOGICAL_ERROR, "Parsing promql query '{}' failed without setting any error message", input);
