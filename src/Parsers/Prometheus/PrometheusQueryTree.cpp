...
Node *PrometheusQueryTree::Scalar::clone(std::vector<std::unique_ptr<Node>> &node_list_) const { return cloneNodeImpl(this, node_list_); }
...
Node *PrometheusQueryTree::Subquery::clone(std::vector<std::unique_ptr<Node>> &node_list_) const { return cloneNodeImpl(this, node_list_); }

Node *PrometheusQueryTree::Offset::clone(std::vector<std::unique_ptr<Node>> &node_list_) const { return cloneNodeImpl(this, node_list_); }

Node *PrometheusQueryTree::Function::clone(std::vector<std::unique_ptr<Node>> &node_list_) const { return cloneNodeImpl(this, node_list_); }
...
PrometheusQueryTree &PrometheusQueryTree::operator=(const PrometheusQueryTree &src) {
  if (this != &src) {
    ...
    if (src.root) new_root = src.root->clone(new_node_list);
...
PrometheusQueryTree::PrometheusQueryTree(std::vector<std::unique_ptr<Node>> node_list_, const Node *root_, UInt32 timestamp_scale_)
    : node_list(std::move(node_list_)), root(root_), timestamp_scale(timestamp_scale_) {}

PrometheusQueryTree::PrometheusQueryTree(std::unique_ptr<Node> single_node_, UInt32 timestamp_scale_) : timestamp_scale(timestamp_scale_) {
...
PrometheusQueryResultType PrometheusQueryTree::getResultType() const {
  if (!root) throw Exception(ErrorCodes::LOGICAL_ERROR, "Prometheus query tree shouldn't be empty");
...
String PrometheusQueryTree::Function::dumpNode(const PrometheusQueryTree &tree, size_t indent) const {
  ...
  for (const auto *argument : arguments) str += fmt::format("\n{}", argument->dumpNode(tree, indent + 1));
  return str;
}
...
String PrometheusQueryTree::BinaryOperator::dumpNode(const PrometheusQueryTree &tree, size_t indent) const {
  ...
  if (group_left || group_right) {
    ...
    if (!extra_labels.empty()) joined_extra_labels += fmt::format(" {}", fmt::join(extra_labels, ", "));
...
String PrometheusQueryTree::AggregationOperator::dumpNode(const PrometheusQueryTree &tree, size_t indent) const {
  ...
  for (const auto *argument : getArguments()) str += fmt::format("\n{}", argument->dumpNode(tree, indent + 1));
  return str;
}
...
void PrometheusQueryTree::parse(std::string_view promql_query_, UInt32 timestamp_scale_) {
  ...
  size_t error_pos;
  if (PrometheusQueryParsingUtil::tryParseQuery(promql_query_, timestamp_scale_, *this, &error_message, &error_pos)) return;

  throw Exception(ErrorCodes::CANNOT_PARSE_PROMQL_QUERY, "{} at position {} while parsing PromQL query: {}", error_message, error_pos,
                  promql_query_);
}
...
namespace {
String quotePromQLString(std::string_view str) { return doubleQuoteString(str); }
}  // namespace
...
String PrometheusQueryTree::StringLiteral::toString(const PrometheusQueryTree &) const { return quotePromQLString(string); }
...
String PrometheusQueryTree::Subquery::toString(const PrometheusQueryTree &tree) const {
  ...
  str += DB::toString(range, tree.timestamp_scale);
  str += ":";
  if (step) str += DB::toString(*step, tree.timestamp_scale);
  str += "]";
...
int PrometheusQueryTree::BinaryOperator::getPrecedence() const {
  if (operator_name == "^") return 2;
  if ((operator_name == "*") || (operator_name == "/") || (operator_name == "%") || (operator_name == "atan2")) return 3;
  if ((operator_name == "+") || (operator_name == "-")) return 4;
  if ((operator_name == "==") || (operator_name == "!=") || (operator_name == "<") || (operator_name == ">") || (operator_name == "<=") ||
      (operator_name == ">="))
    return 5;
  if ((operator_name == "and") || (operator_name == "unless")) return 6;
  if (operator_name == "or") return 7;
