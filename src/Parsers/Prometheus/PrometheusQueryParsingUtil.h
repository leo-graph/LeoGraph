...
struct PrometheusQueryParsingUtil {
  ...
  static bool tryParseQuery(std::string_view input, UInt32 timestamp_scale, PrometheusQueryTree& res_query, String* error_message = nullptr,
  ...
  static bool tryParseStringLiteral(std::string_view input, String& res_string, String* error_message = nullptr,
                                    size_t* error_pos = nullptr);
  ...
  static bool tryParseScalar(std::string_view input, ScalarType& res_scalar, String* error_message = nullptr, size_t* error_pos = nullptr);
  ...
  /// Also underscores (_) can be used in between decimal or hexadecimal digits and they don't mean anything.
  static bool tryParseTimestamp(std::string_view input, UInt32 timestamp_scale, TimestampType& res_timestamp,
                                String* error_message = nullptr, size_t* error_pos = nullptr);
  ...
  /// Also underscores (_) can be used in between decimal or hexadecimal digits and they don't mean anything.
  static bool tryParseDuration(std::string_view input, UInt32 timestamp_scale, DurationType& res_duration, String* error_message = nullptr,
                               size_t* error_pos = nullptr);

  /// Parses the range in a range selector, for example for "[1h30m]" the function parses "1h30m".
  static bool tryParseSelectorRange(std::string_view input, UInt32 timestamp_scale, DurationType& res_range,
                                    String* error_message = nullptr, size_t* error_pos = nullptr);

  /// Parses the range and optionally the step in a subquery, for example for "[1h:5m]" the function parses "1h" and "5m".
  static bool tryParseSubqueryRange(std::string_view input, UInt32 timestamp_scale, DurationType& res_range,
                                    std::optional<DurationType>& res_step, String* error_message = nullptr, size_t* error_pos = nullptr);
};
