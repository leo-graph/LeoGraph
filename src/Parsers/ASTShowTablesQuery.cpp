...
ASTPtr ASTShowTablesQuery::clone() const {
  ...
  res->children.clear();
  if (from) res->set(res->from, from->clone());
...
void ASTShowTablesQuery::formatLimit(WriteBuffer &ostr, const FormatSettings &settings, FormatState &state,
...
void ASTShowTablesQuery::formatQueryImpl(WriteBuffer &ostr, const FormatSettings &settings, FormatState &state,
                                         FormatStateStacked frame) const {
  if (databases) {
    ostr << "SHOW DATABASES";
    ...
    ostr << "SHOW " << (temporary ? "TEMPORARY " : "") << (dictionaries ? "DICTIONARIES" : "TABLES");
