...
class KeyWordToStringConverter {
 public:
  ...
  std::string_view convert(Keyword type) const { return mapping[static_cast<size_t>(type)]; }

  const std::vector<String>& getMapping() const { return mapping; }
...
#define KEYWORD_TYPE_TO_STRING_CONVERTER_ADD_TO_MAPPING(identifier, value) addToMapping(Keyword::identifier, value);
    APPLY_FOR_PARSER_KEYWORDS_WITH_UNDERSCORES(KEYWORD_TYPE_TO_STRING_CONVERTER_ADD_TO_MAPPING)
#undef KEYWORD_TYPE_TO_STRING_CONVERTER_ADD_TO_MAPPING

#ifndef NDEBUG
#  define KEYWORD_TYPE_TO_STRING_CONVERTER_ADD_TO_MAPPING(identifier, value) check(#identifier, value);
    APPLY_FOR_PARSER_KEYWORDS(KEYWORD_TYPE_TO_STRING_CONVERTER_ADD_TO_MAPPING)
    APPLY_FOR_PARSER_KEYWORDS_WITH_UNDERSCORES(KEYWORD_TYPE_TO_STRING_CONVERTER_ADD_TO_MAPPING)
#  undef KEYWORD_TYPE_TO_STRING_CONVERTER_ADD_TO_MAPPING
#endif
  ...
  [[maybe_unused]] void check(std::string_view identifier, std::string_view value) {
    ...
    for (size_t i = 0; i < identifier.size(); ++i) {
      if (std::tolower(identifier[i]) == '_' && std::tolower(value[i]) == ' ') continue;
      if (std::tolower(identifier[i]) == std::tolower(value[i])) continue;
      throw Exception(ErrorCodes::LOGICAL_ERROR, "Keyword identifier {} differs from its value {} in {} position: {} and {}", identifier,
                      value, i, identifier[i], value[i]);
    }
  }
...
std::string_view toStringView(Keyword type) { return KeyWordToStringConverter::instance().convert(type); }

const std::vector<String>& getAllKeyWords() { return KeyWordToStringConverter::instance().getMapping(); }

const std::unordered_set<std::string>& getKeyWordSet() { return KeyWordToStringConverter::instance().getSet(); }

ParserKeyword::ParserKeyword(Keyword keyword) : s(toStringView(keyword)) {}
...
bool ParserKeyword::parseImpl(Pos& pos, [[maybe_unused]] ASTPtr& node, Expected& expected) {
  ...
  while (true) {
    ...
    if (word_length != pos->size()) return false;

    if (0 != strncasecmp(pos->begin, current_word, word_length)) return false;

    ++pos;

    if (!*next_whitespace) break;
