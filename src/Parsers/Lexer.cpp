...
template <char quote>
Token quotedString(const char*& pos, const char* const token_begin, const char* const end, TokenType success_token, TokenType error_token) {
  ++pos;
  while (true) {
    pos = find_first_symbols<quote, '\\'>(pos, end);
    if (pos >= end) return Token(error_token, token_begin, end);
...
Token quotedStringWithUnicodeQuotes(const char*& pos, const char* const token_begin, const char* const end, char expected_end_byte,
  ...
  while (true) {
    ...
    if (pos + 2 >= end) return Token(error_token, token_begin, end);
...
Token quotedHexOrBinString(const char*& pos, const char* const token_begin, const char* const end) {
  ...
  if (hex) {
    while (pos < end && isHexDigit(*pos)) ++pos;
  } else {
...
Token Lexer::nextToken() {
  ...
  if (max_query_size && res.end > begin + max_query_size) res.type = TokenType::ErrorMaxQuerySizeExceeded;
  if (res.isSignificant()) prev_significant_token_type = res.type;
  return res;
}

Token Lexer::nextTokenImpl() {
  if (pos >= end) return Token(TokenType::EndOfStream, end, end);
  ...
  switch (*pos) {
    ...
    case '9': {
      ...
      if (prev_significant_token_type == TokenType::Dot) {
        ++pos;
        while (pos < end && (isNumericASCII(*pos) || isNumberSeparator(false, false, pos, end))) ++pos;
      } else {
      ...
      if (pos < end && isWordCharASCII(*pos)) {
        ...
        for (const char* iterator = token_begin; iterator < pos; ++iterator) {
          if (!isWordCharASCII(*iterator) && *iterator != '$') return Token(TokenType::ErrorWrongNumber, token_begin, pos);
        }
    ...
    {
      ++pos;
      if (pos < end && *pos == '>') return Token(TokenType::Arrow, token_begin, ++pos);
      ...
      if (pos < end && (*pos == '/' || *pos == '*')) {
        ...
        while (pos + 2 <= end) {
          if (pos[0] == '/' && pos[1] == '*') {
            ...
            if (nesting_level == 0) return Token(TokenType::Comment, token_begin, pos);
          } else
            ++pos;
        }
        pos = end;
    ...
    {          /// PostgreSQL has some operators using '#' character.
      ...
      ++pos;
      if (pos < end && (*pos == ' ' || *pos == '!')) return comment_until_end_of_line();
    ...
    {
      ++pos;
      if (pos < end && *pos == '=') return Token(TokenType::NotEquals, token_begin, ++pos);
    ...
    case '<':  /// <, <=, <>, <=>
    {
      ...
      if (pos < end && *pos == '=') return Token(TokenType::LessOrEquals, token_begin, ++pos);
      if (pos < end && *pos == '>') return Token(TokenType::NotEquals, token_begin, ++pos);
    ...
    {
      ++pos;
      if (pos < end && *pos == '=') return Token(TokenType::GreaterOrEquals, token_begin, ++pos);
    ...
    case ':': {
      ++pos;
      if (pos < end && *pos == ':') return Token(TokenType::DoubleColon, token_begin, ++pos);
      return Token(TokenType::Colon, token_begin, pos);
    }
    case '|': {
      ++pos;
      if (pos < end && *pos == '|') return Token(TokenType::Concatenation, token_begin, ++pos);
      return Token(TokenType::PipeMark, token_begin, pos);
    }
    case '@': {
      ++pos;
      if (pos < end && *pos == '@') return Token(TokenType::DoubleAt, token_begin, ++pos);
      return Token(TokenType::At, token_begin, pos);
    }
    case '\\': {
      ++pos;
      if (pos < end && *pos == 'G') return Token(TokenType::VerticalDelimiter, token_begin, ++pos);
      return Token(TokenType::Error, token_begin, pos);
    }
    case '\xE2': {
    ...
    default:
      ...
      if (pos > token_begin) return Token(TokenType::Whitespace, token_begin, pos);

      ++pos;
      while (pos < end && UTF8::isContinuationOctet(*pos)) ++pos;
...
int clickhouse_lexer_token_is_error(unsigned char token) { return token > static_cast<unsigned char>(TokenType::EndOfStream); }

int clickhouse_lexer_token_is_end(unsigned char token) { return token == static_cast<unsigned char>(TokenType::EndOfStream); }
}
