...
[[nodiscard]] static bool parseQueryWithOnClusterAndMaybeTable(boost::intrusive_ptr<ASTSystemQuery> &res, IParser::Pos &pos,
  ...
  if (ParserKeyword{Keyword::ON}.ignore(pos, expected)) {
    if (!ASTQueryWithOnCluster::parse(pos, cluster, expected)) return false;
  ...
  if (!parsed_table) parsed_table = parseDatabaseAndTableAsAST(pos, expected, res->database, res->table);

  if (!parsed_table && require_table) return false;
  ...
  if (!children_already_added) {
    if (res->database) res->children.push_back(res->database);
    if (res->table) res->children.push_back(res->table);
  }
...
[[nodiscard]] static bool parseQueryWithOnClusterAndTarget(boost::intrusive_ptr<ASTSystemQuery> &res, IParser::Pos &pos, Expected &expected,
                                                           SystemQueryTargetType target_type) {
  ...
  if (ParserStringLiteral{}.parse(pos, temporary_string_literal, expected)) {
    ...
    if (!identifier_parser.parse(pos, identifier, expected)) return false;

    if (!tryGetIdentifierNameInto(identifier, target)) return false;
  }
...
[[nodiscard]] static bool parseQueryWithOnCluster(boost::intrusive_ptr<ASTSystemQuery> &res, IParser::Pos &pos, Expected &expected) {
  String cluster_str;
  if (ParserKeyword{Keyword::ON}.ignore(pos, expected)) {
...
[[nodiscard]] static bool parseDropReplica(boost::intrusive_ptr<ASTSystemQuery> &res, IParser::Pos &pos, Expected &expected,
                                           bool database) {
  if (!parseQueryWithOnCluster(res, pos, expected)) return false;

  ASTPtr ast;
  if (!ParserStringLiteral{}.parse(pos, ast, expected)) return false;
  ...
  if (ParserKeyword{Keyword::FROM}.ignore(pos, expected)) {
    ...
    if (ParserKeyword{Keyword::DATABASE}.ignore(pos, expected)) {
      ...
      if (!database_parser.parse(pos, res->database, expected)) return false;
      ...
      ASTPtr path_ast;
      if (!ParserStringLiteral{}.parse(pos, path_ast, expected)) return false;
      String zk_path = path_ast->as<ASTLiteral &>().value.safeGet<String>();
      if (!zk_path.empty() && zk_path[zk_path.size() - 1] == '/') zk_path.pop_back();
      if (!zk_path.empty()) {
    ...
    if (database && ParserKeyword{Keyword::WITH_TABLES}.ignore(pos, expected)) res->with_tables = true;
  } else
...
bool ParserSystemQuery::parseImpl(IParser::Pos &pos, ASTPtr &node, Expected &expected) {
  if (!ParserKeyword{Keyword::SYSTEM}.ignore(pos, expected)) return false;
  ...
  switch (res->type) {
    ...
    case Type::RELOAD_MODEL: {
      if (!parseQueryWithOnClusterAndTarget(res, pos, expected, SystemQueryTargetType::Model)) return false;
      break;
    }
    case Type::RELOAD_FUNCTION: {
      if (!parseQueryWithOnClusterAndTarget(res, pos, expected, SystemQueryTargetType::Function)) return false;
      break;
    }

    case Type::DROP_REPLICA: {
      if (!parseDropReplica(res, pos, expected, /* database */ false)) return false;
      break;
    }
    case Type::DROP_DATABASE_REPLICA: {
      if (!parseDropReplica(res, pos, expected, /* database */ true)) return false;
      break;
    }
    case Type::DROP_CATALOG_REPLICA: {
      if (!parseDropCatalogReplica(res, pos, expected)) return false;
      break;
    }
    case Type::ALLOCATE_MEMORY: {
      ASTPtr ast;
      if (!ParserUnsignedInteger().parse(pos, ast, expected)) return false;
    ...
    case Type::PREWARM_PRIMARY_INDEX_CACHE: {
      if (!parseQueryWithOnCluster(res, pos, expected)) return false;
      if (!parseDatabaseAndTableAsAST(pos, expected, res->database, res->table)) return false;
      if (res->type == Type::SYNC_REPLICA) {
        if (ParserKeyword{Keyword::IF_EXISTS}.ignore(pos, expected)) res->if_exists = true;
        ...
        if (ParserKeyword{Keyword::STRICT}.ignore(pos, expected))
          ...
          if (ParserKeyword{Keyword::FROM}.ignore(pos, expected)) {
            do {
              ASTPtr replica_ast;
              if (!ParserStringLiteral{}.parse(pos, replica_ast, expected)) return false;
    ...
    case Type::SYNC_DATABASE_REPLICA: {
      if (!parseQueryWithOnCluster(res, pos, expected)) return false;
      if (!parseDatabaseAsAST(pos, expected, res->database)) return false;
      if (ParserKeyword{Keyword::STRICT}.ignore(pos, expected)) res->sync_replica_mode = SyncReplicaMode::STRICT;
      break;
    }
    case Type::RESTART_DISK: {
      if (!parseQueryWithOnClusterAndTarget(res, pos, expected, SystemQueryTargetType::Disk)) return false;
      break;
    }
    ...
    case Type::FLUSH_DISTRIBUTED: {
      ...
      if (s_settings.ignore(pos, expected)) {
        ...
        if (!parser_settings.parse(pos, res->query_settings, expected)) return false;
      }
    ...
    case Type::START_MERGES: {
      ...
      auto parse_on_volume = [&]() -> bool {
        ...
        if (!ParserToken{TokenType::Dot}.ignore(pos, expected)) return false;
      ...
      if (ParserKeyword{Keyword::ON_VOLUME}.ignore(pos, expected)) {
        if (!parse_on_volume()) return false;
      } else {
        if (!parseQueryWithOnCluster(res, pos, expected)) return false;
        if (ParserKeyword{Keyword::ON_VOLUME}.ignore(pos, expected)) {
          if (!parse_on_volume()) return false;
      ...
      if (res->volume.empty() && res->storage_policy.empty()) parseDatabaseAndTableAsAST(pos, expected, res->database, res->table);
      break;
    }
    ...
    case Type::TEST_VIEW: {
      ...
      if (ParserKeyword{Keyword::UNSET_FAKE_TIME}.ignore(pos, expected)) break;

      if (!ParserKeyword{Keyword::SET_FAKE_TIME}.ignore(pos, expected)) return false;
      ASTPtr ast;
      if (!ParserStringLiteral{}.parse(pos, ast, expected)) return false;
    ...
    case Type::SUSPEND: {
      ...
      ASTPtr seconds;
      if (!(ParserKeyword{Keyword::FOR}.ignore(pos, expected) && ParserUnsignedInteger().parse(pos, seconds, expected) &&
    ...
    case Type::SYNC_FILESYSTEM_CACHE: {
      ParserLiteral path_parser;
      ASTPtr ast;
      if (path_parser.parse(pos, ast, expected)) res->filesystem_cache_name = ast->as<ASTLiteral>()->value.safeGet<String>();
      if (!parseQueryWithOnCluster(res, pos, expected)) return false;
      break;
    }
    ...
    case Type::STOP_LISTEN: {
      ...
      auto parse_server_type = [&](ServerType::Type &type, std::string &custom_name) -> bool {
        ...
        if (type == ServerType::Type::END) return false;
      ...
      if (!parse_server_type(base_type, base_custom_name)) return false;

      if (ParserKeyword{Keyword::EXCEPT}.ignore(pos, expected)) {
        if (base_type != ServerType::Type::QUERIES_ALL && base_type != ServerType::Type::QUERIES_DEFAULT &&
        ...
        while (true) {
          if (!exclude_type.empty() && !ParserToken(TokenType::Comma).ignore(pos, expected)) break;

          if (!parse_server_type(current_type, current_custom_name)) return false;

          exclude_type.insert(current_type);

          if (current_type == ServerType::Type::CUSTOM) exclude_custom_names.insert(current_custom_name);
        }
      }
    ...
    case Type::FLUSH_LOGS: {
      ...
      do {
        ...
        if (!s_dot.ignore(pos)) {
          ...
          ASTPtr table_second;
          if (!table_parser.parse(pos, table_second, expected)) return false;
    ...
    case Type::INSTRUMENT_ADD: {
      ...
      if (res->instrumentation_parameters.empty()) return false;
  ...
  if (res->database) res->children.push_back(res->database);
  if (res->table) res->children.push_back(res->table);
  if (res->query_settings) res->children.push_back(res->query_settings);
