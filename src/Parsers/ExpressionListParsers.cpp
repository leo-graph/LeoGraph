...
bool ParserList::parseImpl(Pos &pos, ASTPtr &node, Expected &expected) {
  ...
  auto parse_element = [&] {
    ASTPtr element;
    if (!elem_parser->parse(pos, element, expected)) return false;

    elements.push_back(std::move(element));
    return true;
  };

  if (!parseUtil(pos, expected, parse_element, *separator_parser, allow_empty)) return false;
...
bool ParserUnionList::parseImpl(Pos &pos, ASTPtr &node, Expected &expected) {
  ...
  if (!parseUtil(pos, parse_element, parse_separator)) return false;
...
enum class SubqueryFunctionType : uint8_t { NONE, ANY, ALL };
...
bool ParserLeftAssociativeBinaryOperatorList::parseImpl(Pos &pos, ASTPtr &node, Expected &expected) {
  ...
  while (true) {
    if (first) {
      ASTPtr elem;
      if (!elem_parser->parse(pos, elem, expected)) return false;
      ...
      for (it = operators; *it; it += 2)
        if (parseOperator(pos, *it, expected)) break;
       ...
       */
      if (it[0] == "["sv) {
        if (pos->type != TokenType::ClosingSquareBracket) return false;
        ++pos;
      }
...
ParserExpressionWithOptionalAlias::ParserExpressionWithOptionalAlias(bool allow_alias_without_as_keyword, bool is_table_function,
    ...
    : impl(std::make_unique<ParserWithOptionalAlias>(is_table_function
                                                         ? ParserPtr(std::make_unique<ParserTableFunctionExpression>())
...
bool ParserGroupingSetsExpressionListElements::parseImpl(Pos &pos, ASTPtr &node, Expected &expected) {
  ...
  do {
    ...
    if (!s_open.ignore(pos, expected)) {
      ...
      if (!p_command.parse(pos, command, expected)) return false;

      if (!s_close.ignore(pos, expected)) break;
    }
...
bool ParserKeyValuePair::parseImpl(Pos &pos, ASTPtr &node, Expected &expected) {
  ...
  if (!id_parser.parse(pos, identifier, expected)) return false;
  ...
  if (!func_parser.parse(pos, value, expected) && !literal_parser.parse(pos, value, expected) && !id_parser.parse(pos, value, expected)) {
    ...
    if (!open.ignore(pos, expected)) return false;

    if (!kv_pairs_list.parse(pos, value, expected)) return false;

    if (!close.ignore(pos, expected)) return false;
...
enum class Action : uint8_t { NONE, OPERAND, OPERATOR };
...
enum class Checkpoint : uint8_t { None, Interval, Case };
...
class Layer {
 public:
  ...
  OperatorType previousType() const {
    if (operators.empty()) return OperatorType::None;
  ...
  bool popLastNOperands(ASTs &asts, size_t n) {
    if (n > operands.size()) return false;
  ...
  bool mergeElement(bool push_to_elements = true) {
    ...
    while (popOperator(cur_op)) {
      ...
      if (cur_op.type == OperatorType::StartIf || cur_op.type == OperatorType::StartBetween || cur_op.type == OperatorType::StartNotBetween)
        return false;

      if (cur_op.type == OperatorType::FinishIf) {
        Operator tmp;
        if (!popOperator(tmp) || tmp.type != OperatorType::StartIf) return false;
      }

      if (cur_op.type == OperatorType::FinishBetween) {
        Operator tmp_op;
        if (!popOperator(tmp_op)) return false;

        if (tmp_op.type != OperatorType::StartBetween && tmp_op.type != OperatorType::StartNotBetween) return false;

        bool negative = tmp_op.type == OperatorType::StartNotBetween;

        ASTs arguments;
        if (!popLastNOperands(arguments, 3)) return false;
        ...
        if (!popLastNOperands(function->children[0]->children, cur_op.arity)) return false;
      }
    ...
    ASTPtr node;
    if (!popOperand(node)) return false;
  ...
  bool parseLambda() {
    ...
    if (operands.size() != 1 || !operators.empty() || !mergeElement()) return false;
    ...
    for (const auto &child : args_ptr->children) {
      if (typeid_cast<ASTIdentifier *>(child.get())) continue;
  ...
  bool insertAlias(ASTPtr node) {
    if (!mergeElement(false)) return false;
...
static void highlightRegexps(const ASTPtr &node, Expected &expected, size_t depth) {
  ...
  if (!expected.enable_highlighting) return;
  ...
  if (func->name == "like" || func->name == "notLike" || func->name == "ilike" || func->name == "notILike") {
    is_like = true;
  } else if (func->name == "match" || func->name == "extract" || func->name == "extractAll" || func->name == "extractGroups" ||
    ...
    for (const auto &arg : args->children) highlightRegexps(arg, expected, depth + 1);
    return;
  }

  if (args->children.size() < 2) return;

  auto *literal = args->children[1]->as<ASTLiteral>();

  if (!literal || literal->value.getType() != Field::Types::String) return;

  /// Look up token position from the map stored in Expected
  if (!expected.literal_token_map) return;

  auto it = expected.literal_token_map->find(literal);
  if (it == expected.literal_token_map->end()) return;
...
class ExpressionLayer : public Layer {
 public:
  explicit ExpressionLayer(bool is_table_function_, bool allow_trailing_commas_ = false) : Layer(false, false) {
  ...
  bool parse(IParser::Pos &pos, Expected & /*expected*/, Action & /*action*/) override {
    if (pos->type == TokenType::Comma) {
      ...
      if (!allow_trailing_commas) return true;
      ...
      if (test_pos.isValid() && test_pos->type != TokenType::Semicolon) {
        ...
        if (!ParserKeyword(Keyword::FROM).ignore(test_pos, test_expected)) return true;

        // If there is a comma after 'from' then the first one was a name of a column
        if (test_pos->type == TokenType::Comma) return true;
        ...
        if (ParserAlias(false).ignore(test_pos, test_expected)) return true;
        ...
        for (; cur_op != ParserExpressionImpl::operators_table.end(); ++cur_op) {
          if (parseOperator(test_pos, cur_op->first, test_expected)) break;
        }

        if (cur_op != ParserExpressionImpl::operators_table.end()) return true;
      }
 ...
 public:
  ...
  bool parse(IParser::Pos &pos, Expected &expected, Action &action) override {
    ...
    if (state == 0) {
      ...
      if (all.ignore(pos, expected)) has_all = true;

      if (distinct.ignore(pos, expected)) has_distinct = true;

      if (!has_all && all.ignore(pos, expected)) has_all = true;

      if (has_all && has_distinct) return false;
    ...
    if (state == 2) {
      if (has_distinct) function_name += "Distinct";
      ...
      if (filter.ignore(pos, expected)) {
        ...
        if (!filter_parser.parse(pos, function_node_as_iast, expected)) return false;
      }
      ...
      if (over.ignore(pos, expected)) {
        ...
        if (!window_reference.parse(pos, function_node_as_iast, expected)) return false;
      }
      ...
      if (!is_tuple && elements.size() == 1) {
        ...
        if (auto *literal = elements[0]->as<ASTLiteral>()) {
          if (literal->value.getType() == Field::Types::Tuple && elements[0]->tryGetAlias().empty()) {
            ...
            for (auto &elem : tup) elements.push_back(make_intrusive<ASTLiteral>(std::move(elem)));
            is_tuple = true;
          }
        }

        // Special case for f(x, (y) -> z) = f(x, tuple(y) -> z)
        if (pos->type == TokenType::Arrow) is_tuple = true;
      }
 ...
 protected:
  bool getResultImpl(ASTPtr &node) override {
    ...
    if (!is_tuple && elements.size() == 1) {
      ...
      for (auto &elem : elements) tup.push_back(elem->as<ASTLiteral &>().value);
...
class ArrayLayer : public LayerWithSeparator<TokenType::Comma, TokenType::ClosingSquareBracket> {
 public:
  bool parse(IParser::Pos &pos, Expected &expected, Action &action) override { return LayerWithSeparator::parse(pos, expected, action); }
 ...
 protected:
  bool getResultImpl(ASTPtr &node) override {
    if (allElementsAreCompatibleLiterals(elements, Field::Types::Array)) {
      ...
      for (auto &elem : elements) arr.push_back(elem->as<ASTLiteral &>().value);
    ...
    if (state == 0) {
      ...
      if (as_keyword_parser.ignore(pos, expected)) {
        ...
        if (ParserIdentifier().parse(pos, alias, expected) && as_keyword_parser.ignore(pos, expected) &&
            ParserDataType().parse(pos, type_node, expected) && ParserToken(TokenType::ClosingRoundBracket).ignore(pos, expected)) {
          if (!insertAlias(alias)) return false;
...
class PositionLayer : public Layer {
 ...
 protected:
  bool getResultImpl(ASTPtr &node) override {
    if (state == 2 && elements.size() == 2) std::swap(elements[1], elements[0]);
...
class ExistsLayer : public Layer {
 public:
  ...
  bool parse(IParser::Pos &pos, Expected &expected, Action & /*action*/) override {
    ...
    // Recursion
    if (!ParserSelectWithUnionQuery().parse(pos, node, expected)) return false;

    if (!ParserToken(TokenType::ClosingRoundBracket).ignore(pos, expected)) return false;
  ...
  bool parse(IParser::Pos &pos, Expected &expected, Action &action) override {
    ...
    if (state == 0) {
      if (parseIntervalKind(pos, expected, interval_kind)) {
        if (!ParserToken(TokenType::Comma).ignore(pos, expected)) return false;
  ...
  bool getResultImpl(ASTPtr &node) override {
    if (parsed_interval_kind) {
      if (elements.size() == 2)
        ...
        node =
            makeASTFunction("dateDiff", make_intrusive<ASTLiteral>(interval_kind.toDateDiffUnit()), elements[0], elements[1], elements[2]);
      else
        return false;
    } else {
...
class TupleLayer : public LayerWithSeparator<TokenType::Comma, TokenType::ClosingRoundBracket> {
 public:
  bool parse(IParser::Pos &pos, Expected &expected, Action &action) override {
    ...
    if (finished && pos->type == TokenType::Arrow) return false;
  ...
  bool parse(IParser::Pos &pos, Expected &expected, Action &action) override {
    ...
    if (state == 0) {
      ...
      if (ParserStringLiteral{}.parse(pos, string_literal, expected) && string_literal->as<ASTLiteral &>().value.tryGet(literal)) {
        ...
        if (!ParserNumber{}.parse(token_pos, expr, token_expected)) return false;
        ...
        /// case: INTERVAL '1 HOUR'
        if (!parseIntervalKind(token_pos, token_expected, interval_kind)) return false;
        ...
        while (token_pos.isValid()) {
          if (!ParserNumber{}.parse(token_pos, expr, token_expected) || !parseIntervalKind(token_pos, token_expected, interval_kind))
            return false;
  ...
  bool parse(IParser::Pos &pos, Expected &expected, Action &action) override {
    ...
    if (state == 1) {
      if (ParserKeyword(Keyword::WHEN).ignore(pos, expected)) {
        if ((has_case_expr || !elements.empty()) && !mergeElement()) return false;
 ...
 public:
  ...
  bool parse(IParser::Pos &pos, Expected &expected, Action & /*action*/) override {
    ...
    if (state == 0) {
      ...
      if (!ParserSelectWithUnionQuery().parse(pos, query, expected)) return false;
      ...
      if (!ParserKeyword{Keyword::ELSE}.ignore(pos, expected)) return false;
 ...
 public:
  ...
  bool parse(IParser::Pos &pos, Expected &expected, Action & /*action*/) override {
    ...
    if (state == 0) {
      ASTPtr query;
      --pos;
      if (!ParserKQLTableFunction().parse(pos, query, expected)) return false;
      --pos;
      pushResult(query);
...
std::unique_ptr<Layer> getFunctionLayer(ASTPtr identifier, bool is_table_function, bool is_first_identifier,
  ...
  if (is_table_function) {
    if (function_name_lowercase == "view") return std::make_unique<ViewLayer>(false);
    if (function_name_lowercase == "viewifpermitted") return std::make_unique<ViewLayer>(true);
    if (function_name_lowercase == "kql") return std::make_unique<KustoLayer>();
  }

  if (function_name == "tuple") return std::make_unique<TupleLayer>();

  if (function_name_lowercase == "cast") return std::make_unique<CastLayer>();
  if (function_name_lowercase == "extract") return std::make_unique<ExtractLayer>();
  if (function_name_lowercase == "substring") return std::make_unique<SubstringLayer>();
  if (function_name_lowercase == "position") return std::make_unique<PositionLayer>();
  if (function_name_lowercase == "exists") return std::make_unique<ExistsLayer>();
  if (function_name_lowercase == "trim") return std::make_unique<TrimLayer>(false, false);
  if (function_name_lowercase == "ltrim") return std::make_unique<TrimLayer>(true, false);
  if (function_name_lowercase == "rtrim") return std::make_unique<TrimLayer>(false, true);
  ...
  if (function_name_lowercase == "grouping") return std::make_unique<FunctionLayer>(function_name_lowercase, allow_function_parameters_);
...
bool ParseCastExpression(IParser::Pos &pos, ASTPtr &node, Expected &expected) {
  ...
  if (ParserCastOperator().parse(pos, node, expected)) return true;

  pos = begin;

  /// As an exception, negative numbers should be parsed as literals, and not as an application of the operator.
  if (pos->type == TokenType::Minus) {
    if (ParserLiteral().parse(pos, node, expected)) return true;
  }
  return false;
}
...
bool ParseDateOperatorExpression(IParser::Pos &pos, ASTPtr &node, Expected &expected) {
  ...
  if (!ParserKeyword(Keyword::DATE).ignore(pos, expected)) return false;
...
bool ParseTimestampOperatorExpression(IParser::Pos &pos, ASTPtr &node, Expected &expected) {
  ...
  if (!ParserKeyword(Keyword::TIMESTAMP).ignore(pos, expected)) return false;
...
bool ParserExpression::parseImpl(Pos &pos, ASTPtr &node, Expected &expected) {
  ...
  if (expected.enable_highlighting && !expected.literal_token_map) expected.literal_token_map = &local_token_map;
...
bool ParserFunction::parseImpl(Pos &pos, ASTPtr &node, Expected &expected) {
  ...
  if (ParserFunctionName().parse(pos, identifier, expected) && ParserToken(TokenType::OpeningRoundBracket).ignore(pos, expected)) {
    ...
    if (ParserExpressionImpl().parse(std::move(start), pos, node, expected)) return true;
  }
...
bool ParserExpressionWithOptionalArguments::parseImpl(Pos &pos, ASTPtr &node, Expected &expected) {
  ...
  if (ParserFunction(false, false).parse(pos, node, expected)) return true;
...
bool ParserExpressionImpl::parse(std::unique_ptr<Layer> start, IParser::Pos &pos, ASTPtr &node, Expected &expected) {
  ...
  while (true) {
    while (pos.isValid()) {
      if (!layers.back()->parse(pos, expected, next)) break;

      if (layers.back()->isFinished()) {
        if (layers.size() == 1) break;

        next = Action::OPERATOR;

        ASTPtr res;
        if (!layers.back()->getResult(res)) break;
      ...
      if (next == Action::NONE) break;
    }

    /// When we exit the loop we should be on the 1st level
    if (layers.size() == 1 && layers.back()->getResult(node)) return true;

    layers.pop_back();

    /// We try to check whether there was a checkpoint
    while (!layers.empty() && !layers.back()->saved_checkpoint) layers.pop_back();
...
Action ParserExpressionImpl::tryParseOperand(Layers &layers, IParser::Pos &pos, Expected &expected) {
  ...
  if (layers.back()->previousType() == OperatorType::Comparison) {
    ...
    if (subquery_function_type != SubqueryFunctionType::NONE) {
      ...
      if (!layers.back()->popOperator(prev_op)) return Action::NONE;
      if (!layers.back()->popOperand(argument)) return Action::NONE;

      function = makeASTFunction(prev_op, argument, tmp);

      if (!modifyAST(function, subquery_function_type)) return Action::NONE;
  ...
  for (; cur_op != unary_operators_table.end(); ++cur_op) {
    if (parseOperator(pos, cur_op->first, expected)) break;
  }
  ...
  if (ParseDateOperatorExpression(pos, tmp, expected) || ParseTimestampOperatorExpression(pos, tmp, expected) ||
    ...
    if (identifier_parser.parse(pos, tmp, expected)) {
      ...
      if (subquery_parser.startsWithValidSelectOrExplain()) return Action::NONE;
...
Action ParserExpressionImpl::tryParseOperator(Layers &layers, IParser::Pos &pos, Expected &expected) {
  ...
  Expected stub;
  if (ParserKeyword(Keyword::IN_PARTITION).checkWithoutMoving(pos, stub)) return Action::NONE;
  ...
  if (cur_op == operators_table.end()) {
    if (!layers.back()->allow_alias || layers.back()->parsed_alias) return Action::NONE;

    ASTPtr alias;
    ParserAlias alias_parser(layers.back()->allow_alias_without_as_keyword);

    if (!alias_parser.parse(pos, alias, expected) || !layers.back()->insertAlias(alias)) return Action::NONE;
  ...
  while (layers.back()->previousPriority() >= op.priority) {
    ...
    if (prev_op.type == OperatorType::FinishBetween) {
      Operator tmp_op;
      if (!layers.back()->popOperator(tmp_op)) return Action::NONE;

      if (tmp_op.type != OperatorType::StartBetween && tmp_op.type != OperatorType::StartNotBetween) return Action::NONE;

      bool negative = tmp_op.type == OperatorType::StartNotBetween;

      ASTs arguments;
      if (!layers.back()->popLastNOperands(arguments, 3)) return Action::NONE;

      function = makeBetweenOperator(negative, arguments);
    } else {
      function = makeASTFunction(prev_op);

      if (!layers.back()->popLastNOperands(function->children[0]->children, prev_op.arity)) return Action::NONE;
    }
  ...
  if (op.type == OperatorType::TupleElement) {
    ...
    if (asterisk_parser.parse(pos, tmp, expected) || columns_matcher_parser.parse(pos, tmp, expected)) {
      if (auto *asterisk = tmp->as<ASTAsterisk>()) {
        if (!layers.back()->popOperand(asterisk->expression)) return Action::NONE;
      } else if (auto *columns_list_matcher = tmp->as<ASTColumnsListMatcher>()) {
        if (!layers.back()->popOperand(columns_list_matcher->expression)) return Action::NONE;
      } else if (auto *columns_regexp_matcher = tmp->as<ASTColumnsRegexpMatcher>()) {
        if (!layers.back()->popOperand(columns_regexp_matcher->expression)) return Action::NONE;
      }
  ...
  if (op.type == OperatorType::IsNull) {
    ...
    if (!layers.back()->popLastNOperands(function->children[0]->children, 1)) return Action::NONE;
  ...
  if (op.type == OperatorType::Cast) {
    ASTPtr type_ast;
    if (!ParserDataType().parse(pos, type_ast, expected)) return Action::NONE;

    layers.back()->pushOperand(make_intrusive<ASTLiteral>(type_ast->formatWithSecretsOneLine()));
    return Action::OPERATOR;
  }

  if (op.type == OperatorType::ArrayElement) layers.push_back(std::make_unique<ArrayElementLayer>());

  if (op.type == OperatorType::StartBetween || op.type == OperatorType::StartNotBetween) ++layers.back()->between_counter;
