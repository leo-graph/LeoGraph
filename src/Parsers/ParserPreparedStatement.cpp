...
bool ParserPrepare::parseImpl(Pos& pos, ASTPtr& node, Expected& expected) {
  ...
  if (!s_prepare.ignore(pos, expected)) return false;

  ASTPtr ast_ident;
  if (!s_ident.parse(pos, ast_ident, expected)) return false;
...
bool ParserExecute::parseImpl(Pos& pos, ASTPtr& node, Expected& expected) {
  ...
  if (!s_execute.ignore(pos, expected)) return false;
  ...
  ASTPtr ast_args;
  if (!exp_args.parse(pos, ast_args, expected)) return false;
