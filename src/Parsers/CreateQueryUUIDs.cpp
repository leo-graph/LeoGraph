...
CreateQueryUUIDs::CreateQueryUUIDs(const ASTCreateQuery& query, bool generate_random, bool force_random) {
  if (!generate_random || !force_random) {
    uuid = query.uuid;
    if (query.targets) {
      for (const auto& target : query.targets->targets) setTargetInnerUUID(target.kind, target.inner_uuid);
    }
  }

  if (generate_random) {
    if (uuid == UUIDHelpers::Nil) uuid = UUIDHelpers::generateV4();
    ...
    /// If we generate random UUIDs for already existing tables then those UUIDs will not be correct making those inner target table
    /// inaccessible. Thus it's not safe for example to replace "ATTACH MATERIALIZED VIEW mv AS SELECT a FROM b" with "ATTACH MATERIALIZED
    /// VIEW mv TO INNER UUID '123e4567-e89b-12d3-a456-426614174000' AS SELECT a FROM b" This replacement is safe only for CREATE queries
    /// when inner target tables don't exist yet.
    if (!query.attach) {
      ...
      if (query.is_materialized_view && !(query.refresh_strategy && !query.refresh_strategy->append)) generate_target_uuid(ViewTarget::To);
      ...
      if (query.storage && query.storage->engine &&
        ...
        if (query.getTargetInnerUUID(ViewTarget::To) == UUIDHelpers::Nil) setTargetInnerUUID(ViewTarget::To, UUIDHelpers::generateV4());
      }
...
bool CreateQueryUUIDs::empty() const {
  if (uuid != UUIDHelpers::Nil) return false;
  for (const auto& [_, inner_uuid] : targets_inner_uuids) {
    if (inner_uuid != UUIDHelpers::Nil) return false;
  }
  return true;
}
...
String CreateQueryUUIDs::toString() const {
  ...
  auto add_name_and_uuid_to_string = [&](std::string_view name_, const UUID& uuid_) {
    if (std::exchange(need_comma, true)) out << ", ";
    out << "\"" << name_ << "\": \"" << uuid_ << "\"";
  };
  if (uuid != UUIDHelpers::Nil) add_name_and_uuid_to_string("uuid", uuid);
  for (const auto& [kind, inner_uuid] : targets_inner_uuids) {
    if (inner_uuid != UUIDHelpers::Nil) add_name_and_uuid_to_string(::DB::toString(kind), inner_uuid);
  }
  out << "}";
  return out.str();
}
...
void CreateQueryUUIDs::setTargetInnerUUID(ViewTarget::Kind kind, const UUID& new_inner_uuid) {
  ...
  if (new_inner_uuid != UUIDHelpers::Nil) targets_inner_uuids.emplace_back(kind, new_inner_uuid);
}
...
void CreateQueryUUIDs::copyToQuery(ASTCreateQuery& query) const {
  ...
  if (query.targets) query.targets->resetInnerUUIDs();

  if (!targets_inner_uuids.empty()) {
    if (!query.targets) query.set(query.targets, make_intrusive<ASTViewTargets>());

    for (const auto& [kind, inner_uuid] : targets_inner_uuids) {
      if (inner_uuid != UUIDHelpers::Nil) query.targets->setInnerUUID(kind, inner_uuid);
    }
  }
}
