...
ASTPtr ASTCreateWasmFunctionQuery::clone() const {
  ...
  if (source_function_name_ast) res->setSourceFunctionName(source_function_name_ast->clone());
  if (module_hash_ast) res->setModuleHash(module_hash_ast->clone());
  if (abi_ast) res->setAbi(abi_ast->clone());

  return res;
}

void ASTCreateWasmFunctionQuery::formatImpl(WriteBuffer &ostr, const FormatSettings &settings, FormatState &state,
                                            FormatStateStacked frame) const {
  ostr << "CREATE ";

  if (or_replace) ostr << "OR REPLACE ";

  ostr << "FUNCTION ";

  if (if_not_exists) ostr << "IF NOT EXISTS ";

  if (function_name_ast) function_name_ast->format(ostr, settings, state, frame);
...
String ASTCreateWasmFunctionQuery::getFunctionName() const { return getIdentifierName(function_name_ast); }

static String getAstAsStringLiteral(const ASTPtr &ast) {
  if (const auto *literal = typeid_cast<const ASTLiteral *>(ast.get())) return literal->value.safeGet<String>();
  throw Exception(ErrorCodes::LOGICAL_ERROR, "Expected ASTLiteral, got '{}'", ast->formatForErrorMessage());
}

void ASTCreateWasmFunctionQuery::setModuleHash(String hash_str) { module_hash_ast = make_intrusive<ASTLiteral>(hash_str); }
...
ASTCreateWasmFunctionQuery::Definition ASTCreateWasmFunctionQuery::validateAndGetDefinition() const {
  ...
  if (module_hash_ast) info.module_hash = getAstAsStringLiteral(module_hash_ast);

  if (source_function_name_ast) info.source_function_name = getAstAsStringLiteral(source_function_name_ast);
  ...
  for (const auto &setting : function_settings) info.settings.trySet(setting.name, setting.value);
