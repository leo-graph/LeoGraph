...
ASTIndexDeclaration::ASTIndexDeclaration(ASTPtr expression, ASTPtr type, const String &name_) : name(name_) {
  if (!expression) throw Exception(ErrorCodes::LOGICAL_ERROR, "Index declaration must have an expression");
...
ASTPtr ASTIndexDeclaration::getExpression() const {
  if (children.size() <= expression_idx) throw Exception(ErrorCodes::LOGICAL_ERROR, "Index declaration must have an expression");
  return children[expression_idx];
}

boost::intrusive_ptr<ASTFunction> ASTIndexDeclaration::getType() const {
  if (children.size() <= type_idx) return nullptr;
  auto func_ast = boost::dynamic_pointer_cast<ASTFunction>(children[type_idx]);
  if (!func_ast) throw Exception(ErrorCodes::LOGICAL_ERROR, "Index declaration type must be a function");
  return func_ast;
}
...
UInt64 getSecondaryIndexGranularity(const boost::intrusive_ptr<ASTFunction> &type, const ASTPtr &granularity) {
  ...
  if (type && type->name == "text") return ASTIndexDeclaration::DEFAULT_TEXT_INDEX_GRANULARITY;

  if (granularity) return granularity->as<ASTLiteral &>().value.safeGet<UInt64>();

  if (type && type->name == "vector_similarity") return ASTIndexDeclaration::DEFAULT_VECTOR_SIMILARITY_INDEX_GRANULARITY;
