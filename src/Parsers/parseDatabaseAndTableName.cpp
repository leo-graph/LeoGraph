...
bool parseDatabaseAndTableName(IParser::Pos& pos, Expected& expected, String& database_str, String& table_str) {
  ...
  if (!table_parser.parse(pos, database, expected)) return false;
...
bool parseDatabaseAndTableAsAST(IParser::Pos& pos, Expected& expected, ASTPtr& database, ASTPtr& table) {
  ...
  if (!table_parser.parse(pos, table, expected)) return false;
...
bool parseDatabaseAndTableNameOrAsterisks(IParser::Pos& pos, Expected& expected, String& database, String& table, bool& wildcard,
  ...
  return IParserBase::wrapParseImpl(pos, [&] {
    if (ParserToken{TokenType::Asterisk}.ignore(pos, expected)) {
      ...
      if (ParserToken{TokenType::Dot}.ignore(pos, expected) && ParserToken{TokenType::Asterisk}.ignore(pos, expected)) {
        /// *.*
        database.clear();
        table.clear();
        return true;
    ...
    if (identifier_parser.parse(pos, ast, expected)) {
      ...
      if (!wildcard && ParserToken{TokenType::Asterisk}.ignore(pos, expected)) wildcard = true;
