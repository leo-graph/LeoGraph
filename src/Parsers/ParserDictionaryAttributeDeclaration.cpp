...
bool ParserDictionaryAttributeDeclaration::parseImpl(Pos& pos, ASTPtr& node, Expected& expected) {
  ...
  while (true) {
    if (!default_value && s_default.ignore(pos, expected)) {
      if (!default_parser.parse(pos, default_value, expected) && !array_literals_parser.parse(pos, default_value, expected)) return false;

      continue;
    }

    if (!expression && s_expression.ignore(pos, expected)) {
      if (!expression_parser.parse(pos, expression, expected)) return false;
      continue;
    }
