...
bool ParserSelectQuery::parseImpl(Pos& pos, ASTPtr& node, Expected& expected) {
  ...
  /// WITH expr_list
  {
    if (s_with.ignore(pos, expected)) {
      ...
      if (!ParserList(std::make_unique<ParserWithElement>(), std::make_unique<ParserToken>(TokenType::Comma))
        ...
        return false;
      if (with_expression_list->children.empty()) return false;
      ...
      for (const auto& child : with_expression_list->children)  /// For cases: WITH _ (a, b) AS ...      <- (a, b) are aliases
      {
        if (auto* with_element = child->as<ASTWithElement>())
          if (with_element->aliases) expression_list_for_cte_aliases = with_element->aliases;
      }
    }
  }
  ...
  if (s_from.ignore(pos, expected)) {
    if (!ParserTablesInSelectQuery(false).parse(pos, tables, expected)) return false;
  }
  ...
  /// SELECT [ALL/DISTINCT [ON (expr_list)]] [TOP N [WITH TIES]] expr_list
  {
    ...
    if (!s_select.ignore(pos, expected)) {
      ...
      if (!implicit_select || with_expression_list || tables) return false;
    }

    if (s_all.ignore(pos, expected)) has_all = true;

    if (s_distinct_on.ignore(pos, expected)) {
      if (open_bracket.ignore(pos, expected)) {
        if (!exp_list.parse(pos, distinct_on_expression_list, expected)) return false;
        if (!close_bracket.ignore(pos, expected)) return false;
      } else
        return false;
    ...
    if (!has_all && s_all.ignore(pos, expected)) has_all = true;

    if (has_all && (select_query->distinct || distinct_on_expression_list)) return false;

    if (s_top.ignore(pos, expected)) {
      ParserNumber num;

      if (open_bracket.ignore(pos, expected)) {
        if (!num.parse(pos, top_length, expected)) return false;
        if (!close_bracket.ignore(pos, expected)) return false;
      } else {
        if (!num.parse(pos, top_length, expected)) return false;
      }

      if (s_with_ties.ignore(pos, expected)) select_query->limit_with_ties = true;
    }

    if (!exp_list_for_select_clause.parse(pos, select_expression_list, expected)) return false;
  }

  /// FROM database.table or FROM table or FROM (subquery) or FROM tableFunction(...)
  if (!tables && s_from.ignore(pos, expected)) {
    if (!ParserTablesInSelectQuery().parse(pos, tables, expected)) return false;
  }

  if (tables && open_bracket.ignore(pos, expected)) {
    if (!exp_list_for_aliases.parse(pos, expression_list_for_aliases, expected)) return false;
  ...
  if (s_prewhere.ignore(pos, expected)) {
    if (!exp_elem.parse(pos, prewhere_expression, expected)) return false;
  }

  /// WHERE expr
  if (s_where.ignore(pos, expected)) {
    if (!exp_elem.parse(pos, where_expression, expected)) return false;
  }
  ...
  if (s_group_by.ignore(pos, expected)) {
    ...
    if (select_query->group_by_with_grouping_sets) {
      if (!grouping_sets_list.parse(pos, group_expression_list, expected)) return false;
    } else if (!select_query->group_by_all) {
      if (!exp_list.parse(pos, group_expression_list, expected)) return false;
    }
  ...
  /// WITH TOTALS
  if (s_with.ignore(pos, expected)) {
    if (select_query->group_by_with_totals || !s_totals.ignore(pos, expected)) return false;
  ...
  if (s_having.ignore(pos, expected)) {
    if (!exp_elem.parse(pos, having_expression, expected)) return false;
  }
  ...
  if (s_qualify.ignore(pos, expected)) {
    if (!exp_elem.parse(pos, qualify_expression, expected)) return false;
  }
  ...
  if (s_order_by.ignore(pos, expected)) {
    ...
    if (!is_order_by_all) {
      if (!order_list.parse(pos, order_expression_list, expected)) return false;
      ...
      if (std::any_of(order_expression_list->children.begin(), order_expression_list->children.end(),
        ...
        if (s_interpolate.ignore(pos, expected)) {
          if (open_bracket.ignore(pos, expected)) {
            if (!interpolate_list.parse(pos, interpolate_expression_list, expected)) return false;
            if (!close_bracket.ignore(pos, expected)) return false;
          } else
  ...
  if (s_limit.ignore(pos, expected)) {
    ...
    if (!exp_elem.parse(pos, limit_length, expected)) return false;
    ...
    if (s_comma.ignore(pos, expected)) {
      ...
      if (!exp_elem.parse(pos, limit_offset, expected)) return false;
    ...
    if (s_by.ignore(pos, expected)) {
      ...
      if (distinct_on_expression_list) throw Exception(ErrorCodes::SYNTAX_ERROR, "Can not use DISTINCT ON alongside LIMIT BY");
      ...
      if (s_all.ignore(pos, expected)) {
        ...
        if (!exp_list.parse(pos, limit_by_expression_list, expected)) return false;
      }
    }

    if (top_length && limit_length) throw Exception(ErrorCodes::TOP_AND_LIMIT_TOGETHER, "Can not use TOP and LIMIT together");
  } else if (s_offset.ignore(pos, expected)) {
    ...
    if (s_row.ignore(pos, expected)) offset_clause_has_sql_standard_row_or_rows = true;
  ...
  if ((!has_offset_clause || offset_clause_has_sql_standard_row_or_rows) && s_fetch.ignore(pos, expected)) {
    ...
    if (s_first.ignore(pos, expected)) {
      if (s_next.ignore(pos, expected)) throw Exception(ErrorCodes::FIRST_AND_NEXT_TOGETHER, "Can not use FIRST and NEXT together");
    ...
    if (s_row.ignore(pos, expected)) {
      if (s_rows.ignore(pos, expected)) throw Exception(ErrorCodes::ROW_AND_ROWS_TOGETHER, "Can not use ROW and ROWS together");
  ...
  if (top_length) limit_length = top_length;

  /// LIMIT length [WITH TIES] | LIMIT offset, length [WITH TIES]
  if (s_limit.ignore(pos, expected)) {
    if (!limit_by_length || limit_length) return false;
  ...
  if (s_settings.ignore(pos, expected)) {
    ...
    if (!parser_settings.parse(pos, settings, expected)) return false;
  }
