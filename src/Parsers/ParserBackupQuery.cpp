...
bool parsePartitions(IParser::Pos &pos, Expected &expected, std::optional<ASTs> &partitions) {
  if (!ParserKeyword(Keyword::PARTITION).ignore(pos, expected) && !ParserKeyword(Keyword::PARTITIONS).ignore(pos, expected)) return false;

  ASTs result;
  auto parse_list_element = [&] {
    ASTPtr ast;
    if (!ParserPartition{}.parse(pos, ast, expected)) return false;
    result.push_back(ast);
    return true;
  };
  if (!ParserList::parseUtil(pos, expected, parse_list_element, false)) return false;
...
bool parseExceptDatabases(IParser::Pos &pos, Expected &expected, std::set<String> &except_databases) {
  return IParserBase::wrapParseImpl(pos, [&] {
    ...
    auto parse_list_element = [&] {
      ASTPtr ast;
      if (!ParserIdentifier{}.parse(pos, ast, expected)) return false;
...
bool parseExceptTables(IParser::Pos &pos, Expected &expected, const std::optional<String> &database_name,
  ...
  return IParserBase::wrapParseImpl(pos, [&] {
    ...
    auto parse_list_element = [&] {
      ...
      if (!parseDatabaseAndTableName(pos, expected, table_name.first, table_name.second)) return false;

      if (database_name && table_name.first.empty()) table_name.first = *database_name;
...
bool parseElement(IParser::Pos &pos, Expected &expected, Element &element) {
  return IParserBase::wrapParseImpl(pos, [&] {
    if (ParserKeyword(Keyword::TABLE).ignore(pos, expected) || ParserKeyword(Keyword::DICTIONARY).ignore(pos, expected) ||
      ...
      if (!parseDatabaseAndTableName(pos, expected, element.database_name, element.table_name)) return false;
      ...
      if (ParserKeyword(Keyword::AS).ignore(pos, expected)) {
        if (!parseDatabaseAndTableName(pos, expected, element.new_database_name, element.new_table_name)) return false;
      }
...
bool parseElements(IParser::Pos &pos, Expected &expected, std::vector<Element> &elements) {
  return IParserBase::wrapParseImpl(pos, [&] {
    ...
    if (!ParserList::parseUtil(pos, expected, parse_element, false)) return false;
...
bool parseBackupName(IParser::Pos &pos, Expected &expected, ASTPtr &backup_name) {
  if (!ParserIdentifierWithOptionalParameters{}.parse(pos, backup_name, expected)) return false;
...
bool parseBaseBackupSetting(IParser::Pos &pos, Expected &expected, ASTPtr &base_backup_name) {
  return IParserBase::wrapParseImpl(pos, [&] {
    return ParserKeyword{Keyword::BASE_BACKUP}.ignore(pos, expected) && ParserToken(TokenType::Equals).ignore(pos, expected) &&
...
bool parseClusterHostIDs(IParser::Pos &pos, Expected &expected, ASTPtr &cluster_host_ids) {
  ...
  if (ParserArray{}.parse(pos, cluster_host_ids, expected)) return true;

  ASTPtr tmp;
  if (!ParserFunction{}.parse(pos, tmp, expected)) return false;

  auto *func = tmp->as<ASTFunction>();
  if (!func || func->name != "array") return false;
...
bool parseClusterHostIDsSetting(IParser::Pos &pos, Expected &expected, ASTPtr &cluster_host_ids) {
  return IParserBase::wrapParseImpl(pos, [&] {
    return ParserKeyword{Keyword::CLUSTER_HOST_IDS}.ignore(pos, expected) && ParserToken(TokenType::Equals).ignore(pos, expected) &&
...
bool parseSettings(IParser::Pos &pos, Expected &expected, ASTPtr &settings, ASTPtr &base_backup_name, ASTPtr &cluster_host_ids) {
  return IParserBase::wrapParseImpl(pos, [&] {
    if (!ParserKeyword(Keyword::SETTINGS).ignore(pos, expected)) return false;
    ...
    auto parse_setting = [&] {
      if (!res_base_backup_name && parseBaseBackupSetting(pos, expected, res_base_backup_name)) return true;

      if (!res_cluster_host_ids && parseClusterHostIDsSetting(pos, expected, res_cluster_host_ids)) return true;
    ...
    if (!ParserList::parseUtil(pos, expected, parse_setting, false)) return false;
...
bool ParserBackupQuery::parseImpl(Pos &pos, ASTPtr &node, Expected &expected) {
  ...
  if (!parseElements(pos, expected, elements)) return false;

  String cluster;
  parseOnCluster(pos, expected, cluster);

  if (!ParserKeyword((kind == Kind::BACKUP) ? Keyword::TO : Keyword::FROM).ignore(pos, expected)) return false;

  ASTPtr backup_name;
  if (!parseBackupName(pos, expected, backup_name)) return false;
  ...
  if (backup_name) query->set(query->backup_name, backup_name);

  query->settings = std::move(settings);
  query->cluster_host_ids = std::move(cluster_host_ids);

  if (base_backup_name) query->set(query->base_backup_name, base_backup_name);
