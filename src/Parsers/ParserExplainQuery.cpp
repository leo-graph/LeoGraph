...
bool ParserExplainQuery::parseImpl(Pos& pos, ASTPtr& node, Expected& expected) {
  ...
  if (kind == ASTExplainQuery::ExplainKind::ParsedAST) {
    ...
    if (!ParserFunction(true, true).parse(pos, table_function, expected)) return false;
    ASTPtr table_override;
    if (!ParserTableOverrideDeclaration(false).parse(pos, table_override, expected)) return false;
