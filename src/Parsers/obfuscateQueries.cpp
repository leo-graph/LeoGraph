...
const std::unordered_set<std::string>& getObfuscateKeywords() {
  auto initialize = []() {
    ...
    for (const auto& keyword : getAllKeyWords()) {
      ...
      for (const auto& token : tokens) instance.insert(token);
    }
    ...
    for (const auto& kw : additional_keywords) instance.insert(kw);
...
const std::unordered_set<std::string_view> keep_words{"DAY",         "HOUR",        "ID",     "NAME",    "NANOSECOND",
                                                      "MICROSECOND", "MILLISECOND", "SECOND", "MINUTE",  "NUM",
...
std::initializer_list<std::string_view> nouns{"aardvark",
                                              "abacus",
                                              "abbey",
                                              "abbreviation",
                                              "abdomen",
...
void obfuscateIdentifier(std::string_view src, WriteBuffer& result, WordMap& obfuscate_map, WordSet& used_nouns, SipHash hash_func) {
  ...
  while (src_pos < src_end) {
    if (isAlphaNumericASCII(src_pos[0])) word_has_alphanumerics = true;
  ...
  if (word_begin < src_pos) append_word();
}

void obfuscateLiteral(std::string_view src, WriteBuffer& result, SipHash hash_func, KnownIdentifierFunc known_identifier_func) {
  const char* src_pos = src.data();
  const char* src_end = src_pos + src.size();

  /// Preserve hex (0x/0X) and binary (0b/0B) number prefixes and structure.
  if (src_pos + 2 <= src_end && src_pos[0] == '0' && (src_pos[1] == 'x' || src_pos[1] == 'X' || src_pos[1] == 'b' || src_pos[1] == 'B')) {
  ...
  auto isHexRange = [](const char* begin, const char* end) -> bool {
    for (const char* p = begin; p < end; ++p)
      if (!isHexDigit(*p)) return false;
    return begin < end;
  };

  while (src_pos < src_end) {
    /// UUID: xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx (8-4-4-4-12 hex digits with dashes).
    if (src_pos + 36 <= src_end && isHexRange(src_pos, src_pos + 8) && src_pos[8] == '-' && isHexRange(src_pos + 9, src_pos + 13) &&
        src_pos[13] == '-' && isHexRange(src_pos + 14, src_pos + 18) && src_pos[18] == '-' && isHexRange(src_pos + 19, src_pos + 23) &&
        src_pos[23] == '-' && isHexRange(src_pos + 24, src_pos + 36)) {
    ...
    /// Date
    else if (src_pos + strlen("0000-00-00") <= src_end && isNumericASCII(src_pos[0]) && isNumericASCII(src_pos[1]) &&
             isNumericASCII(src_pos[2]) && isNumericASCII(src_pos[3]) && src_pos[4] == '-' && isNumericASCII(src_pos[5]) &&
             isNumericASCII(src_pos[6]) && src_pos[7] == '-' && isNumericASCII(src_pos[8]) && isNumericASCII(src_pos[9])) {
      DayNum date;
      ...
      /// DateTime
      if (src_pos + strlen(" 00:00:00") <= src_end && isNumericASCII(src_pos[1]) && isNumericASCII(src_pos[2]) && src_pos[3] == ':' &&
          isNumericASCII(src_pos[4]) && isNumericASCII(src_pos[5]) && src_pos[6] == ':' && isNumericASCII(src_pos[7]) &&
      ...
      bool matched_ipv4 = false;
      {
        ...
        if (is_ipv4 && p < src_end && (isNumericASCII(*p) || *p == '.')) is_ipv4 = false;
    ...
    } else if (src_pos + 1 < src_end && (src_pos[0] == 'e' || src_pos[0] == 'E') &&
      ...
      while (alpha_end < src_end && isAlphaASCII(*alpha_end)) ++alpha_end;
...
void obfuscateQueries(std::string_view src, WriteBuffer& result, WordMap& obfuscate_map, WordSet& used_nouns, SipHash hash_func,
  ...
  while (true) {
    ...
    if (token.type == TokenType::BareWord) {
      ...
      if (token.size() >= 3 && (*token.begin == 'x' || *token.begin == 'X' || *token.begin == 'b' || *token.begin == 'B') &&
          token.begin[1] == '\'') {
