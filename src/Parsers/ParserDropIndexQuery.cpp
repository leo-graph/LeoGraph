...
bool ParserDropIndexQuery::parseImpl(IParser::Pos& pos, ASTPtr& node, Expected& expected) {
  ...
  if (!s_drop.ignore(pos, expected)) return false;

  if (!s_index.ignore(pos, expected)) return false;

  if (s_if_exists.ignore(pos, expected)) if_exists = true;

  if (!index_name_p.parse(pos, query->index_name, expected)) return false;
  ...
  if (query->index_name) query->children.push_back(query->index_name);
