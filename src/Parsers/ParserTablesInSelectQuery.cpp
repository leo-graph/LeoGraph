...
bool ParserTableExpression::parseImpl(Pos &pos, ASTPtr &node, Expected &expected) {
  ...
  if (!ParserWithOptionalAlias(std::make_unique<ParserSubquery>(), allow_alias_without_as_keyword).parse(pos, res->subquery, expected) &&
      !ParserWithOptionalAlias(std::make_unique<ParserFunction>(false, true), allow_alias_without_as_keyword)
    ...
    if (pos->type == TokenType::OpeningRoundBracket) {
      ...
      if (ParserTablesInSelectQuery(false).parse(pos, tables_in_select, expected) && pos->type == TokenType::ClosingRoundBracket &&
        ...
        ASTPtr alias_node;
        if (alias_parser.parse(pos, alias_node, expected)) res->subquery->setAlias(alias_node->getColumnName());
      } else {
        pos = open_paren;
        return false;
      }
  ...
  if (pos->type == TokenType::OpeningRoundBracket) {
    ...
    if (!column_aliases_parser.parse(pos, res->column_aliases, expected)) return false;
  ...
  /// FINAL
  if (ParserKeyword(Keyword::FINAL).ignore(pos, expected)) res->final = true;

  /// SAMPLE number
  if (ParserKeyword(Keyword::SAMPLE).ignore(pos, expected)) {
    ParserSampleRatio ratio;

    if (!ratio.parse(pos, res->sample_size, expected)) return false;

    /// OFFSET number
    if (ParserKeyword(Keyword::OFFSET).ignore(pos, expected)) {
      if (!ratio.parse(pos, res->sample_offset, expected)) return false;
    }
  }

  if (res->database_and_table_name) res->children.emplace_back(res->database_and_table_name);
  if (res->table_function) res->children.emplace_back(res->table_function);
  if (res->subquery) res->children.emplace_back(res->subquery);
  if (res->sample_size) res->children.emplace_back(res->sample_size);
  if (res->sample_offset) res->children.emplace_back(res->sample_offset);
  if (res->column_aliases) res->children.emplace_back(res->column_aliases);
...
bool ParserArrayJoin::parseImpl(Pos &pos, ASTPtr &node, Expected &expected) {
  ...
  if (!has_array_join) return false;

  if (!ParserExpressionList(false).parse(pos, res->expression_list, expected)) return false;

  if (res->expression_list) res->children.emplace_back(res->expression_list);
...
bool ParserTablesInSelectQueryElement::parseImpl(Pos &pos, ASTPtr &node, Expected &expected) {
  ...
  if (is_first) {
    if (!ParserTableExpression(allow_alias_without_as_keyword).parse(pos, res->table_expression, expected)) return false;
    ...
    if (pos->type == TokenType::Comma) {
      ...
      if (table_join->kind == JoinKind::Left || table_join->kind == JoinKind::Right || table_join->kind == JoinKind::Full) {
      ...
      if (!ParserKeyword(Keyword::JOIN).ignore(pos, expected)) return false;
    }

    if (!ParserTableExpression(allow_alias_without_as_keyword).parse(pos, res->table_expression, expected)) return false;

    if (table_join->kind != JoinKind::Comma && table_join->kind != JoinKind::Cross && table_join->kind != JoinKind::Paste) {
      if (ParserKeyword(Keyword::USING).ignore(pos, expected)) {
        ...
        if (!ParserExpressionList(false).parse(pos, table_join->using_expression_list, expected)) return false;
        ...
        if (!ParserExpression().parse(pos, table_join->on_expression, expected)) return false;
      } else {
        return false;
      }
    }

    if (table_join->using_expression_list) table_join->children.emplace_back(table_join->using_expression_list);
    if (table_join->on_expression) table_join->children.emplace_back(table_join->on_expression);

    res->table_join = table_join;
  }

  if (res->table_expression) res->children.emplace_back(res->table_expression);
  if (res->table_join) res->children.emplace_back(res->table_join);
  if (res->array_join) res->children.emplace_back(res->array_join);
