...
void ASTQueryWithOutput::formatImpl(WriteBuffer &ostr, const FormatSettings &s, FormatState &state, FormatStateStacked frame) const {
  ...
  if (out_file) {
    ...
    if (isOutfileAppend()) ostr << " APPEND";
    if (isOutfileTruncate()) ostr << " TRUNCATE";
    if (isIntoOutfileWithStdout()) ostr << " AND STDOUT";
    if (compression) {
      ostr << " COMPRESSION ";
...
bool ASTQueryWithOutput::hasOutputOptions() const { return out_file || format_ast || settings_ast || compression || compression_level; }
