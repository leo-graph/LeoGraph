...
bool ParserCase::parseImpl(Pos& pos, ASTPtr& node, Expected& expected) {
  ...
  if (!s_case.ignore(pos, expected)) return false;
  ...
  auto parse_branches = [&]() {
    ...
    while (s_when.ignore(pos, expected)) {
      ...
      ASTPtr expr_when;
      if (!p_expr.parse(pos, expr_when, expected)) return false;
      args.push_back(expr_when);

      if (!s_then.ignore(pos, expected)) return false;

      ASTPtr expr_then;
      if (!p_expr.parse(pos, expr_then, expected)) return false;
    ...
    if (s_else.ignore(pos, expected)) {
      if (!p_expr.parse(pos, expr_else, expected)) return false;
    } else {
  ...
  if (has_case_expr) {
    ASTPtr case_expr;
    if (!p_expr.parse(pos, case_expr, expected)) return false;
    args.push_back(case_expr);

    if (!parse_branches()) return false;
