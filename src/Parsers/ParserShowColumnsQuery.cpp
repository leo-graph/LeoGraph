...
bool ParserShowColumnsQuery::parseImpl(Pos &pos, ASTPtr &node, Expected &expected) {
  ...
  if (ParserKeyword(Keyword::FULL).ignore(pos, expected)) query->full = true;

  if (!(ParserKeyword(Keyword::COLUMNS).ignore(pos, expected) || ParserKeyword(Keyword::FIELDS).ignore(pos, expected))) return false;
  ...
  if (ParserKeyword(Keyword::NOT).ignore(pos, expected)) query->not_like = true;
  ...
  if (ParserKeyword(Keyword::LIMIT).ignore(pos, expected))
    if (!ParserExpressionWithOptionalAlias(false).parse(pos, query->limit_length, expected)) return false;
