...
class FunctionSecretArgumentsFinder {
 public:
  struct Result {
    ...
    bool hasSecrets() const { return count != 0 || !nested_maps.empty(); }
  };
 ...
 protected:
  ...
  void markSecretArgument(size_t index, bool argument_is_named = false) {
    if (index >= function->arguments->size()) return;
    if (!result.count) {
      result.start = index;
    ...
    if (!argument_is_named) result.are_named = false;
  }
  ...
  void findOrdinaryFunctionSecretArguments() {
    if ((function->name() == "mysql") || (function->name() == "postgresql")) {
    ...
    } else if ((function->name() == "s3Cluster") || (function->name() == "hudiCluster") || (function->name() == "deltaLakeCluster") ||
               (function->name() == "deltaLakeS3Cluster") || (function->name() == "icebergS3Cluster") ||
               (function->name() == "icebergCluster")) {
      /// s3Cluster('cluster_name', 'url', 'aws_access_key_id', 'aws_secret_access_key', ...)
      findS3FunctionSecretArguments(/* is_cluster_function= */ true);
    } else if ((function->name() == "azureBlobStorage") || (function->name() == "deltaLakeAzure") || (function->name() == "icebergAzure") ||
      ...
      findAzureBlobStorageFunctionSecretArguments(/* is_cluster_function= */ false);
               ...
               (function->name() == "deltaLakeAzureCluster")) {
      /// azureBlobStorageCluster(cluster, connection_string|storage_account_url, container_name, blobpath, [account_name, account_key,
    ...
    } else if ((function->name() == "encrypt") || (function->name() == "decrypt") || (function->name() == "aes_encrypt_mysql") ||
  ...
  void findMongoDBSecretArguments() {
    ...
    if (isNamedCollectionName(0)) {
      ...
      if (findSecretNamedArgument("password", 1)) return;

      /// MongoDB(named_collection, ..., uri = 'mongodb://username:password@127.0.0.1:27017', ...)
      if (findNamedArgument(&uri, "uri", 1) == -1) return;
  ...
  void findS3FunctionSecretArguments(bool is_cluster_function) {
    ...
    if ((url_arg_idx + 3 <= count) && (count <= url_arg_idx + 4)) {
      ...
      if (tryGetStringFromArgument(url_arg_idx + 1, &second_arg)) {
        if (boost::iequals(second_arg, "NOSIGN")) return;  /// The argument after 'url' is "NOSIGN".
    ...
    if (url_arg_idx + 2 < count) markSecretArgument(url_arg_idx + 2);
  }

  void findAzureBlobStorageFunctionSecretArguments(bool is_cluster_function) {
    /// azureBlobStorageCluster('cluster_name', 'conn_string/storage_account_url', ...) has 'conn_string/storage_account_url' as its second
    /// argument.
    size_t url_arg_idx = is_cluster_function ? 1 : 0;

    if (!is_cluster_function && isNamedCollectionName(0)) {
      /// azureBlobStorage(named_collection, ..., account_key = 'account_key', ...)
      if (maskAzureConnectionString(-1, true, 1)) return;
    ...
    if (is_cluster_function && isNamedCollectionName(1)) {
      ...
      if (maskAzureConnectionString(-1, true, 2)) return;
      findSecretNamedArgument("account_key", 2);
      return;
    }

    if (maskAzureConnectionString(url_arg_idx)) return;
    ...
    /// argument azureBlobStorageCluster(cluster, connection_string|storage_account_url, container_name, blobpath, format) -- in this case
    /// there is no account_key argument
    size_t count = function->arguments->size();
    if ((url_arg_idx + 4 <= count) && (count <= url_arg_idx + 7)) {
      String fourth_arg;
      if (tryGetStringFromArgument(url_arg_idx + 3, &fourth_arg)) {
        if (fourth_arg == "auto" || KnownFormatNames::instance().exists(fourth_arg)) return;
      }
    }

    /// We're going to replace 'account_key' with '[HIDDEN]' if account_key is used in the signature
    if (url_arg_idx + 4 < count) markSecretArgument(url_arg_idx + 4);
  }
  ...
  bool maskAzureConnectionString(ssize_t url_arg_idx, bool argument_is_named = false, size_t start = 0) {
    ...
    if (argument_is_named) {
      ...
      if (url_arg_idx == -1 || url_arg.empty()) url_arg_idx = findNamedArgument(&url_arg, "storage_account_url", start);
      if (url_arg_idx == -1 || url_arg.empty()) return false;
    } else {
      if (!tryGetStringFromArgument(url_arg_idx, &url_arg)) return false;
    }
  ...
  void findURLSecretArguments() {
    if (isNamedCollectionName(0)) return;
  ...
  bool tryGetStringFromArgument(size_t arg_idx, String* res, bool allow_identifier = true) const {
    if (arg_idx >= function->arguments->size()) return false;
  ...
  void findRemoteFunctionSecretArguments() {
    ...
    if (function->arguments->size() < 3) return;
    ...
    if (can_be_password) markSecretArgument(arg_num);
  }
  ...
  bool tryGetDatabaseNameOrQualifiedTableName(size_t arg_idx, std::optional<String>& res_database,
                                              std::optional<QualifiedTableName>& res_qualified_table_name) const {
    res_database.reset();
    res_qualified_table_name.reset();

    String str;
    if (!tryGetStringFromArgument(arg_idx, &str, /* allow_identifier= */ true)) return false;
    ...
    if (!qualified_table_name) return false;
  ...
  void findEncryptionFunctionSecretArguments() {
    if (function->arguments->size() == 0) return;
  ...
  void findTableEngineSecretArguments() {
    ...
    if (engine_name == "ExternalDistributed") {
      ...
      findMongoDBSecretArguments();
    } else if ((engine_name == "S3") || (engine_name == "COSN") || (engine_name == "OSS") || (engine_name == "DeltaLake") ||
               (engine_name == "Hudi") || (engine_name == "Iceberg") || (engine_name == "IcebergS3") || (engine_name == "S3Queue")) {
      /// S3('url', ['aws_access_key_id', 'aws_secret_access_key',] ...)
  ...
  void findS3TableEngineSecretArguments() {
    ...
    if (2 < count) markSecretArgument(2);
  }
  ...
  void findBackupDatabaseSecretArguments() {
    if (function->arguments->size() < 2) return;
  ...
  bool isNamedCollectionName(size_t arg_idx) const {
    if (function->arguments->size() <= arg_idx) return false;
  ...
  ssize_t findNamedArgument(String* res, std::string_view key, size_t start = 0) {
    for (size_t i = start; i < function->arguments->size(); ++i) {
      ...
      if (!equals_func || (equals_func->name() != "equals")) continue;

      if (!equals_func->arguments || equals_func->arguments->size() != 2) continue;

      String found_key;
      if (!tryGetStringFromArgument(*equals_func->arguments->at(0), &found_key)) continue;
