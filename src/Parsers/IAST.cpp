...
void intrusive_ptr_add_ref(const IAST *p) noexcept { p->ref_counter.fetch_add(1, std::memory_order_relaxed); }
  ...
  if (p->ref_counter.fetch_sub(1, std::memory_order_acq_rel) == 1) {
    ...
    while (list_head) {
      ...
      for (auto &child : children) {
        if (child == nullptr || child->use_count() != 1) continue;
...
IAST::IAST(const IAST &other) : TypePromotion<IAST>(), children(other.children), ref_counter(0), flags_storage(other.flags_storage) {}
...
size_t IAST::size() const {
  size_t res = 1;
  for (const auto &child : children) res += child->size();
...
size_t IAST::checkSize(size_t max_size) const {
  size_t res = 1;
  for (const auto &child : children) res += child->checkSize(max_size);

  if (res > max_size) throw Exception(ErrorCodes::TOO_BIG_AST, "AST is too big. Maximum: {}", max_size);
...
void IAST::updateTreeHash(SipHash &hash_state, bool ignore_aliases) const {
  ...
  for (const auto &child : children) child->updateTreeHash(hash_state, ignore_aliases);
}
...
size_t IAST::checkDepthImpl(size_t max_depth) const {
  ...
  for (const auto &i : children) stack.push_back({i, 1});
  ...
  while (!stack.empty()) {
    ...
    if (top.second >= max_depth) throw Exception(ErrorCodes::TOO_DEEP_AST, "AST is too deep. Maximum: {}", max_depth);

    res = std::max(res, top.second);

    for (const auto &i : top.first->children) stack.push_back({i, top.second + 1});
  }

  return res;
}

String IAST::formatWithPossiblyHidingSensitiveData(size_t max_length, bool one_line, bool show_secrets, bool print_pretty_type_names,
...
bool IAST::childrenHaveSecretParts() const {
  for (const auto &child : children) {
    if (child->hasSecretParts()) return true;
  }
  return false;
}

void IAST::cloneChildren() {
  for (auto &child : children) child = child->clone();
}
...
void IAST::FormatSettings::writeIdentifier(WriteBuffer &ostr, const String &name, bool ambiguous) const {
  ...
  bool must_quote = (identifier_quoting_rule == IdentifierQuotingRule::Always ||
...
void IAST::dumpTree(WriteBuffer &ostr, size_t indent) const {
  ...
  writeChar('\n', ostr);
  for (const auto &child : children) {
    if (!child) throw Exception(ErrorCodes::UNKNOWN_ELEMENT_IN_AST, "Can't dump a nullptr child");
