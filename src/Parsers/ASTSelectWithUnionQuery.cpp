...
void ASTSelectWithUnionQuery::formatQueryImpl(WriteBuffer& ostr, const FormatSettings& settings, FormatState& state,
                                              FormatStateStacked frame) const {
  std::string indent_str = settings.one_line ? "" : std::string(4 * frame.indent, ' ');

  auto mode_to_str = [&](SelectUnionMode mode) {
    if (mode == SelectUnionMode::UNION_DEFAULT) return "UNION";
    if (mode == SelectUnionMode::UNION_ALL) return "UNION ALL";
    if (mode == SelectUnionMode::UNION_DISTINCT) return "UNION DISTINCT";
    if (mode == SelectUnionMode::EXCEPT_DEFAULT) return "EXCEPT";
    if (mode == SelectUnionMode::EXCEPT_ALL) return "EXCEPT ALL";
    if (mode == SelectUnionMode::EXCEPT_DISTINCT) return "EXCEPT DISTINCT";
    if (mode == SelectUnionMode::INTERSECT_DEFAULT) return "INTERSECT";
    if (mode == SelectUnionMode::INTERSECT_ALL) return "INTERSECT ALL";
    if (mode == SelectUnionMode::INTERSECT_DISTINCT) return "INTERSECT DISTINCT";
    return "";
  };

  auto is_except = [](SelectUnionMode mode) {
    return mode == SelectUnionMode::EXCEPT_DEFAULT || mode == SelectUnionMode::EXCEPT_ALL || mode == SelectUnionMode::EXCEPT_DISTINCT;
  };

  auto get_mode = [&](ASTs::const_iterator it) {
    return is_normalized ? union_mode : list_of_modes[it - list_of_selects->children.begin() - 1];
  };

  for (ASTs::const_iterator it = list_of_selects->children.begin(); it != list_of_selects->children.end(); ++it) {
    if (it != list_of_selects->children.begin()) {
      ostr << settings.nl_or_ws << indent_str << mode_to_str(get_mode(it))
    ...
    if (union_node) need_parens = true;
    ...
    if (settings_ast && !out_file && !format_ast && (*it)->as<ASTSelectQuery>()) need_parens = true;
