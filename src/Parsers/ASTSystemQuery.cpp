...
void ASTSystemQuery::formatImpl(WriteBuffer& ostr, const FormatSettings& settings, FormatState& state, FormatStateStacked frame) const {
  ...
  auto print_database_table = [&]() -> WriteBuffer& {
    ...
    if (if_exists) print_keyword(" IF EXISTS");
  ...
  auto print_drop_replica = [&] {
    ...
    if (!shard.empty()) print_keyword(" FROM SHARD ") << quoteString(shard);
  ...
  if (!queries_with_on_cluster_at_end.contains(type) && !cluster.empty()) formatOnCluster(ostr, settings);
  ...
  switch (type) {
    ...
    case Type::STOP_LISTEN: {
      ...
      if (server_type.type == ServerType::Type::CUSTOM) ostr << ' ' << quoteString(server_type.custom_name);
      ...
      if (!server_type.exclude_types.empty()) {
        ...
        for (auto cur_type : server_type.exclude_types) {
          if (cur_type == ServerType::Type::CUSTOM) continue;
    ...
    case Type::FLUSH_LOGS: {
      ...
      for (const auto& cur_log : tables) {
        ...
        if (!cur_log.first.empty()) print_identifier(cur_log.first) << ".";
    ...
    case Type::INSTRUMENT_ADD: {
      if (!instrumentation_function_name.empty()) ostr << ' ' << quoteString(instrumentation_function_name);
