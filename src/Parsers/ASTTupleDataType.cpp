...
String ASTTupleDataType::getID(char delim) const { return "TupleDataType" + (delim + name); }
...
void ASTTupleDataType::updateTreeHashImpl(SipHash& hash_state, bool ignore_aliases) const {
  ...
  if (const auto arguments = getArguments()) arguments->updateTreeHashImpl(hash_state, ignore_aliases);
}
  ...
  if (arguments && !arguments->children.empty()) {
    ...
    if (use_multiline && element_names.empty()) {
      ...
      for (size_t i = 0; i < arguments->children.size(); ++i) {
        ...
        if (i < element_names.size()) ostr << backQuoteIfNeed(element_names[i]) << ' ';
