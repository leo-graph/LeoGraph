...
bool ParserAlterCommand::parseImpl(Pos &pos, ASTPtr &node, Expected &expected) {
  ...
  ParserList parser_reset_setting(std::make_unique<ParserIdentifier>(), std::make_unique<ParserToken>(TokenType::Comma),
                                  /* allow_empty = */ false);
  ...
  if (with_round_bracket) {
    if (!parser_opening_round_bracket.ignore(pos, expected)) return false;
  }

  switch (alter_object) {
    case ASTAlterQuery::AlterObjectType::DATABASE: {
      if (s_modify_setting.ignore(pos, expected)) {
        if (!parser_settings.parse(pos, command_settings_changes, expected)) return false;
        command->type = ASTAlterCommand::MODIFY_DATABASE_SETTING;
      } else if (s_modify_comment.ignore(pos, expected)) {
        if (!parser_string_literal.parse(pos, command_comment, expected)) return false;
    ...
    case ASTAlterQuery::AlterObjectType::TABLE: {
      if (s_add_column.ignore(pos, expected)) {
        if (s_if_not_exists.ignore(pos, expected)) command->if_not_exists = true;

        if (!parser_col_decl.parse(pos, command_col_decl, expected)) return false;

        if (s_first.ignore(pos, expected))
          command->first = true;
        else if (s_after.ignore(pos, expected)) {
          if (!parser_name.parse(pos, command_column, expected)) return false;
        }

        command->type = ASTAlterCommand::ADD_COLUMN;
      } else if (s_rename_column.ignore(pos, expected)) {
        if (s_if_exists.ignore(pos, expected)) command->if_exists = true;

        if (!parser_name.parse(pos, command_column, expected)) return false;

        if (!s_to.ignore(pos, expected)) return false;

        if (!parser_name.parse(pos, command_rename_to, expected)) return false;
        ...
        if (s_in_partition.ignore(pos, expected)) {
          if (!parser_partition.parse(pos, command_partition, expected)) return false;
        }
        ...
        if (!parser_string_and_substituion.parse(pos, command_partition, expected)) return false;
        ...
        if (!parser_idx_decl.parse(pos, command_index_decl, expected)) return false;

        if (s_first.ignore(pos, expected))
          command->first = true;
        else if (s_after.ignore(pos, expected)) {
          if (!parser_name.parse(pos, command_index, expected)) return false;
        }
        ...
        if (!parser_stat_decl.parse(pos, command_statistics_decl, expected)) return false;
        ...
        if (!parser_stat_decl_without_types.parse(pos, command_statistics_decl, expected)) return false;
        ...
        if (!parser_projection_decl.parse(pos, command_projection_decl, expected)) return false;

        if (s_first.ignore(pos, expected))
          command->first = true;
        else if (s_after.ignore(pos, expected)) {
          if (!parser_name.parse(pos, command_projection, expected)) return false;
        }
        ...
        if (s_to_disk.ignore(pos, expected))
          ...
          if (!parseDatabaseAndTableName(pos, expected, command->to_database, command->to_table)) return false;
        ...
        if (!parser_constraint_decl.parse(pos, command_constraint_decl, expected)) return false;

        command->type = ASTAlterCommand::ADD_CONSTRAINT;
      } else if (s_drop_constraint.ignore(pos, expected)) {
        if (s_if_exists.ignore(pos, expected)) command->if_exists = true;

        if (!parser_name.parse(pos, command_constraint, expected)) return false;
        ...
        if (s_from.ignore(pos, expected)) {
          if (!parseDatabaseAndTableName(pos, expected, command->from_database, command->from_table)) return false;
        ...
        if (!s_from.ignore(pos, expected)) return false;
        ...
        if (s_from.ignore(pos, expected)) {
          ASTPtr ast_from;
          if (!parser_string_literal.parse(pos, ast_from, expected)) return false;
        ...
        if (s_with.ignore(pos, expected)) {
          if (!s_name.ignore(pos, expected)) return false;

          ASTPtr ast_with_name;
          if (!parser_string_literal.parse(pos, ast_with_name, expected)) return false;
        ...
        if (!parser_string_literal.parse(pos, ast_snapshot_name, expected)) return false;
        ...
        if (s_from.ignore(pos, expected)) {
          if (!ParserIdentifierWithOptionalParameters{}.parse(pos, command_snapshot_desc, expected)) return false;
        ...
        if (!is_modify) parser_modify_col_decl.enableCheckTypeKeyword();

        if (!parser_modify_col_decl.parse(pos, command_col_decl, expected)) return false;

        auto check_no_type = [&](const std::string_view keyword) {
          const auto &column_decl = command_col_decl->as<const ASTColumnDeclaration &>();

          if (column_decl.hasChildren() || column_decl.null_modifier.has_value() ||
              column_decl.default_specifier != ColumnDefaultSpecifier::Empty || column_decl.ephemeral_default ||
              column_decl.primary_key_specifier) {
        ...
        if (s_remove.ignore(pos, expected)) {
          ...
          if (!parser_reset_setting.parse(pos, command_settings_resets, expected)) return false;
        } else {
          if (s_first.ignore(pos, expected))
            command->first = true;
          else if (s_after.ignore(pos, expected)) {
        ...
        /// Make sure that type is not populated when REMOVE/MODIFY SETTING/RESET SETTING is used, because we wouldn't modify the type,
        /// which can be confusing
        chassert(nullptr == command_col_decl->as<const ASTColumnDeclaration &>().getType() ||
        ...
        if (!parser_exp_elem.parse(pos, command_order_by, expected)) return false;

        command->type = ASTAlterCommand::MODIFY_ORDER_BY;
      } else if (s_modify_sample_by.ignore(pos, expected)) {
        if (!parser_exp_elem.parse(pos, command_sample_by, expected)) return false;
        ...
        if (!s_where.ignore(pos, expected)) return false;

        if (!parser_exp_elem.parse(pos, command_predicate, expected)) return false;
        ...
        if (!command_predicate->tryGetAlias().empty()) return false;

        command->type = ASTAlterCommand::DELETE;
      } else if (s_update.ignore(pos, expected)) {
        if (!parser_assignment_list.parse(pos, command_update_assignments, expected)) return false;
        ...
        if (s_materialize.checkWithoutMoving(pos, expected) || s_remove.checkWithoutMoving(pos, expected) ||
            s_modify.checkWithoutMoving(pos, expected))
          return false;

        if (!parser_ttl_list.parse(pos, command_ttl, expected)) return false;
        ...
        if (!select_p.parse(pos, command_select, expected)) return false;
        ...
        if (!sql_security_p.parse(pos, command_sql_security, expected)) return false;
        ...
        if (!refresh_p.parse(pos, command->refresh, expected)) return false;
        ...
        if (!command_name_parser.parse(pos, command_name_ast, expected)) return false;
        command->execute_command_name = getIdentifierName(command_name_ast);

        if (!parser_opening_round_bracket.ignore(pos, expected)) return false;

        ASTPtr execute_args_list;
        ParserList args_parser(std::make_unique<ParserExpressionWithOptionalAlias>(false), std::make_unique<ParserToken>(TokenType::Comma),
                               /* allow_empty = */ true);
        if (!args_parser.parse(pos, execute_args_list, expected)) return false;

        if (!parser_closing_round_bracket.ignore(pos, expected)) return false;

        if (execute_args_list) command->execute_args = command->children.emplace_back(std::move(execute_args_list)).get();
      } else
        return false;
      break;
    }
  ...
  if (command_col_decl) command->col_decl = command->children.emplace_back(std::move(command_col_decl)).get();
  if (command_column) command->column = command->children.emplace_back(std::move(command_column)).get();
  if (command_order_by) command->order_by = command->children.emplace_back(std::move(command_order_by)).get();
  if (command_sample_by) command->sample_by = command->children.emplace_back(std::move(command_sample_by)).get();
  if (command_index_decl) command->index_decl = command->children.emplace_back(std::move(command_index_decl)).get();
  if (command_index) command->index = command->children.emplace_back(std::move(command_index)).get();
  if (command_constraint_decl) command->constraint_decl = command->children.emplace_back(std::move(command_constraint_decl)).get();
  if (command_constraint) command->constraint = command->children.emplace_back(std::move(command_constraint)).get();
  if (command_projection_decl) command->projection_decl = command->children.emplace_back(std::move(command_projection_decl)).get();
  if (command_projection) command->projection = command->children.emplace_back(std::move(command_projection)).get();
  if (command_statistics_decl) command->statistics_decl = command->children.emplace_back(std::move(command_statistics_decl)).get();
  if (command_partition) command->partition = command->children.emplace_back(std::move(command_partition)).get();
  if (command_predicate) command->predicate = command->children.emplace_back(std::move(command_predicate)).get();
  if (command_update_assignments) command->update_assignments = command->children.emplace_back(std::move(command_update_assignments)).get();
  if (command_comment) command->comment = command->children.emplace_back(std::move(command_comment)).get();
  if (command_ttl) command->ttl = command->children.emplace_back(std::move(command_ttl)).get();
  if (command_settings_changes) command->settings_changes = command->children.emplace_back(std::move(command_settings_changes)).get();
  if (command_settings_resets) command->settings_resets = command->children.emplace_back(std::move(command_settings_resets)).get();
  if (command_select) command->select = command->children.emplace_back(std::move(command_select)).get();
  if (command_sql_security) command->sql_security = command->children.emplace_back(std::move(command_sql_security)).get();
  if (command_rename_to) command->rename_to = command->children.emplace_back(std::move(command_rename_to)).get();
  if (command_snapshot_desc) command->snapshot_desc = command->children.emplace_back(std::move(command_snapshot_desc)).get();
...
bool ParserAlterQuery::parseImpl(Pos &pos, ASTPtr &node, Expected &expected) {
  ...
  if (alter_object_type == ASTAlterQuery::AlterObjectType::DATABASE) {
    if (!parseDatabaseAsAST(pos, expected, query->database)) return false;
  ...
  ASTPtr command_list;
  if (!p_command_list.parse(pos, command_list, expected)) return false;
