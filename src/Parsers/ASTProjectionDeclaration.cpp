...
ASTPtr ASTProjectionDeclaration::clone() const {
  ...
  res->name = name;
  if (query) res->set(res->query, query->clone());
  if (index) res->set(res->index, index->clone());
  if (type) res->set(res->type, type->clone());
  if (with_settings) res->set(res->with_settings, with_settings->clone());
  return res;
}

void ASTProjectionDeclaration::formatImpl(WriteBuffer& ostr, const FormatSettings& settings, FormatState& state,
