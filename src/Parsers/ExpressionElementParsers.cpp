...
bool ParserSubquery::parseImpl(Pos &pos, ASTPtr &node, Expected &expected) {
  ...
  if (pos->type != TokenType::OpeningRoundBracket) return false;
  ++pos;
  ...
  if (ASTPtr select_node; select.parse(pos, select_node, expected)) {
    ...
    if (ASTPtr settings_ast = explain_query.getSettings()) {
      if (!settings_ast->as<ASTSetQuery>()) throw Exception(ErrorCodes::BAD_ARGUMENTS, "EXPLAIN settings must be a SET query");
    ...
    if (explained_ast) {
      ...
      auto view_explain = makeASTFunction("viewExplain", make_intrusive<ASTLiteral>(kind_str), make_intrusive<ASTLiteral>(settings_str),
  ...
  if (pos->type != TokenType::ClosingRoundBracket) return false;
  ++pos;
...
bool ParserTableAsStringLiteralIdentifier::parseImpl(Pos &pos, ASTPtr &node, Expected &expected) {
  if (pos->type != TokenType::StringLiteral) return false;
...
class ParserArrayOfJSONIdentifierAddition : public IParserBase {
 public:
  String getLastArrayOfJSONSubcolumnIdentifier() const {
    String subcolumn = ":`";
    for (size_t i = 0; i != last_array_level; ++i) subcolumn += "Array(";
    subcolumn += "JSON";
    for (size_t i = 0; i != last_array_level; ++i) subcolumn += ")";
    return subcolumn + "`";
  }
 ...
 protected:
  ...
  bool parseImpl(Pos &pos, ASTPtr & /*node*/, Expected &expected) override {
    ...
    if (!brackets_parser.check(pos, expected)) return false;
    ++last_array_level;
    while (brackets_parser.check(pos, expected)) ++last_array_level;
    return true;
  }
...
bool ParserCompoundIdentifier::parseImpl(Pos &pos, ASTPtr &node, Expected &expected) {
  ...
  delimiter_parsers.emplace_back(std::make_unique<ParserTokenSequence>(std::vector<TokenType>{TokenType::Dot, TokenType::Colon}),
                                 SpecialDelimiter::JSON_PATH_DYNAMIC_TYPE);
  delimiter_parsers.emplace_back(std::make_unique<ParserTokenSequence>(std::vector<TokenType>{TokenType::Dot, TokenType::Caret}),
  ...
  while (true) {
    ...
    if (parts.back().empty()) params.push_back(element->as<ASTIdentifier>()->getParam());
...
bool ParserWindowReference::parseImpl(Pos &pos, ASTPtr &node, Expected &expected) {
  ...
  if (function.window_definition) function.children.push_back(function.window_definition);
  return res;
}
...
bool ParserCodec::parseImpl(Pos &pos, ASTPtr &node, Expected &expected) {
  ...
  ++pos;
  if (!codecs.parse(pos, expr_list_args, expected)) return false;
...
bool ParserStatisticsType::parseImpl(Pos &pos, ASTPtr &node, Expected &expected) {
  ParserList stat_type_parser(std::make_unique<ParserIdentifierWithOptionalParameters>(), std::make_unique<ParserToken>(TokenType::Comma),
                              false);
  ...
  if (!stat_type_parser.parse(pos, stat_type, expected)) return false;
...
bool ParserCollation::parseImpl(Pos &pos, ASTPtr &node, Expected &expected) {
  ...
  if (!ParserIdentifier(true).parse(pos, collation, expected)) return false;
  ...
  if (!valid_collation) return false;
...
bool ParserCastOperator::parseImpl(Pos &pos, ASTPtr &node, Expected &expected) {
  ...
  if (pos->type == Minus) {
    ++pos;
    if (pos->type != Number) return false;
    ...
    if (!ParserStringLiteral().parse(begin, string_literal, expected)) return false;
    ...
    while (pos.isValid()) {
      if (isOneOf<OpeningSquareBracket, OpeningRoundBracket>(pos->type)) {
        ...
        if (!isOneOf<OpeningSquareBracket, OpeningRoundBracket, Comma>(last_token)) return false;
      } else if (pos->type == ClosingSquareBracket) {
        if (isOneOf<Comma, OpeningRoundBracket, Minus>(last_token)) return false;
        if (stack.empty() || stack.back() != OpeningSquareBracket) return false;
        stack.pop_back();
      } else if (pos->type == ClosingRoundBracket) {
        if (isOneOf<Comma, OpeningSquareBracket, Minus>(last_token)) return false;
        if (stack.empty() || stack.back() != OpeningRoundBracket) return false;
        stack.pop_back();
      } else if (pos->type == Comma) {
        if (isOneOf<OpeningSquareBracket, OpeningRoundBracket, Comma, Minus>(last_token)) return false;
...
bool ParserNumber::parseImpl(Pos &pos, ASTPtr &node, Expected &expected) {
  ...
  auto try_read_float = [&](const char *it, const char *end) {
    ...
    if (str_end == buf.c_str() + buf.size() && !overflow) {
      ...
      if (std::isnan(float_value)) float_value = std::numeric_limits<Float64>::quiet_NaN();
...
inline static bool makeHexOrBinStringLiteral(IParser::Pos &pos, ASTPtr &node, bool hex, size_t word_size, Expected &expected) {
  ...
  if (str_begin == str_end) return makeStringLiteral(pos, node, "", expected);
...
bool ParserStringLiteral::parseImpl(Pos &pos, ASTPtr &node, Expected &expected) {
  if (pos->type != TokenType::StringLiteral && pos->type != TokenType::HereDoc) return false;
...
struct CollectionOfLiteralsLayer {
  explicit CollectionOfLiteralsLayer(IParser::Pos &pos) : literal_begin(pos) { ++pos; }
...
bool ParserCollectionOfLiterals<Collection>::parseImpl(Pos &pos, ASTPtr &node, Expected &expected) {
  if (pos->type != opening_bracket) return false;
  ...
  while (pos.isValid()) {
    if (!layers.back().arr.empty()) {
      if (pos->type == closing_bracket) {
        ...
        if (std::is_same_v<Collection, Tuple> && layers.back().arr.size() == 1) return false;
...
bool CommonCollection<Container, end_token>::parse(IParser::Pos &pos, Collections &collections, ASTPtr &node, Expected &expected,
  ...
  while (true) {
    ...
    if (!container.empty() && !comma_p.ignore(pos, expected)) return false;
...
bool MapCollection::parse(IParser::Pos &pos, Collections &collections, ASTPtr &node, Expected &expected, bool allow_map) {
  ...
  while (true) {
    ...
    if (!literal_p.parse(pos, literal, expected)) return false;

    if (!colon_p.parse(pos, literal, expected)) return false;
...
bool ParserAllCollectionsOfLiterals::parseImpl(Pos &pos, ASTPtr &node, Expected &expected) {
  ...
  if (!parseAllCollectionsStart(pos, collections, expected, allow_map)) return false;

  while (!collections.empty()) {
    if (!collections.back()->parse(pos, collections, node, expected, allow_map)) return false;

    if (node) collections.pop_back();
  }
...
bool ParserLiteral::parseImpl(Pos &pos, ASTPtr &node, Expected &expected) {
  ...
  if (null_p.parse(pos, node, expected)) return true;

  if (num_p.parse(pos, node, expected)) return true;

  if (bool_p.parse(pos, node, expected)) return true;

  if (str_p.parse(pos, node, expected)) return true;
...
bool ParserAlias::parseImpl(Pos &pos, ASTPtr &node, Expected &expected) {
  ...
  if (!allow_alias_without_as_keyword && !has_as_word) return false;

  bool is_quoted = pos->type == TokenType::QuotedIdentifier;

  if (!id_p.parse(pos, node, expected)) return false;
  ...
  if (!has_as_word && !is_quoted) {
    ...
    for (const char **keyword = restricted_keywords; *keyword != nullptr; ++keyword)
      if (0 == strcasecmp(name.data(), *keyword)) return false;
  }
...
bool ParserColumnsTransformers::parseImpl(Pos &pos, ASTPtr &node, Expected &expected) {
  ...
  if (allowed_transformers.isSet(ColumnTransformer::APPLY) && apply.ignore(pos, expected)) {
    ...
    if (!lambda) {
      if (!ParserIdentifier().parse(pos, func_name, expected)) return false;

      if (pos->type == TokenType::OpeningRoundBracket) {
        ++pos;
        if (!ParserExpressionList(false).parse(pos, expr_list_args, expected)) return false;
    ...
    if (with_open_round_bracket && pos->type == TokenType::Comma) {
      ...
      if (!parser_string_literal.parse(pos, ast_prefix_name, expected)) return false;
  ...
  if (allowed_transformers.isSet(ColumnTransformer::EXCEPT) && except.ignore(pos, expected)) {
    if (strict.ignore(pos, expected)) is_strict = true;
    ...
    auto parse_id = [&identifiers, &pos, &expected] {
      ASTPtr identifier;
      if (!ParserIdentifier(true).parse(pos, identifier, expected)) return false;
    ...
    if (pos->type == TokenType::OpeningRoundBracket) {
      ...
      ++pos;
      if (!ParserList::parseUtil(pos, expected, parse_id, false) && !regex.parse(pos, regexp_node, expected)) return false;

      if (pos->type != TokenType::ClosingRoundBracket) return false;
      ++pos;
    } else {
      // only one parameter
      if (!parse_id() && !regex.parse(pos, regexp_node, expected)) return false;
    }
  ...
  if (allowed_transformers.isSet(ColumnTransformer::REPLACE) && replace.ignore(pos, expected)) {
    ...
    auto parse_id = [&] {
      ...
      if (!element_p.parse(pos, expr, expected)) return false;
      if (!as.ignore(pos, expected)) return false;

      ASTPtr ident;
      if (!ident_p.parse(pos, ident, expected)) return false;
    ...
    if (pos->type == TokenType::OpeningRoundBracket) {
      ...
      if (!ParserList::parseUtil(pos, expected, parse_id, false)) return false;
...
bool ParserQualifiedAsterisk::parseImpl(Pos &pos, ASTPtr &node, Expected &expected) {
  if (!ParserCompoundIdentifier(false, true).parse(pos, node, expected)) return false;

  if (pos->type != TokenType::Dot) return false;
  ++pos;

  if (pos->type != TokenType::Asterisk) return false;
  ++pos;
...
static bool parseColumnsMatcherBody(IParser::Pos &pos, ASTPtr &node, Expected &expected,
                                    ParserColumnsTransformers::ColumnTransformers allowed_transformers) {
  if (pos->type != TokenType::OpeningRoundBracket) return false;
  ++pos;
  ...
  ASTPtr column_list;
  ASTPtr regexp_node;
  if (!columns_p.parse(pos, column_list, expected) && !regexp.parse(pos, regexp_node, expected)) return false;
...
bool ParserColumnsMatcher::parseImpl(Pos &pos, ASTPtr &node, Expected &expected) {
  ...
  if (!columns.ignore(pos, expected)) return false;
...
bool ParserQualifiedColumnsMatcher::parseImpl(Pos &pos, ASTPtr &node, Expected &expected) {
  ...
  if (name_parts.size() == 1 || name_parts.back() != "COLUMNS") return false;

  name_parts.pop_back();
  identifier_node = make_intrusive<ASTIdentifier>(std::move(name_parts), false, std::move(node->children));

  if (!parseColumnsMatcherBody(pos, node, expected, allowed_transformers)) return false;
...
bool ParserSubstitution::parseImpl(Pos &pos, ASTPtr &node, Expected &expected) {
  if (pos->type != TokenType::OpeningCurlyBrace) return false;
...
bool ParserMySQLGlobalVariable::parseImpl(Pos &pos, ASTPtr &node, Expected &expected) {
  if (pos->type != TokenType::DoubleAt) return false;
...
bool ParserWithOptionalAlias::parseImpl(Pos &pos, ASTPtr &node, Expected &expected) {
  if (!elem_parser->parse(pos, node, expected)) return false;
   ...
   * For example, a column may be called where. And in the query it can be written `SELECT where AS x FROM table` or even `SELECT where x
   * FROM table`. Even can be written `SELECT where AS from FROM table`, but it can not be written `SELECT where from FROM table`. See the
   * ParserAlias implementation for details.
   *
  ...
  if (allow_alias_without_as_keyword)
    if (auto opt_id = tryGetIdentifierName(node))
      if (0 == strcasecmp(opt_id->data(), "FROM")) allow_alias_without_as_keyword_now = false;
...
bool ParserStorageOrderByElement::parseImpl(Pos &pos, ASTPtr &node, Expected &expected) {
  ...
  ASTPtr expr_elem;
  if (!elem_p.parse(pos, expr_elem, expected)) return false;
  ...
  if (!expr_elem->tryGetAlias().empty()) return false;
...
bool ParserOrderByElement::parseImpl(Pos &pos, ASTPtr &node, Expected &expected) {
  ...
  if (collate.ignore(pos, expected)) {
    if (!collate_locale_parser.parse(pos, locale_node, expected)) return false;
  }
  ...
  if (with_fill.ignore(pos, expected)) {
    has_with_fill = true;
    if (from.ignore(pos, expected) && !exp_parser.parse(pos, fill_from, expected)) return false;

    if (to.ignore(pos, expected) && !exp_parser.parse(pos, fill_to, expected)) return false;

    if (step.ignore(pos, expected) && !exp_parser.parse(pos, fill_step, expected)) return false;

    if (staleness.ignore(pos, expected) && !exp_parser.parse(pos, fill_staleness, expected)) return false;
  }
...
bool ParserFunctionWithKeyValueArguments::parseImpl(Pos &pos, ASTPtr &node, Expected &expected) {
  ...
  if (pos.get().type != TokenType::OpeningRoundBracket) {
    if (!brackets_can_be_omitted) return false;
  } else {
    ++pos;
    left_bracket_found = true;
  }

  if (!pairs_list_parser.parse(pos, expr_list_args, expected)) return false;

  if (left_bracket_found) {
    if (pos.get().type != TokenType::ClosingRoundBracket) return false;
    ++pos;
  }
...
bool ParserTTLElement::parseImpl(Pos &pos, ASTPtr &node, Expected &expected) {
  ...
  ASTPtr ttl_expr;
  if (!parser_exp.parse(pos, ttl_expr, expected)) return false;
  ...
  if (mode == TTLMode::MOVE) {
    if (s_if_exists.ignore(pos, expected)) if_exists = true;

    ASTPtr ast_space_name;
    if (!parser_string_literal.parse(pos, ast_space_name, expected)) return false;

    destination_name = ast_space_name->as<ASTLiteral &>().value.safeGet<String>();
  } else if (mode == TTLMode::GROUP_BY) {
    if (!parser_keys_list.parse(pos, group_by_key, expected)) return false;
    ...
    if (s_set.ignore(pos, expected)) {
      ...
      if (!parser_assignment_list.parse(pos, group_by_assignments, expected)) return false;
    }
  } else if (mode == TTLMode::DELETE && s_where.ignore(pos, expected)) {
    if (!parser_exp.parse(pos, where_expr, expected)) return false;
  } else if (mode == TTLMode::RECOMPRESS) {
    if (!s_codec.ignore(pos, expected)) return false;

    if (!parser_codec.parse(pos, recompression_codec, expected)) return false;
  }
  ...
  if (where_expr) ttl_element->setWhere(std::move(where_expr));

  if (mode == TTLMode::GROUP_BY) {
    ttl_element->group_by_key = std::move(group_by_key->children);
    if (group_by_assignments) ttl_element->group_by_assignments = std::move(group_by_assignments->children);
  }

  if (mode == TTLMode::RECOMPRESS) ttl_element->recompression_codec = recompression_codec;
...
bool ParserAssignment::parseImpl(Pos &pos, ASTPtr &node, Expected &expected) {
  ...
  ASTPtr column;
  if (!p_identifier.parse(pos, column, expected)) return false;

  if (!s_equals.ignore(pos, expected)) return false;

  ASTPtr expression;
  if (!p_expression.parse(pos, expression, expected)) return false;

  tryGetIdentifierNameInto(column, assignment->column_name);
  if (expression) assignment->children.push_back(expression);
