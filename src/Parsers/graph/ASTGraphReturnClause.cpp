...
void ASTGraphReturnClause::formatImpl(WriteBuffer &ostr, const FormatSettings &settings, FormatState &state,
  ...
  bool first = true;
  for (const auto &child : children) {
    if (child.get() == group_by || child.get() == order_by || child.get() == limit || child.get() == offset) continue;
