...
ASTPtr ASTGraphReturnItem::clone() const
{
    ...
    if (expression)
        res->setExpression(expression->clone());

    return res;
}

void ASTGraphReturnItem::formatImplWithoutAlias(WriteBuffer & ostr, const FormatSettings & settings, FormatState & state, FormatStateStacked frame) const
{
    if (expression)
...
void ASTGraphReturnItem::forEachPointerToChild(std::function<void(IAST **, boost::intrusive_ptr<IAST> *)> f)
{
    f(&expression, nullptr);
}
