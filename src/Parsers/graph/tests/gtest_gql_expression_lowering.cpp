...
const ActionsDAG::Node & lower(const ASTPtr & ast, ActionsDAG & dag)
{
    return GQL::lowerExpression(asGQLExpr(ast), dag, getLoweringContext());
}
...
TEST(GQLExpressionLowering, IdentifierResolvesExistingUInt64InputColumn)
{
    auto dag = makeDagWithInputs({{"n", std::make_shared<DataTypeUInt64>()}});

    const auto & node = lower(GAST::GQLExpr::identifier("n"), dag);
...
TEST(GQLExpressionLowering, PropertyAccessLowersToTupleElementBeforeComparison)
{
...
TEST(GQLExpressionLowering, NotLowersBooleanExpressionTree)
{
...
TEST(GQLExpressionLowering, UnsupportedKindThrowsNotImplemented)
{
    ...
    try
    {
        (void)lower(GAST::GQLExpr::functionCall("labels", {}), dag);
