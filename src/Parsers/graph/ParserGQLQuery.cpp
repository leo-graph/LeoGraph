...
bool isASCIISpace(char c) { return std::isspace(static_cast<unsigned char>(c)); }
...
std::string_view makeSingleStatementView(const char* begin, const char* end) {
  ...
  while (!query.empty() && isASCIISpace(query.front())) query.remove_prefix(1);

  while (!query.empty() && isASCIISpace(query.back())) query.remove_suffix(1);
...
ASTPtr tryParseGQLQuery(ParserGQLQuery& parser, const char*& out_query_end, const char* end, std::string& out_error_message,
                        int* out_error_code, bool hilite, const std::string& description, bool allow_multi_statements,
                        size_t max_query_size, size_t max_parser_depth, size_t max_parser_backtracks) {
  (void)hilite;
  (void)description;
  ...
  try {
    ...
    if (!res) throw Exception(ErrorCodes::SYNTAX_ERROR, "Cannot parse GQL query");
...
ASTPtr parseGQLQueryAndMovePosition(ParserGQLQuery& parser, const char*& pos, const char* end, const std::string& query_description,
...
ASTPtr parseGQLQuery(ParserGQLQuery& parser, const char* begin, const char* end, const std::string& query_description,
                     size_t max_query_size, size_t max_parser_depth, size_t max_parser_backtracks) {
  return parseGQLQueryAndMovePosition(parser, begin, end, query_description, false, max_query_size, max_parser_depth,
                                      max_parser_backtracks);
}

ASTPtr parseGQLQuery(ParserGQLQuery& parser, const std::string& query, const std::string& query_description, size_t max_query_size,
