...
bool ParserQueryWithOutput::parseImpl(Pos &pos, ASTPtr &node, Expected &expected) {
  ...
  bool parsed = explain_p.parse(pos, query, expected) || select_p.parse(pos, query, expected) ||
                ...
                table_p.parse(pos, query, expected) || describe_cache_p.parse(pos, query, expected) ||
                describe_table_p.parse(pos, query, expected) || show_processlist_p.parse(pos, query, expected) ||
                create_p.parse(pos, query, expected) || alter_p.parse(pos, query, expected) || rename_p.parse(pos, query, expected) ||
                drop_p.parse(pos, query, expected) || undrop_p.parse(pos, query, expected) || check_p.parse(pos, query, expected) ||
                kill_query_p.parse(pos, query, expected) || optimize_p.parse(pos, query, expected) || watch_p.parse(pos, query, expected) ||
                show_access_p.parse(pos, query, expected) || show_access_entities_p.parse(pos, query, expected) ||
                show_grants_p.parse(pos, query, expected) || show_privileges_p.parse(pos, query, expected) ||
  ...
  if (s_into_outfile.ignore(pos, expected)) {
    ...
    if (!out_file_p.parse(pos, query_with_output.out_file, expected)) return false;
    ...
    if (s_compression_method.ignore(pos, expected)) {
      ...
      if (!compression.parse(pos, query_with_output.compression, expected)) return false;
      ...
      if (s_compression_level.ignore(pos, expected)) {
        ...
        if (!compression_level.parse(pos, query_with_output.compression_level, expected)) return false;
  ...
  for (size_t i = 0; i < 2; ++i) {
    if (!query_with_output.format_ast && s_format.ignore(pos, expected)) {
      ...
      if (!format_p.parse(pos, query_with_output.format_ast, expected)) return false;
      ...
      if (!parser_settings.parse(pos, query_with_output.settings_ast, expected)) return false;
  ...
  if (query_with_output.format_ast && query_with_output.settings_ast) {
    ...
    if (fmt_it != ch.end() && set_it != ch.end() && set_it < fmt_it) std::iter_swap(fmt_it, set_it);
  }
