...
struct ASTCheckTableQuery : public ASTQueryWithTableAndOutput {
  ...
  std::variant<std::monostate, ASTPtr, String> getPartitionOrPartitionID() const {
    ...
    if (!part_name.empty()) return part_name;
 ...
 protected:
  void formatQueryImpl(WriteBuffer& ostr, const FormatSettings& settings, FormatState& state, FormatStateStacked frame) const override {
    ...
    if (!part_name.empty()) {
      ostr << indent_str << " PART " << quoteString(part_name);
    }
  }
};
...
struct ASTCheckAllTablesQuery : public ASTQueryWithOutput {
 ...
 protected:
  void formatQueryImpl(WriteBuffer& ostr, const FormatSettings& settings, FormatState& /* state */,
