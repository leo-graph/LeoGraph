...
bool isUnquotedIdentifier(const String& name) {
  ...
  if (is_keyword(Keyword::NULL_KEYWORD) || is_keyword(Keyword::TRUE_KEYWORD) || is_keyword(Keyword::FALSE_KEYWORD)) return false;

  Lexer lexer(name.data(), name.data() + name.size());

  auto maybe_ident = lexer.nextToken();

  if (maybe_ident.type != TokenType::BareWord) return false;
