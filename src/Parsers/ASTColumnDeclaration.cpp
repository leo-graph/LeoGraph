...
const char *toString(ColumnDefaultSpecifier kind) {
  switch (kind) {
    ...
    case ColumnDefaultSpecifier::Alias:
      return "ALIAS";
    case ColumnDefaultSpecifier::Ephemeral:
      return "EPHEMERAL";
...
ColumnDefaultSpecifier toColumnDefaultSpecifier(ColumnDefaultKind kind) {
  switch (kind) {
    case ColumnDefaultKind::Default:
      return ColumnDefaultSpecifier::Default;
    case ColumnDefaultKind::Materialized:
      return ColumnDefaultSpecifier::Materialized;
    case ColumnDefaultKind::Alias:
      return ColumnDefaultSpecifier::Alias;
    case ColumnDefaultKind::Ephemeral:
      return ColumnDefaultSpecifier::Ephemeral;
  }
}
...
ColumnDefaultKind toColumnDefaultKind(ColumnDefaultSpecifier specifier) {
  switch (specifier) {
    ...
    case ColumnDefaultSpecifier::Materialized:
      return ColumnDefaultKind::Materialized;
    case ColumnDefaultSpecifier::Alias:
      return ColumnDefaultKind::Alias;
    case ColumnDefaultSpecifier::Ephemeral:
      return ColumnDefaultKind::Ephemeral;
  }
}
...
ASTPtr ASTColumnDeclaration::clone() const {
  ...
  if (auto node = getType()) res->setType(node->clone());
  if (auto node = getDefaultExpression()) res->setDefaultExpression(node->clone());
  if (auto node = getComment()) res->setComment(node->clone());
  if (auto node = getCodec()) res->setCodec(node->clone());
  if (auto node = getStatisticsDesc()) res->setStatisticsDesc(node->clone());
  if (auto node = getTTL()) res->setTTL(node->clone());
  if (auto node = getCollation()) res->setCollation(node->clone());
  if (auto node = getSettings()) res->setSettings(node->clone());

  return res;
}

void ASTColumnDeclaration::formatImpl(WriteBuffer &ostr, const FormatSettings &format_settings, FormatState &state,
...
void ASTColumnDeclaration::forEachPointerToChild(std::function<void(IAST **, boost::intrusive_ptr<IAST> *)> f) {
  auto callIfSet = [&](IndexSlot slot) {
    ...
    if (idx != kNotSet) f(nullptr, &children[idx]);
  };
  callIfSet(TYPE);
