...
bool ParserCopyQuery::parseImpl(Pos& pos, ASTPtr& node, Expected& expected) {
  ...
  if (!s_copy.ignore(pos, expected)) return false;
  ...
  if (!open_bracket.ignore(pos, expected)) {
    ...
    ASTPtr table_name;
    if (!s_table_identifier.parse(pos, table_name, expected)) return false;
    ...
    if (open_bracket.ignore(pos, expected)) {
      ...
      for (const auto& column_ast : columns->children) copy_element->column_names.push_back(column_ast->as<ASTIdentifier>()->full_name);
    }
    saved_pos = pos;
...
bool ParserCopyQuery::parseOptions(Pos& pos, boost::intrusive_ptr<ASTCopyQuery> node, Expected& expected) {
  ...
  ASTPtr output_name;
  if (!s_output_identifier.parse(pos, output_name, expected)) return false;
  ...
  if (s_format.ignore(pos, expected)) {
    ...
    ASTPtr format;
    if (!s_format_identifier.parse(pos, format, expected)) return false;
