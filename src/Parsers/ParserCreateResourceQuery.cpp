...
bool parseOneOperation(ASTCreateResourceQuery::Operation& operation, IParser::Pos& pos, Expected& expected) {
  ...
  if (mode == ResourceAccessMode::DiskWrite || mode == ResourceAccessMode::DiskRead) {
    if (ParserKeyword(Keyword::ANY).ignore(pos, expected)) {
      if (!ParserKeyword(Keyword::DISK).ignore(pos, expected)) return false;
    } else {
      if (!ParserKeyword(Keyword::DISK).ignore(pos, expected)) return false;

      if (!disk_name_p.parse(pos, node, expected)) return false;

      disk.emplace();
      if (!tryGetIdentifierNameInto(node, *disk)) return false;
    }
  }
...
bool parseOperations(IParser::Pos& pos, Expected& expected, ASTCreateResourceQuery::Operations& operations) {
  return IParserBase::wrapParseImpl(pos, [&] {
    ...
    if (!ParserList::parseUtil(pos, expected, parse_operation, false)) return false;

    if (!s_close.ignore(pos, expected)) return false;
...
bool ParserCreateResourceQuery::parseImpl(IParser::Pos& pos, ASTPtr& node, Expected& expected) {
  ...
  if (!s_resource.ignore(pos, expected)) return false;

  if (!or_replace && s_if_not_exists.ignore(pos, expected)) if_not_exists = true;

  if (!resource_name_p.parse(pos, resource_name, expected)) return false;
  ...
  if (!parseOperations(pos, expected, operations)) return false;
