  ...
  for (const auto &change : changes) {
    ...
    auto format_if_secret = [&]() -> bool {
      ...
      if (change.name == format_avro_schema_registry_url) {
        ...
        if (!maybe_uri || maybe_uri->getUserInfo().empty()) return false;
    ...
    if (format.show_secrets || !format_if_secret()) ostr << " = " << applyVisitor(FieldVisitorToSetting(), change.value);
  }
...
bool ASTSetQuery::hasSecretParts() const {
  for (const auto &change : changes) {
    CustomType custom;
    if (change.value.tryGet<CustomType>(custom) && custom.isSecret()) return true;
    if (DataLake::SETTINGS_TO_HIDE.contains(change.name)) return true;
    if (RabbitMQ::SETTINGS_TO_HIDE.contains(change.name)) return true;
    if (NATS::SETTINGS_TO_HIDE.contains(change.name)) return true;
    if (Kafka::SETTINGS_TO_HIDE.contains(change.name)) return true;
    if (AzureQueue::SETTINGS_TO_HIDE.contains(change.name)) return true;
    if (S3Queue::SETTINGS_TO_HIDE.contains(change.name)) return true;

    if (change.name == format_avro_schema_registry_url) {
      const auto maybe_uri = tryParseURI(change.value.safeGet<String>());
      if (maybe_uri && !maybe_uri->getUserInfo().empty()) return true;
    }
  }
  return false;
}
