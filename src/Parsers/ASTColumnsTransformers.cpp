...
void ASTColumnsTransformerList::formatImpl(WriteBuffer &ostr, const FormatSettings &settings, FormatState &state,
...
void ASTColumnsApplyTransformer::formatImpl(WriteBuffer &ostr, const FormatSettings &settings, FormatState &state,
                                            FormatStateStacked frame) const {
  ostr << "APPLY" << " ";

  if (!column_name_prefix.empty()) ostr << "(";
  ...
  if (!column_name_prefix.empty()) ostr << ", '" << column_name_prefix << "')";
}
...
void ASTColumnsApplyTransformer::transform(ASTs &nodes) const {
  for (auto &column : nodes) {
    ...
    if (!column_name_prefix.empty()) column->setAlias(column_name_prefix + name);
  }
}

void ASTColumnsApplyTransformer::appendColumnName(WriteBuffer &ostr) const {
  writeCString("APPLY ", ostr);
  if (!column_name_prefix.empty()) writeChar('(', ostr);
  ...
  if (lambda)
    ...
    if (parameters) parameters->appendColumnName(ostr);
  }
...
void ASTColumnsApplyTransformer::updateTreeHashImpl(SipHash &hash_state, bool ignore_aliases) const {
  ...
  if (parameters) parameters->updateTreeHashImpl(hash_state, ignore_aliases);

  if (lambda) lambda->updateTreeHashImpl(hash_state, ignore_aliases);
...
void ASTColumnsExceptTransformer::formatImpl(WriteBuffer &ostr, const FormatSettings &settings, FormatState &state,
                                             FormatStateStacked frame) const {
  ostr << "EXCEPT" << (is_strict ? " STRICT " : " ");

  if (children.size() > 1) ostr << "(";
  ...
  if (pattern) ostr << quoteString(*pattern);
...
void ASTColumnsExceptTransformer::appendColumnName(WriteBuffer &ostr) const {
  ...
  if (is_strict) writeCString("STRICT ", ostr);

  if (children.size() > 1) writeChar('(', ostr);

  for (ASTs::const_iterator it = children.begin(); it != children.end(); ++it) {
    if (it != children.begin()) writeCString(", ", ostr);
    (*it)->appendColumnName(ostr);
  }

  if (pattern) writeQuotedString(*pattern, ostr);
...
void ASTColumnsExceptTransformer::transform(ASTs &nodes) const {
  ...
  if (!pattern) {
    for (const auto &child : children) expected_columns.insert(child->as<const ASTIdentifier &>().name());
...
void ASTColumnsExceptTransformer::setPattern(String pattern_) { pattern = std::move(pattern_); }
...
void ASTColumnsReplaceTransformer::Replacement::formatImpl(WriteBuffer &ostr, const FormatSettings &settings, FormatState &state,
                                                           FormatStateStacked frame) const {
...
void ASTColumnsReplaceTransformer::formatImpl(WriteBuffer &ostr, const FormatSettings &settings, FormatState &state,
                                              FormatStateStacked frame) const {
  ostr << "REPLACE" << (is_strict ? " STRICT " : " ");

  ostr << "(";
  for (ASTs::const_iterator it = children.begin(); it != children.end(); ++it) {
    if (it != children.begin()) ostr << ", ";
...
void ASTColumnsReplaceTransformer::replaceChildren(ASTPtr &node, const ASTPtr &replacement, const String &name) {
  for (auto &child : node->children) {
    if (const auto *id = child->as<ASTIdentifier>()) {
      if (id->shortName() == name) child = replacement->clone();
    } else
...
void ASTColumnsReplaceTransformer::transform(ASTs &nodes) const {
  ...
  if (is_strict && !replace_map.empty()) {
    ...
    for (auto &elem : replace_map) {
      if (!expected_columns.empty()) expected_columns += ", ";
      expected_columns += elem.first;
    }
    throw Exception(ErrorCodes::NO_SUCH_COLUMN_IN_TABLE, "Columns transformer REPLACE expects following column(s) : {}", expected_columns);
  }
}
