...
bool ParserAlterNamedCollectionQuery::parseImpl(IParser::Pos& pos, ASTPtr& node, Expected& expected) {
  ...
  if (!s_alter.ignore(pos, expected)) return false;

  if (!s_collection.ignore(pos, expected)) return false;
  ...
  if (s_delete.ignore(pos, expected)) {
    while (true) {
      if (!delete_keys.empty() && !s_comma.ignore(pos)) break;

      ASTPtr key;
      if (!name_p.parse(pos, key, expected)) return false;
