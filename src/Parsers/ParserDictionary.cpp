...
bool ParserDictionaryLifetime::parseImpl(Pos &pos, ASTPtr &node, Expected &expected) {
  ...
  if (!key_value_pairs_p.parse(pos, ast_lifetime, expected)) return false;

  const ASTExpressionList &expr_list = ast_lifetime->as<const ASTExpressionList &>();
  if (expr_list.children.size() != 2) return false;
  ...
  if (!initialized_max) return false;
...
bool ParserDictionaryRange::parseImpl(Pos &pos, ASTPtr &node, Expected &expected) {
  ParserKeyValuePairsList key_value_pairs_p;
  ASTPtr ast_range;
  if (!key_value_pairs_p.parse(pos, ast_range, expected)) return false;
  ...
  if (res->min_attr_name.empty() || res->max_attr_name.empty()) return false;
...
bool ParserDictionaryLayout::parseImpl(Pos &pos, ASTPtr &node, Expected &expected) {
  ...
  ASTPtr ast_func;
  if (!key_value_func_p.parse(pos, ast_func, expected)) return false;
  ...
  if (func.children.size() > 1) return false;
  ...
  if (!type_expr_list.children.empty() && !res->has_brackets) return false;
...
bool ParserDictionary::parseImpl(Pos &pos, ASTPtr &node, Expected &expected) {
  ...
  if (primary_key_keyword.ignore(pos, expected)) {
    ...
    if (open.ignore(pos, expected)) was_open = true;

    if (!expression_list_p.parse(pos, primary_key, expected)) return false;

    if (was_open && !close.ignore(pos, expected)) return false;
  }
  ...
  while (true) {
    if (!ast_source && source_keyword.ignore(pos, expected)) {
      ...
      if (!key_value_pairs_p.parse(pos, ast_source, expected)) return false;
    ...
    if (!ast_lifetime && lifetime_keyword.ignore(pos, expected)) {
      ...
      if (!lifetime_p.parse(pos, ast_lifetime, expected)) return false;
    ...
    if (!ast_layout && layout_keyword.ignore(pos, expected)) {
      ...
      if (!layout_p.parse(pos, ast_layout, expected)) return false;
    ...
    if (!ast_range && range_keyword.ignore(pos, expected)) {
      ...
      if (!range_p.parse(pos, ast_range, expected)) return false;
    ...
    if (!ast_settings && settings_keyword.ignore(pos, expected)) {
      ...
      if (!settings_p.parse(pos, ast_settings, expected)) return false;
  ...
  node = query;
  if (primary_key) query->set(query->primary_key, primary_key);

  if (ast_source) query->set(query->source, ast_source);

  if (ast_lifetime) query->set(query->lifetime, ast_lifetime);

  if (ast_layout) query->set(query->layout, ast_layout);

  if (ast_range) query->set(query->range, ast_range);

  if (ast_settings) query->set(query->dict_settings, ast_settings);
