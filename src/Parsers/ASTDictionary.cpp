...
void ASTDictionaryRange::formatImpl(WriteBuffer &ostr, const FormatSettings &, FormatState &, FormatStateStacked) const {
...
void ASTDictionaryLifetime::formatImpl(WriteBuffer &ostr, const FormatSettings &, FormatState &, FormatStateStacked) const {
...
void ASTDictionaryLayout::formatImpl(WriteBuffer &ostr, const FormatSettings &settings, FormatState &state,
                                     FormatStateStacked frame) const {
  ostr << "LAYOUT(" << Poco::toUpper(layout_type);

  if (has_brackets) ostr << "(";
...
void ASTDictionarySettings::formatImpl(WriteBuffer &ostr, const FormatSettings &, FormatState &, FormatStateStacked) const {
  ostr << "SETTINGS(";
  for (auto it = changes.begin(); it != changes.end(); ++it) {
    if (it != changes.begin()) ostr << ", ";
...
ASTPtr ASTDictionary::clone() const {
  ...
  if (source) res->set(res->source, source->clone());

  if (lifetime) res->set(res->lifetime, lifetime->clone());

  if (layout) res->set(res->layout, layout->clone());

  if (range) res->set(res->range, range->clone());

  if (dict_settings) res->set(res->dict_settings, dict_settings->clone());
