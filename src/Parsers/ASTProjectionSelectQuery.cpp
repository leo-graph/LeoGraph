...
ASTPtr ASTProjectionSelectQuery::cloneToASTSelect() const {
  ...
  if (with()) select_query->setExpression(ASTSelectQuery::Expression::WITH, with()->clone());
  if (select()) {
  ...
  if (groupBy()) select_query->setExpression(ASTSelectQuery::Expression::GROUP_BY, groupBy()->clone());
