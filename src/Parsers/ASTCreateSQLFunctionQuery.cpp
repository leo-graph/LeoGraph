...
void ASTCreateSQLFunctionQuery::formatImpl(WriteBuffer& ostr, const IAST::FormatSettings& settings, IAST::FormatState& state,
...
String ASTCreateSQLFunctionQuery::getFunctionName() const {
  ...
  if (!is_ok) throw Exception(ErrorCodes::LOGICAL_ERROR, "Expected function name, got '{}'", function_name->formatForErrorMessage());
  return name;
}
