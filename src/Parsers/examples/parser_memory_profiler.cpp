...
void flushJemallocThreadCache() {
  ...
  if (ret != 0) std::cerr << "Warning: thread.tcache.flush failed: " << ret << "\n";
}
...
void refreshJemallocEpoch() {
  ...
  if (ret != 0) std::cerr << "Warning: epoch refresh failed: " << ret << "\n";
}
...
size_t getJemallocAllocated() {
  ...
  if (ret != 0) std::cerr << "Warning: stats.allocated failed: " << ret << "\n";
  return allocated;
}
...
bool tryEnableProfiling() {
  ...
  if (isProfilingEnabled()) return true;
...
std::string readQuery() { return std::string(std::istreambuf_iterator<char>(std::cin), std::istreambuf_iterator<char>()); }
...
int main(int argc, char** argv) try {
  ...
  po::options_description desc(
      "Measure memory allocations during SQL parsing.\n"
      "Reads query from stdin, prints tab-separated: query_length before after diff\n\n"
      "Options");
  desc.add_options()("help,h", "Show help")("profile", po::value<std::string>(), "Dump jemalloc heap profiles with this prefix")(
      "symbolize", "Symbolize heap profiles (requires --profile)")("symbolize-batch", po::value<std::vector<std::string>>()->multitoken(),
  ...
  if (enable_profiling) profile_prefix = vm["profile"].as<std::string>();
  ...
  if (enable_profiling) {
    ...
    if (profile_before_path.empty()) return 1;
  }
