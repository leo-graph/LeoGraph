...
struct AggregationFunctionEstimateCompressionRatioData {
  ...
  [[maybe_unused]] ~AggregationFunctionEstimateCompressionRatioData() {
    ...
    if (compressed_buf) compressed_buf->cancel();
    if (null_buf) null_buf->cancel();
  }
};
...
class AggregateFunctionEstimateCompressionRatio final
 ...
 private:
  ...
  void resetBuffersIfNeeded(AggregateDataPtr __restrict place) const {
    ...
    if (!data_ref.null_buf || data_ref.null_buf->isFinalized()) data_ref.null_buf = std::make_unique<NullWriteBuffer>();
    ...
    if (!data_ref.compressed_buf || data_ref.compressed_buf->isFinalized())
      data_ref.compressed_buf = std::make_unique<CompressedWriteBuffer>(*data_ref.null_buf, getCodecOrDefault(),
                                                                        block_size_bytes.value_or(DBMS_DEFAULT_BUFFER_SIZE));
  }
  ...
  CompressionCodecPtr getCodecOrDefault() const {
    if (codec.has_value()) {
      ...
      auto ast = parseQuery(codec_parser, "(" + codec.value() + ")", 0, DBMS_DEFAULT_MAX_PARSER_DEPTH, DBMS_DEFAULT_MAX_PARSER_BACKTRACKS);
 ...
 public:
  [[maybe_unused]] explicit AggregateFunctionEstimateCompressionRatio(const DataTypes &arguments, const Array &params,
                                                                      std::optional<String> codec_, std::optional<UInt64> block_size_bytes_)
  ...
  void addBatchSinglePlaceNotNull(size_t row_begin, size_t row_end, AggregateDataPtr __restrict place, const IColumn **columns,
                                  const UInt8 *, Arena *arena, ssize_t) const override {
    addBatchSinglePlace(row_begin, row_end, place, columns, arena, -1);
  }
  ...
  void insertResultInto(AggregateDataPtr __restrict place, IColumn &to, Arena *) const override {
    ...
    Float64 ratio = 0;
    if (compressed_size > 0) ratio = static_cast<Float64>(uncompressed_size) / static_cast<double>(compressed_size);
...
AggregateFunctionPtr createAggregateFunctionEstimateCompressionRatio(const std::string &name, const DataTypes &arguments,
    ...
    if (param.getType() == Field::Types::String) {
      if (codec.has_value())
        throw Exception(ErrorCodes::BAD_QUERY_PARAMETER,
                        "Multiple string type parameters specified for {}. Expected at most one string type (codec) parameter", name);
      ...
      if (new_block_size_bytes == 0) throw Exception(ErrorCodes::BAD_QUERY_PARAMETER, "block_size_bytes should be greater then 0");

      block_size_bytes = new_block_size_bytes;
    } else {
      throw Exception(ErrorCodes::UNKNOWN_QUERY_PARAMETER,
                      "Invalid parameter type for {}. Expected String (codec) and/or UInt64 (block_size_bytes)", name);
    }
  }
...
:::
  ...
  FunctionDocumentation::Arguments arguments = {{"column", "Column of any type.", {"Any"}}};
  FunctionDocumentation::Parameters parameters = {
      ...
      {"block_size_bytes",
       ...
       "[`max_compress_block_size`](../../../operations/settings/merge-tree-settings.md#max_compress_block_size) and "
       "[`min_compress_block_size`](../../../operations/settings/merge-tree-settings.md#min_compress_block_size). The default value is 1 "
       "MiB (1048576 bytes).",
       {"UInt64"}}};
  FunctionDocumentation::ReturnedValue returned_value = {"Returns an estimate compression ratio for the given column.", {"Float64"}};
  FunctionDocumentation::Examples examples = {{"Basic usage with default codec",
                                               R"(
