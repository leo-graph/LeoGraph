...
struct Adder {
    ...
    if constexpr (Data::is_variadic) {
      if constexpr (IsUniqExactSet<typename Data::Set>::value)
        ...
        data.set.insert(T{UniqVariadicHash<Data::is_exact, Data::argument_is_tuple>::apply(num_args, columns, row_num)});
                         ...
                         std::is_same_v<Data, AggregateFunctionUniqHLL12Data<T, Data::is_able_to_parallelize_merge>>) {
  ...
  static void ALWAYS_INLINE add(Data &data, const IColumn **columns, size_t num_args, size_t row_begin, size_t row_end,
                                const char8_t *flags, const UInt8 *null_map) {
    bool use_single_level_hash_table = true;
    if constexpr (Data::is_able_to_parallelize_merge) use_single_level_hash_table = data.set.isSingleLevel();
    ...
    if constexpr (Data::is_able_to_parallelize_merge) {
      if (data.set.isSingleLevel() && data.set.worthConvertingToTwoLevel(data.set.size())) data.set.convertToTwoLevel();
    }
  }

 private:
  template <SetLevelHint hint>
  static void ALWAYS_INLINE addImpl(Data &data, const IColumn **columns, size_t num_args, size_t row_begin, size_t row_end,
                                    const char8_t *flags, const UInt8 *null_map) {
    if (!flags) {
      if (!null_map) {
        if constexpr (std::is_same_v<Data, AggregateFunctionUniqUniquesHashSetData> && !std::is_same_v<T, String> &&
          ...
          for (size_t row = row_begin; row < row_end; ++row) add<hint>(data, columns, num_args, row);
        }
      } else {
        for (size_t row = row_begin; row < row_end; ++row)
          if (!null_map[row]) add<hint>(data, columns, num_args, row);
      }
    } else {
      if (!null_map) {
        for (size_t row = row_begin; row < row_end; ++row)
          if (flags[row]) add<hint>(data, columns, num_args, row);
      } else {
        for (size_t row = row_begin; row < row_end; ++row)
          if (!null_map[row] && flags[row]) add<hint>(data, columns, num_args, row);
      }
    }
  }
};
...
class AggregateFunctionUniq final : public IAggregateFunctionDataHelper<Data, AggregateFunctionUniq<T, Data>> {
 ...
 public:
  ...
  void ALWAYS_INLINE addBatchSinglePlace(size_t row_begin, size_t row_end, AggregateDataPtr __restrict place, const IColumn **columns,
    ...
    if (if_argument_pos >= 0) flags = assert_cast<const ColumnUInt8 &>(*columns[if_argument_pos]).getData().data();
  ...
  void parallelizeMergePrepare(AggregateDataPtrs &places, ThreadPool &thread_pool, std::atomic<bool> &is_cancelled) const override {
    if constexpr (is_parallelize_merge_prepare_needed) {
      ...
      for (size_t i = 0; i < data_vec.size(); ++i) data_vec[i] = &this->data(places[i]).set;

      DataSet::parallelizeMergePrepare(data_vec, thread_pool, is_cancelled);
    } else {
      throw Exception(ErrorCodes::NOT_IMPLEMENTED,
                      "parallelizeMergePrepare() is only implemented when is_parallelize_merge_prepare_needed is true for {} ", getName());
    }
  }
...
class AggregateFunctionUniqVariadic final : public IAggregateFunctionDataHelper<Data, AggregateFunctionUniqVariadic<Data>> {
 ...
 public:
  ...
  ALWAYS_INLINE void addBatchSinglePlace(size_t row_begin, size_t row_end, AggregateDataPtr __restrict place, const IColumn **columns,
                                         Arena *, ssize_t if_argument_pos) const override {
    const char8_t *flags = nullptr;
    if (if_argument_pos >= 0) flags = assert_cast<const ColumnUInt8 &>(*columns[if_argument_pos]).getData().data();
