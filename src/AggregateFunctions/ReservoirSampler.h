...
/// Call `quantile` takes O(sample_count log sample_count), if after the previous call `quantile` there was at least one call `insert`.
/// Otherwise O(1). That is, it makes sense to first add, then get quantiles without adding.
...
class ReservoirSampler {
 public:
  explicit ReservoirSampler(size_t sample_count_ = DEFAULT_SAMPLE_COUNT) : sample_count(sample_count_) { rng.seed(123456); }
  ...
  void insert(const T& v) {
    ...
    if (samples.size() < sample_count) {
      ...
      if (rnd < sample_count) samples[rnd] = v;
    }
  }
  ...
  T quantileNearest(double level) {
    if (samples.empty()) return onEmpty<T>();
  ...
  void merge(const ReservoirSampler<T, OnEmpty>& b) {
    if (sample_count != b.sample_count) throw Poco::Exception("Cannot merge ReservoirSampler's with different sample_count");
    ...
    if (b.total_values <= sample_count) {
      for (size_t i = 0; i < b.samples.size(); ++i) insert(b.samples[i]);
      ...
      for (size_t i = 0; i < from.size(); ++i) insert(from[i]);
    } else {
      ...
      if (frequency * 2 >= static_cast<double>(sample_count)) {
        ...
        if (rnd < sample_count) samples[rnd] = b.samples[rnd];
      } else {
  ...
  void read(DB::ReadBuffer& buf) {
    ...
    for (size_t i = 0; i < samples.size(); ++i) DB::readBinaryLittleEndian(samples[i], buf);
  ...
  void write(DB::WriteBuffer& buf) const {
    ...
    for (size_t i = 0; i < std::min(sample_count, total_values); ++i) DB::writeBinaryLittleEndian(samples[i], buf);
  }
 ...
 private:
  ...
  UInt64 genRandom(UInt64 limit) {
    ...
    if (limit <= static_cast<UInt64>(pcg32_fast::max())) return rng() % limit;  /// NOLINT(clang-analyzer-core.DivideZero)
  ...
  ResultType onEmpty() const {
    if (OnEmpty == ReservoirSamplerOnEmpty::THROW) throw DB::Exception(DB::ErrorCodes::LOGICAL_ERROR, "Quantile of empty ReservoirSampler");
