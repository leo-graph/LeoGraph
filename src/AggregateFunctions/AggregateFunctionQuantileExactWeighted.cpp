...
struct QuantileExactWeighted {
 ...
 private:
    ...
    requires(!interpolated)
  {
    ...
    if (0 == size) return std::numeric_limits<Value>::quiet_NaN();
    ...
    requires(!interpolated)
  {
    ...
    while (it < end) {
      ...
      while (accumulated >= threshold) {
        ...
        if (level_index == num_levels) return;
    ...
    requires(interpolated)
  {
    ...
    if (0 == size) return std::numeric_limits<Float64>::quiet_NaN();
    ...
    requires(interpolated)
  {
    size_t size = map.size();
    if (0 == size) {
      for (size_t i = 0; i < num_levels; ++i) result[i] = std::numeric_limits<Float64>::quiet_NaN();
      return;
    }
    ...
    requires(interpolated)
  {
    ...
    if (lower_it == array + size) lower_it = array + size - 1;
    if (higher_it == array + size) higher_it = array + size - 1;

    UnderlyingType lower_key = lower_it->first;
    UnderlyingType higher_key = higher_it->first;

    if (lower == higher || lower_key == higher_key) return static_cast<Float64>(lower_key);

    return (static_cast<Float64>(higher) - position) * static_cast<Float64>(lower_key) +
           (position - static_cast<Float64>(lower)) * static_cast<Float64>(higher_key);
  }
};

template <typename Value, bool return_float, bool interpolated>
using FuncQuantileExactWeighted =
    AggregateFunctionQuantile<Value, QuantileExactWeighted<Value, interpolated>,
                              std::conditional_t<interpolated, NameQuantileExactWeightedInterpolated, NameQuantileExactWeighted>, UInt64,
                              std::conditional_t<return_float, Float64, void>, false, false>;
template <typename Value, bool return_float, bool interpolated>
using FuncQuantilesExactWeighted =
    AggregateFunctionQuantile<Value, QuantileExactWeighted<Value, interpolated>,
                              std::conditional_t<interpolated, NameQuantilesExactWeightedInterpolated, NameQuantilesExactWeighted>, UInt64,
                              std::conditional_t<return_float, Float64, void>, true, false>;
...
#define DISPATCH(TYPE) \
  if (which.idx == TypeIndex::TYPE) return std::make_shared<Function<TYPE, interpolated, interpolated>>(argument_types, params);
...
#undef DISPATCH
  ...
  if (which.idx == TypeIndex::DateTime)
    return std::make_shared<Function<DataTypeDateTime::FieldType, false, interpolated>>(argument_types, params);
...
quantileExactWeighted(level)(expr, weight)
  ...
  FunctionDocumentation::Parameters parameters = {
      {"level",
       ...
       "`[0.01, 0.99]`. Default value: 0.5. At `level=0.5` the function calculates median.",
       {"Float*"}}};
  FunctionDocumentation::ReturnedValue returned_value = {"Quantile of the specified level.", {"Float64", "Date", "DateTime"}};
  FunctionDocumentation::Examples examples = {{"Computing exact weighted quantile",
                                               R"(
CREATE TABLE t (
    n Int32,
    val Int32
...
└───────────────────────────────┘
  ...
  factory.registerFunction(NameQuantileExactWeighted::name,
                           {createAggregateFunctionQuantile<FuncQuantileExactWeighted, false>, documentation});
  factory.registerFunction(NameQuantilesExactWeighted::name,
                           {createAggregateFunctionQuantile<FuncQuantilesExactWeighted, false>, {}, properties});
...
quantileExactWeightedInterpolated(level)(expr, weight)
  ...
  FunctionDocumentation::Examples examples_interpolated = {{"Computing exact weighted interpolated quantile",
                                                            R"(
...
└────────────────────────────────────────────────────┴────────────────────────┴───────────────────────────────────────────────┘
  ...
  FunctionDocumentation documentation_interpolated = {description_interpolated,   syntax_interpolated,         arguments_interpolated,
                                                      parameters_interpolated,    returned_value_interpolated, examples_interpolated,
                                                      introduced_in_interpolated, category_interpolated};

  factory.registerFunction(NameQuantileExactWeightedInterpolated::name,
                           {createAggregateFunctionQuantile<FuncQuantileExactWeighted, true>, documentation_interpolated});
  factory.registerFunction(NameQuantilesExactWeightedInterpolated::name,
                           {createAggregateFunctionQuantile<FuncQuantilesExactWeighted, true>, {}, properties});
