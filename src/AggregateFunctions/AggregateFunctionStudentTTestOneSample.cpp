...
struct StudentTTestOneSampleData : public OneSampleTTestMoments<Float64> {
  ...
  std::tuple<Float64, Float64> getResult() const {
    if (!hasEnoughObservations()) return {std::numeric_limits<Float64>::quiet_NaN(), std::numeric_limits<Float64>::quiet_NaN()};
...
AggregateFunctionPtr createAggregateFunctionStudentTTestOneSample(const std::string &name, const DataTypes &argument_types,
  ...
  if (parameters.size() > 1)
    throw Exception(ErrorCodes::NUMBER_OF_ARGUMENTS_DOESNT_MATCH, "Aggregate function {} allows at most one parameter (confidence level).",
                    name);
...
studentTTestOneSample([confidence_level])(sample_data, population_mean)
  ...
  FunctionDocumentation::ReturnedValue returned_value_studentTTestOneSample = {
      "Returns a tuple with two or four elements (if `confidence_level` is specified): calculated t-statistic, calculated p-value "
      "(two-tailed), [calculated confidence-interval-low], [calculated confidence-interval-high]. Confidence intervals are for the sample "
      "mean at the given confidence level.",
      {"Tuple(Float64, Float64)", "Tuple(Float64, Float64, Float64, Float64)"}};
  FunctionDocumentation::Examples examples_studentTTestOneSample = {{"Without confidence interval",
                                                                     R"(
...
SELECT studentTTestOneSample(0.95)(value, 20.0) FROM t;
  ...
  FunctionDocumentation documentation_studentTTestOneSample = {description_studentTTestOneSample,    syntax_studentTTestOneSample,
                                                               arguments_studentTTestOneSample,      parameters_studentTTestOneSample,
                                                               returned_value_studentTTestOneSample, examples_studentTTestOneSample,
                                                               introduced_in_studentTTestOneSample,  category_studentTTestOneSample};
