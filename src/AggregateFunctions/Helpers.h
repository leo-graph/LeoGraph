...
template <template <typename, typename> class AggregateFunctionTemplate, template <typename, bool> class Data, bool bool_param,
          typename... TArgs>
static IAggregateFunction *createWithNumericType(const IDataType &argument_type, TArgs &&...args) {
  WhichDataType which(argument_type);
#define DISPATCH(TYPE) \
...
#undef DISPATCH
  ...
  if (which.idx == TypeIndex::Date) return new AggregateFunctionTemplate<UInt16, Data<UInt16>>(args...);
  if (which.idx == TypeIndex::DateTime) return new AggregateFunctionTemplate<UInt32, Data<UInt32>>(args...);
...
static IAggregateFunction *createWithDecimalType(const IDataType &argument_type, TArgs &&...args) {
  ...
  if constexpr (AggregateFunctionTemplate<DateTime64>::DateTime64Supported) {
    ...
    if (which.idx == TypeIndex::Time64) return new AggregateFunctionTemplate<Time64>(args...);
  }
  return nullptr;
}
...
static IAggregateFunction *createWithDecimalType(const IDataType &argument_type, TArgs &&...args) {
  ...
  if constexpr (AggregateFunctionTemplate<DateTime64, Data>::DateTime64Supported) {
    ...
    if (which.idx == TypeIndex::Time64) return new AggregateFunctionTemplate<Time64, Data>(args...);
  }
  return nullptr;
}
...
#define DISPATCH(TYPE) \
  if (which.idx == TypeIndex::TYPE) return createWithTwoBasicNumericTypesSecond<TYPE, AggregateFunctionTemplate>(second_type, args...);
  FOR_BASIC_NUMERIC_TYPES(DISPATCH)
#undef DISPATCH
  return nullptr;
}
