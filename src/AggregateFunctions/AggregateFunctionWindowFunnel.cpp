...
void mergeEventsList(T &events_list, size_t prefix_size, bool prefix_sorted, bool suffix_sorted) {
  ...
  if (!prefix_sorted && !suffix_sorted)
    ...
    if (!prefix_sorted) std::stable_sort(begin, middle);

    if (!suffix_sorted) std::stable_sort(middle, end);
...
struct AggregateFunctionWindowFunnelData {
  ...
  size_t size() const { return events_list.size(); }
...
struct AggregateFunctionWindowFunnelStrictOnceData {
  ...
  void add(T timestamp, UInt8 event_type) {
    ...
    if (sorted && !events_list.empty()) sorted = events_list.back() <= new_event;
...
class AggregateFunctionWindowFunnel final : public IAggregateFunctionDataHelper<Data, AggregateFunctionWindowFunnel<T, Data>> {
 private:
  UInt64 window;
  UInt8 events_size;
  ...
  UInt8 getEventLevelNonStrictOnce(const AggregateFunctionWindowFunnelData<T>::TimestampEvents &events_list) const {
    ...
    for (size_t i = 0; i < events_list.size(); ++i) {
      ...
      if (event_idx == 0) {
        ...
        if (allow_reentry) continue;

        for (size_t event = 0; event < events_timestamp.size(); ++event) {
          if (!events_timestamp[event].has_value()) return static_cast<UInt8>(event);
        }
      } else if (events_timestamp[event_idx - 1].has_value()) {
        auto first_timestamp = events_timestamp[event_idx - 1]->first;
        bool time_matched = timestamp <= first_timestamp + window;
        if (strict_increase) time_matched = time_matched && events_timestamp[event_idx - 1]->second < timestamp;
        if (time_matched) {
    ...
    for (size_t event = events_timestamp.size(); event > 0; --event) {
      if (events_timestamp[event - 1].has_value()) return static_cast<UInt8>(event);
    }
    return 0;
  }
  ...
  UInt8 getEventLevelStrictOnce(const AggregateFunctionWindowFunnelStrictOnceData<T>::TimestampEvents &events_list) const {
    ...
    struct EventMatchTimeWindow {
      ...
      EventMatchTimeWindow(UInt64 first_ts, UInt64 last_ts) : first_timestamp(first_ts), last_timestamp(last_ts) {}
    };
    ...
    for (size_t i = 0; i < events_list.size(); ++i) {
      ...
      if (strict_order && event_idx == -1) {
        ...
        for (size_t event = 0; event < event_sequences.size(); ++event) {
          if (event_sequences[event].empty()) return static_cast<UInt8>(event);
        }
        ...
        for (auto it = prev_level.begin(); it != prev_level.end();) {
          ...
          if (time_matched && strict_increase) time_matched = it->last_timestamp < timestamp;
    ...
    for (size_t event = event_sequences.size(); event > 0; --event) {
      if (!event_sequences[event - 1].empty()) return static_cast<UInt8>(event);
    }
    return 0;
  }
  ...
  UInt8 getEventLevel(Data &data) const {
    ...
    if (!strict_order && events_size == 1) return 1;
 ...
 public:
  ...
  AggregateFunctionWindowFunnel(const DataTypes &arguments, const Array &params)
    ...
    if (allow_reentry && !strict_order) {
      throw Exception(ErrorCodes::BAD_ARGUMENTS, "allow_reentry option requires strict_order to be enabled in Aggregate function {}",
                      getName());
    }
  }
  ...
  void add(AggregateDataPtr __restrict place, const IColumn **columns, const size_t row_num, Arena *) const override {
    ...
    if (strict_order && !has_event) this->data(place).add(timestamp, 0);

    if constexpr (Data::strict_once_enabled) this->data(place).advanceId();
  }
...
AggregateFunctionPtr createAggregateFunctionWindowFunnel(const std::string &name, const DataTypes &arguments, const Array &params,
  ...
  if (params.empty())
    throw Exception(ErrorCodes::NUMBER_OF_ARGUMENTS_DOESNT_MATCH,
                    "Aggregate function {} requires at least one parameter: <window>, [option, [option, ...]]", name);
  ...
  bool strict_once = params.size() > 1 && std::any_of(params.begin() + 1, params.end(),
                                                      [](const auto &f) { return f.template safeGet<String>() == "strict_once"; });
  if (strict_once) {
    AggregateFunctionPtr res(createWithUnsignedIntegerType<AggregateFunctionWindowFunnel, AggregateFunctionWindowFunnelStrictOnceData>(
        *arguments[0], arguments, params));
    WhichDataType which(arguments.front().get());
    if (res) return res;
    if (which.isDate())
      return std::make_shared<
          AggregateFunctionWindowFunnel<DataTypeDate::FieldType, AggregateFunctionWindowFunnelStrictOnceData<DataTypeDate::FieldType>>>(
          arguments, params);
    if (which.isDateTime())
      return std::make_shared<AggregateFunctionWindowFunnel<DataTypeDateTime::FieldType,
                                                            AggregateFunctionWindowFunnelStrictOnceData<DataTypeDateTime::FieldType>>>(
          arguments, params);
  } else {
    AggregateFunctionPtr res(
        createWithUnsignedIntegerType<AggregateFunctionWindowFunnel, AggregateFunctionWindowFunnelData>(*arguments[0], arguments, params));
    WhichDataType which(arguments.front().get());
    if (res) return res;
    if (which.isDate())
      return std::make_shared<
          AggregateFunctionWindowFunnel<DataTypeDate::FieldType, AggregateFunctionWindowFunnelData<DataTypeDate::FieldType>>>(arguments,
                                                                                                                              params);
    if (which.isDateTime())
      return std::make_shared<
          AggregateFunctionWindowFunnel<DataTypeDateTime::FieldType, AggregateFunctionWindowFunnelData<DataTypeDateTime::FieldType>>>(
          arguments, params);
  }
  throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT,
                  "Illegal type {} of first argument of aggregate function {}, must "
                  "be Unsigned Number, Date, DateTime",
                  arguments.front().get()->getName(), name);
}
