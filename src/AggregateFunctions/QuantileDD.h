...
class QuantileDD {
 public:
  ...
  void add(const Value &x, Weight w) {
    if (!isNaN(x)) data.add(static_cast<Float64>(x), static_cast<Float64>(w));
  }

  void merge(const QuantileDD &other) { data.merge(other.data); }

  void serialize(WriteBuffer &buf) const { data.serialize(buf); }

  void deserialize(ReadBuffer &buf) { data.deserialize(buf); }
 ...
 private:
  ...
  void getManyImpl(const Float64 *levels, const size_t *, size_t num_levels, T *result) const {
    for (size_t i = 0; i < num_levels; ++i) result[i] = getImpl<T>(levels[i]);
  }
};
