...
class ApproxSampler {
 public:
  ...
  void insert(T x) {
    ...
    if (head_sampled.size() >= default_head_size) {
      ...
      if (sampled.size() >= compress_threshold) compress();
    }
  }
  ...
  void query(const Float64 *percentiles, const size_t *indices, size_t size, T *result) const {
    ...
    if (sampled.empty()) {
      for (size_t i = 0; i < size; ++i) result[i] = T();
      return;
    }

    Int64 current_max = std::numeric_limits<Int64>::min();
    for (const auto &stats : sampled) current_max = std::max(stats.delta + stats.g, current_max);
  ...
  void read(ReadBuffer &buf) {
    ...
    if (compress_threshold != default_compress_threshold)
      throw Exception(ErrorCodes::INCORRECT_DATA, "The compress threshold {} isn't the expected one {}", compress_threshold,
 ...
 private:
  ...
  void withHeadBufferInserted() {
    ...
    for (; sample_idx < sampled.size(); ++sample_idx) backup_sampled.emplace_back(sampled[sample_idx]);
  ...
  void doCompress(double merge_threshold) {
    ...
    if (curr_head.value <= head.value && sampled.size() > 1) backup_sampled.emplace_back(sampled.front());
...
class QuantileGK {
 ...
 public:
  ...
  void merge(const QuantileGK &rhs) {
    if (!data.isCompressed()) data.compress();
...
template <typename Value, bool _>
using FuncQuantileGK = AggregateFunctionQuantile<Value, QuantileGK<Value>, NameQuantileGK, void, void, false, true>;
template <typename Value, bool _>
using FuncQuantilesGK = AggregateFunctionQuantile<Value, QuantileGK<Value>, NameQuantilesGK, void, void, true, true>;
...
#undef DISPATCH
  ...
  throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT, "Illegal type {} of argument for aggregate function {}", argument_type->getName(),
                  name);
}
...
quantileGK(accuracy, level)(expr)
  ...
  FunctionDocumentation::Arguments arguments = {{"expr",
                                                 "Expression over the column values resulting in numeric data types, Date or DateTime.",
                                                 {"(U)Int*", "Float*", "Decimal*", "Date", "DateTime"}}};
  FunctionDocumentation::Parameters parameters = {
      {"accuracy",
       "Accuracy of quantile. Constant positive integer. Larger accuracy value means less error. For example, if the accuracy argument is "
       "set to 100, the computed quantile will have an error no greater than 1% with high probability. There is a trade-off between the "
       "accuracy of the computed quantiles and the computational complexity of the algorithm. A larger accuracy requires more memory and "
       "computational resources to compute the quantile accurately, while a smaller accuracy argument allows for a faster and more "
       "memory-efficient computation but with a slightly lower accuracy.",
       {"UInt*"}},
      {"level",
       "Optional. Level of quantile. Constant floating-point number from 0 to 1. Default value: 0.5. At `level=0.5` the function "
       "calculates median.",
       {"Float*"}}};
  FunctionDocumentation::ReturnedValue returned_value = {"Returns the quantile of the specified level and accuracy.",
                                                         {"Float64", "Date", "DateTime"}};
  FunctionDocumentation::Examples examples = {{"Computing quantile with different accuracy levels",
                                               R"(
...
quantilesGK(accuracy, level1, level2, ...)(expr)
  ...
  FunctionDocumentation::Parameters parameters_quantiles = {
      {"accuracy",
       "Accuracy of quantiles. Constant positive integer. Larger accuracy value means less error. For example, if the accuracy argument is "
       "set to 100, the computed quantiles will have an error no greater than 1% with high probability. There is a trade-off between the "
       "accuracy of the computed quantiles and the computational complexity of the algorithm.",
       {"UInt*"}},
      {"level", "Levels of quantiles. One or more constant floating-point numbers from 0 to 1.", {"Float*"}}};
  FunctionDocumentation::ReturnedValue returned_value_quantiles = {
      "Array of quantiles of the specified levels in the same order as the levels were specified.",
      {"Array(Float64)", "Array(Date)", "Array(DateTime)"}};
  FunctionDocumentation::Examples examples_quantiles = {{"Computing multiple quantiles with GK algorithm",
                                                         R"(
...
└────────────────────────────────────────────────────┘
  ...
  FunctionDocumentation documentation_quantiles = {description_quantiles,   syntax_quantiles,         arguments_quantiles,
                                                   parameters_quantiles,    returned_value_quantiles, examples_quantiles,
                                                   introduced_in_quantiles, category_quantiles};
