...
struct CramersVData : CrossTabData {
  static const char *getName() { return "cramersV"; }

  Float64 getResult() const {
    if (count < 2) return std::numeric_limits<Float64>::quiet_NaN();
...
:::
  ...
  FunctionDocumentation::Arguments arguments = {{"column1", "First column to be compared.", {"(U)Int*", "Float*", "Decimal"}},
  ...
  FunctionDocumentation::Examples examples = {{"No association between columns",
                                               R"(
SELECT
    cramersV(a, b)
FROM
...
└────────────────────┘
  ...
  FunctionDocumentation documentation = {description,    syntax,   arguments,     docs_parameters,
                                         returned_value, examples, introduced_in, category};
