...
struct EntropyData {
  ...
  Float64 get() const {
    ...
    for (const auto &pair : map) total_value += pair.getMapped();
...
Calculates the [Shannon entropy](https://en.wikipedia.org/wiki/Entropy_(information_theory)) for a column of values.
  ...
  FunctionDocumentation::Arguments arguments = {{"val", "Column of values of any type.", {"Any"}}};
  FunctionDocumentation::Parameters parameters = {};
  FunctionDocumentation::ReturnedValue returned_value = {"Returns Shannon entropy.", {"Float64"}};
  FunctionDocumentation::Examples examples = {{"Basic entropy calculation",
                                               R"(
