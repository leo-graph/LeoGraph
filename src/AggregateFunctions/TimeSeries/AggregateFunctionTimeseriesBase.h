...
class AggregateFunctionTimeseriesBase : public IAggregateFunctionHelper<AggregateFunctionTimeseriesBase<FunctionImpl, Traits>> {
 public:
  ...
  String getName() const override { return Traits::getName(); }
  ...
  explicit AggregateFunctionTimeseriesBase(const DataTypes &argument_types_, TimestampType start_timestamp_, TimestampType end_timestamp_,
                                           IntervalType step_, IntervalType window_, UInt32 timestamp_scale_)
      : Base(argument_types_,
             {/// Normalize all parameters to decimals with the same scale as the scale of timestamp argument
              DecimalField<Decimal64>(start_timestamp_, timestamp_scale_), DecimalField<Decimal64>(end_timestamp_, timestamp_scale_),
              DecimalField<Decimal64>(step_, timestamp_scale_), DecimalField<Decimal64>(window_, timestamp_scale_)},
             createResultType()),
    ...
    if (window < 0) throw Exception(ErrorCodes::BAD_ARGUMENTS, "Window should be non-negative");
  }
  ...
  static size_t bucketCount(TimestampType start_timestamp, TimestampType end_timestamp, IntervalType step) {
    ...
    if (end_timestamp == start_timestamp) return 1;
  ...
  void NO_SANITIZE_UNDEFINED ALWAYS_INLINE add(AggregateDataPtr __restrict place, TimestampType timestamp, ValueType value) const {
    if (timestamp + window + step < start_timestamp || timestamp > end_timestamp) return;
  ...
  void addMany(AggregateDataPtr __restrict place, const TimestampType *__restrict timestamp_ptr, const ValueType *__restrict value_ptr,
               size_t start, size_t end) const {
    for (size_t i = start; i < end; ++i) add(place, timestamp_ptr[i], value_ptr[i]);
  }

  void addManyNotNull(AggregateDataPtr __restrict place, const TimestampType *__restrict timestamp_ptr,
                      const ValueType *__restrict value_ptr, const UInt8 *__restrict null_map, size_t start, size_t end) const {
    for (size_t i = start; i < end; ++i)
      if (!null_map[i]) add(place, timestamp_ptr[i], value_ptr[i]);
  }

  void addManyConditional(AggregateDataPtr __restrict place, const TimestampType *__restrict timestamp_ptr,
                          const ValueType *__restrict value_ptr, const UInt8 *__restrict condition_map, size_t start, size_t end) const {
    for (size_t i = start; i < end; ++i)
      if (condition_map[i]) add(place, timestamp_ptr[i], value_ptr[i]);
  }
  ...
  void addBatchSinglePlaceWithFlags(size_t row_begin, size_t row_end, AggregateDataPtr __restrict place, const IColumn **columns,
        ...
        for (size_t i = row_begin; i < row_end; ++i) {
          ...
          if (flags_data[i] == flag_value_to_include) {
            ...
            if (timestamp_array_size != value_array_size)
              throw Exception(ErrorCodes::BAD_ARGUMENTS, "Timestamp and value arrays have different sizes at row {} : {} and {}", i,
  ...
  void deserialize(AggregateDataPtr __restrict place, ReadBuffer &buf, std::optional<size_t> /* version */, Arena *) const override {
    ...
    if (size != bucket_count) throw Exception(ErrorCodes::LOGICAL_ERROR, "Cannot deserialize data with different bucket count");

    size_t buckets_size;
    readBinaryLittleEndian(buckets_size, buf);

    if (buckets_size > bucket_count) throw Exception(ErrorCodes::INCORRECT_DATA, "Cannot deserialize data with more buckets than expected");
    ...
    for (size_t i = 0; i < buckets_size; ++i) {
      ...
      if (index >= bucket_count)
        throw Exception(ErrorCodes::INCORRECT_DATA, "Cannot deserialize data with index {} greater than bucket count {}", index,
                        bucket_count);
  ...
  void checkTimestampInRange(const TimestampType timestamp, const size_t bucket_index) const {
    if (timestamp > end_timestamp)
      throw Exception(ErrorCodes::INCORRECT_DATA, "Cannot deserialize data with timestamp {} greater than end timestamp {}",
  ...
  const FunctionImpl &derived() const { return static_cast<const FunctionImpl &>(*this); }

  void insertResultInto(AggregateDataPtr __restrict place, IColumn &to, Arena *) const override { derived().doInsertResultInto(place, to); }
  ...
  void insertResultIntoBatch(size_t row_begin, size_t row_end, AggregateDataPtr *places, size_t place_offset, IColumn &to,
    ...
    try {
      ...
      for (size_t destroy_index = batch_index; destroy_index < row_end; ++destroy_index) destroy(places[destroy_index] + place_offset);
