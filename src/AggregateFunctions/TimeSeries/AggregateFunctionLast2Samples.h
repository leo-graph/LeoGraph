...
class AggregateFunctionLast2Samples final : public IAggregateFunctionHelper<AggregateFunctionLast2Samples<TimestampType, ValueType>> {
 public:
  ...
  String getName() const override { return "timeSeriesLastTwoSamples"; }
  ...
  struct Data {
    ...
    TimestampType
        timestamps[2];  /// Timestamps might be stored as DateTime64, DateTime32 or even as 16-bit delta from the base timestamp of the grid
    ...
    void merge(const Data &rhs) {
      for (size_t i = 0; i < rhs.filled; ++i) add(rhs.timestamps[i], rhs.values[i]);
    }
  };

  explicit AggregateFunctionLast2Samples(const DataTypes &argument_types_) : Base(argument_types_, {}, createResultType(argument_types_)) {}
  ...
  void deserialize(AggregateDataPtr __restrict place, ReadBuffer &buf, std::optional<size_t> /* version */, Arena *) const override {
    ...
    if (data.filled > 2)
      throw Exception(ErrorCodes::INCORRECT_DATA, "Cannot deserialize data with different bucket count, expected 2, got {}", data.filled);

    for (size_t i = 0; i < data.filled; ++i) {
      readBinaryLittleEndian(data.timestamps[i], buf);
      if (i > 0 && data.timestamps[i] > data.timestamps[i - 1])
        throw Exception(ErrorCodes::INCORRECT_DATA, "Timestamps must be in descending order, but got {} after {}",
