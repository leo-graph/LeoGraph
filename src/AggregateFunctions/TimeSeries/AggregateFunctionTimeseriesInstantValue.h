...
struct AggregateFunctionTimeseriesInstantValueTraits {
  ...
  static String getName() { return is_rate_ ? "timeSeriesInstantRateToGrid" : "timeSeriesInstantDeltaToGrid"; }
...
class AggregateFunctionTimeseriesInstantValue final
 ...
 public:
  ...
  void deserializeBucket(Bucket &bucket, ReadBuffer &buf, const size_t bucket_index) const {
    ...
    if (bucket.filled > 2) throw Exception(ErrorCodes::INCORRECT_DATA, "Cannot deserialize data with more than 2 samples in a bucket");
  ...
  void fillResultValue(TimestampType timestamp, ValueType value, TimestampType previous_timestamp, ValueType previous_value,
  ...
  void doInsertResultInto(AggregateDataPtr __restrict place, IColumn &to) const {
    ...
    for (size_t i = 0; i < Base::bucket_count; ++i, current_timestamp += Base::step) {
      ...
      if (bucket_it != buckets.end()) last_2_samples.merge(bucket_it->second);
      ...
      if (last_2_samples.filled == 2 && last_2_samples.timestamps[1] + Base::window > current_timestamp) {
        fillResultValue(last_2_samples.timestamps[0], last_2_samples.values[0], last_2_samples.timestamps[1], last_2_samples.values[1],
                        values[i], nulls[i]);
      }
    }
  }
