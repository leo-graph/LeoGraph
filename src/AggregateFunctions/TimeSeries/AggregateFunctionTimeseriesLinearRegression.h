...
struct AggregateFunctionTimeseriesLinearRegressionTraits {
  ...
  static String getName() { return is_predict ? "timeSeriesPredictLinearToGrid" : "timeSeriesDerivToGrid"; }
  ...
  struct Bucket {
    ...
    void merge(const Bucket &other) {
      ...
      for (const auto &[timestamp, value] : other.samples) add(timestamp, value);
    }
  };
};
...
class AggregateFunctionTimeseriesLinearRegression final
 ...
 public:
  ...
  explicit AggregateFunctionTimeseriesLinearRegression(const DataTypes &argument_types_, TimestampType start_timestamp_,
                                                       TimestampType end_timestamp_, IntervalType step_, IntervalType window_,
                                                       UInt32 timestamp_scale_, Float64 predict_offset_ = 0)
      : Base(argument_types_, start_timestamp_, end_timestamp_, step_, window_, timestamp_scale_), predict_offset(predict_offset_) {}
 ...
 private:
  ...
  void fillResultValue(const TimestampType current_timestamp,
                       const DequeWithMemoryTracking<std::pair<TimestampType, ValueType>> &samples_in_window, ValueType &result,
                       UInt8 &null) const {
  ...
  void doInsertResultInto(AggregateDataPtr __restrict place, IColumn &to) const {
    ...
    for (UInt32 i = 0; i < Base::bucket_count; ++i) {
      ...
      if (bucket_it != buckets.end()) {
        ...
        for (const auto &[timestamp, value] : bucket_it->second.samples) timestamps_buffer.emplace_back(timestamp, value);
        std::sort(timestamps_buffer.begin(), timestamps_buffer.end());

        /// Add samples from the current bucket
        for (const auto &[timestamp, value] : timestamps_buffer) samples_in_window.push_back({timestamp, value});
      }
      ...
      fillResultValue(current_timestamp, samples_in_window, values[i], nulls[i]);
    }
  }
 ...
 protected:
  const Float64 predict_offset{};  /// Predict offset used by timeSeriesPredictLinearToGrid function, used to calculate the timestamp of the
                                   /// predicted value
};
