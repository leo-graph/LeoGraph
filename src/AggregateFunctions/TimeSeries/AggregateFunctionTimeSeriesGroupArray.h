...
class AggregateFunctionTimeSeriesGroupArray final
 ...
 public:
  ...
  String getName() const override { return "timeSeriesGroupArray"; }
  ...
  struct Data {
    ...
    void reserve(size_t new_size, Arena *arena) {
      if (new_size > allocated_size) {
        ...
        elements = reinterpret_cast<Element *>(arena->alignedRealloc(reinterpret_cast<char *>(elements), old_size * sizeof(Element),
                                                                     allocated_size * sizeof(Element), alignof(Element)));
      }
    }
    ...
    void merge(const Data &rhs, Arena *arena) {
      ...
      if (rhs.size) memcpy(elements + size, rhs.elements, rhs.size * sizeof(Element));
      size += rhs.size;
    }
  ...
  static DataTypePtr createResultType(const DataTypes &argument_types_) {
    const auto &timestamp_type =
        array_arguments ? typeid_cast<const DataTypeArray *>(argument_types_[0].get())->getNestedType() : argument_types_[0];
    const auto &value_type =
        array_arguments ? typeid_cast<const DataTypeArray *>(argument_types_[1].get())->getNestedType() : argument_types_[1];
  ...
  void NO_SANITIZE_UNDEFINED ALWAYS_INLINE add(AggregateDataPtr __restrict place, TimestampType timestamp, ValueType value,
                                               Arena *arena) const {
  ...
  void addMany(AggregateDataPtr __restrict place, const TimestampType *__restrict timestamp_ptr, const ValueType *__restrict value_ptr,
               size_t start, size_t end, Arena *arena) const {
    reserveAdd(place, end - start, arena);
    for (size_t i = start; i < end; ++i) add(place, timestamp_ptr[i], value_ptr[i], arena);
  }

  void addManyNotNull(AggregateDataPtr __restrict place, const TimestampType *__restrict timestamp_ptr,
                      const ValueType *__restrict value_ptr, const UInt8 *__restrict null_map, size_t start, size_t end,
                      Arena *arena) const {
    reserveAdd(place, end - start, arena);
    for (size_t i = start; i < end; ++i)
      if (!null_map[i]) add(place, timestamp_ptr[i], value_ptr[i], arena);
  }

  void addManyConditional(AggregateDataPtr __restrict place, const TimestampType *__restrict timestamp_ptr,
                          const ValueType *__restrict value_ptr, const UInt8 *__restrict condition_map, size_t start, size_t end,
                          Arena *arena) const {
    reserveAdd(place, end - start, arena);
    for (size_t i = start; i < end; ++i)
      if (condition_map[i]) add(place, timestamp_ptr[i], value_ptr[i], arena);
  }
  ...
  void addBatchSinglePlaceWithFlags(size_t row_begin, size_t row_end, AggregateDataPtr __restrict place, const IColumn **columns,
                                    Arena *arena, const UInt8 *flags_data) const {
    if (array_arguments) {
  ...
  void addBatchSinglePlaceNotNull(size_t row_begin, size_t row_end, AggregateDataPtr __restrict place, const IColumn **columns,
                                  const UInt8 *null_map, Arena *arena, ssize_t if_argument_pos) const override {
  ...
  void serialize(ConstAggregateDataPtr __restrict place, WriteBuffer &buf, std::optional<size_t> /* version */) const override {
    ...
    for (size_t i = 0; i < data.size; ++i) writeBinaryLittleEndian(data.elements[i].timestamp, buf);

    for (size_t i = 0; i < data.size; ++i) writeBinaryLittleEndian(data.elements[i].value, buf);
  }
  ...
  void deserialize(AggregateDataPtr __restrict place, ReadBuffer &buf, std::optional<size_t> /* version */, Arena *arena) const override {
    ...
    if (format_version != FORMAT_VERSION)
      throw Exception(ErrorCodes::INCORRECT_DATA, "Cannot deserialize data with different format version, expected {}, got {}",
    ...
    for (size_t i = 0; i < size; ++i) readBinaryLittleEndian(data.elements[i].timestamp, buf);

    for (size_t i = 0; i < size; ++i) readBinaryLittleEndian(data.elements[i].value, buf);
  ...
  void insertResultInto(AggregateDataPtr __restrict place, IColumn &to, Arena *) const override {
    ...
    if (tuple.tupleSize() != 2) throw Exception(ErrorCodes::LOGICAL_ERROR, "Expected tuple size 2, got {}", tuple.tupleSize());
