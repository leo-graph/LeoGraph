...
AggregateFunctionPtr createAggregateFunctionTimeseriesGroupArray(const std::string &name, const DataTypes &argument_types,
...
timeSeriesGroupArray(timestamp, value)
  ...
  FunctionDocumentation::ReturnedValue returned_value = {
      "Returns an array of tuples `(timestamp, value)` sorted by timestamp in ascending order. If there are multiple values for the same "
      "timestamp then the function chooses the greatest of these values.",
      {"Array(Tuple(T1, T2))"}};
  FunctionDocumentation::Examples examples = {{"Basic usage with individual values",
                                               R"(
WITH
