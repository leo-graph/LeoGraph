...
struct AggregateFunctionTimeseriesExtrapolatedValueTraits {
  ...
  static String getName() { return is_rate ? "timeSeriesRateToGrid" : "timeSeriesDeltaToGrid"; }
...
class AggregateFunctionTimeseriesExtrapolatedValue final
 ...
 private:
  void fillResultValue(const TimestampType current_timestamp,
                       ...
                       Float64 accumulated_resets_in_window, ValueType &result, UInt8 &null) const {
    ...
    const Float64 average_duration_between_samples =
        static_cast<Float64>(sampled_interval) / static_cast<Float64>(samples_in_window.size() - 1);
    ...
    if (duration_to_start >= extrapolation_threshold) duration_to_start = average_duration_between_samples / 2;
    ...
    if (duration_to_end >= extrapolation_threshold) duration_to_end = average_duration_between_samples / 2;
 ...
 public:
  ...
  void doInsertResultInto(AggregateDataPtr __restrict place, IColumn &to) const {
    ...
    for (UInt32 i = 0; i < Base::bucket_count; ++i) {
      ...
      fillResultValue(current_timestamp, samples_in_window, accumulated_resets_in_window, values[i], nulls[i]);
    }
  }
