...
using FuncQuantileDD =
    AggregateFunctionQuantile<Value, QuantileDD<Value>, NameQuantileDD, void, std::conditional_t<float_return, Float64, void>, false, true>;
template <typename Value, bool float_return>
using FuncQuantilesDD =
    AggregateFunctionQuantile<Value, QuantileDD<Value>, NameQuantilesDD, void, std::conditional_t<float_return, Float64, void>, true, true>;
...
quantileDD(relative_accuracy, [level])(expr)
  ...
  FunctionDocumentation::Arguments arguments = {{"expr", "Column with numeric data.", {"(U)Int*", "Float*"}}};
  FunctionDocumentation::Parameters parameters = {
      {"relative_accuracy",
       "Relative accuracy of the quantile. Possible values are in the range from 0 to 1. The size of the sketch depends on the range of "
       "the data and the relative accuracy. The larger the range and the smaller the relative accuracy, the larger the sketch. The rough "
       "memory size of the sketch is `log(max_value/min_value)/relative_accuracy`. The recommended value is 0.001 or higher.",
       {"Float*"}},
  ...
  FunctionDocumentation::Examples examples = {{"Computing quantile with DD sketch",
                                               R"(
