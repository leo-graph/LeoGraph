 ...
 public:
  AggregateFunctionGroupArrayInsertAtGeneric(const DataTypes &arguments, const Array &params)
    ...
    if (!isUInt(arguments[1]))
      throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT, "Second argument of aggregate function {} must be unsigned integer.",
                      getName());
    ...
    if (default_value.isNull())
      ...
      if (converted.isNull())
        throw Exception(ErrorCodes::CANNOT_CONVERT_TYPE,
                        "Cannot convert parameter of aggregate function {} ({}) "
  ...
  void add(AggregateDataPtr __restrict place, const IColumn **columns, size_t row_num, Arena *) const override {
    ...
    if (length_to_resize && position >= length_to_resize) return;
  ...
  void merge(AggregateDataPtr __restrict place, ConstAggregateDataPtr rhs, Arena *) const override {
    ...
    if (arr_lhs.size() < arr_rhs.size()) arr_lhs.resize(arr_rhs.size());

    for (size_t i = 0, size = arr_rhs.size(); i < size; ++i)
      if (arr_lhs[i].isNull() && !arr_rhs[i].isNull()) arr_lhs[i] = arr_rhs[i];
  }
  ...
  void deserialize(AggregateDataPtr __restrict place, ReadBuffer &buf, std::optional<size_t> /* version */, Arena *) const override {
    ...
    for (size_t i = 0; i < size; ++i) {
      ...
      if (!is_null) serialization->deserializeBinary(arr[i], buf, format_settings);
    }
  }
  ...
  void insertResultInto(AggregateDataPtr __restrict place, IColumn &to, Arena *) const override {
    ...
    /// Pad array if need.
    for (size_t i = arr.size(); i < result_array_size; ++i) to_data.insert(default_value);
...
- If a query is executed in multiple threads, the resulting value is an undetermined one of the inserted values.
  ...
  FunctionDocumentation::Parameters parameters = {
      ...
      {"size",
       "Optional. Length of the resulting array. When using this parameter, the default value `default_x` must be specified.",
       {"UInt32"}}};
  FunctionDocumentation::ReturnedValue returned_value = {"Returns an array with inserted values.", {"Array"}};
  FunctionDocumentation::Examples examples = {{"Basic usage without parameters",
                                               R"(
