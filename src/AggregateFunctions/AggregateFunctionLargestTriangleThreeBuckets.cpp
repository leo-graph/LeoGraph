...
struct LargestTriangleThreeBucketsData : public StatisticalSample<Float64, Float64> {
  ...
  PODArray<std::pair<Float64, Float64>> getResult(size_t total_buckets) {
    ...
    if (size <= total_buckets) {
      for (size_t i = 0; i < size; ++i) result.emplace_back(this->x[index[i]], this->y[index[i]]);
      return result;
    }
    ...
    for (size_t i = 0; i < total_buckets - 2; ++i)  // Skip the first and last bucket
    {
      ...
      for (size_t j = start_index; j < center_index; ++j) {
        Float64 area = std::abs(0.5 * (result.back().first * this->y[index[j]] + this->x[index[j]] * avg_y + avg_x * result.back().second -
                                       result.back().first * avg_y - this->x[index[j]] * result.back().second - avg_x * this->y[index[j]]));
        if (area > max_area) {
          max_area = area;
          max_index = j;
        }
 ...
 public:
  explicit AggregateFunctionLargestTriangleThreeBuckets(const DataTypes &arguments, const Array &params)
    ...
    if (params[0].getType() != Field::Types::UInt64)
      throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT, "Aggregate function {} require first parameter to be a UInt64", getName());
...
AggregateFunctionPtr createAggregateFunctionLargestTriangleThreeBuckets(const std::string &name, const DataTypes &argument_types,
  ...
  if (!(isNumber(argument_types[0]) || isDateOrDate32(argument_types[0]) || isDateTime(argument_types[0]) ||
    ...
    throw Exception(ErrorCodes::NOT_IMPLEMENTED,
                    "Aggregate function {} only supports Date, Date32, DateTime, DateTime64 and Number as the first argument", name);

  if (!(isNumber(argument_types[1]) || isDateOrDate32(argument_types[1]) || isDateTime(argument_types[1]) ||
        isDateTime64(argument_types[1])))
    throw Exception(ErrorCodes::NOT_IMPLEMENTED,
                    "Aggregate function {} only supports Date, Date32, DateTime, DateTime64 and Number as the second argument", name);
...
largestTriangleThreeBuckets(n)(x, y)
  ...
  FunctionDocumentation::Parameters parameters = {{"n", "Number of points in the resulting series.", {"UInt64"}}};
  FunctionDocumentation::ReturnedValue returned_value = {"Returns an array of tuples with two elements..",
                                                         {"Array(Tuple(Float64, Float64))"}};
  FunctionDocumentation::Examples examples = {{"Downsampling time series data",
                                               R"(
...
└───────────────────────────────────────────────────────┘
  ...
  factory.registerFunction(AggregateFunctionLargestTriangleThreeBuckets::name,
                           {createAggregateFunctionLargestTriangleThreeBuckets, documentation});
