...
struct SingleValueDataBase {
  ...
  virtual std::optional<size_t> getSmallestIndexNotNullIf(const IColumn &column, const UInt8 *__restrict null_map,
                                                          const UInt8 *__restrict if_map, size_t row_begin, size_t row_end) const;
  virtual std::optional<size_t> getGreatestIndexNotNullIf(const IColumn &column, const UInt8 *__restrict null_map,
                                                          const UInt8 *__restrict if_map, size_t row_begin, size_t row_end) const;
};
...
struct SingleValueDataFixed {
  ...
  void setSmallestNotNullIf(const IColumn &column, const UInt8 *__restrict null_map, const UInt8 *__restrict if_map, size_t row_begin,
                            size_t row_end, Arena *);
  void setGreatestNotNullIf(const IColumn &column, const UInt8 *__restrict null_map, const UInt8 *__restrict if_map, size_t row_begin,
                            size_t row_end, Arena *);
  ...
  std::optional<size_t> getSmallestIndexNotNullIf(const IColumn &column, const UInt8 *__restrict null_map, const UInt8 *__restrict if_map,
                                                  size_t row_begin, size_t row_end) const;
  std::optional<size_t> getGreatestIndexNotNullIf(const IColumn &column, const UInt8 *__restrict null_map, const UInt8 *__restrict if_map,
                                                  size_t row_begin, size_t row_end) const;
  ...
  static void compileSetValueFromAggregation(llvm::IRBuilderBase &builder, llvm::Value *aggregate_data_ptr,
  ...
  static void compileAnyLastMerge(llvm::IRBuilderBase &builder, llvm::Value *aggregate_data_dst_ptr, llvm::Value *aggregate_data_src_ptr);

  template <bool isMin>
  static void compileMinMax(llvm::IRBuilderBase &builder, llvm::Value *aggregate_data_ptr, llvm::Value *value_to_check);
  template <bool isMin>
  static void compileMinMaxMerge(llvm::IRBuilderBase &builder, llvm::Value *aggregate_data_dst_ptr, llvm::Value *aggregate_data_src_ptr);
...
#define DISPATCH(TYPE)                                                                       \
  ...
  static_assert(sizeof(SingleValueDataFixed<TYPE>) <= SingleValueDataBase::MAX_STORAGE_SIZE, \
                "Incorrect size of SingleValueDataFixed struct");
...
struct SingleValueDataNumeric final : public SingleValueDataBase {
 ...
 public:
  ...
  void setSmallestNotNullIf(const IColumn &column, const UInt8 *__restrict null_map, const UInt8 *__restrict if_map, size_t row_begin,
                            size_t row_end, Arena *arena) override;
  void setGreatestNotNullIf(const IColumn &column, const UInt8 *__restrict null_map, const UInt8 *__restrict if_map, size_t row_begin,
                            size_t row_end, Arena *arena) override;
...
#define DISPATCH(TYPE)                                                                         \
  ...
  static_assert(sizeof(SingleValueDataNumeric<TYPE>) <= SingleValueDataBase::MAX_STORAGE_SIZE, \
                "Incorrect size of SingleValueDataNumeric struct");
...
static_assert(sizeof(SingleValueDataGenericWithColumn) <= SingleValueDataBase::MAX_STORAGE_SIZE,
              "Incorrect size of SingleValueDataGenericWithColumn struct");
...
#define DISPATCH(TYPE) \
  if (which.idx == TypeIndex::TYPE) return new AggregateFunctionTemplate<SingleValueDataFixed<TYPE>, isMin...>(argument_types);  /// NOLINT
  FOR_SINGLE_VALUE_NUMERIC_TYPES(DISPATCH)
#undef DISPATCH
  ...
  if (which.idx == TypeIndex::String) return new AggregateFunctionTemplate<SingleValueDataString, isMin...>(argument_types);

  if (canUseFieldForValueData(value_type)) return new AggregateFunctionTemplate<SingleValueDataGeneric, isMin...>(argument_types);
...
struct SingleValueDataBaseMemoryBlock {
  AlignedUnionT<SingleValueDataBase::MAX_STORAGE_SIZE,
                SingleValueDataNumeric<Decimal256>,  /// We check all types in generateSingleValueFromType
                SingleValueDataString, SingleValueDataGeneric, SingleValueDataGenericWithColumn>
      memory;
