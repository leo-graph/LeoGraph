...
ValueType avgResultToValue(Float64 v, UInt32 scale = 0) {
  if (std::isnan(v)) return ValueType(0);
...
template <typename T>
constexpr bool DecimalOrExtendedInt =
    is_decimal<T> || std::is_same_v<T, Int128> || std::is_same_v<T, Int256> || std::is_same_v<T, UInt128> || std::is_same_v<T, UInt256>;
...
class AggregateFunctionAvgBase : public IAggregateFunctionDataHelper<AvgFraction<TNumerator, TDenominator>, Derived> {
 public:
  ...
  AggregateFunctionAvgBase(const DataTypes &argument_types_, const DataTypePtr &result_type_, UInt32 num_scale_ = 0,
                           UInt32 denom_scale_ = 0)
  ...
  bool isCompilable() const override {
    if constexpr (!canBeNativeType<Numerator>() || !canBeNativeType<Denominator>()) return false;

    bool can_be_compiled = true;

    for (const auto &argument : this->argument_types) can_be_compiled &= canBeNativeType(*argument);
  ...
  void compileCreate(llvm::IRBuilderBase &builder, llvm::Value *aggregate_data_ptr) const override {
    ...
    b.CreateMemSet(aggregate_data_ptr, llvm::ConstantInt::get(b.getInt8Ty(), 0), sizeof(Fraction),
  ...
  llvm::Value *compileGetResult(llvm::IRBuilderBase &builder, llvm::Value *aggregate_data_ptr) const override {
    if constexpr (canBeNativeType<Numerator>() && canBeNativeType<Denominator>()) return compileGetResultImpl(builder, aggregate_data_ptr);
    return nullptr;
  }
...
class AggregateFunctionAvg : public AggregateFunctionAvgBase<AvgFieldType<T>, UInt64, AggregateFunctionAvg<T>> {
 public:
  ...
  void addBatchSinglePlaceNotNull(size_t row_begin, size_t row_end, AggregateDataPtr __restrict place, const IColumn **columns,
                                  const UInt8 *null_map, Arena *, ssize_t if_argument_pos) const final {
    AggregateFunctionSumData<Numerator> sum_data;
    const auto &column = assert_cast<const ColVecType &>(*columns[0]);
    if (if_argument_pos >= 0) {
  ...
  void compileAdd(llvm::IRBuilderBase &builder, llvm::Value *aggregate_data_ptr, const ValuesWithType &arguments) const override {
    if constexpr (canBeNativeType<Numerator>() && canBeNativeType<Denominator>()) compileAddImpl(builder, aggregate_data_ptr, arguments);
  }

#endif

 private:
  void NO_SANITIZE_UNDEFINED increment(AggregateDataPtr __restrict place, Numerator inc) const { this->data(place).numerator += inc; }
};
}  // namespace DB
