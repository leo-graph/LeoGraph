...
using FuncQuantileTiming = AggregateFunctionQuantile<Value, QuantileTiming<Value>, NameQuantileTiming, void, Float32, false, false>;
template <typename Value, bool _>
using FuncQuantilesTiming = AggregateFunctionQuantile<Value, QuantileTiming<Value>, NameQuantilesTiming, void, Float32, true, false>;
...
quantileTiming(level)(expr)
  ...
  FunctionDocumentation::Arguments arguments = {
      {"expr",
       "Expression over a column values returning a Float*-type number. If negative values are passed to the function, the behavior is "
       "undefined. If the value is greater than 30,000 (a page loading time of more than 30 seconds), it is assumed to be 30,000.",
       {"Float*"}}};
  ...
  FunctionDocumentation::ReturnedValue returned_value = {
      "Quantile of the specified level. If no values are passed to the function (when using `quantileTimingIf`), NaN is returned. The "
      "purpose of this is to differentiate these cases from cases that result in zero.",
      {"Float32"}};
  FunctionDocumentation::Examples examples = {{"Computing timing quantile",
                                               R"(
...
quantilesTiming(level1, level2, ...)(expr)
  ...
  FunctionDocumentation::Arguments arguments_quantiles = {
      {"expr",
       ...
       "undefined. If the value is greater than 30,000 (a page loading time of more than 30 seconds), it is assumed to be 30,000.",
       {"Float*"}}};
  FunctionDocumentation::Parameters parameters_quantiles = {{"level",
                                                             "Levels of quantiles. One or more constant floating-point numbers from 0 to "
                                                             "1. We recommend using `level` values in the range of `[0.01, 0.99]`.",
                                                             {"Float*"}}};
  ...
  FunctionDocumentation::Examples examples_quantiles = {{"Computing multiple timing quantiles",
                                                         R"(
...
└─────────────────────────────────────────────────┘
  ...
  factory.registerFunction(NameQuantilesTiming::name,
                           {createAggregateFunctionQuantile<FuncQuantilesTiming>, documentation_quantiles, properties});
