...
class QuantileTDigest {
  ...
  static constexpr size_t bytes_in_arena = 128 - sizeof(PODArray<Centroid>) - sizeof(BetterFloat) -
                                           sizeof(size_t);  // If alignment is imperfect, sizeof(TDigest) will be more than naively expected
 ...
 public:
  ...
  void compress() {
    if (unmerged > 0 || centroids.size() > params.max_centroids) {
      ...
      while (r != centroids.end()) {
        ...
        /// The ratio of the part of the histogram to l, including the half l to the entire histogram. That is, what level quantile in
        /// position l.
        BetterFloat ql = (sum + l_count * 0.5) / count;
        BetterFloat err = ql * (1 - ql);

        /// The ratio of the portion of the histogram to l, including l and half r to the entire histogram. That is, what level is the
        /// quantile in position r.
  ...
  void add(T x, UInt64 cnt = 1) {
    ...
    if (cnt == 0 || std::isnan(vx)) return;  // Count 0 breaks compress() assumptions, Nan breaks sort(). We treat them as no sample.
  ...
  void deserialize(ReadBuffer &buf) {
    ...
    if (size > max_centroids_deserialize) throw Exception(ErrorCodes::TOO_LARGE_ARRAY_SIZE, "Too large t-digest centroids size");
  ...
  Float64 getCountEqual(Float64 value) const {
    ...
    for (const auto &c : centroids) {
      ...
      if (value == c.mean) result += c.count;
    }
    return result;
  }
  ...
  Float64 getCountLessThan(Float64 value) const {
    ...
    for (const auto &c : centroids) {
      ...
      if (c.mean >= value) {
        ...
        Float64 result = checkOverflow<Float64>(
            interpolate(static_cast<Value>(value), prev_mean, static_cast<Value>(left), c.mean, static_cast<Value>(right)));
        return result;
      }
      sum += c.count;
      prev_mean = c.mean;
  ...
  ResultType getImpl(Float64 level) {
    if (centroids.empty()) return is_floating_point<ResultType> ? std::numeric_limits<ResultType>::quiet_NaN() : 0;

    compress();

    if (centroids.size() == 1) return checkOverflow<ResultType>(centroids.front().mean);
    ...
    for (const auto &c : centroids) {
      ...
      if (current_x >= x) {
        ...
        if (x <= left) return checkOverflow<ResultType>(prev_mean);
        if (x >= right) return checkOverflow<ResultType>(c.mean);
  ...
  void getManyImpl(const Float64 *levels, const size_t *levels_permutation, size_t size, ResultType *result) {
    ...
    if (centroids.size() == 1) {
      for (size_t result_num = 0; result_num < size; ++result_num) result[result_num] = static_cast<ResultType>(centroids.front().mean);
      return;
    }
      ...
      if (current_x >= x) {
        ...
        while (current_x >= x) {
          if (x <= left)
            ...
            result[levels_permutation[result_num]] = static_cast<ResultType>(
                interpolate(static_cast<Value>(x), static_cast<Value>(left), prev_mean, static_cast<Value>(right), c.mean));
    ...
    for (; result_num < size; ++result_num) result[levels_permutation[result_num]] = static_cast<ResultType>(rest_of_results);
  }
  ...
  void getMany(const Float64 *levels, const size_t *indices, size_t size, T *result) { getManyImpl(levels, indices, size, result); }
 ...
 private:
  ...
  static ResultType checkOverflow(Value val) {
    ResultType result;
    if (accurate::convertNumeric(val, result)) return result;
