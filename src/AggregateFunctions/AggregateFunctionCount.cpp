...
class AggregateFunctionCountNotNullUnary final
 ...
 public:
  ...
  bool haveSameStateRepresentationImpl(const IAggregateFunction &rhs) const override { return this->getName() == rhs.getName(); }
  ...
  void merge(AggregateDataPtr __restrict place, ConstAggregateDataPtr rhs, Arena *) const override { data(place).count += data(rhs).count; }
...
AggregateFunctionPtr AggregateFunctionCount::getOwnNullAdapter(const AggregateFunctionPtr &, const DataTypes &types, const Array &params,
...
bool AggregateFunctionCount::isCompilable() const {
  ...
  for (const auto &argument_type : argument_types) is_compilable &= canBeNativeType(*argument_type);
...
void AggregateFunctionCount::compileCreate(llvm::IRBuilderBase &builder, llvm::Value *aggregate_data_ptr) const {
  ...
  b.CreateMemSet(aggregate_data_ptr, llvm::ConstantInt::get(b.getInt8Ty(), 0), sizeof(AggregateFunctionCountData),
...
void AggregateFunctionCount::compileMerge(llvm::IRBuilderBase &builder, llvm::Value *aggregate_data_dst_ptr,
...
void AggregateFunctionCountNotNullUnary::compileAdd(llvm::IRBuilderBase &builder, llvm::Value *aggregate_data_ptr,
...
void AggregateFunctionCountNotNullUnary::compileMerge(llvm::IRBuilderBase &builder, llvm::Value *aggregate_data_dst_ptr,
                                                      llvm::Value *aggregate_data_src_ptr) const {
  llvm::IRBuilder<> &b = static_cast<llvm::IRBuilder<> &>(builder);
...
:::
  ...
  FunctionDocumentation::ReturnedValue returned_value = {
      "Returns the a row count if the function is called without parameters, otherwise returns a count of how many times the passed "
      "expression returned not null.",
      {"UInt64"}};
  FunctionDocumentation::Examples examples = {{"Basic row count",
                                               R"(
SELECT count() FROM t
        )",
                                               R"(
