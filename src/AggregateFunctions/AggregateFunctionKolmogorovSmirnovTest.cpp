...
struct KolmogorovSmirnov : public StatisticalSample<Float64, Float64> {
  enum class Alternative : uint8_t { TwoSided, Less, Greater };
  ...
  std::pair<Float64, Float64> getResult(Alternative alternative, String method) {
    ...
    while (pos_x < x.size() && pos_y < y.size()) {
      if (likely(fabs(x[pos_x] - y[pos_y]) >= tol)) {
        ...
        pos_tmp = pos_x + 1;
        while (pos_tmp < x.size() && unlikely(fabs(x[pos_tmp] - x[pos_x]) <= tol)) pos_tmp++;
        now_s += n1_d * static_cast<Float64>(pos_tmp - pos_x);
        pos_x = pos_tmp;
        pos_tmp = pos_y + 1;
        while (pos_tmp < y.size() && unlikely(fabs(y[pos_tmp] - y[pos_y]) <= tol)) pos_tmp++;
    ...
    if (method == "exact") {
      ...
      for (UInt64 i = 1; i <= n2; i++) {
        if (check(k_d, static_cast<Float64>(i) / f_n2, 0.)) c[0] = 1.;
        for (UInt64 j = 1; j <= n1; j++)
      ...
      if (alternative == Alternative::TwoSided) {
        ...
        if (p < 1) {
          ...
          w = log(p);
          s = 0;
          for (UInt64 k = 1; k < k_max; k += 2) s += exp(static_cast<Float64>(k) * static_cast<Float64>(k) * z - w);
...
class AggregateFunctionKolmogorovSmirnov final
 ...
 public:
  explicit AggregateFunctionKolmogorovSmirnov(const DataTypes &arguments, const Array &params)
    ...
    if (param == "two-sided")
      ...
      throw Exception(ErrorCodes::BAD_ARGUMENTS,
                      "Unknown parameter in aggregate function {}. "
                      "It must be one of: 'two-sided', 'less', 'greater'",
                      getName());
    ...
    if (method != "auto" && method != "exact" && method != "asymp" && method != "asymptotic")
      throw Exception(ErrorCodes::BAD_ARGUMENTS,
                      "Unknown method in aggregate function {}. "
                      "It must be one of: 'auto', 'exact', 'asymp' (or 'asymptotic')",
                      getName());
  }

  String getName() const override { return "kolmogorovSmirnovTest"; }
  ...
  static DataTypePtr createResultType() {
    ...
    Strings names{"d_statistic", "p_value"};
...
AggregateFunctionPtr createAggregateFunctionKolmogorovSmirnovTest(const std::string &name, const DataTypes &argument_types,
...
kolmogorovSmirnovTest([alternative, computation_method])(sample_data, sample_index)
  ...
  FunctionDocumentation::Arguments arguments = {{"sample_data", "Sample data.", {"(U)Int*", "Float*", "Decimal"}},
                                                {"sample_index", "Sample index.", {"(U)Int*"}}};
  FunctionDocumentation::Parameters parameters = {
      {"alternative",
       "Alternative hypothesis. (Optional, default: 'two-sided'.) Let `F(x) and G(x)` be the CDFs of the first and second distributions "
       "respectively. 'two-sided': The null hypothesis is that samples come from the same distribution, e.g. `F(x) = G(x)` for all x. And "
       "the alternative is that the distributions are not identical. 'greater': The null hypothesis is that values in the first sample are "
       "stochastically smaller than those in the second one, e.g. the CDF of first distribution lies above and hence to the left of that "
       "for the second one. Which in fact means that `F(x) >= G(x)` for all x. And the alternative in this case is that `F(x) < G(x)` for "
       "at least one x. 'less': The null hypothesis is that values in the first sample are stochastically greater than those in the second "
       "one, e.g. the CDF of first distribution lies below and hence to the right of that for the second one. Which in fact means that "
       "`F(x) <= G(x)` for all x. And the alternative in this case is that `F(x) > G(x)` for at least one x.",
       {"String"}},
      {"computation_method",
       "The method used to compute p-value. (Optional, default: 'auto'.) 'exact': calculation is performed using precise probability "
       "distribution of the test statistics. Compute intensive and wasteful except for small samples. 'asymp' ('asymptotic'): calculation "
       "is performed using an approximation. For large sample sizes, the exact and asymptotic p-values are very similar. 'auto': the "
       "'exact' method is used when a maximum number of samples is less than 10'000.",
       {"String"}}};
  FunctionDocumentation::ReturnedValue returned_value = {
      "Returns a tuple with two elements: a calculated statistic and a calculated p-value.", {"Tuple(Float64, Float64)"}};
  FunctionDocumentation::Examples examples = {{"Same distribution test",
                                               R"(
...
└─────────────────────────────────────────────────────────┘
  ...
  factory.registerFunction("kolmogorovSmirnovTest", {createAggregateFunctionKolmogorovSmirnovTest, documentation},
