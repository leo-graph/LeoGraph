...
enum class GroupArraySortedStrategy : uint8_t { heap, sort };
...
struct GroupArraySortedData {
  ...
  struct Comparator {
    bool operator()(const T &lhs, const T &rhs) { return compare(lhs, rhs); }
  };
  ...
  ALWAYS_INLINE void heapReplaceTop() {
    ...
    if (values.size() > 2 && compare(values[1], values[2])) ++child_index;

    /// Check if we are in order
    if (compare(values[child_index], values[0])) return;
  ...
  ALWAYS_INLINE void sortAndLimit(size_t max_elements, Arena *arena) {
    if constexpr (is_value_generic_field) {
      ...
      if (!try_sort) RadixSort<RadixSortNumTraits<T>>::executeLSD(values.data(), values.size());
    }

    if (values.size() > max_elements) resize(max_elements, arena);
  }

  ALWAYS_INLINE void partialSortAndLimitIfNeeded(size_t max_elements, Arena *arena) {
    if (values.size() < max_elements * partial_sort_max_elements_factor) return;
  ...
  ALWAYS_INLINE void addElement(T &&element, size_t max_elements, Arena *arena) {
    if constexpr (strategy == GroupArraySortedStrategy::heap) {
      if (values.size() >= max_elements) {
        ...
        if (!compare(element, values[0])) return;
  ...
  ALWAYS_INLINE void insertResultInto(IColumn &to, size_t max_elements, Arena *arena) {
    ...
    if constexpr (is_value_generic_field) {
      ...
      for (auto &value : values) result_array_data.insert(value);
    } else {
      ...
      for (size_t i = 0; i < values.size(); ++i) result_array_data[result_array_data_insert_begin + i] = values[i];
    }
  }
};
...
class GroupArraySorted final : public IAggregateFunctionDataHelper<Data, GroupArraySorted<Data, T>> {
 public:
  explicit GroupArraySorted(const DataTypePtr &data_type_, const Array &parameters_, UInt64 max_elements_)
      : IAggregateFunctionDataHelper<Data, GroupArraySorted<Data, T>>({data_type_}, parameters_,
                                                                      std::make_shared<DataTypeArray>(data_type_)),
  ...
  void merge(AggregateDataPtr __restrict place, ConstAggregateDataPtr rhs, Arena *arena) const override {
    ...
    for (auto rhs_element : rhs_values) this->data(place).addElement(std::move(rhs_element), max_elements, arena);
  }
  ...
  void serialize(ConstAggregateDataPtr __restrict place, WriteBuffer &buf, std::optional<size_t> /* version */) const override {
    ...
    if constexpr (std::is_same_v<T, Field>) {
      ...
      if constexpr (std::endian::native == std::endian::little) {
        ...
        for (const auto &element : values) writeBinaryLittleEndian(element, buf);
      }
    }
  }
  ...
  void deserialize(AggregateDataPtr __restrict place, ReadBuffer &buf, std::optional<size_t> /* version */, Arena *arena) const override {
    ...
    if constexpr (Data::is_value_generic_field) {
      ...
      for (Field &element : values) {
        ...
        if (has_value) serialization->deserializeBinary(element, buf, {});
      }
    } else {
      ...
      if constexpr (std::endian::native == std::endian::little) {
        ...
        for (auto &element : values) readBinaryLittleEndian(element, buf);
      }
    }
  }
...
template <template <typename> class AggregateFunctionTemplate, typename... TArgs>
inline AggregateFunctionPtr createAggregateFunctionGroupArraySortedImpl(const DataTypePtr &argument_type, const Array &parameters,
                                                                        TArgs... args) {
...
groupArraySorted(N)(column)
  ...
  FunctionDocumentation::Arguments arguments = {{"column", "Column for which to group into an array.", {"Any"}}};
  FunctionDocumentation::Parameters parameters = {{"N", "The number of elements to return.", {"UInt64"}}};
  FunctionDocumentation::ReturnedValue returned_value = {"Returns an array with the first N items in ascending order.", {"Array"}};
  FunctionDocumentation::Examples examples = {{"Getting first 10 numbers",
                                               R"(
