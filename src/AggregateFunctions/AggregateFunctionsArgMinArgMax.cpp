...
struct AggregateFunctionArgMinMaxData {
 ...
 public:
  ...
  static bool allocatesMemoryInArena(TypeIndex) { return ResultType::allocatesMemoryInArena() || ValueType::allocatesMemoryInArena(); }
};
...
static_assert(sizeof(AggregateFunctionArgMinMaxDataGeneric<Int8>) <= 2 * SingleValueDataBase::MAX_STORAGE_SIZE,
...
class AggregateFunctionArgMinMax final : public IAggregateFunctionDataHelper<Data, AggregateFunctionArgMinMax<Data, isMin>> {
 ...
 public:
  ...
  void add(AggregateDataPtr __restrict place, const IColumn **columns, size_t row_num, Arena *arena) const override {
    if constexpr (isMin) {
      if (this->data(place).value().setIfSmaller(*columns[1], row_num, arena)) this->data(place).result().set(*columns[0], row_num, arena);
    } else {
      if (this->data(place).value().setIfGreater(*columns[1], row_num, arena)) this->data(place).result().set(*columns[0], row_num, arena);
    }
  }
  ...
  void addBatchSinglePlace(size_t row_begin, size_t row_end, AggregateDataPtr __restrict place, const IColumn **__restrict columns,
                           Arena *arena, ssize_t if_argument_pos) const override {
    ...
    if (idx) add(place, columns, *idx, arena);
  }

  void addBatchSinglePlaceNotNull(size_t row_begin, size_t row_end, AggregateDataPtr __restrict place, const IColumn **__restrict columns,
                                  const UInt8 *__restrict null_map, Arena *arena, ssize_t if_argument_pos) const override {
    std::optional<size_t> idx;
    if (if_argument_pos >= 0) {
      const auto &if_map = assert_cast<const ColumnUInt8 &>(*columns[if_argument_pos]).getData();
      if constexpr (isMin)
  ...
  void deserialize(AggregateDataPtr __restrict place, ReadBuffer &buf, std::optional<size_t> /* version */, Arena *arena) const override {
    ...
    if (unlikely(this->data(place).value().has() != this->data(place).result().has()))
      throw Exception(ErrorCodes::INCORRECT_DATA, "Invalid state of the aggregate function {}: has_value ({}) != has_result ({})",
                      getName(), this->data(place).value().has(), this->data(place).result().has());
  }

  bool allocatesMemoryInArena() const override { return Data::allocatesMemoryInArena(result_type_index); }
...
IAggregateFunction *createWithTwoTypes(const DataTypes &argument_types, const bool return_both) {
  ...
  if (which_result.idx == TypeIndex::UInt8) return createWithTwoTypesSecond<isMin, UInt8>(argument_types, return_both);
  if (which_result.idx == TypeIndex::UInt16) return createWithTwoTypesSecond<isMin, UInt16>(argument_types, return_both);
  if (which_result.idx == TypeIndex::UInt32) return createWithTwoTypesSecond<isMin, UInt32>(argument_types, return_both);
  if (which_result.idx == TypeIndex::UInt64) return createWithTwoTypesSecond<isMin, UInt64>(argument_types, return_both);
  if (which_result.idx == TypeIndex::Int8) return createWithTwoTypesSecond<isMin, Int8>(argument_types, return_both);
  if (which_result.idx == TypeIndex::Int16) return createWithTwoTypesSecond<isMin, Int16>(argument_types, return_both);
  if (which_result.idx == TypeIndex::Int32) return createWithTwoTypesSecond<isMin, Int32>(argument_types, return_both);
  if (which_result.idx == TypeIndex::Int64) return createWithTwoTypesSecond<isMin, Int64>(argument_types, return_both);
  if (which_result.idx == TypeIndex::Float32) return createWithTwoTypesSecond<isMin, Float32>(argument_types, return_both);
  if (which_result.idx == TypeIndex::Float64) return createWithTwoTypesSecond<isMin, Float64>(argument_types, return_both);

  return nullptr;
}

template <bool isMin>
AggregateFunctionPtr createAggregateFunctionArgMinMax(const std::string &name, const DataTypes &argument_types, const Array &,
                                                      const Settings *, const bool return_both) {
...
#define DISPATCH(TYPE)                                                                                                                    \
  if (which.idx == TypeIndex::TYPE)                                                                                                       \
    return AggregateFunctionPtr(new AggregateFunctionArgMinMax<AggregateFunctionArgMinMaxDataGeneric<SingleValueDataFixed<TYPE>>, isMin>( \
        argument_types, return_both));  /// NOLINT
...
#undef DISPATCH
    ...
    if (which.idx == TypeIndex::DateTime)
      return AggregateFunctionPtr(
          new AggregateFunctionArgMinMax<AggregateFunctionArgMinMaxDataGeneric<SingleValueDataFixed<DataTypeDateTime::FieldType>>, isMin>(
              argument_types, return_both));
    if (which.idx == TypeIndex::String)
      return AggregateFunctionPtr(
          new AggregateFunctionArgMinMax<AggregateFunctionArgMinMaxDataGeneric<SingleValueDataString>, isMin>(argument_types, return_both));

    if (canUseFieldForValueData(value_type))
      return AggregateFunctionPtr(new AggregateFunctionArgMinMax<AggregateFunctionArgMinMaxDataGeneric<SingleValueDataGeneric>, isMin>(
          argument_types, return_both));
    return AggregateFunctionPtr(
        new AggregateFunctionArgMinMax<AggregateFunctionArgMinMaxDataGeneric<SingleValueDataGenericWithColumn>, isMin>(argument_types,
                                                                                                                       return_both));
  }
  return result;
}
...
argMin(arg, val)
  ...
  FunctionDocumentation::ReturnedValue returned_value_argMin = {
      "Returns the `arg` value that corresponds to minimum `val` value. Type matches `arg` type.", {}};
  FunctionDocumentation::Examples examples_argMin = {{"Basic usage",
                                                      R"(
...
└────────────────────────┴──────────────────┘
  ...
  FunctionDocumentation documentation_argMin = {description_argMin,    syntax_argMin,   arguments_argMin,     parameters_argMin,
                                                returned_value_argMin, examples_argMin, introduced_in_argMin, category_argMin};

  factory.registerFunction("argMin",
...
argMax(arg, val)
  ...
  FunctionDocumentation::ReturnedValue returned_value_argMax = {
      "Returns the `arg` value that corresponds to maximum `val` value. Type matches `arg` type.", {}};
  FunctionDocumentation::Examples examples_argMax = {{"Basic usage",
                                                      R"(
...
└────────────────────────┘
  ...
  FunctionDocumentation documentation_argMax = {description_argMax,    syntax_argMax,   arguments_argMax,     parameters_argMax,
                                                returned_value_argMax, examples_argMax, introduced_in_argMax, category_argMax};

  factory.registerFunction("argMax",
                           {[](const std::string &name, const DataTypes &argument_types, const Array &params, const Settings *settings) {
...
argAndMin(arg, val)
  ...
  FunctionDocumentation::Arguments arguments_argAndMin = {
      ...
      {"val", "The minimum value.", {"(U)Int8/16/32/64", "Float*", "Date", "DateTime", "Tuple"}}};
  FunctionDocumentation::ReturnedValue returned_value_argAndMin = {
      "Returns a tuple containing the `arg` value that corresponds to minimum `val` value and the minimum `val` value.", {"Tuple"}};
  FunctionDocumentation::Examples examples_argAndMin = {{"Basic usage",
                                                         R"(
...
└──────────────────────┴─────────────┘
  ...
  FunctionDocumentation documentation_argAndMin = {description_argAndMin,   syntax_argAndMin,         arguments_argAndMin,
                                                   parameters_argAndMin,    returned_value_argAndMin, examples_argAndMin,
                                                   introduced_in_argAndMin, category_argAndMin};

  factory.registerFunction("argAndMin",
                           {[](const std::string &name, const DataTypes &argument_types, const Array &params, const Settings *settings) {
...
argAndMax(arg, val)
  ...
  FunctionDocumentation::Arguments arguments_argAndMax = {
      {"arg", "Argument for which to find the maximum value.", {"const String"}},
      {"val", "The maximum value.", {"(U)Int8/16/32/64", "Float*", "Date", "DateTime", "Tuple"}}};
  FunctionDocumentation::ReturnedValue returned_value_argAndMax = {
      "Returns a tuple containing the `arg` value that corresponds to maximum `val` value and the maximum `val` value.", {"Tuple"}};
  FunctionDocumentation::Examples examples_argAndMax = {{"Basic usage",
                                                         R"(
...
└──────────────────────┘
  ...
  FunctionDocumentation documentation_argAndMax = {description_argAndMax,   syntax_argAndMax,         arguments_argAndMax,
                                                   parameters_argAndMax,    returned_value_argAndMax, examples_argAndMax,
                                                   introduced_in_argAndMax, category_argAndMax};

  factory.registerFunction("argAndMax",
                           {[](const std::string &name, const DataTypes &argument_types, const Array &params, const Settings *settings) {
