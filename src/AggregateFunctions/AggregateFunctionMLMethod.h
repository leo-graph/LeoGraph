...
class IGradientComputer {
 public:
  ...
  virtual void compute(VectorWithMemoryTracking<Float64> &batch_gradient, const VectorWithMemoryTracking<Float64> &weights, Float64 bias,
                       Float64 l2_reg_coef, Float64 target, const IColumn **columns, size_t row_num) = 0;

  virtual void predict(ColumnVector<Float64>::Container &container, const ColumnsWithTypeAndName &arguments, size_t offset, size_t limit,
                       const VectorWithMemoryTracking<Float64> &weights, Float64 bias, ContextPtr context) const = 0;
};
...
class LinearRegression : public IGradientComputer {
 public:
  ...
  void compute(VectorWithMemoryTracking<Float64> &batch_gradient, const VectorWithMemoryTracking<Float64> &weights, Float64 bias,
               Float64 l2_reg_coef, Float64 target, const IColumn **columns, size_t row_num) override;

  void predict(ColumnVector<Float64>::Container &container, const ColumnsWithTypeAndName &arguments, size_t offset, size_t limit,
               const VectorWithMemoryTracking<Float64> &weights, Float64 bias, ContextPtr context) const override;
};
...
class IWeightsUpdater {
 public:
  ...
  virtual void addToBatch(VectorWithMemoryTracking<Float64> &batch_gradient, IGradientComputer &gradient_computer,
  ...
  virtual void update(UInt64 batch_size, VectorWithMemoryTracking<Float64> &weights, Float64 &bias, Float64 learning_rate,
                      const VectorWithMemoryTracking<Float64> &gradient) = 0;
...
class StochasticGradientDescent : public IWeightsUpdater {
 public:
  void update(UInt64 batch_size, VectorWithMemoryTracking<Float64> &weights, Float64 &bias, Float64 learning_rate,
              const VectorWithMemoryTracking<Float64> &batch_gradient) override;
};

class Momentum : public IWeightsUpdater {
 public:
  explicit Momentum(size_t num_params, Float64 alpha_ = 0.1) : alpha(alpha_) { accumulated_gradient.resize(num_params + 1, 0); }
...
class Nesterov : public IWeightsUpdater {
 public:
  explicit Nesterov(size_t num_params, Float64 alpha_ = 0.9) : alpha(alpha_) { accumulated_gradient.resize(num_params + 1, 0); }

  void addToBatch(VectorWithMemoryTracking<Float64> &batch_gradient, IGradientComputer &gradient_computer,
                  const VectorWithMemoryTracking<Float64> &weights, Float64 bias, Float64 l2_reg_coef, Float64 target,
                  const IColumn **columns, size_t row_num) override;
...
class LinearModelData {
 public:
  ...
  LinearModelData(Float64 learning_rate_, Float64 l2_reg_coef_, UInt64 param_num_, UInt64 batch_capacity_,
                  std::shared_ptr<IGradientComputer> gradient_computer_, std::shared_ptr<IWeightsUpdater> weights_updater_);
...
class AggregateFunctionMLMethod final : public IAggregateFunctionDataHelper<Data, AggregateFunctionMLMethod<Data, Name>> {
 public:
  ...
  explicit AggregateFunctionMLMethod(UInt32 param_num_, std::unique_ptr<IGradientComputer> gradient_computer_,
                                     std::string weights_updater_name_, Float64 learning_rate_, Float64 l2_reg_coef_, UInt64 batch_size_,
                                     const DataTypes &arguments_types, const Array &params)
  ...
  static DataTypePtr createResultType() { return std::make_shared<DataTypeArray>(std::make_shared<DataTypeFloat64>()); }

  bool allocatesMemoryInArena() const override { return false; }

  /// This function is called from evalMLMethod function for correct predictValues call
  DataTypePtr getReturnTypeToPredict() const override { return std::make_shared<DataTypeNumber<Float64>>(); }
  ...
  void predictValues(ConstAggregateDataPtr __restrict place, IColumn &to, const ColumnsWithTypeAndName &arguments, size_t offset,
                     size_t limit, ContextPtr context) const override {
    if (arguments.size() != param_num + 1)
      throw Exception(ErrorCodes::NUMBER_OF_ARGUMENTS_DOESNT_MATCH, "Predict got incorrect number of arguments. Got: {}. Required: {}",
    ...
    if (!column)
      throw Exception(ErrorCodes::LOGICAL_ERROR,
                      "Cast of column of predictions is incorrect. "
   ...
   */
  void insertResultInto(AggregateDataPtr __restrict place, IColumn &to, Arena *) const override { this->data(place).returnWeights(to); }
