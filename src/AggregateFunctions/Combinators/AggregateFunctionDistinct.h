...
struct AggregateFunctionDistinctSingleNumericData {
  ...
  void merge(const Self &rhs, MutableColumns &argument_columns, Arena *) {
    for (const auto &elem : rhs.history) {
      ...
      if (inserted) argument_columns[0]->insert(value);
    }
  }
...
struct AggregateFunctionDistinctGenericData {
  ...
  void serialize(WriteBuffer &buf) const {
    ...
    for (const auto &elem : history) writeStringBinary(elem.getValue(), buf);
  }

  void deserialize(ReadBuffer &buf, Arena *arena) {
    size_t size;
    readVarUInt(size, buf);
    for (size_t i = 0; i < size; ++i) history.insert(readStringBinaryInto(*arena, buf));
  }
};
...
struct AggregateFunctionDistinctSingleGenericData : public AggregateFunctionDistinctGenericData {
  ...
  void merge(const Self &rhs, MutableColumns &argument_columns, Arena *arena) {
    for (const auto &elem : rhs.history) {
      ...
      if (inserted) deserializeAndInsert<is_plain_column>(it->getValue(), *argument_columns[0]);
    }
  }
};
  ...
  void merge(const Self &rhs, MutableColumns &argument_columns, Arena *arena) {
    for (const auto &elem : rhs.history) {
      ...
      if (!history.contains(value)) {
        ...
        auto settings = IColumn::SerializationSettings::createForAggregationState();
        for (auto &column : argument_columns) column->deserializeAndInsertFromArena(in, &settings);
      }
    }
  }
};
...
class AggregateFunctionDistinct final : public IAggregateFunctionDataHelper<Data, AggregateFunctionDistinct<Data>> {
 private:
  ...
  MutableColumns prepareArgumentColumns() const {
    ...
    for (const auto &type : this->argument_types) argument_columns.emplace_back(type->createColumn());
  ...
  void addToNested(AggregateDataPtr __restrict place, const MutableColumns &argument_columns, Arena *arena) const {
    ...
    for (size_t i = 0; i < argument_columns.size(); ++i) arguments_raw[i] = argument_columns[i].get();
 ...
 public:
  ...
  void add(AggregateDataPtr __restrict place, const IColumn **columns, size_t row_num, Arena *arena) const override {
    ...
    if (added) addToNested(row_num, row_num + 1, place, columns, arena);
  }
  ...
  size_t alignOfData() const override { return std::max(alignof(Data), nested_func->alignOfData()); }
