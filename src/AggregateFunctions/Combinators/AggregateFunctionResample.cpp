...
class AggregateFunctionCombinatorResample final : public IAggregateFunctionCombinator {
 public:
  String getName() const override { return "Resample"; }
  ...
  Array transformParameters(const Array &params) const override {
    if (params.size() < 3)
      throw Exception(ErrorCodes::NUMBER_OF_ARGUMENTS_DOESNT_MATCH, "Incorrect number of parameters for aggregate function with {} suffix",
                      getName());
  ...
  AggregateFunctionPtr transformAggregateFunction(const AggregateFunctionPtr &nested_function, const AggregateFunctionProperties &,
    ...
    if (which.isNativeUInt() || which.isDate() || which.isDateTime() || which.isDateTime64()) {
      ...
      return std::make_shared<AggregateFunctionResample<UInt64>>(nested_function, begin, end, step, arguments, params);
    }
    ...
    if (which.isNativeInt() || which.isEnum() || which.isInterval()) {
      ...
      if (!params[params.size() - 3].tryGet<Int64>(begin)) begin = params[params.size() - 3].safeGet<UInt64>();
      if (!params[params.size() - 2].tryGet<Int64>(end)) end = params[params.size() - 2].safeGet<UInt64>();

      UInt64 step = params[params.size() - 1].safeGet<UInt64>();

      return std::make_shared<AggregateFunctionResample<Int64>>(nested_function, begin, end, step, arguments, params);
    }

    throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT,
                    "Illegal types of argument for aggregate function {}, the type "
                    "of the last argument should be native integer or integer-like",
                    getName());
  }
};
