...
class AggregateFunctionNullBase : public IAggregateFunctionHelper<Derived> {
 ...
 public:
  ...
  void destroy(AggregateDataPtr __restrict place) const noexcept override { nested_function->destroy(nestedPlace(place)); }
  ...
  void parallelizeMergePrepare(AggregateDataPtrs &places, ThreadPool &thread_pool, std::atomic<bool> &is_cancelled) const override {
    ...
    for (auto &nested_place : nested_places) nested_place = nestedPlace(nested_place);

    nested_function->parallelizeMergePrepare(nested_places, thread_pool, is_cancelled);
  }

  void merge(AggregateDataPtr __restrict place, ConstAggregateDataPtr rhs, ThreadPool &thread_pool, std::atomic<bool> &is_cancelled,
  ...
  void serialize(ConstAggregateDataPtr __restrict place, WriteBuffer &buf, std::optional<size_t> version) const override {
    ...
    if constexpr (serialize_flag) writeBinary(flag, buf);
    if (flag) nested_function->serialize(nestedPlace(place), buf, version);
  }

  void deserialize(AggregateDataPtr __restrict place, ReadBuffer &buf, std::optional<size_t> version, Arena *arena) const override {
    bool flag = true;
    if constexpr (serialize_flag) readBinary(flag, buf);
    if (flag) {
      setFlag(place);
  ...
  bool isCompilable() const override { return this->nested_function->isCompilable(); }

  void compileCreate(llvm::IRBuilderBase &builder, llvm::Value *aggregate_data_ptr) const override {
    llvm::IRBuilder<> &b = static_cast<llvm::IRBuilder<> &>(builder);

    if constexpr (result_is_nullable)
      b.CreateMemSet(aggregate_data_ptr, llvm::ConstantInt::get(b.getInt8Ty(), 0), this->prefix_size,
  ...
  void compileMerge(llvm::IRBuilderBase &builder, llvm::Value *aggregate_data_dst_ptr, llvm::Value *aggregate_data_src_ptr) const override {
    ...
    auto *aggregate_data_dst_ptr_with_prefix_size_offset =
        b.CreateConstInBoundsGEP1_64(b.getInt8Ty(), aggregate_data_dst_ptr, this->prefix_size);
    auto *aggregate_data_src_ptr_with_prefix_size_offset =
        b.CreateConstInBoundsGEP1_64(b.getInt8Ty(), aggregate_data_src_ptr, this->prefix_size);
...
class AggregateFunctionNullUnary final
    : public AggregateFunctionNullBase<result_is_nullable, serialize_flag, AggregateFunctionNullUnary<result_is_nullable, serialize_flag>> {
 public:
  AggregateFunctionNullUnary(AggregateFunctionPtr nested_function_, const DataTypes &arguments, const Array &params)
      : AggregateFunctionNullBase<result_is_nullable, serialize_flag, AggregateFunctionNullUnary<result_is_nullable, serialize_flag>>(
  ...
  AggregateFunctionPtr getAggregateFunctionForMergingFinal() const override {
    ...
    if (nested_function_for_merging_final.get() != this->nested_function.get())
      return std::make_shared<AggregateFunctionNullUnary<result_is_nullable, serialize_flag>>(nested_function_for_merging_final,
  ...
  void addBatchSinglePlace(  /// NOLINT
    ...
    this->nested_function->addBatchSinglePlaceNotNull(row_begin, row_end, this->nestedPlace(place), &nested_column, null_map, arena,
                                                      if_argument_pos);

    if constexpr (result_is_nullable)
      if (!memoryIsByte(null_map, row_begin, row_end, 1)) this->setFlag(place);
  }
  ...
  void addBatchSparse(size_t row_begin, size_t row_end, AggregateDataPtr *places, size_t place_offset, const IColumn **columns,
    ...
    for (size_t i = from; i < to; ++i) {
      ...
      if (places[offset]) {
        this->nested_function->add(this->nestedPlace(places[offset] + place_offset), &nested_column, i + 1, arena);
  ...
  void compileAdd(llvm::IRBuilderBase &builder, llvm::Value *aggregate_data_ptr, const ValuesWithType &arguments) const override {
    ...
    if constexpr (result_is_nullable) b.CreateStore(llvm::ConstantInt::get(b.getInt8Ty(), 1), aggregate_data_ptr);

    auto *aggregate_data_ptr_with_prefix_size_offset = b.CreateConstInBoundsGEP1_64(b.getInt8Ty(), aggregate_data_ptr, this->prefix_size);
    this->nested_function->compileAdd(b, aggregate_data_ptr_with_prefix_size_offset,
                                      {ValueWithType(wrapped_value, removeNullable(nullable_type))});
    b.CreateBr(join_block);
...
class AggregateFunctionNullVariadic final
 ...
 public:
  AggregateFunctionNullVariadic(AggregateFunctionPtr nested_function_, const DataTypes &arguments, const Array &params)
    ...
    if (number_of_arguments == 1) throw Exception(ErrorCodes::LOGICAL_ERROR, "Single argument is passed to AggregateFunctionNullVariadic");

    if (number_of_arguments > MAX_ARGS)
      throw Exception(ErrorCodes::NUMBER_OF_ARGUMENTS_DOESNT_MATCH,
                      "Maximum number of arguments for aggregate function with Nullable types is {}", size_t(MAX_ARGS));

    for (size_t i = 0; i < number_of_arguments; ++i) is_nullable[i] = arguments[i]->isNullable();
  }
  ...
  AggregateFunctionPtr getAggregateFunctionForMergingFinal() const override {
    ...
    /// Create new aggregate function for merging final states if the nested function has a different implementation
    if (nested_function_for_merging_final.get() != this->nested_function.get())
      return std::make_shared<AggregateFunctionNullVariadic<result_is_nullable, serialize_flag>>(nested_function_for_merging_final,
                                                                                                 argument_types, parameters);
    return IAggregateFunction::getAggregateFunctionForMergingFinal();
  }
  ...
  void addBatchSinglePlace(size_t row_begin, size_t row_end, AggregateDataPtr __restrict place, const IColumn **columns, Arena *arena,
    ...
    if (nullable_filters.size() == 1) {
      ...
      for (size_t filter = filter_start; filter < nullable_filters.size(); filter++) {
        for (size_t i = row_begin; i < row_end; i++) final_flags[i] |= nullable_filters[filter][i];
      }
    ...
    if (!found_one) return;  // Nothing to do and nothing to mark

    this->setFlag(place);
    this->nested_function->addBatchSinglePlaceNotNull(row_begin, row_end, this->nestedPlace(place), nested_columns.data(), final_flags_ptr,
                                                      arena, -1);
  }
