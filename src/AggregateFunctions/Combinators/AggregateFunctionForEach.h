...
class AggregateFunctionForEach final : public IAggregateFunctionDataHelper<AggregateFunctionForEachData, AggregateFunctionForEach> {
    ...
    if (old_size < new_size) {
      ...
      if (common::mulOverflow(new_size, nested_size_of_data, allocation_size))
        throw Exception(
            ...
            "Allocation size ({} * {}) overflows in -ForEach aggregate function, but it should've been prevented by previous checks",
      ...
      for (i = 0; i < old_size; ++i) {
        nested_func->merge(&new_state[i * nested_size_of_data], &old_state[i * nested_size_of_data], &arena);
      }
 ...
 public:
  ...
  static DataTypePtr createResultType(AggregateFunctionPtr nested_) { return std::make_shared<DataTypeArray>(nested_->getResultType()); }
  ...
  void add(AggregateDataPtr __restrict place, const IColumn **columns, size_t row_num, Arena *arena) const override {
    ...
    for (size_t i = 0; i < num_arguments; ++i) nested[i] = &assert_cast<const ColumnArray &>(*columns[i]).getData();
