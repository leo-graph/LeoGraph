...
struct AggregateFunctionMapCombinatorData<String> {
  ...
  static void writeKey(String key, WriteBuffer &buf) { writeStringBinary(key, buf); }
  static void readKey(String &key, ReadBuffer &buf) { readStringBinary(key, buf); }
};
...
struct AggregateFunctionMapCombinatorData<IPv6> {
  ...
  static void writeKey(const IPv6 &key, WriteBuffer &buf) { writeIPv6Binary(key, buf); }
  static void readKey(IPv6 &key, ReadBuffer &buf) { readIPv6Binary(key, buf); }
};
...
class AggregateFunctionMap final
 ...
 public:
  ...
  static DataTypePtr getKeyType(const DataTypes &types, const AggregateFunctionPtr &nested) {
    if (types.size() != 1)
      throw Exception(ErrorCodes::NUMBER_OF_ARGUMENTS_DOESNT_MATCH, "Aggregate function {}Map requires one map argument, but {} found",
                      nested->getName(), types.size());

    const auto *map_type = checkAndGetDataType<DataTypeMap>(types[0].get());
    if (!map_type)
      throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT, "Aggregate function {}Map requires map as argument", nested->getName());
  ...
  void destroy(AggregateDataPtr __restrict place) const noexcept override { destroyImpl<false>(place); }

  bool hasTrivialDestructor() const override { return std::is_trivially_destructible_v<Data> && nested_func->hasTrivialDestructor(); }

  void destroyUpToState(AggregateDataPtr __restrict place) const noexcept override { destroyImpl<true>(place); }
  ...
  AggregateFunctionPtr transformAggregateFunction(const AggregateFunctionPtr &nested_function, const AggregateFunctionProperties &,
    ...
    if (map_type) {
      ...
      switch (key_type->getTypeId()) {
        ...
        default:
          throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT, "Map key type {} is not is not supported by combinator {}",
                          key_type->getName(), getName());
      }
    } else {
      // in case of tuple of arrays or just arrays (checked in transformArguments), try to redirect to sum/min/max-MappedArrays to implement
      // old behavior
      ...
      throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT, "Aggregation '{}Map' is not implemented for mapped arrays", nested_func_name);
    }
  }
};
