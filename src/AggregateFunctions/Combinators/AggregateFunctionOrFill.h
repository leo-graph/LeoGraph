...
class AggregateFunctionOrFill final : public IAggregateFunctionHelper<AggregateFunctionOrFill<UseNull>> {
 ...
 public:
  ...
  size_t sizeOfData() const override { return size_of_data + sizeof(char); }
  ...
  void addBatch(  /// NOLINT
    ...
    if (if_argument_pos >= 0) {
      ...
      for (size_t i = row_begin; i < row_end; ++i) {
        if (flags[i] && places[i]) add(places[i] + place_offset, columns, i, arena);
      }
    } else {
      nested_function->addBatch(row_begin, row_end, places, place_offset, columns, arena, if_argument_pos);
      for (size_t i = row_begin; i < row_end; ++i)
        if (places[i]) (places[i] + place_offset)[size_of_data] = 1;
    }
  }
  ...
  void mergeBatch(size_t row_begin, size_t row_end, AggregateDataPtr *places, size_t place_offset, const AggregateDataPtr *rhs,
    ...
    for (size_t i = row_begin; i < row_end; ++i) (places[i] + place_offset)[size_of_data] |= rhs[i][size_of_data];
  }
  ...
  static DataTypePtr createResultType(const DataTypePtr &inner_type_) {
    if constexpr (UseNull) {
      ...
      if (inner_type_->isNullable()) return inner_type_;
