...
class AggregateFunctionCombinatorIf final : public IAggregateFunctionCombinator {
 public:
  ...
  DataTypes transformArguments(const DataTypes &arguments) const override {
    if (arguments.empty())
      throw Exception(ErrorCodes::NUMBER_OF_ARGUMENTS_DOESNT_MATCH, "Incorrect number of arguments for aggregate function with {} suffix",
                      getName());

    if (!isUInt8(arguments.back()) && !arguments.back()->onlyNull())
      throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT,
                      "Illegal type {} of last argument for "
                      "aggregate function with {} suffix",
...
class AggregateFunctionIfNullUnary final
 ...
 private:
  ...
  using Base =
      AggregateFunctionNullBase<result_is_nullable, serialize_flag, AggregateFunctionIfNullUnary<result_is_nullable, serialize_flag>>;
 ...
 public:
  ...
  AggregateFunctionIfNullUnary(const String &name_, AggregateFunctionPtr nested_function_, const DataTypes &arguments, const Array &params)
      : Base(std::move(nested_function_), arguments, params), num_arguments(arguments.size()), name(name_) {
    if (num_arguments == 0)
  ...
  void addBatchSinglePlace(size_t row_begin, size_t row_end, AggregateDataPtr __restrict place, const IColumn **columns, Arena *arena,
    ...
    if (filter_null_map)
      for (size_t i = row_begin; i < row_end; ++i) final_nulls[i] = (!!null_map[i]) | (!filter_values[i]) | (!!filter_null_map[i]);
    else
      for (size_t i = row_begin; i < row_end; ++i) final_nulls[i] = (!!null_map[i]) | (!filter_values[i]);
    ...
    this->nested_function->addBatchSinglePlaceNotNull(row_begin, row_end, this->nestedPlace(place), columns_param, final_nulls.get(), arena,
                                                      -1);
  }

#if USE_EMBEDDED_COMPILER

  bool isCompilable() const override { return canBeNativeType(*this->argument_types.back()) && this->nested_function->isCompilable(); }
...
class AggregateFunctionIfNullVariadic final
 ...
 public:
  String getName() const override { return Base::getName(); }
  ...
  void addBatchSinglePlace(size_t row_begin, size_t row_end, AggregateDataPtr __restrict place, const IColumn **columns, Arena *arena,
    ...
    if (is_nullable[filter_column_num]) {
      ...
      const UInt8 *filter_values = assert_cast<const ColumnUInt8 *>(filter_column)->getData().data();
      for (size_t i = row_begin; i < row_end; i++) final_null_flags[i] = !filter_values[i];
    }
    ...
    for (size_t arg = 0; arg < number_of_arguments; arg++) {
      if (is_nullable[arg]) {
        ...
        if (arg != filter_column_num) {
          ...
          for (size_t r = row_begin; r < row_end; r++) final_null_flags[r] |= !!col_null_map[r];
        }
    ...
    if (at_least_one) {
      this->setFlag(place);
      this->nested_function->addBatchSinglePlaceNotNull(row_begin, row_end, this->nestedPlace(place), nested_columns.data(),
                                                        final_null_flags.get(), arena, -1);
    }
  }
 ...
 private:
  using Base =
      AggregateFunctionNullBase<result_is_nullable, serialize_flag, AggregateFunctionIfNullVariadic<result_is_nullable, serialize_flag>>;
...
AggregateFunctionPtr AggregateFunctionIf::getOwnNullAdapter(const AggregateFunctionPtr &nested_function, const DataTypes &arguments,
  ...
  bool return_type_is_nullable =
      !properties.returns_default_when_only_null && getResultType()->canBeInsideNullable() &&
  ...
  if (need_to_serialize_flag) return std::make_shared<AggregateFunctionIfNullVariadic<false, true>>(nested_function, arguments, params);
