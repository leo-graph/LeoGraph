...
class AggregateFunctionCombinatorNull final : public IAggregateFunctionCombinator {
 public:
  ...
  std::optional<AggregateFunctionPtr> tryTransformStateFunctionImpl(const AggregateFunctionPtr &nested_function,
                                                                    ...
                                                                    const DataTypes &arguments, const Array &params) const {
    if (const T *function_state = typeid_cast<const T *>(nested_function.get())) {
      ...
      return std::make_shared<T>(transformed_nested_function, transformed_nested_function->getArgumentTypes(),
                                 transformed_nested_function->getParameters());
    }
    return {};
  }

  AggregateFunctionPtr tryTransformStateFunction(const AggregateFunctionPtr &nested_function, const AggregateFunctionProperties &properties,
                                                 const DataTypes &arguments, const Array &params) const {
    return tryTransformStateFunctionImpl<AggregateFunctionState>(nested_function, properties, arguments, params)
        .or_else(
            [&]() { return tryTransformStateFunctionImpl<AggregateFunctionSimpleState>(nested_function, properties, arguments, params); })
        .value_or(nullptr);
  }

  AggregateFunctionPtr transformAggregateFunction(const AggregateFunctionPtr &nested_function,
                                                  const AggregateFunctionProperties &properties, const DataTypes &arguments,
                                                  const Array &params) const override {
    bool has_nullable_types = false;
    bool has_null_types = false;
    UnorderedSetWithMemoryTracking<size_t> arguments_that_can_be_only_null;
    if (nested_function) arguments_that_can_be_only_null = nested_function->getArgumentsThatCanBeOnlyNull();
    ...
    if (!has_nullable_types)
      throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT,
                      "Aggregate function combinator 'Null' "
    ...
    if (has_null_types) {
       ...
       * and we can lose this information. For example, when we have `count(NULL)` replaced with `nothing(NULL) as "count(NULL)"` and send
       * it to the remote server, the remote server will execute `nothing(NULL)` and return `NULL` while `0` is expected.
       *
       * To address this, we handle `nothing` in a special way in `FunctionNode::toASTImpl`.
       */
      if (properties.returns_default_when_only_null) return std::make_shared<AggregateFunctionNothingUInt64>(arguments, params);
      return std::make_shared<AggregateFunctionNothingNull>(arguments, params);
    }

    assert(nested_function);

    if (auto adapter = nested_function->getOwnNullAdapter(nested_function, arguments, params, properties)) return adapter;

    /// If applied to aggregate function with either -State/-SimpleState combinator, we apply -Null combinator to it's nested_function
    /// instead of itself. Because Nullable AggregateFunctionState does not make sense and ruins the logic of managing aggregate function
    /// states.
    ...
    if (arguments.size() == 1) {
      ...
      if (serialize_flag) return std::make_shared<AggregateFunctionNullUnary<false, true>>(nested_function, arguments, params);
