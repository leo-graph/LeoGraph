...
enum class Kind : uint8_t { OrNull, OrDefault };
...
class AggregateFunctionCombinatorOrFill final : public IAggregateFunctionCombinator {
 ...
 public:
  ...
  String getName() const override { return kind == Kind::OrNull ? "OrNull" : "OrDefault"; }
  ...
  AggregateFunctionPtr transformAggregateFunction(const AggregateFunctionPtr &nested_function, const AggregateFunctionProperties &,
    ...
    if (kind == Kind::OrNull) return std::make_shared<AggregateFunctionOrFill<true>>(nested_function, arguments, params);
