...
class AggregateFunctionIf final : public IAggregateFunctionHelper<AggregateFunctionIf> {
 ...
 public:
  ...
  DataTypePtr getNormalizedStateType() const override { return nested_func->getNormalizedStateType(); }
  ...
  void add(AggregateDataPtr __restrict place, const IColumn **columns, size_t row_num, Arena *arena) const override {
    if (only_null_condition) return;
    if (assert_cast<const ColumnUInt8 &>(*columns[num_arguments - 1]).getData()[row_num]) nested_func->add(place, columns, row_num, arena);
  }
  ...
  void addBatch(size_t row_begin, size_t row_end, AggregateDataPtr *__restrict places, size_t place_offset, const IColumn **columns,
                Arena *arena, ssize_t) const override {
    if (only_null_condition) return;
  ...
  void addBatchSinglePlaceNotNull(size_t row_begin, size_t row_end, AggregateDataPtr __restrict place, const IColumn **columns,
                                  const UInt8 *null_map, Arena *arena, ssize_t) const override {
    if (only_null_condition) return;
  ...
  AggregateFunctionPtr getOwnNullAdapter(const AggregateFunctionPtr &nested_function, const DataTypes &arguments, const Array &params,
                                         const AggregateFunctionProperties &properties) const override;

  AggregateFunctionPtr getNestedFunction() const override { return nested_func; }

  UnorderedSetWithMemoryTracking<size_t> getArgumentsThatCanBeOnlyNull() const override { return {num_arguments - 1}; }

#if USE_EMBEDDED_COMPILER

  bool isCompilable() const override { return canBeNativeType(*this->argument_types.back()) && nested_func->isCompilable(); }
