...
std::pair<RanksArray, Float64> computeRanksAndTieCorrection(const Values &values) {
  ...
  std::sort(indexes.begin(), indexes.end(), [&](size_t lhs, size_t rhs) { return values[lhs] < values[rhs]; });
  ...
  while (left < size) {
    size_t right = left;
    while (right < size && values[indexes[left]] == values[indexes[right]]) ++right;
    ...
    for (size_t iter = left; iter < right; ++iter) out[indexes[iter]] = adjusted;
    left = right;
  }
