...
AggregateFunctionPtr createAggregateFunctionUniqCombined(bool use_64_bit_hash, const std::string &name, const DataTypes &argument_types,
                                                         const Array &params) {
  ...
  if (!params.empty()) {
    ...
    if (precision_param > 20 || precision_param < 12)
      throw Exception(ErrorCodes::ARGUMENT_OUT_OF_BOUND, "Parameter for aggregate function {} is out of range: [12, 20].", name);
...
uniqCombined(x[, ...])
  ...
  FunctionDocumentation::Parameters parameters_uniqCombined = {
      {"HLL_precision",
       "Optional. The base-2 logarithm of the number of cells in HyperLogLog. The default value is 17, which is effectively 96 KiB of "
       "space (2^17 cells, 6 bits each). Range: [12, 20].",
       {"UInt8"}}};
  ...
  FunctionDocumentation::ReturnedValue returned_value_uniqCombined = {
      "Returns a UInt64-type number representing the approximate number of different argument values.", {"UInt64"}};
  FunctionDocumentation::Examples examples_uniqCombined = {{"Basic usage",
                                                            R"(
...
└──────────────────────────┘
  ...
  FunctionDocumentation documentation_uniqCombined = {description_uniqCombined,   syntax_uniqCombined,         arguments_uniqCombined,
                                                      parameters_uniqCombined,    returned_value_uniqCombined, examples_uniqCombined,
                                                      introduced_in_uniqCombined, category_uniqCombined};
...
uniqCombined64(x[, ...])
  ...
  FunctionDocumentation::Arguments arguments_uniqCombined64 = {
      {"x", "A variable number of parameters.", {"Tuple(T)", "Array(T)", "Date", "DateTime", "String", "(U)Int*", "Float*", "Decimal"}}};
  FunctionDocumentation::ReturnedValue returned_value_uniqCombined64 = {
      "Returns a UInt64-type number representing the approximate number of different argument values.", {"UInt64"}};
  FunctionDocumentation::Examples examples_uniqCombined64 = {{"Large dataset example",
                                                              R"(
...
└──────────────────────┘
  ...
  FunctionDocumentation documentation_uniqCombined64 = {
      description_uniqCombined64,    syntax_uniqCombined64,   arguments_uniqCombined64,     parameters_uniqCombined64,
      returned_value_uniqCombined64, examples_uniqCombined64, introduced_in_uniqCombined64, category_uniqCombined64};
