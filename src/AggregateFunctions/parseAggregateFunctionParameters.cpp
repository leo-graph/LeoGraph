...
Array getAggregateFunctionParametersArray(const ASTPtr& expression_list, const std::string& error_context, ContextPtr context) {
  ...
  if (parameters.empty()) throw Exception(ErrorCodes::BAD_ARGUMENTS, "Parameters list to aggregate functions cannot be empty");
    ...
    try {
      ...
      if (e.code() == ErrorCodes::BAD_ARGUMENTS)
        throw Exception(ErrorCodes::PARAMETERS_TO_AGGREGATE_FUNCTIONS_MUST_BE_LITERALS,
                        ...
                        "Got parameter '{}'{}",
                        parameters[i]->formatForErrorMessage(), (error_context.empty() ? "" : " (in " + error_context + ")"));
...
void getAggregateFunctionNameAndParametersArray(const std::string& aggregate_function_name_with_params,
                                                std::string& aggregate_function_name, Array& aggregate_function_parameters,
                                                const std::string& error_context, ContextPtr context) {
  ...
  if (pos == std::string::npos || pos + 2 >= aggregate_function_name_with_params.size())
    throw Exception(ErrorCodes::BAD_ARGUMENTS, "{} doesn't look like aggregate function name in {}.", aggregate_function_name_with_params,
                    error_context);
  ...
  ASTPtr args_ast = parseQuery(params_parser, parameters_str.data(), parameters_str.data() + parameters_str.size(),
                               "parameters of aggregate function in " + error_context, 0, DBMS_DEFAULT_MAX_PARSER_DEPTH,
                               DBMS_DEFAULT_MAX_PARSER_BACKTRACKS);

  if (args_ast->children.empty())
    throw Exception(ErrorCodes::BAD_ARGUMENTS, "Incorrect list of parameters to aggregate function {}", aggregate_function_name);
