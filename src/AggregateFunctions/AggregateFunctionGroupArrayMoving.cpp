...
struct MovingSumData : public MovingData<T> {
  ...
  T NO_SANITIZE_UNDEFINED get(size_t idx, UInt64 window_size) const {
    if (idx < window_size) return this->value[idx];
...
struct MovingAvgData : public MovingData<T> {
  ...
  T NO_SANITIZE_UNDEFINED get(size_t idx, UInt64 window_size) const {
    if (idx < window_size) return this->value[idx] / T(window_size);
...
class MovingImpl final : public IAggregateFunctionDataHelper<Data, MovingImpl<T, LimitNumElements, Data>> {
 ...
 public:
  ...
  void NO_SANITIZE_UNDEFINED merge(AggregateDataPtr __restrict place, ConstAggregateDataPtr rhs, Arena *arena) const override {
    ...
    if (rhs_elems.value.size()) cur_elems.value.insert(rhs_elems.value.begin(), rhs_elems.value.end(), arena);
  ...
  void deserialize(AggregateDataPtr __restrict place, ReadBuffer &buf, std::optional<size_t> /* version */, Arena *arena) const override {
    ...
    if (unlikely(size > AGGREGATE_FUNCTION_MOVING_MAX_ARRAY_SIZE))
      throw Exception(ErrorCodes::TOO_LARGE_ARRAY_SIZE, "Too large array size (maximum: {})", AGGREGATE_FUNCTION_MOVING_MAX_ARRAY_SIZE);
...
struct MovingSum {
  using Data = MovingSumData<
      std::conditional_t<is_decimal<T>, std::conditional_t<sizeof(T) <= sizeof(Decimal128), Decimal128, Decimal256>, NearestFieldType<T>>>;
  using Function = MovingImpl<T, LimitNumberOfElements, Data>;
};

template <typename T, typename LimitNumberOfElements>
struct MovingAvg {
  using Data = MovingAvgData<
      std::conditional_t<is_decimal<T>, std::conditional_t<sizeof(T) <= sizeof(Decimal128), Decimal128, Decimal256>, Float64>>;
  using Function = MovingImpl<T, LimitNumberOfElements, Data>;
};
...
AggregateFunctionPtr createAggregateFunctionMoving(const std::string &name, const DataTypes &argument_types, const Array &parameters,
  ...
  if (!limit_size) {
    if (isDecimal(argument_type)) return createAggregateFunctionMovingImpl<Function, std::false_type, std::true_type>(name, argument_type);
...
groupArrayMovingSum(window_size)(numbers_for_summing)
  ...
  FunctionDocumentation::Parameters parameters_sum = {
      {"window_size",
       "Size of the calculation window. If left unspecified, the function takes the window size equal to the number of rows in the column.",
       {"UInt64"}}};
  ...
  FunctionDocumentation::Examples examples_sum = {{"Usage example",
                                                   R"(
CREATE TABLE t
(
    `int` UInt8,
...
└────────────┴─────────────────────────────────┴────────────────────────┘
  ...
  FunctionDocumentation documentation_sum = {description_sum,    syntax_sum,   arguments_sum,     parameters_sum,
