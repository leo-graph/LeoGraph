...
IAggregateFunction *createBSIVectorWithTwoNumericTypesSecond(const IDataType &second_type, const DataTypes &types, const Array &params,
                                                             TArgs &&...args) {
  WhichDataType which(second_type);
#define DISPATCH(TYPE)                                                                                                   \
  if (which.idx == TypeIndex::TYPE)                                                                                      \
    return new AggregateFunctionNumericIndexedVector<VectorImpl<FirstType, TYPE>, std::decay_t<TArgs>...>(types, params, \
...
IAggregateFunction *createBSIVectorWithTwoNumericTypesFirst(const IDataType &first_type, const IDataType &second_type,
                                                            const DataTypes &types, const Array &params, TArgs &&...args) {
...
AggregateFunctionPtr createAggregateFunctionNumericIndexedVector(const std::string &name, const DataTypes &argument_types,
  ...
  if (vector_type_str == "BSI") {
    ...
    if (!(first_which.isInt8() or first_which.isInt16() or first_which.isInt32() or first_which.isUInt8() or first_which.isUInt16() or
          first_which.isUInt32()))
      throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT,
