...
class AggregateFunctionHistogramData {
 ...
 private:
  ...
  struct PriorityQueueStorage {
    ...
    explicit PriorityQueueStorage(T *value) : data_ptr(value) {}
  ...
  void compress(UInt32 max_bins) {
    ...
    std::priority_queue<QueueItem, PriorityQueueStorage<QueueItem>, std::greater<>> queue{std::greater<>(),
                                                                                          PriorityQueueStorage<QueueItem>(storage.data())};

    auto quality = [&](UInt32 i) { return points[next[i]].mean - points[i].mean; };

    for (size_t i = 0; i + 1 < size; ++i) queue.push({quality(static_cast<UInt32>(i)), i});
    ...
    while (new_size > max_bins && !queue.empty()) {
      ...
      if (!active[left] || !active[right] || quality(left) > min_item.first) continue;

      points[left] = points[left] + points[right];

      delete_node(right);
      if (active[next[left]]) queue.push({quality(left), left});
      if (active[previous[left]]) queue.push({quality(previous[left]), previous[left]});
 ...
 public:
  AggregateFunctionHistogramData()
    ...
    static_assert(offsetof(AggregateFunctionHistogramData, points) == sizeof(AggregateFunctionHistogramData),
                  "points should be last member");
  }

  static size_t structSize(size_t max_bins) { return sizeof(AggregateFunctionHistogramData) + max_bins * 2 * sizeof(WeightedValue); }
  ...
  void add(Mean value, Weight weight, UInt32 max_bins) {
    ...
    if (!isFinite(value)) throw Exception(ErrorCodes::INCORRECT_DATA, "Invalid value (inf or nan) for aggregation by 'histogram' function");

    points[size] = {value, weight};
    ++size;
    // check if the input sequence is monotonic increasing, which is a commonly occurring pattern
    if (sorted_prefix == size - 1 && last_inserted <= value) ++sorted_prefix;
    last_inserted = value;
    lower_bound = std::min(lower_bound, value);
    upper_bound = std::max(upper_bound, value);

    if (size >= max_bins * 2) compress(max_bins);
  }
  ...
  void merge(const AggregateFunctionHistogramData &other, UInt32 max_bins) {
    ...
    for (size_t i = 0; i < other.size; ++i) add(other.points[i].mean, other.points[i].weight, max_bins);
  }
  ...
  void read(ReadBuffer &buf, UInt32 max_bins) {
    ...
    if (size > max_bins * 2) throw Exception(ErrorCodes::TOO_LARGE_ARRAY_SIZE, "Too many bins");
    static constexpr size_t max_size = 1_GiB;
    if (size > max_size) throw Exception(ErrorCodes::TOO_LARGE_ARRAY_SIZE, "Too large array size in histogram (maximum: {})", max_size);
...
class AggregateFunctionHistogram final
    : public IAggregateFunctionDataHelper<AggregateFunctionHistogramData, AggregateFunctionHistogram<T>> {
 private:
 ...
 public:
  ...
  size_t sizeOfData() const override { return Data::structSize(max_bins); }
...
AggregateFunctionPtr createAggregateFunctionHistogram(const std::string &name, const DataTypes &arguments, const Array &params,
  ...
  if (params[0].getType() != Field::Types::UInt64) throw Exception(ErrorCodes::UNSUPPORTED_PARAMETER, "Invalid type for bins count");
  ...
  if (bins_count == 0) throw Exception(ErrorCodes::BAD_ARGUMENTS, "Bin count should be positive");
  ...
  if (!res)
    throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT, "Illegal type {} of argument for aggregate function {}", arguments[0]->getName(),
                    name);
