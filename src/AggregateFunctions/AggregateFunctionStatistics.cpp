...
bool areComparable(UInt64 a, UInt64 b) {
  ...
  return (((1 - static_cast<Float64>(res.first) / static_cast<Float64>(res.second)) < sensitivity) &&
          (static_cast<Float64>(res.first) > threshold));
}
 ...
 */
class AggregateFunctionVariance final : public IAggregateFunctionDataHelper<AggregateFunctionVarianceData, AggregateFunctionVariance> {
 private:
  VarKind kind;

  static Float64 getVarSamp(Float64 m2, UInt64 count) {
    if (count < 2) return std::numeric_limits<Float64>::infinity();
    return m2 / static_cast<Float64>(count - 1);
  }

  static Float64 getStddevSamp(Float64 m2, UInt64 count) { return sqrt(getVarSamp(m2, count)); }

  static Float64 getVarPop(Float64 m2, UInt64 count) {
    if (count == 0) return std::numeric_limits<Float64>::infinity();
    if (count == 1) return 0.0;
    return m2 / static_cast<Float64>(count);
  }

  static Float64 getStddevPop(Float64 m2, UInt64 count) { return sqrt(getVarPop(m2, count)); }

  Float64 getResult(ConstAggregateDataPtr __restrict place) const {
    const auto &dt = data(place);
    switch (kind) {
      case VarKind::varSampStable:
        return getVarSamp(dt.m2, dt.count);
      case VarKind::stddevSampStable:
        return getStddevSamp(dt.m2, dt.count);
      case VarKind::varPopStable:
        return getVarPop(dt.m2, dt.count);
      case VarKind::stddevPopStable:
        return getStddevPop(dt.m2, dt.count);
    }
  }

 public:
  explicit AggregateFunctionVariance(VarKind kind_, const DataTypePtr &arg)
      : IAggregateFunctionDataHelper<AggregateFunctionVarianceData, AggregateFunctionVariance>({arg}, {},
  ...
  String getName() const override {
    switch (kind) {
      case VarKind::varSampStable:
        return "varSampStable";
      case VarKind::stddevSampStable:
        return "stddevSampStable";
      case VarKind::varPopStable:
        return "varPopStable";
      case VarKind::stddevPopStable:
        return "stddevPopStable";
    }
  }
  ...
  void merge(AggregateDataPtr __restrict place, ConstAggregateDataPtr rhs, Arena *) const override { data(place).mergeWith(data(rhs)); }
...
struct CovarianceData : public BaseCovarianceData<compute_marginal_moments> {
  ...
  void mergeWith(const CovarianceData &source) {
    ...
    if (areComparable(count, source.count)) {
      left_mean = (static_cast<Float64>(source.count) * source.left_mean + static_cast<Float64>(count) * left_mean) /
                  static_cast<Float64>(total_count);
      right_mean = (static_cast<Float64>(source.count) * source.right_mean + static_cast<Float64>(count) * right_mean) /
                   static_cast<Float64>(total_count);
    } else {
...
class AggregateFunctionCovariance final
    : public IAggregateFunctionDataHelper<CovarianceData<compute_marginal_moments>, AggregateFunctionCovariance<compute_marginal_moments>> {
 private:
  CovarKind kind;
  ...
  Float64 getResult(ConstAggregateDataPtr __restrict place) const {
    ...
    switch (kind) {
      case CovarKind::covarSampStable:
        return getCovarSamp(data.co_moment, data.count);
      case CovarKind::covarPopStable:
        return getCovarPop(data.co_moment, data.count);
 ...
 public:
  explicit AggregateFunctionCovariance(CovarKind kind_, const DataTypes &args)
      : IAggregateFunctionDataHelper<CovarianceData<compute_marginal_moments>, AggregateFunctionCovariance<compute_marginal_moments>>(
            args, {}, std::make_shared<DataTypeFloat64>()),
        kind(kind_) {}

  String getName() const override {
    switch (kind) {
      case CovarKind::covarSampStable:
        return "covarSampStable";
      case CovarKind::covarPopStable:
        return "covarPopStable";
      case CovarKind::corrStable:
        return "corrStable";
    }
  }
...
AggregateFunctionPtr createAggregateFunctionStatisticsUnary(const std::string &name, const DataTypes &argument_types,
...
AggregateFunctionPtr createAggregateFunctionStatisticsBinary(const std::string &name, const DataTypes &argument_types,
                                                             const Array &parameters, const Settings *) {
  assertNoParameters(name, parameters);
...
varSampStable(x)
  ...
  FunctionDocumentation::Examples examples_varSampStable = {{"Computing stable sample variance",
                                                             R"(
...
└─────────────────┘
  ...
  FunctionDocumentation documentation_varSampStable = {
      description_varSampStable,    syntax_varSampStable,   arguments_varSampStable,     {},
      returned_value_varSampStable, examples_varSampStable, introduced_in_varSampStable, category_varSampStable};
...
varPopStable(x)
  ...
  FunctionDocumentation::Examples examples_varPopStable = {{"Computing stable population variance",
                                                            R"(
DROP TABLE IF EXISTS test_data;
CREATE TABLE test_data
(
...
└────────────────┘
  ...
  FunctionDocumentation documentation_varPopStable = {
      description_varPopStable,    syntax_varPopStable,   arguments_varPopStable,     {},
      returned_value_varPopStable, examples_varPopStable, introduced_in_varPopStable, category_varPopStable};
...
stddevSampStable(x)
  ...
  FunctionDocumentation::ReturnedValue returned_value_stddevSampStable = {"Returns the square root of sample variance of `x`.",
                                                                          {"Float64"}};
  FunctionDocumentation::Examples examples_stddevSampStable = {{"Basic usage",
                                                                R"(
DROP TABLE IF EXISTS test_data;
CREATE TABLE test_data
(
...
└──────────────────────────────┘
  ...
  FunctionDocumentation documentation_stddevSampStable = {
      description_stddevSampStable,    syntax_stddevSampStable,   arguments_stddevSampStable,     parameters_stddevSampStable,
      returned_value_stddevSampStable, examples_stddevSampStable, introduced_in_stddevSampStable, category_stddevSampStable};

  factory.registerFunction("stddevSampStable",
                           {[](const std::string &name, const DataTypes &argument_types, const Array &parameters, const Settings *) {
                              assertNoParameters(name, parameters);
                              assertUnary(name, argument_types);
...
stddevPopStable(x)
  ...
  FunctionDocumentation::Examples examples_stddevPopStable = {{"Basic usage",
                                                               R"(
DROP TABLE IF EXISTS test_data;
CREATE TABLE test_data
(
...
└────────────────────┘
  ...
  FunctionDocumentation documentation_stddevPopStable = {
      description_stddevPopStable,    syntax_stddevPopStable,   arguments_stddevPopStable,     parameters_stddevPopStable,
      returned_value_stddevPopStable, examples_stddevPopStable, introduced_in_stddevPopStable, category_stddevPopStable};

  factory.registerFunction("stddevPopStable",
                           {[](const std::string &name, const DataTypes &argument_types, const Array &parameters, const Settings *) {
                              assertNoParameters(name, parameters);
                              assertUnary(name, argument_types);
...
As a result, `covarSampStable` is slower than `covarSamp` but provides a lower computational error.
  ...
  FunctionDocumentation::Arguments covarSampStable_arguments = {{"x", "First variable.", {"(U)Int*", "Float*", "Decimal"}},
                                                                {"y", "Second variable.", {"(U)Int*", "Float*", "Decimal"}}};
  FunctionDocumentation::Parameters covarSampStable_parameters = {};
  FunctionDocumentation::ReturnedValue covarSampStable_returned_value = {
      "Returns the sample covariance between `x` and `y`. For `n <= 1`, `inf` is returned.", {"Float64"}};
  FunctionDocumentation::Examples covarSampStable_examples = {{"Basic sample covariance calculation with stable algorithm",
                                                               R"(
...
└───────────────────────────────────┘
  ...
  FunctionDocumentation covarSampStable_documentation = {
      covarSampStable_description,    covarSampStable_syntax,   covarSampStable_arguments,     covarSampStable_parameters,
      covarSampStable_returned_value, covarSampStable_examples, covarSampStable_introduced_in, covarSampStable_category};
...
It is similar to the [`covarPop`](/sql-reference/aggregate-functions/reference/covarpop) function, but uses a numerically stable algorithm. As a result, `covarPopStable` is slower than `covarPop` but produces a more accurate result.
  ...
  FunctionDocumentation::Arguments covarPopStable_arguments = {{"x", "First variable.", {"(U)Int*", "Float*", "Decimal"}},
                                                               {"y", "Second variable.", {"(U)Int*", "Float*", "Decimal"}}};
  FunctionDocumentation::Parameters covarPopStable_parameters = {};
  FunctionDocumentation::ReturnedValue covarPopStable_returned_value = {"Returns the population covariance between `x` and `y`.",
                                                                        {"Float64"}};
  FunctionDocumentation::Examples covarPopStable_examples = {{"Basic population covariance calculation with stable algorithm",
                                                              R"(
DROP TABLE IF EXISTS series;
CREATE TABLE series(i UInt32, x_value Float64, y_value Float64) ENGINE = Memory;
INSERT INTO series(i, x_value, y_value) VALUES (1, 5.6,-4.4),(2, -9.6,3),(3, -1.3,-4),(4, 5.3,9.7),(5, 4.4,0.037),(6, -8.6,-7.8),(7, 5.1,9.3),(8, 7.9,-3.6),(9, -8.2,0.62),(10, -3,7.3);
...
└──────────────────────────────────┘
  ...
  FunctionDocumentation covarPopStable_documentation = {
      covarPopStable_description,    covarPopStable_syntax,   covarPopStable_arguments,     covarPopStable_parameters,
      covarPopStable_returned_value, covarPopStable_examples, covarPopStable_introduced_in, covarPopStable_category};
...
As a result, `corrStable` is slower than `corr` but produces a more accurate result.
  ...
  FunctionDocumentation::Arguments corrStable_arguments = {{"x", "First variable.", {"(U)Int*", "Float*", "Decimal"}},
                                                           {"y", "Second variable.", {"(U)Int*", "Float*", "Decimal"}}};
  ...
  FunctionDocumentation::Examples corrStable_examples = {{"Basic correlation calculation with stable algorithm",
                                                          R"(
DROP TABLE IF EXISTS series;
CREATE TABLE series
(
...
└──────────────────────────────┘
  ...
  FunctionDocumentation corrStable_documentation = {corrStable_description,   corrStable_syntax,         corrStable_arguments,
                                                    corrStable_parameters,    corrStable_returned_value, corrStable_examples,
                                                    corrStable_introduced_in, corrStable_category};
