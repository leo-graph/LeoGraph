...
enum BitmapKind { Small = 0, Bitmap = 1 };
...
class RoaringBitmapWithSmallSet : private boost::noncopyable {
 private:
  ...
  void toLarge() {
    ...
    for (const auto &x : small) roaring_bitmap->add(static_cast<Value>(x.getValue()));
    small.clear();
  }
 ...
 public:
  ...
  void merge(const RoaringBitmapWithSmallSet &r1) {
    if (r1.isLarge()) {
      ...
      *roaring_bitmap |= *r1.roaring_bitmap;
      ...
      for (const auto &x : r1.small) add(x.getValue());
    }
  }
  ...
  void read(DB::ReadBuffer &in) {
    ...
    if (BitmapKind::Small == kind) {
      ...
      if (size == 0) throw Exception(ErrorCodes::INCORRECT_DATA, "Incorrect size (0) in groupBitmap.");
      if (size > max_size) throw Exception(ErrorCodes::TOO_LARGE_ARRAY_SIZE, "Too large array size in groupBitmap (maximum: {})", max_size);
  ...
  std::shared_ptr<RoaringBitmap> getNewRoaringBitmapFromSmall() const {
    ...
    for (const auto &x : small) ret->add(static_cast<Value>(x.getValue()));
    return ret;
  }
  ...
  void rb_and(const RoaringBitmapWithSmallSet &r1)  /// NOLINT
  {
    ...
    if (isSmall() && r1.isSmall()) {
      ...
      for (const auto &x : small)
        if (r1.small.find(x.getValue()) != r1.small.end()) buffer.push_back(x.getValue());

      // Clear out the original values
      small.clear();

      for (const auto &value : buffer) small.insert(value);

      buffer.clear();
    } else if (isSmall() && r1.isLarge()) {
      for (const auto &x : small) {
        if (r1.roaring_bitmap->contains(static_cast<Value>(x.getValue()))) buffer.push_back(x.getValue());
      }
  ...
  UInt64 rb_and_cardinality(const RoaringBitmapWithSmallSet &r1) const  /// NOLINT
  {
    ...
    if (isSmall() && r1.isSmall()) {
      for (const auto &x : small)
        if (r1.small.find(x.getValue()) != r1.small.end()) ++ret;
    } else if (isSmall() && r1.isLarge()) {
      for (const auto &x : small) {
        if (r1.roaring_bitmap->contains(static_cast<Value>(x.getValue()))) ++ret;
      }
    } else {
      std::shared_ptr<RoaringBitmap> new_rb = r1.isSmall() ? r1.getNewRoaringBitmapFromSmall() : r1.roaring_bitmap;
    ...
    if (isSmall()) {
      if (r1.isSmall()) {
        for (const auto &x : r1.small)
          if (small.find(x.getValue()) != small.end()) return 1;
      } else {
        for (const auto &x : small) {
          if (r1.roaring_bitmap->contains(static_cast<Value>(x.getValue()))) return 1;
        }
      }
    } else if (r1.isSmall()) {
      for (const auto &x : r1.small) {
        if (roaring_bitmap->contains(static_cast<Value>(x.getValue()))) return 1;
      }
    } else {
      if ((*roaring_bitmap & *r1.roaring_bitmap).cardinality() > 0) return 1;
    }
  ...
  UInt8 rb_is_subset(const RoaringBitmapWithSmallSet &r1) const  /// NOLINT
  {
    if (isSmall()) {
      if (r1.isSmall()) {
        for (const auto &x : r1.small)
          if (small.find(x.getValue()) == small.end()) return 0;
      } else {
        UInt64 r1_size = r1.size();

        // A bigger set can not be a subset of ours.
        if (r1_size > small.size()) return 0;
        ...
        for (const auto &x : small) {
          if (!r1.roaring_bitmap->contains(static_cast<Value>(x.getValue())) && ++r1_size > small.size()) return 0;
        }
      }
    } else if (r1.isSmall()) {
      for (const auto &x : r1.small) {
        if (!roaring_bitmap->contains(static_cast<Value>(x.getValue()))) return 0;
      }
    } else {
      if (!r1.roaring_bitmap->isSubset(*roaring_bitmap)) return 0;
    }
    return 1;
  }
  ...
  {
    if (!std::is_same_v<T, UInt64> && x > rb_max()) return 0;

    if (isSmall()) return small.find(static_cast<T>(x)) != small.end();
  ...
  {
    UInt64 count = 0;
    if (range_start >= range_end) return count;
    if (isSmall()) {
      for (const auto &x : small) {
        T val = x.getValue();
  ...
  UInt64 rb_limit(UInt64 range_start, UInt64 limit, RoaringBitmapWithSmallSet &r1) const  /// NOLINT
  {
    ...
    if (isSmall()) {
      ...
      for (const auto &elem : answer) r1.add(elem);
      return answer.size();
    }
  ...
  {
    if (limit == 0 || offset >= size()) return 0;

    if (isSmall()) {
      UInt64 count = 0;
      UInt64 offset_count = 0;
      auto it = small.begin();
      for (; it != small.end() && offset_count < offset; ++it) ++offset_count;

      for (; it != small.end() && count < limit; ++it, ++count) r1.add(it->getValue());
      return count;
    }
    ...
    for (; it != roaring_bitmap->end() && offset_count < offset; ++it) ++offset_count;

    for (; it != roaring_bitmap->end() && count < limit; ++it, ++count) r1.add(static_cast<T>(*it));
    return count;
  }
  ...
  UInt64 rb_min() const  /// NOLINT
  {
    if (isSmall()) {
      if (small.empty()) {
        if constexpr (sizeof(T) >= 8) return std::numeric_limits<UInt64>::max();
  ...
  void rb_replace(const UInt64 *from_vals, const UInt64 *to_vals, size_t num)  /// NOLINT
  {
    ...
    for (size_t i = 0; i < num; ++i) {
      if (from_vals[i] == to_vals[i]) continue;
      bool changed = roaring_bitmap->removeChecked(static_cast<Value>(from_vals[i]));
      if (changed) roaring_bitmap->add(static_cast<Value>(to_vals[i]));
    }
  }
  ...
  inline UInt16 container_to_uint32_array(const UInt16 &container_id, const UInt32 &base, PaddedPODArray<UInt32> &res) const  /// NOLINT
  {
    ...
    if (isSmall()) {
      ...
      for (const auto &x : small)
        if ((static_cast<UInt32>(x.getValue()) >> 16) == container_id) values.insert((static_cast<UInt32>(x.getValue()) & 0xFFFFu) + base);
      UInt16 num_added = 0;
      for (const auto &value : values) res[num_added++] = value;
      return num_added;
    } else {
      if (res.size() < 65536) throw Exception(ErrorCodes::BAD_ARGUMENTS, "PaddedPODArray<UInt32> res size must >= 65536");
  ...
  inline UInt16 ra_get_container_cardinality(const UInt16 &container_id) const  /// NOLINT
  {
    ...
    if (isSmall()) {
      int num_added = 0;
      for (const auto &x : small) {
        if (static_cast<UInt32>(x.getValue()) >> 16 == container_id) num_added++;
      }
  ...
  inline SetWithMemoryTracking<UInt16> ra_get_all_container_ids()  /// NOLINT
  {
    ...
    if (isSmall()) {
      for (const auto &x : small) container_ids.insert(static_cast<UInt32>(x.getValue()) >> 16);
    } else {
      auto *rb32 = reinterpret_cast<roaring::Roaring *>(roaring_bitmap.get());
      auto *ra = &rb32->roaring.high_low_container;
   ...
   */
  inline roaring::internal::container_t *container_and(const RoaringBitmapWithSmallSet<T, small_set_size> *rhs, const UInt16 &container_id,
                                                       uint8_t *result_type) const  /// NOLINT
  {
    if (isSmall() || rhs->isSmall()) throw Exception(ErrorCodes::LOGICAL_ERROR, "Unsupported SmallSet");
    uint8_t type1;
    uint8_t type2;
  ...
  inline UInt16 container_and_to_uint32_array(  /// NOLINT
    ...
    if (sizeof(T) >= 8) throw Exception(ErrorCodes::LOGICAL_ERROR, "Unsupported Roaring64Map");
    if (output->size() < 65536) throw Exception(ErrorCodes::BAD_ARGUMENTS, "PaddedPODArray<UInt32> * output size must >= 65536");
    ...
    if (lhs_small && rhs_small) {
      ...
      for (const auto &lhs_value : small) lhs_values.insert(lhs_value.getValue());
      UInt32 num_added = 0;
