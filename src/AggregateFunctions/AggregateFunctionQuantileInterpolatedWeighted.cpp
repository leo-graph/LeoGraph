...
struct QuantileInterpolatedWeighted {
 ...
 private:
  ...
  T getImpl(Float64 level) const {
    ...
    if (level < lower_percentile) upper_value = lower_value;
    if (level > upper_percentile) lower_value = upper_value;
  ...
  UnderlyingType NO_SANITIZE_UNDEFINED interpolate(Float64 level, Float64 lower_percentile, Float64 upper_percentile,
                                                   UnderlyingType lower_value, UnderlyingType upper_value) const {
    UnderlyingType value_diff = upper_value - lower_value;
    Float64 percentile_diff = upper_percentile - lower_percentile;
    percentile_diff =
        percentile_diff == 0 ? 1 : percentile_diff;  /// to handle NaN behavior that might arise during integer division below.

    /// yl + (dy / dx) * (level - xl)
    return static_cast<UnderlyingType>(static_cast<Float64>(lower_value) +
                                       (static_cast<Float64>(value_diff) / percentile_diff) * (level - lower_percentile));
  }
};

template <typename Value, bool _>
using FuncQuantileInterpolatedWeighted =
    AggregateFunctionQuantile<Value, QuantileInterpolatedWeighted<Value>, NameQuantileInterpolatedWeighted, UInt64, void, false, false>;
template <typename Value, bool _>
using FuncQuantilesInterpolatedWeighted =
    AggregateFunctionQuantile<Value, QuantileInterpolatedWeighted<Value>, NameQuantilesInterpolatedWeighted, UInt64, void, true, false>;
...
quantileInterpolatedWeighted(level)(expr, weight)
  ...
  FunctionDocumentation::Examples examples = {{"Computing interpolated weighted quantile",
                                               R"(
CREATE TABLE t (
    n Int32,
    val Int32
...
└──────────────────────────────────────┘
  ...
  factory.registerFunction(NameQuantileInterpolatedWeighted::name,
                           {createAggregateFunctionQuantile<FuncQuantileInterpolatedWeighted>, documentation});
  factory.registerFunction(NameQuantilesInterpolatedWeighted::name,
                           {createAggregateFunctionQuantile<FuncQuantilesInterpolatedWeighted>, {}, properties});
