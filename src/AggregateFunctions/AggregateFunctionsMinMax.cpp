  ...
  explicit AggregateFunctionMinMax(const DataTypes &argument_types_)
    ...
    if (!this->result_type->isComparable())
      throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT,
                      ...
                      this->result_type->getName(), getName());
...
template <bool isMin>
AggregateFunctionPtr createAggregateFunctionMinMax(const std::string &name, const DataTypes &argument_types, const Array &parameters,
...
min(column)
  ...
  FunctionDocumentation::Arguments min_arguments = {{"column", "Column name or expression.", {"Any"}}};
  FunctionDocumentation::ReturnedValue min_returned_value = {
      "Returns the minimum value across the group with type equal to that of the input.", {"Any"}};
  FunctionDocumentation::Examples min_examples = {{"Simple min example",
                                                   R"(
...
└─────────────┴──────────────┘
  ...
  FunctionDocumentation min_documentation = {min_description,    min_syntax,   min_arguments,     {},
                                             min_returned_value, min_examples, min_introduced_in, min_category};
...
max(column)
  ...
  FunctionDocumentation::Arguments max_arguments = {{"column", "Column name or expression.", {"Any"}}};
  FunctionDocumentation::ReturnedValue max_returned_value = {"The maximum value across the group with type equal to that of the input.",
                                                             {"Any"}};
  FunctionDocumentation::Examples max_examples = {{"Simple max example",
                                                   R"(
CREATE TABLE employees (name String, salary UInt32) ENGINE = Memory;
INSERT INTO employees VALUES ('Alice', 3000), ('Bob', 4000), ('Charlie', 3500);
...
SELECT greatest(a, b) FROM table;
  ...
  FunctionDocumentation max_documentation = {max_description,    max_syntax,   max_arguments,     {},
                                             max_returned_value, max_examples, max_introduced_in, max_category};
