...
struct RankCorrelationData : public StatisticalSample<Float64, Float64> {
  Float64 getResult() {
    ...
    /// Count d^2 sum
    Float64 answer = 0;
    for (size_t j = 0; j < static_cast<size_t>(size); ++j) answer += (ranks_x[j] - ranks_y[j]) * (ranks_x[j] - ranks_y[j]);
...
class AggregateFunctionRankCorrelation final : public IAggregateFunctionDataHelper<RankCorrelationData, AggregateFunctionRankCorrelation> {
 public:
  ...
  String getName() const override { return "rankCorr"; }
...
AggregateFunctionPtr createAggregateFunctionRankCorrelation(const std::string &name, const DataTypes &argument_types,
...
rankCorr(x, y)
  ...
  FunctionDocumentation::Arguments arguments_rankCorr = {{"x", "Arbitrary value.", {"Float*"}}, {"y", "Arbitrary value.", {"Float*"}}};
  FunctionDocumentation::ReturnedValue returned_value_rankCorr = {
      "Returns a rank correlation coefficient of the ranks of x and y. The value ranges from -1 to +1.", {"Float64"}};
  FunctionDocumentation::Examples examples_rankCorr = {{"Perfect correlation",
                                                        R"(
...
└─────────────────────────────────────────────────────┘
  ...
  FunctionDocumentation documentation_rankCorr = {description_rankCorr,    syntax_rankCorr,   arguments_rankCorr,     parameters_rankCorr,
                                                  returned_value_rankCorr, examples_rankCorr, introduced_in_rankCorr, category_rankCorr};
