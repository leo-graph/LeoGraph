...
struct QuantilePrometheusHistogram {
  ...
  void add(const Value &x, CumulativeHistogramValue cumulative_histogram_value) {
    if (!isNaN(x)) map[x] += cumulative_histogram_value;
  }

  void merge(const QuantilePrometheusHistogram &rhs) {
    for (const auto &pair : rhs.map) map[pair.getKey()] += pair.getMapped();
  }
  ...
  void getMany(const Float64 *levels, const size_t *indices, size_t num_levels, Value *result) const {
    size_t size = map.size();
    if (0 == size) {
      for (size_t i = 0; i < num_levels; ++i) result[i] = Value();
      return;
    }
 ...
 private:
  Value getInterpolatedImpl(Float64 level) const {
    ...
    if (size < 2) return std::numeric_limits<Value>::quiet_NaN();
    ...
    if (max_bucket.first != std::numeric_limits<UnderlyingType>::infinity()) return std::numeric_limits<Value>::quiet_NaN();
  ...
  void getManyInterpolatedImpl(const Float64 *levels, const size_t *indices, size_t num_levels, Value *result) const {
    size_t size = map.size();
    if (size < 2) {
      for (size_t i = 0; i < num_levels; ++i) result[i] = std::numeric_limits<Value>::quiet_NaN();
      return;
    }
  ...
  Value quantileInterpolated(const Pair *array, size_t size, Float64 position) const {
    const auto *upper_bound_it =
        std::lower_bound(array, array + size, position, [](const Pair &a, Float64 b) { return static_cast<Float64>(a.second) < b; });
    if (upper_bound_it == array) {
      if (upper_bound_it->first > 0) {
        // If position is in the first bucket and the first bucket's upper bounds is positive, perform interpolation as if the first
        // bucket's lower bounds is 0.
      ...
      // If the position is in the +Inf bucket, return the largest finite bucket's upper bound, which is the second to last bucket's upper
      // bound.
    ...
    return static_cast<Value>(histogram_bucket_lower_bound +
                              (histogram_bucket_upper_bound - histogram_bucket_lower_bound) *
                                  (position - static_cast<Float64>(histogram_bucket_lower_value)) /
                                  static_cast<Float64>(histogram_bucket_upper_value - histogram_bucket_lower_value));
  }
};

template <typename Value, typename CumulativeHistogramValue>
using FuncQuantilePrometheusHistogram =
    AggregateFunctionQuantile<Value, QuantilePrometheusHistogram<Value, CumulativeHistogramValue>, NameQuantilePrometheusHistogram,
                              CumulativeHistogramValue, void, false, false>;
template <typename Value, typename CumulativeHistogramValue>
using FuncQuantilesPrometheusHistogram =
    AggregateFunctionQuantile<Value, QuantilePrometheusHistogram<Value, CumulativeHistogramValue>, NameQuantilesPrometheusHistogram,
                              CumulativeHistogramValue, void, true, false>;
...
quantilePrometheusHistogram(level)(bucket_upper_bound, cumulative_bucket_value)
  ...
  FunctionDocumentation::Parameters parameters_quantilePrometheusHistogram = {
      {"level",
       "Optional. Level of quantile. Constant floating-point number from 0 to 1. We recommend using a `level` value in the range of "
       "`[0.01, 0.99]`. Default value: `0.5`. At `level=0.5` the function calculates [median](https://en.wikipedia.org/wiki/Median).",
       {"Float64"}}};
  FunctionDocumentation::Arguments arguments_quantilePrometheusHistogram = {
      ...
      {"cumulative_bucket_value",
       "Cumulative values of the histogram buckets. Values must be monotonically increasing as the bucket upper bound increases.",
       {"(U)Int*", "Float64"}}};
  FunctionDocumentation::ReturnedValue returned_value_quantilePrometheusHistogram = {"Returns the quantile of the specified level.",
                                                                                     {"Float64"}};
  FunctionDocumentation::Examples examples_quantilePrometheusHistogram = {{"Usage example",
                                                                           R"(
...
└──────────────────────────────────────────────────────────────────────────┘
  ...
  FunctionDocumentation documentation_quantilePrometheusHistogram = {
      description_quantilePrometheusHistogram,   syntax_quantilePrometheusHistogram,         arguments_quantilePrometheusHistogram,
      parameters_quantilePrometheusHistogram,    returned_value_quantilePrometheusHistogram, examples_quantilePrometheusHistogram,
      introduced_in_quantilePrometheusHistogram, category_quantilePrometheusHistogram};

  factory.registerFunction(NameQuantilePrometheusHistogram::name,
                           {createAggregateFunctionQuantile<FuncQuantilePrometheusHistogram>, documentation_quantilePrometheusHistogram});
  factory.registerFunction(NameQuantilesPrometheusHistogram::name, {createAggregateFunctionQuantile<FuncQuantilesPrometheusHistogram>,
                                                                    documentation_quantilePrometheusHistogram, properties});
}
