...
std::unique_ptr<UInt8[]> mergeIfAndNullFlags(const UInt8 *__restrict null_map, const UInt8 *__restrict if_flags, size_t row_begin,
                                             size_t row_end) {
  auto final_flags = std::make_unique<UInt8[]>(row_end);
  for (size_t i = row_begin; i < row_end; ++i) final_flags[i] = (!null_map[i]) & !!if_flags[i];
  return final_flags;
}
...
std::optional<size_t> SingleValueDataBase::getSmallestIndex(const IColumn &column, size_t row_begin, size_t row_end) const {
  ...
  if (row_begin >= row_end) return std::nullopt;
  ...
  if (row_begin != 0 || row_end != column.size()) {
    ...
    for (size_t i = index + 1; i < row_end; i++)
      if ((column.compareAt(i, index, column, nan_null_direction_hint) < 0)) index = i;
    return {index};
  }
...
std::optional<size_t> SingleValueDataBase::getSmallestIndexNotNullIf(const IColumn &column, const UInt8 *__restrict null_map,
                                                                     const UInt8 *__restrict if_map, size_t row_begin,
                                                                     size_t row_end) const {
  static constexpr int nan_null_direction_hint = 1;

  size_t index = row_begin;
  while ((index < row_end) && ((if_map && if_map[index] == 0) || (null_map && null_map[index] != 0))) index++;
  if (index >= row_end) return std::nullopt;
...
std::optional<size_t> SingleValueDataBase::getGreatestIndexNotNullIf(const IColumn &column, const UInt8 *__restrict null_map,
                                                                     const UInt8 *__restrict if_map, size_t row_begin,
                                                                     size_t row_end) const {
  static constexpr int nan_null_direction_hint = -1;
...
void SingleValueDataBase::setSmallest(const IColumn &column, size_t row_begin, size_t row_end, Arena *arena) {
  ...
  if (index) setIfSmaller(column, *index, arena);
}
...
void SingleValueDataBase::setGreatest(const IColumn &column, size_t row_begin, size_t row_end, Arena *arena) {
  ...
  if (index) setIfGreater(column, *index, arena);
}

void SingleValueDataBase::setSmallestNotNullIf(const IColumn &column, const UInt8 *__restrict null_map, const UInt8 *__restrict if_map,
                                               size_t row_begin, size_t row_end, Arena *arena) {
  auto index = getSmallestIndexNotNullIf(column, null_map, if_map, row_begin, row_end);
  if (index) setIfSmaller(column, *index, arena);
}

void SingleValueDataBase::setGreatestNotNullIf(const IColumn &column, const UInt8 *__restrict null_map, const UInt8 *__restrict if_map,
                                               size_t row_begin, size_t row_end, Arena *arena) {
...
void SingleValueDataFixed<T>::setSmallest(const IColumn &column, size_t row_begin, size_t row_end, Arena *arena) {
  ...
  if constexpr (has_find_extreme_implementation<T> || underlying_has_find_extreme_implementation<T>) {
    ...
    if (opt.has_value()) setIfSmaller(*opt);
  } else {
    for (size_t i = row_begin; i < row_end; i++) setIfSmaller(column, i, arena);
  }
}
...
void SingleValueDataFixed<T>::setGreatest(const IColumn &column, size_t row_begin, size_t row_end, Arena *arena) {
  ...
  if constexpr (has_find_extreme_implementation<T> || underlying_has_find_extreme_implementation<T>) {
    ...
    if (opt.has_value()) setIfGreater(*opt);
  } else {
    for (size_t i = row_begin; i < row_end; i++) setIfGreater(column, i, arena);
  }
}

template <typename T>
void SingleValueDataFixed<T>::setSmallestNotNullIf(const IColumn &column, const UInt8 *__restrict null_map, const UInt8 *__restrict if_map,
                                                   size_t row_begin, size_t row_end, Arena *arena) {
  ...
  if constexpr (has_find_extreme_implementation<T> || underlying_has_find_extreme_implementation<T>) {
    ...
    size_t index = row_begin;
    while ((index < row_end) && ((if_map && if_map[index] == 0) || (null_map && null_map[index] != 0))) index++;
    if (index >= row_end) return;

    setIfSmaller(column, index, arena);

    for (size_t i = index + 1; i < row_end; i++)
      if ((!if_map || if_map[i] != 0) && (!null_map || null_map[i] == 0)) setIfSmaller(column, i, arena);
  }
}

template <typename T>
void SingleValueDataFixed<T>::setGreatestNotNullIf(const IColumn &column, const UInt8 *__restrict null_map, const UInt8 *__restrict if_map,
                                                   size_t row_begin, size_t row_end, Arena *arena) {
  chassert(if_map || null_map);
  ...
  if constexpr (has_find_extreme_implementation<T> || underlying_has_find_extreme_implementation<T>) {
    ...
    for (size_t i = index + 1; i < row_end; i++)
      if ((!if_map || if_map[i] != 0) && (!null_map || null_map[i] == 0)) setIfGreater(column, i, arena);
  }
}
...
std::optional<size_t> SingleValueDataFixed<T>::getSmallestIndex(const IColumn &column, size_t row_begin, size_t row_end) const {
  ...
  if constexpr (has_find_extreme_implementation<T> || underlying_has_find_extreme_implementation<T>) {
    ...
    size_t index = row_begin;
    for (size_t i = index + 1; i < row_end; i++)
      if (vec.getData()[i] < vec.getData()[index]) index = i;
    return index;
  }
}
...
template <typename T>
std::optional<size_t> SingleValueDataFixed<T>::getSmallestIndexNotNullIf(const IColumn &column, const UInt8 *__restrict null_map,
                                                                         const UInt8 *__restrict if_map, size_t row_begin,
                                                                         size_t row_end) const {
  if (row_begin >= row_end) return std::nullopt;
  ...
  if constexpr (has_find_extreme_implementation<T> || underlying_has_find_extreme_implementation<T>) {
    ...
    if (!if_map) {
      ...
      for (size_t i = row_begin; i < row_end; i++) {
        if constexpr (is_floating_point<T>) {
          ...
          if (!null_map[i] && vec_data[i] == smallest) return {i};
        }
      }
    } else if (!null_map) {
      opt = findExtremeMinIf(vec.getData().data(), if_map, row_begin, row_end);
      ...
      for (size_t i = row_begin; i < row_end; i++) {
        if constexpr (is_floating_point<T>) {
          ...
          if (if_map[i] && vec_data[i] == smallest) return {i};
        }
      }
    } else {
      auto final_flags = mergeIfAndNullFlags(null_map, if_map, row_begin, row_end);
      ...
      for (size_t i = row_begin; i < row_end; i++) {
        if constexpr (is_floating_point<T>) {
          static_assert(std::is_trivial_v<T> && std::is_standard_layout_v<T>);
          if (final_flags[i] &&
              std::memcmp(&vec_data[i], &smallest, sizeof(T)) == 0)  // NOLINT (we are comparing FP with memcmp on purpose)
            return {i};
        } else {
          if (final_flags[i] && vec_data[i] == smallest) return {i};
        }
      }
    }
    ...
    for (size_t i = index + 1; i < row_end; i++)
      if ((!if_map || if_map[i] != 0) && (!null_map || null_map[i] == 0) && (vec_data[i] < vec_data[index])) index = i;
    return {index};
  }
}

template <typename T>
std::optional<size_t> SingleValueDataFixed<T>::getGreatestIndexNotNullIf(const IColumn &column, const UInt8 *__restrict null_map,
                                                                         const UInt8 *__restrict if_map, size_t row_begin,
                                                                         size_t row_end) const {
  if (row_begin >= row_end) return std::nullopt;
    ...
    if (!if_map) {
      ...
      for (size_t i = row_begin; i < row_end; i++) {
        if constexpr (is_floating_point<T>) {
          ...
          if (!null_map[i] && vec_data[i] == greatest) return {i};
        }
      }
    } else if (!null_map) {
      opt = findExtremeMaxIf(vec.getData().data(), if_map, row_begin, row_end);
      ...
      for (size_t i = row_begin; i < row_end; i++) {
        if constexpr (is_floating_point<T>) {
          ...
          if (if_map[i] && vec_data[i] == greatest) return {i};
        }
      }
    } else {
      auto final_flags = mergeIfAndNullFlags(null_map, if_map, row_begin, row_end);
      ...
      for (size_t i = row_begin; i < row_end; i++) {
        if constexpr (is_floating_point<T>) {
          static_assert(std::is_trivial_v<T> && std::is_standard_layout_v<T>);
          if (final_flags[i] &&
              std::memcmp(&vec_data[i], &greatest, sizeof(T)) == 0)  // NOLINT (we are comparing FP with memcmp on purpose)
            return {i};
        } else {
          if (final_flags[i] && vec_data[i] == greatest) return {i};
        }
      }
    }
    throw Exception(ErrorCodes::LOGICAL_ERROR, "Failed to find index");
...
void SingleValueDataFixed<T>::compileCreate(llvm::IRBuilderBase &builder, llvm::Value *aggregate_data_ptr) {
  ...
  /// to skip a check
  b.CreateMemSet(b.CreateConstInBoundsGEP1_64(b.getInt8Ty(), aggregate_data_ptr, 0), llvm::ConstantInt::get(b.getInt8Ty(), 0),
                 sizeof(SingleValueDataFixed<T>), llvm::assumeAligned(alignof(SingleValueDataFixed<T>)));
}
...
template <typename T>
void SingleValueDataFixed<T>::compileSetValueFromNumber(llvm::IRBuilderBase &builder, llvm::Value *aggregate_data_ptr,
                                                        llvm::Value *value_to_check) {
  llvm::IRBuilder<> &b = static_cast<llvm::IRBuilder<> &>(builder);
...
template <typename T>
void SingleValueDataFixed<T>::compileSetValueFromAggregation(llvm::IRBuilderBase &builder, llvm::Value *aggregate_data_ptr,
                                                             llvm::Value *aggregate_data_src_ptr) {
...
template <typename T>
void SingleValueDataFixed<T>::compileAnyMerge(llvm::IRBuilderBase &builder, llvm::Value *aggregate_data_dst_ptr,
                                              llvm::Value *aggregate_data_src_ptr) {
  llvm::IRBuilder<> &b = static_cast<llvm::IRBuilder<> &>(builder);
...
template <typename T>
void SingleValueDataFixed<T>::compileAnyLastMerge(llvm::IRBuilderBase &builder, llvm::Value *aggregate_data_dst_ptr,
                                                  llvm::Value *aggregate_data_src_ptr) {
  llvm::IRBuilder<> &b = static_cast<llvm::IRBuilder<> &>(builder);
...
template <typename T>
template <bool isMin>
void SingleValueDataFixed<T>::compileMinMaxMerge(llvm::IRBuilderBase &builder, llvm::Value *aggregate_data_dst_ptr,
                                                 llvm::Value *aggregate_data_src_ptr) {
  llvm::IRBuilder<> &b = static_cast<llvm::IRBuilder<> &>(builder);
  ...
  b.CreateCondBr(b.CreateAnd(has_value_src, b.CreateOr(b.CreateNot(has_value_dst), should_change_after_comparison)), if_should_change,
                 join_block);
...
template <typename T>
void SingleValueDataFixed<T>::compileMinMerge(llvm::IRBuilderBase &builder, llvm::Value *aggregate_data_dst_ptr,
                                              llvm::Value *aggregate_data_src_ptr) {
...
template <typename T>
void SingleValueDataFixed<T>::compileMaxMerge(llvm::IRBuilderBase &builder, llvm::Value *aggregate_data_dst_ptr,
                                              llvm::Value *aggregate_data_src_ptr) {
...
template <typename T>
void SingleValueDataNumeric<T>::setSmallestNotNullIf(const IColumn &column, const UInt8 *__restrict null_map,
                                                     const UInt8 *__restrict if_map, size_t row_begin, size_t row_end, Arena *arena) {
  return memory.get().setSmallestNotNullIf(column, null_map, if_map, row_begin, row_end, arena);
}

template <typename T>
void SingleValueDataNumeric<T>::setGreatestNotNullIf(const IColumn &column, const UInt8 *__restrict null_map,
                                                     const UInt8 *__restrict if_map, size_t row_begin, size_t row_end, Arena *arena) {
...
template <typename T>
std::optional<size_t> SingleValueDataNumeric<T>::getSmallestIndexNotNullIf(const IColumn &column, const UInt8 *__restrict null_map,
                                                                           const UInt8 *__restrict if_map, size_t row_begin,
                                                                           size_t row_end) const {
...
template <typename T>
std::optional<size_t> SingleValueDataNumeric<T>::getGreatestIndexNotNullIf(const IColumn &column, const UInt8 *__restrict null_map,
                                                                           const UInt8 *__restrict if_map, size_t row_begin,
                                                                           size_t row_end) const {
...
char *SingleValueDataString::getDataMutable() { return isSmall() ? small_data : large_data; }

const char *SingleValueDataString::getData() const { return isSmall() ? small_data : large_data; }

std::string_view SingleValueDataString::getStringView() const { return std::string_view{getData(), size - 1}; }
...
void SingleValueDataString::allocateLargeDataIfNeeded(UInt32 size_to_reserve, Arena *arena) {
  if (capacity < size_to_reserve) {
    if (unlikely(MAX_STRING_SIZE < size_to_reserve))
      throw Exception(ErrorCodes::TOO_LARGE_STRING_SIZE, "String size is too big ({}), maximum: {}", size_to_reserve, MAX_STRING_SIZE);
...
void SingleValueDataString::changeImpl(std::string_view value, Arena *arena) {
  ...
  if (value_size <= MAX_SMALL_STRING_SIZE && isSmall()) {
    ...
    if (value_size > 0) memcpy(small_data, value.data(), value.size());
  } else {
...
void SingleValueDataString::write(WriteBuffer &buf, const ISerialization & /*serialization*/) const {
  if (unlikely(MAX_STRING_SIZE < size)) throw Exception(ErrorCodes::LOGICAL_ERROR, "String size is too big ({}), it's a bug", size);
...
void SingleValueDataString::set(const SingleValueDataBase &other, Arena *arena) {
  auto const &to = assert_cast<const Self &>(other);
  if (to.has()) changeImpl(to.getStringView(), arena);
}
...
void SingleValueDataGeneric::read(ReadBuffer &buf, const ISerialization &serialization, const DataTypePtr &, Arena *) {
  ...
  if (is_not_null) serialization.deserializeBinary(value, buf, {});
}

bool SingleValueDataGeneric::isEqualTo(const IColumn &column, size_t row_num) const { return has() && value == column[row_num]; }
...
void SingleValueDataGeneric::set(const IColumn &column, size_t row_num, Arena *) { column.get(row_num, value); }

void SingleValueDataGeneric::set(const SingleValueDataBase &other, Arena *) {
  auto const &to = assert_cast<const Self &>(other);
  if (other.has()) value = to.value;
}
...
void SingleValueReference::read(ReadBuffer & /*buf*/, const ISerialization & /*serialization*/, const DataTypePtr & /*type*/,
                                Arena * /*arena*/) {
  /// Not support
...
bool SingleValueReference::isEqualTo(const DB::IColumn &column, size_t row_num) const {
  return has() && column_ref->compareAt(row_number, row_num, column, -1) == 0;
}
...
bool SingleValueReference::setIfSmaller(const IColumn &column, size_t row_num, Arena *arena) {
  if (!has() || column_ref->compareAt(row_number, row_num, column, -1) > 0) {
    set(column, row_num, arena);
    return true;
  }
  return false;
...
bool singleValueTypeAllocatesMemoryInArena(TypeIndex idx) { return idx == TypeIndex::String; }
}  // namespace DB
