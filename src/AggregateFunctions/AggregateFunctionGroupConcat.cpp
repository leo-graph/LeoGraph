...
UInt64 GroupConcatData::getSize(size_t i) const { return offsets[i * 2 + 1] - offsets[i * 2]; }

UInt64 GroupConcatData::getString(size_t i) const { return offsets[i * 2]; }
...
GroupConcatImpl<has_limit>::GroupConcatImpl(const DataTypePtr &data_type_, const Array &parameters_, UInt64 limit_,
                                            const String &delimiter_)
    : IAggregateFunctionDataHelper<GroupConcatData, GroupConcatImpl<has_limit>>({data_type_}, parameters_,
                                                                                std::make_shared<DataTypeString>()),
      limit(limit_),
      delimiter(delimiter_),
      type(data_type_) {
  serialization = isFixedString(type) ? std::make_shared<DataTypeString>()->getDefaultSerialization()
                                      : this->argument_types[0]->getDefaultSerialization();
}
...
template <bool has_limit>
void GroupConcatImpl<has_limit>::add(AggregateDataPtr __restrict place, const IColumn **columns, size_t row_num, Arena *arena) const {
  auto &cur_data = this->data(place);

  if constexpr (has_limit)
    if (cur_data.num_rows >= limit) return;

  if (cur_data.data_size != 0) cur_data.insertChar(delimiter.c_str(), delimiter.size(), arena);
...
void GroupConcatImpl<has_limit>::merge(AggregateDataPtr __restrict place, ConstAggregateDataPtr rhs, Arena *arena) const {
  ...
  if (rhs_data.data_size == 0) return;
...
template <bool has_limit>
void GroupConcatImpl<has_limit>::serialize(ConstAggregateDataPtr __restrict place, WriteBuffer &buf,
  ...
  if constexpr (has_limit) {
    ...
    for (const auto &offset : cur_data.offsets) writeVarUInt(offset, buf);
  }
}

template <bool has_limit>
void GroupConcatImpl<has_limit>::deserialize(AggregateDataPtr __restrict place, ReadBuffer &buf, std::optional<size_t> /* version */,
                                             Arena *arena) const {
  auto &cur_data = this->data(place);

  /// IAggregateFunction::deserialize() must be called only for an empty (just created) state.
  if (cur_data.data_size != 0)
    throw Exception(ErrorCodes::LOGICAL_ERROR, "groupConcat deserialize() expects an empty state (data_size = 0), got data_size = {}",
                    cur_data.data_size);
  ...
  if constexpr (has_limit) {
    ...
    if (cur_data.num_rows > std::numeric_limits<UInt64>::max() / 2)
      throw Exception(ErrorCodes::BAD_ARGUMENTS,
                      "Invalid groupConcat state: num_rows ({}) is too large and would overflow the offsets array.", cur_data.num_rows);
    ...
    for (size_t i = 0; i < cur_data.offsets.size(); ++i) {
      ...
      if (cur_data.offsets[i] > cur_data.data_size)
        throw Exception(ErrorCodes::BAD_ARGUMENTS, "Invalid offset {} in groupConcat state: exceeds data size {}", cur_data.offsets[i],
                        cur_data.data_size);

      if (i != 0 && cur_data.offsets[i] < cur_data.offsets[i - 1])
        throw Exception(ErrorCodes::BAD_ARGUMENTS, "Invalid offsets in groupConcat state: end offset {} is less than start offset {}",
                        cur_data.offsets[i], cur_data.offsets[i - 1]);
    }

    if (cur_data.num_rows != 0 && cur_data.offsets.back() != cur_data.data_size)
      throw Exception(ErrorCodes::BAD_ARGUMENTS, "Invalid offsets in groupConcat state: last offset {} is not equal to data size {}",
                      cur_data.offsets.back(), cur_data.data_size);
  }
}
...
template <bool has_limit>
bool GroupConcatImpl<has_limit>::allocatesMemoryInArena() const {
  return true;
}

// Implementation of add, merge, serialize, deserialize, insertResultInto, etc. remains unchanged.

AggregateFunctionPtr createAggregateFunctionGroupConcat(const std::string &name, const DataTypes &argument_types, const Array &parameters,
                                                        const Settings *) {
  ...
  if (parameters.size() == 2) {
    ...
    if ((type == Field::Types::Int64 && parameters[1].safeGet<Int64>() <= 0) ||
      ...
      throw Exception(ErrorCodes::BAD_ARGUMENTS, "Second parameter for aggregate function {} should be a positive number, got: {}", name,
                      parameters[1].safeGet<Int64>());

    has_limit = true;
    limit = parameters[1].safeGet<UInt64>();
  }

  if (has_limit) return std::make_shared<GroupConcatImpl</* has_limit= */ true>>(argument_types[0], parameters, limit, delimiter);
...
groupConcat[(delimiter [, limit])](expression)
  ...
  FunctionDocumentation::Parameters parameters_groupConcat = {
      {"delimiter",
       "A string that will be used to separate concatenated values. This parameter is optional and defaults to an empty string if not "
       "specified.",
       {"String"}},
      {"limit",
       "A positive integer specifying the maximum number of elements to concatenate. If more elements are present, excess elements are "
  ...
  FunctionDocumentation::Arguments arguments_groupConcat = {
      ...
      {"delimiter",
       "A string that will be used to separate concatenated values. This parameter is optional and defaults to an empty string or "
       "delimiter from parameters if not specified.",
       {"String"}}};
  FunctionDocumentation::ReturnedValue returned_value_groupConcat = {
      "Returns a string consisting of the concatenated values of the column or expression. If the group has no elements or only null "
      "elements, and the function does not specify a handling for only null values, the result is a nullable string with a null value.",
      {"String"}};
  FunctionDocumentation::Examples examples_groupConcat = {{"Basic usage without a delimiter",
                                                           R"(
...
John, Jane
  ...
  FunctionDocumentation documentation_groupConcat = {description_groupConcat,   syntax_groupConcat,         arguments_groupConcat,
                                                     parameters_groupConcat,    returned_value_groupConcat, examples_groupConcat,
                                                     introduced_in_groupConcat, category_groupConcat};

  factory.registerFunction("groupConcat", {createAggregateFunctionGroupConcat, documentation_groupConcat, properties});
  factory.registerAlias(GroupConcatImpl<false>::getNameAndAliases().at(1), GroupConcatImpl<false>::getNameAndAliases().at(0),
