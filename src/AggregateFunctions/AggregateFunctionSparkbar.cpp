 ...
 private:
  ...
  void render(ColumnString &to_column, const AggregateFunctionSparkbarData<X, Y> &data) const {
    ...
    for (const auto &point : data.points) {
      if (point.getKey() < from_x || to_x < point.getKey()) continue;

      X delta = to_x - from_x;
      if (delta < std::numeric_limits<X>::max()) delta = delta + 1;

      X value = point.getKey() - from_x;
      Float64 w = static_cast<Float64>(histogram.size());
      size_t index =
          std::min<size_t>(static_cast<size_t>(w / static_cast<Float64>(delta) * static_cast<Float64>(value)), histogram.size() - 1);
...
#pragma clang loop vectorize(disable)  /// Workaround for a bug in clang-23
    ...
    size_t sz = 0;
    for (const auto &y : histogram) sz += updateFrame(values, y);
 ...
 public:
  AggregateFunctionSparkbar(const DataTypes &arguments, const Array &params)
    ...
    if (width < 2 || 1024 < width) throw Exception(ErrorCodes::BAD_ARGUMENTS, "Parameter width must be in range [2, 1024]");

    if (begin_x >= end_x) throw Exception(ErrorCodes::BAD_ARGUMENTS, "Parameter `min_x` must be less than `max_x`");
  }

  String getName() const override { return "sparkbar"; }
...
IAggregateFunction *createWithUIntegerOrTimeType(const std::string &name, const IDataType &argument_type, TArgs &&...args) {
  ...
  if (which.idx == TypeIndex::Date || which.idx == TypeIndex::UInt16)
    return new AggregateFunctionTemplate<UInt16, Data>(std::forward<TArgs>(args)...);
  if (which.idx == TypeIndex::DateTime || which.idx == TypeIndex::UInt32)
...
AggregateFunctionPtr createAggregateFunctionSparkbarImpl(const std::string &name, const IDataType &x_argument_type,
                                                         const IDataType &y_argument_type, TArgs... args) {
  WhichDataType which(y_argument_type);
#define DISPATCH(TYPE)              \
  if (which.idx == TypeIndex::TYPE) \
    return AggregateFunctionPtr(    \
        createWithUIntegerOrTimeType<AggregateFunctionSparkbar, TYPE>(name, x_argument_type, std::forward<TArgs>(args)...));
...
AggregateFunctionPtr createAggregateFunctionSparkbar(const std::string &name, const DataTypes &arguments, const Array &params,
                                                     const Settings *) {
...
sparkbar(buckets[, min_x, max_x])(x, y)
  ...
  FunctionDocumentation::Parameters parameters_sparkbar = {{"buckets", "The number of segments.", {"(U)Int*"}},
                                                           {"min_x", "Optional. The interval start.", {"(U)Int*", "Float*", "Decimal"}},
                                                           {"max_x", "Optional. The interval end.", {"(U)Int*", "Float*", "Decimal"}}};
  FunctionDocumentation::Arguments arguments_sparkbar = {{"x", "The field with values.", {"const String"}},
  ...
  FunctionDocumentation::Examples examples_sparkbar = {{"Without interval specification",
                                                        R"(
...
└──────────────────────────────────────────────────────────────────────────┘
  ...
  FunctionDocumentation documentation_sparkbar = {description_sparkbar,    syntax_sparkbar,   arguments_sparkbar,     parameters_sparkbar,
                                                  returned_value_sparkbar, examples_sparkbar, introduced_in_sparkbar, category_sparkbar};
