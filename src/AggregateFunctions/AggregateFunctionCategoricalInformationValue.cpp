...
class AggregateFunctionCategoricalIV final : public IAggregateFunctionHelper<AggregateFunctionCategoricalIV> {
 ...
 public:
  ...
  String getName() const override { return "categoricalInformationValue"; }
  ...
  size_t sizeOfData() const override { return sizeof(Counter) * (category_count + 1) * 2; }

  size_t alignOfData() const override { return alignof(Counter); }
  ...
  static DataTypePtr createResultType() { return std::make_shared<DataTypeArray>(std::make_shared<DataTypeNumber<Float64>>()); }
...
AggregateFunctionPtr createAggregateFunctionCategoricalIV(const std::string &name, const DataTypes &arguments, const Array &params,
                                                          const Settings *) {
  assertNoParameters(name, params);

  if (arguments.size() < 2)
    throw Exception(ErrorCodes::NUMBER_OF_ARGUMENTS_DOESNT_MATCH, "Aggregate function {} requires two or more arguments", name);

  for (const auto &argument : arguments) {
    if (!WhichDataType(argument).isUInt8())
      throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT, "All the arguments of aggregate function {} should be UInt8", name);
  }
...
The result indicates how much each discrete (categorical) feature `[category1, category2, ...]` contributes to a learning model which predicts the value of `tag`.
  ...
  FunctionDocumentation::Arguments arguments = {
      {"category1, category2, ...",
       "One or more categorical features to analyze. Each category should contain discrete values.",
       {"UInt8"}},
      {"tag", "Binary target variable for prediction. Should contain values 0 and 1.", {"UInt8"}}};
  FunctionDocumentation::Parameters parameters = {};
  FunctionDocumentation::ReturnedValue returned_value = {
      "Returns an array of Float64 values representing the information value for each unique combination of categories. Each value "
      "indicates the predictive strength of that category combination for the target variable.",
      {"Array(Float64)"}};
  FunctionDocumentation::Examples examples = {{"Basic usage analyzing age groups vs mobile usage",
                                               R"(
