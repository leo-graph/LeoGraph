...
class AggregateFunctionSumCount final : public AggregateFunctionAvg<T> {
 public:
  ...
  void insertResultInto(AggregateDataPtr __restrict place, IColumn &to, Arena *) const final {
    assert_cast<ColumnVectorOrDecimal<AvgFieldType<T>> &>((assert_cast<ColumnTuple &>(to)).getColumn(0))
        .getData()
        .push_back(this->data(place).numerator);

    assert_cast<ColumnUInt64 &>((assert_cast<ColumnTuple &>(to)).getColumn(1)).getData().push_back(this->data(place).denominator);
  }
...
AggregateFunctionPtr createAggregateFunctionSumCount(const std::string &name, const DataTypes &argument_types, const Array &parameters,
  ...
  if (!allowType(data_type))
    throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT, "Illegal type {} of argument for aggregate function {}", data_type->getName(),
                    name);

  if (isDecimal(data_type))
    res.reset(createWithDecimalType<AggregateFunctionSumCount>(*data_type, argument_types, getDecimalScale(*data_type)));
  else
...
sumCount(x)
  ...
  FunctionDocumentation::Arguments arguments_sumCount = {{"x", "Input value.", {"(U)Int*", "Float", "Decimal"}}};
  FunctionDocumentation::ReturnedValue returned_value_sumCount = {
      "Returns a tuple `(sum, count)`, where `sum` is the sum of numbers and `count` is the number of rows with not-NULL values.",
      {"Tuple"}};
  FunctionDocumentation::Examples examples_sumCount = {{"Basic usage",
                                                        R"(
...
└─────────────┘
  ...
  FunctionDocumentation documentation_sumCount = {description_sumCount,    syntax_sumCount,   arguments_sumCount,     parameters_sumCount,
                                                  returned_value_sumCount, examples_sumCount, introduced_in_sumCount, category_sumCount};
