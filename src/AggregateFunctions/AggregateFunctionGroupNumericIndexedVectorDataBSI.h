 ...
 private:
  ...
  constexpr inline static void checkValidValue(const ValueType &value) {
    if constexpr (std::is_floating_point_v<ValueType>) {
      if (isnan(value)) throw Exception(ErrorCodes::INCORRECT_DATA, "NumericIndexedVector does not support NaN");
      if (isinf(value)) throw Exception(ErrorCodes::INCORRECT_DATA, "NumericIndexedVector does not support Inf");
    }
  }
 ...
 public:
    ...
    if (second_which.isUInt() or second_which.isInt()) {
      switch (second_which.idx) {
        ...
        case TypeIndex::Int64:
          ...
          break;
        default:
          throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT, "Unsupported ValueType: {}", DataTypeNumber<ValueType>().getName());
      }
      fraction_bit_num = 0;
    } else {
  ...
  static void checkIntergerFractionBitNum(UInt32 integer_bit_num, UInt32 fraction_bit_num) {
    if (integer_bit_num > MAX_INTEGER_BIT_NUM || fraction_bit_num > MAX_FRACTION_BIT_NUM)
      throw Exception(ErrorCodes::BAD_ARGUMENTS, "integer_bit_num({}) and fraction_bit_num({}) must <= {} and {} respectly",
                      integer_bit_num, fraction_bit_num, MAX_INTEGER_BIT_NUM, MAX_FRACTION_BIT_NUM);
    if (integer_bit_num + fraction_bit_num > MAX_TOTAL_BIT_NUM)
      throw Exception(ErrorCodes::BAD_ARGUMENTS, "integer_bit_num({}) + fraction_bit_num({}) must <= {}", integer_bit_num, fraction_bit_num,
                      MAX_TOTAL_BIT_NUM);
  }

  bool isEmpty() const {
    if (zero_indexes->size() > 0) return false;
    UInt32 total_bit_num = getTotalBitNum();
    for (size_t i = 0; i < total_bit_num; ++i) {
      if (getDataArrayAt(i)->size() > 0) return false;
    }
    return true;
  }
  ...
  bool isValueTypeSigned() const {
    WhichDataType second_which(DataTypeNumber<ValueType>().getTypeId());
    if (second_which.isInt() || second_which.isFloat()) return true;
    return false;
  }
  ...
  void changeSchema(UInt32 new_integer_bit_num, UInt32 new_fraction_bit_num) {
    if (new_integer_bit_num == integer_bit_num && new_fraction_bit_num == fraction_bit_num) return;
    const UInt32 total_bit_num = getTotalBitNum();
  ...
  std::shared_ptr<Roaring> &getDataArrayAt(size_t index) {
    if (index >= data_array.size()) {
      throw Exception(ErrorCodes::LOGICAL_ERROR,
                      "Array index out of bounds. index: {}; integer_bit_num: {}; fraction_bit_num: {}; data_array_size: {}", index,
                      integer_bit_num, fraction_bit_num, data_array.size());
    }
  ...
  const std::shared_ptr<Roaring> &getDataArrayAt(size_t index) const {
    if (index >= data_array.size()) {
      throw Exception(ErrorCodes::LOGICAL_ERROR,
                      ...
                      index, integer_bit_num, fraction_bit_num, data_array.size());
    }
    return data_array[index];
  }
  ...
  bool allValuesEqualOne() const {
    ...
    const UInt32 total_bit_num = getTotalBitNum();
    if (total_bit_num == 0) return false;

    for (size_t i = 0; i < total_bit_num; ++i) {
      if (i == fraction_bit_num) continue;
  ...
  static inline void setContainers(VectorWithMemoryTracking<roaring::internal::container_t *> &ctns, VectorWithMemoryTracking<UInt8> &types,
                                   UInt32 container_id, BSINumericIndexedVector &vector) {
  ...
  static inline UInt64 float64ToUInt64(Float64 d) {
    if (d >= static_cast<Float64>(std::numeric_limits<UInt64>::max())) return std::numeric_limits<UInt64>::max();
    if (d <= static_cast<Float64>(std::numeric_limits<Int64>::lowest())) return std::numeric_limits<UInt64>::lowest();
    if (d >= 0) return static_cast<UInt64>(d);
   ...
   **/
  static void toVectorCompactArray(const PaddedPODArray<UInt32> &indexes, const PaddedPODArray<Float64> &values, const UInt32 &length,
                                   const UInt32 &container_id, PaddedPODArray<UInt64> &buffer, BSINumericIndexedVector &vector) {
    const UInt32 total_bit_num = vector.getTotalBitNum();

    if (total_bit_num > 64) throw Exception(ErrorCodes::LOGICAL_ERROR, "total_bit_num must less than or equal 64");
    if (length > roaring::internal::DEFAULT_MAX_SIZE)
      throw Exception(ErrorCodes::LOGICAL_ERROR, "keys size ({}) must be less than or equal roaring::internal::DEFAULT_MAX_SIZE({})",
                      length, static_cast<UInt32>(roaring::internal::DEFAULT_MAX_SIZE));
  ...
  static UInt32 prepareBuffer(const PaddedPODArray<Float64> &values, const UInt32 length, const UInt32 fraction_bit_num,
                              const UInt32 total_bit_num, PaddedPODArray<UInt64> &buffer, UInt32 &nonzero_cnt) {
    UInt64 mask = 0xFFFFFFFFFFFFFFFFULL;
    if (total_bit_num < 64) {
      mask = (1ULL << (total_bit_num)) - 1;
    }
   ...
   **/
  static void toVectorCompactBitsetDense(const PaddedPODArray<UInt32> &indexes, const PaddedPODArray<Float64> &values, const UInt32 &length,
                                         const UInt32 container_id, PaddedPODArray<UInt64> &buffer, BSINumericIndexedVector &vector) {
    if (vector.getTotalBitNum() > 64) throw Exception(ErrorCodes::LOGICAL_ERROR, "vector.total_bit_num must less than or equal 64");
    if (indexes.size() != values.size()) throw Exception(ErrorCodes::LOGICAL_ERROR, "keys and values size must be equal. ");
    if (length <= roaring::internal::DEFAULT_MAX_SIZE)
    ...
    for (UInt32 offset = 0; offset < length; offset += k_batch_size) {
      ...
      for (size_t i = 0; i < cnt; ++i) {
        ...
        if (types[col] == ARRAY_CONTAINER_TYPE &&
            static_cast<roaring::internal::array_container_t *>(ctns[col])->cardinality + 1 > roaring::internal::DEFAULT_MAX_SIZE) {
          auto *c = roaring::internal::bitset_container_from_array(reinterpret_cast<roaring::internal::array_container_t *>(ctns[col]));
   ...
   **/
  static void toVectorCompactBitset(const PaddedPODArray<UInt32> &indexes, const PaddedPODArray<Float64> &values, const UInt32 &length,
                                    const UInt32 container_id, PaddedPODArray<UInt64> &buffer, BSINumericIndexedVector &vector) {
    if (vector.getTotalBitNum() > 64) throw Exception(ErrorCodes::LOGICAL_ERROR, "vector.total_bit_num must less than or equal 64");
    if (indexes.size() != values.size()) throw Exception(ErrorCodes::LOGICAL_ERROR, "keys and values size must be equal. ");
    if (length <= roaring::internal::DEFAULT_MAX_SIZE)
    ...
    for (UInt32 offset = 0; offset < length; offset += k_batch_size) {
      ...
      for (size_t i = 0; i < total_bit_num; ++i) {
        ...
        if (types[i] == ARRAY_CONTAINER_TYPE &&
            static_cast<roaring::internal::array_container_t *>(ctns[i])->cardinality + cnt[i] > roaring::internal::DEFAULT_MAX_SIZE) {
          auto *c = roaring::internal::bitset_container_from_array(reinterpret_cast<roaring::internal::array_container_t *>(ctns[i]));
  ...
  static void toVectorCompact(const PaddedPODArray<UInt32> &indexes, const PaddedPODArray<Float64> &values, const UInt32 &length,
                              const UInt32 &container_id, PaddedPODArray<UInt64> &buffer, BSINumericIndexedVector &vector) {
    if (length <= roaring::internal::DEFAULT_MAX_SIZE) {
   ...
   */
  static void toVector(const PaddedPODArray<UInt32> &indexes, const PaddedPODArray<Float64> &values, const UInt32 &length,
                       const UInt32 &container_id, BSINumericIndexedVector &vector) {
   ...
   */
  static UInt16 valueToColumn(const BSINumericIndexedVector &vector, const std::shared_ptr<Roaring> &mask, const UInt32 &container_id,
                              Float64 *output) {
    PaddedPODArray<UInt64> buffer(65536);
    ...
    if (mask_container_cardinality == 0) return 0;
   ...
   */
  static void pointwiseRawBinaryOperate(const BSINumericIndexedVector &lhs, const BSINumericIndexedVector &rhs, const UInt32 op_code,
    ...
    for (const auto &container_id : container_ids) {
      ...
      if (indexes_size != lhs_size || lhs_size != rhs_size)
        throw Exception(ErrorCodes::INCORRECT_DATA, "{} keys_size({}), lhs_size({}), rhs_size({}) must equal in valueToColumn", __func__,
                        indexes_size, lhs_size, rhs_size);
   ...
   */
  static void pointwiseRawBinaryOperate(const BSINumericIndexedVector &lhs, const ValueType &rhs, const UInt32 op_code,
                                        BSINumericIndexedVector &res) {
    if (rhs == 0) {
    ...
    for (const auto &container_id : container_ids) {
      ...
      if (indexes_size != lhs_size)
        throw Exception(ErrorCodes::INCORRECT_DATA, "{} keys_size({}), lhs_size({}) must equal in valueToColumn", __func__, indexes_size,
                        lhs_size);
      switch (op_code) {
        case multiply_op_code:
          for (size_t i = 0; i < indexes_size; ++i) {
  ...
  static std::shared_ptr<Roaring> pointwiseEqual(const BSINumericIndexedVector &lhs, const ValueType &rhs) {
    ...
    UInt64 decimal_value =
        static_cast<UInt64>((rhs - static_cast<CalculationType>(long_value)) * static_cast<CalculationType>(1ULL << lhs.fraction_bit_num));
   ...
   *  original_vector(lhs)[index] >= original_vector(rhs)[index]
   */
  static void pointwiseGreaterEqual(const BSINumericIndexedVector &lhs, const BSINumericIndexedVector &rhs, BSINumericIndexedVector &res) {
  ...
  void addValue(IndexType index, ValueType value) {
    ...
    if constexpr (std::is_same_v<ValueType, UInt64>) {
      ...
      if (fabs(value) > lim / static_cast<Float64>(scaling))
        throw Exception(ErrorCodes::INCORRECT_DATA, "Value {} is out of range for BSI with integer_bit_num={} and fraction_bit_num={}",
                        Float64(value), integer_bit_num, fraction_bit_num);
  ...
  ValueType getValue(IndexType index) const {
    if (zero_indexes->rb_contains(index)) return 0;

    const UInt32 total_bit_num = getTotalBitNum();
    if (total_bit_num == 0) return 0;
      ...
      for (auto &[index, value] : index2value) {
        ...
        if (fraction_bit_num == 0) {
          ...
          values_pod.emplace_back(
              static_cast<ValueType>(static_cast<Float64>(static_cast<Int64>(value)) / static_cast<Float64>(1ULL << fraction_bit_num)));
        }
      }
    } else {
  ...
  void write(DB::WriteBuffer &out) const {
    ...
    UInt8 is_zero_indexes_empty = 0;
    if (zero_indexes->size() == 0) is_zero_indexes_empty = 1;
    writeBinary(is_zero_indexes_empty, out);
    if (is_zero_indexes_empty == 0) zero_indexes->write(out);

    const UInt32 total_bit_num = getTotalBitNum();
    for (size_t i = 0; i < total_bit_num; ++i) {
      UInt8 is_empty = 0;
      if (getDataArrayAt(i)->size() == 0) is_empty = 1;
