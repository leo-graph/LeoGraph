...
AggregateFunctionPtr createAggregateFunctionMLMethod(const std::string &name, const DataTypes &argument_types, const Array &parameters,
                                                     const Settings *) {
  if (parameters.size() > 4)
    throw Exception(ErrorCodes::TOO_MANY_ARGUMENTS_FOR_FUNCTION,
                    "Aggregate function {} requires at most four parameters: "
                    "learning_rate, l2_regularization_coef, mini-batch size and weights_updater method",
                    name);
  ...
  for (size_t i = 0; i < argument_types.size(); ++i) {
    if (!isNativeNumber(argument_types[i]))
      throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT, "Argument {} of type {} must be numeric for aggregate function {}", i,
  ...
  if (parameters.size() > 3) {
    ...
    if (weights_updater_name != "SGD" && weights_updater_name != "Momentum" && weights_updater_name != "Nesterov" &&
        weights_updater_name != "Adam")
      throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT,
                      "Invalid parameter for weights updater. "
  ...
  if constexpr (std::is_same_v<Method, FuncLinearRegression>) {
    ...
    []<bool flag = false>() {
      static_assert(flag, "Such gradient computer is not implemented yet");
    }();  // delay static_assert in constexpr if until template instantiation
  }

  return std::make_shared<Method>(argument_types.size() - 1, std::move(gradient_computer), weights_updater_name, learning_rate, l2_reg_coef,
...
So in the example above the query will return a column with 3 values.
  ...
  FunctionDocumentation::Syntax syntax_linear =
  ...
  FunctionDocumentation::Arguments arguments_linear = {
      {"learning_rate",
       "The coefficient on step length when gradient descent step is performed. A learning rate that is too big may cause infinite weights "
       "of the model. Default is `0.00001`.",
       {"Float64"}},
      {"l2_regularization_coef", "L2 regularization coefficient which may help to prevent overfitting. Default is `0.1`.", {"Float64"}},
      {"mini_batch_size",
       "Sets the number of elements which gradients will be computed and summed to perform one step of gradient descent. Pure stochastic "
       "descent uses one element, however having small batches (about 10 elements) makes gradient steps more stable. Default is `15`.",
       {"UInt64"}},
      {"method",
       "Method for updating weights: `Adam` (by default), `SGD`, `Momentum`, `Nesterov`. `Momentum` and `Nesterov` require slightly more "
       "computations and memory, however they happen to be useful in terms of speed of convergence and stability of stochastic gradient "
       "methods.",
       {"const String"}},
  ...
  FunctionDocumentation::ReturnedValue returned_value_linear = {
      "Returns the trained linear regression model weights. First values correspond to the parameters of the model, the last one is bias. "
      "Use `evalMLMethod` for predictions.",
      {"Array(Float64)"}};
...
evalMLMethod(model, x1, x2) FROM test_data
      ...
      {"Getting model weights", "SELECT stochasticLinearRegression(0.01)(target, x1, x2) FROM train_data",
  ...
  FunctionDocumentation documentation_linear = {description_linear,    syntax_linear,   arguments_linear,     {},
                                                returned_value_linear, examples_linear, introduced_in_linear, category_linear};
...
`test_data` is a table like `train_data` but may not contain target value.
  ...
  FunctionDocumentation::Syntax syntax_logistic =
  ...
  FunctionDocumentation::Arguments arguments_logistic = {
      ...
      {"mini_batch_size",
       ...
       {"UInt64"}},
      {"method",
       "Method for updating weights: `Adam` (by default), `SGD`, `Momentum`, `Nesterov`. `Momentum` and `Nesterov` require a little bit "
       "more computations and memory, however they happen to be useful in terms of speed of convergence and stability of stochastic "
       "gradient methods.",
       {"String"}},
      {"target", "Target binary classification labels. Must be in range [-1, 1].", {"Float"}},
      {"x1, x2, ...", "Feature values (independent variables). All must be numeric.", {"Float"}}};
  FunctionDocumentation::ReturnedValue returned_value_logistic = {
      "Returns the trained logistic regression model weights. Use `evalMLMethod` for predictions which returns probabilities of object "
      "having label `1`.",
      {"Array(Float64)"}};
  FunctionDocumentation::Examples examples_logistic = {{"Training a model",
                                                        R"(
CREATE TABLE your_model
ENGINE = MergeTree
ORDER BY tuple()
...
evalMLMethod(model, x1, x2) AS result FROM test_data)
  ...
  FunctionDocumentation documentation_logistic = {description_logistic,    syntax_logistic,   arguments_logistic,     {},
                                                  returned_value_logistic, examples_logistic, introduced_in_logistic, category_logistic};

  factory.registerFunction("stochasticLogisticRegression",
                           {createAggregateFunctionMLMethod<FuncLogisticRegression>, documentation_logistic, {}});
}

LinearModelData::LinearModelData(Float64 learning_rate_, Float64 l2_reg_coef_, UInt64 param_num_, UInt64 batch_capacity_,
...
void LinearModelData::predict(ColumnVector<Float64>::Container &container, const ColumnsWithTypeAndName &arguments, size_t offset,
                              size_t limit, ContextPtr context) const {
...
void LinearModelData::returnWeights(IColumn &to) const {
  ...
  typename ColumnFloat64::Container &val_to = assert_cast<ColumnFloat64 &>(arr_to.getData()).getData();

  val_to.reserve(old_size + size);
  for (size_t i = 0; i + 1 < size; ++i) val_to.push_back(weights[i]);
...
void LinearModelData::merge(const DB::LinearModelData &rhs) {
  if (iter_num == 0 && rhs.iter_num == 0) return;

  updateState();
  /// can't update rhs state because it's constant

  /// squared mean is more stable (in sense of quality of prediction) when two states with quietly different number of learning steps are
  /// merged
...
void LinearModelData::add(const IColumn **columns, size_t row_num) {
  ...
  weights_updater->addToBatch(gradient_batch, *gradient_computer, weights, bias, l2_reg_coef, target, columns + 1, row_num);
...
void Adam::merge(const IWeightsUpdater &rhs, Float64 frac, Float64 rhs_frac) {
  ...
  if (adam_rhs.average_gradient.empty()) return;
...
void Adam::update(UInt64 batch_size, VectorWithMemoryTracking<Float64> &weights, Float64 &bias, Float64 learning_rate,
                  const VectorWithMemoryTracking<Float64> &batch_gradient) {
  ...
  for (size_t i = 0; i != average_gradient.size(); ++i) {
    ...
    average_squared_gradient[i] = beta2 * average_squared_gradient[i] + (1 - beta2) * normed_gradient * normed_gradient;
  }

  for (size_t i = 0; i < weights.size(); ++i) {
    weights[i] +=
        (learning_rate * average_gradient[i]) / ((1 - beta1_powered) * (sqrt(average_squared_gradient[i] / (1 - beta2_powered)) + eps));
  }
...
void Adam::addToBatch(VectorWithMemoryTracking<Float64> &batch_gradient, IGradientComputer &gradient_computer,
                      const VectorWithMemoryTracking<Float64> &weights, Float64 bias, Float64 l2_reg_coef, Float64 target,
...
void Nesterov::read(ReadBuffer &buf) { readBinary(accumulated_gradient, buf); }

void Nesterov::write(WriteBuffer &buf) const { writeBinary(accumulated_gradient, buf); }
...
void Nesterov::update(UInt64 batch_size, VectorWithMemoryTracking<Float64> &weights, Float64 &bias, Float64 learning_rate,
                      const VectorWithMemoryTracking<Float64> &batch_gradient) {
...
void Nesterov::addToBatch(VectorWithMemoryTracking<Float64> &batch_gradient, IGradientComputer &gradient_computer,
                          const VectorWithMemoryTracking<Float64> &weights, Float64 bias, Float64 l2_reg_coef, Float64 target,
                          const IColumn **columns, size_t row_num) {
...
void Momentum::read(ReadBuffer &buf) { readBinary(accumulated_gradient, buf); }

void Momentum::write(WriteBuffer &buf) const { writeBinary(accumulated_gradient, buf); }
...
void Momentum::update(UInt64 batch_size, VectorWithMemoryTracking<Float64> &weights, Float64 &bias, Float64 learning_rate,
                      const VectorWithMemoryTracking<Float64> &batch_gradient) {
...
void StochasticGradientDescent::update(UInt64 batch_size, VectorWithMemoryTracking<Float64> &weights, Float64 &bias, Float64 learning_rate,
                                       const VectorWithMemoryTracking<Float64> &batch_gradient) {
  /// batch_size is already checked to be greater than  0
  for (size_t i = 0; i < weights.size(); ++i) {
...
void IWeightsUpdater::addToBatch(VectorWithMemoryTracking<Float64> &batch_gradient, IGradientComputer &gradient_computer,
                                 const VectorWithMemoryTracking<Float64> &weights, Float64 bias, Float64 l2_reg_coef, Float64 target,
                                 const IColumn **columns, size_t row_num) {
  gradient_computer.compute(batch_gradient, weights, bias, l2_reg_coef, target, columns, row_num);
}
...
void LogisticRegression::predict(ColumnVector<Float64>::Container &container, const ColumnsWithTypeAndName &arguments, size_t offset,
                                 size_t limit, const VectorWithMemoryTracking<Float64> &weights, Float64 bias,
                                 ContextPtr /*context*/) const {
  size_t rows_num = arguments.front().column->size();

  if (offset > rows_num || offset + limit > rows_num)
    throw Exception(ErrorCodes::LOGICAL_ERROR,
                    "Invalid offset and limit for LogisticRegression::predict. "
  ...
  for (size_t i = 1; i < arguments.size(); ++i) {
    ...
    if (!isNativeNumber(cur_col.type)) throw Exception(ErrorCodes::BAD_ARGUMENTS, "Prediction arguments must have numeric type");

    const auto &features_column = cur_col.column;

    for (size_t row_num = 0; row_num < limit; ++row_num) results[row_num] += weights[i - 1] * features_column->getFloat64(offset + row_num);
  }

  container.reserve(container.size() + limit);
  for (size_t row_num = 0; row_num < limit; ++row_num) container.emplace_back(1 / (1 + exp(-results[row_num])));
}

void LogisticRegression::compute(VectorWithMemoryTracking<Float64> &batch_gradient, const VectorWithMemoryTracking<Float64> &weights,
                                 Float64 bias, Float64 l2_reg_coef, Float64 target, const IColumn **columns, size_t row_num) {
...
void LinearRegression::predict(ColumnVector<Float64>::Container &container, const ColumnsWithTypeAndName &arguments, size_t offset,
                               size_t limit, const VectorWithMemoryTracking<Float64> &weights, Float64 bias, ContextPtr /*context*/) const {
  ...
  for (size_t i = 1; i < arguments.size(); ++i) {
    ...
    if (!features_column) throw Exception(ErrorCodes::LOGICAL_ERROR, "Unexpectedly cannot dynamically cast features column {}", i);

    for (size_t row_num = 0; row_num < limit; ++row_num) results[row_num] += weights[i - 1] * features_column->getFloat64(row_num + offset);
  }

  container.reserve(container.size() + limit);
  for (size_t row_num = 0; row_num < limit; ++row_num) container.emplace_back(results[row_num]);
}

void LinearRegression::compute(VectorWithMemoryTracking<Float64> &batch_gradient, const VectorWithMemoryTracking<Float64> &weights,
                               Float64 bias, Float64 l2_reg_coef, Float64 target, const IColumn **columns, size_t row_num) {
