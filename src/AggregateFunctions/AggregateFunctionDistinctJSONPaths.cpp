...
struct AggregateFunctionDistinctJSONPathsData {
  ...
  void add(const ColumnObject &column, size_t row_num, const UnorderedMapWithMemoryTracking<String, String> &) {
    for (const auto &[path, _] : column.getTypedPaths()) data.insert(path);
    for (const auto &[path, dynamic_column] : column.getDynamicPathsPtrs()) {
      ...
      if (!dynamic_column->isNullAt(row_num)) data.insert(path);
    }
    ...
    for (size_t i = start; i != end; ++i) data.insert(std::string{shared_data_paths->getDataAt(i)});
  }
  ...
  void addWholeColumn(const ColumnObject &column, const UnorderedMapWithMemoryTracking<String, String> &) {
    ...
    for (const auto &[path, dynamic_column] : column.getDynamicPathsPtrs()) {
      ...
      if (dynamic_column->getNumberOfDefaultRows() != dynamic_column->size()) data.insert(path);
    }

    /// Iterate over all paths in shared data.
    const auto [shared_data_paths, _] = column.getSharedDataPathsAndValues();
    for (size_t i = 0; i != shared_data_paths->size(); ++i) data.insert(std::string{shared_data_paths->getDataAt(i)});
  }

  void merge(const AggregateFunctionDistinctJSONPathsData &other) { data.insert(other.data.begin(), other.data.end()); }

  void serialize(WriteBuffer &buf) const {
    writeVarUInt(data.size(), buf);
    for (const auto &path : data) writeStringBinary(path, buf);
  }
  ...
  void insertResultInto(IColumn &column) {
    ...
    for (const auto &path : sorted_data) string_column.insertData(path.data(), path.size());
    array_column.getOffsets().push_back(string_column.size());
  }

  static DataTypePtr getResultType() { return std::make_shared<DataTypeArray>(std::make_shared<DataTypeString>()); }
};
...
struct AggregateFunctionDistinctJSONPathsAndTypesData {
  ...
  void add(const ColumnObject &column, size_t row_num, const UnorderedMapWithMemoryTracking<String, String> &typed_paths_type_names) {
    for (const auto &[path, _] : column.getTypedPaths()) data[path].insert(typed_paths_type_names.at(path));
    for (const auto &[path, dynamic_column] : column.getDynamicPathsPtrs()) {
      if (!dynamic_column->isNullAt(row_num)) data[path].insert(dynamic_column->getTypeNameAt(row_num));
    }
  ...
  void addWholeColumn(const ColumnObject &column, const UnorderedMapWithMemoryTracking<String, String> &typed_paths_type_names) {
    ...
    for (const auto &[path, dynamic_column] : column.getDynamicPathsPtrs()) {
      /// Add dynamic path only if it has at least one non-null value.
      /// getNumberOfDefaultRows for Dynamic column is O(1).
      if (dynamic_column->getNumberOfDefaultRows() != dynamic_column->size()) dynamic_column->getAllTypeNamesInto(data[path]);
    }
  ...
  void merge(const AggregateFunctionDistinctJSONPathsAndTypesData &other) {
    for (const auto &[path, types] : other.data) data[path].insert(types.begin(), types.end());
  }
  ...
  void serialize(WriteBuffer &buf) const {
    ...
    for (const auto &[path, types] : data) {
      ...
      for (const auto &type : types) writeStringBinary(type, buf);
    }
  }
  ...
  void deserialize(ReadBuffer &buf) {
    ...
    if (paths_size > DISTINCT_JSON_PATHS_MAX_ARRAY_SIZE)
      throw Exception(ErrorCodes::TOO_LARGE_ARRAY_SIZE, "Too large array size for paths (maximum: {}): {}",
                      DISTINCT_JSON_PATHS_MAX_ARRAY_SIZE, paths_size);
    ...
    for (size_t i = 0; i != paths_size; ++i) {
      ...
      if (types_size > DISTINCT_JSON_PATHS_MAX_ARRAY_SIZE)
        throw Exception(ErrorCodes::TOO_LARGE_ARRAY_SIZE, "Too large array size for types (maximum: {}): {}",
                        DISTINCT_JSON_PATHS_MAX_ARRAY_SIZE, types_size);
  ...
  void insertResultInto(IColumn &column) {
    ...
    for (const auto &[path, types] : sorted_data) {
      ...
      for (const auto &type : types) value_column_data.insertData(type.data(), type.size());
  ...
  static DataTypePtr getResultType() {
    return std::make_shared<DataTypeMap>(std::make_shared<DataTypeString>(),
...
class AggregateFunctionDistinctJSONPathsAndTypes final
    : public IAggregateFunctionDataHelper<Data, AggregateFunctionDistinctJSONPathsAndTypes<Data>> {
 public:
  explicit AggregateFunctionDistinctJSONPathsAndTypes(const DataTypes &argument_types_)
      : IAggregateFunctionDataHelper<Data, AggregateFunctionDistinctJSONPathsAndTypes<Data>>(argument_types_, {}, Data::getResultType()) {
    ...
    for (const auto &[path, type] : typed_paths_types) typed_paths_type_names[path] = type->getName();
  }
  ...
  void ALWAYS_INLINE addBatchSinglePlace(size_t row_begin, size_t row_end, AggregateDataPtr __restrict place, const IColumn **columns,
                                         Arena *arena, ssize_t if_argument_pos) const override {
    if (if_argument_pos >= 0 || row_begin != 0 || row_end != columns[0]->size())
      IAggregateFunctionDataHelper<Data, AggregateFunctionDistinctJSONPathsAndTypes<Data>>::addBatchSinglePlace(
  ...
  void addManyDefaults(AggregateDataPtr __restrict /*place*/, const IColumn ** /*columns*/, size_t /*length*/,
  ...
  void insertResultInto(AggregateDataPtr __restrict place, IColumn &to, Arena *) const override { this->data(place).insertResultInto(to); }
...
template <typename Data>
static AggregateFunctionPtr createAggregateFunctionDistinctJSONPathsAndTypes(const std::string &name, const DataTypes &argument_types,
                                                                             const Array &parameters, const Settings *) {
  assertNoParameters(name, parameters);
  if (argument_types.size() != 1)
    throw Exception(ErrorCodes::NUMBER_OF_ARGUMENTS_DOESNT_MATCH,
                    "Incorrect number of arguments for aggregate function {}. Expected single argument with type JSON, got {} arguments",
                    name, argument_types.size());

  if (!isObject(argument_types[0]))
    throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT, "Illegal type {} of argument for aggregate function {}. Expected type JSON",
...
distinctJSONPaths(json)
  ...
  FunctionDocumentation::Arguments arguments_distinctJSONPaths = {{"json", "JSON column.", {"JSON"}}};
  FunctionDocumentation::ReturnedValue returned_value_distinctJSONPaths = {"Returns the sorted list of paths.", {"Array(String)"}};
  FunctionDocumentation::Examples examples_distinctJSONPaths = {{"Basic usage with nested JSON",
                                                                 R"(
...
└─────────────────────────┘
  ...
  FunctionDocumentation documentation_distinctJSONPaths = {
      description_distinctJSONPaths,    syntax_distinctJSONPaths,   arguments_distinctJSONPaths,     {},
      returned_value_distinctJSONPaths, examples_distinctJSONPaths, introduced_in_distinctJSONPaths, category_distinctJSONPaths};
...
distinctJSONPathsAndTypes(json)
  ...
  FunctionDocumentation::Arguments arguments_distinctJSONPathsAndTypes = {{"json", "JSON column.", {"JSON"}}};
  FunctionDocumentation::ReturnedValue returned_value_distinctJSONPathsAndTypes = {"Returns the sorted map of paths and types.",
                                                                                   {"Map(String, Array(String))"}};
  FunctionDocumentation::Examples examples_distinctJSONPathsAndTypes = {{"Basic usage with mixed types",
                                                                         R"(
DROP TABLE IF EXISTS test_json;
CREATE TABLE test_json(json JSON) ENGINE = Memory;
INSERT INTO test_json VALUES ('{"a" : 42, "b" : "Hello"}'), ('{"b" : [1, 2, 3], "c" : {"d" : {"e" : "2020-01-01"}}}'), ('{"a" : 43, "c" : {"d" : {"f" : [{"g" : 42}]}}}');
...
└────────────────────────────────────────────────────────────────┘
  ...
  FunctionDocumentation documentation_distinctJSONPathsAndTypes = {
      description_distinctJSONPathsAndTypes,    syntax_distinctJSONPathsAndTypes,
      arguments_distinctJSONPathsAndTypes,      {},
      returned_value_distinctJSONPathsAndTypes, examples_distinctJSONPathsAndTypes,
      introduced_in_distinctJSONPathsAndTypes,  category_distinctJSONPathsAndTypes};

  factory.registerFunction(
      "distinctJSONPaths",
      {createAggregateFunctionDistinctJSONPathsAndTypes<AggregateFunctionDistinctJSONPathsData>, documentation_distinctJSONPaths, {}});
  factory.registerFunction("distinctJSONPathsAndTypes",
                           {createAggregateFunctionDistinctJSONPathsAndTypes<AggregateFunctionDistinctJSONPathsAndTypesData>,
                            documentation_distinctJSONPathsAndTypes,
                            {}});
}
