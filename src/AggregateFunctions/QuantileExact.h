...
struct QuantileExactBase {
  ...
  void deserialize(ReadBuffer &buf) {
    ...
    if (unlikely(size > QUANTILE_EXACT_MAX_ARRAY_SIZE))
      throw Exception(ErrorCodes::TOO_LARGE_ARRAY_SIZE, "Too large array size (maximum: {})", QUANTILE_EXACT_MAX_ARRAY_SIZE);
    array.resize(size);
...
struct QuantileExactExclusive : public QuantileExact<Value> {
  ...
  Float64 getFloat(Float64 level) {
    if (!array.empty()) {
      ...
      if (n >= array.size()) return static_cast<Float64>(*std::max_element(array.begin(), array.end()));
      if (n < 1) return static_cast<Float64>(*std::min_element(array.begin(), array.end()));
    ...
    if (!array.empty()) {
      ...
      for (size_t i = 0; i < size; ++i) {
        ...
        if (n >= array.size())
          ...
          result[indices[i]] =
              static_cast<Float64>(array[n - 1]) + (h - static_cast<Float64>(n)) * static_cast<Float64>(*nth_elem - array[n - 1]);
          prev_n = n - 1;
        }
      }
    } else {
      for (size_t i = 0; i < size; ++i) result[i] = std::numeric_limits<Float64>::quiet_NaN();
    }
  }
};
