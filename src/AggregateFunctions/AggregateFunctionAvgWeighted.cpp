...
using MaxFieldType = std::conditional_t<(sizeof(AvgWeightedFieldType<T>) > sizeof(AvgWeightedFieldType<U>)), AvgWeightedFieldType<T>,
                                        AvgWeightedFieldType<U>>;

template <typename Value, typename Weight>
class AggregateFunctionAvgWeighted final : public AggregateFunctionAvgBase<MaxFieldType<Value, Weight>, AvgWeightedFieldType<Weight>,
                                                                           AggregateFunctionAvgWeighted<Value, Weight>> {
 public:
  using Base =
      AggregateFunctionAvgBase<MaxFieldType<Value, Weight>, AvgWeightedFieldType<Weight>, AggregateFunctionAvgWeighted<Value, Weight>>;
  using Base::Base;
  ...
  void NO_SANITIZE_UNDEFINED add(AggregateDataPtr __restrict place, const IColumn **columns, size_t row_num, Arena *) const override {
    ...
    this->data(place).numerator += static_cast<Numerator>(static_cast<const ColumnVector<Value> &>(*columns[0]).getData()[row_num]) *
                                   static_cast<Numerator>(weights.getData()[row_num]);

    this->data(place).denominator += static_cast<Denominator>(weights.getData()[row_num]);
  }

  void NO_SANITIZE_UNDEFINED addBatchSinglePlace(size_t row_begin, size_t row_end, AggregateDataPtr __restrict place,
  ...
  void NO_SANITIZE_UNDEFINED addBatchSinglePlaceNotNull(size_t row_begin, size_t row_end, AggregateDataPtr __restrict place,
                                                        const IColumn **columns, const UInt8 *null_map, Arena *,
  ...
  bool isCompilable() const override {
    if constexpr (!canBeNativeType<Weight>() || !canBeNativeType<Numerator>() || !canBeNativeType<Denominator>()) return false;
...
bool allowTypes(const DataTypePtr &left, const DataTypePtr &right) noexcept {
  ...
  constexpr auto allow = [](WhichDataType t) { return t.isInt() || t.isUInt() || t.isNativeFloat(); };
...
#define LINE(Type)      \
  case TypeIndex::Type: \
    return new AggregateFunctionAvgWeighted<First, Type>(std::forward<TArgs>(args)...)
  AT_SWITCH(LINE)
#undef LINE
}
...
#define LINE(Type)      \
  case TypeIndex::Type: \
    return create<Type, TArgs...>(second_type, std::forward<TArgs>(args)...)
  AT_SWITCH(LINE)
#undef LINE
}
...
AggregateFunctionPtr createAggregateFunctionAvgWeighted(const std::string &name, const DataTypes &argument_types, const Array &parameters,
  ...
  if (!allowTypes(data_type, data_type_weight))
    throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT, "Types {} and {} are non-conforming as arguments for aggregate function {}",
...
Calculates the [weighted arithmetic mean](https://en.wikipedia.org/wiki/Weighted_arithmetic_mean).
  ...
  FunctionDocumentation::Arguments arguments = {{"x", "Values.", {"(U)Int*", "Float*"}},
                                                {"weight", "Weights of the values.", {"(U)Int*", "Float*"}}};
  FunctionDocumentation::Parameters parameters = {};
  FunctionDocumentation::ReturnedValue returned_value = {
      "Returns `NaN` if all the weights are equal to 0 or the supplied weights parameter is empty, or the weighted mean otherwise.",
      {"Float64"}};
