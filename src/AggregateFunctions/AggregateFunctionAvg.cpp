...
bool allowType(const DataTypePtr& type) noexcept {
  ...
  return t.isInt() || t.isUInt() || t.isFloat() || t.isDecimal() || t.isDate() || t.isDate32() || t.isDateTime() || t.isTime() ||
         t.isDateTime64() || t.isTime64();
}
...
avg(x)
  ...
  FunctionDocumentation::Arguments arguments_avg = {{"x", "Input values.", {"(U)Int*", "Float*", "Decimal"}}};
  ...
  FunctionDocumentation::Examples examples_avg = {{"Basic usage",
                                                   R"(
...
└────────┘
  ...
  FunctionDocumentation documentation_avg = {description_avg,    syntax_avg,   arguments_avg,     parameters_avg,
