...
class DDSketchDenseLogarithmic {
 public:
  ...
  DDSketchDenseLogarithmic(std::unique_ptr<DDSketchLogarithmicMapping> mapping_, std::unique_ptr<DDSketchDenseStore> store_,
  ...
  void deserialize(ReadBuffer& buf) {
    ...
    if (!std::isfinite(count)) throw Exception(ErrorCodes::INCORRECT_DATA, "Invalid total count in DDSketch: {}", count);
  }
  ...
  DDSketchDenseLogarithmic changeMapping(Float64 new_gamma) const {
    ...
    auto remap_store = [this, &new_mapping](DDSketchDenseStore& old_store, std::unique_ptr<DDSketchDenseStore>& target_store) {
      for (int i = 0; i < old_store.length(); ++i) {
        ...
        if (!std::isfinite(in_lower_bound) || !std::isfinite(in_upper_bound) || in_size <= 0) continue;
