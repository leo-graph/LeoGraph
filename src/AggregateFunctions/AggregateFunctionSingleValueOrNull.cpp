...
struct AggregateFunctionSingleValueOrNullData {
 ...
 public:
  ...
  void read(ReadBuffer &buf, const ISerialization &serialization, const DataTypePtr &data_type, Arena *arena) {
...
class AggregateFunctionSingleValueOrNull final
 ...
 public:
  explicit AggregateFunctionSingleValueOrNull(const DataTypePtr &type)
      : IAggregateFunctionDataHelper<AggregateFunctionSingleValueOrNullData, AggregateFunctionSingleValueOrNull>({type}, {},
                                                                                                                 makeNullable(type)),
  ...
  void addBatchSinglePlace(size_t row_begin, size_t row_end, AggregateDataPtr __restrict place, const IColumn **__restrict columns,
    ...
    if (data(place).isNull()) return;
    IAggregateFunctionDataHelper<Data, AggregateFunctionSingleValueOrNull>::addBatchSinglePlace(row_begin, row_end, place, columns, arena,
                                                                                                if_argument_pos);
  }
  ...
  void addBatchSinglePlaceNotNull(size_t row_begin, size_t row_end, AggregateDataPtr __restrict place, const IColumn **__restrict columns,
    ...
    if (data(place).isNull()) return;
    IAggregateFunctionDataHelper<Data, AggregateFunctionSingleValueOrNull>::addBatchSinglePlaceNotNull(row_begin, row_end, place, columns,
                                                                                                       null_map, arena, if_argument_pos);
  }
...
AggregateFunctionPtr createAggregateFunctionSingleValueOrNull(const std::string &name, const DataTypes &argument_types,
...
singleValueOrNull(x)
  ...
  FunctionDocumentation::ReturnedValue returned_value_singleValueOrNull = {
      "Returns the unique value if there is only one unique non-NULL value in `x`. Returns `NULL` if there are zero or at least two "
      "distinct values.",
      {"Any", "NULL"}};
  FunctionDocumentation::Examples examples_singleValueOrNull = {{"Single unique value",
                                                                 R"(
...
└──────────────────────┘
  ...
  FunctionDocumentation documentation_singleValueOrNull = {
      description_singleValueOrNull,    syntax_singleValueOrNull,   arguments_singleValueOrNull,     parameters_singleValueOrNull,
      returned_value_singleValueOrNull, examples_singleValueOrNull, introduced_in_singleValueOrNull, category_singleValueOrNull};
