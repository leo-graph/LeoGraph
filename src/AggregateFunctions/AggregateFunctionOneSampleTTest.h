...
class AggregateFunctionOneSampleTTest final : public IAggregateFunctionDataHelper<Data, AggregateFunctionOneSampleTTest<Data>> {
 private:
 ...
 public:
  AggregateFunctionOneSampleTTest(const DataTypes &arguments, const Array &params)
    ...
    if (arguments.size() != 2)
      throw Exception(ErrorCodes::BAD_ARGUMENTS, "Aggregate function {} requires two arguments: sample_data and population_mean.",
                      Data::name);
    ...
    if (!params.empty()) {
      ...
      if (confidence_level <= 0.0 || confidence_level >= 1.0 || fabs(confidence_level - 0.0) < DBL_EPSILON ||
          fabs(confidence_level - 1.0) < DBL_EPSILON)
