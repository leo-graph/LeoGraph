...
struct QuantileTimingTiny {
  ...
  /// It's important that `count` be at the end of the structure, since the beginning of the structure will be subsequently rewritten by
  /// other objects. You must initialize it by zero itself. Why? `count` field is reused even in cases where the union contains other
  /// structures
  ...
  void insert(UInt64 x) {
    if (unlikely(x > BIG_THRESHOLD)) x = BIG_THRESHOLD;
  ...
  void deserialize(ReadBuffer &buf) {
    ...
    if (new_count > TINY_MAX_ELEMS)
      throw Exception(ErrorCodes::INCORRECT_DATA,
                      "The number of elements {} for the 'tiny' kind of quantileTiming is exceeding the maximum of {}", new_count,
                      TINY_MAX_ELEMS);
  ...
  void prepare() const { ::sort(elems, elems + count); }
  ...
  void getManyFloat(const double *levels, size_t size, float *result) const {
    if (count)
      ...
      for (size_t i = 0; i < size; ++i) result[i] = std::numeric_limits<float>::quiet_NaN();
  }
};
...
struct QuantileTimingMedium {
  ...
  UInt16 get(double level) {
    ...
    if (!elems.empty()) {
      size_t n = level < 1 ? static_cast<size_t>(level * static_cast<double>(elems.size())) : (elems.size() - 1);
  ...
  float getFloat(double level) { return !elems.empty() ? get(level) : std::numeric_limits<float>::quiet_NaN(); }
...
class QuantileTimingLarge {
 private:
  ...
  static UInt16 indexInBigToValue(size_t i) {
    return static_cast<UInt16>((i * BIG_PRECISION) + SMALL_THRESHOLD +
                               (intHash32<0>(i) % BIG_PRECISION -
                                (BIG_PRECISION / 2)));  /// A small randomization so that it is not noticeable that all the values are even.
  }
  ...
  class Iterator {
   private:
    ...
    void adjust() {
      while (isValid() && 0 == *pos) ++pos;
    }

   public:
    explicit Iterator(const QuantileTimingLarge &parent) : begin(parent.count_small), pos(begin), end(&parent.count_big[BIG_SIZE]) {
      adjust();
    }
    ...
    UInt16 key() const {
      return pos - begin < SMALL_THRESHOLD ? static_cast<UInt16>(pos - begin) : indexInBigToValue(pos - begin - SMALL_THRESHOLD);
    }
  };
 ...
 public:
  ...
  void merge(const QuantileTimingLarge &rhs) noexcept {
    ...
    for (size_t i = 0; i < SMALL_THRESHOLD; ++i) count_small[i] += rhs.count_small[i];

    for (size_t i = 0; i < BIG_SIZE; ++i) count_big[i] += rhs.count_big[i];
  }
  ...
  void getMany(const double *levels, const size_t *indices, size_t size, ResultType *result) const {
    ...
    while (index != indices_end) {
      result[*index] = std::numeric_limits<ResultType>::max() < BIG_THRESHOLD ? std::numeric_limits<ResultType>::max() : BIG_THRESHOLD;
      ++index;
    }
  }
...
class QuantileTiming : private boost::noncopyable {
 private:
  ...
  enum class Kind : uint8_t { Tiny = 1, Medium = 2, Large = 3 };

  Kind which() const {
    if (tiny.count <= TINY_MAX_ELEMS) return Kind::Tiny;
    if (tiny.count == TINY_MAX_ELEMS + 1) return Kind::Medium;
    return Kind::Large;
  }
  ...
  void mediumToLarge() {
    ...
    for (const auto &elem : medium.elems) tmp_large->insert(elem);  /// Cannot throw, so don't worry about new.
  ...
  void tinyToLarge() {
    ...
    for (size_t i = 0; i < tiny.count; ++i) tmp_large->insert(tiny.elems[i]);  /// Cannot throw, so don't worry about new.
 ...
 public:
  ...
  void add(T x_) {
    ...
    if (tiny.count < TINY_MAX_ELEMS) {
      ...
      if (unlikely(tiny.count == TINY_MAX_ELEMS)) tinyToMedium();
  ...
  void add(T x_, size_t weight) {
    ...
    if (weight < TINY_MAX_ELEMS && tiny.count + weight <= TINY_MAX_ELEMS) {
      for (size_t i = 0; i < weight; ++i) tiny.insert(x);
    } else {
  ...
  void merge(const QuantileTiming &rhs) {
    if (tiny.count + rhs.tiny.count <= TINY_MAX_ELEMS) {
      ...
      if (kind == Kind::Medium && rhs_kind == Kind::Medium) {
        ...
        for (size_t i = 0; i < rhs.tiny.count; ++i) large->insert(rhs.tiny.elems[i]);
      } else if (kind == Kind::Large && rhs_kind == Kind::Medium) {
        for (const auto &elem : rhs.medium.elems) large->insert(elem);
      } else
  ...
  /// The same, but in the case of an empty state, NaN is returned.
  float getFloat(double level) { return tiny.count ? get(level) : std::numeric_limits<float>::quiet_NaN(); }
  ...
  friend void writeBinary(const Kind &x, WriteBuffer &buf) { writePODBinary(x, buf); }

  friend void readBinary(Kind &x, ReadBuffer &buf) { readPODBinary(x, buf); }
};
