...
class AggregateFunctionSequenceBase : public IAggregateFunctionDataHelper<Data, Derived> {
 public:
  AggregateFunctionSequenceBase(const DataTypes &arguments, const Array &params, const String &pattern_, const DataTypePtr &result_type_)
      : IAggregateFunctionDataHelper<Data, Derived>(arguments, params, result_type_), pattern(pattern_) {
    ...
    while (pos < end) {
      if (match("(?")) {
        if (match("t")) {
          ...
          if (event_number > arg_count - 1) throw Exception(ErrorCodes::BAD_ARGUMENTS, "Event number {} is out of range", event_number);
 ...
 protected:
  ...
  bool backtrackingMatch(EventEntry &events_it, const EventEntry events_end,
                         VectorWithMemoryTracking<T> *best_matched_events = nullptr) const {
    ...
    const auto do_backtrack = [&] {
      while (!back_stack.empty()) {
        ...
        if constexpr (remember_matched_events) do_revert_event_if_needed();
    ...
    while (action_it != action_end && events_it != events_end) {
      if (action_it->type == PatternActionType::SpecificEvent) {
        if (events_it->second.test(action_it->extra)) {
          if constexpr (remember_matched_events) do_push_event();
 ...
 private:
  ...
  struct DFAState {
    explicit DFAState(bool has_kleene_ = false) : has_kleene{has_kleene_}, event{0}, transition{DFATransition::None} {}
...
template <typename T, typename Data>
class AggregateFunctionSequenceMatchEvents final
    : public AggregateFunctionSequenceBase<T, Data, AggregateFunctionSequenceMatchEvents<T, Data>> {
 public:
  AggregateFunctionSequenceMatchEvents(const DataTypes &arguments, const Array &params, const String &pattern_)
      : AggregateFunctionSequenceBase<T, Data, AggregateFunctionSequenceMatchEvents<T, Data>>(
            arguments, params, pattern_, std::make_shared<DataTypeArray>(arguments[0])) {}
 ...
 private:
  UInt64 count(ConstAggregateDataPtr __restrict place) const {
    ...
    if (this->couldMatchDeterministicParts(events_begin, events_end)) {
      while (events_it != events_end && this->backtrackingMatch(events_it, events_end)) ++count;
    }
...
AggregateFunctionPtr createAggregateFunctionSequenceBase(const std::string &name, const DataTypes &argument_types, const Array &params,
                                                         const Settings *) {
  if (params.size() != 1)
    throw Exception(ErrorCodes::NUMBER_OF_ARGUMENTS_DOESNT_MATCH, "Aggregate function {} requires exactly one parameter.", name);

  const auto arg_count = argument_types.size();

  if (arg_count < 3)
    throw Exception(ErrorCodes::TOO_FEW_ARGUMENTS_FOR_FUNCTION, "Aggregate function {} requires at least 3 arguments.", name);

  if (arg_count - 1 > max_events)
    throw Exception(ErrorCodes::TOO_MANY_ARGUMENTS_FOR_FUNCTION, "Aggregate function {} supports up to {} event arguments.", name,
                    max_events);
  ...
  if (which.isDateTime())
    return std::make_shared<AggregateFunction<DataTypeDateTime::FieldType, Data<DataTypeDateTime::FieldType>>>(argument_types, params,
                                                                                                               pattern);
  if (which.isDate())
  ...
  throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT, "Illegal type {} of first argument of aggregate function {}, must be DateTime",
...
void registerAggregateFunctionsSequenceMatch(AggregateFunctionFactory &factory) {
  factory.registerFunction("sequenceMatch",
                           {createAggregateFunctionSequenceBase<AggregateFunctionSequenceMatch, AggregateFunctionSequenceMatchData>, {}});
  factory.registerFunction("sequenceCount",
                           {createAggregateFunctionSequenceBase<AggregateFunctionSequenceCount, AggregateFunctionSequenceMatchData>, {}});
  factory.registerFunction(
      "sequenceMatchEvents",
      {createAggregateFunctionSequenceBase<AggregateFunctionSequenceMatchEvents, AggregateFunctionSequenceMatchData>, {}});
}
