...
using FuncQuantileBFloat16Weighted = AggregateFunctionQuantile<Value, QuantileBFloat16Histogram<Value>, NameQuantileBFloat16Weighted,
                                                               UInt64, std::conditional_t<float_return, Float64, void>, false, false>;
template <typename Value, bool float_return>
using FuncQuantilesBFloat16Weighted = AggregateFunctionQuantile<Value, QuantileBFloat16Histogram<Value>, NameQuantilesBFloat16Weighted,
                                                                UInt64, std::conditional_t<float_return, Float64, void>, true, false>;
...
quantileBFloat16Weighted(level)(expr, weight)
  ...
  FunctionDocumentation::Examples examples = {{"Computing weighted quantile with bfloat16",
                                               R"(
...
└──────────────────────────────────────┴──────────────────────────────────────┘
  ...
  factory.registerFunction(NameQuantileBFloat16Weighted::name,
                           {createAggregateFunctionQuantile<FuncQuantileBFloat16Weighted>, documentation});
