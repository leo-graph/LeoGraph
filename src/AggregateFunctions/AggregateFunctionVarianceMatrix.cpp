...
enum class StatisticsMatrixFunctionKind : uint8_t { covarPopMatrix, covarSampMatrix, corrMatrix };
...
struct AggregateFunctionVarianceMatrixData {
  ...
  explicit AggregateFunctionVarianceMatrixData(const size_t _num_args) : num_args(_num_args) {
    data_matrix.resize_fill(num_args * (num_args + 1) / 2, DataType());
  }

  void add(const IColumn **column, const size_t row_num) {
    for (size_t i = 0; i < num_args; ++i)
      for (size_t j = 0; j <= i; ++j) data_matrix[i * (i + 1) / 2 + j].add(column[i]->getFloat64(row_num), column[j]->getFloat64(row_num));
  }

  void merge(const AggregateFunctionVarianceMatrixData &other) {
    for (size_t i = 0; i < num_args; ++i)
      for (size_t j = 0; j <= i; ++j) data_matrix[i * (i + 1) / 2 + j].merge(other.data_matrix[i * (i + 1) / 2 + j]);
  }

  void serialize(WriteBuffer &buf) const {
    for (size_t i = 0; i < num_args; ++i)
      for (size_t j = 0; j <= i; ++j) data_matrix[i * (i + 1) / 2 + j].write(buf);
  }

  void deserialize(ReadBuffer &buf) {
    for (size_t i = 0; i < num_args; ++i)
      for (size_t j = 0; j <= i; ++j) data_matrix[i * (i + 1) / 2 + j].read(buf);
  }
  ...
  void insertResultInto(IColumn &to) const {
    ...
    for (size_t i = 0; i < num_args; ++i) {
      for (size_t j = 0; j < num_args; ++j) {
        ...
        if constexpr (kind == StatisticsMatrixFunctionKind::covarPopMatrix) data_to.push_back(data.getPopulation());
        if constexpr (kind == StatisticsMatrixFunctionKind::covarSampMatrix) data_to.push_back(data.getSample());
        if constexpr (kind == StatisticsMatrixFunctionKind::corrMatrix) data_to.push_back(data.get());
      }
...
class AggregateFunctionVarianceMatrix final : public IAggregateFunctionDataHelper<Data, AggregateFunctionVarianceMatrix<Data>> {
 public:
  ...
  String getName() const override {
    switch (Data::kind) {
      case StatisticsMatrixFunctionKind::covarPopMatrix:
        return "covarPopMatrix";
      case StatisticsMatrixFunctionKind::covarSampMatrix:
        return "covarSampMatrix";
      case StatisticsMatrixFunctionKind::corrMatrix:
        return "corrMatrix";
    }
  }
...
using AggregateFunctionCovarPopMatrix =
    AggregateFunctionVarianceMatrix<AggregateFunctionVarianceMatrixData<StatisticsMatrixFunctionKind::covarPopMatrix>>;
using AggregateFunctionCovarSampMatrix =
    AggregateFunctionVarianceMatrix<AggregateFunctionVarianceMatrixData<StatisticsMatrixFunctionKind::covarSampMatrix>>;
using AggregateFunctionCorrMatrix =
    AggregateFunctionVarianceMatrix<AggregateFunctionVarianceMatrixData<StatisticsMatrixFunctionKind::corrMatrix>>;

template <typename FunctionTemplate>
AggregateFunctionPtr createAggregateFunctionVarianceMatrix(const std::string &name, const DataTypes &argument_types,
...
Returns the sample covariance matrix over N variables.
  ...
  FunctionDocumentation::ReturnedValue covarSampMatrix_returned_value = {"Returns the sample covariance matrix.",
                                                                         {"Array(Array(Float64))"}};
  FunctionDocumentation::Examples covarSampMatrix_examples = {{"Basic sample covariance matrix calculation",
                                                               R"(
...
└─────────────────────────────┘
  ...
  FunctionDocumentation covarSampMatrix_documentation = {
      covarSampMatrix_description,    covarSampMatrix_syntax,   covarSampMatrix_arguments,     covarSampMatrix_parameters,
      covarSampMatrix_returned_value, covarSampMatrix_examples, covarSampMatrix_introduced_in, covarSampMatrix_category};
  factory.registerFunction("covarSampMatrix",
                           {createAggregateFunctionVarianceMatrix<AggregateFunctionCovarSampMatrix>, covarSampMatrix_documentation});
...
Returns the population covariance matrix over N variables.
  ...
  FunctionDocumentation::ReturnedValue covarPopMatrix_returned_value = {"Returns the population covariance matrix.",
                                                                        {"Array(Array(Float64))"}};
  FunctionDocumentation::Examples covarPopMatrix_examples = {{"Basic population covariance matrix calculation",
                                                              R"(
DROP TABLE IF EXISTS test;
CREATE TABLE test
(
...
└───────────────────────────┘
  ...
  FunctionDocumentation covarPopMatrix_documentation = {
      covarPopMatrix_description,    covarPopMatrix_syntax,   covarPopMatrix_arguments,     covarPopMatrix_parameters,
      covarPopMatrix_returned_value, covarPopMatrix_examples, covarPopMatrix_introduced_in, covarPopMatrix_category};

  factory.registerFunction("covarPopMatrix",
                           {createAggregateFunctionVarianceMatrix<AggregateFunctionCovarPopMatrix>, covarPopMatrix_documentation});
...
Computes the correlation matrix over N variables.
  ...
  FunctionDocumentation::Examples corrMatrix_examples = {{"Basic correlation matrix calculation",
                                                          R"(
DROP TABLE IF EXISTS test;
CREATE TABLE test
(
...
└────────────────────────┘
  ...
  FunctionDocumentation corrMatrix_documentation = {corrMatrix_description,   corrMatrix_syntax,         corrMatrix_arguments,
                                                    corrMatrix_parameters,    corrMatrix_returned_value, corrMatrix_examples,
                                                    corrMatrix_introduced_in, corrMatrix_category};
