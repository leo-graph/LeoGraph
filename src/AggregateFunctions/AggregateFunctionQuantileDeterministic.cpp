...
using FuncQuantileDeterministic = AggregateFunctionQuantile<Value, QuantileReservoirSamplerDeterministic<Value>, NameQuantileDeterministic,
                                                            UInt64, std::conditional_t<float_return, Float64, void>, false, false>;
template <typename Value, bool float_return>
using FuncQuantilesDeterministic =
    AggregateFunctionQuantile<Value, QuantileReservoirSamplerDeterministic<Value>, NameQuantilesDeterministic, UInt64,
...
quantileDeterministic(level)(expr, determinator)
  ...
  FunctionDocumentation::Arguments arguments = {
      ...
      {"determinator",
       "Number whose hash is used instead of a random number generator in the reservoir sampling algorithm to make the result of sampling "
       "deterministic. As a determinator you can use any deterministic positive number, for example, a user id or an event id. If the same "
       "determinator value occurs too often, the function works incorrectly.",
       {"(U)Int*"}}};
  FunctionDocumentation::Parameters parameters = {
      {"level",
       ...
       {"Float*"}}};
  FunctionDocumentation::ReturnedValue returned_value = {"Returns an approximate quantile of the specified level.",
                                                         {"Float64", "Date", "DateTime"}};
  FunctionDocumentation::Examples examples = {{"Computing deterministic quantile",
                                               R"(
