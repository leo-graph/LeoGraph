...
enum class AggregateFunctionIntersectionsKind : uint8_t { Count, Position };
...
class AggregateFunctionIntersectionsMax final
 ...
 public:
  AggregateFunctionIntersectionsMax(AggregateFunctionIntersectionsKind kind_, const DataTypes &arguments)
      : IAggregateFunctionDataHelper<MaxIntersectionsData<PointType>, AggregateFunctionIntersectionsMax<PointType>>(
            arguments, {}, createResultType(kind_)),
        kind(kind_) {
  ...
  static DataTypePtr createResultType(AggregateFunctionIntersectionsKind kind_) {
    if (kind_ == AggregateFunctionIntersectionsKind::Count) return std::make_shared<DataTypeUInt64>();
  ...
  void add(AggregateDataPtr __restrict place, const IColumn **columns, size_t row_num, Arena *arena) const override {
    ...
    if (!isNaN(left)) this->data(place).value.push_back(std::make_pair(left, Int64(1)), arena);

    if (!isNaN(right)) this->data(place).value.push_back(std::make_pair(right, Int64(-1)), arena);
  }
  ...
  void deserialize(AggregateDataPtr __restrict place, ReadBuffer &buf, std::optional<size_t> /* version */, Arena *arena) const override {
    ...
    if (unlikely(size > AGGREGATE_FUNCTION_MAX_INTERSECTIONS_MAX_ARRAY_SIZE))
      throw Exception(ErrorCodes::TOO_LARGE_ARRAY_SIZE, "Too large array size (maximum: {})",
                      AGGREGATE_FUNCTION_MAX_INTERSECTIONS_MAX_ARRAY_SIZE);
...
AggregateFunctionPtr createAggregateFunctionMaxIntersections(AggregateFunctionIntersectionsKind kind, const std::string &name,
...
maxIntersections(start_column, end_column)
  ...
  FunctionDocumentation::Arguments arguments = {
      {"start_column",
       "A numeric column that represents the start of each interval. If `start_column` is `NULL` or 0 then the interval will be skipped.",
       {"(U)Int*", "Float*"}},
      {"end_column",
       "A numeric column that represents the end of each interval. If `end_column` is `NULL` or 0 then the interval will be skipped.",
       {"(U)Int*", "Float*"}}};
  FunctionDocumentation::ReturnedValue returned_value = {"Returns the maximum number of intersected intervals.", {"UInt64"}};
  FunctionDocumentation::Examples examples = {{"Calculating maximum intersections",
                                               R"(
...
└──────────────────────────────┘
  ...
  factory.registerFunction("maxIntersections",
                           {[](const std::string &name, const DataTypes &argument_types, const Array &parameters, const Settings *) {
                              return createAggregateFunctionMaxIntersections(AggregateFunctionIntersectionsKind::Count, name,
...
maxIntersectionsPosition(start_column, end_column)
  ...
  FunctionDocumentation::Arguments position_arguments = {
      ...
      {"end_column",
       "A numeric column that represents the end of each interval. If `end_column` is `NULL` or 0 then the interval will be skipped.",
       {"(U)Int*", "Float*"}}};
  FunctionDocumentation::ReturnedValue position_returned_value = {
      "Returns the start positions of the maximum number of intersected intervals.", {"Any"}};
  FunctionDocumentation::Examples position_examples = {{"Finding maximum intersections position",
                                                        R"(
CREATE TABLE my_events (
    start UInt32,
    end UInt32
...
└──────────────────────────────────────┘
  ...
  FunctionDocumentation position_documentation = {position_description,    position_syntax,   position_arguments,     {},
                                                  position_returned_value, position_examples, position_introduced_in, position_category};

  factory.registerFunction("maxIntersectionsPosition",
                           {[](const std::string &name, const DataTypes &argument_types, const Array &parameters, const Settings *) {
                              return createAggregateFunctionMaxIntersections(AggregateFunctionIntersectionsKind::Position, name,
                                                                             argument_types, parameters);
                            },
