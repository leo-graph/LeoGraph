...
class AggregateFunctionTopK : public IAggregateFunctionDataHelper<AggregateFunctionTopKData<T>, AggregateFunctionTopK<T, is_weighted>> {
 protected:
 ...
 public:
  AggregateFunctionTopK(UInt64 threshold_, UInt64 reserved_, bool include_counts_, bool is_approx_top_k_, const DataTypes &argument_types_,
                        const Array &params)
  ...
  AggregateFunctionTopK(UInt64 threshold_, UInt64 reserved_, bool include_counts_, bool is_approx_top_k_, const DataTypes &argument_types_,
                        const Array &params, const DataTypePtr &result_type_)
  ...
  String getName() const override {
    if (is_approx_top_k) return is_weighted ? "approx_top_sum" : "approx_top_k";
  ...
  void ensureCapacity(AggregateFunctionTopKData<T>::Set &set) const {
    if (unlikely(set.capacity() != reserved)) set.resize(reserved);
  }

  void addBatchSinglePlace(size_t row_begin, size_t row_end, AggregateDataPtr __restrict place, const IColumn **columns, Arena *,
  ...
  void addBatchSinglePlaceNotNull(size_t row_begin, size_t row_end, AggregateDataPtr __restrict place, const IColumn **columns,
                                  const UInt8 *null_map, Arena *, ssize_t if_argument_pos) const override {
    auto &set = this->data(place).value;
    ensureCapacity(set);
  ...
  void merge(AggregateDataPtr __restrict place, ConstAggregateDataPtr rhs, Arena *) const override {
    if (this->data(rhs).value.empty()) return;
...
class AggregateFunctionTopKGeneric final
 ...
 public:
  AggregateFunctionTopKGeneric(UInt64 threshold_, UInt64 reserved_, bool include_counts_, bool is_approx_top_k_,
                               const DataTypes &argument_types_, const Array &params)
      : IAggregateFunctionDataHelper<AggregateFunctionTopKGenericData, AggregateFunctionTopKGeneric<is_plain_column, is_weighted>>(
            argument_types_, params, createResultType(argument_types_, include_counts_)),
        threshold(threshold_),
        reserved(reserved_),
        include_counts(include_counts_),
        is_approx_top_k(is_approx_top_k_) {}
...
class AggregateFunctionTopKDate final : public AggregateFunctionTopK<DataTypeDate::FieldType, is_weighted> {
 public:
  ...
  AggregateFunctionTopKDate(UInt64 threshold_, UInt64 reserved_, bool include_counts_, bool is_approx_top_k_,
                            const DataTypes &argument_types_, const Array &params)
      : AggregateFunctionTopK<DataTypeDate::FieldType, is_weighted>(threshold_, reserved_, include_counts_, is_approx_top_k_,
...
class AggregateFunctionTopKDateTime final : public AggregateFunctionTopK<DataTypeDateTime::FieldType, is_weighted> {
 public:
  ...
  AggregateFunctionTopKDateTime(UInt64 threshold_, UInt64 reserved_, bool include_counts_, bool is_approx_top_k_,
                                const DataTypes &argument_types_, const Array &params)
      : AggregateFunctionTopK<DataTypeDateTime::FieldType, is_weighted>(threshold_, reserved_, include_counts_, is_approx_top_k_,
                                                                        argument_types_, params) {}
};
...
class AggregateFunctionTopKIPv4 final : public AggregateFunctionTopK<DataTypeIPv4::FieldType, is_weighted> {
 public:
  ...
  AggregateFunctionTopKIPv4(UInt64 threshold_, UInt64 reserved_, bool include_counts_, bool is_approx_top_k_,
                            const DataTypes &argument_types_, const Array &params)
      : AggregateFunctionTopK<DataTypeIPv4::FieldType, is_weighted>(threshold_, reserved_, include_counts_, is_approx_top_k_,
                                                                    argument_types_, params) {}
};

template <bool is_weighted>
IAggregateFunction *createWithExtraTypes(const DataTypes &argument_types, UInt64 threshold, UInt64 reserved, bool include_counts,
                                         bool is_approx_top_k, const Array &params) {
  if (argument_types.empty()) throw DB::Exception(ErrorCodes::LOGICAL_ERROR, "Got empty arguments list");
  ...
  if (argument_types[0]->isValueUnambiguouslyRepresentedInContiguousMemoryRegion())
    return new AggregateFunctionTopKGeneric<true, is_weighted>(threshold, reserved, include_counts, is_approx_top_k, argument_types,
                                                               params);
...
AggregateFunctionPtr createAggregateFunctionTopK(const std::string &name, const DataTypes &argument_types, const Array &params,
  ...
  if (!is_weighted) {
    ...
    if (!isInteger(argument_types[1]))
      throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT,
                      "The second argument for aggregate function 'topKWeighted' must have integer type");
  }
  ...
  if (!params.empty()) {
    if (params.size() > 3)
      throw Exception(ErrorCodes::TOO_MANY_ARGUMENTS_FOR_FUNCTION, "Aggregate function '{}' requires three parameters or less", name);
    ...
    if (reserved > DB::TOP_K_MAX_SIZE || load_factor > DB::TOP_K_MAX_SIZE || threshold > DB::TOP_K_MAX_SIZE)
      throw Exception(ErrorCodes::ARGUMENT_OUT_OF_BOUND, "Too large parameter(s) for aggregate function '{}' (maximum is {})", name,
  ...
  if (!res)
    res = AggregateFunctionPtr(
        createWithExtraTypes<is_weighted>(argument_types, threshold, reserved, include_counts, is_approx_top_k, params));
...
topK(N, load_factor, 'counts')(column)
  ...
  FunctionDocumentation::Parameters parameters_topK = {
      ...
      {"load_factor",
       "Optional. Defines, how many cells reserved for values. If `uniq(column) > N * load_factor`, result of topK function will be "
       "approximate. Default value: 3.",
       {"UInt64"}},
...
└─────────────────────┘
  ...
  FunctionDocumentation documentation_topK = {description_topK,    syntax_topK,   arguments_topK,     parameters_topK,
                                              returned_value_topK, examples_topK, introduced_in_topK, category_topK};
...
topKWeighted(N, load_factor, 'counts')(column, weight)
  ...
  FunctionDocumentation::ReturnedValue returned_value_topKWeighted = {
      "Returns an array of the values with maximum approximate sum of weights.", {"Array"}};
  FunctionDocumentation::Examples examples_topKWeighted = {{"Usage example",
                                                            R"(
...
└─────────────────────────────────────┘
  ...
  FunctionDocumentation documentation_topKWeighted = {description_topKWeighted,   syntax_topKWeighted,         arguments_topKWeighted,
                                                      parameters_topKWeighted,    returned_value_topKWeighted, examples_topKWeighted,
                                                      introduced_in_topKWeighted, category_topKWeighted};
...
approx_top_k(N[, reserved])(column)
  ...
  FunctionDocumentation::Parameters parameters_approx_top_k = {
      {"N", "The number of elements to return. Default value: `10`. Maximum value of `N = 65536`.", {"UInt64"}},
      {"reserved",
       "Optional. Defines how many cells reserved for values. If `uniq(column) > reserved`, the result will be approximate. Default value: "
       "`N * 3`.",
       {"UInt64"}},
  };
  FunctionDocumentation::ReturnedValue returned_value_approx_top_k = {
      "Returns an array of the approximately most frequent values and their counts, sorted in descending order of approximate frequency.",
      {"Array"}};
  FunctionDocumentation::Examples examples_approx_top_k = {{"Usage example",
                                                            R"(
...
└───────────────────────┘
  ...
  FunctionDocumentation documentation_approx_top_k = {description_approx_top_k,   syntax_approx_top_k,         arguments_approx_top_k,
                                                      parameters_approx_top_k,    returned_value_approx_top_k, examples_approx_top_k,
                                                      introduced_in_approx_top_k, category_approx_top_k};

  factory.registerFunction("approx_top_k", {createAggregateFunctionTopK<false, true>, documentation_approx_top_k, properties},
...
approx_top_sum(N[, reserved])(column, weight)
  ...
  FunctionDocumentation::Parameters parameters_approx_top_sum = {
      ...
      {"reserved",
       "Optional. Defines, how many cells reserved for values. If `uniq(column) > reserved`, result of topK function will be approximate. "
       "Default value: `N * 3`. Maximum value of `N = 65536`.",
       {"UInt64"}}};
  FunctionDocumentation::Arguments arguments_approx_top_sum = {
      {"column", "The name of the column for which to find the most frequent values.", {"String"}},
      {"weight", "The weight. Every value is accounted `weight` times for frequency calculation.", {"UInt64"}}};
  FunctionDocumentation::ReturnedValue returned_value_approx_top_sum = {
      "Returns an array of the approximately most frequent values and their counts, sorted in descending order of approximate frequency.",
      {"Array"}};
  FunctionDocumentation::Examples examples_approx_top_sum = {{"Usage example",
                                                              R"(
...
└─────────────────────────┘
  ...
  FunctionDocumentation documentation_approx_top_sum = {
      description_approx_top_sum,    syntax_approx_top_sum,   arguments_approx_top_sum,     parameters_approx_top_sum,
      returned_value_approx_top_sum, examples_approx_top_sum, introduced_in_approx_top_sum, category_approx_top_sum};

  factory.registerFunction("approx_top_sum", {createAggregateFunctionTopK<true, true>, documentation_approx_top_sum, properties},
                           AggregateFunctionFactory::Case::Insensitive);
