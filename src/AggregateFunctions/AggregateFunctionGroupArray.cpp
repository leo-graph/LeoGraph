...
constexpr const char *getNameByTrait() {
  if constexpr (Trait::last) return "groupArrayLast";
...
struct GroupArraySamplerData {
  ...
  UInt64 genRandom(size_t lim) {
    ...
    if (lim <= static_cast<UInt64>(pcg32_fast::max())) return rng() % lim;  /// NOLINT(clang-analyzer-core.DivideZero)
...
class GroupArrayNumericImpl final
 ...
 public:
  explicit GroupArrayNumericImpl(const DataTypePtr &data_type_, const Array &parameters_, UInt64 max_elems_, std::optional<UInt64> seed_)
  ...
  void insertWithSampler(Data &a, const T &v, Arena *arena) const {
    ...
    if (a.value.size() < max_elems)
      ...
      if (rnd < max_elems) a.value[rnd] = v;
    }
  }
  ...
  void create(AggregateDataPtr __restrict place) const override  /// NOLINT
  {
    ...
    if constexpr (Trait::sampler == Sampler::RNG) a->rng.seed(seed.value_or(thread_local_rng()));
  }
  ...
  void add(AggregateDataPtr __restrict place, const IColumn **columns, size_t row_num, Arena *arena) const override {
    ...
    if constexpr (Trait::sampler == Sampler::NONE) {
      if constexpr (limit_num_elems) {
        if (cur_elems.value.size() >= max_elems) {
          if constexpr (Trait::last) cur_elems.value[(cur_elems.total_values - 1) % max_elems] = row_value;
          return;
        }
      }
    ...
    if constexpr (Trait::sampler == Sampler::RNG) {
      if (cur_elems.value.size() < max_elems)
        ...
        if (rnd < max_elems) cur_elems.value[rnd] = row_value;
      }
    }
  }
  ...
  void merge(AggregateDataPtr __restrict place, ConstAggregateDataPtr rhs, Arena *arena) const override {
    ...
    if (rhs_elems.value.empty()) return;
  ...
  void mergeNoSampler(Data &cur_elems, const Data &rhs_elems, Arena *arena) const {
    if constexpr (!limit_num_elems) {
      if (rhs_elems.value.size()) cur_elems.value.insertByOffsets(rhs_elems.value, 0, rhs_elems.value.size(), arena);
    } else {
      UInt64 elems_to_insert = std::min(static_cast<size_t>(max_elems) - cur_elems.value.size(), rhs_elems.value.size());
      if (elems_to_insert) cur_elems.value.insertByOffsets(rhs_elems.value, 0, elems_to_insert, arena);
    }
  }

  void mergeWithRNGSampler(Data &cur_elems, const Data &rhs_elems, Arena *arena) const {
    if (rhs_elems.total_values <= max_elems) {
      for (size_t i = 0; i < rhs_elems.value.size(); ++i) insertWithSampler(cur_elems, rhs_elems.value[i], arena);
      ...
      for (size_t i = 0; i < from.size(); ++i) insertWithSampler(cur_elems, from[i], arena);
    } else {
      cur_elems.randomShuffle();
      cur_elems.total_values += rhs_elems.total_values;
      for (size_t i = 0; i < max_elems; ++i) {
        UInt64 rnd = cur_elems.genRandom(cur_elems.total_values);
        if (rnd < rhs_elems.total_values) cur_elems.value[i] = rhs_elems.value[i];
      }
    }
  }
  ...
  static void checkArraySize(size_t elems, size_t max_elems) {
    if (unlikely(elems > max_elems))
      throw Exception(ErrorCodes::TOO_LARGE_ARRAY_SIZE, "Too large array size {} (maximum: {})", elems, max_elems);
  }
  ...
  void serialize(ConstAggregateDataPtr __restrict place, WriteBuffer &buf, std::optional<size_t> /* version */) const override {
    ...
    if constexpr (std::endian::native == std::endian::little) {
      ...
      for (const auto &element : value) writeBinaryLittleEndian(element, buf);
    }

    if constexpr (Trait::last) writeBinaryLittleEndian(this->data(place).total_values, buf);
  ...
  void deserialize(AggregateDataPtr __restrict place, ReadBuffer &buf, std::optional<size_t> /* version */, Arena *arena) const override {
    ...
    if constexpr (std::endian::native == std::endian::little) {
      ...
      for (auto &element : value) readBinaryLittleEndian(element, buf);
    }

    if constexpr (Trait::last) readBinaryLittleEndian(this->data(place).total_values, buf);
...
struct GroupArrayNodeBase {
  ...
  static void checkElementSize(size_t size, size_t max_size) {
    if (unlikely(size > max_size))
      throw Exception(ErrorCodes::TOO_LARGE_ARRAY_SIZE, "Too large array element size {} (maximum: {})", size, max_size);
  }
...
struct GroupArrayNodeGeneral : public GroupArrayNodeBase<GroupArrayNodeGeneral> {
  ...
  void insertInto(IColumn &column) { deserializeAndInsert<false>({data(), size}, column); }
};
...
class GroupArrayGeneralImpl final : public IAggregateFunctionDataHelper<GroupArrayGeneralData<Node, Trait::sampler != Sampler::NONE>,
 ...
 public:
  ...
  void insertWithSampler(Data &a, const Node *v, Arena *arena) const {
    ...
    if (a.value.size() < max_elems)
      ...
      UInt64 rnd = a.genRandom(a.total_values);
      if (rnd < max_elems) a.value[rnd] = v->clone(arena);
    }
  }
  ...
  void add(AggregateDataPtr __restrict place, const IColumn **columns, size_t row_num, Arena *arena) const override {
    ...
    if constexpr (Trait::sampler == Sampler::RNG) {
      if (cur_elems.value.size() < max_elems)
        ...
        UInt64 rnd = cur_elems.genRandom(cur_elems.total_values);
        if (rnd < max_elems) cur_elems.value[rnd] = Node::allocate(*columns[0], row_num, arena);
      }
    }
  }
  ...
  void ALWAYS_INLINE mergeNoSampler(Data &cur_elems, const Data &rhs_elems, Arena *arena) const {
    UInt64 new_elems;
    if (limit_num_elems) {
      if (cur_elems.value.size() >= max_elems) return;
      new_elems = std::min(rhs_elems.value.size(), static_cast<size_t>(max_elems) - cur_elems.value.size());
    } else
      new_elems = rhs_elems.value.size();

    for (UInt64 i = 0; i < new_elems; ++i) cur_elems.value.push_back(rhs_elems.value[i]->clone(arena), arena);
  }
  ...
  void ALWAYS_INLINE mergeWithRNGSampler(Data &cur_elems, const Data &rhs_elems, Arena *arena) const {
    if (rhs_elems.total_values <= max_elems) {
      ...
      decltype(cur_elems.value) from;
      from.swap(cur_elems.value, arena);
      for (auto &node : rhs_elems.value) cur_elems.value.push_back(node->clone(arena), arena);
      cur_elems.total_values = rhs_elems.total_values;
      for (size_t i = 0; i < from.size(); ++i) insertWithSampler(cur_elems, from[i], arena);
    } else {
      cur_elems.randomShuffle();
      ...
      for (size_t i = 0; i < max_elems; ++i) {
        UInt64 rnd = cur_elems.genRandom(cur_elems.total_values);
        if (rnd < rhs_elems.total_values) cur_elems.value[i] = rhs_elems.value[i]->clone(arena);
      }
    }
  }
  ...
  void serialize(ConstAggregateDataPtr __restrict place, WriteBuffer &buf, std::optional<size_t> /* version */) const override {
    ...
    for (auto &node : value) node->write(buf);

    if constexpr (Trait::last) writeBinaryLittleEndian(data(place).total_values, buf);
  ...
  void deserialize(AggregateDataPtr __restrict place, ReadBuffer &buf, std::optional<size_t> /* version */, Arena *arena) const override {
    ...
    if (likely(elems > 0)) {
      ...
      for (UInt64 i = 0; i < elems; ++i) value[i] = Node::read(buf, arena);
    }

    if constexpr (Trait::last) readBinaryLittleEndian(data(place).total_values, buf);
  ...
  void insertResultInto(AggregateDataPtr __restrict place, IColumn &to, Arena *) const override {
    ...
    auto &value = data(place).value;
    for (auto &node : value) node->insertInto(column_data);
  }
...
inline AggregateFunctionPtr createAggregateFunctionGroupArrayImpl(const DataTypePtr &argument_type, const Array &parameters,
                                                                  TArgs... args) {
...
AggregateFunctionPtr createAggregateFunctionGroupArray(const std::string &name, const DataTypes &argument_types, const Array &parameters,
  ...
  if (!has_limit) {
    if (Tlast) throw Exception(ErrorCodes::BAD_ARGUMENTS, "groupArrayLast make sense only with max_elems (groupArrayLast(max_elems)())");
    return createAggregateFunctionGroupArrayImpl<GroupArrayTrait</* Thas_limit= */ false, Tlast, /* Tsampler= */ Sampler::NONE>>(
...
AggregateFunctionPtr createAggregateFunctionGroupArraySample(const std::string &name, const DataTypes &argument_types,
  ...
  std::optional<UInt64> seed;
  if (parameters.size() >= 2) seed = get_parameter(1);

  return createAggregateFunctionGroupArrayImpl<GroupArrayTrait</* Thas_limit= */ true, /* Tlast= */ false, /* Tsampler= */ Sampler::RNG>>(
      argument_types[0], parameters, max_elems, seed);
}
...
groupArray(max_size)(x)
  ...
  FunctionDocumentation::Arguments arguments_groupArray = {{"x", "Argument values to collect into an array.", {"Any"}}};
  FunctionDocumentation::ReturnedValue returned_value_groupArray = {"Returns an array of argument values.", {"Array"}};
  FunctionDocumentation::Examples examples_groupArray = {{"Basic usage",
                                                          R"(
...
└────┴──────────────────────┘
  ...
  FunctionDocumentation documentation_groupArray = {description_groupArray,   syntax_groupArray,         arguments_groupArray,
                                                    parameters_groupArray,    returned_value_groupArray, examples_groupArray,
                                                    introduced_in_groupArray, category_groupArray};
...
groupArraySample(max_size[, seed])(x)
  ...
  FunctionDocumentation::Arguments arguments_groupArraySample = {{"array_column", "Column containing arrays to be aggregated.", {"Array"}}};
  ...
  FunctionDocumentation::ReturnedValue returned_value_groupArraySample = {"Array of randomly selected x arguments.", {"Array(T)"}};
  FunctionDocumentation::Examples examples_groupArraySample = {{"Usage example",
                                                                R"(
...
└─────────────────────────────────────────────┘
  ...
  FunctionDocumentation documentation_groupArraySample = {
      description_groupArraySample,    syntax_groupArraySample,   arguments_groupArraySample,     parameters_groupArraySample,
      returned_value_groupArraySample, examples_groupArraySample, introduced_in_groupArraySample, category_groupArraySample};
...
groupArrayLast(max_size)(x)
  ...
  FunctionDocumentation::Arguments arguments_groupArrayLast = {{"max_size", "Maximum size of the resulting array.", {"UInt64"}},
                                                               {"x", "Argument (column name or expression).", {"Any"}}};
  FunctionDocumentation::Parameters parameters_groupArrayLast = {{"max_size", "Maximum size of the resulting array.", {"UInt64"}}};
  FunctionDocumentation::ReturnedValue returned_value_groupArrayLast = {"Returns an array of the last argument values.", {"Array(T)"}};
  FunctionDocumentation::Examples examples_groupArrayLast = {{"Usage example",
                                                              R"(
...
└─────────┘
  ...
  FunctionDocumentation documentation_groupArrayLast = {
      description_groupArrayLast,    syntax_groupArrayLast,   arguments_groupArrayLast,     parameters_groupArrayLast,
      returned_value_groupArrayLast, examples_groupArrayLast, introduced_in_groupArrayLast, category_groupArrayLast};
