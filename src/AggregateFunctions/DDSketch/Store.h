...
class DDSketchDenseStore {
  ...
  void deserialize(ReadBuffer& buf) {
    ...
    if (encoding_mode == enc.BinEncodingContiguousCounts) {
      ...
      for (UInt64 i = 0; i < num_bins; ++i) {
        ...
        if (bin_count > 0) add(start_key, bin_count);
      ...
      for (UInt64 i = 0; i < num_non_empty_bins; ++i) {
        ...
        if (bin_count > 0) add(previous_index, bin_count);
      }
    } else {
 ...
 private:
  ...
  void extendRange(int key, int second_key) {
    ...
    if (range > max_key_range) throw Exception(ErrorCodes::INCORRECT_DATA, "DDSketch store key range too large: {}", range);
