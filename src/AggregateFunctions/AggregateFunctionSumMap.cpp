...
class AggregateFunctionMapBase : public IAggregateFunctionDataHelper<AggregateFunctionMapData, Derived> {
 private:
 ...
 public:
  ...
  AggregateFunctionMapBase(const DataTypePtr &keys_type_, const DataTypes &values_types_, const DataTypes &argument_types_)
    ...
    for (const auto &type : values_types) {
      ...
      if (type->canBePromoted()) {
        if (type->isNullable())
          promoted_values_serializations.emplace_back(makeNullable(removeNullable(type)->promoteNumericType())->getDefaultSerialization());
        else
  ...
  size_t getVersionFromRevision(size_t revision) const override {
    if (revision >= STATE_VERSION_1_MIN_REVISION) return 1;
    return 0;
  }

  static DataTypePtr createResultType(const DataTypePtr &keys_type_, const DataTypes &values_types_) {
    DataTypes types;
    ...
    for (const auto &value_type : values_types_) {
      if constexpr (std::is_same_v<Visitor, FieldVisitorSum>) {
        if (!value_type->isSummable())
          throw Exception{ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT, "Values for -Map cannot be summed, passed type {}", value_type->getName()};
      }
      ...
      if constexpr (overflow) {
        if (value_type->onlyNull())
          throw Exception{ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT, "Cannot calculate -Map of type {}", value_type->getName()};
        ...
        if (!value_type_without_nullable->canBePromoted())
          throw Exception{ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT,
                          "Values for -Map are expected to be Numeric, Float or Decimal, passed type {}", value_type->getName()};
  ...
  void add(AggregateDataPtr __restrict place, const IColumn **columns_, const size_t row_num, Arena *) const override {
    ...
    for (size_t col = 0, size = values_types.size(); col < size; ++col) {
      ...
      if (keys_vec_size != values_vec_size) throw Exception(ErrorCodes::BAD_ARGUMENTS, "Sizes of keys and values arrays do not match");
  ...
  void serialize(ConstAggregateDataPtr __restrict place, WriteBuffer &buf, std::optional<size_t> version) const override {
    if (!version) version = getDefaultVersion();
    ...
    for (const auto &elem : merged_maps) {
      ...
      for (size_t col = 0; col < values_types.size(); ++col) serialize(col, elem.second);
    }
  }
  ...
  void deserialize(AggregateDataPtr __restrict place, ReadBuffer &buf, std::optional<size_t> version, Arena *) const override {
    ...
    for (size_t i = 0; i < size; ++i) {
      ...
      for (size_t col = 0; col < values_types.size(); ++col) deserialize(col, values);
...
class AggregateFunctionSumMap final
 ...
 public:
  AggregateFunctionSumMap(const DataTypePtr &keys_type_, DataTypes &values_types_, const DataTypes &argument_types_, const Array &params_)
...
template <bool overflow, bool tuple_argument>
class AggregateFunctionSumMapFiltered final : public AggregateFunctionMapBase<AggregateFunctionSumMapFiltered<overflow, tuple_argument>,
                                                                              FieldVisitorSum, overflow, tuple_argument, true> {
 private:
 ...
 public:
  AggregateFunctionSumMapFiltered(const DataTypePtr &keys_type_, const DataTypes &values_types_, const DataTypes &argument_types_,
                                  const Array &params_)
      : Base{keys_type_, values_types_, argument_types_} {
    if (params_.size() != 1)
    ...
    if (!params_.front().tryGet<Array>(keys_to_keep_values))
      throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT, "Aggregate function {} requires an Array as a parameter", getNameImpl());

    this->parameters = params_;

    for (const Field &f : keys_to_keep_values) keys_to_keep.emplace(f);
  }
  ...
  bool keepKey(const Field &key) const {
    if (keys_to_keep.contains(key)) return true;
    ...
    if (key.getType() == Field::Types::UInt64) {
      ...
      if (value <= std::numeric_limits<Int64>::max()) return keys_to_keep.contains(Field(Int64(value)));
    } else if (key.getType() == Field::Types::Int64) {
      const auto &value = key.safeGet<Int64>();
      if (value >= 0) return keys_to_keep.contains(Field(UInt64(value)));
    }
...
class FieldVisitorMax : public StaticVisitor<bool> {
 ...
 public:
  ...
  bool operator()(AggregateFunctionStateData &) const {
    throw Exception(ErrorCodes::LOGICAL_ERROR, "Cannot compare AggregateFunctionStates");
  }
  ...
  bool operator()(DecimalField<T> &x) const {
    return compareImpl<DecimalField<T>>(x);
  }
  template <typename T>
  bool operator()(T &x) const {
    return compareImpl<T>(x);
  }
};
...
class AggregateFunctionMinMap final
 ...
 public:
  AggregateFunctionMinMap(const DataTypePtr &keys_type_, DataTypes &values_types_, const DataTypes &argument_types_, const Array &params_)
      : Base{keys_type_, values_types_, argument_types_} {
    // The constructor accepts parameters to have a uniform interface with
    // sumMapFiltered, but this function doesn't have any parameters.
    assertNoParameters(getNameImpl(), params_);
...
class AggregateFunctionMaxMap final
 ...
 public:
  AggregateFunctionMaxMap(const DataTypePtr &keys_type_, DataTypes &values_types_, const DataTypes &argument_types_, const Array &params_)
      : Base{keys_type_, values_types_, argument_types_} {
    // The constructor accepts parameters to have a uniform interface with
    // sumMapFiltered, but this function doesn't have any parameters.
    assertNoParameters(getNameImpl(), params_);
...
auto parseArguments(const std::string &name, const DataTypes &arguments) {
  ...
  if (arguments.size() == 1) {
    ...
    if (!tuple_type) throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT, "When function {} gets one argument it must be a tuple", name);
  ...
  if (!array_type)
    throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT, "Argument #1 for function {} must be an array, not {}", name, args[0]->getName());
  ...
  for (size_t i = 1; i < args.size(); ++i) {
    ...
    if (!array_type)
      throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT, "Argument #{} for function {} must be an array, not {}", i + 1, name,
                      args[i]->getName());
...
sumMappedArrays(Tuple(key, value1 [, value2, ...]))
  ...
  FunctionDocumentation::Arguments sumMappedArrays_arguments = {
      {"key", "Array of keys.", {"Array"}}, {"value1, value2, ...", "Arrays of values to sum for each key.", {"Array"}}};
  FunctionDocumentation::ReturnedValue sumMappedArrays_returned_value = {
      "Returns a tuple of arrays: the first array contains keys in sorted order, followed by arrays containing values summed for the "
      "corresponding keys.",
      {"Tuple"}};
  FunctionDocumentation::Examples sumMappedArrays_examples = {{"Basic usage with Nested type",
                                                               R"(
...
-- Third array: total clicks for each browser
  ...
  FunctionDocumentation sumMappedArrays_documentation = {
      sumMappedArrays_description,    sumMappedArrays_syntax,   sumMappedArrays_arguments,     {},
      sumMappedArrays_returned_value, sumMappedArrays_examples, sumMappedArrays_introduced_in, sumMappedArrays_category};

  factory.registerFunction(
      "sumMappedArrays",
      {[](const std::string &name, const DataTypes &arguments, const Array &params, const Settings *) -> AggregateFunctionPtr {
         auto [keys_type, values_types, tuple_argument] = parseArguments(name, arguments);
         if (tuple_argument) return std::make_shared<AggregateFunctionSumMap<false, true>>(keys_type, values_types, arguments, params);
...
minMappedArrays(Tuple(key, value))
  ...
  FunctionDocumentation::Arguments minMappedArrays_arguments = {{"key", "Array of keys.", {"Array(T)"}},
                                                                {"value", "Array of values.", {"Array(T)"}}};
  FunctionDocumentation::ReturnedValue minMappedArrays_returned_value = {
      "Returns a tuple of two arrays: keys in sorted order, and values calculated for the corresponding keys.",
      {"Tuple(Array(T), Array(T))"}};
  FunctionDocumentation::Examples minMappedArrays_examples = {{"Usage example",
                                                               R"(
...
└─────────────────────────────────┘
  ...
  FunctionDocumentation minMappedArrays_documentation = {
      minMappedArrays_description,    minMappedArrays_syntax,   minMappedArrays_arguments,     {},
      minMappedArrays_returned_value, minMappedArrays_examples, minMappedArrays_introduced_in, minMappedArrays_category};

  factory.registerFunction(
      "minMappedArrays",
      {[](const std::string &name, const DataTypes &arguments, const Array &params, const Settings *) -> AggregateFunctionPtr {
         auto [keys_type, values_types, tuple_argument] = parseArguments(name, arguments);
         if (tuple_argument) return std::make_shared<AggregateFunctionMinMap<true>>(keys_type, values_types, arguments, params);
...
maxMappedArrays(Tuple(key, value))
  ...
  FunctionDocumentation::Arguments maxMappedArrays_arguments = {{"key", "Array of keys.", {"Array(T)"}},
                                                                {"value", "Array of values.", {"Array(T)"}}};
  FunctionDocumentation::ReturnedValue maxMappedArrays_returned_value = {
      "Returns a tuple of two arrays: keys in sorted order, and values calculated for the corresponding keys.",
      {"Tuple(Array(T), Array(T))"}};
  FunctionDocumentation::Examples maxMappedArrays_examples = {{"Usage example",
                                                               R"(
...
└──────────────────────────────────────┘
  ...
  FunctionDocumentation maxMappedArrays_documentation = {
      maxMappedArrays_description,    maxMappedArrays_syntax,   maxMappedArrays_arguments,     {},
      maxMappedArrays_returned_value, maxMappedArrays_examples, maxMappedArrays_introduced_in, maxMappedArrays_category};

  factory.registerFunction(
      "maxMappedArrays",
      {[](const std::string &name, const DataTypes &arguments, const Array &params, const Settings *) -> AggregateFunctionPtr {
         auto [keys_type, values_types, tuple_argument] = parseArguments(name, arguments);
         if (tuple_argument) return std::make_shared<AggregateFunctionMaxMap<true>>(keys_type, values_types, arguments, params);
...
sumMapWithOverflow(Tuple(key, value))
  ...
  FunctionDocumentation::Arguments sumMapWithOverflow_arguments = {{"key", "Array of keys.", {"Array"}},
                                                                   {"value", "Array of values.", {"Array"}}};
  FunctionDocumentation::ReturnedValue sumMapWithOverflow_returned_value = {
      "Returns a tuple of two arrays: keys in sorted order, and values summed for the corresponding keys.", {"Tuple(Array, Array)"}};
  FunctionDocumentation::Examples sumMapWithOverflow_examples = {{"Array syntax demonstrating overflow behavior",
                                                                  R"(
CREATE TABLE sum_map(
    date Date,
    timeslot DateTime,
...
└─────────────────────┴────────────────────────────────────┴─────────────────────────────────┘
  ...
  FunctionDocumentation sumMapWithOverflow_documentation = {
      sumMapWithOverflow_description,    sumMapWithOverflow_syntax,   sumMapWithOverflow_arguments,     {},
      sumMapWithOverflow_returned_value, sumMapWithOverflow_examples, sumMapWithOverflow_introduced_in, sumMapWithOverflow_category};

  factory.registerFunction(
      "sumMapWithOverflow",
      {[](const std::string &name, const DataTypes &arguments, const Array &params, const Settings *) -> AggregateFunctionPtr {
         auto [keys_type, values_types, tuple_argument] = parseArguments(name, arguments);
         if (tuple_argument) return std::make_shared<AggregateFunctionSumMap<true, true>>(keys_type, values_types, arguments, params);
