...
struct QuantileBFloat16Histogram {
  ...
  void add(const Value& x, Weight w) {
    if (!isNaN(x)) data[toBFloat16(x)] += w;
  }

  void merge(const QuantileBFloat16Histogram& rhs) {
    for (const auto& pair : rhs.data) data[pair.getKey()] += pair.getMapped();
  }
 ...
 private:
  ...
  BFloat16 toBFloat16(const Value& x) const { return std::bit_cast<UInt32>(static_cast<Float32>(x)) >> 16; }

  /// Put the bits into most significant 16 bits of the floating point number and fill other bits with zeros.
  Float32 toFloat32(const BFloat16& x) const { return std::bit_cast<Float32>(x << 16); }
  ...
  T getImpl(Float64 level) const {
    ...
    if (0 == size) return std::numeric_limits<T>::quiet_NaN();
    ...
    for (const Pair* p = array; p != (array + size); ++p) {
      ...
      if (accumulated >= threshold) return safeCast<T>(p->first);
    }
  ...
  void getManyImpl(const Float64* levels, const size_t* indices, size_t num_levels, T* result) const {
    ...
    if (0 == size) {
      for (size_t i = 0; i < num_levels; ++i) result[i] = std::numeric_limits<T>::quiet_NaN();
