...
/** The number of least significant bits used for thinning. The remaining high-order bits are used to determine the position in the hash
 * table. (high-order bits are taken because the younger bits will be constant after dropping some of the values)
 */
...
class UniquesHashSet : private HashTableAllocatorWithStackMemory<(1ULL << UNIQUES_HASH_SET_INITIAL_SIZE_DEGREE) * sizeof(UInt32)> {
 private:
  ...
  void resize(size_t new_size_degree = 0) {
    ...
    if (!new_size_degree) new_size_degree = size_degree + 1;
     ...
     * location. There is also a special case if the element was to be at the end of the old buffer,                        [        x] but
     * is at the beginning because of the collision resolution chain,            [o       x] then after resizing, it will first be out of
     * place again,                     [        xo        ] and in order to transfer it to where you need it, will have to be after
     * transferring all elements from the old half             [         o   x    ] process another tail from the collision resolution chain
     * immediately after it [        o    x    ] This is why || buf[i] below.
     */
 ...
 public:
  ...
  size_t size() const {
    if (0 == skip_degree) return m_size;
  ...
  void write(DB::WriteBuffer &wb) const {
    if (m_size > UNIQUES_HASH_MAX_SIZE) throw Poco::Exception("Cannot write UniquesHashSet: too large size_degree.");
    ...
    for (size_t i = 0; i < buf_size(); ++i)
      if (buf[i]) DB::writeBinaryLittleEndian(buf[i], wb);
  }
  ...
  void read(DB::ReadBuffer &rb) {
    ...
    if (m_size > UNIQUES_HASH_MAX_SIZE) throw Poco::Exception("Cannot read UniquesHashSet: too large size_degree.");

    free();

    UInt8 new_size_degree =
        m_size <= 1 ? UNIQUES_HASH_SET_INITIAL_SIZE_DEGREE
  ...
  static void skip(DB::ReadBuffer &rb) {
    ...
    if (size > UNIQUES_HASH_MAX_SIZE) throw Poco::Exception("Cannot read UniquesHashSet: too large size_degree.");
