...
struct AggregateFunctionIntervalLengthSumData {
  ...
  void add(T begin, T end) {
    ...
    if (sorted && !segments.empty()) sorted = segments.back().first <= begin;
  ...
  void merge(const AggregateFunctionIntervalLengthSumData &other) {
    ...
    if (!sorted && !other.sorted) {
      ...
      if (!sorted) ::sort(begin, middle);

      if (!other.sorted) ::sort(middle, end);
...
class AggregateFunctionIntervalLengthSum final : public IAggregateFunctionDataHelper<Data, AggregateFunctionIntervalLengthSum<T, Data>> {
 private:
  static auto NO_SANITIZE_UNDEFINED length(typename Data::Segment segment) { return segment.second - segment.first; }

  template <typename TResult>
  TResult getIntervalLengthSum(Data &data) const {
    if (data.segments.empty()) return 0;
 ...
 public:
  ...
  static DataTypePtr createResultType() {
    if constexpr (is_floating_point<T>) return std::make_shared<DataTypeFloat64>();
...
AggregateFunctionPtr createAggregateFunctionIntervalLengthSum(const std::string &name, const DataTypes &arguments, const Array &,
                                                              const Settings *) {
  if (arguments.size() != 2)
    throw Exception(ErrorCodes::NUMBER_OF_ARGUMENTS_DOESNT_MATCH, "Aggregate function {} requires two timestamps argument.", name);
  ...
  for (const auto &arg : args) {
    if (!isNativeNumber(arg) && !isDate(arg) && !isDateTime(arg))
      throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT,
                      ...
                      "be native integral type, Date/DateTime or Float",
                      arg->getName(), name);
  }
...
intervalLengthSum(start, end)
  ...
  FunctionDocumentation::ReturnedValue returned_value = {
      "Returns the total length of union of all ranges (segments on numeric axis). Depending on the type of the argument, the return value "
      "may be UInt64 or Float64 type.",
      {"UInt64", "Float64"}};
  FunctionDocumentation::Examples examples = {{"Float32 example",
                                               R"(
...
└────┴───────────────────────────────┴───────────────────────────────────────────┘
  ...
  factory.registerFunction("intervalLengthSum",
                           {createAggregateFunctionIntervalLengthSum<AggregateFunctionIntervalLengthSumData>, documentation});
}
