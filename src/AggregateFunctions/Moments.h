...
struct VarMoments {
  ...
  T getMoment4() const {
    if constexpr (_level < 4) {
      ...
      return (m[4] - (4 * m[3] - (6 * m[2] - 3 * m[1] * m[1] / m[0]) * m[1] / m[0]) * m[1] / m[0]) / m[0];
    }
  }
};
...
struct CovarMoments {
  ...
  T NO_SANITIZE_UNDEFINED getPopulation() const { return (xy - x1 * y1 / m0) / m0; }

  T NO_SANITIZE_UNDEFINED getSample() const {
    if (m0 == 0) return std::numeric_limits<T>::quiet_NaN();
...
struct CorrMoments {
  ...
  T NO_SANITIZE_UNDEFINED get() const { return (m0 * xy - x1 * y1) / sqrt((m0 * x2 - x1 * x1) * (m0 * y2 - y1 * y1)); }

  T getSample() const { throw Exception(ErrorCodes::LOGICAL_ERROR, "Correlation moments should be obtained by the 'get' method"); }

  T getPopulation() const { throw Exception(ErrorCodes::LOGICAL_ERROR, "Correlation moments should be obtained by the 'get' method"); }
...
struct TTestMoments {
  ...
  Float64 getMeanX() const { return x1 / nx; }

  Float64 getMeanY() const { return y1 / ny; }
...
struct OneSampleTTestMoments {
  ...
  void setPopulationMean(Float64 mean) { population_mean = mean; }
  ...
  Float64 getMean() const { return x1 / n; }

  Float64 getVariance() const {
    if (n <= 1) return std::numeric_limits<Float64>::quiet_NaN();
  ...
  Float64 getDegreesOfFreedom() const { return n - 1; }
  ...
  bool hasEnoughObservations() const { return n > 1; }
};
...
struct ZTestMoments {
  ...
  std::pair<Float64, Float64> getConfidenceIntervals(Float64 pop_var_x, Float64 pop_var_y, Float64 confidence_level) const {
    ...
    Float64 z = boost::math::quantile(boost::math::complement(boost::math::normal(0.0f, 1.0f), (1.0f - confidence_level) / 2.0f));
...
struct AnalysisOfVarianceMoments {
  ...
  void resizeIfNeeded(size_t possible_size) {
    if (xs1.size() >= possible_size) return;
  ...
  Float64 getMeanAll() const {
    ...
    if (n == 0) throw Exception(ErrorCodes::BAD_ARGUMENTS, "There are no observations to calculate mean value");

    return std::accumulate(xs1.begin(), xs1.end(), 0.0) / static_cast<Float64>(n);
  }

  Float64 getMeanGroup(size_t group) const {
    if (ns[group] == 0) throw Exception(ErrorCodes::BAD_ARGUMENTS, "There is no observations for group {}", group);
  ...
  Float64 getFStatistic() const {
    ...
    if (k == 1) throw Exception(ErrorCodes::BAD_ARGUMENTS, "There should be more than one group to calculate f-statistics");

    if (k == n) throw Exception(ErrorCodes::BAD_ARGUMENTS, "There is only one observation in each group");
  ...
  Float64 getPValue(Float64 f_statistic) const {
    if (unlikely(!std::isfinite(f_statistic))) return std::numeric_limits<Float64>::quiet_NaN();
