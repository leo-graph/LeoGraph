...
class AggregateFunctionGroupArrayIntersect
 ...
 public:
  AggregateFunctionGroupArrayIntersect(const DataTypePtr &argument_type, const Array &parameters_)
      : IAggregateFunctionDataHelper<AggregateFunctionGroupArrayIntersectData<T>, AggregateFunctionGroupArrayIntersect<T>>(
            {argument_type}, parameters_, argument_type) {}
  ...
  void add(AggregateDataPtr __restrict place, const IColumn **columns, size_t row_num, Arena *) const override {
    ...
    ++version;
    if (version == 1) {
      for (size_t i = 0; i < arr_size; ++i) set.insert(static_cast<T>((*data_column)[offset + i].safeGet<T>()));
    } else if (!set.empty()) {
      ...
      for (size_t i = 0; i < arr_size; ++i) {
        ...
        if (set_value != nullptr) new_set.insert(static_cast<T>((*data_column)[offset + i].safeGet<T>()));
      }
  ...
  void merge(AggregateDataPtr __restrict place, ConstAggregateDataPtr rhs, Arena *) const override {
    ...
    if (version == 0) {
      for (auto &rhs_elem : rhs_set) set.insert(rhs_elem.getValue());
      return;
    }
    ...
    if (!set.empty()) {
      auto create_new_set = [](auto &lhs_val, auto &rhs_val) {
        ...
        for (auto &lhs_elem : lhs_val) {
          ...
          if (res != nullptr) new_set.insert(lhs_elem.getValue());
        }
        return new_set;
      };
  ...
  void serialize(ConstAggregateDataPtr __restrict place, WriteBuffer &buf, std::optional<size_t> /* version */) const override {
    ...
    for (const auto &elem : set) writeIntBinary(elem.getValue(), buf);
  }
 ...
 public:
  AggregateFunctionGroupArrayIntersectGeneric(const DataTypePtr &input_data_type_, const Array &parameters_)
      : IAggregateFunctionDataHelper<AggregateFunctionGroupArrayIntersectGenericData,
                                     AggregateFunctionGroupArrayIntersectGeneric<is_plain_column>>({input_data_type_}, parameters_,
                                                                                                   input_data_type_),
    ...
    if (version == 1) {
      ...
      for (size_t i = 0; i < arr_size; ++i) {
        if constexpr (is_plain_column) {
          ...
          if (it != nullptr) new_set.emplace(ArenaKeyHolder{data_column->getDataAt(offset + i), *arena}, it, inserted);
        } else {
          const char *begin = nullptr;
          auto settings = IColumn::SerializationSettings::createForAggregationState();
          auto serialized = data_column->serializeValueIntoArena(offset + i, *arena, begin, &settings);
          ...
          if (it != nullptr) new_set.emplace(SerializedKeyHolder{serialized, *arena}, it, inserted);
        }
      }
      set = std::move(new_set);
    }
    ...
    if (version == 0) {
      ...
      auto create_new_map = [](auto &lhs_val, auto &rhs_val) {
        ...
        for (auto &lhs_elem : lhs_val) {
          ...
          if (val != nullptr) new_map.insert(lhs_elem.getValue());
        }
        return new_map;
      };
...
IAggregateFunction *createWithExtraTypes(const DataTypePtr &argument_type, const Array &parameters) {
  ...
  if (which.idx == TypeIndex::DateTime) return new AggregateFunctionGroupArrayIntersectDateTime(argument_type, parameters);
  if (which.idx == TypeIndex::Date32) return new AggregateFunctionGroupArrayIntersectDate32(argument_type, parameters);
...
inline AggregateFunctionPtr createAggregateFunctionGroupArrayIntersectImpl(const std::string &name, const DataTypePtr &argument_type,
                                                                           const Array &parameters) {
  const auto &nested_type = dynamic_cast<const DataTypeArray &>(*argument_type).getNestedType();
  AggregateFunctionPtr res(
      createWithNumericType<AggregateFunctionGroupArrayIntersect, const DataTypePtr &>(*nested_type, argument_type, parameters));
  if (!res) {
...
AggregateFunctionPtr createAggregateFunctionGroupArrayIntersect(const std::string &name, const DataTypes &argument_types,
  ...
  if (!parameters.empty())
    throw Exception(ErrorCodes::NUMBER_OF_ARGUMENTS_DOESNT_MATCH, "Incorrect number of parameters for aggregate function {}, should be 0",
                    name);
...
Return an intersection of given arrays (Return all items of arrays, that are in all given arrays).
  ...
  FunctionDocumentation::Arguments arguments = {{"x", "Argument (column name or expression).", {"Any"}}};
