...
struct AggregateFunctionSimpleLinearRegressionData final {
  ...
  Float64 getK() const {
    ...
    if (divisor == 0) return std::numeric_limits<Float64>::quiet_NaN();

    return (sum_xy * static_cast<Float64>(count) - sum_x * sum_y) / divisor;
  }

  Float64 getB(Float64 k) const {
    if (count == 0) return std::numeric_limits<Float64>::quiet_NaN();
...
class AggregateFunctionSimpleLinearRegression final
    : public IAggregateFunctionDataHelper<AggregateFunctionSimpleLinearRegressionData, AggregateFunctionSimpleLinearRegression> {
 public:
  AggregateFunctionSimpleLinearRegression(const DataTypes &arguments, const Array &params)
      : IAggregateFunctionDataHelper<AggregateFunctionSimpleLinearRegressionData, AggregateFunctionSimpleLinearRegression>{
            arguments, params, createResultType()} {
    // notice: arguments has been checked before
  }

  String getName() const override { return "simpleLinearRegression"; }
...
AggregateFunctionPtr createAggregateFunctionSimpleLinearRegression(const String &name, const DataTypes &arguments, const Array &params,
                                                                   const Settings *) {
  ...
  if (!isNumber(arguments[0]) || !isNumber(arguments[1]))
    throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT,
                    ...
                    "be Native Ints, Native UInts or Floats",
...
simpleLinearRegression(x, y)
  ...
  FunctionDocumentation::Arguments arguments_simpleLinearRegression = {{"x", "Column with explanatory variable values.", {"Float64"}},
                                                                       {"y", "Column with dependent variable values.", {"Float64"}}};
  FunctionDocumentation::ReturnedValue returned_value_simpleLinearRegression = {
      "Returns constants `(k, b)` of the resulting line `y = k*x + b`.", {"Tuple(Float64, Float64)"}};
  FunctionDocumentation::Examples examples_simpleLinearRegression = {{"Perfect linear fit",
                                                                      R"(
...
└───────────────────────────────────────────────────────────────────┘
  ...
  FunctionDocumentation documentation_simpleLinearRegression = {description_simpleLinearRegression,    syntax_simpleLinearRegression,
                                                                arguments_simpleLinearRegression,      parameters_simpleLinearRegression,
