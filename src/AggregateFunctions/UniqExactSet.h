...
class UniqExactSet {
 ...
 public:
  ...
  auto ALWAYS_INLINE insert(Arg &&arg) {
    if constexpr (hint == SetLevelHint::singleLevel) {
      ...
      if (isSingleLevel()) {
        ...
        if (inserted && worthConvertingToTwoLevel(asSingleLevel().size())) convertToTwoLevel();
      } else {
        asTwoLevel().insert(std::forward<Arg>(arg));
      }
    }
  ...
  /// In merge, if one of the lhs and rhs is twolevelset and the other is singlelevelset, then the singlelevelset will need to
  /// convertToTwoLevel(). It's not in parallel and will cost extra large time if the thread_num is large. This method will convert all the
  /// SingleLevelSet to TwoLevelSet in parallel if the hashsets are not all singlelevel or not all twolevel.
  static void parallelizeMergePrepare(const VectorWithMemoryTracking<UniqExactSet *> &data_vec, ThreadPool &thread_pool,
                                      std::atomic<bool> &is_cancelled) {
    UInt64 single_level_set_num = 0;
    UInt64 all_single_hash_size = 0;

    for (auto ele : data_vec) {
      if (ele->isSingleLevel()) single_level_set_num++;
    }

    if (single_level_set_num == data_vec.size()) {
      for (auto ele : data_vec) all_single_hash_size += ele->size();
    }

    /// If all the hashtables are mixed by singleLevel and twoLevel, or all singleLevel (larger than 6000 for average value), they could be
    /// converted into twoLevel hashtables in parallel and then merge together. please refer to the following PR for more details.
      ...
      try {
        ...
        auto thread_func = [data_vec, data_vec_atomic_index, &is_cancelled, thread_group = CurrentThread::getGroup()]() {
          ...
          while (true) {
            ...
            if (i >= data_vec.size()) return;
            if (data_vec[i]->isSingleLevel()) data_vec[i]->convertToTwoLevel();
          }
        };
  ...
  auto merge(const UniqExactSet &other, ThreadPool *thread_pool = nullptr, std::atomic<bool> *is_cancelled = nullptr) {
    ...
    if (isSingleLevel() && other.isTwoLevel()) convertToTwoLevel();
    ...
    if (isSingleLevel()) {
      ...
      if (other.isSingleLevel()) return lhs.merge(other.asSingleLevel());
        ...
        try {
          ...
          auto thread_func = [&lhs, &rhs, next_bucket_to_merge, is_cancelled]() {
            while (true) {
              ...
              if (bucket >= rhs.NUM_BUCKETS) return;
  ...
  /// To convert set to two level before merging (we cannot just call convertToTwoLevel() on right hand side set, because it is declared
  /// const).
 ...
 private:
  ...
  void doDeepCopyIfNeeded() const {
    if (two_level_set && two_level_set.use_count() > 1) {
      ...
      for (size_t i = 0; i < two_level_set->NUM_BUCKETS; ++i) copy->impls[i].merge(two_level_set->impls[i]);
