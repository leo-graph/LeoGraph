...
struct WelchTTestData : public TTestMoments<Float64> {
  ...
  bool hasEnoughObservations() const { return nx > 1 && ny > 1; }
  ...
  Float64 getDegreesOfFreedom() const {
    ...
    if (result <= 0 || std::isinf(result) || isNaN(result))
      throw Exception(ErrorCodes::BAD_ARGUMENTS,
                ...
                has inappropriate value of degrees of freedom (={}). It should be > 0",
                      result);
  ...
  std::tuple<Float64, Float64> getResult() const {
    ...
    if (unlikely(!std::isfinite(t_stat))) return {std::numeric_limits<Float64>::quiet_NaN(), std::numeric_limits<Float64>::quiet_NaN()};
...
welchTTest([confidence_level])(sample_data, sample_index)
  ...
  FunctionDocumentation::Arguments arguments_welchTTest = {{"sample_data", "Sample data.", {"Int*", "UInt*", "Float*", "Decimal*"}},
                                                           {"sample_index", "Sample index.", {"Int*", "UInt*"}}};
  FunctionDocumentation::ReturnedValue returned_value_welchTTest = {
      "Returns a Tuple with two or four elements (if the optional `confidence_level` is specified): calculated t-statistic, calculated "
      "p-value, and optionally calculated confidence-interval-low and confidence-interval-high.",
      {"Tuple(Float64, Float64)", "Tuple(Float64, Float64, Float64, Float64)"}};
  FunctionDocumentation::Examples examples_welchTTest = {{"Basic Welch's t-test",
                                                          R"(
...
└─────────────────────────────────────────────────────────────────────────────────────┘
  ...
  FunctionDocumentation documentation_welchTTest = {description_welchTTest,   syntax_welchTTest,         arguments_welchTTest,
                                                    parameters_welchTTest,    returned_value_welchTTest, examples_welchTTest,
                                                    introduced_in_welchTTest, category_welchTTest};
