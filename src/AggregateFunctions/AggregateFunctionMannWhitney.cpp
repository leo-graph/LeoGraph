...
struct MannWhitneyData : public StatisticalSample<Float64, Float64> {
  ...
  std::pair<Float64, Float64> getResult(Alternative alternative, bool continuity_correction) {
    ...
    Float64 r1 = 0;
    for (size_t i = 0; i < static_cast<size_t>(n1); ++i) r1 += ranks[i];
    ...
    Float64 sd = 0.0;
    if (std::abs(tie_correction) > std::numeric_limits<Float64>::epsilon()) sd = std::sqrt(tie_correction * n1 * n2 * (n1 + n2 + 1) / 12.0);
    ...
    Float64 z = 0.0;
    if (sd > std::numeric_limits<Float64>::epsilon()) z = (u - meanrank) / sd;

    if (unlikely(!std::isfinite(z))) return {std::numeric_limits<Float64>::quiet_NaN(), std::numeric_limits<Float64>::quiet_NaN()};

    if (alternative == Alternative::TwoSided) z = std::abs(z);
 ...
 private:
  ...
  class ConcatenatedSamples {
   public:
    ConcatenatedSamples(const Sample &first_, const Sample &second_) : first(first_), second(second_) {}

    const Float64 &operator[](size_t ind) const {
      if (ind < first.size()) return first[ind];
...
class AggregateFunctionMannWhitney final : public IAggregateFunctionDataHelper<MannWhitneyData, AggregateFunctionMannWhitney> {
 private:
 ...
 public:
  ...
  String getName() const override { return "mannWhitneyUTest"; }
  ...
  static DataTypePtr createResultType() {
    ...
    Strings names{"u_statistic", "p_value"};
...
AggregateFunctionPtr createAggregateFunctionMannWhitneyUTest(const std::string &name, const DataTypes &argument_types,
...
mannWhitneyUTest[(alternative[, continuity_correction])](sample_data, sample_index)
  ...
  FunctionDocumentation::Parameters parameters = {
      {"alternative",
       "Optional. Alternative hypothesis. 'two-sided' (default): two populations are not stochastically equal. 'greater': values in the "
       "first sample are stochastically greater than those in the second sample. 'less': values in the first sample are stochastically "
       "less than those in the second sample.",
       {"String"}},
      {"continuity_correction",
       "Optional. If not 0 then continuity correction in the normal approximation for the p-value is applied. The default value is 1.",
       {"UInt64"}}};
  FunctionDocumentation::ReturnedValue returned_value = {
      "Returns a tuple with two elements: calculated U-statistic and calculated p-value.", {"Tuple(Float64, Float64)"}};
  FunctionDocumentation::Examples examples = {{"Mann-Whitney U test example",
                                               R"(
