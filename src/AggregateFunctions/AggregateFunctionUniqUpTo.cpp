...
struct AggregateFunctionUniqUpToData {
  ...
  T load(size_t i) const { return unalignedLoad<T>(data_ptr + i * sizeof(T)); }

  void store(size_t i, const T &x) { unalignedStore<T>(data_ptr + i * sizeof(T), x); }
  ...
  void ALWAYS_INLINE insert(T x, UInt8 threshold) {
    ...
    if (count < threshold) store(count, x);
  ...
  void merge(const AggregateFunctionUniqUpToData<T> &rhs, UInt8 threshold) {
    ...
    for (size_t i = 0; i < rhs.count; ++i) insert(rhs.load(i), threshold);
  }

  void write(WriteBuffer &wb, UInt8 threshold) const {
    writeBinary(count, wb);

    /// Write values only if the state is not overflowed. Otherwise, they are not needed, and only the fact that the state is overflowed is
    /// important.
    if (count <= threshold) wb.write(data_ptr, count * sizeof(T));
  }

  void read(ReadBuffer &rb, UInt8 threshold) {
    readBinary(count, rb);

    if (count <= threshold) rb.readStrict(data_ptr, count * sizeof(T));
  }
...
class AggregateFunctionUniqUpToVariadic final
    : public IAggregateFunctionDataHelper<AggregateFunctionUniqUpToData<UInt64>,
                                          AggregateFunctionUniqUpToVariadic<is_exact, argument_is_tuple>> {
 private:
  size_t num_args = 0;
  UInt8 threshold;
 ...
 public:
  AggregateFunctionUniqUpToVariadic(const DataTypes &arguments, const Array &params, UInt8 threshold_)
      : IAggregateFunctionDataHelper<AggregateFunctionUniqUpToData<UInt64>, AggregateFunctionUniqUpToVariadic<is_exact, argument_is_tuple>>(
  ...
  size_t sizeOfData() const override { return sizeof(AggregateFunctionUniqUpToData<UInt64>) + sizeof(UInt64) * threshold; }
...
AggregateFunctionPtr createAggregateFunctionUniqUpTo(const std::string &name, const DataTypes &argument_types, const Array &params,
  ...
  if (!params.empty()) {
    ...
    if (threshold_param > uniq_upto_max_threshold)
      throw Exception(ErrorCodes::ARGUMENT_OUT_OF_BOUND, "Too large parameter for aggregate function {}. Maximum: {}", name,
                      toString(uniq_upto_max_threshold));
  ...
  if (argument_types.size() == 1) {
    ...
    if (res) return res;
    if (which.isDate()) return std::make_shared<AggregateFunctionUniqUpTo<DataTypeDate::FieldType>>(threshold, argument_types, params);
    if (which.isDate32()) return std::make_shared<AggregateFunctionUniqUpTo<DataTypeDate32::FieldType>>(threshold, argument_types, params);
    if (which.isDateTime())
      return std::make_shared<AggregateFunctionUniqUpTo<DataTypeDateTime::FieldType>>(threshold, argument_types, params);
    if (which.isStringOrFixedString()) return std::make_shared<AggregateFunctionUniqUpTo<String>>(threshold, argument_types, params);
    if (which.isUUID()) return std::make_shared<AggregateFunctionUniqUpTo<DataTypeUUID::FieldType>>(threshold, argument_types, params);
    if (which.isTuple()) {
  ...
  if (use_exact_hash_function) return std::make_shared<AggregateFunctionUniqUpToVariadic<true, false>>(argument_types, params, threshold);
