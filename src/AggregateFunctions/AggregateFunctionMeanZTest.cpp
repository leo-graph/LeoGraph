...
class AggregateFunctionMeanZTest final : public IAggregateFunctionDataHelper<Data, AggregateFunctionMeanZTest<Data>> {
 private:
  Float64 pop_var_x;
  Float64 pop_var_y;
 ...
 public:
  AggregateFunctionMeanZTest(const DataTypes &arguments, const Array &params)
    ...
    if (confidence_level <= 0.0 || confidence_level >= 1.0) {
      throw Exception(ErrorCodes::BAD_ARGUMENTS, "Confidence level parameter must be between 0 and 1 in aggregate function {}.",
                      Data::name);
    }
  }
...
struct MeanZTestData : public ZTestMoments<Float64> {
  ...
  std::pair<Float64, Float64> getResult(Float64 pop_var_x, Float64 pop_var_y) const {
    ...
    if (unlikely(!std::isfinite(zstat))) return {std::numeric_limits<Float64>::quiet_NaN(), std::numeric_limits<Float64>::quiet_NaN()};
...
AggregateFunctionPtr createAggregateFunctionMeanZTest(const std::string &name, const DataTypes &argument_types, const Array &parameters,
                                                      const Settings *) {
...
meanZTest(population_variance_x, population_variance_y, confidence_level)(sample_data, sample_index)
  ...
  FunctionDocumentation::ReturnedValue returned_value = {
      "Returns a tuple with four elements: calculated z-statistic, calculated p-value, calculated confidence-interval-low, calculated "
      "confidence-interval-high.",
      {"Tuple(Float64, Float64, Float64, Float64)"}};
  FunctionDocumentation::Examples examples = {{"Mean Z-test example",
                                               R"(
