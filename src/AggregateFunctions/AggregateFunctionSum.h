...
struct AggregateFunctionSumData {
  ...
  void NO_SANITIZE_UNDEFINED ALWAYS_INLINE add(T value) { Impl::add(sum, toAccumulateResult(value)); }

  /// Vectorized version
  MULTITARGET_FUNCTION_X86_V4_V3(MULTITARGET_FUNCTION_HEADER(template <typename Value> void NO_SANITIZE_UNDEFINED NO_INLINE), addManyImpl,
                                 MULTITARGET_FUNCTION_BODY((const Value *__restrict ptr, size_t start, size_t end)  /// NOLINT
                                                           {
                                                             ptr += start;
                                                             ...
                                                             if constexpr (is_floating_point<T>) {
                                                               ...
                                                               /// Something around the number of SSE registers * the number of elements fit
                                                               /// in register.
                                                               ...
                                                               for (size_t i = 0; i < unroll_count; ++i) Impl::add(sum, partial_sums[i]);
                                                             }

                                                             /// clang cannot vectorize the loop if accumulator is class member instead of
                                                             /// local variable.
      ...
      MULTITARGET_FUNCTION_BODY((const Value *__restrict ptr, const UInt8 *__restrict condition_map, size_t start, size_t end)  /// NOLINT
                                {
                                  ...
                                  if constexpr ((is_integer<T> || is_decimal<T>) && !is_over_big_int<T>) {
                                    /// For integers we can vectorize the operation if we replace the null check using a multiplication (by
                                    /// 0 for null, 1 for not null) https://quick-bench.com/q/MLTnfTvwC2qZFVeWHfOBR3U7a8I
                                    AccumulateResult local_sum{};
                                    while (ptr < end_ptr) {
                                    ...
                                    /// For floating point we use a similar trick as above, except that now we reinterpret the floating
                                    /// point number as an unsigned integer of the same size and use a mask instead (0 to discard, 0xFF..FF
                                    /// to keep)
                                  ...
                                  AccumulateResult local_sum{};
                                  while (ptr < end_ptr) {
                                    if (!*condition_map == add_if_zero) Impl::add(local_sum, toAccumulateResult(static_cast<T>(*ptr)));
                                    ++ptr;
                                    ++condition_map;
                                  }
                                  Impl::add(sum, local_sum);
  ...
  void write(WriteBuffer &buf) const { writeBinaryLittleEndian(sum, buf); }

  void read(ReadBuffer &buf) { readBinaryLittleEndian(sum, buf); }
...
struct AggregateFunctionSumKahanData {
  static_assert(is_floating_point<T>, "It doesn't make sense to use Kahan Summation algorithm for non floating point types");
  ...
  void NO_INLINE addMany(const Value *__restrict ptr, size_t start, size_t end) {
    ...
    while (ptr < unrolled_end) {
      for (size_t i = 0; i < unroll_count; ++i) addImpl(ptr[i], partial_sums[i], partial_compensations[i]);
      ptr += unroll_count;
    }

    for (size_t i = 0; i < unroll_count; ++i) mergeImpl(sum, compensation, partial_sums[i], partial_compensations[i]);
  ...
  void NO_INLINE addManyConditionalInternal(const Value *__restrict ptr, const UInt8 *__restrict condition_map, size_t start, size_t end) {
    ...
    while (ptr < unrolled_end) {
      for (size_t i = 0; i < unroll_count; ++i)
        if ((!condition_map[i]) == add_if_zero) addImpl(ptr[i], partial_sums[i], partial_compensations[i]);
      ptr += unroll_count;
      condition_map += unroll_count;
    }
    ...
    while (ptr < end_ptr) {
      if ((!*condition_map) == add_if_zero) addImpl(*ptr, sum, compensation);
      ++ptr;
      ++condition_map;
    }
  }
 ...
 public:
  ...
  AggregateFunctionSum(const IDataType &data_type, const DataTypes &argument_types_)
      : IAggregateFunctionDataHelper<Data, AggregateFunctionSum<T, TResult, Data, Type>>(argument_types_, {},
                                                                                         createResultType(getDecimalScale(data_type))) {}
  ...
  void addBatchSparse(size_t row_begin, size_t row_end, AggregateDataPtr *places, size_t place_offset, const IColumn **columns,
    ...
    for (size_t i = from; i < to; ++i)
      if (places[offsets[i]]) add(places[offsets[i]] + place_offset, &values, i + 1, arena);
  }
  ...
  bool isCompilable() const override {
    if constexpr (Type == AggregateFunctionTypeSumKahan) return false;
  ...
  void compileAdd(llvm::IRBuilderBase &builder, llvm::Value *aggregate_data_ptr, const ValuesWithType &arguments) const override {
    ...
    auto *sum_result_value =
        sum_value->getType()->isIntegerTy() ? b.CreateAdd(sum_value, value_cast_to_result) : b.CreateFAdd(sum_value, value_cast_to_result);
  ...
  void compileMerge(llvm::IRBuilderBase &builder, llvm::Value *aggregate_data_dst_ptr, llvm::Value *aggregate_data_src_ptr) const override {
    ...
    auto *sum_return_value =
        sum_value_dst->getType()->isIntegerTy() ? b.CreateAdd(sum_value_dst, sum_value_src) : b.CreateFAdd(sum_value_dst, sum_value_src);
