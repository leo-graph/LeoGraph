...
class AggregationFunctionDeltaSumTimestamp final
    : public IAggregateFunctionDataHelper<AggregationFunctionDeltaSumTimestampData<ValueType, TimestampType>,
 ...
 public:
  AggregationFunctionDeltaSumTimestamp(const DataTypes &arguments, const Array &params)
      : IAggregateFunctionDataHelper<AggregationFunctionDeltaSumTimestampData<ValueType, TimestampType>,
                                     AggregationFunctionDeltaSumTimestamp<ValueType, TimestampType>>{arguments, params,
                                                                                                     createResultType()} {}
  ...
  void NO_SANITIZE_UNDEFINED ALWAYS_INLINE add(AggregateDataPtr __restrict place, const IColumn **columns, size_t row_num,
  ...
  bool ALWAYS_INLINE before(const AggregationFunctionDeltaSumTimestampData<ValueType, TimestampType> &lhs,
                            const AggregationFunctionDeltaSumTimestampData<ValueType, TimestampType> &rhs) const {
    if (lhs.last_ts < rhs.first_ts) return true;
    if (lhs.last_ts == rhs.first_ts && (lhs.last_ts < rhs.last_ts || lhs.first_ts < rhs.first_ts)) return true;
    return false;
  }
  ...
  void NO_SANITIZE_UNDEFINED merge(AggregateDataPtr __restrict place, ConstAggregateDataPtr rhs, Arena *) const override {
    ...
    if (!place_data.seen && rhs_data.seen) {
      ...
      if (rhs_data.first > place_data.last) place_data.sum += (rhs_data.first - place_data.last);
      place_data.sum += rhs_data.sum;
      place_data.last = rhs_data.last;
      place_data.last_ts = rhs_data.last_ts;
      ...
      if (place_data.first > rhs_data.last) place_data.sum += (place_data.first - rhs_data.last);
      place_data.sum += rhs_data.sum;
      place_data.first = rhs_data.first;
      place_data.first_ts = rhs_data.first_ts;
    } else {
...
AggregateFunctionPtr createAggregateFunctionDeltaSumTimestamp(const String &name, const DataTypes &arguments, const Array &params,
                                                              const Settings *) {
  ...
  if (!isInteger(arguments[0]) && !isFloat(arguments[0]) && !isDate(arguments[0]) && !isDateTime(arguments[0]))
    throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT,
                    ...
                    "must be Int, Float, Date, DateTime",
  ...
  if (!isInteger(arguments[1]) && !isFloat(arguments[1]) && !isDate(arguments[1]) && !isDateTime(arguments[1]))
    throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT,
                    "Illegal type {} of argument for aggregate function {}, "
                    "must be Int, Float, Date, DateTime",
                    arguments[1]->getName(), name);

  auto res =
      AggregateFunctionPtr(createWithTwoTypes<AggregationFunctionDeltaSumTimestamp>(*arguments[0], *arguments[1], arguments, params));
...
To calculate the delta sum across an ordered collection you can simply use the [`deltaSum`](/sql-reference/aggregate-functions/reference/deltasum) function.
  ...
  FunctionDocumentation::ReturnedValue returned_value = {
      "Returns accumulated differences between consecutive values, ordered by the `timestamp` parameter.",
      {"(U)Int*", "Float*", "Date", "DateTime"}};
  FunctionDocumentation::Examples examples = {{"Basic usage with timestamp ordering",
                                               R"(
