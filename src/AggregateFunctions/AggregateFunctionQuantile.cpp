...
struct QuantileReservoirSampler {
  ...
  void getManyFloat(const Float64 *levels, const size_t *indices, size_t size, Float64 *result) {
    for (size_t i = 0; i < size; ++i) result[indices[i]] = data.quantileInterpolated(levels[indices[i]]);
  }
};

template <typename Value, bool float_return>
using FuncQuantile = AggregateFunctionQuantile<Value, QuantileReservoirSampler<Value>, NameQuantile, void,
                                               std::conditional_t<float_return, Float64, void>, false, false>;
template <typename Value, bool float_return>
using FuncQuantiles = AggregateFunctionQuantile<Value, QuantileReservoirSampler<Value>, NameQuantiles, void,
                                                std::conditional_t<float_return, Float64, void>, true, false>;
...
quantile(level)(expr)
  ...
  FunctionDocumentation::Examples examples = {{"Computing quantile",
                                               R"(
...
quantiles(level1, level2, ...)(expr)
  ...
  FunctionDocumentation::Examples examples_quantiles = {{"Computing multiple quantiles efficiently",
                                                         R"(
CREATE TABLE t (val UInt32) ENGINE = Memory;
