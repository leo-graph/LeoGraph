...
/// The `quantile` call takes O(sample_count log sample_count), if after the previous call `quantile` there was at least one call to insert.
...
class ReservoirSamplerDeterministic {
 private:
  bool good(UInt32 hash) const { return (hash & skip_mask) == 0; }
 ...
 public:
  ...
  double quantileInterpolated(double level) {
    if (samples.empty()) return onEmpty<double>();
    ...
    if (right_index == samples.size()) return static_cast<double>(samples[left_index].first);
  ...
  void merge(const ReservoirSamplerDeterministic& b) {
    ...
    if (skip_degree < b.skip_degree) setSkipDegree(b.skip_degree);

    for (const auto& sample : b.samples) insertImpl(sample.first, sample.second);
  ...
  void read(DB::ReadBuffer& buf) {
    ...
    samples.resize(size);
    for (size_t i = 0; i < size; ++i) readBinaryLittleEndian(samples[i], buf);
...
#pragma clang diagnostic pop
 ...
 private:
  ...
  void setSkipDegree(UInt8 skip_degree_) {
    if (skip_degree_ == skip_degree) return;
