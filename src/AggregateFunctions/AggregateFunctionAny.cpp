...
class AggregateFunctionAny final : public IAggregateFunctionDataHelper<Data, AggregateFunctionAny<Data>> {
 ...
 public:
  explicit AggregateFunctionAny(const DataTypes &argument_types_)
    ...
    if constexpr (!std::is_same_v<Data, SingleValueReference>) serialization = this->result_type->getDefaultSerialization();
  }
  ...
  AggregateFunctionPtr getAggregateFunctionForMergingFinal() const override {
    ...
    if (which.isString() || which.isArray() || which.isMap() || which.isObject() || which.isVariant() || which.isDynamic() ||
        (type_fixed_string && type_fixed_string->getN() > 20)) {
  ...
  void add(AggregateDataPtr __restrict place, const IColumn **columns, size_t row_num, Arena *arena) const override {
    if (!this->data(place).has()) this->data(place).set(*columns[0], row_num, arena);
  }
  ...
  void addBatchSinglePlace(size_t row_begin, size_t row_end, AggregateDataPtr __restrict place, const IColumn **__restrict columns,
    ...
    if (this->data(place).has() || row_begin >= row_end) return;
  ...
  void addManyDefaults(AggregateDataPtr __restrict place, const IColumn **columns, size_t, Arena *arena) const override {
    if (!this->data(place).has()) this->data(place).set(*columns[0], 0, arena);
  }

  void merge(AggregateDataPtr __restrict place, ConstAggregateDataPtr rhs, Arena *arena) const override {
    if (!this->data(place).has()) this->data(place).set(this->data(rhs), arena);
  }
...
:::
  ...
  FunctionDocumentation::Arguments arguments = {{"column", "The column name.", {"Any"}}};
...
However, if the function is used with the `RESPECT NULLS` modifier, it returns the last value reads no matter if NULL or not.
  ...
  FunctionDocumentation::Arguments anyLast_arguments = {{"column", "The column name.", {"Any"}}};
  FunctionDocumentation::ReturnedValue anyLast_returned_value = {"Returns the last value encountered.", {"Any"}};
  FunctionDocumentation::Examples anyLast_examples = {{"Usage example",
                                                       R"(
CREATE TABLE tab(city Nullable(String)) ENGINE=Memory;
...
└───────────────┴───────────────────────────┘
  ...
  FunctionDocumentation anyLast_documentation = {anyLast_description,    anyLast_syntax,   anyLast_arguments,     {},
                                                 anyLast_returned_value, anyLast_examples, anyLast_introduced_in, anyLast_category};

  factory.registerFunction("anyLast", {createAggregateFunctionAnyLast, anyLast_documentation, default_properties},
                           AggregateFunctionFactory::Case::Sensitive);
