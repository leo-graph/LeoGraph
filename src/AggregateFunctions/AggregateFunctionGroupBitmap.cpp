...
class AggregateFunctionBitmap final : public IAggregateFunctionDataHelper<Data, AggregateFunctionBitmap<T, Data>> {
 public:
  ...
  void insertResultInto(AggregateDataPtr __restrict place, IColumn &to, Arena *) const override {
    assert_cast<ColumnVector<T> &>(to).getData().push_back(static_cast<T>(this->data(place).roaring_bitmap_with_small_set.size()));
  }
};
...
class AggregateFunctionBitmapL2 final : public IAggregateFunctionDataHelper<Data, AggregateFunctionBitmapL2<T, Data, Policy>> {
 ...
 public:
  ...
  DataTypePtr getStateType() const override { return this->argument_types.at(0); }
  ...
  void serialize(ConstAggregateDataPtr __restrict place, WriteBuffer &buf, std::optional<size_t> version) const override {
    ...
    if (*version >= 1) DB::writeBoolText(this->data(place).init, buf);
  ...
  void deserialize(AggregateDataPtr __restrict place, ReadBuffer &buf, std::optional<size_t> version, Arena *) const override {
    ...
    if (*version >= 1) DB::readBoolText(this->data(place).init, buf);
    this->data(place).roaring_bitmap_with_small_set.read(buf);
  }
...
AggregateFunctionPtr createAggregateFunctionBitmapL2(const std::string &name, const DataTypes &argument_types, const Array &parameters,
  ...
  AggregateFunctionPtr res(
      createWithIntegerType<AggregateFunctionTemplate, AggregateFunctionGroupBitmapData>(*nested_argument_type_ptr, argument_type_ptr));
...
groupBitmapState(expr)
  ...
  FunctionDocumentation::Arguments arguments = {{"expr", "Expression that results in a `UInt*` type.", {"UInt*"}}};
  FunctionDocumentation::Parameters parameters = {};
  FunctionDocumentation::ReturnedValue returned_value = {"Returns the count of type UInt64 type, or a bitmap object when using `-State`.",
                                                         {"UInt64"}};
...
groupBitmapAndState(expr)
  ...
  FunctionDocumentation::ReturnedValue returned_value_and = {"Returns a count of type `UInt64`, or a bitmap object when using `-State`.",
                                                             {"UInt64"}};
  FunctionDocumentation::Examples examples_and = {{"Usage example",
                                                   R"(
...
└──────────────────────────────────────────────────┘
  ...
  FunctionDocumentation documentation_and = {description_and,    syntax_and,   arguments_and,     parameters_and,
                                             returned_value_and, examples_and, introduced_in_and, category_and};
...
groupBitmapOrState(expr)
  ...
  FunctionDocumentation::ReturnedValue returned_value_or = {"Returns a count of type `UInt64`, or a bitmap object when using `-State`.",
                                                            {"UInt64"}};
  FunctionDocumentation::Examples examples_or = {{"Usage example",
                                                  R"(
CREATE TABLE bitmap_column_expr_test2
(
    tag_id String,
...
└─────────────────────────────────────────────────┘
  ...
  FunctionDocumentation documentation_or = {description_or,    syntax_or,   arguments_or,     parameters_or,
                                            returned_value_or, examples_or, introduced_in_or, category_or};
...
groupBitmapXorState(expr)
  ...
  FunctionDocumentation::ReturnedValue returned_value_xor = {"Returns a count of type `UInt64`, or a bitmap object when using `-State`.",
                                                             {"UInt64"}};
  FunctionDocumentation::Examples examples_xor = {{"Usage example",
                                                   R"(
CREATE TABLE bitmap_column_expr_test2
(
    tag_id String,
...
└──────────────────────────────────────────────────┘
  ...
  FunctionDocumentation documentation_xor = {description_xor,    syntax_xor,   arguments_xor,     parameters_xor,
                                             returned_value_xor, examples_xor, introduced_in_xor, category_xor};
