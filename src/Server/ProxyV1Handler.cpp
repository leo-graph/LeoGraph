...
void ProxyV1Handler::run() {
  ...
  if (!readWord(7, word, eol)) throw Exception(ErrorCodes::CANNOT_PARSE_INPUT_ASSERTION_FAILED, "PROXY protocol violation");
  ...
  if (word == "UNKNOWN" && eol) return;

  if (eol) throw Exception(ErrorCodes::CANNOT_PARSE_INPUT_ASSERTION_FAILED, "PROXY protocol violation");

  // read address
  if (!readWord(39, word, eol) || eol) throw Exception(ErrorCodes::CANNOT_PARSE_INPUT_ASSERTION_FAILED, "PROXY protocol violation");
  ...
  // read port
  if (!readWord(5, word, eol) || eol) throw Exception(ErrorCodes::CANNOT_PARSE_INPUT_ASSERTION_FAILED, "PROXY protocol violation");
  ...
  if (!stack_data.forwarded_for.empty()) LOG_TRACE(log, "Forwarded client address from PROXY header: {}", stack_data.forwarded_for);
}
...
bool ProxyV1Handler::readWord(int max_len, std::string &word, bool &eol) {
  ...
  try {
    ...
    throw NetException(ErrorCodes::SOCKET_TIMEOUT, "Timeout exceeded while reading from socket ({}, {} ms)",
                       socket().peerAddress().toString(), socket().getReceiveTimeout().totalMilliseconds());
  } catch (const Poco::IOException &e) {
    throw NetException(ErrorCodes::NETWORK_ERROR, "{}, while reading from socket ({})", e.displayText(), socket().peerAddress().toString());
  }

  if (n < 0) throw NetException(ErrorCodes::CANNOT_READ_FROM_SOCKET, "Cannot read from socket ({})", socket().peerAddress().toString());
