...
std::unique_ptr<HTTPRequestHandler> KeeperHTTPRequestHandlerFactory::createRequestHandler(const HTTPServerRequest &request) {
  ...
  for (auto &handler_factory : child_factories) {
    if (auto handler = handler_factory->createRequestHandler(request)) return handler;
  }
...
void addDashboardHandlersToFactory(KeeperHTTPRequestHandlerFactory &factory, std::shared_ptr<KeeperDispatcher> keeper_dispatcher) {
...
void addReadinessHandlerToFactory(KeeperHTTPRequestHandlerFactory &factory, std::shared_ptr<KeeperDispatcher> keeper_dispatcher,
...
void addStorageHandlersToFactory(KeeperHTTPRequestHandlerFactory &factory, std::shared_ptr<KeeperDispatcher> keeper_dispatcher,
                                 std::shared_ptr<KeeperHTTPClient> keeper_client) {
...
std::shared_ptr<KeeperHTTPClient> createKeeperClient(const IServer &server, std::shared_ptr<KeeperDispatcher> keeper_dispatcher) {
  ...
  auto client_factory = [keeper_dispatcher, session_timeout]() -> std::shared_ptr<zkutil::ZooKeeper> {
    return zkutil::ZooKeeper::createFromImpl([keeper_dispatcher, session_timeout]() {
...
void addDefaultHandlersToFactory(KeeperHTTPRequestHandlerFactory &factory, const IServer &server,
                                 std::shared_ptr<KeeperDispatcher> keeper_dispatcher, const Poco::Util::AbstractConfiguration &config) {
...
static auto createHandlersFactoryFromConfig(const IServer &server, std::shared_ptr<KeeperDispatcher> keeper_dispatcher,
                                            const Poco::Util::AbstractConfiguration &config, const std::string &name,
                                            const String &prefix) {
...
void KeeperHTTPReadinessHandler::handleRequest(HTTPServerRequest & /*request*/, HTTPServerResponse &response,
  ...
  try {
    ...
    if (!status) response.setStatusAndReason(Poco::Net::HTTPResponse::HTTP_SERVICE_UNAVAILABLE);
...
void KeeperHTTPCommandsHandler::handleRequest(HTTPServerRequest &request, HTTPServerResponse &response,
...
HTTPRequestHandlerFactoryPtr createKeeperHTTPHandlerFactory(const IServer &server, const Poco::Util::AbstractConfiguration &config,
                                                            std::shared_ptr<KeeperDispatcher> keeper_dispatcher, const std::string &name) {
