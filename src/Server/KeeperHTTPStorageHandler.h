...
class KeeperHTTPClient {
 public:
  ...
  explicit KeeperHTTPClient(ClientFactory factory_) : factory(std::move(factory_)), log(getLogger("KeeperHTTPClient")) {}
...
class KeeperHTTPStorageHandler : public HTTPRequestHandler {
 public:
  KeeperHTTPStorageHandler(std::shared_ptr<KeeperHTTPClient> keeper_client_, size_t max_request_size_);

  void handleRequest(HTTPServerRequest& request, HTTPServerResponse& response, const ProfileEvents::Event& write_event) override;

 private:
  static std::optional<Coordination::OpNum> getOperationFromRequest(const HTTPServerRequest& request, HTTPServerResponse& response);

  void performZooKeeperRequest(Coordination::OpNum opnum, const std::string& storage_path, HTTPServerRequest& request,
