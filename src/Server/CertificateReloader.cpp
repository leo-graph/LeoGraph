...
int setCertificateCallback(SSL *ssl, const CertificateReloader::Data *current_data, LoggerPtr log) {
  if (current_data->certs_chain.empty()) return -1;
...
void CertificateReloader::tryLoadACMECertificate(SSL_CTX *ctx, const std::string &prefix) {
  try {
    ...
    if (current_version && current_version->hash == key_certificate_pair->version) return;

    LOG_DEBUG(log, "Reloading ACME certificate and key.");
    it->data.set(std::make_unique<const Data>(std::move(key_certificate_pair->private_key), std::move(key_certificate_pair->certificate),
...
void CertificateReloader::tryReloadAll(const Poco::Util::AbstractConfiguration &config) {
  std::lock_guard lock{data_mutex};
  for (auto &item : data_index) tryLoadImpl(config, item.second->ctx, item.first);
}
