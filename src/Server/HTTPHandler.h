...
class HTTPHandler : public HTTPRequestHandler {
 public:
  HTTPHandler(IServer &server_, const HTTPHandlerConnectionConfig &connection_config_, const std::string &name,
 ...
 private:
  struct Output {
    ...
    bool hasDelayed() const { return out_maybe_delayed_and_compressed && out_maybe_delayed_and_compressed != out_maybe_compressed; }
  ...
  void processQuery(HTTPServerRequest &request, HTMLForm &params, HTTPServerResponse &response, Output &used_output,
                    CurrentThread::QueryScope &query_scope, const ProfileEvents::Event &write_event);

  bool trySendExceptionToClient(int exception_code, const std::string &message, HTTPServerRequest &request, HTTPServerResponse &response,
                                Output &used_output);
 ...
 protected:
  ...
  virtual bool authenticateUser(HTTPServerRequest &request, HTMLForm &params, HTTPServerResponse &response);
};
...
class DynamicQueryHandler : public HTTPHandler {
 ...
 public:
  explicit DynamicQueryHandler(IServer &server_, const HTTPHandlerConnectionConfig &connection_config,
...
class PredefinedQueryHandler : public HTTPHandler {
 ...
 public:
  PredefinedQueryHandler(IServer &server_, const HTTPHandlerConnectionConfig &connection_config, const NameSet &receive_params_,
                         const std::string &predefined_query_, const CompiledRegexPtr &url_regex_,
