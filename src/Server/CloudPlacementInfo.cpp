...
namespace {
std::string getConfigPath(std::string_view path) { return fmt::format("{}.{}", PLACEMENT_CONFIG_PREFIX, path); }
...
String loadAvailabilityZoneFromFile(const Poco::Util::AbstractConfiguration& config) {
  ...
  if (!std::filesystem::exists(az_file)) return "";
...
String loadRegionFromFile(const Poco::Util::AbstractConfiguration& config) {
  ...
  if (!std::filesystem::exists(region_file)) return "";
...
void PlacementInfo::initialize(const Poco::Util::AbstractConfiguration& config) try {
  ...
  if (use_imds) {
    ...
    if (availability_zone.empty()) {
      ...
      if (availability_zone.empty()) LOG_WARNING(log, "Availability zone info not found");
    }

    if (region.empty()) region = loadRegionFromFile(config);
  }
...
std::string PlacementInfo::getAvailabilityZone() const {
  if (!initialized) throw Exception(ErrorCodes::LOGICAL_ERROR, "Placement info has not been loaded");
