...
bool ClientEmbeddedRunner::hasStarted() const { return started.test(); }

bool ClientEmbeddedRunner::hasFinished() const { return finished.test(); }

int ClientEmbeddedRunner::getExitCode() const { return exit_code; }
...
void ClientEmbeddedRunner::closeStdIn() const { client_descriptors->closeStdIn(); }

void ClientEmbeddedRunner::run(const NameToNameMap &envs, const String &starting_query) {
  if (started.test_and_set()) throw Exception(ErrorCodes::LOGICAL_ERROR, "Client has been already started");
...
void ClientEmbeddedRunner::changeWindowSize(int width, int height, int width_pixels, int height_pixels) {
  ...
  if (pty_descriptors == nullptr) throw Exception(ErrorCodes::LOGICAL_ERROR, "Accessing window change on non pty descriptors");
