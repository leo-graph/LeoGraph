...
ClientEmbedded::ClientEmbedded(std::unique_ptr<Session>&& session_, int in_fd_, int out_fd_, int err_fd_, std::istream& input_stream_,
...
void ClientEmbedded::connect() {
  ...
  connection = LocalConnection::createConnection(connection_parameters, std::move(session), std_in.get(), need_render_progress,
...
bool ClientEmbedded::isEmbeeddedClient() const { return true; }

int ClientEmbedded::run(const NameToNameMap& envVars, const String& first_query) try {
  ...
  if (!first_query.empty()) queries.push_back(first_query);
