...
HTTPHandlerConnectionConfig::HTTPHandlerConnectionConfig(const Poco::Util::AbstractConfiguration &config,
                                                         const std::string &config_prefix) {
  if (config.has(config_prefix + ".handler.password"))
    throw Exception(ErrorCodes::INVALID_CONFIG_PARAMETER, "{}.handler.password will be ignored. Please remove it from config.",
                    config_prefix);
  if (config.has(config_prefix + ".handler.user")) credentials.emplace(config.getString(config_prefix + ".handler.user", "default"));
}

HTTPHandler::HTTPHandler(IServer &server_, const HTTPHandlerConnectionConfig &connection_config_, const std::string &name,
...
void HTTPHandler::processQuery(HTTPServerRequest &request, HTMLForm &params, HTTPServerResponse &response, Output &used_output,
                               CurrentThread::QueryScope &query_scope, const ProfileEvents::Event &write_event) {
  using namespace Poco::Net;

  LOG_TRACE(log, "Request URI: {}", request.getURI());

  if (!authenticateUser(request, params, response)) return;  // '401 Unauthorized' response with 'Negotiate' has been sent at this point.
  ...
  if (params.getParsed<bool>("close_session", false) && server.config().getBool("enable_http_close_session", true)) close_session = true;

  if (session_is_set) {
    session_id = params.get("session_id");
    if (session_id.empty()) throw Exception(ErrorCodes::SESSION_ID_EMPTY, "Session id query parameter was provided, but it was empty");
  ...
  if (!roles.empty()) context->setCurrentRoles(roles);

  std::string database = request.get("X-ClickHouse-Database", params.get("database", ""));
  if (!database.empty()) context->setCurrentDatabase(database);

  std::string default_format = request.get("X-ClickHouse-Format", params.get("default_format", ""));
  if (!default_format.empty()) context->setDefaultFormat(default_format);
  ...
  auto param_could_be_skipped = [&](const String &name) {
    ...
    static const NameSet reserved_param_names{"compress",      "decompress",
                                              ...
                                              "close_session", "database",
                                              "default_format"};

    if (reserved_param_names.contains(name)) return true;

    if (has_external_data) {
      /// For external data we have unspecified parameters which literally are {'<temp_table_name>_format', '<temp_table_name>_types',
      /// '<temp_table_name>_structure'}. That parameters are not supposed to be used in the query as a settings. They have to be skipped.
      ...
      for (const String &suffix : reserved_param_suffixes) {
        if (endsWith(name, suffix)) return (!context->getAccessControl().isSettingNameAllowed(name));
      }
    }
  ...
  for (const auto &[key, value] : params) {
    if (!param_could_be_skipped(key)) {
      ...
      if (!customizeQueryParam(context, key, value)) settings_changes.setSetting(key, value);
    }
  }
  ...
  if (!params.has("http_response_buffer_size")) buffer_size_http = params.getParsedLast<size_t>("buffer_size", buffer_size_http);

  bool wait_end_of_query = settings[Setting::http_wait_end_of_query];
  /// setting overrides deprecated wait_end_of_query parameter
  if (!params.has("http_wait_end_of_query")) wait_end_of_query = params.getParsedLast<bool>("wait_end_of_query", wait_end_of_query);
  ...
  if (client_supports_http_compression && enable_http_compression) {
    ...
    used_output.wrap_compressed_holder =
        wrapWriteBufferWithCompressionMethod(used_output.out.get(), http_response_compression_method,
                                             static_cast<int>(http_zlib_compression_level), 0, DBMS_DEFAULT_BUFFER_SIZE, nullptr, 0, false);
  ...
  if (buffer_size_http > 0 || wait_end_of_query) {
    ...
    if (buffer_size_http > 0) cascade_buffers.emplace_back(std::make_shared<MemoryWriteBuffer>(buffer_size_http));
    ...
    if (wait_end_of_query) {
      ...
      auto push_memory_buffer_and_continue = [next_buffer = used_output.out](const WriteBufferPtr &prev_buf) {
        ...
        if (!prev_memory_buffer) throw Exception(ErrorCodes::LOGICAL_ERROR, "Expected MemoryWriteBuffer");
  ...
  auto in_post = wrapReadBufferWithCompressionMethod(wrapReadBufferPointer(request.getStream()),
  ...
  if (params.getParsedLast<bool>("decompress", false)) {
    in_post_maybe_compressed =
        std::make_unique<CompressedReadBuffer>(std::move(in_post), /* allow_different_codecs_ = */ false, /* external_data_ = */ true);
  ...
  if (settings[Setting::readonly] > 0 && settings[Setting::cancel_http_readonly_queries_on_client_close]) {
    append_callback([&context, &request](const Progress &) {
      ...
      if (!request.checkPeerConnected()) context->killCurrentQuery();
    });
  }
  ...
  auto set_query_result = [&response, this](const QueryResultDetails &details) {
    ...
    if (details.format) response.add("X-ClickHouse-Format", *details.format);

    if (details.timezone) response.add("X-ClickHouse-Timezone", *details.timezone);

    if (details.query_cache_entry_created_at)
      response.add("Age", std::to_string(std::chrono::duration_cast<std::chrono::seconds>(std::chrono::system_clock::now() -
                                                                                          *details.query_cache_entry_created_at)
                                             .count()));
    ...
    for (const auto &[name, value] : details.additional_headers) response.set(name, value);
  };

  auto handle_exception_in_output_format = [&, session_id, close_session](IOutputFormat &current_output_format, const String &format_name,
    ...
    if (settings[Setting::http_write_exception_in_output_format] && current_output_format.supportsWritingException()) {
      ...
      if (wait_end_of_query) {
        ...
        if (used_output.out_holder->isCanceled()) return;
  ...
  executeQuery(std::move(in), *used_output.out_maybe_delayed_and_compressed, context, set_query_result, QueryFlags{}, {},
               handle_exception_in_output_format, query_finish_callback, http_continue_callback);
}

bool HTTPHandler::trySendExceptionToClient(int exception_code, const std::string &message, HTTPServerRequest &request,
                                           HTTPServerResponse &response, Output &used_output) try {
  ...
  if (used_output.hasDelayed()) {
    ...
    for (auto &wb : write_buffers)
      if (isSharedPtrUnique(wb)) wb->cancel();
...
void HTTPHandler::handleRequest(HTTPServerRequest &request, HTTPServerResponse &response, const ProfileEvents::Event &write_event) {
  ...
  SCOPE_EXIT({
    ...
    if (thread_trace_context) thread_trace_context->root_span.addAttribute("clickhouse.http_status", response.getStatus());
  });
  ...
  try {
    ...
    if (!request.get("Origin", "").empty()) tryAddHTTPOptionHeadersFromConfig(response, server.config());

    /// For keep-alive to work.
    if (request.getVersion() == HTTPServerRequest::HTTP_1_1) response.setChunkedTransferEncoding(true);

    HTMLForm params(default_settings, request);

    if (params.getParsed<bool>("stacktrace", false) && server.config().getBool("enable_http_stacktrace", true)) with_stacktrace = true;
    ...
    if (thread_trace_context) thread_trace_context->root_span.addAttribute(status);
  }
}
...
DynamicQueryHandler::DynamicQueryHandler(IServer &server_, const HTTPHandlerConnectionConfig &connection_config,
                                         const std::string &param_name_, const HTTPResponseHeaderSetup &http_response_headers_override_)
    : HTTPHandler(server_, connection_config, "DynamicQueryHandler", http_response_headers_override_), param_name(param_name_) {}

bool DynamicQueryHandler::customizeQueryParam(ContextMutablePtr context, const std::string &key, const std::string &value) {
  if (key == param_name) return true;  /// do nothing
  ...
  if (startsWith(key, QUERY_PARAMETER_NAME_PREFIX)) {
    ...
    if (!context->getQueryParameters().contains(parameter_name)) context->setQueryParameter(parameter_name, value);
    return true;
  }
...
PredefinedQueryHandler::PredefinedQueryHandler(IServer &server_, const HTTPHandlerConnectionConfig &connection_config,
                                               const NameSet &receive_params_, const std::string &predefined_query_,
...
bool PredefinedQueryHandler::customizeQueryParam(ContextMutablePtr context, const std::string &key, const std::string &value) {
  ...
  if (startsWith(key, QUERY_PARAMETER_NAME_PREFIX)) {
    ...
    if (receive_params.contains(parameter_name)) context->setQueryParameter(parameter_name, value);
    return true;
  }
  ...
  const auto &set_query_params = [&](const char *begin, const char *end, const CompiledRegexPtr &compiled_regex) {
    ...
    if (compiled_regex->Match(input, 0, end - begin, re2::RE2::Anchor::ANCHOR_BOTH, matches.data(), num_captures)) {
      for (const auto &[capturing_name, capturing_index] : compiled_regex->NamedCapturingGroups()) {
        ...
        if (capturing_value.data()) context->setQueryParameter(capturing_name, String(capturing_value.data(), capturing_value.size()));
      }
    }
  };
...
HTTPRequestHandlerFactoryPtr createDynamicHandlerFactory(IServer &server, const Poco::Util::AbstractConfiguration &config,
                                                         const std::string &config_prefix,
...
HTTPRequestHandlerFactoryPtr createPredefinedHandlerFactory(IServer &server, const Poco::Util::AbstractConfiguration &config,
                                                            const std::string &config_prefix,
                                                            std::unordered_map<String, String> &common_headers) {
  ...
  for (const auto &header_name : headers_name) {
    ...
    if (!startsWith(expression, "regex:")) continue;

    expression = expression.substr(6);
    auto regex = getCompiledRegex(expression);
    if (capturingNamedQueryParam(analyze_receive_params, regex)) headers_name_with_regex.emplace(std::make_pair(header_name, regex));
  }
  ...
  if (config.has(config_prefix + ".url")) {
    ...
    if (startsWith(url_expression, "regex:")) url_expression = url_expression.substr(6);

    auto regex = getCompiledRegex(url_expression);
    if (capturingNamedQueryParam(analyze_receive_params, regex)) {
      auto creator = [&server, analyze_receive_params, predefined_query, regex, headers_name_with_regex, http_response_headers_override,
                      connection_config] -> std::unique_ptr<PredefinedQueryHandler> {
        return std::make_unique<PredefinedQueryHandler>(server, connection_config, analyze_receive_params, predefined_query, regex,
  ...
  auto creator = [&server, analyze_receive_params, predefined_query, headers_name_with_regex, http_response_headers_override,
                  connection_config] -> std::unique_ptr<PredefinedQueryHandler> {
    return std::make_unique<PredefinedQueryHandler>(server, connection_config, analyze_receive_params, predefined_query, CompiledRegexPtr{},
                                                    headers_name_with_regex, http_response_headers_override);
  };
  factory = std::make_shared<HandlingRuleHTTPHandlerFactory<PredefinedQueryHandler>>(std::move(creator));
  factory->addFiltersFromConfig(config, config_prefix);
...
void HTTPHandler::Output::pushDelayedResults() const {
  ...
  if (!cascade_buffer) throw Exception(ErrorCodes::LOGICAL_ERROR, "Expected CascadeWriteBuffer");
...
void HTTPHandler::Output::finalize() {
  ...
  if (hasDelayed()) pushDelayedResults();

  if (out_delayed_and_compressed_holder) out_delayed_and_compressed_holder->finalize();
  if (out_compressed_holder) out_compressed_holder->finalize();
  if (wrap_compressed_holder) wrap_compressed_holder->finalize();
  if (out_holder) out_holder->finalize();
}
...
void HTTPHandler::Output::cancel() {
  ...
  if (out_delayed_and_compressed_holder) out_delayed_and_compressed_holder->cancel();
  if (out_compressed_holder) out_compressed_holder->cancel();
  if (wrap_compressed_holder) wrap_compressed_holder->cancel();
  if (out_holder) out_holder->cancel();
}
}  // namespace DB
