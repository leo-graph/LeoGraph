...
#  if defined(POCO_HAVE_FD_EPOLL)
    ...
    if (epollfd < 0) throw ErrnoException(ErrorCodes::SYSTEM_ERROR, "Cannot epoll_create");
...
#  endif
  ...
  PollResult poll(Poco::Timespan remaining_time, const ReadBufferFromPocoSocket &in) {
    ...
    if (in.available() != 0) socket_ready = true;

    if (response_in.available() != 0) fd_ready = true;
...
#  if defined(POCO_HAVE_FD_EPOLL)
      ...
      for (int i = 0; i < rc; ++i) {
        ...
        if (evout[i].data.fd == pipe.fds_rw[0]) fd_ready = true;
      }
#  else
      pollfd poll_buf[2];
      ...
      if (rc >= 1) {
        if (poll_buf[0].revents & POLLIN) socket_ready = true;
        if (poll_buf[1].revents & POLLIN) fd_ready = true;
      }
#  endif
    }
...
KeeperTCPHandler::KeeperTCPHandler(const Poco::Util::AbstractConfiguration &config_ref,
                                   std::shared_ptr<KeeperDispatcher> keeper_dispatcher_, Poco::Timespan receive_timeout_,
                                   Poco::Timespan send_timeout_, const Poco::Net::StreamSocket &socket_)
...
void KeeperTCPHandler::run() { runImpl(); }
...
Poco::Timespan KeeperTCPHandler::receiveHandshake(int32_t handshake_length, bool &use_compression) {
  ...
  if (last_zxid_seen > last_processed_zxid) {
    throw Exception(
        ...
        last_zxid_seen, last_processed_zxid);
  }
  ...
  int8_t readonly;
  if (handshake_length == Coordination::CLIENT_HANDSHAKE_LENGTH_WITH_READONLY) Coordination::read(readonly, *in);
...
void KeeperTCPHandler::runImpl() {
  ...
  if (in->eof()) {
    LOG_WARNING(log, "Client has not sent any data. peer address = {}  address = {}", socket().peerAddress().toString(),
  ...
  auto response_callback = [my_responses = this->responses, my_poll_wrapper = this->poll_wrapper](
    ...
    if (request) ZooKeeperOpentelemetrySpans::maybeInitialize(request->spans.send_response, request->tracing_context);
  ...
  auto log_long_operation = [&](const String &operation) {
    ...
    if (operation_max_ms < elapsed_ms) LOG_INFO(log, "{} for session {} took {} ms", operation, session_id, elapsed_ms);
  ...
  try {
    while (true) {
      ...
      if (result.error) throw Exception(ErrorCodes::SYSTEM_ERROR, "Exception happened while reading from socket");
...
void KeeperTCPHandler::finalizeWriteBuffer() {
  if (compressed_out) compressed_out->finalize();
  out->finalize();
}
...
std::pair<Coordination::OpNum, Coordination::XID> KeeperTCPHandler::receiveRequest() {
  ...
  auto get_read_buffer_with_limit = [&]() -> ReadBuffer & {
    if (max_request_size == 0) return getReadBuffer();
    limited_buffer_holder.emplace(
        getReadBuffer(),
        LimitReadBuffer::Settings{.read_no_more = max_request_size, .expect_eof = false, .excetion_hint = "too long packet."});
  ...
  auto request_validator = [&](const Coordination::ZooKeeperRequest &current_request) {
    if (!keeper_context->isOperationSupported(current_request.getOpNum()))
      throw Exception(ErrorCodes::BAD_ARGUMENTS, "Unsupported operation: {}, Path: '{}'", current_request.getOpNum(),
                      current_request.getPath());
  };
  ...
  if (expect_opentelemetry_tracing_context) {
    ...
    if (has_tracing_context) {
      ...
      ZooKeeperOpentelemetrySpans::maybeFinalize(request->spans.receive_request, [&] {
        return std::vector<OpenTelemetry::SpanAttribute>{
            {"keeper.operation", Coordination::opNumToString(request->getOpNum())},
            {"keeper.session_id", session_id},
            {"keeper.xid", request->xid},
  ...
  if (keeper_context->shouldLogRequests()) LOG_TRACE(log, "Received request:\n{}", request->toString(/*short_format=*/true));
...
void KeeperTCPHandler::updateStats(Coordination::ZooKeeperResponsePtr &response, const Coordination::ZooKeeperRequestPtr &request) {
  ...
  if (response->xid != Coordination::WATCH_XID && response->getOpNum() != Coordination::OpNum::Heartbeat) {
    ...
    if (request &&
        elapsed_ms > static_cast<Int64>(keeper_context->getCoordinationSettings()[CoordinationSetting::log_slow_total_threshold_ms])) {
      LOG_INFO(log, "Total time to process a request in session {} took too long ({}ms).\nRequest info:\n{}", session_id, elapsed_ms,
               request->toString(/*short_format=*/true));
    }
...
KeeperConnectionStats &KeeperTCPHandler::getConnectionStats() { return conn_stats; }
