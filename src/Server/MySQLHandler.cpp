...
static std::optional<String> setSettingReplacementQuery(const String &query, const String &mysql_setting,
                                                        const String &clickhouse_setting) {
  ...
  if (checkShouldReplaceQuery(query, prefix)) return "SET " + clickhouse_setting + String(query.data() + prefix.length());
  return std::nullopt;
}
...
static String showCollationsReplacementQuery(const String & /*query*/) { return "SELECT 1 LIMIT 0"; }
...
MySQLHandler::MySQLHandler(IServer &server_, TCPServer &tcp_server_, const Poco::Net::StreamSocket &socket_, bool ssl_enabled,
                           bool secure_required_, uint32_t connection_id_, const ProfileEvents::Event &read_event_,
  ...
  if (ssl_enabled) server_capabilities |= CLIENT_SSL;
...
void MySQLHandler::run() {
  ...
  try {
    Handshake handshake(server_capabilities, connection_id, VERSION_STRING + String("-") + VERSION_NAME, auth_plugin->getName(),
                        auth_plugin->getAuthPluginData(), CharacterSet::utf8_general_ci);
    ...
    LOG_TRACE(
        ...
        handshake_response.capability_flags, handshake_response.max_packet_size, static_cast<int>(handshake_response.character_set),
        handshake_response.username, handshake_response.auth_response.length(), handshake_response.database,
        handshake_response.auth_plugin_name);

    if (!(client_capabilities & CLIENT_PROTOCOL_41))
      throw Exception(ErrorCodes::MYSQL_CLIENT_INSUFFICIENT_CAPABILITIES, "Required capability: CLIENT_PROTOCOL_41.");

    if (secure_required && !(client_capabilities & CLIENT_SSL)) throw Exception(ErrorCodes::OPENSSL_ERROR, "SSL connection required.");
    ...
    try {
      ...
      if (!handshake_response.database.empty()) session->sessionContext()->setCurrentDatabase(handshake_response.database);
 ...
 *  If we read it from socket, it will be impossible to start SSL connection using Poco. Size of SSLRequest packet payload is 32 bytes, thus
 * we can read at most 36 bytes.
 */
...
void MySQLHandler::comFieldList(ReadBuffer &payload) {
  ...
  for (const NameAndTypePair &column : metadata_snapshot->getColumns().getAll()) {
    ColumnDefinition column_definition(database, packet.table, packet.table, column.name, column.name, CharacterSet::binary, 100,
                                       ColumnType::MYSQL_TYPE_STRING, 0, 0, true);
    packet_endpoint->sendPacket(column_definition);
  }
  packet_endpoint->sendPacket(OKPacket(0xfe, client_capabilities, 0, 0, 0));
}

void MySQLHandler::comPing() { packet_endpoint->sendPacket(OKPacket(0x0, client_capabilities, 0, 0, 0)); }
...
std::optional<UInt32> MySQLHandler::emplacePreparedStatement(String statement) {
  ...
  if (prepared_statements.contains(statement_id)) {
    LOG_ERROR(log, "Failed to store a new statement `{}` with id {}; it is already taken by `{}`", statement, statement_id,
...
void MySQLHandler::finishHandshakeSSL([[maybe_unused]] size_t packet_size, [[maybe_unused]] char *buf, [[maybe_unused]] size_t pos,
...
#if USE_SSL
MySQLHandlerSSL::MySQLHandlerSSL(IServer &server_, TCPServer &tcp_server_, const Poco::Net::StreamSocket &socket_, bool ssl_enabled,
                                 bool secure_required_, uint32_t connection_id_, KeyPair &private_key_,
...
void MySQLHandlerSSL::authPluginSSL() { auth_plugin = std::make_unique<MySQLProtocol::Authentication::Sha256Password>(private_key, log); }

void MySQLHandlerSSL::finishHandshakeSSL(size_t packet_size, char *buf, size_t pos, std::function<void(size_t)> read_bytes,
