...
MySQLHandlerFactory::MySQLHandlerFactory(IServer& server_, bool secure_required_, const ProfileEvents::Event& read_event_,
...
#if USE_SSL
  return new MySQLHandlerSSL(server, tcp_server, socket, ssl_enabled, secure_required, connection_id, keypair);
#else
