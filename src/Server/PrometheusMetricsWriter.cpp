...
void convertHelpToSingleLine(std::string &help) { std::replace(help.begin(), help.end(), '\n', ' '); }
...
void writeEvent(DB::WriteBuffer &wb, ProfileEvents::Event event) {
  ...
  if (!replaceInvalidChars(metric_name)) return;
...
void writeAsyncMetrics(DB::WriteBuffer &wb, const DB::AsynchronousMetricValues &values) {
  for (const auto &name_value : values) {
    ...
    if (!replaceInvalidChars(key)) continue;
...
void PrometheusMetricsWriter::writeEvents(WriteBuffer &wb) const {
  for (ProfileEvents::Event i = ProfileEvents::Event(0), end = ProfileEvents::end(); i < end; ++i) writeEvent(wb, i);
}

void PrometheusMetricsWriter::writeMetrics(WriteBuffer &wb) const {
  for (size_t i = 0, end = CurrentMetrics::end(); i < end; ++i) writeMetric(wb, i);
}
...
void PrometheusMetricsWriter::writeHistogramMetric(WriteBuffer &wb, const HistogramMetrics::MetricFamily &family) {
  ...
  if (!replaceInvalidChars(base_name)) return;
  ...
  family.forEachMetric(
      [&wb, &family, &base_name](const HistogramMetrics::LabelValues &label_values, const HistogramMetrics::Metric &metric) {
...
void PrometheusMetricsWriter::writeHistogramMetrics(WriteBuffer &wb) const {
  HistogramMetrics::Factory::instance().forEachFamily(
      [&wb](const HistogramMetrics::MetricFamily &family) { writeHistogramMetric(wb, family); });
}
...
void PrometheusMetricsWriter::writeDimensionalMetric(WriteBuffer &wb, const DimensionalMetrics::MetricFamily &family) {
  ...
  family.forEachMetric(
      [&wb, &family, &base_name](const DimensionalMetrics::LabelValues &label_values, const DimensionalMetrics::Metric &metric) {
        wb << base_name;
        const auto &labels = family.getLabels();
        if (!labels.empty()) {
          wb << '{';
...
void PrometheusMetricsWriter::writeDimensionalMetrics(WriteBuffer &wb) const {
  DimensionalMetrics::Factory::instance().forEachFamily(
      [&wb](const DimensionalMetrics::MetricFamily &family) { writeDimensionalMetric(wb, family); });
}
...
#if USE_NURAFT
  for (auto event : ProfileEvents::keeper_profile_events) writeEvent(wb, event);
#endif
}

void KeeperPrometheusMetricsWriter::writeMetrics([[maybe_unused]] WriteBuffer &wb) const {
#if USE_NURAFT
  for (auto metric : CurrentMetrics::keeper_metrics) writeMetric(wb, metric);
#endif
}
