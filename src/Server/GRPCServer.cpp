...
void initGRPCLogging(const Poco::Util::AbstractConfiguration &config) {
  ...
  std::call_once(once_flag, [&config] {
    ...
    if (verbose) grpc_tracer_set_enabled("all", true);
...
#  if USE_SSL
    ...
    if (config.getBool("grpc.ssl_require_client_auth", false)) {
      ...
      if (config.has("grpc.ssl_ca_cert_file")) options.pem_root_certs = readFile(config.getString("grpc.ssl_ca_cert_file"));
    }
  ...
  static std::optional<TransportCompression> fromQueryInfo(const GRPCQueryInfo &query_info) {
    ...
    if (query_info.has_obsolete_result_compression()) {
      switch (query_info.obsolete_result_compression().algorithm()) {
        case GRPCObsoleteTransportCompression::NO_COMPRESSION:
          res.algorithm = GRPC_COMPRESS_NONE;
          break;
        case GRPCObsoleteTransportCompression::DEFLATE:
          res.algorithm = GRPC_COMPRESS_DEFLATE;
          break;
        case GRPCObsoleteTransportCompression::GZIP:
          res.algorithm = GRPC_COMPRESS_GZIP;
          break;
        case GRPCObsoleteTransportCompression::STREAM_GZIP:
          throw Exception(ErrorCodes::INVALID_GRPC_QUERY_INFO,
                          "STREAM_GZIP is no longer supported");  /// was flagged experimental in gRPC, removed as per v1.44
        default:
          throw Exception(
              ErrorCodes::INVALID_GRPC_QUERY_INFO, "Unknown compression algorithm: {}",
              GRPCObsoleteTransportCompression::CompressionAlgorithm_Name(query_info.obsolete_result_compression().algorithm()));
      }

      switch (query_info.obsolete_result_compression().level()) {
        case GRPCObsoleteTransportCompression::COMPRESSION_NONE:
          res.level = GRPC_COMPRESS_LEVEL_NONE;
          break;
        case GRPCObsoleteTransportCompression::COMPRESSION_LOW:
          res.level = GRPC_COMPRESS_LEVEL_LOW;
          break;
        case GRPCObsoleteTransportCompression::COMPRESSION_MEDIUM:
          res.level = GRPC_COMPRESS_LEVEL_MED;
          break;
        case GRPCObsoleteTransportCompression::COMPRESSION_HIGH:
          res.level = GRPC_COMPRESS_LEVEL_HIGH;
          break;
        default:
          throw Exception(ErrorCodes::INVALID_GRPC_QUERY_INFO, "Unknown compression level: {}",
                          GRPCObsoleteTransportCompression::CompressionLevel_Name(query_info.obsolete_result_compression().level()));
      }
      return res;
    }
...
String getQueryDescription(const GRPCQueryInfo &query_info) {
  ...
  if (!query_info.query().empty()) {
    ...
    if (query.length() > max_query_length_to_log) query.remove_suffix(query.length() - max_query_length_to_log);
    ...
    if (query != query_info.query()) str.append("...");
    str.append("\"");
  }
  if (!query_info.query_id().empty()) str.append(str.empty() ? "" : ", ").append("query_id: ").append(query_info.query_id());
...
String getResultDescription(const GRPCResult &result) {
  String str;
  if (!result.output().empty()) str.append("output: ").append(std::to_string(result.output().size())).append(" bytes");
  if (!result.totals().empty()) str.append(str.empty() ? "" : ", ").append("totals");
  if (!result.extremes().empty()) str.append(str.empty() ? "" : ", ").append("extremes");
  if (result.has_progress()) str.append(str.empty() ? "" : ", ").append("progress");
  if (result.logs_size())
    str.append(str.empty() ? "" : ", ").append("logs: ").append(std::to_string(result.logs_size())).append(" entries");
  if (result.cancelled()) str.append(str.empty() ? "" : ", ").append("cancelled");
  if (result.has_exception()) str.append(str.empty() ? "" : ", ").append("exception");
  return str;
}
...
class BaseResponder {
 public:
  ...
  virtual void start(GRPCService &grpc_service, grpc::ServerCompletionQueue &new_call_queue,
                     grpc::ServerCompletionQueue &notification_queue, const CompletionCallback &callback) = 0;
  ...
  std::optional<String> getClientHeader(const String &key) const {
    ...
    if (it != client_metadata.end()) return String{it->second.data(), it->second.size()};
    return std::nullopt;
  }
...
const char *getCallName(CallType call_type) {
  switch (call_type) {
    case CALL_SIMPLE:
      return "ExecuteQuery()";
    case CALL_WITH_STREAM_INPUT:
      return "ExecuteQueryWithStreamInput()";
    case CALL_WITH_STREAM_OUTPUT:
      return "ExecuteQueryWithStreamOutput()";
    case CALL_WITH_STREAM_IO:
      return "ExecuteQueryWithStreamIO()";
    case CALL_MAX:
      break;
  }
  UNREACHABLE();
...
bool isInputStreaming(CallType call_type) { return (call_type == CALL_WITH_STREAM_INPUT) || (call_type == CALL_WITH_STREAM_IO); }

bool isOutputStreaming(CallType call_type) { return (call_type == CALL_WITH_STREAM_OUTPUT) || (call_type == CALL_WITH_STREAM_IO); }
...
class Responder<CALL_SIMPLE> : public BaseResponder {
 public:
  void start(GRPCService &grpc_service, grpc::ServerCompletionQueue &new_call_queue, grpc::ServerCompletionQueue &notification_queue,
    ...
    grpc_service.RequestExecuteQuery(&grpc_context, &query_info.emplace(), &response_writer, &new_call_queue, &notification_queue,
                                     getCallbackPtr(callback));
  }
...
class Responder<CALL_WITH_STREAM_OUTPUT> : public BaseResponder {
 public:
  void start(GRPCService &grpc_service, grpc::ServerCompletionQueue &new_call_queue, grpc::ServerCompletionQueue &notification_queue,
             const CompletionCallback &callback) override {
    grpc_service.RequestExecuteQueryWithStreamOutput(&grpc_context, &query_info.emplace(), &writer, &new_call_queue, &notification_queue,
                                                     getCallbackPtr(callback));
  }
  ...
  void write(const GRPCResult &result, const CompletionCallback &callback) override { writer.Write(result, getCallbackPtr(callback)); }
...
class Responder<CALL_WITH_STREAM_IO> : public BaseResponder {
 public:
  void start(GRPCService &grpc_service, grpc::ServerCompletionQueue &new_call_queue, grpc::ServerCompletionQueue &notification_queue,
             const CompletionCallback &callback) override {
    grpc_service.RequestExecuteQueryWithStreamIO(&grpc_context, &reader_writer, &new_call_queue, &notification_queue,
                                                 getCallbackPtr(callback));
  }
...
std::unique_ptr<BaseResponder> makeResponder(CallType call_type) {
  switch (call_type) {
    case CALL_SIMPLE:
      return std::make_unique<Responder<CALL_SIMPLE>>();
    case CALL_WITH_STREAM_INPUT:
      return std::make_unique<Responder<CALL_WITH_STREAM_INPUT>>();
    case CALL_WITH_STREAM_OUTPUT:
      return std::make_unique<Responder<CALL_WITH_STREAM_OUTPUT>>();
    case CALL_WITH_STREAM_IO:
      return std::make_unique<Responder<CALL_WITH_STREAM_IO>>();
    case CALL_MAX:
      break;
  }
  UNREACHABLE();
...
void Call::receiveQuery() {
  ...
  if (query_info.cancel()) throw Exception(ErrorCodes::INVALID_GRPC_QUERY_INFO, "Initial query info cannot set the 'cancel' field");
...
void Call::executeQuery() {
  ...
  if (traceparent) {
    ...
    if (!client_info.client_trace_context.parseTraceparentHeader(traceparent.value(), error)) {
      throw Exception(ErrorCodes::BAD_REQUEST_PARAMETER, "Failed to parse OpenTelemetry traceparent header '{}': {}", traceparent.value(),
                      error);
    }
  ...
  if (!query_info.session_id().empty()) {
    session->makeSessionContext(query_info.session_id(), getSessionTimeout(query_info, iserver.config()), query_info.session_check());
  }
  ...
  thread_trace_context =
      std::make_unique<OpenTelemetry::TracingContextHolder>("GRPCServer", query_context->getClientInfo().client_trace_context,
                                                            query_context->getSettingsRef(), query_context->getOpenTelemetrySpanLog());
  ...
  if (!query_info.database().empty()) query_context->setCurrentDatabase(query_info.database());
  ...
  ast = parseQuery(parser, begin, end, "", settings[Setting::max_query_size], settings[Setting::max_parser_depth],
  ...
  if (insert_query) {
    ...
    if (input_format.empty()) input_format = "Values";
  }
  ...
  if (output_format.empty()) output_format = query_context->getDefaultFormat();
  ...
  if (input_compression_method_str.empty()) input_compression_method_str = query_info.obsolete_compression_type();
  ...
  if (output_compression_method_str.empty()) output_compression_method_str = query_info.obsolete_compression_type();
  ...
  query_context->setExternalTablesInitializer([this](ContextPtr context) {
    if (context != query_context) throw Exception(ErrorCodes::LOGICAL_ERROR, "Unexpected context in external tables initializer");
  ...
  query_context->setInputInitializer([this](ContextPtr context, const StoragePtr &input_storage) {
    if (context != query_context) throw Exception(ErrorCodes::LOGICAL_ERROR, "Unexpected context in Input initializer");
    input_function_is_used = true;
    initializePipeline(input_storage->getInMemoryMetadataPtr()->getSampleBlock());
  });

  query_context->setInputBlocksReaderCallback([this](ContextPtr context) -> Block {
    if (context != query_context) throw Exception(ErrorCodes::LOGICAL_ERROR, "Unexpected context in InputBlocksReader");
...
void Call::processInput() {
  if (!io.pipeline.pushing()) return;

  bool has_data_to_insert = (insert_query && insert_query->data) || !query_info.input_data().empty() || query_info.next_query_info();
  if (!has_data_to_insert) {
    if (!insert_query) throw Exception(ErrorCodes::NO_DATA_TO_INSERT, "Query requires data to insert, but it is not an INSERT query");
  ...
  while (pipeline_executor->pull(block)) {
    if (!block.empty()) executor.push(block);
  }
...
void Call::initializePipeline(const Block &header) {
  ...
  read_buffer = std::make_unique<ReadBufferFromCallback>([this]() -> std::pair<const void *, size_t> {
    ...
    while (true) {
      ...
      if (!query_info.next_query_info()) break;
  ...
  auto source = query_context->getInputFormat(input_format, *read_buffer, header, settings[Setting::max_insert_block_size], std::nullopt,
                                              settings[Setting::max_insert_block_size_bytes], settings[Setting::min_insert_block_size_rows],
...
void Call::createExternalTables() {
  while (true) {
    for (const auto &external_table : query_info.external_tables()) {
      ...
      if (auto resolved = query_context->tryResolveStorageID(temporary_id, Context::ResolveExternal)) {
        ...
        for (size_t column_idx : collections::range(external_table.columns_size())) {
          ...
          if (column.name.empty()) column.name = "_" + std::to_string(column_idx + 1);
      ...
      if (!external_table.data().empty()) {
        ...
        std::unique_ptr<ReadBuffer> buf =
            std::make_unique<ReadBufferFromMemory>(external_table.data().data(), external_table.data().size());
        ...
        if (format.empty()) format = "TabSeparated";
        ...
        if (!external_table.settings().empty()) {
          ...
          SettingsChanges settings_changes;
          for (const auto &[key, value] : external_table.settings()) settings_changes.push_back({key, value});
        ...
        auto in = external_table_context->getInputFormat(
            format, *buf, metadata_snapshot->getSampleBlock(), settings[Setting::max_insert_block_size], std::nullopt,
            settings[Setting::max_insert_block_size_bytes], settings[Setting::min_insert_block_size_rows],
            settings[Setting::min_insert_block_size_bytes]);
        ...
        cur_pipeline.setSinks([&](const SharedHeader &header, Pipe::StreamType) { return std::make_shared<EmptySink>(header); });
    ...
    readQueryInfo();
    if (!query_info.query().empty() || !query_info.query_id().empty() || !query_info.settings().empty() || !query_info.database().empty() ||
        !query_info.input_data_delimiter().empty() || !query_info.output_format().empty() || !query_info.user_name().empty() ||
        !query_info.password().empty() || !query_info.quota().empty() || !query_info.session_id().empty()) {
      throw Exception(ErrorCodes::INVALID_GRPC_QUERY_INFO,
...
void Call::generateOutput() {
  ...
  if (!io.pipeline.initialized() || io.pipeline.pushing()) return;

  Block header;
  if (io.pipeline.pulling()) header = io.pipeline.getHeader();
  ...
  auto has_output = [&] {
    return (nested_write_buffer->position() != output.data()) || (compressing_write_buffer && compressing_write_buffer->offset());
  };
  ...
  if (!input_function_is_used) check_query_info_contains_cancel_only = true;
  ...
  if (io.pipeline.pulling()) {
    ...
    while (check_for_cancel()) {
      if (!executor->pull(block, interactive_delay / 1000)) break;

      throwIfFailedToSendResult();
      if (!check_for_cancel()) break;

      if (!block.empty() && !io.null_format) output_format_processor->write(materializeBlock(block));
      ...
      if (has_output() || result.has_progress() || result.logs_size()) sendResult();
...
void Call::finishQuery() {
  ...
  LOG_INFO(log, "Finished call {} in {} secs. (including reading by client: {}, writing by client: {})", getCallName(call_type),
           query_time.elapsedSeconds(), static_cast<double>(waited_for_client_reading) / 1000000000ULL,
...
void Call::releaseQueryIDAndSessionID() {
  ...
  if (query_context) query_context->setProcessListElement(nullptr);
  if (session) session->releaseSessionID();
}
  ...
  auto start_reading = [&] {
    ...
    responder->read(next_query_info_while_reading, [this](bool ok) {
      ...
      if (ok) {
        ...
        if (nqi.cancel()) want_to_cancel = true;
      } else {
...
void Call::throwIfFailedToReadQueryInfo() {
  if (failed_to_read_query_info) {
    if (initial_query_info_read) throw Exception(ErrorCodes::NETWORK_ERROR, "Failed to read extra QueryInfo");
...
void Call::addOutputFormatToResult() { *result.mutable_output_format() = output_format; }

void Call::addOutputColumnsNamesAndTypesToResult(const Block &header) {
  if (!send_output_columns_names_and_types) return;
...
void Call::addProgressToResult() {
  ...
  if (!values.read_rows && !values.read_bytes && !values.total_rows_to_read && !values.written_rows && !values.written_bytes) return;
...
void Call::sendResult() {
  ...
  if (!send_final_message && !isOutputStreaming(call_type)) return;

  /// Copy output to `result.output`, with optional compressing.
  if (write_buffer) {
    size_t output_size;
    if (send_final_message) {
      if (compressing_write_buffer) LOG_DEBUG(log, "Compressing final {} bytes", compressing_write_buffer->offset());
  ...
  auto callback = [this](bool ok) {
    /// Called on queue_thread.
    if (!ok) failed_to_send_result = true;
...
void Call::throwIfFailedToSendResult() {
  if (failed_to_send_result) throw Exception(ErrorCodes::NETWORK_ERROR, "Failed to send result to the client");
}
...
void Call::sendException(const Exception &exception) {
  ...
  if (send_exception_with_stacktrace) grpc_exception.set_stack_trace(exception.getStackTraceString());
  sendResult();
}
}  // namespace
...
class GRPCServer::Runner {
 ...
 private:
  void startReceivingNewCalls() {
    ...
    for (CallType call_type : collections::range(CALL_MAX)) makeResponderForNewCall(call_type);
  }
  ...
  void makeResponderForNewCall(CallType call_type) {
    ...
    responders_for_new_calls[call_type]->start(owner.grpc_service, *owner.queue, *owner.queue,
  ...
  void run() {
    ...
    while (owner.queue->Next(&tag, &ok)) {
      ...
      if (should_stop && current_calls.empty()) shutdownQueue();
    }
  ...
  void shutdownQueue() {
    ...
    if (queue_is_shut_down) return;

    queue_is_shut_down = true;

    /// Server should be shut down before CompletionQueue.
    if (owner.grpc_server) owner.grpc_server->Shutdown();

    if (owner.queue) owner.queue->Shutdown();
  }
...
GRPCServer::GRPCServer(IServer &iserver_, const Poco::Net::SocketAddress &address_to_listen_)
    : iserver(iserver_), address_to_listen(address_to_listen_), log(getRawLogger("GRPCServer")), runner(std::make_unique<Runner>(*this)) {}

GRPCServer::~GRPCServer() { runner.reset(); }
...
size_t GRPCServer::currentConnections() const { return runner->getNumCurrentCalls(); }
