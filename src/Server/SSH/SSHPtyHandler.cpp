...
class ChannelCallback {
 public:
  ...
  explicit ChannelCallback(::ssh::SSHChannel &&channel_, std::unique_ptr<Session> &&dbSession_, const SSHPtyHandler::Options &options_)
 ...
 private:
  int ptyRequest(ssh_session, ssh_channel, const char *term, int width, int height, int width_pixels, int height_pixels) noexcept {
    ...
    if (!db_session || client_runner.has_value()) return SSH_ERROR;
    try {
  ...
  void eofFunction(ssh_session, ssh_channel) const noexcept {
    if (!client_runner.has_value()) return;
  ...
  int envRequest(ssh_session, ssh_channel, const char *env_name, const char *env_value) {
    ...
    if (enable_client_options_passing) env[env_name] = env_value;
    return SSH_OK;
  }
...
class SessionCallback {
 public:
  explicit SessionCallback(::ssh::SSHSession &session, IServer &server, const Poco::Net::SocketAddress &address_,
      ...
      : server_context(server.context()), peer_address(address_), log(&Poco::Logger::get("SSHSessionCallback")), options(options_) {
...
SSHPtyHandler::SSHPtyHandler(IServer &server_, ::ssh::SSHSession session_, const Poco::Net::StreamSocket &socket_, const Options &options_)
...
SSHPtyHandler::~SSHPtyHandler() { session.disconnect(); }
...
void SSHPtyHandler::run() {
  ...
  while (!sdata.authenticated || !sdata.channel_callback) {
    ...
    if (sdata.auth_attempts >= options.max_auth_attempts || n >= max_iterations) return;
  ...
  do {
    ...
    if (fds_set || sdata.channel_callback->client_input_output.out == -1) continue;
    ...
    if (sdata.channel_callback->client_input_output.out != -1)
      event.addFd(sdata.channel_callback->client_input_output.out, POLLIN, process_stdout,
                  sdata.channel_callback->channel.getCChannelPtr());

    if (sdata.channel_callback->client_input_output.err != -1)
      event.addFd(sdata.channel_callback->client_input_output.err, POLLIN, process_stderr,
                  sdata.channel_callback->channel.getCChannelPtr());

  } while (sdata.channel_callback->channel.isOpen() && !sdata.channel_callback->hasClientFinished() && !server.isCancelled());

  LOG_DEBUG(log, "Finishing connection with state: channel open: {}, embedded client finished: {}, server cancelled: {}",
  ...
  for (n = 0; n < max_iterations && !session.hasFinished(); n++) event.poll(static_cast<int>(options.event_poll_interval_milliseconds));
