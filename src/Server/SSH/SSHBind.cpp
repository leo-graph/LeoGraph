...
String SSHBind::getError() { return String(ssh_get_error(bind.get())); }
...
void SSHBind::setFd(int fd) { ssh_bind_set_fd(bind.get(), fd); }
...
void SSHBind::deleter(BindPtr bind) { ssh_bind_free(bind); }
