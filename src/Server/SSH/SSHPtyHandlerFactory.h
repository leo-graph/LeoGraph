...
class SSHPtyHandlerFactory : public TCPServerConnectionFactory {
 ...
 public:
  explicit SSHPtyHandlerFactory(IServer &server_, const Poco::Util::AbstractConfiguration &config)
    ...
    if (!rsa_key.empty()) ssh_bind.setHostKey(rsa_key);
    if (!ecdsa_key.empty()) ssh_bind.setHostKey(ecdsa_key);
    if (!ed25519_key.empty()) ssh_bind.setHostKey(ed25519_key);
  ...
  Poco::Net::TCPServerConnection *createConnectionImpl(const Poco::Net::StreamSocket &socket, TCPServer &) override {
    ...
    if (duplicated_fd == -1) throw Exception(ErrorCodes::SSH_EXCEPTION, "Failed to duplicate socket file descriptor");
    ...
    return new SSHPtyHandler(server, std::move(session), socket, options);
  }
};
