...
SSHSession::SSHSession() : session(ssh_new()), disconnected(false) {
  if (!session) throw DB::Exception(DB::ErrorCodes::SSH_EXCEPTION, "Failed to create ssh_session");
}
...
SSHSession::SSHSession(SSHSession&& rhs) noexcept { *this = std::move(rhs); }
...
SSHSession::SessionPtr SSHSession::getInternalPtr() const { return session; }

void SSHSession::connect() {
  int rc = ssh_connect(session);
  if (rc != SSH_OK) throw DB::Exception(DB::ErrorCodes::SSH_EXCEPTION, "Failed connecting in ssh session due due to {}", getError());
}
...
void SSHSession::handleKeyExchange() {
  ...
  if (rc != SSH_OK) throw DB::Exception(DB::ErrorCodes::SSH_EXCEPTION, "Failed key exchange for ssh session due to {}", getError());
}
...
String SSHSession::getError() { return String(ssh_get_error(session)); }

bool SSHSession::hasFinished() { return ssh_get_status(session) & (SSH_CLOSED | SSH_CLOSED_ERROR); }
