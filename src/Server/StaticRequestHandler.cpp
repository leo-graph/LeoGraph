...
struct ResponseOutput {
  ...
  explicit ResponseOutput(std::unique_ptr<WriteBufferFromHTTPServerResponse>&& buf) : response_holder(std::move(buf)) {}
  ...
  WriteBuffer* get() const {
    if (compression_holder) return compression_holder.get();
...
static inline ResponseOutput responseWriteBuffer(HTTPServerRequest& request, HTTPServerResponse& response) {
  auto result =
      ResponseOutput(std::make_unique<WriteBufferFromHTTPServerResponse>(response, request.getMethod() == HTTPRequest::HTTP_HEAD));
  ...
  if (http_response_compression_method == CompressionMethod::None) return result;
...
void StaticRequestHandler::handleRequest(HTTPServerRequest& request, HTTPServerResponse& response,
  ...
  if (request.getVersion() == Poco::Net::HTTPServerRequest::HTTP_1_1) response.setChunkedTransferEncoding(true);
  ...
  try {
    ...
    response_output.response_holder->cancelWithException(request, getCurrentExceptionCode(), getCurrentExceptionMessage(false, true),
                                                         response_output.compression_holder.get());
  }
}
...
void StaticRequestHandler::writeResponse(WriteBuffer& out) {
  ...
  if (startsWith(response_expression, file_prefix)) {
    ...
    if (file_name.starts_with('/')) file_name = file_name.substr(1);
...
StaticRequestHandler::StaticRequestHandler(IServer& server_, const String& expression,
                                           const std::unordered_map<String, String>& http_response_headers_override_, int status_)
