...
std::optional<int32_t> getVersionFromRequest(const HTTPServerRequest &request) {
  ...
  const auto version_param = std::ranges::find_if(query_params, [](const auto &param) { return param.first == "version"; });
  if (version_param == query_params.end()) return std::nullopt;
...
std::string getRawBytesFromRequest(HTTPServerRequest &request, const size_t max_request_size) {
  ...
  if (max_request_size > 0) {
    LimitReadBuffer limited_stream(
        *stream,
        LimitReadBuffer::Settings{.read_no_more = max_request_size, .expect_eof = false, .excetion_hint = "request body is too large"});
...
KeeperHTTPStorageHandler::KeeperHTTPStorageHandler(std::shared_ptr<KeeperHTTPClient> keeper_client_, size_t max_request_size_)
    : log(getLogger("KeeperHTTPStorageHandler")), keeper_client(std::move(keeper_client_)), max_request_size(max_request_size_) {}

void KeeperHTTPStorageHandler::performZooKeeperRequest(Coordination::OpNum opnum, const std::string &storage_path,
                                                       HTTPServerRequest &request, HTTPServerResponse &response) const {
  switch (opnum) {
...
void KeeperHTTPStorageHandler::performZooKeeperListRequest(const std::string &storage_path, HTTPServerResponse &response) const {
  ...
  if (setErrorResponseForZKCode(error, response)) return;
...
void KeeperHTTPStorageHandler::performZooKeeperSetRequest(const std::string &storage_path, HTTPServerRequest &request,
  ...
  const auto error =
      keeper_client->get()->trySet(storage_path, getRawBytesFromRequest(request, max_request_size), maybe_request_version.value());
...
void KeeperHTTPStorageHandler::performZooKeeperCreateRequest(const std::string &storage_path, HTTPServerRequest &request,
                                                             HTTPServerResponse &response) const {
  const auto error =
      keeper_client->get()->tryCreate(storage_path, getRawBytesFromRequest(request, max_request_size), zkutil::CreateMode::Persistent);
...
void KeeperHTTPStorageHandler::performZooKeeperRemoveRequest(const std::string &storage_path, const HTTPServerRequest &request,
                                                             HTTPServerResponse &response) const {
  const auto maybe_request_version = getVersionFromRequest(request);
  if (!maybe_request_version.has_value()) {
    response.setStatusAndReason(Poco::Net::HTTPResponse::HTTP_BAD_REQUEST, "Version parameter is not set or invalid.");
...
std::optional<Coordination::OpNum> KeeperHTTPStorageHandler::getOperationFromRequest(const HTTPServerRequest &request,
  ...
  if (method == Poco::Net::HTTPRequest::HTTP_GET) {
    ...
    Poco::URI uri(request.getURI());
    const auto query_params = uri.getQueryParameters();
    const auto children_param = std::ranges::find_if(query_params, [](const auto &param) { return param.first == "children"; });

    if (children_param != query_params.end() && children_param->second == "true") return Coordination::OpNum::List;

    return Coordination::OpNum::Get;
  }
  if (method == Poco::Net::HTTPRequest::HTTP_HEAD) return Coordination::OpNum::Exists;
  if (method == Poco::Net::HTTPRequest::HTTP_POST) return Coordination::OpNum::Create;
  if (method == Poco::Net::HTTPRequest::HTTP_PUT) return Coordination::OpNum::Set;
  if (method == Poco::Net::HTTPRequest::HTTP_DELETE) return Coordination::OpNum::Remove;
...
void KeeperHTTPStorageHandler::handleRequest(HTTPServerRequest &request, HTTPServerResponse &response,
                                             const ProfileEvents::Event & /*write_event*/) try {
  ...
  if (!maybe_opnum.has_value()) return;
  const auto opnum = maybe_opnum.value();

  /// Build storage path from URL segments after /api/v1/storage
  std::string storage_path;
  for (size_t i = uri_segments_prefix_length; i < uri_segments.size(); ++i) storage_path += "/" + uri_segments[i];
  if (storage_path.empty()) storage_path = "/";
