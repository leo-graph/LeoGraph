...
class MySQLHandler : public Poco::Net::TCPServerConnection {
 ...
 public:
  MySQLHandler(IServer &server_, TCPServer &tcp_server_, const Poco::Net::StreamSocket &socket_, bool ssl_enabled, bool secure_required,
               uint32_t connection_id_, const ProfileEvents::Event &read_event_ = ProfileEvents::end(),
 ...
 protected:
  ...
  virtual void finishHandshakeSSL(size_t packet_size, char *buf, size_t pos, std::function<void(size_t)> read_bytes,
...
class MySQLHandlerSSL : public MySQLHandler {
 public:
  MySQLHandlerSSL(IServer &server_, TCPServer &tcp_server_, const Poco::Net::StreamSocket &socket_, bool ssl_enabled, bool secure_required_,
                  uint32_t connection_id_, KeyPair &private_key_, const ProfileEvents::Event &read_event_ = ProfileEvents::end(),
                  const ProfileEvents::Event &write_event_ = ProfileEvents::end());

 private:
  void authPluginSSL() override;

  void finishHandshakeSSL(size_t packet_size, char *buf, size_t pos, std::function<void(size_t)> read_bytes,
