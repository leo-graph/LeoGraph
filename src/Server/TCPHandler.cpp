...
void validateClientInfo(const ClientInfo &session_client_info, const ClientInfo &client_info) {
  ...
  if (client_info.query_kind == ClientInfo::QueryKind::SECONDARY_QUERY) return;

  if (session_client_info.interface != client_info.interface) {
    throw Exception(DB::ErrorCodes::CLIENT_INFO_DOES_NOT_MATCH, "Client info's interface does not match: {} not equal to {}",
                    toString(session_client_info.interface), toString(client_info.interface));
  }

  if (session_client_info.interface == ClientInfo::Interface::TCP) {
    if (session_client_info.client_name != client_info.client_name)
      throw Exception(DB::ErrorCodes::CLIENT_INFO_DOES_NOT_MATCH, "Client info's client_name does not match: {} not equal to {}",
                      session_client_info.client_name, client_info.client_name);
    ...
    if (!session_client_info.clientVersionEquals(client_info, false))
      throw Exception(DB::ErrorCodes::CLIENT_INFO_DOES_NOT_MATCH, "Client info's version does not match: {} not equal to {}",
                      session_client_info.getVersionStr(), client_info.getVersionStr());
...
struct TurnOffBoolSettingTemporary {
  ...
  explicit TurnOffBoolSettingTemporary(bool &setting_) : setting(setting_), prev_val(setting_) {
    if (prev_val) setting = false;
  }

  ~TurnOffBoolSettingTemporary() {
    if (prev_val) setting = true;
  }
};
...
Block convertColumnsToBLOBs(const Block &block, CompressionCodecPtr codec, UInt64 client_revision, const FormatSettings &format_settings) {
  if (block.empty() || !codec || client_revision < DBMS_MIN_REVISON_WITH_PARALLEL_BLOCK_MARSHALLING) return block;
...
TCPHandler::TCPHandler(IServer &server_, TCPServer &tcp_server_, const Poco::Net::StreamSocket &socket_, bool parse_proxy_protocol_,
                       std::string server_display_name_, std::string host_name_, const ProfileEvents::Event &read_event_,
...
TCPHandler::TCPHandler(IServer &server_, TCPServer &tcp_server_, const Poco::Net::StreamSocket &socket_, TCPProtocolStackData &stack_data,
                       std::string server_display_name_, std::string host_name_, const ProfileEvents::Event &read_event_,
                       const ProfileEvents::Event &write_event_)
    : Poco::Net::TCPServerConnection(socket_),
      server(server_),
      ...
      server_display_name(std::move(server_display_name_)),
      host_name(std::move(host_name_)) {
  if (!forwarded_for.empty()) LOG_TRACE(log, "Forwarded client address: {}", forwarded_for);
}
...
void TCPHandler::runImpl() {
  ...
  try {
    ...
    if (!default_database.empty()) DatabaseCatalog::instance().assertDatabaseExists(default_database);

    /// In interserver mode queries are executed without a session context.
    if (!is_interserver_mode) session->makeSessionContext();

    sendHello();

    if (client_tcp_protocol_version >= DBMS_MIN_PROTOCOL_VERSION_WITH_ADDENDUM) receiveAddendum();

    {
      ...
      auto is_chunked = [](const String &chunked_srv_str, const String &chunked_cl_str, const String &direction) {
        ...
        if (optional_srv) return chunked_cl;

        if (chunked_cl != chunked_srv)
          throw NetException(ErrorCodes::NETWORK_ERROR, "Incompatible protocol: {} is {}, client requested {}", direction,
                             chunked_srv ? "chunked" : "notchunked", chunked_cl ? "chunked" : "notchunked");
      ...
      if (out_chunked) out->enableChunked();
      if (in_chunked) in->enableChunked();
    }
    ...
    if (!is_interserver_mode) {
      ...
      if (!default_database.empty()) session->sessionContext()->setCurrentDatabase(default_database);
    }
  ...
  while (tcp_server.isOpen()) {
    ...
    if (is_interserver_mode) session.reset();
    ...
    try {
      ...
      thread_trace_context = std::make_unique<OpenTelemetry::TracingContextHolder>(
          "TCPHandler", query_state->query_context->getClientInfo().client_trace_context, query_state->query_context->getSettingsRef(),
      ...
      query_scope = CurrentThread::QueryScope::create(
          ...
          [tcp_protocol_version = this->client_tcp_protocol_version, out_weak = std::weak_ptr<WriteBufferFromPocoSocketChunked>(this->out),
           query_state_weak = std::weak_ptr<QueryState>(query_state), callback_mutex_weak = std::weak_ptr<std::mutex>(callback_mutex)] {
            auto query_state_ptr = query_state_weak.lock();
            if (!query_state_ptr) return;

            auto out_ptr = out_weak.lock();
            if (!out_ptr) return;

            auto callback_mutex_ptr = callback_mutex_weak.lock();
            if (!callback_mutex_ptr) return;
      ...
      if (!is_interserver_mode) session->checkIfUserIsStillValid();
      ...
      if (query_state->stage == QueryProcessingStage::QueryPlan) {
        ...
        query_state->query_context->setQueryPlanDeserializationCallback([&query_state]() {
          if (!query_state->plan_and_sets) throw Exception(ErrorCodes::INCORRECT_DATA, "Expected query plan packet for QueryPlan stage");
      ...
      query_state->query_context->setInputInitializer([this, &query_state](ContextPtr context, const StoragePtr &input_storage) {
        if (context != query_state->query_context) throw Exception(ErrorCodes::LOGICAL_ERROR, "Unexpected context in Input initializer");
      ...
      query_state->query_context->setInputBlocksReaderCallback([this, &query_state](ContextPtr context) -> Block {
        if (context != query_state->query_context) throw Exception(ErrorCodes::LOGICAL_ERROR, "Unexpected context in InputBlocksReader");

        std::lock_guard lock(*callback_mutex);

        checkIfQueryCanceled(*query_state);

        if (receivePacketsExpectData(*query_state)) return query_state->block_for_input;
      ...
      query_state->query_context->setMergeTreeReadTaskCallback([this, &query_state](
                                                                   ParallelReadRequest request) -> std::optional<ParallelReadResponse> {
        Stopwatch watch;
        ...
        fiu_do_on(FailPoints::parallel_replicas_reading_response_timeout, {
          throw NetException(ErrorCodes::SOCKET_TIMEOUT, "Simulated network error on the first attempt to get a task from the coordinator");
        });
      ...
      query_state->query_context->setBlockMarshallingCallback([this, &query_state](const Block &block) {
        return convertColumnsToBLOBs(block, getCompressionCodec(query_state->query_context->getSettingsRef(), query_state->compression),
                                     client_tcp_protocol_version, getFormatSettings(query_state->query_context));
      });

      query_state->query_context->setInteractiveCancelCallback([this, &query_state]() {
        std::lock_guard lock(*callback_mutex);
        ...
        if (query_state->stop_read_return_partial_result) return true;
...
// or ErrorCodes::LOGICAL_ERROR. This helps catch these errors in tests.
      ...
      if (query_state) query_state->io.onException();
    ...
    if (exception) {
      ...
      if (thread_trace_context) thread_trace_context->root_span.addAttribute(*exception);
      ...
      try {
        ...
        if (!query_state->read_all_data) skipData(*query_state);
      } catch (...) {
        query_state->cancelOut(out);
...
void TCPHandler::logQueryDuration(QueryState &state) {
  if (state.query_duration_already_logged) return;
  ...
  if (elapsed_sec < 1.0 && state.progress.read_rows) return;
  ...
  while (!server.isCancelled()) {
    while (!in->poll(timeout_us)) {
      ...
      if (elapsed > size_t(receive_timeout.totalSeconds())) {
        throw NetException(ErrorCodes::SOCKET_TIMEOUT,
                           "Timeout exceeded while receiving data from client. Waited for {} seconds, timeout is {} seconds.", elapsed,
    ...
    switch (packet_type) {
      ...
      case Protocol::Client::Scalar: {
        ...
        if (empty_block) state.read_all_data = true;
        return !empty_block;
      }
...
void TCPHandler::skipData(QueryState &state) {
  if (in->isCanceled()) return;

  state.skipping_data = true;
  SCOPE_EXIT({ state.skipping_data = false; });

  size_t blocks = 0;
  while (receivePacketsExpectData(state)) ++blocks;
...
AsynchronousInsertQueue::PushResult TCPHandler::processAsyncInsertQuery(QueryState &state, AsynchronousInsertQueue &insert_queue) {
  ...
  Squashing squashing(std::make_shared<const Block>(state.input_header), 0,
                      state.query_context->getSettingsRef()[Setting::async_insert_max_data_size]);
  ...
  Chunk result_chunk = Squashing::squash(squashing.flush(), squashing.getHeader());
  if (!result_chunk) {
  ...
  if (insert_queue && async_insert_enabled && !insert_query.select) {
    ...
    if (result.status == AsynchronousInsertQueue::PushResult::OK) {
      ...
      if (settings[Setting::wait_for_async_insert]) {
        ...
        if (wait_status == std::future_status::deferred) throw Exception(ErrorCodes::LOGICAL_ERROR, "Got future in deferred state");
    ...
    try {
      ...
      while (executor.pull(block, interactive_delay / 1000)) {
...

        {
          ...
          if (!block.empty() && !state.io.null_format) sendData(state, block);
        }
      }
    } catch (...) {
      executor.cancel();
...
void TCPHandler::processTablesStatusRequest() {
  ...
  if (is_interserver_mode) {
    ...
    if (!default_database.empty()) fake_interserver_context->setCurrentDatabase(default_database);
...
void TCPHandler::sendProfileEvents(QueryState &state) {
  if (!state.query_context->getSettingsRef()[Setting::send_profile_events]) return;
  ...
  if (block.rows() != 0) {
    ...
    if (elapsed_milliseconds > 100)
      LOG_DEBUG(log, "Sending profile events block with {} rows, {} bytes took {} milliseconds", block.rows(), block.bytes(),
                elapsed_milliseconds);
  }
}

void TCPHandler::sendSelectProfileEvents(QueryState &state) {
  if (client_tcp_protocol_version < DBMS_MIN_PROTOCOL_VERSION_WITH_INCREMENTAL_PROFILE_EVENTS) return;

  sendProfileEvents(state);
}

void TCPHandler::sendInsertProfileEvents(QueryState &state) {
  if (client_tcp_protocol_version < DBMS_MIN_PROTOCOL_VERSION_WITH_PROFILE_EVENTS_IN_INSERT) return;
  if (query_kind != ClientInfo::QueryKind::INITIAL_QUERY) return;

  sendProfileEvents(state);
}

void TCPHandler::sendTimezone(QueryState &state) {
  if (client_tcp_protocol_version < DBMS_MIN_PROTOCOL_VERSION_WITH_TIMEZONE_UPDATES) return;
...
bool TCPHandler::receiveProxyHeader() {
  ...
  if ('T' == *limit_in.position()) {
    ...
    if (is_tcp6) forwarded_address = "[" + forwarded_address + "]";
    ...
    /// Skip until \r\n
    while (!limit_in.eof() && *limit_in.position() != '\r') ++limit_in.position();
    assertString("\r\n", limit_in);
...
std::string formatHTTPErrorResponseWhenUserIsConnectedToWrongPort(const Poco::Util::AbstractConfiguration &config, bool is_secure) {
  ...
  result +=
      "You can also use the /play endpoint for interactive queries\r\n"
...
void TCPHandler::receiveHello() {
  ...
  if (packet_type != Protocol::Client::Hello) {
    ...
    if (packet_type == 'G' || packet_type == 'P') {
      ...
      throw Exception(ErrorCodes::UNEXPECTED_PACKET_FROM_CLIENT, "Unexpected packet from client (expected Hello, got {})", packet_type);
  }
  ...
  if (!default_db.empty()) default_database = default_db;
  readStringBinary(user, *in);
  readStringBinary(password, *in);

  if (user.empty()) throw Exception(ErrorCodes::UNEXPECTED_PACKET_FROM_CLIENT, "Unexpected packet from client (no user in Hello package)");
  ...
  LOG_DEBUG(log, "Connected {} version {}.{}.{}, revision: {}{}{}.", client_name, client_version_major, client_version_minor,
            client_version_patch, client_tcp_protocol_version, (!default_database.empty() ? ", database: " + default_database : ""),
  ...
  if (is_ssh_based_auth) user.erase(0, std::string_view(EncodedUserInfo::SSH_KEY_AUTHENTICAION_MARKER).size());
  ...
  if (dynamic_cast<Poco::Net::SecureStreamSocketImpl *>(socket().impl())) {
    ...
    if (secure_socket.havePeerCertificate()) {
      try {
        session->authenticate(SSLCertificateCredentials{user, X509Certificate(secure_socket.peerCertificate()).extractAllSubjects()},
                              getClientAddress(client_info), socket().peerAddress());
        return;
      } catch (const Exception &e) {
        if (e.code() != DB::ErrorCodes::AUTHENTICATION_FAILED) throw;
  ...
  if (is_ssh_based_auth) {
    ...
    bool user_supports_ssh_authentication =
        std::find_if(authentication_types.begin(), authentication_types.end(), [](auto authentication_type) {
          return authentication_type == AuthenticationType::SSH_KEY;
        }) != authentication_types.end();

    if (!user_supports_ssh_authentication) throw Exception(ErrorCodes::AUTHENTICATION_FAILED, "Expected authentication with SSH key");
...
void TCPHandler::receiveAddendum() {
  if (client_tcp_protocol_version >= DBMS_MIN_PROTOCOL_VERSION_WITH_QUOTA_KEY) readStringBinary(quota_key, *in);

  if (!is_interserver_mode) session->setQuotaClientKey(quota_key);
...
void TCPHandler::sendHello() {
  ...
  if (client_tcp_protocol_version >= DBMS_MIN_REVISION_WITH_SERVER_TIMEZONE) writeStringBinary(DateLUT::instance().getTimeZone(), *out);
  if (client_tcp_protocol_version >= DBMS_MIN_REVISION_WITH_SERVER_DISPLAY_NAME) writeStringBinary(server_display_name, *out);
  if (client_tcp_protocol_version >= DBMS_MIN_REVISION_WITH_VERSION_PATCH) writeVarUInt(VERSION_PATCH, *out);
  if (client_tcp_protocol_version >= DBMS_MIN_PROTOCOL_VERSION_WITH_CHUNKED_PACKETS) {
  ...
  if (client_tcp_protocol_version >= DBMS_MIN_REVISION_WITH_SERVER_SETTINGS) {
    if (is_interserver_mode || !session->sessionContext()->getSettingsRef()[Setting::apply_settings_from_server])
...
void TCPHandler::processIgnoredPartUUIDs() { readVectorBinary(part_uuids_to_ignore.emplace(), *in); }
...
void TCPHandler::processQuery(std::shared_ptr<QueryState> &state) {
  ...
  if (client_tcp_protocol_version >= DBMS_MIN_REVISION_WITH_CLIENT_INFO) {
    ...
    if (config_ref[ServerSetting::validate_tcp_client_information]) validateClientInfo(session->getClientInfo(), client_info);
  }
  ...
  if (client_tcp_protocol_version >= DBMS_MIN_PROTOCOL_VERSION_WITH_PARAMETERS) passed_params.read(*in, settings_format);
  ...
  if (is_interserver_mode) {
    ...
    if (nonce.has_value()) data += std::to_string(nonce.value());
    data += cluster_secret;
    data += state->query;
    data += state->query_id;
    ...
    if (client_info.initial_user.empty()) {
      ...
      LOG_DEBUG(log, "User (initial, interserver mode): {} (client: {})", client_info.initial_user,
                getClientAddress(client_info).toString());
      ...
      /// address.
      session->authenticate(AlwaysAllowCredentials{client_info.initial_user}, *client_info.initial_address, *client_info.current_address,
                            external_roles);
    }
  ...
  if (is_interserver_mode && !default_database.empty()) state->query_context->setCurrentDatabase(default_database);

  if (state->part_uuids_to_ignore) state->query_context->getIgnoredPartUUIDs()->add(*state->part_uuids_to_ignore);

  std::weak_ptr<QueryState> state_wptr = state;

  state->query_context->setProgressCallback([this, state_wptr](const Progress &value) {
    auto current_state = state_wptr.lock();
    if (!current_state) return;
    this->updateProgress(*current_state, value);
  });
  state->query_context->setFileProgressCallback([this, state_wptr](const FileProgress &value) {
    auto current_state = state_wptr.lock();
    if (!current_state) return;
    this->updateProgress(*current_state, Progress(value));
  });

  state->query_context->setBlockMarshallingCallback([this, &state_wptr](const Block &block) {
    auto current_state = state_wptr.lock();
    if (!current_state) return block;
    return convertColumnsToBLOBs(block, getCompressionCodec(current_state->query_context->getSettingsRef(), current_state->compression),
                                 client_tcp_protocol_version, getFormatSettings(current_state->query_context));
  });
  ...
  if (state->stage == QueryProcessingStage::WithMergeableState &&
      VersionNumber(client_info.connection_client_version_major, client_info.connection_client_version_minor,
                    client_info.connection_client_version_patch) < VersionNumber(25, 10, 0))
...
void TCPHandler::processUnexpectedQuery() {
  ...
  if (client_tcp_protocol_version >= DBMS_MIN_REVISION_WITH_CLIENT_INFO) skip_client_info.read(*in, client_tcp_protocol_version);
  ...
  if (interserver_secret) readStringBinary(skip_hash, *in, 32);
  ...
  if (client_tcp_protocol_version >= DBMS_MIN_PROTOCOL_VERSION_WITH_PARAMETERS) skip_settings.read(*in, settings_format);
...
bool TCPHandler::receiveQueryPlan(QueryState &state) {
  bool unexpected_packet =
      state.stage != QueryProcessingStage::QueryPlan || state.plan_and_sets || !state.query_context || state.read_all_data;
...
bool TCPHandler::processUnexpectedData() {
  ...
  if (last_block_in.compression == Protocol::Compression::Enable)
    maybe_compressed_in = std::make_shared<CompressedReadBuffer>(*in, /* allow_different_codecs */ true,
                                                                 /* external_data */ query_kind != ClientInfo::QueryKind::SECONDARY_QUERY);
  else
...
void TCPHandler::initBlockInput(QueryState &state) {
  if (!state.block_in) {
    ...
    if (state.compression == Protocol::Compression::Enable)
      state.maybe_compressed_in = std::make_shared<CompressedReadBuffer>(
          *in, /* allow_different_codecs */ true, /* external_data */ query_kind != ClientInfo::QueryKind::SECONDARY_QUERY);
    else
    ...
    state.block_in = std::make_unique<NativeReader>(*state.maybe_compressed_in, header, client_tcp_protocol_version,
...
CompressionCodecPtr TCPHandler::getCompressionCodec(const Settings &query_settings, Protocol::Compression compression) {
  ...
  if (method == "ZSTD") level = query_settings[Setting::network_zstd_compression_level];

  if (compression == Protocol::Compression::Enable) {
    CompressionCodecFactory::instance().validateCodec(method, level, !query_settings[Setting::allow_suspicious_codecs],
...
void TCPHandler::initMaybeCompressedOut(QueryState &state) { initMaybeCompressedOut(state, out); }
...
void TCPHandler::initBlockOutput(QueryState &state, const Block &block) {
  if (!state.block_out) {
    ...
    state.block_out = std::make_unique<NativeWriter>(
        *state.maybe_compressed_out, client_tcp_protocol_version, std::make_shared<const Block>(block.cloneEmpty()),
        getFormatSettings(state.query_context), !query_settings[Setting::low_cardinality_allow_in_native_format]);
  }
}

void TCPHandler::initLogsBlockOutput(QueryState &state, const Block &block, std::shared_ptr<WriteBufferFromPocoSocketChunked> out,
                                     UInt32 client_tcp_protocol_version) {
  if (!state.logs_block_out) {
    ...
    state.logs_block_out = std::make_unique<NativeWriter>(
        *logs_buf, client_tcp_protocol_version, std::make_shared<const Block>(block.cloneEmpty()), getFormatSettings(state.query_context),
...
void TCPHandler::initProfileEventsBlockOutput(QueryState &state, const Block &block) {
  if (!state.profile_events_block_out) {
    ...
    state.profile_events_block_out = std::make_unique<NativeWriter>(
        *profile_events_buf, client_tcp_protocol_version, std::make_shared<const Block>(block.cloneEmpty()),
        getFormatSettings(state.query_context), !query_settings[Setting::low_cardinality_allow_in_native_format]);
  }
}
...
void TCPHandler::receivePacketsExpectCancel(QueryState &state) {
  if (after_check_cancelled.elapsed() / 1000 < interactive_delay) return;
  ...
  if (in->poll(0)) {
    try {
      if (in->isCanceled() || in->eof()) throw NetException(ErrorCodes::ABORTED, "Client has dropped the connection, cancel the query.");
...
void TCPHandler::sendData(QueryState &state, const Block &block) {
  ...
  try {
    ...
    if (unknown_packet_in_send_data) {
      ...
      if (unknown_packet_in_send_data == 0) writeVarUInt(marker, *out);
    }
    ...
    if (state.maybe_compressed_out != out) state.maybe_compressed_out->next();
    ...
    if (out->offset() >= extra_bytes_written_out) out->position() -= extra_bytes_written_out;

    throw;
  }
}

void TCPHandler::sendLogData(QueryState &state, const Block &block, std::shared_ptr<WriteBufferFromPocoSocketChunked> out,
                             UInt32 client_tcp_protocol_version) {
...
void TCPHandler::updateProgress(QueryState &state, const Progress &value) { state.progress.incrementPiecewiseAtomically(value); }
...
void TCPHandler::sendLogs(QueryState &state) { TCPHandler::sendLogs(state, out, client_tcp_protocol_version); }
...
bool TCPHandler::connectionLimitReached() {
  ...
  if (limit_reached) {
    ...
    if (max_queries_exceeded) limit_info += fmt::format("queries={}/{}", query_count, max_queries);
    if (max_seconds_exceeded) {
      if (!limit_info.empty()) limit_info += ", ";
...
Poco::Net::SocketAddress TCPHandler::getClientAddress(const ClientInfo &client_info) {
  ...
  if (forwarded_address && server.config().getBool("auth_use_forwarded_address", false)) return *forwarded_address;
