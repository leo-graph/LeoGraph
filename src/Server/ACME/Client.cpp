...
void Client::shutdown() {
  if (refresh_certificates_task) refresh_certificates_task->deactivate();

  if (authentication_task) authentication_task->deactivate();

  if (refresh_key_task) refresh_key_task->deactivate();
}
...
std::optional<VersionedCertificate> Client::requestCertificate() const {
  ...
  return VersionedCertificate{.private_key = KeyPair::fromBuffer(pkey),
...
void Client::initialize(const Poco::Util::AbstractConfiguration& config) {
...

  {
    ...
    if (private_acme_key && api && api->isReady()) return;
  }

  auto http_port = config.getInt("http_port");
  if (http_port != 80) LOG_WARNING(log, "For ACME HTTP challenge HTTP port must be 80, but is {}", http_port);
  ...
  for (const auto& key : domains_keys) served_domains.push_back(config.getString("acme.domains." + key));
  ...
  refresh_certificates_task_interval_ms =
      config.getInt("acme.refresh_certificates_task_interval_seconds", default_refresh_certificates_task_interval) * 1000;
  ...
  for (const auto& domain : domains) zk->createIfNotExists(fs::path(zookeeper_path) / acme_hostname / "domains" / domain, "");

  BackgroundSchedulePool& bgpool = Context::getGlobalContextInstance()->getSchedulePool();

  refresh_certificates_task =
      bgpool.createTask(StorageID::createEmpty(), "ACME::refreshCertificatesTask", [this, &config] { refreshCertificatesTask(config); });
  ...
  {
    std::lock_guard key_lock(private_acme_key_mutex);
    if (!private_acme_key) refresh_key_task->activateAndSchedule();
  }
...
void Client::refreshCertificatesTask(const Poco::Util::AbstractConfiguration& config) {
  ...
  try {
    ...
    if ((!lock || !lock->isLocked()) && zk->exists(active_order_path)) {
      LOG_DEBUG(log, "Certificate order lock {} is active; retrying after {}ms", std::string(active_order_path),
...
void Client::authenticationTask() {
...

  {
    ...
    if (!private_acme_key) throw Exception(ErrorCodes::LOGICAL_ERROR, "Private ACME key is not initialized");
  }
...
void Client::refreshKeyTask() {
  ...
  try {
    ...
    if (private_key.empty()) {
      ...
      auto code =
          zk->tryCreate(fs::path(zookeeper_path) / acme_hostname / "account_private_key", private_key, zkutil::CreateMode::Persistent);
...
std::string Client::requestChallenge(const std::string& uri) {
  ...
  if (!uri.starts_with(CHALLENGE_HTTP_PATH)) return "";
