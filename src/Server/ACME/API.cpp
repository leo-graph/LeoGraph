...
Directory Directory::parse(const std::string &json_data) {
  ...
  LOG_TEST(&Poco::Logger::get("ACME::Directory"), "Directory: newAccount: {}, newOrder: {}, newNonce: {}", dir.new_account.toString(),
           dir.new_order.toString(), dir.new_nonce.toString());
...
std::string readURLUntilEOF(const Poco::URI &url) {
  ...
  auto reader = std::make_unique<ReadBufferFromWebServer>(url.toString(), Context::getGlobalContextInstance(), DBMS_DEFAULT_BUFFER_SIZE,
                                                          read_settings,
...
std::string API::requestNonce() const {
  if (!directory) throw Exception(ErrorCodes::LOGICAL_ERROR, "Directory is not initialized");
...
std::string API::formatJWSRequestData(const Poco::URI &url, const std::string &payload, const std::string &nonce) const {
  ...
  return R"({"protected":")" + protected_enc + R"(","payload":")" + payload_enc + R"(","signature":")" + encoded_signature + R"("})";
}

std::string API::doJWSRequest(const Poco::URI &url, const std::string &payload, std::shared_ptr<Poco::Net::HTTPResponse> response) const {
  ...
  if (!response) response = std::make_shared<Poco::Net::HTTPResponse>();
...
Poco::JSON::Object::Ptr API::doJWSRequestExpectingJSON(const Poco::URI &url, const std::string &payload,
...
std::string API::authenticate() {
  if (!key_id.empty()) return key_id;

  std::string payload;

  if (!configuration.contact_email.empty())
    payload = fmt::format(R"({{"contact":["mailto:{}"],"termsOfServiceAgreed":{}}})", configuration.contact_email,
...
std::string API::order(const Domains &domains, OrderCallback callback) const {
  ...
  for (const auto &auth : *authorizations) processAuthorization(Poco::URI(auth.toString()), callback);
...
Order API::describeOrder(const Poco::URI &order_url) const {
  ...
  if (json->has("certificate")) certificate = json->getValue<std::string>("certificate");
...
std::string API::pullCertificate(const Poco::URI &certificate_url) const { return doJWSRequestWithEmptyPayload(certificate_url); }
