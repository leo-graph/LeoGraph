...
HTTPServerConnection::HTTPServerConnection(HTTPContextPtr context_, TCPServer &tcp_server_, const Poco::Net::StreamSocket &socket,
                                           Poco::Net::HTTPServerParams::Ptr params_, HTTPRequestHandlerFactoryPtr factory_,
...
void HTTPServerConnection::run() {
  ...
  while (!stopped && tcp_server.isOpen() && session.connected()) {
    ...
    if (!session.hasMoreRequests()) {
      ...
      if (!is_first_request) ProfileEvents::increment(ProfileEvents::HTTPServerConnectionsReused);
    }
    ...
    try {
      ...
      if (!stopped && tcp_server.isOpen() && session.connected()) {
        ...
        if (!forwarded_for.empty()) request.set("X-Forwarded-For", forwarded_for);

        if (request.isSecure()) {
          size_t hsts_max_age = context->getMaxHstsAge();

          if (hsts_max_age > 0) response.add("Strict-Transport-Security", "max-age=" + std::to_string(hsts_max_age));
        }
        ...
        try {
          ...
          if (handler) {
            if (!shouldDeferHTTP100Continue(request) && request.getExpectContinue() &&
      ...
      LOG_DEBUG(LogFrequencyLimiter(getLogger("HTTPServerConnection"), 10), "HTTP request failed: {}: {}",
                HTTPResponse::HTTP_REASON_BAD_REQUEST, e.displayText());
      ...
      if (e.code() == POCO_ENOTCONN) {
        LOG_DEBUG(LogFrequencyLimiter(getLogger("HTTPServerConnection"), 10), "Connection reset by peer while processing HTTP request: {}",
                  e.message());
        break;
      }
...
void HTTPServerConnection::sendErrorResponse(Poco::Net::HTTPServerSession &session, Poco::Net::HTTPResponse::HTTPStatus status,
  ...
  if (!message.empty()) out->write(message.data(), message.size());
