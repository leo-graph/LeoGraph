...
void readHeaders(Poco::Net::MessageHeader& headers, ReadBuffer& in, size_t max_fields_number, size_t max_name_length,
  ...
  while (true) {
    if (fields > max_fields_number) throw Poco::Net::MessageException("Too many header fields");
    ...
    if (in.eof()) throw Poco::Net::MessageException("Field is invalid");

    if (name.empty()) {
      if (ch == '\r')
        /// Start of the empty-line delimiter
        break;
      if (ch == ':') throw Poco::Net::MessageException("Field name is empty");
    } else {
      if (name.size() > max_name_length) throw Poco::Net::MessageException("Field name is too long");
      if (ch != ':') throw Poco::Net::MessageException(fmt::format("Field name is invalid or no colon found: \"{}\"", name));
    }
    ...
    while (in.read(ch) && ch != '\r' && ch != '\n' && value.size() <= max_value_length) value += ch;

    if (in.eof()) throw Poco::Net::MessageException("Field is invalid");

    if (ch == '\n') throw Poco::Net::MessageException("No CRLF found");

    if (value.size() > max_value_length) throw Poco::Net::MessageException("Field value is too long");
