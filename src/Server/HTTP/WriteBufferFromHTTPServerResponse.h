...
class WriteBufferFromHTTPServerResponse final : public HTTPWriteBuffer {
 public:
  ...
  WriteBufferFromHTTPServerResponse(HTTPServerResponse& response_, bool is_http_method_head_,
  ...
  void addHeaderCORS(bool enable_cors) { add_cors_header = enable_cors; }
  ...
  void setSendProgressInterval(size_t send_progress_interval_ms_) { send_progress_interval_ms = send_progress_interval_ms_; }

  /// Content-Encoding header will be set on first data package
  void setCompressionMethodHeader(const CompressionMethod& compression_method_) { compression_method = compression_method_; }
  ...
  bool cancelWithException(HTTPServerRequest& request, int exception_code_, const std::string& message,
