...
HTMLForm::HTMLForm(const Settings &settings, const std::string &encoding_) : HTMLForm(settings) { encoding = encoding_; }
...
void HTMLForm::read(ReadBuffer &in) { readQuery(in); }
...
void HTMLForm::readQuery(ReadBuffer &in) {
  ...
  while (true) {
    if (max_fields_number > 0 && fields == max_fields_number) throw Poco::Net::HTMLFormException("Too many form fields");
    ...
    if (is_first) Poco::UTF8::removeBOM(name);
...
void HTMLForm::readMultipart(ReadBuffer &in_, PartHandler &handler) {
  ...
  if (!in.skipToNextBoundary()) throw Poco::Net::HTMLFormException("No boundary line found");

  /// Read each part until next boundary (or last boundary)
  while (!in.eof()) {
    if (max_fields_number && fields > max_fields_number) throw Poco::Net::HTMLFormException("Too many form fields");
    ...
    if (params.has("filename"))
      ...
      while (in.read(ch)) {
        if (value.size() > max_field_value_size) throw Poco::Net::HTMLFormException("Field value too long");
        value += ch;
      }
    ...
    if (!in.skipToNextBoundary()) break;
  }
