...
void drainRequestIfNeeded(HTTPServerRequest& request, HTTPServerResponse& response) noexcept {
  ...
  if (input_stream->isCanceled()) {
    ...
    LOG_WARNING(getLogger("sendExceptionToHTTPClient"),
                "Cannot read remaining request body during exception handling. The request read buffer is canceled. Set keep alive to "
  ...
  if (request.getMethod() == Poco::Net::HTTPRequest::HTTP_POST && (request.getChunkedTransferEncoding() || request.hasContentLength()) &&
    ...
    if (request.getExpectContinue() && response.getStatus() != Poco::Net::HTTPResponse::HTTP_CONTINUE) {
      ...
      try {
        ...
        tryLogCurrentException("sendExceptionToHTTPClient",
                               "Cannot read remaining request body during exception handling. Set keep alive to false on the response.");
        response.setKeepAlive(false);
      }
    }
  }
