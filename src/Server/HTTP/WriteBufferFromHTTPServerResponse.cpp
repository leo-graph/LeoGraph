...
void WriteBufferFromHTTPServerResponse::startSendHeaders() {
  if (headers_started_sending) return;
...
void WriteBufferFromHTTPServerResponse::writeHeaderProgressImpl(const char* header_name, Progress::DisplayMode mode) {
  if (is_http_method_head || headers_finished_sending || !headers_started_sending) return;
...
void WriteBufferFromHTTPServerResponse::writeExceptionCode() {
  if (headers_finished_sending || !exception_code) return;
...
void WriteBufferFromHTTPServerResponse::finishSendHeaders() {
  if (headers_finished_sending) return;
  ...
  if (count() && compression_method != CompressionMethod::None) response.set("Content-Encoding", toContentEncodingName(compression_method));
...
void WriteBufferFromHTTPServerResponse::nextImpl() {
  ...
  if (!is_http_method_head) HTTPWriteBuffer::nextImpl();
}

WriteBufferFromHTTPServerResponse::WriteBufferFromHTTPServerResponse(HTTPServerResponse& response_, bool is_http_method_head_,
                                                                     const ProfileEvents::Event& write_event_)
    : HTTPWriteBuffer(response_.getSocket(), write_event_), response(response_), is_http_method_head(is_http_method_head_) {
  if (response.getChunkedTransferEncoding()) setChunked();
...
void WriteBufferFromHTTPServerResponse::onProgress(const Progress& progress, ContextPtr context) {
  ...
  if (progress.memory_usage) {
    ...
    if (process_list_elem) accumulated_progress.memory_usage = process_list_elem->getInfo().memory_usage;
  }
...
bool WriteBufferFromHTTPServerResponse::cancelWithException(HTTPServerRequest& request, int exception_code_, const std::string& message,
                                                            WriteBuffer* compression_buffer) noexcept {
  bool use_compression_buffer = compression_buffer && !compression_buffer->isCanceled() && !compression_buffer->isFinalized();

  try {
    if (isCanceled()) throw Exception(ErrorCodes::ABORTED, "Write buffer has been canceled.");

    if (isFinalized()) throw Exception(ErrorCodes::CANNOT_WRITE_AFTER_END_OF_BUFFER, "Write buffer has been finalized.");
    ...
    if (use_compression_buffer) {
      ...
      if (!data_sent) compression_discarded_data = compression_buffer->rejectBufferedDataSave();
    }
    data_sent |= (count() != offset());
    if (!data_sent) discarded_data = rejectBufferedDataSave();
    ...
    if (!is_response_sent) {
      ...
      if (use_compression_buffer) compression_buffer->finalize();
      finalize();

      LOG_DEBUG(getLogger("WriteBufferFromHTTPServerResponse"),
                ...
                response.getStatus(), response.getReason(), exception_code_, message, use_compression_buffer, data_sent,
                compression_discarded_data, discarded_data);
    } else {
      ...
      if (use_compression_buffer) compression_buffer->next();
      next();
      ...
      size_t size_message_excluded =
          2 + EXCEPTION_MARKER.size() + 2 + EXCEPTION_TAG_LENGTH + 2 + 8 + 1 + EXCEPTION_TAG_LENGTH + 2 + EXCEPTION_MARKER.size() + 2;
      ...
      if (limited_message.size() > max_exception_message_size) limited_message = limited_message.substr(0, max_exception_message_size);
      ...
      // the aim is to break HTTP
      if (use_compression_buffer) compression_buffer->cancel();
      cancel();
    }
