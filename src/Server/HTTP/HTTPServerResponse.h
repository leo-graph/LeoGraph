...
class HTTPWriteBufferFixedLength : public WriteBufferFromPocoSocket {
 public:
  ...
  explicit HTTPWriteBufferFixedLength(Poco::Net::Socket& socket_, size_t fixed_length_, const ProfileEvents::Event& write_event_,
 ...
 protected:
  void nextImpl() override {
    if (count_length >= fixed_length || offset() == 0) return;

    if (count_length + offset() > fixed_length) pos -= offset() - (fixed_length - count_length);
...
class HTTPWriteBuffer : public WriteBufferFromPocoSocket {
 public:
  ...
  size_t isFixedLength() const { return chunked ? 0 : fixed_length; }

  size_t fixedLengthLeft() const {
    chassert(isFixedLength());

    if (fixed_length <= count_length - offset()) return 0;
  ...
  bool isPlain() const { return !(isChunked() || isFixedLength()); }

 protected:
  void finalizeImpl() override {
    WriteBufferFromPocoSocket::finalizeImpl();

    if (chunked) socketSendBytes("0\r\n\r\n", 5);
  }

  void breakFixedLength() {
    if (fixed_length > 1) fixed_length -= 1;
  }
