...
class HTTPServerRequest : public HTTPRequest {
 public:
  ...
  bool canKeepAlive() const {
    ...
    if (!stream_is_bounded) return false;

    if (stream.use_count() > 1) {
      LOG_ERROR(getLogger("HTTPServerRequest"),
                "Request stream is shared by multiple threads. HTTP keep alive is not possible. Use count {}", stream.use_count());
      return false;
    } else {
