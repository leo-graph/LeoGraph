...
class PostgreSQLHandler : public Poco::Net::TCPServerConnection {
 public:
  PostgreSQLHandler(const Poco::Net::StreamSocket& socket_,
#if USE_SSL
 ...
 private:
  ...
  std::function<void(const Progress&)> createProgressCallback(ContextMutablePtr query_context, std::atomic<UInt64>& result_rows,
                                                              std::atomic<UInt64>& written_rows);

  UInt64 executeQueryWithTracking(String&& sql_query, ContextMutablePtr query_context,
