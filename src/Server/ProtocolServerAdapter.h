...
class ProtocolServerAdapter {
 ...
 public:
  ...
  ProtocolServerAdapter(const std::string& listen_host_, const char* port_name_, const std::string& description_,
                        std::unique_ptr<TCPServer> tcp_server_, bool supports_runtime_reconfiguration_ = true);

#if USE_GRPC
  ProtocolServerAdapter(const std::string& listen_host_, const char* port_name_, const std::string& description_,
                        std::unique_ptr<IGRPCServer> grpc_server_, bool supports_runtime_reconfiguration_ = true);
#endif
