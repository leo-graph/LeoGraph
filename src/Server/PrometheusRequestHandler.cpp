...
class PrometheusRequestHandler::ExposeMetricsImpl : public Impl {
 public:
  ...
  void handleRequest(HTTPServerRequest& /* request */, HTTPServerResponse& response) override {
    ...
    if (config().expose_info) metrics_writer().writeInfo(out);

    if (config().expose_events) metrics_writer().writeEvents(out);

    if (config().expose_metrics) metrics_writer().writeMetrics(out);

    if (config().expose_asynchronous_metrics) metrics_writer().writeAsynchronousMetrics(out, parent().async_metrics);

    if (config().expose_errors) metrics_writer().writeErrors(out);

    if (config().expose_histograms) metrics_writer().writeHistogramMetrics(out);

    if (config().expose_dimensional_metrics) metrics_writer().writeDimensionalMetrics(out);
  }
};
...
class PrometheusRequestHandler::ImplWithContext : public Impl {
 ...
 protected:
  void handleRequest(HTTPServerRequest& request, HTTPServerResponse& response) override {
    ...
    if (context) query_scope = CurrentThread::QueryScope::create(context);
  ...
  bool authenticateUserAndMakeContext(HTTPServerRequest& request, HTTPServerResponse& response) {
    ...
    if (!authenticateUser(request, response)) return false;
  ...
  bool authenticateUser(HTTPServerRequest& request, HTTPServerResponse& response) {
    return authenticateUserByHTTP(request, *params, response, *session, request_credentials, config().connection_config, server().context(),
                                  log());
  }
...
class PrometheusRequestHandler::RemoteWriteImpl : public ImplWithContext {
 public:
  ...
  void handlingRequestWithContext([[maybe_unused]] HTTPServerRequest& request, [[maybe_unused]] HTTPServerResponse& response) override {
    ...
    if (write_request.timeseries_size()) protocol.writeTimeSeries(write_request.timeseries());

    if (write_request.metadata_size()) protocol.writeMetricsMetadata(write_request.metadata());
 ...
 public:
  ...
  void handlingRequestWithContext([[maybe_unused]] HTTPServerRequest& request, [[maybe_unused]] HTTPServerResponse& response) override {
    ...
    for (size_t i = 0; i != num_queries; ++i) {
      ...
      protocol.readTimeSeries(*new_query_result.mutable_timeseries(), query.start_timestamp_ms(), query.end_timestamp_ms(),
                              query.matchers(), query.hints());
    }
...
PrometheusRequestHandler::PrometheusRequestHandler(IServer& server_, const PrometheusRequestHandlerConfig& config_,
...
WriteBufferFromHTTPServerResponse& PrometheusRequestHandler::getOutputStream(HTTPServerResponse& response) {
  if (write_buffer_from_response) return *write_buffer_from_response;

  write_buffer_from_response =
      std::make_unique<WriteBufferFromHTTPServerResponse>(response, http_method == HTTPRequest::HTTP_HEAD, write_event);
