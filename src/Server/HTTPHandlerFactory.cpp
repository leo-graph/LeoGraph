...
HTTPRequestHandlerFactoryPtr createRedirectHandlerFactory(const Poco::Util::AbstractConfiguration &config, const std::string &config_prefix,
...
static void addDefaultHandlersFactory(HTTPRequestHandlerFactoryMain &factory, IServer &server,
                                      const Poco::Util::AbstractConfiguration &config, AsynchronousMetrics &async_metrics);
...
static auto createPingHandlerFactory(IServer &server) {
  auto creator = [&server]() -> std::unique_ptr<StaticRequestHandler> {
    ...
    return std::make_unique<StaticRequestHandler>(server, ping_response_expression, parseHTTPResponseHeaders("text/html; charset=UTF-8"));
  };
...
static auto createPingHandlerFactory(IServer &server, const Poco::Util::AbstractConfiguration &config, const String &config_prefix,
  ...
  auto creator = [&server, &config, config_prefix, common_headers]() -> std::unique_ptr<StaticRequestHandler> {
    ...
    return std::make_unique<StaticRequestHandler>(server, ping_response_expression, headers);
  };
  return std::make_shared<HandlingRuleHTTPHandlerFactory<StaticRequestHandler>>(std::move(creator));
}
...
static inline auto createHandlersFactoryFromConfig(IServer &server, const Poco::Util::AbstractConfiguration &config,
                                                   const std::string &name, const String &prefix, AsynchronousMetrics &async_metrics) {
      ...
      if (handler_type == "static") {
        ...
        if (path != "/js/uplot.js" && path != "/js/lz-string.js") {
          throw Exception(ErrorCodes::INVALID_CONFIG_PARAMETER,
                          ...
                          "Configured path here: {}",
                          path);
        }

        auto handler =
            createWebUIHandlerFactory<JavaScriptWebUIRequestHandler>(server, config, prefix + "." + key, common_headers_override);
        handler->addFiltersFromConfig(config, prefix + "." + key);
        main_handler_factory->addHandler(std::move(handler));
      }
#if USE_SSL
        ...
        throw Exception(ErrorCodes::INVALID_CONFIG_PARAMETER, "Unknown handler type '{}' in config here: {}.{}.handler.type", handler_type,
                        prefix, key);
...
static inline HTTPRequestHandlerFactoryPtr createHTTPHandlerFactory(IServer &server, const Poco::Util::AbstractConfiguration &config,
                                                                    const std::string &name, AsynchronousMetrics &async_metrics,
...
HTTPRequestHandlerFactoryPtr createHandlerFactory(IServer &server, const Poco::Util::AbstractConfiguration &config,
                                                  AsynchronousMetrics &async_metrics, const std::string &name,
  ...
  if (name == "PrometheusHandler-factory") return createPrometheusHandlerFactory(server, config, async_metrics, name);
  if (name == "KeeperPrometheusHandler-factory") return createKeeperPrometheusHandlerFactory(server, config, async_metrics, name);
...
void addCommonDefaultHandlersFactory(HTTPRequestHandlerFactoryMain &factory, IServer &server,
  ...
  if (config.has("http_server_default_response")) {
    auto root_creator = [&server]() -> std::unique_ptr<StaticRequestHandler> {
      ...
      return std::make_unique<StaticRequestHandler>(server, root_response_expression, parseHTTPResponseHeaders("text/html; charset=UTF-8"));
    };
...
void addDefaultHandlersFactory(HTTPRequestHandlerFactoryMain &factory, IServer &server, const Poco::Util::AbstractConfiguration &config,
