...
class HandlingRuleHTTPHandlerFactory : public HTTPRequestHandlerFactory {
 public:
  ...
  explicit HandlingRuleHTTPHandlerFactory(Creator &&creator_) : creator(std::move(creator_)) {}
  ...
  void addFiltersFromConfig(const Poco::Util::AbstractConfiguration &config, const std::string &prefix) {
    ...
    for (const auto &filter_type : filters_type) {
      if (filter_type == "handler") continue;
      /// URI (path and query string)
      if (filter_type == "url") addFilter(urlFilter(config, prefix + ".url"));
  ...
  void allowPostAndGetParamsAndOptionsRequest() {
    addFilter([](const auto &request) {
      return (request.getURI().contains('?') &&
             ...
             request.getMethod() == Poco::Net::HTTPRequest::HTTP_OPTIONS || request.getMethod() == Poco::Net::HTTPRequest::HTTP_POST;
    });
  }
  ...
  void allowRESTMethods() {
    addFilter([](const auto &request) {
      return request.getMethod() == Poco::Net::HTTPRequest::HTTP_GET || request.getMethod() == Poco::Net::HTTPRequest::HTTP_HEAD ||
             request.getMethod() == Poco::Net::HTTPRequest::HTTP_POST || request.getMethod() == Poco::Net::HTTPRequest::HTTP_PUT ||
...
HTTPRequestHandlerFactoryPtr createStaticHandlerFactory(IServer &server, const Poco::Util::AbstractConfiguration &config,
...
HTTPRequestHandlerFactoryPtr createReplicasStatusHandlerFactory(IServer &server, const Poco::Util::AbstractConfiguration &config,
                                                                const std::string &config_prefix,
                                                                std::unordered_map<String, String> &common_headers);
