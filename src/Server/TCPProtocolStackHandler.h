...
class TCPProtocolStackHandler : public Poco::Net::TCPServerConnection {
 ...
 public:
  TCPProtocolStackHandler(IServer& server_, TCPServer& tcp_server_, const StreamSocket& socket,
                          const std::vector<TCPServerConnectionFactory::Ptr>& stack_, const std::string& conf_name_)
  ...
  void run() override {
    ...
    for (auto& factory : stack) {
      ...
      connection->run();
      if (stack_data.socket != socket()) socket() = stack_data.socket;
    }
  }
};
