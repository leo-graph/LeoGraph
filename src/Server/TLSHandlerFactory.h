...
class TLSHandlerFactory : public TCPServerConnectionFactory {
 ...
 public:
  ...
  Poco::Net::TCPServerConnection *createConnectionImpl(const Poco::Net::StreamSocket &socket, TCPServer & /* tcp_server*/,
                                                       TCPProtocolStackData &stack_data) override {
    try {
      LOG_TRACE(log, "TCP Request. Address: {}", socket.peerAddress().toString());
      return new TLSHandler(socket, server.config(), conf_name + ".", stack_data);
