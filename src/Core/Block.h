...
class Block {
 ...
 public:
  ...
  ColumnWithTypeAndName *findByName(const std::string &name, bool case_insensitive = false) {
    return const_cast<ColumnWithTypeAndName *>(const_cast<const Block *>(this)->findByName(name, case_insensitive));
  }
  ...
  ColumnWithTypeAndName &getByName(const std::string &name, bool case_insensitive = false) {
    return const_cast<ColumnWithTypeAndName &>(const_cast<const Block *>(this)->getByName(name, case_insensitive));
  }
...
/// Calculate difference in structure of blocks and write description into output strings. NOTE It doesn't compare values of constant
/// columns.
