...
Range Range::createWholeUniverse() { return Range(NEGATIVE_INFINITY, true, POSITIVE_INFINITY, true); }

Range Range::createWholeUniverseWithoutNull() { return Range(NEGATIVE_INFINITY, false, POSITIVE_INFINITY, false); }
...
Range Range::createRightBounded(const FieldRef& right_point, bool right_included, bool with_null) {
  ...
  if (r.right.isNegativeInfinity() && right_included) r.left_included = true;
  return r;
}
...
Range Range::createLeftBounded(const FieldRef& left_point, bool left_included, bool with_null) {
  ...
  r.shrinkToIncludedIfPossible();
  // Special case for [+Inf, +Inf]
  if (r.left.isPositiveInfinity() && left_included) r.right_included = true;
  return r;
}
...
bool Range::equals(const Field& lhs, const Field& rhs) { return accurateEquals(lhs, rhs); }

bool Range::less(const Field& lhs, const Field& rhs) { return accurateLess(lhs, rhs); }

bool Range::empty() const { return less(right, left) || ((!left_included || !right_included) && !less(left, right)); }

/// x contained in the range
bool Range::contains(const FieldRef& x) const { return !leftThan(x) && !rightThan(x); }

/// x is to the left
bool Range::rightThan(const FieldRef& x) const { return less(left, x) || (left_included && equals(x, left)); }

/// x is to the right
bool Range::leftThan(const FieldRef& x) const { return less(x, right) || (right_included && equals(x, right)); }
...
bool Range::fullBounded() const { return left.getType() != Field::Types::Null && right.getType() != Field::Types::Null; }

/// (-inf, +inf)
bool Range::isInfinite() const { return left.isNegativeInfinity() && right.isPositiveInfinity(); }

bool Range::intersectsRange(const Range& r) const {
  /// r to the left of me.
  if (less(r.right, left) || ((!left_included || !r.right_included) && equals(r.right, left))) return false;

  /// r to the right of me.
  if (less(right, r.left) || ((!right_included || !r.left_included) && equals(r.left, right))) return false;
...
bool Range::containsRange(const Range& r) const {
  ...
  if (less(r.left, left) || (r.left_included && !left_included && equals(r.left, left))) return false;

  /// r ends right of me.
  if (less(right, r.right) || (r.right_included && !right_included && equals(r.right, right))) return false;
...
void Range::invert() {
  std::swap(left, right);
  if (left.isPositiveInfinity()) left = NEGATIVE_INFINITY;
  if (right.isNegativeInfinity()) right = POSITIVE_INFINITY;
...
std::optional<Range> Range::intersectWith(const Range& r) const {
  ...
  if (less(left, r.left) || ((!left_included && r.left_included) && equals(left, r.left))) left_bound_use_mine = false;

  if (less(r.right, right) || ((!r.right_included && right_included) && equals(r.right, right))) right_bound_use_mine = false;
...
std::optional<Range> Range::unionWith(const Range& r) const {
  ...
  if (less(left, r.left) || ((!left_included && r.left_included) && equals(left, r.left))) left_bound_use_mine = true;

  if (less(r.right, right) || ((!r.right_included && right_included) && equals(r.right, right))) right_bound_use_mine = true;
...
bool Range::nearByWith(const Range& r) const {
  /// me locates at left
  if (((right_included && !r.left_included) || (!right_included && r.left_included)) && equals(right, r.left)) return true;

  /// r locate left
  if (((r.right_included && !left_included) || (r.right_included && !left_included)) && equals(r.right, left)) return true;
...
Hyperrectangle intersect(const Hyperrectangle& a, const Hyperrectangle& b) {
  ...
  for (size_t i = 0; i < result_size; ++i) res.push_back(intersect(a[i], b[i]));
