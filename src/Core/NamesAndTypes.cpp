...
NameAndTypePair::NameAndTypePair(const String &name_in_storage_, const String &subcolumn_name_, const DataTypePtr &type_in_storage_,
...
String NameAndTypePair::getNameInStorage() const {
  if (!subcolumn_delimiter_position) return name;
...
bool NameAndTypePair::operator==(const NameAndTypePair &rhs) const { return name == rhs.name && type->equals(*rhs.type); }

String NameAndTypePair::getSubcolumnName() const {
  if (!subcolumn_delimiter_position) return "";
...
void NamesAndTypesList::getDifference(const NamesAndTypesList &rhs, NamesAndTypesList &deleted, NamesAndTypesList &added) const {
  ...
  std::set_difference(lhs_vector.begin(), lhs_vector.end(), rhs_vector.begin(), rhs_vector.end(), std::back_inserter(deleted));
  std::set_difference(rhs_vector.begin(), rhs_vector.end(), lhs_vector.begin(), lhs_vector.end(), std::back_inserter(added));
}

Names NamesAndTypesList::getNames() const {
  Names res;
  res.reserve(size());
  for (const NameAndTypePair &column : *this) res.push_back(column.name);
  return res;
}
...
NameSet NamesAndTypesList::getNameSet() const {
  NameSet res;
  res.reserve(size());
  for (const NameAndTypePair &column : *this) res.insert(column.name);
  return res;
}
...
std::unordered_map<std::string, DataTypePtr> NamesAndTypesList::getNameToTypeMap() const {
  ...
  res.reserve(size());
  for (const NameAndTypePair &column : *this) res.emplace(column.name, column.type);
  return res;
}
...
DataTypes NamesAndTypesList::getTypes() const {
  DataTypes res;
  res.reserve(size());
  for (const NameAndTypePair &column : *this) res.push_back(column.type);
  return res;
}
...
NamesAndTypesList NamesAndTypesList::filter(const NameSet &names) const {
  ...
  for (const NameAndTypePair &column : *this) {
    if (names.contains(column.name)) res.push_back(column);
  }
  return res;
}
...
NamesAndTypesList NamesAndTypesList::addTypes(const Names &names) const {
  ...
  for (const auto &column : *this) types[column.name] = &column.type;

  NamesAndTypesList res;
  for (const String &name : names) {
    const auto *it = types.find(name);
    if (it == types.end()) throw Exception(ErrorCodes::THERE_IS_NO_COLUMN, "No column {}", name);
...
bool NamesAndTypesList::containsCaseInsensitive(const String &name) const {
  for (const NameAndTypePair &column : *this) {
    if (boost::iequals(column.name, name)) return true;
  }
  return false;
}
...
String NamesAndTypesList::toNamesAndTypesDescription() const {
  ...
  for (const auto &name_and_type : *this) {
    if (!std::exchange(first, false)) writeCString(", ", buf);
...
void NamesAndTypesList::readTextWithNamesInStorage(ReadBuffer &buf) {
  ...
  for (size_t i = 0; i < count; ++i) {
    ...
    emplace_back(column_name, name_in_storage, data_type_factory.get(type_in_storage), data_type_factory.get(type_name));
  }
}
