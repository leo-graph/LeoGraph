...
ExternalResultDescription::ExternalResultDescription(const Block &sample_block_) { init(sample_block_); }
...
void ExternalResultDescription::init(const Block &sample_block_) {
  ...
  for (auto &elem : sample_block) {
    ...
    if (elem.column->empty()) elem.column = elem.type->createColumnConstWithDefaultValue(1)->convertToFullColumnIfConst();
