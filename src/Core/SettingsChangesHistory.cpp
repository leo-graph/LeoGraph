...
static void addSettingsChanges(VersionToSettingsChangesMap& settings_changes_history, std::string_view version,
                               SettingsChangesHistory::SettingsChanges&& changes) {
  /// Forbid duplicate versions
  auto [_, inserted] = settings_changes_history.emplace(ClickHouseVersion(version), std::move(changes));
  if (!inserted) throw Exception{ErrorCodes::LOGICAL_ERROR, "Detected duplicate version '{}'", ClickHouseVersion(version).toString()};
}
...
const VersionToSettingsChangesMap& getSettingsChangesHistory() {
  ...
  std::call_once(initialized_flag, [&] {
    // clang-format off
        ...
        {
            {"allow_experimental_gql_dialect", false, false, "A new setting to enable GQL dialect"},
            {"query_language", "clickhouse", "clickhouse", "Added as an alias for the `dialect` setting"},
