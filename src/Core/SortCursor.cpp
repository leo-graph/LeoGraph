...
void SortCursorImpl::reset(const Block& block, IColumnPermutation* perm) {
  if (block.getColumns().empty()) throw Exception(ErrorCodes::LOGICAL_ERROR, "Empty column list in block");
...
#if USE_EMBEDDED_COMPILER
    if (desc.compiled_sort_description) raw_sort_columns_data.emplace_back(getColumnData(sort_columns.back()));
#endif
...
SortQueueVariants::SortQueueVariants(const DataTypes& sort_description_types, const SortDescription& sort_description) {
  ...
  if (sort_description.size() == 1) {
    ...
    if (!sort_description_types[0]->isNullable()) {
      ...
      result = callOnIndexAndDataType<void>(column_type_index, [&](const auto& types) {
