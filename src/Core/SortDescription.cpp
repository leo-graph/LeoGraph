...
void dumpSortDescription(const SortDescription &description, WriteBuffer &out) {
  ...
  for (const auto &desc : description) {
    ...
    if (desc.with_fill) out << " WITH FILL";
  }
}
...
bool SortDescription::hasPrefix(const Names &prefix) const {
  ...
  for (size_t i = 0; i < prefix.size(); ++i) {
    if ((*this)[i].column_name != prefix[i]) return false;
  }
  return true;
}
...
class CompiledSortDescriptionFunctionHolder final : public CompiledExpressionCacheEntry {
 public:
  explicit CompiledSortDescriptionFunctionHolder(CompiledSortDescriptionFunction compiled_function_)
      : CompiledExpressionCacheEntry(compiled_function_.compiled_module.size), compiled_sort_description_function(compiled_function_) {}
...
static std::string getSortDescriptionDump(const SortDescription &description, const DataTypes &header_types) {
  ...
  for (size_t i = 0; i < description.size(); ++i) {
    ...
    buffer << "(type: " << header_types[i]->getName() << ", direction: " << description[i].direction
...
static LoggerPtr getLogger() { return ::getLogger("SortDescription"); }
...
void compileSortDescriptionIfNeeded(SortDescription &description, const DataTypes &sort_description_types, bool increase_compile_attempts) {
  ...
  if (!description.compile_sort_description || sort_description_types.empty()) return;

  for (const auto &type : sort_description_types) {
    if (!type->createColumn()->isComparatorCompilable() || !canBeNativeType(*type)) return;
  }
...
void serializeSortDescription(const SortDescription &sort_description, WriteBuffer &out) {
  ...
  for (const auto &desc : sort_description) {
    ...
    UInt8 flags = 0;
    if (desc.direction > 0) flags |= 1;
    if (desc.nulls_direction > 0) flags |= 2;
    if (desc.collator) flags |= 4;
    if (desc.with_fill) flags |= 8;

    writeIntBinary(flags, out);

    if (desc.collator) writeStringBinary(desc.collator->getLocale(), out);

    if (desc.with_fill) throw Exception(ErrorCodes::NOT_IMPLEMENTED, "WITH FILL is not supported in serialized sort description");
  }
}
...
void deserializeSortDescription(SortDescription &sort_description, ReadBuffer &in) {
  ...
  for (auto &desc : sort_description) {
    ...
    if (flags & 4) {
      ...
      if (!collator_locale.empty()) desc.collator = std::make_shared<Collator>(collator_locale);
    }

    if (flags & 8) throw Exception(ErrorCodes::NOT_IMPLEMENTED, "WITH FILL is not supported in deserialized sort description");
  }
}
