...
/// NOLINTNEXTLINE
#define DECLARE_SETTING_ENUM(ENUM_TYPE) DECLARE_SETTING_ENUM_WITH_RENAME(ENUM_TYPE, ENUM_TYPE)
...
#define IMPLEMENT_SETTING_ENUM_IMPL(NEW_NAME, ERROR_CODE_FOR_UNEXPECTED_NAME, PAIRS_TYPE, ...)                                         \
  const String &SettingField##NEW_NAME##Traits::toString(typename SettingField##NEW_NAME::EnumType value) {                            \
    static const std::unordered_map<EnumType, String> map = [] {                                                                       \
      ...
      for (const auto &[name, val] : PAIRS_TYPE __VA_ARGS__) res.emplace(val, name);                                                   \
      return res;                                                                                                                      \
    }();                                                                                                                               \
    auto it = map.find(value);                                                                                                         \
    if (it != map.end()) return it->second;                                                                                            \
    throw Exception(ERROR_CODE_FOR_UNEXPECTED_NAME, "Unexpected value of " #NEW_NAME ":{}",                                            \
                    std::to_string(std::underlying_type_t<EnumType>(value)));                                                          \
  }                                                                                                                                    \
                                                                                                                                       \
  typename SettingField##NEW_NAME::EnumType SettingField##NEW_NAME##Traits::fromString(std::string_view str) {                         \
    static const std::unordered_map<std::string_view, EnumType> map = [] {                                                             \
      ...
      for (const auto &[name, val] : PAIRS_TYPE __VA_ARGS__) res.emplace(name, val);                                                   \
      return res;                                                                                                                      \
    }();                                                                                                                               \
    ...
    for (auto &name : map | boost::adaptors::map_keys) {                                                                               \
      if (std::exchange(need_comma, true)) msg += ", ";                                                                                \
      msg += "'" + String{name} + "'";                                                                                                 \
    }                                                                                                                                  \
...
/// NOLINTNEXTLINE
#define DECLARE_SETTING_MULTI_ENUM(ENUM_TYPE) DECLARE_SETTING_MULTI_ENUM_WITH_RENAME(ENUM_TYPE, ENUM_TYPE)
...
#define IMPLEMENT_SETTING_MULTI_AUTO_ENUM(NEW_NAME, ERROR_CODE_FOR_UNEXPECTED_NAME) \
  ...
  size_t SettingField##NEW_NAME##Traits::getEnumSize() { return getEnumValues<EnumType>().size(); }
...
enum class QueryResultCacheNondeterministicFunctionHandling : uint8_t { Throw, Save, Ignore };
...
enum class QueryResultCacheSystemTableHandling : uint8_t { Throw, Save, Ignore };
...
enum class MySQLDataTypesSupport : uint8_t {
  ...
  DATETIME64,   // convert MySQL's DATETIME and TIMESTAMP and ClickHouse DateTime64 if precision is > 0 or range is greater that for
                // DateTime.
...
enum class GroupArrayActionWhenLimitReached : uint8_t { THROW, DISCARD };
...
enum class SearchOrphanedPartsDisks : uint8_t { NONE, LOCAL, ANY };
...
enum class ArrowFlightDescriptorType : uint8_t { Path = 0, Command };

DECLARE_SETTING_ENUM(ArrowFlightDescriptorType)

enum class DeduplicateInsertSelectMode : uint8_t { DISABLE = 0, FORCE_ENABLE, ENABLE_WHEN_POSSIBLE, ENABLE_EVEN_FOR_BAD_QUERIES };

DECLARE_SETTING_ENUM(DeduplicateInsertSelectMode)

enum class DeduplicateInsertMode : uint8_t { BACKWARD_COMPATIBLE_CHOICE = 0, ENABLE, DISABLE };
...
enum class JemallocProfileFormat : uint8_t { Raw = 0, Symbolized, Collapsed };
