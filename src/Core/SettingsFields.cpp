...
void validateFloatingPointSettingValue(T value) {
  if constexpr (std::is_floating_point_v<T>) {
    if (!std::isfinite(value)) throw Exception(ErrorCodes::CANNOT_PARSE_NUMBER, "Float setting value must be finite, got {}", value);
  }
}
...
UInt64 stringToMaxThreads(const String& str) {
  if (startsWith(str, "auto") || startsWith(str, "'auto")) return 0;
  return parseFromString<UInt64>(str);
}

UInt64 fieldToMaxThreads(const Field& f) {
  if (f.getType() == Field::Types::String) return stringToMaxThreads(f.safeGet<String>());
...
void SettingFieldMaxThreads::parseFromString(const String& str) { *this = stringToMaxThreads(str); }

void SettingFieldMaxThreads::writeBinary(WriteBuffer& out) const { writeVarUInt(is_auto ? 0 : value, out); }
...
UInt64 SettingFieldMaxThreads::getAuto() { return getNumberOfCPUCoresToUse(); }

namespace {
Poco::Timespan::TimeDiff float64AsSecondsToTimespan(Float64 d) {
  if (d != 0.0 && !std::isnormal(d))
    throw Exception(ErrorCodes::CANNOT_PARSE_NUMBER, "A setting's value in seconds must be a normal floating point number or zero. Got {}",
                    d);
#pragma clang diagnostic push
#pragma clang diagnostic ignored "-Wimplicit-const-int-float-conversion"
  if (d * 1000000 > std::numeric_limits<Poco::Timespan::TimeDiff>::max() ||
      d * 1000000 < std::numeric_limits<Poco::Timespan::TimeDiff>::min())
    throw Exception(ErrorCodes::BAD_ARGUMENTS, "Cannot convert seconds to microseconds: the setting's value in seconds is too big: {}", d);
#pragma clang diagnostic pop
...
void SettingFieldString::writeBinary(WriteBuffer& out) const { writeStringBinary(value, out); }
...
void SettingFieldMap::parseFromString(const String& str) { *this = stringToMap(str); }

void SettingFieldMap::writeBinary(WriteBuffer& out) const { DB::writeBinary(value, out); }
...
char fieldToChar(const Field& f) { return stringToChar(f.safeGet<String>()); }
}  // namespace
...
void SettingFieldChar::parseFromString(const String& str) { *this = stringToChar(str); }

void SettingFieldChar::writeBinary(WriteBuffer& out) const { writeStringBinary(toString(), out); }
...
void SettingFieldURI::writeBinary(WriteBuffer& out) const { writeStringBinary(value.toString(), out); }
...
void SettingFieldEnumHelpers::writeBinary(std::string_view str, WriteBuffer& out) { writeStringBinary(str, out); }
...
void SettingFieldTimezone::writeBinary(WriteBuffer& out) const { writeStringBinary(value, out); }
...
String SettingFieldCustom::toString() const { return value.dump(); }

void SettingFieldCustom::parseFromString(const String& str) { *this = Field::restoreFromDump(str); }

void SettingFieldCustom::writeBinary(WriteBuffer& out) const { writeStringBinary(toString(), out); }
...
SettingFieldNonZeroUInt64::SettingFieldNonZeroUInt64(UInt64 x) : SettingFieldUInt64(x) { checkValueNonZero(); }

SettingFieldNonZeroUInt64::SettingFieldNonZeroUInt64(const DB::Field& f) : SettingFieldUInt64(f) { checkValueNonZero(); }
...
void SettingFieldNonZeroUInt64::checkValueNonZero() const {
  if (value == 0) throw Exception(ErrorCodes::BAD_ARGUMENTS, "A setting's value has to be greater than 0");
}
