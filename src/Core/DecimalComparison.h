...
inline bool allowDecimalComparison(const DataTypePtr &left_type, const DataTypePtr &right_type) {
  if (isColumnedAsDecimal(left_type)) {
    if (isColumnedAsDecimal(right_type) || isNotDecimalButComparableToDecimal(right_type)) return true;
...
template <>
struct ConstructDecInt<1> {
  using Type = Int32;
};
template <>
struct ConstructDecInt<2> {
  using Type = Int32;
};
template <>
struct ConstructDecInt<4> {
  using Type = Int32;
};
template <>
...
class DecimalComparison {
 public:
  ...
  static ColumnPtr apply(const ColumnWithTypeAndName &col_left, const ColumnWithTypeAndName &col_right, bool check_overflow) {
    ...
    if (check_overflow) return applyWithScale<true>(col_left.column, col_right.column, shift);
  ...
  static bool compare(A a, B b, UInt32 scale_a, UInt32 scale_b, bool check_overflow) {
    ...
    if (scale_a > max_scale || scale_b > max_scale) throw Exception(ErrorCodes::DECIMAL_OVERFLOW, "Bad scale of decimal field");

    Shift shift;
    if (scale_a < scale_b) shift.a = static_cast<CompareInt>(DecimalUtils::scaleMultiplier<B>(scale_b - scale_a));
    if (scale_a > scale_b) shift.b = static_cast<CompareInt>(DecimalUtils::scaleMultiplier<A>(scale_a - scale_b));

    if (check_overflow) return applyWithScale<true>(a, b, shift);
 ...
 private:
  ...
  static auto applyWithScale(T a, U b, const Shift &shift) {
    if (shift.left()) return apply<check_overflow, true, false>(a, b, shift.a);
    if (shift.right()) return apply<check_overflow, false, true>(a, b, shift.b);
  ...
  static Shift getScales(const DataTypePtr &left_type, const DataTypePtr &right_type) {
    ...
    if (decimal0 && decimal1) {
      ...
      shift.a = static_cast<CompareInt>(
          result_type.scaleFactorFor(DecimalUtils::DataTypeDecimalTrait<T>{decimal0->getPrecision(), decimal0->getScale()}, false).value);
      shift.b = static_cast<CompareInt>(
          result_type.scaleFactorFor(DecimalUtils::DataTypeDecimalTrait<U>{decimal1->getPrecision(), decimal1->getScale()}, false).value);
    } else if (decimal0)
  ...
  static Shift getScales(const DataTypePtr &left_type, const DataTypePtr &) {
    Shift shift;
    const DataTypeDecimalBase<T> *decimal0 = checkDecimalBase<T>(*left_type);
    if (decimal0) shift.b = static_cast<CompareInt>(decimal0->getScaleMultiplier().value);
    return shift;
  }
  ...
  static Shift getScales(const DataTypePtr &, const DataTypePtr &right_type) {
    Shift shift;
    const DataTypeDecimalBase<U> *decimal1 = checkDecimalBase<U>(*right_type);
    if (decimal1) shift.a = static_cast<CompareInt>(decimal1->getScaleMultiplier().value);
    return shift;
  }
  ...
  static ALWAYS_INLINE UInt8 apply(A a, B b, CompareInt scale [[maybe_unused]]) {
    ...
    if constexpr (check_overflow) {
      ...
      if constexpr (sizeof(A) > sizeof(CompareInt)) overflow |= (static_cast<A>(x) != a);
      if constexpr (sizeof(B) > sizeof(CompareInt)) overflow |= (static_cast<B>(y) != b);
      if constexpr (is_unsigned_v<A>) overflow |= (x < 0);
      if constexpr (is_unsigned_v<B>) overflow |= (y < 0);
      ...
      if (overflow) throw Exception(ErrorCodes::DECIMAL_OVERFLOW, "Can't compare decimal number due to overflow");
    } else {
      if constexpr (scale_left) x = common::mulIgnoreOverflow(x, scale);
      if constexpr (scale_right) y = common::mulIgnoreOverflow(y, scale);
    }
  ...
  MULTITARGET_FUNCTION_X86_V4_V3(MULTITARGET_FUNCTION_HEADER(template <bool check_overflow, bool scale_left, bool scale_right>
