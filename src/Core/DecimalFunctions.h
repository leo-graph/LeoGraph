...
struct DataTypeDecimalTrait {
  ...
  DataTypeDecimalTrait(UInt32 precision_, UInt32 scale_) : precision(precision_), scale(scale_) {}

  /// @returns multiplier for U to become T with correct scale
  template <typename U>
  T scaleFactorFor(const DataTypeDecimalTrait<U> &x, bool) const {
    if (scale < x.scale) throw Exception(ErrorCodes::ARGUMENT_OUT_OF_BOUND, "Decimal result's scale is less than argument's one");
...
inline bool multiplyAdd(const T &x, const T &multiplier, const T &delta, T &result) {
  ...
  if (common::mulOverflow(x, multiplier, multiplied)) {
    if constexpr (throw_on_error) throw Exception(ErrorCodes::DECIMAL_OVERFLOW, "Decimal math overflow");
    return false;
  }
...
inline bool decimalFromComponentsWithMultiplierImpl(const typename DecimalType::NativeType &whole,
                                                    const typename DecimalType::NativeType &fractional,
                                                    typename DecimalType::NativeType scale_multiplier, DecimalType &result) {
  using T = typename DecimalType::NativeType;
  const auto fractional_sign = whole < 0 ? -1 : 1;
  T value;
  if (!multiplyAdd<T, throw_on_error>(whole, scale_multiplier, fractional_sign * (fractional % scale_multiplier), value)) return false;
  result = DecimalType(value);
  return true;
}

template <typename DecimalType>
inline DecimalType decimalFromComponentsWithMultiplier(const typename DecimalType::NativeType &whole,
                                                       const typename DecimalType::NativeType &fractional,
...
template <typename DecimalType>
inline bool tryGetDecimalFromComponentsWithMultiplier(const typename DecimalType::NativeType &whole,
                                                      const typename DecimalType::NativeType &fractional,
                                                      typename DecimalType::NativeType scale_multiplier, DecimalType &result) {
...
template <typename DecimalType>
inline DecimalType decimalFromComponentsWithMultiplier(const DecimalComponents<DecimalType> &components,
                                                       typename DecimalType::NativeType scale_multiplier) {
  return decimalFromComponentsWithMultiplier<DecimalType>(components.whole, components.fractional, scale_multiplier);
}

template <typename DecimalType>
inline bool tryGetDecimalFromComponentsWithMultiplier(const DecimalComponents<DecimalType> &components,
                                                      typename DecimalType::NativeType scale_multiplier, DecimalType &result) {
 ...
 */
template <typename DecimalType>
inline DecimalType decimalFromComponents(const typename DecimalType::NativeType &whole, const typename DecimalType::NativeType &fractional,
                                         UInt32 scale) {
  using T = typename DecimalType::NativeType;
...
template <typename DecimalType>
inline bool tryGetDecimalFromComponents(const typename DecimalType::NativeType &whole, const typename DecimalType::NativeType &fractional,
                                        UInt32 scale, DecimalType &result) {
  using T = typename DecimalType::NativeType;
...
inline bool dateTimeFromComponentsWithMultiplierImpl(const Int64 &whole_seconds, const Int64 &fractional,
                                                     DateTime64::NativeType scale_multiplier, DateTime64 &result) {
  using T = DateTime64::NativeType;
  T result_value;

  if (!multiplyAdd<T, throw_on_error>(T(whole_seconds), scale_multiplier, fractional % scale_multiplier, result_value)) return false;

  result = DateTime64(result_value);
  return true;
}

inline DateTime64 dateTimeFromComponentsWithMultiplier(const Int64 &whole_seconds, const Int64 &fractional,
...
inline bool tryGetDateTimeFromComponentsWithMultiplier(const Int64 &whole_seconds, const Int64 &fractional,
                                                       DateTime64::NativeType scale_multiplier, DateTime64 &result) {
  return dateTimeFromComponentsWithMultiplierImpl<false>(whole_seconds, fractional, scale_multiplier, result);
}

inline DateTime64 dateTimeFromComponents(const Int64 &whole_seconds, const Int64 &fractional, UInt32 scale) {
  return dateTimeFromComponentsWithMultiplier(whole_seconds, fractional, scaleMultiplier<DateTime64>(scale));
}

inline bool tryGetDateTimeFromComponents(const Int64 &whole_seconds, const Int64 &fractional, UInt32 scale, DateTime64 &result) {
 ...
 */
template <typename DecimalType>
inline DecimalComponents<DecimalType> splitWithScaleMultiplier(const DecimalType &decimal,
                                                               typename DecimalType::NativeType scale_multiplier) {
  using T = typename DecimalType::NativeType;
  const auto whole = decimal.value / scale_multiplier;
  auto fractional = decimal.value % scale_multiplier;
  if (whole && fractional < T(0)) fractional *= T(-1);
...
inline typename DecimalType::NativeType getWholePart(const DecimalType &decimal, UInt32 scale) {
  if (scale == 0) return decimal.value;
...
inline typename DecimalType::NativeType getFractionalPartWithScaleMultiplier(const DecimalType &decimal,
                                                                             typename DecimalType::NativeType scale_multiplier) {
  using T = typename DecimalType::NativeType;
  ...
  if constexpr (!keep_sign)
    if (decimal.value / scale_multiplier && result < T(0)) result = -result;
