...
static ReturnType onError(int code [[maybe_unused]], FormatStringHelper<FmtArgs...> fmt_string [[maybe_unused]],
...
static const IColumn *getActualColumn(const IColumn *column) {
  ...
  if (const auto *column_const = typeid_cast<const ColumnConst *>(column)) return getActualColumn(&column_const->getDataColumn());
  ...
  if (const auto *column_sparse = typeid_cast<const ColumnSparse *>(column)) return getActualColumn(&column_sparse->getValuesColumn());
...
static ReturnType checkColumnStructure(const ColumnWithTypeAndName &actual, const ColumnWithTypeAndName &expected,
  ...
  if (actual.name != expected.name)
    return onError<ReturnType>(code, "Block structure mismatch in {} stream: different names of columns:\n{}\n{}", context_description,
                               actual.dumpStructure(), expected.dumpStructure());
  ...
  if (!actual.column || !expected.column) return ReturnType(true);
  ...
  if (actual_column_maybe_agg && expected_column_maybe_agg) {
    if (!actual_column_maybe_agg->getAggregateFunction()->haveSameStateRepresentation(*expected_column_maybe_agg->getAggregateFunction()))
      return onError<ReturnType>(code, "Block structure mismatch in {} stream: different columns:\n{}\n{}", context_description,
                                 actual.dumpStructure(), expected.dumpStructure());
      ...
      !expected.column->empty())  /// don't check values in empty columns
  {
    ...
    if (actual_value != expected_value)
      return onError<ReturnType>(
          code, "Block structure mismatch in {} stream: different values of constants in column '{}': actual: {}, expected: {}",
          context_description, actual.name, applyVisitor(FieldVisitorToString(), actual_value),
...
static ReturnType checkBlockStructure(const Block &lhs, const Block &rhs, std::string_view context_description, bool allow_materialize) {
  ...
  if (&lhs == &rhs) return ReturnType(true);
...
Block::Block(std::initializer_list<ColumnWithTypeAndName> il) : data{il} { initializeIndexByName(); }

Block::Block(const ColumnsWithTypeAndName &data_) : data{data_} { initializeIndexByName(); }

Block::Block(ColumnsWithTypeAndName &&data_) : data{std::move(data_)} { initializeIndexByName(); }

void Block::initializeIndexByName() {
  for (size_t i = 0, size = data.size(); i < size; ++i) index_by_name.emplace(data[i].name, i);
}
...
void Block::insert(size_t position, ColumnWithTypeAndName elem) {
  ...
  if (elem.name.empty()) throw Exception(ErrorCodes::AMBIGUOUS_COLUMN_NAME, "Column name in Block cannot be empty");

  auto [new_it, inserted] = index_by_name.emplace(elem.name, position);
  if (!inserted)
    checkColumnStructure<void>(data[new_it->second], elem, "(columns with identical name must have identical structure)", true,
                               ErrorCodes::AMBIGUOUS_COLUMN_NAME);

  for (auto it = index_by_name.begin(); it != index_by_name.end(); ++it) {
    if (it->second >= position && (!inserted || it != new_it)) ++it->second;
  }
...
void Block::insert(ColumnWithTypeAndName elem) {
  ...
  if (!inserted)
    checkColumnStructure<void>(data[it->second], elem, "(columns with identical name must have identical structure)", true,
                               ErrorCodes::AMBIGUOUS_COLUMN_NAME);
...
void Block::insertUnique(ColumnWithTypeAndName elem) {
  ...
  if (!index_by_name.contains(elem.name)) insert(std::move(elem));
}
...
void Block::erase(size_t position) {
  if (data.empty()) throw Exception(ErrorCodes::POSITION_OUT_OF_BOUND, "Block is empty");
...
void Block::erase(const String &name) {
  ...
  if (index_it == index_by_name.end()) throw Exception(ErrorCodes::NOT_FOUND_COLUMN_IN_BLOCK, "No such name in Block::erase(): '{}'", name);
...
ColumnWithTypeAndName &Block::safeGetByPosition(size_t position) {
  ...
  if (position >= data.size())
    throw Exception(ErrorCodes::POSITION_OUT_OF_BOUND,
                    ...
                    toString(position), toString(data.size() - 1), dumpNames());
...
std::optional<ColumnWithTypeAndName> Block::findSubcolumnByName(const std::string &name) const {
  for (auto [column_name, subcolumn_name] : Nested::getAllColumnAndSubcolumnPairs(name)) {
    ...
    if (subcolumn_type && subcolumn) return ColumnWithTypeAndName(subcolumn, subcolumn_type, name);
  }
...
std::optional<ColumnWithTypeAndName> Block::findColumnOrSubcolumnByName(const std::string &name) const {
  if (const auto *column = findByName(name, false)) return *column;
...
ColumnWithTypeAndName Block::getSubcolumnByName(const std::string &name) const {
  ...
  if (!result)
    throw Exception(ErrorCodes::NOT_FOUND_COLUMN_IN_BLOCK, "Not found subcolumn {} in block. There are only columns: {}", name,
                    dumpNames());
...
ColumnWithTypeAndName Block::getColumnOrSubcolumnByName(const std::string &name) const {
  ...
  if (!result)
    throw Exception(ErrorCodes::NOT_FOUND_COLUMN_IN_BLOCK, "Not found column or subcolumn {} in block. There are only columns: {}", name,
                    dumpNames());

  return *result;
}

bool Block::has(const std::string &name, bool case_insensitive) const { return findPositionByName(name, case_insensitive).has_value(); }
...
size_t Block::getPositionByName(const std::string &name, bool case_insensitive) const {
  const auto pos = findPositionByName(name, case_insensitive);
  if (!pos.has_value())
    throw Exception(ErrorCodes::NOT_FOUND_COLUMN_IN_BLOCK, "Not found column {} in block. There are only columns: {}", name, dumpNames());
  return pos.value();
}
...
void Block::checkNumberOfRows(bool allow_null_columns) const {
  ...
  for (const auto &elem : data) {
    ...
    if (rows == -1)
      ...
      throw Exception(ErrorCodes::SIZES_OF_COLUMNS_DOESNT_MATCH, "Sizes of columns doesn't match: {}: {}, {}: {}", data.front().name, rows,
...
size_t Block::bytes() const {
  size_t res = 0;
  for (const auto &elem : data)
    if (elem.column) res += elem.column->byteSize();
...
size_t Block::allocatedBytes() const {
  size_t res = 0;
  for (const auto &elem : data)
    if (elem.column) res += elem.column->allocatedBytes();
...
MutableColumns Block::cloneEmptyColumns() const {
  ...
  for (size_t i = 0; i < num_columns; ++i) columns[i] = data[i].column ? data[i].column->cloneEmpty() : data[i].type->createColumn();
  return columns;
}
...
MutableColumns Block::cloneEmptyColumns(const Serializations &serializations) const {
  size_t num_columns = data.size();
  MutableColumns columns(num_columns);
  for (size_t i = 0; i < num_columns; ++i) columns[i] = data[i].type->createColumn(*serializations[i]);
  return columns;
}
...
Columns Block::detachColumns() {
  size_t num_columns = data.size();
  Columns columns(num_columns);
  for (size_t i = 0; i < num_columns; ++i) columns[i] = data[i].column ? std::move(data[i].column) : data[i].type->createColumn();
  return columns;
}
...
void Block::setColumn(size_t position, ColumnWithTypeAndName column) {
  if (position >= data.size())
    throw Exception(ErrorCodes::POSITION_OUT_OF_BOUND, "Position {} out of bound in Block::setColumn(), max position {}", position,
                    data.size());
...
Block Block::cloneWithColumns(MutableColumns &&columns) const {
  ...
  if (num_columns != columns.size()) {
    ...
    throw Exception(ErrorCodes::LOGICAL_ERROR,
                    "Cannot clone block with columns because block [{}] has {} columns, but {} columns given [{}]", dumpStructure(),
                    num_columns, columns.size(), fmt::join(columns | dump_columns, ", "));
  }

  res.reserve(num_columns);

  for (size_t i = 0; i < num_columns; ++i) res.insert({std::move(columns[i]), data[i].type, data[i].name});
...
Block Block::cloneWithCutColumns(size_t start, size_t length) const {
  ...
  for (auto &column_to_cut : copy.data) column_to_cut.column = column_to_cut.column->cut(start, length);
...
Block Block::sortColumns() const {
  ...
  {
    size_t i = 0;
    for (auto it = index_by_name.begin(); it != index_by_name.end(); ++it) sorted_index_by_name[i++] = it;
  }
  ::sort(sorted_index_by_name.begin(), sorted_index_by_name.end(),
         [](const auto &lhs, const auto &rhs) { return lhs->first < rhs->first; });

  for (const auto &it : sorted_index_by_name) sorted_block.insert(data[it->second]);
...
Block Block::shrinkToFit() const {
  ...
  for (size_t i = 0; i < data.size(); ++i) new_columns[i] = data[i].column->cloneResized(data[i].column->size());
...
Block Block::compress() const {
  ...
  for (size_t i = 0; i < num_columns; ++i) new_columns[i] = data[i].column->compress(/*force_compression=*/false);
  return cloneWithColumns(new_columns);
}
...
Block Block::decompress() const {
  size_t num_columns = data.size();
  Columns new_columns(num_columns);
  for (size_t i = 0; i < num_columns; ++i) new_columns[i] = data[i].column->decompress();
  return cloneWithColumns(new_columns);
}

const ColumnsWithTypeAndName &Block::getColumnsWithTypeAndName() const { return data; }
...
NameSet Block::getNameSet() const {
  ...
  for (const auto &elem : data) res.insert(elem.name);
  return res;
}
...
Names Block::getDataTypeNames() const {
  ...
  for (const auto &elem : data) res.push_back(elem.type->getName());

  return res;
}

bool blocksHaveEqualStructure(const Block &lhs, const Block &rhs) { return checkBlockStructure<bool>(lhs, rhs, "", false); }
...
bool isCompatibleHeader(const Block &actual, const Block &desired) { return checkBlockStructure<bool>(actual, desired, "", true); }
...
void getBlocksDifference(const Block &lhs, const Block &rhs, std::string &out_lhs_diff, std::string &out_rhs_diff) {
  ...
  for (auto &v : lcs) v.resize(rhs.columns() + 1);
  ...
  while (l > 0) left_columns.push_back(lhs.safeGetByPosition(--l));
  while (r > 0) right_columns.push_back(rhs.safeGetByPosition(--r));
...
Serializations Block::getSerializations() const {
  ...
  for (const auto &column : data) res.push_back(column.type->getDefaultSerialization());
...
void removeSpecialColumnRepresentations(Block &block) {
  for (auto &column : block) column.column = removeSpecialRepresentations(column.column);
}
...
Block materializeBlock(const Block &block, bool remove_special_column_representations) {
  ...
  for (size_t i = 0; i < columns; ++i) {
    ...
    if (remove_special_column_representations) element.column = removeSpecialRepresentations(element.column);
  }
...
void materializeBlockInplace(Block &block, bool remove_special_column_representations) {
  for (size_t i = 0; i < block.columns(); ++i) {
    ...
    if (remove_special_column_representations) block.getByPosition(i).column = removeSpecialRepresentations(block.getByPosition(i).column);
  }
}
...
Block concatenateBlocks(const std::vector<Block> &blocks) {
  ...
  size_t num_rows = 0;
  for (const auto &block : blocks) num_rows += block.rows();
