...
bool nestedEpollWorks(LoggerPtr log) {
  if (Poco::Environment::os() != POCO_OS_LINUX) return true;
...
void applySettingsQuirks(Settings& settings, LoggerPtr log) {
  if (!nestedEpollWorks(log)) {
    if (!settings[Setting::async_socket_for_remote].changed && settings[Setting::async_socket_for_remote]) {
      ...
      if (log) LOG_WARNING(log, "async_socket_for_remote has been disabled (you can explicitly enable it still)");
    }
    if (!settings[Setting::async_query_sending_for_remote].changed && settings[Setting::async_query_sending_for_remote]) {
      ...
      if (log) LOG_WARNING(log, "async_query_sending_for_remote has been disabled (you can explicitly enable it still)");
    }
    if (!settings[Setting::use_hedged_requests].changed && settings[Setting::use_hedged_requests]) {
      ...
      if (log) LOG_WARNING(log, "use_hedged_requests has been disabled (you can explicitly enable it still)");
    }
  }
  ...
  if (!queryProfilerWorks()) {
    if (settings[Setting::query_profiler_real_time_period_ns]) {
      ...
      if (log) LOG_WARNING(log, "query_profiler_real_time_period_ns has been disabled (due to server had been compiled with sanitizers)");
    }
    if (settings[Setting::query_profiler_cpu_time_period_ns]) {
      settings[Setting::query_profiler_cpu_time_period_ns] = 0;
      if (log) LOG_WARNING(log, "query_profiler_cpu_time_period_ns has been disabled (due to server had been compiled with sanitizers)");
    }
  }
}
...
void doSettingsSanityCheckClamp(Settings& current_settings, LoggerPtr log) {
  ...
  if (max_threads > max_threads_max_value) {
    if (log) LOG_WARNING(log, "Sanity check: Too many threads requested ({}). Reduced to {}", max_threads, max_threads_max_value);
...
#define CHECK_MAX_VALUE(SETTING_VALUE)                                                                                                    \
  if (UInt64 block_size = current_settings[Setting::SETTING_VALUE]; block_size > max_sane_block_rows_size) {                              \
    if (log)                                                                                                                              \
      LOG_WARNING(log, "Sanity check: '{}' value is too high ({}). Reduced to {}", #SETTING_VALUE, block_size, max_sane_block_rows_size); \
...
#undef CHECK_MAX_VALUE
  ...
  if (auto max_block_size = current_settings[Setting::max_block_size]; max_block_size == 0) {
    if (log) LOG_WARNING(log, "Sanity check: 'max_block_size' cannot be 0. Set to default value {}", DEFAULT_BLOCK_SIZE);
