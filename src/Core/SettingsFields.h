...
struct SettingAutoWrapper final : SettingFieldBase {
  ...
  explicit SettingAutoWrapper(const Field &f) : is_auto(isAuto(f)) {
    if (!is_auto) base = Base(f);
  }

  SettingAutoWrapper &operator=(const Field &f) override {
    changed = true;
    if (is_auto = isAuto(f); !is_auto) base = f;
    return *this;
  }
  ...
  void parseFromString(const String &str) override {
    changed = true;
    if (is_auto = isAuto(str); !is_auto) base.parseFromString(str);
  }
...
struct SettingFieldMaxThreads final : SettingFieldBase {
  ...
  SettingFieldMaxThreads(const SettingFieldMaxThreads &o) : is_auto(o.is_auto), value(o.value), changed(o.changed) {}

  SettingFieldMaxThreads &operator=(UInt64 x) {
    is_auto = !x;
    value = is_auto ? getAuto() : x;
    changed = true;
    return *this;
  }
...
enum class SettingFieldTimespanUnit : uint8_t { Millisecond, Second };
...
struct SettingFieldTimespan final : SettingFieldBase {
  ...
  explicit SettingFieldTimespan(UInt64 x)
      : SettingFieldTimespan(Poco::Timespan{static_cast<Poco::Timespan::TimeDiff>(x * microseconds_per_unit)}) {}
  explicit SettingFieldTimespan(const Field &f);

  SettingFieldTimespan &operator=(const Poco::Timespan &x) {
    value = x;
    changed = true;
    return *this;
  }

  SettingFieldTimespan &operator=(UInt64 x) {
    *this = Poco::Timespan{static_cast<Poco::Timespan::TimeDiff>(x * microseconds_per_unit)};
    return *this;
  }
...
struct SettingFieldString final : SettingFieldBase {
  ...
  SettingFieldString &operator=(std::string_view str) {
    value = str;
    changed = true;
    return *this;
  }
  SettingFieldString &operator=(const String &str) {
    *this = std::string_view{str};
    return *this;
  }
  SettingFieldString &operator=(String &&str) {
    value = std::move(str);
    changed = true;
    return *this;
  }
  SettingFieldString &operator=(const char *str) {
    *this = std::string_view{str};
    return *this;
  }
...
struct SettingFieldMap final : SettingFieldBase {
 public:
  ...
  SettingFieldMap &operator=(const Map &map) {
    value = map;
    changed = true;
    return *this;
  }
...
struct SettingFieldChar final : SettingFieldBase {
 public:
  ...
  SettingFieldChar &operator=(char c) {
    value = c;
    changed = true;
    return *this;
  }
...
struct SettingFieldURI final : SettingFieldBase {
  ...
  SettingFieldURI &operator=(const Poco::URI &x) {
    value = x;
    changed = true;
    return *this;
  }
  SettingFieldURI &operator=(const String &str) {
    *this = Poco::URI{str};
    return *this;
  }
  SettingFieldURI &operator=(const char *str) {
    *this = Poco::URI{str};
    return *this;
  }
...
struct SettingFieldEnum final : SettingFieldBase {
  ...
  SettingFieldEnum &operator=(EnumType x) {
    value = x;
    changed = true;
    return *this;
  }
  SettingFieldEnum &operator=(const Field &f) override {
    *this = Traits::fromString(f.safeGet<String>());
    return *this;
  }
...
struct SettingFieldMultiEnum final : SettingFieldBase {
  ...
  SettingFieldMultiEnum &operator=(ValueType x) {
    changed = true;
    value = x;
    return *this;
  }
  SettingFieldMultiEnum &operator=(const Field &x) override {
    parseFromString(x.safeGet<String>());
    return *this;
  }
  ...
  operator MultiEnum<EnumType>() const  /// NOLINT
  {
    ...
    for (const auto &v : value) res.set(v);
    return res;
  }
  ...
  String toString() const override {
    ...
    if (!result.empty()) result.erase(result.size() - separator.size());
 ...
 private:
  static ValueType parseValueFromString(const std::string_view str) {
    ...
    while (value_start != std::string::npos) {
      ...
      if (value_end == std::string::npos) value_end = str_view.size();
...
struct SettingFieldTimezone final : SettingFieldBase {
  ...
  explicit SettingFieldTimezone(std::string_view str = {}) {
    validateTimezone(std::string(str));
    value = str;
  }
  explicit SettingFieldTimezone(const String &str) {
    validateTimezone(str);
    value = str;
  }
  explicit SettingFieldTimezone(String &&str) {
    validateTimezone(str);
    value = std::move(str);
  }
  explicit SettingFieldTimezone(const char *str) {
    validateTimezone(str);
    value = str;
  }
  explicit SettingFieldTimezone(const Field &f) {
    const String &str = f.safeGet<String>();
    validateTimezone(str);
    value = str;
  }
  SettingFieldTimezone(const SettingFieldTimezone &o) : value(o.value), changed(o.changed) {}

  SettingFieldTimezone &operator=(std::string_view str) {
    validateTimezone(std::string(str));
    value = str;
    changed = true;
    return *this;
  }
  SettingFieldTimezone &operator=(const String &str) {
    *this = std::string_view{str};
    return *this;
  }
  SettingFieldTimezone &operator=(String &&str) {
    validateTimezone(str);
    value = std::move(str);
    changed = true;
    return *this;
  }
  SettingFieldTimezone &operator=(const char *str) {
    *this = std::string_view{str};
    return *this;
  }
