...
namespace DB {
InterpolateDescription::InterpolateDescription(ActionsDAG actions_, const Aliases& aliases) : actions(std::move(actions_)) {
  ...
  for (const ColumnWithTypeAndName& column : actions.getResultColumns()) {
    ...
    if (const auto& p = aliases.find(name); p != aliases.end()) name = p->second->getColumnName();
