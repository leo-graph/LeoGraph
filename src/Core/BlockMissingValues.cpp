...
void BlockMissingValues::init(size_t num_columns) { rows_mask_by_column_id.resize(num_columns); }
...
void BlockMissingValues::setBitsFromNullMap(size_t column_idx, const PaddedPODArray<UInt8>& null_map) {
  ...
  for (size_t i = 0; i < null_map.size(); ++i) mask.set(i, bool(null_map[i]));
}
...
void BlockMissingValues::clear() {
  for (auto& mask : rows_mask_by_column_id) mask.clear();
}

bool BlockMissingValues::empty() const {
  return std::ranges::all_of(rows_mask_by_column_id, [&](const auto& mask) { return mask.empty(); });
}

size_t BlockMissingValues::getNumColumns() const { return rows_mask_by_column_id.size(); }
