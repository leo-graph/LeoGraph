...
void insertDefaultPostgreSQLValue(IColumn &column, const IColumn &sample_column) { column.insertFrom(sample_column, 0); }

void insertPostgreSQLValue(IColumn &column, std::string_view value, ExternalResultDescription::ValueType type, DataTypePtr data_type,
  ...
  switch (type) {
    ...
    case ExternalResultDescription::ValueType::vtDecimal32:
      [[fallthrough]];
    case ExternalResultDescription::ValueType::vtDecimal64:
      [[fallthrough]];
    case ExternalResultDescription::ValueType::vtDecimal128:
      [[fallthrough]];
    ...
    case ExternalResultDescription::ValueType::vtArray: {
      ...
      if (max_dimension < expected_dimensions)
        throw Exception(ErrorCodes::BAD_ARGUMENTS, "Got less dimensions than expected. ({} instead of {})", max_dimension,
                        expected_dimensions);
...
void preparePostgreSQLArrayInfo(std::unordered_map<size_t, PostgreSQLArrayInfo> &array_info, size_t column_idx, DataTypePtr data_type) {
  ...
  if (nested->isNullable()) nested = static_cast<const DataTypeNullable *>(nested.get())->getNestedType();
