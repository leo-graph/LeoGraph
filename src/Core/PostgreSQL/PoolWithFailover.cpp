  ...
  return [weak_pool = pool, pool_wait_timeout](UInt64 interval_milliseconds) {
    ...
    if (!shared_pool->online) {
      ...
      if (!connection_available) {
        LOG_WARNING(logger, "Reestablishing connection to {} has failed: unable to fetch connection within the timeout.",
                    connection->getInfoForLog());
        return true;
      }

      try {
        /// Create a new connection or reopen an old connection if it became invalid.
        if (!connection) connection = std::make_unique<Connection>(shared_pool->connection_info);
        ...
        if (interval_milliseconds >= 1000) LOG_WARNING(logger, "Reestablishing connection to {} has failed.", connection->getInfoForLog());
        shared_pool->online = false;
        shared_pool->pool->returnObject(std::move(connection));
      }
    }
...
PoolWithFailover::PoolWithFailover(const ReplicasConfigurationByPriority& configurations_by_priority, size_t pool_size,
                                   size_t pool_wait_timeout_, size_t max_tries_, bool auto_close_connection_,
                                   size_t connection_attempt_timeout_, bool bg_reconnect_)
    : pool_wait_timeout(pool_wait_timeout_),
      max_tries(max_tries_),
      auto_close_connection(auto_close_connection_),
      bg_reconnect(bg_reconnect_) {
  LOG_TRACE(getLogger("PostgreSQLConnectionPool"),
            "PostgreSQL connection pool size: {}, connection wait timeout: {}, max failover tries: {}", pool_size, pool_wait_timeout,
            max_tries_);
  ...
  for (const auto& [priority, configurations] : configurations_by_priority) {
    for (const auto& replica_configuration : configurations) {
      auto connection_info =
          formatConnectionString(replica_configuration.database, replica_configuration.host, replica_configuration.port,
                                 replica_configuration.username, replica_configuration.password, connection_attempt_timeout_);
      replicas_with_priority[priority].emplace_back(std::make_shared<PoolHolder>(connection_info, pool_size));
      if (bg_reconnect)
        DB::ReplicasReconnector::instance().add(
            connectionReestablisher(std::weak_ptr(replicas_with_priority[priority].back()), pool_wait_timeout));
    }
  }
}

PoolWithFailover::PoolWithFailover(const DB::StoragePostgreSQL::Configuration& configuration, size_t pool_size, size_t pool_wait_timeout_,
                                   size_t max_tries_, bool auto_close_connection_, size_t connection_attempt_timeout_, bool bg_reconnect_)
    : pool_wait_timeout(pool_wait_timeout_),
      max_tries(max_tries_),
      auto_close_connection(auto_close_connection_),
      bg_reconnect(bg_reconnect_) {
...
ConnectionHolderPtr PoolWithFailover::get() {
  ...
  if (replicas_with_priority.empty()) throw DB::Exception(DB::ErrorCodes::LOGICAL_ERROR, "No address specified");
  ...
  for (size_t try_idx = 0; try_idx < max_tries; ++try_idx) {
    for (auto& priority : replicas_with_priority) {
      ...
      for (size_t i = 0; i < replicas.size(); ++i) {
        ...
        if (bg_reconnect && !replica->online) continue;
        ...
        try {
          ...
          error_message = PreformattedMessage::create("Try {}. Connection to {} failed with error: {}\n", try_idx + 1,
                                                      DB::backQuote(replica->connection_info.host_port), pqxx_error.what());
        ...
        if (replicas.size() > 1) std::rotate(replicas.begin(), replicas.begin() + i + 1, replicas.end());
