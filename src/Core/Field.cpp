...
bool AggregateFunctionStateData::operator==(const AggregateFunctionStateData &rhs) const {
  if (name != rhs.name)
    throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT, "Comparing aggregate functions with different types: {} and {}", name, rhs.name);
...
const DecimalField<T> &DecimalField<T>::operator+=(const DecimalField<T> &r) {
  if (scale != r.getScale()) throw Exception(ErrorCodes::LOGICAL_ERROR, "Add different decimal fields");
  dec += r.getValue();
  return *this;
}
...
template <is_decimal T>
const DecimalField<T> &DecimalField<T>::operator-=(const DecimalField<T> &r) {
  if (scale != r.getScale()) throw Exception(ErrorCodes::LOGICAL_ERROR, "Sub different decimal fields");
  dec -= r.getValue();
  return *this;
}
...
bool Field::operator<(const Field &rhs) const {
  ...
  switch (which) {
    ...
    case Types::UInt256:
      return get<UInt256>() < rhs.get<UInt256>();
    case Types::Int64:
      return get<Int64>() < rhs.get<Int64>();
    case Types::Int128:
      return get<Int128>() < rhs.get<Int128>();
    case Types::Int256:
      return get<Int256>() < rhs.get<Int256>();
    case Types::UUID:
      return get<UUID>() < rhs.get<UUID>();
    case Types::IPv4:
    ...
    case Types::CustomType:
      return get<CustomType>() < rhs.get<CustomType>();
  }
...
bool Field::operator<=(const Field &rhs) const {
  ...
  switch (which) {
    ...
    case Types::Int256:
      return get<Int256>() <= rhs.get<Int256>();
    case Types::UUID:
      return get<UUID>().toUnderType() <= rhs.get<UUID>().toUnderType();
    case Types::IPv4:
      return get<IPv4>() <= rhs.get<IPv4>();
    case Types::IPv6:
      return get<IPv6>() <= rhs.get<IPv6>();
...
void readBinaryArray(Array &x, ReadBuffer &buf) {
  ...
  for (size_t index = 0; index < size; ++index) x.push_back(readFieldBinary(buf));
}
...
void writeBinaryArray(const Array &x, WriteBuffer &buf) {
  ...
  for (const auto &elem : x) writeFieldBinary(elem, buf);
}
...
void writeText(const Tuple &x, WriteBuffer &buf) { writeFieldText(Field(x), buf); }
...
void writeText(const Map &x, WriteBuffer &buf) { writeFieldText(Field(x), buf); }
...
void writeText(const Object &x, WriteBuffer &buf) { writeFieldText(Field(x), buf); }
...
void writeText(const CustomType &x, WriteBuffer &buf) { writeFieldText(Field(x), buf); }
...
[[noreturn]] void writeQuoted(const Object &, WriteBuffer &) {
  throw Exception(ErrorCodes::NOT_IMPLEMENTED, "Cannot write Object quoted.");
}
...
String Field::dump() const { return applyVisitor(FieldVisitorDump(), *this); }
...
Field Field::restoreFromDump(std::string_view dump_) {
  ...
  if (dump.starts_with(prefix)) {
    ...
    if ((comma == std::string_view::npos) || (end != after_prefix.length() - 1)) show_error();
...
void writeText(const Null &x, WriteBuffer &buf) {
  if (x.isNegativeInfinity()) writeText("-Inf", buf);
...
String fieldToString(const Field &x) {
  return Field::dispatch([](const auto &value) { return toString(value); }, x);
}
...
std::string_view fieldTypeToString(Field::Types::Which type) {
  switch (type) {
    ...
    case Field::Types::Which::AggregateFunctionState:
      return "AggregateFunctionState"sv;
    ...
    case Field::Types::Which::Decimal128:
      return "Decimal128"sv;
    case Field::Types::Which::Decimal256:
      return "Decimal256"sv;
    ...
    case Field::Types::Which::CustomType:
      return "CustomType"sv;
  }
}

/// Keep in mind, that "magic_enum" is very expensive for compiler, that's why we don't use it.
std::string_view Field::getTypeName() const { return fieldTypeToString(which); }
