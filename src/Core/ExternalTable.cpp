...
ExternalTableDataPtr BaseExternalTable::getData(ContextPtr context) {
  ...
  initSampleBlock();
  auto input =
      context->getInputFormat(format, *read_buffer, sample_block, context->getSettingsRef().get("max_block_size").safeGet<UInt64>());
...
void BaseExternalTable::parseStructureFromStructureField(const std::string& argument) {
  ...
  String error;
  ASTPtr columns_list_raw = tryParseQuery(parser, pos, pos + argument.size(), error, false, "", false, DBMS_DEFAULT_MAX_QUERY_SIZE,
                                          DBMS_DEFAULT_MAX_PARSER_DEPTH, DBMS_DEFAULT_MAX_PARSER_BACKTRACKS, true);

  if (!columns_list_raw) throw Exception(ErrorCodes::BAD_ARGUMENTS, "Error while parsing table structure: {}", error);
  ...
  for (auto& child : columns_list_raw->children) {
    ...
    if (column)
      structure.emplace_back(column->name, column->type->formatWithPossiblyHidingSensitiveData(
                                               /*max_length=*/0,
                                               /*one_line=*/true,
                                               /*show_secrets=*/true,
      ...
      throw Exception(ErrorCodes::BAD_ARGUMENTS, "Error while parsing table structure: expected column definition, got {}",
...
void BaseExternalTable::parseStructureFromTypesField(const std::string& argument) {
  ...
  const auto* pos = argument.data();
  String error;
  ASTPtr type_list_raw = tryParseQuery(parser, pos, pos + argument.size(), error, false, "", false, DBMS_DEFAULT_MAX_QUERY_SIZE,
                                       DBMS_DEFAULT_MAX_PARSER_DEPTH, DBMS_DEFAULT_MAX_PARSER_BACKTRACKS, true);

  if (!type_list_raw) throw Exception(ErrorCodes::BAD_ARGUMENTS, "Error while parsing table structure: {}", error);

  for (size_t i = 0; i < type_list_raw->children.size(); ++i)
    structure.emplace_back("_" + toString(i + 1), type_list_raw->children[i]->formatWithPossiblyHidingSensitiveData(
                                                      /*max_length=*/0,
                                                      /*one_line=*/true,
                                                      /*show_secrets=*/true,
                                                      /*print_pretty_type_names=*/false,
...
void ExternalTablesHandler::handlePart(const Poco::Net::MessageHeader& header, ReadBuffer& stream) {
  ...
  if (settings[Setting::http_max_multipart_form_data_size])
    read_buffer = std::make_unique<LimitReadBuffer>(
        stream, LimitReadBuffer::Settings{
                    .read_no_more = settings[Setting::http_max_multipart_form_data_size],
                    .expect_eof = true,
                    .excetion_hint =
                        "the maximum size of multipart/form-data. This limit can be tuned by 'http_max_multipart_form_data_size' setting",
                });
  else
  ...
  if (params.has(name + "_structure"))
    ...
    throw Exception(ErrorCodes::BAD_ARGUMENTS,
                    ...
                    "Use fields {}_structure or {}_types to do so.",
                    name, name, name);
