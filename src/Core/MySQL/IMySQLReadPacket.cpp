...
void IMySQLReadPacket::readPayload(ReadBuffer &in, uint8_t &sequence_id) {
  ...
  if (!payload.eof()) {
    throw Exception(ErrorCodes::UNKNOWN_PACKET_FROM_CLIENT,
                    "Packet payload is not fully read. Stopped after {} bytes, while {} bytes are in buffer.", payload.count(),
                    payload.available());
  }
}

void IMySQLReadPacket::readPayloadWithUnpacked(ReadBuffer &in) { readPayloadImpl(in); }
