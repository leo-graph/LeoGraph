...
PacketEndpoint::PacketEndpoint(WriteBuffer &out_, uint8_t &sequence_id_) : sequence_id(sequence_id_), in(nullptr), out(&out_) {}
...
void PacketEndpoint::receivePacket(IMySQLReadPacket &packet) { packet.readPayload(*in, sequence_id); }
...
bool PacketEndpoint::tryReceivePacket(IMySQLReadPacket &packet, UInt64 millisecond) {
  if (millisecond != 0) {
    ...
    if (!socket_in) throw Exception(ErrorCodes::LOGICAL_ERROR, "Attempt to pull the duration in a non socket stream");

    if (!socket_in->poll(millisecond * 1000)) return false;
  }
...
void PacketEndpoint::resetSequenceId() { sequence_id = 0; }
