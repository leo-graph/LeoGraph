...
Handshake::Handshake(uint32_t capability_flags_, uint32_t connection_id_, String server_version_, String auth_plugin_name_,
                     String auth_plugin_data_, uint8_t charset_)
...
size_t Handshake::getPayloadSize() const { return 26 + server_version.size() + auth_plugin_data.size() + auth_plugin_name.size(); }
...
HandshakeResponse::HandshakeResponse(UInt32 capability_flags_, UInt32 max_packet_size_, UInt8 character_set_, const String &username_,
                                     const String &database_, const String &auth_response_, const String &auth_plugin_name_)
    : capability_flags(capability_flags_),
...
size_t AuthSwitchRequest::getPayloadSize() const { return 2 + plugin_name.size() + auth_plugin_data.size(); }
...
void AuthSwitchResponse::readPayloadImpl(ReadBuffer &payload) { readStringUntilEOF(value, payload); }

size_t AuthMoreData::getPayloadSize() const { return 1 + data.size(); }
