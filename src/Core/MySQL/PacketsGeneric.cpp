...
OKPacket::OKPacket(uint8_t header_, uint32_t capabilities_, uint64_t affected_rows_, uint32_t status_flags_, int16_t warnings_,
...
size_t OKPacket::getPayloadSize() const {
  ...
  if (capabilities & CLIENT_SESSION_TRACK) {
    ...
    if (status_flags & SERVER_SESSION_STATE_CHANGED) result += getLengthEncodedStringSize(session_state_changes);
  } else {
    result += info.size();
  }
...

{
  ...
  if (capabilities & CLIENT_SESSION_TRACK) {
    ...
    if (status_flags & SERVER_SESSION_STATE_CHANGED) writeLengthEncodedString(session_state_changes, buffer);
  } else {
...
size_t EOFPacket::getPayloadSize() const { return 5; }
...
size_t ERRPacket::getPayloadSize() const { return 4 + sql_state.length() + std::min(error_message.length(), MYSQL_ERRMSG_SIZE); }
...
size_t LengthEncodedNumber::getPayloadSize() const { return getLengthEncodedNumberSize(value); }

void LengthEncodedNumber::writePayloadImpl(WriteBuffer &buffer) const { writeLengthEncodedNumber(value, buffer); }
