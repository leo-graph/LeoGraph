...
static String generateScramble() {
  ...
  for (size_t i = 0; i < SCRAMBLE_LENGTH; ++i) {
    ...
    scramble[i] &= 0x7f;
    if (scramble[i] == '\0' || scramble[i] == '$') scramble[i] = scramble[i] + 1;
  }
...
Native41::Native41(const String &password_, const String &scramble_) {
  ...
  for (size_t i = 0; i < SCRAMBLE_LENGTH; ++i) scramble[i] = static_cast<unsigned char>(password_sha1[i] ^ digest[i]);
}

void Native41::authenticate(const String &user_name, Session &session, std::optional<String> auth_response,
...
Sha256Password::Sha256Password(KeyPair &private_key_, LoggerPtr log_) : private_key(private_key_), log(log_) {
...
void Sha256Password::authenticate(const String &user_name, Session &session, std::optional<String> auth_response,
                                  std::shared_ptr<PacketEndpoint> packet_endpoint, bool is_secure_connection,
  ...
  if (!is_secure_connection && !auth_response->empty() && auth_response != String("\0", 1)) {
    ...
    if (!ctx) throw Exception(ErrorCodes::OPENSSL_ERROR, "Failed to create EVP_PKEY_CTX");

    if (EVP_PKEY_decrypt_init(ctx.get()) <= 0 || EVP_PKEY_CTX_set_rsa_padding(ctx.get(), RSA_PKCS1_OAEP_PADDING) <= 0) {
