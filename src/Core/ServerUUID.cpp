...
UUID ServerUUID::get() {
  if (server_uuid == UUIDHelpers::Nil && (Context::getGlobalContextInstance()->getApplicationType() == Context::ApplicationType::SERVER ||
...
void ServerUUID::load(const fs::path &server_uuid_file, Poco::Logger *log) { server_uuid = loadServerUUID(server_uuid_file, log); }
...
UUID loadServerUUID(const fs::path &server_uuid_file, Poco::Logger *log) {
  ...
  if (fs::exists(server_uuid_file)) {
    try {
      ...
      LOG_ERROR(log, "Cannot read server UUID from file {}: {}. Will overwrite it", server_uuid_file.string(),
...
void ServerUUID::set(UUID &uuid) { server_uuid = uuid; }

void ServerUUID::setRandomForUnitTests() { server_uuid = UUIDHelpers::generateV4(); }
