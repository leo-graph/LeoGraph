...
YTsaurusClient::YTsaurusClient(const YTsaurusClient& other)
    : context(other.context), connection_info(other.connection_info), log(getLogger("YTsaurusClient")) {}
...
YTsaurusNodeType YTsaurusClient::getNodeTypeFromAttributes(const Poco::JSON::Object::Ptr& json_ptr) {
  if (!json_ptr->has("type")) throw Exception(ErrorCodes::INCORRECT_DATA, "Incorrect json with yt attributes, no field 'type'.");

  if (json_ptr->getValue<String>("type") == "table" || json_ptr->getValue<String>("type") == "replicated_table") {
    if (!json_ptr->has("dynamic")) throw Exception(ErrorCodes::INCORRECT_DATA, "Incorrect json with yt attributes, no field 'dynamic'.");
...
ReadBufferPtr YTsaurusClient::selectRows(const String& cypress_path, const ColumnsWithTypeAndName& columns) {
  ...
  for (size_t i = 0; i < columns.size(); ++i) {
    ...
    if (i + 1 != columns.size()) columns_names_str += ", ";
  }
...
ReadBufferPtr YTsaurusClient::executeQuery(const YTsaurusQueryPtr query, const ReadWriteBufferFromHTTP::OutStreamCallback&& out_callback) {
  for (size_t num_try = 0; num_try < connection_info.http_proxy_urls.size(); ++num_try) {
    ...
    try {
      if (connection_info.enable_heavy_proxy_redirection && query->isHeavyQuery()) host_for_request = getHeavyProxyURI(host_for_request);
...
YTsaurusClient::URI YTsaurusClient::getHeavyProxyURI(const URI& uri) {
  ...
  if (!list_of_proxies || !list_of_proxies->size()) throw Exception(ErrorCodes::LOGICAL_ERROR, "Can't take the list of heavy proxies.");
...
ReadBufferPtr YTsaurusClient::createQueryRWBuffer(const URI& uri, const ReadWriteBufferFromHTTP::OutStreamCallback& out_callback,
                                                  const std::string& http_method) {
  std::string output_params =
      fmt::format("<uuid_mode=text_yql;complex_type_mode=positional;encode_utf8={}>", connection_info.encode_utf8 ? "true" : "false");
...
YTsaurusClient::SchemaDescription YTsaurusClient::getTableSchema(const String& cypress_path) {
  ...
  if (!schema_json->has("$attributes")) throw Exception(ErrorCodes::INCORRECT_DATA, "No \"$attributes\" property in yt table schema");
  ...
  for (const auto& yt_column : *columns_array) {
    ...
    if (!yt_column_json->has("name")) throw Exception(ErrorCodes::INCORRECT_DATA, "Broken YtSaurus schema json. Missing `name` field.");
...
bool YTsaurusClient::checkSchemaCompatibility(const String& table_path, const SharedHeader& sample_block, String& reason,
  ...
  if (!yt_schema.is_strict) return true;
  ...
  for (const auto& column_type_with_name : sample_block->getColumnsWithTypeAndName()) {
    ...
    if (!isYTSaurusTypesCompatible(column_type_with_name.type, yt_column_type, allow_nullable)) {
      reason = fmt::format("Column {} types mismatch. YtSaurus converted type {} table column type {}", column_type_with_name.name,
                           yt_column_type->getName(), column_type_with_name.type->getName());
      return false;
    }
  }
  return true;
...
YTsaurusTableLock::~YTsaurusTableLock() {
  if (!transaction_id.empty()) {
    try {
      ...
      LOG_WARNING(getLogger("YTsaurusTableLock"), "Can't commit transaction {}. Leave it. Exception: {}", transaction_id, e.displayText());
    }
  }
}
