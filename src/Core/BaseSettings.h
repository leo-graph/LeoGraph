...
class BaseSettings : public TTraits::Data {
 ...
 public:
  ...
  class Range {
   public:
    Range(BaseSettings &settings_, SkipFlags skip_flags_)
        : settings(settings_), accessor(Traits::Accessor::instance()), skip_flags(skip_flags_) {}
  ...
  class MutableRange {
   public:
    MutableRange(BaseSettings &settings_, SkipFlags skip_flags_)
        : settings(settings_), accessor(Traits::Accessor::instance()), skip_flags(skip_flags_) {}
...
Field BaseSettings<TTraits>::get(std::string_view name) const {
  name = TTraits::resolveName(name);
  const auto &accessor = Traits::Accessor::instance();
  if (size_t index = accessor.find(name); index != static_cast<size_t>(-1)) return accessor.getValue(*this, index);
...
bool BaseSettings<TTraits>::isChanged(std::string_view name) const {
  name = TTraits::resolveName(name);
  const auto &accessor = Traits::Accessor::instance();
  if (size_t index = accessor.find(name); index != static_cast<size_t>(-1)) return accessor.isValueChanged(*this, index);
...
SettingsChanges BaseSettings<TTraits>::changes() const {
  SettingsChanges res;
  for (const auto &field : *this) res.emplace_back(field.getName(), field.getValue());
  return res;
}
...
void BaseSettings<TTraits>::resetToDefault() {
  ...
  for (size_t i = 0; i < accessor.size(); i++) {
    if (accessor.isValueChanged(*this, i)) accessor.resetValueToDefault(*this, i);
  }

  if constexpr (Traits::allow_custom_settings) custom_settings_map.clear();
}
...
void BaseSettings<TTraits>::resetToDefault(std::string_view name) {
  ...
  if constexpr (Traits::allow_custom_settings) custom_settings_map.erase(String{name});
}
...
std::string_view BaseSettings<TTraits>::getTypeName(std::string_view name) const {
  name = TTraits::resolveName(name);
  const auto &accessor = Traits::Accessor::instance();
  if (size_t index = accessor.find(name); index != static_cast<size_t>(-1)) return accessor.getTypeName(index);
  if (tryGetCustomSetting(name)) return "Custom";
...
std::string_view BaseSettings<TTraits>::getDescription(std::string_view name) const {
  name = TTraits::resolveName(name);
  const auto &accessor = Traits::Accessor::instance();
  if (size_t index = accessor.find(name); index != static_cast<size_t>(-1)) return accessor.getDescription(index);
  if (tryGetCustomSetting(name)) return "Custom";
  BaseSettingsHelpers::throwSettingNotFound(name);
}
...
SettingsTierType BaseSettings<TTraits>::getTier(std::string_view name) const {
  name = TTraits::resolveName(name);
  const auto &accessor = Traits::Accessor::instance();
  if (size_t index = accessor.find(name); index != static_cast<size_t>(-1)) return accessor.getTier(index);
  if (tryGetCustomSetting(name)) return SettingsTierType::PRODUCTION;
  BaseSettingsHelpers::throwSettingNotFound(name);
}
...
Field BaseSettings<TTraits>::castValueUtil(std::string_view name, const Field &value) {
  name = TTraits::resolveName(name);
  const auto &accessor = Traits::Accessor::instance();
  if (size_t index = accessor.find(name); index != static_cast<size_t>(-1)) return accessor.castValueUtil(index, value);
...
String BaseSettings<TTraits>::valueToStringUtil(std::string_view name, const Field &value) {
  name = TTraits::resolveName(name);
  const auto &accessor = Traits::Accessor::instance();
  if (size_t index = accessor.find(name); index != static_cast<size_t>(-1)) return accessor.valueToStringUtil(index, value);
  if constexpr (Traits::allow_custom_settings)
    return value.dump();
  else
    BaseSettingsHelpers::throwSettingNotFound(name);
...
Field BaseSettings<TTraits>::stringToValueUtil(std::string_view name, const String &str) {
  ...
  try {
    const auto &accessor = Traits::Accessor::instance();
    if (size_t index = accessor.find(name); index != static_cast<size_t>(-1)) return accessor.stringToValueUtil(index, str);
    if constexpr (Traits::allow_custom_settings)
...
void BaseSettings<TTraits>::writeChangedBinary(WriteBuffer &out) const {
  ...
  for (auto it = this->begin(); it != this->end(); ++it) ++num_settings;
...
void BaseSettings<TTraits>::readBinary(ReadBuffer &in) {
  ...
  for (size_t i = 0; i < num_settings; ++i) {
    ...
    if (index == static_cast<size_t>(-1)) BaseSettingsHelpers::throwSettingNotFound(name);
...
void BaseSettings<TTraits>::read(ReadBuffer &in, SettingsWriteFormat format) {
  ...
  while (true) {
    String read_name = BaseSettingsHelpers::readString(in);
    if (read_name.empty() /* empty string is a marker of the end of settings */) break;
    ...
    using Flags = BaseSettingsHelpers::Flags;
    UInt64 flags{};
    if (format >= SettingsWriteFormat::STRINGS_WITH_FLAGS) flags = BaseSettingsHelpers::readFlags(in);
...
bool operator==(const BaseSettings<TTraits> &left, const BaseSettings<TTraits> &right) {
  ...
  for (const auto &r : right) {
    if ((l == left.end()) || (*l != r)) return false;
    ++l;
  }
  return l == left.end();
}
...
SettingFieldCustom &BaseSettings<TTraits>::getCustomSetting(std::string_view name) {
  if constexpr (Traits::allow_custom_settings) {
    ...
    if (it == custom_settings_map.end()) it = custom_settings_map.emplace(String{name}, SettingFieldCustom{}).first;
    return it->second;
  }
  BaseSettingsHelpers::throwSettingNotFound(name);
}
...
const SettingFieldCustom &BaseSettings<TTraits>::getCustomSetting(std::string_view name) const {
  if constexpr (Traits::allow_custom_settings) {
    auto it = custom_settings_map.find(name);
    if (it != custom_settings_map.end()) return it->second;
  }
  BaseSettingsHelpers::throwSettingNotFound(name);
}
...
BaseSettings<TTraits>::Iterator::Iterator(BaseSettings &settings_, const typename Traits::Accessor &accessor_, SkipFlags skip_flags_)
  ...
  if (skip_flags & SKIP_CHANGED) {
    ...
    if (skip_flags & SKIP_UNCHANGED) skip_flags = static_cast<SkipFlags>(skip_flags | SKIP_BUILTIN);
  }
...
typename BaseSettings<TTraits>::Iterator &BaseSettings<TTraits>::Iterator::operator++() {
  ...
  if (index != accessor.size())
    ...
    if constexpr (Traits::allow_custom_settings) ++custom_settings_iterator;
  }
  doSkip();
  setPointerToCustomSetting();
  return *this;
...
void BaseSettings<TTraits>::Iterator::doSkip() {
  ...
  auto &index = field_ref.index;
  if (skip_flags & SKIP_CHANGED) {
    while ((index != accessor.size()) && accessor.isValueChanged(settings, index)) ++index;
...
bool BaseSettings<TTraits>::Iterator::operator==(const typename BaseSettings<TTraits>::Iterator &other) const {
  if constexpr (TTraits::allow_custom_settings) {
    if (custom_settings_iterator != other.custom_settings_iterator) return false;
  }
...
const String &BaseSettings<TTraits>::SettingFieldRef::getName() const {
  if constexpr (Traits::allow_custom_settings) {
    if (custom_setting) return (*custom_setting)->first;
  }
...
Field BaseSettings<TTraits>::SettingFieldRef::getValue() const {
  if constexpr (Traits::allow_custom_settings) {
    if (custom_setting) return static_cast<Field>((*custom_setting)->second);
  }
...
void BaseSettings<TTraits>::SettingFieldRef::setValue(const Field &value) {
  if constexpr (Traits::allow_custom_settings) {
    if (custom_setting) (*custom_setting)->second = value;
  } else
...
String BaseSettings<TTraits>::SettingFieldRef::getValueString() const {
  if constexpr (Traits::allow_custom_settings) {
    if (custom_setting) return (*custom_setting)->second.toString();
  }
...
std::string_view BaseSettings<TTraits>::SettingFieldRef::getTypeName() const {
  if constexpr (Traits::allow_custom_settings) {
    if (custom_setting) return "Custom";
  }
...
SettingsTierType BaseSettings<TTraits>::SettingFieldRef::getTier() const {
  if constexpr (Traits::allow_custom_settings) {
    if (custom_setting) return SettingsTierType::PRODUCTION;
  }
...
/// NOLINTNEXTLINE
#define DECLARE_SETTINGS_TRAITS_WITH_PATH_ALLOW_CUSTOM_SETTINGS(SETTINGS_TRAITS_NAME, LIST_OF_SETTINGS_WITHOUT_PATH_MACRO, \
                                                                LIST_OF_SETTINGS_WITH_PATH_MACRO)                          \
 ...
 */
/// NOLINTNEXTLINE
#define DECLARE_SETTINGS_TRAITS_COMMON(SETTINGS_TRAITS_NAME, LIST_OF_SETTINGS_WITHOUT_PATH_MACRO, LIST_OF_SETTINGS_WITH_PATH_MACRO,   \
  ...
  struct SETTINGS_TRAITS_NAME {                                                                                                       \
    ...
    class Accessor {                                                                                                                  \
     public:                                                                                                                          \
      ...
      void setValue(Data &data, size_t index, const Field &value) const { *field_infos[index].get_data_function(data) = value; }      \
...
/// NOLINTNEXTLINE
#define DECLARE_SETTINGS_TRAITS_(TYPE, NAME, DEFAULT, DESCRIPTION, FLAGS, ...) SettingField##TYPE NAME{DEFAULT};

/// Generates an alias mapping entry
/// NOLINTNEXTLINE
#define DECLARE_SETTINGS_WITH_ALIAS_TRAITS_(TYPE, NAME, DEFAULT, DESCRIPTION, FLAGS, ALIAS) {#ALIAS, #NAME},
...
#define IMPLEMENT_SETTINGS_TRAITS_(TYPE, NAME, DEFAULT, DESCRIPTION, FLAGS, ...)                           \
  res.field_infos.emplace_back(FieldInfo{                                                                  \
      #NAME,                                                                                               \
      "",                                                                                                  \
      #TYPE,                                                                                               \
      DESCRIPTION,                                                                                         \
