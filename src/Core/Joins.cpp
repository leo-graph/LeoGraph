...
const char* toString(JoinTableSide join_table_side) {
  switch (join_table_side) {
    case JoinTableSide::Left:
      return "LEFT";
    case JoinTableSide::Right:
      return "RIGHT";
  }
}

JoinKind reverseJoinKind(JoinKind kind) {
  if (kind == JoinKind::Right) return JoinKind::Left;
  if (kind == JoinKind::Left) return JoinKind::Right;
  return kind;
}
...
JoinKind deserializeJoinKind(ReadBuffer& in) {
  ...
  if (val > JoinKindMax) throw Exception(ErrorCodes::INCORRECT_DATA, "Cannot convert {} to JoinKind", val);
...
JoinStrictness deserializeJoinStrictness(ReadBuffer& in) {
  ...
  if (val > JoinStrictnessMax) throw Exception(ErrorCodes::INCORRECT_DATA, "Cannot convert {} to JoinStrictness", val);
...
JoinLocality deserializeJoinLocality(ReadBuffer& in) {
  ...
  if (val > JoinLocalityMax) throw Exception(ErrorCodes::INCORRECT_DATA, "Cannot convert {} to JoinLocality", UInt16(val));
