...
struct SortCursorImpl {
  ...
  SortCursorImpl(const Block &header, const Columns &columns, size_t num_rows, const SortDescription &desc_, size_t order_ = 0,
  ...
  size_t getRow() const {
    if (permutation) return (*permutation)[pos];
    return pos;
  }
...
struct SortCursorHelper {
  ...
  bool ALWAYS_INLINE operator<(const SortCursorHelper &rhs) const { return derived().greater(rhs.derived()); }

  /// Checks that all rows in the current block of this cursor are less than or equal to all the rows of the current block of another
  /// cursor.
...
struct SortCursor : SortCursorHelper<SortCursor> {
  ...
  bool ALWAYS_INLINE greaterAt(const SortCursor &rhs, size_t lhs_pos, size_t rhs_pos) const {
    ...
    if (impl->desc.compiled_sort_description && rhs.impl->desc.compiled_sort_description) {
      ...
      int res = sort_description_func_typed(lhs_pos, rhs_pos, impl->raw_sort_columns_data.data(),
                                            rhs.impl->raw_sort_columns_data.data());  /// NOLINT
...
struct SimpleSortCursor : SortCursorHelper<SimpleSortCursor> {
  ...
  bool ALWAYS_INLINE greaterAt(const SimpleSortCursor &rhs, size_t lhs_pos, size_t rhs_pos) const {
    ...
    if (impl->desc.compiled_sort_description && rhs.impl->desc.compiled_sort_description) {
      ...
      auto sort_description_func_typed = reinterpret_cast<JITSortDescriptionFunc>(impl->desc.compiled_sort_description);
      res = sort_description_func_typed(lhs_pos, rhs_pos, impl->raw_sort_columns_data.data(),
                                        rhs.impl->raw_sort_columns_data.data());  /// NOLINT
    } else
#endif
    {
...
struct SpecializedSingleNullableColumnSortCursor : SortCursorHelper<SpecializedSingleNullableColumnSortCursor<ColumnType>> {
  ...
  template <bool consider_order = true>
  bool ALWAYS_INLINE greaterAt(const SortCursorHelper<SpecializedSingleNullableColumnSortCursor> &rhs, size_t lhs_pos,
                               size_t rhs_pos) const {
    auto &this_impl = this->impl;
  ...
  bool ALWAYS_INLINE greaterAt(const SortCursorWithCollation &rhs, size_t lhs_pos, size_t rhs_pos) const {
    for (size_t i = 0; i < impl->sort_columns_size; ++i) {
      ...
      if (impl->need_collation[i])
        res = impl->sort_columns[i]->compareAtWithCollation(lhs_pos, rhs_pos, *(rhs.impl->sort_columns[i]), nulls_direction,
 ...
 public:
  ...
  explicit SortingQueueImpl(Cursors &cursors) {
    ...
    if constexpr (strategy == SortingQueueStrategy::Batch) {
      if (!queue.empty()) updateBatchSize();
    }
  }
  ...
  void push(SortCursorImpl &cursor) {
    ...
    if constexpr (strategy == SortingQueueStrategy::Batch) updateBatchSize();
  }
 ...
 private:
  ...
  size_t ALWAYS_INLINE nextChildIndex() {
    if (next_child_idx == 0) {
      ...
      if (queue.size() > 2 && queue[1].greater(queue[2])) ++next_child_idx;
    }
  ...
  void updateBatchSize() {
    ...
    if (i < max_linear_detection) return;
...
class SortQueueVariants {
 ...
 private:
  ...
  using QueueVariants = std::variant<SortingQueueImpl<SpecializedSingleColumnSortCursor<ColumnVector<UInt8>>, strategy>,
