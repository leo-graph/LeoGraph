...
struct MultiEnum {
  ...
  constexpr explicit MultiEnum(ValueType v) : bitset(static_cast<StorageType>(v)) {
  ...
  bool isSet(EnumType value) const { return bitset & toBitFlag(value); }

  void set(EnumType value) { bitset |= toBitFlag(value); }

  void unset(EnumType value) { bitset &= ~(toBitFlag(value)); }

  void reset() { bitset = 0; }

  StorageType getValue() const { return bitset; }
  ...
  bool operator==(const MultiEnum& other) const { return bitset == other.bitset; }
