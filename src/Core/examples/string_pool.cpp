...
int main(int argc, char** argv) {
...

  {
    ...
    std::cerr << "Read and inserted into vector in " << watch.elapsedSeconds() << " sec, "
              ...
              << static_cast<double>(in.count()) / watch.elapsedSeconds() / 1000000 << " MB/sec." << std::endl;
  }

  {
    ...
    for (Vec::iterator it = vec.begin(); it != vec.end(); ++it) {
      ...
      if (it == vec.begin()) res = tmp;
    }

    std::cerr << "Inserted into pool in " << watch.elapsedSeconds() << " sec, " << static_cast<double>(vec.size()) / watch.elapsedSeconds()
              << " rows/sec., " << static_cast<double>(in.count()) / watch.elapsedSeconds() / 1000000 << " MB/sec." << std::endl;
...

  {
    ...
    for (const auto& elem : vec) set[elem] = 0;
...

  {
    ...
    for (const auto& elem : vec) set[std::string_view(elem)] = 0;
...

  {
    ...
    for (const auto& elem : vec) set[std::string_view(pool.insert(elem.data(), elem.size()), elem.size())] = 0;
...

  {
    ...
    for (const auto& elem : vec) set[std::string_view(elem.data(), elem.size())] = 0;
