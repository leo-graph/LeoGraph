...
PlainRanges::PlainRanges(const Range& range) { ranges.push_back(range); }
...
bool PlainRanges::compareByLeftBound(const Range& lhs, const Range& rhs) {
  if (lhs.left == NEGATIVE_INFINITY && rhs.left == NEGATIVE_INFINITY) return false;
...
bool PlainRanges::compareByRightBound(const Range& lhs, const Range& rhs) {
  if (lhs.right == POSITIVE_INFINITY && rhs.right == POSITIVE_INFINITY) return false;
...
std::vector<Ranges> PlainRanges::invert(const Ranges& to_invert_ranges) {
  ...
  if (to_invert_ranges.empty()) return {makeUniverse().ranges};
