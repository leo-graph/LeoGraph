...
bool lessOp(A a, B b) {
  ...
  /// float vs float
  if constexpr (is_floating_point<A> && is_floating_point<B>) return a < b;
  ...
  if constexpr (is_integer<A> && is_integer<B>) {
    ...
    if constexpr (is_signed_v<A> && !is_signed_v<B>) return a < 0 || static_cast<make_unsigned_t<A>>(a) < b;

    if constexpr (!is_signed_v<A> && is_signed_v<B>) return b >= 0 && a < static_cast<make_unsigned_t<B>>(b);
  }

  /// int vs float
  if constexpr (is_integer<A> && is_floating_point<B>) {
    if constexpr (sizeof(A) <= 4) return static_cast<double>(a) < static_cast<double>(b);

    return DecomposedFloat<B>(b).greater(a);
  }

  if constexpr (is_floating_point<A> && is_integer<B>) {
    if constexpr (sizeof(B) <= 4) return static_cast<double>(a) < static_cast<double>(b);
...
inline bool NO_SANITIZE_UNDEFINED convertNumeric(From value, To& result) {
  ...
  if (greaterOp(value, std::numeric_limits<To>::max()) || lessOp(value, std::numeric_limits<To>::lowest())) {
    return false;
  }
