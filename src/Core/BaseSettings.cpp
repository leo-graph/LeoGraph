...
void BaseSettingsHelpers::writeString(std::string_view str, WriteBuffer& out) { writeStringBinary(str, out); }
...
void BaseSettingsHelpers::writeFlags(Flags flags, WriteBuffer& out) { writeVarUInt(flags, out); }
...
SettingsTierType BaseSettingsHelpers::getTier(UInt64 flags) {
  ...
  if (tier > SettingsTierType::BETA) throw Exception(ErrorCodes::INCORRECT_DATA, "Unknown tier value: '{}'", tier);
...
void BaseSettingsHelpers::warningSettingNotFound(std::string_view name) {
  ...
  if (!unknown_settings_warning_logged) {
    ...
    if (unknown_settings.size() > MAX_UNKNOWN_SETTINGS_FOR_LOGGING) {
      ...
      LOG_WARNING(getLogger("Settings"), "Unknown settings: {} and {} more, skipping", fmt::join(first_few, ", "),
