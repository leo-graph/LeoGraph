...
bool DataTypeFunction::equals(const IDataType& rhs) const { return typeid(rhs) == typeid(*this) && getName() == rhs.getName(); }
...
void DataTypeFunction::updateHashImpl(SipHash& hash) const {
  ...
  for (const auto& arg_type : argument_types) {
    ...
    if (arg_type) arg_type->updateHash(hash);
  }

  hash.update(return_type != nullptr);
  if (return_type) return_type->updateHash(hash);
}
