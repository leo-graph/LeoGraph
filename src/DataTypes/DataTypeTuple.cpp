...
DataTypeTuple::DataTypeTuple(const DataTypes &elems_) : elems(elems_), has_explicit_names(false) {
  /// Automatically assigned names in form of '1', '2', ...
  size_t size = elems.size();
  names.resize(size);
  for (size_t i = 0; i < size; ++i) names[i] = toString(i + 1);
}

static std::optional<Exception> checkTupleNames(const Strings &names) {
  std::unordered_set<String> names_set;
  for (const auto &name : names) {
    if (name.empty()) return Exception(ErrorCodes::BAD_ARGUMENTS, "Names of tuple elements cannot be empty");
...
DataTypeTuple::DataTypeTuple(const DataTypes &elems_, const Strings &names_) : elems(elems_), names(names_), has_explicit_names(true) {
  size_t size = elems.size();
  if (names.size() != size)
    throw Exception(ErrorCodes::NUMBER_OF_ARGUMENTS_DOESNT_MATCH, "Wrong number of names passed to constructor of DataTypeTuple");

  if (auto exception = checkTupleNames(names)) throw std::move(*exception);
}
...
std::string DataTypeTuple::doGetName() const {
  ...
  for (size_t i = 0; i < size; ++i) {
    ...
    if (has_explicit_names) s << backQuoteIfNeed(names[i]) << ' ';
...
std::string DataTypeTuple::doGetPrettyName(size_t indent) const {
  ...
  if (has_explicit_names) {
    ...
    for (size_t i = 0; i != size; ++i) {
      ...
      s << fourSpaceIndent(indent + 1) << backQuoteIfNeed(names[i]) << ' ' << elems[i]->getPrettyName(indent + 1);
    }
...
DataTypePtr DataTypeTuple::getNormalizedType() const {
  ...
  for (const auto &elem : elems) normalized_elems.emplace_back(elem->getNormalizedType());
...
static void addElementSafe(const DataTypes &elems, IColumn &column, F &&impl) {
  ...
  try {
    ...
    for (const auto &i : collections::range(0, elems.size())) {
      ...
      if (element_column.size() > old_size) element_column.popBack(1);
    }
...
MutableColumnPtr DataTypeTuple::createColumn() const {
  if (elems.empty()) return ColumnTuple::create(0);

  size_t size = elems.size();
  MutableColumns tuple_columns(size);
  for (size_t i = 0; i < size; ++i) tuple_columns[i] = elems[i]->createColumn();
...
MutableColumnPtr DataTypeTuple::createColumn(const ISerialization &serialization) const {
  ...
  if (!serialization_tuple) throw Exception(ErrorCodes::LOGICAL_ERROR, "Unexpected serialization to create column of type Tuple");

  if (elems.empty()) return IDataType::createColumn(serialization);
  ...
  MutableColumns tuple_columns(size);
  for (size_t i = 0; i < size; ++i) tuple_columns[i] = elems[i]->createColumn(*element_serializations[i]->getNested());
...
void DataTypeTuple::insertDefaultInto(IColumn &column) const {
  ...
  addElementSafe(elems, column, [&] {
    for (const auto &i : collections::range(0, elems.size())) elems[i]->insertDefaultInto(extractElementColumn(column, i));
  });
}

bool DataTypeTuple::equals(const IDataType &rhs) const {
  if (typeid(rhs) != typeid(*this)) return false;

  const DataTypeTuple &rhs_tuple = static_cast<const DataTypeTuple &>(rhs);

  size_t size = elems.size();
  if (size != rhs_tuple.elems.size()) return false;

  for (size_t i = 0; i < size; ++i)
    if (!elems[i]->equals(*rhs_tuple.elems[i]) || names[i] != rhs_tuple.names[i]) return false;
...
size_t DataTypeTuple::getPositionByName(std::string_view name, bool case_insensitive) const {
  for (size_t i = 0; i < elems.size(); ++i) {
    if (case_insensitive) {
      if (boost::iequals(names[i], name)) return i;
    } else {
      if (boost::equals(names[i], name)) return i;
    }
  }
...
size_t DataTypeTuple::getMaximumSizeOfValueInMemory() const {
  size_t res = 0;
  for (const auto &elem : elems) res += elem->getMaximumSizeOfValueInMemory();
  return res;
}

size_t DataTypeTuple::getSizeOfValueInMemory() const {
  size_t res = 0;
  for (const auto &elem : elems) res += elem->getSizeOfValueInMemory();
  return res;
}
...
SerializationPtr DataTypeTuple::getSerialization(const SerializationInfo &info) const {
  ...
  return wrapSerializationBasedOnKindStack(std::make_shared<SerializationTuple>(std::move(serializations), has_explicit_names), kinds,
                                           info.getSettings());
}
...
MutableSerializationInfoPtr DataTypeTuple::createSerializationInfo(const SerializationInfoSettings &settings) const {
  ...
  for (const auto &elem : elems) infos.push_back(elem->createSerializationInfo(settings));
...
SerializationInfoPtr DataTypeTuple::getSerializationInfo(const IColumn &column) const {
  if (const auto *column_const = checkAndGetColumn<ColumnConst>(&column)) return getSerializationInfo(column_const->getDataColumn());
...
static DataTypePtr create(const ASTPtr &arguments) {
  ...
  if (names.empty()) return std::make_shared<DataTypeTuple>(nested_types);
...
void registerDataTypeTuple(DataTypeFactory &factory) { factory.registerDataType("Tuple", create); }
