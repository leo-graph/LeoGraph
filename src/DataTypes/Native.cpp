...
bool canBeNativeType(const IDataType &type) {
  ...
  return (data_type.isInt() || data_type.isUInt() || data_type.isNativeFloat() || data_type.isDate() || data_type.isDate32() ||
          data_type.isDateTime() || data_type.isDateTime64() || data_type.isTime() || data_type.isTime64() || data_type.isEnum() ||
          data_type.isDecimal()) &&
         type.getSizeOfValueInMemory() <= MAX_NATIVE_INT_SIZE;
}

bool canBeNativeType(const DataTypePtr &type) { return canBeNativeType(*type); }
...
llvm::Type *toNativeType(llvm::IRBuilderBase &builder, const IDataType &type) {
  ...
  if (data_type.isInt8() || data_type.isUInt8()) return builder.getInt8Ty();
  if (data_type.isInt16() || data_type.isUInt16() || data_type.isDate()) return builder.getInt16Ty();
  ...
  if (data_type.isInt128() || data_type.isUInt128() || data_type.isDecimal128()) return builder.getInt128Ty();
  if (data_type.isInt256() || data_type.isUInt256() || data_type.isDecimal256()) return builder.getIntNTy(256);
  if (data_type.isFloat32()) return builder.getFloatTy();
  if (data_type.isFloat64()) return builder.getDoubleTy();
  if (data_type.isEnum8()) return builder.getInt8Ty();
  if (data_type.isEnum16()) return builder.getInt16Ty();
  throw Exception(ErrorCodes::LOGICAL_ERROR, "Invalid cast from {} to native type", type.getName());
}

llvm::Type *toNativeType(llvm::IRBuilderBase &builder, const DataTypePtr &type) { return toNativeType(builder, *type); }
...
llvm::Value *nativeBoolCast(llvm::IRBuilderBase &b, const DataTypePtr &from_type, llvm::Value *value) {
  ...
  if (value->getType()->isIntegerTy()) return b.CreateICmpNE(value, zero);
  if (value->getType()->isFloatingPointTy()) return b.CreateFCmpUNE(value, zero);
...
llvm::Value *nativeCast(llvm::IRBuilderBase &b, const DataTypePtr &from_type, llvm::Value *value, const DataTypePtr &to_type) {
  ...
  /// (e.g., avg) already manage Decimal scale conversion themselves.
  {
    auto get_effective_scale = [](const DataTypePtr &type) -> std::optional<UInt32> {
      ...
      if (which.isDateTime() || which.isTime()) return 0u;
      if (which.isDateTime64()) return typeid_cast<const DataTypeDateTime64 *>(type.get())->getScale();
      if (which.isTime64()) return typeid_cast<const DataTypeTime64 *>(type.get())->getScale();
      return std::nullopt;
    };
    ...
    if (from_scale && to_scale && *from_scale != *to_scale) {
      ...
      if (from_native_type != to_native_type) value = b.CreateIntCast(value, to_native_type, typeIsSigned(*from_type));
      ...
      for (UInt32 i = 0; i < scale_diff; ++i) scale_factor *= 10;
  ...
  if (from_native_type == to_native_type) return value;
  ...
  if (from_native_type->isFloatingPointTy() && to_native_type->isFloatingPointTy()) return b.CreateFPCast(value, to_native_type);

  throw Exception(ErrorCodes::LOGICAL_ERROR, "Invalid cast to native value from type {} to type {}", from_type->getName(),
                  to_type->getName());
}
...
llvm::Type *toNativeType(llvm::IRBuilderBase &builder) {
  if constexpr (std::is_same_v<ToType, Int8> || std::is_same_v<ToType, UInt8>)
    ...
    return builder.getInt32Ty();
  else if constexpr (std::is_same_v<ToType, Int64> || std::is_same_v<ToType, UInt64> || std::is_same_v<ToType, DateTime64> ||
