...
DataTypePtr getMostSubtype(const DataTypes &types, bool throw_if_result_is_nothing, bool force_support_conversion) {
  ...
  if (types.empty()) {
    if (throw_if_result_is_nothing) throw Exception(ErrorCodes::NO_COMMON_TYPE, "There is no common type for empty type list");
    return std::make_shared<DataTypeNothing>();
  }
  ...
  {
    for (const auto &type : types)
      if (typeid_cast<const DataTypeNothing *>(type.get())) return get_nothing_or_throw(" because some of them are Nothing");
  }
  ...
  /// For Arrays
  {
    ...
    if (have_array) {
      if (!all_arrays) return get_nothing_or_throw(" because some of them are Array and some of them are not");
  ...
  /// For tuples
  {
    ...
    for (const auto &type : types) {
      if (const auto *type_tuple = typeid_cast<const DataTypeTuple *>(type.get())) {
        if (!have_tuple) {
          ...
          for (size_t elem_idx = 0; elem_idx < tuple_size; ++elem_idx) nested_types[elem_idx].reserve(types.size());
    ...
    if (have_tuple) {
      if (!all_tuples) return get_nothing_or_throw(" because some of them are Tuple and some of them are not");
  ...
  /// No other types are compatible with Strings. TODO Enums?
  {
    ...
    if (have_string) {
      if (!all_strings) return get_nothing_or_throw(" because some of them are String/FixedString and some of them are not");
  ...
  /// For Date and DateTime, the common type is Date. No other types are compatible.
  {
    ...
    if (have_date_or_datetime) {
      if (!all_date_or_datetime) return get_nothing_or_throw(" because some of them are Date/DateTime and some of them are not");
  ...
  /// For numeric types, the most complicated part.
  {
    ...
    auto minimize = [](size_t &what, size_t value) {
      if (what == 0 || value < what) what = value;
    };
    ...
    if (min_bits_of_signed_integer || min_bits_of_unsigned_integer || min_mantissa_bits_of_floating) {
      if (!all_numbers) return get_nothing_or_throw(" because some of them are numbers and some of them are not");
      ...
      if (!min_bits_of_signed_integer && !min_bits_of_unsigned_integer) {
        ...
        if (min_mantissa_bits_of_floating <= 53) return std::make_shared<DataTypeFloat64>();
      ...
      if (min_bits_of_unsigned_integer && (min_bits_of_signed_integer == 0 || min_bits_of_unsigned_integer <= min_bits_of_signed_integer)) {
        if (min_bits_of_unsigned_integer <= 8) return std::make_shared<DataTypeUInt8>();
        if (min_bits_of_unsigned_integer <= 16) return std::make_shared<DataTypeUInt16>();
        if (min_bits_of_unsigned_integer <= 32) return std::make_shared<DataTypeUInt32>();
        if (min_bits_of_unsigned_integer <= 64) return std::make_shared<DataTypeUInt64>();
        if (min_bits_of_unsigned_integer <= 128) return std::make_shared<DataTypeUInt128>();
        if (min_bits_of_unsigned_integer <= 256) return std::make_shared<DataTypeUInt256>();
        throw Exception(ErrorCodes::NO_COMMON_TYPE,
      ...
      /// All signed.
      {
        if (min_bits_of_signed_integer <= 8) return std::make_shared<DataTypeInt8>();
        if (min_bits_of_signed_integer <= 16) return std::make_shared<DataTypeInt16>();
        if (min_bits_of_signed_integer <= 32) return std::make_shared<DataTypeInt32>();
        if (min_bits_of_signed_integer <= 64) return std::make_shared<DataTypeInt64>();
        if (min_bits_of_signed_integer <= 128) return std::make_shared<DataTypeInt128>();
        if (min_bits_of_signed_integer <= 256) return std::make_shared<DataTypeInt256>();
        throw Exception(ErrorCodes::NO_COMMON_TYPE,
                        "Logical error: {} but as all data types are integers, "
  ...
  /// Decimals
  {
    ...
    if (all_decimals) return createDecimal<DataTypeDecimal>(min_precision, min_scale);
  }
