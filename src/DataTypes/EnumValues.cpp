...
template <typename T>
EnumValues<T>::EnumValues(const Values& values_) : values(values_), name_to_value_map(std::make_unique<NameToValueMap>()) {
  if (values.empty()) throw Exception(ErrorCodes::EMPTY_DATA_PASSED, "DataTypeEnum enumeration cannot be empty");

  ::sort(std::begin(values), std::end(values), [](auto& left, auto& right) { return left.second < right.second; });
...
void EnumValues<T>::fillMaps() {
  for (const auto& name_and_value : values) {
    const auto inserted_value = name_to_value_map->insert({std::string_view{name_and_value.first}, name_and_value.second});

    if (!inserted_value.second)
      throw Exception(ErrorCodes::SYNTAX_ERROR, "Duplicate names in enum: '{}' = {} and {}", name_and_value.first,
                      toString(name_and_value.second), toString(inserted_value.first->getMapped()));

    const auto inserted_name = value_to_name_map.insert({name_and_value.second, std::string_view{name_and_value.first}});

    if (!inserted_name.second)
      throw Exception(ErrorCodes::SYNTAX_ERROR, "Duplicate values in enum: '{}' = {} and '{}'", name_and_value.first,
                      toString(name_and_value.second), toString((*inserted_name.first).first));
  }
}
...
EnumValues<T>::ValueToNameMap::const_iterator EnumValues<T>::findByValue(const T& value) const {
  ...
  if (it == value_to_name_map.end()) throw Exception(ErrorCodes::UNKNOWN_ELEMENT_OF_ENUM, "Unexpected value {} in enum", toString(value));
...
Names EnumValues<T>::getAllRegisteredNames() const {
  Names result;
  for (const auto& value : values) result.emplace_back(value.first);
  return result;
}
...
std::unordered_set<String> EnumValues<T>::getSetOfAllNames(bool to_lower) const {
  ...
  for (const auto& value : values) result.insert(to_lower ? boost::algorithm::to_lower_copy(value.first) : value.first);
  return result;
}
...
std::unordered_set<T> EnumValues<T>::getSetOfAllValues() const {
  ...
  for (const auto& value : values) result.insert(value.second);
  return result;
}
...
bool EnumValues<T>::containsAll(const TValues& rhs_values) const {
  auto check = [&](const auto& value) {
    ...
    if (it == name_to_value_map->end()) return value_to_name_map.contains(static_cast<T>(value.second));
