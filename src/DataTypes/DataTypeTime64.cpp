...
DataTypeTime64::DataTypeTime64(UInt32 scale_) : DataTypeDecimalBase<Time64>(DecimalUtils::max_precision<Time64>, scale_) {
...
std::string DataTypeTime64::doGetName() const { return std::string(getFamilyName()) + "(" + std::to_string(this->scale) + ")"; }

void DataTypeTime64::updateHashImpl(SipHash &hash) const { hash.update(this->scale); }

bool DataTypeTime64::equals(const IDataType &rhs) const {
  if (const auto *ptype = typeid_cast<const DataTypeTime64 *>(&rhs)) return this->scale == ptype->getScale();
  return false;
}
...
const DateLUTImpl &DataTypeTime64::getTimeZone() const { return DateLUT::instance(); }

std::string getTimeTimezone(const IDataType &data_type) {
  if (typeid_cast<const DataTypeTime *>(&data_type) || typeid_cast<const DataTypeTime64 *>(&data_type)) return std::string();
