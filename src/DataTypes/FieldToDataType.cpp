...
DataTypePtr FieldToDataType<on_error>::operator()(const Array &x) const {
  ...
  for (const Field &elem : x) element_types.emplace_back(applyVisitor(*this, elem));
...
DataTypePtr FieldToDataType<on_error>::operator()(const Tuple &tuple) const {
  ...
  for (const auto &element : tuple) element_types.push_back(applyVisitor(*this, element));
...
DataTypePtr FieldToDataType<on_error>::operator()(const Map &map) const {
  ...
  return std::make_shared<DataTypeMap>(getLeastSupertype<on_error>(key_types), getLeastSupertype<on_error>(value_types));
}
