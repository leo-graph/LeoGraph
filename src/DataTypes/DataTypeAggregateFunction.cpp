...
DataTypeAggregateFunction::DataTypeAggregateFunction(AggregateFunctionPtr function_, const DataTypes &argument_types_,
    ...
    : function(std::move(function_)), argument_types(argument_types_), parameters(parameters_), version(version_) {}

String DataTypeAggregateFunction::getFunctionName() const { return function->getName(); }

String DataTypeAggregateFunction::doGetName() const { return getNameImpl(true); }

String DataTypeAggregateFunction::getNameWithoutVersion() const { return getNameImpl(false); }
...
DataTypePtr DataTypeAggregateFunction::getReturnType() const { return function->getResultType(); }

DataTypePtr DataTypeAggregateFunction::getReturnTypeToPredict() const { return function->getReturnTypeToPredict(); }

bool DataTypeAggregateFunction::isVersioned() const { return function->isVersioned(); }
...
String DataTypeAggregateFunction::getNameImpl(bool with_version) const {
  ...
  if (with_version && data_type_version) stream << data_type_version << ", ";
...
MutableColumnPtr DataTypeAggregateFunction::createColumn() const { return ColumnAggregateFunction::create(function, getVersion()); }
...
bool DataTypeAggregateFunction::strictEquals(const DataTypePtr &lhs_state_type, const DataTypePtr &rhs_state_type) {
  ...
  if (!lhs_state || !rhs_state) return false;

  if (lhs_state->function->getName() != rhs_state->function->getName()) return false;

  if (lhs_state->parameters.size() != rhs_state->parameters.size()) return false;

  for (size_t i = 0; i < lhs_state->parameters.size(); ++i)
    if (lhs_state->parameters[i] != rhs_state->parameters[i]) return false;

  if (lhs_state->argument_types.size() != rhs_state->argument_types.size()) return false;

  for (size_t i = 0; i < lhs_state->argument_types.size(); ++i)
    if (!lhs_state->argument_types[i]->equals(*rhs_state->argument_types[i])) return false;
...
void DataTypeAggregateFunction::updateHashImpl(SipHash &hash) const {
  ...
  for (const auto &param : parameters) hash.update(param.getType());
  hash.update(argument_types.size());
  for (const auto &arg_type : argument_types) arg_type->updateHash(hash);
  if (version) hash.update(*version);
}
...
static DataTypePtr create(const ASTPtr &arguments) {
  ...
  if (const auto *parametric = data_type_ast->as<ASTFunction>()) {
    if (parametric->parameters) throw Exception(ErrorCodes::SYNTAX_ERROR, "Unexpected level of parameters to aggregate function");
  ...
  if (function_name.empty()) throw Exception(ErrorCodes::LOGICAL_ERROR, "Empty name of aggregate function passed");
...
void registerDataTypeAggregateFunction(DataTypeFactory &factory) { factory.registerDataType("AggregateFunction", create); }

bool hasAggregateFunctionType(const DataTypePtr &type) {
  auto result = false;
  auto check = [&](const IDataType &t) { result |= WhichDataType(t).isAggregateFunction(); };
