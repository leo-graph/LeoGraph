...
DataTypeLowCardinality::DataTypeLowCardinality(DataTypePtr dictionary_type_) : dictionary_type(std::move(dictionary_type_)) {
  auto inner_type = dictionary_type;
  if (dictionary_type->isNullable()) inner_type = static_cast<const DataTypeNullable &>(*dictionary_type).getNestedType();
...
struct CreateColumnVector {
  ...
  void operator()(TypeList<T>) {
    if (typeid_cast<const DataTypeNumber<T> *>(&keys_type)) column = creator(static_cast<ColumnVector<T> *>(nullptr));
  }
};
}  // namespace
...
MutableColumnUniquePtr DataTypeLowCardinality::createColumnUniqueImpl(const IDataType &keys_type, const Creator &creator) {
  ...
  if (const auto *nullable_type = typeid_cast<const DataTypeNullable *>(&keys_type)) type = nullable_type->getNestedType().get();

  WhichDataType which(type);

  if (which.isString()) return creator(static_cast<ColumnString *>(nullptr));
  if (which.isFixedString()) return creator(static_cast<ColumnFixedString *>(nullptr));
  if (which.isDate()) return creator(static_cast<ColumnVector<UInt16> *>(nullptr));
  if (which.isDate32()) return creator(static_cast<ColumnVector<Int32> *>(nullptr));
  if (which.isDateTime()) return creator(static_cast<ColumnVector<UInt32> *>(nullptr));
  if (which.isTime()) return creator(static_cast<ColumnVector<Int32> *>(nullptr));
  if (which.isUUID()) return creator(static_cast<ColumnVector<UUID> *>(nullptr));
  if (which.isIPv4()) return creator(static_cast<ColumnVector<IPv4> *>(nullptr));
  if (which.isIPv6()) return creator(static_cast<ColumnVector<IPv6> *>(nullptr));
  if (which.isInterval()) return creator(static_cast<DataTypeInterval::ColumnType *>(nullptr));
  if (which.isInt() || which.isUInt() || which.isFloat()) {
    ...
    if (!column) throw Exception(ErrorCodes::LOGICAL_ERROR, "Unexpected numeric type: {}", type->getName());

    return column;
  }

  throw Exception(ErrorCodes::LOGICAL_ERROR, "Unexpected dictionary type for DataTypeLowCardinality: {}", type->getName());
}
...
Field DataTypeLowCardinality::getDefault() const { return dictionary_type->getDefault(); }
...
void DataTypeLowCardinality::updateHashImpl(SipHash &hash) const { dictionary_type->updateHash(hash); }
...
void registerDataTypeLowCardinality(DataTypeFactory &factory) { factory.registerDataType("LowCardinality", create); }
...
DataTypePtr removeLowCardinalityAndNullable(const DataTypePtr &type) { return removeNullable(removeLowCardinality(type)); };
}  // namespace DB
