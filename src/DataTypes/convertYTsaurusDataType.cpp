...
bool isTypeComplex(const Poco::Dynamic::Var &item) { return !item.isString(); }
...
bool isYTSaurusTypesCompatible(std::shared_ptr<const IDataType> ch_type, std::shared_ptr<const IDataType> yt_type, bool allow_nullable) {
  return yt_type->getColumnType() == TypeIndex::Dynamic || ch_type->getColumnType() == TypeIndex::Dynamic || ch_type->equals(*yt_type) ||
         (allow_nullable && yt_type->getColumnType() == TypeIndex::Nullable &&
