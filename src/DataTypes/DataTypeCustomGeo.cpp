...
void registerDataTypeDomainGeo(DataTypeFactory& factory) {
  ...
  factory.registerSimpleDataTypeCustom("Geometry", [] {
    ...
    auto variant_type = std::make_shared<DataTypeVariant>(
        std::vector{point_type, linestring_type, polygon_type, multipolygon_type, ring_type, multi_linestring_type});

    return std::make_pair(variant_type, std::make_unique<DataTypeCustomDesc>(std::make_unique<DataTypeGeometryName>()));
  });
