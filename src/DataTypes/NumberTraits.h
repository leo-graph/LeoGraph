...
constexpr size_t min(size_t x, size_t y) { return x < y ? x : y; }
...
struct ResultOfAdditionMultiplication {
  using Type = typename Construct<is_signed_v<A> || is_signed_v<B>, is_floating_point<A> || is_floating_point<B>,
                                  nextSize(max(sizeof(A), sizeof(B)))>::Type;
};

template <typename A, typename B>
struct ResultOfSubtraction {
  using Type = typename Construct<true, is_floating_point<A> || is_floating_point<B>, nextSize(max(sizeof(A), sizeof(B)))>::Type;
};
...
struct ResultOfIntegerDivision {
  using Type = typename Construct<is_signed_v<A> || is_signed_v<B>, false, sizeof(A)>::Type;
};
...
struct ResultOfNegate {
  using Type = typename Construct<true, is_floating_point<A>, is_signed_v<A> ? sizeof(A) : nextSize(sizeof(A))>::Type;
};

template <typename A>
struct ResultOfAbs {
  using Type = typename Construct<false, is_floating_point<A>, sizeof(A)>::Type;
};
...
template <typename A, typename B>
struct ResultOfBit {
  using Type = typename Construct<is_signed_v<A> || is_signed_v<B>, false,
                                  is_floating_point<A> || is_floating_point<B> ? 8 : max(sizeof(A), sizeof(B))>::Type;
};

template <typename A>
struct ResultOfBitNot {
  using Type = typename Construct<is_signed_v<A>, false, sizeof(A)>::Type;
};
...
struct ResultOfIf {
  ...
  using Type = std::conditional_t<std::is_same_v<A, B>, A,
                                  std::conditional_t<is_decimal<A> && is_decimal<B>, std::conditional_t<(sizeof(A) > sizeof(B)), A, B>,
                                                     std::conditional_t<!is_decimal<A> && !is_decimal<B>, ConstructedType, Error>>>;
};
...
struct ResultOfModuloNativePromotion {
  ...
  using ConstructedType = typename Construct<has_signed, false,
                                             (has_signed ^ has_unsigned) ? max(max_size_of_unsigned_integer * 2, max_size_of_signed_integer)
                                                                         : max(sizeof(A), sizeof(B))>::Type;
...
template <typename A>
struct ToInteger {
  using Type = typename Construct<is_signed_v<A>, false, is_floating_point<A> ? 8 : sizeof(A)>::Type;
};
...
constexpr bool LeastGreatestSpecialCase =
    std::is_integral_v<A> && std::is_integral_v<B> && (8 == sizeof(A) && sizeof(A) == sizeof(B)) && (is_signed_v<A> ^ is_signed_v<B>);
/// NOLINTEND(misc-redundant-expression)

template <typename A, typename B>
using ResultOfLeast =
    std::conditional_t<LeastGreatestSpecialCase<A, B>, typename Construct<true, false, sizeof(A)>::Type, typename ResultOfIf<A, B>::Type>;

template <typename A, typename B>
using ResultOfGreatest =
    std::conditional_t<LeastGreatestSpecialCase<A, B>, typename Construct<false, false, sizeof(A)>::Type, typename ResultOfIf<A, B>::Type>;
