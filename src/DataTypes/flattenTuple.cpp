...
void flattenTupleTypeImpl(const DataTypePtr &type, const String &current_path, std::vector<String> &flattened_paths,
...
void flattenTupleColumnImpl(const ColumnPtr &column, Columns &flattened_columns, Columns &offsets_columns) {
  if (const auto *column_tuple = checkAndGetColumn<ColumnTuple>(removeNullable(column).get())) {
    ...
    for (const auto &subcolumn : subcolumns) flattenTupleColumnImpl(subcolumn, flattened_columns, offsets_columns);
    ...
    if (!offsets_columns.empty()) {
      ...
      for (auto it = offsets_columns.rbegin() + 1; it != offsets_columns.rend(); ++it) new_column = ColumnArray::create(new_column, *it);
