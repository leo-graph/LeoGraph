...
inline std::optional<UInt32> tryGetDecimalScale(const IDataType &data_type) {
  if (const auto *decimal_type = checkDecimal<Decimal32>(data_type)) return decimal_type->getScale();
  if (const auto *decimal_type = checkDecimal<Decimal64>(data_type)) return decimal_type->getScale();
  if (const auto *decimal_type = checkDecimal<Decimal128>(data_type)) return decimal_type->getScale();
  if (const auto *decimal_type = checkDecimal<Decimal256>(data_type)) return decimal_type->getScale();
  if (const auto *date_time_type = typeid_cast<const DataTypeDateTime64 *>(&data_type)) return date_time_type->getScale();
  if (const auto *time_type = typeid_cast<const DataTypeTime64 *>(&data_type)) return time_type->getScale();
  return {};
}

inline UInt32 getDecimalScale(const IDataType &data_type) {
  if (auto scale = tryGetDecimalScale(data_type)) return *scale;
...
inline UInt32 getDecimalPrecision(const IDataType &data_type) {
  if (const auto *decimal_type = checkDecimal<Decimal32>(data_type)) return decimal_type->getPrecision();
  if (const auto *decimal_type = checkDecimal<Decimal64>(data_type)) return decimal_type->getPrecision();
  if (const auto *decimal_type = checkDecimal<Decimal128>(data_type)) return decimal_type->getPrecision();
  if (const auto *decimal_type = checkDecimal<Decimal256>(data_type)) return decimal_type->getPrecision();
  if (const auto *date_time_type = typeid_cast<const DataTypeDateTime64 *>(&data_type)) return date_time_type->getPrecision();
  if (const auto *time_type = typeid_cast<const DataTypeTime64 *>(&data_type)) return time_type->getPrecision();
...
ReturnType ALWAYS_INLINE convertDecimalsImpl(const typename FromDataType::FieldType &value, UInt32 scale_from, UInt32 scale_to,
                                             typename ToDataType::FieldType &result);

#define DISPATCH(FROM_DATA_TYPE, TO_DATA_TYPE)                                                                                        \
  extern template ALWAYS_INLINE void convertDecimalsImpl<FROM_DATA_TYPE, TO_DATA_TYPE, void>(                                         \
      const typename FROM_DATA_TYPE::FieldType &value, UInt32 scale_from, UInt32 scale_to, typename TO_DATA_TYPE::FieldType &result); \
  extern template ALWAYS_INLINE bool convertDecimalsImpl<FROM_DATA_TYPE, TO_DATA_TYPE, bool>(                                         \
      const typename FROM_DATA_TYPE::FieldType &value, UInt32 scale_from, UInt32 scale_to, typename TO_DATA_TYPE::FieldType &result);
...
#define DISPATCH(FROM_DATA_TYPE, TO_DATA_TYPE)                                                    \
  extern template typename TO_DATA_TYPE::FieldType convertDecimals<FROM_DATA_TYPE, TO_DATA_TYPE>( \
      const typename FROM_DATA_TYPE::FieldType &value, UInt32 scale_from, UInt32 scale_to);
#define INVOKE(X) FOR_EACH_DECIMAL_TYPE_PASS(DISPATCH, X)
FOR_EACH_DECIMAL_TYPE(INVOKE);
#undef INVOKE
#undef DISPATCH
...
template <typename FromDataType, typename ToDataType>
  requires(IsDataTypeDecimal<FromDataType> && IsDataTypeDecimal<ToDataType>)
bool tryConvertDecimals(const typename FromDataType::FieldType &value, UInt32 scale_from, UInt32 scale_to,
                        typename ToDataType::FieldType &result);

#define DISPATCH(FROM_DATA_TYPE, TO_DATA_TYPE)                           \
  extern template bool tryConvertDecimals<FROM_DATA_TYPE, TO_DATA_TYPE>( \
      const typename FROM_DATA_TYPE::FieldType &value, UInt32 scale_from, UInt32 scale_to, typename TO_DATA_TYPE::FieldType &result);
#define INVOKE(X) FOR_EACH_DECIMAL_TYPE_PASS(DISPATCH, X)
FOR_EACH_DECIMAL_TYPE(INVOKE);
#undef INVOKE
...
#define DISPATCH(FROM_DATA_TYPE, TO_DATA_TYPE)                                                                                             \
  extern template void convertFromDecimalImpl<FROM_DATA_TYPE, TO_DATA_TYPE>(const typename FROM_DATA_TYPE::FieldType &value, UInt32 scale, \
                                                                            typename TO_DATA_TYPE::FieldType &result);                     \
  extern template bool convertFromDecimalImpl<FROM_DATA_TYPE, TO_DATA_TYPE>(const typename FROM_DATA_TYPE::FieldType &value, UInt32 scale, \
                                                                            typename TO_DATA_TYPE::FieldType &result);
#define INVOKE(X) FOR_EACH_DECIMAL_TYPE_PASS(DISPATCH, X)
...
#define DISPATCH(FROM_DATA_TYPE, TO_DATA_TYPE)                                                       \
  extern template typename TO_DATA_TYPE::FieldType convertFromDecimal<FROM_DATA_TYPE, TO_DATA_TYPE>( \
      const typename FROM_DATA_TYPE::FieldType &value, UInt32 scale);
#define INVOKE(X) FOR_EACH_DECIMAL_TYPE_PASS(DISPATCH, X)
FOR_EACH_ARITHMETIC_TYPE(INVOKE);
#undef INVOKE
#undef DISPATCH
...
#define DISPATCH(FROM_DATA_TYPE, TO_DATA_TYPE)                                                                                            \
  extern template bool tryConvertFromDecimal<FROM_DATA_TYPE, TO_DATA_TYPE>(const typename FROM_DATA_TYPE::FieldType &value, UInt32 scale, \
                                                                           typename TO_DATA_TYPE::FieldType &result);
#define INVOKE(X) FOR_EACH_DECIMAL_TYPE_PASS(DISPATCH, X)
FOR_EACH_ARITHMETIC_TYPE(INVOKE);
#undef INVOKE
...
#define DISPATCH(FROM_DATA_TYPE, TO_DATA_TYPE)                                                                                           \
  extern template void convertToDecimalImpl<FROM_DATA_TYPE, TO_DATA_TYPE>(const typename FROM_DATA_TYPE::FieldType &value, UInt32 scale, \
                                                                          typename TO_DATA_TYPE::FieldType &result);                     \
  extern template bool convertToDecimalImpl<FROM_DATA_TYPE, TO_DATA_TYPE>(const typename FROM_DATA_TYPE::FieldType &value, UInt32 scale, \
                                                                          typename TO_DATA_TYPE::FieldType &result);
...
#define DISPATCH(FROM_DATA_TYPE, TO_DATA_TYPE)                                                     \
  extern template typename TO_DATA_TYPE::FieldType convertToDecimal<FROM_DATA_TYPE, TO_DATA_TYPE>( \
      const typename FROM_DATA_TYPE::FieldType &value, UInt32 scale);
#define INVOKE(X) FOR_EACH_ARITHMETIC_TYPE_PASS(DISPATCH, X)
FOR_EACH_DECIMAL_TYPE(INVOKE);
#undef INVOKE
...
#define DISPATCH(FROM_DATA_TYPE, TO_DATA_TYPE)                                                                                          \
  extern template bool tryConvertToDecimal<FROM_DATA_TYPE, TO_DATA_TYPE>(const typename FROM_DATA_TYPE::FieldType &value, UInt32 scale, \
                                                                         typename TO_DATA_TYPE::FieldType &result);
#define INVOKE(X) FOR_EACH_ARITHMETIC_TYPE_PASS(DISPATCH, X)
FOR_EACH_DECIMAL_TYPE(INVOKE);
#undef INVOKE
