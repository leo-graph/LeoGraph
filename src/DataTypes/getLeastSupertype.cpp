...
DataTypePtr throwOrReturn(const DataTypes &types, std::string_view message_suffix, int error_code) {
  if constexpr (on_error == LeastSupertypeOnError::String) return std::make_shared<DataTypeString>();

  if constexpr (on_error == LeastSupertypeOnError::Null) return nullptr;

  if (message_suffix.empty()) throw Exception(error_code, "There is no supertype for types {}", getExceptionMessagePrefix(types));
...
DataTypePtr getNumericType(const TypeIndexSet &types) {
  ...
  if (max_bits_of_signed_integer || max_bits_of_unsigned_integer || max_mantissa_bits_of_floating) {
    ...
    if (max_mantissa_bits_of_floating) {
      ...
      if (min_mantissa_bits <= 53) return std::make_shared<DataTypeFloat64>();
    ...
    if (max_bits_of_signed_integer) {
      if (min_bit_width_of_integer <= 8) return std::make_shared<DataTypeInt8>();
      if (min_bit_width_of_integer <= 16) return std::make_shared<DataTypeInt16>();
      if (min_bit_width_of_integer <= 32) return std::make_shared<DataTypeInt32>();
      if (min_bit_width_of_integer <= 64) return std::make_shared<DataTypeInt64>();
      if (min_bit_width_of_integer <= 128) return std::make_shared<DataTypeInt128>();
      if (min_bit_width_of_integer <= 256) return std::make_shared<DataTypeInt256>();
      return throwOrReturn<on_error>(
          types,
          " because some of them are signed integers and some are unsigned integers,"
          " but there is no signed integer type, that can exactly represent all required unsigned integer values",
    ...
    /// All unsigned.
    {
      if (min_bit_width_of_integer <= 8) return std::make_shared<DataTypeUInt8>();
      if (min_bit_width_of_integer <= 16) return std::make_shared<DataTypeUInt16>();
      if (min_bit_width_of_integer <= 32) return std::make_shared<DataTypeUInt32>();
      if (min_bit_width_of_integer <= 64) return std::make_shared<DataTypeUInt64>();
      if (min_bit_width_of_integer <= 128) return std::make_shared<DataTypeUInt128>();
      if (min_bit_width_of_integer <= 256) return std::make_shared<DataTypeUInt256>();
      return throwOrReturn<on_error>(types,
...
void convertUInt64toInt64IfPossible(const DataTypes &types, TypeIndexSet &types_set) {
  ...
  if (!types_set.contains(TypeIndex::UInt64) || (!types_set.contains(TypeIndex::Int8) && !types_set.contains(TypeIndex::Int16) &&
                                                 !types_set.contains(TypeIndex::Int32) && !types_set.contains(TypeIndex::Int64)))
    return;
...
DataTypePtr findSmallestIntervalSuperType(const DataTypes &types, TypeIndexSet &types_set) {
  ...
  for (const auto &type : types) {
    if (const auto *interval_type = typeid_cast<const DataTypeInterval *>(type.get())) {
      ...
      if (current_interval > IntervalKind::Kind::Week) is_higher_interval = true;
  ...
  if (is_higher_interval && min_interval <= IntervalKind::Kind::Week)
    throw Exception(ErrorCodes::NO_COMMON_TYPE, "Cannot compare intervals {} and {} because the number of days in a month is not fixed",
                    types[0]->getName(), types[1]->getName());
...
DataTypePtr getLeastSuperTypeForTuple(const DataTypes &types) {
  ...
  for (const auto &type : types) {
    ...
    if (const auto *type_tuple = typeid_cast<const DataTypeTuple *>(unwrapped_type)) {
      ...
      if (element_types.empty()) {
        ...
        for (size_t i = 0; i < element_size; ++i) element_types[i].reserve(types.size());
        if (type_tuple->hasExplicitNames()) element_names = type_tuple->getElementNames();
      }
      ...
      for (size_t i = 0; i < element_size; ++i) element_types[i].emplace_back(current_elements[i]);

      // If there are different names, drop all names. The result will be an unnamed tuple.
      if (element_names.empty() == type_tuple->hasExplicitNames()) element_names.clear();
  ...
  if (have_nullable && result_type->canBeInsideNullable()) result_type = std::make_shared<DataTypeNullable>(result_type);
...
DataTypePtr getLeastSupertype(const DataTypes &types) {
  ...
  /// If one of the types is Dynamic, the supertype is Dynamic
  {
    ...
    if (have_dynamic) return std::make_shared<DataTypeDynamic>(max_dynamic_types);
  }
  ...
  /// If there are Nothing types, skip them
  {
    ...
    for (const auto &type : types)
      if (!typeid_cast<const DataTypeNothing *>(type.get())) non_nothing_types.emplace_back(type);

    if (non_nothing_types.size() < types.size()) return getLeastSupertype<on_error>(non_nothing_types);
  }
  ...
  /// For tuples
  {
    ...
    if (have_tuple) return getLeastSuperTypeForTuple<on_error>(types);
  }
  ...
  /// For maps
  {
    ...
    if (have_maps) {
      ...
      if (!keys_common_type || !values_common_type) return nullptr;
  ...
  /// For LowCardinality. This is above Nullable, because LowCardinality can contain Nullable but cannot be inside Nullable.
  {
    ...
    if (have_low_cardinality) {
      if (have_not_low_cardinality) return getLeastSupertype<on_error>(nested_types);
  ...
  /// For Nullable
  {
    ...
    for (const auto &type : types) {
      if (const DataTypeNullable *type_nullable = typeid_cast<const DataTypeNullable *>(type.get())) {
        ...
        if (!type_nullable->onlyNull()) nested_types.emplace_back(type_nullable->getNestedType());
      } else
        nested_types.emplace_back(type);
    }
    ...
    if (have_nullable) {
      ...
      if (isVariant(nested_type)) return nested_type;
    ...
    if (have_object) {
      ...
      for (size_t i = 1; i < types.size(); ++i) {
        ...
        for (auto &[path, type] : merged_typed_paths) {
          ...
          if (it == current_typed_paths.end()) continue;  /// Path not in current type — drop it.

          auto common_type = getLeastSupertype<on_error>(DataTypes{type, it->second});
          if (!common_type) continue;  /// Types are incompatible — drop the path.
        ...
        std::set_intersection(merged_paths_to_skip.begin(), merged_paths_to_skip.end(), skip_set.begin(), skip_set.end(),
        ...
        std::set_intersection(merged_regexps_to_skip.begin(), merged_regexps_to_skip.end(), regexp_set.begin(), regexp_set.end(),
      ...
      return std::make_shared<DataTypeObject>(schema_format, std::move(merged_typed_paths),
                                              ...
                                              merged_max_dynamic_paths, merged_max_dynamic_types);
    }
  }
  ...
  /// No other types are compatible with Strings.
  {
    ...
    if (have_string || have_fixed_string) {
      ...
      if (!all_compatible_with_string)
        return throwOrReturn<on_error>(types, "because some of them are String/FixedString/Enum and some of them are not",
                                       ErrorCodes::NO_COMMON_TYPE);
  ...
  /// For Date and DateTime/DateTime64, the common type is DateTime/DateTime64. No other types are compatible.
  {
    ...
    if (have_date || have_date32 || have_datetime || have_datetime64) {
      ...
      if (!all_date_or_datetime)
        return throwOrReturn<on_error>(types, "because some of them are Date/Date32/DateTime/DateTime64 and some of them are not",
                                       ErrorCodes::NO_COMMON_TYPE);

      if (have_datetime64 == 0 && have_date32 == 0) {
        for (const auto &type : types) {
          if (isDateTime(type)) return type;
        }
...

  {
    ...
    if (have_time || have_time64) {
      ...
      if (!all_time_or_time64)
        return throwOrReturn<on_error>(types, "because some of them are Time/Time64 and some of them are not", ErrorCodes::NO_COMMON_TYPE);
  ...
  /// Decimals
  {
    ...
    if (have_decimal32 || have_decimal64 || have_decimal128 || have_decimal256) {
      ...
      if (min_precision > DataTypeDecimal<Decimal256>::maxPrecision())
        return throwOrReturn<on_error>(
            types, "because the least supertype is Decimal(" + toString(min_precision) + ',' + toString(max_scale) + ')',
            ErrorCodes::NO_COMMON_TYPE);
...
DataTypePtr getLeastSupertypeOrString(const DataTypes &types) { return getLeastSupertype<LeastSupertypeOnError::String>(types); }
...
DataTypePtr getLeastSupertypeOrVariant(const DataTypes &types) { return getLeastSupertype<LeastSupertypeOnError::Variant>(types); }

DataTypePtr tryGetLeastSupertype(const DataTypes &types) { return getLeastSupertype<LeastSupertypeOnError::Null>(types); }
...
DataTypePtr getLeastSupertype(const TypeIndexSet &types) {
  ...
  if (types.size() == 1) {
    ...
    if (which.isNothing()) return std::make_shared<DataTypeNothing>();

#define DISPATCH(TYPE) \
  if (which.idx == TypeIndex::TYPE) return std::make_shared<DataTypeNumber<TYPE>>();  /// NOLINT

    FOR_NUMERIC_TYPES(DISPATCH)
#undef DISPATCH

    if (which.isString()) return std::make_shared<DataTypeString>();

    return throwOrReturn<on_error>(types, "because cannot get common type by type indexes with non-simple types",
                                   ErrorCodes::NO_COMMON_TYPE);
  }
...
DataTypePtr getLeastSupertypeOrString(const TypeIndexSet &types) { return getLeastSupertype<LeastSupertypeOnError::String>(types); }

DataTypePtr tryGetLeastSupertype(const TypeIndexSet &types) { return getLeastSupertype<LeastSupertypeOnError::Null>(types); }
