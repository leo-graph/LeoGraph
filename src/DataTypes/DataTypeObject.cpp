...
DataTypeObject::DataTypeObject(const SchemaFormat &schema_format_, std::unordered_map<String, DataTypePtr> typed_paths_,
                               std::unordered_set<String> paths_to_skip_, std::vector<String> path_regexps_to_skip_,
                               size_t max_dynamic_paths_, size_t max_dynamic_types_)
  ...
  for (const auto &regexp_str : path_regexps_to_skip) {
    ...
    if (!regexp.ok()) throw Exception(ErrorCodes::CANNOT_COMPILE_REGEXP, "Invalid regexp '{}': {}", regexp_str, regexp.error());
  }
  ...
  for (const auto &[typed_path, type] : typed_paths) {
    for (const auto &path_to_skip : paths_to_skip) {
      if (typed_path.starts_with(path_to_skip))
        throw Exception(ErrorCodes::BAD_ARGUMENTS, "Path '{}' is specified with the data type ('{}') and matches the SKIP path prefix '{}'",
                        typed_path, type->getName(), path_to_skip);
    }

    for (const auto &path_regex_to_skip : path_regexps_to_skip) {
      if (re2::RE2::FullMatch(typed_path, re2::RE2(path_regex_to_skip)))
        throw Exception(ErrorCodes::BAD_ARGUMENTS, "Path '{}' is specified with the data type ('{}') and matches the SKIP REGEXP '{}'",
                        typed_path, type->getName(), path_regex_to_skip);
    }
  }
}
...
DataTypeObject::DataTypeObject(const DB::DataTypeObject::SchemaFormat &schema_format_, size_t max_dynamic_paths_, size_t max_dynamic_types_)
    : schema_format(schema_format_), max_dynamic_paths(max_dynamic_paths_), max_dynamic_types(max_dynamic_types_) {}

bool DataTypeObject::equals(const IDataType &rhs) const {
  if (const auto *object = typeid_cast<const DataTypeObject *>(&rhs)) {
    if (typed_paths.size() != object->typed_paths.size()) return false;

    for (const auto &[path, type] : typed_paths) {
      auto it = object->typed_paths.find(path);
      if (it == object->typed_paths.end()) return false;
      if (!type->equals(*it->second)) return false;
    }

    return schema_format == object->schema_format && paths_to_skip == object->paths_to_skip &&
           path_regexps_to_skip == object->path_regexps_to_skip && max_dynamic_types == object->max_dynamic_types &&
           max_dynamic_paths == object->max_dynamic_paths;
  }
...
SerializationPtr DataTypeObject::doGetSerialization(const SerializationInfoSettings &settings) const {
  ...
  for (const auto &[path, type] : typed_paths)
    typed_paths_serializations[path] = settings.propagate_types_serialization_versions_to_nested_types ? type->getSerialization(settings)
  ...
  switch (schema_format) {
    case SchemaFormat::JSON:
      ...
      if (!context) context = Context::getGlobalContextInstance();
      if (context->getSettingsRef()[Setting::allow_simdjson])
        return std::make_shared<SerializationJSON<SimdJSONParser>>(typed_paths, typed_paths_serializations, paths_to_skip,
                                                                   path_regexps_to_skip, getDynamicType(), dynamic_serialization,
                                                                   buildJSONExtractTree<SimdJSONParser>(getPtr(), "JSON serialization"));
#endif

#if USE_RAPIDJSON
      return std::make_shared<SerializationJSON<RapidJSONParser>>(typed_paths, typed_paths_serializations, paths_to_skip,
                                                                  path_regexps_to_skip, getDynamicType(), dynamic_serialization,
                                                                  buildJSONExtractTree<RapidJSONParser>(getPtr(), "JSON serialization"));
#else
      return std::make_shared<SerializationJSON<DummyJSONParser>>(typed_paths, typed_paths_serializations, paths_to_skip,
                                                                  path_regexps_to_skip, getDynamicType(), dynamic_serialization,
...
String DataTypeObject::getSchemaFormatString() const { return String{magic_enum::enum_name(schema_format)}; }
...
String DataTypeObject::doGetName() const {
  ...
  for (const auto &[path, _] : typed_paths) sorted_typed_paths.push_back(path);
  ...
  for (const auto &skip_path : paths_to_skip) sorted_skip_paths.push_back(skip_path);
...
MutableColumnPtr DataTypeObject::createColumn() const {
  ...
  for (const auto &[path, type] : typed_paths) typed_path_columns[path] = type->createColumn();
...
void replaceJSONTypeNameIfNeeded(String &type_name, const String &nested_json_type_name) {
  ...
  while (pos != String::npos) {
    ...
    if (pos + 4 == type_name.size() || type_name[pos + 4] != '(') type_name.replace(pos, 4, nested_json_type_name);
...
std::pair<String, String> splitPathAndDynamicTypeSubcolumn(std::string_view subcolumn_name, const String &nested_json_type_name) {
  ...
  if (pos == std::string_view::npos) return {String(subcolumn_name), ""};

  ReadBufferFromMemory buf(subcolumn_name.substr(pos + 2));
  String dynamic_subcolumn;
  /// Try to read back quoted type name.
  if (!tryReadBackQuotedString(dynamic_subcolumn, buf)) return {String(subcolumn_name), ""};
  ...
  if (!buf.eof()) dynamic_subcolumn += String(buf.position(), buf.available());
...
std::optional<String> tryGetSubObjectSubcolumn(std::string_view subcolumn_name) {
  if (!subcolumn_name.starts_with("^`")) return std::nullopt;

  ReadBufferFromMemory buf(subcolumn_name.substr(1));
  String path;
  /// Try to read back-quoted first path element.
  if (!tryReadBackQuotedString(path, buf)) return std::nullopt;
...
std::unique_ptr<ISerialization::SubstreamData> DataTypeObject::getDynamicSubcolumnData(std::string_view subcolumn_name,
  ...
  if (subcolumn_name == SPECIAL_SUBCOLUMN_NAME_FOR_DISTINCT_PATHS_CALCULATION) {
    ...
    for (const auto &[path, _] : typed_paths) typed_path_names.push_back(path);

    std::unique_ptr<SubstreamData> res =
        std::make_unique<SubstreamData>(std::make_shared<SerializationObjectDistinctPaths>(typed_path_names));
    ...
    if (data.column) {
      ...
      if (!object_column.empty()) {
        ...
        for (const auto &path : typed_path_names) result_paths_column.insertData(path.data(), path.size());
        for (const auto &[path, _] : object_column.getDynamicPaths()) result_paths_column.insertData(path.data(), path.size());
  ...
  if (auto sub_object_subcolumn = tryGetSubObjectSubcolumn(subcolumn_name)) {
    ...
    for (const auto &[path, type] : typed_paths) {
      if (path.starts_with(prefix)) {
        ...
        typed_sub_paths_serializations[path] = typed_paths_serializations.at(path);
      }
    }

    std::unique_ptr<SubstreamData> res = std::make_unique<SubstreamData>(
        std::make_shared<SerializationSubObject>(prefix, typed_sub_paths_serializations, getDynamicType(), dynamic_path_serialization));
    /// Keep all current constraints like limits and skip paths/prefixes/regexps.
    res->type = std::make_shared<DataTypeObject>(schema_format, typed_sub_paths, paths_to_skip, path_regexps_to_skip, max_dynamic_paths,
                                                 max_dynamic_types);
    /// If column was provided, we should create a column for the requested subcolumn.
    if (data.column) {
      const auto &object_column = assert_cast<const ColumnObject &>(*data.column);
      ...
      for (const auto &[path, column] : object_column.getTypedPaths()) {
        if (path.starts_with(prefix)) result_typed_columns[path.substr(prefix.size())] = column;
      }

      std::vector<std::pair<String, ColumnPtr>> result_dynamic_paths;
      for (const auto &[path, column] : object_column.getDynamicPaths()) {
        if (path.starts_with(prefix)) result_dynamic_paths.emplace_back(path.substr(prefix.size()), column);
      }
  ...
  if (typed_paths.contains(path))
    ...
    res->serialization = std::make_shared<SerializationObjectDynamicPath>(res->serialization, path, path_subcolumn, getDynamicType(),
                                                                          dynamic_path_serialization, res->type);

  return res;
}

static DataTypePtr createObject(const ASTPtr &arguments, const DataTypeObject::SchemaFormat &schema_format) {
  if (!arguments || arguments->children.empty()) return std::make_shared<DataTypeObject>(schema_format);
  ...
  for (const auto &argument : arguments->children) {
    ...
    if (object_type_argument->parameter) {
      ...
      if (!function || function->name != "equals")
        throw Exception(ErrorCodes::UNEXPECTED_AST_STRUCTURE, "Unexpected parameter in {} type arguments: {}",
                        magic_enum::enum_name(schema_format), function->formatForErrorMessage());

      const auto *identifier = function->arguments->children[0]->as<ASTIdentifier>();
      if (!identifier)
        throw Exception(ErrorCodes::UNEXPECTED_AST_STRUCTURE,
                        "Unexpected {} type argument: {}. Expected expression 'max_dynamic_types=N' or 'max_dynamic_paths=N'",
                        magic_enum::enum_name(schema_format), function->formatForErrorMessage());

      auto identifier_name = identifier->name();
      if (identifier_name != "max_dynamic_types" && identifier_name != "max_dynamic_paths")
        throw Exception(ErrorCodes::UNEXPECTED_AST_STRUCTURE,
                        "Unexpected parameter in {} type arguments: {}. Expected 'max_dynamic_types' or `max_dynamic_paths`",
                        magic_enum::enum_name(schema_format), identifier_name);

      auto *literal = function->arguments->children[1]->as<ASTLiteral>();
      size_t max_value =
          identifier_name == "max_dynamic_types" ? ColumnDynamic::MAX_DYNAMIC_TYPES_LIMIT : DataTypeObject::MAX_DYNAMIC_PATHS_LIMIT;
      if (!literal || literal->value.getType() != Field::Types::UInt64 || literal->value.safeGet<UInt64>() > max_value)
        throw Exception(ErrorCodes::UNEXPECTED_AST_STRUCTURE,
                        "'{}' parameter for {} type should be a positive integer between 0 and {}. Got {}", identifier_name,
                        magic_enum::enum_name(schema_format), max_value, function->arguments->children[1]->formatForErrorMessage());
      ...
      if (!identifier)
        throw Exception(ErrorCodes::UNEXPECTED_AST_STRUCTURE,
                        "Unexpected AST in SKIP section of {} type arguments: {}. Expected identifier with path name",
                        magic_enum::enum_name(schema_format), object_type_argument->skip_path->formatForErrorMessage());
  ...
  return std::make_shared<DataTypeObject>(schema_format, std::move(typed_paths), std::move(paths_to_skip), std::move(path_regexps_to_skip),
                                          max_dynamic_paths, max_dynamic_types);
}

const DataTypePtr &DataTypeObject::getTypeOfSharedData() {
  /// Array(Tuple(String, String))
  thread_local static DataTypePtr type = std::make_shared<DataTypeArray>(std::make_shared<DataTypeTuple>(
      DataTypes{std::make_shared<DataTypeString>(), std::make_shared<DataTypeString>()}, Names{"paths", "values"}));
  return type;
}
...
void DataTypeObject::updateHashImpl(SipHash &hash) const {
  ...
  for (const auto &[path, type] : typed_paths) sorted_paths.push_back(path);
  ...
  for (const auto &path : paths_to_skip) hash.update(path);

  // Include path regexps to skip in the hash
  hash.update(path_regexps_to_skip.size());
  for (const auto &regexp : path_regexps_to_skip) hash.update(regexp);
}

DataTypePtr DataTypeObject::getTypeOfNestedObjects() const {
  return std::make_shared<DataTypeObject>(schema_format, max_dynamic_paths / NESTED_OBJECT_MAX_DYNAMIC_PATHS_REDUCE_FACTOR,
                                          max_dynamic_types / NESTED_OBJECT_MAX_DYNAMIC_TYPES_REDUCE_FACTOR);
}

DataTypePtr DataTypeObject::getDynamicType() const { return std::make_shared<DataTypeDynamic>(max_dynamic_types); }

static DataTypePtr createJSON(const ASTPtr &arguments) { return createObject(arguments, DataTypeObject::SchemaFormat::JSON); }

void registerDataTypeJSON(DataTypeFactory &factory) { factory.registerDataType("JSON", createJSON, DataTypeFactory::Case::Insensitive); }
