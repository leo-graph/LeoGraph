...
TypeIndexesSet getTypesIndexes(const DataTypes &types) {
  ...
  for (const auto &type : types) type_indexes.insert(type->getTypeId());
  return type_indexes;
}
...
void transformTypesRecursively(DataTypes &types, std::function<void(DataTypes &, TypeIndexesSet &)> transform_simple_types,
  ...
  if (type_indexes.contains(TypeIndex::Nullable)) {
    ...
    for (size_t i = 0; i != types.size(); ++i) {
      ...
      const bool can_make_nullable =
          format_settings ? canBeInsideNullableBySchemaSettings(nested_types[i], *format_settings) : nested_types[i]->canBeInsideNullable();
  ...
  if (type_indexes.contains(TypeIndex::Array)) {
    ...
    if (type_indexes.size() == 1) {
      DataTypes nested_types;
      for (const auto &type : types) nested_types.push_back(typeid_cast<const DataTypeArray *>(type.get())->getNestedType());

      transformTypesRecursively(nested_types, transform_simple_types, transform_complex_types, format_settings);
      for (size_t i = 0; i != types.size(); ++i) types[i] = std::make_shared<DataTypeArray>(nested_types[i]);
    }

    if (transform_complex_types) transform_complex_types(types, type_indexes);
    ...
    if (type_indexes.size() == 1) {
      ...
      if (sizes_are_equal && all_element_names_are_equal) {
        ...
        for (size_t elem_idx = 0; elem_idx < tuple_size; ++elem_idx) {
          ...
          for (size_t i = 0; i != types.size(); ++i) transposed_nested_types[i].push_back(nested_types[elem_idx][i]);
        }
  ...
  if (type_indexes.contains(TypeIndex::Map)) {
    ...
    if (type_indexes.size() == 1) {
      ...
      for (size_t i = 0; i != types.size(); ++i) types[i] = std::make_shared<DataTypeMap>(key_types[i], value_types[i]);
    }
