...
enum class BinaryTypeIndex : uint8_t {
  ...
  Please don't use 0x33 and 0x35, because older client might try to serialise data as TimeWithTimezone/Time64WithTimezone, and newer server
  would deserialise them as incorrect types. */
  Time64 = 0x34,
  /// reserved = 0x35
  QBit = 0x36
};
...
BinaryTypeIndex getBinaryTypeIndex(const DataTypePtr &type) {
  ...
  if (type->hasCustomName()) {
    ...
    if (isBool(type)) return BinaryTypeIndex::Bool;

    if (typeid_cast<const DataTypeCustomSimpleAggregateFunction *>(type->getCustomName())) return BinaryTypeIndex::SimpleAggregateFunction;

    if (isNested(type)) return BinaryTypeIndex::Nested;
  ...
  switch (type->getTypeId()) {
    ...
    case TypeIndex::DateTime:
      if (assert_cast<const DataTypeDateTime &>(*type).hasExplicitTimeZone()) return BinaryTypeIndex::DateTimeWithTimezone;
      return BinaryTypeIndex::DateTimeUTC;
    case TypeIndex::DateTime64:
      if (assert_cast<const DataTypeDateTime64 &>(*type).hasExplicitTimeZone()) return BinaryTypeIndex::DateTime64WithTimezone;
    ...
    case TypeIndex::Tuple: {
      ...
      if (tuple_type.hasExplicitNames()) return BinaryTypeIndex::NamedTuple;
...
void encodeAggregateFunction(const String &function_name, const Array &parameters, const DataTypes &arguments_types, WriteBuffer &buf) {
  ...
  for (const auto &param : parameters) encodeField(param, buf);
  writeVarUInt(arguments_types.size(), buf);
  for (const auto &argument_type : arguments_types) encodeDataTypeImpl<encode_for_hash_calculation>(argument_type, buf);
}
...
std::tuple<AggregateFunctionPtr, Array, DataTypes> decodeAggregateFunction(ReadBuffer &buf) {
  ...
  if (num_parameters > MAX_ARRAY_SIZE)
    throw Exception(ErrorCodes::INCORRECT_DATA, "Too many parameters during AggregateFunction type decoding: {}. Maximum: {}",
                    num_parameters, MAX_ARRAY_SIZE);

  Array parameters;
  parameters.reserve(num_parameters);
  for (size_t i = 0; i != num_parameters; ++i) parameters.push_back(decodeField(buf));
  size_t num_arguments;
  readVarUInt(num_arguments, buf);
  if (num_arguments > MAX_ARRAY_SIZE)
    throw Exception(ErrorCodes::INCORRECT_DATA, "Too many function arguments during AggregateFunction type decoding: {}. Maximum: {}",
                    num_parameters, MAX_ARRAY_SIZE);

  DataTypes arguments_types;
  arguments_types.reserve(num_arguments);
  for (size_t i = 0; i != num_arguments; ++i) arguments_types.push_back(decodeDataType(buf));
...
void encodeDataTypeImpl(const DataTypePtr &type, WriteBuffer &buf) {
  ...
  switch (binary_type_index) {
    ...
    case BinaryTypeIndex::UnnamedTuple: {
      ...
      for (const auto &element_type : element_types) encodeDataTypeImpl<encode_for_hash_calculation>(element_type, buf);
      break;
    }
    ...
    case BinaryTypeIndex::Variant: {
      ...
      for (const auto &variant : variants) encodeDataTypeImpl<encode_for_hash_calculation>(variant, buf);
      break;
    }
    case BinaryTypeIndex::Dynamic: {
      /// Skip serialization of max_types parameter for hash calculation.
      if constexpr (encode_for_hash_calculation) break;
    ...
    case BinaryTypeIndex::AggregateFunction: {
      ...
      encodeAggregateFunction<encode_for_hash_calculation>(aggregate_function_type.getFunctionName(),
                                                           aggregate_function_type.getParameters(),
                                                           aggregate_function_type.getArgumentsDataTypes(), buf);
      break;
    }
    case BinaryTypeIndex::SimpleAggregateFunction: {
      ...
      encodeAggregateFunction<encode_for_hash_calculation>(simple_aggregate_function_type.getFunctionName(),
                                                           simple_aggregate_function_type.getParameters(),
                                                           simple_aggregate_function_type.getArgumentsDataTypes(), buf);
      break;
    }
    ...
    case BinaryTypeIndex::JSON: {
      ...
      for (const auto &path : paths_to_skip) writeStringBinary(path, buf);
      const auto &path_regexps_to_skip = object_type.getPathRegexpsToSkip();
      writeVarUInt(path_regexps_to_skip.size(), buf);
      for (const auto &regexp : path_regexps_to_skip) writeStringBinary(regexp, buf);
      break;
    }
    default:
      break;
...
void encodeDataType(const DataTypePtr &type, WriteBuffer &buf) { encodeDataTypeImpl<false>(type, buf); }

void encodeDataTypeForHashCalculation(const DataTypePtr &type, WriteBuffer &buf) { encodeDataTypeImpl<true>(type, buf); }
...
DataTypePtr decodeDataType(ReadBuffer &buf, size_t &complexity) {
  ...
  if (max_complexity > 0 && complexity > max_complexity)
    throw Exception(ErrorCodes::INCORRECT_DATA,
                    "Binary type decoding complexity limit exceeded: {} > {} (adjust input_format_binary_max_type_complexity)", complexity,
                    max_complexity);
  if (complexity % 128 == 0) checkStackSize();
  ...
  switch (BinaryTypeIndex(type)) {
    ...
    case BinaryTypeIndex::NamedTuple: {
      ...
      readVarUInt(size, buf);
      if (size > MAX_ARRAY_SIZE)
        throw Exception(ErrorCodes::INCORRECT_DATA, "Too many tuple elements during Tuple type decoding: {}. Maximum: {}", size,
                        MAX_ARRAY_SIZE);
    ...
    case BinaryTypeIndex::UnnamedTuple: {
      ...
      DataTypes elements;
      elements.reserve(size);
      for (size_t i = 0; i != size; ++i) elements.push_back(decodeDataType(buf, complexity));
    ...
    case BinaryTypeIndex::Function: {
      ...
      if (arguments_size > MAX_ARRAY_SIZE)
        throw Exception(ErrorCodes::INCORRECT_DATA, "Too many function arguments during Function type decoding: {}. Maximum: {}",
                        arguments_size, MAX_ARRAY_SIZE);

      DataTypes arguments;
      arguments.reserve(arguments_size);
      for (size_t i = 0; i != arguments_size; ++i) arguments.push_back(decodeDataType(buf, complexity));
    ...
    case BinaryTypeIndex::Variant: {
      ...
      readVarUInt(size, buf);
      if (size > MAX_ARRAY_SIZE)
        throw Exception(ErrorCodes::INCORRECT_DATA, "Too many variants during Variant type decoding: {}. Maximum: {}", size,
                        MAX_ARRAY_SIZE);

      DataTypes variants;
      variants.reserve(size);
      for (size_t i = 0; i != size; ++i) variants.push_back(decodeDataType(buf, complexity));
    ...
    case BinaryTypeIndex::JSON: {
      ...
      if (max_dynamic_paths > DataTypeObject::MAX_DYNAMIC_PATHS_LIMIT)
        throw Exception(ErrorCodes::INCORRECT_DATA, "'max_dynamic_paths' for JSON type is too large: {}. The maximum is {}",
                        max_dynamic_paths, DataTypeObject::MAX_DYNAMIC_PATHS_LIMIT);

      UInt8 max_dynamic_types;
      readBinary(max_dynamic_types, buf);
      if (max_dynamic_types > ColumnDynamic::MAX_DYNAMIC_TYPES_LIMIT)
        throw Exception(ErrorCodes::INCORRECT_DATA, "'max_dynamic_types' for JSON type is too large: {}. The maximum is {}",
                        UInt32(max_dynamic_types), ColumnDynamic::MAX_DYNAMIC_TYPES_LIMIT);

      size_t typed_paths_size;
      readVarUInt(typed_paths_size, buf);
      if (typed_paths_size > DataTypeObject::MAX_TYPED_PATHS)
        throw Exception(ErrorCodes::INCORRECT_DATA, "Too many typed paths during JSON type decoding: {}. Maximum: {}", typed_paths_size,
                        DataTypeObject::MAX_TYPED_PATHS);
      ...
      if (paths_to_skip_size > MAX_ARRAY_SIZE)
        throw Exception(ErrorCodes::INCORRECT_DATA, "Too many paths to skip during JSON type decoding: {}. Maximum: {}", paths_to_skip_size,
                        MAX_ARRAY_SIZE);
      ...
      if (path_regexps_to_skip_size > MAX_ARRAY_SIZE)
        throw Exception(ErrorCodes::INCORRECT_DATA, "Too many path regexps to skip during JSON type decoding: {}. Maximum: {}",
                        paths_to_skip_size, MAX_ARRAY_SIZE);
      ...
      return std::make_shared<DataTypeObject>(DataTypeObject::SchemaFormat::JSON, typed_paths, paths_to_skip, path_regexps_to_skip,
