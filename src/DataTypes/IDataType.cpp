...
String IDataType::getName() const {
  if (custom_name) return custom_name->getName();
  return doGetName();
}
...
ColumnPtr IDataType::createColumnConstWithDefaultValue(size_t size) const { return createColumnConst(size, getDefault()); }
...
std::unique_ptr<IDataType::SubstreamData> IDataType::getSubcolumnData(std::string_view subcolumn_name, const SubstreamData &data,
      ...
      if (!subpath[i].visited && ISerialization::hasSubcolumnForPath(subpath, prefix_len)) {
        ...
        /// Check if this subcolumn is a prefix of requested subcolumn and it can create dynamic subcolumns.
          ...
          auto dynamic_subcolumn_data = subpath[i].data.type->getDynamicSubcolumnData(
              dynamic_subcolumn_name, subpath[i].data, initial_array_level + ISerialization::getArrayLevel(subpath, prefix_len), false);
...
std::unique_ptr<IDataType::SubstreamData> IDataType::getDynamicSubcolumnData(std::string_view /*subcolumn_name*/,
...
bool IDataType::hasSubcolumn(std::string_view subcolumn_name) const { return tryGetSubcolumnType(subcolumn_name) != nullptr; }

bool IDataType::hasDynamicSubcolumns() const {
  if (hasDynamicSubcolumnsData()) return true;
...
void IDataType::insertManyDefaultsInto(IColumn &column, size_t n) const {
  ...
  for (size_t i = 0; i < n; ++i) insertDefaultInto(column);
}

void IDataType::setCustomization(DataTypeCustomDescPtr custom_desc_) const {
  /// replace only if not null
  if (custom_desc_->name) custom_name = std::move(custom_desc_->name);

  if (custom_desc_->serialization) custom_serialization = std::move(custom_desc_->serialization);
}
...
SerializationInfoPtr IDataType::getSerializationInfo(const IColumn &column) const {
  ...
  return std::make_shared<SerializationInfo>(ISerialization::getKindStack(column),
...
SerializationPtr IDataType::getDefaultSerialization() const {
  ...
  if (custom_serialization) return custom_serialization;

  return doGetSerialization(SerializationInfoSettings{});
}

SerializationPtr IDataType::wrapSerializationBasedOnKindStack(SerializationPtr serialization, const ISerialization::KindStack &kind_stack,
