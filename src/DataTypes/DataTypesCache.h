...
class DataTypesCache {
 public:
  ...
  DataTypePtr getType(const String& type_name) { return getCacheElement(type_name).type; }

  SerializationPtr getSerialization(const String& type_name) { return getCacheElement(type_name).serialization; }
 ...
 private:
  const Element& getCacheElement(const String& type_name) {
    ...
    if (cache.size() >= MAX_DATA_TYPES_ELEMENTS) cache.clear();
