...
DataTypeVariant::DataTypeVariant(const DataTypes &variants_) {
  ...
  for (const auto &type : variants_) {
    ...
    if (type->getTypeId() == TypeIndex::Variant) throw Exception(ErrorCodes::BAD_ARGUMENTS, "Nested Variant types are not allowed");
    ...
    if (!isNothing(type)) name_to_type[type->getName()] = type;
  }

  variants.reserve(name_to_type.size());
  for (const auto &[_, type] : name_to_type) variants.push_back(type);

  if (variants.empty()) throw Exception(ErrorCodes::BAD_ARGUMENTS, "Variant type should have at least one nested type");
...
void DataTypeVariant::updateHashImpl(SipHash &hash) const {
  ...
  for (const auto &variant : variants) variant->updateHash(hash);
}
...
MutableColumnPtr DataTypeVariant::createColumn() const {
  ...
  for (size_t i = 0; i < size; ++i) nested_columns.push_back(variants[i]->createColumn());

  return ColumnVariant::create(std::move(nested_columns));
}

Field DataTypeVariant::getDefault() const { return Null(); }
...
bool DataTypeVariant::equals(const IDataType &rhs) const {
  ...
  size_t size = variants.size();
  if (size != rhs_variant.variants.size()) return false;

  for (size_t i = 0; i < size; ++i) {
    if (!variants[i]->equals(*rhs_variant.variants[i])) return false;

    /// The same data types with different custom names considered different.
    /// For example, UInt8 and Bool.
    if ((variants[i]->hasCustomName() || rhs_variant.variants[i]->hasCustomName()) &&
        variants[i]->getName() != rhs_variant.variants[i]->getName())
      return false;
  }
...
std::optional<ColumnVariant::Discriminator> DataTypeVariant::tryGetVariantDiscriminator(const String &type_name) const {
  for (size_t i = 0; i != variants.size(); ++i) {
    if (variants[i]->getName() == type_name) return i;
  }
...
size_t DataTypeVariant::getMaximumSizeOfValueInMemory() const {
  size_t max_size = 0;
  for (const auto &elem : variants) max_size = std::max(max_size, elem->getMaximumSizeOfValueInMemory());
  return max_size;
}
...
static DataTypePtr create(const ASTPtr &arguments) {
  if (!arguments || arguments->children.empty()) return std::make_shared<DataTypeVariant>(DataTypes{});

  DataTypes nested_types;
  nested_types.reserve(arguments->children.size());

  for (const ASTPtr &child : arguments->children) nested_types.emplace_back(DataTypeFactory::instance().get(child));
...
bool isVariantExtension(const DataTypePtr &from_type, const DataTypePtr &to_type) {
  ...
  if (!from_variant || !to_variant) return false;
  ...
  for (const auto &variant : to_variants) to_variant_types.insert(variant->getName());

  for (const auto &variant : from_variant->getVariants()) {
    if (!to_variant_types.contains(variant->getName())) return false;
  }

  return true;
}

void registerDataTypeVariant(DataTypeFactory &factory) { factory.registerDataType("Variant", create); }
