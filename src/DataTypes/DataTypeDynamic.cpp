...
MutableColumnPtr DataTypeDynamic::createColumn() const { return ColumnDynamic::create(max_dynamic_types); }

String DataTypeDynamic::doGetName() const {
  if (max_dynamic_types == DEFAULT_MAX_DYNAMIC_TYPES) return "Dynamic";
  return "Dynamic(max_types=" + toString(max_dynamic_types) + ")";
}

void DataTypeDynamic::updateHashImpl(SipHash &hash) const { hash.update(max_dynamic_types); }

Field DataTypeDynamic::getDefault() const { return Field(Null()); }
...
static DataTypePtr create(const ASTPtr &arguments) {
  if (!arguments || arguments->children.empty()) return std::make_shared<DataTypeDynamic>();
  ...
  if (!identifier)
    throw Exception(ErrorCodes::UNEXPECTED_AST_STRUCTURE, "Unexpected Dynamic type argument: {}. Expected expression 'max_types=N'",
                    identifier->formatForErrorMessage());

  auto identifier_name = identifier->name();
  if (identifier_name != "max_types")
    throw Exception(ErrorCodes::UNEXPECTED_AST_STRUCTURE,
                    "Unexpected identifier: {}. Dynamic data type argument should be in a form 'max_types=N'", identifier_name);

  auto *literal = argument->arguments->children[1]->as<ASTLiteral>();

  if (!literal || literal->value.getType() != Field::Types::UInt64 ||
      literal->value.safeGet<UInt64>() > ColumnDynamic::MAX_DYNAMIC_TYPES_LIMIT)
    throw Exception(ErrorCodes::UNEXPECTED_AST_STRUCTURE,
                    "'max_types' argument for Dynamic type should be a positive integer between 0 and {}",
                    ColumnDynamic::MAX_DYNAMIC_TYPES_LIMIT);

  return std::make_shared<DataTypeDynamic>(literal->value.safeGet<UInt64>());
}

void registerDataTypeDynamic(DataTypeFactory &factory) { factory.registerDataType("Dynamic", create); }
...
std::pair<std::string_view, std::string_view> splitSubcolumnName(std::string_view subcolumn_name) {
  ...
  if (pos == end) return {subcolumn_name, {}};

  return {std::string_view(subcolumn_name.data(), pos),
          std::string_view(pos + 1, end)};  /// NOLINT(bugprone-suspicious-stringview-data-usage)
}

}  // namespace

std::unique_ptr<IDataType::SubstreamData> DataTypeDynamic::getDynamicSubcolumnData(std::string_view subcolumn_name,
                                                                                   const SubstreamData &data, size_t initial_array_level,
  ...
  if (!subcolumn_type) {
    if (throw_if_null) throw Exception(ErrorCodes::ILLEGAL_COLUMN, "Dynamic type doesn't have subcolumn '{}'", type_subcolumn_name);
    return nullptr;
  }
  ...
  if (is_null_map_subcolumn) {
    ...
    if (!res) {
      if (throw_if_null)
        throw Exception(ErrorCodes::LOGICAL_ERROR, "Expected getSubcolumnData() to throw for subcolumn '{}' in throw_if_null mode",
                        subcolumn_name);
      return nullptr;
    }
  }
  ...
  res->serialization = std::make_shared<SerializationDynamicElement>(
      res->serialization, dynamic_serialization.createSerializationForType(ColumnDynamic::getSharedVariantDataType()),
      subcolumn_type->getName(), String(subcolumn_nested_name), is_null_map_subcolumn);
  ...
  if (!is_null_map_subcolumn && make_subcolumn_nullable) res->type = makeNullableOrLowCardinalityNullableSafe(res->type);
  ...
  if (data.column) {
    ...
    if (discriminator) {
      ...
      if (is_null_map_subcolumn)
        creator = std::make_unique<SerializationVariantElementNullMap::VariantNullMapSubcolumnCreator>(
            variant_column.getLocalDiscriminatorsPtr(), "", *discriminator, variant_column.localDiscriminatorByGlobal(*discriminator));
      else
    ...
    /// VariantSubcolumnCreator to create full subcolumn from variant according to created null map.
      ...
      if (is_null_map_subcolumn) {
        ...
        SerializationVariantElement::VariantSubcolumnCreator creator(null_map_for_variant_from_shared_variant, "", 0, 0,
                                                                     make_subcolumn_nullable, null_map_for_variant_from_shared_variant);
        res->column = creator.create(res->column);
      }
    }
