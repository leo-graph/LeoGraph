...
DataTypeQBit::DataTypeQBit(const DataTypePtr &element_type_, const size_t dimension_) : element_type(element_type_), dimension(dimension_) {
  ...
  if (element_type_->getTypeId() != TypeIndex::BFloat16 && element_type_->getTypeId() != TypeIndex::Float32 &&
    ...
    throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT,
                    "QBit data type only supports BFloat16, Float32, or Float64 as element type. Got: {}", element_type_->getName());
...
std::string DataTypeQBit::doGetName() const { return "QBit(" + element_type->getName() + ", " + toString(dimension) + ")"; }
...
MutableColumnPtr DataTypeQBit::createColumn() const {
  ...
  for (size_t i = 0; i < getElementSize(); ++i) tuple_columns[i] = ColumnFixedString::create(bytes);
...
DataTypePtr DataTypeQBit::getNestedTupleElementType() const { return std::make_shared<DataTypeFixedString>(bitsToBytes(dimension)); }
...
Field DataTypeQBit::getDefault() const { return getNestedType()->getDefault(); }
...
static DataTypePtr create(const ASTPtr &arguments) {
  ...
  if (type->getTypeId() != TypeIndex::BFloat16 && type->getTypeId() != TypeIndex::Float32 && type->getTypeId() != TypeIndex::Float64)
    throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT,
                    "QBit data type only supports BFloat16, Float32, or Float64 as element type. Got: {}", type->getName());
...
void registerDataTypeQBit(DataTypeFactory &factory) { factory.registerDataType("QBit", create); }
