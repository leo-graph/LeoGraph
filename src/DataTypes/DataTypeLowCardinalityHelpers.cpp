...
DataTypePtr recursiveRemoveLowCardinality(const DataTypePtr &type) {
  ...
  if (const auto *tuple_type = typeid_cast<const DataTypeTuple *>(type.get())) {
    ...
    for (auto &element : elements) element = recursiveRemoveLowCardinality(element);

    if (tuple_type->hasExplicitNames()) return std::make_shared<DataTypeTuple>(elements, tuple_type->getElementNames());
    return std::make_shared<DataTypeTuple>(elements);
  }

  if (const auto *map_type = typeid_cast<const DataTypeMap *>(type.get())) {
    return std::make_shared<DataTypeMap>(recursiveRemoveLowCardinality(map_type->getKeyType()),
...
ColumnPtr recursiveRemoveLowCardinality(const ColumnPtr &column) {
  ...
  if (const auto *column_nullable = typeid_cast<const ColumnNullable *>(column.get())) {
    ...
    if (nested.get() != nested_no_lc.get()) res = ColumnNullable::create(nested_no_lc, column_nullable->getNullMapColumnPtr());
    ...
    if (data.get() != data_no_lc.get()) res = ColumnArray::create(data_no_lc, column_array->getOffsetsPtr());
  } else if (const auto *column_const = typeid_cast<const ColumnConst *>(column.get())) {
    const auto &nested = column_const->getDataColumnPtr();
    auto nested_no_lc = recursiveRemoveLowCardinality(nested);
    if (nested.get() != nested_no_lc.get()) res = ColumnConst::create(nested_no_lc, column_const->size());
    ...
    for (auto &element : columns) element = recursiveRemoveLowCardinality(element);
    ...
    auto nested_no_lc = recursiveRemoveLowCardinality(nested);
    if (nested.get() != nested_no_lc.get()) res = ColumnMap::create(nested_no_lc);
  }
  ...
  /// when function will be executed.
    ...
    if (column_function->isShortCircuitArgument()) res = column_function->recursivelyConvertResultToFullColumnIfLowCardinality();
...
ColumnPtr recursiveLowCardinalityTypeConversion(const ColumnPtr &column, const DataTypePtr &from_type, const DataTypePtr &to_type) {
  ...
  if (from_type->equals(*to_type)) return column;

  /// We can allow insert enum column if it's numeric type is the same as the column's type in table.
  if (WhichDataType(to_type).isEnum() && from_type->getTypeId() == to_type->getTypeId()) return column;
  ...
  if (const auto *from_nullable_type = typeid_cast<const DataTypeNullable *>(from_type.get())) {
    if (const auto *to_nullable_type = typeid_cast<const DataTypeNullable *>(to_type.get())) {
      ...
      if (nested_result.get() == column_nullable->getNestedColumnPtr().get()) return column;
  ...
  if (const auto *column_const = typeid_cast<const ColumnConst *>(column.get())) {
    ...
    if (nested.get() == nested_no_lc.get()) return column;
  ...
  if (const auto *low_cardinality_type = typeid_cast<const DataTypeLowCardinality *>(from_type.get())) {
    if (to_type->equals(*low_cardinality_type->getDictionaryType())) return column->convertToFullColumnIfLowCardinality();
  }
  ...
  if (const auto *from_array_type = typeid_cast<const DataTypeArray *>(from_type.get())) {
    if (const auto *to_array_type = typeid_cast<const DataTypeArray *>(to_type.get())) {
      ...
      return ColumnArray::create(recursiveLowCardinalityTypeConversion(column_array->getDataPtr(), nested_from, nested_to),
  ...
  if (const auto *from_tuple_type = typeid_cast<const DataTypeTuple *>(from_type.get())) {
    if (const auto *to_tuple_type = typeid_cast<const DataTypeTuple *>(to_type.get())) {
      ...
      if (!has_converted) return column;
