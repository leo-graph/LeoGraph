...
bool canBeSafelyCast(const DataTypePtr &from_type, const DataTypePtr &to_type) {
  ...
  if (from_type->equals(*to_type_unwrapped)) return true;
  ...
  switch (from_which_type.idx) {
    ...
    case TypeIndex::UInt256: {
      if (to_which_type.isUInt() && to_type_unwrapped->getSizeOfValueInMemory() >= from_type->getSizeOfValueInMemory()) return true;

      if (to_which_type.isString()) return true;
    ...
    case TypeIndex::Int256: {
      if (to_which_type.isInt() && to_type_unwrapped->getSizeOfValueInMemory() >= from_type->getSizeOfValueInMemory()) return true;
    ...
    case TypeIndex::BFloat16: {
      if (to_which_type.isFloat32() || to_which_type.isFloat64() || to_which_type.isString()) return true;

      return false;
    }
    case TypeIndex::Float32: {
      if (to_which_type.isFloat64() || to_which_type.isString()) return true;
    ...
    case TypeIndex::Decimal256: {
      if (to_which_type.isDecimal()) {
        ...
        if (from_type_decimal_precision > to_type_decimal_precision) return false;

        auto from_type_decimal_scale = getDecimalScale(*from_type);
        auto to_type_decimal_scale = getDecimalScale(*to_type_unwrapped);
        if (from_type_decimal_scale > to_type_decimal_scale) return false;
    ...
    case TypeIndex::UUID: {
      if (to_which_type.isUInt128() || to_which_type.isString()) return true;

      return false;
    }
    case TypeIndex::IPv4: {
      if (to_which_type.isUInt32() || to_which_type.isUInt64() || to_which_type.isString()) return true;
    ...
    case TypeIndex::Map: {
      if (to_which_type.isMap()) {
        ...
        if (!canBeSafelyCast(from_type_map.getKeyType(), to_type_map.getKeyType())) return false;

        if (!canBeSafelyCast(from_type_map.getValueType(), to_type_map.getValueType())) return false;
      ...
      if (to_which_type.isArray()) {
        ...
        if (!to_type_nested_tuple_type) return false;

        const auto &to_type_tuple_elements = to_type_nested_tuple_type->getElements();
        if (to_type_tuple_elements.size() != 2) return false;

        if (!canBeSafelyCast(from_type_map.getKeyType(), to_type_tuple_elements[0])) return false;

        if (!canBeSafelyCast(from_type_map.getValueType(), to_type_tuple_elements[1])) return false;
    ...
    case TypeIndex::Tuple: {
      if (to_which_type.isTuple()) {
        ...
        if (lhs_type_elements_size != to_tuple_type_elements.size()) return false;

        for (size_t i = 0; i < lhs_type_elements_size; ++i)
          if (!canBeSafelyCast(from_tuple_type_elements[i], to_tuple_type_elements[i])) return false;
