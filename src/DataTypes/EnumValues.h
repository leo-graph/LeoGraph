...
class EnumValues : public IHints<> {
 ...
 public:
  ...
  bool hasValue(const T& value) const { return value_to_name_map.contains(value); }

  /// throws exception if value is not valid
  std::string_view getNameForValue(const T& value) const { return findByValue(value)->second; }

  /// returns false if value is not valid
  bool getNameForValue(const T& value, std::string_view& result) const {
    const auto it = value_to_name_map.find(value);
    if (it == value_to_name_map.end()) return false;
