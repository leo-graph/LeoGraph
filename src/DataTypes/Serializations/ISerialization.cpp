...
void throwEmptySerializationState(const ISerialization *serialization) {
  throw Exception(ErrorCodes::LOGICAL_ERROR, "Got empty state for {}", demangle(typeid(*serialization).name()));
}
...
void throwInvalidSerializationState(const ISerialization *serialization, const std::type_info &expected, const std::type_info &got) {
  throw Exception(ErrorCodes::LOGICAL_ERROR, "Invalid State for {}. Expected: {}, got {}", demangle(typeid(*serialization).name()),
                  demangle(expected.name()), demangle(got.name()));
}
...
String ISerialization::kindStackToString(const KindStack &kind_stack) {
  ...
  String result;
  if (kind_stack.size() == 1) return kindToString(kind_stack.front());

  for (ssize_t i = kind_stack.size() - 1; i >= 1; i--) {
    if (!result.empty()) result += "Over";
...
ISerialization::KindStack ISerialization::stringToKindStack(const String &str) {
  ...
  for (size_t i = 0; i != kind_strings.size(); ++i) {
    ...
    if (i == 0 && kind != Kind::DEFAULT) kind_stack.push_back(Kind::DEFAULT);
...
const std::set<SubstreamType> ISerialization::Substream::named_types{
    StringSizes, InlinedStringSizes, TupleElement, NamedOffsets, NamedNullMap, NamedVariantDiscriminators,
};

String ISerialization::Substream::toString() const {
  if (named_types.contains(type)) return fmt::format("{}({})", type, name_of_substream);

  if (type == VariantElement) return fmt::format("VariantElement({})", variant_element_name);

  if (type == VariantElementNullMap) return fmt::format("VariantElementNullMap({}.null)", variant_element_name);
...
void ISerialization::enumerateStreams(EnumerateStreamsSettings &settings, const StreamCallback &callback, const SubstreamData &data) const {
...
void ISerialization::serializeBinaryBulkWithMultipleStreams(const IColumn &column, size_t offset, size_t limit,
                                                            SerializeBinaryBulkSettings &settings,
                                                            SerializeBinaryBulkStatePtr & /* state */) const {
  settings.path.push_back(Substream::Regular);
  if (WriteBuffer *stream = settings.getter(settings.path)) serializeBinaryBulk(column, *stream, offset, limit);
  settings.path.pop_back();
}

void ISerialization::deserializeBinaryBulkWithMultipleStreams(ColumnPtr &column, size_t rows_offset, size_t limit,
  ...
  if (insertDataFromSubstreamsCacheIfAny(cache, settings, column)) {
    ...
    if (settings.get_avg_value_size_hint_callback) avg_value_size_hint = settings.get_avg_value_size_hint_callback(settings.path);
    ...
    if (settings.update_avg_value_size_hint_callback) settings.update_avg_value_size_hint_callback(settings.path, *column);
  }
...
String getNameForSubstreamPath(String stream_name, SubstreamIterator begin, SubstreamIterator end, bool escape_for_file_name,
                               bool encode_sparse_stream, bool escape_variant_substreams, size_t initial_array_level = 0) {
...
static bool isPossibleOffsetsOfNested(const ISerialization::SubstreamPath &path) {
  ...
  if (path.size() == 1 && path[0].type == ISerialization::Substream::ArraySizes) return true;
...
String ISerialization::getFileNameForRenamedColumnStream(const String &name_from, const String &name_to, const String &file_name) {
  ...
  if (file_name.starts_with(name_from_escaped)) return escapeForFileName(name_to) + file_name.substr(0, name_from_escaped.size());
...
String ISerialization::getFileNameForRenamedColumnStream(const NameAndTypePair &column_from, const NameAndTypePair &column_to,
...
String ISerialization::getSubcolumnNameForStream(const SubstreamPath &path, size_t prefix_len, bool encode_sparse_stream,
  ...
  auto subcolumn_name =
      getNameForSubstreamPath("", path.begin(), path.begin() + prefix_len, false, encode_sparse_stream, false, initial_array_level);
  if (!subcolumn_name.empty()) subcolumn_name = subcolumn_name.substr(1);  // It starts with a dot.
...
struct SubstreamsCacheColumnWithNumReadRowsElement : public ISerialization::ISubstreamsCacheElement {
  explicit SubstreamsCacheColumnWithNumReadRowsElement(ColumnPtr column_, size_t num_read_rows_)
      : column(column_), num_read_rows(num_read_rows_) {}
...
void ISerialization::addColumnWithNumReadRowsToSubstreamsCache(SubstreamsCache *cache, const SubstreamPath &path, ColumnPtr column,
                                                               size_t num_read_rows) {
  addElementToSubstreamsCache(cache, path, std::make_unique<SubstreamsCacheColumnWithNumReadRowsElement>(column, num_read_rows));
}

std::optional<std::pair<ColumnPtr, size_t>> ISerialization::getColumnWithNumReadRowsFromSubstreamsCache(SubstreamsCache *cache,
...
void ISerialization::addElementToSubstreamsCache(ISerialization::SubstreamsCache *cache, const ISerialization::SubstreamPath &path,
                                                 std::unique_ptr<ISubstreamsCacheElement> &&element) {
  if (!cache || path.empty()) return;

  cache->insert_or_assign(getSubcolumnNameForStream(path, true), std::move(element));
}

ISerialization::ISubstreamsCacheElement *ISerialization::getElementFromSubstreamsCache(ISerialization::SubstreamsCache *cache,
                                                                                       const ISerialization::SubstreamPath &path) {
  if (!cache || path.empty()) return nullptr;

  auto it = cache->find(getSubcolumnNameForStream(path, true));
  return it == cache->end() ? nullptr : it->second.get();
}

void ISerialization::addToSubstreamsDeserializeStatesCache(SubstreamsDeserializeStatesCache *cache, const SubstreamPath &path,
...
ISerialization::DeserializeBinaryBulkStatePtr ISerialization::getFromSubstreamsDeserializeStatesCache(
    SubstreamsDeserializeStatesCache *cache, const SubstreamPath &path) {
  if (!cache || path.empty()) return nullptr;
...
void ISerialization::serializeTextMarkdown(const DB::IColumn &column, size_t row_num, DB::WriteBuffer &ostr,
...
size_t ISerialization::getArrayLevel(const SubstreamPath &path, size_t prefix_len) {
  size_t level = 0;
  for (size_t i = 0; i < prefix_len; ++i) level += path[i].type == Substream::ArrayElements;
  return level;
}

bool ISerialization::hasSubcolumnForPath(const SubstreamPath &path, size_t prefix_len) {
  if (prefix_len == 0 || prefix_len > path.size()) return false;
...
void ISerialization::throwUnexpectedDataAfterParsedValue(IColumn &column, ReadBuffer &istr, const FormatSettings &settings,
                                                         const String &type_name) const {
  WriteBufferFromOwnString ostr;
  serializeText(column, column.size() - 1, ostr, settings);
  /// Restore correct column size.
  column.popBack(1);
  throw Exception(ErrorCodes::UNEXPECTED_DATA_AFTER_PARSED_VALUE, "Unexpected data '{}' after parsed {} value '{}'",
                  std::string(istr.position(), std::min(size_t(10), istr.available())), type_name, ostr.str());
}
...
void ISerialization::addSubstreamAndCallCallback(ISerialization::SubstreamPath &path, const ISerialization::StreamCallback &callback,
                                                 ISerialization::Substream substream) const {
  path.push_back(substream);
  callback(path);
  path.pop_back();
}

bool ISerialization::insertDataFromSubstreamsCacheIfAny(SubstreamsCache *cache, const DeserializeBinaryBulkSettings &settings,
                                                        ColumnPtr &result_column) {
  auto cached_column_with_num_read_rows = getColumnWithNumReadRowsFromSubstreamsCache(cache, settings.path);
  if (!cached_column_with_num_read_rows) return false;
  ...
  insertDataFromCachedColumn(settings, result_column, cached_column_with_num_read_rows->first, cached_column_with_num_read_rows->second,
                             cache, /*update_cache_after_insert=*/true);
  return true;
}

void ISerialization::insertDataFromCachedColumn(const ISerialization::DeserializeBinaryBulkSettings &settings, ColumnPtr &result_column,
                                                const ColumnPtr &cached_column, size_t num_read_rows, SubstreamsCache *cache,
...
bool ISerialization::isVariantSubcolumn(const SubstreamPath &substream_path) {
  for (const auto &stream : substream_path) {
    if (stream.type == Substream::VariantElement) return true;
  }

  return false;
}

bool ISerialization::tryToChangeStreamFileNameSettingsForNotFoundStream(const ISerialization::SubstreamPath &substream_path,
