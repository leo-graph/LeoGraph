...
ColumnPtr createIndexes(const ColumnDynamic &dynamic_column,
...
FlattenedDynamicColumn flattenDynamicColumn(const ColumnDynamic &dynamic_column) {
  ...
  for (ColumnVariant::Discriminator i = 0; i != variant_types.size(); ++i) {
    ...
    if (i == shared_variant_discr) continue;
  ...
  for (size_t i = 0; i != shared_variant_column.size(); ++i) {
    ...
    type->getDefaultSerialization()->deserializeBinary(*flattened_dynamic_column.columns[it->second]->assumeMutable(), buf,
                                                       format_settings);
  }
  ...
  switch (flattened_dynamic_column.indexes_type->getTypeId()) {
    case TypeIndex::UInt8:
      flattened_dynamic_column.indexes_column = createIndexes<ColumnUInt8>(
          dynamic_column, discriminator_to_index, shared_variant_type_to_index, flattened_dynamic_column.types.size());
      break;
    case TypeIndex::UInt16:
      flattened_dynamic_column.indexes_column = createIndexes<ColumnUInt16>(
          dynamic_column, discriminator_to_index, shared_variant_type_to_index, flattened_dynamic_column.types.size());
      break;
    case TypeIndex::UInt32:
      flattened_dynamic_column.indexes_column = createIndexes<ColumnUInt32>(
          dynamic_column, discriminator_to_index, shared_variant_type_to_index, flattened_dynamic_column.types.size());
      break;
    case TypeIndex::UInt64:
      flattened_dynamic_column.indexes_column = createIndexes<ColumnUInt64>(
          dynamic_column, discriminator_to_index, shared_variant_type_to_index, flattened_dynamic_column.types.size());
      break;
    default:
      throw Exception(ErrorCodes::LOGICAL_ERROR, "Unexpected type as type of indices column type: {}",
                      flattened_dynamic_column.indexes_type->getName());
  }
...
template <typename IndexesColumn>
void fillDynamicColumn(FlattenedDynamicColumn &&flattened_column, ColumnDynamic &dynamic_column,
  ...
  for (size_t i = 0; i != indexes_data.size(); ++i) {
    ...
    if (index == null_index) {
      ...
      ColumnDynamic::serializeValueIntoSharedVariant(shared_variant, *flattened_column.columns[index], flattened_column.types[index],
...
void unflattenDynamicColumn(FlattenedDynamicColumn &&flattened_column, ColumnDynamic &dynamic_column) {
  ...
  switch (flattened_column.indexes_type->getTypeId()) {
    ...
    case TypeIndex::UInt64:
      ...
      break;
    default:
      throw Exception(ErrorCodes::LOGICAL_ERROR, "Unexpected type of indexes in flattened Dynamic column: {}",
                      flattened_column.indexes_type->getName());
  }
}
...
std::vector<size_t> getLimitsImpl(const IColumn &indexes_column, size_t num_types) {
  ...
  for (size_t i = 0; i != data.size(); ++i) {
    if (data[i] < num_types) ++limits[data[i]];
  }
...
std::vector<size_t> getLimitsForFlattenedDynamicColumn(const IColumn &indexes_column, size_t num_types) {
  switch (indexes_column.getDataType()) {
    ...
    default:
      throw Exception(ErrorCodes::LOGICAL_ERROR, "Unexpected column type of indexes in flattened Dynamic column: {}",
