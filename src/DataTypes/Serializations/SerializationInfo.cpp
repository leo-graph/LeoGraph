...
SerializationInfo::SerializationInfo(ISerialization::KindStack kind_stack_, const Settings &settings_, const Data &data_)
    : settings(settings_), kind_stack(kind_stack_), data(data_) {}

void SerializationInfo::add(const IColumn &column) {
  data.add(column);
  if (settings.choose_kind) kind_stack = chooseKindStack(data, settings);
}
...
void SerializationInfo::replaceData(const SerializationInfo &other) { data = other.data; }

MutableSerializationInfoPtr SerializationInfo::clone() const { return std::make_shared<SerializationInfo>(kind_stack, settings, data); }
...
static bool preserveDefaultsAfterConversion(const IDataType &lhs, const IDataType &rhs) {
  ...
  if (lhs_is_columned_as_numeric && rhs_is_columned_as_numeric) return true;

  if (isStringOrFixedString(lhs) && isStringOrFixedString(rhs)) return true;

  return false;
}

std::shared_ptr<SerializationInfo> SerializationInfo::createWithType(const IDataType &old_type, const IDataType &new_type,
...
void SerializationInfo::serialializeKindStackBinary(WriteBuffer &out) const {
  if (kind_stack == ISerialization::KindStack{ISerialization::Kind::DEFAULT}) {
    ...
    for (auto kind : kind_stack) writeBinary(static_cast<UInt8>(kind), out);
  }
}
...
void SerializationInfo::deserializeFromKindsBinary(ReadBuffer &in) {
  ...
  if (!maybe_type) throw Exception(ErrorCodes::CORRUPTED_DATA, "Unknown serialization kind type {}", UInt32(type));
  ...
  switch (*maybe_type) {
    ...
    case KindStackBinarySerializationType::COMBINATION: {
      ...
      for (size_t i = 0; i != num_kinds; ++i) {
        ...
        if (!maybe_kind) throw Exception(ErrorCodes::CORRUPTED_DATA, "Unknown serialization kind {}", UInt32(kind));
...
void SerializationInfo::fromJSON(const Poco::JSON::Object &object) {
  if (!object.has(KEY_KIND) || !object.has(KEY_NUM_DEFAULTS) || !object.has(KEY_NUM_ROWS))
    throw Exception(ErrorCodes::CORRUPTED_DATA, "Missed field '{}' or '{}' or '{}' in SerializationInfo of columns", KEY_KIND,
...
ISerialization::KindStack SerializationInfo::chooseKindStack(const Data &data, const Settings &settings) {
  ...
  if (ratio > settings.ratio_of_defaults_for_sparse) kind_stack.push_back(ISerialization::Kind::SPARSE);
  return kind_stack;
}

SerializationInfoByName::SerializationInfoByName(const SerializationInfo::Settings &settings_) : settings(settings_) {
...
SerializationInfoByName::SerializationInfoByName(const NamesAndTypesList &columns, const SerializationInfo::Settings &settings_)
  ...
  if (settings.isAlwaysDefault()) return;

  for (const auto &column : columns) {
    if (settings.canUseSparseSerialization(*column.type)) emplace(column.name, column.type->createSerializationInfo(settings));
  }
}
...
MergeTreeSerializationInfoVersion SerializationInfoByName::getVersion() const { return settings.version; }

bool SerializationInfoByName::needsPersistence() const { return !empty() || getVersion() > MergeTreeSerializationInfoVersion::BASIC; }
...
void SerializationInfoByName::writeJSON(WriteBuffer &out) const {
  ...
  if (version >= MergeTreeSerializationInfoVersion::WITH_TYPES) {
    ...
    if (settings.propagate_types_serialization_versions_to_nested_types)
      object.set(KEY_PROPAGATE_DATA_TYPES_SERIALIZATION_VERSIONS_TO_NESTED_TYPES,
                 settings.propagate_types_serialization_versions_to_nested_types);
  }
...
SerializationInfoByName SerializationInfoByName::clone() const {
  ...
  for (const auto &[name, info] : *this) res.emplace(name, info->clone());
  return res;
}
...
SerializationInfoByName SerializationInfoByName::readJSONFromString(const NamesAndTypesList &columns, const std::string &json_str) {
  ...
  if (!object->has(KEY_VERSION)) throw Exception(ErrorCodes::CORRUPTED_DATA, "Missed version of serialization infos");
  ...
  MergeTreeSerializationInfoVersion version = MergeTreeSerializationInfoVersion::BASIC;
  {
    ...
    if (!maybe_enum) throw Exception(ErrorCodes::CORRUPTED_DATA, "Unknown version of serialization infos ({})", version_value);
    version = *maybe_enum;
  }
  ...
  SerializationInfoSettings settings(1.0 /* Doesn't matter when constructing from JSON */,
                                     false /* Cannot choose kind when constructing from JSON */, version, string_serialization_version,
                                     nullable_serialization_version, propagate_types_serialization_versions_to_nested_types);

  SerializationInfoByName infos(settings);
  if (columns_array) {
    std::unordered_map<std::string_view, const IDataType *> column_type_by_name;
    for (const auto &[name, type] : columns) column_type_by_name.emplace(name, type.get());

    for (const auto &elem : *columns_array) {
      const auto &elem_object = elem.extract<Poco::JSON::Object::Ptr>();

      if (!elem_object->has(KEY_NAME)) throw Exception(ErrorCodes::CORRUPTED_DATA, "Missed field '{}' in serialization infos", KEY_NAME);
