...
SerializationJSON<Parser>::SerializationJSON(const std::unordered_map<String, DataTypePtr> &typed_paths_types_,
                                             const std::unordered_map<String, SerializationPtr> &typed_paths_serializations_,
                                             const std::unordered_set<String> &paths_to_skip_,
                                             const std::vector<String> &path_regexps_to_skip_, const DataTypePtr &dynamic_type_,
                                             const SerializationPtr &dynamic_serialization_,
                                             std::unique_ptr<JSONExtractTreeNode<Parser>> json_extract_tree_)
    : SerializationObject(typed_paths_types_, typed_paths_serializations_, paths_to_skip_, path_regexps_to_skip_, dynamic_type_,
...
struct Prefix {
  ...
  void shrinkToCommonPrefix(const PathElements &path_elements) {
    ...
    size_t i = 0;
    while (i != elements.size() && i != (path_elements.elements.size() - 1) && elements[i].first == path_elements.elements[i]) ++i;
    elements.resize(i);
  }
...
template <typename Parser>
void SerializationJSON<Parser>::serializeTextImpl(const IColumn &column, size_t row_num, WriteBuffer &ostr, const FormatSettings &settings,
                                                  bool pretty, size_t indent) const {
  ...
  /// For example:
  /// b.c, a.b, a.a, b.e, g, h.u.t -> a.a, a.b, b.c, b.e, g, h.u.t -> {"a" : {"a" : ..., "b" : ...}, "b" : {"c" : ..., "e" : ...}, "g" :
  /// ..., "h" : {"u" : {"t" : ...}}}.
  ...
  for (auto it = ColumnObject::SortedPathsIterator(column_object, row_num); !it.end(); it.next()) {
    ...
    if (prefix_size != prev_prefix_size) {
      ...
      if (pretty) {
        ...
        for (size_t i = 0; i != objects_to_close; ++i) {
          writeChar(settings.json.pretty_print_indent,
                    (indent + prefix_size + objects_to_close - i) * settings.json.pretty_print_indent_multiplier, ostr);
        ...
        for (size_t i = 0; i != objects_to_close; ++i) writeChar('}', ostr);
      }
    }
    ...
    if (pretty) {
      writeChar(settings.json.pretty_print_indent, (indent + current_prefix.size() + 1) * settings.json.pretty_print_indent_multiplier,
                ostr);
    ...
    if (path_info.type == ColumnObject::SortedPathsIterator::PathType::TYPED) {
      if (pretty)
        typed_paths_serializations.at(String(path_info.path))
            ->serializeTextJSONPretty(*path_info.column, path_info.row, ostr, settings, indent + current_prefix.size() + 1);
      else
        typed_paths_serializations.at(String(path_info.path))->serializeTextJSON(*path_info.column, path_info.row, ostr, settings);
    } else {
      if (pretty)
        dynamic_serialization->serializeTextJSONPretty(*path_info.column, path_info.row, ostr, settings,
                                                       indent + current_prefix.size() + 1);
      else
  ...
  if (pretty) {
    ...
    for (size_t i = 0; i != current_prefix.elements.size(); ++i) {
      writeChar(settings.json.pretty_print_indent, (indent + current_prefix.size() - i) * settings.json.pretty_print_indent_multiplier,
                ostr);
      writeCString("}\n", ostr);
    }
    ...
    for (size_t i = 0; i != current_prefix.elements.size(); ++i) writeChar('}', ostr);
    writeChar('}', ostr);
  }
}
...
void SerializationJSON<Parser>::deserializeObject(IColumn &column, std::string_view object, const FormatSettings &settings) const {
  ...
  if (!parser->parse(object, document))
    throw Exception(ErrorCodes::INCORRECT_DATA, "Cannot parse JSON object here: {}{}", object.substr(0, std::min(object.size(), 1000uz)),
                    object.size() > 1000 ? "... (JSON object is too long to display as a whole)" : "");
...
template <typename Parser>
void SerializationJSON<Parser>::serializeText(const IColumn &column, size_t row_num, WriteBuffer &ostr,
...
template <typename Parser>
void SerializationJSON<Parser>::serializeTextEscaped(const IColumn &column, size_t row_num, WriteBuffer &ostr,
                                                     const FormatSettings &settings) const {
...
template <typename Parser>
void SerializationJSON<Parser>::serializeTextQuoted(const IColumn &column, size_t row_num, WriteBuffer &ostr,
                                                    const FormatSettings &settings) const {
  WriteBufferFromOwnString buf;
  serializeTextImpl(column, row_num, buf, settings);
...
template <typename Parser>
void SerializationJSON<Parser>::serializeTextCSV(const IColumn &column, size_t row_num, WriteBuffer &ostr,
                                                 const FormatSettings &settings) const {
  WriteBufferFromOwnString buf;
  serializeTextImpl(column, row_num, buf, settings);
...
template <typename Parser>
void SerializationJSON<Parser>::serializeTextXML(const IColumn &column, size_t row_num, WriteBuffer &ostr,
                                                 const FormatSettings &settings) const {
  WriteBufferFromOwnString buf;
  serializeTextImpl(column, row_num, buf, settings);
...
template <typename Parser>
void SerializationJSON<Parser>::serializeTextJSON(const IColumn &column, size_t row_num, WriteBuffer &ostr,
                                                  const FormatSettings &settings) const {
  serializeTextImpl(column, row_num, ostr, settings);
}

template <typename Parser>
void SerializationJSON<Parser>::serializeTextJSONPretty(const IColumn &column, size_t row_num, WriteBuffer &ostr,
