...
void SerializationFixedString::serializeBinary(const Field &field, WriteBuffer &ostr, const FormatSettings &) const {
  ...
  if (s.size() < n)
    for (size_t i = s.size(); i < n; ++i) ostr.write(0);
}
...
void SerializationFixedString::serializeBinaryBulk(const IColumn &column, WriteBuffer &ostr, size_t offset, size_t limit) const {
  ...
  if (limit) ostr.write(reinterpret_cast<const char *>(&data[n * offset]), n * limit);
}

void SerializationFixedString::deserializeBinaryBulk(IColumn &column, ReadBuffer &istr, size_t rows_offset, size_t limit,
  ...
  if (read_bytes % n != 0)
    throw Exception(ErrorCodes::CANNOT_READ_ALL_DATA,
                    "Cannot read all data of type FixedString. "
                    "Bytes read:{}. String size:{}.",
                    read_bytes, toString(n));
...
void SerializationFixedString::serializeTextEscaped(const IColumn &column, size_t row_num, WriteBuffer &ostr,
...
bool SerializationFixedString::tryDeserializeTextEscaped(IColumn &column, ReadBuffer &istr, const FormatSettings &) const {
  return tryRead(*this, column, [&istr](ColumnFixedString::Chars &data) {
    readEscapedStringInto<PaddedPODArray<UInt8>, false>(data, istr);
    return true;
  });
}

void SerializationFixedString::serializeTextQuoted(const IColumn &column, size_t row_num, WriteBuffer &ostr,
...
void SerializationFixedString::serializeTextJSON(const IColumn &column, size_t row_num, WriteBuffer &ostr,
                                                 const FormatSettings &settings) const {
  const char *pos = reinterpret_cast<const char *>(&assert_cast<const ColumnFixedString &>(column).getChars()[n * row_num]);
...
bool SerializationFixedString::tryDeserializeTextJSON(IColumn &column, ReadBuffer &istr, const FormatSettings &settings) const {
  return tryRead(*this, column,
                 [&istr, &settings](ColumnFixedString::Chars &data) { return tryReadJSONStringInto(data, istr, settings.json); });
}
...
bool SerializationFixedString::tryDeserializeTextCSV(IColumn &column, ReadBuffer &istr, const FormatSettings &settings) const {
  return tryRead(*this, column, [&istr, &csv = settings.csv](ColumnFixedString::Chars &data) {
    readCSVStringInto<ColumnFixedString::Chars, false, false>(data, istr, csv);
    return true;
  });
}

void SerializationFixedString::serializeTextMarkdown(const DB::IColumn &column, size_t row_num, DB::WriteBuffer &ostr,
                                                     const DB::FormatSettings &settings) const {
  if (settings.markdown.escape_special_characters) {
    writeMarkdownEscapedString(reinterpret_cast<const char *>(&(assert_cast<const ColumnFixedString &>(column).getChars()[n * row_num])), n,
                               ostr);
  } else
