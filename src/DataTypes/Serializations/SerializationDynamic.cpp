...
void SerializationDynamic::enumerateStreams(EnumerateStreamsSettings &settings, const StreamCallback &callback,
  ...
  const auto *deserialize_state =
      data.deserialize_state ? checkAndGetState<DeserializeBinaryBulkStateDynamic>(data.deserialize_state) : nullptr;

  /// If column is nullptr and we don't have deserialize state yet, nothing to enumerate as we don't have any variants.
  if (!settings.enumerate_dynamic_streams || (!column_dynamic && !deserialize_state)) return;

  const auto &variant_type =
      column_dynamic ? column_dynamic->getVariantInfo().variant_type
                     : checkAndGetState<DeserializeBinaryBulkStateDynamicStructure>(deserialize_state->structure_state)->variant_type;
...
void SerializationDynamic::serializeBinaryBulkStatePrefix(const DB::IColumn &column, SerializeBinaryBulkSettings &settings,
  ...
  if (!stream)
    throw Exception(ErrorCodes::LOGICAL_ERROR,
                    "Missing stream for Dynamic column structure during serialization of binary bulk state prefix");
  ...
  if (structure_version.value == SerializationVersion::FLATTENED) {
    ...
    indexes_serialization->serializeBinaryBulkStatePrefix(*flattened_column.indexes_column, settings,
                                                          dynamic_state->flattened_indexes_state);
  ...
  if (structure_version.value == SerializationVersion::V1) writeVarUInt(dynamic_state->num_dynamic_types, *stream);
  ...
  if ((settings.native_format && settings.format_settings && settings.format_settings->native.encode_types_in_binary_format) ||
    ...
    for (const auto &variant : variants) {
      if (variant->getName() != ColumnDynamic::getSharedVariantTypeName()) encodeDataType(variant, *stream);
    }
  } else {
    for (const auto &name : dynamic_state->variant_names) {
      if (name != ColumnDynamic::getSharedVariantTypeName()) writeStringBinary(name, *stream);
    }
  }
  ...
  if (settings.object_and_dynamic_write_statistics == SerializeBinaryBulkSettings::ObjectAndDynamicStatisticsMode::PREFIX) {
    ...
    if (structure_version.value == SerializationVersion::V3) writeBinary(true, *stream);
    ...
    /// Otherwise serialize the minimum version of statistics.
      ...
      for (size_t i = 0; i != variant_info.variant_names.size(); ++i) writeVarUInt(0, *stream);
...
void SerializationDynamic::deserializeBinaryBulkStatePrefix(DeserializeBinaryBulkSettings &settings, DeserializeBinaryBulkStatePtr &state,
  ...
  if (structure_state_typed->structure_version.value == SerializationVersion::FLATTENED) {
    ...
    settings.path.push_back(Substream::DynamicData);
    structure_state_typed->flattened_indexes_type->getDefaultSerialization()->deserializeBinaryBulkStatePrefix(
        settings, dynamic_state->flattened_indexes_state, cache);
  ...
  if (settings.dynamic_subcolumns_callback) {
    ...
    dynamic_state->variant_serialization->enumerateStreams(enumerate_settings, settings.dynamic_subcolumns_callback,
                                                           SubstreamData(dynamic_state->variant_serialization));
  }
  ...
  if (auto cached_state = getFromSubstreamsDeserializeStatesCache(cache, settings.path)) {
    ...
    if (structure_state->structure_version.value == SerializationVersion::FLATTENED) {
      ...
      if ((settings.native_format && settings.format_settings && settings.format_settings->native.decode_types_in_binary_format) ||
        ...
        for (size_t i = 0; i != structure_state->num_dynamic_types; ++i) variants.push_back(decodeDataType(*structure_stream));
      } else {
        String data_type_name;
      ...
      if (settings.object_and_dynamic_read_statistics) {
        ...
        if (structure_state->structure_version.value == SerializationVersion::V3) readBinary(has_statistics, *structure_stream);
        if (has_statistics) {
...
void SerializationDynamic::serializeBinaryBulkStateSuffix(SerializeBinaryBulkSettings &settings, SerializeBinaryBulkStatePtr &state) const {
  ...
  if (settings.object_and_dynamic_write_statistics == SerializeBinaryBulkSettings::ObjectAndDynamicStatisticsMode::SUFFIX) {
    ...
    if (!stream)
      throw Exception(ErrorCodes::LOGICAL_ERROR,
                      "Missing stream for Dynamic column structure during serialization of binary bulk state suffix");

    /// In V3 version we have additional flag that indicates if we have statistics or not.
    /// It is needed to be able to write empty statistics if needed.
    if (dynamic_state->structure_version.value == SerializationVersion::V3) writeBinary(true, *stream);
...
void SerializationDynamic::serializeBinaryBulkWithMultipleStreams(const DB::IColumn &column, size_t offset, size_t limit,
...
void SerializationDynamic::serializeBinaryBulkWithMultipleStreamsAndCountTotalSizeOfVariants(const IColumn &column, size_t offset,
                                                                                             size_t limit,
                                                                                             SerializeBinaryBulkSettings &settings,
  ...
  if (dynamic_state->structure_version.value == SerializationVersion::FLATTENED) {
    if (offset != 0 || (limit != 0 && limit != column.size()))
      throw Exception(ErrorCodes::LOGICAL_ERROR,
                      "Flattened serialization for Dynamic column is supported only when the whole column is serialized. Provided offset "
                      "and limit: {}/{}",
                      offset, limit);

    settings.path.push_back(Substream::DynamicData);
    /// First, write indexes.
    dynamic_state->flattened_column->indexes_type->getDefaultSerialization()->serializeBinaryBulkWithMultipleStreams(
        *dynamic_state->flattened_column->indexes_column, 0, 0, settings, dynamic_state->flattened_indexes_state);
    /// Second, write all data of flattened types in corresponding order.
    for (size_t i = 0; i != dynamic_state->flattened_column->types.size(); ++i) {
      auto serialization = dynamic_state->flattened_column->types[i]->getDefaultSerialization();
      serialization->serializeBinaryBulkWithMultipleStreams(*dynamic_state->flattened_column->columns[i], 0, 0, settings,
                                                            dynamic_state->flattened_states[i]);
    }
    settings.path.pop_back();
    return;
  }
  ...
  if (!variant_info.variant_type->equals(*dynamic_state->variant_type))
    throw Exception(ErrorCodes::LOGICAL_ERROR, "Mismatch of internal columns of Dynamic. Expected: {}, Got: {}",
                    dynamic_state->variant_type->getName(), variant_info.variant_type->getName());
  ...
  assert_cast<const SerializationVariant &>(*dynamic_state->variant_serialization)
      .serializeBinaryBulkWithMultipleStreamsAndUpdateVariantStatistics(
          *variant_column, offset, limit, settings, dynamic_state->variant_state, dynamic_state->statistics.variants_statistics,
    ...
    if (!shared_variant.empty()) {
      ...
      for (size_t i = offset; i != end; ++i) {
        if (local_discriminators[i] == shared_variant_discr) {
          ...
          auto type = decodeDataType(buf);
          auto type_name = type->getName();
          if (auto it = dynamic_state->statistics.shared_variants_statistics.find(type_name);
              it != dynamic_state->statistics.shared_variants_statistics.end())
            ++it->second;
          else if (dynamic_state->statistics.shared_variants_statistics.size() <
                   ColumnDynamic::Statistics::MAX_SHARED_VARIANT_STATISTICS_SIZE)
...
void SerializationDynamic::deserializeBinaryBulkWithMultipleStreams(DB::ColumnPtr &column, size_t rows_offset, size_t limit,
  ...
  if (structure_state->structure_version.value == SerializationVersion::FLATTENED) {
    ...
    /// First, read indexes.
    auto indexes_serialization = flattened_column.indexes_type->getDefaultSerialization();
    indexes_serialization->deserializeBinaryBulkWithMultipleStreams(flattened_column.indexes_column, 0, limit, settings,
                                                                    dynamic_state->flattened_indexes_state, cache);
    ...
    for (size_t i = 0; i != flattened_column.types.size(); ++i) {
      ...
      flattened_column.types[i]->getDefaultSerialization()->deserializeBinaryBulkWithMultipleStreams(
          type_column, 0, flattened_limits[i], settings, dynamic_state->flattened_states[i], cache);
  ...
  if (!variant_info.variant_type->equals(*structure_state->variant_type))
    throw Exception(ErrorCodes::LOGICAL_ERROR, "Mismatch of internal columns of Dynamic. Expected: {}, Got: {}",
                    structure_state->variant_type->getName(), variant_info.variant_type->getName());

  settings.path.push_back(Substream::DynamicData);
  dynamic_state->variant_serialization->deserializeBinaryBulkWithMultipleStreams(column_dynamic.getVariantColumnPtr(), rows_offset, limit,
                                                                                 settings, dynamic_state->variant_state, cache);
  settings.path.pop_back();
...
void SerializationDynamic::serializeBinary(const ColumnDynamic &dynamic_column, size_t row_num, WriteBuffer &ostr,
  ...
  getDataTypesCache()
      .getSerialization(variant_type_name)
      ->serializeBinary(variant_column.getVariantByGlobalDiscriminator(global_discr), variant_column.offsetAt(row_num), ostr, settings);
}
...
void SerializationDynamic::serializeForHashCalculation(const IColumn &column, size_t row_num, WriteBuffer &ostr) const {
  ...
  const auto &variant_type_name = variant_info.variant_names[global_discr];
  const auto &variant_type = assert_cast<const DataTypeVariant &>(*variant_info.variant_type).getVariant(global_discr);
  serializeVariantForHashCalculation(variant_column.getVariantByGlobalDiscriminator(global_discr),
                                     getDataTypesCache().getSerialization(variant_type_name), variant_type,
                                     variant_column.offsetAt(row_num), ostr);
}

void SerializationDynamic::serializeVariantForHashCalculation(const IColumn &column, const SerializationPtr &serialization,
                                                              const DataTypePtr &type, size_t row_num, WriteBuffer &ostr) {
...
static ReturnType deserializeVariant(ColumnVariant &variant_column, const SerializationPtr &variant_serialization,
                                     ColumnVariant::Discriminator global_discr, ReadBuffer &istr, DeserializeFunc deserialize) {
  auto &variant = variant_column.getVariantByGlobalDiscriminator(global_discr);
  if constexpr (std::is_same_v<ReturnType, bool>) {
    if (!deserialize(*variant_serialization, variant, istr)) return ReturnType(false);
  } else {
...
void SerializationDynamic::deserializeBinary(ColumnDynamic &dynamic_column, ReadBuffer &istr, const FormatSettings &settings) const {
  ...
  if (it != variant_info.variant_name_to_discriminator.end()) {
    deserializeVariant(dynamic_column.getVariantColumn(), variant_serialization, it->second, istr,
                       [&settings](const ISerialization &serialization, IColumn &variant, ReadBuffer &buf) {
                         serialization.deserializeBinary(variant, buf, settings);
                       });
    return;
  }
  ...
  if (dynamic_column.addNewVariant(variant_type)) {
    ...
    deserializeVariant(dynamic_column.getVariantColumn(), variant_serialization, discr, istr,
                       [&settings](const ISerialization &serialization, IColumn &variant, ReadBuffer &buf) {
                         serialization.deserializeBinary(variant, buf, settings);
                       });
    return;
...
static void deserializeTextImpl(IColumn &column, ReadBuffer &istr, const FormatSettings &settings, ReadFieldFunc read_field,
                                FormatSettings::EscapingRule escaping_rule, TryDeserializeVariantFunc try_deserialize_variant,
  ...
  if (escaping_rule == FormatSettings::EscapingRule::JSON) transformFinalInferredJSONTypeIfNeeded(variant_type, settings, &json_info);
  ...
  if (!checkIfTypeIsComplete(variant_type)) {
    ...
    for (ColumnVariant::Discriminator i = 0; i != variant_types.size(); ++i) {
      ...
      if (i != shared_variant_discr &&
          deserializeVariant<bool>(variant_column, dynamic_column.getVariantSerialization(variant_types[i], variant_info.variant_names[i]),
                                   i, *field_buf, try_deserialize_variant))
        return;
    }
  ...
  if (dynamic_column.addNewVariant(variant_type, variant_type->getName())) {
    ...
    deserializeVariant(dynamic_column.getVariantColumn(), dynamic_column.getVariantSerialization(variant_type), discr, *field_buf,
                       deserialize_variant);
    return;
  }
...
static void serializeTextImpl(const IColumn &column, size_t row_num, WriteBuffer &ostr, NestedSerialize nested_serialize) {
  const auto &dynamic_column = assert_cast<const ColumnDynamic &>(column);
  const auto &variant_column = dynamic_column.getVariantColumn();
...
void SerializationDynamic::serializeTextCSV(const IColumn &column, size_t row_num, WriteBuffer &ostr,
                                            const FormatSettings &settings) const {
...
void SerializationDynamic::serializeTextEscaped(const IColumn &column, size_t row_num, WriteBuffer &ostr,
                                                const FormatSettings &settings) const {
  auto nested_serialize = [&settings](const ISerialization &serialization, const IColumn &col, size_t row, WriteBuffer &buf) {
...
void SerializationDynamic::deserializeTextEscaped(IColumn &column, ReadBuffer &istr, const FormatSettings &settings) const {
  ...
  deserializeTextImpl(column, istr, settings, read_field, FormatSettings::EscapingRule::Escaped, try_deserialize_variant,
                      deserialize_variant);
}
...
void SerializationDynamic::serializeTextQuoted(const IColumn &column, size_t row_num, WriteBuffer &ostr,
                                               const FormatSettings &settings) const {
  auto nested_serialize = [&settings](const ISerialization &serialization, const IColumn &col, size_t row, WriteBuffer &buf) {
...
void SerializationDynamic::deserializeTextQuoted(IColumn &column, ReadBuffer &istr, const FormatSettings &settings) const {
  ...
  deserializeTextImpl(column, istr, settings, read_field, FormatSettings::EscapingRule::Quoted, try_deserialize_variant,
                      deserialize_variant);
}
...
void SerializationDynamic::serializeTextJSON(const IColumn &column, size_t row_num, WriteBuffer &ostr,
                                             const FormatSettings &settings) const {
  auto nested_serialize = [&settings](const ISerialization &serialization, const IColumn &col, size_t row, WriteBuffer &buf) {
...
void SerializationDynamic::serializeTextJSONPretty(const IColumn &column, size_t row_num, WriteBuffer &ostr, const FormatSettings &settings,
                                                   size_t indent) const {
...
void SerializationDynamic::serializeTextRaw(const IColumn &column, size_t row_num, WriteBuffer &ostr,
                                            const FormatSettings &settings) const {
  auto nested_serialize = [&settings](const ISerialization &serialization, const IColumn &col, size_t row, WriteBuffer &buf) {
...
void SerializationDynamic::serializeTextXML(const IColumn &column, size_t row_num, WriteBuffer &ostr,
                                            const FormatSettings &settings) const {
  auto nested_serialize = [&settings](const ISerialization &serialization, const IColumn &col, size_t row, WriteBuffer &buf) {
