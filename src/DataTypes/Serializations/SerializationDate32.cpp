...
void SerializationDate32::deserializeWholeText(IColumn &column, ReadBuffer &istr, const FormatSettings &settings) const {
  ...
  if (!istr.eof()) throwUnexpectedDataAfterParsedValue(column, istr, settings, "Date32");
}

bool SerializationDate32::tryDeserializeWholeText(IColumn &column, ReadBuffer &istr, const FormatSettings &) const {
  ExtendedDayNum x;
  if (!tryReadDateText(x, istr, time_zone) || !istr.eof()) return false;
...
bool SerializationDate32::tryDeserializeTextEscaped(IColumn &column, ReadBuffer &istr, const FormatSettings &) const {
  ExtendedDayNum x;
  if (!tryReadDateText(x, istr, time_zone)) return false;
  assert_cast<ColumnInt32 &>(column).getData().push_back(x);
  return true;
}

void SerializationDate32::serializeTextEscaped(const IColumn &column, size_t row_num, WriteBuffer &ostr,
                                               const FormatSettings &settings) const {
  serializeText(column, row_num, ostr, settings);
}

void SerializationDate32::serializeTextQuoted(const IColumn &column, size_t row_num, WriteBuffer &ostr,
                                              const FormatSettings &settings) const {
  writeChar('\'', ostr);
  serializeText(column, row_num, ostr, settings);
  writeChar('\'', ostr);
...
bool SerializationDate32::tryDeserializeTextQuoted(IColumn &column, ReadBuffer &istr, const FormatSettings &) const {
  ExtendedDayNum x;
  if (!checkChar('\'', istr) || !tryReadDateText(x, istr, time_zone) || !checkChar('\'', istr)) return false;
  assert_cast<ColumnInt32 &>(column).getData().push_back(x);  /// It's important to do this at the end - for exception safety.
  return true;
}

void SerializationDate32::serializeTextJSON(const IColumn &column, size_t row_num, WriteBuffer &ostr,
                                            const FormatSettings &settings) const {
  writeChar('"', ostr);
  serializeText(column, row_num, ostr, settings);
  writeChar('"', ostr);
...
bool SerializationDate32::tryDeserializeTextJSON(IColumn &column, ReadBuffer &istr, const FormatSettings &format_settings) const {
  if (!checkChar('"', istr)) return SerializationNumber<Int32>::tryDeserializeTextJSON(column, istr, format_settings);

  ExtendedDayNum x;
  if (!tryReadDateText(x, istr, time_zone) || !checkChar('"', istr)) return false;
  assert_cast<ColumnInt32 &>(column).getData().push_back(x);
  return true;
}
