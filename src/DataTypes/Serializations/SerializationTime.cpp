...
inline void readTimeText(time_t &x, ReadBuffer &istr, const FormatSettings & /*settings*/, const DateLUTImpl &time_zone,
...
inline bool tryReadTimeText(time_t &x, ReadBuffer &istr, const FormatSettings & /*settings*/, const DateLUTImpl &time_zone,
                            const DateLUTImpl & /*utc_time_zone*/) {
  bool res;
...
void SerializationTime::deserializeWholeText(IColumn &column, ReadBuffer &istr, const FormatSettings &settings) const {
  ...
  if (!istr.eof()) throwUnexpectedDataAfterParsedValue(column, istr, settings, "Time");
}

bool SerializationTime::tryDeserializeWholeText(IColumn &column, ReadBuffer &istr, const FormatSettings & /*settings*/) const {
  time_t x = 0;
  if (!tryReadTimeText(x, istr) || !istr.eof()) return false;
  assert_cast<ColumnType &>(column).getData().push_back(static_cast<Int32>(x));
  return true;
}

void SerializationTime::serializeTextEscaped(const IColumn &column, size_t row_num, WriteBuffer &ostr,
...
bool SerializationTime::tryDeserializeTextEscaped(IColumn &column, ReadBuffer &istr, const FormatSettings & /*settings*/) const {
  time_t x = 0;
  if (!tryReadTimeText(x, istr)) return false;
  assert_cast<ColumnType &>(column).getData().push_back(static_cast<Int32>(x));
  return true;
}

void SerializationTime::serializeTextQuoted(const IColumn &column, size_t row_num, WriteBuffer &ostr,
                                            const FormatSettings &settings) const {
  writeChar('\'', ostr);
  serializeText(column, row_num, ostr, settings);
  writeChar('\'', ostr);
...
bool SerializationTime::tryDeserializeTextQuoted(IColumn &column, ReadBuffer &istr, const FormatSettings & /*settings*/) const {
  ...
  {
    if (!tryReadTimeText(x, istr) || !checkChar('\'', istr)) return false;
  } else {
...
void SerializationTime::deserializeTextCSV(IColumn &column, ReadBuffer &istr, const FormatSettings &settings) const {
  ...
  if (maybe_quote == '\'' || maybe_quote == '"') {
    ...
    readTimeText(x, buf);
    if (!buf.eof())
      throw Exception(ErrorCodes::UNEXPECTED_DATA_AFTER_PARSED_VALUE, "Unexpected data '{}' after parsed Time value '{}'",
...
bool SerializationTime::tryDeserializeTextCSV(IColumn &column, ReadBuffer &istr, const FormatSettings &settings) const {
  ...
  if (maybe_quote == '\'' || maybe_quote == '"') {
    ++istr.position();
    if (!tryReadTimeText(x, istr) || !checkChar(maybe_quote, istr)) return false;
  } else {
    String time_str;
    readCSVString(time_str, istr, settings.csv);
    ReadBufferFromString buf(time_str);
    if (!tryReadTimeText(x, buf) || !buf.eof()) return false;
  }
