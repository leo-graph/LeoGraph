...
void SerializationIP<IPv>::serializeText(const DB::IColumn &column, size_t row_num, DB::WriteBuffer &ostr,
                                         const DB::FormatSettings &) const {
  writeText(assert_cast<const ColumnVector<IPv> &>(column).getData()[row_num], ostr);
}

template <typename IPv>
void SerializationIP<IPv>::deserializeText(DB::IColumn &column, DB::ReadBuffer &istr, const DB::FormatSettings &settings,
                                           bool whole) const {
  IPv x;
  readText(x, istr);
  ...
  if (whole && !istr.eof()) throwUnexpectedDataAfterParsedValue(column, istr, settings, {TypeName<IPv>.data(), TypeName<IPv>.size()});
}
...
bool SerializationIP<IPv>::tryDeserializeText(DB::IColumn &column, DB::ReadBuffer &istr, const DB::FormatSettings &, bool whole) const {
  IPv x;
  if (!tryReadText(x, istr) || (whole && !istr.eof())) return false;
...
template <typename IPv>
void SerializationIP<IPv>::serializeTextQuoted(const DB::IColumn &column, size_t row_num, DB::WriteBuffer &ostr,
...
bool SerializationIP<IPv>::tryDeserializeTextQuoted(DB::IColumn &column, DB::ReadBuffer &istr, const DB::FormatSettings &) const {
  IPv x;
  if (!checkChar('\'', istr) || !tryReadText(x, istr) || !checkChar('\'', istr)) return false;
  assert_cast<ColumnVector<IPv> &>(column).getData().push_back(x);
  return true;
}

template <typename IPv>
void SerializationIP<IPv>::serializeTextJSON(const DB::IColumn &column, size_t row_num, DB::WriteBuffer &ostr,
                                             const DB::FormatSettings &settings) const {
  writeChar('"', ostr);
  serializeText(column, row_num, ostr, settings);
  writeChar('"', ostr);
...
void SerializationIP<IPv>::deserializeTextJSON(DB::IColumn &column, DB::ReadBuffer &istr, const DB::FormatSettings &settings) const {
  ...
  if (istr.eof()) assertChar('"', istr);
  assert_cast<ColumnVector<IPv> &>(column).getData().push_back(x);
  if (*istr.position() != '"') throwUnexpectedDataAfterParsedValue(column, istr, settings, {TypeName<IPv>.data(), TypeName<IPv>.size()});
  istr.ignore();
}
...
template <typename IPv>
void SerializationIP<IPv>::serializeTextCSV(const DB::IColumn &column, size_t row_num, DB::WriteBuffer &ostr,
                                            const DB::FormatSettings &settings) const {
  writeChar('"', ostr);
  serializeText(column, row_num, ostr, settings);
  writeChar('"', ostr);
...
bool SerializationIP<IPv>::tryDeserializeTextCSV(DB::IColumn &column, DB::ReadBuffer &istr, const DB::FormatSettings &) const {
  IPv value;
  if (!tryReadCSV(value, istr)) return false;
...
template <typename IPv>
void SerializationIP<IPv>::serializeBinary(const DB::IColumn &column, size_t row_num, DB::WriteBuffer &ostr,
                                           const DB::FormatSettings &) const {
...
void SerializationIP<IPv>::serializeBinaryBulk(const DB::IColumn &column, DB::WriteBuffer &ostr, size_t offset, size_t limit) const {
  ...
  if (limit) ostr.write(reinterpret_cast<const char *>(&x[offset]), sizeof(IPv) * limit);
}

template <typename IPv>
void SerializationIP<IPv>::deserializeBinaryBulk(DB::IColumn &column, DB::ReadBuffer &istr, size_t rows_offset, size_t limit,
                                                 double) const {
