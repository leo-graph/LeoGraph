...
SerializationSubObjectSharedData::SerializationSubObjectSharedData(
    SerializationObjectSharedData::SerializationVersion serialization_version_, size_t buckets_, const String &paths_prefix_,
...
void SerializationSubObjectSharedData::enumerateStreams(ISerialization::EnumerateStreamsSettings &settings,
                                                        const ISerialization::StreamCallback &callback,
                                                        const ISerialization::SubstreamData &data) const {
  const auto *sub_object_shared_data_state =
      data.deserialize_state ? checkAndGetState<DeserializeBinaryBulkStateSubObjectSharedData>(data.deserialize_state) : nullptr;
  ...
  for (size_t bucket = 0; bucket != buckets; ++bucket) {
    ...
    if (serialization_version.value == SerializationObjectSharedData::SerializationVersion::MAP_WITH_BUCKETS) {
      auto map_data =
          SubstreamData(serialization_map)
              .withColumn(data.column)
              .withType(data.type)
              .withSerializationInfo(data.serialization_info)
...
void SerializationSubObjectSharedData::serializeBinaryBulkStatePrefix(const IColumn &, ISerialization::SerializeBinaryBulkSettings &,
                                                                      ISerialization::SerializeBinaryBulkStatePtr &) const {
  throw Exception(ErrorCodes::NOT_IMPLEMENTED,
                  "Method serializeBinaryBulkStatePrefix is not implemented for SerializationSubObjectSharedData");
}

void SerializationSubObjectSharedData::serializeBinaryBulkWithMultipleStreams(const IColumn &, size_t, size_t,
                                                                              ISerialization::SerializeBinaryBulkSettings &,
                                                                              ISerialization::SerializeBinaryBulkStatePtr &) const {
  throw Exception(ErrorCodes::NOT_IMPLEMENTED,
                  "Method serializeBinaryBulkWithMultipleStreams is not implemented for SerializationSubObjectSharedData");
}

void SerializationSubObjectSharedData::serializeBinaryBulkStateSuffix(ISerialization::SerializeBinaryBulkSettings &,
                                                                      ISerialization::SerializeBinaryBulkStatePtr &) const {
  throw Exception(ErrorCodes::NOT_IMPLEMENTED,
                  "Method serializeBinaryBulkStateSuffix is not implemented for SerializationSubObjectSharedData");
}

void SerializationSubObjectSharedData::deserializeBinaryBulkStatePrefix(ISerialization::DeserializeBinaryBulkSettings &settings,
  ...
  if (serialization_version.value == SerializationObjectSharedData::SerializationVersion::MAP) {
    ...
    for (size_t bucket = 0; bucket != buckets; ++bucket) {
      ...
      auto *structure_state_concrete = checkAndGetState<SerializationObjectSharedData::DeserializeBinaryBulkStateObjectSharedDataStructure>(
          shared_data_state->bucket_structure_states[bucket]);
      ...
      std::erase_if(structure_state_concrete->requested_paths_subcolumns,
                    [&](const auto &pair) { return pair.first.starts_with(paths_prefix); });
      settings.path.pop_back();
    }
  } else {
    /// If we add new serialization version in future and forget to implement something, better to get an exception instead of doing
...
void SerializationSubObjectSharedData::deserializeBinaryBulkWithMultipleStreams(ColumnPtr &column, size_t rows_offset, size_t limit,
  ...
  if (serialization_version.value == SerializationObjectSharedData::SerializationVersion::MAP) {
    ...
    /// If we don't have it in cache, deserialize and put deserialized map in cache.
      ...
      serialization_map->deserializeBinaryBulkWithMultipleStreams(sub_object_shared_data_state->map_column, rows_offset, limit, settings,
                                                                  sub_object_shared_data_state->map_state, cache);
    ...
    /// Iterate over new rows in map column and extract paths with requested prefix.
      ...
      auto [src_shared_data_paths, src_shared_data_values, src_shared_data_offsets] =
          ColumnObject::getSharedDataPathsValuesAndOffsets(*sub_object_shared_data_state->map_column);
      auto [dst_shared_data_paths, dst_shared_data_values, dst_shared_data_offsets] =
          ColumnObject::getSharedDataPathsValuesAndOffsets(*column->assumeMutable());
      ...
      for (size_t i = map_column_offset; i != sub_object_shared_data_state->map_column->size(); ++i) {
        ...
        for (; lower_bound_index != end; ++lower_bound_index) {
          ...
          if (!path.starts_with(paths_prefix)) break;
    ...
    for (size_t bucket = 0; bucket != buckets; ++bucket) {
      ...
      /// If we don't have it in cache, deserialize and put deserialized map in cache.
        ...
        serialization_map->deserializeBinaryBulkWithMultipleStreams(bucket_map_columns[bucket], rows_offset, limit, settings,
                                                                    sub_object_shared_data_state->bucket_map_states[bucket], cache);
    ...
    for (size_t bucket = 0; bucket != buckets; ++bucket) {
      ...
      auto *shared_data_structure_state =
          checkAndGetState<SerializationObjectSharedData::DeserializeBinaryBulkStateObjectSharedDataStructure>(
              sub_object_shared_data_state->bucket_structure_states[bucket]);
      auto structure_granules =
          SerializationObjectSharedData::deserializeStructure(rows_offset, limit, settings, *shared_data_structure_state, cache);
      auto paths_infos_granules =
          SerializationObjectSharedData::deserializePathsInfos(*structure_granules, *shared_data_structure_state, settings, cache);
      bucket_paths_data_granules[bucket] = SerializationObjectSharedData::deserializePathsData(
          *structure_granules, *paths_infos_granules, *shared_data_structure_state, settings, dynamic_type, dynamic_serialization, cache);
    ...
    auto [shared_data_paths, shared_data_values, shared_data_offsets] =
        ColumnObject::getSharedDataPathsValuesAndOffsets(*column->assumeMutable());
    ...
    /// nothing.
    throw Exception(ErrorCodes::NOT_IMPLEMENTED,
                    "deserializeBinaryBulkWithMultipleStreams is not implemented for shared data serialization version {}",
                    serialization_version.value);
  }
}
