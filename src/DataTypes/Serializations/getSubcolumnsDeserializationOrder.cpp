...
std::vector<size_t> getSubcolumnsDeserializationOrder(const String& column_name,
  ...
  for (size_t i = 0; i != substreams_in_serialization_order.size(); ++i) substream_to_pos[substreams_in_serialization_order[i]] = i;
  ...
  for (size_t i = 0; i != subcolumns_positions.size(); ++i) subcolumns_positions[i] = i;
  std::sort(subcolumns_positions.begin(), subcolumns_positions.end(),
            [&](size_t left, size_t right) { return subcolumns_substreams_positions[left] < subcolumns_substreams_positions[right]; });
