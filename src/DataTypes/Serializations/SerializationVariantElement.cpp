...
void SerializationVariantElement::enumerateStreams(DB::ISerialization::EnumerateStreamsSettings &settings,
  ...
  const auto *deserialize_state =
      data.deserialize_state ? checkAndGetState<DeserializeBinaryBulkStateVariantElement>(data.deserialize_state) : nullptr;
...
void SerializationVariantElement::serializeBinaryBulkStatePrefix(const IColumn &, SerializeBinaryBulkSettings &,
                                                                 SerializeBinaryBulkStatePtr &) const {
  throw Exception(ErrorCodes::NOT_IMPLEMENTED, "Method serializeBinaryBulkStatePrefix is not implemented for SerializationVariantElement");
}
...
void SerializationVariantElement::serializeBinaryBulkStateSuffix(SerializeBinaryBulkSettings &, SerializeBinaryBulkStatePtr &) const {
  throw Exception(ErrorCodes::NOT_IMPLEMENTED, "Method serializeBinaryBulkStateSuffix is not implemented for SerializationVariantElement");
}

void SerializationVariantElement::deserializeBinaryBulkStatePrefix(DeserializeBinaryBulkSettings &settings,
...
void SerializationVariantElement::serializeBinaryBulkWithMultipleStreams(const IColumn &, size_t, size_t, SerializeBinaryBulkSettings &,
                                                                         SerializeBinaryBulkStatePtr &) const {
  throw Exception(ErrorCodes::NOT_IMPLEMENTED,
                  "Method serializeBinaryBulkWithMultipleStreams is not implemented for SerializationVariantElement");
}

void SerializationVariantElement::deserializeBinaryBulkWithMultipleStreams(ColumnPtr &result_column, size_t rows_offset, size_t limit,
  ...
  if (auto cached_column_with_num_read_rows = getColumnWithNumReadRowsFromSubstreamsCache(cache, settings.path)) {
    ...
    if (discriminators_state->mode.value == SerializationVariant::DiscriminatorsSerializationMode::BASIC) {
      SerializationNumber<ColumnVariant::Discriminator>().deserializeBinaryBulk(*variant_element_state->discriminators->assumeMutable(),
                                                                                *discriminators_stream, 0, rows_offset + limit, 0);
    } else {
      auto variant_pair = deserializeCompactDiscriminators(variant_element_state->discriminators, variant_discriminator, rows_offset, limit,
                                                           discriminators_stream, settings.continuous_reading,
                                                           variant_element_state->discriminators_state, this);
  ...
  if (!variant_rows_offset) {
    ...
    if (rows_offset) {
      const auto &discriminators_data =
          assert_cast<const ColumnVariant::ColumnDiscriminators &>(*variant_element_state->discriminators).getData();
  ...
  nested_serialization->deserializeBinaryBulkWithMultipleStreams(variant_element_state->variant, *variant_rows_offset, *variant_limit,
                                                                 nested_settings, variant_element_state->variant_element_state, cache);
  removeVariantFromPath(settings.path);
  ...
  if (mutable_column->empty()) mutable_column->takeDynamicStructureFromColumn(variant_element_state->variant);

  /// If there was nothing to deserialize or nothing was actually deserialized when variant_limit > 0, just insert defaults.
  /// The second case means that we don't have a stream for such sub-column. It may happen during ALTER MODIFY column with Variant
  /// extension.
  ...
  if (variant_element_state->variant->size() < *variant_limit)
    throw Exception(ErrorCodes::LOGICAL_ERROR, "Size of deserialized variant column less than the limit: {} < {}",
                    variant_element_state->variant->size(), *variant_limit);
...
std::pair<size_t, size_t> SerializationVariantElement::deserializeCompactDiscriminators(
    DB::ColumnPtr &discriminators_column, ColumnVariant::Discriminator variant_discriminator, size_t rows_offset, size_t limit,
    DB::ReadBuffer *stream, bool continuous_reading, DeserializeBinaryBulkStatePtr &discriminators_state_,
    const ISerialization *serialization) {
  auto *discriminators_state =
      checkAndGetState<SerializationVariant::DeserializeBinaryBulkStateVariantDiscriminators>(discriminators_state_, serialization);
  ...
  if (!continuous_reading) discriminators_state->remaining_rows_in_granule = 0;
  ...
  while (limit) {
    ...
    if (discriminators_state->remaining_rows_in_granule == 0) {
      if (stream->eof()) return {variant_rows_offset, variant_limit};
    ...
    if (discriminators_state->granule_format == SerializationVariant::CompactDiscriminatorsGranuleFormat::COMPACT) {
      ...
      if (rows_offset) {
        ...
        if (discriminators_state->compact_discr == variant_discriminator) variant_rows_offset += skipped_rows;
      ...
      for (size_t i = start; i != start + skipped_rows; ++i) variant_rows_offset += (discriminators_data[i] == variant_discriminator);
...
SerializationVariantElement::VariantSubcolumnCreator::VariantSubcolumnCreator(const ColumnPtr &local_discriminators_,
                                                                              ...
                                                                              bool make_nullable_, const ColumnPtr &null_map_)
...
ColumnPtr SerializationVariantElement::VariantSubcolumnCreator::create(const DB::ColumnPtr &prev) const {
  ...
  if (prev->size() == local_discriminators->size()) return make_nullable ? makeNullableOrLowCardinalityNullableSafe(prev) : prev;
  ...
  if (!null_map) {
    ...
    for (auto local_discr : local_discriminators_data) null_map_from_discriminators->push_back(local_discr != local_variant_discriminator);
  }
  ...
  if (make_nullable && prev->lowCardinality()) res_column = assert_cast<ColumnLowCardinality &>(*res_column).cloneNullable();
