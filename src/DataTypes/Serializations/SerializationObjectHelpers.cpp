...
std::vector<std::vector<std::pair<String, ColumnPtr>>> flattenAndBucketSharedDataPaths(const IColumn &shared_data_column, size_t start,
                                                                                       size_t end, const DataTypePtr &dynamic_type,
                                                                                       size_t num_buckets) {
  ...
  const auto [shared_data_paths, shared_data_values, shared_data_offsets] =
      ColumnObject::getSharedDataPathsValuesAndOffsets(shared_data_column);
  for (size_t i = start; i != end; ++i) {
    ...
    for (const auto &[_, column] : flattened_shared_data_paths) {
      if (column->size() != i + 1 - start) column->insertDefault();
    }
  }
...
std::vector<std::pair<String, ColumnPtr>> flattenPaths(const ColumnObject &object_column) {
  auto all_paths = std::move(
      flattenAndBucketSharedDataPaths(*object_column.getSharedDataPtr(), 0, object_column.size(), object_column.getDynamicType(), 1)[0]);
  for (const auto &[path, column] : object_column.getDynamicPaths()) all_paths.emplace_back(path, column);
  std::sort(all_paths.begin(), all_paths.end());
  return all_paths;
}

void unflattenAndInsertPaths(const std::vector<String> &flattened_paths, std::vector<ColumnPtr> &&flattened_columns,
                             ColumnObject &object_column, size_t num_rows) {
  ...
  for (const auto &[path, column] : paths_for_shared_data) dynamic_columns_ptrs[path] = assert_cast<const ColumnDynamic *>(column.get());
...
std::vector<ColumnPtr> splitSharedDataPathsToBuckets(const IColumn &shared_data_column, size_t start, size_t end, size_t num_buckets) {
  ...
  for (size_t i = 0; i != num_buckets; ++i) {
    ...
    std::tie(shared_data_paths_buckets[i], shared_data_values_buckets[i], shared_data_offsets_buckets[i]) =
        ColumnObject::getSharedDataPathsValuesAndOffsets(*column);
...
void collectSharedDataFromBuckets(const std::vector<ColumnPtr> &shared_data_buckets, IColumn &shared_data_column,
  ...
  for (size_t i = 0; i != shared_data_buckets.size(); ++i)
    std::tie(shared_data_paths_buckets[i], shared_data_values_buckets[i], shared_data_offsets_buckets[i]) =
        ColumnObject::getSharedDataPathsValuesAndOffsets(*shared_data_buckets[i]);
    ...
    for (size_t bucket = 0; bucket != shared_data_buckets.size(); ++bucket) {
      ...
      /// Otherwise collect only paths that match the prefix.
        ...
        size_t lower_bound_index =
            ColumnObject::findPathLowerBoundInSharedData(*paths_prefix, *shared_data_paths_buckets[bucket], offset_start, offset_end);
...
template <typename T>
ColumnPtr createPathsIndexesImpl(const std::unordered_map<std::string_view, size_t> &path_to_index, const ColumnString &paths_column,
                                 size_t start, size_t end) {
  auto indexes_column = ColumnVector<T>::create();
  auto &data = indexes_column->getData();
  data.reserve(end - start);
  for (size_t i = start; i < end; ++i) data.push_back(static_cast<T>(path_to_index.at(paths_column.getDataAt(i))));
  return indexes_column;
}

template <typename T = UInt8>
void deserializeIndexesAndCollectPathsImpl(ColumnString &paths_column, ReadBuffer &istr, std::vector<String> &&paths, size_t rows_offset,
                                           size_t limit) {
...
std::pair<ColumnPtr, DataTypePtr> createPathsIndexes(const std::unordered_map<std::string_view, size_t> &path_to_index,
                                                     const IColumn &paths_column, size_t start, size_t end) {
...
void deserializeIndexesAndCollectPaths(IColumn &paths_column, ReadBuffer &istr, std::vector<String> &&paths, size_t rows_offset,
                                       size_t limit) {
