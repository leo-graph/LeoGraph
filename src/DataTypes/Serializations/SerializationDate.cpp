...
void SerializationDate::deserializeWholeText(IColumn &column, ReadBuffer &istr, const FormatSettings &settings) const {
  ...
  if (!istr.eof()) throwUnexpectedDataAfterParsedValue(column, istr, settings, "Date");
}
...
void SerializationDate::serializeTextEscaped(const IColumn &column, size_t row_num, WriteBuffer &ostr,
                                             const FormatSettings &settings) const {
  serializeText(column, row_num, ostr, settings);
}

void SerializationDate::serializeTextQuoted(const IColumn &column, size_t row_num, WriteBuffer &ostr,
                                            const FormatSettings &settings) const {
  writeChar('\'', ostr);
  serializeText(column, row_num, ostr, settings);
  writeChar('\'', ostr);
...
bool SerializationDate::tryDeserializeTextJSON(IColumn &column, ReadBuffer &istr, const FormatSettings &format_settings) const {
  if (!checkChar('"', istr)) return SerializationNumber<UInt16>::tryDeserializeTextJSON(column, istr, format_settings);
...
bool SerializationDate::tryDeserializeTextCSV(IColumn &column, ReadBuffer &istr, const FormatSettings &) const {
  DayNum value;
  if (!tryReadCSV(value, istr, time_zone)) return false;
  assert_cast<ColumnUInt16 &>(column).getData().push_back(value);
  return true;
}
