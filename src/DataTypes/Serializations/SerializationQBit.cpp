...
static const ColumnTuple &extractNestedColumn(const IColumn &column) { return assert_cast<const ColumnQBit &>(column).getNestedData(); }

static ColumnTuple &extractNestedColumn(IColumn &column) { return assert_cast<ColumnQBit &>(column).getNestedData(); }

/* Get const idx-th column */
static inline const IColumn &extractElementColumn(const IColumn &column, size_t idx) { return extractNestedColumn(column).getColumn(idx); }

/* Get idx-th column */
static inline IColumn &extractElementColumn(IColumn &column, size_t idx) { return extractNestedColumn(column).getColumn(idx); }
...
size_t SerializationQBit::validateAndReadQBitSize(ReadBuffer &istr, const FormatSettings &settings) const {
  ...
  if (size != dimension)
    throw Exception(ErrorCodes::SERIALIZATION_ERROR, "Dimension of the read QBit {} doesn't match expected dimension {}", size, dimension);
...
Tuple SerializationQBit::deserializeFloatsToQBitTuple(ReadBuffer &istr) const {
  ...
  for (size_t col_idx = 0; col_idx < element_size; ++col_idx) plane_ptrs[col_idx] = reinterpret_cast<char *>(planes[col_idx].data());
  ...
  for (size_t col_idx = 0; col_idx < element_size; ++col_idx) tuple_elements.push_back(Field(std::move(planes[col_idx])));
...
void SerializationQBit::serializeBinary(const Field &field, WriteBuffer &ostr, const FormatSettings &) const {
  ...
  if (tuple.size() != element_size)
    throw Exception(ErrorCodes::SERIALIZATION_ERROR, "QBit tuple size {} doesn't match expected element_size {}", tuple.size(),
                    element_size);
...
void SerializationQBit::deserializeText(IColumn &column, ReadBuffer &istr, const FormatSettings &settings, bool whole) const {
  ...
  auto deserialize = [&]() -> bool {
    ...
    if (whole && !istr.eof()) throwUnexpectedDataAfterParsedValue(column, istr, settings, "QBit");
...
void SerializationQBit::enumerateStreams(EnumerateStreamsSettings &settings, const StreamCallback &callback,
...
void SerializationQBit::serializeBinaryBulkStatePrefix(const IColumn &column, SerializeBinaryBulkSettings &settings,
...
void SerializationQBit::deserializeBinaryBulkStatePrefix(DeserializeBinaryBulkSettings &settings, DeserializeBinaryBulkStatePtr &state,
                                                         SubstreamsDeserializeStatesCache *cache) const {
  nested->deserializeBinaryBulkStatePrefix(settings, state, cache);
}

void SerializationQBit::serializeBinaryBulkWithMultipleStreams(const IColumn &column, size_t offset, size_t limit,
                                                               SerializeBinaryBulkSettings &settings,
                                                               SerializeBinaryBulkStatePtr &state) const {
  nested->serializeBinaryBulkWithMultipleStreams(extractNestedColumn(column), offset, limit, settings, state);
}

void SerializationQBit::deserializeBinaryBulkWithMultipleStreams(ColumnPtr &column, size_t rows_offset, size_t limit,
...
DECLARE_DEFAULT_CODE(template <typename T> ALWAYS_INLINE inline void untransposeBitPlaneImpl(const UInt8 *__restrict src, T *__restrict dst,
                                                                                             size_t stride_len, T bit_mask) {
...
DECLARE_X86_64_V4_SPECIFIC_CODE(void untransposeBitPlaneFloat64Impl(const UInt8 *__restrict src, UInt64 *__restrict dst, size_t stride_len,
                                                                    UInt64 bit_mask) {
  const size_t bytes_per_fs = stride_len / 8;
  ssize_t row_base = stride_len - 1;
...
DECLARE_X86_64_V4_SPECIFIC_CODE(void untransposeBitPlaneFloat32Impl(const UInt8 *__restrict src, UInt32 *__restrict dst, size_t stride_len,
                                                                    UInt32 bit_mask) {
  const size_t bytes_per_fs = stride_len / 8;
  ssize_t row_base = stride_len - 1;
