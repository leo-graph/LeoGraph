...
void SerializationDynamicElement::enumerateStreams(DB::ISerialization::EnumerateStreamsSettings &settings,
  ...
  if (!deserialize_state->variant_serialization) return;

  settings.path.push_back(Substream::DynamicData);
  auto variant_data =
      SubstreamData(deserialize_state->variant_serialization)
...
void SerializationDynamicElement::serializeBinaryBulkStatePrefix(const IColumn &, SerializeBinaryBulkSettings &,
                                                                 SerializeBinaryBulkStatePtr &) const {
  throw Exception(ErrorCodes::NOT_IMPLEMENTED, "Method serializeBinaryBulkStatePrefix is not implemented for SerializationDynamicElement");
}
...
void SerializationDynamicElement::serializeBinaryBulkStateSuffix(SerializeBinaryBulkSettings &, SerializeBinaryBulkStatePtr &) const {
  throw Exception(ErrorCodes::NOT_IMPLEMENTED, "Method serializeBinaryBulkStateSuffix is not implemented for SerializationDynamicElement");
}

void SerializationDynamicElement::deserializeBinaryBulkStatePrefix(DeserializeBinaryBulkSettings &settings,
  ...
  if (auto global_discr = variant_type.tryGetVariantDiscriminator(dynamic_element_name)) {
    ...
    if (is_null_map_subcolumn)
      dynamic_element_state->variant_serialization =
          std::make_shared<SerializationVariantElementNullMap>(dynamic_element_name, *global_discr);
    else
      dynamic_element_state->variant_serialization =
          std::make_shared<SerializationVariantElement>(nested_serialization, dynamic_element_name, *global_discr);
  ...
  /// If we don't have this element in the Variant, we will read shared variant and try to find it there.
    ...
    dynamic_element_state->variant_serialization = std::make_shared<SerializationVariantElement>(
        shared_variant_serialization, ColumnDynamic::getSharedVariantTypeName(), *shared_variant_global_discr);
    dynamic_element_state->variant_serialization->deserializeBinaryBulkStatePrefix(settings, dynamic_element_state->variant_element_state,
                                                                                   cache);
...
void SerializationDynamicElement::serializeBinaryBulkWithMultipleStreams(const IColumn &, size_t, size_t, SerializeBinaryBulkSettings &,
                                                                         SerializeBinaryBulkStatePtr &) const {
  throw Exception(ErrorCodes::NOT_IMPLEMENTED,
                  "Method serializeBinaryBulkWithMultipleStreams is not implemented for SerializationDynamicElement");
}

void SerializationDynamicElement::deserializeBinaryBulkWithMultipleStreams(ColumnPtr &result_column, size_t rows_offset, size_t limit,
  ...
  /// Otherwise, read the shared variant column and extract requested type from it.
    ...
    MutableColumnPtr variant_column =
        nested_subcolumn.empty() || is_null_map_subcolumn ? result_column->assumeMutable() : result_type->createColumn();
      ...
      if (!shared_null_map[i]) {
        ...
        if (type->getName() == dynamic_element_name) {
          ...
          if (is_low_cardinality_nullable) {
            ...
            if (null_map) null_map->push_back(static_cast<UInt8>(0));
          }
        } else if (is_null_map_subcolumn) {
