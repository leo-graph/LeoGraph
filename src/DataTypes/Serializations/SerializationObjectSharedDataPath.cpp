...
SerializationObjectSharedDataPath::SerializationObjectSharedDataPath(
    const SerializationPtr &nested_, SerializationObjectSharedData::SerializationVersion serialization_version_, const String &path_,
    const String &path_subcolumn_, const DataTypePtr &dynamic_type_, const SerializationPtr &dynamic_serialization_,
    const DataTypePtr &subcolumn_type_, size_t bucket_)
...
void SerializationObjectSharedDataPath::enumerateStreams(ISerialization::EnumerateStreamsSettings &settings,
  ...
  if (serialization_version.value == SerializationObjectSharedData::SerializationVersion::MAP ||
      serialization_version.value == SerializationObjectSharedData::SerializationVersion::MAP_WITH_BUCKETS) {
    ...
    const auto *deserialize_state =
        data.deserialize_state ? checkAndGetState<DeserializeBinaryBulkStateObjectSharedDataPath>(data.deserialize_state) : nullptr;
    ...
    if (serialization_version.value == SerializationObjectSharedData::SerializationVersion::MAP_WITH_BUCKETS) settings.path.pop_back();
...
void SerializationObjectSharedDataPath::serializeBinaryBulkStatePrefix(const IColumn &, ISerialization::SerializeBinaryBulkSettings &,
                                                                       ISerialization::SerializeBinaryBulkStatePtr &) const {
  throw Exception(ErrorCodes::NOT_IMPLEMENTED,
                  "Method serializeBinaryBulkStatePrefix is not implemented for SerializationObjectSharedDataPath");
}

void SerializationObjectSharedDataPath::serializeBinaryBulkWithMultipleStreams(const IColumn &, size_t, size_t,
...
void SerializationObjectSharedDataPath::serializeBinaryBulkStateSuffix(ISerialization::SerializeBinaryBulkSettings &,
                                                                       ISerialization::SerializeBinaryBulkStatePtr &) const {
  throw Exception(ErrorCodes::NOT_IMPLEMENTED,
                  "Method serializeBinaryBulkStateSuffix is not implemented for SerializationObjectSharedDataPath");
}

void SerializationObjectSharedDataPath::deserializeBinaryBulkStatePrefix(ISerialization::DeserializeBinaryBulkSettings &settings,
  ...
  if (serialization_version.value == SerializationObjectSharedData::SerializationVersion::MAP ||
    ...
    auto *structure_state_concrete = checkAndGetState<SerializationObjectSharedData::DeserializeBinaryBulkStateObjectSharedDataStructure>(
        shared_data_path_state->structure_state);
...
void SerializationObjectSharedDataPath::deserializeBinaryBulkWithMultipleStreams(ColumnPtr &column, size_t rows_offset, size_t limit,
  ...
  if (serialization_version.value == SerializationObjectSharedData::SerializationVersion::MAP ||
    ...
    if (serialization_version.value == SerializationObjectSharedData::SerializationVersion::MAP) {
      ...
      /// If we don't have it in cache, deserialize and put deserialized map in cache.
        ...
        serialization_map->deserializeBinaryBulkWithMultipleStreams(shared_data_path_state->map_column, rows_offset, limit, settings,
                                                                    shared_data_path_state->map_state, cache);
      ...
      /// If we don't have it in cache, deserialize and put deserialized map in cache.
        ...
        /// so we don't need to preserve old rows to keep valid usage of substreams cache (when column in cache is also present in the
        /// result columns). Here we can store only rows from current deserialization even in cache.
        map_column = DataTypeObject::getTypeOfSharedData()->createColumn();
        serialization_map->deserializeBinaryBulkWithMultipleStreams(map_column, rows_offset, limit, settings,
                                                                    shared_data_path_state->map_state, cache);
    ...
    auto paths_data_granules = SerializationObjectSharedData::deserializePathsData(
    ...
    for (size_t granule = 0; granule != structure_granules->size(); ++granule) {
      ...
      if (!structure_granule.limit) continue;
