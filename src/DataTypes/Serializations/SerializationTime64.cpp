...
SerializationTime64::SerializationTime64(UInt32 scale_) : SerializationDecimalBase<Time64>(DecimalUtils::max_precision<Time64>, scale_) {}
...
void SerializationTime64::serializeText(const IColumn &column, size_t row_num, WriteBuffer &ostr,
...
void SerializationTime64::deserializeText(IColumn &column, ReadBuffer &istr, const FormatSettings &settings, bool whole) const {
  ...
  if (whole && !istr.eof()) throwUnexpectedDataAfterParsedValue(column, istr, settings, "Time64");
}

bool SerializationTime64::tryDeserializeText(IColumn &column, ReadBuffer &istr, const FormatSettings &, bool whole) const {
  Time64 result = 0;
  if (!tryReadTime64Text(result, scale, istr, DateLUT::instance()) || (whole && !istr.eof())) return false;
...
void SerializationTime64::deserializeWholeText(IColumn &column, ReadBuffer &istr, const FormatSettings &settings) const {
  ...
  if (!istr.eof()) throwUnexpectedDataAfterParsedValue(column, istr, settings, "Time64");
}

void SerializationTime64::serializeTextEscaped(const IColumn &column, size_t row_num, WriteBuffer &ostr,
                                               const FormatSettings &settings) const {
  serializeText(column, row_num, ostr, settings);
}

static inline void readText(Time64 &x, UInt32 scale, ReadBuffer &istr, const FormatSettings & /*settings*/,
                            const DateLUTImpl & /*time_zone*/, const DateLUTImpl & /*utc_time_zone*/) {
  readTime64Text(x, scale, istr);
}

static inline bool tryReadText(Time64 &x, UInt32 scale, ReadBuffer &istr, const FormatSettings & /*settings*/,
                               const DateLUTImpl & /*time_zone*/, const DateLUTImpl & /*utc_time_zone*/) {
...
bool SerializationTime64::tryDeserializeWholeText(IColumn &column, ReadBuffer &istr, const FormatSettings &settings) const {
  Time64 x = 0;
  if (!tryReadText(x, scale, istr, settings, DateLUT::instance(), DateLUT::instance()) || !istr.eof()) return false;
...
bool SerializationTime64::tryDeserializeTextEscaped(IColumn &column, ReadBuffer &istr, const FormatSettings &settings) const {
  Time64 x = 0;
  if (!tryReadText(x, scale, istr, settings, DateLUT::instance(), DateLUT::instance())) return false;
  assert_cast<ColumnType &>(column).getData().push_back(x);
  return true;
}

void SerializationTime64::serializeTextQuoted(const IColumn &column, size_t row_num, WriteBuffer &ostr,
                                              const FormatSettings &settings) const {
  writeChar('\'', ostr);
  serializeText(column, row_num, ostr, settings);
  writeChar('\'', ostr);
...
bool SerializationTime64::tryDeserializeTextQuoted(IColumn &column, ReadBuffer &istr, const FormatSettings &settings) const {
  ...
  if (checkChar('\'', istr))  /// Cases: '18:36:48' or '1504193808'
  {
    if (!tryReadText(x, scale, istr, settings, DateLUT::instance(), DateLUT::instance()) || !checkChar('\'', istr)) return false;
  } else {
...
void SerializationTime64::serializeTextJSON(const IColumn &column, size_t row_num, WriteBuffer &ostr,
                                            const FormatSettings &settings) const {
  writeChar('"', ostr);
  serializeText(column, row_num, ostr, settings);
  writeChar('"', ostr);
...
bool SerializationTime64::tryDeserializeTextCSV(IColumn &column, ReadBuffer &istr, const FormatSettings &settings) const {
  ...
  if (maybe_quote == '\'' || maybe_quote == '\"') {
    ++istr.position();
    if (!tryReadText(x, scale, istr, settings, DateLUT::instance(), DateLUT::instance()) || !checkChar(maybe_quote, istr)) return false;
  } else {
    String datetime_str;
    readCSVString(datetime_str, istr, settings.csv);
    ReadBufferFromString buf(datetime_str);
    if (!tryReadText(x, scale, buf, settings, DateLUT::instance(), DateLUT::instance()) || !buf.eof()) return false;
  }
