...
void SerializationUUID::deserializeText(IColumn &column, ReadBuffer &istr, const FormatSettings &settings, bool whole) const {
  ...
  if (whole && !istr.eof()) throwUnexpectedDataAfterParsedValue(column, istr, settings, "UUID");
}
...
void SerializationUUID::serializeTextQuoted(const IColumn &column, size_t row_num, WriteBuffer &ostr,
...
bool SerializationUUID::tryDeserializeTextQuoted(IColumn &column, ReadBuffer &istr, const FormatSettings &) const {
  UUID uuid;
  if (!checkChar('\'', istr) || !tryReadText(uuid, istr) || !checkChar('\'', istr)) return false;
...
void SerializationUUID::serializeBinaryBulk(const IColumn &column, WriteBuffer &ostr, size_t offset, size_t limit) const {
  ...
  if (const size_t size = x.size(); limit == 0 || offset + limit > size) limit = size - offset;
...
#pragma clang diagnostic ignored "-Wunreachable-code"
  if constexpr (std::endian::native == std::endian::big) {
    for (size_t i = offset; i < offset + limit; ++i) writeBinaryLittleEndian(x[i], ostr);
  } else
    ostr.write(reinterpret_cast<const char *>(&x[offset]), sizeof(UUID) * limit);
#pragma clang diagnostic pop
}

void SerializationUUID::deserializeBinaryBulk(IColumn &column, ReadBuffer &istr, size_t rows_offset, size_t limit,
...
#pragma clang diagnostic ignored "-Wunreachable-code"
  if constexpr (std::endian::native == std::endian::big)
    for (size_t i = initial_size; i < x.size(); ++i) transformEndianness<std::endian::big, std::endian::little>(x[i]);
#pragma clang diagnostic pop
}
