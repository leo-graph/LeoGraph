...
namespace {
const ColumnLowCardinality &getColumnLowCardinality(const IColumn &column) { return typeid_cast<const ColumnLowCardinality &>(column); }

ColumnLowCardinality &getColumnLowCardinality(IColumn &column) { return typeid_cast<ColumnLowCardinality &>(column); }
}  // namespace

SerializationLowCardinality::SerializationLowCardinality(const DataTypePtr &dictionary_type_)
    : dictionary_type(dictionary_type_), dict_inner_serialization(removeNullable(dictionary_type_)->getDefaultSerialization()) {}

void SerializationLowCardinality::enumerateStreams(EnumerateStreamsSettings &settings, const StreamCallback &callback,
...
struct IndexesSerializationType {
  ...
  void serialize(WriteBuffer &buffer) const {
    ...
    if (has_additional_keys) val |= HasAdditionalKeysBit;
    if (need_global_dictionary) val |= NeedGlobalDictionaryBit;
    if (need_update_dictionary) val |= NeedUpdateDictionary;
  ...
  IndexesSerializationType(const IColumn &column, bool has_additional_keys_, bool need_global_dictionary_, bool enumerate_dictionaries)
    ...
    if (typeid_cast<const ColumnUInt8 *>(&column))
      ...
      throw Exception(ErrorCodes::LOGICAL_ERROR,
                      "Invalid Indexes column for IndexesSerializationType. "
                      "Expected ColumnUInt*, got {}",
                      column.getName());
  }

  DataTypePtr getDataType() const {
    if (type == TUInt8) return std::make_shared<DataTypeUInt8>();
    if (type == TUInt16) return std::make_shared<DataTypeUInt16>();
    if (type == TUInt32) return std::make_shared<DataTypeUInt32>();
    if (type == TUInt64) return std::make_shared<DataTypeUInt64>();
...
struct DeserializeStateLowCardinality : public ISerialization::DeserializeBinaryBulkState {
  ...
  ISerialization::DeserializeBinaryBulkStatePtr clone() const override { return std::make_shared<DeserializeStateLowCardinality>(*this); }
};

void SerializationLowCardinality::serializeBinaryBulkStatePrefix(const IColumn & /*column*/, SerializeBinaryBulkSettings &settings,
                                                                 SerializeBinaryBulkStatePtr &state) const {
  settings.path.push_back(settings.use_specialized_prefixes_and_suffixes_substreams ? Substream::DictionaryKeysPrefix
                                                                                    : Substream::DictionaryKeys);
...
void SerializationLowCardinality::serializeBinaryBulkStateSuffix(SerializeBinaryBulkSettings &settings,
                                                                 SerializeBinaryBulkStatePtr &state) const {
...
void SerializationLowCardinality::deserializeBinaryBulkStatePrefix(DeserializeBinaryBulkSettings &settings,
...
IndexMapsWithAdditionalKeys mapIndexWithAdditionalKeys(PaddedPODArray<T> &index, size_t dict_size) {
  ...
  if (size == 0) return {ColumnVector<T>::create(), ColumnVector<T>::create()};

  for (size_t i = 0; i < size; ++i) {
    auto val = index[i];
    if (val < dict_size) max_less_dict_size = std::max(max_less_dict_size, val);
  ...
  for (size_t i = 0; i < map_size; ++i)
    if (map[i]) dict_data[map[i]] = static_cast<T>(i);

  for (size_t i = 0; i < overflow_map_size; ++i)
    if (overflow_map[i]) add_keys_data[overflow_map[i]] = static_cast<T>(i);

  if (cur_pos) dict_data[0] = zero_pos_value;
  if (cur_overflowed_pos) add_keys_data[0] = zero_pos_overflowed_value;
...
IndexMapsWithAdditionalKeys mapIndexWithAdditionalKeys(IColumn &column, size_t dict_size) {
  if (auto *data_uint8 = getIndexesData<UInt8>(column)) return mapIndexWithAdditionalKeys(*data_uint8, dict_size);
  if (auto *data_uint16 = getIndexesData<UInt16>(column)) return mapIndexWithAdditionalKeys(*data_uint16, dict_size);
  if (auto *data_uint32 = getIndexesData<UInt32>(column)) return mapIndexWithAdditionalKeys(*data_uint32, dict_size);
  if (auto *data_uint64 = getIndexesData<UInt64>(column)) return mapIndexWithAdditionalKeys(*data_uint64, dict_size);
  throw Exception(ErrorCodes::LOGICAL_ERROR, "Indexes column for mapIndexWithAdditionalKeys must be UInt, got {}", column.getName());
}
}  // namespace

void SerializationLowCardinality::serializeBinaryBulkWithMultipleStreams(const IColumn &column, size_t offset, size_t limit,
  ...
  if (!keys_stream && !indexes_stream) return;

  if (!keys_stream) throw Exception(ErrorCodes::LOGICAL_ERROR, "Got empty stream for SerializationLowCardinality keys.");

  if (!indexes_stream) throw Exception(ErrorCodes::LOGICAL_ERROR, "Got empty stream for SerializationLowCardinality indexes.");
  ...
  if (need_update_dictionary) global_dictionary = DataTypeLowCardinality::createColumnUnique(*dictionary_type);
  ...
  if (settings.low_cardinality_max_dictionary_size) {
    ...
    auto indexes_with_overflow =
        global_dictionary->uniqueInsertRangeWithOverflow(*keys, 0, keys->size(), settings.low_cardinality_max_dictionary_size);

    if (global_dictionary->size() > settings.low_cardinality_max_dictionary_size)
      throw Exception(ErrorCodes::LOGICAL_ERROR, "Got dictionary with size {} but max dictionary size is {}", global_dictionary->size(),
    ...
    if (global_dictionary->size() < settings.low_cardinality_max_dictionary_size && !keys->empty())
      throw Exception(ErrorCodes::LOGICAL_ERROR,
                      "Has additional keys, but dict size is {} which is less "
                      "then max dictionary size ({})",
                      global_dictionary->size(), settings.low_cardinality_max_dictionary_size);
  }

  if (const auto *nullable_keys = checkAndGetColumn<ColumnNullable>(&*keys)) keys = nullable_keys->getNestedColumnPtr();

  bool need_additional_keys = !keys->empty();
  bool need_dictionary = settings.low_cardinality_max_dictionary_size != 0;
  bool need_write_dictionary =
      !settings.low_cardinality_use_single_dictionary_for_part && global_dictionary->size() >= settings.low_cardinality_max_dictionary_size;
...
void SerializationLowCardinality::deserializeBinaryBulkWithMultipleStreams(ColumnPtr &column, size_t rows_offset, size_t limit,
  ...
  auto read_additional_keys = [this, low_cardinality_state, indexes_stream]() {
    ...
    if (!low_cardinality_state->index_type.need_global_dictionary && dictionary_type->isNullable()) {
      ...
      if (num_keys) null_map->getElement(0) = 1;

      low_cardinality_state->null_map = std::move(null_map);
    }
  };

  auto read_indexes = [this, low_cardinality_state, indexes_stream, &low_cardinality_column](UInt64 skipped_rows, UInt64 num_rows) {
    auto indexes_type = low_cardinality_state->index_type.getDataType();
    MutableColumnPtr indexes_column = indexes_type->createColumn();
    indexes_type->getDefaultSerialization()->deserializeBinaryBulk(*indexes_column, *indexes_stream, skipped_rows, num_rows, 0);
    ...
    if (!low_cardinality_state->index_type.need_global_dictionary) {
      if (additional_keys == nullptr) throw Exception(ErrorCodes::INCORRECT_DATA, "No additional keys found.");

      ColumnPtr keys_column = additional_keys;
      if (low_cardinality_state->null_map) keys_column = ColumnNullable::create(additional_keys, low_cardinality_state->null_map);
      low_cardinality_column.insertRangeFromDictionaryEncodedColumn(*keys_column, *indexes_column);
    } else if (!has_additional_keys) {
      if (column_is_empty) low_cardinality_column.setSharedDictionary(global_dictionary);
  ...
  while (limit) {
    ...
    if (rows_offset) {
      skipped_rows = std::min<UInt64>(rows_offset, low_cardinality_state->num_pending_rows);
...
void SerializationLowCardinality::serializeBinary(const IColumn &column, size_t row_num, WriteBuffer &ostr,
...
void SerializationLowCardinality::serializeTextEscaped(const IColumn &column, size_t row_num, WriteBuffer &ostr,
                                                       const FormatSettings &settings) const {
...
void SerializationLowCardinality::serializeTextQuoted(const IColumn &column, size_t row_num, WriteBuffer &ostr,
                                                      const FormatSettings &settings) const {
...
void SerializationLowCardinality::serializeTextCSV(const IColumn &column, size_t row_num, WriteBuffer &ostr,
                                                   const FormatSettings &settings) const {
...
void SerializationLowCardinality::serializeText(const IColumn &column, size_t row_num, WriteBuffer &ostr,
                                                const FormatSettings &settings) const {
  serializeImpl(column, row_num, &ISerialization::serializeText, ostr, settings);
}

void SerializationLowCardinality::serializeTextJSON(const IColumn &column, size_t row_num, WriteBuffer &ostr,
                                                    const FormatSettings &settings) const {
...
void SerializationLowCardinality::serializeTextXML(const IColumn &column, size_t row_num, WriteBuffer &ostr,
                                                   const FormatSettings &settings) const {
...
void SerializationLowCardinality::serializeTextRaw(const IColumn &column, size_t row_num, WriteBuffer &ostr,
                                                   const FormatSettings &settings) const {
  serializeImpl(column, row_num, &ISerialization::serializeTextRaw, ostr, settings);
}

template <typename... Params, typename... Args>
void SerializationLowCardinality::serializeImpl(const IColumn &column, size_t row_num,
                                                SerializationLowCardinality::SerializeFunctionPtr<Params...> func, Args &&...args) const {
...
template <typename... Params, typename... Args>
void SerializationLowCardinality::deserializeImpl(IColumn &column, SerializationLowCardinality::DeserializeFunctionPtr<Params...> func,
                                                  Args &&...args) const {
...
template <typename... Params, typename... Args>
bool SerializationLowCardinality::tryDeserializeImpl(IColumn &column,
                                                     SerializationLowCardinality::TryDeserializeFunctionPtr<Params...> func,
                                                     Args &&...args) const {
  auto &low_cardinality_column = getColumnLowCardinality(column);
  auto temp_column = low_cardinality_column.getDictionary().getNestedColumn()->cloneEmpty();

  auto serialization = dictionary_type->getDefaultSerialization();
  if (!(serialization.get()->*func)(*temp_column, std::forward<Args>(args)...)) return false;
