...
static const IColumn &extractNestedColumn(const IColumn &column) { return assert_cast<const ColumnMap &>(column).getNestedColumn(); }

static IColumn &extractNestedColumn(IColumn &column) { return assert_cast<ColumnMap &>(column).getNestedColumn(); }
...
void SerializationMap::readMapSafe(DB::IColumn &column, std::function<void()> &&read_func) {
  ...
  try {
    ...
    if (keys_column.size() > offsets.back()) keys_column.popBack(keys_column.size() - offsets.back());

    if (values_column.size() > offsets.back()) values_column.popBack(values_column.size() - offsets.back());
...
void SerializationMap::serializeTextImpl(const IColumn &column, size_t row_num, WriteBuffer &ostr, KeyWriter &&key_writer,
    ...
    while (!istr.eof() && *istr.position() != '}') {
      if (!first) {
        if (*istr.position() == ',') {
          ...
          if constexpr (throw_exception) throw Exception(ErrorCodes::CANNOT_READ_MAP_FROM_TEXT, "Cannot read Map from text");
...
void SerializationMap::deserializeText(IColumn &column, ReadBuffer &istr, const FormatSettings &settings, bool whole) const {
  deserializeTextImpl(column, istr, [&settings](ReadBuffer &buf, const SerializationPtr &subcolumn_serialization, IColumn &subcolumn) {
  ...
  if (whole && !istr.eof()) throwUnexpectedDataAfterParsedValue(column, istr, settings, "Map");
}
...
void SerializationMap::serializeTextJSONPretty(const IColumn &column, size_t row_num, WriteBuffer &ostr, const FormatSettings &settings,
                                               size_t indent) const {
  if (settings.json.write_map_as_array_of_tuples) {
...
ReturnType SerializationMap::deserializeTextJSONImpl(IColumn &column, ReadBuffer &istr, const FormatSettings &settings) const {
  auto deserialize_nested = [&settings](IColumn &subcolumn, ReadBuffer &buf,
                                        const SerializationPtr &subcolumn_serialization) -> ReturnType {
    ...
    return deserializeTextImpl<ReturnType>(
        ...
        [&deserialize_nested](ReadBuffer &buf, const SerializationPtr &subcolumn_serialization, IColumn &subcolumn) -> ReturnType {
          return JSONUtils::deserializeEmpyStringAsDefaultOrNested<ReturnType>(
              subcolumn, buf, [&deserialize_nested, &subcolumn_serialization](IColumn &subcolumn_, ReadBuffer &buf_) -> ReturnType {
...
void SerializationMap::enumerateStreams(EnumerateStreamsSettings &settings, const StreamCallback &callback,
...
void SerializationMap::serializeBinaryBulkStatePrefix(const IColumn &column, SerializeBinaryBulkSettings &settings,
                                                      SerializeBinaryBulkStatePtr &state) const {
  nested->serializeBinaryBulkStatePrefix(extractNestedColumn(column), settings, state);
}

void SerializationMap::serializeBinaryBulkStateSuffix(SerializeBinaryBulkSettings &settings, SerializeBinaryBulkStatePtr &state) const {
  nested->serializeBinaryBulkStateSuffix(settings, state);
}

void SerializationMap::deserializeBinaryBulkStatePrefix(DeserializeBinaryBulkSettings &settings, DeserializeBinaryBulkStatePtr &state,
                                                        SubstreamsDeserializeStatesCache *cache) const {
  nested->deserializeBinaryBulkStatePrefix(settings, state, cache);
}

void SerializationMap::serializeBinaryBulkWithMultipleStreams(const IColumn &column, size_t offset, size_t limit,
                                                              SerializeBinaryBulkSettings &settings,
                                                              SerializeBinaryBulkStatePtr &state) const {
  nested->serializeBinaryBulkWithMultipleStreams(extractNestedColumn(column), offset, limit, settings, state);
}

void SerializationMap::deserializeBinaryBulkWithMultipleStreams(ColumnPtr &column, size_t rows_offset, size_t limit,
