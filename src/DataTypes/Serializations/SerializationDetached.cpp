...
void SerializationDetached::serializeBinaryBulk(const IColumn &column, WriteBuffer &ostr, [[maybe_unused]] size_t offset,
                                                [[maybe_unused]] size_t limit) const {
  ...
  if (auto *compressed_buf = typeid_cast<DB::CompressedWriteBuffer *>(&ostr)) compressed_buf->setCodec(original_codec);
}

void SerializationDetached::deserializeBinaryBulk(IColumn &column, ReadBuffer &istr, [[maybe_unused]] size_t rows_offset,
                                                  [[maybe_unused]] size_t limit, [[maybe_unused]] double avg_value_size_hint) const {
...
void SerializationDetached::deserializeBinaryBulkWithMultipleStreams(ColumnPtr &column, size_t rows_offset, size_t limit,
                                                                     DeserializeBinaryBulkSettings &settings,
                                                                     DeserializeBinaryBulkStatePtr &state, SubstreamsCache *cache) const {
  auto task = [wrapped_column = column, nested_serialization = nested, limit,
