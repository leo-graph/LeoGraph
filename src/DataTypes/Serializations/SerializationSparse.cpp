...
size_t deserializeOffsets(IColumn::Offsets &offsets, ReadBuffer &istr, size_t start, size_t offset, size_t limit,
                          size_t &skipped_values_rows, DeserializeStateSparse &state) {
  ...
  while (!istr.eof()) {
    ...
    if (end_of_granule) {
      ...
      if (!first && !offsets.empty()) start_of_group = offsets.back() + 1;
...
size_t readOrGetCachedSparseOffsets(ColumnPtr &offsets_column, ISerialization::DeserializeBinaryBulkSettings &settings,
                                    ISerialization::SubstreamsCache *cache, DeserializeStateSparse &state_sparse, size_t prev_size,
                                    size_t rows_offset, size_t limit, size_t &read_rows, size_t &skipped_values_rows) {
  ...
  if (cached_element) {
    ...
    if (!settings.continuous_reading) state_sparse.reset();
...
SerializationSparse::SerializationSparse(const SerializationPtr &nested_) : nested(nested_) {
...
ColumnPtr SerializationSparse::SubcolumnCreator::create(const ColumnPtr &prev) const { return ColumnSparse::create(prev, offsets, size); }
...
ColumnPtr SerializationSparse::NullMapSubcolumnCreator::create(const ColumnPtr & /* prev */) const {
  ...
  for (UInt64 offset : offsets_data) null_map_data[offset] = 0;
  return null_map_col;
}

void SerializationSparse::enumerateStreams(EnumerateStreamsSettings &settings, const StreamCallback &callback,
...
void SerializationSparse::serializeBinaryBulkStatePrefix(const IColumn &column, SerializeBinaryBulkSettings &settings,
...
void SerializationSparse::serializeBinaryBulkWithMultipleStreams(const IColumn &column, size_t offset, size_t limit,
...
void SerializationSparse::deserializeBinaryBulkStatePrefix(DeserializeBinaryBulkSettings &settings, DeserializeBinaryBulkStatePtr &state,
...
void SerializationSparse::deserializeBinaryBulkWithMultipleStreams(ColumnPtr &column, size_t rows_offset, size_t limit,
  ...
  size_t num_read_offsets = readOrGetCachedSparseOffsets(offsets_column, settings, cache, *state_sparse, prev_size, rows_offset, limit,
                                                         read_rows, skipped_values_rows);
  ...
  nested->deserializeBinaryBulkWithMultipleStreams(values_column, skipped_values_rows, num_read_offsets, values_settings,
                                                   state_sparse->nested, cache);
  settings.path.pop_back();
  ...
  if (offsets_column->size() + 1 != values_column->size()) {
    throw Exception(ErrorCodes::LOGICAL_ERROR,
                    ...
                    offsets_column->size(), values_column->size());
  }
...
void SerializationSparse::serializeTextEscaped(const IColumn &column, size_t row_num, WriteBuffer &ostr,
...
void SerializationSparse::serializeTextQuoted(const IColumn &column, size_t row_num, WriteBuffer &ostr,
                                              const FormatSettings &settings) const {
  const auto &column_sparse = assert_cast<const ColumnSparse &>(column);
...
void SerializationSparse::serializeTextJSON(const IColumn &column, size_t row_num, WriteBuffer &ostr,
                                            const FormatSettings &settings) const {
  const auto &column_sparse = assert_cast<const ColumnSparse &>(column);
...
void SerializationSparseNullMap::serializeBinaryBulkStatePrefix(const IColumn &column, SerializeBinaryBulkSettings &settings,
                                                                SerializeBinaryBulkStatePtr &state) const {
  assertSettings(settings);
  Base::serializeBinaryBulkStatePrefix(column, settings, state);
}

void SerializationSparseNullMap::serializeBinaryBulkStateSuffix(SerializeBinaryBulkSettings &settings,
                                                                SerializeBinaryBulkStatePtr &state) const {
  assertSettings(settings);
  Base::serializeBinaryBulkStateSuffix(settings, state);
}

void SerializationSparseNullMap::serializeBinaryBulkWithMultipleStreams(const IColumn &column, size_t offset, size_t limit,
                                                                        SerializeBinaryBulkSettings &settings,
                                                                        SerializeBinaryBulkStatePtr &state) const {
  assertSettings(settings);
...
void SerializationSparseNullMap::enumerateStreams(EnumerateStreamsSettings &settings, const StreamCallback &callback,
                                                  const SubstreamData &data) const {
...
void SerializationSparseNullMap::deserializeBinaryBulkStatePrefix(DeserializeBinaryBulkSettings &settings,
...
void SerializationSparseNullMap::deserializeBinaryBulkWithMultipleStreams(ColumnPtr &column, size_t rows_offset, size_t limit,
                                                                          DeserializeBinaryBulkSettings &settings,
                                                                          DeserializeBinaryBulkStatePtr &state,
  ...
  if (!state_sparse->column_offsets || column->empty()) state_sparse->column_offsets = ColumnUInt64::create();

  size_t prev_size = column->size();
  size_t read_rows = 0;
  size_t skipped_values_rows = 0;
  size_t num_read_offsets = readOrGetCachedSparseOffsets(state_sparse->column_offsets, settings, cache, *state_sparse, prev_size,
                                                         rows_offset, limit, read_rows, skipped_values_rows);
  ...
  if (read_rows) {
    ...
    for (size_t i = total_offsets - num_read_offsets; i < total_offsets; ++i) null_map_data[offsets_data[i]] = 0;
