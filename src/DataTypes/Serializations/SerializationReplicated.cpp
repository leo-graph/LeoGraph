...
SerializationReplicated::SerializationReplicated(const SerializationPtr &nested_) : nested(nested_) {}
...
ColumnPtr SerializationReplicated::SubcolumnCreator::create(const ColumnPtr &prev) const { return ColumnReplicated::create(prev, indexes); }

void SerializationReplicated::enumerateStreams(EnumerateStreamsSettings &settings, const StreamCallback &callback,
  ...
  if (column_replicated) settings.path.back().creator = std::make_shared<SubcolumnCreator>(column_replicated->getIndexesColumn());
...
void SerializationReplicated::serializeBinaryBulkStatePrefix(const IColumn &column, SerializeBinaryBulkSettings &settings,
...
void SerializationReplicated::serializeBinaryBulkWithMultipleStreams(const IColumn &column, size_t offset, size_t limit,
  ...
  if (!elements_stream) throw Exception(ErrorCodes::LOGICAL_ERROR, "Got empty stream for SerializationReplicated elements.");
...
void SerializationReplicated::serializeBinaryBulkStateSuffix(SerializeBinaryBulkSettings &settings,
                                                             SerializeBinaryBulkStatePtr &state) const {
  settings.path.push_back(Substream::ReplicatedElements);
...
void SerializationReplicated::deserializeBinaryBulkStatePrefix(DeserializeBinaryBulkSettings &settings,
...
void SerializationReplicated::deserializeBinaryBulkWithMultipleStreams(ColumnPtr &column, size_t rows_offset, size_t limit,
  ...
  if (num_rows != limit)
    throw Exception(ErrorCodes::LOGICAL_ERROR,
                    "Unexpected number of rows in indexes column in ColumnReplicated in Native format: {}. Expected {}", num_rows, limit);
...
void SerializationReplicated::serializeBinary(const IColumn &column, size_t row_num, WriteBuffer &ostr,
...
void SerializationReplicated::serializeTextEscaped(const IColumn &column, size_t row_num, WriteBuffer &ostr,
                                                   const FormatSettings &settings) const {
  const auto &column_replicated = assert_cast<const ColumnReplicated &>(column);
...
void SerializationReplicated::serializeTextQuoted(const IColumn &column, size_t row_num, WriteBuffer &ostr,
                                                  const FormatSettings &settings) const {
  const auto &column_replicated = assert_cast<const ColumnReplicated &>(column);
...
void SerializationReplicated::serializeTextCSV(const IColumn &column, size_t row_num, WriteBuffer &ostr,
                                               const FormatSettings &settings) const {
  const auto &column_replicated = assert_cast<const ColumnReplicated &>(column);
...
void SerializationReplicated::serializeText(const IColumn &column, size_t row_num, WriteBuffer &ostr,
                                            const FormatSettings &settings) const {
  const auto &column_replicated = assert_cast<const ColumnReplicated &>(column);
...
void SerializationReplicated::serializeTextJSON(const IColumn &column, size_t row_num, WriteBuffer &ostr,
                                                const FormatSettings &settings) const {
  const auto &column_replicated = assert_cast<const ColumnReplicated &>(column);
...
void SerializationReplicated::serializeTextXML(const IColumn &column, size_t row_num, WriteBuffer &ostr,
                                               const FormatSettings &settings) const {
  const auto &column_replicated = assert_cast<const ColumnReplicated &>(column);
