...
struct DeserializeBinaryBulkStateVariant : public ISerialization::DeserializeBinaryBulkState {
  ...
  ISerialization::DeserializeBinaryBulkStatePtr clone() const override {
    ...
    for (const auto &variant_state : variant_states) new_state->variant_states.push_back(variant_state ? variant_state->clone() : nullptr);
    return new_state;
  }
};

SerializationVariant::SerializationVariant(const DataTypes &variant_types_, const VariantSerializations &variant_serializations_,
...
void SerializationVariant::enumerateStreams(EnumerateStreamsSettings &settings, const StreamCallback &callback,
  ...
  const auto *variant_deserialize_state =
      data.deserialize_state ? checkAndGetState<DeserializeBinaryBulkStateVariant>(data.deserialize_state) : nullptr;

  auto discriminators_serialization = std::make_shared<SerializationNamed>(
      std::make_shared<SerializationNumber<ColumnVariant::Discriminator>>(), "discr", SubstreamType::NamedVariantDiscriminators);
  ...
  for (ColumnVariant::Discriminator i = 0; i < static_cast<ColumnVariant::Discriminator>(variant_serializations.size()); ++i) {
    ...
    settings.path.back().creator = std::make_shared<SerializationVariantElement::VariantSubcolumnCreator>(
        local_discriminators, variant_names[i], i, column_variant ? column_variant->localDiscriminatorByGlobal(i) : i,
  ...
  /// Variant subcolumns like variant.Type have type Nullable(Type), so we want to support reading null map subcolumn from it:
  /// variant.Type.null. Nullable column is created during deserialization of a variant subcolumn according to the discriminators, so we
  /// don't have actual Nullable serialization with null map subcolumn. To be able to read null map subcolumn from the variant subcolumn we
  /// use special serialization SerializationVariantElementNullMap.
  ...
  chassert(variant_serializations.size() <= std::numeric_limits<ColumnVariant::Discriminator>::max());
  for (ColumnVariant::Discriminator i = 0; i < static_cast<ColumnVariant::Discriminator>(variant_serializations.size()); ++i) {
    if (!canExtractedSubcolumnsBeInsideNullable(variant_types[i])) continue;

    settings.path.back().creator = std::make_shared<SerializationVariantElementNullMap::VariantNullMapSubcolumnCreator>(
        local_discriminators, variant_names[i], i, column_variant ? column_variant->localDiscriminatorByGlobal(i) : i);
...
void SerializationVariant::serializeBinaryBulkStatePrefix(const IColumn &column, SerializeBinaryBulkSettings &settings,
                                                          SerializeBinaryBulkStatePtr &state) const {
  settings.path.push_back(settings.use_specialized_prefixes_and_suffixes_substreams ? Substream::VariantDiscriminatorsPrefix
                                                                                    : Substream::VariantDiscriminators);
  auto *discriminators_stream = settings.getter(settings.path);
  settings.path.pop_back();

  if (!discriminators_stream)
    throw Exception(ErrorCodes::LOGICAL_ERROR,
                    "Got empty stream for VariantDiscriminators in SerializationVariant::serializeBinaryBulkStatePrefix");

  UInt64 mode = settings.use_compact_variant_discriminators_serialization ? DiscriminatorsSerializationMode::COMPACT
                                                                          : DiscriminatorsSerializationMode::BASIC;
  ...
  for (size_t i = 0; i < variant_serializations.size(); ++i) {
    addVariantElementToPath(settings.path, i);
    variant_serializations[i]->serializeBinaryBulkStatePrefix(col.getVariantByGlobalDiscriminator(i), settings,
...
void SerializationVariant::serializeBinaryBulkStateSuffix(SerializeBinaryBulkSettings &settings, SerializeBinaryBulkStatePtr &state) const {
...
void SerializationVariant::deserializeBinaryBulkStatePrefix(DeserializeBinaryBulkSettings &settings, DeserializeBinaryBulkStatePtr &state,
                                                            SubstreamsDeserializeStatesCache *cache) const {
  DeserializeBinaryBulkStatePtr discriminators_state = deserializeDiscriminatorsStatePrefix(settings, cache);
  if (!discriminators_state) return;
...
ISerialization::DeserializeBinaryBulkStatePtr SerializationVariant::deserializeDiscriminatorsStatePrefix(
  ...
  if (auto cached_state = getFromSubstreamsDeserializeStatesCache(cache, settings.path)) {
    ...
    if (settings.release_all_prefixes_streams && settings.release_stream_callback) settings.release_stream_callback(settings.path);
  }
...
void SerializationVariant::serializeBinaryBulkWithMultipleStreamsAndUpdateVariantStatistics(
    const IColumn &column, size_t offset, size_t limit, SerializeBinaryBulkSettings &settings, SerializeBinaryBulkStatePtr &state,
    std::unordered_map<String, size_t> &variants_statistics, size_t &total_size_of_variants) const {
  const ColumnVariant &col = assert_cast<const ColumnVariant &>(column);
  if (offset == 0) col.validateState();

  if (const size_t size = col.size(); limit == 0 || offset + limit > size) limit = size - offset;
  ...
  if (!discriminators_stream)
    throw Exception(ErrorCodes::LOGICAL_ERROR,
                    "Got empty stream for VariantDiscriminators in SerializationVariant::serializeBinaryBulkWithMultipleStreams");
  ...
  if (limit == 0) {
    ...
    for (size_t i = 0; i != variant_serializations.size(); ++i) {
      addVariantElementToPath(settings.path, i);
      variant_serializations[i]->serializeBinaryBulkWithMultipleStreams(col.getVariantByGlobalDiscriminator(i),
                                                                        col.getVariantByGlobalDiscriminator(i).size(), 0, settings,
                                                                        variant_state->variant_states[i]);
      settings.path.pop_back();
    }
    settings.path.pop_back();
  ...
  if (auto non_empty_local_discr = col.getLocalDiscriminatorOfOneNoneEmptyVariantNoNulls()) {
    ...
    /// For basic mode just serialize this discriminator limit times.
      ...
      for (size_t i = 0; i < limit; ++i) writeBinaryLittleEndian(non_empty_global_discr, *discriminators_stream);
    }
    ...
    for (size_t i = 0; i != variant_serializations.size(); ++i) {
      ...
      if (i == non_empty_global_discr) {
        ...
        if (variant_column.size() < offset + limit)
          throw Exception(ErrorCodes::LOGICAL_ERROR, "Variant {} has less rows ({}) than expected rows to serialize ({})", variant_names[i],
                          variant_column.size(), offset + limit);

        variant_serializations[i]->serializeBinaryBulkWithMultipleStreams(variant_column, offset, limit, settings,
                                                                          variant_state->variant_states[i]);
      } else {
        variant_serializations[i]->serializeBinaryBulkWithMultipleStreams(col.getVariantByGlobalDiscriminator(i),
                                                                          col.getVariantByGlobalDiscriminator(i).size(), 0, settings,
  ...
  if (col.hasOnlyNulls()) {
    ...
    /// In basic mode write NULL_DISCRIMINATOR limit times.
      ...
      for (size_t i = 0; i < limit; ++i) writeBinaryLittleEndian(ColumnVariant::NULL_DISCRIMINATOR, *discriminators_stream);
    }
  ...
  if ((offset == 0 && limit == col.size())) {
    ...
    if (col.hasGlobalVariantsOrder()) {
      SerializationNumber<ColumnVariant::Discriminator>().serializeBinaryBulk(col.getLocalDiscriminatorsColumn(), *discriminators_stream,
                                                                              offset, limit);
    }
    /// If local and global discriminators are different, we should convert local to global before serializing (because we don't serialize
    /// the mapping).
    else {
    ...
    for (size_t i = 0; i != variant_serializations.size(); ++i) {
      addVariantElementToPath(settings.path, i);
      variant_serializations[i]->serializeBinaryBulkWithMultipleStreams(col.getVariantByGlobalDiscriminator(i), 0, 0, settings,
                                                                        variant_state->variant_states[i]);
  ...
  /// In general case we should iterate through local discriminators in range [offset, offset + limit] to serialize global discriminators
  /// and calculate offset/limit pair for each variant.
  const auto &local_discriminators = col.getLocalDiscriminators();
  ...
  for (size_t i = offset; i < end; ++i) {
    ...
    if (global_discr != ColumnVariant::NULL_DISCRIMINATOR) {
      ...
      if (!variant_offsets_and_limits[global_discr].second) variant_offsets_and_limits[global_discr].first = offsets[i];
  ...
  for (size_t i = 0; i != variant_serializations.size(); ++i) {
    ...
    if (variant_column.size() < variant_offsets_and_limits[i].first + variant_offsets_and_limits[i].second)
      throw Exception(ErrorCodes::LOGICAL_ERROR, "Variant {} has less rows ({}) than expected rows to serialize ({})", variant_names[i],
                      variant_column.size(), variant_offsets_and_limits[i].first + variant_offsets_and_limits[i].second);
    ...
    variant_serializations[i]->serializeBinaryBulkWithMultipleStreams(
        ...
        variant_offsets_and_limits[i].second, settings, variant_state->variant_states[i]);
...
void SerializationVariant::serializeBinaryBulkWithMultipleStreams(const DB::IColumn &column, size_t offset, size_t limit,
...
void SerializationVariant::deserializeBinaryBulkWithMultipleStreams(ColumnPtr &column, size_t rows_offset, size_t limit,
  ...
  if (col.getNumVariants() != variant_serializations.size())
    throw Exception(ErrorCodes::LOGICAL_ERROR, "Number of variants in column doesn't match number of variant serializations: {} != {}",
                    col.getNumVariants(), variant_serializations.size());
  ...
  if (auto cached_column_with_num_read_rows = getColumnWithNumReadRowsFromSubstreamsCache(cache, settings.path)) {
    ...
    if (rows_offset)
      col.getLocalDiscriminatorsPtr()->assumeMutable()->insertRangeFrom(*cached_column, cached_column->size() - num_read_rows,
                                                                        num_read_rows);
    else
    ...
    if (discriminators_state->mode.value == DiscriminatorsSerializationMode::BASIC) {
      SerializationNumber<ColumnVariant::Discriminator>().deserializeBinaryBulk(*col.getLocalDiscriminatorsPtr()->assumeMutable(),
                                                                                *discriminators_stream, 0, rows_offset + limit, 0);
    } else {
      auto variant_pair = deserializeCompactDiscriminators(col.getLocalDiscriminatorsPtr(), rows_offset, limit, discriminators_stream,
                                                           settings.continuous_reading, *discriminators_state);

      variant_rows_offsets = variant_pair.first;
      variant_limits = variant_pair.second;
    }

    num_read_discriminators = col.getLocalDiscriminatorsPtr()->size() - prev_size;

    if (cache) {
      ColumnPtr discriminators_for_cache;
      /// If rows_offset != 0 we should keep discriminators without applied offsets in the cache to be able
      /// to calculate variants offsets later. As we will apply offsets to the current discriminators column
      /// we cannot put in the cache, so we use cut() method to create a separate column with all the data from
      /// current deserialization.
      if (rows_offset)
        discriminators_for_cache = col.getLocalDiscriminatorsPtr()->cut(prev_size, num_read_discriminators);
      else
        discriminators_for_cache = col.getLocalDiscriminatorsPtr();

      addColumnWithNumReadRowsToSubstreamsCache(cache, settings.path, discriminators_for_cache, num_read_discriminators);
    }
  }
  ...
  if (variant_rows_offsets.empty()) {
    ...
    if (rows_offset) {
      ...
      for (size_t i = discriminators_offset; i != discriminators_offset + rows_offset; ++i) {
        ...
        if (discr != ColumnVariant::NULL_DISCRIMINATOR) ++variant_rows_offsets[discr];
      }
    }
  }
  ...
  if (variant_limits.empty()) {
    ...
    for (size_t i = discriminators_offset; i != discriminators_data.size(); ++i) {
      ColumnVariant::Discriminator discr = discriminators_data[i];
      if (discr != ColumnVariant::NULL_DISCRIMINATOR) ++variant_limits[discr];
    }
  }
  ...
  for (size_t i = 0; i != variant_serializations.size(); ++i) {
    ...
    if (!variant_column) throw Exception(ErrorCodes::LOGICAL_ERROR, "Column for variant {} is invalid", variant_names[i]);

    addVariantElementToPath(settings.path, i);
    variant_serializations[i]->deserializeBinaryBulkWithMultipleStreams(variant_column, variant_rows_offsets[i], variant_limits[i],
    ...
    if (variant_limits[i] && col.getVariantPtrByLocalDiscriminator(i)->empty())
      throw Exception(settings.native_format ? ErrorCodes::INCORRECT_DATA : ErrorCodes::LOGICAL_ERROR,
                      "Variant {} is empty, but expected to be read {} values", variant_names[i], variant_limits[i]);
  }
  settings.path.pop_back();
  ...
  /// If insert_only_rows_in_current_range_from_substreams_cache is set we cannot use offsets from
  /// cache and need to calculate correct offsets for current read range.
  if (settings.insert_only_rows_in_current_range_from_substreams_cache ||
      !insertDataFromSubstreamsCacheIfAny(cache, settings, col.getOffsetsPtr())) {
    ...
    for (ColumnVariant::Discriminator i = 0; i != variant_serializations.size(); ++i) {
      ...
      if (col.getVariantByLocalDiscriminator(i).size() < variant_limits[i])
        throw Exception(ErrorCodes::LOGICAL_ERROR,
                        "Size of variant {} is expected to be not less than {} according to discriminators, but it is {}", variant_names[i],
                        variant_limits[i], col.getVariantByLocalDiscriminator(i).size());
    ...
    /// If there is only 1 variant and no NULLs was read, fill offsets with sequential offsets of this variant.
      ...
      for (size_t i = 0; i != num_new_offsets; ++i) offsets.push_back(first_offset + i);
    }
...
std::pair<std::vector<size_t>, std::vector<size_t>> SerializationVariant::deserializeCompactDiscriminators(
    DB::ColumnPtr &discriminators_column, size_t rows_offset, size_t limit, ReadBuffer *stream, bool continuous_reading,
  ...
  if (!continuous_reading) state.remaining_rows_in_granule = 0;
  ...
  while (limit) {
    ...
    if (state.remaining_rows_in_granule == 0) {
      if (stream->eof()) return {variant_rows_offsets, variant_limits};
    ...
    if (state.granule_format == CompactDiscriminatorsGranuleFormat::COMPACT) {
      ...
      if (rows_offset) {
        ...
        if (state.compact_discr != ColumnVariant::NULL_DISCRIMINATOR) variant_rows_offsets[state.compact_discr] += skipped_rows;
...
void SerializationVariant::readDiscriminatorsGranuleStart(DeserializeBinaryBulkStateVariantDiscriminators &state, DB::ReadBuffer *stream) {
  ...
  if (granule_format == CompactDiscriminatorsGranuleFormat::COMPACT) readBinaryLittleEndian(state.compact_discr, *stream);
}
...
void SerializationVariant::serializeBinary(const IColumn &column, size_t row_num, WriteBuffer &ostr, const FormatSettings &settings) const {
  ...
  if (global_discr != ColumnVariant::NULL_DISCRIMINATOR)
    variant_serializations[global_discr]->serializeBinary(col.getVariantByGlobalDiscriminator(global_discr), col.offsetAt(row_num), ostr,
                                                          settings);
}
...
void SerializationVariant::serializeForHashCalculation(const IColumn &column, size_t row_num, WriteBuffer &ostr) const {
  ...
  writeBinaryLittleEndian(global_discr, ostr);
  if (global_discr != ColumnVariant::NULL_DISCRIMINATOR)
    variant_serializations[global_discr]->serializeForHashCalculation(col.getVariantByGlobalDiscriminator(global_discr),
                                                                      col.offsetAt(row_num), ostr);
}
...
const std::unordered_map<TypeIndex, size_t> &getTypesTextDeserializePriorityMap() {
  static std::unordered_map<TypeIndex, size_t> priority_map = [] {
    ...
    for (size_t i = 0; i != priorities.size(); ++i) pm[priorities[i]] = priorities.size() - i;
    return pm;
  }();
...
std::tuple<size_t, size_t, size_t> getTypeTextDeserializePriority(const DataTypePtr &type, size_t nested_depth, size_t simple_nested_depth,
                                                                  const std::unordered_map<TypeIndex, size_t> &priority_map) {
  ...
  if (const auto *array_type = typeid_cast<const DataTypeArray *>(type.get())) {
    auto [elements_nested_depth, elements_priority, elements_simple_nested_depth] =
        getTypeTextDeserializePriority(array_type->getNestedType(), nested_depth + 1, simple_nested_depth, priority_map);
  ...
  if (const auto *tuple_type = typeid_cast<const DataTypeTuple *>(type.get())) {
    ...
    for (const auto &elem : tuple_type->getElements()) {
      auto [elem_nested_depth, elem_priority, elem_simple_nested_depth] =
          getTypeTextDeserializePriority(elem, nested_depth + 1, simple_nested_depth, priority_map);
  ...
  if (const auto *map_type = typeid_cast<const DataTypeMap *>(type.get())) {
    auto [key_max_depth, key_priority, key_simple_nested_depth] =
        getTypeTextDeserializePriority(map_type->getKeyType(), nested_depth + 1, simple_nested_depth, priority_map);
    auto [value_max_depth, value_priority, value_simple_nested_depth] =
        getTypeTextDeserializePriority(map_type->getValueType(), nested_depth + 1, simple_nested_depth, priority_map);
    return {std::max(key_max_depth, value_max_depth), key_priority + value_priority + priority_map.at(TypeIndex::Map),
            std::max(key_simple_nested_depth, value_simple_nested_depth)};
  }
  ...
  if (const auto *variant_type = typeid_cast<const DataTypeVariant *>(type.get())) {
    ...
    for (const auto &variant : variant_type->getVariants()) {
      auto [variant_max_depth, variant_priority, variant_simple_nested_depth] =
          getTypeTextDeserializePriority(variant, nested_depth, simple_nested_depth, priority_map);
  ...
  if (isBool(type)) return {nested_depth, priority_map.at(TypeIndex::Int8) + 1, simple_nested_depth};
...
bool SerializationVariant::tryDeserializeImpl(
    IColumn &column, const String &field, std::function<bool(ReadBuffer &)> check_for_null,
    std::function<bool(IColumn &variant_column, const SerializationPtr &variant_serialization, ReadBuffer &, const FormatSettings &)>
  ...
  for (size_t global_discr : deserialize_text_order) {
    ...
    if (try_deserialize_nested(variant_column, variant_serializations[global_discr], variant_buf, modified_settings) && variant_buf.eof()) {
      column_variant.getLocalDiscriminators().push_back(
          column_variant.localDiscriminatorByGlobal(static_cast<ColumnVariant::Discriminator>(global_discr)));
...
void SerializationVariant::serializeTextEscaped(const IColumn &column, size_t row_num, WriteBuffer &ostr,
                                                const FormatSettings &settings) const {
  const ColumnVariant &col = assert_cast<const ColumnVariant &>(column);
  auto global_discr = col.globalDiscriminatorAt(row_num);
  if (global_discr == ColumnVariant::NULL_DISCRIMINATOR)
    ...
    variant_serializations[global_discr]->serializeTextEscaped(col.getVariantByGlobalDiscriminator(global_discr), col.offsetAt(row_num),
                                                               ostr, settings);
}
...
bool SerializationVariant::tryDeserializeTextEscapedImpl(DB::IColumn &column, const String &field,
                                                         const DB::FormatSettings &settings) const {
  auto check_for_null = [&](ReadBuffer &buf) { return SerializationNullable::tryDeserializeNullEscaped(buf, settings); };
...
void SerializationVariant::serializeTextRaw(const IColumn &column, size_t row_num, WriteBuffer &ostr,
                                            const FormatSettings &settings) const {
  const ColumnVariant &col = assert_cast<const ColumnVariant &>(column);
  auto global_discr = col.globalDiscriminatorAt(row_num);
  if (global_discr == ColumnVariant::NULL_DISCRIMINATOR)
    ...
    variant_serializations[global_discr]->serializeTextRaw(col.getVariantByGlobalDiscriminator(global_discr), col.offsetAt(row_num), ostr,
                                                           settings);
}
...
bool SerializationVariant::tryDeserializeTextRawImpl(DB::IColumn &column, const String &field, const DB::FormatSettings &settings) const {
  auto check_for_null = [&](ReadBuffer &buf) { return SerializationNullable::tryDeserializeNullRaw(buf, settings); };
  auto try_deserialize_variant = [](IColumn &variant_column, const SerializationPtr &variant_serialization, ReadBuffer &buf,
                                    const FormatSettings &settings_) {
...
void SerializationVariant::serializeTextQuoted(const IColumn &column, size_t row_num, WriteBuffer &ostr,
                                               const FormatSettings &settings) const {
  const ColumnVariant &col = assert_cast<const ColumnVariant &>(column);
  auto global_discr = col.globalDiscriminatorAt(row_num);
  if (global_discr == ColumnVariant::NULL_DISCRIMINATOR)
    ...
    variant_serializations[global_discr]->serializeTextQuoted(col.getVariantByGlobalDiscriminator(global_discr), col.offsetAt(row_num),
                                                              ostr, settings);
}

bool SerializationVariant::tryDeserializeTextQuoted(IColumn &column, ReadBuffer &istr, const FormatSettings &settings) const {
  String field;
  if (!tryReadQuotedField(field, istr)) return false;
...
bool SerializationVariant::tryDeserializeTextQuotedImpl(DB::IColumn &column, const String &field,
                                                        const DB::FormatSettings &settings) const {
  auto check_for_null = [&](ReadBuffer &buf) { return SerializationNullable::tryDeserializeNullQuoted(buf); };
  auto try_deserialize_variant = [](IColumn &variant_column, const SerializationPtr &variant_serialization, ReadBuffer &buf,
                                    const FormatSettings &settings_) {
...
void SerializationVariant::serializeTextCSV(const IColumn &column, size_t row_num, WriteBuffer &ostr,
                                            const FormatSettings &settings) const {
  const ColumnVariant &col = assert_cast<const ColumnVariant &>(column);
  auto global_discr = col.globalDiscriminatorAt(row_num);
  if (global_discr == ColumnVariant::NULL_DISCRIMINATOR)
    ...
    variant_serializations[global_discr]->serializeTextCSV(col.getVariantByGlobalDiscriminator(global_discr), col.offsetAt(row_num), ostr,
                                                           settings);
}
...
bool SerializationVariant::tryDeserializeTextCSVImpl(DB::IColumn &column, const String &field, const DB::FormatSettings &settings) const {
  auto check_for_null = [&](ReadBuffer &buf) { return SerializationNullable::tryDeserializeNullCSV(buf, settings); };
  auto try_deserialize_variant = [](IColumn &variant_column, const SerializationPtr &variant_serialization, ReadBuffer &buf,
                                    const FormatSettings &settings_) {
...
void SerializationVariant::serializeText(const IColumn &column, size_t row_num, WriteBuffer &ostr, const FormatSettings &settings) const {
  ...
  if (global_discr == ColumnVariant::NULL_DISCRIMINATOR)
    ...
    variant_serializations[global_discr]->serializeText(col.getVariantByGlobalDiscriminator(global_discr), col.offsetAt(row_num), ostr,
                                                        settings);
}
...
bool SerializationVariant::tryDeserializeWholeTextImpl(DB::IColumn &column, const String &field, const DB::FormatSettings &settings) const {
  auto check_for_null = [&](ReadBuffer &buf) { return SerializationNullable::tryDeserializeNullText(buf); };
  auto try_deserialize_variant = [](IColumn &variant_column, const SerializationPtr &variant_serialization, ReadBuffer &buf,
                                    const FormatSettings &settings_) {
...
void SerializationVariant::serializeTextJSON(const IColumn &column, size_t row_num, WriteBuffer &ostr,
                                             const FormatSettings &settings) const {
  const ColumnVariant &col = assert_cast<const ColumnVariant &>(column);
  auto global_discr = col.globalDiscriminatorAt(row_num);
  if (global_discr == ColumnVariant::NULL_DISCRIMINATOR)
    ...
    variant_serializations[global_discr]->serializeTextJSON(col.getVariantByGlobalDiscriminator(global_discr), col.offsetAt(row_num), ostr,
                                                            settings);
}

void SerializationVariant::serializeTextJSONPretty(const IColumn &column, size_t row_num, WriteBuffer &ostr, const FormatSettings &settings,
                                                   size_t indent) const {
  const ColumnVariant &col = assert_cast<const ColumnVariant &>(column);
  auto global_discr = col.globalDiscriminatorAt(row_num);
  if (global_discr == ColumnVariant::NULL_DISCRIMINATOR)
    ...
    variant_serializations[global_discr]->serializeTextJSONPretty(col.getVariantByGlobalDiscriminator(global_discr), col.offsetAt(row_num),
                                                                  ostr, settings, indent);
}

bool SerializationVariant::tryDeserializeTextJSON(IColumn &column, ReadBuffer &istr, const FormatSettings &settings) const {
  String field;
  if (!tryReadJSONField(field, istr, settings.json)) return false;
...
bool SerializationVariant::tryDeserializeTextJSONImpl(DB::IColumn &column, const String &field, const DB::FormatSettings &settings) const {
  auto check_for_null = [&](ReadBuffer &buf) { return SerializationNullable::tryDeserializeNullJSON(buf); };
  auto try_deserialize_variant = [](IColumn &variant_column, const SerializationPtr &variant_serialization, ReadBuffer &buf,
                                    const FormatSettings &settings_) {
...
void SerializationVariant::serializeTextXML(const IColumn &column, size_t row_num, WriteBuffer &ostr,
                                            const FormatSettings &settings) const {
  const ColumnVariant &col = assert_cast<const ColumnVariant &>(column);
  auto global_discr = col.globalDiscriminatorAt(row_num);
  if (global_discr == ColumnVariant::NULL_DISCRIMINATOR)
    ...
    variant_serializations[global_discr]->serializeTextXML(col.getVariantByGlobalDiscriminator(global_discr), col.offsetAt(row_num), ostr,
                                                           settings);
}
