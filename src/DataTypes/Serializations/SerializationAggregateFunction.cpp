...
void SerializationAggregateFunction::serializeBinary(const IColumn &column, size_t row_num, WriteBuffer &ostr,
...
void SerializationAggregateFunction::serializeBinaryBulk(const IColumn &column, WriteBuffer &ostr, size_t offset, size_t limit) const {
  ...
  size_t end = vec.size();
  if (limit) end = std::min(end, offset + limit);

  function->serializeBatch(vec, offset, end, ostr, version);
}

void SerializationAggregateFunction::deserializeBinaryBulk(IColumn &column, ReadBuffer &istr, size_t rows_offset, size_t limit,
...
static void deserializeFromString(const AggregateFunctionPtr &function, IColumn &column, const String &s, size_t version) {
  ...
  try {
    ...
    if (trailing_bytes != 0) {
      throw Exception(ErrorCodes::BAD_ARGUMENTS, "AggregateFunction state for `{}` has {} trailing byte(s) after deserialization",
                      function->getName(), trailing_bytes);
    }
  } catch (...) {
    function->destroy(place);
    throw;
...
static void deserializeFromValue(const AggregateFunctionPtr &function, IColumn &column, const String &value_str,
  ...
  try {
    ...
    if (argument_types.size() == 1) {
      ...
      for (const auto &col : assert_cast<ColumnTuple *>(tmp_column.get())->getColumns()) columns_ptrs.push_back(col.get());
...
static void deserializeFromArray(const AggregateFunctionPtr &function, IColumn &column, const String &array_str,
                                 const FormatSettings &settings) {
  ColumnAggregateFunction &column_concrete = assert_cast<ColumnAggregateFunction &>(column);
  ...
  try {
    ...
    if (argument_types.size() == 1)
      ...
      for (const auto &col : assert_cast<const ColumnTuple *>(tmp_column.get())->getColumns()) columns_ptrs.push_back(col.get());
...
void SerializationAggregateFunction::serializeTextEscaped(const IColumn &column, size_t row_num, WriteBuffer &ostr,
                                                          const FormatSettings &) const {
...
void SerializationAggregateFunction::deserializeBasedOnInput(IColumn &column, const FormatSettings &settings, const String &s) const {
  if (settings.aggregate_function_input_format == DB::FormatSettings::AggregateFunctionInputFormat::State) {
    ...
    throw Exception(ErrorCodes::BAD_ARGUMENTS, "Invalid value for aggregate_function_input_format: '{}'. Expected '{}', '{}', or '{}'",
                    settings.aggregate_function_input_format, DB::FormatSettings::AggregateFunctionInputFormat::State,
                    DB::FormatSettings::AggregateFunctionInputFormat::Value, DB::FormatSettings::AggregateFunctionInputFormat::Array);
  }
}
...
void SerializationAggregateFunction::serializeTextQuoted(const IColumn &column, size_t row_num, WriteBuffer &ostr,
                                                         const FormatSettings &) const {
...
void SerializationAggregateFunction::serializeTextJSON(const IColumn &column, size_t row_num, WriteBuffer &ostr,
...
void SerializationAggregateFunction::serializeTextXML(const IColumn &column, size_t row_num, WriteBuffer &ostr,
                                                      const FormatSettings &) const {
  writeXMLStringForTextElement(serializeToString(function, column, row_num, version), ostr);
}

void SerializationAggregateFunction::serializeTextCSV(const IColumn &column, size_t row_num, WriteBuffer &ostr,
                                                      const FormatSettings &) const {
