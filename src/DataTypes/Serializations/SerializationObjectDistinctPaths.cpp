...
SerializationObjectDistinctPaths::SerializationObjectDistinctPaths(const std::vector<String> &typed_paths_) : typed_paths(typed_paths_) {
...
struct DeserializeBinaryBulkStateObjectDistinctPaths : public ISerialization::DeserializeBinaryBulkState {
  ...
  ISerialization::DeserializeBinaryBulkStatePtr clone() const override {
    ...
    for (size_t bucket = 0; bucket != bucket_shared_data_paths_state.size(); ++bucket)
      new_state->bucket_shared_data_paths_state[bucket] =
          bucket_shared_data_paths_state[bucket] ? bucket_shared_data_paths_state[bucket]->clone() : nullptr;
    for (size_t bucket = 0; bucket != bucket_shared_data_structure_states.size(); ++bucket)
      new_state->bucket_shared_data_structure_states[bucket] =
          bucket_shared_data_structure_states[bucket] ? bucket_shared_data_structure_states[bucket]->clone() : nullptr;
    return new_state;
  }
};

void SerializationObjectDistinctPaths::enumerateStreams(EnumerateStreamsSettings &settings, const StreamCallback &callback,
                                                        const SubstreamData &data) const {
  settings.path.push_back(Substream::ObjectStructure);
  callback(settings.path);
  settings.path.pop_back();

  if (!data.deserialize_state) return;
  ...
  const auto *object_structure_state =
      checkAndGetState<SerializationObject::DeserializeBinaryBulkStateObjectStructure>(deserialize_state->object_structure_state);
...
void SerializationObjectDistinctPaths::serializeBinaryBulkStatePrefix(const IColumn &, ISerialization::SerializeBinaryBulkSettings &,
                                                                      ISerialization::SerializeBinaryBulkStatePtr &) const {
  throw Exception(ErrorCodes::NOT_IMPLEMENTED,
                  "Method serializeBinaryBulkStatePrefix is not implemented for SerializationObjectDistinctPaths");
}

void SerializationObjectDistinctPaths::serializeBinaryBulkWithMultipleStreams(const IColumn &, size_t, size_t,
...
void SerializationObjectDistinctPaths::serializeBinaryBulkStateSuffix(ISerialization::SerializeBinaryBulkSettings &,
                                                                      ISerialization::SerializeBinaryBulkStatePtr &) const {
  throw Exception(ErrorCodes::NOT_IMPLEMENTED,
                  "Method serializeBinaryBulkStateSuffix is not implemented for SerializationObjectDistinctPaths");
}

void SerializationObjectDistinctPaths::deserializeBinaryBulkStatePrefix(DeserializeBinaryBulkSettings &settings,
  ...
  if (!object_distinct_paths_state->object_structure_state) return;

  settings.path.push_back(Substream::ObjectData);
  settings.path.push_back(Substream::ObjectSharedData);

  const auto *object_structure_state =
      checkAndGetState<SerializationObject::DeserializeBinaryBulkStateObjectStructure>(object_distinct_paths_state->object_structure_state);
  switch (object_structure_state->shared_data_serialization_version.value) {
    case SerializationObjectSharedData::SerializationVersion::MAP: {
      shared_data_paths_serialization->deserializeBinaryBulkStatePrefix(settings, object_distinct_paths_state->shared_data_paths_state,
                                                                        cache);
      break;
    }
    case SerializationObjectSharedData::SerializationVersion::MAP_WITH_BUCKETS: {
      ...
      for (size_t bucket = 0; bucket != object_structure_state->shared_data_buckets; ++bucket) {
        settings.path.push_back(Substream::ObjectSharedDataBucket);
        settings.path.back().object_shared_data_bucket = bucket;
        shared_data_paths_serialization->deserializeBinaryBulkStatePrefix(
            settings, object_distinct_paths_state->bucket_shared_data_paths_state[bucket], cache);
        settings.path.pop_back();
      }
      break;
    }
    case SerializationObjectSharedData::SerializationVersion::ADVANCED: {
      ...
      for (size_t bucket = 0; bucket != object_structure_state->shared_data_buckets; ++bucket) {
        settings.path.push_back(Substream::ObjectSharedDataBucket);
        settings.path.back().object_shared_data_bucket = bucket;
        object_distinct_paths_state->bucket_shared_data_structure_states[bucket] =
            SerializationObjectSharedData::deserializeStructureStatePrefix(settings, cache);
        auto *shared_data_structure_state_concrete =
...
void SerializationObjectDistinctPaths::deserializeBinaryBulkWithMultipleStreams(ColumnPtr &column, size_t rows_offset, size_t limit,
                                                                                DeserializeBinaryBulkSettings &settings,
                                                                                DeserializeBinaryBulkStatePtr &state,
                                                                                SubstreamsCache *cache) const {
  if (!state) return;

  if (rows_offset + limit == 0) return;
  ...
  auto *object_structure_state =
      checkAndGetState<SerializationObject::DeserializeBinaryBulkStateObjectStructure>(object_distinct_paths_state->object_structure_state);
  if (object_distinct_paths_state->insert_dynamic_and_typed_paths) {
    for (const auto &path : typed_paths) paths_column.insertData(path.data(), path.size());
    for (const auto &path : *object_structure_state->sorted_dynamic_paths) paths_column.insertData(path.data(), path.size());
  ...
  switch (object_structure_state->shared_data_serialization_version.value) {
    case SerializationObjectSharedData::SerializationVersion::MAP: {
      ...
      shared_data_paths_serialization->deserializeBinaryBulkWithMultipleStreams(
          shared_data_paths_column, rows_offset, limit, settings_copy, object_distinct_paths_state->shared_data_paths_state, cache);
    ...
    case SerializationObjectSharedData::SerializationVersion::MAP_WITH_BUCKETS: {
      for (size_t bucket = 0; bucket < object_structure_state->shared_data_buckets; ++bucket) {
        ...
        shared_data_paths_serialization->deserializeBinaryBulkWithMultipleStreams(
            bucket_shared_data_paths_column, rows_offset, limit, settings,
            object_distinct_paths_state->bucket_shared_data_paths_state[bucket], cache);
        settings.path.pop_back();

        const auto &shared_data_paths_array_column = assert_cast<const ColumnArray &>(*bucket_shared_data_paths_column);
        paths_column.insertRangeFrom(shared_data_paths_array_column.getData(), 0, shared_data_paths_array_column.getData().size());

        if (bucket == 0) num_new_rows = bucket_shared_data_paths_column->size();
      }
      break;
    }
    case SerializationObjectSharedData::SerializationVersion::ADVANCED: {
      for (size_t bucket = 0; bucket < object_structure_state->shared_data_buckets; ++bucket) {
        ...
        for (const auto &structure_granule : *structure_granules) {
          for (const auto &path : structure_granule.all_paths) paths_column.insertData(path.data(), path.size());

          if (bucket == 0) num_new_rows += structure_granule.limit;
        }
        settings.path.pop_back();
      }
      break;
