...
SerializationStringSize::SerializationStringSize(MergeTreeStringSerializationVersion version_)
    : version(version_), serialization_string(version) {}

void SerializationStringSize::enumerateStreams(EnumerateStreamsSettings &settings, const StreamCallback &callback,
...
void SerializationStringSize::deserializeBinaryBulkWithMultipleStreams(ColumnPtr &column, size_t rows_offset, size_t limit,
...
void SerializationStringSize::deserializeBinaryBulkStatePrefix(DeserializeBinaryBulkSettings &settings,
  ...
  if (version == MergeTreeStringSerializationVersion::SINGLE_STREAM) {
    ...
    if (auto cached_state = getFromSubstreamsDeserializeStatesCache(cache, settings.path)) {
      ...
      if (!cache) string_state->need_string_data = true;
      state = string_state;
...
void SerializationStringSize::deserializeBinaryBulkWithoutSizeStream(ColumnPtr &column, size_t rows_offset, size_t limit,
                                                                     DeserializeBinaryBulkSettings &settings,
                                                                     DeserializeBinaryBulkStatePtr &state, SubstreamsCache *cache) const {
  settings.path.push_back(Substream::Regular);
...
void SerializationStringSize::deserializeWithStringData(ColumnPtr &column, size_t rows_offset, size_t limit,
                                                        DeserializeBinaryBulkSettings &settings,
  ...
  if (auto cached_column_with_num_read_rows = getColumnWithNumReadRowsFromSubstreamsCache(cache, settings.path)) {
    ...
    if (!string_state.column || column->empty()) string_state.column = ColumnString::create();

    size_t prev_size = string_state.column->size();
    double avg_value_size_hint = settings.get_avg_value_size_hint_callback ? settings.get_avg_value_size_hint_callback(settings.path) : 0.0;
    ...
    if (settings.update_avg_value_size_hint_callback) settings.update_avg_value_size_hint_callback(settings.path, *string_state.column);
  } else {
    return;
  }
  ...
  for (size_t i = prev_size; i != offsets.size(); ++i) sizes_data.push_back(offsets[i] - offsets[i - 1]);
}

void SerializationStringSize::deserializeWithoutStringData(ColumnPtr &column, size_t rows_offset, size_t limit,
                                                           DeserializeBinaryBulkSettings &settings, SubstreamsCache *cache) const {
...
void SerializationStringSize::deserializeBinaryBulkWithSizeStream(ColumnPtr &column, size_t rows_offset, size_t limit,
                                                                  DeserializeBinaryBulkSettings &settings,
                                                                  DeserializeBinaryBulkStatePtr & /* state */,
                                                                  SubstreamsCache *cache) const {
  settings.path.push_back(Substream::StringSizes);
  ...
  if (rows_offset) {
    ...
    for (size_t i = prev_size; i != actual_new_size; ++i) data[i] = data[i + rows_offset];
