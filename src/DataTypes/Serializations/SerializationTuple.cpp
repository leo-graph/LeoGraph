...
static inline IColumn &extractElementColumn(IColumn &column, size_t idx) { return assert_cast<ColumnTuple &>(column).getColumn(idx); }
...
void SerializationTuple::deserializeBinary(Field &field, ReadBuffer &istr, const FormatSettings &settings) const {
  ...
  tuple.reserve(size);
  for (size_t i = 0; i < size; ++i) elems[i]->deserializeBinary(tuple.emplace_back(), istr, settings);
}
    ...
    for (size_t i = 1; i < num_elems; ++i) {
      ...
      if (element_column.size() != new_size) {
        ...
        if constexpr (throw_exception)
          throw Exception(ErrorCodes::SIZES_OF_COLUMNS_IN_TUPLE_DOESNT_MATCH, "Cannot read a tuple because not all elements are present");
        return ReturnType(false);
      }
    }
  } catch (...) {
...
void SerializationTuple::readElementsSafe(DB::IColumn &column, std::function<void()> &&read_func) {
  addElementSafe<void>(assert_cast<ColumnTuple &>(column).getColumns().size(), column, [&]() {
    read_func();
    return true;
  });
}
...
void SerializationTuple::deserializeBinary(IColumn &column, ReadBuffer &istr, const FormatSettings &settings) const {
  addElementSafe<void>(elems.size(), column, [&] {
    for (size_t i = 0; i < elems.size(); ++i) elems[i]->deserializeBinary(extractElementColumn(column, i), istr, settings);
    return true;
  });
}
...
ReturnType SerializationTuple::deserializeTextImpl(IColumn &column, ReadBuffer &istr, const FormatSettings &settings, bool whole) const {
  ...
  auto impl = [&]() {
    ...
    if (whole && !istr.eof()) {
      if constexpr (throw_exception) throwUnexpectedDataAfterParsedValue(column, istr, settings, "Tuple");
      return false;
    }
...
bool SerializationTuple::tryDeserializeText(DB::IColumn &column, DB::ReadBuffer &istr, const DB::FormatSettings &settings,
                                            bool whole) const {
...
void SerializationTuple::serializeTextJSON(const IColumn &column, size_t row_num, WriteBuffer &ostr, const FormatSettings &settings) const {
  if (settings.json.write_named_tuples_as_objects && has_explicit_names) {
    writeChar('{', ostr);

    bool first = true;
    for (size_t i = 0; i < elems.size(); ++i) {
      const auto &element_column = extractElementColumn(column, i);
      if (settings.json.skip_null_value_in_named_tuples && element_column.isNullAt(row_num)) continue;

      if (!first) writeChar(',', ostr);
...
void SerializationTuple::serializeTextJSONPretty(const IColumn &column, size_t row_num, WriteBuffer &ostr, const FormatSettings &settings,
                                                 size_t indent) const {
  if (settings.json.write_named_tuples_as_objects && has_explicit_names) {
    writeCString("{\n", ostr);
    ...
    if (final_indent > 1) writeChar(settings.json.pretty_print_indent, final_indent, ostr);
    writeChar('}', ostr);
  } else {
    writeCString("[\n", ostr);
    for (size_t i = 0; i < elems.size(); ++i) {
...
template <typename ReturnType>
ReturnType SerializationTuple::deserializeTupleJSONImpl(IColumn &column, ReadBuffer &istr, const FormatSettings &settings,
                                                        auto &&deserialize_element) const {
  static constexpr auto throw_exception = std::is_same_v<ReturnType, void>;

  if (settings.json.read_named_tuples_as_objects && has_explicit_names) {
    skipWhitespaceIfAny(istr);
    if constexpr (throw_exception)
      assertChar('{', istr);
    else if (!checkChar('{', istr))
    ...
    auto impl = [&]() {
      ...
      while (!istr.eof() && *istr.position() != '}') {
        if (!settings.json.ignore_unknown_keys_in_named_tuple && processed == elems.size()) {
          if constexpr (throw_exception)
            throw Exception(ErrorCodes::INCORRECT_DATA,
                            "Unexpected number of elements in named tuple. Expected no more than {} (consider enabling "
                            "input_format_json_ignore_unknown_keys_in_named_tuple setting)",
                            elems.size());
          return false;
        }
        ...
        if (seen_elements[element_pos]) throw Exception(ErrorCodes::INCORRECT_DATA, "JSON object contains duplicate key '{}'", name);
        ...
        if constexpr (throw_exception) {
          ...
          if (!deserialize_element(element_column, element_pos)) return false;
        }
      ...
      if (processed != elems.size()) {
        for (size_t element_pos = 0; element_pos != seen_elements.size(); ++element_pos) {
          if (seen_elements[element_pos]) continue;
...
ReturnType SerializationTuple::deserializeTextJSONImpl(IColumn &column, ReadBuffer &istr, const FormatSettings &settings) const {
  auto deserialize_nested = [&settings](IColumn &nested_column, ReadBuffer &buf,
                                        const SerializationPtr &nested_column_serialization) -> ReturnType {
    if constexpr (std::is_same_v<ReturnType, void>) {
      ...
      if (settings.null_as_default && !isColumnNullableOrLowCardinalityNullable(nested_column))
        return SerializationNullable::tryDeserializeNullAsDefaultOrNestedTextJSON(nested_column, buf, settings,
                                                                                  nested_column_serialization);
  ...
  if (settings.json.empty_as_default)
    return deserializeTupleJSONImpl<ReturnType>(
        column, istr, settings, [&deserialize_nested, &istr, this](IColumn &nested_column, size_t element_pos) -> ReturnType {
          return JSONUtils::deserializeEmpyStringAsDefaultOrNested<ReturnType>(
              nested_column, istr, [&deserialize_nested, element_pos, this](IColumn &nested_column_, ReadBuffer &buf) -> ReturnType {
...
void SerializationTuple::serializeTextCSV(const IColumn &column, size_t row_num, WriteBuffer &ostr, const FormatSettings &settings) const {
  if (settings.csv.serialize_tuple_into_separate_columns) {
    for (size_t i = 0; i < elems.size(); ++i) {
      if (i != 0) writeChar(settings.csv.tuple_delimiter, ostr);
  ...
  if (settings.csv.deserialize_separate_columns_into_tuple) {
    return addElementSafe<bool>(elems.size(), column, [&] {
      ...
      for (size_t i = 0; i < size; ++i) {
        if (i != 0) {
          skipWhitespaceIfAny(istr);
          if (!checkChar(settings.csv.tuple_delimiter, istr)) return false;
          skipWhitespaceIfAny(istr);
        }

        auto &element_column = extractElementColumn(column, i);
        if (settings.null_as_default && !isColumnNullableOrLowCardinalityNullable(element_column)) {
          if (!SerializationNullable::tryDeserializeNullAsDefaultOrNestedTextCSV(element_column, istr, settings, elems[i])) return false;
        } else {
          if (!elems[i]->tryDeserializeTextCSV(element_column, istr, settings)) return false;
        }
      }
...
struct DeserializeBinaryBulkStateTuple : public ISerialization::DeserializeBinaryBulkState {
  ...
  ISerialization::DeserializeBinaryBulkStatePtr clone() const override {
    ...
    for (const auto &state : states) new_state->states.push_back(state ? state->clone() : nullptr);

    return new_state;
  }
};

void SerializationTuple::enumerateStreams(EnumerateStreamsSettings &settings, const StreamCallback &callback,
  ...
  const auto *tuple_deserialize_state =
      data.deserialize_state ? checkAndGetState<DeserializeBinaryBulkStateTuple>(data.deserialize_state) : nullptr;
...
void SerializationTuple::serializeBinaryBulkStatePrefix(const IColumn &column, SerializeBinaryBulkSettings &settings,
...
void SerializationTuple::serializeBinaryBulkStateSuffix(SerializeBinaryBulkSettings &settings, SerializeBinaryBulkStatePtr &state) const {
  auto *tuple_state = checkAndGetState<SerializeBinaryBulkStateTuple>(state);

  for (size_t i = 0; i < elems.size(); ++i) elems[i]->serializeBinaryBulkStateSuffix(settings, tuple_state->states[i]);
}

void SerializationTuple::deserializeBinaryBulkStatePrefix(DeserializeBinaryBulkSettings &settings, DeserializeBinaryBulkStatePtr &state,
                                                          SubstreamsDeserializeStatesCache *cache) const {
  auto tuple_state = std::make_shared<DeserializeBinaryBulkStateTuple>();
  tuple_state->states.resize(elems.size());

  for (size_t i = 0; i < elems.size(); ++i) elems[i]->deserializeBinaryBulkStatePrefix(settings, tuple_state->states[i], cache);

  state = std::move(tuple_state);
}

void SerializationTuple::serializeBinaryBulkWithMultipleStreams(const IColumn &column, size_t offset, size_t limit,
  ...
  if (elems.empty()) {
    if (WriteBuffer *stream = settings.getter(settings.path)) {
      ...
      if (limit == 0 || offset + limit > size) limit = size - offset;

      for (size_t i = 0; i < limit; ++i) stream->write('0');
    }
...
void SerializationTuple::deserializeBinaryBulkWithMultipleStreams(ColumnPtr &column, size_t rows_offset, size_t limit,
  ...
  for (size_t i = 0; i < elems.size(); ++i) {
    elems[i]->deserializeBinaryBulkWithMultipleStreams(column_tuple.getColumnPtr(i), rows_offset, limit, settings, tuple_state->states[i],
                                                       cache);
  }
  ...
  for (size_t i = 1; i < elems.size(); ++i) {
    if (column_tuple.getColumn(i).size() != expected_size)
      throw Exception(settings.native_format ? ErrorCodes::INCORRECT_DATA : ErrorCodes::LOGICAL_ERROR,
                      "Unexpected size of tuple element {}: {}. Expected size: {}", i, column_tuple.getColumn(i).size(), expected_size);
  }
...
size_t SerializationTuple::getPositionByName(const String &name) const {
  ...
  for (size_t i = 0; i < size; ++i)
    if (elems[i]->getElementName() == name) return i;
