...
class SerializationSparse final : public ISerialization {
 ...
 private:
  ...
  struct NullMapSubcolumnCreator : public ISubcolumnCreator {
    ...
    NullMapSubcolumnCreator(const ColumnPtr &offsets_, size_t size_) : offsets(offsets_), size(size_) {}
...
struct SubstreamsCacheSparseOffsetsElement : public ISerialization::ISubstreamsCacheElement {
  explicit SubstreamsCacheSparseOffsetsElement(ColumnPtr offsets_, size_t old_size_, size_t read_rows_, size_t skipped_values_rows_)
      : offsets(std::move(offsets_)), old_size(old_size_), read_rows(read_rows_), skipped_values_rows(skipped_values_rows_) {}
