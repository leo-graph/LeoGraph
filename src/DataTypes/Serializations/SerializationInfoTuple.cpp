...
SerializationInfoTuple::SerializationInfoTuple(MutableSerializationInfos elems_, Names names_)
  ...
  for (size_t i = 0; i < names.size(); ++i) name_to_elem[names[i]] = elems[i];
}
...
bool SerializationInfoTuple::structureEquals(const SerializationInfo &rhs) const {
  ...
  if (!rhs_tuple || elems.size() != rhs_tuple->elems.size()) return false;

  for (size_t i = 0; i < elems.size(); ++i)
    if (!elems[i]->structureEquals(*rhs_tuple->elems[i])) return false;
...
void SerializationInfoTuple::add(const IColumn &column) {
  ...
  for (size_t i = 0; i < elems.size(); ++i) elems[i]->add(*right_elems[i]);
}
...
void SerializationInfoTuple::remove(const SerializationInfo &other) {
  if (!structureEquals(other)) throw Exception(ErrorCodes::NOT_IMPLEMENTED, "Cannot remove from serialization info different structure");
  ...
  for (size_t i = 0; i < elems.size(); ++i) elems[i]->remove(*other_elems[i]);
}

void SerializationInfoTuple::addDefaults(size_t length) {
  SerializationInfo::addDefaults(length);

  for (const auto &elem : elems) elem->addDefaults(length);
}
...
void SerializationInfoTuple::replaceData(const SerializationInfo &other) {
  ...
  for (const auto &[name, elem] : name_to_elem) {
    auto it = other_info.name_to_elem.find(name);
    if (it != other_info.name_to_elem.end()) elem->replaceData(*it->second);
  }
}
...
MutableSerializationInfoPtr SerializationInfoTuple::clone() const {
  ...
  for (const auto &elem : elems) elems_cloned.push_back(elem ? elem->clone() : nullptr);
...
MutableSerializationInfoPtr SerializationInfoTuple::createWithType(const IDataType &old_type, const IDataType &new_type,
  ...
  for (size_t i = 0; i < elems.size(); ++i) infos.push_back(elems[i]->createWithType(*old_elements[i], *new_elements[i], new_settings));
...
void SerializationInfoTuple::serialializeKindStackBinary(WriteBuffer &out) const {
  ...
  for (const auto &elem : elems) elem->serialializeKindStackBinary(out);
}

void SerializationInfoTuple::deserializeFromKindsBinary(ReadBuffer &in) {
  SerializationInfo::deserializeFromKindsBinary(in);
  for (const auto &elem : elems) elem->deserializeFromKindsBinary(in);
}
...
void SerializationInfoTuple::fromJSON(const Poco::JSON::Object &object) {
  ...
  if (!object.has("subcolumns"))
    throw Exception(ErrorCodes::CORRUPTED_DATA, "Missed field 'subcolumns' in SerializationInfo of columns SerializationInfoTuple");
  ...
  if (elems.size() != subcolumns->size())
    throw Exception(ErrorCodes::THERE_IS_NO_COLUMN,
                    ...
                    "Expected: {}, got: {}",
                    elems.size(), subcolumns->size());

  for (size_t i = 0; i < elems.size(); ++i) elems[i]->fromJSON(*subcolumns->getObject(static_cast<unsigned>(i)));
}
