...
void SerializationArray::deserializeBinary(Field &field, ReadBuffer &istr, const FormatSettings &settings) const {
  ...
  if (settings.binary.max_binary_array_size && size > settings.binary.max_binary_array_size)
    throw Exception(ErrorCodes::TOO_LARGE_ARRAY_SIZE,
                    ...
                    size, settings.binary.max_binary_array_size);

  field = Array();
  Array &arr = field.safeGet<Array>();
  arr.reserve(size);
  for (size_t i = 0; i < size; ++i) nested->deserializeBinary(arr.emplace_back(), istr, settings);
}
...
void SerializationArray::serializeBinary(const IColumn &column, size_t row_num, WriteBuffer &ostr, const FormatSettings &settings) const {
  ...
  for (size_t i = offset; i < next_offset; ++i) nested->serializeBinary(nested_column, i, ostr, settings);
}
...
void SerializationArray::deserializeBinary(IColumn &column, ReadBuffer &istr, const FormatSettings &settings) const {
  ...
  size_t i = 0;
  try {
    for (; i < size; ++i) nested->deserializeBinary(nested_column, istr, settings);
  } catch (...) {
    if (i) nested_column.popBack(i);
    throw;
  }
...
void SerializationArray::serializeForHashCalculation(const IColumn &column, size_t row_num, WriteBuffer &ostr) const {
  ...
  const IColumn &nested_column = column_array.getData();
  for (size_t i = offset; i < next_offset; ++i) nested->serializeForHashCalculation(nested_column, i, ostr);
}

namespace {
void serializeArraySizesPositionIndependent(const IColumn &offsets_column, WriteBuffer &ostr, UInt64 offset, UInt64 limit) {
...
void deserializeArraySizesPositionIndependent(IColumn &offsets_column, ReadBuffer &istr, UInt64 limit) {
  ...
  while (i < initial_size + limit && !istr.eof()) {
    ...
    if (unlikely(current_size > MAX_ARRAY_SIZE))
      throw Exception(ErrorCodes::TOO_LARGE_ARRAY_SIZE, "Array size at index {} is too large: {}", i, current_size);
...
ColumnPtr arraySizesToOffsets(const IColumn &column) {
  ...
  if (column_sizes.empty()) return column_offsets;
...
ColumnPtr arrayOffsetsToSizes(const IColumn &column) {
  ...
  if (column_offsets.empty()) return column_sizes;
...
DataTypePtr SerializationArray::SubcolumnCreator::create(const DataTypePtr &prev) const { return std::make_shared<DataTypeArray>(prev); }
...
ColumnPtr SerializationArray::SubcolumnCreator::create(const ColumnPtr &prev) const { return ColumnArray::create(prev, offsets); }

void SerializationArray::enumerateStreams(EnumerateStreamsSettings &settings, const StreamCallback &callback,
  ...
  auto offsets_serialization =
      std::make_shared<SerializationNamed>(std::make_shared<SerializationArrayOffsets>(), subcolumn_name, SubstreamType::NamedOffsets);

  auto offsets_column = offsets && !settings.position_independent_encoding ? arrayOffsetsToSizes(*offsets) : offsets;
...
void SerializationArray::serializeBinaryBulkStatePrefix(const IColumn &column, SerializeBinaryBulkSettings &settings,
...
void SerializationArray::serializeBinaryBulkStateSuffix(SerializeBinaryBulkSettings &settings, SerializeBinaryBulkStatePtr &state) const {
  settings.path.push_back(Substream::ArrayElements);
  nested->serializeBinaryBulkStateSuffix(settings, state);
  settings.path.pop_back();
}

void SerializationArray::deserializeBinaryBulkStatePrefix(DeserializeBinaryBulkSettings &settings, DeserializeBinaryBulkStatePtr &state,
...
void SerializationArray::serializeOffsetsBinaryBulk(const IColumn &offsets_column, size_t offset, size_t limit,
...
void SerializationArray::serializeBinaryBulkWithMultipleStreams(const IColumn &column, size_t offset, size_t limit,
  ...
  settings.path.push_back(Substream::ArraySizes);
  serializeOffsetsBinaryBulk(column_array.getOffsetsColumn(), offset, limit, settings);
...
bool SerializationArray::deserializeOffsetsBinaryBulk(ColumnPtr &offsets_column, size_t limit,
  ...
  if (auto cached_column_with_num_read_rows = getColumnWithNumReadRowsFromSubstreamsCache(cache, settings.path)) {
    ...
    /// But sometimes in cache we might have column with rows from the current range only (for example when we don't store this column but
    /// need it for constructing another column). In this case we need to insert data into resulting column from cached column. To determine
    /// what case we have we store number of read rows in last range in cache.
    ...
    if ((settings.insert_only_rows_in_current_range_from_substreams_cache) ||
        (!offsets_column->empty() && cached_column->size() == num_read_rows))
    ...
    if (settings.native_format) {
      ...
      if (it != offsets.end()) {
        throw Exception(ErrorCodes::INCORRECT_DATA, "Arrays offsets are not monotonically increasing (starting at {}, value {})",
                        std::distance(offsets.begin(), it), *it);
      }
    }

    /// Add array sizes read from current range into the cache.
    if (cache)
      addColumnWithNumReadRowsToSubstreamsCache(cache, settings.path, arrayOffsetsToSizes(*offsets_column),
                                                offsets_column->size() - prev_size);
...
std::pair<size_t, size_t> SerializationArray::deserializeOffsetsBinaryBulkAndGetNestedOffsetAndLimit(
    ColumnPtr &offsets_column, size_t offset, size_t limit, ISerialization::DeserializeBinaryBulkSettings &settings,
    ISerialization::SubstreamsCache *cache) {
  ...
  if (!deserializeOffsetsBinaryBulk(offsets_column, offset + limit, settings, cache)) return {0, 0};
  ...
  if (last_offset < prev_last_offset)
    throw Exception(ErrorCodes::LOGICAL_ERROR, "Array elements column is longer (>{}) than the last offset ({})", prev_last_offset,
                    last_offset);
  size_t nested_limit = last_offset - prev_last_offset;
  return {skipped_nested_rows, nested_limit};
}

void SerializationArray::deserializeBinaryBulkWithMultipleStreams(ColumnPtr &column, size_t rows_offset, size_t limit,
  ...
  auto [skipped_nested_rows, nested_limit] =
      deserializeOffsetsBinaryBulkAndGetNestedOffsetAndLimit(column_array.getOffsetsPtr(), rows_offset, limit, settings, cache);
  ...
  nested->deserializeBinaryBulkWithMultipleStreams(nested_column, skipped_nested_rows, nested_limit, settings, state, cache);
  ...
  if (!nested_column->empty() && nested_column->size() != last_offset)
    throw Exception(ErrorCodes::CANNOT_READ_ALL_DATA, "Cannot read all array values: read just {} of {}", toString(nested_column->size()),
...
static ReturnType deserializeTextImpl(IColumn &column, ReadBuffer &istr, Reader &&read_nested, bool allow_unenclosed) {
  ...
  if (checkChar('[', istr))
    ...
    if constexpr (throw_exception) throw Exception(ErrorCodes::CANNOT_READ_ARRAY_FROM_TEXT, "Array does not start with '[' character");
    ...
    if (istr.eof()) {
      if constexpr (throw_exception)
        throw Exception(ErrorCodes::CANNOT_READ_ARRAY_FROM_TEXT,
                        "Cannot read array from text, expected opening bracket '[' or array element");
      return ReturnType(false);
    }
  }

  auto on_error_no_throw = [&]() {
    if (size) nested_column.popBack(size);
    return ReturnType(false);
  };
  ...
  try {
    ...
    while (!istr.eof() && *istr.position() != ']') {
      if (!first) {
        if (*istr.position() == ',') {
          ...
          if constexpr (throw_exception)
            throw Exception(ErrorCodes::CANNOT_READ_ARRAY_FROM_TEXT,
                            "Cannot read array from text, expected comma or end of array, found '{}'", *istr.position());
    ...
    if (size) nested_column.popBack(size);
    if constexpr (throw_exception) throw;
    return ReturnType(false);
  }
...
void SerializationArray::readArraySafe(DB::IColumn &column, std::function<void()> &&read_func) {
  ...
  try {
    ...
    if (nested_column.size() > offsets.back()) nested_column.popBack(nested_column.size() - offsets.back());
...
void SerializationArray::serializeText(const IColumn &column, size_t row_num, WriteBuffer &ostr, const FormatSettings &settings) const {
  serializeTextImpl(column, row_num, ostr,
                    [&](const IColumn &nested_column, size_t i) { nested->serializeTextQuoted(nested_column, i, ostr, settings); });
}
...
void SerializationArray::deserializeText(IColumn &column, ReadBuffer &istr, const FormatSettings &settings, bool whole) const {
  ...
  if (whole && !istr.eof()) throwUnexpectedDataAfterParsedValue(column, istr, settings, "Array");
}
...
void SerializationArray::serializeTextJSONPretty(const IColumn &column, size_t row_num, WriteBuffer &ostr, const FormatSettings &settings,
                                                 size_t indent) const {
  const ColumnArray &column_array = assert_cast<const ColumnArray &>(column);
  const ColumnArray::Offsets &offsets = column_array.getOffsets();
  ...
  bool print_each_element_on_separate_line = nested_column_type == TypeIndex::Array || nested_column_type == TypeIndex::Map ||
                                             nested_column_type == TypeIndex::Object || nested_column_type == TypeIndex::Tuple;
...
ReturnType SerializationArray::deserializeTextJSONImpl(IColumn &column, ReadBuffer &istr, const FormatSettings &settings) const {
  ...
  if (settings.json.empty_as_default)
    return deserializeTextImpl<ReturnType>(
        ...
        false);
  return deserializeTextImpl<ReturnType>(
      column, istr, [&deserialize_nested, &istr](IColumn &nested_column) -> ReturnType { return deserialize_nested(nested_column, istr); },
      false);
}
...
void SerializationArray::serializeTextCSV(const IColumn &column, size_t row_num, WriteBuffer &ostr, const FormatSettings &settings) const {
  /// There is no good way to serialize an array in CSV. Therefore, we serialize it into a string, and then write the resulting string in
  /// CSV.
...
bool SerializationArray::tryDeserializeTextCSV(IColumn &column, ReadBuffer &istr, const FormatSettings &settings) const {
  String s;
  if (!tryReadCSV(s, istr, settings.csv)) return false;
  ReadBufferFromString rb(s);
