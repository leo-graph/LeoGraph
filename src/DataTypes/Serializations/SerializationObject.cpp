...
SerializationObject::SerializationObject(const std::unordered_map<String, DataTypePtr> &typed_paths_types_,
                                         ...
                                         const DataTypePtr &dynamic_type_, const SerializationPtr &dynamic_serialization_)
  ...
  for (const auto &[path, _] : typed_paths_types) sorted_typed_paths.emplace_back(path);
  ...
  for (const auto &regexp_str : path_regexps_to_skip_) path_regexps_to_skip.emplace_back(regexp_str);
}
...
bool SerializationObject::shouldSkipPath(const String &path) const {
  ...
  if (it != sorted_paths_to_skip.end() && it != sorted_paths_to_skip.begin() && path.starts_with(*std::prev(it))) return true;
...
void SerializationObject::enumerateStreams(EnumerateStreamsSettings &settings, const StreamCallback &callback,
  ...
  const auto *deserialize_state =
      data.deserialize_state ? checkAndGetState<DeserializeBinaryBulkStateObject>(data.deserialize_state) : nullptr;
  const auto *structure_state =
      deserialize_state ? checkAndGetState<DeserializeBinaryBulkStateObjectStructure>(deserialize_state->structure_state) : nullptr;
  ...
  if (settings.enumerate_dynamic_streams && (column_object || structure_state)) {
    ...
    if (structure_state) {
      ...
      for (const auto &[path, _] : *dynamic_paths) sorted_dynamic_paths->push_back(path);
    ...
    if (deserialize_state) {
      ...
      SerializationObjectSharedData::SerializationVersion shared_data_serialization_version(
          SerializationObjectSharedData::SerializationVersion::MAP);
      ...
      if (serialization_version.value == SerializationVersion::V3) {
        shared_data_serialization_version =
            SerializationObjectSharedData::SerializationVersion(settings.object_shared_data_serialization_version);
      ...
      shared_data_serialization = std::make_shared<SerializationObjectSharedData>(shared_data_serialization_version, dynamic_type,
                                                                                  dynamic_serialization, num_buckets);
    }
...
void SerializationObject::serializeBinaryBulkStatePrefix(const IColumn &column, SerializeBinaryBulkSettings &settings,
  ...
  if (!stream)
    throw Exception(ErrorCodes::LOGICAL_ERROR,
                    "Missing stream for Object column structure during serialization of binary bulk state prefix");
  ...
  else if (settings.native_format && settings.format_settings &&
           settings.format_settings->native.use_flattened_dynamic_and_json_serialization)
  ...
  if (serialization_version.value == SerializationVersion::FLATTENED) {
    ...
    for (const auto &[path, _] : object_state->flattened_paths) writeStringBinary(path, *stream);
    ...
    for (const auto &path : sorted_typed_paths) {
      settings.path.push_back(Substream::ObjectTypedPath);
      settings.path.back().object_path_name = path;
      typed_paths_serializations.at(path)->serializeBinaryBulkStatePrefix(*typed_paths.at(path), settings,
                                                                          object_state->typed_path_states[path]);
      settings.path.pop_back();
    }
  ...
  for (const auto &[path, _] : dynamic_paths) object_state->sorted_dynamic_paths.push_back(path);
  ...
  if (serialization_version.value == SerializationVersion::V1) writeVarUInt(object_state->sorted_dynamic_paths.size(), *stream);

  writeVarUInt(object_state->sorted_dynamic_paths.size(), *stream);
  for (const auto &path : object_state->sorted_dynamic_paths) writeStringBinary(path, *stream);
  ...
  /// In V3 serialization we can choose different serialize version of shared data serialization and number of buckets if this serialization
  /// supports it. We need to write selected serialization version and the number of buckets to be able to deserialize it back.
  if (serialization_version.value == SerializationVersion::V3) {
    shared_data_serialization_version =
        SerializationObjectSharedData::SerializationVersion(settings.object_shared_data_serialization_version);
  ...
  if (settings.object_and_dynamic_write_statistics == SerializeBinaryBulkSettings::ObjectAndDynamicStatisticsMode::PREFIX) {
    ...
    if (serialization_version.value == SerializationVersion::V3) writeBinary(true, *stream);
    ...
    /// If we don't have statistics for shared data from merge, calculate it from the column.
      ...
      std::unordered_map<String, size_t, StringHashForHeterogeneousLookup, StringHashForHeterogeneousLookup::transparent_key_equal>
          shared_data_paths_statistics;
  ...
  /// For other granules we write empty statistics.
    ...
    /// Otherwise serialize the minimum version of statistics.
      ...
      for (size_t i = 0; i != object_state->sorted_dynamic_paths.size(); ++i) writeVarUInt(0, *stream);
  ...
  settings.path.push_back(Substream::ObjectSharedData);
  object_state->shared_data_serialization = std::make_shared<SerializationObjectSharedData>(shared_data_serialization_version, dynamic_type,
                                                                                            dynamic_serialization, shared_data_buckets);
...
void SerializationObject::deserializeBinaryBulkStatePrefix(DeserializeBinaryBulkSettings &settings, DeserializeBinaryBulkStatePtr &state,
                                                           SubstreamsDeserializeStatesCache *cache) const {
  auto structure_state = deserializeObjectStructureStatePrefix(settings, cache);
  if (!structure_state) return;
  ...
  if (settings.dynamic_subcolumns_callback) {
    ...
    for (const auto &path : *structure_state_concrete->sorted_dynamic_paths) {
      ...
      dynamic_serialization->enumerateStreams(enumerate_settings, settings.dynamic_subcolumns_callback,
                                              SubstreamData(dynamic_serialization));
  ...
  if (settings.prefixes_prefetch_callback) {
    ...
    auto enumerate_callback = [&](const SubstreamPath &path) {
      if (hasPrefix(path)) settings.prefixes_prefetch_callback(path);
    };
  ...
  if (settings.prefixes_deserialization_thread_pool && !structure_state_concrete->sorted_dynamic_paths->empty()) {
    ...
    size_t num_tasks =
        std::min(settings.prefixes_deserialization_thread_pool->getMaxThreads(), structure_state_concrete->sorted_dynamic_paths->size());
    ...
    for (const auto &path : *structure_state_concrete->sorted_dynamic_paths) object_state->dynamic_path_states[path] = nullptr;
    ...
    for (size_t i = 0; i != num_tasks; ++i) {
      ...
      auto deserialize = [&, batch_start, batch_end, cache_ptr = cache_copy.get()]() {
        ...
        settings_copy.dynamic_subcolumns_callback =
            settings.dynamic_subcolumns_callback ? safe_dynamic_subcolumns_callback : StreamCallback{};
        ...
        for (size_t j = batch_start; j != batch_end; ++j) {
          ...
          dynamic_serialization->deserializeBinaryBulkStatePrefix(
              settings_copy, object_state->dynamic_path_states.at((*structure_state_concrete->sorted_dynamic_paths)[j]), cache_ptr);
      ...
      static_cast<void>(
          settings.prefixes_deserialization_thread_pool->trySchedule([task_ptr = task, thread_group = CurrentThread::getGroup()]() {
    ...
    for (const auto &task : tasks) task->tryExecute();
    ...
    for (const auto &task : tasks) {
      if (auto e = task->wait()) exception = e;
    }
    ...
    if (cache) {
      for (const auto &cache_copy : caches) cache->merge(*cache_copy);
    }
  } else {
    for (const auto &path : *structure_state_concrete->sorted_dynamic_paths) {
      settings.path.push_back(Substream::ObjectDynamicPath);
  ...
  settings.path.push_back(Substream::ObjectSharedData);
  object_state->shared_data_serialization =
      std::make_shared<SerializationObjectSharedData>(structure_state_concrete->shared_data_serialization_version, dynamic_type,
                                                      dynamic_serialization, structure_state_concrete->shared_data_buckets);
...
ISerialization::DeserializeBinaryBulkStatePtr SerializationObject::deserializeObjectStructureStatePrefix(
  ...
  if (auto cached_state = getFromSubstreamsDeserializeStatesCache(cache, settings.path)) {
    ...
    if (structure_state->serialization_version.value == SerializationVersion::FLATTENED) {
      ...
      for (size_t i = 0; i != paths_size; ++i) readStringBinary(structure_state->flattened_paths[i], *structure_stream);
      ...
      for (size_t i = 0; i != dynamic_paths_size; ++i) readStringBinary((*structure_state->sorted_dynamic_paths)[i], *structure_stream);
      ...
      if (structure_state->serialization_version.value == SerializationVersion::V3) {
        ...
        structure_state->shared_data_serialization_version =
            SerializationObjectSharedData::SerializationVersion(shared_data_serialization_version);
        /// If shared serialization version supports buckets, read number of buckets.
        if (structure_state->shared_data_serialization_version.value ==
                SerializationObjectSharedData::SerializationVersion::MAP_WITH_BUCKETS ||
      ...
      if (settings.object_and_dynamic_read_statistics) {
        ...
        if (structure_state->serialization_version.value == SerializationVersion::V3) readBinary(has_statistics, *structure_stream);
        if (has_statistics) {
    ...
    if (settings.release_stream_callback) settings.release_stream_callback(settings.path);
  }
...
void SerializationObject::serializeBinaryBulkWithMultipleStreams(const IColumn &column, size_t offset, size_t limit,
  ...
  if (object_state->serialization_version.value == SerializationVersion::STRING) {
    ...
    if (!data_stream)
      throw Exception(ErrorCodes::LOGICAL_ERROR,
                      "Got empty stream for String data in SerializationObject::serializeBinaryBulkWithMultipleStreams");
  ...
  if (object_state->serialization_version.value == SerializationVersion::FLATTENED) {
    ...
    for (const auto &path : sorted_typed_paths) {
      settings.path.push_back(Substream::ObjectTypedPath);
      settings.path.back().object_path_name = path;
      typed_paths_serializations.at(path)->serializeBinaryBulkWithMultipleStreams(*typed_paths.at(path), offset, limit, settings,
                                                                                  object_state->typed_path_states[path]);
      settings.path.pop_back();
    }
    ...
    for (const auto &[path, path_column] : object_state->flattened_paths) {
      settings.path.push_back(Substream::ObjectDynamicPath);
      settings.path.back().object_path_name = path;
      dynamic_serialization->serializeBinaryBulkWithMultipleStreams(*path_column, offset, limit, settings,
                                                                    object_state->dynamic_path_states[path]);
      settings.path.pop_back();
    }
  ...
  if (column_object.getDynamicPaths().size() != object_state->sorted_dynamic_paths.size())
    throw Exception(ErrorCodes::LOGICAL_ERROR, "Mismatch of number of dynamic paths in Object. Expected: {}, Got: {}",
                    object_state->sorted_dynamic_paths.size(), column_object.getDynamicPaths().size());
  ...
  for (const auto &path : object_state->sorted_dynamic_paths) {
    ...
    if (it == dynamic_paths.end())
      throw Exception(ErrorCodes::LOGICAL_ERROR,
                      "Dynamic structure mismatch for Object column: dynamic path '{}' is not found in the column", path);
    if (object_state->recalculate_statistics) {
      size_t number_of_non_null_values = 0;
      dynamic_serialization_typed->serializeBinaryBulkWithMultipleStreamsAndCountTotalSizeOfVariants(
          *it->second, offset, limit, settings, object_state->dynamic_path_states[path], number_of_non_null_values);
      object_state->statistics.dynamic_paths_statistics[path] += number_of_non_null_values;
    } else {
      dynamic_serialization_typed->serializeBinaryBulkWithMultipleStreams(*it->second, offset, limit, settings,
                                                                          object_state->dynamic_path_states[path]);
    }
    settings.path.pop_back();
  }
  ...
  settings.path.push_back(Substream::ObjectSharedData);
  object_state->shared_data_serialization->serializeBinaryBulkWithMultipleStreams(*shared_data, offset, limit, settings,
                                                                                  object_state->shared_data_state);
  if (object_state->recalculate_statistics) {
    ...
    size_t end =
        limit == 0 || offset + limit > shared_data_offsets.size() ? shared_data_paths->size() : shared_data_offsets[offset + limit - 1];
    for (size_t i = start; i != end; ++i) {
      auto path = shared_data_paths->getDataAt(i);
      if (auto it = object_state->statistics.shared_data_paths_statistics.find(path);
          it != object_state->statistics.shared_data_paths_statistics.end())
        ++it->second;
...
void SerializationObject::serializeBinaryBulkStateSuffix(SerializeBinaryBulkSettings &settings, SerializeBinaryBulkStatePtr &state) const {
  auto *object_state = checkAndGetState<SerializeBinaryBulkStateObject>(state);
  if (object_state->serialization_version.value == SerializationVersion::STRING) return;
  ...
  if (settings.object_and_dynamic_write_statistics == SerializeBinaryBulkSettings::ObjectAndDynamicStatisticsMode::SUFFIX) {
    ...
    if (!stream)
      throw Exception(ErrorCodes::LOGICAL_ERROR,
                      "Missing stream for Object column structure during serialization of binary bulk state suffix");

    /// In V3 serialization version write flag that statistics is not empty.
    /// It is needed to be able to write empty statistics if needed.
    if (object_state->serialization_version.value == SerializationVersion::V3) writeBinary(true, *stream);
...
void SerializationObject::deserializeBinaryBulkWithMultipleStreams(ColumnPtr &column, size_t rows_offset, size_t limit,
  ...
  if (structure_state->serialization_version.value == SerializationVersion::FLATTENED) {
    ...
    for (const auto &path : sorted_typed_paths) {
      settings.path.push_back(Substream::ObjectTypedPath);
      settings.path.back().object_path_name = path;
      typed_paths_serializations.at(path)->deserializeBinaryBulkWithMultipleStreams(typed_paths[path], rows_offset, limit, settings,
                                                                                    object_state->typed_path_states[path], cache);
      settings.path.pop_back();
    }
    ...
    for (const auto &path : structure_state->flattened_paths) {
      ...
      dynamic_serialization->deserializeBinaryBulkWithMultipleStreams(flattened_paths_columns.back(), rows_offset, limit, settings,
                                                                      object_state->dynamic_path_states[path], cache);
      settings.path.pop_back();
    }
  ...
  for (const auto &path : *structure_state->sorted_dynamic_paths) {
    settings.path.push_back(Substream::ObjectDynamicPath);
    settings.path.back().object_path_name = path;
    dynamic_serialization->deserializeBinaryBulkWithMultipleStreams(dynamic_paths[path], rows_offset, limit, settings,
                                                                    object_state->dynamic_path_states[path], cache);
    settings.path.pop_back();
  }
  ...
  settings.path.push_back(Substream::ObjectSharedData);
  object_state->shared_data_serialization->deserializeBinaryBulkWithMultipleStreams(shared_data, rows_offset, limit, settings,
                                                                                    object_state->shared_data_state, cache);
  settings.path.pop_back();
  settings.path.pop_back();
  ...
  for (const auto &[path, path_column] : typed_paths) {
    if (path_column->size() != expected_size)
      throw Exception(settings.native_format ? ErrorCodes::INCORRECT_DATA : ErrorCodes::LOGICAL_ERROR,
                      "Unexpected size of typed path {}: {}. Expected size {}", path, path_column->size(), expected_size);
  }

  for (const auto &[path, path_column] : dynamic_paths) {
    if (path_column->size() != expected_size)
      throw Exception(settings.native_format ? ErrorCodes::INCORRECT_DATA : ErrorCodes::LOGICAL_ERROR,
                      "Unexpected size of dynamic path {}: {}. Expected size {}", path, path_column->size(), expected_size);
  }
...
void SerializationObject::serializeBinary(const IColumn &col, size_t row_num, WriteBuffer &ostr, const FormatSettings &settings) const {
  ...
  for (const auto &[_, column] : dynamic_paths) non_null_dynamic_pats += !column->isNullAt(row_num);
  /// Serialize number of paths and then pairs (path, value).
...
void SerializationObject::deserializeBinary(Field &field, ReadBuffer &istr, const FormatSettings &settings) const {
  ...
  if (settings.binary.max_object_size && number_of_paths > settings.binary.max_object_size)
    throw Exception(ErrorCodes::TOO_LARGE_ARRAY_SIZE,
                    ...
                    number_of_paths, settings.binary.max_object_size);
  ...
  for (auto it = ColumnObject::SortedPathsIterator(column_object, row_num); !it.end(); it.next()) {
    ...
    if (path_info.type == ColumnObject::SortedPathsIterator::PathType::TYPED) {
      ...
      if (isDynamic(typed_paths_types.at(path))) {
        ...
        SerializationDynamic::serializeVariantForHashCalculation(*path_info.column, typed_paths_serializations.at(path),
                                                                 typed_paths_types.at(path), path_info.row, ostr);
      }
    } else {
...
void SerializationObject::restoreColumnObject(ColumnObject &column_object, size_t prev_size) {
  ...
  if (shared_data_offsets.size() > prev_size) shared_data_offsets.resize(prev_size);
  size_t prev_shared_data_offset = shared_data_offsets.back();
  if (shared_data_paths->size() > prev_shared_data_offset) shared_data_paths->popBack(shared_data_paths->size() - prev_shared_data_offset);
    ...
    for (size_t i = 0; i != number_of_paths; ++i) {
      ...
      if (!shouldSkipPath(path)) {
          ...
          if (typed_column->size() > prev_size) {
            if (!settings.json.type_json_skip_duplicated_paths)
              throw Exception(ErrorCodes::INCORRECT_DATA,
                              "Found duplicated path during binary deserialization of JSON type: {}. You can enable setting "
        ...
        /// Otherwise this path should go to shared data.
          ...
          if (!field.isNull()) paths_and_values_for_shared_data.emplace_back(std::move(path), std::move(value));
        }
      } else {
        /// Skip value of this path.
        Field tmp;
...
void SerializationObject::updateMaxDynamicPathsLimitIfNeeded(IColumn &column, const FormatSettings &format_settings) const {
  if (!format_settings.json.max_dynamic_subcolumns_in_json_type_parsing || !column.empty()) return;
