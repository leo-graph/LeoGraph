...
SerializationInfoSettings::SerializationInfoSettings(double ratio_of_defaults_for_sparse_, bool choose_kind_,
...
void SerializationInfoSettings::tryDowngradeToBasic() {
  if (version == MergeTreeSerializationInfoVersion::BASIC) return;

  bool no_specialization = string_serialization_version == MergeTreeStringSerializationVersion::SINGLE_STREAM &&
                           nullable_serialization_version == MergeTreeNullableSerializationVersion::BASIC;

  if (no_specialization) version = MergeTreeSerializationInfoVersion::BASIC;
}

bool SerializationInfoSettings::canUseSparseSerialization(const IDataType &type) const {
  if (type.isNullable()) {
    if (nullable_serialization_version == MergeTreeNullableSerializationVersion::BASIC) return false;
    ...
    if (const auto *nullable_type = assert_cast<const DataTypeNullable *>(&type)) {
      if (isTuple(nullable_type->getNestedType())) return false;
    }
  }
