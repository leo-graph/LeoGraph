...
const ColumnUInt8 *checkAndGetSerializeColumnType(const IColumn &column) {
  ...
  if (!col) throw Exception(ErrorCodes::ILLEGAL_COLUMN, "Bool type can only serialize columns of type UInt8.{}", column.getName());
  return col;
}

ColumnUInt8 *checkAndGetDeserializeColumnType(IColumn &column) {
  auto *col = typeid_cast<ColumnUInt8 *>(&column);
  if (!col) throw Exception(ErrorCodes::ILLEGAL_COLUMN, "Bool type can only deserialize columns of type UInt8.{}", column.getName());
  return col;
}
...
bool tryDeserializeAllVariants(ColumnUInt8 *column, ReadBuffer &istr) {
  ...
  /// 'True' and 'T'
    ...
    if (checkCharCaseInsensitive('r', istr)) {
      if (!checkStringCaseInsensitive("ue", istr)) return false;
    }
    column->insert(true);
  }
  /// 'False' and 'F'
    ...
    if (checkCharCaseInsensitive('a', istr)) {
      if (!checkStringCaseInsensitive("lse", istr)) return false;
    }
    column->insert(false);
  }
  /// 'Yes' and 'Y'
    ...
    if (checkCharCaseInsensitive('e', istr)) {
      if (!checkCharCaseInsensitive('s', istr)) return false;
    }
    column->insert(true);
  }
  /// 'No' and 'N'
...
ReturnType deserializeImpl(IColumn &column, ReadBuffer &istr, const FormatSettings &settings,
                           std::function<bool(ReadBuffer &)> check_end_of_value) {
  static constexpr bool throw_exception = std::is_same_v<ReturnType, void>;

  ColumnUInt8 *col = checkAndGetDeserializeColumnType(column);
  auto restore_column_if_needed = [&, prev_size = col->size()]() {
    if (col->size() > prev_size) col->popBack(1);
  };
  ...
  if constexpr (throw_exception)
    throw Exception(ErrorCodes::CANNOT_PARSE_BOOL,
                    ...
                    String(buf.position(), std::min(10lu, buf.available())), settings.bool_true_representation,
...
void SerializationBool::deserializeBinary(DB::Field &field, DB::ReadBuffer &istr, const DB::FormatSettings &settings) const {
  ...
  if (!settings.binary.read_bool_field_as_int) field = bool(field.safeGet<bool>());
}

SerializationBool::SerializationBool(const SerializationPtr &nested_) : SerializationWrapper(nested_) {}
...
void SerializationBool::serializeTextEscaped(const IColumn &column, size_t row_num, WriteBuffer &ostr,
...
void SerializationBool::deserializeTextEscaped(IColumn &column, ReadBuffer &istr, const FormatSettings &settings) const {
  if (istr.eof()) throw Exception(ErrorCodes::CANNOT_PARSE_BOOL, "Expected boolean value but get EOF.");
  if (settings.tsv.crlf_end_of_line_input)
    deserializeImpl(column, istr, settings, [](ReadBuffer &buf) {
      return buf.eof() || *buf.position() == '\t' || *buf.position() == '\n' || *buf.position() == '\r';
    });
  else
    deserializeImpl(column, istr, settings,
                    [](ReadBuffer &buf) { return buf.eof() || *buf.position() == '\t' || *buf.position() == '\n'; });
}
...
bool SerializationBool::tryDeserializeTextEscaped(IColumn &column, ReadBuffer &istr, const FormatSettings &settings) const {
  ...
  return deserializeImpl<bool>(column, istr, settings,
                               [](ReadBuffer &buf) { return buf.eof() || *buf.position() == '\t' || *buf.position() == '\n'; });
}
...
void SerializationBool::serializeTextJSONPretty(const IColumn &column, size_t row_num, WriteBuffer &ostr, const FormatSettings &settings,
                                                size_t) const {
  serializeSimple(column, row_num, ostr, settings);
}
...
bool SerializationBool::tryDeserializeTextJSON(DB::IColumn &column, DB::ReadBuffer &istr, const DB::FormatSettings &) const {
  ...
  char first_char = *istr.position();
  if (first_char == 't' || first_char == 'f') {
    if (!readBoolTextWord<bool>(value, istr)) return false;
  } else if (first_char == '1' || first_char == '0') {
    /// Doesn't throw.
    readBoolText(value, istr);
  } else {
...
bool SerializationBool::tryDeserializeTextCSV(IColumn &column, ReadBuffer &istr, const FormatSettings &settings) const {
  ...
  return deserializeImpl<bool>(column, istr, settings, [&](ReadBuffer &buf) {
    return buf.eof() || *buf.position() == settings.csv.delimiter || *buf.position() == '\n' || *buf.position() == '\r';
  });
}
...
void SerializationBool::serializeTextQuoted(const IColumn &column, size_t row_num, WriteBuffer &ostr,
                                            const FormatSettings &settings) const {
  serializeSimple(column, row_num, ostr, settings);
}
...
ReturnType deserializeTextQuotedImpl(IColumn &column, ReadBuffer &istr, const FormatSettings &settings) {
  ...
  if (istr.eof()) {
    if constexpr (throw_exception) throw Exception(ErrorCodes::CANNOT_PARSE_BOOL, "Expected boolean value but get EOF.");
    return ReturnType(false);
  }
  ...
  switch (symbol) {
    ...
    case '\'':
      ...
      if constexpr (throw_exception) {
        ...
        if (!deserializeImpl<bool>(column, istr, settings, [](ReadBuffer &buf) { return !buf.eof() && *buf.position() == '\''; }) ||
            !checkChar('\'', istr))
          return ReturnType(false);
      }
      break;
