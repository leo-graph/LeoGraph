...
SerializationObjectSharedData::SerializationObjectSharedData(SerializationVersion serialization_version_, const DataTypePtr &dynamic_type_,
                                                             const SerializationPtr &dynamic_serialization_, size_t buckets_)
...
void SerializationObjectSharedData::enumerateStreams(ISerialization::EnumerateStreamsSettings &settings,
                                                     const ISerialization::StreamCallback &callback,
                                                     const ISerialization::SubstreamData &data) const {
  const auto *shared_data_state =
      data.deserialize_state ? checkAndGetState<DeserializeBinaryBulkStateObjectSharedData>(data.deserialize_state) : nullptr;
  ...
  for (size_t bucket = 0; bucket != buckets; ++bucket) {
    ...
    if (serialization_version.value == SerializationVersion::MAP_WITH_BUCKETS) {
      ...
      /// If we add new serialization version in future and forget to implement something, better to get an exception instead of doing
      /// nothing.
      throw Exception(ErrorCodes::NOT_IMPLEMENTED, "enumerateStreams is not implemented for shared data serialization version {}",
...
void SerializationObjectSharedData::serializeBinaryBulkStatePrefix(const IColumn &column,
...
void SerializationObjectSharedData::serializeBinaryBulkWithMultipleStreams(const IColumn &column, size_t offset, size_t limit,
                                                                           ISerialization::SerializeBinaryBulkSettings &settings,
                                                                           ISerialization::SerializeBinaryBulkStatePtr &state) const {
  ...
  if (serialization_version.value == SerializationVersion::MAP) {
    ...
    for (size_t bucket = 0; bucket != buckets; ++bucket) {
      settings.path.push_back(Substream::ObjectSharedDataBucket);
      settings.path.back().object_shared_data_bucket = bucket;
      serialization_map->serializeBinaryBulkWithMultipleStreams(*shared_data_buckets[bucket], 0, 0, settings,
                                                                shared_data_state->bucket_map_states[bucket]);
      settings.path.pop_back();
    }
  } else if (serialization_version.value == SerializationVersion::ADVANCED) {
    size_t end = limit && offset + limit < column.size() ? offset + limit : column.size();
    ...
    for (size_t bucket = 0; bucket != buckets; ++bucket) {
      ...
      Substream structure_stream_type = settings.use_specialized_prefixes_and_suffixes_substreams
                                            ? Substream::ObjectSharedDataStructurePrefix
      ...
      for (const auto &[path, _] : flattened_paths) writeStringBinary(path, *structure_stream);
      ...
      data_serialization_settings.object_and_dynamic_write_statistics =
          ISerialization::SerializeBinaryBulkSettings::ObjectAndDynamicStatisticsMode::NONE;
      data_serialization_settings.stream_mark_getter = [&](const SubstreamPath &) -> MarkInCompressedFile {
        return settings.stream_mark_getter(settings.path);
      };
      ...
      for (const auto &[path, path_column] : flattened_paths) {
        ...
        data_serialization_settings.getter = [&](const SubstreamPath &substream_path) -> WriteBuffer * {
          ...
          paths_substreams.back().push_back(
              ISerialization::getFileNameForStream(NameAndTypePair("", dynamic_type), substream_path, stream_file_name_settings));
      ...
      for (const auto &path_substreams : paths_substreams) {
        ...
        for (const auto &substream : path_substreams) writeStringBinary(substream, *paths_substreams_stream);
      }
      ...
      if (!paths_substreams_metadata_stream)
        throw Exception(ErrorCodes::LOGICAL_ERROR,
                        "Got empty stream for shared data paths substreams metadata in "
                        "SerializationObjectSharedData::serializeBinaryBulkWithMultipleStreams");
    ...
    auto [indexes_column, indexes_type] =
        createPathsIndexes(path_to_index, shared_data_tuple_column.getColumn(0), nested_offset, nested_end);
    ...
    if (nested_limit) SerializationString().serializeBinaryBulk(values_column, *copy_values_stream, nested_offset, nested_limit);
    settings.path.pop_back();
    ...
    throw Exception(ErrorCodes::NOT_IMPLEMENTED,
                    "serializeBinaryBulkWithMultipleStreams is not implemented for shared data serialization version {}",
                    serialization_version.value);
  }
}

void SerializationObjectSharedData::serializeBinaryBulkStateSuffix(ISerialization::SerializeBinaryBulkSettings &settings,
                                                                   ISerialization::SerializeBinaryBulkStatePtr &state) const {
  auto *shared_data_state = checkAndGetState<SerializeBinaryBulkStateObjectSharedData>(state);
  ...
  if (serialization_version.value == SerializationVersion::MAP) {
    ...
    /// nothing.
    throw Exception(ErrorCodes::NOT_IMPLEMENTED,
                    "serializeBinaryBulkStateSuffix is not implemented for shared data serialization version {}",
                    serialization_version.value);
  }
}

void SerializationObjectSharedData::deserializeBinaryBulkStatePrefix(ISerialization::DeserializeBinaryBulkSettings &settings,
  ...
  if (serialization_version.value == SerializationVersion::MAP) {
    ...
    for (size_t bucket = 0; bucket != buckets; ++bucket) {
      ...
      auto *structure_state_concrete =
          checkAndGetState<DeserializeBinaryBulkStateObjectSharedDataStructure>(shared_data_state->bucket_structure_states[bucket]);
    ...
    /// nothing.
    throw Exception(ErrorCodes::NOT_IMPLEMENTED,
                    "deserializeBinaryBulkStatePrefix is not implemented for shared data serialization version {}",
                    serialization_version.value);
  }
...
ISerialization::DeserializeBinaryBulkStatePtr SerializationObjectSharedData::deserializeStructureStatePrefix(
...
void SerializationObjectSharedData::deserializeStructureGranulePrefix(
    ReadBuffer &buf, SerializationObjectSharedData::StructureGranule &structure_granule,
  ...
  if (structure_state.need_all_paths) structure_granule.all_paths.reserve(structure_granule.num_paths);

  /// Read list of paths.
  for (size_t i = 0; i != structure_granule.num_paths; ++i) {
    readStringBinary(path, buf);
    if (structure_state.requested_paths.contains(path) || structure_state.requested_paths_subcolumns.contains(path) ||
        structure_state.checkIfPathMatchesAnyRequestedPrefix(path))
      structure_granule.position_to_requested_path[i] = path;

    if (structure_state.need_all_paths) structure_granule.all_paths.push_back(path);
  }
}

void SerializationObjectSharedData::deserializeStructureGranuleSuffix(ReadBuffer &buf,
...
std::shared_ptr<SerializationObjectSharedData::StructureGranules> SerializationObjectSharedData::deserializeStructure(
    size_t rows_offset, size_t limit, ISerialization::DeserializeBinaryBulkSettings &settings,
    DeserializeBinaryBulkStateObjectSharedDataStructure &structure_state, ISerialization::SubstreamsCache *cache) {
  ...
  if (settings.data_part_type == MergeTreeDataPartType::Compact) {
    ...
    if (structure_granule.num_rows != rows_offset + limit)
      throw Exception(ErrorCodes::LOGICAL_ERROR, "Expected reading a single granule with {} rows, requested {} rows in Compact part",
                      structure_granule.num_rows, rows_offset + limit);
    ...
    if (!structure_suffix_stream) throw Exception(ErrorCodes::LOGICAL_ERROR, "Got empty structure suffix stream for object shared data");
  ...
  /// In Wide part we can read multiple granules together and read only part of last granule.
    ...
    if (!structure_stream) throw Exception(ErrorCodes::LOGICAL_ERROR, "Got empty structure stream for object shared data");

    /// Reset last read granule state if we don't continue reading from it.
    if (!settings.continuous_reading) structure_state.last_granule_structure.clear();
...
std::shared_ptr<SerializationObjectSharedData::PathsInfosGranules> SerializationObjectSharedData::deserializePathsInfos(
    ...
    ISerialization::DeserializeBinaryBulkSettings &settings, ISerialization::SubstreamsCache *cache) {
  ...
  for (const auto &structure_granule : structure_granules) {
    ...
    if (structure_granule.limit == 0 || structure_granule.position_to_requested_path.empty()) continue;
    ...
    if (!settings.seek_stream_to_mark_callback)
      throw Exception(ErrorCodes::LOGICAL_ERROR,
                      "Cannot read paths from object shared data with ADVANCED serialization version because seek_stream_to_mark_callback "
                      "is not initialized");
    ...
    if (need_paths_marks) {
      ...
      if (!paths_marks_stream) throw Exception(ErrorCodes::LOGICAL_ERROR, "Got empty stream for shared data paths marks");
    ...
    if (need_subcolumns_info) {
      ...
      if (!paths_substreams_stream) throw Exception(ErrorCodes::LOGICAL_ERROR, "Got empty stream for shared data paths substreams");

      for (const auto &[_, requested_path] : structure_granule.position_to_requested_path) {
        if (!structure_state.requested_paths_subcolumns.contains(requested_path)) continue;
...
std::shared_ptr<SerializationObjectSharedData::PathsDataGranules> SerializationObjectSharedData::deserializePathsData(
    const SerializationObjectSharedData::StructureGranules &structure_granules, const PathsInfosGranules &paths_infos_granules,
    const SerializationObjectSharedData::DeserializeBinaryBulkStateObjectSharedDataStructure &structure_state,
    ISerialization::DeserializeBinaryBulkSettings &settings, const DataTypePtr &dynamic_type, const SerializationPtr &dynamic_serialization,
  ...
  auto *data_stream = settings.getter(settings.path);
  if (!data_stream) throw Exception(ErrorCodes::LOGICAL_ERROR, "Got empty stream for shared data data");
  ...
  if (!settings.seek_stream_to_mark_callback)
    throw Exception(ErrorCodes::LOGICAL_ERROR,
                    "Cannot read paths from object shared data with ADVANCED serialization version because seek_stream_to_mark_callback is "
                    "not initialized");
  ...
  for (size_t granule = 0; granule != structure_granules.size(); ++granule) {
    ...
    if (!structure_granule.limit || path_to_info.empty()) continue;
    ...
    for (const auto &[_, requested_path] : structure_granule.position_to_requested_path) {
      ...
      if (paths_subcolumns_it != structure_state.requested_paths_subcolumns.end()) {
        ...
        deserialization_settings.seek_stream_to_current_mark_callback = [&](const SubstreamPath &substream_path) {
          ...
          if (it == path_info.substream_to_mark.end())
            throw Exception(ErrorCodes::LOGICAL_ERROR, "Substream {} for path {} is requested but not found in substreams list",
                            stream_name, requested_path);
        ...
        for (auto pos : order) {
          ...
          subcolumns_substream_data[pos].serialization->deserializeBinaryBulkWithMultipleStreams(
              subcolumn, 0, structure_granule.num_rows, deserialization_settings, subcolumns_substream_data[pos].deserialize_state,
              &cache_for_subcolumns);
      ...
      /// Otherwise read the whole path data.
        ...
        dynamic_serialization->deserializeBinaryBulkWithMultipleStreams(dynamic_column, 0, structure_granule.num_rows,
                                                                        deserialization_settings, path_state, nullptr);
...
void SerializationObjectSharedData::deserializeBinaryBulkWithMultipleStreams(ColumnPtr &column, size_t rows_offset, size_t limit,
  ...
  if (serialization_version.value == SerializationVersion::MAP) {
    ...
    if (!insertDataFromSubstreamsCacheIfAny(cache, settings, column)) {
      ...
      serialization_map->deserializeBinaryBulkWithMultipleStreams(column, rows_offset, limit, settings, shared_data_state->map_state,
                                                                  cache);
    ...
    for (size_t bucket = 0; bucket != buckets; ++bucket) {
      ...
      /// If we don't have it in cache, deserialize and put deserialized map in cache.
        ...
        serialization_map->deserializeBinaryBulkWithMultipleStreams(shared_data_buckets[bucket], rows_offset, limit, settings,
                                                                    shared_data_state->bucket_map_states[bucket], cache);
    ...
    if (insertDataFromSubstreamsCacheIfAny(cache, settings, column)) return;
    ...
    if (settings.data_part_type == MergeTreeDataPartType::Compact) {
      ...
      for (size_t bucket = 0; bucket != buckets; ++bucket) {
        ...
        settings.path.push_back(Substream::ObjectSharedDataStructurePrefix);
        auto *structure_prefix_stream = settings.getter(settings.path);
        if (!structure_prefix_stream) return;
        ...
        if (settings.seek_stream_to_current_mark_callback) settings.seek_stream_to_current_mark_callback(settings.path);

        auto *structure_state =
            checkAndGetState<DeserializeBinaryBulkStateObjectSharedDataStructure>(shared_data_state->bucket_structure_states[bucket]);
        StructureGranule structure_granule;
        deserializeStructureGranulePrefix(*structure_prefix_stream, structure_granule, *structure_state);
        ...
        if (structure_granule.num_rows != rows_offset + limit)
          throw Exception(ErrorCodes::LOGICAL_ERROR,
                          "Expected reading a single granule with {} rows, requested {} rows in Compact part in bucket {}",
                          structure_granule.num_rows, rows_offset + limit, bucket);
        ...
        if (!data_stream) throw Exception(ErrorCodes::LOGICAL_ERROR, "Got empty stream for object shared data data");
        ...
        for (size_t i = 0; i != structure_granule.num_paths; ++i) {
          ...
          DeserializeBinaryBulkStatePtr path_state;
          dynamic_serialization->deserializeBinaryBulkStatePrefix(deserialization_settings, path_state, nullptr);
          dynamic_serialization->deserializeBinaryBulkWithMultipleStreams(path_column, structure_granule.num_rows, 0,
                                                                          deserialization_settings, path_state, nullptr);
        }
        ...
        if (!paths_marks_stream) throw Exception(ErrorCodes::LOGICAL_ERROR, "Got empty stream for object shared data paths marks");
        ...
        for (size_t i = 0; i != structure_granule.num_paths; ++i) {
          ...
          for (size_t j = 0; j != num_substreams; ++j) skipStringBinary(*paths_substreams_stream);
        }
      ...
      auto [skipped_nested_rows, nested_limit] =
          SerializationArray::deserializeOffsetsBinaryBulkAndGetNestedOffsetAndLimit(offsets_column, rows_offset, limit, settings, cache);
      settings.path.pop_back();

      /// Read paths indexes.
      settings.path.push_back(Substream::ObjectSharedDataCopyPathsIndexes);
      auto *indexes_stream = settings.getter(settings.path);
      if (!indexes_stream) throw Exception(ErrorCodes::LOGICAL_ERROR, "Got empty stream for shared data copy indexes");
      ...
      if (!values_stream) throw Exception(ErrorCodes::LOGICAL_ERROR, "Got empty stream for shared data copy values");
    ...
    /// In Wide part we can read multiple granules together and read only part of last granule.
      ...
      if (!settings.continuous_reading) shared_data_state->last_incomplete_granule_offset = 0;
      ...
      for (size_t bucket = 0; bucket != buckets; ++bucket) {
        ...
        if (bucket == 0) {
          ...
          for (size_t granule = 0; granule != structure_granules->size(); ++granule) {
            ...
            if (granule == 0) granules_offsets.back() -= shared_data_state->last_incomplete_granule_offset;
        ...
        for (size_t granule = 0; granule != structure_granules->size(); ++granule)
          granules_paths[granule].insert(granules_paths[granule].end(), (*structure_granules)[granule].all_paths.begin(),
                                         (*structure_granules)[granule].all_paths.end());
      ...
      if (!indexes_stream) throw Exception(ErrorCodes::LOGICAL_ERROR, "Got empty stream for object shared data copy indexes");
      ...
      for (size_t granule = 0; granule != granules_paths.size(); ++granule) {
        ...
        size_t nested_offset = offsets[offsets_current_granule_start + granules_offsets[granule] - ssize_t(1)] -
                               offsets[offsets_current_granule_start - ssize_t(1)];
        /// Calculate how many rows should be read in this granule.
        size_t nested_limit = offsets[offsets_current_granule_start + granules_offsets[granule] + granules_limits[granule] - ssize_t(1)] -
      ...
      if (rows_offset) {
        ...
        for (auto i = prev_offset_size; i + rows_offset < offsets.size(); ++i) offsets[i] = offsets[i + rows_offset] - nested_offset;
