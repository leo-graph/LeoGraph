...
bool SerializationDecimal<T>::tryReadText(T &x, ReadBuffer &istr, UInt32 precision, UInt32 scale, bool csv) {
  ...
  if (csv) {
    if (!tryReadCSVDecimalText(istr, x, precision, unread_scale)) return false;
  } else {
    if (!tryReadDecimalText(istr, x, precision, unread_scale)) return false;
  }

  if (common::mulOverflow(x.value, DecimalUtils::scaleMultiplier<T>(unread_scale), x.value)) return false;
...
template <typename T>
void SerializationDecimal<T>::serializeText(const IColumn &column, size_t row_num, WriteBuffer &ostr,
...
void SerializationDecimal<T>::deserializeText(IColumn &column, ReadBuffer &istr, const FormatSettings &settings, bool whole) const {
  ...
  if (whole && !istr.eof()) ISerialization::throwUnexpectedDataAfterParsedValue(column, istr, settings, "Decimal");
}
...
bool SerializationDecimal<T>::tryDeserializeTextCSV(IColumn &column, ReadBuffer &istr, const FormatSettings &) const {
  T x;
  if (!tryReadText(x, istr, true)) return false;
  assert_cast<ColumnType &>(column).getData().push_back(x);
  return true;
}

template <typename T>
void SerializationDecimal<T>::serializeTextJSON(const IColumn &column, size_t row_num, WriteBuffer &ostr,
                                                const FormatSettings &settings) const {
  if (settings.json.quote_decimals) writeChar('"', ostr);
...
void SerializationDecimal<T>::deserializeTextJSON(IColumn &column, ReadBuffer &istr, const FormatSettings &settings) const {
  ...
  if (have_quotes) assertChar('"', istr);
}
...
bool SerializationDecimal<T>::tryDeserializeTextJSON(IColumn &column, ReadBuffer &istr, const FormatSettings &) const {
  bool have_quotes = checkChar('"', istr);
  T x;
  if (!tryReadText(x, istr) || (have_quotes && !checkChar('"', istr))) return false;
