...
static bool operator==(const IDataType& left, const IDataType& right) { return left.equals(right); }
...
static auto typesFromString(const std::string& str) {
  ...
  while (data_types_stream >> data_type) data_types.push_back(typeFromString(data_type));
...
INSTANTIATE_TEST_SUITE_P(data_type, MostSubtypeTest,
                         ::testing::ValuesIn(std::initializer_list<TypesTestCase>{
                             {"", "Nothing"},
                             {"Nothing", "Nothing"},
