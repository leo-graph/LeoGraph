...
GTEST_TEST(DataTypesBinaryEncoding, EncodeAndDecode) {
  ...
  check(std::make_shared<DataTypeFunction>(DataTypes{std::make_shared<DataTypeInt8>(), std::make_shared<DataTypeDate>(),
  ...
  AggregateFunctionPtr function =
      AggregateFunctionFactory::instance().get("quantiles", NullsAction::EMPTY, argument_types, parameters, properties);
  ...
  check(DataTypeFactory::instance().get(
      "Tuple(Map(LowCardinality(String), Array(AggregateFunction(2, quantiles(0.1, 0.2), Float32))), Array(Array(Tuple(UInt32, Tuple(a "
      "Map(String, String), b Nullable(Date), c Variant(Tuple(g String, d Array(UInt32)), Date, Map(String, String)))))))"));
  check(DataTypeFactory::instance().get("JSON"));
  check(DataTypeFactory::instance().get("JSON(max_dynamic_paths=10)"));
  check(
      DataTypeFactory::instance().get("JSON(max_dynamic_paths=10, max_dynamic_types=10, a.b.c UInt32, SKIP a.c, b.g String, SKIP l.d.f)"));
}
