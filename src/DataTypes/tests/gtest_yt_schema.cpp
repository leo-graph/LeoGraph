...
std::string createSimpleTypeJson(const std::string& simple_type, bool required) {
  return fmt::format(R"({{"name": "id", "type": "{}", "required": {}}})", simple_type, required);
}

std::string createComplexTypeJson(const std::string& type_v3) {
  return fmt::format(R"({{"name": "id", "type": "any", "required": false, "type_v3": {}}})", type_v3);
}
...
TEST(YTDataType, CheckStruct) {
  {  // Struct
    ...
    json->set("type_v3", s);
    ASSERT_TRUE(checkColumnType(json, CH_TYPE("Tuple(key Int32, value String)")));
  }
}
