...
enum class ArgumentKind : uint8_t { Optional, Mandatory };

PreformattedMessage getExceptionMessage(const String& message, size_t argument_index, const char* argument_name,
                                        const std::string& context_data_type_name, Field::Types::Which field_type) {
  return PreformattedMessage::create("Parameter #{} '{}' for {}{}, expected {} literal", argument_index, argument_name,
                                     context_data_type_name, message, field_type);
}

template <typename T, ArgumentKind Kind>
std::conditional_t<Kind == ArgumentKind::Optional, std::optional<T>, T> getArgument(const ASTPtr& arguments, size_t argument_index,
                                                                                    const char* argument_name [[maybe_unused]],
  ...
  if (!arguments || arguments->children.size() <= argument_index || !(argument = arguments->children[argument_index]->as<ASTLiteral>()) ||
      argument->value.getType() != field_type) {
    if constexpr (Kind == ArgumentKind::Optional)
      return {};
    else {
      if (argument && argument->value.getType() != field_type)
        throw Exception(getExceptionMessage(fmt::format(" has wrong type: {}", argument->value.getTypeName()), argument_index,
                                            argument_name, context_data_type_name, field_type),
...
static DataTypePtr create(const ASTPtr& arguments) {
  if (!arguments || arguments->children.empty()) return std::make_shared<DataTypeDateTime>();
  ...
  if (scale && scale.value() != 0) return std::make_shared<DataTypeDateTime64>(scale.value(), timezone.value_or(String{}));
...
static DataTypePtr create64(const ASTPtr& arguments) {
  if (!arguments || arguments->children.empty()) return std::make_shared<DataTypeDateTime64>(DataTypeDateTime64::default_scale);
...
static DataTypePtr createTime(const ASTPtr& arguments) {
  if (!arguments || arguments->children.empty()) return std::make_shared<DataTypeTime>();
  ...
  if (timezone && !timezone->empty())
    throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT, "Specifying timezone for Time type is not allowed");
  ...
  if (scale && scale.value() != 0) return std::make_shared<DataTypeTime64>(scale.value());

  return std::make_shared<DataTypeTime>();
}

static DataTypePtr createTime64(const ASTPtr& arguments) {
  if (!arguments || arguments->children.empty()) return std::make_shared<DataTypeTime64>(DataTypeTime64::default_scale);

  if (arguments->children.size() > 2)
    throw Exception(ErrorCodes::NUMBER_OF_ARGUMENTS_DOESNT_MATCH, "Time64 data type can optionally have only one argument - scale");
  ...
  if (timezone && !timezone->empty())
    throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT, "Specifying timezone for Time64 type is not allowed");
