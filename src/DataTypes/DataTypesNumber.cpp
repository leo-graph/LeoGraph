    ...
    if (std::is_integral_v<T>) {
      ...
      if (arguments->children.size() > 2)
        throw Exception(ErrorCodes::NUMBER_OF_ARGUMENTS_DOESNT_MATCH,
                        ...
                        "of digits and number of digits following the decimal point",
                        TypeName<T>);
    }
  }
...
DataTypePtr getSmallestIndexesType(size_t num_indexes) {
  if (num_indexes <= std::numeric_limits<UInt8>::max() + 1ull) return std::make_shared<DataTypeUInt8>();
  if (num_indexes <= std::numeric_limits<UInt16>::max() + 1ull) return std::make_shared<DataTypeUInt16>();
  if (num_indexes <= std::numeric_limits<UInt32>::max() + 1ull) return std::make_shared<DataTypeUInt32>();
