...
#define DISPATCH(TYPE) \
  if (which.idx == TypeIndex::TYPE) return f(TYPE());
...
class SortedLookupVector : public SortedLookupVectorBase {
  struct Entry {
    ...
    Entry() = delete;
    Entry(TKey value_, uint32_t row_ref_index_) : value(value_), row_ref_index(row_ref_index_) {}
  };

  struct LessEntryOperator {
    ALWAYS_INLINE bool operator()(const Entry &lhs, const Entry &rhs) const { return lhs.value < rhs.value; }
  };
...
#undef BOUND_ITERATION
 ...
 private:
  ...
  void sort() {
    if (sorted.load(std::memory_order_acquire)) return;
...
ColumnsInfo::ColumnsInfo(Columns &&columns_) : columns(std::move(columns_)) {
  ...
  for (size_t i = 0; i != columns.size(); ++i) replicated_columns[i] = typeid_cast<const ColumnReplicated *>(columns[i].get());
}
