...
bool NgramsTokenizer::nextInString(const char* data, size_t length, size_t& __restrict pos, size_t& __restrict token_start,
...
bool NgramsTokenizer::nextInStringLike(const char* data, size_t length, size_t& pos, String& token) const {
  ...
  for (size_t i = pos; i < length;) {
    if (escaped && (data[i] == '%' || data[i] == '_' || data[i] == '\\')) {
      ...
      /// The sequence length is determined by the first character, but we should always check it does not go beyond the buffer length.
      for (size_t j = 0; j < sz && i + j < length; ++j) token += data[i + j];
      i += sz;
      ++code_points;
      escaped = false;
    }
...
bool SplitByNonAlphaTokenizer::nextInString(const char* data, size_t length, size_t& __restrict pos, size_t& __restrict token_start,
                                            size_t& __restrict token_length) const {
  token_start = pos;
  token_length = 0;
...
bool SplitByNonAlphaTokenizer::nextInStringLike(const char* data, size_t length, size_t& pos, String& token) const {
  ...
  while (pos < length) {
    if (!escaped && (data[pos] == '%' || data[pos] == '_')) {
      ...
      if (!bad_token && !token.empty()) return true;
...
void SplitByNonAlphaTokenizer::substringToBloomFilter(const char* data, size_t length, BloomFilter& bloom_filter, bool is_prefix,
  ...
  while (cur < length && nextInString(data, length, cur, token_start, token_len)) {
    ...
    if ((token_start > 0 || is_prefix) && (token_start + token_len < length || is_suffix)) bloom_filter.add(data + token_start, token_len);
  }
}

void SplitByNonAlphaTokenizer::substringToTokens(const char* data, size_t length, std::vector<String>& tokens, bool is_prefix,
                                                 bool is_suffix) const {
  size_t cur = 0;
  size_t token_start = 0;
  size_t token_len = 0;
  ...
  while (cur < length && nextInString(data, length, cur, token_start, token_len)) {
    /// In order to avoid adding incomplete tokens, first token is ignored unless substring is prefix and last token is ignored, unless
    /// substring is suffix. Ex: If we want to match row "Service is not ready", and substring is "Serv" or "eady", we don't want to add
    /// either of these substrings as tokens since they will not match any of the real tokens. However if our token string is "Service " or
    /// " not ", we want to add these full tokens to our tokens vector.
...
bool SplitByStringTokenizer::nextInString(const char* data, size_t length, size_t& pos, size_t& token_start, size_t& token_length) const {
  ...
  while (i < length && startsWithSeparator(data, length, i, separators, matched_separators)) i += matched_separators.size();
  ...
  size_t start = i;
  while (i < length && !startsWithSeparator(data, length, i, separators, matched_separators)) ++i;
...
bool SparseGramsTokenizer::nextInString(const char* data, size_t length, size_t& __restrict pos, size_t& __restrict token_start,
                                        size_t& __restrict token_length) const {
  ...
  while (true) {
    ...
    for (size_t i = next_begin - data; i < static_cast<size_t>(next_end - data);) {
      ...
      if (data[i] == '%' || data[i] == '_' || data[i] == '\\') {
        ...
        const size_t sz = UTF8::seqLength(static_cast<UInt8>(data[i]));
        /// The sequence length is determined by the first character, but we should always check it does not go beyond the buffer length.
        for (size_t j = 0; j < sz && i + j < length; ++j) token.push_back(data[i + j]);
        i += sz;
      }
    }
    if (match_substring) {
...
std::vector<String> SparseGramsTokenizer::compactTokens(const std::vector<String>& tokens) const {
  ...
  std::ranges::sort(sorted_tokens,
                    [](const auto& lhs, const auto& rhs) { return lhs.size() > rhs.size(); });  /// NOLINT(clang-analyzer-cplusplus.Move)
  ...
  for (const auto& token : sorted_tokens) {
    ...
    if (!is_covered) result.insert(token);
  }
...
String SparseGramsTokenizer::getDescription() const {
  ...
  if (min_cutoff_length.has_value()) result += fmt::format(", {}", *min_cutoff_length);
  return result + ")";
}

void forEachTokenToBloomFilter(const ITokenizer& tokenizer, const char* data, size_t length, BloomFilter& bloom_filter) {
  forEachToken(tokenizer, data, length, [&](const char* token_start, size_t token_length) {
