...
struct AggregatedDataVariants : private boost::noncopyable {
   ...
   *   are written to ColumnAggregateFunction; ColumnAggregateFunction "acquires ownership" of them, that is - calls `destroy` in its
   * destructor.
   ...
   *
   * PS. This can be corrected by making a pool that knows about which states of aggregate functions and in which order are put in it, and
   * knows how to destroy them. But this can hardly be done simply because it is planned to put variable-length strings into the same pool.
  ...
  std::unique_ptr<AggregationMethodFixedStringNoCache<AggregatedDataWithNullableShortStringKeyTwoLevel, true>>
      nullable_key_fixed_string_two_level;
  ...
  std::unique_ptr<AggregationMethodSingleLowCardinalityColumn<AggregationMethodOneNumber<UInt8, AggregatedDataWithNullableUInt8Key, false>>>
