...
class ViewErrorsRegistry {
  ...
  class SetOnce {
   ...
   public:
    bool set(T value_) {
      if (is_set.test_and_set()) return false;

      value = value_;
      return true;
    }
    bool isSet() const { return is_set.test(); }
    T get() const {
      if (is_set.test()) return value;
      return T{};
    }
  };
 ...
 public:
  class ViewErrors {
   ...
   public:
    ...
    void registerFinalizer() { finalizers_count.fetch_add(1); }

    bool needLogQueryView() { return finalizers_count.fetch_sub(1) == 1; }
  };

 private:
  using MapIdViewExceptions =
      std::unordered_map<StorageIDMaybeEmpty, ViewErrors, StorageID::DatabaseAndTableNameHash, StorageID::DatabaseAndTableNameEqual>;
  MapIdViewExceptions view_errors;

 public:
  SetOnce<std::exception_ptr> global_exception;

  void init(const StorageIDMaybeEmpty &view_id) { view_errors.try_emplace(view_id); }

  const ViewErrors &getErrors(const StorageIDMaybeEmpty &view_id) const { return view_errors.at(view_id); }

  ViewErrors &getErrors(const StorageIDMaybeEmpty &view_id) { return view_errors.at(view_id); }
  ...
  std::exception_ptr getFinalError(const StorageIDMaybeEmpty &view_id, bool ignore_global) const {
    ...
    if (auto e = errors.current_exception.get()) return e;
    if (auto e = errors.external_exception.get()) return e;
    if (!ignore_global) return global_exception.get();
    return nullptr;
  }
};
...
class PushingToWindowViewSink final : public SinkToStorage {
 public:
  PushingToWindowViewSink(SharedHeader header, StorageWindowView &window_view_, ContextPtr context_)
      : SinkToStorage(header), window_view(window_view_), context(std::move(context_)) {}
  String getName() const override { return "PushingToWindowViewSink"; }
  void consume(Chunk &chunk) override {
    Progress local_progress(chunk.getNumRows(), chunk.bytes(), 0);
    StorageWindowView::writeIntoWindowView(window_view, getHeader().cloneWithColumns(chunk.getColumns()), std::move(chunk.getChunkInfos()),
                                           context);

    if (auto process = context->getProcessListElement()) process->updateProgressIn(local_progress);
...
class BeginingViewsTransform final : public ISimpleTransform {
 public:
  explicit BeginingViewsTransform(SharedHeader header) : ISimpleTransform(header, header, false) {}
  ...
  void transform(std::exception_ptr &e) override { e = std::make_exception_ptr(ExternalException{e}); }
};

class FinalizingViewsTransform final : public IProcessor {
  static InputPorts initPorts(std::vector<Block> headers) {
    InputPorts res;
    for (auto &header : headers) res.emplace_back(std::move(header));
    return res;
  }

 public:
  FinalizingViewsTransform(std::vector<Block> headers, std::vector<StorageID> views,
                           InsertDependenciesBuilder::ConstPtr insert_dependencies_, ViewErrorsRegistryPtr views_error_registry_)
  ...
  Status prepare() override {
    if (output.isFinished()) throw Exception(ErrorCodes::LOGICAL_ERROR, "Cannot finalize views because output port is finished");
    ...
    for (auto &input : inputs) {
      ...
      input.setNeeded();
      if (!input.hasData()) continue;

      auto data = input.pullData();

      if (!data.exception) continue;
      ...
      if (!view_errors.current_exception.set(exception_with_storage)) continue;
      ...
      tryLogException(exception_with_storage, getLogger("FinalizingViewsTransform"),
  ...
  void writeViewLogs() {
    for (auto &status : statuses) {
      ...
      if (!errors.needLogQueryView()) continue;
 ...
 private:
  struct ViewStatus {
    explicit ViewStatus(StorageID view_id_) : view_id(std::move(view_id_)) {}
...
DB::ConstraintsDescription buildConstraints(StorageMetadataPtr metadata, StoragePtr storage) {
  ...
  if (storage_merge_tree &&
    ...
    sign_column_check_constraint->set(sign_column_check_constraint->expr,
                                      makeASTOperator("in", std::move(sign_column_ast), std::move(valid_values_ast)));
...
class ExecutingInnerQueryFromViewTransform final : public ExceptionKeepingTransform {
 public:
  ExecutingInnerQueryFromViewTransform(SharedHeader input_header, SharedHeader output_header, ASTPtr select_query_, StorageID source_id_,
                                       StoragePtr source_storage_, StorageMetadataPtr source_metadata_, StorageID view_id_,
                                       StoragePtr view_storage_, StorageMetadataPtr view_metadata_, StorageID inner_id_,
                                       StoragePtr inner_storage_, StorageMetadataPtr inner_metadata_, ContextPtr context_)
 ...
 private:
  ...
  struct State {
    ...
    explicit State(QueryPipeline pipeline_) : pipeline(std::move(pipeline_)), executor(pipeline) {}
  };
  ...
  QueryPipeline process(Block data_block, Chunk::ChunkInfoCollection &&chunk_infos) {
    ...
    local_context->addViewSource(std::make_shared<StorageValues>(source_id, source_metadata->getColumns(), std::move(data_block),
                                                                 *source_storage->getVirtualsPtr()));

    QueryPipelineBuilder pipeline;

    if (local_context->getSettingsRef()[Setting::allow_experimental_analyzer]) {
      InterpreterSelectQueryAnalyzer interpreter(select_query, local_context, SelectQueryOptions().ignoreAccessCheck(),
    ...
    auto construct_columns_to_convert = [](const ColumnsWithTypeAndName &src, const ColumnsWithTypeAndName &dst) {
      ...
      for (size_t i = 0; i < dst.size(); ++i) name_to_index_dst_map[dst[i].name] = i;
    ...
    auto build_conversion = [&](const Block &input, StorageMetadataPtr result_metadata) {
      ...
      auto to_convert =
          construct_columns_to_convert(input.getColumnsWithTypeAndName(), result_metadata->getSampleBlock().getColumnsWithTypeAndName());

      auto converting_types_dag = ActionsDAG::makeConvertingActions(input.getColumnsWithTypeAndName(), to_convert,
                                                                    ActionsDAG::MatchColumnsMode::Name, local_context);

      auto adding_missing_defaults_dag = addMissingDefaults(Block(to_convert), result_metadata->getSampleBlock().getNamesAndTypesList(),
                                                            result_metadata->getColumns(), local_context, insert_null_as_default);

      auto extracting_subcolumns_dag =
          createSubcolumnsExtractionActions(Block(to_convert), adding_missing_defaults_dag.getRequiredColumnsNames(), local_context);

      return ActionsDAG::merge(std::move(converting_types_dag),
    ...
    pipeline.addTransform(std::make_shared<ExpressionTransform>(
        pipeline.getSharedHeader(), std::make_shared<ExpressionActions>(build_conversion(pipeline.getHeader(), inner_metadata))));
    ...
    pipeline.addTransform(std::make_shared<SquashingTransform>(pipeline.getSharedHeader(),
...
InsertDependenciesBuilder::InsertDependenciesBuilder(StoragePtr table, ASTPtr query, SharedHeader insert_header, bool async_insert_,
                                                     bool skip_destination_table_, size_t max_insert_threads, ContextPtr context)
  ...
  deduplicate_blocks_in_dependent_materialized_views =
      deduplicate_blocks && settings[Setting::deduplicate_blocks_in_dependent_materialized_views];
  ...
  squash_parallel_inserts =
      !deduplicate_blocks_in_dependent_materialized_views && settings[Setting::materialized_views_squash_parallel_inserts];
  ...
  auto all_sinks_support_parallel_insert =
      std::ranges::all_of(storages, [&](auto storage) { return isView(storage.first) || storage.second->supportsParallelInsert(); });
...
std::vector<Chain> InsertDependenciesBuilder::createChainWithDependenciesForAllStreams() const {
  ...
  std::unordered_map<StorageIDMaybeEmpty, SquashingTransformContext, StorageID::DatabaseAndTableNameHash,
                     StorageID::DatabaseAndTableNameEqual>
      views_to_squashing_context;

  insert_chains.reserve(sink_stream_size);

  if (squash_parallel_inserts) squashing_processor_maps.reserve(sink_stream_size);
  ...
  if (!squash_parallel_inserts || !has_squashing_transforms) return insert_chains;
  ...
  for (auto &&[chain, squashing_processors_for_chain] : std::views::zip(insert_chains, squashing_processor_maps)) {
    ...
    for (const auto &[view_id, apply_squashing_processors] : squashing_processors_for_chain) {
      ...
      if (!squashing_context.squashing_transform_added) {
        ...
        auto &plan_squashing_transform = squashing_processors_list.emplace_back(std::make_shared<PlanSquashingTransform>(
            output_header, table_prefers_large_blocks ? settings[Setting::min_insert_block_size_rows] : settings[Setting::max_block_size],
...
Chain InsertDependenciesBuilder::createRedefineDeduplicationInfoWithDataHashTransformChain() const {
  ...
  if (dependent_views_ids.empty()) return {};
...
Chain InsertDependenciesBuilder::createChainWithDependencies() const {
  ...
  // When *Log storages push data to the dependent views, then `skip_destination_table` is true, data is pushed to the views only, not to
  ...
  if (skip_destination_table && result.empty())
    throw Exception(ErrorCodes::DEPENDENCIES_NOT_FOUND, "Table '{}' doesn't have any dependencies.", init_table_id);
...
std::pair<ContextPtr, ContextPtr> InsertDependenciesBuilder::createSelectInsertContext(const DependencyPath &path) {
  ...
  if (!deduplicate_blocks_in_dependent_materialized_views) insert_context->setSetting("insert_deduplicate", Field{false});

  const auto &insert_settings = insert_context->getSettingsRef();
  // Separate min_insert_block_size_rows/min_insert_block_size_bytes for children
  if (insert_settings[Setting::min_insert_block_size_rows_for_materialized_views])
    insert_context->setSetting("min_insert_block_size_rows",
                               insert_settings[Setting::min_insert_block_size_rows_for_materialized_views].value);
  if (insert_settings[Setting::min_insert_block_size_bytes_for_materialized_views])
    insert_context->setSetting("min_insert_block_size_bytes",
                               insert_settings[Setting::min_insert_block_size_bytes_for_materialized_views].value);
...
String InsertDependenciesBuilder::debugTree() const {
  ...
  std::function<void(StorageIDMaybeEmpty)> visit = [&](StorageIDMaybeEmpty id) {
    ...
    if (isView(id)) {
      if (inner_tables.contains(id)) visit(inner_tables.at(id));
    } else {
      auto view_id = path.parent(1);
      chassert(isView(view_id));
      for (const auto &child_id : dependent_views.at(view_id)) visit(child_id);
    }
  };
...
bool InsertDependenciesBuilder::observePath(const DependencyPath &path) {
  ...
  auto lock =
      storage ? storage->tryLockForShare(init_context->getInitialQueryId(), init_context->getSettingsRef()[Setting::lock_acquire_timeout])
              : nullptr;
  if (!lock) {
    if (current == init_table_id) throw Exception(ErrorCodes::UNKNOWN_TABLE, "Table '{}' doesn't exists.", init_table_id);

    if (isView(parent)) {
      if (parent == init_table_id)
        throw Exception(ErrorCodes::UNKNOWN_TABLE, "Target table '{}' of view '{}' doesn't exists.", current, init_table_id);
  ...
  if (auto *materialized_view = dynamic_cast<StorageMaterializedView *>(storage.get())) {
    ...
    if (select_table_id != parent) {
      ...
      LOG_INFO(logger, "Table '{}' is not a source for view '{}' anymore, current source is '{}'", parent, current, select_table_id);
      return false;
    }
    ...
    if (init_context->hasQueryContext()) init_context->getQueryContext()->addViewAccessInfo(current.getFullTableName());
    ...
    if (init_context->hasQueryContext()) init_context->getQueryContext()->addQueryAccessInfo(current, /*column_names=*/{});
...
void InsertDependenciesBuilder::collectAllDependencies() {
  ...
  std::function<void(StorageIDMaybeEmpty)> expand = [&](StorageIDMaybeEmpty id) {
    ...
    try {
      if (!observePath(path)) return;
    } catch (...) {
      if (!materialized_views_ignore_errors) throw;

      if (id == init_table_id) throw;

      auto view_id = isView(id) ? id : path.parent(1);

      if (view_id == init_table_id) throw;

      auto exception = addStorageToException(std::current_exception(), view_id);
      tryLogException(exception, logger,
                      "Cannot push to the storage. Error is ignored because the setting materialized_views_ignore_errors is enabled.",
                      LogsLevel::warning);
    ...
    if (isView(id)) {
      ...
      if (!inner_table.empty() && inner_table != id) expand(inner_table);
    } else {
      // Destination tables for StorageMaterializedView does not have id in inner_tables
      for (auto &child : DatabaseCatalog::instance().getDependentViews(id)) expand(child);
    }
  };
...
Chain InsertDependenciesBuilder::createSelect(StorageIDMaybeEmpty view_id) const {
  ...
  if (select_context->getSettingsRef()[Setting::use_async_executor_for_materialized_views]) {
    auto executing_inner_query = std::make_shared<ExecutingInnerQueryFromViewTransform<PullingAsyncPipelineExecutor>>(
        input_header, output_header, select_query, source_table_id, storages.at(source_table_id), metadata_snapshots.at(source_table_id),
        view_id, storages.at(view_id), metadata_snapshots.at(view_id), inner_table_id, inner_storage, metadata_snapshots.at(inner_table_id),
        select_context);
...
Chain InsertDependenciesBuilder::createPreSink(StorageIDMaybeEmpty view_id) const {
  ...
  auto adding_missing_defaults_dag = addMissingDefaults(*input_headers.at(view_id), output_header->getNamesAndTypesList(),
                                                        inner_metadata->getColumns(), insert_context, insert_null_as_default);

  auto extracting_subcolumns_dag =
      createSubcolumnsExtractionActions(*input_headers.at(view_id), adding_missing_defaults_dag.getRequiredColumnsNames(), insert_context);
  ...
  result.addSink(
      std::make_shared<ConvertingTransform>(input_headers.at(view_id), std::make_shared<ExpressionActions>(std::move(merged_dag))));
...
Chain InsertDependenciesBuilder::createSink(StorageIDMaybeEmpty view_id) const {
  ...
  if (!inner_storage->supportsSparseSerialization()) result.addSink(std::make_shared<RemovingSparseTransform>(header));
  ...
  if (auto *window_view = dynamic_cast<StorageWindowView *>(inner_storage.get())) {
    auto sink = std::make_shared<PushingToWindowViewSink>(std::make_shared<const Block>(window_view->getInputHeader()), *window_view,
                                                          insert_context);
...
Chain InsertDependenciesBuilder::createPostSink(StorageIDMaybeEmpty view_id) const {
  ...
  auto finalizing_views = std::make_shared<FinalizingViewsTransform>(std::move(output_view_chains_headers), dependent_views_ids,
                                                                     shared_from_this(), views_error_registry);
...
String getCleanQueryAst(const ASTPtr q, ContextPtr context) {
  ...
  if (auto masker = SensitiveDataMasker::getInstance()) masker->wipeSensitiveData(res);
...
void InsertDependenciesBuilder::logQueryView(StorageID view_id, std::exception_ptr exception, bool before_start) const {
  const auto &settings = init_context->getSettingsRef();
  if (!view_id || !settings[Setting::log_queries] || !settings[Setting::log_query_views]) return;

  auto event_status = getQueryViewStatus(exception, before_start);
  if (event_status < settings[Setting::log_queries_min_type]) return;

  if (!thread_groups.contains(view_id) || !view_types.contains(view_id) || !inner_tables.contains(view_id)) return;

  const auto &thread_group = thread_groups.at(view_id);
  if (!thread_group) return;
  ...
  if (min_query_duration && elapsed_ms <= min_query_duration) return;
  ...
  auto profile_counters =
      std::make_shared<ProfileEvents::Counters::Snapshot>(thread_group->performance_counters.getPartiallyAtomicSnapshot());
  ...
  if (settings[Setting::log_profile_events] != 0) element.profile_counters = std::move(profile_counters);
  ...
  if (exception) {
    ...
    if (settings[Setting::calculate_text_stack_trace]) element.stack_trace = getExceptionStackTraceString(exception);
  }
...
void InsertDependenciesBuilder::DependencyPath::pushBack(StorageIDMaybeEmpty id) {
  if (visited.contains(id))
    throw Exception(ErrorCodes::TOO_DEEP_RECURSION, "Dependencies of the table {} are cyclic. Dependencies {} are pointing to {}.",
                    path.front(), debugInfo(), id);
...
String InsertDependenciesBuilder::DependencyPath::debugInfo() const { return fmt::format("{}", fmt::join(path, " -> ")); }
...
QueryViewsLogElement::ViewStatus InsertDependenciesBuilder::getQueryViewStatus(std::exception_ptr exception, bool before_start) {
  if (before_start) return QueryViewsLogElement::ViewStatus::EXCEPTION_BEFORE_START;

  if (exception) return QueryViewsLogElement::ViewStatus::EXCEPTION_WHILE_PROCESSING;

  return QueryViewsLogElement::ViewStatus::QUERY_FINISH;
}

bool InsertDependenciesBuilder::isViewsInvolved() const { return isView(init_table_id) || !dependent_views.at(root_view).empty(); }
...
bool InsertDependenciesBuilder::isView(StorageIDMaybeEmpty id) const { return inner_tables.contains(id); }

Chain InsertDependenciesBuilder::createRetry(const std::vector<StorageIDMaybeEmpty> &path, StorageIDMaybeEmpty start_from,
  ...
  result.addSink(std::make_shared<SelectPartitionTransform>(partition, metadata_snapshots.at(inner_tables.at(view_id)),
                                                            insert_contexts.at(view_id), output_headers.at(view_id)));
