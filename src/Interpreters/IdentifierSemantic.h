...
struct IdentifierSemantic {
  ...
  static std::optional<size_t> chooseTable(const ASTIdentifier &, const TablesWithColumns &tables, bool allow_ambiguous = false);
  static std::optional<size_t> chooseTableColumnMatch(const ASTIdentifier &, const TablesWithColumns &tables, bool allow_ambiguous = false);
  ...
  static std::optional<size_t> getIdentsMembership(ASTPtr ast, const std::vector<TableWithColumnNamesAndTypes> &tables,
