...
AddIndexConstraintsOptimizer::AddIndexConstraintsOptimizer(const StorageMetadataPtr& metadata_snapshot_)
...
bool onlyIndexColumns(const ASTPtr& ast, const std::unordered_set<std::string_view>& primary_key_set) {
  ...
  if (identifier && !primary_key_set.contains(identifier->name())) return false;
  for (auto& child : ast->children)
    if (!onlyIndexColumns(child, primary_key_set)) return false;
  return true;
}
...
bool onlyConstants(const ASTPtr& ast) {
  ...
  for (auto& child : ast->children)
    if (!onlyConstants(child)) return false;
  return true;
}
...
bool canBeSequence(const ComparisonGraphCompareResult left, const ComparisonGraphCompareResult right) {
  ...
  if (left == CR::UNKNOWN || right == CR::UNKNOWN || left == CR::NOT_EQUAL || right == CR::NOT_EQUAL) return false;
  if ((left == CR::GREATER || left == CR::GREATER_OR_EQUAL) && (right == CR::LESS || right == CR::LESS_OR_EQUAL)) return false;
  if ((right == CR::GREATER || right == CR::GREATER_OR_EQUAL) && (left == CR::LESS || left == CR::LESS_OR_EQUAL)) return false;
  return true;
}

ComparisonGraphCompareResult mostStrict(const ComparisonGraphCompareResult left, const ComparisonGraphCompareResult right) {
  using CR = ComparisonGraphCompareResult;
  if (left == CR::LESS || left == CR::GREATER) return left;
  if (right == CR::LESS || right == CR::GREATER) return right;
  if (left == CR::LESS_OR_EQUAL || left == CR::GREATER_OR_EQUAL) return left;
  if (right == CR::LESS_OR_EQUAL || right == CR::GREATER_OR_EQUAL) return right;
  if (left == CR::EQUAL) return left;
  if (right == CR::EQUAL) return right;
  return CR::UNKNOWN;
}
...
CNFQuery::OrGroup createIndexHintGroup(const CNFQuery::OrGroup& group, const ComparisonGraph<ASTPtr>& graph,
  ...
  for (const auto& atom : group) {
    ...
    if (func && func->arguments->children.size() == 2 && getRelationMap().contains(func->name)) {
      auto check_and_insert = [&](const size_t index, const ComparisonGraphCompareResult need_result) {
        if (!onlyConstants(func->arguments->children[1 - index])) return false;
...
void AddIndexConstraintsOptimizer::perform(CNFQuery& cnf_query) {
  ...
  for (const auto& vertex : graph.getVertices())
    for (const auto& ast : vertex)
      if (onlyIndexColumns(ast, primary_key_set)) primary_key_only_asts.push_back(ast);
  ...
  cnf_query.iterateGroups([&](const auto& or_group) {
    ...
    if (!add_group.empty()) and_group.emplace(std::move(add_group));
  });

  if (!and_group.empty()) {
    CNFQuery::OrGroup new_or_group;
    new_or_group.insert(
        CNFQueryAtomicFormula{false, makeASTFunction("indexHint", TreeCNFConverter::fromCNF(CNFQuery(std::move(and_group))))});
