...
BlockIO InterpreterAlterQuery::executeToTable(const ASTAlterQuery &alter) {
  ...
  if (!table_id) throw Exception(ErrorCodes::UNKNOWN_DATABASE, "Database {} does not exist", backQuoteIfNeed(alter.getDatabase()));
  ...
  if (!table) throw Exception(ErrorCodes::UNKNOWN_TABLE, "Could not find table: {}", table_id.table_name);
    ...
    if (command_ast->type == ASTAlterCommand::EXECUTE_COMMAND) {
      ...
      if (mut_command->type == MutationCommand::UPDATE || mut_command->type == MutationCommand::DELETE) {
        ...
        if (rewritten_command_ast) {
          ...
          if (!mut_command)
            throw Exception(ErrorCodes::LOGICAL_ERROR, "Alter command '{}' is rewritten to invalid command '{}'",
  ...
  if (typeid_cast<DatabaseReplicated *>(database.get())) {
    ...
    bool mixed_settings_amd_metadata_alter =
        alter_commands.hasNonReplicatedAlterCommand() && !alter_commands.areNonReplicatedAlterCommands();
  ...
  if (!alter_commands.empty() && mutation_commands.hasNonEmptyMutationCommands()) {
    ...
    for (const auto &command : alter_commands)
      if (command.type == AlterCommand::RENAME_COLUMN) columns_to_rename.insert(command.column_name);
          ...
          for (const auto *identifier : identifiers) {
            ...
            if (column_name && columns_to_rename.contains(*column_name)) {
              throw Exception(ErrorCodes::BAD_ARGUMENTS,
                              ...
                              "Please split into separate ALTER statements.",
                              backQuote(*column_name), command.type == MutationCommand::DELETE ? "DELETE" : "UPDATE");
            }
          }
        }
      }
    ...
    auto supports_lightweight_update = [&] -> std::expected<void, PreformattedMessage> {
      if (!settings[Setting::enable_lightweight_update])
        return std::unexpected(
            PreformattedMessage::create("Lightweight updates are not allowed. Set 'enable_lightweight_update = 1' to allow them"));
  ...
  if (!partition_commands.empty()) {
    ...
    if (!partition_commands_pipe.empty()) res.pipeline = QueryPipeline(std::move(partition_commands_pipe));
  }
  ...
  for (const auto *execute_command : execute_commands) {
    ...
    if (!execute_pipe.empty()) res.pipeline = QueryPipeline(std::move(execute_pipe));
  }
...
BlockIO InterpreterAlterQuery::executeToDatabase(const ASTAlterQuery &alter) {
  ...
  bool managed_by_shared_catalog =
      SharedDatabaseCatalog::initialized() && SharedDatabaseCatalog::isDatabaseEngineSupported(database->getEngineName());
...
AccessRightsElements InterpreterAlterQuery::getRequiredAccess() const {
  ...
  const auto &alter = query_ptr->as<ASTAlterQuery &>();
  for (const auto &child : alter.command_list->children)
    boost::range::push_back(required_access,
                            getRequiredAccessForCommand(child->as<ASTAlterCommand &>(), alter.getDatabase(), alter.getTable()));
  return required_access;
}

AccessRightsElements InterpreterAlterQuery::getRequiredAccessForCommand(const ASTAlterCommand &command, const String &database,
                                                                        const String &table) {
  AccessRightsElements required_access;
...
void InterpreterAlterQuery::extendQueryLogElemImpl(QueryLogElement &elem, const ASTPtr &ast, ContextPtr query_context) const {
  ...
  if (alter.command_list != nullptr && alter.alter_object != ASTAlterQuery::AlterObjectType::DATABASE) {
    ...
    if (main_database.empty()) main_database = query_context->getCurrentDatabase();
    ...
    for (const auto &child : alter.command_list->children) {
      ...
      if (command->column) elem.query_columns.insert(prefix + command->column->getColumnName());

      if (command->rename_to) elem.query_columns.insert(prefix + command->rename_to->getColumnName());
