...
void NO_INLINE Set::insertFromBlockImpl(Method &method, const ColumnRawPtrs &key_columns, size_t rows, SetVariants &variants,
                                        ConstNullMapPtr null_map, ColumnUInt8::Container *out_filter) {
  if (null_map) {
    if (out_filter)
...
void NO_INLINE Set::insertFromBlockImplCase(Method &method, const ColumnRawPtrs &key_columns, size_t rows, SetVariants &variants,
  ...
  for (size_t i = 0; i < rows; ++i) {
    ...
    if constexpr (build_filter) (*out_filter)[i] = emplace_result.isInserted();
  }
}
...
DataTypes Set::getElementTypes(DataTypes types, bool transform_null_in) {
  for (auto &type : types) {
    ...
    if (!transform_null_in) type = removeNullable(type);
  }
...
void Set::fillSetElements() {
  ...
  for (const auto &type : set_elements_types) set_elements.emplace_back(type->createColumn());
}

bool Set::insertFromBlock(const ColumnsWithTypeAndName &columns) {
  Columns cols;
  cols.reserve(columns.size());
  for (const auto &column : columns) cols.emplace_back(column.column);
...
bool Set::insertFromColumns(const Columns &columns) {
  ...
  if (fill_set_elements) holder.filter = ColumnUInt8::create(rows);
...
bool Set::insertFromColumns(const Columns &columns, SetKeyColumns &holder) {
  ...
  if (data.empty()) throw Exception(ErrorCodes::LOGICAL_ERROR, "Method Set::setHeader must be called before Set::insertFromBlock");
  ...
  ConstNullMapPtr null_map{};
  ColumnPtr null_map_holder;
  if (!transform_null_in) null_map_holder = extractNestedColumnsAndNullMap(holder.key_columns, null_map);
...
void Set::appendSetElements(SetKeyColumns &holder) {
  if (holder.key_columns.size() != keys_size || set_elements.size() != keys_size)
    throw Exception(ErrorCodes::LOGICAL_ERROR, "Invalid number of key columns for set. Expected {} got {} and {}", keys_size,
                    holder.key_columns.size(), set_elements.size());
  ...
  for (size_t i = 0; i < keys_size; ++i) {
    ...
    if (transform_null_in && holder.null_map_holder) set_elements[i]->insert(Null{});
  }
}

void Set::checkIsCreated() const {
  if (!is_created.load()) throw Exception(ErrorCodes::LOGICAL_ERROR, "Trying to use set before it has been built.");
}
...
ColumnUInt8::Ptr checkDateTimePrecision(const ColumnWithTypeAndName &column_to_cast) {
  ...
  const IColumn *original_nested_column =
      original_nullable_column ? &original_nullable_column->getNestedColumn() : column_to_cast.column.get();
  ...
  if (!original_decimal_column) throw Exception(ErrorCodes::LOGICAL_ERROR, "Expected ColumnDecimal for DateTime64");
...
ColumnPtr mergeNullMaps(const ColumnPtr &null_map_column1, const ColumnUInt8::Ptr &null_map_column2) {
  if (!null_map_column1) return null_map_column2;
  if (!null_map_column2) return null_map_column1;
  ...
  if (size != null_map2.size()) throw Exception(ErrorCodes::LOGICAL_ERROR, "Null maps have different sizes");

  auto merged_null_map_column = ColumnUInt8::create(size);
  auto &merged_null_map = merged_null_map_column->getData();

  for (size_t i = 0; i < size; ++i) merged_null_map[i] = null_map1[i] || null_map2[i];

  return merged_null_map_column;
}

void Set::processDateTime64Column(const ColumnWithTypeAndName &column_to_cast, ColumnPtr &result, ColumnPtr &null_map_holder,
  ...
  const IColumn *nested_result_column = result_nullable_column ? &result_nullable_column->getNestedColumn() : result.get();

  ColumnPtr existing_null_map_column = result_nullable_column ? result_nullable_column->getNullMapColumnPtr() : nullptr;
  ...
  if (transform_null_in) {
    ...
    if (!null_map_column) throw Exception(ErrorCodes::LOGICAL_ERROR, "Null map must be ColumnUInt8");
...
ColumnPtr Set::execute(const ColumnsWithTypeAndName &columns, bool negative) const {
  ...
  if (0 == num_key_columns) throw Exception(ErrorCodes::LOGICAL_ERROR, "No columns passed to Set::execute method.");

  auto res = ColumnUInt8::create();
  ColumnUInt8::Container &vec_res = res->getData();
  vec_res.resize(columns.at(0).column->size());

  if (vec_res.empty()) return res;
  ...
  for (size_t i = 0; i < num_key_columns; ++i) {
    ...
    if (use_cast_accurate_or_null) {
      ...
      if (transform_null_in && column_to_cast.type->isNullable() && !data_types[i]->isNullable()) {
        ...
        result = castColumnAccurate(ColumnWithTypeAndName(column_nullable.getNestedColumnPtr(), nested_type, column_to_cast.name),
                                    data_types[i], cast_cache.get());
        if (!null_map_holder) {
          ...
          for (size_t j = 0, size = mutable_null_map.size(); j < size; ++j) mutable_null_map[j] |= other_null_map[j];
  ...
  if (!transform_null_in) null_map_holder = extractNestedColumnsAndNullMap(key_columns, null_map);
...
bool Set::hasNull() const {
  ...
  if (!transform_null_in) return false;

  if (data_types.size() != 1) return false;

  if (!data_types[0]->isNullable()) return false;
...
template <typename Method>
void NO_INLINE Set::executeImpl(Method &method, const ColumnRawPtrs &key_columns, ColumnUInt8::Container &vec_res, bool negative,
                                size_t rows, ConstNullMapPtr null_map) const {
  if (null_map)
...
void NO_INLINE Set::executeImplCase(Method &method, const ColumnRawPtrs &key_columns, ColumnUInt8::Container &vec_res, bool negative,
                                    size_t rows, ConstNullMapPtr null_map) const {
  Arena pool;
  typename Method::State state(key_columns, key_sizes, nullptr);
...
void Set::executeOrdinary(const ColumnRawPtrs &key_columns, ColumnUInt8::Container &vec_res, bool negative,
                          ConstNullMapPtr null_map) const {
...
void Set::checkColumnsNumber(size_t num_key_columns) const {
  if (data_types.size() != num_key_columns) {
    throw Exception(ErrorCodes::NUMBER_OF_COLUMNS_DOESNT_MATCH, "Number of columns in section IN doesn't match. {} at left, {} at right.",
...
bool Set::areTypesEqual(size_t set_type_idx, const DataTypePtr &other_type) const {
  ...
  if (set_type_idx >= data_types.size()) return false;
...
void Set::checkTypesEqual(size_t set_type_idx, const DataTypePtr &other_type) const {
  if (!this->areTypesEqual(set_type_idx, other_type))
    throw Exception(ErrorCodes::TYPE_MISMATCH,
                    "Types of column {} in section IN don't match: "
                    "{} on the left, {} on the right",
                    toString(set_type_idx + 1), other_type->getName(), data_types[set_type_idx]->getName());
}
...
MergeTreeSetIndex::MergeTreeSetIndex(const Columns &set_elements, std::vector<KeyTuplePositionMapping> &&indexes_mapping_)
  ...
  ::sort(indexes_mapping.begin(), indexes_mapping.end(), [](const KeyTuplePositionMapping &l, const KeyTuplePositionMapping &r) {
  ...
  ///     (x + 1) IN (10)
  indexes_mapping.erase(
      std::unique(indexes_mapping.begin(), indexes_mapping.end(),
  ...
  for (size_t i = 0; i < tuple_size; ++i) ordered_set[i] = set_elements[indexes_mapping[i].tuple_index];
  ...
  for (size_t i = 0; i < tuple_size; ++i) ordered_set[i] = block_to_sort.getByPosition(i).column;
}
...
BoolMask MergeTreeSetIndex::checkInRange(const std::vector<Range> &key_ranges, const DataTypes &data_types, bool single_point) const {
  ...
  for (size_t i = 0; i < tuple_size; ++i) {
    std::optional<Range> new_range = KeyCondition::applyMonotonicFunctionsChainToRange(
        key_ranges[indexes_mapping[i].key_index], indexes_mapping[i].functions, data_types[indexes_mapping[i].key_index], single_point);

    if (!new_range) return {true, true};
  ...
  auto compare = [](const IColumn &lhs, const FieldValue &rhs, size_t row) {
    if (rhs.isNegativeInfinity()) return +1;
    if (rhs.isPositiveInfinity()) return lhs.isNullAt(row) ? 0 : -1;  // +Inf == +Inf
  ...
  size_t begin = std::partition_point(indices.begin(), indices.end(),
                                      [&](size_t row) {
                                        ...
                                        for (size_t i = 0; i < tuple_size; ++i) {
                                          ...
                                          if (cmp < 0 || (cmp == 0 && !ranges[i].left_included)) return true;
                                        }
                                        return false;
                                      }) -
                 indices.begin();
  size_t end = std::partition_point(indices.begin(), indices.end(),
                                    [&](size_t row) {
                                      /// Return false if set[row] is above the key range.
                                      for (size_t i = 0; i < tuple_size; ++i) {
                                        int cmp = compare(*ordered_set[i], ranges[i].right, row);

                                        if (cmp > 0 || (cmp == 0 && !ranges[i].right_included)) return false;
  ...
  for (size_t i = 0; i < tuple_size; ++i) {
    ...
    } else if ((r.left.isPositiveInfinity() && r.right.isPositiveInfinity()) ||
               (r.left.isNegativeInfinity() && r.right.isNegativeInfinity())) {
...
bool MergeTreeSetIndex::hasMonotonicFunctionsChain() const {
  for (const auto &mapping : indexes_mapping)
    if (!mapping.functions.empty()) return true;
  return false;
}
...
void MergeTreeSetIndex::FieldValue::update(const Field &x) {
  if (x.isNegativeInfinity() || x.isPositiveInfinity())
    ...
    if (!column->empty()) column->popBack(1);
    column->insert(x);
