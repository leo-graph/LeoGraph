...
fs::path getShardsListPath(const String& zk_root) { return fs::path(zk_root + "/shards"); }

}  // namespace

ClusterDiscovery::ClusterInfo::ClusterInfo(const String& name_, const String& zk_name_, const String& zk_root_, const String& host_name,
                                           const String& username_, const String& password_, const String& cluster_secret_, UInt16 port,
                                           bool secure, size_t shard_id, bool observer_mode, bool invisible, size_t zk_root_index_)
    : name(name_),
      zk_name(zk_name_),
      zk_root(zk_root_),
...
class ClusterDiscovery::Flags {
 public:
  ...
  Flags(It begin, It end) {
    for (auto it = begin; it != end; ++it) flags.emplace(*it, false);
  }
...
ClusterDiscovery::ClusterDiscovery(const Poco::Util::AbstractConfiguration& config, ContextPtr context_,
                                   MultiVersion<Macros>::Version macros_, const String& config_prefix)
  ...
  for (const auto& key : config_keys) {
    ...
    if (!config.has(cluster_config_prefix)) continue;
    ...
    if (!password.empty() && !cluster_secret.empty())
      throw Exception(ErrorCodes::NO_ELEMENTS_IN_CONFIG,
                      "Both 'password' and 'secret' are specified for cluster '{}', only one option can be used at the same time", key);

    if (!zk_multicluster_name_and_root.empty()) {
      if (!zk_name_and_root.empty())
        throw Exception(ErrorCodes::EXCESSIVE_ELEMENT_IN_CONFIG,
                        "Autodiscovery cluster node {} has 'path' and 'multicluster_root_path' subnodes simultaneously", key);
      if (!is_observer) throw Exception(ErrorCodes::NO_ELEMENTS_IN_CONFIG, "Autodiscovery cluster node {} must be in observer mode", key);
    ...
    if (zk_name_and_root.empty()) throw Exception(ErrorCodes::NO_ELEMENTS_IN_CONFIG, "ZooKeeper path for cluster '{}' is empty", key);
    ...
    clusters_info.emplace(key, ClusterInfo(
                                   /* name_= */ key,
                                   /* zk_name_= */ zk_name,
                                   /* zk_root_= */ zk_root,
  ...
  for (const auto& e : clusters_info) clusters_info_names.emplace_back(e.first);
  ...
  for (const auto& e : clusters_info)
    get_nodes_callbacks[e.first] = std::make_shared<Coordination::WatchCallback>(
        [cluster_name = e.first, my_clusters_to_update = clusters_to_update](auto) { my_clusters_to_update->set(cluster_name); });

  for (auto& path : multicluster_discovery_paths) {
    path.watch_callback =
        std::make_shared<Coordination::WatchCallback>([my_need_update = path.need_update, my_flag = clusters_to_update](auto) {
...
Strings ClusterDiscovery::getNodeNames(zkutil::ZooKeeperPtr& zk, const String& zk_root, const String& cluster_name, int* version,
                                       bool set_callback, size_t zk_root_index) {
  ...
  if (set_callback) {
    ...
    if (callback == get_nodes_callbacks.end()) {
      auto watch_dynamic_callback = std::make_shared<Coordination::WatchCallback>(
          [cluster_name, my_clusters_to_update = clusters_to_update,
           my_discovery_paths_need_update = multicluster_discovery_paths[zk_root_index - 1].need_update](auto) {
  ...
  if (version) *version = stat.cversion;
  return nodes;
}
...
ClusterDiscovery::NodesInfo ClusterDiscovery::getNodes(zkutil::ZooKeeperPtr& zk, const String& zk_root, const Strings& node_uuids) {
  ...
  for (const auto& node_uuid : node_uuids) {
    String payload;
    bool ok = zk->tryGet(getShardsListPath(zk_root) / node_uuid, payload) && NodeInfo::parse(payload, result[node_uuid]);
    if (!ok) {
...
bool ClusterDiscovery::needUpdate(const Strings& node_uuids, const NodesInfo& nodes) {
                        ...
                        std::any_of(node_uuids.begin(), node_uuids.end(), [&nodes](auto u) { return !nodes.contains(u); });
  {
    ...
    for (const auto& [name, _] : nodes) old_names.emplace(name);
    ...
    auto format_cluster_update = [](const std::set<String>& s1, const std::set<String>& s2) {
      ...
      if (need_crop) diff.resize(max_to_show);

      if (sz == 0) return fmt::format("{} nodes", sz);
      return fmt::format("{} node{} [{}{}]", sz, sz != 1 ? "s" : "", fmt::join(diff, ", "), need_crop ? ",..." : "");
    };

    LOG_DEBUG(log, "Cluster update: added {}, removed {}", format_cluster_update(new_names, old_names),
...
ClusterPtr ClusterDiscovery::makeCluster(const ClusterInfo& cluster_info) {
  std::vector<Strings> shards;
  {
    ...
    for (auto& [_, replicas] : replica_addresses) shards.emplace_back(std::move(replicas));
  }
  ...
  auto cluster = std::make_shared<Cluster>(context->getSettingsRef(), shards, params);
  return cluster;
}

static bool contains(const Strings& list, const String& value) { return std::find(list.begin(), list.end(), value) != list.end(); }
...
bool ClusterDiscovery::upsertCluster(ClusterInfo& cluster_info) {
  ...
  if (bool ok = on_exit(); !ok) return false;
...
void ClusterDiscovery::initialUpdate() {
  ...
  fiu_do_on(FailPoints::cluster_discovery_faults, {
    ...
    if (is_failed) throw Exception(ErrorCodes::KEEPER_EXCEPTION, "Failpoint cluster_discovery_faults is triggered");
  });
...
void ClusterDiscovery::findDynamicClusters(std::unordered_map<String, ClusterDiscovery::ClusterInfo>& info,
  ...
  for (const auto& path : multicluster_discovery_paths) {
    ...
    for (const auto& cluster : clusters) {
      ...
      if (info.contains(cluster)) {
        ...
        LOG_WARNING(log, "Found dynamic duplicate of cluster '{}' in Keeper, skipped record by path {}:{}", cluster, path.zk_name,
                    path.zk_path);
        continue;
      }

      info.emplace(cluster, ClusterInfo(
                                /* name_= */ cluster,
...
bool ClusterDiscovery::runMainThread(std::function<void()> up_to_date_callback) {
  ...
  if (!is_initialized) initialUpdate();
  ...
  while (!finished) {
    ...
    for (const auto& [cluster_name, info] : clusters_info) {
      if (!info.zk_root_index) continue;
      if (!new_dynamic_clusters_info.erase(cluster_name) && !unchanged_roots.contains(info.zk_root_index))
        clusters_to_remove.insert(cluster_name);
    }
    /// new_dynamic_clusters_info now contains only new clusters
    for (const auto& [cluster_name, _] : new_dynamic_clusters_info) clusters_to_insert.insert(cluster_name);

    for (const auto& cluster_name : clusters_to_remove) removeCluster(cluster_name);
...
ClusterPtr ClusterDiscovery::getCluster(const String& cluster_name) const {
  ...
  if (it == cluster_impls.end()) return nullptr;
  return it->second;
}
...
bool ClusterDiscovery::NodeInfo::parse(const String& data, NodeInfo& result) {
  try {
    ...
    if (ver == data_ver) {
      ...
      LOG_ERROR(getLogger("ClusterDiscovery"), "Unsupported version '{}' of data in zk node '{}'", ver,
                data.size() < 1024 ? data : "[data too long]");
    }
  } catch (Poco::Exception& e) {
    LOG_WARNING(getLogger("ClusterDiscovery"), "Can't parse '{}' from node: {}", data.size() < 1024 ? data : "[data too long]",
                e.displayText());
    return false;
  }
  return true;
