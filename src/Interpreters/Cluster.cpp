...
bool Cluster::Address::isLocal(UInt16 clickhouse_port) const {
  if (auto resolved = getResolvedAddress()) return isLocalImpl(*this, *resolved, clickhouse_port);
  return false;
}

Cluster::Address::Address(const Poco::Util::AbstractConfiguration &config, const String &config_prefix, const String &cluster_,
                          const String &cluster_secret_, UInt32 shard_index_, UInt32 replica_index_)
    : cluster(cluster_), cluster_secret(cluster_secret_), shard_index(shard_index_), replica_index(replica_index_) {
  host_name = config.getString(config_prefix + ".host");
  if (config.has(config_prefix + ".user")) user_specified = true;

  user = config.getString(config_prefix + ".user", "default");
  password = config.getString(config_prefix + ".password", "");
  default_database = config.getString(config_prefix + ".default_database", "");
  secure = ConfigHelper::getBool(config, config_prefix + ".secure", false, /* empty_as */ true) ? Protocol::Secure::Enable
  ...
  if (!port) throw Exception(ErrorCodes::NO_ELEMENTS_IN_CONFIG, "Port is not specified in cluster configuration: {}.port", config_prefix);
  ...
  compression = config.getBool(config_prefix + ".compression", !is_local) ? Protocol::Compression::Enable : Protocol::Compression::Disable;
}

Cluster::Address::Address(const DatabaseReplicaInfo &info, const ClusterConnectionParameters &params, UInt32 shard_index_,
...
String Cluster::Address::toString() const { return toString(host_name, port); }

String Cluster::Address::toString(const String &host_name, UInt16 port) { return escapeForFileName(host_name) + ':' + DB::toString(port); }
...
std::pair<String, UInt16> Cluster::Address::fromString(const String &host_port_string) {
  ...
  if (pos == std::string::npos) throw Exception(ErrorCodes::SYNTAX_ERROR, "Incorrect <host>:<port> format {}", host_port_string);
...
Cluster::Address Cluster::Address::fromFullString(std::string_view full_string) {
  ...
  if (auto pos = full_string.find('@'); pos != std::string_view::npos) user_password = full_string.substr(pos + 1);
    ...
    if (auto underscore_pos = full_string.find('_'); underscore_pos != std::string_view::npos) {
      ...
      if (full_string.substr(underscore_pos + 1).starts_with("replica")) {
        ...
        throw Exception(ErrorCodes::SYNTAX_ERROR,
                        "Incorrect address '{}', should be in a form of `shardN_all_replicas` or `shardN_replicaM`", full_string);
    } else {
  ...
  if (!host_end) throw Exception(ErrorCodes::SYNTAX_ERROR, "Incorrect address '{}', it does not contain port", full_string);
...
Clusters::Clusters(const Poco::Util::AbstractConfiguration &config, const Settings &settings, MultiVersion<Macros>::Version macros,
...
void Clusters::updateClusters(const Poco::Util::AbstractConfiguration &new_config, const Settings &settings, const String &config_prefix,
                              Poco::Util::AbstractConfiguration *old_config) {
  ...
  if (old_config) {
    ...
    std::set_difference(old_config_keys.begin(), old_config_keys.end(), new_config_keys.begin(), new_config_keys.end(),
                        std::back_inserter(deleted_keys));
  }
  ...
  if (old_config) {
    for (const auto &key : deleted_keys) {
      if (!automatic_clusters.contains(key)) impl.erase(key);
    }
  } else {
  ...
  for (const auto &key : new_config_keys) {
    ...
    if (key.contains('.')) throw Exception(ErrorCodes::SYNTAX_ERROR, "Cluster names with dots are not supported: '{}'", key);
...
Cluster::Cluster(const Poco::Util::AbstractConfiguration &config, const Settings &settings, const String &config_prefix_,
  ...
  if (config_keys.empty())
    throw Exception(ErrorCodes::SHARD_HAS_NO_CONNECTIONS, "No cluster elements (shard, node) specified in config at path {}",
                    config_prefix);

  size_t shards_with_name_count =
      std::ranges::count_if(config_keys.begin(), config_keys.end(),
                            [&config, &config_prefix](const String &key) { return config.has(config_prefix + key + ".name"); });

  if (shards_with_name_count != 0 && shards_with_name_count != config_keys.size()) {
    throw Exception(ErrorCodes::INVALID_SHARD_ID, "name must be specified for every shard(node) in the config or for none. Config: {}",
                    config_prefix);
  }
  ...
  for (const auto &key : config_keys) {
    ...
    if (shard_without_replicas) {
      ...
      if (address.is_local) info.local_addresses.push_back(address);

      auto pool = ConnectionPoolFactory::instance().get(static_cast<unsigned>(settings[Setting::distributed_connections_pool_size]),
                                                        address.host_name, address.port, address.default_database, address.user,
                                                        address.password, address.proto_send_chunked, address.proto_recv_chunked,
                                                        address.quota_key, address.cluster, address.cluster_secret, "server",
                                                        address.compression, address.secure, address.bind_host, address.priority);
      ...
      if (weight) slot_to_shard.insert(std::end(slot_to_shard), weight, shards_info.size());
      ...
      for (const auto &replica_key : replica_keys) {
        ...
        if (startsWith(replica_key, "replica")) {
          ...
          if (internal_replication) {
            ...
            if (!replica_addresses.back().is_local) concatInsertPath(insert_paths.prefer_localhost_replica, dir_name);
      ...
      addShard(settings, replica_addresses,
               /* treat_local_as_remote = */ false, current_shard_num, std::move(shard_name), weight, std::move(insert_paths),
...
Cluster::Cluster(const Settings &settings, const std::vector<std::vector<String>> &names, const ClusterConnectionParameters &params) {
  UInt32 current_shard_num = 1;

  secret = params.cluster_secret;

  for (const auto &shard : names) {
    if (shard.empty()) throw Exception(ErrorCodes::BAD_ARGUMENTS, "Shard contains zero number of replicas");

    Addresses current;
    for (const auto &replica : shard)
      current.emplace_back(DatabaseReplicaInfo{replica, "", ""}, params, current_shard_num, current.size() + 1);
...
Cluster::Cluster(const Settings &settings, const std::vector<std::vector<DatabaseReplicaInfo>> &infos,
  ...
  for (const auto &shard : infos) {
    Addresses current;
    for (const auto &replica : shard) current.emplace_back(replica, params, current_shard_num, current.size() + 1);
...
void Cluster::addShard(const Settings &settings, Addresses addresses, bool treat_local_as_remote, UInt32 current_shard_num,
                       String current_shard_name, UInt32 weight, ShardInfoInsertPathForInternalReplication insert_paths,
  ...
  for (const auto &replica : addresses) {
    auto replica_pool = ConnectionPoolFactory::instance().get(
        static_cast<unsigned>(settings[Setting::distributed_connections_pool_size]), replica.host_name, replica.port,
        replica.default_database, replica.user, replica.password, replica.proto_send_chunked, replica.proto_recv_chunked, replica.quota_key,
        replica.cluster, replica.cluster_secret, "server", replica.compression, replica.secure, replica.bind_host, replica.priority);

    all_replicas_pools.emplace_back(replica_pool);
    if (replica.is_local && !treat_local_as_remote) shard_local_addresses.push_back(replica);
  }
  ConnectionPoolWithFailoverPtr shard_pool = std::make_shared<ConnectionPoolWithFailover>(
      all_replicas_pools, settings[Setting::load_balancing], settings[Setting::distributed_replica_error_half_life].totalSeconds(),
      settings[Setting::distributed_replica_error_cap]);

  if (weight) slot_to_shard.insert(std::end(slot_to_shard), weight, shards_info.size());

  shards_info.push_back({std::move(insert_paths), current_shard_num, std::move(current_shard_name), weight,
...
Poco::Timespan Cluster::saturate(Poco::Timespan v, Poco::Timespan limit) {
  if (limit.totalMicroseconds() == 0) return v;
...
Cluster::Cluster(Cluster::ReplicasAsShardsTag, const Cluster &from, const Settings &settings, size_t max_replicas_from_shard) {
  if (from.addresses_with_failover.empty()) throw Exception(ErrorCodes::LOGICAL_ERROR, "Cluster is empty");
  ...
  for (size_t shard_index : collections::range(0, from.shards_info.size())) {
    auto create_shards_from_replicas = [&](std::span<const Address> replicas) {
      for (const auto &address : replicas) {
        if (!unique_hosts.emplace(address.host_name, address.port).second) continue;  /// Duplicate host, skip.
...
Cluster::Cluster(Cluster::SubclusterTag, const Cluster &from, const std::vector<size_t> &indices) {
  for (size_t index : indices) {
    ...
    if (from_shard.weight) slot_to_shard.insert(std::end(slot_to_shard), from_shard.weight, shards_info.size());
    shards_info.emplace_back(from_shard);

    if (!from.addresses_with_failover.empty()) addresses_with_failover.emplace_back(from.addresses_with_failover.at(index));
  }
...
std::vector<Strings> Cluster::getHostIDs() const {
  ...
  for (size_t i = 0; i != addresses_with_failover.size(); ++i) {
    ...
    for (size_t j = 0; j != addresses.size(); ++j) host_ids[i][j] = addresses[j].toString();
  }
  return host_ids;
}
...
std::vector<const Cluster::Address *> Cluster::filterAddressesByShardOrReplica(size_t only_shard_num, size_t only_replica_num) const {
  ...
  auto enumerate_replicas = [&](size_t shard_index) {
    ...
    if (only_replica_num) {
      if (only_replica_num > replicas.size())
        throw Exception(ErrorCodes::NO_SUCH_REPLICA, "Cluster {} doesn't have replica #{} in shard #{}", name, only_replica_num,
                        shard_index);
      res.emplace_back(&replicas[only_replica_num - 1]);
    } else {
      for (const auto &addr : replicas) res.emplace_back(&addr);
    }
  };
  ...
  if (only_shard_num) {
    ...
    for (size_t shard_index = 1; shard_index <= addresses_with_failover.size(); ++shard_index) enumerate_replicas(shard_index);
  }
...
const std::string &Cluster::ShardInfo::insertPathForInternalReplication(bool prefer_localhost_replica, bool use_compact_format) const {
  if (!has_internal_replication) throw Exception(ErrorCodes::LOGICAL_ERROR, "internal_replication is not set");
  ...
  if (!use_compact_format) {
    ...
    if (path.size() > NAME_MAX) {
      throw Exception(ErrorCodes::LOGICAL_ERROR, "Path '{}' for async distributed INSERT is too long (exceed {} limit)", path, NAME_MAX);
    }
    return path;
  }
...
bool Cluster::maybeCrossReplication() const {
  ...
  if (addresses_with_failover.empty()) return false;
  ...
  for (const auto &shard : addresses_with_failover)
    for (const auto &replica : shard)
      if (replica.default_database != database_name) return true;
