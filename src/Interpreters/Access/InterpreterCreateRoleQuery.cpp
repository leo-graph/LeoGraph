...
namespace {
void updateRoleFromQueryImpl(Role &role, const ASTCreateRoleQuery &query, const String &override_name,
...
BlockIO InterpreterCreateRoleQuery::execute() {
  ...
  for (const auto &name : query.names) getContext()->checkAccess(access_type, name);

  if (!query.new_name.empty() && !query.alter) getContext()->checkAccess(AccessType::CREATE_ROLE, query.new_name);
  ...
  if (settings_from_query && !query.attach) getContext()->checkSettingsConstraints(*settings_from_query, SettingSource::ROLE);
    ...
    if (!query.storage_name.empty()) {
      for (const auto &name : query.names) {
        if (auto another_storage_ptr = access_control.findExcludingStorage(AccessEntityType::ROLE, name, storage_ptr))
          throw Exception(ErrorCodes::ACCESS_ENTITY_ALREADY_EXISTS, "Role {} already exists in storage {}", name,
