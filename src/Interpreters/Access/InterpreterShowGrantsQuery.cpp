...
namespace {
void getGrantsFromAccess(ASTs &res, const AccessRights &access, const boost::intrusive_ptr<ASTRolesOrUsersSet> grantees,
                         const AccessControl *access_control, bool attach_mode = false, bool with_implicit = false) {
  ...
  for (auto &element : elements) {
    ...
    if (current_query) {
      ...
      if (!continue_with_current_query) current_query = nullptr;
    }
    ...
    if (!current_query) {
      ...
      if (element.is_partial_revoke) current_query->is_revoke = true;
...
void unionAccessFromRoles(AccessRights &res, const T &entity, const AccessControl *access_control) {
  ...
  std::vector granted_roles_with_admin_option(entity.granted_roles.getGrantedWithAdminOption().begin(),
                                              entity.granted_roles.getGrantedWithAdminOption().end());
...
template <typename T>
ASTs getGrantQueriesImpl(const T &grantee, const AccessControl *access_control /* not used if attach_mode == true */,
                         bool attach_mode = false, bool with_implicit = false, bool final = false) {
  ASTs res;
  ...
  if (final) unionAccessFromRoles(access, grantee, access_control);
...
ASTs getGrantQueriesImpl(const IAccessEntity &entity, const AccessControl *access_control /* not used if attach_mode == true */,
                         bool attach_mode = false, bool with_implicit = false, bool final = false) {
...
QueryPipeline InterpreterShowGrantsQuery::executeImpl() {
  ...
  String prefix = "SHOW ";
  if (desc.starts_with(prefix)) desc = desc.substr(prefix.length());  /// `desc` always starts with "SHOW ", so we can trim this prefix.
...
std::vector<AccessEntityPtr> InterpreterShowGrantsQuery::getEntities() const {
  ...
  for (const auto &id : ids) {
    ...
    bool is_enabled_or_granted_role =
        entity->isTypeOf<Role>() && (current_user->granted_roles.isGranted(id) || roles_info->enabled_roles.contains(id));

    if ((is_current_user /* Any user can see his own grants */) || (is_enabled_or_granted_role /* and grants from the granted roles */) ||
