...
String InterpreterShowAccessEntitiesQuery::getRewrittenQuery() const {
  ...
  switch (query.type) {
    case AccessEntityType::ROW_POLICY: {
      ...
      if (!query.short_name.empty()) filter = "short_name = " + quoteString(query.short_name);
      ...
      if (query.database_and_table_name) {
        ...
        if (!database.empty()) filter += String{filter.empty() ? "" : " AND "} + "database = " + quoteString(database);
        if (!table_name.empty()) filter += String{filter.empty() ? "" : " AND "} + "table = " + quoteString(table_name);
        if (!database.empty() && !table_name.empty()) expr = "short_name";
      }
      break;
    }
  ...
  if (origin.empty()) throw Exception(ErrorCodes::NOT_IMPLEMENTED, "{}: type is not supported by SHOW query", toString(query.type));

  if (order.empty() && expr != "*") order = expr;

  return "SELECT " + expr + " from system." + origin + (filter.empty() ? "" : " WHERE " + filter) +
