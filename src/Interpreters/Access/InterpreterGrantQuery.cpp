...
void collectAccessRightsElementsToGrantOrRevoke(const ASTGrantQuery &query, AccessRightsElements &elements_to_grant,
...
void collectRolesToGrantOrRevoke(const AccessControl &access_control, const ASTGrantQuery &query, std::vector<UUID> &roles_to_grant,
                                 RolesOrUsersSet &roles_to_revoke) {
  roles_to_grant.clear();
  roles_to_revoke.clear();

  RolesOrUsersSet roles_to_grant_or_revoke;
  if (query.roles) roles_to_grant_or_revoke = RolesOrUsersSet{*query.roles, access_control};
...
/// Extracts roles which are going to be granted or revoked from a query.
void collectRolesToGrantOrRevoke(const ASTGrantQuery &query, std::vector<UUID> &roles_to_grant, RolesOrUsersSet &roles_to_revoke) {
  roles_to_grant.clear();
  roles_to_revoke.clear();

  RolesOrUsersSet roles_to_grant_or_revoke;
  if (query.roles) roles_to_grant_or_revoke = RolesOrUsersSet{*query.roles};
...
void checkGrantOption(const AccessControl &access_control, const ContextAccessWrapper &current_user_access,
                      const std::vector<UUID> &grantees_from_query, bool &need_check_grantees_are_allowed,
                      const AccessRightsElements &elements_to_grant, AccessRightsElements &elements_to_revoke) {
  ...
  for (const auto &id : grantees_from_query) {
    ...
    if (auto role = typeid_cast<RolePtr>(entity)) {
      if (need_check_grantees_are_allowed) current_user_access.checkGranteeIsAllowed(id, *role);
      all_granted_access.makeUnion(role->access);
    } else if (auto user = typeid_cast<UserPtr>(entity)) {
      if (need_check_grantees_are_allowed) current_user_access.checkGranteeIsAllowed(id, *user);
  ...
  if (!elements_to_revoke.empty() && elements_to_revoke[0].is_partial_revoke)
    std::for_each(elements_to_revoke.begin(), elements_to_revoke.end(),
                  [&](AccessRightsElement &element) { element.is_partial_revoke = false; });
  ...
  if (current_user_access.getAccessRightsWithImplicit()->containsWithGrantOption(access_to_revoke)) return;

  /// Technically, this check always fails if `containsWithGrantOption` returns `false`. But we still call it to get a nice exception
  /// message.
...
void checkAdminOption(const AccessControl &access_control, const ContextAccessWrapper &current_user_access,
                      const std::vector<UUID> &grantees_from_query, bool &need_check_grantees_are_allowed,
                      const std::vector<UUID> &roles_to_grant, RolesOrUsersSet &roles_to_revoke, bool admin_option) {
...
AccessRightsElements getRequiredAccessForExecutingOnCluster(const AccessRightsElements &elements_to_grant,
...
/// we just don't have the full information about them).
void checkAdminOptionForExecutingOnCluster(const ContextAccessWrapper &current_user_access, const std::vector<UUID> roles_to_grant,
  ...
  if (current_user_access.isGranted(AccessType::ROLE_ADMIN)) return;

  for (const auto &role_id : roles_to_grant) current_user_access.checkAdminOption(role_id);

  for (const auto &role_id : roles_to_revoke.getMatchingIDs()) current_user_access.checkAdminOption(role_id);
}

template <typename T>
void updateGrantedAccessRightsAndRolesTemplate(T &grantee, const AccessRightsElements &elements_to_grant,
                                               const AccessRightsElements &elements_to_revoke, const std::vector<UUID> &roles_to_grant,
                                               const RolesOrUsersSet &roles_to_revoke, bool admin_option) {
  if (!elements_to_revoke.empty()) grantee.access.revoke(elements_to_revoke);

  if (!elements_to_grant.empty()) grantee.access.grant(elements_to_grant);
  ...
  if (!roles_to_revoke.empty()) {
    if (admin_option) {
      ...
      if constexpr (std::is_same_v<T, User>) {
        for (const auto &id : found_roles_to_revoke) grantee.default_roles.ids.erase(id);
      }
    }
  }
...
void updateGrantedAccessRightsAndRoles(IAccessEntity &grantee, const AccessRightsElements &elements_to_grant,
                                       const AccessRightsElements &elements_to_revoke, const std::vector<UUID> &roles_to_grant,
                                       const RolesOrUsersSet &roles_to_revoke, bool admin_option) {
...
template <typename T>
void grantCurrentGrantsTemplate(T &grantee, const AccessRights &rights_to_grant, const AccessRightsElements &elements_to_revoke) {
  if (!elements_to_revoke.empty()) grantee.access.revoke(elements_to_revoke);
...
void grantCurrentGrants(IAccessEntity &grantee, const AccessRights &new_rights, const AccessRightsElements &elements_to_revoke) {
  if (auto *user = typeid_cast<User *>(&grantee))
...
void calculateCurrentGrantRightsWithIntersection(AccessRights &rights, std::shared_ptr<const ContextAccessWrapper> current_user_access,
...
BlockIO InterpreterGrantQuery::execute() {
  ...
  for (const auto &element : query.access_rights_elements) {
    if (element.isGlobalWithParameter() && (element.access_flags.getParameterType() == AccessFlags::TABLE_ENGINE) &&
  ...
  if (!query.cluster.empty()) {
    if (query.current_grants) throw Exception(ErrorCodes::BAD_ARGUMENTS, "GRANT CURRENT GRANTS can't be executed on cluster.");
  ...
  if (!query.current_grants)
    checkGrantOption(access_control, *current_user_access, grantees, need_check_grantees_are_allowed, elements_to_grant,
                     elements_to_revoke);

  /// Check if the current user has corresponding roles granted with admin option.
  checkAdminOption(access_control, *current_user_access, grantees, need_check_grantees_are_allowed, roles_to_grant, roles_to_revoke,
                   query.admin_option);

  if (need_check_grantees_are_allowed) current_user_access->checkGranteesAreAllowed(grantees);

  AccessRights new_rights;
  if (query.current_grants) calculateCurrentGrantRightsWithIntersection(new_rights, current_user_access, elements_to_grant);
...
void InterpreterGrantQuery::updateUserFromQuery(User &user, const ASTGrantQuery &query) { updateFromQuery(user, query); }

void InterpreterGrantQuery::updateRoleFromQuery(Role &role, const ASTGrantQuery &query) { updateFromQuery(role, query); }
