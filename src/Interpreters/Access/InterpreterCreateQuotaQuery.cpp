...
namespace {
void updateQuotaFromQueryImpl(Quota &quota, const ASTCreateQuotaQuery &query, const String &override_name,
  ...
  if (query.key_type) quota.key_type = *query.key_type;
  ...
  for (const auto &query_limits : query.all_limits) {
    ...
    if (query_limits.drop) {
      if (it != quota_all_limits.end()) quota_all_limits.erase(it);
      continue;
    }
    ...
    if (it == quota_all_limits.end()) {
      ...
      it = quota_all_limits.insert(
          boost::range::upper_bound(quota_all_limits, duration,
...
BlockIO InterpreterCreateQuotaQuery::execute() {
  ...
  if (query.roles) roles_from_query = RolesOrUsersSet{*query.roles, access_control, getContext()->getUserID()};
    ...
    if (!query.storage_name.empty()) {
      for (const auto &name : query.names) {
        if (auto another_storage_ptr = access_control.findExcludingStorage(AccessEntityType::QUOTA, name, storage_ptr))
          throw Exception(ErrorCodes::ACCESS_ENTITY_ALREADY_EXISTS, "Quota {} already exists in storage {}", name,
