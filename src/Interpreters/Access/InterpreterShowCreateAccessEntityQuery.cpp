...
namespace {
ASTPtr getCreateQueryImpl(const User &user, const AccessControl *access_control /* not used if attach_mode == true */, bool attach_mode) {
  auto query = make_intrusive<ASTCreateUserQuery>();
  query->names = make_intrusive<ASTUserNamesWithHost>(user.getName());
  query->attach = attach_mode;

  if (user.allowed_client_hosts != AllowedClientHosts::AnyHostTag{}) query->hosts = user.allowed_client_hosts;
  ...
  if (!user.settings.empty()) {
    ...
    if (!query_settings->empty()) query->settings = query_settings;
  }
...
ASTPtr getCreateQueryImpl(const Quota &quota, const AccessControl *access_control /* not used if attach_mode == true */, bool attach_mode) {
  auto query = make_intrusive<ASTCreateQuotaQuery>();
  query->names.emplace_back(quota.getName());
  query->attach = attach_mode;

  if (quota.key_type != QuotaKeyType::NONE) query->key_type = quota.key_type;
...
ASTPtr getCreateQueryImpl(const RowPolicy &policy, const AccessControl *access_control /* not used if attach_mode == true */,
                          bool attach_mode) {
  ...
  if (policy.isRestrictive()) query->is_restrictive = policy.isRestrictive();
...
ASTPtr getCreateQueryImpl(const IAccessEntity &entity, const AccessControl *access_control /* not used if attach_mode == true */,
                          bool attach_mode) {
  if (const User *user = typeid_cast<const User *>(&entity)) return getCreateQueryImpl(*user, access_control, attach_mode);
  if (const Role *role = typeid_cast<const Role *>(&entity)) return getCreateQueryImpl(*role, access_control, attach_mode);
  if (const RowPolicy *policy = typeid_cast<const RowPolicy *>(&entity)) return getCreateQueryImpl(*policy, access_control, attach_mode);
  if (const Quota *quota = typeid_cast<const Quota *>(&entity)) return getCreateQueryImpl(*quota, access_control, attach_mode);
...
QueryPipeline InterpreterShowCreateAccessEntityQuery::executeImpl() {
  ...
  for (const auto &create_query : create_queries) column->insert(format({getContext(), *create_query}));
  ...
  String prefix = "SHOW ";
  if (startsWith(desc, prefix)) desc = desc.substr(prefix.length());  /// `desc` always starts with "SHOW ", so we can trim this prefix.

  return QueryPipeline(std::make_shared<SourceFromSingleChunk>(
      std::make_shared<const Block>(Block{{std::move(column), std::make_shared<DataTypeString>(), desc}})));
}
...
std::vector<AccessEntityPtr> InterpreterShowCreateAccessEntityQuery::getEntities() const {
  ...
  if (show_query.all) {
    ...
    for (const auto &id : ids) {
      if (auto entity = access_control.tryRead(id)) entities.push_back(entity);
    }
    ...
    if (usage) entities.push_back(access_control.read<Quota>(usage->quota_id));
    ...
    if (show_query.row_policy_names) {
      for (const String &name : show_query.row_policy_names->toStrings()) entities.push_back(access_control.read<RowPolicy>(name));
    } else {
      for (const auto &id : ids) {
        auto policy = access_control.tryRead<RowPolicy>(id);
        if (!policy) continue;
        if (!show_query.short_name.empty() && (policy->getShortName() != show_query.short_name)) continue;
        if (show_query.database_and_table_name) {
          ...
          if (!database.empty() && (policy->getDatabase() != database)) continue;
          if (!table_name.empty() && (policy->getTableName() != table_name)) continue;
        }
    ...
    for (const String &name : show_query.names) entities.push_back(access_control.read(access_control.getID(show_query.type, name)));
  }
...
ASTs InterpreterShowCreateAccessEntityQuery::getCreateQueries() const {
  ...
  ASTs list;
  const auto &access_control = getContext()->getAccessControl();
  for (const auto &entity : entities) list.push_back(getCreateQuery(*entity, access_control));
    ...
    case AccessEntityType::ROW_POLICY: {
      if (show_query.row_policy_names) {
        ...
        if (show_query.database_and_table_name->second.empty())
          ...
          res.emplace_back(AccessType::SHOW_ROW_POLICIES, show_query.database_and_table_name->first,
                           show_query.database_and_table_name->second);
      } else {
