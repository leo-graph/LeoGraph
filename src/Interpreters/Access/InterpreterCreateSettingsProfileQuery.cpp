...
namespace {
void updateSettingsProfileFromQueryImpl(SettingsProfile &profile, const ASTCreateSettingsProfileQuery &query, const String &override_name,
...
BlockIO InterpreterCreateSettingsProfileQuery::execute() {
  ...
  if (settings_from_query && !query.attach) getContext()->checkSettingsConstraints(*settings_from_query, SettingSource::PROFILE);
  ...
  if (query.to_roles) roles_from_query = RolesOrUsersSet{*query.to_roles, access_control, getContext()->getUserID()};
    ...
    if (!query.storage_name.empty()) {
      for (const auto &name : query.names) {
        if (auto another_storage_ptr = access_control.findExcludingStorage(AccessEntityType::SETTINGS_PROFILE, name, storage_ptr))
          throw Exception(ErrorCodes::ACCESS_ENTITY_ALREADY_EXISTS, "Settings profile {} already exists in storage {}", name,
...
void InterpreterCreateSettingsProfileQuery::updateSettingsProfileFromQuery(SettingsProfile &SettingsProfile,
