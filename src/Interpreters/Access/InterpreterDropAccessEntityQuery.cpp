  ...
  if (query.type == AccessEntityType::USER) {
    ...
    for (const auto &name : query.names) {
      if (view_definer_dependencies.hasViewDependencies(name)) {
        ...
        for (const auto &id : views_storage_ids) views.push_back(id.getNameForLogs());
...
AccessRightsElements InterpreterDropAccessEntityQuery::getRequiredAccess() const {
  ...
  switch (query.type) {
    case AccessEntityType::USER: {
      for (const auto &name : query.names) res.emplace_back(AccessType::DROP_USER, name);
      return res;
    }
    case AccessEntityType::ROLE: {
      for (const auto &name : query.names) res.emplace_back(AccessType::DROP_ROLE, name);
      return res;
    }
