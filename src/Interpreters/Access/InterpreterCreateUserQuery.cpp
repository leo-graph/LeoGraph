...
namespace {
void updateUserFromQueryImpl(User &user, const ASTCreateUserQuery &query, const std::vector<AuthenticationData> authentication_methods,
                             ...
                             const std::optional<RolesOrUsersSet> &override_grantees, const std::optional<time_t> &global_valid_until,
                             bool reset_authentication_methods, bool replace_authentication_methods, bool allow_implicit_no_password,
                             bool allow_no_password, bool allow_plaintext_password, std::size_t max_number_of_authentication_methods) {
  if (override_name)
  ...
  if (query.remove_hosts) user.allowed_client_hosts.remove(*query.remove_hosts);
  if (query.add_hosts) user.allowed_client_hosts.add(*query.add_hosts);

  auto grant_roles = [&](const RolesOrUsersSet &roles_, bool as_default_role_) {
    if (as_default_role_ && (query.alter || roles_.all)) return;
  ...
  auto set_default_roles = [&](const RolesOrUsersSet &roles_) { InterpreterSetRoleQuery::updateUserSetDefaultRoles(user, roles_); };
  ...
  if (query.default_database) user.default_database = query.default_database->database_name;
...
BlockIO InterpreterCreateUserQuery::execute() {
  ...
  if (query.new_name && !query.alter) access->checkAccess(AccessType::CREATE_USER, *query.new_name);
  ...
  if (query.global_valid_until) global_valid_until = getValidUntilFromAST(query.global_valid_until, getContext());

  std::optional<RolesOrUsersSet> roles_from_query;
  if (query.roles) {
    roles_from_query = RolesOrUsersSet{*query.roles, access_control};
    chassert(!query.alter && !roles_from_query->all);
    for (const UUID &role : roles_from_query->getMatchingIDs()) access->checkAdminOption(role);
  }
  ...
  if (query.default_roles) {
    ...
    if (!query.alter && !default_roles_from_query->all) {
      for (const UUID &role : default_roles_from_query->getMatchingIDs()) access->checkAdminOption(role);
    }
  }
  ...
  if (settings_from_query && !query.attach) getContext()->checkSettingsConstraints(*settings_from_query, SettingSource::USER);

  if (!query.cluster.empty()) return executeDDLQueryOnCluster(updated_query_ptr, getContext());
  ...
  if (query.alter) {
    ...
    auto update_func = [&](const AccessEntityPtr &entity, const UUID &) -> AccessEntityPtr {
      ...
      updateUserFromQueryImpl(*updated_user, query, authentication_methods, {}, roles_from_query, default_roles_from_query,
                              settings_from_query, grantees_from_query, global_valid_until, query.reset_authentication_methods_to_new,
                              query.replace_authentication_methods, implicit_no_password_allowed, no_password_allowed,
                              plaintext_password_allowed,
                              getContext()->getServerSettings()[ServerSetting::max_authentication_methods_per_user]);
      return updated_user;
    };
    ...
    for (const auto &name : *query.names) {
      ...
      updateUserFromQueryImpl(*new_user, query, authentication_methods, name_with_host, roles_from_query, default_roles_from_query,
                              settings_from_query, RolesOrUsersSet::AllTag{}, global_valid_until, query.reset_authentication_methods_to_new,
                              query.replace_authentication_methods, implicit_no_password_allowed, no_password_allowed,
                              plaintext_password_allowed,
                              getContext()->getServerSettings()[ServerSetting::max_authentication_methods_per_user]);
    ...
    if (!query.storage_name.empty()) {
      for (const auto &name : names) {
        if (auto another_storage_ptr = access_control.findExcludingStorage(AccessEntityType::USER, name, storage_ptr))
          throw Exception(ErrorCodes::ACCESS_ENTITY_ALREADY_EXISTS, "User {} already exists in storage {}", name,
                          another_storage_ptr->getStorageName());
      }
    }
...
void InterpreterCreateUserQuery::updateUserFromQuery(User &user, const ASTCreateUserQuery &query, bool allow_no_password,
                                                     bool allow_plaintext_password, std::size_t max_number_of_authentication_methods) {
  std::vector<AuthenticationData> authentication_methods;
  if (!query.authentication_methods.empty()) {
    for (const auto &authentication_method_ast : query.authentication_methods) {
  ...
  std::optional<time_t> global_valid_until;
  if (query.global_valid_until) global_valid_until = getValidUntilFromAST(query.global_valid_until, {});

  updateUserFromQueryImpl(user, query, authentication_methods, {}, {}, {}, {}, {}, global_valid_until,
                          query.reset_authentication_methods_to_new, query.replace_authentication_methods, allow_no_password,
                          allow_plaintext_password, true, max_number_of_authentication_methods);
}
