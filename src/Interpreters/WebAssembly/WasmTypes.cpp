...
String formatFunctionDeclaration(const WasmFunctionDeclaration& wasm_func) {
  ...
  return fmt::format("{}({}) -> {}", wasm_func.getName(), fmt::join(wasm_func.getArgumentTypes(), ", "),
...
void checkFunctionDeclarationMatches(const WasmFunctionDeclaration& actual, const WasmFunctionDeclaration& expected) {
  ...
  if (actual.getReturnType() != expected.getReturnType())
    throw Exception(ErrorCodes::BAD_ARGUMENTS, "Function have unexpected return type: {}, expected {}", formatFunctionDeclaration(actual),
                    formatFunctionDeclaration(expected));
}
...
String toString(WebAssembly::WasmValKind kind) {
  switch (kind) {
#define M(T)                        \
  case WebAssembly::WasmValKind::T: \
    return #T;
