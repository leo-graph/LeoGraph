...
WasmMemoryGuard::WasmMemoryGuard(WasmMemoryGuard&& other) noexcept { *this = std::move(other); }
...
WasmMemoryGuard::~WasmMemoryGuard() { reset(); }
...
WasmMemoryGuard allocateInWasmMemory(const WasmMemoryManager* wmm, size_t size) {
  ...
  if (buf == 0) throw Exception(ErrorCodes::WASM_ERROR, "Cannot allocate buffer of size {}", size);
