...
struct WasmValTypeToIndex {
  ...
  static constexpr std::underlying_type_t<WasmValKind> value = std::conditional_t < std::is_same_v<T, TVal> ||
                                                               (sizeof(T) == sizeof(TVal) && is_integer<T> && is_integer<TVal>),
                                                       std::integral_constant<std::underlying_type_t<WasmValKind>, index>,
                                                       WasmValTypeToIndex<T, index + 1> > ::value;
};

template <typename T>
struct WasmValTypeToKind {
  static constexpr WasmValKind value = static_cast<WasmValKind>(WasmValTypeToIndex<T>::value);
};
template <typename T>
struct NativeToWasmType {
  using Type = std::variant_alternative_t<WasmValTypeToIndex<T>::value, WasmVal>;
};
...
class WasmFunctionDeclaration {
 public:
  WasmFunctionDeclaration(std::string_view function_name_, std::vector<WasmValKind> argument_types_,
                          std::optional<WasmValKind> return_type_)
      : function_name(std::move(function_name_)), argument_types(std::move(argument_types_)), return_type(return_type_) {}
