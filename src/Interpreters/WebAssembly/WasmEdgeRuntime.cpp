...
#  define M(T) \
    if (WasmEdge_ValTypeIs##T(val_type)) return WasmValKind::T;
...
#  define M(T) \
    if (k == WasmValKind::T) return WasmEdge_ValTypeGen##T();
...
#  define M(T)                                                                                         \
    {                                                                                                  \
      ...
      if (val.index() == Index) return WasmEdge_ValueGen##T(WasmEdgeNativeType(std::get<Index>(val))); \
    }
...
#  define WASM_EDGE_DELETER_TRAIT_SPECIALIZATION(T)                   \
    ...
    struct WasmEdgeDeleterTrait<WasmEdge_##T##Context> {              \
      ...
      void operator()(auto *ptr) const { WasmEdge_##T##Delete(ptr); } \
    };
...
auto WasmEdgeResourcePtrCreate(Args &&...args) {
  ...
  if (!resource) throw Exception(ErrorCodes::WASM_ERROR, "Cannot create {}", WasmEdgeDeleterTrait<ResourceT>::name);
...
void wasmedgeCheckResult(WasmEdge_Result result, const std::string_view &msg) {
  if (!WasmEdge_ResultOK(result)) throw Exception(ErrorCodes::WASM_ERROR, "{}: {}", msg, WasmEdge_ResultGetMessage(result));
}
...
struct WasmEdgeFunctionProps {
  ...
  WasmFunctionDeclaration getFunctionDeclaration() const {
    if (returns_count > 1) throw Exception(ErrorCodes::WASM_ERROR, "Function '{}' has more than one return value", function_name);
...
auto getWasmEdgeVmConfig(WasmModule::Config cfg) {
  ...
  if (!config) throw Exception(ErrorCodes::WASM_ERROR, "Cannot create WasmEdge config");
...
class HostFunctionAdapter : private boost::noncopyable {
 public:
  ...
  static WasmEdge_Result callFunction(void *payload [[maybe_unused]], const WasmEdge_CallingFrameContext *call_frame_ctx [[maybe_unused]],
  ...
  void linkTo(WasmEdge_ModuleInstanceContext *module_instance_ctx) {
    ...
    if (auto return_type = func_decl.getReturnType()) returns.push_back(toWasmEdgeValueType(*return_type));

    auto func_type = WasmEdgeResourcePtrCreate<WasmEdge_FunctionTypeCreate>(params.data(), static_cast<uint32_t>(params.size()),
...
WasmEdge_Result HostFunctionAdapter::callFunction(void *payload [[maybe_unused]],
  ...
  try {
    ...
    for (size_t i = 0; i < argument_types.size(); ++i) {
      ...
      if (getWasmValKind(args[i]) != argument_types[i])
        throw Exception(ErrorCodes::WASM_ERROR, "Function {} invoked with wrong argument types [{}]", formatFunctionDeclaration(func_decl),
                        fmt::join(args | std::views::transform(getWasmValKind), ", "));
    }

    auto result_val = host_func(compartment, args);
    if (result_val) out[0] = toWasmEdgeValue(*result_val);
  } catch (Exception &e) {
...
void WasmEdgeCompartment::loadModuleImpl() {
  ...
  if (!vm_instance_cxt) throw Exception(ErrorCodes::WASM_ERROR, "Cannot get active module");
  ...
  for (size_t i = 0; i < number_of_functions; ++i) {
    ...
    if (!inserted) throw Exception(ErrorCodes::WASM_ERROR, "Module has multiple '{}' functions", func_name);
  }
}
...
std::span<uint8_t> WasmEdgeCompartment::getMemory(WasmPtr ptr, WasmSizeT size) {
  ...
  if (memory_ctx == nullptr) throw Exception(ErrorCodes::WASM_ERROR, "Cannot find memory export in wasm module");
  ...
  if (data == nullptr) {
    ...
    throw Exception(ErrorCodes::WASM_ERROR, "Cannot get memory at offset {} and size {} from wasm module with size {}", ptr, size,
                    total_memory);
  }
...
std::vector<WasmVal> WasmEdgeCompartment::invokeImpl(std::string_view function_name, const std::vector<WasmVal> &params,
  ...
  if (func_it == imported_functions.end()) throw Exception(ErrorCodes::WASM_ERROR, "Function '{}' is not found in module", function_name);
  ...
  if (params_count != params.size())
    throw Exception(ErrorCodes::WASM_ERROR,
                    ...
                    function_name, params.size(), params_count, formatFunctionDeclaration(func_it->second.getFunctionDeclaration()));

  std::vector<WasmEdge_Value> params_values(params.size());
  for (size_t i = 0; i < params.size(); ++i) params_values[i] = toWasmEdgeValue(params[i]);
  ...
  std::vector<WasmEdge_Value> returns_values(returns_count);
  {
    ...
    WasmEdge_Result result = WasmEdge_VMExecute(vm_cxt.get(), wasmedgeStringWrap(function_name), params_values.data(),
                                                static_cast<uint32_t>(params_values.size()), returns_values.data(),
                                                static_cast<uint32_t>(returns_values.size()));

    if (last_exception) last_exception->rethrow();
...
class WasmEdgeModule : public WasmModule {
 public:
  explicit WasmEdgeModule(WasmEdge_ASTModuleContext *ast_module_ptr) : ast_module(ast_module_ptr) {
    ...
    if (exports_length >= 512) throw Exception(ErrorCodes::WASM_ERROR, "Module has too many exports");
    ...
    for (const auto *export_ctx : exports_list) {
      ...
      if (export_name.Length == 0) throw Exception(ErrorCodes::WASM_ERROR, "Cannot get export name");
      auto external_type = WasmEdge_ExportTypeGetExternalType(export_ctx);
      if (external_type == WasmEdge_ExternalType_Function) exports.emplace(std::string(export_name.Buf, export_name.Length), export_ctx);
    }
  }
  ...
  std::unique_ptr<WasmCompartment> instantiate(Config cfg) const override {
    ...
    for (const auto &host_function : host_functions) compartment->addHostFunction(&host_function);
  ...
  std::vector<WasmFunctionDeclaration> getImports() const override {
    ...
    for (const auto *import_ctx : imports) {
      ...
      if (import_name.Length == 0) throw Exception(ErrorCodes::WASM_ERROR, "Cannot get import name");

      const auto *function_type = WasmEdge_ImportTypeGetFunctionType(ast_module.get(), import_ctx);
      if (!function_type)
        throw Exception(ErrorCodes::WASM_ERROR, "Cannot get function for import '{}'",
                        std::string_view(import_name.Buf, import_name.Length));
  ...
  void linkFunction(WasmHostFunction import_host_function) override { host_functions.push_back(std::move(import_host_function)); }
  ...
  WasmFunctionDeclaration getExport(std::string_view function_name) const override {
    ...
    if (!function_type) throw Exception(ErrorCodes::WASM_ERROR, "Cannot get function for export '{}'", function_name);
...
WasmEdgeRuntime::WasmEdgeRuntime() { setLogLevel(LogsLevel::warning); }
...
std::unique_ptr<WasmModule> WasmEdgeRuntime::compileModule(std::string_view wasm_code) const {
  ...
  if (!ast_module_ptr) throw Exception(ErrorCodes::WASM_ERROR, "Cannot parse module");
