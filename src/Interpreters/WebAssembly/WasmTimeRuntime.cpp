...
#  define M(T) \
    if (value == WasmValKind::T) return wasmtime::ValKind::T;
...
#  define M(T) \
    if (wasmtime::ValKind::T == val_type) return WasmValKind::T;
...
#  define M(T)                                                                                   \
    {                                                                                            \
      ...
      constexpr auto Index = std::to_underlying(WasmValKind::T);                                 \
      if (val.index() == Index) return wasmtime::Val(std::bit_cast<Type>(std::get<Index>(val))); \
    }
...
WasmTimeRuntime::WasmTimeRuntime() : impl(std::make_unique<Impl>()) { setLogLevel(LogsLevel::warning); }

void WasmTimeRuntime::setLogLevel(LogsLevel) {}

class WasmTimeCompartment : public WasmCompartment {
 public:
  explicit WasmTimeCompartment(const wasmtime::Engine &engine_, wasmtime::Store &&wasm_store, wasmtime::Instance &&instance_,
                               WasmModule::Config cfg_)
      : engine(engine_), store(std::move(wasm_store)), instance(std::move(instance_)), cfg(std::move(cfg_)) {
    ...
    store.epoch_deadline_callback(
        [](wasmtime::Store::Context ctx, uint64_t &epoch_deadline_delta) -> wasmtime::Result<wasmtime::DeadlineKind> {
          ...
          if (compartment_ptr && compartment_ptr->stop_requested.load()) return wasmtime::Error("WASM execution was stopped by request");
  ...
  std::span<uint8_t> getMemory(WasmPtr ptr, WasmSizeT size) override {
    ...
    if (ptr + size >= memory_span.size()) {
      throw Exception(ErrorCodes::WASM_ERROR, "Cannot get memory at offset {} and size {} from wasm compartment memory with size {}", ptr,
                      size, memory_span.size());
    }
  ...
  std::vector<WasmVal> invokeImpl(std::string_view function_name, const std::vector<WasmVal> &params, StopToken stop_token) override {
    {
      ...
      if (!result) throw Exception(ErrorCodes::WASM_ERROR, "Failed to set fuel to wasm instance: {}", result.err().message());
    }
    ...
    if (params_count != params.size())
      throw Exception(ErrorCodes::WASM_ERROR,
                      ...
                      "expected {}",
                      function_name, params.size(), params_count);

    std::vector<wasmtime::Val> params_values;
    params_values.reserve(params.size());
    for (auto param : params) params_values.emplace_back(toWasmTimeValue(param));
...
wasmtime::Result<std::monostate, wasmtime::Trap> callHostFunction(WasmTimeCompartment *compartment,
  ...
  try {
    ...
    if (argument_types.size() != params.size()) {
      throw Exception(ErrorCodes::WASM_ERROR, "Function {} was called from wasm with different number of arguments {} != {}",
                      formatFunctionDeclaration(func_decl), params.size(), argument_types.size());
    }
    ...
    if (result_val) {
      if (results.size() != 1) {
        throw Exception(ErrorCodes::WASM_ERROR, "Function {} invoked with different number of return values 1 != {}",
                        formatFunctionDeclaration(func_decl), results.size());
      }
...
class WasmTimeModule : public WasmModule {
 public:
  explicit WasmTimeModule(wasmtime::Engine engine_, wasmtime::Module &&module_) : engine(std::move(engine_)), module(std::move(module_)) {
    all_exports_list = module.exports();
    if (all_exports_list.size() >= 512) throw Exception(ErrorCodes::WASM_ERROR, "Module has too many exports");
    ...
    if (all_imports_list.size() >= 512) throw Exception(ErrorCodes::WASM_ERROR, "Module has too many imports");
  ...
  std::unique_ptr<WasmCompartment> instantiate(Config cfg) const override {
    ...
    if (cfg.memory_limit) store.limiter(cfg.memory_limit, -1, -1, -1, -1);

    {
      auto result = store.context().set_fuel(cfg.fuel_limit ? cfg.fuel_limit : std::numeric_limits<uint64_t>::max());
      if (!result) throw Exception(ErrorCodes::WASM_ERROR, "Failed to set fuel for wasm module instantiation: {}", result.err().message());
    }

    wasmtime::Linker linker(engine);
    for (const auto &host_function : host_functions) {
      const auto &func_decl = host_function.getFunctionDeclaration();
      auto add_host_func_result =
          linker.func_new("env", func_decl.getName(), toWasmFunctionType(func_decl),
                          [host_function_raw_ptr = &host_function](
                              wasmtime::Caller caller, wasmtime::Span<const wasmtime::Val> params,
      ...
      if (!add_host_func_result) {
        throw Exception(ErrorCodes::WASM_ERROR, "Failed to add host function to module instance: {}", add_host_func_result.err().message());
      }
    }
  ...
  void linkFunction(WasmHostFunction import_host_function) override { host_functions.emplace_back(std::move(import_host_function)); }
