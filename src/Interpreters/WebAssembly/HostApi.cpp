...
Int64 wasmExportServerVer(WasmCompartment *) { return static_cast<UInt64>(VERSION_INTEGER); }
...
void wasmExportRandom(WasmCompartment *compartment, WasmPtr wasm_ptr, WasmSizeT size) {
  ...
  for (WasmSizeT i = 0; i < size; ++i) data[i] = static_cast<ValueType>(dist(thread_local_rng));
}
 ...
 public:
  ...
  std::optional<WasmVal> operator()(WasmCompartment *compartment, std::span<WasmVal> args) const {
    if (args.size() != sizeof...(Args))
      throw Exception(ErrorCodes::WASM_ERROR, "WebAssembly function '{}' expects {} arguments, got {}", function_name, sizeof...(Args),
                      args.size());
...
WasmHostFunction makeHostFunction(std::string_view function_name, ReturnType (*host_function)(WasmCompartment *, Args...)) {
  ...
  WasmFunctionDeclaration func_decl(function_name, WasmHostFunctionAdapter<FuncPtr>::getArgumentTypes(),
...
WasmHostFunction getHostFunction(std::string_view function_name) {
  ...
  for (const auto &function : exported_functions) {
    if (function.getFunctionDeclaration().getName() == function_name) return function;
  }
