...
bool allowEarlyConstantFolding(const ActionsDAG &actions, const Settings &settings) {
  if (!settings[Setting::enable_early_constant_folding]) return false;

  for (const auto &node : actions.getNodes()) {
    if (node.type == ActionsDAG::ActionType::FUNCTION && node.function_base) {
      if (!node.function_base->isSuitableForConstantFolding()) return false;
    }
  }
  return true;
}
      ...
      size_limits_for_set_used_with_index(
          (settings_[Setting::use_index_for_in_with_subqueries_max_values] &&
           settings_[Setting::use_index_for_in_with_subqueries_max_values] < settings_[Setting::max_rows_in_set])
              ? size_limits_for_set
              : SizeLimits(settings_[Setting::use_index_for_in_with_subqueries_max_values], settings_[Setting::max_bytes_in_set],
                           OverflowMode::BREAK)),
...
ExpressionAnalyzer::ExpressionAnalyzer(const ASTPtr &query_, const TreeRewriterResultPtr &syntax_analyzer_result_, ContextPtr context_,
                                       size_t subquery_depth_, bool do_global, bool is_explain, PreparedSetsPtr prepared_sets_,
...
NamesAndTypesList ExpressionAnalyzer::getColumnsAfterArrayJoin(ActionsDAG &actions, const NamesAndTypesList &src_columns) {
  ...
  for (const auto &column : src_columns)
    if (!added_columns.contains(column.name)) new_columns_after_array_join.emplace_back(column.name, column.type);
...
void ExpressionAnalyzer::analyzeAggregation(ActionsDAG &temp_actions) {
  ...
  if (select_query) {
    if (ASTPtr group_by_ast = select_query->groupBy()) {
      ...
      if (group_by_kind != GroupByKind::ORDINARY) aggregated_columns.emplace_back("__grouping_set", std::make_shared<DataTypeUInt64>());
      ...
      for (ssize_t i = 0; i < static_cast<ssize_t>(group_asts.size()); ++i) {
        ...
        if (select_query->group_by_with_grouping_sets) {
          ...
          for (ssize_t j = 0; j < ssize_t(group_elements_ast.size()); ++j) {
            ...
            if (!node) throw Exception(ErrorCodes::UNKNOWN_IDENTIFIER, "Unknown identifier (in GROUP BY): {}", column_name);
            ...
            if (getContext()->getClientInfo().distributed_depth == 0 || settings.distributed_group_by_no_merge > 0) {
              ...
              if (node->column && isColumnConst(*node->column)) {
                ...
                if (!aggregate_descriptions.empty() || group_size > 1) {
                  if (j + 1 < group_size) group_elements_ast[j] = std::move(group_elements_ast.back());
          ...
          if (getContext()->getClientInfo().distributed_depth == 0 || settings.distributed_group_by_no_merge > 0) {
            ...
            if (node->column && isColumnConst(*node->column)) {
              ...
              if (!aggregate_descriptions.empty() || size > 1) {
                if (i + 1 < size) group_asts[i] = std::move(group_asts.back());
      ...
      if (!select_query->group_by_with_grouping_sets) {
        ...
        for (size_t i = 0; i < aggregation_keys.size(); ++i) list.push_back(i);
      }
  ...
  for (const auto &desc : aggregate_descriptions) aggregated_columns.emplace_back(desc.column_name, desc.function->getResultType());
}

void ExpressionAnalyzer::initGlobalSubqueriesAndExternalTables(bool do_global, bool is_explain) {
  if (do_global) {
    GlobalSubqueriesVisitor::Data subqueries_data(getContext(), subquery_depth, isRemoteStorage(), is_explain, external_tables,
                                                  prepared_sets, has_global_subqueries, syntax->analyzed_join.get());
...
SetPtr ExpressionAnalyzer::isPlainStorageSetInSubquery(const ASTPtr &subquery_or_table_name) {
  ...
  if (storage->getName() != "Set") return nullptr;
...
void ExpressionAnalyzer::getRootActions(const ASTPtr &ast, bool no_makeset_for_subqueries, ActionsDAG &actions, bool only_consts) {
  LogAST log;
  ActionsVisitor::Data visitor_data(getContext(), settings.size_limits_for_set, subquery_depth, sourceColumns(), std::move(actions),
                                    prepared_sets, no_makeset_for_subqueries, false /* no_makeset */, only_consts, getAggregationKeysInfo(),
                                    false /* build_expression_with_window_functions */, is_create_parameterized_view);
...
void ExpressionAnalyzer::getRootActionsNoMakeSet(const ASTPtr &ast, ActionsDAG &actions, bool only_consts) {
  LogAST log;
  ActionsVisitor::Data visitor_data(getContext(), settings.size_limits_for_set, subquery_depth, sourceColumns(), std::move(actions),
                                    prepared_sets, true /* no_makeset_for_subqueries, no_makeset implies no_makeset_for_subqueries */,
                                    true /* no_makeset */, only_consts, getAggregationKeysInfo(),
                                    false /* build_expression_with_window_functions */, is_create_parameterized_view);
  ActionsVisitor(visitor_data, log.stream()).visit(ast);
  actions = visitor_data.getActions();
}

void ExpressionAnalyzer::getRootActionsForHaving(const ASTPtr &ast, bool no_makeset_for_subqueries, ActionsDAG &actions, bool only_consts) {
  LogAST log;
  ActionsVisitor::Data visitor_data(getContext(), settings.size_limits_for_set, subquery_depth, sourceColumns(), std::move(actions),
                                    prepared_sets, no_makeset_for_subqueries, false /* no_makeset */, only_consts, getAggregationKeysInfo(),
                                    false /* build_expression_with_window_functions */, is_create_parameterized_view);
...
void ExpressionAnalyzer::getRootActionsForWindowFunctions(const ASTPtr &ast, bool no_makeset_for_subqueries, ActionsDAG &actions) {
  LogAST log;
  ActionsVisitor::Data visitor_data(getContext(), settings.size_limits_for_set, subquery_depth, sourceColumns(), std::move(actions),
                                    prepared_sets, no_makeset_for_subqueries, false /* no_makeset */, false /*only_consts */,
                                    getAggregationKeysInfo(), true);
  ActionsVisitor(visitor_data, log.stream()).visit(ast);
  actions = visitor_data.getActions();
}
...
void ExpressionAnalyzer::makeAggregateDescriptions(ActionsDAG &actions, AggregateDescriptions &descriptions) {
  for (const ASTPtr &ast : aggregates()) {
    ...
    if (node.arguments) getRootActionsNoMakeSet(node.arguments, actions);
    ...
    for (size_t i = 0; i < arguments.size(); ++i) {
      ...
      if (!dag_node) {
        throw Exception(ErrorCodes::UNKNOWN_IDENTIFIER, "Unknown identifier '{}' in aggregate function '{}'", name,
...
void ExpressionAnalyzer::makeWindowDescriptionFromAST(const Context &context_, const WindowDescriptions &existing_descriptions,
                                                      AggregateFunctionPtr aggregate_function, WindowDescription &desc, const IAST *ast) {
  ...
  if (!definition.parent_window_name.empty()) {
    ...
    if (it == existing_descriptions.end()) {
      throw Exception(ErrorCodes::BAD_ARGUMENTS, "Window definition '{}' references an unknown window '{}'",
                      definition.formatForErrorMessage(), definition.parent_window_name);
    }
    ...
    if (definition.partition_by) {
      throw Exception(ErrorCodes::BAD_ARGUMENTS, "Derived window definition '{}' is not allowed to override PARTITION BY",
                      definition.formatForErrorMessage());
    }

    if (definition.order_by && !parent.order_by.empty()) {
      throw Exception(ErrorCodes::BAD_ARGUMENTS, "Derived window definition '{}' is not allowed to override a non-empty ORDER BY",
                      definition.formatForErrorMessage());
    }
    ...
    if (!parent.frame.is_default) {
      throw Exception(ErrorCodes::BAD_ARGUMENTS,
                      ...
                      definition.parent_window_name, definition.formatForErrorMessage());
    }
  }
  ...
  if (definition.order_by) {
    for (const auto &column_ast : definition.order_by->children) {
      ...
      desc.order_by.push_back(SortColumnDescription(order_by_element.children.front()->getColumnName(), order_by_element.direction,
  ...
  if (definition.frame_type != WindowFrame::FrameType::ROWS && definition.frame_type != WindowFrame::FrameType::RANGE) {
    throw Exception(ErrorCodes::NOT_IMPLEMENTED, "Window frame '{}' is not implemented (while processing '{}')", definition.frame_type,
...
void ExpressionAnalyzer::makeWindowDescriptions(ActionsDAG &actions) {
  ...
  if (select_query && select_query->window()) {
    for (const auto &ptr : select_query->window()->children) {
      ...
      makeWindowDescriptionFromAST(*current_context, window_descriptions, nullptr, desc, elem.definition.get());

      auto [it, inserted] = window_descriptions.insert({elem.name, std::move(desc)});

      if (!inserted) {
        throw Exception(ErrorCodes::BAD_ARGUMENTS, "Window '{}' is defined twice in the WINDOW clause", elem.name);
      }
    }
  }
  ...
  for (const ASTPtr &ast : syntax->window_function_asts) {
    ...
    for (size_t i = 0; i < arguments.size(); ++i) {
      ...
      if (!node) {
        throw Exception(ErrorCodes::UNKNOWN_IDENTIFIER, "Unknown identifier '{}' in window function '{}'", name,
    ...
    window_function.aggregate_function =
        AggregateFunctionFactory::instance().get(window_function.function_node->name, window_function.function_node->getNullsAction(),
                                                 window_function.argument_types, window_function.function_parameters, properties);
    ...
    if (!function_node.window_name.empty()) {
      ...
      if (it == std::end(window_descriptions)) {
        throw Exception(ErrorCodes::UNKNOWN_IDENTIFIER, "Window '{}' is not defined (referenced by '{}')", function_node.window_name,
      ...
      makeWindowDescriptionFromAST(*current_context, window_descriptions, window_function.aggregate_function, desc, &definition);

      auto full_sort_description = desc.full_sort_description;

      auto [it, inserted] = window_descriptions.insert({desc.window_name, std::move(desc)});
...
const ASTSelectQuery *ExpressionAnalyzer::getSelectQuery() const {
  const auto *select_query = query->as<ASTSelectQuery>();
  if (!select_query) throw Exception(ErrorCodes::LOGICAL_ERROR, "Not a select query");
  return select_query;
}
...
const ASTSelectQuery *SelectQueryExpressionAnalyzer::getAggregatingQuery() const {
  if (!has_aggregation) throw Exception(ErrorCodes::LOGICAL_ERROR, "No aggregation");
...
std::optional<ArrayJoin> SelectQueryExpressionAnalyzer::appendArrayJoin(ExpressionActionsChain &chain,
                                                                        ActionsAndProjectInputsFlagPtr &before_array_join,
                                                                        bool only_types) {
  const auto *select_query = getSelectQuery();

  auto [array_join_expression_list, is_array_join_left] = select_query->arrayJoinExpressionList();
  if (!array_join_expression_list) return {};
...
JoinPtr SelectQueryExpressionAnalyzer::appendJoin(ExpressionActionsChain &chain, ActionsAndProjectInputsFlagPtr &converting_join_columns) {
  ...
  ExpressionActionsChainSteps::Step &step = chain.lastStep(columns_after_array_join);
  chain.steps.push_back(std::make_unique<ExpressionActionsChainSteps::JoinStep>(syntax->analyzed_join, join, step.getResultColumns()));
  chain.addStep();
  return join;
}
...
static std::shared_ptr<IJoin> tryCreateJoin(JoinAlgorithm algorithm, std::shared_ptr<TableJoin> analyzed_join,
                                            const ColumnsWithTypeAndName &left_sample_columns, SharedHeader right_sample_block,
                                            std::unique_ptr<QueryPlan> &joined_plan, ContextPtr context) {
  if (analyzed_join->kind() == JoinKind::Paste) return std::make_shared<PasteJoin>(analyzed_join, right_sample_block);
  ...
  if (algorithm == JoinAlgorithm::PARTIAL_MERGE || algorithm == JoinAlgorithm::PREFER_PARTIAL_MERGE) {
    if (MergeJoin::isSupported(analyzed_join)) return std::make_shared<MergeJoin>(analyzed_join, right_sample_block);
  }
  ...
  if (algorithm == JoinAlgorithm::HASH ||
    ...
    if (analyzed_join->allowParallelHashJoin())
      return std::make_shared<ConcurrentHashJoin>(analyzed_join, settings[Setting::max_threads], right_sample_block,
                                                  StatsCollectingParams{});
    return std::make_shared<HashJoin>(analyzed_join, right_sample_block);
  }

  if (algorithm == JoinAlgorithm::FULL_SORTING_MERGE) {
    if (FullSortingMergeJoin::isSupported(analyzed_join)) return std::make_shared<FullSortingMergeJoin>(analyzed_join, right_sample_block);
  }
  ...
  if (algorithm == JoinAlgorithm::GRACE_HASH) {
    ...
    if (sanitizeBlock(left_sample_block, false) && GraceHashJoin::isSupported(analyzed_join))
      return std::make_shared<GraceHashJoin>(context->getSettingsRef()[Setting::grace_hash_join_initial_buckets],
                                             context->getSettingsRef()[Setting::grace_hash_join_max_buckets], analyzed_join,
                                             std::make_shared<const Block>(std::move(left_sample_block)), right_sample_block,
                                             context->getTempDataOnDisk());
  }

  if (algorithm == JoinAlgorithm::AUTO) {
    if (MergeJoin::isSupported(analyzed_join)) return std::make_shared<JoinSwitcher>(analyzed_join, right_sample_block);
    return std::make_shared<HashJoin>(analyzed_join, right_sample_block);
  }
  return nullptr;
}

static std::shared_ptr<IJoin> chooseJoinAlgorithm(std::shared_ptr<TableJoin> analyzed_join,
...
static std::unique_ptr<QueryPlan> buildJoinedPlan(ContextPtr context, PreparedSetsPtr prepared_sets,
                                                  const ASTTablesInSelectQueryElement &join_element, TableJoin &analyzed_join,
                                                  SelectQueryOptions query_options) {
  /// Actions which need to be calculated on joined block.
  auto joined_block_actions = analyzed_join.createJoinedBlockActions(context, std::move(prepared_sets));
  NamesWithAliases required_columns_with_aliases = analyzed_join.getRequiredColumns(Block(joined_block_actions.getResultColumns()),
                                                                                    joined_block_actions.getRequiredColumns().getNames());

  Names original_right_column_names;
  for (auto &pr : required_columns_with_aliases) original_right_column_names.push_back(pr.first);
   ...
   */
  auto interpreter = interpretSubquery(join_element.table_expression, context, original_right_column_names,
...
std::shared_ptr<DirectKeyValueJoin> tryKeyValueJoin(std::shared_ptr<TableJoin> analyzed_join, const Block &right_sample_block) {
  ...
  bool only_one_key = clauses.size() == 1 && clauses[0].key_names_left.size() == 1 && clauses[0].key_names_right.size() == 1 &&
  ...
  if (storage_primary_key.size() != 1 || storage_primary_key[0] != original_key_name) {
    LOG_TRACE(getLogger(), "Can't use direct join: join key '{}' doesn't match to storage key ({})", original_key_name,
              fmt::join(storage_primary_key, ", "));
    return nullptr;
  }
...
JoinPtr SelectQueryExpressionAnalyzer::makeJoin(const ASTTablesInSelectQueryElement &join_element,
                                                const ColumnsWithTypeAndName &left_columns,
                                                std::optional<ActionsDAG> &left_convert_actions) {
  /// Two JOINs are not supported with the same subquery, but different USINGs.

  if (joined_plan) throw Exception(ErrorCodes::LOGICAL_ERROR, "Table join was already created for query");
  ...
  if (auto storage = analyzed_join->getStorageJoin()) {
    ...
    std::tie(left_convert_actions, right_convert_actions) =
        analyzed_join->createConvertingActions(left_columns, right_columns, getContext());
...
ActionsAndProjectInputsFlagPtr SelectQueryExpressionAnalyzer::appendPrewhere(ExpressionActionsChain &chain, bool only_types) {
  const auto *select_query = getSelectQuery();
  if (!select_query->prewhere()) return {};

  Names first_action_names;
  if (!chain.steps.empty()) first_action_names = chain.steps.front()->getRequiredColumns().getNames();
  ...
  ActionsAndProjectInputsFlagPtr prewhere_actions;
  {
    ...
    for (const auto &column : sourceColumns())
      if (!required_source_columns.contains(column.name)) name_set.erase(column.name);
...

  {
    ...
    for (const auto &col : required_columns) prewhere_input_names.insert(col.name);
    ...
    if (storage() && getSelectQuery()->final()) {
      for (const auto &column : metadata_snapshot->getColumns().getOrdinary()) {
        if (!prewhere_input_names.contains(column.name)) continue;
    ...
    chain.steps.emplace_back(std::make_unique<ExpressionActionsChainSteps::ExpressionActionsStep>(std::move(actions)));
...
bool SelectQueryExpressionAnalyzer::appendWhere(ExpressionActionsChain &chain, bool only_types) {
  ...
  if (!select_query->where()) return false;
...
bool SelectQueryExpressionAnalyzer::appendGroupBy(ExpressionActionsChain &chain, bool only_types, bool optimize_aggregation_in_order,
  ...
  if (!select_query->groupBy()) return false;
  ...
  for (const auto &result_column : step.getResultColumns()) {
    if (group_by_keys.contains(result_column.name)) validateGroupByKeyType(result_column.type);
  }
...
void SelectQueryExpressionAnalyzer::validateGroupByKeyType(const DB::DataTypePtr &key_type) const {
  if (getContext()->getSettingsRef()[Setting::allow_suspicious_types_in_group_by]) return;
...
void SelectQueryExpressionAnalyzer::appendAggregateFunctionsArguments(ExpressionActionsChain &chain, bool only_types) {
  ...
  for (const auto &desc : aggregate_descriptions)
    for (const auto &name : desc.argument_names) step.addRequiredOutput(name);
  ...
  if (select_query->having()) GetAggregatesVisitor(data).visit(select_query->having());

  if (select_query->orderBy()) GetAggregatesVisitor(data).visit(select_query->orderBy());

  /// TODO: data.aggregates -> aggregates()
  for (const ASTPtr &ast : data.aggregates) {
    const ASTFunction &node = typeid_cast<const ASTFunction &>(*ast);
    if (node.arguments)
      for (auto &argument : node.arguments->children) getRootActions(argument, only_types, step.actions()->dag);
  }
}
...
void SelectQueryExpressionAnalyzer::appendGroupByModifiers(ActionsDAG &before_aggregation, ExpressionActionsChain &chain,
                                                           bool /* only_types */) {
  const auto *select_query = getAggregatingQuery();

  if (!select_query->groupBy() || !(select_query->group_by_with_rollup || select_query->group_by_with_cube)) return;
  ...
  step.actions()->dag =
      ActionsDAG::makeConvertingActions(source_columns, result_columns, ActionsDAG::MatchColumnsMode::Position, getContext());
}
...
void SelectQueryExpressionAnalyzer::appendSelectSkipWindowExpressions(ExpressionActionsChainSteps::Step &step, ASTPtr const &node) {
  if (auto *function = node->as<ASTFunction>()) {
    ...
    if (function->isWindowFunction() || function->name == "lambda") return;
    if (function->computeAfterWindowFunctions()) {
      for (auto &arg : function->arguments->children) appendSelectSkipWindowExpressions(step, arg);
      return;
    }
  }
...
bool SelectQueryExpressionAnalyzer::appendHaving(ExpressionActionsChain &chain, bool only_types) {
  ...
  if (!select_query->having()) return false;
...
void SelectQueryExpressionAnalyzer::appendSelect(ExpressionActionsChain &chain, bool only_types) {
  ...
  for (const auto &child : select_query->select()->children) appendSelectSkipWindowExpressions(step, child);
}

ActionsAndProjectInputsFlagPtr SelectQueryExpressionAnalyzer::appendOrderBy(ExpressionActionsChain &chain, bool only_types,
                                                                            bool optimize_read_in_order,
  ...
  for (auto &child : select_query->orderBy()->children) {
    ...
    if (!ast || ast->children.empty()) throw Exception(ErrorCodes::UNKNOWN_TYPE_OF_AST_NODE, "Bad ORDER BY expression AST");
  ...
  for (auto &child : select_query->orderBy()->children) {
    ...
    if (ast->with_fill) with_fill = true;
  }

  for (const auto &result_column : step.getResultColumns()) {
    if (order_by_keys.contains(result_column.name)) validateOrderByKeyType(result_column.type);
  }

  if (auto interpolate_list = select_query->interpolate()) {
    NameSet select;
    for (const auto &child : select_query->select()->children) select.insert(child->getAliasOrColumnName());
    ...
    auto find_columns = [&step, &select, &required_by_interpolate](IAST *function) {
      auto f_impl = [&step, &select, &required_by_interpolate](IAST *fn, auto fi) {
        if (auto *ident = fn->as<ASTIdentifier>()) {
          ...
          if (!select.contains(ident->getColumnName())) step.addRequiredOutput(ident->getColumnName());
          return;
        }
        if (fn->as<ASTFunction>() || fn->as<ASTExpressionList>())
          for (const auto &ch : fn->children) fi(ch.get(), fi);
        return;
      };
      f_impl(function, f_impl);
    };

    for (const auto &interpolate : interpolate_list->children) find_columns(interpolate->as<ASTInterpolateElement>()->expr.get());
    ...
    if (!required_result_columns.empty()) {
      ...
      for (const auto &name : required_by_interpolate)
        if (!required_result_columns_set.contains(name)) required_result_columns.push_back(name);
    }
  }
  ...
  if (with_fill) {
    for (const auto &column : step.getResultColumns()) non_constant_inputs.insert(column.name);
  }
...
bool SelectQueryExpressionAnalyzer::appendLimitBy(ExpressionActionsChain &chain, bool only_types) {
  ...
  if (!select_query->limitBy()) return false;
  ...
  for (auto &child : children) {
    ...
    if (!existing_column_names.contains(child_name)) step.addRequiredOutput(child_name);
  }
...
ActionsAndProjectInputsFlagPtr SelectQueryExpressionAnalyzer::appendProjectResult(ExpressionActionsChain &chain) const {
  ...
  if (!required_result_columns.empty()) {
    ...
    for (const auto &column : required_result_columns) result_columns.emplace_back(column, std::string{});
    actions->dag.project(result_columns);
  }
...
ExpressionActionsPtr ExpressionAnalyzer::getActions(bool add_aliases, bool remove_unused_result, CompileExpressions compile_expressions) {
  return std::make_shared<ExpressionActions>(getActionsDAG(add_aliases, remove_unused_result),
                                             ExpressionActionsSettings(getContext(), compile_expressions),
                                             add_aliases && remove_unused_result);
}
...
std::unique_ptr<QueryPlan> SelectQueryExpressionAnalyzer::getJoinedPlan() { return std::move(joined_plan); }
...
ExpressionAnalysisResult::ExpressionAnalysisResult(SelectQueryExpressionAnalyzer &query_analyzer,
                                                   const StorageMetadataPtr &metadata_snapshot, bool first_stage_, bool second_stage_,
                                                   bool only_types, const FilterDAGInfoPtr &row_policy_info_,
                                                   const FilterDAGInfoPtr &additional_filter, const Block &source_header)
  ...
  auto finalize_chain = [&](ExpressionActionsChain &chain) -> ColumnsWithTypeAndName {
    if (prewhere_step_num >= 0) {
      ...
      for (const auto &column : additional_required_columns_after_prewhere) {
        if (required_source_columns.contains(column)) step.addRequiredOutput(column);
      }
    }
...

  {
    ...
    if (storage && (query.sampleSize() || settings[Setting::parallel_replicas_count] > 1)) {
      ...
      if (storage->getName() == "Merge") {
        ...
        additional_required_columns_after_prewhere.insert(additional_required_columns_after_prewhere.end(), columns_for_sampling.begin(),
    ...
    if (storage && query.final()) {
      ...
      additional_required_columns_after_prewhere.insert(additional_required_columns_after_prewhere.end(), columns_for_final.begin(),
                                                        columns_for_final.end());
    }
    ...
    if (prewhere_dag_and_flags = query_analyzer.appendPrewhere(chain, !first_stage); prewhere_dag_and_flags) {
      ...
      if (allowEarlyConstantFolding(prewhere_dag_and_flags->dag, settings)) {
        ...
        if (sanitizeBlock(before_prewhere_sample)) {
          ExpressionActions(prewhere_dag_and_flags->dag.clone(), ExpressionActionsSettings(context->getSettingsRef()))
              .execute(before_prewhere_sample);
          auto &column_elem = before_prewhere_sample.getByName(query.prewhere()->getColumnName());
          /// If the filter column is a constant, record it.
          if (column_elem.column) prewhere_constant_filter_description = ConstantFilterDescription(*column_elem.column);
        }
      }
    }
    ...
    if (query_analyzer.appendWhere(chain, only_types || !first_stage)) {
      ...
      if (allowEarlyConstantFolding(before_where->dag, settings)) {
        ...
        if (sanitizeBlock(before_where_sample)) {
          ...
          if (!extracting_subcolumns_dag.getNodes().empty()) dag = ActionsDAG::merge(std::move(extracting_subcolumns_dag), std::move(dag));

          ExpressionActions(std::move(dag), ExpressionActionsSettings(context->getSettingsRef())).execute(before_where_sample);

          auto &column_elem = before_where_sample.getByName(query.where()->getColumnName());
          /// If the filter column is a constant, record it.
          if (column_elem.column) where_constant_filter_description = ConstantFilterDescription(*column_elem.column);
        }
      }
      chain.addStep();
    }
    ...
    if (need_aggregate) {
      ...
      if (settings[Setting::group_by_use_nulls]) query_analyzer.appendGroupByModifiers(before_aggregation->dag, chain, only_types);
      ...
      /// * or remove ExpressionActionsChain completely and re-implement its logic on top of the query plan
      {
        for (auto &col : columns_before_aggregation)
          if (!col.column) col.column = col.type->createColumn();
        ...
        auto actual_header =
            Aggregator::Params::getHeader(header_before_aggregation, /*only_merge*/ false, keys, aggregates, /*final*/ true);
        actual_header = AggregatingStep::appendGroupingColumn(actual_header, keys, has_grouping, settings[Setting::group_by_use_nulls]);
        ...
        if (!blocksHaveEqualStructure(actual_header, expected_header)) {
          auto converting =
              ActionsDAG::makeConvertingActions(actual_header.getColumnsWithTypeAndName(), expected_header.getColumnsWithTypeAndName(),
    ...
    optimize_read_in_order = settings[Setting::optimize_read_in_order] && (!settings[Setting::query_plan_read_in_order]) && storage &&
    ...
    for (const auto &it : chain.getLastStep().required_output) selected_columns.emplace_back(it.first);
...
void ExpressionAnalysisResult::finalize(const ExpressionActionsChain &chain, ssize_t &prewhere_step_num, ssize_t &where_step_num,
                                        ssize_t &having_step_num, const ASTSelectQuery &query) {
  if (prewhere_step_num >= 0) {
...
void ExpressionAnalysisResult::removeExtraColumns() const {
  if (hasWhere()) before_where->project_input = true;
  if (hasHaving()) before_having->project_input = true;
}
