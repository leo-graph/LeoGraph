...
void GlobalSubqueriesMatcher::Data::addExternalStorage(ASTPtr &ast, const Names &required_columns, bool set_alias) {
  ...
  if (!subquery_or_table_name) throw Exception(ErrorCodes::WRONG_GLOBAL_SUBQUERY, "Global subquery requires subquery or table name");
  ...
  if (is_table) {
    ...
    bool exists_in_context =
        static_cast<bool>(getContext()->tryResolveStorageID(StorageID("", temporary_table_name), Context::ResolveExternal));
    if (exists_in_local_map || exists_in_context) return;
  }
  ...
  if (set_alias) {
    if (auto *table_name = subquery_or_table_name->as<ASTTableIdentifier>())
      if (alias.empty()) alias = table_name->shortName();
  ...
  if (external_tables.contains(external_table_name)) return;
  ...
  auto external_storage_holder =
      std::make_shared<TemporaryTableHolder>(getContext(), ColumnsDescription{columns}, ConstraintsDescription{}, nullptr,
  ...
  if (!prepared_sets->findSubquery(set_key)) {
    ...
    auto future_set = prepared_sets->addFromSubquery(set_key, ast, std::move(source), std::move(external_storage), nullptr,
...
void GlobalSubqueriesMatcher::visit(ASTFunction &func, ASTPtr &, Data &data) {
...
void GlobalSubqueriesMatcher::visit(ASTTablesInSelectQueryElement &table_elem, ASTPtr &, Data &data) {
  const Settings &settings = data.getContext()->getSettingsRef();
  const bool prefer_global = settings[Setting::prefer_global_in_and_join];
  const bool enable_parallel_processing_of_joins = data.getContext()->canUseParallelReplicasOnInitiator();

  if (table_elem.table_join && (table_elem.table_join->as<ASTTableJoin &>().locality == JoinLocality::Global || prefer_global ||
    ...
    if (data.table_join) {
      ...
      for (auto &pr : required_columns_with_aliases) required_columns.push_back(pr.first);
    }
