...
class Lemmatizer : public ILemmatizer {
 ...
 public:
  ...
  TokenPtr lemmatize(const char* token) override { return TokenPtr(lemmatizer.Lemmatize(token)); }
};

Lemmatizers::Lemmatizers(const Poco::Util::AbstractConfiguration& config) {
  const String prefix = "lemmatizers";

  if (!config.has(prefix)) return;
    ...
    if (key.starts_with("lemmatizer")) {
      ...
      if (lemm_name.empty())
        throw Exception(ErrorCodes::INVALID_CONFIG_PARAMETER,
                        "Lemmatizer language in config is not specified here: "
                        "{}.{}.lang",
                        prefix, key);
      if (lemm_path.empty())
        throw Exception(ErrorCodes::INVALID_CONFIG_PARAMETER, "Path to lemmatizer in config is not specified here: {}.{}.path", prefix,
                        key);

      paths[lemm_name] = lemm_path;
    } else
      throw Exception(ErrorCodes::UNKNOWN_ELEMENT_IN_CONFIG, "Unknown element in config: {}.{}, must be 'lemmatizer'", prefix, key);
  }
}
...
Lemmatizers::LemmPtr Lemmatizers::getLemmatizer(const String& name) {
  ...
  if (lemmatizers.contains(name)) return lemmatizers[name];
