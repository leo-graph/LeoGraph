...
ColumnsDescription AggregatedZooKeeperLogElement::getColumnsDescription() {
  ...
  result.add({"hostname", std::make_shared<DataTypeLowCardinality>(std::make_shared<DataTypeString>()), "Hostname of the server."});

  result.add({"event_date", std::make_shared<DataTypeDate>(), "Date the group was flushed."});

  result.add({"event_time", std::make_shared<DataTypeDateTime>(), "Time the group was flushed."});

  result.add({"session_id", std::make_shared<DataTypeInt64>(), "Session id."});

  result.add({"parent_path", std::make_shared<DataTypeString>(), "Prefix of the path."});

  result.add({"operation", Coordination::SystemTablesDataTypes::operationEnum(), "Type of ZooKeeper operation."});

  result.add({"count", std::make_shared<DataTypeUInt32>(), "Number of operations in the (session_id, parent_path, operation) group."});
  ...
  result.add({"average_latency", std::make_shared<DataTypeFloat64>(),
              "Average latency across all operations in (session_id, parent_path, operation) group, in microseconds."});

  result.add(
      {"component", std::make_shared<DataTypeLowCardinality>(std::make_shared<DataTypeString>()), "Component that caused the event."});
  return result;
}
...
void AggregatedZooKeeperLog::observe(Int64 session_id, Int32 operation, const std::filesystem::path &path, UInt64 latency_microseconds,
                                     Coordination::Error error, StaticString component) {
  std::lock_guard lock(stats_mutex);

  EntryKey entry_key{.session_id = session_id, .operation = operation, .parent_path = path.parent_path(), .component = component};
...
size_t AggregatedZooKeeperLog::EntryKeyHash::operator()(const EntryKey &entry_key) const {
  return CityHash_v1_0_2::CityHash64WithSeed(entry_key.parent_path.data(), entry_key.parent_path.size(),
