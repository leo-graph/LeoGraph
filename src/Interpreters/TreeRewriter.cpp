...
struct CustomizeAggregateFunctionsSuffixData {
  ...
  void visit(ASTFunction &func, ASTPtr &) const {
    ...
    if (instance.isAggregateFunctionName(func.name) && !endsWith(func.name, customized_func_suffix) &&
        !endsWith(func.name, customized_func_suffix + "If")) {
...
struct CustomizeAggregateFunctionsMoveSuffixData {
  ...
  String moveSuffixAhead(const String &name) const {
    ...
    if (endsWith(prefix, "MergeState")) return prefix.substr(0, prefix_size - 10) + customized_func_suffix + "MergeState";

    if (endsWith(prefix, "Merge")) return prefix.substr(0, prefix_size - 5) + customized_func_suffix + "Merge";

    if (endsWith(prefix, "State")) return prefix.substr(0, prefix_size - 5) + customized_func_suffix + "State";

    if (endsWith(prefix, "If")) return prefix.substr(0, prefix_size - 2) + customized_func_suffix + "If";
...
struct ExistsExpressionData {
  ...
  static void visit(ASTFunction &func, ASTPtr) {
    bool exists_expression = func.name == "exists" && func.arguments && func.arguments->children.size() == 1 &&
                             typeid_cast<const ASTSubquery *>(func.arguments->children[0].get());

    if (!exists_expression) return;
...
bool hasArrayJoin(const ASTPtr &ast) {
  if (const ASTFunction *function = ast->as<ASTFunction>())
    if (function->name == "arrayJoin") return true;

  for (const auto &child : ast->children)
    if (!child->as<ASTSelectQuery>() && hasArrayJoin(child)) return true;
...
void renameDuplicatedColumns(const ASTSelectQuery *select_query) {
  ...
  for (auto &expr : elements) all_column_names.insert(expr->getAliasOrColumnName());

  for (auto &expr : elements) {
    auto name = expr->getAliasOrColumnName();
    if (!assigned_column_names.insert(name).second) {
      /// We can't rename with aliases if it doesn't support alias (e.g. asterisk)
      if (!dynamic_cast<ASTWithAlias *>(expr.get())) continue;

      size_t i = 1;
      while (all_column_names.contains(name + "_" + toString(i))) ++i;
...
void removeUnneededColumnsFromSelectClause(ASTSelectQuery *select_query, const Names &required_result_columns, bool remove_dups) {
  ...
  if (!required_result_columns.empty()) {
    ...
    for (const auto &elem : elements) required_columns_with_duplicate_count.emplace(elem->getAliasOrColumnName(), 1);
  } else
    return;
      ...
      auto interpolate_visitor = [](const ASTPtr ast, NameSet &columns) -> void {
        auto interpolate_visitor_impl = [](const ASTPtr node, NameSet &cols, auto self) -> void {
          if (const auto *ident = node->as<ASTIdentifier>())
            ...
            for (const auto &elem : func->arguments->children) self(elem, cols, self);
        };
  ...
  for (const auto &elem : elements) {
    ...
    if (required_columns_with_duplicate_count.end() != it && it->second) {
      ...
      if (func && func->name == "untuple") new_elements.push_back(elem);
      ...
      if (func && !select_query->groupBy()) {
        ...
        if (!data.aggregates.empty()) new_elements.push_back(elem);
      }
    }
  }
  ...
  if (select_query->interpolate()) {
    ...
    if (!children.empty()) {
      ...
      if (children.empty()) select_query->setExpression(ASTSelectQuery::Expression::INTERPOLATE, nullptr);
    }
  }
...
void executeScalarSubqueries(ASTPtr &query, ContextPtr context, size_t subquery_depth, Scalars &scalars, Scalars &local_scalars,
                             bool only_analyze, bool is_create_parameterized_view) {
  LogAST log;
  ExecuteScalarSubqueriesVisitor::Data visitor_data{WithContext{context},
                                                    subquery_depth,
                                                    scalars,
                                                    local_scalars,
                                                    only_analyze,
                                                    is_create_parameterized_view,
                                                    /*replace_only_to_literals=*/false,
                                                    /*max_literal_size=*/std::nullopt};
...
void getArrayJoinedColumns(ASTPtr &query, TreeRewriterResult &result, const ASTSelectQuery *select_query,
  ...
  if (!select_query->arrayJoinExpressionList().first) return;

  ArrayJoinedColumnsVisitor::Data visitor_data{result.aliases, result.array_join_name_to_alias, result.array_join_alias_to_name,
  ...
  if (result.array_join_result_to_source.empty()) {
    ...
    } else  /// This is a nested table.
    {
      ...
      if (!found) throw Exception(ErrorCodes::EMPTY_NESTED_TABLE, "No columns in nested table {}", source_name);
    }
  }
}

void setJoinStrictness(ASTSelectQuery &select_query, JoinStrictness join_default_strictness, bool old_any,
  ...
  if (old_any) {
    ...
    if (table_join.strictness == JoinStrictness::Any) table_join.strictness = JoinStrictness::RightAny;
  } else {
...
std::optional<bool> tryEvaluateConstCondition(ASTPtr expr, ContextPtr context) {
  ...
  DataTypePtr eval_res_type;
  {
    ...
    if (!constant_expression_result) return {};
  ...
  if (auto which = WhichDataType(eval_res_type); !which.isUInt8() && !which.isNothing()) return {};

  if (eval_res.isNull()) return false;
...
bool tryJoinOnConst(TableJoin &analyzed_join, const ASTPtr &on_expression, ContextPtr context) {
  if (!analyzed_join.isEnabledAlgorithm(JoinAlgorithm::HASH)) return false;

  if (analyzed_join.strictness() == JoinStrictness::Asof) return false;

  if (analyzed_join.isSpecialStorage()) return false;
...
void collectJoinedColumns(TableJoin &analyzed_join, ASTTableJoin &table_join, const TablesWithColumns &tables, const Aliases &aliases,
                          ContextPtr context) {
  ...
  if (table_join.using_expression_list) {
    ...
    if (any_keys_empty)
      throw DB::Exception(ErrorCodes::INVALID_JOIN_ON_EXPRESSION, "Cannot get JOIN keys from JOIN ON section: '{}', found keys: {}",
...
std::pair<bool, UInt64> recursivelyCollectMaxOrdinaryExpressions(const ASTPtr &expr, ASTExpressionList &into) {
  ...
  if (AggregateUtils::isAggregateFunction(*function)) return {true, 0};
  ...
  if (!has_aggregate) {
    for (UInt64 i = 0; i < pushed_children; i++) into.children.pop_back();
...
void expandGroupByAll(ASTSelectQuery *select_query) {
  ...
  for (const auto &expr : select_query->select()->children) recursivelyCollectMaxOrdinaryExpressions(expr, *group_expression_list);
...
void expandOrderByAll(ASTSelectQuery *select_query, [[maybe_unused]] const TablesWithColumns &tables_with_columns) {
  ...
  if (!all_elem) throw Exception(ErrorCodes::LOGICAL_ERROR, "Select analyze for not order by asts.");
...
ASTs getAggregates(ASTPtr &query, const ASTSelectQuery &select_query) {
  ...
  if (select_query.where()) assertNoAggregates(select_query.where(), "in WHERE");
  if (select_query.prewhere()) assertNoAggregates(select_query.prewhere(), "in PREWHERE");
...
ASTs getWindowFunctions(ASTPtr &query, const ASTSelectQuery &select_query) {
  ...
  if (select_query.having()) assertNoWindows(select_query.having(), "in HAVING");
  if (select_query.where()) assertNoWindows(select_query.where(), "in WHERE");
  if (select_query.prewhere()) assertNoWindows(select_query.prewhere(), "in PREWHERE");
  if (select_query.window()) assertNoWindows(select_query.window(), "in WINDOW");
  ...
  for (const ASTPtr &ast : data.window_functions) {
    ...
    if (node.arguments)
      for (auto &arg : node.arguments->children) assertNoWindows(arg, "inside another window function");

    if (node.window_definition) assertNoWindows(node.window_definition, "inside window definition");
  }
...
class MarkTupleLiteralsAsLegacyData {
 public:
  ...
  static void visitLiteral(ASTLiteral &literal, ASTPtr &) {
    if (literal.value.getType() == Field::Types::Tuple) literal.setUseLegacyColumnNameOfTuple(true);
  }
  ...
  static void visit(ASTPtr &ast, Data &) {
    if (auto *identifier = typeid_cast<ASTFunction *>(ast.get())) visitFunction(*identifier, ast);
    if (auto *identifier = typeid_cast<ASTLiteral *>(ast.get())) visitLiteral(*identifier, ast);
  }

  static bool needChildVisit(const ASTPtr & /*parent*/, const ASTPtr & /*child*/) { return true; }
};
...
struct RewriteShardNum {
  ...
  static void visit(ASTPtr &ast, Data &) {
    if (auto *identifier = typeid_cast<ASTIdentifier *>(ast.get())) visit(*identifier, ast);
  }

  static void visit(ASTIdentifier &identifier, ASTPtr &ast) {
    if (identifier.shortName() != "_shard_num") return;

    String alias = identifier.tryGetAlias();
    if (alias.empty()) alias = "_shard_num";
...
TreeRewriterResult::TreeRewriterResult(const NamesAndTypesList &source_columns_, ConstStoragePtr storage_,
                                       const StorageSnapshotPtr &storage_snapshot_, bool add_special)
    : storage(storage_), storage_snapshot(storage_snapshot_), source_columns(source_columns_) {
...
bool TreeRewriterResult::collectUsedColumns(const ASTPtr &query, bool is_select, bool no_throw) {
  ...
  for (const auto &column : source_columns) source_column_names.insert(column.name);

  NameSet required = columns_context.requiredColumns();
  if (columns_context.has_table_join) {
    NameSet available_columns;
    for (const auto &name : source_columns) available_columns.insert(name.name);
    ...
    for (const auto &joined_column : analyzed_join->columnsFromJoinedTable()) {
      ...
      if (available_columns.contains(name)) continue;

      if (required.contains(name)) {
        /// Optimization: do not add columns needed only in JOIN ON section.
        if (columns_context.nameInclusion(name) > analyzed_join->rightKeyInclusion(name)) analyzed_join->addJoinedColumn(joined_column);
  ...
  if (columns_context.has_array_join) {
    ...
    for (const auto &result_source : array_join_result_to_source) array_join_sources.insert(result_source.second);

    for (const auto &column_name_type : source_columns)
      if (array_join_sources.contains(column_name_type.name)) required.insert(column_name_type.name);
  }
  ...
  if (is_select && !has_explicit_columns) {
    ...
    if (storage) {
      ...
      for (auto &source_column : source_columns) {
        ...
        if (c == column_sizes.end()) continue;
  ...
  if (!unknown_required_source_columns.empty()) {
    ...
    ss << "Missing columns:";
    for (const auto &name : unknown_required_source_columns) ss << " '" << name << "'";
    ss << " while processing: '" << query->formatWithSecretsOneLine() << "'";

    ss << ", required columns:";
    for (const auto &name : columns_context.requiredColumns()) ss << " '" << name << "'";
    ...
    if (storage) {
      ...
      for (const auto &col : columns_context.requiredColumns()) {
        for (const auto &hint : storage->getHints(col)) {
          ...
          if (inserted) hints.push_back(hint);
        }
      }
      ...
      if (!source_column_names.empty()) {
        ...
        for (const auto &name : source_column_names) ss << " '" << name << "'";
      } else
    ...
    if (columns_context.has_table_join) {
      ss << ", joined columns:";
      for (const auto &column : analyzed_join->columnsFromJoinedTable()) ss << " '" << column.name << "'";
    }

    if (!array_join_sources.empty()) {
      ss << ", arrayJoin columns:";
      for (const auto &name : array_join_sources) ss << " '" << name << "'";
    }
...
NameSet TreeRewriterResult::getArrayJoinSourceNameSet() const {
  ...
  for (const auto &elem : array_join_result_to_source) forbidden_columns.insert(elem.first);
  return forbidden_columns;
}

TreeRewriterResultPtr TreeRewriter::analyzeSelect(ASTPtr &query, TreeRewriterResult &&result, const SelectQueryOptions &select_options,
                                                  const TablesWithColumns &tables_with_columns, const Names &required_result_columns,
                                                  std::shared_ptr<TableJoin> table_join) const {
  auto *select_query = query->as<ASTSelectQuery>();
  if (!select_query) throw Exception(ErrorCodes::LOGICAL_ERROR, "Select analyze for not select asts.");
  ...
  if (remove_duplicates) {
    ...
    normalize(query, aliases, name_set, select_options.ignore_alias, settings, /* allow_self_aliases = */ true, getContext(),
  ...
  if (settings[Setting::optimize_normalize_count_variants]) TreeOptimizer::optimizeCountConstantAndSumOne(query, getContext());
  ...
  if (tables_with_columns.size() > 1) {
    ...
    columns_from_left_table.insert(columns_from_left_table.end(), tables_with_columns[0].hidden_columns.begin(),
                                   tables_with_columns[0].hidden_columns.end());
    result.analyzed_join->setColumnsFromJoinedTable(std::move(columns_from_joined_table), source_columns_set,
                                                    right_table.table.getQualifiedNamePrefix(), columns_from_left_table);
  }
  ...
  if (table_join) {
    for (const auto &[name, _] : table_join->columnsFromJoinedTable()) all_source_columns_set.insert(name);
  }

  normalize(query, result.aliases, all_source_columns_set, select_options.ignore_alias, settings, /* allow_self_aliases = */ true,
            getContext(), select_options.is_create_parameterized_view);
  ...
  executeScalarSubqueries(query, getContext(), subquery_depth, scalars, local_scalars, select_options.only_analyze,
                          select_options.is_create_parameterized_view);
  ...
  if (!select_options.only_analyze && getContext()->hasQueryContext()) {
    for (const auto &it : scalars) getContext()->getQueryContext()->addScalar(it.first, it.second);
  }

  if (settings[Setting::legacy_column_name_of_tuple_literal]) markTupleLiteralsAsLegacy(query);
  ...
  if (ast_optimizations_allowed) TreeOptimizer::apply(query, result, tables_with_columns, getContext());

  /// array_join_alias_to_name, array_join_result_to_source.
  getArrayJoinedColumns(query, result, select_query, result.source_columns, source_columns_set);

  setJoinStrictness(*select_query, settings[Setting::join_default_strictness], settings[Setting::any_join_distinct_right_table_keys],
                    result.analyzed_join);
  ...
  if (!result.missed_subcolumns.empty()) {
    for (const String &column_name : result.missed_subcolumns) replaceMissedSubcolumnsInQuery(query, column_name);
  ...
  if (settings[Setting::optimize_respect_aliases] && result.storage_snapshot && is_initiator) {
    std::unordered_set<IAST *> excluded_nodes;
    {
      ...
      if (table_join_ast && table_join_ast->on_expression) excluded_nodes.insert(table_join_ast->on_expression.get());
      if (table_join_ast && table_join_ast->using_expression_list) excluded_nodes.insert(table_join_ast->using_expression_list.get());
    }
  ...
  if (result.optimize_trivial_count)
    result.optimize_trivial_count = settings[Setting::optimize_trivial_count_query] && !select_query->groupBy() &&
                                    !select_query->having() && !select_query->sampleSize() && !select_query->sampleOffset() &&
                                    !select_query->final() && (tables_with_columns.size() < 2 || isLeft(result.analyzed_join->kind()));
...
TreeRewriterResultPtr TreeRewriter::analyze(ASTPtr &query, const NamesAndTypesList &source_columns, ConstStoragePtr storage,
                                            const StorageSnapshotPtr &storage_snapshot, bool allow_aggregations, bool allow_self_aliases,
                                            bool execute_scalar_subqueries, bool is_create_parameterized_view) const {
  if (query->as<ASTSelectQuery>()) throw Exception(ErrorCodes::LOGICAL_ERROR, "Not select analyze for select asts.");
...
void TreeRewriter::normalize(ASTPtr &query, Aliases &aliases, const NameSet &source_columns_set, bool ignore_alias,
                             const Settings &settings, bool allow_self_aliases, ContextPtr context_, bool is_create_parameterized_view) {
  if (!UserDefinedSQLFunctionFactory::instance().empty()) UserDefinedSQLFunctionVisitor::visit(query, context_);
  ...
  QueryNormalizer::Data normalizer_data(aliases, source_columns_set, ignore_alias, QueryNormalizer::ExtractedSettings(settings),
                                        allow_self_aliases, is_create_parameterized_view);
