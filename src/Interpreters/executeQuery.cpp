...
static void checkASTSizeLimits(const IAST &ast, const Settings &settings) {
  if (settings[Setting::max_ast_depth]) ast.checkDepth(settings[Setting::max_ast_depth]);
  if (settings[Setting::max_ast_elements]) ast.checkSize(settings[Setting::max_ast_elements]);
}
...
static void logQuery(const String &query, ContextPtr context, bool internal, QueryProcessingStage::Enum stage) {
  if (internal) {
    ...
    if (comment.size() > max_query_size) comment.resize(max_query_size);
    ...
    if (auto txn = context->getCurrentTransaction()) transaction_info = fmt::format(" (TID: {}, TIDH: {})", txn->tid, txn->tid.getHash());

    LOG_DEBUG(
        getLogger("executeQuery"), "(from {}{}{}){}{}{} {} (stage: {})", client_info.current_address->toString(),
        (current_user != "default" ? ", user: " + current_user : ""),
        (!initial_query_id.empty() && current_query_id != initial_query_id ? ", initial_query_id: " + initial_query_id : std::string()),
        transaction_info, comment, line_info, toOneLineQuery(query), QueryProcessingStage::toString(stage));

    if (client_info.client_trace_context.trace_id != UUID()) {
      LOG_TRACE(getLogger("executeQuery"), "OpenTelemetry traceparent '{}'", client_info.client_trace_context.composeTraceparentHeader());
    }
  }
}
...
static void logException(ContextPtr context, QueryLogElement &elem, bool log_error = true) {
  String comment;
  if (!elem.log_comment.empty()) comment = fmt::format(" (comment: {})", elem.log_comment);
  ...
  if (elem.stack_trace.empty() || !log_error)
    message.text = fmt::format("{} (from {}){}{} (in query: {})", elem.exception, context->getClientInfo().current_address->toString(),
                               comment, line_info, toOneLineQuery(elem.query));
  else
    message.text = fmt::format(
        "{} (from {}){}{} (in query: {}), Stack trace (when copying this message, always include the lines below):\n\n{}", elem.exception,
        context->getClientInfo().current_address->toString(), comment, line_info, toOneLineQuery(elem.query), elem.stack_trace);
...
static void addPrivilegesInfoToQueryLogElement(QueryLogElement &element, const ContextPtr context_ptr) {
...
static void addStatusInfoToQueryLogElement(QueryLogElement &element, const QueryStatusInfo &info, const ASTPtr query_ast,
                                           const ContextPtr context_ptr, std::chrono::system_clock::time_point time) {
...
QueryLogElement logQueryStart(const std::chrono::time_point<std::chrono::system_clock> &query_start_time, const ContextMutablePtr &context,
                              const String &query_for_logging, UInt64 normalized_query_hash, const ASTPtr &query_ast,
                              const QueryPipeline &pipeline, const IInterpreter *interpreter, bool internal, const String &query_database,
                              const String &query_table, bool async_insert) {
  ...
  if (query_ast && settings[Setting::log_formatted_queries]) elem.formatted_query = query_ast->formatWithSecretsOneLine();
  ...
  if (log_queries) {
    ...
    if (settings[Setting::log_query_settings]) elem.query_settings = std::make_shared<Settings>(context->getSettingsRef());

    elem.log_comment = settings[Setting::log_comment];
    if (elem.log_comment.size() > settings[Setting::max_query_size]) elem.log_comment.resize(settings[Setting::max_query_size]);
  ...
  if (auto query_metric_log = context->getQueryMetricLog(); query_metric_log && !internal) {
    ...
    if (interval_milliseconds > 0) query_metric_log->startQuery(elem.client_info.current_query_id, query_start_time, interval_milliseconds);
  }

  return elem;
}

void logQueryMetricLogFinish(ContextPtr context, bool internal, String query_id, std::chrono::system_clock::time_point finish_time,
...
static ResultProgress flushQueryProgress(const QueryPipeline &pipeline, bool pulling_pipeline, const ProgressCallback &progress_callback,
  ...
  if (process_list_elem) res.memory_usage = std::max<Int64>(process_list_elem->getInfo().peak_memory_usage, 0);
...
QueryPipelineFinalizedInfo finalizeQueryPipelineBeforeLogging(QueryPipeline &&query_pipeline,
                                                              QueryResultCacheUsage query_result_cache_usage, bool pulling_pipeline) {
  ...
  return QueryPipelineFinalizedInfo{.result_progress = std::move(result_progress),
                                    .processors_profile_infos = std::move(processors_profile_infos),
                                    .pipeline_dump = std::move(pipeline_dump)};
}

void logQueryFinishImpl(QueryLogElement &elem, const ContextMutablePtr &context, const ASTPtr &query_ast,
                        const QueryPipelineFinalizedInfo &query_pipeline_finalized_info, bool pulling_pipeline,
                        std::shared_ptr<OpenTelemetry::SpanHolder> query_span, QueryResultCacheUsage query_result_cache_usage,
                        bool internal, std::chrono::system_clock::time_point time) {
  const Settings &settings = context->getSettingsRef();
  ...
  if (QueryStatusPtr process_list_elem = context->getProcessListElement()) {
    ...
    if (elem.read_rows != 0) {
      ...
      LOG_DEBUG(getLogger("executeQuery"), "Read {} rows, {} in {} sec., {} rows/sec., {}/sec.", elem.read_rows,
                ReadableSize(elem.read_bytes), elapsed_seconds, rows_per_second, ReadableSize(bytes_per_second));
    }
    ...
    if (log_queries && elem.type >= settings[Setting::log_queries_min_type] &&
      ...
      if (auto query_log = context->getQueryLog()) query_log->add(elem);
    }
  }
...
void logQueryFinish(QueryLogElement &elem, const ContextMutablePtr &context, const ASTPtr &query_ast, QueryPipeline &&query_pipeline,
                    bool pulling_pipeline, std::shared_ptr<OpenTelemetry::SpanHolder> query_span,
                    QueryResultCacheUsage query_result_cache_usage, bool internal) {
  const auto time_now = std::chrono::system_clock::now();
  auto query_pipeline_finalized_info =
      finalizeQueryPipelineBeforeLogging(std::move(query_pipeline), query_result_cache_usage, pulling_pipeline);
  logQueryFinishImpl(elem, context, query_ast, query_pipeline_finalized_info, pulling_pipeline, query_span, query_result_cache_usage,
                     internal, time_now);
}

void logQueryException(QueryLogElement &elem, const ContextMutablePtr &context, const Stopwatch &start_watch, const ASTPtr &query_ast,
                       std::shared_ptr<OpenTelemetry::SpanHolder> query_span, bool internal, bool log_error) {
  const Settings &settings = context->getSettingsRef();
  auto log_queries = settings[Setting::log_queries];
  ...
  if (settings[Setting::calculate_text_stack_trace] && log_error) elem.stack_trace = getExceptionStackTraceString(std::current_exception());
...
void logExceptionBeforeStart(const String &query_for_logging, UInt64 normalized_query_hash, ContextPtr context, ASTPtr ast,
                             const std::shared_ptr<OpenTelemetry::SpanHolder> &query_span, UInt64 elapsed_milliseconds, bool internal) {
  auto query_end_time = std::chrono::system_clock::now();

  /// Exception before the query execution.
  if (auto quota = context->getQuota()) quota->used(QuotaType::ERRORS, 1, /* check_exceeded = */ false);
  ...
  if (ast) {
    ...
    if (settings[Setting::log_formatted_queries]) elem.formatted_query = ast->formatWithSecretsOneLine();
  }
  ...
  if (settings[Setting::log_query_settings]) elem.query_settings = std::make_shared<Settings>(settings);

  if (settings[Setting::calculate_text_stack_trace]) elem.stack_trace = getExceptionStackTraceString(std::current_exception());

  elem.is_internal = internal;

  bool log_error =
      elem.exception_code != ErrorCodes::QUERY_WAS_CANCELLED_BY_CLIENT && elem.exception_code != ErrorCodes::QUERY_WAS_CANCELLED;
  logException(context, elem, log_error);
      ...
      if (auto *value = set_query->changes.tryGet("allow_experimental_analyzer")) {
        if (top_level != field_to_bool(*value))
          throw Exception(ErrorCodes::INCORRECT_QUERY,
                          "Setting 'allow_experimental_analyzer' is changed in the subquery. Top level value: {}", top_level);
      }

      if (auto *value = set_query->changes.tryGet("enable_analyzer")) {
        if (top_level != field_to_bool(*value))
          throw Exception(ErrorCodes::INCORRECT_QUERY, "Setting 'enable_analyzer' is changed in the subquery. Top level value: {}",
                          top_level);
      }
    }

    for (auto child : node->children) {
      if (child) nodes_to_process.push_back(std::move(child));
    }
  }
}
...
static BlockIO executeQueryImpl(const char *begin, const char *end, ContextMutablePtr context, QueryFlags flags,
                                QueryProcessingStage::Enum stage, ReadBufferUniquePtr &istr, ASTPtr &out_ast,
                                ImplicitTransactionControlExecutorPtr implicit_tcl_executor, HTTPContinueCallback http_continue_callback,
  ...
  if (internal || client_info.query_kind == ClientInfo::QueryKind::SECONDARY_QUERY) max_query_size = 0;
  ...
  try {
    if (stage == QueryProcessingStage::QueryPlan) {
      ...
      out_ast = parseKQLQuery(parser, begin, end, "", max_query_size, settings[Setting::max_parser_depth],
      ...
      if (!settings[Setting::allow_experimental_time_series_table])
        throw Exception(ErrorCodes::SUPPORT_IS_DISABLED,
                        "Support for PromQL dialect is disabled (turn on setting 'allow_experimental_time_series_table')");
      ParserPrometheusQuery parser(settings[Setting::promql_database], settings[Setting::promql_table],
      ...
      try {
        ...
        ASTPtr ast2;
        try {
          ast2 = parseQuery(parser, formatted1.data(), formatted1.data() + formatted1.size(), "", new_max_query_size,
        ...
        if (formatted1 != formatted2) {
          struct ASTDifference {
            enum class Type : uint8_t { ID, FORMAT };
          ...
          const auto search_difference_in_asts = [&](this const auto &self, ASTPtr lhs, ASTPtr rhs) -> std::optional<ASTDifference> {
            if (lhs->getID() != rhs->getID()) return std::make_optional(ASTDifference{lhs, rhs, ASTDifference::Type::ID});
            ...
            if (format_ast(lhs) != format_ast(rhs)) return std::make_optional(ASTDifference{lhs, rhs, ASTDifference::Type::FORMAT});
          ...
          if (auto difference = search_difference_in_asts(out_ast, ast2)) {
            ...
            throw Exception(ErrorCodes::LOGICAL_ERROR,
                            ...
                            lhs->getID(), rhs->getID(), original_query, formatted1, formatted2, format_ast(lhs), lhs->dumpTree(),
        ...
        if (e.code() != ErrorCodes::NOT_IMPLEMENTED) throw;
      }
#endif
    }
    ...
    if (out_ast) {
      if (const auto *insert_query = out_ast->as<ASTInsertQuery>(); insert_query && insert_query->data) query_end = insert_query->data;
    ...
    if (out_ast && !is_create_parameterized_view && probably_has_params) {
      ...
      /// Copy query into string. It will be written to log and presented in processlist. If an INSERT query, string will not include data
      /// to insertion.
      query.assign(begin, query_end);
    }
    ...
    if (query.empty()) query.assign(begin, std::min(static_cast<size_t>(end - begin), max_query_size));
  ...
  try {
    ...
    if (out_ast) {
      ...
      if (auto *insert_query = out_ast->as<ASTInsertQuery>()) insert_query->tail = std::move(istr);
    ...
    if (!(out_ast && out_ast->as<ASTShowProcesslistQuery>())) {
      ...
      process_list_entry = context->getProcessList().insert(query_for_logging, normalized_query_hash, out_ast.get(), context,
    ...
    if (stage == QueryProcessingStage::QueryPlan) query_plan = context->getDeserializedQueryPlan();

    ASTInsertQuery *insert_query = nullptr;
    if (out_ast) insert_query = out_ast->as<ASTInsertQuery>();
    ...
    if (insert_query && insert_query->select) {
      ...
      if (insert_query->tail) {
        ...
        if (input_function) {
          ...
          auto pipe = getSourceFromASTInsertQuery(out_ast, true, input_metadata_snapshot->getSampleBlock(), context, input_function);
    ...
    if (async_insert) {
      ...
      if (quorum_is_enabled && !settings[Setting::insert_quorum_parallel])
        throw Exception(ErrorCodes::UNSUPPORTED_PARAMETER,
                        "Async inserts with quorum only make sense with enabled insert_quorum_parallel setting, either disable quorum or "
                        "set insert_quorum_parallel=1 or do not use async inserts");
      ...
      if (http_continue_callback && !internal) http_continue_callback();
      ...
      if (result.status == AsynchronousInsertQueue::PushResult::OK) {
        ...
        if (!table_id.empty()) context->setInsertionTable(table_id);
    ...
    if (settings[Setting::use_query_cache] &&
        (settings[Setting::read_overflow_mode] != OverflowMode::THROW ||
         ...
         settings[Setting::group_by_overflow_mode] != OverflowMode::THROW || settings[Setting::sort_overflow_mode] != OverflowMode::THROW ||
         settings[Setting::result_overflow_mode] != OverflowMode::THROW ||
         settings[Setting::timeout_overflow_mode] != OverflowMode::THROW || settings[Setting::set_overflow_mode] != OverflowMode::THROW ||
         settings[Setting::join_overflow_mode] != OverflowMode::THROW || settings[Setting::transfer_overflow_mode] != OverflowMode::THROW ||
         settings[Setting::distinct_overflow_mode] != OverflowMode::THROW))
      throw Exception(ErrorCodes::QUERY_CACHE_USED_WITH_NON_THROW_OVERFLOW_MODE,
                      "use_query_cache and overflow_mode != 'throw' cannot be used together");
    ...
    if (can_use_query_result_cache) settings_copy = settings;
    ...
    if (!async_insert) {
      ...
      auto get_result_from_query_result_cache = [&]() {
        if (out_ast && can_use_query_result_cache && settings[Setting::enable_reads_from_query_cache]) {
          QueryResultCache::Key key(out_ast, context->getCurrentDatabase(), *settings_copy, context->getCurrentQueryId(),
      ...
      if (!get_result_from_query_result_cache()) {
        ...
        if (!context->getCurrentTransaction() && settings[Setting::implicit_transaction] &&
            !(out_ast && out_ast->as<ASTTransactionControl>())) {
          try {
            if (context->isGlobalContext()) throw Exception(ErrorCodes::LOGICAL_ERROR, "Global context cannot create transactions");
        ...
        if (out_ast) interpreter = InterpreterFactory::instance().get(out_ast, context, SelectQueryOptions(stage).setInternal(internal));
        ...
        if (interpreter) {
          if (!interpreter->ignoreLimits()) {
            ...
            limits.size_limits = SizeLimits(settings[Setting::max_result_rows], settings[Setting::max_result_bytes],
          ...
          if (can_use_query_result_cache && settings[Setting::enable_writes_to_query_cache]) {
            /// Only use the query result cache if the query does not contain non-deterministic functions or system tables (which are
            ...
            if (ast_contains_nondeterministic_functions &&
                nondeterministic_function_handling == QueryResultCacheNondeterministicFunctionHandling::Throw)
              throw Exception(ErrorCodes::QUERY_CACHE_USED_WITH_NONDETERMINISTIC_FUNCTIONS,
                              ...
                              " Use setting `query_cache_nondeterministic_function_handling = 'save'` or `= 'ignore'` to cache the query "
                              "result regardless or to omit caching");
            ...
            if (ast_contains_system_tables && system_table_handling == QueryResultCacheSystemTableHandling::Throw)
              throw Exception(ErrorCodes::QUERY_CACHE_USED_WITH_SYSTEM_TABLE,
                              ...
                              " Use setting `query_cache_system_table_handling = 'save'` or `= 'ignore'` to cache the query result "
                              "regardless or to omit caching");

            if ((!ast_contains_nondeterministic_functions ||
                 nondeterministic_function_handling == QueryResultCacheNondeterministicFunctionHandling::Save) &&
              ...
              QueryResultCache::Key key(out_ast, context->getCurrentDatabase(), *settings_copy, res.pipeline.getSharedHeader(),
                                        context->getCurrentQueryId(), context->getUserID(), context->getCurrentRoles(),
                                        settings[Setting::query_cache_share_between_users], created_at, expires_at,
                                        settings[Setting::query_cache_compress_entries]);

              const size_t num_query_runs = settings[Setting::query_cache_min_query_runs]
                                                ? query_result_cache->recordQueryRun(key)
                                                : 1;  /// try to avoid locking a mutex in recordQueryRun()
              if (num_query_runs <= settings[Setting::query_cache_min_query_runs]) {
                LOG_TRACE(getLogger("QueryResultCache"),
                          "Skipped insert because the query ran {} times but the minimum required number of query runs to cache the query "
                          "result is {}",
                ...
                auto query_result_cache_writer = std::make_shared<QueryResultCacheWriter>(query_result_cache->createWriter(
                    key, std::chrono::milliseconds(settings[Setting::query_cache_min_query_duration].totalMilliseconds()),
                    settings[Setting::query_cache_squash_partial_results], settings[Setting::max_block_size],
                    settings[Setting::query_cache_max_size_in_bytes], settings[Setting::query_cache_max_entries]));
              ...
              /// We will expose the info in HTTP headers, but only if the cache is enabled for reading (otherwise browsers should not cache
              /// either) Set only "expires_at", not "Age" as the entry has not aged at this moment in time.
              if (settings[Setting::enable_reads_from_query_cache]) result_details.query_cache_entry_expires_at = expires_at;
            }
          }
        }
      }
    ...
    if (process_list_entry) {
      ...
      if (process_list_entry->getQueryStatus()->isKilled())
        throw Exception(ErrorCodes::QUERY_WAS_CANCELLED, "Query '{}' is killed in pending state",
                        process_list_entry->getQueryStatus()->getInfo().client_info.current_query_id);
    }
    ...
    if (pipeline.pulling() || pipeline.completed()) {
      ...
      if (stage == QueryProcessingStage::Complete && pipeline.pulling()) pipeline.setLimitsAndQuota(limits, quota);
    ...
    {
      QueryLogElement elem = logQueryStart(query_start_time, context, query_for_logging, normalized_query_hash, out_ast, pipeline,
                                           interpreter.get(), internal, query_database, query_table, async_insert);
      ...
      auto finish_callback = [elem, context, out_ast, query_result_cache_usage, internal, implicit_tcl_executor,
                              // Need to be cached, since will be changed after complete()
                              pulling_pipeline = pipeline.pulling(),
                              query_span](const QueryPipelineFinalizedInfo &query_pipeline_finalized_info,
                                          std::chrono::system_clock::time_point finish_time) mutable {
        logQueryFinishImpl(elem, context, out_ast, query_pipeline_finalized_info, pulling_pipeline, query_span, query_result_cache_usage,
                           internal, finish_time);
      ...
      auto exception_callback = [start_watch, elem, context, out_ast, internal, my_quota(quota), implicit_tcl_executor,
        ...
        if (!internal) {
          if (my_quota) my_quota->used(QuotaType::ERRORS, 1, /* check_exceeded = */ false);
        }
...
static bool isReadOnlyQuery(const ASTPtr &ast) {
  ...
  return kind == IAST::QueryKind::Select || kind == IAST::QueryKind::Explain || kind == IAST::QueryKind::Show ||
         kind == IAST::QueryKind::Describe || kind == IAST::QueryKind::Exists;
}
...
static void executeASTFuzzerQueries(const ASTPtr &ast, const ContextMutablePtr &context, Float64 ast_fuzzer_runs_value, bool any_query) {
  if (!any_query && !isReadOnlyQuery(ast)) return;
  ...
  if (fractional > 0) {
    ...
    if (dist(thread_local_rng)) ++num_runs;
  }
      ...
      if (result.second.pipeline.initialized()) {
        if (result.second.pipeline.pushing()) {
          ...
          if (result.second.pipeline.pulling()) {
            result.second.pipeline.complete(
                std::make_shared<NullOutputFormat>(std::make_shared<const Block>(result.second.pipeline.getHeader())));
          }
...
std::pair<ASTPtr, BlockIO> executeQuery(const String &query, ContextMutablePtr context, QueryFlags flags,
                                        QueryProcessingStage::Enum stage) {
  if (isCrashed()) throw Exception(ErrorCodes::ABORTED, "The server is shutting down due to a fatal error");
  ...
  if (const auto *ast_query_with_output = dynamic_cast<const ASTQueryWithOutput *>(ast.get())) {
    String format_name =
        ast_query_with_output->format_ast ? getIdentifierName(ast_query_with_output->format_ast) : context->getDefaultFormat();

    const bool ignore_null_for_explain = context->getSettingsRef()[Setting::ignore_format_null_for_explain];
    if (boost::iequals(format_name, "Null") && !(ast->as<ASTExplainQuery>() && ignore_null_for_explain)) res.null_format = true;
  }
...
void executeQuery(ReadBuffer &istr, WriteBuffer &ostr, ContextMutablePtr context, SetResultDetailsFunc set_result_details, QueryFlags flags,
                  const std::optional<FormatSettings> &output_format_settings,
                  HandleExceptionInOutputFormatFunc handle_exception_in_output_format, QueryFinishCallback query_finish_callback,
                  HTTPContinueCallback http_continue_callback) {
  executeQuery(wrapReadBufferReference(istr), ostr, context, std::move(set_result_details), flags, output_format_settings,
               std::move(handle_exception_in_output_format), std::move(query_finish_callback), std::move(http_continue_callback));
}

void executeQuery(ReadBufferUniquePtr istr, WriteBuffer &ostr, ContextMutablePtr context, SetResultDetailsFunc set_result_details,
                  QueryFlags flags, const std::optional<FormatSettings> &output_format_settings,
                  HandleExceptionInOutputFormatFunc handle_exception_in_output_format, QueryFinishCallback query_finish_callback,
                  HTTPContinueCallback http_continue_callback) {
  if (isCrashed()) throw Exception(ErrorCodes::ABORTED, "The server is shutting down due to a fatal error");
  ...
  try {
    streams = executeQueryImpl(begin, end, context, flags, QueryProcessingStage::Complete, istr, ast, implicit_tcl_executor,
                               http_continue_callback, result_details);
  } catch (...) {
    if (handle_exception_in_output_format) {
      update_format_on_exception_if_needed();
      if (output_format) handle_exception_in_output_format(*output_format, format_name, context, output_format_settings);
    }
  ...
  try {
    if (pipeline.pushing()) {
      ...
      output_format = FormatFactory::instance().getOutputFormatParallelIfPossible(
          format_name, *out_buf, materializeBlock(pipeline.getHeader()), context, output_format_settings);
      ...
      pipeline.setProgressCallback([output_format, previous_progress_callback](const Progress &progress) {
        if (previous_progress_callback) previous_progress_callback(progress);
  ...
  /// It releases the session and finalizes the output. The client might use the same session to query other queries. Hence, the transaction
  /// must be committed before `query_finish_callback`. Refer: https://github.com/ClickHouse/ClickHouse/issues/80428
  if (implicit_tcl_executor->transactionRunning()) implicit_tcl_executor->commit(context);
...
void executeTrivialBlockIO(BlockIO &streams, ContextPtr context, bool with_interactive_cancel) {
  try {
    if (!streams.pipeline.initialized()) return;
