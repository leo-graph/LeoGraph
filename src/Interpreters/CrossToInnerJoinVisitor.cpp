...
struct JoinedElement {
  ...
  void checkTableName(const DatabaseAndTableWithAlias &table, const String &current_database) const {
    if (!element.table_expression) throw Exception(ErrorCodes::LOGICAL_ERROR, "Not a table expression in JOIN (ARRAY JOIN?)");

    ASTTableExpression *table_expression = element.table_expression->as<ASTTableExpression>();
    if (!table_expression) throw Exception(ErrorCodes::LOGICAL_ERROR, "Wrong table expression in JOIN");
  ...
  void rewriteCommaToCross() {
    if (join && join->kind == JoinKind::Comma) join->kind = JoinKind::Cross;
  }

  JoinKind getOriginalKind() const { return original_kind; }

  bool rewriteCrossToInner(ASTPtr on_expression) {
    if (join->kind != JoinKind::Cross) return false;
...
bool isAllowedToRewriteCrossJoin(const ASTPtr &node, const Aliases &aliases) {
  if (node->as<ASTFunction>()) {
    ...
    for (const auto *ident : idents) {
      if (ident->isShort() && aliases.contains(ident->shortName())) return false;
    }
    return true;
  }
...
std::map<size_t, std::vector<ASTPtr>> moveExpressionToJoinOn(const ASTPtr &ast, const std::vector<JoinedElement> &joined_tables,
  ...
  for (const auto &node : splitConjunctionsAst(ast)) {
    if (const auto *func = node->as<ASTFunction>(); func && func->name == "equals") {
      if (!func->arguments || func->arguments->children.size() != 2) return {};
    ...
    if (!isAllowedToRewriteCrossJoin(node, aliases)) return {};
  }
  return asts_to_join_on;
}

ASTPtr makeOnExpression(const std::vector<ASTPtr> &expressions) {
  if (expressions.size() == 1) return expressions[0]->clone();

  ASTs arguments;
  arguments.reserve(expressions.size());
  for (const auto &ast : expressions) arguments.emplace_back(ast->clone());
...
std::vector<JoinedElement> getTables(const ASTSelectQuery &select) {
  ...
  for (const auto &child : tables->children) {
    ...
    if (!table_element) throw Exception(ErrorCodes::LOGICAL_ERROR, "TablesInSelectQueryElement expected");

    JoinedElement &t = joined_tables.emplace_back(*table_element);
    t.rewriteCommaToCross();

    if (t.arrayJoin()) return {};

    if (t.hasUsing()) {
      if (has_using) throw Exception(ErrorCodes::NOT_IMPLEMENTED, "Multiple USING statements are not supported");
      has_using = true;
    }

    if (const auto *join = t.tableJoin(); join && isCrossOrComma(join->kind)) {
      if (!join->children.empty())
        throw Exception(ErrorCodes::LOGICAL_ERROR, "CROSS JOIN has {} expressions: [{}, ...]", join->children.size(),
                        join->children[0]->formatWithSecretsOneLine());
    }
  }
...
void CrossToInnerJoinMatcher::visit(ASTSelectQuery &select, ASTPtr &, Data &data) {
  ...
  if (data.cross_to_inner_join_rewrite && select.where()) {
    ...
    for (size_t i = 1; i < joined_tables.size(); ++i) {
      ...
      if (joined.tableJoin()->kind != JoinKind::Cross) continue;
      ...
      if (joined.getOriginalKind() == JoinKind::Comma && data.cross_to_inner_join_rewrite > 1 && !rewritten) {
