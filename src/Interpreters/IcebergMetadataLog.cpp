...
auto iceberg_pruning_status_datatype = std::make_shared<DataTypeEnum8>(
    DataTypeEnum8::Values{{"NotPruned", static_cast<Int8>(Iceberg::PruningReturnStatus::NOT_PRUNED)},
...
ColumnsDescription IcebergMetadataLogElement::getColumnsDescription() {
  auto iceberg_metadata_log_entry_datatype = std::make_shared<DataTypeEnum8>(
      DataTypeEnum8::Values{{"None", static_cast<Int8>(IcebergMetadataLogLevel::None)},
...
void insertRowToLogTable(const ContextPtr& local_context, String row, IcebergMetadataLogLevel row_log_level, const String& table_path,
                         const String& file_path, std::optional<UInt64> row_in_file,
  ...
  if (set_log_level < row_log_level) return;
  timespec spec{};
  if (clock_gettime(CLOCK_REALTIME, &spec)) throw ErrnoException(ErrorCodes::CANNOT_CLOCK_GETTIME, "Cannot clock_gettime");
  ...
  iceberg_metadata_log->add(DB::IcebergMetadataLogElement{.current_time = spec.tv_sec,
