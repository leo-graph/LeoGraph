...
MergeTreeTransactionHolder::MergeTreeTransactionHolder(const MergeTreeTransactionPtr& txn_, bool autocommit_ = false,
                                                       const Context* owned_by_session_context_)
    : txn(txn_), autocommit(autocommit_), owned_by_session_context(owned_by_session_context_) {
...
MergeTreeTransactionHolder::MergeTreeTransactionHolder(MergeTreeTransactionHolder&& rhs) noexcept { *this = std::move(rhs); }
...
MergeTreeTransactionHolder::~MergeTreeTransactionHolder() { onDestroy(); }

void MergeTreeTransactionHolder::onDestroy() noexcept {
  if (!txn) return;
  if (txn->getState() != MergeTreeTransaction::RUNNING) return;
...
MergeTreeTransactionHolder::MergeTreeTransactionHolder(const MergeTreeTransactionHolder& rhs) { *this = rhs; }
