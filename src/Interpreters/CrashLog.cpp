...
ColumnsDescription CrashLogElement::getColumnsDescription() {
  return ColumnsDescription{
      ...
      {"trace", std::make_shared<DataTypeArray>(std::make_shared<DataTypeUInt64>()),
       "Stack trace at the moment of crash. Each element is a virtual memory address inside ClickHouse server process."},
      {"trace_full", std::make_shared<DataTypeArray>(std::make_shared<DataTypeString>()),
       "Stack trace at the moment of crash. Each element contains a called method inside ClickHouse server process."},
      ...
      {"signal_description", std::make_shared<DataTypeString>(),
       "Human-readable description based on signal_code (e.g., 'Address not mapped to object')."},
      {"current_exception_trace_full", std::make_shared<DataTypeArray>(std::make_shared<DataTypeString>()),
       "Current exception stack trace with symbolized frames (std::terminate crashes only)."},
  ...
  if (auto crash_log_owned = CrashLog::crash_log.lock()) {
    ...
    if (current_exception_trace_size > 0) {
      ...
      StackTrace::toStringEveryLine(
          const_cast<void **>(current_exception_trace.data()), 0, current_exception_trace_size,
          [&current_exception_trace_full](std::string_view line) { current_exception_trace_full.push_back(line); });
    }

    CrashLogElement element{static_cast<time_t>(time / 1000000000),
                            time,
                            signal,
                            signal_code,
                            thread_id,
