...
[[noreturn]] void throwIDWasAddedWithOtherTags(const IDType &id, const TagNamesAndValuesPtr &tags,
                                               const TagNamesAndValuesPtr &existing_tags) {
  throw Exception(ErrorCodes::BAD_ARGUMENTS, "Cannot add identifier {} with tags {} because it was added before with tags {}", toString(id),
                  ContextTimeSeriesTagsCollector::toString(*tags), ContextTimeSeriesTagsCollector::toString(*existing_tags));
}
...
class TransformFunc2To1Adapter {
 public:
  TransformFunc2To1Adapter(TransformFunc2 &&transform_func_, const TagNamesAndValuesPtr &other_argument_, bool is_other_argument_second_)
      : transform_func(std::move(transform_func_)), other_argument(other_argument_), is_other_argument_second(is_other_argument_second_) {}
...
class RemoveTagTransformFunc {
 public:
  explicit RemoveTagTransformFunc(const String &tag_to_remove_) : tag_to_remove(tag_to_remove_) {}
  ...
  TagNamesAndValuesPtr operator()(const TagNamesAndValuesPtr &old_tags) const {
    ...
    if (remove_pos == static_cast<size_t>(-1)) return old_tags;
    ...
    if (remove_pos + 1 < old_size) new_tags->insert(new_tags->end(), old_tags->begin() + remove_pos + 1, old_tags->end());
...
class RemoveTagsTransformFunc {
 public:
  explicit RemoveTagsTransformFunc(const Strings &tags_to_remove_) : tags_to_remove(tags_to_remove_.begin(), tags_to_remove_.end()) {}
  ...
  TagNamesAndValuesPtr operator()(const TagNamesAndValuesPtr &old_tags) const {
    ...
    for (size_t i = remove_pos + 1; i != old_size; ++i) {
      ...
      if (!tags_to_remove.contains(tag_name_and_value.first)) new_tags->emplace_back(tag_name_and_value);
    }
...
class RemoveAllTagsExceptTransformFunc {
 public:
  explicit RemoveAllTagsExceptTransformFunc(const Strings &tags_to_keep_) : tags_to_keep(tags_to_keep_.begin(), tags_to_keep_.end()) {}
  ...
  TagNamesAndValuesPtr operator()(const TagNamesAndValuesPtr &old_tags) const {
    ...
    for (size_t i = remove_pos + 1; i != old_size; ++i) {
      const auto &tag_name_and_value = (*old_tags)[i];
      if (tags_to_keep.contains(tag_name_and_value.first)) new_tags->emplace_back(tag_name_and_value);
    }
...
class CopyTagTransformFunc2 {
 public:
  explicit CopyTagTransformFunc2(const String &tag_to_copy_) : tag_to_copy(tag_to_copy_) {}
  ...
  TagNamesAndValuesPtr operator()(const TagNamesAndValuesPtr &dest_tags, const TagNamesAndValuesPtr &src_tags) {
    ...
    if (current_value == new_value) return dest_tags;  /// No need to copy.
    ...
    if (!new_value.empty()) new_tags->emplace_back(tag_to_copy, new_value);
...
class CopyTagsTransformFunc2 {
 public:
  explicit CopyTagsTransformFunc2(const Strings &tags_to_copy_) {
    ...
    for (const auto &tag_name : tags_to_copy_) tags_to_copy.emplace_back(tag_name, std::string_view{});
    ...
    for (size_t i = 0; i != tags_to_copy.size(); ++i) positions_in_tags_to_copy[tags_to_copy[i].first] = i;
  ...
  TagNamesAndValuesPtr operator()(const TagNamesAndValuesPtr &dest_tags, const TagNamesAndValuesPtr &src_tags) {
    ...
    for (auto &[_, new_value] : tags_to_copy) new_value = {};
...
TagNamesAndValuesPtr addDestTag(const TagNamesAndValuesPtr &old_tags, std::string_view dest_tag, String &&dest_value) {
  ...
  if (current_value == dest_value) return old_tags;
  ...
  if (!dest_value.empty()) new_tags->emplace_back(dest_tag, std::move(dest_value));
...
class JoinTagsTransformFunc {
 public:
  JoinTagsTransformFunc(const String &dest_tag_, const String &separator_, const Strings &src_tags_)
    ...
    for (size_t i = 0; i != src_tags_.size(); ++i) positions_in_src_tags[src_tags_[i]].push_back(i);
  ...
  TagNamesAndValuesPtr operator()(const TagNamesAndValuesPtr &old_tags) {
    /// Clear the values which were copied last time operator() was called.
    for (auto &src_value : src_values) src_value = {};
...
class ReplaceTagTransformFunc {
 public:
  ReplaceTagTransformFunc(const String &dest_tag_, const String &replacement_, const String &src_tag_, const String &regex_)
      : dest_tag(dest_tag_), src_tag(src_tag_), regex(regex_) {
  ...
  TagNamesAndValuesPtr operator()(const TagNamesAndValuesPtr &old_tags) {
    ...
    for (const auto &[tag_name, tag_value] : *old_tags) {
      if (tag_name == src_tag) src_value = tag_value;
    }
 ...
 private:
  void parseReplacementPattern(std::string_view replacement_) {
    for (size_t pos = 0; pos != replacement_.length();) {
      if (replacement_[pos] != '$') {
        ...
        if (next_dollar == String::npos) next_dollar = replacement_.length();
        ...
        size_t i = pos + 2;
        while ((i < replacement_.length()) && (std::isalnum(replacement_[i]) || (replacement_[i] == '_'))) ++i;
        size_t after_name = i;
  ...
  void addTextFragment(char c) { addTextFragment(std::string_view{&c, 1}); }

  void addCapturingGroupFragment(int capturing_group) {
    if (capturing_group <= regex.NumberOfCapturingGroups()) replacement_fragments.emplace_back().capturing_group = capturing_group;
  }
  ...
  void addCapturingGroupFragment(std::string_view named_capturing_group) {
    ...
    if (it != groups.end()) addCapturingGroupFragment(it->second);
  }
...
String ContextTimeSeriesTagsCollector::toString(const TagNamesAndValuesPtr &tags) { return toString(*tags); }
...
Group ContextTimeSeriesTagsCollector::getGroupForTags(const TagNamesAndValuesPtr &tags) {
  {
    ...
    if (it != groups_for_tags.end()) return it->second;
  }
...
std::vector<Group> ContextTimeSeriesTagsCollector::getGroupForTags(const std::vector<TagNamesAndValuesPtr> &tags_vector) {
  ...
  if (num_found != tags_vector.size()) {
    std::lock_guard lock{mutex};
    for (size_t i = 0; i != tags_vector.size(); ++i) {
      if (res[i] != INVALID_GROUP) continue;
      const auto &tags = tags_vector[i];
      res[i] = tryAddGroupUnlocked(tags);
      if (++num_found == tags_vector.size()) break;
    }
  }
...
Group ContextTimeSeriesTagsCollector::tryAddGroupUnlocked(const TagNamesAndValuesPtr &tags) {
  ...
  if (inserted) groups.push_back(tags);
  return it->second;
}

TagNamesAndValuesPtr ContextTimeSeriesTagsCollector::getTagsByGroup(Group group) const {
  SharedLockGuard lock{mutex};
  if (group >= groups.size()) throwGroupOutOfBound(group, groups.size());
  return groups[group];
}
...
String ContextTimeSeriesTagsCollector::extractTag(Group group, const String &tag_to_extract) const {
  ...
  for (const auto &[tag_name, tag_value] : tags) {
    if (tag_name == tag_to_extract) return tag_value;
  }
  return {};
}
...
void ContextTimeSeriesTagsCollector::extractTag(const std::vector<Group> &groups_, const String &tag_to_extract,
                                                ColumnString &out_column) const {
  ...
  for (Group group : groups_) {
    ...
    if (!found) out_column.insertDefault();
  }
}
...
void ContextTimeSeriesTagsCollector::storeTags(const IDType &id, const TagNamesAndValuesPtr &tags) {
  {
    ...
    if (it != groups_by_id.end()) {
      ...
      if (*tags != *groups.at(existing_group)) throwIDWasAddedWithOtherTags(id, tags, groups.at(existing_group));
      return;
    }
  }

  {
    ...
    if (it->second != group) throwIDWasAddedWithOtherTags(id, tags, groups.at(it->second));
  }
}
...
void ContextTimeSeriesTagsCollector::storeTags(const std::vector<IDType> &ids, const std::vector<TagNamesAndValuesPtr> &tags_vector) {
  ...
  if (num_found_groups == tags_vector.size()) return;

  {
    ...
    for (size_t i = 0; i != tags_vector.size(); ++i) {
      if (found_groups[i] != INVALID_GROUP) continue;
      const auto &id = ids[i];
      const auto &tags = tags_vector[i];
      ...
      if (++num_found_groups == tags_vector.size()) break;
    }
  }
}
...
Group ContextTimeSeriesTagsCollector::getGroupByID(const IDType &id) const {
  ...
  auto it = groups_by_id.find(id);
  if (it == groups_by_id.end()) throwUnknownID(id);
...
Group ContextTimeSeriesTagsCollector::transformTags(Group group, TransformFunc &&transform_func) {
  ...
  if (*new_tags == *old_tags) return group;
...
template <typename TransformFunc2>
std::vector<Group> ContextTimeSeriesTagsCollector::transformTags2(const std::vector<Group> &groups1, Group group2,
                                                                  TransformFunc2 &&transform_func) {
  return transformTags(groups1, TransformFunc2To1Adapter<TransformFunc2>{std::forward<TransformFunc2>(transform_func),
                                                                         /* other_argument = */ getTagsByGroup(group2),
                                                                         /* is_other_argument_second = */ true});
}

template <typename TransformFunc2>
std::vector<Group> ContextTimeSeriesTagsCollector::transformTags2(Group group1, const std::vector<Group> &groups2,
                                                                  TransformFunc2 &&transform_func) {
  return transformTags(groups2, TransformFunc2To1Adapter<TransformFunc2>{std::forward<TransformFunc2>(transform_func),
                                                                         /* other_argument = */ getTagsByGroup(group1),
                                                                         /* is_other_argument_second = */ false});
}

template <typename TransformFunc2>
std::vector<Group> ContextTimeSeriesTagsCollector::transformTags2(const std::vector<Group> &groups1, const std::vector<Group> &groups2,
                                                                  TransformFunc2 &&transform_func) {
...
std::vector<Group> ContextTimeSeriesTagsCollector::copyTag(Group dest_group, const std::vector<Group> &src_groups,
                                                           const String &tag_to_copy) {
  return transformTags2(dest_group, src_groups, CopyTagTransformFunc2{tag_to_copy});
}

std::vector<Group> ContextTimeSeriesTagsCollector::copyTag(const std::vector<Group> &dest_groups, Group src_group,
                                                           const String &tag_to_copy) {
  return transformTags2(dest_groups, src_group, CopyTagTransformFunc2{tag_to_copy});
}

std::vector<Group> ContextTimeSeriesTagsCollector::copyTag(const std::vector<Group> &dest_groups, const std::vector<Group> &src_groups,
                                                           const String &tag_to_copy) {
...
std::vector<Group> ContextTimeSeriesTagsCollector::copyTags(Group dest_group, const std::vector<Group> &src_groups,
                                                            const Strings &tags_to_copy) {
  return transformTags2(dest_group, src_groups, CopyTagsTransformFunc2{tags_to_copy});
}

std::vector<Group> ContextTimeSeriesTagsCollector::copyTags(const std::vector<Group> &dest_groups, Group src_group,
                                                            const Strings &tags_to_copy) {
  return transformTags2(dest_groups, src_group, CopyTagsTransformFunc2{tags_to_copy});
}

std::vector<Group> ContextTimeSeriesTagsCollector::copyTags(const std::vector<Group> &dest_groups, const std::vector<Group> &src_groups,
                                                            const Strings &tags_to_copy) {
...
std::vector<Group> ContextTimeSeriesTagsCollector::joinTags(const std::vector<Group> &groups_, const String &dest_tag,
                                                            const String &separator, const Strings &src_tags) {
  return transformTags(groups_, JoinTagsTransformFunc{dest_tag, separator, src_tags});
}

Group ContextTimeSeriesTagsCollector::replaceTag(Group group, const String &dest_tag, const String &replacement, const String &src_tag,
                                                 const String &regex) {
  return transformTags(group, ReplaceTagTransformFunc{dest_tag, replacement, src_tag, regex});
}

std::vector<Group> ContextTimeSeriesTagsCollector::replaceTag(const std::vector<Group> &groups_, const String &dest_tag,
                                                              const String &replacement, const String &src_tag, const String &regex) {
...
#define TIME_SERIES_ID_TO_TAGS_MAP_INSTANTIATE(IDType)                                                                                \
  ...
  template void ContextTimeSeriesTagsCollector::storeTags<IDType>(const std::vector<IDType> &ids,                                     \
                                                                  const std::vector<TagNamesAndValuesPtr> &tags_vector);              \
  ...
  template ContextTimeSeriesTagsCollector::TagNamesAndValuesPtr ContextTimeSeriesTagsCollector::getTagsByID<IDType>(const IDType &id) \
      const;                                                                                                                          \
  template std::vector<ContextTimeSeriesTagsCollector::TagNamesAndValuesPtr> ContextTimeSeriesTagsCollector::getTagsByID<IDType>(     \
