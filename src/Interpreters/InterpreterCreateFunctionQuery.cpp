...
std::optional<BlockIO> tryExecute(const ASTPtr& query_ptr, ContextMutablePtr current_context) {
  ...
  if (!create_function_query) return std::nullopt;

  AccessRightsElements access_rights_elements;
  access_rights_elements.emplace_back(AccessType::CREATE_FUNCTION);

  if (create_function_query->or_replace) access_rights_elements.emplace_back(AccessType::DROP_FUNCTION);
  ...
  UserDefinedSQLFunctionFactory::instance().registerFunction(current_context, function_name, updated_query_ptr, throw_if_exists,
                                                             replace_if_exists);
...
BlockIO InterpreterCreateFunctionQuery::execute() {
  if (auto res = tryExecute<ASTCreateSQLFunctionQuery>(query_ptr, getContext())) return std::move(res.value());

  if (auto res = tryExecute<ASTCreateWasmFunctionQuery>(query_ptr, getContext())) return std::move(res.value());
