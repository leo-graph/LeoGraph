...
void ApplyWithGlobalVisitor::visit(ASTSelectQuery &select, const std::map<String, ASTPtr> &exprs, const ASTPtr &with_expression_list) {
  ...
  if (with) {
    ...
    for (const auto &child : with->children) {
      if (const auto *ast_with_alias = dynamic_cast<const ASTWithAlias *>(child.get())) current_names.insert(ast_with_alias->alias);
    }
    for (const auto &with_alias : exprs) {
      if (!current_names.contains(with_alias.first)) with->children.push_back(with_alias.second->clone());
    }
  } else
    select.setExpression(ASTSelectQuery::Expression::WITH, with_expression_list->clone());
}

void ApplyWithGlobalVisitor::visit(ASTSelectWithUnionQuery &selects, const std::map<String, ASTPtr> &exprs,
...
void ApplyWithGlobalVisitor::visit(ASTSelectIntersectExceptQuery &selects, const std::map<String, ASTPtr> &exprs,
                                   const ASTPtr &with_expression_list) {
...
void ApplyWithGlobalVisitor::visit(ASTPtr &ast) {
  ...
  if (ASTSelectWithUnionQuery *node_union = ast->as<ASTSelectWithUnionQuery>()) {
    if (auto *first_select = typeid_cast<ASTSelectQuery *>(node_union->list_of_selects->children[0].get())) {
      ...
      if (with_expression_list) {
        ...
        for (auto &child : with_expression_list->children) {
          if (auto *ast_with_alias = dynamic_cast<ASTWithAlias *>(child.get())) exprs[ast_with_alias->alias] = child;
        }
     ...
     */
    for (auto &child : node_union->list_of_selects->children) visit(child);
  } else {
