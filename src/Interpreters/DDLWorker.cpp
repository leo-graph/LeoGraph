...
DDLWorker::DDLWorker(int pool_size_, const std::string &zk_queue_dir, const std::string &zk_replicas_dir, ContextPtr context_,
                     const Poco::Util::AbstractConfiguration *config, const String &prefix, const String &zookeeper_name_,
  ...
  if (max_entry_metric) CurrentMetrics::set(*max_entry_metric, 0);

  if (max_pushed_entry_metric) CurrentMetrics::set(*max_pushed_entry_metric, 0);

  if (1 < pool_size) {
    LOG_WARNING(log,
                "DDLWorker is configured to use multiple threads. "
                "It's not recommended because queries can be reordered. Also it may cause some unknown issues to appear.");
    worker_pool = std::make_unique<ThreadPool>(CurrentMetrics::DDLWorkerThreads, CurrentMetrics::DDLWorkerThreadsActive,
                                               CurrentMetrics::DDLWorkerThreadsScheduled, pool_size);
  }

  queue_dir = zk_queue_dir;
  if (queue_dir.back() == '/') queue_dir.resize(queue_dir.size() - 1);

  replicas_dir = zk_replicas_dir;
  if (replicas_dir.back() == '/') replicas_dir.resize(replicas_dir.size() - 1);
  ...
  if (config) {
    ...
    if (config->has(prefix + ".host_name")) config_host_name = config->getString(prefix + ".host_name");

    if (config->has(prefix + ".profile")) context->setSetting("profile", config->getString(prefix + ".profile"));
  }

  if (context->getSettingsRef()[Setting::readonly]) {
    LOG_WARNING(log,
                "Distributed DDL worker is run with readonly settings, it will not be able to execute DDL queries Set appropriate "
...
void DDLWorker::shutdown() {
  ...
  if (!prev_stop_flag) {
    ...
    cleanup_event->set();
    if (main_thread) main_thread->join();
    if (cleanup_thread) cleanup_thread->join();
    worker_pool.reset();
  }
...
DDLWorker::~DDLWorker() { DDLWorker::shutdown(); }

ZooKeeperPtr DDLWorker::getZooKeeperFromContext() const { return context->getDefaultOrAuxiliaryZooKeeper(zookeeper_name); }

ZooKeeperPtr DDLWorker::getZooKeeper() const {
  std::lock_guard lock(zookeeper_mutex);
  if (!current_zookeeper) throw Exception(ErrorCodes::NO_ZOOKEEPER, "Unable to get zookeeper");
...
ZooKeeperPtr DDLWorker::getAndSetZooKeeper() {
  ...
  if (!current_zookeeper || current_zookeeper->expired()) current_zookeeper = getZooKeeperFromContext();
...
DDLTaskPtr DDLWorker::initAndCheckTask(const String &entry_name, String &out_reason, const ZooKeeperPtr &zookeeper, bool /*dry_run*/) {
  if (entries_to_skip.contains(entry_name)) return {};
...
void DDLWorker::scheduleTasks(bool reinitialized) {
  ...
  if (reinitialized) {
    ...
    while (task_it != current_tasks.end()) {
      ...
      if (task->completely_processed) {
        ...
        if (!first_failed_task_name || task->entry_name < *first_failed_task_name) first_failed_task_name = task->entry_name;
  ...
  LOG_TRACE(log,
            "scheduleTasks: initialized={}, size_before_filtering={}, queue_size={}, "
            "entries={}..{}, "
            "first_failed_task_name={}, current_tasks_size={}, "
            "last_current_task={}, "
            "last_skipped_entry_name={}",
            initialized.load(), size_before_filtering, queue_nodes.size(), queue_nodes.empty() ? "none" : queue_nodes.front(),
            queue_nodes.empty() ? "none" : queue_nodes.back(), first_failed_task_name ? *first_failed_task_name : "none",
            current_tasks.size(), current_tasks.empty() ? "none" : current_tasks.back()->entry_name,
            last_skipped_entry_name ? *last_skipped_entry_name : "none");

  if (max_tasks_in_queue < queue_nodes.size()) cleanup_event->set();
  ...
  if (first_failed_task_name) {
    ...
    if (!current_tasks.empty()) last_task_name = current_tasks.back()->entry_name;
    if (last_skipped_entry_name && last_task_name < *last_skipped_entry_name) last_task_name = *last_skipped_entry_name;
  ...
  chassert(have_no_tasks_info || queue_nodes.end() == std::find_if(queue_nodes.begin(), queue_nodes.end(), [&](const String &entry_name) {
                                   ...
                                   bool maybe_currently_processing =
                                       current_tasks.end() != std::find_if(current_tasks.begin(), current_tasks.end(),
                                                                           [&](const auto &t) { return t->entry_name == entry_name; });
                                   ...
                                   if (begin_node == queue_nodes.end() || entry_name < *begin_node) {
                                     ...
                                     /// if someone is deleting outdated entry right now (including finished/ nodes), so we also check
                                     /// active/ status dir.
                                   ...
                                   if (last_skipped_entry_name.has_value() && !queue_fully_loaded_after_initialization_debug_helper) {
                                     ...
                                     /// we may start processing from finished task (because we don't know yet that it's finished) and it's
                                     /// ok.
                                     return false;
                                   }
...
DDLTaskBase &DDLWorker::saveTask(DDLTaskPtr &&task) {
  ...
  if (first_failed_task_name && *first_failed_task_name == current_tasks.back()->entry_name) first_failed_task_name.reset();
...
bool DDLWorker::tryExecuteQuery(DDLTaskBase &task, const ZooKeeperPtr &zookeeper, bool internal) {
  ...
  try {
    ...
    if (!task.is_initial_query) query_scope = CurrentThread::QueryScope::create(query_context);

    NullWriteBuffer nullwb;
    executeQuery(istr, nullwb, query_context, {},
                 QueryFlags{.internal = internal, .distributed_backup_restore = task.entry.is_backup_restore});
    ...
    if (auto txn = query_context->getZooKeeperMetadataTransaction()) {
      ...
      if (!txn->isExecuted()) txn->commit();
    }
  } catch (const DB::Exception &e) {
    if (task.is_initial_query) throw;
    ...
    bool no_sense_to_retry = e.code() != ErrorCodes::KEEPER_EXCEPTION && e.code() != ErrorCodes::UNFINISHED &&
                             e.code() != ErrorCodes::NOT_A_LEADER && e.code() != ErrorCodes::TABLE_IS_READ_ONLY &&
                             e.code() != ErrorCodes::CANNOT_ASSIGN_ALTER && e.code() != ErrorCodes::CANNOT_ALLOCATE_MEMORY &&
                             e.code() != ErrorCodes::TOO_MANY_SIMULTANEOUS_QUERIES && e.code() != ErrorCodes::MEMORY_LIMIT_EXCEEDED;
...
void DDLWorker::updateMaxDDLEntryID(const String &entry_name) {
  ...
  while (prev_id < id) {
    if (max_id.compare_exchange_weak(prev_id, id)) {
      if (max_entry_metric) CurrentMetrics::set(*max_entry_metric, id);
      break;
    }
  }
}
...
void DDLWorker::processTask(DDLTaskBase &task, const ZooKeeperPtr &zookeeper, bool internal_query) {
  ...
  LOG_DEBUG(log, "Processing task {} (query: {}, backup restore: {})", task.entry_name, task.query_for_logging,
            task.entry.is_backup_restore);
  chassert(!task.completely_processed);

  /// Setup tracing context on current thread for current DDL
  OpenTelemetry::TracingContextHolder tracing_ctx_holder(__PRETTY_FUNCTION__, task.entry.tracing_context,
  ...
  if (!task.was_executed) {
    ...
    task.ops.emplace_back(
        zkutil::makeCreateRequest(finished_node_path, ExecutionStatus(0).serializeText(), zkutil::CreateMode::Persistent));
...
bool DDLWorker::taskShouldBeExecutedOnLeader(const ASTPtr &ast_ddl, const StoragePtr storage) {
  ...
  if (auto *query = ast_ddl->as<ASTDropQuery>(); query && query->kind != ASTDropQuery::Kind::Truncate) return false;

  if (!ast_ddl->as<ASTAlterQuery>() && !ast_ddl->as<ASTOptimizeQuery>() && !ast_ddl->as<ASTDropQuery>() &&
      !ast_ddl->as<ASTCreateIndexQuery>() && !ast_ddl->as<ASTDropIndexQuery>())
    return false;

  if (auto *alter = ast_ddl->as<ASTAlterQuery>()) {
    // Setting alters should be executed on all replicas
    if (alter->isSettingsAlter() || alter->isFreezeAlter() || alter->isUnlockSnapshot() || alter->isMovePartitionToDiskOrVolumeAlter() ||
...
bool DDLWorker::tryExecuteQueryOnSingleReplica(DDLTaskBase &task, StoragePtr storage, const String & /*node_path*/,
  ...
  if (zookeeper->exists(is_executed_path, nullptr, event)) {
    LOG_DEBUG(log, "Task {} has already been executed by replica ({}) of the same shard.", task.entry_name,
              zookeeper->get(is_executed_path));
    if (auto op = task.getOpToUpdateLogPointer()) task.ops.push_back(op);
    return true;
  }
  ...
  while (stopwatch.elapsedSeconds() <= MAX_EXECUTION_TIMEOUT_SEC) {
    ...
    if (replica_dropped) {
      ...
      task.execution_status =
          ExecutionStatus(ErrorCodes::UNFINISHED, "Cannot execute replicated DDL query, table is dropped or detached permanently");
      return false;
    }
    ...
    if (execute_on_single_replica_lock->tryLock()) {
      ...
      SCOPE_EXIT_MEMORY({
        if (!executed_by_us && !task.ops.empty()) task.ops.pop_back();
      });
      ...
      if (extra_attempt_for_replicated_database) break;
    }
    ...
    LOG_TRACE(log, "Task {} still not executed, will try to wait for it or execute ourselves, tries count {}", task.entry_name,
              tries_count);
  }
  ...
  if (!executed_by_us && !executed_by_other_leader) {
    ...
    if (stopwatch.elapsedSeconds() >= MAX_EXECUTION_TIMEOUT_SEC) {
      LOG_WARNING(log, "Task {} was not executed by anyone, maximum timeout {} seconds exceeded", task.entry_name,
...
void DDLWorker::cleanupQueue(Int64, const ZooKeeperPtr &zookeeper) {
  ...
  for (auto it = queue_nodes.cbegin(); it < queue_nodes.cend(); ++it) {
    ...
    try {
      /// Already deleted
      if (!zookeeper->exists(node_path, &stat)) continue;

      if (!canRemoveQueueEntry(node_name, stat)) continue;
      ...
      if (rm_entry_res == Coordination::Error::ZNONODE) {
        ...
        if (entry_removed_concurrently) continue;

        /// Possible rare case: initiator node has lost connection after enqueueing entry and failed to create status dirs.
        /// No one has started to process the entry, so node_path/active and node_path/finished nodes were never created, node_path has no
        /// children. Entry became outdated, but we cannot remove remove it in a transaction with node_path/finished.
...
void DDLWorker::createStatusDirs(const std::string &node_path, const ZooKeeperPtr &zookeeper) {
  ...
  assert(!both_already_exists ||
         (zookeeper->exists(fs::path(node_path) / "active") && zookeeper->exists(fs::path(node_path) / "finished")));

  /// Failed on attempt to create node_path/finished, but node_path/active does not exist
  bool is_currently_deleting =
      responses.size() == 2 && responses[0]->error == Coordination::Error::ZOK && responses[1]->error == Coordination::Error::ZNODEEXISTS;
  if (both_created || both_already_exists) return;

  if (is_currently_deleting) {
    cleanup_event->set();
    throw Exception(ErrorCodes::UNFINISHED,
                    "Cannot create znodes (status) for {} in [Zoo]Keeper, "
                    "most likely because someone is deleting it concurrently",
                    node_path);
  }
...
String DDLWorker::enqueueQuery(DDLLogEntry &entry, const ZooKeeperRetriesInfo &retries_info) {
  ...
  if (stop_flag) throw Exception(ErrorCodes::LOGICAL_ERROR, "Can't enqueue a query after shutdown");

  if (entry.hosts.empty()) throw Exception(ErrorCodes::LOGICAL_ERROR, "Empty host list in a distributed DDL task");

  String node_path;
  if (retries_info.max_retries > 0) {
    ZooKeeperRetriesControl retries_ctl{"DDLWorker::enqueueQuery", log, retries_info};
    retries_ctl.retryLoop([&] { node_path = enqueueQueryAttempt(entry); });
  } else {
...
String DDLWorker::enqueueQueryAttempt(DDLLogEntry &entry) {
  ...
  try {
    ...
    LOG_INFO(log, "An error occurred while creating auxiliary ZooKeeper directories in {} . They will be created later. Error : {}",
             node_path, getCurrentExceptionMessage(true));
  }
...
void DDLWorker::runMainThread() {
  ...
  while (!stop_flag) {
    try {
      ...
      if (reinitialized) {
        /// Stopped
        if (!initializeMainThread()) break;

        LOG_DEBUG(log, "Initialized DDLWorker thread");
      }

      if (host_ids_updated.exchange(false)) markReplicasActive(/*reinitialized=*/false);
...
void DDLWorker::markReplicasActive(bool reinitialized) {
  ...
  if (reinitialized) {
    ...
    for (auto &it : active_node_holders) {
      ...
      if (active_node_holder) active_node_holder->setAlreadyRemoved();
  ...
  for (const auto &host_id : local_host_ids) {
    auto it = active_node_holders.find(host_id);
    if (it != active_node_holders.end()) continue;
    ...
    if (code != Coordination::Error::ZOK) {
      LOG_WARNING(log, "Cannot mark a replica active: active_path={}, active_id={}, code={}", active_path, active_id,
...
NameSet DDLWorker::getAllHostIDsFromClusters() const {
  ...
  for (const auto &it : context->getClusters()) {
    ...
    for (const auto &host_ids : cluster->getHostIDs())
      for (const auto &host_id : host_ids) host_id_set.emplace(host_id);
  }
  return host_id_set;
}
}  // namespace DB
