...
DataTypePtr convertTypeToNullable(const DataTypePtr &type) {
  if (isNullableOrLowCardinalityNullable(type)) return type;

  if (const auto *low_cardinality_type = typeid_cast<const DataTypeLowCardinality *>(type.get())) {
    const auto &dict_type = low_cardinality_type->getDictionaryType();
    if (dict_type->canBeInsideNullable()) return std::make_shared<DataTypeLowCardinality>(makeNullable(dict_type));
  }
...
static ColumnPtr tryConvertColumnToNullable(ColumnPtr col) {
  ...
  if (isColumnNullable(*col) || col->canBeInsideNullable()) return makeNullable(col);
  ...
  if (col->lowCardinality()) {
    ...
    if (nested->lowCardinality()) {
      ...
      if (nested_nullable) return ColumnConst::create(nested_nullable, col_const->size());
    }
  }
  return nullptr;
}
...
void convertColumnsToNullable(Block &block, size_t starting_pos) {
  for (size_t i = starting_pos; i < block.columns(); ++i) convertColumnToNullable(block.getByPosition(i));
}
...
ColumnPtr emptyNotNullableClone(const ColumnPtr &column) {
  if (column->isNullable()) return checkAndGetColumn<ColumnNullable>(*column).getNestedColumnPtr()->cloneEmpty();
...
ColumnRawPtrs getRawPointers(const Columns &columns) {
  ...
  for (const auto &column : columns) ptrs.push_back(column.get());
...
void restoreLowCardinalityInplace(Block &block, const Names &lowcard_keys) {
  for (const auto &column_name : lowcard_keys) {
    if (!block.has(column_name)) continue;
    if (auto &col = block.getByName(column_name); !col.type->lowCardinality()) JoinCommon::changeLowCardinalityInplace(col);
  }
...
ColumnRawPtrs extractKeysForJoin(const Block &block_keys, const Names &key_names) {
  ...
  for (size_t i = 0; i < keys_size; ++i) {
    ...
    if (const auto *nullable = checkAndGetColumn<ColumnNullable>(&*key_columns[i])) key_columns[i] = &nullable->getNestedColumn();

    if (const auto *sparse = checkAndGetColumn<ColumnSparse>(&*key_columns[i])) key_columns[i] = &sparse->getValuesColumn();
  }
...
void checkTypesOfKeys(const Block &block_left, const Names &key_names_left, const Block &block_right, const Names &key_names_right) {
  ...
  for (size_t i = 0; i < keys_size; ++i) {
    ...
    if (!left_type->equals(*right_type)) {
      throw DB::Exception(ErrorCodes::TYPE_MISMATCH, "Type mismatch of columns to JOIN by: {} :: {} at left, {} :: {} at right",
                          key_names_left[i], left_type->getName(), key_names_right[i], right_type->getName());
    }
  }
}

void checkTypesOfKeys(const Block &block_left, const Names &key_names_left, const String &condition_name_left, const Block &block_right,
                      const Names &key_names_right, const String &condition_name_right) {
...
void checkTypesOfMasks(const Block &block_left, const String &condition_name_left, const Block &block_right,
                       const String &condition_name_right) {
  auto check_cond_column_type = [](const Block &block, const String &col_name) {
    ...
    if (!dtype->canBeUsedInBooleanContext())
      throw Exception(ErrorCodes::INVALID_JOIN_ON_EXPRESSION,
                      "Expected logical expression in JOIN ON section, got unexpected column '{}' of type '{}'", col_name,
                      dtype->getName());
  };
...
void joinTotals(Block left_totals, Block right_totals, const TableJoin &table_join, Block &out_block) {
  if (table_join.forceNullableLeft()) JoinCommon::convertColumnsToNullable(left_totals);

  if (table_join.forceNullableRight()) JoinCommon::convertColumnsToNullable(right_totals);
  ...
  for (auto &col : out_block) {
    ...
    if (col.column->size() != 1) col.column = col.column->cloneResized(1);
  }
}

void addDefaultValues(IColumn &column, const DataTypePtr &type, size_t count) { type->insertManyDefaultsInto(column, count); }
...
ColumnPtr castToBoolColumn(ColumnPtr column) {
  if (!typeid_cast<const ColumnUInt8 *>(column.get())) {
    ...
    if (!tryConvertAnyColumnToBool(*column, casted_column->getData()))
      throw Exception(ErrorCodes::LOGICAL_ERROR, "Illegal type {} of column for JOIN filter. Must be Number or Nullable(Number).",
                      column->getName());
...
JoinMask getColumnAsMask(const Block &block, const String &column_name) {
  ...
  if (const auto *nullable_col = typeid_cast<const ColumnNullable *>(join_condition_col.get())) {
    if (isNothing(assert_cast<const DataTypeNullable &>(*col_type).getNestedType())) return JoinMask(false, block.rows());
    ...
    for (size_t i = 0, sz = nullable_col->size(); i < sz; ++i) res->getData()[i] = !null_map.getData()[i] && nest_col.getData()[i];
...
static IColumn::Selector hashToSelector(const WeakHash32 &hash, Sharder sharder) {
  ...
  for (size_t i = 0; i < num_rows; ++i) selector[i] = sharder(intHashCRC32(hashes[i]));
  return selector;
}
...
Blocks scatterBlockByHash(const Strings &key_columns_names, const Block &block, size_t num_shards) {
  if (num_shards == 0) throw Exception(ErrorCodes::LOGICAL_ERROR, "Number of shards must be positive");
...
static Blocks scatterBlockByHashForList(const Strings &key_columns_names, const T &blocks, size_t num_shards) {
  ...
  for (const auto &block : blocks) {
    ...
    for (size_t i = 0; i < num_shards; ++i) scattered_blocks[i].emplace_back(std::move(scattered[i]));
  }
...
NotJoinedBlocks::NotJoinedBlocks(std::unique_ptr<RightColumnsFiller> filler_, const Block &result_sample_block_, size_t left_columns_count,
                                 const TableJoin &table_join)
    : filler(std::move(filler_)), saved_block_sample(filler->getEmptyBlock()), result_sample_block(materializeBlock(result_sample_block_)) {
  ...
  for (const auto &[right_name, right_pos] : getNamesToIndexesMap(saved_block_sample)) {
    ...
    for (size_t result_pos = left_columns_count; result_pos < result_sample_block.columns(); ++result_pos) {
      ...
      if (result_name == column_name) setRightIndex(right_pos, result_pos);
    }
  }
...
void NotJoinedBlocks::extractColumnChanges(size_t right_pos, size_t result_pos) {
  ...
  if (src_props.is_nullable != dst_props.is_nullable) right_nullability_changes.push_back({result_pos, dst_props.is_nullable});

  if (src_props.is_lowcard != dst_props.is_lowcard) right_lowcard_changes.push_back({result_pos, dst_props.is_lowcard});
}
...
void NotJoinedBlocks::correctLowcardAndNullability(Block &block) {
  ...
  for (auto &[pos, added] : right_lowcard_changes) {
    ...
    if (added) {
      if (!col.type->lowCardinality()) col.type = std::make_shared<DataTypeLowCardinality>(col.type);
      col.column = changeLowCardinality(col.column, col.type->createColumn());
    } else {
