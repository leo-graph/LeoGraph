...
ColumnsDescription PartLogElement::getColumnsDescription() {
  ...
  auto merge_reason_datatype = std::make_shared<DataTypeEnum8>(DataTypeEnum8::Values{
  ...
  auto merge_algorithm_datatype = std::make_shared<DataTypeEnum8>(DataTypeEnum8::Values{
  ...
  return ColumnsDescription{
      ...
      {"event_type", std::move(event_type_datatype),
       ...
       "RemovePart — Removing or detaching a data part using [DETACH "
       "PARTITION](/sql-reference/statements/alter/partition#detach-partitionpart)."
      ...
      {"part_storage_type", std::make_shared<DataTypeString>(),
       "The type of DataPartStorage. Possible values: Packed - all files are stored in a single blob, Full - a blob per file."},
      ...
      {"deduplication_block_ids", std::make_shared<DataTypeArray>(std::make_shared<DataTypeString>()),
       "An array of block IDs used for deduplication when inserting this part."},
      ...
      /// Mutation IDs
      {"mutation_ids", std::make_shared<DataTypeArray>(std::make_shared<DataTypeString>()),
       "An array of mutation IDs applied to the source part (merged_from) for the event with type MUTATE_PART_START and MUTATE_PART."},
...
void PartLogElement::appendToBlock(MutableColumns& columns) const {
  ...
  for (const auto& name : source_part_names) source_part_names_array.push_back(name);
  ...
  for (const auto& id : deduplication_block_ids) deduplication_block_ids_array.push_back(id);
  ...
  for (const auto& id : mutation_ids) mutation_ids_array.push_back(id);
...
bool PartLog::addNewPartsImpl(ContextPtr current_context, const PartLog::PartLogEntries& parts,
                              std::vector<Strings> deduplication_block_ids, const ExecutionStatus& execution_status) {
  ...
  try {
    ...
    for (size_t i = 0; i < parts.size(); ++i) {
      ...
      if (!query_id.empty()) elem.query_id.insert(0, query_id.data(), query_id.size());
...
bool PartLog::addNewParts(ContextPtr current_context, const PartLog::PartLogEntries& parts, const ExecutionStatus& execution_status) {
  return addNewPartsImpl(current_context, parts, {}, execution_status);
}

bool PartLog::addNewPart(ContextPtr context_, const PartLog::PartLogEntry& part, Strings block_ids,
                         const ExecutionStatus& execution_status) {
  return addNewPartsImpl(context_, {part}, {std::move(block_ids)}, execution_status);
}

PartLog::PartLogEntries PartLog::createPartLogEntries(const MutableDataPartsVector& parts, UInt64 elapsed_ns,
                                                      ProfileCountersSnapshotPtr profile_counters) {
  PartLogEntries part_log_entries;
  part_log_entries.reserve(parts.size());

  for (const auto& part : parts) part_log_entries.emplace_back(part, elapsed_ns, profile_counters);
