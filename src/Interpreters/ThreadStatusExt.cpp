...
ThreadGroup::ThreadGroup(ContextPtr query_context_, Int32 os_threads_nice_value_, FatalErrorCallback fatal_error_callback_)
    : master_thread_id(CurrentThread::get().thread_id),
      ...
      memory_spill_scheduler(
          std::make_shared<MemorySpillScheduler>(query_context_->getSettingsRef()[Setting::enable_adaptive_memory_spill_scheduler])) {
...
void ThreadGroup::linkThread(UInt64 thread_id) {
  ...
  if (active_thread_count == 0) effective_group_stopwatch.restart();
...
void ThreadGroup::unlinkThread() {
  ...
  if (active_thread_count == 0) elapsed_group_ms += effective_group_stopwatch.elapsedMilliseconds();
}
...
void ThreadStatus::attachQueryForLog(const String& query_) {
  ...
  if (!thread_group) throw Exception(ErrorCodes::LOGICAL_ERROR, "No thread group attached to the thread {}", thread_id);
  ...
  try {
    ...
    if (prev_thread_group) {
      if (prev_thread_group == thread_group) {
        ...
        throw Exception(ErrorCodes::LOGICAL_ERROR, "Thread ({}) is already attached to a group (master_thread_id {})", thread_name,
                        prev_thread_group->master_thread_id);
      else
  ...
  try {
    ...
    if (cur_thread != prev_thread)
      throw Exception(ErrorCodes::LOGICAL_ERROR,
                      "ThreadGroupSwitcher-s are not properly nested: current thread changed between scope start ({}) and end ({})",
                      prev_thread ? std::to_string(prev_thread->thread_id) : "nullptr",
                      cur_thread ? std::to_string(cur_thread->thread_id) : "nullptr");
    if (cur_thread_group != thread_group)
      throw Exception(ErrorCodes::LOGICAL_ERROR,
                      "ThreadGroupSwitcher-s are not properly nested: current thread group changed between scope start (master_thread_id "
                      "{}) and end ({})",
                      thread_group->master_thread_id,
                      cur_thread_group ? "master_thread_id " + std::to_string(cur_thread_group->master_thread_id) : "nullptr");
    thread_group.reset();
...
void CurrentThread::attachInternalProfileEventsQueue(const InternalProfileEventsQueuePtr& queue) {
  if (unlikely(!current_thread)) return;
...
void ThreadStatus::applyQuerySettings() {
  ...
  if (settings[Setting::jemalloc_collect_profile_samples_in_trace_log]) Jemalloc::setCollectLocalProfileSamplesInTraceLog(true);
#endif
}
...
void ThreadStatus::attachToGroup(const ThreadGroupPtr& thread_group_, bool check_detached) {
  ...
  if (!thread_group_) throw Exception(ErrorCodes::LOGICAL_ERROR, "Attempt to attach to nullptr thread group");
...
ProfileEvents::Counters* ThreadStatus::attachProfileCountersScope(ProfileEvents::Counters* performance_counters_scope) {
  ...
  if (performance_counters_scope != &performance_counters) performance_counters_scope->setParent(&performance_counters);
...
void ThreadStatus::TimePoint::setUp() { point = std::chrono::system_clock::now(); }

UInt64 ThreadStatus::TimePoint::nanoseconds() const { return timeInNanoseconds(point); }

UInt64 ThreadStatus::TimePoint::microseconds() const { return timeInMicroseconds(point); }

UInt64 ThreadStatus::TimePoint::seconds() const { return timeInSeconds(point); }
...
void ThreadStatus::initPerformanceCounters() {
  ...
  *last_rusage = RUsageCounters::current();
  ...
  if (taskstats) (*taskstats).reset();
}

void ThreadStatus::finalizePerformanceCounters() {
  if (performance_counters_finalized) return;

  if (last_rusage->thread_id == 0) return;  // Performance counters are not initialized
  ...
  try {
    ...
    if (close_perf_descriptors) current_thread_counters.closeEventDescriptors();
  } catch (...) {
    tryLogCurrentException(log);
  }
...
void ThreadStatus::initGlobalProfiler([[maybe_unused]] UInt64 global_profiler_real_time_period,
                                      [[maybe_unused]] UInt64 global_profiler_cpu_time_period) {
#if !defined(SANITIZER) && defined(SIGEV_THREAD_ID)
  /// profilers are useless without trace collector
  auto context = Context::getGlobalContextInstance();
  if (!context->hasTraceCollector()) return;
  ...
  try {
    ...
    tryLogCurrentException(LogFrequencyLimiter(log, 10),
                           "Cannot initialize GlobalProfiler. This usually happens when RLIMIT_SIGPENDING is too low. You may tune it via "
                           "pending_signals in config.");
  }
#endif
}
...
void ThreadStatus::initQueryProfiler() {
  ...
  auto global_context_ptr = global_context.lock();
  if (!global_context_ptr || !global_context_ptr->hasTraceCollector()) return;
  ...
  try {
    ...
    tryLogCurrentException(LogFrequencyLimiter(log, 10),
                           "Cannot initialize QueryProfiler. This usually happens when RLIMIT_SIGPENDING is too low. You may tune it via "
                           "pending_signals in config.");
  }
}
...
CurrentThread::QueryScope::QueryScope(bool initialized_) : initialized(initialized_) {}

CurrentThread::QueryScope::QueryScope(QueryScope&& other) noexcept : initialized(other.initialized) { other.initialized = false; }
...
CurrentThread::QueryScope CurrentThread::QueryScope::create(ContextPtr query_context, std::function<void()> fatal_error_callback) {
  if (!query_context->hasQueryContext()) throw Exception(ErrorCodes::LOGICAL_ERROR, "Cannot initialize query scope without query context");
...
CurrentThread::QueryScope CurrentThread::QueryScope::create(ContextMutablePtr query_context, std::function<void()> fatal_error_callback) {
  if (!query_context->hasQueryContext()) query_context->makeQueryContext();
...
CurrentThread::QueryScope::~QueryScope() {
  ...
  try {
    if (log_peak_memory_usage_in_destructor) logPeakMemoryUsage();
