...
class InterpreterInsertQuery : public IInterpreter, WithMutableContext {
 public:
  InterpreterInsertQuery(const ASTPtr& query_ptr_, ContextMutablePtr context_, bool allow_materialized_, bool no_squash_,
                         bool no_destination, bool async_insert_);
  ...
  static Block getSampleBlock(const ASTInsertQuery& query, const StoragePtr& table, const StorageMetadataPtr& metadata_snapshot,
                              ContextPtr context_, bool no_destination = false, bool allow_materialized = false);
 ...
 private:
  static Block getSampleBlock(const Names& names, const StoragePtr& table, const StorageMetadataPtr& metadata_snapshot, bool allow_virtuals,
  ...
  std::pair<QueryPipeline, ParallelReplicasReadingCoordinatorPtr> buildLocalInsertSelectPipelineForParallelReplicas(
      ASTInsertQuery& query, const StoragePtr& table);

  // if applicable, build pipeline for replicated MergeTree from cluster storage
  std::optional<QueryPipeline> distributedWriteIntoReplicatedMergeTreeOrDataLakeFromClusterStorage(const ASTInsertQuery& query,
