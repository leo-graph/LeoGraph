...
struct WindowExpressionsCollectorMatcher {
  ...
  static bool needVisitChild(ASTPtr &node, const ASTPtr &child) {
    ...
    if (auto * /*func*/ _ = node->as<ASTFunction>()) return false;

    return true;
  }

  WindowExpressionsCollectorChildInfo visitNode(ASTPtr &ast, const ASTPtr &parent, WindowExpressionsCollectorChildInfo const &) {
    return visitNode(ast, parent);
  }

  WindowExpressionsCollectorChildInfo visitNode(ASTPtr &ast, const ASTPtr &parent) {
    checkStackSize();

    if (auto *func = ast->as<ASTFunction>()) {
      if (func->isWindowFunction()) return {.window_function_in_subtree = true};
      ...
      if (result.window_function_in_subtree) {
        ...
        if ((!parent || !parent->as<ASTFunction>())) expressions_with_window_functions.push_back(ast);
      }
