...
static Block originalRightBlock(const Block &block, const TableJoin &table_join) {
  ...
  for (const auto &col : block) original_right_block.insert({col.column, col.type, table_join.getOriginalName(col.name)});
...
static MutableColumns convertBlockStructure(const Block &source_sample_block, const Block &result_sample_block, MutableColumns &&columns,
  ...
  for (const auto &out_sample_col : result_sample_block) {
    ...
    if (!source_sample_block.has(out_sample_col.name)) continue;
...
DirectKeyValueJoin::DirectKeyValueJoin(std::shared_ptr<TableJoin> table_join_, const Block &right_sample_block_,
                                       std::shared_ptr<const IKeyValueEntity> storage_)
    : table_join(table_join_), storage(storage_), right_sample_block(right_sample_block_), log(getLogger("DirectKeyValueJoin")) {
  if (!table_join->oneDisjunct() || table_join->getOnlyClause().key_names_left.size() != 1 ||
  ...
  bool allowed_inner =
      isInner(table_join->kind()) && (table_join->strictness() == JoinStrictness::All || table_join->strictness() == JoinStrictness::Any ||
  ...
  if (!allowed_inner && !allowed_left) {
    throw DB::Exception(ErrorCodes::NOT_IMPLEMENTED, "Strictness {} and kind {} is not supported by direct JOIN", table_join->strictness(),
                        table_join->kind());
  }
...
bool DirectKeyValueJoin::addBlockToJoin(const Block &, bool) { throw DB::Exception(ErrorCodes::LOGICAL_ERROR, "Unreachable code reached"); }
...
static void selectFirstMatchForEachKey(const IColumn::Offsets &offsets, MutableColumns &columns, NullMap &null_map) {
  ...
  for (auto &&col : columns) col = IColumn::mutate(col->filter(filter, offsets.size()));
...
JoinResultPtr DirectKeyValueJoin::joinBlock(Block block) {
  ...
  if (!key_col.column) return {};

  Block original_right_block = originalRightBlock(right_sample_block, *table_join);
  Block right_block_to_use =
      !right_sample_block_with_storage_column_names.empty() ? right_sample_block_with_storage_column_names : original_right_block;
  ...
  if (!offsets.empty()) {
    ...
    if (is_all_join) {
      ...
      for (auto &&col : replicated_columns) col = IColumn::mutate(col->replicate(offsets));
  ...
  if (has_null_matches && need_filtering) {
    ...
    for (auto &&col : dst_columns) col = IColumn::mutate(col->filter(null_map, non_null_rows));
