...
class NamedSessionsStorage {
 public:
  ...
  std::pair<std::shared_ptr<NamedSessionData>, bool> acquireSession(const ContextPtr &global_context, const UUID &user_id,
                                                                    const String &session_id, std::chrono::steady_clock::duration timeout,
    ...
    if (it == sessions.end()) {
      if (throw_if_not_found) throw Exception(ErrorCodes::SESSION_NOT_FOUND, "Session {} not found", session_id);

      /// Create a new session from current context.
      it = sessions.insert(std::make_pair(key, std::make_shared<NamedSessionData>(key, global_context, timeout, *this))).first;
      const auto &session = it->second;

      if (!thread.joinable()) thread = ThreadFromGlobalPool{&NamedSessionsStorage::cleanThread, this};
    ...
    if (session->close_time_bucket != std::chrono::steady_clock::time_point{}) {
      ...
      if (bucket_sessions.empty()) close_time_buckets.erase(bucket_it);
 ...
 private:
  ...
  constexpr static std::chrono::nanoseconds::rep close_interval_ns =
      std::chrono::duration_cast<std::chrono::nanoseconds>(close_interval).count();
  ...
  void scheduleCloseSession(NamedSessionData &session, std::unique_lock<std::mutex> &) {
    ...
    LOG_TEST(log, "Schedule closing session with session_id: {}, user_id: {}", session.key.second, session.key.first);
  }
  ...
  void cleanThread() {
    ...
    while (!quit) {
      ...
      lock.lock();
      if (cond.wait_for(lock, close_interval, [this]() -> bool { return quit; })) break;
    }
  }
  ...
  std::vector<std::shared_ptr<NamedSessionData>> closeSessions(std::unique_lock<std::mutex> &lock) {
    ...
    for (auto bucket_it = close_time_buckets.begin(); bucket_it != close_time_buckets.end();
         bucket_it = close_time_buckets.erase(bucket_it)) {
      const auto &[time_bucket, session_keys] = *bucket_it;
      if (time_bucket > now) break;

      for (const auto &key : session_keys) {
        const auto &session_it = sessions.find(key);

        if (session_it == sessions.end()) continue;
        ...
        if (session.use_count() != 1) {
          ...
          /// resetting a pointer.
          LOG_TEST(log, "Delay closing session with session_id: {}, user_id: {}, refcount: {}", key.second, key.first, session.use_count());
...
void NamedSessionData::release() { parent.releaseSession(*this); }

void Session::shutdownNamedSessions() { NamedSessionsStorage::instance().shutdown(); }
...
Session::~Session() {
  ...
  if (named_session) named_session->release();
...
std::unordered_set<AuthenticationType> Session::getAuthenticationTypesOrLogInFailure(const String &user_name) const {
  try {
    ...
    if (auto session_log = getSessionLog()) session_log->addLoginFailure(auth_id, getClientInfo(), user_name, e);
...
void Session::authenticate(const String &user_name, const String &password, const Poco::Net::SocketAddress &address,
                           const std::optional<Poco::Net::SocketAddress> &connection_address, const Strings &external_roles_) {
...
void Session::authenticate(const Credentials &credentials_, const Poco::Net::SocketAddress &address_,
                           const std::optional<Poco::Net::SocketAddress> &connection_address, const Strings &external_roles_) {
  if (session_context) throw Exception(ErrorCodes::LOGICAL_ERROR, "If there is a session context it must be created after authentication");

  if (session_tracker_handle) throw Exception(ErrorCodes::LOGICAL_ERROR, "Session tracker handle was created before authentication finish");
  ...
  try {
    ...
    LOG_DEBUG(log, "{} Authenticated with global context as user {}", toString(auth_id), toString(*user_id));
    ...
    if (!external_roles_.empty() && global_context->getSettingsRef()[Setting::push_external_roles_in_interserver_queries]) {
      ...
      LOG_DEBUG(log, "User {} has external_roles applied: [{}] ({})", toString(*user_id), fmt::join(external_roles_, ", "),
                external_roles_.size());
    }
  } catch (const Exception &e) {
...
void Session::checkIfUserIsStillValid() {
  if (const auto valid_until = user_authenticated_with.getValidUntil()) {
    ...
    if (now > valid_until) throw Exception(ErrorCodes::USER_EXPIRED, "Authentication method used has expired");
  }
}

void Session::onAuthenticationFailure(const std::optional<String> &user_name, const Poco::Net::SocketAddress &address_,
                                      const Exception &e) {
...
const ClientInfo &Session::getClientInfo() const { return session_context ? session_context->getClientInfo() : *prepared_client_info; }
...
void Session::setClientVersion(UInt64 client_version_major, UInt64 client_version_minor, UInt64 client_version_patch,
...
void Session::setConnectionClientVersion(UInt64 client_version_major, UInt64 client_version_minor, UInt64 client_version_patch,
                                         unsigned client_tcp_protocol_version) {
  if (session_context) {
    session_context->setConnectionClientVersion(client_version_major, client_version_minor, client_version_patch,
...
const OpenTelemetry::TracingContext &Session::getClientTraceContext() const {
  if (session_context) return session_context->getClientTraceContext();
...
ContextMutablePtr Session::makeSessionContext() {
  if (session_context) throw Exception(ErrorCodes::LOGICAL_ERROR, "Session context already exists");
  if (query_context_created) throw Exception(ErrorCodes::LOGICAL_ERROR, "Session context must be created before any query context");
  if (!user_id) throw Exception(ErrorCodes::LOGICAL_ERROR, "Session context must be created after authentication");
  if (session_tracker_handle) throw Exception(ErrorCodes::LOGICAL_ERROR, "Session tracker handle was created before making session");

  LOG_DEBUG(log, "Creating session context with user_id: {}", toString(*user_id));
  ...
  session_tracker_handle =
      session_context->getSessionTracker().trackSession(*user_id, {}, session_context->getSettingsRef()[Setting::max_sessions_for_user]);
...
ContextMutablePtr Session::makeSessionContext(const String &session_name_, std::chrono::steady_clock::duration timeout_,
                                              bool session_check_) {
  if (session_context) throw Exception(ErrorCodes::LOGICAL_ERROR, "Session context already exists");
  if (query_context_created) throw Exception(ErrorCodes::LOGICAL_ERROR, "Session context must be created before any query context");
  if (!user_id) throw Exception(ErrorCodes::LOGICAL_ERROR, "Session context must be created after authentication");
  ...
  LOG_DEBUG(log, "Creating named session context with name: {}, user_id: {}", session_name_, toString(*user_id));
  ...
  if (!access->tryGetUser()) {
    ...
    if (max_session_for_user_field) max_sessions_for_user = max_session_for_user_field->safeGet<UInt64>();
  }
  ...
  session_tracker_handle = session_context->getSessionTracker().trackSession(*user_id, {session_name_}, max_sessions_for_user);
...
ContextMutablePtr Session::makeQueryContextImpl(const ClientInfo *client_info_to_copy, ClientInfo *client_info_to_move) const {
  ...
  if (auto query_context_user = query_context->getAccess()->tryGetUser()) {
    LOG_TRACE(log, "{} Creating query context from {} context, user_id: {}, parent context user: {}", toString(auth_id),
              from_session_context ? "session" : "global", toString(*user_id), query_context_user->getName());
  }
  ...
  /// Set user information for the new context: current profiles, roles, access rights.
  if (user_id && !query_context->getAccess()->tryGetUser()) query_context->setUser(*user_id, external_roles);

  /// Query context is ready.
  query_context_created = true;
  if (user_id) user = query_context->getUser();
...
void Session::recordLoginSuccess(ContextPtr login_context) const {
  if (notified_session_log_about_login) return;

  if (!login_context) throw Exception(ErrorCodes::LOGICAL_ERROR, "Session or query context must be created");

  if (auto session_log = getSessionLog()) {
    const auto &settings = login_context->getSettingsRef();
    const auto access = login_context->getAccess();

    session_log->addLoginSuccess(auth_id, named_session ? named_session->key.second : "", settings, access->getAccess(), getClientInfo(),
                                 user, user_authenticated_with);
  }
