...
void replaceJoinedTable(const ASTSelectQuery &select_query) {
   ...
   */
  if (table_join.strictness == JoinStrictness::Asof) return;
...
class RenameQualifiedIdentifiersMatcher {
 public:
  ...
  static void visit(ASTPtr &ast, Data &data) {
    ...
    if (auto *node = ast->as<ASTQualifiedAsterisk>()) visit(*node, ast, data);
  }

  static bool needChildVisit(ASTPtr &node, const ASTPtr &child) {
    if (node->as<ASTTableExpression>() || node->as<ASTQualifiedAsterisk>() || child->as<ASTSubquery>()) return false;  // NOLINT
    return true;
  }

 private:
  static void visit(ASTIdentifier &identifier, ASTPtr &, Data &data) {
    if (identifier.isShort()) return;
    ...
    for (const auto &table : data) {
      ...
      if (match == IdentifierSemantic::ColumnMatch::AliasedTableName || match == IdentifierSemantic::ColumnMatch::DBAndTable) {
        if (rewritten)
...
bool JoinedTables::isLeftTableSubquery() const { return left_table_expression && left_table_expression->as<ASTSelectWithUnionQuery>(); }

bool JoinedTables::isLeftTableFunction() const { return left_table_expression && left_table_expression->as<ASTFunction>(); }
...
bool JoinedTables::resolveTables() {
  ...
  if (tables_with_columns.size() != table_expressions.size()) throw Exception(ErrorCodes::LOGICAL_ERROR, "Unexpected tables count");
...
void JoinedTables::rewriteDistributedInAndJoins(ASTPtr &query) {
  ...
  String database;
  if (!renamed_tables.empty()) database = context->getCurrentDatabase();

  for (auto &[subquery, ast_tables] : renamed_tables) {
    std::vector<DatabaseAndTableWithAlias> renamed;
    renamed.reserve(ast_tables.size());
    for (auto &ast : ast_tables) renamed.emplace_back(DatabaseAndTableWithAlias(ast->as<ASTTableIdentifier &>(), database));
...
std::shared_ptr<TableJoin> JoinedTables::makeTableJoin(const ASTSelectQuery &select_query_) {
  if (tables_with_columns.size() < 2) return {};
  ...
  if (!table_join->isSpecialStorage() && settings[Setting::enable_optimize_predicate_expression]) replaceJoinedTable(select_query_);
