...
std::optional<Poco::Net::SocketAddress> ClientInfo::getLastForwardedFor() const {
  if (forwarded_for.empty()) return {};
  String last = forwarded_for.substr(forwarded_for.find_last_of(',') + 1);
  boost::trim(last);

  /// IPv6 address with port
  if (last[0] == '[') return Poco::Net::SocketAddress{Poco::Net::AddressFamily::IPv6, last};

  const auto colons = std::count(last.begin(), last.end(), ':');

  /// IPv6 address without port
  if (colons > 1) return Poco::Net::SocketAddress{Poco::Net::AddressFamily::IPv6, last, 0};

  /// IPv4 address with port
  if (colons == 1) return Poco::Net::SocketAddress{Poco::Net::AddressFamily::IPv4, last};
...
void ClientInfo::write(WriteBuffer& out, UInt64 server_protocol_revision) const {
  ...
  if (interface == Interface::TCP) {
    ...
    if (server_protocol_revision >= DBMS_MIN_REVISION_WITH_X_FORWARDED_FOR_IN_CLIENT_INFO) writeBinary(forwarded_for, out);

    if (server_protocol_revision >= DBMS_MIN_REVISION_WITH_REFERER_IN_CLIENT_INFO) writeBinary(http_referer, out);
  }

  if (server_protocol_revision >= DBMS_MIN_REVISION_WITH_QUOTA_KEY_IN_CLIENT_INFO) writeBinary(quota_key, out);

  if (server_protocol_revision >= DBMS_MIN_PROTOCOL_VERSION_WITH_DISTRIBUTED_DEPTH) writeVarUInt(distributed_depth, out);

  if (interface == Interface::TCP) {
    if (server_protocol_revision >= DBMS_MIN_REVISION_WITH_VERSION_PATCH) writeVarUInt(client_version_patch, out);
  }
...
void ClientInfo::read(ReadBuffer& in, UInt64 client_protocol_revision) {
  ...
  if (interface == Interface::TCP) {
    ...
    if (client_protocol_revision >= DBMS_MIN_REVISION_WITH_X_FORWARDED_FOR_IN_CLIENT_INFO) readBinary(forwarded_for, in);

    if (client_protocol_revision >= DBMS_MIN_REVISION_WITH_REFERER_IN_CLIENT_INFO) readBinary(http_referer, in);
  }

  if (client_protocol_revision >= DBMS_MIN_REVISION_WITH_QUOTA_KEY_IN_CLIENT_INFO) readBinary(quota_key, in);

  if (client_protocol_revision >= DBMS_MIN_PROTOCOL_VERSION_WITH_DISTRIBUTED_DEPTH) readVarUInt(distributed_depth, in);
...
bool ClientInfo::clientVersionEquals(const ClientInfo& other, bool compare_patch) const {
  ...
  return client_version_major == other.client_version_major && client_version_minor == other.client_version_minor && patch_equals &&
...
void ClientInfo::setFromHTTPRequest(const Poco::Net::HTTPRequest& request) {
  ...
  for (const auto& header : request) {
    ...
    if (key_lowercase.starts_with("x-clickhouse") || key_lowercase == "authentication") continue;
