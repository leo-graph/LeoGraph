    ...
    const PlanScope * initial_scope)
{
    ...
    buildRootQueryPlanImpl(*plan, *query, context, environment, initial_scope, nullptr);
    return plan;
}
    ...
    PlanScope * output_scope)
{
    if (initial_scope)
    {
        PlanBuilder builder(context, *initial_scope, environment);
    ...
    PlanBuilder builder(context, environment);
    builder.buildSingleQuery(query_plan, query);
    if (output_scope)
        *output_scope = builder.getScope();
}
...
void buildRootQueryPlan(
    ...
    const PlanEnvironment & environment)
{
    buildRootQueryPlanImpl(query_plan, query, std::move(context), environment, nullptr, nullptr);
}
    ...
    PlanScope & scope)
{
    ...
    buildRootQueryPlanImpl(query_plan, query, std::move(context), environment, &initial_scope, &scope);
}
