...
String deriveTempName(const String &name, JoinTableSide block_side) {
  if (block_side == JoinTableSide::Left) return "--pmj_cond_left_" + name;
...
ColumnWithTypeAndName condtitionColumnToJoinable(const Block &block, const String &src_column_name, JoinTableSide block_side) {
  ...
  if (!src_column_name.empty()) {
    ...
    if (join_mask.hasData()) {
      for (size_t i = 0; i < res_size; ++i) null_map->getData()[i] = join_mask.isRowFiltered(i);
    }
  }
...
int nullableCompareAt(const IColumn &left_column, const IColumn &right_column, size_t lhs_pos, size_t rhs_pos) {
  ...
  if constexpr (has_left_nulls && has_right_nulls) {
    ...
    if (left_nullable && right_nullable) {
      ...
      /// NULL != NULL case
      if (left_column.isNullAt(lhs_pos)) return null_direction_hint;
  ...
  if constexpr (has_right_nulls) {
    if (const auto *right_nullable = checkAndGetColumn<ColumnNullable>(&right_column)) {
      if (right_column.isNullAt(rhs_pos)) return -null_direction_hint;
...
Block extractMinMax(const Block &block, const Block &keys) {
  if (block.rows() == 0) throw Exception(ErrorCodes::LOGICAL_ERROR, "Unexpected empty block");
  ...
  for (auto &column : min_max) column.column = column.column->convertToFullColumnIfLowCardinality();
  return min_max;
}
...
class RowBitmaps {
 public:
  ...
  explicit RowBitmaps(size_t size) {
    maps.reserve(size);
    for (size_t i = 0; i < size; ++i) maps.emplace_back(std::make_unique<Bitmap>());
  }

  bool used(size_t bitmap_number) const { return !maps[bitmap_number]->empty(); }

  void applyOr(size_t bitmap_number, Container &&addition) noexcept { maps[bitmap_number]->applyOr(std::move(addition)); }
  ...
  IColumn::Filter getNotUsed(size_t bitmap_number) const {
    ...
    for (size_t i = 0; i < bitmap.size(); ++i) filter[i] = !bitmap[i];
    return filter;
  }
...
class MergeJoinCursor {
 public:
  MergeJoinCursor(const Block &block, const SortDescription &desc_) : impl(block, desc_) {
    ...
    if (impl.permutation) throw Exception(ErrorCodes::LOGICAL_ERROR, "MergeJoinCursor doesn't support permutation");
  ...
  MergeJoinEqualRange getNextEqualRange(MergeJoinCursor &rhs) {
    if (has_left_nullable && has_right_nullable) return getNextEqualRangeImpl<true, true>(rhs);
    if (has_left_nullable) return getNextEqualRangeImpl<true, false>(rhs);
    if (has_right_nullable) return getNextEqualRangeImpl<false, true>(rhs);
    return getNextEqualRangeImpl<false, false>(rhs);
  }

  int intersect(const Block &min_max, const Names &key_names) {
    if (end() == 0 || min_max.rows() != 2) throw Exception(ErrorCodes::LOGICAL_ERROR, "Unexpected block size");
    ...
    for (size_t i = 0; i < impl.sort_columns_size; ++i) {
      ...
      if (!first_vs_max) first_vs_max = nullableCompareAt<true, true>(left_column, right_column, position(), 1);

      if (!last_vs_min) last_vs_min = nullableCompareAt<true, true>(left_column, right_column, last_position, 0);
    }

    if (first_vs_max > 0) return 1;
    if (last_vs_min < 0) return -1;
    return 0;
  }
 ...
 private:
  ...
  MergeJoinEqualRange getNextEqualRangeImpl(MergeJoinCursor &rhs) {
    if (atEnd() || rhs.atEnd()) return MergeJoinEqualRange{position(), rhs.position(), 0, 0};
...
void makeSortAndMerge(const Names &keys, SortDescription &sort, SortDescription &merge) {
  NameSet unique_keys;
  for (const auto &sd : merge) unique_keys.insert(sd.column_name);
...
void copyRightRange(const Block &right_block, const Block &right_columns_to_add, MutableColumns &columns, size_t row_position,
                    size_t rows_to_add) {
...
void joinEqualsAnyLeft(const Block &right_block, const Block &right_columns_to_add, MutableColumns &right_columns,
                       const MergeJoinEqualRange &range) {
  copyRightRange(right_block, right_columns_to_add, right_columns, range.right_start, range.left_length);
}

template <bool is_all>
bool joinEquals(const Block &left_block, const Block &right_block, const Block &right_columns_to_add, MutableColumns &left_columns,
                MutableColumns &right_columns, MergeJoinEqualRange &range, size_t max_rows [[maybe_unused]]) {
  bool one_more = true;
  ...
  if constexpr (is_all) {
    ...
    if (range_rows > max_rows) {
      ...
      if (!range.right_length) range.right_length = 1;
      one_more = false;
    }
...
void joinInequalsLeft(const Block &left_block, MutableColumns &left_columns, const Block &right_block, MutableColumns &right_columns,
                      size_t start, size_t end) {
  if (end <= start) return;

  size_t rows_to_add = end - start;
  if constexpr (copy_left) copyLeftRange(left_block, left_columns, start, rows_to_add);
...
MergeJoin::MergeJoin(std::shared_ptr<TableJoin> table_join_, SharedHeader right_sample_block_)
    ...
    case JoinStrictness::Semi:
      if (!is_left && !is_inner)
        throw Exception(ErrorCodes::NOT_IMPLEMENTED,
                        "Not supported. MergeJoin supports SEMI and ANY variants only for LEFT and INNER JOINs.");
      break;
    default:
  ...
  if (max_files_to_merge < 2) throw Exception(ErrorCodes::PARAMETER_OUT_OF_BOUND, "max_files_to_merge cannot be less than 2");
  ...
  if (!table_join->oneDisjunct()) throw DB::Exception(ErrorCodes::NOT_IMPLEMENTED, "MergeJoin does not support OR in JOIN ON section");
  ...
  for (const auto &column : right_table_keys)
    if (required_right_keys.contains(column.name)) right_columns_to_add.insert(ColumnWithTypeAndName{nullptr, column.type, column.name});
...
void MergeJoin::setTotals(const Block &totals_block) {
  ...
  if (is_right || is_full) used_rows_bitmap = std::make_shared<RowBitmaps>(getRightBlocksCount());
}
...
void MergeJoin::mergeInMemoryRightBlocks() {
  ...
  builder.addTransform(std::make_shared<MergeSortingTransform>(builder.getSharedHeader(), right_sort_description, max_rows_in_right_block,
                                                               /*max_block_bytes=*/0,
                                                               /*limit_=*/0,
  ...
  while (executor.pull(block)) {
    ...
    if (skip_not_intersected) min_max_right_blocks.emplace_back(extractMinMax(block, right_table_keys));
...
void MergeJoin::mergeFlushedRightBlocks() {
  ...
  if (!memory_limit && rows_limit) memory_limit = right_blocks.bytes * rows_limit / right_blocks.row_count;

  cached_right_blocks =
      std::make_unique<Cache>(CurrentMetrics::MergeJoinBlocksCacheBytes, CurrentMetrics::MergeJoinBlocksCacheCount, memory_limit);
}
...
Block MergeJoin::modifyRightBlock(const Block &src_block) const { return materializeBlock(src_block); }
...
class MergeJoinResult : public IJoinResult {
 ...
 public:
  MergeJoinResult(Block block_, MergeJoin &merge_join_) : block(std::move(block_)), merge_join(merge_join_) {}
...
JoinResultPtr MergeJoin::joinBlock(Block block) { return std::make_unique<MergeJoinResult>(std::move(block), *this); }
...
void MergeJoin::joinBlock(Block &block, std::optional<MergeJoin::NotProcessed> &not_processed) {
  ...
  if (!block.empty()) {
    ...
    for (const auto &column_name : key_names_left) {
      if (block.getByName(column_name).type->lowCardinality()) lowcard_keys.push_back(column_name);
    }
  ...
  if (!not_processed && left_blocks_buffer) {
    if (block.empty() || block.rows()) block = left_blocks_buffer->exchange(std::move(block));
  ...
  if (!not_processed && left_blocks_buffer) not_processed = NotProcessed{{}, 0, 0, 0, 0};

  if (needConditionJoinColumn()) block.erase(deriveTempName(mask_column_name_left, JoinTableSide::Left));
}
  ...
  if (with_left_inequals) {
    for (size_t i = starting_right_block; i < right_blocks_count; ++i) {
      ...
      if (!leftJoin<is_all>(left_cursor, block, right_block, left_columns, right_columns, left_key_tail)) {
        not_processed = extraBlock<is_all>(block, std::move(left_columns), std::move(right_columns), left_cursor.position(), left_key_tail,
                                           skip_right, i);
        return;
      }
    }
    ...
    for (size_t i = starting_right_block; i < right_blocks_count; ++i) {
      ...
      if constexpr (is_all) {
        if (!allInnerJoin(left_cursor, block, right_block, left_columns, right_columns, left_key_tail)) {
          not_processed = extraBlock<is_all>(block, std::move(left_columns), std::move(right_columns), left_cursor.position(),
                                             left_key_tail, skip_right, i);
          return;
        }
      } else
...
static size_t maxRangeRows(size_t current_rows, size_t max_rows) {
  if (!max_rows) return std::numeric_limits<size_t>::max();
  if (current_rows >= max_rows) return 0;
...
template <bool is_all>
std::optional<MergeJoin::NotProcessed> MergeJoin::extraBlock(Block &processed, MutableColumns &&left_columns,
                                                             MutableColumns &&right_columns, size_t left_position [[maybe_unused]],
                                                             size_t left_key_tail [[maybe_unused]], size_t right_position [[maybe_unused]],
  ...
  if constexpr (!in_memory) {
    auto load_func = [&]() -> std::shared_ptr<Block> {
      ...
      if (Block eof_block = input->read(); !eof_block.empty()) {
        throw Exception(ErrorCodes::LOGICAL_ERROR, "Expected one block per file, got block {} in file {}", eof_block.dumpStructure(),
                        flushed_right_blocks[pos].getHolder()->describeFilePath());
      }
      return result;
    };
...
void MergeJoin::initRightTableWriter() {
  disk_writer = std::make_unique<SortedBlocksWriter>(size_limits, table_join->getTempDataOnDisk(), right_sample_block,
                                                     right_sort_description, max_rows_in_right_block, max_files_to_merge);
...
class NotJoinedMerge final : public NotJoinedBlocks::RightColumnsFiller {
 public:
  NotJoinedMerge(const MergeJoin &parent_, UInt64 max_block_size_) : parent(parent_), max_block_size(max_block_size_) {}
...
IBlocksStreamPtr MergeJoin::getNonJoinedBlocks(const Block &left_sample_block, const Block &result_sample_block,
...
bool MergeJoin::needConditionJoinColumn() const { return !mask_column_name_left.empty() || !mask_column_name_right.empty(); }
...
MergeJoin::RightBlockInfo::RightBlockInfo(std::shared_ptr<Block> block_, size_t block_number_, size_t &skip_, RowBitmaps *bitmaps_)
    : block(block_), block_number(block_number_), skip(skip_), bitmaps(bitmaps_) {}

MergeJoin::RightBlockInfo::~RightBlockInfo() {
  if (used_bitmap) bitmaps->applyOr(block_number, std::move(*used_bitmap));
}

void MergeJoin::RightBlockInfo::setUsed(size_t start, size_t length) {
  if (bitmaps) {
    if (!used_bitmap) used_bitmap = std::make_unique<std::vector<bool>>(block->rows(), false);

    for (size_t i = 0; i < length; ++i) (*used_bitmap)[start + i] = true;
  }
}
