...
BlockIO InterpreterCreateQuery::createDatabase(ASTCreateQuery &create) {
  ...
  if (DatabaseCatalog::instance().isDatabaseExist(database_name)) {
    if (create.if_not_exists) return {};
  ...
  if (db_num_limit > 0 && !internal) {
    ...
    for (const auto &system_database : system_databases) {
      if (db_count > 0 && DatabaseCatalog::instance().isDatabaseExist(std::string(system_database))) --db_count;
    }
  ...
  if (!create.storage && create.attach) {
    ...
    if (create.table || !create.storage)
      throw Exception(ErrorCodes::INCORRECT_QUERY, "Metadata file {} contains incorrect CREATE DATABASE query",
    ...
    if (create.attach) throw Exception(ErrorCodes::UNKNOWN_DATABASE_ENGINE, "Database engine must be specified for ATTACH DATABASE query");
    if (!create.storage) {
  ...
  if (create.storage && !create.storage->engine) throw Exception(ErrorCodes::INCORRECT_QUERY, "Database engine must be specified");
  ...
  if (create.storage->engine->name == "Atomic" || create.storage->engine->name == "Replicated" ||
    ...
    if (create.attach && create.uuid == UUIDHelpers::Nil)
      throw Exception(ErrorCodes::INCORRECT_QUERY,
                      ...
                      create.getDatabase());
    if (create.uuid == UUIDHelpers::Nil) create.uuid = UUIDHelpers::generateV4();
  ...
  if (create.uuid != UUIDHelpers::Nil) create.setDatabase(TABLE_WITH_UUID_NAME_PLACEHOLDER);
  ...
  if (need_write_metadata) {
    ...
    writeMetadataFile(default_db_disk,
  ...
  try {
    ...
    if (added) DatabaseCatalog::instance().detachDatabase(getContext(), database_name, false, false);
...
ASTPtr InterpreterCreateQuery::formatColumns(const NamesAndTypesList &columns) {
  ...
  for (const auto &column : columns) {
    ...
    column_declaration->setType(
        parseQuery(type_parser, pos, end, "data type", 0, DBMS_DEFAULT_MAX_PARSER_DEPTH, DBMS_DEFAULT_MAX_PARSER_BACKTRACKS));
...
ASTPtr InterpreterCreateQuery::formatColumns(const NamesAndTypesList &columns, const NamesAndAliases &alias_columns) {
  ...
  for (const auto &alias_column : alias_columns) {
    ...
    column_declaration->setType(
        parseQuery(type_parser, type_pos, type_end, "data type", 0, DBMS_DEFAULT_MAX_PARSER_DEPTH, DBMS_DEFAULT_MAX_PARSER_BACKTRACKS));
    ...
    column_declaration->setDefaultExpression(parseQuery(expression_parser, alias_pos, alias_end, "expression", 0,
...
ASTPtr InterpreterCreateQuery::formatColumns(const ColumnsDescription &columns) {
  ...
  for (const auto &column : columns) {
    ...
    column_declaration->setType(parseQuery(type_parser, type_name_pos, type_name_end, "data type", 0, DBMS_DEFAULT_MAX_PARSER_DEPTH,
...
ASTPtr InterpreterCreateQuery::formatIndices(const IndicesDescription &indices) {
  ...
  for (const auto &index : indices)
    if (!index.isImplicitlyCreated()) res->children.push_back(index.definition_ast->clone());
...
ASTPtr InterpreterCreateQuery::formatConstraints(const ConstraintsDescription &constraints) {
  ...
  for (const auto &constraint : constraints.getConstraints()) res->children.push_back(constraint->clone());
...
ASTPtr InterpreterCreateQuery::formatProjections(const ProjectionsDescription &projections) {
  ...
  for (const auto &projection : projections) res->children.push_back(projection.definition_ast->clone());

  return res;
}

DataTypePtr InterpreterCreateQuery::getColumnType(const ASTColumnDeclaration &col_decl, const LoadingStrictnessLevel mode,
  ...
  if (LoadingStrictnessLevel::ATTACH <= mode) setVersionToAggregateFunctions(column_type, true);

  if (col_decl.null_modifier) {
    if (column_type->isNullable())
      throw Exception(ErrorCodes::ILLEGAL_SYNTAX_FOR_DATA_TYPE, "Can't use [NOT] NULL modifier with Nullable type");
    if (*col_decl.null_modifier) column_type = makeNullable(column_type);
...
ColumnsDescription InterpreterCreateQuery::getColumnsDescription(const ASTExpressionList &columns_ast, ContextPtr context_,
  ...
  for (const auto &ast : columns_ast.children) {
    ...
    if (col_decl.getCollation() && !context_->getSettingsRef()[Setting::compatibility_ignore_collation_in_create_table]) {
      throw Exception(ErrorCodes::NOT_IMPLEMENTED,
                      "Cannot support collation, please set compatibility_ignore_collation_in_create_table=true");
    }
  ...
  for (auto ast_it = columns_ast.children.begin(); ast_it != columns_ast.children.end(); ++ast_it, ++name_type_it) {
    ...
    if (auto default_expression = col_decl.getDefaultExpression()) {
      ...
      if (col_decl.getType())
        ...
        if ((col_decl.null_modifier && *col_decl.null_modifier) || make_columns_nullable) column.type = makeNullable(column.type);
      }
    ...
    if (auto comment = col_decl.getComment()) column.comment = comment->as<ASTLiteral &>().value.safeGet<String>();
    ...
    if (auto ttl = col_decl.getTTL()) column.ttl = ttl;
...
ConstraintsDescription InterpreterCreateQuery::getConstraintsDescription(const ASTExpressionList *constraints,
                                                                         const ColumnsDescription &columns, ContextPtr local_context) {
...
InterpreterCreateQuery::TableProperties InterpreterCreateQuery::getTablePropertiesAndNormalizeCreateQuery(ASTCreateQuery &create,
                                                                                                          LoadingStrictnessLevel mode) {
  /// Set the table engine if it was not specified explicitly.
  setEngine(create);

  /// We have to check access rights again (in case engine was changed).
  if (create.storage && create.storage->engine) getContext()->checkAccess(AccessType::TABLE_ENGINE, create.storage->engine->name);

  /// If this is a TimeSeries table then we need to normalize list of columns (add missing columns and reorder), and also set inner table
  /// engines.
  ...
  if (create.columns_list) {
    ...
    if (create.columns_list->indices) {
      for (const auto &index : create.columns_list->indices->children) {
        ...
        IndexDescription index_desc = IndexDescription::getIndexFromAST(index->clone(), properties.columns, is_implicitly_created,
                                                                        escape_index_filenames, getContext());
        if (properties.indices.has(index_desc.name))
          throw Exception(ErrorCodes::ILLEGAL_INDEX, "Duplicated index name {} is not allowed. Please use a different index name",
    ...
    as_storage_lock =
        as_storage->lockForShare(getContext()->getCurrentQueryId(), getContext()->getSettingsRef()[Setting::lock_acquire_timeout]);
    ...
    if (create.storage && endsWith(create.storage->engine->name, "MergeTree")) {
      ...
      for (const auto &index : indices)
        if (!index.isImplicitlyCreated()) properties.indices.push_back(index);
    ...
    if (create.is_clone_as) {
      ...
      if (create.storage) {
        ...
        if (as_storage_sorting_key_str != storage_sorting_key_str) {
          /// It is possible that the storage only has primary key and an empty sorting key, and as_storage has both primary key and sorting
    ...
    if (create.isParameterizedView()) return properties;
    ...
    if (create.aliases_list) {
      ...
      if (!select_with_union_query) throw Exception(ErrorCodes::LOGICAL_ERROR, "Expected ASTSelectWithUnionQuery");
      ...
      for (const auto &select : selects) {
        ...
        if (!select_query) throw Exception(ErrorCodes::LOGICAL_ERROR, "Expected ASTSelectQuery inside ASTSelectWithUnionQuery");

        auto select_expression_list = select_query->select();

        if (!select_expression_list) throw Exception(ErrorCodes::LOGICAL_ERROR, "No select expressions in SELECT query");
    ...
    if (getContext()->getSettingsRef()[Setting::allow_experimental_analyzer]) {
      as_select_sample = InterpreterSelectQueryAnalyzer::getSampleBlock(create.select->clone(), select_context,
      ...
      as_select_sample = InterpreterSelectWithUnionQuery::getSampleBlock(create.select->clone(), select_context, false /* is_subquery */,
    ...
    if (!create.dictionary || !create.dictionary->source) return {};
  ...
  if (!create.columns_list) create.set(create.columns_list, make_intrusive<ASTColumns>());
  ...
  if (!as_table_saved.empty()) create.is_create_empty = false;
...
void InterpreterCreateQuery::validateMaterializedViewColumnsAndEngine(const ASTCreateQuery &create, const TableProperties &properties,
  ...
  if (create.hasTargetTableID(ViewTarget::To)) {
    ...
    try {
      ...
      if (!getContext()->getSettingsRef()[Setting::allow_materialized_view_with_bad_select]) throw;
    }
  ...
  if (create.refresh_strategy && !create.refresh_strategy->append) {
    if (database && database->getEngineName() != "Atomic" && database->getEngineName() != "Replicated")
      throw Exception(ErrorCodes::INCORRECT_QUERY,
                      "Refreshable materialized views (except with APPEND) only support Atomic and Replicated database engines, but "
                      "database {} has engine {}",
                      create.getDatabase(), database->getEngineName());
  ...
  if (check_columns) {
    try {
      if (getContext()->getSettingsRef()[Setting::allow_experimental_analyzer]) {
        ...
        if (create.refresh_strategy) context->setCurrentDatabaseUnchecked(create.getDatabase());
  ...
  if (check_columns) {
    ...
    for (const NameAndTypePair &nt : all_output_columns) output_types[nt.name] = nt.type;
    ...
    if (input_columns.empty())
      throw Exception(ErrorCodes::THERE_IS_NO_COLUMN,
                      "None of the columns produced by the SELECT query are present in the target table. Use 'AS' to assign aliases that "
                      "match column names");
...
void setDefaultTableEngine(ASTStorage &storage, DefaultTableEngine engine) {
  if (engine == DefaultTableEngine::None) throw Exception(ErrorCodes::ENGINE_REQUIRED, "Table engine is not specified in CREATE query");
...
void setNullTableEngine(ASTStorage &storage) {
  storage.forEachPointerToChild([](IAST **ptr, boost::intrusive_ptr<IAST> *) { *ptr = nullptr; });
...
ASTs *getEngineArgsFromCreateQuery(ASTCreateQuery &create_query) {
  ...
  if (!storage_def) return nullptr;

  if (!storage_def->engine) return nullptr;

  const ASTFunction &engine_def = *storage_def->engine;
  if (!engine_def.arguments) return nullptr;

  return &engine_def.arguments->children;
}

bool hasDynamicSubcolumns(const ColumnsDescription &columns) {
  return std::any_of(columns.begin(), columns.end(), [](const auto &column) { return column.type->hasDynamicSubcolumns(); });
}
...
void InterpreterCreateQuery::setEngine(ASTCreateQuery &create) const {
  if (create.as_table_function) {
    if (getContext()->getSettingsRef()[Setting::restore_replace_external_table_functions_to_null]) {
      ...
      if (properties && properties->allow_readonly) return;
      if (!create.storage) {
  ...
  if (create.is_dictionary || create.is_ordinary_view || create.is_window_view) return;
  ...
  if (create.isTemporary()) {
    ...
    if (!create.cluster.empty()) throw Exception(ErrorCodes::INCORRECT_QUERY, "Temporary tables cannot be created with ON CLUSTER clause");
  ...
  if (create.is_materialized_view) {
    ...
    if (create.is_materialized_view_with_external_target()) return;
  ...
  if (!create.as_table.empty()) {
    ...
    if (as_create.is_materialized_view_with_external_target()) {
      throw Exception(ErrorCodes::INCORRECT_QUERY,
                      "Cannot CREATE a table AS {}, it is a Materialized View without storage. Use \"AS {}\" instead", qualified_name,
...
void InterpreterCreateQuery::assertOrSetUUID(ASTCreateQuery &create, const DatabasePtr &database) const {
  ...
  bool is_replicated_database_internal =
      database->getEngineName() == "Replicated" && getContext()->getClientInfo().is_replicated_database_internal;
  bool from_path = create.has_attach_from_path;
  bool is_on_cluster = getContext()->getClientInfo().query_kind == ClientInfo::QueryKind::SECONDARY_QUERY;

  if (database->getEngineName() == "Replicated" && create.uuid != UUIDHelpers::Nil && !is_replicated_database_internal && !internal &&
      !is_on_cluster && !create.attach) {
    if (getContext()->getSettingsRef()[Setting::database_replicated_allow_explicit_uuid] == 0) {
      throw Exception(ErrorCodes::BAD_ARGUMENTS,
                      "It's not allowed to explicitly specify UUIDs for tables in Replicated databases, "
    ...
    if (getContext()->getSettingsRef()[Setting::database_replicated_allow_explicit_uuid] == 1) {
      LOG_WARNING(&Poco::Logger::get("InterpreterCreateQuery"),
      ...
      LOG_WARNING(&Poco::Logger::get("InterpreterCreateQuery"),
                  ...
                  old_uuid, create.uuid);
    }
  }

  if (is_replicated_database_internal && !internal) {
    if (create.uuid == UUIDHelpers::Nil) throw Exception(ErrorCodes::LOGICAL_ERROR, "Table UUID is not specified in DDL log");
  }
  ...
  if (database->getUUID() != UUIDHelpers::Nil) {
    if (create.attach && !from_path && create.uuid == UUIDHelpers::Nil) {
      throw Exception(ErrorCodes::INCORRECT_QUERY,
                      ...
                      kind_upper, kind_upper, create.table->formatForErrorMessage(), kind_upper, create.table->formatForErrorMessage(),
                      kind_upper, create.table->formatForErrorMessage(), kind_upper, create.table->formatForErrorMessage());
    }
    ...
    if (has_uuid && !is_on_cluster && !internal) {
      ...
      throw Exception(ErrorCodes::INCORRECT_QUERY, "{} UUID specified, but engine of database {} is not Atomic", kind,
                      create.getDatabase());
    }
...
void addTableDependencies(const ASTCreateQuery &create, const ASTPtr &query_ptr, const ContextPtr &context) {
  ...
  DatabaseCatalog::instance().addDependencies(
      qualified_name, ref_dependencies.dependencies, loading_dependencies,
...
void checkTableCanBeAddedWithNoCyclicDependencies(const ASTCreateQuery &create, const ASTPtr &query_ptr, const ContextPtr &context) {
  ...
  auto ref_dependencies = getDependenciesFromCreateQuery(context->getGlobalContext(), qualified_name, query_ptr,
                                                         context->getCurrentDatabase(), /*can_throw*/ true);
  auto loading_dependencies =
      getLoadingDependenciesFromCreateQuery(context->getGlobalContext(), qualified_name, query_ptr, /*can_throw*/ true);
  DatabaseCatalog::instance().checkTableCanBeAddedWithNoCyclicDependencies(qualified_name, ref_dependencies.dependencies,
                                                                           loading_dependencies);
}

bool isReplicated(const ASTStorage &storage) {
  if (!storage.engine) return false;
...
BlockIO InterpreterCreateQuery::createTable(ASTCreateQuery &create) {
  ...
  if (create.isTemporary() && create.attach) throw Exception(ErrorCodes::SYNTAX_ERROR, "ATTACH of TEMPORARY tables are not supported");
  ...
  bool is_secondary_query =
      getContext()->getZooKeeperMetadataTransaction() && !getContext()->getZooKeeperMetadataTransaction()->isInitialQuery();
  auto mode = getLoadingStrictnessLevel(create.attach, /*force_attach*/ false, /*has_force_restore_data_flag*/ false,
  ...
  if (create.attach && (!create.storage || !create.storage->engine) && !create.columns_list) {
    ...
    if (database && database->shouldReplicateQuery(getContext(), query_ptr)) {
      ...
      return database->tryEnqueueReplicatedDDL(query_ptr, getContext(),
                                               QueryFlags{.internal = internal, .distributed_backup_restore = is_restore_from_backup},
                                               std::move(guard));
    }

    if (!create.cluster.empty()) return executeQueryOnCluster(create);

    if (!database) throw Exception(ErrorCodes::UNKNOWN_DATABASE, "Database {} does not exist", backQuoteIfNeed(database_name));
    ...
    if (likely(need_ddl_guard)) ddl_guard = DatabaseCatalog::instance().getDDLGuard(database_name, create.getTable(), database.get());
    ...
    if (create.attach_as_replicated.has_value()) {
      if (database->isTableExist(create.getTable(), getContext()))
        throw Exception(ErrorCodes::TABLE_ALREADY_EXISTS, "Table {}.{} already exists", backQuoteIfNeed(create.getDatabase()),
    ...
    if (!create.is_dictionary && create_query.is_dictionary)
      throw Exception(ErrorCodes::INCORRECT_QUERY, "Cannot ATTACH TABLE {}.{}, it is a Dictionary", backQuoteIfNeed(database_name),
                      backQuoteIfNeed(create.getTable()));

    if (create.is_dictionary && !create_query.is_dictionary)
      throw Exception(ErrorCodes::INCORRECT_QUERY, "Cannot ATTACH DICTIONARY {}.{}, it is a Table", backQuoteIfNeed(database_name),
                      backQuoteIfNeed(create.getTable()));
  ...
  if (!create.attach_short_syntax && create.attach_as_replicated.has_value())
    throw Exception(ErrorCodes::SUPPORT_IS_DISABLED, "Attaching table as [not] replicated is supported only for short attach queries");
  ...
  if (create.has_attach_from_path) {
    ...
    if (getContext()->getClientInfo().query_kind == ClientInfo::QueryKind::INITIAL_QUERY) {
      ...
      if (data_path.is_relative()) data_path = (user_files / data_path).lexically_normal();
      if (!startsWith(data_path, user_files))
        throw Exception(ErrorCodes::PATH_ACCESS_DENIED, "Data directory {} must be inside {} to attach it", String(data_path),
                        String(user_files));
    ...
    LOG_WARNING(log,
                "ATTACH TABLE query with full table definition is not recommended: "
  ...
  if (!create.isTemporary() && !create.database) create.setDatabase(current_database);

  if (create.targets) create.targets->setCurrentDatabase(current_database);
  ...
  if (!UserDefinedSQLFunctionFactory::instance().empty()) UserDefinedSQLFunctionVisitor::visit(query_ptr, getContext());
  ...
  // In case of an ON CLUSTER query, the database may not be present on the initiator node
  if (need_add_to_database) database = DatabaseCatalog::instance().tryGetDatabase(database_name);
  ...
  if (create.select && create.is_materialized_view && mode <= LoadingStrictnessLevel::CREATE) {
    ...
    if (create.is_materialized_view_with_inner_table()) getContext()->setSetting("flatten_nested", false);
    validateMaterializedViewColumnsAndEngine(create, properties, database);
  }

  bool is_storage_replicated = false;
  if (create.storage && isReplicated(*create.storage)) is_storage_replicated = true;

  if (create.targets) {
    for (const auto &inner_table_engine : create.targets->getInnerEngines()) {
      if (isReplicated(*inner_table_engine)) is_storage_replicated = true;
    }
  }
  ...
  if (database && database->shouldReplicateQuery(getContext(), query_ptr)) {
    ...
    guard->releaseTableLock();
    return database->tryEnqueueReplicatedDDL(
        query_ptr, getContext(), QueryFlags{.internal = internal, .distributed_backup_restore = is_restore_from_backup}, std::move(guard));
  }
...
void checkForUnsupportedColumns(IStorage &storage, LoadingStrictnessLevel mode) {
  if (mode <= LoadingStrictnessLevel::CREATE && hasDynamicSubcolumns(storage.getInMemoryMetadataPtr()->getColumns()) &&
      !storage.supportsDynamicSubcolumns()) {
...
void validateStorage(IStorage &storage, LoadingStrictnessLevel mode) try {
...
bool InterpreterCreateQuery::doCreateTable(ASTCreateQuery &create, const InterpreterCreateQuery::TableProperties &properties,
                                           DDLGuardPtr &ddl_guard, LoadingStrictnessLevel mode) {
  if (create.isTemporary()) {
    if (create.if_not_exists && getContext()->tryResolveStorageID({"", create.getTable()}, Context::ResolveExternal)) return false;
    ...
    auto creator = [&](const StorageID &table_id) {
      auto res = StorageFactory::instance().get(create, database->getTableDataPath(table_id.getTableName()), getContext(),
                                                getContext()->getGlobalContext(), properties.columns, properties.constraints, mode,
  ...
  if (database->isTableExist(create.getTable(), getContext())) {
    ...
    if (create.if_not_exists) return false;
    if (create.replace_view) {
      ...
      if (database->getTable(create.getTable(), getContext())->isDictionary())
        throw Exception(ErrorCodes::DICTIONARY_ALREADY_EXISTS, "Dictionary {}.{} already exists", backQuoteIfNeed(create.getDatabase()),
                        backQuoteIfNeed(create.getTable()));
      throw Exception(ErrorCodes::TABLE_ALREADY_EXISTS, "Table {}.{} already exists", backQuoteIfNeed(create.getDatabase()),
                      backQuoteIfNeed(create.getTable()));
    }
    ...
    if (mode <= LoadingStrictnessLevel::CREATE) database->checkTableNameLength(create.getTable());
  }

  data_path = database->getTableDataPath(create);
  // When creating a table, when checking if the data path exists, it should use the local disk to check, not the database disk. Because the
  // database disk stores metadata files only.
  auto full_data_path = fs::path{getContext()->getPath()} / data_path;

  if (!create.attach && !data_path.empty() && fs::exists(full_data_path)) {
    if (getContext()->getZooKeeperMetadataTransaction() && !getContext()->getZooKeeperMetadataTransaction()->isInitialQuery() &&
        !DatabaseCatalog::instance().hasUUIDMapping(create.uuid) && Context::getGlobalContextInstance()->isServerCompletelyStarted() &&
      ...
      throw Exception(storage_already_exists_error_code, "Directory for {} data {} already exists", Poco::toLower(storage_name),
                      String(data_path));
    }
  }
  ...
  if (from_path) {
    if (data_path.empty())
      throw Exception(ErrorCodes::NOT_IMPLEMENTED, "ATTACH ... FROM ... query is not supported for {} database engine",
  ...
  if (!internal && is_initial_query && !is_predefined_database) throwIfTooManyEntities(create);
  ...
  if (create.as_table_function) {
    ...
    res = StorageFactory::instance().get(create, data_path, getContext(), getContext()->getGlobalContext(), properties.columns,
                                         properties.constraints, mode, is_restore_from_backup);
    ...
    if (auto *engine_args = getEngineArgsFromCreateQuery(create_query)) res->addInferredEngineArgsToCreateQuery(*engine_args, getContext());
  }
  ...
  if (replicated_storage) {
    ...
    if (fault(thread_local_rng)) {
      ...
      if (!create.attach) replicated_storage->dropIfEmpty();
  ...
  if (from_path) res->rename(actual_data_path, {create.getDatabase(), create.getTable(), create.uuid});
...
void InterpreterCreateQuery::throwIfTooManyEntities(ASTCreateQuery &create) const {
  ...
  if (create.is_dictionary)
    check_and_throw(ServerSetting::max_dictionary_num_to_throw, CurrentMetrics::AttachedDictionary, "max_dictionary_num_to_throw",
                    "dictionaries");
    ...
    check_and_throw(ServerSetting::max_replicated_table_num_to_throw, CurrentMetrics::AttachedReplicatedTable,
                    "max_replicated_table_num_to_throw", "replicated tables");
  else
    check_and_throw(ServerSetting::max_table_num_to_throw, CurrentMetrics::AttachedTable, "max_table_num_to_throw", "tables");
}

BlockIO InterpreterCreateQuery::doCreateOrReplaceTable(ASTCreateQuery &create, const InterpreterCreateQuery::TableProperties &properties,
                                                       LoadingStrictnessLevel mode) {
  /// Replicated database requires separate contexts for each DDL query
  ContextPtr current_context = getContext();
  if (auto txn = current_context->getZooKeeperMetadataTransaction()) txn->setIsCreateOrReplaceQuery();
...

  {
    ...
    if (database->getUUID() == UUIDHelpers::Nil)
      throw Exception(ErrorCodes::INCORRECT_QUERY, "{} query is supported only for Atomic databases",
                      create.create_or_replace ? "CREATE OR REPLACE TABLE" : "REPLACE TABLE");

    if (mode <= LoadingStrictnessLevel::CREATE) database->checkTableNameLength(table_to_replace_name);
  }
  ...
  try {
    ...
    ASTRenameQuery::Element elem{
        ASTRenameQuery::Table{create.getDatabase().empty() ? nullptr : make_intrusive<ASTIdentifier>(create.getDatabase()),
...
BlockIO InterpreterCreateQuery::doCreateOrReplaceTemporaryTable(ASTCreateQuery &create,
  ...
  /// Note, until BlockIO will be "executed" - the table is empty, so it is not atomic, but this is OK, since concurrent access from the
  /// same session to a temporary table is not possible
  return fillTableIfNeeded(create);
}
...
BlockIO InterpreterCreateQuery::fillTableIfNeeded(const ASTCreateQuery &create) {
  ...
  if (create.isCreateQueryWithImmediateInsertSelect()) {
    ...
    return InterpreterInsertQuery(insert, getContext(), getContext()->getSettingsRef()[Setting::insert_allow_materialized_columns],
                                  /* no_squash */ false,
...
void InterpreterCreateQuery::prepareOnClusterQuery(ASTCreateQuery &create, ContextPtr local_context, const String &cluster_name) {
  ...
  if (cluster->maybeCrossReplication()) {
    ...
    if (DDLLogEntry::NORMALIZE_CREATE_ON_INITIATOR_VERSION <= on_cluster_version)
      throw Exception(ErrorCodes::NOT_IMPLEMENTED,
                      "Value {} of setting distributed_ddl_entry_format_version "
                      "is incompatible with cross-replication",
                      on_cluster_version);
    ...
    if (!create.storage) return;
    if (!create.storage->engine) return;
    if (!startsWith(create.storage->engine->name, "Replicated")) return;

    bool has_explicit_zk_path_arg =
        create.storage->engine->arguments && create.storage->engine->arguments->children.size() >= 2 &&
...
BlockIO InterpreterCreateQuery::execute() {
  ...
  if (!create.cluster.empty() && !maybeRemoveOnCluster(query_ptr, getContext())) {
    ...
    if (is_create_database || on_cluster_version < DDLLogEntry::NORMALIZE_CREATE_ON_INITIATOR_VERSION) return executeQueryOnCluster(create);
  }
  ...
  if (is_create_database) return createDatabase(create);
...
AccessRightsElements InterpreterCreateQuery::getRequiredAccess() const {
  ...
  if (!create.table) {
    ...
    if (create.isTemporary()) {
      ...
      if (create.replace_table) required_access.emplace_back(AccessType::DROP_TABLE, create.getDatabase(), create.getTable());
  ...
  if (create.targets) {
    for (const auto &target : create.targets->targets) {
      ...
      if (target_id) required_access.emplace_back(AccessType::SELECT | AccessType::INSERT, target_id.database_name, target_id.table_name);
    }
  }

  if (create.storage && create.storage->engine) required_access.emplace_back(AccessType::TABLE_ENGINE, create.storage->engine->name);
...
void InterpreterCreateQuery::processSQLSecurityOption(ContextMutablePtr context_, ASTSQLSecurity &sql_security, bool is_materialized_view,
                                                      LoadingStrictnessLevel mode) {
  ...
  if (sql_security.definer) {
    ...
    if (definer_name != current_user_name) context_->checkAccess(AccessType::SET_DEFINER, definer_name);
  ...
  if (sql_security.type == SQLSecurityType::NONE) context_->checkAccess(AccessType::ALLOW_SQL_SECURITY_NONE);
}
...
void InterpreterCreateQuery::convertMergeTreeTableIfPossible(ASTCreateQuery &create, DatabasePtr database, bool to_replicated) {
  ...
  if (database->getEngineName() != "Atomic")
    throw Exception(ErrorCodes::NOT_IMPLEMENTED, "Table engine conversion to replicated is supported only for Atomic databases");

  if (!create.storage || !create.storage->engine || create.storage->engine->name.find("MergeTree") == std::string::npos)
    throw Exception(ErrorCodes::NOT_IMPLEMENTED, "Table engine conversion is supported only for MergeTree family engines");

  String engine_name = create.storage->engine->name;
  if (engine_name.starts_with("Replicated")) {
    if (to_replicated) throw Exception(ErrorCodes::INCORRECT_QUERY, "Can not attach table as replicated, table is already replicated");
...
void InterpreterCreateQuery::clearTransactionMetadata(const String &table_data_path, ContextPtr local_context) {
  LOG_INFO(getLogger("InterpreterCreateQuery"), "Clearing transaction metadata for table, relative path: {} when ATTACH AS REPLICATED.",
           table_data_path);
  ...
  for (const auto &[disk_name, disk] : disks) {
    try {
      ...
      if (!disk->existsDirectory(table_data_path)) continue;
      ...
      for (auto it = disk->iterateDirectory(table_data_path); it->isValid(); it->next()) {
        ...
        if (!disk->existsDirectory(part_path)) continue;
      ...
      throw Exception(ErrorCodes::CANNOT_RESTORE_TABLE,
                      "Cannot ATTACH AS REPLICATED: failed to clear transaction metadata on disk {}, due to {}", disk_name,
                      getCurrentExceptionMessage(false));
    }
  }

  LOG_INFO(getLogger("InterpreterCreateQuery"), "Removed {} transaction metadata files for table, relative path: {}.", total_removed,
           table_data_path);
}
