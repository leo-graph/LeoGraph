...
ASTPtr rewriteSelectQuery(ContextPtr context, const ASTPtr &query, const std::string &remote_database, const std::string &remote_table,
...
SelectStreamFactory::SelectStreamFactory(SharedHeader header_, const StorageSnapshotPtr &storage_snapshot_,
                                         QueryProcessingStage::Enum processed_stage_)
    : header(header_), storage_snapshot(storage_snapshot_), processed_stage(processed_stage_) {}

void SelectStreamFactory::createForShard(const Cluster::ShardInfo &shard_info, const ASTPtr &query_ast, const StorageID &main_table,
                                         const ASTPtr &table_func_ptr, ContextPtr context, std::vector<QueryPlanPtr> &local_plans,
                                         Shards &remote_shards, UInt32 shard_count, bool parallel_replicas_enabled,
                                         AdditionalShardFilterGenerator shard_filter_generator) {
  createForShardImpl(shard_info, query_ast, {}, main_table, table_func_ptr, std::move(context), local_plans, remote_shards, shard_count,
                     parallel_replicas_enabled, std::move(shard_filter_generator));
}

void SelectStreamFactory::createForShardImpl(const Cluster::ShardInfo &shard_info, const ASTPtr &query_ast,
                                             const QueryTreeNodePtr &query_tree, const StorageID &main_table, const ASTPtr &table_func_ptr,
                                             ContextPtr context, std::vector<QueryPlanPtr> &local_plans, Shards &remote_shards,
                                             UInt32 shard_count, bool parallel_replicas_enabled,
  ...
  // If lazy is true, a lazy pipe will be created. It will try to use the local replica and, if not possible, will use DelayedSource for
  // reading from remote replica.
  auto emplace_remote_stream = [&](bool lazy = false) {
    ...
    if (settings[Setting::allow_experimental_analyzer] && settings[Setting::serialize_query_plan] &&
      ...
      query_plan = createLocalPlan(query_ast, *header, context, processed_stage, shard_info.shard_num, shard_count, true,
                                   shard_info.default_database);

      shard_header = query_plan->getCurrentHeader();
    } else {
      if (settings[Setting::allow_experimental_analyzer])
        std::tie(shard_header, planner_context) = InterpreterSelectQueryAnalyzer::getSampleBlockAndPlannerContext(
  ...
  if (settings[Setting::prefer_localhost_replica] && shard_info.isLocal() && !parallel_replicas_enabled) {
    ...
    LOG_WARNING(getLogger("ClusterProxy::SelectStreamFactory"), "Local replica of shard {} is stale (delay: {}s.)", shard_info.shard_num,
                local_delay);
    ...
    if (!settings[Setting::fallback_to_stale_replicas_for_distributed_queries]) {
      ...
      throw Exception(ErrorCodes::ALL_REPLICAS_ARE_STALE,
                      ...
                      shard_info.shard_num, toString(local_delay));
    }
...
void SelectStreamFactory::createForShard(const Cluster::ShardInfo &shard_info, const QueryTreeNodePtr &query_tree,
                                         const StorageID &main_table, const ASTPtr &table_func_ptr, ContextPtr context,
                                         std::vector<QueryPlanPtr> &local_plans, Shards &remote_shards, UInt32 shard_count,
                                         bool parallel_replicas_enabled, AdditionalShardFilterGenerator shard_filter_generator) {
  createForShardImpl(shard_info, queryNodeToDistributedSelectQuery(query_tree), query_tree, main_table, table_func_ptr, std::move(context),
                     local_plans, remote_shards, shard_count, parallel_replicas_enabled, std::move(shard_filter_generator));
}
