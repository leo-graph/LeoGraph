...
ContextMutablePtr updateSettingsAndClientInfoForCluster(const Cluster &cluster, bool is_remote_function, ContextPtr context,
                                                        const Settings &settings, const StorageID &main_table, ASTPtr additional_filter_ast,
                                                        LoggerPtr log, const DistributedSettings *distributed_settings) {
  ...
  if (settings[Setting::force_optimize_skip_unused_shards_nesting] && settings[Setting::force_optimize_skip_unused_shards]) {
    if (new_settings[Setting::force_optimize_skip_unused_shards_nesting] == 1) {
      ...
      if (log)
        LOG_TRACE(log, "force_optimize_skip_unused_shards_nesting is now {}",
                  new_settings[Setting::force_optimize_skip_unused_shards_nesting].value);
    }
  }
  ...
  if (settings[Setting::optimize_skip_unused_shards_nesting] && settings[Setting::optimize_skip_unused_shards]) {
    if (new_settings[Setting::optimize_skip_unused_shards_nesting] == 1) {
      ...
      if (log) LOG_TRACE(log, "Disabling optimize_skip_unused_shards for nested queries (optimize_skip_unused_shards_nesting exceeded)");
    } else {
  ...
  if (context->canUseTaskBasedParallelReplicas()) {
    ...
    if (disable_parallel_replicas) new_settings[Setting::allow_experimental_parallel_reading_from_replicas] = 0;
  }
  ...
  if (context->canUseOffsetParallelReplicas()) new_settings[Setting::serialize_query_plan] = false;

  auto new_context = Context::createCopy(context);
  new_context->setSettings(new_settings);
  new_context->setClientInfo(new_client_info);

  if (context->canUseParallelReplicasCustomKeyForCluster(cluster)) new_context->disableOffsetParallelReplicas();
...
ContextMutablePtr updateSettingsForCluster(const Cluster &cluster, ContextPtr context, const Settings &settings,
  ...
  return updateSettingsAndClientInfoForCluster(cluster,
                                               /* is_remote_function= */ false, context, settings, main_table,
...
static ThrottlerPtr getThrottler(const ContextPtr &context) {
  ...
  if (auto process_list_element = context->getProcessListElement()) user_level_throttler = process_list_element->getUserNetworkThrottler();

  /// Network bandwidth limit, if needed.
  ThrottlerPtr throttler;
  if (settings[Setting::max_network_bandwidth] || settings[Setting::max_network_bytes]) {
    throttler = std::make_shared<Throttler>(settings[Setting::max_network_bandwidth], settings[Setting::max_network_bytes],
                                            "Limit for bytes to send or receive over network exceeded.", user_level_throttler);
  } else
...
AdditionalShardFilterGenerator getShardFilterGeneratorForCustomKey(const Cluster &cluster, ContextPtr context,
  ...
  if (!context->canUseParallelReplicasCustomKeyForCluster(cluster)) return {};

  const auto &settings = context->getSettingsRef();
  auto custom_key_ast = parseCustomKeyForTable(settings[Setting::parallel_replicas_custom_key], *context);
  if (custom_key_ast == nullptr) return {};

  return [my_custom_key_ast = std::move(custom_key_ast), column_description = columns,
          ...
          custom_key_range_upper = settings[Setting::parallel_replicas_custom_key_range_upper].value, query_context = context,
          replica_count = cluster.getShardsInfo().front().per_replica_pools.size()](uint64_t replica_num) -> ASTPtr {
    return getCustomKeyFilterForParallelReplica(replica_count, replica_num - 1, my_custom_key_ast,
                                                {custom_key_type, custom_key_range_lower, custom_key_range_upper}, column_description,
                                                query_context);
  };
}
...
void executeQuery(QueryPlan &query_plan, SharedHeader header, QueryProcessingStage::Enum processed_stage, const StorageID &main_table,
  ...
  auto new_context = updateSettingsAndClientInfoForCluster(*cluster, is_remote_function, context, settings, main_table,
                                                           query_info.additional_filter_ast, log, &distributed_settings);
  if (context->getSettingsRef()[Setting::allow_experimental_parallel_reading_from_replicas].value &&
    ...
    LOG_TRACE(log, "Parallel reading from replicas is disabled for cluster. There are no shards with more than 1 replica: cluster={}",
              cluster->getName());
  }
  ...
  if (context->getSettingsRef()[Setting::allow_experimental_analyzer]) {
    for (size_t i = 0, s = cluster->getShardsInfo().size(); i < s; ++i) {
      ...
      stream_factory.createForShard(shard_info, query_for_shard, main_table, table_func_ptr, new_context, plans, remote_shards,
                                    static_cast<UInt32>(shards), parallel_replicas_enabled, shard_filter_generator);
    }
  } else {
    for (size_t i = 0, s = cluster->getShardsInfo().size(); i < s; ++i) {
      const auto &shard_info = cluster->getShardsInfo()[i];
      ...
      stream_factory.createForShard(shard_info, query_ast_for_shard, main_table, table_func_ptr, new_context, plans, remote_shards,
                                    static_cast<UInt32>(shards), parallel_replicas_enabled, shard_filter_generator);
    }
  }
  ...
  if (!remote_shards.empty()) {
    ...
    scalars.emplace("_shard_count",
                    Block{{DataTypeUInt32().createColumnConst(1, shards), std::make_shared<DataTypeUInt32>(), "_shard_count"}});
    auto external_tables = context->getExternalTables();

    auto plan = std::make_unique<QueryPlan>();
    auto read_from_remote = std::make_unique<ReadFromRemote>(
        std::move(remote_shards), header, processed_stage, main_table, table_func_ptr, new_context, getThrottler(context),
        std::move(scalars), std::move(external_tables), log, shards, query_info.storage_limits, not_optimized_cluster->getName());
...
static ContextMutablePtr updateContextForParallelReplicas(const LoggerPtr &logger, const ContextPtr &context, const UInt64 &shard_num) {
  ...
  if (shard_num > 0 && settings[Setting::parallel_replicas_support_projection].value) {
    LOG_TRACE(logger,
              "Disabling 'parallel_replicas_support_projection'. Currently, it's not supported for queries with parallel replicas over "
              "distributed tables");
...
static std::pair<ClusterPtr, size_t> prepareClusterForParallelReplicas(const LoggerPtr &logger, const ContextPtr &context) {
  ...
  if (shard_num > 0) {
    ...
    if (shard_num > shard_count)
      throw Exception(ErrorCodes::LOGICAL_ERROR, "Shard number is greater than shard count: shard_num={} shard_count={} cluster={}",
                      shard_num, shard_count, not_optimized_cluster->getName());
...
static std::pair<std::vector<ConnectionPoolPtr>, size_t> prepareConnectionPoolsForParallelReplicas(const LoggerPtr &logger,
  ...
  if (max_replicas_to_use > shard.getAllNodeCount()) {
    LOG_TRACE(logger,
              ...
              settings[Setting::max_parallel_replicas].value, shard.getAllNodeCount());
  ...
  for (auto &pool : shuffled_pool) pools_to_use.emplace_back(std::move(pool.pool));

  return {pools_to_use, max_replicas_to_use};
}

static std::optional<size_t> findLocalReplicaIndexAndUpdatePools(std::vector<ConnectionPoolPtr> &pools, size_t max_replicas_to_use,
                                                                 const ClusterPtr &cluster) {
  const auto &shard = cluster->getShardsInfo().at(0);
  ...
  for (size_t i = 0, s = pools.size(); i < s; ++i) {
    ...
    const auto found = std::find_if(
        begin(shard.local_addresses), end(shard.local_addresses),
        [&hostname, &port](const Cluster::Address &local_addr) { return hostname == local_addr.host_name && port == local_addr.port; });
...
void executeQueryWithParallelReplicas(QueryPlan &query_plan, const StorageID &storage_id, SharedHeader header,
  ...
  LOG_DEBUG(logger, "Executing read from {}, header {}, query ({}), stage {} with parallel replicas", storage_id.getNameForLogs(),
  ...
  if (settings[Setting::allow_experimental_analyzer] && settings[Setting::parallel_replicas_local_plan] && !shard_num) {
    ...
    auto [local_plan, with_parallel_replicas] =
        createLocalPlanForParallelReplicas(query_ast, *header, new_context, processed_stage, coordinator,
                                           std::move(analyzed_read_from_merge_tree), local_replica_index.value());
    ...
    auto read_from_remote = std::make_unique<ReadFromParallelRemoteReplicasStep>(
        query_ast, query_tree, planner_context, cluster, storage_id, coordinator, header, processed_stage, new_context,
        getThrottler(new_context), std::move(scalars), std::move(external_tables), getLogger("ReadFromParallelRemoteReplicasStep"),
        std::move(storage_limits), std::move(connection_pools), local_replica_index, shard.pool, std::move(remote_query_plan));
    ...
    auto read_from_remote = std::make_unique<ReadFromParallelRemoteReplicasStep>(
        query_ast, query_tree, planner_context, cluster, storage_id, std::move(coordinator), header, processed_stage, new_context,
        getThrottler(new_context), std::move(scalars), std::move(external_tables), getLogger("ReadFromParallelRemoteReplicasStep"),
        std::move(storage_limits), std::move(connection_pools), std::nullopt, shard.pool);
...
void executeQueryWithParallelReplicas(QueryPlan &query_plan, const StorageID &storage_id, QueryProcessingStage::Enum processed_stage,
  ...
  executeQueryWithParallelReplicas(query_plan, storage_id, header, processed_stage, modified_query_ast, modified_query_tree,
                                   new_planner_context, context, storage_limits, std::move(analyzed_read_from_merge_tree));
}

void executeQueryWithParallelReplicas(QueryPlan &query_plan, const StorageID &storage_id, QueryProcessingStage::Enum processed_stage,
                                      const ASTPtr &query_ast, ContextPtr context,
                                      std::shared_ptr<const StorageLimitsList> storage_limits) {
  auto modified_query_ast = ClusterProxy::rewriteSelectQuery(context, query_ast, storage_id.database_name, storage_id.table_name,
                                                             /*remote_table_function_ptr*/ nullptr);
  auto header = InterpreterSelectQuery(modified_query_ast, context, SelectQueryOptions(processed_stage).analyze()).getSampleBlock();

  executeQueryWithParallelReplicas(query_plan, storage_id, header, processed_stage, modified_query_ast, nullptr, nullptr, context,
                                   storage_limits, nullptr);
}
...
void executeQueryWithParallelReplicasCustomKey(QueryPlan &query_plan, const StorageID &storage_id, const SelectQueryInfo &query_info,
  ...
  if (query_info.getCluster()->getShardsInfo().empty()) {
    if (context->getSettingsRef()[Setting::allow_experimental_analyzer]) return;
  ...
  ClusterProxy::SelectStreamFactory select_stream_factory = ClusterProxy::SelectStreamFactory(header, snapshot, processed_stage);
  ...
  ClusterProxy::executeQuery(query_plan, header, processed_stage, storage_id,
                             /*table_func_ptr=*/nullptr, select_stream_factory, getLogger("executeQueryWithParallelReplicasCustomKey"),
                             context, query_info,
                             /*sharding_key_expr=*/nullptr,
                             /*sharding_key_column_name=*/{},
                             /*distributed_settings=*/{}, shard_filter_generator,
...
void executeQueryWithParallelReplicasCustomKey(QueryPlan &query_plan, const StorageID &storage_id, SelectQueryInfo query_info,
  ...
  query_info.query = ClusterProxy::rewriteSelectQuery(context, query_info.query, storage_id.getDatabaseName(), storage_id.getTableName(),
                                                      /*table_function_ptr=*/nullptr);
  executeQueryWithParallelReplicasCustomKey(query_plan, storage_id, query_info, columns, snapshot, processed_stage, header, context);
}

bool canUseParallelReplicasOnInitiator(const ContextPtr &context) {
  if (!context->canUseParallelReplicasOnInitiator()) return false;

  auto cluster = context->getClusterForParallelReplicas();
  if (cluster->getShardCount() == 1) return cluster->getShardsInfo()[0].getAllNodeCount() > 1;
  ...
  auto is_reading_with_parallel_replicas = [](const QueryPlan::Node *node) -> bool {
    ...
    while (!stack.empty()) {
      ...
      if (frame.node->children.empty()) {
        ...
        if (typeid_cast<const ReadFromParallelRemoteReplicasStep *>(step)) return true;
      }
...
std::optional<QueryPipeline> executeInsertSelectWithParallelReplicas(const ASTInsertQuery &query_ast, const ContextPtr &context,
  ...
  if (!coordinator) coordinator = std::make_shared<ParallelReplicasReadingCoordinator>(max_replicas_to_use);

  for (size_t i = 0; i < connection_pools.size(); ++i) {
    if (local_replica_index && i == *local_replica_index) continue;
    ...
    auto remote_query_executor = std::make_shared<RemoteQueryExecutor>(
        connection_pools[i], formatted_query, std::make_shared<const Block>(Block{}), new_context, null_throttler, Scalars{}, Tables{},
