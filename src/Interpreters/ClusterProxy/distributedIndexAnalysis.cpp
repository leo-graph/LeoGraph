...
std::string buildAnalyzeIndexQuery(const StorageID &storage_id, const std::optional<std::string> &filter,
  ...
  if (vector_search_parameters) {
    query += fmt::format(", 'vector_search_index_analysis', array('{}', '{}', {}, {}, {}, {})", vector_search_parameters->column,
                         vector_search_parameters->distance_function, vector_search_parameters->limit,
                         vector_search_parameters->reference_vector, vector_search_parameters->additional_filters_present,
                         vector_search_parameters->return_distances);
  }
...
IndexAnalysisPartsRanges getIndexAnalysisFromReplicaSync(const LoggerPtr &logger, const StorageID &storage_id,
                                                         const std::optional<std::string> &filter,
                                                         const OptionalVectorSearchParameters &vector_search_parameters, ContextPtr context,
                                                         const Tables &external_tables, const std::vector<std::string_view> &parts,
  ...
  auto remote_query_executor =
      std::make_shared<RemoteQueryExecutor>(connection, query, sample_block, context, ThrottlerPtr{}, Scalars{}, external_tables);
  ...
  Block block;
  while (executor.pull(block)) parseIndexAnalysisBlock(std::move(block), res);

  return res;
}

ASTPtr getFilterAST(const ActionsDAG &filter_actions_dag, const NameSet &indexes_column_names, ContextMutablePtr &context,
  ...
  ASTPtr predicate = tryBuildAdditionalFilterAST(filter_actions_dag,
                                                 ...
                                                 /*external_tables=*/external_tables, context);
...
class DistributedIndexAnalyzer {
 public:
  DistributedIndexAnalyzer(const StorageID &storage_id_, const ActionsDAG *filter_actions_dag, ASTPtr sampling_filter,
                           const NameSet &indexes_column_names, const RangesInDataParts &parts_with_ranges_,
                           const OptionalVectorSearchParameters &vector_search_parameters_,
                           LocalIndexAnalysisCallback local_index_analysis_callback_, ContextPtr context_)
...
#endif
  {
    ...
    for (size_t i = 0; i < remote_replicas; ++i) replica_addresses[i] = dynamic_cast<ConnectionPool &>(*remote_pools[i].pool).getAddress();

    execution_context = Context::createCopy(context);
    external_tables = execution_context->getExternalTables();

    ASTPtr filter_ast;
    if (filter_actions_dag) filter_ast = getFilterAST(*filter_actions_dag, indexes_column_names, execution_context, &external_tables);
    ...
    if (filter_ast) filter_query = filter_ast->formatWithSecretsOneLine();
  }
  ...
  DistributedIndexAnalysisPartsRanges run() {
    ...
    for (const auto i : active_remote_indexes) res[i].first = replica_addresses[i];
    ...
    try {
      ...
      /// It make sense to re-throw exception from local resolving, since it should be more meaningful (it does not includes any network
      /// problems).
 ...
 private:
  ...
  std::vector<Connection *> establishConnections(const ConnectionTimeouts &timeouts) {
    if (use_hedged_requests) return establishConnectionsAsync(timeouts);
  ...
  std::vector<Connection *> establishConnectionsAsync(const ConnectionTimeouts &timeouts) {
    ...
    for (const auto &pool : remote_pools) failover_pools.push_back(pool.pool);
    ...
    hedged_factory.emplace(remote_pool, settings, timeouts, settings[Setting::connections_with_failover_max_tries],
                           ...
                           /*table_to_check=*/nullptr, replicaIndexPriorityFunc());
    ...
    for (size_t i = 0; i < remote_replicas; ++i) address_to_replica[host_port(dynamic_cast<ConnectionPool &>(*remote_pools[i].pool))] = i;
    for (auto *conn : ready_connections) {
      if (auto it = address_to_replica.find(host_port(*conn)); it != address_to_replica.end()) connections[it->second] = conn;
    }
  ...
  ThreadPool &getThreadPool() {
    if (!sync_thread_pool)
      sync_thread_pool.emplace(CurrentMetrics::DistributedIndexAnalysisThreads, CurrentMetrics::DistributedIndexAnalysisThreadsActive,
                               CurrentMetrics::DistributedIndexAnalysisThreadsScheduled, remote_replicas);
  ...
  std::vector<Connection *> establishConnectionsSync(const ConnectionTimeouts &timeouts) {
...

    {
      ...
      for (size_t i = 0; i < remote_replicas; ++i) {
        connect_runner.enqueueAndKeepTrack(
            [this, i, &timeouts]() {
              try {
                ...
                tryLogCurrentException(
                    logger,
                    fmt::format("Cannot connect to {}. It will not participate in distributed index analysis", replica_addresses[i]),
                    LogsLevel::warning);
              }
            },
            Priority{});
    ...
    for (size_t i = 0; i < remote_replicas; ++i) {
      if (!connection_entries[i].isNull()) connections[i] = &*connection_entries[i];
    }
  ...
  std::vector<size_t> selectActiveRemoteReplicas(const std::vector<Connection *> &connections) const {
    ...
    for (size_t i = 0; i < remote_replicas && active_remote_indexes.size() + 1 < max_active_replicas; ++i) {
      if (connections[i] != nullptr) active_remote_indexes.push_back(i);
    }

    LOG_DEBUG(logger, "Distributed index analysis for {} (total replicas: {}, remote active: {} remote ({}))", storage_id.getNameForLogs(),
  ...
  DistributedParts distributePartsToReplicas(const std::vector<size_t> &active_remote_indexes) const {
    ...
    for (const auto i : active_remote_indexes) active_original_indexes.push_back(remote_pools[i].index);
    ...
    for (const auto i : active_remote_indexes) original_to_remote[remote_pools[i].index] = i;
  ...
  LocalAnalysisResult executeLocalAnalysis(const std::vector<std::string_view> &local_parts, size_t local_marks, size_t local_rows,
    ...
    if (!local_parts.empty()) {
      ...
      result.thread = ThreadFromGlobalPool(
          [this, thread_group = CurrentThread::getGroup(), &local_parts, local_marks, local_rows, &local_result, exception_ptr]() {
            ThreadGroupSwitcher switcher(thread_group, ThreadName::DISTRIBUTED_INDEX_ANALYSIS);
            try {
              LOG_TRACE(logger, "Resolving {} parts ({} marks, {} rows) from local replica {}: {}", local_parts.size(), local_marks,
                        local_rows, local_address, local_parts);
              auto parts_ranges = local_index_analysis_callback(local_parts);
              LOG_TRACE(logger, "Received {} parts from local replica {}: {}", parts_ranges.size(), local_address, parts_ranges);
  ...
  void executeRemoteAnalysis(const std::vector<size_t> &active_remote_indexes, const std::vector<Connection *> &connections,
                             const std::vector<std::vector<std::string_view>> &remote_parts, const std::vector<size_t> &remote_marks,
                             const std::vector<size_t> &remote_rows, DistributedIndexAnalysisPartsRanges &res) {
...
#if defined(OS_LINUX)
  void executeRemoteAnalysisAsync(const std::vector<size_t> &active_remote_indexes, const std::vector<Connection *> &connections,
                                  const std::vector<std::vector<std::string_view>> &remote_parts, const std::vector<size_t> &remote_marks,
                                  const std::vector<size_t> &remote_rows, DistributedIndexAnalysisPartsRanges &res) {
    ...
    for (const auto i : active_remote_indexes) {
      ...
      if (!connection || remote_parts[i].empty()) continue;

      ProfileEvents::increment(ProfileEvents::DistributedIndexAnalysisScheduledReplicas);
      auto query = buildAnalyzeIndexQuery(storage_id, filter_query, vector_search_parameters, remote_parts[i]);
      auto executor = std::make_shared<RemoteQueryExecutor>(*connection, query, sample_block, execution_context, ThrottlerPtr{}, Scalars{},
                                                            external_tables);
      executor->setLogger(logger);

      LOG_TRACE(logger, "Sending {} parts ({} marks, {} rows) to {}: {}", remote_parts[i].size(), remote_marks[i], remote_rows[i],
                replica_addresses[i], remote_parts[i]);
    ...
    auto process_reader = [&](size_t reader_index) {
      ...
      try {
        ...
        if (e.code() == ErrorCodes::QUERY_WAS_CANCELLED) {
          ...
          if (!cancellation_exception) cancellation_exception = std::current_exception();
          return;
        }
        ProfileEvents::increment(ProfileEvents::DistributedIndexAnalysisReplicaFallback);
        tryLogCurrentException(logger,
                               fmt::format("Cannot analyze parts on {} replica. They will be analyzed on initiator", replica_addresses[i]),
                               LogsLevel::warning);
      } catch (...) {
        ProfileEvents::increment(ProfileEvents::DistributedIndexAnalysisReplicaFallback);
        tryLogCurrentException(logger,
    ...
    for (size_t ri = 0; ri < readers.size(); ++ri) {
      if (readers[ri].query_sent) process_reader(ri);
    }
    ...
    if (cancellation_exception) std::rethrow_exception(cancellation_exception);
  }
#else
  void executeRemoteAnalysisAsync(const std::vector<size_t> &, const std::vector<Connection *> &,
                                  const std::vector<std::vector<std::string_view>> &, const std::vector<size_t> &,
                                  const std::vector<size_t> &, DistributedIndexAnalysisPartsRanges &) {
    throw Exception(ErrorCodes::LOGICAL_ERROR, "Async remote analysis is not supported on this platform");
  }
#endif

  void executeRemoteAnalysisSync(const std::vector<size_t> &active_remote_indexes, const std::vector<Connection *> &connections,
                                 const std::vector<std::vector<std::string_view>> &remote_parts, const std::vector<size_t> &remote_marks,
                                 const std::vector<size_t> &remote_rows, DistributedIndexAnalysisPartsRanges &res) {
    ...
    for (const auto i : active_remote_indexes) {
      ...
      runner.enqueueAndKeepTrack(
          [this, i, connection, &remote_parts, &remote_marks, &remote_rows, &res]() {
            try {
              LOG_TRACE(logger, "Sending {} parts ({} marks, {} rows) to {}: {}", remote_parts[i].size(), remote_marks[i], remote_rows[i],
                        replica_addresses[i], remote_parts[i]);
              auto parts_ranges = getIndexAnalysisFromReplicaSync(logger, storage_id, filter_query, vector_search_parameters,
                                                                  execution_context, external_tables, remote_parts[i], *connection);
              ...
              if (e.code() == ErrorCodes::QUERY_WAS_CANCELLED) throw;
              ProfileEvents::increment(ProfileEvents::DistributedIndexAnalysisReplicaFallback);
              tryLogCurrentException(
                  logger, fmt::format("Cannot analyze parts on {} replica. They will be analyzed on initiator", replica_addresses[i]),
                  LogsLevel::warning);
            } catch (...) {
              ProfileEvents::increment(ProfileEvents::DistributedIndexAnalysisReplicaFallback);
              tryLogCurrentException(
  ...
  void resolveMissingParts(std::pair<std::string, IndexAnalysisPartsRanges> &local_result,
                           const DistributedIndexAnalysisPartsRanges &remote_results) const {
    std::unordered_set<std::string_view> resolved_parts;
    for (const auto &[part, _] : local_result.second) resolved_parts.insert(part);
    for (const auto &[_, parts_ranges] : remote_results) {
      for (const auto &[part, replica_ranges] : parts_ranges) resolved_parts.insert(part);
    }
    ...
    for (const auto &part_ranges : parts_with_ranges) {
      const auto &part_name = part_ranges.data_part->name;
      if (resolved_parts.contains(part_name)) continue;
    ...
    if (!missing_parts.empty()) {
      ...
      LOG_TRACE(logger, "Resolving {} missing parts ({} marks, {} rows) from local replica {}: {}", missing_parts.size(),
                missing_parts_marks, missing_parts_rows, local_address, missing_parts);
...
DistributedIndexAnalysisPartsRanges distributedIndexAnalysisOnReplicas(const StorageID &storage_id, const ActionsDAG *filter_actions_dag,
                                                                       ASTPtr sampling_filter, const NameSet &indexes_column_names,
                                                                       const RangesInDataParts &parts_with_ranges,
                                                                       const OptionalVectorSearchParameters &vector_search_parameters,
                                                                       LocalIndexAnalysisCallback local_index_analysis_callback,
                                                                       ContextPtr context) {
  ...
  DistributedIndexAnalyzer analyzer(storage_id, filter_actions_dag, std::move(sampling_filter), indexes_column_names, parts_with_ranges,
                                    vector_search_parameters, std::move(local_index_analysis_callback), std::move(context));
