...
ColumnPtr getArrayJoinColumn(const ColumnPtr &column) {
  if (typeid_cast<const ColumnArray *>(column.get())) return column;
  if (const auto *map = typeid_cast<const ColumnMap *>(column.get())) return map->getNestedColumnPtr();
  return nullptr;
}

const ColumnArray *getArrayJoinColumnRawPtr(const ColumnPtr &column) {
  if (const auto &col_arr = getArrayJoinColumn(column)) return typeid_cast<const ColumnArray *>(col_arr.get());
  return nullptr;
}
...
ArrayJoinAction::ArrayJoinAction(const Names &columns_, bool is_left_, bool is_unaligned_, size_t max_block_size_,
                                 bool enable_lazy_columns_replication_)
  ...
  if (columns.empty()) throw Exception(ErrorCodes::LOGICAL_ERROR, "No arrays to join");
...
void ArrayJoinAction::prepare(const NameSet &columns, ColumnsWithTypeAndName &sample) {
  for (auto &current : sample) {
    if (!columns.contains(current.name)) continue;
...
static void updateMaxLength(ColumnUInt64 &max_length, UInt64 length) {
  for (auto &value : max_length.getData()) value = std::max(value, length);
}
...
static void updateMaxLength(ColumnUInt64 &max_length, const IColumn &length) {
  ...
  if (!length_uint64) throw Exception(ErrorCodes::LOGICAL_ERROR, "Expected UInt64 for array length, got {}", length.getName());
  ...
  if (num_rows != length_data.size())
    throw Exception(ErrorCodes::LOGICAL_ERROR, "Different columns sizes in ARRAY JOIN: {} and {}", num_rows, length_data.size());

  for (size_t row = 0; row < num_rows; ++row) max_length_data[row] = std::max(max_length_data[row], length_data[row]);
}
...
ArrayJoinResultIterator::ArrayJoinResultIterator(const ArrayJoinAction *array_join_, Block block_, bool enable_lazy_columns_replication_)
  ...
  if (!any_array) throw Exception(ErrorCodes::TYPE_MISMATCH, "ARRAY JOIN requires array or map argument");
  ...
  if (is_unaligned) {
    ...
    for (const auto &name : columns) {
      ...
      non_empty_array_columns[name] =
          function_builder->build(tmp_block)->execute(tmp_block, array_col.type, array_col.column->size(), /* dry_run = */ false);
    }
...
bool ArrayJoinResultIterator::hasNext() const { return total_rows != 0 && current_row < total_rows; }

Block ArrayJoinResultIterator::next() {
  if (!hasNext()) throw Exception(ErrorCodes::LOGICAL_ERROR, "No more elements in ArrayJoinResultIterator.");
  ...
  for (; next_row < total_rows; ++next_row) {
    if (offsets[next_row] - offsets[current_row - 1] >= max_block_size) break;
  }
  if (next_row == current_row) ++next_row;
    ...
    if (columns.contains(current.name)) {
      if (const auto &type = getArrayJoinDataType(current.type)) {
        ...
        if (typeid_cast<const DataTypeArray *>(current.type.get())) {
          array_ptr =
              (is_left && !is_unaligned) ? non_empty_array_columns[current.name]->cut(current_row, next_row - current_row) : current.column;
      ...
      if (enable_lazy_columns_replication && isLazyReplicationUseful(current.column)) {
        if (!indexes_for_lazy_replication) indexes_for_lazy_replication = convertOffsetsToIndexes(cut_any_array->getOffsets());
