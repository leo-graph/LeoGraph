...
static std::vector<JoinActionRef> deserializeNodeList(ReadBuffer &in, const ActionsDAG::NodeRawConstPtrs &id_to_node,
...
String JoinOperator::dump() const {
  return fmt::format("JoinOperator(kind={}, strictness={}, locality={}, expression=[{}], residual_filter=[{}])", toString(kind),
                     toString(strictness), toString(locality), fmt::join(expression | std::views::transform(&JoinActionRef::dump), ", "),
