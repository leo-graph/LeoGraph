...
ColumnsDescription QueryLogElement::getColumnsDescription() {
  auto query_status_datatype =
      std::make_shared<DataTypeEnum8>(DataTypeEnum8::Values{{"QueryStart", static_cast<Int8>(QUERY_START)},
  ...
  auto query_result_cache_usage_datatype =
      std::make_shared<DataTypeEnum8>(DataTypeEnum8::Values{{"Unknown", static_cast<Int8>(QueryResultCacheUsage::Unknown)},
  ...
  return ColumnsDescription{
      ...
      {"read_rows", std::make_shared<DataTypeUInt64>(),
       "Total number of rows read from all tables and table functions participated in query. It includes usual subqueries, subqueries for "
       "IN and JOIN. For distributed queries read_rows includes the total number of rows read at all replicas. Each replica sends it's "
       "read_rows value, and the server-initiator of the query summarizes all received and local values. The cache volumes do not affect "
       "this value."},
      {"read_bytes", std::make_shared<DataTypeUInt64>(),
       "Total number of bytes read from all tables and table functions participated in query. It includes usual subqueries, subqueries for "
       "IN and JOIN. For distributed queries read_bytes includes the total number of rows read at all replicas. Each replica sends it's "
       "read_bytes value, and the server-initiator of the query summarizes all received and local values. The cache volumes do not affect "
       "this value."},
      ...
      {"normalized_query_hash", std::make_shared<DataTypeUInt64>(),
       "A numeric hash value, such as it is identical for queries differ only by values of literals."},
      ...
      {"is_initial_query", std::make_shared<DataTypeUInt8>(),
       "Query type. Possible values: 1 — query was initiated by the client, 0 — query was initiated by another query as part of "
      ...
      {"script_query_number", std::make_shared<DataTypeUInt32>(),
       "The query number in a script with multiple queries for clickhouse-client."},
      {"script_line_number", std::make_shared<DataTypeUInt32>(),
       "The line number of the query start in a script with multiple queries for clickhouse-client."},
      {"http_method", std::make_shared<DataTypeUInt8>(),
       "HTTP method that initiated the query. Possible values: 0 — The query was launched from the TCP interface, 1 — GET method was used, "
      ...
      {"Settings", std::make_shared<DataTypeMap>(low_cardinality_string, low_cardinality_string),
       "Settings that were changed when the client ran the query. To enable logging changes to settings, set the log_query_settings "
       "parameter to 1."},
      ...
      {"used_executable_user_defined_functions", array_low_cardinality_string,
       "Canonical names of executable user defined functions, which were used during query execution."},
      {"used_sql_user_defined_functions", array_low_cardinality_string,
       "Canonical names of sql user defined functions, which were used during query execution."},
      ...
      {"query_cache_usage", std::move(query_result_cache_usage_datatype),
       "Usage of the query cache during query execution. Values: 'Unknown' = Status unknown, 'None' = The query result was neither written "
       "into nor read from the query result cache, 'Write' = The query result was written into the query result cache, 'Read' = The query "
       "result was read from the query result cache."},
...
NamesAndAliases QueryLogElement::getNamesAndAliases() {
  ...
  return {{"ProfileEvents.Names", array_low_cardinality_string, "mapKeys(ProfileEvents)"},
...
void QueryLogElement::appendToBlock(MutableColumns &columns) const {
  ...
  typeid_cast<ColumnUInt16 &>(*columns[i++])
      .getData()
      .push_back(static_cast<UInt16>(DateLUT::instance().toDayNum(event_time).toUnderType()));
...

  {
    ...
    for (const UInt64 thread_id : thread_ids) column_thread_ids_data.getData().emplace_back(thread_id);
...
void QueryLogElement::appendClientInfo(const ClientInfo &client_info, MutableColumns &columns, size_t &i) {
  ...
  typeid_cast<ColumnLowCardinality &>(*columns[i++])
      .insertData(client_info.authenticated_user.data(), client_info.authenticated_user.size());
