...
InterpreterDeleteQuery::InterpreterDeleteQuery(const ASTPtr &query_ptr_, ContextPtr context_)
  ...
  if (table->supportsLightweightDelete()) {
    ...
    if (metadata_snapshot->hasProjections()) {
      if (const auto *merge_tree_data = dynamic_cast<const MergeTreeData *>(table.get()))
        if ((*merge_tree_data->getSettings())[MergeTreeSetting::lightweight_mutation_projection_mode] ==
    ...
    if (!supports_lightweight_update && lightweight_delete_mode == LIGHTWEIGHT_UPDATE_FORCE) {
      ...
      String update_query = fmt::format(update_query_template, table->getStorageID().getFullTableName(),
      ...
      ASTPtr update_ast = parseQuery(parser, update_query.data(), update_query.data() + update_query.size(), "UPDATE query", 0,
      ...
      String alter_query = fmt::format(alter_query_template, table->getStorageID().getFullTableName(),
                                       delete_query.cluster.empty() ? "" : " ON CLUSTER " + backQuoteIfNeed(delete_query.cluster),
                                       delete_query.partition ? " IN PARTITION " + delete_query.partition->formatWithSecretsOneLine() : "",
                                       delete_query.predicate->formatWithSecretsOneLine());

      ParserAlterQuery parser;
      ASTPtr alter_ast = parseQuery(parser, alter_query.data(), alter_query.data() + alter_query.size(), "ALTER query", 0,
                                    DBMS_DEFAULT_MAX_PARSER_DEPTH, DBMS_DEFAULT_MAX_PARSER_BACKTRACKS);
