...
namespace {
std::pair<Field, DataTypePtr> buildCollectionFieldAndTypeFromASTFunction(const boost::intrusive_ptr<ASTFunction> &func,
                                                                         ContextPtr context) {
  if (!func) throw Exception(ErrorCodes::LOGICAL_ERROR, "IN: empty function AST for constant set");
  ...
  if (func->name == "array") {
    ...
    for (size_t i = 0; i < rhs_array.size(); ++i) {
      if (!rhs_array[i].isNull()) rhs_array[i] = convertFieldToType(rhs_array[i], *nested_type, element_types[i].get());
    }
 ...
 */
ColumnsWithTypeAndName createBlockForSet(const DataTypePtr &left_arg_type, const ASTPtr &right_arg, ContextPtr context) {
 ...
 */
ColumnsWithTypeAndName createBlockForSet(const DataTypePtr &left_arg_type, const boost::intrusive_ptr<ASTFunction> &right_arg,
                                         ContextPtr context) {
  GetSetElementParams params{
      .transform_null_in = context->getSettingsRef()[Setting::transform_null_in],
      .forbid_unknown_enum_values = context->getSettingsRef()[Setting::validate_enum_literals_in_operators],
...
FutureSetPtr makeExplicitSet(const ASTFunction *node, const ActionsDAG &actions, ContextPtr context, PreparedSets &prepared_sets) {
  ...
  if (block.empty()) throw Exception(ErrorCodes::LOGICAL_ERROR, "Function 'in' second argument evaluated to Block with no columns");

  DataTypes set_element_types;
  set_element_types.reserve(block.size());
  for (const auto &elem : block) set_element_types.push_back(elem.type);

  auto set_element_keys = Set::getElementTypes(std::move(set_element_types), context->getSettingsRef()[Setting::transform_null_in]);

  auto set_key = right_arg->getTreeHash(/*ignore_aliases=*/true);
  if (auto set = prepared_sets.findTuple(set_key, set_element_keys)) return set;  /// Already prepared.
...
class ScopeStack::Index {
 ...
 public:
  explicit Index(ActionsDAG::NodeRawConstPtrs &index_) : index(index_) {
    for (const auto *node : index) map.emplace(node->result_name, node);
  }
  ~Index() = default;

  void addNode(const ActionsDAG::Node *node) {
    bool inserted = map.emplace(node->result_name, node).second;
    if (!inserted) throw Exception(ErrorCodes::DUPLICATE_COLUMN, "Column '{}' already exists", node->result_name);
  ...
  const ActionsDAG::Node &getNode(const std::string &name) const {
    ...
    if (!node) throw Exception(ErrorCodes::UNKNOWN_IDENTIFIER, "Unknown identifier: '{}'", name);
  ...
  std::vector<std::string_view> getAllNames() const {
    ...
    for (auto const &e : map) result.emplace_back(e.first);
    return result;
  }
};
...
ActionsMatcher::Data::Data(ContextPtr context_, SizeLimits set_size_limit_, size_t subquery_depth_,
                           std::reference_wrapper<const NamesAndTypesList> source_columns_, ActionsDAG actions_dag,
                           PreparedSetsPtr prepared_sets_, bool no_subqueries_, bool no_makeset_, bool only_consts_,
                           AggregationKeysInfo aggregation_keys_info_, bool build_expression_with_window_functions_,
...
bool ActionsMatcher::Data::hasColumn(const String &column_name) const { return actions_stack.getLastActionsIndex().contains(column_name); }
...
ScopeStack::ScopeStack(ActionsDAG actions_dag, ContextPtr context_) : WithContext(context_) {
  ...
  for (const auto &node : level.actions_dag.getOutputs())
    if (node->type == ActionsDAG::ActionType::INPUT) level.inputs.emplace(node->result_name);
}
...
size_t ScopeStack::getColumnLevel(const std::string &name) {
  for (size_t i = stack.size(); i > 0;) {
    ...
    if (stack[i].inputs.contains(name)) return i;

    const auto *node = stack[i].index->tryGetNode(name);
    if (node && node->type != ActionsDAG::ActionType::INPUT) return i;
  }
...
void ScopeStack::addArrayJoin(const std::string &source_name, std::string result_name) {
  ...
  if (!source_node)
    throw Exception(ErrorCodes::BAD_ARGUMENTS, "Expression with arrayJoin cannot depend on lambda argument: {}", source_name);
...
void ScopeStack::addFunction(const FunctionOverloadResolverPtr &function, const Names &argument_names, std::string result_name) {
  size_t level = 0;
  for (const auto &argument : argument_names) level = std::max(level, getColumnLevel(argument));

  ActionsDAG::NodeRawConstPtrs children;
  children.reserve(argument_names.size());
  for (const auto &argument : argument_names) children.push_back(&stack[level].index->getNode(argument));
...
std::string ScopeStack::dumpNames() const { return stack.back().actions_dag.dumpNames(); }

const ActionsDAG &ScopeStack::getLastActions() const { return stack.back().actions_dag; }

const ScopeStack::Index &ScopeStack::getLastActionsIndex() const { return *stack.back().index; }

bool ActionsMatcher::needChildVisit(const ASTPtr &node, const ASTPtr &child) {
  /// Visit children themself
  if (node->as<ASTIdentifier>() || node->as<ASTTableIdentifier>() || node->as<ASTFunction>() || node->as<ASTLiteral>() ||
      node->as<ASTExpressionList>())
    return false;

  /// Do not go to FROM, JOIN, UNION
  if (child->as<ASTTableExpression>() || child->as<ASTSelectQuery>()) return false;
...
void ActionsMatcher::visit(const ASTPtr &ast, Data &data) {
  ...
  if (const auto *identifier = ast->as<ASTIdentifier>())
    ...
    for (auto &child : ast->children)
      if (needChildVisit(ast, child)) visit(child, data);
  }
}
...
std::optional<NameAndTypePair> ActionsMatcher::getNameAndTypeFromAST(const ASTPtr &ast, Data &data) {
  ...
  if (const auto *node = index.tryGetNode(child_column_name)) return NameAndTypePair(child_column_name, node->result_type);

  if (!data.only_consts)
    throw Exception(ErrorCodes::UNKNOWN_IDENTIFIER, "Unknown identifier: {}; there are columns: {}", child_column_name,
                    data.actions_stack.dumpNames());
...
ASTs ActionsMatcher::doUntuple(const ASTFunction *function, ActionsMatcher::Data &data) {
  if (function->arguments->children.size() != 1)
    throw Exception(ErrorCodes::NUMBER_OF_ARGUMENTS_DOESNT_MATCH,
                    "Number of arguments for function untuple doesn't match. Passed {}, should be 1", function->arguments->children.size());
  ...
  if (!tuple_type)
    throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT, "Function untuple expect tuple argument, got {}", result_type->getName());
  ...
  for (const auto &element_name : tuple_type->getElementNames()) {
    ...
    if (tid != 0) tuple_ast = tuple_ast->clone();
...
void ActionsMatcher::visit(const ASTIdentifier &identifier, const ASTPtr &, Data &data) {
  ...
  if (data.hasColumn(column_name)) return;
  ...
  if (!data.only_consts) {
    ...
    for (const auto &column_name_type : data.source_columns) {
      if (column_name_type.name == column_name) {
        throw Exception(ErrorCodes::NOT_AN_AGGREGATE, "Column {} is not under aggregate function and not in GROUP BY. Have columns: {}",
    ...
    if (identifier.preferAliasToColumnName() && !identifier.alias.empty()) data.addAlias(identifier.name(), identifier.alias);
  }
}
...
void checkFunctionHasEmptyNullsAction(const ASTFunction &node) {
  if (node.getNullsAction() != NullsAction::EMPTY)
    throw Exception(ErrorCodes::SYNTAX_ERROR, "Function {} cannot use {} NULLS", node.name,
...
void ActionsMatcher::visit(const ASTFunction &node, const ASTPtr &ast, Data &data) {
  ...
  if (node.name == "lambda") throw Exception(ErrorCodes::UNEXPECTED_EXPRESSION, "Unexpected lambda expression");

  /// Function arrayJoin.
  if (node.name == "arrayJoin") {
    if (node.arguments->children.size() != 1) throw Exception(ErrorCodes::TYPE_MISMATCH, "arrayJoin requires exactly 1 argument");
    checkFunctionHasEmptyNullsAction(node);

    ASTPtr arg = node.arguments->children.at(0);
    visit(arg, data);
    if (!data.only_consts) data.addArrayJoin(arg->getColumnName(), column_name);
  ...
  if (node.name == "grouping") {
    checkFunctionHasEmptyNullsAction(node);
    if (data.only_consts) return;  // Can not perform constant folding, because this function can be executed only after GROUP BY

    size_t arguments_size = node.arguments->children.size();
    if (arguments_size == 0) throw Exception(ErrorCodes::TOO_FEW_ARGUMENTS_FOR_FUNCTION, "Function GROUPING expects at least one argument");
    ...
    for (auto const &arg : node.arguments->children) {
      ...
      if (pos == aggregation_keys_number)
        throw Exception(ErrorCodes::BAD_ARGUMENTS, "Argument of GROUPING function {} is not a part of GROUP BY clause",
                        arg->getColumnName());
    ...
    switch (keys_info.group_by_kind) {
      case GroupByKind::GROUPING_SETS: {
        data.addFunction(std::make_shared<FunctionToOverloadResolverAdaptor>(std::make_shared<FunctionGroupingForGroupingSets>(
                             std::move(arguments_indexes), keys_info.grouping_set_keys,
                             data.getContext()->getSettingsRef()[Setting::force_grouping_standard_compatibility])),
                         {"__grouping_set"}, column_name);
        break;
      }
      case GroupByKind::ROLLUP:
        data.addFunction(std::make_shared<FunctionToOverloadResolverAdaptor>(std::make_shared<FunctionGroupingForRollup>(
                             std::move(arguments_indexes), aggregation_keys_number,
                             data.getContext()->getSettingsRef()[Setting::force_grouping_standard_compatibility])),
                         {"__grouping_set"}, column_name);
        break;
      case GroupByKind::CUBE: {
        data.addFunction(std::make_shared<FunctionToOverloadResolverAdaptor>(std::make_shared<FunctionGroupingForCube>(
                             std::move(arguments_indexes), aggregation_keys_number,
                             data.getContext()->getSettingsRef()[Setting::force_grouping_standard_compatibility])),
                         {"__grouping_set"}, column_name);
        break;
      ...
      case GroupByKind::ORDINARY: {
        data.addFunction(
            std::make_shared<FunctionToOverloadResolverAdaptor>(std::make_shared<FunctionGroupingOrdinary>(
                std::move(arguments_indexes), data.getContext()->getSettingsRef()[Setting::force_grouping_standard_compatibility])),
            {}, column_name);
        break;
      }
      default:
        throw Exception(ErrorCodes::LOGICAL_ERROR, "Unexpected kind of GROUP BY clause for GROUPING function: {}", keys_info.group_by_kind);
    }
    return;
  }
  ...
  if (checkFunctionIsInOrGlobalInOperator(node)) {
    ...
    if (prepared_set) {
      ...
      if (!data.only_consts) {
        ...
        data.addFunction(FunctionFactory::instance().get(node.name + "IgnoreSet", data.getContext()), {argument_name, argument_name},
                         column_name);
      }
      return;
    }
  ...
  if (node.name == "indexHint") {
    ...
    if (data.only_consts) {
      ...
      if (node.arguments) {
        for (const auto &arg : node.arguments->children) visit(arg, data);
      }
      return;
    }
    ...
    Data index_hint_data(data.getContext(), data.set_size_limit, data.subquery_depth, data.source_columns, ActionsDAG(data.source_columns),
                         data.prepared_sets, data.no_subqueries, data.no_makeset, data.only_consts, data.aggregation_keys_info);
  ...
  if (AggregateUtils::isAggregateFunction(node)) return;

  FunctionOverloadResolverPtr function_builder;

  auto current_context = data.getContext();

  if (UserDefinedExecutableFunctionFactory::instance().has(node.name,
                                                           current_context))  /// NOLINT(readability-static-accessed-through-instance)
  {
    Array parameters;
    if (node.parameters) {
    ...
    function_builder = UserDefinedExecutableFunctionFactory::instance().tryGet(
        node.name, current_context, parameters);  /// NOLINT(readability-static-accessed-through-instance)
  }
  ...
  if (!function_builder) {
    try {
      ...
      if (!hints.empty()) e.addMessage("Or unknown aggregate function " + node.name + ". Maybe you meant: " + toString(hints));
      throw;
    }

    /// Normal functions are not parametric for now.
    if (node.parameters) throw Exception(ErrorCodes::FUNCTION_CANNOT_HAVE_PARAMETERS, "Function {} is not parametric", node.name);
  }
  ...
  if (node.arguments) {
    ...
    for (size_t arg = 0; arg < num_arguments; ++arg) {
      ...
      if (function && function->name == "lambda") {
        ...
        ColumnWithTypeAndName column(ColumnConst::create(std::move(column_string), 1), std::make_shared<DataTypeString>(),
    ...
    if (data.only_consts && !arguments_present) return;
      ...
      for (size_t i = 0; i < node.arguments->children.size(); ++i) {
        ...
        if (lambda && lambda->name == "lambda") {
          ...
          for (size_t j = 0; j < lambda_arg_asts.size(); ++j) {
            ...
            if (!opt_arg_name) throw Exception(ErrorCodes::TYPE_MISMATCH, "lambda argument declarations must be identifiers");
          ...
          for (const auto &required_arg : required)
            if (findColumn(required_arg, lambda_arguments) == lambda_arguments.end()) captured.push_back(required_arg);
          ...
          auto function_capture = std::make_shared<FunctionCaptureOverloadResolver>(std::move(lambda_dag), actions_settings, captured,
                                                                                    lambda_arguments, result_type, result_name, false);
...
void ActionsMatcher::visit(const ASTLiteral &literal, const ASTPtr & /* ast */, Data &data) {
  ...
  if (literal.unique_column_name.empty()) {
     ...
     */
    if (existing_column && existing_column->column && isColumnConst(*existing_column->column) && existing_column->column->size() == 1 &&
        existing_column->column->operator[](0) == value) {
      const_cast<ASTLiteral &>(literal).unique_column_name = default_name;
    } else {
      const_cast<ASTLiteral &>(literal).unique_column_name = data.getUniqueName(default_name);
    }
  }
...
FutureSetPtr ActionsMatcher::makeSet(const ASTFunction &node, Data &data, bool no_subqueries) {
  ...
  if (right_in_operand->as<ASTSubquery>() || identifier) {
    ...
    if (data.getContext()->getSettingsRef()[Setting::allow_experimental_analyzer] && !identifier) {
      ...
      if (auto *query_node = query_tree->as<QueryNode>()) query_node->setIsSubquery(true);
      set_key = query_tree->getTreeHash({.ignore_cte = true});
    } else
      set_key = right_in_operand->getTreeHash(/*ignore_aliases=*/true);

    if (auto set = data.prepared_sets->findSubquery(set_key)) return set;
    ...
    if (identifier) {
      ...
      if (table) {
        if (auto set = data.prepared_sets->findStorage(set_key)) return set;
      ...
      if (!data.getContext()->isGlobalContext()) {
        ...
        if (auto tmp_table = data.getContext()->findExternalTable(identifier->getColumnName())) external_table_set = tmp_table->future_set;
      }
    }
    ...
    return data.prepared_sets->addFromSubquery(set_key, right_in_operand, std::move(source), nullptr, std::move(external_table_set),
