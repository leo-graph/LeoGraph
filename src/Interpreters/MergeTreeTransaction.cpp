...
static void checkNotOrdinaryDatabase(const StoragePtr &storage) {
  if (storage->getStorageID().uuid != UUIDHelpers::Nil) return;

  throw Exception(ErrorCodes::NOT_IMPLEMENTED,
                  "Table {} belongs to database with Ordinary engine. "
                  "This engine is deprecated and is not supported in transactions.",
...
MergeTreeTransaction::MergeTreeTransaction(CSN snapshot_, LocalTID local_tid_, UUID host_id, std::list<CSN>::iterator snapshot_it_)
    : tid({snapshot_, local_tid_, host_id}), snapshot(snapshot_), snapshot_in_use_it(snapshot_it_), csn(Tx::UnknownCSN) {}

void MergeTreeTransaction::setSnapshot(CSN new_snapshot) { snapshot.store(new_snapshot, std::memory_order_relaxed); }

MergeTreeTransaction::State MergeTreeTransaction::getState() const {
  CSN c = csn.load();
  if (c == Tx::UnknownCSN) return RUNNING;
  if (c == Tx::CommittingCSN) return COMMITTING;
  if (c == Tx::RolledBackCSN) return ROLLED_BACK;
  return COMMITTED;
}
...
void MergeTreeTransaction::checkIsNotCancelled() const {
  CSN c = csn.load();
  if (c == Tx::RolledBackCSN) throw Exception(ErrorCodes::INVALID_TRANSACTION, "Transaction was cancelled");
  if (c != Tx::UnknownCSN) throw Exception(ErrorCodes::LOGICAL_ERROR, "Unexpected CSN state: {}", c);
}
...
void MergeTreeTransaction::addNewPart(const StoragePtr &storage, const DataPartPtr &new_part, MergeTreeTransaction *txn) {
  ...
  if (txn) {
    ...
    tryWriteEventToSystemLog(new_part->version.log, TransactionsInfoLogElement::ADD_PART, txn->tid,
...
void MergeTreeTransaction::removeOldPart(const StoragePtr &storage, const DataPartPtr &part_to_remove, MergeTreeTransaction *txn) {
  ...
  if (txn) {
    ...
    if (part_to_remove->wasInvolvedInTransaction()) part_to_remove->appendRemovalTIDToVersionMetadata();
  }
}

void MergeTreeTransaction::addNewPartAndRemoveCovered(const StoragePtr &storage, const DataPartPtr &new_part,
                                                      const DataPartsVector &covered_parts, MergeTreeTransaction *txn) {
  ...
  if (txn) {
    ...
    for (const auto &covered : covered_parts) {
      ...
      if (covered->wasInvolvedInTransaction()) covered->appendRemovalTIDToVersionMetadata();
    }
  }
}
...
bool MergeTreeTransaction::isReadOnly() const {
  std::lock_guard lock{mutex};
  if (finalized) return is_read_only;
...
scope_guard MergeTreeTransaction::beforeCommit() {
  ...
  if (!can_commit) {
    ...
    if (expected == Tx::RolledBackCSN) throw Exception(ErrorCodes::INVALID_TRANSACTION, "Transaction was cancelled");
...
void MergeTreeTransaction::afterCommit(CSN assigned_csn) noexcept {
  ...
  for (const auto &part : removed_parts) {
    ...
    if (!part->version.creation_csn.load(std::memory_order_relaxed)) {
      ...
      if (creation) part->version.creation_csn.store(creation, std::memory_order_relaxed);
    }
  ...
  for (const auto &storage_and_mutation : committed_mutations) storage_and_mutation.first->setMutationCSN(storage_and_mutation.second, csn);
}
...
bool MergeTreeTransaction::rollback() noexcept {
  ...
  for (const auto &table_and_mutation : mutations_to_kill) table_and_mutation.first->killMutation(table_and_mutation.second);
  ...
  for (const auto &part : parts_to_activate)
    if (part->version.getCreationTID() != tid) const_cast<MergeTreeData &>(part->storage).restoreAndActivatePart(part);
...
void MergeTreeTransaction::onException() { TransactionLog::instance().rollbackTransaction(shared_from_this()); }
...
String MergeTreeTransaction::dumpDescription() const {
  ...
  for (const auto &part : creating_parts) std::get<0>(storage_to_changes[&(part->storage)]).push_back(part->name);
  ...
  for (const auto &mutation : mutations) std::get<2>(storage_to_changes[mutation.first.get()]).push_back(mutation.second);
  ...
  for (const auto &storage_changes : storage_to_changes) {
    ...
    if (!creating_info.empty()) res += fmt::format("\n\t\tcreating parts:\n\t\t\t{}", fmt::join(creating_info, "\n\t\t\t"));
