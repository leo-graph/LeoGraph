...
DistributedQueryStatusSource::DistributedQueryStatusSource(const String& zookeeper_name_, const String& zk_node_path,
                                                           const String& zk_replicas_path, SharedHeader block, ContextPtr context_,
                                                           const Strings& hosts_to_wait, const char* logger_name)
    : ISource(block),
...
NameSet DistributedQueryStatusSource::getOfflineHosts(const NameSet& hosts_to_wait, const ZooKeeperPtr& zookeeper) {
  ...
  for (size_t i = 0; i < res.size(); ++i) {
    if (res[i].error == Coordination::Error::ZNONODE) offline.insert(hosts_array[i]);
  ...
  if (offline.size() == hosts_to_wait.size()) {
    ...
    LOG_WARNING(log, "Did not find active hosts, will wait for all hosts: {}. This should not happen often",
                fmt::join(hosts_to_wait, ", "));
    return {};
  }
...
ExecutionStatus DistributedQueryStatusSource::getExecutionStatus(const fs::path& status_path) {
  ...
  retries_ctl.retryLoop(
      [&]() { finished_exists = context->getDefaultOrAuxiliaryZooKeeper(zookeeper_name)->tryGet(status_path, status_data); });
  if (finished_exists) status.tryDeserializeText(status_data);
...
ZooKeeperRetriesInfo DistributedQueryStatusSource::getRetriesInfo() const {
  ...
  return ZooKeeperRetriesInfo(config_ref.getInt("distributed_ddl_keeper_max_retries", 5),
                              config_ref.getInt("distributed_ddl_keeper_initial_backoff_ms", 100),
                              config_ref.getInt("distributed_ddl_keeper_max_backoff_ms", 5000), context->getProcessListElement());
}
...
Chunk DistributedQueryStatusSource::generate() {
  ...
  if (all_hosts_finished || timeout_exceeded) return {};
  ...
  while (true) {
    ...
    auto component_guard = Coordination::setCurrentComponent("DistributedQueryStatusSource::generate");
    {
      ...
      retries_ctl.retryLoop([&]() {
        ...
        if (only_running_hosts) offline_hosts = getOfflineHosts(waiting_hosts, zookeeper);
      });
    }
    ...
    if (!node_exists) {
      ...
      if (!first_exception)
        first_exception = std::make_unique<Exception>(
            Exception(ErrorCodes::UNFINISHED,
    ...
    if (only_running_hosts) {
      ...
      for (const auto& host : waiting_hosts) num_finished_or_offline += finished_hosts.contains(host) || offline_hosts.contains(host);

      if (num_finished_or_offline == waiting_hosts.size()) stop_waiting_offline_hosts = true;
    }

    if (new_hosts.empty()) continue;
