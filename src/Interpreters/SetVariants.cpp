...
typename SetVariantsTemplate<Variant>::Type SetVariantsTemplate<Variant>::chooseMethod(const ColumnRawPtrs &key_columns, Sizes &key_sizes) {
  ...
  if (has_nullable_key) {
    ...
    if ((keys_size == 1) && (nested_key_columns[0]->isNumeric())) {
      ...
      if ((size_of_field == 1) || (size_of_field == 2) || (size_of_field == 4) || (size_of_field == 8)) return Type::nullable_keys128;
    ...
    if (all_fixed) {
      ...
      if ((std::tuple_size_v<KeysNullMap<UInt128>> + keys_bytes) <= 16) return Type::nullable_keys128;
      if ((std::tuple_size_v<KeysNullMap<UInt256>> + keys_bytes) <= 32) return Type::nullable_keys256;
    }
  ...
  if (keys_size == 1 && (typeid_cast<const ColumnString *>(nested_key_columns[0]) ||
                         (isColumnConst(*nested_key_columns[0]) &&
                          typeid_cast<const ColumnString *>(&assert_cast<const ColumnConst *>(nested_key_columns[0])->getDataColumn()))))
    return Type::key_string;

  if (keys_size == 1 && typeid_cast<const ColumnFixedString *>(nested_key_columns[0])) return Type::key_fixed_string;
