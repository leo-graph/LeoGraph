...
class QueryStatus : public WithContext {
 protected:
  ...
  enum class QueryStreamsStatus : uint8_t { NotInitialized, Initialized, Released };
 ...
 public:
  QueryStatus(ContextPtr context_, const String &query_, UInt64 normalized_query_hash_, const ClientInfo &client_info_,
              QueryPriorities::Handle &&priority_handle_, QuerySlotPtr &&query_slot_, ThreadGroupPtr &&thread_group_,
              IAST::QueryKind query_kind_, const Settings &query_settings_, UInt64 watch_start_nanoseconds, bool is_internal);
  ...
  MemoryTracker *getMemoryTracker() const {
    if (!thread_group) return nullptr;
    return &thread_group->memory_tracker;
  }

  bool hasThreadGroup() const { return bool(thread_group); }
  ...
  bool updateProgressIn(const Progress &value) {
    ...
    if (priority_handle) priority_handle->waitIfNeed();
...
struct ProcessListForUser {
  ...
  /// query_id -> ProcessListElement(s). There can be multiple queries with the same query_id as long as all queries except one are
  /// cancelled.
...
class ProcessList {
 ...
 public:
   ...
   */
  EntryPtr insert(const String &query_, UInt64 normalized_query_hash, const IAST *ast, ContextMutablePtr query_context,
                  UInt64 watch_start_nanoseconds, bool is_internal);
  ...
  QueryStatusInfoPtr getQueryInfo(const String &query_id, bool get_thread_list = false, bool get_profile_events = false,
                                  bool get_settings = false) const;
  ...
  void setMaxWaitingQueriesAmount(UInt64 max_waiting_queries_amount_) {
    ...
    // NOTE: We cannot cancel waiting queries when limit is lowered. They have to wait anyways, but new queries will be canceled instead of
    // waiting.
  }

  size_t getMaxWaitingQueriesAmount() const { return max_waiting_queries_amount.load(); }
