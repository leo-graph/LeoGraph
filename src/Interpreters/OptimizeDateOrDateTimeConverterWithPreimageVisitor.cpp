...
ASTPtr generateOptimizedDateFilterAST(const String& comparator, const NameAndTypePair& column, const FieldInterval& range) {
  ...
  if (comparator == "equals") {
    return makeASTOperator(
        "and",
        makeASTOperator("greaterOrEquals", make_intrusive<ASTIdentifier>(column_name), make_intrusive<ASTLiteral>(start_date_or_date_time)),
        makeASTOperator("less", make_intrusive<ASTIdentifier>(column_name), make_intrusive<ASTLiteral>(end_date_or_date_time)));
  }
  if (comparator == "notEquals") {
    return makeASTOperator(
        "or", makeASTOperator("less", make_intrusive<ASTIdentifier>(column_name), make_intrusive<ASTLiteral>(start_date_or_date_time)),
        makeASTOperator("greaterOrEquals", make_intrusive<ASTIdentifier>(column_name), make_intrusive<ASTLiteral>(end_date_or_date_time)));
  }
  if (comparator == "greater") {
    return makeASTOperator("greaterOrEquals", make_intrusive<ASTIdentifier>(column_name),
                           make_intrusive<ASTLiteral>(end_date_or_date_time));
  }
  if (comparator == "lessOrEquals") {
    return makeASTOperator("less", make_intrusive<ASTIdentifier>(column_name), make_intrusive<ASTLiteral>(end_date_or_date_time));
  }
  if (comparator == "less" || comparator == "greaterOrEquals") {
    return makeASTOperator(comparator, make_intrusive<ASTIdentifier>(column_name), make_intrusive<ASTLiteral>(start_date_or_date_time));
  }
  [[unlikely]] {
    throw Exception(ErrorCodes::LOGICAL_ERROR, "Expected equals, notEquals, less, lessOrEquals, greater, greaterOrEquals. Actual {}",
                    comparator);
  }
}
...
void OptimizeDateOrDateTimeConverterWithPreimageMatcher::visit(const ASTFunction& function, ASTPtr& ast, const Data& data) {
  const static std::unordered_map<String, String> swap_relations = {
      {"equals", "equals"}, {"notEquals", "notEquals"},          {"less", "greater"},
      {"greater", "less"},  {"lessOrEquals", "greaterOrEquals"}, {"greaterOrEquals", "lessOrEquals"},
  };
  ...
  for (size_t i = 0; i < function.arguments->children.size(); i++)
    if (const auto* /*func*/ _ = function.arguments->children[i]->as<ASTFunction>()) func_id = i;
  ...
  if (!pos) pos = IdentifierSemantic::chooseTableColumnMatch(*column_id, data.tables, true);
  if (!pos) return;

  if (*pos >= data.tables.size()) return;
