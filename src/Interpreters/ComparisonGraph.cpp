...
const FunctionNode *tryGetFunctionNode(const QueryTreeNodePtr &node) { return node->as<FunctionNode>(); }

const ASTFunction *tryGetFunctionNode(const ASTPtr &node) { return node->as<ASTFunction>(); }

std::string functionName(const QueryTreeNodePtr &node) { return node->as<FunctionNode &>().getFunctionName(); }

std::string functionName(const ASTPtr &node) { return node->as<ASTFunction &>().name; }

std::optional<Field> tryGetConstantValue(const QueryTreeNodePtr &node) {
  if (const auto *constant = node->as<ConstantNode>()) return constant->getValue();

  return {};
}

std::optional<Field> tryGetConstantValue(const ASTPtr &node) {
  if (const auto *constant = node->as<ASTLiteral>()) return constant->value;
...
const auto &getNode(const Analyzer::CNFAtomicFormula &atom) { return atom.node_with_hash.node; }

const auto &getNode(const CNFQueryAtomicFormula &atom) { return atom.ast; }

std::string nodeToString(const ASTPtr &ast) { return ast->formatWithSecretsOneLine(); }

std::string nodeToString(const QueryTreeNodePtr &node) { return node->toAST()->formatWithSecretsOneLine(); }

const auto &getArguments(const ASTFunction *function) { return function->arguments->children; }

const auto &getArguments(const FunctionNode *function) { return function->getArguments().getNodes(); }
...
bool greater(const Field &lhs, const Field &rhs) { return less(rhs, lhs); }
...
ComparisonGraphCompareResult inverseCompareResult(ComparisonGraphCompareResult result) {
  ...
  static const std::unordered_map<ComparisonGraphCompareResult, ComparisonGraphCompareResult> inverse_relations = {
      {NOT_EQUAL, EQUAL},       {EQUAL, NOT_EQUAL},       {GREATER_OR_EQUAL, LESS}, {GREATER, LESS_OR_EQUAL},
      {LESS, GREATER_OR_EQUAL}, {LESS_OR_EQUAL, GREATER}, {UNKNOWN, UNKNOWN},
  };
...
ComparisonGraph<Node>::ComparisonGraph(const NodeContainer &atomic_formulas, ContextPtr context) {
  if (atomic_formulas.empty()) return;
  ...
  for (const auto &atom_raw : atomic_formulas) {
    ...
    auto get_index = [](const Node &node, Graph &nodes_graph) -> std::optional<size_t> {
      ...
      if (it != std::end(nodes_graph.node_hash_to_component)) {
        if (!std::any_of(std::cbegin(nodes_graph.vertices[it->second].nodes), std::cend(nodes_graph.vertices[it->second].nodes),
      ...
      if (arguments.size() == 2) {
        ...
        if (index_left && index_right) {
          if (const auto it = relation_to_enum.find(functionName(atom)); it != std::end(relation_to_enum)) {
            ...
            if (it->second == Graph::Edge::EQUAL) g.edges[*index_right].push_back({it->second, *index_left});
          }
        }
      }
    }
      ...
      if (arguments.size() == 2) {
        ...
        if (index_left == index_right) {
          throw Exception(ErrorCodes::VIOLATED_CONSTRAINT,
                          "Found expression '{}', but its arguments considered equal according to constraints", nodeToString(atom));
        }
...
ComparisonGraphCompareResult ComparisonGraph<Node>::pathToCompareResult(Path path, bool inverse) {
  switch (path) {
    case Path::GREATER:
      return inverse ? ComparisonGraphCompareResult::LESS : ComparisonGraphCompareResult::GREATER;
    case Path::GREATER_OR_EQUAL:
      return inverse ? ComparisonGraphCompareResult::LESS_OR_EQUAL : ComparisonGraphCompareResult::GREATER_OR_EQUAL;
  }
}
...
ComparisonGraphCompareResult ComparisonGraph<Node>::compare(const Node &left, const Node &right) const {
    ...
    auto result = ComparisonGraphCompareResult::UNKNOWN;
    {
      ...
      if (left_bound && right_bound) {
        if (greater(left_bound->first, right_bound->first))
          ...
          result = left_bound->second || right_bound->second ? ComparisonGraphCompareResult::GREATER
                                                             : ComparisonGraphCompareResult::GREATER_OR_EQUAL;
      }
    }
    {
      ...
      if (left_bound && right_bound) {
        if (less(left_bound->first, right_bound->first))
          ...
          result =
              left_bound->second || right_bound->second ? ComparisonGraphCompareResult::LESS : ComparisonGraphCompareResult::LESS_OR_EQUAL;
      }
    }
  ...
  if (start == finish) return ComparisonGraphCompareResult::EQUAL;

  if (auto path = findPath(start, finish)) return pathToCompareResult(*path, /*inverse=*/false);

  if (auto path = findPath(finish, start)) return pathToCompareResult(*path, /*inverse=*/true);

  if (not_equal.contains({start, finish})) return ComparisonGraphCompareResult::NOT_EQUAL;
...
bool ComparisonGraph<Node>::isPossibleCompare(ComparisonGraphCompareResult expected, const Node &left, const Node &right) const {
  ...
  using enum ComparisonGraphCompareResult;
  if (expected == UNKNOWN || result == UNKNOWN) return true;
...
bool ComparisonGraph<Node>::isAlwaysCompare(ComparisonGraphCompareResult expected, const Node &left, const Node &right) const {
  ...
  using enum ComparisonGraphCompareResult;
  if (expected == UNKNOWN || result == UNKNOWN) return false;

  if (expected == result) return true;

  static const std::set<std::pair<ComparisonGraphCompareResult, ComparisonGraphCompareResult>> possible_pairs = {
      {LESS_OR_EQUAL, LESS},     {LESS_OR_EQUAL, EQUAL}, {GREATER_OR_EQUAL, GREATER},
      {GREATER_OR_EQUAL, EQUAL}, {NOT_EQUAL, GREATER},   {NOT_EQUAL, LESS},
  };
...
std::optional<size_t> ComparisonGraph<Node>::getComponentId(const Node &node) const {
  ...
  if (hash_it == std::end(graph.node_hash_to_component)) return {};

  const size_t index = hash_it->second;
  if (std::any_of(std::cbegin(graph.vertices[index].nodes), std::cend(graph.vertices[index].nodes), [node](const Node &constraint_node) {
        if constexpr (with_ast)
          return constraint_node->getTreeHash(/*ignore_aliases=*/true) == node->getTreeHash(/*ignore_aliases=*/true) &&
                 constraint_node->getColumnName() == node->getColumnName();
        else
...
ComparisonGraphCompareResult ComparisonGraph<Node>::atomToCompareResult(const Formula &atom) {
  ...
  if (tryGetFunctionNode(node) != nullptr) {
    ...
    if (atom.negative) expected = inverseCompareResult(expected);
    return expected;
  }
...
std::optional<Node> ComparisonGraph<Node>::getEqualConst(const Node &node) const {
  const auto hash_it = graph.node_hash_to_component.find(Graph::getHash(node));
  if (hash_it == std::end(graph.node_hash_to_component)) return std::nullopt;

  const size_t index = hash_it->second;

  if (!graph.vertices[index].hasConstant()) return std::nullopt;
...
std::optional<std::pair<Field, bool>> ComparisonGraph<Node>::getConstUpperBound(const Node &node) const {
  if (const auto constant = tryGetConstantValue(node)) return std::make_pair(*constant, false);

  const auto it = graph.node_hash_to_component.find(Graph::getHash(node));
  if (it == std::end(graph.node_hash_to_component)) return std::nullopt;

  const size_t to = it->second;
  const ssize_t from = node_const_upper_bound[to];
  if (from == -1) return std::nullopt;
...
void ComparisonGraph<Node>::dfsOrder(const Graph &nodes_graph, size_t v, std::vector<bool> &visited, std::vector<size_t> &order) {
  ...
  for (const auto &edge : nodes_graph.edges[v])
    if (!visited[edge.to]) dfsOrder(nodes_graph, edge.to, visited, order);
...
typename ComparisonGraph<Node>::Graph ComparisonGraph<Node>::reverseGraph(const Graph &nodes_graph) {
  ...
  for (size_t v = 0; v < nodes_graph.vertices.size(); ++v)
    for (const auto &edge : nodes_graph.edges[v]) g.edges[edge.to].push_back({edge.type, v});
...
std::vector<typename ComparisonGraph<Node>::NodeContainer> ComparisonGraph<Node>::getVertices() const {
  ...
  for (const auto &vertex : graph.vertices) {
    result.emplace_back();
    for (const auto &node : vertex.nodes) result.back().push_back(node);
  }
  return result;
}

template <ComparisonGraphNodeType Node>
void ComparisonGraph<Node>::dfsComponents(const Graph &reversed_graph, size_t v, OptionalIndices &components, size_t component) {
  components[v] = component;
  for (const auto &edge : reversed_graph.edges[v])
    if (!components[edge.to]) dfsComponents(reversed_graph, edge.to, components, component);
}
...
typename ComparisonGraph<Node>::Graph ComparisonGraph<Node>::buildGraphFromNodesGraph(const Graph &nodes_graph) {
  ...
  std::vector<size_t> order;
  {
    ...
    for (size_t v = 0; v < n; ++v) {
      if (!visited[v]) dfsOrder(nodes_graph, v, visited, order);
    }
  }
  ...
  for (const auto &[hash, index] : nodes_graph.node_hash_to_component) {
    ...
    result.vertices[*components[index]].nodes.insert(
        std::end(result.vertices[*components[index]].nodes), std::begin(nodes_graph.vertices[index].nodes),
        std::end(nodes_graph.vertices[index].nodes));  // asts_graph has only one ast per vertex
  }

  /// Calculate constants
  for (auto &vertex : result.vertices) vertex.buildConstants();

  /// For each edge in initial graph, we add an edge between components in condensation graph.
  for (size_t v = 0; v < n; ++v) {
    for (const auto &edge : nodes_graph.edges[v]) result.edges[*components[v]].push_back({edge.type, *components[edge.to]});
  ...
  for (size_t v = 0; v < result.vertices.size(); ++v) {
    for (size_t u = 0; u < result.vertices.size(); ++u) {
      if (v != u && result.vertices[v].hasConstant() && result.vertices[u].hasConstant()) {
        ...
        if (greater(left, right)) result.edges[v].push_back({Graph::Edge::GREATER, u});
      }
    }
  }
  ...
  for (size_t k = 0; k < n; ++k)
    for (size_t v = 0; v < n; ++v)
      for (size_t u = 0; u < n; ++u)
        if (results[v][k] != inf && results[k][u] != inf) results[v][u] = std::min({results[v][u], results[v][k], results[k][u]});

  std::map<std::pair<size_t, size_t>, Path> path;
  for (size_t v = 0; v < n; ++v)
    for (size_t u = 0; u < n; ++u)
      if (results[v][u] != inf) path[std::make_pair(v, u)] = (results[v][u] == -1 ? Path::GREATER : Path::GREATER_OR_EQUAL);
...
std::pair<std::vector<ssize_t>, std::vector<ssize_t>> ComparisonGraph<Node>::buildConstBounds() const {
  ...
  auto get_value = [this](const size_t vertex) -> Field { return getConstantValue(graph.vertices[vertex].getConstant()); };

  for (const auto &[edge, path] : dists) {
    const auto [from, to] = edge;

    if (graph.vertices[to].hasConstant()) {
      if (lower[from] == -1 || greater(get_value(to), get_value(lower[from])) ||
          (equals(get_value(to), get_value(lower[from])) && path == Path::GREATER))
        lower[from] = to;
    }

    if (graph.vertices[from].hasConstant()) {
      if (upper[to] == -1 || less(get_value(from), get_value(upper[to])) ||
