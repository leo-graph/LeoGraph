...
void EmbeddedDictionaries::handleException(const bool throw_on_error) const {
  ...
  if (throw_on_error) std::rethrow_exception(exception_ptr);
}

template <typename Dictionary>
bool EmbeddedDictionaries::reloadDictionary(MultiVersion<Dictionary>& dictionary, DictionaryReloader<Dictionary> reload_dictionary,
                                            const bool throw_on_error, const bool force_reload) {
  ...
  if (force_reload || !is_fast_start_stage || not_initialized) {
    try {
      ...
      if (new_dictionary) dictionary.set(std::move(new_dictionary));
    } catch (...) {
...
bool EmbeddedDictionaries::reloadImpl(const bool throw_on_error, const bool force_reload) {
  ...
  if (!reloadDictionary<RegionsNames>(regions_names, std::move(reload_regions_names), throw_on_error, force_reload)) was_exception = true;

  if (!was_exception) LOG_INFO(log, "Loaded dictionaries.");
...
void EmbeddedDictionaries::reloadPeriodically() {
  ...
  while (true) {
    if (destroy.tryWait(cur_reload_period * 1000)) return;
...
EmbeddedDictionaries::EmbeddedDictionaries(std::unique_ptr<GeoDictionariesLoader> geo_dictionaries_loader_, ContextPtr context_,
...
void EmbeddedDictionaries::reload() {
  if (!reloadImpl(true, true)) throw Exception(ErrorCodes::UNFINISHED, "Some embedded dictionaries were not successfully reloaded");
}
