...
class TraceLog : public SystemLog<TraceLogElement> {
 ...
 public:
  TraceLog(ContextPtr context_, const SystemLogSettings& settings_, std::shared_ptr<SystemLogQueue<TraceLogElement>> queue_ = nullptr)
      : SystemLog<TraceLogElement>(context_, settings_, queue_), symbolize(settings_.symbolize_traces) {}
