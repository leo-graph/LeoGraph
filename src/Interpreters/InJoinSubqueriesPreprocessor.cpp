...
struct NonGlobalTableData : public WithContext {
  ...
  NonGlobalTableData(ContextPtr context_, const CheckShardsAndTables &checker_, std::vector<ASTPtr> &renamed_tables_,
                     ASTFunction *function_, ASTTableJoin *table_join_)
  ...
  void visit(ASTTableExpression &node, ASTPtr &) {
    ...
    if (database_and_table) renameIfNeeded(database_and_table);
  }
 ...
 private:
  void renameIfNeeded(ASTPtr &database_and_table) {
    ...
    if (!storage || !checker.hasAtLeastTwoShards(*storage)) return;
    ...
    } else if (getContext()->getSettingsRef()[Setting::prefer_global_in_and_join] ||
...
class NonGlobalSubqueryMatcher {
 public:
  ...
  static void visit(ASTPtr &node, Data &data) {
    if (auto *function = node->as<ASTFunction>()) visit(*function, node, data);
    if (const auto *tables = node->as<ASTTablesInSelectQueryElement>()) visit(*tables, node, data);
  }

  static bool needChildVisit(ASTPtr &node, const ASTPtr &child) {
    if (auto *function = node->as<ASTFunction>())
      if (function->name == "in" || function->name == "notIn") return false;  /// Processed, process others

    if (const auto *t = node->as<ASTTablesInSelectQueryElement>())
      if (t->table_join && t->table_expression) return false;  /// Processed, process others
  ...
  static void visit(ASTFunction &node, ASTPtr &, Data &data) {
    if (node.name == "in" || node.name == "notIn") {
      if (node.arguments->children.size() != 2) {
        throw Exception(ErrorCodes::BAD_ARGUMENTS, "Function '{}' expects two arguments, given: '{}'", node.name,
      ...
      if (!renamed.empty()) data.renamed_tables.emplace_back(subquery, std::move(renamed));
    }
  }

  static void visit(const ASTTablesInSelectQueryElement &node, ASTPtr &, Data &data) {
    if (!node.table_join || !node.table_expression) return;
    ...
    if (table_join->locality != JoinLocality::Global) {
      if (auto *table = node.table_expression->as<ASTTableExpression>()) {
        if (auto &subquery = table->subquery) {
          ...
          if (!renamed.empty()) data.renamed_tables.emplace_back(tb, std::move(renamed));
        }
      }
    }
  }
...
void InJoinSubqueriesPreprocessor::visit(ASTPtr &ast) const {
  ...
  if (!query || !query->tables()) return;

  if (getContext()->getSettingsRef()[Setting::distributed_product_mode] == DistributedProductMode::ALLOW) return;
  ...
  if (!table_expression->database_and_table_name) return;
...
std::pair<std::string, std::string> InJoinSubqueriesPreprocessor::CheckShardsAndTables::getRemoteDatabaseAndTableName(
    const IStorage &table) const {
  const StorageDistributed &distributed = dynamic_cast<const StorageDistributed &>(table);
