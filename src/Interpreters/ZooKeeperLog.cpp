...
ColumnsDescription ZooKeeperLogElement::getColumnsDescription() {
  ...
  return ColumnsDescription{
      ...
      {"type", std::move(type_enum),
       "Event type in the ZooKeeper client. Can have one of the following values: Request — The request has been sent, Response — The "
       "response was received, Finalize — The connection is lost, no response was received."},
      ...
      {"xid", std::make_shared<DataTypeInt64>(),
       "The ID of the request within the session. This is usually a sequential request number. It is the same for the request row and the "
       "paired response/finalize row."},
      ...
      {"path", std::make_shared<DataTypeString>(),
       "The path to the ZooKeeper node specified in the request, or an empty string if the request not requires specifying a path."},

      {"data", std::make_shared<DataTypeString>(),
       "The data written to the ZooKeeper node (for the SET and CREATE requests — what the request wanted to write, for the response to "
       "the GET request — what was read) or an empty string."},
      ...
      {"version", std::make_shared<DataTypeNullable>(std::make_shared<DataTypeInt32>()),
       "The version of the ZooKeeper node that the request expects when executing. This is supported for CHECK, SET, REMOVE requests (is "
       "relevant -1 if the request does not check the version or NULL for other requests that do not support version checking)."},

      {"requests_size", std::make_shared<DataTypeUInt32>(),
       "The number of requests included in the multi request (this is a special request that consists of several consecutive ordinary "
       "requests and executes them atomically). All requests included in multi request will have the same xid."},
      {"request_idx", std::make_shared<DataTypeUInt32>(),
       "The number of the request included in multi request (for multi request — 0, then in order from 1)."},

      {"zxid", std::make_shared<DataTypeInt64>(),
       "ZooKeeper transaction ID. The serial number issued by the ZooKeeper server in response to a successfully executed request (0 if "
       "the request was not executed/returned an error/the client does not know whether the request was executed)."},
      {"error", std::make_shared<DataTypeNullable>(Coordination::SystemTablesDataTypes::errorCodeEnum()),
       "Error code. Can have many values, here are just some of them: ZOK — The request was executed successfully, ZCONNECTIONLOSS — The "
       "connection was lost, ZOPERATIONTIMEOUT — The request execution timeout has expired, ZSESSIONEXPIRED — The session has expired, "
       "NULL — The request is completed."},

      {"watch_type", std::make_shared<DataTypeNullable>(Coordination::SystemTablesDataTypes::watchEventTypeEnum()),
       "The type of the watch event (for responses with op_num = Watch), for the remaining responses: NULL."},
      {"watch_state", std::make_shared<DataTypeNullable>(Coordination::SystemTablesDataTypes::watchStateEnum()),
       "The status of the watch event (for responses with op_num = Watch), for the remaining responses: NULL."},

      {"path_created", std::make_shared<DataTypeString>(),
       "The path to the created ZooKeeper node (for responses to the CREATE request), may differ from the path if the node is created as a "
       "sequential."},
      ...
      {"stat_pzxid", std::make_shared<DataTypeInt64>(),
       "The transaction ID of the change that last modified children of this ZooKeeper node."},
...
void ZooKeeperLogElement::appendToBlock(MutableColumns& columns) const {
  ...
  for (const auto& c : children) children_array.emplace_back(c);
