...
BackupLogElement::BackupLogElement(BackupOperationInfo info_)
    : event_time(std::chrono::system_clock::now()), event_time_usec(timeInMicroseconds(event_time)), info(std::move(info_)) {}
...
ColumnsDescription BackupLogElement::getColumnsDescription() {
  return ColumnsDescription{
      ...
      {"num_entries", std::make_shared<DataTypeUInt64>(),
       "Number of entries in the backup, i.e. the number of files inside the folder if the backup is stored as a folder, or the number of "
       "files inside the archive if the backup is stored as an archive. It is not the same as num_files if it's an incremental backup or "
       "if it contains empty files or duplicates. The following is always true: num_entries <= num_files."},
