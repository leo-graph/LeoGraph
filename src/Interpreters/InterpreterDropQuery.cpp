...
BlockIO InterpreterDropQuery::executeSingleDropQuery(const ASTPtr &drop_query_ptr) {
  ...
  if (getContext()->getSettingsRef()[Setting::database_atomic_wait_for_drop_and_detach_synchronously]) drop.sync = true;

  if (drop.table) return executeToTable(drop);
  if (drop.database && !drop.cluster.empty() && !maybeRemoveOnCluster(current_query_ptr, getContext())) {
    DDLQueryOnClusterParams params;
    params.access_to_check = getRequiredAccessForDDLOnCluster();
    return executeDDLQueryOnCluster(current_query_ptr, getContext(), params);
  }
  if (drop.database) return executeToDatabase(drop);
  throw Exception(ErrorCodes::LOGICAL_ERROR, "Nothing to drop, both names are empty");
}

void InterpreterDropQuery::waitForTableToBeActuallyDroppedOrDetached(const ASTDropQuery &query, const DatabasePtr &db,
                                                                     const UUID &uuid_to_wait, ContextPtr context_) {
  if (uuid_to_wait == UUIDHelpers::Nil) return;
  ...
  if (query.kind == ASTDropQuery::Kind::Drop) {
    ...
    DatabaseCatalog::instance().waitTableFinallyDropped(uuid_to_wait, [&]() {
      if (query_status) query_status->throwIfKilled();
    });
...
BlockIO InterpreterDropQuery::executeToTable(ASTDropQuery &query) {
  ...
  if (query.sync) waitForTableToBeActuallyDroppedOrDetached(query, database, table_to_wait_on, getContext());
  return res;
}
...
BlockIO InterpreterDropQuery::executeToTableImpl(const ContextPtr &context_, ASTDropQuery &query, DatabasePtr &db, UUID &uuid_to_wait) {
  ...
  if (query.isTemporary()) {
    if (query.if_exists) return {};
    throw Exception(ErrorCodes::UNKNOWN_TABLE, "Temporary table {} doesn't exist", backQuoteIfNeed(table_id.table_name));
  }

  auto ddl_guard =
      (!query.no_ddl_lock ? DatabaseCatalog::instance().getDDLGuard(table_id.database_name, table_id.table_name, nullptr) : nullptr);
  ...
  if (database && table) {
    ...
    if (ast_drop_query.is_view && !table->isView())
      throw Exception(ErrorCodes::INCORRECT_QUERY, "Table {} is not a View", table_id.getNameForLogs());

    if (ast_drop_query.is_dictionary && !table->isDictionary())
      throw Exception(ErrorCodes::INCORRECT_QUERY, "Table {} is not a Dictionary", table_id.getNameForLogs());
    ...
    if (!secondary_query && settings[Setting::ignore_drop_queries_probability] != 0 && ast_drop_query.kind == ASTDropQuery::Kind::Drop &&
      ...
      LOG_TEST(getLogger("InterpreterDropQuery"), "Replace DROP TABLE query to TRUNCATE TABLE for table {}.{}", table_id.database_name,
               table_id.table_name);
    ...
    if (query.kind == ASTDropQuery::Kind::Detach) {
      ...
      if (table->isDictionary()) {
        ...
        if (!query.is_dictionary) table->checkTableCanBeDetached();
      } else
      ...
      if (table->isDictionary()) throw Exception(ErrorCodes::SYNTAX_ERROR, "Cannot TRUNCATE dictionary");

      context_->checkAccess(AccessType::TRUNCATE, table_id);
      if (table->isStaticStorage()) throw Exception(ErrorCodes::TABLE_IS_READ_ONLY, "Table is read-only");
      ...
      if (table->isDictionary()) {
        /// If DROP DICTIONARY query is not used, check if Dictionary can be dropped with DROP TABLE query
        if (!query.is_dictionary) table->checkTableCanBeDropped(context_);
      } else
        table->checkTableCanBeDropped(context_);
      ...
      if (database->getUUID() == UUIDHelpers::Nil) context_->clearMMappedFileCache();
    }

    db = database;
    /// Truncate does not enqueue the table for dropping.
    if (query.kind != ASTDropQuery::Kind::Truncate) uuid_to_wait = table_id.uuid;
  }
...
BlockIO InterpreterDropQuery::executeToDatabase(const ASTDropQuery &query) {
  ...
  try {
    ...
    if (query.sync) {
      for (const auto &table_uuid : tables_to_wait) waitForTableToBeActuallyDroppedOrDetached(query, database, table_uuid, getContext());
    }
    throw;
  }
...
bool matchesLikePattern(const String &haystack, const String &like_pattern, bool case_insensitive) {
  ...
  if (case_insensitive) options.set_case_sensitive(false);

  /// Builds the RE2 regex
  RE2 re(regex_str, options);
  if (!re.ok()) throw Exception(ErrorCodes::SYNTAX_ERROR, "Invalid regex: {}", regex_str);
...
BlockIO InterpreterDropQuery::executeToDatabaseImpl(const ASTDropQuery &query, DatabasePtr &database, std::vector<UUID> &uuids_to_wait) {
  ...
  if (query.if_empty) throw Exception(ErrorCodes::NOT_IMPLEMENTED, "DROP IF EMPTY is not implemented for databases");

  if (!truncate && database->hasReplicationThread()) database->stopReplication();
  ...
  if (database->shouldBeEmptyOnDetach()) {
    ...
    if (truncate) query_for_table.kind = query.has_tables ? ASTDropQuery::Kind::Truncate : ASTDropQuery::Kind::Drop;
    if (database->getDisk()->isReadOnly()) query_for_table.kind = ASTDropQuery::Detach;
    ...
    if (!truncate || !query.has_tables || query.like.empty()) {
      ...
      auto collect_tables = [&] {
        ...
        for (auto iterator = database->getTablesIterator(table_context); iterator->isValid(); iterator->next()) {
          ...
          ///  in such DBs.)
          if (!storage_id.hasUUID() || !prepared_tables.contains(storage_id.uuid)) tables_to_prepare.push_back(table_ptr);
        }
      };
      ...
      auto prepare_tables = [&](std::vector<StoragePtr> &tables) {
        ...
        for (StoragePtr &table_ptr : tables) {
          StorageID storage_id = table_ptr->getStorageID();
          if (storage_id.hasUUID()) prepared_tables.insert(storage_id.uuid);
          runner.enqueueAndKeepTrack([my_table_ptr = std::move(table_ptr)]() { my_table_ptr->flushAndPrepareForShutdown(); });
        }
      ...
      for (const StoragePtr &table_ptr : tables_to_prepare) {
        if (const auto *materialized_view = typeid_cast<const StorageMaterializedView *>(table_ptr.get())) {
          if (materialized_view->canCreateOrDropOtherTables()) tables_to_prepare_early.push_back(table_ptr);
        }
      }
      ...
      /// dependencies intact and can be loaded on restart.
      {
        ...
        for (const auto &[id, _] : tables_to_drop) table_names_in_drop.insert(id.getFullTableName());
        ...
        for (const auto &[id, _] : tables_to_drop) {
          ...
          for (const auto &dep : deps)
            if (table_names_in_drop.contains(dep.getFullTableName())) relevant_deps.push_back(dep);
        ...
        for (size_t i = 0; i < sorted.size(); ++i) position[sorted[i].getFullTableName()] = i;
        ...
        std::sort(tables_to_drop.begin(), tables_to_drop.end(), [&](const auto &a, const auto &b) {
          ...
          if (a_is_inner != b_is_inner) return a_is_inner;

          size_t pos_a = 0;
          size_t pos_b = 0;
          if (auto it = position.find(a.first.getFullTableName()); it != position.end()) pos_a = it->second;
          if (auto it = position.find(b.first.getFullTableName()); it != position.end()) pos_b = it->second;
          return pos_a > pos_b;
        });
      }
  ...
  if (truncate && query.has_tables && !query.like.empty()) {
    ...
    ThreadPoolCallbackRunnerLocal<void> runner(getDatabaseCatalogDropTablesThreadPool().get(), ThreadName::TRUNCATE_TABLE);
  ...
  if (!drop && !truncate && query.sync) {
    ...
    for (const auto &table_uuid : uuids_to_wait) database->waitDetachedTableNotInUse(table_uuid);
  }
  ...
  // only if operation is DETACH
  if (!drop && !truncate) database->assertCanBeDetached(true);

  /// DETACH or DROP database itself. If TRUNCATE skip dropping/erasing the database.
  if (!truncate) DatabaseCatalog::instance().detachDatabase(getContext(), database_name, drop, database->shouldBeEmptyOnDetach());
...
void InterpreterDropQuery::extendQueryLogElemImpl(DB::QueryLogElement &elem, const DB::ASTPtr &ast, DB::ContextPtr context_) const {
  ...
  if (drop.database_and_tables) {
    ...
    for (auto &child : list.children) {
      ...
      if (!identifier) throw Exception(ErrorCodes::SYNTAX_ERROR, "Unexpected type for list of table names.");
      ...
      if (!query_database.empty() && query_table.empty()) {
        ...
        auto quoted_database = query_database.empty() ? backQuoteIfNeed(context_->getCurrentDatabase()) : backQuoteIfNeed(query_database);
...
void InterpreterDropQuery::executeDropQuery(ASTDropQuery::Kind kind, ContextPtr global_context, ContextPtr current_context,
  ...
  auto ddl_guard =
      (need_ddl_guard ? DatabaseCatalog::instance().getDDLGuard(target_table_id.database_name, target_table_id.table_name, nullptr)
                      : nullptr);
  if (DatabaseCatalog::instance().tryGetTable(target_table_id, current_context)) {
    ...
    if (ignore_sync_setting) drop_context->setSetting("database_atomic_wait_for_drop_and_detach_synchronously", false);
...
bool InterpreterDropQuery::supportsTransactions() const {
  ...
  return drop.cluster.empty() && !drop.isTemporary() && drop.kind == ASTDropQuery::Kind::Truncate && drop.table;
}
