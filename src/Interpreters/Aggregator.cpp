...
namespace {
bool worthConvertToTwoLevel(size_t group_by_two_level_threshold, size_t result_size, size_t group_by_two_level_threshold_bytes,
                            auto result_size_bytes) {
  // params.group_by_two_level_threshold will be equal to 0 if we have only one thread to execute aggregation (refer to
...
void initDataVariantsWithSizeHint(DB::AggregatedDataVariants &result, DB::AggregatedDataVariants::Type method_chosen,
  ...
  if (auto hint = getSizeHint(stats_collecting_params, /*tables_cnt=*/max_threads)) {
    if (worthConvertToTwoLevel(params.group_by_two_level_threshold, hint->sum_of_sizes,
...
void updateStatistics(const DB::ManyAggregatedDataVariants &data_variants, const DB::StatsCollectingParams &params) {
  if (!params.isCollectionAndUseEnabled()) return;

  std::vector<size_t> sizes(data_variants.size());
  for (size_t i = 0; i < data_variants.size(); ++i) sizes[i] = data_variants[i]->size();
...
DB::ColumnNumbers calculateKeysPositions(const DB::Block &header, const DB::Aggregator::Params &params) {
  ...
  for (size_t i = 0; i < params.keys_size; ++i) keys_positions[i] = header.getPositionByName(params.keys[i]);
...
size_t getMinBytesForPrefetch() {
  ...
  if (auto ret = sysconf(_SC_LEVEL2_CACHE_SIZE); ret != -1) l2_size = ret;
#endif
...
UInt64 &getInlineCountState(DB::AggregateDataPtr &ptr) { return getCountState(reinterpret_cast<DB::AggregateDataPtr>(&ptr)); }
...
size_t Aggregator::estimateSizeOfCompressedState(AggregatedDataVariants &result, ssize_t bucket) const {
  auto estimate_size_of_compressed_state = [&](auto &table) {
    ...
    for (size_t j = 0; j < params.aggregates_size; ++j) {
      ...
      size_t it = 0;
      table.forEachMapped([&](AggregateDataPtr place) {
        chassert(place);
        if (it++ % period == 0) {
  ...
  if (result.type == AggregatedDataVariants::Type::EMPTY) {
    ...
    if (!result.without_key) throw Exception(ErrorCodes::LOGICAL_ERROR, "Empty without_key data passed to Aggregator::applyToAllStates");
...
Block Aggregator::getHeader(bool final) const { return params.getHeader(header, final); }

Aggregator::Params::Params(const Names &keys_, const AggregateDescriptions &aggregates_, bool overflow_row_, size_t max_rows_to_group_by_,
                           OverflowMode group_by_overflow_mode_, size_t group_by_two_level_threshold_,
                           size_t group_by_two_level_threshold_bytes_, size_t max_bytes_before_external_group_by_,
                           bool empty_result_for_aggregation_by_empty_set_, TemporaryDataOnDiskScopePtr tmp_data_scope_,
                           size_t max_threads_, size_t min_free_disk_space_, bool compile_aggregate_expressions_,
                           size_t min_count_to_compile_aggregate_expression_, size_t max_block_size_, bool enable_prefetch_,
                           bool only_merge_,  // true for projections
                           bool optimize_group_by_constant_keys_, float min_hit_rate_to_use_consecutive_keys_optimization_,
                           const StatsCollectingParams &stats_collecting_params_,
                           bool enable_producing_buckets_out_of_order_in_aggregation_, bool serialize_string_with_zero_byte_)
    : keys(keys_),
      keys_size(keys.size()),
...
size_t Aggregator::Params::getMaxBytesBeforeExternalGroupBy(size_t max_bytes_before_external_group_by,
                                                            double max_bytes_ratio_before_external_group_by) {
  std::optional<size_t> threshold;
  if (max_bytes_before_external_group_by != 0) threshold = max_bytes_before_external_group_by;
  ...
  if (max_bytes_ratio_before_external_group_by != 0.) {
    ...
    if (available_system_memory.has_value()) {
      ...
      LOG_TRACE(getLogger("Aggregator"),
                "Adjusting memory limit before external aggregation with {} (ratio: {}, available system memory: {})",
                formatReadableSizeWithBinarySuffix(ratio_in_bytes), ratio, formatReadableSizeWithBinarySuffix(*available_system_memory));
    } else {
...
Aggregator::Params::Params(const Names &keys_, const AggregateDescriptions &aggregates_, bool overflow_row_, size_t max_threads_,
                           size_t max_block_size_, float min_hit_rate_to_use_consecutive_keys_optimization_,
...
Block Aggregator::Params::getHeader(const Block &header, bool only_merge, const Names &keys, const AggregateDescriptions &aggregates,
                                    bool final) {
  Block res;

  if (only_merge) {
    NameSet needed_columns(keys.begin(), keys.end());
    for (const auto &aggregate : aggregates) needed_columns.emplace(aggregate.column_name);

    for (const auto &column : header) {
      if (needed_columns.contains(column.name)) res.insert(column.cloneEmpty());
    }
    ...
    for (const auto &key : keys) res.insert(header.getByName(key).cloneEmpty());

    for (const auto &aggregate : aggregates) {
      size_t arguments_size = aggregate.argument_names.size();
      DataTypes argument_types(arguments_size);
      for (size_t j = 0; j < arguments_size; ++j) argument_types[j] = header.getByName(aggregate.argument_names[j]).type;
...
ColumnRawPtrs Aggregator::Params::makeRawKeyColumns(const Block &block) const {
  ...
  for (size_t i = 0; i < keys_size; ++i) {
    ...
    if (column.name != keys[i]) {
      throw Exception(ErrorCodes::LOGICAL_ERROR, "Wrong key in block [{}] at position {}, expected keys: [{}]", block.dumpStructure(), i,
                      fmt::join(keys, ", "));
    }
#endif
...
void Aggregator::Params::explain(WriteBuffer &out, size_t indent) const {
  ...
  if (!aggregates.empty()) {
    ...
    for (const auto &aggregate : aggregates) aggregate.explain(out, indent + 4);
  }
}
...
void Aggregator::Params::explain(JSONBuilder::JSONMap &map) const {
  ...
  for (const auto &key : keys) keys_array->add(key);
...
class CompiledAggregateFunctionsHolder final : public CompiledExpressionCacheEntry {
 public:
  ...
  ~CompiledAggregateFunctionsHolder() override { getJITInstance().deleteCompiledModule(compiled_aggregate_functions.compiled_module); }
...
Aggregator::Aggregator(const Block &header_, const Params &params_)
    : header(header_),
      ...
      params(params_),
      tmp_data(params.tmp_data_scope
                   ? params.tmp_data_scope->childScope({.current_metric = CurrentMetrics::TemporaryFilesForAggregation,
      ...
      thread_pool{CurrentMetrics::AggregatorThreads, CurrentMetrics::AggregatorThreadsActive, CurrentMetrics::AggregatorThreadsScheduled,
                  params.max_threads} {
  memory_usage_before_aggregation = getCurrentQueryMemoryUsage();

  aggregate_functions.resize(params.aggregates_size);
  for (size_t i = 0; i < params.aggregates_size; ++i) aggregate_functions[i] = params.aggregates[i].function.get();
  ...
  if (params.aggregates_size == 1) {
    ...
    if (typeid_cast<const AggregateFunctionCount *>(params.aggregates[0].function.get())) is_simple_count = true;
  }
...
void Aggregator::compileAggregateFunctionsIfNeeded() {
  ...
  if (!params.compile_aggregate_expressions) return;

  if (is_simple_count) return;
  ...
  if (functions_to_compile.empty()) return;
...
AggregatedDataVariants::Type Aggregator::chooseAggregationMethod() {
  ...
  if (params.keys_size == 0) return AggregatedDataVariants::Type::without_key;
  ...
  if (has_nullable_key) {
    ...
    if (params.keys_size == 1 && !has_low_cardinality) {
      if (types_removed_nullable[0]->isValueRepresentedByNumber()) {
        ...
        if (size_of_field == 1) return AggregatedDataVariants::Type::nullable_key8;
        if (size_of_field == 2) return AggregatedDataVariants::Type::nullable_key16;
        if (size_of_field == 4) return AggregatedDataVariants::Type::nullable_key32;
        if (size_of_field == 8) return AggregatedDataVariants::Type::nullable_key64;
      }
    ...
    if (params.keys_size == num_fixed_contiguous_keys && !has_low_cardinality) {
      ...
      if (std::tuple_size_v<KeysNullMap<UInt128>> + keys_bytes <= 16) return AggregatedDataVariants::Type::nullable_keys128;
      if (std::tuple_size_v<KeysNullMap<UInt256>> + keys_bytes <= 32) return AggregatedDataVariants::Type::nullable_keys256;
    }
    ...
    if (has_low_cardinality && params.keys_size == 1) {
      if (types_removed_nullable[0]->isValueRepresentedByNumber()) {
        ...
        if (size_of_field == 1) return AggregatedDataVariants::Type::low_cardinality_key8;
        if (size_of_field == 2) return AggregatedDataVariants::Type::low_cardinality_key16;
        if (size_of_field == 4) return AggregatedDataVariants::Type::low_cardinality_key32;
        if (size_of_field == 8) return AggregatedDataVariants::Type::low_cardinality_key64;
    ...
    if (params.keys_size > 1 && all_keys_are_numbers_or_strings) return AggregatedDataVariants::Type::nullable_prealloc_serialized;
  ...
  if (params.keys_size == 1 && types_removed_nullable[0]->isValueRepresentedByNumber()) {
    ...
    if (has_low_cardinality) {
      ...
      if (size_of_field == 4) return AggregatedDataVariants::Type::low_cardinality_key32;
      if (size_of_field == 8) return AggregatedDataVariants::Type::low_cardinality_key64;
      if (size_of_field == 16) return AggregatedDataVariants::Type::low_cardinality_keys128;
      if (size_of_field == 32) return AggregatedDataVariants::Type::low_cardinality_keys256;
      throw Exception(ErrorCodes::LOGICAL_ERROR, "LowCardinality numeric column has sizeOfField not in 1, 2, 4, 8, 16, 32.");
    }

    if (size_of_field == 1) return AggregatedDataVariants::Type::key8;
    if (size_of_field == 2) return AggregatedDataVariants::Type::key16;
    if (size_of_field == 4) return AggregatedDataVariants::Type::key32;
    if (size_of_field == 8) return AggregatedDataVariants::Type::key64;
    if (size_of_field == 16) return AggregatedDataVariants::Type::keys128;
    if (size_of_field == 32) return AggregatedDataVariants::Type::keys256;
  ...
  if (params.keys_size == 1 && isFixedString(types_removed_nullable[0])) {
    if (has_low_cardinality) return AggregatedDataVariants::Type::low_cardinality_key_fixed_string;
  ...
  if (params.keys_size == num_fixed_contiguous_keys) {
    if (has_low_cardinality) {
      if (keys_bytes <= 16) return AggregatedDataVariants::Type::low_cardinality_keys128;
      if (keys_bytes <= 32) return AggregatedDataVariants::Type::low_cardinality_keys256;
    }

    if (keys_bytes <= 2) return AggregatedDataVariants::Type::keys16;
    if (keys_bytes <= 4) return AggregatedDataVariants::Type::keys32;
    if (keys_bytes <= 8) return AggregatedDataVariants::Type::keys64;
    if (keys_bytes <= 16) return AggregatedDataVariants::Type::keys128;
    if (keys_bytes <= 32) return AggregatedDataVariants::Type::keys256;
  }

  /// If single string key - will use hash table with references to it. Strings itself are stored separately in Arena.
  if (params.keys_size == 1 && isString(types_removed_nullable[0])) {
    if (has_low_cardinality) return AggregatedDataVariants::Type::low_cardinality_key_string;
    return AggregatedDataVariants::Type::key_string;
  }

  if (params.keys_size > 1 && all_keys_are_numbers_or_strings) return AggregatedDataVariants::Type::prealloc_serialized;
...
void Aggregator::createAggregateStates(AggregateDataPtr &aggregate_data) const {
  for (size_t j = 0; j < params.aggregates_size; ++j) {
    if constexpr (skip_compiled_aggregate_functions)
      if (is_aggregate_function_compiled[j]) continue;
...
bool Aggregator::hasSparseArguments(AggregateFunctionInstruction *aggregate_instructions) {
  for (auto *inst = aggregate_instructions; inst->that; ++inst)
    if (inst->has_sparse_arguments) return true;
  return false;
}

void Aggregator::executeOnBlockSmall(AggregatedDataVariants &result, size_t row_begin, size_t row_end, ColumnRawPtrs &key_columns,
...
void Aggregator::mergeOnBlockSmall(AggregatedDataVariants &result, size_t row_begin, size_t row_end,
                                   const AggregateColumnsConstData &aggregate_columns_data, const ColumnRawPtrs &key_columns) const {
  /// `result` will destroy the states of aggregate functions in the destructor
  result.aggregator = this;
...
#define M(NAME, IS_TWO_LEVEL)                                                                                           \
      ...
      /* no_more_keys= */ false, row_begin, row_end, aggregate_columns_data, key_columns, is_cancelled, result.aggregates_pool);
...
void Aggregator::executeImpl(AggregatedDataVariants &result, size_t row_begin, size_t row_end, ColumnRawPtrs &key_columns,
                             AggregateFunctionInstruction *aggregate_instructions, bool no_more_keys, bool all_keys_are_const,
...
void NO_INLINE Aggregator::executeImpl(Method &method, Arena *aggregates_pool, size_t row_begin, size_t row_end, ColumnRawPtrs &key_columns,
                                       AggregateFunctionInstruction *aggregate_instructions,
                                       LastElementCacheStats &consecutive_keys_cache_stats, bool no_more_keys, bool all_keys_are_const,
                                       AggregateDataPtr overflow_row) const {
...
void NO_INLINE Aggregator::executeImpl(Method &method, State &state, Arena *aggregates_pool, size_t row_begin, size_t row_end,
                                       AggregateFunctionInstruction *aggregate_instructions, bool no_more_keys, bool all_keys_are_const,
                                       AggregateDataPtr overflow_row) const {
  if (!no_more_keys) {
    /// Prefetching doesn't make sense for small hash tables, because they fit in caches entirely.
    const bool prefetch =
        Method::State::has_cheap_key_calculation && params.enable_prefetch && (method.data.getBufferSizeInBytes() > min_bytes_for_prefetch);

#if USE_EMBEDDED_COMPILER
    if (compiled_aggregate_functions_holder && !hasSparseArguments(aggregate_instructions)) {
      if (prefetch)
        executeImplBatch<true>(method, state, aggregates_pool, row_begin, row_end, aggregate_instructions, false, all_keys_are_const, true,
                               overflow_row);
      else
        executeImplBatch<false>(method, state, aggregates_pool, row_begin, row_end, aggregate_instructions, false, all_keys_are_const, true,
                                overflow_row);
    } else
#endif
    {
      if (prefetch)
        executeImplBatch<true>(method, state, aggregates_pool, row_begin, row_end, aggregate_instructions, false, all_keys_are_const, false,
                               overflow_row);
      else
        executeImplBatch<false>(method, state, aggregates_pool, row_begin, row_end, aggregate_instructions, false, all_keys_are_const,
                                false, overflow_row);
    }
  } else {
    executeImplBatch<false>(method, state, aggregates_pool, row_begin, row_end, aggregate_instructions, true, all_keys_are_const, false,
                            overflow_row);
  }
}

template <bool prefetch, typename Method, typename State>
void NO_INLINE Aggregator::executeImplBatch(Method &method, State &state, Arena *aggregates_pool, size_t row_begin, size_t row_end,
                                            AggregateFunctionInstruction *aggregate_instructions, bool no_more_keys,
                                            bool all_keys_are_const, bool use_compiled_functions [[maybe_unused]],
                                            AggregateDataPtr overflow_row) const {
    ...
    if (all_keys_are_const) {
      ...
      for (size_t i = row_begin; i < row_end; ++i) {
        if constexpr (prefetch && HasPrefetchMemberFunc<decltype(method.data), KeyHolder>) {
          if (i == row_begin + PrefetchingHelper::iterationsToMeasure()) prefetch_look_ahead = prefetching.calcPrefetchLookAhead();
    ...
    if constexpr (std::is_same_v<Method, typename decltype(AggregatedDataVariants::key8)::element_type>) {
      if (!all_keys_are_const) {
        if (is_simple_count) {
          ...
          for (size_t i = row_begin; i < row_end; ++i) ++map[key[i]];
          return;
        }
        ...
        if (!has_arrays && !hasSparseArguments(aggregate_instructions)) {
          for (AggregateFunctionInstruction *inst = aggregate_instructions; inst->that; ++inst) {
            inst->batch_that->addBatchLookupTable8(
                row_begin, row_end, reinterpret_cast<AggregateDataPtr *>(method.data.data()), inst->state_offset,
                [&](AggregateDataPtr &aggregate_data) {
                  AggregateDataPtr place = aggregates_pool->alignedAlloc(total_size_of_aggregate_states, align_aggregate_states);
                  createAggregateStates(place);
                  aggregate_data = place;
                },
                state.getKeyData(), inst->batch_arguments, aggregates_pool);
          }
          return;
        }
      }
  ...
  if (!no_more_keys) {
    for (size_t i = key_start; i < key_end; ++i) {
      ...
      if constexpr (prefetch && HasPrefetchMemberFunc<decltype(method.data), KeyHolder>) {
        if (i == key_start + PrefetchingHelper::iterationsToMeasure()) prefetch_look_ahead = prefetching.calcPrefetchLookAhead();
  ...
  executeAggregateInstructions(aggregates_pool, row_begin, row_end, aggregate_instructions, places.get(), key_start,
                               state.hasOnlyOneValueSinceLastReset(), all_keys_are_const, use_compiled_functions);
}

void Aggregator::executeAggregateInstructions(Arena *aggregates_pool, size_t row_begin, size_t row_end,
                                              AggregateFunctionInstruction *aggregate_instructions, AggregateDataPtr *places,
                                              size_t key_start, bool has_only_one_value_since_last_reset, bool all_keys_are_const,
  ...
  if (use_compiled_functions) {
    ...
    for (size_t i = 0; i < aggregate_functions.size(); ++i) {
      if (!is_aggregate_function_compiled[i]) continue;
    ...
    if (all_keys_are_const || (can_optimize_equal_keys_ranges && has_only_one_value_since_last_reset)) {
      ...
      auto add_into_aggregate_states_function_single_place =
          compiled_aggregate_functions_holder->compiled_aggregate_functions.add_into_aggregate_states_function_single_place;
  ...
  for (size_t i = 0; i < aggregate_functions.size(); ++i) {
#if USE_EMBEDDED_COMPILER
    if (use_compiled_functions && is_aggregate_function_compiled[i]) continue;
#endif
...
void NO_INLINE Aggregator::executeWithoutKeyImpl(AggregatedDataWithoutKey &res, size_t row_begin, size_t row_end,
                                                 AggregateFunctionInstruction *aggregate_instructions, Arena *arena,
                                                 bool use_compiled_functions [[maybe_unused]]) const {
...
void Aggregator::addBatch(size_t row_begin, size_t row_end, AggregateFunctionInstruction *inst, AggregateDataPtr *places, Arena *arena) {
  if (inst->offsets)
    inst->batch_that->addBatchArray(row_begin, row_end, places, inst->state_offset, inst->batch_arguments, inst->offsets, arena);
  else if (inst->has_sparse_arguments)
    inst->batch_that->addBatchSparse(row_begin, row_end, places, inst->state_offset, inst->batch_arguments, arena);
  else
    inst->batch_that->addBatch(row_begin, row_end, places, inst->state_offset, inst->batch_arguments, arena);
}

void Aggregator::addBatchSinglePlace(size_t row_begin, size_t row_end, AggregateFunctionInstruction *inst, AggregateDataPtr place,
                                     Arena *arena) {
  if (inst->offsets)
    inst->batch_that->addBatchSinglePlace(inst->offsets[static_cast<ssize_t>(row_begin) - 1], inst->offsets[row_end - 1], place,
                                          inst->batch_arguments, arena);
  else if (inst->has_sparse_arguments)
    inst->batch_that->addBatchSparseSinglePlace(row_begin, row_end, place, inst->batch_arguments, arena);
  else
    inst->batch_that->addBatchSinglePlace(row_begin, row_end, place, inst->batch_arguments, arena);
}

void NO_INLINE Aggregator::executeOnIntervalWithoutKey(AggregatedDataVariants &data_variants, size_t row_begin, size_t row_end,
                                                       AggregateFunctionInstruction *aggregate_instructions) const {
  ...
  for (AggregateFunctionInstruction *inst = aggregate_instructions; inst->that; ++inst) {
    if (inst->offsets)
      inst->batch_that->addBatchSinglePlace(inst->offsets[static_cast<ssize_t>(row_begin) - 1], inst->offsets[row_end - 1],
                                            res + inst->state_offset, inst->batch_arguments, data_variants.aggregates_pool);
    else
      inst->batch_that->addBatchSinglePlace(row_begin, row_end, res + inst->state_offset, inst->batch_arguments,
                                            data_variants.aggregates_pool);
  }
}

void NO_INLINE Aggregator::mergeOnIntervalWithoutKey(AggregatedDataVariants &data_variants, size_t row_begin, size_t row_end,
...
void Aggregator::prepareAggregateInstructions(Columns columns, AggregateColumns &aggregate_columns, Columns &materialized_columns,
  ...
  for (size_t i = 0; i < params.aggregates_size; ++i) aggregate_columns[i].resize(params.aggregates[i].argument_names.size());
  ...
  for (size_t i = 0; i < params.aggregates_size; ++i) {
    ...
    for (size_t j = 0; j < aggregate_columns[i].size(); ++j) {
      ...
      if (aggregate_columns[i][j]->isSparse() && aggregate_columns[i][j]->getNumberOfDefaultRows() == 0) allow_sparse_arguments = false;
      ...
      if (aggregate_columns[i][j]->isSparse()) has_sparse_arguments = true;
    }
    ...
    while (const auto *func = typeid_cast<const AggregateFunctionState *>(that)) that = func->getNestedFunction().get();
    ...
    if (const auto *func = typeid_cast<const AggregateFunctionArray *>(that)) {
      ...
      while (const auto *nested_func = typeid_cast<const AggregateFunctionState *>(that)) that = nested_func->getNestedFunction().get();
...
bool Aggregator::executeOnBlock(const Block &block, AggregatedDataVariants &result, ColumnRawPtrs &key_columns,
                                AggregateColumns &aggregate_columns, bool &no_more_keys) const {
  return executeOnBlock(block.getColumns(),
                        /* row_begin= */ 0, block.rows(), result, key_columns, aggregate_columns, no_more_keys);
}

bool Aggregator::executeOnBlock(Columns columns, size_t row_begin, size_t row_end, AggregatedDataVariants &result,
  ...
  if (params.optimize_group_by_constant_keys) {
    ...
    for (size_t i = 0; i < params.keys_size; ++i) all_keys_are_const &= isColumnConst(*columns.at(keys_positions[i]));
  }
  ...
  if (result.type == AggregatedDataVariants::Type::without_key) {
    ...
    executeImpl(result, row_begin, row_end, key_columns, aggregate_functions_instructions.data(), no_more_keys, all_keys_are_const,
                overflow_row_ptr);
  }
  ...
  bool worth_convert_to_two_level = worthConvertToTwoLevel(params.group_by_two_level_threshold, result_size,
                                                           params.group_by_two_level_threshold_bytes, result_size_bytes);

  /** Converting to a two-level data structure.
   * It allows you to make, in the subsequent, an effective merge - either economical from memory or parallel.
   */
  if (result.isConvertibleToTwoLevel() && worth_convert_to_two_level) result.convertToTwoLevel();

  /// Checking the constraints.
  if (!checkLimits(result_size, no_more_keys)) return false;

  /** Flush data to disk if too much RAM is consumed.
   * Data can only be flushed to disk if a two-level aggregation structure is used.
   */
  if (params.max_bytes_before_external_group_by && result.isTwoLevel() &&
      current_memory_usage > static_cast<Int64>(params.max_bytes_before_external_group_by) && worth_convert_to_two_level) {
...
void Aggregator::writeToTemporaryFile(AggregatedDataVariants &data_variants, size_t max_temp_file_size) const {
  if (!tmp_data) throw Exception(ErrorCodes::LOGICAL_ERROR, "Cannot write to temporary file because temporary file is not initialized");
...
#undef M
  ...
  LOG_DEBUG(log,
            ...
            elapsed_seconds, rows, ReadableSize(uncompressed_size), ReadableSize(compressed_size),
            ...
            static_cast<double>(uncompressed_size) / compressed_size, static_cast<double>(rows) / elapsed_seconds,
...
template <typename Method>
Block Aggregator::convertOneBucketToBlock(AggregatedDataVariants &data_variants, Method &method, Arena *arena, bool final,
                                          Int32 bucket) const {
  ...
  Block block = std::get<Block>(convertToBlockImpl(method, method.data.impls[bucket], arena, data_variants.aggregates_pools, final,
                                                   method.data.impls[bucket].size(), return_single_block));

  block.info.bucket_num = static_cast<int>(bucket);
  return block;
}

Block Aggregator::mergeAndConvertOneBucketToBlock(ManyAggregatedDataVariants &variants, Arena *arena, bool final, Int32 bucket,
                                                  std::atomic<bool> &is_cancelled, RuntimeDataflowStatisticsCacheUpdaterPtr updater) const {
...
#define M(NAME)                                                                                       \
    ...
    if (updater) updater->recordAggregationStateSizes(merged_data, bucket);                           \
    if (is_cancelled.load(std::memory_order_seq_cst)) return {};                                      \
    block = convertOneBucketToBlock(merged_data, *merged_data.NAME, arena, final, bucket);            \
    if (updater) updater->recordAggregationKeySizes(*this, block);                                    \
  }
...
template <typename Method>
void Aggregator::mergeSingleLevelDataImplFixedMap(ManyAggregatedDataVariants &non_empty_data, Arena *arena, const UInt32 worker_id,
                                                  const UInt32 total_worker, std::atomic<bool> &is_cancelled) const {
  ...
  for (size_t result_num = 1, size = non_empty_data.size(); result_num < size; ++result_num) {
    ...
    if (compiled_aggregate_functions_holder) {
      mergeDataImpl<Method>(getDataVariant<Method>(*res).data, getDataVariant<Method>(current).data, arena, true, false, /*prefetch*/
                            is_cancelled, &parallel_param);
    } else
#endif
    {
      mergeDataImpl<Method>(getDataVariant<Method>(*res).data, getDataVariant<Method>(current).data, arena, false, false, /*prefetch*/
                            is_cancelled, &parallel_param);
    }
  }
}
...
#define M(NAME) \
  else if (method == AggregatedDataVariants::Type::NAME) block = convertOneBucketToBlock(variants, *variants.NAME, arena, final, bucket);
...
template <typename Method>
void Aggregator::writeToTemporaryFileImpl(AggregatedDataVariants &data_variants, Method &method, TemporaryBlockStreamHolder &out) const {
  ...
  if (!no_more_keys && params.max_rows_to_group_by && result_size > params.max_rows_to_group_by) {
    switch (params.group_by_overflow_mode) {
      case OverflowMode::THROW:
        ...
        throw Exception(ErrorCodes::TOO_MANY_ROWS, "Limit for rows to GROUP BY exceeded: has {} rows, maximum: {}", result_size,
...
bool Aggregator::isTypeFixedSize(const ManyAggregatedDataVariants &data_variants) const {
  if (data_variants.empty()) return false;

  const auto &first = data_variants.at(0);
  return first->type == AggregatedDataVariants::Type::key8 || first->type == AggregatedDataVariants::Type::key16;
}
...
void Aggregator::disableMinMaxOptimizationForFixedHashMaps(ManyAggregatedDataVariants &data_variants) const {
  ...
  if (first->type == AggregatedDataVariants::Type::key8)
    ...
    throw Exception(ErrorCodes::UNKNOWN_AGGREGATED_DATA_VARIANT,
                    "disableMinMaxOptimizationForFixedHashMaps only supports key8 and key16 variants.");
}

template <typename Method, typename Table>
Aggregator::ConvertToBlockResVariant Aggregator::convertToBlockImpl(Method &method, Table &data, Arena *arena, Arenas &aggregates_pools,
                                                                    bool final, size_t rows, bool return_single_block) const {
  if (data.empty()) {
    ...
    if (return_single_block) return std::move(finalized_block);
  ...
  if (is_simple_count) {
    ...
    auto init_out_cols = [&]() {
      ...
      if (final) out_count_col = assert_cast<ColumnUInt64 *>(out_cols->final_aggregate_columns[0].get());
    ...
    if (return_single_block) return finalizeBlock(params, getHeader(final), std::move(out_cols).value(), final, rows_in_current_block);
...
inline void Aggregator::insertAggregatesIntoColumns(Mapped &mapped, MutableColumns &final_aggregate_columns, Arena *arena) const {
  ...
  try {
    ...
    for (; insert_i < params.aggregates_size; ++insert_i)
      aggregate_functions[insert_i]->insertResultInto(mapped + offsets_of_aggregate_states[insert_i], *final_aggregate_columns[insert_i],
                                                      arena);
  } catch (...) {
...
Block Aggregator::insertResultsIntoColumns(PaddedPODArray<AggregateDataPtr> &places, OutputBlockColumns &&out_cols, Arena *arena,
                                           bool has_null_key_data [[maybe_unused]], bool use_compiled_functions [[maybe_unused]]) const {
  std::exception_ptr exception;
...
template <typename Method, typename Table>
Aggregator::ConvertToBlockResVariant Aggregator::convertToBlockImplFinal(Method &method, Table &data, Arena *arena,
  ...
  data.forEachValue([&](const auto &key, auto &mapped) {
    if (unlikely(!out_cols.has_value())) init_out_cols();
...
template <typename Method, typename Table>
Aggregator::ConvertToBlockResVariant NO_INLINE Aggregator::convertToBlockImplNotFinal(Method &method, Table &data, Arenas &aggregates_pools,
                                                                                      size_t, bool return_single_block) const {
  /// +1 for nullKeyData, if `data` doesn't have it - not a problem, just some memory for one excessive row will be preallocated
  const size_t max_block_size = (return_single_block ? data.size() : std::min(params.max_block_size, data.size())) + 1;
...
void Aggregator::addSingleKeyToAggregateColumns(AggregatedDataVariants &data_variants, MutableColumns &aggregate_columns) const {
...
void Aggregator::addArenasToAggregateColumns(const AggregatedDataVariants &data_variants, MutableColumns &aggregate_columns) const {
  for (size_t i = 0; i < params.aggregates_size; ++i) {
    auto &column_aggregate_func = assert_cast<ColumnAggregateFunction &>(*aggregate_columns[i]);
    for (const auto &pool : data_variants.aggregates_pools) column_aggregate_func.addArena(pool);
  }
}

void Aggregator::createStatesAndFillKeyColumnsWithSingleKey(AggregatedDataVariants &data_variants, Columns &key_columns, size_t key_row,
...
Block Aggregator::prepareBlockAndFillWithoutKey(AggregatedDataVariants &data_variants, bool final, bool is_overflows) const {
  size_t rows = 1;
  auto &&out_cols = prepareOutputBlockColumns(params, aggregate_functions, getHeader(final), data_variants.aggregates_pools, final, rows);
  ...
  if (data_variants.type == AggregatedDataVariants::Type::without_key || params.overflow_row) {
    ...
    if (!data) throw Exception(ErrorCodes::LOGICAL_ERROR, "Wrong data variant passed.");

    if (!final) {
      for (size_t i = 0; i < params.aggregates_size; ++i) aggregate_columns_data[i]->push_back(data + offsets_of_aggregate_states[i]);
      data = nullptr;
    } else {
    ...
    if (params.overflow_row)
      for (size_t i = 0; i < params.keys_size; ++i) key_columns[i]->insertDefault();
  }

  Block block = finalizeBlock(params, getHeader(final), std::move(out_cols), final, rows);

  if (is_overflows) block.info.is_overflows = true;

  if (final) destroyWithoutKey(data_variants);

  return block;
}

template <bool return_single_block>
Aggregator::ConvertToBlockRes<return_single_block> Aggregator::prepareBlockAndFillSingleLevel(AggregatedDataVariants &data_variants,
                                                                                              bool final) const {
  ConvertToBlockResVariant res_variant;
  const size_t rows = data_variants.sizeWithoutOverflowRow();
#define M(NAME)                                                                                                    \
  else if (data_variants.type == AggregatedDataVariants::Type::NAME) {                                             \
    res_variant = convertToBlockImpl(*data_variants.NAME, data_variants.NAME->data, data_variants.aggregates_pool, \
                                     data_variants.aggregates_pools, final, rows, return_single_block);            \
  }
...
#define M(NAME)                                                                                                             \
  else if (data_variants.type == AggregatedDataVariants::Type::NAME) return prepareBlocksAndFillTwoLevelImpl(data_variants, \
                                                                                                             *data_variants.NAME, final);
...
BlocksList Aggregator::prepareBlocksAndFillTwoLevelImpl(AggregatedDataVariants &data_variants, Method &method, bool final) const {
  ...
  auto converter = [this, &next_bucket_to_merge, &method, &data_variants, &res, final](size_t thread_id) {
    while (true) {
      ...
      if (bucket >= Method::Data::NUM_BUCKETS) break;

      if (method.data.impls[bucket].empty()) continue;
  ...
  BlocksList blocks;
  for (auto &blocks_list : res) blocks.splice(blocks.end(), std::move(blocks_list));
  return blocks;
}
...
BlocksList Aggregator::convertToBlocks(AggregatedDataVariants &data_variants, bool final) const {
  ...
  if (data_variants.empty()) return blocks;

  if (data_variants.without_key)
    blocks.emplace_back(
        prepareBlockAndFillWithoutKey(data_variants, final, data_variants.type != AggregatedDataVariants::Type::without_key));
  ...
  double elapsed_seconds = watch.elapsedSeconds();
  LOG_DEBUG(log, "Converted aggregated data to blocks. {} rows, {} in {} sec. ({:.3f} rows/sec., {}/sec.)", rows, ReadableSize(bytes),
            elapsed_seconds, static_cast<double>(rows) / elapsed_seconds, ReadableSize(static_cast<double>(bytes) / elapsed_seconds));

  return blocks;
}

template <typename Method, typename Table>
void NO_INLINE Aggregator::mergeDataNullKey(Table &table_dst, Table &table_src, Arena *arena) const {
  if constexpr (Method::low_cardinality_optimization || Method::one_key_nullable_optimization) {
    if (table_src.hasNullKeyData()) {
      if (!table_dst.hasNullKeyData()) {
        ...
        for (size_t i = 0; i < params.aggregates_size; ++i)
          aggregate_functions[i]->merge(table_dst.getNullKeyData() + offsets_of_aggregate_states[i],
                                        table_src.getNullKeyData() + offsets_of_aggregate_states[i], arena);

        for (size_t i = 0; i < params.aggregates_size; ++i)
          aggregate_functions[i]->destroy(table_src.getNullKeyData() + offsets_of_aggregate_states[i]);
      }
...
template <typename Method, typename Table>
void NO_INLINE Aggregator::mergeDataImpl(Table &table_dst, Table &table_src, Arena *arena, bool use_compiled_functions [[maybe_unused]],
  ...
  if (use_compiled_functions) {
    ...
    for (size_t i = 0; i < params.aggregates_size; ++i) {
      if (!is_aggregate_function_compiled[i])
        aggregate_functions[i]->mergeAndDestroyBatch(dst_places.data(), src_places.data(), dst_places.size(),
                                                     offsets_of_aggregate_states[i], thread_pool, is_cancelled, arena);
    }
  ...
  for (size_t i = 0; i < params.aggregates_size; ++i) {
    aggregate_functions[i]->mergeAndDestroyBatch(dst_places.data(), src_places.data(), dst_places.size(), offsets_of_aggregate_states[i],
                                                 thread_pool, is_cancelled, arena);
  }
}

template <typename Method, typename Table>
void NO_INLINE Aggregator::mergeDataNoMoreKeysImpl(Table &table_dst, AggregatedDataWithoutKey &overflows, Table &table_src,
                                                   Arena *arena) const {
  if (is_simple_count) {
    if constexpr (Method::low_cardinality_optimization || Method::one_key_nullable_optimization)
      mergeDataNullKeySimpleCount(table_dst, table_src);
  ...
  table_src.mergeToViaFind(table_dst, [&](AggregateDataPtr dst, AggregateDataPtr &src, bool found) {
    ...
    for (size_t i = 0; i < params.aggregates_size; ++i) aggregate_functions[i]->destroy(src + offsets_of_aggregate_states[i]);
...
void NO_INLINE Aggregator::mergeWithoutKeyDataImpl(ManyAggregatedDataVariants &non_empty_data, std::atomic<bool> &is_cancelled) const {
  AggregatedDataVariantsPtr &res = non_empty_data[0];
  ...
  for (size_t result_num = 1, size = non_empty_data.size(); result_num < size; ++result_num) {
    ...
    for (size_t i = 0; i < params.aggregates_size; ++i)
      if (aggregate_functions[i]->isAbleToParallelizeMerge())
        aggregate_functions[i]->merge(res_data + offsets_of_aggregate_states[i], current_data + offsets_of_aggregate_states[i], thread_pool,
                                      is_cancelled, res->aggregates_pool);
      else
        aggregate_functions[i]->merge(res_data + offsets_of_aggregate_states[i], current_data + offsets_of_aggregate_states[i],
                                      res->aggregates_pool);

    for (size_t i = 0; i < params.aggregates_size; ++i) aggregate_functions[i]->destroy(current_data + offsets_of_aggregate_states[i]);
...
template <typename Method>
void NO_INLINE Aggregator::mergeSingleLevelDataImpl(ManyAggregatedDataVariants &non_empty_data, std::atomic<bool> &is_cancelled) const {
  AggregatedDataVariantsPtr &res = non_empty_data[0];
  bool no_more_keys = false;
  ...
  for (size_t result_num = 1, size = non_empty_data.size(); result_num < size; ++result_num) {
    if (!checkLimits(res->sizeWithoutOverflowRow(), no_more_keys)) break;
...
#if USE_EMBEDDED_COMPILER
      if (compiled_aggregate_functions_holder) {
        mergeDataImpl<Method>(getDataVariant<Method>(*res).data, getDataVariant<Method>(current).data, res->aggregates_pool, true, prefetch,
                              is_cancelled);
      } else
#endif
      {
        mergeDataImpl<Method>(getDataVariant<Method>(*res).data, getDataVariant<Method>(current).data, res->aggregates_pool, false,
                              prefetch, is_cancelled);
      }
    } else if (res->without_key) {
      mergeDataNoMoreKeysImpl<Method>(getDataVariant<Method>(*res).data, res->without_key, getDataVariant<Method>(current).data,
                                      res->aggregates_pool);
    } else {
      mergeDataOnlyExistingKeysImpl<Method>(getDataVariant<Method>(*res).data, getDataVariant<Method>(current).data, res->aggregates_pool);
    }
...
void Aggregator::mergeSingleLevelDataImplFixedMap(ManyAggregatedDataVariants &non_empty_data, Arena *arena, UInt32 worker_id,
                                                  UInt32 total_worker, std::atomic<bool> &is_cancelled) const {
  if (non_empty_data.empty()) return;
...
void Aggregator::resetAggregatorExceptFirst(ManyAggregatedDataVariants &data_variants) const {
  for (size_t i = 1, size = data_variants.size(); i < size; ++i) data_variants[i]->aggregator = nullptr;
}

template <typename Method>
void NO_INLINE Aggregator::mergeBucketImpl(ManyAggregatedDataVariants &data, Int32 bucket, Arena *arena,
                                           std::atomic<bool> &is_cancelled) const {
  /// We merge all aggregation results to the first.
  AggregatedDataVariantsPtr &res = data[0];

  const bool prefetch =
      Method::State::has_cheap_key_calculation && params.enable_prefetch &&
...
#if USE_EMBEDDED_COMPILER
    if (compiled_aggregate_functions_holder) {
      mergeDataImpl<Method>(getDataVariant<Method>(*res).data.impls[bucket], getDataVariant<Method>(current).data.impls[bucket], arena,
                            true, prefetch, is_cancelled);
    } else
#endif
    {
      mergeDataImpl<Method>(getDataVariant<Method>(*res).data.impls[bucket], getDataVariant<Method>(current).data.impls[bucket], arena,
                            false, prefetch, is_cancelled);
    }
  }
}

ManyAggregatedDataVariants Aggregator::prepareVariantsToMerge(ManyAggregatedDataVariants &&data_variants) const {
  if (data_variants.empty()) throw Exception(ErrorCodes::EMPTY_DATA_PASSED, "Empty data passed to Aggregator::prepareVariantsToMerge.");
  ...
  for (auto &data : data_variants)
    if (!data->empty()) non_empty_data.push_back(data);

  if (non_empty_data.empty()) return {};

  if (non_empty_data.size() > 1) {
    /// Sort the states in descending order so that the merge is more efficient (since all states are merged into the first).
    ::sort(non_empty_data.begin(), non_empty_data.end(), [](const AggregatedDataVariantsPtr &lhs, const AggregatedDataVariantsPtr &rhs) {
  ...
  if (has_at_least_one_two_level)
    for (auto &variant : non_empty_data)
      if (!variant->isTwoLevel()) variant->convertToTwoLevel();
  ...
  for (size_t i = 1, size = non_empty_data.size(); i < size; ++i) {
     ...
     */
    first->aggregates_pools.insert(first->aggregates_pools.end(), non_empty_data[i]->aggregates_pools.begin(),
...
void NO_INLINE Aggregator::mergeStreamsImplCase(Arena *aggregates_pool, State &state, Table &data, bool no_more_keys,
                                                AggregateDataPtr overflow_row, size_t row_begin, size_t row_end,
                                                const AggregateColumnsConstData &aggregate_columns_data, std::atomic<bool> &is_cancelled,
                                                Arena *arena_for_keys) const {
  chassert(!is_simple_count);
  std::unique_ptr<AggregateDataPtr[]> places(new AggregateDataPtr[row_end]);

  if (!arena_for_keys) arena_for_keys = aggregates_pool;
  ...
  for (size_t j = 0; j < params.aggregates_size; ++j) {
    ...
    aggregate_functions[j]->mergeBatch(row_begin, row_end, places.get(), offsets_of_aggregate_states[j], aggregate_columns_data[j]->data(),
                                       thread_pool, is_cancelled, aggregates_pool);
  }
}

template <typename Method, typename Table>
void NO_INLINE Aggregator::mergeStreamsImpl(Block block, Arena *aggregates_pool, Method &method, Table &data, AggregateDataPtr overflow_row,
                                            LastElementCacheStats &consecutive_keys_cache_stats, bool no_more_keys,
                                            std::atomic<bool> &is_cancelled, Arena *arena_for_keys) const {
  ...
  mergeStreamsImpl<Method, Table>(aggregates_pool, method, data, overflow_row, consecutive_keys_cache_stats, no_more_keys, 0, block.rows(),
                                  aggregate_columns_data, key_columns, is_cancelled, arena_for_keys);
}

template <typename Method, typename Table>
void NO_INLINE Aggregator::mergeStreamsImpl(Arena *aggregates_pool, Method &method [[maybe_unused]], Table &data,
                                            AggregateDataPtr overflow_row, LastElementCacheStats &consecutive_keys_cache_stats,
                                            bool no_more_keys, size_t row_begin, size_t row_end,
                                            const AggregateColumnsConstData &aggregate_columns_data, const ColumnRawPtrs &key_columns,
                                            std::atomic<bool> &is_cancelled, Arena *arena_for_keys) const {
  UInt64 total_rows = consecutive_keys_cache_stats.hits + consecutive_keys_cache_stats.misses;
  double cache_hit_rate = total_rows ? static_cast<double>(consecutive_keys_cache_stats.hits) / static_cast<double>(total_rows) : 1.0;
  bool use_cache = !is_simple_count && cache_hit_rate >= params.min_hit_rate_to_use_consecutive_keys_optimization;
  ...
  if (use_cache) {
    ...
    if (is_simple_count) {
      ...
      mergeStreamsImplCase(aggregates_pool, state, data, no_more_keys, overflow_row, row_begin, row_end, aggregate_columns_data,
                           is_cancelled, arena_for_keys);
    }
...
void NO_INLINE Aggregator::mergeBlockWithoutKeyStreamsImpl(Block block, AggregatedDataVariants &result,
                                                           std::atomic<bool> &is_cancelled) const {
...
void NO_INLINE Aggregator::mergeWithoutKeyStreamsImpl(AggregatedDataVariants &result, size_t row_begin, size_t row_end,
                                                      const AggregateColumnsConstData &aggregate_columns_data,
                                                      std::atomic<bool> &is_cancelled) const {
  ...
  if (is_simple_count) {
    chassert(aggregate_columns_data.size() == 1);
    const auto &other_aggregated_counts = *aggregate_columns_data[0];
    for (size_t row = row_begin; row < row_end; ++row) getCountState(res) += getCountState(other_aggregated_counts[row]);
  ...
  for (size_t row = row_begin; row < row_end; ++row) {
    ...
    for (size_t i = 0; i < params.aggregates_size; ++i) {
      ...
      if (aggregate_functions[i]->isAbleToParallelizeMerge())
        aggregate_functions[i]->merge(res + offsets_of_aggregate_states[i], (*aggregate_columns_data[i])[row], thread_pool, is_cancelled,
...
#define M(NAME, IS_TWO_LEVEL)                                                                                         \
      ...
      mergeStreamsImpl(std::move(block), result.aggregates_pool, *result.NAME, result.NAME->data, result.without_key, \
                       result.consecutive_keys_cache_stats, no_more_keys, is_cancelled);

  APPLY_FOR_AGGREGATED_VARIANTS(M)
#undef M
  else if (result.type != AggregatedDataVariants::Type::without_key) throw Exception(ErrorCodes::UNKNOWN_AGGREGATED_DATA_VARIANT,
                                                                                     "Unknown aggregated data variant.");
...
void Aggregator::mergeBlocks(BucketToBlocks bucket_to_blocks, AggregatedDataVariants &result, std::atomic<bool> &is_cancelled) {
  if (bucket_to_blocks.empty()) return;

  UInt64 total_input_rows = 0;
  for (auto &bucket : bucket_to_blocks)
    for (auto &block : bucket.second) total_input_rows += block.rows();
  ...
  if (has_two_level) {
#define M(NAME) \
  if (method_chosen == AggregatedDataVariants::Type::NAME) method_chosen = AggregatedDataVariants::Type::NAME##_two_level;
  ...
  if (has_two_level) {
    ...
    auto merge_bucket = [&bucket_to_blocks, &result, &is_cancelled, &next_bucket_to_merge, max_bucket, this](Arena *aggregates_pool) {
      while (true) {
        ...
        if (bucket > max_bucket) break;

        if (!bucket_to_blocks.contains(bucket)) continue;
...
#define M(NAME)                                                                                                   \
      ...
      mergeStreamsImpl(std::move(block), aggregates_pool, *result.NAME, result.NAME->data.impls[bucket], nullptr, \
                       consecutive_keys_cache_stats_copy, false, is_cancelled);
...
#undef M
  ...
  if (has_blocks_with_unknown_bucket) {
    ...
    for (Block &block : blocks) {
      if (!checkLimits(result.sizeWithoutOverflowRow(), no_more_keys)) break;
...
#define M(NAME) \
  if (merge_method == AggregatedDataVariants::Type::NAME) merge_method = AggregatedDataVariants::Type::NAME##_hash64;
...
#undef APPLY_FOR_VARIANTS_THAT_MAY_USE_BETTER_HASH_FUNCTION
  ...
  /// Memory occupied by them will have the same lifetime as aggregate function states, while it is not actually necessary and leads to
  /// excessive memory consumption. To avoid this we use a separate arena to allocate memory for aggregation keys. Its memory will be freed
  /// at this function return.
  auto arena_for_keys = std::make_shared<Arena>();

  for (Block &block : blocks) {
    source_rows += block.rows();

    if (bucket_num >= 0 && block.info.bucket_num != bucket_num) bucket_num = -1;
...
#define M(NAME, IS_TWO_LEVEL)                                                                              \
      ...
      mergeStreamsImpl(std::move(block), result.aggregates_pool, *result.NAME, result.NAME->data, nullptr, \
                       result.consecutive_keys_cache_stats, false, is_cancelled, arena_for_keys.get());
...
#undef M
  ...
  LOG_DEBUG(
      ...
      bucket_num, rows, ReadableSize(bytes), source_rows, elapsed_seconds, rows / elapsed_seconds, ReadableSize(bytes / elapsed_seconds));
...
template <typename Method>
void NO_INLINE Aggregator::convertBlockToTwoLevelImpl(Method &method, Arena *pool, ColumnRawPtrs &key_columns, const Block &source,
...
#define M(NAME) else if (type == AggregatedDataVariants::Type::NAME) type = AggregatedDataVariants::Type::NAME##_two_level;
...
#define M(NAME) else if (data.type == AggregatedDataVariants::Type::NAME) num_buckets = data.NAME->data.NUM_BUCKETS;
...
#define M(NAME)                                             \
      ...
      convertBlockToTwoLevelImpl(*data.NAME, data.aggregates_pool, key_columns, block, split_blocks);
...
void NO_INLINE Aggregator::destroyImpl(Table &table) const {
  table.forEachMapped([&](AggregateDataPtr &data) {
    ...
    for (size_t i = 0; i < params.aggregates_size; ++i) aggregate_functions[i]->destroy(data + offsets_of_aggregate_states[i]);
...
void Aggregator::destroyWithoutKey(AggregatedDataVariants &result) const {
  ...
  if (nullptr != res_data) {
    for (size_t i = 0; i < params.aggregates_size; ++i) aggregate_functions[i]->destroy(res_data + offsets_of_aggregate_states[i]);
...
void Aggregator::destroyAllAggregateStates(AggregatedDataVariants &result) const {
  ...
  /// In what data structure is the data aggregated?
  if (result.type == AggregatedDataVariants::Type::without_key || params.overflow_row) destroyWithoutKey(result);

#define M(NAME, IS_TWO_LEVEL) \
  else if (result.type == AggregatedDataVariants::Type::NAME) destroyImpl<decltype(result.NAME)::element_type>(result.NAME->data);
...
UInt64 calculateCacheKey(const DB::ASTPtr &select_query) {
  if (!select_query) throw DB::Exception(DB::ErrorCodes::LOGICAL_ERROR, "Query ptr cannot be null");

  const auto &select = select_query->as<DB::ASTSelectQuery &>();

  // It may happen in some corner cases like `select 1 as num group by num`.
  if (!select.tables()) return 0;

  SipHash hash;
  hash.update(select.tables()->getTreeHash(/*ignore_aliases=*/true));
  if (const auto prewhere = select.prewhere()) hash.update(prewhere->getTreeHash(/*ignore_aliases=*/true));
  if (const auto where = select.where()) hash.update(where->getTreeHash(/*ignore_aliases=*/true));
  if (const auto group_by = select.groupBy()) hash.update(group_by->getTreeHash(/*ignore_aliases=*/true));
  return hash.get64();
}
}  // namespace DB
