...
struct SortedBlocksWriter {
  ...
  void addBlocks(const Blocks &blocks) { sorted_files.emplace_back(flush(blocks.blocks)); }
...
class SortedBlocksBuffer {
 public:
  SortedBlocksBuffer(const SortDescription &sort_description_, size_t max_bytes_)
      : max_bytes(max_bytes_), current_bytes(0), sort_description(sort_description_) {}
