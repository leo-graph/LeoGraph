...
void validateDataType(const DataTypePtr &type_to_check, const DataTypeValidationSettings &settings) {
      ...
      if (const auto *fixed_string = typeid_cast<const DataTypeFixedString *>(&data_type)) {
        if (fixed_string->getN() > MAX_FIXEDSTRING_SIZE_WITHOUT_SUSPICIOUS)
          throw Exception(ErrorCodes::ILLEGAL_COLUMN,
                          ...
                          data_type.getName(), MAX_FIXEDSTRING_SIZE_WITHOUT_SUSPICIOUS);
      }
    }
      ...
      if (const auto *variant_type = typeid_cast<const DataTypeVariant *>(&data_type)) {
        ...
        for (size_t i = 0; i < variants.size() - 1; ++i) {
          for (size_t j = i + 1; j < variants.size(); ++j) {
            ...
            if (isBool(variants[i]) || isBool(variants[j])) continue;

            const auto *custom_name = variant_type->getCustomName();
            if (custom_name && custom_name->getName() == "Geometry") continue;
            ...
            if (auto supertype = tryGetLeastSupertype(DataTypes{variants[i], variants[j]})) {
              throw Exception(
                  ...
                  data_type.getName(), variants[i]->getName(), variants[j]->getName(), supertype->getName());
            }
          }
        }
      }
  ...
  if (settings.validate_nested_types) type_to_check->forEachChild(validate_callback);
}
...
ColumnsDescription parseColumnsListFromString(const std::string &structure, const ContextPtr &context) {
  ...
  ASTPtr columns_list_raw = parseQuery(parser, structure, "columns declaration list", settings[Setting::max_query_size],
  ...
  if (!columns_list) throw Exception(ErrorCodes::LOGICAL_ERROR, "Could not cast AST to ASTExpressionList");
  ...
  for (const auto &[name, type] : columns.getAll()) validateDataType(type, validation_settings);
  return columns;
}
...
bool tryParseColumnsListFromString(const std::string &structure, ColumnsDescription &columns, const ContextPtr &context, String &error) {
  ...
  ASTPtr columns_list_raw =
      tryParseQuery(parser, start, end, error, false, "columns declaration list", false, settings[Setting::max_query_size],
                    settings[Setting::max_parser_depth], settings[Setting::max_parser_backtracks], true);
  if (!columns_list_raw) return false;
