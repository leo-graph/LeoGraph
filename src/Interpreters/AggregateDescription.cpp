...
void AggregateDescription::explain(JSONBuilder::JSONMap& map) const {
  ...
  if (function) {
    ...
    if (!params.empty()) {
      ...
      for (const auto& param : params) params_array->add(applyVisitor(FieldVisitorToString(), param));

      function_map->add("Parameters", std::move(params_array));
    }

    auto args_array = std::make_unique<JSONBuilder::JSONArray>();
    for (const auto& type : function->getArgumentTypes()) args_array->add(type->getName());
  ...
  auto args_array = std::make_unique<JSONBuilder::JSONArray>();
  for (const auto& name : argument_names) args_array->add(name);
...
void serializeAggregateDescriptions(const AggregateDescriptions& aggregates, WriteBuffer& out) {
  ...
  for (const auto& aggregate : aggregates) {
    ...
    if (argument_types.size() != num_args) {
      ...
      throw Exception(ErrorCodes::LOGICAL_ERROR, "Invalid number of for aggregate function. Expected {}, got {}. Description:\n{}",
    ...
    for (const auto& param : aggregate.parameters) writeFieldBinary(param, out);
  }
}
...
void deserializeAggregateDescriptions(AggregateDescriptions& aggregates, ReadBuffer& in) {
  ...
  for (auto& aggregate : aggregates) {
    ...
    for (auto& param : aggregate.parameters) param = readFieldBinary(in);

    auto action = NullsAction::EMPTY;  /// As I understand, it should be resolved to function name.
    AggregateFunctionProperties properties;
    aggregate.function = AggregateFunctionFactory::instance().get(function_name, action, argument_types, aggregate.parameters, properties);
  }
}
