...
void addDefaultRequiredExpressionsRecursively(const Block &block, const String &required_column_name, DataTypePtr required_column_type,
                                              const ColumnsDescription &columns, ASTPtr default_expr_list_accum, NameSet &added_columns,
  ...
  if (is_column_in_query)
    convert_null_to_default = null_as_default && isNullableOrLowCardinalityNullable(block.findByName(required_column_name)->type) &&
                              !isNullableOrLowCardinalityNullable(required_column_type);

  if ((is_column_in_query && !convert_null_to_default) || added_columns.contains(required_column_name)) return;
  ...
  if (column_default) {
    ...
    for (const auto &next_required_column_name : required_columns_names) {
      ...
      addDefaultRequiredExpressionsRecursively(block, next_required_column_name, required_column_type, columns, default_expr_list_accum,
                                               added_columns, recursive_null_as_default);
    }
    ...
    if (is_column_in_query && convert_null_to_default) {
      ...
      auto cast_expr =
          makeASTFunction("_CAST", std::move(expr), make_intrusive<ASTLiteral>(columns.get(required_column_name).type->getName()));
...
ASTPtr defaultRequiredExpressions(const Block &block, const NamesAndTypesList &required_columns, const ColumnsDescription &columns,
  ...
  if (default_expr_list->children.empty()) return nullptr;

  return default_expr_list;
}

ASTPtr convertRequiredExpressions(Block &block, const NamesAndTypesList &required_columns, const ColumnDefaults &column_defaults,
                                  bool forbid_default_defaults) {
  ASTPtr conversion_expr_list = make_intrusive<ASTExpressionList>();
  for (const auto &required_column : required_columns) {
    if (!block.has(required_column.name)) continue;

    const auto &column_in_block = block.getByName(required_column.name);
    if (column_in_block.type->equals(*required_column.type)) continue;
    ...
    if (isNullableOrLowCardinalityNullable(column_in_block.type) && !isNullableOrLowCardinalityNullable(required_column.type)) {
      ...
      auto convert_func =
          makeASTFunction("_CAST", makeASTFunction("ifNull", make_intrusive<ASTIdentifier>(required_column.name), default_value),
    ...
    auto cast_func = makeASTFunction("_CAST", make_intrusive<ASTIdentifier>(required_column.name),
                                     make_intrusive<ASTLiteral>(required_column.type->getName()));
...
std::optional<ActionsDAG> createExpressions(const Block &header, ASTPtr expr_list, bool save_unneeded_columns, ContextPtr context) {
  if (!expr_list) return {};
...
std::optional<ActionsDAG> createExpressionsAnalyzer(const Block &header, ASTPtr expr_list, bool save_unneeded_columns, ContextPtr context) {
  if (!expr_list) return {};
  ...
  for (const auto &column : header.getIndexByName())
    fake_column_descriptions.add(ColumnDescription(column.first, header.getByPosition(column.second).type), /*after_column=*/"",
                                 /*first=*/false, /*add_subcolumns=*/false);
  ...
  NameSet outputs;
  for (const auto &output : actions.getOutputs()) outputs.insert(output->result_name);
  for (const auto &input : actions.getInputs())
    if (!outputs.contains(input->result_name)) actions.getOutputs().push_back(input);
...
bool needConvertAnyNullToDefault(const Block &header, const NamesAndTypesList &required_columns, const ColumnsDescription &columns) {
  for (const auto &required_column : required_columns) {
    if (columns.has(required_column.name) && isNullableOrLowCardinalityNullable(header.findByName(required_column.name)->type) &&
        !isNullableOrLowCardinalityNullable(required_column.type))
      return true;
  }
  return false;
}
...
std::optional<ActionsDAG> evaluateMissingDefaults(const Block &header, const NamesAndTypesList &required_columns,
                                                  const ColumnsDescription &columns, ContextPtr context, bool save_unneeded_columns,
                                                  bool null_as_default) {
  if (!columns.hasDefaults() && (!null_as_default || !needConvertAnyNullToDefault(header, required_columns, columns))) return {};
...
static std::unordered_map<String, ColumnPtr> collectOffsetsColumns(const NamesAndTypesList &available_columns, const Columns &res_columns) {
  ...
  for (size_t i = 0; i < available_columns.size(); ++i, ++available_column) {
    if (res_columns[i] == nullptr || isColumnConst(*res_columns[i])) continue;

    /// Small hack. Currently sparse serialization is not supported with Arrays.
    if (res_columns[i]->isSparse()) continue;
        ...
        [&](const auto &subpath) {
          ...
          if (current_offsets_column && !current_offsets_column->empty()) {
            ...
            if (!offsets_column) {
              ...
              for (const auto &elem : subpath) inside_variant_element |= elem.type == ISerialization::Substream::VariantElement;
...
static ColumnPtr createColumnWithDefaultValue(const IDataType &data_type, const String &subcolumn_name, size_t num_rows) {
  ...
  if (subcolumn_name.empty()) return column->convertToFullColumnIfConst();
...
static bool hasDefault(const StorageSnapshotPtr &storage_snapshot, const NameAndTypePair &column) {
  if (!storage_snapshot) return false;

  if (storage_snapshot->getDefault(column.name).has_value()) return true;
...
static String removeTupleElementsFromSubcolumn(String subcolumn_name, const Names &tuple_elements) {
  ...
  for (const auto &elem : tuple_elements) {
    ...
    if (pos != std::string::npos) subcolumn_name.erase(pos, elem.size() + 1);
  }

  if (subcolumn_name.ends_with(".")) subcolumn_name.pop_back();
...
void fillMissingColumns(Columns &res_columns, size_t num_rows, const NamesAndTypesList &requested_columns,
  ...
  if (num_columns != res_columns.size())
    throw Exception(ErrorCodes::LOGICAL_ERROR, "Invalid number of columns passed to fillMissingColumns. Expected {}, got {}", num_columns,
                    res_columns.size());
  ...
  for (size_t i = 0; i < num_columns; ++i, ++requested_column) {
    if (res_columns[i] && partially_read_columns.contains(requested_column->name)) res_columns[i] = nullptr;

    /// Nothing to fill or default should be filled in evaluateMissingDefaults.
    if (res_columns[i] || hasDefault(storage_snapshot, *requested_column)) continue;
    ...
    if (!current_offsets.empty()) {
      ...
      IDataType::forEachSubcolumn(
          [&](const auto &path, const auto &, const auto &) {
            if (path.back().type == ISerialization::Substream::TupleElement) tuple_elements.push_back(path.back().name_of_substream);
          },
