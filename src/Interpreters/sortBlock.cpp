...
inline bool isCollationRequired(const SortColumnDescription &description) { return description.collator != nullptr; }
    ...
    if (isCollationRequired(sort_column_description)) {
      if (!column.column->isCollationSupported())
        throw Exception(ErrorCodes::BAD_COLLATION,
                        "Collations could be specified only for String, LowCardinality(String), "
...
void getBlockSortPermutationImpl(const Block &block, const SortDescription &description, IColumn::PermutationSortStability stability,
                                 UInt64 limit, IColumn::Permutation &permutation) {
  ...
  if (columns_with_sort_descriptions.size() == 1) {
    ...
    IColumn::PermutationSortDirection direction = column_with_sort_description.description.direction == -1
    ...
    if (isCollationRequired(column_with_sort_description.description))
      column->getPermutationWithCollation(*column_with_sort_description.description.collator, direction, stability, limit,
                                          nan_direction_hint, permutation);
    else
    ...
    for (const auto &column_with_sort_description : columns_with_sort_descriptions) {
      while (!ranges.empty() && limit && limit <= ranges.back().from) ranges.pop_back();

      if (ranges.empty()) break;

      if (column_with_sort_description.column_const) continue;
      ...
      if (is_collation_required) {
        column->updatePermutationWithCollation(*column_with_sort_description.description.collator, direction, stability, limit,
                                               nan_direction_hint, permutation, ranges);
      } else {
...
bool isIdentityPermutation(const IColumn::Permutation &permutation, size_t limit) {
  ...
  if (permutation[0] != 0) return false;
  ...
  if (size >= 8) {
    ...
    for (; i < size_unrolled; i += 8) {
      ...
      if (permutation_equals_vector_mask != compare_all_elements_equal_mask) return false;
    }
  }
#endif
  ...
  for (; i < size; ++i)
    if (permutation[i] != (permutation[i - 1] + 1)) return false;
...
bool isAlreadySortedImpl(size_t rows, Comparator compare) {
  ...
  if (rows > num_rows_to_try * 5) {
    for (size_t i = 1; i < num_rows_to_try; ++i) {
      ...
      if (compare(curr_position, prev_position)) return false;
    }
  }
...
void checkSortedWithPermutationImpl(size_t rows, Comparator compare, UInt64 limit, const IColumn::Permutation &permutation) {
  if (limit && limit < rows) rows = limit;
  ...
  for (size_t i = 1; i < rows; ++i) {
    ...
    if (compare(current_row, previous_row))
      throw Exception(ErrorCodes::LOGICAL_ERROR,
                      "Rows are not sorted with permutation, position {}, previous_row index {}, current_row index {}", i, previous_row,
                      current_row);
  }
}

void checkSortedWithPermutation(const Block &block, const SortDescription &description, UInt64 limit,
...
void sortBlock(Block &block, const SortDescription &description, UInt64 limit, IColumn::PermutationSortStability stability) {
  ...
  if (is_identity_permutation && limit == 0) return;
