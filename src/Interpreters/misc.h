...
inline bool functionIsJoinGet(const std::string& name) { return startsWith(name, "joinGet"); }
  ...
  if (functionIsInOrGlobalInOperator(func.name)) {
    ...
    if (num_arguments != 2)
      throw Exception(ErrorCodes::NUMBER_OF_ARGUMENTS_DOESNT_MATCH,
                      "Wrong number of arguments passed to function in. Expected: 2, passed: {}", num_arguments);
