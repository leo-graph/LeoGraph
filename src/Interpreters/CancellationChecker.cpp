...
bool CancellationChecker::CompareEndTime::operator()(const CancellationChecker::QueryToTrack& a,
                                                     const CancellationChecker::QueryToTrack& b) const {
  ...
  while (!stop_thread) {
    ...
    if (!query_set.empty()) {
      ...
      while (!query_set.empty()) {
        ...
        if (next_task_it->endtime > now_ms || next_task_it->timeout == 0) break;

        LOG_DEBUG(log, "Cancelling the task because of the timeout: {} ms, query_id: {}", next_task_it->timeout,
    ...
    if (query_set.empty()) {
      ...
      cond_var.wait_for(lock, std::chrono::milliseconds(query_set.begin()->endtime - now_ms), [&] {
