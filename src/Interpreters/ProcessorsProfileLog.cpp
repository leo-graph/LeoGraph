...
ColumnsDescription ProcessorProfileLogElement::getColumnsDescription() {
  return ColumnsDescription{
      ...
      {"plan_group", std::make_shared<DataTypeUInt64>(),
       "Group of the processor if it was created by query plan step. A group is a logical partitioning of processors added from the same "
       "query plan step. Group is used only for beautifying the result of EXPLAIN PIPELINE result."},
...
void ProcessorProfileLogElement::appendToBlock(MutableColumns& columns) const {
  ...
  columns[i++]->insert(id);
  {
    ...
    for (const UInt64 parent : parent_ids) parent_ids_array.emplace_back(parent);
