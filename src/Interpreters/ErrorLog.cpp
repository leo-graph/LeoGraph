...
ColumnsDescription ErrorLogElement::getColumnsDescription() {
  ...
  return ColumnsDescription{
      ...
      {"code", std::make_shared<DataTypeInt32>(),
       parseQuery(codec_parser, "(ZSTD(1))", 0, DBMS_DEFAULT_MAX_PARSER_DEPTH, DBMS_DEFAULT_MAX_PARSER_BACKTRACKS), "Error code."},
      {"error", std::make_shared<DataTypeLowCardinality>(std::make_shared<DataTypeString>()),
       parseQuery(codec_parser, "(ZSTD(1))", 0, DBMS_DEFAULT_MAX_PARSER_DEPTH, DBMS_DEFAULT_MAX_PARSER_BACKTRACKS), "Error name."},
      ...
      {"last_error_query_id", std::make_shared<DataTypeString>(),
       parseQuery(codec_parser, "(ZSTD(1))", 0, DBMS_DEFAULT_MAX_PARSER_DEPTH, DBMS_DEFAULT_MAX_PARSER_BACKTRACKS),
...
void ErrorLogElement::appendToBlock(MutableColumns& columns) const {
  ...
  for (auto* ptr : last_error_trace) last_error_trace_array.emplace_back(reinterpret_cast<uintptr_t>(ptr));
...
void ErrorLog::stepFunction(TimePoint current_time) {
  ...
  for (ErrorCodes::ErrorCode code = 0, end = ErrorCodes::end(); code < end; ++code) {
    ...
    if (error.local.count != previous_values.at(code).local) {
      ErrorLogElement local_elem{.event_time = event_time,
                                 .code = code,
    ...
    if (error.remote.count != previous_values.at(code).remote) {
      ErrorLogElement remote_elem{.event_time = event_time,
                                  .code = code,
