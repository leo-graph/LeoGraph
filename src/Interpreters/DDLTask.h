...
struct HostID {
  ...
  HostID(const String &host_name_, UInt16 port_) : host_name(host_name_), port(port_) {}
...
struct DDLTask : public DDLTaskBase {
  ...
  bool findCurrentHostID(ContextPtr global_context, LoggerPtr log, const ZooKeeperPtr &zookeeper,
  ...
  static bool isSelfHostID(LoggerPtr log, const HostID &checking_host_id, std::optional<UInt16> maybe_self_secure_port, UInt16 self_port);
  static bool isSelfHostname(LoggerPtr log, const String &checking_host_name, std::optional<UInt16> maybe_self_secure_port,
                             UInt16 self_port);
...
class ZooKeeperMetadataTransaction {
  enum State { CREATED, COMMITTED, FAILED };
 ...
 public:
  ...
  void addOp(Coordination::RequestPtr &&op) {
    if (isExecuted()) throw Exception(ErrorCodes::LOGICAL_ERROR, "Cannot add ZooKeeper operation because query is executed. It's a bug.");
    ops.emplace_back(op);
  }
  ...
  void addFinalizer(FinalizerCallback &&callback) {
    ...
    if (finalizer) throw Exception(ErrorCodes::LOGICAL_ERROR, "Finalizer already set. It's a bug.");
