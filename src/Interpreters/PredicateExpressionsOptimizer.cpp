...
PredicateExpressionsOptimizer::PredicateExpressionsOptimizer(ContextPtr context_, const TablesWithColumns &tables_with_columns_,
...
bool PredicateExpressionsOptimizer::optimize(ASTSelectQuery &select_query) {
  if (!enable_optimize_predicate_expression) return false;

  const bool has_incompatible_constructs = select_query.group_by_with_cube || select_query.group_by_with_rollup ||
                                           select_query.group_by_with_totals || select_query.group_by_with_grouping_sets;
  if (select_query.having() && !has_incompatible_constructs) tryMovePredicatesFromHavingToWhere(select_query);

  if (!select_query.tables() || select_query.tables()->children.empty()) return false;

  if ((!select_query.where() && !select_query.prewhere()) || select_query.arrayJoinExpressionList().first) return false;

  const auto &tables_predicates = extractTablesPredicates(select_query.where(), select_query.prewhere());

  if (!tables_predicates.empty()) return tryRewritePredicatesToTables(select_query.refTables()->children, tables_predicates);

  return false;
}

static bool hasInputTableFunction(const ASTPtr &expr) {
  if (const auto *func = typeid_cast<const ASTFunction *>(expr.get()); func && func->name == "input") return true;

  for (const auto &child : expr->children)
    if (hasInputTableFunction(child)) return true;
...
std::vector<ASTs> PredicateExpressionsOptimizer::extractTablesPredicates(const ASTPtr &where, const ASTPtr &prewhere) {
  ...
  for (const auto &predicate_expression : predicate_expressions) {
    ...
    if (expression_info.is_stateful_function || !expression_info.is_deterministic_function || expression_info.is_window_function) {
    ...
    if (hasInputTableFunction(predicate_expression)) return {};
    ...
    if (!expression_info.is_array_join) {
      if (expression_info.unique_reference_tables_pos.size() == 1)
        ...
        for (auto &predicate : tables_predicates) predicate.emplace_back(predicate_expression);
      }
    }
  }
...
bool PredicateExpressionsOptimizer::tryRewritePredicatesToTables(ASTs &tables_element, const std::vector<ASTs> &tables_predicates) {
  ...
  for (size_t index = tables_element.size(); index > 0; --index) {
    ...
    if (const auto &table_element = tables_element[table_pos]->as<ASTTablesInSelectQueryElement>()) {
      ...
      is_rewrite_tables |=
          tryRewritePredicatesToTable(tables_element[table_pos], tables_predicates[table_pos], tables_with_columns[table_pos]);
...
bool PredicateExpressionsOptimizer::tryRewritePredicatesToTable(ASTPtr &table_element, const ASTs &table_predicates,
                                                                const TableWithColumnNamesAndTypes &table_columns) const {
...
bool PredicateExpressionsOptimizer::tryMovePredicatesFromHavingToWhere(ASTSelectQuery &select_query) {
  ...
  const auto &reduce_predicates = [&](const ASTs &predicates) {
    ...
    for (size_t index = 1; index < predicates.size(); ++index) res = makeASTOperator("and", res, predicates[index]);
  ...
  for (const auto &moving_predicate : splitConjunctionsAst(select_query.having())) {
    ...
    if (expression_info.is_stateful_function) return false;
