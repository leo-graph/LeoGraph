...
BlockIO InterpreterRenameQuery::execute() {
  ...
  if (rename.database) return executeToDatabase(rename, descriptions);
...
BlockIO InterpreterRenameQuery::executeToTables(const ASTRenameQuery &rename, const RenameDescriptions &descriptions,
  ...
  for (const auto &elem : descriptions) {
    if (elem.if_exists) {
      ...
      if (!database_catalog.isTableExist(StorageID(elem.from_database_name, elem.from_table_name), getContext())) continue;
    }
    ...
    if (exchange_tables) {
      ...
      std::tie(from_ref_dependencies, from_loading_dependencies, from_mv_dependencies) =
          database_catalog.removeDependencies(from_table_id, false, false, false, /*mv*/ true);
      std::tie(to_ref_dependencies, to_loading_dependencies, to_mv_dependencies) =
          database_catalog.removeDependencies(to_table_id, false, false, false, /*mv*/ true);
    } else {
      ...
      std::tie(from_ref_dependencies, from_loading_dependencies, from_mv_dependencies) =
          database_catalog.removeDependencies(from_table_id, check_ref_deps, check_loading_deps, false, /*mv*/ true);
    }
    try {
      database->renameTable(getContext(), elem.from_table_name, *database_catalog.getDatabase(elem.to_database_name), elem.to_table_name,
                            exchange_tables, rename.dictionary);
      ...
      if (exchange_tables) NamedCollectionFactory::instance().renameDependencies(to_table_id, from_table_id);
    } catch (...) {
