...
InterpreterDescribeQuery::InterpreterDescribeQuery(const ASTPtr &query_ptr_, ContextPtr context_)
    : WithContext(context_), query_ptr(query_ptr_), settings(getContext()->getSettingsRef()) {}
...
BlockIO InterpreterDescribeQuery::execute() {
  ...
  Block sample_block = getSampleBlock(settings[Setting::describe_include_subcolumns], settings[Setting::describe_include_virtual_columns],
                                      settings[Setting::describe_compact_output]);

  MutableColumns res_columns = sample_block.cloneEmptyColumns();

  for (const auto &column : columns) addColumn(column, false, res_columns);

  for (const auto &column : virtual_columns) addColumn(column, true, res_columns);

  if (settings[Setting::describe_include_subcolumns]) {
    for (const auto &column : columns) addSubcolumns(column, false, res_columns);

    for (const auto &column : virtual_columns) addSubcolumns(column, true, res_columns);
  }
...
void InterpreterDescribeQuery::fillColumnsFromSubquery(const ASTTableExpression &table_expression) {
  ...
  for (auto &&column : *sample_block) columns.emplace_back(column.name, column.type);
}
...
void InterpreterDescribeQuery::fillColumnsFromTableFunction(const ASTTableExpression &table_expression) {
  ...
  for (const auto &column : column_descriptions) columns.emplace_back(column);
  ...
  if (settings[Setting::describe_include_virtual_columns]) {
    ...
    if (table) {
      ...
      for (const auto &column : common_virtuals) {
        if (!column_descriptions.has(column.name) && !column_names.contains(column.name)) virtual_columns.push_back(column);
      }
    }
  }
}
...
void InterpreterDescribeQuery::addColumn(const ColumnDescription &column, bool is_virtual, MutableColumns &res_columns) {
  ...
  if (settings[Setting::describe_include_subcolumns]) res_columns[i++]->insertDefault();

  if (settings[Setting::describe_include_virtual_columns]) res_columns[i++]->insert(is_virtual);
}
