...
class InterpreterSystemQuery : public IInterpreter, WithMutableContext {
 public:
  ...
  static void startStopActionInDatabase(StorageActionBlockType action_type, bool start, const String& database_name,
                                        const DatabasePtr& database, const ContextPtr& local_context, LoggerPtr log);

  static bool trySyncReplica(StoragePtr table, SyncReplicaMode sync_replica_mode, const std::unordered_set<String>& src_replicas,
                             ContextPtr context_);
 ...
 private:
  ...
  DatabasePtr restoreDatabaseFromKeeperPath(const String& zookeeper_name, const String& zookeeper_path, const String& full_replica_name,
                                            const String& restoring_database_name);
