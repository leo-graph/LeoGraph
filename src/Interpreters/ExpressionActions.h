...
struct Step {
  ...
  explicit Step(Names required_output_) {
    for (const auto &name : required_output_) required_output[name] = true;
  }
...
struct ExpressionActionsStep : public Step {
  ...
  explicit ExpressionActionsStep(ActionsAndProjectInputsFlagPtr actiactions_and_flags_, Names required_output_ = Names())
      : Step(std::move(required_output_)), actions_and_flags(std::move(actiactions_and_flags_)) {}

  NamesAndTypesList getRequiredColumns() const override { return actions_and_flags->dag.getRequiredColumns(); }

  ColumnsWithTypeAndName getResultColumns() const override { return actions_and_flags->dag.getResultColumns(); }

  void finalize(const NameSet &required_output_) override {
    if (!is_final_projection) actions_and_flags->dag.removeUnusedActions(required_output_);
  }

  void prependProjectInput() override { actions_and_flags->project_input = true; }

  std::string dump() const override { return actions_and_flags->dag.dumpDAG(); }
};
...
struct ExpressionActionsChain : WithContext {
  ...
  ActionsAndProjectInputsFlagPtr getLastActions(bool allow_empty = false) {
    if (auto *step = getLastExpressionStep(allow_empty)) return step->actions_and_flags;

    return nullptr;
  }

  ExpressionActionsChainSteps::Step &getLastStep() {
    if (steps.empty()) throw Exception(ErrorCodes::LOGICAL_ERROR, "Empty ExpressionActionsChain");

    return *steps.back();
  }

  ExpressionActionsChainSteps::Step &lastStep(const NamesAndTypesList &columns) {
    if (steps.empty()) return addStep(columns);
    return *steps.back();
  }
