...
static LoggerPtr getLogger() { return ::getLogger("ExpressionJIT"); }
...
class CompiledFunctionHolder : public CompiledExpressionCacheEntry {
 public:
  explicit CompiledFunctionHolder(CompiledFunction compiled_function_)
      : CompiledExpressionCacheEntry(compiled_function_.compiled_module.size), compiled_function(compiled_function_) {}

  ~CompiledFunctionHolder() override { getJITInstance().deleteCompiledModule(compiled_function.compiled_module); }
...
class LLVMExecutableFunction : public IExecutableFunction {
 public:
  explicit LLVMExecutableFunction(const std::string &name_, std::shared_ptr<CompiledFunctionHolder> compiled_function_holder_)
      : name(name_), compiled_function_holder(compiled_function_holder_) {}
...
class LLVMFunction : public IFunctionBase {
 public:
  explicit LLVMFunction(const CompileDAG &dag_) : name(dag_.dump()), dag(dag_) {
  ...
  bool isSuitableForShortCircuitArgumentsExecution(const DataTypesWithConstInfo &arguments) const override {
    for (const auto &f : nested_functions)
      if (!f->isSuitableForShortCircuitArgumentsExecution(arguments)) return false;
  ...
  ExecutableFunctionPtr prepare(const ColumnsWithTypeAndName &) const override {
    if (!compiled_function_holder) throw Exception(ErrorCodes::LOGICAL_ERROR, "Compiled function was not initialized {}", name);
  ...
  bool isDeterministic() const override {
    for (const auto &f : nested_functions)
      if (!f->isDeterministic()) return false;
  ...
  bool isDeterministicInScopeOfQuery() const override {
    for (const auto &f : nested_functions)
      if (!f->isDeterministicInScopeOfQuery()) return false;
  ...
  bool isSuitableForConstantFolding() const override {
    for (const auto &f : nested_functions)
      if (!f->isSuitableForConstantFolding()) return false;
  ...
  bool isInjective(const ColumnsWithTypeAndName &sample_block) const override {
    for (const auto &f : nested_functions)
      if (!f->isInjective(sample_block)) return false;
  ...
  bool hasInformationAboutMonotonicity() const override {
    for (const auto &f : nested_functions)
      if (!f->hasInformationAboutMonotonicity()) return false;
  ...
  Monotonicity getMonotonicityForRange(const IDataType &type, const Field &left, const Field &right) const override {
    ...
    for (size_t i = 0; i < nested_functions.size(); ++i) {
      ...
      if (!m.is_monotonic) return m;
      result.is_positive ^= !m.is_positive;
      result.is_always_monotonic &= m.is_always_monotonic;
      if (i + 1 < nested_functions.size()) {
        if (left_mut != Field()) applyFunction(*nested_functions[i], left_mut);
        if (right_mut != Field()) applyFunction(*nested_functions[i], right_mut);
        if (!m.is_positive) std::swap(left_mut, right_mut);
...
static FunctionBasePtr compile(const CompileDAG &dag, size_t min_count_to_compile_expression) {
  ...
  auto hash_key = dag.hash();
  {
    ...
    if (counter[hash_key]++ < min_count_to_compile_expression) return nullptr;
  }
  ...
  if (auto *compilation_cache = CompiledExpressionCacheFactory::instance().tryGetCache()) {
    ...
    std::shared_ptr<CompiledFunctionHolder> compiled_function_holder =
        std::static_pointer_cast<CompiledFunctionHolder>(compiled_function_cache_entry);
...
static const ActionsDAG::Node *removeAliasIfNecessary(const ActionsDAG::Node *node) {
  ...
  while (node_no_alias->type == ActionsDAG::ActionType::ALIAS) node_no_alias = node_no_alias->children[0];
...
static bool isCompilableFunction(const ActionsDAG::Node &node, const std::unordered_set<const ActionsDAG::Node *> &lazy_executed_nodes) {
  ...
  if (function.isShortCircuit(settings, node.children.size())) {
    for (const auto &child : node.children) {
      ...
      if (lazy_executed_nodes.contains(child_no_alias)) return false;
    }
  }
  ...
  for (size_t i = 0; i < argument_types.size(); ++i) {
    if (std::find(skip_arguments.begin(), skip_arguments.end(), i) != skip_arguments.end()) continue;
...
static CompileDAG getCompilableDAG(const ActionsDAG::Node *root, ActionsDAG::NodeRawConstPtrs &children,
  ...
  while (!stack.empty()) {
    ...
    while (frame.next_child_to_visit < node->children.size()) {
      ...
      if (skip_compile &&
          (!child->column || !isColumnConst(*child->column) || dynamic_cast<const ColumnConst *>(child->column.get())->getField().isNull()))
    ...
    if (!all_children_visited) continue;
    ...
    for (const auto *child : node->children) compile_node.arguments.push_back(visited_node_to_compile_dag_position[child]);
...
void ActionsDAG::compileFunctions(size_t min_count_to_compile_expression,
  ...
  for (const auto &node : nodes) {
    ...
    for (const auto &child : node.children) node_to_data[child].all_parents_compilable &= node_is_valid_for_compilation;
  }
   ...
   */
  ::sort(nodes_to_compile.begin(), nodes_to_compile.end(),
         [&](const Node *lhs, const Node *rhs) { return node_to_data[lhs].children_size > node_to_data[rhs].children_size; });
  ...
  for (auto &node : nodes_to_compile) {
    ...
    if (dag.getInputNodesCount() == 0) continue;

    if (auto fn = compile(dag, min_count_to_compile_expression)) {
      ColumnsWithTypeAndName arguments;
      arguments.reserve(new_children.size());
      for (const auto *child : new_children) arguments.emplace_back(child->column, child->result_type, child->result_name);
