...
String HostID::readableString() const { return host_name + ":" + DB::toString(port); }
...
void DDLLogEntry::assertVersion() const {
  if (version == 0
      ...
      || version == NORMALIZE_CREATE_ON_INITIATOR_VERSION || version > DDL_ENTRY_FORMAT_MAX_VERSION)
...
void DDLLogEntry::setSettingsIfRequired(ContextPtr context) {
  ...
  if (version <= 0 || version > DDL_ENTRY_FORMAT_MAX_VERSION)
    throw Exception(ErrorCodes::UNKNOWN_FORMAT_VERSION,
                    "Unknown distributed_ddl_entry_format_version: {}."
                    "Maximum supported version is {}.",
                    version, DDL_ENTRY_FORMAT_MAX_VERSION);

  parent_table_uuid = context->getParentTable();
  if (parent_table_uuid.has_value()) version = std::max(version, PARENT_TABLE_UUID_VERSION);

  /// NORMALIZE_CREATE_ON_INITIATOR_VERSION does not affect entry format in ZooKeeper
  if (version == NORMALIZE_CREATE_ON_INITIATOR_VERSION) version = SETTINGS_IN_ZK_VERSION;

  if (version >= SETTINGS_IN_ZK_VERSION) settings.emplace(context->getSettingsRef().changes());
}
...
String DDLLogEntry::toString() const {
  ...
  if (version >= BACKUP_RESTORE_FLAG_IN_ZK_VERSION) wb << "is_backup_restore: " << is_backup_restore << "\n";
...
void DDLLogEntry::parse(const String &data) {
  ...
  if (version == OLDEST_VERSION) {
    ...
    if (!rb.eof() && *rb.position() == 'h') rb >> "hosts: " >> host_id_strings >> "\n";
    ...
    if (!rb.eof() && *rb.position() == 's') {
      ...
      ASTPtr settings_ast = parseQuery(parser, settings_str, Context::getGlobalContextInstance()->getSettingsRef()[Setting::max_query_size],
  ...
  if (version >= OPENTELEMETRY_ENABLED_VERSION) {
    if (!rb.eof() && *rb.position() == 't') rb >> "tracing: " >> this->tracing_context;
  }
...
void DDLTaskBase::parseQueryFromEntry(ContextPtr context) {
  ...
  String description;
  query =
      parseQuery(parser_query, begin, end, description, 0, settings[Setting::max_parser_depth], settings[Setting::max_parser_backtracks]);
}
...
ContextMutablePtr DDLTaskBase::makeQueryContext(ContextPtr from_context, const ZooKeeperPtr & /*zookeeper*/) {
  ...
  if (preserve_user && !entry.initiator_user.empty()) {
    ...
    if (!user_id)
      throw Exception(ErrorCodes::UNKNOWN_USER, "User '{}' required for executing distributed DDL query is not found on this instance",
                      entry.initiator_user);
    ...
    for (const auto &role_name : entry.initiator_user_roles) {
      ...
      if (!role_id)
        throw Exception(ErrorCodes::UNKNOWN_ROLE, "Role '{}' required for executing distributed DDL query is not found on this instance",
                        role_name);
  ...
  if (entry.settings) query_context->applySettingsChanges(*entry.settings);

  return query_context;
}

bool DDLTask::findCurrentHostID(ContextPtr global_context, LoggerPtr log, const ZooKeeperPtr &zookeeper,
                                const std::optional<std::string> &config_host_name) {
  ...
  if (config_host_name) {
    if (!isSelfHostname(log, *config_host_name, maybe_secure_port, port))
      throw Exception(ErrorCodes::DNS_ERROR, "{} is not a local address. Check parameter 'host_name' in the configuration",
                      *config_host_name);
  }

  for (const HostID &host : entry.hosts) {
    if (config_host_name) {
      if (config_host_name != host.host_name) continue;

      if (maybe_secure_port != host.port && port != host.port) continue;
    ...
    try {
      if (!isSelfHostID(log, host, maybe_secure_port, port)) continue;
      ...
      if (e.code() != ErrorCodes::DNS_ERROR) throw;
    ...
    if (host_in_hostlist) {
      ...
      LOG_WARNING(log, "There are two the same ClickHouse instances in task {}: {} and {}. Will use the first one only.", entry_name,
                  host_id.readableString(), host.readableString());
    } else {
      host_in_hostlist = true;
      host_id = host;
      host_id_str = host.toString();
  ...
  if (!host_in_hostlist && first_exception) {
    ...
    if (entry.hosts.size() == finished_nodes_count) {
      LOG_WARNING(log,
                  "Failed to find current host ID, but assuming that {} is finished because the number of finished nodes ({}) "
...
void DDLTask::setClusterInfo(ContextPtr context, LoggerPtr log) {
  ...
  if (!query_on_cluster) throw Exception(ErrorCodes::UNKNOWN_TYPE_OF_QUERY, "Received unknown DDL query");
  ...
  if (!cluster)
    throw Exception(ErrorCodes::INCONSISTENT_CLUSTER_DEFINITION,
                    "DDL task {} contains current host {} in cluster {}, but there is no such cluster here.", entry_name,
  ...
  if (!tryFindHostInCluster()) {
    LOG_WARNING(
        log, "Not found the exact match of host {} from task {} in cluster {} definition. Will try to find it using host name resolving.",
    ...
    LOG_INFO(log, "Resolved host {} from task {} as host {} in definition of cluster {}", host_id.readableString(), entry_name,
             address_in_cluster.readableString(), cluster_name);
  }
      ...
      if (address.host_name == host_id.host_name && address.port == host_id.port) {
        if (found_exact_match) {
          if (default_database == address.default_database) {
            throw Exception(ErrorCodes::INCONSISTENT_CLUSTER_DEFINITION,
                            "There are two exactly the same ClickHouse instances {} in cluster {}", address.readableString(), cluster_name);
          }
          ...
          if (query_with_table) {
            ...
            if (default_database == query_with_table->getDatabase()) return true;
          }
        }
    ...
    for (size_t replica_num = 0; replica_num < shards[shard_num].size(); ++replica_num) {
      ...
      if (auto resolved = address.getResolvedAddress();
        ...
        if (found_via_resolving) {
          throw Exception(ErrorCodes::INCONSISTENT_CLUSTER_DEFINITION,
                          "There are two the same ClickHouse instances in cluster {} : {} and {}", cluster_name,
                          address_in_cluster.readableString(), address.readableString());
        }
...
String DDLTask::getShardID() const {
  ...
  for (const Cluster::Address &address : shard_addresses) replica_names.emplace_back(address.readableString());
  ::sort(replica_names.begin(), replica_names.end());

  String res;
  for (auto it = replica_names.begin(); it != replica_names.end(); ++it) res += *it + (std::next(it) != replica_names.end() ? "," : "");
...
bool DDLTask::isSelfHostname(LoggerPtr log, const String &checking_host_name, std::optional<UInt16> maybe_self_secure_port,
                             UInt16 self_port) {
  try {
...
DatabaseReplicatedTask::DatabaseReplicatedTask(const String &name, const String &path, DatabaseReplicated *database_)
    : DDLTaskBase(name, path), database(database_) {
  host_id_str = database->getFullReplicaName();
}

String DatabaseReplicatedTask::getShardID() const { return database->shard_name; }
...
ContextMutablePtr DatabaseReplicatedTask::makeQueryContext(ContextPtr from_context, const ZooKeeperPtr &zookeeper) {
  ...
  for (auto &op : ops) txn->addOp(std::move(op));
  ops.clear();
...
void DatabaseReplicatedTask::createSyncedNodeIfNeed(const ZooKeeperPtr &zookeeper) {
  ...
  Field value;
  if (!entry.settings->tryGet("database_replicated_enforce_synchronous_settings", value)) return;
  ...
  if (!value.safeGet<UInt64>()) return;

  zookeeper->createIfNotExists(getSyncedNodePath(), "");
}

String DDLTaskBase::getLogEntryName(UInt32 log_entry_number) { return zkutil::getSequentialNodeName("query-", log_entry_number); }
...
void ZooKeeperMetadataTransaction::commit() {
  if (state != CREATED) throw Exception(ErrorCodes::LOGICAL_ERROR, "Incorrect state ({}), it's a bug", state);
  state = FAILED;
...
ClusterPtr tryGetReplicatedDatabaseCluster(const String &cluster_name) {
  ...
  if (const auto *replicated_db = dynamic_cast<const DatabaseReplicated *>(DatabaseCatalog::instance().tryGetDatabase(name).get())) {
    if (all_groups) return replicated_db->tryGetAllGroupsCluster();
