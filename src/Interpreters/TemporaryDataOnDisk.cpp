...
inline CompressionCodecPtr getCodec(const TemporaryDataOnDiskSettings &settings) {
  if (settings.compression_codec.empty()) return CompressionCodecFactory::instance().get("LZ4");
...
TemporaryFileHolder::TemporaryFileHolder(const TemporaryDataMetrics &metrics) : metric_increment(metrics.current_metric) {
  ProfileEvents::increment(ProfileEvents::ExternalProcessingFilesTotal);
  if (metrics.num_files) ProfileEvents::increment(metrics.num_files.value());
}

class TemporaryFileInLocalCache : public TemporaryFileHolder {
 public:
  explicit TemporaryFileInLocalCache(FileCache &file_cache, size_t reserve_size, const TemporaryDataOnDiskSettings &settings)
      : TemporaryFileHolder(settings.metrics), buffer_size(settings.buffer_size) {
    ...
    segment_holder = file_cache.set(key, 0, std::max<size_t>(1, reserve_size), CreateFileSegmentSettings(FileSegmentKind::Ephemeral),
  ...
  String describeFilePath() const override { return fmt::format("fscache://{}", segment_holder->front().getPath()); }
...
class TemporaryFileInDistributedCache final : public TemporaryFileHolder {
 public:
  ...
  ~TemporaryFileInDistributedCache() override {
    try {
      if (cache_client) cache_client->makeDropCacheRequest(file_key, /*connection_info_hash=*/0, /*is_temporary_data=*/true);
    } catch (...) {
  ...
  std::unique_ptr<WriteBuffer> write() override {
    ...
    return std::make_unique<WriteBufferFromDistributedCache>(file_key, write_settings, timeouts, receive_throttler, send_throttler,
                                                             distributed_cache_server, distributed_cache_log, buffer_size);
  }

  std::unique_ptr<SeekableReadBuffer> read(size_t buffer_size_) const override {
    if (!cache_client) return std::make_unique<EmptyReadBuffer>();

    if (buffer_size_ == 0) buffer_size_ = DBMS_DEFAULT_BUFFER_SIZE;

    auto local_read_settings = read_settings;
    local_read_settings.remote_fs_buffer_size = buffer_size_;
    return std::make_unique<ReadBufferFromDistributedCache>(file_key, bytes_written, local_read_settings, timeouts, receive_throttler,
                                                            send_throttler, distributed_cache_server, distributed_cache_log);
  }
  ...
  String describeFilePath() const override { return fmt::format("distrcache://{}", file_key); }
...
class TemporaryFileOnLocalDisk : public TemporaryFileHolder {
 public:
    ...
    if (reserve_size > 0) {
      ...
      if (!reservation) {
        ...
        for (const auto &d : disks) {
          ...
          disks_info.push_back(fmt::format("{}: available: {} unreserved: {}, total: {}, keeping: {}", d->getName(),
        ...
        throw Exception(ErrorCodes::NOT_ENOUGH_SPACE, "Not enough space on temporary disk, cannot reserve {} bytes on [{}]", reserve_size,
                        fmt::join(disks_info, ", "));
      }
  ...
  std::unique_ptr<WriteBuffer> write() override { return disk->writeFile(path_to_file, buffer_size); }
  ...
  String describeFilePath() const override { return fmt::format("disk({})://{}/{}", disk->getName(), disk->getPath(), path_to_file); }
  ...
  ~TemporaryFileOnLocalDisk() override {
    try {
      if (disk->existsFile(path_to_file)) {
        ...
        LOG_WARNING(getLogger("TemporaryFileOnLocalDisk"), "Temporary path '{}' does not exist in '{}' on disk {}", path_to_file,
                    disk->getPath(), disk->getName());
      }
    } catch (...) {
...
TemporaryFileProvider createTemporaryFileProvider(VolumePtr volume) {
  if (!volume) throw Exception(ErrorCodes::LOGICAL_ERROR, "Volume is not initialized");
...
TemporaryFileProvider createTemporaryFileProvider(FileCache *file_cache) {
  if (!file_cache || !file_cache->isInitialized()) throw Exception(ErrorCodes::LOGICAL_ERROR, "File cache is not initialized");
...
TemporaryDataOnDiskScopePtr TemporaryDataOnDiskScope::childScope(TemporaryDataMetrics metrics_, UInt64 buffer_size_,
                                                                 String compression_codec_) {
  TemporaryDataOnDiskSettings child_settings = settings;
  child_settings.metrics = metrics_;
  if (buffer_size_) child_settings.buffer_size = buffer_size_;
  if (!compression_codec_.empty()) child_settings.compression_codec = compression_codec_;
  return std::make_shared<TemporaryDataOnDiskScope>(shared_from_this(), child_settings);
}

TemporaryDataReadBuffer::TemporaryDataReadBuffer(std::unique_ptr<ReadBuffer> in_) : ReadBuffer(nullptr, 0), compressed_buf(std::move(in_)) {
...
void TemporaryDataBuffer::nextImpl() {
  if (!out_compressed_buf) throw Exception(ErrorCodes::LOGICAL_ERROR, "Temporary file buffer writing has been finished");
...
String TemporaryDataBuffer::describeFilePath() const { return file_holder->describeFilePath(); }
...
TemporaryDataBuffer::Stat TemporaryDataBuffer::finishWriting() {
  ...
  std::call_once(write_finished, [this] {
    if (canceled) throw Exception(ErrorCodes::INVALID_STATE, "Writing to temporary file buffer was not successful");
    next();
    finalize();
  });
  return stat;
...
TemporaryDataBuffer::Stat TemporaryDataBuffer::getStat() const { return stat; }

std::unique_ptr<ReadBuffer> TemporaryDataBuffer::read() { return std::make_unique<TemporaryDataReadBuffer>(readRaw()); }

std::unique_ptr<SeekableReadBuffer> TemporaryDataBuffer::readRaw() {
  finishWriting();

  if (stat.compressed_size == 0 && stat.uncompressed_size == 0) return std::make_unique<ReadBufferFromEmptyFile>();
...
CompressedWriteBuffer &TemporaryDataBuffer::getCompressedWriteBuffer() { return *out_compressed_buf; }
...
void TemporaryDataBuffer::updateAllocAndCheck() {
  ...
  if (unlikely(new_compressed_size < stat.compressed_size || new_uncompressed_size < stat.uncompressed_size)) {
    throw Exception(ErrorCodes::LOGICAL_ERROR, "Temporary file {} size decreased after write: compressed: {} -> {}, uncompressed: {} -> {}",
                    file_holder ? file_holder->describeFilePath() : "NULL", new_compressed_size, stat.compressed_size,
                    new_uncompressed_size, stat.uncompressed_size);
  }
  ...
  if (metrics.bytes_compressed) ProfileEvents::increment(metrics.bytes_compressed.value(), compressed_delta);
  if (metrics.bytes_uncompressed) ProfileEvents::increment(metrics.bytes_uncompressed.value(), uncompressed_delta);
...
void TemporaryDataBuffer::freeAlloc() {
  if (parent) parent->deltaAllocAndCheck(-stat.compressed_size, -stat.uncompressed_size);
...
void TemporaryDataOnDiskScope::deltaAllocAndCheck(ssize_t compressed_delta, ssize_t uncompressed_delta) {
  if (parent) parent->deltaAllocAndCheck(compressed_delta, uncompressed_delta);
  ...
  if (compressed_delta > 0 && settings.max_size_on_disk && new_consumption > settings.max_size_on_disk)
    throw Exception(ErrorCodes::TOO_MANY_ROWS_OR_BYTES, "Limit for temporary files size exceeded (would consume {} / {} bytes)",
                    new_consumption, settings.max_size_on_disk);

  stat.compressed_size += compressed_delta;
  stat.uncompressed_size += uncompressed_delta;
}

TemporaryBlockStreamHolder::TemporaryBlockStreamHolder(SharedHeader header_, std::shared_ptr<TemporaryDataOnDiskScope> parent_,
                                                       size_t reserve_size)
...
TemporaryDataBuffer::Stat TemporaryBlockStreamHolder::finishWriting() const {
  if (!holder) throw Exception(ErrorCodes::LOGICAL_ERROR, "Temporary block stream is not initialized");
