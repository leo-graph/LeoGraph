...
struct ExplainAnalyzedSyntaxMatcher {
  ...
  static void visit(ASTPtr &ast, Data &data) {
    if (auto *select = ast->as<ASTSelectQuery>()) visit(*select, ast, data);
  }

  static void visit(ASTSelectQuery &select, ASTPtr &node, Data &data) {
    InterpreterSelectQuery interpreter(node, data.getContext(), SelectQueryOptions(QueryProcessingStage::FetchColumns).analyze().modify());
...
class TableFunctionSecretsVisitor : public InDepthQueryTreeVisitor<TableFunctionSecretsVisitor> {
  ...
  void visitImpl(VisitQueryTreeNodeType &query_tree_node) {
    ...
    if (!table_function_node_ptr) return;

    if (FunctionSecretArgumentsFinder::Result secret_arguments =
            TableFunctionSecretArgumentsFinderTreeNode(*table_function_node_ptr).getResult();
      ...
      for (size_t n = secret_arguments.start; n < secret_arguments.start + secret_arguments.count; ++n) {
        ...
        if (constant_node) constant_node->setMaskId();
      }
    }
  }
};
...
Block InterpreterExplainQuery::getSampleBlock(const ASTExplainQuery::ExplainKind kind) {
  if (kind == ASTExplainQuery::ExplainKind::QueryEstimates) {
    auto cols = NamesAndTypes{
        {"database", std::make_shared<DataTypeString>()}, {"table", std::make_shared<DataTypeString>()},
        {"parts", std::make_shared<DataTypeUInt64>()},    {"rows", std::make_shared<DataTypeUInt64>()},
...
static void fillColumn(IColumn &column, const std::string &str) {
  ...
  if (start < end) column.insertData(str.data() + start, end - start);
}
...
struct QueryASTSettings {
  ...
  std::unordered_map<std::string, std::reference_wrapper<bool>> boolean_settings = {{"graph", graph}, {"optimize", optimize}};
...
struct QueryTreeSettings {
  ...
  std::unordered_map<std::string, std::reference_wrapper<bool>> boolean_settings = {
      {"run_passes", run_passes}, {"dump_tree", dump_tree}, {"dump_passes", dump_passes}, {"dump_ast", dump_ast}};
...
struct ExplainSettings : public Settings {
  ...
  bool has(const std::string &name_) const { return hasBooleanSetting(name_) || hasIntegerSetting(name_); }

  bool hasBooleanSetting(const std::string &name_) const { return boolean_settings.count(name_) > 0; }

  bool hasIntegerSetting(const std::string &name_) const { return integer_settings.count(name_) > 0; }

  void setBooleanSetting(const std::string &name_, bool value) {
    auto it = boolean_settings.find(name_);
    if (it == boolean_settings.end()) throw Exception(ErrorCodes::LOGICAL_ERROR, "Unknown setting for ExplainSettings: {}", name_);
  ...
  void setIntegerSetting(const std::string &name_, Int64 value) {
    ...
    if (it == integer_settings.end()) throw Exception(ErrorCodes::LOGICAL_ERROR, "Unknown setting for ExplainSettings: {}", name_);
...
struct QuerySyntaxSettings {
  ...
  std::unordered_map<std::string, std::reference_wrapper<bool>> boolean_settings = {{"oneline", oneline},
                                                                                    {"run_query_tree_passes", run_query_tree_passes}};

  std::unordered_map<std::string, std::reference_wrapper<Int64>> integer_settings = {{"query_tree_passes", query_tree_passes}};
};

template <typename Settings>
ExplainSettings<Settings> checkAndGetSettings(const ASTPtr &ast_settings) {
  if (!ast_settings) return {};
  ...
  for (const auto &change : set_query.changes) {
    if (!settings.has(change.name))
      throw Exception(ErrorCodes::UNKNOWN_SETTING,
                      "Unknown setting \"{}\" for EXPLAIN {} query. "
                      "Supported settings: {}",
                      change.name, Settings::name, settings.getSettingsList());

    if (change.value.getType() != Field::Types::UInt64)
      throw Exception(ErrorCodes::INVALID_SETTING_VALUE, "Invalid type {} for setting \"{}\" only integer settings are supported",
...
bool explainQueryTree(ASTPtr explained_query, ContextPtr query_context, const QueryTreeSettings &settings, WriteBuffer &buf) {
  if (explained_query->as<ASTSelectWithUnionQuery>() == nullptr) return false;
...
QueryPipeline InterpreterExplainQuery::executeImpl() {
  ...
  switch (ast.getKind()) {
    ...
    case ASTExplainQuery::AnalyzedSyntax: {
      ...
      if (query_context->getSettingsRef()[Setting::allow_experimental_analyzer]) {
        bool explain_ok = explainQueryTree(ast.getExplainedQuery(), query_context,
                                           QueryTreeSettings{
    ...
    case ASTExplainQuery::QueryTree: {
      if (!query_context->getSettingsRef()[Setting::allow_experimental_analyzer])
        throw Exception(ErrorCodes::NOT_IMPLEMENTED, "EXPLAIN QUERY TREE is only supported with the analyzer. SET enable_analyzer = 1.");
    ...
    case ASTExplainQuery::QueryPlan: {
      ...
      if (settings.json) {
        ...
        plan.explainPlan(buf, settings.query_plan_options, 0,
                         query_context->getSettingsRef()[Setting::query_plan_max_step_description_length]);
      break;
    }
    case ASTExplainQuery::QueryPipeline: {
      if (dynamic_cast<const ASTSelectWithUnionQuery *>(ast.getExplainedQuery().get())) {
        ...
        InterpreterInsertQuery insert(ast.getExplainedQuery(), insert_context,
    ...
    case ASTExplainQuery::TableOverride: {
      if (auto *table_function = ast.getTableFunction()->as<ASTFunction>(); !table_function || table_function->name != "mysql") {
        throw Exception(ErrorCodes::INCORRECT_QUERY, "EXPLAIN TABLE OVERRIDE is not supported for the {}() table function",
                        table_function->name);
      }
  ...
  return QueryPipeline(
      std::make_shared<SourceFromSingleChunk>(std::make_shared<const Block>(sample_block.cloneWithColumns(std::move(res_columns)))));
}
