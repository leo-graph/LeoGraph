...
InterpreterSelectWithUnionQuery::InterpreterSelectWithUnionQuery(const ASTPtr &query_ptr_, ContextPtr context_,
...
InterpreterSelectWithUnionQuery::InterpreterSelectWithUnionQuery(const ASTPtr &query_ptr_, ContextMutablePtr context_,
                                                                 const SelectQueryOptions &options_,
                                                                 const Names &required_result_column_names)
  ...
  if (options.subquery_depth == 0 && (settings[Setting::limit] > 0 || settings[Setting::offset] > 0)) settings_limit_offset_needed = true;

  size_t num_children = ast->list_of_selects->children.size();
  if (!num_children) throw Exception(ErrorCodes::LOGICAL_ERROR, "No children in ASTSelectWithUnionQuery");
  ...
  if (num_children == 1 && settings_limit_offset_needed && !options.settings_limit_offset_done) {
    ...
    if (!select_query) throw Exception(ErrorCodes::LOGICAL_ERROR, "Invalid type in list_of_selects: {}", first_select_ast->getID());
    ...
    if (!select_query->withFill() && !select_query->limit_with_ties) {
      ...
      if (limit_length_ast) {
        ...
        if (settings[Setting::offset] == 0)
          ...
          new_limit_length = settings[Setting::limit]
                                 ? std::min(settings[Setting::limit].value, limit_length - settings[Setting::offset].value)
  ...
  for (size_t query_num = 0; query_num < num_children; ++query_num) {
    ...
    nested_interpreters.emplace_back(buildCurrentChildInterpreter(ast->list_of_selects->children.at(query_num),
    ...
    for (size_t query_num = 0; query_num < num_children; ++query_num) {
      ...
      if (!current_required_result_column_names.empty()) {
        ...
        if (current_required_result_column_names != header_columns) {
          throw Exception(ErrorCodes::LOGICAL_ERROR, "Different order of columns in UNION subquery: {} and {}",
                          fmt::join(current_required_result_column_names, ", "), fmt::join(header_columns, ", "));
        }
      }
    }
  ...
  for (auto &interpreter : nested_interpreters) {
    if (!interpreter->ignoreLimits()) all_nested_ignore_limits = false;
    if (!interpreter->ignoreQuota()) all_nested_ignore_quota = false;
  }
...
Block InterpreterSelectWithUnionQuery::getCommonHeaderForUnion(const SharedHeaders &headers) {
  ...
  for (size_t query_num = 1; query_num < num_selects; ++query_num) {
    if (headers[query_num]->columns() != num_columns)
      throw Exception(ErrorCodes::UNION_ALL_RESULT_STRUCTURES_MISMATCH, "Different number of columns in UNION ALL elements:\n{}\nand\n{}\n",
  ...
  for (size_t column_num = 0; column_num < num_columns; ++column_num) {
    for (size_t i = 0; i < num_selects; ++i) columns[i] = &headers[i]->getByPosition(column_num);
...
SharedHeader InterpreterSelectWithUnionQuery::getCurrentChildResultHeader(const ASTPtr &ast_ptr_,
                                                                          const Names &required_result_column_names) {
...
std::unique_ptr<IInterpreterUnionOrSelectQuery> InterpreterSelectWithUnionQuery::buildCurrentChildInterpreter(
...
SharedHeader InterpreterSelectWithUnionQuery::getSampleBlock(const ASTPtr &query_ptr_, ContextPtr context_, bool is_subquery,
                                                             bool is_create_parameterized_view) {
  if (!context_->hasQueryContext()) {
    SelectQueryOptions options;
    if (is_subquery) options = options.subquery();
    if (is_create_parameterized_view) options = options.createParameterizedView();
...
void InterpreterSelectWithUnionQuery::buildQueryPlan(QueryPlan &query_plan) {
  ...
  for (auto &interpreter : nested_interpreters) interpreter->addStorageLimits(storage_limits);
  ...
  if (num_plans == 1) {
    ...
    for (size_t i = 0; i < num_plans; ++i) {
      ...
      if (!blocksHaveEqualStructure(*plans[i]->getCurrentHeader(), *result_header)) {
        auto actions_dag =
            ActionsDAG::makeConvertingActions(plans[i]->getCurrentHeader()->getColumnsWithTypeAndName(),
                                              result_header->getColumnsWithTypeAndName(), ActionsDAG::MatchColumnsMode::Position, context);
    ...
    if (query.union_mode == SelectUnionMode::UNION_DISTINCT) {
      ...
      auto distinct_step = std::make_unique<DistinctStep>(query_plan.getCurrentHeader(), limits, 0, result_header->getNames(), false);
  ...
  if (settings_limit_offset_needed && !options.settings_limit_offset_done) {
    if (settings[Setting::limit] > 0) {
      auto limit = std::make_unique<LimitStep>(query_plan.getCurrentHeader(), settings[Setting::limit], settings[Setting::offset],
