...
ColumnsDescription OpenTelemetrySpanLogElement::getColumnsDescription() {
  auto span_kind_type =
      std::make_shared<DataTypeEnum8>(DataTypeEnum8::Values{{"INTERNAL", static_cast<Int8>(OpenTelemetry::SpanKind::INTERNAL)},
  ...
  auto status_code_type =
      std::make_shared<DataTypeEnum8>(DataTypeEnum8::Values{{"UNSET", static_cast<Int8>(OpenTelemetry::SpanStatus::UNSET)},
  ...
  return ColumnsDescription{
      ...
      {"kind", std::move(span_kind_type),
       ...
       "PRODUCER — Indicates that the span describes the initiators of an asynchronous request. This parent span will often end before the "
       "corresponding child CONSUMER span, possibly even before the child span starts. "
      ...
      {"attribute", std::make_shared<DataTypeMap>(low_cardinality_string, std::make_shared<DataTypeString>()),
       "Attribute depending on the trace span. They are filled in according to the recommendations in the OpenTelemetry standard."},
  };
}
...
NamesAndAliases OpenTelemetrySpanLogElement::getNamesAndAliases() {
  ...
  return {{"attribute.names", std::make_shared<DataTypeArray>(low_cardinality_string), "mapKeys(attribute)"},
...
void OpenTelemetrySpanLogElement::appendToBlock(MutableColumns& columns) const {
  ...
  for (const auto& attribute : span.attributes) attributes_map.push_back(Tuple{attribute.getKey(), attribute.getValue()});
