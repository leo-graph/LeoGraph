...
class EmbeddedDictionaries : WithContext {
 private:
  ...
  template <typename Dictionary>
  bool reloadDictionary(MultiVersion<Dictionary>& dictionary, DictionaryReloader<Dictionary> reload_dictionary, bool throw_on_error,
                        bool force_reload);

 public:
  /// Every reload_period seconds directories are updated inside a separate thread.
  EmbeddedDictionaries(std::unique_ptr<GeoDictionariesLoader> geo_dictionaries_loader, ContextPtr context, bool throw_on_error);
  ...
  MultiVersion<RegionsHierarchies>::Version getRegionsHierarchies() const { return regions_hierarchies.get(); }

  MultiVersion<RegionsNames>::Version getRegionsNames() const { return regions_names.get(); }
};
