...
struct GraphComponent {
  ...
  struct ASTHash {
    size_t operator()(const IASTHash& hash) const { return hash.low64; }
  };
  ...
  using NodeHashToComponentContainer =
      std::conditional_t<with_ast, std::unordered_map<IASTHash, size_t, ASTHash>, QueryTreeNodePtrWithHashMap<size_t>>;
...
class ComparisonGraph {
 ...
 private:
  ...
  static void dfsComponents(const Graph& reversed_graph, size_t v, OptionalIndices& components, size_t component);
