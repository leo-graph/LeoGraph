...
BlockIO InterpreterParallelWithQuery::execute() {
  ...
  if (max_threads > 1) {
    ...
    thread_pool = std::make_unique<ThreadPool>(CurrentMetrics::ParallelWithQueryThreads, CurrentMetrics::ParallelWithQueryActiveThreads,
                                               CurrentMetrics::ParallelWithQueryScheduledThreads, max_threads);
...
void InterpreterParallelWithQuery::executeSubquery(ASTPtr subquery, ContextMutablePtr subquery_context) {
  ...
  if (!pipeline.completed()) {
    ...
    if (subquery->getQueryKind() == IAST::QueryKind::Select) reason += " (Use UNION to combine select queries)";
...
void InterpreterParallelWithQuery::executeCombinedPipeline() {
  std::lock_guard lock{mutex};
  if (!combined_pipeline.initialized()) return;  /// `combined_pipeline` is empty, skipping
