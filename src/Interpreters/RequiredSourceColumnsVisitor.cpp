...
std::vector<String> RequiredSourceColumnsMatcher::extractNamesFromLambda(const ASTFunction &node) {
  if (node.arguments->children.size() != 2) throw Exception(ErrorCodes::NUMBER_OF_ARGUMENTS_DOESNT_MATCH, "lambda requires two arguments");
  ...
  for (auto &child : lambda_args_tuple->arguments->children) {
    ...
    if (!identifier) throw Exception(ErrorCodes::TYPE_MISMATCH, "lambda argument declarations must be identifiers");
...
bool RequiredSourceColumnsMatcher::needChildVisit(const ASTPtr &node, const ASTPtr &child) {
  if (child->as<ASTSelectQuery>()) return false;
...
void RequiredSourceColumnsMatcher::visit(const ASTSelectQuery &select, const ASTPtr &, Data &data) {
    ...
    auto find_columns = [&data, &select_columns](IAST *function) {
      auto f_impl = [&data, &select_columns](IAST *fn, auto fi) {
        if (auto *ident = fn->as<ASTIdentifier>()) {
          if (!select_columns.contains(ident->getColumnName())) data.addColumnIdentifier(*ident);
          return;
        }
  ...
  for (const auto &node : select.children) {
    ...
    if (node != select.select() && node != select.with()) Visitor(data).visit(node);
  }
...
void RequiredSourceColumnsMatcher::visit(const ASTIdentifier &node, const ASTPtr &, Data &data) {
  ...
  if (node.name().empty()) throw Exception(ErrorCodes::NUMBER_OF_ARGUMENTS_DOESNT_MATCH, "Expected not empty name");

  if (!data.private_aliases.contains(node.name())) data.addColumnIdentifier(node);
}
