...
ReturnType convertTo(ExternalLoader::LoadResults results) {
  if constexpr (std::is_same_v<ReturnType, ExternalLoader::LoadResults>)
    ...
    for (auto &&result : results) {
      if (auto object = std::move(result.object)) objects.push_back(std::move(object));
    }
    return objects;
  }
}
...
struct LoadingGuardForAsyncLoad {
  ...
  LoadingGuardForAsyncLoad(bool async, std::mutex &mutex) {
    if (async) lock = std::unique_lock(mutex);
  }
};
}  // namespace
 ...
 private:
  ...
  void readRepositories(const std::optional<String> &only_repository_name = {}, const std::optional<String> &only_path = {}) {
    for (auto &[repository, repository_info] : repositories) {
      if (only_repository_name && (repository->getName() != *only_repository_name)) continue;

      for (auto &file_info : repository_info.files | boost::adaptors::map_values) file_info.in_use = false;

      Strings existing_paths;
      if (only_path) {
        if (repository->exists(*only_path)) existing_paths.push_back(*only_path);
      } else
      ...
      for (const auto &path : existing_paths) {
        ...
        if (it != repository_info.files.end()) {
          ...
          if (readFileInfo(file_info, *repository, path)) need_collect_object_configs = true;
        } else {
          FileInfo file_info;
      ...
      for (auto &[path, file_info] : repository_info.files) {
        if (file_info.in_use) continue;

        if (only_path && (*only_path != path)) continue;

        deleted_paths.emplace_back(path);
      }

      if (!deleted_paths.empty()) {
        for (const String &deleted_path : deleted_paths) repository_info.files.erase(deleted_path);
        need_collect_object_configs = true;
      }
    }
  }
  ...
  bool readFileInfo(FileInfo &file_info, IExternalLoaderConfigRepository &repository, const String &path) const {
    try {
      ...
      for (const auto &key : keys) {
        ...
        if (object_uuid.empty()) {
          String database;
          if (!settings.external_database.empty()) database = file_contents.getString(key + "." + settings.external_database, "");
          if (!database.empty()) object_name = database + "." + object_name;
        }
  ...
  void collectObjectConfigs() {
    if (!need_collect_object_configs) return;
          ...
          if (already_added_it == new_configs->configs_by_name.end()) {
            ...
            if (!already_added->from_temp_repository && !repository->isTemporary()) {
              if (path == already_added->path && repository->getName() == already_added->repository_name)
                ...
                LOG_WARNING(log, "{} '{}' is found both in file '{}' and '{}'", type_name, object_name, already_added->path, path);
            }
          }
        }
      }
...
class ExternalLoader::LoadingDispatcher : private boost::noncopyable {
 public:
  ...
  void setConfiguration(const ObjectConfigsPtr &new_configs) {
    ...
    if (configs && (configs->counter >= new_configs->counter)) return;
    ...
    for (auto &[name, info] : infos) {
      ...
      if (new_config_it == new_configs->configs_by_name.end()) {
        ...
        bool config_changed =
            !isSameConfiguration(*previous_config->config, previous_config->key_in_config, *new_config->config, new_config->key_in_config);
        if (config_changed) {
          if (info.object)
            external_loader.updateObjectFromConfigWithoutReloading(*info.object, *new_config->config, new_config->key_in_config);

          if (info.triedToLoad()) {
            bool config_change_requires_reloading = external_loader.doesConfigChangeRequiresReloadingObject(
                *previous_config->config, previous_config->key_in_config, *new_config->config, new_config->key_in_config);
  ...
  void enableAlwaysLoadEverything(bool enable) {
    std::lock_guard lock{mutex};
    if (always_load_everything == enable) return;
    ...
    if (enable) {
      ...
      for (auto &[name, info] : infos)
        if (!info.triedToLoad()) startLoading(info);
    }
  }
  ...
  Status getCurrentStatus(const String &name) const {
    ...
    if (!info) return Status::NOT_EXIST;
    return info->status();
  }
  ...
  ReturnType getLoadResult(const String &name) const {
    std::lock_guard lock{mutex};
    const Info *info = getInfo(name);
    if (!info) return notExists<ReturnType>(name);
  ...
  bool hasLoadedObjects() const {
    ...
    for (const auto &name_info : infos)
      if (name_info.second.loaded()) return true;
    return false;
  }
  ...
  Strings getAllTriedToLoadNames() const {
    ...
    for (const auto &[name, info] : infos)
      if (info.triedToLoad()) names.push_back(name);
    return names;
  }
  ...
  void reloadOutdated() {
    ...
    for (auto &[object, should_update_flag] : should_update_map) {
      try {
        ...
        if (!should_update_flag) should_update_flag = object->isModified();
      } catch (...) {
        ...
        if ((now >= info.next_update_time) && !info.isLoading()) {
          if (info.loaded()) {
            ...
            if (it == should_update_map.end())
              continue;  /// Object has been just loaded (it wasn't loaded while we were building the map `should_update_map`), so we don't
                         /// have to reload it right now.
 ...
 private:
  struct Info {
    ...
    Status status() const {
      if (object) return isLoading() ? Status::LOADED_AND_RELOADING : Status::LOADED;
      if (exception) return isLoading() ? Status::FAILED_AND_RELOADING : Status::FAILED;
      return isLoading() ? Status::LOADING : Status::NOT_LOADED;
    }

    Duration loadingDuration() const {
      if (isLoading()) return std::chrono::duration_cast<Duration>(std::chrono::system_clock::now() - loading_start_time);
  ...
  Info *loadImpl(const String &name, Duration timeout, bool forced_to_reload, std::unique_lock<std::mutex> &lock) {
    ...
    if (objects_being_loaded_by_current_thread().contains(name))
      throw Exception(ErrorCodes::DEADLOCK_AVOIDED,
                      ...
                      " and loading it again would cause a deadlock",
                      type_name, name);
    ...
    auto pred = [&] {
      info = getInfo(name);
      if (!info) return true;  /// stop

      if (!min_id) min_id = getMinIDToFinishLoading(forced_to_reload);

      if (info->loading_id < min_id) startLoading(*info, forced_to_reload, *min_id);
  ...
  size_t getMinIDToFinishLoading(bool forced_to_reload) const {
    ...
    if (it != min_id_to_finish_loading_dependencies.end()) return it->second;
  ...
  void startLoading(Info &info, bool forced_to_reload = false, size_t min_id_to_finish_loading_dependencies_ = 1) {
    ...
    LOG_TRACE(log, "Will load the object '{}' {}, force = {}, loading_id = {}", info.name,
              (enable_async_loading ? std::string("in background") : "immediately"), forced_to_reload, info.loading_id);
    ...
    if (enable_async_loading) {
      ...
      try {
        thread = ThreadFromGlobalPool{
            &LoadingDispatcher::doLoading, this, info.name, loading_id, forced_to_reload, min_id_to_finish_loading_dependencies_, true,
  ...
  void doLoading(const String &name, size_t loading_id, bool forced_to_reload, size_t min_id_to_finish_loading_dependencies_, bool async,
    ...
    try {
      ...
      if (forced_to_reload) previous_version_as_base_for_loading = nullptr;  /// Need complete reloading, cannot use the previous version.
  ...
  std::optional<Info> prepareToLoadSingleObject(const String &name, size_t loading_id, size_t min_id_to_finish_loading_dependencies_,
    ...
    if (!info || !info->isLoading() || (info->loading_id != loading_id)) return {};
  ...
  std::pair<LoadableMutablePtr, std::exception_ptr> loadSingleObject(const String &name, const ObjectConfig &config,
  ...
  void saveResultOfLoadingSingleObject(const String &name, size_t loading_id, LoadablePtr previous_version, LoadableMutablePtr new_object,
                                       std::exception_ptr new_exception, size_t error_count, const LoadingGuardForAsyncLoad &) {
    ...
    if (info->loading_id != loading_id) {
      LOG_TRACE(log,
                ...
                name, info->loading_id, loading_id);
      return;
    }

    if (new_exception) {
      auto next_update_time_description = [next_update_time] {
        if (next_update_time == TimePoint::max()) return String();
        return ", next update is scheduled at " + to_string(next_update_time);
      };
      if (previous_version)
        tryLogException(new_exception, log,
    ...
    if (!info->exception) info->last_successful_update_time = current_time;
  ...
  TimePoint calculateNextUpdateTime(const LoadablePtr &loaded_object, size_t error_count) const {
    ...
    if (loaded_object) {
      ...
      if (!error_count) {
        ...
        LOG_TRACE(log, "Supposed update time for '{}' is {} (loaded, lifetime [{}, {}], no errors)", loaded_object->getLoadableName(),
                  to_string(result), lifetime.min_sec, lifetime.max_sec);
        return result;
      }

      auto result = std::chrono::system_clock::now() + std::chrono::seconds(calculateDurationWithBackoff(rnd_engine, error_count));
      LOG_TRACE(log, "Supposed update time for '{}' is {} (backoff, {} errors)", loaded_object->getLoadableName(), to_string(result),
                error_count);
      return result;
    }
...
class ExternalLoader::PeriodicUpdater : private boost::noncopyable {
 ...
 private:
  void doPeriodicUpdates() {
    ...
    SCOPE_EXIT_SAFE({ LOG_DEBUG(log, "Stopped periodic updates (enabled: {})", enabled); });
...
void ExternalLoader::setConfigSettings(const ExternalLoaderConfigSettings &settings) { config_files_reader->setConfigSettings(settings); }

void ExternalLoader::enableAlwaysLoadEverything(bool enable) { loading_dispatcher->enableAlwaysLoadEverything(enable); }

void ExternalLoader::enableAsyncLoading(bool enable) { loading_dispatcher->enableAsyncLoading(enable); }

void ExternalLoader::enablePeriodicUpdates(bool enable_) { periodic_updater->enable(enable_); }

bool ExternalLoader::hasLoadedObjects() const { return loading_dispatcher->hasLoadedObjects(); }

ExternalLoader::Status ExternalLoader::getCurrentStatus(const String &name) const { return loading_dispatcher->getCurrentStatus(name); }
...
ExternalLoader::Loadables ExternalLoader::getLoadedObjects() const { return getLoadResults<Loadables>(); }
...
size_t ExternalLoader::getNumberOfLoadedObjects() const { return loading_dispatcher->getNumberOfLoadedObjects(); }

size_t ExternalLoader::getNumberOfObjects() const { return loading_dispatcher->getNumberOfObjects(); }
...
bool ExternalLoader::has(const String &name) const { return loading_dispatcher->has(name); }

Strings ExternalLoader::getAllTriedToLoadNames() const { return loading_dispatcher->getAllTriedToLoadNames(); }

void ExternalLoader::checkLoaded(const ExternalLoader::LoadResult &result, bool check_no_errors) const {
  if (result.object && (!check_no_errors || !result.exception)) return;
  ...
  if (result.exception) {
    ...
    try {
      ...
      throw DB::Exception(ErrorCodes::DICTIONARIES_WAS_NOT_LOADED, "Failed to load dictionary '{}': {}", result.name,
...
void ExternalLoader::reloadConfig() const { loading_dispatcher->setConfiguration(config_files_reader->read()); }
...
ExternalLoader::LoadableMutablePtr ExternalLoader::createOrCloneObject(const String &name, const ObjectConfig &config,
                                                                       const LoadablePtr &previous_version) const {
  if (previous_version) return previous_version->clone();
