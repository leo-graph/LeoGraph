...
inline bool operator==(CompactStringRef lhs, CompactStringRef rhs) {
  ...
  for (size_t pos = lhs.size - 1; pos < lhs.size; --pos)
    if (lhs_data[pos] != rhs_data[pos]) return false;
...
struct DefaultHash<CompactStringRef> {
  size_t operator()(CompactStringRef x) const { return CityHash_v1_0_2::CityHash64(x.data(), x.size); }
};
...
struct Grower : public HashTableGrower<> {
  ...
  [[noreturn]] static void set(size_t /*num_elems*/) { throw Poco::Exception(__PRETTY_FUNCTION__); }
};
...
int main(int argc, char **argv) {
  ...
  if (!m || m == 1) {
    ...
    watch.stop();
    std::cerr << std::fixed << std::setprecision(2) << "HashMap (CityHash64). Size: " << map.size()
  ...
  if (!m || m == 2) {
    ...
    watch.stop();
    std::cerr << std::fixed << std::setprecision(2) << "HashMap (FastHash64). Size: " << map.size()
              << ", elapsed: " << watch.elapsedSeconds() << " (" << static_cast<double>(n) / watch.elapsedSeconds() << " elem/sec.)"
#ifdef DBMS_HASH_MAP_COUNT_COLLISIONS
              << ", collisions: " << map.getCollisions()
#endif
  ...
  if (!m || m == 3) {
    ...
    watch.stop();
    std::cerr << std::fixed << std::setprecision(2) << "HashMap (CrapWow). Size: " << map.size() << ", elapsed: " << watch.elapsedSeconds()
...
#  endif
  ...
  if (!m || m == 4) {
    ...
    watch.stop();
    std::cerr << std::fixed << std::setprecision(2) << "HashMap (SimpleHash). Size: " << map.size()
              << ", elapsed: " << watch.elapsedSeconds() << " (" << static_cast<double>(n) / watch.elapsedSeconds() << " elem/sec.)"
#ifdef DBMS_HASH_MAP_COUNT_COLLISIONS
              << ", collisions: " << map.getCollisions()
#endif
  ...
  if (!m || m == 5) {
    ...
    for (size_t i = 0; i < n; ++i) ++map[data[i]];
