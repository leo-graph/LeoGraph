...
template <>
inline bool check<SmallStringView>(SmallStringView x) {
  return x.size == 0;
}

template <>
inline void set<SmallStringView>(SmallStringView &x) {
  x.size = 0;
}
}  // namespace ZeroTraits
...
struct DefaultHash<SmallStringView> {
  size_t operator()(SmallStringView x) const { return DefaultHash<std::string_view>()(std::string_view(x.data(), x.size)); }
};
...
int main(int argc, char **argv) {
  ...
  if (!m || m == 1) {
    ...
    watch.stop();
    std::cerr << std::fixed << std::setprecision(2) << "HashMap (std::string_view). Size: " << map.size()
  ...
  if (!m || m == 2) {
    ...
    watch.stop();
    std::cerr << std::fixed << std::setprecision(2) << "HashMap (SmallStringView). Size: " << map.size()
              << ", elapsed: " << watch.elapsedSeconds() << " (" << static_cast<double>(n) / watch.elapsedSeconds() << " elem/sec.)"
#ifdef DBMS_HASH_MAP_COUNT_COLLISIONS
              << ", collisions: " << map.getCollisions()
#endif
