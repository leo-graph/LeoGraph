...
int main(int argc, char** argv) {
  ...
  if (!m || m == 1) bench<StringHashSet<>>(data, pool, "StringHashSet");
  if (!m || m == 2) bench<HashSetWithSavedHash<std::string_view>>(data, pool, "HashSetWithSavedHash");
  if (!m || m == 3) bench<HashSet<std::string_view>>(data, pool, "HashSet");
  return 0;
}
