...
#define DefineStringView(STRUCT)                                                                  \
  ...
  struct DefaultHash<STRUCT> {                                                                    \
    size_t operator()(STRUCT x) const { return CityHash_v1_0_2::CityHash64(x.data(), x.size()); } \
  };

DefineStringView(StringView_CompareMemcmp) DefineStringView(StringView_CompareAlwaysTrue)

    inline bool
    operator==(StringView_CompareMemcmp lhs, StringView_CompareMemcmp rhs) {
...
struct FarmHash64 {
  size_t operator()(std::string_view x) const { return NAMESPACE_FOR_HASH_FUNCTIONS::Hash64(x.data(), x.size()); }
};
