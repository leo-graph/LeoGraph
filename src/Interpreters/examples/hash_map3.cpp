...
int main(int, char **) {
  using Map = HashMapWithDump<std::string_view, UInt64, SimpleHash, Grower,
                              HashTableAllocatorWithStackMemory<4 * sizeof(HashMapCell<std::string_view, UInt64, SimpleHash>)>>;
  ...
  for (auto x : map) std::cerr << x.getKey() << " -> " << x.getMapped() << std::endl;
