...
std::string_view toString(JoinConditionOperator op) {
  switch (op) {
    ...
    case JoinConditionOperator::Or:
      return "OR";
    case JoinConditionOperator::Unknown:
      break;
  }
...
std::optional<size_t> BitSet::getSingleBit() const {
  if (bitset.count() == 1) return bitset.find_first();
  return {};
}
...
JoinExpressionActions::JoinExpressionActions() : data(std::make_shared<Data>(ActionsDAG(), Data::NodeToSourceMapping())) {}
...
JoinExpressionActions::JoinExpressionActions(const Block &left_header, const Block &right_header, ActionsDAG &&actions_dag_) {
  ...
  if (input_nodes.size() != left_header.columns() + right_header.columns())
    throw Exception(ErrorCodes::LOGICAL_ERROR, "Input nodes size mismatch in dag: {}, expected: [{}], [{}]", actions_dag_.dumpDAG(),
                    left_header.dumpNames(), right_header.dumpNames());

  auto left_column_names =
      std::ranges::to<std::unordered_set<std::string_view>>(left_header | std::views::transform(&ColumnWithTypeAndName::name));
  auto right_column_names =
      std::ranges::to<std::unordered_set<std::string_view>>(right_header | std::views::transform(&ColumnWithTypeAndName::name));

  if (std::ranges::any_of(left_column_names, [&right_column_names](const auto &name) { return right_column_names.contains(name); }))
    throw Exception(ErrorCodes::LOGICAL_ERROR, "Left and right columns have same names: [{}], [{}]", left_header.dumpNames(),
                    right_header.dumpNames());

  for (size_t i = 0; i < input_nodes.size(); ++i) {
    BitSet rels;
    if (input_nodes[i]->type != ActionsDAG::ActionType::INPUT)
      throw Exception(ErrorCodes::LOGICAL_ERROR, "Input node {} is not INPUT in dag: {}", i, actions_dag_.dumpDAG());
    ...
    if (left_column_names.contains(column_name))
      ...
      throw Exception(ErrorCodes::LOGICAL_ERROR, "Input node {} from is not in headers: [{}] [{}], dag: {}", column_name,
...
BitSet getExpressionSourcesImpl(std::unordered_map<NodeRawPtr, BitSet> &expression_sources, const JoinActionRef &action) {
  ...
  if (auto it = expression_sources.find(node); it != expression_sources.end()) return it->second;
  ...
  while (!stack.empty()) {
    ...
    if (current->type == ActionsDAG::ActionType::INPUT)
      throw Exception(ErrorCodes::LOGICAL_ERROR, "Unknown input node {} in expression sources", current->result_name);

    if (child_idx >= current->children.size()) {
      BitSet sources;
      for (const auto &child : current->children) sources = sources | expression_sources.at(child);
    ...
    if (!expression_sources.contains(child)) stack.push({child, 0});
  }
  return expression_sources.at(node);
}

std::shared_ptr<ActionsDAG> JoinExpressionActions::getActionsDAG() const { return std::shared_ptr<ActionsDAG>(data, &data->actions_dag); }
...
JoinActionRef::JoinActionRef(NodeRawPtr node_, std::weak_ptr<JoinExpressionActions::Data> data_) : node_ptr(node_), data(data_) {
#ifndef NDEBUG
  /// TODO: once we have map with sources initialized in advance we may do lookup there instead
  if (node_ptr) {
    auto data_ptr = data.lock();
    if (!data_ptr) throw Exception(ErrorCodes::LOGICAL_ERROR, "Cannot create JoinActionRef nullptr data");
    ...
    if (!std::ranges::contains(raw_nodes, node_ptr))
      throw Exception(ErrorCodes::LOGICAL_ERROR, "Cannot create JoinActionRef for node {} not in actions DAG: [{}] <- {}",
...
JoinActionRef::JoinActionRef(NodeRawPtr node_, const JoinExpressionActions &expression_actions_)
  ...
  if (!expression_actions_.data) throw Exception(ErrorCodes::LOGICAL_ERROR, "Cannot create JoinActionRef for empty expression actions");
}

const ActionsDAG::Node *JoinActionRef::getNode() const {
  if (!node_ptr) throw Exception(ErrorCodes::LOGICAL_ERROR, "Cannot get node for nullptr");
  if (data.expired()) throw Exception(ErrorCodes::LOGICAL_ERROR, "Pointer to actions DAG is expired");
  return node_ptr;
}
...
const String &JoinActionRef::getColumnName() const { return getNode()->result_name; }

DataTypePtr JoinActionRef::getType() const { return getNode()->result_type; }
...
JoinActionRef JoinExpressionActions::findNode(const String &column_name, bool is_input, bool throw_if_not_found) const {
  ...
  for (const auto &node : nodes)
    if (node->result_name == column_name) return JoinActionRef(node, data);
  if (throw_if_not_found)
    throw Exception(ErrorCodes::NOT_FOUND_COLUMN_IN_BLOCK, "Cannot find column {} in actions DAG {}:\n{}", column_name,
                    is_input ? "input" : "output", data->actions_dag.dumpDAG());
...
ActionsDAG JoinExpressionActions::getSubDAG(JoinActionRef action) { return getSubDAG(std::views::single(action)); }
...
JoinExpressionActions JoinExpressionActions::clone(ActionsDAG::NodeMapping &node_map) const {
  ...
  for (const auto &[node, source] : data->expression_sources) {
    ...
    if (it == node_map.end()) throw Exception(ErrorCodes::LOGICAL_ERROR, "Cannot find node {} in node map", node->result_name);
...
BitSet JoinActionRef::getSourceRelations() const { return getExpressionSourcesImpl(getData()->expression_sources, *this); }
...
void JoinActionRef::setSourceRelations(const BitSet &source_relations) const {
  ...
  while (!stack.empty()) {
    ...
    stack.pop();
    if (expression_sources[current] == source_relations) break;
...
std::string operatorToFunctionName(JoinConditionOperator op) {
  switch (op) {
    case JoinConditionOperator::And:
      return FunctionAnd::name;
    case JoinConditionOperator::Or:
      return FunctionOr::name;
    case JoinConditionOperator::NullSafeEquals:
      return FunctionIsNotDistinctFrom::name;
    case JoinConditionOperator::Equals:
      return NameEquals::name;
    case JoinConditionOperator::Less:
      return NameLess::name;
    case JoinConditionOperator::LessOrEquals:
      return NameLessOrEquals::name;
    case JoinConditionOperator::Greater:
      return NameGreater::name;
    case JoinConditionOperator::GreaterOrEquals:
      return NameGreaterOrEquals::name;
    case JoinConditionOperator::Unknown:
      break;
  }
  throw Exception(ErrorCodes::LOGICAL_ERROR, "Illegal value for JoinConditionOperator: {}", static_cast<Int32>(op));
...
JoinConditionOperator functionNameToOperator(std::string_view name) {
  ...
  for (UnderlyingType i = 0; i < static_cast<UnderlyingType>(JoinConditionOperator::Unknown); ++i) {
    if (operatorToFunctionName(static_cast<JoinConditionOperator>(i)) == name) return static_cast<JoinConditionOperator>(i);
  }
...
bool JoinActionRef::isFunction(JoinConditionOperator op) const {
  const auto *node = getNode();
  if (node->type != ActionsDAG::ActionType::FUNCTION) return false;
...
bool JoinActionRef::fromNone() const { return getSourceRelations().none(); }
...
std::tuple<JoinConditionOperator, JoinActionRef, JoinActionRef> JoinActionRef::asBinaryPredicate() const {
  ...
  if (op == JoinConditionOperator::Unknown) return {JoinConditionOperator::Unknown, nullptr, nullptr};
...
std::vector<JoinActionRef> JoinActionRef::getArguments(bool recursive) const {
  ...
  auto data_ptr = getData();
  for (const auto &child : node->children) arguments.emplace_back(child, data_ptr);
  return arguments;
}

std::shared_ptr<JoinExpressionActions::Data> JoinActionRef::getData() const {
  auto data_ptr = data.lock();
  if (!data_ptr) throw Exception(ErrorCodes::LOGICAL_ERROR, "Cannot get data for JoinActionRef");
  return data_ptr;
}

std::shared_ptr<JoinExpressionActions::Data> JoinActionRef::getData(const std::vector<JoinActionRef> &actions) {
  if (actions.empty()) throw Exception(ErrorCodes::LOGICAL_ERROR, "Cannot get actions DAG for empty actions");

  auto data_ptr = actions.front().getData();
  for (const auto &action : actions) {
    if (data_ptr.get() != action.getData().get()) throw Exception(ErrorCodes::LOGICAL_ERROR, "All actions must have the same actions DAG");
  }
  return data_ptr;
}

ActionsDAG &JoinActionRef::getActionsDAG(JoinExpressionActions::Data &data_) { return data_.actions_dag; }
...
JoinActionRef::AddFunction::AddFunction(std::shared_ptr<IFunction> function_)
    : AddFunction(std::make_shared<FunctionToOverloadResolverAdaptor>(function_)) {}
