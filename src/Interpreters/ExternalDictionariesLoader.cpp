...
ExternalDictionariesLoader::ExternalDictionariesLoader(ContextPtr global_context_)
    : ExternalLoader("external dictionary", getLogger("ExternalDictionariesLoader")), WithContext(global_context_) {
  setConfigSettings({"dictionary", "name", "database", "uuid"});
  enableAsyncLoading(true);
  if (getContext()->getApplicationType() == Context::ApplicationType::SERVER) enablePeriodicUpdates(true);
}

ExternalLoader::LoadableMutablePtr ExternalDictionariesLoader::createObject(const std::string &name,
...
bool ExternalDictionariesLoader::doesConfigChangeRequiresReloadingObject(const Poco::Util::AbstractConfiguration &old_config,
...
void ExternalDictionariesLoader::updateObjectFromConfigWithoutReloading(IExternalLoadable &object,
                                                                        const Poco::Util::AbstractConfiguration &config,
                                                                        const String &key_in_config) const {
...
ExternalDictionariesLoader::DictPtr ExternalDictionariesLoader::getDictionary(const std::string &dictionary_name,
  ...
  if (local_context->hasQueryContext()) process_list_element = local_context->getProcessListElement();
...
ExternalDictionariesLoader::DictPtr ExternalDictionariesLoader::tryGetDictionary(const std::string &dictionary_name,
                                                                                 ContextPtr local_context) const {
  std::string resolved_dictionary_name = resolveDictionaryName(dictionary_name, local_context->getCurrentDatabase());
...
DictionaryStructure ExternalDictionariesLoader::getDictionaryStructure(const std::string &dictionary_name, ContextPtr query_context) const {
  ...
  if (!load_result.config) throw Exception(ErrorCodes::BAD_ARGUMENTS, "Dictionary {} config not found", backQuote(dictionary_name));
...
QualifiedTableName ExternalDictionariesLoader::qualifyDictionaryNameWithDatabase(const std::string &dictionary_name,
  ...
  if (qualified_name->database.empty() && !has(qualified_name->table)) {
    ...
    if (has(resolved_name)) qualified_name->database = std::move(current_database_name);
  }

  return *qualified_name;
}

std::string ExternalDictionariesLoader::resolveDictionaryName(const std::string &dictionary_name,
                                                              const std::string &current_database_name) const {
  if (has(dictionary_name)) return dictionary_name;

  std::string resolved_name = resolveDictionaryNameFromDatabaseCatalog(dictionary_name, current_database_name);

  if (has(resolved_name)) return resolved_name;

  throw Exception(ErrorCodes::BAD_ARGUMENTS, "Dictionary ({}) not found", backQuote(dictionary_name));
}

std::string ExternalDictionariesLoader::resolveDictionaryNameFromDatabaseCatalog(const std::string &name,
                                                                                 const std::string &current_database_name) const {
  ...
  if (qualified_name->database.empty()) {
    ...
    if (is_xml_dictionary) return res;

    qualified_name->database = current_database_name;
    res = current_database_name + '.' + name;
  }

  auto [db, table] = DatabaseCatalog::instance().tryGetDatabaseAndTable({qualified_name->database, qualified_name->table},
                                                                        const_pointer_cast<Context>(getContext()));

  if (!db) return res;
  assert(table);

  if (db->getUUID() == UUIDHelpers::Nil) return res;
  if (table->getName() != "Dictionary") return res;

  return toString(table->getStorageID().uuid);
}

DictionaryStructure ExternalDictionariesLoader::getDictionaryStructure(const Poco::Util::AbstractConfiguration &config,
                                                                       const std::string &key_in_config) {
...
void ExternalDictionariesLoader::reloadAllTriedToLoadInOrder() const {
  ...
  for (const auto &name : all_names) in_degree[name] = 0;
  ...
  for (const auto &info : dict_infos) {
    ...
    try {
      ...
      LOG_WARNING(getLogger("ExternalDictionariesLoader"),
                  "Failed to parse source config for dictionary '{}', skipping dependency resolution: {}", info.loader_name,
                  getCurrentExceptionMessage(false));
      continue;
    }

    if (!source_info || !source_info->is_local || source_info->table_name.table.empty()) continue;
    ...
    if (source_info->table_name.database.empty() && !info.database.empty()) {
      ...
      if (it != qualified_name_to_loader.end()) dep_name = it->second;
    }
  ...
  for (const auto &name : all_names) {
    if (in_degree[name] == 0) current_level.push_back(name);
  }

  std::unordered_set<String> processed;
  while (!current_level.empty()) {
    levels.push_back(current_level);
    for (const auto &name : current_level) processed.insert(name);
    ...
    for (const auto &name : current_level) {
      ...
      for (const auto &dep : it->second) {
        if (--in_degree[dep] == 0) next_level.push_back(dep);
      }
    }
  ...
  Strings circular;
  for (const auto &name : all_names) {
    if (!processed.contains(name)) circular.push_back(name);
  }
