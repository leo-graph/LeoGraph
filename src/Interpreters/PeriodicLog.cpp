...
void PeriodicLog<LogElement>::stopCollect() {
  bool old_val = false;
  if (!is_shutdown_metric_thread.compare_exchange_strong(old_val, true)) return;
  if (collecting_thread) collecting_thread->join();
}
...
void PeriodicLog<LogElement>::threadFunction() {
  ...
  while (!is_shutdown_metric_thread) {
    try {
      ...
      while (desired_timepoint <= current_time) desired_timepoint += std::chrono::milliseconds(collect_interval_milliseconds);
