  ...
  static void visit(const ASTFunction& node, const ASTPtr& ast, Data& data) {
    if (isAggregateFunction(node)) {
      if (data.assert_no_aggregates)
        throw Exception(ErrorCodes::ILLEGAL_AGGREGATION, "Aggregate function {} is found {} in query", node.getColumnName(),
                        String(data.assert_no_aggregates));

      String column_name = node.getColumnName();
      if (data.uniq_names.contains(column_name)) return;

      data.uniq_names.insert(column_name);
      data.aggregates.push_back(ast);
    } else if (node.isWindowFunction()) {
      if (data.assert_no_windows)
        throw Exception(ErrorCodes::ILLEGAL_AGGREGATION, "Window function {} is found {} in query", node.getColumnName(),
                        String(data.assert_no_windows));

      if (node.name == "lag" || node.name == "lead") {
        throw Exception(ErrorCodes::NOT_IMPLEMENTED, "Window function '{}' is supported only with enabled analyzer",
