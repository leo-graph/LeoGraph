...
class GroupByFunctionKeysMatcher {
 public:
  ...
  static void visit(ASTFunction *function_node, Data &data) {
    ...
    if (!keep_key) (data.key_names_to_keep).erase(function_node->getColumnName());
  }

  static void visit(const ASTPtr &ast, Data &data) {
    if (auto *function_node = ast->as<ASTFunction>()) {
      if (function_node->arguments && !function_node->arguments->children.empty()) visit(function_node, data);
    }
  }
};
