...
class ArrayJoinedColumnsMatcher {
 public:
  ...
  static bool needChildVisit(ASTPtr &node, const ASTPtr &child) {
    if (node->as<ASTTablesInSelectQuery>()) return false;
  ...
  static void visit(ASTPtr &ast, Data &data) {
    ...
    if (const auto *t = ast->as<ASTSelectQuery>()) visit(*t, ast, data);
  }
 ...
 private:
  static void visit(const ASTSelectQuery &node, ASTPtr &, Data &data) {
    ...
    if (!array_join_expression_list) throw Exception(ErrorCodes::LOGICAL_ERROR, "No ARRAY JOIN");
    ...
    for (ASTPtr &ast : array_join_expression_list->children) {
      ...
      for (ASTPtr &child2 : ast->children) out.emplace_back(&child2);
    }
