...
OutputBlockColumns prepareOutputBlockColumns(const Aggregator::Params &params,
                                             const Aggregator::AggregateFunctionsPlainPtrs &aggregate_functions, const Block &res_header,
                                             Arenas &aggregates_pools, bool final, size_t rows) {
  ...
  for (size_t i = 0; i < params.aggregates_size; ++i) {
    if (!final) {
      ...
      for (auto &pool : aggregates_pools) column_aggregate_func.addArena(pool);
  ...
  if (key_columns.size() != params.keys_size) throw Exception(ErrorCodes::LOGICAL_ERROR, "Aggregate. Unexpected key columns size.");

  std::vector<IColumn *> raw_key_columns;
  raw_key_columns.reserve(key_columns.size());
  for (auto &column : key_columns) raw_key_columns.push_back(column.get());
...
Block finalizeBlock(const Aggregator::Params &params, const Block &res_header, OutputBlockColumns &&out_cols, bool final, size_t rows) {
  ...
  for (size_t i = 0; i < params.keys_size; ++i) res.getByPosition(i).column = std::move(key_columns[i]);
  ...
  for (size_t i = 0; i < columns; ++i)
    if (isColumnConst(*res.getByPosition(i).column)) res.getByPosition(i).column = res.getByPosition(i).column->cut(0, rows);
