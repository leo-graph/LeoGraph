...
INSTANTIATE_TEST_SUITE_P(
    ...
    ::testing::ValuesIn(std::initializer_list<TableOverrideTestCase>{
        {"CREATE DATABASE db", "CREATE TABLE db.t (id Int64) ENGINE=Log", "CREATE TABLE db.t (`id` Int64) ENGINE = Log"},
        {"CREATE DATABASE db TABLE OVERRIDE t (PARTITION BY tuple())", "CREATE TABLE db.t (id Int64) ENGINE=MergeTree",
        ...
        {"CREATE DATABASE db TABLE OVERRIDE a (PARTITION BY modulo(id, 3)), TABLE OVERRIDE b (PARTITION BY modulo(id, 5))",
         "CREATE TABLE db.a (id Int64) ENGINE=MergeTree", "CREATE TABLE db.a (`id` Int64) ENGINE = MergeTree PARTITION BY modulo(id, 3)"},
        {"CREATE DATABASE db TABLE OVERRIDE a (PARTITION BY modulo(id, 3)), TABLE OVERRIDE b (PARTITION BY modulo(id, 5))",
         "CREATE TABLE db.b (id Int64) ENGINE=MergeTree", "CREATE TABLE db.b (`id` Int64) ENGINE = MergeTree PARTITION BY modulo(id, 5)"},
        {
