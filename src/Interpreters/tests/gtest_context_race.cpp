...
void run(Ptr context) {
  for (size_t i = 0; i < 100; ++i) {
    ...
    std::thread t2([context] { Context::createCopy(context); });
...
TEST(Context, GetAccessRace) {
  ...
  for (size_t i = 0; i < num_reader_threads; ++i) {
    readers.emplace_back([&context, &stop] {
      while (!stop.load(std::memory_order_relaxed)) context->getAccess();
    });
  }
  ...
  std::thread writer([&context, &stop] {
    for (size_t i = 0; i < num_iterations; ++i) context->setSetting("allow_ddl", Field(UInt64(1)));
