...
void download(const std::string &cache_base_path, DB::FileSegment &file_segment) {
  ...
  if (!fs::exists(subdir)) fs::create_directories(subdir);
...
void assertEqual(const FileSegmentsHolderPtr &file_segments, const Ranges &expected_ranges, const States &expected_states = {}) {
  std::cerr << "\nFile segments: ";
  for (const auto &file_segment : *file_segments) std::cerr << file_segment->range().toString() << ", ";
  std::cerr << "\nExpected: ";
  for (const auto &r : expected_ranges) std::cerr << r.toString() << ", ";

  ASSERT_EQ(file_segments->size(), expected_ranges.size());

  if (!expected_states.empty()) ASSERT_EQ(file_segments->size(), expected_states.size());
...
FileSegmentPtr get(const HolderPtr &holder, int i) {
  ...
  if (it == holder->end()) std::terminate();
  return *it;
}
...
void increasePriority(const HolderPtr &holder) {
  for (auto &it : *holder) it->increasePriority();
}
...
class FileCacheTest : public ::testing::Test {
 public:
  ...
  void SetUp() override {
    ...
    if (fs::exists(cache_base_path)) fs::remove_all(cache_base_path);
    if (fs::exists(cache_base_path2)) fs::remove_all(cache_base_path2);
...
TEST_F(FileCacheTest, LRUPolicy) {
  ...
  const auto &user = FileCache::getCommonOrigin();
  {
    ...
    auto get_or_set = [&](size_t offset, size_t size) { return cache.getOrSet(key, offset, size, file_size, {}, 0, user); };
    ...
    std::cerr << "Step 5\n";
    {
      ...
      assertEqual(holder, {Range(0, 9), Range(10, 14), Range(15, 16), Range(17, 20), Range(21, 23), Range(24, 26)},
      ...
      assertEqual(holder, {Range(0, 9), Range(10, 14), Range(15, 16), Range(17, 20), Range(21, 23), Range(24, 26)},
                  {State::DOWNLOADED, State::DOWNLOADED, State::DOWNLOADED, State::DOWNLOADED, State::PARTIALLY_DOWNLOADED_NO_CONTINUATION,
                   State::DOWNLOADED});
...

    {
      ...
      assertEqual(holder, {Range(10, 14), Range(15, 16), Range(17, 20), Range(21, 21)},
    ...
    std::cerr << "Step 7\n";
    {
      ...
      assertEqual(holder, {Range(23, 23), Range(24, 26), Range(27, 27)}, {State::EMPTY, State::DOWNLOADED, State::EMPTY});
    ...
    std::cerr << "Step 8\n";
    {
      ...
      assertEqual(holder6,
                  {Range(0, 1), Range(2, 4), Range(5, 22), Range(23, 23), Range(24, 26), Range(27, 27), Range(28, 29), Range(30, 31),
                   Range(32, 39)},
                  {State::EMPTY, State::DOWNLOADED, State::EMPTY, State::DOWNLOADED, State::DOWNLOADED, State::DOWNLOADED, State::EMPTY,
                   State::DOWNLOADED, State::EMPTY});
...

    {
      ...
      assertEqual(holder, {Range(24, 26), Range(27, 27), Range(28, 29)}, {State::DOWNLOADED, State::DOWNLOADED, State::EMPTY});
      ...
      std::thread other_1([&] {
        ...
        assertEqual(holder2, {Range(24, 26), Range(27, 27), Range(28, 29)}, {State::DOWNLOADED, State::DOWNLOADED, State::DOWNLOADING});
    ...
    std::cerr << "Step 10\n";
    {
      ...
      assertEqual(holder, {Range(2, 4), Range(5, 23), Range(24, 26)}, {State::DOWNLOADED, State::EMPTY, State::DOWNLOADED});
      ...
      std::thread other_1([&] {
        ...
        assertEqual(holder, {Range(2, 4), Range(5, 23), Range(24, 26)}, {State::DOWNLOADED, State::DOWNLOADING, State::DOWNLOADED});
  ...
  std::cerr << "Step 11\n";
  {
    ...
    /// Get [2, 29]
    assertEqual(cache2.getOrSet(key, 2, 28, file_size, {}, 0, user),
  ...
  std::cerr << "Step 12\n";
  {
    ...
    /// Get [0, 24]
    assertEqual(cache2.getOrSet(key, 0, 25, file_size, {}, 0, user), {Range(0, 9), Range(10, 19), Range(20, 24)},
...
TEST_F(FileCacheTest, writeBuffer) {
  ...
  auto write_to_cache = [&, this](const String &key, const Strings &data, bool flush, ReadBufferPtr *out_read_buffer = nullptr) {
    ...
    out.finalize();
    if (out_read_buffer) *out_read_buffer = out.tryGetReadBuffer();
    return holder;
  };
...
static Block generateBlock(size_t size = 0) {
...

  {
    ...
    for (size_t i = 0; i < size; ++i) mut_col->insert(i);
...
TEST_F(FileCacheTest, CachedReadBuffer) {
  ...
  auto read_buffer_creator = [&]() { return createReadBufferFromFileBase(file_path, read_settings, std::nullopt, std::nullopt); };
  ...
  {
    auto cached_buffer = std::make_shared<CachedOnDiskReadBufferFromFile>(file_path, key, cache, user, read_buffer_creator, read_settings,
...
TEST_F(FileCacheTest, SLRUPolicy) {
...

  {
    ...
    auto read_and_check = [&](const std::string &file, const FileCacheKey &key, const std::string &expect_result) {
      auto read_buffer_creator = [&]() { return createReadBufferFromFileBase(file, read_settings, std::nullopt, std::nullopt); };
...
TEST_F(FileCacheTest, ContinueEvictionPos) {
  ...
  auto add_file_segment = [&](size_t offset, size_t size) {
    ...
    auto file_segment = std::make_shared<FileSegment>(key, offset, size, FileSegment::State::DOWNLOADED, CreateFileSegmentSettings{}, false,
                                                      nullptr, key_metadata, it);
  ...
  priority.collectCandidatesForEviction(*eviction_info, stat, *evicted, invalidated_entries, nullptr, true, 0, false, origin, cache_guard,
                                        state_guard);
  eviction_info.reset();
  ...
  auto get_file_segment = [&](size_t offset) { return LockedKey(key_metadata).getByOffset(offset)->file_segment; };
