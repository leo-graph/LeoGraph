...
void executeCommandsAndThrowIfError(std::vector<std::function<void()>> commands) {
  ...
  for (auto &command : commands) {
    try {
      ...
      if (result.code == 0) result.code = current_result.code;
  ...
  if (result.code != 0) throw Exception::createDeprecated(result.message, result.code);
}

AccessType getRequiredAccessType(StorageActionBlockType action_type) {
  if (action_type == ActionLocks::PartsMerge) return AccessType::SYSTEM_MERGES;
  if (action_type == ActionLocks::PartsFetch) return AccessType::SYSTEM_FETCHES;
  if (action_type == ActionLocks::PartsSend) return AccessType::SYSTEM_REPLICATED_SENDS;
  if (action_type == ActionLocks::ReplicationQueue) return AccessType::SYSTEM_REPLICATION_QUEUES;
  if (action_type == ActionLocks::DistributedSend) return AccessType::SYSTEM_DISTRIBUTED_SENDS;
  if (action_type == ActionLocks::PartsTTLMerge) return AccessType::SYSTEM_TTL_MERGES;
  if (action_type == ActionLocks::PartsMove) return AccessType::SYSTEM_MOVES;
  if (action_type == ActionLocks::PullReplicationLog) return AccessType::SYSTEM_PULLING_REPLICATION_LOG;
  if (action_type == ActionLocks::Cleanup) return AccessType::SYSTEM_CLEANUP;
  if (action_type == ActionLocks::ViewRefresh) return AccessType::SYSTEM_VIEWS;
...
void InterpreterSystemQuery::startStopActionInDatabase(StorageActionBlockType action_type, bool start, const String &database_name,
                                                       const DatabasePtr &database, const ContextPtr &local_context, LoggerPtr log) {
...
BlockIO InterpreterSystemQuery::execute() {
  ...
  if (query.type == Type::RELOAD_DICTIONARY) {
    if (query.database) query.setTable(query.getDatabase() + "." + query.getTable());
  } else if (query.table) {
  ...
  switch (query.type) {
    case Type::SHUTDOWN: {
      ...
      if (kill(0, SIGTERM)) throw ErrnoException(ErrorCodes::CANNOT_KILL, "System call kill(0, SIGTERM) failed");
      break;
    }
    case Type::KILL: {
      getContext()->checkAccess(AccessType::SYSTEM_SHUTDOWN);
    ...
    case Type::SUSPEND: {
      ...
      if (!out.str().empty()) LOG_DEBUG(log, "The command {} returned output: {}", command, out.str());
      res->wait();
      break;
    }
    ...
    case Type::CLEAR_FILESYSTEM_CACHE: {
      ...
      if (query.filesystem_cache_name.empty()) {
        for (const auto &cache_data : FileCacheFactory::instance().getUniqueInstances()) {
          if (!cache_data->cache->isInitialized()) continue;
    ...
    case Type::SYNC_FILESYSTEM_CACHE: {
      ...
      Block sample_block;
      for (const auto &column : columns) sample_block.insert({column.type->createColumn(), column.type, column.name});
      ...
      auto fill_data = [&](const std::string &cache_name, const FileCachePtr &cache, const std::vector<FileSegment::Info> &file_segments) {
        for (const auto &file_segment : file_segments) {
          size_t i = 0;
          const auto path = cache->getFileSegmentPath(file_segment.key, file_segment.offset, file_segment.kind, file_segment.origin);
      ...
      auto source = std::make_shared<SourceFromSingleChunk>(std::make_shared<const Block>(std::move(sample_block)),
                                                            Chunk(std::move(res_columns), num_rows));
    ...
    case Type::CLEAR_DISK_METADATA_CACHE: {
      ...
      if (metadata) metadata->dropCache();
    ...
    case Type::CLEAR_SCHEMA_CACHE: {
      ...
      if (caches_to_drop.contains("FILE")) StorageFile::getSchemaCache(getContext()).clear();
#if USE_AWS_S3
      if (caches_to_drop.contains("S3")) StorageObjectStorage::getSchemaCache(getContext(), StorageS3Configuration::type_name).clear();
#endif
#if USE_HDFS
      if (caches_to_drop.contains("HDFS")) StorageObjectStorage::getSchemaCache(getContext(), StorageHDFSConfiguration::type_name).clear();
#endif
      if (caches_to_drop.contains("URL")) StorageURL::getSchemaCache(getContext()).clear();
...
#if USE_PROTOBUF
      if (caches_to_drop.contains("Protobuf")) ProtobufSchemas::instance().clear();
#endif
    ...
    case Type::RELOAD_DICTIONARIES: {
      getContext()->checkAccess(AccessType::SYSTEM_RELOAD_DICTIONARY);
      executeCommandsAndThrowIfError({[&] { system_context->getExternalDictionariesLoader().reloadAllTriedToLoadInOrder(); },
    ...
    case Type::RELOAD_CONFIG: {
      if (system_context->getApplicationType() == Context::ApplicationType::LOCAL)
        throw Exception::createDeprecated("SYSTEM RELOAD CONFIG query is not supported in clickhouse-local",
    ...
    case Type::RELOAD_ASYNCHRONOUS_METRICS: {
      ...
      if (asynchronous_metrics) asynchronous_metrics->update(std::chrono::system_clock::now(), /*force_update*/ true);
      break;
    }
    ...
    case Type::START_REPLICATED_VIEW:
      for (const auto &task : getRefreshTasks()) task->startReplicated();
      break;
    case Type::STOP_REPLICATED_VIEW:
      for (const auto &task : getRefreshTasks()) task->stopReplicated("SYSTEM STOP REPLICATED VIEW");
      break;
    case Type::REFRESH_VIEW:
      for (const auto &task : getRefreshTasks()) task->run();
      break;
    case Type::WAIT_VIEW:
      for (const auto &task : getRefreshTasks()) task->wait();
      break;
    case Type::CANCEL_VIEW:
      for (const auto &task : getRefreshTasks()) task->cancel();
      break;
    case Type::TEST_VIEW:
      for (const auto &task : getRefreshTasks()) task->setFakeTime(query.fake_time_for_view);
      break;
    ...
    case Type::FLUSH_ASYNC_INSERT_QUEUE: {
      ...
      if (!queue) throw Exception(ErrorCodes::BAD_ARGUMENTS, "Cannot flush asynchronous insert queue because it is not initialized");
    ...
    case Type::START_THREAD_FUZZER:
      ...
      CannotAllocateThreadFaultInjector::setFaultProbability(
          getContext()->getServerSettings()[ServerSetting::cannot_allocate_thread_fault_injection_probability]);
      break;
    case Type::UNFREEZE: {
    ...
    case Type::ALLOCATE_MEMORY: {
      ...
      if (!holder) throw Exception(ErrorCodes::BAD_ARGUMENTS, "SYSTEM ALLOCATE MEMORY is not enabled");
    ...
    case Type::JEMALLOC_FLUSH_PROFILE: {
      ...
      SharedHeader header =
          std::make_shared<Block>(Block{ColumnWithTypeAndName(ColumnString::create(), std::make_shared<DataTypeString>(), "filename")});
...
void InterpreterSystemQuery::restoreReplica() {
  ...
  table_replicated_ptr->restoreMetadataInZooKeeper(
      ZooKeeperRetriesInfo{settings[Setting::keeper_max_retries], settings[Setting::keeper_retry_initial_backoff_ms],
                           settings[Setting::keeper_retry_max_backoff_ms], getContext()->getProcessListElementSafe()},
      false);
}
...
StoragePtr InterpreterSystemQuery::doRestartReplica(const StorageID &replica, ContextMutablePtr system_context, bool throw_on_error) {
  ...
  if (!table) {
    if (throw_on_error) throw Exception(*exception);
  ...
  if (replica_table_id.table_name != replica.getTableName()) {
    if (throw_on_error)
      throw Exception(ErrorCodes::BAD_ARGUMENTS, "Table {} was concurrently renamed to {} during RESTART REPLICA", replica.getNameForLogs(),
                      replica_table_id.getNameForLogs());
  ...
  if (!dynamic_cast<const StorageReplicatedMergeTree *>(table.get())) {
    if (throw_on_error) throw Exception(ErrorCodes::BAD_ARGUMENTS, table_is_not_replicated.data(), replica.getNameForLogs());
    LOG_WARNING(getLogger("InterpreterSystemQuery"), "Cannot RESTART REPLICA {}: not a ReplicatedMergeTree anymore",
                replica.getNameForLogs());
    return nullptr;
  }

  SCOPE_EXIT({
    if (table) table->is_being_restarted = false;
  });
  ...
  if (auto *replicated_db = typeid_cast<DatabaseReplicated *>(database.get())) restart_guard.emplace(*replicated_db);
  ...
  auto columns =
      InterpreterCreateQuery::getColumnsDescription(*create.columns_list->columns, system_context, LoadingStrictnessLevel::ATTACH);
  ...
  while (true) {
    try {
      new_table = StorageFactory::instance().get(create, data_path, system_context, system_context->getGlobalContext(), columns,
                                                 constraints, LoadingStrictnessLevel::ATTACH);
      ...
      tryLogCurrentException(getLogger("InterpreterSystemQuery"),
      ...
      if (auto process_list_element = getContext()->getProcessListElementSafe()) process_list_element->checkTimeLimit();

      sleepForSeconds(1);
    } catch (...) {
      if (++non_zk_retries > max_non_zk_retries) throw;

      tryLogCurrentException(getLogger("InterpreterSystemQuery"),
                             fmt::format("Failed to restart replica {} (attempt {}/{}), will retry", replica_table_id.getNameForLogs(),
                                         non_zk_retries, max_non_zk_retries));
  ...
  if (new_table->getStorageID().uuid != replica_table_id.uuid)
    throw Exception(ErrorCodes::LOGICAL_ERROR, "Tables UUID does not match after RESTART REPLICA (old: {}, new: {})", replica_table_id.uuid,
                    new_table->getStorageID().uuid);
...
void InterpreterSystemQuery::restartReplicas(ContextMutablePtr system_context) {
  ...
  for (auto &elem : catalog.getDatabases(GetDatabasesOptions{.with_datalake_catalogs = false})) {
    if (elem.second->isExternal()) continue;
  ...
  ThreadPool pool(CurrentMetrics::RestartReplicaThreads, CurrentMetrics::RestartReplicaThreadsActive,
                  CurrentMetrics::RestartReplicaThreadsScheduled, threads);
...
void InterpreterSystemQuery::dropReplica(ASTSystemQuery &query) {
  ...
  if (query.replica.empty()) throw Exception(ErrorCodes::BAD_ARGUMENTS, "Replica name is empty");
  ...
  if (!table_id.empty()) {
    ...
    if (!required_access.empty())
      throw Exception(ErrorCodes::ACCESS_DENIED, "Access denied for {}. Not enough permissions to drop these databases: {}",
                      "SYSTEM DROP REPLICA", fmt::join(required_access, ", "));
    ...
    bool looks_like_table_path =
        zookeeper->exists(query.replica_zk_path + "/replicas") || zookeeper->exists(query.replica_zk_path + "/dropped");
    if (!looks_like_table_path)
      throw Exception(ErrorCodes::TABLE_WAS_NOT_DROPPED, "Specified path {} does not look like a table path", query.replica_zk_path);
...
bool InterpreterSystemQuery::dropStorageReplica(const String &query_replica, const StoragePtr &storage) {
  ...
  if (!storage_replicated) return false;
...
DatabasePtr InterpreterSystemQuery::restoreDatabaseFromKeeperPath(const String &zookeeper_name, const String &zookeeper_path,
                                                                  const String &full_replica_name, const String &restoring_database_name) {
  ...
  for (const auto &table : escaped_table_names) paths_to_fetch.push_back(zookeeper_path + "/metadata/" + table);
  ...
  for (size_t i = 0; i < escaped_table_names.size(); ++i) {
    ...
    if (res.error != Coordination::Error::ZOK) throw zkutil::KeeperException::fromPath(res.error, fs::path(zookeeper_path) / "metadata");
  ...
  for (const auto &[table_name, create_table_query] : table_name_to_metadata) {
    ...
    auto query_ast = DatabaseReplicated::parseQueryFromMetadataInZooKeeper(getContext(),
                                                                           /*database_name_=*/restoring_database_name,
                                                                           /*zookeeper_path_=*/zookeeper_path,
                                                                           /*node_name=*/table_name,
                                                                           /*query=*/create_table_query);
    LOG_TEST(log, "table_name {}, create_table_query: {}, query_ast: {}", table_name, create_table_query, query_ast->formatForLogging());
    ...
    for (const auto &storage_id : tables_to_create) {
      ...
      if (metadata_it == table_name_to_metadata.end()) {
        ...
        LOG_WARNING(log,
                    ...
                    storage_id.getNameForLogs(), restoring_database_name);
        continue;
      }
...
std::optional<String> InterpreterSystemQuery::getDetachedDatabaseFromKeeperPath(const ASTSystemQuery &query_) {
  ...
  for (const auto it = default_db_disk->iterateDirectory(metadata_dir_path); it->isValid(); it->next()) {
    ...
    if (!default_db_disk->existsFile(sub_path)) continue;

    String db_name = sub_path.filename().string();
    if (sub_path.extension() == ".sql") db_name = sub_path.stem();

    auto buf = default_db_disk->readFile(sub_path, getContext()->getReadSettings());
    std::string query;
    readStringUntilEOF(query, *buf);
    ParserCreateQuery parser;
    auto ast = parseQuery(parser, query, "", 0, getContext()->getSettingsRef()[Setting::max_parser_depth],
    ...
    if (!engine_define) continue;
    const ASTFunction *engine = engine_define->engine;
    if (!engine || engine->name != "Replicated") continue;

    auto &arguments = engine->arguments->children;
    if (!engine->arguments || engine->arguments->children.size() != 3) continue;
    ...
    if (engine_zookeeper_path != query_.replica_zk_path) continue;

    String full_replica_name = query_.shard.empty() ? query_.replica : DatabaseReplicated::getFullReplicaName(query_.shard, query_.replica);
    if (DatabaseReplicated::getFullReplicaName(shard_name, replica_name) != full_replica_name) continue;
...
void InterpreterSystemQuery::dropDatabaseReplica(ASTSystemQuery &query) {
  ...
  const String full_replica_name = query.shard.empty() ? query.replica : DatabaseReplicated::getFullReplicaName(query.shard, query.replica);
  ...
  auto check_not_local_replica = [](const DatabaseReplicated *replicated, const String &full_replica_name_,
                                    const fs::path &query_replica_zk_path_) {
    if (!query_replica_zk_path_.empty() && fs::path(replicated->getZooKeeperPath()) != query_replica_zk_path_) return;
    if (replicated->getFullReplicaName() != full_replica_name_) return;

    throw Exception(ErrorCodes::TABLE_WAS_NOT_DROPPED,
                    "There is a local database {}, which has the same path in ZooKeeper "
                    "and the same replica name. Please check the path in query. "
                    "If you want to drop replica of this database, use `DROP DATABASE`",
  ...
  if (query.database) {
    ...
    if (auto *replicated = dynamic_cast<DatabaseReplicated *>(database.get())) {
      ...
      if (query.with_tables) dropStorageReplicasFromDatabase(query.replica, database);
      DatabaseReplicated::dropReplica(replicated, replicated->getZooKeeperName(), replicated->getZooKeeperPath(), query.shard,
                                      query.replica, /*throw_if_noop*/ true);
    } else
    ...
    for (auto &elem : databases) {
      ...
      if (query.with_tables) dropStorageReplicasFromDatabase(query.replica, database);
      DatabaseReplicated::dropReplica(replicated, replicated->getZooKeeperName(), replicated->getZooKeeperPath(), query.shard,
                                      query.replica, /*throw_if_noop*/ false);
      LOG_TRACE(log, "Dropped replica {} of Replicated database {}", query.replica, backQuoteIfNeed(database->getDatabaseName()));
    }
  } else if (query.replica_zk_path.empty()) {
    throw Exception(ErrorCodes::BAD_ARGUMENTS, "ZooKeeper path is empty");
...
bool InterpreterSystemQuery::trySyncReplica(StoragePtr table, SyncReplicaMode sync_replica_mode,
                                            const std::unordered_set<String> &src_replicas, ContextPtr context_) {
  auto table_id_ = table->getStorageID();

  /// If materialized view, sync its target table.
  for (int i = 0;; ++i) {
    if (i >= 100) throw Exception(ErrorCodes::TOO_DEEP_RECURSION, "Materialized view targets form a cycle or a very long chain");
...
void InterpreterSystemQuery::waitLoadingParts() {
  ...
  if (auto *merge_tree = dynamic_cast<MergeTreeData *>(table.get())) {
    ...
    throw Exception(ErrorCodes::BAD_ARGUMENTS, "Command WAIT LOADING PARTS is supported only for MergeTree table, but got: {}",
                    table->getName());
  }
}

void InterpreterSystemQuery::loadPrimaryKeys() { loadOrUnloadPrimaryKeysImpl(true); };

void InterpreterSystemQuery::unloadPrimaryKeys() { loadOrUnloadPrimaryKeysImpl(false); }

void InterpreterSystemQuery::loadOrUnloadPrimaryKeysImpl(bool load) {
  if (!table_id.empty()) {
    getContext()->checkAccess(load ? AccessType::SYSTEM_LOAD_PRIMARY_KEY : AccessType::SYSTEM_UNLOAD_PRIMARY_KEY, table_id.database_name,
                              table_id.table_name);
    StoragePtr table = DatabaseCatalog::instance().getTable(table_id, getContext());
    ...
    if (auto *merge_tree = dynamic_cast<MergeTreeData *>(table.get())) {
      ...
      throw Exception(ErrorCodes::BAD_ARGUMENTS, "Command {} PRIMARY KEY is supported only for MergeTree tables, but got: {}",
                      load ? "LOAD" : "UNLOAD", table->getName());
    }
  } else {
...
void InterpreterSystemQuery::instrumentWithXRay(bool add, ASTSystemQuery &query) {
  ...
  try {
    if (add) {
      InstrumentationManager::instance().patchFunction(getContext(), query.instrumentation_function_name,
                                                       query.instrumentation_handler_name, query.instrumentation_entry_type,
                                                       query.instrumentation_parameters);
    } else {
      if (!query.instrumentation_subquery.empty()) {
        ...
        if (!block_io.pipeline.initialized()) throw Exception(ErrorCodes::BAD_ARGUMENTS, "Failed to execute subquery");
        ...
        while (executor.pull(block)) {
          if (block.columns() != 1)
            throw Exception(ErrorCodes::BAD_ARGUMENTS, "Expected only one column as result of the subquery, but {} were found",
                            block.columns());
    ...
    throw Exception(ErrorCodes::BAD_ARGUMENTS, "Failed to instrument function '{}' with handler '{}', entry type '{}'{}: {}",
                    query.instrumentation_function_name, query.instrumentation_handler_name,
                    Instrumentation::entryTypeToString(query.instrumentation_entry_type), id, e.what());
  }
}
#endif
...
void InterpreterSystemQuery::syncReplicatedDatabase(ASTSystemQuery &query) {
  ...
  if (auto *ptr = typeid_cast<DatabaseReplicated *>(database.get())) {
    ...
    if (!ptr->waitForReplicaToProcessAllEntries(getContext()->getSettingsRef()[Setting::receive_timeout].totalMilliseconds())) {
      throw Exception(ErrorCodes::TIMEOUT_EXCEEDED,
                      "SYNC DATABASE REPLICA {}: database is readonly or command timed out. "
                      "See the 'receive_timeout' setting",
                      database_name);
    }
...
void InterpreterSystemQuery::flushDistributed(ASTSystemQuery &query) {
  ...
  if (query.query_settings) settings_changes = query.query_settings->as<ASTSetQuery>()->changes;
...
RefreshTaskList InterpreterSystemQuery::getRefreshTasks() {
  ...
  if (tasks.empty()) throw Exception(ErrorCodes::BAD_ARGUMENTS, "Refreshable view {} doesn't exist", table_id.getNameForLogs());
  return tasks;
}

void InterpreterSystemQuery::prewarmMarkCache() {
  if (table_id.empty()) throw Exception(ErrorCodes::BAD_ARGUMENTS, "Table is not specified for PREWARM MARK CACHE command");
  ...
  if (!merge_tree)
    throw Exception(ErrorCodes::BAD_ARGUMENTS, "Command PREWARM MARK CACHE is supported only for MergeTree table, but got: {}",
                    table_ptr->getName());

  ThreadPool pool(CurrentMetrics::MergeTreePartsLoaderThreads, CurrentMetrics::MergeTreePartsLoaderThreadsActive,
                  CurrentMetrics::MergeTreePartsLoaderThreadsScheduled, getContext()->getSettingsRef()[Setting::max_threads]);
...
void InterpreterSystemQuery::prewarmPrimaryIndexCache() {
  if (table_id.empty()) throw Exception(ErrorCodes::BAD_ARGUMENTS, "Table is not specified for PREWARM PRIMARY INDEX CACHE command");
  ...
  auto *merge_tree = dynamic_cast<MergeTreeData *>(table_ptr.get());
  if (!merge_tree)
    throw Exception(ErrorCodes::BAD_ARGUMENTS, "Command PREWARM PRIMARY INDEX CACHE is supported only for MergeTree table, but got: {}",
                    table_ptr->getName());

  auto index_cache = merge_tree->getPrimaryIndexCache();
  if (!index_cache)
    throw Exception(ErrorCodes::BAD_ARGUMENTS, "Primary index cache is not configured or is not enabled for table {}",
