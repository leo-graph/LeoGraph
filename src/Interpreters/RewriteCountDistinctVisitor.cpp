...
void RewriteCountDistinctFunctionMatcher::visit(ASTPtr& ast, Data& /*data*/) {
  ...
  if (!func || (Poco::toLower(func->name) != "countdistinct" && Poco::toLower(func->name) != "uniqexact")) return;
  auto arg = func->arguments->children;
  if (arg.size() != 1) return;
  if (!arg[0]->as<ASTIdentifier>()) return;
  ...
  if (!table_expr || table_expr->size() != 1 || !table_expr->database_and_table_name) return;
