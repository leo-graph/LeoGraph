...
std::shared_ptr<InterpreterSelectWithUnionQuery> interpretSubquery(const ASTPtr &table_expression, ContextPtr context,
                                                                   size_t subquery_depth, const Names &required_source_columns) {
...
std::shared_ptr<InterpreterSelectWithUnionQuery> interpretSubquery(const ASTPtr &table_expression, ContextPtr context,
  ...
  /// The calculation of `extremes` does not make sense and is not necessary (if you do it, then the `extremes` of the subquery can be taken
  ...
  if (table || function) {
    ...
    for (const auto &column : columns) select_expression_list->children.emplace_back(make_intrusive<ASTIdentifier>(column.name));
  } else {
