...
void ReplaceQueryParameterVisitor::visitChildren(ASTPtr &ast) {
  for (auto &child : ast->children) {
    ...
    if (child.get() != old_ptr) ast->updatePointerToChild(old_ptr, child);
  }
}

const String &ReplaceQueryParameterVisitor::getParamValue(const String &name) {
  auto search = query_parameters.find(name);
  if (search == query_parameters.end()) throw Exception(ErrorCodes::UNKNOWN_QUERY_PARAMETER, "Substitution {} is not set", backQuote(name));
...
bool needCastFromString(const DataTypePtr &type) {
  if (type->getCustomSerialization()) return true;
...
void ReplaceQueryParameterVisitor::visitIdentifier(ASTPtr &ast) {
  ...
  if (ast_identifier->children.empty()) return;
  ...
  if (!replaced_parameter) return;

  /// FIXME: what should this mean?
  if (!ast_identifier->semantic->special && name_parts.size() >= 2) ast_identifier->semantic->table = ast_identifier->name_parts.end()[-2];
...
void ReplaceQueryParameterVisitor::resolveParameterizedAlias(ASTPtr &ast) {
  ...
  if (!ast_with_alias) return;

  if (ast_with_alias->parametrised_alias) setAlias(ast, getParamValue(ast_with_alias->parametrised_alias->name));
}
}  // namespace DB
