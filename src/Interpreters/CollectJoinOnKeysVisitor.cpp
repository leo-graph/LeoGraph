...
bool isDeterminedIdentifier(JoinIdentifierPos pos) { return pos == JoinIdentifierPos::Left || pos == JoinIdentifierPos::Right; }
...
bool isRightIdentifier(JoinIdentifierPos pos) { return pos == JoinIdentifierPos::Right; }
...
void CollectJoinOnKeysMatcher::Data::addAsofJoinKeys(const ASTPtr &left_ast, const ASTPtr &right_ast, JoinIdentifierPosPair table_pos,
                                                     const ASOFJoinInequality &inequality) {
  if (isLeftIdentifier(table_pos.first) && isRightIdentifier(table_pos.second)) {
...
void CollectJoinOnKeysMatcher::Data::asofToJoinKeys() {
  if (!asof_left_key || !asof_right_key) throw Exception(ErrorCodes::INVALID_JOIN_ON_EXPRESSION, "No inequality in ASOF JOIN ON section.");
...
void CollectJoinOnKeysMatcher::visit(const ASTFunction &func, const ASTPtr &ast, Data &data) {
  if (func.name == "and") return;  /// go into children
  ...
  if (func.name == "equals" || func.name == "isNotDistinctFrom" || inequality != ASOFJoinInequality::None) {
    if (func.arguments->children.size() != 2)
      throw Exception(ErrorCodes::SYNTAX_ERROR, "Function {} takes two arguments, got '{}' instead", func.name,
  ...
  if (func.name == "equals" || func.name == "isNotDistinctFrom") {
    ...
    if (table_numbers.first == table_numbers.second) {
      if (!isDeterminedIdentifier(table_numbers.first))
        throw Exception(ErrorCodes::AMBIGUOUS_COLUMN_NAME, "Ambiguous columns in expression '{}' in JOIN ON section",
  ...
  throw Exception(ErrorCodes::INVALID_JOIN_ON_EXPRESSION, "Unsupported JOIN ON conditions. Unexpected '{}'", ast->formatForErrorMessage());
}
...
void CollectJoinOnKeysMatcher::getIdentifiers(const ASTPtr &ast, std::vector<const ASTIdentifier *> &out) {
  if (const auto *func = ast->as<ASTFunction>()) {
    ...
    if (IdentifierSemantic::getColumnName(*ident)) out.push_back(ident);
    return;
  }

  for (const auto &child : ast->children) getIdentifiers(child, out);
}
...
const ASTIdentifier *CollectJoinOnKeysMatcher::unrollAliases(const ASTIdentifier *identifier, const Aliases &aliases) {
  if (identifier->supposedToBeCompound()) return identifier;
  ...
  for (auto it = aliases.find(identifier->name()); it != aliases.end();) {
    ...
    if (!identifier) break;                         /// not a column alias
    if (identifier == parent) break;                /// alias to itself with the same name: 'a as a'
    if (identifier->supposedToBeCompound()) break;  /// not an alias. Break to prevent cycle through short names: 'a as b, t1.b as a'

    it = aliases.find(identifier->name());
    if (!max_attempts--) throw Exception(ErrorCodes::LOGICAL_ERROR, "Cannot unroll aliases for '{}'", identifier->name());
  }
...
JoinIdentifierPos CollectJoinOnKeysMatcher::getTableForIdentifiers(const ASTPtr &ast, bool throw_on_table_mix, const Data &data) {
  ...
  if (identifiers.empty()) return JoinIdentifierPos::NotColumn;
  ...
  for (auto &ident : identifiers) {
    ...
    if (auto opt = IdentifierSemantic::getMembership(*identifier); opt.has_value()) {
      if (*opt == 0)
        ...
        throw DB::Exception(ErrorCodes::AMBIGUOUS_COLUMN_NAME, "Position of identifier {} can't be deteminated.", identifier->name());
    }
    ...
    if (membership == JoinIdentifierPos::Unknown) {
      ...
      if (in_left_table && in_right_table) {
        ...
        if (auto original_name = IdentifierSemantic::uncover(*identifier)) {
          ...
          if (match == IdentifierSemantic::ColumnMatch::NoMatch) in_right_table = false;
    ...
    if (membership != JoinIdentifierPos::Unknown && membership != table_number) {
      if (throw_on_table_mix)
        throw Exception(ErrorCodes::INVALID_JOIN_ON_EXPRESSION,
                        "Invalid columns in JOIN ON section. "
