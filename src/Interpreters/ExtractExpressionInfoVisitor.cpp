...
void ExpressionInfoMatcher::visit(const ASTFunction &ast_function, const ASTPtr &, Data &data) {
  ...
  // WHERE or HAVING.
  else if (!ast_function.isWindowFunction() && AggregateFunctionFactory::instance().isAggregateFunctionName(ast_function.name)) {
    ...
    if (function) {
      if (function->isStateful()) data.is_stateful_function = true;

      if (!function->isDeterministicInScopeOfQuery()) data.is_deterministic_function = false;
    }
  }
}
...
bool ExpressionInfoMatcher::needChildVisit(const ASTPtr &node, const ASTPtr &) { return !node->as<ASTSubquery>(); }
...
bool hasNonRewritableFunction(const ASTPtr &node, ContextPtr context) {
  for (const auto &select_expression : node->children) {
    ...
    if (expression_info.is_stateful_function || expression_info.is_window_function) {
