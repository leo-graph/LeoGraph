...
class JoinMask {
 public:
  ...
  explicit JoinMask() : column(nullptr), size(0), kind(Kind::Unknown) {}

  JoinMask(bool value, size_t size_) : column(nullptr), size(size_), kind(value ? Kind::AllTrue : Kind::AllFalse) {}

  explicit JoinMask(ColumnPtr col) : column(col), size(col ? col->size() : 0), kind(Kind::Unknown) {}
...
class NotJoinedBlocks final : public IBlocksStream {
 public:
  ...
  NotJoinedBlocks(std::unique_ptr<RightColumnsFiller> filler_, const Block &result_sample_block_, size_t left_columns_count,
