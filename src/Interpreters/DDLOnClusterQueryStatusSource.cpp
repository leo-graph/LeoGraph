...
DDLOnClusterQueryStatusSource::DDLOnClusterQueryStatusSource(const String& zk_node_path, const String& zk_replicas_path,
                                                             ContextPtr context_, const Strings& hosts_to_wait)
    : DistributedQueryStatusSource(
          "default", zk_node_path, zk_replicas_path,
          std::make_shared<const Block>(getSampleBlock(context_->getSettingsRef()[Setting::distributed_ddl_output_mode])), context_,
          hosts_to_wait, "DDLOnClusterQueryStatusSource"),
...
Chunk DDLOnClusterQueryStatusSource::generateChunkWithUnfinishedHosts() const {
  ...
  for (const auto& host_id : finished_hosts) unfinished_hosts.erase(host_id);
...
Chunk DDLOnClusterQueryStatusSource::handleTimeoutExceeded() {
  ...
  constexpr auto msg_format =
      "Distributed DDL task {} is not finished on {} of {} hosts "
  ...
  if (throw_on_timeout || (throw_on_timeout_only_active && !stop_waiting_offline_hosts)) {
    if (!first_exception)
      first_exception =
          std::make_unique<Exception>(Exception(ErrorCodes::TIMEOUT_EXCEEDED, msg_format, node_path, num_unfinished_hosts,
                                                waiting_hosts.size(), num_active_hosts, offline_hosts.size(), watch.elapsedSeconds(),
...
void DDLOnClusterQueryStatusSource::handleNonZeroStatusCode(const ExecutionStatus& status, const String& host_id) {
  ...
  if (!first_exception && context->getSettingsRef()[Setting::distributed_ddl_output_mode] != DistributedDDLOutputMode::NEVER_THROW) {
    auto [host, port] = parseHostAndPort(host_id);
    first_exception = std::make_unique<Exception>(Exception(status.code, "There was an error on [{}:{}]: {}", host, port, status.message));
  }
}
...
bool DDLOnClusterQueryStatusSource::nullableErrorAndStatusFields(DistributedDDLOutputMode output_mode) {
  return !(output_mode == DistributedDDLOutputMode::THROW ||          ///
