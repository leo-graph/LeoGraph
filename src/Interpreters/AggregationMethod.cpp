...
void AggregationMethodStringNoCache<TData, nullable>::insertKeyIntoColumns(std::string_view key, std::vector<IColumn *> &key_columns,
                                                                           const Sizes &, const IColumn::SerializationSettings *) {
  if constexpr (nullable) {
...
template <typename TData, bool nullable>
void AggregationMethodFixedStringNoCache<TData, nullable>::insertKeyIntoColumns(std::string_view key, std::vector<IColumn *> &key_columns,
                                                                                const Sizes &, const IColumn::SerializationSettings *) {
  if constexpr (nullable)
...
template struct AggregationMethodSingleLowCardinalityColumn<
    AggregationMethodOneNumber<UInt32, AggregatedDataWithNullableUInt64KeyTwoLevel>>;
template struct AggregationMethodSingleLowCardinalityColumn<
    AggregationMethodOneNumber<UInt64, AggregatedDataWithNullableUInt64KeyTwoLevel>>;
...
void AggregationMethodKeysFixed<TData, has_nullable_keys, has_low_cardinality, consecutive_keys_optimization>::insertKeyIntoColumns(
  ...
  for (size_t i = 0; i < keys_size; ++i) {
    ...
    if constexpr (has_nullable_keys) column_nullable = isColumnNullable(*key_columns[i]);
...
void AggregationMethodSerialized<TData, nullable, prealloc>::insertKeyIntoColumns(std::string_view key, std::vector<IColumn *> &key_columns,
                                                                                  const Sizes &,
                                                                                  const IColumn::SerializationSettings *settings) {
  ReadBufferFromString buf(key);
  for (auto &column : key_columns) column->deserializeAndInsertFromArena(buf, settings);
}
