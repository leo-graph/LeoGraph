...
FilterDAGInfoPtr generateFilterActions(const StorageID &table_id, const ASTPtr &row_policy_filter_expression, const ContextPtr &context,
                                       const StoragePtr &storage, const StorageSnapshotPtr &storage_snapshot,
                                       const StorageMetadataPtr &metadata_snapshot, Names &prerequisite_columns,
                                       PreparedSetsPtr prepared_sets) try {
  ...
  for (const auto &column_str : prerequisite_columns) {
    ...
    expr_list->children.push_back(parseQuery(expr_parser, backQuoteIfNeed(column_str), 0,
  ...
  for (const auto *node : filter_info->actions.getInputs()) filter_info->actions.getOutputs().push_back(node);
...
InterpreterSelectQuery::InterpreterSelectQuery(const ASTPtr &query_ptr_, const ContextPtr &context_, const SelectQueryOptions &options_,
                                               const Names &required_result_column_names_)
    : InterpreterSelectQuery(query_ptr_, context_, std::nullopt, nullptr, options_, required_result_column_names_) {}

InterpreterSelectQuery::InterpreterSelectQuery(const ASTPtr &query_ptr_, const ContextMutablePtr &context_,
                                               const SelectQueryOptions &options_, const Names &required_result_column_names_)
    : InterpreterSelectQuery(query_ptr_, context_, std::nullopt, nullptr, options_, required_result_column_names_) {}

InterpreterSelectQuery::InterpreterSelectQuery(const ASTPtr &query_ptr_, const ContextPtr &context_, Pipe input_pipe_,
                                               const SelectQueryOptions &options_)
    : InterpreterSelectQuery(query_ptr_, context_, std::move(input_pipe_), nullptr, options_.copy().noSubquery()) {}

InterpreterSelectQuery::InterpreterSelectQuery(const ASTPtr &query_ptr_, const ContextPtr &context_, const StoragePtr &storage_,
                                               const StorageMetadataPtr &metadata_snapshot_, const SelectQueryOptions &options_)
...
ContextPtr getSubqueryContext(const ContextPtr &context) {
  ...
  /// The calculation of extremes does not make sense and is not necessary (if you do it, then the extremes of the subquery can be taken for
  /// whole query).
...
void rewriteMultipleJoins(ASTPtr &query, const TablesWithColumns &tables, const String &database, const Settings &settings) {
  ...
  Aliases aliases;
  if (ASTPtr with = select.with()) QueryAliasesNoSubqueriesVisitor(aliases).visit(with);
...
void checkAccessRightsForSelect(const ContextPtr &context, const StorageID &table_id, const StorageMetadataPtr &table_metadata,
  ...
  if (!syntax_analyzer_result.has_explicit_columns && table_metadata && !table_metadata->getColumns().empty()) {
    ...
    for (const auto &column : table_metadata->getColumns()) {
      if (access->isGranted(AccessType::SELECT, table_id.database_name, table_id.table_name, column.name)) return;
    }
    throw Exception(
        ErrorCodes::ACCESS_DENIED,
        "{}: Not enough privileges. To execute this query, it's necessary to have the grant SELECT for at least one column on {}",
        context->getUserName(), table_id.getFullTableName());
  }
...
ASTPtr parseAdditionalFilterConditionForTable(const Map &additional_table_filters, const DatabaseAndTableWithAlias &target,
  ...
  for (const auto &additional_filter : additional_table_filters) {
    ...
    if (table == target.alias || (table == target.table && context.getCurrentDatabase() == target.database) ||
        (table == target.database + '.' + target.table)) {
      /// Try to parse expression
      ParserExpression parser;
      const auto &settings = context.getSettingsRef();
      return parseQuery(parser, filter.data(), filter.data() + filter.size(), "additional filter", settings[Setting::max_query_size],
...
bool shouldIgnoreQuotaAndLimits(const StorageID &table_id) {
  if (table_id.database_name == DatabaseCatalog::SYSTEM_DATABASE) {
    ...
    if (tables_ignoring_quota.count(table_id.table_name)) return true;
  }
  return false;
}
...
GroupingSetsParamsList getAggregatorGroupingSetsParams(const NamesAndTypesLists &aggregation_keys_list, const Names &all_keys) {
  ...
  for (const auto &aggregation_keys : aggregation_keys_list) {
    NameSet keys;
    for (const auto &key : aggregation_keys) keys.insert(key.name);

    Names missing_keys;
    for (const auto &key : all_keys)
      if (!keys.contains(key)) missing_keys.push_back(key);
...
InterpreterSelectQuery::InterpreterSelectQuery(const ASTPtr &query_ptr_, const ContextPtr &context_, std::optional<Pipe> input_pipe_,
                                               const StoragePtr &storage_, const SelectQueryOptions &options_,
                                               const Names &required_result_column_names, const StorageMetadataPtr &metadata_snapshot_,
                                               PreparedSetsPtr prepared_sets_)
    : InterpreterSelectQuery(query_ptr_, Context::createCopy(context_), std::move(input_pipe_), storage_, options_,
                             required_result_column_names, metadata_snapshot_, prepared_sets_) {}

InterpreterSelectQuery::InterpreterSelectQuery(const ASTPtr &query_ptr_, const ContextMutablePtr &context_, std::optional<Pipe> input_pipe_,
                                               const StoragePtr &storage_, const SelectQueryOptions &options_,
                                               const Names &required_result_column_names, const StorageMetadataPtr &metadata_snapshot_,
                                               PreparedSetsPtr prepared_sets_)
  ...
  if (!prepared_sets) prepared_sets = std::make_shared<PreparedSets>();
  ...
  if (!options.is_subquery) {
    if (context->getSettingsRef()[Setting::enable_global_with_statement]) ApplyWithAliasVisitor::visit(query_ptr);
  ...
  if (storage) {
    ...
    if (!metadata_snapshot) metadata_snapshot = storage->getInMemoryMetadataPtr();
  ...
  if (is_plain_merge_tree && settings[Setting::allow_experimental_parallel_reading_from_replicas] > 0 &&
    ...
    if (settings[Setting::allow_experimental_parallel_reading_from_replicas] == 1) {
      LOG_DEBUG(log,
                "To use parallel replicas with plain MergeTree tables please enable setting "
                "`parallel_replicas_for_non_replicated_merge_tree`. For now query will be executed without using them.");
      context->setSetting("allow_experimental_parallel_reading_from_replicas", Field(0));
    } else if (settings[Setting::allow_experimental_parallel_reading_from_replicas] >= 2) {
      throw Exception(
          ErrorCodes::SUPPORT_IS_DISABLED,
          "To use parallel replicas with plain MergeTree tables please enable setting `parallel_replicas_for_non_replicated_merge_tree`");
    }
  }
  ...
  if (storage)
    row_policy_filter =
        context->getRowPolicyFilter(table_id.getDatabaseName(), table_id.getTableName(), RowPolicyFilterType::SELECT_FILTER);

  StorageView *view = nullptr;
  if (storage) view = dynamic_cast<StorageView *>(storage.get());

  if (!settings[Setting::additional_table_filters].value.empty() && storage && !joined_tables.tablesWithColumns().empty())
    query_info.additional_filter_ast = parseAdditionalFilterConditionForTable(settings[Setting::additional_table_filters],
                                                                              joined_tables.tablesWithColumns().front().table, *context);
  ...
  if (storage && context->canUseParallelReplicasCustomKey() && !joined_tables.tablesWithColumns().empty()) {
    if (settings[Setting::parallel_replicas_count] > 1) {
      if (auto custom_key_ast = parseCustomKeyForTable(settings[Setting::parallel_replicas_custom_key], *context)) {
        ...
        parallel_replicas_custom_filter_ast = getCustomKeyFilterForParallelReplica(
            settings[Setting::parallel_replicas_count], settings[Setting::parallel_replica_offset], std::move(custom_key_ast),
            {settings[Setting::parallel_replicas_mode], settings[Setting::parallel_replicas_custom_key_range_lower],
             settings[Setting::parallel_replicas_custom_key_range_upper]},
            storage->getInMemoryMetadataPtr()->columns, context);
    ...
    /// local plan and remote replicas
      ...
      context->setSetting("prefer_localhost_replica", Field(0));
               ...
               (storage->supportsReplication() || settings[Setting::parallel_replicas_for_non_replicated_merge_tree]) &&
  ...
  auto analyze = [&](bool try_move_to_prewhere) {
    ...
    syntax_analyzer_result =
        TreeRewriter(context).analyzeSelect(query_ptr, TreeRewriterResult(source_header->getNamesAndTypesList(), storage, storage_snapshot),
                                            options, joined_tables.tablesWithColumns(), required_result_column_names, table_join);

    query_info.syntax_analyzer_result = syntax_analyzer_result;
    context->setDistributed(syntax_analyzer_result->is_remote_storage);

    if (storage && !query.final() && storage->needRewriteQueryWithFinal(syntax_analyzer_result->requiredSourceColumns())) query.setFinal();
        ...
        !query.hasJoin())  /// Join may produce rows with nulls or default values, it's difficult to analyze if they affected or not.
    {
      ...
      if (const auto &column_sizes = storage->getColumnSizes(); !column_sizes.empty()) {
        ...
        for (const auto &[name, sizes] : column_sizes) column_compressed_sizes[name] = sizes.data_compressed;
        ...
        if (storage_snapshot->data) {
          ...
          if (parts) parts_for_estimator = *parts;
        }
        ...
        MergeTreeWhereOptimizer where_optimizer{
            std::move(column_compressed_sizes), storage_snapshot, estimator, queried_columns, supported_prewhere_columns, log};
    ...
    if (query.prewhere() && query.where()) {
      ...
      query.setExpression(ASTSelectQuery::Expression::WHERE, makeASTOperator("and", query.prewhere()->clone(), query.where()->clone()));
    }

    query_analyzer =
        std::make_unique<SelectQueryExpressionAnalyzer>(query_ptr, syntax_analyzer_result, context, metadata_snapshot,
                                                        required_result_column_names, !options.only_analyze, options, prepared_sets);
    ...
    if (!options.only_analyze) {
      ...
      if (query.final() && (input_pipe || !storage || !storage->supportsFinal())) {
        if (!input_pipe && storage) throw Exception(ErrorCodes::ILLEGAL_FINAL, "Storage {} doesn't support FINAL", storage->getName());
    ...
    if (interpreter_subquery) {
      ...
      if (query_analyzer->hasAggregation()) interpreter_subquery->ignoreWithTotals();
      uses_view_source |= interpreter_subquery->usesViewSource();
    }
    ...
    if (storage) {
      ...
      if (row_policy_filter && !row_policy_filter->isAlwaysTrue()) {
        row_policy_info = generateFilterActions(table_id, row_policy_filter->expression, context, storage, storage_snapshot,
                                                metadata_snapshot, required_columns, prepared_sets);

        query_info.filter_asts.push_back(row_policy_filter->expression);
      }

      if (query_info.additional_filter_ast) {
        additional_filter_info = generateFilterActions(table_id, query_info.additional_filter_ast, context, storage, storage_snapshot,
                                                       metadata_snapshot, required_columns, prepared_sets);
      ...
      if (parallel_replicas_custom_filter_ast) {
        parallel_replicas_custom_filter_info = generateFilterActions(table_id, parallel_replicas_custom_filter_ast, context, storage,
                                                                     storage_snapshot, metadata_snapshot, required_columns, prepared_sets);
  ...
  UInt64 parallel_replicas_before_analysis =
      context->hasQueryContext() ? context->getQueryContext()->getSettingsRef()[Setting::allow_experimental_parallel_reading_from_replicas]
                                 : 0;

  /// Conditionally support AST-based PREWHERE optimization.
  analyze(shouldMoveToPrewhere() &&
          (!settings[Setting::query_plan_optimize_prewhere] || !settings[Setting::query_plan_enable_optimizations]));
  ...
  if (can_analyze_again &&
      (analysis_result.where_constant_filter_description.always_false || analysis_result.where_constant_filter_description.always_true)) {
  ...
  if (can_analyze_again) need_analyze_again |= adjustParallelReplicasAfterAnalysis();
  ...
  if (query.prewhere() && !query.where()) analysis_result.prewhere_info->need_filter = true;
...
bool InterpreterSelectQuery::adjustParallelReplicasAfterAnalysis() {
  ...
  if (!storage || options.only_analyze || !context->canUseParallelReplicasOnInitiator()) return false;
  ...
  if (!storage_merge_tree || settings[Setting::parallel_replicas_min_number_of_rows_per_replica] == 0) return false;
  ...
  if (analysis_copy.prewhere_info) {
    ...
    if (query.prewhere() && !query.where()) query_info_copy.prewhere_info->need_filter = true;
  }

  ActionDAGNodes added_filter_nodes = MergeTreeData::getFiltersForPrimaryKeyAnalysis(*this);
  if (query_info_copy.prewhere_info) {
    {
      const auto &node = query_info_copy.prewhere_info->prewhere_actions.findInOutputs(query_info_copy.prewhere_info->prewhere_column_name);
      added_filter_nodes.nodes.push_back(&node);
    }
  }

  if (query_info_copy.row_level_filter) {
    const auto &node = query_info_copy.row_level_filter->actions.findInOutputs(query_info_copy.row_level_filter->column_name);
    added_filter_nodes.nodes.push_back(&node);
  }
...
void InterpreterSelectQuery::buildQueryPlan(QueryPlan &query_plan) {
  ...
  if (!blocksHaveEqualStructure(*query_plan.getCurrentHeader(), *result_header)) {
    auto convert_actions_dag =
        ActionsDAG::makeConvertingActions(query_plan.getCurrentHeader()->getColumnsWithTypeAndName(),
                                          result_header->getColumnsWithTypeAndName(), ActionsDAG::MatchColumnsMode::Name, context, true);
  ...
  if (table_lock) query_plan.addTableLock(std::move(table_lock));
  if (storage) query_plan.addStorageHolder(storage);
}
...
Block InterpreterSelectQuery::getSampleBlockImpl() {
  ...
  bool second_stage = from_stage <= QueryProcessingStage::WithMergeableState && options.to_stage > QueryProcessingStage::WithMergeableState;

  analysis_result = ExpressionAnalysisResult(*query_analyzer, metadata_snapshot, first_stage, second_stage, options.only_analyze,
                                             row_policy_info, additional_filter_info, *source_header);
  ...
  if (options.to_stage == QueryProcessingStage::Enum::FetchColumns) {
    ...
    if (analysis_result.prewhere_info) {
      ...
      if (analysis_result.prewhere_info->remove_prewhere_column) header.erase(analysis_result.prewhere_info->prewhere_column_name);
    }
    return header;
  }
  ...
  if (options.to_stage == QueryProcessingStage::Enum::WithMergeableState) {
    if (!analysis_result.need_aggregate) {
      ...
      if (analysis_result.before_window) return analysis_result.before_window->dag.getResultColumns();
    ...
    if (analysis_result.use_grouping_set_key) res.insert({nullptr, std::make_shared<DataTypeUInt64>(), "__grouping_set"});
    ...
    if (context->getSettingsRef()[Setting::group_by_use_nulls] && analysis_result.use_grouping_set_key) {
      ...
      for (const auto &key : query_analyzer->aggregationKeys()) res.insert({nullptr, header.getByName(key.name).type, key.name});
    }
  ...
  if (options.to_stage >= QueryProcessingStage::Enum::WithMergeableStateAfterAggregation) {
    ...
    if (analysis_result.before_limit_by) return analysis_result.before_limit_by->dag.getResultColumns();
...
static std::pair<Field, DataTypePtr> getWithFillFieldValue(const ASTPtr &node, ContextPtr context) {
  ...
  if (!isColumnedAsNumber(field_type.second))
    throw Exception(ErrorCodes::INVALID_WITH_FILL_EXPRESSION, "Illegal type {} of WITH FILL expression, must be numeric type",
                    field_type.second->getName());
...
static std::pair<Field, std::optional<IntervalKind>> getWithFillValueWithIntervalKind(const ASTPtr &node, const ContextPtr &context) {
  ...
  if (isColumnedAsNumber(type)) return std::make_pair(std::move(field), std::nullopt);
...
static FillColumnDescription getWithFillDescription(const ASTOrderByElement &order_by_elem, const ContextPtr &context) {
  ...
  if (order_by_elem.getFillTo()) std::tie(descr.fill_to, descr.fill_to_type) = getWithFillFieldValue(order_by_elem.getFillTo(), context);
  ...
  if (order_by_elem.getFillStaleness()) {
    ...
    if (order_by_elem.getFillFrom())
      throw Exception(ErrorCodes::INVALID_WITH_FILL_EXPRESSION, "WITH FILL STALENESS cannot be used together with WITH FILL FROM");
  }
  ...
  if (order_by_elem.direction == 1) {
    ...
    if (!descr.fill_from.isNull() && !descr.fill_to.isNull() && accurateLess(descr.fill_to, descr.fill_from)) {
      throw Exception(ErrorCodes::INVALID_WITH_FILL_EXPRESSION,
    ...
    if (!descr.fill_from.isNull() && !descr.fill_to.isNull() && accurateLess(descr.fill_from, descr.fill_to)) {
      throw Exception(ErrorCodes::INVALID_WITH_FILL_EXPRESSION,
...
SortDescription InterpreterSelectQuery::getSortDescription(const ASTSelectQuery &query, const ContextPtr &context_) {
  ...
  for (const auto &elem : query.orderBy()->children) {
    ...
    std::string alias;
    if (auto *identifier = order_by_elem.children[0]->as<ASTIdentifier>()) alias = identifier->name();
...
static InterpolateDescriptionPtr getInterpolateDescription(const ASTSelectQuery &query, const Block &source_block,
                                                           const Block &result_block, const Aliases &aliases, ContextPtr context) {
  ...
  if (query.interpolate()) {
    ...
    if (query.interpolate()->children.empty()) {
      ...
      for (const auto &column : result_block.getColumnsWithTypeAndName()) column_names[column.name] = column.type;
      for (const auto &elem : query.orderBy()->children)
        if (elem->as<ASTOrderByElement>()->with_fill) column_names.erase(elem->as<ASTOrderByElement>()->children.front()->getColumnName());
      ...
      for (const auto &elem : query.interpolate()->children) {
        ...
        if (const ColumnWithTypeAndName *result_block_column = result_block.findByName(interpolate.column)) {
          if (!col_set.insert(result_block_column->name).second)
            throw Exception(ErrorCodes::INVALID_WITH_FILL_EXPRESSION, "Duplicate INTERPOLATE column '{}'", interpolate.column);

          result_columns.emplace_back(result_block_column->type, result_block_column->name);
        } else
          throw Exception(ErrorCodes::UNKNOWN_IDENTIFIER, "Missing column '{}' as an INTERPOLATE expression target", interpolate.column);
      ...
      for (const auto &column : source_block)
        if (!col_set.contains(column.name)) source_columns.emplace_back(column.name, column.type);
    }
    ...
    ActionsDAG conv_dag = ActionsDAG::makeConvertingActions(actions.getResultColumns(), result_columns,
                                                            ActionsDAG::MatchColumnsMode::Position, context, true);
...
static std::tuple<UInt64, Float64, bool> getLimitOffsetValue(const ASTPtr &node, const ContextPtr &context, const std::string &expr) {
  ...
  if (!isNativeNumber(type))
    throw Exception(ErrorCodes::INVALID_LIMIT_EXPRESSION, "Illegal type {} of {} expression, must be numeric type", type->getName(), expr);
  ...
  // First check if it is nonnegative limit since they are more common
  {
    ...
    if (!converted_value.isNull()) return {converted_value.safeGet<UInt64>(), 0, false};
  }
...

  {
    ...
    if (!converted_value.isNull()) {
      ...
      if (value < 1 && value > 0) return {0, value, false};
    }
  }
...
UInt64 InterpreterSelectQuery::getLimitForSorting(const ASTSelectQuery &query, const ContextPtr &context_) {
  ...
  if (!query.distinct && !query.limitBy() && !query.limit_with_ties && !query.arrayJoinExpressionList().first && query.limitLength()) {
    ...
    if (lim_info.is_limit_length_negative || lim_info.fractional_offset > 0 || lim_info.fractional_limit > 0) return 0;

    if (lim_info.limit_length > std::numeric_limits<UInt64>::max() - lim_info.limit_offset) return 0;
...
static bool hasWithTotalsInAnySubqueryInFromClause(const ASTSelectQuery &query) {
  if (query.group_by_with_totals) return true;
    ...
    if (const auto *ast_union = query_table->as<ASTSelectWithUnionQuery>()) {
      ...
      std::function<bool(ASTPtr)> traverse_recursively = [&](ASTPtr child_ast) -> bool {
        if (const auto *select_child = child_ast->as<ASTSelectQuery>()) {
          if (hasWithTotalsInAnySubqueryInFromClause(select_child->as<ASTSelectQuery &>())) return true;
          ...
          for (const auto &subchild : union_child->list_of_selects->children)
            if (traverse_recursively(subchild)) return true;
      ...
      for (const auto &elem : ast_union->list_of_selects->children)
        if (traverse_recursively(elem)) return true;
    }
  }
...
ALWAYS_INLINE void executeExpression(QueryPlan &query_plan, const ActionsAndProjectInputsFlagPtr &expression,
                                     const char (&description)[size]) {
  if (!expression) return;

  auto dag = expression->dag.clone();
  if (expression->project_input) dag.appendInputsForUnusedColumns(*query_plan.getCurrentHeader());
...
void InterpreterSelectQuery::executeImpl(QueryPlan &query_plan, std::optional<Pipe> prepared_pipe) {
  ...
  bool aggregate_overflow_row = expressions.need_aggregate && query.group_by_with_totals && settings[Setting::max_rows_to_group_by] &&
  ...
  if (query.qualify()) throw Exception(ErrorCodes::NOT_IMPLEMENTED, "QUALIFY clause is not supported in the old analyzer");
  ...
  if (options.only_analyze) {
    ...
    if (expressions.row_policy_info) {
      auto row_level_security_step =
          std::make_unique<FilterStep>(query_plan.getCurrentHeader(), expressions.row_policy_info->actions.clone(),
                                       expressions.row_policy_info->column_name, expressions.row_policy_info->do_remove_column);
    ...
    if (expressions.prewhere_info) {
      auto prewhere_step =
          std::make_unique<FilterStep>(query_plan.getCurrentHeader(), expressions.prewhere_info->prewhere_actions.clone(),
    ...
    if (from_stage >= QueryProcessingStage::WithMergeableStateAfterAggregation) from_aggregation_stage = true;
    /// Is running on remote servers during distributed processing?
    if (options.to_stage >= QueryProcessingStage::WithMergeableStateAfterAggregation) to_aggregation_stage = true;
  ...
  if (!expressions.need_aggregate) input_order_info_for_order = query_info.input_order_info;
  ...
  if (options.to_stage > QueryProcessingStage::FetchColumns) {
    auto preliminary_sort = [&]() {
      ...
      if (!expressions.second_stage && !expressions.need_aggregate && !expressions.hasHaving() && !expressions.has_window) {
        if (expressions.has_order_by) executeOrder(query_plan, input_order_info_for_order);
        ...
        if (query.limitLength()) executeDistinct(query_plan, false, expressions.selected_columns, false);
        ...
        if (query.limitLength() && !query.limitBy() && !query.limit_with_ties) {
          ...
          if (lim_info.fractional_offset == 0 && lim_info.fractional_limit == 0) executePreLimit(query_plan, true);
        }
      }
    };
    ...
    if (intermediate_stage) {
      ...
      preliminary_sort();
      if (expressions.need_aggregate) executeMergeAggregated(query_plan, aggregate_overflow_row, aggregate_final, use_grouping_set_key);
    }
    ...
    if (expressions.first_stage) {
      ...
      const auto add_filter_step = [&](auto &new_filter_info, const auto &description) {
        auto filter_step = std::make_unique<FilterStep>(query_plan.getCurrentHeader(), std::move(new_filter_info->actions),
                                                        new_filter_info->column_name, new_filter_info->do_remove_column);

        filter_step->setStepDescription(description);
        query_plan.addStep(std::move(filter_step));
      };

      if (additional_filter_info) add_filter_step(additional_filter_info, "Additional filter");

      if (parallel_replicas_custom_filter_info) add_filter_step(parallel_replicas_custom_filter_info, "Parallel replica custom key filter");

      if (expressions.before_array_join) executeExpression(query_plan, expressions.before_array_join, "Before ARRAY JOIN");

      if (expressions.array_join) {
        QueryPlanStepPtr array_join_step = std::make_unique<ArrayJoinStep>(
            query_plan.getCurrentHeader(), *expressions.array_join, settings[Setting::enable_unaligned_array_join],
            settings[Setting::max_block_size], settings[Setting::enable_lazy_columns_replication]);

        array_join_step->setStepDescription("ARRAY JOIN");
        query_plan.addStep(std::move(array_join_step));
      }

      if (expressions.before_join) executeExpression(query_plan, expressions.before_join, "Before JOIN");

      /// Optional step to convert key columns to common supertype.
      if (expressions.converting_join_columns) executeExpression(query_plan, expressions.converting_join_columns, "Convert JOIN columns");
      ...
      if (expressions.hasJoin()) {
        if (expressions.join->isFilled()) {
          ...
          if (!joined_plan) throw Exception(ErrorCodes::LOGICAL_ERROR, "There is no joined plan for query");

          auto add_sorting = [this](QueryPlan &plan, const Names &key_names, JoinTableSide join_pos) {
            SortDescription order_descr;
            order_descr.reserve(key_names.size());
            for (const auto &key_name : key_names) order_descr.emplace_back(key_name);

            SortingStep::Settings sort_settings(context->getSettingsRef());

            auto sorting_step =
                std::make_unique<SortingStep>(plan.getCurrentHeader(), std::move(order_descr), 0 /* LIMIT */, sort_settings);
          ...
          auto add_create_set = [&settings, crosswise_connection, max_len = options.max_step_description_length](
                                    QueryPlan &plan, const Names &key_names, JoinTableSide join_pos) {
            auto creating_set_step = std::make_unique<CreateSetAndFilterOnTheFlyStep>(
                plan.getCurrentHeader(), key_names, settings[Setting::max_rows_in_set_to_optimize_join], crosswise_connection, join_pos);
          ...
          if (expressions.join->pipelineType() == JoinPipelineType::YShaped && expressions.join->getTableJoin().kind() != JoinKind::Paste) {
            ...
            auto has_non_const = [](const Block &block, const auto &keys) {
              for (const auto &key : keys) {
                ...
                if (column && !isColumnConst(*column)) return true;
              }
              return false;
            };
            ...
            /// No checks here because read in order is not applied if we have `CreateSetAndFilterOnTheFlyStep` in the pipeline between the
            /// reading and sorting steps.
            ...
            if (settings[Setting::max_rows_in_set_to_optimize_join] > 0 && join_type_allows_filtering && has_non_const_keys) {
              ...
              if (isInnerOrLeft(join_kind)) right_set->setFiltering(left_set->getSet());

              if (isInnerOrRight(join_kind)) left_set->setFiltering(right_set->getSet());
            }
          ...
          QueryPlanStepPtr join_step = std::make_unique<JoinStep>(
              query_plan.getCurrentHeader(), joined_plan->getCurrentHeader(), expressions.join, settings[Setting::max_block_size],
              /* min_block_size_rows_ */ 0,
              /* min_block_size_bytes_ */ 0, max_streams,
              /* required_output_ = */ NameSet{}, analysis_result.optimize_read_in_order,
      ...
      if (expressions.hasWhere()) executeWhere(query_plan, expressions.before_where, expressions.remove_where_filter);
    ...
    if (expressions.second_stage || from_aggregation_stage) {
      if (from_aggregation_stage) {
        ...
        if (!expressions.first_stage) executeMergeAggregated(query_plan, aggregate_overflow_row, aggregate_final, use_grouping_set_key);

        if (!aggregate_final) {
          if (query.group_by_with_totals) {
            bool final = !query.group_by_with_rollup && !query.group_by_with_cube;
            executeTotalsAndHaving(query_plan, expressions.hasHaving(), expressions.before_having, expressions.remove_having_filter,
      ...
      if (expressions.has_order_by) {
        ...
        else if (!expressions.first_stage && !expressions.need_aggregate && !expressions.has_window &&
      ...
      bool apply_prelimit = apply_limit && query.limitLength() && !query.limit_with_ties &&
                            !hasWithTotalsInAnySubqueryInFromClause(query) && !query.arrayJoinExpressionList().first && !query.distinct &&
                            !expressions.hasLimitBy() && !settings[Setting::extremes] && !has_withfill;
      /// Preliminary Limits mustn't be added if there is a fractional limit/offset
      /// because in order to correctly calculate the number of rows to be produced
      /// based on the given fraction the final limit/offset processor must count the entire dataset.
      /// For example, LIMIT 0.1 and 30 rows in the sources - we must read all 30 rows to calculate that rows_cnt * 0.1 = 3.
       ...
       */
      if (!from_aggregation_stage && query.distinct) executeDistinct(query_plan, false, expressions.selected_columns, false);
      ...
      if (apply_limit && !limit_applied && apply_offset) executeLimit(query_plan);

      if (apply_offset) executeOffset(query_plan);
    }
  }
...
static void executeMergeAggregatedImpl(QueryPlan &query_plan, bool overflow_row, bool final, bool is_remote_storage, bool has_grouping_sets,
                                       const Settings &settings, const NamesAndTypesList &aggregation_keys,
                                       const NamesAndTypesLists &aggregation_keys_list, const AggregateDescriptions &aggregates,
  ...
  Aggregator::Params params(keys, aggregates, overflow_row, settings[Setting::max_threads], settings[Setting::max_block_size],
  ...
  auto merging_aggregated = std::make_unique<MergingAggregatedStep>(
      query_plan.getCurrentHeader(), params, grouping_sets_params, final,
      /// Grouping sets don't work with distributed_aggregation_memory_efficient enabled (#43989)
      settings[Setting::distributed_aggregation_memory_efficient] && is_remote_storage && !has_grouping_sets,
      settings[Setting::aggregation_memory_efficient_merge_threads], should_produce_results_in_order_of_bucket_number,
      settings[Setting::max_block_size], settings[Setting::aggregation_in_order_max_block_bytes],
      settings[Setting::enable_memory_bound_merging_of_aggregation_results]);

  query_plan.addStep(std::move(merging_aggregated));
}

void InterpreterSelectQuery::addEmptySourceToQueryPlan(QueryPlan &query_plan, const Block &source_header,
  ...
  if (query_info.row_level_filter) {
    ...
    pipe.addSimpleTransform([&](const SharedHeader &header) {
      return std::make_shared<FilterTransform>(header, row_level_actions, query_info.row_level_filter->column_name,
  ...
  if (query_info.prewhere_info) {
    ...
    pipe.addSimpleTransform([&](const SharedHeader &header) {
      return std::make_shared<FilterTransform>(header, filter_actions, prewhere_info.prewhere_column_name,
...
RowPolicyFilterPtr InterpreterSelectQuery::getRowPolicyFilter() const { return row_policy_filter; }
  ...
  if (alias_columns_required) {
    ...
    for (const auto &column : required_columns) {
      ...
      if (is_alias) {
        ...
        replaceAliasColumnsInQuery(column_expr, metadata_snapshot->getColumns(), syntax_analyzer_result->array_join_result_to_source,
                                   context);

        column_expr =
            addTypeConversionToAST(std::move(column_expr), column_decl.type->getName(), metadata_snapshot->getColumns().getAll(), context);
      ...
      if (required_columns_from_prewhere.contains(column)) {
        ...
        if (is_alias) required_aliases_from_prewhere.insert(column);
      } else
    ...
    if (prewhere_info) {
      ...
      for (const auto &column : prewhere_actions_result) {
        if (prewhere_info->remove_prewhere_column && column.name == prewhere_info->prewhere_column_name) continue;

        if (columns_to_remove.contains(column.name)) continue;
      ...
      required_columns_after_prewhere_set =
          NameSet{std::from_range_t{}, required_columns_after_prewhere | std::views::transform([](const auto &it) { return it.name; })};
    }

    auto syntax_result = TreeRewriter(context).analyze(required_columns_all_expr, required_columns_after_prewhere, storage,
                                                       storage_snapshot, options.is_create_parameterized_view);
    ...
    if (prewhere_info) {
      ...
      for (const auto &name : required_columns) prewhere_info->prewhere_actions.tryRestoreColumn(name);
...
std::optional<UInt64> InterpreterSelectQuery::getTrivialCount(UInt64 allow_experimental_parallel_reading_from_replicas) {
  ...
  bool empty_result_for_aggregation_by_empty_set = settings[Setting::empty_result_for_aggregation_by_empty_set] ||
                                                   (settings[Setting::empty_result_for_aggregation_by_constant_keys_on_empty_set] &&
                                                    query_analyzer->aggregationKeys().empty() && query_analyzer->hasConstAggregationKeys());

  bool optimize_trivial_count =
      syntax_analyzer_result->optimize_trivial_count && (allow_experimental_parallel_reading_from_replicas == 0) &&
      !settings[Setting::allow_experimental_query_deduplication] && !empty_result_for_aggregation_by_empty_set && storage &&
      storage->supportsTrivialCountOptimization(storage_snapshot, getContext()) && query_info.filter_asts.empty() &&
  ...
  if (!optimize_trivial_count) return {};
...
UInt64 InterpreterSelectQuery::maxBlockSizeByLimit() const {
  ...
  if (!query.distinct && !query.limit_with_ties && !query.prewhere() && !query.where() && query_info.filter_asts.empty() &&
      !query.groupBy() && !query.having() && !query.orderBy() && !query.limitBy() && !query.join() && !query_analyzer->hasAggregation() &&
      !query_analyzer->hasWindow() && lim_info.fractional_limit == 0 && lim_info.fractional_offset == 0 && query.limitLength() &&
      lim_info.limit_length <= std::numeric_limits<UInt64>::max() - lim_info.limit_offset && !lim_info.is_limit_length_negative)
    return lim_info.limit_length + lim_info.limit_offset;
...
void InterpreterSelectQuery::executeFetchColumns(QueryProcessingStage::Enum processing_stage, QueryPlan &query_plan) {
  ...
  if (processing_stage == QueryProcessingStage::FetchColumns &&
    ...
    DataTypes argument_types(arguments_size);
    for (size_t j = 0; j < arguments_size; ++j) argument_types[j] = header.getByName(desc.argument_names[j]).type;

    auto block_with_count = std::make_shared<const Block>(
        Block{{std::move(column), std::make_shared<DataTypeAggregateFunction>(func, argument_types, desc.parameters), desc.column_name}});
  ...
  if (!options.only_analyze && settings[Setting::max_columns_to_read] && required_columns.size() > settings[Setting::max_columns_to_read])
    throw Exception(ErrorCodes::TOO_MANY_COLUMNS, "Limit for number of columns to read exceeded. Requested: {}, maximum: {}",
                    required_columns.size(), settings[Setting::max_columns_to_read].value);
  ...
  if (UInt64 max_block_limited = maxBlockSizeByLimit()) {
    ...
    if (local_limits.local_limits.size_limits.max_rows != 0) {
      ...
      else if (local_limits.local_limits.size_limits.max_rows <
               std::numeric_limits<UInt64>::max())  /// Ask to read just enough rows to make the max_rows limit effective (so it has a
  ...
  if (!max_block_size) throw Exception(ErrorCodes::PARAMETER_OUT_OF_BOUND, "Setting 'max_block_size' cannot be zero");
  ...
  if (query_plan.isInitialized()) {
    ...
    if (!subquery) throw Exception(ErrorCodes::LOGICAL_ERROR, "Subquery expected");

    interpreter_subquery = std::make_unique<InterpreterSelectWithUnionQuery>(subquery, getSubqueryContext(context),
    ...
    if (analysis_result.prewhere_info) query_info.prewhere_info = analysis_result.prewhere_info;
    ...
    if (optimize_read_in_order) {
      query_info.order_optimizer = std::make_shared<ReadInOrderOptimizer>(
          query, analysis_result.order_by_elements_actions, getSortDescription(query, context), query_info.syntax_analyzer_result);
      ...
      query_info.order_optimizer = std::make_shared<ReadInOrderOptimizer>(
          query, analysis_result.group_by_elements_actions, getSortDescriptionFromGroupBy(query), query_info.syntax_analyzer_result);
    ...
    if (context->hasQueryContext() && !options.is_internal) {
      ...
      context->getQueryContext()->addQueryAccessInfo(backQuoteIfNeed(local_storage_id.getDatabaseName()),
                                                     local_storage_id.getFullTableName(), required_columns);
    }
  ...
  if (!query_plan.getMaxThreads() || is_sync_remote) query_plan.setMaxThreads(max_threads_execute_query);
...
void InterpreterSelectQuery::executeWhere(QueryPlan &query_plan, const ActionsAndProjectInputsFlagPtr &expression, bool remove_filter) {
  ...
  auto where_step =
      std::make_unique<FilterStep>(query_plan.getCurrentHeader(), std::move(dag), getSelectQuery().where()->getColumnName(), remove_filter);

  where_step->setStepDescription("WHERE");
  query_plan.addStep(std::move(where_step));
}

static Aggregator::Params getAggregatorParams(const ASTPtr &query_ptr, const SelectQueryExpressionAnalyzer &query_analyzer,
                                              const Context &context, const Names &keys, const AggregateDescriptions &aggregates,
                                              bool overflow_row, const Settings &settings, size_t group_by_two_level_threshold,
                                              size_t group_by_two_level_threshold_bytes) {
  const auto stats_collecting_params =
      StatsCollectingParams(calculateCacheKey(query_ptr), settings[Setting::collect_hash_table_stats_during_aggregation],
  ...
  return Aggregator::Params{
      ...
      Aggregator::Params::getMaxBytesBeforeExternalGroupBy(settings[Setting::max_bytes_before_external_group_by],
...
void InterpreterSelectQuery::executeAggregation(QueryPlan &query_plan, const ActionsAndProjectInputsFlagPtr &expression, bool overflow_row,
                                                bool final, InputOrderInfoPtr group_by_info) {
  ...
  auto aggregator_params =
      getAggregatorParams(query_ptr, *query_analyzer, *context, keys, aggregates, overflow_row, settings,
                          settings[Setting::group_by_two_level_threshold], settings[Setting::group_by_two_level_threshold_bytes]);
  ...
  const bool should_produce_results_in_order_of_bucket_number = options.to_stage == QueryProcessingStage::WithMergeableState &&
                                                                (settings[Setting::distributed_aggregation_memory_efficient] ||
                                                                 settings[Setting::enable_memory_bound_merging_of_aggregation_results]);

  auto aggregating_step = std::make_unique<AggregatingStep>(
      query_plan.getCurrentHeader(), std::move(aggregator_params), std::move(grouping_sets_params), final,
      settings[Setting::max_block_size], settings[Setting::aggregation_in_order_max_block_bytes], merge_threads,
      temporary_data_merge_threads, storage_has_evenly_distributed_read, settings[Setting::group_by_use_nulls],
      std::move(sort_description_for_merging), std::move(group_by_sort_description), should_produce_results_in_order_of_bucket_number,
      settings[Setting::enable_memory_bound_merging_of_aggregation_results],
...
void InterpreterSelectQuery::executeMergeAggregated(QueryPlan &query_plan, bool overflow_row, bool final, bool has_grouping_sets) {
  ...
  executeMergeAggregatedImpl(query_plan, overflow_row, final, storage && (storage->isRemote() || parallel_replicas_from_merge_tree),
                             has_grouping_sets, context->getSettingsRef(), query_analyzer->aggregationKeys(),
                             query_analyzer->aggregationKeysList(), query_analyzer->aggregates(),
...
void InterpreterSelectQuery::executeHaving(QueryPlan &query_plan, const ActionsAndProjectInputsFlagPtr &expression, bool remove_filter) {
  ...
  auto having_step = std::make_unique<FilterStep>(query_plan.getCurrentHeader(), std::move(dag), getSelectQuery().having()->getColumnName(),
                                                  remove_filter);
...
void InterpreterSelectQuery::executeTotalsAndHaving(QueryPlan &query_plan, bool has_having,
                                                    const ActionsAndProjectInputsFlagPtr &expression, bool remove_filter, bool overflow_row,
                                                    bool final) {
  ...
  auto totals_having_step =
      std::make_unique<TotalsHavingStep>(query_plan.getCurrentHeader(), query_analyzer->aggregates(), overflow_row, std::move(dag),
                                         has_having ? getSelectQuery().having()->getColumnName() : "", remove_filter,
                                         settings[Setting::totals_mode], settings[Setting::totals_auto_threshold], final);
...
void InterpreterSelectQuery::executeRollupOrCube(QueryPlan &query_plan, Modificator modificator) {
  ...
  // Arguments will not be present in Rollup / Cube input header and they don't actually needed 'cause these steps will work with
  // AggregateFunctionState-s anyway.
  auto aggregates = query_analyzer->aggregates();
  for (auto &aggregate : aggregates) aggregate.argument_names.clear();
...
static bool windowDescriptionComparator(const WindowDescription *_left, const WindowDescription *_right) {
  ...
  for (size_t i = 0; i < std::min(left.size(), right.size()); ++i) {
    if (left[i].column_name < right[i].column_name) return true;
    if (left[i].column_name > right[i].column_name) return false;
    if (left[i].direction < right[i].direction) return true;
    if (left[i].direction > right[i].direction) return false;
    if (left[i].nulls_direction < right[i].nulls_direction) return true;
    if (left[i].nulls_direction > right[i].nulls_direction) return false;
...
void InterpreterSelectQuery::executeWindow(QueryPlan &query_plan) {
  ...
  for (const auto &[_, window] : query_analyzer->windowDescriptions()) windows_sorted.push_back(&window);
  ...
  for (size_t i = 0; i < windows_sorted.size(); ++i) {
    ...
    if (need_sort) {
      ...
      auto sorting_step = std::make_unique<SortingStep>(query_plan.getCurrentHeader(), window.full_sort_description, window.partition_by,
...
void InterpreterSelectQuery::executeOrderOptimized(QueryPlan &query_plan, InputOrderInfoPtr input_sorting_info, UInt64 limit,
                                                   SortDescription &output_order_descr) {
  const Settings &settings = context->getSettingsRef();

  auto finish_sorting_step = std::make_unique<SortingStep>(query_plan.getCurrentHeader(), input_sorting_info->sort_description_for_merging,
                                                           output_order_descr, settings[Setting::max_block_size], limit);
...
void InterpreterSelectQuery::executeOrder(QueryPlan &query_plan, InputOrderInfoPtr input_sorting_info) {
  ...
  auto sorting_step = std::make_unique<SortingStep>(query_plan.getCurrentHeader(), output_order_descr, limit, sort_settings);
...
void InterpreterSelectQuery::executeMergeSorted(QueryPlan &query_plan, const std::string &description) {
  ...
  auto merging_sorted = std::make_unique<SortingStep>(query_plan.getCurrentHeader(), std::move(sort_description), max_block_size, limit,
...
void InterpreterSelectQuery::executeDistinct(QueryPlan &query_plan, bool before_order, Names columns, bool pre_distinct) {
  ...
  if (query.distinct) {
    ...
    SizeLimits limits(settings[Setting::max_rows_in_distinct], settings[Setting::max_bytes_in_distinct],
                      settings[Setting::distinct_overflow_mode]);

    auto distinct_step = std::make_unique<DistinctStep>(query_plan.getCurrentHeader(), limits, limit_for_distinct, columns, pre_distinct);

    if (pre_distinct) distinct_step->setStepDescription("Preliminary DISTINCT");
...
void InterpreterSelectQuery::executePreLimit(QueryPlan &query_plan, bool do_not_skip_offset) {
  ...
  if (query.limitLength()) {
    ...
    if (do_not_skip_offset) {
      if (lim_info.limit_length > std::numeric_limits<UInt64>::max() - lim_info.limit_offset) return;
    ...
    if (!lim_info.is_limit_length_negative && !lim_info.is_limit_offset_negative) [[likely]] {
      auto limit = std::make_unique<LimitStep>(query_plan.getCurrentHeader(), lim_info.limit_length, lim_info.limit_offset,
    ...
    } else  // if (!lim_info.is_limit_length_negative && lim_info.is_limit_offset_negative)
    {
      ...
      auto limit =
          std::make_unique<LimitStep>(query_plan.getCurrentHeader(), lim_info.limit_length, 0, settings[Setting::exact_rows_before_limit]);
...
void InterpreterSelectQuery::executeLimitBy(QueryPlan &query_plan) {
  auto &query = getSelectQuery();
  if (!query.limitByLength() || !query.limitBy()) return;

  Names columns;
  for (const auto &elem : query.limitBy()->children) columns.emplace_back(elem->getColumnName());

  auto [limit_length, fractional_limit, is_limit_length_negative] = getLimitOffsetValue(query.limitByLength(), context, "LIMIT");
...
void InterpreterSelectQuery::executeWithFill(QueryPlan &query_plan) {
  ...
  if (query.orderBy()) {
    ...
    for (auto &desc : sort_description) {
      if (desc.with_fill) fill_description.push_back(desc);
    }

    if (fill_description.empty()) return;
    ...
    const Settings &settings = context->getSettingsRef();
    auto filling_step =
        std::make_unique<FillingStep>(query_plan.getCurrentHeader(), std::move(sort_description), std::move(fill_description),
                                      interpolate_descr, settings[Setting::use_with_fill_by_sorting_prefix]);
...
void InterpreterSelectQuery::executeLimit(QueryPlan &query_plan) {
  ...
  if (query.limitLength()) {
    ...
    if (query.group_by_with_totals && !query.orderBy()) always_read_till_end = true;

    if (!query.group_by_with_totals && hasWithTotalsInAnySubqueryInFromClause(query)) always_read_till_end = true;

    const LimitInfo lim_info = getLimitLengthAndOffset(query, context);

    SortDescription order_descr;
    if (query.limit_with_ties) {
      if (!query.orderBy()) throw Exception(ErrorCodes::LOGICAL_ERROR, "LIMIT WITH TIES without ORDER BY");
    ...
    if (!lim_info.is_limit_length_negative && !lim_info.is_limit_offset_negative && lim_info.fractional_offset == 0 &&
        lim_info.fractional_limit == 0) [[likely]] {
      auto limit = std::make_unique<LimitStep>(query_plan.getCurrentHeader(), lim_info.limit_length, lim_info.limit_offset,
                                               always_read_till_end, query.limit_with_ties, order_descr);

      if (query.limit_with_ties) limit->setStepDescription("LIMIT WITH TIES");
      ...
      auto limit = std::make_unique<LimitStep>(query_plan.getCurrentHeader(), lim_info.limit_length, 0, always_read_till_end,
                                               query.limit_with_ties, order_descr);
      ...
      auto fractional_limit_step =
          std::make_unique<FractionalLimitStep>(query_plan.getCurrentHeader(), lim_info.fractional_limit, lim_info.fractional_offset,
                                                lim_info.limit_offset, query.limit_with_ties, order_descr);
...
void InterpreterSelectQuery::executeExtremes(QueryPlan &query_plan) {
  if (!context->getSettingsRef()[Setting::extremes]) return;
...
void InterpreterSelectQuery::executeSubqueriesInSetsAndJoins(QueryPlan &query_plan) {
  ...
  if (!subqueries.empty()) {
    const auto &settings = context->getSettingsRef();
    SizeLimits network_transfer_limits(settings[Setting::max_rows_to_transfer], settings[Setting::max_bytes_to_transfer],
                                       settings[Setting::transfer_overflow_mode]);
    auto prepared_sets_cache = context->getPreparedSetsCache();

    auto step = std::make_unique<DelayedCreatingSetsStep>(query_plan.getCurrentHeader(), std::move(subqueries), network_transfer_limits,
                                                          prepared_sets_cache);
...
void InterpreterSelectQuery::ignoreWithTotals() { getSelectQuery().group_by_with_totals = false; }
...
void InterpreterSelectQuery::initSettings() {
  auto &query = getSelectQuery();
  if (query.settings()) InterpreterSetQuery(query.settings(), context).executeForCurrentContext(options.ignore_setting_constraints);
...
bool InterpreterSelectQuery::isQueryWithFinal(const SelectQueryInfo &info) {
  ...
  if (info.table_expression_modifiers) result |= info.table_expression_modifiers->hasFinal();
