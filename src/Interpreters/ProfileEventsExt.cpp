...
void dumpToMapColumn(const Counters::Snapshot &counters, DB::IColumn *column, bool nonzero_only) {
  ...
  for (Event event = Event(0); event < Counters::num_counters; ++event) {
    ...
    if (nonzero_only && 0 == value) continue;
...
DB::Block getSampleBlock() {
  ...
  static const NamesAndTypesList column_names_and_types = {
      {"host_name", std::make_shared<DataTypeString>()}, {"current_time", std::make_shared<DataTypeDateTime>()},
      {"thread_id", std::make_shared<DataTypeUInt64>()}, {"type", TypeEnum},
      {"name", std::make_shared<DataTypeString>()},      {"value", std::make_shared<DataTypeInt64>()},
  };

  ColumnsWithTypeAndName temp_columns;
  for (auto const &name_and_type : column_names_and_types) temp_columns.emplace_back(name_and_type.type, name_and_type.name);

  return Block(std::move(temp_columns));
}

DB::Block getProfileEvents(const String &host_name, DB::InternalProfileEventsQueuePtr profile_queue,
                           ThreadIdToCountersSnapshot &last_sent_snapshots) {
  using namespace DB;

  if (!CurrentThread::isInitialized()) throw Exception(ErrorCodes::LOGICAL_ERROR, "CurrentThread is not initialized");

  auto thread_group = CurrentThread::getGroup();

  if (!thread_group) throw Exception(ErrorCodes::LOGICAL_ERROR, "Current thread is not attached to any thread group");
  ...
  ProfileEventsSnapshot group_snapshot;
  {
    ...
    group_snapshot.counters = prev_group_snapshot != last_sent_snapshots.end()
  ...
  while (profile_queue && profile_queue->tryPop(curr_block)) {
    ...
    for (size_t j = 0; j < curr_columns.size(); ++j) columns[j]->insertRangeFrom(*curr_columns[j], 0, curr_columns[j]->size());
  }

  bool empty = columns[0]->empty();
  if (!empty) block.setColumns(std::move(columns));
