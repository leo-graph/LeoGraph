...
class ClusterDiscovery {
 public:
  ClusterDiscovery(const Poco::Util::AbstractConfiguration& config, ContextPtr context_, MultiVersion<Macros>::Version macros_,
 ...
 private:
  struct NodeInfo {
    ...
    NodeInfo() = default;
    explicit NodeInfo(const String& address_, bool secure_, size_t shard_id_) : address(address_), secure(secure_), shard_id(shard_id_) {}
  ...
  struct ClusterInfo {
    ...
    ClusterInfo(const String& name_, const String& zk_name_, const String& zk_root_, const String& host_name, const String& username_,
                const String& password_, const String& cluster_secret_, UInt16 port, bool secure, size_t shard_id, bool observer_mode,
                bool invisible, size_t zk_root_index_ = 0);
  };
  ...
  Strings getNodeNames(zkutil::ZooKeeperPtr& zk, const String& zk_root, const String& cluster_name, int* version, bool set_callback,
                       size_t zk_root_index);
  ...
  struct MulticlusterDiscovery {
    ...
    MulticlusterDiscovery(const String& zk_name_, const String& zk_path_, bool is_secure_connection_, const String& username_,
                          const String& password_, const String& cluster_secret_)
        : zk_name(zk_name_),
          zk_path(zk_path_),
