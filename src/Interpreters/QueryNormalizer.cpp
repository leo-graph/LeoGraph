...
class CheckASTDepth {
 public:
  explicit CheckASTDepth(QueryNormalizer::Data &data_) : data(data_) {
...
class RestoreAliasOnExitScope {
 public:
  explicit RestoreAliasOnExitScope(String &alias_) : alias(alias_), copy(alias_) {}

  ~RestoreAliasOnExitScope() { alias = copy; }
...
void QueryNormalizer::visit(ASTIdentifier &node, ASTPtr &ast, Data &data) {
  ...
  if (!IdentifierSemantic::getColumnName(node)) return;

  if (data.settings.prefer_column_name_to_alias) {
    if (data.source_columns_set.contains(node.name())) return;
  }

  /// If it is an alias, but not a parent alias (for constructs like "SELECT column + 1 AS column").
  if (!data.allow_self_aliases && current_alias == node.name())
    throw Exception(ErrorCodes::CYCLIC_ALIASES, "Self referencing of {} to {}. Cyclic alias", backQuote(current_alias),
                    backQuote(node.name()));
  auto it_alias = data.aliases.find(node.name());

  if (it_alias != data.aliases.end() && current_alias != node.name()) {
    if (!IdentifierSemantic::canBeAlias(node)) return;
    ...
    if (!node_alias.empty() && node_alias != our_alias_or_name) {
      ...
      if (!is_cycle) {
        ...
        if (data.finished_asts.contains(alias_node)) data.finished_asts[ast] = ast;

        /// If we had an alias for node_alias, point it instead to the new node so we don't have to revisit it
        /// on subsequent calls
        if (auto existing_alias = data.aliases.find(node_alias); existing_alias != data.aliases.end()) existing_alias->second = ast;
      }
    } else {
      /// Check size of the alias before cloning too large alias AST
      alias_node->checkSize(data.settings.max_expanded_ast_elements);
...
void QueryNormalizer::visit(ASTTablesInSelectQueryElement &node, const ASTPtr &, Data &data) {
  ...
  if (node.table_join) {
    ...
    if (join.on_expression) {
      ...
      if (join.on_expression != original_on_expression) join.children = {join.on_expression};
    }
  }
}
...
void QueryNormalizer::visit(ASTSelectQuery &select, const ASTPtr &, Data &data) {
  for (auto &child : select.children) {
    if (needVisitChild(child)) visit(child, data);
  }

  /// If the WHERE clause or HAVING consists of a single alias, the reference must be replaced not only in children,
  /// but also in where_expression and having_expression.
  if (select.prewhere()) visit(select.refPrewhere(), data);
  if (select.where()) visit(select.refWhere(), data);
  if (select.having()) visit(select.refHaving(), data);
}
...
void QueryNormalizer::visitChildren(IAST *node, Data &data) {
  if (auto *func_node = node->as<ASTFunction>()) {
    ...
    size_t first_pos = 0;
    if (func_node->name == "lambda") first_pos = 1;
...
void QueryNormalizer::visit(ASTPtr &ast, Data &data) {
...

  {
    ...
    if (!my_alias.empty()) data.current_alias = my_alias;
  }
  ...
  if (auto *node_id = ast->as<ASTIdentifier>())
    ...
    if (node_function->parameters) visit(node_function->parameters, data);
  ...
  if (data.ignore_alias && !ast->tryGetAlias().empty()) ast->setAlias("");
