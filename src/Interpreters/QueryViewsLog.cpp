...
ColumnsDescription QueryViewsLogElement::getColumnsDescription() {
  ...
  auto view_type_datatype =
      std::make_shared<DataTypeEnum8>(DataTypeEnum8::Values{{"Default", static_cast<Int8>(ViewType::DEFAULT)},
  ...
  return ColumnsDescription{
      ...
      {"event_time_microseconds", std::make_shared<DataTypeDateTime64>(6),
       "The date and time when the view finished execution with microseconds precision."},
      ...
      {"view_type", std::move(view_type_datatype),
       "Type of the view. Values: 'Default' = 1 — Default views. Should not appear in this log, 'Materialized' = 2 — Materialized views, "
       "'Live' = 3 — Live views."},
      ...
      {"peak_memory_usage", std::make_shared<DataTypeInt64>(),
       "The maximum difference between the amount of allocated and freed memory in context of this view."},
...
NamesAndAliases QueryViewsLogElement::getNamesAndAliases() {
  return {{"ProfileEvents.Names", {std::make_shared<DataTypeArray>(std::make_shared<DataTypeString>())}, "mapKeys(ProfileEvents)"},
