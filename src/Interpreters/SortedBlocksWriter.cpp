...
TemporaryBlockStreamHolder flushToFile(const TemporaryDataOnDiskScopePtr &tmp_data, const Block &header, QueryPipelineBuilder pipeline) {
  ...
  Block block;
  while (executor.pull(block)) stream_holder->write(block);
...
TemporaryBlockStreamHolder SortedBlocksWriter::flush(const BlocksList &blocks) const {
  ...
  for (const auto &block : blocks)
    if (auto num_rows = block.rows())
      pipes.emplace_back(
          std::make_shared<SourceFromSingleChunk>(std::make_shared<const Block>(block.cloneEmpty()), Chunk(block.getColumns(), num_rows)));

  if (pipes.empty()) throw Exception(ErrorCodes::LOGICAL_ERROR, "Empty block");
  ...
  if (pipeline.getNumStreams() > 1) {
    auto transform =
        std::make_shared<MergingSortedTransform>(pipeline.getSharedHeader(), pipeline.getNumStreams(), sort_description, rows_in_block,
...
class TemporaryFileLazySource : public ISource {
 public:
  explicit TemporaryFileLazySource(TemporaryBlockStreamReaderHolder reader_)
      : ISource(std::make_shared<const Block>(reader_->getHeader()), true), reader(std::move(reader_)), done(false) {}
...
SortedBlocksWriter::PremergedFiles SortedBlocksWriter::premerge() {
  ...
  /// flush not flushed
  if (!blocks.empty()) files.emplace_back(flush(blocks));
  ...
  /// Merge by parts to save memory. It's possible to exchange disk I/O and memory by num_files_for_merge.
  {
        ...
        if (pipes.size() == num_files_for_merge || &file == &files.back()) {
          ...
          if (pipeline.getNumStreams() > 1) {
            auto transform = std::make_shared<MergingSortedTransform>(
                pipeline.getSharedHeader(), pipeline.getNumStreams(), sort_description, rows_in_block,
                /*max_block_size_bytes=*/0,
                /*max_dynamic_subcolumns=*/std::nullopt, SortingQueueStrategy::Default);
    ...
    for (const auto &file : files) pipes.emplace_back(streamFromFile(file));
  }
...
Block SortedBlocksBuffer::exchange(Block &&block) {
...

  {
    ...
    if (!block.empty()) {
      ...
      if (current_bytes < max_bytes) return empty_out;
    }
...
Block SortedBlocksBuffer::mergeBlocks(Blocks &&blocks) const {
  ...
  {  /// Merge sort blocks
    ...
    if (builder.getNumStreams() > 1) {
      auto transform =
          std::make_shared<MergingSortedTransform>(builder.getSharedHeader(), builder.getNumStreams(), sort_description, num_rows,
                                                   /*max_block_size_bytes=*/0,
                                                   /*max_dynamic_subcolumns=*/std::nullopt, SortingQueueStrategy::Default);
    ...
    PullingPipelineExecutor executor(pipeline);
    Block block;
    while (executor.pull(block)) tmp_blocks.emplace_back(block);
