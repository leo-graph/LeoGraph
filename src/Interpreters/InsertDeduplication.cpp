...
DeduplicationHash::DeduplicationHash(UInt128 hash_, std::string partition_id_, HashType htype)
    : hash(std::move(hash_)), partition_id(std::move(partition_id_)), hash_type(htype) {}
...
std::string DeduplicationHash::getBlockId() const { return fmt::format("{}_{}_{}", partition_id, hash.items[0], hash.items[1]); }
...
void DeduplicationHash::setConflictPartName(const std::string &part_name) { conflicted_part_name = part_name; }

bool DeduplicationHash::hasConflictPartName() const { return conflicted_part_name.has_value(); }
...
DeduplicationInfo::FilterResult DeduplicationInfo::deduplicateSelf(bool deduplication_enabled, const std::string &partition_id,
...
DeduplicationInfo::FilterResult DeduplicationInfo::recalculateBlock(DeduplicationInfo::FilterResult &&filtered,
                                                                    const std::string &partition_id, ContextPtr context) const {
  if (filtered.removed_rows == 0) return {};
...
std::set<size_t> DeduplicationInfo::filterSelf(const String &partition_id) const {
  ...
  for (auto &[_, block_offsets] : block_id_to_offsets) {
    if (block_offsets.size() > 1) fitered_offsets.insert(block_offsets.begin() + 1, block_offsets.end());
  }
...
DeduplicationInfo::FilterResult DeduplicationInfo::filterImpl(const std::set<size_t> &collision_offsets) const {
  if (collision_offsets.empty()) return {};
  ...
  for (size_t i = 0; i < offsets.size(); ++i) {
    if (collision_offsets.contains(i)) {
      ...
      removed_tokens += 1;
      for (auto row_id = getTokenBegin(i); row_id < getTokenEnd(i); ++row_id) filer_column[row_id] = 0;
    } else {
  ...
  if (removed_rows == block.rows()) {
    ...
    LOG_DEBUG(logger, "All {} rows are removed due to duplicate, debug: {}", block.rows(), debug());

    return {.filtered_block = new_tokens->original_block,
            .deduplication_info = new_tokens,
  ...
  for (auto &col : cols) col = col->filter(filer_column, block.rows() - removed_rows);
  ...
  LOG_DEBUG(logger, "Filtered {}/{} rows with {}/{} tokens as duplicates, remaining rows/count={}/{}, debug: {}", removed_rows,
            block.rows(), collision_offsets.size(), getCount(), new_tokens->original_block->rows(), new_tokens->getCount(), debug());
...
UInt128 DeduplicationInfo::calculateDataHash(size_t offset, const Block &block) const {
  ...
  if (tokens[offset].data_hash.has_value()) return tokens[offset].data_hash.value();
  ...
  for (size_t j = getTokenBegin(offset); j < getTokenEnd(offset); ++j) {
    for (const auto &col : cols) col->updateHashWithValue(j, hash);
  }

  /// be careful, hash.get128() method is not const because of caching of calculated hash in token, so it can return different results on
  /// multiple calls
...
DeduplicationHash DeduplicationInfo::getBlockHash(size_t offset, const std::string &partition_id) const {
  ...
  // for other token sources addition parts are appended
  for (const auto &extra : token.extra_tokens) {
    if (extra.type == TokenDefinition::Extra::SOURCE_ID) continue;  // do not include source id
...
std::unordered_map<std::string, std::vector<size_t>> DeduplicationInfo::buildBlockIdToOffsetsMap(const std::string &partition_id) const {
  ...
  for (size_t offset = 0; offset < offsets.size(); ++offset) {
    for (auto &block_hash : chooseDeduplicationHashes(offset, partition_id)) result[block_hash.getBlockId()].push_back(offset);
  }
...
std::vector<DeduplicationHash> DeduplicationInfo::getDeduplicationHashes(const std::string &partition_id,
                                                                         bool deduplication_enabled) const {
  if (disabled || !deduplication_enabled) return {};

  std::vector<DeduplicationHash> result;
  result.reserve(2 * offsets.size());

  for (size_t offset = 0; offset < offsets.size(); ++offset) {
    for (auto &block_hash : chooseDeduplicationHashes(offset, partition_id)) result.push_back(std::move(block_hash));
  }

  return result;
}

size_t DeduplicationInfo::getCount() const { return offsets.size(); }
...
std::string DeduplicationInfo::debug() const {
  ...
  return fmt::format(
      "instance_id: {}, {}, {}, level {}, rows/tokens {}/{}, in block: {}, tokens: {}:[{}], visited views: {}:[{}], retried view id: {}, "
      "original block id: {}, data_hashes: {}, unification_stage {}",
      instance_id, is_async_insert ? "async" : "sync", disabled ? "disabled" : "enabled", level, getRows(), getCount(), block_str,
      getCount(), fmt::join(token_strs, ","), visited_views.size(), fmt::join(visited_views, ","), retried_view_id, original_block_view_id,
      fmt::join(data_hashes, ","), unification_stage);
}
...
DeduplicationInfo::Ptr DeduplicationInfo::cloneSelf() const { return std::make_shared<DeduplicationInfo>(*this); }

ChunkInfo::Ptr DeduplicationInfo::clone() const { return std::static_pointer_cast<ChunkInfo>(cloneSelf()); }

void DeduplicationInfo::setPartWriterHashForPartition(UInt128 hash, size_t /* count */) const {
  if (disabled) return;

  if (level != Level::SOURCE) return;

  if (is_async_insert) return;

  chassert(getCount() >= 1);

  if (getCount() > 1) return;

  if (!tokens[0].empty()) return;
...
void DeduplicationInfo::redefineTokensWithDataHash(const Block &block) {
  ...
  if (disabled || level != Level::SOURCE) return;
...
DeduplicationInfo::DeduplicationInfo(const DeduplicationInfo &other)
  ...
  if (!disabled) LOG_TEST(logger, "Clone DeduplicationInfo {} from {}", instance_id, other.debug());
}
...
void DeduplicationInfo::truncateTokensForRetry() {
  ...
  for (auto &token : tokens) {
    auto it = std::find_if(token.extra_tokens.begin(), token.extra_tokens.end(), [&](const TokenDefinition::Extra &extra) {
    ...
    if (it == token.extra_tokens.end())
      throw Exception(
          ...
          original_block_view_id, debug());

    auto next = ++it;
    if (next == token.extra_tokens.end() ||
        (next->type != TokenDefinition::Extra::Type::SOURCE_NUMBER && next->type != TokenDefinition::Extra::Type::VIEW_NUMBER))
...
Block DeduplicationInfo::goRetry(SharedHeader &&header, Chunk &&filtered_data, Ptr filtered_info, const std::string &partition_id,
                                 ContextPtr context) const {
  ...
  if (is_empty) return result_header->cloneEmpty();
  ...
  if (result_chunks.size() > 1) {
    throw Exception(ErrorCodes::LOGICAL_ERROR,
                    ...
                    result_chunks.size(), filtered_info->debug());
  }

  if (result_chunks.empty()) return result_header->cloneEmpty();
...
void DeduplicationInfo::updateOriginalBlock(const Chunk &chunk, SharedHeader header) {
  ...
  if (!original_block) original_block_view_id = visited_views.back();
  ...
  if (!is_async_insert && getCount() == 1) {
    ...
    /// if there is a duplicate is found in the original block then we tottaly filter out all rows in the block and original block will be
    /// not used at all
    ...
    if (unification_stage != InsertDeduplicationVersions::OLD_SEPARATE_HASHES) {
      ...
      /// it is enough to call calculateDataHash for one of tokens, the hash would be saved for this token in `data_hash` field and used
      ...
      LOG_TEST(
          ...
          block.columns(), block.rows(), debug());
    }
...
void DeduplicationInfo::setRootViewID(const StorageIDMaybeEmpty &id) {
  ...
  if (!insert_dependencies || !insert_dependencies->deduplicate_blocks) disabled = true;
...
void DeduplicationInfo::setViewID(const StorageID &id) {
  if (level == Level::SOURCE) level = Level::VIEW;
...
void DeduplicationInfo::setViewBlockNumber(size_t block_number) {
  ...
  if (!is_async_insert || disabled) return;

  if (block_number > 0)
    throw Exception(ErrorCodes::NOT_IMPLEMENTED,
                    "The deduplication with async insert is only supported for the materialized view with the inner query which produces "
                    "no more rows than in the input. "
                    " You can either use sync inserts with deduplicaton or disable deduplication in dependent materialised views with "
                    "async insert (deduplicate_blocks_in_dependent_materialized_views=0). "
                    " Inner query of the materialized view '{}' produced more that one block. "
                    " debug: {}",
                    visited_views.back(), debug());
}

DeduplicationInfo::FilterResult DeduplicationInfo::deduplicateBlock(const std::vector<std::string> &existing_block_ids,
                                                                    const std::string &partition_id, ContextPtr context) const {
...
const std::vector<StorageIDMaybeEmpty> &DeduplicationInfo::getVisitedViews() const { return visited_views; }
...
size_t DeduplicationInfo::getTokenRows(size_t pos) const { return getTokenEnd(pos) - getTokenBegin(pos); }

void DeduplicationInfo::addExtraPart(const TokenDefinition::Extra &extra) {
  for (auto &token : tokens) token.extra_tokens.push_back(extra);
}

bool DeduplicationInfo::TokenDefinition::empty() const { return by_user.empty() && !by_part_writer.has_value(); }

bool DeduplicationInfo::TokenDefinition::operator==(const TokenDefinition &other) const {
  return by_user == other.by_user && by_part_writer == other.by_part_writer && data_hash == other.data_hash &&
...
DeduplicationInfo::Ptr DeduplicationInfo::mergeSelf(const Ptr &right) const {
  ...
  if (!disabled) LOG_TEST(logger, "Merging:\n left: {}\n right: {}\n", debug(), right->debug());
  ...
  if (!disabled && is_async_insert && visited_views.size() > 1)
    throw Exception(ErrorCodes::LOGICAL_ERROR,
                    "Merging deduplication infos with more than one visited view is not supported with deduplication on async insert, "
                    "left: {}, right: {}",
                    debug(), right->debug());

  if (!disabled && !is_async_insert && visited_views.size() > 2)
    throw Exception(ErrorCodes::LOGICAL_ERROR,
                    "Merging deduplication infos with more than two visited view is not supported with deduplication on sync insert, left: "
                    "{}, right: {}",
                    debug(), right->debug());
  ...
  auto do_extend = [&]() {
    ...
    if (new_instance->level == Level::SOURCE)
      new_instance->tokens.back().data_hash.reset();  // reset data hash because the last block has changed and data hash should be
                                                      // recalculated later if it is needed
  };
  ...
  auto do_concat = [&]() {
    ...
    for (const auto &offset : right->offsets) new_instance->offsets.push_back(rows + offset);
  ...
  if (!disabled) LOG_TEST(logger, "Merged: {}", new_instance->debug());
...
std::string DeduplicationInfo::TokenDefinition::Extra::debug() const { return toStringImpl(true); }

std::string DeduplicationInfo::TokenDefinition::Extra::toString() const { return toStringImpl(false); }
...
bool DeduplicationInfo::TokenDefinition::canBeExtended(const TokenDefinition &right) const {
  if (by_user != right.by_user || by_part_writer != right.by_part_writer) return false;

  if (extra_tokens.size() != right.extra_tokens.size()) return false;

  if (extra_tokens.empty()) return true;

  const auto &left_last_extra = extra_tokens.back();
  const auto &right_last_extra = right.extra_tokens.back();

  if (left_last_extra.type != right_last_extra.type) return false;
  ...
  if (left_last_extra != right_last_extra) {
    ...
    switch (left_last_extra.type) {
      ...
      case Extra::Type::VIEW_NUMBER: {
        ...
        if (left_range.second == right_range.first) break;  // continue and check other extras
        return false;
      }
    }
  }
  ...
  for (size_t i = 0; i < extra_tokens.size() - 1; ++i) {
    if (extra_tokens[i] != right.extra_tokens[i]) return false;
  }
...
void DeduplicationInfo::TokenDefinition::doExtend(const TokenDefinition &right) {
  ...
  if (extra_tokens.empty()) return;

  auto &left_last_extra = extra_tokens.back();
  const auto &right_last_extra = right.extra_tokens.back();

  if (left_last_extra == right_last_extra) return;
