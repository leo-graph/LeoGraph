...
std::pair<ColumnsWithTypeAndName, bool> getFunctionArguments(const ActionsDAG::NodeRawConstPtrs &children) {
  ...
  for (size_t i = 0; i < num_arguments; ++i) {
    ...
    if (!argument.column || !isColumnConst(*argument.column)) all_const = false;
...
bool isConstantFromScalarSubquery(const ActionsDAG::Node *node) {
  ...
  while (!stack.empty()) {
    ...
    if (arg->column && isColumnConst(*arg->column)) continue;

    while (arg->type == ActionsDAG::ActionType::ALIAS) arg = arg->children.at(0);

    if (arg->type != ActionsDAG::ActionType::FUNCTION) return false;

    if (arg->function_base->getName() == "__scalarSubqueryResult") continue;

    if (arg->children.empty() || !arg->function_base->isSuitableForConstantFolding()) return false;

    for (const auto *child : arg->children) stack.push(child);
  }
...
void ActionsDAG::Node::toTree(JSONBuilder::JSONMap &map) const {
  ...
  if (result_type) map.add("Result Type", result_type->getName());

  if (!result_name.empty()) map.add("Result Name", result_name);

  if (column) map.add("Column", column->getName());

  if (function_base) map.add("Function", function_base->getName());

  if (type == ActionType::FUNCTION) map.add("Compiled", is_function_compiled);
}
...
void ActionsDAG::Node::updateHash(SipHash &hash_state) const {
  ...
  if (!result_name.empty()) hash_state.update(result_name);

  if (result_type) hash_state.update(result_type->getName());

  if (function_base) hash_state.update(function_base->getName());

  if (function) hash_state.update(function->getName());
  ...
  if (column) {
    ...
    if (isColumnConst(*column)) column->updateHashWithValue(0, hash_state);
  }

  for (const auto &child : children) child->updateHash(hash_state);
}
...
void ActionsDAG::updateHash(SipHash &hash_state) const {
  ...
  for (const auto &node : outputs) stack.push({.node = node});
...
ActionsDAG::ActionsDAG(const NamesAndTypesList &inputs_) {
  for (const auto &input : inputs_) outputs.push_back(&addInput(input.name, input.type));
}
...
ActionsDAG::Node &ActionsDAG::addNode(Node node) {
  ...
  if (res.type != ActionType::PLACEHOLDER) {
    ...
    if (!valid_column) res.column = nullptr;
  }

  if (res.type == ActionType::INPUT) inputs.emplace_back(&res);
...
const ActionsDAG::Node &ActionsDAG::addColumn(ColumnWithTypeAndName column, bool is_deterministic_constant) {
  if (!column.column) throw Exception(ErrorCodes::LOGICAL_ERROR, "Cannot add column {} because it is nullptr", column.name);
...
const ActionsDAG::Node &ActionsDAG::addAlias(const Node &child, std::string alias) { return addNode(createAlias(child, std::move(alias))); }
...
const ActionsDAG::Node &ActionsDAG::addArrayJoin(const Node &child, std::string result_name) {
  ...
  if (!array_type) throw Exception(ErrorCodes::TYPE_MISMATCH, "ARRAY JOIN requires array argument");

  if (result_name.empty()) result_name = "arrayJoin(" + child.result_name + ")";
...
const ActionsDAG::Node &ActionsDAG::addFunction(const FunctionOverloadResolverPtr &function, NodeRawConstPtrs children,
  ...
  for (size_t pos : constant_args) {
    if (pos >= children.size()) continue;

    if (arguments[pos].column && isColumnConst(*arguments[pos].column)) continue;

    if (isConstantFromScalarSubquery(children[pos])) arguments[pos].column = arguments[pos].type->createColumnConstWithDefaultValue(0);
  }

  auto function_base = function->build(arguments);
  return addFunctionImpl(function_base, std::move(children), std::move(arguments), std::move(result_name), function_base->getResultType(),
                         all_const);
}

const ActionsDAG::Node &ActionsDAG::addFunction(const FunctionNode &function, NodeRawConstPtrs children, std::string result_name) {
  auto [arguments, all_const] = getFunctionArguments(children);

  return addFunctionImpl(function.getFunction(), std::move(children), std::move(arguments), std::move(result_name),
                         function.getResultType(), all_const);
}

const ActionsDAG::Node &ActionsDAG::addFunction(const FunctionBasePtr &function_base, NodeRawConstPtrs children, std::string result_name) {
  auto [arguments, all_const] = getFunctionArguments(children);
...
const ActionsDAG::Node &ActionsDAG::addCast(const Node &node_to_cast, const DataTypePtr &cast_type, std::string result_name,
  ...
  auto func_base_cast = createInternalCast(ColumnWithTypeAndName{node_to_cast.result_type, node_to_cast.result_name}, cast_type,
                                           CastType::nonAccurate, {}, context);

  return addFunction(func_base_cast, std::move(children), result_name);
}

const ActionsDAG::Node &ActionsDAG::addFunctionImpl(const FunctionBasePtr &function_base, NodeRawConstPtrs children,
                                                    ColumnsWithTypeAndName arguments, std::string result_name, DataTypePtr result_type,
                                                    bool all_const) {
  size_t num_arguments = children.size();
  ...
  if (node.function_base->isSuitableForConstantFolding()) {
    ...
    if (all_const) {
      ...
      if (column->getDataType() != node.result_type->getColumnType())
        throw Exception(ErrorCodes::LOGICAL_ERROR, "Unexpected return type from {}. Expected {}. Got {}", node.function->getName(),
                        node.result_type->getColumnType(), column->getDataType());
    } else {
    ...
    if (column && isColumnConst(*column)) {
      ...
      if (column->empty()) column = column->cloneResized(1);
...
const ActionsDAG::Node &ActionsDAG::findInOutputs(const std::string &name) const {
  if (const auto *node = tryFindInOutputs(name)) return *node;
...
const ActionsDAG::Node *ActionsDAG::tryFindInOutputs(const std::string &name) const {
  for (const auto &node : outputs)
    if (node->result_name == name) return node;
...
ActionsDAG::NodeRawConstPtrs ActionsDAG::findInOutputs(const Names &names) const {
  ...
  for (const auto *node : outputs) names_map[node->result_name] = node;
...
ActionsDAG::SplitPossibleOutputNamesResult ActionsDAG::splitPossibleOutputNames(NameMultiSet possible_output_names) const {
  ...
  result.not_output_names.assign(std::make_move_iterator(possible_output_names.begin()),
                                 std::make_move_iterator(possible_output_names.end()));
  return result;
}
...
void ActionsDAG::addOrReplaceInOutputs(const Node &node) {
  ...
  if (!replaced) outputs.push_back(&node);
}

ActionsDAG::NodeRawConstPtrs ActionsDAG::getNodesPointers() const {
  NodeRawConstPtrs result;
  result.reserve(nodes.size());
  for (const auto &node : nodes) result.push_back(&node);
  return result;
}

NamesAndTypesList ActionsDAG::getRequiredColumns() const {
  NamesAndTypesList result;
  for (const auto &input_node : inputs) result.emplace_back(input_node->result_name, input_node->result_type);
...
Names ActionsDAG::getRequiredColumnsNames() const {
  ...
  for (const auto &input_node : inputs) result.emplace_back(input_node->result_name);
...
ColumnsWithTypeAndName ActionsDAG::getResultColumns() const {
  ...
  for (const auto &node : outputs) result.emplace_back(node->column, node->result_type, node->result_name);
...
NamesAndTypesList ActionsDAG::getNamesAndTypesList() const {
  NamesAndTypesList result;
  for (const auto &node : outputs) result.emplace_back(node->result_name, node->result_type);
...
Names ActionsDAG::getNames() const {
  ...
  for (const auto &node : outputs) names.emplace_back(node->result_name);
...
bool ActionsDAG::removeUnusedActions(const NameSet &required_names, bool allow_remove_inputs, bool allow_constant_folding) {
  ...
  if (removeUnusedActions(allow_remove_inputs, allow_constant_folding)) return true;

  if (required_nodes.size() != outputs.size()) return true;

  for (auto old_it = required_nodes.begin(), new_it = outputs.begin(); new_it != outputs.end(); ++old_it, ++new_it)
    if (*old_it != *new_it) return true;
...
bool ActionsDAG::removeUnusedActions(bool allow_remove_inputs, bool allow_constant_folding) {
  ...
  if (!allow_remove_inputs) {
    for (const auto *input : inputs) used_inputs.insert(input);
  }
...
bool ActionsDAG::removeUnusedActions(const std::unordered_set<const Node *> &used_inputs, bool allow_constant_folding) {
  ...
  for (auto &node : nodes) {
    ...
    if (node.type == ActionType::ARRAY_JOIN) roots.push_back(&node);

    if (node.type == ActionType::INPUT && used_inputs.contains(&node)) roots.push_back(&node);
  }
  ...
  for (auto stage : {VisitStage::NonDeterministic, VisitStage::Required}) {
    ...
    for (const auto *root : roots) {
      ...
      while (!stack.empty()) {
        ...
        if (stack.top().node != node) continue;

        stack.pop();

        if (stage == VisitStage::Required) continue;
...
void ActionsDAG::removeAliasesForFilter(const std::string &filter_name) {
  ...
  while (!stack.empty()) {
    ...
    for (auto &child : node->children) {
      while (child->type == ActionType::ALIAS) child = child->children.front();
...
ActionsDAG ActionsDAG::cloneSubDAG(const NodeRawConstPtrs &outputs, NodeMapping &copy_map, bool remove_aliases) {
  ...
  for (const auto *output : outputs) {
    if (copy_map.contains(output)) continue;

    stack.push(Frame{output});
    while (!stack.empty()) {
      auto &frame = stack.top();
      const auto &children = frame.node->children;
      while (frame.next_child < children.size() && copy_map.contains(children[frame.next_child])) ++frame.next_child;
      ...
      if (frame.node->type == ActionType::INPUT) actions.inputs.push_back(copy_node);
  ...
  for (auto &node : actions.nodes)
    for (auto &child : node.children) child = copy_map[child];

  for (const auto *output : outputs) actions.outputs.push_back(copy_map[output]);

  return actions;
}

static ColumnWithTypeAndName executeActionForPartialResult(const ActionsDAG::Node *node, ColumnsWithTypeAndName arguments,
                                                           size_t input_rows_count, bool only_constant_arguments) {
  ...
  switch (node->type) {
    case ActionsDAG::ActionType::FUNCTION: {
      try {
        ...
        for (const auto &arg : arguments) {
          if (!arguments_description.empty()) arguments_description += ", ";
    ...
    case ActionsDAG::ActionType::ARRAY_JOIN: {
      ...
      if (input_rows_count > 0) break;

      key.column = key.column->convertToFullColumnIfConst();

      const auto *array = getArrayJoinColumnRawPtr(key.column);
      if (!array) throw Exception(ErrorCodes::TYPE_MISMATCH, "ARRAY JOIN of not array nor map: {}", node->result_name);
    ...
    case ActionsDAG::ActionType::COLUMN: {
      ...
      if (input_rows_count != column->size()) column = column->cloneResized(input_rows_count);
...
Block ActionsDAG::updateHeader(const Block &header) const {
...

  {
    ...
    for (size_t pos = inputs.size(); pos != 0; pos--) input_positions[inputs[pos - 1]->result_name].emplace_back(pos - 1);
  ...
  try {
    ...
    if (e.code() == ErrorCodes::NOT_FOUND_COLUMN_IN_BLOCK) e.addMessage("in block {}", header.dumpStructure());
  ...
  for (auto &col : result_columns) res.insert(std::move(col));

  res.reserve(header.columns() - pos_to_remove.size());
  for (size_t i = 0; i < header.columns(); i++) {
    if (!pos_to_remove.contains(i)) res.insert(header.data[i]);
  }

  return res;
}

ColumnsWithTypeAndName ActionsDAG::evaluatePartialResult(IntermediateExecutionResult &node_to_column, const NodeRawConstPtrs &outputs,
                                                         size_t input_rows_count, PartialEvaluationParameters params) {
      ...
      if (!node_to_column.contains(output_node)) {
        ...
        while (!stack.empty()) {
          ...
          if (node->type == ActionsDAG::ActionType::PLACEHOLDER) {
            ...
            node_to_column[node] =
                ColumnWithTypeAndName(node->column->cloneResized(input_rows_count), node->result_type, node->result_name);
            continue;
          }
          ...
          for (size_t i = 0; i < arguments.size(); ++i) {
            ...
            if (!arguments[i].column) has_all_arguments = false;
          ...
          if (node->type == ActionsDAG::ActionType::INPUT && params.throw_on_error)
            throw Exception(ErrorCodes::NOT_FOUND_COLUMN_IN_BLOCK, "Not found column {}", node->result_name);
...
NameSet ActionsDAG::foldActionsByProjection(const NameSet &required_columns, const Block &projection_block_for_keys,
                                            const String &predicate_column_name, bool add_missing_keys) {
  ...
  for (const auto &input_node : inputs) next_required_columns.insert(input_node->result_name);

  return next_required_columns;
}

ActionsDAG ActionsDAG::foldActionsByProjection(const std::unordered_map<const Node *, const Node *> &new_inputs,
  ...
  for (const auto *output : required_outputs) {
    if (mapping.contains(output)) continue;
    ...
    while (!stack.empty()) {
        ...
        if (it != new_inputs.end()) {
          ...
          if (!node) {
            ...
            if (!mapped_input) {
              ...
              if (should_rename) mapped_input = &dag.addAlias(*mapped_input, new_input->result_name);
            }
      ...
      while (frame.next_child < children.size() && !mapping.emplace(children[frame.next_child], nullptr).second) ++frame.next_child;
      ...
      for (auto &child : node.children) child = mapping[child];
  ...
  for (const auto *output : required_outputs) {
    ...
    if (output->result_name != mapped_output->result_name) mapped_output = &dag.addAlias(*mapped_output, output->result_name);
...
void ActionsDAG::addAliases(const NamesWithAliases &aliases) {
  ...
  for (size_t i = 0; i < output_nodes_size; ++i) names_map[outputs[i]->result_name] = i;
...
void ActionsDAG::project(const NamesWithAliases &projection) {
  std::unordered_map<std::string_view, const Node *> names_map;
  for (const auto *output_node : outputs) names_map.emplace(output_node->result_name, output_node);
...
void ActionsDAG::appendInputsForUnusedColumns(const Block &sample_block) {
  ...
  for (size_t pos = 0; pos < num_columns; ++pos) names_map[sample_block.getByPosition(pos).name].push_back(pos);

  for (const auto *input : inputs) {
    auto &positions = names_map[input->result_name];
    if (positions.empty())
      throw Exception(ErrorCodes::NOT_FOUND_COLUMN_IN_BLOCK, "Not found column {} in block {}", input->result_name,
...
bool ActionsDAG::tryRestoreColumn(const std::string &column_name) {
  for (const auto *output_node : outputs)
    if (output_node->result_name == column_name) return true;
...
bool ActionsDAG::removeUnusedResult(const std::string &column_name) {
  ...
  const Node *col;
  {
    ...
    for (; it != outputs.end(); ++it)
      if ((*it)->result_name == column_name) break;

    if (it == outputs.end()) throw Exception(ErrorCodes::LOGICAL_ERROR, "Not found result {} in ActionsDAG\n{}", column_name, dumpDAG());
...
void ActionsDAG::removeFromOutputs(const std::string &node_name) {
  auto it = std::find_if(outputs.begin(), outputs.end(), [&node_name](const Node *node) { return node->result_name == node_name; });
...
ActionsDAG ActionsDAG::clone(std::unordered_map<const Node *, const Node *> &old_to_new_nodes) const {
  ...
  for (auto &node : actions.nodes)
    for (auto &child : node.children) child = old_to_new_nodes[child];

  for (const auto &output_node : outputs) actions.outputs.push_back(old_to_new_nodes[output_node]);

  for (const auto &input_node : inputs) actions.inputs.push_back(old_to_new_nodes[input_node]);
...
std::string ActionsDAG::dumpDAG() const {
  ...
  for (const auto &node : nodes) {
    ...
    if (node.function_base) out << " [" << node.function_base->getName() << "]";

    if (node.is_function_compiled) out << " [compiled]";

    out << "\n";
  }

  out << "Output nodes:";
  for (const auto *node : outputs) out << ' ' << map[node];
  out << '\n';
...
bool ActionsDAG::hasCorrelatedColumns() const noexcept {
  for (const auto &node : nodes)
    if (node.type == ActionType::PLACEHOLDER) return true;
...
bool ActionsDAG::hasArrayJoin() const noexcept {
  for (const auto &node : nodes)
    if (node.type == ActionType::ARRAY_JOIN) return true;
...
bool ActionsDAG::hasStatefulFunctions() const {
  for (const auto &node : nodes)
    if (node.type == ActionType::FUNCTION && node.function_base->isStateful()) return true;
...
bool ActionsDAG::trivial() const noexcept {
  for (const auto &node : nodes)
    if (node.type == ActionType::FUNCTION || node.type == ActionType::ARRAY_JOIN) return false;
...
void ActionsDAG::assertDeterministic() const {
  for (const auto &node : nodes)
    if (!node.isDeterministic())
      throw Exception(ErrorCodes::BAD_ARGUMENTS, "Expression must be deterministic but it contains non-deterministic part `{}`",
                      node.result_name);
}

bool ActionsDAG::hasNonDeterministic() const {
  for (const auto &node : nodes)
    if (!node.isDeterministic()) return true;
  return false;
}
...
void ActionsDAG::addMaterializingOutputActions(bool materialize_sparse) {
  for (auto &output_node : outputs) output_node = &materializeNode(*output_node, materialize_sparse);
}
...
void ActionsDAG::removeTrivialWrappers() {
  ...
  for (auto &node : nodes)
    for (auto &child : node.children)
      while (is_trivial_wrapper(child)) child = child->children[0];

  for (auto *&output : outputs)
    while (is_trivial_wrapper(output)) output = output->children[0];

  removeUnusedActions();
}

ActionsDAG ActionsDAG::makeConvertingActions(const ColumnsWithTypeAndName &source, const ColumnsWithTypeAndName &result,
                                             MatchColumnsMode mode, ContextPtr context, bool ignore_constant_values, bool add_cast_columns,
                                             NameToNameMap *new_names, NameSet *columns_contain_compiled_function) {
  ...
  if (mode == MatchColumnsMode::Position && num_input_columns != num_result_columns)
    throw Exception(ErrorCodes::NUMBER_OF_COLUMNS_DOESNT_MATCH, "Number of columns doesn't match (source: {} and result: {})",
                    num_input_columns, num_result_columns);
  ...
  if (mode == MatchColumnsMode::Name) {
    ...
    for (size_t pos = 0; pos < input_nodes_size; ++pos) inputs[actions_dag.inputs[pos]->result_name].push_back(pos);
  }
  ...
  for (size_t result_col_num = 0; result_col_num < num_result_columns; ++result_col_num) {
      ...
      case MatchColumnsMode::Name: {
        ...
        if (input.empty()) {
          ...
          if (ignore_constant_values && res_const)
            ...
            throw Exception(ErrorCodes::THERE_IS_NO_COLUMN, "Cannot find column `{}` in source stream, there are only columns: [{}]",
    ...
    if (const auto *res_const = typeid_cast<const ColumnConst *>(res_elem.column.get())) {
      if (const auto *src_const = typeid_cast<const ColumnConst *>(dst_node->column.get())) {
        ...
        /// e.g. expression "and(equals(materialize(null::Nullable(UInt64)), null::Nullable(UInt64)), equals(null::Nullable(UInt64),
        /// null::Nullable(UInt64)))" compiled expression is "and(equals(input: Nullable(UInt64), null), null). Partial evaluation of the
        /// compiled expression isn't able to infer that the result column is constant. It causes inconsistency between pipeline
        /// header(source column is not constant) and output header of ExpressionStep(destination column is constant). So we need to convert
        /// non-constant column to constant column under this condition.
        dst_node = &actions_dag.addColumn(res_elem);
      } else
        throw Exception(ErrorCodes::ILLEGAL_COLUMN,
    ...
    if (!res_elem.type->equals(*dst_node->result_type)) {
      ...
      auto func_base_cast =
          createInternalCast(std::move(left_column), res_elem.type, CastType::nonAccurate, std::move(diagnostic), context);
    ...
    if (dst_node->result_name != res_elem.name) {
      if (add_cast_columns) {
        ...
        if (new_names) new_names->emplace(res_elem.name, dst_node->result_name);
...
void ActionsDAG::mergeInplace(ActionsDAG &&second, std::unordered_map<const Node *, const Node *> &inputs_map,
                              bool remove_dangling_inputs) {
  auto &first = *this;
  ...
  /// Update inputs list.
  {
    ...
    for (const auto &output_node : first.outputs) first_result[output_node->result_name].push_back(output_node);
  ...
  for (auto &node : second.nodes) {
    for (auto &child : node.children) {
      if (child->type == ActionType::INPUT) {
        ...
        if (it != inputs_map.end()) child = it->second;
      }
    }
  }
  ...
  for (auto &output_node : second.outputs) {
    if (output_node->type == ActionType::INPUT) {
      ...
      if (it != inputs_map.end()) output_node = it->second;
    }
  }
...
void ActionsDAG::mergeNodes(ActionsDAG &&second, NodeRawConstPtrs *out_outputs) {
  ...
  for (auto &node : nodes) node_name_to_node.emplace(node.result_name, &node);
  ...
  for (auto &node : second.nodes) const_node_to_node.emplace(&node, &node);

  std::vector<Frame> nodes_to_process;
  nodes_to_process.reserve(second.getOutputs().size());
  for (auto &node : second.getOutputs()) nodes_to_process.push_back({const_node_to_node.at(node), false /*visited_children*/});
  ...
  while (!nodes_to_process.empty()) {
    ...
    if (!node_to_process.visited_children) {
      ...
      for (auto &child : node->children) nodes_to_process.push_back({const_node_to_node.at(child), false /*visited_children*/});

      /// If node has children process them first
      if (!node->children.empty()) continue;
    }

    for (auto &child : node->children) child = node_name_to_node.at(child->result_name);
  ...
  if (out_outputs) {
    for (auto &node : second.getOutputs()) out_outputs->push_back(node_name_to_node.at(node->result_name));
  }

  if (nodes_to_move_from_second_dag.empty()) return;
  ...
  for (auto second_node_it = second.nodes.begin(); second_node_it != second_nodes_end;) {
    ...
    if (node_to_move_it->type == ActionType::INPUT) inputs.push_back(&(*node_to_move_it));
  }
}
...
ActionsDAG::SplitResult ActionsDAG::split(std::unordered_set<const Node *> split_nodes, bool create_split_nodes_mapping,
                                          bool avoid_duplicate_inputs) const {
  ...
  if (avoid_duplicate_inputs)
    for (const auto *input : inputs) duplicate_inputs.insert(input->result_name);
  ...
  for (const auto &output_node : outputs) data[output_node].used_in_result = true;

  /// DFS. Decide if node is needed by split.
  for (const auto &node : nodes) {
    if (!split_nodes.contains(&node)) continue;

    auto &cur_data = data[&node];
    if (cur_data.needed_by_split_node) continue;
  ...
  for (const auto &node : nodes) {
    if (!data[&node].visited) stack.push({.node = &node});
      ...
      if (cur.next_child_to_visit == cur.node->children.size()) {
        ...
        if (!cur_data.needed_by_split_node) {
          ...
          for (auto &child : copy.children) {
            ...
            if (!child_data.to_second) {
              if (child->type == ActionType::COLUMN)  /// Just create new node for COLUMN action.
              {
                ...
                if (child->type != ActionType::INPUT) new_inputs.push_back(child);
              }
            }
          ...
          if (cur_data.used_in_result) {
            ...
            if (cur.node->type != ActionType::INPUT) new_inputs.push_back(cur.node);
          }
        }
      }
    }
  ...
  for (const auto *output_node : outputs) second_outputs.push_back(data[output_node].to_second);
  ...
  for (const auto *input_node : inputs) {
    ...
    if (cur.to_first) {
      ...
      if (cur.to_second) first_outputs.push_back(cur.to_first);
    }
  }
  ...
  for (const auto *input_node : inputs) {
    const auto &cur = data[input_node];
    if (cur.to_second) second_inputs.push_back(cur.to_second);
  }
  ...
  if (create_split_nodes_mapping) {
    for (const auto *node : split_nodes) split_nodes_mapping[node] = data[node].to_first;
  }
  ...
  for (const auto &node : nodes) {
    ...
    while (!stack.empty()) {
      ...
      if (cur.next_child_to_visit == cur.node->children.size()) {
        ...
        if (cur.node->type == ActionType::INPUT && array_joined_columns_set.contains(cur.node->result_name)) depend_on_array_join = true;

        for (const auto *child : cur.node->children) {
          if (!split_nodes.contains(child)) depend_on_array_join = true;
        }

        if (!depend_on_array_join) split_nodes.insert(cur.node);
...
ActionsDAG::SplitResult ActionsDAG::splitActionsBySortingDescription(const NameSet &sort_columns) const {
  ...
  for (const auto &sort_column : sort_columns)
    if (const auto *node = tryFindInOutputs(sort_column)) {
      ...
      throw Exception(ErrorCodes::LOGICAL_ERROR, "Sorting column {} wasn't found in the ActionsDAG's outputs. DAG:\n{}", sort_column,
                      dumpDAG());
...
bool ActionsDAG::isFilterAlwaysFalseForDefaultValueInputs(const std::string &filter_name, const Block &input_stream_header) const {
  ...
  if (!filter_node)
    throw Exception(ErrorCodes::LOGICAL_ERROR, "Outputs for ActionsDAG does not contain filter column name {}. DAG:\n{}", filter_name,
                    dumpDAG());

  std::unordered_map<std::string, ColumnWithTypeAndName> input_node_name_to_default_input_column;

  for (const auto *input : inputs) {
    if (!input_stream_header.has(input->result_name)) continue;
  ...
  if (!filter_with_default_value_inputs) return false;
  ...
  if (!constant_type->canBeUsedInBooleanContext()) return false;
...
ConjunctionNodes getConjunctionNodes(ActionsDAG::Node *predicate, std::unordered_set<const ActionsDAG::Node *> allowed_nodes,
  ...
  while (!stack.empty()) {
    ...
    while (cur.next_child_to_visit < cur.node->children.size()) {
      ...
      if (allowed_nodes.contains(child)) ++cur.num_allowed_children;
      ++cur.next_child_to_visit;
    }

    if (cur.next_child_to_visit == cur.node->children.size()) {
      if (cur.num_allowed_children == cur.node->children.size()) {
        bool is_deprecated_function = !allow_non_deterministic_functions && cur.node->type == ActionsDAG::ActionType::FUNCTION &&
      ...
      if (predicates.contains(cur.node)) {
        if (allowed_nodes.contains(cur.node)) {
          if (allowed.insert(cur.node).second) conjunction.allowed.push_back(cur.node);

        } else {
          if (rejected.insert(cur.node).second) conjunction.rejected.push_back(cur.node);
        }
      }
...
std::optional<ActionsDAG::ActionsForFilterPushDown> ActionsDAG::createActionsForConjunction(NodeRawConstPtrs conjunction,
                                                                                            const ColumnsWithTypeAndName &all_inputs) {
  if (conjunction.empty()) return {};
  ...
  for (const auto *predicate : conjunction) {
    if (nodes_mapping.contains(predicate)) continue;
    ...
    while (!stack.empty()) {
      ...
      if (cur.next_child_to_visit == cur.node->children.size()) {
        ...
        for (auto &child : node.children) child = nodes_mapping[child];

        if (node.type == ActionType::INPUT) required_inputs[node.result_name].push_back(&node);
  ...
  if (conjunction.size() > 1) {
    ...
    for (const auto *predicate : conjunction) args.emplace_back(nodes_mapping[predicate]);
  ...
  if (has_input_name_collision) {
    for (size_t idx = 0;; ++idx) {
      ...
      if (required_inputs.contains(rename)) continue;
  ...
  if (remove_filter) actions.outputs.insert(actions.outputs.begin(), result_predicate);
...
std::optional<ActionsDAG::ActionsForFilterPushDown> ActionsDAG::splitActionsForFilterPushDown(const std::string &filter_name,
                                                                                              bool removes_filter,
  ...
  if (!predicate)
    throw Exception(ErrorCodes::LOGICAL_ERROR, "Output nodes for ActionsDAG do not contain filter column name {}. DAG:\n{}", filter_name,
                    dumpDAG());

  /// If condition is constant let's do nothing.
  /// It means there is nothing to push down or optimization was already applied.
  if (predicate->type == ActionType::COLUMN) return {};
  ...
  /// Get input nodes from available_inputs names.
  {
    ...
    for (const auto &input_node : inputs) inputs_map[input_node->result_name].emplace_back(input_node);

    for (const auto &name : available_inputs) {
      auto &inputs_list = inputs_map[name];
      if (inputs_list.empty()) continue;
  ...
  if (conjunction.allowed.empty()) return {};
...
ActionsDAG::ActionsForJOINFilterPushDown ActionsDAG::splitActionsForJOINFilterPushDown(
    const std::string &filter_name, bool removes_filter, const Names &left_stream_available_columns_to_push_down,
  ...
  std::unordered_set<const Node *> left_stream_allowed_conjunctions_set(left_stream_allowed_conjunctions.begin(),
                                                                        left_stream_allowed_conjunctions.end());
  std::unordered_set<const Node *> right_stream_allowed_conjunctions_set(right_stream_allowed_conjunctions.begin(),
                                                                         right_stream_allowed_conjunctions.end());
  ...
  rejected_conjunctions_set.insert(right_stream_push_down_conjunctions.rejected.begin(),
                                   right_stream_push_down_conjunctions.rejected.end());
  rejected_conjunctions_set.insert(both_streams_push_down_conjunctions.rejected.begin(),
                                   both_streams_push_down_conjunctions.rejected.end());
  ...
  auto left_stream_filter_to_push_down =
      createActionsForConjunction(left_stream_allowed_conjunctions, left_stream_header.getColumnsWithTypeAndName());
  auto right_stream_filter_to_push_down =
      createActionsForConjunction(right_stream_allowed_conjunctions, right_stream_header.getColumnsWithTypeAndName());

  auto replace_equivalent_columns_in_filter = [](const ActionsDAG &filter, size_t filter_pos, const Block &stream_header,
    ...
    for (const auto &input : updated_filter.getInputs()) updated_filter_inputs[input->result_name].push_back(input);

    for (const auto &input : filter.getInputs()) {
      if (updated_filter_inputs.contains(input->result_name)) continue;
    ...
    for (const auto &input_column : stream_header.getColumnsWithTypeAndName()) {
      ...
      if (input != updated_filter.getOutputs()[0]) updated_filter.outputs.push_back(input);
    }
  ...
  if (left_stream_filter_to_push_down)
    left_stream_filter_to_push_down->dag =
        replace_equivalent_columns_in_filter(left_stream_filter_to_push_down->dag, left_stream_filter_to_push_down->filter_pos,
                                             left_stream_header, equivalent_right_stream_column_to_left_stream_column);

  if (right_stream_filter_to_push_down)
    right_stream_filter_to_push_down->dag =
        replace_equivalent_columns_in_filter(right_stream_filter_to_push_down->dag, right_stream_filter_to_push_down->filter_pos,
                                             right_stream_header, equivalent_left_stream_column_to_right_stream_column);
  ...
  ActionsDAG::ActionsForJOINFilterPushDown result{.left_stream_filter_to_push_down = std::move(left_stream_filter_to_push_down_dag),
  ...
  if (!result.left_stream_filter_to_push_down && !result.right_stream_filter_to_push_down) return result;
    ...
    if (removes_filter) {
      ...
      if (predicate->type != ActionType::INPUT)
        *predicate = std::move(node);
        ...
        for (auto &output_node : outputs)
          if (output_node == predicate) output_node = new_predicate;
      }
    ...
    if (new_children.size() == 1) {
      ...
      if (!removes_filter) {
        ...
        if (isFloat(removeLowCardinalityAndNullable(child->result_type))) {
          ...
          if (isNullableOrLowCardinalityNullable(child->result_type)) cast_type = std::make_shared<DataTypeNullable>(std::move(cast_type));
          child = &addCast(*child, cast_type, {}, nullptr);
        }

        if (!child->result_type->equals(*predicate->result_type)) child = &addCast(*child, predicate->result_type, {}, nullptr);
      }
...
static bool isColumnSortingPreserved(const ActionsDAG::Node *start_node, const String &sorted_column) {
  ...
  while (node) {
    ...
    if (node->children.empty()) break;  /// column not found
  ...
  while (node) {
    if (node->type == ActionsDAG::ActionType::FUNCTION) {
      ...
      if (func) {
        if (!func->hasInformationAboutMonotonicity()) return false;
        ...
        if (!monotonicity.is_always_monotonic) return false;
      }
    }
...
bool ActionsDAG::isSortingPreserved(const Block &input_header, const SortDescription &sort_description,
                                    const String &ignore_output_column) const {
  if (sort_description.empty()) return true;

  if (hasArrayJoin()) return false;
  ...
  for (const auto &desc : sort_description) {
    ...
    for (const auto *output_node : outputs) {
      if (output_node->result_name == ignore_output_column) continue;
...
std::optional<ActionsDAG> ActionsDAG::buildFilterActionsDAG(
    const NodeRawConstPtrs &filter_nodes, const std::unordered_map<std::string, ColumnWithTypeAndName> &node_name_to_input_node_column,
    bool single_output_condition_node) {
  if (filter_nodes.empty()) return {};
  ...
  for (const auto &node : filter_nodes) nodes_to_process.push_back({node, false /*visited_children*/});
  ...
  while (!nodes_to_process.empty()) {
    ...
    if (input_node_it != node_name_to_input_node_column.end()) {
      ...
      if (!result_input) result_input = &result_dag.addInput(input_node_it->second);
    ...
    if (!node_to_process.visited_children) {
      ...
      for (const auto &child : node->children) nodes_to_process.push_back({child, false /*visited_children*/});
    ...
    switch (node_type) {
      case ActionsDAG::ActionType::INPUT: {
        ...
        if (!result_input) result_input = &result_dag.addInput({node->column, node->result_type, node->result_name});
      ...
      case ActionsDAG::ActionType::FUNCTION: {
          ...
          if (const auto *adaptor = typeid_cast<const FunctionToFunctionBaseAdaptor *>(node->function_base.get())) {
            if (const auto *index_hint = typeid_cast<const FunctionIndexHint *>(adaptor->getFunction().get())) {
              ...
              if (!index_hint_args.empty())
                index_hint_filter_dag =
                    *buildFilterActionsDAG(index_hint_args, node_name_to_input_node_column, false /*single_output_condition_node*/);
        ...
        for (const auto &child : node->children) function_children.push_back(node_to_result_node.find(child)->second);
        ...
        result_node = &result_dag.addFunctionImpl(function_base, std::move(function_children), std::move(arguments), result_name,
                                                  node->result_type, all_const);
        break;
      }
      case ActionsDAG::ActionType::PLACEHOLDER: {
  ...
  for (const auto &node : filter_nodes) result_dag_outputs.push_back(node_to_result_node.find(node)->second);
...
ActionsDAG::NodeRawConstPtrs ActionsDAG::extractConjunctionAtoms(const Node *predicate) {
  ...
  while (!stack.empty()) {
    ...
    if (node->type == ActionsDAG::ActionType::FUNCTION) {
      ...
      if (name == "and") {
        for (const auto *arg : node->children) stack.push(arg);
...
ActionsDAG::NodeRawConstPtrs ActionsDAG::filterNodesByAllowedInputs(NodeRawConstPtrs nodes,
  ...
  for (const auto *node : nodes) {
    if (!can_compute.contains(node)) stack.push({node});
...
FindOriginalNodeForOutputName::FindOriginalNodeForOutputName(const ActionsDAG &actions_) {
  ...
  for (const auto *output_node : actions_outputs) {
    ...
    if (node && node->type == ActionsDAG::ActionType::INPUT) index.emplace(output_node->result_name, node);
  }
}
...
static void serializeCapture(const LambdaCapture &capture, WriteBuffer &out) {
  ...
  for (const auto &name : capture.captured_names) writeStringBinary(name, out);

  writeVarUInt(capture.captured_types.size(), out);
  for (const auto &type : capture.captured_types) encodeDataType(type, out);
...
static void deserializeCapture(LambdaCapture &capture, ReadBuffer &in) {
  ...
  for (auto &name : capture.captured_names) readStringBinary(name, in);

  UInt64 num_types;
  readVarUInt(num_types, in);
  capture.captured_types.resize(num_types);
  for (auto &type : capture.captured_types) type = decodeDataType(in);
...
static void serializeConstant(const IDataType &type, const IColumn &value, WriteBuffer &out, SerializedSetsRegistry &registry) {
  if (WhichDataType(type).isSet()) {
    ...
    if (!column_set) throw Exception(ErrorCodes::LOGICAL_ERROR, "ColumnSet is expected for DataTypeSet. Got {}", value.getName());

    UInt8 flags = 0;
    if (is_constant) flags |= 1;
  ...
  if (WhichDataType(type).isFunction()) {
    ...
    if (const auto *column_const = typeid_cast<const ColumnConst *>(maybe_function)) maybe_function = &column_const->getDataColumn();

    const auto *column_function = typeid_cast<const ColumnFunction *>(maybe_function);
    if (!column_function) throw Exception(ErrorCodes::LOGICAL_ERROR, "ColumnSet is expected for DataTypeSet. Got {}", value.getName());
    ...
    if (!function_expression)
      throw Exception(ErrorCodes::LOGICAL_ERROR, "Expected FunctionExpression for ColumnFunction. Got {}", function->getName());
  ...
  if (!const_column) throw Exception(ErrorCodes::LOGICAL_ERROR, "Cannot serialize non-constant column {} as constant", value.getName());

  const auto &data = const_column->getDataColumn();
  type.getDefaultSerialization()->serializeBinary(data, 0, out, FormatSettings{});
}

static MutableColumnPtr deserializeConstant(const IDataType &type, ReadBuffer &in, DeserializedSetsRegistry &registry,
  ...
  if (WhichDataType(type).isFunction()) {
    ...
    auto function_expression = std::make_shared<FunctionExpression>(
        ...
        std::make_shared<ExpressionActions>(std::move(capture_dag), ExpressionActionsSettings(context, CompileExpressions::yes)));
...
std::unordered_map<const ActionsDAG::Node *, size_t> ActionsDAG::getNodeToIdMap() const {
  ...
  for (const auto &node : nodes) node_to_id.emplace(&node, node_to_id.size());

  if (nodes.size() != node_to_id.size()) throw Exception(ErrorCodes::LOGICAL_ERROR, "Duplicate nodes in ActionsDAG");
...
static void addChildrenBeforeNode(std::vector<const ActionsDAG::Node *> &reordered_nodes,
                                  std::unordered_set<const ActionsDAG::Node *> &already_added_nodes, const ActionsDAG::Node *node) {
  if (already_added_nodes.contains(node)) return;

  for (const auto *child : node->children) addChildrenBeforeNode(reordered_nodes, already_added_nodes, child);
...
void ActionsDAG::serialize(WriteBuffer &out, SerializedSetsRegistry &registry) const {
  ...
  std::vector<const Node *> reordered_nodes;
  {
    ...
    for (const auto &node : nodes) addChildrenBeforeNode(reordered_nodes, already_added_nodes, &node);
  }

  std::unordered_map<const Node *, size_t> node_to_id;
  for (const auto *node : reordered_nodes) node_to_id.emplace(node, node_to_id.size());
  ...
  for (size_t node_id = 0; node_id < reordered_nodes.size(); ++node_id) {
    ...
    if (has_column) {
      column_flags |= 1;
      if (node.is_deterministic_constant) column_flags |= 2;
    }

    const auto *function_capture = typeid_cast<const FunctionCapture *>(node.function_base.get());
    if (function_capture) column_flags |= 4;

    writeIntBinary(column_flags, out);

    if (has_column) serializeConstant(*node.result_type, *node.column, out, registry);
  ...
  for (const auto *input : inputs) writeVarUInt(node_to_id.at(input), out);

  writeVarUInt(outputs.size(), out);
  for (const auto *output : outputs) writeVarUInt(node_to_id.at(output), out);
}
...
ActionsDAG ActionsDAG::deserialize(ReadBuffer &in, DeserializedSetsRegistry &registry, const ContextPtr &context) {
  ...
  for (size_t i = 0; i < nodes_size; ++i) id_to_node.emplace(i, &nodes.emplace_back(Node{}));
  ...
  for (size_t i = 0; i < nodes_size; ++i) {
    ...
    if (column_flags & 1) {
      if ((column_flags & 2) == 0) node.is_deterministic_constant = false;
    ...
    if (node.type == ActionType::INPUT) {
      ...
      if (!node.children.empty()) throw Exception(ErrorCodes::INCORRECT_DATA, "Deserialized input can't have children");
    } else if (node.type == ActionType::COLUMN) {
      /// Column is already deserialized
      if (!node.children.empty()) throw Exception(ErrorCodes::INCORRECT_DATA, "Deserialized column can't have children");
    } else if (node.type == ActionType::ALIAS) {
      /// nothing to deserialize
      if (node.children.size() != 1) throw Exception(ErrorCodes::INCORRECT_DATA, "Deserialized alias must have one children");
    } else if (node.type == ActionType::FUNCTION) {
      ...
      if (column_flags & 4) {
        ...
        node.function_base = std::make_shared<FunctionCapture>(
            std::make_shared<ExpressionActions>(std::move(capture_dag), ExpressionActionsSettings(context, CompileExpressions::yes)),
            std::make_shared<LambdaCapture>(std::move(capture)), node.result_type, function_name);
        ...
        if (!lhs_type->equals(*rhs_type))
          throw Exception(ErrorCodes::INCORRECT_DATA, "Deserialized function {} has invalid type. Expected {}, deserialized {}.",
                          function_name, rhs_type->getName(), lhs_type->getName());
      }
    } else if (node.type == ActionType::ARRAY_JOIN) {
      /// nothing to deserialize
      if (node.children.size() != 1) throw Exception(ErrorCodes::INCORRECT_DATA, "Deserialized array join must have one children");
    } else {
      throw Exception(ErrorCodes::LOGICAL_ERROR, "Unknown node type {}", static_cast<size_t>(node.type));
    }
  }
  ...
  for (size_t i = 0; i < inputs_size; ++i) {
    ...
    if (input->type != ActionType::INPUT)
      throw Exception(ErrorCodes::INCORRECT_DATA, "Deserialized input {} has type {}", input->result_name,
                      magic_enum::enum_name(input->type));

    if (!inputs_set.emplace(input).second) throw Exception(ErrorCodes::INCORRECT_DATA, "Duplicate input {}", input->result_name);
  ...
  for (const auto &node : nodes)
    if (node.type == ActionType::INPUT && !inputs_set.contains(&node))
      throw Exception(ErrorCodes::INCORRECT_DATA, "Deserialized input {} is not in the inputs list", node.result_name);
