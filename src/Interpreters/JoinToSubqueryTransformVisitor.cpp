...
ASTPtr makeSubqueryTemplate(const String &table_alias) {
  ...
  if (!table_alias.empty()) query_template += " as " + table_alias;
  ...
  if (!subquery_template) throw Exception(ErrorCodes::LOGICAL_ERROR, "Cannot parse subquery template");
 ...
 public:
  struct Data {
    ...
    void addTableColumns(
        const String &table_name, ASTs &columns,
      ...
      if (it == table_columns.end()) {
        ...
        if (table_name_it != table_name_alias.end()) {
          ...
          if (it == table_columns.end()) throw Exception(ErrorCodes::UNKNOWN_IDENTIFIER, "Unknown qualified identifier: {}", table_name);
        } else
  ...
  static void visit(const ASTPtr &ast, Data &data) {
    if (auto *t = ast->as<ASTExpressionList>()) visit(*t, ast, data);
  }
  ...
  static void visit(const ASTExpressionList &node, const ASTPtr &, Data &data) {
    ...
    for (const auto &child : node.children) {
      ...
      if (const auto *asterisk = child->as<ASTAsterisk>()) {
        ...
        for (auto &table_name : data.tables_order) data.addTableColumns(table_name, columns);

        if (asterisk->transformers) {
          for (const auto &transformer : asterisk->transformers->children) IASTColumnsTransformer::transform(transformer, columns);
        }
      } else if (const auto *qualified_asterisk = child->as<ASTQualifiedAsterisk>()) {
        has_asterisks = true;

        if (!qualified_asterisk->qualifier) throw Exception(ErrorCodes::LOGICAL_ERROR, "Qualified asterisk must have a qualifier");
        ...
        if (qualified_asterisk->transformers) {
          for (const auto &transformer : qualified_asterisk->transformers->children) {
            if (transformer->as<ASTColumnsApplyTransformer>() || transformer->as<ASTColumnsExceptTransformer>() ||
        ...
        for (const auto &ident : columns_list_matcher->column_list->children) columns.emplace_back(ident->clone());
        ...
        if (!regexp.ok())
          throw Exception(ErrorCodes::CANNOT_COMPILE_REGEXP, "COLUMNS pattern {} cannot be compiled: {}", pattern, regexp.error());

        for (auto &table_name : data.tables_order)
          data.addTableColumns(table_name, columns, [&](const String &column_name) { return re2::RE2::PartialMatch(column_name, regexp); });
      ...
      data.new_select_expression_list->children.insert(data.new_select_expression_list->children.end(),
                                                       std::make_move_iterator(columns.begin()), std::make_move_iterator(columns.end()));
    }

    if (!has_asterisks) data.new_select_expression_list.reset();
  }
};
...
bool needRewrite(ASTSelectQuery &select, std::vector<const ASTTableExpression *> &table_expressions) {
  ...
  for (size_t i = 0; i < num_tables; ++i) {
    ...
    if (!table) throw Exception(ErrorCodes::LOGICAL_ERROR, "Table expected");

    if (table->table_expression)
      if (const auto *expression = table->table_expression->as<ASTTableExpression>()) table_expressions.push_back(expression);
    if (!i) continue;

    if (!table->table_join && !table->array_join) throw Exception(ErrorCodes::LOGICAL_ERROR, "Joined table expected");
  ...
  if (num_tables - num_array_join <= 2) return false;

  /// it's not trivial to support mix of JOIN ON & JOIN USING cause of short names
  if (num_array_join) throw Exception(ErrorCodes::NOT_IMPLEMENTED, "Multiple JOIN does not support mix with ARRAY JOINs");
  return true;
}
...
struct CollectColumnIdentifiersMatcher {
  ...
  struct Data {
    ...
    explicit Data(std::vector<ASTIdentifier *> &identifiers_) : identifiers(identifiers_) {}

    void addIdentifier(const ASTIdentifier &ident) {
      for (const auto &aliases : ignored)
        if (aliases.contains(ident.name())) return;
      identifiers.push_back(const_cast<ASTIdentifier *>(&ident));
    }

    void pushIgnored(const Names &names) { ignored.emplace_back(std::unordered_set<String>(names.begin(), names.end())); }

    void popIgnored() { ignored.pop_back(); }
  };

  static bool needChildVisit(const ASTPtr &node, const ASTPtr &) {
    /// "lambda" visit children itself.
    if (const auto *f = node->as<ASTFunction>())
      if (f->name == "lambda") return false;

    /// Do not go into subqueries. Do not collect table identifiers. Do not get identifier from 't.*'.
    return !node->as<ASTSubquery>() && !node->as<ASTTablesInSelectQuery>() && !node->as<ASTQualifiedAsterisk>();
  }
  ...
  static void visit(const ASTIdentifier &ident, const ASTPtr &, Data &data) { data.addIdentifier(ident); }
...
struct CheckAliasDependencyVisitorData {
  ...
  void visit(ASTIdentifier &ident, ASTPtr &) {
    if (!dependency && aliases.contains(ident.name())) dependency = &ident;
  }
};
...
struct RewriteWithAliasMatcher {
  ...
  static bool needChildVisit(const ASTPtr &node, const ASTPtr &) { return !node->as<ASTSubquery>(); }

  static void visit(ASTPtr &ast, Data &data) {
    String alias = ast->tryGetAlias();
    if (!alias.empty()) {
      auto it = data.find(alias);
      if (it != data.end() && it->second.get() == ast.get()) ast = make_intrusive<ASTIdentifier>(alias);
    }
  }
};
...
class SubqueryExpressionsRewriteMatcher {
 public:
  ...
  static void visit(ASTPtr &ast, Data &data) {
    if (auto *t = ast->as<ASTSelectQuery>()) visit(*t, ast, data);
  }

 private:
  static void visit(ASTSelectQuery &select, ASTPtr &, Data &data) {
    if (!data.done) {
      if (data.expression_list->children.empty()) data.expression_list->children.emplace_back(make_intrusive<ASTAsterisk>());
...
struct TableNeededColumns {
  ...
  void fillExpressionList(ASTExpressionList &expression_list) const {
    ...
    for (const auto &column : no_clashes) addShortName(column, expression_list);

    for (const auto &column : alias_clashes) addShortName(column, expression_list);

    for (const auto &[column, alias] : column_clashes) addAliasedName(table_name, column, alias, expression_list);
  }
  ...
  static void addAliasedName(const String &table, const String &column, const String &alias, ASTExpressionList &expression_list) {
    ...
    if (!table.empty()) name_parts.push_back(table);
...
class UniqueShortNames {
 public:
  ...
  String longToShort(const String &long_name) {
    ...
    if (it != long_to_short.end()) return it->second;
 ...
 private:
  ...
  static String generateUniqueName(const String &long_name) { return String(pattern) + long_name; }
};

size_t countTablesWithColumn(const std::vector<TableWithColumnNamesAndTypes> &tables, const String &short_name) {
  size_t count = 0;
  for (const auto &table : tables)
    if (table.hasColumn(short_name)) ++count;
  return count;
}
...
void restoreName(ASTIdentifier &ident, const String &original_name, NameSet &restored_names) {
  if (!ident.tryGetAlias().empty()) return;
...
std::vector<TableNeededColumns> normalizeColumnNamesExtractNeeded(const std::vector<TableWithColumnNamesAndTypes> &tables,
                                                                  const Aliases &aliases, const std::vector<ASTIdentifier *> &identifiers,
  ...
  for (const auto &table : tables) needed_columns.push_back(TableNeededColumns{table.table});
  ...
  for (ASTIdentifier *ident : identifiers) {
    ...
    if (auto table_pos = IdentifierSemantic::chooseTableColumnMatch(*ident, tables, allow_ambiguous)) {
      if (!ident->isShort()) {
        ...
        if (public_identifiers.contains(ident)) original_long_name = ident->name();
        ...
        if (count > 1 || aliases.contains(short_name)) {
          ...
          if (!table.hasColumn(short_name)) {
            throw Exception(ErrorCodes::UNKNOWN_IDENTIFIER, "There's no column '{}' in table '{}'", ident->name(),
...
boost::intrusive_ptr<ASTExpressionList> subqueryExpressionList(size_t table_pos, const std::vector<TableNeededColumns> &needed_columns,
  ...
  for (const auto &expr : alias_pushdown[table_pos]) expression_list->children.emplace_back(expr);
...
bool JoinToSubqueryTransformMatcher::needChildVisit(ASTPtr &node, const ASTPtr &) {
  if (node->as<ASTSubquery>()) return false;

  if (node->as<ASTTableExpression>()) return false;
...
void JoinToSubqueryTransformMatcher::visit(ASTSelectQuery &select, ASTPtr &ast, Data &data) {
  ...
  if (!needRewrite(select, table_expressions)) return;

  auto &src_tables = select.tables()->children;
  size_t tables_count = src_tables.size();

  if (table_expressions.size() != data.tables.size() || tables_count != data.tables.size())
  ...
  for (auto &top_level_child : select.select()->children)
    if (auto *ident = top_level_child->as<ASTIdentifier>()) {
      if (!data.try_to_keep_original_names || startsWith(ident->name(), UniqueShortNames::pattern)) public_identifiers.insert(ident);
    }
  ...
  auto last_select = last_select_template->clone();
  {
    ...
    if (!last_table_elem || !last_select_elem) throw Exception(ErrorCodes::LOGICAL_ERROR, "Two TablesInSelectQueryElements expected");

    if (!last_table_elem->table_join) throw Exception(ErrorCodes::LOGICAL_ERROR, "Table join expected");
...
ASTPtr JoinToSubqueryTransformMatcher::replaceJoin(ASTPtr ast_left, ASTPtr ast_right, ASTPtr subquery_template) {
  ...
  if (!left || !right) throw Exception(ErrorCodes::LOGICAL_ERROR, "Two TablesInSelectQueryElements expected");

  if (!right->table_join) throw Exception(ErrorCodes::LOGICAL_ERROR, "Table join expected");
