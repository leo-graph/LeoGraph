...
void RewriteSumFunctionWithSumAndCountMatcher::visit(ASTPtr& ast, const Data& data) {
  if (auto* func = ast->as<ASTFunction>()) visit(*func, ast, data);
}
...
void RewriteSumFunctionWithSumAndCountMatcher::visit(const ASTFunction& function, ASTPtr& ast, const Data& data) {
  static const std::unordered_set<String> function_supported = {"plus", "minus"};

  if (!function.arguments || Poco::toLower(function.name) != "sum" || function.arguments->children.size() != 1) return;

  const auto* func_plus_minus = function.arguments->children[0]->as<ASTFunction>();

  if (!func_plus_minus || !function_supported.contains(Poco::toLower(func_plus_minus->name)) ||
      func_plus_minus->arguments->children.size() != 2)
    return;
  ...
  if (literal_type != Field::Types::UInt64 && literal_type != Field::Types::Int64 && literal_type != Field::Types::UInt128 &&
      literal_type != Field::Types::Int128 && literal_type != Field::Types::UInt256 && literal_type != Field::Types::Int256 &&
      literal_type != Field::Types::Float64 && literal_type != Field::Types::Decimal32 && literal_type != Field::Types::Decimal64 &&
      literal_type != Field::Types::Decimal128 && literal_type != Field::Types::Decimal256)
    return;
  ...
  if (!pos) pos = IdentifierSemantic::chooseTableColumnMatch(*column, data.tables, true);
  if (!pos) return;

  if (*pos >= data.tables.size()) return;

  auto column_type_name = data.tables[*pos].columns.tryGetByName(column->shortName());
  if (!column_type_name) return;

  const auto column_type = column_type_name->type;
  if (!column_type || !isNumber(*column_type)) return;

  const String& column_name = column_type_name->name;

  if (column_id == 0) {
    const auto new_ast = makeASTOperator(func_plus_minus->name, makeASTFunction("sum", make_intrusive<ASTIdentifier>(column_name)),
                                         makeASTOperator("multiply", make_intrusive<ASTLiteral>(*literal),
    ...
    const auto new_ast = makeASTFunction(func_plus_minus->name,
                                         makeASTOperator("multiply", make_intrusive<ASTLiteral>(*literal),
                                                         makeASTFunction("count", make_intrusive<ASTIdentifier>(column_name))),
                                         makeASTFunction("sum", make_intrusive<ASTIdentifier>(column_name)));
    if (!new_ast) return;
