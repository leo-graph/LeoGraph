...
static QueryDescriptors extractQueriesExceptMeAndCheckAccess(const Block &processes_block, ContextPtr context) {
  ...
  auto is_kill_query_granted = [&]() -> bool {
    if (!is_kill_query_granted_value) {
      ...
      if (!*is_kill_query_granted_value) access_denied = true;
    }
  ...
  for (size_t i = 0; i < num_processes; ++i) {
    if ((my_client.current_query_id == query_id_col.getDataAt(i)) && (my_client.current_user == user_col.getDataAt(i))) continue;

    std::string query_id{query_id_col.getDataAt(i)};
    query_user = user_col.getDataAt(i);

    if ((my_client.current_user != query_user) && !is_kill_query_granted()) continue;
 ...
 public:
  ...
  Chunk generate() override {
    ...
    do {
      ...
      if (num_processed_queries < num_result_queries) std::this_thread::sleep_for(std::chrono::milliseconds(100));
...
BlockIO InterpreterKillQueryQuery::execute() {
  ...
  switch (query.type) {
    case ASTKillQueryQuery::Type::Query: {
      ...
      if (processes_block.empty()) return res_io;
      ...
      if (!query.sync || query.test) {
        ...
        for (const auto &query_desc : queries_to_stop) {
          if (!query.test) LOG_DEBUG(getLogger("KillQuery"), "Will kill query {} (asynchronously)", query_desc.query_id);
        ...
        res_io.pipeline = QueryPipeline(
            std::make_shared<SourceFromSingleChunk>(std::make_shared<const Block>(header.cloneWithColumns(std::move(res_columns)))));
      } else {
    ...
    case ASTKillQueryQuery::Type::Mutation: {
      ...
      if (mutations_block.empty()) return res_io;
      ...
      for (size_t i = 0; i < mutations_block.rows(); ++i) {
        ...
        if (!query.test) {
          ...
          if (!storage)
            ...
            auto command_ast = parseQuery(parser, alter_command, 0, getContext()->getSettingsRef()[Setting::max_parser_depth],
                                          getContext()->getSettingsRef()[Setting::max_parser_backtracks]);
            required_access_rights = InterpreterAlterQuery::getRequiredAccessForCommand(command_ast->as<const ASTAlterCommand &>(),
      ...
      if (res_columns[0]->empty() && access_denied)
        throw Exception(ErrorCodes::ACCESS_DENIED,
                        ...
                        required_access_rights.toString());

      res_io.pipeline = QueryPipeline(
          Pipe(std::make_shared<SourceFromSingleChunk>(std::make_shared<const Block>(header.cloneWithColumns(std::move(res_columns))))));

      break;
    }
    case ASTKillQueryQuery::Type::PartMoveToShard: {
      if (query.sync) throw Exception(ErrorCodes::NOT_IMPLEMENTED, "SYNC modifier is not supported for this statement.");

      Block moves_block =
          getSelectResult("database, table, task_name, task_uuid, part_name, to_shard, state", "system.part_moves_between_shards");

      if (moves_block.empty()) return res_io;
      ...
      for (size_t i = 0; i < moves_block.rows(); ++i) {
        ...
        if (!query.test) {
          ...
          if (!storage)
            ...
            required_access_rights =
                InterpreterAlterQuery::getRequiredAccessForCommand(alter_command, table_id.database_name, table_id.table_name);
            if (!access->isGranted(required_access_rights)) {
              access_denied = true;
              continue;
            }
      ...
      if (res_columns[0]->empty() && access_denied)
        throw Exception(ErrorCodes::ACCESS_DENIED,
                        "Not allowed to kill move partition. "
                        "To execute this query, it's necessary to have the grant {}",
                        required_access_rights.toString());
    ...
    case ASTKillQueryQuery::Type::Transaction: {
      ...
      if (transactions_block.empty()) return res_io;
...
Block InterpreterKillQueryQuery::getSelectResult(const String &columns, const String &table) {
  ...
  if (where_expression) select_query += " WHERE " + where_expression->formatWithSecretsOneLine();
  ...
  if (!tmp_block.empty()) throw Exception(ErrorCodes::LOGICAL_ERROR, "Expected one block from input stream");
...
AccessRightsElements InterpreterKillQueryQuery::getRequiredAccessForDDLOnCluster() const {
  ...
  if (query.type == ASTKillQueryQuery::Type::Query)
    ...
    required_access.emplace_back(AccessType::ALTER_UPDATE | AccessType::ALTER_DELETE | AccessType::ALTER_MATERIALIZE_INDEX |
                                 AccessType::ALTER_MATERIALIZE_COLUMN | AccessType::ALTER_MATERIALIZE_TTL |
