...
Type castAs(const Field& field, std::string_view argument_name) {
  ...
  if (expected_type != field.getType()) {
    throw Exception(ErrorCodes::BAD_ARGUMENTS, "Tokenizer argument '{}' expected to be of type {}, but got type: {}", argument_name,
                    fieldTypeToString(expected_type), field.getTypeName());
  }
...
void assertParamsCount(size_t params_count, size_t max_count, std::string_view tokenizer) {
  if (params_count > max_count) {
    throw Exception(ErrorCodes::BAD_ARGUMENTS, "'{}' tokenizer accepts at most {} parameters, but got {}", tokenizer, max_count,
                    params_count);
  }
}
...
std::unordered_map<String, ITokenizer::Type> TokenizerFactory::getAllTokenizers() const {
  ...
  for (const auto& tokenizer : tokenizers) result[tokenizer.first] = tokenizer.second.type;
  return result;
}
...
TokenizerFactory::TokenizerFactory() { registerTokenizers(*this); }
...
std::unique_ptr<ITokenizer> TokenizerFactory::get(std::string_view full_name) const {
  auto try_parse = [&](auto& parser) -> ASTPtr {
    ...
    return tryParseQuery(parser, start, start + full_name.size(), out_err,
                         /*hilite=*/false, "tokenizer",
                         /*allow_multi_statements=*/false, DBMS_DEFAULT_MAX_QUERY_SIZE, DBMS_DEFAULT_MAX_PARSER_DEPTH,
                         DBMS_DEFAULT_MAX_PARSER_BACKTRACKS,
                         /*skip_insignificant=*/true);
  };

  ParserFunction parser_function;
  if (auto ast = try_parse(parser_function)) return get(ast);

  ParserIdentifier parser_identifier;
  if (auto ast = try_parse(parser_identifier)) return get(ast);
...
std::unique_ptr<ITokenizer> TokenizerFactory::get(std::string_view name, const FieldVector& args,
                                                  const std::set<ITokenizer::Type>& allowed) const {
  auto it = tokenizers.find(String(name));
  if (it == tokenizers.end()) throw Exception(ErrorCodes::BAD_ARGUMENTS, "Unknown tokenizer: '{}'", name);
...
static void registerTokenizers(TokenizerFactory& factory) {
  auto ngrams_creator = [](const FieldVector& args) -> std::unique_ptr<ITokenizer> {
    ...
    if (ngram_size < MIN_NGRAM_SIZE)
      throw Exception(ErrorCodes::BAD_ARGUMENTS, "Incorrect parameter of tokenizer '{}': ngram length must be at least {}, but got {}",
                      NgramsTokenizer::getExternalName(), MIN_NGRAM_SIZE, ngram_size);
  ...
  auto split_by_string_creator = [](const FieldVector& args) -> std::unique_ptr<ITokenizer> {
    ...
    if (args.empty()) return std::make_unique<SplitByStringTokenizer>(std::vector<String>{" "});

    auto array = castAs<Array>(args[0], "separators");
    std::vector<String> values;
    for (const auto& value : array) values.emplace_back(castAs<String>(value, "separator"));

    if (values.empty())
      throw Exception(ErrorCodes::BAD_ARGUMENTS, "Incorrect parameter of tokenizer '{}': separators cannot be empty",
  ...
  auto sparse_grams_creator = [](const FieldVector& args) -> std::unique_ptr<ITokenizer> {
    ...
    if (!args.empty()) min_length = castAs<UInt64>(args[0], "min_length");

    if (args.size() > 1) max_length = castAs<UInt64>(args[1], "max_length");

    if (args.size() > 2) min_cutoff_length = castAs<UInt64>(args[2], "min_cutoff_length");

    if (min_length < DEFAULT_SPARSE_GRAMS_MIN_LENGTH)
      throw Exception(ErrorCodes::BAD_ARGUMENTS, "Unexpected parameter of tokenizer '{}': minimal length must be at least {}, but got {}",
                      tokenizer_name, DEFAULT_SPARSE_GRAMS_MIN_LENGTH, min_length);

    if (max_length > DEFAULT_SPARSE_GRAMS_MAX_LENGTH)
      throw Exception(ErrorCodes::BAD_ARGUMENTS, "Unexpected parameter of tokenizer '{}': maximal length must be at most {}, but got {}",
                      tokenizer_name, DEFAULT_SPARSE_GRAMS_MAX_LENGTH, max_length);

    if (min_length > max_length)
      throw Exception(ErrorCodes::BAD_ARGUMENTS,
                      "Unexpected parameter of tokenizer '{}': minimal length {} cannot be larger than maximal length {}", tokenizer_name,
