...
class PlainSynonymsExtension : public ISynonymsExtension {
 ...
 public:
  explicit PlainSynonymsExtension(const String &path) {
    ...
    if (!file.is_open()) throw Exception(ErrorCodes::INVALID_CONFIG_PARAMETER, "Cannot find synonyms extension at: {}", path);
    ...
    while (std::getline(file, line)) {
      ...
      if (!synset.empty()) {
        ...
        for (const auto &word : synsets.back()) table[word] = &synsets.back();
      }
    }
  }
...
class WordnetSynonymsExtension : public ISynonymsExtension {
 ...
 public:
  ...
  const Synset *getSynonyms(std::string_view token) const override { return wn.get_synset(std::string(token)); }
};
...
SynonymsExtensions::SynonymsExtensions(const Poco::Util::AbstractConfiguration &config) {
  ...
  if (!config.has(prefix))
    throw Exception(ErrorCodes::INVALID_CONFIG_PARAMETER, "No synonims extensions specified in server config on prefix '{}'", prefix);
  ...
  for (const auto &key : keys) {
    if (startsWith(key, "extension")) {
      ...
      if (ext_name.empty())
        throw Exception(ErrorCodes::INVALID_CONFIG_PARAMETER, "Extension name in config is not specified here: {}.{}.name", prefix, key);
      if (ext_path.empty())
        throw Exception(ErrorCodes::INVALID_CONFIG_PARAMETER, "Extension path in config is not specified here: {}.{}.path", prefix, key);
      if (ext_type.empty())
        throw Exception(ErrorCodes::INVALID_CONFIG_PARAMETER, "Extension type in config is not specified here: {}.{}.type", prefix, key);
      if (ext_type != "plain" && ext_type != "wordnet")
        throw Exception(ErrorCodes::INVALID_CONFIG_PARAMETER,
                        "Unknown extension type in config: "
                        "{}.{}.type, must be 'plain' or 'wordnet'",
                        prefix, key);

      info[ext_name].path = ext_path;
      info[ext_name].type = ext_type;
    } else
      throw Exception(ErrorCodes::UNKNOWN_ELEMENT_IN_CONFIG, "Unknown element in config: {}.{}, must be 'extension'", prefix, key);
  }
}
...
SynonymsExtensions::ExtPtr SynonymsExtensions::getExtension(const String &name) {
  ...
  if (extensions.contains(name)) return extensions[name];
