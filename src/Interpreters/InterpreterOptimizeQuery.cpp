...
BlockIO InterpreterOptimizeQuery::execute() {
  ...
  if (ast.deduplicate_by_columns) {
    // User requested custom set of columns for deduplication, possibly with Column Transformer expression.
    {
      ...
      for (const auto &col : cols->children) column_names.emplace_back(col->getColumnName());
    }
  ...
  if (ast.dry_run) {
    ...
    if (!merge_tree_data) throw Exception(ErrorCodes::BAD_ARGUMENTS, "OPTIMIZE DRY RUN is only supported for MergeTree family tables");

    if (ast.final) throw Exception(ErrorCodes::BAD_ARGUMENTS, "OPTIMIZE DRY RUN is incompatible with FINAL");

    if (ast.partition) throw Exception(ErrorCodes::BAD_ARGUMENTS, "OPTIMIZE DRY RUN is incompatible with PARTITION");

    if (!ast.parts_list) throw Exception(ErrorCodes::BAD_ARGUMENTS, "OPTIMIZE DRY RUN requires at least one part name");
  ...
  if (auto *snapshot_data = dynamic_cast<MergeTreeData::SnapshotData *>(storage_snapshot->data.get())) snapshot_data->parts = {};
