...
std::string hashToHex(const UInt256 &hash) {
  ...
  for (size_t i = 0; i < sizeof(UInt256); ++i) writeHexByteLowercase(hash_data[i], &hash_hex[2 * i]);
  return hash_hex;
}
...
std::expected<void, PreformattedMessage> checkValidWasmCode(std::string_view name, std::string_view wasm_code) {
  ...
  if (!std::ranges::equal(wasm_magic_number, std::views::take(wasm_code, wasm_magic_number.size()))) {
    return formatUnexpected("Cannot read magic number for WebAssembly module '{}', binary module version 0x1 is expected, got '{}'", name,
...
std::expected<String, PreformattedMessage> validateModuleFile(const DiskPtr &user_scripts_disk, const String &path) {
  ...
  if (n != file_header.size()) return formatUnexpected("File '{}' is too small to be a valid WebAssembly module", path);

  auto module_name = file_name.substr(0, file_name.size() - strlen(FILE_EXTENSION));
  if (auto res = checkValidWasmCode(module_name, file_header); !res) return std::unexpected(std::move(res.error()));
  return module_name;
}

static std::unique_ptr<WebAssembly::IWasmEngine> createEngine(std::string_view engine_name) {
  if (engine_name == "wasmtime") return std::make_unique<WasmTimeRuntime>();
  if (engine_name == "wasmedge") return std::make_unique<WasmEdgeRuntime>();
  throw Exception(ErrorCodes::UNKNOWN_ELEMENT_IN_CONFIG, "Unknown WebAssembly engine '{}', available engines: 'wasmtime', 'wasmedge'",
                  engine_name);
}
...
void WasmModuleManager::saveModule(std::string_view module_name, std::string_view wasm_code, UInt256 expected_hash) {
  if (auto res = checkValidWasmCode(module_name, wasm_code); !res) throw Exception(std::move(res.error()), ErrorCodes::INCORRECT_DATA);

  UInt256 actual_hash = calculateHash(wasm_code);
  if (expected_hash && actual_hash != expected_hash)
    throw Exception(ErrorCodes::INCORRECT_DATA, "Hash mismatch for WebAssembly module '{}', expected {}, got {}", module_name,
                    hashToHex(expected_hash), hashToHex(actual_hash));

  {
    ...
    if (!inserted) {
      ...
      if (!actual_hash) actual_hash = calculateHash(wasm_code);
  ...
  SCOPE_EXIT_SAFE({
    ...
    UniqueLock lock(modules_mutex);
    if (auto it = modules.find(module_name); it != modules.end()) modules.erase(it);
  });
...
void WasmModuleManager::registerExistingModules() {
  UniqueLock lock(modules_mutex);
  LOG_DEBUG(log, "Loading WASM modules from '{}/{}' at disk '{}'", user_scripts_disk->getPath(), user_scripts_path,
...
std::vector<std::pair<std::string, UInt256>> WasmModuleManager::getModulesList() const {
  ...
  for (const auto &[name, module] : modules) result.push_back({name, module.hash});
  return result;
}
